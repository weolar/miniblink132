...
{
    std::vector<std::string_view> numbers = SplitStringPiece(version_str, ".", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    if (numbers.empty())
        return false;
    ...
    for (auto it = numbers.begin(); it != numbers.end(); ++it) {
        ...
        static_assert(sizeof(uint32_t) == sizeof(unsigned int), "uint32_t must be same as unsigned int");
...
int Version::CompareToWildcardString(std::string_view wildcard_string) const
{
    ...
    std::vector<uint32_t> parsed;
    const bool success = ParseVersionNumbers(wildcard_string.substr(0, wildcard_string.length() - 2), &parsed);
    DCHECK(success);
