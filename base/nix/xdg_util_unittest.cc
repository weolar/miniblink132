...
// failure messages.
std::vector<std::string> FilePathsToStrings(const std::vector<base::FilePath>& paths)
{
...
TEST(XDGUtilTest, GetXDGDataWriteLocation)
{
    // Test that it returns $XDG_DATA_HOME.
    {
        ...
        EXPECT_CALL(getter, GetVar(Eq("XDG_DATA_HOME"), _)).WillOnce(DoAll(SetArgPointee<1>("/user/path"), Return(true)));
...
TEST(XDGUtilTest, GetXDGDataSearchLocations)
{
    // Test that it returns $XDG_DATA_HOME + $XDG_DATA_DIRS.
    {
        MockEnvironment getter;
        EXPECT_CALL(getter, GetVar(Eq("XDG_DATA_HOME"), _)).WillOnce(DoAll(SetArgPointee<1>("/user/path"), Return(true)));
        EXPECT_CALL(getter, GetVar(Eq("XDG_DATA_DIRS"), _)).WillOnce(DoAll(SetArgPointee<1>("/system/path/1:/system/path/2"), Return(true)));
        ScopedPathOverride home_override(DIR_HOME, FilePath("/home/user"),
            /*is_absolute=*/true, /*create=*/false);
        EXPECT_THAT(FilePathsToStrings(GetXDGDataSearchLocations(&getter)), testing::ElementsAre("/user/path", "/system/path/1", "/system/path/2"));
    }
    ...
    // Test that $XDG_DATA_HOME falls back to $HOME/.local/share.
    {
        ...
        ScopedPathOverride home_override(DIR_HOME, FilePath("/home/user"),
            /*is_absolute=*/true, /*create=*/false);
        EXPECT_THAT(
            FilePathsToStrings(GetXDGDataSearchLocations(&getter)), testing::ElementsAre("/home/user/.local/share", "/system/path/1", "/system/path/2"));
    }
    ...
    // succeeds.
    {
        ...
        EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
        EXPECT_CALL(getter, GetVar(Eq("XDG_DATA_DIRS"), _)).WillOnce(DoAll(SetArgPointee<1>("/system/path/1:/system/path/2"), Return(true)));
        std::vector<std::string> results = FilePathsToStrings(GetXDGDataSearchLocations(&getter));
    ...
    // Test that $XDG_DATA_DIRS falls back to the two default paths.
    {
        ...
        ScopedPathOverride home_override(DIR_HOME, FilePath("/home/user"),
            /*is_absolute=*/true, /*create=*/false);
        EXPECT_THAT(FilePathsToStrings(GetXDGDataSearchLocations(&getter)), testing::ElementsAre("/user/path", "/usr/local/share", "/usr/share"));
    }
}
...
TEST(XDGUtilTest, GetDesktopEnvironmentGnome)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopGnome), Return(true)));
...
TEST(XDGUtilTest, GetDesktopEnvironmentMATE)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopMATE), Return(true)));
...
TEST(XDGUtilTest, GetDesktopEnvironmentKDE4)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopKDE4), Return(true)));
...
TEST(XDGUtilTest, GetDesktopEnvironmentKDE3)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopKDE), Return(true)));
...
TEST(XDGUtilTest, GetDesktopEnvironmentXFCE)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopXFCE), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopCinnamon)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopCinnamon), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopDeepin)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopDeepin), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopGnome)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopGNOME), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopGnomeClassic)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopGNOMEClassic), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopGnomeFallback)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopUnity), Return(true)));
    EXPECT_CALL(getter, GetVar(Eq(kDesktopSession), _)).WillOnce(DoAll(SetArgPointee<1>(kDesktopGnomeFallback), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopKDE5)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopKDE), Return(true)));
    EXPECT_CALL(getter, GetVar(Eq(kKDESession), _)).WillOnce(DoAll(SetArgPointee<1>(kKDESessionKDE5), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopKDE6)
{
    ...
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopKDE), Return(true)));
    EXPECT_CALL(getter, GetVar(Eq(kKDESession), _)).WillOnce(DoAll(SetArgPointee<1>(kKDESessionKDE6), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopPantheon)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopPantheon), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopUKUI)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopUKUI), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopUnity7)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopUnity7), Return(true)));
...
TEST(XDGUtilTest, GetXdgDesktopUnity8)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgCurrentDesktopEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgDesktopUnity8), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeOther)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionUnknown), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeUnspecified)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionUnspecified), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeTty)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionTty), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeMir)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionMir), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeX11)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionX11), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeWayland)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionWayland), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeWaylandCapital)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionWaylandCapital), Return(true)));
...
TEST(XDGUtilTest, GetXdgSessionTypeWaylandWhitespace)
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(getter, GetVar(Eq(kXdgSessionTypeEnvVar), _)).WillOnce(DoAll(SetArgPointee<1>(kSessionWaylandWhitespace), Return(true)));
...
{
    MockEnvironment getter;
    EXPECT_CALL(getter, GetVar(Eq("XDG_ACTIVATION_TOKEN"), _)).WillOnce(DoAll(SetArgPointee<1>(kXdgActivationTokenFromEnv), Return(true)));
    EXPECT_CALL(getter, UnSetVar(Eq("XDG_ACTIVATION_TOKEN")));
    EXPECT_EQ(kXdgActivationTokenFromEnv, ExtractXdgActivationTokenFromEnv(getter));
...
TEST(XDGUtilTest, ExtractXdgActivationTokenFromCmdLine)
{
    ...
    command_line.AppendSwitchASCII(kXdgActivationTokenSwitch, kXdgActivationTokenFromCmdLine);
    ExtractXdgActivationTokenFromCmdLine(command_line);
...
TEST(XDGUtilTest, ScopedXdgActivationTokenInjector)
{
    ...
    EXPECT_CALL(getter, UnSetVar(Eq("XDG_ACTIVATION_TOKEN")));
    {
        ...
        EXPECT_EQ(kXdgActivationTokenFromEnv, cmd_line.GetSwitchValueASCII(kXdgActivationTokenSwitch));
    }
    for (const auto& pair : initial_switches) {
        EXPECT_TRUE(cmd_line.HasSwitch(pair.first));
    }
...
TEST(XDGUtilTest, LaunchOptionsWithXdgActivation)
{
    ...
    CreateLaunchOptionsWithXdgActivation(base::BindLambdaForTesting([&received_empty_launch_options](LaunchOptions options) {
        EXPECT_TRUE(options.environment.empty());
        received_empty_launch_options = true;
    }));
    EXPECT_TRUE(received_empty_launch_options);

    absl::Cleanup reset_token_creator = [] { SetXdgActivationTokenCreator(XdgActivationTokenCreator()); };
    SetXdgActivationTokenCreator(base::BindRepeating([](XdgActivationTokenCallback callback) { std::move(callback).Run(kXdgActivationTokenFromEnv); }));

    bool received_launch_options_with_test_token = false;
    CreateLaunchOptionsWithXdgActivation(base::BindLambdaForTesting([&received_launch_options_with_test_token](LaunchOptions options) {
        EXPECT_EQ(options.environment["XDG_ACTIVATION_TOKEN"], kXdgActivationTokenFromEnv);
