...
FilePath GetXDGDirectory(Environment* env, const char* env_name, const char* fallback_dir)
{
    FilePath path;
...
DesktopEnvironment GetDesktopEnvironment(Environment* env)
{
    ...
    if (env->GetVar(kXdgCurrentDesktopEnvVar, &xdg_current_desktop)) {
        ...
        for (const auto& value : SplitStringPiece(xdg_current_desktop, ":", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
            if (value == "Unity") {
...
SessionType GetSessionType(Environment& env)
{
    ...
    TrimWhitespaceASCII(ToLowerASCII(xdg_session_type), TrimPositions::TRIM_ALL, &xdg_session_type);
...
void CreateLaunchOptionsWithXdgActivation(XdgActivationLaunchOptionsCallback callback)
{
    ...
    auto create_token_cb = [](XdgActivationLaunchOptionsCallback launch_options_cb, std::string token) {
    ...
    GetXdgActivationTokenCreator().Run(base::BindOnce(create_token_cb, std::move(callback)));
}

std::string XdgDesktopPortalRequestPath(const std::string& sender, const std::string& token)
{
    ...
    static constexpr char kObjectPathRequestFormat[] = "/org/freedesktop/portal/desktop/request/$1/$2";

    auto sender_name = !sender.empty() && sender[0] == ':' ? sender.substr(1) : sender;
    std::string bus_name;
    base::ReplaceChars(sender_name, ".", "_", &bus_name);
    return ReplaceStringPlaceholders(kObjectPathRequestFormat, std::vector<std::string> { bus_name, token }, nullptr);
}
