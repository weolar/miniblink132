...
using XdgActivationTokenCreator = base::RepeatingCallback<void(XdgActivationTokenCallback callback)>;
using XdgActivationLaunchOptionsCallback = base::OnceCallback<void(LaunchOptions)>;
...
BASE_EXPORT FilePath GetXDGDirectory(Environment* env, const char* env_name, const char* fallback_dir);
...
BASE_EXPORT FilePath GetXDGUserDirectory(const char* dir_name, const char* fallback_dir);
...
// child processes.
BASE_EXPORT std::optional<std::string> ExtractXdgActivationTokenFromEnv(Environment& env);

// Sets the global activation token from the command line if it exists and
// removes it from the command line.
BASE_EXPORT void ExtractXdgActivationTokenFromCmdLine(base::CommandLine& cmd_line);
...
BASE_EXPORT void SetXdgActivationTokenCreator(XdgActivationTokenCreator token_creator);
...
BASE_EXPORT void CreateLaunchOptionsWithXdgActivation(XdgActivationLaunchOptionsCallback callback);
