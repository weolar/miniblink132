...
PerfettoPlatform::PerfettoPlatform(PerfettoTaskRunner* task_runner)
    ...
    , thread_local_object_([](void* object) { delete static_cast<ThreadLocalObject*>(object); })
{
}

PerfettoPlatform::~PerfettoPlatform() = default;

PerfettoPlatform::ThreadLocalObject* PerfettoPlatform::GetOrCreateThreadLocalObject()
{
...
std::unique_ptr<perfetto::base::TaskRunner> PerfettoPlatform::CreateTaskRunner(const CreateTaskRunnerArgs&)
{
    // TODO(b/242965112): Add support for the builtin task runner
    return std::make_unique<PerfettoTaskRunner>(task_runner_->GetOrCreateTaskRunner());
}
...
std::string PerfettoPlatform::GetCurrentProcessName()
{
    ...
    if (host_package_name) {
        process_name = StrCat({ kProcessNamePrefix, host_package_name, "-", NumberToString(trace_event::TraceLog::GetInstance()->process_id()) });
    } else {
        process_name = StrCat({ kProcessNamePrefix, NumberToString(trace_event::TraceLog::GetInstance()->process_id()) });
    }
    return process_name;
}
