...
public:
    TaskDestination(size_t number_of_sequences, size_t expected_tasks, base::OnceClosure on_complete)
...
public:
    PosterThread(PerfettoTaskRunner* task_runner, base::WeakPtr<TaskDestination> weak_ptr, int n, size_t sequence_number)
    ...
    void Run() override
    {
        for (int i = 0; i < n_; ++i) {
            ...
            task_runner_->PostTask([weak_ptr, i, sequence_number] { weak_ptr->TestTask(i, sequence_number); });
        }
    }
...
public:
    ...
    {
        return base::ThreadPool::CreateSingleThreadTaskRunner({ base::MayBlock() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED);
    }
    void SetTaskExpectations(base::OnceClosure on_complete, size_t expected_tasks, size_t number_of_sequences = 1)
    {
        task_destination_ = std::make_unique<TaskDestination>(number_of_sequences, expected_tasks, std::move(on_complete));
    }
...
TEST_F(PerfettoTaskRunnerTest, FileDescriptorReuse)
{
    ...
    task_runner()->GetOrCreateTaskRunner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
