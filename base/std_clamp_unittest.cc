...
TEST(ClampTest, Death)
{
    ...
    auto compare_another_type = [](const auto& lhs, const auto& rhs) { return lhs.some_other_int < rhs.some_other_int; };

    EXPECT_DEATH_IF_SUPPORTED(std::ignore = std::clamp(one_type_3, one_type_10, one_type_0), "");
    EXPECT_DEATH_IF_SUPPORTED(std::ignore = std::clamp(another_type_3, another_type_10, another_type_0, compare_another_type), "");
}
