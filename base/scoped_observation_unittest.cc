...
public:
    void AddFoo(TestSourceObserver* observer)
    {
...
using TestScopedObservationWithNonDefaultNames = ScopedObservation<TestSourceWithNonDefaultNames, TestSourceObserver>;

} // namespace

template <> struct ScopedObservationTraits<TestSourceWithNonDefaultNames, TestSourceObserver> {
    static void AddObserver(TestSourceWithNonDefaultNames* source, TestSourceObserver* observer)
    {
        source->AddFoo(observer);
    }
    static void RemoveObserver(TestSourceWithNonDefaultNames* source, TestSourceObserver* observer)
    {
...
public:
    ...
    // defined.
    explicit ObservationWithNonDefaultNamesHolder(TestSourceWithNonDefaultNamesFwd* source);

private:
    // ScopedObservation<> is instantiated with a forward-declared parameter.
    ScopedObservation<TestSourceWithNonDefaultNamesFwd, TestSourceObserver> obs_ { this };
};
...
template <> struct ScopedObservationTraits<TestSourceWithNonDefaultNamesFwd, TestSourceObserver> {
    static void AddObserver(TestSourceWithNonDefaultNamesFwd* source, TestSourceObserver* observer)
    {
        source->AddFoo(observer);
    }
    static void RemoveObserver(TestSourceWithNonDefaultNamesFwd* source, TestSourceObserver* observer)
    {
        source->RemoveFoo(observer);
    }
};
...
// a definition.
ObservationWithNonDefaultNamesHolder::ObservationWithNonDefaultNamesHolder(TestSourceWithNonDefaultNamesFwd* source)
{
    obs_.Observe(source);
}
