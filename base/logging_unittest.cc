...
public:
    MOCK_METHOD4(HandleLogAssert, void(const char*, int, const std::string_view, const std::string_view));
};
...
TEST_F(LoggingTest, BasicLogging)
{
    ...
    EXPECT_CALL(mock_log_source, Log()).Times(expected_logs).WillRepeatedly(Return("log message"));
...
namespace {
void TestForLogToStderr(int log_destinations, bool* did_log_info, bool* did_log_error)
{
    ...
    base::File stderr_logs = base::File(stderr_logs_path, base::File::FLAG_CREATE | base::File::FLAG_WRITE | base::File::FLAG_READ);
...
TEST_F(LoggingTest, CheckCausesDistinctBreakpoints)
{
    ...
    } __except (GetCheckExceptionData(GetExceptionInformation(), &code1, &addr1)) {
    }

    __try {
        CheckContainingFunc(2);
    } __except (GetCheckExceptionData(GetExceptionInformation(), &code2, &addr2)) {
    }

    __try {
        CheckContainingFunc(3);
    } __except (GetCheckExceptionData(GetExceptionInformation(), &code3, &addr3)) {
    }
...
void* CrashThread(void* arg)
{
    ...
    zx_status_t status = zx::thread::self()->create_exception_channel(0, &data->channel);
    if (status != ZX_OK) {
...
void SpawnCrashThread(int death_location, uintptr_t* child_crash_addr)
{
    ...
    status = thread_data.event.wait_one(kChannelReadySignal | kCrashThreadErrorSignal, zx::time::infinite(), &signals);
    ASSERT_EQ(status, ZX_OK);
    ASSERT_EQ(signals, kChannelReadySignal);

    // Wait for the exception and read it out of the channel.
    status = thread_data.channel.wait_one(ZX_CHANNEL_READABLE | ZX_CHANNEL_PEER_CLOSED, zx::time::infinite(), &signals);
    ASSERT_EQ(status, ZX_OK);
    ...
    status = thread_data.channel.read(0, &exception_info, exception.reset_and_get_address(), sizeof(exception_info), 1, nullptr, nullptr);
    ASSERT_EQ(status, ZX_OK);
    ...
    status = zircon_thread.read_state(ZX_THREAD_STATE_GENERAL_REGS, &buffer, sizeof(buffer));
    ASSERT_EQ(status, ZX_OK);
...
#endif
    ASSERT_EQ(zircon_thread.write_state(ZX_THREAD_STATE_GENERAL_REGS, &buffer, sizeof(buffer)), ZX_OK);
...
#elif BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_IOS) && (defined(ARCH_CPU_X86_FAMILY) || defined(ARCH_CPU_ARM_FAMILY))
...
TEST_F(LoggingTest, NestedLogAssertHandlers)
{
    ...
    EXPECT_CALL(handler_a, HandleLogAssert(_, _, std::string_view("First assert must be caught by handler_a"), _));
    EXPECT_CALL(handler_b, HandleLogAssert(_, _, std::string_view("Second assert must be caught by handler_b"), _));
    EXPECT_CALL(handler_a, HandleLogAssert(_, _, std::string_view("Last assert must be caught by handler_a again"), _));

    logging::ScopedLogAssertHandler scoped_handler_a(base::BindRepeating(&MockLogAssertHandler::HandleLogAssert, base::Unretained(&handler_a)));
    ...
    {
        logging::ScopedLogAssertHandler scoped_handler_b(base::BindRepeating(&MockLogAssertHandler::HandleLogAssert, base::Unretained(&handler_b)));
...
class Streamable { };
[[maybe_unused]] std::ostream& operator<<(std::ostream& out, const Streamable&)
{
...
TEST_F(LoggingTest, LogPrefix)
{
    ...
    SetLogMessageHandler([](int severity, const char* file, int line, size_t start, const std::string& str) -> bool {
        *log_string = str;
        return true;
    });
...
TEST_F(LoggingTest, LogCrosSyslogFormat)
{
...

    {
        ...
        const char* kExpected = R"(\S+ \d+ ERROR \S+\[\d+:\d+\]\: \[\S+\] message\n)";
...
TEST_F(LoggingTest, String16)
{
    // Basic stream test.
    {
        ...
        stream << "Empty '" << std::u16string() << "' standard '" << std::u16string(u"Hello, world") << "'";
    ...
    // Interesting edge cases.
    {
        ...
        std::ostringstream stream;
        stream << initial_surrogate << "," << final_surrogate << "," << surrogate_pair << "," << unterminated_surrogate;

        EXPECT_STREQ("\xef\xbf\xbd,\xef\xbf\xbd,\xf0\x90\x8c\x80z,\xef\xbf\xbds", stream.str().c_str());
    }
}
...
{
    EXPECT_EQ("file.cc:42: ", LogMessage("file.cc", 42, LOGGING_ERROR).BuildCrashString());
...
TEST_F(LoggingTest, SystemErrorNotChanged)
{
    ...
    LOG(WARNING) << (during_streaming = GetLastSystemErrorCode()) << (set_last_error(SystemErrorCode(42)), set_during_streaming = GetLastSystemErrorCode());
    ...
    set_last_error(SystemErrorCode(123));
    PLOG(ERROR) << (during_streaming = GetLastSystemErrorCode()) << (set_last_error(SystemErrorCode(42)), set_during_streaming = GetLastSystemErrorCode());
...
TEST_F(LoggingTest, CorrectSystemErrorUsed)
{
    ...
    const std::string kExpectedSystemErrorMsg = SystemErrorCodeToString(kTestError);
