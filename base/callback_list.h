...
template <typename Signature> struct CallbackListTraits<OnceCallbackList<Signature>> {
    using CallbackType = OnceCallback<Signature>;
    using Callbacks = std::list<CallbackType>;
};
template <typename Signature> struct CallbackListTraits<RepeatingCallbackList<Signature>> {
...
public:
    ...
    [[nodiscard]] CallbackListSubscription Add(CallbackType cb)
    {
        ...
        return CallbackListSubscription(
            base::BindOnce(&CallbackListBase::CancelCallback, weak_ptr_factory_.GetWeakPtr(), callbacks_.insert(callbacks_.end(), std::move(cb))));
    }
    ...
    template <typename... RunArgs> void Notify(RunArgs&&... args)
    {
...

        {
            ...
            const auto next_valid
                = [this](const auto it) { return std::find_if_not(it, callbacks_.end(), [](const auto& callback) { return callback.is_null(); }); };
            for (auto it = next_valid(callbacks_.begin()); it != callbacks_.end(); it = next_valid(it))
        ...
        const size_t erased_callbacks = std::erase_if(callbacks_, [](const auto& cb) { return cb.is_null(); });
        ...
        if (removal_callback_ && (erased_callbacks || is_instantiation<OnceCallback, CallbackType>)) {
...
template <typename Signature> class OnceCallbackList : public internal::CallbackListBase<OnceCallbackList<Signature>> {
private:
    ...
    template <typename... RunArgs> void RunCallback(typename Traits::Callbacks::iterator it, RunArgs&&... args)
    {
...
template <typename Signature> class RepeatingCallbackList : public internal::CallbackListBase<RepeatingCallbackList<Signature>> {
private:
