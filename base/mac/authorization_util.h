...
BASE_EXPORT ScopedAuthorizationRef GetAuthorizationRightsWithPrompt(AuthorizationRights* rights, CFStringRef prompt, AuthorizationFlags extra_flags);

// Obtains an AuthorizationRef (using |GetAuthorizationRightsWithPrompt|) that
// can be used to run commands as root.
BASE_EXPORT ScopedAuthorizationRef AuthorizationCreateToRunAsRoot(CFStringRef prompt);
...
BASE_EXPORT OSStatus ExecuteWithPrivilegesAndGetPID(
    AuthorizationRef authorization, const char* tool_path, AuthorizationFlags options, const char** arguments, FILE** pipe, pid_t* pid);
...
// to exit.
BASE_EXPORT OSStatus ExecuteWithPrivilegesAndWait(
    AuthorizationRef authorization, const char* tool_path, AuthorizationFlags options, const char** arguments, FILE** pipe, int* exit_status);
