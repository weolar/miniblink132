...
launch_data_t MessageForJob(const std::string& job_label, const char* operation)
{
    ...
    if (!launch_data_dict_insert(message.get(), job_label_launchd.release(), operation)) {
        return NULL;
    }
...
pid_t PIDForJob(const std::string& job_label)
{
    ...
    if (response_type != LAUNCH_DATA_DICTIONARY) {
        if (response_type == LAUNCH_DATA_ERRNO) {
            LOG(ERROR) << "PIDForJob: error " << launch_data_get_errno(response.get());
        } else {
