...
public:
    ...
    bool ValidateProcess(audit_token_t audit_token, base::span<const uint8_t> info_plist_data = {}) const;
    ...
    // application on disk.
    bool ShouldCheckDynamicValidityOnly() const
    {
        return dynamic_validity_only_;
    }
    ...
    struct CSOpsSystemCallProvider {
        ...
        virtual int csops(pid_t pid, unsigned int ops, void* useraddr, size_t usersize) = 0;
    ...
    static void SetCSOpsSystemCallProviderForTesting(CSOpsSystemCallProvider* csops_provider);

private:
    ProcessRequirement(std::vector<std::string> identifiers, std::string team_identifier, ValidationCategory validation_category, bool dynamic_validity_only);
    ...
    static apple::ScopedCFTypeRef<SecRequirementRef> AsSecRequirementForTesting(ForTesting for_testing);
