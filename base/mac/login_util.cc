...
{
    static void* login_framework = dlopen("/System/Library/PrivateFrameworks/login.framework/Versions/A/login", RTLD_LAZY | RTLD_LOCAL);
...
std::optional<bool> IsScreenLockEnabled()
{
    ...
    static auto func = reinterpret_cast<SACScreenLockEnabledType>(dlsym(GetLoginFramework(), "SACScreenLockEnabled"));
    if (!func) {
        return std::nullopt;
    }
    return func();
...
std::optional<OSStatus> SwitchToLoginWindow()
{
    ...
    static auto func = reinterpret_cast<SACSwitchToLoginWindowType>(dlsym(GetLoginFramework(), "SACSwitchToLoginWindow"));
    if (!func) {
        return std::nullopt;
    }
    return func();
