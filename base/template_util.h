...
// template <typename T> struct SupportsOstreamOperator<T, decltype(void(std::declval<std::ostream&>() << std::declval<T>()))> : std::true_type { };
//
// template <typename T, typename = void> struct SupportsToString : std::false_type { };
// template <typename T> struct SupportsToString<T, decltype(void(std::declval<T>().ToString()))> : std::true_type { };
...
template <typename T> struct is_iterator<T, std::void_t<typename std::iterator_traits<T>::iterator_category>> : std::true_type { };
...
template <typename T> struct IsScopedEnumImpl<T, /*std::is_enum<T>::value=*/true> : std::negation<std::is_convertible<T, std::underlying_type_t<T>>> { };
...
template <typename T> struct is_scoped_enum : internal::IsScopedEnumImpl<T> { };
...
template <typename T, bool Cond = false> struct IterValuePointerImpl {
    // The `iterator_traits<T*>::value_type` member is not defined if T is not an
    // object in C++20.
};
template <typename T> struct IterValuePointerImpl<T*, true> {
    using value_type = typename std::iterator_traits<T*>::value_type;
};

template <typename T> struct IterValueImpl<T*> {
    using value_type = typename IterValuePointerImpl<T*, std::is_object_v<T>>::value_type;
};
...
template <typename Iter> using iter_reference_t = decltype(*std::declval<Iter&>());
...
template <typename Func, typename... Iters> using indirect_result_t = std::invoke_result_t<Func, iter_reference_t<Iters>...>;
...
template <typename Iter, typename Proj, typename IndirectResultT = indirect_result_t<Proj, Iter>> struct projected {
