...
template <class ObserverType, bool check_empty = false, bool allow_reentrancy = true, class ObserverStorageType = internal::CheckedObserverAdapter>
class ObserverList {
public:
    ...
    using Unchecked = ObserverList<ObserverType, check_empty, allow_reentrancy, internal::UncheckedObserverAdapter<>>;
    ...
    // remove this alias.
    using UncheckedAndDanglingUntriaged = ObserverList<ObserverType, check_empty, allow_reentrancy, internal::UncheckedObserverAdapter<DanglingUntriaged>>;
    ...
    using UncheckedAndRawPtrExcluded
        = ObserverList<ObserverType, check_empty, allow_reentrancy, internal::UncheckedObserverAdapter<RawPtrTraits::kEmpty, true>>;
    ...
    public:
        ...
        explicit Iter(const ObserverList* list)
            ...
            , index_(0)
            , max_index_(list->policy_ == ObserverListPolicy::ALL ? std::numeric_limits<size_t>::max() : list->observers_.size())
        {
            DCHECK(list);
        ...
        {
            return (is_end() && other.is_end()) || (list_.get() == other.list_.get() && index_ == other.index_);
        }
    ...
    {
        DCHECK(obs);
        const auto it = ranges::find_if(observers_, [obs](const auto& o) { return o.IsEqual(obs); });
    ...
    bool HasObserver(const ObserverType* obs) const
    {
        ...
        if (obs == nullptr)
            return false;
        return ranges::find_if(observers_, [obs](const auto& o) { return o.IsEqual(obs); }) != observers_.end();
    }
...
private:
    ...
    void Compact()
    {
        ...
        std::erase_if(observers_, [](const auto& o) { return o.IsMarkedForRemoval(); });
    }
...
template <class ObserverType, bool check_empty = false> using ReentrantObserverList = ObserverList<ObserverType, check_empty, true>;
