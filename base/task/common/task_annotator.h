...
public:
    ...
    static void OnIPCReceived(const char* interface_name, uint32_t (*method_info)(), bool is_response);
    ...
    void WillQueueTask(perfetto::StaticString trace_event_name, TaskMetadata* pending_task);
    ...
    template <typename... Args> void RunTask(perfetto::StaticString event_name, PendingTask& pending_task, Args&&... args)
    {
        TRACE_EVENT(
            "toplevel", event_name,
...
private:
    ...
    // EventContext.
    static void EmitTaskLocation(perfetto::EventContext& ctx, const PendingTask& task);
    static void MaybeEmitDelayAndPolicy(perfetto::EventContext& ctx, const PendingTask& task);
    ...
    void MaybeEmitIncomingTaskFlow(perfetto::EventContext& ctx, const PendingTask& task) const;

    void MaybeEmitIPCHash(perfetto::EventContext& ctx, const PendingTask& task) const;
...
public:
    ...
    uint32_t GetIpcHash() const
    {
        return ipc_hash_;
    }
    const char* GetIpcInterfaceName() const
    {
...
{
public:
    explicit LongTaskTracker(const TickClock* tick_clock, PendingTask& pending_task, TaskAnnotator* task_annotator, TimeTicks task_start_time);

    LongTaskTracker(const LongTaskTracker&) = delete;

    ~LongTaskTracker();

    void SetIpcDetails(const char* interface_name, uint32_t (*method_info)(), bool is_response);
    ...
    TimeTicks GetTaskStartTime() const
    {
