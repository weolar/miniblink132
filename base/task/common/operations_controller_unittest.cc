...
public:
    ScopedShutdown(OperationsController* controller)
...
TEST(OperationsControllerTest, StartAcceptingOperationsReturnsFalseIfNoRejectedBeginAttempts)
{
    OperationsController controller;
...
TEST(OperationsControllerTest, StartAcceptingOperationsReturnsTrueIfFailedBeginAttempts)
{
    OperationsController controller;
    ScopedShutdown cleanup(&controller);
...
public:
    explicit TestThread(OperationsController* ref_controller, std::atomic<bool>* started, std::atomic<int32_t>* thread_counter)
...
TEST(OperationsControllerTest, BeginsFromMultipleThreads)
{
    ...
    for (int32_t i = 0; i < 10; ++i) {
        ...
        for (int j = 0; j < kNumThreads; ++j) {
            threads.push_back(std::make_unique<TestThread>(&ref_controller, &started, &running_threads));
            threads.back()->Start();
        }
