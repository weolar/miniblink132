...
constinit thread_local TaskAnnotator::ScopedSetIpcHash* current_scoped_ipc_hash = nullptr;

constinit thread_local TaskAnnotator::LongTaskTracker* current_long_task_tracker = nullptr;
...
TaskAnnotator::ScopedSetIpcHash* GetCurrentScopedIpcHash()
{
    ...
    MSAN_UNPOISON(&current_scoped_ipc_hash, sizeof(TaskAnnotator::ScopedSetIpcHash*));
...
TaskAnnotator::LongTaskTracker* GetCurrentLongTaskTracker()
{
    ...
    // thread_local storage for loaded libraries:
    // https://github.com/google/sanitizers/issues/1265
    MSAN_UNPOISON(&current_long_task_tracker, sizeof(TaskAnnotator::LongTaskTracker*));

    return current_long_task_tracker;
}

#if BUILDFLAG(ENABLE_BASE_TRACING)
perfetto::protos::pbzero::ChromeTaskAnnotator::DelayPolicy ToProtoEnum(subtle::DelayPolicy type)
{
...
void TaskAnnotator::OnIPCReceived(const char* interface_name, uint32_t (*method_info)(), bool is_response)
{
...
void TaskAnnotator::WillQueueTask(perfetto::StaticString trace_event_name, TaskMetadata* pending_task)
{
    DCHECK(pending_task);
    TRACE_EVENT_INSTANT("toplevel.flow", trace_event_name, perfetto::Flow::ProcessScoped(GetTaskTraceID(*pending_task)));

    DCHECK(!pending_task->task_backtrace[0]) << "Task backtrace was already set, task posted twice??";
    ...
    std::copy(parent_task->task_backtrace.begin(), parent_task->task_backtrace.end() - 1, pending_task->task_backtrace.begin() + 1);
    pending_task->task_backtrace_overflow = parent_task->task_backtrace_overflow || parent_task->task_backtrace.back() != nullptr;
}

void TaskAnnotator::RunTaskImpl(PendingTask& pending_task)
{
    TRACE_HEAP_PROFILER_API_SCOPED_TASK_EXECUTION(pending_task.posted_from.file_name());
    ...
    task_backtrace[kStackTaskTraceSnapshotSize - 2] = reinterpret_cast<void*>(pending_task.ipc_hash);
...
// EventContext.
void TaskAnnotator::EmitTaskLocation(perfetto::EventContext& ctx, const PendingTask& task)
{
    ctx.event()->set_task_execution()->set_posted_from_iid(base::trace_event::InternedSourceLocation::Get(&ctx, task.posted_from));
}
...
void TaskAnnotator::MaybeEmitIncomingTaskFlow(perfetto::EventContext& ctx, const PendingTask& task) const
{
...
void TaskAnnotator::EmitTaskTimingDetails(perfetto::EventContext& ctx)
{
    ...
    if (tracker) {
        ...
        perfetto::protos::pbzero::CurrentTask* current_task = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_current_task();
        current_task->set_event_offset_from_task_start_time_us(static_cast<uint64_t>((event_start_time - task_start_time).InMicroseconds()));
        current_task->set_task_start_time_us(static_cast<uint64_t>(task_start_time.since_origin().InMicroseconds()));
    }
}

// static
void TaskAnnotator::MaybeEmitDelayAndPolicy(perfetto::EventContext& ctx, const PendingTask& task)
{
    ...
    annotator->set_task_delay_us(static_cast<uint64_t>((task.delayed_run_time - task.queue_time).InMicroseconds()));
    annotator->set_delay_policy(ToProtoEnum(task.delay_policy));
}

void TaskAnnotator::MaybeEmitIPCHash(perfetto::EventContext& ctx, const PendingTask& task) const
{
    static const uint8_t* toplevel_ipc_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("toplevel.ipc"));
...
// Static
uint32_t TaskAnnotator::ScopedSetIpcHash::MD5HashMetricName(std::string_view name)
{
...
TaskAnnotator::LongTaskTracker::LongTaskTracker(
    const TickClock* tick_clock, PendingTask& pending_task, TaskAnnotator* task_annotator, TimeTicks task_start_time)
...
TaskAnnotator::LongTaskTracker::~LongTaskTracker()
{
    ...
    if ((task_end_time_ - task_start_time_) >= kMaxTaskDurationTimeDelta) {
        TRACE_EVENT_BEGIN(
            "scheduler.long_tasks", "LongTaskTracker", perfetto::Track::ThreadScoped(task_annotator_), task_start_time_, [&](perfetto::EventContext& ctx) {
                TaskAnnotator::EmitTaskLocation(ctx, pending_task_);
                EmitReceivedIPCDetails(ctx);
            });
        TRACE_EVENT_END("scheduler.long_tasks", perfetto::Track::ThreadScoped(task_annotator_), task_end_time_);
    }
}

void TaskAnnotator::LongTaskTracker::SetIpcDetails(const char* interface_name, uint32_t (*method_info)(), bool is_response)
{
...
void TaskAnnotator::LongTaskTracker::EmitReceivedIPCDetails(perfetto::EventContext& ctx)
{
    ...
    const std::optional<size_t> location_iid = base::trace_event::InternedUnsymbolizedSourceLocation::Get(&ctx, ipc_method_address);
    if (location_iid) {
...
void TaskAnnotator::LongTaskTracker::MaybeTraceInterestingTaskDetails()
{
    if (is_interesting_task && ipc_interface_name_) {
        ...
        TRACE_EVENT_INSTANT("scheduler.long_tasks", "InterestingTask_QueueingTime", perfetto::Track::ThreadScoped(task_annotator_), pending_task_.queue_time,
            perfetto::Flow::ProcessScoped(task_annotator_->GetTaskTraceID(pending_task_)));
        ...
        TRACE_EVENT_BEGIN("scheduler.long_tasks", "InterestingTask_ProcessingTime", perfetto::Track::ThreadScoped(task_annotator_), task_start_time_,
            [&](perfetto::EventContext& ctx) {
                perfetto::TerminatingFlow::ProcessScoped(task_annotator_->GetTaskTraceID(pending_task_))(ctx);
                auto* info = ctx.event()->set_chrome_mojo_event_info();
                info->set_mojo_interface_tag(ipc_interface_name_);
            });
