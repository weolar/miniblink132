...
public:
    explicit BasicLockAcquireAndWaitThread(CheckedLock* lock)
        ...
        , lock_(lock)
        , lock_acquire_event_(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED)
        , main_thread_continue_event_(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }

    BasicLockAcquireAndWaitThread(const BasicLockAcquireAndWaitThread&) = delete;
    BasicLockAcquireAndWaitThread& operator=(const BasicLockAcquireAndWaitThread&) = delete;
...
TEST(CheckedLockTest, AcquireLocksWithPredecessorDifferentThreadsSafelyPredecessorFirst)
{
...
TEST(CheckedLockTest, AcquireLocksWithPredecessorDifferentThreadsSafelyPredecessorLast)
{
    // A lock and its predecessor may be safely acquired on different threads.
    // This Thread                Other Thread
    // lock.Acquire()
...
TEST(CheckedLockTest, AcquireLocksWithPredecessorDifferentThreadsSafelyNoInterference)
{
