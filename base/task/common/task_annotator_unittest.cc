...
class TaskAnnotatorBacktraceIntegrationTest : public ::testing::Test, public TaskAnnotator::ObserverForTesting {
public:
    using ExpectedTrace = std::vector<const void*>;

    TaskAnnotatorBacktraceIntegrationTest() = default;

    TaskAnnotatorBacktraceIntegrationTest(const TaskAnnotatorBacktraceIntegrationTest&) = delete;
    TaskAnnotatorBacktraceIntegrationTest& operator=(const TaskAnnotatorBacktraceIntegrationTest&) = delete;
    ...
    void SetUp() override
    {
        TaskAnnotator::RegisterObserverForTesting(this);
    }
    ...
    void VerifyTraceAndPost(const scoped_refptr<SequencedTaskRunner>& task_runner, const Location& posted_from, const Location& next_from_here,
        const ExpectedTrace& expected_trace, uint32_t expected_ipc_hash, OnceClosure task)
    {
    ...
    void VerifyTraceAndPostWithIpcContext(const scoped_refptr<SequencedTaskRunner>& task_runner, const Location& posted_from, const Location& next_from_here,
        const ExpectedTrace& expected_trace, uint32_t expected_ipc_hash, OnceClosure task, uint32_t new_ipc_hash)
    {
        TaskAnnotator::ScopedSetIpcHash scoped_ipc_hash(new_ipc_hash);
        VerifyTraceAndPost(task_runner, posted_from, next_from_here, expected_trace, expected_ipc_hash, std::move(task));
    }
    ...
    // signaled.
    void VerifyTraceAndPostWithBlocker(const scoped_refptr<SequencedTaskRunner>& task_runner, const Location& posted_from, const Location& next_from_here,
        const ExpectedTrace& expected_trace, uint32_t expected_ipc_hash, OnceClosure task, WaitableEvent* wait_before_next_task)
    {
        ...
        AutoLock auto_lock(on_before_run_task_lock_);
        task_runner->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Wait, Unretained(wait_before_next_task)));
        VerifyTraceAndPost(task_runner, posted_from, next_from_here, expected_trace, expected_ipc_hash, std::move(task));
    }
...
private:
    ...
    std::array<const void*, PendingTask::kTaskBacktraceLength> last_task_backtrace_ = {};
...
TEST_F(TaskAnnotatorBacktraceIntegrationTest, SingleThreadedSimple)
{
    ...
    OnceClosure task5
        = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(), location5,
            FROM_HERE, ExpectedTrace({ location4.program_counter(), location3.program_counter(), location2.program_counter(), location1.program_counter() }), 0,
            run_loop.QuitClosure());

    // Task i=4/3/2/1/0 have tasks [0,i) as parents.
    OnceClosure task4
        = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(), location4,
            location5, ExpectedTrace({ location3.program_counter(), location2.program_counter(), location1.program_counter(), location0.program_counter() }), 0,
            std::move(task5));
    OnceClosure task3 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(),
        location3, location4, ExpectedTrace({ location2.program_counter(), location1.program_counter(), location0.program_counter() }), 0, std::move(task4));
    OnceClosure task2 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(),
        location2, location3, ExpectedTrace({ location1.program_counter(), location0.program_counter() }), dummy_ipc_hash, std::move(task3));
    OnceClosure task1 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this),
        SingleThreadTaskRunner::GetCurrentDefault(), location1, location2, ExpectedTrace({ location0.program_counter() }), 0, std::move(task2), dummy_ipc_hash);
    OnceClosure task0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(),
        location0, location1, ExpectedTrace({}), 0, std::move(task1));

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(location0, std::move(task0));
...
TEST_F(TaskAnnotatorBacktraceIntegrationTest, MultipleThreads)
{
    ...
    auto task_runner_c = ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::WithBaseSyncPrimitives() });
    ...
    WaitableEvent lock_step(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    ...
    OnceClosure task_a2 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), main_thread_a, location_a2, location_a3,
        ExpectedTrace({ location_c0.program_counter(), location_b0.program_counter(), location_a1.program_counter(), location_a0.program_counter() }),
        dummy_ipc_hash1, run_loop.QuitClosure());
    OnceClosure task_c0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this), main_thread_a, location_c0,
        location_a2, ExpectedTrace({ location_b0.program_counter(), location_a1.program_counter(), location_a0.program_counter() }), 0, std::move(task_a2),
        dummy_ipc_hash1);
    ...
    OnceClosure task_b0_fork = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithBlocker, Unretained(this), task_runner_c, location_b0,
        location_c0, ExpectedTrace({ location_a1.program_counter(), location_a0.program_counter() }), 0, std::move(task_c0), &lock_step);
    OnceClosure task_b0_local = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this), task_runner_b, location_b0,
        location_b1, ExpectedTrace({ location_a1.program_counter(), location_a0.program_counter() }), 0,
        BindOnce(&WaitableEvent::Signal, Unretained(&lock_step)), dummy_ipc_hash0);

    OnceClosure task_a1 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), task_runner_b, location_a1, location_b0,
        ExpectedTrace({ location_a0.program_counter() }), 0,
        BindOnce(&TaskAnnotatorBacktraceIntegrationTest::RunTwo, std::move(task_b0_local), std::move(task_b0_fork)));
    OnceClosure task_a0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), main_thread_a, location_a0, location_a1,
...
TEST_F(TaskAnnotatorBacktraceIntegrationTest, SingleThreadedNested)
{
    ...
    OnceClosure run_task_3_then_quit_nested_loop1 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::RunTwo, std::move(task3), nested_run_loop1.QuitClosure());

    OnceClosure task2 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault(),
        location2, location3, ExpectedTrace({ location1.program_counter(), location0.program_counter() }), 0, std::move(run_task_3_then_quit_nested_loop1));
    ...
    OnceClosure task1 = BindOnce(BindLambdaForTesting([dummy_ipc_hash1](RunLoop* nested_run_loop, const Location& location2, OnceClosure task2) {
        {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(location2, std::move(task2));
    }),
        Unretained(&nested_run_loop2), location2, std::move(task2));

    OnceClosure task0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this),
        SingleThreadTaskRunner::GetCurrentDefault(), location0, location1, ExpectedTrace({}), 0, std::move(task1), dummy_ipc_hash);

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(location0, std::move(task0));

    {
        TaskAnnotator::ScopedSetIpcHash scoped_ipc_hash(dummy_ipc_hash2);
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RunLoop::Run, Unretained(&nested_run_loop1), FROM_HERE));
    }
