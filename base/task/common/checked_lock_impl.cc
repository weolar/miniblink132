...
public:
    ...
    void RegisterLock(const CheckedLockImpl* const lock, const CheckedLockImpl* const predecessor)
    {
...
private:
    ...
    void AssertSafePredecessor(const CheckedLockImpl* lock) const
    {
        ...
        if (predecessor) {
            DCHECK(allowed_predecessor_map_.find(predecessor) != allowed_predecessor_map_.end()) << "CheckedLock was registered before its predecessor. "
...
void CheckedLockImpl::CreateConditionVariableAndEmplace(std::optional<ConditionVariable>& opt)
{
    opt.emplace(&lock_);
}
