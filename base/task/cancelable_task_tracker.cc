...
void RunOrPostToTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner, OnceClosure closure)
{
...
CancelableTaskTracker::TaskId CancelableTaskTracker::PostTask(TaskRunner* task_runner, const Location& from_here, OnceClosure task)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(weak_this_);
...
CancelableTaskTracker::TaskId CancelableTaskTracker::PostTaskAndReply(TaskRunner* task_runner, const Location& from_here, OnceClosure task, OnceClosure reply)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(weak_this_);
    ...
    bool success = task_runner->PostTaskAndReply(from_here, BindOnce(&RunIfNotCanceled, flag, std::move(task)),
        BindOnce(&RunThenUntrackIfNotCanceled, flag, std::move(reply), std::move(untrack_closure)));
...
CancelableTaskTracker::TaskId CancelableTaskTracker::NewTrackedTaskId(IsCanceledCallback* is_canceled_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(SequencedTaskRunner::HasCurrentDefault());
    ...
    ScopedClosureRunner untrack_runner(
        BindOnce(&RunOrPostToTaskRunner, SequencedTaskRunner::GetCurrentDefault(), BindOnce(&RunIfNotCanceled, flag, std::move(untrack_closure))));
...
// static
void CancelableTaskTracker::RunIfNotCanceled(const scoped_refptr<TaskCancellationFlag>& flag, OnceClosure task)
{
    if (!flag->data.IsSet()) {
        std::move(task).Run();
    }
...
// static
void CancelableTaskTracker::RunThenUntrackIfNotCanceled(const scoped_refptr<TaskCancellationFlag>& flag, OnceClosure task, OnceClosure untrack)
{
...
// static
bool CancelableTaskTracker::IsCanceled(const scoped_refptr<TaskCancellationFlag>& flag, const ScopedClosureRunner& cleanup_runner)
{
    return flag->data.IsSet();
}

void CancelableTaskTracker::Track(TaskId id, scoped_refptr<TaskCancellationFlag> flag)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(weak_this_);
