...
DelayedTaskManager::DelayedTask::DelayedTask(Task task, PostTaskNowCallback callback)
    : task(std::move(task))
...
DelayedTaskManager::DelayedTask& DelayedTaskManager::DelayedTask::operator=(DelayedTaskManager::DelayedTask&& other) = default;

bool DelayedTaskManager::DelayedTask::operator>(const DelayedTask& other) const
{
    TimeTicks latest_delayed_run_time = task.latest_delayed_run_time();
    TimeTicks other_latest_delayed_run_time = other.task.latest_delayed_run_time();
    return std::tie(latest_delayed_run_time, task.sequence_num) > std::tie(other_latest_delayed_run_time, other.task.sequence_num);
}

DelayedTaskManager::DelayedTaskManager(const TickClock* tick_clock)
    : process_ripe_tasks_closure_(BindRepeating(&DelayedTaskManager::ProcessRipeTasks, Unretained(this)))
    , schedule_process_ripe_tasks_closure_(BindRepeating(&DelayedTaskManager::ScheduleProcessRipeTasksOnServiceThread, Unretained(this)))
...
void DelayedTaskManager::Start(scoped_refptr<SequencedTaskRunner> service_thread_task_runner)
{
    ...
    subtle::DelayPolicy delay_policy;
    {
        ...
        std::tie(process_ripe_tasks_time, delay_policy) = GetTimeAndDelayPolicyToScheduleProcessRipeTasksLockRequired();
    }
    if (!process_ripe_tasks_time.is_max()) {
        service_thread_task_runner_->PostTask(FROM_HERE, schedule_process_ripe_tasks_closure_);
    }
}

void DelayedTaskManager::AddDelayedTask(Task task, PostTaskNowCallback post_task_now_callback)
{
    DCHECK(task.task);
    ...
    {
        CheckedAutoLock auto_lock(queue_lock_);
        task.delay_policy = subtle::MaybeOverrideDelayPolicy(task.delay_policy, task.delayed_run_time - task.queue_time, max_precise_delay);

        auto [old_process_ripe_tasks_time, old_delay_policy] = GetTimeAndDelayPolicyToScheduleProcessRipeTasksLockRequired();
        delayed_task_queue_.insert(DelayedTask(std::move(task), std::move(post_task_now_callback)));
        ...
        if (old_process_ripe_tasks_time == process_ripe_tasks_time && old_delay_policy == delay_policy) {
            return;
        }
    }
    if (!process_ripe_tasks_time.is_max()) {
...
void DelayedTaskManager::ProcessRipeTasks()
{
...

    {
        ...
        while (!delayed_task_queue_.empty()
            && (delayed_task_queue_.top().task.earliest_delayed_run_time() <= now || !delayed_task_queue_.top().task.task.MaybeValid())) {
            ...
            ripe_delayed_tasks.push_back(std::move(const_cast<DelayedTask&>(delayed_task_queue_.top())));
            delayed_task_queue_.pop();
        }
        std::tie(process_ripe_tasks_time, std::ignore) = GetTimeAndDelayPolicyToScheduleProcessRipeTasksLockRequired();
    }
    if (!process_ripe_tasks_time.is_max()) {
...
void DelayedTaskManager::Shutdown()
{
    ...
    if (service_thread_task_runner) {
        ...
        service_thread_task_runner->PostTask(FROM_HERE,
            base::BindOnce(
...
std::pair<TimeTicks, subtle::DelayPolicy> DelayedTaskManager::GetTimeAndDelayPolicyToScheduleProcessRipeTasksLockRequired()
{
    queue_lock_.AssertAcquired();
    if (delayed_task_queue_.empty()) {
        return std::make_pair(TimeTicks::Max(), subtle::DelayPolicy::kFlexibleNoSooner);
    }
    ...
    return std::make_pair(ripest_delayed_task.task.delayed_run_time, delay_policy);
}
...
void DelayedTaskManager::ScheduleProcessRipeTasksOnServiceThread()
{
    ...
    if (process_ripe_tasks_time.is_max())
        return;
    delayed_task_handle_.CancelTask();
    delayed_task_handle_ = service_thread_task_runner_->PostCancelableDelayedTaskAt(
        subtle::PostDelayedTaskPassKey(), FROM_HERE, process_ripe_tasks_closure_, process_ripe_tasks_time, delay_policy);
}
