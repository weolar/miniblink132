...
public:
    ...
    bool under_destruction() const
    {
        return under_destruction_.IsSet();
    }
...
TEST(TrackedRefTest, TrackedRefObjectDeletion)
{
    ...
    std::unique_ptr<ObjectWithTrackedRefs> obj = std::make_unique<ObjectWithTrackedRefs>();
