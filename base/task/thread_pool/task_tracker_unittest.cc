...
public:
    ...
    ThreadPostingAndRunningTask(TaskTracker* tracker, scoped_refptr<Sequence> sequence, Action action, bool expect_post_succeeds, Task task)
    ...
    ThreadPostingAndRunningTask(TaskTracker* tracker, RegisteredTaskSource task_source)
        : SimpleThread("ThreadPostingAndRunningTask")
        , tracker_(tracker)
    ...
    ThreadPostingAndRunningTask& operator=(const ThreadPostingAndRunningTask&) = delete;

    RegisteredTaskSource TakeTaskSource()
    {
...
private:
    void Run() override
    {
        ...
        if (action_ == Action::WILL_POST || action_ == Action::WILL_POST_AND_RUN) {
            ...
            post_and_queue_succeeded = tracker_->WillPostTask(&task_, sequence_->shutdown_behavior());
            {
        ...
        if (post_and_queue_succeeded && (action_ == Action::RUN || action_ == Action::WILL_POST_AND_RUN)) {
            EXPECT_TRUE(task_source_);
...
class ThreadPoolTaskTrackerTest : public testing::TestWithParam<TaskShutdownBehavior> {
public:
    ThreadPoolTaskTrackerTest(const ThreadPoolTaskTrackerTest&) = delete;
    ThreadPoolTaskTrackerTest& operator=(const ThreadPoolTaskTrackerTest&) = delete;
...
protected:
    ...
    Task CreateTask()
    {
        return Task(FROM_HERE, BindOnce(&ThreadPoolTaskTrackerTest::RunTaskCallback, Unretained(this)), TimeTicks::Now(), TimeDelta());
    }

    RegisteredTaskSource WillPostTaskAndQueueTaskSource(Task task, const TaskTraits& traits)
    {
    ...
    void ExpectAsyncCompleteShutdownBlocks()
    {
        ...
        thread_calling_shutdown_ = std::make_unique<CallbackThread>(BindOnce(&TaskTracker::CompleteShutdown, Unretained(&tracker_)));
    ...
    {
        threads_calling_flush_.push_back(std::make_unique<CallbackThread>(BindOnce(&TaskTracker::FlushForTesting, Unretained(&tracker_))));
...
TEST_P(ThreadPoolTaskTrackerTest, WillPostAndRunBeforeShutdown)
{
    ...
    test::QueueAndRunTaskSource(&tracker_, test::CreateSequenceWithTask(std::move(task), { GetParam() }));
...
TEST_P(ThreadPoolTaskTrackerTest, WillPostAndRunLongTaskBeforeShutdown)
{
    ...
    auto sequence = WillPostTaskAndQueueTaskSource(std::move(blocked_task), { GetParam() });
    EXPECT_TRUE(sequence);

    // Create a thread to run the task. Wait until the task starts running.
    ThreadPostingAndRunningTask thread_running_task(&tracker_, std::move(sequence));
...
TEST_F(ThreadPoolTaskTrackerTest, PostAfterShutdownFromContinueOnShutdown)
{
    ...
    Task poster(FROM_HERE, BindLambdaForTesting([&] {
        ...
        EXPECT_TRUE(tracker_.IsShutdownComplete());
        EXPECT_FALSE(tracker_.WillPostTask(&task, TaskShutdownBehavior::BLOCK_SHUTDOWN));
    }),
        TimeTicks::Now(), TimeDelta());

    // Inform |task_tracker_| that |blocked_task| will be posted.
    auto sequence = WillPostTaskAndQueueTaskSource(std::move(poster), TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN);
    EXPECT_TRUE(sequence);
...
TEST_P(ThreadPoolTaskTrackerTest, WillPostBeforeShutdownQueueDuringShutdown)
{
    ...
    // block shutdown.
    auto block_shutdown_sequence = WillPostTaskAndQueueTaskSource(CreateTask(), { TaskShutdownBehavior::BLOCK_SHUTDOWN });
...
TEST_P(ThreadPoolTaskTrackerTest, WillPostAndRunDuringShutdown)
{
    ...
    RunAndPopNextTask(std::move(block_shutdown_sequence));
    EXPECT_EQ(GetParam() == TaskShutdownBehavior::BLOCK_SHUTDOWN ? 2U : 1U, NumTasksExecuted());
    WAIT_FOR_ASYNC_SHUTDOWN_COMPLETED();
}
...
TEST_P(ThreadPoolTaskTrackerTest, SingletonAllowed)
{
    ...
    Task task(FROM_HERE, BindOnce(&internal::AssertSingletonAllowed), TimeTicks::Now(), TimeDelta());
...
TEST_P(ThreadPoolTaskTrackerTest, IOAllowed)
{
    ...
    Task task_with_may_block(FROM_HERE, BindOnce([] {
        // Shouldn't fail.
        ScopedBlockingCall scope_blocking_call(FROM_HERE, BlockingType::WILL_BLOCK);
    }),
        TimeTicks::Now(), TimeDelta());
    TaskTraits traits_with_may_block { MayBlock(), GetParam() };
    auto sequence_with_may_block = WillPostTaskAndQueueTaskSource(std::move(task_with_may_block), traits_with_may_block);
    ...
    Task task_without_may_block(FROM_HERE,
        BindOnce([] { EXPECT_DCHECK_DEATH({ ScopedBlockingCall scope_blocking_call(FROM_HERE, BlockingType::WILL_BLOCK); }); }), TimeTicks::Now(), TimeDelta());
    TaskTraits traits_without_may_block = TaskTraits(GetParam());
    auto sequence_without_may_block = WillPostTaskAndQueueTaskSource(std::move(task_without_may_block), traits_without_may_block);
...
static void RunTaskRunnerCurrentDefaultHandleVerificationTask(
    TaskTracker* tracker, Task verify_task, TaskTraits traits, scoped_refptr<SequencedTaskRunner> task_runner, TaskSourceExecutionMode execution_mode)
{
    ...
    test::QueueAndRunTaskSource(tracker, test::CreateSequenceWithTask(std::move(verify_task), traits, std::move(task_runner), execution_mode));
...
TEST_P(ThreadPoolTaskTrackerTest, TaskRunnerHandleIsNotSetOnParallel)
{
    ...
    Task verify_task(FROM_HERE, BindOnce(&VerifyNoTaskRunnerCurrentDefaultHandle), TimeTicks::Now(), TimeDelta());

    RunTaskRunnerCurrentDefaultHandleVerificationTask(&tracker_, std::move(verify_task), TaskTraits(GetParam()), nullptr, TaskSourceExecutionMode::kParallel);
}

static void VerifySequencedTaskRunnerCurrentDefaultHandle(const SequencedTaskRunner* expected_task_runner)
{
    EXPECT_FALSE(SingleThreadTaskRunner::HasCurrentDefault());
...
TEST_P(ThreadPoolTaskTrackerTest, SequencedTaskRunnerHasCurrentDefaultOnSequenced)
{
    ...
    // to it.
    Task verify_task(FROM_HERE, BindOnce(&VerifySequencedTaskRunnerCurrentDefaultHandle, Unretained(test_task_runner.get())), TimeTicks::Now(), TimeDelta());

    RunTaskRunnerCurrentDefaultHandleVerificationTask(
        &tracker_, std::move(verify_task), TaskTraits(GetParam()), std::move(test_task_runner), TaskSourceExecutionMode::kSequenced);
}

static void VerifySingleThreadTaskRunnerCurrentDefaultHandle(const SingleThreadTaskRunner* expected_task_runner)
{
...
{
    scoped_refptr<SingleThreadTaskRunner> test_task_runner(new TestSimpleTaskRunner);
    ...
    // set on it.
    Task verify_task(FROM_HERE, BindOnce(&VerifySingleThreadTaskRunnerCurrentDefaultHandle, Unretained(test_task_runner.get())), TimeTicks::Now(), TimeDelta());

    RunTaskRunnerCurrentDefaultHandleVerificationTask(
        &tracker_, std::move(verify_task), TaskTraits(GetParam()), std::move(test_task_runner), TaskSourceExecutionMode::kSingleThread);
}
...
TEST_P(ThreadPoolTaskTrackerTest, FlushAsyncForTestingPendingDelayedTask)
{
    ...
    tracker_.FlushAsyncForTesting(BindOnce([](bool* called_back) { *called_back = true; }, Unretained(&called_back)));
...
TEST_P(ThreadPoolTaskTrackerTest, FlushPendingUndelayedTask)
{
    ...
    auto undelayed_sequence = WillPostTaskAndQueueTaskSource(std::move(undelayed_task), { GetParam() });
...
TEST_P(ThreadPoolTaskTrackerTest, FlushAsyncForTestingPendingUndelayedTask)
{
    ...
    tracker_.FlushAsyncForTesting(BindOnce(&TestWaitableEvent::Signal, Unretained(&event)));
...
TEST_P(ThreadPoolTaskTrackerTest, MultipleFlushAsyncForTesting)
{
    ...
    auto on_flush_done = BarrierClosure(3, BindOnce(&TestWaitableEvent::Signal, Unretained(&three_callbacks_ran)));
...
TEST_P(ThreadPoolTaskTrackerTest, PostTaskDuringFlush)
{
    ...
    Task other_undelayed_task(FROM_HERE, DoNothing(), TimeTicks::Now(), TimeDelta());
    auto other_undelayed_sequence = WillPostTaskAndQueueTaskSource(std::move(other_undelayed_task), { GetParam() });
...
TEST_P(ThreadPoolTaskTrackerTest, RunDelayedTaskDuringFlush)
{
    ...
    Task delayed_task(FROM_HERE, DoNothing(), TimeTicks::Now(), Days(1));
    auto delayed_sequence = WillPostTaskAndQueueTaskSource(std::move(delayed_task), { GetParam() });
    Task undelayed_task(FROM_HERE, DoNothing(), TimeTicks::Now(), TimeDelta());
    auto undelayed_sequence = WillPostTaskAndQueueTaskSource(std::move(undelayed_task), { GetParam() });
...
TEST_P(ThreadPoolTaskTrackerTest, DelayedRunTasks)
{
    ...
    Task delayed_task(FROM_HERE, DoNothing(), TimeTicks::Now(), Days(1));
    auto sequence = WillPostTaskAndQueueTaskSource(std::move(delayed_task), { GetParam() });
    EXPECT_TRUE(sequence);
...
INSTANTIATE_TEST_SUITE_P(ContinueOnShutdown, ThreadPoolTaskTrackerTest, ::testing::Values(TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN));
INSTANTIATE_TEST_SUITE_P(SkipOnShutdown, ThreadPoolTaskTrackerTest, ::testing::Values(TaskShutdownBehavior::SKIP_ON_SHUTDOWN));
INSTANTIATE_TEST_SUITE_P(BlockShutdown, ThreadPoolTaskTrackerTest, ::testing::Values(TaskShutdownBehavior::BLOCK_SHUTDOWN));
...
{
    scoped_refptr<Sequence> sequence = MakeRefCounted<Sequence>(TaskTraits(), nullptr, TaskSourceExecutionMode::kParallel);

    const SequenceToken sequence_token = sequence->token();
    Task task(FROM_HERE, BindOnce(&ExpectSequenceToken, sequence_token), TimeTicks::Now(), TimeDelta());
...
TEST_F(ThreadPoolTaskTrackerTest, LoadWillPostAndRunBeforeShutdown)
{
    ...
    for (size_t i = 0; i < kLoadTestNumIterations; ++i) {
        threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_,
            MakeRefCounted<Sequence>(TaskTraits { TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, nullptr, TaskSourceExecutionMode::kParallel),
            ThreadPostingAndRunningTask::Action::WILL_POST_AND_RUN, true, CreateTask()));
        threads.back()->Start();

        threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_,
            MakeRefCounted<Sequence>(TaskTraits { TaskShutdownBehavior::SKIP_ON_SHUTDOWN }, nullptr, TaskSourceExecutionMode::kParallel),
            ThreadPostingAndRunningTask::Action::WILL_POST_AND_RUN, true, CreateTask()));
        threads.back()->Start();

        threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_,
            MakeRefCounted<Sequence>(TaskTraits { TaskShutdownBehavior::BLOCK_SHUTDOWN }, nullptr, TaskSourceExecutionMode::kParallel),
            ThreadPostingAndRunningTask::Action::WILL_POST_AND_RUN, true, CreateTask()));
        threads.back()->Start();
    }
...
TEST_F(ThreadPoolTaskTrackerTest, LoadWillPostBeforeShutdownAndRunDuringShutdown)
{
    constexpr TaskTraits traits_continue_on_shutdown = TaskTraits(TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN);
    constexpr TaskTraits traits_skip_on_shutdown = TaskTraits(TaskShutdownBehavior::SKIP_ON_SHUTDOWN);
    constexpr TaskTraits traits_block_shutdown = TaskTraits(TaskShutdownBehavior::BLOCK_SHUTDOWN);
    ...
    std::vector<std::unique_ptr<ThreadPostingAndRunningTask>> post_threads;
    {
        ...
        for (size_t i = 0; i < kLoadTestNumIterations; ++i) {
            sequences_continue_on_shutdown.push_back(MakeRefCounted<Sequence>(traits_continue_on_shutdown, nullptr, TaskSourceExecutionMode::kParallel));
            sequences_skip_on_shutdown.push_back(MakeRefCounted<Sequence>(traits_skip_on_shutdown, nullptr, TaskSourceExecutionMode::kParallel));
            sequences_block_shutdown.push_back(MakeRefCounted<Sequence>(traits_block_shutdown, nullptr, TaskSourceExecutionMode::kParallel));
        }

        for (size_t i = 0; i < kLoadTestNumIterations; ++i) {
            post_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(
                &tracker_, sequences_continue_on_shutdown[i], ThreadPostingAndRunningTask::Action::WILL_POST, true, CreateTask()));
            post_threads.back()->Start();

            post_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(
                &tracker_, sequences_skip_on_shutdown[i], ThreadPostingAndRunningTask::Action::WILL_POST, true, CreateTask()));
            post_threads.back()->Start();

            post_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(
                &tracker_, sequences_block_shutdown[i], ThreadPostingAndRunningTask::Action::WILL_POST, true, CreateTask()));
            post_threads.back()->Start();
        }
    }
    ...
    for (size_t i = 0; i < kLoadTestNumIterations; ++i) {
        run_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_, post_threads[i * 3]->TakeTaskSource()));
        run_threads.back()->Start();

        run_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_, post_threads[i * 3 + 1]->TakeTaskSource()));
        run_threads.back()->Start();

        run_threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_, post_threads[i * 3 + 2]->TakeTaskSource()));
        run_threads.back()->Start();
    }
...
TEST_F(ThreadPoolTaskTrackerTest, LoadWillPostAndRunDuringShutdown)
{
    ...
    for (size_t i = 0; i < kLoadTestNumIterations; ++i) {
        threads.push_back(std::make_unique<ThreadPostingAndRunningTask>(&tracker_,
            MakeRefCounted<Sequence>(TaskTraits { TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, nullptr, TaskSourceExecutionMode::kParallel),
            ThreadPostingAndRunningTask::Action::WILL_POST_AND_RUN, false, CreateTask()));
        threads.back()->Start();
...
TEST_F(ThreadPoolTaskTrackerTest, RunAndPopNextTaskReturnsSequenceToReschedule)
{
    TaskTraits default_traits;
    Task task_1(FROM_HERE, DoNothing(), TimeTicks::Now(), TimeDelta());
    EXPECT_TRUE(tracker_.WillPostTask(&task_1, default_traits.shutdown_behavior()));
    Task task_2(FROM_HERE, DoNothing(), TimeTicks::Now(), TimeDelta());
    EXPECT_TRUE(tracker_.WillPostTask(&task_2, default_traits.shutdown_behavior()));

    scoped_refptr<Sequence> sequence = test::CreateSequenceWithTask(std::move(task_1), default_traits);
    {
...
private:
    void Run() override
    {
        ...
        Task task_without_sync_primitives(
            FROM_HERE, BindOnce([] { EXPECT_DCHECK_DEATH({ internal::AssertBaseSyncPrimitivesAllowed(); }); }), TimeTicks::Now(), TimeDelta());
        TaskTraits default_traits;
        EXPECT_TRUE(task_tracker->WillPostTask(&task_without_sync_primitives, default_traits.shutdown_behavior()));
        auto sequence_without_sync_primitives = test::CreateSequenceWithTask(std::move(task_without_sync_primitives), default_traits);
        test::QueueAndRunTaskSource(task_tracker.get(), std::move(sequence_without_sync_primitives));

        // Expect TaskTracker to keep waiting allowed when running a task with the
        // WithBaseSyncPrimitives() trait.
        internal::AssertBaseSyncPrimitivesAllowed();
        Task task_with_sync_primitives(FROM_HERE, BindOnce([] {
            // Shouldn't fail.
            internal::AssertBaseSyncPrimitivesAllowed();
        }),
            TimeTicks::Now(), TimeDelta());
        TaskTraits traits_with_sync_primitives = TaskTraits(WithBaseSyncPrimitives());
        EXPECT_TRUE(task_tracker->WillPostTask(&task_with_sync_primitives, traits_with_sync_primitives.shutdown_behavior()));
        auto sequence_with_sync_primitives = test::CreateSequenceWithTask(std::move(task_with_sync_primitives), traits_with_sync_primitives);
        test::QueueAndRunTaskSource(task_tracker.get(), std::move(sequence_with_sync_primitives));

        ScopedAllowBaseSyncPrimitivesForTesting allow_wait_in_task_tracker_destructor;
        task_tracker.reset();
    }
};
