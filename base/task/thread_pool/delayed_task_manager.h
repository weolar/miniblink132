...
private:
    ...
    std::pair<TimeTicks, subtle::DelayPolicy> GetTimeAndDelayPolicyToScheduleProcessRipeTasksLockRequired() EXCLUSIVE_LOCKS_REQUIRED(queue_lock_);
    ...
    IntrusiveHeap<DelayedTask, std::greater<>> delayed_task_queue_ GUARDED_BY(queue_lock_);

    base::TimeDelta max_precise_delay GUARDED_BY(queue_lock_) = kDefaultMaxPreciseDelay;
