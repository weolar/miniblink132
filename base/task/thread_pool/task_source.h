...
public:
    ...
    ExecutionEnvironment(SequenceToken token, SequenceLocalStorageMap* sequence_local_storage, SingleThreadTaskRunner* single_thread_task_runner)
        : token(token)
    ...
    ExecutionEnvironment(SequenceToken token, SequenceLocalStorageMap* sequence_local_storage, SequencedTaskRunner* sequenced_task_runner)
        : token(token)
        , sequence_local_storage(sequence_local_storage)
...
public:
    ...
    HeapHandle immediate_heap_handle() const
    {
        return immediate_pq_heap_handle_;
    }
    ...
    HeapHandle GetDelayedHeapHandle() const
    {
        return delayed_pq_heap_handle_;
    }

    HeapHandle delayed_heap_handle() const
    {
        return delayed_pq_heap_handle_;
    }
    ...
    ThreadPolicy thread_policy() const
    {
...
protected:
    ...
    virtual bool WillReEnqueue(TimeTicks now, TaskSource::Transaction* transaction) = 0;
...
public:
    ...
    static RegisteredTaskSource CreateForTesting(scoped_refptr<TaskSource> task_source, TaskTracker* task_tracker = nullptr);
    ...
    bool WillReEnqueue(TimeTicks now, TaskSource::Transaction* transaction = nullptr);
    ...
    // of a transaction.
    [[nodiscard]] std::optional<Task> Clear(TaskSource::Transaction* transaction = nullptr);
...
public:
    RegisteredTaskSourceAndTransaction(RegisteredTaskSource task_source_in, TaskSource::Transaction transaction_in);

    RegisteredTaskSourceAndTransaction(RegisteredTaskSourceAndTransaction&& other) = default;
    RegisteredTaskSourceAndTransaction(const RegisteredTaskSourceAndTransaction&) = delete;
    RegisteredTaskSourceAndTransaction& operator=(const RegisteredTaskSourceAndTransaction&) = delete;
    ~RegisteredTaskSourceAndTransaction() = default;

    static RegisteredTaskSourceAndTransaction FromTaskSource(RegisteredTaskSource task_source_in);
...
public:
    TaskSourceAndTransaction(scoped_refptr<TaskSource> task_source_in, TaskSource::Transaction transaction_in);
    ...
    static TaskSourceAndTransaction FromTaskSource(scoped_refptr<TaskSource> task_source_in);
