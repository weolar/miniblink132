...
public:
    ...
    scoped_refptr<TaskSource> RemoveTaskSource(scoped_refptr<TaskSource> task_source);
...
private:
    ...
    struct DelayedPriorityQueueComparisonOperator {
        bool operator()(const TaskSourceAndDelayedSortKey& lhs, const TaskSourceAndDelayedSortKey& rhs) const;
    };

    using ContainerType = IntrusiveHeap<TaskSourceAndDelayedSortKey, DelayedPriorityQueueComparisonOperator>;
