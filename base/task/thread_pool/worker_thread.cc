...
void WorkerThread::Delegate::WaitForWork()
{
    ...
    // many times.
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_CONFIG(THREAD_CACHE_SUPPORTED)
    const TimeDelta sleep_duration_before_purge = GetSleepDurationBeforePurge(base::TimeTicks::Now());

    const bool was_signaled = TimedWait(std::min(sleep_duration_before_purge, sleep_duration_before_worker_reclaim));
    // Timed out.
    if (!was_signaled) {
        ...
        if (sleep_duration_before_worker_reclaim > sleep_duration_before_purge) {
            TimedWait(sleep_duration_before_worker_reclaim.is_max() ? TimeDelta::Max() : sleep_duration_before_worker_reclaim - sleep_duration_before_purge);
        }
    }
#else
...
TimeDelta WorkerThread::Delegate::GetSleepDurationBeforePurge(TimeTicks now)
{
    ...
    const TimeTicks first_sleep_time_to_use = !first_sleep_time_for_testing_.is_null() ? first_sleep_time_for_testing_ : first_sleep_time;
    const base::TimeTicks first_wake_time = first_sleep_time_to_use + kFirstSleepDurationBeforePurge;
    ...
    if (now <= first_wake_time) {
        ...
        sleep_duration_before_purge = std::max(kFirstSleepDurationBeforePurge, sleep_duration_before_purge);
    }
    ...
    // number of workers.
    const TimeTicks snapped_purge_time = (now + sleep_duration_before_purge).SnappedToNextTick(TimeTicks(), kPurgeThreadCacheIdleDelay);
...
WorkerThread::WorkerThread(ThreadType thread_type_hint, std::unique_ptr<Delegate> delegate, TrackedRef<TaskTracker> task_tracker, size_t sequence_num,
    const CheckedLock* predecessor_lock, void* flow_terminator)
    : thread_lock_(predecessor_lock)
    , task_tracker_(std::move(task_tracker))
    , thread_type_hint_(thread_type_hint)
    , current_thread_type_(GetDesiredThreadType())
    , sequence_num_(sequence_num)
    , flow_terminator_(flow_terminator == nullptr ? reinterpret_cast<intptr_t>(this) : reinterpret_cast<intptr_t>(flow_terminator))
    , delegate_(std::move(delegate))
{
    DCHECK(task_tracker_);
    DCHECK(CanUseBackgroundThreadTypeForWorkerThread() || thread_type_hint_ != ThreadType::kBackground);
    DCHECK(CanUseUtilityThreadTypeForWorkerThread() || thread_type_hint != ThreadType::kUtility);
    DCHECK(delegate_);
    delegate_->wake_up_event_.declare_only_used_while_idle();
}

bool WorkerThread::Start(scoped_refptr<SingleThreadTaskRunner> io_thread_task_runner, WorkerThreadObserver* worker_thread_observer)
{
    ...
    PlatformThread::CreateWithType(kDefaultStackSize, this, &thread_handle_, current_thread_type_);
...
void WorkerThread::WakeUp()
{
    ...
    DCHECK(!join_called_for_testing_.IsSet());
    DCHECK(!should_exit_.IsSet());
    TRACE_EVENT_INSTANT("wakeup.flow", "WorkerThread::WakeUp", perfetto::Flow::FromPointer(this));
...
bool WorkerThread::ShouldExit() const
{
    ...
    // first.
    return should_exit_.IsSet() || join_called_for_testing_.IsSet() || task_tracker_->IsShutdownComplete();
}
...
void WorkerThread::RunWorker()
{
    ...
    const bool watch_for_hangs = base::HangWatcher::IsThreadPoolHangWatchingEnabled() && GetDesiredThreadType() != ThreadType::kBackground;

    // If this process has a HangWatcher register this thread for watching.
    base::ScopedClosureRunner unregister_for_hang_watching;
    if (watch_for_hangs) {
        unregister_for_hang_watching = base::HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kThreadPoolThread);
    }
    ...
    while (!ShouldExit()) {
        ...
        TRACE_EVENT_BEGIN("base", "WorkerThread active", perfetto::TerminatingFlow::FromPointer(reinterpret_cast<void*>(flow_terminator_)));
        ...
        while (task_source) {
            ...
            RegisteredTaskSource new_task_source = delegate()->SwapProcessedTask(std::move(task_source), this);
