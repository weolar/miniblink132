...
Task ConstructMockedTask(testing::StrictMock<MockCallback>& mock_task, TimeTicks now, TimeDelta delay)
{
    Task task(FROM_HERE, BindOnce(&MockCallback::Run, Unretained(&mock_task)), now, delay);
    return task;
}

Task ConstructMockedTask(testing::StrictMock<MockCallback>& mock_task, TimeTicks now, TimeTicks delayed_run_time, subtle::DelayPolicy delay_policy)
{
    Task task(FROM_HERE, BindOnce(&MockCallback::Run, Unretained(&mock_task)), now, delayed_run_time, kLeeway, delay_policy);
    return task;
}

class ThreadPoolDelayedTaskManagerTest : public testing::Test {
public:
    ThreadPoolDelayedTaskManagerTest(const ThreadPoolDelayedTaskManagerTest&) = delete;
    ThreadPoolDelayedTaskManagerTest& operator=(const ThreadPoolDelayedTaskManagerTest&) = delete;
...
protected:
    ThreadPoolDelayedTaskManagerTest()
    {
        ...
        task_ = ConstructMockedTask(mock_callback_, service_thread_task_runner_->NowTicks(), kLongDelay);
    }
    ~ThreadPoolDelayedTaskManagerTest() override = default;

    const scoped_refptr<TestMockTimeTaskRunner> service_thread_task_runner_ = MakeRefCounted<TestMockTimeTaskRunner>();
    DelayedTaskManager delayed_task_manager_ { service_thread_task_runner_->GetMockTickClock() };
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskPostedBeforeStartExpiresAfterStartRunsOnExpire)
{
    // Send |task| to the DelayedTaskManager.
    delayed_task_manager_.AddDelayedTask(std::move(task_), BindOnce(&PostTaskNow));
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskPostedBeforeStartExpiresBeforeStartRunsOnStart)
{
    // Send |task| to the DelayedTaskManager.
    delayed_task_manager_.AddDelayedTask(std::move(task_), BindOnce(&PostTaskNow));
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskRunsAtTime_FlexiblePreferEarly)
{
    ...
    Task task = ConstructMockedTask(mock_callback_, now, now + kUnalignedLongDelay, base::subtle::DelayPolicy::kFlexiblePreferEarly);
    ...
    service_thread_task_runner_->FastForwardBy(kUnalignedLongDelay - kLeeway - Milliseconds(1));
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskRunsAfterCancelled)
{
    ...
    Task cancelable_task(FROM_HERE, cancelable_closure.callback(), TimeTicks::Now(), kLongerDelay);
    auto post_cancelable_task_now = BindLambdaForTesting([&](Task task) { post_cancelable_task_now_invoked = true; });
    delayed_task_manager_.AddDelayedTask(std::move(cancelable_task), post_cancelable_task_now);
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTasksRunAfterDelay)
{
    ...
    Task task_a = ConstructMockedTask(mock_callback_a, service_thread_task_runner_->NowTicks(), Hours(1));

    testing::StrictMock<MockCallback> mock_callback_b;
    Task task_b = ConstructMockedTask(mock_callback_b, service_thread_task_runner_->NowTicks(), Hours(2));

    testing::StrictMock<MockCallback> mock_callback_c;
    Task task_c = ConstructMockedTask(mock_callback_c, service_thread_task_runner_->NowTicks(), Hours(1));

    // Send tasks to the DelayedTaskManager.
    delayed_task_manager_.AddDelayedTask(std::move(task_a), BindOnce(&PostTaskNow));
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTasksRunAtTime_MixedDelayPolicy)
{
    delayed_task_manager_.Start(service_thread_task_runner_);

    TimeTicks now = service_thread_task_runner_->NowTicks();
    testing::StrictMock<MockCallback> mock_callback_a;
    Task task_a = ConstructMockedTask(mock_callback_a, now, now + Milliseconds(8), base::subtle::DelayPolicy::kFlexibleNoSooner);

    testing::StrictMock<MockCallback> mock_callback_b;
    Task task_b = ConstructMockedTask(mock_callback_b, now, now + Milliseconds(10), base::subtle::DelayPolicy::kPrecise);

    // Send tasks to the DelayedTaskManager.
    delayed_task_manager_.AddDelayedTask(std::move(task_a), BindOnce(&PostTaskNow));
    EXPECT_EQ(base::subtle::DelayPolicy::kFlexibleNoSooner, delayed_task_manager_.TopTaskDelayPolicyForTesting());
    delayed_task_manager_.AddDelayedTask(std::move(task_b), BindOnce(&PostTaskNow));
    EXPECT_EQ(base::subtle::DelayPolicy::kPrecise, delayed_task_manager_.TopTaskDelayPolicyForTesting());

    // The task doesn't run before the delay has completed.
    service_thread_task_runner_->FastForwardBy(Milliseconds(10) - Milliseconds(1));
