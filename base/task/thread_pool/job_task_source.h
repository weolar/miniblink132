...
public:
    JobTaskSource(const Location& from_here, const TaskTraits& traits, RepeatingCallback<void(JobDelegate*)> worker_task,
    ...
    static JobHandle CreateJobHandle(scoped_refptr<internal::JobTaskSource> task_source)
    {
    ...
    PooledTaskRunnerDelegate* delegate() const
    {
        return delegate_;
    }
...
private:
    ...
    public:
        ...
        static constexpr uint32_t kWorkerCountIncrement = 1 << kWorkerCountBitOffset;
    ...
    public:
        ...
        // property.
        static_assert((kWaitingForWorkerToYield & kWaitingForWorkerToSignal) == kWaitingForWorkerToSignal, "");
    ...
    bool WillReEnqueue(TimeTicks now, TaskSource::Transaction* transaction) override;
    ...
    std::optional<ConditionVariable> worker_released_condition_ GUARDED_BY(worker_lock_);
