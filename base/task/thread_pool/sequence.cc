...
public:
    explicit AnnotateLockAcquired(const CheckedLock& lock) EXCLUSIVE_LOCK_FUNCTION(lock)
        : acquired_lock_(lock)
    {
...
void MaybeMakeCriticalClosure(TaskShutdownBehavior shutdown_behavior, Task& task)
{
    ...
    case TaskShutdownBehavior::BLOCK_SHUTDOWN:
        task.task = MakeCriticalClosure(task.posted_from, std::move(task.task),
...
bool Sequence::Transaction::WillPushImmediateTask()
{
    ...
    bool was_immediate = sequence()->is_immediate_.exchange(true, std::memory_order_relaxed);
...
bool Sequence::DelayedTaskGreater::operator()(const Task& lhs, const Task& rhs) const
{
    ...
    return std::tie(lhs_latest_delayed_run_time, lhs.sequence_num) > std::tie(rhs_latest_delayed_run_time, rhs.sequence_num);
}
...
Task Sequence::TakeEarliestTask()
{
    ...
    // be taken.
    if (queue_.front().queue_time <= delayed_queue_.top().latest_delayed_run_time())
        return TakeNextImmediateTask();
...
void Sequence::UpdateReadyTimes()
{
    DCHECK(!IsEmpty());
    if (queue_.empty()) {
        latest_ready_time_.store(delayed_queue_.top().latest_delayed_run_time(), std::memory_order_relaxed);
        earliest_ready_time_.store(delayed_queue_.top().earliest_delayed_run_time(), std::memory_order_relaxed);
        return;
    }

    if (delayed_queue_.empty()) {
        latest_ready_time_.store(queue_.front().queue_time, std::memory_order_relaxed);
    } else {
        latest_ready_time_.store(std::min(queue_.front().queue_time, delayed_queue_.top().latest_delayed_run_time()), std::memory_order_relaxed);
    }
...
bool Sequence::WillReEnqueue(TimeTicks now, TaskSource::Transaction* transaction)
{
    CheckedAutoLockMaybe auto_lock(transaction ? nullptr : &lock_);
    AnnotateLockAcquired annotate(lock_);
...
bool Sequence::DelayedSortKeyWillChange(const Task& delayed_task) const
{
    ...
    return delayed_task.latest_delayed_run_time() < delayed_queue_.top().latest_delayed_run_time();
}

bool Sequence::HasReadyTasks(TimeTicks now) const
{
    return now >= TS_UNCHECKED_READ(earliest_ready_time_).load(std::memory_order_relaxed);
}
...
{
    return TaskSourceSortKey(priority_racy(), TS_UNCHECKED_READ(latest_ready_time_).load(std::memory_order_relaxed));
}
...
std::optional<Task> Sequence::Clear(TaskSource::Transaction* transaction)
{
    ...
    return Task(FROM_HERE,
        base::BindOnce(
            [](base::queue<Task> queue, base::IntrusiveHeap<Task, DelayedTaskGreater> delayed_queue) {
                while (!queue.empty())
                    queue.pop();
        ...
        TimeTicks(), TimeDelta(), TimeDelta(), static_cast<int>(reinterpret_cast<intptr_t>(this)));
}
...
Sequence::Sequence(const TaskTraits& traits, SequencedTaskRunner* task_runner, TaskSourceExecutionMode execution_mode)
...
ExecutionEnvironment Sequence::GetExecutionEnvironment()
{
    if (execution_mode() == TaskSourceExecutionMode::kSingleThread) {
        return { token_, &sequence_local_storage_, static_cast<SingleThreadTaskRunner*>(task_runner()) };
    }
