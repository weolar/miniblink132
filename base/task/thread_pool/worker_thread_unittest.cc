...
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_CONFIG(THREAD_CACHE_SUPPORTED)
...
public:
    ...
    WorkerThreadDefaultDelegate& operator=(const WorkerThreadDefaultDelegate&) = delete;
...
public:
    ...
    ThreadPoolWorkerTestParam& operator=(const ThreadPoolWorkerTestParam&) = delete;
...
protected:
    ...
    void SetUp() override
    {
        worker_ = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::make_unique<TestWorkerThreadDelegate>(this), task_tracker_.GetTrackedRef(), 0);
        ASSERT_TRUE(worker_);
...
private:
    ...
    public:
        ...
        TestWorkerThreadDelegate& operator=(const TestWorkerThreadDelegate&) = delete;
        ...
        RegisteredTaskSource GetWork(WorkerThread* worker) override
        {
            ...
            for (int i = 0; i < outer_->TasksPerSequence(); ++i) {
                Task task(FROM_HERE, BindOnce(&ThreadPoolWorkerTestParam::RunTaskCallback, Unretained(outer_)), TimeTicks::Now(), TimeDelta());
                sequence_transaction.WillPushImmediateTask();
                EXPECT_TRUE(outer_->task_tracker_.WillPostTask(&task, sequence->shutdown_behavior()));
                sequence_transaction.PushImmediateTask(std::move(task));
            }
            auto registered_task_source = outer_->task_tracker_.RegisterTaskSource(sequence);
        ...
        RegisteredTaskSource SwapProcessedTask(RegisteredTaskSource registered_task_source, WorkerThread* worker) override
        {
            {
            ...
            if (outer_->TasksPerSequence() == 1) {
                ...
                scoped_refptr<TaskSource> task_source = registered_task_source.Unregister();
                {
                    // Add |task_source| to |did_run_task_sources_|.
                    CheckedAutoLock auto_lock(outer_->lock_);
                    outer_->did_run_task_sources_.push_back(std::move(task_source));
                    EXPECT_LE(outer_->did_run_task_sources_.size(), outer_->created_sequences_.size());
                }
            }
...
INSTANTIATE_TEST_SUITE_P(OneTaskPerSequence, ThreadPoolWorkerTestParam, ::testing::Values(1));
INSTANTIATE_TEST_SUITE_P(TwoTasksPerSequence, ThreadPoolWorkerTestParam, ::testing::Values(2));
...
public:
    ...
    public:
        ...
        void WaitForCleanupRequest()
        {
            cleanup_requested_.Wait();
        }

        void WaitForDelegateDestroy()
        {
            destroyed_.Wait();
        }

        void WaitForMainExit()
        {
            exited_.Wait();
        }
        ...
        void set_can_cleanup(bool can_cleanup)
        {
            can_cleanup_ = can_cleanup;
        }
    ...
    private:
        ...
        TestWaitableEvent work_running_ { WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::SIGNALED };
    ...
    ControllableCleanupDelegate& operator=(const ControllableCleanupDelegate&) = delete;
    ~ControllableCleanupDelegate() override
    {
        controls_->destroyed_.Signal();
    }
    ...
    RegisteredTaskSource GetWork(WorkerThread* worker) override
    {
        ...
        scoped_refptr<Sequence> sequence = MakeRefCounted<Sequence>(
            TaskTraits(WithBaseSyncPrimitives(), TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN), nullptr, TaskSourceExecutionMode::kParallel);
        Task task(FROM_HERE,
            BindOnce(
                [](TestWaitableEvent* work_processed, TestWaitableEvent* work_running) {
                    work_processed->Signal();
                    work_running->Wait();
                },
                Unretained(&controls_->work_processed_), Unretained(&controls_->work_running_)),
            TimeTicks::Now(), TimeDelta());
        auto transaction = sequence->BeginTransaction();
        transaction.WillPushImmediateTask();
        EXPECT_TRUE(task_tracker_->WillPostTask(&task, sequence->shutdown_behavior()));
        transaction.PushImmediateTask(std::move(task));
        auto registered_task_source = task_tracker_->RegisterTaskSource(std::move(sequence));
        EXPECT_TRUE(registered_task_source);
        registered_task_source.WillRunTask();
        return registered_task_source;
    }
    ...
    scoped_refptr<Controls> controls()
    {
        return controls_;
    }
...
public:
    ...
    MockedControllableCleanupDelegate(const MockedControllableCleanupDelegate&) = delete;
    MockedControllableCleanupDelegate& operator=(const MockedControllableCleanupDelegate&) = delete;
...
TEST_F(ThreadPoolWorkerTest, WorkerCleanupFromGetWork)
{
    ...
    MockedControllableCleanupDelegate* delegate = new StrictMock<MockedControllableCleanupDelegate>(&task_tracker);
    scoped_refptr<ControllableCleanupDelegate::Controls> controls = delegate->controls();
    controls->set_can_cleanup(true);
    EXPECT_CALL(*delegate, OnMainEntry(_));
    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kDefault, WrapUnique(delegate), task_tracker.GetTrackedRef(), 0);
...
TEST_F(ThreadPoolWorkerTest, WorkerCleanupDuringWork)
{
    ...
    std::unique_ptr<ControllableCleanupDelegate> delegate = std::make_unique<ControllableCleanupDelegate>(&task_tracker);
    scoped_refptr<ControllableCleanupDelegate::Controls> controls = delegate->controls();

    controls->HaveWorkBlock();

    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::move(delegate), task_tracker.GetTrackedRef(), 0);
    worker->Start(service_thread_.task_runner());
    worker->WakeUp();
...
public:
    ExpectThreadTypeDelegate()
        : thread_type_verified_in_get_work_event_(WaitableEvent::ResetPolicy::AUTOMATIC)
    {
    }
...
TEST_F(ThreadPoolWorkerTest, BumpThreadTypeOfAliveThreadDuringShutdown)
{
    ...
    auto registered_task_source = task_tracker.RegisterTaskSource(std::move(sequence));

    std::unique_ptr<ExpectThreadTypeDelegate> delegate(new ExpectThreadTypeDelegate);
    ExpectThreadTypeDelegate* delegate_raw = delegate.get();
    delegate_raw->SetExpectedThreadType(ThreadType::kBackground);
    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kBackground, std::move(delegate), task_tracker.GetTrackedRef(), 0);
    worker->Start(service_thread_.task_runner());
...
public:
    explicit VerifyCallsToObserverDelegate(test::MockWorkerThreadObserver* observer)
        : observer_(observer)
    {
    }
    VerifyCallsToObserverDelegate(const VerifyCallsToObserverDelegate&) = delete;
    VerifyCallsToObserverDelegate& operator=(const VerifyCallsToObserverDelegate&) = delete;
...
public:
    ...
    TimeDelta GetSleepDurationBeforePurge(TimeTicks) override
    {
        ...
        EXPECT_THAT(WorkerThreadDefaultDelegate::GetSleepDurationBeforePurge(
                        /* now=*/first_sleep_time_for_testing_),
            AllOf(Ge(WorkerThread::Delegate::kFirstSleepDurationBeforePurge),
                Le(WorkerThread::Delegate::kFirstSleepDurationBeforePurge + WorkerThread::Delegate::kPurgeThreadCacheIdleDelay)));
        ...
        EXPECT_THAT(WorkerThreadDefaultDelegate::GetSleepDurationBeforePurge(
                        /* now=*/first_sleep_time_for_testing_ + WorkerThread::Delegate::kFirstSleepDurationBeforePurge + kEpsilon),
            AllOf(Ge(WorkerThread::Delegate::kPurgeThreadCacheIdleDelay),
                Le(WorkerThread::Delegate::kPurgeThreadCacheIdleDelay + WorkerThread::Delegate::kPurgeThreadCacheIdleDelay)));
    ...
    void WaitForWork() override
    {
        ...
        for (size_t size = 8; size < partition_alloc::ThreadCache::kDefaultSizeThreshold; size++) {
        ...
        size_t cached_memory_before = partition_alloc::ThreadCache::Get()->CachedMemory();
        WorkerThreadDefaultDelegate::WaitForWork();
        size_t cached_memory_after = partition_alloc::ThreadCache::Get()->CachedMemory();
    ...
    TimeDelta GetSleepTimeout() override
    {
        return WorkerThread::Delegate::kPurgeThreadCacheIdleDelay + TestTimeouts::tiny_timeout();
    }
...
TEST_F(ThreadPoolWorkerTest, WorkerThreadCacheNoPurgeOnSignal)
{
    ...
    partition_alloc::internal::ThreadCacheProcessScopeForTesting scope(allocator_shim::internal::PartitionAllocMalloc::Allocator());
