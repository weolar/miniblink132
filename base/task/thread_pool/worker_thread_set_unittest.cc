...
protected:
    void SetUp() override
    {
        worker_a_ = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::make_unique<MockWorkerThreadDelegate>(), task_tracker_.GetTrackedRef(), 0);
        ASSERT_TRUE(worker_a_);
        worker_b_ = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::make_unique<MockWorkerThreadDelegate>(), task_tracker_.GetTrackedRef(), 1);
        ASSERT_TRUE(worker_b_);
        worker_c_ = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::make_unique<MockWorkerThreadDelegate>(), task_tracker_.GetTrackedRef(), 2);
