...
public:
    ...
    Sequence(const TaskTraits& traits, SequencedTaskRunner* task_runner, TaskSourceExecutionMode execution_mode);
    ...
    const SequenceToken& token() const LIFETIME_BOUND
    {
        return token_;
    }
    ...
    bool is_immediate_for_testing() const
    {
        return is_immediate_;
    }
    bool IsEmptyForTesting() const NO_THREAD_SAFETY_ANALYSIS
    {
        return IsEmpty();
    }
...
private:
    ...
    // key to change.
    bool DelayedSortKeyWillChange(const Task& delayed_task) const EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    base::IntrusiveHeap<Task, DelayedTaskGreater> delayed_queue_ GUARDED_BY(lock_);
