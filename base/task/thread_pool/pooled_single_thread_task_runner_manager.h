...
public:
    PooledSingleThreadTaskRunnerManager(TrackedRef<TaskTracker> task_tracker, DelayedTaskManager* delayed_task_manager);
    PooledSingleThreadTaskRunnerManager(const PooledSingleThreadTaskRunnerManager&) = delete;
    PooledSingleThreadTaskRunnerManager& operator=(const PooledSingleThreadTaskRunnerManager&) = delete;
    ...
    // production).
    void Start(scoped_refptr<SingleThreadTaskRunner> io_thread_task_runner, WorkerThreadObserver* worker_thread_observer = nullptr);
    ...
    // index.
    scoped_refptr<SingleThreadTaskRunner> CreateSingleThreadTaskRunner(const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode);
    ...
    // kEnvironmentParams[GetEnvironmentIndexForTraits(traits)].name_suffix +
    // index.
    scoped_refptr<SingleThreadTaskRunner> CreateCOMSTATaskRunner(const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode);
...
private:
    ...
    static ContinueOnShutdown TraitsToContinueOnShutdown(const TaskTraits& traits);

    template <typename DelegateType>
    scoped_refptr<PooledSingleThreadTaskRunner> CreateTaskRunnerImpl(const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode);

    template <typename DelegateType>
    std::unique_ptr<WorkerThreadDelegate> CreateWorkerThreadDelegate(const std::string& name, int id, SingleThreadTaskRunnerThreadMode thread_mode);

    template <typename DelegateType>
    WorkerThread* CreateAndRegisterWorkerThread(const std::string& name, SingleThreadTaskRunnerThreadMode thread_mode, ThreadType thread_type_hint)
        EXCLUSIVE_LOCKS_REQUIRED(lock_);

    template <typename DelegateType> WorkerThread*& GetSharedWorkerThreadForTraits(const TaskTraits& traits);
    ...
    WorkerThread* shared_worker_threads_[ENVIRONMENT_COUNT][CONTINUE_ON_SHUTDOWN_COUNT] GUARDED_BY(lock_) = {};
#if BUILDFLAG(IS_WIN)
    WorkerThread* shared_com_worker_threads_[ENVIRONMENT_COUNT][CONTINUE_ON_SHUTDOWN_COUNT] GUARDED_BY(lock_) = {};
#endif // BUILDFLAG(IS_WIN)
