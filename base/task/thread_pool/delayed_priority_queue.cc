...
public:
    ...
    TaskSourceAndDelayedSortKey(scoped_refptr<TaskSource> task_source, const TimeTicks& delayed_sort_key)
    ...
    TaskSourceAndDelayedSortKey& operator=(const TaskSourceAndDelayedSortKey&) = delete;
    ...
    TaskSourceAndDelayedSortKey& operator=(TaskSourceAndDelayedSortKey&& other) = default;
    ...
    scoped_refptr<TaskSource> task_source() const
    {
        return task_source_;
    }

    TimeTicks delayed_sort_key() const
    {
        return delayed_sort_key_;
    }
...
DelayedPriorityQueue& DelayedPriorityQueue::operator=(DelayedPriorityQueue&& other) = default;

void DelayedPriorityQueue::Push(scoped_refptr<TaskSource> task_source, TimeTicks task_source_delayed_sort_key)
{
    container_.insert(TaskSourceAndDelayedSortKey(std::move(task_source), task_source_delayed_sort_key));
}
...
scoped_refptr<TaskSource> DelayedPriorityQueue::PopTaskSource()
{
    ...
    scoped_refptr<TaskSource> task_source = task_source_and_delayed_sort_key.take_task_source();
    return task_source;
}

scoped_refptr<TaskSource> DelayedPriorityQueue::RemoveTaskSource(scoped_refptr<TaskSource> task_source)
{
    if (IsEmpty())
        return nullptr;
    ...
    TaskSourceAndDelayedSortKey& task_source_and_delayed_sort_key = const_cast<DelayedPriorityQueue::TaskSourceAndDelayedSortKey&>(container_.at(heap_handle));
...
void DelayedPriorityQueue::UpdateDelayedSortKey(scoped_refptr<TaskSource> task_source)
{
    if (IsEmpty())
        return;
    ...
    task_source = const_cast<DelayedPriorityQueue::TaskSourceAndDelayedSortKey&>(container_.at(heap_handle)).take_task_source();
    auto delayed_sort_key = task_source->GetDelayedSortKey();
    container_.Replace(heap_handle, TaskSourceAndDelayedSortKey(std::move(task_source), delayed_sort_key));
}
...
bool DelayedPriorityQueue::DelayedPriorityQueueComparisonOperator::operator()(
    const TaskSourceAndDelayedSortKey& lhs, const TaskSourceAndDelayedSortKey& rhs) const
{
