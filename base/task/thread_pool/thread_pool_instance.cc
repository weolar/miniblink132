...
size_t GetDefaultMaxNumUtilityThreads(size_t max_num_foreground_threads_in)
{
    ...
    if (num_of_efficient_processors != 0) {
        ...
        return std::max<size_t>(2, std::min(max_num_foreground_threads_in, static_cast<size_t>(num_of_efficient_processors)));
    }
...
ThreadPoolInstance::InitParams::InitParams(size_t max_num_foreground_threads_in)
    ...
    , max_num_utility_threads(GetDefaultMaxNumUtilityThreads(max_num_foreground_threads_in))
{
}

ThreadPoolInstance::InitParams::InitParams(size_t max_num_foreground_threads_in, size_t max_num_utility_threads_in)
    : max_num_foreground_threads(max_num_foreground_threads_in)
...
ThreadPoolInstance::ScopedFizzleBlockShutdownTasks::ScopedFizzleBlockShutdownTasks()
{
...
void ThreadPoolInstance::StartWithDefaultParams()
{
    ...
    //   |num_cores - 1|.
    const size_t max_num_foreground_threads = static_cast<size_t>(std::max(3, SysInfo::NumberOfProcessors() - 1));
