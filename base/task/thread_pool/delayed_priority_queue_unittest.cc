...
protected:
    scoped_refptr<Sequence> MakeSequenceWithDelayedTask(TimeDelta delayed_run_time)
    {
        ...
        sequence->BeginTransaction().PushDelayedTask(Task(FROM_HERE, DoNothing(), TimeTicks::Now(), delayed_run_time));
        return sequence;
    }
    ...
    scoped_refptr<Sequence> sequence_a = MakeSequenceWithDelayedTask(Milliseconds(90));
    TimeTicks sort_key_a = sequence_a->GetDelayedSortKey();

    scoped_refptr<Sequence> sequence_b = MakeSequenceWithDelayedTask(Milliseconds(70));
    TimeTicks sort_key_b = sequence_b->GetDelayedSortKey();

    scoped_refptr<Sequence> sequence_c = MakeSequenceWithDelayedTask(Milliseconds(80));
    TimeTicks sort_key_c = sequence_c->GetDelayedSortKey();

    scoped_refptr<Sequence> sequence_d = MakeSequenceWithDelayedTask(Milliseconds(100));
...
TEST_F(DelayedPriorityQueueWithSequencesTest, UpdateDelayedSortKey)
{
...

    {
        ...
        sequence_c->BeginTransaction().PushDelayedTask(Task(FROM_HERE, DoNothing(), TimeTicks::Now(), Milliseconds(60)));
...

    {
        ...
        sequence_d->BeginTransaction().PushDelayedTask(Task(FROM_HERE, DoNothing(), TimeTicks::Now(), Milliseconds(50)));
