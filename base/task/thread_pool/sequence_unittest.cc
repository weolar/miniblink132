...
{
    return Task(FROM_HERE, BindOnce(&MockTask::Run, Unretained(mock_task)), now, TimeDelta());
}

Task CreateDelayedTask(MockTask* mock_task, TimeDelta delay, TimeTicks now = TimeTicks::Now())
{
    return Task(FROM_HERE, BindOnce(&MockTask::Run, Unretained(mock_task)), now, delay);
}
...
TEST(ThreadPoolSequenceTest, PushTakeRemove)
{
    ...
    scoped_refptr<Sequence> sequence = MakeRefCounted<Sequence>(TaskTraits(TaskPriority::BEST_EFFORT), nullptr, TaskSourceExecutionMode::kParallel);
    ...
    auto registered_task_source = RegisteredTaskSource::CreateForTesting(sequence);
    registered_task_source.WillRunTask();
    std::optional<Task> task = registered_task_source.TakeTask(&sequence_transaction);
    ...
    EXPECT_TRUE(registered_task_source.WillReEnqueue(TimeTicks::Now(), &sequence_transaction));
...
TEST(ThreadPoolSequenceTest, GetSortKeyBestEffort)
{
    ...
    scoped_refptr<Sequence> best_effort_sequence = MakeRefCounted<Sequence>(TaskTraits(TaskPriority::BEST_EFFORT), nullptr, TaskSourceExecutionMode::kParallel);
    Sequence::Transaction best_effort_sequence_transaction(best_effort_sequence->BeginTransaction());
    best_effort_sequence_transaction.WillPushImmediateTask();
    best_effort_sequence_transaction.PushImmediateTask(std::move(best_effort_task));

    // Get the sort key.
    const TaskSourceSortKey best_effort_sort_key = best_effort_sequence->GetSortKey();

    // Take the task from the sequence, so that its sequenced time is available
    // for the check below.
    auto best_effort_registered_task_source = RegisteredTaskSource::CreateForTesting(best_effort_sequence);
    best_effort_registered_task_source.WillRunTask();
    auto take_best_effort_task = best_effort_registered_task_source.TakeTask(&best_effort_sequence_transaction);

    // Verify the sort key.
    EXPECT_EQ(TaskPriority::BEST_EFFORT, best_effort_sort_key.priority());
    EXPECT_EQ(take_best_effort_task.queue_time, best_effort_sort_key.ready_time());

    // DidProcessTask for correctness.
    best_effort_registered_task_source.DidProcessTask(&best_effort_sequence_transaction);
}
...
TEST(ThreadPoolSequenceTest, GetSortKeyForeground)
{
    ...
    scoped_refptr<Sequence> foreground_sequence = MakeRefCounted<Sequence>(TaskTraits(TaskPriority::USER_VISIBLE), nullptr, TaskSourceExecutionMode::kParallel);
    Sequence::Transaction foreground_sequence_transaction(foreground_sequence->BeginTransaction());
    ...
    // Get the sort key.
    const TaskSourceSortKey foreground_sort_key = foreground_sequence->GetSortKey();

    // Take the task from the sequence, so that its sequenced time is available
    // for the check below.
    auto foreground_registered_task_source = RegisteredTaskSource::CreateForTesting(foreground_sequence);
    foreground_registered_task_source.WillRunTask();
    auto take_foreground_task = foreground_registered_task_source.TakeTask(&foreground_sequence_transaction);
    ...
    // DidProcessTask for correctness.
    foreground_registered_task_source.DidProcessTask(&foreground_sequence_transaction);
}
...
TEST(ThreadPoolSequenceTest, DidProcessTaskWithoutWillRunTask)
{
    ...
    Sequence::Transaction sequence_transaction(sequence->BeginTransaction());
    EXPECT_TRUE(sequence_transaction.WillPushImmediateTask());
    sequence_transaction.PushImmediateTask(Task(FROM_HERE, DoNothing(), TimeTicks::Now(), TimeDelta()));

    auto registered_task_source = RegisteredTaskSource::CreateForTesting(sequence);
    EXPECT_DCHECK_DEATH({ registered_task_source.DidProcessTask(&sequence_transaction); });
}
...
TEST(ThreadPoolSequenceTest, SequenceHasWorker)
{
    ...
    std::optional<Task> task_a = registered_task_source.TakeTask(&sequence_transaction);
    ...
    std::optional<Task> task_b = registered_task_source.TakeTask(&sequence_transaction);
...
TEST(ThreadPoolSequenceTest, PushTakeRemoveDelayedTasks)
{
    ...
    EXPECT_FALSE(registered_task_source.WillReEnqueue(now, &sequence_transaction));
    EXPECT_FALSE(sequence->is_immediate_for_testing());
...
TEST(ThreadPoolSequenceTest, TestPushDelayedTaskMethodUsage)
{
    ...
    EXPECT_DCHECK_DEATH({ sequence_transaction.PushDelayedTask(std::move(task_a)); });
}
...
TEST(ThreadPoolSequenceTest, GetDelayedSortKeyMixedtasks)
{
    ...
    EXPECT_TRUE(sequence_transaction.PushDelayedTask(CreateDelayedTask(&mock_task_a, Milliseconds(10), now)));
    ...
    auto registered_task_source = RegisteredTaskSource::CreateForTesting(sequence);
    registered_task_source.WillRunTask();
    std::optional<Task> take_delayed_task = registered_task_source.TakeTask(&sequence_transaction);
    ...
    // is available for the check below.
    registered_task_source.WillRunTask();
    std::optional<Task> take_immediate_task = registered_task_source.TakeTask(&sequence_transaction);
...
TEST(ThreadPoolSequenceTest, GetDelayedSortKeyDelayedtasks)
{
    ...
    // Create a first delayed task.
    sequence_transaction.PushDelayedTask(CreateDelayedTask(&mock_task_a, Milliseconds(15), now));

    // Get the delayed sort key (first time).
    const TimeTicks sort_key_1 = sequence->GetDelayedSortKey();

    // Create a first delayed task.
    sequence_transaction.PushDelayedTask(CreateDelayedTask(&mock_task_b, Milliseconds(10), now));
