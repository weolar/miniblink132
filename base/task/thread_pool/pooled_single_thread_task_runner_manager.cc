...
{
    const bool is_background = traits.priority() == TaskPriority::BEST_EFFORT && traits.thread_policy() == ThreadPolicy::PREFER_BACKGROUND
        && CanUseBackgroundThreadTypeForWorkerThread();
    const bool is_utility = !is_background && traits.priority() <= TaskPriority::USER_VISIBLE && traits.thread_policy() == ThreadPolicy::PREFER_BACKGROUND
        && g_use_utility_thread_group;
    if (traits.may_block() || traits.with_base_sync_primitives()) {
        return is_background ? BACKGROUND_BLOCKING : is_utility ? UTILITY_BLOCKING : FOREGROUND_BLOCKING;
    }
...
public:
    WorkerThreadDelegate(const std::string& thread_name, WorkerThread::ThreadLabel thread_label, TrackedRef<TaskTracker> task_tracker)
    ...
    {
        std::optional<RegisteredTaskSourceAndTransaction> task_source_with_transaction;
        if (task_source) {
            task_source_with_transaction.emplace(RegisteredTaskSourceAndTransaction::FromTaskSource(std::move(task_source)));
            task_source_with_transaction->task_source.WillReEnqueue(TimeTicks::Now(), &task_source_with_transaction->transaction);
        }
        CheckedAutoLock auto_lock(lock_);
    ...
    bool PostTaskNow(scoped_refptr<Sequence> sequence, scoped_refptr<SingleThreadTaskRunner> task_runner, Task task)
    {
        ...
        if (task_source) {
            ...
            {
                CheckedAutoLock auto_lock(lock_);
                should_wakeup = EnqueueTaskSourceLockRequired({ std::move(task_source), std::move(transaction) });
            }
            if (should_wakeup) {
                worker_->WakeUp();
            }
...
protected:
    RegisteredTaskSource GetWorkLockRequired(WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
...
private:
    ...
    bool EnqueueTaskSourceLockRequired(RegisteredTaskSourceAndTransaction transaction_with_task_source) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
    ...
    {
        return !priority_queue_.IsEmpty() && task_tracker_->CanRunPriority(priority_queue_.PeekSortKey().priority());
    }
...
public:
    WorkerThreadCOMDelegate(const std::string& thread_name, WorkerThread::ThreadLabel thread_label, TrackedRef<TaskTracker> task_tracker)
    ...
    void OnMainEntry(WorkerThread* worker) override
    {
        ...
        if (!scoped_com_initializer_->Succeeded()) {
            ...
            SCOPED_CRASH_KEY_NUMBER("WorkerThreadCOMDelegate", "hr", scoped_com_initializer_->hr());
            NOTREACHED();
        }
    }
    ...
    void WaitForWork() override
    {
        ...
        const DWORD milliseconds_wait = checked_cast<DWORD>(sleep_time.is_max() ? INFINITE : sleep_time.InMilliseconds());
...
private:
    RegisteredTaskSource GetWorkFromWindowsMessageQueue()
    {
        ...
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE) != FALSE) {
            ...
            if (task_tracker()->WillPostTask(&pump_message_task, TaskShutdownBehavior::SKIP_ON_SHUTDOWN)) {
                auto transaction = message_pump_sequence_->BeginTransaction();
                const bool sequence_should_be_queued = transaction.WillPushImmediateTask();
                DCHECK(sequence_should_be_queued) << "GetWorkFromWindowsMessageQueue() does not expect "
                                                     "queueing of pump tasks.";
                auto registered_task_source = task_tracker_->RegisterTaskSource(std::move(message_pump_sequence_));
    ...
    const scoped_refptr<Sequence> message_pump_sequence_ = MakeRefCounted<Sequence>(TaskTraits { MayBlock() }, nullptr, TaskSourceExecutionMode::kParallel);
...
class PooledSingleThreadTaskRunnerManager::PooledSingleThreadTaskRunner : public SingleThreadTaskRunner {
public:
    ...
    PooledSingleThreadTaskRunner(
        PooledSingleThreadTaskRunnerManager* const outer, const TaskTraits& traits, WorkerThread* worker, SingleThreadTaskRunnerThreadMode thread_mode)
        : outer_(outer)
        , worker_(worker)
    ...
    PooledSingleThreadTaskRunner& operator=(const PooledSingleThreadTaskRunner&) = delete;

    // SingleThreadTaskRunner:
    bool PostDelayedTask(const Location& from_here, OnceClosure closure, TimeDelta delay) override
    {
        if (!g_manager_is_alive)
            return false;

        Task task(from_here, std::move(closure), TimeTicks::Now(), delay, MessagePump::GetLeewayIgnoringThreadOverride());
        return PostTask(std::move(task));
    }

    bool PostDelayedTaskAt(
        subtle::PostDelayedTaskPassKey, const Location& from_here, OnceClosure closure, TimeTicks delayed_run_time, subtle::DelayPolicy delay_policy) override
    {
        if (!g_manager_is_alive)
            return false;

        Task task(from_here, std::move(closure), TimeTicks::Now(), delayed_run_time, MessagePump::GetLeewayIgnoringThreadOverride(), delay_policy);
        return PostTask(std::move(task));
    }

    bool PostNonNestableDelayedTask(const Location& from_here, OnceClosure closure, TimeDelta delay) override
    {
...
private:
    ...
    bool PostTask(Task task)
    {
        ...
        outer_->delayed_task_manager_->AddDelayedTask(
            std::move(task), BindOnce(IgnoreResult(&WorkerThreadDelegate::PostTaskNow), Unretained(GetDelegate()), sequence_, base::WrapRefCounted(this)));
        return true;
    }
    ...
    const raw_ptr<PooledSingleThreadTaskRunnerManager, DisableDanglingPtrDetection> outer_;
...
PooledSingleThreadTaskRunnerManager::PooledSingleThreadTaskRunnerManager(TrackedRef<TaskTracker> task_tracker, DelayedTaskManager* delayed_task_manager)
    : task_tracker_(std::move(task_tracker))
...
#if BUILDFLAG(IS_WIN)
    static_assert(std::extent<decltype(shared_com_worker_threads_)>() == std::extent<decltype(shared_worker_threads_)>(),
        "The size of |shared_com_worker_threads_| must match "
        "|shared_worker_threads_|");
    static_assert(std::extent<std::remove_reference<decltype(shared_com_worker_threads_[0])>>()
...
void PooledSingleThreadTaskRunnerManager::Start(scoped_refptr<SingleThreadTaskRunner> io_thread_task_runner, WorkerThreadObserver* worker_thread_observer)
{
    ...
    g_use_utility_thread_group = CanUseUtilityThreadTypeForWorkerThread() && FeatureList::IsEnabled(kUseUtilityThreadGroup);
...
scoped_refptr<SingleThreadTaskRunner> PooledSingleThreadTaskRunnerManager::CreateSingleThreadTaskRunner(
...
#if BUILDFLAG(IS_WIN)
scoped_refptr<SingleThreadTaskRunner> PooledSingleThreadTaskRunnerManager::CreateCOMSTATaskRunner(
    const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode)
{
...
// static
PooledSingleThreadTaskRunnerManager::ContinueOnShutdown PooledSingleThreadTaskRunnerManager::TraitsToContinueOnShutdown(const TaskTraits& traits)
{
...
scoped_refptr<PooledSingleThreadTaskRunnerManager::PooledSingleThreadTaskRunner> PooledSingleThreadTaskRunnerManager::CreateTaskRunnerImpl(
    const TaskTraits& traits, SingleThreadTaskRunnerThreadMode thread_mode)
{
    DCHECK(thread_mode != SingleThreadTaskRunnerThreadMode::SHARED || !traits.with_base_sync_primitives())
    ...
    WorkerThread*& worker
        = thread_mode == SingleThreadTaskRunnerThreadMode::DEDICATED ? dedicated_worker : GetSharedWorkerThreadForTraits<DelegateType>(traits);
    bool new_worker = false;
    bool started;
    {
        CheckedAutoLock auto_lock(lock_);
        if (!worker) {
            const auto& environment_params = kEnvironmentParams[GetEnvironmentIndexForTraits(traits)];
            std::string worker_name;
            if (thread_mode == SingleThreadTaskRunnerThreadMode::SHARED)
                worker_name += "Shared";
            worker_name += environment_params.name_suffix;
            worker = CreateAndRegisterWorkerThread<DelegateType>(worker_name, thread_mode, environment_params.thread_type_hint);
            new_worker = true;
        }
        started = started_;
    }
...
void PooledSingleThreadTaskRunnerManager::JoinForTesting()
{
    ...
    for (const auto& worker : local_workers) {
        static_cast<WorkerThreadDelegate*>(worker->delegate())->EnableFlushPriorityQueueTaskSourcesOnDestroyForTesting();
...
template <>
std::unique_ptr<WorkerThreadDelegate> PooledSingleThreadTaskRunnerManager::CreateWorkerThreadDelegate<WorkerThreadDelegate>(
    const std::string& name, int id, SingleThreadTaskRunnerThreadMode thread_mode)
{
    return std::make_unique<WorkerThreadDelegate>(StringPrintf("ThreadPoolSingleThread%s%d", name.c_str(), id),
        thread_mode == SingleThreadTaskRunnerThreadMode::DEDICATED ? WorkerThread::ThreadLabel::DEDICATED : WorkerThread::ThreadLabel::SHARED, task_tracker_);
}

#if BUILDFLAG(IS_WIN)
template <>
std::unique_ptr<WorkerThreadDelegate> PooledSingleThreadTaskRunnerManager::CreateWorkerThreadDelegate<WorkerThreadCOMDelegate>(
    const std::string& name, int id, SingleThreadTaskRunnerThreadMode thread_mode)
{
    return std::make_unique<WorkerThreadCOMDelegate>(StringPrintf("ThreadPoolSingleThreadCOMSTA%s%d", name.c_str(), id),
        thread_mode == SingleThreadTaskRunnerThreadMode::DEDICATED ? WorkerThread::ThreadLabel::DEDICATED_COM : WorkerThread::ThreadLabel::SHARED_COM,
        task_tracker_);
}
#endif // BUILDFLAG(IS_WIN)

template <typename DelegateType>
WorkerThread* PooledSingleThreadTaskRunnerManager::CreateAndRegisterWorkerThread(
    const std::string& name, SingleThreadTaskRunnerThreadMode thread_mode, ThreadType thread_type_hint)
{
    int id = next_worker_id_++;
    std::unique_ptr<WorkerThreadDelegate> delegate = CreateWorkerThreadDelegate<DelegateType>(name, id, thread_mode);
    WorkerThreadDelegate* delegate_raw = delegate.get();
    scoped_refptr<WorkerThread> worker = MakeRefCounted<WorkerThread>(thread_type_hint, std::move(delegate), task_tracker_, workers_.size());
    delegate_raw->set_worker(worker.get());
    workers_.emplace_back(std::move(worker));
    return workers_.back().get();
}

template <> WorkerThread*& PooledSingleThreadTaskRunnerManager::GetSharedWorkerThreadForTraits<WorkerThreadDelegate>(const TaskTraits& traits)
{
    return shared_worker_threads_[GetEnvironmentIndexForTraits(traits)][TraitsToContinueOnShutdown(traits)];
}

#if BUILDFLAG(IS_WIN)
template <> WorkerThread*& PooledSingleThreadTaskRunnerManager::GetSharedWorkerThreadForTraits<WorkerThreadCOMDelegate>(const TaskTraits& traits)
{
    return shared_com_worker_threads_[GetEnvironmentIndexForTraits(traits)][TraitsToContinueOnShutdown(traits)];
}
#endif // BUILDFLAG(IS_WIN)

void PooledSingleThreadTaskRunnerManager::UnregisterWorkerThread(WorkerThread* worker)
{
