...
public:
    ...
    virtual bool PostTaskWithSequence(Task task, scoped_refptr<Sequence> sequence) = 0;
    ...
    virtual bool EnqueueJobTaskSource(scoped_refptr<JobTaskSource> task_source) = 0;

    // Removes |task_source| from the priority queue.
    virtual void RemoveJobTaskSource(scoped_refptr<JobTaskSource> task_source) = 0;
    ...
    // thread group.
    virtual void UpdatePriority(scoped_refptr<TaskSource> task_source, TaskPriority priority) = 0;
    virtual void UpdateJobPriority(scoped_refptr<TaskSource> task_source, TaskPriority priority) = 0;
};
