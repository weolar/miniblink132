...
public:
    ...
    // task when it runs.
    ThreadPostingTasks(
        test::MockPooledTaskRunnerDelegate* mock_pooled_task_runner_delegate_, TaskSourceExecutionMode execution_mode, PostNestedTask post_nested_task)
        : SimpleThread("ThreadPostingTasks")
        , post_nested_task_(post_nested_task)
        , factory_(test::CreatePooledTaskRunnerWithExecutionMode(execution_mode, mock_pooled_task_runner_delegate_), execution_mode)
    {
    }
...
protected:
    ...
    void CreateThreadGroup()
    {
        ...
        thread_group_ = std::make_unique<ThreadGroupImpl>(
            "TestThreadGroup", "A", ThreadType::kDefault, task_tracker_.GetTrackedRef(), tracked_ref_factory_.GetTrackedRef());

        mock_pooled_task_runner_delegate_.SetThreadGroup(thread_group_.get());
    }

    void StartThreadGroup(ThreadGroup::WorkerEnvironment worker_environment = ThreadGroup::WorkerEnvironment::NONE)
    {
        ASSERT_TRUE(thread_group_);
        ThreadGroupImpl* thread_group_impl = static_cast<ThreadGroupImpl*>(thread_group_.get());
        thread_group_impl->Start(kMaxTasks, kMaxBestEffortTasks, TimeDelta::Max(), service_thread_.task_runner(), nullptr, worker_environment,
...
// parameter.
class ThreadGroupTestAllExecutionModes : public ThreadGroupTestBase, public testing::WithParamInterface<TaskSourceExecutionMode> {
public:
    ThreadGroupTestAllExecutionModes() = default;
    ThreadGroupTestAllExecutionModes(const ThreadGroupTestAllExecutionModes&) = delete;
    ThreadGroupTestAllExecutionModes& operator=(const ThreadGroupTestAllExecutionModes&) = delete;
    ...
    scoped_refptr<TaskRunner> CreatePooledTaskRunner(const TaskTraits& traits = {})
    {
        return test::CreatePooledTaskRunnerWithExecutionMode(execution_mode(), &mock_pooled_task_runner_delegate_, traits);
    }
};
...
TEST_P(ThreadGroupTestAllExecutionModes, PostTasks)
{
    ...
    for (size_t i = 0; i < kNumThreadsPostingTasks; ++i) {
        threads_posting_tasks.push_back(std::make_unique<ThreadPostingTasks>(&mock_pooled_task_runner_delegate_, execution_mode(), PostNestedTask::NO));
...
TEST_P(ThreadGroupTestAllExecutionModes, NestedPostTasks)
{
    ...
    std::vector<std::unique_ptr<ThreadPostingTasks>> threads_posting_tasks;
    for (size_t i = 0; i < kNumThreadsPostingTasks; ++i) {
        threads_posting_tasks.push_back(std::make_unique<ThreadPostingTasks>(&mock_pooled_task_runner_delegate_, execution_mode(), PostNestedTask::YES));
        threads_posting_tasks.back()->Start();
    }
...
TEST_P(ThreadGroupTestAllExecutionModes, SequencedRunsTasksInCurrentSequence)
{
    StartThreadGroup();
    auto task_runner = CreatePooledTaskRunner();
    auto sequenced_task_runner = test::CreatePooledSequencedTaskRunner(TaskTraits(), &mock_pooled_task_runner_delegate_);
...
TEST_P(ThreadGroupTestAllExecutionModes, PostBeforeStart)
{
    ...
    auto task_runner = CreatePooledTaskRunner();
    task_runner->PostTask(FROM_HERE, BindOnce(&TestWaitableEvent::Signal, Unretained(&task_1_running)));
    task_runner->PostTask(FROM_HERE, BindOnce(&TestWaitableEvent::Signal, Unretained(&task_2_running)));
...
TEST_P(ThreadGroupTestAllExecutionModes, CanRunPolicyBasic)
{
    ...
    test::TestCanRunPolicyBasic(
        thread_group_.get(), [this](TaskPriority priority) { return CreatePooledTaskRunner({ priority }); }, &task_tracker_);
}
...
TEST_F(ThreadGroupTest, CanRunPolicyUpdatedBeforeRun)
{
    ...
    test::TestCanRunPolicyChangedBeforeRun(
        thread_group_.get(), [this](TaskPriority priority) { return test::CreatePooledSequencedTaskRunner({ priority }, &mock_pooled_task_runner_delegate_); },
        &task_tracker_);
}
...
TEST_F(ThreadGroupTest, SetMaxTasks)
{
    ...
    const scoped_refptr<TaskRunner> task_runner = test::CreatePooledTaskRunner({ MayBlock(), WithBaseSyncPrimitives() }, &mock_pooled_task_runner_delegate_);

    RepeatingClosure threads_running_barrier = BarrierClosure(kNewMaxTasks, BindOnce(&TestWaitableEvent::Signal, Unretained(&threads_running)));

    // Posting these tasks should cause new workers to be created.
    for (size_t i = 0; i < kNewMaxTasks; ++i) {
        task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&busy_threads_continue, &threads_running_barrier]() {
    ...
    // unblocked.
    for (size_t i = 0; i < kNewMaxTasks; ++i) {
        task_runner->PostTask(FROM_HERE, BindOnce([](AtomicFlag* is_exiting) { EXPECT_TRUE(is_exiting->IsSet()); }, Unretained(&is_exiting)));
    }
...
TEST_F(ThreadGroupTest, UpdatePriorityBestEffortToUserBlocking)
{
    ...
    ConditionVariable num_tasks_running_cv = num_tasks_running_lock.CreateConditionVariable();
    ...
    for (size_t i = 0; i < kMaxTasks; ++i) {
        task_runners.push_back(MakeRefCounted<PooledSequencedTaskRunner>(TaskTraits(TaskPriority::BEST_EFFORT), &mock_pooled_task_runner_delegate_));
        task_runners.back()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
...
{
    StartThreadGroup();
    auto task_runner = test::CreatePooledTaskRunnerWithExecutionMode(execution_mode(), &mock_pooled_task_runner_delegate_, { MayBlock() });
...
TEST_P(ThreadGroupTestAllExecutionModes, COMMTAWorkerEnvironment)
{
    ...
    auto task_runner = test::CreatePooledTaskRunnerWithExecutionMode(execution_mode(), &mock_pooled_task_runner_delegate_);
...
TEST_F(ThreadGroupTest, ShouldYieldSingleTask)
{
    ...
    test::CreatePooledTaskRunner({ TaskPriority::USER_BLOCKING }, &mock_pooled_task_runner_delegate_)->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        EXPECT_FALSE(thread_group_->ShouldYield({ TaskPriority::BEST_EFFORT, TimeTicks::Now() }));
        EXPECT_FALSE(thread_group_->ShouldYield({ TaskPriority::USER_VISIBLE, TimeTicks::Now() }));
        EXPECT_FALSE(thread_group_->ShouldYield({ TaskPriority::USER_VISIBLE, TimeTicks::Now() }));
    }));
...
TEST_F(ThreadGroupTest, ScheduleJobTaskSource)
{
    ...
    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&threads_running_barrier, &threads_continue](JobDelegate*) {
        threads_running_barrier.Run();
        threads_continue.Wait();
    }),
        /* num_tasks_to_run */ kMaxTasks);
    scoped_refptr<JobTaskSource> task_source = job_task->GetJobTaskSource(FROM_HERE, {}, &mock_pooled_task_runner_delegate_);

    auto registered_task_source = task_tracker_.RegisterTaskSource(std::move(task_source));
    EXPECT_TRUE(registered_task_source);
    thread_group_->PushTaskSourceAndWakeUpWorkers(RegisteredTaskSourceAndTransaction::FromTaskSource(std::move(registered_task_source)));
...
TEST_F(ThreadGroupTest, ScheduleJobTaskSourceMultipleTime)
{
    ...
    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&thread_running, &thread_continue](JobDelegate*) {
    ...
    thread_group_->PushTaskSourceAndWakeUpWorkers(RegisteredTaskSourceAndTransaction::FromTaskSource(task_tracker_.RegisterTaskSource(task_source)));
...
TEST_F(ThreadGroupTest, CancelJobTaskSource)
{
    ...
    ConditionVariable tasks_running_cv = tasks_running_lock.CreateConditionVariable();
    bool tasks_running = false;

    // Schedule a big number of tasks.
    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&](JobDelegate* delegate) {
        {
...
TEST_F(ThreadGroupTest, JobTaskSourceConcurrencyIncrease)
{
    ...
    RepeatingClosure threads_running_barrier = BarrierClosure(kMaxTasks / 2, BindOnce(&TestWaitableEvent::Signal, Unretained(&threads_running_a)));

    auto job_state = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&threads_running_barrier, &threads_continue](JobDelegate*) {
        threads_running_barrier.Run();
        threads_continue.Wait();
    }),
        /* num_tasks_to_run */ kMaxTasks / 2);
    auto task_source = job_state->GetJobTaskSource(FROM_HERE, {}, &mock_pooled_task_runner_delegate_);
    ...
    threads_running_barrier = BarrierClosure(kMaxTasks / 2, BindOnce(&TestWaitableEvent::Signal, Unretained(&threads_running_b)));
...
TEST_F(ThreadGroupTest, ScheduleEmptyJobTaskSource)
{
    ...
    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindRepeating([](JobDelegate*) { ShouldNotRun(); }),
        /* num_tasks_to_run */ 1);
    scoped_refptr<JobTaskSource> task_source = job_task->GetJobTaskSource(FROM_HERE, {}, &mock_pooled_task_runner_delegate_);
...
TEST_F(ThreadGroupTest, JoinJobTaskSource)
{
    ...
    TestWaitableEvent threads_continue;
    RepeatingClosure threads_continue_barrier = BarrierClosure(kMaxTasks + 1, BindOnce(&TestWaitableEvent::Signal, Unretained(&threads_continue)));

    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&](JobDelegate*) {
...
TEST_F(ThreadGroupTest, JoinJobTaskSourceStaleConcurrency)
{
    ...
    auto task_source = MakeRefCounted<JobTaskSource>(FROM_HERE, TaskTraits {}, BindLambdaForTesting([&](JobDelegate*) { thread_running.Signal(); }),
        BindLambdaForTesting([&](size_t /*worker_count*/) -> size_t { return max_concurrency; }), &mock_pooled_task_runner_delegate_);
...
TEST_F(ThreadGroupTest, CancelJobTaskSourceWithStaleConcurrency)
{
    ...
    TestWaitableEvent thread_running;
    auto task_source = MakeRefCounted<JobTaskSource>(FROM_HERE, TaskTraits {}, BindLambdaForTesting([&](JobDelegate*) { thread_running.Signal(); }),
        BindRepeating([](size_t /*worker_count*/) -> size_t { return 1; }), &mock_pooled_task_runner_delegate_);
...
TEST_F(ThreadGroupTest, JobTaskSourceUpdatePriority)
{
    ...
    auto job_task = base::MakeRefCounted<test::MockJobTask>(BindLambdaForTesting([&](JobDelegate*) {
        ...
        /* num_tasks_to_run */ kMaxTasks);
    scoped_refptr<JobTaskSource> task_source = job_task->GetJobTaskSource(FROM_HERE, { TaskPriority::BEST_EFFORT }, &mock_pooled_task_runner_delegate_);
...
INSTANTIATE_TEST_SUITE_P(GenericParallel, ThreadGroupTestAllExecutionModes, ::testing::Values(TaskSourceExecutionMode::kParallel));
INSTANTIATE_TEST_SUITE_P(GenericSequenced, ThreadGroupTestAllExecutionModes, ::testing::Values(TaskSourceExecutionMode::kSequenced));
INSTANTIATE_TEST_SUITE_P(GenericJob, ThreadGroupTestAllExecutionModes, ::testing::Values(TaskSourceExecutionMode::kJob));
