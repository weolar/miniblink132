...
public:
    ...
    RegisteredTaskSource& task_source() LIFETIME_BOUND
    {
        return task_source_;
    }

    const TaskSourceSortKey& sort_key() const LIFETIME_BOUND
    {
        return sort_key_;
    }
...
void PriorityQueue::Push(RegisteredTaskSource task_source, TaskSourceSortKey task_source_sort_key)
{
...
RegisteredTaskSource& PriorityQueue::PeekTaskSource() const
{
    ...
    auto& task_source_and_sort_key = const_cast<PriorityQueue::TaskSourceAndSortKey&>(container_.top());
...
RegisteredTaskSource PriorityQueue::PopTaskSource()
{
    ...
    auto& task_source_and_sort_key = const_cast<TaskSourceAndSortKey&>(container_.top());
...
RegisteredTaskSource PriorityQueue::RemoveTaskSource(const TaskSource& task_source)
{
    if (IsEmpty())
        return nullptr;
    ...
    TaskSourceAndSortKey& task_source_and_sort_key = const_cast<PriorityQueue::TaskSourceAndSortKey&>(container_.at(heap_handle));
...
void PriorityQueue::UpdateSortKey(const TaskSource& task_source, TaskSourceSortKey sort_key)
{
    if (IsEmpty())
        return;
    ...
    auto registered_task_source = const_cast<PriorityQueue::TaskSourceAndSortKey&>(container_.at(heap_handle)).take_task_source();

    DecrementNumTaskSourcesForPriority(old_sort_key.priority());
    IncrementNumTaskSourcesForPriority(sort_key.priority());

    container_.Replace(heap_handle, TaskSourceAndSortKey(std::move(registered_task_source), sort_key));
}
...
void PriorityQueue::swap(PriorityQueue& other)
{
    ...
    std::swap(is_flush_task_sources_on_destroy_enabled_, other.is_flush_task_sources_on_destroy_enabled_);
}
