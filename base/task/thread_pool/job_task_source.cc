...
static_assert(kMaxWorkersPerJob <= std::numeric_limits<std::invoke_result<decltype(&JobDelegate::GetTaskId), JobDelegate>::type>::max(),
...
{
    const uint32_t value_before_sub = value_.fetch_sub(kWorkerCountIncrement, std::memory_order_relaxed);
...
{
    uint32_t value_before_add = value_.fetch_add(kWorkerCountIncrement, std::memory_order_relaxed);
...
bool JobTaskSource::JoinFlag::ShouldWorkerYield()
{
    ...
    return value_.fetch_and(kWaitingForWorkerToSignal, std::memory_order_relaxed) == kWaitingForWorkerToYield;
}
...
JobTaskSource::JobTaskSource(const Location& from_here, const TaskTraits& traits, RepeatingCallback<void(JobDelegate*)> worker_task,
    MaxConcurrencyCallback max_concurrency_callback, PooledTaskRunnerDelegate* delegate)
...
bool JobTaskSource::WillJoin()
{
    ...
    if (!state_before_add.is_canceled() && state_before_add.worker_count() < GetMaxConcurrency(state_before_add.worker_count())) {
        return true;
    }
...
bool JobTaskSource::WaitForParticipationOpportunity()
{
    ...
    while (!((state.worker_count() <= max_concurrency && !state.is_canceled()) || state.worker_count() == 1)) {
        // std::memory_order_relaxed is sufficient because no other state is
...
TaskSource::RunStatus JobTaskSource::WillRunTask()
{
    ...
    return max_concurrency == worker_count_before_add + 1 ? RunStatus::kAllowedSaturated : RunStatus::kAllowedNotSaturated;
}
...
{
    static_assert(kMaxWorkersPerJob <= sizeof(assigned_task_ids_) * 8, "TaskId bitfield isn't big enough to fit kMaxWorkersPerJob.");
    ...
    } while (!assigned_task_ids_.compare_exchange_weak(assigned_task_ids, new_assigned_task_ids, std::memory_order_acquire, std::memory_order_relaxed));
...
void JobTaskSource::ReleaseTaskId(uint8_t task_id)
{
    ...
    uint32_t previous_task_ids = assigned_task_ids_.fetch_and(~(uint32_t(1) << task_id), std::memory_order_release);
...
bool JobTaskSource::ShouldYield()
{
    ...
    return TS_UNCHECKED_READ(join_flag_).ShouldWorkerYield() || TS_UNCHECKED_READ(state_).Load().is_canceled();
}
...
bool JobTaskSource::DidProcessTask(TaskSource::Transaction* /*transaction*/)
{
    ...
    return state_before_sub.worker_count() <= GetMaxConcurrency(state_before_sub.worker_count() - 1);
}

// This is a no-op and should always return true.
bool JobTaskSource::WillReEnqueue(TimeTicks now, TaskSource::Transaction* /*transaction*/)
{
    return true;
}
...
{
    return TaskSourceSortKey(priority_racy(), ready_time_, TS_UNCHECKED_READ(state_).Load().worker_count());
}
