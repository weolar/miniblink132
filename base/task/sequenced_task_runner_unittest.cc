...
class FlagOnDelete {
public:
    FlagOnDelete(bool* deleted, scoped_refptr<SequencedTaskRunner> expected_deletion_sequence)
        : deleted_(deleted)
...
using SequenceBoundUniquePtr = std::unique_ptr<FlagOnDelete, OnTaskRunnerDeleter>;
...
TEST_F(SequencedTaskRunnerTest, OnTaskRunnerDeleterOnMainThread)
{
    ...
    SequenceBoundUniquePtr ptr(
        new FlagOnDelete(&deleted_on_main_thread, SequencedTaskRunner::GetCurrentDefault()), OnTaskRunnerDeleter(SequencedTaskRunner::GetCurrentDefault()));
    EXPECT_FALSE(deleted_on_main_thread);
    foreign_runner_->PostTask(FROM_HERE, DoNothingWithBoundArgs(std::move(ptr)));

    {
        RunLoop run_loop;
        foreign_runner_->PostTaskAndReply(FROM_HERE, BindOnce([] {}), run_loop.QuitClosure());
        run_loop.Run();
    }
...
{
    bool deleted_on_main_thread = false;
    FlagOnDelete* raw = new FlagOnDelete(&deleted_on_main_thread, SequencedTaskRunner::GetCurrentDefault());
...
TEST_F(SequencedTaskRunnerTest, DelayedTaskHandle_RunTask)
{
    ...
    bool task_ran = false;
    DelayedTaskHandle delayed_task_handle = task_runner->PostCancelableDelayedTask(
        subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&task_ran] { task_ran = true; }), Seconds(1));
...
protected:
    ...
    static void VerifyCurrentSequencedTaskRunner()
    {
        ...
        scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunner::GetCurrentDefault();
        ASSERT_TRUE(task_runner);

        // Use SequenceCheckerImpl to make sure it's not a no-op in Release builds.
        std::unique_ptr<SequenceCheckerImpl> sequence_checker = std::make_unique<SequenceCheckerImpl>();
        task_runner->PostTask(FROM_HERE, base::BindOnce(&SequencedTaskRunnerCurrentDefaultHandleTest::CheckValidSequence, std::move(sequence_checker)));
    }

    static void CheckValidSequence(std::unique_ptr<SequenceCheckerImpl> sequence_checker)
    {
...
TEST_F(SequencedTaskRunnerCurrentDefaultHandleTest, FromThreadPoolSequencedTask)
{
    base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&SequencedTaskRunnerCurrentDefaultHandleTest::VerifyCurrentSequencedTaskRunner));
    task_environment_.RunUntilIdle();
}

TEST_F(SequencedTaskRunnerCurrentDefaultHandleTest, NoHandleFromUnsequencedTask)
{
    base::ThreadPool::PostTask(base::BindOnce([] { EXPECT_FALSE(SequencedTaskRunner::HasCurrentDefault()); }));
    task_environment_.RunUntilIdle();
}
...
TEST_F(SequencedTaskRunnerCurrentDefaultHandleTest, OverrideWithNull)
{
    ...
    {
        SequencedTaskRunner::CurrentDefaultHandle handle(nullptr, SequencedTaskRunner::CurrentDefaultHandle::MayAlreadyExist {});
...
TEST_F(SequencedTaskRunnerCurrentDefaultHandleTest, OverrideWithNonNull)
{
...

    {
        ...
        SequencedTaskRunner::CurrentDefaultHandle handle(tr2, SequencedTaskRunner::CurrentDefaultHandle::MayAlreadyExist {});
        EXPECT_TRUE(SequencedTaskRunner::HasCurrentDefault());
...
TEST(SequencedTaskRunnerCurrentDefaultHandleTestWithoutTaskEnvironment, FromHandleInScope)
{
    scoped_refptr<SequencedTaskRunner> test_task_runner = MakeRefCounted<TestSimpleTaskRunner>();
    EXPECT_FALSE(SequencedTaskRunner::HasCurrentDefault());
