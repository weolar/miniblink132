...
constexpr char kStoryBusyWaitLoopAroundDisrupted[] = "busy_wait_loop_around_disrupted";
...
public:
    ...
    bool TryAcquire(size_t index)
    {
        return items_[index].TryAcquire();
    }
...
void DisruptivePostTasks(size_t task_count, TimeDelta delay)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, { TaskPriority::USER_BLOCKING }, BindOnce(&DisruptivePostTasks, task_count, delay), delay);
}
...
public:
    ...
    void RunJobWithNaiveAssignment(const std::string& story_name, size_t num_work_items, RepeatingCallback<void(size_t)> process_item)
    {
        ...
        auto handle = PostJob(FROM_HERE, { TaskPriority::USER_VISIBLE },
            BindRepeating(
                [](WorkList* work_list, WaitableEvent* complete, JobDelegate* delegate) {
                    for (size_t i = 0; i < work_list->NumWorkItems() && work_list->NumIncompleteWorkItems(0) != 0 && !delegate->ShouldYield(); ++i) {
            ...
            BindRepeating(&WorkList::NumIncompleteWorkItems, Unretained(&work_list)));
        ...
        reporter.AddResult(kMetricWorkThroughput, size_t(num_work_items / job_duration.InMilliseconds()));
    }

    // Process |num_work_items| items with |process_item| in parallel. Work is
    // assigned by having each worker sequentially traversing all items
    // synchronized with an atomic variable.
    void RunJobWithAtomicAssignment(
        const std::string& story_name, size_t num_work_items, RepeatingCallback<void(size_t)> process_item, bool disruptive_post_tasks = false)
    {
        WorkList work_list(num_work_items, std::move(process_item));
        ...
        auto handle = PostJob(FROM_HERE, { TaskPriority::USER_VISIBLE },
            BindRepeating(
                [](WorkList* work_list, WaitableEvent* complete, std::atomic_size_t* index, JobDelegate* delegate) {
                    while (!delegate->ShouldYield()) {
                        const size_t i = index->fetch_add(1, std::memory_order_relaxed);
                        if (i >= work_list->NumWorkItems() || !work_list->ProcessWorkItem(i)) {
                            complete->Signal();
                            return;
                        }
                    }
    ...
    void RunJobWithDynamicAssignment(
        const std::string& story_name, size_t num_work_items, RepeatingCallback<void(size_t)> process_item, bool disruptive_post_tasks = false)
    {
        WorkList work_list(num_work_items, std::move(process_item));
        ...
        auto handle = PostJob(FROM_HERE, { TaskPriority::USER_VISIBLE },
            BindRepeating(
                [](IndexGenerator* generator, WorkList* work_list, WaitableEvent* complete, JobDelegate* delegate) {
                    while (work_list->NumIncompleteWorkItems(0) != 0 && !delegate->ShouldYield()) {
                ...
                Unretained(&generator), Unretained(&work_list), Unretained(&complete)),
            BindRepeating(&WorkList::NumIncompleteWorkItems, Unretained(&work_list)));
...
{
    RunJobWithAtomicAssignment(kStoryNoOpAtomicDisrupted, 10000000, DoNothing(), true);
}
...
{
    RepeatingCallback<void(size_t)> callback = BusyWaitCallback(Microseconds(5));
    RunJobWithAtomicAssignment(kStoryBusyWaitAtomicDisrupted, 500000, std::move(callback), true);
}
...
{
    RunJobWithDynamicAssignment(kStoryNoOpDynamicDisrupted, 10000000, DoNothing(), true);
}

TEST_F(JobPerfTest, BusyWaitWorkDynamicAssignment)
{
    RepeatingCallback<void(size_t)> callback = BusyWaitCallback(Microseconds(5));
    RunJobWithDynamicAssignment(kStoryBusyWaitDynamic, 500000, std::move(callback));
}

TEST_F(JobPerfTest, BusyWaitDisruptedWorkDynamicAssignment)
{
    RepeatingCallback<void(size_t)> callback = BusyWaitCallback(Microseconds(5));
    RunJobWithDynamicAssignment(kStoryBusyWaitDynamicDisrupted, 500000, std::move(callback), true);
}
...
{
    RunJobWithLoopAround(kStoryNoOpLoopAroundDisrupted, 10000000, DoNothing(), true);
}
...
{
    RepeatingCallback<void(size_t)> callback = BusyWaitCallback(Microseconds(5));
    RunJobWithLoopAround(kStoryBusyWaitLoopAroundDisrupted, 500000, std::move(callback), true);
}
