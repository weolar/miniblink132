...
template <typename CallbackType> class BindPostTaskTrampoline {
public:
    BindPostTaskTrampoline(scoped_refptr<TaskRunner> task_runner, const Location& location, CallbackType callback)
    ...
    BindPostTaskTrampoline& operator=(const BindPostTaskTrampoline& other) = delete;
    ...
    ~BindPostTaskTrampoline()
    {
        if (callback_) {
            ...
            task_runner_->PostTask(location_, base::BindOnce(&DestroyCallbackOnTaskRunner, std::move(callback_)));
        }
    }
...
private:
    ...
    template <typename... Args> static OnceClosure GetClosure(OnceCallback<void(Args...)>* callback, Args&&... args)
    {
    ...
    template <typename... Args> static OnceClosure GetClosure(RepeatingCallback<void(Args...)>* callback, Args&&... args)
    {
