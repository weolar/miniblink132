...
public:
    ...
    TaskQueueImpl(SequenceManagerImpl* sequence_manager, WakeUpQueue* wake_up_queue, const TaskQueue::Spec& spec);
    ...
    using TaskExecutionTraceLogger = RepeatingCallback<void(perfetto::EventContext&, const Task&)>;
    ...
    scoped_refptr<SingleThreadTaskRunner> CreateTaskRunner(TaskType task_type) const override;
    ...
    [[nodiscard]] std::unique_ptr<TaskQueue::OnTaskPostedCallbackHandle> AddOnTaskPostedHandler(OnTaskPostedHandler handler) override;
    ...
    bool GetQuiescenceMonitored() const
    {
    ...
    void NotifyWillProcessTask(const Task& task, bool was_blocked_or_low_priority);
    ...
    bool HasTaskToRunImmediatelyLocked() const EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    ...
    void MoveReadyDelayedTasksToWorkQueue(LazyNow* lazy_now, EnqueueOrder enqueue_order);
    ...
    void OnTaskStarted(const Task& task, const TaskQueue::TaskTiming& task_timing);
    void OnTaskCompleted(const Task& task, TaskQueue::TaskTiming* task_timing, LazyNow* lazy_now);
    ...
    void AddQueueEnabledVoter(bool voter_is_enabled, TaskQueue::QueueEnabledVoter& voter);
    void RemoveQueueEnabledVoter(bool voter_is_enabled, TaskQueue::QueueEnabledVoter& voter);
...
private:
    ...
    public:
        explicit TaskRunner(scoped_refptr<GuardedTaskPoster> task_poster, scoped_refptr<AssociatedThreadId> associated_thread, TaskType task_type);

        bool PostDelayedTask(const Location& location, OnceClosure callback, TimeDelta delay) final;
        bool PostDelayedTaskAt(subtle::PostDelayedTaskPassKey, const Location& location, OnceClosure callback, TimeTicks delayed_run_time,
            base::subtle::DelayPolicy delay_policy) final;
        DelayedTaskHandle PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKey, const Location& location, OnceClosure callback,
            TimeTicks delayed_run_time, base::subtle::DelayPolicy delay_policy) final;
        DelayedTaskHandle PostCancelableDelayedTask(subtle::PostDelayedTaskPassKey, const Location& location, OnceClosure callback, TimeDelta delay) final;
        bool PostNonNestableDelayedTask(const Location& location, OnceClosure callback, TimeDelta delay) final;
        bool RunOrPostTask(subtle::RunOrPostTaskPassKey, const Location& from_here, OnceClosure task) final;
    ...
    class OnTaskPostedCallbackHandleImpl : public TaskQueue::OnTaskPostedCallbackHandle {
    public:
        OnTaskPostedCallbackHandleImpl(TaskQueueImpl* task_queue_impl, scoped_refptr<const AssociatedThreadId> associated_thread_);
    ...
    // only fast path.
    void PushOntoDelayedIncomingQueueFromMainThread(Task pending_task, LazyNow* lazy_now, bool notify_task_annotator);
    ...
    void MoveReadyImmediateTasksToImmediateWorkQueueLocked() EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    ...
    void UpdateCrossThreadQueueStateLocked() EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    ...
    bool ShouldReportIpcTaskQueuedFromAnyThreadLocked(base::TimeDelta* time_since_disabled) EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    void MaybeReportIpcTaskQueuedFromAnyThreadLocked(const Task& pending_task) EXCLUSIVE_LOCKS_REQUIRED(any_thread_lock_);
    void MaybeReportIpcTaskQueuedFromAnyThreadUnlocked(const Task& pending_task);
    void ReportIpcTaskQueued(const Task& pending_task, const base::TimeDelta& time_since_disabled);
    ...
    void RemoveOnTaskPostedHandler(OnTaskPostedCallbackHandleImpl* on_task_posted_callback_handle);

    TaskQueue::QueuePriority DefaultPriority() const;

    bool AreAllQueueEnabledVotersEnabled() const
    {
        return main_thread_only().enabled_voter_count == main_thread_only().voter_count;
    }
    ...
    QueueName name_;
    const raw_ptr<SequenceManagerImpl, AcrossTasksDanglingUntriaged> sequence_manager_;
