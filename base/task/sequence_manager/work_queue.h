...
public:
    ...
    TaskQueueImpl* task_queue() const
    {
        return task_queue_;
    }

    WorkQueueSets* work_queue_sets() const
    {
        return work_queue_sets_;
    }

    size_t work_queue_set_index() const
    {
    ...
    void set_heap_handle(HeapHandle handle)
    {
        heap_handle_ = handle;
    }
    ...
    // |result|.
    void CollectTasksOlderThan(TaskOrder reference, std::vector<const Task*>* result) const;
