...
WorkQueue::~WorkQueue()
{
    DCHECK(!work_queue_sets_) << task_queue_->GetName() << " : " << work_queue_sets_->GetName() << " : " << name_;
}
...
std::optional<TaskOrder> WorkQueue::GetFrontTaskOrder() const
{
    if (tasks_.empty() || BlockedByFence())
        return std::nullopt;
    // Quick sanity check.
    DCHECK(tasks_.front().task_order() <= tasks_.back().task_order()) << task_queue_->GetName() << " : " << work_queue_sets_->GetName() << " : " << name_;
...
void WorkQueue::Push(Task task)
{
    ...
    DCHECK(was_empty || tasks_.back().enqueue_order() < task.enqueue_order()
        || (queue_type_ == QueueType::kDelayed && tasks_.back().enqueue_order() == task.enqueue_order()));
...
WorkQueue::TaskPusher::TaskPusher(WorkQueue* work_queue)
    ...
    , was_empty_(work_queue->Empty())
{
}

WorkQueue::TaskPusher::TaskPusher(TaskPusher&& other)
    : work_queue_(other.work_queue_)
    , was_empty_(other.was_empty_)
{
...
void WorkQueue::TaskPusher::Push(Task task)
{
    ...
    // Make sure the task order is strictly increasing.
    DCHECK(work_queue_->tasks_.empty() || work_queue_->tasks_.back().task_order() < task.task_order());
    // Make sure enqueue order is strictly increasing for immediate queues and
    // monotonically increasing for delayed queues.
    DCHECK(work_queue_->tasks_.empty() || work_queue_->tasks_.back().enqueue_order() < task.enqueue_order()
        || (work_queue_->queue_type_ == QueueType::kDelayed && work_queue_->tasks_.back().enqueue_order() == task.enqueue_order()));
...
WorkQueue::TaskPusher::~TaskPusher()
{
    ...
    if (was_empty_ && work_queue_ && !work_queue_->Empty() && work_queue_->work_queue_sets_ && !work_queue_->BlockedByFence()) {
...
void WorkQueue::PushNonNestableTaskToFront(Task task)
{
    ...
    if (!was_empty) {
        // Make sure the task order is strictly increasing.
        DCHECK(task.task_order() < tasks_.front().task_order()) << task_queue_->GetName() << " : " << work_queue_sets_->GetName() << " : " << name_;
        ...
        DCHECK(task.enqueue_order() < tasks_.front().enqueue_order()
            || (queue_type_ == QueueType::kDelayed && task.enqueue_order() == tasks_.front().enqueue_order()))
...
{
    DCHECK(!fence_ || fence.task_order() >= fence_->task_order() || fence.IsBlockingFence());
...
void WorkQueue::CollectTasksOlderThan(TaskOrder reference, std::vector<const Task*>* result) const
{
    for (const Task& task : tasks_) {
