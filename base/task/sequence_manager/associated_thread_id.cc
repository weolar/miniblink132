...
{
#if DCHECK_IS_ON()
    const auto prev_thread_ref = bound_thread_ref_.load(std::memory_order_relaxed);
    DCHECK(prev_thread_ref.is_null() || prev_thread_ref == PlatformThread::CurrentRef());
#endif
    sequence_token_ = base::internal::SequenceToken::GetForCurrentThread();
    bound_thread_ref_.store(PlatformThread::CurrentRef(), std::memory_order_release);
...
{
    const PlatformThreadRef bound_thread_ref = bound_thread_ref_.load(std::memory_order_relaxed);
    const PlatformThreadRef in_sequence_thread_ref = in_sequence_thread_ref_.load(std::memory_order_relaxed);
...
void AssociatedThreadId::StartInSequenceWithCurrentThread()
{
    ...
    bool succeeded = in_sequence_thread_ref_.compare_exchange_strong(expected, PlatformThread::CurrentRef(), std::memory_order_relaxed);
    CHECK(succeeded);
}
...
void AssociatedThreadId::StopInSequenceWithCurrentThread()
{
    ...
    bool succeeded = in_sequence_thread_ref_.compare_exchange_strong(expected, PlatformThreadRef(), std::memory_order_relaxed);
    CHECK(succeeded);
}
