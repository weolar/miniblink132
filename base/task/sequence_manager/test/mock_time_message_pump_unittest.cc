...
TEST(MockMessagePumpTest, AdvancesTimeAsAllowed)
{
    ...
    EXPECT_CALL(delegate, DoWork).Times(3).WillRepeatedly(Invoke([&] { return NextWorkInfo(mock_clock.NowTicks() + Seconds(1)); }));
...
TEST(MockMessagePumpTest, AdvancesUntilAllowedTime)
{
    ...
    pump.SetAllowTimeToAutoAdvanceUntil(kEndTime);
    pump.SetStopWhenMessagePumpIsIdle(true);
    EXPECT_CALL(delegate, DoWork).Times(2).WillRepeatedly(Return(NextWorkInfo(kNextDelayedWorkTime)));
...
TEST(MockMessagePumpTest, StoresNextWakeUpTime)
{
    ...
    pump.SetAllowTimeToAutoAdvanceUntil(kEndTime);
    pump.SetStopWhenMessagePumpIsIdle(true);
    EXPECT_CALL(delegate, DoWork).WillOnce(Return(NextWorkInfo(kNextDelayedWorkTime)));
...
TEST(MockMessagePumpTest, StoresNextWakeUpTimeInScheduleDelayedWork)
{
    ...
    pump.ScheduleDelayedWork(MessagePump::Delegate::NextWorkInfo { kNextDelayedWorkTime, TimeDelta(), kStartTime });
...
TEST(MockMessagePumpTest, AdvancesUntilAllowedTimeWhenNextDelayedWorkTimeIsMax)
{
    SimpleTestTickClock mock_clock;
    mock_clock.Advance(Hours(42));
    StrictMock<MockMessagePumpDelegate> delegate;
    ...
    EXPECT_CALL(delegate, DoWork).WillRepeatedly(Return(NextWorkInfo(TimeTicks::Max())));
    EXPECT_CALL(delegate, DoIdleWork).Times(2);
