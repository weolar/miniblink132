...
public:
    ...
    struct BASE_EXPORT SelectedTask {
        ...
        SelectedTask(Task& task, TaskExecutionTraceLogger task_execution_trace_logger, TaskQueue::QueuePriority priority, QueueName task_queue_name);
        ~SelectedTask();
        ...
        TaskExecutionTraceLogger task_execution_trace_logger = TaskExecutionTraceLogger();
    ...
    virtual void SetRunTaskSynchronouslyAllowed(bool can_run_tasks_synchronously) = 0;
    ...
    virtual std::optional<SelectedTask> SelectNextTask(LazyNow& lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) = 0;
    ...
    virtual std::optional<WakeUp> GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) = 0;
    ...
    virtual void MaybeEmitTaskDetails(perfetto::EventContext& ctx, const SelectedTask& selected_task) const = 0;
};
