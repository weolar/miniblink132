...
public:
    WorkQueueTest()
        : WorkQueueTest(WorkQueue::QueueType::kImmediate)
    {
    }
    ...
    void SetUp() override
    {
        task_queue_ = std::make_unique<TaskQueueImpl>(
            /*sequence_manager=*/nullptr, /*wake_up_queue=*/nullptr, TaskQueue::Spec(QueueName::TEST_TQ));

        work_queue_ = std::make_unique<WorkQueue>(task_queue_.get(), "test", queue_type_);
        mock_observer_ = std::make_unique<MockObserver>();
        work_queue_sets_ = std::make_unique<WorkQueueSets>("test", mock_observer_.get(), SequenceManager::Settings());
...
protected:
    Task FakeCancelableTaskWithEnqueueOrder(int enqueue_order, WeakPtr<Cancelable> weak_ptr)
    {
        Task fake_task(
            PostedTask(nullptr, BindOnce(&Cancelable::NopTask, weak_ptr), FROM_HERE), EnqueueOrder(), EnqueueOrder::FromIntForTesting(enqueue_order));
        return fake_task;
    }

    Task FakeTaskWithEnqueueOrder(int enqueue_order)
    {
        Task fake_task(PostedTask(nullptr, BindOnce(&NopTask), FROM_HERE), EnqueueOrder(), EnqueueOrder::FromIntForTesting(enqueue_order));
        return fake_task;
    }
    ...
    {
        Task fake_task(PostedTask(nullptr, BindOnce(&NopTask), FROM_HERE, task_order.delayed_run_time(), subtle::DelayPolicy::kFlexibleNoSooner),
            EnqueueOrder::FromIntForTesting(task_order.sequence_num()), task_order.enqueue_order(), TimeTicks() + Milliseconds(1));
        return fake_task;
    }

    Fence CreateFenceWithEnqueueOrder(int enqueue_order)
    {
        return Fence(TaskOrder::CreateForTesting(EnqueueOrder::FromIntForTesting(enqueue_order)));
    }

    WorkQueue* GetOldestQueueInSet(int set)
    {
        if (auto queue_and_task_order = work_queue_sets_->GetOldestQueueAndTaskOrderInSet(set)) {
...
public:
    DelayedWorkQueueTest()
        : WorkQueueTest(WorkQueue::QueueType::kDelayed)
    {
    }
};
...
TEST_F(WorkQueueTest, PushNonNestableTaskToFront)
{
    ...
    work_queue_->PushNonNestableTaskToFront(FakeNonNestableTaskWithEnqueueOrder(3));
    EXPECT_EQ(work_queue_.get(), GetOldestQueueInSet(0));

    work_queue_->PushNonNestableTaskToFront(FakeNonNestableTaskWithEnqueueOrder(2));
    EXPECT_EQ(2ull, work_queue_->GetFrontTask()->enqueue_order());
...
{
    {
        ...
        work_queue_->Push(FakeCancelableTaskWithEnqueueOrder(2, cancelable.weak_ptr_factory.GetWeakPtr()));
        work_queue_->Push(FakeCancelableTaskWithEnqueueOrder(3, cancelable.weak_ptr_factory.GetWeakPtr()));
        work_queue_->Push(FakeCancelableTaskWithEnqueueOrder(4, cancelable.weak_ptr_factory.GetWeakPtr()));
        work_queue_->Push(FakeTaskWithEnqueueOrder(5));
    }
...
TEST_F(WorkQueueTest, CollectTasksOlderThan)
{
    ...
    work_queue_->CollectTasksOlderThan(TaskOrder::CreateForTesting(EnqueueOrder::FromIntForTesting(4), TimeTicks(), 0), &result);
...
TEST_F(DelayedWorkQueueTest, DelayedFenceInDelayedTaskGroup)
{
    ...
    EXPECT_EQ(work_queue_.get(), GetOldestQueueInSet(0));
    EXPECT_FALSE(work_queue_->Empty());
    EXPECT_TRUE(task_orders[0] == work_queue_->TakeTaskFromWorkQueue().task_order());

    EXPECT_FALSE(work_queue_->BlockedByFence());
    EXPECT_EQ(work_queue_.get(), GetOldestQueueInSet(0));
    EXPECT_FALSE(work_queue_->Empty());
    EXPECT_TRUE(task_orders[1] == work_queue_->TakeTaskFromWorkQueue().task_order());
