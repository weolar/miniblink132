...
public:
    ThreadControllerForTest(std::unique_ptr<MessagePump> pump, const SequenceManager::Settings& settings)
    ...
    using ThreadControllerWithMessagePumpImpl::ThreadControllerPowerMonitorForTesting;

    class MockTraceObserver : public ThreadController::RunLevelTracker::TraceObserverForTesting {
    public:
...
public:
    ...
    void ScheduleDelayedWork(const MessagePump::Delegate::NextWorkInfo& next_work_info) override
    {
...
public:
    bool PostDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay) override
    {
        return true;
    }

    bool PostNonNestableDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay) override
    {
        return true;
    }
...
public:
    explicit FakeSequencedTaskSource(TickClock* clock)
        : clock_(clock)
    {
    }
    ~FakeSequencedTaskSource() override = default;

    void SetRunTaskSynchronouslyAllowed(bool can_run_tasks_synchronously) override
    {
    }

    std::optional<SelectedTask> SelectNextTask(LazyNow& lazy_now, SelectTaskOption option) override
    {
        if (tasks_.empty())
            return std::nullopt;
        if (tasks_.front().delayed_run_time > clock_->NowTicks())
            return std::nullopt;
        if (option == SequencedTaskSource::SelectTaskOption::kSkipDelayedTask && !tasks_.front().delayed_run_time.is_null()) {
            return std::nullopt;
        }
        task_execution_stack_.push_back(std::move(tasks_.front()));
        tasks_.pop();
        return SelectedTask(task_execution_stack_.back(), TaskExecutionTraceLogger(),
            static_cast<TaskQueue::QueuePriority>(TaskQueue::DefaultQueuePriority::kNormalPriority), QueueName::TEST_TQ);
    }
    ...
    std::optional<WakeUp> GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option) override
    {
        if (tasks_.empty())
            return std::nullopt;
        if (option == SequencedTaskSource::SelectTaskOption::kSkipDelayedTask && !tasks_.front().delayed_run_time.is_null()) {
    ...
    void AddTask(Location posted_from, OnceClosure task, TimeTicks delayed_run_time = TimeTicks(), TimeTicks queue_time = TimeTicks())
    {
        DCHECK(tasks_.empty() || delayed_run_time.is_null() || tasks_.back().delayed_run_time < delayed_run_time);
        tasks_.push(Task(PostedTask(nullptr, std::move(task), posted_from, delayed_run_time, base::subtle::DelayPolicy::kFlexibleNoSooner),
    ...
    void MaybeEmitTaskDetails(perfetto::EventContext& ctx, const SelectedTask& selected_task) const override
    {
    }
...
public:
    explicit ThreadControllerWithMessagePumpTestBase(bool can_run_tasks_by_batches)
        : settings_(SequenceManager::Settings::Builder().SetTickClock(&clock_).SetCanRunTasksByBatches(can_run_tasks_by_batches).Build())
        , thread_controller_(std::make_unique<testing::StrictMock<MockMessagePump>>(), settings_)
        , message_pump_(static_cast<MockMessagePump*>(thread_controller_.GetBoundMessagePump()))
        , task_source_(&clock_)
    {
    ...
    TimeTicks FromNow(TimeDelta delta)
    {
        return clock_.NowTicks() + delta;
    }
...
class ThreadControllerWithMessagePumpTest : public ThreadControllerWithMessagePumpTestBase {
public:
...
class ThreadControllerWithMessagePumpNoBatchesTest : public ThreadControllerWithMessagePumpTestBase {
public:
...
TEST_F(ThreadControllerWithMessagePumpTest, ScheduleDelayedWork)
{
    ...
    task_source_.AddTask(FROM_HERE, task1.Get(), FromNow(Seconds(10)), clock_.NowTicks());
    MockCallback<OnceClosure> task2;
    task_source_.AddTask(FROM_HERE, task2.Get());
    MockCallback<OnceClosure> task3;
    task_source_.AddTask(FROM_HERE, task3.Get(), FromNow(Seconds(20)), clock_.NowTicks());
...
{
    EXPECT_CALL(*message_pump_, ScheduleDelayedWork_TimeTicks(FromNow(Seconds(123))));

    LazyNow lazy_now(&clock_);
    thread_controller_.SetNextDelayedDoWork(&lazy_now, WakeUp { FromNow(Seconds(123)) });
}
...
{
    MockCallback<OnceClosure> task1;
    task_source_.AddTask(FROM_HERE, task1.Get(), FromNow(Days(10)), clock_.NowTicks());
...
TEST_F(ThreadControllerWithMessagePumpTest, NestedExecution)
{
    ...
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    task_source_.AddTask(FROM_HERE,
        base::BindOnce(
            [](std::vector<std::string>* log, ThreadControllerForTest* controller) {
    ...
    EXPECT_THAT(log,
        ElementsAre("entering top-level runloop", "task1", "entering nested runloop", "exiting nested runloop", "task2", "task3", "exiting top-level runloop"));
    testing::Mock::VerifyAndClearExpectations(message_pump_);
}

TEST_F(ThreadControllerWithMessagePumpTest, NestedExecutionWithApplicationTasks)
{
    ...
    EXPECT_THAT(log,
        ElementsAre("entering top-level runloop", "task1", "entering nested runloop", "task2", "task3", "exiting nested runloop", "exiting top-level runloop"));
    testing::Mock::VerifyAndClearExpectations(message_pump_);
}

TEST_F(ThreadControllerWithMessagePumpTest, SetDefaultTaskRunner)
{
    scoped_refptr<SingleThreadTaskRunner> task_runner1 = MakeRefCounted<FakeTaskRunner>();
    ...
    scoped_refptr<SingleThreadTaskRunner> task_runner2 = MakeRefCounted<FakeTaskRunner>();
...
TEST_F(ThreadControllerWithMessagePumpTest, WorkBatching)
{
    ...
    int task_count = 0;
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        EXPECT_EQ(delegate->DoWork().delayed_run_time, TimeTicks::Max());
        EXPECT_EQ(5, task_count);
    }));

    for (int i = 0; i < kBatchSize; i++) {
        task_source_.AddTask(FROM_HERE, BindLambdaForTesting([&] { task_count++; }), TimeTicks());
    }
...
TEST_F(ThreadControllerWithMessagePumpTest, EarlyQuit)
{
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([this](MessagePump::Delegate* delegate) {
        EXPECT_EQ(delegate, &thread_controller_);
        EXPECT_TRUE(delegate->DoWork().is_immediate());
        EXPECT_EQ(delegate->DoWork().delayed_run_time, TimeTicks::Max());
    }));
    ...
    task_source_.AddTask(FROM_HERE, base::BindOnce([](std::vector<std::string>* log) { log->push_back("task1"); }, &log), TimeTicks());
    task_source_.AddTask(FROM_HERE, base::BindOnce([](std::vector<std::string>* log) { log->push_back("task2"); }, &log), TimeTicks());
...
{
    MockCallback<OnceClosure> task1;
    task_source_.AddTask(FROM_HERE, task1.Get(), FromNow(Seconds(5)), clock_.NowTicks());
    MockCallback<OnceClosure> task2;
    task_source_.AddTask(FROM_HERE, task2.Get(), FromNow(Seconds(10)), clock_.NowTicks());
    MockCallback<OnceClosure> task3;
    task_source_.AddTask(FROM_HERE, task3.Get(), FromNow(Seconds(20)), clock_.NowTicks());

    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate*) {
        clock_.Advance(Seconds(5));
        EXPECT_CALL(task1, Run()).Times(1);
...
TEST_F(ThreadControllerWithMessagePumpTest, SetHighResolutionTimer)
{
    ...
    task_source_.AddTask(FROM_HERE, task.Get(), FromNow(Seconds(5)), clock_.NowTicks());

    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());

    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        // Should initially not be in high resolution.
        EXPECT_FALSE(thread_controller_.MainThreadOnlyForTesting().in_high_res_mode);

        // Ensures timer resolution is set to high resolution.
        task_source_.SetHasPendingHighResolutionTasks(true);
        delegate->DoIdleWork();
        EXPECT_TRUE(thread_controller_.MainThreadOnlyForTesting().in_high_res_mode);
...
#if BUILDFLAG(IS_WIN)
TEST_F(ThreadControllerWithMessagePumpTest, SetHighResolutionTimerWithPowerSuspend)
{
    MockCallback<OnceClosure> task;
    task_source_.AddTask(FROM_HERE, task.Get(), FromNow(Seconds(5)), clock_.NowTicks());
...
TEST_F(ThreadControllerWithMessagePumpTest, ScheduleDelayedWorkWithPowerSuspend)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    task_source_.AddTask(FROM_HERE, task1.Get(), FromNow(Seconds(10)), clock_.NowTicks());
    MockCallback<OnceClosure> task2;
    task_source_.AddTask(FROM_HERE, task2.Get(), FromNow(Seconds(15)), clock_.NowTicks());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveSingleApplicationTask)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        for (int i = 0; i < 5; ++i, first_pass = false) {
            ...
            if (!first_pass) {
                ...
                EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kPumpOverhead));
            }
            EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kSelectingApplicationTask));
            EXPECT_CALL(task, Run());
            EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kApplicationTask));
            EXPECT_EQ(thread_controller_.DoWork().delayed_run_time, TimeTicks::Max());

            testing::Mock::VerifyAndClearExpectations(&*thread_controller_.trace_observer_);

            EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kIdleWork));
            EXPECT_CALL(*thread_controller_.trace_observer_, OnThreadControllerActiveEnd);
            thread_controller_.DoIdleWork();
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveMultipleApplicationTasks)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        for (size_t i = 0; i < std::size(tasks); ++i) {
            const TimeTicks expected_delayed_run_time = i < std::size(tasks) - 1 ? TimeTicks() : TimeTicks::Max();
            ...
            EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kApplicationTask));
            EXPECT_EQ(thread_controller_.DoWork().delayed_run_time, expected_delayed_run_time);
        }
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveWakeUpForNothing)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, DoWorkBatches)
{
    ...
    int task_counter = 0;
    for (int i = 0; i < 2; i++) {
        task_source_.AddTask(FROM_HERE, BindLambdaForTesting([&] { task_counter++; }), TimeTicks());
    }
...
#define MAYBE_ThreadControllerActiveAdvancedNesting DISABLED_ThreadControllerActiveAdvancedNesting
#else
#define MAYBE_ThreadControllerActiveAdvancedNesting ThreadControllerActiveAdvancedNesting
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveAdvancedNesting)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        EXPECT_CALL(tasks[0], Run()).WillOnce(Invoke([&] {
            ...
            EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
                ...
                EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kNested));
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveNestedNativeLoop DISABLED_ThreadControllerActiveNestedNativeLoop
#else
#define MAYBE_ThreadControllerActiveNestedNativeLoop ThreadControllerActiveNestedNativeLoop
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveNestedNativeLoop)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveUnusedNativeLoop DISABLED_ThreadControllerActiveUnusedNativeLoop
#else
#define MAYBE_ThreadControllerActiveUnusedNativeLoop ThreadControllerActiveUnusedNativeLoop
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveUnusedNativeLoop)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveNestedNativeLoopWithoutAllowance DISABLED_ThreadControllerActiveNestedNativeLoopWithoutAllowance
#else
#define MAYBE_ThreadControllerActiveNestedNativeLoopWithoutAllowance ThreadControllerActiveNestedNativeLoopWithoutAllowance
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveNestedNativeLoopWithoutAllowance)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask DISABLED_ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask
#else
#define MAYBE_ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveNativeLoopsReachingIdle DISABLED_ThreadControllerActiveNativeLoopsReachingIdle
#else
#define MAYBE_ThreadControllerActiveNativeLoopsReachingIdle ThreadControllerActiveNativeLoopsReachingIdle
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveNativeLoopsReachingIdle)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// TODO(https://crbug.com/341965228): Deflake and re-enable.
#if BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ThreadControllerActiveQuitNestedWhileApplicationIdle DISABLED_ThreadControllerActiveQuitNestedWhileApplicationIdle
#else
#define MAYBE_ThreadControllerActiveQuitNestedWhileApplicationIdle ThreadControllerActiveQuitNestedWhileApplicationIdle
#endif
TEST_F(ThreadControllerWithMessagePumpTest, MAYBE_ThreadControllerActiveQuitNestedWhileApplicationIdle)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
// case as well.
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveNestedWithinNativeAllowsApplicationTasks)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        EXPECT_CALL(*thread_controller_.trace_observer_, OnThreadControllerActiveBegin).WillOnce(Invoke([&] {
            // C:
            EXPECT_TRUE(thread_controller_.IsTaskExecutionAllowed());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveDummyScopedAllowApplicationTasks)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, MessagePumpPhasesWithQueuingTime)
{
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        EXPECT_CALL(*thread_controller_.trace_observer_, OnThreadControllerActiveBegin);
        EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kScheduled));
        EXPECT_CALL(*thread_controller_.trace_observer_, OnPhaseRecorded(ThreadController::kSelectingApplicationTask));
...
TEST_F(ThreadControllerWithMessagePumpNoBatchesTest, WorkIdIncrementedForEveryWorkItem)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce([&](MessagePump::Delegate* delegate) {
...
TEST_F(ThreadControllerWithMessagePumpTest, WorkIdIncrementedForEveryWorkItemInBatches)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, WorkIdIncrementedScopedDoWorkItem)
{
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce([&](MessagePump::Delegate* delegate) {
        MessagePump::Delegate::ScopedDoWorkItem scoped_do_work_item = delegate->BeginWorkItem();
...
TEST_F(ThreadControllerWithMessagePumpTest, WorkIdIncrementedDelegateBeforeWait)
{
    SingleThreadTaskRunner::CurrentDefaultHandle handle(MakeRefCounted<FakeTaskRunner>());
    WorkIdProvider* work_id_provider = WorkIdProvider::GetForCurrentThread();
...
TEST_F(ThreadControllerWithMessagePumpTest, WorkIdIncrementedDelegateRun)
{
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce([&](MessagePump::Delegate* delegate) { EXPECT_EQ(work_id_provider->GetWorkId(), 0u); });
