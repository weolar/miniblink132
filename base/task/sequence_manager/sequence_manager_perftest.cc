...
{
    perf_test::PerfResultReporter reporter(kMetricPrefixSequenceManager, story_name);
...
public:
    ...
    bool MaybeFastForwardToWakeUp(std::optional<WakeUp> wake_up, bool quit_when_idle_requested) override
    {
        if (wake_up) {
...
public:
    ...
    bool VirtualTimeIsSupported() const override
    {
        return true;
    }

    bool MultipleQueuesSupported() const override
    {
        return true;
    }
    ...
    {
        owned_task_queues_.push_back(manager_->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ)));
    ...
    SequenceManager* GetManager() const
    {
        return manager_.get();
    }
...
class SequenceManagerWithMessagePumpPerfTestDelegate : public BaseSequenceManagerPerfTestDelegate {
public:
    SequenceManagerWithMessagePumpPerfTestDelegate(const char* name, MessagePumpType type, bool randomised_sampling_enabled = false)
        : name_(name)
    {
        auto settings = SequenceManager::Settings::Builder().SetRandomisedSamplingEnabled(randomised_sampling_enabled).Build();
        SetSequenceManager(SequenceManagerForTest::Create(
            std::make_unique<internal::ThreadControllerWithMessagePumpImpl>(MessagePump::Create(type), settings), std::move(settings)));

        // ThreadControllerWithMessagePumpImpl doesn't provide a default task
        // runner.
        default_task_queue_ = GetManager()->CreateTaskQueue(TaskQueue::Spec(QueueName::DEFAULT_TQ));
        GetManager()->SetDefaultTaskRunner(default_task_queue_->task_runner());
    }

    ~SequenceManagerWithMessagePumpPerfTestDelegate() override
    {
        ShutDown();
    }
...
public:
    SingleThreadInThreadPoolPerfTestDelegate()
        : done_cond_(&done_lock_)
    {
        ThreadPoolInstance::Set(std::make_unique<::base::internal::ThreadPoolImpl>("Test"));
    ...
    scoped_refptr<TaskRunner> CreateTaskRunner() override
    {
        return ThreadPool::CreateSingleThreadTaskRunner({ TaskPriority::USER_BLOCKING });
    }
...
public:
    ...
    explicit TestCase(PerfTestDelegate* delegate)
        : delegate_(delegate)
    {
    }
...
public:
    SameThreadTaskSource(std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
        : num_queues_(task_runners.size())
        , num_tasks_(num_tasks)
        , task_closure_(BindRepeating(&SameThreadTaskSource::TestTask, Unretained(this)))
...
protected:
    ...
    void TestTask()
    {
        ...
        for (unsigned int i = 0; i < max_tasks_to_post && num_tasks_in_flight_ < max_tasks_in_flight_ && num_tasks_to_post_ > 0; i++) {
...
public:
    CrossThreadTaskSource(std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
        : num_queues_(task_runners.size())
        , num_tasks_(num_tasks)
        , task_closure_(BindRepeating(&CrossThreadTaskSource::TestTask, Unretained(this)))
        , task_runners_(std::move(task_runners))
    {
    }
    ...
    void Start() override
    {
        ...
        for (size_t i = 0; i < num_tasks_; i++) {
            while (num_tasks_in_flight_.load(std::memory_order_acquire) > max_tasks_in_flight_) {
...
public:
    SingleThreadImmediateTestCase(PerfTestDelegate* delegate, std::vector<scoped_refptr<TaskRunner>> task_runners)
        : TestCase(delegate)
        , task_source_(std::make_unique<SingleThreadImmediateTaskSource>(delegate, std::move(task_runners), kNumTasks))
    {
    }
...
private:
    ...
    public:
        SingleThreadImmediateTaskSource(PerfTestDelegate* delegate, std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
...
public:
    SingleThreadDelayedTestCase(PerfTestDelegate* delegate, std::vector<scoped_refptr<TaskRunner>> task_runners)
        : TestCase(delegate)
        , task_source_(std::make_unique<SingleThreadDelayedTaskSource>(delegate, std::move(task_runners), kNumTasks))
    {
    }
...
private:
    ...
    public:
        explicit SingleThreadDelayedTaskSource(PerfTestDelegate* delegate, std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
            : SameThreadTaskSource(std::move(task_runners), num_tasks)
            , delegate_(delegate)
        {
        }
        ...
        void PostTask(unsigned int queue) override
        {
            ...
            task_runners_[queue]->PostDelayedTask(FROM_HERE, task_closure_, Milliseconds(delay));
        }
...
public:
    TwoThreadTestCase(PerfTestDelegate* delegate, std::vector<scoped_refptr<TaskRunner>> task_runners)
        : TestCase(delegate)
        , task_runners_(std::move(task_runners))
    ...
    ~TwoThreadTestCase() override
    {
        auxiliary_thread_.Stop();
    }
...
protected:
    ...
    {
        done_count_ = 0;
        same_thread_task_source_ = std::make_unique<SingleThreadImmediateTaskSource>(this, task_runners_, num_tasks_ / 2);
        cross_thread_task_scorce_ = std::make_unique<CrossThreadImmediateTaskSource>(this, task_runners_, num_tasks_ / 2);

        auxiliary_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&CrossThreadImmediateTaskSource::Start, Unretained(cross_thread_task_scorce_.get())));
        same_thread_task_source_->Start();
    }

    class SingleThreadImmediateTaskSource : public SameThreadTaskSource {
    public:
        SingleThreadImmediateTaskSource(TwoThreadTestCase* two_thread_test_case, std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
            : SameThreadTaskSource(std::move(task_runners), num_tasks)
        ...
        void SignalDone() override
        {
            two_thread_test_case_->SignalDone();
        }
    ...
    public:
        CrossThreadImmediateTaskSource(TwoThreadTestCase* two_thread_test_case, std::vector<scoped_refptr<TaskRunner>> task_runners, size_t num_tasks)
...
public:
    ...
    std::unique_ptr<PerfTestDelegate> CreateDelegate()
    {
        ...
        case PerfTestType::kUseSequenceManagerWithMessagePump:
            return std::make_unique<SequenceManagerWithMessagePumpPerfTestDelegate>(" SequenceManager with MessagePumpDefault ", MessagePumpType::DEFAULT);

        case PerfTestType::kUseSequenceManagerWithUIMessagePump:
            return std::make_unique<SequenceManagerWithMessagePumpPerfTestDelegate>(" SequenceManager with MessagePumpForUI ", MessagePumpType::UI);

        case PerfTestType::kUseSequenceManagerWithIOMessagePump:
            return std::make_unique<SequenceManagerWithMessagePumpPerfTestDelegate>(" SequenceManager with MessagePumpForIO ", MessagePumpType::IO);
        ...
        case PerfTestType::kUseSequenceManagerWithMessagePumpAndRandomSampling:
            return std::make_unique<SequenceManagerWithMessagePumpPerfTestDelegate>(
                " SequenceManager with MessagePumpDefault and random sampling ", MessagePumpType::DEFAULT, true);
    ...
    bool ShouldMeasureQueueScaling() const
    {
        ...
        return delegate_->MultipleQueuesSupported() && GetParam() == PerfTestType::kUseSequenceManagerWithUIMessagePump;
    }
    ...
    void Benchmark(const std::string& story_prefix, TestCase* TestCase)
    {
        ...
        reporter.AddResult(kMetricPostTimePerTask, (now - start).InMicroseconds() / static_cast<double>(kNumTasks));
    }

    std::unique_ptr<PerfTestDelegate> delegate_;
};

INSTANTIATE_TEST_SUITE_P(All, SequenceManagerPerfTest,
    testing::Values(PerfTestType::kUseSequenceManagerWithMessagePump, PerfTestType::kUseSequenceManagerWithUIMessagePump,
        PerfTestType::kUseSequenceManagerWithIOMessagePump, PerfTestType::kUseSingleThreadInThreadPool,
...
TEST_P(SequenceManagerPerfTest, PostDelayedTasks_ThirtyTwoQueues)
{
    ...
    SingleThreadDelayedTestCase task_source(delegate_.get(), CreateTaskRunners(32));
...
{
    SingleThreadImmediateTestCase task_source(delegate_.get(), CreateTaskRunners(1));
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasks_FourQueues)
{
    ...
    SingleThreadImmediateTestCase task_source(delegate_.get(), CreateTaskRunners(4));
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasks_EightQueues)
{
    ...
    SingleThreadImmediateTestCase task_source(delegate_.get(), CreateTaskRunners(8));
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasks_ThirtyTwoQueues)
{
    ...
    SingleThreadImmediateTestCase task_source(delegate_.get(), CreateTaskRunners(32));
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasksFromTwoThreads_OneQueue)
{
    ...
    Benchmark("post immediate tasks with one queue from two threads", &task_source);
}
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasksFromTwoThreads_FourQueues)
{
    ...
    Benchmark("post immediate tasks with four queues from two threads", &task_source);
}
...
TEST_P(SequenceManagerPerfTest, PostImmediateTasksFromTwoThreads_EightQueues)
{
    ...
    Benchmark("post immediate tasks with eight queues from two threads", &task_source);
}

TEST_P(SequenceManagerPerfTest, PostImmediateTasksFromTwoThreads_ThirtyTwoQueues)
{
    if (!ShouldMeasureQueueScaling()) {
        LOG(INFO) << "Unsupported";
        return;
    ...
    Benchmark("post immediate tasks with thirty two queues from two threads", &task_source);
}
