...
public:
    ...
    virtual void SetNextDelayedDoWork(LazyNow* lazy_now, std::optional<WakeUp> wake_up) = 0;
    ...
    virtual void BindToCurrentThread(std::unique_ptr<MessagePump> message_pump) = 0;
    ...
    static void InitializeFeatures(features::EmitThreadControllerProfilerMetadata emit_profiler_metadata);
    ...
    void EnableMessagePumpTimeKeeperMetrics(const char* thread_name, bool wall_time_based_metrics_enabled_for_testing);
...
protected:
    ...
    public:
        ...
        void EnableTimeKeeperMetrics(const char* thread_name, bool wall_time_based_metrics_enabled_for_testing);
        ...
        static void SetTraceObserverForTesting(TraceObserverForTesting* trace_observer_for_testing);
    ...
    private:
        ...
        public:
            ...
            void EnableRecording(const char* thread_name, bool wall_time_based_metrics_enabled_for_testing);
        ...
        private:
            ...
            // as needed.
            void RecordTimeInPhase(Phase phase, TimeTicks phase_begin, TimeTicks phase_end);
        ...
        class RunLevel {
        public:
            RunLevel(State initial_state, bool is_nested, TimeKeeper& time_keeper, LazyNow& lazy_now);
            ~RunLevel();
        ...
        private:
            ...
            void LogPercentageMetric(const char* name, int value, base::TimeDelta interval_duration);
            void LogIntervalMetric(const char* name, base::TimeDelta value, base::TimeDelta interval_duration);
            ...
            public:
                ...
                TruePostMove(TruePostMove&& other)
                {
                    other.was_moved_ = true;
                }
        ...
        std::stack<RunLevel, std::vector<RunLevel>> run_levels_ GUARDED_BY_CONTEXT(outer_->associated_thread_->thread_checker);
