...
BASE_FEATURE(kAvoidScheduleWorkDuringNativeEventProcessing, "AvoidScheduleWorkDuringNativeEventProcessing", base::FEATURE_DISABLED_BY_DEFAULT);
...
{
    g_run_tasks_by_batches.store(FeatureList::IsEnabled(base::kRunTasksByBatches), std::memory_order_relaxed);
    g_avoid_schedule_calls_during_native_event_processing.store(
        FeatureList::IsEnabled(kAvoidScheduleWorkDuringNativeEventProcessing), std::memory_order_relaxed);
}

// static
void ThreadControllerWithMessagePumpImpl::ResetFeatures()
{
    g_run_tasks_by_batches.store(base::kRunTasksByBatches.default_state == FEATURE_ENABLED_BY_DEFAULT, std::memory_order_relaxed);
}

ThreadControllerWithMessagePumpImpl::ThreadControllerWithMessagePumpImpl(const SequenceManager::Settings& settings)
...
ThreadControllerWithMessagePumpImpl::ThreadControllerWithMessagePumpImpl(std::unique_ptr<MessagePump> message_pump, const SequenceManager::Settings& settings)
...
// static
std::unique_ptr<ThreadControllerWithMessagePumpImpl> ThreadControllerWithMessagePumpImpl::CreateUnbound(const SequenceManager::Settings& settings)
{
...
void ThreadControllerWithMessagePumpImpl::SetSequencedTaskSource(SequencedTaskSource* task_source)
{
    DCHECK(task_source);
...
void ThreadControllerWithMessagePumpImpl::BindToCurrentThread(std::unique_ptr<MessagePump> message_pump)
{
    ...
    scoped_set_sequence_local_storage_map_for_current_thread_
        = std::make_unique<base::internal::ScopedSetSequenceLocalStorageMapForCurrentThread>(&sequence_local_storage_map_);
    {
...
void ThreadControllerWithMessagePumpImpl::SetWorkBatchSize(int work_batch_size)
{
...
void ThreadControllerWithMessagePumpImpl::WillQueueTask(PendingTask* pending_task)
{
...
void ThreadControllerWithMessagePumpImpl::SetNextDelayedDoWork(LazyNow* lazy_now, std::optional<WakeUp> wake_up)
{
    ...
    TimeTicks run_time = wake_up.has_value() ? pump_->AdjustDelayedRunTime(wake_up->earliest_time(), wake_up->time, wake_up->latest_time()) : TimeTicks::Max();
    ...
    // task.
    pump_->ScheduleDelayedWork({ run_time, GetLeewayForWakeUp(wake_up), lazy_now->Now() });
}
...
void ThreadControllerWithMessagePumpImpl::SetDefaultTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner)
{
...
void ThreadControllerWithMessagePumpImpl::InitializeSingleThreadTaskRunnerCurrentDefaultHandle()
{
    ...
    main_thread_only().thread_task_runner_handle = std::make_unique<SingleThreadTaskRunner::CurrentDefaultHandle>(task_runner_);
...
scoped_refptr<SingleThreadTaskRunner> ThreadControllerWithMessagePumpImpl::GetDefaultTaskRunner()
{
    base::internal::CheckedAutoLock lock(task_runner_lock_);
    return task_runner_;
}
...
void ThreadControllerWithMessagePumpImpl::AddNestingObserver(RunLoop::NestingObserver* observer)
{
...
void ThreadControllerWithMessagePumpImpl::RemoveNestingObserver(RunLoop::NestingObserver* observer)
{
...
void ThreadControllerWithMessagePumpImpl::OnBeginWorkItemImpl(LazyNow& lazy_now)
{
...
MessagePump::Delegate::NextWorkInfo ThreadControllerWithMessagePumpImpl::DoWork()
{
    ...
    if (RunsTasksByBatches()
        || (!main_thread_only().yield_to_native_after_batch.is_null() && continuation_lazy_now.Now() < main_thread_only().yield_to_native_after_batch)) {
    ...
    WorkDeduplicator::NextTask next_task
        = (next_wake_up && next_wake_up->is_immediate()) ? WorkDeduplicator::NextTask::kIsImmediate : WorkDeduplicator::NextTask::kIsDelayed;
    ...
    TimeTicks next_delayed_do_work = pump_->AdjustDelayedRunTime(next_wake_up->earliest_time(), next_wake_up->time, next_wake_up->latest_time());
    ...
    next_work_info.delayed_run_time = CapAtOneDay(next_delayed_do_work, &continuation_lazy_now);
    next_work_info.leeway = GetLeewayForWakeUp(next_wake_up);
    next_work_info.recent_now = continuation_lazy_now.Now();
    return next_work_info;
}

std::optional<WakeUp> ThreadControllerWithMessagePumpImpl::DoWorkImpl(LazyNow* continuation_lazy_now)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "ThreadControllerImpl::DoWork");
    ...
    // run by batches.
    const base::TimeDelta batch_duration = RunsTasksByBatches() ? base::Milliseconds(8) : base::Milliseconds(0);

    const std::optional<base::TimeTicks> start_time = batch_duration.is_zero() ? std::nullopt : std::optional<base::TimeTicks>(time_source_->NowTicks());
    std::optional<base::TimeTicks> recent_time = start_time;

    // Loops for |batch_duration|, or |work_batch_size| times if |batch_duration|
    // is zero.
    for (int num_tasks_executed = 0; (!batch_duration.is_zero() && (recent_time.value() - start_time.value()) < batch_duration)
         || (batch_duration.is_zero() && num_tasks_executed < main_thread_only().work_batch_size);
         ++num_tasks_executed) {
        ...
        const SequencedTaskSource::SelectTaskOption select_task_option = power_monitor_.IsProcessInPowerSuspendState()
        ...
        run_level_tracker_.OnApplicationTaskSelected(
            (selected_task && selected_task->task.delayed_run_time.is_null()) ? selected_task->task.queue_time : TimeTicks(), lazy_now_task_selected);
        if (!selected_task) {
        ...
        AutoReset<bool> ban_nested_application_tasks(&main_thread_only().task_execution_allowed, false);
...

        {
            ...
            TaskAnnotator::LongTaskTracker long_task_tracker(time_source_, selected_task->task, &task_annotator_, lazy_now_task_selected.Now());
            ...
            task_annotator_.RunTask("ThreadControllerImpl::RunTask", selected_task->task, [&selected_task, &source](perfetto::EventContext& ctx) {
                if (selected_task->task_execution_trace_logger) {
                    selected_task->task_execution_trace_logger.Run(ctx, selected_task->task);
                }
    ...
    const SequencedTaskSource::SelectTaskOption select_task_option = power_monitor_.IsProcessInPowerSuspendState()
        ? SequencedTaskSource::SelectTaskOption::kSkipDelayedTask
        : SequencedTaskSource::SelectTaskOption::kDefault;
    return main_thread_only().task_source->GetPendingWakeUp(continuation_lazy_now, select_task_option);
}

bool ThreadControllerWithMessagePumpImpl::RunsTasksByBatches() const
{
    return can_run_tasks_by_batches_ && g_run_tasks_by_batches.load(std::memory_order_relaxed);
}
...
void ThreadControllerWithMessagePumpImpl::DoIdleWork()
{
    ...
    public:
        OnIdle(const TickClock* time_source, RunLevelTracker& run_level_tracker_ref)
            : lazy_now(time_source)
            , run_level_tracker(run_level_tracker_ref)
        {
        }
        ...
        ~OnIdle()
        {
            run_level_tracker.OnIdle(lazy_now);
        }
    ...
    if (!power_monitor_.IsProcessInPowerSuspendState()) {
        ...
        const bool need_high_res_mode = main_thread_only().task_source->HasPendingHighResolutionTasks();
    ...
    if (main_thread_only().quit_runloop_after != TimeTicks::Max() && main_thread_only().quit_runloop_after <= on_idle->lazy_now.Now()) {
        Quit();
        return;
    }
...
void ThreadControllerWithMessagePumpImpl::Run(bool application_tasks_allowed, TimeDelta timeout)
{
    ...
    AutoReset<TimeTicks> quit_runloop_after(
        &main_thread_only().quit_runloop_after, (timeout == TimeDelta::Max()) ? TimeTicks::Max() : lazy_now_run_loop_start.Now() + timeout);

    run_level_tracker_.OnRunLoopStarted(RunLevelTracker::kInBetweenWorkItems, lazy_now_run_loop_start);
...
void ThreadControllerWithMessagePumpImpl::SetTaskExecutionAllowedInNativeNestedLoop(bool allowed)
{
    if (allowed) {
...
void ThreadControllerWithMessagePumpImpl::PrioritizeYieldingToNative(base::TimeTicks prioritize_until)
{
