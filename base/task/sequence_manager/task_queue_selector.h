...
public:
    ...
    TaskQueueSelector(scoped_refptr<const AssociatedThreadId> associated_thread, const SequenceManager::Settings& settings);
    ...
    // on the main thread.
    void AddQueue(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    ...
    void SetQueuePriority(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    ...
    WorkQueue* SelectWorkQueueToService(SelectTaskOption option = SelectTaskOption::kDefault);
    ...
    // O(1).
    std::optional<TaskQueue::QueuePriority> GetHighestPendingPriority(SelectTaskOption option = SelectTaskOption::kDefault) const;
    ...
    void CollectSkippedOverLowerPriorityTasks(const internal::WorkQueue* selected_work_queue, std::vector<const Task*>* result) const;
...
protected:
    ...
    private:
        static_assert(SequenceManager::PrioritySettings::kMaxPriorities < sizeof(size_t) * 8,
   ...
   */
    ...
    struct SetOperationOldest {
        static std::optional<WorkQueueAndTaskOrder> GetWithPriority(const WorkQueueSets& sets, TaskQueue::QueuePriority priority)
        {
    ...
    template <typename SetOperation> WorkQueue* ChooseWithPriority(TaskQueue::QueuePriority priority) const
    {
    ...
    template <typename SetOperation> WorkQueue* ChooseImmediateOnlyWithPriority(TaskQueue::QueuePriority priority) const
    {
        if (auto queue_and_order = SetOperation::GetWithPriority(immediate_work_queue_sets_, priority)) {
            return queue_and_order->queue;
        }
        return nullptr;
    }

    template <typename SetOperation> WorkQueue* ChooseDelayedOnlyWithPriority(TaskQueue::QueuePriority priority) const
    {
        if (auto queue_and_order = SetOperation::GetWithPriority(delayed_work_queue_sets_, priority)) {
            return queue_and_order->queue;
        }
        return nullptr;
    }
...
private:
    size_t priority_count() const
    {
        return non_empty_set_counts_.size();
    }

    void ChangeSetIndex(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    void AddQueueImpl(internal::TaskQueueImpl* queue, TaskQueue::QueuePriority priority);
    ...
    template <typename SetOperation> WorkQueue* ChooseImmediateOrDelayedTaskWithPriority(TaskQueue::QueuePriority priority) const
    {
        if (auto immediate_queue_and_order = SetOperation::GetWithPriority(immediate_work_queue_sets_, priority)) {
            if (auto delayed_queue_and_order = SetOperation::GetWithPriority(delayed_work_queue_sets_, priority)) {
                return immediate_queue_and_order->order < delayed_queue_and_order->order ? immediate_queue_and_order->queue : delayed_queue_and_order->queue;
            }
