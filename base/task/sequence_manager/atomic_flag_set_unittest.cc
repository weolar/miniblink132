...
public:
    explicit AtomicFlagSetForTest(scoped_refptr<AssociatedThreadId> associated_thread)
...
public:
    void CreateFlags(size_t number_of_flags, RepeatingCallback<void(size_t index)> callback)
    {
        atomic_flags_.reserve(number_of_flags);
        for (size_t i = 0; i < number_of_flags; i++) {
            atomic_flags_.push_back(
                atomic_flag_set_.AddFlag(base::BindRepeating([](RepeatingCallback<void(size_t index)> callback, size_t i) { callback.Run(i); }, callback, i)));
        }
    }
...
TEST_F(AtomicFlagSetTest, SetActiveOneFlag)
{
    ...
    CreateFlags(3, BindLambdaForTesting([&](size_t index) { flags_visited.push_back(index); }));
...
TEST_F(AtomicFlagSetTest, SetActiveManyFlags)
{
    ...
    CreateFlags(num_flags, BindLambdaForTesting([&](size_t index) { flags_visited.insert(index); }));
...
{
    CreateFlags(AtomicFlagSetForTest::Group::kNumFlags - 1, BindLambdaForTesting([](size_t index) {}));
    AtomicFlagSetForTest::Group* group1 = atomic_flag_set_.GetAllocListForTesting();
...
{
    CreateFlags(AtomicFlagSetForTest::Group::kNumFlags * 2 + 1, BindLambdaForTesting([](size_t index) {}));
...
TEST_F(AtomicFlagSetTest, GroupBecomesEmptyMiddleOfPartiallyFreeList)
{
    ...
    for (size_t i = AtomicFlagSetForTest::Group::kNumFlags; i < AtomicFlagSetForTest::Group::kNumFlags * 2; i++) {
        atomic_flags_[i].ReleaseAtomicFlag();
    }
    EXPECT_EQ(group3, atomic_flag_set_.GetPartiallyFreeListForTesting());
    EXPECT_EQ(group3->partially_free_list_prev, nullptr);
