...
SyncWorkAuthorization& SyncWorkAuthorization::operator=(SyncWorkAuthorization&& other)
{
    tracker_ = other.tracker_;
    other.tracker_ = nullptr;
    return *this;
...
SyncWorkAuthorization::~SyncWorkAuthorization()
{
...

    {
        ...
        uint32_t prev = tracker_->state_.fetch_and(~WorkTracker::kActiveSyncWork, WorkTracker::kMemoryReleaseAllowWork);
...
void WorkTracker::SetRunTaskSynchronouslyAllowed(bool can_run_tasks_synchronously)
{
    ...
    if (can_run_tasks_synchronously) {
        ...
        uint32_t prev = state_.fetch_and(~kSyncWorkSupported, kMemoryAcquireBeforeWork);
...
void WorkTracker::WillRequestReloadImmediateWorkQueue()
{
    ...
    // `OnIdle()` are called.
    state_.fetch_or(kImmediateWorkQueueNeedsReload, kMemoryRelaxedNotAllowOrBeforeWork);
}
...
void WorkTracker::WillReloadImmediateWorkQueues()
{
    ...
    state_.fetch_and(~(kImmediateWorkQueueNeedsReload | kWorkQueuesEmptyAndNoWorkRunning), kMemoryRelaxedNotAllowOrBeforeWork);
}
...
void WorkTracker::OnBeginWork()
{
    ...
    uint32_t prev = state_.fetch_and(~kWorkQueuesEmptyAndNoWorkRunning, kMemoryAcquireBeforeWork);
    if (prev & kActiveSyncWork) {
...
SyncWorkAuthorization WorkTracker::TryAcquireSyncWorkAuthorization()
{
    ...
    if (state == (kSyncWorkSupported | kWorkQueuesEmptyAndNoWorkRunning)
        && state_.compare_exchange_strong(state, state | kActiveSyncWork, std::memory_order_acquire, std::memory_order_relaxed)) {
...
{
    CHECK(!(state_.load(std::memory_order_relaxed) & kWorkQueuesEmptyAndNoWorkRunning));
}
