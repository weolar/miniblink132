...
//
class BASE_EXPORT SequenceManagerImpl : public SequenceManager,
...
public:
    ...
    void SetDefaultTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner) override;
    ...
    std::optional<SelectedTask> SelectNextTask(LazyNow& lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) override;
    void DidRunTask(LazyNow& lazy_now) override;
    std::optional<WakeUp> GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) override;
    bool HasPendingHighResolutionTasks() override;
    void OnBeginWork() override;
    bool OnIdle() override;
    void MaybeEmitTaskDetails(perfetto::EventContext& ctx, const SequencedTaskSource::SelectedTask& selected_task) const override;

    void AddDestructionObserver(CurrentThread::DestructionObserver* destruction_observer);
    void RemoveDestructionObserver(CurrentThread::DestructionObserver* destruction_observer);
    [[nodiscard]] CallbackListSubscription RegisterOnNextIdleCallback(OnceClosure on_next_idle_callback);
    ...
    void EnableMessagePumpTimeKeeperMetrics(const char* thread_name, bool wall_time_based_metrics_enabled_for_testing = false);
    ...
    void UnregisterTaskQueueImpl(std::unique_ptr<internal::TaskQueueImpl> task_queue);
    ...
    const Settings& settings() const LIFETIME_BOUND
    {
        return settings_;
    }
...
protected:
    static std::unique_ptr<ThreadControllerImpl> CreateThreadControllerImplForCurrentThread(const TickClock* clock);

    // Create a task queue manager where |controller| controls the thread
    // on which the tasks are eventually run.
    SequenceManagerImpl(std::unique_ptr<internal::ThreadController> controller, SequenceManager::Settings settings = Settings());
...
private:
    ...
    friend std::unique_ptr<SequenceManager> sequence_manager::CreateSequenceManagerOnCurrentThread(SequenceManager::Settings);
    friend std::unique_ptr<SequenceManager> sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(
        std::unique_ptr<MessagePump> message_pump, SequenceManager::Settings);
    friend std::unique_ptr<SequenceManager> sequence_manager::CreateUnboundSequenceManager(SequenceManager::Settings);
    friend std::unique_ptr<SequenceManagerImpl> sequence_manager::internal::CreateUnboundSequenceManagerImpl(
        PassKey<base::internal::SequenceManagerThreadDelegate>, SequenceManager::Settings);
    ...
    static std::unique_ptr<SequenceManagerImpl> CreateOnCurrentThread(SequenceManager::Settings settings);
    ...
    // methods.
    static std::unique_ptr<SequenceManagerImpl> CreateUnbound(SequenceManager::Settings settings);
    ...
    struct ExecutingTask {
        ExecutingTask(Task&& task, internal::TaskQueueImpl* task_queue, TaskQueue::TaskTiming task_timing)
    ...
    struct MainThreadOnly {
        explicit MainThreadOnly(SequenceManagerImpl* sequence_manager, const scoped_refptr<AssociatedThreadId>& associated_thread,
        ...
        std::map<internal::TaskQueueImpl*, std::unique_ptr<internal::TaskQueueImpl>> queues_to_delete;
        ...
        ObserverList<CurrentThread::DestructionObserver>::UncheckedAndDanglingUntriaged destruction_observers;
    ...
    std::unique_ptr<trace_event::ConvertableToTraceFormat> AsValueWithSelectorResultForTracing(
        internal::WorkQueue* selected_work_queue, bool force_verbose) const;
    Value::Dict AsValueWithSelectorResult(internal::WorkQueue* selected_work_queue, bool force_verbose) const;
    ...
    // queue's destruction.
    AtomicFlagSet::AtomicFlag GetFlagToRequestReloadForEmptyQueue(TaskQueueImpl* task_queue);
    ...
    std::unique_ptr<internal::TaskQueueImpl> CreateTaskQueueImpl(const TaskQueue::Spec& spec);
    ...
    TaskQueue::TaskTiming::TimeRecordingPolicy ShouldRecordTaskTiming(const internal::TaskQueueImpl* task_queue);
    ...
    // in SelectNextTask().
    std::optional<SelectedTask> SelectNextTaskImpl(LazyNow& lazy_now, SelectTaskOption option);
    ...
    std::optional<WakeUp> GetNextDelayedWakeUpWithOption(SelectTaskOption option) const;
    ...
    std::optional<WakeUp> AdjustWakeUp(std::optional<WakeUp> wake_up, LazyNow* lazy_now) const;
    ...
    // task.
    TaskQueue::TaskTiming InitializeTaskTiming(internal::TaskQueueImpl* task_queue);
