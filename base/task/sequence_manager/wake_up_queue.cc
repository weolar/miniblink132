...
WakeUpQueue::WakeUpQueue(scoped_refptr<const internal::AssociatedThreadId> associated_thread)
...
void WakeUpQueue::SetNextWakeUpForQueue(internal::TaskQueueImpl* queue, LazyNow* lazy_now, std::optional<WakeUp> wake_up)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    ...
    if (queue->heap_handle().IsValid()) {
        previous_queue_resolution = wake_up_queue_.at(queue->heap_handle()).wake_up.resolution;
    }
    ...
    if (previous_queue_resolution && *previous_queue_resolution == WakeUpResolution::kHigh) {
...
void WakeUpQueue::MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now, EnqueueOrder enqueue_order)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    bool update_needed = false;
    while (!wake_up_queue_.empty() && wake_up_queue_.top().wake_up.earliest_time() <= lazy_now->Now()) {
    ...
    while (!wake_up_queue_.empty()) {
        internal::TaskQueueImpl* old_queue = std::exchange(queue, wake_up_queue_.top().queue);
        if (old_queue == queue)
            break;
        queue->UpdateWakeUp(lazy_now);
    }
...
DefaultWakeUpQueue::DefaultWakeUpQueue(scoped_refptr<internal::AssociatedThreadId> associated_thread, internal::SequenceManagerImpl* sequence_manager)
...
void DefaultWakeUpQueue::OnNextWakeUpChanged(LazyNow* lazy_now, std::optional<WakeUp> wake_up)
{
...
NonWakingWakeUpQueue::NonWakingWakeUpQueue(scoped_refptr<internal::AssociatedThreadId> associated_thread)
    : WakeUpQueue(std::move(associated_thread))
{
}
...
void NonWakingWakeUpQueue::OnNextWakeUpChanged(LazyNow* lazy_now, std::optional<WakeUp> wake_up)
{
}
