...
public:
    ...
    bool empty() const
    {
        return wake_up_queue_.empty();
    }

    // Moves ready delayed tasks in TaskQueues to delayed WorkQueues, consuming
    // expired wake-ups in the process.
    void MoveReadyDelayedTasksToWorkQueues(LazyNow* lazy_now, EnqueueOrder enqueue_order);
    ...
    void SetNextWakeUpForQueue(internal::TaskQueueImpl* queue, LazyNow* lazy_now, std::optional<WakeUp> wake_up);
...
protected:
    explicit WakeUpQueue(scoped_refptr<const internal::AssociatedThreadId> associated_thread);
    ...
    virtual void OnNextWakeUpChanged(LazyNow* lazy_now, std::optional<WakeUp> next_wake_up) = 0;
...
public:
    DefaultWakeUpQueue(scoped_refptr<internal::AssociatedThreadId> associated_thread, internal::SequenceManagerImpl* sequence_manager);
...
public:
    explicit NonWakingWakeUpQueue(scoped_refptr<internal::AssociatedThreadId> associated_thread);
