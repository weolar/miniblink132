...
AtomicFlagSet::AtomicFlagSet(scoped_refptr<const AssociatedThreadId> associated_thread)
...
void AtomicFlagSet::RunActiveCallbacks() const
{
    ...
    for (Group* iter = alloc_list_head_.get(); iter; iter = iter->next.get()) {
        ...
        size_t active_flags = std::atomic_exchange_explicit(&iter->flags, size_t { 0 }, std::memory_order_acquire);
