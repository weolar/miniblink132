...
template <typename T, TimeTicks (*now_source)() = TimeTicks::Now> class LazilyDeallocatedDeque {
public:
    enum {
...
private:
    ...
    struct Ring {
        ...
        bool empty() const
        {
            return back_index_ == before_front_index_;
        }
    ...
    public:
        ...
        Iterator& operator++()
        {
            if (index_ == ring_->back_index_) {
                ...
                index_ = ring_ ? ring_->CircularIncrement(ring_->before_front_index_) : 0;
            } else {
