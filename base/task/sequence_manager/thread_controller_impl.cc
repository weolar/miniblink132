...
ThreadControllerImpl::ThreadControllerImpl(
    SequenceManagerImpl* funneled_sequence_manager, scoped_refptr<SingleThreadTaskRunner> task_runner, const TickClock* time_source)
...
std::unique_ptr<ThreadControllerImpl> ThreadControllerImpl::Create(SequenceManagerImpl* funneled_sequence_manager, const TickClock* time_source)
{
    return WrapUnique(
        new ThreadControllerImpl(funneled_sequence_manager, funneled_sequence_manager ? funneled_sequence_manager->GetTaskRunner() : nullptr, time_source));
}

void ThreadControllerImpl::SetSequencedTaskSource(SequencedTaskSource* sequence)
{
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "ThreadControllerImpl::ScheduleWork::PostTask");
...
void ThreadControllerImpl::SetNextDelayedDoWork(LazyNow* lazy_now, std::optional<WakeUp> wake_up)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
    DCHECK(sequence_);
    ...
    base::TimeDelta delay = std::max(TimeDelta(), wake_up->time - lazy_now->Now());
    TRACE_EVENT1(
        TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "ThreadControllerImpl::SetNextDelayedDoWork::PostDelayedTask", "delay_ms", delay.InMillisecondsF());
...
void ThreadControllerImpl::BindToCurrentThread(std::unique_ptr<MessagePump> message_pump)
{
    NOTREACHED();
}
...
void ThreadControllerImpl::DoWork(WorkType work_type)
{
    ...
    for (int i = 0; i < main_sequence_only().work_batch_size_; i++) {
        ...
        std::optional<SequencedTaskSource::SelectedTask> selected_task = sequence_->SelectNextTask(lazy_now_select_task);
...

        {
            ...
            if (lazy_now_after_run_task.has_value()) {
                recent_time = std::optional<base::TimeTicks>(lazy_now_after_run_task.Now());
            } else {
                recent_time.reset();
            }
        }
    ...
    std::optional<WakeUp> next_wake_up = sequence_->GetPendingWakeUp(&lazy_now_after_work);
    ...
    task_runner_->PostDelayedTask(FROM_HERE, cancelable_delayed_do_work_closure_.callback(), next_wake_up_time - lazy_now_after_work.Now());
}

void ThreadControllerImpl::AddNestingObserver(RunLoop::NestingObserver* observer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
...
void ThreadControllerImpl::RemoveNestingObserver(RunLoop::NestingObserver* observer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
...
{
    LazyNow lazy_now(time_source_);
    run_level_tracker_.OnRunLoopStarted(RunLevelTracker::kInBetweenWorkItems, lazy_now);
...
void ThreadControllerImpl::SetTaskExecutionAllowedInNativeNestedLoop(bool allowed)
{
    NOTREACHED();
}
