...
BASE_FEATURE(kUsePowerMonitorWithThreadController, "UsePowerMonitorWithThreadController", FEATURE_ENABLED_BY_DEFAULT);
...
void ThreadControllerPowerMonitor::InitializeFeatures()
{
    ...
    g_use_thread_controller_power_monitor_ = FeatureList::IsEnabled(kUsePowerMonitorWithThreadController);
}

// static
void ThreadControllerPowerMonitor::OverrideUsePowerMonitorForTesting(bool use_power_monitor)
{
...
void ThreadControllerPowerMonitor::OnSuspend()
{
    ...
    TRACE_EVENT_BEGIN("base", "ThreadController::Suspended", perfetto::Track(reinterpret_cast<uint64_t>(this), perfetto::ThreadTrack::Current()));
...
void ThreadControllerPowerMonitor::OnResume()
{
    ...
    if (is_power_suspended_) {
        TRACE_EVENT_END("base" /* ThreadController::Suspended */, perfetto::Track(reinterpret_cast<uint64_t>(this), perfetto::ThreadTrack::Current()));
