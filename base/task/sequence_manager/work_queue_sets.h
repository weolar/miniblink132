...
public:
    ...
    // O(1)
    std::optional<WorkQueueAndTaskOrder> GetOldestQueueAndTaskOrderInSet(size_t set_index) const;

#if DCHECK_IS_ON()
    // O(1)
    std::optional<WorkQueueAndTaskOrder> GetRandomQueueAndTaskOrderInSet(size_t set_index) const;
#endif
...
private:
    struct OldestTaskOrder {
        ...
        {
            value->set_heap_handle(HeapHandle());
        }
