...
std::string GetTestNameSuffix(const testing::TestParamInfo<std::tuple<RunnerType, WakeUpType, MetricsSampling>>& info)
{
    return StrCat({ "With", ToString(std::get<0>(info.param)).substr(1), ToString(std::get<1>(info.param)), ToString(std::get<2>(info.param)) });
}
...
public:
    ...
    explicit CallCountingTickClock(TickClock* clock)
        : CallCountingTickClock(BindLambdaForTesting([clock] { return clock->NowTicks(); }))
    {
    }
    ...
    void Reset()
    {
        now_call_count_.store(0);
    }

    int now_call_count() const
    {
        return now_call_count_;
    }
...
public:
    FixtureWithMockTaskRunner()
        : test_task_runner_(MakeRefCounted<TestMockTimeTaskRunner>(TestMockTimeTaskRunner::Type::kBoundToThread))
        , call_counting_clock_(BindRepeating(&TestMockTimeTaskRunner::NowTicks, test_task_runner_))
        , sequence_manager_(SequenceManagerForTest::Create(nullptr, SingleThreadTaskRunner::GetCurrentDefault(), mock_tick_clock(),
              SequenceManager::Settings::Builder()
                  .SetMessagePumpType(MessagePumpType::DEFAULT)
                  .SetRandomisedSamplingEnabled(false)
                  .SetTickClock(mock_tick_clock())
                  .SetPrioritySettings(SequenceManager::PrioritySettings(TestQueuePriority::kQueuePriorityCount, TestQueuePriority::kDefaultPriority))
                  .Build()))
    {
    ...
    {
        return test_task_runner_->NowTicks() + test_task_runner_->NextPendingTaskDelay();
    }
...
public:
    explicit FixtureWithMockMessagePump(WakeUpType wake_up_type)
        ...
        , wake_up_type_(wake_up_type)
    {
        if (wake_up_type_ == WakeUpType::kAlign) {
            feature_list_.InitWithFeatures({ kAlignWakeUps, kExplicitHighResolutionTimerWin }, {});
        } else {
            feature_list_.InitWithFeatures({}, { kAlignWakeUps, kExplicitHighResolutionTimerWin });
        }
        // A null clock triggers some assertions.
        ...
        auto settings = SequenceManager::Settings::Builder()
                            ...
                            .SetPrioritySettings(SequenceManager::PrioritySettings(TestQueuePriority::kQueuePriorityCount, TestQueuePriority::kDefaultPriority))
                            .Build();
        auto thread_controller = std::make_unique<ThreadControllerWithMessagePumpImpl>(std::move(pump), settings);
        MessagePump::InitializeFeatures();
        ThreadControllerWithMessagePumpImpl::InitializeFeatures();
        sequence_manager_ = SequenceManagerForTest::Create(std::move(thread_controller), std::move(settings));
    ...
    void FastForwardBy(TimeDelta delta) override
    {
        pump_->SetAllowTimeToAutoAdvanceUntil(mock_tick_clock()->NowTicks() + delta);
...
class SequenceManagerTest : public testing::TestWithParam<std::tuple<RunnerType, WakeUpType, MetricsSampling>>, public Fixture {
public:
    SequenceManagerTest()
    {
    ...
    TaskQueue::Handle CreateTaskQueue(TaskQueue::Spec spec = TaskQueue::Spec(QueueName::TEST_TQ))
    {
    ...
    debug::CrashKeyString* dummy_key()
    {
        return &dummy_key_;
    }
    ...
    void DestroySequenceManager() override
    {
        fixture_->DestroySequenceManager();
    }
    ...
    RunnerType GetUnderlyingRunnerType()
    {
...
private:
    std::optional<base::MetricsSubSampler::ScopedAlwaysSampleForTesting> always_sample_scoper_;
    std::optional<base::MetricsSubSampler::ScopedNeverSampleForTesting> never_sample_scoper_;
...
auto GetTestTypes()
{
    return testing::Values(std::make_tuple(RunnerType::kMessagePump, WakeUpType::kDefault, MetricsSampling::kMetricsOn),
        std::make_tuple(RunnerType::kMessagePump, WakeUpType::kDefault, MetricsSampling::kMetricsOff),
#if !BUILDFLAG(IS_WIN)
        std::make_tuple(RunnerType::kMessagePump, WakeUpType::kAlign, MetricsSampling::kMetricsOn),
        std::make_tuple(RunnerType::kMessagePump, WakeUpType::kAlign, MetricsSampling::kMetricsOff),
#endif
        std::make_tuple(RunnerType::kMockTaskRunner, WakeUpType::kDefault, MetricsSampling::kMetricsOn),
        std::make_tuple(RunnerType::kMockTaskRunner, WakeUpType::kDefault, MetricsSampling::kMetricsOff));
}

INSTANTIATE_TEST_SUITE_P(All, SequenceManagerTest, GetTestTypes(), GetTestNameSuffix);

void PostFromNestedRunloop(scoped_refptr<SingleThreadTaskRunner> runner, std::vector<std::pair<OnceClosure, bool>>* tasks)
{
...
public:
    ...
    int now_calls_count() const
    {
        return now_calls_count_;
    }
...
public:
    void WillProcessTask(const PendingTask& pending_task, bool was_blocked_or_low_priority) override
    {
    ...
    std::vector<TimeTicks> queue_times() const
    {
        return queue_times_;
    }
...
TEST_P(SequenceManagerTest, GetCorrectTaskRunnerForCurrentTask)
{
    ...
    queue->task_runner()->PostTask(
        FROM_HERE, BindLambdaForTesting([&] { EXPECT_EQ(queue->task_runner(), sequence_manager()->GetTaskRunnerForCurrentTask()); }));
...
TEST_P(SequenceManagerTest, NowNotCalledIfUnneeded)
{
    ...
    int extra_call_count = GetUnderlyingRunnerType() == RunnerType::kMockTaskRunner ? 0 : 6;
    if (GetSampling() == MetricsSampling::kMetricsOn) {
        extra_call_count += 2;
    }
...
TEST_P(SequenceManagerTest, NowCalledMinimumNumberOfTimesToComputeTaskDurations)
{
    ...
    // runner.
    int task_start_now_samples = GetUnderlyingRunnerType() == RunnerType::kMockTaskRunner ? 0 : 6;
    EXPECT_EQ(7 + GetExtraNowSampleCount() + task_start_now_samples, GetNowTicksCallCount());
...
TEST_P(SequenceManagerTest, NowCalledMinimumNumberOfTimesToComputeTaskDurationsDelayedFenceAllowed)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        queues.push_back(CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetDelayedFencesAllowed(true)));
    }
    ...
    // non-mock runner.
    int task_start_now_samples = GetUnderlyingRunnerType() == RunnerType::kMockTaskRunner ? 0 : 6;
    EXPECT_EQ(13 + GetExtraNowSampleCount() + task_start_now_samples, GetNowTicksCallCount());
    sequence_manager()->RemoveTaskTimeObserver(&time_observer);
}
...
void DisableQueueTestTask(uint64_t value, std::vector<EnqueueOrder>* out_result, TaskQueue::QueueEnabledVoter* voter)
{
    out_result->push_back(EnqueueOrder::FromIntForTesting(value));
...
TEST_P(SequenceManagerTest, MultiQueuePosting)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order));
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order));
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order));
    queues[2]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 5, &run_order));
    queues[2]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 6, &run_order));
...
TEST_P(SequenceManagerTest, NonNestableTaskPosting)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostNonNestableTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
...
TEST_P(SequenceManagerTest, NonNestableTaskExecutesInExpectedOrder)
{
    ...
    queue->task_runner()->PostNonNestableTask(FROM_HERE, BindOnce(&TestTask, 5, &run_order));
...
TEST_P(SequenceManagerTest, NonNestableTasksDoesntExecuteInNestedLoop)
{
    ...
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 3, &run_order), false));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 4, &run_order), false));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 5, &run_order), true));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 6, &run_order), true));

    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostFromNestedRunloop, queue->task_runner(), Unretained(&tasks_to_post_from_nested_loop)));
...
TEST_P(SequenceManagerTest, NonNestableTasksShutdownQueue)
{
    ...
    std::vector<std::pair<OnceClosure, bool>> tasks_to_post_from_nested_loop;
    tasks_to_post_from_nested_loop.emplace_back(BindOnce(&TestTask, 1, &run_order), false);
    tasks_to_post_from_nested_loop.emplace_back(BindOnce(&TestTask, 2, &run_order), true);
    tasks_to_post_from_nested_loop.emplace_back(BindLambdaForTesting([&queue] { queue.reset(); }), true);
...
TEST_P(SequenceManagerTest, NonNestableTaskQueueTimeShiftsToEndOfNestedLoop)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order));
    queue->task_runner()->PostNonNestableTask(FROM_HERE, BindOnce(&TestTask, 7, &run_order));
    queue->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        TestTask(4, &run_order);
        AdvanceMockTickClock(kTimeSpentInNestedLoop);
    }));
    queue->task_runner()->PostNonNestableTask(FROM_HERE, BindLambdaForTesting([&] {
    ...
    queue->task_runner()->PostNonNestableTask(FROM_HERE, BindOnce(&TestTask, 9, &run_order));
    queue->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
    ...
    const TimeTicks expected_adjusted_queueing_time = start_time + kTimeSpentInNestedLoop;
    EXPECT_THAT(observer.queue_times(),
        ElementsAre(start_time, start_time, start_time, start_time, start_time, start_time, expected_adjusted_queueing_time, expected_adjusted_queueing_time,
...
void InsertFenceAndPostTestTask(int id, std::vector<EnqueueOrder>* run_order, TaskQueue* task_queue, SequenceManagerForTest* manager)
{
    run_order->push_back(EnqueueOrder::FromIntForTesting(id));
    task_queue->InsertFence(TaskQueue::InsertFencePosition::kNow);
    task_queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, id + 1, run_order));
...
TEST_P(SequenceManagerTest, TaskQueueDisabledFromNestedLoop)
{
    ...
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 1, &run_order), false));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&InsertFenceAndPostTestTask, 2, &run_order, queue.get(), sequence_manager()), true));
...
TEST_P(SequenceManagerTest, HasTaskToRunImmediatelyOrReadyDelayedTask_ImmediateTask)
{
    auto queue = CreateTaskQueue();
...
TEST_P(SequenceManagerTest, HasTaskToRunImmediatelyOrReadyDelayedTask_DelayedTask)
{
    auto queue = CreateTaskQueue();

    std::vector<EnqueueOrder> run_order;
    TimeDelta delay(Milliseconds(10));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), delay);
    EXPECT_FALSE(queue->HasTaskToRunImmediatelyOrReadyDelayedTask());
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting)
{
    ...
    auto handle = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order),
        sequence_manager()->NowTicks() + kDelay, subtle::DelayPolicy::kFlexibleNoSooner);
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting_FlexiblePreferEarly)
{
    ...
    auto handle = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order),
        sequence_manager()->NowTicks() + kDelay, subtle::DelayPolicy::kFlexiblePreferEarly);
    TimeTicks expected_run_time = start_time + kDelay;
    if (GetWakeUpType() == WakeUpType::kAlign) {
        expected_run_time = (start_time + kDelay - kLeeway).SnappedToNextTick(TimeTicks(), kLeeway);
    }
    EXPECT_EQ(expected_run_time, NextPendingTaskTime());
    EXPECT_FALSE(queue->HasTaskToRunImmediatelyOrReadyDelayedTask());
    EXPECT_TRUE(run_order.empty());
    LazyNow lazy_now(mock_tick_clock());
    EXPECT_EQ((WakeUp { start_time + kDelay, kLeeway, WakeUpResolution::kLow, subtle::DelayPolicy::kFlexiblePreferEarly }),
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting_MixedDelayPolicy)
{
    ...
    TimeTicks start_time = sequence_manager()->NowTicks();
    std::vector<EnqueueOrder> run_order;
    auto handle1 = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 2, &run_order),
        sequence_manager()->NowTicks() + Milliseconds(8), subtle::DelayPolicy::kFlexibleNoSooner);
    auto handle2 = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order),
        sequence_manager()->NowTicks() + Milliseconds(10), subtle::DelayPolicy::kPrecise);
    EXPECT_EQ(start_time + Milliseconds(10), NextPendingTaskTime());
    EXPECT_FALSE(queue->HasTaskToRunImmediatelyOrReadyDelayedTask());
    EXPECT_TRUE(run_order.empty());
    LazyNow lazy_now(mock_tick_clock());
    EXPECT_EQ((WakeUp { start_time + Milliseconds(10), kLeeway, WakeUpResolution::kLow, subtle::DelayPolicy::kPrecise }),
        sequence_manager()->GetPendingWakeUp(&lazy_now));
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting_Immediate)
{
    ...
    auto handle = queue->task_runner()->PostCancelableDelayedTaskAt(
        subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order), TimeTicks(), subtle::DelayPolicy::kFlexibleNoSooner);
...
TEST(SequenceManagerTestWithMockTaskRunner, DelayedTaskExecutedInOneMessageLoopTask)
{
    FixtureWithMockTaskRunner fixture;
    auto queue = fixture.sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(10));
    RunLoop().RunUntilIdle();
...
TEST_P(SequenceManagerTest, DelayedTaskPosting_MultipleTasks_DecendingOrder)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(10));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(8));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(5));
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting_MultipleTasks_DescendingOrder)
{
    auto queue = CreateTaskQueue();

    std::vector<EnqueueOrder> run_order;
    auto handle1 = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order),
        sequence_manager()->NowTicks() + Milliseconds(10), subtle::DelayPolicy::kFlexibleNoSooner);

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(8));

    auto handle2 = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 3, &run_order),
        sequence_manager()->NowTicks() + Milliseconds(5), subtle::DelayPolicy::kFlexibleNoSooner);
...
TEST_P(SequenceManagerTest, DelayedTaskPosting_MultipleTasks_AscendingOrder)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(1));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(5));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(10));
...
TEST_P(SequenceManagerTest, DelayedTaskAtPosting_MultipleTasks_AscendingOrder)
{
    ...
    std::vector<EnqueueOrder> run_order;
    auto handle1 = queue->task_runner()->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&TestTask, 1, &run_order),
        sequence_manager()->NowTicks() + Milliseconds(1), subtle::DelayPolicy::kFlexibleNoSooner);
...
TEST(SequenceManagerTestWithMockTaskRunner, PostDelayedTask_SharesUnderlyingDelayedTasks)
{
    FixtureWithMockTaskRunner fixture;
    auto queue = fixture.sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    ...
    TimeDelta delay(Milliseconds(10));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), delay);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), delay);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), delay);

    EXPECT_EQ(1u, fixture.test_task_runner()->GetPendingTaskCount());
}

TEST(SequenceManagerTestWithMockTaskRunner, CrossThreadTaskPostingToDisabledQueueDoesntScheduleWork)
{
    FixtureWithMockTaskRunner fixture;
    auto queue = fixture.sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = queue->CreateQueueEnabledVoter();
...
TEST(SequenceManagerTestWithMockTaskRunner, CrossThreadTaskPostingToBlockedQueueDoesntScheduleWork)
{
    FixtureWithMockTaskRunner fixture;
    auto queue = fixture.sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
...
public:
    ...
    void Run()
    {
        FAIL() << "TestObject::Run should not be called";
    }
...
TEST_P(SequenceManagerTest, PendingDelayedTasksRemovedOnShutdown)
{
    ...
    TimeDelta delay(Milliseconds(10));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestObject::Run, Owned(new TestObject())), delay);
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestObject::Run, Owned(new TestObject())));
...
TEST_P(SequenceManagerTest, RemovingFenceWithMultipleDelayedTasks)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), delay1);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), delay2);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), delay3);
...
void RecordTimeAndQueueTask(std::vector<std::pair<TaskQueue*, TimeTicks>>* run_times, TaskQueue* task_queue, const TickClock* clock)
{
...
{
    TaskQueue::Handle queue = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetDelayedFencesAllowed(true));

    std::vector<TimeTicks> run_times;
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(100));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(200));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(300));
    ...
    EXPECT_THAT(run_times, ElementsAre(FromStartAligned(Milliseconds(100)), FromStartAligned(Milliseconds(200))));
    run_times.clear();
...
TEST_P(SequenceManagerTest, DelayedFence_ImmediateTasks)
{
    ...
    for (int i = 0; i < 5; ++i) {
        queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()));
    ...
    EXPECT_THAT(run_times, ElementsAre(kStartTime, kStartTime + Milliseconds(100), kStartTime + Milliseconds(200)));
    run_times.clear();

    queue->RemoveFence();
    FastForwardUntilNoTasksRemain();

    EXPECT_THAT(run_times, ElementsAre(kStartTime + Milliseconds(500), kStartTime + Milliseconds(500)));
}
...
TEST_P(SequenceManagerTest, DelayedFence_RemovedFenceDoesNotActivate)
{
    ...
    EXPECT_THAT(run_times,
        ElementsAre(
            kStartTime, kStartTime + Milliseconds(100), kStartTime + Milliseconds(200), kStartTime + Milliseconds(300), kStartTime + Milliseconds(400)));
}
...
TEST_P(SequenceManagerTest, DelayedFence_TakeIncomingImmediateQueue)
{
    ...
    const auto kStartTime = mock_tick_clock()->NowTicks();
    TaskQueue::Handle queue1 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetDelayedFencesAllowed(true));
    TaskQueue::Handle queue2 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST2_TQ).SetDelayedFencesAllowed(true));
    ...
    queue1->task_runner()->PostTask(FROM_HERE, BindOnce(&RecordTimeAndQueueTask, &run_times, Unretained(queue1.get()), mock_tick_clock()));
    // Force reload of immediate work queue. In real life the same effect can be
    // achieved with cross-thread posting.
    ...
    queue2->task_runner()->PostTask(FROM_HERE, BindOnce(&RecordTimeAndQueueTask, &run_times, Unretained(queue2.get()), mock_tick_clock()));

    FastForwardUntilNoTasksRemain();

    EXPECT_THAT(
        run_times, ElementsAre(std::make_pair(queue1.get(), kStartTime + Milliseconds(300)), std::make_pair(queue2.get(), kStartTime + Milliseconds(300))));
}

namespace {

void ReentrantTestTask(TaskQueue* runner, int countdown, std::vector<EnqueueOrder>* out_result)
{
    out_result->push_back(EnqueueOrder::FromIntForTesting(countdown));
    if (--countdown) {
        runner->task_runner()->PostTask(FROM_HERE, BindOnce(&ReentrantTestTask, Unretained(runner), countdown, out_result));
    }
}
...
TEST_P(SequenceManagerTest, ReentrantPosting)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&ReentrantTestTask, Unretained(queue.get()), 3, &run_order));
...
public:
    ...
    {
        return BindOnce([](OnceCallback<void()> cb, WeakPtr<bool>) { std::move(cb).Run(); }, std::move(cb), task_references_.GetWeakPtr());
    }

    bool HasReferences() const
    {
        return task_references_.HasWeakPtrs();
    }
...
TEST_P(SequenceManagerTest, PostFromThread)
{
    ...
    Thread thread("TestThread");
    thread.Start();
    thread.task_runner()->PostTask(FROM_HERE, BindOnce(&PostTaskToRunner, Unretained(queue.get()), &run_order));
    thread.Stop();
...
{
    (*run_count)++;
    runner->task_runner()->PostTask(FROM_HERE, BindOnce(&RePostingTestTask, Unretained(runner), run_count));
}
...
TEST_P(SequenceManagerTest, DoWorkCantPostItselfMultipleTimes)
{
    ...
    int run_count = 0;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RePostingTestTask, Unretained(queue.get()), &run_count));
...
TEST_P(SequenceManagerTest, PostFromNestedRunloop)
{
    ...
    std::vector<EnqueueOrder> run_order;
    std::vector<std::pair<OnceClosure, bool>> tasks_to_post_from_nested_loop;
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TestTask, 1, &run_order), true));
...
TEST_P(SequenceManagerTest, WorkBatching)
{
    ...
    std::vector<EnqueueOrder> run_order;
    for (int i = 0; i < 4; ++i) {
        queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, i, &run_order));
    }
...
public:
    ...
    MOCK_METHOD2(WillProcessTask, void(const PendingTask& task, bool was_blocked_or_low_priority));
};
...
TEST_P(SequenceManagerTest, TaskObserverAdding)
{
    ...
    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/false)).Times(2);
...
TEST_P(SequenceManagerTest, TaskObserverRemovingInsideTask)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RemoveObserverTask, sequence_manager(), &observer));

    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/false)).Times(1);
    EXPECT_CALL(observer, DidProcessTask(_)).Times(0);
    RunLoop().RunUntilIdle();
}
...
TEST_P(SequenceManagerTest, QueueTaskObserverAdding)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order));
...
TEST_P(SequenceManagerTest, QueueTaskObserverRemoving)
{
    ...
    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/false)).Times(0);
    EXPECT_CALL(observer, DidProcessTask(_)).Times(0);
...
TEST_P(SequenceManagerTest, QueueTaskObserverRemovingInsideTask)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RemoveQueueObserverTask, Unretained(queue.get()), &observer));
...
TEST_P(SequenceManagerTest, CancelHandleInsideTaskObserver)
{
    ...
    public:
        ...
        void WillProcessTask(const PendingTask& task, bool was_blocked_or_low_priority) override
        {
            handle.CancelTask();
    ...
    auto handle = queue->task_runner()->PostCancelableDelayedTask(
        subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([] { FAIL(); }), base::TimeDelta());
...
TEST_P(SequenceManagerTest, GetNextDelayedWakeUp)
{
    ...
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), expected_delay);
    EXPECT_EQ(lazy_now_1.Now() + expected_delay, sequence_manager()->GetNextDelayedWakeUp()->time);

    // With another delayed task in the same queue with a longer delay.
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(100));
    EXPECT_EQ(lazy_now_1.Now() + expected_delay, sequence_manager()->GetNextDelayedWakeUp()->time);
    ...
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), expected_delay);
    EXPECT_EQ(lazy_now_1.Now() + expected_delay, sequence_manager()->GetNextDelayedWakeUp()->time);
}
...
TEST_P(SequenceManagerTest, GetNextDelayedWakeUp_MultipleQueues)
{
    ...
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay1);
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay2);
    queues[2]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay3);
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&NopTask));

    LazyNow lazy_now(mock_tick_clock());
    EXPECT_EQ(lazy_now.Now() + delay2, sequence_manager()->GetNextDelayedWakeUp()->time);
}
...
TEST(SequenceManagerWithTaskRunnerTest, DeleteSequenceManagerInsideATask)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] { fixture.DestroySequenceManager(); }));
...
TEST_P(SequenceManagerTest, GetAndClearSystemIsQuiescentBit)
{
    ...
    TaskQueue::Handle queue0 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetShouldMonitorQuiescence(true));
    TaskQueue::Handle queue1 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST2_TQ).SetShouldMonitorQuiescence(true));
...
TEST_P(SequenceManagerTest, HasTaskToRunImmediatelyOrReadyDelayedTask_DelayedTasks)
{
    ...
    EXPECT_FALSE(queue->HasTaskToRunImmediatelyOrReadyDelayedTask());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(NullTask), Milliseconds(12));
    EXPECT_FALSE(queue->HasTaskToRunImmediatelyOrReadyDelayedTask());
...
TEST_P(SequenceManagerTest, ImmediateTasksAreNotStarvedByDelayedTasks)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&] {
        for (int i = 0; i < 9; i++) {
            queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, i, &run_order));
        }
    }),
    ...
    for (int i = 10; i < 19; i++) {
        queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, i, &run_order), kDelay);
    }
    ...
    uint64_t expected_run_order[] = { 10, 11, 12, 0, 13, 14, 15, 1, 16, 17, 18, 2, 3, 4, 5, 6, 7, 8 };
    EXPECT_THAT(run_order, ElementsAreArray(expected_run_order));
}

TEST_P(SequenceManagerTest, DelayedTaskDoesNotSkipAHeadOfNonDelayedTask_SameQueue)
{
    auto queue = CreateTaskQueue();
...
TEST_P(SequenceManagerTest, DelayedTaskDoesNotSkipAHeadOfNonDelayedTask_DifferentQueues)
{
    auto queues = CreateTaskQueues(2u);
    ...
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order));
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order));
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), delay);
...
TEST_P(SequenceManagerTest, DelayedTaskDoesNotSkipAHeadOfShorterDelayedTask)
{
    ...
    TimeDelta delay2 = Milliseconds(5);
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), delay1);
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), delay2);
...
void PostAndQuitFromNestedRunloop(RunLoop* run_loop, TaskQueue* runner, bool* was_nested)
{
    runner->task_runner()->PostTask(FROM_HERE, run_loop->QuitClosure());
    runner->task_runner()->PostTask(FROM_HERE, BindOnce(&CheckIsNested, was_nested));
    run_loop->Run();
}
...
TEST_P(SequenceManagerTest, QuitWhileNested)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostAndQuitFromNestedRunloop, Unretained(&run_loop), queue.get(), Unretained(&was_nested)));
...
public:
    ...
    const std::vector<int>& sequence_numbers() const
    {
...
TEST_P(SequenceManagerTest, SequenceNumSetWhenTaskIsPosted)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(30));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(20));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(10));
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order));
...
TEST_P(SequenceManagerTest, NewTaskQueues)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue1->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
    queue2->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order));
    queue3->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order));
    RunLoop().RunUntilIdle();
...
TEST_P(SequenceManagerTest, ShutdownTaskQueue_TaskRunnersDetaching)
{
    ...
    queue.reset();
    EXPECT_FALSE(runner1->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order)));
    EXPECT_FALSE(runner2->PostTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order)));
...
TEST_P(SequenceManagerTest, ShutdownTaskQueue_WithDelayedTasks)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(10));
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(20));
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(30));
...
TEST_P(SequenceManagerTest, ShutdownTaskQueue_InTasks)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&ShutdownQueue, std::move(queues[1])));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&ShutdownQueue, std::move(queues[2])));
...
TEST_P(SequenceManagerTest, ShutdownTaskQueueInNestedLoop)
{
    ...
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&NopTask), true));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindLambdaForTesting([&] { queue_to_delete.reset(); }), true));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&NopTask), true));
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostFromNestedRunloop, queue->task_runner(), Unretained(&tasks_to_post_from_nested_loop)));
    RunLoop().RunUntilIdle();
...
TEST_P(SequenceManagerTest, TimeDomainMigrationWithIncomingImmediateTasks)
{
    ...
    std::unique_ptr<MockTimeDomain> domain_a = std::make_unique<MockTimeDomain>(start_time_ticks);
    std::unique_ptr<MockTimeDomain> domain_b = std::make_unique<MockTimeDomain>(start_time_ticks);
...
TEST_P(SequenceManagerTest, TimeDomainDoesNotCauseWakeUp)
{
    ...
    std::unique_ptr<MockTimeDomain> domain = std::make_unique<MockTimeDomain>(sequence_manager()->NowTicks());
...
TEST_P(SequenceManagerTest, PostDelayedTasksReverseOrderAlternatingTimeDomains)
{
    auto queue = CreateTaskQueue();

    std::vector<EnqueueOrder> run_order;

    std::unique_ptr<sequence_manager::MockTimeDomain> domain = std::make_unique<sequence_manager::MockTimeDomain>(mock_tick_clock()->NowTicks());

    sequence_manager()->SetTimeDomain(domain.get());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(400));

    sequence_manager()->ResetTimeDomain();
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(300));

    sequence_manager()->SetTimeDomain(domain.get());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(200));

    sequence_manager()->ResetTimeDomain();
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order), Milliseconds(100));
...
public:
    ...
    std::optional<WakeUp> GetNextAllowedWakeUp(LazyNow* lazy_now, std::optional<WakeUp> next_desired_wake_up, bool has_immediate_work) override
    {
...
TEST_P(SequenceManagerTest, TaskQueueThrottler_DelayedTask)
{
    ...
    // empty queue.
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay10s));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay10s);
    Mock::VerifyAndClearExpectations(&throttler);

    // GetNextAllowedWakeUp should be given the same delay when a longer delay
    // task is posted.
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay10s));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay100s);
    Mock::VerifyAndClearExpectations(&throttler);

    // GetNextAllowedWakeUp should be given the new delay when a task is posted
    // with a shorter delay.
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay1s));
...
TEST_P(SequenceManagerTest, TaskQueueThrottler_OnWakeUp)
{
    ...
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay));
...
TEST_P(SequenceManagerTest, TaskQueueThrottler_ResetThrottler)
{
    ...
    // GetNextAllowedWakeUp should be called when a delayed task is posted on an
    // empty queue.
    throttler.SetNextAllowedWakeUp(base::sequence_manager::WakeUp { start_time + delay10s });
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay1s));
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay1s);
    Mock::VerifyAndClearExpectations(&throttler);
    ...
    EXPECT_EQ((WakeUp { start_time + delay1s, kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));
}
...
TEST_P(SequenceManagerTest, TaskQueueThrottler_DelayedTaskMultipleQueues)
{
    ...
    EXPECT_CALL(throttler0, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay1s)).Times(1);
    EXPECT_CALL(throttler1, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay10s)).Times(1);
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay1s);
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), delay10s);
    testing::Mock::VerifyAndClearExpectations(&throttler0);
    testing::Mock::VerifyAndClearExpectations(&throttler1);

    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter0 = queues[0]->CreateQueueEnabledVoter();
    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter1 = queues[1]->CreateQueueEnabledVoter();
    ...
    // notification.
    EXPECT_CALL(throttler0, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay1s));
    ...
    EXPECT_CALL(throttler1, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay10s));
...
public:
    ...
    template <typename... Args> void FailTask(Args...)
    {
        FAIL();
    }
...
TEST_P(SequenceManagerTest, TaskQueueThrottler_SweepCanceledDelayedTasks)
{
    ...
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay1)).Times(2);

    CancelableTask task1(mock_tick_clock());
    CancelableTask task2(mock_tick_clock());
    std::vector<TimeTicks> run_times;
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task1.weak_factory_.GetWeakPtr(), &run_times), delay1);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task2.weak_factory_.GetWeakPtr(), &run_times), delay2);

    task1.weak_factory_.InvalidateWeakPtrs();

    // Sweeping away canceled delayed tasks should trigger a notification.
    EXPECT_CALL(throttler, GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + delay2)).Times(1);
...
TEST_P(SequenceManagerTest, SweepLastTaskInQueue)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::FailTask<>, task.weak_factory_.GetWeakPtr()), base::Seconds(1));
...
TEST_P(SequenceManagerTest, CancelledTaskPostAnother_ReclaimMemory)
{
    ...
    auto on_destroy = BindLambdaForTesting([&] {
        queue->task_runner()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([] {}), base::Seconds(1));
        did_destroy = true;
    });
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(&CancelableTask::FailTask<DestructionCallback>, task.weak_factory_.GetWeakPtr(), std::move(destruction_observer)), base::Seconds(1));
...
TEST_P(SequenceManagerTest, CancelledTaskPostAnother_MoveReadyDelayedTasksToWorkQueues)
{
    // This check ensures that a task whose destruction causes another task to be
    // posted as a side-effect doesn't cause us to access invalid iterators while
    // sweeping away cancelled tasks.
...
TEST_P(SequenceManagerTest, CancelledTaskPostAnother_RemoveAllCanceledDelayedTasksFromFront)
{
    // This check ensures that a task whose destruction causes another task to be
    // posted as a side-effect doesn't cause us to access invalid iterators while
...
TEST_P(SequenceManagerTest, CancelledImmediateTaskShutsDownQueue)
{
    ...
    CancelableTask task(mock_tick_clock());
    queue->task_runner()->PostTask(
        FROM_HERE, BindOnce(&CancelableTask::FailTask<DestructionCallback>, task.weak_factory_.GetWeakPtr(), std::move(destruction_observer)));
...
void ChromiumRunloopInspectionTask(scoped_refptr<TestMockTimeTaskRunner> test_task_runner)
{
...
TEST(SequenceManagerTestWithMockTaskRunner, NumberOfPendingTasksOnChromiumRunLoop)
{
    FixtureWithMockTaskRunner fixture;
    auto queue = fixture.sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    ...
    for (int i = 1; i < 100; i++) {
        queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&ChromiumRunloopInspectionTask, fixture.test_task_runner()), Milliseconds(i));
    }
    fixture.FastForwardUntilNoTasksRemain();
}
...
class QuadraticTask {
public:
    QuadraticTask(scoped_refptr<TaskRunner> task_runner, TimeDelta delay, Fixture* fixture)
        : count_(0)
    ...
    void Run()
    {
        if (should_exit_.Run())
            return;
        count_++;
        task_runner_->PostDelayedTask(FROM_HERE, BindOnce(&QuadraticTask::Run, Unretained(this)), delay_);
        task_runner_->PostDelayedTask(FROM_HERE, BindOnce(&QuadraticTask::Run, Unretained(this)), delay_);
...
class LinearTask {
public:
    LinearTask(scoped_refptr<TaskRunner> task_runner, TimeDelta delay, Fixture* fixture)
        : count_(0)
        , task_runner_(task_runner)
        , delay_(delay)
        , fixture_(fixture)
    ...
    void Run()
    {
        if (should_exit_.Run())
            return;
        count_++;
        task_runner_->PostDelayedTask(FROM_HERE, BindOnce(&LinearTask::Run, Unretained(this)), delay_);
        fixture_->AdvanceMockTickClock(Milliseconds(5));
    }
...
TEST_P(SequenceManagerTest, DelayedTasksDontBadlyStarveNonDelayedWork_SameQueue)
{
    auto queue = CreateTaskQueue();

    QuadraticTask quadratic_delayed_task(queue->task_runner(), Milliseconds(10), this);
    LinearTask linear_immediate_task(queue->task_runner(), TimeDelta(), this);
    RepeatingCallback<bool()> should_exit = BindRepeating(ShouldExit, &quadratic_delayed_task, &linear_immediate_task);
...
TEST_P(SequenceManagerTest, ImmediateWorkCanStarveDelayedTasks_SameQueue)
{
    ...
    QuadraticTask quadratic_immediate_task(queue->task_runner(), TimeDelta(), this);
    LinearTask linear_delayed_task(queue->task_runner(), Milliseconds(10), this);
    RepeatingCallback<bool()> should_exit = BindRepeating(&ShouldExit, &quadratic_immediate_task, &linear_delayed_task);
...
TEST_P(SequenceManagerTest, DelayedTasksDontBadlyStarveNonDelayedWork_DifferentQueue)
{
    auto queues = CreateTaskQueues(2u);

    QuadraticTask quadratic_delayed_task(queues[0]->task_runner(), Milliseconds(10), this);
...
TEST_P(SequenceManagerTest, ImmediateWorkCanStarveDelayedTasks_DifferentQueue)
{
    ...
    QuadraticTask quadratic_immediate_task(queues[0]->task_runner(), TimeDelta(), this);
    LinearTask linear_delayed_task(queues[1]->task_runner(), Milliseconds(10), this);
    RepeatingCallback<bool()> should_exit = BindRepeating(&ShouldExit, &quadratic_immediate_task, &linear_delayed_task);
...
namespace {
void CurrentlyExecutingTaskQueueTestTask(SequenceManagerImpl* sequence_manager, std::vector<internal::TaskQueueImpl*>* task_sources)
{
...
TEST_P(SequenceManagerTest, CurrentlyExecutingTaskQueue_TaskRunning)
{
    ...
    queue0->task_runner()->PostTask(FROM_HERE, BindOnce(&CurrentlyExecutingTaskQueueTestTask, sequence_manager(), &task_sources));
    queue1->task_runner()->PostTask(FROM_HERE, BindOnce(&CurrentlyExecutingTaskQueueTestTask, sequence_manager(), &task_sources));
    RunLoop().RunUntilIdle();

    EXPECT_THAT(task_sources, ElementsAre(GetTaskQueueImpl(queue0), GetTaskQueueImpl(queue1)));
    EXPECT_EQ(nullptr, sequence_manager()->currently_executing_task_queue());
}

namespace {
void RunloopCurrentlyExecutingTaskQueueTestTask(
    SequenceManagerImpl* sequence_manager, std::vector<internal::TaskQueueImpl*>* task_sources, std::vector<std::pair<OnceClosure, TaskQueue*>>* tasks)
{
    task_sources->push_back(sequence_manager->currently_executing_task_queue());
...
TEST_P(SequenceManagerTest, CurrentlyExecutingTaskQueue_NestedLoop)
{
    ...
    std::vector<internal::TaskQueueImpl*> task_sources;
    std::vector<std::pair<OnceClosure, TaskQueue*>> tasks_to_post_from_nested_loop;
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&CurrentlyExecutingTaskQueueTestTask, sequence_manager(), &task_sources), queue1));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&CurrentlyExecutingTaskQueueTestTask, sequence_manager(), &task_sources), queue2));

    queue0->task_runner()->PostTask(
        FROM_HERE, BindOnce(&RunloopCurrentlyExecutingTaskQueueTestTask, sequence_manager(), &task_sources, &tasks_to_post_from_nested_loop));

    RunLoop().RunUntilIdle();
    EXPECT_THAT(task_sources, UnorderedElementsAre(GetTaskQueueImpl(queue0), GetTaskQueueImpl(queue1), GetTaskQueueImpl(queue2), GetTaskQueueImpl(queue0)));
    EXPECT_EQ(nullptr, sequence_manager()->currently_executing_task_queue());
}
...
TEST_P(SequenceManagerTest, NoWakeUpsForCanceledDelayedTasks)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task1.weak_factory_.GetWeakPtr(), &run_times), delay1);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task2.weak_factory_.GetWeakPtr(), &run_times), delay2);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task3.weak_factory_.GetWeakPtr(), &run_times), delay3);
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task4.weak_factory_.GetWeakPtr(), &run_times), delay4);
    ...
    RunUntilManagerIsIdle(BindRepeating(
        [](std::set<TimeTicks>* wake_up_times, const TickClock* clock) { wake_up_times->insert(clock->NowTicks()); }, &wake_up_times, mock_tick_clock()));

    EXPECT_THAT(wake_up_times, ElementsAre(start_time + delay1, start_time + delay4));
...
TEST_P(SequenceManagerTest, TimeDomainWakeUpOnlyCancelledIfAllUsesCancelled)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, Unretained(&task3), &run_times), delay3);
    ...
    EXPECT_THAT(wake_up_times, ElementsAre(start_time + delay1, start_time + delay3, start_time + delay4));
...
TEST_P(SequenceManagerTest, SweepCanceledDelayedTasks_ManyTasks)
{
    ...
    for (int i = 0; i < kNumTasks; i++) {
        ...
        queue->task_runner()->PostDelayedTask(
            FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, tasks[i]->weak_factory_.GetWeakPtr(), &run_times), Seconds(i + 1));
    }
...
TEST_P(SequenceManagerTest, DelayedTasksNotSelected)
{
    ...
    EXPECT_EQ(std::nullopt, sequence_manager()->GetPendingWakeUp(&lazy_now, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));

    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), kDelay);

    // No task should be ready to execute.
    EXPECT_FALSE(sequence_manager()->SelectNextTask(lazy_now, SequencedTaskSource::SelectTaskOption::kDefault));
    EXPECT_FALSE(sequence_manager()->SelectNextTask(lazy_now, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));

    EXPECT_EQ((WakeUp { lazy_now.Now() + kDelay, kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));
    EXPECT_EQ(std::nullopt, sequence_manager()->GetPendingWakeUp(&lazy_now, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));
    ...
    // suspend state.
    EXPECT_FALSE(sequence_manager()->SelectNextTask(lazy_now2, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));
    EXPECT_EQ(std::nullopt, sequence_manager()->GetPendingWakeUp(&lazy_now2, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));

    // Execute the delayed task.
    EXPECT_TRUE(sequence_manager()->SelectNextTask(lazy_now2, SequencedTaskSource::SelectTaskOption::kDefault));
...
TEST_P(SequenceManagerTest, DelayedTasksNotSelectedWithImmediateTask)
{
    ...
    EXPECT_EQ(WakeUp {}, sequence_manager()->GetPendingWakeUp(&lazy_now, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));

    AdvanceMockTickClock(kDelay);
    LazyNow lazy_now2(mock_tick_clock());

    // An immediate task is present, even if we skip the delayed tasks.
    EXPECT_EQ(WakeUp {}, sequence_manager()->GetPendingWakeUp(&lazy_now2, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));

    // Immediate task should be ready to execute, execute it.
    EXPECT_TRUE(sequence_manager()->SelectNextTask(lazy_now2, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));
    sequence_manager()->DidRunTask(lazy_now);
...
TEST_P(SequenceManagerTest, DelayedTasksNotSelectedWithImmediateTaskWithPriority)
{
    ...
    // Post delayed tasks.
    constexpr TimeDelta kDelay(Milliseconds(10));
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), kDelay);
    queues[3]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), kDelay);
...
TEST_P(SequenceManagerTest, GetPendingWakeUp)
{
    ...
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Seconds(10));

    EXPECT_EQ((WakeUp { lazy_now.Now() + Seconds(10), kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));

    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Seconds(15));

    EXPECT_EQ((WakeUp { lazy_now.Now() + Seconds(10), kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));

    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Seconds(5));

    EXPECT_EQ((WakeUp { lazy_now.Now() + Seconds(5), kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));
...
TEST_P(SequenceManagerTest, GetPendingWakeUp_DelayedTaskReady)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Seconds(1));
...
TEST_P(SequenceManagerTest, RemoveAllCanceledDelayedTasksFromFront)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, cancelable_closure.callback(), kDelay);
...
TEST_P(SequenceManagerTest, RemoveAllCanceledDelayedTasksFromFront_MultipleQueues)
{
    auto queues = CreateTaskQueues(2u);
    ...
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, cancelable_closure_1.callback(), kDelay1);

    CancelableOnceClosure cancelable_closure_2(base::BindOnce(&NopTask));
    constexpr TimeDelta kDelay2 = Seconds(2);
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, cancelable_closure_2.callback(), kDelay2);

    // The task from the first queue is picked to calculate the next task time.
    LazyNow lazy_now(mock_tick_clock());
    EXPECT_EQ((WakeUp { lazy_now.Now() + kDelay1, kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));
    ...
    EXPECT_EQ((WakeUp { lazy_now.Now() + kDelay2, kLeeway }), sequence_manager()->GetPendingWakeUp(&lazy_now));
...
{
    RunLoop run_loop(RunLoop::Type::kNestableTasksAllowed);
    task_queue->task_runner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), Milliseconds(100));
...
TEST_P(SequenceManagerTest, DelayedTaskRunsInNestedMessageLoop)
{
    ...
    auto queue = CreateTaskQueue();
    RunLoop run_loop;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&MessageLoopTaskWithDelayedQuit, this, Unretained(queue.get())));
    run_loop.RunUntilIdle();
}

namespace {
void MessageLoopTaskWithImmediateQuit(OnceClosure non_nested_quit_closure, TaskQueue* task_queue)
{
    RunLoop run_loop(RunLoop::Type::kNestableTasksAllowed);
...
TEST_P(SequenceManagerTest, DelayedNestedMessageLoopDoesntPreventTasksRunning)
{
    ...
    RunLoop run_loop;
    queue->task_runner()->PostDelayedTask(
        FROM_HERE, BindOnce(&MessageLoopTaskWithImmediateQuit, run_loop.QuitClosure(), Unretained(queue.get())), Milliseconds(100));
...
TEST_P(SequenceManagerTest, DelayedDoWorkNotPostedForDisabledQueue)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(1));
    EXPECT_EQ(FromStartAligned(Milliseconds(1)), NextPendingTaskTime());
...
{
    auto queues = CreateTaskQueues(3u);
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(1));
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(10));
    queues[2]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(100));

    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter0 = queues[0]->CreateQueueEnabledVoter();
    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter1 = queues[1]->CreateQueueEnabledVoter();
    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter2 = queues[2]->CreateQueueEnabledVoter();
...
namespace {
void SetOnTaskHandlers(TaskQueue* task_queue, int* start_counter, int* complete_counter)
{
    GetTaskQueueImpl(task_queue)
        ->SetOnTaskStartedHandler(BindRepeating([](int* counter, const Task& task, const TaskQueue::TaskTiming& task_timing) { ++(*counter); }, start_counter));
    GetTaskQueueImpl(task_queue)
        ->SetOnTaskCompletedHandler(
            BindRepeating([](int* counter, const Task& task, TaskQueue::TaskTiming* task_timing, LazyNow* lazy_now) { ++(*counter); }, complete_counter));
}

void UnsetOnTaskHandlers(TaskQueue* task_queue)
{
    GetTaskQueueImpl(task_queue)->SetOnTaskStartedHandler(internal::TaskQueueImpl::OnTaskStartedHandler());
    GetTaskQueueImpl(task_queue)->SetOnTaskCompletedHandler(internal::TaskQueueImpl::OnTaskCompletedHandler());
}
} // namespace
...
TEST_P(SequenceManagerTest, GracefulShutdown_ManagerDeletedInFlight)
{
    ...
    for (int i = 1; i <= 5; ++i) {
        main_tqs[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(i * 100));
    }
...
TEST_P(SequenceManagerTest, SequenceManagerDeletedWithQueuesToDelete)
{
    ...
    for (int i = 1; i <= 5; ++i) {
        main_tq->task_runner()->PostDelayedTask(
            FROM_HERE, counter.WrapCallback(BindOnce(&RecordTimeTask, &run_times, mock_tick_clock())), Milliseconds(i * 100));
    }
    FastForwardBy(Milliseconds(250));
...
{
    auto base_sequence_manager = sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(MessagePump::Create(MessagePumpType::DEFAULT));
    auto queue = base_sequence_manager->CreateTaskQueue(sequence_manager::TaskQueue::Spec(QueueName::DEFAULT_TQ));
    base_sequence_manager->SetDefaultTaskRunner(queue->task_runner());

    scoped_refptr<SingleThreadTaskRunner> original_task_runner = SingleThreadTaskRunner::GetCurrentDefault();
    scoped_refptr<SingleThreadTaskRunner> custom_task_runner = MakeRefCounted<TestSimpleTaskRunner>();
    {
        std::unique_ptr<SequenceManager> manager = CreateSequenceManagerOnCurrentThread(SequenceManager::Settings());
...
TEST_P(SequenceManagerTest, CanceledTasksInQueueCantMakeOtherTasksSkipAhead)
{
    ...
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task1.weak_factory_.GetWeakPtr(), &run_times));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&CancelableTask::RecordTimeTask, task2.weak_factory_.GetWeakPtr(), &run_times));

    std::vector<EnqueueOrder> run_order;
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
...
TEST_P(SequenceManagerTest, TaskQueueDeleted)
{
    std::vector<TimeTicks> run_times;
    TaskQueue::Handle main_tq = CreateTaskQueue();
    scoped_refptr<TaskRunner> main_task_runner = main_tq->CreateTaskRunner(kTaskTypeNone);

    TaskQueue::Handle other_tq = CreateTaskQueue();
    scoped_refptr<TaskRunner> other_task_runner = other_tq->CreateTaskRunner(kTaskTypeNone);
    ...
    for (int i = 1; i <= 5; ++i) {
        main_task_runner->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(i * 100));
    }

    other_task_runner->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(600));
...
{
    return base::BindOnce([](std::unique_ptr<RunOnDestructionHelper>) {}, std::make_unique<RunOnDestructionHelper>(std::move(task)));
}

base::OnceClosure PostOnDestruction(TaskQueue* task_queue, base::OnceClosure task)
{
    return RunOnDestruction(
        base::BindOnce([](base::OnceClosure task, TaskQueue* task_queue) { task_queue->task_runner()->PostTask(FROM_HERE, std::move(task)); }, std::move(task),
            Unretained(task_queue)));
}
...
TEST_P(SequenceManagerTest, TaskQueueUsedInTaskDestructorAfterShutdown)
{
    ...
    WaitableEvent test_executed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    ...
    thread->task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](TaskQueue* task_queue, WaitableEvent* test_executed) {
                task_queue->task_runner()->PostTask(FROM_HERE, PostOnDestruction(task_queue, base::BindOnce([] {})));
...
TEST_P(SequenceManagerTest, TaskQueueTaskRunnerDetach)
{
    ...
    std::unique_ptr<TaskQueueImpl> queue2 = std::make_unique<TaskQueueImpl>(nullptr, nullptr, TaskQueue::Spec(QueueName::TEST_TQ));
    scoped_refptr<SingleThreadTaskRunner> task_runner2 = queue2->CreateTaskRunner(0);
...
TEST_P(SequenceManagerTest, DestructorPostChainDuringShutdown)
{
    ...
    task_queue->task_runner()->PostTask(FROM_HERE,
        PostOnDestruction(task_queue.get(), PostOnDestruction(task_queue.get(), RunOnDestruction(base::BindOnce([](bool* run) { *run = true; }, &run)))));
...
TEST_P(SequenceManagerTest, DestructorPostsViaTaskRunnerHandleDuringShutdown)
{
    TaskQueue::Handle task_queue = CreateTaskQueue();
    bool run = false;
    task_queue->task_runner()->PostTask(FROM_HERE, RunOnDestruction(BindLambdaForTesting([&] {
...
TEST_P(SequenceManagerTest, HasPendingHighResolutionTasks)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&NopTask));
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(100));
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
    queue->task_runner()->PostDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&NopTask),
        sequence_manager()->NowTicks() + Milliseconds(10), subtle::DelayPolicy::kPrecise);
    EXPECT_EQ(sequence_manager()->HasPendingHighResolutionTasks(), supports_high_res);
...
TEST_P(SequenceManagerTest, HasPendingHighResolutionTasksLowAndNormalPriorityQueues)
{
    ...
    queueLow->task_runner()->PostDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE, BindOnce(&NopTask),
        sequence_manager()->NowTicks() + Milliseconds(10), subtle::DelayPolicy::kPrecise);
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
    queueNormal->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(100));
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
...
class PostTaskWhenDeleted {
public:
    PostTaskWhenDeleted(std::string name, scoped_refptr<SingleThreadTaskRunner> task_runner, size_t depth, std::set<std::string>* tasks_alive,
    ...
    ~PostTaskWhenDeleted()
    {
        CHECK(tasks_alive_->find(full_name()) != tasks_alive_->end(), base::NotFatalUntil::M125);
        tasks_alive_->erase(full_name());
        tasks_deleted_->push_back(full_name());

        if (depth_ > 0) {
            task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&CallbackWithDestructor, std::make_unique<PostTaskWhenDeleted>(name_, task_runner_, depth_ - 1, tasks_alive_, tasks_deleted_)));
        }
    }
...
private:
    ...
    {
        return name_ + " " + NumberToString(depth_);
    }
...
TEST_P(SequenceManagerTest, RecordsQueueTimeIfSettingTrue)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&NopTask));
    RunLoop().RunUntilIdle();
    EXPECT_THAT(observer.queue_times(), ElementsAre(kStartTime + Milliseconds(99)));
...
public:
    DestructionObserverProbe(bool* task_destroyed, bool* destruction_observer_called)
...
TEST_P(SequenceManagerTest, DestructionObserverTest)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(&DestructionObserverProbe::Run, base::MakeRefCounted<DestructionObserverProbe>(&task_destroyed, &destruction_observer_called)), kDelay);
...
public:
    ...
    // TimeDomain:
    bool MaybeFastForwardToWakeUp(std::optional<WakeUp> wakeup, bool quit_when_idle_requested) override
    {
...
TEST_P(SequenceManagerTest, OnIdleTimeDomainNotification)
{
    ...
    EXPECT_CALL(mock_time_domain, MaybeFastForwardToWakeUp(false)).WillOnce(Return(false));
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        StrictMock<MockTimeDomain> mock_time_domain;
        EXPECT_CALL(mock_time_domain, MaybeFastForwardToWakeUp(true)).WillOnce(Return(false));
        sequence_manager()->SetTimeDomain(&mock_time_domain);
        sequence_manager()->OnIdle();
        sequence_manager()->ResetTimeDomain();
    }));
...
{
    TaskQueue::Handle task_queue = sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
...
TEST_P(SequenceManagerTest, GetPendingTaskCountForTesting)
{
    ...
    queues[2]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(20));
...
{
    TaskQueue::Handle main_tq = CreateTaskQueue();
    scoped_refptr<TaskRunner> task_runner = main_tq->CreateTaskRunner(kTaskTypeNone);
    sequence_manager()->SetAddQueueTimeToTasks(true);

    Thread thread("test thread");
    thread.StartAndWaitForTesting();

    WaitableEvent task_posted(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    thread.task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](scoped_refptr<TaskRunner> task_runner, WaitableEvent* task_posted) {
                task_runner->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), base::Milliseconds(10));
                task_posted->Signal();
            },
...
{
    task_runner->PostTask(FROM_HERE, BindOnce(&NopTask));
    task_runner->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), base::Milliseconds(20));
}

void PostTaskC(scoped_refptr<TaskRunner> task_runner)
{
    task_runner->PostTask(FROM_HERE, BindOnce(&NopTask));
    task_runner->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), base::Milliseconds(30));
}
...
{
    auto queues = CreateTaskQueues(static_cast<size_t>(TestQueuePriority::kQueuePriorityCount));

    for (uint8_t priority = 0; priority < static_cast<uint8_t>(TestQueuePriority::kQueuePriorityCount); priority++) {
        if (priority != static_cast<uint8_t>(TestQueuePriority::kNormalPriority)) {
            queues[priority]->SetQueuePriority(static_cast<TaskQueue::QueuePriority>(priority));
        }
    }
    ...
    for (int i = 0; i < 60; i++) {
        for (uint8_t priority = 0; priority < static_cast<uint8_t>(TestQueuePriority::kQueuePriorityCount); priority++) {
            queues[priority]->task_runner()->PostTask(FROM_HERE, base::BindOnce([](std::string* str, char c) { str->push_back(c); }, &order, '0' + priority));
        }
    }
...
TEST_P(SequenceManagerTest, PeriodicHousekeeping)
{
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), SequenceManagerImpl::kReclaimMemoryInterval);
    ...
    queue->task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(&CancelableTaskWithDestructionObserver::Task, task1.weak_factory_.GetWeakPtr(),
            std::make_unique<ScopedClosureRunner>(BindLambdaForTesting([&] { task1_deleted = true; }))),
        Hours(1));

    queue->task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(&CancelableTaskWithDestructionObserver::Task, task2.weak_factory_.GetWeakPtr(),
            std::make_unique<ScopedClosureRunner>(BindLambdaForTesting([&] { task2_deleted = true; }))),
        Hours(2));

    queue->task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(&CancelableTaskWithDestructionObserver::Task, task3.weak_factory_.GetWeakPtr(),
            std::make_unique<ScopedClosureRunner>(BindLambdaForTesting([&] { task3_deleted = true; }))),
        Hours(3));
...
public:
    MOCK_METHOD2(Allocate, debug::CrashKeyString*(const char name[], debug::CrashKeySize));
...
TEST_P(SequenceManagerTest, CrossQueueTaskPostingWhenQueueDeleted)
{
    ...
    queue_1->task_runner()->PostDelayedTask(FROM_HERE, PostOnDestruction(queue_2.get(), task.Get()), Minutes(1));
...
TEST_P(SequenceManagerTest, ReclaimMemoryRemovesCorrectQueueFromSet)
{
    ...
    CancelableRepeatingClosure cancelable_closure1(BindLambdaForTesting([&] { order.push_back(10); }));
    CancelableRepeatingClosure cancelable_closure2(BindLambdaForTesting([&] { order.push_back(11); }));
    ...
    queue2->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] { order.push_back(2); }));
    queue3->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] { order.push_back(3); }));
...
public:
    ...
    int num_will_process_task() const
    {
        return num_will_process_task_;
    }
    int num_did_process_task() const
    {
        return num_did_process_task_;
    }

private:
    void WillProcessTask(const base::PendingTask& pending_task, bool was_blocked_or_low_priority) override
    {
...
TEST_P(SequenceManagerTest, NoDelayedRunTimeForImmediateTask)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] { run_loop.Quit(); }));
    run_loop.Run();
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_QueueDisabled)
{
    ...
    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/true));
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_FenceBeginningOfTime)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_PostedBeforeFenceNow)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
    ...
    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/false));
    EXPECT_CALL(observer, DidProcessTask(_));
    RunLoop().RunUntilIdle();
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_PostedAfterFenceNow)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_LowerPriorityWhileQueued)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_LowPriorityWhenQueueing)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_LowPriorityWhenRunning)
{
    auto queue = CreateTaskQueue();
    testing::StrictMock<MockTaskObserver> observer;
    sequence_manager()->AddTaskObserver(&observer);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_TaskObserverUnblockedWithBacklog)
{
    ...
    EXPECT_CALL(observer, WillProcessTask(_, /*was_blocked_or_low_priority=*/true)).Times(3);
...
TEST_P(SequenceManagerTest, DelayedTaskOrderFromMultipleQueues)
{
    ...
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(9));
    queues[1]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(10));
    queues[2]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(10));
    queues[0]->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order), Milliseconds(100));
...
TEST_P(SequenceManagerTest, OnTaskPostedCallbacks)
{
    ...
    std::unique_ptr<TaskQueue::OnTaskPostedCallbackHandle> handle1
        = queue->AddOnTaskPostedHandler(BindRepeating([](int* counter, const Task& task) { ++(*counter); }, &counter1));
    ...
    std::unique_ptr<TaskQueue::OnTaskPostedCallbackHandle> handle2
        = queue->AddOnTaskPostedHandler(BindRepeating([](int* counter, const Task& task) { ++(*counter); }, &counter2));
...
public:
    SequenceManagerRunOrPostTaskTest()
    {
        ...
        auto thread_controller = std::make_unique<ThreadControllerWithMessagePumpImpl>(std::make_unique<MessagePumpDefault>(), settings);
        sequence_manager_ = SequenceManagerForTest::Create(std::move(thread_controller), std::move(settings));
        queue_ = sequence_manager_->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
        other_queue_ = sequence_manager_->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    ...
    TaskQueue* other_queue()
    {
        return other_queue_.get();
    }
    SingleThreadTaskRunner* task_runner()
    {
        return queue_->task_runner().get();
    }
...
{
    bool did_run = false;
    EXPECT_TRUE(task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] { did_run = true; })));
...
{
    bool did_run = false;
    other_thread_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        EXPECT_TRUE(task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] { did_run = true; })));
    }));
...
TEST_F(SequenceManagerRunOrPostTaskTest, FromOtherThreadWithQueuedTask)
{
    ...
    bool did_run = false;
    EXPECT_TRUE(other_thread_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        EXPECT_TRUE(task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] { did_run = true; })));
        EXPECT_FALSE(did_run);
    })));
...
// `SequenceManager`.
TEST_F(SequenceManagerRunOrPostTaskTest, FromOtherThreadWithQueuedTaskOtherQueue)
{
    SimulateInsideRunLoop();
...
// if there is no running task and the only queued task is in a different queue
// which is disabled.
TEST_F(SequenceManagerRunOrPostTaskTest, FromOtherThreadQueuedTaskInDisabledOtherQueue)
{
    bool did_run_other_task = false;
    EXPECT_TRUE(other_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] { did_run_other_task = true; })));
...
TEST_F(SequenceManagerRunOrPostTaskTest, FromOtherThreadWithRunningTaskInNestedLoop)
{
    WaitableEvent main_thread_task_running;
    RunLoop nested_run_loop(RunLoop::Type::kNestableTasksAllowed);
    ...
    EXPECT_TRUE(other_thread_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        ...
        EXPECT_TRUE(task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] {
...
// `SequenceManager`.
TEST_F(SequenceManagerRunOrPostTaskTest, FromOtherThreadWithRunningTaskOtherQueue)
{
    WaitableEvent main_thread_task_running;
    WaitableEvent run_or_post_task_done;
...
TEST_F(SequenceManagerRunOrPostTaskTest, MainThreadCantStartTaskDuringRunOrPostTask)
{
    RunLoop run_loop;
...
TEST_F(SequenceManagerRunOrPostTaskTest, ConcurrentCalls)
{
    ...
    EXPECT_TRUE(other_thread_2.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        did_start_task_1.Wait();
        EXPECT_TRUE(
            task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] { did_complete_task_2 = true; })));
...
TEST_F(SequenceManagerRunOrPostTaskTest, SequenceAndThreadChecker)
{
    ...
    EXPECT_TRUE(other_thread_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        ...
        task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] {
...
// `TaskScope`).
TEST_F(SequenceManagerRunOrPostTaskTest, SequenceAndThreadCheckerFromThreadPool)
{
    SimulateInsideRunLoop();
    ...
    EXPECT_TRUE(ThreadPool::PostTask(FROM_HERE, BindLambdaForTesting([&] {
        bool did_run = false;
        task_runner()->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] {
            EXPECT_TRUE(sequence_checker.CalledOnValidSequence());
            EXPECT_FALSE(thread_checker.CalledOnValidThread());
...
TEST_F(SequenceManagerRunOrPostTaskTest, CurrentDefaultTaskRunner)
{
    ...
    EXPECT_TRUE(other_thread_task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        ...
        EXPECT_EQ(other_thread_task_runner(), SingleThreadTaskRunner::GetCurrentDefault());
        EXPECT_TRUE(SequencedTaskRunner::HasCurrentDefault());
        EXPECT_EQ(other_thread_task_runner(), SequencedTaskRunner::GetCurrentDefault());
        ...
        for (auto* tested_task_runner : { task_runner(), other_task_runner() }) {
            ...
            tested_task_runner->RunOrPostTask(subtle::RunOrPostTaskPassKeyForTesting(), FROM_HERE, BindLambdaForTesting([&] {
                did_run = true;
...
{
    auto sequence_manager = sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(MessagePump::Create(MessagePumpType::DEFAULT));
    auto queue = sequence_manager->CreateTaskQueue(sequence_manager::TaskQueue::Spec(QueueName::DEFAULT_TQ));
    sequence_manager->SetDefaultTaskRunner(queue->task_runner());

    scoped_refptr<SingleThreadTaskRunner> expected_task_runner = SingleThreadTaskRunner::GetCurrentDefault();

    StrictMock<MockCallback<base::OnceCallback<void()>>> cb;
    EXPECT_CALL(cb, Run).WillOnce(testing::Invoke([expected_task_runner] { EXPECT_EQ(SingleThreadTaskRunner::GetCurrentDefault(), expected_task_runner); }));

    static base::SequenceLocalStorageSlot<std::unique_ptr<DestructionCallback>> storage_slot;
    storage_slot.GetOrCreateValue() = std::make_unique<DestructionCallback>(cb.Get());
...
TEST(SequenceManagerTest, BindOnDifferentThreadWithActiveVoters)
{
    ...
    auto queue = sequence_manager->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    auto voter = queue->CreateQueueEnabledVoter();
    {
