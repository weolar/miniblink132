...
public:
    TaskQueueImplForTest(internal::SequenceManagerImpl* sequence_manager, WakeUpQueue* wake_up_queue, const TaskQueue::Spec& spec)
...
public:
    ...
    void OnNextWakeUpChanged(LazyNow* lazy_now, std::optional<WakeUp> wake_up) override
    {
...
public:
    void SetUp() final
    {
        ...
        task_queue_ = std::make_unique<TaskQueueImplForTest>(nullptr, wake_up_queue_.get(), TaskQueue::Spec(QueueName::TEST_TQ));
    }
...
TEST_F(WakeUpQueueTest, ScheduleWakeUpForQueue)
{
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime));
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(TimeTicks::Max())).Times(AnyNumber());
}
...
TEST_F(WakeUpQueueTest, ScheduleWakeUpForQueueSupersedesPreviousWakeUp)
{
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime1));
    LazyNow lazy_now(now);
    ...
    // requested one.
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime2));
...
TEST_F(WakeUpQueueTest, ScheduleFlexibleNoSoonerWakeUpForQueueSupersedesPreviousWakeUpWithLeeway)
{
    TimeTicks now = tick_clock_.NowTicks();
    TimeDelta delay1 = Milliseconds(10);
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime1));
    LazyNow lazy_now(now);
    task_queue_->SetNextWakeUp(&lazy_now, WakeUp { delayed_runtime1, Milliseconds(10), WakeUpResolution::kLow, subtle::DelayPolicy::kFlexibleNoSooner });

    EXPECT_EQ((WakeUp { delayed_runtime1, Milliseconds(10), WakeUpResolution::kLow, subtle::DelayPolicy::kFlexibleNoSooner }),
    ...
    // requested one.
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime2));
    task_queue_->SetNextWakeUp(&lazy_now, WakeUp { delayed_runtime2, TimeDelta(), WakeUpResolution::kLow, subtle::DelayPolicy::kPrecise });

    EXPECT_EQ((WakeUp { delayed_runtime2, TimeDelta(), WakeUpResolution::kLow, subtle::DelayPolicy::kPrecise }), wake_up_queue_->GetNextDelayedWakeUp());
    Mock::VerifyAndClearExpectations(wake_up_queue_.get());

    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(TimeTicks::Max())).Times(AnyNumber());
}

TEST_F(WakeUpQueueTest, ScheduleFlexiblePreferEarlyWakeUpForQueueSupersedesPreviousWakeUp)
{
    TimeTicks now = tick_clock_.NowTicks();
    TimeDelta delay1 = Milliseconds(10);
    TimeDelta delay2 = Milliseconds(15);
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime1));
    LazyNow lazy_now(now);
    task_queue_->SetNextWakeUp(&lazy_now, WakeUp { delayed_runtime1, TimeDelta(), WakeUpResolution::kLow, subtle::DelayPolicy::kPrecise });

    EXPECT_EQ((WakeUp { delayed_runtime1, TimeDelta(), WakeUpResolution::kLow, subtle::DelayPolicy::kPrecise }), wake_up_queue_->GetNextDelayedWakeUp());
    ...
    // requested one.
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(delayed_runtime2));
    task_queue_->SetNextWakeUp(&lazy_now, WakeUp { delayed_runtime2, Milliseconds(10), WakeUpResolution::kLow, subtle::DelayPolicy::kFlexiblePreferEarly });

    EXPECT_EQ((WakeUp { delayed_runtime2, Milliseconds(10), WakeUpResolution::kLow, subtle::DelayPolicy::kFlexiblePreferEarly }),
        wake_up_queue_->GetNextDelayedWakeUp());
    Mock::VerifyAndClearExpectations(wake_up_queue_.get());
...
TEST_F(WakeUpQueueTest, SetNextDelayedDoWork_OnlyCalledForEarlierTasks)
{
    std::unique_ptr<TaskQueueImplForTest> task_queue2
        = std::make_unique<TaskQueueImplForTest>(nullptr, wake_up_queue_.get(), TaskQueue::Spec(QueueName::TEST_TQ));
    ...
    TimeTicks now = tick_clock_.NowTicks();
    LazyNow lazy_now(now);
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(now + delay1));
    ...
    Mock::VerifyAndClearExpectations(wake_up_queue_.get());
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(now + delay4));
...
TEST_F(WakeUpQueueTest, UnregisterQueue)
{
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(wake_up1)).Times(1);
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(wake_up2)).Times(1);
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(TimeTicks::Max())).Times(1);
...
TEST_F(WakeUpQueueTest, MoveReadyDelayedTasksToWorkQueues)
{
    ...
    wake_up_queue_->MoveReadyDelayedTasksToWorkQueues(&lazy_now_1, EnqueueOrder());
    EXPECT_EQ(delayed_runtime, wake_up_queue_->NextScheduledRunTime());

    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(TimeTicks::Max()));
    tick_clock_.SetNowTicks(delayed_runtime);
    LazyNow lazy_now_2(&tick_clock_);
    wake_up_queue_->MoveReadyDelayedTasksToWorkQueues(&lazy_now_2, EnqueueOrder());
...
TEST_F(WakeUpQueueTest, MoveReadyDelayedTasksToWorkQueuesWithLeeway)
{
    ...
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(now + Milliseconds(10)));
    task_queue2->SetNextWakeUp(&lazy_now_1, WakeUp { now + Milliseconds(10), Milliseconds(4) });
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(now + Milliseconds(11)));
    task_queue_->SetNextWakeUp(&lazy_now_1, WakeUp { now + Milliseconds(11), TimeDelta() });
    ...
    Mock::VerifyAndClearExpectations(wake_up_queue_.get());
    EXPECT_CALL(*wake_up_queue_.get(), OnNextWakeUpChanged_TimeTicks(_)).Times(AnyNumber());
    // Tidy up.
    task_queue2->UnregisterTaskQueue();
}
...
TEST_F(WakeUpQueueTest, HighResolutionWakeUps)
{
    ...
    TimeTicks run_time1 = now + Milliseconds(20);
    TimeTicks run_time2 = now + Milliseconds(40);
    TaskQueueImplForTest q1(nullptr, wake_up_queue_.get(), TaskQueue::Spec(QueueName::TEST_TQ));
    TaskQueueImplForTest q2(nullptr, wake_up_queue_.get(), TaskQueue::Spec(QueueName::TEST_TQ));

    // Add two high resolution wake-ups.
    EXPECT_FALSE(wake_up_queue_->has_pending_high_resolution_tasks());
    wake_up_queue_->SetNextWakeUpForQueue(&q1, &lazy_now, WakeUp { run_time1, TimeDelta(), WakeUpResolution::kHigh });
    EXPECT_TRUE(wake_up_queue_->has_pending_high_resolution_tasks());
    wake_up_queue_->SetNextWakeUpForQueue(&q2, &lazy_now, WakeUp { run_time2, TimeDelta(), WakeUpResolution::kHigh });
    EXPECT_TRUE(wake_up_queue_->has_pending_high_resolution_tasks());
    ...
    wake_up_queue_->SetNextWakeUpForQueue(&q1, &lazy_now, WakeUp { run_time1, TimeDelta(), WakeUpResolution::kLow });
    EXPECT_FALSE(wake_up_queue_->has_pending_high_resolution_tasks());
    wake_up_queue_->SetNextWakeUpForQueue(&q1, &lazy_now, WakeUp { run_time1, TimeDelta(), WakeUpResolution::kHigh });
    EXPECT_TRUE(wake_up_queue_->has_pending_high_resolution_tasks());

    // Move a high resolution wake-up in time.
    wake_up_queue_->SetNextWakeUpForQueue(&q1, &lazy_now, WakeUp { run_time2, TimeDelta(), WakeUpResolution::kHigh });
    EXPECT_TRUE(wake_up_queue_->has_pending_high_resolution_tasks());
...
TEST_F(WakeUpQueueTest, SetNextWakeUpForQueueInThePast)
{
    ...
    auto sequence_manager
        = sequence_manager::CreateUnboundSequenceManager(SequenceManager::Settings::Builder()
                                                             .SetMessagePumpType(kType)
                                                             .SetTickClock(&tick_clock_)
                                                             .SetPrioritySettings(SequenceManager::PrioritySettings(kPriorityCount, kDefaultPriority))
                                                             .Build());
    sequence_manager->BindToMessagePump(MessagePump::Create(kType));
    auto high_prio_queue = sequence_manager->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
    high_prio_queue->SetQueuePriority(kHighestPriority);
    auto high_prio_runner = high_prio_queue->CreateTaskRunner(kTaskTypeNone);
    auto low_prio_queue = sequence_manager->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST2_TQ));
    ...
    high_prio_runner->PostTask(
        FROM_HERE, BindOnce([](SimpleTestTickClock* clock, TimeDelta delay) { clock->Advance(delay); }, base::Unretained(&tick_clock_), kDelay * 2));
