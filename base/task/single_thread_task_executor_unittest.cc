...
public:
    ...
    const std::string& result() const LIFETIME_BOUND
    {
        return result_;
    }
...
static void SlowFunc(TimeDelta pause, int* quit_counter, base::OnceClosure quit_closure)
{
...
static void RecordRunTimeFunc(TimeTicks* run_time, int* quit_counter, base::OnceClosure quit_closure)
{
...
public:
    ...
    int num_tasks_started() const
    {
        return num_tasks_started_;
    }
    int num_tasks_processed() const
    {
        return num_tasks_processed_;
    }
...
void RecursiveFunc(TaskList* order, int cookie, int depth)
{
    ...
    if (depth > 0) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, order, cookie, depth - 1));
    }
...
{
    CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow_nestable_tasks;
    MSG msg;
...
void MessageBoxFunc(TaskList* order, int cookie, bool is_reentrant)
{
    ...
    std::optional<CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop> maybe_allow_nesting;
    if (is_reentrant)
...
void RecursiveFuncWin(scoped_refptr<SingleThreadTaskRunner> task_runner, HANDLE event, bool expect_window, TaskList* order, bool message_box_is_reentrant,
    base::OnceClosure quit_closure)
{
    task_runner->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, order, 1, 2));
    task_runner->PostTask(FROM_HERE, BindOnce(&MessageBoxFunc, order, 2, message_box_is_reentrant));
    ...
    task_runner->PostTask(FROM_HERE, BindOnce(&QuitFunc, order, 5, std::move(quit_closure)));
...
void Post128KTasksThenQuit(SingleThreadTaskRunner* executor_task_runner, TimeTicks begin_ticks, TimeTicks last_post_ticks, TimeDelta slowest_delay,
    OnceClosure on_done, int num_posts_done = 0)
{
    ...
    if (num_posts_done == kNumTimes) {
        ...
        ADD_FAILURE() << "Couldn't run all tasks."
                      << "\nNumber of tasks remaining: " << kNumTimes - num_posts_done << "\nSlowest scheduling delay: " << slowest_delay
                      << "\nAverage per task: " << (now - begin_ticks) / num_posts_done;
        std::move(on_done).Run();
        return;
    }

    executor_task_runner->PostTask(
        FROM_HERE, BindOnce(&Post128KTasksThenQuit, Unretained(executor_task_runner), begin_ticks, now, slowest_delay, std::move(on_done), num_posts_done + 1));
}
...
public:
    ...
    void OnIOCompleted(MessagePumpForIO::IOContext* context, DWORD bytes_transfered, DWORD error) override;
...
TestIOHandler::TestIOHandler(const wchar_t* name, HANDLE signal)
    : MessagePumpForIO::IOHandler(FROM_HERE)
    , signal_(signal)
{
    memset(buffer_, 0, sizeof(buffer_));

    file_.Set(CreateFile(name, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL));
    EXPECT_TRUE(file_.is_valid());
}

void TestIOHandler::Init()
{
    const bool success = CurrentIOThread::Get()->RegisterIOHandler(file_.get(), this);
    ASSERT_TRUE(success);
...
void TestIOHandler::OnIOCompleted(MessagePumpForIO::IOContext* context, DWORD bytes_transfered, DWORD error)
{
...
void RunTest_IOHandler()
{
    ...
    win::ScopedHandle server(CreateNamedPipe(kPipeName, PIPE_ACCESS_OUTBOUND, 0, 1, 0, 0, 0, NULL));
    ...
    thread.task_runner()->PostTask(FROM_HERE, BindOnce(&TestIOHandler::Init, Unretained(&handler)));
...
class SingleThreadTaskExecutorTypedTest : public ::testing::TestWithParam<MessagePumpType> {
public:
    SingleThreadTaskExecutorTypedTest() = default;

    SingleThreadTaskExecutorTypedTest(const SingleThreadTaskExecutorTypedTest&) = delete;
    SingleThreadTaskExecutorTypedTest& operator=(const SingleThreadTaskExecutorTypedTest&) = delete;

    ~SingleThreadTaskExecutorTypedTest() = default;

    static std::string ParamInfoToString(::testing::TestParamInfo<MessagePumpType> param_info)
    {
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostTask)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test0, foo));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test1ConstRef, foo, a));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test1Ptr, foo, &b));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test1Int, foo, 100));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test2Ptr, foo, &a, &c));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&Foo::Test2Mixed, foo, a, &d));
    // After all tests, post a message that will shut down the message loop
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(loop.QuitWhenIdleClosure()));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_Basic)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks, loop.QuitWhenIdleClosure()), kDelay);
    loop.Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InDelayOrder)
{
    ...
    base::RunLoop loop;
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks, loop.QuitWhenIdleClosure()), Milliseconds(200));
    // If we get a large pause in execution (due to a context switch) here, this
    // test could fail.
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks, loop.QuitWhenIdleClosure()), Milliseconds(10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder)
{
    ...
    TimeTicks run_time1, run_time2;
    base::RunLoop loop;
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks, loop.QuitWhenIdleClosure()), kDelay);
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks, loop.QuitWhenIdleClosure()), kDelay);
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder_2)
{
    ...
    TimeTicks run_time;
    base::RunLoop loop;
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SlowFunc, kPause, &num_tasks, loop.QuitWhenIdleClosure()));
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks, loop.QuitWhenIdleClosure()), Milliseconds(10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder_3)
{
    ...
    for (int i = 1; i < num_tasks; ++i)
        executor.task_runner()->PostTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks, loop.QuitWhenIdleClosure()));

    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks, loop.QuitWhenIdleClosure()), Milliseconds(1));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_SharedTimer)
{
    ...
    TimeTicks run_time1, run_time2;
    base::RunLoop loop;
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks, loop.QuitWhenIdleClosure()), Seconds(1000));
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks, loop.QuitWhenIdleClosure()), Milliseconds(10));
    ...
    ~RecordDeletionProbe()
    {
        *was_deleted_ = true;
        if (post_on_delete_.get())
            SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RecordDeletionProbe::Run, post_on_delete_));
    }
...
TEST_P(SingleThreadTaskExecutorTypedTest, DISABLED_EnsureDeletion)
{
    ...
    {
        SingleThreadTaskExecutor executor(GetParam());
        executor.task_runner()->PostTask(FROM_HERE, BindOnce(&RecordDeletionProbe::Run, new RecordDeletionProbe(nullptr, &a_was_deleted)));
        // TODO(ajwong): Do we really need 1000ms here?
        executor.task_runner()->PostDelayedTask(
            FROM_HERE, BindOnce(&RecordDeletionProbe::Run, new RecordDeletionProbe(nullptr, &b_was_deleted)), Milliseconds(1000));
    }
...
TEST_P(SingleThreadTaskExecutorTypedTest, DISABLED_EnsureDeletion_Chain)
{
    ...
    bool c_was_deleted = false;
    {
        ...
        executor.task_runner()->PostTask(FROM_HERE, BindOnce(&RecordDeletionProbe::Run, c));
    }
    EXPECT_TRUE(a_was_deleted);
    EXPECT_TRUE(b_was_deleted);
...
void NestingFunc(int* depth, base::OnceClosure quit_closure)
{
    if (*depth > 0) {
        *depth -= 1;
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&NestingFunc, depth, loop1.QuitWhenIdleClosure()));
...
TEST_P(SingleThreadTaskExecutorTypedTest, Nesting)
{
    ...
    base::RunLoop loop;
    int depth = 50;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&NestingFunc, &depth, loop.QuitWhenIdleClosure()));
    loop.Run();
    EXPECT_EQ(depth, 0);
}
...
TEST_P(SingleThreadTaskExecutorTypedTest, Recursive)
{
    ...
    TaskList order;
    base::RunLoop loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, &order, 1, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, &order, 2, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitFunc, &order, 3, loop.QuitWhenIdleClosure()));
...
TEST_P(SingleThreadTaskExecutorTypedTest, NonNestableWithNoNesting)
{
    ...
    TaskList order;
    base::RunLoop loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitFunc, &order, 3, loop.QuitWhenIdleClosure()));
    loop.Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, NonNestableDelayedInNestedLoop)
{
    ...
    TaskList order;
    base::RunLoop loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatPumps, &order, 1));
    SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&SleepFunc, &order, 4, Milliseconds(50)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 5));
    SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(FROM_HERE, BindOnce(&QuitFunc, &order, 6, loop.QuitWhenIdleClosure()));
...
TEST_P(SingleThreadTaskExecutorTypedTest, QuitNow)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow, nested_run_loop.QuitClosure()));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow, outer_run_loop.QuitClosure()));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 4)); // never runs
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitTop)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
...
void QuitAndRunNestedLoop(TaskList* order, int cookie, RunLoop* outer_run_loop, RunLoop* nested_run_loop)
{
    order->RecordStart(RUNS, cookie);
    outer_run_loop->Quit();
    nested_run_loop->Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopNestedAfterQuit)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitAndRunNestedLoop, &order, 1, &outer_run_loop, &nested_run_loop));
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitBogus)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, bogus_run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitDeep)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_loop1)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 2, Unretained(&nested_loop2)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 3, Unretained(&nested_loop3)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 4, Unretained(&nested_loop4)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 5));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 6));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_loop1.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 7));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_loop2.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 8));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_loop3.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 9));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_loop4.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderBefore)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1)); // never runs
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow,
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderDuring)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_loop.QuitClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2)); // never runs
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow,
                                                                         run_loop.QuitClosure())); // never runs
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderAfter)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow, nested_run_loop.QuitClosure()));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, nested_run_loop.QuitClosure()); // has no affect
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 4));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow, outer_run_loop.QuitClosure()));
...
TEST_P(SingleThreadTaskExecutorTypedTest, MAYBE_RecursivePostsDoNotFloodPipe)
{
    ...
    RunLoop run_loop;
    Post128KTasksThenQuit(executor.task_runner().get(), begin_ticks, begin_ticks, TimeDelta(), run_loop.QuitClosure());
    run_loop.Run();
}

TEST_P(SingleThreadTaskExecutorTypedTest, ApplicationTasksAllowedInNativeNestedLoopAtTopLevel)
{
    SingleThreadTaskExecutor executor(GetParam());
    EXPECT_TRUE(CurrentThread::Get()->ApplicationTasksAllowedInNativeNestedLoop());
}
...
TEST_P(SingleThreadTaskExecutorTypedTest, NestableTasksDisallowedByDefault)
{
    ...
    executor.task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](RunLoop* run_loop) {
                EXPECT_FALSE(CurrentThread::Get()->ApplicationTasksAllowedInNativeNestedLoop());
                run_loop->Quit();
            },
            Unretained(&run_loop)));
    run_loop.Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, NestableTasksProcessedWhenRunLoopAllows)
{
    SingleThreadTaskExecutor executor(GetParam());
    RunLoop run_loop;
    executor.task_runner()->PostTask(FROM_HERE,
...
TEST_P(SingleThreadTaskExecutorTypedTest, IsIdleForTesting)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce([] {}), Milliseconds(10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, IsIdleForTestingNonNestableTask)
{
    ...
    executor.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        RunLoop nested_run_loop(RunLoop::Type::kNestableTasksAllowed);

        executor.task_runner()->PostNonNestableTask(FROM_HERE, BindLambdaForTesting([&] { nested_task_run = true; }));
...
INSTANTIATE_TEST_SUITE_P(All, SingleThreadTaskExecutorTypedTest, ::testing::Values(MessagePumpType::DEFAULT, MessagePumpType::UI, MessagePumpType::IO),
...
TEST(SingleThreadTaskExecutorTest, PostDelayedTask_SharedTimer_SubPump)
{
    ...
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SubPumpFunc, run_loop.QuitClosure()));

    // This very delayed task should never run.
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks, run_loop.QuitWhenIdleClosure()), Seconds(1000));

    // This slightly delayed task should run from within SubPumpFunc.
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&::PostQuitMessage, 0), Milliseconds(10));
...
bool QuitOnSystemTimer(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result)
{
    if (message == static_cast<UINT>(WM_TIMER)) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&::PostQuitMessage, 0));
    }
    *result = 0;
    return true;
}
...
// application task to quit the native loop.
bool DelayedQuitOnSystemTimer(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result)
{
    if (message == static_cast<UINT>(WM_TIMER)) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce(&::PostQuitMessage, 0), Milliseconds(10));
    }
    *result = 0;
    return true;
}
...
TEST(SingleThreadTaskExecutorTest, RepostingWmQuitDoesntStarveUpcomingNativeLoop)
{
    SingleThreadTaskExecutor executor(MessagePumpType::UI);
...
TEST(SingleThreadTaskExecutorTest, DISABLED_UnwindingMultipleSubPumpsDoesntStarveApplicationTasks)
{
    ...
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SubPumpFunc, run_loop.QuitClosure()));
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SubPumpFunc, DoNothing()));
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SubPumpFunc, DoNothing()));
    ...
    executor.task_runner()->PostTask(FROM_HERE, BindOnce([](bool* to_set) { *to_set = true; }, Unretained(&last_task_ran)));
...
void RunTest_NestingDenial2(MessagePumpType message_pump_type)
{
    ...
    worker.task_runner()->PostTask(
        FROM_HERE, BindOnce(&RecursiveFuncWin, SingleThreadTaskRunner::GetCurrentDefault(), event.get(), true, &order, false, loop.QuitWhenIdleClosure()));
...
TEST(SingleThreadTaskExecutorTest, NestingSupport2)
{
    ...
    win::ScopedHandle event(CreateEvent(NULL, FALSE, FALSE, NULL));
    worker.task_runner()->PostTask(
        FROM_HERE, BindOnce(&RecursiveFuncWin, SingleThreadTaskRunner::GetCurrentDefault(), event.get(), false, &order, true, loop.QuitWhenIdleClosure()));
    // Let the other thread execute.
    WaitForSingleObject(event.get(), INFINITE);
    loop.Run();
...
{
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&EmptyFunction));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&EmptyFunction));
}
...
LRESULT CALLBACK TestWndProcThunk(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    if (message == WM_CLOSE)
    ...
    case 1:
        ...
        // test.
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&PostMultipleTasks));
        // Next, we post a task that posts a windows message to trigger the second
        // stage of the test.
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&PostWindowsMessage, hwnd));
        break;
    case 2:
        ...
        CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow_nestable_tasks;
        bool did_run = false;
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&EndTest, &did_run, hwnd));
...
TEST(SingleThreadTaskExecutorTest, AlwaysHaveUserMessageWhenNesting)
{
    ...
    HWND message_hwnd = CreateWindow(MAKEINTATOM(atom), 0, 0, 0, 0, 0, 0, HWND_MESSAGE, 0, instance, 0);
...
TEST(SingleThreadTaskExecutorTest, ApplicationTasksAllowedInNativeNestedLoopExplicitlyInScope)
{
...
TEST(SingleThreadTaskExecutorTest, SequenceLocalStorageSetGet)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { slot.emplace(11); }));

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { EXPECT_EQ(*slot, 11); }));
...
TEST(SingleThreadTaskExecutorTest, SequenceLocalStorageDifferentMessageLoops)
{
    ...
    SingleThreadTaskExecutor executor;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { EXPECT_FALSE(slot); }));
...
public:
    PostTaskOnDestroy(int times)
        : times_remaining_(times)
    {
    }
    ...
    ~PostTaskOnDestroy()
    {
        PostTaskWithPostingDestructor(times_remaining_);
    }
    ...
    {
        if (times > 0) {
            SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, DoNothingWithBoundArgs(std::make_unique<PostTaskOnDestroy>(times - 1)));
        }
    }
...
TEST(SingleThreadTaskExecutorDestructionTest, DestroysFineWithPostTaskOnDestroy)
{
    SingleThreadTaskExecutor executor;
