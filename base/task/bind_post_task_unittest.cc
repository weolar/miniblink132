...
TEST_F(BindPostTaskTest, OnceCallback)
{
    ...
    OnceCallback<void(bool*, bool)> post_cb = BindPostTask(task_runner_, std::move(cb));
...
{
    int val = 0;
    OnceClosure cb = BindOnce(&SetIntFromUniquePtr, &val, std::make_unique<int>(10));
    OnceClosure post_cb = BindPostTask(task_runner_, std::move(cb));
...
{
    int val = 0;
    OnceCallback<void(std::unique_ptr<int>)> cb = BindOnce(&SetIntFromUniquePtr, &val);
    OnceCallback<void(std::unique_ptr<int>)> post_cb = BindPostTask(task_runner_, std::move(cb));
...
{
    OnceCallback<void(int)> post_cb = BindPostTask(task_runner_, BindOnce(IgnoreResult(&Multiply)));
...
TEST_F(BindPostTaskTest, OnceThen)
{
    ...
    OnceClosure then_cb = BindOnce(&Multiply, 5).Then(BindPostTask(task_runner_, BindOnce(&SetInt, &value)));
...
TEST_F(BindPostTaskTest, OnceRunDestroyedOnBound)
{
    ...
    OnceClosure cb = BindOnce(&SequenceRestrictionChecker::Run, std::move(checker));
    OnceClosure post_cb = BindPostTask(task_runner_, std::move(cb));
...
TEST_F(BindPostTaskTest, OnceNotRunDestroyedOnBound)
{
    ...
    OnceClosure cb = BindOnce(&SequenceRestrictionChecker::Run, std::move(checker));
    OnceClosure post_cb = BindPostTask(task_runner_, std::move(cb));
    target_thread.task_runner()->PostTask(FROM_HERE, BindOnce(&ClearReference, std::move(post_cb)));
...
TEST_F(BindPostTaskTest, RepeatingCallback)
{
    ...
    RepeatingCallback<void(bool*, bool)> post_cb = BindPostTask(task_runner_, std::move(cb));
...
{
    int val = 0;
    RepeatingCallback<void(std::unique_ptr<int>)> cb = BindRepeating(&SetIntFromUniquePtr, &val);
    RepeatingCallback<void(std::unique_ptr<int>)> post_cb = BindPostTask(task_runner_, std::move(cb));
...
{
    RepeatingCallback<void(int)> post_cb = BindPostTask(task_runner_, BindRepeating(IgnoreResult(&Multiply)));
    std::move(post_cb).Run(1);
    RunLoop().RunUntilIdle();
}
...
TEST_F(BindPostTaskTest, RepeatingThen)
{
    ...
    // Multiply() returns an int and SetInt() takes an int as a parameter.
    RepeatingCallback<void(int)> then_cb = BindRepeating(&Multiply).Then(BindPostTask(task_runner_, BindRepeating(&SetInt, &value)));
...
TEST_F(BindPostTaskTest, RepeatingRunDestroyedOnBound)
{
    ...
    // thread to invoke `cb` which runs SequenceRestrictionChecker::Run(). After
    // `cb` has been invoked `checker` is destroyed along with the BindState.
    RepeatingClosure cb = BindRepeating(&SequenceRestrictionChecker::Run, std::move(checker));
    RepeatingClosure post_cb = BindPostTask(task_runner_, std::move(cb));
    target_thread.task_runner()->PostTask(FROM_HERE, std::move(post_cb));
...
TEST_F(BindPostTaskTest, RepeatingNotRunDestroyedOnBound)
{
    ...
    RepeatingClosure cb = BindRepeating(&SequenceRestrictionChecker::Run, std::move(checker));
    RepeatingClosure post_cb = BindPostTask(task_runner_, std::move(cb));
    target_thread.task_runner()->PostTask(FROM_HERE, BindRepeating(&ClearReference, std::move(post_cb)));
