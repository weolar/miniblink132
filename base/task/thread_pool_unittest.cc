...
TEST(ThreadPool, PostTaskAndReplyWithResultThreeArgs)
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        FROM_HERE, base::BindOnce([] { return 3; }), base::OnceCallback<void(int)>(base::BindLambdaForTesting([&run_loop](int x) {
            EXPECT_EQ(x, 3);
            run_loop.Quit();
        })));
    run_loop.Run();
...
TEST(ThreadPool, PostTaskAndReplyWithResultFourArgs)
{
    ...
    base::RunLoop run_loop;
    base::ThreadPool::PostTaskAndReplyWithResult(
        FROM_HERE, /*traits=*/ {}, base::BindOnce([] { return 3; }), base::OnceCallback<void(int)>(base::BindLambdaForTesting([&run_loop](int x) {
            EXPECT_EQ(x, 3);
            run_loop.Quit();
        })));
    run_loop.Run();
...
TEST(ThreadPool, BindPostTaskFizzlesOnShutdown)
{
    ...
    base::ThreadPoolInstance::CreateAndStartWithDefaultParams("BindPostTaskFizzlesOnShutdownTest");

    {
        ...
        // triggering it.
        auto bound_callback
            = base::BindPostTask(base::ThreadPool::CreateSequencedTaskRunner({ TaskShutdownBehavior::BLOCK_SHUTDOWN }), base::BindOnce([] { ADD_FAILURE(); }));
...
TEST(ThreadPool, PostTaskAndReplyFizzlesOnShutdown)
{
    ...
    base::ThreadPoolInstance::CreateAndStartWithDefaultParams("PostTaskAndReplyFizzlesOnShutdown");

    {
        base::SingleThreadTaskExecutor executor;
        auto blocking_task_runner = base::ThreadPool::CreateSequencedTaskRunner({ TaskShutdownBehavior::BLOCK_SHUTDOWN });
        ...
        blocking_task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
            // Enqueue a task whose only purpose is to exit the run loop, ensuring
            // the following task is never run.
            executor.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
                event.Wait();
                run_loop.Quit();
            }));
            ...
            executor.task_runner()->PostTaskAndReply(FROM_HERE, base::BindOnce([] { ADD_FAILURE(); }), base::BindOnce([] { ADD_FAILURE(); }));
