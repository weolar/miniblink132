...
protected:
    ~CancelableTaskTrackerTest() override
    {
...
TEST_F(CancelableTaskTrackerTest, NoCancel)
{
    ...
    std::ignore = task_tracker_.PostTask(worker_thread.task_runner().get(), FROM_HERE, MakeExpectedRunClosure(FROM_HERE));

    std::ignore
        = task_tracker_.PostTaskAndReply(worker_thread.task_runner().get(), FROM_HERE, MakeExpectedRunClosure(FROM_HERE), MakeExpectedRunClosure(FROM_HERE));
...
{
    scoped_refptr<TestSimpleTaskRunner> test_task_runner(new TestSimpleTaskRunner());

    CancelableTaskTracker::TaskId task_id = task_tracker_.PostTask(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE));
...
TEST_F(CancelableTaskTrackerTest, CancelPostedTaskAndReply)
{
    ...
    CancelableTaskTracker::TaskId task_id
        = task_tracker_.PostTaskAndReply(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), MakeExpectedNotRunClosure(FROM_HERE));
    EXPECT_NE(CancelableTaskTracker::kBadTaskId, task_id);
...
TEST_F(CancelableTaskTrackerTest, CancelReply)
{
    ...
    CancelableTaskTracker::TaskId task_id
        = task_tracker_.PostTaskAndReply(test_task_runner.get(), FROM_HERE, MakeExpectedRunClosure(FROM_HERE), MakeExpectedNotRunClosure(FROM_HERE));
    EXPECT_NE(CancelableTaskTracker::kBadTaskId, task_id);
...
TEST_F(CancelableTaskTrackerTest, CancelReplyDifferentThread)
{
    ...
    CancelableTaskTracker::TaskId task_id
        = task_tracker_.PostTaskAndReply(worker_thread.task_runner().get(), FROM_HERE, DoNothing(), MakeExpectedNotRunClosure(FROM_HERE));
    EXPECT_NE(CancelableTaskTracker::kBadTaskId, task_id);
...
TEST_F(CancelableTaskTrackerTest, NewTrackedTaskIdDifferentThread)
{
    ...
    other_thread.task_runner()->PostTask(FROM_HERE, is_canceled.Then(BindRepeating([](bool result) { EXPECT_FALSE(result); })));
    other_thread.Stop();

    task_tracker_.TryCancel(task_id);

    ASSERT_TRUE(other_thread.Start());
    other_thread.task_runner()->PostTask(FROM_HERE, is_canceled.Then(BindRepeating([](bool result) { EXPECT_TRUE(result); })));
    other_thread.Stop();
}
...
TEST_F(CancelableTaskTrackerTest, CancelAll)
{
    ...
    std::ignore = task_tracker_.PostTask(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE));

    std::ignore = task_tracker_.PostTaskAndReply(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), MakeExpectedNotRunClosure(FROM_HERE));
...
TEST_F(CancelableTaskTrackerTest, DestructionCancelsAll)
{
...

    {
        ...
        std::ignore = task_tracker.PostTask(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE));

        std::ignore
            = task_tracker.PostTaskAndReply(test_task_runner.get(), FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), MakeExpectedNotRunClosure(FROM_HERE));
...
void MaybeRunDeadlyTaskTrackerMemberFunction(CancelableTaskTracker* task_tracker, OnceCallback<void(CancelableTaskTracker*)> fn)
{
...
TEST_F(CancelableTaskTrackerDeathTest, PostFromDifferentThread)
{
    ...
    bad_thread.task_runner()->PostTask(
        FROM_HERE, BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_), BindOnce([](CancelableTaskTracker* task_tracker) {
            std::ignore = task_tracker->PostTask(scoped_refptr<TestSimpleTaskRunner>(new TestSimpleTaskRunner()).get(), FROM_HERE, DoNothing());
        })));
}
...
TEST_F(CancelableTaskTrackerDeathTest, CancelOnDifferentThread)
{
    ...
    CancelableTaskTracker::TaskId task_id = task_tracker_.PostTask(test_task_runner.get(), FROM_HERE, DoNothing());
    EXPECT_NE(CancelableTaskTracker::kBadTaskId, task_id);

    bad_thread.task_runner()->PostTask(FROM_HERE,
        BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_),
            BindOnce([](CancelableTaskTracker::TaskId task_id, CancelableTaskTracker* task_tracker) { task_tracker->TryCancel(task_id); }, task_id)));
...
TEST_F(CancelableTaskTrackerDeathTest, CancelAllOnDifferentThread)
{
    ...
    bad_thread.task_runner()->PostTask(FROM_HERE,
        BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_),
            BindOnce([](CancelableTaskTracker* task_tracker) { task_tracker->TryCancelAll(); })));
