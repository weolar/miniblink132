...
// ```
#define RETURN_IF_ERROR(rexpr, ...) BASE_INTERNAL_EXPECTED_RETURN_IF_ERROR_IMPL(return, rexpr, __VA_ARGS__)
...
// ```
#define ASSIGN_OR_RETURN(lhs, rexpr, ...) BASE_INTERNAL_EXPECTED_ASSIGN_OR_RETURN_IMPL(return, lhs, rexpr, __VA_ARGS__)
...
template <typename Lambda, typename E = std::invoke_result_t<Lambda&&>> class UnexpectedDeducer {
public:
...
template <typename Lambda> UnexpectedDeducer(Lambda) -> UnexpectedDeducer<Lambda>;

} // namespace base::internal

#define BASE_INTERNAL_EXPECTED_BODY(expected, rexpr, name, return_keyword, error_expr)                                                                         \
    decltype(auto) expected = (rexpr);                                                                                                                         \
    {                                                                                                                                                          \
        static_assert(base::internal::IsExpected<decltype(expected)>, #name " should only be used with base::expected<>");                                     \
    }                                                                                                                                                          \
    if (!expected.has_value()) [[unlikely]] {                                                                                                                  \
        return_keyword base::internal::UnexpectedDeducer([&] { return error_expr; }).Ret();                                                                    \
    }

#define BASE_INTERNAL_EXPECTED_RETURN_IF_ERROR(expected, rexpr, return_keyword, error_expr)                                                                    \
    do {                                                                                                                                                       \
        BASE_INTERNAL_EXPECTED_BODY(expected, rexpr, RETURN_IF_ERROR, return_keyword, error_expr);                                                             \
    } while (false)

#define BASE_INTERNAL_EXPECTED_ASSIGN_OR_RETURN(expected, rexpr, return_keyword, error_expr, lhs)                                                              \
    BASE_INTERNAL_EXPECTED_BODY(expected, rexpr, ASSIGN_OR_RETURN, return_keyword, error_expr);                                                                \
    {                                                                                                                                                          \
        constexpr auto lhs_v = std::string_view(#lhs);                                                                                                         \
        static_assert(!(lhs_v.front() == '(' && lhs_v.back() == ')' && lhs_v.rfind('?') != std::string_view::npos),                                            \
...
#define BASE_INTERNAL_EXPECTED_ARGS(temp_name, return_keyword, rexpr, ...)                                                                                     \
    temp_name, rexpr, return_keyword,                                                                                                                          \
        (__VA_OPT__(BASE_INTERNAL_EXPECTED_BEGIN_INVOKE) __VA_ARGS__ __VA_OPT__(, ) std::move(temp_name).error()                                               \
                __VA_OPT__(BASE_INTERNAL_EXPECTED_END_INVOKE))

#define BASE_INTERNAL_EXPECTED_RETURN_IF_ERROR_IMPL(return_keyword, rexpr, ...)                                                                                \
    BASE_INTERNAL_EXPECTED_PASS_ARGS(                                                                                                                          \
        BASE_INTERNAL_EXPECTED_RETURN_IF_ERROR, BASE_INTERNAL_EXPECTED_ARGS(BASE_UNIQUIFY(_expected_value), return_keyword, rexpr, __VA_ARGS__))

#define BASE_INTERNAL_EXPECTED_ASSIGN_OR_RETURN_IMPL(return_keyword, lhs, rexpr, ...)                                                                          \
    BASE_INTERNAL_EXPECTED_PASS_ARGS(                                                                                                                          \
        BASE_INTERNAL_EXPECTED_ASSIGN_OR_RETURN, BASE_INTERNAL_EXPECTED_ARGS(BASE_UNIQUIFY(_expected_value), return_keyword, rexpr, __VA_ARGS__), lhs)
