...
template <typename T, bool = std::is_enum_v<T>> struct IsScopedEnumImpl : std::false_type { };

template <typename T> struct IsScopedEnumImpl<T, /*std::is_enum_v<T>=*/true> : std::negation<std::is_convertible<T, std::underlying_type_t<T>>> { };
