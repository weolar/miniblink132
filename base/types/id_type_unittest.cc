...
{
    using TestId = IdType<class TestIdTag, uint32_t, std::numeric_limits<uint32_t>::max()>;
...
protected:
    FooId test_id()
    {
        return FooId::FromUnsafeValue(GetParam());
    }
...
INSTANTIATE_TEST_SUITE_P(All, IdTypeSpecificValueTest, ::testing::Values(std::numeric_limits<int>::min(), -1, 0, 1, 123, std::numeric_limits<int>::max()));
