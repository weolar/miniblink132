...
template <typename Variant, typename T> struct VariantIndexOfTypeHelper {
    static_assert(AlwaysFalse<Variant>, "Variant must be an absl::variant<...>");
};

template <typename... Ts, typename T> struct VariantIndexOfTypeHelper<absl::variant<Ts...>, T> {
    static constexpr size_t Index()
    {
        static_assert(std::is_constructible_v<absl::variant<LiteralType<Ts>...>, LiteralType<T>>, "Variant is not constructible from T");
    ...
    template <typename Tag> struct LiteralType { };
};
...
// contains duplicates.
template <typename Variant, typename T> constexpr size_t VariantIndexOfType()
{
