...
template <typename T, size_t N = absl::kFixedArrayUseDefault, typename A = std::allocator<T>> class FixedArray : public absl::FixedArray<T, N, A> {
public:
    using absl::FixedArray<T, N, A>::FixedArray;
    explicit FixedArray(
        absl::FixedArray<T, N, A>::size_type n, const absl::FixedArray<T, N, A>::allocator_type& a = typename absl::FixedArray<T, N, A>::allocator_type())
        : FixedArray(n, T(), a)
    {
    }
};
