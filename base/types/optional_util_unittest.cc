...
TEST(OptionalUtilTest, OptionalToExpected)
{
    ...
    auto exp_with_str_error = OptionalToExpected<int, std::string>(std::nullopt, "is null");
    ...
    auto exp_with_ptr_error = OptionalToExpected<int, std::unique_ptr<int>>(std::nullopt, std::make_unique<int>(-1));
...
TEST(OptionalUtilTest, OptionalUnwrapTo_ProjectionLambda)
{
    ...
    S s;
    EXPECT_FALSE(OptionalUnwrapTo(std::optional<int>(), s, [](int i) { return S(i); }));
    EXPECT_EQ(s.i, -404);
    EXPECT_TRUE(OptionalUnwrapTo(std::optional<int>(5), s, [](int i) { return S(i); }));
    EXPECT_EQ(s.i, 5);
}
...
TEST(OptionalUtilTest, OptionalUnwrapTo_ConstRefOptional)
{
    ...
    int out = -404;
    EXPECT_FALSE(OptionalUnwrapTo(empty, out, [](const NoCopyMove& n) { return n.i; }));
    EXPECT_EQ(out, -404);
    std::optional<NoCopyMove> full(std::in_place, 5);
    EXPECT_TRUE(OptionalUnwrapTo(full, out, [](const NoCopyMove& n) { return n.i; }));
    EXPECT_EQ(out, 5);
}
...
TEST(OptionalUtilTest, OptionalUnwrapTo_RvalueOptional)
{
    ...
    int out = -404;
    EXPECT_FALSE(OptionalUnwrapTo(std::optional<MoveOnly>(), out, [](MoveOnly&& n) { return n.i; }));
    EXPECT_EQ(out, -404);
    EXPECT_TRUE(OptionalUnwrapTo(std::optional<MoveOnly>(std::in_place, 5), out, [](MoveOnly&& n) { return n.i; }));
    EXPECT_EQ(out, 5);
}
