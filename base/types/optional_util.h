...
template <class T> const T* OptionalToPtr(const std::optional<T>& optional LIFETIME_BOUND)
{
    return optional.has_value() ? &optional.value() : nullptr;
}

template <class T> T* OptionalToPtr(std::optional<T>& optional LIFETIME_BOUND)
{
    return optional.has_value() ? &optional.value() : nullptr;
}
...
template <class T> std::optional<T> OptionalFromPtr(const T* value)
{
...
base::expected<U, F> OptionalToExpected(const std::optional<T>& opt, E&& err)
    requires(std::convertible_to<T, U> && std::copyable<T> && std::convertible_to<E, F> && std::movable<E>)
{
    if (opt.has_value()) {
...
// a value.
template <class T, class E, class U = T, class F = E>
base::expected<U, F> OptionalToExpected(const std::optional<T>& opt, const E& err)
    requires(std::convertible_to<T, U> && std::copyable<T> && std::convertible_to<E, F> && std::copyable<E>)
{
    if (opt.has_value()) {
        return base::ok(opt.value());
    }
...
template <class T, class O, class P = std::identity>
    requires(std::invocable<P, const T&>) && requires(O& out, std::invoke_result_t<P, const T&> val) { out = val; }
...
template <class T, class O, class P = std::identity>
    requires(std::invocable<P, T &&>) && requires(O& out, std::invoke_result_t<P, T&&> val) { out = val; }
