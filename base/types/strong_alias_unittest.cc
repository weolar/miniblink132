...
template <typename T> T GetExampleValue(int index);

template <> int GetExampleValue<int>(int index)
{
    return 5 + index;
}
template <> uint64_t GetExampleValue<uint64_t>(int index)
{
    return 500U + index;
}

template <> std::string GetExampleValue<std::string>(int index)
{
    return std::string('a', index);
}

template <typename T, typename U> bool StreamOutputSame(const T& a, const U& b)
{
...
template <typename T> class StrongAliasTest : public ::testing::Test { };
...
TYPED_TEST(StrongAliasTest, ValueAccessesUnderlyingValue)
{
    ...
    static_assert(
        std::is_const_v<typename std::remove_reference<decltype(const_alias.value())>::type>, "Reference returned by const value getter should be const.");
}
...
TYPED_TEST(StrongAliasTest, CanBeMoveConstructed)
{
    ...
    static_assert(std::is_nothrow_move_constructible_v<FooAlias>, "Error: Alias is not nothow move constructible");
}
...
{
    using FooAlias = StrongAlias<class FooTag, TypeParam>;
    static_assert(sizeof(FooAlias) == sizeof(TypeParam), "StrongAlias should be as large as the underlying type.");
}

TYPED_TEST(StrongAliasTest, IsDefaultConstructible)
{
    using FooAlias = StrongAlias<class FooTag, TypeParam>;
    static_assert(std::is_default_constructible_v<FooAlias>, "Should be possible to default-construct a StrongAlias.");
    static_assert(std::is_trivially_default_constructible_v<FooAlias> == std::is_trivially_default_constructible_v<TypeParam>,
...
TEST(StrongAliasTest, TrivialTypeAliasIsStandardLayout)
{
    ...
    static_assert(std::is_standard_layout_v<FooAlias>, "int-based alias should have standard layout. ");
    static_assert(std::is_trivially_copyable_v<FooAlias>, "int-based alias should be trivially copyable. ");
}
...
TYPED_TEST(StrongAliasTest, CannotBeCreatedFromDifferentAlias)
{
    ...
    static_assert(!std::is_constructible_v<FooAlias, BarAlias>, "Should be impossible to construct FooAlias from a BarAlias.");
    static_assert(!std::is_convertible_v<BarAlias, FooAlias>, "Should be impossible to convert a BarAlias into FooAlias.");
}
...
TYPED_TEST(StrongAliasTest, CanDifferentiateOverloads)
{
    ...
    class Scope {
    public:
        static std::string Overload(FooAlias)
        {
            return "FooAlias";
        }
        static std::string Overload(BarAlias)
        {
            return "BarAlias";
        }
    };
