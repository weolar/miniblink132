...
template <class... Args> expected<std::tuple<Args...>, std::string> ReturnTupleValue(Args&&... v)
{
    return std::tuple<Args...>(std::forward<Args>(v)...);
}

template <class... Args> expected<std::tuple<Args...>, std::string> ReturnTupleError(std::string_view msg)
{
    return unexpected(std::string(msg));
}
...
{
    const auto adaptor = [](std::unique_ptr<int> error) { return std::make_unique<int>(2); };
    const auto func = [&]() -> std::unique_ptr<int> {
...
TEST(AssignOrReturn, Works)
{
    const auto func = []() -> std::string {
        ...
        EXPECT_EQ(3, value3);
        ASSIGN_OR_RETURN([[maybe_unused]] const int value4, ReturnError("EXPECTED"));
        return "ERROR";
    };
...
TEST(AssignOrReturn, WorksWithLambda)
{
    const auto func = []() -> std::string {
        ...
        EXPECT_EQ(1, value1);
        ASSIGN_OR_RETURN([[maybe_unused]] const int value2, [] { return ReturnError("EXPECTED"); }());
        return "ERROR";
    };
...
{
    const auto func = []() -> std::unique_ptr<int> {
        ASSIGN_OR_RETURN([[maybe_unused]] const std::string s, [] { return ReturnPtrError(1); }());
        return nullptr;
    };
...
TEST(AssignOrReturn, WorksWithCommasInType)
{
    const auto func = []() -> std::string {
        ...
        ASSIGN_OR_RETURN((const std::tuple<int, std::tuple<int, int>, int> t2), ReturnTupleValue(1, std::tuple(1, 1), 1));
        EXPECT_EQ((std::tuple(1, std::tuple(1, 1), 1)), t2);
        ASSIGN_OR_RETURN(
            ([[maybe_unused]] const std::tuple<int, std::tuple<int, int>, int> t3), (ReturnTupleError<int, std::tuple<int, int>, int>("EXPECTED")));
        return "ERROR";
    };
...
{
    const auto func = []() -> std::string {
        ASSIGN_OR_RETURN((const auto& [t1, t2, t3, t4, t5]), ReturnTupleValue(std::tuple(1, 1), 1, 2, 3, 4));
        EXPECT_EQ((std::tuple(1, 1)), t1);
        EXPECT_EQ(1, t2);
        EXPECT_EQ(2, t3);
        EXPECT_EQ(3, t4);
...
TEST(AssignOrReturn, WorksWithAdaptorFunc)
{
    ...
    const auto adaptor = [](std::string error) { return error + " EXPECTED B"; };
    const auto func = [&]() -> std::string {
        ASSIGN_OR_RETURN([[maybe_unused]] int value, ReturnValue(1), fail_test_if_called);
...
TEST(AssignOrReturn, WorksWithAdaptorFuncAndExpectedReturn)
{
    const auto adaptor = [](std::string error) { return error + " EXPECTED B"; };
    const auto func = [&]() -> expected<void, std::string> {
        ASSIGN_OR_RETURN([[maybe_unused]] const int value, ReturnError("EXPECTED A"), adaptor);
        return ok();
    };
...
TEST(AssignOrReturn, WorksWithAdaptorFuncAndMoveOnlyType)
{
    const auto adaptor = [](std::unique_ptr<int> error) { return std::make_unique<int>(2); };
    const auto func = [&]() -> std::unique_ptr<int> {
        ASSIGN_OR_RETURN([[maybe_unused]] const std::string s, ReturnPtrError(1), adaptor);
        return nullptr;
    };
...
TEST(AssignOrReturn, WorksWithThirdArgumentAndCommas)
{
    ...
    const auto adaptor = [](std::string error) { return error + " EXPECTED B"; };
    const auto func = [&]() -> std::string {
        ASSIGN_OR_RETURN((const auto& [t1, t2, t3]), ReturnTupleValue(1, 2, 3), fail_test_if_called);
        EXPECT_EQ(t1, 1);
        EXPECT_EQ(t2, 2);
        EXPECT_EQ(t3, 3);
        ASSIGN_OR_RETURN(([[maybe_unused]] const auto& [t4, t5, t6]), (ReturnTupleError<int, int, int>("EXPECTED A")), adaptor);
        return "ERROR";
    };
...
TEST(AssignOrReturn, WorksWithAppendIncludingLocals)
{
    const auto func = [&](const std::string& str) -> std::string {
        ASSIGN_OR_RETURN([[maybe_unused]] const int value, ReturnError("EXPECTED A"), [&](std::string e) { return e + str; });
        return "ERROR";
    };
