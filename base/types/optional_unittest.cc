...
public:
    ...
    int move_ctors_count() const
    {
        return move_ctors_count_;
    }
...
public:
    ...
    DeletedDefaultConstructor(int foo)
        : foo_(foo)
    {
    }
...
public:
    ...
    NonTriviallyDestructibleDeletedCopyConstructor(const NonTriviallyDestructibleDeletedCopyConstructor&) = delete;
    NonTriviallyDestructibleDeletedCopyConstructor(NonTriviallyDestructibleDeletedCopyConstructor&&) = default;
...
static_assert(std::is_trivially_destructible_v<std::optional<int>>, "OptionalIsTriviallyDestructible");

static_assert(!std::is_trivially_destructible_v<std::optional<NonTriviallyDestructible>>, "OptionalIsTriviallyDestructible");
...
TEST(OptionalTest, MoveConstructor)
{
    ...
    {
        std::optional<NonTriviallyDestructibleDeletedCopyConstructor> first(std::in_place, 42);
        std::optional<NonTriviallyDestructibleDeletedCopyConstructor> second(std::move(first));
...
TEST(OptionalTest, ForwardConstructor)
{
    ...
    // std::optional<T>(std::optional<U>) constructor.
    {
        ...
        struct Test {
            Test()
                : param_type(ParamType::DEFAULT_CONSTRUCTED)
            {
            }
            ...
            explicit Test(int param)
                : param_type(ParamType::INT)
            {
            }
            explicit Test(std::in_place_t param)
...

    {
        ...
        static_assert(std::is_convertible_v<int, Test>, "Int should be convertible to Test.");
...
TEST(OptionalTest, AssignOverload)
{
    ...
    struct Test2 {
        ...
        explicit Test2(Test1&& test1)
            : state(State::MOVE_CONSTRUCTED_FROM_TEST1)
        {
...
TEST(OptionalTest, OperatorStar_rvalue)
{
    ...
    EXPECT_TRUE(TestObject(3, 0.1) == *std::optional<TestObject>(TestObject(3, 0.1)));
}
...
TEST(OptionalTest, Value_rvalue)
{
    ...
    EXPECT_TRUE(TestObject(3, 0.1) == std::optional<TestObject>(TestObject(3, 0.1)).value());
}
...
TEST(OptionalTest, MakeOptional)
{
...

    {
        ...
        EXPECT_EQ(TestObject::State::MOVE_CONSTRUCTED, std::make_optional(std::move(value))->state());
        // NOLINTEND(bugprone-use-after-move)
    }

    {
        struct Test {
            Test(int a, double b, bool c)
                : a(a)
                , b(b)
                , c(c)
            {
...

    {
        ...
        auto str2 = std::make_optional<std::string>({ 'a', 'b', 'c' }, std::allocator<char>());
        EXPECT_EQ("abc", *str2);
    }
}
...
TEST(OptionalTest, Hash_OptionalReflectsInternal)
{
...

    {
        ...
        EXPECT_EQ(str_hash(std::string("foobar")), opt_str_hash(std::optional<std::string>(std::string("foobar"))));
    }
}
...
TEST(OptionalTest, Hash_NullOptEqualsNullOpt)
{
    ...
    EXPECT_EQ(opt_str_hash(std::optional<std::string>()), opt_int_hash(std::optional<int>()));
}
...
TEST(OptionalTest, Noexcept)
{
    ...
    static_assert(noexcept(std::optional<int>(std::declval<std::optional<int>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(trivial copy, trivial move)");
    static_assert(!noexcept(std::optional<Test1>(std::declval<std::optional<Test1>>())),
        "move constructor for non-noexcept move-constructible T must not be "
        "noexcept (trivial copy)");
    static_assert(noexcept(std::optional<Test2>(std::declval<std::optional<Test2>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(non-trivial copy, trivial move)");
    static_assert(noexcept(std::optional<Test3>(std::declval<std::optional<Test3>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(trivial copy, non-trivial move)");
    static_assert(noexcept(std::optional<Test4>(std::declval<std::optional<Test4>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(non-trivial copy, non-trivial move)");
    static_assert(!noexcept(std::optional<Test5>(std::declval<std::optional<Test5>>())),
        "move constructor for non-noexcept move-constructible T must not be "
        "noexcept (non-trivial copy)");

    static_assert(noexcept(std::declval<std::optional<int>>() = std::declval<std::optional<int>>()),
        "move assign for noexcept move-constructible/move-assignable T "
        "must be noexcept");
    static_assert(!noexcept(std::declval<std::optional<Test1>>() = std::declval<std::optional<Test1>>()),
        "move assign for non-noexcept move-constructible T must not be noexcept");
    static_assert(!noexcept(std::declval<std::optional<Test2>>() = std::declval<std::optional<Test2>>()),
...
TEST(OptionalTest, OverrideAddressOf)
{
    ...
    static_assert(std::is_trivially_destructible_v<TriviallyDestructibleOverloadAddressOf>, "Trivially...AddressOf must be trivially destructible.");
    ...
    static_assert(!std::is_trivially_destructible_v<NonTriviallyDestructibleOverloadAddressOf>, "NotTrivially...AddressOf must not be trivially destructible.");
