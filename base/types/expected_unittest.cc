...
template <typename T> struct Strong {
    constexpr explicit Strong(T value)
...
template <typename T> struct StrongMoveOnly {
    constexpr explicit StrongMoveOnly(T&& value)
        : value(std::move(value))
    {
    }
...
template <typename T> struct WeakMoveOnly {
    // NOLINTNEXTLINE(google-explicit-constructor)
    constexpr WeakMoveOnly(T&& value)
        : value(std::move(value))
    {
    }
...
struct SaveCvRef {
    ...
    constexpr SaveCvRef(SaveCvRef&)
        : cvref(CvRef::kRef)
    {
    }
    constexpr SaveCvRef(const SaveCvRef&)
        : cvref(CvRef::kConstRef)
    {
    }
    constexpr SaveCvRef(SaveCvRef&&)
        : cvref(CvRef::kRRef)
    ...
    constexpr SaveCvRef(const SaveCvRef&&)
        : cvref(CvRef::kConstRRef)
    {
    }

    constexpr explicit SaveCvRef(CvRef cvref)
        : cvref(cvref)
    {
    }
...
TEST(Ok, ValueIsQualified)
{
    ...
    static_assert(std::is_same_v<decltype(std::declval<const Ok&>().value()), const int&>);
    static_assert(std::is_same_v<decltype(std::declval<Ok>().value()), int&&>);
    static_assert(std::is_same_v<decltype(std::declval<const Ok>().value()), const int&&>);
}
...
TEST(Unexpected, ErrorIsQualified)
{
    ...
    static_assert(std::is_same_v<decltype(std::declval<const Unex&>().error()), const int&>);
    static_assert(std::is_same_v<decltype(std::declval<Unex>().error()), int&&>);
    static_assert(std::is_same_v<decltype(std::declval<const Unex>().error()), const int&&>);
}
...
{
    {
        ...
        static_assert(!std::is_convertible_v<decltype(std::move(ex1)), decltype(ex2)>);
        ASSERT_TRUE(ex2.has_value());
        EXPECT_EQ(ex2.value().value, 42);
    }
...
TEST(Expected, OperatorStar)
{
    ...
    *ex = 1;
    ...
    static_assert(std::is_same_v<decltype(*std::declval<const Ex&>()), const int&>);
    static_assert(std::is_same_v<decltype(*std::declval<Ex&&>()), int&&>);
    static_assert(std::is_same_v<decltype(*std::declval<const Ex&&>()), const int&&>);
}
...
TEST(Expected, Value)
{
    ...
    static_assert(std::is_same_v<decltype(std::declval<const Ex&>().value()), const int&>);
    static_assert(std::is_same_v<decltype(std::declval<Ex&&>().value()), int&&>);
    static_assert(std::is_same_v<decltype(std::declval<const Ex&&>().value()), const int&&>);
}
...
TEST(Expected, Error)
{
    ...
    static_assert(std::is_same_v<decltype(std::declval<const Ex&>().error()), const int&>);
    static_assert(std::is_same_v<decltype(std::declval<Ex&&>().error()), int&&>);
    static_assert(std::is_same_v<decltype(std::declval<const Ex&&>().error()), const int&&>);
}
...
TEST(Expected, ToString)
{
    ...
    const std::string error_str = ToString(expected<int, int>(unexpected(123456)));
...
TEST(Expected, AndThen)
{
    ...
    auto get_ex_cvref = [](auto&& x) -> ExOut { return SaveCvRef(std::forward<decltype(x)>(x)).cvref; };
    ...
    EXPECT_EQ(std::move(std::as_const(ex)).and_then(get_ex_cvref), CvRef::kConstRRef);

    ExIn unex(unexpect);
    EXPECT_EQ(unex.and_then(get_ex_cvref).error().cvref, CvRef::kRef);
    EXPECT_EQ(std::as_const(unex).and_then(get_ex_cvref).error().cvref, CvRef::kConstRef);
    EXPECT_EQ(std::move(unex).and_then(get_ex_cvref).error().cvref, CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(unex)).and_then(get_ex_cvref).error().cvref, CvRef::kConstRRef);

    static_assert(std::is_same_v<decltype(std::declval<ExIn&>().and_then(get_ex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().and_then(get_ex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().and_then(get_ex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().and_then(get_ex_cvref)), ExOut>);
}
...
TEST(Expected, OrElse)
{
    ...
    auto get_unex_cvref = [](auto&& x) -> ExOut { return unexpected(SaveCvRef(std::forward<decltype(x)>(x)).cvref); };

    ExIn ex;
    EXPECT_EQ(ex.or_else(get_unex_cvref).value().cvref, CvRef::kRef);
    EXPECT_EQ(std::as_const(ex).or_else(get_unex_cvref).value().cvref, CvRef::kConstRef);
    EXPECT_EQ(std::move(ex).or_else(get_unex_cvref).value().cvref, CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(ex)).or_else(get_unex_cvref).value().cvref, CvRef::kConstRRef);

    ExIn unex(unexpect);
    EXPECT_EQ(unex.or_else(get_unex_cvref).error(), CvRef::kRef);
    EXPECT_EQ(std::as_const(unex).or_else(get_unex_cvref).error(), CvRef::kConstRef);
    EXPECT_EQ(std::move(unex).or_else(get_unex_cvref).error(), CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(unex)).or_else(get_unex_cvref).error(), CvRef::kConstRRef);

    static_assert(std::is_same_v<decltype(std::declval<ExIn&>().or_else(get_unex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().or_else(get_unex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().or_else(get_unex_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().or_else(get_unex_cvref)), ExOut>);
}
...
TEST(Expected, Transform)
{
    ...
    auto get_cvref = [](auto&& x) { return SaveCvRef(std::forward<decltype(x)>(x)).cvref; };

    {
        ...
        EXPECT_EQ(std::move(std::as_const(ex)).transform(get_cvref), CvRef::kConstRRef);

        ExIn unex(unexpect);
        EXPECT_EQ(unex.transform(get_cvref).error().cvref, CvRef::kRef);
        EXPECT_EQ(std::as_const(unex).transform(get_cvref).error().cvref, CvRef::kConstRef);
        EXPECT_EQ(std::move(unex).transform(get_cvref).error().cvref, CvRef::kRRef);
        EXPECT_EQ(std::move(std::as_const(unex)).transform(get_cvref).error().cvref, CvRef::kConstRRef);

        static_assert(std::is_same_v<decltype(std::declval<ExIn&>().transform(get_cvref)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().transform(get_cvref)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().transform(get_cvref)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().transform(get_cvref)), ExOut>);
    }
    ...
    // Test void transform.
    {
        ...
        CvRef cvref = CvRef::kNone;
        auto write_cvref = [&cvref](auto&& x) { cvref = SaveCvRef(std::forward<decltype(x)>(x)).cvref; };
        ...
        EXPECT_TRUE(std::move(std::as_const(ex)).transform(write_cvref).has_value());
        ...
        EXPECT_EQ(std::as_const(unex).transform(write_cvref).error().cvref, CvRef::kConstRef);
        EXPECT_EQ(cvref, CvRef::kNone);
        EXPECT_EQ(std::move(unex).transform(write_cvref).error().cvref, CvRef::kRRef);
        EXPECT_EQ(cvref, CvRef::kNone);
        EXPECT_EQ(std::move(std::as_const(unex)).transform(write_cvref).error().cvref, CvRef::kConstRRef);
        EXPECT_EQ(cvref, CvRef::kNone);

        static_assert(std::is_same_v<decltype(std::declval<ExIn&>().transform(write_cvref)), ExOutVoid>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().transform(write_cvref)), ExOutVoid>);
        static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().transform(write_cvref)), ExOutVoid>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().transform(write_cvref)), ExOutVoid>);
    }
}
...
TEST(Expected, TransformError)
{
    ...
    EXPECT_EQ(std::as_const(ex).transform_error(get_cvref).value().cvref, CvRef::kConstRef);
    EXPECT_EQ(std::move(ex).transform_error(get_cvref).value().cvref, CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(ex)).transform_error(get_cvref).value().cvref, CvRef::kConstRRef);

    ExIn unex(unexpect);
    EXPECT_EQ(unex.transform_error(get_cvref).error(), CvRef::kRef);
    EXPECT_EQ(std::as_const(unex).transform_error(get_cvref).error(), CvRef::kConstRef);
    EXPECT_EQ(std::move(unex).transform_error(get_cvref).error(), CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(unex)).transform_error(get_cvref).error(), CvRef::kConstRRef);

    static_assert(std::is_same_v<decltype(std::declval<ExIn&>().transform_error(get_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().transform_error(get_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().transform_error(get_cvref)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().transform_error(get_cvref)), ExOut>);
}
...
TEST(ExpectedVoid, ToString)
{
    ...
    EXPECT_FALSE(base::Contains(value_str, "-byte object at "));
    const std::string error_str = ToString(expected<void, int>(unexpected(123456)));
    EXPECT_FALSE(base::Contains(error_str, "-byte object at "));
    EXPECT_TRUE(base::Contains(error_str, "123456"));
}
...
TEST(ExpectedVoid, AndThen)
{
    ...
    EXPECT_EQ(std::as_const(unex).and_then(get_true).error().cvref, CvRef::kConstRef);
    EXPECT_EQ(std::move(unex).and_then(get_true).error().cvref, CvRef::kRRef);
    EXPECT_EQ(std::move(std::as_const(unex)).and_then(get_true).error().cvref, CvRef::kConstRRef);

    static_assert(std::is_same_v<decltype(std::declval<ExIn&>().and_then(get_true)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().and_then(get_true)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().and_then(get_true)), ExOut>);
    static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().and_then(get_true)), ExOut>);
}
...
TEST(ExpectedVoid, Transform)
{
...

    {
        ...
        EXPECT_EQ(std::as_const(unex).transform(get_true).error().cvref, CvRef::kConstRef);
        EXPECT_EQ(std::move(unex).transform(get_true).error().cvref, CvRef::kRRef);
        EXPECT_EQ(std::move(std::as_const(unex)).transform(get_true).error().cvref, CvRef::kConstRRef);

        static_assert(std::is_same_v<decltype(std::declval<ExIn&>().transform(get_true)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().transform(get_true)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().transform(get_true)), ExOut>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().transform(get_true)), ExOut>);
    }
    ...
    // Test void transform.
    {
        ...
        EXPECT_EQ(std::as_const(unex).transform(do_nothing).error().cvref, CvRef::kConstRef);
        EXPECT_EQ(std::move(unex).transform(do_nothing).error().cvref, CvRef::kRRef);
        EXPECT_EQ(std::move(std::as_const(unex)).transform(do_nothing).error().cvref, CvRef::kConstRRef);

        static_assert(std::is_same_v<decltype(std::declval<ExIn&>().transform(do_nothing)), ExIn>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&>().transform(do_nothing)), ExIn>);
        static_assert(std::is_same_v<decltype(std::declval<ExIn&&>().transform(do_nothing)), ExIn>);
        static_assert(std::is_same_v<decltype(std::declval<const ExIn&&>().transform(do_nothing)), ExIn>);
    }
}
...
TEST(ExpectedVoid, TransformError)
{
    ...
    EXPECT_TRUE(std::move(std::as_const(ex)).transform_error(get_cvref).has_value());
