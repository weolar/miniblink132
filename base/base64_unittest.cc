...
TEST(Base64Test, Forgiving)
{
    ...
    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(::testing::Message() << EscapeAllExceptUnreserved(test_case.in));
        std::string output;
        bool success = Base64Decode(test_case.in, &output, Base64DecodePolicy::kForgiving);
...
TEST(Base64Test, Binary)
{
    ...
    std::string string_piece_encoded = Base64Encode(std::string_view(reinterpret_cast<const char*>(kData), sizeof(kData)));

    EXPECT_EQ(binary_encoded, string_piece_encoded);

    EXPECT_THAT(Base64Decode(binary_encoded), testing::Optional(testing::ElementsAreArray(kData)));
