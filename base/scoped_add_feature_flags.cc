...
{
    std::string enabled_features = command_line->GetSwitchValueASCII(switches::kEnableFeatures);
    std::string disabled_features = command_line->GetSwitchValueASCII(switches::kDisableFeatures);
    for (std::string_view feature : FeatureList::SplitFeatureListString(enabled_features)) {
        enabled_features_.emplace_back(feature);
    }
    for (std::string_view feature : FeatureList::SplitFeatureListString(disabled_features)) {
...
ScopedAddFeatureFlags::~ScopedAddFeatureFlags()
{
    ...
    command_line_->AppendSwitchASCII(switches::kEnableFeatures, JoinString(enabled_features_, ","));
    command_line_->RemoveSwitch(switches::kDisableFeatures);
    command_line_->AppendSwitchASCII(switches::kDisableFeatures, JoinString(disabled_features_, ","));
}
...
void ScopedAddFeatureFlags::EnableIfNotSetWithParameter(const Feature& feature, std::string_view name, std::string_view value)
{
...
bool ScopedAddFeatureFlags::IsEnabledWithParameter(const Feature& feature, std::string_view parameter_name, std::string_view parameter_value)
{
...
void ScopedAddFeatureFlags::AddFeatureIfNotSet(const Feature& feature, std::string_view suffix, bool enable)
{
