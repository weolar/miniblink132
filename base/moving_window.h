...
// Main template.
template <typename T, typename... Features> class MovingWindow;

// Convenience shortcuts.
template <typename T> using MovingMax = MovingWindow<T, MovingWindowFeatures::Max>;

template <typename T> using MovingMin = MovingWindow<T, MovingWindowFeatures::Min>;

template <typename T> using MovingMinMax = MovingWindow<T, MovingWindowFeatures::Min, MovingWindowFeatures::Max>;

template <typename T, typename SumType> using MovingAverage = MovingWindow<T, MovingWindowFeatures::Mean<SumType>>;

template <typename T> using MovingAverageDeviation = MovingWindow<T, MovingWindowFeatures::Mean<T>, MovingWindowFeatures::Deviation<double>>;
...
template <typename T, typename Comparator> class MovingExtremumBase {
public:
...
template <typename T> struct NullExtremumImpl {
...
template <typename T> class MovingWindowBase {
public:
    explicit MovingWindowBase(size_t window_size)
        : values_(window_size)
    {
    }
    ...
    bool IsLastIdx() const
    {
        return cur_idx_ + 1 == values_.size();
    }
...
// Null implementation of the above class to be used when feature is disabled.
template <typename T> struct NullWindowImpl {
...
// General template.
template <typename SumType, typename ReturnType> struct DivideInternal {
...
template <typename T, typename SumType, bool IsFloating> class MovingMeanBase {
public:
    explicit MovingMeanBase(size_t window_size)
        : sum_()
    {
    }
    ...
    template <typename ReturnType = SumType> ReturnType Mean(const size_t count) const
    {
        if (count == 0) {
            return ReturnType();
        }
...
// accumulation.
template <typename T, typename SumType> class MovingMeanBase<T, SumType, true> {
public:
    explicit MovingMeanBase(size_t window_size)
        : sum_()
        , running_sum_()
...
// Null implementation of the above class to be used when feature is disabled.
template <typename T> struct NullMeanImpl {
...
// General template.
template <typename SumType, typename ReturnType> struct DeivationInternal {
    static ReturnType Compute(const SumType& sum_squares, const SumType& square_of_sum, const size_t count)
    {
        return static_cast<ReturnType>(std::sqrt((static_cast<double>(sum_squares) - static_cast<double>(square_of_sum) / count) / count));
    }
};

// Class to compute square of the number.
// General template
template <typename T, typename SquareType> struct SquareInternal {
...
template <typename T, typename SumType, bool IsFloating> class MovingDeviationBase {
public:
    ...
    void AddSample(const T& sample, const T& replaced_value, bool is_last_idx)
    {
        sum_sq_ += SquareInternal<T, SumType>::Compute(sample) - SquareInternal<T, SumType>::Compute(replaced_value);
    }

    template <typename ReturnType, typename U> ReturnType Deviation(const size_t count, const U& sum) const
    {
        if (count == 0) {
            return ReturnType();
        }
        return DeivationInternal<SumType, ReturnType>::Compute(sum_sq_, SquareInternal<U, SumType>::Compute(sum), count);
    }
    void Reset()
    {
        sum_sq_ = SumType();
...
// Variant for float types with running sum to avoid rounding errors
// accumulation.
template <typename T, typename SumType> class MovingDeviationBase<T, SumType, true> {
public:
    explicit MovingDeviationBase(size_t window_size)
        : sum_sq_()
        , running_sum_()
...
// Null implementation of the above class to be used when feature is disabled.
template <typename T> struct NullDeviationImpl {
public:
...
template <typename... Features> struct EnabledFeatures : public Features... { };
...
template <typename T> struct get_type_mean {
    using type = void;
};
template <typename T>
...
template <typename T> using mean_t = typename get_type_mean<T>::type;

template <typename T> struct get_type_deviation {
    using type = void;
};
template <typename T>
...
template <typename T> using deviation_t = typename get_type_deviation<T>::type;

// Performs division allowing the class to work with more types.
// Specific template for TimeDelta.
template <> struct DivideInternal<TimeDelta, TimeDelta> {
...
// Deviation is equal to mean of squared values minus squared mean value.
// Specific template for TimeDelta.
template <> struct DeivationInternal<double, TimeDelta> {
    static TimeDelta Compute(const double sum_squares, const double square_of_sum, const size_t count)
    {
...
// Class to compute square of the number.
// Specific template for TimeDelta.
template <> struct SquareInternal<TimeDelta, double> {
...
public:
    ...
    void AddSample(const T& sample)
    {
        ...
        mean_impl_.AddSample(sample, window_impl_.GetValue(), window_impl_.IsLastIdx());
        deviation_impl_.AddSample(sample, window_impl_.GetValue(), window_impl_.IsLastIdx());
    ...
    {
        return mean_impl_.template Mean<ReturnType>(std::min(total_added_, window_impl_.Size()));
    }
    ...
    ReturnType Deviation() const
    {
        ...
        return deviation_impl_.template Deviation<ReturnType>(count, mean_impl_.Sum());
    }
        ...
        requires internal::has_member_iteration<EnabledFeatures>
    {
        ...
        size_t idx = (total_added_ < window_impl_.Size()) ? 0 : window_impl_.CurIdx();
...
private:
    ...
    std::conditional_t<internal::has_member_min<EnabledFeatures>, internal::MovingExtremumBase<T, std::greater<>>, internal::NullExtremumImpl<T>> min_impl_;

    // Member for calculating min.
    // Conditionally enabled on Min feature.
    std::conditional_t<internal::has_member_max<EnabledFeatures>, internal::MovingExtremumBase<T, std::less<>>, internal::NullExtremumImpl<T>> max_impl_;
    ...
    using MeanSumType
        = std::conditional_t<internal::has_member_mean<EnabledFeatures>, internal::mean_t<EnabledFeatures>, internal::deviation_t<EnabledFeatures>>;
    ...
    std::conditional_t<internal::has_member_mean<EnabledFeatures> || internal::has_member_deviation<EnabledFeatures>,
        internal::MovingMeanBase<T, MeanSumType, std::is_floating_point_v<MeanSumType>>, internal::NullMeanImpl<T>>
        mean_impl_;

    // Member for calculating deviation.
    // Conditionally enabled on Deviation feature.
    std::conditional_t<internal::has_member_deviation<EnabledFeatures>,
        internal::MovingDeviationBase<T, internal::deviation_t<EnabledFeatures>, std::is_floating_point_v<internal::deviation_t<EnabledFeatures>>>,
    ...
    std::conditional_t<internal::has_member_mean<EnabledFeatures> || internal::has_member_deviation<EnabledFeatures>
        ...
        internal::MovingWindowBase<T>, internal::NullWindowImpl<T>>
        window_impl_;
