...
using ScopedEventLogHandle = base::win::GenericScopedHandle<EventLogHandleTraits, base::win::DummyVerifierTraits>;

} // namespace

void SetEventSource(const std::string& name, uint16_t category, uint32_t event_id)
{
...
EventLogMessage::~EventLogMessage()
{
    ...
    ScopedEventLogHandle event_log_handle(RegisterEventSourceA(nullptr, g_event_source_name->c_str()));
    ...
    if (!ReportEventA(event_log_handle.get(), log_type, g_category, g_event_id, user_sid, 1, 0, strings, nullptr)) {
        stream() << " !!NOT ADDED TO EVENTLOG!!";
    }
