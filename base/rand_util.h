...
public:
    ...
    {
        uint64_t result;
        RAND_get_system_entropy_for_custom_prng(reinterpret_cast<uint8_t*>(&result), sizeof(result));
        return result;
    }
...
template <typename Itr> void RandomShuffle(Itr first, Itr last)
{
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(RandUtilTest, InsecureRandomGeneratorProducesBothValuesOfAllBits);
