...
template <typename S> bool EscapeJSONStringImpl(const S& str, bool put_in_quotes, std::string* dest)
{
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        if (!ReadUnicodeCharacter(str.data(), length, &i, &code_point) || code_point == CBU_SENTINEL) {
...
bool EscapeJSONString(std::string_view str, bool put_in_quotes, std::string* dest)
{
    return EscapeJSONStringImpl(str, put_in_quotes, dest);
}

bool EscapeJSONString(std::u16string_view str, bool put_in_quotes, std::string* dest)
{
    return EscapeJSONStringImpl(str, put_in_quotes, dest);
}
...
std::string EscapeBytesAsInvalidJSONString(std::string_view str, bool put_in_quotes)
{
    std::string dest;
    ...
    for (char c : str) {
        ...
        if (c < 32 || c > 126) {
            base::StringAppendF(&dest, kU16EscapeFormat, static_cast<unsigned char>(c));
        } else {
            dest.push_back(c);
        }
    }
