...
TEST(JsonWriterTest, NestedTypes)
{
    ...
    auto dict = Value::Dict().Set("list", Value::List().Append(Value::Dict().Set("inner int", 10)).Append(Value::Dict()).Append(Value::List()).Append(true));

    EXPECT_EQ(WriteJson(dict), "{\"list\":[{\"inner int\":10},{},[],true]}");

    // Test the pretty-printer.
    EXPECT_EQ(WriteJsonWithOptions(dict, JSONWriter::OPTIONS_PRETTY_PRINT), FixNewlines(R"({
   "list": [ {
      "inner int": 10
   }, {
   }, [  ], true ]
...
TEST(JsonWriterTest, BinaryValues)
{
    ...
    EXPECT_EQ(WriteJsonWithOptions(Value(kBinaryData), JsonOptions::OPTIONS_OMIT_BINARY_VALUES), "");

    auto binary_list = Value::List().Append(Value(kBinaryData)).Append(5).Append(Value(kBinaryData)).Append(2).Append(Value(kBinaryData));
    EXPECT_EQ(WriteJson(binary_list), std::nullopt);
    EXPECT_EQ(WriteJsonWithOptions(binary_list, JSONWriter::OPTIONS_OMIT_BINARY_VALUES), "[5,2]");

    auto binary_dict = Value::Dict().Set("a", Value(kBinaryData)).Set("b", 5).Set("c", Value(kBinaryData)).Set("d", 2).Set("e", Value(kBinaryData));
    EXPECT_EQ(WriteJson(binary_dict), std::nullopt);
    EXPECT_EQ(WriteJsonWithOptions(binary_dict, JSONWriter::OPTIONS_OMIT_BINARY_VALUES), R"({"b":5,"d":2})");
}
...
TEST(JsonWriterTest, DoublesAsInts)
{
    ...
    EXPECT_EQ(WriteJsonWithOptions(double_value, JSONWriter::OPTIONS_OMIT_DOUBLE_TYPE_PRESERVATION), "10000000000");
}
...
TEST(JsonWriterTest, StackOverflow)
{
    ...
    EXPECT_EQ(WriteJsonWithOptions(deep_list_value, JSONWriter::OPTIONS_PRETTY_PRINT), std::nullopt);
...
TEST(JsonWriterTest, TestMaxDepthWithValidNodes)
{
    ...
    ASSERT_OK_AND_ASSIGN(Value value, JSONReader::ReadAndReturnValueWithError(nested_json, JSON_ALLOW_TRAILING_COMMAS));
...
TEST(JsonWriterTest, JSONWriterWriteSuccess)
{
    ...
    EXPECT_TRUE(JSONWriter::Write(base::Value::Dict().Set("key", "value"), &output_js));
...
TEST(JsonWriterTest, JSONWriterWriteFailure)
{
    ...
    EXPECT_FALSE(JSONWriter::Write(base::Value::Dict() //
                                       .Set("key", base::Value::Dict().Set("nested-key", base::Value::Dict())),
...
{
    std::string output_js;
    EXPECT_TRUE(JSONWriter::WriteWithOptions(base::Value::Dict().Set("key", "value"), JSONWriter::OPTIONS_PRETTY_PRINT, &output_js));
...
TEST(JsonWriterTest, JSONWriterWriteWithOptionsFailure)
{
    ...
    EXPECT_FALSE(JSONWriter::WriteWithOptions(base::Value::Dict().Set("key", base::Value::Dict().Set("nested-key", base::Value::Dict())),
