...
const char kExtensionHistogramName[] = "Security.JSONParser.ChromiumExtensionUsage";
...
bool JSONParser::EatComment()
{
    ...
    if (comment_start == "//") {
        UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kCppComment);
        if (!comments_allowed) {
        ...
        UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kCComment);
        if (!comments_allowed) {
            ReportError(JSON_UNEXPECTED_TOKEN, 0);
            return false;
        }
...
std::optional<Value> JSONParser::ConsumeDictionary()
{
    ...
    return Value(Value::Dict(std::make_move_iterator(values.begin()), std::make_move_iterator(values.end())));
}
    ...
    for (;;) {
        ...
        case StringResult::kEscape:
            ...
            case 'x': { // UTF-8 sequence.
                ...
                // parser.
                UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kXEscape);
            ...
            case 'v': // Not listed as valid escape sequence in the RFC.
                UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kVerticalTabEscape);
...
std::pair<JSONParser::StringResult, std::string_view> JSONParser::ConsumeStringPart()
{
    ...
    while (std::optional<char> c = PeekChar()) {
        ...
        if (static_cast<unsigned char>(*c) >= kExtendedASCIIStart) {
            ...
            if (!ReadUnicodeCharacter(input_.data(), input_.length(), &index_, &next_char)) {
                ...
                ConsumeChar();
                return { StringResult::kReplacementCharacter, input_.substr(start_index, last_index - start_index) };
            }
        ...
        // through U+001F)".
        if (*c == '\n' || *c == '\r') {
            UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kNewlineInString);
            if (!(options_ & (JSON_ALLOW_NEWLINES_IN_STRINGS | JSON_ALLOW_CONTROL_CHARS))) {
                ReportError(JSON_UNSUPPORTED_ENCODING, -1);
                return { StringResult::kError, {} }; // No need to return consumed data.
            }
        } else if (*c <= 0x1F) {
            UmaHistogramEnumeration(kExtensionHistogramName, ChromiumJsonExtension::kControlCharacter);
...
bool JSONParser::DecodeUTF16(base_icu::UChar32* out_code_point)
{
    ...
    if (CBU16_IS_SURROGATE(code_unit16_high)) {
        ...
        base_icu::UChar32 code_point = CBU16_GET_SUPPLEMENTARY(code_unit16_high, code_unit16_low);
...
// static
std::string JSONParser::FormatErrorMessage(int line, int column, const std::string& description)
{
    if (line || column) {
