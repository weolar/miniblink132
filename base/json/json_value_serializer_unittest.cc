...
void CheckJSONIsStillTheSame(const Value& value)
{
    ...
    ReplaceSubstringsAfterOffset(&serialized_json, 0, kWinLineEnds, kLinuxLineEnds);
...
TEST(JSONValueDeserializerTest, ReadJSONWithTrailingCommasFromString)
{
    ...
    error_code = -789;
    JSONStringValueDeserializer str_deserializer2(kProperJSONWithCommas, JSON_ALLOW_TRAILING_COMMAS);
...
TEST(JSONValueDeserializerTest, ReadJSONWithCommasFromFile)
{
    ...
    // (-789) and ASSERT_EQ that it remains that nonsense value.
    error_code = -789;
    JSONFileValueDeserializer file_deserializer2(temp_file, JSON_ALLOW_TRAILING_COMMAS);
...
TEST(JSONValueDeserializerTest, AllowTrailingComma)
{
    ...
    JSONStringValueDeserializer deserializer(kTestWithCommas, JSON_ALLOW_TRAILING_COMMAS);
...
#define JSON_NEWLINE "\n"
    ...
    const std::string pretty_serialization
        = "{" JSON_NEWLINE "   \"bool\": true," JSON_NEWLINE "   \"double\": 3.14," JSON_NEWLINE "   \"int\": 42," JSON_NEWLINE
          "   \"list\": [ 1, 2 ]," JSON_NEWLINE "   \"null\": null" JSON_NEWLINE "}" JSON_NEWLINE;
#undef JSON_NEWLINE
