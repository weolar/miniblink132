...
bool JSONStringValueSerializer::SerializeAndOmitBinaryValues(base::ValueView root)
{
    return SerializeInternal(root, true);
}

bool JSONStringValueSerializer::SerializeInternal(base::ValueView root, bool omit_binary_values)
{
    if (!json_string_)
        return false;
...
JSONStringValueDeserializer::JSONStringValueDeserializer(std::string_view json_string, int options)
    : json_string_(json_string)
    , options_(options)
{
}
...
std::unique_ptr<Value> JSONStringValueDeserializer::Deserialize(int* error_code, std::string* error_str)
{
    auto ret = base::JSONReader::ReadAndReturnValueWithError(json_string_, options_);
    if (ret.has_value())
