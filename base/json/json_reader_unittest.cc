...
std::string_view MakeNotNullTerminatedInput(const char* input, std::unique_ptr<char[]>* owner)
{
...
public:
    void SetUp() override
    {
        feature_list_.InitWithFeatureState(base::features::kUseRustJsonParser, using_rust_);
    }
...
TEST_P(JSONReaderTest, EmbeddedComments)
{
    ...
    root = JSONReader::Read("{   \"cheese\": 3// Here's a comment // and another\n}");
    ASSERT_TRUE(root);
    EXPECT_TRUE(root->is_dict());
...
TEST_P(JSONReaderTest, Doubles)
{
    ...
    auto value = JSONReader::ReadAndReturnValueWithError("1e1000", JSON_PARSE_RFC);
...
{
    std::optional<Value> root = JSONReader::Read("\" \\\"\\\\\\/\\b\\f\\n\\r\\t\"");
    ASSERT_TRUE(root);
    ASSERT_TRUE(root->is_string());
    EXPECT_EQ(" \"\\/\b\f\n\r\t", root->GetString());
}
...
TEST_P(JSONReaderTest, UnicodeEscapes)
{
    ...
    std::optional<Value> root = JSONReader::Read("\"\\x41\\xFF\\x00\\u1234\\u0000\"");
    ASSERT_TRUE(root);
    ASSERT_TRUE(root->is_string());
...
TEST_P(JSONReaderTest, BasicArray)
{
    ...
    std::optional<Value> root2 = JSONReader::Read("[true, false, null, ]", JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(root2);
...
{
    std::optional<Value> value = JSONReader::Read("[[true], [], {\"smell\": \"nice\",\"taste\": \"yummy\" }, [false, [], "
                                                  "[null]], null]");
    ASSERT_TRUE(value);
    Value::List* list = value->GetIfList();
    ASSERT_TRUE(list);
    ...
    std::optional<Value> root2 = JSONReader::Read("[[true], [], {\"smell\": \"nice\",\"taste\": \"yummy\" }, [false, [], "
                                                  "[null, ]  , ], null,]",
...
TEST_P(JSONReaderTest, ArrayTrailingComma)
{
    ...
    std::optional<Value> value = JSONReader::Read("[true,]", JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(value);
    Value::List* list = value->GetIfList();
    ASSERT_TRUE(list);
...
{
    std::optional<Value> root1 = JSONReader::Read("{\"number\":9.87654321, \"null\":null , \"\\x53\" : \"str\", \"bool\": "
                                                  "false, \"more\": {} }");
    ASSERT_TRUE(root1);
    ...
    std::optional<Value> root2 = JSONReader::Read("{\"number\":9.87654321, \"null\":null , \"\\x53\" : \"str\", \"bool\": "
                                                  "false, \"more\": {},}",
...
{
    std::optional<Value> root1 = JSONReader::Read("{\"inner\":{\"array\":[true, 3, 4.56, null]},\"false\":false,\"d\":{}}");
    ASSERT_TRUE(root1);
    ...
    std::optional<Value> root2 = JSONReader::Read("{\"inner\": {\"array\":[true, 3, 4.56, null] , "
                                                  "},\"false\":false,\"d\":{},}",
        JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(root2);
...
{
    std::optional<Value> root = JSONReader::Read("{\"a.b\":3,\"c\":2,\"d.e.f\":{\"g.h.i.j\":1}}");
    ASSERT_TRUE(root);
...
TEST_P(JSONReaderTest, InvalidDictionaries)
{
    ...
    EXPECT_FALSE(JSONReader::Read("{\"a\":true,,\"b\":false}", JSON_ALLOW_TRAILING_COMMAS));
...
TEST_P(JSONReaderTest, UTF8Input)
{
    ...
    for (auto* noncharacter : noncharacters) {
        ...
        ASSERT_TRUE(root);
        ASSERT_TRUE(root->is_string());
        EXPECT_EQ(std::string(noncharacter + 1, strlen(noncharacter) - 2), root->GetString());
    }
}
...
TEST_P(JSONReaderTest, ReadFromFile)
{
    ...
    EXPECT_THAT(JSONReader::ReadAndReturnValueWithError(input), base::test::ValueIs(::testing::Property(&base::Value::is_dict, true)));
}
...
TEST_P(JSONReaderTest, ReplaceInvalidCharacters)
{
    ...
    std::optional<Value> value = JSONReader::Read(invalid_high, JSON_REPLACE_INVALID_CHARACTERS);
    ASSERT_TRUE(value);
...
TEST_P(JSONReaderTest, ParseNumberErrors)
{
    const struct {
        ...
        // clang-format on
        { "1", true, 1 }, { "2.", false, 0 }, { "42", true, 42 }, { "6e", false, 0 }, { "43e2", true, 4300 }, { "43e-", false, 0 }, { "9e-3", true, 0.009 },
        { "2e+", false, 0 }, { "2e+2", true, 200 },
        // clang-format on
    };
    ...
    for (unsigned int i = 0; i < std::size(kCases); ++i) {
        ...
        std::string_view input = MakeNotNullTerminatedInput(test_case.input, &input_owner);
...
TEST_P(JSONReaderTest, UnterminatedInputs)
{
    ...
    for (unsigned int i = 0; i < std::size(kCases); ++i) {
        ...
        std::unique_ptr<char[]> input_owner;
        std::string_view input = MakeNotNullTerminatedInput(test_case, &input_owner);
...
TEST_P(JSONReaderTest, LineColumnCounting)
{
    ...
    for (unsigned int i = 0; i < std::size(kCases); ++i) {
        ...
        auto root = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_RFC | JSON_ALLOW_NEWLINES_IN_STRINGS);
        EXPECT_FALSE(root.has_value());
...
TEST_P(JSONReaderTest, ChromiumExtensions)
{
    ...
    for (size_t i = 0; i < std::size(kCases); ++i) {
        ...
        auto result = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_RFC);
        EXPECT_FALSE(result.has_value());

        result = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_RFC | test_case.option);
        EXPECT_TRUE(result.has_value());

        result = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_CHROMIUM_EXTENSIONS);
        EXPECT_TRUE(result.has_value());

        result = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_CHROMIUM_EXTENSIONS & ~test_case.option);
        EXPECT_FALSE(result.has_value());
    }
}
...
INSTANTIATE_TEST_SUITE_P(All, JSONReaderTest, testing::Bool(), [](const testing::TestParamInfo<bool>& info) { return info.param ? "Rust" : "Cpp"; });
