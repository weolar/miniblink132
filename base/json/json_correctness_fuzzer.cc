...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    auto input = base::HeapArray<char>::CopiedFrom(base::as_chars(all_input.first(size - 1)));

    std::string_view input_string = base::as_string_view(input.as_span());

    const int options = all_input[size - 1];
    auto result = base::JSONReader::ReadAndReturnValueWithError(input_string, options);
    if (!result.has_value())
        return 0;
    ...
    auto double_result = base::JSONReader::ReadAndReturnValueWithError(parsed_output, options);
    ...
    LOG_ASSERT(parsed_output == double_parsed_output) << "Parser/Writer mismatch."
