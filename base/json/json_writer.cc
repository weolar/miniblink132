...
// static
bool JSONWriter::WriteWithOptions(ValueView node, int options, std::string* json, size_t max_depth)
{
    json->clear();
    ...
    bool result = node.Visit([&writer](const auto& member) { return writer.BuildJSONString(member, 0); });
...
{
    if (omit_double_type_preservation_ && IsValueInRangeForNumericType<int64_t>(node) && std::floor(node) == node) {
...
bool JSONWriter::BuildJSONString(const Value::Dict& node, size_t depth)
{
    ...
    for (const auto [key, value] : node) {
        ...
        result &= value.Visit([this, depth = depth + 1](const auto& member) { return BuildJSONString(member, depth); });
...
bool JSONWriter::BuildJSONString(const Value::List& node, size_t depth)
{
    ...
    for (const auto& value : node) {
        ...
        result &= value.Visit([this, depth](const auto& member) { return BuildJSONString(member, depth); });
...
std::optional<std::string> WriteJsonWithOptions(ValueView node, uint32_t options, size_t max_depth)
{
    std::string result;
    if (!JSONWriter::WriteWithOptions(node, static_cast<int>(options), &result, max_depth)) {
        return std::nullopt;
    }
    return result;
}
