...
template <class T, class As = T> void ListAppendValue(ContextPointer& ctx, T v)
{
    auto& value = reinterpret_cast<base::Value&>(ctx);
    value.GetList().Append(As { v });
}

ContextPointer& DictSetList(ContextPointer& ctx, rust::Str key, size_t reserve)
{
    ...
    return reinterpret_cast<ContextPointer&>(*dict.Find(base::RustStrToStringView(key)));
}
...
template <class T, class As = T> void DictSetValue(ContextPointer& ctx, rust::Str key, T v)
{
    auto& dict = reinterpret_cast<base::Value&>(ctx).GetDict();
    dict.Set(base::RustStrToStringView(key), base::Value(As { v }));
}

JSONReader::Result DecodeJSONInRust(std::string_view json, int options, size_t max_depth)
{
    const serde_json_lenient::JsonOptions rust_options = {
        .allow_trailing_commas = (options & base::JSON_ALLOW_TRAILING_COMMAS) != 0,
        .replace_invalid_characters = (options & base::JSON_REPLACE_INVALID_CHARACTERS) != 0,
    ...
    bool ok = serde_json_lenient::decode_json(base::StringViewToRustSlice(json), rust_options, functions, ctx, error);
...
// static
std::optional<Value> JSONReader::Read(std::string_view json, int options, size_t max_depth)
{
#if 0 // BUILDFLAG(IS_NACL)
...
// static
std::optional<Value::Dict> JSONReader::ReadDict(std::string_view json, int options, size_t max_depth)
{
...
// static
JSONReader::Result JSONReader::ReadAndReturnValueWithError(std::string_view json, int options)
{
#if 0 // BUILDFLAG(IS_NACL)
