...
constexpr char kExpectedChromiumTestsRoot[] = "/storage/emulated/0/chromium_tests_root";
#endif
...
#endif
#if BUILDFLAG(IS_MAC)
    if (key != DIR_EXE && key != DIR_MODULE && key != FILE_EXE && key != FILE_MODULE) {
...
TEST_F(PathServiceTest, Get)
{
    ...
    constexpr std::array kUnsupportedKeys = { // Though DIR_HOME is not intended to be supported, PathProviderPosix
    ...
    constexpr std::array kUnsupportedKeys = { // TODO(crbug.com/42050322): Implement DIR_USER_DESKTOP.
        DIR_USER_DESKTOP
    };
#else
    ...
    for (int key = PATH_START + 1; key < PATH_END; ++key) {
        EXPECT_PRED1(Contains(kUnsupportedKeys, key) ? &ReturnsInvalidPath : &ReturnsValidPath, key);
    }
#if BUILDFLAG(IS_WIN)
    for (int key = PATH_WIN_START + 1; key < PATH_WIN_END; ++key) {
        if (key == DIR_SYSTEM_TEMP) {
            EXPECT_PRED1(::IsUserAnAdmin() ? &ReturnsValidPath : &ReturnsInvalidPath, key);
        } else {
...
TEST_F(PathServiceTest, Override)
{
    ...
    PathService::OverrideAndCreateIfNeeded(my_special_key, fake_cache_dir2, false, false);
    EXPECT_FALSE(PathExists(fake_cache_dir2));
    EXPECT_TRUE(PathService::OverrideAndCreateIfNeeded(my_special_key, fake_cache_dir2, false, true));
    EXPECT_TRUE(PathExists(fake_cache_dir2));

#if BUILDFLAG(IS_POSIX)
    FilePath non_existent(MakeAbsoluteFilePath(temp_dir.GetPath()).AppendASCII("non_existent"));
    ...
    EXPECT_FALSE(PathService::OverrideAndCreateIfNeeded(my_special_key, non_existent, false, false));
#endif // !BUILDFLAG(IS_ANDROID)
    // This works because indicating that |non_existent| is absolute skips the
    // internal MakeAbsoluteFilePath call.
    EXPECT_TRUE(PathService::OverrideAndCreateIfNeeded(my_special_key, non_existent, true, false));
...
TEST_F(PathServiceTest, GetProgramFiles)
{
    ...
    EXPECT_EQ(programfiles_dir.value(), FILE_PATH_LITERAL("C:\\Program Files (x86)"));
    EXPECT_TRUE(PathService::Get(DIR_PROGRAM_FILES6432, &programfiles_dir));
    EXPECT_EQ(programfiles_dir.value(), FILE_PATH_LITERAL("C:\\Program Files"));
#else
    if (base::win::OSInfo::GetInstance()->IsWowX86OnAMD64() || base::win::OSInfo::GetInstance()->IsWowX86OnARM64()) {
        // 32-bit on 64-bit.
        EXPECT_TRUE(PathService::Get(DIR_PROGRAM_FILES, &programfiles_dir));
        EXPECT_EQ(programfiles_dir.value(), FILE_PATH_LITERAL("C:\\Program Files (x86)"));
        EXPECT_TRUE(PathService::Get(DIR_PROGRAM_FILESX86, &programfiles_dir));
...
TEST_F(PathServiceTest, GetSystemTemp)
{
    ...
    EXPECT_EQ(PathService::Get(DIR_SYSTEM_TEMP, &secure_system_temp), ::IsUserAnAdmin());
    if (!secure_system_temp.empty()) {
        ...
        ASSERT_TRUE((dir_windows.AppendASCII("SystemTemp") == secure_system_temp) || (dir_program_files == secure_system_temp));
    }
}
#endif // BUILDFLAG(IS_WIN)
...
TEST_F(PathServiceTest, DIR_GEN_TEST_DATA_ROOT)
{
    ...
    EXPECT_TRUE(base::PathExists(path.Append(FILE_PATH_LITERAL("base/generated_file_for_test.txt"))));
}

#if ((BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_ANDROID)) || BUILDFLAG(IS_WIN))
...
#if BUILDFLAG(IS_WIN)
    auto scoped_env = base::ScopedEnvironmentVariableOverride("CR_SOURCE_ROOT", base::WideToUTF8(tempdir.GetPath().value()));
#else
    auto scoped_env = base::ScopedEnvironmentVariableOverride("CR_SOURCE_ROOT", tempdir.GetPath().value());
#endif
...
TEST_F(PathServiceTest, SetTestDataRootAsRelativePath)
{
    ...
    PathService::DisableCache();
#if BUILDFLAG(IS_WIN)
    auto scoped_env = base::ScopedEnvironmentVariableOverride("CR_SOURCE_ROOT", base::WideToUTF8(base::FilePath::kParentDirectory));
#else
    auto scoped_env = base::ScopedEnvironmentVariableOverride("CR_SOURCE_ROOT", base::FilePath::kParentDirectory);
#endif
    base::FilePath path;
...
{
    EXPECT_EQ(PathService::CheckedGet(DIR_ANDROID_APP_DATA).value(), kExpectedChromiumTestsRoot);
    EXPECT_EQ(PathService::CheckedGet(DIR_ASSETS).value(), kExpectedChromiumTestsRoot);
    EXPECT_EQ(PathService::CheckedGet(DIR_SRC_TEST_DATA_ROOT).value(), kExpectedChromiumTestsRoot);
    EXPECT_EQ(PathService::CheckedGet(DIR_OUT_TEST_DATA_ROOT).value(), kExpectedChromiumTestsRoot);
}
