...
bool NeedsLazyInstance(std::atomic<uintptr_t>& state)
{
    ...
    if (state.compare_exchange_strong(expected, kLazyInstanceStateCreating, std::memory_order_relaxed, std::memory_order_relaxed)) {
...
void CompleteLazyInstance(std::atomic<uintptr_t>& state, uintptr_t new_instance, void (*destructor)(void*), void* destructor_arg)
{
