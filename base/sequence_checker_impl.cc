...
SequenceCheckerImpl& SequenceCheckerImpl::operator=(SequenceCheckerImpl&& other)
{
    // Verify that `other` is called on the correct thread.
    // Note: This binds `other` if not already bound.
    CHECK(other.CalledOnValidSequence());
...
bool SequenceCheckerImpl::CalledOnValidSequence(std::unique_ptr<debug::StackTrace>* out_bound_at) const
{
    AutoLock auto_lock(lock_);
    EnsureAssigned();
    ...
    bool is_valid = (sequence_token_ == internal::SequenceToken::GetForCurrentThread());
    ...
    if (!is_valid) {
        is_valid = ThreadLocalStorage::HasBeenDestroyed() && thread_ref_ == PlatformThread::CurrentRef();
    }
    ...
    std::erase_if(locks_, [](uintptr_t lock_ptr) { return !Contains(subtle::GetTrackedLocksHeldByCurrentThread(), lock_ptr); });
#endif // DCHECK_IS_ON()
...
void SequenceCheckerImpl::EnsureAssigned() const
{
    ...
    DCHECK(locks_.empty());
    ranges::remove_copy(subtle::GetTrackedLocksHeldByCurrentThread(), std::back_inserter(locks_), reinterpret_cast<uintptr_t>(&lock_));
#endif // DCHECK_IS_ON()
