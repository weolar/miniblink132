...
void GetRegionInfo(vm_address_t* region_address, vm_size_t* region_size)
{
    ...
    kern_return_t kr = vm_region_64(
        mach_task_self(), region_address, region_size, VM_REGION_BASIC_INFO_64, reinterpret_cast<vm_region_info_t>(&region_info), &count, &object);
...
TEST(ScopedMachVMTest, ResetLargerAddressAndSize)
{
    ...
    vm_address_t address;
    kern_return_t kr = vm_allocate(mach_task_self(), &address, kThreePages, VM_FLAGS_ANYWHERE);
    ASSERT_EQ(KERN_SUCCESS, kr);
...
TEST(ScopedMachVMTest, ResetLargerAddress)
{
    ...
    vm_address_t address;
    kern_return_t kr = vm_allocate(mach_task_self(), &address, kSixPages, VM_FLAGS_ANYWHERE);
    ASSERT_EQ(KERN_SUCCESS, kr);
...
TEST(ScopedMachVMTest, ResetUnaligned)
{
    ...
    vm_address_t address;
    kern_return_t kr = vm_allocate(mach_task_self(), &address, kTwoPages, VM_FLAGS_ANYWHERE);
    ASSERT_EQ(KERN_SUCCESS, kr);
