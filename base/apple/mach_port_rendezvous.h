...
protected:
    ...
    // retrieve ports.
    virtual std::optional<MachPortsForRendezvous> PortsForClient(audit_token_t audit_token) = 0;
    ...
    virtual std::vector<uint8_t> AdditionalDataForReply(mach_msg_id_t request) const = 0;
    ...
    // `additional_data`.
    std::unique_ptr<uint8_t[]> CreateReplyMessage(mach_port_t reply_port, const MachPortsForRendezvous& ports, std::vector<uint8_t> additional_data);
};
...
class BASE_EXPORT MachPortRendezvousServerIOS final : public MachPortRendezvousServerBase {
public:
    ...
    MachPortRendezvousServerIOS& operator=(const MachPortRendezvousServerIOS&) = delete;
...
// processes.
class BASE_EXPORT MachPortRendezvousServerMac final : public MachPortRendezvousServerBase {
public:
    ...
    MachPortRendezvousServerMac& operator=(const MachPortRendezvousServerMac&) = delete;
    ...
    void RegisterPortsForPid(pid_t pid, const MachPortsForRendezvous& ports) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    ...
    //
    // This must be called with the lock from GetLock() held.
    void SetProcessRequirementForPid(pid_t pid, mac::ProcessRequirement requirement) EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    ...
    // registered.
    Lock& GetLock() LOCK_RETURNED(lock_)
    {
        return lock_;
    }
...
protected:
    ...
    // `client_data_`.
    std::optional<MachPortsForRendezvous> PortsForClient(audit_token_t audit_token) override;

    bool IsValidAdditionalMessageId(mach_msg_id_t) const override;
    std::vector<uint8_t> AdditionalDataForReply(mach_msg_id_t request) const override;
...
public:
    ...
    apple::ScopedMachSendRight TakeSendRight(MachPortsForRendezvous::key_type key);

    // Returns the Mach receive right that was registered with |key|. If no such
    // right exists, or it was already taken, returns an invalid right. Safe to
    // call from any thread. DCHECKs if the right referenced by |key| is not a
    // receive right.
    apple::ScopedMachReceiveRight TakeReceiveRight(MachPortsForRendezvous::key_type key);
...
protected:
    ...
    // that sent it.
    virtual bool ValidateMessage(mach_msg_base_t* message, BufferIterator<uint8_t> body) = 0;

    // Sends the actual IPC message to |server_port| and parses the reply.
    bool SendRequest(apple::ScopedMachSendRight server_port, mach_msg_id_t request_id, size_t additional_response_data_size = 0)
...
#if BUILDFLAG(IS_IOS)
BASE_EXPORT
class BASE_EXPORT MachPortRendezvousClientIOS final : public MachPortRendezvousClient {
public:
    // Initialize the MacPortRendezvousClient using `server_port`.
    static bool Initialize(apple::ScopedMachSendRight server_port);

protected:
    bool ValidateMessage(mach_msg_base_t* message, BufferIterator<uint8_t> body) override;
...
class BASE_EXPORT MachPortRendezvousClientMac final : public MachPortRendezvousClient {
public:
    ...
    static MachPortRendezvousPeerValidationPolicy PeerValidationPolicyForTesting();
...
private:
    ...
    static std::optional<mac::ProcessRequirement> TakeServerCodeSigningRequirement();
