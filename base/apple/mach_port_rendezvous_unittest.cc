...
test::ScopedFeatureList ApplyPeerValidationPolicy(MachPortRendezvousPeerValidationPolicy policy)
{
    switch (policy) {
    case MachPortRendezvousPeerValidationPolicy::kNoValidation:
        return {};
    case MachPortRendezvousPeerValidationPolicy::kValidateOnly:
        return test::ScopedFeatureList(kMachPortRendezvousValidatePeerRequirements);
    case MachPortRendezvousPeerValidationPolicy::kEnforce:
        return test::ScopedFeatureList(kMachPortRendezvousEnforcePeerRequirements);
    }
}

} // namespace

class MachPortRendezvousServerTest : public MultiProcessTest, public testing::WithParamInterface<MachPortRendezvousPeerValidationPolicy> {
public:
...
MULTIPROCESS_TEST_MAIN(TakeSendRight)
{
    ...
    apple::ScopedMachSendRight port = rendezvous_client->TakeSendRight(kTestPortKey);
    CHECK(port.is_valid());
...
TEST_P(MachPortRendezvousServerTest, SendRight)
{
    ...
    Process child;
    {
        ...
        server->RegisterPortsForPid(child.Pid(), { std::make_pair(kTestPortKey, rendezvous_port) });
    }

    struct : mach_msg_base_t {
        mach_msg_trailer_t trailer;
    } msg {};
    kr = mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0, sizeof(msg), port.get(), TestTimeouts::action_timeout().InMilliseconds(), MACH_PORT_NULL);
...
TEST_P(MachPortRendezvousServerTest, DestroyRight)
{
    const struct {
        ...
        { true, MACH_MSG_TYPE_MOVE_RECEIVE, true, 0 }, { true, MACH_MSG_TYPE_MOVE_SEND, false, 0 }, { true, MACH_MSG_TYPE_COPY_SEND, false, 1 },
        { true, MACH_MSG_TYPE_MAKE_SEND, false, 1 }, { false, MACH_MSG_TYPE_MAKE_SEND, false, 0 }, { true, MACH_MSG_TYPE_MAKE_SEND_ONCE, false, 1 },
    ...
    for (size_t i = 0; i < std::size(kCases); ++i) {
        ...
        kr = mach_port_get_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND, &refs);
        ASSERT_EQ(kr, KERN_SUCCESS);
...
TEST_P(MachPortRendezvousServerTest, ValidateChildCodeSigningRequirement_Success)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);
    ...
    Process child;
    {
        ...
        server->RegisterPortsForPid(child.Pid(), { std::make_pair(kTestPortKey, rendezvous_port) });
        server->SetProcessRequirementForPid(child.Pid(), mac::ProcessRequirement::AlwaysMatchesForTesting());
    }
...
MULTIPROCESS_TEST_MAIN(ValidateChildCodeSigningRequirement_Failed)
{
    ...
    CHECK_EQ(policy == MachPortRendezvousPeerValidationPolicy::kEnforce ? 0u : 1u, rendezvous_client->GetPortCount());

    return 0;
}

TEST_P(MachPortRendezvousServerTest, ValidateChildCodeSigningRequirement_Failed)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);
    ...
    Process child;
    {
        ...
        server->RegisterPortsForPid(child.Pid(), { std::make_pair(kTestPortKey, rendezvous_port) });
        server->SetProcessRequirementForPid(child.Pid(), mac::ProcessRequirement::NeverMatchesForTesting());
    }
...
{
    MachPortRendezvousClientMac::SetServerProcessRequirement(mac::ProcessRequirement::AlwaysMatchesForTesting());
    auto* rendezvous_client = MachPortRendezvousClient::GetInstance();
    CHECK(rendezvous_client);
...
TEST_P(MachPortRendezvousServerTest, ValidateParentCodeSigningRequirement_Success)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);
...
TEST_P(MachPortRendezvousServerTest, ValidateParentCodeSigningRequirement_NoRights_Success)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);

    Process child = SpawnChild("ValidateParentCodeSigningRequirement_NoRights_Success");
...
{
    MachPortRendezvousClientMac::SetServerProcessRequirement(mac::ProcessRequirement::NeverMatchesForTesting());
    auto policy = MachPortRendezvousClientMac::PeerValidationPolicyForTesting();
    auto* rendezvous_client = MachPortRendezvousClient::GetInstance();
    CHECK(policy == MachPortRendezvousPeerValidationPolicy::kEnforce ? !rendezvous_client : bool(rendezvous_client));
    return 0;
}

TEST_P(MachPortRendezvousServerTest, ValidateParentCodeSigningRequirement_Failure)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);
...
TEST_P(MachPortRendezvousServerTest, ValidateParentCodeSigningRequirement_NoRights_Failure)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);

    Process child = SpawnChild("ValidateParentCodeSigningRequirement_NoRights_Failure");
...
{
    mac::ProcessRequirement requirement = mac::ProcessRequirement::AlwaysMatchesForTesting();
...
TEST_P(MachPortRendezvousServerTest, ValidateParentCodeSigningRequirement_DynamicOnly)
{
    auto* server = MachPortRendezvousServerMac::GetInstance();
    ASSERT_TRUE(server);

    Process child = SpawnChild("ValidateParentCodeSigningRequirement_DynamicOnly");
...
INSTANTIATE_TEST_SUITE_P(All, MachPortRendezvousServerTest,
    testing::Values(MachPortRendezvousPeerValidationPolicy::kNoValidation, MachPortRendezvousPeerValidationPolicy::kValidateOnly,
