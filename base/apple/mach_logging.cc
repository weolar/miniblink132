...
void MachLogMessage::AppendError()
{
    ...
    stream() << ": " << mach_error_string(mach_err_) << FormatMachErrorNumber(mach_err_);
}
...
BootstrapLogMessage::BootstrapLogMessage(const char* file_path, int line, LogSeverity severity, kern_return_t bootstrap_err)
    : LogMessage(file_path, line, severity)
