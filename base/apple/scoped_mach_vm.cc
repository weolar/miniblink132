...
void ScopedMachVM::reset_unaligned(vm_address_t address, vm_size_t size)
{
    if (size_) {
        if (address_ < address) {
            kern_return_t kr = vm_deallocate(mach_task_self(), address_, std::min(size_, address - address_));
        ...
        if (address_ + size_ > address + size) {
            ...
            kern_return_t kr = vm_deallocate(mach_task_self(), deallocate_start, address_ + size_ - deallocate_start);
            MACH_DCHECK(kr == KERN_SUCCESS, kr) << "vm_deallocate";
        }
    }
