...
BASE_EXPORT bool GetSearchPathDirectory(NSSearchPathDirectory directory, NSSearchPathDomainMask domain_mask, FilePath* result);

// Searches for directories for the given key in only the local domain.
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
BASE_EXPORT bool GetLocalDirectory(NSSearchPathDirectory directory, FilePath* result);

// Searches for directories for the given key in only the user domain.
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
BASE_EXPORT bool GetUserDirectory(NSSearchPathDirectory directory, FilePath* result);
...
#define TYPE_NAME_FOR_CF_TYPE_DECL(TypeCF) BASE_EXPORT std::string TypeNameForCFType(TypeCF##Ref)
...
template <typename T> T CFCast(const CFTypeRef& cf_val);

template <typename T> T CFCastStrict(const CFTypeRef& cf_val);

#define CF_CAST_DECL(TypeCF)                                                                                                                                   \
    template <> BASE_EXPORT TypeCF##Ref CFCast<TypeCF##Ref>(const CFTypeRef& cf_val);                                                                          \
                                                                                                                                                               \
    template <> BASE_EXPORT TypeCF##Ref CFCastStrict<TypeCF##Ref>(const CFTypeRef& cf_val)
...
template <typename T> T* ObjCCast(id objc_val)
{
...
template <typename T> T* ObjCCastStrict(id objc_val)
{
...
BASE_EXPORT std::string GetValueFromDictionaryErrorMessage(CFStringRef key, const std::string& expected_type, CFTypeRef value);

// Utility function to pull out a value from a dictionary, check its type, and
// return it. Returns NULL if the key is not present or of the wrong type.
template <typename T> T GetValueFromDictionary(CFDictionaryRef dict, CFStringRef key)
{
    ...
    if (value && !value_specific) {
        ...
        DLOG(WARNING) << GetValueFromDictionaryErrorMessage(key, expected_type, value);
    }
...
BASE_EXPORT ScopedCFTypeRef<CFStringRef> FilePathToCFString(const FilePath& path);
...
inline span<const uint8_t> NSDataToSpan(NSData* data)
{
    ...
    return UNSAFE_BUFFERS(make_span(static_cast<const uint8_t*>(data.bytes), data.length));
}
...
inline span<uint8_t> NSMutableDataToSpan(NSMutableData* data)
{
    ...
    // in size.
    return UNSAFE_BUFFERS(make_span(static_cast<uint8_t*>(data.mutableBytes), data.length));
}
...
BASE_EXPORT extern std::ostream& operator<<(std::ostream& o, const CFErrorRef err);
BASE_EXPORT extern std::ostream& operator<<(std::ostream& o, const CFStringRef str);
