...
void SendRightTraits::Free(mach_port_t port)
{
    ...
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "ScopedMachSendRight mach_port_deallocate";
}
...
void ReceiveRightTraits::Free(mach_port_t port)
{
    ...
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "ScopedMachReceiveRight mach_port_mod_refs";
}
...
void PortSetTraits::Free(mach_port_t port)
{
    ...
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "ScopedMachPortSet mach_port_mod_refs";
}

} // namespace internal

bool CreateMachPort(ScopedMachReceiveRight* receive, ScopedMachSendRight* send, std::optional<mach_port_msgcount_t> queue_limit)
{
    ...
    kern_return_t kr = mach_port_construct(mach_task_self(), &options, 0, ScopedMachReceiveRight::Receiver(*receive).get());
    if (kr != KERN_SUCCESS) {
