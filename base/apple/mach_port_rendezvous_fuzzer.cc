...
struct MachPortRendezvousFuzzer {
    MachPortRendezvousFuzzer()
    {
        ...
        mach_port_t port = base::MachPortRendezvousServerMac::GetInstance()->server_port_.get();
    ...
    void ClearClientData() EXCLUSIVE_LOCKS_REQUIRED(base::MachPortRendezvousServerMac::GetInstance()->GetLock())
    {
        base::MachPortRendezvousServerMac::GetInstance()->ClearClientDataForTesting();
    }
...
DEFINE_BINARY_PROTO_FUZZER(const mach_fuzzer::MachMessage& message)
{
    ...
    {
        base::AutoLock lock(base::MachPortRendezvousServerMac::GetInstance()->GetLock());
        environment.ClearClientData();
        base::MachPortRendezvousServerMac::GetInstance()->RegisterPortsForPid(
            getpid(), { std::make_pair(0xbadbeef, base::MachRendezvousPort { mach_task_self(), MACH_MSG_TYPE_COPY_SEND }) });
    }
