...
TEST(ScopedCFTypeRefTest, ConstructionSameType)
{
    ...
    ScopedCFTypeRef<CFMutableArrayRef> retain_scoper(array, base::scoped_policy::RETAIN);
    EXPECT_EQ(array, retain_scoper.get());
    EXPECT_EQ(2, CFGetRetainCount(array));

    ScopedCFTypeRef<CFMutableArrayRef> assume_scoper(array, base::scoped_policy::ASSUME);
    EXPECT_EQ(array, assume_scoper.get());
    EXPECT_EQ(2, CFGetRetainCount(array));

    CFMutableArrayRef array2 = CFArrayCreateMutable(nullptr, /*capacity=*/0, &kCFTypeArrayCallBacks);
...
{
    ScopedCFTypeRef<CFMutableArrayRef> original(CFArrayCreateMutable(nullptr, /*capacity=*/0, &kCFTypeArrayCallBacks));
...
TEST(ScopedCFTypeRefTest, CopyAssignmentSameType)
{
    ...
    ScopedCFTypeRef<CFMutableArrayRef> new_object(CFArrayCreateMutable(nullptr, /*capacity=*/0, &kCFTypeArrayCallBacks));
...
{
    ScopedCFTypeRef<CFArrayRef> original(CFArrayCreateMutable(nullptr, /*capacity=*/0, &kCFTypeArrayCallBacks));
    EXPECT_EQ(1, CFGetRetainCount(original.get()));
