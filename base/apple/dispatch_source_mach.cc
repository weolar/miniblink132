...
DispatchSourceMach::DispatchSourceMach(const char* name, mach_port_t port, void (^event_handler)())
    : DispatchSourceMach(dispatch_queue_create(name, DISPATCH_QUEUE_SERIAL), port, event_handler)
{
...
DispatchSourceMach::DispatchSourceMach(dispatch_queue_t queue, mach_port_t port, void (^event_handler)())
    : storage_(std::make_unique<Storage>())
{
    storage_->queue.reset(queue, base::scoped_policy::RETAIN);
    storage_->source.reset(dispatch_source_create(DISPATCH_SOURCE_TYPE_MACH_RECV, port, 0, storage_->queue.get()));
...
DispatchSourceMach::~DispatchSourceMach()
{
    ...
    dispatch_semaphore_wait(storage_->source_canceled.get(), DISPATCH_TIME_FOREVER);
}
