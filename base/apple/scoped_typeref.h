...
public:
    ...
    explicit constexpr ScopedTypeRef(element_type object = Traits::InvalidValue(), base::scoped_policy::OwnershipPolicy policy = base::scoped_policy::ASSUME)
        : object_(object)
    {
        if (object_ != Traits::InvalidValue() && policy == base::scoped_policy::RETAIN) {
    ...
    ScopedTypeRef(const ScopedTypeRef<R, RTraits>& that)
        : object_(that.get())
    {
        if (object_ != Traits::InvalidValue()) {
            object_ = Traits::Retain(object_);
    ...
    template <typename R, typename RTraits> ScopedTypeRef& operator=(const ScopedTypeRef<R, RTraits>& that)
    {
        reset(that.get(), base::scoped_policy::RETAIN);
        return *this;
    }
    ...
    ScopedTypeRef(ScopedTypeRef<T, Traits>&& that)
        : object_(that.release())
    {
    }

    template <typename R, typename RTraits>
    ScopedTypeRef(ScopedTypeRef<R, RTraits>&& that)
        : object_(that.release())
    {
    }
    ...
    template <typename R, typename RTraits> ScopedTypeRef& operator=(ScopedTypeRef<R, RTraits>&& that)
    {
        reset(that.release(), base::scoped_policy::ASSUME);
        return *this;
    }
    ...
    template <typename R, typename RTraits> void reset(const ScopedTypeRef<R, RTraits>& that)
    {
        reset(that.get(), base::scoped_policy::RETAIN);
    }
    ...
    {
        if (object != Traits::InvalidValue() && policy == base::scoped_policy::RETAIN) {
