...
template <class H> void ExpectHeap(const H& heap)
{
    ...
    for (size_t i = 0; i < heap.size(); ++i) {
        ...
        intrusive_heap::CheckInvalidOrEqualTo(handle_access.GetHeapHandle(&heap[i]), i);
    }
}
...
enum OperationTypes : int { kGrowing, kShrinking, kSameSize, kOperationTypesCount };
...
enum ShrinkingOperations : int { kTake, kTakeTop, kErase, kPop, kShrinkingOperationsCount };

// The operations that keep a heap the same size.
enum SameSizeOperations : int { kReplace, kReplaceTop, kUpdate, kSameSizeOperationsCount };
...
template <typename T> void DoGrowingOperation(IntrusiveHeap<T>* heap)
{
    GrowingOperations op = static_cast<GrowingOperations>(base::RandInt(0, kGrowingOperationsCount - 1));
...
template <typename T> struct NotMovable {
    static constexpr bool value = !std::is_nothrow_move_constructible_v<T> && std::is_copy_constructible_v<T>;
};

// Invokes "take" if the type is movable, otherwise invokes erase.
template <typename T, bool kNotMovable = NotMovable<T>::value> struct Take;
template <typename T> struct Take<T, true> {
    static void Do(IntrusiveHeap<T>* heap, size_t index)
    {
        heap->erase(index);
    }
};
template <typename T> struct Take<T, false> {
    static void Do(IntrusiveHeap<T>* heap, size_t index)
    {
...
template <typename T, bool kNotMovable = NotMovable<T>::value> struct TakeTop;
template <typename T> struct TakeTop<T, true> {
    static void Do(IntrusiveHeap<T>* heap)
    {
        heap->pop();
    }
};
template <typename T> struct TakeTop<T, false> {
    static void Do(IntrusiveHeap<T>* heap)
    {
...
template <typename T> void DoShrinkingOperation(IntrusiveHeap<T>* heap)
{
    ShrinkingOperations op = static_cast<ShrinkingOperations>(base::RandInt(0, kShrinkingOperationsCount - 1));
...
// validates that the operation completed as expected and resulted in a valid
// heap.
template <typename T> void DoSameSizeOperation(IntrusiveHeap<T>* heap)
{
    SameSizeOperations op = static_cast<SameSizeOperations>(base::RandInt(0, kSameSizeOperationsCount - 1));
    ...
    case kUpdate: {
        it = heap->Modify(index, [&new_value](T& element) { element.set_value(new_value); });
        break;
    }
...
template <typename T> void DoRandomHeapOperation(IntrusiveHeap<T>* heap)
{
    static constexpr int kMinHeapSize = 10u;
    static constexpr int kMaxHeapSize = 100u;

    OperationTypes operation_type = static_cast<OperationTypes>(base::RandInt(0, kOperationTypesCount - 1));
...
// operations.
template <typename T> void MoveStressTest()
{
...
// operations.
template <typename T> void CopyStressTest()
{
    IntrusiveHeap<T> heap({ 2, 4, 6, 8 });
    EXPECT_EQ(4u, heap.size());
    EXPECT_FALSE(heap.empty());
...
template <typename T> void GeneralStressTest()
{
...
// default-constructors, move-operations and copy-operations.
template <typename ValueType, bool D, bool M, bool C> void ValidateValueType()
{
...
class Comparator {
public:
    bool operator()(const WithHeapHandle<ScopedClosureRunner>& lhs, const WithHeapHandle<ScopedClosureRunner>& rhs)
    {
