...
// with span directly).
template <class T> class SpanWriter {
    static_assert(!std::is_const_v<T>, "SpanWriter needs mutable access to its buffer");

public:
    // Construct SpanWriter that writes to `buf`.
    explicit SpanWriter(span<T> buf)
        : buf_(buf)
        , original_size_(buf_.size())
    {
    }
    ...
    template <size_t N> std::optional<span<T, N>> Skip()
    {
        if (N > remaining()) {
            return std::nullopt;
        }
    ...
    {
        std::array<uint8_t, 1u> big_endian = U8ToBigEndian(value);
        return Write(base::span<const uint8_t>(big_endian.data(), big_endian.size()));
    }
    bool WriteU16BigEndian(uint16_t value)
        requires(std::same_as<T, uint8_t>)
    {
        std::array<uint8_t, 2u> big_endian = U16ToBigEndian(value);
        return Write(base::span<const uint8_t>(big_endian.data(), big_endian.size()));
    }
    bool WriteU32BigEndian(uint32_t value)
        requires(std::same_as<T, uint8_t>)
    {
        std::array<uint8_t, 4u> big_endian = U32ToBigEndian(value);
        return Write(base::span<const uint8_t>(big_endian.data(), big_endian.size()));
    }
    bool WriteU64BigEndian(uint64_t value)
        requires(std::same_as<T, uint8_t>)
    {
        std::array<uint8_t, 8u> big_endian = U64ToBigEndian(value);
        return Write(base::span<const uint8_t>(big_endian.data(), big_endian.size()));
    }
    ...
    {
        return original_size_ - buf_.size();
    }
...
template <class T, size_t N> SpanWriter(span<T, N>) -> SpanWriter<T>;
