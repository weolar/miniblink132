...
template <class inner> static bool SmallMapIsSubset(small_map<inner> const& a, small_map<inner> const& b)
{
...
template <class inner> static bool SmallMapEqual(small_map<inner> const& a, small_map<inner> const& b)
{
...
TEST(SmallMap, NonHashMap)
{
    ...
    small_map<std::map<int, int>, 4, std::equal_to<int>>::iterator iter(m.begin());
    ASSERT_TRUE(iter != m.end());
    EXPECT_EQ(iter->first, 9);
    EXPECT_EQ(iter->second, 2);
    ++iter;
...
{
    small_map<unordered_map_add_item, 4, std::equal_to<int>, void (&)(unordered_map_add_item*)> m(InitMap);
...
{
    small_map<unordered_map_add_item, 4, std::equal_to<int>, unordered_map_add_item_initializer> m(unordered_map_add_item_initializer(-1));
...
template <typename V> class MoveOnlyType {
public:
    MoveOnlyType()
        : value_(0)
    {
    }
    explicit MoveOnlyType(V value)
        : value_(value)
    {
    }
