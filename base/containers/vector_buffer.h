...
public:
    ...
    span<T> subspan(size_t index)
    {
        return as_span().subspan(index);
    }
    ...
    template <typename T2> static inline constexpr bool is_trivially_copyable_or_relocatable = std::is_trivially_copyable_v<T2> || IS_TRIVIALLY_RELOCATABLE(T2);
