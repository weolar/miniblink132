...
template <typename Container, typename Value> constexpr bool Contains(const Container& container, const Value& value)
{
    if constexpr (requires {
        ...
        static_assert(!internal::HasKeyType<Container>,
...
template <typename Container, typename Value, typename Proj> constexpr bool Contains(const Container& container, const Value& value, Proj proj)
{
    return ranges::find(container, value, std::move(proj)) != std::ranges::end(container);
}
