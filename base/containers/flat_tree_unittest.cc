...
template <typename It> InputIterator<It> MakeInputIterator(It it)
{
...
public:
    ...
    Emplaceable(int i, double d)
        : int_(i)
        , double_(d)
    {
    }
    Emplaceable(Emplaceable&& other)
        : int_(other.int_)
...
struct TemplateConstructor {
    template <typename T> TemplateConstructor(const T&)
    {
    }

    friend bool operator<(const TemplateConstructor&, const TemplateConstructor&)
    {
        return false;
    }
};
...
template <class PairType> struct LessByFirst {
...
// Common test trees.
template <typename ContainerT> using TypedTree = flat_tree<typename ContainerT::value_type, std::identity, std::less<>, ContainerT>;
...
using EmplaceableTree = flat_tree<Emplaceable, std::identity, std::less<>, std::vector<Emplaceable>>;
using ReversedTree = flat_tree<int, std::identity, std::greater<int>, std::vector<int>>;

using TreeWithStrangeCompare = flat_tree<int, std::identity, NonDefaultConstructibleCompare, std::vector<int>>;
...
template <typename T> class FlatTreeTest : public testing::Test { };
...
TEST(FlatTree, NoExcept)
{
    ...
    using MoveThrowsTree = flat_tree<MoveThrows, std::identity, std::less<>, std::array<MoveThrows, 1>>;

    static_assert(std::is_nothrow_move_constructible_v<IntTree>, "Error: IntTree is not nothrow move constructible");
    static_assert(std::is_nothrow_move_assignable_v<IntTree>, "Error: IntTree is not nothrow move assignable");

    static_assert(!std::is_nothrow_move_constructible_v<MoveThrowsTree>, "Error: MoveThrowsTree is nothrow move constructible");
    static_assert(!std::is_nothrow_move_assignable_v<MoveThrowsTree>, "Error: MoveThrowsTree is nothrow move assignable");
}
...
TEST(FlatTree, Stability)
{
    ...
    auto AllOfSecondsAreZero = [&cont] { return ranges::all_of(cont, [](const Pair& elem) { return elem.second == 0; }); };
...
{
    {
        IntPair input_vals[] = { { 1, 1 }, { 1, 2 }, { 2, 1 }, { 2, 2 }, { 1, 3 }, { 2, 3 }, { 3, 1 }, { 3, 2 }, { 3, 3 } };

        IntPairTree first_of(MakeInputIterator(std::begin(input_vals)), MakeInputIterator(std::end(input_vals)));
    ...
    }
    {
        ...
        TreeWithStrangeCompare cont(MakeInputIterator(std::begin(input_vals)), MakeInputIterator(std::end(input_vals)), NonDefaultConstructibleCompare(0));
...
{
    {
        ...
        IntPairTree first_of(sorted_unique, MakeInputIterator(std::begin(input_vals)), MakeInputIterator(std::end(input_vals)));
        EXPECT_THAT(first_of, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1)));
    }
    {
        TreeWithStrangeCompare::value_type input_vals[] = { 1, 2, 3 };

        TreeWithStrangeCompare cont(
            sorted_unique, MakeInputIterator(std::begin(input_vals)), MakeInputIterator(std::end(input_vals)), NonDefaultConstructibleCompare(0));
        EXPECT_THAT(cont, ElementsAre(1, 2, 3));
    }
}
...
TYPED_TEST_P(FlatTreeTest, SortedUniqueInitializerListConstructor)
{
    ...
    {
        TreeWithStrangeCompare cont(sorted_unique, { 1, 2, 3, 4, 5, 6, 8, 10 }, NonDefaultConstructibleCompare(0));
        EXPECT_THAT(cont, ElementsAre(1, 2, 3, 4, 5, 6, 8, 10));
    }
    {
...
TEST(FlatTree, InsertIterIter)
{
...

    {
        ...
        UNSAFE_BUFFERS(cont.insert(std::begin(int_pairs), std::end(int_pairs)));
        EXPECT_THAT(cont, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1), IntPair(4, 1), IntPair(5, 1)));
    }
    ...
    {
        IntIntMap cont({ { 1, 1 }, { 2, 1 }, { 3, 1 }, { 4, 1 } });
        IntPair int_pairs[] = { { 3, 2 }, { 1, 2 }, { 4, 2 }, { 2, 2 }, { 7, 2 }, { 6, 2 }, { 8, 2 }, { 5, 2 }, { 5, 3 }, { 6, 3 }, { 7, 3 }, { 8, 3 } };
        UNSAFE_BUFFERS(cont.insert(std::begin(int_pairs), std::end(int_pairs)));
        EXPECT_THAT(cont, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1), IntPair(4, 1), IntPair(5, 2), IntPair(6, 2), IntPair(7, 2), IntPair(8, 2)));
    }
}
...
{
    {
        ...
        std::pair<typename TypedTree<TypeParam>::iterator, typename TypedTree<TypeParam>::iterator> result = cont.equal_range(5);
    ...
    }
    {
        ...
        std::pair<typename TypedTree<TypeParam>::const_iterator, typename TypedTree<TypeParam>::const_iterator> result = cont.equal_range(5);
        EXPECT_EQ(std::next(cont.begin(), 0), result.first);
        EXPECT_EQ(std::next(cont.begin(), 1), result.second);
        result = cont.equal_range(7);
        EXPECT_EQ(std::next(cont.begin(), 1), result.first);
...
{
    int unsorted[] = { 2, 1 };
    EXPECT_DCHECK_DEATH(TypedTree<TypeParam>(sorted_unique, std::begin(unsorted), std::end(unsorted)));

    int repeated[] = { 1, 2, 2 };
    EXPECT_DCHECK_DEATH(TypedTree<TypeParam>(sorted_unique, std::begin(repeated), std::end(repeated)));
}
...
REGISTER_TYPED_TEST_SUITE_P(FlatTreeTest, DefaultConstructor, CopyConstructor, ContainerCopyConstructor, InitializerListConstructor,
    SortedUniqueContainerCopyConstructor, SortedUniqueInitializerListConstructor, CopyAssignable, InitializerListAssignable, Clear, Size, Empty, Iterators,
    InsertLValue, InsertPositionLValue, Emplace, EmplacePosition, Extract, Replace, ErasePosition, EraseRange, EraseKey, EraseEndDeath, Count, Find, Contains,
    EqualRange, LowerBound, UpperBound, Swap, EraseIf, SortedUniqueRangeConstructorDCHECKs, SortedUniqueVectorCopyConstructorDCHECKs,
    SortedUniqueVectorMoveConstructorDCHECKs, SortedUniqueInitializerListConstructorDCHECKs, ReplaceDCHECKs);
