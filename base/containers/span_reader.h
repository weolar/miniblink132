...
// with span directly).
template <class T> class SpanReader {
    STACK_ALLOCATED();

public:
    // Construct SpanReader from a span.
    explicit SpanReader(span<T> buf)
        : buf_(buf)
    ...
    template <size_t N> std::optional<span<T, N>> Read()
    {
        if (N > remaining()) {
            return std::nullopt;
        }
...
template <typename T, size_t N, typename InternalPtrType> SpanReader(span<T, N, InternalPtrType>) -> SpanReader<T>;
