...
// each cycle.
template <class QueueT, class Tester> void CycleTest(circular_deque<QueueT>& queue, const Tester& tester)
{
...
public:
    ...
    int ref_count_changes() const
    {
        return ref_count_changes_;
    }
...
TEST(CircularDeque, IteratorConstructor)
{
    ...
    auto first = UNSAFE_BUFFERS(circular_deque<CopyOnlyInt>(std::begin(values), std::end(values)));
...
{
    circular_deque<CopyOnlyInt> deq(base::from_range, std::vector({ 1, 2, 3, 4, 5, 6 }));
