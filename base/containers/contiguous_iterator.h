...
template <typename T> struct PointsToObject : std::true_type { };
// std::iter_value_t is not defined for `T*` where T is not an object type.
template <typename T> struct PointsToObject<T*> : std::is_object<T> { };

// A pointer is a contiguous iterator.
// Reference: https://wg21.link/iterator.traits#5
template <typename T> struct IsPointer : std::is_pointer<T> { };

template <typename T, typename StringT = std::basic_string<iter_value_t<T>>>
struct IsStringIterImpl : std::disjunction<std::is_same<T, typename StringT::const_iterator>, std::is_same<T, typename StringT::iterator>> { };
...
template <typename T> struct IsStringIter : std::conjunction<std::is_trivial<iter_value_t<T>>, IsStringIterImpl<T>> { };
...
struct IsArrayIter : std::disjunction<std::is_same<T, typename ArrayT::const_iterator>, std::is_same<T, typename ArrayT::iterator>> { };
...
struct IsVectorIter : std::conjunction<std::negation<std::is_same<iter_value_t<T>, bool>>,
                          std::disjunction<std::is_same<T, typename VectorT::const_iterator>, std::is_same<T, typename VectorT::iterator>>> { };
...
template <typename T> struct IsValueArrayIter : std::is_pointer<T> { };
...
struct IsCheckedContiguousIter
    : std::disjunction<std::is_same<T, base::CheckedContiguousConstIterator<ValueT>>, std::is_same<T, base::CheckedContiguousIterator<ValueT>>> { };

// Check that the iterator points to an actual object, and is one of the
// iterator types mentioned above.
template <typename T, bool B = PointsToObject<T>::value> struct IsContiguousIteratorImpl : std::false_type { };
template <typename T>
struct IsContiguousIteratorImpl<T, true>
    : std::disjunction<IsPointer<T>, IsStringIter<T>, IsArrayIter<T>, IsVectorIter<T>, IsValueArrayIter<T>, IsCheckedContiguousIter<T>> { };
...
template <typename T> struct IsContiguousIterator : internal::IsContiguousIteratorImpl<std::remove_cvref_t<T>> { };
