...
template <class C> void IdentityTest()
{
    C c = { 1, 2, 3, 4, 5 };
    auto vec = ToVector(c);
...
template <class C> void ProjectionTest()
{
    C c = { 1, 2, 3, 4, 5 };
...
TEST(ToVectorTest, MoveOnly)
{
    ...
    auto mapped_vec = ToVector(std::move(vec), [](MoveOnly& value) { return std::move(value); });
    EXPECT_EQ(mapped_vec.size(), 10U);
}

template <typename C, typename Proj, typename T>
constexpr bool CorrectlyProjected = std::is_same_v<T, typename decltype(ToVector(std::declval<C>(), std::declval<Proj>()))::value_type>;
...
TEST(ToVectorTest, CorrectlyProjected)
{
    ...
    static_assert(CorrectlyProjected<std::vector<std::string>, decltype(proj), std::string>);
    static_assert(CorrectlyProjected<std::set<std::string>, decltype(&std::string::length), std::size_t>);
}
