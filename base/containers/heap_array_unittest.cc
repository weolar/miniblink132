...
{
    auto vec = UNSAFE_BUFFERS(HeapArray<uint32_t>::FromOwningPointer(new uint32_t[3], 3u));
...
TEST(HeapArray, IteratorAndIndex)
{
    ...
    static_assert(std::is_const_v<std::remove_reference_t<decltype(*empty.begin())>>);
    static_assert(std::is_const_v<std::remove_reference_t<decltype(*empty.end())>>);
