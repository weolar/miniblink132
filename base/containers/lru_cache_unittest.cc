...
struct CachedItem {
    ...
    CachedItem(const CachedItem& other)
        : value(other.value)
    {
        cached_item_live_count++;
    }
...
template <typename LRUCacheTemplate> class LRUCacheTest : public testing::Test { };

struct LRUCacheTemplate {
    template <class Key, class Value, class KeyCompare = std::less<Key>> using Type = base::LRUCache<Key, Value, KeyCompare>;
};

struct HashingLRUCacheTemplate {
    template <class Key, class Value, class KeyHash = std::hash<Key>, class KeyEqual = std::equal_to<Key>>
    using Type = base::HashingLRUCache<Key, Value, KeyHash, KeyEqual>;
};

using LRUCacheTemplates = testing::Types<LRUCacheTemplate, HashingLRUCacheTemplate>;
TYPED_TEST_SUITE(LRUCacheTest, LRUCacheTemplates);

template <typename LRUCacheSetTemplate> class LRUCacheSetTest : public testing::Test { };

struct LRUCacheSetTemplate {
    template <class Value, class Compare = std::less<Value>> using Type = base::LRUCacheSet<Value, Compare>;
};

struct HashingLRUCacheSetTemplate {
    template <class Value, class Hash = std::hash<Value>, class Equal = std::equal_to<Value>> using Type = base::HashingLRUCacheSet<Value, Hash, Equal>;
};

using LRUCacheSetTemplates = testing::Types<LRUCacheSetTemplate, HashingLRUCacheSetTemplate>;
...
{
    using Cache = typename TypeParam::template Type<int, std::unique_ptr<CachedItem>>;
    Cache cache(Cache::NO_AUTO_EVICT);
...
template <typename T> struct Deref {
    using Target = T;
    static const Target& deref(const T& x)
    {
        return x;
    }
};
...
template <typename T> struct Deref<RefCountedData<T>> {
...
template <typename T> struct Deref<scoped_refptr<T>> {
    using Target = typename Deref<T>::Target;
...
// before comparison.
template <typename T> struct DerefCompare {
...
template <typename T, template <class> typename HashT = std::hash> struct DerefHash {
...
TYPED_TEST(LRUCacheSetTest, ReplacementIdentity)
{
    ...
    constexpr auto kCreateCache = [] {
        ...
        } else if constexpr (std::is_same_v<TypeParam, HashingLRUCacheSetTemplate>) {
            using Cache = typename TypeParam::template Type<Ptr, DerefHash<Ptr>, DerefEqual<Ptr>>;
            return Cache(Cache::NO_AUTO_EVICT);
        } else {
...
TYPED_TEST(LRUCacheTest, EstimateMemory)
{
    ...
    EXPECT_GT(trace_event::EstimateMemoryUsage(cache), trace_event::EstimateMemoryUsage(key));
}
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
...
TEST(LRUCacheIndexOrderTest, IndexIteration)
{
    ...
    std::ranges::transform(ordered.index(), std::back_inserter(ordered_keys), [](const auto& key_value_pair) -> int { return key_value_pair.first; });
    EXPECT_THAT(ordered_keys, testing::ElementsAre(kItem1Key, kItem2Key, kItem3Key, kItem4Key));

    std::vector<int> unordered_keys;
    std::ranges::transform(unordered.index(), std::back_inserter(unordered_keys), [](const auto& key_value_pair) -> int { return key_value_pair.first; });
    EXPECT_THAT(unordered_keys, testing::UnorderedElementsAre(kItem1Key, kItem2Key, kItem3Key, kItem4Key));
}
