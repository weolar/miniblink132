...
public:
    ...
    friend bool operator==(const HeapHandle& lhs, const HeapHandle& rhs) = default;
    friend std::strong_ordering operator<=>(const HeapHandle& lhs, const HeapHandle& rhs) = default;

private:
    template <typename T, typename Compare, typename HeapHandleAccessor> friend class IntrusiveHeap;

    // Only IntrusiveHeaps can create valid HeapHandles.
    explicit HeapHandle(size_t index)
        : index_(index)
    {
    }
...
// type.
template <typename T> struct DefaultHeapHandleAccessor {
...
// container).
template <typename T, typename Compare = std::less<T>, typename HeapHandleAccessor = DefaultHeapHandleAccessor<T>> class IntrusiveHeap {
private:
...
public:
    ...
    IntrusiveHeap(base::from_range_t, Range&& range, const value_compare& comp = value_compare(), const heap_handle_accessor& access = heap_handle_accessor())
        : impl_(comp, access)
    {
    ...
    UNSAFE_BUFFER_USAGE IntrusiveHeap(
        InputIterator first, InputIterator last, const value_compare& comp = value_compare(), const heap_handle_accessor& access = heap_handle_accessor())
        : impl_(comp, access)
    {
    ...
    template <typename U>
    IntrusiveHeap(std::initializer_list<U> ilist, const value_compare& comp = value_compare(), const heap_handle_accessor& access = heap_handle_accessor())
        : impl_(comp, access)
    {
        insert_range(ilist);
    }
    ...
    template <typename... Args> const_iterator emplace(Args&&... args)
    {
    ...
    template <typename P> value_type take(P pos)
    {
    ...
    template <typename P> void erase(P pos)
    {
        erase(ToIndex(pos));
    }
    ...
    template <typename Functor> void EraseIf(Functor predicate)
    {
        // Stable partition ensures that if no elements are erased, the heap remains
        // intact.
        auto erase_start = std::stable_partition(impl_.heap_.begin(), impl_.heap_.end(), [&](const auto& element) { return !predicate(element); });

        // Clear the heap handle of every element that will be erased.
        for (size_t i = static_cast<size_t>(erase_start - impl_.heap_.begin()); i < impl_.heap_.size(); ++i) {
            ClearHeapHandle(i);
        }
        ...
        std::move(erase_start, impl_.heap_.end(), std::back_inserter(elements_to_delete));
    ...
    template <typename P> const_iterator Replace(P pos, const T& element)
    {
        return ReplaceImpl(ToIndex(pos), element);
    }
    template <typename P> const_iterator Replace(P pos, T&& element)
    {
    ...
    template <typename P> const_iterator Update(P pos)
    {
    ...
    template <typename P, typename UnaryOperation> const_iterator Modify(P pos, UnaryOperation unary_op)
    {
...
private:
    ...
    // integral types.
    template <typename I, typename = std::enable_if_t<std::is_integral_v<I>>> size_type ToIndex(I pos)
    {
    ...
    template <typename U> void FillHole(size_type hole, U element);
    ...
    template <typename U> size_type MoveHoleUpAndFill(size_type hole_pos, U element);
    ...
    template <typename FillElementType, typename U> size_type MoveHoleDownAndFill(size_type hole_pos, U element);

    // Implementation of Insert and Replace built on top of the MoveHole
    // primitives.
    template <typename U> const_iterator InsertImpl(U element);
    template <typename U> const_iterator ReplaceImpl(size_type pos, U element);
    template <typename U> const_iterator ReplaceTopImpl(U element);
    ...
    struct Impl : private value_compare, private heap_handle_accessor {
        Impl(const value_compare& value_comp, const heap_handle_accessor& heap_handle_access)
            ...
            , heap_handle_accessor(heap_handle_access)
        {
        }
        ...
        value_compare& get_value_compare()
        {
            return *this;
        }

        const heap_handle_accessor& get_heap_handle_access() const
        {
            return *this;
        }
        heap_handle_accessor& get_heap_handle_access()
        {
            return *this;
        }
...
public:
    ...
    InternalHeapHandleStorage& operator=(const InternalHeapHandleStorage&) = delete;
    ...
    friend void swap(InternalHeapHandleStorage& lhs, InternalHeapHandleStorage& rhs)
    {
        lhs.swap(rhs);
    }
...
template <typename T> class WithHeapHandle : public InternalHeapHandleStorage {
public:
    ...
    template <typename U>
    WithHeapHandle(U value)
        : value_(std::move_if_noexcept(value))
    {
    }
    WithHeapHandle(T&& value) noexcept
        : value_(std::move(value))
    {
    }
    // Constructor that forwards all arguments along to |value_|.
    template <class... Args> explicit WithHeapHandle(Args&&... args);
    ...
    T& value() LIFETIME_BOUND
    {
        return value_;
    }
    const T& value() const LIFETIME_BOUND
    {
        return value_;
    }
    ...
    friend void swap(WithHeapHandle& lhs, WithHeapHandle& rhs)
    {
        lhs.swap(rhs);
    }
    ...
    friend auto operator<=>(const WithHeapHandle& lhs, const WithHeapHandle& rhs)
    {
        return lhs.value_ <=> rhs.value_;
    }
...
template <typename HandleType> bool IsInvalid(const HandleType& handle)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor> IntrusiveHeap<T, Compare, HeapHandleAccessor>::~IntrusiveHeap()
{
    clear();
}

template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>& IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(IntrusiveHeap&& other) noexcept
{
    clear();
...
template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>& IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(const IntrusiveHeap& other)
{
    clear();
    impl_ = other.impl_;
    for (size_t i = 0; i < size(); ++i) {
...
template <typename T, typename Compare, typename HeapHandleAccessor>
IntrusiveHeap<T, Compare, HeapHandleAccessor>& IntrusiveHeap<T, Compare, HeapHandleAccessor>::operator=(std::initializer_list<value_type> ilist)
{
    clear();
    insert_range(ilist);
}

template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::clear()
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::value_type IntrusiveHeap<T, Compare, HeapHandleAccessor>::take(size_type pos)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::erase(size_type pos)
{
    DCHECK_LT(pos, size());
    // Make a hole by taking the element out of the heap.
    MakeHole(pos);
    ...
    if (pos != GetLastIndex()) {
        ...
        // Move the hole down the heap, filling it with the current leaf at the
        // very end of the heap.
        MoveHoleDownAndFill<WithLeafElement>(pos, std::move_if_noexcept(impl_.heap_[GetLastIndex()]));
    }
...
template <typename T, typename Compare, typename HeapHandleAccessor>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::const_iterator IntrusiveHeap<T, Compare, HeapHandleAccessor>::Update(size_type pos)
{
    DCHECK_LT(pos, size());
    MakeHole(pos);
...
template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::swap(IntrusiveHeap& other) noexcept
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type IntrusiveHeap<T, Compare, HeapHandleAccessor>::ToIndex(const_iterator pos)
{
    DCHECK(cbegin() <= pos);
    DCHECK(pos <= cend());
    if (pos == cend())
...
template <typename T, typename Compare, typename HeapHandleAccessor>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type IntrusiveHeap<T, Compare, HeapHandleAccessor>::ToIndex(const_reverse_iterator pos)
{
    DCHECK(crbegin() <= pos);
    DCHECK(pos <= crend());
    if (pos == crend())
...
template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::SetHeapHandle(size_type i)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::ClearHeapHandle(size_type i)
{
    impl_.get_heap_handle_access().ClearHeapHandle(&impl_.heap_[i]);
    DCHECK(!GetHeapHandle(i).IsValid());
}

template <typename T, typename Compare, typename HeapHandleAccessor> HeapHandle IntrusiveHeap<T, Compare, HeapHandleAccessor>::GetHeapHandle(size_type i)
{
    return impl_.get_heap_handle_access().GetHeapHandle(&impl_.heap_[i]);
}

template <typename T, typename Compare, typename HeapHandleAccessor> bool IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(size_type i, size_type j)
{
    DCHECK_LT(i, size());
    DCHECK_LT(j, size());
...
template <typename T, typename Compare, typename HeapHandleAccessor> bool IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(const T& element, size_type i)
{
    DCHECK_LT(i, size());
    return impl_.get_value_compare()(element, impl_.heap_[i]);
}

template <typename T, typename Compare, typename HeapHandleAccessor> bool IntrusiveHeap<T, Compare, HeapHandleAccessor>::Less(size_type i, const T& element)
{
    DCHECK_LT(i, size());
    return impl_.get_value_compare()(impl_.heap_[i], element);
}

template <typename T, typename Compare, typename HeapHandleAccessor> void IntrusiveHeap<T, Compare, HeapHandleAccessor>::MakeHole(size_type pos)
{
    DCHECK_LT(pos, size());
    ClearHeapHandle(pos);
}
...
template <typename T, typename Compare, typename HeapHandleAccessor>
template <typename U>
void IntrusiveHeap<T, Compare, HeapHandleAccessor>::FillHole(size_type hole_pos, U element)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
void IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHole(size_type new_hole_pos, size_type old_hole_pos)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHoleUpAndFill(
    size_type hole_pos, U element)
{
...
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHoleDownAndFill(
    size_type hole_pos, U element)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::const_iterator IntrusiveHeap<T, Compare, HeapHandleAccessor>::InsertImpl(U element)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::const_iterator IntrusiveHeap<T, Compare, HeapHandleAccessor>::ReplaceImpl(size_type pos, U element)
{
...
template <typename T, typename Compare, typename HeapHandleAccessor>
template <typename U>
typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::const_iterator IntrusiveHeap<T, Compare, HeapHandleAccessor>::ReplaceTopImpl(U element)
{
    MakeHole(0u);
...
template <typename T> void WithHeapHandle<T>::swap(WithHeapHandle& other) noexcept
{
