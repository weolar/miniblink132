...
template <class T> class circular_deque;
...
template <typename T> class circular_deque_const_iterator {
public:
    ...
    friend circular_deque_const_iterator operator+(const circular_deque_const_iterator& iter, difference_type offset)
    {
    ...
    friend std::ptrdiff_t operator-(const circular_deque_const_iterator& lhs, const circular_deque_const_iterator& rhs)
    {
        lhs.CheckComparable(rhs);
        return static_cast<std::ptrdiff_t>(lhs.OffsetFromBegin() - rhs.OffsetFromBegin());
    }

    // Comparisons.
    friend bool operator==(const circular_deque_const_iterator& lhs, const circular_deque_const_iterator& rhs)
    {
        lhs.CheckComparable(rhs);
        return lhs.index_ == rhs.index_;
    }
    friend std::strong_ordering operator<=>(const circular_deque_const_iterator& lhs, const circular_deque_const_iterator& rhs)
    {
        lhs.CheckComparable(rhs);
...
protected:
    ...
    void CheckUnstableUsage() const
    {
        ...
        DCHECK_EQ(created_generation_, parent_deque_->generation_) << "circular_deque iterator dereferenced after mutation.";
    }
...
template <typename T> class circular_deque_iterator : public circular_deque_const_iterator<T> {
...
public:
    ...
    circular_deque_iterator()
        : circular_deque_const_iterator<T>()
    {
    }
    ...
    // Random access mutation.
    friend circular_deque_iterator operator+(const circular_deque_iterator& iter, difference_type offset)
    {
...
template <typename T> class circular_deque {
private:
...
public:
    ...
    circular_deque(base::from_range_t, Range&& value)
        : circular_deque()
    {
    ...
    void insert(const_iterator pos, InputIterator first, InputIterator last)
    {
        ...
        const size_t inserted_items = checked_cast<size_t>(std::distance(first, last));
        ...
        if (pos == begin()) {
            ...
            begin_ = (old_begin + buffer_.capacity() - inserted_items) % buffer_.capacity();
            insert_cur = begin();
    ...
    template <class... Args> iterator emplace(const_iterator pos, Args&&... args)
    {
        ...
        std::construct_at(buffer_.get_at(insert_begin.index_), std::forward<Args>(args)...);
    ...
    iterator erase(const_iterator pos_begin, const_iterator pos_end)
    {
        ...
        while (move_src != move_src_end) {
            VectorBuffer::MoveConstructRange(buffer_.subspan(move_src, 1u), buffer_.subspan(move_dest, 1u));
    ...
    friend void swap(circular_deque& lhs, circular_deque& rhs)
    {
        lhs.swap(rhs);
    }
...
private:
    ...
    UNSAFE_BUFFER_USAGE static void MoveBuffer(
        VectorBuffer& from_buf, size_t from_begin, size_t from_end, VectorBuffer& to_buf, size_t* to_begin, size_t* to_end)
    {
        *to_begin = 0;
        if (from_begin < from_end) {
            // Contiguous.
            VectorBuffer::MoveConstructRange(from_buf.subspan(from_begin, from_end - from_begin), to_buf.subspan(0u, from_end - from_begin));
            *to_end = from_end - from_begin;
            ...
            VectorBuffer::MoveConstructRange(right_side, to_buf.subspan(0u, right_side.size()));
            // Append the left side.
            span<T> left_side = from_buf.subspan(0u, from_end);
            VectorBuffer::MoveConstructRange(left_side, to_buf.subspan(right_side.size(), left_side.size()));
    ...
    void SetCapacityTo(size_t new_capacity)
    {
        ...
        UNSAFE_BUFFERS(MoveBuffer(buffer_, begin_, end_, new_buffer, &begin_, &end_));
    ...
    void MakeRoomFor(size_t count, iterator* insert_begin, iterator* insert_end)
    {
        ...
        *insert_end = iterator(this, (insert_begin->index_ + count) % cap);
        ...
        while (true) {
            ...
            VectorBuffer::MoveConstructRange(buffer_.subspan(src, 1u), buffer_.subspan(dest, 1u));
        }
    }
...
template <class T, class Value> size_t Erase(circular_deque<T>& container, const Value& value)
{
...
template <class T, class Predicate> size_t EraseIf(circular_deque<T>& container, Predicate pred)
{
