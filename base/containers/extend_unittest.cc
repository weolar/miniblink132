...
struct CopyableMovable {
    bool copied_;
    char c_;
    explicit CopyableMovable(char c)
        : copied_(false)
        , c_(c)
    {
    }
...
TEST(ExtendTest, ExtendWithSpan)
{
    ...
    static constexpr auto kVectorGenerator = []() -> std::vector<uint8_t> { return { 9, 10, 11 }; };
    ...
    EXPECT_THAT(dst, ElementsAre(3, 4, 5, 6, 7, 8, 9, 10, 11, 9, 10, 11, 3, 4, 5));
}
