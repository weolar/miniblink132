...
TEST(FixedFlatMapTest, MakeFixedFlatMap_SortedInput)
{
    ...
    static_assert(ranges::adjacent_find(kSquares) == kSquares.end(), "Error: Map contains repeated elements.");
...
{
    constexpr auto kMap = MakeFixedFlatMap<std::string_view, int>({ { "foo", 1 }, { "bar", 2 }, { "baz", 3 } });
    static_assert(ranges::is_sorted(kMap), "Error: Map is not sorted.");
    static_assert(ranges::adjacent_find(kMap) == kMap.end(), "Error: Map contains repeated elements.");
...
TEST(FixedFlatMapTest, UnsortableValues)
{
    ...
    EXPECT_THAT(kSquares, ElementsAre(PairType(1, { 1 }), PairType(2, { 4 }), PairType(3, { 9 }), PairType(4, { 16 })));
}
