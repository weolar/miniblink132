...
template <class LruCacheType> size_t DoEstimateMemoryUsageForLruCache(const LruCacheType&);
...
struct GetKeyFromKVPair {
    template <typename T1, typename T2> constexpr const T1& operator()(const std::pair<T1, T2>& pair)
    {
        return pair.first;
    }
};
...
template <class ValueType, class GetKeyFromValue, class KeyIndexTemplate> class LRUCacheBase {
public:
...
private:
    ...
    using KeyIndex = typename KeyIndexTemplate::template Type<typename ValueList::iterator>;
...
public:
    ...
    explicit LRUCacheBase(size_type max_size)
        : max_size_(max_size)
    {
    }
    ...
    IndexRange index() const
    {
        return IndexRange(index_.begin(), index_.end());
    }
...
private:
    template <class LruCacheType> friend size_t trace_event::internal::DoEstimateMemoryUsageForLruCache(const LruCacheType&);
...
template <class KeyType, class KeyCompare> struct LRUCacheKeyIndex {
    template <class ValueType> using Type = std::map<KeyType, ValueType, KeyCompare>;
};

template <class KeyType, class KeyHash, class KeyEqual> struct HashingLRUCacheKeyIndex {
    template <class ValueType> using Type = std::unordered_map<KeyType, ValueType, KeyHash, KeyEqual>;
};
...
using LRUCache = internal::LRUCacheBase<std::pair<KeyType, ValueType>, internal::GetKeyFromKVPair, internal::LRUCacheKeyIndex<KeyType, KeyCompare>>;
...
template <class KeyType, class ValueType, class KeyHash = std::hash<KeyType>, class KeyEqual = std::equal_to<KeyType>>
using HashingLRUCache
    = internal::LRUCacheBase<std::pair<KeyType, ValueType>, internal::GetKeyFromKVPair, internal::HashingLRUCacheKeyIndex<KeyType, KeyHash, KeyEqual>>;
...
using LRUCacheSet = internal::LRUCacheBase<ValueType, std::identity, internal::LRUCacheKeyIndex<ValueType, Compare>>;
...
// `Get(v)`.
template <class ValueType, class Hash = std::hash<ValueType>, class Equal = std::equal_to<ValueType>>
using HashingLRUCacheSet = internal::LRUCacheBase<ValueType, std::identity, internal::HashingLRUCacheKeyIndex<ValueType, Hash, Equal>>;
