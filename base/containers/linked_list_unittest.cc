...
class MultipleInheritanceNode : public MultipleInheritanceNodeBase, public LinkNode<MultipleInheritanceNode> {
public:
...
void ExpectListContentsForDirection(const LinkedList<Node>& list, base::span<const int> node_ids, bool forward)
{
    size_t i = 0;
    for (const LinkNode<Node>* node = (forward ? list.head() : list.tail()); node != list.end(); node = (forward ? node->next() : node->previous())) {
...
void ExpectListContents(const LinkedList<Node>& list, base::span<const int> node_ids)
{
    {
