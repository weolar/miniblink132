...
[[maybe_unused]] void TestDeductionGuides()
{
    // Tests for span(It, EndOrSize) deduction guide.
    {
        ...
        static_assert(std::is_same_v<decltype(span(v.cbegin(), v.size())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.begin(), v.size())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.data(), v.size())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.cbegin(), std::integral_constant<size_t, 0>())), span<const int, 0>>);
    }

    {
        std::vector<int> v;
        static_assert(std::is_same_v<decltype(span(v.cbegin(), v.size())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.begin(), v.size())), span<int>>);
        static_assert(std::is_same_v<decltype(span(v.data(), v.size())), span<int>>);
        static_assert(std::is_same_v<decltype(span(v.cbegin(), std::integral_constant<size_t, 0>())), span<const int, 0>>);
    }

    {
        const std::vector<int> v;
        static_assert(std::is_same_v<decltype(span(v.cbegin(), v.cend())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.begin(), v.end())), span<const int>>);
    }

    {
        std::vector<int> v;
        static_assert(std::is_same_v<decltype(span(v.cbegin(), v.cend())), span<const int>>);
        static_assert(std::is_same_v<decltype(span(v.begin(), v.end())), span<int>>);
    }
    ...
    static_assert(std::is_same_v<decltype(span(std::declval<std::array<const bool, 3>&>())), span<const bool, 3>>);
    static_assert(std::is_same_v<decltype(span(std::declval<std::array<bool, 3>&>())), span<bool, 3>>);

    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<const bool, 3>&>())), span<const bool, 3>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<const bool, 3>&&>())), span<const bool, 3>>);
    static_assert(std::is_same_v<decltype(span(std::declval<std::array<const bool, 3>&&>())), span<const bool, 3>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<bool, 3>&>())), span<const bool, 3>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<bool, 3>&&>())), span<const bool, 3>>);

    static_assert(std::is_same_v<decltype(span(std::declval<const std::string&>())), span<const char>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::string&&>())), span<const char>>);
    static_assert(std::is_same_v<decltype(span(std::declval<std::string&>())), span<char>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::u16string&>())), span<const char16_t>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::u16string&&>())), span<const char16_t>>);
    static_assert(std::is_same_v<decltype(span(std::declval<std::u16string&>())), span<char16_t>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<float, 9>&>())), span<const float, 9>>);
    static_assert(std::is_same_v<decltype(span(std::declval<const std::array<float, 9>&&>())), span<const float, 9>>);
    static_assert(std::is_same_v<decltype(span(std::declval<std::array<float, 9>&>())), span<float, 9>>);
}
...
TEST(SpanTest, ConstructFromIterPair)
{
    ...
    span<int> UNSAFE_BUFFERS(dynamic_span(vector.begin(), vector.begin() + vector.size() / 2));
    EXPECT_EQ(vector.data(), dynamic_span.data());
    ...
    // SAFETY: `vector.size()` describes valid portion of `vector.data()`,
    // thus one-half `vector.size()` is within this range.
    span<int, 3> UNSAFE_BUFFERS(static_span(vector.begin(), vector.begin() + vector.size() / 2));
    EXPECT_EQ(vector.data(), static_span.data());
...
TEST(SpanTest, AllowedConversionsFromStdArray)
{
    ...
    static_assert(std::is_convertible_v<std::array<int, 3>&, base::span<int>>,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<int> with dynamic extent.");
    static_assert(std::is_convertible_v<std::array<int, 3>&, base::span<int, 3>>,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<int> with the same static extent.");
    static_assert(std::is_convertible_v<std::array<int, 3>&, base::span<const int>>,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible_v<std::array<int, 3>&, base::span<const int, 3>>,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<const int> with the same static extent.");
    static_assert(std::is_convertible_v<const std::array<int, 3>&, base::span<const int>>,
        "Error: const l-value reference to std::array<int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible_v<const std::array<int, 3>&, base::span<const int, 3>>,
        "Error: const l-value reference to std::array<int> should be convertible "
        "to base::span<const int> with the same static extent.");
    static_assert(std::is_convertible_v<std::array<const int, 3>&, base::span<const int>>,
        "Error: l-value reference to std::array<const int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible_v<std::array<const int, 3>&, base::span<const int, 3>>,
        "Error: l-value reference to std::array<const int> should be convertible "
        "to base::span<const int> with the same static extent.");
    static_assert(std::is_convertible_v<const std::array<const int, 3>&, base::span<const int>>,
        "Error: const l-value reference to std::array<const int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible_v<const std::array<const int, 3>&, base::span<const int, 3>>,
        "Error: const l-value reference to std::array<const int> should be "
...
TEST(SpanTest, DisallowedConstructionsFromStdArray)
{
    ...
    // as well.
    static_assert(!std::is_constructible_v<base::span<int>, const std::array<int, 3>&>,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from const l-value reference to std::array<int>");

    static_assert(!std::is_constructible_v<base::span<int>, std::array<const int, 3>&>,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from l-value reference to std::array<const int>");

    static_assert(!std::is_constructible_v<base::span<int>, const std::array<const int, 3>&>,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "const from l-value reference to std::array<const int>");

    static_assert(!std::is_constructible_v<base::span<int, 2>, std::array<int, 3>&>,
    ...
    static_assert(!std::is_constructible_v<base::span<int, 4>, std::array<int, 3>&>,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from l-value reference to std::array<int> with different extent");

    static_assert(!std::is_constructible_v<base::span<int>, std::array<bool, 3>&>,
        "Error: base::span<int> with dynamic extent should not be constructible "
...
TEST(SpanTest, ConstructFromVolatileArray)
{
    ...
    static_assert(std::is_same_v<decltype(const_span.data()), const volatile int*>);
    EXPECT_EQ(array, const_span.data());
...
TEST(SpanTest, ConstructFromRange)
{
    struct Range {
        ...
        {
            return base::span(arr_).begin();
        }
        iterator end() const
        {
            return base::span(arr_).end();
        }
    ...
    {
        // Explicit.
        static_assert(!std::convertible_to<const std::vector<int>, span<const int, 3u>>);
        static_assert(std::constructible_from<span<const int, 3u>, const std::vector<int>>);
        const std::vector<int> i { 1, 2, 3 };
    ...
    static_assert(!std::constructible_from<span<const bool>, const std::vector<bool>>);
    static_assert(!std::constructible_from<span<const bool, 3u>, const std::vector<bool>>);
}
...
{
    int x = 123;
    static_assert(std::is_same_v<decltype(span_from_ref(std::move(x))), span<const int, 1u>>);
...
TEST(SpanTest, NoLifetimeWarnings)
{
    ...
    [[maybe_unused]] auto d = l2(span<const int, 3>({ 1, 2, 3 })); // Constructor is explicit.
...
TEST(SpanTest, ConvertNonConstPointerToConst)
{
    ...
    // SAFETY: `vector.size()` describes valid portion of `vector.data()`.
    span<int*, 3> UNSAFE_BUFFERS(static_non_const_pointer_span(vector.data(), vector.size()));
    ...
    EXPECT_THAT(static_const_pointer_span, Pointwise(Eq(), static_non_const_pointer_span));
}
...
TEST(SpanTest, ConvertBetweenEquivalentTypes)
{
    ...
    // SAFETY: `vector.size()` describes valid portion of `vector.data()`.
    span<int32_t, 5> UNSAFE_BUFFERS(static_int32_t_span(vector.data(), vector.size()));
...
{
    {
        ...
        static_assert(std::same_as<typename decltype(static_span)::value_type, span<const int, 3>>);
...
TEST(SpanTest, OperatorAt)
{
    ...
    static_assert(&kArray[0] == &span[0], "span[0] does not refer to the same element as kArray[0]");
    static_assert(&kArray[1] == &span[1], "span[1] does not refer to the same element as kArray[1]");
    static_assert(&kArray[2] == &span[2], "span[2] does not refer to the same element as kArray[2]");
    static_assert(&kArray[3] == &span[3], "span[3] does not refer to the same element as kArray[3]");
    static_assert(&kArray[4] == &span[4], "span[4] does not refer to the same element as kArray[4]");
}
...
TEST(SpanTest, Front)
{
    static constexpr int kArray[] = { 1, 6, 1, 8, 0 };
    constexpr span<const int> span(kArray);
    static_assert(&kArray[0] == &span.front(), "span.front() does not refer to the same element as kArray[0]");
}
...
TEST(SpanTest, Back)
{
    static constexpr int kArray[] = { 1, 6, 1, 8, 0 };
    constexpr span<const int> span(kArray);
    static_assert(&kArray[4] == &span.back(), "span.back() does not refer to the same element as kArray[4]");
}
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(bytes_span), base::span<const uint8_t, sizeof(kArray)>>);
    ...
    }
    {
        ...
        static_assert(std::is_same_v<decltype(bytes_span), base::span<const uint8_t>>);
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(writable_bytes_span), base::span<uint8_t>>);
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(chars_span), base::span<const char, sizeof(kArray)>>);
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(writable_chars_span), base::span<char>>);
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(byte_span), span<const uint8_t, 6u * sizeof(int)>>);
    ...
    }
    {
        ...
        static_assert(std::is_same_v<decltype(byte_span), span<const uint8_t, 4u * sizeof(int)>>);
    ...
    }
    {
        ...
        static_assert(std::is_same_v<decltype(byte_span), span<const uint8_t>>);
        EXPECT_EQ(byte_span.data(), reinterpret_cast<const uint8_t*>(kMutVec.data()));
...
{
    {
        ...
        static_assert(std::is_same_v<decltype(byte_span), span<uint8_t, 4u * sizeof(int)>>);
    ...
    // Result can be passed as rvalue.
    {
        ...
        [](auto byte_span) {
            static_assert(std::is_same_v<decltype(byte_span), span<uint8_t, 6u * sizeof(int)>>);
            EXPECT_EQ(byte_span.size(), 6u * sizeof(int));
            // Little endian puts the low bits in the first byte.
            EXPECT_EQ(byte_span[0u], 2);
...
TEST(SpanTest, MakeSpanFromDataAndSize)
{
    ...
    static_assert(std::is_same_v<decltype(expected_span), decltype(made_span)>, "the type of made_span differs from expected_span!");
}
...
TEST(SpanTest, MakeSpanFromPointerPair)
{
    ...
    // SAFETY: `vector.size()` describes valid portion of `vector.data()`.
    span<int> UNSAFE_BUFFERS(expected_span(vector.data(), vector.size()));
    auto made_span = UNSAFE_BUFFERS(make_span(vector.data(), vector.data() + vector.size()));
    EXPECT_EQ(expected_span.data(), made_span.data());
    EXPECT_EQ(expected_span.size(), made_span.size());
    static_assert(decltype(made_span)::extent == dynamic_extent, "");
    static_assert(std::is_same_v<decltype(expected_span), decltype(made_span)>, "the type of made_span differs from expected_span!");
...
TEST(SpanTest, MakeSpanFromDynamicSpan)
{
    ...
    static_assert(
        std::is_same_v<decltype(expected_span)::element_type, decltype(made_span)::element_type>, "make_span(span) should have the same element_type as span");

    static_assert(expected_span.data() == made_span.data(), "make_span(span) should have the same data() as span");

    static_assert(expected_span.size() == made_span.size(), "make_span(span) should have the same size() as span");

    static_assert(decltype(made_span)::extent == decltype(expected_span)::extent, "make_span(span) should have the same extent as span");
...
TEST(SpanTest, EnsureConstexprGoodness)
{
    ...
    constexpr span<const int> subspan = constexpr_span.subspan(start, start + size);
...
TEST(SpanTest, OutOfBoundsDeath)
{
    ...
    ASSERT_DEATH_IF_SUPPORTED(std::copy(span_len3.begin(), span_len3.end(), span_len2.begin()), "");
    ASSERT_DEATH_IF_SUPPORTED(std::ranges::copy(span_len3, span_len2.begin()), "");
    ASSERT_DEATH_IF_SUPPORTED(std::copy_n(span_len3.begin(), 3, span_len2.begin()), "");

    // Copying more values than exist in the source.
    ASSERT_DEATH_IF_SUPPORTED(std::copy_n(span_len2.begin(), 3, span_len3.begin()), "");
}
...
TEST(SpanTest, IteratorIsRangeMoveSafe)
{
    ...
    for (const int dest_start_index : kOverlappingStartIndexes) {
        EXPECT_FALSE(CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.begin(), span.end(),
            // SAFETY: TODO(tsepez): iterator constructor safety is dubious
            // given that we are adding indices like -4 to `data()`.
            UNSAFE_BUFFERS(CheckedContiguousIterator<const int>(span.data() + dest_start_index, span.data() + dest_start_index + kNumElements))));
    }

    // Non-overlapping ranges.
    for (const int dest_start_index : kNonOverlappingStartIndexes) {
        EXPECT_TRUE(CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.begin(), span.end(),
            // SAFETY: TODO(tsepez): iterator constructor safety is dubious
    ...
    EXPECT_TRUE(CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.begin(), span.begin(),
        // SAFETY: Empty range at the start of a span is always valid.
        UNSAFE_BUFFERS(CheckedContiguousIterator<const int>(span.data(), span.data()))));

    // IsRangeMoveSafe is false if end < begin.
    EXPECT_FALSE(CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.end(), span.begin(),
        // SAFETY: Empty range at the start of a span is always valid.
        UNSAFE_BUFFERS(CheckedContiguousIterator<const int>(span.data(), span.data()))));
}
...
TEST(SpanTest, SpanExtentConversions)
{
    ...
    static_assert(std::is_constructible_v<span<int, 0>, span<int>>, "Error: static span should be constructible from dynamic span");

    static_assert(!std::is_convertible_v<span<int>, span<int, 0>>, "Error: static span should not be convertible from dynamic span");
    ...
    static_assert(std::is_convertible_v<span<int, 0>, span<int>>, "Error: static span should be convertible to dynamic span");

    static_assert(std::is_convertible_v<span<int>, span<int>>, "Error: dynamic span should be convertible to dynamic span");

    static_assert(std::is_convertible_v<span<int, 2>, span<int, 2>>, "Error: static span should be convertible to static span");
}

TEST(SpanTest, IteratorConversions)
{
    static_assert(std::is_convertible_v<span<int>::iterator, span<const int>::iterator>, "Error: iterator should be convertible to const iterator");

    static_assert(!std::is_convertible_v<span<const int>::iterator, span<int>::iterator>, "Error: const iterator should not be convertible to iterator");
}
...
TEST(SpanTest, CopyFromNonoverlapping)
{
    ...
    UNSAFE_BUFFERS({
        ...
        static_span.first(empty_static_span.size()).copy_from_nonoverlapping(empty_static_span);
        dynamic_span.first(empty_dynamic_span.size()).copy_from_nonoverlapping(empty_dynamic_span);
        EXPECT_THAT(arr, ElementsAre(1, 2, 3));
        EXPECT_THAT(vec, ElementsAre(4, 5, 6));

        // Test too small destinations.
        EXPECT_DEATH_IF_SUPPORTED(empty_static_span.copy_from_nonoverlapping(dynamic_span), "");
        EXPECT_DEATH_IF_SUPPORTED(empty_dynamic_span.copy_from_nonoverlapping(static_span), "");
        EXPECT_DEATH_IF_SUPPORTED(empty_dynamic_span.copy_from_nonoverlapping(dynamic_span), "");
        EXPECT_DEATH_IF_SUPPORTED(dynamic_span.last(2u).copy_from_nonoverlapping(static_span), "");
...
TEST(SpanTest, Example_UnsafeBuffersPatterns)
{
    ...
    {
        uint8_t array[44] = {};
        [[maybe_unused]] uint32_t v1 = base::U32FromLittleEndian(base::span(array).first<4u>()); // Front.
        [[maybe_unused]] uint64_t v2 = base::U64FromLittleEndian(base::span(array).subspan<6u, 8u>()); // Middle.
    }
    ...
    UNSAFE_BUFFERS({
        ...
        [[maybe_unused]] uint32_t v1 = *reinterpret_cast<const uint32_t*>(array); // Front.
        [[maybe_unused]] uint64_t v2 = *reinterpret_cast<const uint64_t*>(array + 16); // Middle.
    })

    {
        uint8_t array[44] = {};
        [[maybe_unused]] uint32_t v1 = base::U32FromLittleEndian(base::span(array).first<4u>()); // Front.
        [[maybe_unused]] uint64_t v2 = base::U64FromLittleEndian(base::span(array).subspan<16u, 8u>()); // Middle.
    }
...
TEST(SpanTest, Printing)
{
    ...
    EXPECT_EQ(testing::PrintToString(base::span<const int>({ 1, 2, 3 })), "[1, 2, 3]");
    EXPECT_EQ(testing::PrintToString(base::span<const S>({ S(), S() })), "[S(), S()]");
    EXPECT_EQ(testing::PrintToString(base::span<const char>({ 'a', 'b', 'c' })), "[\"abc\"]");
    EXPECT_EQ(testing::PrintToString(base::span<const char>({ 'a', 'b', 'c', '\0' })), std::string_view("[\"abc\0\"]", 8u));
    EXPECT_EQ(testing::PrintToString(base::span<const char>({ 'a', 'b', '\0', 'c', '\0' })), std::string_view("[\"ab\0c\0\"]", 9u));
    EXPECT_EQ(testing::PrintToString(base::span<int>()), "[]");
    EXPECT_EQ(testing::PrintToString(base::span<char>()), "[\"\"]");

    EXPECT_EQ(testing::PrintToString(base::span<const char16_t>({ u'a', u'b', u'c' })), "[u\"abc\"]");
    EXPECT_EQ(testing::PrintToString(base::span<const wchar_t>({ L'a', L'b', L'c' })), "[L\"abc\"]");
    ...
    EXPECT_EQ(base::ToString(base::span<const char>({ 'a', 'b', 'c' })), "[\"abc\"]");
    EXPECT_EQ(base::ToString(base::span<const char>({ 'a', 'b', 'c', '\0' })), std::string_view("[\"abc\0\"]", 8u));
    EXPECT_EQ(base::ToString(base::span<const char>({ 'a', 'b', '\0', 'c', '\0' })), std::string_view("[\"ab\0c\0\"]", 9u));
    EXPECT_EQ(base::ToString(base::span<int>()), "[]");
    EXPECT_EQ(base::ToString(base::span<char>()), "[\"\"]");

    EXPECT_EQ(base::ToString(base::span<const char16_t>({ u'a', u'b', u'c' })), "[u\"abc\"]");
    EXPECT_EQ(base::ToString(base::span<const wchar_t>({ L'a', L'b', L'c' })), "[L\"abc\"]");
}
