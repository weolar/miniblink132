...
template <typename T, size_t N, typename Ptr> inline constexpr bool std::ranges::enable_view<base::span<T, N, Ptr>> = true;
template <typename T, size_t N, typename Ptr> inline constexpr bool std::ranges::enable_borrowed_range<base::span<T, N, Ptr>> = true;
...
template <typename T>
concept IntegralConstantLike = std::is_integral_v<decltype(T::value)> && !std::is_same_v<bool, std::remove_const_t<decltype(T::value)>>
    && std::convertible_to<T, decltype(T::value)> && std::equality_comparable_with<T, decltype(T::value)> && std::bool_constant<T() == T::value>::value
    && std::bool_constant<static_cast<decltype(T::value)>(T()) == T::value>::value;

// Exposition-only concept from [span.syn]
template <typename T> inline constexpr size_t MaybeStaticExt = dynamic_extent;
template <typename T>
...
concept CompatibleIter = std::contiguous_iterator<It> && LegalDataConversion<std::remove_reference_t<std::iter_reference_t<It>>, T>;
...
template <typename T> inline constexpr bool kCompatibleRangeType = !std::is_array_v<T>;
// `span`s should go through the copy constructor.
template <typename T, size_t N, typename P> inline constexpr bool kCompatibleRangeType<span<T, N, P>> = false;

template <typename T, typename R>
concept CompatibleRange = std::ranges::contiguous_range<R> && std::ranges::sized_range<R>
    && (std::ranges::borrowed_range<R> || std::is_const_v<T>) && kCompatibleRangeType<std::remove_cvref_t<R>>
...
concept FixedExtentConstructibleFromExtent = X ==
N || X == dynamic_extent;

// Computes a fixed extent if possible from a source container type `T`.
template <typename T> inline constexpr size_t kComputedExtentImpl = dynamic_extent;
template <typename T, size_t N> inline constexpr size_t kComputedExtentImpl<T[N]> = N;
template <typename T, size_t N> inline constexpr size_t kComputedExtentImpl<std::array<T, N>> = N;
template <typename T, size_t N> inline constexpr size_t kComputedExtentImpl<std::span<T, N>> = N;
template <typename T, size_t N, typename InternalPtrType> inline constexpr size_t kComputedExtentImpl<span<T, N, InternalPtrType>> = N;
template <typename T> inline constexpr size_t kComputedExtent = kComputedExtentImpl<std::remove_cvref_t<T>>;

template <class T, class U, size_t N, size_t M>
    requires((N == M || N == dynamic_extent || M == dynamic_extent) && std::equality_comparable_with<T, U>)
constexpr bool span_eq(span<T, N> l, span<U, M> r);
template <class T, class U, size_t N, size_t M>
    requires((N == M || N == dynamic_extent || M == dynamic_extent) && std::three_way_comparable_with<T, U>)
constexpr auto span_cmp(span<T, N> l, span<U, M> r) -> decltype(l[0u] <=> r[0u]);
template <class T, size_t N> constexpr std::ostream& span_stream(std::ostream& l, span<T, N> r);

} // namespace internal

// [span], class template span
template <typename T, size_t N, typename InternalPtrType> class GSL_POINTER span {
public:
    ...
    template <typename R, size_t X = internal::kComputedExtent<R>>
        requires(internal::CompatibleRange<T, R> && internal::FixedExtentConstructibleFromExtent<N, X>)
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename R, size_t X = internal::kComputedExtent<R>>
        requires(internal::CompatibleRange<T, R> && internal::FixedExtentConstructibleFromExtent<N, X> && std::ranges::borrowed_range<R>)
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename OtherT, size_t OtherN, typename OtherInternalPtrType>
        requires((OtherN == dynamic_extent || N == OtherN) && internal::LegalDataConversion<OtherT, T>)
    constexpr explicit(OtherN == dynamic_extent) span(const span<OtherT, OtherN, OtherInternalPtrType>& s) noexcept
    ...
    // std::size().
    //   template <>
    //   span<std::string_view>(std::string_view s) noexcept
    //     : span(std::data(s), std::size(s))
    //   {
    //   }
    ...
    template <size_t Offset>
        requires(Offset <= N)
    constexpr std::pair<span<T, Offset>, span<T, N - Offset>> split_at() const noexcept
    {
        ...
        requires(!std::is_const_v<T>)
    {
        if constexpr (std::is_trivially_copyable_v<T>) {
            ...
            if (data_intptr < other_data_intptr) {
                ...
                // pointers of the same type) through the parameter's type, so `data()`
                // and `other.data()` both point to at least `N` elements.
                UNSAFE_BUFFERS(std::copy_backward(other.data(), other.data() + N, data() + N));
            }
        }
    }
    ...
    UNSAFE_BUFFER_USAGE constexpr void copy_from_nonoverlapping(span<const T, N> other)
        requires(!std::is_const_v<T>)
    {
    ...
    template <class R, size_t X = internal::kComputedExtent<R>>
        requires((X <= N || X == dynamic_extent) && std::convertible_to<R, span<const T, X>>)
    ...
    operator std::span<T, N>() const
    {
        return std::span<T, N>(*this);
    }
    ...
    template <class U, size_t M>
        requires((N == M || M == dynamic_extent) && std::equality_comparable_with<const T, const U>)
    ...
    template <class U, size_t M>
        requires((N == M || M == dynamic_extent) && std::three_way_comparable_with<const T, const U>)
...
public:
    ...
    //--weolar
    span& operator=(const std::vector<uint8_t>& other)
    {
        data_ = other.data();
        size_ = other.size();
        return *this;
...
span(It, EndOrSize) -> span<std::remove_reference_t<std::iter_reference_t<It>>, internal::MaybeStaticExt<EndOrSize>>;

template <typename T, size_t N> span(T (&)[N]) -> span<T, N>;

template <typename R>
    requires(std::ranges::contiguous_range<R>)
span(R&&) -> span<std::remove_reference_t<std::ranges::range_reference_t<R>>, internal::kComputedExtent<R>>;

// [span.objectrep], views of object representation
template <typename T, size_t X, typename InternalPtrType> constexpr auto as_bytes(span<T, X, InternalPtrType> s) noexcept
{
    ...
    return UNSAFE_BUFFERS(span<const uint8_t, N>(reinterpret_cast<const uint8_t*>(s.data()), s.size_bytes()));
}
...
constexpr auto as_writable_bytes(span<T, X, InternalPtrType> s) noexcept
{
    ...
    // The reinterpret_cast is valid as the alignment of uint8_t (which is 1) is
    // always less-than or equal to the alignment of T.
    return UNSAFE_BUFFERS(span<uint8_t, N>(reinterpret_cast<uint8_t*>(s.data()), s.size_bytes()));
}
...
template <typename T, size_t X, typename InternalPtrType> constexpr auto as_chars(span<T, X, InternalPtrType> s) noexcept
{
    constexpr size_t N = X == dynamic_extent ? dynamic_extent : sizeof(T) * X;
    // SAFETY: span provides that data() points to at least size_bytes() many
    ...
    // always less-than or equal to the alignment of T.
    return UNSAFE_BUFFERS(span<const char, N>(reinterpret_cast<const char*>(s.data()), s.size_bytes()));
}
...
constexpr std::string_view as_string_view(span<const unsigned char> s) noexcept
{
...
auto as_writable_chars(span<T, X, InternalPtrType> s) noexcept
{
    ...
    // The reinterpret_cast is valid as the alignment of char (which is 1) is
    // always less-than or equal to the alignment of T.
    return UNSAFE_BUFFERS(span<char, N>(reinterpret_cast<char*>(s.data()), s.size_bytes()));
}
...
template <int&... ExplicitArgumentBarrier, typename Container>
    requires(internal::SpanConstructibleFrom<Container &&> && std::ranges::borrowed_range<Container>)
...
template <typename T> constexpr span<const T, 1u> span_from_ref(const T& single_object LIFETIME_BOUND) noexcept
{
...
template <typename T> constexpr span<T, 1u> span_from_ref(T& single_object LIFETIME_BOUND) noexcept
{
    // SAFETY: Given a valid reference to `single_object` the span of size 1 will
    // be a valid span that points to the `single_object`.
...
template <typename T> constexpr span<const uint8_t, sizeof(T)> byte_span_from_ref(const T& single_object LIFETIME_BOUND) noexcept
{
    return as_bytes(span_from_ref(single_object));
}
template <typename T> constexpr span<uint8_t, sizeof(T)> byte_span_from_ref(T& single_object LIFETIME_BOUND) noexcept
{
...
constexpr span<const CharT, N - 1> span_from_cstring(const CharT (&lit LIFETIME_BOUND)[N])
    ENABLE_IF_ATTR(lit[N - 1u] == CharT { 0 }, "requires string literal as input")
{
...
// always preserved.
template <class CharT, size_t N>
constexpr span<const CharT, N> span_with_nul_from_cstring(const CharT (&lit LIFETIME_BOUND)[N])
    ENABLE_IF_ATTR(lit[N - 1u] == CharT { 0 }, "requires string literal as input")
{
    return span(lit);
}
...
// always preserved.
template <size_t N>
constexpr span<const uint8_t, N - 1> byte_span_from_cstring(const char (&lit LIFETIME_BOUND)[N])
...
// always preserved.
template <size_t N>
constexpr span<const uint8_t, N> byte_span_with_nul_from_cstring(const char (&lit LIFETIME_BOUND)[N])
    ENABLE_IF_ATTR(lit[N - 1u] == '\0', "requires string literal as input")
{
...
template <int&... ExplicitArgumentBarrier, typename Spannable>
  requires(internal::SpanConstructibleFrom<const Spannable&> && std::ranges::borrowed_range<Spannable>)
constexpr auto as_byte_span(const Spannable& arg) {
  return as_bytes(span(arg));
}
#else
template <typename Spannable> constexpr auto as_byte_span(const Spannable& arg)
{
    return as_bytes(span(arg));
}
template <> constexpr auto as_byte_span<std::string>(const std::string& arg)
{
    return as_bytes(span(arg.data(), arg.size()));
}

template <> constexpr auto as_byte_span<std::string_view>(const std::string_view& arg)
{
    return as_bytes(span(arg.data(), arg.size()));
}
#endif

template <int&... ExplicitArgumentBarrier, typename T, size_t N> constexpr span<const uint8_t, N * sizeof(T)> as_byte_span(const T (&arr LIFETIME_BOUND)[N])
{
...
template <int&... ExplicitArgumentBarrier, typename Spannable>
    requires(internal::SpanConstructibleFrom<Spannable &&> && !std::is_const_v<typename decltype(span(std::declval<Spannable>()))::element_type>)
...
template <int&... ExplicitArgumentBarrier, typename T, size_t N>
    requires(!std::is_const_v<T>)
constexpr span<uint8_t, N * sizeof(T)> as_writable_byte_span(T (&arr LIFETIME_BOUND)[N])
{
