...
inline constexpr size_t kUsingFullMapSentinel = std::numeric_limits<size_t>::max();
...
template <typename NormalMap> class small_map_default_init {
public:
...
template <typename M> struct has_key_equal {
...
template <typename M, bool has_key_equal_value> struct select_equal_key {
    struct equal_key {
...
// unordered_map<>.
template <typename M> struct select_equal_key<M, true> {
...
public:
    ...
    public:
        ...
        {
            return has_array_iter() ? std::addressof(array_iter_->value) : std::addressof(*map_iter_);
        }
    ...
    private:
        ...
        constexpr explicit iterator(const map_iterator& init)
            : map_iter_(init)
        {
        }
    ...
    // Invalidates iterators.
    template <class InputIterator> constexpr void insert(InputIterator f, InputIterator l)
    {
        while (f != l) {
            insert(*f);
            ++f;
    ...
    // Invalidates iterators.
    template <typename... Args> constexpr std::pair<iterator, bool> emplace(Args&&... args)
    {
        key_equal compare;
    ...
    {
        return UsingFullMap() ? iterator(map_.begin()) : iterator(sized_array_span().begin());
    }

    constexpr const_iterator begin() const
    {
        return UsingFullMap() ? const_iterator(map_.begin()) : const_iterator(sized_array_span().begin());
    }

    constexpr iterator end()
    {
        return UsingFullMap() ? iterator(map_.end()) : iterator(sized_array_span().end());
    }

    constexpr const_iterator end() const
    {
        return UsingFullMap() ? const_iterator(map_.end()) : const_iterator(sized_array_span().end());
    }
    ...
    constexpr iterator erase(const iterator& position)
    {
        ...
        if (erase_pos == last_pos) {
            ...
            erase_pos->value.~value_type();
            std::construct_at(std::addressof(erase_pos->value), std::move(last_pos->value));
