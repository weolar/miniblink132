...
template <class C> void IdentityTest()
{
    ...
    EXPECT_THAT(ToValueList(c), testing::ElementsAre(IsInt(1), IsInt(2), IsInt(3), IsInt(4), IsInt(5)));
}
...
template <class C> void ProjectionTest()
{
    C c = { 1, 2, 3, 4, 5 };
    // See comment in `IsInt()` above.
    EXPECT_THAT(ToValueList(c, [](int x) { return x + 1; }), testing::ElementsAre(IsInt(2), IsInt(3), IsInt(4), IsInt(5), IsInt(6)));
}
...
TEST(ToListTest, MoveOnly)
{
    Value::List list;
    list.resize(10);
    Value::List mapped_list = ToValueList(std::move(list), [](Value& value) { return std::move(value); });
