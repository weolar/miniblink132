...
using StringToStringUniquePtrMap = base::flat_map<std::string, std::unique_ptr<std::string>>;
...
TEST(MapUtilTest, FindPtrOrNullForPointers)
{
    ...
    EXPECT_THAT(FindPtrOrNull(mapping, kKey), AllOf(Eq(val.get()), Pointee(Eq(kValue))));
