...
template <typename Map> using MappedType = typename Map::mapped_type;

} // namespace internal

// Returns a pointer to the const value associated with the given key if it
// exists, or null otherwise.
template <typename Map, typename Key> constexpr const internal::MappedType<Map>* FindOrNull(const Map& map, const Key& key)
{
...
// null otherwise.
template <typename Map, typename Key> constexpr internal::MappedType<Map>* FindOrNull(Map& map, const Key& key)
{
    auto it = map.find(key);
    return it != map.end() ? &it->second : nullptr;
}
...
// to a null value.
template <typename Map, typename Key, typename MappedElementType = std::pointer_traits<internal::MappedType<Map>>::element_type>
constexpr const MappedElementType* FindPtrOrNull(const Map& map, const Key& key)
{
    auto it = map.find(key);
