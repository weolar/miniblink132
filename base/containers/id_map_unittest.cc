...
struct RepeatingKeyType {
    explicit RepeatingKeyType(int i)
        : i(i)
    {
    }
    ...
    constexpr RepeatingKeyType& operator++(int)
    {
        return *this;
    }
...
namespace std {
template <> struct hash<::base::test::id_map::RepeatingKeyType> {
    size_t operator()(const ::base::test::id_map::RepeatingKeyType& k) const noexcept
    {
...
TEST(IDMapTest, OwningPointersDeletesThemOnRemove)
{
    ...
    for (int i = 0; i < kCount; ++i) {
        ...
        map_owned_ids[i] = map_owned.Add(std::make_unique<DestructorCounter>(&owned_del_count));
    }
