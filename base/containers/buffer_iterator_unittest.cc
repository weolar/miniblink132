...
TEST(BufferIteratorTest, Object)
{
    ...
    }
    {
        ...
        BufferIterator<char> iterator(span<char>(buffer).first<sizeof(buffer) - 1u>());
        const TestStruct* actual = iterator.Object<TestStruct>();
        EXPECT_FALSE(actual);
    }
}
...
TEST(BufferIteratorTest, FixedSpan)
{
...

    {
        ...
        static_assert(std::same_as<std::optional<base::span<TestStruct, 3u>>, decltype(span)>);
        for (auto& ts : *span) {
            memcpy(&ts, &expected, sizeof(expected));
        }
    }
...
TEST(BufferIteratorTest, SpanOverflow)
{
    ...
    constexpr size_t kInvalidU64Size = (std::numeric_limits<size_t>::max() / sizeof(uint64_t)) + 1u;
    {
        span<const uint64_t> empty_span = iterator.Span<uint64_t>(kInvalidU64Size);
        EXPECT_TRUE(empty_span.empty());
    }
    {
        span<const uint64_t> empty_span = iterator.Span<uint64_t>(std::numeric_limits<size_t>::max());
        EXPECT_TRUE(empty_span.empty());
    }
}
...
TEST(BufferIteratorTest, CopyObject)
{
    ...
    for (int i = 0; i < kNumCopies; i++) {
        as_writable_bytes(span(buffer)).subspan(i * sizeof(TestStruct)).first<sizeof(TestStruct)>().copy_from(byte_span_from_ref(expected));
    }
