...
template <typename B> class BufferIterator {
public:
    static_assert(
        std::same_as<std::remove_const_t<B>, char> || std::same_as<std::remove_const_t<B>, unsigned char>, "Underlying buffer type must be char-type.");
    ...
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>> std::optional<T> CopyObject()
    {
        std::optional<T> t;
    ...
    // production.
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>> const T* Object()
    {
    ...
    // TODO(danakj): We should probably CHECK this instead of allowing UB into
    // production.
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>> T* MutableObject()
    {
        T* t = nullptr;
        if (remaining_.size() >= sizeof(T)) {
            auto [source, remain] = remaining_.template split_at<sizeof(T)>();
    ...
    // TODO(danakj): We should probably CHECK this instead of allowing UB into
    // production.
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>> span<T> MutableSpan(size_t count)
    {
        size_t byte_size;
    ...
    // a fixed-size span.
    template <typename T, size_t N, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>>
        requires(N <= std::numeric_limits<size_t>::max() / sizeof(T))
    std::optional<span<T, N>> MutableSpan()
    {
        constexpr size_t byte_size = N * sizeof(T); // Overflow is checked by `requires`.
        if (byte_size > remaining_.size()) {
            return std::nullopt;
        }
        auto [lhs, rhs] = remaining_.split_at(byte_size);
    ...
    // TODO(danakj): We should probably CHECK this instead of allowing UB into
    // production.
    template <typename T, typename = std::enable_if_t<std::is_trivially_copyable_v<T>>> span<const T> Span(size_t count)
    {
    ...
    {
        remaining_ = remaining_.first(size);
    }
    ...
    size_t position() const
    {
        ...
        // non-negative.
        return static_cast<size_t>(UNSAFE_BUFFERS(remaining_.data() - buffer_.data()));
    }
