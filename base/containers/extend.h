...
template <typename T> void Extend(std::vector<T>& dst, std::vector<T>&& src)
{
...
// `vector<T>` parameter).
template <typename T> void Extend(std::vector<T>& dst, std::type_identity_t<span<const T>> src)
{
