...
template <typename T> class SpanOrSize {
public:
    explicit constexpr SpanOrSize(base::span<T> span)
        : span_or_size_(span)
    {
    }
    explicit constexpr SpanOrSize(size_t size)
        : span_or_size_(size)
    {
    }
    ...
    {
        return std::visit(base::Overloaded {
                              [](const base::span<T>& span) { return optional_ref<const base::span<T>>(span); },
