...
// static method.
template <typename T, typename Deleter = void> class TRIVIAL_ABI GSL_OWNER HeapArray {
public:
    static_assert(!std::is_const_v<T>, "HeapArray cannot hold const types");
    static_assert(!std::is_reference_v<T>, "HeapArray cannot hold reference types");
    ...
    using deleter_type = std::conditional_t<std::is_void_v<Deleter>, std::default_delete<T[]>, Deleter>;
    ...
    T* data() LIFETIME_BOUND
    {
        return data_.get();
    }
    const T* data() const LIFETIME_BOUND
    {
        return data_.get();
    }
    ...
    T& operator[](size_t idx) LIFETIME_BOUND
    {
        return as_span()[idx];
    }
    ...
    // the HeapArray.
    base::span<T> subspan(size_t offset, size_t count = base::dynamic_extent) LIFETIME_BOUND
    {
        return as_span().subspan(offset, count);
    }
    base::span<const T> subspan(size_t offset, size_t count = base::dynamic_extent) const LIFETIME_BOUND
    {
        return as_span().subspan(offset, count);
    }
