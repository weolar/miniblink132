...
template <typename T> class CheckedContiguousIterator {
public:
    ...
    template <typename U> friend class CheckedContiguousIterator;

    // Required to be able to get to the underlying pointer without triggering
    // CHECK failures.
    template <typename Ptr> friend struct std::pointer_traits;
    ...
    UNSAFE_BUFFER_USAGE constexpr CheckedContiguousIterator(T* start, const T* end)
        : start_(start)
        , current_(start)
        , end_(end)
    {
    ...
    // otherwise.
    UNSAFE_BUFFER_USAGE constexpr CheckedContiguousIterator(const T* start, T* current, const T* end)
        : start_(start)
        , current_(current)
        , end_(end)
    {
    ...
    constexpr CheckedContiguousIterator& operator=(const CheckedContiguousIterator& other) = default;

    friend constexpr bool operator==(const CheckedContiguousIterator& lhs, const CheckedContiguousIterator& rhs)
    {
        lhs.CheckComparable(rhs);
        return lhs.current_ == rhs.current_;
    }

    friend constexpr auto operator<=>(const CheckedContiguousIterator& lhs, const CheckedContiguousIterator& rhs)
    {
        lhs.CheckComparable(rhs);
        return lhs.current_ <=> rhs.current_;
    }
    ...
    constexpr friend CheckedContiguousIterator operator+(difference_type lhs, const CheckedContiguousIterator& rhs)
    {
        return rhs + lhs;
    }
    ...
    constexpr friend difference_type operator-(const CheckedContiguousIterator& lhs, const CheckedContiguousIterator& rhs)
    {
        lhs.CheckComparable(rhs);
        return lhs.current_ - rhs.current_;
    }
    ...
    [[nodiscard]] static bool IsRangeMoveSafe(
        const CheckedContiguousIterator& from_begin, const CheckedContiguousIterator& from_end, const CheckedContiguousIterator& to)
    {
        ...
        const auto to_end_uintptr = get_uintptr((to + std::distance(from_begin, from_end)).current_);
...
template <typename T> using CheckedContiguousConstIterator = CheckedContiguousIterator<const T>;
...
template <typename T> struct std::pointer_traits<::base::CheckedContiguousIterator<T>> {
    using pointer = ::base::CheckedContiguousIterator<T>;
    using element_type = T;
    using difference_type = ptrdiff_t;

    template <typename U> using rebind = ::base::CheckedContiguousIterator<U>;
