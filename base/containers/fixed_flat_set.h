...
//   bool operator<=(const fixed_flat_set&, const fixed_flat_set&);
//
template <class Key, size_t N, class Compare = std::less<>> using fixed_flat_set = base::flat_set<Key, Compare, std::array<const Key, N>>;
...
consteval fixed_flat_set<Key, N, Compare> MakeFixedFlatSet(sorted_unique_t, std::common_type_t<Key> (&&data)[N], const Compare& comp = Compare())
{
...
template <class Key, size_t N, class Compare = std::less<>>
consteval fixed_flat_set<Key, N, Compare> MakeFixedFlatSet(std::common_type_t<Key> (&&data)[N], const Compare& comp = Compare())
{
