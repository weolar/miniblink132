...
#define EAT_CHECK_STREAM_PARAMS(expr) true ? (void)0 : ::logging::VoidifyStream(expr) & (*::logging::g_swallow_stream)
...
public:
    static CheckError Check(const char* condition, base::NotFatalUntil fatal_milestone = base::NotFatalUntil::NoSpecifiedMilestoneInternal,
        const base::Location& location = base::Location::Current());
    // Takes ownership over (free()s after using) `log_message_str`, for use with
    // CHECK_op macros.
    static CheckError CheckOp(char* log_message_str, base::NotFatalUntil fatal_milestone = base::NotFatalUntil::NoSpecifiedMilestoneInternal,
        const base::Location& location = base::Location::Current());

    static CheckError DCheck(const char* condition, const base::Location& location = base::Location::Current());
    // Takes ownership over (free()s after using) `log_message_str`, for use with
    // DCHECK_op macros.
    static CheckError DCheckOp(char* log_message_str, const base::Location& location = base::Location::Current());

    static CheckError DumpWillBeCheck(const char* condition, const base::Location& location = base::Location::Current());
    // Takes ownership over (free()s after using) `log_message_str`, for use with
    // DUMP_WILL_BE_CHECK_op macros.
    static CheckError DumpWillBeCheckOp(char* log_message_str, const base::Location& location = base::Location::Current());

    static CheckError PCheck(const char* condition, const base::Location& location = base::Location::Current());
    static CheckError PCheck(const base::Location& location = base::Location::Current());

    static CheckError DPCheck(const char* condition, const base::Location& location = base::Location::Current());

    static CheckError DumpWillBeNotReachedNoreturn(const base::Location& location = base::Location::Current());

    static CheckError NotImplemented(const char* function, const base::Location& location = base::Location::Current());
    ...
    template <typename T> std::ostream& operator<<(T&& streamed_type)
    {
...
public:
    static NotReachedError NotReached(
        base::NotFatalUntil fatal_milestone = base::NotFatalUntil::NoSpecifiedMilestoneInternal, const base::Location& location = base::Location::Current());
...
public:
    explicit NotReachedNoreturnError(const base::Location& location = base::Location::Current());
...
#define LOGGING_CHECK_FUNCTION_IMPL(check_stream, condition)                                                                                                   \
    ...
    default:                                                                                                                                                   \
        ...
        if (const bool probably_true = static_cast<bool>(condition); ANALYZER_ASSUME_TRUE(probably_true))                                                      \
            [[likely]];                                                                                                                                        \
        else                                                                                                                                                   \
            (check_stream)
...
#define CHECK(cond, ...)                                                                                                                                       \
    BASE_IF(BASE_IS_EMPTY(__VA_ARGS__), BASE_INTERNAL_EXPECT_FALSE(cond) ? logging::CheckFailure() : EAT_CHECK_STREAM_PARAMS(),                                \
        LOGGING_CHECK_FUNCTION_IMPL(logging::CheckError::Check(#cond, __VA_ARGS__), cond))

#define CHECK_WILL_STREAM() false

// Strip the conditional string from official builds.
#define PCHECK(condition) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::PCheck(), condition)

#else

#define CHECK_WILL_STREAM() true

#define CHECK(condition, ...) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::Check(#condition __VA_OPT__(, ) __VA_ARGS__), condition)

#define PCHECK(condition) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::PCheck(#condition), condition)

#endif

#if DCHECK_IS_ON()

#define DCHECK(condition) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::DCheck(#condition), condition)
#define DPCHECK(condition) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::DPCheck(#condition), condition)
...
#define DUMP_WILL_BE_CHECK(condition, ...) LOGGING_CHECK_FUNCTION_IMPL(::logging::CheckError::DumpWillBeCheck(#condition __VA_OPT__(, ) __VA_ARGS__), condition)
