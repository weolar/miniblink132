...
// it gets ugly.
template <class Foo> struct PickObserverList { };
template <> struct PickObserverList<CheckedBase> {
    template <class TypeParam, bool check_empty = false, bool allow_reentrancy = true>
    using ObserverListType = ObserverList<TypeParam, check_empty, allow_reentrancy>;
};
template <> struct PickObserverList<UncheckedBase> {
    template <class TypeParam, bool check_empty = false, bool allow_reentrancy = true>
    using ObserverListType = typename ObserverList<TypeParam, check_empty, allow_reentrancy>::Unchecked;
};

template <class Foo> class AdderT : public Foo {
public:
    explicit AdderT(int scaler)
        : total(0)
        , scaler_(scaler)
    {
    }
...
template <class ObserverListType, class Foo = typename ObserverListType::value_type> class DisrupterT : public Foo {
public:
...
template <class ObserverListType, class Foo = typename ObserverListType::value_type> class AddInObserve : public Foo {
public:
...
template <class ObserverListType> class ObserverListCreator : public DelegateSimpleThread::Delegate {
public:
    std::unique_ptr<ObserverListType> Create(std::optional<base::ObserverListPolicy> policy = std::nullopt)
    {
        policy_ = policy;
...
public:
    ...
    template <class T> const decltype(T::list_.get()) list(const T& iter)
    {
        return iter.list_.get();
    }

    template <class T> typename T::value_type* GetCurrent(T* iter)
    {
...
template <class ObserverType> class ObserverListTest : public ObserverListTestBase, public ::testing::Test {
public:
    template <class T> using ObserverList = typename PickObserverList<ObserverType>::template ObserverListType<T>;
...
#define DECLARE_TYPES                                                                                                                                          \
    using Foo = TypeParam;                                                                                                                                     \
    using ObserverListFoo = typename PickObserverList<TypeParam>::template ObserverListType<Foo>;                                                              \
...
TYPED_TEST(ObserverListTest, CreatedAndUsedOnDifferentThreads)
{
    ...
    {
        std::unique_ptr<ObserverListFoo> observer_list = list_creator.Create(base::ObserverListPolicy::EXISTING_ONLY);
        observer_list->AddObserver(&a);
        for (auto& observer : *observer_list) {
            observer.Observe(1);
        }
...
{
    DECLARE_TYPES;
    using ObserverListConstFoo = typename TestFixture::template ObserverList<const Foo>;
...
template <class ObserverListType, class Foo = typename ObserverListType::value_type> class AddInClearObserve : public Foo {
public:
...
template <class ObserverListType, class Foo = typename ObserverListType::value_type> class ListDestructor : public Foo {
public:
    explicit ListDestructor(ObserverListType* list)
        : list_(list)
    {
    }
    ...
    void Observe(int x) override
    {
        delete list_.ExtractAsDangling();
    }
...
TYPED_TEST(ObserverListTest, BasicStdIterator)
{
    ...
    for (const_iterator i = const_list.begin(), e = const_list.end(); i != e; ++i) {
...
{
    DECLARE_TYPES;
    using NonReentrantObserverListFoo = typename PickObserverList<Foo>::template ObserverListType<Foo, /*check_empty=*/false,
...
{
    DECLARE_TYPES;
    using ReentrantObserverListFoo = typename PickObserverList<Foo>::template ObserverListType<Foo, /*check_empty=*/false,
...
public:
    explicit TestCheckedObserver(int* count)
        : count_(count)
    {
    }
...
public:
    explicit TestCheckedObserver2(int* count)
        : count_(count)
    {
    }
...
class MultiObserver : public TestCheckedObserver, public TestCheckedObserver2, public AdderT<UncheckedBase> {
public:
