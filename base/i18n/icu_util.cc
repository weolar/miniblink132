...
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_CHROMEOS) || (BUILDFLAG(IS_LINUX) && !BUILDFLAG(IS_CASTOS))
...
#if BUILDFLAG(IS_ANDROID)
    int fd = android::OpenApkAsset(kAndroidAssetsIcuDataFileName, &g_icudtl_region);
    g_icudtl_pf = fd;
    if (fd != -1) {
        return;
    }
...
int LoadIcuData(
    PlatformFile data_fd, const MemoryMappedFile::Region& data_region, std::unique_ptr<MemoryMappedFile>* out_mapped_data_file, UErrorCode* out_error_code)
{
...
void InitializeIcuTimeZone()
{
    ...
    icu::TimeZone::adoptDefault(icu::TimeZone::createTimeZone(icu::UnicodeString(false, zone_id.data(), zone_id.length())));
    ...
    icu::TimeZone::adoptDefault(icu::TimeZone::createTimeZone(icu::UnicodeString::fromUTF8(zone_id)));
