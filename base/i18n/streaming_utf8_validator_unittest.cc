...
protected:
    ...
    void TestNumber(uint32_t n) const
    {
        ...
        EXPECT_EQ(IsStringUtf8(test), validator.AddBytes(test) == VALID_ENDPOINT)
            << "Difference of opinion for \"" << base::StringPrintf("\\x%02X\\x%02X\\x%02X\\x%02X", test[0], test[1], test[2], test[3]) << "\"";
    }
...
public:
    ...
    void TestRange(uint32_t begin, uint32_t size)
    {
        ...
        ++tasks_finished_;
        LOG(INFO) << tasks_finished_ << " / " << tasks_dispatched_ << " tasks done\n";
    }
...
TEST_F(StreamingUtf8ValidatorThoroughTest, DISABLED_TestEverything)
{
    base::test::TaskEnvironment task_environment;
    {
        ...
        uint32_t begin = 0;
        do {
            base::ThreadPool::PostTask(FROM_HERE, { base::TaskShutdownBehavior::BLOCK_SHUTDOWN },
                base::BindOnce(&StreamingUtf8ValidatorThoroughTest::TestRange, base::Unretained(this), begin, kThoroughTestChunkSize));
            ++tasks_dispatched_;
...
public:
    ...
    static PartialIterator end()
    {
        return PartialIterator(std::size(valid), 1);
    }
...
private:
    ...
    void Advance()
    {
        if (index_ < std::size(valid) && prefix_length_ < strlen(valid[index_]))
            ++prefix_length_;
        while (index_ < std::size(valid) && prefix_length_ == strlen(valid[index_])) {
            ++index_;
            prefix_length_ = 1;
        }
    }
...
protected:
    ...
    template <typename Iterator> void CheckRange(Iterator begin, Iterator end, StreamingUtf8Validator::State expected)
    {
        for (Iterator it = begin; it != end; ++it) {
            StreamingUtf8Validator validator;
            std::string_view sequence = *it;
            EXPECT_EQ(expected, validator.AddBytes(base::as_byte_span(sequence))) << "Failed for \"" << sequence << "\"";
        }
    }

    // Adding input a byte at a time should make absolutely no difference.
    template <typename Iterator> void CheckRangeByteAtATime(Iterator begin, Iterator end, StreamingUtf8Validator::State expected)
    {
        for (Iterator it = begin; it != end; ++it) {
            StreamingUtf8Validator validator;
            std::string_view sequence = *it;
...
protected:
    ...
    void CheckCombinations(Iterator1 begin1, Iterator1 end1, Iterator2 begin2, Iterator2 end2, StreamingUtf8Validator::State expected)
    {
        StreamingUtf8Validator validator;
        for (Iterator1 it1 = begin1; it1 != end1; ++it1) {
            ...
            for (Iterator2 it2 = begin2; it2 != end2; ++it2) {
                ...
                EXPECT_EQ(expected, validator.AddBytes(base::as_byte_span(c2))) << "Failed for \"" << c1 << c2 << "\"";
                validator.Reset();
            }
        }
    }
...
TEST(StreamingUtf8ValidatorTest, NulIsValid)
{
    ...
    EXPECT_EQ(VALID_ENDPOINT, StreamingUtf8Validator().AddBytes(base::as_bytes(base::make_span(kNul, 1u))));
}
...
TEST(StreamingUtf8ValidatorTest, HelloWorld)
{
    ...
    EXPECT_EQ(VALID_ENDPOINT, StreamingUtf8Validator().AddBytes(base::as_bytes(base::make_span(kHelloWorld, strlen(kHelloWorld)))));
}
...
{
    StreamingUtf8Validator validator;
    EXPECT_EQ(INVALID, validator.AddBytes(base::as_bytes(base::make_span("\xC0", 1u))));
    EXPECT_EQ(INVALID, validator.AddBytes(base::as_bytes(base::make_span("a", 1u))));
    validator.Reset();
    EXPECT_EQ(VALID_ENDPOINT, validator.AddBytes(base::as_bytes(base::make_span("a", 1u))));
}
...
{
    CheckRangeByteAtATime(PartialIterator(), PartialIterator::end(), VALID_MIDPOINT);
}
...
{
    CheckCombinations(valid, valid_end, PartialIterator(), PartialIterator::end(), VALID_MIDPOINT);
}

TEST_F(StreamingUtf8ValidatorDoubleSequenceTest, PartialPlusValidIsInvalid)
{
    CheckCombinations(PartialIterator(), PartialIterator::end(), valid, valid_end, INVALID);
}

TEST_F(StreamingUtf8ValidatorDoubleSequenceTest, PartialPlusPartialIsInvalid)
{
    CheckCombinations(PartialIterator(), PartialIterator::end(), PartialIterator(), PartialIterator::end(), INVALID);
}
...
{
    CheckCombinations(invalid, invalid_end, PartialIterator(), PartialIterator::end(), INVALID);
}

TEST_F(StreamingUtf8ValidatorDoubleSequenceTest, PartialPlusInvalidIsInvalid)
{
    CheckCombinations(PartialIterator(), PartialIterator::end(), invalid, invalid_end, INVALID);
}
