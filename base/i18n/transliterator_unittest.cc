...
TEST(TransliteratorTest, LowerCorrect)
{
    ...
    std::unique_ptr<icu::Transliterator> transliterator(icu::Transliterator::createInstance("Lower", UTRANS_FORWARD, parseErr, err));
...
TEST(TransliteratorTest, LatinASCIICorrect)
{
    UParseError parseErr;
    UErrorCode err = U_ZERO_ERROR;
    std::unique_ptr<icu::Transliterator> transliterator(icu::Transliterator::createInstance("Latin-ASCII", UTRANS_FORWARD, parseErr, err));
    ASSERT_TRUE(U_SUCCESS(err));
    icu::UnicodeString text(u"ÎÑŢÉRÑÅŢÎÖÑÅĻÎŽÅŢÎÖÑ");
    transliterator->transliterate(text);
...
TEST(TransliteratorTest, LowerLatinASCIICorrect)
{
    UParseError parseErr;
    UErrorCode err = U_ZERO_ERROR;
    std::unique_ptr<icu::Transliterator> transliterator(icu::Transliterator::createInstance("Lower;Latin-ASCII", UTRANS_FORWARD, parseErr, err));
    ASSERT_TRUE(U_SUCCESS(err));
    icu::UnicodeString text(u"ÎÑŢÉRÑÅŢÎÖÑÅĻÎŽÅŢÎÖÑ");
    transliterator->transliterate(text);
    EXPECT_EQ(base::i18n::UnicodeStringToString16(text), u"internationalization");
