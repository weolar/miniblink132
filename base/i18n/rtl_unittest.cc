...
#error wchar_t should be either UTF-16 or UTF-32
    ...
    for (auto& i : cases)
        EXPECT_EQ(i.direction, GetFirstStrongCharacterDirection(WideToUTF16(i.text)));
}
...
#error wchar_t should be either UTF-16 or UTF-32
    ...
    for (auto& i : cases)
        EXPECT_EQ(i.direction, GetLastStrongCharacterDirection(WideToUTF16(i.text)));
}
...
{
    const wchar_t* cases[] = { // Test common path, such as "c:\foo\bar".
        L"c:/foo/bar",
...
TEST_F(RTLTest, UnadjustStringForLocaleDirection)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        for (auto*& test_case : cases) {
            ...
            EXPECT_EQ(unadjusted_string, adjusted_string) << " for test case [" << unadjusted_string << "] with IsRTL() == " << IsRTL();
        }
    }
...
TEST_F(RTLTest, EnsureTerminatedDirectionalFormatting)
{
    struct {
        ...
        { L"\x202egoogle\x202c\x202c.\x202eom", L"\x202egoogle\x202c\x202c.\x202eom\x202c" },
        ...
        { L"\x202egoogle\x202e.com/\x202eguest", L"\x202egoogle\x202e.com/\x202eguest\x202c\x202c\x202c" },
...
TEST_F(RTLTest, SanitizeUserSuppliedString)
{
    struct {
        ...
        { L"\x202eكبير Google\x202cالتطبيق\x202c", L"\x202eكبير Google\x202cالتطبيق\x202c" },
        // Tests RTL string that over-terminates then re-opens.
        { L"\x202eكبير Google\x202c\x202cالتطبيق\x202e", L"\x202eكبير Google\x202c\x202cالتطبيق\x202e\x202c" },
        // Tests RTL string with multiple dir-formatting characters.
        { L"\x202eك\x202eبير Google الت\x202eطبيق", L"\x202eك\x202eبير Google الت\x202eطبيق\x202c\x202c\x202c" },
        // Test the other dir-formatting characters (U+202A, U+202B, and U+202D).
    ...
    for (auto& i : cases) {
        ...
        std::u16string sanitized_text = prefix + WideToUTF16(i.formatted_text) + suffix;
