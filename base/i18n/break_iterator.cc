...
template <UBreakIteratorType break_type> class DefaultLocaleBreakIteratorCache {
public:
    DefaultLocaleBreakIteratorCache()
    {
        main_ = UBreakIteratorPtr(ubrk_open(break_type, nullptr, nullptr, 0, &main_status_));
        if (U_FAILURE(main_status_)) {
            NOTREACHED() << "ubrk_open failed for type " << break_type << " with error " << main_status_;
        }
    }
    UBreakIteratorPtr Lease(UErrorCode& status)
    {
        ...
        // object instead.
        UBreakIteratorPtr result(ubrk_open(break_type, nullptr, nullptr, 0, &status));
        if (U_FAILURE(status)) {
...
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_CHARACTER>>::Leaky char_break_cache = LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_WORD>>::Leaky word_break_cache = LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_SENTENCE>>::Leaky sentence_break_cache = LAZY_INSTANCE_INITIALIZER;
static LazyInstance<DefaultLocaleBreakIteratorCache<UBRK_LINE>>::Leaky line_break_cache = LAZY_INSTANCE_INITIALIZER;
...
BreakIterator::BreakIterator(std::u16string_view str, const std::u16string& rules)
    : string_(str)
    , rules_(rules)
...
bool BreakIterator::Init()
{
    ...
    case BREAK_NEWLINE:
        ...
        break;
    case RULE_BASED:
        iter_ = UBreakIteratorPtr(ubrk_openRules(rules_.c_str(), static_cast<int32_t>(rules_.length()), nullptr, 0, &parse_error, &status));
        if (U_FAILURE(status)) {
            NOTREACHED() << "ubrk_openRules failed to parse rule string at line " << parse_error.line << ", offset " << parse_error.offset;
        }
        break;
    }
    ...
    if (string_.data() != nullptr) {
        ubrk_setText(iter_.get(), string_.data(), static_cast<int32_t>(string_.size()), &status);
        if (U_FAILURE(status)) {
            return false;
        }
    }
