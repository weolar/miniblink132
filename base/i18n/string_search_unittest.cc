...
#define EXPECT_MATCH_IGNORE_CASE(find_this, in_this, ex_start, ex_len)                                                                                         \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_TRUE(StringSearchIgnoringCaseAndAccents(find_this, in_this, &index, &length));                                                                  \
        EXPECT_EQ(ex_start, index);                                                                                                                            \
        EXPECT_EQ(ex_len, length);                                                                                                                             \
        index = 0;                                                                                                                                             \
        length = 0;                                                                                                                                            \
        EXPECT_TRUE(StringSearch(find_this, in_this, &index, &length, false, true));                                                                           \
        EXPECT_EQ(ex_start, index);                                                                                                                            \
        EXPECT_EQ(ex_len, length);                                                                                                                             \
    }
...
#define EXPECT_MATCH_SENSITIVE(find_this, in_this, ex_start, ex_len)                                                                                           \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_TRUE(StringSearch(find_this, in_this, &index, &length, true, true));                                                                            \
        EXPECT_EQ(ex_start, index);                                                                                                                            \
        EXPECT_EQ(ex_len, length);                                                                                                                             \
    }

#define EXPECT_MATCH_IGNORE_CASE_BACKWARDS(find_this, in_this, ex_start, ex_len)                                                                               \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_TRUE(StringSearch(find_this, in_this, &index, &length, false, false));                                                                          \
        EXPECT_EQ(ex_start, index);                                                                                                                            \
        EXPECT_EQ(ex_len, length);                                                                                                                             \
    }
...
#define EXPECT_MATCH_SENSITIVE_BACKWARDS(find_this, in_this, ex_start, ex_len)                                                                                 \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_TRUE(StringSearch(find_this, in_this, &index, &length, true, false));                                                                           \
        EXPECT_EQ(ex_start, index);                                                                                                                            \
        EXPECT_EQ(ex_len, length);                                                                                                                             \
    }
...
#define EXPECT_MISS_IGNORE_CASE(find_this, in_this)                                                                                                            \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_FALSE(StringSearchIgnoringCaseAndAccents(find_this, in_this, &index, &length));                                                                 \
        index = 0;                                                                                                                                             \
        length = 0;                                                                                                                                            \
        EXPECT_FALSE(StringSearch(find_this, in_this, &index, &length, false, true));                                                                          \
    }
...
#define EXPECT_MISS_SENSITIVE(find_this, in_this)                                                                                                              \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_FALSE(StringSearch(find_this, in_this, &index, &length, true, true));                                                                           \
    }
...
#define EXPECT_MISS_IGNORE_CASE_BACKWARDS(find_this, in_this)                                                                                                  \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_FALSE(StringSearch(find_this, in_this, &index, &length, false, false));                                                                         \
    }
...
#define EXPECT_MISS_SENSITIVE_BACKWARDS(find_this, in_this)                                                                                                    \
    {                                                                                                                                                          \
        size_t index = 0;                                                                                                                                      \
        size_t length = 0;                                                                                                                                     \
        EXPECT_FALSE(StringSearch(find_this, in_this, &index, &length, true, false));                                                                          \
    }
...
TEST(StringSearchTest, ASCII)
{
    ...
    EXPECT_MATCH_IGNORE_CASE(std::u16string(), u"searching for empty string", 0U, 0U);

    EXPECT_MATCH_IGNORE_CASE(u"case insensitivity", u"CaSe InSeNsItIvItY", 0U, 18U);

    EXPECT_MATCH_SENSITIVE(u"aabaaa", u"aaabaabaaa", 4U, 6U);

    EXPECT_MISS_SENSITIVE(u"searching within empty string", std::u16string());

    EXPECT_MATCH_SENSITIVE(std::u16string(), u"searching for empty string", 0U, 0U);
...
TEST(StringSearchTest, UnicodeLocaleIndependent)
{
    ...
    EXPECT_MATCH_IGNORE_CASE(e_base, e_with_acute_accent, 0U, e_with_acute_accent.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_accent, e_base, 0U, e_base.size());

    EXPECT_MATCH_IGNORE_CASE(e_base, e_with_acute_combining_mark, 0U, e_with_acute_combining_mark.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_combining_mark, e_base, 0U, e_base.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_combining_mark, e_with_acute_accent, 0U, e_with_acute_accent.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_accent, e_with_acute_combining_mark, 0U, e_with_acute_combining_mark.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_combining_mark, e_with_grave_combining_mark, 0U, e_with_grave_combining_mark.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_grave_combining_mark, e_with_acute_combining_mark, 0U, e_with_acute_combining_mark.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_acute_combining_mark, e_with_grave_accent, 0U, e_with_grave_accent.size());

    EXPECT_MATCH_IGNORE_CASE(e_with_grave_accent, e_with_acute_combining_mark, 0U, e_with_acute_combining_mark.size());

    EXPECT_MATCH_IGNORE_CASE(E_with_acute_accent, e_with_acute_accent, 0U, e_with_acute_accent.size());

    EXPECT_MATCH_IGNORE_CASE(E_with_grave_accent, e_with_acute_accent, 0U, e_with_acute_accent.size());

    EXPECT_MATCH_IGNORE_CASE(E_with_acute_combining_mark, e_with_grave_accent, 0U, e_with_grave_accent.size());

    EXPECT_MATCH_IGNORE_CASE(E_with_grave_combining_mark, e_with_acute_accent, 0U, e_with_acute_accent.size());

    EXPECT_MATCH_IGNORE_CASE(E_base, e_with_grave_accent, 0U, e_with_grave_accent.size());

    EXPECT_MISS_IGNORE_CASE(a_with_acute_accent, e_with_acute_accent);

    EXPECT_MISS_IGNORE_CASE(a_with_acute_combining_mark, e_with_acute_combining_mark);
    ...
    EXPECT_MATCH_SENSITIVE(e_with_acute_combining_mark, e_with_acute_accent, 0U, 1U);

    EXPECT_MATCH_SENSITIVE(e_with_acute_accent, e_with_acute_combining_mark, 0U, 2U);

    EXPECT_MISS_SENSITIVE(e_with_acute_combining_mark, e_with_grave_combining_mark);

    EXPECT_MISS_SENSITIVE(e_with_grave_combining_mark, e_with_acute_combining_mark);
    ...
    EXPECT_MISS_SENSITIVE(a_with_acute_combining_mark, e_with_acute_combining_mark);

    EXPECT_MATCH_SENSITIVE(a_with_acute_combining_mark, a_with_acute_combining_mark, 0U, 2U);
...
TEST(StringSearchTest, UnicodeLocaleDependent)
{
    ...
    EXPECT_TRUE(StringSearchIgnoringCaseAndAccents(a_base, a_with_ring, nullptr, nullptr));
    ...
    EXPECT_FALSE(StringSearchIgnoringCaseAndAccents(a_base, a_with_ring, nullptr, nullptr));
    EXPECT_FALSE(StringSearch(a_base, a_with_ring, nullptr, nullptr, false, true));
...
TEST(StringSearchTest, RepeatingStringSearch)
{
    ...
    // Case sensitive.
    {
        ...
        while (searcher.NextMatchResult(match_index, match_length)) {
            results.push_back({ .match_index = match_index, .match_length = match_length });
        }
