...
const char kProlog[] = "// Copyright 2013 The Chromium Authors\n"
...
void ConstructPairAndAppend(const Character& character, const Range& new_range, const StringSet& existing_set, PairVector* pairs)
{
...
void MoveRightMostCharToSet(PairVector* pairs)
{
    ...
    while (it != pairs->end()) {
        ...
        if (current_pair.character.size() == unconverted_bytes.size() + 1
            && std::equal(unconverted_bytes.begin(), unconverted_bytes.end(), current_pair.character.begin()) && converted == current_pair.set) {
        ...
        unconverted_bytes = Character(current_pair.character.begin(), current_pair.character.end() - 1);
...
void LogStringSets(const PairVector& pairs)
{
    for (const auto& pair_it : pairs) {
        ...
        for (auto set_it = pair_it.set.begin(); set_it != pair_it.set.end(); ++set_it) {
            set_as_string += base::StringPrintf("[\\x%02x-\\x%02x]", static_cast<int>(set_it->from()), static_cast<int>(set_it->to()));
        }
...
uint8_t MakeState(const StringSet& set, std::vector<State>* states, StateMap* state_map)
{
    DCHECK(!set.empty());
    ...
    const StateRange new_state_initializer[] = { { 0, 1 }, { range.from(), target_state }, { static_cast<uint8_t>(range.to() + 1), 1 } };
    states->push_back(State(new_state_initializer, new_state_initializer + std::size(new_state_initializer)));
...
std::vector<State> GenerateStates(const PairVector& pairs)
{
    ...
    for (auto it = pairs.begin(); it != pairs.end(); ++it) {
        ...
        if (states[0].back().from == range.from()) {
            ...
            DCHECK_LT(range.to(), 0xFF);
            const StateRange new_range_initializer[] = { { range.from(), target_state }, { static_cast<uint8_t>(range.to() + 1), 1 } };
            states[0].insert(states[0].end(), new_range_initializer, new_range_initializer + std::size(new_range_initializer));
        }
    }
    return states;
}
...
void PrintStates(const std::vector<State>& states, FILE* stream)
{
    ...
    for (uint8_t state_index = 0; state_index < states.size(); ++state_index) {
        ...
        fprintf(stream, "    // State %d, offset 0x%02x\n", static_cast<int>(state_index), static_cast<int>(state_offset[state_index]));
