...
std::string NextUtf8Sequence(const std::string& previous)
{
    ...
    DCHECK(StreamingUtf8Validator::Validate(next)) << "Result \"" << next << "\" failed validation";
    return next;
}
...
// using |description|.
bool RunTest(const std::string& description, TestTargetType target, const std::string& test_string, int times)
{
...
// |length|.
std::string ConstructRepeatedTestString(const std::string& input, size_t length)
{
...
// sequences.
std::string ConstructRangedTestString(const std::string& input_start, const std::string& input_end, size_t length)
{
...
const TestFunctionDescription kTestFunctions[]
    = { { &StreamingUtf8Validator::Validate, "StreamingUtf8Validator" }, { &IsStringUTF8, "IsStringUTF8" }, { &IsString7Bit, "IsString7Bit" } };
...
// is around 16MB.
void RunSomeTests(const char format[], base::RepeatingCallback<std::string(size_t length)> construct_test_string, const TestFunctionDescription* test_functions,
    size_t test_count)
{
    for (auto length : kTestLengths) {
        ...
        for (size_t test_index = 0; test_index < test_count; ++test_index) {
            EXPECT_TRUE(RunTest(StringPrintfNonConstexpr(format, test_functions[test_index].function_name, real_length, times),
...
{
    RunSomeTests("%s: bytes=1 repeated length=%d repeat=%d", base::BindRepeating(ConstructRepeatedTestString, kOneByteSeqRangeStart), kTestFunctions, 3);
}

TEST(StreamingUtf8ValidatorPerfTest, OneByteRange)
{
    RunSomeTests("%s: bytes=1 ranged length=%d repeat=%d", base::BindRepeating(ConstructRangedTestString, kOneByteSeqRangeStart, kOneByteSeqRangeEnd),
        kTestFunctions, 3);
}

TEST(StreamingUtf8ValidatorPerfTest, TwoByteRepeated)
{
    RunSomeTests("%s: bytes=2 repeated length=%d repeat=%d", base::BindRepeating(ConstructRepeatedTestString, kTwoByteSeqRangeStart), kTestFunctions, 2);
}

TEST(StreamingUtf8ValidatorPerfTest, TwoByteRange)
{
    RunSomeTests("%s: bytes=2 ranged length=%d repeat=%d", base::BindRepeating(ConstructRangedTestString, kTwoByteSeqRangeStart, kTwoByteSeqRangeEnd),
        kTestFunctions, 2);
}

TEST(StreamingUtf8ValidatorPerfTest, ThreeByteRepeated)
{
    RunSomeTests("%s: bytes=3 repeated length=%d repeat=%d", base::BindRepeating(ConstructRepeatedTestString, kThreeByteSeqRangeStart), kTestFunctions, 2);
}

TEST(StreamingUtf8ValidatorPerfTest, ThreeByteRange)
{
    RunSomeTests("%s: bytes=3 ranged length=%d repeat=%d", base::BindRepeating(ConstructRangedTestString, kThreeByteSeqRangeStart, kThreeByteSeqRangeEnd),
        kTestFunctions, 2);
}

TEST(StreamingUtf8ValidatorPerfTest, FourByteRepeated)
{
    RunSomeTests("%s: bytes=4 repeated length=%d repeat=%d", base::BindRepeating(ConstructRepeatedTestString, kFourByteSeqRangeStart), kTestFunctions, 2);
}

TEST(StreamingUtf8ValidatorPerfTest, FourByteRange)
{
    RunSomeTests("%s: bytes=4 ranged length=%d repeat=%d", base::BindRepeating(ConstructRangedTestString, kFourByteSeqRangeStart, kFourByteSeqRangeEnd),
        kTestFunctions, 2);
}
