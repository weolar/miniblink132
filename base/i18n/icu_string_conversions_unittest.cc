...
static const struct {
    ...
    { "big5", "\xA7\x41\xA6", OnStringConversionError::FAIL, false, L"", nullptr },
    { "big5", "\xA7\x41\xA6", OnStringConversionError::SKIP, true, L"\x4F60", nullptr },
    { "big5", "\xA7\x41\xA6", OnStringConversionError::SUBSTITUTE, true, L"\x4F60\xFFFD", nullptr },
    // Arabic (ISO-8859)
    { "iso-8859-6",
    ...
    { "gb2312", "\xC4\xE3\xBA\xC3", OnStringConversionError::FAIL, true, L"\x4F60\x597D", nullptr },
    // Chinese (GB18030) : 4 byte sequences mapped to BMP characters
    { "gb18030", "\x81\x30\x84\x36\xA1\xA7", OnStringConversionError::FAIL, true, L"\x00A5\x00A8", nullptr },
...
#endif
        L"\xD840\xDC00\x4E00" },
    { "big5", "\xA7\x41\xA6\x6E", OnStringConversionError::FAIL, true, L"\x4F60\x597D", nullptr },
    // Greek (ISO-8859)
    { "iso-8859-7",
        "\xE3\xE5\xE9\xDC"
        " "
        ...
        nullptr },
    // Hebrew (Windows)
    { "windows-1255", "\xF9\xD1\xC8\xEC\xE5\xC9\xED", OnStringConversionError::FAIL, true, L"\x05E9\x05C1\x05B8\x05DC\x05D5\x05B9\x05DD", nullptr },
    // Korean (EUC)
    { "euc-kr", "\xBE\xC8\xB3\xE7\xC7\xCF\xBC\xBC\xBF\xE4", OnStringConversionError::FAIL, true, L"\xC548\xB155\xD558\xC138\xC694", nullptr },
    // Japanese (EUC)
    { "euc-jp", "\xA4\xB3\xA4\xF3\xA4\xCB\xA4\xC1\xA4\xCF\xB0\xEC\x8E\xA6", OnStringConversionError::FAIL, true, L"\x3053\x3093\x306B\x3061\x306F\x4E00\xFF66",
        nullptr },
    // Japanese (ISO-2022)
    { "iso-2022-jp",
        "\x1B$B"
        ...
        nullptr },
    // Japanese (Shift-JIS)
    { "sjis", "\x82\xB1\x82\xF1\x82\xC9\x82\xBF\x82\xCD\x88\xEA\xA6", OnStringConversionError::FAIL, true, L"\x3053\x3093\x306B\x3061\x306F\x4E00\xFF66",
        nullptr },
    // Russian (KOI8)
    { "koi8-r", "\xDA\xC4\xD2\xC1\xD7\xD3\xD4\xD7\xD5\xCA\xD4\xC5", OnStringConversionError::FAIL, true,
...
TEST(ICUStringConversionsTest, ConvertBetweenCodepageAndUTF16)
{
    for (size_t i = 0; i < std::size(kConvertCodepageCases); ++i) {
        SCOPED_TRACE(
            base::StringPrintf("Test[%" PRIuS "]: <encoded: %s> <codepage: %s>", i, kConvertCodepageCases[i].encoded, kConvertCodepageCases[i].codepage_name));

        std::u16string utf16;
        bool success = CodepageToUTF16(kConvertCodepageCases[i].encoded, kConvertCodepageCases[i].codepage_name, kConvertCodepageCases[i].on_error, &utf16);
        ...
        if (success && kConvertCodepageCases[i].on_error == OnStringConversionError::FAIL) {
            std::string encoded;
            success = UTF16ToCodepage(utf16, kConvertCodepageCases[i].codepage_name, kConvertCodepageCases[i].on_error, &encoded);
            EXPECT_EQ(kConvertCodepageCases[i].success, success);
...
TEST(ICUStringConversionsTest, ConvertToUtf8AndNormalize)
{
    ...
    for (size_t i = 0; i < std::size(kConvertAndNormalizeCases); ++i) {
        SCOPED_TRACE(base::StringPrintf(
            "Test[%" PRIuS "]: <encoded: %s> <codepage: %s>", i, kConvertAndNormalizeCases[i].encoded, kConvertAndNormalizeCases[i].codepage_name));

        bool success = ConvertToUtf8AndNormalize(kConvertAndNormalizeCases[i].encoded, kConvertAndNormalizeCases[i].codepage_name, &result);
