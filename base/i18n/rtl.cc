...
std::string ICULocaleName(const std::string& locale_string)
{
    ...
    if (EqualsCaseInsensitiveASCII(locale_string, "es-419")) {
        ...
        const char* country = locale.getCountry();
        if (EqualsCaseInsensitiveASCII(language, "es") && !EqualsCaseInsensitiveASCII(country, "es")) {
            language += '-';
            language += country;
            return language;
        }
...
{
#if BUILDFLAG(IS_IOS)
    static base::debug::CrashKeyString* crash_key_locale = base::debug::AllocateCrashKeyString("icu_locale_input", base::debug::CrashKeySize::Size256);
    ...
    if (lang != nullptr && *lang != '\0') {
        ...
        LOG(ERROR) << "Failed to set the ICU default locale to " << locale_string << ". Falling back to en-US.";
...
TextDirection GetTextDirectionForLocaleInStartUp(const char* locale_name)
{
    ...
    static constexpr auto kRtlLanguageCodes = base::MakeFixedFlatSet<std::string_view>({ "ar", "fa", "he", "iw", "ur" });
    std::vector<std::string_view> locale_split = SplitStringPiece(locale_name, "-_", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    std::string_view language_code = locale_split[0];
    return kRtlLanguageCodes.contains(language_code) ? RIGHT_TO_LEFT : LEFT_TO_RIGHT;
}
...
void EnsureTerminatedDirectionalFormatting(std::u16string* text)
{
    int count = 0;
    for (auto c : *text) {
        if (c == kLeftToRightEmbeddingMark || c == kRightToLeftEmbeddingMark || c == kLeftToRightOverride || c == kRightToLeftOverride) {
            ++count;
...
void WrapPathWithLTRFormatting(const FilePath& path, std::u16string* rtl_safe_path)
{
