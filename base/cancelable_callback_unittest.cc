...
{
    int count = 0;
    CancelableRepeatingClosure cancelable(BindRepeating(&Increment, Unretained(&count)));
...
TEST(CancelableCallbackTest, PostTask)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, cancelable.callback());
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, cancelable.callback().Then(BindRepeating(&IncrementBy, Unretained(&count), 2)));
...
TEST(CancelableCallbackTest, PostTaskAndReply)
{
    ...
    int count = 0;
    CancelableRepeatingClosure cancelable_reply(BindRepeating(&Increment, Unretained(&count)));

    std::optional<RunLoop> loop;
    loop.emplace();
    SingleThreadTaskRunner::GetCurrentDefault()->PostTaskAndReply(FROM_HERE, DoNothing(), cancelable_reply.callback().Then(loop->QuitClosure()));
    loop->Run();
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTaskAndReply(FROM_HERE, DoNothing(), cancelable_reply.callback());
...
TEST(CancelableCallbackTest, MoveOnlyType)
{
    ...
    int result = 0;
    CancelableRepeatingCallback<void(std::unique_ptr<int>)> cb(BindRepeating(&OnMoveOnlyReceived, Unretained(&result)));
