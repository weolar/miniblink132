...
X86ModelInfo ComputeX86FamilyAndModel(const std::string& vendor, int signature)
{
    ...
    if (results.family == 0xf || (results.family == 0x6 && vendor == "GenuineIntel")) {
...
void __cpuidex(int cpu_info[4], int eax, int ecx)
{
    ...
    UNSAFE_BUFFERS(__asm__ volatile("mov %%ebx, %%edi\n"
                                    "cpuid\n"
                                    "xchg %%edi, %%ebx\n"
...
{
    // SAFETY: `cpu_info` has length 4 and therefore all accesses below are valid.
    UNSAFE_BUFFERS(__asm__ volatile("cpuid\n" : "=a"(cpu_info[0]), "=b"(cpu_info[1]), "=c"(cpu_info[2]), "=d"(cpu_info[3]) : "a"(eax), "c"(ecx)));
}
...
void CPU::Initialize()
{
    ...
    if (num_ids > 0) {
        ...
        internal::X86ModelInfo results = internal::ComputeX86FamilyAndModel(cpu_vendor_, signature_);
        ...
        has_avx_ = (cpu_info[2] & 0x10000000) != 0 && (cpu_info[2] & 0x04000000) != 0 /* XSAVE */ && (cpu_info[2] & 0x08000000) != 0 /* OSXSAVE */
    ...
    static constexpr uint32_t kParameterSize = kParameterEnd - kParameterStart + 1;
    static_assert(kParameterSize * sizeof(cpu_info) == kBrandNameSize, "cpu_brand_ has wrong size");

    if (max_parameter >= kParameterEnd) {
        SpanWriter writer { span(cpu_brand_) };
        for (uint32_t parameter = kParameterStart; parameter <= kParameterEnd; ++parameter) {
    ...
    static constexpr uint32_t kParameterContainingNonStopTimeStampCounter = 0x80000007;
    if (max_parameter >= kParameterContainingNonStopTimeStampCounter) {
        __cpuid(cpu_info, static_cast<int>(kParameterContainingNonStopTimeStampCounter));
...
#if defined(ARCH_CPU_ARM64) && (BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
