...
TEST_F(SampleVectorTest, ExtractingIterator_InvalidSingleSample)
{
    ...
    *samples.SingleSampleForTesting() = invalid_single_sample;
    ...
    HistogramSamples::SingleSample current_single_sample = samples.SingleSampleForTesting()->Load();
...
TEST_F(SampleVectorTest, PersistentSampleVector)
{
    ...
    const size_t counts_bytes = sizeof(HistogramBase::AtomicCount) * ranges.bucket_count();
    const DelayedPersistentAllocation allocation(&allocator, &samples_ref, 1, counts_bytes, false);
