...
public:
    ...
    void PrepareDeltas(const std::vector<HistogramBase*>& histograms, HistogramBase::Flags flags_to_set, HistogramBase::Flags required_flags);
    ...
    void SnapshotUnloggedSamples(const std::vector<HistogramBase*>& histograms, HistogramBase::Flags required_flags);
    ...
    void ResetFlattener()
    {
        histogram_flattener_ = nullptr;
    }

private:
    FRIEND_TEST_ALL_PREFIXES(HistogramSnapshotManagerTest, CheckMerge);

    using HistogramSnapshotPair = std::pair<HistogramBase*, std::unique_ptr<HistogramSamples>>;

    // Capture and hold samples from a histogram. This does all the heavy
    // lifting for PrepareDelta() and PrepareFinalDelta().
    void PrepareSamples(const HistogramBase* histogram, const HistogramSamples& samples);
