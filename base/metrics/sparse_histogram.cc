...
// static
HistogramBase* SparseHistogram::FactoryGet(std::string_view name, int32_t flags)
{
    HistogramBase* histogram = StatisticsRecorder::FindHistogram(name);
    if (!histogram) {
        bool should_record = StatisticsRecorder::ShouldRecordHistogram(HashMetricNameAs32Bits(name));
        if (!should_record) {
        ...
        if (allocator) {
            tentative_histogram = allocator->AllocateHistogram(SPARSE_HISTOGRAM, name, 0, 0, nullptr, flags, &histogram_ref);
        }
    ...
    if (histogram->GetHistogramType() != SPARSE_HISTOGRAM) {
        ...
        UmaHistogramSparse("Histogram.MismatchedConstructionArguments", static_cast<Sample>(HashMetricName(name)));
...
std::unique_ptr<HistogramBase> SparseHistogram::PersistentCreate(
    PersistentHistogramAllocator* allocator, const char* name, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
{
...
bool SparseHistogram::HasConstructionArguments(Sample expected_minimum, Sample expected_maximum, size_t expected_bucket_count) const
{
...
std::unique_ptr<HistogramSamples> SparseHistogram::SnapshotUnloggedSamples() const
{
    std::unique_ptr<SampleMap> snapshot(new SampleMap(name_hash()));
...
std::unique_ptr<HistogramSamples> SparseHistogram::SnapshotDelta()
{
    ...
    std::unique_ptr<SampleMap> snapshot = std::make_unique<SampleMap>(name_hash());
    base::AutoLock auto_lock(lock_);
