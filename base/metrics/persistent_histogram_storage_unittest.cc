...
public:
    PersistentHistogramStorageTest(const PersistentHistogramStorageTest&) = delete;
    PersistentHistogramStorageTest& operator=(const PersistentHistogramStorageTest&) = delete;
...
protected:
    ...
    void SetUp() override
    {
        ...
        test_storage_dir_ = temp_dir_path().AppendASCII(kTestHistogramAllocatorName);
    }
    ...
    const FilePath& temp_dir_path()
    {
        return temp_dir_.GetPath();
    }

    const FilePath& test_storage_dir()
    {
        return test_storage_dir_;
    }
...
TEST_F(PersistentHistogramStorageTest, HistogramWriteTest)
{
    auto persistent_histogram_storage
        = std::make_unique<PersistentHistogramStorage>(kTestHistogramAllocatorName, PersistentHistogramStorage::StorageDirManagement::kCreate);
...
TEST_F(PersistentHistogramStorageTest, TimeCreationTest)
{
    ...
    FileEnumerator enumerator(test_storage_dir(), /*recursive=*/false, FileEnumerator::FILES,
        FilePath(FILE_PATH_LITERAL("*")).AddExtension(PersistentMemoryAllocator::kFileExtension).value());
    int num_files = 0;
