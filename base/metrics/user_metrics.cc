...
void RecordComputedActionSince(const std::string& action, TimeDelta time_since)
{
...
void RecordComputedActionAt(const std::string& action, TimeTicks action_time)
{
    ...
    if (!g_task_runner.Get()->BelongsToCurrentThread()) {
        g_task_runner.Get()->PostTask(FROM_HERE, BindOnce(&RecordComputedActionAt, action, action_time));
        return;
    }
...
void SetRecordActionTaskRunner(scoped_refptr<SingleThreadTaskRunner> task_runner)
{
