...
struct BASE_EXPORT HistogramSharedMemory {
    ...
    struct BASE_EXPORT SharedMemory {
        ...
        SharedMemory(UnsafeSharedMemoryRegion, std::unique_ptr<PersistentMemoryAllocator>);
        ~SharedMemory();
    ...
    static std::optional<SharedMemory> Create(int process_id, const Config& config);
    ...
    // zygote.
    static void AddToLaunchParameters(UnsafeSharedMemoryRegion unsafe_memory_region,
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE)
