...
TEST_F(HistogramBaseTest, DeserializeHistogram)
{
    HistogramBase* histogram
        = Histogram::FactoryGet("TestHistogram", 1, 1000, 10, (HistogramBase::kUmaTargetedHistogramFlag | HistogramBase::kIPCSerializationSourceFlag));
...
{
    HistogramBase* histogram = LinearHistogram::FactoryGet("TestHistogram", 1, 1000, 10, HistogramBase::kIPCSerializationSourceFlag);
...
{
    HistogramBase* histogram = BooleanHistogram::FactoryGet("TestHistogram", HistogramBase::kIPCSerializationSourceFlag);
...
TEST_F(HistogramBaseTest, DeserializeCustomHistogram)
{
    ...
    HistogramBase* histogram = CustomHistogram::FactoryGet("TestHistogram", ranges, HistogramBase::kIPCSerializationSourceFlag);
...
{
    HistogramBase* histogram = SparseHistogram::FactoryGet("TestHistogram", HistogramBase::kIPCSerializationSourceFlag);
...
{
    HistogramBase* histogram = LinearHistogram::FactoryGet("TestAddKiloHistogram", 1, 1000, 100, 0);
...
{
    HistogramBase* histogram = LinearHistogram::FactoryGet("TestAddKiBHistogram", 1, 1000, 100, 0);
...
{
    const HistogramBase::Sample sample_max = std::numeric_limits<HistogramBase::Sample>::max() / 2;
    HistogramBase* histogram = LinearHistogram::FactoryGet("TestAddTimeMillisecondsGranularity1", 1, sample_max, 100, 0);
    ...
    histogram = LinearHistogram::FactoryGet("TestAddTimeMillisecondsGranularity2", 1, sample_max, 100, 0);
...
TEST_F(HistogramBaseTest, AddTimeMicrosecondsGranularityOverflow)
{
    ...
    const HistogramBase::Sample sample_max = std::numeric_limits<HistogramBase::Sample>::max() / 2;
    HistogramBase* histogram = LinearHistogram::FactoryGet("TestAddTimeMicrosecondsGranularity1", 1, sample_max, 100, 0);
    int64_t large_positive = std::numeric_limits<int64_t>::max();
    // |add_count| is the number of large values that have been added to the
    // histogram. We consider a number to be 'large' if it cannot be represented
    // in a HistogramBase::Sample.
    ...
    // We now perform the analoguous operations, now with negative values with a
    // large absolute value.
    histogram = LinearHistogram::FactoryGet("TestAddTimeMicrosecondsGranularity2", 1, sample_max, 100, 0);
    int64_t large_negative = std::numeric_limits<int64_t>::min();
    add_count = 0;
    while (large_negative < std::numeric_limits<HistogramBase::Sample>::min()) {
