...
void SampleCountPickleIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count)
{
    DCHECK(!Done());
    *min = min_;
    *max = max_;
...
static_assert(sizeof(HistogramSamples::AtomicSingleSample) == sizeof(subtle::Atomic32), "AtomicSingleSample isn't 32 bits");

HistogramSamples::SingleSample HistogramSamples::AtomicSingleSample::Load() const
{
...
HistogramSamples::SingleSample HistogramSamples::AtomicSingleSample::Extract(AtomicSingleSample new_value)
{
    DCHECK(new_value.as_atomic != kDisabledSingleSample) << "Disabling an AtomicSingleSample should be done through "
                                                            "ExtractAndDisable().";
    ...
    while (true) {
        ...
        subtle::Atomic32 existing = subtle::Release_CompareAndSwap(&as_atomic, old_value.as_atomic, new_value.as_atomic);
...
HistogramSamples::SingleSample HistogramSamples::AtomicSingleSample::ExtractAndDisable()
{
    AtomicSingleSample old_value(subtle::NoBarrier_AtomicExchange(&as_atomic, kDisabledSingleSample));
...
bool HistogramSamples::AtomicSingleSample::Accumulate(size_t bucket, HistogramBase::Count count)
{
    if (count == 0)
        return true;
    ...
    if (count < -std::numeric_limits<uint16_t>::max() || count > std::numeric_limits<uint16_t>::max() || bucket > std::numeric_limits<uint16_t>::max()) {
        return false;
    }
    ...
    do {
        ...
        subtle::Atomic32 existing = subtle::Release_CompareAndSwap(&as_atomic, original, single_sample.as_atomic);
...
bool HistogramSamples::Extract(HistogramSamples& other)
{
    ...
    subtle::Atomic64 other_sum = subtle::NoBarrier_AtomicExchange(&other.meta_->sum, 0);
#else
    ...
    HistogramBase::AtomicCount other_redundant_count = subtle::NoBarrier_AtomicExchange(&other.meta_->redundant_count, 0);
...
bool HistogramSamples::AccumulateSingleSample(HistogramBase::Sample value, HistogramBase::Count count, size_t bucket)
{
...
void HistogramSamples::IncreaseSumAndCount(int64_t sum, HistogramBase::Count count)
{
#ifdef ARCH_CPU_64_BITS
...
void HistogramSamples::RecordNegativeSample(NegativeSampleReason reason, HistogramBase::Count increment)
{
...
base::Value::Dict HistogramSamples::ToGraphDict(std::string_view histogram_name, int32_t flags) const
{
...
std::string HistogramSamples::GetAsciiHeader(std::string_view histogram_name, int32_t flags) const
{
    std::string output;
    StrAppend(&output, { "Histogram: ", histogram_name, " recorded ", NumberToString(TotalCount()), " samples" });
    if (flags)
...
// static
void HistogramSamples::WriteAsciiBucketGraph(double x_count, int line_length, std::string* output)
{
...
void HistogramSamples::WriteAsciiBucketValue(HistogramBase::Count current, double scaled_sum, std::string* output) const
{
    StringAppendF(output, " (%d = %3.1f%%)", current, current / scaled_sum);
}

const std::string HistogramSamples::GetSimpleAsciiBucketRange(HistogramBase::Sample sample) const
{
...
SingleSampleIterator::SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count, size_t bucket_index, bool value_was_extracted)
    : min_(min)
    , max_(max)
...
void SingleSampleIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count)
{
    DCHECK(!Done());
    *min = min_;
    *max = max_;
