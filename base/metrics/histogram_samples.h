...
public:
    ...
    union BASE_EXPORT AtomicSingleSample {
        ...
        explicit AtomicSingleSample(subtle::Atomic32 rhs)
            : as_atomic(rhs)
        {
        }
    ...
    virtual void Accumulate(HistogramBase::Sample value, HistogramBase::Count count) = 0;
    ...
    base::Value::Dict ToGraphDict(std::string_view histogram_name, int32_t flags) const;
...
protected:
    ...
    // the success case.
    bool AccumulateSingleSample(HistogramBase::Sample value, HistogramBase::Count count, size_t bucket);
    ...
    void RecordNegativeSample(NegativeSampleReason reason, HistogramBase::Count increment);
    ...
    static void WriteAsciiBucketGraph(double x_count, int line_length, std::string* output);

    // Writes textual description of the bucket contents (relative to histogram).
    // Output is the count in the buckets, as well as the percentage.
    void WriteAsciiBucketValue(HistogramBase::Count current, double scaled_sum, std::string* output) const;
    ...
    virtual std::string GetAsciiHeader(std::string_view histogram_name, int32_t flags) const;

    // Returns a string description of what goes in a given bucket.
    const std::string GetSimpleAsciiBucketRange(HistogramBase::Sample sample) const;
...
public:
    ...
    // Requires: !Done();
    virtual void Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) = 0;
    static_assert(
        std::numeric_limits<HistogramBase::Sample>::max() < std::numeric_limits<int64_t>::max(), "Get() |max| must be able to hold Histogram::Sample max + 1");
...
public:
    SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count, size_t bucket_index, bool value_was_extracted);
