...
// static
void SingleSampleMetricsFactory::SetFactory(std::unique_ptr<SingleSampleMetricsFactory> factory)
{
    DCHECK(!g_factory);
...
std::unique_ptr<SingleSampleMetric> DefaultSingleSampleMetricsFactory::CreateCustomCountsMetric(
    const std::string& histogram_name, HistogramBase::Sample min, HistogramBase::Sample max, uint32_t bucket_count)
{
    return std::make_unique<DefaultSingleSampleMetric>(histogram_name, min, max, bucket_count, HistogramBase::kUmaTargetedHistogramFlag);
}

DefaultSingleSampleMetric::DefaultSingleSampleMetric(
    const std::string& histogram_name, HistogramBase::Sample min, HistogramBase::Sample max, uint32_t bucket_count, int32_t flags)
