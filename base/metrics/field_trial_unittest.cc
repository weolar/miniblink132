...
scoped_refptr<FieldTrial> CreateFieldTrial(
    const std::string& trial_name, int total_probability, const std::string& default_group_name, bool is_low_anonymity = false)
{
    MockEntropyProvider entropy_provider(0.9);
    return FieldTrialList::FactoryGetFieldTrial(trial_name, total_probability, default_group_name, entropy_provider, 0, is_low_anonymity);
}
...
public:
    ...
    void OnFieldTrialGroupFinalized(const FieldTrial& trial, const std::string& group) override
    {
        trial_name_ = trial.trial_name();
        group_name_ = group;
    }

    const std::string& trial_name() const LIFETIME_BOUND
    {
        return trial_name_;
    }
    const std::string& group_name() const LIFETIME_BOUND
    {
        return group_name_;
    }
...
public:
    ...
    explicit FieldTrialObserverAccessingGroup(scoped_refptr<FieldTrial> trial_to_access)
        : trial_to_access_(trial_to_access)
    {
        FieldTrialList::AddObserver(this);
    }
    FieldTrialObserverAccessingGroup(const FieldTrialObserverAccessingGroup&) = delete;
    FieldTrialObserverAccessingGroup& operator=(const FieldTrialObserverAccessingGroup&) = delete;
    ...
    void OnFieldTrialGroupFinalized(const base::FieldTrial& trial, const std::string& group) override
    {
...
// trials too.
class TestFieldTrialObserverIncludingLowAnonymity : public FieldTrialList::Observer {
public:
    TestFieldTrialObserverIncludingLowAnonymity()
    {
        FieldTrialListIncludingLowAnonymity::AddObserver(this);
    }
    TestFieldTrialObserverIncludingLowAnonymity(const TestFieldTrialObserverIncludingLowAnonymity&) = delete;
    TestFieldTrialObserverIncludingLowAnonymity& operator=(const TestFieldTrialObserverIncludingLowAnonymity&) = delete;
...
MATCHER(CompareActiveGroupToFieldTrial, "")
{
    ...
    return lhs.trial_name == rhs->trial_name() && lhs.group_name == rhs->group_name_internal();
}
...
TEST_F(FieldTrialTest, Registration)
{
    ...
    scoped_refptr<FieldTrial> trial1 = CreateFieldTrial(name1, 10, "default name 1 test");
    ...
    scoped_refptr<FieldTrial> trial2 = CreateFieldTrial(name2, 10, "default name 2 test");
...
TEST_F(FieldTrialTest, AbsoluteProbabilities)
{
    ...
    scoped_refptr<FieldTrial> trial = FieldTrialList::FactoryGetFieldTrial("trial name", 100, "Default", entropy_provider);
...
TEST_F(FieldTrialTest, SmallProbabilities_49)
{
    ...
    scoped_refptr<FieldTrial> trial = FieldTrialList::FactoryGetFieldTrial("trial name", 2, "Default", entropy_provider);
...
{
    MockEntropyProvider entropy_provider(0.49);
    scoped_refptr<FieldTrial> trial = FieldTrialList::FactoryGetFieldTrial("trial name", 10, "Default", entropy_provider);
...
TEST_F(FieldTrialTest, ActiveGroupsNotFinalized)
{
    ...
    scoped_refptr<FieldTrial> trial = CreateFieldTrial(kTrialName, 100, kDefaultGroupName);
    ...
    EXPECT_TRUE(kDefaultGroupName == active_group.group_name || kSecondaryGroupName == active_group.group_name);
...
TEST_F(FieldTrialTest, SaveAll)
{
    ...
    scoped_refptr<FieldTrial> trial = CreateFieldTrial("Some name", 10, "Default some name");
    ...
    EXPECT_EQ("Some name/Default some name/*trial2/Winner/*xxx/yyyy", save_string);
    save_string.clear();
    ...
    FieldTrialList::AllStatesToString(&save_string);
    EXPECT_EQ("Some name/Default some name/*trial2/Winner/*xxx/yyyy/zzz/default", save_string);
...
{
    scoped_refptr<FieldTrial> trial = CreateFieldTrial("Some name", 10, "Default");
    trial->AppendGroup("Winner", 10);
    trial->Activate();
    std::string save_string;
    FieldTrialList::AllStatesToString(&save_string);
...
TEST_F(FieldTrialTest, CreateTrialsFromStringForceActivation)
{
    ...
    ASSERT_FALSE(FieldTrialList::TrialExists("Xyz"));
    ASSERT_TRUE(FieldTrialList::CreateTrialsFromString("*Abc/cba/def/fed/*Xyz/zyx/"));
...
TEST_F(FieldTrialTest, ForcedFieldTrials)
{
    ...
    FieldTrial* forced_trial = FieldTrialList::CreateFieldTrial("Use the", "Force");
    EXPECT_STREQ("Force", forced_trial->group_name().c_str());

    scoped_refptr<FieldTrial> factory_trial = CreateFieldTrial("Use the", 1000, "default");
...
TEST_F(FieldTrialTest, ForcedFieldTrialsDefaultGroup)
{
    ...
    FieldTrial* forced_trial = FieldTrialList::CreateFieldTrial("Trial Name", "Default");
    scoped_refptr<FieldTrial> factory_trial = CreateFieldTrial("Trial Name", 1000, "Default");
...
TEST_F(FieldTrialTest, SetForced)
{
    ...
    scoped_refptr<FieldTrial> forced_trial = CreateFieldTrial("Use the", 1, "default");
    ...
    scoped_refptr<FieldTrial> hard_coded_trial = CreateFieldTrial("Use the", 1, "default");
    ...
    scoped_refptr<FieldTrial> other_hard_coded_trial = CreateFieldTrial("Use the", 1, "default");
...
TEST_F(FieldTrialTest, SetForcedTurnFeatureOn)
{
    ...
    scoped_refptr<FieldTrial> forced_trial = CreateFieldTrial(kTrialName, 100, kDefaultGroupName);
    forced_trial->AppendGroup(kExtraGroupName, 100);
    forced_trial->SetForced();

    scoped_refptr<FieldTrial> client_trial = CreateFieldTrial(kTrialName, 100, kDefaultGroupName);
...
TEST_F(FieldTrialTest, SetForcedChangeDefault_Default)
{
    ...
    scoped_refptr<FieldTrial> forced_trial = CreateFieldTrial(kTrialName, 100, kGroupAName);
    forced_trial->AppendGroup(kGroupBName, 100);
    forced_trial->SetForced();

    scoped_refptr<FieldTrial> client_trial = CreateFieldTrial(kTrialName, 100, kGroupBName);
...
TEST_F(FieldTrialTest, Observe)
{
    ...
    EXPECT_TRUE(chosen_group_name == kDefaultGroupName || chosen_group_name == kSecondaryGroupName);
...
TEST_F(FieldTrialTest, ObserveReentrancy)
{
    ...
    scoped_refptr<FieldTrial> trial_1 = CreateFieldTrial(kTrialName1, 100, kDefaultGroupName);

    FieldTrialObserverAccessingGroup observer(trial_1);

    scoped_refptr<FieldTrial> trial_2 = CreateFieldTrial(kTrialName2, 100, kDefaultGroupName);
...
TEST_F(FieldTrialTest, NotDisabled)
{
    ...
    scoped_refptr<FieldTrial> trial = CreateFieldTrial(kTrialName, kProbability, kDefaultGroupName);
...
TEST_F(FieldTrialTest, FloatBoundariesGiveEqualGroupSizes)
{
    ...
    for (int i = 0; i < kBucketCount; ++i) {
        ...
        scoped_refptr<FieldTrial> trial(new FieldTrial("test", kBucketCount, "default", entropy,
...
TEST_F(FieldTrialTest, DoesNotSurpassTotalProbability)
{
    ...
    scoped_refptr<FieldTrial> trial(new FieldTrial("test", 2, "default", kEntropyValue,
        /*is_low_anonymity=*/false, /*is_overridden=*/false));
...
TEST_F(FieldTrialTest, CreateSimulatedFieldTrial)
{
    ...
    for (auto& test_case : test_cases) {
        TestFieldTrialObserver observer;
        scoped_refptr<FieldTrial> trial(FieldTrial::CreateSimulatedFieldTrial(kTrialName, 100, kDefaultGroupName, test_case.entropy_value));
...
TEST(FieldTrialTestWithoutList, StatesStringFormat)
{
    ...
    // importing function.
    {
        ...
        scoped_refptr<FieldTrial> trial = CreateFieldTrial("Abc", 10, "Default some name");
        trial->AppendGroup("cba", 10);
        trial->Activate();
        scoped_refptr<FieldTrial> trial2 = CreateFieldTrial("Xyz", 10, "Default xxx");
...
TEST_F(FieldTrialListTest, DoNotAddSimulatedFieldTrialsToAllocator)
{
    ...
    base::ReadOnlySharedMemoryRegion shm_region;
    {
        ...
        scoped_refptr<FieldTrial> simulated_trial = FieldTrial::CreateSimulatedFieldTrial(kTrialName, 100, "Simulated", 0.95);
        simulated_trial->Activate();

        // This should add to the allocator.
        FieldTrial* real_trial = FieldTrialList::CreateFieldTrial(kTrialName, "Real");
        real_trial->Activate();
...
TEST_F(FieldTrialListTest, AssociateFieldTrialParams)
{
    ...
    params["key1"] = "value1";
    params["key2"] = "value2";
    FieldTrialParamAssociator::GetInstance()->AssociateFieldTrialParams(trial_name, group_name, params);
    FieldTrialList::InstantiateFieldTrialAllocatorIfNeeded();
    ...
    FieldTrialParamAssociator::GetInstance()->GetFieldTrialParamsWithoutFallback(trial_name, group_name, &cached_params);
...
TEST_F(FieldTrialListTest, ClearParamsFromSharedMemory)
{
    ...
    base::ReadOnlySharedMemoryRegion shm_region;
    {
        ...
        // Create a field trial with some params.
        FieldTrial* trial = FieldTrialList::CreateFieldTrial(trial_name, group_name);
        std::map<std::string, std::string> params;
        params["key1"] = "value1";
        params["key2"] = "value2";
        FieldTrialParamAssociator::GetInstance()->AssociateFieldTrialParams(trial_name, group_name, params);
...
TEST_F(FieldTrialListTest, DumpAndFetchFromSharedMemory)
{
    ...
    std::vector<const FieldTrial::FieldTrialEntry*> entries = FieldTrialList::GetAllFieldTrialsFromPersistentAllocator(allocator);
...
TEST_F(FieldTrialListTest, PassFieldTrialSharedMemoryOnCommandLine)
{
    ...
    feature_list->RegisterFieldTrialOverride(kTestFeatureC.name, FeatureList::OVERRIDE_DISABLE_FEATURE, trial);
    test::ScopedFeatureList scoped_feature_list2;
    ...
    EXPECT_EQ(kTestFeatureA.name, command_line.GetSwitchValueASCII(switches::kEnableFeatures));
    EXPECT_EQ(kTestFeatureB.name, command_line.GetSwitchValueASCII(switches::kDisableFeatures));

    // Run the child.
    Process process = SpawnMultiProcessTestChild("CreateTrialsInChildProcess", command_line, launch_options);
    int exit_code = -1;
    EXPECT_TRUE(WaitForMultiprocessTestChildExit(process, TestTimeouts::action_timeout(), &exit_code));
...
TEST_F(FieldTrialListTest, CheckReadOnlySharedMemoryRegion)
{
    ...
    base::ReadOnlySharedMemoryRegion region = FieldTrialList::DuplicateFieldTrialSharedMemoryForTesting();
    ASSERT_TRUE(region.IsValid());

    ASSERT_TRUE(CheckReadOnlyPlatformSharedMemoryRegionForTesting(base::ReadOnlySharedMemoryRegion::TakeHandleForSerialization(std::move(region))));
}
...
TEST_F(FieldTrialListTest, TestGetRandomizedFieldTrialCount)
{
    ...
    scoped_refptr<FieldTrial> trial1 = FieldTrialList::CreateFieldTrial(name1, group1);
    ...
    scoped_refptr<FieldTrial> trial3 = CreateFieldTrial(name3, 10, "default name 3 test");
...
TEST_F(FieldTrialTest, TestAllParamsToString)
{
    ...
    params["p1"] = "v1";
    params["p2"] = "v2";
    FieldTrialParamAssociator::GetInstance()->AssociateFieldTrialParams("t1", "g1", params);
    ...
    trial1->Activate();
    EXPECT_EQ(exptected_output, FieldTrialList::AllParamsToString(&MockEscapeQueryParamValue));

    // Create study with two groups and params that don't belog to the assigned
    // group. This should be in the output.
    FieldTrialParamAssociator::GetInstance()->AssociateFieldTrialParams("t2", "g2", params);
...
TEST_F(FieldTrialTest, GetActiveFieldTrialGroups_LowAnonymity)
{
    ...
    // low-anonymity.
    scoped_refptr<FieldTrial> trial_2 = CreateFieldTrial("Low anonymity", 10, "Default", /*is_low_anonymity=*/true);
    ...
    EXPECT_THAT(active_groups_for_metrics, testing::UnorderedPointwise(CompareActiveGroupToFieldTrial(), { trial_1 }));

    // Check that
    // |FieldTrialListIncludingLowAnonymity::GetActiveFieldTrialGroups()| includes
    // both trials.
    FieldTrial::ActiveGroups active_groups;
    FieldTrialListIncludingLowAnonymity::GetActiveFieldTrialGroupsForTesting(&active_groups);
    EXPECT_THAT(active_groups, testing::UnorderedPointwise(CompareActiveGroupToFieldTrial(), { trial_1, trial_2 }));
}
...
TEST_F(FieldTrialTest, ObserveIncludingLowAnonymity)
{
    ...
    const char kTrialName[] = "TrialToObserve1";
    scoped_refptr<FieldTrial> trial = CreateFieldTrial(kTrialName, 100, kDefaultGroupName, /*is_low_anonymity=*/true);
    trial->Activate();
...
TEST_F(FieldTrialTest, ParseFieldTrialsString)
{
    ...
    ASSERT_TRUE(FieldTrial::ParseFieldTrialsString("Trial1/Group1", /*override_trials=*/false, entries));
...
{
    std::vector<FieldTrial::State> entries;
    ASSERT_TRUE(FieldTrial::ParseFieldTrialsString("Trial1/Group1/*Trial2/Group2/", /*override_trials=*/false, entries));
...
{
    std::vector<FieldTrial::State> entries;
    ASSERT_TRUE(FieldTrial::ParseFieldTrialsString("", /*override_trials=*/false, entries));
...
{
    std::vector<FieldTrial::State> entries;
    EXPECT_FALSE(FieldTrial::ParseFieldTrialsString("A/", /*override_trials=*/false, entries));
    EXPECT_FALSE(FieldTrial::ParseFieldTrialsString("/A", /*override_trials=*/false, entries));
    EXPECT_FALSE(FieldTrial::ParseFieldTrialsString("//", /*override_trials=*/false, entries));
    EXPECT_FALSE(FieldTrial::ParseFieldTrialsString("///", /*override_trials=*/false, entries));
}
...
TEST_F(FieldTrialTest, BuildFieldTrialStateString)
{
    ...
    EXPECT_EQ("Trial/Group/*Foo/Bar", FieldTrial::BuildFieldTrialStateString({ state1, state2 }));
}
