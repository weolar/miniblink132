...
void SetFlag(volatile std::atomic<uint32_t>* flags, uint32_t flag)
{
    ...
    for (;;) {
        ...
        // of all other values.
        if (flags->compare_exchange_weak(loaded_flags, new_flags, std::memory_order_relaxed, std::memory_order_relaxed)) {
            break;
        }
    }
}
...
const PersistentMemoryAllocator::Reference PersistentMemoryAllocator::kReferenceQueue = offsetof(SharedMetadata, queue);
...
PersistentMemoryAllocator::Iterator::Iterator(const PersistentMemoryAllocator* allocator, Reference starting_after)
    : allocator_(allocator)
    , last_record_(0)
    , record_count_(0)
{
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Iterator::GetLast()
{
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Iterator::GetNext(uint32_t* type_return, size_t* alloc_size)
{
    ...
    while (true) {
        ...
        if (last_record_.compare_exchange_strong(last, next, std::memory_order_acq_rel, std::memory_order_acquire)) {
    ...
    const uint32_t freeptr = std::min(allocator_->shared_meta()->freeptr.load(std::memory_order_relaxed), allocator_->mem_size_);
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Iterator::GetNextOfType(uint32_t type_match, size_t* alloc_size)
{
    Reference ref;
    size_t size;
    uint32_t type_found;
...
// static
bool PersistentMemoryAllocator::IsMemoryAcceptable(const void* base, size_t size, size_t page_size, bool readonly)
{
    return ((base && reinterpret_cast<uintptr_t>(base) % kAllocAlignment == 0) && (size >= sizeof(SharedMetadata) && size <= kSegmentMaxSize)
        && (size % kAllocAlignment == 0 || readonly) && (page_size == 0 || size % page_size == 0 || readonly));
}

PersistentMemoryAllocator::PersistentMemoryAllocator(void* base, size_t size, size_t page_size, uint64_t id, std::string_view name, AccessMode access_mode)
    : PersistentMemoryAllocator(Memory(base, MEM_EXTERNAL), size, page_size, id, name, access_mode)
{
}

PersistentMemoryAllocator::PersistentMemoryAllocator(Memory memory, size_t size, size_t page_size, uint64_t id, std::string_view name, AccessMode access_mode)
    ...
    access_mode_(access_mode)
{
    ...
    static_assert(sizeof(PersistentMemoryAllocator::BlockHeader) == 16, "struct is not portable across different natural word widths");
    static_assert(sizeof(PersistentMemoryAllocator::SharedMetadata) == 64, "struct is not portable across different natural word widths");

    static_assert(sizeof(BlockHeader) % kAllocAlignment == 0, "BlockHeader is not a multiple of kAllocAlignment");
    static_assert(sizeof(SharedMetadata) % kAllocAlignment == 0, "SharedMetadata is not a multiple of kAllocAlignment");
    static_assert(kReferenceQueue % kAllocAlignment == 0, "\"queue\" is not aligned properly; must be at end of struct");
    ...
    if (shared_meta()->cookie != kGlobalCookie) {
        ...
        if (shared_meta()->cookie != 0 || shared_meta()->size != 0 || shared_meta()->version != 0 || shared_meta()->freeptr.load(std::memory_order_relaxed) != 0
            || shared_meta()->flags.load(std::memory_order_relaxed) != 0 || shared_meta()->id != 0 || shared_meta()->name != 0 || shared_meta()->tailptr != 0
            || shared_meta()->queue.cookie != 0 || shared_meta()->queue.next.load(std::memory_order_relaxed) != 0 || first_block->size != 0
            || first_block->cookie != 0 || first_block->type_id.load(std::memory_order_relaxed) != 0 || first_block->next != 0) {
        ...
        if (shared_meta()->size == 0 || (shared_meta()->version != kGlobalVersion && !Contains(kOldCompatibleVersions, shared_meta()->version))
            || shared_meta()->freeptr.load(std::memory_order_relaxed) == 0 || shared_meta()->tailptr == 0 || shared_meta()->queue.cookie == 0
...
const char* PersistentMemoryAllocator::Name() const
{
    ...
    const char* name_cstr = GetAsArray<char>(name_ref, 0, PersistentMemoryAllocator::kSizeAny, &name_length);
    if (!name_cstr)
        return "";
...
void PersistentMemoryAllocator::CreateTrackingHistograms(std::string_view name)
{
    ...
    used_histogram_ = LinearHistogram::FactoryGet("UMA.PersistentAllocator." + name_string + ".UsedPct", 1, 101, 21, HistogramBase::kUmaTargetedHistogramFlag);
}
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::GetAsReference(const void* memory, uint32_t type_id) const
{
...
bool PersistentMemoryAllocator::ChangeType(Reference ref, uint32_t to_type_id, uint32_t from_type_id, bool clear)
{
    ...
    if (clear) {
        ...
        if (!block->type_id.compare_exchange_strong(from_type_id, kTypeIdTransitioning, std::memory_order_acquire, std::memory_order_acquire)) {
        ...
        volatile std::atomic<int>* data = reinterpret_cast<volatile std::atomic<int>*>(reinterpret_cast<volatile char*>(block) + sizeof(BlockHeader));
    ...
    return block->type_id.compare_exchange_strong(from_type_id, to_type_id, std::memory_order_acq_rel, std::memory_order_acquire);
}

PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Allocate(size_t req_size, uint32_t type_id, size_t* alloc_size)
{
    return AllocateImpl(req_size, type_id, alloc_size);
}

PersistentMemoryAllocator::Reference PersistentMemoryAllocator::AllocateImpl(size_t req_size, uint32_t type_id, size_t* alloc_size)
{
    DCHECK_NE(access_mode_, kReadOnly);
    ...
    for (;;) {
        ...
        if (size > page_free) {
            ...
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "mem_size_", mem_size_);
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "mem_page_", mem_page_);
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "freeptr", freeptr);
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "page_free", page_free);
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "size", size);
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "req_size", req_size);
            ...
            if (allocator && allocator->HasPersistentLocation()) {
                persistent_file_name = allocator->GetPersistentLocation().BaseName().AsUTF8Unsafe();
            }
            SCOPED_CRASH_KEY_STRING256("PersistentMemoryAllocator", "file_name", persistent_file_name);
            debug::DumpWithoutCrashing();
#endif // !BUILDFLAG(IS_NACL)

            const uint32_t new_freeptr = freeptr + page_free;
            if (shared_meta()->freeptr.compare_exchange_strong(freeptr, new_freeptr, std::memory_order_acq_rel, std::memory_order_acquire)) {
        ...
        if (block->size != 0 || block->cookie != kBlockCookieFree || block->type_id.load(std::memory_order_relaxed) != 0
        ...
        volatile char* mem_begin = reinterpret_cast<volatile char*>(
            (reinterpret_cast<uintptr_t>(block) + sizeof(BlockHeader) + (vm_page_size_ - 1)) & ~static_cast<uintptr_t>(vm_page_size_ - 1));
...
{
    uint32_t remaining = std::max(mem_size_ - shared_meta()->freeptr.load(std::memory_order_relaxed), (uint32_t)sizeof(BlockHeader));
...
void PersistentMemoryAllocator::MakeIterable(Reference ref)
{
    ...
    for (;;) {
        ...
        if (block->next.compare_exchange_strong(next, ref, std::memory_order_acq_rel, std::memory_order_acquire)) {
            ...
            shared_meta()->tailptr.compare_exchange_strong(tail, ref, std::memory_order_release, std::memory_order_relaxed);
            return;
        }
        ...
        shared_meta()->tailptr.compare_exchange_strong(tail, next, std::memory_order_acq_rel, std::memory_order_acquire);
    }
}
...
{
    if (!corrupt_.load(std::memory_order_relaxed) && !CheckFlag(const_cast<volatile std::atomic<uint32_t>*>(&shared_meta()->flags), kFlagCorrupt)) {
...
const volatile PersistentMemoryAllocator::BlockHeader* PersistentMemoryAllocator::GetBlock(
    Reference ref, uint32_t type_id, size_t size, bool queue_ok, bool free_ok, size_t* alloc_size) const
{
...
const volatile void* PersistentMemoryAllocator::GetBlockData(Reference ref, uint32_t type_id, size_t size, size_t* alloc_size) const
{
    DCHECK(size > 0);
    const volatile BlockHeader* block = GetBlock(ref, type_id, size, false, false, alloc_size);
    if (!block)
        return nullptr;
...
void PersistentMemoryAllocator::UpdateTrackingHistograms()
{
    ...
    if (used_histogram_) {
        ...
        HistogramBase::Sample used_percent = static_cast<HistogramBase::Sample>(((meminfo.total - meminfo.free) * 100ULL / meminfo.total));
...
LocalPersistentMemoryAllocator::LocalPersistentMemoryAllocator(size_t size, uint64_t id, std::string_view name)
    : PersistentMemoryAllocator(AllocateLocalMemory(size, name), size, 0, id, name, kReadWrite)
{
}
...
// static
PersistentMemoryAllocator::Memory LocalPersistentMemoryAllocator::AllocateLocalMemory(size_t size, std::string_view name)
{
    void* address;
...
// static
void LocalPersistentMemoryAllocator::DeallocateLocalMemory(void* memory, size_t size, MemoryType type)
{
    if (type == MEM_MALLOC) {
        free(memory);
        return;
...
WritableSharedPersistentMemoryAllocator::WritableSharedPersistentMemoryAllocator(base::WritableSharedMemoryMapping memory, uint64_t id, std::string_view name)
    : PersistentMemoryAllocator(Memory(memory.memory(), MEM_SHARED), memory.size(), 0, id, name, kReadWrite)
...
// static
bool WritableSharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(const base::WritableSharedMemoryMapping& memory)
{
...
ReadOnlySharedPersistentMemoryAllocator::ReadOnlySharedPersistentMemoryAllocator(base::ReadOnlySharedMemoryMapping memory, uint64_t id, std::string_view name)
    : PersistentMemoryAllocator(Memory(const_cast<void*>(memory.memory()), MEM_SHARED), memory.size(), 0, id, name, kReadOnly)
    , shared_memory_(std::move(memory))
{
}
...
// static
bool ReadOnlySharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(const base::ReadOnlySharedMemoryMapping& memory)
{
...
FilePersistentMemoryAllocator::FilePersistentMemoryAllocator(
    std::unique_ptr<MemoryMappedFile> file, size_t max_size, uint64_t id, std::string_view name, AccessMode access_mode)
    : PersistentMemoryAllocator(Memory(const_cast<uint8_t*>(file->data()), MEM_FILE), max_size != 0 ? max_size : file->length(), 0, id, name, access_mode)
...
// static
bool FilePersistentMemoryAllocator::IsFileAcceptable(const MemoryMappedFile& file, bool readonly)
{
...
void FilePersistentMemoryAllocator::FlushPartial(size_t length, bool sync)
{
    ...
    int result = ::msync(const_cast<void*>(data()), length, MS_INVALIDATE | (sync ? MS_SYNC : MS_ASYNC));
    DCHECK_NE(EINVAL, result);
#else
...
DelayedPersistentAllocation::DelayedPersistentAllocation(
    PersistentMemoryAllocator* allocator, std::atomic<Reference>* ref, uint32_t type, size_t size, size_t offset)
    : allocator_(allocator)
    , type_(type)
...
span<uint8_t> DelayedPersistentAllocation::GetUntyped() const
{
    ...
    if (!ref) {
        ...
        if (!reference_->compare_exchange_strong(existing, ref, std::memory_order_release, std::memory_order_relaxed)) {
    ...
    if (!mem) {
        ...
        // TODO(crbug.com/40064026): Remove these. They are used to investigate
        // unexpected failures.
        SCOPED_CRASH_KEY_BOOL("PersistentMemoryAllocator", "full", allocator_->IsFull());
        SCOPED_CRASH_KEY_BOOL("PersistentMemoryAllocator", "corrupted", allocator_->IsCorrupt());
        SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "freeptr", allocator_->freeptr());
        ...
        SCOPED_CRASH_KEY_NUMBER(
            "PersistentMemoryAllocator", "cookie", static_cast<volatile PersistentMemoryAllocator::SharedMetadata*>(allocator_->shared_meta())->cookie);
        ...
        if (ref == 0xC8799269) {
            ...
            // went terribly wrong...
            SCOPED_CRASH_KEY_NUMBER("PersistentMemoryAllocator", "ref_before", (reference_ - 1)->load(std::memory_order_relaxed));
