...
TEST(RangesManagerTest, GetOrRegisterCanonicalRanges)
{
    ...
    EXPECT_THAT(ranges_manager.GetBucketRanges(), UnorderedElementsAre(ranges1, ranges2));
...
{
    std::unique_ptr<RangesManager> ranges_manager = std::make_unique<RangesManager>();
