...
SeedSimulationResult ComputeDifferences(
    const std::vector<ProcessedStudy>& processed_studies, const VariationsLayers& layers, const EntropyProviders& entropy_providers);
} // namespace variations
...
public:
    ...
    static void GetActiveFieldTrialGroupsForTesting(FieldTrial::ActiveGroups* active_groups)
    {
    ...
    friend variations::SeedSimulationResult variations::ComputeDifferences(const std::vector<variations::ProcessedStudy>& processed_studies,
        const variations::VariationsLayers& layers, const variations::EntropyProviders& entropy_providers);
