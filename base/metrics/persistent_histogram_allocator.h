...
public:
    ...
    PersistentSparseHistogramDataManager(const PersistentSparseHistogramDataManager&) = delete;
    PersistentSparseHistogramDataManager& operator=(const PersistentSparseHistogramDataManager&) = delete;
    ...
    std::unique_ptr<PersistentSampleMapRecords> CreateSampleMapRecords(uint64_t id);
    ...
    template <typename T> T* GetAsObject(PersistentMemoryAllocator::Reference ref)
    {
...
private:
    ...
    std::vector<ReferenceAndSample>* GetSampleMapRecordsWhileLocked(uint64_t id) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::map<uint64_t, std::unique_ptr<std::vector<ReferenceAndSample>>> sample_records_ GUARDED_BY(lock_);
...
public:
    ...
    PersistentSampleMapRecords(PersistentSparseHistogramDataManager* data_manager, uint64_t sample_map_id,
        std::vector<PersistentSparseHistogramDataManager::ReferenceAndSample>* records);

    PersistentSampleMapRecords(const PersistentSampleMapRecords&) = delete;
    PersistentSampleMapRecords& operator=(const PersistentSampleMapRecords&) = delete;
    ...
    // records.
    std::vector<PersistentMemoryAllocator::Reference> GetNextRecords(std::optional<HistogramBase::Sample> until_value);
...
private:
    ...
    raw_ptr<std::vector<PersistentSparseHistogramDataManager::ReferenceAndSample>> records_;
};
...
public:
    ...
    public:
        ...
        std::unique_ptr<HistogramBase> GetNext()
        {
    ...
    PersistentHistogramAllocator& operator=(const PersistentHistogramAllocator&) = delete;
    ...
    bool MergeHistogramFinalDeltaToStatisticsRecorder(const HistogramBase* histogram);
...
protected:
    ...
    std::unique_ptr<HistogramBase> GetNextHistogramWithIgnore(Iterator* iter, Reference ignore);

private:
    // Create a histogram based on saved (persistent) information about it.
    std::unique_ptr<HistogramBase> CreateHistogram(PersistentHistogramData* histogram_data_ptr);
    ...
    HistogramBase* GetOrCreateStatisticsRecorderHistogram(const HistogramBase* histogram);
...
class BASE_EXPORT GlobalHistogramAllocator : public PersistentHistogramAllocator {
public:
    ...
    static void CreateWithPersistentMemory(void* base, size_t size, size_t page_size, uint64_t id, std::string_view name);

    // Create a global allocator using an internal block of memory of the
    // specified `size` taken from the heap.
    static void CreateWithLocalMemory(size_t size, uint64_t id, std::string_view name);
    ...
    static bool CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, std::string_view name, bool exclusive_write = false);
    ...
    static bool CreateWithActiveFile(
    ...
    // `dir`.
    static bool CreateWithActiveFileInDir(const FilePath& dir, size_t size, uint64_t id, std::string_view name);
    ...
    static FilePath ConstructFilePathForActiveFile(const FilePath& dir, std::string_view name);

    // Like above but with timestamp and pid for use in upload directories.
    static FilePath ConstructFilePathForUploadDir(const FilePath& dir, std::string_view name, base::Time stamp, ProcessId pid);

    // Override that uses the current time stamp and current process id.
    static FilePath ConstructFilePathForUploadDir(const FilePath& dir, std::string_view name);

    // Parses a filename to extract name, timestamp, and pid.
    static bool ParseFilePath(const FilePath& path, std::string* out_name, Time* out_stamp, ProcessId* out_pid);
    ...
    static void CreateWithSharedMemoryRegion(const UnsafeSharedMemoryRegion& region);
...
private:
    ...
    static FilePath MakeMetricsFilePath(const FilePath& dir, std::string_view name);
