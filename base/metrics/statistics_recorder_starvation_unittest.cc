...
public:
    explicit BaseThread(const std::string& thread_name)
        : SimpleThread(thread_name, Options())
    ...
    TimeDelta running_time()
    {
        return end_time_ - start_time_;
    }
    TimeDelta average_time_per_iter()
    {
        return running_time() / iter_count_;
    }
...
public:
    ...
    void Run() override
    {
        ...
        const std::string histogram_name = StrCat({ kHistogramNamePrefix, "ReadThreadHistogram" });
        while (!ShouldStop()) {
...
public:
    ...
    void Run() override
    {
        ...
        const std::string base_name = StrCat({ kHistogramNamePrefix, thread_name(), ".Iteration" });
        while (!ShouldStop()) {
            ...
            UmaHistogramBoolean(StrCat({ base_name, NumberToString(iter_count()) }), false);
            IncrementIterCount();
        }
    }
};
...
class StatisticsRecorderStarvationTest : public testing::TestWithParam<std::tuple<StarvationTestParams, FirstThreadsToStart>> {
public:
    StatisticsRecorderStarvationTest() = default;

    StatisticsRecorderStarvationTest(const StatisticsRecorderStarvationTest&) = delete;
    StatisticsRecorderStarvationTest& operator=(const StatisticsRecorderStarvationTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        for (size_t i = 0; i < 10000; ++i) {
            UmaHistogramBoolean(StrCat({ kHistogramNamePrefix, "Dummy", NumberToString(i) }), false);
        }
    }
    ...
    size_t num_read_threads()
    {
        return std::get<0>(GetParam()).num_read_threads;
    }
...
TEST_P(StatisticsRecorderStarvationTest, StatisticsRecorderNoStarvation)
{
    ...
    LOG(INFO) << "Params: num_read_threads=" << num_read_threads() << ", num_write_threads=" << num_write_threads()
              << ", FirstThreadsToStart=" << static_cast<int>(GetFirstThreadsToStart());
    for (auto* thread : threads) {
        LOG(INFO) << thread->thread_name() << " iter_count=" << thread->iter_count() << ", running_time=" << thread->running_time()
                  << ", average_time_per_iter=" << thread->average_time_per_iter();
    }
}

INSTANTIATE_TEST_SUITE_P(All, StatisticsRecorderStarvationTest,
    testing::Combine(
        testing::Values(StarvationTestParams { .num_read_threads = 10, .num_write_threads = 1 },
            StarvationTestParams { .num_read_threads = 1, .num_write_threads = 10 }, StarvationTestParams { .num_read_threads = 1, .num_write_threads = 1 },
            StarvationTestParams { .num_read_threads = 5, .num_write_threads = 5 }),
        testing::Values(FirstThreadsToStart::kReaders, FirstThreadsToStart::kWriters)));
