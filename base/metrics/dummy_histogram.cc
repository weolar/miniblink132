...
bool DummyHistogram::HasConstructionArguments(Sample expected_minimum, Sample expected_maximum, size_t expected_bucket_count) const
{
    return true;
}
...
std::unique_ptr<HistogramSamples> DummyHistogram::SnapshotUnloggedSamples() const
{
    return std::make_unique<DummyHistogramSamples>();
}
