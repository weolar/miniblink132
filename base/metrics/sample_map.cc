...
typedef IteratorTemplate<const SampleToCountMap, SampleToCountMap::const_iterator> SampleMapIterator;

template <> SampleMapIterator::~IteratorTemplate() = default;

// Get() for an iterator of a SampleMap.
template <> void SampleMapIterator::Get(Sample* min, int64_t* max, Count* count)
{
    DCHECK(!Done());
    *min = iter_->first;
...
typedef IteratorTemplate<SampleToCountMap, SampleToCountMap::iterator> ExtractingSampleMapIterator;

template <> ExtractingSampleMapIterator::~IteratorTemplate()
{
...
template <> void ExtractingSampleMapIterator::Get(Sample* min, int64_t* max, Count* count)
{
    DCHECK(!Done());
    *min = iter_->first;
    *max = strict_cast<int64_t>(iter_->first) + 1;
