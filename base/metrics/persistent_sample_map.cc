...
template <typename T, typename I> class IteratorTemplate : public SampleCountIterator {
public:
...
typedef std::map<HistogramBase::Sample, raw_ptr<HistogramBase::Count, CtnExperimental>> SampleToCountMap;
typedef IteratorTemplate<const SampleToCountMap, SampleToCountMap::const_iterator> PersistentSampleMapIterator;

template <> PersistentSampleMapIterator::~IteratorTemplate() = default;

// Get() for an iterator of a PersistentSampleMap.
template <> void PersistentSampleMapIterator::Get(Sample* min, int64_t* max, Count* count)
{
    DCHECK(!Done());
    *min = iter_->first;
...
typedef IteratorTemplate<SampleToCountMap, SampleToCountMap::iterator> ExtractingPersistentSampleMapIterator;

template <> ExtractingPersistentSampleMapIterator::~IteratorTemplate()
{
...
template <> void ExtractingPersistentSampleMapIterator::Get(Sample* min, int64_t* max, Count* count)
{
    DCHECK(!Done());
    *min = iter_->first;
    *max = strict_cast<int64_t>(iter_->first) + 1;
...
PersistentSampleMap::PersistentSampleMap(uint64_t id, PersistentHistogramAllocator* allocator, Metadata* meta)
...
void PersistentSampleMap::Accumulate(Sample value, Count count)
{
    ...
    subtle::NoBarrier_AtomicIncrement(GetOrCreateSampleCountStorage(value), count);
...
std::unique_ptr<SampleCountIterator> PersistentSampleMap::ExtractingIterator()
{
    ...
    return std::make_unique<ExtractingPersistentSampleMapIterator>(sample_counts_);
}
...
// static
PersistentMemoryAllocator::Reference PersistentSampleMap::GetNextPersistentRecord(
    PersistentMemoryAllocator::Iterator& iterator, uint64_t* sample_map_id, Sample* value)
{
...
// static
PersistentMemoryAllocator::Reference PersistentSampleMap::CreatePersistentRecord(PersistentMemoryAllocator* allocator, uint64_t sample_map_id, Sample value)
{
    ...
    if (!record) {
        if (!allocator->IsFull()) {
            ...
            // unexpected failures.
            SCOPED_CRASH_KEY_BOOL("PersistentSampleMap", "corrupted", allocator->IsCorrupt());
...
bool PersistentSampleMap::AddSubtractImpl(SampleCountIterator* iter, Operator op)
{
    Sample min;
    int64_t max;
    Count count;
    for (; !iter->Done(); iter->Next()) {
        ...
        // concurrently modify the value.
        subtle::Barrier_AtomicIncrement(GetOrCreateSampleCountStorage(min), (op == HistogramSamples::ADD) ? count : -count);
    }
    return true;
}
