...
protected:
        ...
        : use_persistent_histogram_allocator_(GetParam())
    {
        ...
        if (use_persistent_histogram_allocator_) {
            GlobalHistogramAllocator::CreateWithLocalMemory(kAllocatorMemorySize, 0, "StatisticsRecorderTest");
        }
    }
    ...
    void UninitializeStatisticsRecorder()
    {
        ...
        // StatisticsRecorder destructor expects that the lock isn't already held.
        {
            ...
            if (statistics_recorder_) {
                ...
                statistics_recorder_->ranges_manager_.DoNotReleaseRangesOnDestroyForTesting();
            }
        }
        statistics_recorder_.reset();
    ...
    Histogram* CreateHistogram(const char* name, HistogramBase::Sample min, HistogramBase::Sample max, size_t bucket_count)
    {
    ...
    bool IsVLogInitialized()
    {
        return StatisticsRecorder::is_vlog_initialized_;
    }
...
TEST_P(StatisticsRecorderTest, NotInitialized)
{
    ...
    HistogramBase* const histogram = CreateHistogram("TestHistogram", 1, 1000, 10);
    EXPECT_EQ(StatisticsRecorder::RegisterOrDeleteDuplicate(histogram), histogram);
    EXPECT_TRUE(HasGlobalRecorder());
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram));
    ...
    EXPECT_EQ(StatisticsRecorder::RegisterOrDeleteDuplicateRanges(ranges), ranges);
    EXPECT_TRUE(HasGlobalRecorder());
    EXPECT_THAT(StatisticsRecorder::GetBucketRanges(), UnorderedElementsAre(ranges));
}
...
TEST_P(StatisticsRecorderTest, RegisterHistogram)
{
    ...
    EXPECT_EQ(histogram1, StatisticsRecorder::RegisterOrDeleteDuplicate(histogram1));
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1));
    ...
    EXPECT_EQ(histogram1, StatisticsRecorder::RegisterOrDeleteDuplicate(histogram2));
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1));
    ...
    EXPECT_EQ(histogram3, StatisticsRecorder::RegisterOrDeleteDuplicate(histogram3));
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1, histogram3));
}

TEST_P(StatisticsRecorderTest, FindHistogram)
{
    HistogramBase* histogram1 = Histogram::FactoryGet("TestHistogram1", 1, 1000, 10, HistogramBase::kNoFlags);
    HistogramBase* histogram2 = Histogram::FactoryGet("TestHistogram2", 1, 1000, 10, HistogramBase::kNoFlags);
    ...
    GlobalHistogramAllocator* old_global_allocator = GlobalHistogramAllocator::ReleaseForTesting();
    if (use_persistent_histogram_allocator_) {
        GlobalHistogramAllocator::CreateWithPersistentMemory(
            const_cast<void*>(old_global_allocator->data()), old_global_allocator->length(), 0, old_global_allocator->Id(), old_global_allocator->Name());
    }
...
TEST_P(StatisticsRecorderTest, WithName)
{
    ...
    EXPECT_THAT(StatisticsRecorder::WithName(histograms, "", /*case_sensitive=*/true), SizeIs(3));
    EXPECT_THAT(StatisticsRecorder::WithName(histograms, "Test", /*case_sensitive=*/true), SizeIs(3));
    EXPECT_THAT(StatisticsRecorder::WithName(histograms, "1", /*case_sensitive=*/true), SizeIs(1));
    ...
    EXPECT_THAT(StatisticsRecorder::WithName(histograms, "hello",
        ...
        IsEmpty());
    EXPECT_THAT(StatisticsRecorder::WithName(histograms, "test", /*case_sensitive=*/true), IsEmpty());
...
TEST_P(StatisticsRecorderTest, RegisterHistogramWithFactoryGet)
{
    ...
    // Create a histogram.
    HistogramBase* const histogram1 = Histogram::FactoryGet("TestHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1));

    // Get an existing histogram.
    HistogramBase* const histogram2 = Histogram::FactoryGet("TestHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    EXPECT_EQ(histogram1, histogram2);
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1));

    // Create a LinearHistogram.
    HistogramBase* const histogram3 = LinearHistogram::FactoryGet("TestLinearHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1, histogram3));

    // Create a BooleanHistogram.
    HistogramBase* const histogram4 = BooleanHistogram::FactoryGet("TestBooleanHistogram", HistogramBase::kNoFlags);
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1, histogram3, histogram4));

    // Create a CustomHistogram.
    std::vector<int> custom_ranges;
    custom_ranges.push_back(1);
    custom_ranges.push_back(5);
    HistogramBase* const histogram5 = CustomHistogram::FactoryGet("TestCustomHistogram", custom_ranges, HistogramBase::kNoFlags);
    EXPECT_THAT(StatisticsRecorder::GetHistograms(), UnorderedElementsAre(histogram1, histogram3, histogram4, histogram5));
}
...
TEST_P(StatisticsRecorderTest, RegisterHistogramWithMacros)
{
    ...
    HistogramBase* histogram = Histogram::FactoryGet("TestHistogramCounts", 1, 1000000, 50, HistogramBase::kNoFlags);
...
{
    Histogram::FactoryGet("TestHistogram1", 1, 1000, 50, HistogramBase::kNoFlags)->Add(30);
    Histogram::FactoryGet("TestHistogram1", 1, 1000, 50, HistogramBase::kNoFlags)->Add(40);
    Histogram::FactoryGet("TestHistogram2", 1, 1000, 50, HistogramBase::kNoFlags)->Add(30);
    Histogram::FactoryGet("TestHistogram2", 1, 1000, 50, HistogramBase::kNoFlags)->Add(40);
...
TEST_P(StatisticsRecorderTest, ToJSONOmitBuckets)
{
    ...
    std::string json = StatisticsRecorder::ToJSON(JSON_VERBOSITY_LEVEL_OMIT_BUCKETS);
    std::optional<Value> root = JSONReader::Read(json);
    ASSERT_TRUE(root);
    Value::Dict* root_dict = root->GetIfDict();
    ASSERT_TRUE(root_dict);
...
struct CallbackCheckWrapper {
    ...
    void OnHistogramChanged(const char* histogram_name, uint64_t name_hash, base::HistogramBase::Sample histogram_value)
    {
        called = true;
...
TEST_P(StatisticsRecorderTest, AddHistogramCallbackBeforeHistogramRegistration)
{
    test::TaskEnvironment task_environment;
    const char* histogram_name = "TestHistogram";
    CallbackCheckWrapper callback_wrapper;

    auto callback = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
        histogram_name, base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper)));
...
TEST_P(StatisticsRecorderTest, RemoveHistogramCallbackBeforeHistogramRegistrationWithMultipleClients)
{
    test::TaskEnvironment task_environment;
    const char* histogram_name = "TestHistogram";
    ...
    auto callback1 = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
        histogram_name, base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper1)));
    EXPECT_TRUE(base::StatisticsRecorder::have_active_callbacks());

    auto callback2 = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
        histogram_name, base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper2)));
    EXPECT_TRUE(base::StatisticsRecorder::have_active_callbacks());
...
TEST_P(StatisticsRecorderTest, AddHistogramCallbackWithMultipleClients)
{
    ...
    HistogramBase* histogram = Histogram::FactoryGet(histogram_name, 1, 1000, 10, HistogramBase::kNoFlags);
...
TEST_P(StatisticsRecorderTest, CallbackUsedTest)
{
    test::TaskEnvironment task_environment;
    {
        ...
        auto callback = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
            "TestHistogram", base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper)));
...

    {
        ...
        auto callback = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
            "TestLinearHistogram", base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper)));
        ...
        EXPECT_EQ(callback_wrapper.last_name_hash, HashMetricName("TestLinearHistogram"));
        EXPECT_EQ(callback_wrapper.last_histogram_value, 1);
    }

    {
        ...
        auto callback = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
            "TestCustomHistogram", base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper)));
        ...
        EXPECT_EQ(callback_wrapper.last_name_hash, HashMetricName("TestCustomHistogram"));
        EXPECT_EQ(callback_wrapper.last_histogram_value, 1);
    }

    {
        HistogramBase* custom_histogram = SparseHistogram::FactoryGet("TestSparseHistogram", HistogramBase::kNoFlags);

        CallbackCheckWrapper callback_wrapper;

        auto callback = std::make_unique<base::StatisticsRecorder::ScopedHistogramSampleObserver>(
            "TestSparseHistogram", base::BindRepeating(&CallbackCheckWrapper::OnHistogramChanged, base::Unretained(&callback_wrapper)));
        ...
        EXPECT_EQ(callback_wrapper.last_name_hash, HashMetricName("TestSparseHistogram"));
        EXPECT_EQ(callback_wrapper.last_histogram_value, 1);
    }
}
...
TEST_P(StatisticsRecorderTest, GlobalCallbackCalled)
{
    ...
    auto callback = [](const char* histogram_name, uint64_t name_hash, HistogramBase::Sample sample) {
...
TEST_P(StatisticsRecorderTest, ImportHistogramsTest)
{
    ...
    std::unique_ptr<StatisticsRecorder> temp_sr = StatisticsRecorder::CreateTemporaryForTesting();

    // Extract any existing global allocator so a new one can be created.
    GlobalHistogramAllocator* old_allocator = GlobalHistogramAllocator::ReleaseForTesting();
    ...
    GlobalHistogramAllocator* new_allocator = GlobalHistogramAllocator::ReleaseForTesting();
    ...
    HistogramBase* found = StatisticsRecorder::FindHistogram(histogram->histogram_name());
    ASSERT_TRUE(found);
