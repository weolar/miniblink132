...
protected:
    ...
    {
        GlobalHistogramAllocator::CreateWithLocalMemory(kAllocatorMemorySize, 0, "SparseHistogramAllocatorTest");
...
INSTANTIATE_TEST_SUITE_P(HeapAndPersistent, SparseHistogramTest, testing::Bool());
...
TEST_P(SparseHistogramTest, MacroBasicTest)
{
    ...
    const StatisticsRecorder::Histograms histograms = StatisticsRecorder::GetHistograms();
    ...
    EXPECT_EQ(HistogramBase::kUmaTargetedHistogramFlag | (use_persistent_histogram_allocator_ ? HistogramBase::kIsPersistent : 0), sparse_histogram->flags());
...
TEST_P(SparseHistogramTest, MacroInLoopTest)
{
    ...
    const StatisticsRecorder::Histograms histograms = StatisticsRecorder::Sort(StatisticsRecorder::GetHistograms());
...
TEST_P(SparseHistogramTest, FactoryTime)
{
    ...
    for (int i = 0; i < kTestLookupCount; ++i) {
        ...
        SparseHistogram::FactoryGet(histogram_names[index], HistogramBase::kNoFlags);
    }
...
TEST_P(SparseHistogramTest, ExtremeValues)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        HistogramBase* histogram = SparseHistogram::FactoryGet(StringPrintf("ExtremeValues_%zu", i), HistogramBase::kUmaTargetedHistogramFlag);
...
TEST_P(SparseHistogramTest, HistogramNameHash)
{
    ...
    HistogramBase* histogram = SparseHistogram::FactoryGet(kName, HistogramBase::kUmaTargetedHistogramFlag);
...
TEST_P(SparseHistogramTest, CheckGetCountAndBucketData)
{
    ...
    const CountAndBucketData count_and_data_bucket = GetCountAndBucketData(histogram.get());
...
{
    HistogramBase* histogram = SparseHistogram::FactoryGet("AsciiOut", HistogramBase::kNoFlags);
    ...
    const char kOutputFormatRe[] = R"(Histogram: AsciiOut recorded 20 samples.*\n)"
                                   R"(4   -+O +\(5 = 25.0%\)\n)"
                                   R"(10  -+O +\(15 = 75.0%\)\n)";
...
{
    HistogramBase* histogram = SparseHistogram::FactoryGet("HTMLOut", HistogramBase::kNoFlags);
    histogram->AddCount(/*sample=*/4, /*count=*/5);
    histogram->AddCount(/*sample=*/10, /*count=*/15);
    ...
    const char kOutputHeaderFormatRe[] = R"(Histogram: HTMLOut recorded 20 samples.*)";
