...
public:
    ...
    AtomicSingleSample* SingleSampleForTesting()
    {
...
protected:
    ...
    SampleVectorBase(uint64_t id, std::unique_ptr<Metadata> meta, const BucketRanges* bucket_ranges);
    ...
    size_t GetDestinationBucketIndexAndCount(SampleCountIterator& iter, HistogramBase::Count* count);
    ...
    {
        HistogramBase::AtomicCount* data = counts_data_.load(std::memory_order_acquire);
        if (data == nullptr) {
            return std::nullopt;
        }
    ...
    {
        const HistogramBase::AtomicCount* data = counts_data_.load(std::memory_order_acquire);
        if (data == nullptr) {
            return std::nullopt;
        }
        return std::make_optional(make_span(data, counts_size_));
...
private:
    ...
    std::string GetAsciiHeader(std::string_view histogram_name, int32_t flags) const override;
    ...
    void WriteAsciiBucketContext(int64_t past, HistogramBase::Count current, int64_t remaining, uint32_t current_bucket_index, std::string* output) const;
...
public:
    PersistentSampleVector(uint64_t id, const BucketRanges* bucket_ranges, Metadata* meta, const DelayedPersistentAllocation& counts);
