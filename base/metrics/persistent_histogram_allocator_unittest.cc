...
public:
    PersistentHistogramAllocatorTest(const PersistentHistogramAllocatorTest&) = delete;
    PersistentHistogramAllocatorTest& operator=(const PersistentHistogramAllocatorTest&) = delete;
...
protected:
    ...
    void CreatePersistentHistogramAllocator()
    {
        ...
        GlobalHistogramAllocator::CreateWithPersistentMemory(allocator_memory_, kAllocatorMemorySize, 0, 0, "PersistentHistogramAllocatorTest");
...
TEST_F(PersistentHistogramAllocatorTest, CreateAndIterate)
{
    ...
    HistogramBase* histogram = Histogram::FactoryGet("TestHistogram", 1, 1000, 10, HistogramBase::kIsPersistent);
    ...
    HistogramBase* linear_histogram = LinearHistogram::FactoryGet("TestLinearHistogram", 1, 1000, 10, HistogramBase::kIsPersistent);
    ...
    HistogramBase* boolean_histogram = BooleanHistogram::FactoryGet("TestBooleanHistogram", HistogramBase::kIsPersistent);
    ...
    HistogramBase* custom_histogram = CustomHistogram::FactoryGet("TestCustomHistogram", custom_ranges, HistogramBase::kIsPersistent);
    ...
    PersistentHistogramAllocator recovery(
        std::make_unique<PersistentMemoryAllocator>(allocator_memory_, kAllocatorMemorySize, 0, 0, "", PersistentMemoryAllocator::kReadWrite));
...
TEST_F(PersistentHistogramAllocatorTest, ConstructPaths)
{
    ...
    const std::string dir_string = dir_path.NormalizePathSeparators().AsUTF8Unsafe();
    ...
    Time stamp;
    ProcessId pid;
    EXPECT_FALSE(GlobalHistogramAllocator::ParseFilePath(path, &name, nullptr, nullptr));
    EXPECT_FALSE(GlobalHistogramAllocator::ParseFilePath(path, nullptr, &stamp, nullptr));
    EXPECT_FALSE(GlobalHistogramAllocator::ParseFilePath(path, nullptr, nullptr, &pid));

    path = GlobalHistogramAllocator::ConstructFilePathForUploadDir(dir_path, "bar", Time::FromTimeT(12345), 6789);
    EXPECT_EQ(dir_string + "bar-3039-1A85.pma", path.AsUTF8Unsafe());
    ASSERT_TRUE(GlobalHistogramAllocator::ParseFilePath(path, &name, &stamp, &pid));
    EXPECT_EQ(name, "bar");
...
TEST_F(PersistentHistogramAllocatorTest, CreateWithFile)
{
    ...
    EXPECT_EQ(std::string(temp_name), GlobalHistogramAllocator::Get()->memory_allocator()->Name());
...
TEST_F(PersistentHistogramAllocatorTest, CreateSpareFile)
{
    ...
    for (size_t pos = 0; pos < temp_size; pos += sizeof(buffer)) {
        ASSERT_EQ(static_cast<int>(sizeof(buffer)), file.ReadAtCurrentPos(buffer, sizeof(buffer)));
...
TEST_F(PersistentHistogramAllocatorTest, StatisticsRecorderMerge)
{
    ...
    const size_t global_sr_initial_histogram_count = StatisticsRecorder::GetHistogramCount();
    const size_t global_sr_initial_bucket_ranges_count = StatisticsRecorder::GetBucketRanges().size();
    ...
    HistogramBase* histogram1 = LinearHistogram::FactoryGet(LinearHistogramName, 1, 10, 10, 0);
    ...
    HistogramBase* histogram2 = SparseHistogram::FactoryGet(SparseHistogramName, 0);
    ...
    local_sr.reset();
    EXPECT_EQ(global_sr_initial_histogram_count, StatisticsRecorder::GetHistogramCount());
    EXPECT_EQ(global_sr_initial_bucket_ranges_count, StatisticsRecorder::GetBucketRanges().size());
    GlobalHistogramAllocator::Set(old_allocator);

    // Create a "recovery" allocator using the same memory as the local one.
    PersistentHistogramAllocator recovery1(std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(new_allocator->memory_allocator()->data()),
        new_allocator->memory_allocator()->size(), 0, 0, "", PersistentMemoryAllocator::kReadWrite));
    ...
    EXPECT_EQ(global_sr_initial_histogram_count + 2, StatisticsRecorder::GetHistogramCount());
    ...
    // RangesManager.
    ASSERT_EQ(global_sr_initial_bucket_ranges_count + 1, StatisticsRecorder::GetBucketRanges().size());
    ...
    // Do another merge.
    PersistentHistogramAllocator recovery2(std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(new_allocator->memory_allocator()->data()),
        new_allocator->memory_allocator()->size(), 0, 0, "", PersistentMemoryAllocator::kReadWrite));
...
TEST_F(PersistentHistogramAllocatorTest, StatisticsRecorderMerge_IsDefinitelyEmpty)
{
    const size_t global_sr_initial_histogram_count = StatisticsRecorder::GetHistogramCount();
    const size_t global_sr_initial_bucket_ranges_count = StatisticsRecorder::GetBucketRanges().size();
    ...
    HistogramBase* histogram2 = LinearHistogram::FactoryGet("SRTLinearHistogram2", 1, 10, 10, 0);
    histogram2->Add(3);
    histogram2->SnapshotDelta();
    HistogramBase* histogram3 = LinearHistogram::FactoryGet("SRTLinearHistogram3", 1, 10, 10, 0);
    histogram3->Add(1);
    histogram3->Add(10);
    histogram3->SnapshotDelta();
    SparseHistogram::FactoryGet("SRTSparseHistogram1", 0);
    HistogramBase* sparse_histogram2 = SparseHistogram::FactoryGet("SRTSparseHistogram2", 0);
    sparse_histogram2->Add(3);
    sparse_histogram2->SnapshotDelta();
    HistogramBase* sparse_histogram3 = SparseHistogram::FactoryGet("SRTSparseHistogram3", 0);
...
TEST_F(PersistentHistogramAllocatorTest, MultipleSameSparseHistograms)
{
    ...
    sparse->AddCount(1, 3);
    std::unique_ptr<HistogramSamples> snapshot = sparse->SnapshotUnloggedSamples();
    std::unique_ptr<HistogramSamples> snapshot2 = sparse2->SnapshotUnloggedSamples();
...
TEST_F(PersistentHistogramAllocatorTest, CustomRangesManager)
{
    ...
    HistogramBase* histogram = LinearHistogram::FactoryGet(LinearHistogramName, /*minimum=*/1, /*maximum=*/10, /*bucket_count=*/10,
        /*flags=*/0);
    ...
    // Create a "recovery" allocator using the same memory as the local one.
    PersistentHistogramAllocator recovery(std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(new_allocator->memory_allocator()->data()),
        new_allocator->memory_allocator()->size(), 0, 0, "", PersistentMemoryAllocator::kReadWrite));
...
TEST_F(PersistentHistogramAllocatorTest, RangesDeDuplication)
{
    ...
    HistogramBase* histogram1 = Histogram::FactoryGet("TestHistogram1", 1, 1000, 10, 0);
    HistogramBase* histogram2 = Histogram::FactoryGet("TestHistogram2", 1, 1000, 10, 0);
    const uint32_t ranges_ref = static_cast<Histogram*>(histogram1)->bucket_ranges()->persistent_reference();
    ASSERT_NE(0U, ranges_ref);
    EXPECT_EQ(ranges_ref, static_cast<Histogram*>(histogram2)->bucket_ranges()->persistent_reference());
    ...
    uint32_t* data1 = allocator_->GetAsArray<uint32_t>(ref1, 0, kRangesRefIndex + 1);
    uint32_t* data2 = allocator_->GetAsArray<uint32_t>(ref2, 0, kRangesRefIndex + 1);
...
TEST_F(PersistentHistogramAllocatorTest, MovePersistentFile)
{
    ...
    EXPECT_EQ(file.Read(/*offset=*/0, data.get(), temp_size), static_cast<int>(temp_size));

    // Create an allocator and iterator using the file's data.
    PersistentHistogramAllocator new_file_allocator(
        std::make_unique<PersistentMemoryAllocator>(data.get(), temp_size, 0, 0, "", PersistentMemoryAllocator::kReadWrite));
