...
public:
    ...
    public:
        ...
        virtual double GetEntropyForTrial(std::string_view trial_name, uint32_t randomization_seed) const = 0;
    ...
    struct BASE_EXPORT FieldTrialEntry {
        ...
        bool GetState(std::string_view& trial_name, std::string_view& group_name, bool& is_overridden) const;
    ...
    private:
        ...
        // and |group_name|.
        bool ReadStringPair(PickleIterator* iter, std::string_view* trial_name, std::string_view* group_name) const;

        // Reads the field trial header, which includes the name of the trial and
        // group, and the is_overridden bool.
        bool ReadHeader(PickleIterator& iter, std::string_view& trial_name, std::string_view& group_name, bool& is_overridden) const;
    };
    ...
    static bool ParseFieldTrialsString(std::string_view field_trials_string, bool override_trials, std::vector<State>& entries);

    // Returns a '--force-fieldtrials' formatted string representing the list of
    // provided trial states.
    static std::string BuildFieldTrialStateString(const std::vector<State>& states);
    ...
    bool is_low_anonymity() const
    {
        return is_low_anonymity_;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(FieldTrialListTest, TestGetRandomizedFieldTrialCount);
    ...
    FieldTrial(std::string_view trial_name, Probability total_probability, std::string_view default_group_name, double entropy_value, bool is_low_anonymity,
        bool is_overridden);
...
public:
    ...
    public:
        ...
        virtual void OnFieldTrialGroupFinalized(const FieldTrial& trial, const std::string& group_name) = 0;
    ...
    static FieldTrial* FactoryGetFieldTrial(std::string_view trial_name, FieldTrial::Probability total_probability, std::string_view default_group_name,
        const FieldTrial::EntropyProvider& entropy_provider, uint32_t randomization_seed = 0, bool is_low_anonymity = false, bool is_overridden = false);
    ...
    // created trials.
    static bool CreateTrialsFromString(const std::string& trials_string, bool override_trials = false);
    ...
    // |FieldTrialListIncludingLowAnonymity|.
    static FieldTrial* CreateFieldTrial(std::string_view name, std::string_view group_name, bool is_low_anonymity = false, bool is_overridden = false);
    ...
    static bool GetParamsFromSharedMemory(FieldTrial* field_trial, std::map<std::string, std::string>* params);
    ...
    // crash.
    static void DumpAllFieldTrialsToPersistentAllocator(PersistentMemoryAllocator* allocator);
    ...
    static std::vector<const FieldTrial::FieldTrialEntry*> GetAllFieldTrialsFromPersistentAllocator(PersistentMemoryAllocator const& allocator);
    ...
    static bool CreateTrialsFromFieldTrialStates(PassKey<test::ScopedFeatureList>, const std::vector<FieldTrial::State>& entries);
...
private:
    ...
    static std::string SerializeSharedMemoryRegionMetadata(const ReadOnlySharedMemoryRegion& shm, LaunchOptions* launch_options);
    ...
    static base::shared_memory::SharedMemoryError CreateTrialsFromSwitchValue(const std::string& switch_value);
#endif // BUILDFLAG(USE_BLINK)
    ...
    static bool CreateTrialsFromSharedMemoryRegion(const ReadOnlySharedMemoryRegion& shm_region);
    ...
    static bool CreateTrialsFromSharedMemoryMapping(ReadOnlySharedMemoryMapping shm_mapping);
    ...
    // calling this.
    static void AddToAllocatorWhileLocked(PersistentMemoryAllocator* allocator, FieldTrial* field_trial);
    ...
    typedef std::map<std::string, raw_ptr<FieldTrial, CtnExperimental>, std::less<>> RegistrationMap;

    // Helper function should be called only while holding lock_.
    FieldTrial* PreLockedFind(std::string_view name) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    // method internally.
    static bool CreateTrialsFromFieldTrialStatesInternal(const std::vector<FieldTrial::State>& entries);
    ...
    // |FieldTrialListIncludingLowAnonymity::GetActiveFieldTrialGroups|.
    static void GetActiveFieldTrialGroupsInternal(FieldTrial::ActiveGroups* active_groups, bool include_low_anonymity);

    // The same as |AddObserver| but is notified for low anonymity field trials
    // too.
    // Restricted to specifically allowed friends - access via
    // |FieldTrialListIncludingLowAnonymity::AddObserver|.
    static bool AddObserverInternal(Observer* observer, bool include_low_anonymity);
    ...
    static void RemoveObserverInternal(Observer* observer, bool include_low_anonymity);
    ...
    std::vector<raw_ptr<Observer, VectorExperimental>> observers_ GUARDED_BY(lock_);

    // List of observers to be notified when a group is selected for a FieldTrial.
    // Includes low anonymity field trials.
    std::vector<raw_ptr<Observer, VectorExperimental>> observers_including_low_anonymity_ GUARDED_BY(lock_);
