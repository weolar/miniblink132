...
enum UmaHistogramTestingEnum { UMA_HISTOGRAM_TESTING_ENUM_FIRST, UMA_HISTOGRAM_TESTING_ENUM_SECOND, UMA_HISTOGRAM_TESTING_ENUM_THIRD };
...
TEST(HistogramFunctionsTest, Enumeration)
{
    ...
    HistogramTester tester;
    UmaHistogramEnumeration(histogram, UMA_HISTOGRAM_TESTING_ENUM_FIRST, UMA_HISTOGRAM_TESTING_ENUM_THIRD);
    tester.ExpectUniqueSample(histogram, UMA_HISTOGRAM_TESTING_ENUM_FIRST, 1);

    // Verify the overflow & underflow bucket exists.
    UMA_HISTOGRAM_ENUMERATION(histogram, static_cast<int>(UMA_HISTOGRAM_TESTING_ENUM_THIRD) + 10, static_cast<int>(UMA_HISTOGRAM_TESTING_ENUM_THIRD));
    tester.ExpectBucketCount(histogram, static_cast<int>(UMA_HISTOGRAM_TESTING_ENUM_THIRD) + 1, 1);
...
TEST(HistogramFunctionsTest, Counts)
{
    ...
    EXPECT_THAT(tester.GetAllSamples(histogram), testing::ElementsAre(Bucket(0, 1), Bucket(1, 1), Bucket(16, 3), Bucket(54, 1), Bucket(100, 2)));
}
...
TEST(HistogramFunctionsTest, Sparse_SupportsLargeRange)
{
    ...
    UmaHistogramSparse(histogram, 123456789);
    EXPECT_THAT(tester.GetAllSamples(histogram), testing::ElementsAre(Bucket(0, 1), Bucket(123456789, 2)));
}
