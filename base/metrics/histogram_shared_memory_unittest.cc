...
TEST(HistogramSharedMemoryTest, Create)
{
    ...
    auto shared_memory = HistogramSharedMemory::Create(kProcessId, { kProcessType, kProcessName, kArbitrarySize });
...
//    service, so we don't have to manually update the mapping here.
    ...
    HistogramSharedMemory::InitFromLaunchParameters(*CommandLine::ForCurrentProcess());
...
INSTANTIATE_TEST_SUITE_P(All, HistogramSharedMemoryTest,
...
TEST_P(HistogramSharedMemoryTest, PassSharedMemoryRegion_Enabled)
{
    ...
    SCOPED_TRACE(command_line.GetSwitchValueASCII(switches::kMetricsSharedMemoryHandle));
    ...
    Process process = SpawnMultiProcessTestChild("InitFromLaunchParameters", command_line, launch_options);
