...
scoped_refptr<FieldTrial> CreateFieldTrial(const std::string& trial_name, int total_probability, const std::string& default_group_name)
{
    MockEntropyProvider entropy_provider(0.9);
    return FieldTrialList::FactoryGetFieldTrial(trial_name, total_probability, default_group_name, entropy_provider);
}
...
public:
    ...
    void CreateFeatureWithTrial(const Feature& feature, FeatureList::OverrideState override_state, FieldTrial* trial)
    {
        std::unique_ptr<FeatureList> feature_list(new FeatureList);
        feature_list->RegisterFieldTrialOverride(feature.name, override_state, trial);
...
TEST_F(FieldTrialParamsTest, GetFieldTrialParamsByFeature)
{
    ...
    CreateFeatureWithTrial(kFeature, FeatureList::OVERRIDE_ENABLE_FEATURE, trial.get());
...
TEST_F(FieldTrialParamsTest, GetFieldTrialParamsByFeature_Disable)
{
    ...
    CreateFeatureWithTrial(kFeature, FeatureList::OVERRIDE_DISABLE_FEATURE, trial.get());
...
TEST_F(FieldTrialParamsTest, FeatureParamDouble)
{
    ...
    EXPECT_EQ(0, GetFieldTrialParamByFeatureAsDouble(kFeature, "d", 0)); // invalid
...
TEST_F(FieldTrialParamsTest, FeatureParamEnum)
{
    ...
    static const FeatureParam<Hand>::Option hands[] = { { ROCK, "rock" }, { PAPER, "paper" }, { SCISSORS, "scissors" } };
    static BASE_FEATURE(kFeature, "TestFeature", FEATURE_DISABLED_BY_DEFAULT);
...
TEST_F(FieldTrialParamsTest, FeatureParamEnumClass)
{
    ...
    static const FeatureParam<UI>::Option uis[] = { { UI::ONE_D, "1d" }, { UI::TWO_D, "2d" }, { UI::THREE_D, "3d" } };
    static BASE_FEATURE(kFeature, "TestFeature", FEATURE_DISABLED_BY_DEFAULT);
