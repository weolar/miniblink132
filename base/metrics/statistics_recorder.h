...
public:
    ...
    public:
        ...
        virtual void MergeHistogramDeltas(bool async, OnceClosure done_callback) = 0;
    };
    ...
    using OnSampleCallback = base::RepeatingCallback<void(const char* /*=histogram_name*/, uint64_t /*=name_hash*/, HistogramBase::Sample)>;
    ...
    public:
        ...
        explicit ScopedHistogramSampleObserver(const std::string& histogram_name, OnSampleCallback callback);
    ...
    private:
        ...
        // Runs the callback.
        void RunCallback(const char* histogram_name, uint64_t name_hash, HistogramBase::Sample sample);
    ...
    //
    // This method is thread safe.
    static void RegisterHistogramProvider(const WeakPtr<HistogramProvider>& provider);
    ...
    //
    // This method is thread safe.
    static const BucketRanges* RegisterOrDeleteDuplicateRanges(const BucketRanges* ranges);
    ...
    //
    // This method is thread safe.
    static Histograms GetHistograms(bool include_persistent = true) LOCKS_EXCLUDED(GetLock());
    ...
    static SnapshotTransactionId PrepareDeltas(bool include_persistent, HistogramBase::Flags flags_to_set, HistogramBase::Flags required_flags,
        HistogramSnapshotManager* snapshot_manager) LOCKS_EXCLUDED(snapshot_lock_.Pointer());
    ...
    static SnapshotTransactionId SnapshotUnloggedSamples(HistogramBase::Flags required_flags, HistogramSnapshotManager* snapshot_manager)
    ...
    static SnapshotTransactionId GetLastSnapshotTransactionId() LOCKS_EXCLUDED(snapshot_lock_.Pointer());

    // Retrieves and runs the list of callbacks for the histogram referred to by
    // |histogram_name|, if any.
    //
    // This method is thread safe.
    static void FindAndRunHistogramCallbacks(base::PassKey<HistogramBase>, const char* histogram_name, uint64_t name_hash, HistogramBase::Sample sample);
    ...
    //
    // This method is thread safe.
    [[nodiscard]] static std::unique_ptr<StatisticsRecorder> CreateTemporaryForTesting();
    ...
    // mutate any state.
    static void SetRecordChecker(std::unique_ptr<RecordHistogramChecker> record_checker);
    ...
    // case sensitive way.
    static Histograms WithName(Histograms histograms, const std::string& query, bool case_sensitive = true);

    using GlobalSampleCallback = void (*)(const char* /*=histogram_name*/, uint64_t /*=name_hash*/, HistogramBase::Sample);
    ...
    static void SetGlobalSampleCallback(const GlobalSampleCallback& global_sample_callback);
...
private:
    ...
    HistogramBase* FindHistogramByHashInternal(uint64_t hash, std::string_view name) const EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    ...
    //
    // This method is thread safe.
    static void AddHistogramSampleObserver(const std::string& histogram_name, ScopedHistogramSampleObserver* observer);

    // Clears the given |observer| set on the histogram referred to by
    // |histogram_name|.
    //
    // This method is thread safe.
    static void RemoveHistogramSampleObserver(const std::string& histogram_name, ScopedHistogramSampleObserver* observer);
    ...
    // creation.
    using HistogramSampleObserverList = base::ObserverListThreadSafe<ScopedHistogramSampleObserver>;
    typedef std::unordered_map<uint64_t, scoped_refptr<HistogramSampleObserverList>> ObserverMap;
    ...
    // multiple times.
    static void EnsureGlobalRecorderWhileLocked() EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    ...
    static void InitLogOnShutdownWhileLocked() EXCLUSIVE_LOCKS_REQUIRED(GetLock());
    ...
    static SnapshotTransactionId last_snapshot_transaction_id_ GUARDED_BY(snapshot_lock_.Get());
