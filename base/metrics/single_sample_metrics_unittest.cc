...
TEST_F(SingleSampleMetricsTest, DefaultSingleSampleMetricNoValue)
{
    ...
    std::unique_ptr<SingleSampleMetric> metric = factory->CreateCustomCountsMetric(kMetricName, kMin, kMax, kBucketCount);
    metric.reset();
...
TEST_F(SingleSampleMetricsTest, DefaultSingleSampleMetricWithValue)
{
    ...
    EXPECT_EQ(DummyHistogram::GetInstance(), Histogram::FactoryGet(kMetricName, 1, 3, 3, HistogramBase::kNoFlags));
    EXPECT_NE(DummyHistogram::GetInstance(), Histogram::FactoryGet(kMetricName, kMin, kMax, kBucketCount, HistogramBase::kUmaTargetedHistogramFlag));
}
...
TEST_F(SingleSampleMetricsTest, MultipleMetricsAreDistinct)
{
    ...
    HistogramTester tester;
    std::unique_ptr<SingleSampleMetric> metric = factory->CreateCustomCountsMetric(kMetricName, kMin, kMax, kBucketCount);
    std::unique_ptr<SingleSampleMetric> metric2 = factory->CreateCustomCountsMetric(kMetricName, kMin, kMax, kBucketCount);
    const char kMetricName2[] = "Single.Sample.Metric.2";
    std::unique_ptr<SingleSampleMetric> metric3 = factory->CreateCustomCountsMetric(kMetricName2, kMin, kMax, kBucketCount);
