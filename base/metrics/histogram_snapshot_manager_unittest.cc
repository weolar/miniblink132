...
{
    HistogramBase* histogram = BooleanHistogram::FactoryGet(name, HistogramBase::kUmaStabilityHistogramFlag);
...
public:
    ...
    HistogramFlattenerDeltaRecorder(const HistogramFlattenerDeltaRecorder&) = delete;
    HistogramFlattenerDeltaRecorder& operator=(const HistogramFlattenerDeltaRecorder&) = delete;

    void RecordDelta(const HistogramBase& histogram, const HistogramSamples& snapshot) override
    {
    ...
    std::vector<raw_ptr<const HistogramBase, VectorExperimental>>& GetRecordedDeltaHistograms()
    {
...
private:
    std::vector<raw_ptr<const HistogramBase, VectorExperimental>> recorded_delta_histograms_;
...
protected:
    ...
    int64_t GetRecordedDeltaHistogramSum(const std::string& name)
    {
        return histogram_flattener_delta_recorder_.GetRecordedDeltaHistogramSum(name);
    }
...
TEST_F(HistogramSnapshotManagerTest, PrepareDeltasNoFlagsFilter)
{
    ...
    const std::vector<raw_ptr<const HistogramBase, VectorExperimental>>& histograms = histogram_flattener_delta_recorder_.GetRecordedDeltaHistograms();
...
TEST_F(HistogramSnapshotManagerTest, PrepareDeltasUmaHistogramFlagFilter)
{
    ...
    StatisticsRecorder::PrepareDeltas(
        /*include_persistent=*/false, /*flags_to_set=*/HistogramBase::kNoFlags,
        /*required_flags=*/HistogramBase::kUmaTargetedHistogramFlag, &histogram_snapshot_manager_);
...
TEST_F(HistogramSnapshotManagerTest, PrepareDeltasUmaStabilityHistogramFlagFilter)
{
    base::UmaHistogramBoolean(kHistogramName, true);
    UmaStabilityHistogramBoolean(kStabilityHistogramName, true);
    ...
    StatisticsRecorder::PrepareDeltas(
        /*include_persistent=*/false, /*flags_to_set=*/HistogramBase::kNoFlags,
        /*required_flags=*/HistogramBase::kUmaStabilityHistogramFlag, &histogram_snapshot_manager_);
...
TEST_F(HistogramSnapshotManagerTest, SnapshotUnloggedSamplesNoFlagsFilter)
{
    ...
    std::unique_ptr<HistogramSamples> samples = histograms[0]->SnapshotUnloggedSamples();
...
TEST_F(HistogramSnapshotManagerTest, SnapshotUnloggedSamplesUmaHistogramFlagFilter)
{
    // Note that kUmaStabilityHistogramFlag includes kUmaTargetedHistogramFlag.
    base::UmaHistogramBoolean(kHistogramName, true);
    UmaStabilityHistogramBoolean(kStabilityHistogramName, true);
...
TEST_F(HistogramSnapshotManagerTest, SnapshotUnloggedSamplesUmaStabilityHistogramFlagFilter)
{
    base::UmaHistogramBoolean(kHistogramName, true);
    UmaStabilityHistogramBoolean(kStabilityHistogramName, true);
