...
public:
    ...
    void SetUp() override
    {
        ...
        allocator_ = std::make_unique<PersistentMemoryAllocator>(
            mem_segment_.data(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, TEST_ID, TEST_NAME, PersistentMemoryAllocator::kReadWrite);
    }
...
TEST_F(PersistentMemoryAllocatorTest, AllocateAndIterate)
{
    ...
    EXPECT_EQ("UMA.PersistentAllocator." + base_name + ".UsedPct", allocator_->used_histogram_->histogram_name());
    EXPECT_EQ(PersistentMemoryAllocator::MEMORY_INITIALIZED, allocator_->GetMemoryState());
    ...
    EXPECT_NE(nullptr, allocator_->GetAsObject<TestObject1>(block1, &alloc_size_1));
    ...
    EXPECT_NE(nullptr, allocator_->GetAsObject<TestObject2>(block2, &alloc_size_2));
    ...
    std::unique_ptr<HistogramSamples> used_samples(allocator_->used_histogram_->SnapshotSamples());
    ...
    std::unique_ptr<PersistentMemoryAllocator> allocator2(
        new PersistentMemoryAllocator(mem_segment_.data(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, 0, "", PersistentMemoryAllocator::kReadWrite));
    ...
    std::unique_ptr<const PersistentMemoryAllocator> allocator3(
        new PersistentMemoryAllocator(mem_segment_.data(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, 0, "", PersistentMemoryAllocator::kReadOnly));
    ...
    EXPECT_EQ(PersistentMemoryAllocator::MEMORY_DELETED, allocator_->GetMemoryState());
}
...
public:
    AllocatorThread(const std::string& name, void* base, uint32_t size, uint32_t page_size)
        : SimpleThread(name, Options())
        , count_(0)
        , iterable_(0)
        , allocator_(base, size, page_size, 0, "", PersistentMemoryAllocator::kReadWrite)
    {
    }
...
TEST_F(PersistentMemoryAllocatorTest, ParallelismTest)
{
    ...
    EXPECT_EQ(CountIterables(), t1.iterable() + t2.iterable() + t3.iterable() + t4.iterable() + t5.iterable());
}

// A simple thread that makes all objects passed iterable.
class MakeIterableThread : public SimpleThread {
public:
    MakeIterableThread(const std::string& name, PersistentMemoryAllocator* allocator, span<Reference> refs)
        : SimpleThread(name, Options())
...
public:
    CounterThread(const std::string& name, PersistentMemoryAllocator::Iterator* iterator, Lock* lock, ConditionVariable* condition, bool* wake_up)
        : SimpleThread(name, Options())
        , iterator_(iterator)
        , lock_(lock)
...
TEST_F(PersistentMemoryAllocatorTest, IteratorParallelismTest)
{
    ...
    EXPECT_EQ(iterable_count, t1.count() + t2.count() + t3.count() + t4.count() + t5.count());
...
TEST_F(PersistentMemoryAllocatorTest, DelayedAllocationTest)
{
    ...
    EXPECT_EQ(allocator_->GetAsReference(mem1.data(), 1001u), ref1.load(std::memory_order_relaxed));
    ...
    EXPECT_EQ(allocator_->GetAsReference(mem2a.data(), 2002u), ref2.load(std::memory_order_relaxed));
    ...
    EXPECT_EQ(0U, iter.GetNext(&type));
    EXPECT_EQ(reinterpret_cast<uintptr_t>(mem2a.data()) + 5u, reinterpret_cast<uintptr_t>(mem2b.data()));
    ...
    EXPECT_EQ(0U, iter.GetNext(&type));
    EXPECT_EQ(reinterpret_cast<uintptr_t>(mem2a.data()) + 8u, reinterpret_cast<uintptr_t>(mem2c.data()));
...
{
    base::WritableSharedMemoryRegion rw_region = base::WritableSharedMemoryRegion::Create(TEST_MEMORY_SIZE);
    ...
    base::ReadOnlySharedMemoryRegion ro_region = base::WritableSharedMemoryRegion::ConvertToReadOnly(std::move(rw_region));
...
TEST(FilePersistentMemoryAllocatorTest, CreationTest)
{
    ...
    FilePersistentMemoryAllocator file(std::move(mmfile), 0, 0, "", FilePersistentMemoryAllocator::kReadWrite);
...
TEST(FilePersistentMemoryAllocatorTest, ExtendTest)
{
    ...
    {
        std::unique_ptr<MemoryMappedFile> mmfile(new MemoryMappedFile());
        ASSERT_TRUE(mmfile->Initialize(File(file_path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE), region, MemoryMappedFile::READ_WRITE_EXTEND));
        FilePersistentMemoryAllocator allocator(std::move(mmfile), region.size, 0, "", FilePersistentMemoryAllocator::kReadWrite);
    ...
    // Verify that it's still an acceptable file.
    {
        ...
        EXPECT_TRUE(FilePersistentMemoryAllocator::IsFileAcceptable(*mmfile, false));
    }
}
    ...
    for (size_t filesize = minsize; filesize > 0; --filesize) {
        ...
        if (FilePersistentMemoryAllocator::IsFileAcceptable(*mmfile, read_only)) {
            ...
            FilePersistentMemoryAllocator allocator(
                std::move(mmfile), 0, 0, "", read_only ? FilePersistentMemoryAllocator::kReadOnly : FilePersistentMemoryAllocator::kReadWrite);
            ...
            while ((ref = iter.GetNext(&type_id)) != 0) {
                size_t size = 0;
                const char* data = allocator.GetAsArray<char>(ref, 0, PersistentMemoryAllocator::kSizeAny, &size);
...
TEST_F(PersistentMemoryAllocatorTest, TruncateTest)
{
    ...
    ASSERT_FALSE(PathExists(file_path));
    {
        ...
        File writer(file_path, File::FLAG_CREATE | File::FLAG_WRITE);
        ASSERT_TRUE(writer.IsValid());
        writer.Write(0, static_cast<const char*>(allocator.data()), allocator.size());
    }
    ASSERT_TRUE(PathExists(file_path));
    ...
    for (size_t file_length : { a2_used, a1_used, a1_used / 2 }) {
        ...
        for (bool read_only : { false, true }) {
            ...
            std::unique_ptr<MemoryMappedFile> mmfile(new MemoryMappedFile());
            ASSERT_TRUE(mmfile->Initialize(File(file_path, File::FLAG_OPEN | (read_only ? File::FLAG_READ : File::FLAG_READ | File::FLAG_WRITE)),
                read_only ? MemoryMappedFile::READ_ONLY : MemoryMappedFile::READ_WRITE));
            ASSERT_TRUE(FilePersistentMemoryAllocator::IsFileAcceptable(*mmfile, read_only));
