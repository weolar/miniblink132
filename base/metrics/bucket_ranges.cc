...
uint32_t BucketRanges::CalculateChecksum() const
{
    ...
    return Crc32(static_cast<uint32_t>(ranges_size), 
        //base::as_byte_span(ranges_)
        span<const uint8_t>((const uint8_t*)(&(ranges_[0])), sizeof(HistogramBase::Sample) * ranges_.size())
    );
}
