...
bool ReadHistogramArguments(
    PickleIterator* iter, std::string* histogram_name, int* flags, int* declared_min, int* declared_max, size_t* bucket_count, uint32_t* range_checksum)
{
    uint32_t bucket_count_u32;
    if (!iter->ReadString(histogram_name) || !iter->ReadInt(flags) || !iter->ReadInt(declared_min) || !iter->ReadInt(declared_max)
        || !iter->ReadUInt32(&bucket_count_u32) || !iter->ReadUInt32(range_checksum)) {
    ...
    *bucket_count = bucket_count_u32;
    ...
    if (*declared_max <= 0 || *declared_min <= 0 || *declared_max < *declared_min || INT_MAX / sizeof(HistogramBase::Count) <= *bucket_count
        || *bucket_count < 2) {
...
bool ValidateRangeChecksum(const HistogramBase& histogram, uint32_t range_checksum)
{
...
public:
    Factory(std::string_view name, HistogramBase::Sample minimum, HistogramBase::Sample maximum, size_t bucket_count, int32_t flags)
...
protected:
    Factory(
        std::string_view name, HistogramType histogram_type, HistogramBase::Sample minimum, HistogramBase::Sample maximum, size_t bucket_count, int32_t flags)
        : name_(name)
...
HistogramBase* Histogram::Factory::Build()
{
    ...
    if (!histogram) {
        ...
        bool should_record = StatisticsRecorder::ShouldRecordHistogram(HashMetricNameAs32Bits(name_));
        if (!should_record)
        ...
        if (allocator) {
            tentative_histogram = allocator->AllocateHistogram(histogram_type_, name_, minimum_, maximum_, registered_ranges, flags_, &histogram_ref);
        }
    ...
    if (histogram_type_ != histogram->GetHistogramType() || (bucket_count_ != 0 && !histogram->HasConstructionArguments(minimum_, maximum_, bucket_count_))) {
        ...
        // analysis.
        UmaHistogramSparse("Histogram.MismatchedConstructionArguments", static_cast<Sample>(HashMetricName(name_)));
...
HistogramBase* Histogram::FactoryGet(std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryTimeGet(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryMicrosecondsTimeGet(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryMicrosecondsTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryGet(const std::string& name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryTimeGet(const std::string& name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryMicrosecondsTimeGet(const std::string& name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryMicrosecondsTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryGet(const char* name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryTimeGet(const char* name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* Histogram::FactoryMicrosecondsTimeGet(const char* name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryMicrosecondsTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

std::unique_ptr<HistogramBase> Histogram::PersistentCreate(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
{
    return WrapUnique(new Histogram(name, ranges, counts, logged_counts, meta, logged_meta));
}
...
// static
bool Histogram::InspectConstructionArguments(std::string_view name, Sample* minimum, Sample* maximum, size_t* bucket_count)
{
    ...
    if (*bucket_count > kBucketCount_MAX) {
        UmaHistogramSparse("Histogram.TooManyBuckets.1000", static_cast<Sample>(HashMetricName(name)));

        // Blink.UseCounter legitimately has more than 1000 entries in its enum.
        if (!StartsWith(name, "Blink.UseCounter")) {
            DLOG(ERROR) << "Histogram: " << name << " has bad bucket_count: " << *bucket_count << " (limit " << kBucketCount_MAX << ")";
    ...
    if (!check_okay) {
        UmaHistogramSparse("Histogram.BadConstructionArguments", static_cast<Sample>(HashMetricName(name)));
    }
...
bool Histogram::HasConstructionArguments(Sample expected_minimum, Sample expected_maximum, size_t expected_bucket_count) const
{
    return (expected_bucket_count == bucket_count() && expected_minimum == declared_min() && expected_maximum == declared_max());
}
...
std::unique_ptr<HistogramSamples> Histogram::SnapshotDelta()
{
    ...
    std::unique_ptr<HistogramSamples> snapshot = std::make_unique<SampleVector>(unlogged_samples_->id(), bucket_ranges());
...
Histogram::Histogram(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts, const DelayedPersistentAllocation& logged_counts,
    HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
    : HistogramBase(name)
{
    DCHECK(ranges) << name;
    unlogged_samples_ = std::make_unique<PersistentSampleVector>(HashMetricName(name), ranges, meta, counts);
    logged_samples_ = std::make_unique<PersistentSampleVector>(unlogged_samples_->id(), ranges, logged_meta, logged_counts);
}
...
HistogramBase* Histogram::DeserializeInfoImpl(PickleIterator* iter)
{
    ...
    if (!ReadHistogramArguments(iter, &histogram_name, &flags, &declared_min, &declared_max, &bucket_count, &range_checksum)) {
        return nullptr;
    }

    // Find or create the local version of the histogram in this process.
    HistogramBase* histogram = Histogram::FactoryGet(histogram_name, declared_min, declared_max, bucket_count, flags);
    if (!histogram)
        return nullptr;
...
// static
HistogramBase* Histogram::FactoryGetInternal(std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
...
// static
HistogramBase* Histogram::FactoryTimeGetInternal(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    ...
    return FactoryGetInternal(name, static_cast<Sample>(minimum.InMilliseconds()), static_cast<Sample>(maximum.InMilliseconds()), bucket_count, flags);
}

// static
HistogramBase* Histogram::FactoryMicrosecondsTimeGetInternal(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    ...
    return FactoryGetInternal(name, static_cast<Sample>(minimum.InMicroseconds()), static_cast<Sample>(maximum.InMicroseconds()), bucket_count, flags);
}
...
{
    std::unique_ptr<SampleVector> samples(new SampleVector(unlogged_samples_->id(), bucket_ranges()));
...
protected:
    ...
    std::unique_ptr<HistogramBase> HeapAlloc(const BucketRanges* ranges) override
    {
...
HistogramBase* LinearHistogram::FactoryGet(std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* LinearHistogram::FactoryTimeGet(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* LinearHistogram::FactoryGet(const std::string& name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* LinearHistogram::FactoryTimeGet(const std::string& name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* LinearHistogram::FactoryGet(const char* name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetInternal(name, minimum, maximum, bucket_count, flags);
}

HistogramBase* LinearHistogram::FactoryTimeGet(const char* name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    return FactoryTimeGetInternal(name, minimum, maximum, bucket_count, flags);
}

std::unique_ptr<HistogramBase> LinearHistogram::PersistentCreate(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
{
    return WrapUnique(new LinearHistogram(name, ranges, counts, logged_counts, meta, logged_meta));
}

HistogramBase* LinearHistogram::FactoryGetWithRangeDescription(
    std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags, const DescriptionPair descriptions[])
{
    ...
    bool valid_arguments = Histogram::InspectConstructionArguments(name, &minimum, &maximum, &bucket_count);
    DCHECK(valid_arguments) << name;
    if (!valid_arguments) {
        DLOG(ERROR) << "Histogram " << name << " dropped for invalid parameters.";
        return DummyHistogram::GetInstance();
...
LinearHistogram::LinearHistogram(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
...
// static
void LinearHistogram::InitializeBucketRanges(Sample minimum, Sample maximum, BucketRanges* ranges)
{
    double min = minimum;
    double max = maximum;
    size_t bucket_count = ranges->bucket_count();
...
// static
HistogramBase* LinearHistogram::FactoryGetInternal(std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t flags)
{
    return FactoryGetWithRangeDescription(name, minimum, maximum, bucket_count, flags, nullptr);
}

// static
HistogramBase* LinearHistogram::FactoryTimeGetInternal(std::string_view name, TimeDelta minimum, TimeDelta maximum, size_t bucket_count, int32_t flags)
{
    DCHECK_LT(minimum.InMilliseconds(), std::numeric_limits<Sample>::max());
    DCHECK_LT(maximum.InMilliseconds(), std::numeric_limits<Sample>::max());
    return FactoryGetInternal(name, static_cast<Sample>(minimum.InMilliseconds()), static_cast<Sample>(maximum.InMilliseconds()), bucket_count, flags);
...
HistogramBase* LinearHistogram::DeserializeInfoImpl(PickleIterator* iter)
{
    ...
    HistogramBase* histogram = LinearHistogram::FactoryGet(histogram_name, declared_min, declared_max, bucket_count, flags);
    if (!histogram)
        return nullptr;
...
ScaledLinearHistogram::ScaledLinearHistogram(std::string_view name, Sample minimum, Sample maximum, size_t bucket_count, int32_t scale, int32_t flags)
    ...
    , scale_(scale)
{
    ...
    DCHECK_LT(1, scale);
    DCHECK_EQ(1, minimum);
    CHECK_EQ(static_cast<Sample>(bucket_count), maximum - minimum + 2) << " ScaledLinearHistogram requires buckets of size 1";
...
ScaledLinearHistogram::ScaledLinearHistogram(const std::string& name, Sample minimum, Sample maximum, size_t bucket_count, int32_t scale, int32_t flags)
    : ScaledLinearHistogram(std::string_view(name), minimum, maximum, bucket_count, scale, flags)
{
}

ScaledLinearHistogram::ScaledLinearHistogram(const char* name, Sample minimum, Sample maximum, size_t bucket_count, int32_t scale, int32_t flags)
    : ScaledLinearHistogram(std::string_view(name), minimum, maximum, bucket_count, scale, flags)
{
}
...
void ScaledLinearHistogram::AddScaledCount(Sample value, int64_t count)
{
    ...
    if (remainder > 0) {
        remainder = subtle::NoBarrier_AtomicIncrement(&remainders_[static_cast<size_t>(value)], remainder);
        ...
        if (remainder >= scale_ / 2) {
            scaled_count += 1;
            subtle::NoBarrier_AtomicIncrement(&remainders_[static_cast<size_t>(value)], -scale_);
        }
    }
...
HistogramBase* BooleanHistogram::FactoryGet(std::string_view name, int32_t flags)
{
    return FactoryGetInternal(name, flags);
}

HistogramBase* BooleanHistogram::FactoryGet(const std::string& name, int32_t flags)
{
    return FactoryGetInternal(name, flags);
}
...
std::unique_ptr<HistogramBase> BooleanHistogram::PersistentCreate(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
{
...
// static
HistogramBase* BooleanHistogram::FactoryGetInternal(std::string_view name, int32_t flags)
{
...
BooleanHistogram::BooleanHistogram(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
...
public:
    Factory(std::string_view name, const std::vector<Sample>* custom_ranges, int32_t flags)
...
HistogramBase* CustomHistogram::FactoryGet(std::string_view name, const std::vector<Sample>& custom_ranges, int32_t flags)
{
    return FactoryGetInternal(name, custom_ranges, flags);
}

HistogramBase* CustomHistogram::FactoryGet(const std::string& name, const std::vector<Sample>& custom_ranges, int32_t flags)
{
    return FactoryGetInternal(name, custom_ranges, flags);
}

HistogramBase* CustomHistogram::FactoryGet(const char* name, const std::vector<Sample>& custom_ranges, int32_t flags)
{
    return FactoryGetInternal(name, custom_ranges, flags);
}

std::unique_ptr<HistogramBase> CustomHistogram::PersistentCreate(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
{
...
// static
std::vector<Sample> CustomHistogram::ArrayToCustomEnumRanges(base::span<const Sample> values)
{
...
CustomHistogram::CustomHistogram(const char* name, const BucketRanges* ranges, const DelayedPersistentAllocation& counts,
    const DelayedPersistentAllocation& logged_counts, HistogramSamples::Metadata* meta, HistogramSamples::Metadata* logged_meta)
    : Histogram(name, ranges, counts, logged_counts, meta, logged_meta)
{
}
...
// static
HistogramBase* CustomHistogram::FactoryGetInternal(std::string_view name, const std::vector<Sample>& custom_ranges, int32_t flags)
{
...
// static
bool CustomHistogram::ValidateCustomRanges(const std::vector<Sample>& custom_ranges)
{
...
void SetSharedLastForegroundTimeForMetrics(const std::atomic<TimeTicks>* last_foreground_time_ref)
{
    g_last_foreground_time_ref.store(last_foreground_time_ref, std::memory_order_release);
}

const std::atomic<TimeTicks>* GetSharedLastForegroundTimeForMetricsForTesting()
{
...
bool OverlapsBestEffortRange(TimeTicks sample_time, TimeDelta sample_interval)
{
    ...
    auto last_foreground_time_ref = g_last_foreground_time_ref.load(std::memory_order_acquire);
    ...
    // on the result.
    auto last_foreground_time = last_foreground_time_ref->load(std::memory_order_relaxed);
    ...
    return last_foreground_time.is_null() || (sample_time - sample_interval) < last_foreground_time;
}
