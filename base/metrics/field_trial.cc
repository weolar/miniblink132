...
void WriteStringPair(Pickle* pickle, std::string_view string1, std::string_view string2)
{
...
// is_overridden.
void PickleFieldTrial(const FieldTrial::PickleState& trial_state, Pickle* pickle)
{
    ...
    FieldTrialParamAssociator::GetInstance()->GetFieldTrialParamsWithoutFallback(*trial_state.trial_name, *trial_state.group_name, &params);
...
FieldTrial::Probability GetGroupBoundaryValue(FieldTrial::Probability divisor, double entropy_value)
{
...
void AppendFieldTrialGroupToString(bool activated, std::string_view trial_name, std::string_view group_name, std::string& field_trials_string)
{
    DCHECK_EQ(std::string::npos, trial_name.find(kPersistentStringSeparator)) << " in name " << trial_name;
    DCHECK_EQ(std::string::npos, group_name.find(kPersistentStringSeparator)) << " in name " << group_name;
    ...
    base::StrAppend(&field_trials_string, { trial_name, std::string_view(&kPersistentStringSeparator, 1), group_name });
}
...
uint32_t FieldTrial::EntropyProvider::GetPseudorandomValue(uint32_t salt, uint32_t output_range) const
{
    ...
    return static_cast<uint32_t>(GetGroupBoundaryValue(static_cast<FieldTrial::Probability>(output_range), entropy_value));
}
...
bool FieldTrial::FieldTrialEntry::GetState(std::string_view& trial_name, std::string_view& group_name, bool& overridden) const
{
    PickleIterator iter = GetPickleIterator();
    return ReadHeader(iter, trial_name, group_name, overridden);
}

bool FieldTrial::FieldTrialEntry::GetParams(std::map<std::string, std::string>* params) const
{
    PickleIterator iter = GetPickleIterator();
...
PickleIterator FieldTrial::FieldTrialEntry::GetPickleIterator() const
{
    Pickle pickle = Pickle::WithUnownedBuffer(
        ...
        UNSAFE_TODO(span(GetPickledDataPtr(), checked_cast<size_t>(pickle_size))));
    return PickleIterator(pickle);
}

bool FieldTrial::FieldTrialEntry::ReadHeader(PickleIterator& iter, std::string_view& trial_name, std::string_view& group_name, bool& overridden) const
{
    return ReadStringPair(&iter, &trial_name, &group_name) && iter.ReadBool(&overridden);
}

bool FieldTrial::FieldTrialEntry::ReadStringPair(PickleIterator* iter, std::string_view* trial_name, std::string_view* group_name) const
{
...
void FieldTrial::AppendGroup(const std::string& name, Probability group_probability)
{
...
FieldTrial* FieldTrial::CreateSimulatedFieldTrial(
    std::string_view trial_name, Probability total_probability, std::string_view default_group_name, double entropy_value)
{
    ...
    return new FieldTrial(trial_name, total_probability, default_group_name, entropy_value, /*is_low_anonymity=*/false,
        /*is_overridden=*/false);
}

// static
bool FieldTrial::ParseFieldTrialsString(std::string_view trials_string, bool override_trials, std::vector<State>& entries)
{
    size_t next_item = 0;
    while (next_item < trials_string.length()) {
        ...
        size_t trial_name_end = trials_string.find(kPersistentStringSeparator, next_item);
        if (trial_name_end == trials_string.npos || next_item == trial_name_end) {
            return false;
        }
        // Find the second delimiter, or end of string.
        size_t group_name_end = trials_string.find(kPersistentStringSeparator, trial_name_end + 1);
        ...
        entry.trial_name = trials_string.substr(next_item, trial_name_end - next_item);
        entry.group_name = trials_string.substr(trial_name_end + 1, group_name_end - trial_name_end - 1);
...
// static
std::string FieldTrial::BuildFieldTrialStateString(const std::vector<State>& states)
{
    std::string result;
    for (const State& state : states) {
        AppendFieldTrialGroupToString(state.activated, state.trial_name, state.group_name, result);
    }
    return result;
}

FieldTrial::FieldTrial(std::string_view trial_name, const Probability total_probability, std::string_view default_group_name, double entropy_value,
    bool is_low_anonymity, bool is_overridden)
    ...
    , is_low_anonymity_(is_low_anonymity)
{
    ...
    DCHECK(!default_group_name_.empty()) << "Trial " << trial_name << " is missing a default group name.";
}
...
// static
FieldTrial* FieldTrialList::FactoryGetFieldTrial(std::string_view trial_name, FieldTrial::Probability total_probability, std::string_view default_group_name,
    const FieldTrial::EntropyProvider& entropy_provider, uint32_t randomization_seed, bool is_low_anonymity, bool is_overridden)
{
    ...
    double entropy_value = entropy_provider.GetEntropyForTrial(trial_name, randomization_seed);

    FieldTrial* field_trial = new FieldTrial(trial_name, total_probability, default_group_name, entropy_value, is_low_anonymity, is_overridden);
...
// static
std::vector<FieldTrial::State> FieldTrialList::GetAllFieldTrialStates(PassKey<test::ScopedFeatureList>)
{
...
void FieldTrialList::AllStatesToString(std::string* output)
{
    ...
    for (const auto& registered : global_->registered_) {
        FieldTrial::PickleState trial;
        registered.second->GetStateWhileLocked(&trial);
        AppendFieldTrialGroupToString(trial.activated, *trial.trial_name, *trial.group_name, *output);
    }
}
...
std::string FieldTrialList::AllParamsToString(EscapeDataFunc encode_data_func)
{
    ...
    for (const auto& registered : GetRegisteredTrials()) {
        ...
        DCHECK_EQ(std::string::npos, trial.group_name->find(kPersistentStringSeparator));
        std::map<std::string, std::string> params;
        if (params_associator->GetFieldTrialParamsWithoutFallback(*trial.trial_name, *trial.group_name, &params)) {
            if (params.size() > 0) {
...
// static
void FieldTrialList::GetActiveFieldTrialGroups(FieldTrial::ActiveGroups* active_groups)
{
...
std::set<std::string> FieldTrialList::GetActiveTrialsOfParentProcess()
{
    ...
    while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) != nullptr) {
        ...
        bool is_overridden;
        if (subtle::NoBarrier_Load(&entry->activated) && entry->GetState(trial_name, group_name, is_overridden)) {
...
// static
bool FieldTrialList::CreateTrialsFromString(const std::string& trials_string, bool override_trials)
{
    DCHECK(global_);
    ...
    if (!FieldTrial::ParseFieldTrialsString(trials_string, override_trials, entries)) {
        return false;
    }
...
// static
bool FieldTrialList::CreateTrialsFromFieldTrialStates(PassKey<test::ScopedFeatureList>, const std::vector<FieldTrial::State>& entries)
{
    return CreateTrialsFromFieldTrialStatesInternal(entries);
}
...
void FieldTrialList::CreateTrialsInChildProcess(const CommandLine& cmd_line)
{
    ...
    if (cmd_line.HasSwitch(switches::kFieldTrialHandle)) {
        ...
        SCOPED_CRASH_KEY_NUMBER("FieldTrialList", "SharedMemoryError", static_cast<int>(result));
...
// static
void FieldTrialList::ApplyFeatureOverridesInChildProcess(FeatureList* feature_list)
{
    CHECK(global_->create_trials_in_child_process_called_);
    // TODO(crbug.com/41403903): Change to a CHECK.
    if (global_->field_trial_allocator_) {
...
void FieldTrialList::PopulateLaunchOptionsWithFieldTrialState(
    ...
    GlobalDescriptors::Key descriptor_key, ScopedFD& descriptor_to_share,
#endif
    CommandLine* command_line, LaunchOptions* launch_options)
{
    CHECK(command_line);
    ...
    shared_memory::AddToLaunchParameters(switches::kFieldTrialHandle, global_->readonly_allocator_region_.Duplicate(),
    ...
    FeatureList::GetInstance()->GetCommandLineFeatureOverrides(&enabled_features, &disabled_features);

    if (!enabled_features.empty()) {
        command_line->AppendSwitchASCII(switches::kEnableFeatures, enabled_features);
    }
    if (!disabled_features.empty()) {
        command_line->AppendSwitchASCII(switches::kDisableFeatures, disabled_features);
    }
}
#endif // BUILDFLAG(USE_BLINK)

// static
ReadOnlySharedMemoryRegion FieldTrialList::DuplicateFieldTrialSharedMemoryForTesting()
{
    if (!global_)
...
// static
FieldTrial* FieldTrialList::CreateFieldTrial(std::string_view name, std::string_view group_name, bool is_low_anonymity, bool is_overridden)
{
    DCHECK(global_);
    ...
    field_trial = new FieldTrial(name, kTotalProbability, group_name, 0, is_low_anonymity, is_overridden);
...
void FieldTrialList::NotifyFieldTrialGroupSelection(FieldTrial* field_trial)
{
    ...
    std::vector<raw_ptr<Observer, VectorExperimental>> local_observers_including_low_anonymity;

    {
        ...
        local_observers_including_low_anonymity = global_->observers_including_low_anonymity_;
    }

    if (!field_trial->is_low_anonymity_) {
        for (Observer* observer : local_observers) {
            observer->OnFieldTrialGroupFinalized(*field_trial, field_trial->group_name_internal());
        }
    }
    ...
    int previous_num_ongoing_notify_field_trial_group_selection_calls = global_->num_ongoing_notify_field_trial_group_selection_calls_--;
...
// static
bool FieldTrialList::GetParamsFromSharedMemory(FieldTrial* field_trial, std::map<std::string, std::string>* params)
{
    DCHECK(global_);
    ...
    const FieldTrial::FieldTrialEntry* entry = global_->field_trial_allocator_->GetAsObject<FieldTrial::FieldTrialEntry>(field_trial->ref_, &allocated_size);
    CHECK(entry);

    uint64_t actual_size = sizeof(FieldTrial::FieldTrialEntry) + entry->pickle_size;
...
void FieldTrialList::ClearParamsFromSharedMemoryForTesting()
{
    ...
    while ((prev_ref = mem_iter.GetNextOfType<FieldTrial::FieldTrialEntry>()) != FieldTrialAllocator::kReferenceNull) {
        // Get the existing field trial entry in shared memory.
        const FieldTrial::FieldTrialEntry* prev_entry = allocator->GetAsObject<FieldTrial::FieldTrialEntry>(prev_ref);
        std::string_view trial_name;
        std::string_view group_name;
        bool is_overridden;
        ...
        if (prev_entry->pickle_size == pickle.size() && memcmp(prev_entry->GetPickledDataPtr(), pickle.data(), pickle.size()) == 0) {
        ...
        FieldTrial::FieldTrialEntry* new_entry = allocator->New<FieldTrial::FieldTrialEntry>(total_size);
        DCHECK(new_entry) << "Failed to allocate a new entry, likely because the allocator is "
                             "full. Consider increasing kFieldTrialAllocationSize.";
        subtle::NoBarrier_Store(&new_entry->activated, subtle::NoBarrier_Load(&prev_entry->activated));
        ...
        allocator->ChangeType(prev_ref, 0, FieldTrial::FieldTrialEntry::kPersistentTypeId,
            /*clear=*/false);
    }
...
// static
void FieldTrialList::DumpAllFieldTrialsToPersistentAllocator(PersistentMemoryAllocator* allocator)
{
    if (!global_)
        return;
    AutoLock auto_lock(global_->lock_);
...
// static
std::vector<const FieldTrial::FieldTrialEntry*> FieldTrialList::GetAllFieldTrialsFromPersistentAllocator(PersistentMemoryAllocator const& allocator)
{
...
// static
SharedMemoryError FieldTrialList::CreateTrialsFromSwitchValue(const std::string& switch_value)
{
...
// static
bool FieldTrialList::CreateTrialsFromSharedMemoryRegion(const ReadOnlySharedMemoryRegion& shm_region)
{
...
// static
bool FieldTrialList::CreateTrialsFromSharedMemoryMapping(ReadOnlySharedMemoryMapping shm_mapping)
{
    global_->field_trial_allocator_ = std::make_unique<ReadOnlySharedPersistentMemoryAllocator>(std::move(shm_mapping), 0, kAllocatorName);
    ...
    while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) != nullptr) {
        ...
        FieldTrial* trial = CreateFieldTrial(trial_name, group_name, /*is_low_anonymity=*/false, is_overridden);
...
void FieldTrialList::InstantiateFieldTrialAllocatorIfNeeded()
{
    ...
    global_->field_trial_allocator_ = std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(shm.mapping), 0, kAllocatorName);
...
// static
void FieldTrialList::AddToAllocatorWhileLocked(PersistentMemoryAllocator* allocator, FieldTrial* field_trial)
{
    ...
    size_t total_size = sizeof(FieldTrial::FieldTrialEntry) + pickle.size();
    FieldTrial::FieldTrialRef ref = allocator->Allocate(total_size, FieldTrial::FieldTrialEntry::kPersistentTypeId);
...
// static
void FieldTrialList::ActivateFieldTrialEntryWhileLocked(FieldTrial* field_trial)
{
    FieldTrialAllocator* allocator = global_->field_trial_allocator_.get();
...
// static
bool FieldTrialList::CreateTrialsFromFieldTrialStatesInternal(const std::vector<FieldTrial::State>& entries)
{
    DCHECK(global_);
...
// static
void FieldTrialList::GetActiveFieldTrialGroupsInternal(FieldTrial::ActiveGroups* active_groups, bool include_low_anonymity)
{
    ...
    for (const auto& registered : global_->registered_) {
        ...
        if ((include_low_anonymity || !trial.is_low_anonymity_) && trial.GetActiveGroup(&active_group)) {
...
// static
bool FieldTrialList::AddObserverInternal(Observer* observer, bool include_low_anonymity)
{
    if (!global_) {
        return false;
    }
...
// static
void FieldTrialList::RemoveObserverInternal(Observer* observer, bool include_low_anonymity)
{
    if (!global_) {
        return;
    }
    AutoLock auto_lock(global_->lock_);
    std::erase(include_low_anonymity ? global_->observers_including_low_anonymity_ : global_->observers_, observer);
    DCHECK_EQ(global_->num_ongoing_notify_field_trial_group_selection_calls_, 0) << "Cannot call RemoveObserver while accessing FieldTrial::group_name().";
}
