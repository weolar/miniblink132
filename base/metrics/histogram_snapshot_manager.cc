...
void HistogramSnapshotManager::PrepareDeltas(
    const std::vector<HistogramBase*>& histograms, HistogramBase::Flags flags_to_set, HistogramBase::Flags required_flags)
{
...
void HistogramSnapshotManager::SnapshotUnloggedSamples(const std::vector<HistogramBase*>& histograms, HistogramBase::Flags required_flags)
{
    DCHECK(!unlogged_samples_snapshot_taken_);
    unlogged_samples_snapshot_taken_ = true;
    for (HistogramBase* const histogram : histograms) {
        if (histogram->HasFlags(required_flags)) {
            const HistogramSnapshotPair& histogram_snapshot_pair = histograms_and_snapshots_.emplace_back(histogram, histogram->SnapshotUnloggedSamples());
            PrepareSamples(histogram_snapshot_pair.first, *histogram_snapshot_pair.second);
        }
    }
}
...
void HistogramSnapshotManager::PrepareFinalDelta(const HistogramBase* histogram)
{
    std::unique_ptr<HistogramSamples> samples = histogram->SnapshotFinalDelta();
    PrepareSamples(histogram, *samples);
}

void HistogramSnapshotManager::PrepareSamples(const HistogramBase* histogram, const HistogramSamples& samples)
{
    DCHECK(histogram_flattener_);
    ...
    if (corruption) {
        DLOG(ERROR) << "Histogram: \"" << histogram->histogram_name() << "\" has data corruption: " << corruption;
