...
template <typename T> class IteratorTemplate : public SampleCountIterator {
public:
...
template <> SampleVectorIterator::~IteratorTemplate() = default;

// Get() for an iterator of a SampleVector.
template <> void SampleVectorIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count)
{
    DCHECK(!Done());
...
using ExtractingSampleVectorIterator = IteratorTemplate<HistogramBase::AtomicCount>;

template <> ExtractingSampleVectorIterator::~IteratorTemplate()
{
...
template <> void ExtractingSampleVectorIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count)
{
    DCHECK(!Done());
    *min = bucket_ranges_->range(index_);
    *max = strict_cast<int64_t>(bucket_ranges_->range(index_ + 1));
...
SampleVectorBase::SampleVectorBase(uint64_t id, std::unique_ptr<Metadata> meta, const BucketRanges* bucket_ranges)
...
void SampleVectorBase::Accumulate(Sample value, Count count)
{
    ...
    Count new_bucket_count = subtle::NoBarrier_AtomicIncrement(&counts_at(bucket_index), count);
    IncreaseSumAndCount(strict_cast<int64_t>(count) * value, count);

    // TODO(bcwhite) Remove after crbug.com/682680.
    Count old_bucket_count = new_bucket_count - count;
    bool record_negative_sample = (new_bucket_count >= 0) != (old_bucket_count >= 0) && count > 0;
...
std::unique_ptr<SampleCountIterator> SampleVectorBase::Iterator() const
{
    ...
    if (sample.count != 0) {
        ...
        if (sample.bucket >= bucket_ranges_->bucket_count()) {
            ...
            return std::make_unique<SampleVectorIterator>(base::span<const HistogramBase::AtomicCount>(), bucket_ranges_);
        }

        return std::make_unique<SingleSampleIterator>(bucket_ranges_->range(sample.bucket), bucket_ranges_->range(sample.bucket + 1), sample.count,
            sample.bucket,
...
std::unique_ptr<SampleCountIterator> SampleVectorBase::ExtractingIterator()
{
    ...
    if (sample.count != 0) {
        ...
        if (sample.bucket >= bucket_ranges_->bucket_count()) {
            ...
            return std::make_unique<ExtractingSampleVectorIterator>(base::span<HistogramBase::AtomicCount>(), bucket_ranges_);
        }
    ...
    // Handle the multi-sample case.
    if (counts().has_value() || MountExistingCountsStorage()) {
        return std::make_unique<ExtractingSampleVectorIterator>(*counts(), bucket_ranges_);
    }
...
bool SampleVectorBase::AddSubtractImpl(SampleCountIterator* iter, HistogramSamples::Operator op)
{
    ...
    if (!counts().has_value()) {
        if (iter->Done()) {
            ...
            if (single_sample().Accumulate(dest_index, op == HistogramSamples::ADD ? count : -count)) {
    ...
    while (true) {
        ...
        subtle::NoBarrier_AtomicIncrement(&counts_at(dest_index), op == HistogramSamples::ADD ? count : -count);
        if (iter->Done()) {
            return true;
        }
...
size_t SampleVectorBase::GetDestinationBucketIndexAndCount(SampleCountIterator& iter, HistogramBase::Count* count)
{
    ...
    if (bucket_index >= counts_size() || min != bucket_ranges_->range(bucket_index) || max != bucket_ranges_->range(bucket_index + 1)) {
        return SIZE_MAX;
    }
    return bucket_index;
}
...
bool SampleVector::IsDefinitelyEmpty() const
{
    ...
    return HistogramSamples::IsDefinitelyEmpty() && !sample.IsDisabled() && sample.Load().count == 0;
}
...
std::string SampleVector::GetAsciiHeader(std::string_view histogram_name, int32_t flags) const
{
    Count sample_count = TotalCount();
    std::string output;
    StrAppend(&output, { "Histogram: ", histogram_name, " recorded ", NumberToString(sample_count), " samples" });
    if (sample_count == 0) {
        DCHECK_EQ(sum(), 0);
    } else {
...
void SampleVector::WriteAsciiBucketContext(int64_t past, Count current, int64_t remaining, uint32_t current_bucket_index, std::string* output) const
{
...
span<HistogramBase::AtomicCount> SampleVector::CreateCountsStorageWhileLocked()
{
    local_counts_.resize(counts_size());
    return span<HistogramBase::AtomicCount>(local_counts_.data(), local_counts_.size());
}

PersistentSampleVector::PersistentSampleVector(uint64_t id, const BucketRanges* bucket_ranges, Metadata* meta, const DelayedPersistentAllocation& counts)
    : SampleVectorBase(id, meta, bucket_ranges)
    , persistent_counts_(counts)
{
...
bool PersistentSampleVector::MountExistingCountsStorage() const
{
    ...
    span<HistogramBase::AtomicCount> mem = persistent_counts_.Get<HistogramBase::AtomicCount>();
    if (mem.empty()) {
        return false;
    }
...
span<HistogramBase::AtomicCount> PersistentSampleVector::CreateCountsStorageWhileLocked()
{
    span<HistogramBase::AtomicCount> mem = persistent_counts_.Get<HistogramBase::AtomicCount>();
    if (mem.empty()) {
