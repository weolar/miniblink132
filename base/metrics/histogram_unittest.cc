...
protected:
    ...
    {
        GlobalHistogramAllocator::CreateWithLocalMemory(kAllocatorMemorySize, 0, "HistogramAllocatorTest");
...
TEST_P(HistogramTest, BasicTest)
{
    ...
    HistogramBase* histogram = Histogram::FactoryGet("TestHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    EXPECT_TRUE(histogram);

    HistogramBase* linear_histogram = LinearHistogram::FactoryGet("TestLinearHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    EXPECT_TRUE(linear_histogram);

    std::vector<int> custom_ranges;
    custom_ranges.push_back(1);
    custom_ranges.push_back(5);
    HistogramBase* custom_histogram = CustomHistogram::FactoryGet("TestCustomHistogram", custom_ranges, HistogramBase::kNoFlags);
...
TEST_P(HistogramTest, NameMatchTest)
{
    ...
    LOCAL_HISTOGRAM_PERCENTAGE("DuplicatedHistogram", 10);
    HistogramBase* histogram = LinearHistogram::FactoryGet("DuplicatedHistogram", 1, 101, 102, HistogramBase::kNoFlags);
...
{
    HistogramBase* histogram = Histogram::FactoryGet("DeltaHistogram", 1, 64, 8, HistogramBase::kNoFlags);
    histogram->Add(1);
    histogram->Add(10);
    histogram->Add(50);
...
TEST_P(HistogramTest, UnloggedSamplesTest)
{
    ...
    std::unique_ptr<HistogramSamples> samples = histogram->SnapshotUnloggedSamples();
    EXPECT_EQ(3, samples->TotalCount());
    EXPECT_EQ(1, samples->GetCount(1));
    EXPECT_EQ(1, samples->GetCount(10));
    EXPECT_EQ(1, samples->GetCount(50));
...
{
    HistogramBase* histogram = Histogram::FactoryGet("FinalDeltaHistogram", 1, 64, 8, HistogramBase::kNoFlags);
    histogram->Add(1);
    histogram->Add(10);
    histogram->Add(50);
...
TEST_P(HistogramTest, ExponentialRangesTest)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("Histogram", 1, 64, 8, HistogramBase::kNoFlags));
    ...
    // Check the corresponding Histogram will use the correct ranges.
    Histogram* histogram2 = static_cast<Histogram*>(Histogram::FactoryGet("Histogram2", 1, 32, 15, HistogramBase::kNoFlags));
...
TEST_P(HistogramTest, LinearRangesTest)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(LinearHistogram::FactoryGet("Linear", 1, 7, 8, HistogramBase::kNoFlags));
    EXPECT_TRUE(ranges.Equals(histogram->bucket_ranges()));
    ...
    // The correspoding LinearHistogram should use the correct ranges.
    Histogram* histogram2 = static_cast<Histogram*>(LinearHistogram::FactoryGet("Linear2", 1, 6, 5, HistogramBase::kNoFlags));
    EXPECT_TRUE(ranges2.Equals(histogram2->bucket_ranges()));
}
...
TEST_P(HistogramTest, SingleValueEnumerationHistogram)
{
    ...
    HistogramBase* histogram = LinearHistogram::FactoryGet("SingleValueEnum", 1, 1, 2, HistogramBase::kNoFlags);
    EXPECT_TRUE(histogram);
...
TEST_P(HistogramTest, ArrayToCustomEnumRangesTest)
{
    ...
    std::vector<HistogramBase::Sample> ranges_vec = CustomHistogram::ArrayToCustomEnumRanges(ranges);
...
TEST_P(HistogramTest, CustomHistogramTest)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(CustomHistogram::FactoryGet("TestCustomHistogram1", custom_ranges, HistogramBase::kNoFlags));
    ...
    custom_ranges.push_back(2);
    custom_ranges.push_back(1);
    histogram = static_cast<Histogram*>(CustomHistogram::FactoryGet("TestCustomHistogram2", custom_ranges, HistogramBase::kNoFlags));
    ...
    custom_ranges.push_back(1);
    custom_ranges.push_back(4);
    histogram = static_cast<Histogram*>(CustomHistogram::FactoryGet("TestCustomHistogram3", custom_ranges, HistogramBase::kNoFlags));
    ranges = histogram->bucket_ranges();
    ASSERT_EQ(4u, ranges->size());
    EXPECT_EQ(0, ranges->range(0));
    EXPECT_EQ(1, ranges->range(1));
...
TEST_P(HistogramTest, CustomHistogramWithOnly2Buckets)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(CustomHistogram::FactoryGet("2BucketsCustomHistogram", custom_ranges, HistogramBase::kNoFlags));
    const BucketRanges* ranges = histogram->bucket_ranges();
...
TEST_P(HistogramTest, AddCountTest)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("AddCountHistogram", 10, 100, kBucketCount, HistogramBase::kNoFlags));
...
{
    const size_t kBucketCount = 50;
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("AddCountHistogram", 10, 1000000000, kBucketCount, HistogramBase::kNoFlags));
...
TEST_P(HistogramTest, AddCount_LargeCountsDontOverflow)
{
    ...
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("AddCountHistogram", 10, 50, kBucketCount, HistogramBase::kNoFlags));
...
{
    const size_t kBucketCount = 50;
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("Bounded", 10, 100, kBucketCount, HistogramBase::kNoFlags));
    ...
    Histogram* test_custom_histogram
        = static_cast<Histogram*>(CustomHistogram::FactoryGet("TestCustomRangeBoundedHistogram", custom_ranges, HistogramBase::kNoFlags));
    ...
    // Verify they landed in the underflow, and overflow buckets.
    std::unique_ptr<SampleVector> custom_samples = test_custom_histogram->SnapshotAllSamples();
...
TEST_P(HistogramTest, CorruptSampleCounts)
{
    ...
    EXPECT_EQ(HistogramBase::NO_INCONSISTENCIES, histogram->FindCorruption(*snapshot));
    EXPECT_EQ(2, snapshot->redundant_count());
    EXPECT_EQ(2, snapshot->TotalCount());

    // Sample count won't match redundant count.
    snapshot->counts().value()[3u] += 100;
    EXPECT_EQ(HistogramBase::COUNT_LOW_ERROR, histogram->FindCorruption(*snapshot));
    snapshot->counts().value()[2u] -= 200;
    EXPECT_EQ(HistogramBase::COUNT_HIGH_ERROR, histogram->FindCorruption(*snapshot));
...
TEST_P(HistogramTest, CorruptBucketBounds)
{
    ...
    EXPECT_EQ(HistogramBase::BUCKET_ORDER_ERROR | HistogramBase::RANGE_CHECKSUM_ERROR, histogram->FindCorruption(*snapshot));
    ...
    EXPECT_EQ(HistogramBase::RANGE_CHECKSUM_ERROR, histogram->FindCorruption(*snapshot));
...
{
    Histogram* histogram = static_cast<Histogram*>(Histogram::FactoryGet("Histogram", 1, 64, 8, HistogramBase::kIPCSerializationSourceFlag));
    Pickle pickle;
    ...
    EXPECT_EQ(HistogramBase::kIPCSerializationSourceFlag, flag & ~HistogramBase::kIsPersistent);
...
TEST_P(HistogramTest, CustomHistogramSerializeInfo)
{
    ...
    HistogramBase* custom_histogram = CustomHistogram::FactoryGet("TestCustomRangeBoundedHistogram", custom_ranges, HistogramBase::kNoFlags);
    Pickle pickle;
    ...
    uint32_t bucket_count;
    uint32_t ui32;
    EXPECT_TRUE(iter.ReadInt(&i) && iter.ReadString(&s) && iter.ReadInt(&i) && iter.ReadInt(&i) && iter.ReadInt(&i) && iter.ReadUInt32(&bucket_count)
        && iter.ReadUInt32(&ui32));
...
{
    HistogramBase* histogram = Histogram::FactoryGet("BadConstruction", 0, 100, 8, HistogramBase::kNoFlags);
    EXPECT_TRUE(histogram->HasConstructionArguments(1, 100, 8));

    // Try to get the same histogram name with different arguments.
    HistogramBase* bad_histogram = Histogram::FactoryGet("BadConstruction", 0, 100, 7, HistogramBase::kNoFlags);
    EXPECT_EQ(DummyHistogram::GetInstance(), bad_histogram);
    bad_histogram = Histogram::FactoryGet("BadConstruction", 0, 99, 8, HistogramBase::kNoFlags);
    EXPECT_EQ(DummyHistogram::GetInstance(), bad_histogram);

    HistogramBase* linear_histogram = LinearHistogram::FactoryGet("BadConstructionLinear", 0, 100, 8, HistogramBase::kNoFlags);
    EXPECT_TRUE(linear_histogram->HasConstructionArguments(1, 100, 8));

    // Try to get the same histogram name with different arguments.
    bad_histogram = LinearHistogram::FactoryGet("BadConstructionLinear", 0, 100, 7, HistogramBase::kNoFlags);
    EXPECT_EQ(DummyHistogram::GetInstance(), bad_histogram);
    bad_histogram = LinearHistogram::FactoryGet("BadConstructionLinear", 10, 100, 8, HistogramBase::kNoFlags);
    EXPECT_EQ(DummyHistogram::GetInstance(), bad_histogram);
}
...
TEST_P(HistogramTest, FactoryTime)
{
    ...
    for (int i = 0; i < kTestCreateCount; ++i) {
        histogram_names.push_back(StringPrintf("TestHistogram.%d", i % kTestCreateCount));
    }

    // Calculate cost of creating histograms.
    TimeTicks create_start = TimeTicks::Now();
    for (int i = 0; i < kTestCreateCount; ++i) {
        Histogram::FactoryGet(histogram_names[i], 1, 100, 10, HistogramBase::kNoFlags);
    }
    TimeDelta create_ticks = TimeTicks::Now() - create_start;
    int64_t create_ms = create_ticks.InMilliseconds();

    VLOG(1) << kTestCreateCount << " histogram creations took " << create_ms << "ms or about " << (create_ms * 1000000) / kTestCreateCount << "ns each.";
    ...
    for (int i = 0; i < kTestLookupCount; ++i) {
        ...
        Histogram::FactoryGet(histogram_names[index], 1, 100, 10, HistogramBase::kNoFlags);
    }
    TimeDelta lookup_ticks = TimeTicks::Now() - lookup_start;
    int64_t lookup_ms = lookup_ticks.InMilliseconds();

    VLOG(1) << kTestLookupCount << " histogram lookups took " << lookup_ms << "ms or about " << (lookup_ms * 1000000) / kTestLookupCount << "ns each.";

    // Calculate cost of accessing histograms.
    HistogramBase* histogram = Histogram::FactoryGet(histogram_names[0], 1, 100, 10, HistogramBase::kNoFlags);
    ...
    VLOG(1) << kTestAddCount << " histogram adds took " << add_ms << "ms or about " << (add_ms * 1000000) / kTestAddCount << "ns each.";
}
...
TEST_P(HistogramTest, ScaledLinearHistogram)
{
    ...
    std::unique_ptr<SampleVector> samples = SnapshotAllSamples(static_cast<Histogram*>(scaled.histogram()));
...
{
    HistogramBase* histogram = Histogram::FactoryGet("BadRanges", 0, HistogramBase::kSampleType_MAX, 8, HistogramBase::kNoFlags);
    EXPECT_TRUE(histogram->HasConstructionArguments(1, HistogramBase::kSampleType_MAX - 1, 8));

    HistogramBase* linear_histogram = LinearHistogram::FactoryGet("BadRangesLinear", 0, HistogramBase::kSampleType_MAX, 8, HistogramBase::kNoFlags);
    EXPECT_TRUE(linear_histogram->HasConstructionArguments(1, HistogramBase::kSampleType_MAX - 1, 8));

    std::vector<int> custom_ranges;
    custom_ranges.push_back(0);
    custom_ranges.push_back(5);
    Histogram* custom_histogram = static_cast<Histogram*>(CustomHistogram::FactoryGet("BadRangesCustom", custom_ranges, HistogramBase::kNoFlags));
    ...
    EXPECT_DEATH_IF_SUPPORTED(CustomHistogram::FactoryGet("BadRangesCustom2", custom_ranges, HistogramBase::kNoFlags), "");

    // CustomHistogram needs at least 1 valid range.
    custom_ranges.clear();
    custom_ranges.push_back(0);
    EXPECT_DEATH_IF_SUPPORTED(CustomHistogram::FactoryGet("BadRangesCustom3", custom_ranges, HistogramBase::kNoFlags), "");
}
...
TEST_P(HistogramTest, ExpiredHistogramTest)
{
    ...
    HistogramBase* expired = Histogram::FactoryGet(kExpiredHistogramName, 1, 1000, 10, HistogramBase::kNoFlags);
    ASSERT_TRUE(expired);
    expired->Add(5);
    expired->Add(500);
    ...
    HistogramBase* linear_expired = LinearHistogram::FactoryGet(kExpiredHistogramName, 1, 1000, 10, HistogramBase::kNoFlags);
    ...
    ScaledLinearHistogram scaled_linear_expired(kExpiredHistogramName, 1, 5, 6, 100, HistogramBase::kNoFlags);
    ...
    custom_ranges.push_back(1);
    custom_ranges.push_back(5);
    HistogramBase* custom_expired = CustomHistogram::FactoryGet(kExpiredHistogramName, custom_ranges, HistogramBase::kNoFlags);
    ...
    HistogramBase* valid = Histogram::FactoryGet("ValidHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    ASSERT_TRUE(valid);
    valid->Add(5);
    valid->Add(500);
    ...
    HistogramBase* linear_valid = LinearHistogram::FactoryGet("LinearHistogram", 1, 1000, 10, HistogramBase::kNoFlags);
    ...
    HistogramBase* custom_valid = CustomHistogram::FactoryGet("CustomHistogram", custom_ranges, HistogramBase::kNoFlags);
...
TEST_P(HistogramTest, CheckGetCountAndBucketData)
{
    ...
    const CountAndBucketData count_and_data_bucket = GetCountAndBucketData(histogram);
...
{
    HistogramBase* histogram = LinearHistogram::FactoryGet("AsciiOut", /*minimum=*/1, /*maximum=*/10,
    ...
    const char kOutputFormatRe[] = R"(Histogram: AsciiOut recorded 5 samples, mean = 4\.0.*\n)"
                                   R"(0  \.\.\. \n)"
                                   R"(4  -+O \s* \(5 = 100\.0%\) \{0\.0%\}\n)"
                                   R"(7  \.\.\. \n)";
...
{
    HistogramBase* histogram = LinearHistogram::FactoryGet("HTMLOut", /*minimum=*/1, /*maximum=*/10,
        /*bucket_count=*/5, HistogramBase::kNoFlags);
    histogram->AddCount(/*sample=*/4, /*value=*/5);
    ...
    const char kOutputHeaderFormatRe[] = R"(Histogram: HTMLOut recorded 5 samples, mean = 4\.0.*)";
...
TEST_P(HistogramTest, ToGraphDictNormalize)
{
    ...
    HistogramBase* histogram = LinearHistogram::FactoryGet("AsciiOut", /*minimum=*/1, /*maximum=*/100,
    ...
    const char kOutputHeaderFormatRe[] = R"(Histogram: AsciiOut recorded 120 samples, mean = 4\.3.*)";
    const char kOutputBodyFormatRe[] = R"(0  \.\.\. \n)"
                                       R"(4  ---------------------------------------------------)"
                                       R"(---------------------O \(80 = 66\.7%\) \{0\.0%\}\n)"
                                       R"(5  ----------------)"
