...
HistogramBase::CountAndBucketData::CountAndBucketData(Count count, int64_t sum, Value::List buckets)
    : count(count)
    , sum(sum)
...
HistogramBase::CountAndBucketData::CountAndBucketData(CountAndBucketData&& other) = default;

HistogramBase::CountAndBucketData& HistogramBase::CountAndBucketData::operator=(CountAndBucketData&& other) = default;
...
{
    DCHECK_EQ(std::string_view(histogram_name()), name) << "Provided histogram name doesn't match instance name. Are you using a "
...
void HistogramBase::WriteJSON(std::string* output, JSONVerbosityLevel verbosity_level) const
{
...
{
    StatisticsRecorder::GlobalSampleCallback global_sample_callback = StatisticsRecorder::global_sample_callback();
    ...
    StatisticsRecorder::FindAndRunHistogramCallbacks(base::PassKey<HistogramBase>(), histogram_name(), name_hash(), sample);
}
...
void HistogramBase::WriteAsciiBucketGraph(double x_count, int line_length, std::string* output) const
{
...
const std::string HistogramBase::GetSimpleAsciiBucketRange(Sample sample) const
{
    return StringPrintf("%d", sample);
}

void HistogramBase::WriteAsciiBucketValue(Count current, double scaled_sum, std::string* output) const
{
