...
std::unique_ptr<BucketRanges> CreateRangesFromData(HistogramBase::Sample* ranges_data, uint32_t ranges_checksum, size_t count)
{
...
bool MergeSamplesToExistingHistogram(HistogramBase* existing, const HistogramBase* histogram, std::unique_ptr<HistogramSamples> samples)
{
    ...
    if (existing_type == HistogramType::HISTOGRAM || existing_type == HistogramType::LINEAR_HISTOGRAM || existing_type == HistogramType::BOOLEAN_HISTOGRAM
        || existing_type == HistogramType::CUSTOM_HISTOGRAM) {
        // Only numeric histograms make use of BucketRanges.
        const BucketRanges* existing_buckets = static_cast<const Histogram*>(existing)->bucket_ranges();
        const BucketRanges* histogram_buckets = static_cast<const Histogram*>(histogram)->bucket_ranges();
...
PersistentSparseHistogramDataManager::PersistentSparseHistogramDataManager(PersistentMemoryAllocator* allocator)
...
std::unique_ptr<PersistentSampleMapRecords> PersistentSparseHistogramDataManager::CreateSampleMapRecords(uint64_t id)
{
    base::AutoLock auto_lock(lock_);
    return std::make_unique<PersistentSampleMapRecords>(this, id, GetSampleMapRecordsWhileLocked(id));
}

std::vector<PersistentSparseHistogramDataManager::ReferenceAndSample>* PersistentSparseHistogramDataManager::GetSampleMapRecordsWhileLocked(uint64_t id)
{
...
std::vector<PersistentMemoryAllocator::Reference> PersistentSparseHistogramDataManager::LoadRecords(
    PersistentSampleMapRecords* sample_map_records, std::optional<HistogramBase::Sample> until_value)
{
    ...
    // different call.
    std::vector<PersistentSparseHistogramDataManager::ReferenceAndSample>& found_records = *sample_map_records->records_;
    ...
    while (!found || new_records < kMinimumNumberToLoad) {
        ...
        PersistentMemoryAllocator::Reference ref = PersistentSampleMap::GetNextPersistentRecord(record_iterator_, &found_id, &value);
        ...
        if (found_id == match_id) {
            ...
            std::vector<ReferenceAndSample>* samples = GetSampleMapRecordsWhileLocked(found_id);
            CHECK(samples);
    ...
    auto new_found_records = base::span<PersistentSparseHistogramDataManager::ReferenceAndSample>(
        found_records.data(), found_records.size()).subspan(/*offset=*/sample_map_records->seen_);
...
PersistentSampleMapRecords::PersistentSampleMapRecords(
    PersistentSparseHistogramDataManager* data_manager, uint64_t sample_map_id, std::vector<PersistentSparseHistogramDataManager::ReferenceAndSample>* records)
...
std::vector<PersistentMemoryAllocator::Reference> PersistentSampleMapRecords::GetNextRecords(std::optional<HistogramBase::Sample> until_value)
{
...
PersistentMemoryAllocator::Reference PersistentSampleMapRecords::CreateNew(HistogramBase::Sample value)
{
    return PersistentSampleMap::CreatePersistentRecord(data_manager_->allocator_, sample_map_id_, value);
}
...
struct PersistentHistogramAllocator::PersistentHistogramData {
    ...
    static constexpr size_t kExpectedInstanceSize = 40 + 2 * HistogramSamples::Metadata::kExpectedInstanceSize;
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::Iterator::GetNextWithIgnore(Reference ignore)
{
...
PersistentHistogramAllocator::PersistentHistogramAllocator(std::unique_ptr<PersistentMemoryAllocator> memory)
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::GetHistogram(Reference ref)
{
    ...
    size_t length = 0;
    PersistentHistogramData* data = memory_allocator_->GetAsObject<PersistentHistogramData>(ref, &length);
    ...
    // unset/invalid).
    if (!data || data->name[0] == '\0' || reinterpret_cast<char*>(data)[length - 1] != '\0' || data->samples_metadata.id == 0 || data->logged_metadata.id == 0
        ||
        // Note: Sparse histograms use `id + 1` in `logged_metadata`.
        (data->logged_metadata.id != data->samples_metadata.id && data->logged_metadata.id != data->samples_metadata.id + 1) ||
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::AllocateHistogram(
    HistogramType histogram_type, std::string_view name, int minimum, int maximum, const BucketRanges* bucket_ranges, int32_t flags, Reference* ref_ptr)
{
    ...
    PersistentHistogramData* histogram_data = memory_allocator_->New<PersistentHistogramData>(offsetof(PersistentHistogramData, name) + name.size() + 1);
    if (histogram_data) {
    ...
    if (histogram_type != SPARSE_HISTOGRAM) {
        ...
        if (!ranges_ref) {
            ...
            if (ranges_ref) {
                HistogramBase::Sample* ranges_data = memory_allocator_->GetAsArray<HistogramBase::Sample>(ranges_ref, kTypeIdRangesArray, ranges_count);
                if (ranges_data) {
    ...
    if (histogram_data) {
        ...
        PersistentMemoryAllocator::Reference histogram_ref = memory_allocator_->GetAsReference(histogram_data);
        if (ref_ptr != nullptr)
...
void PersistentHistogramAllocator::FinalizeHistogram(Reference ref, bool registered)
{
    if (registered) {
        ...
        memory_allocator_->ChangeType(ref, 0, PersistentHistogramData::kPersistentTypeId,
            /*clear=*/false);
    }
}

bool PersistentHistogramAllocator::MergeHistogramDeltaToStatisticsRecorder(HistogramBase* histogram)
{
    DCHECK(histogram);
    ...
    return MergeSamplesToExistingHistogram(existing, histogram, std::move(samples));
}

bool PersistentHistogramAllocator::MergeHistogramFinalDeltaToStatisticsRecorder(const HistogramBase* histogram)
{
    DCHECK(histogram);
...
std::unique_ptr<PersistentSampleMapRecords> PersistentHistogramAllocator::CreateSampleMapRecords(uint64_t id)
{
    return sparse_histogram_data_manager_.CreateSampleMapRecords(id);
}

void PersistentHistogramAllocator::CreateTrackingHistograms(std::string_view name)
{
...
void PersistentHistogramAllocator::SetRangesManager(RangesManager* ranges_manager)
{
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::CreateHistogram(PersistentHistogramData* histogram_data_ptr)
{
    ...
    if (histogram_data_ptr->histogram_type == SPARSE_HISTOGRAM) {
        std::unique_ptr<HistogramBase> histogram
            = SparseHistogram::PersistentCreate(this, histogram_data_ptr->name, &histogram_data_ptr->samples_metadata, &histogram_data_ptr->logged_metadata);
        DCHECK(histogram);
    ...
    HistogramBase::Sample* ranges_data
        = memory_allocator_->GetAsArray<HistogramBase::Sample>(histogram_ranges_ref, kTypeIdRangesArray, PersistentMemoryAllocator::kSizeAny, &allocated_bytes);
    ...
    if (!ranges_data || histogram_bucket_count < 2 || histogram_bucket_count >= max_buckets || allocated_bytes < required_bytes) {
        return nullptr;
    }

    std::unique_ptr<const BucketRanges> created_ranges = CreateRangesFromData(ranges_data, histogram_ranges_checksum, histogram_bucket_count + 1);
    if (!created_ranges || created_ranges->size() != histogram_bucket_count + 1 || created_ranges->range(1) != histogram_minimum
        || created_ranges->range(histogram_bucket_count - 1) != histogram_maximum) {
        return nullptr;
    }
    const BucketRanges* ranges;
    if (ranges_manager_) {
        ranges = ranges_manager_->GetOrRegisterCanonicalRanges(created_ranges.get());
        if (ranges == created_ranges.get()) {
            // `ranges_manager_` took ownership of `created_ranges`.
            created_ranges.release();
        }
    } else {
        ranges = StatisticsRecorder::RegisterOrDeleteDuplicateRanges(created_ranges.release());
    }
    ...
    PersistentMemoryAllocator::Reference counts_ref = histogram_data_ptr->counts_ref.load(std::memory_order_acquire);
    if (counts_ref != 0 && !memory_allocator_->GetAsArray<uint8_t>(counts_ref, kTypeIdCountsArray, counts_bytes)) {
        return nullptr;
    }
    ...
    DelayedPersistentAllocation counts_data(memory_allocator_.get(), &histogram_data_ptr->counts_ref, kTypeIdCountsArray, counts_bytes);
    ...
    DelayedPersistentAllocation logged_data(memory_allocator_.get(), &histogram_data_ptr->counts_ref, kTypeIdCountsArray, counts_bytes, counts_bytes / 2);
    ...
    case HISTOGRAM:
        histogram
            = Histogram::PersistentCreate(name, ranges, counts_data, logged_data, &histogram_data_ptr->samples_metadata, &histogram_data_ptr->logged_metadata);
        DCHECK(histogram);
        break;
    case LINEAR_HISTOGRAM:
        histogram = LinearHistogram::PersistentCreate(
            name, ranges, counts_data, logged_data, &histogram_data_ptr->samples_metadata, &histogram_data_ptr->logged_metadata);
        DCHECK(histogram);
        break;
...
HistogramBase* PersistentHistogramAllocator::GetOrCreateStatisticsRecorderHistogram(const HistogramBase* histogram)
{
...
// static
void GlobalHistogramAllocator::CreateWithPersistentMemory(void* base, size_t size, size_t page_size, uint64_t id, std::string_view name)
{
    Set(new GlobalHistogramAllocator(std::make_unique<PersistentMemoryAllocator>(base, size, page_size, id, name, PersistentMemoryAllocator::kReadWrite)));
}

// static
void GlobalHistogramAllocator::CreateWithLocalMemory(size_t size, uint64_t id, std::string_view name)
{
    Set(new GlobalHistogramAllocator(std::make_unique<LocalPersistentMemoryAllocator>(size, id, name)));
}

#if !BUILDFLAG(IS_NACL)
// static
bool GlobalHistogramAllocator::CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, std::string_view name, bool exclusive_write)
{
    uint32_t flags = File::FLAG_OPEN_ALWAYS | File::FLAG_WIN_SHARE_DELETE | File::FLAG_READ | File::FLAG_WRITE;
    if (exclusive_write)
    ...
    if (file_created) {
        success = mmfile->Initialize(std::move(file), { 0, size }, MemoryMappedFile::READ_WRITE_EXTEND);
    } else {
    ...
    Set(new GlobalHistogramAllocator(std::make_unique<FilePersistentMemoryAllocator>(std::move(mmfile), 0, id, name, PersistentMemoryAllocator::kReadWrite)));
    Get()->SetPersistentLocation(file_path);
    return true;
}

// static
bool GlobalHistogramAllocator::CreateWithActiveFile(
    const FilePath& base_path, const FilePath& active_path, const FilePath& spare_path, size_t size, uint64_t id, std::string_view name)
{
...
// static
bool GlobalHistogramAllocator::CreateWithActiveFileInDir(const FilePath& dir, size_t size, uint64_t id, std::string_view name)
{
...
// static
FilePath GlobalHistogramAllocator::ConstructFilePath(const FilePath& dir, std::string_view name)
{
    return dir.AppendASCII(name).AddExtension(PersistentMemoryAllocator::kFileExtension);
}

// static
FilePath GlobalHistogramAllocator::ConstructFilePathForActiveFile(const FilePath& dir, std::string_view name)
{
    return ConstructFilePath(dir, std::string(name) + "-active");
}

// static
FilePath GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir, std::string_view name, base::Time stamp, ProcessId pid)
{
    return ConstructFilePath(
        dir, StringPrintf("%.*s-%lX-%lX", static_cast<int>(name.length()), name.data(), static_cast<long>(stamp.ToTimeT()), static_cast<long>(pid)));
}

// static
FilePath GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir, std::string_view name)
{
    return ConstructFilePathForUploadDir(dir, name, Time::Now(), GetCurrentProcId());
}

// static
bool GlobalHistogramAllocator::ParseFilePath(const FilePath& path, std::string* out_name, Time* out_stamp, ProcessId* out_pid)
{
    std::string filename = path.BaseName().AsUTF8Unsafe();
    std::vector<std::string_view> parts = base::SplitStringPiece(filename, "-.", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    if (parts.size() != 4)
        return false;
...
bool GlobalHistogramAllocator::CreateSpareFile(const FilePath& spare_path, size_t size)
{
    ...
    bool success;
    {
        File spare_file(temp_spare_path, File::FLAG_CREATE_ALWAYS | File::FLAG_READ | File::FLAG_WRITE);
        success = spare_file.IsValid();

        if (success) {
            MemoryMappedFile mmfile;
            success = mmfile.Initialize(std::move(spare_file), { 0, size }, MemoryMappedFile::READ_WRITE_EXTEND);
        }
    }
...
// static
void GlobalHistogramAllocator::CreateWithSharedMemoryRegion(const UnsafeSharedMemoryRegion& region)
{
    CHECK_EQ(Get(), nullptr) << "Histogram allocator has already been created";

    base::WritableSharedMemoryMapping mapping = region.Map();
    if (!mapping.IsValid() || !WritableSharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(mapping)) {
        DVLOG(1) << "Shared memory region is invalid or unacceptable.";
        return;
    }

    DVLOG(1) << "Global histogram allocator initialized.";
    Set(new GlobalHistogramAllocator(std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(mapping), 0, std::string_view())));
}
...
void GlobalHistogramAllocator::Set(GlobalHistogramAllocator* allocator)
{
    ...
    subtle::Release_Store(&g_histogram_allocator, reinterpret_cast<intptr_t>(allocator));
    ...
    if (histogram_count != 0) {
        DVLOG(1) << histogram_count << " histogram(s) created before persistence was enabled.";

        if (allocator && allocator->Name() && allocator->Name()[0]) {
            UmaHistogramCounts100(StrCat({ "UMA.PersistentAllocator.EarlyHistograms.", allocator->Name() }), static_cast<int>(histogram_count));
        }
    }
}
...
{
    return reinterpret_cast<GlobalHistogramAllocator*>(subtle::Acquire_Load(&g_histogram_allocator));
}
...
bool GlobalHistogramAllocator::WriteToPersistentLocation()
{
    ...
    if (!ImportantFileWriter::WriteFileAtomically(persistent_location_, contents, "PersistentHistogramAllocator")) {
...
GlobalHistogramAllocator::GlobalHistogramAllocator(std::unique_ptr<PersistentMemoryAllocator> memory)
