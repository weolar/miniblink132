...
BASE_FEATURE(kPassHistogramSharedMemoryOnLaunch, "PassHistogramSharedMemoryOnLaunch", FEATURE_DISABLED_BY_DEFAULT);

#if BUILDFLAG(IS_APPLE)
const MachPortsForRendezvous::key_type HistogramSharedMemory::kRendezvousKey = 'hsmr';
#endif

HistogramSharedMemory::SharedMemory::SharedMemory(UnsafeSharedMemoryRegion r, std::unique_ptr<PersistentMemoryAllocator> a)
    : region(std::move(r))
...
HistogramSharedMemory::SharedMemory::SharedMemory(HistogramSharedMemory::SharedMemory&&) = default;

HistogramSharedMemory::SharedMemory& HistogramSharedMemory::SharedMemory::operator=(HistogramSharedMemory::SharedMemory&&) = default;

// static
std::optional<HistogramSharedMemory::SharedMemory> HistogramSharedMemory::Create(int process_id, const HistogramSharedMemory::Config& config)
{
    ...
    return SharedMemory { std::move(region),
        std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(mapping), static_cast<uint64_t>(process_id), config.allocator_name) };
}

// static
bool HistogramSharedMemory::PassOnCommandLineIsEnabled(std::string_view process_type)
{
...
// static
void HistogramSharedMemory::AddToLaunchParameters(UnsafeSharedMemoryRegion histogram_shmem_region,
    ...
    CommandLine* command_line, LaunchOptions* launch_options)
{
    ...
    DVLOG(1) << (enabled ? "A" : "Not a") << "dding histogram shared memory launch parameters for " << process_type << " process.";

    if (!enabled) {
        return;
    }

    shared_memory::AddToLaunchParameters(::switches::kMetricsSharedMemoryHandle, std::move(histogram_shmem_region),
#if BUILDFLAG(IS_APPLE)
        kRendezvousKey,
...
// static
void HistogramSharedMemory::InitFromLaunchParameters(const CommandLine& command_line)
{
    ...
    DVLOG(1) << "Initializing histogram shared memory from command line for " << command_line.GetSwitchValueASCII("type");

    auto shmem_region = shared_memory::UnsafeSharedMemoryRegionFrom(command_line.GetSwitchValueASCII(switches::kMetricsSharedMemoryHandle));

    SCOPED_CRASH_KEY_NUMBER(
        "HistogramAllocator", "SharedMemError", static_cast<int>(shmem_region.has_value() ? shared_memory::SharedMemoryError::kNoError : shmem_region.error()));

    CHECK(shmem_region.has_value() && shmem_region.value().IsValid()) << "Invald memory region passed on command line.";
