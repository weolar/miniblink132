...
struct EnumSizeTraits {
    static constexpr Enum Count()
    {
        if constexpr (requires { Enum::kMaxValue; }) {
            ...
            static_assert(sizeof(Enum) == 0, "enumerator must define kMaxValue enumerator to use this macro!");
            return Enum();
        }
    }
};
...
#define HISTOGRAM_POINTER_USE(atomic_histogram_pointer, constant_histogram_name, histogram_add_method_invocation, histogram_factory_get_invocation)            \
    do {                                                                                                                                                       \
        base::HistogramBase* histogram_pointer(reinterpret_cast<base::HistogramBase*>(atomic_histogram_pointer->load(std::memory_order_acquire)));             \
       ...
       * makes things clear.                                             \
       */                                                                               \
            atomic_histogram_pointer->store(reinterpret_cast<uintptr_t>(histogram_pointer), std::memory_order_release);                                        \
        }                                                                                                                                                      \
        if (DCHECK_IS_ON())                                                                                                                                    \
...
#define STATIC_HISTOGRAM_POINTER_BLOCK(constant_histogram_name, histogram_add_method_invocation, histogram_factory_get_invocation)                             \
    do {                                                                                                                                                       \
        /*                                                                        \
     ...
     */                                                                            \
        ...
        HISTOGRAM_POINTER_USE(                                                                                                                                 \
            std::addressof(atomic_histogram_pointer), constant_histogram_name, histogram_add_method_invocation, histogram_factory_get_invocation);             \
    } while (0)

// This is a helper macro used by other macros and shouldn't be used directly.
#define INTERNAL_HISTOGRAM_CUSTOM_COUNTS_WITH_FLAG(name, sample, min, max, bucket_count, flag)                                                                 \
    STATIC_HISTOGRAM_POINTER_BLOCK(name, Add(sample), base::Histogram::FactoryGet(name, min, max, bucket_count, flag))
...
#define INTERNAL_HISTOGRAM_EXACT_LINEAR_WITH_FLAG(name, sample, boundary, flag)                                                                                \
    do {                                                                                                                                                       \
        static_assert(!std::is_enum_v<std::decay_t<decltype(sample)>>, "|sample| should not be an enum type!");                                                \
        static_assert(!std::is_enum_v<std::decay_t<decltype(boundary)>>, "|boundary| should not be an enum type!");                                            \
        STATIC_HISTOGRAM_POINTER_BLOCK(name, Add(sample), base::LinearHistogram::FactoryGet(name, 1, boundary, boundary + 1, flag));                           \
    } while (0)
...
#define INTERNAL_HISTOGRAM_SCALED_EXACT_LINEAR_WITH_FLAG(name, sample, count, boundary, scale, flag)                                                           \
    do {                                                                                                                                                       \
        static_assert(!std::is_enum_v<std::decay_t<decltype(sample)>>, "|sample| should not be an enum type!");                                                \
        static_assert(!std::is_enum_v<std::decay_t<decltype(boundary)>>, "|boundary| should not be an enum type!");                                            \
        ...
        static base::LazyInstance<ScaledLinearHistogramInstance>::Leaky scaled_leaky;                                                                          \
...
#define INTERNAL_UMA_HISTOGRAM_ENUMERATION_DEDUCE_BOUNDARY(name, sample, flags)                                                                                \
    INTERNAL_HISTOGRAM_ENUMERATION_WITH_FLAG(name, sample, base::internal::EnumSizeTraits<std::decay_t<decltype(sample)>>::Count(), flags)
...
#define INTERNAL_HISTOGRAM_ENUMERATION_WITH_FLAG(name, sample, boundary, flag)                                                                                 \
    do {                                                                                                                                                       \
        ...
        static_assert(!std::is_enum_v<decayed_boundary> || std::is_enum_v<decayed_sample>, "Unexpected: |boundary| is enum, but |sample| is not.");            \
        static_assert(!std::is_enum_v<decayed_sample> || !std::is_enum_v<decayed_boundary> || std::is_same_v<decayed_sample, decayed_boundary>,                \
            "|sample| and |boundary| shouldn't be of different enums");                                                                                        \
        static_assert(static_cast<uintmax_t>(boundary) < static_cast<uintmax_t>(std::numeric_limits<base::HistogramBase::Sample>::max()),                      \
            "|boundary| is out of range of HistogramBase::Sample");                                                                                            \
        INTERNAL_HISTOGRAM_EXACT_LINEAR_WITH_FLAG(                                                                                                             \
            name, static_cast<base::HistogramBase::Sample>(sample), static_cast<base::HistogramBase::Sample>(boundary), flag);                                 \
    } while (0)

#define INTERNAL_HISTOGRAM_SCALED_ENUMERATION_WITH_FLAG(name, sample, count, scale, flag)                                                                      \
    do {                                                                                                                                                       \
        using decayed_sample = std::decay<decltype(sample)>::type;                                                                                             \
        static_assert(std::is_enum_v<decayed_sample>, "Unexpected: |sample| is not at enum.");                                                                 \
        constexpr auto boundary = base::internal::EnumSizeTraits<std::decay_t<decltype(sample)>>::Count();                                                     \
        static_assert(static_cast<uintmax_t>(boundary) < static_cast<uintmax_t>(std::numeric_limits<base::HistogramBase::Sample>::max()),                      \
            "|boundary| is out of range of HistogramBase::Sample");                                                                                            \
        INTERNAL_HISTOGRAM_SCALED_EXACT_LINEAR_WITH_FLAG(                                                                                                      \
            name, static_cast<base::HistogramBase::Sample>(sample), count, static_cast<base::HistogramBase::Sample>(boundary), scale, flag);                   \
    } while (0)

// This is a helper macro used by other macros and shouldn't be used directly.
// This is necessary to expand __COUNTER__ to an actual value.
#define INTERNAL_SCOPED_UMA_HISTOGRAM_TIMER_EXPANDER(name, timing, key) INTERNAL_SCOPED_UMA_HISTOGRAM_TIMER_UNIQUE(name, timing, key)
    ...
    public:                                                                                                                                                    \
        ...
        {                                                                                                                                                      \
            ...
            case ScopedHistogramTiming::kMicrosecondTimes:                                                                                                     \
                UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(name, elapsed, base::Microseconds(1), base::Seconds(1), 50);                                           \
                break;                                                                                                                                         \
