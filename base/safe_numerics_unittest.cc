...
template <typename Dst, typename Src> Dst GetMaxConvertibleToFloat()
{
...
static_assert(std::is_same_v<BigEnoughPromotion<int16_t, int8_t>::type, int16_t>, "");
static_assert(std::is_same_v<BigEnoughPromotion<int32_t, uint32_t>::type, int64_t>, "");
static_assert(std::is_same_v<BigEnoughPromotion<intmax_t, int8_t>::type, intmax_t>, "");
static_assert(std::is_same_v<BigEnoughPromotion<uintmax_t, int8_t>::type, uintmax_t>, "");
static_assert(BigEnoughPromotion<int16_t, int8_t>::is_contained, "");
static_assert(BigEnoughPromotion<int32_t, uint32_t>::is_contained, "");
static_assert(BigEnoughPromotion<intmax_t, int8_t>::is_contained, "");
static_assert(!BigEnoughPromotion<uintmax_t, int8_t>::is_contained, "");
static_assert(std::is_same_v<FastIntegerArithmeticPromotion<int16_t, int8_t>::type, int32_t>, "");
static_assert(std::is_same_v<FastIntegerArithmeticPromotion<int32_t, uint32_t>::type, int64_t>, "");
static_assert(std::is_same_v<FastIntegerArithmeticPromotion<intmax_t, int8_t>::type, intmax_t>, "");
static_assert(std::is_same_v<FastIntegerArithmeticPromotion<uintmax_t, int8_t>::type, uintmax_t>, "");
...
static_assert(2U == (MakeCheckedNum(kFloatOne) + 1).Cast<unsigned>().ValueOrDie(), "");
static_assert(0U == unsigned { MakeClampedNum(kFloatOne) - 1 }, "");
static_assert(0U == (MakeCheckedNum(kFloatOne) - 1).Cast<unsigned>().ValueOrDie(), "");
...
static_assert(1U == (MakeCheckedNum(kFloatOne) * 1).Cast<unsigned>().ValueOrDie(), "");
static_assert(1U == unsigned { MakeClampedNum(kFloatOne) / 1 }, "");
static_assert(1U == (MakeCheckedNum(kFloatOne) / 1).Cast<unsigned>().ValueOrDie(), "");
static_assert(1 == int { MakeClampedNum(-kFloatOne).Abs() }, "");
static_assert(1 == MakeCheckedNum(-kFloatOne).Abs().Cast<int>().ValueOrDie(), "");

template <typename U> U GetNumericValueForTest(const CheckedNumeric<U>& src)
{
    return src.state_.value();
}

template <typename U> U GetNumericValueForTest(const ClampedNumeric<U>& src)
{
    return static_cast<U>(src);
}

template <typename U> U GetNumericValueForTest(const U& src)
{
    return src;
}
...
struct LogOnFailure {
    template <typename T> static T HandleFailure()
    {
...
template <typename T> constexpr T GetValue(const T& src)
{
    return src;
}

template <typename T, typename U> constexpr T GetValueAsDest(const U& src)
{
    return static_cast<T>(src);
}

template <typename T> constexpr T GetValue(const CheckedNumeric<T>& src)
{
    return src.template ValueOrDie<T, LogOnFailure>();
}

template <typename T, typename U> constexpr T GetValueAsDest(const CheckedNumeric<U>& src)
{
    return src.template ValueOrDie<T, LogOnFailure>();
}

template <typename T> constexpr T GetValue(const ClampedNumeric<T>& src)
{
    return static_cast<T>(src);
}

template <typename T, typename U> constexpr T GetValueAsDest(const ClampedNumeric<U>& src)
{
    return static_cast<T>(src);
}
...
#define TEST_EXPECTED_VALIDITY(expected, actual)                                                                                                               \
    EXPECT_EQ(expected, (actual).template Cast<Dst>().IsValid())                                                                                               \
        << "Result test: Value " << GetNumericValueForTest(actual) << " as " << dst << " on line " << line
...
template <typename Dst> void TestStrictPointerMath()
{
    Dst dummy_value = 0;
    Dst* dummy_ptr = &dummy_value;
    static const Dst kDummyOffset = 2; // Don't want to go too far.
    EXPECT_EQ(dummy_ptr + kDummyOffset, dummy_ptr + StrictNumeric<Dst>(kDummyOffset));
    EXPECT_EQ(dummy_ptr - kDummyOffset, dummy_ptr - StrictNumeric<Dst>(kDummyOffset));
    EXPECT_NE(dummy_ptr, dummy_ptr + StrictNumeric<Dst>(kDummyOffset));
    EXPECT_NE(dummy_ptr, dummy_ptr - StrictNumeric<Dst>(kDummyOffset));
    EXPECT_DEATH_IF_SUPPORTED(dummy_ptr + StrictNumeric<size_t>(std::numeric_limits<size_t>::max()), "");
}

// Signed integer arithmetic.
template <typename Dst>
static void TestSpecializedArithmetic(const char* dst, int line, std::enable_if_t<numeric_limits<Dst>::is_integer && numeric_limits<Dst>::is_signed, int> = 0)
{
    ...
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeCheckedNum(-DstLimits::max()).Abs());

    TEST_EXPECTED_VALUE(DstLimits::Overflow(), -ClampedNumeric<Dst>(DstLimits::lowest()));
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::lowest()).Abs());
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(-1).Abs());
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeClampedNum(-DstLimits::max()).Abs());

    TEST_EXPECTED_SUCCESS(CheckedNumeric<Dst>(DstLimits::max()) + -1);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) + -1);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) + DstLimits::lowest());

    TEST_EXPECTED_VALUE(DstLimits::max() - 1, ClampedNumeric<Dst>(DstLimits::max()) + -1);
    TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::lowest()) + -1);
    TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::lowest()) + DstLimits::lowest());

    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) - 1);
    TEST_EXPECTED_SUCCESS(CheckedNumeric<Dst>(DstLimits::lowest()) - -1);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::max()) - DstLimits::lowest());
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) - DstLimits::max());

    TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::lowest()) - 1);
    TEST_EXPECTED_VALUE(DstLimits::lowest() + 1, ClampedNumeric<Dst>(DstLimits::lowest()) - -1);
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::max()) - DstLimits::lowest());
    TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::lowest()) - DstLimits::max());

    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) * 2);
    TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::lowest()) * 2);

    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) / -1);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(-1) / 2);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::lowest()) * -1);
    TEST_EXPECTED_VALUE(DstLimits::max(), CheckedNumeric<Dst>(DstLimits::lowest() + 1) * Dst(-1));
    TEST_EXPECTED_VALUE(DstLimits::max(), CheckedNumeric<Dst>(-1) * Dst(DstLimits::lowest() + 1));
    TEST_EXPECTED_VALUE(DstLimits::lowest(), CheckedNumeric<Dst>(DstLimits::lowest()) * Dst(1));
    TEST_EXPECTED_VALUE(DstLimits::lowest(), CheckedNumeric<Dst>(1) * Dst(DstLimits::lowest()));
    TEST_EXPECTED_VALUE(typename std::make_unsigned<Dst>::type(0) - DstLimits::lowest(), MakeCheckedNum(DstLimits::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeCheckedNum(DstLimits::max()).UnsignedAbs());
    ...
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::lowest()) * -1);
    TEST_EXPECTED_VALUE(DstLimits::max(), ClampedNumeric<Dst>(DstLimits::lowest() + 1) * Dst(-1));
    TEST_EXPECTED_VALUE(DstLimits::max(), ClampedNumeric<Dst>(-1) * Dst(DstLimits::lowest() + 1));
    TEST_EXPECTED_VALUE(DstLimits::lowest(), ClampedNumeric<Dst>(DstLimits::lowest()) * Dst(1));
    TEST_EXPECTED_VALUE(DstLimits::lowest(), ClampedNumeric<Dst>(1) * Dst(DstLimits::lowest()));
    TEST_EXPECTED_VALUE(typename std::make_unsigned<Dst>::type(0) - DstLimits::lowest(), MakeClampedNum(DstLimits::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeClampedNum(DstLimits::max()).UnsignedAbs());
    ...
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1));
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(0) << IntegerBitsPlusSign<Dst>::value);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::max()) << 1);
    TEST_EXPECTED_VALUE(static_cast<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 2), CheckedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 2));
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(0) << (IntegerBitsPlusSign<Dst>::value - 1));
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) << 0);
    TEST_EXPECTED_VALUE(2, CheckedNumeric<Dst>(1) << 1);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(1) >> IntegerBitsPlusSign<Dst>::value);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(1) >> (IntegerBitsPlusSign<Dst>::value - 1));
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(1) >> negative_one);
    ...
    // Test bit shifts.
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(Dst(0), ClampedNumeric<Dst>(0) << (IntegerBitsPlusSign<Dst>::value + 0U));
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::max()) << 1U);
    TEST_EXPECTED_VALUE(static_cast<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 2U), ClampedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 2U));
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(0) << (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) << 0U);
    TEST_EXPECTED_VALUE(2, ClampedNumeric<Dst>(1) << 1U);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) >> (IntegerBitsPlusSign<Dst>::value + 0U));
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) >> (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(-1, ClampedNumeric<Dst>(-1) >> (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(-1, ClampedNumeric<Dst>(DstLimits::lowest()) >> (IntegerBitsPlusSign<Dst>::value - 0U));
...
static void TestSpecializedArithmetic(const char* dst, int line, std::enable_if_t<numeric_limits<Dst>::is_integer && !numeric_limits<Dst>::is_signed, int> = 0)
{
    ...
    TEST_EXPECTED_SUCCESS(
        CheckedNumeric<typename std::make_signed<Dst>::type>(std::numeric_limits<typename std::make_signed<Dst>::type>::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::lowest(), MakeCheckedNum(DstLimits::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeCheckedNum(DstLimits::max()).UnsignedAbs());
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(0).UnsignedAbs());
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1).UnsignedAbs());
    ...
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(DstLimits::lowest()) * 2);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) / 2);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(DstLimits::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(as_unsigned(std::numeric_limits<typename std::make_signed<Dst>::type>::lowest()),
        ClampedNumeric<typename std::make_signed<Dst>::type>(std::numeric_limits<typename std::make_signed<Dst>::type>::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::lowest(), MakeClampedNum(DstLimits::lowest()).UnsignedAbs());
    TEST_EXPECTED_VALUE(DstLimits::max(), MakeClampedNum(DstLimits::max()).UnsignedAbs());
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(0).UnsignedAbs());
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1).UnsignedAbs());
    ...
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(0) << IntegerBitsPlusSign<Dst>::value);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::max()) << 1);
    TEST_EXPECTED_VALUE(static_cast<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1), CheckedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1));
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) << 0);
    TEST_EXPECTED_VALUE(2, CheckedNumeric<Dst>(1) << 1);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(1) >> IntegerBitsPlusSign<Dst>::value);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(1) >> (IntegerBitsPlusSign<Dst>::value - 1));
    ...
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(0) & 1);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(1) & 0);
    TEST_EXPECTED_VALUE(std::numeric_limits<Dst>::max(), MakeCheckedNum(DstLimits::max()) & -1);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) | 1);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) | 0);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(0) | 1);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(0) | 0);
    TEST_EXPECTED_VALUE(std::numeric_limits<Dst>::max(), CheckedNumeric<Dst>(0) | static_cast<Dst>(-1));
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(1) ^ 1);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) ^ 0);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(0) ^ 1);
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>(0) ^ 0);
    ...
    TEST_EXPECTED_VALUE(Dst(1), ClampedNumeric<Dst>(1) % 0);
    // Test bit shifts.
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(1) << as_unsigned(IntegerBitsPlusSign<Dst>::value));
    TEST_EXPECTED_VALUE(Dst(0), ClampedNumeric<Dst>(0) << as_unsigned(IntegerBitsPlusSign<Dst>::value));
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::max()) << 1U);
    TEST_EXPECTED_VALUE(static_cast<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1U), ClampedNumeric<Dst>(1) << (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) << 0U);
    TEST_EXPECTED_VALUE(2, ClampedNumeric<Dst>(1) << 1U);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) >> as_unsigned(IntegerBitsPlusSign<Dst>::value));
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) >> (IntegerBitsPlusSign<Dst>::value - 1U));
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) & 1);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) & 0);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(0) & 1);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) & 0);
    TEST_EXPECTED_VALUE(std::numeric_limits<Dst>::max(), MakeClampedNum(DstLimits::max()) & -1);
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) | 1);
    ...
    TEST_EXPECTED_VALUE(std::numeric_limits<Dst>::max(), ClampedNumeric<Dst>(0) | static_cast<Dst>(-1));
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(1) ^ 1);
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) ^ 0);
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(0) ^ 1);
    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(0) ^ 0);
...
template <typename Dst> void TestSpecializedArithmetic(const char* dst, int line, std::enable_if_t<numeric_limits<Dst>::is_iec559, int> = 0)
{
    using DstLimits = SaturationDefaultLimits<Dst>;
    TEST_EXPECTED_SUCCESS(-CheckedNumeric<Dst>(DstLimits::lowest()));
    ...
    TEST_EXPECTED_VALUE(DstLimits::max(), -ClampedNumeric<Dst>(DstLimits::lowest()));

    TEST_EXPECTED_VALUE(DstLimits::max(), ClampedNumeric<Dst>(DstLimits::lowest()).Abs());
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(-1).Abs());
...
template <typename Dst> static void TestArithmetic(const char* dst, int line)
{
    using DstLimits = SaturationDefaultLimits<Dst>;

    // Test C++17 class template argument deduction
    static_assert(std::is_same_v<Dst, typename decltype(CheckedNumeric(Dst { 0 }))::type>);
    static_assert(std::is_same_v<Dst, typename decltype(ClampedNumeric(Dst { 0 }))::type>);
    static_assert(std::is_same_v<Dst, typename decltype(StrictNumeric(Dst { 0 }))::type>);

    EXPECT_EQ(true, CheckedNumeric<Dst>().IsValid());
    EXPECT_EQ(false, CheckedNumeric<Dst>(CheckedNumeric<Dst>(DstLimits::max()) * DstLimits::max()).IsValid());
    EXPECT_EQ(static_cast<Dst>(0), CheckedNumeric<Dst>().ValueOrDie());
    EXPECT_EQ(static_cast<Dst>(0), CheckedNumeric<Dst>().ValueOrDefault(1));
    EXPECT_EQ(static_cast<Dst>(1), CheckedNumeric<Dst>(CheckedNumeric<Dst>(DstLimits::max()) * DstLimits::max()).ValueOrDefault(1));
    ...
    if (DstLimits::is_signed) {
        ...
        TEST_EXPECTED_VALUE(1, -CheckedNumeric<Dst>(-1));
        TEST_EXPECTED_VALUE(static_cast<Dst>(DstLimits::max() * -1), -CheckedNumeric<Dst>(DstLimits::max()));

        TEST_EXPECTED_VALUE(0, -ClampedNumeric<Dst>());
        TEST_EXPECTED_VALUE(-1, -ClampedNumeric<Dst>(1));
        TEST_EXPECTED_VALUE(1, -ClampedNumeric<Dst>(-1));
        TEST_EXPECTED_VALUE(static_cast<Dst>(DstLimits::max() * -1), -ClampedNumeric<Dst>(DstLimits::max()));
    ...
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1).Abs());
    TEST_EXPECTED_VALUE(DstLimits::max(), ClampedNumeric<Dst>(DstLimits::max()).Abs());
    ...
    if (numeric_limits<Dst>::is_signed)
        TEST_EXPECTED_VALUE(0, (ClampedNumeric<Dst>(-1) + 1));
    TEST_EXPECTED_VALUE(DstLimits::lowest() + 1, ClampedNumeric<Dst>(DstLimits::lowest()) + 1);
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::max()) + DstLimits::max());
    ...
    if (numeric_limits<Dst>::is_signed) {
        ...
        TEST_EXPECTED_VALUE(DstLimits::max(), ClampedNumeric<Dst>(DstLimits::max()) - -1);
    }
    ...
    if (numeric_limits<Dst>::is_signed) {
        ...
        TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::max()) * CheckedNumeric<uintmax_t>(-2));
    }
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(DstLimits::max()) * DstLimits::max());
    ...
    if (numeric_limits<Dst>::is_signed) {
        ...
        TEST_EXPECTED_VALUE(DstLimits::Underflow(), ClampedNumeric<Dst>(DstLimits::max()) * -2);
        TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>(DstLimits::max()) * ClampedNumeric<uintmax_t>(-2));
    }
    TEST_EXPECTED_VALUE(DstLimits::Overflow(), ClampedNumeric<Dst>(DstLimits::max()) * DstLimits::max());
    ...
    TEST_EXPECTED_VALUE(0, CheckedNumeric<Dst>() / 1);
    TEST_EXPECTED_VALUE(1, CheckedNumeric<Dst>(1) / 1);
    TEST_EXPECTED_VALUE(DstLimits::lowest() / 2, CheckedNumeric<Dst>(DstLimits::lowest()) / 2);
    TEST_EXPECTED_VALUE(DstLimits::max() / 2, CheckedNumeric<Dst>(DstLimits::max()) / 2);
    TEST_EXPECTED_FAILURE(CheckedNumeric<Dst>(1) / 0);

    TEST_EXPECTED_VALUE(0, ClampedNumeric<Dst>() / 1);
    TEST_EXPECTED_VALUE(1, ClampedNumeric<Dst>(1) / 1);
    TEST_EXPECTED_VALUE(DstLimits::lowest() / 2, ClampedNumeric<Dst>(DstLimits::lowest()) / 2);
    TEST_EXPECTED_VALUE(DstLimits::max() / 2, ClampedNumeric<Dst>(DstLimits::max()) / 2);
...
template <typename Dst, typename Src, NumericConversionType conversion> struct TestNumericConversion { };
...
{
    return static_cast<RangeConstraint>(static_cast<int>(constraint.IsOverflowFlagSet()) << 1 | static_cast<int>(constraint.IsUnderflowFlagSet()));
}

// EXPECT_EQ wrappers providing specific detail on test failures.
#define TEST_EXPECTED_RANGE(expected, actual)                                                                                                                  \
    EXPECT_EQ(expected, RangeCheckToEnum(DstRangeRelationToSrcRange<Dst>(actual)))                                                                             \
        << "Conversion test: " << src << " value " << actual << " to " << dst << " on line " << line

template <typename Dst, typename Src> void TestStrictComparison(const char* dst, const char* src, int line)
{
    using DstLimits = numeric_limits<Dst>;
    using SrcLimits = numeric_limits<Src>;
    ...
    EXPECT_EQ(SrcLimits::max(), MakeCheckedNum(SrcLimits::max()).Max(DstLimits::lowest()).ValueOrDie());
    EXPECT_EQ(DstLimits::max(), MakeCheckedNum(SrcLimits::lowest()).Max(DstLimits::max()).ValueOrDie());
    EXPECT_EQ(DstLimits::lowest(), MakeCheckedNum(SrcLimits::max()).Min(DstLimits::lowest()).ValueOrDie());
    EXPECT_EQ(SrcLimits::lowest(), MakeCheckedNum(SrcLimits::lowest()).Min(DstLimits::max()).ValueOrDie());
    EXPECT_EQ(SrcLimits::lowest(), CheckMin(MakeStrictNum(1), MakeCheckedNum(0), DstLimits::max(), SrcLimits::lowest()).ValueOrDie());
    EXPECT_EQ(DstLimits::max(), CheckMax(MakeStrictNum(1), MakeCheckedNum(0), DstLimits::max(), SrcLimits::lowest()).ValueOrDie());

    EXPECT_EQ(SrcLimits::max(), MakeClampedNum(SrcLimits::max()).Max(DstLimits::lowest()));
    EXPECT_EQ(DstLimits::max(), MakeClampedNum(SrcLimits::lowest()).Max(DstLimits::max()));
    EXPECT_EQ(DstLimits::lowest(), MakeClampedNum(SrcLimits::max()).Min(DstLimits::lowest()));
    EXPECT_EQ(SrcLimits::lowest(), MakeClampedNum(SrcLimits::lowest()).Min(DstLimits::max()));
    EXPECT_EQ(SrcLimits::lowest(), ClampMin(MakeStrictNum(1), MakeClampedNum(0), DstLimits::max(), SrcLimits::lowest()));
    EXPECT_EQ(DstLimits::max(), ClampMax(MakeStrictNum(1), MakeClampedNum(0), DstLimits::max(), SrcLimits::lowest()));

    if (IsValueInRangeForNumericType<Dst>(SrcLimits::max())) {
        TEST_EXPECTED_VALUE(Dst(SrcLimits::max()), (CommonMax<Dst, Src>()));
        TEST_EXPECTED_VALUE(Dst(SrcLimits::max()), (CommonMaxOrMin<Dst, Src>(false)));
    } else {
    ...
    if (IsValueInRangeForNumericType<Dst>(SrcLimits::lowest())) {
        ...
        TEST_EXPECTED_VALUE(Dst(SrcLimits::lowest()), (CommonMaxOrMin<Dst, Src>(true)));
    } else {
...
template <typename Dst, typename Src> struct TestNumericConversion<Dst, Src, SIGN_PRESERVING_VALUE_PRESERVING> {
    static void Test(const char* dst, const char* src, int line)
    {
        ...
        static_assert((DstLimits::is_iec559 && SrcLimits::is_integer) ||
                ...
                (!(DstLimits::is_integer && SrcLimits::is_iec559) &&
                    ...
                    ((SrcLimits::is_signed == DstLimits::is_signed && MaxExponent<Dst>::value >= MaxExponent<Src>::value) ||
                        // Or signed destination and source is smaller
                        (DstLimits::is_signed && MaxExponent<Dst>::value >= MaxExponent<Src>::value))),
        ...
        if (MaxExponent<Dst>::value > MaxExponent<Src>::value) {
            if (MaxExponent<Dst>::value >= MaxExponent<Src>::value * 2 - 1) {
                ...
                TEST_EXPECTED_VALUE(SrcLimits::max() * clamped_dst, Dst(SrcLimits::max()) * Dst(SrcLimits::max()));
                ...
                TEST_EXPECTED_VALUE(DstLimits::Overflow(), SrcLimits::max() * clamped_dst);
                TEST_EXPECTED_VALUE(Dst(SrcLimits::max()) + Dst(1), clamped_dst + Dst(1));
            }
...
template <typename Dst, typename Src> struct TestNumericConversion<Dst, Src, SIGN_PRESERVING_NARROW> {
    static void Test(const char* dst, const char* src, int line)
    {
        using SrcLimits = SaturationDefaultLimits<Src>;
        using DstLimits = SaturationDefaultLimits<Dst>;
        static_assert(SrcLimits::is_signed == DstLimits::is_signed, "Destination and source sign must be the same");
        static_assert(MaxExponent<Dst>::value <= MaxExponent<Src>::value, "Destination must be narrower than source");
        ...
        if (SrcLimits::is_iec559) {
            ...
            if (DstLimits::is_integer) {
                ...
                TEST_EXPECTED_RANGE(RANGE_VALID, static_cast<Src>(GetMaxConvertibleToFloat<Src, Dst>()));
            ...
            TEST_EXPECTED_VALUE(Src(Src(0) - DstLimits::lowest()), ClampDiv(DstLimits::lowest(), Src(-1)));
...
template <typename Dst, typename Src> struct TestNumericConversion<Dst, Src, SIGN_TO_UNSIGN_WIDEN_OR_EQUAL> {
    static void Test(const char* dst, const char* src, int line)
    {
        using SrcLimits = SaturationDefaultLimits<Src>;
        using DstLimits = SaturationDefaultLimits<Dst>;
        static_assert(MaxExponent<Dst>::value >= MaxExponent<Src>::value, "Destination must be equal or wider than source.");
        ...
        TEST_EXPECTED_VALUE(DstLimits::Underflow(), clamped_dst + static_cast<Src>(-1));
        TEST_EXPECTED_VALUE(0, clamped_dst * static_cast<Src>(-1));
        TEST_EXPECTED_VALUE(DstLimits::Underflow(), clamped_dst + SrcLimits::lowest());
...
template <typename Dst, typename Src> struct TestNumericConversion<Dst, Src, SIGN_TO_UNSIGN_NARROW> {
    static void Test(const char* dst, const char* src, int line)
    {
        using SrcLimits = SaturationDefaultLimits<Src>;
        using DstLimits = SaturationDefaultLimits<Dst>;
...
template <typename Dst, typename Src> struct TestNumericConversion<Dst, Src, UNSIGN_TO_SIGN_NARROW_OR_EQUAL> {
    static void Test(const char* dst, const char* src, int line)
    {
        using SrcLimits = SaturationDefaultLimits<Src>;
        using DstLimits = SaturationDefaultLimits<Dst>;
        static_assert(MaxExponent<Dst>::value <= MaxExponent<Src>::value, "Destination must be narrower or equal to source.");
...
TEST(SafeNumerics, Int16Operations)
{
    ...
    TEST_NUMERIC_CONVERSION(int16_t, unsigned int, UNSIGN_TO_SIGN_NARROW_OR_EQUAL);
...
static_assert(std::is_same_v<decltype(TestOverload(StrictNumeric<size_t>())), size_t>, "");

template <typename T> struct CastTest1 {
...
template <typename T> struct CastTest2 {
    static constexpr T NaN()
    {
        return 11;
    }
...
#pragma warning(disable : 4756)
    ...
    EXPECT_EQ(saturated_cast<float>(-double_large), -double_infinity);
    EXPECT_EQ(numeric_limits<int>::lowest(), saturated_cast<int>(double_small_int));
    ...
    EXPECT_EQ(CastTest1<int>::max(), (saturated_cast<int, CastTest1>(FloatLimits::infinity())));
    EXPECT_EQ(CastTest1<int>::max(), (saturated_cast<int, CastTest1>(FloatLimits::max())));
    EXPECT_EQ(CastTest1<int>::max(), (saturated_cast<int, CastTest1>(float(IntLimits::max()))));
    EXPECT_EQ(CastTest1<int>::lowest(), (saturated_cast<int, CastTest1>(-FloatLimits::infinity())));
    EXPECT_EQ(CastTest1<int>::lowest(), (saturated_cast<int, CastTest1>(FloatLimits::lowest())));
    ...
    EXPECT_EQ(-1, (saturated_cast<int, CastTest1>(-1)));
    EXPECT_EQ(CastTest1<int>::lowest(), (saturated_cast<int, CastTest1>(float(IntLimits::lowest()))));
    ...
    static_assert(std::is_same_v<int16_t, decltype(int8_min.ValueOrDie<int16_t>())::type>, "ValueOrDie returning incorrect type.");
    static_assert(std::is_same_v<int16_t, decltype(int8_min.ValueOrDefault<int16_t>(0))::type>, "ValueOrDefault returning incorrect type.");
    EXPECT_FALSE(IsValidForType<uint8_t>(int8_min));
    EXPECT_TRUE(IsValidForType<uint8_t>(int8_max));
    EXPECT_EQ(static_cast<int>(numeric_limits<int8_t>::lowest()), ValueOrDieForType<int>(int8_min));
    EXPECT_TRUE(IsValidForType<uint32_t>(int8_max));
    EXPECT_EQ(static_cast<int>(numeric_limits<int8_t>::max()), ValueOrDieForType<int>(int8_max));
...
TEST(SafeNumerics, IsValueInRangeForNumericType)
{
    ...
    EXPECT_FALSE(IsValueInRangeForNumericType<uint32_t>(std::numeric_limits<int32_t>::lowest()));
    EXPECT_FALSE(IsValueInRangeForNumericType<uint32_t>(std::numeric_limits<int64_t>::lowest()));
    ...
    EXPECT_TRUE(IsValueInRangeForNumericType<int32_t>(std::numeric_limits<int32_t>::lowest()));
    EXPECT_TRUE(IsValueInRangeForNumericType<int32_t>(static_cast<int64_t>(std::numeric_limits<int32_t>::lowest())));
    EXPECT_FALSE(IsValueInRangeForNumericType<int32_t>(static_cast<int64_t>(std::numeric_limits<int32_t>::lowest()) - 1));
    EXPECT_FALSE(IsValueInRangeForNumericType<int32_t>(std::numeric_limits<int64_t>::lowest()));
    ...
    EXPECT_FALSE(IsValueInRangeForNumericType<uint64_t>(std::numeric_limits<int32_t>::lowest()));
    EXPECT_FALSE(IsValueInRangeForNumericType<uint64_t>(INT64_C(-1)));
    EXPECT_FALSE(IsValueInRangeForNumericType<uint64_t>(std::numeric_limits<int64_t>::lowest()));
    ...
    EXPECT_TRUE(IsValueInRangeForNumericType<int64_t>(std::numeric_limits<int32_t>::lowest()));
    EXPECT_TRUE(IsValueInRangeForNumericType<int64_t>(static_cast<int64_t>(std::numeric_limits<int32_t>::lowest())));
    EXPECT_TRUE(IsValueInRangeForNumericType<int64_t>(std::numeric_limits<int64_t>::lowest()));
}
...
TEST(SafeNumerics, TemplatedSafeMath)
{
    ...
    EXPECT_FALSE(CheckMul(short_one_thousand, short_one_thousand).AssignIfValid<uint64_t>(&result));
    ...
    // overflow.
    EXPECT_TRUE(CheckMul<uint64_t>(short_one_thousand, short_one_thousand).AssignIfValid(&result));
    EXPECT_TRUE(CheckMul<uint64_t>(short_one_thousand, short_one_thousand).AssignIfValid<uint64_t>(&result));
    ...
    EXPECT_TRUE(CheckMul(big_one_thousand, short_one_thousand).AssignIfValid(&result));
    EXPECT_TRUE(CheckMul(short_one_thousand, big_one_thousand).AssignIfValid(&result));
    ...
    // Same as above.
    EXPECT_TRUE((CheckMul<uint64_t, uint64_t>(1000, 1000).AssignIfValid(&result)));
}
...
template <typename T> void TestWrappingMathSigned()
{
...
template <typename T> void TestWrappingMathUnsigned()
{
