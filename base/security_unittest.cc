...
template <typename Type> NOINLINE Type HideValueFromCompiler(Type value)
{
#if defined(__GNUC__)
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_APPLE)
...
#if BUILDFLAG(IS_APPLE) || defined(ADDRESS_SANITIZER) || defined(THREAD_SANITIZER) || defined(MEMORY_SANITIZER) || BUILDFLAG(IS_HWASAN)                        \
...
TEST(SecurityTest, MAYBE_NewOverflow)
{
    ...
    [[maybe_unused]] const size_t kDynamicArraySize = HideValueFromCompiler(kArraySize);
