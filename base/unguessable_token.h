...
public:
    ...
    static std::optional<UnguessableToken> Deserialize(uint64_t high, uint64_t low);
    ...
    static std::optional<UnguessableToken> DeserializeFromString(std::string_view string_representation);
    ...
    {
        return token_.AsBytes();
    }

    friend constexpr auto operator<=>(const UnguessableToken& lhs, const UnguessableToken& rhs) = default;

    // operator== uses constant-time comparison for security where available.
    friend BASE_EXPORT bool operator==(const UnguessableToken& lhs, const UnguessableToken& rhs);
...
BASE_EXPORT bool operator==(const UnguessableToken& lhs, const UnguessableToken& rhs);

BASE_EXPORT std::ostream& operator<<(std::ostream& out, const UnguessableToken& token);
