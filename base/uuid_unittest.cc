...
bool IsValidV4(const Uuid& guid)
{
    ...
    return guid.is_valid() && lowercase[14] == '4' && (lowercase[19] == '8' || lowercase[19] == '9' || lowercase[19] == 'a' || lowercase[19] == 'b');
}
...
void TestUuidValidity(std::string_view input, bool case_insensitive, bool strict)
{
    SCOPED_TRACE(input);
    {
...
{
    static constexpr char kCanonicalStr[] = "deadbeef-dead-4eef-bead-beefdeadbeef";

    const Uuid from_lower = Uuid::ParseCaseInsensitive(ToLowerASCII(kCanonicalStr));
    EXPECT_EQ(kCanonicalStr, from_lower.AsLowercaseString());

    const Uuid from_upper = Uuid::ParseCaseInsensitive(ToUpperASCII(kCanonicalStr));
...
{
    static constexpr uint64_t bytes1a[] = { 0x0123456789abcdefull, 0x5a5a5a5aa5a5a5a5ull };
    static constexpr uint64_t bytes1b[] = { bytes1a[0], bytes1a[1] };
    static constexpr uint64_t bytes2[] = { 0xfffffffffffffffdull, 0xfffffffffffffffeull };
    static constexpr uint64_t bytes3[] = { 0xfffffffffffffffdull, 0xfffffffffffffffcull };

    const Uuid guid1a = Uuid::FormatRandomDataAsV4ForTesting(as_bytes(make_span(bytes1a)));
    const Uuid guid1b = Uuid::FormatRandomDataAsV4ForTesting(as_bytes(make_span(bytes1b)));
    const Uuid guid2 = Uuid::FormatRandomDataAsV4ForTesting(as_bytes(make_span(bytes2)));
    const Uuid guid3 = Uuid::FormatRandomDataAsV4ForTesting(as_bytes(make_span(bytes3)));
