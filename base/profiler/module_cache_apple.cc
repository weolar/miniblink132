...
void GetUniqueIdAndTextSize(const void* module_addr, std::string* unique_id, size_t* text_size)
{
    const MachHeaderType* mach_header = reinterpret_cast<const MachHeaderType*>(module_addr);
    ...
    for (uint32_t i = 0; i < mach_header->ncmds; ++i) {
        ...
        const load_command* load_cmd = reinterpret_cast<const load_command*>(reinterpret_cast<const uint8_t*>(mach_header) + offset);
        ...
        if (load_cmd->cmd == LC_UUID) {
            if (load_cmd->cmdsize < sizeof(uuid_command)) {
                ...
                const uuid_command* uuid_cmd = reinterpret_cast<const uuid_command*>(load_cmd);
                static_assert(sizeof(uuid_cmd->uuid) == sizeof(uuid_t), "UUID field of UUID command should be 16 bytes.");
                // The ID comprises the UUID concatenated with the Mac's "age" value
                // which is always 0.
                unique_id->assign(HexEncode(&uuid_cmd->uuid, sizeof(uuid_cmd->uuid)) + "0");
            }
            if (found_text_size) {
                return;
            }
            ...
            const SegmentCommandType* segment_cmd = reinterpret_cast<const SegmentCommandType*>(load_cmd);
            if (strncmp(segment_cmd->segname, SEG_TEXT, sizeof(segment_cmd->segname)) == 0) {
