...
TEST(SampleMetadataTest, ScopedSampleMetadata)
{
    ...
    size_t initial_item_count = MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items);

    {
        ScopedSampleMetadata m("myname", 100, SampleMetadataScope::kProcess);

        ASSERT_EQ(initial_item_count + 1, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items));
    ...
    ASSERT_EQ(initial_item_count, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items));
}
...
TEST(SampleMetadataTest, SampleMetadataWithThreadId)
{
    ...
    metadata.Set(100);
    ASSERT_EQ(0u, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), kInvalidThreadId).GetItems(&items));
    ASSERT_EQ(initial_item_count + 1, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items));
    EXPECT_EQ(HashMetricName("myname"), items[initial_item_count].name_hash);
    EXPECT_FALSE(items[initial_item_count].key.has_value());
    EXPECT_EQ(100, items[initial_item_count].value);
