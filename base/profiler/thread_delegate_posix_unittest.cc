...
{
    auto delegate = ThreadDelegatePosix::Create(GetSamplingProfilerCurrentThreadToken());
...
TEST(ThreadDelegatePosixTest, MainThreadStackBase)
{
    ...
    auto delegate = ThreadDelegatePosix::Create(SamplingProfilerThreadToken { GetCurrentProcId(), pthread_t() });
    ASSERT_TRUE(delegate);
    uintptr_t base = delegate->GetStackBaseAddress();
    EXPECT_GT(base, 0u);
}
