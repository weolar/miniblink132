...
public:
    ...
    SamplingProfilerThreadToken thread_token() const
    {
...
private:
    static FunctionAddressRange InvokeSetupFunction(const SetupFunction& setup_function, SampleEvents* events);
...
public:
    explicit TestModule(uintptr_t base_address = 0, size_t size = 0, bool is_native = true)
...
// modules.
FunctionAddressRange CallThroughOtherLibrary(NativeLibrary library, OnceClosure wait_for_sample);
...
std::vector<Frame> SampleScenario(UnwindScenario* scenario, ModuleCache* module_cache, UnwinderFactory aux_unwinder_factory = UnwinderFactory());
