...
TEST(StackBufferTest, BufferAllocated)
{
    ...
    EXPECT_TRUE(IsAligned(const_cast<uintptr_t*>(buffer), StackBuffer::kPlatformStackAlignment));
...
TEST(StackBufferTest, MarkBufferContentsAsUnneeded)
{
    ...
    stack_buffer.MarkUpperBufferContentsAsUnneeded(kUndiscardedElements * sizeof(buffer[0]));
    ...
    const size_t kExpectedDiscardStartPoint = bits::AlignUp(kUndiscardedElements * sizeof(buffer[0]), GetPageSize()) / sizeof(buffer[0]);
    for (size_t i = kExpectedDiscardStartPoint; i < kBufferSize / sizeof(buffer[0]); i++) {
