...
public:
    ...
    virtual void RecordMetadata(const MetadataRecorder::MetadataProvider& metadata_provider)
    {
    }
    ...
    // <= TimeTicks::Now().
    virtual void ApplyMetadataRetrospectively(TimeTicks period_start, TimeTicks period_end, const MetadataRecorder::Item& item)
    {
    }
    ...
    virtual void OnSampleCompleted(std::vector<Frame> frames, TimeTicks sample_timestamp) = 0;

    // Finishes the profile construction with |profile_duration| and
    // |sampling_period|. Invoked when sampling a profile completes.
    virtual void OnProfileCompleted(TimeDelta profile_duration, TimeDelta sampling_period) = 0;
};
