...
PeriodicSamplingScheduler::PeriodicSamplingScheduler(TimeDelta sampling_duration, double fraction_of_execution_time_to_sample, TimeTicks start_time)
    : period_duration_(sampling_duration / fraction_of_execution_time_to_sample)
...
TimeDelta PeriodicSamplingScheduler::GetTimeToNextCollection()
{
    ...
    const TimeDelta sampling_offset = (period_duration_ - sampling_duration_) * RandDouble();
