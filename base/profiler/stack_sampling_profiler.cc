...
class ScopedAllowThreadRecallForStackSamplingProfiler : public ScopedAllowBaseSyncPrimitivesOutsideBlockingScope { };
...
constexpr WaitableEvent::ResetPolicy kResetPolicy = WaitableEvent::ResetPolicy::MANUAL;
...
TimeTicks GetNextSampleTimeImpl(TimeTicks scheduled_current_sample_time, TimeDelta sampling_interval, TimeTicks now)
{
    ...
    const TimeDelta minimum_time_delta_to_next_sample = earliest_next_sample_time - scheduled_current_sample_time;
    ...
    const int64_t required_sampling_intervals = static_cast<int64_t>(std::ceil(minimum_time_delta_to_next_sample / sampling_interval));
    return scheduled_current_sample_time + required_sampling_intervals * sampling_interval;
}
...
public:
    ...
    private:
        ...
        static void ShutdownTaskAndSignalEvent(SamplingThread* sampler, int add_events, WaitableEvent* event);
    };

    struct CollectionContext {
        CollectionContext(PlatformThreadId thread_id, const SamplingParams& params, WaitableEvent* finished, std::unique_ptr<StackSampler> sampler)
        ...
        const raw_ptr<WaitableEvent> finished; // Signaled when all sampling complete.
    ...
    void ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash, std::optional<int64_t> key, int64_t value,
    ...
    void AddProfileMetadata(uint64_t name_hash, std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id);
...
private:
    ...
    scoped_refptr<SingleThreadTaskRunner> GetTaskRunner(ThreadExecutionState* out_state);
    ...
    void AddAuxUnwinderTask(int collection_id, std::unique_ptr<Unwinder> unwinder);
    void ApplyMetadataToPastSamplesTask(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash, std::optional<int64_t> key, int64_t value,
        std::optional<PlatformThreadId> thread_id);
    void AddProfileMetadataTask(uint64_t name_hash, std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id);
    ...
    ThreadExecutionState thread_execution_state_ GUARDED_BY(thread_execution_state_lock_) = NOT_STARTED;
    scoped_refptr<SingleThreadTaskRunner> thread_execution_state_task_runner_ GUARDED_BY(thread_execution_state_lock_);
    bool thread_execution_state_disable_idle_shutdown_for_testing_ GUARDED_BY(thread_execution_state_lock_) = false;
    ...
    int thread_execution_state_add_events_ GUARDED_BY(thread_execution_state_lock_) = 0;
};
...
// static
void StackSamplingProfiler::SamplingThread::TestPeer::ShutdownAssumingIdle(bool simulate_intervening_add)
{
    SamplingThread* sampler = SamplingThread::GetInstance();

    ThreadExecutionState state;
    scoped_refptr<SingleThreadTaskRunner> task_runner = sampler->GetTaskRunner(&state);
    ...
    WaitableEvent executed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    // PostTaskAndReply won't work because thread and associated message-loop may
    // be shut down.
    task_runner->PostTask(FROM_HERE, BindOnce(&ShutdownTaskAndSignalEvent, Unretained(sampler), add_events, Unretained(&executed)));
    executed.Wait();
}

// static
void StackSamplingProfiler::SamplingThread::TestPeer::ShutdownTaskAndSignalEvent(SamplingThread* sampler, int add_events, WaitableEvent* event)
{
    sampler->ShutdownTask(add_events);
    event->Signal();
}

AtomicSequenceNumber StackSamplingProfiler::SamplingThread::CollectionContext::next_collection_id;
...
StackSamplingProfiler::SamplingThread* StackSamplingProfiler::SamplingThread::GetInstance()
{
    return Singleton<SamplingThread, LeakySingletonTraits<SamplingThread>>::get();
}

int StackSamplingProfiler::SamplingThread::Add(std::unique_ptr<CollectionContext> collection)
{
    // This is not to be run on the sampling thread.

    int collection_id = collection->collection_id;
    scoped_refptr<SingleThreadTaskRunner> task_runner = GetOrCreateTaskRunnerForAdd();

    task_runner->PostTask(FROM_HERE, BindOnce(&SamplingThread::AddCollectionTask, Unretained(this), std::move(collection)));

    return collection_id;
}

void StackSamplingProfiler::SamplingThread::AddAuxUnwinder(int collection_id, std::unique_ptr<Unwinder> unwinder)
{
    ThreadExecutionState state;
    ...
    if (state != RUNNING)
        return;
    DCHECK(task_runner);
    task_runner->PostTask(FROM_HERE, BindOnce(&SamplingThread::AddAuxUnwinderTask, Unretained(this), collection_id, std::move(unwinder)));
}

void StackSamplingProfiler::SamplingThread::ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash,
    std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id)
{
    ThreadExecutionState state;
    scoped_refptr<SingleThreadTaskRunner> task_runner = GetTaskRunner(&state);
    if (state != RUNNING)
    ...
    task_runner->PostTask(
        FROM_HERE, BindOnce(&SamplingThread::ApplyMetadataToPastSamplesTask, Unretained(this), period_start, period_end, name_hash, key, value, thread_id));
}

void StackSamplingProfiler::SamplingThread::AddProfileMetadata(
    uint64_t name_hash, std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id)
{
    ThreadExecutionState state;
    scoped_refptr<SingleThreadTaskRunner> task_runner = GetTaskRunner(&state);
    if (state != RUNNING) {
    ...
    DCHECK(task_runner);
    task_runner->PostTask(FROM_HERE, BindOnce(&SamplingThread::AddProfileMetadataTask, Unretained(this), name_hash, key, value, thread_id));
}
...
void StackSamplingProfiler::SamplingThread::Remove(int collection_id)
{
    ...
    task_runner->PostTask(FROM_HERE, BindOnce(&SamplingThread::ScheduleCollectionStop, Unretained(this), collection_id));
}

scoped_refptr<SingleThreadTaskRunner> StackSamplingProfiler::SamplingThread::GetOrCreateTaskRunnerForAdd()
{
...
scoped_refptr<SingleThreadTaskRunner> StackSamplingProfiler::SamplingThread::GetTaskRunner(ThreadExecutionState* out_state)
{
    AutoLock lock(thread_execution_state_lock_);
    if (out_state)
...
scoped_refptr<SingleThreadTaskRunner> StackSamplingProfiler::SamplingThread::GetTaskRunnerOnSamplingThread()
{
...
void StackSamplingProfiler::SamplingThread::FinishCollection(std::unique_ptr<CollectionContext> collection)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
    DCHECK_EQ(0u, active_collections_.count(collection->collection_id));

    TimeDelta profile_duration = TimeTicks::Now() - collection->profile_start_time + collection->params.sampling_interval;

    collection->sampler->GetStackUnwindData()->profile_builder()->OnProfileCompleted(profile_duration, collection->params.sampling_interval);
...
void StackSamplingProfiler::SamplingThread::ScheduleShutdownIfIdle()
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::SamplingThread::ScheduleShutdownIfIdle");
    ...
    GetTaskRunnerOnSamplingThread()->PostDelayedTask(FROM_HERE, BindOnce(&SamplingThread::ShutdownTask, Unretained(this), add_events), Seconds(60));
}

void StackSamplingProfiler::SamplingThread::AddAuxUnwinderTask(int collection_id, std::unique_ptr<Unwinder> unwinder)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
...
void StackSamplingProfiler::SamplingThread::ApplyMetadataToPastSamplesTask(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash,
    std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
    ...
    for (auto& id_collection_pair : active_collections_) {
        if (thread_id && id_collection_pair.second->thread_id != thread_id)
            continue;
        id_collection_pair.second->sampler->GetStackUnwindData()->profile_builder()->ApplyMetadataRetrospectively(period_start, period_end, item);
    }
}
    ...
    uint64_t name_hash, std::optional<int64_t> key, int64_t value, std::optional<PlatformThreadId> thread_id)
{
    ...
    for (auto& id_collection_pair : active_collections_) {
        ...
        id_collection_pair.second->sampler->GetStackUnwindData()->profile_builder()->AddProfileMetadata(item);
    }
}

void StackSamplingProfiler::SamplingThread::AddCollectionTask(std::unique_ptr<CollectionContext> collection)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
    ...
    active_collections_.insert(std::make_pair(collection_id, std::move(collection)));

    GetTaskRunnerOnSamplingThread()->PostDelayedTask(FROM_HERE, BindOnce(&SamplingThread::RecordSampleTask, Unretained(this), collection_id), initial_delay);
...
void StackSamplingProfiler::SamplingThread::RemoveCollectionTask(int collection_id)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
...
void StackSamplingProfiler::SamplingThread::ScheduleCollectionStop(int collection_id)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
    ...
    collection->sampler->Stop(BindOnce(&SamplingThread::RemoveCollectionTask, Unretained(this), collection_id));
}

void StackSamplingProfiler::SamplingThread::RecordSampleTask(int collection_id)
{
    DCHECK_EQ(GetThreadId(), PlatformThread::CurrentId());
    ...
    bool more_collections_remaining = ++collection->sample_count < collection->params.samples_per_profile;
    // Record a single sample.
    collection->sampler->RecordStackFrames(stack_buffer_.get(), collection->thread_id,
        more_collections_remaining ? DoNothing() : BindOnce(&SamplingThread::RemoveCollectionTask, Unretained(this), collection_id));
    // Schedule the next sample recording if there is one.
    if (more_collections_remaining) {
        collection->next_sample_time = GetNextSampleTimeImpl(collection->next_sample_time, collection->params.sampling_interval, TimeTicks::Now());
        bool success = GetTaskRunnerOnSamplingThread()->PostDelayedTask(FROM_HERE, BindOnce(&SamplingThread::RecordSampleTask, Unretained(this), collection_id),
...
void StackSamplingProfiler::SamplingThread::ShutdownTask(int add_events)
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::SamplingThread::ShutdownTask");
...
// static
void StackSamplingProfiler::TestPeer::PerformSamplingThreadIdleShutdown(bool simulate_intervening_start)
{
    SamplingThread::TestPeer::ShutdownAssumingIdle(simulate_intervening_start);
}

// static
TimeTicks StackSamplingProfiler::TestPeer::GetNextSampleTime(TimeTicks scheduled_current_sample_time, TimeDelta sampling_interval, TimeTicks now)
{
    return GetNextSampleTimeImpl(scheduled_current_sample_time, sampling_interval, now);
}
...
bool StackSamplingProfiler::IsSupportedForCurrentPlatform()
{
    ...
    || (BUILDFLAG(IS_ANDROID) && ((defined(ARCH_CPU_ARMEL) && BUILDFLAG(ENABLE_ARM_CFI_TABLE)) || (defined(ARCH_CPU_ARM64))))                                  \
    || (BUILDFLAG(IS_CHROMEOS) && (defined(ARCH_CPU_X86_64) || defined(ARCH_CPU_ARM64)))
#if BUILDFLAG(IS_WIN)
...
StackSamplingProfiler::StackSamplingProfiler(SamplingProfilerThreadToken thread_token, const SamplingParams& params,
    std::unique_ptr<ProfileBuilder> profile_builder, UnwindersFactory core_unwinders_factory, RepeatingClosure record_sample_callback,
    StackSamplerTestDelegate* test_delegate)
    : thread_token_(thread_token)
    , params_(params)
    , sampler_(StackSampler::Create(thread_token, std::make_unique<StackUnwindData>(std::move(profile_builder)), std::move(core_unwinders_factory),
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::StackSamplingProfiler");
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::Start");
    ...
    static_assert(kResetPolicy == WaitableEvent::ResetPolicy::MANUAL, "The reset policy must be set to MANUAL");
    ...
    profiler_id_ = SamplingThread::GetInstance()->Add(
        std::make_unique<SamplingThread::CollectionContext>(thread_token_.id, params_, &profiling_inactive_, std::move(sampler_)));
    DCHECK_NE(kNullProfilerId, profiler_id_);

    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::Started", "profiler_id", profiler_id_);
}

void StackSamplingProfiler::Stop()
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "StackSamplingProfiler::Stop", "profiler_id", profiler_id_);
...
void StackSamplingProfiler::AddAuxUnwinder(std::unique_ptr<Unwinder> unwinder)
{
    ...
    SamplingThread::GetInstance()->AddAuxUnwinder(profiler_id_, std::move(unwinder));
}

// static
void StackSamplingProfiler::ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash, std::optional<int64_t> key,
    int64_t value, std::optional<PlatformThreadId> thread_id)
{
    SamplingThread::GetInstance()->ApplyMetadataToPastSamples(period_start, period_end, name_hash, key, value, thread_id);
}

// static
void StackSamplingProfiler::AddProfileMetadata(uint64_t name_hash, int64_t key, int64_t value, std::optional<PlatformThreadId> thread_id)
{
    SamplingThread::GetInstance()->AddProfileMetadata(name_hash, key, value, thread_id);
}
