...
public:
    TestScheduler(TimeDelta sampling_duration, double fraction_of_execution_time_to_sample)
        : PeriodicSamplingScheduler(sampling_duration, fraction_of_execution_time_to_sample, kStartTime)
    {
    ...
    void SetRandDouble(double value)
    {
        rand_double_value_ = value;
    }
    SimpleTestTickClock& tick_clock()
    {
        return tick_clock_;
    }
...
TEST(PeriodicSamplingSchedulerTest, ScheduleCollections)
{
    ...
    const TimeDelta expected_period = sampling_duration / fraction_of_execution_time_to_sample;

    TestScheduler scheduler(sampling_duration, fraction_of_execution_time_to_sample);
    ...
    EXPECT_EQ(2 * expected_period - sampling_duration, scheduler.GetTimeToNextCollection());
    ...
    EXPECT_EQ(2 * expected_period + 0.25 * (expected_period - sampling_duration), scheduler.GetTimeToNextCollection());
}
...
TEST(PeriodicSamplingSchedulerTest, ScheduleWithJumpInTimeTicks)
{
    ...
    EXPECT_EQ(0.5 * (expected_period - sampling_duration), scheduler.GetTimeToNextCollection());
}
