...
bool operator==(const MetadataRecorder::Item& lhs, const MetadataRecorder::Item& rhs)
{
...
TEST(MetadataRecorderTest, GetItems_Empty)
{
    ...
    EXPECT_EQ(0u, MetadataRecorder::MetadataProvider(&recorder, PlatformThread::CurrentId()).GetItems(&items));
    EXPECT_EQ(0u, MetadataRecorder::MetadataProvider(&recorder, PlatformThread::CurrentId()).GetItems(&items));
}
...
TEST(MetadataRecorderTest, Set_NewNameHash)
{
    ...
    size_t item_count;
    {
        item_count = MetadataRecorder::MetadataProvider(&recorder, PlatformThread::CurrentId()).GetItems(&items);
...
TEST(MetadataRecorderTest, Set_ExistingNameNash)
{
    ...
    MetadataRecorder::ItemArray items;
    size_t item_count = MetadataRecorder::MetadataProvider(&recorder, PlatformThread::CurrentId()).GetItems(&items);
    ASSERT_EQ(1u, item_count);
    EXPECT_EQ(10u, items[0].name_hash);
    EXPECT_FALSE(items[0].key.has_value());
    EXPECT_FALSE(items[0].thread_id.has_value());
...
TEST(MetadataRecorderTest, Set_ThreadIdIsScoped)
{
    ...
    {
        item_count = MetadataRecorder::MetadataProvider(&recorder, kInvalidThreadId).GetItems(&items);
...
TEST(MetadataRecorderTest, Remove_NulloptThreadIsIndependentOfNonNulloptThread)
{
    MetadataRecorder recorder;
...
TEST(MetadataRecorderTest, ReclaimInactiveSlots)
{
    ...
    for (size_t i = 0; i < MetadataRecorder::MAX_METADATA_COUNT; ++i) {
        ...
        items_set.insert(MetadataRecorder::Item { i, std::nullopt, std::nullopt, static_cast<int64_t>(i) });
    }
    ...
    for (size_t i = 3; i < MetadataRecorder::MAX_METADATA_COUNT; i += 4) {
        ...
        ++entries_removed;
        items_set.erase(MetadataRecorder::Item { i, std::nullopt, std::nullopt, static_cast<int64_t>(i) });
    }
    ...
    for (size_t i = 1; i <= entries_removed; ++i) {
        ...
        items_set.insert(MetadataRecorder::Item { i * 100, std::nullopt, std::nullopt, static_cast<int64_t>(i * 100) });
    }
    ...
    size_t recorder_item_count = MetadataRecorder::MetadataProvider(&recorder, PlatformThread::CurrentId()).GetItems(&recorder_items);
