...
PRUNTIME_FUNCTION TestUnwindFunctions::LookupFunctionEntry(DWORD64 program_counter, PDWORD64 image_base)
{
...
void TestUnwindFunctions::VirtualUnwind(DWORD64 image_base, DWORD64 program_counter, PRUNTIME_FUNCTION runtime_function, CONTEXT* context)
{
    ASSERT_NE(kInvalidRuntimeFunction, runtime_function) << "expected call to SetHasRuntimeFunction() or SetNoRuntimeFunction() "
...
void TestUnwindFunctions::SetHasRuntimeFunction(CONTEXT* context)
{
    ...
    expected_program_counter_ = next_image_base_ + runtime_function.BeginAddress + 8;
...
public:
    ...
    Win32StackFrameUnwinderTest& operator=(const Win32StackFrameUnwinderTest&) = delete;
...
std::unique_ptr<Win32StackFrameUnwinder> Win32StackFrameUnwinderTest::CreateUnwinder()
{
    std::unique_ptr<TestUnwindFunctions> unwind_functions(new TestUnwindFunctions);
