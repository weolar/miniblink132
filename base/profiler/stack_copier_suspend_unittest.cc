...
public:
    class TestScopedSuspendThread : public SuspendableThreadDelegate::ScopedSuspendThread {
    public:
    ...
    TestSuspendableThreadDelegate& operator=(const TestSuspendableThreadDelegate&) = delete;
    ...
    bool GetThreadContext(RegisterContext* thread_context) override
    {
        ...
        RegisterContextStackPointer(thread_context) = reinterpret_cast<uintptr_t>(&(*fake_stack_)[0]);
        RegisterContextInstructionPointer(thread_context) = reinterpret_cast<uintptr_t>((*fake_stack_)[0]);
        return true;
    }
...
public:
    ...
    bool on_stack_copy_was_invoked() const
    {
        return on_stack_copy_was_invoked_;
    }
...
TEST(StackCopierSuspendTest, CopyStack)
{
    ...
    StackCopierSuspend stack_copier_suspend(std::make_unique<TestSuspendableThreadDelegate>(stack));
    ...
    stack_copier_suspend.CopyStack(stack_buffer.get(), &stack_top, &timestamp, &register_context, &stack_copier_delegate);

    uintptr_t* stack_copy_bottom = reinterpret_cast<uintptr_t*>(stack_buffer.get()->buffer());
    std::vector<uintptr_t> stack_copy(stack_copy_bottom, stack_copy_bottom + stack.size());
...
TEST(StackCopierSuspendTest, CopyStackBufferTooSmall)
{
    ...
    std::unique_ptr<StackBuffer> stack_buffer = std::make_unique<StackBuffer>((stack.size() - 1) * sizeof(stack[0]));
    // Make the buffer different than the input stack.
    constexpr uintptr_t kBufferInitializer = 100;
    size_t stack_buffer_elements = stack_buffer->size() / sizeof(stack_buffer->buffer()[0]);
    std::fill_n(stack_buffer->buffer(), stack_buffer_elements, kBufferInitializer);
    uintptr_t stack_top = 0;
    TimeTicks timestamp;
    RegisterContext register_context {};
    TestStackCopierDelegate stack_copier_delegate;
    ...
    uintptr_t* stack_copy_bottom = reinterpret_cast<uintptr_t*>(stack_buffer.get()->buffer());
    std::vector<uintptr_t> stack_copy(stack_copy_bottom, stack_copy_bottom + stack_buffer_elements);
...
TEST(StackCopierSuspendTest, CopyStackAndRewritePointers)
{
    ...
    uintptr_t* stack_copy_bottom = reinterpret_cast<uintptr_t*>(stack_buffer.get()->buffer());
    std::vector<uintptr_t> stack_copy(stack_copy_bottom, stack_copy_bottom + stack.size());
    EXPECT_THAT(stack_copy, ElementsAre(reinterpret_cast<uintptr_t>(stack_copy_bottom), reinterpret_cast<uintptr_t>(stack_copy_bottom) + sizeof(uintptr_t)));
}
...
TEST(StackCopierSuspendTest, RewriteRegisters)
{
    ...
    RegisterContext register_context {};
    TestStackCopierDelegate stack_copier_delegate;
    RegisterContextFramePointer(&register_context) = reinterpret_cast<uintptr_t>(&stack[1]);
    StackCopierSuspend stack_copier_suspend(std::make_unique<TestSuspendableThreadDelegate>(stack, &register_context));
    ...
    uintptr_t stack_copy_bottom = reinterpret_cast<uintptr_t>(stack_buffer.get()->buffer());
    EXPECT_EQ(stack_copy_bottom + sizeof(uintptr_t), RegisterContextFramePointer(&register_context));
}
