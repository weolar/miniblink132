...
MetadataRecorder::Item::Item(uint64_t name_hash, std::optional<int64_t> key, std::optional<PlatformThreadId> thread_id, int64_t value)
    : name_hash(name_hash)
    , key(key)
    , thread_id(thread_id)
    , value(value)
...
MetadataRecorder::Item& MetadataRecorder::Item::Item::operator=(const Item& other) = default;
...
void MetadataRecorder::Set(uint64_t name_hash, std::optional<int64_t> key, std::optional<PlatformThreadId> thread_id, int64_t value)
{
    ...
    for (size_t i = 0; i < item_slots_used; ++i) {
        auto& item = items_[i];
        if (item.name_hash == name_hash && item.key == key && item.thread_id == thread_id) {
            item.value.store(value, std::memory_order_relaxed);

            const bool was_active = item.is_active.exchange(true, std::memory_order_release);
            if (!was_active)
...
void MetadataRecorder::Remove(uint64_t name_hash, std::optional<int64_t> key, std::optional<PlatformThreadId> thread_id)
{
    AutoLock lock(write_lock_);
    ...
    for (size_t i = 0; i < item_slots_used; ++i) {
        ...
        if (item.name_hash == name_hash && item.key == key && item.thread_id == thread_id) {
            ...
            const bool was_active = item.is_active.exchange(false, std::memory_order_relaxed);
            if (was_active)
                inactive_item_count_++;
...
MetadataRecorder::MetadataProvider::MetadataProvider(MetadataRecorder* metadata_recorder, PlatformThreadId thread_id)
...
size_t MetadataRecorder::MetadataProvider::GetItems(ItemArray* const items) const
{
    return metadata_recorder_->GetItems(items, thread_id_);
}

size_t MetadataRecorder::GetItems(ItemArray* const items, PlatformThreadId thread_id) const
{
    ...
    for (size_t read_index = 0; read_index < item_slots_used; ++read_index) {
        ...
        if (item.is_active.load(std::memory_order_acquire) && (!item.thread_id.has_value() || item.thread_id == thread_id)) {
            (*items)[write_index++] = Item { item.name_hash, item.key, item.thread_id, item.value.load(std::memory_order_relaxed) };
        }
    }
...
size_t MetadataRecorder::ReclaimInactiveSlots(size_t item_slots_used)
{
    ...
    while (first_inactive_item_idx < last_active_item_idx) {
        ...
        inactive_item.value.store(active_item.value.load(std::memory_order_relaxed), std::memory_order_relaxed);
