...
{
    std::optional<uintptr_t> base = GetThreadStackBaseAddress(PlatformThread::CurrentId(), pthread_self());
...
TEST(GetThreadStackBaseAddressTest, MainThread)
{
    ...
    // platforms.
    std::optional<uintptr_t> base = GetThreadStackBaseAddress(GetCurrentProcId(), pthread_t());
    EXPECT_THAT(base, Optional(Gt(0u)));
}
