...
public:
    ...
    using UnwindersFactory = OnceCallback<std::vector<std::unique_ptr<Unwinder>>()>;
    ...
    static std::unique_ptr<StackSampler> Create(SamplingProfilerThreadToken thread_token, std::unique_ptr<StackUnwindData> stack_unwind_data,
    ...
    void RecordStackFrames(StackBuffer* stack_buffer, PlatformThreadId thread_id, OnceClosure done_callback);
    ...
    static std::unique_ptr<StackSampler> CreateForTesting(std::unique_ptr<StackCopier> stack_copier, std::unique_ptr<StackUnwindData> stack_unwind_data,
        UnwindersFactory core_unwinders_factory, RepeatingClosure record_sample_callback = RepeatingClosure(),
...
private:
    FRIEND_TEST_ALL_PREFIXES(StackSamplerTest, AuxUnwinderInvokedWhileRecordingStackFrames);

    StackSampler(std::unique_ptr<StackCopier> stack_copier, std::unique_ptr<StackUnwindData> stack_unwind_data, UnwindersFactory core_unwinders_factory,
        RepeatingClosure record_sample_callback, StackSamplerTestDelegate* test_delegate);

    static std::vector<Frame> WalkStack(
        ModuleCache* module_cache, RegisterContext* thread_context, uintptr_t stack_top, std::vector<UnwinderCapture> unwinders);

    void UnwindComplete(TimeTicks timestamp, OnceClosure done_callback, std::vector<Frame> frames);
