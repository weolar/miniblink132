...
StackUnwindData::StackUnwindData(std::unique_ptr<ProfileBuilder> profile_builder)
...
std::vector<UnwinderCapture> StackUnwindData::GetUnwinderSnapshot()
{
    ...
    for (const auto& unwinder : unwinders_) {
        unwinders.emplace_back(unwinder.get(), unwinder->CreateUnwinderStateCapture());
    }
    return unwinders;
}

void StackUnwindData::Initialize(std::vector<std::unique_ptr<Unwinder>> unwinders)
{
    ...
    unwinders_.insert(unwinders_.end(), std::make_move_iterator(unwinders.rbegin()), std::make_move_iterator(unwinders.rend()));
