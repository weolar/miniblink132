...
void StackBuffer::MarkUpperBufferContentsAsUnneeded(size_t retained_bytes)
{
    ...
    uint8_t* start_of_discard = reinterpret_cast<uint8_t*>(buffer_.get()) + actual_retained_bytes;
    ...
    , buffer_(static_cast<uintptr_t*>(AlignedAlloc(size_, GetPageSize())))
{
    ...
    : size_(buffer_size)
    , buffer_(static_cast<uintptr_t*>(AlignedAlloc(size_, kPlatformStackAlignment)))
{
