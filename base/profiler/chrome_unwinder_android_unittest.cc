...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerIncrementMinValue)
{
    ...
    ASSERT_EQ(ExecuteUnwindInstruction(current_instruction, pc_was_updated, &thread_context), UnwindInstructionResult::kInstructionPending);
    EXPECT_FALSE(pc_was_updated);
    ASSERT_EQ(current_instruction, &instruction + 1);
    EXPECT_EQ(0x10000004ul, thread_context.arm_sp);
}

TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerIncrementMidValue)
{
...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerIncrementMaxValue)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerIncrementOverflow)
{
    RegisterContext thread_context = {};
    const uint8_t instruction = 0b00111111;
    const uint8_t* current_instruction = &instruction;
    ...
    bool pc_was_updated = false;
    ASSERT_EQ(ExecuteUnwindInstruction(current_instruction, pc_was_updated, &thread_context), UnwindInstructionResult::kAborted);
    ASSERT_EQ(current_instruction, &instruction + 1);
    EXPECT_EQ(0xffffffff, thread_context.arm_sp);
}

TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerDecrementMinValue)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerDecrementMidValue)
{
    // xxxxxx = 4; vsp = vsp - (4 << 2) - 4 = vsp - 16 - 4 = vsp - 0x14.
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerDecrementMaxValue)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestSmallStackPointerDecrementUnderflow)
{
    RegisterContext thread_context = {};
    const uint8_t instruction = 0b01111111;
    const uint8_t* current_instruction = &instruction;
...
using ChromeAndroidUnwindSetStackPointerFromRegisterValueTest = ::testing::TestWithParam<uint8_t>;

INSTANTIATE_TEST_SUITE_P(All, ChromeAndroidUnwindSetStackPointerFromRegisterValueTest,
...
TEST_P(ChromeAndroidUnwindSetStackPointerFromRegisterValueTest, TestSetStackPointerFromRegisterValue)
{
...
TEST(ChromeAndroidUnwindInstructionTest, TestCompleteWithNoPriorPCUpdate)
{
    ...
    const uint8_t* current_instruction = &instruction;
    bool pc_was_updated = false;
    ASSERT_EQ(ExecuteUnwindInstruction(current_instruction, pc_was_updated, &thread_context), UnwindInstructionResult::kCompleted);
    ASSERT_EQ(current_instruction, &instruction + 1);
...
TEST(ChromeAndroidUnwindInstructionTest, TestPopDiscontinuousRegistersIncludingPC)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestPopDiscontinuousRegistersOverflow)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestPopRegistersIncludingR14MinRegisters)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestPopRegistersIncludingR14MidRegisters)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestPopRegistersIncludingR14MaxRegisters)
{
    RegisterContext thread_context = {};
...
TEST(ChromeAndroidUnwindInstructionTest, TestBigStackPointerIncrementLargeValue)
{
    RegisterContext thread_context = {};
    thread_context.arm_sp = 0x10000000;
...
TEST(ChromeAndroidUnwindInstructionTest, TestBigStackPointerIncrementOverflow)
{
    ...
    ASSERT_EQ(ExecuteUnwindInstruction(current_instruction, pc_was_updated, &thread_context), UnwindInstructionResult::kAborted);
    EXPECT_FALSE(pc_was_updated);
    ASSERT_EQ(current_instruction, increment_overflow + sizeof(increment_overflow));
    EXPECT_EQ(0xfffffffful, thread_context.arm_sp);
}

TEST(ChromeUnwinderAndroidTest, TestFunctionOffsetTableLookupExactMatchingOffset)
{
    ...
    EXPECT_EQ(3ul,
        GetFirstUnwindInstructionIndexFromFunctionOffsetTableEntry(&function_offset_table[0],
            /* instruction_offset_from_function_start */ 128));
}

TEST(ChromeUnwinderAndroidTest, TestFunctionOffsetTableLookupNonExactMatchingOffset)
{
    const uint8_t function_offset_table[] = {
        // Function 1: [(130, 2), (128, 3), (0, 4)]
        // offset = 130
...
TEST(ChromeUnwinderAndroidTest, TestAddressTableLookupEntryInPage)
{
...

    {
        ...
        const auto entry_found = GetFunctionTableIndexFromInstructionOffset(page_start_instructions, function_offset_table_indices,
            /* instruction_offset */ (page_instruction_offset << 1) + (page_number << 17));
...
TEST(ChromeUnwinderAndroidTest, TestAddressTableLookupOnSecondPageOfFunctionSpanningPageBoundary)
{
    const uint32_t page_start_instructions[] = { 0, 1, 2 };
    const FunctionTableEntry function_offset_table_indices[] = { // Page 0
        {
            /* function_start_address_page_instruction_offset */ 0,
            /* function_offset_table_byte_index */ 20,
        },
...
TEST(ChromeUnwinderAndroidTest, TestAddressTableLookupWithinFunctionSpanningMultiplePages)
{
...
const ModuleCache::Module* AddNativeModule(ModuleCache* cache, std::unique_ptr<const ModuleCache::Module> module)
{
...
TEST(ChromeUnwinderAndroidTest, CanUnwindFrom)
{
    ...
    ChromeUnwinderAndroid unwinder(dummy_unwind_info, chrome_module->GetBaseAddress(),
...
namespace {
void ExpectFramesEq(const std::vector<Frame>& expected, const std::vector<Frame>& actual)
{
...
public:
    AlignedStackMemory(std::initializer_list<uintptr_t> values)
        : size_(values.size())
        , stack_memory_(static_cast<uintptr_t*>(AlignedAlloc(size_ * sizeof(uintptr_t), 2 * sizeof(uintptr_t))))
    {
...
TEST(ChromeUnwinderAndroidTest, TryUnwind)
{
    ...
    const ModuleCache::Module* chrome_module
        = AddNativeModule(&module_cache, std::make_unique<TestModule>(0x1000, number_of_pages * page_size, "ChromeModule"));

    uintptr_t text_section_start_address = 0x1100;
    ChromeUnwinderAndroid unwinder(unwind_info, chrome_module->GetBaseAddress(), text_section_start_address);
    ...
    EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder.TryUnwind(/*capture_state=*/nullptr, &context, stack_memory.stack_end_address(), &unwound_frames));
    ExpectFramesEq(std::vector<Frame>({ { first_pc, chrome_module }, { second_pc, chrome_module }, { third_pc, nullptr } }), unwound_frames);
}
...
TEST(ChromeUnwinderAndroidTest, TryUnwindInfiniteLoopSingleFrame)
{
    ...
    EXPECT_EQ(UnwindResult::kAborted, unwinder.TryUnwind(/*capture_state=*/nullptr, &context, stack_memory.stack_end_address(), &unwound_frames));
...
TEST(ChromeUnwinderAndroidTest, TryUnwindInfiniteLoopMultipleFrames)
{
    ...
    EXPECT_EQ(UnwindResult::kAborted, unwinder.TryUnwind(/*capture_state=*/nullptr, &context, stack_memory.stack_end_address(), &unwound_frames));
    ExpectFramesEq(std::vector<Frame>({ { first_pc, chrome_module }, { second_pc, chrome_module } }), unwound_frames);
}
...
TEST(ChromeUnwinderAndroidTest, TryUnwindUnalignedSPFrameUnwind)
{
    ...
    RegisterContextStackPointer(&context) = stack_memory.stack_start_address() + sizeof(uintptr_t);
    ...
    context.arm_lr = text_section_start_address + (number_of_pages + 1) * page_size;
