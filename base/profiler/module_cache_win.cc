...
void GetDebugInfoForModule(HMODULE module_handle, std::string* build_id, FilePath* pdb_name)
{
    GUID guid;
    DWORD age;
    LPCSTR pdb_file = nullptr;
    size_t pdb_file_length = 0;
    if (!win::PEImage(module_handle).GetDebugId(&guid, &age, &pdb_file, &pdb_file_length)) {
        return;
    }
...
bool IsValidUserSpaceAddress(uintptr_t address)
{
    ...
    return reinterpret_cast<LPVOID>(address) >= min_app_addr && reinterpret_cast<LPVOID>(address) <= max_app_addr;
}
...
public:
    ...
    static bool CloseHandle(HMODULE handle)
    {
        return ::FreeLibrary(handle) != 0;
    }
    static bool IsHandleValid(HMODULE handle)
    {
        return handle != nullptr;
    }
    static HMODULE NullHandle()
    {
        return nullptr;
    }
};
...
public:
    WindowsModule(ScopedModuleHandle module_handle, const MODULEINFO module_info, const std::string& id, const FilePath& debug_basename)
    ...
    {
        return module_info_.SizeOfImage;
    }
...
ScopedModuleHandle GetModuleHandleForAddress(uintptr_t address)
{
    ...
    if (!::GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, reinterpret_cast<LPCTSTR>(address), &module_handle)) {
...
std::unique_ptr<ModuleCache::Module> CreateModuleForHandle(ScopedModuleHandle module_handle)
{
    FilePath pdb_name;
    std::string build_id;
    ...
    if (!::GetModuleInformation(GetCurrentProcessHandle(), module_handle.get(), &module_info, sizeof(module_info))) {
        return nullptr;
    }

    return std::make_unique<WindowsModule>(std::move(module_handle), module_info, build_id, pdb_name);
}
