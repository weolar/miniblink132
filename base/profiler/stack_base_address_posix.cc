...
uintptr_t GetThreadStackBaseAddressImpl(pthread_t pthread_id)
{
    ...
    CHECK_EQ(result, 0) << "pthread_getattr_np returned " << logging::SystemErrorCodeToString(result);
    // See crbug.com/617730 for limitations of this approach on Linux-like
    // systems.
    void* address;
    size_t size;
    result = pthread_attr_getstack(&attr, &address, &size);
    CHECK_EQ(result, 0) << "pthread_attr_getstack returned " << logging::SystemErrorCodeToString(result);
...
std::optional<uintptr_t> GetThreadStackBaseAddress(PlatformThreadId id, pthread_t pthread_id)
{
#if BUILDFLAG(IS_LINUX)
    ...
    if (is_main_thread) {
        ...
        static const std::optional<uintptr_t> main_thread_base_address = GetAndroidMainThreadStackBaseAddressImpl();
