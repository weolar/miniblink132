...
public:
    ...
    void OnSampleCompleted(std::vector<Frame> frames, TimeTicks sample_timestamp) override
    {
...
public:
    TestStackCopier(const std::vector<uintptr_t>& fake_stack, TimeTicks timestamp = TimeTicks())
        : fake_stack_(fake_stack)
        , timestamp_(timestamp)
    {
    }

    bool CopyStack(StackBuffer* stack_buffer, uintptr_t* stack_top, TimeTicks* timestamp, RegisterContext* thread_context, Delegate* delegate) override
    {
        std::memcpy(stack_buffer->buffer(), &(*fake_stack_)[0], fake_stack_->size() * sizeof((*fake_stack_)[0]));
        *stack_top = reinterpret_cast<uintptr_t>(stack_buffer->buffer() + fake_stack_->size());
        // Set the stack pointer to be consistent with the copied stack.
        *thread_context = {};
        RegisterContextStackPointer(thread_context) = reinterpret_cast<uintptr_t>(stack_buffer->buffer());
...
protected:
    std::vector<uintptr_t*> GetRegistersToRewrite(RegisterContext* thread_context) override
    {
...
public:
    bool CopyStack(StackBuffer* stack_buffer, uintptr_t* stack_top, TimeTicks* timestamp, RegisterContext* thread_context, Delegate* delegate) override
    {
        ...
        *stack_top = reinterpret_cast<uintptr_t>(stack_buffer->buffer() + stack_buffer->size());
        // Set the stack pointer to be consistent with the copied stack.
        *thread_context = {};
        RegisterContextStackPointer(thread_context) = reinterpret_cast<uintptr_t>(stack_buffer->buffer());
...
public:
    ...
    {
        auto* bottom = reinterpret_cast<uintptr_t*>(RegisterContextStackPointer(thread_context));
        *stack_copy_ = std::vector<uintptr_t>(bottom, reinterpret_cast<uintptr_t*>(stack_top));
...
std::vector<std::unique_ptr<const ModuleCache::Module>> ToModuleVector(std::unique_ptr<const ModuleCache::Module> module)
{
    return std::vector<std::unique_ptr<const ModuleCache::Module>>(std::make_move_iterator(&module), std::make_move_iterator(&module + 1));
}
...
void InjectModuleForContextInstructionPointer(const std::vector<uintptr_t>& stack, ModuleCache* module_cache)
{
    module_cache->AddCustomNativeModule(std::make_unique<TestModule>(stack[0], sizeof(uintptr_t)));
}
...
public:
    struct Result {
        Result(bool can_unwind)
            ...
            , result(UnwindResult::kUnrecognizedFrame)
        {
        }
    ...
    UnwindResult TryUnwind(UnwinderStateCapture* capture_state, RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* stack) override
    {
        ...
        for (const auto instruction_pointer : current_result.instruction_pointers)
            stack->emplace_back(instruction_pointer, module_cache()->GetModuleForAddress(instruction_pointer));
...
StackSampler::UnwindersFactory MakeUnwindersFactory(std::unique_ptr<Unwinder> unwinder)
{
    return BindOnce(
...
std::vector<UnwinderCapture> MakeUnwinderStateVector(Unwinder* native_unwinder, Unwinder* aux_unwinder)
{
...
TEST_F(StackSamplerTest, CopyStack)
{
    ...
    auto unwind_data = std::make_unique<StackUnwindData>(std::make_unique<TestProfileBuilder>(&module_cache_));
    ...
    std::unique_ptr<StackSampler> stack_sampler = StackSampler::CreateForTesting(
        std::make_unique<TestStackCopier>(stack), std::move(unwind_data), MakeUnwindersFactory(std::make_unique<TestUnwinder>(&stack_copy)));

    stack_sampler->Initialize();

    std::unique_ptr<StackBuffer> stack_buffer = std::make_unique<StackBuffer>(stack.size() * sizeof(uintptr_t));
    stack_sampler->RecordStackFrames(stack_buffer.get(), PlatformThread::CurrentId(), sample_completed.GetCallback());
...
TEST_F(StackSamplerTest, RecordStackFramesUMAMetric)
{
    ...
    for (uint32_t i = 0; i < StackSampler::kUMAHistogramDownsampleAmount - 1; i++) {
        stack_sampler->RecordStackFrames(stack_buffer.get(), PlatformThread::CurrentId(), DoNothing());

        // Should have no new samples in the
        // Memory.StackSamplingProfiler.StackSampleSize2 histogram.
        histogram_tester.ExpectUniqueSample("Memory.StackSamplingProfiler.StackSampleSize2", kExpectedSizeKB, 0);
    }

    stack_sampler->RecordStackFrames(stack_buffer.get(), PlatformThread::CurrentId(), sample_completed.GetCallback());
    ASSERT_TRUE(sample_completed.Wait());
    histogram_tester.ExpectUniqueSample("Memory.StackSamplingProfiler.StackSampleSize2", kExpectedSizeKB, 1);
}
...
TEST_F(StackSamplerTest, CopyStackTimestamp)
{
    ...
    std::unique_ptr<StackSampler> stack_sampler = StackSampler::CreateForTesting(
        std::make_unique<TestStackCopier>(stack, timestamp), std::move(unwind_data), MakeUnwindersFactory(std::make_unique<TestUnwinder>(&stack_copy)));
    ...
    ASSERT_TRUE(sample_completed.Wait());
    EXPECT_EQ(timestamp, static_cast<TestProfileBuilder*>(stack_sampler->GetStackUnwindData()->profile_builder())->last_timestamp());
}
...
TEST_F(StackSamplerTest, UnwinderInvokedWhileRecordingStackFrames)
{
    ...
    auto unwind_data = std::make_unique<StackUnwindData>(std::make_unique<TestProfileBuilder>(&module_cache_));
    std::unique_ptr<StackSampler> stack_sampler = StackSampler::CreateForTesting(
        std::make_unique<DelegateInvokingStackCopier>(), std::move(unwind_data), MakeUnwindersFactory(std::move(owned_unwinder)));
...
TEST_F(StackSamplerTest, AuxUnwinderInvokedWhileRecordingStackFrames)
{
    ...
    auto unwind_data = std::make_unique<StackUnwindData>(std::make_unique<TestProfileBuilder>(&module_cache_));
    std::unique_ptr<StackSampler> stack_sampler = StackSampler::CreateForTesting(
        std::make_unique<DelegateInvokingStackCopier>(), std::move(unwind_data), MakeUnwindersFactory(std::make_unique<CallRecordingUnwinder>()));
    ...
    if (stack_sampler->thread_pool_runner_) {
        stack_sampler->thread_pool_runner_->PostTaskAndReply(FROM_HERE, base::DoNothing(), unwinder_added.GetCallback());
...
TEST_F(StackSamplerTest, WalkStack_Completed)
{
    ...
    RegisterContextInstructionPointer(&thread_context) = GetTestInstructionPointer();
    module_cache_.AddCustomNativeModule(std::make_unique<TestModule>(1u, 1u));
    auto native_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kCompleted, { 1u } } }));
    native_unwinder->Initialize(&module_cache_);

    std::vector<Frame> stack = StackSampler::WalkStackForTesting(&module_cache_, &thread_context, 0u, MakeUnwinderStateVector(native_unwinder.get(), nullptr));
...
TEST_F(StackSamplerTest, WalkStack_Aborted)
{
    ...
    RegisterContextInstructionPointer(&thread_context) = GetTestInstructionPointer();
    module_cache_.AddCustomNativeModule(std::make_unique<TestModule>(1u, 1u));
    auto native_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kAborted, { 1u } } }));
    native_unwinder->Initialize(&module_cache_);
...
TEST_F(StackSamplerTest, WalkStack_NotUnwound)
{
    RegisterContext thread_context;
    RegisterContextInstructionPointer(&thread_context) = GetTestInstructionPointer();
    auto native_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kUnrecognizedFrame, {} } }));
    native_unwinder->Initialize(&module_cache_);
...
TEST_F(StackSamplerTest, WalkStack_AuxUnwind)
{
    ...
    // non-native module.
    module_cache_.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(GetTestInstructionPointer(), 1u, false)));

    auto aux_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kAborted, { 1u } } }));
    aux_unwinder->Initialize(&module_cache_);
    std::vector<Frame> stack = StackSampler::WalkStackForTesting(&module_cache_, &thread_context, 0u, MakeUnwinderStateVector(nullptr, aux_unwinder.get()));
...
TEST_F(StackSamplerTest, WalkStack_AuxThenNative)
{
    ...
    // Treat the context instruction pointer as being in the aux unwinder's
    // non-native module.
    module_cache_.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(0u, 1u, false)));
    // Inject a fake native module for the second frame.
    module_cache_.AddCustomNativeModule(std::make_unique<TestModule>(1u, 1u));

    auto aux_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kUnrecognizedFrame, { 1u } }, false }));
    aux_unwinder->Initialize(&module_cache_);
    auto native_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kCompleted, { 2u } } }));
    native_unwinder->Initialize(&module_cache_);

    std::vector<Frame> stack
        = StackSampler::WalkStackForTesting(&module_cache_, &thread_context, 0u, MakeUnwinderStateVector(native_unwinder.get(), aux_unwinder.get()));
...
TEST_F(StackSamplerTest, WalkStack_NativeThenAux)
{
    ...
    // module.
    module_cache_.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(1u, 1u, false)));

    auto aux_unwinder = WrapUnique(new FakeTestUnwinder({ { false }, { UnwindResult::kUnrecognizedFrame, { 2u } }, { false } }));
    aux_unwinder->Initialize(&module_cache_);
    auto native_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kUnrecognizedFrame, { 1u } }, { UnwindResult::kCompleted, { 3u } } }));
    native_unwinder->Initialize(&module_cache_);

    std::vector<Frame> stack = StackSampler::WalkStackForTesting(&module_cache_,

        &thread_context, 0u, MakeUnwinderStateVector(native_unwinder.get(), aux_unwinder.get()));
