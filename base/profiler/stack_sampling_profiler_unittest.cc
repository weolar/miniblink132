...
// initialized.
    ...
    || (BUILDFLAG(IS_CHROMEOS) && (defined(ARCH_CPU_X86_64) || defined(ARCH_CPU_ARM64)) && !defined(MEMORY_SANITIZER))
...
#else
#define PROFILER_TEST_F(TestClass, TestName) TEST_F(TestClass, DISABLED_##TestName)
#endif
...
public:
    TestProfileBuilder(ModuleCache* module_cache, ProfileCompletedCallback callback);
    ...
    void RecordMetadata(const MetadataRecorder::MetadataProvider& metadata_provider) override;
    void ApplyMetadataRetrospectively(TimeTicks period_start, TimeTicks period_end, const MetadataRecorder::Item& item) override;
    void AddProfileMetadata(const MetadataRecorder::Item& item) override;
    void OnSampleCompleted(std::vector<Frame> sample, TimeTicks sample_timestamp) override;
    void OnProfileCompleted(TimeDelta profile_duration, TimeDelta sampling_period) override;
...
TestProfileBuilder::TestProfileBuilder(ModuleCache* module_cache, ProfileCompletedCallback callback)
...
void TestProfileBuilder::RecordMetadata(const MetadataRecorder::MetadataProvider& metadata_provider)
{
    ++record_metadata_count_;
}

void TestProfileBuilder::ApplyMetadataRetrospectively(TimeTicks period_start, TimeTicks period_end, const MetadataRecorder::Item& item)
{
    retrospective_metadata_.push_back(RetrospectiveMetadata { period_start, period_end, item });
}

void TestProfileBuilder::AddProfileMetadata(const MetadataRecorder::Item& item)
{
    profile_metadata_.push_back(item);
}

void TestProfileBuilder::OnSampleCompleted(std::vector<Frame> sample, TimeTicks sample_timestamp)
{
    samples_.push_back(std::move(sample));
}

void TestProfileBuilder::OnProfileCompleted(TimeDelta profile_duration, TimeDelta sampling_period)
{
    std::move(callback_).Run(Profile { samples_, record_metadata_count_, retrospective_metadata_, profile_metadata_, profile_duration, sampling_period });
}
...
void SynchronousUnloadNativeLibrary(NativeLibrary library)
{
    ...
    while (::GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
               reinterpret_cast<LPCTSTR>(module_base_address), &module_handle)
...
struct TestProfilerInfo {
    TestProfilerInfo(
        SamplingProfilerThreadToken thread_token, const SamplingParams& params, ModuleCache* module_cache, StackSamplerTestDelegate* delegate = nullptr)
        : completed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
        , profiler(thread_token, params, std::make_unique<TestProfileBuilder>(module_cache, BindLambdaForTesting([this](Profile result_profile) {
            profile = std::move(result_profile);
            completed.Signal();
        })),
              CreateCoreUnwindersFactoryForTesting(module_cache), RepeatingClosure(), delegate)
    {
    }
...
std::vector<std::vector<Frame>> CaptureSamples(const SamplingParams& params, TimeDelta profiler_wait_time, ModuleCache* module_cache)
{
    std::vector<std::vector<Frame>> samples;
...
size_t WaitForSamplingComplete(const std::vector<std::unique_ptr<TestProfilerInfo>>& infos)
{
    // Map unique_ptrs to something that WaitMany can accept.
    std::vector<WaitableEvent*> sampling_completed_rawptrs(infos.size());
    ranges::transform(infos, sampling_completed_rawptrs.begin(), [](const std::unique_ptr<TestProfilerInfo>& info) { return &info.get()->completed; });
    // Wait for one profiler to finish.
    return WaitableEvent::WaitMany(sampling_completed_rawptrs.data(), sampling_completed_rawptrs.size());
}
...
void TestLibraryUnload(bool wait_until_unloaded, ModuleCache* module_cache)
{
    ...
    public:
        StackCopiedSignaler(WaitableEvent* stack_copied, WaitableEvent* start_stack_walk, bool wait_to_walk_stack)
    ...
    UnwindScenario scenario(BindRepeating(&CallThroughOtherLibrary, UnsafeDanglingUntriaged(other_library)));

    UnwindScenario::SampleEvents events;
    TargetThread target_thread(BindLambdaForTesting([&] { scenario.Execute(&events); }));
    target_thread.Start();
    events.ready_for_sample.Wait();

    WaitableEvent sampling_thread_completed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    Profile profile;

    WaitableEvent stack_copied(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    WaitableEvent start_stack_walk(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    StackCopiedSignaler test_delegate(&stack_copied, &start_stack_walk, wait_until_unloaded);
    StackSamplingProfiler profiler(target_thread.thread_token(), params,
        std::make_unique<TestProfileBuilder>(module_cache, BindLambdaForTesting([&profile, &sampling_thread_completed](Profile result_profile) {
            profile = std::move(result_profile);
            sampling_thread_completed.Signal();
        })),
        CreateCoreUnwindersFactoryForTesting(module_cache), RepeatingClosure(), &test_delegate);
    ...
    if (wait_until_unloaded) {
        ...
        EXPECT_EQ(nullptr, sample.back().module) << "Stack:\n" << FormatSampleForDiagnosticOutput(sample);

        ExpectStackContains(sample, { scenario.GetWaitForSampleAddressRange() });
        ExpectStackDoesNotContain(sample, { scenario.GetSetupFunctionAddressRange(), scenario.GetOuterFunctionAddressRange() });
    } else {
        ...
        ExpectStackContains(
            sample, { scenario.GetWaitForSampleAddressRange(), scenario.GetSetupFunctionAddressRange(), scenario.GetOuterFunctionAddressRange() });
    }
}
...
// ChromeOS device.
#if (defined(ADDRESS_SANITIZER) && BUILDFLAG(IS_APPLE)) || (defined(ADDRESS_SANITIZER) && BUILDFLAG(IS_ANDROID))                                               \
...
PROFILER_TEST_F(StackSamplingProfilerTest, MAYBE_Basic)
{
    ...
    ExpectStackContains(sample, { scenario.GetWaitForSampleAddressRange(), scenario.GetSetupFunctionAddressRange(), scenario.GetOuterFunctionAddressRange() });
}

// A simple unwinder that always generates one frame then aborts the stack walk.
class TestAuxUnwinder : public Unwinder {
public:
    TestAuxUnwinder(const Frame& frame_to_report, base::RepeatingClosure add_initial_modules_callback)
    ...
    UnwindResult TryUnwind(UnwinderStateCapture* capture_state, RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* stack) override
    {
...
// because Ubuntu's libc isn't compiled with frame pointers. Skip if not a real
// ChromeOS device.
#if (defined(ADDRESS_SANITIZER) && BUILDFLAG(IS_APPLE)) || BUILDFLAG(IS_ANDROID) || (BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE))
...
// because Ubuntu's libc isn't compiled with frame pointers. Skip if not a real
// ChromeOS device.
#if (defined(ADDRESS_SANITIZER) && BUILDFLAG(IS_APPLE)) || BUILDFLAG(IS_IOS) || (BUILDFLAG(IS_ANDROID) && BUILDFLAG(EXCLUDE_UNWIND_TABLES))                    \
    || (BUILDFLAG(IS_ANDROID) && defined(ADDRESS_SANITIZER)) || (BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE))
...
PROFILER_TEST_F(StackSamplingProfilerTest, MAYBE_OtherLibrary)
{
    ...
    UnwindScenario scenario(BindRepeating(&CallThroughOtherLibrary, Unretained(other_library.get())));
    const std::vector<Frame>& sample = SampleScenario(&scenario, module_cache());
...
// because Ubuntu's libc isn't compiled with frame pointers. Skip if not a real
// ChromeOS device.
#if BUILDFLAG(IS_APPLE) || (BUILDFLAG(IS_ANDROID) && BUILDFLAG(EXCLUDE_UNWIND_TABLES)) || (BUILDFLAG(IS_ANDROID) && defined(ADDRESS_SANITIZER))                \
    || (BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE))
...
// because Ubuntu's libc isn't compiled with frame pointers. Skip if not a real
// ChromeOS device.
#if (defined(ADDRESS_SANITIZER) && BUILDFLAG(IS_APPLE)) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS)                                                          \
    || (BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE))
...
{
    WithTargetThread(BindLambdaForTesting([this](SamplingProfilerThreadToken target_thread_token) {
        SamplingParams params;
        params.sampling_interval = Milliseconds(0);
        params.samples_per_profile = 1;

        Profile profile;
        WaitableEvent sampling_completed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);

        StackSamplingProfiler profiler(target_thread_token, params,
            std::make_unique<TestProfileBuilder>(module_cache(), BindLambdaForTesting([&profile, &sampling_completed](Profile result_profile) {
                profile = std::move(result_profile);
...
PROFILER_TEST_F(StackSamplingProfilerTest, StopSafely)
{
    ...
    WithTargetThread(BindLambdaForTesting([](SamplingProfilerThreadToken target_thread_token) {
        ...
        TestProfilerInfo profiler_info0(target_thread_token, params[0], &module_cache1, &samples_recorded[0]);
        TestProfilerInfo profiler_info1(target_thread_token, params[1], &module_cache2, &samples_recorded[1]);
...
PROFILER_TEST_F(StackSamplingProfilerTest, StopDuringInitialDelay)
{
    ...
    std::vector<std::vector<Frame>> samples = CaptureSamples(params, Milliseconds(0), module_cache());
...
PROFILER_TEST_F(StackSamplingProfilerTest, StopDuringInterSampleInterval)
{
    ...
    public:
        SampleRecordedEvent()
            : sample_recorded_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
        {
        }

        void OnPreStackWalk() override
        {
            sample_recorded_.Signal();
        }
    ...
    WithTargetThread(BindLambdaForTesting([this](SamplingProfilerThreadToken target_thread_token) {
        ...
        TestProfilerInfo profiler_info(target_thread_token, params, module_cache(), &samples_recorded);
...
{
    const auto& GetNextSampleTime = StackSamplingProfiler::TestPeer::GetNextSampleTime;
    ...
    EXPECT_EQ(
        scheduled_current_sample_time + sampling_interval, GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time));
    ...
    EXPECT_EQ(scheduled_current_sample_time + sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 0.4 * sampling_interval));
...
PROFILER_TEST_F(StackSamplingProfilerTest, GetNextSampleTime_DelayedExecution)
{
    ...
    EXPECT_EQ(scheduled_current_sample_time + 2 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 0.6 * sampling_interval));
    EXPECT_EQ(scheduled_current_sample_time + 2 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 1.0 * sampling_interval));
    EXPECT_EQ(scheduled_current_sample_time + 2 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 1.4 * sampling_interval));
    ...
    EXPECT_EQ(scheduled_current_sample_time + 11 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 9.6 * sampling_interval));
    EXPECT_EQ(scheduled_current_sample_time + 11 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 10.0 * sampling_interval));
    EXPECT_EQ(scheduled_current_sample_time + 11 * sampling_interval,
        GetNextSampleTime(scheduled_current_sample_time, sampling_interval, scheduled_current_sample_time + 10.4 * sampling_interval));
}
...
PROFILER_TEST_F(StackSamplingProfilerTest, DestroyProfilerWhileProfiling)
{
    ...
    WithTargetThread(BindLambdaForTesting([&, this](SamplingProfilerThreadToken target_thread_token) {
        ...
        auto profile_builder = std::make_unique<TestProfileBuilder>(
            module_cache(), BindLambdaForTesting([&profile](Profile result_profile) { profile = std::move(result_profile); }));
        profiler = std::make_unique<StackSamplingProfiler>(
            target_thread_token, params, std::move(profile_builder), CreateCoreUnwindersFactoryForTesting(module_cache()));
        profiler->Start();
        profiler.reset();
...
PROFILER_TEST_F(StackSamplingProfilerTest, CanRunMultipleProfilers)
{
    ...
    std::vector<std::vector<Frame>> samples = CaptureSamples(params, AVeryLongTimeDelta(), module_cache());
...
PROFILER_TEST_F(StackSamplingProfilerTest, MultipleStart)
{
    WithTargetThread(BindLambdaForTesting([](SamplingProfilerThreadToken target_thread_token) {
        ...
        TestProfilerInfo profiler_info1(target_thread_token, params1, &module_cache1);

        SamplingParams params2;
        params2.sampling_interval = Milliseconds(1);
        params2.samples_per_profile = 1;
        ModuleCache module_cache2;
        TestProfilerInfo profiler_info2(target_thread_token, params2, &module_cache2);
...
PROFILER_TEST_F(StackSamplingProfilerTest, ProfileGeneralInfo)
{
    WithTargetThread(BindLambdaForTesting([this](SamplingProfilerThreadToken target_thread_token) {
        ...
        TestProfilerInfo profiler_info(target_thread_token, params, module_cache());
        ...
        // intervals.
        EXPECT_GT(profiler_info.profile.profile_duration, profiler_info.profile.sampling_period * 3);
...
PROFILER_TEST_F(StackSamplingProfilerTest, WillRestartSamplerAfterIdleShutdown)
{
    SamplingParams params;
    params.sampling_interval = Milliseconds(0);
    params.samples_per_profile = 1;
...
// started.
PROFILER_TEST_F(StackSamplingProfilerTest, ProfileBeforeAndAfterSamplingThreadRunning)
{
    WithTargetThread(BindLambdaForTesting([](SamplingProfilerThreadToken target_thread_token) {
        ModuleCache module_cache1;
        ModuleCache module_cache2;
        ...
        profiler_infos.push_back(std::make_unique<TestProfilerInfo>(target_thread_token,
...
PROFILER_TEST_F(StackSamplingProfilerTest, IdleShutdownAbort)
{
    WithTargetThread(BindLambdaForTesting([this](SamplingProfilerThreadToken target_thread_token) {
        ...
        TestProfilerInfo another_info(target_thread_token, params, module_cache());
...
PROFILER_TEST_F(StackSamplingProfilerTest, MultipleSampledThreads)
{
    ...
    TargetThread target_thread1(BindLambdaForTesting([&] { scenario1.Execute(&events1); }));
    ...
    TargetThread target_thread2(BindLambdaForTesting([&] { scenario2.Execute(&events2); }));
    ...
    WaitableEvent sampling_thread_completed1(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    StackSamplingProfiler profiler1(target_thread1.thread_token(), params1,
        std::make_unique<TestProfileBuilder>(&module_cache1, BindLambdaForTesting([&profile1, &sampling_thread_completed1](Profile result_profile) {
            profile1 = std::move(result_profile);
            sampling_thread_completed1.Signal();
        })),
        CreateCoreUnwindersFactoryForTesting(&module_cache1));

    WaitableEvent sampling_thread_completed2(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    StackSamplingProfiler profiler2(target_thread2.thread_token(), params2,
        std::make_unique<TestProfileBuilder>(&module_cache2, BindLambdaForTesting([&profile2, &sampling_thread_completed2](Profile result_profile) {
...
public:
    ProfilerThread(const std::string& name, SamplingProfilerThreadToken thread_token, const SamplingParams& params, ModuleCache* module_cache)
        : SimpleThread(name, Options())
        , run_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
        , completed_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
        , profiler_(thread_token, params, std::make_unique<TestProfileBuilder>(module_cache, BindLambdaForTesting([this](Profile result_profile) {
...
PROFILER_TEST_F(StackSamplingProfilerTest, MultipleProfilerThreads)
{
    WithTargetThread(BindLambdaForTesting([](SamplingProfilerThreadToken target_thread_token) {
        ...
        ModuleCache module_cache1;
        ProfilerThread profiler_thread1("profiler1", target_thread_token, params1, &module_cache1);
        ModuleCache module_cache2;
        ProfilerThread profiler_thread2("profiler2", target_thread_token, params2, &module_cache2);
...
PROFILER_TEST_F(StackSamplingProfilerTest, AddAuxUnwinder_BeforeStart)
{
    ...
    const auto add_initial_modules_callback = [&add_initial_modules_invocation_count] { ++add_initial_modules_invocation_count; };

    Profile profile;
    WithTargetThread(&scenario, BindLambdaForTesting([&](SamplingProfilerThreadToken target_thread_token) {
        WaitableEvent sampling_thread_completed(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        StackSamplingProfiler profiler(target_thread_token, params,
            std::make_unique<TestProfileBuilder>(module_cache(), BindLambdaForTesting([&profile, &sampling_thread_completed](Profile result_profile) {
                profile = std::move(result_profile);
            ...
            CreateCoreUnwindersFactoryForTesting(module_cache()));
        profiler.AddAuxUnwinder(std::make_unique<TestAuxUnwinder>(Frame(23, nullptr), BindLambdaForTesting(add_initial_modules_callback)));
        profiler.Start();
        sampling_thread_completed.Wait();
    }));
...
PROFILER_TEST_F(StackSamplingProfilerTest, AddAuxUnwinder_AfterStop)
{
    ...
    WithTargetThread(&scenario, BindLambdaForTesting([&](SamplingProfilerThreadToken target_thread_token) {
        ...
        profiler.Start();
        profiler.Stop();
        profiler.AddAuxUnwinder(std::make_unique<TestAuxUnwinder>(Frame(23, nullptr), base::RepeatingClosure()));
        sampling_thread_completed.Wait();
    }));
...
// profile builder.
PROFILER_TEST_F(StackSamplingProfilerTest, ApplyMetadataToPastSamples_PassedToProfileBuilder)
{
    ...
    WithTargetThread(&scenario, BindLambdaForTesting([&](SamplingProfilerThreadToken target_thread_token) {
        ...
        StackSamplingProfiler profiler(target_thread_token, params,
            std::make_unique<TestProfileBuilder>(
                module_cache(), BindLambdaForTesting([&profile](Profile result_profile) { profile = std::move(result_profile); })),
            CreateCoreUnwindersFactoryForTesting(module_cache()), RepeatingClosure(), &post_sample_invoker);
        profiler.Start();
        ...
        ApplyMetadataToPastSamples(sample_times[1], sample_times[3], "TestMetadata1", 10, base::SampleMetadataScope::kProcess);
        ApplyMetadataToPastSamples(sample_times[2], sample_times[4], "TestMetadata2", 100, 11, base::SampleMetadataScope::kProcess);
        profiler.Stop();
    }));
...
PROFILER_TEST_F(StackSamplingProfilerTest, ApplyMetadataToPastSamples_PassedToProfileBuilder_MultipleCollections)
{
    SamplingParams params;
    params.sampling_interval = Milliseconds(10);
    // 10,000 samples ensures the profiler continues running until manually
    ...
    TargetThread target_thread1(BindLambdaForTesting([&] {
        StackSamplingProfiler profiler1(target_thread1.thread_token(), params,
            std::make_unique<TestProfileBuilder>(&module_cache1, BindLambdaForTesting([&](Profile result_profile) {
                profile1 = std::move(result_profile);
                sampling_completed1.Signal();
            })),
            CreateCoreUnwindersFactoryForTesting(&module_cache1), RepeatingClosure());
        profiler1.Start();
        ...
        // ProfileBuilder.
        ApplyMetadataToPastSamples(TimeTicks(), TimeTicks::Now(), "TestMetadata1", 10, 10, SampleMetadataScope::kThread);
    ...
    TargetThread target_thread2(BindLambdaForTesting([&] {
        StackSamplingProfiler profiler2(target_thread2.thread_token(), params,
            std::make_unique<TestProfileBuilder>(&module_cache2, BindLambdaForTesting([&](Profile result_profile) {
                profile2 = std::move(result_profile);
                sampling_completed2.Signal();
            })),
            CreateCoreUnwindersFactoryForTesting(&module_cache2), RepeatingClosure());
        profiler2.Start();
        profiler2_started.Signal();
        profiler1_started.Wait();

        // Record metadata on past samples only for this thread.
        ApplyMetadataToPastSamples(TimeTicks(), TimeTicks::Now(), "TestMetadata2", 20, 20, SampleMetadataScope::kThread);
...
// builder.
PROFILER_TEST_F(StackSamplingProfilerTest, AddProfileMetadata_PassedToProfileBuilder)
{
    // Runs the passed closure on the profiler thread after a sample is taken.
    class PostSampleInvoker : public StackSamplerTestDelegate {
    public:
    ...
    WithTargetThread(&scenario, BindLambdaForTesting([&](SamplingProfilerThreadToken target_thread_token) {
        WaitableEvent sample_seen(WaitableEvent::ResetPolicy::AUTOMATIC);
        PostSampleInvoker post_sample_invoker(BindLambdaForTesting([&] { sample_seen.Signal(); }));
        ...
        profiler.Start();
        sample_seen.Wait();
        AddProfileMetadata("TestMetadata", 1, 2, SampleMetadataScope::kProcess);
        profiler.Stop();
    }));
...
PROFILER_TEST_F(StackSamplingProfilerTest, AddProfileMetadata_PassedToProfileBuilder_MultipleCollections)
{
    SamplingParams params;
    params.sampling_interval = Milliseconds(10);
    // 10,000 samples ensures the profiler continues running until manually
