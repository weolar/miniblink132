...
// Windows.
class BASE_EXPORT SuspendableThreadDelegateWin : public SuspendableThreadDelegate {
public:
    class ScopedSuspendThread : public SuspendableThreadDelegate::ScopedSuspendThread {
    public:
    ...
    explicit SuspendableThreadDelegateWin(SamplingProfilerThreadToken thread_token);
    ~SuspendableThreadDelegateWin() override;

    SuspendableThreadDelegateWin(const SuspendableThreadDelegateWin&) = delete;
    SuspendableThreadDelegateWin& operator=(const SuspendableThreadDelegateWin&) = delete;

    // SuspendableThreadDelegate
    std::unique_ptr<SuspendableThreadDelegate::ScopedSuspendThread> CreateScopedSuspendThread() override;
    ...
    std::vector<uintptr_t*> GetRegistersToRewrite(CONTEXT* thread_context) override;
