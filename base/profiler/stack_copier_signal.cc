...
public:
    ...
    bool Wait()
    {
        ...
        while (true) {
            long res = syscall(SYS_futex, futex_ptr(), FUTEX_WAIT | FUTEX_PRIVATE_FLAG, kNotSignaled, nullptr, nullptr, 0);
    ...
    void Signal()
    {
        ...
        syscall(SYS_futex, futex_ptr(), FUTEX_WAKE | FUTEX_PRIVATE_FLAG, 1, nullptr, nullptr, 0);
    }
...
void CopyStackSignalHandler(int n, siginfo_t* siginfo, void* sigcontext)
{
    ...
    *params->stack_copy_bottom = StackCopierSignal::CopyStackContentsAndRewritePointers(
        reinterpret_cast<uint8_t*>(bottom), reinterpret_cast<uintptr_t*>(top), StackBuffer::kPlatformStackAlignment, params->stack_buffer->buffer());
...
public:
    ScopedSigaction(int signal, struct sigaction* action, struct sigaction* original_action)
        : signal_(signal)
        , action_(action)
...
bool StackCopierSignal::CopyStack(StackBuffer* stack_buffer, uintptr_t* stack_top, TimeTicks* timestamp, RegisterContext* thread_context, Delegate* delegate)
{
    ...
    HandlerParams params = { stack_base_address, &wait_event, &copied, thread_context, stack_buffer, &stack_copy_bottom, &maybe_timestamp, delegate };
    {
        ...
        TRACE_EVENT_BEGIN0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "StackCopierSignal copy stack");
        ...
        if (syscall(SYS_tgkill, getpid(), thread_delegate_->GetThreadId(), SIGURG) != 0) {
            NOTREACHED();
        }
        bool finished_waiting = wait_event.Wait();
        TRACE_EVENT_END0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "StackCopierSignal copy stack");
        ...
        if (maybe_timestamp.has_value())
            *timestamp = maybe_timestamp.value();
            ...
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "Fallback on TimeTicks::Now()");
            *timestamp = TimeTicks::Now();
        }
    }

    const uintptr_t bottom = RegisterContextStackPointer(params.context);
    for (uintptr_t* reg : thread_delegate_->GetRegistersToRewrite(thread_context)) {
        *reg = StackCopierSignal::RewritePointerIfInOriginalStack(
            reinterpret_cast<uint8_t*>(bottom), reinterpret_cast<uintptr_t*>(stack_base_address), stack_copy_bottom, *reg);
    }

    *stack_top = reinterpret_cast<uintptr_t>(stack_copy_bottom) + (stack_base_address - bottom);

    return copied;
}

std::vector<uintptr_t*> StackCopierSignal::GetRegistersToRewrite(RegisterContext* thread_context)
{
