...
// expects.
BASE_EXPORT std::string TransformModuleIDToSymbolServerFormat(std::string_view module_id);
...
public:
    ...
    public:
        ...
        virtual std::unique_ptr<const Module> TryCreateModuleForAddress(uintptr_t address) = 0;
    ...
    // the same call.
    void UpdateNonNativeModules(const std::vector<const Module*>& defunct_modules, std::vector<std::unique_ptr<const Module>> new_modules);
    ...
    void RegisterAuxiliaryModuleProvider(AuxiliaryModuleProvider* auxiliary_module_provider);

    // Unregisters the custom module provider.
    void UnregisterAuxiliaryModuleProvider(AuxiliaryModuleProvider* auxiliary_module_provider);
...
private:
    ...
    struct ModuleAndAddressCompare {
        ...
        bool operator()(const std::unique_ptr<const Module>& m1, const std::unique_ptr<const Module>& m2) const;
        bool operator()(const std::unique_ptr<const Module>& m1, uintptr_t address) const;
        bool operator()(uintptr_t address, const std::unique_ptr<const Module>& m2) const;
    };

    // Creates a Module object for the specified memory address. Returns null if
    // the address does not belong to a module.
    static std::unique_ptr<const Module> CreateModuleForAddress(uintptr_t address);
    ...
    std::set<std::unique_ptr<const Module>, ModuleAndAddressCompare> native_modules_;
    ...
    base::flat_set<std::unique_ptr<const Module>, ModuleAndAddressCompare> non_native_modules_;
