...
public:
    ...
    virtual bool CopyStack(StackBuffer* stack_buffer, uintptr_t* stack_top, TimeTicks* timestamp, RegisterContext* thread_context, Delegate* delegate) = 0;
    ...
    std::unique_ptr<StackBuffer> CloneStack(const StackBuffer& stack_buffer, uintptr_t* stack_top, RegisterContext* thread_context);

protected:
    virtual std::vector<uintptr_t*> GetRegistersToRewrite(RegisterContext* thread_context) = 0;
