...
{
#if defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_32_BITS)
    return base::WrapUnique<unwindstack::Regs>(unwindstack::RegsArm::Read(reinterpret_cast<void*>(&thread_context->arm_r0)));
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_64_BITS)
    return base::WrapUnique<unwindstack::Regs>(unwindstack::RegsArm64::Read(reinterpret_cast<void*>(&thread_context->regs[0])));
#else // #if defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_32_BITS)
    NOTREACHED();
#endif // #if defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_32_BITS)
}

void WriteLibunwindstackTraceEventArgs(unwindstack::ErrorCode error_code, std::optional<int> num_frames, perfetto::EventContext& ctx)
{
...
bool IsJavaModule(const base::ModuleCache::Module* module)
{
    ...
    return debug_basename.find("chrome.apk") != std::string::npos || debug_basename.find("base.apk") != std::string::npos;
}

} // namespace

LibunwindstackUnwinderAndroid::LibunwindstackUnwinderAndroid()
    : memory_regions_map_(static_cast<NativeUnwinderAndroidMemoryRegionsMapImpl*>(NativeUnwinderAndroid::CreateMemoryRegionsMap(
        /*use_updatable_maps=*/false)
                                                                                      .release()))
{
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "LibunwindstackUnwinderAndroid::LibunwindstackUnwinderAndroid");
}
...
bool LibunwindstackUnwinderAndroid::CanUnwindFrom(const Frame& current_frame) const
{
    return true;
}

unwindstack::JitDebug* LibunwindstackUnwinderAndroid::GetOrCreateJitDebug(unwindstack::ArchEnum arch)
{
    if (!jit_debug_) {
        jit_debug_ = unwindstack::CreateJitDebug(arch, memory_regions_map_->memory(), search_libs_);
    }
    return jit_debug_.get();
}

unwindstack::DexFiles* LibunwindstackUnwinderAndroid::GetOrCreateDexFiles(unwindstack::ArchEnum arch)
{
    if (!dex_files_) {
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "LibunwindstackUnwinderAndroid::TryUnwind");
    ...
    std::unique_ptr<unwindstack::Regs> regs = CreateFromRegisterContext(thread_context);
    DCHECK(regs);

    TRACE_EVENT_BEGIN(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "libunwindstack::Unwind");
    unwindstack::Unwinder unwinder(kMaxFrames, memory_regions_map_->maps(), regs.get(), memory_regions_map_->memory());
    ...
    UnwindValues values = UnwindValues { unwinder.LastErrorCode(), /*unwinder.warnings()*/ 0, unwinder.ConsumeFrames() };

    if (values.error_code != unwindstack::ERROR_NONE) {
        TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "Libunwindstack Failure",
            [&values](perfetto::EventContext& ctx) { WriteLibunwindstackTraceEventArgs(values.error_code, values.frames.size(), ctx); });
    }
    ...
    for (const unwindstack::FrameData& frame : values.frames) {
        const ModuleCache::Module* module = module_cache()->GetModuleForAddress(frame.pc);
        if (module == nullptr && frame.map_info != nullptr) {
            ...
            if (module == nullptr) {
                auto module_for_caching = std::make_unique<NonElfModule>(frame.map_info.get());
                module = module_for_caching.get();
                module_cache()->AddCustomNativeModule(std::move(module_for_caching));
            }
            if (frame.pc < frame.map_info->start() || frame.pc >= frame.map_info->end()) {
                TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler"), "PC out of map range",
                    [&values](perfetto::EventContext& ctx) { WriteLibunwindstackTraceEventArgs(values.error_code, std::nullopt, ctx); });
            }
        }
