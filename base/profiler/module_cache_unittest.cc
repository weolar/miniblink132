...
public:
    explicit IsolatedModule(bool is_native = true)
        ...
        , memory_region_(new char[kRegionSize])
    {
    }
...
public:
    FakeModule(uintptr_t base_address, size_t size, bool is_native = true, OnceClosure destruction_closure = OnceClosure())
...
const ModuleCache::Module* AddNonNativeModule(ModuleCache* cache, std::unique_ptr<const ModuleCache::Module> module)
{
...
#if (BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_IOS) && !defined(ARCH_CPU_ARM64)) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_WIN)
...
{
    ModuleCache cache;
    const ModuleCache::Module* module = cache.GetModuleForAddress(reinterpret_cast<uintptr_t>(&AFunctionForTest));
    ASSERT_NE(nullptr, module);
#if BUILDFLAG(IS_ANDROID)
    EXPECT_EQ("libbase_unittests__library", module->GetDebugBasename().RemoveFinalExtension().value());
...
MAYBE_TEST(ModuleCacheTest, LookupRange)
{
    ...
    EXPECT_EQ(module, cache.GetModuleForAddress(module->GetBaseAddress() + module->GetSize() - 1));
    EXPECT_EQ(nullptr, cache.GetModuleForAddress(module->GetBaseAddress() + module->GetSize()));
}

MAYBE_TEST(ModuleCacheTest, LookupNonNativeModule)
{
    ModuleCache cache;
    const ModuleCache::Module* module = AddNonNativeModule(&cache, std::make_unique<IsolatedModule>(false));
...
MAYBE_TEST(ModuleCacheTest, LookupOverlaidNonNativeModule)
{
    ...
    const ModuleCache::Module* non_native_module
        = AddNonNativeModule(&cache, std::make_unique<FakeModule>(native_module->GetBaseAddress() + 8, native_module->GetSize() - 16, false));

    EXPECT_EQ(native_module, cache.GetModuleForAddress(non_native_module->GetBaseAddress() - 1));
    EXPECT_EQ(non_native_module, cache.GetModuleForAddress(non_native_module->GetBaseAddress()));
    EXPECT_EQ(non_native_module, cache.GetModuleForAddress(non_native_module->GetBaseAddress() + non_native_module->GetSize() - 1));
    EXPECT_EQ(native_module, cache.GetModuleForAddress(non_native_module->GetBaseAddress() + non_native_module->GetSize()));
}
...
MAYBE_TEST(ModuleCacheTest, UpdateNonNativeModulesRemoveModuleIsNotDestroyed)
{
    bool was_destroyed = false;
    {
        ModuleCache cache;
        std::vector<std::unique_ptr<const ModuleCache::Module>> modules;
        modules.push_back(std::make_unique<FakeModule>(1, 1, false, BindLambdaForTesting([&was_destroyed] { was_destroyed = true; })));
        const ModuleCache::Module* module = modules.back().get();
        cache.UpdateNonNativeModules({}, std::move(modules));
        cache.UpdateNonNativeModules({ module }, {});
...
MAYBE_TEST(ModuleCacheTest, UpdateNonNativeModulesPartitioning)
{
    ...
    const auto record_destroyed = [&destroyed_count] { ++destroyed_count; };
    {
        ModuleCache cache;
        std::vector<std::unique_ptr<const ModuleCache::Module>> modules;
        modules.push_back(std::make_unique<FakeModule>(1, 1, false, BindLambdaForTesting(record_destroyed)));
        const ModuleCache::Module* module1 = modules.back().get();
        modules.push_back(std::make_unique<FakeModule>(2, 1, false, BindLambdaForTesting(record_destroyed)));
        cache.UpdateNonNativeModules({}, std::move(modules));
...
MAYBE_TEST(ModuleCacheTest, UpdateNonNativeModulesMultipleRemovedModulesAtSameAddress)
{
    int destroyed_count = 0;
    const auto record_destroyed = [&destroyed_count] { ++destroyed_count; };
    ModuleCache cache;
    ...
    std::vector<std::unique_ptr<const ModuleCache::Module>> modules1;
    modules1.push_back(std::make_unique<FakeModule>(1, 1, false, BindLambdaForTesting(record_destroyed)));
    const ModuleCache::Module* module1 = modules1.back().get();

    std::vector<std::unique_ptr<const ModuleCache::Module>> modules2;
    modules2.push_back(std::make_unique<FakeModule>(1, 1, false, BindLambdaForTesting(record_destroyed)));
    const ModuleCache::Module* module2 = modules2.back().get();
...
MAYBE_TEST(ModuleCacheTest, UpdateNonNativeModulesCorrectModulesRemoved)
{
    ...
    std::vector<const ModuleCache::Module*> to_remove = { to_add[1].get(), to_add[3].get() };
...
MAYBE_TEST(ModuleCacheTest, ModulesList)
{
    ...
    const ModuleCache::Module* non_native_module = AddNonNativeModule(&cache, std::make_unique<FakeModule>(1, 2, false));
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || (BUILDFLAG(IS_ANDROID) && !defined(ARCH_CPU_ARM64))
...
TEST(ModuleCacheTest, CheckAgainstProcMaps)
{
    ...
    const auto find_last_executable_region = [](const RegionVector& regions) {
        const auto rloc = base::ranges::find_if(base::Reversed(regions),
            [](const debug::MappedMemoryRegion* region) { return static_cast<bool>(region->permissions & debug::MappedMemoryRegion::EXECUTE); });
    ...
    for (const auto& path_regions_pair : path_regions) {
        ...
        const debug::MappedMemoryRegion* const last_executable_region = find_last_executable_region(path_regions_pair.second);
        ...
        const uintptr_t expected_base_address = path_regions_pair.second.front()->start;
        for (const auto* region : path_regions_pair.second) {
            ModuleCache cache;
            const ModuleCache::Module* module = cache.GetModuleForAddress(region->start);
            ...
            EXPECT_LE(module->GetSize(), last_executable_region->end - expected_base_address)
                << "base address: " << std::hex << module->GetBaseAddress() << std::endl
...
public:
    ...
    std::unique_ptr<const ModuleCache::Module> TryCreateModuleForAddress(uintptr_t address) override
    {
...
TEST(ModuleCacheTest, TransformELFToSymbolServerFormat)
{
    ...
    EXPECT_EQ(TransformModuleIDToSymbolServerFormat("7F0715C286F8B16C10E4AD349CDA3B9B56C7A773"), "C215077FF8866CB110E4AD349CDA3B9B0");
}
#endif
