...
TEST(StackCopierTest, RewritePointerIfInOriginalStack_InStack)
{
    ...
    EXPECT_EQ(reinterpret_cast<uintptr_t>(&stack_copy[2]),
        CopyFunctions::RewritePointerIfInOriginalStack(reinterpret_cast<uint8_t*>(&original_stack[0]), &original_stack[0] + std::size(original_stack),
            reinterpret_cast<uint8_t*>(&stack_copy[0]), reinterpret_cast<uintptr_t>(&original_stack[2])));
}
...
TEST(StackCopierTest, RewritePointerIfInOriginalStack_NotInStack)
{
    ...
    EXPECT_EQ(reinterpret_cast<uintptr_t>(&non_stack_location),
        CopyFunctions::RewritePointerIfInOriginalStack(reinterpret_cast<uint8_t*>(&original_stack[0]), &original_stack[0] + std::size(original_stack),
            reinterpret_cast<uint8_t*>(&stack_copy[0]), reinterpret_cast<uintptr_t>(&non_stack_location)));
}
...
TEST(StackCopierTest, StackCopy)
{
    ...
    std::iota(&original_stack.as_uintptr[0], &original_stack.as_uintptr[0] + std::size(original_stack.as_uintptr), 100);
    // Replace the third value with an address within the buffer.
    original_stack.as_uintptr[2] = reinterpret_cast<uintptr_t>(&original_stack.as_uintptr[1]);
    TestStackBuffer stack_copy;

    CopyFunctions::CopyStackContentsAndRewritePointers(&original_stack.as_uint8[0], &original_stack.as_uintptr[0] + std::size(original_stack.as_uintptr),
    ...
    EXPECT_EQ(reinterpret_cast<uintptr_t>(&stack_copy.as_uintptr[1]), stack_copy.as_uintptr[2]);
...
TEST(StackCopierTest, StackCopy_NonAlignedStackPointerCopy)
{
    ...
    std::iota(&stack_buffer.as_uint16[0], &stack_buffer.as_uint16[0] + std::size(stack_buffer.as_uint16), 100);

    // Set the stack bottom to the unaligned location one uint16_t into the
    // buffer.
    uint8_t* unaligned_stack_bottom = reinterpret_cast<uint8_t*>(&stack_buffer.as_uint16[1]);
    ...
    uintptr_t* stack_top = &stack_buffer.as_uintptr[std::size(stack_buffer.as_uintptr) - extra_space / sizeof(uintptr_t)];

    // Initialize the copy to all zeros.
    TestStackBuffer stack_copy_buffer = { { 0 } };

    const uint8_t* stack_copy_bottom = CopyFunctions::CopyStackContentsAndRewritePointers(
        unaligned_stack_bottom, stack_top, StackBuffer::kPlatformStackAlignment, &stack_copy_buffer.as_uintptr[0]);
    ...
    EXPECT_EQ(unaligned_stack_bottom - &stack_buffer.as_uint8[0], stack_copy_bottom - &stack_copy_buffer.as_uint8[0]);
    ...
    const size_t max_index = std::size(stack_copy_buffer.as_uint16) - extra_space / sizeof(uint16_t);
...
TEST(StackCopierTest, StackCopy_NonAlignedStackPointerUnalignedRewriteAtStart)
{
    ...
    uintptr_t within_stack_pointer = reinterpret_cast<uintptr_t>(&stack_buffer.as_uintptr[2]);
    std::memcpy(unaligned_stack_bottom, &within_stack_pointer, sizeof(within_stack_pointer));

    TestStackBuffer stack_copy_buffer = { { 0 } };

    const uint8_t* stack_copy_bottom = CopyFunctions::CopyStackContentsAndRewritePointers(unaligned_stack_bottom,
        &stack_buffer.as_uintptr[0] + std::size(stack_buffer.as_uintptr), StackBuffer::kPlatformStackAlignment, &stack_copy_buffer.as_uintptr[0]);

    uintptr_t copied_within_stack_pointer;
    std::memcpy(&copied_within_stack_pointer, stack_copy_bottom, sizeof(copied_within_stack_pointer));
...
// not rewritten.
TEST(StackCopierTest, StackCopy_NonAlignedStackPointerUnalignedRewriteAfterStart)
{
    // Initially fill the buffer with 0s.
    TestStackBuffer stack_buffer = { { 0 } };
    ...
    // stack.
    uintptr_t within_stack_pointer = reinterpret_cast<uintptr_t>(&stack_buffer.as_uintptr[2]);
    std::memcpy(unaligned_stack_bottom + sizeof(uintptr_t), &within_stack_pointer, sizeof(within_stack_pointer));
    ...
    uintptr_t copied_within_stack_pointer;
    std::memcpy(&copied_within_stack_pointer, stack_copy_bottom + sizeof(uintptr_t), sizeof(copied_within_stack_pointer));
...
TEST(StackCopierTest, StackCopy_NonAlignedStackPointerAlignedRewrite)
{
    ...
    stack_buffer.as_uintptr[1] = reinterpret_cast<uintptr_t>(&stack_buffer.as_uintptr[2]);

    TestStackBuffer stack_copy_buffer = { { 0 } };

    CopyFunctions::CopyStackContentsAndRewritePointers(unaligned_stack_bottom, &stack_buffer.as_uintptr[0] + std::size(stack_buffer.as_uintptr),
    ...
    // copy.
    EXPECT_EQ(reinterpret_cast<uintptr_t>(&stack_copy_buffer.as_uintptr[2]), stack_copy_buffer.as_uintptr[1]);
}
...
TEST(StackCopierTest, CloneStack)
{
    ...
    // Fill the stack buffer with increasing uintptr_t values.
    std::iota(&original_stack.buffer()[0], &original_stack.buffer()[0] + (original_stack.size() / sizeof(uintptr_t)), 100);
    // Replace the third value with an address within the buffer.
    original_stack.buffer()[2] = reinterpret_cast<uintptr_t>(&original_stack.buffer()[1]);

    uintptr_t stack_top = reinterpret_cast<uintptr_t>(original_stack.buffer()) + original_stack.size();
    CopyFunctions copy_functions;
    RegisterContext thread_context;
    RegisterContextStackPointer(&thread_context) = reinterpret_cast<uintptr_t>(original_stack.buffer());
    std::unique_ptr<StackBuffer> cloned_stack = copy_functions.CloneStack(original_stack, &stack_top, &thread_context);

    EXPECT_EQ(original_stack.buffer()[0], cloned_stack->buffer()[0]);
    EXPECT_EQ(original_stack.buffer()[1], cloned_stack->buffer()[1]);
    EXPECT_EQ(reinterpret_cast<uintptr_t>(&cloned_stack->buffer()[1]), cloned_stack->buffer()[2]);
    EXPECT_EQ(original_stack.buffer()[3], cloned_stack->buffer()[3]);
    uintptr_t expected_stack_top = reinterpret_cast<uintptr_t>(cloned_stack->buffer()) + original_stack.size();
    EXPECT_EQ(RegisterContextStackPointer(&thread_context), reinterpret_cast<uintptr_t>(cloned_stack->buffer()));
