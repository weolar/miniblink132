...
struct InputStack {
    ...
    uintptr_t top() const
    {
        return bottom() + buffer.size() * sizeof(uintptr_t);
    }
...
protected:
    FramePointerUnwinderTest()
    {
...
#else
        {
            ...
            auto non_native_module = std::make_unique<TestModule>(kNonNativeModuleStart, kModuleSize, false);
    ...
    ModuleCache::Module* non_native_module()
    {
        return non_native_module_;
    }
    Unwinder* unwinder()
    {
        return unwinder_.get();
    }
...
TEST_F(FramePointerUnwinderTest, FPPointsOutsideOfStack)
{
    ...
    std::vector<Frame> stack = { Frame(RegisterContextInstructionPointer(&context), module()) };

    EXPECT_EQ(UnwindResult::kAborted, unwinder()->TryUnwind(/*state_capture=*/nullptr, &context, input.top(), &stack));
...
TEST_F(FramePointerUnwinderTest, NoModuleForIP)
{
    ...
    EXPECT_EQ(UnwindResult::kAborted, unwinder()->TryUnwind(/*state_capture=*/nullptr, &context, input.top(), &stack));
    EXPECT_EQ(std::vector<Frame>({ { kModuleStart, module() }, { not_in_module, nullptr } }), stack);
}
...
TEST_F(FramePointerUnwinderTest, RegularUnwind)
{
    ...
    EXPECT_EQ(UnwindResult::kCompleted, unwinder()->TryUnwind(/*state_capture=*/nullptr, &context, input.top(), &stack));
    EXPECT_EQ(std::vector<Frame>({
                  { kModuleStart, module() },
...
TEST_F(FramePointerUnwinderTest, NonNativeFrame)
{
    ...
    EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder()->TryUnwind(/*state_capture=*/nullptr, &context, input.top(), &stack));
    EXPECT_EQ(std::vector<Frame>({
                  { kModuleStart, module() },
                  { kModuleStart + 0x20, module() },
