...
void AddMapInfo(uint64_t start, uint64_t end, uint64_t offset, uint64_t flags, std::string name, const std::string& binary_build_id, unwindstack::Maps& maps)
{
...
std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMapImpl> CreateMemoryRegionsMap()
{
    std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMap> memory_regions_map = NativeUnwinderAndroid::CreateMemoryRegionsMap();
    std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMapImpl> downcast(static_cast<NativeUnwinderAndroidMemoryRegionsMapImpl*>(memory_regions_map.release()));
    return downcast;
}

class NativeUnwinderAndroidMapDelegateForTesting : public NativeUnwinderAndroidMapDelegate {
public:
    explicit NativeUnwinderAndroidMapDelegateForTesting(std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMapImpl> memory_regions_map)
...
private:
    const std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMapImpl> memory_regions_map_;
...
std::vector<Frame> CaptureScenario(UnwindScenario* scenario, ModuleCache* module_cache,
    OnceCallback<void(UnwinderStateCapture*, RegisterContext*, uintptr_t, std::vector<Frame>*)> unwind_callback)
{
    std::vector<Frame> sample;
    WithTargetThread(scenario, BindLambdaForTesting([&](SamplingProfilerThreadToken target_thread_token) {
        auto thread_delegate = ThreadDelegatePosix::Create(target_thread_token);
        ASSERT_TRUE(thread_delegate);
        auto stack_copier = std::make_unique<StackCopierSignal>(std::move(thread_delegate));
        std::unique_ptr<StackBuffer> stack_buffer = StackSampler::CreateStackBuffer();

        RegisterContext thread_context;
        uintptr_t stack_top;
        TimeTicks timestamp;
        TestStackCopierDelegate delegate;
        bool success = stack_copier->CopyStack(stack_buffer.get(), &stack_top, &timestamp, &thread_context, &delegate);
        ASSERT_TRUE(success);
        ...
        std::move(unwind_callback).Run(nullptr, &thread_context, stack_top, &sample);
    }));
...
TEST(NativeUnwinderAndroidTest, PlainFunction)
{
    ...
    NativeUnwinderAndroidMapDelegateForTesting map_delegate(CreateMemoryRegionsMap());

    ModuleCache module_cache;
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(0, &map_delegate);

    unwinder->Initialize(&module_cache);
    std::vector<Frame> sample = CaptureScenario(&scenario, &module_cache,
        BindLambdaForTesting([&](UnwinderStateCapture* capture_state, RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
            ASSERT_TRUE(unwinder->CanUnwindFrom(sample->back()));
            UnwindResult result = unwinder->TryUnwind(capture_state, thread_context, stack_top, sample);
...
TEST(NativeUnwinderAndroidTest, OtherLibrary)
{
    ...
    UnwindScenario scenario(BindRepeating(&CallThroughOtherLibrary, Unretained(other_library)));
...
TEST(NativeUnwinderAndroidTest, ExcludeOtherLibrary)
{
    ...
    NativeUnwinderAndroidMapDelegateForTesting map_delegate(CreateMemoryRegionsMap());
    ModuleCache module_cache;
    unwindstack::MapInfo* other_library_map = map_delegate.GetMapReference()->maps()->Find(GetAddressInOtherLibrary(other_library)).get();
    ASSERT_NE(nullptr, other_library_map);
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(other_library_map->start(), &map_delegate);
    unwinder->Initialize(&module_cache);

    std::vector<Frame> sample = CaptureScenario(&scenario, &module_cache,
        BindLambdaForTesting([&](UnwinderStateCapture* capture_state, RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
            ASSERT_TRUE(unwinder->CanUnwindFrom(sample->back()));
            EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder->TryUnwind(capture_state, thread_context, stack_top, sample));
...
TEST(NativeUnwinderAndroidTest, ResumeUnwinding)
{
    ...
    auto unwinder_for_all = std::make_unique<NativeUnwinderAndroid>(0, &map_delegate);
    unwinder_for_all->Initialize(&module_cache_for_all);

    ModuleCache module_cache_for_native;
    auto unwinder_for_native = std::make_unique<NativeUnwinderAndroid>(reinterpret_cast<uintptr_t>(&__executable_start), &map_delegate);
    unwinder_for_native->Initialize(&module_cache_for_native);

    ModuleCache module_cache_for_chrome;
    unwindstack::MapInfo* other_library_map = map_delegate.GetMapReference()->maps()->Find(GetAddressInOtherLibrary(other_library)).get();
    ASSERT_NE(nullptr, other_library_map);
    auto unwinder_for_chrome = std::make_unique<NativeUnwinderAndroid>(other_library_map->start(), &map_delegate);
    unwinder_for_chrome->Initialize(&module_cache_for_chrome);

    std::vector<Frame> sample = CaptureScenario(&scenario, &module_cache_for_native,
        BindLambdaForTesting([&](UnwinderStateCapture* capture_state, RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
            ...
            EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder_for_native->TryUnwind(capture_state, thread_context, stack_top, sample));
            ...
            EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder_for_chrome->TryUnwind(capture_state, thread_context, stack_top, sample));
            ...
            EXPECT_EQ(UnwindResult::kCompleted, unwinder_for_all->TryUnwind(capture_state, thread_context, stack_top, sample));
        }));
...
TEST(NativeUnwinderAndroidTest, UnwindStackMemoryTest)
{
    ...
    const auto check_read_succeeds = [&](uintptr_t addr, size_t size) {
        ...
        EXPECT_EQ(0, memcmp(reinterpret_cast<const uint8_t*>(addr), output.data(), size));
    };
...
TEST(NativeUnwinderAndroidTest, ModuleDebugBasenameForNonElf)
{
    ...
    AddMapInfo(0x1000u, 0x2000u, 0u, PROT_READ | PROT_EXEC, "[foo / bar]", { 0xAA }, *maps);

    ModuleCache module_cache;

    std::unique_ptr<NativeUnwinderAndroidMemoryRegionsMapImpl> memory_regions_map = CreateMemoryRegionsMap();
    memory_regions_map->SetMapsForTesting(std::move(maps));
    NativeUnwinderAndroidMapDelegateForTesting map_delegate(std::move(memory_regions_map));
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(0, &map_delegate);
    unwinder->Initialize(&module_cache);
...
TEST(NativeUnwinderAndroidTest, ModulesCreatedOnlyForExecutableRegions)
{
    ...
    const ModuleCache::Module* module1 = module_cache.GetModuleForAddress(0x1000u);
    const ModuleCache::Module* module2 = module_cache.GetModuleForAddress(0x2000u);
    const ModuleCache::Module* module3 = module_cache.GetModuleForAddress(0x3000u);
...
TEST(NativeUnwinderAndroidTest, AcquireAndReleaseMemoryRegionsMapThroughMapDelegate)
{
    NativeUnwinderAndroidMapDelegateForTesting map_delegate(CreateMemoryRegionsMap());
