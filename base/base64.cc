...
void Base64EncodeAppend(span<const uint8_t> input, std::string* output)
{
    ...
    const size_t after_size = base::CheckAdd(encode_data_len, output->size()).ValueOrDie();
    output->resize(after_size);

    span<const char> read = base::as_chars(input);
    span<char> write = base::span(output->data(), output->size()).last(encode_data_len);

    const size_t written_size = modp_b64_encode_data(write.data(), // This must point to `encode_data_len` many chars.
        read.data(), read.size());
...
bool Base64Decode(std::string_view input, std::string* output, Base64DecodePolicy policy)
{
    ...
    size_t written_size = modp_b64_decode(decode_buf.data(), input.data(), input.size(), GetModpPolicy(policy));
    ...
    if (written_size == MODP_B64_ERROR && policy == Base64DecodePolicy::kForgiving) {
        ...
        written_size = modp_b64_decode(decode_buf.data(), input_without_whitespace.data(), input_without_whitespace.size(), GetModpPolicy(policy));
    }
...
std::optional<std::vector<uint8_t>> Base64Decode(std::string_view input)
{
    ...
    span<char> write = base::as_writable_chars(base::span(write_buf.data(), write_buf.size()));

    size_t written_size = modp_b64_decode(write.data(), input.data(), input.size());
    if (written_size == MODP_B64_ERROR) {
        return std::nullopt;
    }
