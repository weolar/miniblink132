...
void RunTest(const char* hash_name, void (*hash)(base::span<const uint8_t>), const size_t len)
{
    ...
    perf_test::PerfResultReporter reporter(hash_name, NumberToString(len) + "_bytes");
    ...
    const auto rate = [len](TimeDelta t) { return kBytesPerMegabyte * (len / t.InMicrosecondsF()); };
    ...
    ranges::transform(utime, rate_strings.begin(), [rate](const auto& t) { return NumberToString(rate(t)); });
