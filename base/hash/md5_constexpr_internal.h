...
struct MD5CE {
    ...
    static constexpr std::array<uint32_t, 64> kConstants = { { 0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
        0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be, 0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821, 0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa,
        0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8, 0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed, 0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
        0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c, 0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70, 0x289b7ec6, 0xeaa127fa, 0xd4ef3085, 0x04881d05,
        0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665, 0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039, 0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1,
    ...
    static constexpr uint8_t GetPaddedMessageByte(std::string_view data, const uint32_t m, const uint32_t i)
    {
        DCHECK_LT(i, m);
    ...
    // endian order.
    static constexpr uint32_t GetPaddedMessageWord(std::string_view data, const uint32_t m, const uint32_t i)
    {
        DCHECK_EQ(i % 4, 0u);
        DCHECK_LT(i, m);
        DCHECK_LT(data.size(), m);
        DCHECK_EQ(m % 64, 0u);
        return static_cast<uint32_t>(GetPaddedMessageByte(data, m, i)) | static_cast<uint32_t>((GetPaddedMessageByte(data, m, i + 1)) << 8)
            | static_cast<uint32_t>((GetPaddedMessageByte(data, m, i + 2)) << 16) | static_cast<uint32_t>((GetPaddedMessageByte(data, m, i + 3)) << 24);
    }

    // Given an input buffer |data|, extracts one round worth of data starting at
    // offset |i|.
    static constexpr RoundData GetRoundData(std::string_view data, const uint32_t m, const uint32_t i)
    {
        DCHECK_EQ(i % 64, 0u);
        DCHECK_LT(i, m);
        DCHECK_LT(data.size(), m);
        DCHECK_EQ(m % 64, 0u);
        return RoundData { { GetPaddedMessageWord(data, m, i), GetPaddedMessageWord(data, m, i + 4), GetPaddedMessageWord(data, m, i + 8),
            GetPaddedMessageWord(data, m, i + 12), GetPaddedMessageWord(data, m, i + 16), GetPaddedMessageWord(data, m, i + 20),
            GetPaddedMessageWord(data, m, i + 24), GetPaddedMessageWord(data, m, i + 28), GetPaddedMessageWord(data, m, i + 32),
            GetPaddedMessageWord(data, m, i + 36), GetPaddedMessageWord(data, m, i + 40), GetPaddedMessageWord(data, m, i + 44),
            GetPaddedMessageWord(data, m, i + 48), GetPaddedMessageWord(data, m, i + 52), GetPaddedMessageWord(data, m, i + 56),
    ...
    {
        return IntermediateData { intermediate1.a + intermediate2.a, intermediate1.b + intermediate2.b, intermediate1.c + intermediate2.c,
    ...
    {
        const uint32_t m = GetPaddedMessageLength(checked_cast<uint32_t>(message.size()));
