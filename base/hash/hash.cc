...
size_t FastHashImpl(base::span<const uint8_t> data)
{
    ...
    if constexpr (sizeof(size_t) > 4) {
        return base::internal::cityhash_v111::CityHash64(chars.data(), chars.size());
    } else {
        return base::internal::cityhash_v111::CityHash32(chars.data(), chars.size());
    }
}
...
template <typename T> T Scramble(T input)
{
#if DCHECK_IS_ON()
...
uint32_t PersistentHash(std::string_view str)
{
    return PersistentHash(
        //as_bytes(make_span(str))
        span<const uint8_t>((const uint8_t*)str.data(), str.size())
    );
}
