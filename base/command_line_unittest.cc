...
TEST(CommandLineTest, CommandLineConstructor)
{
    const CommandLine::CharType* argv[]
        = { FILE_PATH_LITERAL("program"), FILE_PATH_LITERAL("--foo="), FILE_PATH_LITERAL("-bAr"), FILE_PATH_LITERAL("-spaetzel=pierogi"),
              FILE_PATH_LITERAL("-baz"), FILE_PATH_LITERAL("flim"), FILE_PATH_LITERAL("--other-switches=--dog=canine --cat=feline"),
              FILE_PATH_LITERAL("-spaetzle=Crepe"), FILE_PATH_LITERAL("-=loosevalue"), FILE_PATH_LITERAL("-"), FILE_PATH_LITERAL("FLAN"),
              FILE_PATH_LITERAL("a"), FILE_PATH_LITERAL("--input-translation=45--output-rotation"), FILE_PATH_LITERAL("--"), FILE_PATH_LITERAL("--"),
              FILE_PATH_LITERAL("--not-a-switch"), FILE_PATH_LITERAL("\"in the time of submarines...\""), FILE_PATH_LITERAL("unquoted arg-with-space") };
    ...
    EXPECT_EQ(FilePath(FILE_PATH_LITERAL("program")).value(), cl.GetProgram().value());
...
{
    CommandLine::StringVector argv = { FILE_PATH_LITERAL("--switch1"), FILE_PATH_LITERAL("--switch2=value2") };
...
{
#if BUILDFLAG(IS_WIN)
    CommandLine cl = CommandLine::FromString(L"program --foo= -bAr  /Spaetzel=pierogi /Baz flim "
                                             L"--other-switches=\"--dog=canine --cat=feline\" "
                                             L"-spaetzle=Crepe   -=loosevalue  FLAN "
                                             L"--input-translation=\"45\"--output-rotation "
                                             L"--quotes="
        + kTrickyQuoted + L" -- -- --not-a-switch \"in the time of submarines...\"");
...
{
    const CommandLine::CharType* const raw_argv[] = { FILE_PATH_LITERAL("prog"), FILE_PATH_LITERAL("--"), FILE_PATH_LITERAL("--arg1") };
    ...
    EXPECT_EQ(FILE_PATH_LITERAL("prog --switch1 --switch2=foo -- --arg1 --arg2"), cl.GetCommandLineString());
...
class CommandLineQuoteTest : public ::testing::TestWithParam<CommandLineQuoteTestCase> { };

INSTANTIATE_TEST_SUITE_P(CommandLineQuoteTestCases, CommandLineQuoteTest,
...
{
    EXPECT_EQ(CommandLine::QuoteForCommandLineToArgvW(GetParam().input_arg), GetParam().expected_output_arg);
}
...
class CommandLineQuoteAfterTest : public ::testing::TestWithParam<CommandLineQuoteAfterTestCase> { };

INSTANTIATE_TEST_SUITE_P(CommandLineQuoteAfterTestCases, CommandLineQuoteAfterTest,
...
{
    std::wstring input_command_line = base::StrCat({ LR"(c:\test\process.exe )", base::JoinString(GetParam().input_args, L" ") });
    int num_args = 0;
    base::win::ScopedLocalAllocTyped<wchar_t*> argv(::CommandLineToArgvW(&input_command_line[0], &num_args));
    ASSERT_EQ(num_args - 1U, GetParam().input_args.size());

    std::wstring recreated_command_line;
    for (int i = 1; i < num_args; ++i) {
        recreated_command_line.append(CommandLine::QuoteForCommandLineToArgvW(argv.get()[i]));
...
{
    CommandLine cl = CommandLine::FromString(FILE_PATH_LITERAL("program --switch /switch2 --"));
    EXPECT_EQ(cl.GetCommandLineStringForShell(), FILE_PATH_LITERAL("program --switch /switch2 -- --single-argument %1"));
}
...
TEST(CommandLineTest, GetCommandLineStringWithUnsafeInsertSequences)
{
    ...
    cl.AppendSwitch("%2");
    cl.AppendArg("%3");
    EXPECT_EQ(FILE_PATH_LITERAL("program --switch=%1 --%2 %3"), cl.GetCommandLineStringWithUnsafeInsertSequences());
}
...
TEST(CommandLineTest, HasSingleArgument)
{
    ...
    CommandLine cl_for_shell(CommandLine::FromString(cl.GetCommandLineStringForShell()));
...
TEST(CommandLineTest, MaintainSingleArgument)
{
    ...
    static const CommandLine::StringType kCommandLine = FILE_PATH_LITERAL("program --switch --single-argument foo bar.html");
...
TEST(CommandLineTest, CopySwitches)
{
    ...
    EXPECT_THAT(source.argv(), testing::ElementsAre(FILE_PATH_LITERAL(""), FILE_PATH_LITERAL("--a"), FILE_PATH_LITERAL("--bbbb"), FILE_PATH_LITERAL("--c")));
    ...
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL(""), FILE_PATH_LITERAL("--a"), FILE_PATH_LITERAL("--c")));
}
...
TEST(CommandLineTest, Move)
{
    ...
    EXPECT_THAT(move_constructed.GetArgs(), ::testing::ElementsAre(kArgs[0], kArgs[1]));
...
TEST(CommandLineTest, PrependComplexWrapper)
{
    ...
    cl.AppendSwitch("a");
    cl.AppendSwitch("b");
    cl.PrependWrapper(FILE_PATH_LITERAL("wrapper --foo='hello world' --bar=\"let's go\""));
...
TEST(CommandLineTest, RemoveSwitch)
{
    ...
    EXPECT_TRUE(cl.HasSwitch(switch2));
    EXPECT_EQ(value2, cl.GetSwitchValueASCII(switch2));
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--switch1"), FILE_PATH_LITERAL("--switch2=value")));
    ...
    EXPECT_TRUE(cl.HasSwitch(switch2));
    EXPECT_EQ(value2, cl.GetSwitchValueASCII(switch2));
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--switch2=value")));
}
...
TEST(CommandLineTest, RemoveSwitchWithValue)
{
    ...
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--switch1")));
}
...
TEST(CommandLineTest, RemoveSwitchDropsMultipleSameSwitches)
{
    ...
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--switch1"), FILE_PATH_LITERAL("--switch1=value2")));
...
TEST(CommandLineTest, AppendAndRemoveSwitchWithDefaultPrefix)
{
    ...
    cl.AppendSwitch("foo");
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--foo")));
...
TEST(CommandLineTest, AppendAndRemoveSwitchPreservesOtherSwitchesAndArgs)
{
    ...
    cl.AppendArg("arg");
    EXPECT_THAT(
        cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--foo"), FILE_PATH_LITERAL("--bar"), FILE_PATH_LITERAL("arg")));
    EXPECT_THAT(cl.GetArgs(), testing::ElementsAre(FILE_PATH_LITERAL("arg")));

    cl.RemoveSwitch("foo");
    EXPECT_THAT(cl.argv(), testing::ElementsAre(FILE_PATH_LITERAL("Program"), FILE_PATH_LITERAL("--bar"), FILE_PATH_LITERAL("arg")));
    EXPECT_THAT(cl.GetArgs(), testing::ElementsAre(FILE_PATH_LITERAL("arg")));
}
...
public:
    ...
    void ResolveDuplicate(std::string_view key, CommandLine::StringViewType new_value, CommandLine::StringType& out_value) override;
};

MergeDuplicateFoosSemicolon::~MergeDuplicateFoosSemicolon() = default;

void MergeDuplicateFoosSemicolon::ResolveDuplicate(std::string_view key, CommandLine::StringViewType new_value, CommandLine::StringType& out_value)
{
...
TEST(CommandLineTest, MultipleFilterFileSwitch)
{
    ...
    CommandLine::SetDuplicateSwitchHandler(std::make_unique<MergeDuplicateFoosSemicolon>());
...
{
    CommandLine cl = CommandLine::FromString(FILE_PATH_LITERAL("program --switch_before arg_before "
    ...
    EXPECT_EQ(cl.GetArgs(), CommandLine::StringVector({ FILE_PATH_LITERAL("arg with spaces \"and quotes\" \"") }));

    CommandLine cl_without_arg = CommandLine::FromString(FILE_PATH_LITERAL("program --single-argument "));

    EXPECT_FALSE(cl_without_arg.GetCommandLineString().empty());
    EXPECT_EQ(FilePath(FILE_PATH_LITERAL("program")), cl_without_arg.GetProgram());
