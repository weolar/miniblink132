...
private:
    ...
    template <typename Type> bool ReadBuiltinType(Type* result);
    ...
    template <typename Type> const char* GetReadPointerAndAdvance();
    ...
    const char* GetReadPointerAndAdvance(size_t num_elements, size_t size_element);
...
public:
    ...
    inline Pickle(const char* data, size_t data_len)
        : Pickle(kUnownedData, span<const uint8_t>((const uint8_t*)data, data_len))
    {
    }
    ...
    virtual bool ReadAttachment(PickleIterator* iter, scoped_refptr<Attachment>* attachment) const;
