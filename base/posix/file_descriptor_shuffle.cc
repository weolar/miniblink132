...
bool PerformInjectiveMultimapDestructive(InjectiveMultimap* m, InjectionDelegate* delegate)
{
    ...
    for (size_t i_index = 0; i_index < m->size(); ++i_index) {
        ...
        for (size_t j_index = i_index + 1; j_index < m->size(); ++j_index) {
            ...
            DCHECK(i->dest != j->dest) << "Both fd " << i->source << " and " << j->source << " map to " << i->dest;
        }
...
bool PerformInjectiveMultimap(const InjectiveMultimap& m_in, InjectionDelegate* delegate)
{
