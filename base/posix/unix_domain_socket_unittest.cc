...
void CreateSocketPair(int fds[2])
{
    ...
    int nosigpipe = 1;
    ASSERT_EQ(0, setsockopt(fds[0], SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe, sizeof(nosigpipe)));
    ASSERT_EQ(0, setsockopt(fds[1], SOL_SOCKET, SO_NOSIGPIPE, &nosigpipe, sizeof(nosigpipe)));
#endif
}
...
TEST(UnixDomainSocketTest, SendRecvMsgAbortOnReplyFDClose)
{
    ...
    Pickle request;
    message_thread.task_runner()->PostTask(FROM_HERE, BindOnce(IgnoreResult(&UnixDomainSocket::SendRecvMsg), fds[1], nullptr, 0U, nullptr, request));

    // Receive the message.
    std::vector<ScopedFD> message_fds;
    uint8_t buffer[16];
    ASSERT_EQ(static_cast<int>(request.size()), UnixDomainSocket::RecvMsg(fds[0], buffer, sizeof(buffer), &message_fds));
    ...
    message_thread.task_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&event)));
...
TEST(UnixDomainSocketTest, SendRecvMsgAvoidsSIGPIPE)
{
    ...
    Pickle request;
    ASSERT_EQ(-1, UnixDomainSocket::SendRecvMsg(fds[1], nullptr, 0U, nullptr, request));
...
TEST(UnixDomainSocketTest, RecvPid)
{
    ...
    ASSERT_TRUE(UnixDomainSocket::SendMsg(send_sock.get(), kHello, sizeof(kHello), std::vector<int>()));
    ...
    const ssize_t nread = UnixDomainSocket::RecvMsgWithPid(recv_sock.get(), buf, sizeof(buf), &fd_vec, &sender_pid);
...
TEST(UnixDomainSocketTest, RecvPidWithMaxDescriptors)
{
    ...
    static const char kHello[] = "hello";
    std::vector<int> send_fds(UnixDomainSocket::kMaxFileDescriptors, send_sock.get());
    ASSERT_TRUE(UnixDomainSocket::SendMsg(send_sock.get(), kHello, sizeof(kHello), send_fds));
    ...
    const ssize_t nread = UnixDomainSocket::RecvMsgWithPid(recv_sock.get(), buf, sizeof(buf), &recv_fds, &sender_pid);
    ASSERT_EQ(sizeof(kHello), static_cast<size_t>(nread));
    ASSERT_EQ(0, memcmp(buf, kHello, sizeof(kHello)));
...
TEST(UnixDomianSocketTest, RecvPidDisconnectedSocket)
{
    ...
    ProcessId sender_pid;
    std::vector<ScopedFD> recv_fds;
    const ssize_t nread = UnixDomainSocket::RecvMsgWithPid(recv_sock.get(), &ch, sizeof(ch), &recv_fds, &sender_pid);
    ASSERT_EQ(0, nread);
