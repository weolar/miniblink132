...
public:
    ...
    static bool SendMsg(int fd, const void* msg, size_t length, const std::vector<int>& fds);
    ...
    static ssize_t RecvMsg(int fd, void* msg, size_t length, std::vector<ScopedFD>* fds);
    ...
    // receiving socket.
    static ssize_t RecvMsgWithPid(int fd, void* msg, size_t length, std::vector<ScopedFD>* fds, ProcessId* pid);
    ...
    static ssize_t SendRecvMsg(int fd, uint8_t* reply, unsigned reply_len, int* result_fd, const Pickle& request);

    // Similar to SendRecvMsg(), but |recvmsg_flags| allows to control the flags
    // of the recvmsg(2) call.
    static ssize_t SendRecvMsgWithFlags(int fd, uint8_t* reply, unsigned reply_len, int recvmsg_flags, int* result_fd, const Pickle& request);

private:
    // Similar to RecvMsg, but allows to specify |flags| for recvmsg(2).
    static ssize_t RecvMsgWithFlags(int fd, void* msg, size_t length, int flags, std::vector<ScopedFD>* fds, ProcessId* pid);
};
