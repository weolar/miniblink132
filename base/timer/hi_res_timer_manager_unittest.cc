...
TEST(HiResTimerManagerTest, ToggleOnOff)
{
    ...
    for (int times = 0; times != 3; ++times) {
        ...
        power_monitor_source.GeneratePowerStateEvent(PowerStateObserver::BatteryPowerStatus::kBatteryPower);

        EXPECT_FALSE(manager.hi_res_clock_available());
        EXPECT_FALSE(base::Time::IsHighResolutionTimerInUse());

        // Back to on-AC power.
        power_monitor_source.GeneratePowerStateEvent(PowerStateObserver::BatteryPowerStatus::kExternalPower);
        EXPECT_TRUE(manager.hi_res_clock_available());
        EXPECT_TRUE(base::Time::IsHighResolutionTimerInUse());
...
TEST(HiResTimerManagerTest, DisableFromCommandLine)
{
    ...
    command_line.GetProcessCommandLine()->AppendSwitch(switches::kDisableHighResTimer);
