...
TEST_F(WallClockTimerTest, PowerResume)
{
    ...
    WallClockTimer wall_clock_timer(&clock_, task_environment_.GetMockTickClock());
...
TEST_F(WallClockTimerTest, UseTimerTwiceInRow)
{
    ...
    EXPECT_CALL(first_callback, Run()).WillOnce(::testing::InvokeWithoutArgs([this, &wall_clock_timer, &second_callback, delay] {
        wall_clock_timer.Start(FROM_HERE, clock_.Now() + delay, second_callback.Get());
    }));
