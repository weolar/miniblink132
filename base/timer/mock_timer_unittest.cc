...
TEST(MockTimerTest, FiresOnce)
{
    ...
    timer.Start(FROM_HERE, delay, base::BindOnce(&CallMeMaybe, base::Unretained(&calls)));
...
TEST(MockTimerTest, FiresRepeatedly)
{
    ...
    base::TimeDelta delay = base::Seconds(2);
    timer.Start(FROM_HERE, delay, base::BindRepeating(&CallMeMaybe, base::Unretained(&calls)));
    timer.Fire();
    EXPECT_TRUE(timer.IsRunning());
    timer.Fire();
    timer.Fire();
...
TEST(MockTimerTest, DoesNotRetainClosure)
{
    ...
    timer.Start(FROM_HERE, delay, base::BindOnce([](HasWeakPtr*) {}, base::Owned(has_weak_ptr)));
    ASSERT_TRUE(weak_ptr.get());
    timer.Fire();
