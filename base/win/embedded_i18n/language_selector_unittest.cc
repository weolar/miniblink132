...
constexpr const wchar_t* kExactMatchCandidates[] = { L"am", L"ar", L"bg", L"bn", L"ca", L"cs", L"da", L"de", L"el", L"en-gb", L"en-us", L"es", L"es-419", L"et",
    L"fa", L"fi", L"fil", L"fr", L"gu", L"hi", L"hr", L"hu", L"id", L"it", L"iw", L"ja", L"kn", L"ko", L"lt", L"lv", L"ml", L"mr", L"nl", L"no", L"pl",
    L"pt-br", L"pt-pt", L"ro", L"ru", L"sk", L"sl", L"sr", L"sv", L"sw", L"ta", L"te", L"th", L"tr", L"uk", L"vi", L"zh-cn", L"zh-tw" };

constexpr const wchar_t* kAliasMatchCandidates[] = { L"he", L"nb", L"tl", L"zh-chs", L"zh-cht", L"zh-hans", L"zh-hant", L"zh-hk", L"zh-mo" };

constexpr const wchar_t* kWildcardMatchCandidates[] = { L"en-AU", L"es-CO", L"pt-AB", L"zh-SG" };
...
public:
    TestLanguageSelector()
        : TestLanguageSelector(std::vector<std::wstring>())
    {
    }
    ...
    TestLanguageSelector(const std::vector<std::wstring>& candidates, span<const LanguageSelector::LangToOffset> languages_to_offset)
...
class LanguageSelectorMatchCandidateTest : public ::testing::TestWithParam<const wchar_t*> { };
...
INSTANTIATE_TEST_SUITE_P(TestExactMatches, LanguageSelectorMatchCandidateTest, ::testing::ValuesIn(kExactMatchCandidates));

// Test the alias matches.
INSTANTIATE_TEST_SUITE_P(TestAliasMatches, LanguageSelectorMatchCandidateTest, ::testing::ValuesIn(kAliasMatchCandidates));

// Test a few wildcard matches.
INSTANTIATE_TEST_SUITE_P(TestWildcardMatches, LanguageSelectorMatchCandidateTest, ::testing::ValuesIn(kWildcardMatchCandidates));
...
class LanguageSelectorAliasTest : public ::testing::TestWithParam<std::tuple<const wchar_t*, const wchar_t*>> { };
...
INSTANTIATE_TEST_SUITE_P(
    EnGbAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"en-gb"), ::testing::Values(L"en-au", L"en-ca", L"en-nz", L"en-za")));

INSTANTIATE_TEST_SUITE_P(IwAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"iw"), ::testing::Values(L"he")));

INSTANTIATE_TEST_SUITE_P(NoAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"no"), ::testing::Values(L"nb")));

INSTANTIATE_TEST_SUITE_P(FilAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"fil"), ::testing::Values(L"tl")));

INSTANTIATE_TEST_SUITE_P(
    ZhCnAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"zh-cn"), ::testing::Values(L"zh-chs", L"zh-hans", L"zh-sg")));

INSTANTIATE_TEST_SUITE_P(
    ZhTwAliases, LanguageSelectorAliasTest, ::testing::Combine(::testing::Values(L"zh-tw"), ::testing::Values(L"zh-cht", L"zh-hant", L"zh-hk", L"zh-mo")));
...
TEST(LanguageSelectorTest, InvalidLanguageCasing)
{
    ...
    EXPECT_DCHECK_DEATH(LanguageSelector instance(std::vector<std::wstring>({ L"en-us" }), kLangToOffset));
}
...
{
    constexpr LanguageSelector::LangToOffset kLangToOffset[] = { { L"en-us", 0 }, { L"en-gb", 1 } };
    EXPECT_DCHECK_DEATH(LanguageSelector instance(std::vector<std::wstring>({ L"en-us" }), kLangToOffset));
}
...
TEST(LanguageSelectorTest, NoFallbackLanguageAvailable)
{
    ...
    EXPECT_DCHECK_DEATH(LanguageSelector instance(std::vector<std::wstring>({ L"aa-bb" }), kLangToOffset));
}

// No languages available.
TEST(LanguageSelectorTest, NoLanguagesAvailable)
{
    EXPECT_DCHECK_DEATH(LanguageSelector instance(std::vector<std::wstring>({ L"en-us" }), {}));
}
