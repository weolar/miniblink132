...
public:
    ...
    const VersionNumber& version_number() const
    {
    ...
    bool IsWowAMD64OnARM64() const; // Chrome AMD64 build on an ARM64 host machine.
    ...
    const std::string& release_id() const LIFETIME_BOUND
    {
        return release_id_;
    }
...
private:
    ...
    OSInfo(const _OSVERSIONINFOEXW& version_info, const _SYSTEM_INFO& system_info, DWORD os_type);
    ~OSInfo();

    // Returns a Version value for a given OS version tuple.
    static Version MajorMinorBuildToVersion(uint32_t major, uint32_t minor, uint32_t build);
