...
bool ProcessImportChunk(const PEImage& image, LPCSTR module, PIMAGE_THUNK_DATA name_table, PIMAGE_THUNK_DATA iat, PVOID cookie)
{
...
bool ProcessDelayImportChunk(
    const PEImage& image, PImgDelayDescr delay_descriptor, LPCSTR module, PIMAGE_THUNK_DATA name_table, PIMAGE_THUNK_DATA iat, PVOID cookie)
{
    EnumAllImportsStorage& storage = *reinterpret_cast<EnumAllImportsStorage*>(cookie);

    return image.EnumOneDelayImportChunk(storage.callback, delay_descriptor, module, name_table, iat, storage.cookie);
}
...
PIMAGE_SECTION_HEADER PEImage::GetImageSectionHeaderByName(LPCSTR section_name) const
{
    ...
    for (WORD i = 0; i < num_sections; ++i) {
        ...
        if (_strnicmp(reinterpret_cast<LPCSTR>(section->Name), section_name, sizeof(section->Name)) == 0) {
            return section;
        }
    }
...
bool PEImage::GetDebugId(LPGUID guid, LPDWORD age, LPCSTR* pdb_filename, size_t* pdb_filename_length) const
{
    DWORD debug_directory_size = GetImageDirectoryEntrySize(IMAGE_DIRECTORY_ENTRY_DEBUG);
    PIMAGE_DEBUG_DIRECTORY debug_directory = reinterpret_cast<PIMAGE_DEBUG_DIRECTORY>(GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_DEBUG));
    if (!debug_directory)
        return false;
    ...
    for (size_t index = 0; index < directory_count; ++index) {
        ...
        const PdbInfo* pdb_info = reinterpret_cast<const PdbInfo*>(RVAToAddr(entry.AddressOfRawData));
        if (!pdb_info)
        ...
        if (pdb_filename) {
            ...
            for (const char* const end = pdb_info->PdbFileName + length_max; eos < end && *eos; ++eos)
                ;
...
FARPROC PEImage::GetProcAddress(LPCSTR function_name) const
{
    ...
    PBYTE exports = reinterpret_cast<PBYTE>(GetImageDirectoryEntryAddr(IMAGE_DIRECTORY_ENTRY_EXPORT));
...
bool PEImage::EnumImportChunks(EnumImportChunksFunction callback, PVOID cookie, LPCSTR target_module_name) const
{
    ...
    for (; import->FirstThunk; import++) {
        ...
        PIMAGE_THUNK_DATA name_table = reinterpret_cast<PIMAGE_THUNK_DATA>(RVAToAddr(import->OriginalFirstThunk));
...
bool PEImage::EnumOneImportChunk(EnumImportsFunction callback, LPCSTR module_name, PIMAGE_THUNK_DATA name_table, PIMAGE_THUNK_DATA iat, PVOID cookie) const
{
    ...
    for (; name_table && name_table->u1.Ordinal; name_table++, iat++) {
        ...
        if (IMAGE_SNAP_BY_ORDINAL(name_table->u1.Ordinal)) {
            ...
            PIMAGE_IMPORT_BY_NAME import = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(RVAToAddr(name_table->u1.ForwarderString));
...
bool PEImage::EnumAllImports(EnumImportsFunction callback, PVOID cookie, LPCSTR target_module_name) const
{
    EnumAllImportsStorage temp = { callback, cookie };
    return EnumImportChunks(ProcessImportChunk, &temp, target_module_name);
}

bool PEImage::EnumDelayImportChunks(EnumDelayImportChunksFunction callback, PVOID cookie, LPCSTR target_module_name) const
{
    *(int*)1 = 1;
...
bool PEImage::EnumOneDelayImportChunk(
    EnumImportsFunction callback, PImgDelayDescr delay_descriptor, LPCSTR module_name, PIMAGE_THUNK_DATA name_table, PIMAGE_THUNK_DATA iat, PVOID cookie) const
{
    for (; name_table->u1.Ordinal; name_table++, iat++) {
        ...
        if (IMAGE_SNAP_BY_ORDINAL(name_table->u1.Ordinal)) {
            ...
            if (rvas) {
                import = reinterpret_cast<PIMAGE_IMPORT_BY_NAME>(RVAToAddr(name_table->u1.ForwarderString));
            } else {
...
bool PEImage::EnumAllDelayImports(EnumImportsFunction callback, PVOID cookie, LPCSTR target_module_name) const
{
    EnumAllImportsStorage temp = { callback, cookie };
...
bool PEImage::ImageRVAToOnDiskOffset(uintptr_t rva, DWORD* on_disk_offset) const
{
...
bool PEImage::ImageAddrToOnDiskOffset(LPVOID address, DWORD* on_disk_offset) const
{
    ...
    DWORD offset_within_section = static_cast<DWORD>(reinterpret_cast<uintptr_t>(address))
        - static_cast<DWORD>(reinterpret_cast<uintptr_t>(PEImage::RVAToAddr(section_header->VirtualAddress)));
...
const IMAGE_DATA_DIRECTORY* PEImage::GetDataDirectory(UINT directory) const
{
    ...
    if (nt_headers->FileHeader.SizeOfOptionalHeader < (offsetof(IMAGE_OPTIONAL_HEADER, DataDirectory) + (directory + 1) * sizeof(IMAGE_DATA_DIRECTORY))) {
        return nullptr;
    }
