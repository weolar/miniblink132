...
TEST(WinrtStorageUtilTest, CreateBufferFromData)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(CreateIBufferFromData(data.data(), data.size(), &buffer));

    uint8_t* p_buffer_data;
    uint32_t length;
    ASSERT_HRESULT_SUCCEEDED(GetPointerToBufferData(buffer.Get(), &p_buffer_data, &length));
