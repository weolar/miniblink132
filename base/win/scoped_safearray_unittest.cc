...
static void PopulateScopedSafearrayOfInts(ScopedSafearray& scoped_safe_array)
{
    ...
    int* int_array;
    ASSERT_HRESULT_SUCCEEDED(SafeArrayAccessData(scoped_safe_array.Get(), reinterpret_cast<void**>(&int_array)));
...
TEST(ScopedSafearrayTest, ScopedSafearrayMethods)
{
    ...
    SAFEARRAY* safe_array = SafeArrayCreateVector(VT_R8 /* element type */, 0 /* lower bound */, 4 /* elements */);
    ...
    ScopedSafearray safe_array_received(SafeArrayCreateVector(VT_R8 /* element type */, 0 /* lower bound */, 4 /* elements */));
...
{
    SAFEARRAY* safe_array = SafeArrayCreateVector(VT_R8 /* element type */, 1 /* lower bound */, 5 /* elements */);
    ScopedSafearray scoped_safe_array(safe_array);
    EXPECT_EQ(SafeArrayGetDim(scoped_safe_array.Get()), 1U);

    LONG lower_bound;
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetLBound(scoped_safe_array.Get(), 1, &lower_bound));
    EXPECT_EQ(lower_bound, 1);

    LONG upper_bound;
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetUBound(scoped_safe_array.Get(), 1, &upper_bound));
    EXPECT_EQ(upper_bound, 5);

    VARTYPE variable_type;
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetVartype(scoped_safe_array.Get(), &variable_type));
...
TEST(ScopedSafearrayTest, ScopedSafearrayInitialLockScope)
{
    ...
    std::optional<ScopedSafearray::LockScope<VT_I4>> lock_scope = scoped_safe_array.CreateLockScope<VT_I4>();
...
TEST(ScopedSafearrayTest, ScopedSafearrayLockScopeMoveConstructor)
{
    ...
    std::optional<ScopedSafearray::LockScope<VT_I4>> first = scoped_safe_array.CreateLockScope<VT_I4>();
...
TEST(ScopedSafearrayTest, ScopedSafearrayLockScopeTypeMismatch)
{
    ...
    {
        std::optional<ScopedSafearray::LockScope<VT_BSTR>> invalid_lock_scope = scoped_safe_array.CreateLockScope<VT_BSTR>();
        EXPECT_FALSE(invalid_lock_scope.has_value());
    }

    {
        std::optional<ScopedSafearray::LockScope<VT_UI4>> invalid_lock_scope = scoped_safe_array.CreateLockScope<VT_UI4>();
        EXPECT_FALSE(invalid_lock_scope.has_value());
    }
}
