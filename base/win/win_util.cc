...
using QueryKeyFunction = ScopedDeviceConvertibilityStateForTesting::QueryFunction;
...
bool EnablePerMonitorV2()
{
    ...
    static const auto set_process_dpi_awareness_context_func
        = reinterpret_cast<decltype(&::SetProcessDpiAwarenessContext)>(GetUser32FunctionPointer("SetProcessDpiAwarenessContext"));
    ...
    DCHECK_LT(GetVersion(), Version::WIN10_RS2) << "SetProcessDpiAwarenessContext should be available on all platforms"
...
bool* GetRegisteredWithManagementStateStorage()
{
    static bool state = [] {
        ...
        IsDeviceRegisteredWithManagementFunction is_device_registered_with_management_function
            = reinterpret_cast<IsDeviceRegisteredWithManagementFunction>(library.GetFunctionPointer("IsDeviceRegisteredWithManagement"));
...
bool* GetAzureADJoinStateStorage()
{
    static bool state = [] {
        ...
        ScopedNativeLibrary netapi32(base::LoadSystemLibrary(FILE_PATH_LITERAL("netapi32.dll")));
        if (!netapi32.is_valid())
            return false;

        const auto net_get_aad_join_information_function
            = reinterpret_cast<decltype(&::NetGetAadJoinInformation)>(netapi32.GetFunctionPointer("NetGetAadJoinInformation"));
        if (!net_get_aad_join_information_function)
            return false;

        const auto net_free_aad_join_information_function
            = reinterpret_cast<decltype(&::NetFreeAadJoinInformation)>(netapi32.GetFunctionPointer("NetFreeAadJoinInformation"));
        DPCHECK(net_free_aad_join_information_function);

        DSREG_JOIN_INFO* join_info = nullptr;
        HRESULT hr = net_get_aad_join_information_function(/*pcszTenantId=*/nullptr, &join_info);
        const bool is_aad_joined = SUCCEEDED(hr) && join_info;
        if (join_info) {
            net_free_aad_join_information_function(join_info);
        }

        base::UmaHistogramTimes("EnterpriseCheck.AzureADJoinStatusCheckTime", timer.Elapsed());
...
NativeLibrary PinUser32Internal(NativeLibraryLoadError* error)
{
    ...
    static const NativeLibrary user32_module = PinSystemLibrary(FILE_PATH_LITERAL("user32.dll"), &load_error);
...
bool& IsDeviceFormConvertible()
{
    static bool is_convertible = [] {
        ...
        using lpfnRtlGetDeviceFamilyInfo = VOID(WINAPI*)(ULONGLONG*, DWORD*, DWORD*);
        static const lpfnRtlGetDeviceFamilyInfo get_device_family_info_fn
            = reinterpret_cast<lpfnRtlGetDeviceFamilyInfo>(GetProcAddress(::GetModuleHandle(L"ntdll.dll"), "RtlGetDeviceFamilyInfoEnum"));
        ...
        return (deviceForm == DEVICEFAMILYDEVICEFORM_TABLET) || (deviceForm == DEVICEFAMILYDEVICEFORM_CONVERTIBLE)
...
{
    *(int*)1 = 1;
    static bool chassis_convertible = false;
    return chassis_convertible;
//     static bool chassis_convertible = [] {
//         ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
//         AssertComApartmentType(ComApartmentType::STA);
// 
//         constexpr std::wstring_view kQuery = L"select ChassisTypes from Win32_SystemEnclosure";
//         Microsoft::WRL::ComPtr<IEnumWbemClassObject> enumerator;
//         if (RunWmiQuery(kCimV2ServerName, std::wstring(kQuery), &enumerator).has_value()) {
//             return false;
//         }
// 
//         ULONG obj_count = 0;
...
//             hr = info->Get(L"ChassisTypes", 0, chassisTypeVariant.Receive(), nullptr, nullptr);
//             if (FAILED(hr)) {
//                 return false;
//             }
// 
...
//         }
// 
//         return (chassis_type_id == kTabletChassisType) || (chassis_type_id == kConvertibleChassisType) || (chassis_type_id == kDetachableChassisType);
//     }();
//     return chassis_convertible;
}
...
{
    static std::optional<bool> convertibility_enabled = []() -> std::optional<bool> {
        DWORD data;
        base::win::RegKey key(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\PriorityControl", KEY_QUERY_VALUE);
        return key.ReadValueDW(L"ConvertibilityEnabled", &data) == ERROR_SUCCESS ? std::make_optional(data != 0) : std::nullopt;
    }();
...
std::optional<bool>& GetConvertibleChassisKeyValue()
{
    static std::optional<bool> convertible_chassis = []() -> std::optional<bool> {
        DWORD data;
        base::win::RegKey key(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\TabletTip\\ConvertibleChassis", KEY_QUERY_VALUE);
        return key.ReadValueDW(L"ConvertibleChassis", &data) == ERROR_SUCCESS ? std::make_optional(data != 0) : std::nullopt;
    }();
...
QueryKeyFunction& HasCSMStateChanged()
{
    static QueryKeyFunction state = []() {
        DWORD data;
        base::win::RegKey key(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\TabletTip\\ConvertibleSlateModeChanged", KEY_QUERY_VALUE);
        bool value_exists = key.ReadValueDW(L"ConvertibleSlateModeChanged", &data) == ERROR_SUCCESS;
...
//         base::win::RegKey registry_key(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\PriorityControl", KEY_QUERY_VALUE);
//         DWORD slate_mode = 0;
//         bool value_exists = registry_key.ReadValueDW(L"ConvertibleSlateMode", &slate_mode) == ERROR_SUCCESS;
...
//         return !(value_exists && slate_mode == kKeyboardPresent) && IsDeviceUsedAsATablet(/*reason=*/nullptr);
//     }
// 
//     ScopedHString view_settings_guid = ScopedHString::Create(RuntimeClass_Windows_UI_ViewManagement_UIViewSettings);
//     Microsoft::WRL::ComPtr<IUIViewSettingsInterop> view_settings_interop;
//     HRESULT hr = ::RoGetActivationFactory(view_settings_guid.get(), IID_PPV_ARGS(&view_settings_interop));
//     if (FAILED(hr))
//         return false;
// 
...
//         return false;
// 
//     ABI::Windows::UI::ViewManagement::UserInteractionMode mode = ABI::Windows::UI::ViewManagement::UserInteractionMode_Mouse;
...
bool IsKeyboardPresentOnSlate(HWND hwnd, std::string* reason)
{
    ...
    *reason += "Not a tablet device";
    ...
    static const auto get_rotation_state = reinterpret_cast<GetAutoRotationState>(GetUser32FunctionPointer("GetAutoRotationState"));
    ...
    HDEVINFO device_info = SetupDiGetClassDevs(&KEYBOARD_CLASS_GUID, nullptr, nullptr, DIGCF_PRESENT);
    ...
    for (DWORD i = 0;; ++i) {
        ...
        CONFIGRET status = CM_Get_Device_ID(device_info_data.DevInst, device_id, MAX_DEVICE_ID_LEN, 0);
        if (status == CR_SUCCESS) {
            // To reduce the scope of the hack we only look for ACPI and HID\\VID
            // prefixes in the keyboard device ids.
            if (StartsWith(device_id, L"ACPI", CompareCase::INSENSITIVE_ASCII) || StartsWith(device_id, L"HID\\VID", CompareCase::INSENSITIVE_ASCII)) {
                if (reason) {
                    *reason += "device: ";
...
bool UserAccountControlIsEnabled()
{
    ...
    RegKey key(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System", KEY_READ);
    DWORD uac_enabled;
...
bool IsDeviceUsedAsATablet(std::string* reason)
{
    ...
    static const auto get_auto_rotation_state_func = reinterpret_cast<GetAutoRotationStateType>(GetUser32FunctionPointer("GetAutoRotationState"));
    if (get_auto_rotation_state_func) {
        AR_STATE rotation_state = AR_ENABLED;
        if (get_auto_rotation_state_func(&rotation_state) && (rotation_state & (AR_NOT_SUPPORTED | AR_LAPTOP | AR_NOSENSOR)) != 0) {
...
bool IsUser32AndGdi32Available()
{
    static const bool is_user32_and_gdi32_available = [] {
        ...
        if (::GetProcessMitigationPolicy(GetCurrentProcess(), ProcessSystemCallDisablePolicy, &policy, sizeof(policy))) {
...
bool GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot)
{
    ...
    do {
        ...
        if (!::EnumProcessModules(process, &(*snapshot)[0], static_cast<DWORD>(snapshot->size() * sizeof(HMODULE)), &bytes_required)) {
        ...
        if (num_modules <= snapshot->size()) {
            ...
            snapshot->erase(snapshot->begin() + static_cast<ptrdiff_t>(num_modules), snapshot->end());
            return true;
        }
...
{
    ::SetProp(hwnd, MICROSOFT_TABLETPENSERVICE_PROPERTY, reinterpret_cast<HANDLE>(TABLET_DISABLE_FLICKS | TABLET_DISABLE_FLICKFALLBACKKEYS));
}
...
std::wstring WStringFromGUID(const ::GUID& rguid)
{
    ...
    CHECK(SUCCEEDED(StringCchPrintfW(guid_string, kGuidStringCharacters, L"{%08lX-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X}", rguid.Data1, rguid.Data2,
        rguid.Data3, rguid.Data4[0], rguid.Data4[1], rguid.Data4[2], rguid.Data4[3], rguid.Data4[4], rguid.Data4[5], rguid.Data4[6], rguid.Data4[7])));
...
std::wstring GetWindowObjectName(HANDLE handle)
{
    ...
    if (!::GetUserObjectInformation(handle, UOI_NAME, WriteInto(&object_name, size / sizeof(wchar_t)), size, &size)) {
        DPCHECK(false);
    }
...
bool IsRunningUnderDesktopName(std::wstring_view desktop_name)
{
    ...
    return EqualsCaseInsensitiveASCII(AsStringPiece16(current_desktop_name), AsStringPiece16(desktop_name));
}
...
bool IsCurrentSessionRemote()
{
    ...
    static constexpr wchar_t kRdpSettingsKeyName[] = L"SYSTEM\\CurrentControlSet\\Control\\Terminal Server";
...
std::optional<std::wstring> ExpandEnvironmentVariables(wcstring_view str)
{
    ...
    for (int iterations = 0; iterations < 5; iterations++) {
        DWORD result = ::ExpandEnvironmentStringsW(str.c_str(), base::WriteInto(&path_expanded, path_len), path_len);
        if (!result) {
...
ScopedDeviceRegisteredWithManagementForTesting::ScopedDeviceRegisteredWithManagementForTesting(bool state)
...
ScopedDeviceConvertibilityStateForTesting::ScopedDeviceConvertibilityStateForTesting(bool form_convertible, bool chassis_convertible,
    QueryKeyFunction csm_changed, std::optional<bool> convertible_chassis_key, std::optional<bool> convertibility_enabled)
    : initial_form_convertible_(&IsDeviceFormConvertible(), form_convertible)
    , initial_chassis_convertible_(&IsChassisConvertible(), chassis_convertible)
    , initial_csm_changed_(&HasCSMStateChanged(), csm_changed)
    , initial_convertible_chassis_key_(&GetConvertibleChassisKeyValue(), convertible_chassis_key)
    , initial_convertibility_enabled_(&GetConvertibilityEnabledOverride(), convertibility_enabled)
{
}

ScopedDeviceConvertibilityStateForTesting::~ScopedDeviceConvertibilityStateForTesting() = default;
