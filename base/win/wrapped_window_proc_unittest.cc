...
TEST(WrappedWindowProc, CatchesExceptions)
{
    ...
    HWND window = CreateWindow(class_name.c_str(), nullptr, 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, hinst, nullptr);
    ASSERT_TRUE(window);

    // Before generating the exception we make sure that the filter will see it.
    TestWrappedExceptionFiter wrapper;
    base::win::WinProcExceptionFilter old_filter = base::win::SetWinProcExceptionFilter(TestWrappedExceptionFiter::Filter);
