...
constexpr wchar_t kTest2DaclNoInherit[] = L"D:P(A;;FA;;;BA)(A;;FA;;;AU)(A;;FA;;;WD)";
...
constexpr wchar_t kBaseDir2DaclNoInherit[] = L"D:P(A;OICI;FR;;;AU)(A;OICI;FA;;;WD)";
...
TEST(SecurityUtilTest, GrantAccessToPathErrorCase)
{
    ...
    EXPECT_FALSE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, true));
    EXPECT_FALSE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, false));
    ASSERT_TRUE(CreateWithDacl(path, kBaseDacl, false));
    EXPECT_TRUE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, true));
    EXPECT_TRUE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, false));
    std::vector<Sid> large_sid_list;
    while (large_sid_list.size() < 0x10000) {
        auto sid = Sid::FromSddlString(L"S-1-5-1234-" + NumberToWString(large_sid_list.size()));
        ASSERT_TRUE(sid);
        large_sid_list.emplace_back(std::move(*sid));
    }
    EXPECT_FALSE(GrantAccessToPath(path, large_sid_list, FILE_GENERIC_READ, NO_INHERITANCE, false));
...
TEST(SecurityUtilTest, GrantAccessToPathFile)
{
    ...
    ASSERT_TRUE(sids2);
    EXPECT_TRUE(GrantAccessToPath(path, *sids2, GENERIC_ALL, NO_INHERITANCE, true));
...
TEST(SecurityUtilTest, GrantAccessToPathFileNoInherit)
{
    ...
    ASSERT_TRUE(CreateWithDacl(path, kBaseDacl, false));
    EXPECT_EQ(kBaseDacl, GetFileDacl(path));
    EXPECT_TRUE(GrantAccessToPath(path, {}, FILE_GENERIC_READ, NO_INHERITANCE, false));
    EXPECT_EQ(kBaseDacl, GetFileDacl(path));
    auto sids = Sid::FromSddlStringVector({ kAuthenticatedUsersSid });
    ASSERT_TRUE(sids);
    EXPECT_TRUE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, false));
    ...
    auto sids2 = Sid::FromSddlStringVector({ L"S-1-5-11", L"BA" });
    ASSERT_TRUE(sids2);
    EXPECT_TRUE(GrantAccessToPath(path, *sids2, GENERIC_ALL, NO_INHERITANCE, false));
...
TEST(SecurityUtilTest, DenyAccessToPathFile)
{
    ...
    ASSERT_TRUE(CreateWithDacl(path, kBaseDacl, false));
    EXPECT_EQ(kBaseDacl, GetFileDacl(path));
    EXPECT_TRUE(DenyAccessToPath(path, {}, FILE_GENERIC_READ, NO_INHERITANCE, true));
    EXPECT_EQ(kBaseDacl, GetFileDacl(path));
    auto sids = Sid::FromSddlStringVector({ kLocalGuestSid });
    ASSERT_TRUE(sids);
    EXPECT_TRUE(DenyAccessToPath(path, *sids, FILE_GENERIC_READ, NO_INHERITANCE, true));
...
TEST(SecurityUtilTest, GrantAccessToPathDirectory)
{
    ...
    auto sids = Sid::FromSddlStringVector({ kAuthenticatedUsersSid });
    ASSERT_TRUE(sids);
    EXPECT_TRUE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE, true));
...
TEST(SecurityUtilTest, GrantAccessToPathDirectoryNoInherit)
{
    ...
    auto sids = Sid::FromSddlStringVector({ kAuthenticatedUsersSid });
    ASSERT_TRUE(sids);
    EXPECT_TRUE(GrantAccessToPath(path, *sids, FILE_GENERIC_READ, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE, false));
...
{
    std::vector<Sid> sids = Sid::FromKnownSidVector({ WellKnownSid::kNull, WellKnownSid::kWorld });
...
TEST(SecurityUtilTest, AppendSidVector)
{
    ...
    std::vector<Sid> sids2 = Sid::FromKnownSidVector({ WellKnownSid::kCreatorOwner, WellKnownSid::kNetwork });
