...
public:
    explicit DecrementCountDelegate(int* counter)
        : counter_(counter)
    {
    }
    void OnObjectSignaled(HANDLE object) override
...
void RunTest_BasicSignal(test::TaskEnvironment::MainThreadType main_thread_type)
{
...
void RunTest_BasicCancel(test::TaskEnvironment::MainThreadType main_thread_type)
{
    test::TaskEnvironment task_environment(main_thread_type);
...
void RunTest_CancelAfterSet(test::TaskEnvironment::MainThreadType main_thread_type)
{
    test::TaskEnvironment task_environment(main_thread_type);
...
void RunTest_SignalBeforeWatch(test::TaskEnvironment::MainThreadType main_thread_type)
{
    test::TaskEnvironment task_environment(main_thread_type);
...
void RunTest_OutlivesTaskEnvironment(test::TaskEnvironment::MainThreadType main_thread_type)
{
...
public:
    QuitAfterMultipleDelegate(HANDLE event, int iterations, base::OnceClosure quit_closure)
        : event_(event)
...
void RunTest_ExecuteMultipleTimes(test::TaskEnvironment::MainThreadType main_thread_type)
{
    test::TaskEnvironment task_environment(main_thread_type);
...
{
    RunTest_OutlivesTaskEnvironment(test::TaskEnvironment::MainThreadType::DEFAULT);
