...
TEST_P(PeImageReaderTest, GetCoffFileHeader)
{
    ...
    EXPECT_EQ(expected_data_->optional_header_size, file_header->SizeOfOptionalHeader);
}

TEST_P(PeImageReaderTest, GetOptionalHeaderData)
{
    span<const uint8_t> optional_header_data = image_reader_.GetOptionalHeaderData();
    ASSERT_THAT(optional_header_data, testing::Not(testing::IsEmpty()));
}

TEST_P(PeImageReaderTest, GetNumberOfSections)
{
    EXPECT_EQ(expected_data_->number_of_sections, image_reader_.GetNumberOfSections());
}
...
TEST_P(PeImageReaderTest, GetSectionHeaderAt)
{
    ...
    for (size_t i = 0; i < number_of_sections; ++i) {
        const IMAGE_SECTION_HEADER* section_header = image_reader_.GetSectionHeaderAt(i);
...
TEST_P(PeImageReaderTest, InitializeFailTruncatedFile)
{
    ...
    const IMAGE_SECTION_HEADER* last_section_header = image_reader_.GetSectionHeaderAt(image_reader_.GetNumberOfSections() - 1);
    const uint8_t* headers_end = reinterpret_cast<const uint8_t*>(last_section_header) + sizeof(*last_section_header);
...
{
    EXPECT_EQ(expected_data_->number_of_debug_entries, image_reader_.GetNumberOfDebugEntries());
}
...
TEST_P(PeImageReaderTest, GetDebugEntry)
{
    ...
    for (size_t i = 0; i < number_of_debug_entries; ++i) {
        ...
        const IMAGE_DEBUG_DIRECTORY* entry = image_reader_.GetDebugEntry(i, raw_data);
...
INSTANTIATE_TEST_SUITE_P(WordSize32, PeImageReaderTest, testing::Values(&kTestData[0]));
INSTANTIATE_TEST_SUITE_P(WordSize64, PeImageReaderTest, testing::Values(&kTestData[1]));
...
public:
    ...
    static bool OnCertificateCallback(uint16_t revision, uint16_t certificate_type, base::span<const uint8_t> certificate_data, void* context)
    {
        return reinterpret_cast<CertificateReceiver*>(context)->OnCertificate(revision, certificate_type, certificate_data);
    }

protected:
    CertificateReceiver() = default;
    virtual ~CertificateReceiver() = default;
    virtual bool OnCertificate(uint16_t revision, uint16_t certificate_type, base::span<const uint8_t> certificate_data) = 0;
};
...
public:
    ...
    MOCK_METHOD3(OnCertificate, bool(uint16_t, uint16_t, base::span<const uint8_t>));
};
...
class PeImageReaderCertificateTest : public testing::TestWithParam<const CertificateTestData*> {
protected:
...
TEST_P(PeImageReaderCertificateTest, EnumCertificates)
{
    ...
    if (expected_data_->num_signers) {
        EXPECT_CALL(receiver, OnCertificate(WIN_CERT_REVISION_2_0, WIN_CERT_TYPE_PKCS_SIGNED_DATA, testing::Not(testing::IsEmpty())))
            .Times(expected_data_->num_signers)
            .WillRepeatedly(Return(true));
    }
    EXPECT_TRUE(image_reader_.EnumCertificates(&CertificateReceiver::OnCertificateCallback, receiver.AsContext()));
}
...
TEST_P(PeImageReaderCertificateTest, AbortEnum)
{
    ...
    if (expected_data_->num_signers) {
        ...
        EXPECT_FALSE(image_reader_.EnumCertificates(&CertificateReceiver::OnCertificateCallback, receiver.AsContext()));
    } else {
...
INSTANTIATE_TEST_SUITE_P(SignedExe, PeImageReaderCertificateTest, testing::Values(&kCertificateTestData[0]));
INSTANTIATE_TEST_SUITE_P(UnsignedExe, PeImageReaderCertificateTest, testing::Values(&kCertificateTestData[1]));
INSTANTIATE_TEST_SUITE_P(DisableOutdatedBuildDetectorExe, PeImageReaderCertificateTest, testing::Values(&kCertificateTestData[2]));
INSTANTIATE_TEST_SUITE_P(SignedTwiceExe, PeImageReaderCertificateTest, testing::Values(&kCertificateTestData[3]));
