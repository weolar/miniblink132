...
public:
    ...
    template <VARTYPE ElementVartype> class BASE_EXPORT LockScope final {
    public:
        // Type declarations to support std::iterator_traits
        using iterator_category = std::random_access_iterator_tag;
        using value_type = typename internal::VariantConverter<ElementVartype>::Type;
    ...
    template <VARTYPE ElementVartype> std::optional<LockScope<ElementVartype>> CreateLockScope() const
    {
        ...
        if (FAILED(hr) || !internal::VariantConverter<ElementVartype>::IsConvertibleTo(vartype)) {
            return std::nullopt;
        }
