...
bool RegKey::Watcher::StartWatching(HKEY key, ChangeCallback callback)
{
    ...
    DWORD filter = REG_NOTIFY_CHANGE_NAME | REG_NOTIFY_CHANGE_ATTRIBUTES | REG_NOTIFY_CHANGE_LAST_SET | REG_NOTIFY_CHANGE_SECURITY | REG_NOTIFY_THREAD_AGNOSTIC;
    // Watch the registry key for a change of value.
    LONG result = RegNotifyChangeKeyValue(key, /*bWatchSubtree=*/TRUE, filter, watch_event_.get(), /*fAsynchronous=*/TRUE);
...
LONG RegKey::CreateWithDisposition(HKEY rootkey, const wchar_t* subkey, DWORD* disposition, REGSAM access)
{
    DCHECK(rootkey && subkey && access && disposition);
    HKEY subhkey = nullptr;
    LONG result = RegCreateKeyEx(rootkey, subkey, 0, nullptr, REG_OPTION_NON_VOLATILE, access, nullptr, &subhkey, disposition);
    if (result == ERROR_SUCCESS) {
        Close();
        key_ = subhkey;
        wow64access_ = access & kWow64AccessMask;
...
LONG RegKey::CreateKey(const wchar_t* name, REGSAM access)
{
    ...
    HKEY subkey = nullptr;
    LONG result = RegCreateKeyEx(key_, name, 0, nullptr, REG_OPTION_NON_VOLATILE, access, nullptr, &subkey, nullptr);
    if (result == ERROR_SUCCESS) {
        Close();
        key_ = subkey;
        wow64access_ = access & kWow64AccessMask;
...
{
    DWORD count = 0;
    LONG result = RegQueryInfoKey(key_, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &count, nullptr, nullptr, nullptr, nullptr);
    if (result == ERROR_SUCCESS) {
...
LONG RegKey::DeleteKey(const wchar_t* name, RecursiveDelete recursive)
{
    ...
    // behavior.
    RegKey target_key;
    LONG result = target_key.Open(key_, name, REG_OPTION_OPEN_LINK, wow64access_ | KEY_QUERY_VALUE | DELETE);
    if (result != ERROR_SUCCESS) {
        return result;
    }
...
{
    LONG result = RegQueryValueEx(key_, name, nullptr, dtype, reinterpret_cast<LPBYTE>(data), dsize);
    return result;
}
...
LONG RegKey::Open(HKEY rootkey, const wchar_t* subkey, DWORD options, REGSAM access)
{
...
std::optional<LONG> RegKey::DeleteIfLink()
{
    ...
    static const RtlNtStatusToDosErrorFunction rtl_nt_status_to_dos_error
        = reinterpret_cast<RtlNtStatusToDosErrorFunction>(::GetProcAddress(::GetModuleHandle(L"ntdll.dll"), "RtlNtStatusToDosError"));
    ...
    return rtl_nt_status_to_dos_error ? static_cast<LONG>(rtl_nt_status_to_dos_error(delete_result)) : ERROR_ACCESS_DENIED;
}
...
LONG RegKey::RegDelRecurse(HKEY root_key, const wchar_t* name, REGSAM access)
{
    ...
    RegKey target_key;
    LONG result = target_key.Open(root_key, name, REG_OPTION_OPEN_LINK, access | KEY_ENUMERATE_SUB_KEYS | KEY_QUERY_VALUE | DELETE);
    ...
    while (true) {
        ...
        if (::RegEnumKeyEx(target_key.key_, 0, &subkey_buffer[0], &key_size, nullptr, nullptr, nullptr, nullptr) != ERROR_SUCCESS) {
            break;
        }
        CHECK_LT(key_size, kMaxKeyNameLength);
        CHECK_EQ(subkey_buffer[key_size], L'\0');
        if (RegDelRecurse(target_key.key_, &subkey_buffer[0], access) != ERROR_SUCCESS) {
            break;
        }
    }
...
RegistryValueIterator::RegistryValueIterator(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
    : name_(MAX_PATH, '\0')
    , value_(MAX_PATH, '\0')
{
...
void RegistryValueIterator::Initialize(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
{
    ...
    if (result != ERROR_SUCCESS) {
        ...
        DWORD count = 0;
        result = ::RegQueryInfoKey(key_, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &count, nullptr, nullptr, nullptr, nullptr);
...
bool RegistryValueIterator::Read()
{
    if (Valid()) {
        ...
        LONG result
            = ::RegEnumValue(key_, index_, WriteInto(&name_, name_size), &name_size, nullptr, &type_, reinterpret_cast<BYTE*>(value_.data()), &value_size_);
...
RegistryKeyIterator::RegistryKeyIterator(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
{
    Initialize(root_key, folder_key, wow64access);
}
...
{
    DWORD count = 0;
    LONG result = ::RegQueryInfoKey(key_, nullptr, nullptr, nullptr, &count, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
    if (result != ERROR_SUCCESS)
        return 0;
...
void RegistryKeyIterator::Initialize(HKEY root_key, const wchar_t* folder_key, REGSAM wow64access)
{
    DCHECK_EQ(wow64access & ~kWow64AccessMask, static_cast<REGSAM>(0));
    LONG result = RegOpenKeyEx(root_key, folder_key, 0, KEY_READ | wow64access, &key_);
    if (result != ERROR_SUCCESS) {
        ...
        DWORD count = 0;
        result = ::RegQueryInfoKey(key_, nullptr, nullptr, nullptr, &count, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
