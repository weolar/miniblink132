...
bool InternalRunThreadTest()
{
    ...
    for (size_t i = 0; i < kNumThreads; i++) {
        HANDLE thread_handle = ::CreateThread(nullptr, 0, ThreadFunc, reinterpret_cast<void*>(&thread_params), 0, nullptr);
        if (!thread_handle)
            break;
...
bool InternalRunLocationTest()
{
    ...
    HMODULE verifier_module = base::win::internal::GetHandleVerifierModuleForTesting();
    if (!verifier_module)
        return false;
