...
template <class HEADER_TYPE> struct OptionalHeaderTraits { };

template <> struct OptionalHeaderTraits<IMAGE_OPTIONAL_HEADER32> {
    static const PeImageReader::WordSize word_size = PeImageReader::WORD_SIZE_32;
};

template <> struct OptionalHeaderTraits<IMAGE_OPTIONAL_HEADER64> {
...
template <class OPTIONAL_HEADER_TYPE> class PeImageReader::OptionalHeaderImpl : public PeImageReader::OptionalHeader {
public:
...
bool PeImageReader::Initialize(span<const uint8_t> image_data)
{
    ...
    if (!ValidateDosHeader() || !ValidatePeSignature() || !ValidateCoffFileHeader() || !ValidateOptionalHeader() || !ValidateSectionHeaders()) {
        Clear();
        return false;
    }
...
const IMAGE_FILE_HEADER* PeImageReader::GetCoffFileHeader()
{
    ...
    return reinterpret_cast<const IMAGE_FILE_HEADER*>(image_data_.subspan(checked_cast<size_t>(GetDosHeader()->e_lfanew) + sizeof(DWORD)).data());
}
...
const IMAGE_SECTION_HEADER* PeImageReader::GetSectionHeaderAt(size_t index)
{
    ...
    return reinterpret_cast<const IMAGE_SECTION_HEADER*>(GetOptionalHeaderStart() + GetOptionalHeaderSize() + (sizeof(IMAGE_SECTION_HEADER) * index));
}
...
{
    return GetImageData(IMAGE_DIRECTORY_ENTRY_DEBUG).size() / sizeof(IMAGE_DEBUG_DIRECTORY);
}

const IMAGE_DEBUG_DIRECTORY* PeImageReader::GetDebugEntry(size_t index, span<const uint8_t>& raw_data)
{
    DCHECK_LT(index, GetNumberOfDebugEntries());

    // Get the debug directory.
    span<const uint8_t> debug_directory_data = GetImageData(IMAGE_DIRECTORY_ENTRY_DEBUG);
    if (debug_directory_data.empty()) {
        return nullptr;
    }

    const IMAGE_DEBUG_DIRECTORY& entry = reinterpret_cast<const IMAGE_DEBUG_DIRECTORY&>(debug_directory_data[index * sizeof(IMAGE_DEBUG_DIRECTORY)]);
...
bool PeImageReader::EnumCertificates(EnumCertificatesCallback callback, void* context)
{
    ...
    while (!data.empty()) {
        const WIN_CERTIFICATE* win_certificate = reinterpret_cast<const WIN_CERTIFICATE*>(data.data());
        if (kWinCertificateSize > data.size() || kWinCertificateSize > win_certificate->dwLength || win_certificate->dwLength > data.size()) {
            return false;
        }
        if (!(*callback)(win_certificate->wRevision, win_certificate->wCertificateType,
                base::span(&win_certificate->bCertificate[0], win_certificate->dwLength - kWinCertificateSize), context)) {
            return false;
        }
...
bool PeImageReader::ValidateDosHeader()
{
    ...
    if (!GetStructureAt(0, &dos_header) || dos_header->e_magic != IMAGE_DOS_SIGNATURE || dos_header->e_lfanew < 0) {
        return false;
    }
...
bool PeImageReader::ValidatePeSignature()
{
    ...
    if (!GetStructureAt(static_cast<size_t>(GetDosHeader()->e_lfanew), &signature) || *signature != IMAGE_NT_SIGNATURE) {
        return false;
    }
...
bool PeImageReader::ValidateCoffFileHeader()
{
    ...
    if (!GetStructureAt(static_cast<size_t>(GetDosHeader()->e_lfanew) + offsetof(IMAGE_NT_HEADERS32, FileHeader), &file_header)) {
        return false;
    }
...
bool PeImageReader::ValidateOptionalHeader()
{
    ...
    const size_t optional_header_offset = static_cast<size_t>(GetDosHeader()->e_lfanew) + offsetof(IMAGE_NT_HEADERS32, OptionalHeader);
    const size_t optional_header_size = file_header->SizeOfOptionalHeader;
    const WORD* optional_header_magic = nullptr;

    if (optional_header_size < sizeof(*optional_header_magic) || !GetStructureAt(optional_header_offset, &optional_header_magic)) {
        return false;
    }

    std::unique_ptr<OptionalHeader> optional_header;
    if (*optional_header_magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER32>>(image_data_.subspan(optional_header_offset).data());
    } else if (*optional_header_magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER64>>(image_data_.subspan(optional_header_offset).data());
    } else {
        return false;
    }
    ...
    if (optional_header->GetDataDirectorySize() > ((optional_header_size - optional_header->GetDataDirectoryOffset()) / sizeof(IMAGE_DATA_DIRECTORY))) {
        return false;
    }
...
bool PeImageReader::ValidateSectionHeaders()
{
    ...
    if (!GetStructureAt(
            static_cast<size_t>(first_section_header - image_data_.data()), number_of_sections * sizeof(IMAGE_SECTION_HEADER), &first_section_header)) {
        return false;
    }
...
const uint8_t* PeImageReader::GetOptionalHeaderStart()
{
    ...
    return image_data_.subspan(checked_cast<size_t>(GetDosHeader()->e_lfanew) + offsetof(IMAGE_NT_HEADERS32, OptionalHeader)).data();
}
...
const IMAGE_DATA_DIRECTORY* PeImageReader::GetDataDirectoryEntryAt(size_t index)
{
    DCHECK_NE((validation_state_ & VALID_OPTIONAL_HEADER), 0U);
...
const IMAGE_SECTION_HEADER* PeImageReader::FindSectionFromRva(uint32_t relative_address)
{
    const size_t number_of_sections = GetNumberOfSections();
    for (size_t i = 0; i < number_of_sections; ++i) {
        ...
        if (!GetStructureAt(section_header->PointerToRawData, section_header->SizeOfRawData, &section_data)) {
            continue;
        }
...
span<const uint8_t> PeImageReader::GetImageData(size_t index)
{
    ...
    if (index == IMAGE_DIRECTORY_ENTRY_SECURITY) {
        ...
        if (entry->VirtualAddress > image_data_.size() || image_data_.size() - entry->VirtualAddress < entry->Size) {
            return span<const uint8_t>();
        }
    ...
    return image_data_.subspan(header->PointerToRawData + data_offset, entry->Size);
}
