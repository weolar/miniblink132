...
class FooBar : public WRL::RuntimeClass<WRL::RuntimeClassFlags<WRL::WinRt | WRL::InhibitRoOriginateError>, IFooBar> { };
...
template <>
struct DECLSPEC_UUID("124858e4-f97e-409c-86ae-418c4781144c") IAsyncOperation<FooBar*> : IAsyncOperation_impl<Internal::AggregateType<FooBar*, IFooBar*>> {
...
template <>
struct DECLSPEC_UUID("9e49373c-200c-4715-abd7-4214ba669c81") IAsyncOperationCompletedHandler<FooBar*>
    : IAsyncOperationCompletedHandler_impl<Internal::AggregateType<FooBar*, IFooBar*>> {
    static const wchar_t* z_get_rc_name_impl()
    {
...
template <typename T> void PutCallback(AsyncOperation<T>* async_op, bool* called_cb)
{
    async_op->put_Completed(WRL::Callback<IAsyncOperationCompletedHandler<T>>([=](IAsyncOperation<T>* iasync_op, AsyncStatus status) {
