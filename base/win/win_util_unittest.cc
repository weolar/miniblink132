...
public:
    ...
    ~ThreadLocaleSaver()
    {
        SetThreadLocale(original_locale_id_);
    }
...
{
    const GUID kGuid = { 0x7698f759, 0xf5b0, 0x4328, { 0x92, 0x38, 0xbd, 0x70, 0x8a, 0x6d, 0xc9, 0x63 } };
...
TEST(BaseWinUtilTest, GetWindowObjectName)
{
    ...
    HDESK desktop_handle = ::CreateDesktop(
        created_desktop_name.c_str(), nullptr, nullptr, 0, DESKTOP_CREATEWINDOW | DESKTOP_READOBJECTS | READ_CONTROL | WRITE_DAC | WRITE_OWNER, nullptr);
...
TEST(BaseWinUtilTest, IsRunningUnderDesktopName)
{
    ...
    EXPECT_TRUE(IsRunningUnderDesktopName(AsWString(ToLowerASCII(AsStringPiece16(desktop_name)))));
    EXPECT_TRUE(IsRunningUnderDesktopName(AsWString(ToUpperASCII(AsStringPiece16(desktop_name)))));
    EXPECT_FALSE(IsRunningUnderDesktopName(desktop_name + L"_non_existent_desktop_name"));
}
...
TEST(BaseWinUtilTest, ExpandEnvironmentVariables)
{
    ...
    EXPECT_EQ(ExpandEnvironmentVariables(path_with_env_var).value(), path_expanded);
}
...
TEST(DeviceConvertibilityTest, None)
{
    ...
    ScopedDeviceConvertibilityStateForTesting scoper(false, false, csm_false, std::nullopt, std::nullopt);
...
{
    RegKey key(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\PriorityControl", KEY_READ);
...
{
    RegKey key(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\TabletTip\\ConvertibleChassis", KEY_READ);
