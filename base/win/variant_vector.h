...
public:
    ...
    void Insert(typename internal::VariantConverter<ExpectedVartype>::Type value)
    {
    ...
    template <VARTYPE ExpectedVartype, std::enable_if_t<ExpectedVartype == VT_BOOL, int> = 0> void Insert(bool value)
    {
        if (vartype_ == VT_EMPTY)
            vartype_ = ExpectedVartype;
        AssertVartype<ExpectedVartype>();
    ...
    template <> void Insert<VT_DATE>(typename internal::VariantConverter<VT_DATE>::Type value)
    {
        if (vartype_ == VT_EMPTY)
            vartype_ = VT_DATE;
...
private:
    ...
    template <VARTYPE ExpectedVartype> void AssertVartype() const
    {
        DCHECK(internal::VariantConverter<ExpectedVartype>::IsConvertibleTo(vartype_))
    ...
    template <VARTYPE ElementVartype> SAFEARRAY* CreateAndPopulateSafearray();
