...
// descriptor.
enum class SecurityObjectType { kFile, kRegistry, kWindowStation, kDesktop, kKernel };
...
public:
    ...
    static std::optional<SecurityDescriptor> FromPointer(PSECURITY_DESCRIPTOR security_descriptor);

    // Create from the security descriptor of an existing file.
    // |path| the path to the file.
    // |security_info| indicates what parts to read.
    static std::optional<SecurityDescriptor> FromFile(const base::FilePath& path, SECURITY_INFORMATION security_info);
    ...
    // |security_info| indicates what parts to read.
    static std::optional<SecurityDescriptor> FromName(const std::wstring& name, SecurityObjectType object_type, SECURITY_INFORMATION security_info);
    ...
    // |security_info| indicates what parts to read.
    static std::optional<SecurityDescriptor> FromHandle(HANDLE handle, SecurityObjectType object_type, SECURITY_INFORMATION security_info);
    ...
    bool WriteToFile(const base::FilePath& path, SECURITY_INFORMATION security_info) const;
    ...
    // |security_info| indicates what parts to write.
    bool WriteToName(const std::wstring& name, SecurityObjectType object_type, SECURITY_INFORMATION security_info) const;
    ...
    bool WriteToHandle(HANDLE handle, SecurityObjectType object_type, SECURITY_INFORMATION security_info) const;
    ...
    bool SetMandatoryLabel(DWORD integrity_level, DWORD inheritance, DWORD mandatory_policy);
    ...
    // If DACL is not present a NULL ACL will be added first.
    bool SetDaclEntry(const Sid& sid, SecurityAccessMode mode, DWORD access_mask, DWORD inheritance);
    ...
    // error, with the Win32 last error set.
    // If DACL is not present a NULL ACL will be added first.
    bool SetDaclEntry(WellKnownSid known_sid, SecurityAccessMode mode, DWORD access_mask, DWORD inheritance);
    ...
    std::optional<AccessCheckResult> AccessCheck(const AccessToken& token, ACCESS_MASK desired_access, const GENERIC_MAPPING& generic_mapping);
    ...
    // Returns the result of the access check. If an empty result is returned the
    // call to the AccessCheck API failed.
    std::optional<AccessCheckResult> AccessCheck(const AccessToken& token, ACCESS_MASK desired_access, SecurityObjectType object_type);

    // Get, set and clear owner member.
    const std::optional<Sid>& owner() const
    {
        return owner_;
    }
    std::optional<Sid>& owner()
    {
        return owner_;
    }
    void set_owner(const Sid& owner)
    {
        owner_ = owner.Clone();
    }
    void clear_owner()
    ...
    const std::optional<Sid>& group() const
    {
        return group_;
    }
    ...
    void set_group(const Sid& group)
    {
        group_ = group.Clone();
    }
    void clear_group()
    ...
    const std::optional<AccessControlList>& dacl() const
    {
        return dacl_;
    }
    ...
    void set_dacl(const AccessControlList& dacl)
    {
        dacl_ = dacl.Clone();
    }
    void clear_dacl()
    ...
    bool dacl_protected() const
    {
        return dacl_protected_;
    }
    ...
    const std::optional<AccessControlList>& sacl() const
    {
        return sacl_;
    }
    ...
    void set_sacl(const AccessControlList& sacl)
    {
        sacl_ = sacl.Clone();
    }
    void clear_sacl()
    ...
    bool sacl_protected() const
    {
        return sacl_protected_;
    }
...
private:
    SecurityDescriptor(std::optional<Sid>&& owner, std::optional<Sid>&& group, std::optional<AccessControlList>&& dacl, bool dacl_protected,
