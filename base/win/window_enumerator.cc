...
bool IsShellWindow(HWND hwnd)
{
    ...
    return class_name == L"Button" || class_name == L"Shell_TrayWnd" || class_name == L"Shell_SecondaryTrayWnd";
}
...
std::wstring GetWindowClass(HWND hwnd)
{
    ...
    const int name_len = ::GetClassName(hwnd, window_class.data(), kMaxWindowClassNameLength);
