...
{
    HANDLE handle;
    if (!::DuplicateHandle(::GetCurrentProcess(), ::GetCurrentProcess(), ::GetCurrentProcess(), &handle, PROCESS_QUERY_LIMITED_INFORMATION, TRUE, 0)) {
...
{
    HANDLE temp_handle;
    if (!::DuplicateHandle(process, check_handle, ::GetCurrentProcess(), &temp_handle, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        return false;
    }
...
TEST(StartupInformationTest, InheritStdOut)
{
    ...
    ASSERT_TRUE(startup_info.UpdateProcThreadAttribute(PROC_THREAD_ATTRIBUTE_HANDLE_LIST, &inherit_process, sizeof(inherit_process)));
    ...
    ASSERT_TRUE(::CreateProcess(exe_path.value().c_str(), cmd_line, nullptr, nullptr, TRUE, EXTENDED_STARTUPINFO_PRESENT | CREATE_SUSPENDED, nullptr, nullptr,
    ...
    EXPECT_FALSE(CheckInheritedHandle(temp_process_info.hProcess, handle_1.get()));
}
