...
{
    EnumerateChildWindows(::GetDesktopWindow(), base::BindLambdaForTesting([&](HWND hwnd) {
        ...
        EXPECT_EQ(IsTopmostWindow(hwnd), (::GetWindowLong(hwnd, GWL_EXSTYLE) & WS_EX_TOPMOST) != 0);

        EXPECT_EQ(IsSystemDialog(hwnd), window_class == L"#32770");

        EXPECT_EQ(IsShellWindow(hwnd), window_class == L"Button" || window_class == L"Shell_TrayWnd" || window_class == L"Shell_SecondaryTrayWnd");
        EXPECT_EQ(GetWindowTextString(hwnd), [&] {
            ...
            if (!::GetWindowText(hwnd, &text.front(), static_cast<int>(text.size()))) {
                return std::wstring();
            }
