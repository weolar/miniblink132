...
{
    static constexpr std::wstring_view kSystemSettingsDefaultAppsPrefix(L"SystemSettings_DefaultApps_");
...
bool LaunchDefaultAppsSettingsModernDialog(std::wstring_view protocol)
{
    ...
    static constexpr wchar_t kControlPanelAppModelId[] = L"windows.immersivecontrolpanel_cw5n1h2txyewy"
                                                         L"!microsoft.windows.immersivecontrolpanel";

    Microsoft::WRL::ComPtr<IApplicationActivationManager> activator;
    HRESULT hr = ::CoCreateInstance(CLSID_ApplicationActivationManager, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&activator));
    if (FAILED(hr))
        return false;

    DWORD pid = 0;
    CoAllowSetForegroundWindow(activator.Get(), nullptr);
    hr = activator->ActivateApplication(kControlPanelAppModelId, L"page=SettingsPageAppsDefaults", AO_NONE, &pid);
    if (FAILED(hr))
        return false;
    if (protocol.empty())
        return true;
    ...
    hr = activator->ActivateApplication(
        kControlPanelAppModelId, base::StrCat({ L"page=SettingsPageAppsDefaults&target=", GetTargetForDefaultAppsSettings(protocol) }).c_str(), AO_NONE, &pid);
    return SUCCEEDED(hr);
}
