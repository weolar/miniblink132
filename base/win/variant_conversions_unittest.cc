...
static constexpr VARTYPE kSupportedVartypes[]
    = { VT_BOOL, VT_I1, VT_UI1, VT_I2, VT_UI2, VT_I4, VT_UI4, VT_I8, VT_UI8, VT_R4, VT_R8, VT_DATE, VT_BSTR, VT_UNKNOWN, VT_DISPATCH };

template <VARTYPE ElementVartype> static bool TestIsConvertibleTo(const std::set<VARTYPE>& allowed_vartypes)
{
    for (VARTYPE vartype : kSupportedVartypes) {
        if (VariantConverter<ElementVartype>::IsConvertibleTo(vartype) != base::Contains(allowed_vartypes, vartype)) {
            return false;
        }
    }
    return true;
...
template <VARTYPE ElementVartype> static bool TestIsConvertibleFrom(const std::set<VARTYPE>& allowed_vartypes)
{
    for (VARTYPE vartype : kSupportedVartypes) {
        if (VariantConverter<ElementVartype>::IsConvertibleFrom(vartype) != base::Contains(allowed_vartypes, vartype)) {
            return false;
        }
    }
    return true;
...
{
    Microsoft::WRL::ComPtr<IUnknown> unknown = Microsoft::WRL::Make<DispatchStub>();
...
{
    Microsoft::WRL::ComPtr<IDispatch> dispatch = Microsoft::WRL::Make<DispatchStub>();
