...
{
    win::MessageWindow window;
    EXPECT_TRUE(window.CreateNamed(base::BindRepeating(&HandleMessage), UTF8ToWide("test_message_window")));
}
...
{
    std::wstring name = UTF8ToWide(base::Uuid::GenerateRandomV4().AsLowercaseString());
    win::MessageWindow window;
