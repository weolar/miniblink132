...
void ScopedCOMInitializer::Initialize(COINIT init, Uninitialization uninitialization)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (uninitialization == Uninitialization::kBlockPremature) {
        com_balancer_ = Microsoft::WRL::Details::Make<internal::ComInitBalancer>(init | COINIT_DISABLE_OLE1DDE);
    }
    ...
    if (init == COINIT_APARTMENTTHREADED && hr_ == HRESULT_FROM_WIN32(ERROR_NOT_ENOUGH_MEMORY)) {
