...
template <class T> struct ScopedGDIObjectTraits {
...
template <> void inline ScopedGDIObjectTraits<HICON>::Free(HICON icon)
{
    DestroyIcon(icon);
}
...
template <class T> using ScopedGDIObject = ScopedGeneric<T, internal::ScopedGDIObjectTraits<T>>;
