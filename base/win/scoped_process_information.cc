...
bool CheckAndDuplicateHandle(HANDLE source, ScopedHandle* target)
{
    ...
    HANDLE temp = nullptr;
    if (!::DuplicateHandle(::GetCurrentProcess(), source, ::GetCurrentProcess(), &temp, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
...
ScopedProcessInformation::ScopedProcessInformation(ScopedProcessInformation&&) = default;

ScopedProcessInformation& ScopedProcessInformation::operator=(ScopedProcessInformation&&) = default;
...
bool ScopedProcessInformation::DuplicateFrom(const ScopedProcessInformation& other)
{
