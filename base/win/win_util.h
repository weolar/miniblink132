...
BASE_EXPORT bool SetBooleanValueForPropertyStore(IPropertyStore* property_store, const PROPERTYKEY& property_key, bool property_bool_value);

// Sets the string value for a given key in given IPropertyStore.
BASE_EXPORT bool SetStringValueForPropertyStore(IPropertyStore* property_store, const PROPERTYKEY& property_key, const wchar_t* property_string_value);

// Sets the CLSID value for a given key in a given IPropertyStore.
BASE_EXPORT bool SetClsidForPropertyStore(IPropertyStore* property_store, const PROPERTYKEY& property_key, const CLSID& property_clsid_value);
...
BASE_EXPORT bool SetAppIdForPropertyStore(IPropertyStore* property_store, const wchar_t* app_id);

// Adds the specified |command| using the specified |name| to the AutoRun key.
// |root_key| could be HKCU or HKLM or the root of any user hive.
BASE_EXPORT bool AddCommandToAutoRun(HKEY root_key, const std::wstring& name, const std::wstring& command);
...
BASE_EXPORT bool ReadCommandFromAutoRun(HKEY root_key, const std::wstring& name, std::wstring* command);
...
#define SIZEOF_STRUCT_WITH_SPECIFIED_LAST_MEMBER(struct_name, member) offsetof(struct_name, member) + (sizeof static_cast<struct_name*>(NULL)->member)
...
BASE_EXPORT bool GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot);
...
// cached.
BASE_EXPORT void* GetUser32FunctionPointer(const char* function_name, NativeLibraryLoadError* error = nullptr);
...
BASE_EXPORT bool RegisterPointerDeviceNotifications(HWND hwnd, bool notify_proximity_changes = false);
...
BASE_EXPORT std::optional<std::wstring> ExpandEnvironmentVariables(wcstring_view str);
...
public:
    ...
    ScopedDomainStateForTesting& operator=(const ScopedDomainStateForTesting&) = delete;
...
public:
    ...
    ScopedDeviceRegisteredWithManagementForTesting(const ScopedDeviceRegisteredWithManagementForTesting&) = delete;
    ScopedDeviceRegisteredWithManagementForTesting& operator=(const ScopedDeviceRegisteredWithManagementForTesting&) = delete;
...
public:
    ...
    ScopedAzureADJoinStateForTesting(const ScopedAzureADJoinStateForTesting&) = delete;
    ScopedAzureADJoinStateForTesting& operator=(const ScopedAzureADJoinStateForTesting&) = delete;
...
class BASE_EXPORT [[maybe_unused, nodiscard]] ScopedDeviceConvertibilityStateForTesting
{
public:
    using QueryFunction = bool (*)();
    ScopedDeviceConvertibilityStateForTesting(bool form_convertible, bool chassis_convertible, QueryFunction csm_changed,
        std::optional<bool> convertible_chassis_key, std::optional<bool> convertibility_enabled);
    ScopedDeviceConvertibilityStateForTesting(const ScopedDeviceConvertibilityStateForTesting&) = delete;
    ScopedDeviceConvertibilityStateForTesting& operator=(const ScopedDeviceConvertibilityStateForTesting&) = delete;
    ~ScopedDeviceConvertibilityStateForTesting();
