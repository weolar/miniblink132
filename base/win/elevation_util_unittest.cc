...
TEST(ElevationUtil, RunDeElevated)
{
    ...
    absl::Cleanup terminate_process = [&] { EXPECT_TRUE(process.Terminate(0, false)); };

    ASSERT_TRUE(IsProcessRunningAtMediumOrLower(process.Pid()));
}

class ElevationUtilRunDeElevatedNoWaitTest : public ::testing::TestWithParam<RepeatingCallback<HRESULT()>> { };

INSTANTIATE_TEST_SUITE_P(ElevationUtilRunDeElevatedNoWaitTestCases, ElevationUtilRunDeElevatedNoWaitTest,
    ::testing::Values(BindRepeating([] { return RunDeElevatedNoWait(CommandLine::FromString(kMoreExecutable)); }),
        BindRepeating([] { return RunDeElevatedNoWait(kMoreExecutable, {}); })));
...
TEST_P(ElevationUtilRunDeElevatedNoWaitTest, TestCases)
{
    ...
    ASSERT_EQ(GetProcessCount(kMoreExecutable, /*filter=*/nullptr), 0) << "This test requires that no instances of the `more` command are "
                                                                          "running.";
