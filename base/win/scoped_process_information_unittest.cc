...
protected:
    void DoCreateProcess(const std::string& main_id, PROCESS_INFORMATION* process_handle);
};
...
void ScopedProcessInformationTest::DoCreateProcess(const std::string& main_id, PROCESS_INFORMATION* process_handle)
{
    base::CommandLine::StringType cmd_line = MakeCmdLine(main_id).GetCommandLineString();
    STARTUPINFO startup_info = {};
    startup_info.cb = sizeof(startup_info);

    EXPECT_TRUE(::CreateProcess(nullptr, std::data(cmd_line), nullptr, nullptr, false, 0, nullptr, nullptr, &startup_info, process_handle));
}
