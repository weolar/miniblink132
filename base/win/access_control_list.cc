...
base::HeapArray<uint8_t> AclToBuffer(const ACL* acl)
{
    ...
    return base::HeapArray<uint8_t>::CopiedFrom(UNSAFE_BUFFERS(base::span(reinterpret_cast<const uint8_t*>(acl), size)));
}
...
base::HeapArray<uint8_t> AddACEToAcl(ACL* old_acl, const std::vector<ExplicitAccessEntry>& entries)
{
    ...
    DWORD error = ::SetEntriesInAcl(checked_cast<ULONG>(access_entries.size()), access_entries.data(), old_acl, &new_acl);
...
ExplicitAccessEntry::ExplicitAccessEntry(const Sid& sid, SecurityAccessMode mode, DWORD access_mask, DWORD inheritance)
    : sid_(sid.Clone())
    , mode_(mode)
...
ExplicitAccessEntry::ExplicitAccessEntry(WellKnownSid known_sid, SecurityAccessMode mode, DWORD access_mask, DWORD inheritance)
...
ExplicitAccessEntry& ExplicitAccessEntry::operator=(ExplicitAccessEntry&&) = default;
...
std::optional<AccessControlList> AccessControlList::FromMandatoryLabel(DWORD integrity_level, DWORD inheritance, DWORD mandatory_policy)
{
    ...
    DWORD length = sizeof(ACL) + sizeof(SYSTEM_MANDATORY_LABEL_ACE) + ::GetLengthSid(sid.GetPSID()) - sizeof(DWORD);
    ...
    if (!::AddMandatoryAce(sacl, ACL_REVISION, inheritance, mandatory_policy, sid.GetPSID())) {
        return std::nullopt;
    }
...
bool AccessControlList::SetEntries(const std::vector<ExplicitAccessEntry>& entries)
{
    if (entries.empty())
        return true;
...
bool AccessControlList::SetEntry(const Sid& sid, SecurityAccessMode mode, DWORD access_mask, DWORD inheritance)
{
...
AccessControlList::AccessControlList(const ACL* acl)
    : acl_(AclToBuffer(acl))
{
}
