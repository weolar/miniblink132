...
protected:
    DarkModeSupportTest()
    {
        ux_theme_lib_ = base::ScopedNativeLibrary(base::LoadSystemLibrary(L"uxtheme.dll", &error_));
    }
    ...
    void SetUp() override
    {
        ...
        ASSERT_FALSE(error_.code) << "Failed to load uxtheme.dll with error " << error_.ToString();
    }
...
TEST_F(DarkModeSupportTest, TestAllowDarkModeForWindowNoCrash)
{
    ...
    HWND hwnd = ::CreateWindow(
        kWndClassName, L"DarkModeTest", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr, instance, nullptr);
