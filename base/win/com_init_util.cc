...
ComApartmentType GetComApartmentTypeForThread()
{
    ...
    if ((ole_tls_data->apartment_flags & OleTlsData::ApartmentFlags::MTA) == OleTlsData::ApartmentFlags::MTA) {
...
void AssertComInitialized(const char* message)
{
    ...
    OleTlsData* ole_tls_data = GetOleTlsData();
    if (ole_tls_data && (ole_tls_data->apartment_flags & OleTlsData::ApartmentFlags::LOGICAL_THREAD_REGISTERED)) {
        return;
    }
