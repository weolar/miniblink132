...
enum class PreferredAppMode { kDefault, kAllowDark, kForceDark, kForceLight, kMax };
...
const DarkModeSupport& GetDarkModeSupport()
{
    static const DarkModeSupport dark_mode_support = [] {
        ...
        if (os_info->version() >= base::win::Version::WIN10_RS5 && base::win::IsUser32AndGdi32Available()) {
            ...
            if (os_info->version() >= base::win::Version::WIN10_19H1) {
                dark_mode_support.set_preferred_app_mode
                    = reinterpret_cast<UxThemeSetPreferredAppModeFunc>(::GetProcAddress(ux_theme_lib, MAKEINTRESOURCEA(kUxThemeSetPreferredAppModeOrdinal)));
            } else {
                dark_mode_support.allow_dark_mode_for_app
                    = reinterpret_cast<UxThemeAllowDarkModeForAppFunc>(::GetProcAddress(ux_theme_lib, MAKEINTRESOURCEA(kUxThemeAllowDarkModeForAppOrdinal)));
            }
            dark_mode_support.allow_dark_mode_for_window
                = reinterpret_cast<UxThemeAllowDarkModeForWindowFunc>(::GetProcAddress(ux_theme_lib, MAKEINTRESOURCEA(kUxThemeAllowDarkModeForWindowOrdinal)));
        }
...
bool IsDarkModeAvailable()
{
    ...
    return (dark_mode_support.allow_dark_mode_for_app || dark_mode_support.set_preferred_app_mode) && dark_mode_support.allow_dark_mode_for_window;
}
...
void AllowDarkModeForApp(bool allow)
{
    ...
    if (dark_mode_support.set_preferred_app_mode) {
        dark_mode_support.set_preferred_app_mode(allow ? PreferredAppMode::kAllowDark : PreferredAppMode::kDefault);
    } else {
