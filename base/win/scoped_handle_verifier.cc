...
NOINLINE void ReportErrorOnScopedHandleOperation(const debug::StackTrace& creation_stack, HandleOperation operation)
{
...
NOINLINE void ReportErrorOnScopedHandleOperation(const debug::StackTrace& creation_stack, const ScopedHandleVerifierInfo& other, HandleOperation operation)
{
...
ScopedHandleVerifierInfo::ScopedHandleVerifierInfo(
    const void* owner, const void* pc1, const void* pc2, std::unique_ptr<debug::StackTrace> stack, DWORD thread_id)
    : owner(owner)
    , pc1(pc1)
    , pc2(pc2)
...
ScopedHandleVerifierInfo& ScopedHandleVerifierInfo::operator=(ScopedHandleVerifierInfo&&) noexcept = default;
...
// static
void ScopedHandleVerifier::ThreadSafeAssignOrCreateScopedHandleVerifier(ScopedHandleVerifier* existing_verifier, bool enabled)
{
...
void ScopedHandleVerifier::InstallVerifier()
{
    ...
    GetHandleVerifierFn get_handle_verifier = reinterpret_cast<GetHandleVerifierFn>(::GetProcAddress(main_module, "GetHandleVerifier"));
    ...
    ScopedHandleVerifier* main_module_verifier = reinterpret_cast<ScopedHandleVerifier*>(get_handle_verifier());
...
void ScopedHandleVerifier::StartTracking(HANDLE handle, const void* owner, const void* pc1, const void* pc2)
{
    if (enabled_)
        StartTrackingImpl(handle, owner, pc1, pc2);
}

void ScopedHandleVerifier::StopTracking(HANDLE handle, const void* owner, const void* pc1, const void* pc2)
{
    if (enabled_)
...
void ScopedHandleVerifier::OnHandleBeingClosed(HANDLE handle, HandleOperation operation)
{
    if (enabled_)
...
NOINLINE void ScopedHandleVerifier::StartTrackingImpl(HANDLE handle, const void* owner, const void* pc1, const void* pc2)
{
    ...
    std::pair<HandleMap::iterator, bool> result = map_.emplace(handle, ScopedHandleVerifierInfo { owner, pc1, pc2, std::move(stacktrace), thread_id });
    if (!result.second) {
        // Attempt to start tracking already tracked handle.
        ReportErrorOnScopedHandleOperation(creation_stack_, result.first->second, HandleOperation::kHandleAlreadyTracked);
    }
}

NOINLINE void ScopedHandleVerifier::StopTrackingImpl(HANDLE handle, const void* owner, const void* pc1, const void* pc2)
{
    AutoNativeLock lock(*lock_);
    HandleMap::iterator i = map_.find(handle);
    if (i == map_.end()) {
        // Attempting to close an untracked handle.
        ReportErrorOnScopedHandleOperation(creation_stack_, HandleOperation::kCloseHandleNotTracked);
    }

    if (i->second.owner != owner) {
        // Attempting to close a handle not owned by opener.
        ReportErrorOnScopedHandleOperation(creation_stack_, i->second, HandleOperation::kCloseHandleNotOwner);
    }

    map_.erase(i);
}

NOINLINE void ScopedHandleVerifier::OnHandleBeingClosedImpl(HANDLE handle, HandleOperation operation)
{
    if (closing) {
        return;
    }
