...
private:
    ...
    void PostEventsDisabled() override
    {
        ::SetEvent(callback_event_.get());
    }
...
TEST_F(EtwTraceControllerTest, StartFileSession)
{
    ...
    EtwTraceController controller;
    HRESULT hr = controller.StartFileSession(session_name_.c_str(), temp.value().c_str());
    if (hr == E_ACCESSDENIED) {
        VLOG(1) << "You must be an administrator to run this test on Vista";
        DeleteFile(temp);
        return;
...
TEST_F(EtwTraceControllerTest, DISABLED_EnableDisable)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(controller.EnableProvider(test_provider_, TRACE_LEVEL_VERBOSE, kTestProviderFlags));
