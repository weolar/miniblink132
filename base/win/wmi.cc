...
std::optional<WmiError> CreateLocalWmiConnection(bool set_blanket, const std::wstring& server_name, ComPtr<IWbemServices>* wmi_services)
{
    DCHECK(wmi_services);
    ...
    hr = wmi_locator->ConnectServer(base::win::ScopedBstr(server_name).Get(), nullptr, nullptr, nullptr, 0, nullptr, nullptr, &wmi_services_r);
    if (FAILED(hr))
        return WmiError::kFailedToConnectToWMI;

    if (set_blanket) {
        hr = ::CoSetProxyBlanket(
            wmi_services_r.Get(), RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE, nullptr, EOAC_NONE);
        if (FAILED(hr))
...
bool TryRunQuery(const std::wstring& query, const ComPtr<IWbemServices>& wmi_services, ComPtr<IEnumWbemClassObject>* enumerator)
{
    DCHECK(enumerator);
    ...
    HRESULT hr = wmi_services->ExecQuery(query_language.Get(), query_bstr.Get(), WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr, &enumerator_r);
...
std::optional<WmiError> RunWmiQuery(const std::wstring& server_name, const std::wstring& query, ComPtr<IEnumWbemClassObject>* enumerator)
{
    ...
    auto error = CreateLocalWmiConnection(/*set_blanket=*/true, server_name, &wmi_services);
...
bool CreateLocalWmiConnection(bool set_blanket, ComPtr<IWbemServices>* wmi_services)
{
    ...
    auto error = CreateLocalWmiConnection(set_blanket, kCimV2ServerName, wmi_services);
    return !error.has_value();
}

ComPtr<IWbemServices> CreateWmiConnection(bool set_blanket, const std::wstring& resource)
{
    // Mitigate the issues caused by loading DLLs on a background thread
    // (http://crbug/973868).
    SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY();
...
bool CreateWmiClassMethodObject(
    IWbemServices* wmi_services, std::wstring_view class_name, std::wstring_view method_name, ComPtr<IWbemClassObject>* class_instance)
{
...
bool WmiLaunchProcess(const std::wstring& command_line, int* process_id)
{
    ...
    HRESULT hr = wmi_local->ExecMethod(ScopedBstr(class_name).Get(), ScopedBstr(method_name).Get(), 0, nullptr, process_create.Get(), &out_params, nullptr);
    if (FAILED(hr))
        return false;
...
WmiComputerSystemInfo WmiComputerSystemInfo::Get()
{
    static const base::NoDestructor<WmiComputerSystemInfo> static_info([] {
        ...
        auto error = RunWmiQuery(kCimV2ServerName, kSerialNumberQuery, &enumerator_bios);
        if (!error.has_value())
...
void WmiComputerSystemInfo::PopulateSerialNumber(const ComPtr<IEnumWbemClassObject>& enumerator_bios)
{
    ComPtr<IWbemClassObject> class_obj;
    ULONG items_returned = 0;
    HRESULT hr = enumerator_bios->Next(WBEM_INFINITE, 1, &class_obj, &items_returned);
    ...
    if (SUCCEEDED(hr) && serial_number.type() == VT_BSTR) {
        serial_number_.assign(V_BSTR(serial_number.ptr()), ::SysStringLen(V_BSTR(serial_number.ptr())));
    }
}
