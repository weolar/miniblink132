...
// be released.
void InitializeShortcutInterfaces(const wchar_t* shortcut, ComPtr<IShellLink>* i_shell_link, ComPtr<IPersistFile>* i_persist_file)
{
...
bool CreateOrUpdateShortcutLink(const FilePath& shortcut_path, const ShortcutProperties& properties, ShortcutOperation operation)
{
    ...
    if (operation == ShortcutOperation::kCreateAlways && !base::CreateDirectory(shortcut_path.DirName())) {
        DLOG(ERROR) << "CreateDirectory " << shortcut_path.DirName() << " failed";
        return false;
    }
    // A target is required unless |operation| is kUpdateExisting.
    if (operation != ShortcutOperation::kUpdateExisting && !(properties.options & ShortcutProperties::PROPERTIES_TARGET)) {
        NOTREACHED();
    }
    ...
    case ShortcutOperation::kUpdateExisting:
        InitializeShortcutInterfaces(shortcut_path.value().c_str(), &i_shell_link, &i_persist_file);
        break;
    case ShortcutOperation::kReplaceExisting:
        InitializeShortcutInterfaces(shortcut_path.value().c_str(), &old_i_shell_link, &old_i_persist_file);
    ...
    if ((properties.options & ShortcutProperties::PROPERTIES_ICON)
        && FAILED(i_shell_link->SetIconLocation(properties.icon.value().c_str(), properties.icon_index))) {
        return false;
    }
    ...
    if (has_app_id || has_dual_mode || has_toast_activator_clsid) {
        ...
        if (has_toast_activator_clsid
            && !SetClsidForPropertyStore(property_store.Get(), PKEY_AppUserModel_ToastActivatorCLSID, properties.toast_activator_clsid)) {
            return false;
        }
    }
    ...
    SHChangeNotify(shortcut_existed ? SHCNE_UPDATEITEM : SHCNE_CREATE, SHCNF_PATH | SHCNF_FLUSH, shortcut_path.value().c_str(), nullptr);

    return true;
}

bool ResolveShortcutProperties(const FilePath& shortcut_path, uint32_t options, ShortcutProperties* properties)
{
    ...
    if (FAILED(::CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&i_shell_link)))) {
        return false;
    }
    ...
    if (options & (ShortcutProperties::PROPERTIES_APP_ID | ShortcutProperties::PROPERTIES_DUAL_MODE | ShortcutProperties::PROPERTIES_TOAST_ACTIVATOR_CLSID)) {
        ComPtr<IPropertyStore> property_store;
        ...
        if (options & ShortcutProperties::PROPERTIES_TOAST_ACTIVATOR_CLSID) {
            ...
            if (property_store->GetValue(PKEY_AppUserModel_ToastActivatorCLSID, pv_toast_activator_clsid.Receive()) != S_OK) {
                return false;
            }
            ...
            case VT_CLSID:
                ...
                break;
            default:
                NOTREACHED() << "Unexpected variant type: " << pv_toast_activator_clsid.get().vt;
            }
        }
    }
...
bool ResolveShortcut(const FilePath& shortcut_path, FilePath* target_path, std::wstring* args)
{
    uint32_t options = 0;
    if (target_path)
