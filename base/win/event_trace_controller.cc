...
HRESULT EtwTraceController::Start(const wchar_t* session_name, EtwTraceProperties* prop)
{
...
HRESULT EtwTraceController::StartFileSession(const wchar_t* session_name, const wchar_t* logfile_path, bool realtime)
{
    DCHECK(NULL == session_ && session_name_.empty());
...
HRESULT EtwTraceController::StartRealtimeSession(const wchar_t* session_name, size_t buffer_size)
{
    ...
    p.BufferSize = checked_cast<ULONG>(buffer_size ? buffer_size : kDefaultRealtimeBufferSizeKb);
...
HRESULT EtwTraceController::Start(const wchar_t* session_name, EtwTraceProperties* properties, TRACEHANDLE* session_handle)
{
...
HRESULT EtwTraceController::Query(const wchar_t* session_name, EtwTraceProperties* properties)
{
    ULONG err = ::ControlTrace(NULL, session_name, properties->get(), EVENT_TRACE_CONTROL_QUERY);
    return HRESULT_FROM_WIN32(err);
}

HRESULT EtwTraceController::Update(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
...
HRESULT EtwTraceController::Stop(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
...
HRESULT EtwTraceController::Flush(const wchar_t* session_name, EtwTraceProperties* properties)
{
    DCHECK(properties != nullptr);
