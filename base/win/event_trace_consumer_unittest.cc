...
public:
    ...
    void ClearQueue()
    {
        for (EventQueue::const_iterator it(events_.begin()), end(events_.end()); it != end; ++it) {
...
{
    TestConsumer consumer_;
    ASSERT_HRESULT_SUCCEEDED(consumer_.OpenRealtimeSession(session_name_.c_str()));
}
...
public:
    ...
    HRESULT StartConsumerThread()
    {
        ...
        consumer_thread_.Set(::CreateThread(nullptr, 0, ConsumerThreadMainProc, this, 0, nullptr));
        if (consumer_thread_.get() == nullptr)
            return HRESULT_FROM_WIN32(::GetLastError());

        HANDLE events[] = { consumer_ready_.get(), consumer_thread_.get() };
        DWORD result = ::WaitForMultipleObjects(std::size(events), events, FALSE, INFINITE);
        switch (result) {
        case WAIT_OBJECT_0:
...
TEST_F(EtwTraceConsumerRealtimeTest, ConsumerReturnsWhenSessionClosed)
{
    ...
    if (controller.StartRealtimeSession(session_name_.c_str(), 1024) == E_ACCESSDENIED) {
    ...
    ASSERT_EQ(static_cast<DWORD>(WAIT_TIMEOUT), ::WaitForSingleObject(consumer_thread_.get(), 500));
...
DEFINE_GUID(kTestEventType, 0x57e47923, 0xa549, 0x476f, 0x86, 0xca, 0x50, 0x3d, 0x57, 0xf5, 0x9e, 0x62);
// clang-format on
...
TEST_F(EtwTraceConsumerRealtimeTest, ConsumeEvent)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(controller.EnableProvider(test_provider_, TRACE_LEVEL_VERBOSE, 0xFFFFFFFF));
    ...
    EXPECT_EQ(WAIT_OBJECT_0, ::WaitForSingleObject(TestConsumer::sank_event_.get(), INFINITE));
    ASSERT_HRESULT_SUCCEEDED(controller.Stop(nullptr));
    ASSERT_HRESULT_SUCCEEDED(JoinConsumerThread());
...
public:
    ...
    HRESULT LogEventToTempSession(PEVENT_TRACE_HEADER header)
    {
        ...
        HRESULT hr = controller.StartFileSession(session_name_.c_str(), temp_file_.value().c_str());
        if (FAILED(hr))
            return hr;

        // Enable our provider.
        EXPECT_HRESULT_SUCCEEDED(controller.EnableProvider(test_provider_, TRACE_LEVEL_VERBOSE, 0xFFFFFFFF));
