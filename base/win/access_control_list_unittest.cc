...
constexpr wchar_t kEventWithSystemInherit[] = L"(D;OICI;DC;;;SY)(A;;0x1f0003;;;WD)";
...
std::vector<char> ConvertSddlToAcl(const wchar_t* sddl)
{
    ...
    CHECK(::ConvertStringSecurityDescriptorToSecurityDescriptor(sddl_dacl.c_str(), SDDL_REVISION_1, &sd, nullptr));
...
std::wstring ConvertAclToSddl(const AccessControlList& acl, bool label = false)
{
    ...
    CHECK(::ConvertSecurityDescriptorToStringSecurityDescriptor(
        &sd, SDDL_REVISION_1, label ? LABEL_SECURITY_INFORMATION : DACL_SECURITY_INFORMATION, &sddl_str, nullptr));
...
TEST(AccessControlListTest, FromPACL)
{
    ...
    auto test_acl = AccessControlList::FromPACL(reinterpret_cast<ACL*>(compare_acl.data()));
...
TEST(AccessControlListTest, FromMandatoryLabel)
{
    ...
    acl = AccessControlList::FromMandatoryLabel(SECURITY_MANDATORY_SYSTEM_RID, 0, 0);
    ASSERT_TRUE(acl);
    EXPECT_EQ(ConvertAclToSddl(*acl, true), kSystemLabel);
    acl = AccessControlList::FromMandatoryLabel(SECURITY_MANDATORY_SYSTEM_RID, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE, 0);
    ASSERT_TRUE(acl);
    EXPECT_EQ(ConvertAclToSddl(*acl, true), kSystemLabelInherit);
    acl = AccessControlList::FromMandatoryLabel(
        SECURITY_MANDATORY_SYSTEM_RID, 0, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP | SYSTEM_MANDATORY_LABEL_NO_READ_UP | SYSTEM_MANDATORY_LABEL_NO_EXECUTE_UP);
    ASSERT_TRUE(acl);
    EXPECT_EQ(ConvertAclToSddl(*acl, true), kSystemLabelPolicy);
    acl = AccessControlList::FromMandatoryLabel(SECURITY_MANDATORY_SYSTEM_RID, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE,
...
TEST(AccessControlListTest, ExplicitAccessEntry)
{
    ...
    ExplicitAccessEntry ace(sid, SecurityAccessMode::kGrant, FakeAccess, FakeInherit);
    ...
    ExplicitAccessEntry ace_known(WellKnownSid::kSelf, SecurityAccessMode::kRevoke, ~FakeAccess, ~FakeInherit);
...
TEST(AccessControlListTest, SetEntries)
{
    ...
    ace_list.emplace_back(Sid(WellKnownSid::kLocalSystem), SecurityAccessMode::kDeny, EVENT_MODIFY_STATE, CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE);
    EXPECT_TRUE(acl.SetEntries(ace_list));
...
{
    AccessControlList acl;
    EXPECT_TRUE(acl.SetEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kGrant, READ_CONTROL, 0));
    EXPECT_EQ(ConvertAclToSddl(acl), kEventReadControl);
    EXPECT_TRUE(acl.SetEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kGrant, EVENT_MODIFY_STATE, 0));
    EXPECT_EQ(ConvertAclToSddl(acl), kEventReadControlModify);
    EXPECT_TRUE(acl.SetEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kSet, EVENT_ALL_ACCESS, 0));
    EXPECT_EQ(ConvertAclToSddl(acl), kEvent);
    EXPECT_TRUE(acl.SetEntry(Sid(WellKnownSid::kLocalSystem), SecurityAccessMode::kDeny, EVENT_MODIFY_STATE, CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE));
    EXPECT_EQ(ConvertAclToSddl(acl), kEventWithSystemInherit);
    EXPECT_TRUE(acl.SetEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kRevoke, EVENT_ALL_ACCESS, 0));
    EXPECT_EQ(ConvertAclToSddl(acl), kEventSystemOnlyInherit);
}
...
TEST(AccessControlListTest, SetEntriesError)
{
    ...
    while (ace_list.size() < 0x10000) {
        auto sid = Sid::FromSddlString(L"S-1-5-1234-" + NumberToWString(ace_list.size()));
        ASSERT_TRUE(sid);
