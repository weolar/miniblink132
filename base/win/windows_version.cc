...
constexpr wchar_t kRegKeyWindowsNTCurrentVersion[] = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion";
...
bool OSInfo::IsRunningEmulatedOnArm64()
{
    ...
    IsWow64Process2Function is_wow64_process2
        = reinterpret_cast<IsWow64Process2Function>(::GetProcAddress(::GetModuleHandleA("kernel32.dll"), "IsWow64Process2"));
    if (!is_wow64_process2) {
        return false;
    }
    USHORT process_machine;
    USHORT native_machine;
    bool retval = is_wow64_process2(::GetCurrentProcess(), &process_machine, &native_machine);
    if (!retval) {
        return false;
    }
...
OSInfo::OSInfo(const _OSVERSIONINFOEXW& version_info, const _SYSTEM_INFO& system_info, DWORD os_type)
    ...
    , os_type_(os_type)
{
    ...
    version_ = MajorMinorBuildToVersion(version_number_.major, version_number_.minor, version_number_.build);
...
Version OSInfo::Kernel32Version()
{
    static const Version kernel32_version
        = MajorMinorBuildToVersion(Kernel32BaseVersion().components()[0], Kernel32BaseVersion().components()[1], Kernel32BaseVersion().components()[2]);
...
OSInfo::VersionNumber OSInfo::Kernel32VersionNumber()
{
    ...
    static const VersionNumber version = { .major = Kernel32BaseVersion().components()[0],
...
base::Version OSInfo::Kernel32BaseVersion()
{
    static const NoDestructor<base::Version> version([] {
        ...
        std::unique_ptr<FileVersionInfoWin> file_version_info = FileVersionInfoWin::CreateFileVersionInfoWin(FilePath(FILE_PATH_LITERAL("kernel32.dll")));
        if (!file_version_info) {
            ...
            file_version_info = FileVersionInfoWin::CreateFileVersionInfoWin(FilePath(FILE_PATH_LITERAL("kernelbase.dll")));
        }
...
{
    return (wow_process_machine_ == WowProcessMachine::kX86 && wow_native_machine_ == WowNativeMachine::kAMD64);
}

bool OSInfo::IsWowX86OnARM64() const
{
    return (wow_process_machine_ == WowProcessMachine::kX86 && wow_native_machine_ == WowNativeMachine::kARM64);
}
...
bool OSInfo::IsWowAMD64OnARM64() const
{
    ...
    return (wow_process_machine_ == WowProcessMachine::kDisabled && wow_native_machine_ == WowNativeMachine::kARM64);
#else
    return false;
#endif
}
...
{
    return (wow_process_machine_ == WowProcessMachine::kX86 && wow_native_machine_ == WowNativeMachine::kOther);
}
...
// static
Version OSInfo::MajorMinorBuildToVersion(uint32_t major, uint32_t minor, uint32_t build)
{
    if (major == 11) {
    ...
    if (major > 6) {
        ...
        LOG(DFATAL) << "Unsupported version: " << major << "." << minor << "." << build;
...
OSInfo::WowProcessMachine OSInfo::GetWowProcessMachineArchitecture(const int process_machine)
{
...
OSInfo::WowNativeMachine OSInfo::GetWowNativeMachineArchitecture(const int native_machine)
{
...
{
    static const auto is_wow64_process2 = reinterpret_cast<decltype(&IsWow64Process2)>(::GetProcAddress(::GetModuleHandle(L"kernel32.dll"), "IsWow64Process2"));
    if (!is_wow64_process2) {
