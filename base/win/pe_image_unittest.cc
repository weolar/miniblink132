...
bool ImportsCallback(const PEImage& image, LPCSTR module, DWORD ordinal, LPCSTR name, DWORD hint, PIMAGE_THUNK_DATA iat, PVOID cookie)
{
...
// Just counts the number of invocations.
bool SectionsCallback(const PEImage& image, PIMAGE_SECTION_HEADER header, PVOID section_start, DWORD section_size, PVOID cookie)
{
    int* count = reinterpret_cast<int*>(cookie);
    (*count)++;
    return true;
...
// Just counts the number of invocations.
bool RelocsCallback(const PEImage& image, WORD type, PVOID address, PVOID cookie)
{
    int* count = reinterpret_cast<int*>(cookie);
    (*count)++;
    return true;
...
// Just counts the number of invocations.
bool ImportChunksCallback(const PEImage& image, LPCSTR module, PIMAGE_THUNK_DATA name_table, PIMAGE_THUNK_DATA iat, PVOID cookie)
{
    int* count = reinterpret_cast<int*>(cookie);
    (*count)++;
    return true;
...
// Just counts the number of invocations.
bool DelayImportChunksCallback(
...
// Just counts the number of invocations.
bool ExportsCallback(const PEImage& image, DWORD ordinal, DWORD hint, LPCSTR name, PVOID function, LPCSTR forward, PVOID cookie)
{
    int* count = reinterpret_cast<int*>(cookie);
    (*count)++;
    return true;
...
base::FilePath GetPEImageTestPath()
{
    ...
    pe_image_test_path = pe_image_test_path.Append(FILE_PATH_LITERAL("pe_image_test_arm64.dll"));
...
TEST(PEImageTest, EnumeratesPEWithTargetModule)
{
    ...
    count = 0;
    pe.EnumDelayImportChunks(DelayImportChunksCallback, &count, kTargetModuleDelay);
    EXPECT_EQ(kDelayDlls, count);
