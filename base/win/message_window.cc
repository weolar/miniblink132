...
public:
    ...
    {
        static base::NoDestructor<base::ThreadLocalOwnedPointer<MessageWindowMap>> instance;
        if (!instance->Get()) {
...
private:
    ...
    std::map<HWND, const raw_ref<base::win::MessageWindow>> map_ GUARDED_BY_CONTEXT(thread_checker_);
};
...
static LazyInstance<MessageWindow::WindowClass>::DestructorAtExit g_window_class = LAZY_INSTANCE_INITIALIZER;
...
bool MessageWindow::CreateNamed(MessageCallback message_callback, const std::wstring& window_name)
{
...
bool MessageWindow::DoCreate(MessageCallback message_callback, const wchar_t* window_name)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    window_ = CreateWindow(MAKEINTATOM(window_class.atom()), window_name, 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, window_class.instance(), this);
    if (!window_) {
