...
constexpr wchar_t kEventProtectedWithLabel[] = L"D:P(A;;0x1f0003;;;WD)S:(ML;;NW;;;ME)";
...
constexpr SECURITY_INFORMATION kDaclLabelSecurityInfo = DACL_SECURITY_INFORMATION | LABEL_SECURITY_INFORMATION;

base::win::ScopedLocalAllocTyped<void> ConvertSddlToSd(const wchar_t* sddl)
{
    PSECURITY_DESCRIPTOR sd = nullptr;
    CHECK(ConvertStringSecurityDescriptorToSecurityDescriptor(sddl, SDDL_REVISION_1, &sd, nullptr));
...
bool CreateFileWithSd(const FilePath& path, void* sd, bool directory)
{
    ...
    return ScopedHandle(::CreateFile(path.value().c_str(), GENERIC_ALL, 0, &security_attr, CREATE_ALWAYS, 0, nullptr)).is_valid();
}

bool CreateFileWithDacl(const FilePath& path, const wchar_t* sddl, bool directory)
{
...
base::win::ScopedHandle CreateEventWithDacl(const wchar_t* name, const wchar_t* sddl)
{
    auto sd_ptr = ConvertSddlToSd(sddl);
    CHECK(sd_ptr);
    SECURITY_ATTRIBUTES security_attr = {};
    ...
    return base::win::ScopedHandle(::CreateEvent(&security_attr, FALSE, FALSE, name));
}

base::win::ScopedHandle DuplicateHandle(const base::win::ScopedHandle& handle, DWORD access_mask)
{
    HANDLE dup_handle;
    CHECK(::DuplicateHandle(::GetCurrentProcess(), handle.get(), ::GetCurrentProcess(), &dup_handle, access_mask, FALSE, 0));
...
void AccessCheckError(const std::optional<AccessCheckResult>& result, DWORD expected)
{
    EXPECT_FALSE(result.has_value());
    EXPECT_EQ(::GetLastError(), expected);
}

void AccessCheckStatusError(const std::optional<AccessCheckResult>& result, DWORD expected)
{
...
void AccessCheckTest(const std::optional<AccessCheckResult>& result, ACCESS_MASK expected_access)
{
    ASSERT_TRUE(result.has_value());
...
template <typename T> void AccessCheckTest(T type, ACCESS_MASK generic_read, ACCESS_MASK generic_write, ACCESS_MASK generic_execute, ACCESS_MASK generic_all)
{
    std::optional<SecurityDescriptor> sd = SecurityDescriptor::FromSddl(kAccessCheckSd);
    ASSERT_TRUE(sd);
    ...
    AccessCheckTest(sd->AccessCheck(*token, GENERIC_EXECUTE, type), generic_execute);
    ...
    ASSERT_TRUE(sd->SetMandatoryLabel(SECURITY_MANDATORY_LOW_RID, 0, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP));
    ASSERT_TRUE(token->SetIntegrityLevel(SECURITY_MANDATORY_UNTRUSTED_RID));
    AccessCheckTest(sd->AccessCheck(*token, GENERIC_READ, type), generic_read);
    AccessCheckTest(sd->AccessCheck(*token, GENERIC_EXECUTE, type), generic_execute);
    AccessCheckStatusError(sd->AccessCheck(*token, GENERIC_WRITE, type), ERROR_ACCESS_DENIED);
    ASSERT_TRUE(sd->SetMandatoryLabel(SECURITY_MANDATORY_UNTRUSTED_RID, 0, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP));
    AccessCheckTest(sd->AccessCheck(*token, GENERIC_ALL, type), generic_all);
}

void AccessCheckTest(const GENERIC_MAPPING& type)
{
    AccessCheckTest(type, type.GenericRead, type.GenericWrite, type.GenericExecute, type.GenericAll);
}
...
TEST(SecurityDescriptorTest, FromPointer)
{
    ...
    EXPECT_TRUE(sd->dacl());
    EXPECT_FALSE(sd->dacl_protected());
    sd = SecurityDescriptor::FromPointer(ConvertSddlToSd(kProtectedDaclOnly).get());
    ASSERT_TRUE(sd);
    EXPECT_TRUE(sd->dacl());
    ...
    EXPECT_TRUE(sd->sacl());
    EXPECT_FALSE(sd->sacl_protected());
    sd = SecurityDescriptor::FromPointer(ConvertSddlToSd(kProtectedSaclOnly).get());
    ASSERT_TRUE(sd);
    EXPECT_TRUE(sd->sacl());
...
TEST(SecurityDescriptorTest, ToAbsolute)
{
    ...
    EXPECT_EQ(sd_abs.Control, SE_DACL_PRESENT | SE_DACL_PROTECTED | SE_SACL_PRESENT | SE_SACL_PROTECTED);
...
TEST(SecurityDescriptorTest, SetMandatoryLabel)
{
    ...
    sd.SetMandatoryLabel(SECURITY_MANDATORY_MEDIUM_RID, OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP);
    EXPECT_TRUE(sd.sacl());
...
TEST(SecurityDescriptorTest, SetDaclEntries)
{
    ...
    ace_list.emplace_back(Sid(WellKnownSid::kWorld), SecurityAccessMode::kGrant, EVENT_ALL_ACCESS, 0);
    EXPECT_TRUE(sd.SetDaclEntries(ace_list));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEvent);
    ace_list.emplace_back(Sid(WellKnownSid::kLocalSystem), SecurityAccessMode::kDeny, EVENT_MODIFY_STATE, 0);
    EXPECT_TRUE(sd.SetDaclEntries(ace_list));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventWithSystem);
    ace_list.emplace_back(Sid(WellKnownSid::kWorld), SecurityAccessMode::kRevoke, EVENT_MODIFY_STATE, 0);
    EXPECT_TRUE(sd.SetDaclEntries(ace_list));
...
{
    SecurityDescriptor sd;
    EXPECT_TRUE(sd.SetDaclEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kGrant, READ_CONTROL, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventReadControl);
    EXPECT_TRUE(sd.SetDaclEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kGrant, EVENT_MODIFY_STATE, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventReadControlModify);
    EXPECT_TRUE(sd.SetDaclEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kSet, EVENT_ALL_ACCESS, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEvent);
    EXPECT_TRUE(sd.SetDaclEntry(Sid(WellKnownSid::kLocalSystem), SecurityAccessMode::kDeny, EVENT_MODIFY_STATE, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventWithSystem);
    EXPECT_TRUE(sd.SetDaclEntry(Sid(WellKnownSid::kWorld), SecurityAccessMode::kRevoke, EVENT_ALL_ACCESS, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventSystemOnly);
    sd.clear_dacl();
    EXPECT_TRUE(sd.SetDaclEntry(WellKnownSid::kWorld, SecurityAccessMode::kGrant, READ_CONTROL, 0));
    EXPECT_EQ(sd.ToSddl(DACL_SECURITY_INFORMATION), kEventReadControl);
}
...
TEST(SecurityDescriptorTest, WriteToFile)
{
    ...
    EXPECT_TRUE(new_acl.SetEntry(Sid(WellKnownSid::kBuiltinUsers), SecurityAccessMode::kGrant, FILE_ALL_ACCESS, 0));
    ...
    auto label_acl = AccessControlList::FromMandatoryLabel(SECURITY_MANDATORY_MEDIUM_RID, 0, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP);
...
{
    std::wstring name = base::ASCIIToWide(base::UnguessableToken::Create().ToString());
    EXPECT_FALSE(SecurityDescriptor::FromName(name.c_str(), SecurityObjectType::kKernel, kAllSecurityInfo));
    base::win::ScopedHandle handle = CreateEventWithDacl(name.c_str(), kEvent);
    ASSERT_TRUE(handle.is_valid());
    auto curr_sd = SecurityDescriptor::FromName(name.c_str(), SecurityObjectType::kKernel, kAllSecurityInfo);
    ASSERT_TRUE(curr_sd);
    EXPECT_EQ(curr_sd->ToSddl(DACL_SECURITY_INFORMATION), kEvent);
    EXPECT_TRUE(SecurityDescriptor::FromName(L"MACHINE\\SOFTWARE", SecurityObjectType::kRegistry, kAllSecurityInfo));
    EXPECT_TRUE(SecurityDescriptor::FromName(L".", SecurityObjectType::kFile, kAllSecurityInfo));
    EXPECT_FALSE(SecurityDescriptor::FromName(L"WinSta0", SecurityObjectType::kWindowStation, kAllSecurityInfo));
    EXPECT_FALSE(SecurityDescriptor::FromName(L"Default", SecurityObjectType::kDesktop, kAllSecurityInfo));
}

TEST(SecurityDescriptorTest, WriteToName)
{
    std::wstring name = base::ASCIIToWide(base::UnguessableToken::Create().ToString());
    EXPECT_FALSE(SecurityDescriptor().WriteToName(name.c_str(), SecurityObjectType::kKernel, kAllSecurityInfo));
    base::win::ScopedHandle handle = CreateEventWithDacl(name.c_str(), kEvent);
    ASSERT_TRUE(handle.is_valid());
    auto curr_sd = SecurityDescriptor::FromName(name.c_str(), SecurityObjectType::kKernel, kAllSecurityInfo);
    ASSERT_TRUE(curr_sd);
    curr_sd->set_dacl_protected(true);
    curr_sd->SetMandatoryLabel(SECURITY_MANDATORY_MEDIUM_RID, 0, SYSTEM_MANDATORY_LABEL_NO_WRITE_UP);

    EXPECT_TRUE(curr_sd->WriteToName(name.c_str(), SecurityObjectType::kKernel, kDaclLabelSecurityInfo));

    curr_sd = SecurityDescriptor::FromName(name.c_str(), SecurityObjectType::kKernel, kAllSecurityInfo);
    ASSERT_TRUE(curr_sd);
...
{
    EXPECT_FALSE(SecurityDescriptor::FromHandle(nullptr, SecurityObjectType::kKernel, kAllSecurityInfo));
    auto handle = CreateEventWithDacl(nullptr, kEvent);
    ASSERT_TRUE(handle.is_valid());
    auto curr_sd = SecurityDescriptor::FromHandle(handle.get(), SecurityObjectType::kKernel, kAllSecurityInfo);
    ASSERT_TRUE(curr_sd);
    EXPECT_EQ(curr_sd->ToSddl(DACL_SECURITY_INFORMATION), kEvent);
    auto dup_handle = DuplicateHandle(handle, EVENT_MODIFY_STATE);
    EXPECT_FALSE(SecurityDescriptor::FromHandle(dup_handle.get(), SecurityObjectType::kKernel, kAllSecurityInfo));
    EXPECT_TRUE(SecurityDescriptor::FromHandle(::GetProcessWindowStation(), SecurityObjectType::kWindowStation, kAllSecurityInfo));
    EXPECT_TRUE(SecurityDescriptor::FromHandle(::GetThreadDesktop(::GetCurrentThreadId()), SecurityObjectType::kDesktop, kAllSecurityInfo));
}

TEST(SecurityDescriptorTest, WriteToHandle)
{
    EXPECT_FALSE(SecurityDescriptor().WriteToHandle(nullptr, SecurityObjectType::kKernel, kAllSecurityInfo));
    ...
    EXPECT_TRUE(curr_sd->WriteToHandle(handle.get(), SecurityObjectType::kKernel, kDaclLabelSecurityInfo));

    curr_sd = SecurityDescriptor::FromHandle(handle.get(), SecurityObjectType::kKernel, kAllSecurityInfo);
    ASSERT_TRUE(curr_sd);
    EXPECT_EQ(curr_sd->ToSddl(kDaclLabelSecurityInfo), kEventProtectedWithLabel);
}
...
TEST(SecurityDescriptorTest, AccessCheck)
{
    ...
    std::optional<AccessToken> token = AccessToken::FromCurrentProcess();
    ASSERT_TRUE(token);
    AccessCheckError(sd->AccessCheck(*token, 1, SecurityObjectType::kFile), ERROR_NO_IMPERSONATION_TOKEN);
    token = AccessToken::FromCurrentProcess(/*impersonation=*/true);
    ASSERT_TRUE(token);
    AccessCheckError(sd->AccessCheck(*token, 1, SecurityObjectType::kKernel), ERROR_INVALID_PARAMETER);
    sd->set_dacl(AccessControlList());
    AccessCheckStatusError(sd->AccessCheck(*token, 1, SecurityObjectType::kFile), ERROR_ACCESS_DENIED);
    sd->clear_owner();
    AccessCheckError(sd->AccessCheck(*token, 1, SecurityObjectType::kFile), ERROR_INVALID_SECURITY_DESCR);
    AccessCheckTest({ 1, 2, 4, 8 });
    AccessCheckTest(SecurityObjectType::kFile, FILE_GENERIC_READ, FILE_GENERIC_WRITE, FILE_GENERIC_EXECUTE, FILE_ALL_ACCESS);
    AccessCheckTest(SecurityObjectType::kRegistry, KEY_READ, KEY_WRITE, KEY_EXECUTE, KEY_ALL_ACCESS);
    AccessCheckTest(SecurityObjectType::kDesktop, STANDARD_RIGHTS_READ | DESKTOP_READOBJECTS | DESKTOP_ENUMERATE,
        STANDARD_RIGHTS_WRITE | DESKTOP_CREATEWINDOW | DESKTOP_CREATEMENU | DESKTOP_HOOKCONTROL | DESKTOP_JOURNALRECORD | DESKTOP_JOURNALPLAYBACK
            | DESKTOP_WRITEOBJECTS,
        STANDARD_RIGHTS_EXECUTE | DESKTOP_SWITCHDESKTOP,
        STANDARD_RIGHTS_REQUIRED | DESKTOP_CREATEMENU | DESKTOP_CREATEWINDOW | DESKTOP_ENUMERATE | DESKTOP_HOOKCONTROL | DESKTOP_JOURNALPLAYBACK
            | DESKTOP_JOURNALRECORD | DESKTOP_READOBJECTS | DESKTOP_SWITCHDESKTOP | DESKTOP_WRITEOBJECTS);
    AccessCheckTest(SecurityObjectType::kWindowStation,
