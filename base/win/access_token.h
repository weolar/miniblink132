...
enum class SecurityImpersonationLevel { kAnonymous, kIdentification, kImpersonation, kDelegation };
...
public:
    ...
    public:
        // Get the group SID.
        const Sid& GetSid() const LIFETIME_BOUND
        {
            return sid_;
        }
    ...
    static std::optional<AccessToken> FromToken(HANDLE token, ACCESS_MASK desired_access = 0);
    ...
    // |desired_access| specifies additional access for the token handle,
    // TOKEN_QUERY will always be requested.
    static std::optional<AccessToken> FromProcess(HANDLE process, bool impersonation = false, ACCESS_MASK desired_access = 0);
    ...
    // |desired_access| specifies additional access for the token handle,
    // TOKEN_QUERY will always be requested.
    static std::optional<AccessToken> FromCurrentProcess(bool impersonation = false, ACCESS_MASK desired_access = 0);
    ...
    // |desired_access| specifies additional access for the token handle,
    // TOKEN_QUERY will always be requested.
    static std::optional<AccessToken> FromThread(HANDLE thread, bool open_as_self = true, ACCESS_MASK desired_access = 0);
    ...
    // |desired_access| specifies additional access for the token handle,
    // TOKEN_QUERY will always be requested.
    static std::optional<AccessToken> FromCurrentThread(bool open_as_self = true, ACCESS_MASK desired_access = 0);
    ...
    // |desired_access| specifies additional access for the token handle,
    // TOKEN_QUERY will always be requested.
    static std::optional<AccessToken> FromEffective(ACCESS_MASK desired_access = 0);
    ...
    // duplicate the token.
    std::optional<AccessToken> DuplicatePrimary(ACCESS_MASK desired_access = 0) const;
    ...
    std::optional<AccessToken> DuplicateImpersonation(
        SecurityImpersonationLevel impersonation_level = SecurityImpersonationLevel::kImpersonation, ACCESS_MASK desired_access = 0) const;
    ...
    std::optional<AccessToken> CreateRestricted(DWORD flags, const std::vector<Sid>& sids_to_disable, const std::vector<std::wstring>& privileges_to_delete,
        const std::vector<Sid>& sids_to_restrict, ACCESS_MASK desired_access = 0) const;
    ...
    // |desired_access| specifies additional access for the token handle.
    // The token needs to be opened with TOKEN_DUPLICATE access.
    std::optional<AccessToken> CreateAppContainer(const Sid& appcontainer_sid, const std::vector<Sid>& capabilities, ACCESS_MASK desired_access = 0) const;
