...
{
    bool done = false;
    auto barrier_callback = base::BarrierCallback<int>(0, base::BindLambdaForTesting([&done](std::vector<int> results) {
...
{
    auto barrier_callback = base::BarrierCallback<int>(0, base::BindOnce([](std::vector<int>) {}));
...
{
    bool done = false;
    auto barrier_callback = base::BarrierCallback<int>(3, base::BindLambdaForTesting([&done](std::vector<int> results) {
...
{
    bool done = false;
    const auto barrier_callback = base::BarrierCallback<int>(3, base::BindLambdaForTesting([&done](std::vector<int> results) {
        EXPECT_THAT(results, testing::ElementsAre(1, 3, 2));
        done = true;
    }));
    EXPECT_FALSE(done);
...
template <typename... Args> class DestructionIndicator {
public:
    // Sets `*destructed` to true in destructor.
    explicit DestructionIndicator(bool* destructed)
...
TEST(BarrierCallbackTest, ReleasesDoneCallbackWhenDone)
{
    ...
    auto barrier_callback = base::BarrierCallback<bool>(
        1, base::BindOnce(&DestructionIndicator<std::vector<bool>>::DoNothing, std::make_unique<DestructionIndicator<std::vector<bool>>>(&done_destructed)));
...
TEST(BarrierCallbackTest, KeepingCallbackAliveUntilDone)
{
    ...
    barrier_callback = base::BarrierCallback<bool>(1, base::BindLambdaForTesting([&barrier_callback](std::vector<bool> results) {
...
TEST(BarrierCallbackTest, SupportsMoveonlyTypes)
{
    ...
    auto barrier_callback = base::BarrierCallback<MoveOnly>(1, base::BindOnce([](std::vector<MoveOnly>) {}));
    barrier_callback.Run(MoveOnly());

    auto barrier_callback2 = base::BarrierCallback<MoveOnly>(1, base::BindOnce([](const std::vector<MoveOnly>&) {}));
    barrier_callback2.Run(MoveOnly());
}

TEST(BarrierCallbackTest, SupportsConstRefResults)
{
    auto barrier_callback = base::BarrierCallback<int>(1, base::BindOnce([](const std::vector<int>&) {}));
...
TEST(BarrierCallbackTest, SupportsReferenceTypes)
{
    ...
    auto barrier_callback = base::BarrierCallback<const Referenceable&>(1, base::BindOnce([](std::vector<Referenceable>) {}));
    barrier_callback.Run(ref);

    auto barrier_callback2 = base::BarrierCallback<const Referenceable&>(1, base::BindOnce([](const std::vector<Referenceable>&) {}));
