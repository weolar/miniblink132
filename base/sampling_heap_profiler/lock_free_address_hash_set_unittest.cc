...
public:
    static bool IsSubset(const LockFreeAddressHashSet& superset, const LockFreeAddressHashSet& subset)
    {
        for (const std::atomic<LockFreeAddressHashSet::Node*>& bucket : subset.buckets_) {
            for (LockFreeAddressHashSet::Node* node = bucket.load(std::memory_order_acquire); node; node = node->next) {
    ...
    static bool Equals(const LockFreeAddressHashSet& set1, const LockFreeAddressHashSet& set2)
    {
    ...
    {
        size_t count = 0;
        LockFreeAddressHashSet::Node* node = set.buckets_[bucket].load(std::memory_order_acquire);
...
public:
    ...
    void Run() override
    {
        for (size_t value = 42; !cancel_->load(std::memory_order_acquire); ++value) {
