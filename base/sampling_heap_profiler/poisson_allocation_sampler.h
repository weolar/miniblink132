...
public:
    ...
    public:
        ...
        virtual void SampleAdded(void* address, size_t size, size_t total, base::allocator::dispatcher::AllocationSubsystem type, const char* context) = 0;
    ...
    public:
        ...
        ScopedSuppressRandomnessForTesting(const ScopedSuppressRandomnessForTesting&) = delete;
        ScopedSuppressRandomnessForTesting& operator=(const ScopedSuppressRandomnessForTesting&) = delete;
    ...
    public:
        ...
        // Move-only.
        ScopedMuteHookedSamplesForTesting(const ScopedMuteHookedSamplesForTesting&) = delete;
        ScopedMuteHookedSamplesForTesting& operator=(const ScopedMuteHookedSamplesForTesting&) = delete;
        ScopedMuteHookedSamplesForTesting(ScopedMuteHookedSamplesForTesting&&);
        ScopedMuteHookedSamplesForTesting& operator=(ScopedMuteHookedSamplesForTesting&&);
    ...
    ALWAYS_INLINE void OnAllocation(const base::allocator::dispatcher::AllocationNotificationData& allocation_data);
    ALWAYS_INLINE void OnFree(const base::allocator::dispatcher::FreeNotificationData& free_data);
    ...
    {
        return profiling_state_.load(std::memory_order_relaxed) & ProfilingStateFlag::kHookedSamplesMutedForTesting;
    }
...
ALWAYS_INLINE void PoissonAllocationSampler::OnAllocation(const base::allocator::dispatcher::AllocationNotificationData& allocation_data)
{
    ...
    const ProfilingStateFlagMask state = profiling_state_.load(std::memory_order_relaxed);
    ...
    // already skips them.)
    if ((state & ProfilingStateFlag::kHookedSamplesMutedForTesting) && type != base::allocator::dispatcher::AllocationSubsystem::kManualForTesting)
        [[unlikely]] {
        return;
    }
    ...
    DoRecordAllocation(state, allocation_data.address(), allocation_data.size(), type, allocation_data.type_name());
}

ALWAYS_INLINE void PoissonAllocationSampler::OnFree(const base::allocator::dispatcher::FreeNotificationData& free_data)
{
    // The allocation hooks may be installed before the sampler is started. Check
    // if its ever been started first to avoid extra work on the fast path,
