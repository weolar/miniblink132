...
ThreadLocalData* GetThreadLocalData()
{
    ...
    static base::NoDestructor<base::allocator::dispatcher::ThreadLocalStorage<ThreadLocalData>> thread_local_data("sampling_heap_profiler");
...
const char* GetAndLeakThreadName()
{
    ...
    snprintf(name, sizeof(name), "Thread %lu", static_cast<unsigned long>(base::PlatformThread::CurrentId()));
    return strdup(name);
}
...
[[maybe_unused]] StackUnwinder CheckForDefaultUnwindTables()
{
    ...
    return frame_count > 1 ? StackUnwinder::kDefault : StackUnwinder::kUnavailable;
}
...
{
    bool was_enabled = record_thread_names_.exchange(/*desired=*/true, std::memory_order_acq_rel);
    if (!was_enabled) {
...
span<const void*> SamplingHeapProfiler::CaptureStackTrace(span<const void*> frames)
{
...
void SamplingHeapProfiler::SampleAdded(void* address, size_t size, size_t total, AllocationSubsystem type, const char* context)
{
    ...
    uint32_t previous_last = last_sample_ordinal_.fetch_add(1, std::memory_order_acq_rel);
    Sample sample(size, total, previous_last + 1);
    sample.allocator = type;
    CaptureNativeStack(context, &sample);
    AutoLock lock(mutex_);
    if (PoissonAllocationSampler::AreHookedSamplesMuted() && type != AllocationSubsystem::kManualForTesting) [[unlikely]] {
...
void SamplingHeapProfiler::CaptureNativeStack(const char* context, Sample* sample)
{
    const void* stack[kMaxStackEntries];
...
std::vector<SamplingHeapProfiler::Sample> SamplingHeapProfiler::GetSamples(uint32_t profile_id)
{
...
PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting SamplingHeapProfiler::MuteHookedSamplesForTesting()
{
    // Only one ScopedMuteHookedSamplesForTesting can exist at a time.
    CHECK(!PoissonAllocationSampler::AreHookedSamplesMuted());
    PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting mute_hooked_samples;
