...
using ScopedSuppressRandomnessForTesting = PoissonAllocationSampler::ScopedSuppressRandomnessForTesting;
...
public:
    void SetUp() override
    {
        ...
        allocator::dispatcher::Dispatcher::GetInstance().InitializeForTesting(PoissonAllocationSampler::Get());
    }
...
public:
    explicit SamplesCollector(size_t watch_size)
        : watch_size_(watch_size)
    {
    }

    void SampleAdded(void* address, size_t size, size_t, AllocationSubsystem, const char*) override
    {
...
void CheckAllocationPattern(void (*allocate_callback)())
{
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        allocate_callback();
        std::vector<SamplingHeapProfiler::Sample> samples = profiler->GetSamples(id);
        profiler->Stop();
    ...
    for (auto sum : sums) {
        ...
        printf("%zu:\tmean: %zu\trelative error: %.2f%%\n", sum.first, actual, 100. * (actual - expected) / expected);
    }
}
...
TEST_F(SamplingHeapProfilerTest, MAYBE_MANUAL_SamplerMicroBenchmark)
{
    ...
    for (int i = 1; i <= kNumAllocations; ++i) {
        sampler->OnAllocation(
            AllocationNotificationData(reinterpret_cast<void*>(static_cast<intptr_t>(i)), allocation_size, nullptr, AllocationSubsystem::kAllocatorShim));
    }
    base::TimeTicks t1 = base::TimeTicks::Now();
    for (int i = 1; i <= kNumAllocations; ++i)
        sampler->OnFree(FreeNotificationData(reinterpret_cast<void*>(static_cast<intptr_t>(i)), AllocationSubsystem::kAllocatorShim));
    base::TimeTicks t2 = base::TimeTicks::Now();

    printf("alloc+free[ns]: %.3f   alloc[ns]: %.3f   free[ns]: %.3f   "
           "alloc+free[mln/s]: %.1f   total[ms]: %.1f\n",
        (t2 - t0).InNanoseconds() * 1. / kNumAllocations, (t1 - t0).InNanoseconds() * 1. / kNumAllocations, (t2 - t1).InNanoseconds() * 1. / kNumAllocations,
        kNumAllocations / (t2 - t0).InMicrosecondsF(), (t2 - t0).InMillisecondsF());
...
TEST_F(SamplingHeapProfilerTest, HookedAllocatorMuted)
{
...

    {
        ...
        sampler->OnAllocation(AllocationNotificationData(kAddress, 10000, nullptr, AllocationSubsystem::kManualForTesting));
        sampler->OnFree(FreeNotificationData(kAddress, AllocationSubsystem::kManualForTesting));
        sampler->RemoveSamplesObserver(&collector);
        EXPECT_TRUE(collector.sample_added);
        EXPECT_TRUE(collector.sample_removed);
    }
