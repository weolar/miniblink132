...
ThreadLocalData* GetThreadLocalData()
{
    ...
    static base::NoDestructor<base::allocator::dispatcher::ThreadLocalStorage<ThreadLocalData>> thread_local_data("poisson_allocation_sampler");
...
PoissonAllocationSampler::ScopedMuteThreadSamples::ScopedMuteThreadSamples()
{
    ...
    thread_local_data->accumulated_bytes_snapshot = thread_local_data->accumulated_bytes;
...
PoissonAllocationSampler::ScopedMuteThreadSamples::~ScopedMuteThreadSamples()
{
    ...
    thread_local_data->accumulated_bytes = thread_local_data->accumulated_bytes_snapshot;
}
...
PoissonAllocationSampler::ScopedSuppressRandomnessForTesting::ScopedSuppressRandomnessForTesting()
{
...
// static
bool PoissonAllocationSampler::ScopedSuppressRandomnessForTesting::IsSuppressed()
{
    return g_deterministic;
}

PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting::ScopedMuteHookedSamplesForTesting()
{
...
PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting::ScopedMuteHookedSamplesForTesting(ScopedMuteHookedSamplesForTesting&&) = default;

PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting& PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting::operator=(
    ScopedMuteHookedSamplesForTesting&&)
    = default;

// static
constinit std::atomic<PoissonAllocationSampler::ProfilingStateFlagMask> PoissonAllocationSampler::profiling_state_ { 0 };
...
void PoissonAllocationSampler::SetSamplingInterval(size_t sampling_interval_bytes)
{
...
void PoissonAllocationSampler::DoRecordAllocation(
    const ProfilingStateFlagMask state, void* address, size_t size, base::allocator::dispatcher::AllocationSubsystem type, const char* context)
{
    ThreadLocalData* const thread_local_data = GetThreadLocalData();
    ...
    for (base::PoissonAllocationSampler::SamplesObserver* observer : observers_copy) {
...
void PoissonAllocationSampler::SetProfilingStateFlag(ProfilingStateFlag flag)
{
    ...
    ProfilingStateFlagMask old_state = profiling_state_.fetch_or(flags, std::memory_order_relaxed);
    DCHECK(!(old_state & flag));
}

// static
void PoissonAllocationSampler::ResetProfilingStateFlag(ProfilingStateFlag flag)
{
    DCHECK_NE(flag, kWasStarted);
    ProfilingStateFlagMask old_state = profiling_state_.fetch_and(~flag, std::memory_order_relaxed);
    DCHECK(old_state & flag);
}
...
void PoissonAllocationSampler::AddSamplesObserver(SamplesObserver* observer)
{
    ...
    DCHECK(profiling_state_.load(std::memory_order_relaxed) & ProfilingStateFlag::kIsRunning);
}

void PoissonAllocationSampler::RemoveSamplesObserver(SamplesObserver* observer)
{
    // The following implementation (including ScopedMuteThreadSamples) will use
    // `thread_local`, which may cause a reentrancy issue.  So, temporarily
    // disable the sampling by having a ReentryGuard.
