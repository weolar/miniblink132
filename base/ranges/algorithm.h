...
template <typename Pred, typename Proj> constexpr auto ProjectedUnaryPredicate(Pred& pred, Proj& proj) noexcept
{
    return [&pred, &proj](auto&& arg) -> bool { return std::invoke(pred, std::invoke(proj, std::forward<decltype(arg)>(arg))); };
}
...
template <typename BinaryPred, typename ProjT, typename ProjU, typename T, typename U>
concept BinaryPredicateProjectorIsInvokable = requires(BinaryPred& predicate, ProjT& project_t, ProjU& project_u, T&& t, U&& u) {
    {
        std::invoke(predicate, std::invoke(project_t, std::forward<T>(t)), std::invoke(project_u, std::forward<U>(u)))
    } -> std::same_as<bool>;
};
...
//
// Ensures that the return type of `invoke(pred, ...)` is convertible to bool.
template <typename Pred, typename Proj1, typename Proj2, bool kPermute = false> class BinaryPredicateProjector {
public:
...
private:
    template <typename T, typename U> constexpr auto GetProjs() const
    {
        if constexpr (BinaryPredicateProjectorIsInvokable<Pred, Proj1, Proj2, T, U>) {
            return std::pair<Proj1&, Proj2&>(proj1_, proj2_);
        } else if constexpr (kPermute && BinaryPredicateProjectorIsInvokable<Pred, Proj2, Proj1, T, U>) {
            return std::pair<Proj2&, Proj1&>(proj2_, proj1_);
        } else if constexpr (kPermute && BinaryPredicateProjectorIsInvokable<Pred, Proj1, Proj1, T, U>) {
            return std::pair<Proj1&, Proj1&>(proj1_, proj1_);
        } else if constexpr (kPermute && BinaryPredicateProjectorIsInvokable<Pred, Proj2, Proj2, T, U>) {
            return std::pair<Proj2&, Proj2&>(proj2_, proj2_);
        }
    }

public:
    template <typename T, typename U> constexpr bool operator()(T&& lhs, U&& rhs) const
    {
        auto projs = GetProjs<T, U>();
        return std::invoke(pred_, std::invoke(projs.first, std::forward<T>(lhs)), std::invoke(projs.second, std::forward<U>(rhs)));
    }
...
// readable.
template <typename Pred, typename Proj1, typename Proj2> constexpr auto ProjectedBinaryPredicate(Pred& pred, Proj1& proj1, Proj2& proj2) noexcept
{
    return BinaryPredicateProjector<Pred, Proj1, Proj2>(pred, proj1, proj2);
}

template <typename Pred, typename Proj1, typename Proj2> constexpr auto PermutedProjectedBinaryPredicate(Pred& pred, Proj1& proj1, Proj2& proj2) noexcept
{
...
template <typename Range> using range_category_t = iterator_category_t<ranges::iterator_t<Range>>;
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr bool all_of(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
...
template <typename Range, typename Pred, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr bool all_of(Range&& range, Pred pred, Proj proj = {})
{
    return ranges::all_of(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr bool any_of(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
    for (; first != last; ++first) {
        if (std::invoke(pred, std::invoke(proj, *first))) {
            return true;
...
{
    return ranges::any_of(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr bool none_of(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
    for (; first != last; ++first) {
        if (std::invoke(pred, std::invoke(proj, *first))) {
            return false;
...
{
    return ranges::none_of(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename I, typename F> using for_each_result = std::ranges::in_fun_result<I, F>;
...
template <typename InputIterator, typename Fun, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto for_each(InputIterator first, InputIterator last, Fun f, Proj proj = {})
{
    for (; first != last; ++first)
...
template <typename Range, typename Fun, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto for_each(Range&& range, Fun f, Proj proj = {})
{
    return ranges::for_each(std::ranges::begin(range), std::ranges::end(range), std::move(f), std::move(proj));
}

// Reference: https://wg21.link/algorithm.syn#:~:text=for_each_n_result
template <typename I, typename F> using for_each_n_result = std::ranges::in_fun_result<I, F>;
...
template <typename InputIterator, typename Size, typename Fun, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
...
template <typename InputIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto find(InputIterator first, InputIterator last, const T& value, Proj proj = {})
{
    for (; first != last; ++first) {
...
template <typename Range, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto find(Range&& range, const T& value, Proj proj = {})
{
    return ranges::find(std::ranges::begin(range), std::ranges::end(range), value, std::move(proj));
}
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto find_if(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::find_if(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto find_if_not(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::find_if_not(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename Pred = ranges::equal_to, typename Proj1 = std::identity,
    typename Proj2 = std::identity, typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = std::indirect_result_t<Pred&, std::projected<ForwardIterator1, Proj1>, std::projected<ForwardIterator2, Proj2>>>
constexpr auto find_end(
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return std::find_end(first1, last1, first2, last2, internal::ProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr auto find_end(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::find_end(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
...
constexpr auto find_first_of(
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return std::find_first_of(first1, last1, first2, last2, internal::ProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr auto find_first_of(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::find_first_of(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
        std::move(proj1), std::move(proj2));
}
...
template <typename ForwardIterator, typename Pred = ranges::equal_to, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto adjacent_find(ForwardIterator first, ForwardIterator last, Pred pred = {}, Proj proj = {})
{
...
template <typename Range, typename Pred = ranges::equal_to, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto adjacent_find(Range&& range, Pred pred = {}, Proj proj = {})
{
    return ranges::adjacent_find(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto count(InputIterator first, InputIterator last, const T& value, Proj proj = {})
{
    ...
    return std::count_if(first, last, [&proj, &value](auto&& lhs) { return std::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
{
    return ranges::count(std::ranges::begin(range), std::ranges::end(range), value, std::move(proj));
}
...
template <typename InputIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>>
constexpr auto count_if(InputIterator first, InputIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::count_if(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
constexpr auto mismatch(
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return std::mismatch(first1, last1, first2, last2, internal::ProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr auto mismatch(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::mismatch(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
        std::move(proj1), std::move(proj2));
}
    ...
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    ...
    return std::equal(first1, last1, first2, last2, internal::ProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr bool equal(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::equal(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
        std::move(proj1), std::move(proj2));
}
    ...
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    ...
    return std::is_permutation(first1, last1, first2, last2, internal::PermutedProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr bool is_permutation(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::is_permutation(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
        std::move(proj1), std::move(proj2));
}
...
constexpr auto search(
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return std::search(first1, last1, first2, last2, internal::ProjectedBinaryPredicate(pred, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Pred&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr auto search(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::search(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(pred),
        std::move(proj1), std::move(proj2));
}
...
template <typename ForwardIterator, typename Size, typename T, typename Pred = ranges::equal_to, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto search_n(ForwardIterator first, ForwardIterator last, Size count, const T& value, Pred pred = {}, Proj proj = {})
{
    // The second arg is guaranteed to be `value`, so we'll simply apply the
    // std::identity projection.
    std::identity value_proj;
    return std::search_n(first, last, count, value, internal::ProjectedBinaryPredicate(pred, proj, value_proj));
}
...
template <typename Range, typename Size, typename T, typename Pred = ranges::equal_to, typename Proj = std::identity,
    typename = internal::range_category_t<Range>>
constexpr auto search_n(Range&& range, Size count, const T& value, Pred pred = {}, Proj proj = {})
{
    return ranges::search_n(std::ranges::begin(range), std::ranges::end(range), count, value, std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto copy(InputIterator first, InputIterator last, OutputIterator result)
{
...
template <typename Range, typename OutputIterator, typename = internal::range_category_t<Range>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto copy(Range&& range, OutputIterator result)
{
    return ranges::copy(std::ranges::begin(range), std::ranges::end(range), result);
}
...
template <typename InputIterator, typename Size, typename OutputIterator, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
...
template <typename InputIterator, typename OutputIterator, typename Pred, typename Proj = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto copy_if(InputIterator first, InputIterator last, OutputIterator result, Pred pred, Proj proj = {})
{
...
template <typename Range, typename OutputIterator, typename Pred, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto copy_if(Range&& range, OutputIterator result, Pred pred, Proj proj = {})
{
    return ranges::copy_if(std::ranges::begin(range), std::ranges::end(range), result, std::move(pred), std::move(proj));
}
...
template <typename BidirectionalIterator1, typename BidirectionalIterator2, typename = internal::iterator_category_t<BidirectionalIterator1>,
    typename = internal::iterator_category_t<BidirectionalIterator2>>
constexpr auto copy_backward(BidirectionalIterator1 first, BidirectionalIterator1 last, BidirectionalIterator2 result)
{
...
template <typename Range, typename BidirectionalIterator, typename = internal::range_category_t<Range>,
...
{
    return ranges::copy_backward(std::ranges::begin(range), std::ranges::end(range), result);
}
...
template <typename InputIterator, typename OutputIterator, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto move(InputIterator first, InputIterator last, OutputIterator result)
{
...
{
    return ranges::move(std::ranges::begin(range), std::ranges::end(range), result);
}
...
template <typename BidirectionalIterator1, typename BidirectionalIterator2, typename = internal::iterator_category_t<BidirectionalIterator1>,
    typename = internal::iterator_category_t<BidirectionalIterator2>>
constexpr auto move_backward(BidirectionalIterator1 first, BidirectionalIterator1 last, BidirectionalIterator2 result)
{
...
{
    return ranges::move_backward(std::ranges::begin(range), std::ranges::end(range), result);
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename = internal::iterator_category_t<ForwardIterator1>,
    typename = internal::iterator_category_t<ForwardIterator2>>
constexpr auto swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2)
{
...
template <typename Range1, typename Range2, typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>>
constexpr auto swap_ranges(Range1&& range1, Range2&& range2)
{
    return ranges::swap_ranges(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2));
}
...
template <typename InputIterator, typename OutputIterator, typename UnaryOperation, typename Proj = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator>,
    typename = std::indirect_result_t<UnaryOperation&, std::projected<InputIterator, Proj>>>
constexpr auto transform(InputIterator first1, InputIterator last1, OutputIterator result, UnaryOperation op, Proj proj = {})
{
    return std::transform(first1, last1, result, [&op, &proj](auto&& arg) { return std::invoke(op, std::invoke(proj, std::forward<decltype(arg)>(arg))); });
}
...
template <typename Range, typename OutputIterator, typename UnaryOperation, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>, typename = std::indirect_result_t<UnaryOperation&, std::projected<iterator_t<Range>, Proj>>>
constexpr auto transform(Range&& range, OutputIterator result, UnaryOperation op, Proj proj = {})
{
    return ranges::transform(std::ranges::begin(range), std::ranges::end(range), result, std::move(op), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename OutputIterator, typename BinaryOperation, typename Proj1 = std::identity,
    typename Proj2 = std::identity, typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = internal::iterator_category_t<OutputIterator>,
    typename = std::indirect_result_t<BinaryOperation&, std::projected<ForwardIterator1, Proj1>, std::projected<ForwardIterator2, Proj2>>>
constexpr auto transform(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, OutputIterator result,
    BinaryOperation binary_op, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // std::transform does not have a `last2` overload. Thus we need to adjust
    // `last1` to ensure to not read past `last2`.
    last1 = std::next(first1, std::min(std::distance(first1, last1), std::distance(first2, last2)));
    return std::transform(first1, last1, first2, result, [&binary_op, &proj1, &proj2](auto&& lhs, auto&& rhs) {
        return std::invoke(binary_op, std::invoke(proj1, std::forward<decltype(lhs)>(lhs)), std::invoke(proj2, std::forward<decltype(rhs)>(rhs)));
    });
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename BinaryOperation, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>, typename = internal::iterator_category_t<OutputIterator>,
    typename = std::indirect_result_t<BinaryOperation&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>>
constexpr auto transform(Range1&& range1, Range2&& range2, OutputIterator result, BinaryOperation binary_op, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::transform(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result,
...
template <typename ForwardIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto replace(ForwardIterator first, ForwardIterator last, const T& old_value, const T& new_value, Proj proj = {})
{
    // Note: In order to be able to apply `proj` to each element in [first, last)
    // we are dispatching to std::replace_if instead of std::replace.
    std::replace_if(
        first, last, [&proj, &old_value](auto&& lhs) { return std::invoke(proj, std::forward<decltype(lhs)>(lhs)) == old_value; }, new_value);
    return last;
}
...
template <typename Range, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto replace(Range&& range, const T& old_value, const T& new_value, Proj proj = {})
{
    return ranges::replace(std::ranges::begin(range), std::ranges::end(range), old_value, new_value, std::move(proj));
}
...
template <typename ForwardIterator, typename Predicate, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto replace_if(ForwardIterator first, ForwardIterator last, Predicate pred, const T& new_value, Proj proj = {})
{
...
template <typename Range, typename Predicate, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto replace_if(Range&& range, Predicate pred, const T& new_value, Proj proj = {})
{
    return ranges::replace_if(std::ranges::begin(range), std::ranges::end(range), std::move(pred), new_value, std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy(InputIterator first, InputIterator last, OutputIterator result, const T& old_value, const T& new_value, Proj proj = {})
{
    // Note: In order to be able to apply `proj` to each element in [first, last)
    // we are dispatching to std::replace_copy_if instead of std::replace_copy.
    std::replace_copy_if(
        first, last, result, [&proj, &old_value](auto&& lhs) { return std::invoke(proj, std::forward<decltype(lhs)>(lhs)) == old_value; }, new_value);
    return last;
}
...
template <typename Range, typename OutputIterator, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy(Range&& range, OutputIterator result, const T& old_value, const T& new_value, Proj proj = {})
{
    return ranges::replace_copy(std::ranges::begin(range), std::ranges::end(range), result, old_value, new_value, std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename Predicate, typename T, typename Proj = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy_if(InputIterator first, InputIterator last, OutputIterator result, Predicate pred, const T& new_value, Proj proj = {})
{
    return std::replace_copy_if(first, last, result, internal::ProjectedUnaryPredicate(pred, proj), new_value);
}
...
template <typename Range, typename OutputIterator, typename Predicate, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy_if(Range&& range, OutputIterator result, Predicate pred, const T& new_value, Proj proj = {})
{
    return ranges::replace_copy_if(std::ranges::begin(range), std::ranges::end(range), result, pred, new_value, std::move(proj));
}
...
template <typename OutputIterator, typename T, typename = internal::iterator_category_t<OutputIterator>>
...
template <typename Range, typename T, typename = internal::range_category_t<Range>> constexpr auto fill(Range&& range, const T& value)
{
    return ranges::fill(std::ranges::begin(range), std::ranges::end(range), value);
}
...
template <typename OutputIterator, typename Size, typename T, typename = internal::iterator_category_t<OutputIterator>>
...
template <typename OutputIterator, typename Generator, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto generate(OutputIterator first, OutputIterator last, Generator gen)
{
...
template <typename Range, typename Generator, typename = internal::range_category_t<Range>> constexpr auto generate(Range&& range, Generator gen)
{
    return ranges::generate(std::ranges::begin(range), std::ranges::end(range), std::move(gen));
}
...
template <typename OutputIterator, typename Size, typename Generator, typename = internal::iterator_category_t<OutputIterator>>
...
template <typename ForwardIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto remove(ForwardIterator first, ForwardIterator last, const T& value, Proj proj = {})
{
    // Note: In order to be able to apply `proj` to each element in [first, last)
    // we are dispatching to std::remove_if instead of std::remove.
    return std::remove_if(first, last, [&proj, &value](auto&& lhs) { return std::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
{
    return ranges::remove(std::ranges::begin(range), std::ranges::end(range), value, std::move(proj));
}
...
template <typename ForwardIterator, typename Predicate, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto remove_if(ForwardIterator first, ForwardIterator last, Predicate pred, Proj proj = {})
{
...
template <typename Range, typename Predicate, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto remove_if(Range&& range, Predicate pred, Proj proj = {})
{
    return ranges::remove_if(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename T, typename Proj = std::identity, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto remove_copy(InputIterator first, InputIterator last, OutputIterator result, const T& value, Proj proj = {})
{
    // Note: In order to be able to apply `proj` to each element in [first, last)
    // we are dispatching to std::remove_copy_if instead of std::remove_copy.
    return std::remove_copy_if(first, last, result, [&proj, &value](auto&& lhs) { return std::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
template <typename Range, typename OutputIterator, typename T, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto remove_copy(Range&& range, OutputIterator result, const T& value, Proj proj = {})
{
    return ranges::remove_copy(std::ranges::begin(range), std::ranges::end(range), result, value, std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename Pred, typename Proj = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto remove_copy_if(InputIterator first, InputIterator last, OutputIterator result, Pred pred, Proj proj = {})
{
...
template <typename Range, typename OutputIterator, typename Pred, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto remove_copy_if(Range&& range, OutputIterator result, Pred pred, Proj proj = {})
{
    return ranges::remove_copy_if(std::ranges::begin(range), std::ranges::end(range), result, std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator, typename Comp = ranges::equal_to, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto unique(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename Comp = ranges::equal_to, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range>, Proj>, std::projected<iterator_t<Range>, Proj>>>
constexpr auto unique(Range&& range, Comp comp = {}, Proj proj = {})
{
    return ranges::unique(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename OutputIterator, typename Comp = ranges::equal_to, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto unique_copy(ForwardIterator first, ForwardIterator last, OutputIterator result, Comp comp = {}, Proj proj = {})
{
    return std::unique_copy(first, last, result, internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
template <typename Range, typename OutputIterator, typename Comp = ranges::equal_to, typename Proj = std::identity,
    typename = internal::range_category_t<Range>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto unique_copy(Range&& range, OutputIterator result, Comp comp = {}, Proj proj = {})
{
    return ranges::unique_copy(std::ranges::begin(range), std::ranges::end(range), result, std::move(comp), std::move(proj));
}
...
template <typename BidirectionalIterator, typename = internal::iterator_category_t<BidirectionalIterator>>
constexpr auto reverse(BidirectionalIterator first, BidirectionalIterator last)
{
...
template <typename Range, typename = internal::range_category_t<Range>> constexpr auto reverse(Range&& range)
{
...
template <typename BidirectionalIterator, typename OutputIterator, typename = internal::iterator_category_t<BidirectionalIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto reverse_copy(BidirectionalIterator first, BidirectionalIterator last, OutputIterator result)
{
...
{
    return ranges::reverse_copy(std::ranges::begin(range), std::ranges::end(range), result);
}
...
constexpr auto rotate(ForwardIterator first, ForwardIterator middle, ForwardIterator last)
{
...
template <typename Range, typename = internal::range_category_t<Range>> constexpr auto rotate(Range&& range, iterator_t<Range> middle)
{
    return ranges::rotate(std::ranges::begin(range), middle, std::ranges::end(range));
}
...
template <typename ForwardIterator, typename OutputIterator, typename = internal::iterator_category_t<ForwardIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto rotate_copy(ForwardIterator first, ForwardIterator middle, ForwardIterator last, OutputIterator result)
{
...
template <typename Range, typename OutputIterator, typename = internal::range_category_t<Range>, typename = internal::iterator_category_t<OutputIterator>>
constexpr auto rotate_copy(Range&& range, iterator_t<Range> middle, OutputIterator result)
{
    return ranges::rotate_copy(std::ranges::begin(range), middle, std::ranges::end(range), result);
}
...
template <typename RandomAccessIterator, typename UniformRandomBitGenerator, typename = internal::iterator_category_t<RandomAccessIterator>>
constexpr auto shuffle(RandomAccessIterator first, RandomAccessIterator last, UniformRandomBitGenerator&& g)
{
...
template <typename Range, typename UniformRandomBitGenerator, typename = internal::range_category_t<Range>>
constexpr auto shuffle(Range&& range, UniformRandomBitGenerator&& g)
{
    return ranges::shuffle(std::ranges::begin(range), std::ranges::end(range), std::forward<UniformRandomBitGenerator>(g));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto sort(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range>, Proj>, std::projected<iterator_t<Range>, Proj>>>
constexpr auto sort(Range&& range, Comp comp = {}, Proj proj = {})
{
    return ranges::sort(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto stable_sort(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::stable_sort(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range>, Proj>, std::projected<iterator_t<Range>, Proj>>>
constexpr auto partial_sort(Range&& range, iterator_t<Range> middle, Comp comp = {}, Proj proj = {})
{
    return ranges::partial_sort(std::ranges::begin(range), middle, std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename InputIterator, typename RandomAccessIterator, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator, Proj1>, std::projected<RandomAccessIterator, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj2>, std::projected<InputIterator, Proj1>>>
constexpr auto partial_sort_copy(InputIterator first, InputIterator last, RandomAccessIterator result_first, RandomAccessIterator result_last, Comp comp = {},
    Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // Needs to opt-in to all permutations, since std::partial_sort_copy expects
    // comp(proj2(lhs), proj1(rhs)) to compile.
    return std::partial_sort_copy(first, last, result_first, result_last, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto partial_sort_copy(Range1&& range, Range2&& result_range, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::partial_sort_copy(std::ranges::begin(range), std::ranges::end(range), std::ranges::begin(result_range), std::ranges::end(result_range),
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto is_sorted_until(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
    // Implementation inspired by cppreference.com:
...
{
    return ranges::is_sorted_until(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto is_sorted(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::is_sorted(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range>, Proj>, std::projected<iterator_t<Range>, Proj>>>
constexpr auto nth_element(Range&& range, iterator_t<Range> nth, Comp comp = {}, Proj proj = {})
{
    return ranges::nth_element(std::ranges::begin(range), nth, std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename T, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto lower_bound(ForwardIterator first, ForwardIterator last, const T& value, Comp comp = {}, Proj proj = {})
{
    // The second arg is guaranteed to be `value`, so we'll simply apply the
    // std::identity projection.
    std::identity value_proj;
    return std::lower_bound(first, last, value, internal::ProjectedBinaryPredicate(comp, proj, value_proj));
}
...
template <typename Range, typename T, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto lower_bound(Range&& range, const T& value, Comp comp = {}, Proj proj = {})
{
    return ranges::lower_bound(std::ranges::begin(range), std::ranges::end(range), value, std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename T, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto upper_bound(ForwardIterator first, ForwardIterator last, const T& value, Comp comp = {}, Proj proj = {})
{
    // The first arg is guaranteed to be `value`, so we'll simply apply the
    // std::identity projection.
    std::identity value_proj;
    return std::upper_bound(first, last, value, internal::ProjectedBinaryPredicate(comp, value_proj, proj));
}
...
template <typename Range, typename T, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto upper_bound(Range&& range, const T& value, Comp comp = {}, Proj proj = {})
{
    return ranges::upper_bound(std::ranges::begin(range), std::ranges::end(range), value, std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename T, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto equal_range(ForwardIterator first, ForwardIterator last, const T& value, Comp comp = {}, Proj proj = {})
{
    ...
    // unintended behavior.
    return std::make_pair(ranges::lower_bound(first, last, value, comp, proj), ranges::upper_bound(first, last, value, comp, proj));
}
...
template <typename Range, typename T, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto equal_range(Range&& range, const T& value, Comp comp = {}, Proj proj = {})
{
    return ranges::equal_range(std::ranges::begin(range), std::ranges::end(range), value, std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename T, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto binary_search(ForwardIterator first, ForwardIterator last, const T& value, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename T, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto binary_search(Range&& range, const T& value, Comp comp = {}, Proj proj = {})
{
    return ranges::binary_search(std::ranges::begin(range), std::ranges::end(range), value, std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto is_partitioned(ForwardIterator first, ForwardIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::is_partitioned(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto partition(ForwardIterator first, ForwardIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::partition(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename BidirectionalIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<BidirectionalIterator>>
constexpr auto stable_partition(BidirectionalIterator first, BidirectionalIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::stable_partition(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator1, typename OutputIterator2, typename Pred, typename Proj = std::identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator1>,
    typename = internal::iterator_category_t<OutputIterator2>>
constexpr auto partition_copy(InputIterator first, InputIterator last, OutputIterator1 out_true, OutputIterator2 out_false, Pred pred, Proj proj = {})
{
    return std::partition_copy(first, last, out_true, out_false, internal::ProjectedUnaryPredicate(pred, proj));
}
...
template <typename Range, typename OutputIterator1, typename OutputIterator2, typename Pred, typename Proj = std::identity,
    typename = internal::range_category_t<Range>, typename = internal::iterator_category_t<OutputIterator1>,
    typename = internal::iterator_category_t<OutputIterator2>>
constexpr auto partition_copy(Range&& range, OutputIterator1 out_true, OutputIterator2 out_false, Pred pred, Proj proj = {})
{
    return ranges::partition_copy(std::ranges::begin(range), std::ranges::end(range), out_true, out_false, std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator, typename Pred, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto partition_point(ForwardIterator first, ForwardIterator last, Pred pred, Proj proj = {})
{
...
{
    return ranges::partition_point(std::ranges::begin(range), std::ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    typename Proj2 = std::identity, typename = internal::iterator_category_t<InputIterator1>, typename = internal::iterator_category_t<InputIterator2>,
    typename = internal::iterator_category_t<OutputIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto merge(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result, Comp comp = {},
    Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // Needs to opt-in to all permutations, since std::merge expects
    // comp(proj2(lhs), proj1(rhs)) to compile.
    return std::merge(first1, last1, first2, last2, result, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    typename Proj2 = std::identity, typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = internal::iterator_category_t<OutputIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto merge(Range1&& range1, Range2&& range2, OutputIterator result, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::merge(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result, std::move(comp),
        std::move(proj1), std::move(proj2));
}
...
template <typename BidirectionalIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<BidirectionalIterator>>
constexpr auto inplace_merge(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last, Comp comp = {}, Proj proj = {})
{
...
template <typename Range, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::range_category_t<Range>>
constexpr auto inplace_merge(Range&& range, iterator_t<Range> middle, Comp comp = {}, Proj proj = {})
{
    return ranges::inplace_merge(std::ranges::begin(range), middle, std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename InputIterator1, typename InputIterator2, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::iterator_category_t<InputIterator1>, typename = internal::iterator_category_t<InputIterator2>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto includes(
    InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    ...
    return std::includes(first1, last1, first2, last2, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto includes(Range1&& range1, Range2&& range2, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::includes(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), std::move(comp),
        std::move(proj1), std::move(proj2));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto set_union(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result, Comp comp = {},
    Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // Needs to opt-in to all permutations, since std::set_union expects
    // comp(proj1(lhs), proj2(rhs)) and comp(proj2(lhs), proj1(rhs)) to compile.
    return std::set_union(first1, last1, first2, last2, result, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto set_union(Range1&& range1, Range2&& range2, OutputIterator result, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::set_union(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result,
        std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto set_intersection(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result, Comp comp = {},
    Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // Needs to opt-in to all permutations, since std::set_intersection expects
    // comp(proj1(lhs), proj2(rhs)) and comp(proj2(lhs), proj1(rhs)) to compile.
    return std::set_intersection(first1, last1, first2, last2, result, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto set_intersection(Range1&& range1, Range2&& range2, OutputIterator result, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::set_intersection(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result,
        std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto set_difference(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result, Comp comp = {},
    Proj1 proj1 = {}, Proj2 proj2 = {})
{
    // Needs to opt-in to all permutations, since std::set_difference expects
    // comp(proj1(lhs), proj2(rhs)) and comp(proj2(lhs), proj1(rhs)) to compile.
    return std::set_difference(first1, last1, first2, last2, result, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto set_difference(Range1&& range1, Range2&& range2, OutputIterator result, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::set_difference(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result,
        std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator1, Proj1>, std::projected<InputIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<InputIterator2, Proj2>, std::projected<InputIterator1, Proj1>>>
constexpr auto set_symmetric_difference(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result,
    Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    ...
    // compile.
    return std::set_symmetric_difference(first1, last1, first2, last2, result, internal::PermutedProjectedBinaryPredicate(comp, proj1, proj2));
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr auto set_symmetric_difference(Range1&& range1, Range2&& range2, OutputIterator result, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::set_symmetric_difference(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2), result,
        std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto push_heap(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::push_heap(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto pop_heap(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::pop_heap(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto make_heap(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::make_heap(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto sort_heap(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::sort_heap(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto is_heap(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::is_heap(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<RandomAccessIterator, Proj>, std::projected<RandomAccessIterator, Proj>>>
constexpr auto is_heap_until(RandomAccessIterator first, RandomAccessIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::is_heap_until(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = std::identity>
...
template <typename T, typename Comp = ranges::less, typename Proj = std::identity>
constexpr T min(std::initializer_list<T> ilist, Comp comp = {}, Proj proj = {})
{
    return *std::min_element(ilist.begin(), ilist.end(), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
{
    return *std::min_element(std::ranges::begin(range), std::ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = std::identity>
constexpr T max(std::initializer_list<T> ilist, Comp comp = {}, Proj proj = {})
{
    return *std::max_element(ilist.begin(), ilist.end(), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
{
    return *std::max_element(std::ranges::begin(range), std::ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = std::identity> constexpr auto minmax(const T& a, const T& b, Comp comp = {}, Proj proj = {})
{
...
template <typename T, typename Comp = ranges::less, typename Proj = std::identity>
constexpr auto minmax(std::initializer_list<T> ilist, Comp comp = {}, Proj proj = {})
{
    auto it = std::minmax_element(std::ranges::begin(ilist), std::ranges::end(ilist), internal::ProjectedBinaryPredicate(comp, proj, proj));
...
constexpr auto minmax(Range&& range, Comp comp = {}, Proj proj = {})
{
    ...
    auto it = std::minmax_element(std::ranges::begin(range), std::ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
    return std::pair<T, T> { *it.first, *it.second };
}
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto min_element(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::min_element(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto max_element(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::max_element(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = std::identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator, Proj>, std::projected<ForwardIterator, Proj>>>
constexpr auto minmax_element(ForwardIterator first, ForwardIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::minmax_element(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator1, Proj1>, std::projected<ForwardIterator2, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<ForwardIterator2, Proj2>, std::projected<ForwardIterator1, Proj1>>>
constexpr bool lexicographical_compare(
    ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    for (; first1 != last1 && first2 != last2; ++first1, ++first2) {
...
template <typename Range1, typename Range2, typename Comp = ranges::less, typename Proj1 = std::identity, typename Proj2 = std::identity,
    ...
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range1>, Proj1>, std::projected<iterator_t<Range2>, Proj2>>,
    typename = std::indirect_result_t<Comp&, std::projected<iterator_t<Range2>, Proj2>, std::projected<iterator_t<Range1>, Proj1>>>
constexpr bool lexicographical_compare(Range1&& range1, Range2&& range2, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::lexicographical_compare(std::ranges::begin(range1), std::ranges::end(range1), std::ranges::begin(range2), std::ranges::end(range2),
        std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename BidirectionalIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<BidirectionalIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<BidirectionalIterator, Proj>, std::projected<BidirectionalIterator, Proj>>>
constexpr auto next_permutation(BidirectionalIterator first, BidirectionalIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::next_permutation(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename BidirectionalIterator, typename Comp = ranges::less, typename Proj = std::identity,
    typename = internal::iterator_category_t<BidirectionalIterator>,
    typename = std::indirect_result_t<Comp&, std::projected<BidirectionalIterator, Proj>, std::projected<BidirectionalIterator, Proj>>>
constexpr auto prev_permutation(BidirectionalIterator first, BidirectionalIterator last, Comp comp = {}, Proj proj = {})
{
...
{
    return ranges::prev_permutation(std::ranges::begin(range), std::ranges::end(range), std::move(comp), std::move(proj));
}
