...
bool CreatePairImpl(ScopedHandle* socket_a, ScopedHandle* socket_b, bool overlapped)
{
    ...
    do {
        ...
        swprintf(name, kPipePathMax, kPipeNameFormat, GetCurrentProcessId(), GetCurrentThreadId(), rnd_name);

        handle_a.Set(CreateNamedPipeW(name, flags, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE, 1, kOutBufferSize, kInBufferSize, kDefaultTimeoutMilliSeconds, NULL));
...
size_t CancelableFileOperation(Function operation, HANDLE file, span<DataType> buffer, WaitableEvent* io_event, WaitableEvent* cancel_event,
    CancelableSyncSocket* socket, DWORD timeout_in_ms)
{
    ...
    do {
        ...
        // `chunk_size`.
        const BOOL operation_ok = operation(file, operation_buffer.data(), static_cast<DWORD>(operation_buffer.size()), &len, &ol);
        if (!operation_ok) {
            if (::GetLastError() == ERROR_IO_PENDING) {
                HANDLE events[] = { io_event->handle(), cancel_event->handle() };
                const DWORD wait_result = WaitForMultipleObjects(std::size(events), events, FALSE,
    ...
    } while (count < buffer.size() && (timeout_in_ms == INFINITE || current_time < finish_time));
...
size_t SyncSocket::Send(span<const uint8_t> data)
{
    ...
    while (count < data.size()) {
        ...
        if (::WriteFile(handle(), data_chunk.data(), static_cast<DWORD>(data_chunk.size()), &len, NULL) == FALSE) {
            return count;
        }
        count += len;
    }
...
size_t SyncSocket::Receive(span<uint8_t> buffer)
{
    ...
    while (count < buffer.size()) {
        ...
        // SAFETY: The below static_cast is in range for DWORD because `data_chunk`
        // is constructed with a DWORD length above from `chunk_size`.
        if (::ReadFile(handle(), data_chunk.data(), static_cast<DWORD>(data_chunk.size()), &len, NULL) == FALSE) {
            return count;
        }
        count += len;
    }
...
size_t CancelableSyncSocket::Send(span<const uint8_t> data)
{
    ...
    return CancelableFileOperation(&::WriteFile, handle(), data, &file_operation_, &shutdown_event_, this, kWaitTimeOutInMs);
}

size_t CancelableSyncSocket::Receive(span<uint8_t> buffer)
{
    return CancelableFileOperation(&::ReadFile, handle(), buffer, &file_operation_, &shutdown_event_, this, INFINITE);
}

size_t CancelableSyncSocket::ReceiveWithTimeout(span<uint8_t> buffer, TimeDelta timeout)
{
    return CancelableFileOperation(&::ReadFile, handle(), buffer, &file_operation_, &shutdown_event_, this, static_cast<DWORD>(timeout.InMilliseconds()));
}

// static
bool CancelableSyncSocket::CreatePair(CancelableSyncSocket* socket_a, CancelableSyncSocket* socket_b)
{
