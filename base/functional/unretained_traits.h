...
template <typename T>
concept SafeIncompleteTypeForUnretained = SameAsAny<std::remove_cvref_t<T>,
...
template <typename T> inline constexpr bool kCustomizeSupportsUnretained = true;
...
template <typename T> struct SupportsUnretainedImpl {
    // For context on this "templated struct with a lambda that asserts" pattern,
    // see comments in `Invoker<>`.
    template <bool v = IsComplete<T> || SafeIncompleteTypeForUnretained<T>> struct AllowlistIncompleteTypes {
...
// official builds, and then in non-test code as well.
    ...
    || (!defined(UNIT_TEST) && !defined(OFFICIAL_BUILD) && (BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_WIN)))
            static_assert(v,
    ...
    template <bool v = !DisallowsUnretained<T>> struct AllowsUnretained {
        static constexpr bool value = [] {
            static_assert(v,
    ...
    static constexpr bool value = std::conjunction_v<AllowlistIncompleteTypes<>, AllowsUnretained<>>;
};
