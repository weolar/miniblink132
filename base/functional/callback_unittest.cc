...
struct FakeBindState : internal::BindStateBase {
    FakeBindState()
        : BindStateBase(&NopInvokeFunc, &Destroy)
    {
    }
...
public:
    CallbackTest()
        : callback_a_(new FakeBindState())
        , callback_b_(new FakeBindState())
    {
    }
...
{
    RepeatingCallback<void(void*)> cb = BindRepeating([](void* param) { EXPECT_TRUE(static_cast<RepeatingCallback<void(void*)>*>(param)->is_null()); });
    ASSERT_TRUE(cb);
    std::move(cb).Run(&cb);
    EXPECT_FALSE(cb);
    ...
    OnceCallback<void(void*)> cb3 = BindOnce([](void* param) { EXPECT_TRUE(static_cast<OnceCallback<void(void*)>*>(param)->is_null()); });
    ASSERT_TRUE(cb3);
...
{
    {
        ...
        RepeatingCallback<int(int)> repeating_callback = base::BindRepeating([](int i) { return i + 1; });
        OnceCallback<int(int)> once_callback = base::BindOnce([](int i) { return i * 2; });
    ...
    }
    {
        ...
        RepeatingCallback<int(int)> repeating_callback = base::BindRepeating([](int i) { return i + 1; });
        OnceCallback<int(int)> once_callback = base::BindOnce([](int i) { return i * 2; });
        EXPECT_EQ(3, std::move(once_callback).Then(std::move(repeating_callback)).Run(1));
    }
}
...
{
    {
        OnceCallback<std::string()> once_callback = BindOnce([] { return std::string("hello"); });
        EXPECT_EQ("hello", std::move(once_callback).Then(BindOnce([](const std::string& s) { return s; })).Run());
    }
    ...
    {
        OnceCallback<NotCopied()> once_callback = BindOnce([] { return NotCopied(); });
...
template <bool use_once, typename R, typename ThenR, typename... Args> class CallbackThenTest;
template <bool use_once, typename R, typename ThenR, typename... Args> class CallbackThenTest<use_once, R(Args...), ThenR> {
public:
    using CallbackType = typename std::conditional<use_once, OnceCallback<R(Args...)>, RepeatingCallback<R(Args...)>>::type;
    using ThenType = typename std::conditional<use_once, OnceClosure, RepeatingClosure>::type;
    ...
    {
        s = "";
        return Bind([](std::string* s, Args... args) { return Outer(s, std::forward<Args>(args)...); }, &s);
    }
    ...
    static auto GetInner(std::string& s)
    {
        return Bind(&Inner<R, ThenR>, &s);
    }

private:
    template <bool bind_once = use_once, typename F, typename... FArgs, std::enable_if_t<bind_once, int> = 0> static auto Bind(F function, FArgs... args)
    {
    ...
    template <typename R2 = R, std::enable_if_t<!std::is_void_v<R2>, int> = 0> static int Outer(std::string* s, std::unique_ptr<int> a, std::unique_ptr<int> b)
    {
        *s += "Outer";
        *s += base::NumberToString(*a) + base::NumberToString(*b);
        return *a + *b;
    }
    template <typename R2 = R, std::enable_if_t<!std::is_void_v<R2>, int> = 0> static int Outer(std::string* s, int a, int b)
    {
        *s += "Outer";
        *s += base::NumberToString(a) + base::NumberToString(b);
        return a + b;
    }
    template <typename R2 = R, std::enable_if_t<!std::is_void_v<R2>, int> = 0> static int Outer(std::string* s)
    {
        *s += "Outer";
        *s += "None";
        return 99;
    ...
    template <typename R2 = R, std::enable_if_t<std::is_void_v<R2>, int> = 0> static void Outer(std::string* s, std::unique_ptr<int> a, std::unique_ptr<int> b)
    {
        *s += "Outer";
        *s += base::NumberToString(*a) + base::NumberToString(*b);
    }
    template <typename R2 = R, std::enable_if_t<std::is_void_v<R2>, int> = 0> static void Outer(std::string* s, int a, int b)
    {
        *s += "Outer";
        *s += base::NumberToString(a) + base::NumberToString(b);
    }
    template <typename R2 = R, std::enable_if_t<std::is_void_v<R2>, int> = 0> static void Outer(std::string* s)
    {
        *s += "Outer";
        *s += "None";
    }

    template <typename OuterR, typename InnerR, std::enable_if_t<!std::is_void_v<OuterR>, int> = 0, std::enable_if_t<!std::is_void_v<InnerR>, int> = 0>
...
template <typename R, typename ThenR = void, typename... Args> using CallbackThenOnceTest = CallbackThenTest<true, R, ThenR, Args...>;
template <typename R, typename ThenR = void, typename... Args> using CallbackThenRepeatingTest = CallbackThenTest<false, R, ThenR, Args...>;
...
TEST_F(CallbackTest, ThenOnce)
{
    ...
    {
        using VoidReturnWithMoveOnlyArgs = void(std::unique_ptr<int>, std::unique_ptr<int>);
        using ThenReturn = void;
        using Test = CallbackThenOnceTest<VoidReturnWithMoveOnlyArgs, ThenReturn>;
        Test::GetOuter(s).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
        EXPECT_EQ(s, "Outer12InnerNone");
    }
    ...
    }
    {
        ...
        using ThenReturn = int;
        using Test = CallbackThenOnceTest<VoidReturnWithMoveOnlyArgs, ThenReturn>;
        EXPECT_EQ(99, Test::GetOuter(s).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12InnerNone");
    }
    ...
    {
        using NonVoidReturnWithMoveOnlyArgs = int(std::unique_ptr<int>, std::unique_ptr<int>);
        using ThenReturn = void;
        using Test = CallbackThenOnceTest<NonVoidReturnWithMoveOnlyArgs, ThenReturn>;
        Test::GetOuter(s).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
        EXPECT_EQ(s, "Outer12Inner3");
    }
    ...
    }
    {
        ...
        using ThenReturn = int;
        using Test = CallbackThenOnceTest<NonVoidReturnWithMoveOnlyArgs, ThenReturn>;
        EXPECT_EQ(3, Test::GetOuter(s).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12Inner3");
    }
}
...
TEST_F(CallbackTest, ThenRepeating)
{
    ...
    }
    {
        using VoidReturnWithMoveOnlyArgs = void(std::unique_ptr<int>, std::unique_ptr<int>);
        using ThenReturn = void;
        using Test = CallbackThenRepeatingTest<VoidReturnWithMoveOnlyArgs, ThenReturn>;
        auto outer = Test::GetOuter(s);
        outer.Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
        EXPECT_EQ(s, "Outer12InnerNone");
        std::move(outer).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
        EXPECT_EQ(s, "Outer12InnerNoneOuter12InnerNone");
    }
    ...
    }
    {
        ...
        using Test = CallbackThenRepeatingTest<VoidReturnWithMoveOnlyArgs, ThenReturn>;
        auto outer = Test::GetOuter(s);
        EXPECT_EQ(99, outer.Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12InnerNone");
        EXPECT_EQ(99, std::move(outer).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12InnerNoneOuter12InnerNone");
    }
    ...
    // Non-void return from outer + void return from Then().
    {
        using NonVoidReturnWithoutArgs = int();
        using ThenReturn = void;
        using Test = CallbackThenRepeatingTest<NonVoidReturnWithoutArgs, ThenReturn>;
        auto outer = Test::GetOuter(s);
        outer.Then(Test::GetInner(s)).Run();
        EXPECT_EQ(s, "OuterNoneInner99");
        std::move(outer).Then(Test::GetInner(s)).Run();
    ...
    }
    {
        using NonVoidReturnWithMoveOnlyArgs = int(std::unique_ptr<int>, std::unique_ptr<int>);
        using ThenReturn = void;
        using Test = CallbackThenRepeatingTest<NonVoidReturnWithMoveOnlyArgs, ThenReturn>;
        auto outer = Test::GetOuter(s);
        outer.Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
        EXPECT_EQ(s, "Outer12Inner3");
        std::move(outer).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2));
    ...
    }
    {
        ...
        using Test = CallbackThenRepeatingTest<NonVoidReturnWithMoveOnlyArgs, ThenReturn>;
        auto outer = Test::GetOuter(s);
        EXPECT_EQ(3, outer.Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12Inner3");
        EXPECT_EQ(3, std::move(outer).Then(Test::GetInner(s)).Run(std::make_unique<int>(1), std::make_unique<int>(2)));
        EXPECT_EQ(s, "Outer12Inner3Outer12Inner3");
    }
}
...
TEST_F(CallbackTest, MaybeValidInvalidateWeakPtrsOnSameSequence)
{
    ...
    RepeatingCallback<void()> cb = BindRepeating(&ClassWithAMethod::TheMethod, ptr);
    EXPECT_TRUE(cb.MaybeValid());
...
TEST_F(CallbackTest, MaybeValidInvalidateWeakPtrsOnOtherSequence)
{
    ...
    other_thread.task_runner()->PostTask(FROM_HERE,
        BindOnce(
...
TEST_F(CallbackTest, ThenAfterWeakPtr)
{
    ...
    OnceClosure closure = BindOnce(&ClassWithAMethod::TheMethod, ptr).Then(BindLambdaForTesting([&chained_closure_called] { chained_closure_called = true; }));
    factory.InvalidateWeakPtrs();
