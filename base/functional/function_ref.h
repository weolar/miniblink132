...
template <typename R, typename... Args> class FunctionRef<R(Args...)> {
    template <typename Functor, typename RunType = typename internal::FunctorTraits<Functor>::RunType>
    static constexpr bool kCompatibleFunctor
        = std::convertible_to<internal::ExtractReturnType<RunType>, R> && std::same_as<internal::ExtractArgs<RunType>, internal::TypeList<Args...>>;
...
public:
    ...
    template <typename Functor>
        ...
        (!internal::is_instantiation_v<FunctionRef, std::decay_t<Functor>>) &&
        // For the same reason as the second case above, prevent
        // construction from `absl::FunctionRef`.
        (!internal::is_instantiation_v<absl::FunctionRef, std::decay_t<Functor>>)
