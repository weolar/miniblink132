...
public:
    ...
    // std::move().
    void Done(OnceCallback<void(Results)> done_callback, const Location& location = FROM_HERE) &&
    {
        CHECK(info_);
        DCHECK_CALLED_ON_VALID_SEQUENCE(info_->sequence_checker_);
        info_->done_callback_ = BindPostTask(SequencedTaskRunner::GetCurrentDefault(), std::move(done_callback), location);
...
private:
    ...
    public:
        ...
        OnceCallback<void(Results)> done_callback_ GUARDED_BY_CONTEXT(sequence_checker_);
