...
struct DerivedCopyMoveCounter {
    DerivedCopyMoveCounter(int* copies, int* assigns, int* move_constructs, int* move_assigns)
        : copies_(copies)
        , assigns_(assigns)
...
public:
    CopyMoveCounter(int* copies, int* assigns, int* move_constructs, int* move_assigns)
        : copies_(copies)
        , assigns_(assigns)
        , move_constructs_(move_constructs)
        , move_assigns_(move_assigns)
...
template <typename T> T PassThru(T scoper)
{
    return scoper;
}

// Some test functions that we can Bind to.
template <typename T> T PolymorphicIdentity(T t)
{
    return t;
}

template <typename... Ts> struct VoidPolymorphic {
...
public:
    ...
    static int IntFunc0()
    {
        return static_func_mock_ptr->IntMethod0();
    }
    int NoexceptMethod() noexcept
    {
        return 42;
    }
    int ConstNoexceptMethod() const noexcept
    {
        return 42;
    }
...
{
    int n = 0;
    RepeatingClosure cb = BindRepeating(&TakesACallback, BindRepeating(&PtrArgSet, &n));
...
TEST_F(BindTest, IgnoreResultForRepeating)
{
    ...
    RepeatingClosure non_void_weak_method_cb = BindRepeating(IgnoreResult(&NoRef::IntMethod0), weak_factory.GetWeakPtr());
    non_void_weak_method_cb.Run();

    RepeatingClosure non_void_weak_const_method_cb = BindRepeating(IgnoreResult(&NoRef::IntConstMethod0), weak_factory.GetWeakPtr());
...
TEST_F(BindTest, IgnoreResultForOnce)
{
    ...
    OnceClosure non_void_weak_method_cb = BindOnce(IgnoreResult(&NoRef::IntMethod0), weak_factory.GetWeakPtr());
    OnceClosure non_void_weak_const_method_cb = BindOnce(IgnoreResult(&NoRef::IntConstMethod0), weak_factory.GetWeakPtr());
...
TEST_F(BindTest, WeakPtrForOnce)
{
    ...
    OnceCallback<int(int)> normal_func_cb = BindOnce(&FunctionWithWeakFirstParam, weak_factory.GetWeakPtr());
...
TEST_F(BindTest, StdCrefForRepeating)
{
    ...
    RepeatingCallback<int()> const_ref_cb = BindRepeating(&Identity, std::cref(n));
    ...
    RepeatingCallback<int()> all_const_ref_cb = BindRepeating(&GetCopies, std::cref(counter));
...
TEST_F(BindTest, StdCrefForOnce)
{
    ...
    int move_assigns = 0;
    CopyMoveCounter counter(&copies, &assigns, &move_constructs, &move_assigns);
    OnceCallback<int()> all_const_ref_cb = BindOnce(&GetCopies, std::cref(counter));
...
TEST_F(BindTest, OwnedForRepeatingUniquePtr)
{
    ...
    // If we don't capture, delete happens on Callback destruction/reset.
    // return the same value.
    RepeatingCallback<DeleteCounter*()> no_capture_cb = BindRepeating(&PolymorphicIdentity<DeleteCounter*>, Owned(std::move(counter)));
    ...
    RepeatingClosure own_object_cb = BindRepeating(&DeleteCounter::VoidMethod0, Owned(std::move(counter)));
    own_object_cb.Run();
    EXPECT_EQ(0, deletes);
    own_object_cb.Reset();
    EXPECT_EQ(1, deletes);
...
TEST_F(BindTest, OwnedForOnceUniquePtr)
{
    ...
    // If we don't capture, delete happens on Callback destruction/reset.
    // return the same value.
    OnceCallback<DeleteCounter*()> no_capture_cb = BindOnce(&PolymorphicIdentity<DeleteCounter*>, Owned(std::move(counter)));
    EXPECT_EQ(0, deletes);
    no_capture_cb.Reset(); // This should trigger a delete.
    EXPECT_EQ(1, deletes);
    ...
    deletes = 0;
    counter = std::make_unique<DeleteCounter>(&deletes);
    OnceClosure own_object_cb = BindOnce(&DeleteCounter::VoidMethod0, Owned(std::move(counter)));
    EXPECT_EQ(0, deletes);
    own_object_cb.Reset();
    EXPECT_EQ(1, deletes);
}
...
{
    int counter = 0;
    RepeatingCallback<int()> counter_callback = BindRepeating([](int& counter) { return ++counter; }, OwnedRef(counter));
...
{
    OnceCallback<std::string(std::string)> echo_callback = BindOnce([](int& ignore, std::string s) { return s; }, OwnedRef(0));

    EXPECT_EQ("Hello World", std::move(echo_callback).Run("Hello World"));
}

template <typename T> class BindVariantsTest : public ::testing::Test { };

struct RepeatingTestConfig {
    template <typename Signature> using CallbackType = RepeatingCallback<Signature>;
    using ClosureType = RepeatingClosure;

    template <typename F, typename... Args> static auto Bind(F&& f, Args&&... args)
    {
...
struct OnceTestConfig {
    template <typename Signature> using CallbackType = OnceCallback<Signature>;
...
using BindVariantsTestConfig = ::testing::Types<RepeatingTestConfig, OnceTestConfig>;
TYPED_TEST_SUITE(BindVariantsTest, BindVariantsTestConfig);

template <typename TypeParam, typename Signature> using CallbackType = typename TypeParam::template CallbackType<Signature>;
...
TYPED_TEST(BindVariantsTest, FunctionTypeSupport)
{
    ...
    ClosureType method_refptr_cb = TypeParam::Bind(&HasRef::VoidMethod0, WrapRefCounted(&has_ref));
...
TYPED_TEST(BindVariantsTest, ReturnValues)
{
    ...
    EXPECT_CALL(has_ref, IntConstMethod0()).WillOnce(Return(41337)).WillOnce(Return(51337));
    EXPECT_CALL(has_ref, UniquePtrMethod0()).WillOnce(Return(ByMove(std::make_unique<int>(42))));
...
TYPED_TEST(BindVariantsTest, ScopedRefptr)
{
    ...
    CallbackType<TypeParam, int()> scoped_refptr_const_ref_cb = TypeParam::Bind(&FunctionWithScopedRefptrFirstParam, std::cref(refptr), 1);
...
TYPED_TEST(BindVariantsTest, UnretainedRawRefReceiver)
{
    ...
    auto has_ref_cb = TypeParam::Bind(&HasRef::HasAtLeastOneRef, Unretained(raw_has_ref));
...
template <typename T> class BindMoveOnlyTypeTest : public ::testing::Test { };

struct CustomDeleter {
    void operator()(DeleteCounter* c)
    {
        delete c;
    }
};

using MoveOnlyTypesToTest = ::testing::Types<std::unique_ptr<DeleteCounter>, std::unique_ptr<DeleteCounter, CustomDeleter>>;
...
TYPED_TEST(BindMoveOnlyTypeTest, PassedToBoundCallback)
{
    ...
    RepeatingCallback<TypeParam()> callback = BindRepeating(&PassThru<TypeParam>, Passed(&ptr));
...
{
    int deletes = 0;
    RepeatingCallback<TypeParam()> callback = BindRepeating(&PassThru<TypeParam>, Passed(TypeParam(new DeleteCounter(&deletes))));
    EXPECT_EQ(0, deletes);
...
TYPED_TEST(BindMoveOnlyTypeTest, ReturnMoveOnlyType)
{
    int deletes = 0;
    DeleteCounter* counter = new DeleteCounter(&deletes);
    RepeatingCallback<TypeParam()> callback = BindRepeating(&PassThru<TypeParam>, Passed(TypeParam(counter)));
...
TYPED_TEST(BindMoveOnlyTypeTest, UnboundForwarding)
{
    ...
    RepeatingCallback<TypeParam(TypeParam)> cb_unbound = BindRepeating(&PassThru<TypeParam>);
...
std::vector<std::unique_ptr<int>> AcceptAndReturnMoveOnlyVector(std::vector<std::unique_ptr<int>> v)
{
    VerifyVector(v);
    return v;
}
...
TEST_F(BindTest, BindMoveOnlyVector)
{
    ...
    RepeatingCallback<MoveOnlyVector()> bound_cb = BindRepeating(&AcceptAndReturnMoveOnlyVector, Passed(&v));
    ...
    RepeatingCallback<MoveOnlyVector(MoveOnlyVector)> unbound_cb = BindRepeating(&AcceptAndReturnMoveOnlyVector);
...
TEST_F(BindTest, ArgumentCopies)
{
    ...
    copies = 0;
    assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyCounter>::Run, CopyCounter(&copies, &assigns));
    EXPECT_EQ(1, copies);
    EXPECT_EQ(0, assigns);
    ...
    copies = 0;
    assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyCounter>::Run).Run(CopyCounter(&copies, &assigns));
    EXPECT_EQ(1, copies);
    EXPECT_EQ(0, assigns);
    ...
    copies = 0;
    assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyCounter>::Run).Run(CopyCounter(DerivedCopyMoveCounter(&copies, &assigns, nullptr, nullptr)));
    EXPECT_EQ(2, copies);
    EXPECT_EQ(0, assigns);
}
...
TEST_F(BindTest, ArgumentMoves)
{
    ...
    BindRepeating(&VoidPolymorphic<const MoveCounter&>::Run, MoveCounter(&move_constructs, &move_assigns));
    ...
    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<MoveCounter>::Run).Run(MoveCounter(&move_constructs, &move_assigns));
    EXPECT_EQ(1, move_constructs);
    EXPECT_EQ(0, move_assigns);

    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<MoveCounter>::Run).Run(MoveCounter(DerivedCopyMoveCounter(nullptr, nullptr, &move_constructs, &move_assigns)));
...
TEST_F(BindTest, ArgumentCopiesAndMoves)
{
    ...
    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyMoveCounter>::Run, CopyMoveCounter(&copies, &assigns, &move_constructs, &move_assigns));
    EXPECT_EQ(0, copies);
    EXPECT_EQ(0, assigns);
    EXPECT_EQ(1, move_constructs);
    EXPECT_EQ(0, move_assigns);
    ...
    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyMoveCounter>::Run).Run(CopyMoveCounter(&copies, &assigns, &move_constructs, &move_assigns));
    EXPECT_EQ(0, copies);
    EXPECT_EQ(0, assigns);
    EXPECT_EQ(1, move_constructs);
    EXPECT_EQ(0, move_assigns);

    DerivedCopyMoveCounter derived_counter(&copies, &assigns, &move_constructs, &move_assigns);
    copies = 0;
    assigns = 0;
    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyMoveCounter>::Run).Run(CopyMoveCounter(derived_counter));
    EXPECT_EQ(1, copies);
    EXPECT_EQ(0, assigns);
    EXPECT_EQ(1, move_constructs);
    EXPECT_EQ(0, move_assigns);
    ...
    move_constructs = 0;
    move_assigns = 0;
    BindRepeating(&VoidPolymorphic<CopyMoveCounter>::Run).Run(CopyMoveCounter(DerivedCopyMoveCounter(&copies, &assigns, &move_constructs, &move_assigns)));
    EXPECT_EQ(0, copies);
    EXPECT_EQ(0, assigns);
    EXPECT_EQ(2, move_constructs);
    EXPECT_EQ(0, move_assigns);
...
TEST_F(BindTest, CapturelessLambda)
{
    ...
    int x = 1;
    RepeatingCallback<void(int)> cb = BindRepeating([](int* x, int i) { *x *= i; }, Unretained(&x));
    cb.Run(6);
    EXPECT_EQ(6, x);
    cb.Run(7);
    EXPECT_EQ(42, x);
...
TEST_F(BindTest, CapturingLambdaForTesting)
{
    ...
    EXPECT_EQ(42, BindLambdaForTesting([=](int y) mutable { return x *= y; }).Run(7));
    ...
    auto g = [y = std::move(y)](int& x) mutable { return x * *std::exchange(y, nullptr); };
...
TEST_F(BindTest, OnceCallback)
{
    ...
    static_assert(std::is_constructible_v<RepeatingClosure, const RepeatingClosure&>, "RepeatingClosure should be copyable.");
    static_assert(std::is_assignable_v<RepeatingClosure, const RepeatingClosure&>, "RepeatingClosure should be copy-assignable.");

    // Move constructor and assignment of RepeatingCallback.
    static_assert(std::is_constructible_v<RepeatingClosure, RepeatingClosure&&>, "RepeatingClosure should be movable.");
    static_assert(std::is_assignable_v<RepeatingClosure, RepeatingClosure&&>, "RepeatingClosure should be move-assignable");

    // Conversions from OnceCallback to RepeatingCallback.
    static_assert(!std::is_constructible_v<RepeatingClosure, const OnceClosure&>, "OnceClosure should not be convertible to RepeatingClosure.");
    static_assert(!std::is_assignable_v<RepeatingClosure, const OnceClosure&>, "OnceClosure should not be convertible to RepeatingClosure.");

    // Destructive conversions from OnceCallback to RepeatingCallback.
    static_assert(!std::is_constructible_v<RepeatingClosure, OnceClosure&&>, "OnceClosure should not be convertible to RepeatingClosure.");
    static_assert(!std::is_assignable_v<RepeatingClosure, OnceClosure&&>, "OnceClosure should not be convertible to RepeatingClosure.");

    // Copy constructor and assignment of OnceCallback.
    static_assert(!std::is_constructible_v<OnceClosure, const OnceClosure&>, "OnceClosure should not be copyable.");
    static_assert(!std::is_assignable_v<OnceClosure, const OnceClosure&>, "OnceClosure should not be copy-assignable");

    // Move constructor and assignment of OnceCallback.
    static_assert(std::is_constructible_v<OnceClosure, OnceClosure&&>, "OnceClosure should be movable.");
    static_assert(std::is_assignable_v<OnceClosure, OnceClosure&&>, "OnceClosure should be move-assignable.");

    // Conversions from RepeatingCallback to OnceCallback.
    static_assert(std::is_constructible_v<OnceClosure, const RepeatingClosure&>, "RepeatingClosure should be convertible to OnceClosure.");
    static_assert(std::is_assignable_v<OnceClosure, const RepeatingClosure&>, "RepeatingClosure should be convertible to OnceClosure.");

    // Destructive conversions from RepeatingCallback to OnceCallback.
    static_assert(std::is_constructible_v<OnceClosure, RepeatingClosure&&>, "RepeatingClosure should be convertible to OnceClosure.");
    static_assert(std::is_assignable_v<OnceClosure, RepeatingClosure&&>, "RepeatingClosure should be covretible to OnceClosure.");
    ...
    OnceCallback<void(int)> cb4 = BindOnce(&VoidPolymorphic<std::unique_ptr<int>, int>::Run, std::make_unique<int>(0));
...
TEST_F(BindTest, WindowsCallingConventions)
{
    ...
    class MethodHolder {
    public:
        int __stdcall Func(int n)
        {
            return n;
        }
        int __stdcall ConstFunc(int n) const
        {
            return -n;
        }
    };
    ...
    MethodHolder obj;
    auto stdcall_method_cb = BindRepeating(&MethodHolder::Func, Unretained(&obj), 1);
    EXPECT_EQ(1, stdcall_method_cb.Run());

    const MethodHolder const_obj;
    auto stdcall_const_method_cb = BindRepeating(&MethodHolder::ConstFunc, Unretained(&const_obj), 1);
...
TEST_F(BindTest, BindNoexcept)
{
    ...
    EXPECT_EQ(42, BindOnce(&BindTest::ConstNoexceptMethod, Unretained(this)).Run());
...
template <typename T> struct MockRacyWeakPtr {
...
template <typename T> struct IsWeakReceiver<MockRacyWeakPtr<T>> : std::true_type { };

template <typename T> struct BindUnwrapTraits<MockRacyWeakPtr<T>> {
    static T* Unwrap(const MockRacyWeakPtr<T>& o)
    {
        return o.Lock();
    }
};

template <typename T> struct MaybeValidTraits<MockRacyWeakPtr<T>> {
    static bool MaybeValid(const MockRacyWeakPtr<T>& o)
    {
        return true;
    }
};
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) && PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL)
...
public:
    void SetUp() override
    {
        ...
        enabled_feature_list_.InitWithFeaturesAndParameters(
            { { features::kPartitionAllocUnretainedDanglingPtr, { { "mode", "crash" } } } }, { /* disabled_features */ });
    ...
    // root.
    template <typename T, RawPtrTraits Traits = RawPtrTraits::kEmpty, typename... Args> raw_ptr<T, Traits> Alloc(Args&&... args)
    {
        void* ptr = allocator_.root()->Alloc(sizeof(T), "");
        T* p = new (reinterpret_cast<T*>(ptr)) T(std::forward<Args>(args)...);
        return raw_ptr<T, Traits>(p);
    }
    template <typename T, RawPtrTraits Traits> void Free(raw_ptr<T, Traits>& ptr)
    {
...
private:
    ...
    partition_alloc::PartitionAllocatorForTesting allocator_ { kOnlyEnableBackupRefPtrOptions };
};

class BindUnretainedDanglingTest : public BindUnretainedDanglingInternalFixture { };
class BindUnretainedDanglingDeathTest : public BindUnretainedDanglingInternalFixture { };
...
bool MayBeDanglingAndDummyTraitCheckFn(MayBeDangling<int, RawPtrTraits::kDummyForTest> p)
{
    return p != nullptr;
}
...
public:
    ...
    void RawPtrArg(int* p)
    {
        *p = 123;
    }
    void RawRefArg(int& p)
    {
        p = 123;
    }
    WeakPtr<ClassWithWeakPtr> GetWeakPtr()
    {
...
{
    raw_ptr<int, RawPtrTraits::kDummyForTest> p = Alloc<int, RawPtrTraits::kDummyForTest>(3);
    auto callback = BindOnce(MayBeDanglingAndDummyTraitCheckFn, UnsafeDangling(p));
    Free(p);
    EXPECT_EQ(std::move(callback).Run(), true);
}

TEST_F(BindUnretainedDanglingTest, UnsafeDanglingPtrWithDummyAndDanglingTraits)
{
    raw_ptr<int, RawPtrTraits::kDummyForTest | RawPtrTraits::kMayDangle> p = Alloc<int, RawPtrTraits::kDummyForTest | RawPtrTraits::kMayDangle>(3);
    auto callback = BindOnce(MayBeDanglingAndDummyTraitCheckFn, UnsafeDangling(p));
    Free(p);
    EXPECT_EQ(std::move(callback).Run(), true);
}
...
TEST_F(BindUnretainedDanglingTest, UnsafeDanglingPtrNoRawPtrReceiver)
{
    ...
    int val = 0;
    auto callback = BindOnce(&ClassWithWeakPtr::RawPtrArg, UnsafeDangling(r.get()), Unretained(&val));
    std::move(callback).Run();
    EXPECT_EQ(val, 123);
}
...
TEST_F(BindUnretainedDanglingTest, UnretainedWeakReceiverValidNoDangling)
{
    raw_ptr<int> p = Alloc<int>(3);
    std::unique_ptr<ClassWithWeakPtr> r = std::make_unique<ClassWithWeakPtr>();
    auto callback = BindOnce(&ClassWithWeakPtr::RawPtrArg, r->GetWeakPtr(), Unretained(p));
    std::move(callback).Run();
    EXPECT_EQ(*p, 123);
    Free(p);
}
...
TEST_F(BindUnretainedDanglingTest, UnretainedRefWeakReceiverValidNoDangling)
{
    ...
    int& ref = *p;
    std::unique_ptr<ClassWithWeakPtr> r = std::make_unique<ClassWithWeakPtr>();
    auto callback = BindOnce(&ClassWithWeakPtr::RawRefArg, r->GetWeakPtr(), std::ref(ref));
    std::move(callback).Run();
    EXPECT_EQ(*p, 123);
    Free(p);
}
...
TEST_F(BindUnretainedDanglingTest, UnretainedRefUnsafeDanglingUntriaged)
{
    raw_ptr<int> p = Alloc<int>(3);
    int& ref = *p;
    auto callback = BindOnce(RefCheckFn, UnsafeDanglingUntriaged(raw_ref<const int>(ref)));
    Free(p);
    EXPECT_EQ(std::move(callback).Run(), true);
    // Should reach this point without crashing; there is a dangling pointer, but
...
TEST_F(BindUnretainedDanglingDeathTest, UnretainedRefWithManualUnretainedDanglingPtr)
{
    raw_ptr<int> p = Alloc<int>(3);
    int& ref = *p;
