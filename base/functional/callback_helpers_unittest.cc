...
template <typename TagType, typename CallbackType> struct TestConversionAndAssignmentImpl {
    static constexpr bool kSupportsConversion = std::is_convertible_v<TagType, CallbackType>;
    static constexpr bool kSupportsAssignment = std::is_assignable_v<CallbackType, TagType>;
    static_assert(kSupportsConversion == kSupportsAssignment);

    static constexpr bool kValue = kSupportsConversion;
};

template <typename T, typename U> constexpr bool TestConversionAndAssignment = TestConversionAndAssignmentImpl<T, U>::kValue;

#define VOID_RETURN_CALLBACK_TAG_TEST(CallbackType, Sig, BadSig, BoundArg)                                                                                     \
    static_assert(TestConversionAndAssignment<decltype(base::NullCallback()), CallbackType<Sig>>);                                                             \
    static_assert(TestConversionAndAssignment<decltype(base::NullCallbackAs<Sig>()), CallbackType<Sig>>);                                                      \
    static_assert(TestConversionAndAssignment<decltype(base::DoNothing()), CallbackType<Sig>>);                                                                \
    static_assert(TestConversionAndAssignment<decltype(base::DoNothingAs<Sig>()), CallbackType<Sig>>);                                                         \
    static_assert(TestConversionAndAssignment<decltype(base::DoNothingWithBoundArgs(BoundArg)), CallbackType<Sig>>);                                           \
                                                                                                                                                               \
    static_assert(!TestConversionAndAssignment<decltype(base::NullCallbackAs<BadSig>()), CallbackType<Sig>>);                                                  \
    static_assert(!TestConversionAndAssignment<decltype(base::DoNothingAs<BadSig>()), CallbackType<Sig>>);                                                     \
    static_assert(TestConversionAndAssignment<decltype(base::DoNothingWithBoundArgs(BadArg())), CallbackType<Sig>>)
...
VOID_RETURN_CALLBACK_TAG_TEST(base::RepeatingCallback, void(int), void(char), 8);
NON_VOID_RETURN_CALLBACK_TAG_TEST(base::RepeatingCallback, int(int), char(int), 8);
...
TEST(CallbackHelpersTest, ScopedClosureRunnerMoveConstructor)
{
    int run_count = 0;
    {
        std::unique_ptr<base::ScopedClosureRunner> runner(new base::ScopedClosureRunner(base::BindOnce(&Increment, &run_count)));
...
TEST(CallbackHelpersTest, ScopedClosureRunnerMoveAssignment)
{
    ...
    int run_count_2 = 0;
    {
        ...
        {
            base::ScopedClosureRunner runner2(base::BindOnce(&Increment, &run_count_2));
...
TEST(CallbackHelpersTest, SplitOnceCallback_EmptyCallback)
{
    ...
    static_assert(std::is_same_v<decltype(split), std::pair<base::OnceCallback<void(int*)>, base::OnceCallback<void(int*)>>>, "");
...
{
    int count = 0;
    base::OnceCallback<void(int*)> cb = base::BindOnce([](int* count) { ++*count; });
...
{
    int count = 0;
    base::RepeatingClosure repeating_closure = base::BindRepeating(&Increment, &count);
    base::OnceClosure once_closure = base::BindOnce(&Increment, &count);

    base::RepeatingCallback<void(int)> repeating_int_cb = base::IgnoreArgs<int>(repeating_closure);
    EXPECT_EQ(0, count);
    ...
    base::OnceCallback<void(int)> once_int_cb = base::IgnoreArgs<int>(std::move(once_closure));
    EXPECT_EQ(2, count);
    ...
    auto once_cb_with_extra_arg = base::IgnoreArgs<char, bool>(repeating_callback);
...
{
    base::RepeatingCallback<void(int)> repeating_int_cb = base::IgnoreArgs<int>(base::RepeatingClosure());
    EXPECT_FALSE(repeating_int_cb);

    base::OnceCallback<void(int)> once_int_cb = base::IgnoreArgs<int>(base::OnceClosure());
...
{
    int count = 0;
    base::RepeatingCallback<int(void)> repeating_no_param_cb = base::BindRepeating(&IncrementAndReturn, &count);
    base::OnceCallback<int(void)> once_no_param_cb = base::BindOnce(&IncrementAndReturn, &count);

    base::RepeatingCallback<int(int)> repeating_int_cb = base::IgnoreArgs<int>(repeating_no_param_cb);
    EXPECT_EQ(0, count);
    ...
    base::OnceCallback<int(int)> once_int_cb = base::IgnoreArgs<int>(std::move(once_no_param_cb));
    EXPECT_EQ(2, count);
    ...
    auto once_cb_with_extra_arg = base::IgnoreArgs<char, bool>(std::move(once_cb));
...
{
    int count = 0;
    auto tie_cb = base::ForwardRepeatingCallbacks({ base::BindRepeating(&IncrementWithRef), base::BindRepeating(&IncrementWithRef) });
...
TEST(CallbackHelpersTest, ReturnValueOnce)
{
    ...
    static_assert(std::is_same_v<decltype(string_factory), base::OnceCallback<std::string(void)>>);
    ...
    static_assert(std::is_same_v<decltype(unique_ptr_factory), base::OnceCallback<std::unique_ptr<int>(void)>>);
