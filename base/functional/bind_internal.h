...
template <typename Functor, typename BoundArgsTuple> struct CallbackCancellationTraits;
...
template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits = RawPtrTraits::kEmpty> class UnretainedWrapper {
...
public:
    ...
    using GetPtrType
        = std::conditional_t<raw_ptr_traits::IsSupportedType<T>::value && std::same_as<UnretainedTrait, unretained_traits::MayDangle>, DanglingRawPtrType, T*>;
...
private:
    ...
    {
        if constexpr (std::same_as<UnretainedTrait, unretained_traits::MayNotDangle>) {
    ...
    using StorageType = std::conditional_t<raw_ptr_traits::IsSupportedType<T>::value, DanglingRawPtrType, T*>;
    ...
    static_assert(std::is_pointer_v<GetPtrType> || std::same_as<GetPtrType, StorageType>);
...
template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits = RawPtrTraits::kEmpty> class UnretainedRefWrapper {
public:
...
private:
    ...
    // `UnsafeDangling()` and `UnsafeDanglingUntriaged()`), when retrieving the
    // pointer value via `get()` above.
    using StorageType = std::conditional_t<raw_ptr_traits::IsSupportedType<T>::value, raw_ref<T, DisableDanglingPtrDetection>, T&>;
...
template <template <typename, typename, RawPtrTraits> typename WrapperT, typename T> inline constexpr bool kIsUnretainedWrapper = false;

template <template <typename, typename, RawPtrTraits> typename WrapperT, typename T, typename UnretainedTrait, RawPtrTraits PtrTraits>
inline constexpr bool kIsUnretainedWrapper<WrapperT, WrapperT<T, UnretainedTrait, PtrTraits>> = true;
...
template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits> class UnretainedRefWrapperReceiver {
public:
    // NOLINTNEXTLINE(google-explicit-constructor)
    UnretainedRefWrapperReceiver(UnretainedRefWrapper<T, UnretainedTrait, PtrTraits>&& obj)
        : obj_(std::move(obj))
    {
    }
...
template <typename T> struct MethodReceiverStorage {
    using Type = std::conditional_t<IsRawPointer<T>, scoped_refptr<RemoveRawPointerT<T>>, T>;
};

template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits> struct MethodReceiverStorage<UnretainedRefWrapper<T, UnretainedTrait, PtrTraits>> {
...
template <typename T, typename Deleter = std::default_delete<T>> class OwnedWrapper {
public:
...
public:
    explicit OwnedRefWrapper(const T& t)
        : t_(t)
    {
    }
    explicit OwnedRefWrapper(T&& t)
        : t_(std::move(t))
    {
    }
    T& get() const
...
// the method.
template <bool is_method, typename... Args> inline constexpr bool kIsWeakMethod = false;

template <typename T, typename... Args> inline constexpr bool kIsWeakMethod<true, T, Args...> = IsWeakReceiver<T>::value;
...
template <size_t n, typename List> using DropTypeListItem = typename DropTypeListItemImpl<n, List>::Type;
...
template <size_t n, typename T, typename... List, typename... Accum>
    requires(n > 0)
struct TakeTypeListItemImpl<n, TypeList<T, List...>, Accum...> : TakeTypeListItemImpl<n - 1, TypeList<List...>, Accum..., T> { };
...
template <size_t n, typename List> using TakeTypeListItem = typename TakeTypeListItemImpl<n, List>::Type;

// Implements `MakeFunctionType`.
template <typename R, typename ArgList> struct MakeFunctionTypeImpl;

template <typename R, typename... Args> struct MakeFunctionTypeImpl<R, TypeList<Args...>> {
    using Type = R(Args...);
};

// A type-level function that constructs a function type that has `R` as its
// return type and has a `TypeList`'s items as its arguments.
template <typename R, typename ArgList> using MakeFunctionType = typename MakeFunctionTypeImpl<R, ArgList>::Type;
...
template <typename Signature> using ExtractArgs = typename ExtractArgsImpl<Signature>::ArgsList;

// A type-level function that extracts the return type of a function.
// e.g. `ExtractReturnType<R(A, B, C)>` -> `R`.
template <typename Signature> using ExtractReturnType = typename ExtractArgsImpl<Signature>::ReturnType;

template <typename Callable, typename Signature = decltype(&Callable::operator())> struct ExtractCallableRunTypeImpl;

#define BIND_INTERNAL_EXTRACT_CALLABLE_RUN_TYPE_WITH_QUALS(quals)                                                                                              \
    template <typename Callable, typename R, typename... Args> struct ExtractCallableRunTypeImpl<Callable, R (Callable::*)(Args...) quals> {                   \
        using Type = R(Args...);                                                                                                                               \
    }
...
//     `double(int, char*)`.
template <typename Callable> using ExtractCallableRunType = typename ExtractCallableRunTypeImpl<Callable>::Type;
...
template <typename T> inline constexpr bool IsObjCArcBlockPointer = false;

#if __OBJC__ && HAS_FEATURE(objc_arc)
template <typename R, typename... Args> inline constexpr bool IsObjCArcBlockPointer<R (^)(Args...)> = true;
#endif
...
concept HasOverloadedCallOp = requires {
    ...
    requires requires(Functor && f, BoundArgs && ... args) { std::forward<Functor>(f)(std::forward<BoundArgs>(args)...); };
...
concept HasRefCountedTypeAsRawPtr = std::disjunction_v<NeedsScopedRefptrButGetsRawPtr<Ts>...>;
...
template <typename Functor, typename... BoundArgs> struct FunctorTraits;

// Provides functor traits for pre-decayed functor types.
template <typename Functor, typename... BoundArgs> struct DecayedFunctorTraits;
...
struct DecayedFunctorTraits<Functor, BoundArgs...> {
    ...
    template <typename RunFunctor, typename... RunArgs> static ExtractReturnType<RunType> Invoke(RunFunctor&& functor, RunArgs&&... args)
    {
...
// Functions.
template <typename R, typename... Args, typename... BoundArgs> struct DecayedFunctorTraits<R (*)(Args...), BoundArgs...> {
    ...
    template <typename Function, typename... RunArgs> static R Invoke(Function&& function, RunArgs&&... args)
    {
...
struct DecayedFunctorTraits<R (*)(Args...) noexcept, BoundArgs...> : DecayedFunctorTraits<R (*)(Args...), BoundArgs...> { };
...
#define BIND_INTERNAL_DECAYED_FUNCTOR_TRAITS_WITH_CONV_AND_QUALS(conv, quals)                                                                                  \
    template <typename R, typename... Args, typename... BoundArgs>                                                                                             \
    struct DecayedFunctorTraits<R(conv*)(Args...) quals, BoundArgs...> : DecayedFunctorTraits<R (*)(Args...) quals, BoundArgs...> { }
...
template <typename R, typename... Args, typename... BoundArgs> struct DecayedFunctorTraits<R (^)(Args...), BoundArgs...> {
    ...
    template <typename BlockType, typename... RunArgs> static R Invoke(BlockType&& block, RunArgs&&... args)
    {
...
// Methods.
template <typename R, typename Receiver, typename... Args, typename... BoundArgs> struct DecayedFunctorTraits<R (Receiver::*)(Args...), BoundArgs...> {
    ...
    template <typename Method, typename ReceiverPtr, typename... RunArgs> static R Invoke(Method method, ReceiverPtr&& receiver_ptr, RunArgs&&... args)
    {
...
struct DecayedFunctorTraits<R (Receiver::*)(Args...) const, BoundArgs...> : DecayedFunctorTraits<R (Receiver::*)(Args...), BoundArgs...> {
    using RunType = R(const Receiver*, Args...);
};

#define BIND_INTERNAL_DECAYED_FUNCTOR_TRAITS_WITH_CONST_AND_QUALS(constqual, quals)                                                                            \
    template <typename R, typename Receiver, typename... Args, typename... BoundArgs>                                                                          \
    struct DecayedFunctorTraits<R (Receiver::*)(Args...) constqual quals, BoundArgs...>                                                                        \
        : DecayedFunctorTraits<R (Receiver::*)(Args...) constqual, BoundArgs...> { }
...
#define BIND_INTERNAL_DECAYED_FUNCTOR_TRAITS_STDCALL_WITH_QUALS(quals)                                                                                         \
    template <typename R, typename Receiver, typename... Args, typename... BoundArgs>                                                                          \
    struct DecayedFunctorTraits<R (__stdcall Receiver::*)(Args...) quals, BoundArgs...>                                                                        \
        : public DecayedFunctorTraits<R (Receiver::*)(Args...) quals, BoundArgs...> { }
...
// `IgnoreResult`s.
template <typename T, typename... BoundArgs> struct DecayedFunctorTraits<IgnoreResultHelper<T>, BoundArgs...> : FunctorTraits<T, BoundArgs...> {
    using RunType = typename ForceVoidReturn<typename FunctorTraits<T, BoundArgs...>::RunType>::RunType;

    template <typename IgnoreResultType, typename... RunArgs> static void Invoke(IgnoreResultType&& ignore_result_helper, RunArgs&&... args)
    {
        FunctorTraits<T, BoundArgs...>::Invoke(std::forward<IgnoreResultType>(ignore_result_helper).functor_, std::forward<RunArgs>(args)...);
    }
};

// `OnceCallback`s.
template <typename R, typename... Args, typename... BoundArgs> struct DecayedFunctorTraits<OnceCallback<R(Args...)>, BoundArgs...> {
    using RunType = R(Args...);
    static constexpr bool is_method = false;
    static constexpr bool is_nullable = true;
    ...
    template <typename CallbackType, typename... RunArgs> static R Invoke(CallbackType&& callback, RunArgs&&... args)
    {
...
// `RepeatingCallback`s.
template <typename R, typename... Args, typename... BoundArgs> struct DecayedFunctorTraits<RepeatingCallback<R(Args...)>, BoundArgs...> {
    using RunType = R(Args...);
    static constexpr bool is_method = false;
    static constexpr bool is_nullable = true;
    static constexpr bool is_callback = true;
...
struct FunctorTraits<Functor, BoundArgs...> : DecayedFunctorTraits<std::decay_t<Functor>, BoundArgs...> { };
...
struct FunctorTraits<Functor, BoundArgs...> {
    ...
    using RunType = decltype(std::declval<Functor>()(std::declval<BoundArgs>()...))(std::decay_t<BoundArgs>...);
    static constexpr bool is_method = false;
    static constexpr bool is_nullable = false;
    static constexpr bool is_callback = false;
...
template <typename T> struct StorageTraits {
...
template <typename T> struct StorageTraits<T*> {
...
template <typename T, RawPtrTraits PtrTraits> struct StorageTraits<raw_ptr<T, PtrTraits>> {
...
template <typename T> struct StorageTraits<std::reference_wrapper<T>> {
    using Type = UnretainedRefWrapper<T, unretained_traits::MayNotDangle>;
    static constexpr bool value = Type::value;
};

template <typename T> using ValidateStorageTraits = StorageTraits<std::decay_t<T>>;
...
template <bool is_weak_call, typename Traits, typename ReturnType, size_t... indices> struct InvokeHelper;

template <typename Traits, typename ReturnType, size_t... indices> struct InvokeHelper<false, Traits, ReturnType, indices...> {
    template <typename Functor, typename BoundArgsTuple, typename... RunArgs>
    static inline ReturnType MakeItSo(Functor&& functor, BoundArgsTuple&& bound, RunArgs&&... args)
    {
        return Traits::Invoke(
            Unwrap(std::forward<Functor>(functor)), Unwrap(std::get<indices>(std::forward<BoundArgsTuple>(bound)))..., std::forward<RunArgs>(args)...);
    }
};

template <typename Traits, typename ReturnType, size_t index_target, size_t... index_tail>
struct InvokeHelper<true, Traits, ReturnType, index_target, index_tail...> {
    template <typename Functor, typename BoundArgsTuple, typename... RunArgs>
    static inline void MakeItSo(Functor&& functor, BoundArgsTuple&& bound, RunArgs&&... args)
    {
        ...
        Traits::Invoke(Unwrap(std::forward<Functor>(functor)), target, Unwrap(std::get<index_tail>(std::forward<BoundArgsTuple>(bound)))...,
...
template <typename Traits, typename StorageType, typename UnboundRunType> struct Invoker;

template <typename Traits, typename StorageType, typename R, typename... UnboundArgs> struct Invoker<Traits, StorageType, R(UnboundArgs...)> {
private:
    using Indices = std::make_index_sequence<std::tuple_size_v<decltype(StorageType::bound_args_)>>;

public:
    static R RunOnce(BindStateBase* base, PassingType<UnboundArgs>... unbound_args)
    {
        auto* const storage = static_cast<StorageType*>(base);
        return RunImpl(std::move(storage->functor_), std::move(storage->bound_args_), Indices(), std::forward<UnboundArgs>(unbound_args)...);
    }
    ...
    static R Run(BindStateBase* base, PassingType<UnboundArgs>... unbound_args)
    {
        ...
        return RunImpl(storage->functor_, storage->bound_args_, Indices(), std::forward<UnboundArgs>(unbound_args)...);
    }
...
private:
    ...
    template <bool is_weak_call, bool v = !is_weak_call || std::is_void_v<R>> struct WeakCallReturnsVoid {
        static constexpr bool value = [] {
            static_assert(v, "WeakPtrs can only bind to methods without return values.");
            return v;
        }();
    };

    template <typename Functor, typename BoundArgsTuple, size_t... indices>
    static inline R RunImpl(Functor&& functor, BoundArgsTuple&& bound, std::index_sequence<indices...>, UnboundArgs&&... unbound_args)
    {
#if PA_BUILDFLAG(USE_ASAN_BACKUP_REF_PTR)
        RawPtrAsanBoundArgTracker raw_ptr_asan_bound_arg_tracker;
        raw_ptr_asan_bound_arg_tracker.AddArgs(std::get<indices>(std::forward<BoundArgsTuple>(bound))..., std::forward<UnboundArgs>(unbound_args)...);
#endif // PA_BUILDFLAG(USE_ASAN_BACKUP_REF_PTR)

        using DecayedArgsTuple = std::decay_t<BoundArgsTuple>;
        static constexpr bool kIsWeakCall = kIsWeakMethod<Traits::is_method, std::tuple_element_t<indices, DecayedArgsTuple>...>;
        if constexpr (WeakCallReturnsVoid<kIsWeakCall>::value) {
            ...
            return InvokeHelper<kIsWeakCall, Traits, R, indices...>::MakeItSo(
                std::forward<Functor>(functor), std::forward<BoundArgsTuple>(bound), std::forward<UnboundArgs>(unbound_args)...);
        }
    }
};
...
template <typename... Unused> void VerifyMethodReceiver(Unused&&...)
{
}

template <typename Receiver, typename... Unused> void VerifyMethodReceiver(Receiver&& receiver, Unused&&...)
{
    // Asserts that a callback is not the first owner of a ref-counted receiver.
    if constexpr (IsRawPointer<std::decay_t<Receiver>> && IsRefCountedType<RemoveRawPointerT<std::decay_t<Receiver>>>) {
        DCHECK(receiver);
...
template <bool is_method, bool is_nullable, bool is_callback, typename Functor, typename... BoundArgs> struct BindState final : BindStateBase {
private:
    using BoundArgsTuple = std::tuple<BoundArgs...>;

public:
    template <typename ForwardFunctor, typename... ForwardBoundArgs>
    static BindState* Create(BindStateBase::InvokeFuncStorage invoke_func, ForwardFunctor&& functor, ForwardBoundArgs&&... bound_args)
    {
        if constexpr (is_method) {
            VerifyMethodReceiver(bound_args...);
        }
        return new BindState(invoke_func, std::forward<ForwardFunctor>(functor), std::forward<ForwardBoundArgs>(bound_args)...);
    }
...
private:
    using CancellationTraits = CallbackCancellationTraits<Functor, BoundArgsTuple>;

    template <typename ForwardFunctor, typename... ForwardBoundArgs>
        requires CancellationTraits::is_cancellable
    explicit BindState(BindStateBase::InvokeFuncStorage invoke_func, ForwardFunctor&& functor, ForwardBoundArgs&&... bound_args)
    ...
    static bool QueryCancellationTraits(const BindStateBase* base, BindStateBase::CancellationQueryMode mode)
    {
        auto* const storage = static_cast<const BindState*>(base);
        static constexpr std::make_index_sequence<sizeof...(BoundArgs)> kIndices;
        return (mode == BindStateBase::CancellationQueryMode::kIsCancelled) ? storage->IsCancelled(kIndices) : storage->MaybeValid(kIndices);
    }
    ...
    template <size_t... indices> bool IsCancelled(std::index_sequence<indices...>) const
    {
        return CancellationTraits::IsCancelled(functor_, std::get<indices>(bound_args_)...);
    }

    template <size_t... indices> bool MaybeValid(std::index_sequence<indices...>) const
    {
        return CancellationTraits::MaybeValid(functor_, std::get<indices>(bound_args_)...);
    }
...
template <bool is_method, bool is_nullable, bool is_callback, typename Functor, typename... BoundArgs> struct ValidateBindStateType;

template <bool is_nullable, bool is_callback, typename Functor, typename... BoundArgs>
struct ValidateBindStateType<false, is_nullable, is_callback, Functor, BoundArgs...> {
private:
    template <bool v = !HasRefCountedTypeAsRawPtr<std::decay_t<BoundArgs>...>> struct NoRawPtrsToRefCountedTypes {
        static constexpr bool value = [] {
            static_assert(v, "A parameter is a refcounted type and needs scoped_refptr.");
            return v;
        }();
    };

public:
    using Type = BindState<false, is_nullable, is_callback, std::decay_t<Functor>, typename ValidateStorageTraits<BoundArgs>::Type...>;
    static constexpr bool value = std::conjunction_v<NoRawPtrsToRefCountedTypes<>, ValidateStorageTraits<BoundArgs>...>;
};

template <bool is_nullable, bool is_callback, typename Functor> struct ValidateBindStateType<true, is_nullable, is_callback, Functor> {
    using Type = BindState<true, is_nullable, is_callback, std::decay_t<Functor>>;
    static constexpr bool value = true;
};

template <bool is_nullable, bool is_callback, typename Functor, typename Receiver, typename... BoundArgs>
struct ValidateBindStateType<true, is_nullable, is_callback, Functor, Receiver, BoundArgs...> {
private:
    using DecayedReceiver = std::decay_t<Receiver>;
    using ReceiverStorageType = typename MethodReceiverStorage<DecayedReceiver>::Type;

    template <bool v = !std::is_array_v<std::remove_reference_t<Receiver>>> struct FirstBoundArgIsNotArray {
        static constexpr bool value = [] {
    ...
    template <bool v = !IsRawRefV<DecayedReceiver>> struct ReceiverIsNotRawRef {
        static constexpr bool value = [] {
            static_assert(v,
                "Receivers may not be raw_ref<T>. If using a raw_ref<T> "
    ...
    template <bool v = !IsRawPointer<DecayedReceiver> || IsRefCountedType<RemoveRawPointerT<DecayedReceiver>>> struct ReceiverIsNotRawPtr {
        static constexpr bool value = [] {
            static_assert(v,
...
public:
    using Type = BindState<true, is_nullable, is_callback, std::decay_t<Functor>, ReceiverStorageType, typename ValidateStorageTraits<BoundArgs>::Type...>;
    static constexpr bool value = std::conjunction_v<FirstBoundArgIsNotArray<>, ReceiverIsNotRawRef<>, ReceiverIsNotRawPtr<>, NoRawPtrsToRefCountedTypes<>,
...
template <bool is_once, typename T, typename StoredType = std::decay_t<T>,
    typename ForwardedType = std::conditional_t<is_once, StoredType&&, const StoredType&>>
using TransformToUnwrappedType = decltype(Unwrap(std::declval<ForwardedType>()));
...
template <typename T> struct ValidateReceiverType {
private:
    // Pointer-like receivers use a different specialization, so this never
    // succeeds.
    template <bool v = AlwaysFalse<T>> struct ReceiverMustBePointerLike {
        static constexpr bool value = [] {
            static_assert(v,
...
// pointers.
template <bool is_once, bool is_method, typename... Args> struct ValidateUnwrappedTypeList {
    // These members are similar in intent to those in `StorageTraits`; see
    // comments there.
...
template <bool is_once, typename Receiver, typename... Args> struct ValidateUnwrappedTypeList<is_once, true, Receiver, Args...> {
private:
    using ReceiverStorageType = typename MethodReceiverStorage<std::decay_t<Receiver>>::Type;
    using UnwrappedReceiver = TransformToUnwrappedType<is_once, ReceiverStorageType>;
    using ValidatedReceiver = ValidateReceiverType<UnwrappedReceiver>;

public:
    using Type = TypeList<typename ValidatedReceiver::Type, TransformToUnwrappedType<is_once, Args>...>;
...
template <typename StorageType> inline constexpr bool IsUnretainedMayDangle = false;

template <typename T, RawPtrTraits PtrTraits> inline constexpr bool IsUnretainedMayDangle<UnretainedWrapper<T, unretained_traits::MayDangle, PtrTraits>> = true;
...
template <typename StorageType, typename FunctionParamType> inline constexpr bool UnretainedAndRawPtrHaveCompatibleTraits = false;

template <typename T, RawPtrTraits PtrTraitsInUnretained, RawPtrTraits PtrTraitsInReceiver>
inline constexpr bool
    UnretainedAndRawPtrHaveCompatibleTraits<UnretainedWrapper<T, unretained_traits::MayDangle, PtrTraitsInUnretained>, raw_ptr<T, PtrTraitsInReceiver>>
    = std::same_as<typename UnretainedWrapper<T, unretained_traits::MayDangle, PtrTraitsInUnretained>::GetPtrType, raw_ptr<T, PtrTraitsInReceiver>>;
...
template <int i> struct BindArgument {
    template <typename ForwardingType> struct ForwardedAs {
        template <typename FunctorParamType> struct ToParamWithType {
            ...
            static constexpr bool kRawPtrMayBeDangling = IsRawPtrMayDangleV<FunctorParamType>;
            static constexpr bool kCanBeForwardedToBoundFunctor = std::is_convertible_v<ForwardingType, FunctorParamType>;
            ...
            static constexpr bool kIsUnwrappedForwardableNonConstReference = std::is_lvalue_reference_v<FunctorParamType>
                && !std::is_const_v<std::remove_reference_t<FunctorParamType>> && std::is_convertible_v<std::decay_t<ForwardingType>&, FunctorParamType>;
            ...
            // been used.
            static constexpr bool kWouldBeForwardableWithPassed = std::is_convertible_v<std::decay_t<ForwardingType>&&, FunctorParamType>;
        };
    };

    template <typename BoundAsType> struct BoundAs {
        template <typename StorageType> struct StoredAs {
            static constexpr bool kBindArgumentCanBeCaptured = std::constructible_from<StorageType, BoundAsType>;
            ...
            static constexpr bool kWouldBeCapturableWithStdMove = std::constructible_from<StorageType, std::decay_t<BoundAsType>&&>;
        };
    };

    template <typename FunctionParamType> struct ToParamWithType {
        template <typename StorageType> struct StoredAs {
            static constexpr bool kBoundPtrMayDangle = IsUnretainedMayDangle<StorageType>;

            static constexpr bool kMayDangleAndMayBeDanglingHaveMatchingTraits = UnretainedAndRawPtrHaveCompatibleTraits<StorageType, FunctionParamType>;
        };
    };
};
...
template <int ii, bool is_method, typename Arg, typename Storage, typename Unwrapped, typename Param> struct ParamCanBeBound {
private:
    using UnwrappedParam = typename BindArgument<ii>::template ForwardedAs<Unwrapped>::template ToParamWithType<Param>;
    using ParamStorage = typename BindArgument<ii>::template ToParamWithType<Param>::template StoredAs<Storage>;
    using BoundStorage = typename BindArgument<ii>::template BoundAs<Arg>::template StoredAs<Storage>;

    template <bool v = !UnwrappedParam::kRawPtr || UnwrappedParam::kRawPtrMayBeDangling> struct NotRawPtr {
        static constexpr bool value = [] {
            static_assert(v,
                "Use T* or T& instead of raw_ptr<T> for function parameters, "
                "unless you must mark the parameter as MayBeDangling<T>.");
            return v;
        }();
    };

    template <bool v = !ParamStorage::kBoundPtrMayDangle || UnwrappedParam::kRawPtrMayBeDangling ||
    ...
    struct MayBeDanglingPtrPassedCorrectly {
        static constexpr bool value = [] {
            static_assert(v,
                "base::UnsafeDangling() pointers should only be passed "
    ...
    template <bool v
        = !UnwrappedParam::kRawPtrMayBeDangling || (ParamStorage::kBoundPtrMayDangle && ParamStorage::kMayDangleAndMayBeDanglingHaveMatchingTraits)>
    struct MayDangleAndMayBeDanglingHaveMatchingTraits {
        static constexpr bool value = [] {
            static_assert(v,
                "Pointers passed to MayBeDangling<T> parameters must be created "
    ...
    // migrated.
    template <bool v = UnwrappedParam::kCanBeForwardedToBoundFunctor || !UnwrappedParam::kWouldBeForwardableWithPassed> struct MoveOnlyTypeMustUseBasePassed {
        static constexpr bool value = [] {
            static_assert(v,
    ...
    template <bool v = UnwrappedParam::kCanBeForwardedToBoundFunctor || !UnwrappedParam::kIsUnwrappedForwardableNonConstReference>
    ...
    template <bool v = UnwrappedParam::kCanBeForwardedToBoundFunctor> struct CanBeForwardedToBoundFunctor {
        static constexpr bool value = [] {
            static_assert(v,
    ...
    template <bool v = BoundStorage::kBindArgumentCanBeCaptured || !BoundStorage::kWouldBeCapturableWithStdMove> struct MoveOnlyTypeMustUseStdMove {
        static constexpr bool value = [] {
            static_assert(v,
    ...
    template <bool v = BoundStorage::kBindArgumentCanBeCaptured> struct BindArgumentCanBeCaptured {
        static constexpr bool value = [] {
            // In practice, failing this precondition should be rare, as the storage
            // type is deduced from the arguments passed to `Bind()`.
            static_assert(v, "Cannot capture argument: is the argument copyable or movable?");
            return v;
        }();
    };
...
public:
    static constexpr bool value
        = std::conjunction_v<NotRawPtr<>, MayBeDanglingPtrPassedCorrectly<>, MayDangleAndMayBeDanglingHaveMatchingTraits<>, MoveOnlyTypeMustUseBasePassed<>,
            NonConstRefParamMustBeWrapped<>, CanBeForwardedToBoundFunctor<>, MoveOnlyTypeMustUseStdMove<>, BindArgumentCanBeCaptured<>>;
};

// Takes three same-length `TypeList`s, and checks `ParamCanBeBound` for each
// triple.
template <bool is_method, typename Index, typename Args, typename UnwrappedTypeList, typename ParamsList> struct ParamsCanBeBound {
    static constexpr bool value = false;
};

template <bool is_method, size_t... Ns, typename... Args, typename... UnwrappedTypes, typename... Params>
struct ParamsCanBeBound<is_method, std::index_sequence<Ns...>, TypeList<Args...>, TypeList<UnwrappedTypes...>, TypeList<Params...>> {
    static constexpr bool value = std::conjunction_v<ParamCanBeBound<Ns, is_method, Args, std::decay_t<Args>, UnwrappedTypes, Params>...>;
};

// Core implementation of `Bind()`, which checks common preconditions before
// returning an appropriate callback.
template <template <typename> class CallbackT> struct BindHelper {
private:
    static constexpr bool kIsOnce = is_instantiation<OnceCallback, CallbackT<void()>>;

    template <typename Traits, bool v = IsComplete<Traits>> struct TraitsAreInstantiable {
        static constexpr bool value = [] {
            static_assert(v,
                "Could not determine how to invoke functor. If this functor has "
    ...
    template <typename Functor,
        bool v = !is_instantiation<OnceCallback, std::decay_t<Functor>>
            || (kIsOnce && std::is_rvalue_reference_v<Functor&&> && !std::is_const_v<std::remove_reference_t<Functor>>)>
    struct OnceCallbackFunctorIsValid {
        static constexpr bool value = [] {
            if constexpr (kIsOnce) {
                ...
                static_assert(v,
                    "BindRepeating() cannot bind OnceCallback. Use "
    ...
    template <typename... Args> struct NoBindArgToOnceCallbackIsBasePassed {
        static constexpr bool value = [] {
            // Can't use a defaulted template param since it can't come after `Args`.
            constexpr bool v = !kIsOnce || (... && !is_instantiation<PassedWrapper, std::decay_t<Args>>);
            static_assert(v, "Use std::move() instead of base::Passed() with base::BindOnce().");
            return v;
        }();
    };

    template <typename Functor,
        bool v = !is_instantiation<FunctionRef, std::remove_cvref_t<Functor>> && !is_instantiation<absl::FunctionRef, std::remove_cvref_t<Functor>>>
    ...
    template <typename Traits, bool v = Traits::is_stateless> struct IsStateless {
        static constexpr bool value = [] {
            static_assert(v,
                "Capturing lambdas and stateful functors are intentionally not "
    ...
    template <typename Functor, typename... Args> static auto BindImpl(Functor&& functor, Args&&... args)
    {
        ...
        //     ...
        // ```
        using Traits = FunctorTraits<TransformToUnwrappedType<kIsOnce, Functor&&>, TransformToUnwrappedType<kIsOnce, Args&&>...>;
        if constexpr (TraitsAreInstantiable<Traits>::value) {
            using ValidatedUnwrappedTypes = ValidateUnwrappedTypeList<kIsOnce, Traits::is_method, Args&&...>;
            ...
            using ValidatedBindState = ValidateBindStateType<Traits::is_method, Traits::is_nullable, Traits::is_callback, Functor, Args...>;
            ...
            // readable.
            if constexpr (std::conjunction_v<NotFunctionRef<Functor>, IsStateless<Traits>, ValidatedUnwrappedTypes,
                              ParamsCanBeBound<Traits::is_method, std::make_index_sequence<sizeof...(Args)>, BoundArgsList,
                                  typename ValidatedUnwrappedTypes::Type, BoundParamsList>,
                              ValidatedBindState>) {
                ...
                using Invoker = Invoker<Traits, typename ValidatedBindState::Type, UnboundRunType>;
                if constexpr (kIsOnce) {
                ...
                return CallbackType(ValidatedBindState::Type::Create(
                    reinterpret_cast<BindStateBase::InvokeFuncStorage>(invoke_func), std::forward<Functor>(functor), std::forward<Args>(args)...));
            }
        }
    }
    ...
    static OnceCallback<Signature> BindImpl(RepeatingCallback<Signature> callback)
    {
        return BindImpl(OnceCallback<Signature>(callback));
    }

    // Must be defined after `BindImpl()` since it refers to it.
    template <typename Functor, typename... Args> struct BindImplWouldSucceed {
        // Can't use a defaulted template param since it can't come after `Args`.
        //
        ...
        // type.
        static constexpr bool value = !std::same_as<void, decltype(BindImpl(std::declval<Functor&&>(), std::declval<Args&&>()...))>;
    };

public:
    template <typename Functor, typename... Args> static auto Bind(Functor&& functor, Args&&... args)
    {
        if constexpr (std::conjunction_v<OnceCallbackFunctorIsValid<Functor>, NoBindArgToOnceCallbackIsBasePassed<Args...>,
...
// ```
template <typename T> struct IsWeakReceiver : std::bool_constant<is_instantiation<WeakPtr, T>> { };

template <typename T> struct IsWeakReceiver<std::reference_wrapper<T>> : IsWeakReceiver<T> { };
...
template <typename> struct BindUnwrapTraits {
    template <typename T> static T&& Unwrap(T&& o)
    {
...
template <typename T>
    requires internal::kIsUnretainedWrapper<internal::UnretainedWrapper, T> || internal::kIsUnretainedWrapper<internal::UnretainedRefWrapper, T>
    || is_instantiation<internal::RetainedRefWrapper, T> || is_instantiation<internal::OwnedWrapper, T> || is_instantiation<internal::OwnedRefWrapper, T>
struct BindUnwrapTraits<T> {
    static decltype(auto) Unwrap(const T& o)
    {
        return o.get();
    }
};

template <typename T> struct BindUnwrapTraits<internal::PassedWrapper<T>> {
...
template <typename T> struct BindUnwrapTraits<Microsoft::WRL::ComPtr<T>> {
    static T* Unwrap(const Microsoft::WRL::ComPtr<T>& ptr)
    {
        return ptr.Get();
    }
};
...
template <typename Functor, typename... BoundArgs>
    requires internal::kIsWeakMethod<internal::FunctorTraits<Functor, BoundArgs...>::is_method, BoundArgs...>
struct CallbackCancellationTraits<Functor, std::tuple<BoundArgs...>> {
    static constexpr bool is_cancellable = true;

    template <typename Receiver, typename... Args> static bool IsCancelled(const Functor&, const Receiver& receiver, const Args&...)
    {
        return !receiver;
    }

    template <typename Receiver, typename... Args> static bool MaybeValid(const Functor&, const Receiver& receiver, const Args&...)
    {
...
template <typename Functor, typename... BoundArgs>
    requires is_instantiation<OnceCallback, Functor> || is_instantiation<RepeatingCallback, Functor>
struct CallbackCancellationTraits<Functor, std::tuple<BoundArgs...>> {
    static constexpr bool is_cancellable = true;
