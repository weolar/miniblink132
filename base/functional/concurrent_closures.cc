...
void ConcurrentClosures::Done(OnceClosure done_closure, const Location& location) &&
{
    CHECK(info_);
    DCHECK_CALLED_ON_VALID_SEQUENCE(info_->sequence_checker_);
    info_->done_closure_ = BindPostTask(SequencedTaskRunner::GetCurrentDefault(), std::move(done_closure), location);
...
void ConcurrentClosures::Info::Run()
{
    ...
    if (done_closure_ && pending_ == 0u) {
        SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(done_closure_));
    }
}
