...
template <class ObserverType> struct Pick {
...
template <> struct Pick<UnsafeObserver> {
    using ObserverListType = ObserverList<ObserverInterface>::Unchecked;
    static const char* GetName()
    {
        return "UnsafeObserver";
    }
};

template <class ObserverType> class ObserverListPerfTest : public ::testing::Test {
public:
...
TYPED_TEST(ObserverListPerfTest, NotifyPerformance)
{
    ...
    for (int observer_count = 0; observer_count <= kMaxObservers; observer_count = observer_count ? observer_count * 2 : 1) {
        ...
        EXPECT_EQ(observer_count * weighted_laps, g_observer_list_perf_test_counter);
        EXPECT_TRUE(observer_count == 0 || !list.empty());

        std::string story_name = base::StringPrintf("%s_%d", Pick<TypeParam>::GetName(), observer_count);
        ...
        reporter.AddResult(kMetricNotifyTimePerObserver, duration.InNanoseconds() / static_cast<double>(g_observer_list_perf_test_counter + weighted_laps));
    }
}
