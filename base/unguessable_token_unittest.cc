...
void TestSmallerThanOperator(const UnguessableToken& a, const UnguessableToken& b)
{
    EXPECT_TRUE(a < b);
    EXPECT_FALSE(b < a);
}
...
TEST(UnguessableTokenTest, VerifyEveryBit)
{
    ...
    for (uint64_t bit = 1; bit != 0; bit <<= 1) {
        ...
        std::optional<UnguessableToken> new_token = UnguessableToken::Deserialize(new_high, low);
    ...
    for (uint64_t bit = 1; bit != 0; bit <<= 1) {
        ...
        std::optional<UnguessableToken> new_token = UnguessableToken::Deserialize(high, new_low);
        ASSERT_TRUE(new_token.has_value());
        EXPECT_FALSE(*token == *new_token);
    }
}
...
TEST(UnguessableTokenTest, VerifySerialization)
{
    ...
    std::optional<UnguessableToken> Deserialized = UnguessableToken::Deserialize(high, low);
...
{
    UnguessableToken token = UnguessableToken::Deserialize(0x1234567890ABCDEF, 0xFEDCBA0987654321).value();
...
{
    const UnguessableToken token1 = UnguessableToken::Deserialize(0x0000000012345678, 0x0000000123456789).value();
    const UnguessableToken token2 = UnguessableToken::Deserialize(0x0000000123456781, 0x0000000023456789).value();
...
TEST(UnguessableTokenTest, VerifyDeserializeFromString)
{
    ...
    auto actual = UnguessableToken::DeserializeFromString("00000000000000010000000000000002");
...
{
    const char* invalid_representations[] = { // Not a hex string representing 128 bits.
        "1234",
    ...
    for (auto* invalid_representation : invalid_representations) {
        auto actual = UnguessableToken::DeserializeFromString(invalid_representation);
        EXPECT_FALSE(actual.has_value()) << "'" << invalid_representation << "' should not be deserialized to an UnguessableToken.";
        ;
    }
}
...
TEST(UnguessableTokenTest, VerifySmallerThanOperator)
{
    ...
    // Use UnguessableToken::Create() in production code instead.
    {
        ...
        TestSmallerThanOperator(UnguessableToken::Deserialize(0, 1).value(), UnguessableToken::Deserialize(0, 5).value());
    }
    {
        SCOPED_TRACE("a.low == b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 0).value(), UnguessableToken::Deserialize(5, 0).value());
    }
    {
        SCOPED_TRACE("a.low < b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 1).value(), UnguessableToken::Deserialize(5, 5).value());
    }
    {
        SCOPED_TRACE("a.low > b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 10).value(), UnguessableToken::Deserialize(10, 1).value());
    }
}
...
TEST(UnguessableTokenTest, VerifyHash)
{
    ...
    EXPECT_EQ(base::HashInts64(token.GetHighForSerialization(), token.GetLowForSerialization()), UnguessableTokenHash()(token));
}
