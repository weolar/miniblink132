...
BASE_EXPORT zx::result<> Connect(fidl::ServerEnd<Protocol> server_end, std::string name = fidl::DiscoverableProtocolName<Protocol>)
{
    return component::ConnectAt<Protocol>(base::BorrowIncomingServiceDirectoryForProcess(), std::move(server_end), name);
}

template <typename Protocol, typename = std::enable_if_t<fidl::IsProtocolV<Protocol>>>
BASE_EXPORT zx::result<fidl::ClientEnd<Protocol>> Connect(std::string name = fidl::DiscoverableProtocolName<Protocol>)
{
    return component::ConnectAt<Protocol>(base::BorrowIncomingServiceDirectoryForProcess(), name);
}

template <typename Protocol, typename = std::enable_if_t<fidl::IsProtocolV<Protocol>>>
BASE_EXPORT zx::result<fidl::ClientEnd<Protocol>> ConnectAt(
    fidl::UnownedClientEnd<fuchsia_io::Directory> service_directory, std::string name = fidl::DiscoverableProtocolName<Protocol>)
{
