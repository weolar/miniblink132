...
protected:
        ...
        : filtered_service_directory_(test_context_.published_services())
    {
        ...
        EXPECT_EQ(filtered_service_directory_.ConnectClient(directory.NewRequest()), ZX_OK);
        filtered_client_ = std::make_shared<sys::ServiceDirectory>(std::move(directory));
    }
...
{
    ScopedServiceBinding<testfidl::TestInterface> publish_test_service(ComponentContextForProcess()->outgoing().get(), &test_service_);

    ASSERT_EQ(filtered_service_directory_.AddService(testfidl::TestInterface::Name_), ZX_OK);
...
TEST_F(FilteredServiceDirectoryTest, NoServiceDir)
{
    ...
    fidl::InterfaceRequest<fuchsia::io::Directory> directory_request(directory_handle.NewRequest());
    auto service_directory = std::make_shared<sys::ServiceDirectory>(std::move(directory_handle));
    ...
    ASSERT_EQ(filtered_directory.AddService(testfidl::TestInterface::Name_), ZX_OK);
...
{
    ScopedServiceBinding<testfidl::TestInterface> binding(filtered_service_directory_.outgoing_directory(), &test_service_);
