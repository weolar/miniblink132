...
public:
    using ProfileChangeCallback = base::RepeatingCallback<void(const ::fuchsia::intl::Profile&)>;
    ...
    FuchsiaIntlProfileWatcher& operator=(const FuchsiaIntlProfileWatcher&) = delete;
    ...
    static std::string GetPrimaryTimeZoneIdFromProfile(const ::fuchsia::intl::Profile& profile);
    ...
    // Returns an empty string if the ID cannot be obtained.
    static std::string GetPrimaryLocaleIdFromProfile(const ::fuchsia::intl::Profile& profile);
...
private:
    ...
    FuchsiaIntlProfileWatcher(::fuchsia::intl::PropertyProviderPtr property_provider, ProfileChangeCallback on_profile_changed);
