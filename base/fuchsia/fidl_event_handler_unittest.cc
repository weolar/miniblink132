...
public:
    ...
    {
        test_context_.AddService(fidl::DiscoverableProtocolName<fuchsia_logger::Log>);
...
protected:
    test::SingleThreadTaskEnvironment task_environment_ { test::SingleThreadTaskEnvironment::MainThreadType::IO };
...
TEST_F(FidlEventHandlerTest, FidlErrorEventLogger)
{
    ...
    constexpr char kLogMessage[] = "base.testfidl.TestInterface was disconnected with ZX_ERR_PEER_CLOSED";
    std::optional<fuchsia_logger::LogMessage> logged_message = listener_.RunUntilMessageReceived(kLogMessage);

    ASSERT_TRUE(logged_message.has_value());
    EXPECT_EQ(logged_message->severity(), static_cast<int32_t>(fuchsia_logger::LogLevelFilter::kError));
...
{
    FidlErrorEventLogger<base_testfidl::TestInterface> event_handler("test_protocol_name");

    event_handler.on_fidl_error(fidl::UnbindInfo::PeerClosed(ZX_ERR_PEER_CLOSED));

    constexpr char kLogMessage[] = "test_protocol_name was disconnected with ZX_ERR_PEER_CLOSED";
    std::optional<fuchsia_logger::LogMessage> logged_message = listener_.RunUntilMessageReceived(kLogMessage);
...
{
    FidlErrorEventLogger<base_testfidl::TestInterface> event_handler;
    auto client_end = fuchsia_component::ConnectAt<base_testfidl::TestInterface>(test_context_.published_services_natural());
    EXPECT_TRUE(client_end.is_ok());
    fidl::Client client(std::move(*client_end), async_get_default_dispatcher(), &event_handler);

    {
        ScopedNaturalServiceBinding<base_testfidl::TestInterface> binding(ComponentContextForProcess()->outgoing().get(), &test_service_);
...
TEST(FidlEventHandlerDeathTest, FidlErrorEventProcessExiter)
{
    ...
    EXPECT_DEATH(event_handler.on_fidl_error(fidl::UnbindInfo::PeerClosed(ZX_ERR_PEER_CLOSED)),
        testing::HasSubstr("base.testfidl.TestInterface disconnected "
                           "unexpectedly, exiting: ZX_ERR_PEER_CLOSED (-24)"));
}

TEST(FidlEventHandlerDeathTest, FidlErrorEventProcessExiter_CustomProtocolName)
{
    FidlErrorEventProcessExiter<base_testfidl::TestInterface> event_handler("test_protocol_name");
...
TEST(FidlEventHandlerDeathTest, FidlErrorEventProcessExiter_LogsOnServiceClosure)
{
    test::SingleThreadTaskEnvironment task_environment_ { test::SingleThreadTaskEnvironment::MainThreadType::IO };
    TestComponentContextForProcess test_context;
    FidlErrorEventProcessExiter<base_testfidl::TestInterface> event_handler;
    auto client_end = fuchsia_component::ConnectAt<base_testfidl::TestInterface>(test_context.published_services_natural());
    EXPECT_TRUE(client_end.is_ok());
    fidl::Client client(std::move(*client_end), async_get_default_dispatcher(), &event_handler);
    ...
    auto bind_and_close_service = [&] {
        {
            ...
            ScopedNaturalServiceBinding<base_testfidl::TestInterface> binding(ComponentContextForProcess()->outgoing().get(), &test_service);
    ...
    EXPECT_DEATH(bind_and_close_service(),
        testing::HasSubstr("base.testfidl.TestInterface disconnected "
                           "unexpectedly, exiting: ZX_ERR_PEER_CLOSED (-24)"));
}
...
{
    RunLoop loop;
    FidlErrorEventHandler<base_testfidl::TestInterface> event_handler(base::BindLambdaForTesting([quit_closure = loop.QuitClosure()](fidl::UnbindInfo error) {
