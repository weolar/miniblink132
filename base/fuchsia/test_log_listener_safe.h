...
// created.
class TestLogListenerSafe final : public fidl::Server<fuchsia_logger::LogListenerSafe> {
public:
    using OnLogMessageCallback = base::RepeatingCallback<void(const fuchsia_logger::LogMessage&)>;
...
public:
    ...
    void ListenToLog(const fidl::Client<fuchsia_logger::Log>& log, std::unique_ptr<fuchsia_logger::LogFilterOptions> options);
    ...
    std::optional<fuchsia_logger::LogMessage> RunUntilMessageReceived(std::string_view expected_string);
