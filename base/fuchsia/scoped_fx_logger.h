...
public:
    ...
    static ScopedFxLogger CreateForProcess(std::vector<std::string_view> tags = {});

    // Returns an instance connected to the specified LogSink.
    static ScopedFxLogger CreateFromLogSink(fidl::ClientEnd<fuchsia_logger::LogSink> client_end, std::vector<std::string_view> tags = {});

    void LogMessage(std::string_view file, uint32_t line_number, std::string_view msg, logging::LogSeverity severity);
