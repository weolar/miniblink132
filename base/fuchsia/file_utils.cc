...
fidl::InterfaceHandle<::fuchsia::io::Directory> OpenDirectoryHandleInternal(const base::FilePath& path, bool read_only)
{
    ScopedFD fd(open(path.value().c_str(), O_DIRECTORY | (read_only ? O_RDONLY : O_RDWR)));
    if (!fd.is_valid()) {
...
fidl::InterfaceHandle<::fuchsia::io::Directory> OpenDirectoryHandle(const base::FilePath& path)
{
    return OpenDirectoryHandleInternal(path, true);
}

fidl::InterfaceHandle<::fuchsia::io::Directory> OpenWritableDirectoryHandle(const base::FilePath& path)
{
