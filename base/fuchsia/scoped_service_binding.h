...
template <typename Interface> class BASE_EXPORT ScopedServiceBinding {
public:
    ...
    ScopedServiceBinding(sys::OutgoingDirectory* outgoing_directory, Interface* impl, std::string_view name = Interface::Name_)
    ...
    ScopedServiceBinding(vfs::PseudoDir* pseudo_dir, Interface* impl, std::string_view name = Interface::Name_)
    ...
    {
        bindings_.set_empty_set_handler([callback = std::move(on_last_client_callback)] { callback.Run(); });
    }
...
template <typename Protocol> class BASE_EXPORT ScopedNaturalServiceBinding {
public:
    // Publishes a public service in the specified |outgoing_directory|.
    // |outgoing_directory| and |impl| must outlive the binding. The service is
    // unpublished on destruction.
    ScopedNaturalServiceBinding(
        sys::OutgoingDirectory* outgoing_directory, fidl::Server<Protocol>* impl, std::string_view name = fidl::DiscoverableProtocolName<Protocol>)
        : publisher_(outgoing_directory,
            bindings_.CreateHandler(impl,
                ...
                async_get_default_dispatcher(), [](fidl::UnbindInfo info) {}),
            name)
    {
    }
    ...
    // Publishes a service in the specified |pseudo_dir|. |pseudo_dir| and |impl|
    // must outlive the binding. The service is unpublished on destruction.
    ScopedNaturalServiceBinding(vfs::PseudoDir* pseudo_dir, fidl::Server<Protocol>* impl, std::string_view name = fidl::DiscoverableProtocolName<Protocol>)
    ...
    ScopedNaturalServiceBinding& operator=(const ScopedNaturalServiceBinding&) = delete;
...
enum class ScopedServiceBindingPolicy { kPreferNew, kPreferExisting, kConnectOnce };

template <typename Interface, ScopedServiceBindingPolicy Policy = ScopedServiceBindingPolicy::kPreferNew> class BASE_EXPORT ScopedSingleClientServiceBinding {
public:
    // |outgoing_directory| and |impl| must outlive the binding.
    ScopedSingleClientServiceBinding(sys::OutgoingDirectory* outgoing_directory, Interface* impl, std::string_view name = Interface::Name_)
        : binding_(impl)
    {
        publisher_.emplace(outgoing_directory, fit::bind_member(this, &ScopedSingleClientServiceBinding::BindClient), name);
        binding_.set_error_handler(fit::bind_member(this, &ScopedSingleClientServiceBinding::OnBindingEmpty));
    }

    ScopedSingleClientServiceBinding(vfs::PseudoDir* publish_to, Interface* impl, std::string_view name = Interface::Name_)
        : binding_(impl)
    {
        publisher_.emplace(publish_to, fit::bind_member(this, &ScopedSingleClientServiceBinding::BindClient), name);
        binding_.set_error_handler(fit::bind_member(this, &ScopedSingleClientServiceBinding::OnBindingEmpty));
    }

    ScopedSingleClientServiceBinding(const ScopedSingleClientServiceBinding&) = delete;
    ScopedSingleClientServiceBinding& operator=(const ScopedSingleClientServiceBinding&) = delete;

    ~ScopedSingleClientServiceBinding() = default;

    typename Interface::EventSender_& events()
    {
        return binding_.events();
    }
...
private:
    ...
    {
        if (Policy == ScopedServiceBindingPolicy::kPreferExisting && binding_.is_bound()) {
            return;
        }
