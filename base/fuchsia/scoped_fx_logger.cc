...
inline FuchsiaLogSeverity LogSeverityToFuchsiaLogSeverity(logging::LogSeverity severity)
{
    switch (severity) {
...
// static
ScopedFxLogger ScopedFxLogger::CreateForProcess(std::vector<std::string_view> tags)
{
    ...
    std::string program_name = base::CommandLine::ForCurrentProcess()->GetProgram().BaseName().AsUTF8Unsafe();
    tags.insert(tags.begin(), program_name);

    return CreateFromLogSink(std::move(log_sink_client_end.value()), std::move(tags));
}

// static
ScopedFxLogger ScopedFxLogger::CreateFromLogSink(fidl::ClientEnd<fuchsia_logger::LogSink> log_sink_client_end, std::vector<std::string_view> tags)
{
    // CHECK()ing or LOG()ing inside this function is safe, since it is only
    // called to initialize logging, not during individual logging operations.
    ...
    zx_status_t socket_status = zx::socket::create(ZX_SOCKET_DATAGRAM, &local, &remote);
    ZX_CHECK(socket_status == ZX_OK, socket_status) << "zx_socket_create() failed";
    ...
    auto connect_structured_result = log_sink->ConnectStructured(std::move(remote));
    if (connect_structured_result.is_error()) {
        ZX_LOG(ERROR, connect_structured_result.error_value().status()) << "ConnectStructured() failed";
        return {};
    }
...
void ScopedFxLogger::LogMessage(std::string_view file, uint32_t line_number, std::string_view msg, logging::LogSeverity severity)
{
    if (!socket_.is_valid())
        return;
    ...
    buffer.BeginRecord(fuchsia_severity, cpp17::string_view(file.data(), file.size()), line_number, cpp17::string_view(msg.data(), msg.size()),
        socket_.borrow(), 0, base::Process::Current().Pid(), base::PlatformThread::CurrentId());
...
ScopedFxLogger::ScopedFxLogger(std::vector<std::string_view> tags, zx::socket socket)
