...
StartupContext::StartupContext(fuchsia::component::runner::ComponentStartInfo start_info)
{
    ...
    for (auto& entry : namespace_entries) {
        ...
        if (entry.path() == kServiceDirectoryPath) {
            incoming_services = std::make_unique<sys::ServiceDirectory>(std::move(*entry.mutable_directory()));
            break;
        }
    }
    ...
    component_context_ = std::make_unique<sys::ComponentContext>(std::move(incoming_services));
