...
protected:
    ...
    const base::test::SingleThreadTaskEnvironment task_environment_ { base::test::SingleThreadTaskEnvironment::MainThreadType::IO };
...
{
    ScopedServiceBinding<testfidl::TestInterface> binding(ComponentContextForProcess()->outgoing().get(), &test_service_);

    auto stub = test_context_.published_services()->Connect<testfidl::TestInterface>();
    auto stub2 = test_context_.published_services()->Connect<testfidl::TestInterface>();
...
TEST_F(ScopedServiceBindingTest, ConnectTwiceNewName)
{
    ...
    ScopedServiceBinding<testfidl::TestInterface> new_service_binding(ComponentContextForProcess()->outgoing().get(), &test_service_, kInterfaceName);

    testfidl::TestInterfacePtr stub, stub2;
    test_context_.published_services()->Connect(kInterfaceName, stub.NewRequest().TakeChannel());
    test_context_.published_services()->Connect(kInterfaceName, stub2.NewRequest().TakeChannel());
    EXPECT_EQ(VerifyTestInterface(stub), ZX_OK);
    EXPECT_EQ(VerifyTestInterface(stub2), ZX_OK);
}
...
{
    vfs::PseudoDir* const debug_dir = ComponentContextForProcess()->outgoing()->debug_dir();

    // Publish the test service to the "debug" directory.
    ScopedServiceBinding<testfidl::TestInterface> debug_service_binding(debug_dir, &test_service_);

    // Connect a ServiceDirectory to the "debug" subdirectory.
    fidl::InterfaceHandle<fuchsia::io::Directory> debug_handle;
    debug_dir->Serve(fuchsia::io::OpenFlags::RIGHT_READABLE | fuchsia::io::OpenFlags::RIGHT_WRITABLE, debug_handle.NewRequest().TakeChannel());
    ...
    auto release_stub = test_context_.published_services()->Connect<testfidl::TestInterface>();
...
TEST_F(ScopedServiceBindingTest, SingleClientConnectNewName)
{
    ...
    ScopedSingleClientServiceBinding<testfidl::TestInterface> binding(ComponentContextForProcess()->outgoing().get(), &test_service_, kInterfaceName);
...
{
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kPreferNew> binding(
        ComponentContextForProcess()->outgoing().get(), &test_service_);

    // Connect the first client, and verify that it is functional.
    auto existing_client = test_context_.published_services()->Connect<testfidl::TestInterface>();
    ...
    auto new_client = test_context_.published_services()->Connect<testfidl::TestInterface>();
...
{
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kPreferExisting> binding(
        ComponentContextForProcess()->outgoing().get(), &test_service_);
...
{
    ScopedSingleClientServiceBinding<testfidl::TestInterface> binding(ComponentContextForProcess()->outgoing().get(), &test_service_);
...
TEST_F(ScopedServiceBindingTest, SingleClientPublishToPseudoDir)
{
    ...
    ScopedSingleClientServiceBinding<testfidl::TestInterface> binding(debug_dir, &test_service_);
...
{
    ScopedSingleClientServiceBinding<testfidl::TestInterface> single_service_binding(ComponentContextForProcess()->outgoing().get(), &test_service_);

    base::RunLoop run_loop;
    single_service_binding.SetOnLastClientCallback(run_loop.QuitClosure());

    auto current_client = test_context_.published_services()->Connect<testfidl::TestInterface>();
...
{
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kConnectOnce> binding(
        ComponentContextForProcess()->outgoing().get(), &test_service_);
    ...
    new_client = test_context_.published_services()->Connect<testfidl::TestInterface>();
    RunLoop().RunUntilIdle();
    EXPECT_FALSE(new_client);
}
...
TEST_F(ScopedServiceBindingTest, MultipleLastClientCallback)
{
    ...
    binding.SetOnLastClientCallback(BindLambdaForTesting([&disconnect_count] { ++disconnect_count; }));
