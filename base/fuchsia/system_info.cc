...
bool FetchAndCacheBuildInfo()
{
    ...
    auto provider_client_end = fuchsia_component::Connect<fuchsia_buildinfo::Provider>();
...
{
    DCHECK(!CachedBuildInfo().IsEmpty()) << "FetchAndCacheSystemInfo() has not been called in this process";
...
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::WILL_BLOCK);
    auto product_client_end = fuchsia_component::Connect<fuchsia_hwinfo::Product>();
