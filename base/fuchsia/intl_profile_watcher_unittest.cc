...
template <typename FuchsiaStruct> void CopyIdsToFuchsiaStruct(const std::vector<std::string>& raw_ids, std::vector<FuchsiaStruct>* fuchsia_ids)
{
    fuchsia_ids->clear();
    for (auto id : raw_ids) {
...
fuchsia::intl::Profile CreateProfileWithTimeZones(const std::vector<std::string>& zone_ids)
{
...
fuchsia::intl::Profile CreateProfileWithLocales(const std::vector<std::string>& locale_ids)
{
    fuchsia::intl::Profile profile;
...
class FakePropertyProvider : public ::fuchsia::intl::testing::PropertyProvider_TestBase {
public:
    explicit FakePropertyProvider(fidl::InterfaceRequest<::fuchsia::intl::PropertyProvider> provider_request)
        : binding_(this)
    {
...
public:
    explicit FakePropertyProviderAsync(fidl::InterfaceRequest<::fuchsia::intl::PropertyProvider> provider_request)
        : thread_("Property Provider Thread")
    {
        CHECK(thread_.StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0)));
        property_provider_ = base::SequenceBound<FakePropertyProvider>(thread_.task_runner(), std::move(provider_request));
    }
    FakePropertyProviderAsync(const FakePropertyProviderAsync&) = delete;
    FakePropertyProviderAsync& operator=(const FakePropertyProviderAsync&) = delete;
    ~FakePropertyProviderAsync() = default;

    void Close()
    {
        property_provider_.AsyncCall(&FakePropertyProvider::Close);
    }
    void SetTimeZones(const std::vector<std::string>& zone_ids)
    {
        property_provider_.AsyncCall(&FakePropertyProvider::SetTimeZones).WithArgs(zone_ids);
    }
    void SetLocales(const std::vector<std::string>& locale_ids)
    {
        property_provider_.AsyncCall(&FakePropertyProvider::SetLocales).WithArgs(locale_ids);
    }
    void NotifyChange()
    {
...
public:
    ...
    GetValuesFromIntlPropertyProviderTest(const GetValuesFromIntlPropertyProviderTest&) = delete;
    GetValuesFromIntlPropertyProviderTest& operator=(const GetValuesFromIntlPropertyProviderTest&) = delete;
    ~GetValuesFromIntlPropertyProviderTest() override = default;

protected:
    std::string GetPrimaryLocaleId()
    {
        fuchsia::intl::Profile profile = GetProfileFromPropertyProvider(std::move(property_provider_ptr_));
    ...
    static fuchsia::intl::Profile GetProfileFromPropertyProvider(::fuchsia::intl::PropertyProviderSyncPtr property_provider)
    {
        return FuchsiaIntlProfileWatcher::GetProfileFromPropertyProvider(std::move(property_provider));
    }
...
protected:
    ...
    std::unique_ptr<FuchsiaIntlProfileWatcher> CreateIntlProfileWatcher(FuchsiaIntlProfileWatcher::ProfileChangeCallback on_profile_changed)
    {
        return base::WrapUnique(new FuchsiaIntlProfileWatcher(std::move(property_provider_ptr_), std::move(on_profile_changed)));
    }
...
TEST(IntlServiceNotAvailableTest, FuchsiaIntlProfileWatcher)
{
    ...
    base::MockCallback<FuchsiaIntlProfileWatcher::ProfileChangeCallback> on_profile_changed;
    EXPECT_CALL(on_profile_changed, Run(testing::_)).Times(0);
    auto watcher = std::make_unique<FuchsiaIntlProfileWatcher>(on_profile_changed.Get());
    EXPECT_TRUE(watcher);

    run_loop.RunUntilIdle();
}

TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryTimeZoneId_RemoteNotBound)
{
...
TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryTimeZoneId_SingleZoneIsEmpty)
{
    property_provider_.SetTimeZones({ "" });
    EXPECT_STREQ("", GetPrimaryTimeZoneId().c_str());
}

TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryTimeZoneId_MoreThanOneZone)
{
    property_provider_.SetTimeZones({ kPrimaryTimeZoneName, kSecondaryTimeZoneName });
    EXPECT_STREQ(kPrimaryTimeZoneName, GetPrimaryTimeZoneId().c_str());
}

TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryLocaleId_RemoteNotBound)
{
    // Simulate the service not actually being available.
    property_provider_.Close();
...
TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryLocaleId_SingleLocaleIsEmpty)
{
    property_provider_.SetLocales({ "" });
    EXPECT_STREQ("", GetPrimaryLocaleId().c_str());
}

TEST_F(GetValuesFromIntlPropertyProviderTest, GetPrimaryLocaleId_MoreThanOneLocale)
{
...
{
    auto watcher = CreateIntlProfileWatcher(base::BindLambdaForTesting([quit_loop = run_loop_.QuitClosure()](const fuchsia::intl::Profile& profile) {
        EXPECT_EQ(kPrimaryTimeZoneName, FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(profile));
        quit_loop.Run();
    }));
...
{
    EXPECT_EQ("", FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(fuchsia::intl::Profile()));
}

TEST(IntlProfileWatcherGetPrimaryTimeZoneIdFromProfileTest, EmptyZonesList)
{
    EXPECT_EQ("", FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(CreateProfileWithTimeZones({})));
}

TEST(IntlProfileWatcherGetPrimaryTimeZoneIdFromProfileTest, OneZone)
{
    EXPECT_EQ(kPrimaryTimeZoneName, FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(CreateProfileWithTimeZones({ kPrimaryTimeZoneName })));
}

TEST(IntlProfileWatcherGetPrimaryTimeZoneIdFromProfileTest, TwoZones)
{
    EXPECT_EQ(kPrimaryTimeZoneName,
        FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(CreateProfileWithTimeZones({ kPrimaryTimeZoneName, kSecondaryTimeZoneName })));
}

TEST(IntlProfileWatcherGetPrimaryLocaleIdFromProfileTest, NoLocales)
{
    EXPECT_EQ("", FuchsiaIntlProfileWatcher::GetPrimaryLocaleIdFromProfile(fuchsia::intl::Profile()));
}
...
{
    EXPECT_EQ(kPrimaryLocaleName, FuchsiaIntlProfileWatcher::GetPrimaryLocaleIdFromProfile(CreateProfileWithLocales({ kPrimaryLocaleName })));
}

TEST(IntlProfileWatcherGetPrimaryLocaleIdFromProfileTest, MultipleLocales)
{
    EXPECT_EQ(
        kPrimaryLocaleName, FuchsiaIntlProfileWatcher::GetPrimaryLocaleIdFromProfile(CreateProfileWithLocales({ kPrimaryLocaleName, kSecondaryLocaleName })));
}
