...
namespace fidl {
template <typename Protocol> class AsyncEventHandler;
}
...
template <typename Protocol> class FidlErrorEventLogger : public fidl::AsyncEventHandler<Protocol> {
public:
    explicit FidlErrorEventLogger(std::string protocol_name = fidl::DiscoverableProtocolName<Protocol>)
    ...
    {
        LOG(ERROR) << protocol_name_ << " was disconnected with " << error.status_string() << ".";
    }
...
// used with.
template <typename Protocol> class FidlErrorEventProcessExiter : public fidl::AsyncEventHandler<Protocol> {
public:
    explicit FidlErrorEventProcessExiter(std::string protocol_name = fidl::DiscoverableProtocolName<Protocol>)
        : protocol_name_(std::move(protocol_name))
    {
    }
...
// it is used with.
template <typename Protocol> class FidlErrorEventHandler : public fidl::AsyncEventHandler<Protocol> {
public:
