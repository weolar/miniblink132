    ...
    : on_stop_(std::move(on_stop))
{
    ...
    fidl::ServerEnd<fuchsia_process_lifecycle::Lifecycle> lifecycle_server_end(std::move(lifecycle_server_channel));
    binding_.emplace(async_get_default_dispatcher(), std::move(lifecycle_server_end), this, fidl::kIgnoreBindingClosure);
}
