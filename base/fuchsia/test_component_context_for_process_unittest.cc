...
public:
    ...
    {
        auto test_interface_ptr = ComponentContextForProcess()->svc()->Connect<testfidl::TestInterface>();
        return VerifyTestInterface(test_interface_ptr) == ZX_OK;
    }

    bool CanConnectToTestInterfaceServiceNatural()
    {
        auto client_end = fuchsia_component::Connect<base_testfidl::TestInterface>();
        EXPECT_TRUE(client_end.is_ok()) << client_end.status_string();
        fidl::Client client(std::move(client_end.value()), async_get_default_dispatcher());
        return VerifyTestInterface(client) == ZX_OK;
    }

    bool HasPublishedTestInterfaceHlcpp()
    {
        auto test_interface_ptr = test_context_.published_services()->Connect<testfidl::TestInterface>();
        return VerifyTestInterface(test_interface_ptr) == ZX_OK;
    }
...
TEST_F(TestComponentContextForProcessTest, InjectTestInterface)
{
    ...
    base::ScopedServiceBinding<testfidl::TestInterface> service_binding(test_context_.additional_services(), &test_interface_impl);
...
TEST_F(TestComponentContextForProcessTest, PublishTestInterface)
{
    ...
    base::ScopedServiceBinding<testfidl::TestInterface> service_binding(ComponentContextForProcess()->outgoing().get(), &test_interface_impl);
...
TEST_F(TestComponentContextForProcessTest, ProvideSystemService)
{
    ...
    // tests.
    const std::string_view kServiceNames[] = { ::fuchsia::buildinfo::Provider::Name_ };
    ...
    RunLoop wait_loop;
    auto provider = ComponentContextForProcess()->svc()->Connect<::fuchsia::buildinfo::Provider>();
    provider.set_error_handler([quit_loop = wait_loop.QuitClosure()](zx_status_t status) {
        ZX_LOG(ERROR, status);
        ADD_FAILURE();
        quit_loop.Run();
    });
    ...
    provider->GetBuildInfo([quit_loop = wait_loop.QuitClosure()](auto build_info) { quit_loop.Run(); });
    wait_loop.Run();
}
...
TEST_F(TestComponentContextForProcessTest, ProvideSystemServiceNatural)
{
    ...
    // of the ambient services in Fuchsia's hermetic environment for Chromium
    // tests.
    const std::string_view kServiceNames[] = { fidl::DiscoverableProtocolName<fuchsia_buildinfo::Provider> };
    test_context_.AddServices(kServiceNames);
    ...
    fidl::Client provider(std::move(client_end.value()), async_get_default_dispatcher());

    // If the BuildInfo service is actually connected then GetBuildInfo() will
    // return a result, otherwise the bindings will report an error.
    provider->GetBuildInfo().Then([quit_loop = wait_loop.QuitClosure()](auto build_info) {
