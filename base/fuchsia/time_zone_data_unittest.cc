...
TEST_F(TimeZoneDataTest, CompareSystemRevisionWithExpected)
{
    ...
    std::string expected;
    ASSERT_TRUE(base::ReadFileToString(base::FilePath(kTZDataRevisionFilePath), &expected)) << "Could not read from path: " << kTZDataRevisionFilePath;
    std::string actual;
...
TEST_F(TimeZoneDataDeathTest, CrashesWithNonexistentPath)
{
    ...
    EXPECT_DEATH(InitializeICU(), "Could not open directory: '/some/nonexistent/path'");
}
