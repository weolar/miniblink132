...
std::optional<std::u16string> ReadUTF8FromVMOAsUTF16(const fuchsia::mem::Buffer& buffer)
{
    std::optional<std::string> output_utf8 = StringFromMemBuffer(buffer);
    if (!output_utf8)
        return std::nullopt;
    std::u16string output;
    return UTF8ToUTF16(output_utf8->data(), output_utf8->size(), &output) ? std::optional<std::u16string>(std::move(output)) : std::nullopt;
}
...
fuchsia::mem::Buffer MemBufferFromString(std::string_view data, std::string_view name)
{
...
fuchsia::mem::Buffer MemBufferFromString16(std::u16string_view data, std::string_view name)
{
    return MemBufferFromString(std::string_view(reinterpret_cast<const char*>(data.data()), data.size() * sizeof(char16_t)), name);
}
...
std::optional<std::string> StringFromMemBuffer(const fuchsia::mem::Buffer& buffer)
{
    std::string result;
...
fuchsia::mem::Buffer MemBufferFromFile(File file)
{
    ...
    zx::vmo vmo;
    zx_status_t status = fdio_get_vmo_copy(file.GetPlatformFile(), vmo.reset_and_get_address());
    if (status != ZX_OK) {
...
fuchsia::mem::Buffer CloneBuffer(const fuchsia::mem::Buffer& buffer, std::string_view name)
{
    fuchsia::mem::Buffer output;
    output.size = buffer.size;
    zx_status_t status = buffer.vmo.create_child(ZX_VMO_CHILD_SNAPSHOT_AT_LEAST_ON_WRITE, 0, buffer.size, &output.vmo);
