...
public:
    FakeHardwareInfoProduct(std::string_view model, std::string_view manufacturer, sys::OutgoingDirectory* outgoing_services)
        : model_(model)
...
protected:
    ProductInfoTest()
        : task_environment_(base::test::SingleThreadTaskEnvironment::MainThreadType::IO)
        , thread_("ProductInfo Retrieval Thread")
    {
        ...
        component_context_.AddService(fidl::DiscoverableProtocolName<fuchsia_buildinfo::Provider>);
    }
    ~ProductInfoTest() override
    {
        ClearCachedSystemInfoForTesting();
    }
    ...
    fuchsia_hwinfo::ProductInfo GetProductInfoViaTask()
    {
        ...
        thread_.task_runner()->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&GetProductInfo),
            base::BindOnce(
                [](base::RunLoop& run_loop, fuchsia_hwinfo::ProductInfo& product_info, fuchsia_hwinfo::ProductInfo result) {
...
{
    FakeHardwareInfoProduct hwinfo_product_provider("test.model", "test.manufacturer", component_context_.additional_services());
...
{
    component_context_.AddService(fidl::DiscoverableProtocolName<fuchsia_hwinfo::Product>);
