...
TestComponentContextForProcess::TestComponentContextForProcess(InitialState initial_state)
{
    ...
    context_services_ = std::make_unique<FilteredServiceDirectory>(base::ComponentContextForProcess()->svc());
    ...
    if (initial_state == InitialState::kCloneAll) {
        ...
        base::FileEnumerator file_enum(base::FilePath("/svc"), /*recursive=*/false, base::FileEnumerator::NAMES_ONLY);
    ...
    zx_status_t status = context_services_->ConnectClient(incoming_directory.NewRequest());
    ZX_CHECK(status == ZX_OK, status) << "ConnectClient failed";
    auto incoming_services = std::make_shared<sys::ServiceDirectory>(std::move(incoming_directory));
    ...
    old_context_ = ReplaceComponentContextForProcessForTest(
        std::make_unique<sys::ComponentContext>(std::move(incoming_services), published_root_directory.NewRequest()));
    ...
    status = fdio_service_connect_at(published_root_directory.channel().get(), "svc", published_services.NewRequest().TakeChannel().release());
    ZX_CHECK(status == ZX_OK, status) << "fdio_service_connect_at() to /svc";
    published_services_ = std::make_shared<sys::ServiceDirectory>(std::move(published_services));
    published_services_natural_ = fidl::HLCPPToNatural(published_services_->CloneChannel());
}
...
void TestComponentContextForProcess::AddServices(base::span<const std::string_view> services)
{
    for (auto service : services)
        AddService(service);
}

fidl::UnownedClientEnd<fuchsia_io::Directory> TestComponentContextForProcess::published_services_natural()
{
