...
zx_status_t VerifyTestInterface(fidl::InterfacePtr<testfidl::TestInterface>& ptr)
{
    ...
    ptr.set_error_handler([quit = run_loop.QuitClosure(), weak_result = weak_result.GetWeakPtr()](zx_status_t status) {
        if (weak_result)
            *weak_result = status;
        std::move(quit).Run();
    });

    ptr->Add(2, 2, [quit = run_loop.QuitClosure(), weak_result = weak_result.GetWeakPtr()](int32_t value) {
        EXPECT_EQ(value, 4);
        if (weak_result)
            *weak_result = ZX_OK;
        std::move(quit).Run();
