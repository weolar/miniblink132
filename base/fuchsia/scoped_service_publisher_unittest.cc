...
TEST_F(ScopedServicePublisherTest, OutgoingDirectory)
{
...

    {
        ScopedServicePublisher<testfidl::TestInterface> publisher(
            base::ComponentContextForProcess()->outgoing().get(), test_service_.bindings().GetHandler(&test_service_));
        client_a = test_context_.published_services()->Connect<testfidl::TestInterface>();
    ...
    auto client_b = test_context_.published_services()->Connect<testfidl::TestInterface>();
...
TEST_F(ScopedServicePublisherTest, PseudoDir)
{
    ...
    directory.Serve(fuchsia::io::OpenFlags::RIGHT_READABLE | fuchsia::io::OpenFlags::RIGHT_WRITABLE, directory_handle.NewRequest().TakeChannel());
    sys::ServiceDirectory services(std::move(directory_handle));

    fidl::InterfacePtr<testfidl::TestInterface> client_a;

    {
        ScopedServicePublisher<testfidl::TestInterface> publisher(&directory, test_service_.bindings().GetHandler(&test_service_));
