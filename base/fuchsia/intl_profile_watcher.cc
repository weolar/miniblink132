...
FuchsiaIntlProfileWatcher::FuchsiaIntlProfileWatcher(ProfileChangeCallback on_profile_changed)
    : FuchsiaIntlProfileWatcher(ComponentContextForProcess()->svc()->Connect<::fuchsia::intl::PropertyProvider>(), on_profile_changed)
{
}

FuchsiaIntlProfileWatcher::FuchsiaIntlProfileWatcher(::fuchsia::intl::PropertyProviderPtr property_provider, ProfileChangeCallback on_profile_changed)
    ...
    , on_profile_changed_(std::move(on_profile_changed))
{
    ...
    property_provider_.events().OnChange = [this] { property_provider_->GetProfile([this](Profile profile) { on_profile_changed_.Run(profile); }); };
}

FuchsiaIntlProfileWatcher::~FuchsiaIntlProfileWatcher() = default;

// static
std::string FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdFromProfile(const Profile& profile)
{
    ...
    const std::vector<::fuchsia::intl::TimeZoneId>& time_zones = profile.time_zones();
    if (time_zones.empty()) {
...
// static
std::string FuchsiaIntlProfileWatcher::GetPrimaryTimeZoneIdForIcuInitialization()
{
    return GetPrimaryTimeZoneIdFromProfile(GetCurrentProfileSync());
}

// static
std::string FuchsiaIntlProfileWatcher::GetPrimaryLocaleIdFromProfile(const ::fuchsia::intl::Profile& profile)
{
    ...
    const std::vector<::fuchsia::intl::LocaleId>& locale_preferences = profile.locales();
...
// static
Profile FuchsiaIntlProfileWatcher::GetProfileFromPropertyProvider(::fuchsia::intl::PropertyProviderSyncPtr property_provider)
{
