...
std::unique_ptr<sys::ComponentContext>* GetComponentContextPtr()
{
    static base::NoDestructor<std::unique_ptr<sys::ComponentContext>> value(
        std::make_unique<sys::ComponentContext>(sys::ServiceDirectory::CreateFromNamespace()));
    return value.get();
}

fidl::ClientEnd<fuchsia_io::Directory>* GetIncomingServiceDirectory()
{
    static base::NoDestructor<fidl::ClientEnd<fuchsia_io::Directory>> value(fidl::HLCPPToNatural(GetComponentContextPtr()->get()->svc()->CloneChannel()));
    return value.get();
}
...
fidl::UnownedClientEnd<fuchsia_io::Directory> BorrowIncomingServiceDirectoryForProcess()
{
...
std::unique_ptr<sys::ComponentContext> ReplaceComponentContextForProcessForTest(std::unique_ptr<sys::ComponentContext> context)
{
    ...
    fidl::ClientEnd<fuchsia_io::Directory> incoming_service_directory(fidl::HLCPPToNatural(GetComponentContextPtr()->get()->svc()->CloneChannel()));
