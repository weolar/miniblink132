...
template <typename PointerType> struct PartialWinPeb {
...
template <typename PointerType> struct GdiTableEntry {
...
struct WowProcessTypes {
    ...
    using ReadVirtualMemoryFunc
        = NTSTATUS NTAPI(IN HANDLE ProcessHandle, IN uint64_t BaseAddress, OUT PVOID Buffer, IN ULONG64 Size, OUT PULONG64 NumberOfBytesRead);
    static const char* read_virtual_memory_func_name;
...
template <typename ProcessType> std::vector<GdiTableEntry<typename ProcessType::NativePointerType>> GetGdiTableEntries(const base::Process& process)
{
    ...
    static auto query_information_process_func
        = reinterpret_cast<typename ProcessType::QueryInformationProcessFunc*>(GetProcAddress(ntdll, ProcessType::query_information_process_name));
    ...
    NTSTATUS result = query_information_process_func(process.Handle(), ProcessBasicInformation, &basic_info, sizeof(basic_info), nullptr);
    if (result != 0) {
        LOG(ERROR) << ProcessType::query_information_process_name << " Failed " << std::hex << result;
        return GdiTableEntryVector();
    }

    static auto read_virtual_mem_func
        = reinterpret_cast<typename ProcessType::ReadVirtualMemoryFunc*>(GetProcAddress(ntdll, ProcessType::read_virtual_memory_func_name));
    ...
    result = read_virtual_mem_func(process.Handle(), basic_info.PebBaseAddress, &peb, sizeof(peb), nullptr);
    if (result != 0) {
        LOG(ERROR) << ProcessType::read_virtual_memory_func_name << " PEB Failed " << std::hex << result;
        return GdiTableEntryVector();
    }
    ...
    result = read_virtual_mem_func(
        process.Handle(), peb.GdiSharedHandleTable, &entries[0], sizeof(typename GdiTableEntryVector::value_type) * entries.size(), nullptr);
    if (result != 0) {
        LOG(ERROR) << ProcessType::read_virtual_memory_func_name << " GDI Handle Table Failed " << std::hex << result;
        return GdiTableEntryVector();
    }
...
template <typename PointerType> base::debug::GdiHandleCounts CountHandleTypesFromTable(DWORD pid, const std::vector<GdiTableEntry<PointerType>>& gdi_table)
{
    ...
    counts.total_tracked = counts.dcs + counts.regions + counts.bitmaps + counts.palettes + counts.fonts + counts.brushes + counts.pens + counts.unknown;
    return counts;
}

template <typename ProcessType> std::optional<base::debug::GdiHandleCounts> CollectGdiHandleCountsImpl(DWORD pid)
{
    base::Process process = base::Process::OpenWithExtraPrivileges(pid);
    if (!process.IsValid())
        return std::nullopt;

    std::vector<GdiTableEntry<typename ProcessType::NativePointerType>> gdi_entries = GetGdiTableEntries<ProcessType>(process);
...
NOINLINE void CrashIfPagefileUsageTooLarge(const PROCESS_MEMORY_COUNTERS_EX& pmc)
{
    CHECK_LE(pmc.PagefileUsage, kLotsOfMemory);
}

NOINLINE void CrashIfPrivateUsageTooLarge(const PROCESS_MEMORY_COUNTERS_EX& pmc)
{
    CHECK_LE(pmc.PrivateUsage, kLotsOfMemory);
}

NOINLINE void CrashIfCannotAllocateSmallBitmap(BITMAPINFOHEADER* header, HANDLE shared_section)
{
    void* small_data = nullptr;
    base::debug::Alias(&small_data);
    header->biWidth = 5;
    header->biHeight = -5;
    HBITMAP small_bitmap = CreateDIBSection(nullptr, reinterpret_cast<BITMAPINFO*>(&header), 0, &small_data, shared_section, 0);
...
{
    pmc->cb = sizeof(*pmc);
    CHECK(GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(pmc), sizeof(*pmc)));
}
...
void CollectChildGDIUsageAndDie(DWORD parent_pid)
{
    ...
    do {
        base::win::ScopedHandle process(OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, proc_entry.th32ProcessID));
        if (!process.is_valid())
            continue;
...
void CollectGDIUsageAndDie(BITMAPINFOHEADER* header, HANDLE shared_section)
{
    ...
    std::optional<GdiHandleCounts> optional_handle_counts = CollectGdiHandleCounts(GetCurrentProcessId());
...
{
    std::optional<GdiHandleCounts> handle_counts = CollectGdiHandleCounts(GetCurrentProcessId());
