...
#else
AllocationTraceRecorderStatistics::AllocationTraceRecorderStatistics(size_t total_number_of_allocations)
    : total_number_of_allocations(total_number_of_allocations)
{
}
#endif

void AllocationTraceRecorder::OnAllocation(const void* allocated_address, size_t allocated_size)
{
    ...
    for (auto index = GetNextIndex(); !alloc_trace_buffer_[index].InitializeAllocation(allocated_address, allocated_size); index = GetNextIndex()) {
#if BUILDFLAG(ENABLE_ALLOCATION_TRACE_RECORDER_FULL_REPORTING)
...
void AllocationTraceRecorder::OnFree(const void* freed_address)
{
    ...
    for (auto index = GetNextIndex(); !alloc_trace_buffer_[index].InitializeFree(freed_address); index = GetNextIndex()) {
#if BUILDFLAG(ENABLE_ALLOCATION_TRACE_RECORDER_FULL_REPORTING)
        total_number_of_collisions_.fetch_add(1, std::memory_order_relaxed);
#endif
    }
...
{
    return std::min(kMaximumNumberOfMemoryOperationTraces, total_number_of_records_.load(std::memory_order_relaxed));
}
...
const OperationRecord& AllocationTraceRecorder::operator[](size_t idx) const
{
    ...
    const size_t array_index = size() < GetMaximumNumberOfTraces() ? idx : WrapIdxIfNeeded(total_number_of_records_.load(std::memory_order_relaxed) + idx);
...
AllocationTraceRecorderStatistics AllocationTraceRecorder::GetRecorderStatistics() const
{
#if BUILDFLAG(ENABLE_ALLOCATION_TRACE_RECORDER_FULL_REPORTING)
    return { total_number_of_records_.load(std::memory_order_relaxed), total_number_of_collisions_.load(std::memory_order_relaxed) };
#else
