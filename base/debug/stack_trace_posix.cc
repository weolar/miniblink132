...
// libc.
#if BUILDFLAG(IS_APPLE) || (defined(__GLIBC__) && !defined(__UCLIBC__) && !defined(__AIX))
...
void DemangleSymbols(std::string* text)
{
    ...
    while (search_from < text->size()) {
        ...
        int status = 0;
        std::unique_ptr<char, base::FreeDeleter> demangled_symbol(abi::__cxa_demangle(mangled_symbol.c_str(), nullptr, 0, &status));
...
void ProcessBacktrace(span<const void* const> traces, cstring_view prefix_string, BacktraceOutputHandler* handler)
{
    ...
    for (size_t i = 0; i < traces.size(); ++i) {
        ...
        const void* address = UNSAFE_BUFFERS(static_cast<const char*>(traces[i]) - 1);
        if (google::Symbolize(const_cast<void*>(address), buf.data(), buf.size())) {
            ...
            if (GetDwarfSourceLineNumber(address, cu_offsets[i], buf.data(), buf.size())) {
                handler->HandleOutput(" [");
                handler->HandleOutput(buf.data());
                handler->HandleOutput("]");
            }
    ...
    if (in_signal_handler == 0 && IsValueInRangeForNumericType<int>(traces.size())) {
        ...
        for (size_t i = 0; i < traces.size(); ++i) {
            ...
            if (dl_info_found) {
                ...
                if (size_t last_sep = dli_fname.rfind('/'); last_sep != base::cstring_view::npos) {
        ...
        auto trace_symbols =
            ...
            UNSAFE_BUFFERS(base::HeapArray<char*, FreeDeleter>::FromOwningPointer(
                backtrace_symbols(const_cast<void* const*>(traces.data()), static_cast<int>(traces.size())), traces.size()));
...
void AlarmSignalHandler(int signal, siginfo_t* info, void* void_context)
{
    ...
    PrintToStderr("Warning: Default signal handler failed to terminate process.\n");
...
// and the risk is low given we're likely crashing already.
...
#if defined(ARCH_CPU_X86_64) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS))
    ...
    if (signal == SIGSEGV && info->si_code == SI_KERNEL) {
        PrintToStderr(" Possibly a General Protection Fault, can be due to a non-canonical "
...
#if ARCH_CPU_32_BITS
        { "  gs: ", gregs[REG_GS] }, { "  fs: ", gregs[REG_FS] }, { "  es: ", gregs[REG_ES] }, { "  ds: ", gregs[REG_DS] }, { " edi: ", gregs[REG_EDI] },
            { " esi: ", gregs[REG_ESI] }, { " ebp: ", gregs[REG_EBP] }, { " esp: ", gregs[REG_ESP] }, { " ebx: ", gregs[REG_EBX] },
            { " edx: ", gregs[REG_EDX] }, { " ecx: ", gregs[REG_ECX] }, { " eax: ", gregs[REG_EAX] }, { " trp: ", gregs[REG_TRAPNO] },
            { " err: ", gregs[REG_ERR] }, { "  ip: ", gregs[REG_EIP] }, { "  cs: ", gregs[REG_CS] }, { " efl: ", gregs[REG_EFL] },
            { " usp: ", gregs[REG_UESP] }, { "  ss: ", gregs[REG_SS] },
#elif ARCH_CPU_64_BITS
        { "  r8: ", gregs[REG_R8] }, { "  r9: ", gregs[REG_R9] }, { " r10: ", gregs[REG_R10] }, { " r11: ", gregs[REG_R11] }, { " r12: ", gregs[REG_R12] },
            { " r13: ", gregs[REG_R13] }, { " r14: ", gregs[REG_R14] }, { " r15: ", gregs[REG_R15] }, { "  di: ", gregs[REG_RDI] },
            { "  si: ", gregs[REG_RSI] }, { "  bp: ", gregs[REG_RBP] }, { "  bx: ", gregs[REG_RBX] }, { "  dx: ", gregs[REG_RDX] },
            { "  ax: ", gregs[REG_RAX] }, { "  cx: ", gregs[REG_RCX] }, { "  sp: ", gregs[REG_RSP] }, { "  ip: ", gregs[REG_RIP] },
            { " efl: ", gregs[REG_EFL] }, { " cgf: ", gregs[REG_CSGSFS] }, { " erf: ", gregs[REG_ERR] }, { " trp: ", gregs[REG_TRAPNO] },
            { " msk: ", gregs[REG_OLDMASK] }, { " cr2: ", gregs[REG_CR2] },
    ...
    long retval = syscall(SYS_rt_tgsigqueueinfo, getpid(), syscall(SYS_gettid), info->si_signo, info);
    if (retval == 0) {
        return;
    }
...
public:
    ...
    {
        return Singleton<SandboxSymbolizeHelper, LeakySingletonTraits<SandboxSymbolizeHelper>>::get();
    }
...
private:
    ...
    // IMPORTANT: This function must be async-signal-safe because it can be
    // called from a signal handler (symbolizing stack frames for a crash).
    static int OpenObjectFileContainingPc(uint64_t pc, uint64_t& start_address, uint64_t& base_address, char* file_path_ptr, size_t file_path_size)
    {
        auto file_path =
    ...
    void SetBaseAddressesForMemoryRegions()
    {
        base::ScopedFD mem_fd;
        {
            ...
            mem_fd = base::ScopedFD(HANDLE_EINTR(open("/proc/self/mem", O_RDONLY | O_CLOEXEC)));
            if (!mem_fd.is_valid()) {
                return;
            }
        }
        ...
        auto safe_memcpy = [&mem_fd](void* dst, uintptr_t src, size_t size) {
            return HANDLE_EINTR(pread(mem_fd.get(), dst, size, static_cast<off_t>(src))) == ssize_t(size);
        };
        ...
        for (auto& r : regions_) {
            ...
            if ((r.permissions & MappedMemoryRegion::READ) && safe_memcpy(&ehdr, r.start, sizeof(ElfW(Ehdr))) && memcmp(ehdr.e_ident, ELFMAG, SELFMAG) == 0) {
                switch (ehdr.e_type) {
                case ET_EXEC:
                    cur_base = 0;
                    break;
                case ET_DYN:
                    ...
                    for (unsigned i = 0; i != ehdr.e_phnum; ++i) {
                        ElfW(Phdr) phdr;
                        if (safe_memcpy(&phdr, r.start + ehdr.e_phoff + i * sizeof(phdr), sizeof(phdr)) && phdr.p_type == PT_LOAD && phdr.p_offset == 0) {
    ...
    void OpenSymbolFiles()
    {
        ...
        for (it = regions_.begin(); it != regions_.end(); ++it) {
            ...
            if ((region.permissions & MappedMemoryRegion::READ) == MappedMemoryRegion::READ && (region.permissions & MappedMemoryRegion::WRITE) == 0
                && (region.permissions & MappedMemoryRegion::EXECUTE) == MappedMemoryRegion::EXECUTE) {
...
size_t CollectStackTrace(span<const void*> trace)
{
    ...
    return base::saturated_cast<size_t>(backtrace(const_cast<void**>(trace.data()), base::saturated_cast<int>(trace.size())));
#else
    return 0;
#endif
}
