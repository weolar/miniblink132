...
bool ParseProcMaps(const std::string& input, std::vector<MappedMemoryRegion>* regions_out)
{
    CHECK(regions_out);
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (sscanf(line, "%" SCNxPTR "-%" SCNxPTR " %4c %llx %hhx:%hhx %ld %n", &region.start, &region.end, permissions, &region.offset, &dev_major, &dev_minor,
                &inode, &path_index)
            < 7) {
...
std::optional<SmapsRollup> ReadAndParseSmapsRollup()
{
    ...
    ssize_t bytes_read = HANDLE_EINTR(read(fd.get(), static_cast<void*>(buffer.data()), read_size));
    if (bytes_read < 0) {
...
std::optional<SmapsRollup> ParseSmapsRollupForTesting(const std::string& smaps_rollup)
{
