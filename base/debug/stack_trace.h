...
BASE_EXPORT bool SetStackDumpFirstChanceCallback(bool (*handler)(int, siginfo_t*, void*));
#endif
...
public:
    ...
    void OutputToStreamWithPrefix(std::ostream* os, cstring_view prefix_string) const;
#endif
...
private:
    ...
    static void PrintMessageWithPrefix(cstring_view prefix_string, cstring_view message);

    void PrintWithPrefixImpl(cstring_view prefix_string) const;
#if !defined(__UCLIBC__) && !defined(_AIX)
    void OutputToStreamWithPrefixImpl(std::ostream* os, cstring_view prefix_string) const;
#endif
...
public:
    ...
    OverrideStackTraceOutputForTesting(const OverrideStackTraceOutputForTesting&) = delete;
    OverrideStackTraceOutputForTesting& operator=(const OverrideStackTraceOutputForTesting&) = delete;
...
BASE_EXPORT size_t TraceStackFramePointers(span<const void*> out_trace, size_t skip_initial, bool enable_scanning = kEnableScanningByDefault);
...
// appended.
BASE_EXPORT void itoa_r(intptr_t i, int base, size_t padding, base::span<char> buf);
