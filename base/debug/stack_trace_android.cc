...
{
    StackCrawlState state(reinterpret_cast<uintptr_t*>(trace.data()), trace.size());
...
void StackTrace::PrintMessageWithPrefix(cstring_view prefix_string, cstring_view message)
{
    if (!prefix_string.empty()) {
        __android_log_write(ANDROID_LOG_ERROR, "chromium", StrCat({ prefix_string, message }).c_str());
    } else {
...
void StackTrace::OutputToStreamWithPrefixImpl(std::ostream* os, cstring_view prefix_string) const
{
    ...
    if (!ReadProcMaps(&proc_maps)) {
        __android_log_write(ANDROID_LOG_ERROR, "chromium", "Failed to read /proc/self/maps");
    } else if (!ParseProcMaps(proc_maps, &regions)) {
        __android_log_write(ANDROID_LOG_ERROR, "chromium", "Failed to parse /proc/self/maps");
    }
