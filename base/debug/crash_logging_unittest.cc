...
public:
    ...
    TestCrashKeyImplementation& operator=(const TestCrashKeyImplementation&) = delete;
...
public:
    CrashLoggingTest()
    {
        SetCrashKeyImplementation(std::make_unique<TestCrashKeyImplementation>(data_));
    }

    ~CrashLoggingTest() override
    {
        SetCrashKeyImplementation(nullptr);
    }
...
TEST_F(CrashLoggingTest, MultipleCrashKeysInSameScope)
{
    ...
    EXPECT_THAT(data(), ElementsAre(Pair("category-bool-value", "false"), Pair("category-int-value", "1")));
