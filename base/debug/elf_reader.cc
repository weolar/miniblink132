...
size_t ReadElfBuildId(const void* elf_mapped_base, bool uppercase, ElfBuildIdBuffer build_id)
{
    ...
    for (const Phdr& header : GetElfProgramHeaders(elf_mapped_base)) {
        ...
        const char* current_section = reinterpret_cast<const char*>(header.p_vaddr + relocation_offset);
        ...
        while (current_section < section_end) {
            ...
            if (current_note->n_type == NT_GNU_BUILD_ID) {
                std::string_view note_name(current_section + sizeof(Nhdr), current_note->n_namesz);
            ...
            size_t section_size = bits::AlignUp(current_note->n_namesz, 4u) + bits::AlignUp(current_note->n_descsz, 4u) + sizeof(Nhdr);
        ...
        const uint8_t* build_id_raw = reinterpret_cast<const uint8_t*>(current_note) + sizeof(Nhdr) + bits::AlignUp(current_note->n_namesz, 4u);
        size_t i = 0;
        for (i = 0; i < current_note->n_descsz; ++i) {
            strings::SafeSNPrintf(&build_id[i * 2], 3, (uppercase ? "%02X" : "%02x"), build_id_raw[i]);
        }
        build_id[i * 2] = '\0';
...
std::optional<std::string_view> ReadElfLibraryName(const void* elf_mapped_base)
{
    // NOTE: Function should use async signal safe calls only.
    ...
    for (const Phdr& header : GetElfProgramHeaders(elf_mapped_base)) {
        ...
        const Dyn* dynamic_end = reinterpret_cast<const Dyn*>(header.p_vaddr + relocation_offset + header.p_memsz);
        ...
        for (const Dyn* dynamic_iter = dynamic_start; dynamic_iter < dynamic_end; ++dynamic_iter) {
            if (dynamic_iter->d_tag == DT_STRTAB) {
                ...
                strtab_addr = static_cast<size_t>(dynamic_iter->d_un.d_ptr) + reinterpret_cast<const char*>(relocation_offset);
#else
...
size_t GetRelocationOffset(const void* elf_mapped_base)
{
    ...
    for (const Phdr& header : headers) {
        if (header.p_type == PT_LOAD) {
            ...
            // ELF image.
            const char* const mapped_address = reinterpret_cast<const char*>(elf_mapped_base) + header.p_offset;
    ...
    return static_cast<size_t>(reinterpret_cast<uintptr_t>(elf_mapped_base) - reinterpret_cast<uintptr_t>(nullptr));
}
