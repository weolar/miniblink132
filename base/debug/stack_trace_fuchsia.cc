...
_Unwind_Reason_Code UnwindStore(struct _Unwind_Context* context, void* user_data)
{
...
public:
    ...
    span<Module> GetModules()
    {
        return { modules_.data(), count_ };
    }
...
void SymbolMap::Populate()
{
    ...
    zx_status_t status = zx_object_get_property(process, ZX_PROP_PROCESS_DEBUG_ADDR, &debug_addr, sizeof(debug_addr));
    if (status != ZX_OK) {
    ...
    while (lmap != nullptr) {
        ...
        for (const Elf64_Phdr& phdr : GetElfProgramHeaders(next_entry.addr)) {
            ...
            Segment segment;
            segment.addr = reinterpret_cast<const char*>(next_entry.addr.get()) + phdr.p_vaddr;
        ...
        std::optional<std::string_view> elf_library_name = ReadElfLibraryName(next_entry.addr);
        if (elf_library_name) {
            strlcpy(next_entry.name, elf_library_name->data(), elf_library_name->size() + 1);
        } else {
            std::string_view link_map_name(lmap->l_name[0] ? lmap->l_name : "<executable>");
            ...
            if (directory_prefix_idx != std::string_view::npos) {
                link_map_name = link_map_name.substr(directory_prefix_idx + 1, link_map_name.size() - directory_prefix_idx - 1);
            }
...
bool ModuleContainsFrameAddress(const void* address, const SymbolMap::Module& module_entry)
{
    for (size_t i = 0; i < module_entry.segment_count; ++i) {
        const SymbolMap::Segment& segment = module_entry.segments[i];
        const void* segment_end = reinterpret_cast<const void*>(reinterpret_cast<const char*>(segment.addr.get()) + segment.size - 1);
...
// static
void StackTrace::PrintMessageWithPrefix(cstring_view prefix_string, cstring_view message)
{
...
void StackTrace::OutputToStreamWithPrefixImpl(std::ostream* os, cstring_view prefix_string) const
{
    SymbolMap map;
    ...
    for (const SymbolMap::Module& module_entry : map.GetModules()) {
        ...
        *os << "{{{module:" << module_id << ":" << module_entry.name << ":elf:" << module_entry.build_id << "}}}\n";

        for (size_t i = 0; i < module_entry.segment_count; ++i) {
            const SymbolMap::Segment& segment = module_entry.segments[i];

            char permission_string[4] = {};
            *os << "{{{mmap:" << segment.addr << ":0x" << std::hex << segment.size << std::dec << ":load:" << module_id << ":"
                << PermissionFlagsToString(segment.permission_flags, permission_string) << ":"
