...
public:
    ...
    TestElfImageBuilder& AddNoteSegment(Word type, std::string_view name, span<const uint8_t> desc);
...
private:
    ...
    template <typename T> static uint8_t* AppendHdr(const T& hdr, uint8_t* loc);

    Ehdr CreateEhdr(Half phnum);
    Phdr CreatePhdr(Word type, Word flags, size_t align, Off offset, Addr vaddr, size_t size);
