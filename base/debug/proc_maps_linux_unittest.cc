...
TEST(ProcMapsTest, Permissions)
{
    ...
    for (size_t i = 0; i < std::size(kTestCases); ++i) {
        SCOPED_TRACE(base::StringPrintf("kTestCases[%zu] = %s", i, kTestCases[i].input));
...
__attribute__((no_sanitize("address", "hwaddress"))) void CheckProcMapsRegions(const std::vector<MappedMemoryRegion>& regions)
{
