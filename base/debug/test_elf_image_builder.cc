...
TestElfImageBuilder& TestElfImageBuilder::AddLoadSegment(Word flags, size_t size)
{
    load_segments_.push_back({ flags, static_cast<Word>(size) });
    return *this;
}

TestElfImageBuilder& TestElfImageBuilder::AddNoteSegment(Word type, std::string_view name, span<const uint8_t> desc)
{
    const size_t name_with_null_size = name.size() + 1;
    std::vector<uint8_t> buffer(sizeof(Nhdr) + bits::AlignUp(name_with_null_size, size_t { 4 }) + bits::AlignUp(desc.size(), size_t { 4 }), '\0');
...
Addr TestElfImageBuilder::GetVirtualAddressForOffset(Off offset, const uint8_t* elf_start) const
{
...
TestElfImageBuilder::ImageMeasures TestElfImageBuilder::MeasureSizesAndOffsets() const
{
...
TestElfImage TestElfImageBuilder::Build()
{
    ...
    const size_t load_bias = mapping_type_ == RELOCATABLE_WITH_BIAS ? kLoadBias : 0;
    std::vector<uint8_t> buffer(load_bias + (kPageSize - 1) + measures.total_size, '\0');
    uint8_t* const elf_start = bits::AlignUp(&buffer.front() + load_bias, kPageSize);
    ...
    loc = AppendHdr(
        CreatePhdr(PT_PHDR, PF_R, kPhdrAlign, loc - elf_start, GetVirtualAddressForOffset(loc - elf_start, elf_start), sizeof(Phdr) * measures.phdrs_required),
        loc);
    for (size_t i = 0; i < load_segments_.size(); ++i) {
        ...
        if (i == 0)
            size += loc - elf_start;
        loc = AppendHdr(CreatePhdr(PT_LOAD, load_segment.flags, kLoadAlign, measures.load_segment_start[i],
                            GetVirtualAddressForOffset(measures.load_segment_start[i], elf_start), size),
            loc);
    }
    if (measures.note_size != 0) {
        loc = AppendHdr(
            CreatePhdr(PT_NOTE, PF_R, kNoteAlign, measures.note_start, GetVirtualAddressForOffset(measures.note_start, elf_start), measures.note_size), loc);
    }
    if (soname_) {
        loc = AppendHdr(CreatePhdr(PT_DYNAMIC, PF_R | PF_W, kDynamicAlign, measures.dynamic_start,
                            GetVirtualAddressForOffset(measures.dynamic_start, elf_start), sizeof(Dyn) * 2),
            loc);
    }
    ...
    strtab_dyn.d_un.d_ptr = GetVirtualAddressForOffset(measures.strtab_start, elf_start);
#else
    ...
    strtab_dyn.d_un.d_ptr = reinterpret_cast<uintptr_t>(elf_start + measures.strtab_start);
#endif
...
// static
template <typename T> uint8_t* TestElfImageBuilder::AppendHdr(const T& hdr, uint8_t* loc)
{
...
Phdr TestElfImageBuilder::CreatePhdr(Word type, Word flags, size_t align, Off offset, Addr vaddr, size_t size)
{
    Phdr phdr;
    phdr.p_type = type;
    phdr.p_flags = flags;
