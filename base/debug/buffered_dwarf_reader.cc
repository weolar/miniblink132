...
BufferedDwarfReader::BufferedDwarfReader(int fd, uint64_t position)
    : fd_(fd)
    , next_chunk_start_(position)
    , last_chunk_start_(position)
{
}

size_t BufferedDwarfReader::ReadCString(uint64_t max_position, char* out, size_t out_size)
{
    char character;
...
bool BufferedDwarfReader::ReadCommonHeader(bool& is_64bit, uint64_t& length, uint16_t& version, uint64_t& offset, uint8_t& address_size, uint64_t& end_position)
{
    ...
    while (bytes_left > 0) {
        ...
        if (unconsumed_amount_ == 0) {
            if (!base::IsValueInRangeForNumericType<size_t>(next_chunk_start_))
                return false;
            const ssize_t unconsumed_amount = google::ReadFromOffset(fd_, buf_, sizeof(buf_), static_cast<size_t>(next_chunk_start_));
