...
#elif BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_AIX)
...
void VerifyDebugger()
{
    ...
    FilePath cmdline_file = FilePath("/proc").Append(NumberToString(proc.Handle())).Append("cmdline");
    std::string cmdline;
    ...
    DCHECK(ToLowerASCII(exe).find("gdb") == std::string::npos) << "Detected gdb without sourcing //tools/gdb/gdbinit.  gdb may not be "
