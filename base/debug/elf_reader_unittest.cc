...
std::string ParamInfoToString(const ::testing::TestParamInfo<base::TestElfImageBuilder::MappingType>& param_info)
{
...
using ElfReaderTest = ::testing::TestWithParam<TestElfImageBuilder::MappingType>;

TEST_P(ElfReaderTest, ReadElfBuildIdUppercase)
{
    TestElfImage image
        = TestElfImageBuilder(GetParam()).AddLoadSegment(PF_R | PF_X, /* size = */ 2000).AddNoteSegment(NT_GNU_BUILD_ID, "GNU", kBuildIdBytes).Build();
...
TEST_P(ElfReaderTest, ReadElfBuildIdLowercase)
{
    ...
    EXPECT_EQ(8u, build_id_size);
    EXPECT_EQ(ToLowerASCII(kBuildIdHexStringLower), std::string_view(&build_id[0], build_id_size));
}
...
{
    TestElfImage image
        = TestElfImageBuilder(GetParam()).AddLoadSegment(PF_R | PF_X, /* size = */ 2000).AddNoteSegment(NT_GNU_BUILD_ID, "ABC", kBuildIdBytes).Build();
...
{
    TestElfImage image
        = TestElfImageBuilder(GetParam()).AddLoadSegment(PF_R | PF_X, /* size = */ 2000).AddNoteSegment(NT_GNU_BUILD_ID + 1, "GNU", kBuildIdBytes).Build();
...
{
    TestElfImage image = TestElfImageBuilder(GetParam()).AddLoadSegment(PF_R | PF_X, /* size = */ 2000).Build();
...
{
    TestElfImage image = TestElfImageBuilder(GetParam()).AddLoadSegment(PF_R | PF_X, /* size = */ 2000).AddSoName("mysoname").Build();

    std::optional<std::string_view> library_name = ReadElfLibraryName(image.elf_start());
...
TEST_P(ElfReaderTest, GetRelocationOffset)
{
    ...
    switch (GetParam()) {
    case TestElfImageBuilder::RELOCATABLE:
        EXPECT_EQ(reinterpret_cast<size_t>(image.elf_start()), GetRelocationOffset(image.elf_start()));
        break;

    case TestElfImageBuilder::RELOCATABLE_WITH_BIAS:
        EXPECT_EQ(reinterpret_cast<size_t>(image.elf_start()) - TestElfImageBuilder::kLoadBias, GetRelocationOffset(image.elf_start()));
        break;
...
INSTANTIATE_TEST_SUITE_P(MappingTypes, ElfReaderTest,
    ::testing::Values(TestElfImageBuilder::RELOCATABLE, TestElfImageBuilder::RELOCATABLE_WITH_BIAS, TestElfImageBuilder::NON_RELOCATABLE), &ParamInfoToString);
...
TEST(ElfReaderTestWithCurrentImage, ReadElfBuildId)
{
    ...
    NativeLibrary library = LoadNativeLibrary(base::FilePath(kLibraryName), &error);
    void* init_addr = GetFunctionPointerFromNativeLibrary(library, "MallocWrapper");
    ...
    ASSERT_TRUE(name);
    EXPECT_NE(std::string::npos, name->find(kLibraryName)) << "Library name " << *name << " doesn't contain expected " << kLibraryName;
