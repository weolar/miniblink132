...
public:
    ...
    ALWAYS_INLINE bool InitializeAllocation(const void* allocated_address, size_t allocated_size)
    {
        return InitializeOperationRecord(allocated_address, allocated_size, OperationType::kAllocation);
    }
...
private:
    ...
    // thread is capturing.
    ALWAYS_INLINE bool InitializeOperationRecord(const void* address, size_t size, OperationType operation_type);
...
ALWAYS_INLINE bool OperationRecord::InitializeOperationRecord(const void* address, size_t size, OperationType operation_type)
{
...
public:
    ...
    inline void OnAllocation(const base::allocator::dispatcher::AllocationNotificationData& allocation_data);

    // Handle all free events.
    inline void OnFree(const base::allocator::dispatcher::FreeNotificationData& free_data);
...
private:
    ...
    NOINLINE void OnAllocation(const void* allocated_address, size_t allocated_size);
    ...
    std::array<OperationRecord, kMaximumNumberOfMemoryOperationTraces> alloc_trace_buffer_ = {};
...
inline void AllocationTraceRecorder::OnAllocation(const base::allocator::dispatcher::AllocationNotificationData& allocation_data)
{
    OnAllocation(allocation_data.address(), allocation_data.size());
}

// Handle all free events.
inline void AllocationTraceRecorder::OnFree(const base::allocator::dispatcher::FreeNotificationData& free_data)
{
    OnFree(free_data.address());
}

ALWAYS_INLINE constexpr size_t AllocationTraceRecorder::WrapIdxIfNeeded(size_t idx)
{
    ...
    // of two .
    static_assert(std::has_single_bit(kMaximumNumberOfMemoryOperationTraces),
...
{
    const auto raw_idx = total_number_of_records_.fetch_add(1, std::memory_order_relaxed);
