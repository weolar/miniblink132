...
public:
    ...
    {
        return last_chunk_start_ + cursor_in_buffer_;
    }
    ...
    bool ReadCommonHeader(bool& is_64bit, uint64_t& length, uint16_t& version, uint64_t& offset, uint8_t& address_size, uint64_t& end_position);

private:
    // Generic helper to read an integral value. The size read is determined by
    // the width of `value`
    template <typename IntType> bool ReadInt(IntType& value)
    {
