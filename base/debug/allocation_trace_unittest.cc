...
template <typename Iterator> std::string MakeString(Iterator begin, Iterator end)
{
...
template <typename C> std::string MakeString(const C& data)
{
    return MakeString(std::begin(data), std::end(data));
}

void AreEqual(const base::debug::tracer::OperationRecord& expected, const base::debug::tracer::OperationRecord& is)
{
...
protected:
    ...
    // under test.
    struct Buffer {
        alignas(AllocationTraceRecorder) std::array<uint8_t, sizeof(AllocationTraceRecorder)> data;
    };

protected:
    AllocationNotificationData CreateAllocationData(void* address, size_t size, MTEMode mte_mode = MTEMode::kUndefined)
    {
        return AllocationNotificationData(address, size, nullptr, AllocationSubsystem::kPartitionAllocator)
    ...
    FreeNotificationData CreateFreeData(void* address, MTEMode mte_mode = MTEMode::kUndefined)
    {
        return FreeNotificationData(address, AllocationSubsystem::kPartitionAllocator)
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
            .SetMteReportingMode(mte_mode)
#endif
            ;
...
private:
    ...
    std::unique_ptr<AllocationTraceRecorder> const subject_under_test_ = std::make_unique<AllocationTraceRecorder>();
};
...
TEST_F(AllocationTraceRecorderTest, VerifyBinaryCopy)
{
    ...
    for (size_t index = 0; index < number_of_records; ++index) {
        if (index & 0x1) {
            subject_under_test.OnAllocation(CreateAllocationData(this, sizeof(*this)));
        } else {
    ...
    AllocationTraceRecorder* const buffered_recorder = new (buffer->data.data()) AllocationTraceRecorder();

    static_assert(std::is_trivially_copyable_v<AllocationTraceRecorder>);
    base::byte_span_from_ref(*buffered_recorder).copy_from(base::byte_span_from_ref(subject_under_test));
...
TEST_F(AllocationTraceRecorderTest, VerifySingleAllocation)
{
    ...
    subject_under_test.OnAllocation(CreateAllocationData(&subject_under_test, sizeof(subject_under_test)));
...
TEST_F(AllocationTraceRecorderTest, VerifyMultipleOperations)
{
    ...
    subject_under_test.OnAllocation(CreateAllocationData(addr0, 1 * sizeof(*this)));
    subject_under_test.OnFree(CreateFreeData(addr2));
    subject_under_test.OnAllocation(CreateAllocationData(addr3, 3 * sizeof(*this)));
    subject_under_test.OnAllocation(CreateAllocationData(addr4, 4 * sizeof(*this)));
...
TEST_F(AllocationTraceRecorderTest, VerifyOverflowOfOperations)
{
    ...
    // So the following allocation should overwrite the first record.
    {
        ...
        subject_under_test.OnAllocation(CreateAllocationData(addr(idx), idx));
        ASSERT_EQ(subject_under_test.size(), subject_under_test.GetMaximumNumberOfTraces());
        const auto& last_entry = subject_under_test[subject_under_test.GetMaximumNumberOfTraces() - 1];
        ASSERT_EQ(last_entry.GetOperationType(), OperationType::kAllocation);
        ASSERT_EQ(last_entry.GetAddress(), addr(idx));
...
TEST_F(AllocationTraceRecorderTest, VerifyWithHooks)
{
    ...
    std::make_unique<std::string>("Just enforce an allocation and free to trigger notification of the "
...
protected:
    ...
    ReferenceStackTrace GetReferenceTrace()
    {
        ...
        const auto num_frames = base::debug::TraceStackFramePointers(frame_pointers, 0);
        frame_pointers.resize(num_frames);
        frame_pointers.shrink_to_fit();
        return frame_pointers;
    }

    void VerifyStackTrace(const ReferenceStackTrace& reference_stack_trace, const base::debug::tracer::StackTraceContainer& stack_trace)
    {
        ...
        auto const it_stack_trace_end = std::find(it_stack_trace_begin, std::end(stack_trace), nullptr);
        auto const it_reference_stack_trace_end = std::end(reference_stack_trace);

        auto const it_stack_trace
            = std::find_first_of(it_stack_trace_begin, it_stack_trace_end, std::begin(reference_stack_trace), it_reference_stack_trace_end);

        ASSERT_NE(it_stack_trace, it_stack_trace_end) << "stack-trace and reference-stack-trace share no common frame!\n"
        ...
        const auto it_reference_stack_trace = std::find(std::begin(reference_stack_trace), it_reference_stack_trace_end, *it_stack_trace);

        const auto number_of_expected_common_frames
            = std::min(std::distance(it_stack_trace, it_stack_trace_end), std::distance(it_reference_stack_trace, it_reference_stack_trace_end));

        // Check if we have any difference within the section of frames that we
        // expect to be equal.
        const auto mismatch = std::mismatch(it_reference_stack_trace, it_reference_stack_trace + number_of_expected_common_frames, it_stack_trace);

        ASSERT_EQ(mismatch.first, (it_reference_stack_trace + number_of_expected_common_frames))
            << "found difference in the range of frames expected to be equal!\n"
            << "position = " << std::distance(it_reference_stack_trace, mismatch.first) << '\n'
            << "stack trace = " << MakeString(it_stack_trace, it_stack_trace + number_of_expected_common_frames) << '\n'
            << "reference stack trace = " << MakeString(it_reference_stack_trace, it_reference_stack_trace + number_of_expected_common_frames);
    }
};
