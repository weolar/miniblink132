...
struct LineNumberRegisters {
    ...
    {
        address += program_info->minimum_instruction_length * ((op_index + op_advance) / program_info->maximum_operations_per_instruction);

        op_index = (op_index + op_advance) % program_info->maximum_operations_per_instruction;
    }
...
void EvaluateLineNumberProgram(const int fd, LineNumberInfo* info, uint64_t base_address, uint64_t start, const ProgramInfo& program_info)
{
    ...
    public:
        OnCommitImpl(LineNumberInfo* info, uint64_t module_relative_pc, const ProgramInfo& program_info)
            : info(info)
        ...
        void Do(LineNumberRegisters* registers) override
        {
            ...
            if (module_relative_pc < registers->last_address || module_relative_pc >= registers->address)
                return;
            ...
            if (registers->last_file < program_info->num_filenames) {
                ...
                if (registers->last_file == 0 && program_info->filename_offsets[0] == 0 && 1 < program_info->num_filenames) {
                ...
                if (registers->last_file < kMaxFilenames) {
                    info->module_filename_offset = program_info->filename_offsets[registers->last_file];
    ...
    while (reader.position() < program_info.end_offset && info->line == 0) {
        ...
        if (opcode >= program_info.opcode_base) {
            ...
            registers.OpAdvance(&program_info, program_info.OpcodeToAdvance(adjusted_opcode));
            const int line_adjust = program_info.line_base + (adjusted_opcode % program_info.line_range);
            if (line_adjust < 0) {
            ...
            case 8:
                // DW_LNS_const_add_pc
                registers.OpAdvance(&program_info, program_info.OpcodeToAdvance(255 - program_info.opcode_base));
                break;
...
bool ParseDwarf4ProgramInfo(BufferedDwarfReader* reader, bool is_64bit, uint64_t cu_name_offset, ProgramInfo* program_info)
{
    ...
    if (!reader->ReadInt8(program_info->minimum_instruction_length) || !reader->ReadInt8(program_info->maximum_operations_per_instruction)
        || !reader->ReadInt8(program_info->default_is_stmt) || !reader->ReadInt8(program_info->line_base) || !reader->ReadInt8(program_info->line_range)
...
bool ReadProgramInfo(const int fd, uint64_t start, uint64_t cu_name_offset, ProgramInfo* program_info)
{
    BufferedDwarfReader reader(fd, start);
    ...
    if (version == 4) {
        return ParseDwarf4ProgramInfo(&reader, is_64bit, cu_name_offset, program_info);
    }
...
uint64_t GetLineNumbersInProgram(const int fd, LineNumberInfo* info, uint64_t base_address, uint64_t start, uint64_t cu_name_offset)
{
    // Open the program.
    ProgramInfo program_info;
    if (ReadProgramInfo(fd, start, cu_name_offset, &program_info)) {
        EvaluateLineNumberProgram(fd, info, base_address, program_info.start_offset, program_info);
    }
...
bool AdvancedReaderToAttributeList(BufferedDwarfReader& reader, uint64_t table_end, uint64_t wanted_abbreviation_code, uint64_t& tag, bool& has_children)
{
...
bool GetCompileUnitName(int fd, uint64_t debug_info_start, uint64_t debug_info_end, uint64_t pc, uint64_t module_base_address, uint64_t* debug_line_offset,
    uint64_t* cu_name_offset)
{
    ...
    *cu_name_offset = 0;
    ...
    if (!google::GetSectionHeaderByName(fd, kDebugAbbrevSectionName, sizeof(kDebugAbbrevSectionName), &debug_abbrev)) {
        return false;
    }
    ...
    if (!google::GetSectionHeaderByName(fd, kDebugStrSectionName, sizeof(kDebugStrSectionName), &debug_str)) {
        return false;
    }
    ...
    if (!google::GetSectionHeaderByName(fd, kDebugRangesSectionName, sizeof(kDebugRangesSectionName), &debug_ranges)) {
        return false;
    }
    ...
    for (BufferedDwarfReader reader(fd, debug_info_start); reader.position() < debug_info_end; reader.set_position(next_compilation_unit)) {
        bool is_64bit;
        uint64_t length;
        uint16_t dwarf_version;
        uint64_t abbrev_offset;
        uint8_t address_size;
        if (!reader.ReadCommonHeader(is_64bit, length, dwarf_version, abbrev_offset, address_size, next_compilation_unit)) {
            return false;
        }
        ...
        BufferedDwarfReader abbrev_reader(fd, debug_abbrev.sh_offset + abbrev_offset);
        uint64_t tag;
        bool has_children;
        AdvancedReaderToAttributeList(abbrev_reader, debug_abbrev_end, abbreviation_code, tag, has_children);
        ...
        do {
            ...
            case kDW_FORM_sec_offset: {
                ...
                if (attr == kDW_AT_ranges) {
                    ...
                    BufferedDwarfReader ranges_reader(fd, debug_ranges.sh_offset + value);
                    ...
                    while (ranges_reader.position() < debug_ranges_end) {
                        ...
                        if (range_start == 0 && range_end == 0) {
                            ...
                            break;
                        } else if (((address_size == 4) && (range_start == 0xffffffffUL)) || ((address_size == 8) && (range_start == 0xffffffffffffffffULL))) {
...
bool ReadCompileUnit(int fd, uint64_t pc, uint64_t cu_offset, uint64_t base_address, uint64_t* debug_line_offset, uint64_t* cu_name_offset)
{
    if (cu_offset == 0) {
        return false;
    }
    ...
    if (!google::GetSectionHeaderByName(fd, kDebugInfoSectionName, sizeof(kDebugInfoSectionName), &debug_info)) {
        return false;
    }
    uint64_t debug_info_end = debug_info.sh_offset + debug_info.sh_size;

    return GetCompileUnitName(fd, debug_info.sh_offset + cu_offset, debug_info_end, pc, base_address, debug_line_offset, cu_name_offset);
}
...
void SerializeLineNumberInfoToString(int fd, const LineNumberInfo& info, char* out, size_t out_size)
{
    size_t out_pos = 0;
    if (info.module_filename_offset) {
        BufferedDwarfReader reader(fd, info.module_dir_offset);
        if (info.module_dir_offset != 0) {
            out_pos += reader.ReadCString(kMaxOffset, out + out_pos, out_size - out_pos);
            out[out_pos - 1] = '/';
        }
    ...
    out[out_pos - 1] = ':';
    auto result = std::to_chars(out + out_pos, out + out_size, static_cast<intptr_t>(info.line));
    ...
    out[out_pos++] = ':';
    result = std::to_chars(out + out_pos, out + out_size, static_cast<intptr_t>(info.column));
    if (result.ec != std::errc()) {
        out[out_pos - 1] = '\0';
        return;
    }
...
bool GetLineNumberInfoFromObject(int fd, uint64_t pc, uint64_t cu_offset, uint64_t base_address, char* out, size_t out_size)
{
    uint64_t cu_name_offset;
    uint64_t debug_line_offset;
    if (!ReadCompileUnit(fd, pc, cu_offset, base_address, &debug_line_offset, &cu_name_offset)) {
        return false;
    }

    ElfW(Shdr) debug_line;
    constexpr static char kDebugLineSectionName[] = ".debug_line";
    if (!google::GetSectionHeaderByName(fd, kDebugLineSectionName, sizeof(kDebugLineSectionName), &debug_line)) {
        return false;
    }
    ...
    GetLineNumbersInProgram(fd, &info, base_address, line_info_program_offset, cu_name_offset);
...
size_t ProcessFlatArangeSet(
    BufferedDwarfReader* reader, uint64_t next_set, uint8_t address_size, uint64_t base_address, uint64_t cu_offset, FrameInfo* frame_info, size_t num_frames)
{
...
void PopulateCompileUnitOffsets(int fd, FrameInfo* frame_info, size_t num_frames, uint64_t base_address)
{
    ElfW(Shdr) debug_aranges;
    constexpr static char kDebugArangesSectionName[] = ".debug_aranges";
    if (!google::GetSectionHeaderByName(fd, kDebugArangesSectionName, sizeof(kDebugArangesSectionName), &debug_aranges)) {
        return;
    }
    uint64_t debug_aranges_end = debug_aranges.sh_offset + debug_aranges.sh_size;
    uint64_t next_arange_set = kMaxOffset;
    size_t unsorted_start = 0;
    for (BufferedDwarfReader reader(fd, debug_aranges.sh_offset); unsorted_start < num_frames && reader.position() < debug_aranges_end;
        ...
        uint8_t address_size;
        if (!reader.ReadCommonHeader(is_64bit, length, arange_version, debug_info_offset, address_size, next_arange_set)) {
            return;
        }
        ...
        while (((reader.position() - debug_aranges.sh_offset) % (2 * address_size)) != 0) {
            uint8_t dummy;
            if (!reader.ReadInt8(dummy)) {
                return;
            }
        ...
        unsorted_start += ProcessFlatArangeSet(
            &reader, next_arange_set, address_size, base_address, debug_info_offset, &frame_info[unsorted_start], num_frames - unsorted_start);
    }
}

} // namespace

bool GetDwarfSourceLineNumber(const void* pc, uint64_t cu_offset, char* out, size_t out_size)
{
    uint64_t pc0 = reinterpret_cast<uint64_t>(pc);
    uint64_t object_start_address = 0;
    uint64_t object_base_address = 0;

    google::FileDescriptor object_fd(
        google::FileDescriptor(google::OpenObjectFileContainingPcAndGetStartAddress(pc0, object_start_address, object_base_address, nullptr, 0)));

    if (!object_fd.get()) {
        return false;
    }

    if (!GetLineNumberInfoFromObject(object_fd.get(), pc0, cu_offset, object_base_address, out, out_size)) {
        return false;
    }
...
void GetDwarfCompileUnitOffsets(const void* const* trace, uint64_t* cu_offsets, size_t num_frames)
{
    ...
    auto pc_comparator = [](const FrameInfo& lhs, const FrameInfo& rhs) { return lhs.pc < rhs.pc; };
    ...
    for (size_t cur_frame = 0; cur_frame < num_frames; ++cur_frame) {
        ...
        google::FileDescriptor object_fd(google::FileDescriptor(
            google::OpenObjectFileContainingPcAndGetStartAddress(frame_info[cur_frame].pc, object_start_address, object_base_address, nullptr, 0)));
        ...
        PopulateCompileUnitOffsets(object_fd.get(), &frame_info[cur_frame], 1, object_base_address);
    }
}
