...
BASE_EXPORT void SetCrashKeyString(CrashKeyString* crash_key, std::string_view value);
...
#define SCOPED_CRASH_KEY_STRING_INTERNAL2(category, name, nonce, data, key_size)                                                                               \
    static_assert(::std::size(category "-" name) < 40, "Crash key names must be shorter than 40 characters.");                                                 \
    static_assert(::std::string_view(category "-" name).find(':') == ::std::string_view::npos, "Crash key names must not contain the ':' character.");         \
    ::base::debug::ScopedCrashKeyString scoped_crash_key_helper##nonce(                                                                                        \
        [] {                                                                                                                                                   \
            static auto* const key = ::base::debug::AllocateCrashKeyString(category "-" name, key_size);                                                       \
            return key;                                                                                                                                        \
        }(),                                                                                                                                                   \
        (data))

// This indirection is needed to expand __COUNTER__.
#define SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, nonce, data, key_size) SCOPED_CRASH_KEY_STRING_INTERNAL2(category, name, nonce, data, key_size)
...
#define SCOPED_CRASH_KEY_STRING32(category, name, data)                                                                                                        \
    SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size32)

#define SCOPED_CRASH_KEY_STRING64(category, name, data)                                                                                                        \
    SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size64)

#define SCOPED_CRASH_KEY_STRING256(category, name, data)                                                                                                       \
    SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size256)

#define SCOPED_CRASH_KEY_STRING1024(category, name, data)                                                                                                      \
    SCOPED_CRASH_KEY_STRING_INTERNAL(category, name, __COUNTER__, (data), ::base::debug::CrashKeySize::Size1024)

#define SCOPED_CRASH_KEY_BOOL(category, name, data)                                                                                                            \
    static_assert(std::is_same_v<std::decay_t<decltype(data)>, bool>, "SCOPED_CRASH_KEY_BOOL must be passed a boolean value.");                                \
    SCOPED_CRASH_KEY_STRING32(category, name, (data) ? "true" : "false")

#define SCOPED_CRASH_KEY_NUMBER(category, name, data) SCOPED_CRASH_KEY_STRING32(category, name, ::base::NumberToString(data))
...
BASE_EXPORT void SetCrashKeyImplementation(std::unique_ptr<CrashKeyImplementation> impl);
