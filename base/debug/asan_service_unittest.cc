...
protected:
    void SetUp() override
    {
        AsanService::GetInstance()->Initialize();
    }
};
...
TEST_F(AsanServiceTest, MAYBE_ErrorCallback)
{
    ...
    AsanService::GetInstance()->AddErrorCallback([](const char*, bool*) { AsanService::GetInstance()->Log("\nErrorCallback1"); });
...
TEST_F(AsanServiceTest, MAYBE_CrashInErrorCallback)
{
    ...
    EXPECT_DEATH(AsanHeapUseAfterFree(), "AddressSanitizer: nested bug in the same thread");
}
...
TEST_F(AsanServiceTest, MAYBE_ShouldExitCleanly)
{
    ...
    AsanService::GetInstance()->AddErrorCallback([](const char* reason, bool* should_exit_cleanly) {
        AsanService::GetInstance()->Log("\nShouldExitCleanly");
...
public:
    ...
    void Run()
    {
        task_runner_->PostTask(FROM_HERE, BindOnce(&AsanTaskTraceTest::PostingTask, Unretained(this)));
...
private:
    ...
    const raw_ref<SingleThreadTaskRunner> task_runner_ { *task_environment_.GetMainThreadTaskRunner() };
};
