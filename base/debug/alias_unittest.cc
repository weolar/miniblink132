...
TEST(DebugAlias, Test)
{
    ...
    std::unique_ptr<std::string> input = std::make_unique<std::string>(kTestString);
    ...
    DEBUG_ALIAS_FOR_CSTR(copy1, input->c_str(), kTestStringLength + 1 /* == input->size() + 1 */);
    static_assert(std::is_same_v<decltype(copy1), char[kTestStringLength + 1]>);
    EXPECT_TRUE(std::equal(std::begin(kTestString), std::end(kTestString), copy1));
    ...
    EXPECT_TRUE(std::equal(std::begin(kTestString), std::end(kTestString), copy3));
}
...
TEST(DebugAlias, U16String)
{
    ...
    static_assert(std::is_same_v<decltype(aliased_copy), char16_t[kTestStringLength + 1]>);
    EXPECT_TRUE(std::equal(std::begin(kTestString), std::end(kTestString), aliased_copy));

    // Verify that the copy is properly null-terminated even when it is larger
    // than the input string.
    DEBUG_ALIAS_FOR_U16CSTR(aliased_copy2, input.c_str(), kTestStringLength + 1);
    static_assert(std::is_same_v<decltype(aliased_copy2), char16_t[kTestStringLength + 1]>);
    EXPECT_TRUE(std::equal(std::begin(kTestString), std::end(kTestString), aliased_copy2));
}
