...
TEST_F(StackTraceTest, OutputToStream)
{
...
#if defined(OFFICIAL_BUILD) && ((BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE)) || BUILDFLAG(IS_FUCHSIA))
    ...
    ASSERT_EQ(backtrace_message.find("Dumping unresolved backtrace"), std::string::npos) << "Unable to resolve symbols.";
    ...
    EXPECT_TRUE(backtrace_message.find("testing::UnitTest::Run") != std::string::npos) << "Expected a demangled symbol in backtrace:\n" << backtrace_message;

    // Expect to at least find main.
    EXPECT_TRUE(backtrace_message.find("main") != std::string::npos) << "Expected to find main in backtrace:\n" << backtrace_message;
...
template <size_t Depth> NOINLINE NOOPT void ExpectStackFramePointers(span<const void*> frames)
{
code_start:
...
template <> NOINLINE NOOPT void ExpectStackFramePointers<1>(span<const void*> frames)
{
code_start:
    // Calling __builtin_frame_address() forces compiler to emit
    // frame pointers, even if they are not enabled.
...
TEST(CheckExitCodeAfterSignalHandlerDeathTest, CheckSIGSEGVNonCanonicalAddress)
{
    // Pointee and pointer are volatile to prevent reordering of instructions,
    // i.e. for optimization. Reordering may lead to tests erroneously failing due
    // to SIGSEGV being raised outside of EXPECT_EXIT.
    ...
    volatile int* const volatile p_int = reinterpret_cast<int*>(0xabcdabcdabcdabcdULL);
