...
bool InitializeSymbols()
{
    ...
    std::wstring new_path = StrCat({ symbols_path, L";", GetExePath().DirName().value() });
...
public:
    ...
    void OutputTraceToStream(base::span<const void* const> traces, std::ostream* os, cstring_view prefix_string)
    {
        AutoLock lock(lock_);
...
NOINLINE size_t CollectStackTrace(span<const void*> trace)
{
    ...
    return CaptureStackBackTrace(0, trace.size(), const_cast<void**>(trace.data()), NULL);
}
...
#error Unsupported Windows Arch
    ...
    while (StackWalk64(
               machine_type, GetCurrentProcess(), GetCurrentThread(), &stack_frame, &context_copy, NULL, &SymFunctionTableAccess64, &SymGetModuleBase64, NULL)
