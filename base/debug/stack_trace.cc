...
OverrideStackTraceOutputForTesting::Mode g_override_suppression = OverrideStackTraceOutputForTesting::Mode::kUnset;
...
uintptr_t GetStackEnd()
{
    ...
    if (!error) {
        error = pthread_attr_getstack(&attributes, reinterpret_cast<void**>(&stack_begin), &stack_size);
...
StackTrace::StackTrace()
    : StackTrace(std::size(trace_))
{
}

StackTrace::StackTrace(size_t count)
    : count_(ShouldSuppressOutput() ? 0 : CollectStackTrace(base::span(trace_).first(std::min(count, std::size(trace_)))))
{
}
...
bool StackTrace::WillSymbolizeToStreamForTesting()
{
    ...
    return false;
#elif defined(OFFICIAL_BUILD) && ((BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE)) || BUILDFLAG(IS_FUCHSIA))
...
void StackTrace::OutputToStreamWithPrefix(std::ostream* os, cstring_view prefix_string) const
{
    if (!count_ || ShouldSuppressOutput()) {
        if (g_stack_trace_message) {
...
{
    delete std::exchange(g_stack_trace_message, (message.empty() ? nullptr : new std::string(std::move(message))));
}
...
bool StackTrace::ShouldSuppressOutput()
{
    ...
    return g_override_suppression != Mode::kUnset ? (g_override_suppression == Mode::kSuppressOutput) : (g_stack_trace_message != nullptr);
}
...
OverrideStackTraceOutputForTesting::OverrideStackTraceOutputForTesting(Mode mode)
{
...
NOINLINE size_t TraceStackFramePointers(span<const void*> out_trace, size_t skip_initial, bool enable_scanning)
{
    ...
    while (depth < out_trace.size()) {
        ...
        if (skip_initial != 0) {
            ...
            out_trace[depth++] = reinterpret_cast<const void*>(pc - kCallInstructionSize);
        }
