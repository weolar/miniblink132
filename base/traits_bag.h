...
template <typename... TraitsToExclude> struct Exclude {
    template <typename T> static constexpr auto Filter(T t)
    {
        if constexpr (ParameterPack<TraitsToExclude...>::template HasType<T>::value) {
            return EmptyTrait();
        } else {
            return t;
        }
...
template <class TraitFilterType, class ArgType> constexpr InvalidTrait GetTraitFromArg(CallSecondTag, ArgType arg)
{
    return InvalidTrait();
}
...
constexpr TraitFilterType GetTraitFromArgListImpl(CallFirstTag, ArgTypes... args)
{
    return std::get<TraitFilterType>(std::make_tuple(GetTraitFromArg<TraitFilterType>(CallFirstTag(), args)...));
}

template <class TraitFilterType, class... ArgTypes> constexpr TraitFilterType GetTraitFromArgListImpl(CallSecondTag, ArgTypes... args)
{
    static_assert(std::is_constructible_v<TraitFilterType>, "The traits bag is missing a required trait.");
...
template <class TraitFilterType, class... ArgTypes> constexpr typename TraitFilterType::ValueType GetTraitFromArgList(ArgTypes... args)
{
    static_assert(count({ std::is_constructible_v<TraitFilterType, ArgTypes>... }, true) <= 1, "The traits bag contains multiple traits of the same type.");
    return GetTraitFromArgListImpl<TraitFilterType>(CallFirstTag(), args...);
}

// Helper class to implemnent a |TraitFilterType|.
template <typename T, typename _ValueType = T> struct BasicTraitFilter {
    using ValueType = _ValueType;

    constexpr BasicTraitFilter(ValueType v)
        : value(v)
    {
    }
...
template <typename ArgType, ArgType DefaultValue> struct EnumTraitFilter : public BasicTraitFilter<ArgType> {
    constexpr EnumTraitFilter()
        : BasicTraitFilter<ArgType>(DefaultValue)
    {
    }
    constexpr EnumTraitFilter(ArgType arg)
        : BasicTraitFilter<ArgType>(arg)
    {
    }
};

template <typename ArgType> struct OptionalEnumTraitFilter : public BasicTraitFilter<ArgType, std::optional<ArgType>> {
...
template <typename ArgType> struct RequiredEnumTraitFilter : public BasicTraitFilter<ArgType> {
...
concept IsValidTrait = std::constructible_from<ValidTraits, T> || std::same_as<T, EmptyTrait>;
...
template <typename Enum, typename... Args> static constexpr Enum GetEnum(Args... args)
{
...
template <typename Enum, Enum DefaultValue, typename... Args> static constexpr Enum GetEnum(Args... args)
{
...
// readable.
template <typename Enum, typename... Args> static constexpr std::optional<Enum> GetOptionalEnum(Args... args)
{
...
template <typename Trait, typename... Args> struct HasTrait : ParameterPack<Args...>::template HasType<Trait> {
    static_assert(count({ std::is_constructible_v<Trait, Args>... }, true) <= 1, "The traits bag contains multiple traits of the same type.");
};
