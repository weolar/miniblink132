...
constexpr TimeDelta kConstexprTimeDelta = Milliseconds(kExpectedDeltaInMilliseconds);
...
TimeTicks GetTSC()
{
    ...
    return TimeTicks() + Microseconds(ReadCycleCounter() * Time::kMicrosecondsPerSecond / 10000000);
}
...
TEST(TimeTicks, MAYBE_WinRollover)
{
    ...
    for (int loop = 0; loop < 4096; loop++) {
        ...
        for (int index = 0; index < kThreads; index++) {
            ...
            unsigned thread_id;
            threads[index] = reinterpret_cast<HANDLE>(_beginthreadex(NULL, 0, RolloverTestThreadMain, argument, 0, &thread_id));
...
TEST(TimeTicks, TimerPerformance)
{
    ...
    static_assert(sizeof(TimeTicks) == sizeof(Time), "TimeTicks and Time must be the same size");
    ...
    if (ThreadTicks::IsSupported()) {
        ...
        cases.push_back({ reinterpret_cast<TestFunc>(&ThreadTicks::Now), "ThreadTicks::Now" });
    }
    ...
    for (const auto& test_case : cases) {
        ...
        printf("%s: %1.2fus per call\n", test_case.description, (stop - start).InMillisecondsF() * 1000 / kLoops);
    }
}
...
TEST(TimeTicks, TSCTicksPerSecond)
{
    if (time_internal::HasConstantRateTSC()) {
        ...
        base::win::RegKey processor_key(HKEY_LOCAL_MACHINE, L"Hardware\\Description\\System\\CentralProcessor\\0", KEY_QUERY_VALUE);
        ASSERT_TRUE(processor_key.Valid());
        DWORD processor_mhz_from_registry;
        ASSERT_EQ(ERROR_SUCCESS, processor_key.ReadValueDW(L"~MHz", &processor_mhz_from_registry));
        ...
        EXPECT_NEAR(tsc_mhz_measured, processor_mhz_from_registry, 0.005 * processor_mhz_from_registry);
    }
}
#endif
...
TEST(TimeTicks, FromQPCValue)
{
    ...
    for (int64_t ticks : test_cases) {
        ...
        const double converted_microseconds_since_origin = (converted_value - TimeTicks()).InMicrosecondsF();
        ...
        double epsilon = nextafter(expected_microseconds_since_origin, INFINITY) - expected_microseconds_since_origin;
        ...
        EXPECT_NEAR(expected_microseconds_since_origin, converted_microseconds_since_origin, epsilon)
            << "ticks=" << ticks << ", to be converted via logic path: " << (ticks < Time::kQPCOverflowThreshold ? "FAST" : "SAFE");
    }
}
