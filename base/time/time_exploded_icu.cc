...
std::unique_ptr<icu::Calendar> CreateCalendar(bool is_local)
{
    ...
    if (is_local) {
        ...
        calendar = std::make_unique<icu::GregorianCalendar>(*icu::TimeZone::getGMT(), icu::Locale::getUS(), status);
    }
...
bool ExplodeUsingIcuCalendar(int64_t millis_since_unix_epoch, bool is_local, Time::Exploded* exploded)
{
    ...
    constexpr int64_t kInputLowerBound = -Time::kTimeTToMicrosecondsOffset / Time::kMicrosecondsPerMillisecond;
    static_assert(Time::kTimeTToMicrosecondsOffset % Time::kMicrosecondsPerMillisecond == 0, "assumption: no epoch offset sub-milliseconds");
...
// static
void Time::ExplodeUsingIcu(int64_t millis_since_unix_epoch, bool is_local, Exploded* exploded)
{
...
// static
bool Time::FromExplodedUsingIcu(bool is_local, const Exploded& exploded, int64_t* millis_since_unix_epoch)
{
    // ICU's UCalendarMonths is 0-based. E.g., 0 for January.
    ...
    calendar->set(exploded.year, month.ValueOrDie(), exploded.day_of_month, exploded.hour, exploded.minute, exploded.second);
