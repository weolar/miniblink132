...
std::atomic<TimeNowFunction> g_time_now_from_system_time_function { &subtle::TimeNowFromSystemTimeIgnoringOverride };

std::atomic<TimeTicksNowFunction> g_time_ticks_now_function { &subtle::TimeTicksNowIgnoringOverride };

std::atomic<TimeTicksLowResolutionNowFunction> g_time_ticks_low_resolution_now_function { &subtle::TimeTicksLowResolutionNowIgnoringOverride };

std::atomic<LiveTicksNowFunction> g_live_ticks_now_function { &subtle::LiveTicksNowIgnoringOverride };

std::atomic<ThreadTicksNowFunction> g_thread_ticks_now_function { &subtle::ThreadTicksNowIgnoringOverride };
...
TimeDelta TimeDelta::RoundToMultiple(TimeDelta interval) const
{
    ...
    return (delta_ < 0) ? (*this - half).CeilToMultiple(interval) : (*this + half).FloorToMultiple(interval);
}
...
{
    return std::tie(lhs.year, lhs.month, lhs.day_of_month, lhs.hour, lhs.minute, lhs.second, lhs.millisecond)
        == std::tie(rhs.year, rhs.month, rhs.day_of_month, rhs.hour, rhs.minute, rhs.second, rhs.millisecond);
}
...
int64_t Time::ToRoundedDownMillisecondsSinceUnixEpoch() const
{
    ...
    static_assert(kTimeTToMicrosecondsOffset % kMicrosecondsPerMillisecond == 0, "assumption: no epoch offset sub-milliseconds");
...
std::ostream& operator<<(std::ostream& os, Time time)
{
    ...
    return os << StringPrintf("%04d-%02d-%02d %02d:%02d:%02d.%06" PRId64 " UTC", exploded.year, exploded.month, exploded.day_of_month, exploded.hour,
               exploded.minute, exploded.second, time.ToDeltaSinceWindowsEpoch().InMicroseconds() % Time::kMicrosecondsPerSecond);
}
    ...
    struct StaticUnixEpoch {
        StaticUnixEpoch()
            : epoch(g_shared_time_ticks_at_unix_epoch.is_null()
                    ? subtle::TimeTicksNowIgnoringOverride() - (subtle::TimeNowIgnoringOverride() - Time::UnixEpoch())
...
{
    // clang-format on
    return (1 <= month) && (month <= 12) && (0 <= day_of_week) && (day_of_week <= 6) && (1 <= day_of_month) && (day_of_month <= 31) && (0 <= hour)
        && (hour <= 23) && (0 <= minute) && (minute <= 59) && (0 <= second) && (second <= 60) && (0 <= millisecond) && (millisecond <= 999);
    // clang-format on
}
