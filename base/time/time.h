...
template <typename T> constexpr TimeDelta Microseconds(T n);
...
public:
    ...
    template <typename T> constexpr TimeDelta operator*(T a) const
    {
    ...
    template <typename T> constexpr TimeDelta& operator*=(T a)
    {
        return *this = (*this * a);
    }
    template <typename T> constexpr TimeDelta& operator/=(T a)
    ...
    constexpr int64_t IntDiv(TimeDelta a) const
    {
        ...
        CHECK(!is_zero() || !a.is_zero());
        CHECK(!is_inf() || !a.is_inf());
        return ((delta_ < 0) == (a.delta_ < 0)) ? std::numeric_limits<int64_t>::max() : std::numeric_limits<int64_t>::min();
    }
...
private:
    ...
    constexpr double ToDouble() const
    {
        ...
        return (delta_ < 0) ? -std::numeric_limits<double>::infinity() : std::numeric_limits<double>::infinity();
    }
...
template <typename T> constexpr TimeDelta operator*(T a, TimeDelta td)
{
    return td * a;
}
...
// source or timeline.
template <class TimeClass> class TimeBase {
public:
    ...
    static constexpr int64_t kMillisecondsPerDay = kMillisecondsPerSecond * kSecondsPerHour * kHoursPerDay;
...
protected:
    constexpr explicit TimeBase(int64_t us)
        : us_(us)
    {
    }
...
template <class TimeClass> inline constexpr TimeClass operator+(TimeDelta delta, TimeClass t)
{
    return t + delta;
}
...
// UTCExplode() and LocalExplode(). See method comments for further details.
    ...
    static constexpr int kExplodedMaxYear = std::numeric_limits<int>::max();
#else
    static constexpr int kExplodedMinYear = (sizeof(time_t) == 4 ? 1902 : std::numeric_limits<int>::min());
    static constexpr int kExplodedMaxYear = (sizeof(time_t) == 4 ? 2037 : std::numeric_limits<int>::max());
#endif
    ...
    // double.
    template <typename T>
        requires(std::integral<T> && !std::same_as<T, int64_t> && (sizeof(T) < sizeof(int64_t) || (sizeof(T) == sizeof(int64_t) && std::is_signed_v<T>)))
    ...
    static constexpr Time FromInternalValue(int64_t us)
    {
        return Time(us);
    }
...
private:
    ...
    static void ExplodeUsingIcu(int64_t millis_since_unix_epoch, bool is_local, Exploded* exploded);
    [[nodiscard]] static bool FromExplodedUsingIcu(bool is_local, const Exploded& exploded, int64_t* millis_since_unix_epoch);
    ...
    [[nodiscard]] static bool FromStringInternal(const char* time_string, bool is_local, Time* parsed_time);
    ...
    [[nodiscard]] static bool FromMillisecondsSinceUnixEpoch(int64_t unix_milliseconds, Time* time);
...
template <typename T> constexpr TimeDelta Nanoseconds(T n)
{
    return TimeDelta::FromInternalValue(MakeClampedNum(n) / Time::kNanosecondsPerMicrosecond);
}
template <typename T> constexpr TimeDelta Hertz(T n)
{
    return n ? TimeDelta::FromInternalValue(Time::kMicrosecondsPerSecond / MakeClampedNum(n)) : TimeDelta::Max();
}
...
template <class TimeClass> constexpr TimeDelta TimeBase<TimeClass>::since_origin() const
{
    return Microseconds(us_);
}

template <class TimeClass> constexpr TimeDelta TimeBase<TimeClass>::operator-(const TimeBase<TimeClass>& other) const
{
    return Microseconds(us_ - other.us_);
}

template <class TimeClass> constexpr TimeClass TimeBase<TimeClass>::operator+(TimeDelta delta) const
{
...
constexpr double Time::InSecondsFSinceUnixEpoch() const
{
    ...
    return (us_ < 0) ? -std::numeric_limits<double>::infinity() : std::numeric_limits<double>::infinity();
}
...
{
    return FromSecondsSinceUnixEpoch(ts.tv_sec + static_cast<double>(ts.tv_nsec) / kNanosecondsPerSecond);
}
#endif
...
public:
    ...
    static mach_timebase_info_data_t SetMachTimebaseInfoForTesting(mach_timebase_info_data_t timebase);
...
private:
    ...
    // Please use Now() to create a new object. This is for internal use
    // and testing.
    constexpr explicit LiveTicks(int64_t us)
        : TimeBase(us)
    {
    }
};
...
public:
    ...
    {
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FUCHSIA)
        return true;
...
private:
    ...
    constexpr explicit ThreadTicks(int64_t us)
        : TimeBase(us)
    {
    }
