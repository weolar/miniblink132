...
// adding the icui18n dep significantly increases the binary size.
    ...
    // granularity.
    auto seconds = base::checked_cast<SysTime>(millis_since_unix_epoch / Time::kMillisecondsPerSecond);
...
bool Time::FromExploded(bool is_local, const Exploded& exploded, Time* time)
{
    ...
    if (seconds == -1 && (exploded.year < 1969 || exploded.year > 1970)) {
        ...
        // overflows.
        const int64_t min_seconds = (sizeof(SysTime) < sizeof(int64_t)) ? std::numeric_limits<SysTime>::min() : std::numeric_limits<int32_t>::min();
        const int64_t max_seconds = (sizeof(SysTime) < sizeof(int64_t)) ? std::numeric_limits<SysTime>::max() : std::numeric_limits<int32_t>::max();
