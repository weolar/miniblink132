...
std::optional<base::TimeDelta> GetTimeZoneOffsetAtTime(const char* timezone_id, Time time)
{
    ...
    tz->getOffset(time.InSecondsFSinceUnixEpoch(), false, raw_offset, dst_offset, ec);
    if (!U_SUCCESS(ec)) {
        return {};
    }
...
{
    return base::Hours(time.hour) + base::Minutes(time.minute) + base::Seconds(time.second) + base::Milliseconds(time.millisecond);
}
...
TEST_F(TimeTest, DeltaSinceWindowsEpoch)
{
    ...
    EXPECT_EQ(delta, Time::FromDeltaSinceWindowsEpoch(delta).ToDeltaSinceWindowsEpoch());

    const Time now = Time::Now();
    const Time actual = Time::FromDeltaSinceWindowsEpoch(now.ToDeltaSinceWindowsEpoch());
    ...
    // deserialization.
    const Time should_be_null = Time::FromDeltaSinceWindowsEpoch(Time().ToDeltaSinceWindowsEpoch());
    EXPECT_TRUE(should_be_null.is_null());

    {
        constexpr Time constexpr_time = Time::FromDeltaSinceWindowsEpoch(Microseconds(123));
        constexpr TimeDelta constexpr_delta = constexpr_time.ToDeltaSinceWindowsEpoch();
...
TEST_F(TimeTest, LocalExplodeIsLocal)
{
    ...
    EXPECT_TRUE(Time::FromLocalExploded(local_exploded, &time_from_local_exploded));
    ...
    EXPECT_EQ(comparison_time_utc + Hours(kHonoluluOffsetHours), time_from_utc_exploded);
}
...
TEST_F(TimeTest, LocalMidnightIsLocal)
{
    ...
    EXPECT_EQ(utc_midnight_exploded.day_of_week, local_midnight_utc_exploded.day_of_week);
    EXPECT_EQ(utc_midnight_exploded.day_of_month, local_midnight_utc_exploded.day_of_month);
    ...
    EXPECT_EQ(utc_midnight_exploded.day_of_week, local_midnight_exploded.day_of_week);
    EXPECT_EQ(utc_midnight_exploded.day_of_month, local_midnight_exploded.day_of_month);
...
TEST_F(TimeTest, MaxConversions)
{
    ...
    Time t = Time::FromSecondsSinceUnixEpoch(std::numeric_limits<double>::infinity());
    EXPECT_TRUE(t.is_max());
    EXPECT_EQ(std::numeric_limits<double>::infinity(), t.InSecondsFSinceUnixEpoch());

    t = Time::FromMillisecondsSinceUnixEpoch(std::numeric_limits<double>::infinity());
    EXPECT_TRUE(t.is_max());
    EXPECT_EQ(std::numeric_limits<double>::infinity(), t.InMillisecondsFSinceUnixEpoch());
    ...
    EXPECT_EQ(static_cast<suseconds_t>(Time::kMicrosecondsPerSecond) - 1, tval.tv_usec);
#endif

#if BUILDFLAG(IS_APPLE)
    t = Time::FromCFAbsoluteTime(std::numeric_limits<CFAbsoluteTime>::infinity());
    EXPECT_TRUE(t.is_max());
    EXPECT_EQ(std::numeric_limits<CFAbsoluteTime>::infinity(), t.ToCFAbsoluteTime());
#endif
...
TEST_F(TimeTest, TimeTOverflow)
{
    ...
    // value of time_t.
    if (time_t_is_32_bit) {
        constexpr Time kMaxMinusOne = Time() + base::Microseconds(std::numeric_limits<int64_t>::max() - 1);
    ...
    constexpr Time kMinPlusOne = Time() + base::Microseconds(std::numeric_limits<int64_t>::min() + 1);
    ...
    constexpr Time kMinPlusUnix = Time() + base::Microseconds(std::numeric_limits<int64_t>::min() + Time::kTimeTToMicrosecondsOffset);
    ...
    constexpr Time kMinPlusUnixPlusOne = Time() + base::Microseconds(std::numeric_limits<int64_t>::min() + Time::kTimeTToMicrosecondsOffset + 1);
    static_assert(!kMinPlusUnixPlusOne.is_min());
    if (time_t_is_32_bit) {
        EXPECT_EQ(std::numeric_limits<time_t>::min(), kMinPlusUnixPlusOne.ToTimeT());
    } else {
        EXPECT_NE(std::numeric_limits<time_t>::min(), kMinPlusUnixPlusOne.ToTimeT());
    }
}
...
TEST_F(TimeTest, Explode_Y10KCompliance)
{
    ...
    constexpr int64_t kYear2038MicrosOffset = Time::kTimeTToMicrosecondsOffset + (std::numeric_limits<int32_t>::max() * Time::kMicrosecondsPerSecond);
    ...
    constexpr int64_t kMarch10000MicrosOffset = Time::kTimeTToMicrosecondsOffset
        + Days(kYear10000YearsOffset * kDaysPerYear + kExtraLeapDaysOverThoseYears + kDaysFromJanToMar10000).InMicroseconds() + Hours(12).InMicroseconds();

    // Windows uses a 64-bit signed integer type that reperesents the number of
    // 1/10 microsecond ticks.
    constexpr int64_t kWindowsMaxMicrosOffset = std::numeric_limits<int64_t>::max() / 10;
    ...
    constexpr int64_t kMaxIntegerAsDoubleMillis = int64_t { 1 } << std::numeric_limits<double>::digits;
    constexpr int64_t kIcuMaxMicrosOffset = Time::kTimeTToMicrosecondsOffset + (kMaxIntegerAsDoubleMillis * Time::kMicrosecondsPerMillisecond + 999);

    const auto make_time = [](int64_t micros) { return Time::FromDeltaSinceWindowsEpoch(Microseconds(micros)); };
    ...
    const struct TestCase {
        ...
        { make_time(-kHalfYearInMicros), Time::Exploded { 1600, 7, 1, 3, 0, 0, 0, 0 } },
        ...
        { make_time(Time::kTimeTToMicrosecondsOffset - kHalfYearInMicros), Time::Exploded { 1969, 7, 4, 3, 0, 0, 0, 0 } },
        { make_time(Time::kTimeTToMicrosecondsOffset), Time::Exploded { 1970, 1, 4, 1, 0, 0, 0, 0 } },
        { make_time(Time::kTimeTToMicrosecondsOffset + kHalfYearInMicros), Time::Exploded { 1970, 7, 4, 2, 0, 0, 0, 0 } },

        // Before/On/After 19 January 2038.
        { make_time(kYear2038MicrosOffset - kHalfYearInMicros), Time::Exploded { 2037, 7, 2, 21, 3, 14, 7, 0 } },
        { make_time(kYear2038MicrosOffset), Time::Exploded { 2038, 1, 2, 19, 3, 14, 7, 0 } },
        { make_time(kYear2038MicrosOffset + kHalfYearInMicros), Time::Exploded { 2038, 7, 2, 20, 3, 14, 7, 0 } },

        // Before/On/After 1 March 10000 at noon.
        { make_time(kMarch10000MicrosOffset - kHalfYearInMicros), Time::Exploded { 9999, 9, 3, 1, 12, 0, 0, 0 } },
        { make_time(kMarch10000MicrosOffset), Time::Exploded { 10000, 3, 3, 1, 12, 0, 0, 0 } },
        { make_time(kMarch10000MicrosOffset + kHalfYearInMicros), Time::Exploded { 10000, 8, 3, 30, 12, 0, 0, 0 } },

        // Before/On/After Windows Max (14 September 30828).
        { make_time(kWindowsMaxMicrosOffset - kHalfYearInMicros), Time::Exploded { 30828, 3, 4, 16, 2, 48, 5, 477 } },
        { make_time(kWindowsMaxMicrosOffset), Time::Exploded { 30828, 9, 4, 14, 2, 48, 5, 477 } },
        { make_time(kWindowsMaxMicrosOffset + kHalfYearInMicros), Time::Exploded { 30829, 3, 4, 15, 2, 48, 5, 477 } },

        // Before/On/After ICU Max.
        { make_time(kIcuMaxMicrosOffset - kHalfYearInMicros), Time::Exploded { 287396, 4, 3, 13, 8, 59, 0, 992 } },
        { make_time(kIcuMaxMicrosOffset), Time::Exploded { 287396, 10, 3, 12, 8, 59, 0, 992 } },
        { make_time(kIcuMaxMicrosOffset + kHalfYearInMicros), Time::Exploded { 287397, 4, 3, 12, 8, 59, 0, 992 } },
    ...
    for (const TestCase& test_case : kTestCases) {
        ...
        if (exploded.HasValidValues()) {
            ...
            const bool is_in_range = test_case.time >= make_time(0) && test_case.time <= make_time(kWindowsMaxMicrosOffset);
...
TEST_F(TimeTest, LocalExplodedIsLocaleIndependent)
{
    ...
    std::optional<TimeDelta> expected_delta = GetTimeZoneOffsetAtTime(kBangkokTimeZoneId, time);
    ...
    TimeDelta actual_delta = TimePassedAfterMidnight(local_exploded) - TimePassedAfterMidnight(utc_exploded_orig);
...
TEST_F(TimeTest, MAYBE_NowOverride)
{
    ...
    {
        // Set override.
        subtle::ScopedTimeClockOverrides overrides(&TimeOverride::Now, nullptr, nullptr);
...
TEST(TimeTicks, MaybeHighRes)
{
    ...
    TimeDelta delta;
    while (!success && retries--) {
        std::optional<TimeTicks> maybe_ticks = subtle::MaybeTimeTicksNowIgnoringOverride();
...
TEST(TimeTicks, NowOverride)
{
    ...
    {
        // Set override.
        subtle::ScopedTimeClockOverrides overrides(nullptr, &TimeTicksOverride::Now, nullptr);
...
TEST(TimeTicks, LowResolutionNowOverride)
{
    ...
    EXPECT_LT(TimeTicks::UnixEpoch(), subtle::TimeTicksLowResolutionNowIgnoringOverride());
    EXPECT_GT(TimeTicks::Max(), subtle::TimeTicksLowResolutionNowIgnoringOverride());

    {
        // Set override.
        subtle::ScopedTimeClockOverrides overrides(nullptr, nullptr, nullptr, nullptr, &TimeTicksLowResolutionOverride::Now);
...
TEST(ThreadTicks, MAYBE_NowOverride)
{
    ...
    {
        // Set override.
        subtle::ScopedTimeClockOverrides overrides(nullptr, nullptr, &ThreadTicksOverride::Now);
...
TEST(TimeTicks, SnappedToNextTickOverflow)
{
    ...
    EXPECT_EQ(8635916564000, big_timestamp.SnappedToNextTick(phase, interval).ToInternalValue());
    EXPECT_EQ(8635916564000, big_timestamp.SnappedToNextTick(big_timestamp, interval).ToInternalValue());
}
...
TEST(TimeTicks, Android_FromUptimeMillis_ClocksMatch)
{
    ...
    android::ScopedJavaLocalRef<jclass> clazz(android::GetClass(env, "android/os/SystemClock"));
    ASSERT_TRUE(clazz.obj());
    const jmethodID method_id = android::MethodID::Get<android::MethodID::TYPE_STATIC>(env, clazz.obj(), "uptimeMillis", "()J");
    ...
    const TimeTicks converted_ticks = TimeTicks::FromUptimeMillis(env->CallStaticLongMethod(clazz.obj(), method_id));
...
{
    JNIEnv* const env = android::AttachCurrentThread();
    android::ScopedJavaLocalRef<jclass> clazz(android::GetClass(env, "java/lang/System"));
    ASSERT_TRUE(clazz.obj());
    const jmethodID method_id = android::MethodID::Get<android::MethodID::TYPE_STATIC>(env, clazz.obj(), "nanoTime", "()J");
    ASSERT_FALSE(!method_id);
    const TimeTicks lower_bound_ticks = TimeTicks::Now();
    const TimeTicks converted_ticks = TimeTicks::FromJavaNanoTime(env->CallStaticLongMethod(clazz.obj(), method_id));
...
TEST(LiveTicks, NowOverride)
{
    ...
    {
        // Set override.
        subtle::ScopedTimeClockOverrides overrides(nullptr, nullptr, nullptr, &LiveTicksOverride::Now);
...
{
    constexpr TimeDelta kLargeDelta = Microseconds(std::numeric_limits<int64_t>::max() - 1);
    ...
    static_assert(std::numeric_limits<int64_t>::max() == kLargeDelta.InNanoseconds(), "");

    constexpr TimeDelta kLargeNegative = Microseconds(std::numeric_limits<int64_t>::min() + 1);
    ...
    static_assert(std::numeric_limits<int64_t>::min() == kLargeNegative.InNanoseconds(), "");
}
...
TEST(TimeDelta, Magnitude)
{
    ...
    constexpr int64_t max_int64_minus_one = std::numeric_limits<int64_t>::max() - 1;
    constexpr int64_t min_int64_plus_two = std::numeric_limits<int64_t>::min() + 2;
    static_assert(Microseconds(max_int64_minus_one) == Microseconds(max_int64_minus_one).magnitude(), "");
    static_assert(Microseconds(max_int64_minus_one) == Microseconds(min_int64_plus_two).magnitude(), "");
...
TEST(TimeDelta, MaxConversions)
{
    ...
    static_assert(kMax.InMillisecondsF() == std::numeric_limits<double>::infinity());
    static_assert(kMax.InMilliseconds() == std::numeric_limits<int64_t>::max());
    static_assert(kMax.InMillisecondsRoundedUp() == std::numeric_limits<int64_t>::max());
    ...
    static_assert(Milliseconds(max_int / Time::kMillisecondsPerSecond + 1).is_max(), "");
    ...
    static_assert(Microseconds(max_d).is_max(), "Make sure that 2^63 correctly gets clamped to `max` (crbug.com/612601)");
...
TEST(TimeDelta, MinConversions)
{
    ...
    static_assert(kMin.InMillisecondsF() == -std::numeric_limits<double>::infinity());
    static_assert(kMin.InMilliseconds() == std::numeric_limits<int64_t>::min());
    static_assert(kMin.InMillisecondsRoundedUp() == std::numeric_limits<int64_t>::min());
}
...
TEST(TimeDelta, Overflows)
{
    ...
    // +/- infinities.
    static_assert(TimeDelta::Max() / kOneSecond == std::numeric_limits<double>::infinity(), "");
    static_assert(TimeDelta::Max() / -kOneSecond == -std::numeric_limits<double>::infinity(), "");
    static_assert(TimeDelta::Min() / kOneSecond == -std::numeric_limits<double>::infinity(), "");
    static_assert(TimeDelta::Min() / -kOneSecond == std::numeric_limits<double>::infinity(), "");
    static_assert(TimeDelta::Max().IntDiv(kOneSecond) == std::numeric_limits<int64_t>::max(), "");
    static_assert(TimeDelta::Max().IntDiv(-kOneSecond) == std::numeric_limits<int64_t>::min(), "");
    static_assert(TimeDelta::Min().IntDiv(kOneSecond) == std::numeric_limits<int64_t>::min(), "");
    static_assert(TimeDelta::Min().IntDiv(-kOneSecond) == std::numeric_limits<int64_t>::max(), "");
    static_assert(TimeDelta::Max() % kOneSecond == TimeDelta::Max());
    static_assert(TimeDelta::Max() % -kOneSecond == TimeDelta::Max());
    static_assert(TimeDelta::Min() % kOneSecond == TimeDelta::Min());
    static_assert(TimeDelta::Min() % -kOneSecond == TimeDelta::Min());
    ...
    EXPECT_EQ(-std::numeric_limits<double>::infinity(), -kOneSecond / TimeDelta());
    EXPECT_EQ(std::numeric_limits<double>::infinity(), TimeDelta::Max() / TimeDelta());
    EXPECT_EQ(-std::numeric_limits<double>::infinity(), TimeDelta::Min() / TimeDelta());
    static_assert(kOneSecond.IntDiv(TimeDelta()) == std::numeric_limits<int64_t>::max(), "");
    static_assert((-kOneSecond).IntDiv(TimeDelta()) == std::numeric_limits<int64_t>::min(), "");
    static_assert(TimeDelta::Max().IntDiv(TimeDelta()) == std::numeric_limits<int64_t>::max(), "");
    static_assert(TimeDelta::Min().IntDiv(TimeDelta()) == std::numeric_limits<int64_t>::min(), "");
    ...
    static_assert((kLargeDelta + kOneSecond).InSecondsF() == std::numeric_limits<double>::infinity(), "");
    static_assert((kLargeDelta + kOneSecond).InMillisecondsF() == std::numeric_limits<double>::infinity());
    static_assert((kLargeDelta + kOneSecond).InMicrosecondsF() == std::numeric_limits<double>::infinity());
...
{
    constexpr TimeTicks kLargeTimeTicks = TimeTicks::FromInternalValue(std::numeric_limits<int64_t>::max() - 1);

    constexpr TimeTicks kLargeNegativeTimeTicks = TimeTicks::FromInternalValue(std::numeric_limits<int64_t>::min() + 1);
    ...
    EXPECT_TRUE((TimeTicks::Now() + TimeDelta::Max()).is_max()) << (TimeTicks::Now() + TimeDelta::Max());
    EXPECT_TRUE((TimeTicks::Now() - TimeDelta::Max()).is_min()) << (TimeTicks::Now() - TimeDelta::Max());
    ...
    EXPECT_TRUE((TimeTicks::Now() + TimeDelta::Min()).is_min()) << (TimeTicks::Now() + TimeDelta::Min());
    EXPECT_TRUE((TimeTicks::Now() - TimeDelta::Min()).is_max()) << (TimeTicks::Now() - TimeDelta::Min());
}
...
TEST(TimeTicks, ConstexprAndTriviallyCopiable)
{
    ...
    // Copy assignment.
    static_assert(a.ToInternalValue() == TestTimeTicksConstexprCopyAssignment().ToInternalValue(), "");
}
...
TEST(TimeLogging, Microseconds)
{
    ...
    Time now = Time::Now();
    if (now.ToDeltaSinceWindowsEpoch().InMicroseconds() % Time::kMicrosecondsPerMillisecond == 0) {
        now += Microseconds(1);
    }
