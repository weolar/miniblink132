...
{
    DCHECK(CanConvertToFileTime(us)) << "Out-of-range: Cannot convert " << us << " microseconds to FILETIME units.";
...
void UpdateTimerIntervalLocked()
{
    ...
    if (g_last_interval_requested_ms) {
        ...
        g_high_res_timer_usage += subtle::TimeTicksNowIgnoringOverride() - g_high_res_timer_last_activation;
...
bool Time::FromExploded(bool is_local, const Exploded& exploded, Time* time)
{
    ...
    SYSTEMTIME st;
    if (!SafeConvertToWord(exploded.year, &st.wYear) || !SafeConvertToWord(exploded.month, &st.wMonth)
        || !SafeConvertToWord(exploded.day_of_week, &st.wDayOfWeek) || !SafeConvertToWord(exploded.day_of_month, &st.wDay)
        || !SafeConvertToWord(exploded.hour, &st.wHour) || !SafeConvertToWord(exploded.minute, &st.wMinute) || !SafeConvertToWord(exploded.second, &st.wSecond)
...
static_assert(sizeof(LastTimeAndRolloversState) <= sizeof(g_last_time_and_rollovers), "LastTimeAndRolloversState does not fit in a single atomic word");
...
TimeTicks RolloverProtectedNow()
{
    ...
    while (true) {
        ...
        bool success = g_last_time_and_rollovers.compare_exchange_strong(original, state.as_opaque_32, std::memory_order_release);
        if (success)
            break;
    ...
    return TimeTicks() + Milliseconds(now + (static_cast<uint64_t>(state.as_values.rollovers) << 32));
}
...
std::atomic<TimeTicksNowFunction> g_time_ticks_now_ignoring_override_function { &InitialNowFunction };
...
TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value)
{
    ...
    if (qpc_value < Time::kQPCOverflowThreshold) {
        return Microseconds(qpc_value * Time::kMicrosecondsPerSecond / g_qpc_ticks_per_second);
    }
    // Otherwise, calculate microseconds in a round about manner to avoid
    // overflow and precision issues.
    int64_t whole_seconds = qpc_value / g_qpc_ticks_per_second;
    int64_t leftover_ticks = qpc_value - (whole_seconds * g_qpc_ticks_per_second);
    return Microseconds((whole_seconds * Time::kMicrosecondsPerSecond) + ((leftover_ticks * Time::kMicrosecondsPerSecond) / g_qpc_ticks_per_second));
}
...
void InitializeNowFunctionPointer()
{
    ...
    CPU cpu;
    const TimeTicksNowFunction now_function = (ticks_per_sec.QuadPart <= 0 || !cpu.has_non_stop_time_stamp_counter()) ? &RolloverProtectedNow : &QPCNow;
    ...
    base::TimeTicksNowFunction initial_time_ticks_now_function = &subtle::TimeTicksNowIgnoringOverride;
    internal::g_time_ticks_now_function.compare_exchange_strong(initial_time_ticks_now_function, now_function, std::memory_order_relaxed);
...
TimeTicks InitialNowFunction()
{
    ...
    return g_time_ticks_now_ignoring_override_function.load(std::memory_order_relaxed)();
}

} // namespace

// static
TimeTicks::TickFunctionType TimeTicks::SetMockTickFunction(TickFunctionType ticker)
{
...
// static
ThreadTicks ThreadTicks::GetForThread(const PlatformThreadHandle& thread_handle)
{
    DCHECK(IsSupported());
    ...
    ::GetThreadTimes(thread_handle.platform_handle(), &creation_time, &exit_time, &kernel_time, &user_time);
...
double TSCTicksPerSecond()
{
    ...
    const double elapsed_time_seconds = perf_counter_ticks / static_cast<double>(perf_counter_frequency.QuadPart);
