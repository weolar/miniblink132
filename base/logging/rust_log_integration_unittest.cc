...
TEST_F(RustLogIntegrationTest, MAYBE_CheckAllSeverity)
{
    ...
    EXPECT_CALL(log_, Log(LOGGING_INFO, _, _, _, testing::HasSubstr("test trace log"))).WillOnce(testing::Return(true));

    EXPECT_CALL(log_, Log(LOGGING_INFO, _, _, _, testing::HasSubstr("test debug log"))).WillOnce(testing::Return(true));
#endif

    EXPECT_CALL(log_, Log(LOGGING_INFO, _, _, _, testing::HasSubstr("test info log"))).WillOnce(testing::Return(true));

    EXPECT_CALL(log_, Log(LOGGING_WARNING, _, _, _, testing::HasSubstr("test warning log"))).WillOnce(testing::Return(true));

    EXPECT_CALL(log_, Log(LOGGING_ERROR, _, _, _, testing::HasSubstr("test error log"))).WillOnce(testing::Return(true));
...
{
    EXPECT_CALL(log_, Log(LOGGING_ERROR, _, _, _, testing::HasSubstr("test log with placeholder 2"))).WillOnce(testing::Return(true));
