...
VARIANT_ID=workstation)";
    ...
    std::string value = base::GetKeyValueFromOSReleaseFileForTesting(kOsRelease, kPrettyName);
    EXPECT_EQ(value, "Fedora 30 (Workstation Edition)");
    // Missing key in the file
    value = base::GetKeyValueFromOSReleaseFileForTesting(kOsReleaseMissingPrettyName, kPrettyName);
    EXPECT_EQ(value, "");
    // Value quoted with single ticks
    value = base::GetKeyValueFromOSReleaseFileForTesting(kOsReleaseMissingPrettyName, "VERSION");
    EXPECT_EQ(value, "30 (Workstation Edition)");
    // Empty file
    value = base::GetKeyValueFromOSReleaseFileForTesting("", kPrettyName);
    EXPECT_EQ(value, "");
    // Misspelled key
    value = base::GetKeyValueFromOSReleaseFileForTesting(kOsRelease, "PRETY_NAME");
    EXPECT_EQ(value, "");
    // Broken key=value format
    value = base::GetKeyValueFromOSReleaseFileForTesting("A/B", kPrettyName);
    EXPECT_EQ(value, "");
    // Empty values
    value = base::GetKeyValueFromOSReleaseFileForTesting("PRETTY_NAME=", kPrettyName);
    EXPECT_EQ(value, "");
    value = base::GetKeyValueFromOSReleaseFileForTesting("PRETTY_NAME=\"\"", kPrettyName);
    EXPECT_EQ(value, "");
    // Only one key=value in the whole file
    value = base::GetKeyValueFromOSReleaseFileForTesting("PRETTY_NAME=\"Linux\"", kPrettyName);
