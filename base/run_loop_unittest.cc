...
void RunNestedLoopTask(int* counter)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&nested_run_loop), Unretained(counter)));

    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
public:
    ...
    SimpleSingleThreadTaskRunner& operator=(const SimpleSingleThreadTaskRunner&) = delete;

    bool PostDelayedTask(const Location& from_here, OnceClosure task, base::TimeDelta delay) override
    {
    ...
    bool PostNonNestableDelayedTask(const Location& from_here, OnceClosure task, base::TimeDelta delay) override
    {
...
public:
    ...
    {
        thread_task_runner_handle_ = std::make_unique<SingleThreadTaskRunner::CurrentDefaultHandle>(simple_task_runner_);
...
private:
    ...
    scoped_refptr<SimpleSingleThreadTaskRunner> simple_task_runner_ = MakeRefCounted<SimpleSingleThreadTaskRunner>();

    std::unique_ptr<SingleThreadTaskRunner::CurrentDefaultHandle> thread_task_runner_handle_;
...
{
    int counter = 0;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&run_loop_), Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
{
    int counter = 0;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&RunNestedLoopTask, Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&run_loop_), Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
{
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_loop_.QuitWhenIdleClosure());
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
TEST_P(RunLoopTest, QuitFromOtherSequence)
{
    ...
    other_thread.Start();
    scoped_refptr<SequencedTaskRunner> other_sequence = other_thread.task_runner();

    // Always expected to run before asynchronous Quit() kicks in.
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));

    WaitableEvent loop_was_quit(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    other_sequence->PostTask(FROM_HERE, base::BindOnce([](RunLoop* run_loop) { run_loop->Quit(); }, Unretained(&run_loop_)));
    other_sequence->PostTask(FROM_HERE, base::BindOnce(&WaitableEvent::Signal, base::Unretained(&loop_was_quit)));

    // Anything that's posted after the Quit closure was posted back to this
    // sequence shouldn't get a chance to run.
    loop_was_quit.Wait();
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE));
...
TEST_P(RunLoopTest, QuitWhenIdleFromOtherSequence)
{
    ...
    other_sequence->PostTask(FROM_HERE, base::BindOnce([](RunLoop* run_loop) { run_loop->QuitWhenIdle(); }, Unretained(&run_loop_)));
...
TEST_P(RunLoopTest, IsRunningOnCurrentThread)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([] { EXPECT_TRUE(RunLoop::IsRunningOnCurrentThread()); }));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_loop_.QuitClosure());
    run_loop_.Run();
}
...
TEST_P(RunLoopTest, IsNestedOnCurrentThread)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([] {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([] { EXPECT_TRUE(RunLoop::IsNestedOnCurrentThread()); }));
...
TEST_P(RunLoopTest, NestingObservers)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_nested_loop);
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&MockTask::Task, base::Unretained(&mock_task_a)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_nested_loop);
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&MockTask::Task, base::Unretained(&mock_task_b)));
...
INSTANTIATE_TEST_SUITE_P(Real, RunLoopTest, testing::Values(RunLoopTestType::kRealEnvironment));
INSTANTIATE_TEST_SUITE_P(Mock, RunLoopTest, testing::Values(RunLoopTestType::kTestDelegate));
...
TEST(RunLoopDelegateTest, NestableTasksDontRunInDefaultNestedLoops)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce([](RunLoop* nested_run_loop) { nested_run_loop->Run(); }, Unretained(&nested_run_loop)));
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
        BindOnce(
            [](const bool& nested_run_loop_ended, OnceClosure continuation_callback) {
    ...
    other_thread.task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce(
    ...
    other_thread.task_runner()->PostDelayedTask(FROM_HERE,
        BindOnce([](TestBoundDelegate* test_delegate, OnceClosure injected_closure) { test_delegate->InjectClosureOnDelegate(std::move(injected_closure)); },
