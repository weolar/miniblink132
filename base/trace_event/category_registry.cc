...
static_assert(std::is_trivial_v<TraceCategory> && std::is_standard_layout_v<TraceCategory>, "TraceCategory must be POD");

} // namespace

// static
TraceCategory CategoryRegistry::categories_[kMaxCategories] = { INTERNAL_TRACE_LIST_BUILTIN_CATEGORIES(INTERNAL_TRACE_INIT_CATEGORY) };

// static
std::atomic<size_t> CategoryRegistry::category_index_ { BuiltinCategories::Size() };

// static
TraceCategory* const CategoryRegistry::kCategoryAlreadyShutdown = &categories_[1];
...
void CategoryRegistry::Initialize()
{
    ...
    for (size_t i = 0; i < kMaxCategories; ++i) {
        ABSL_ANNOTATE_BENIGN_RACE(categories_[i].state_ptr(), "trace_event category enabled");
...
{
    DCHECK(!strchr(category_name, '"')) << "Category names may not contain double quote";
...
bool CategoryRegistry::GetOrCreateCategoryLocked(const char* category_name, CategoryInitializerFn category_initializer_fn, TraceCategory** category)
{
...
// static
const TraceCategory* CategoryRegistry::GetCategoryByStatePtr(const uint8_t* category_state)
{
...
bool CategoryRegistry::IsValidCategoryPtr(const TraceCategory* category)
{
    ...
    return ptr % sizeof(void*) == 0 && ptr >= reinterpret_cast<uintptr_t>(&categories_[0])
