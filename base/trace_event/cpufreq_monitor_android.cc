...
std::string CPUFreqMonitorDelegate::GetScalingCurFreqPathString(unsigned int cpu_id) const
{
    return base::StringPrintf("/sys/devices/system/cpu/cpu%d/cpufreq/scaling_cur_freq", cpu_id);
}
...
{
    std::string str;
    if (!base::ReadFileToString(base::FilePath("/sys/devices/system/cpu/kernel_max"), &str)) {
...
std::string CPUFreqMonitorDelegate::GetRelatedCPUsPathString(unsigned int cpu_id) const
{
    return base::StringPrintf("/sys/devices/system/cpu/cpu%d/cpufreq/related_cpus", cpu_id);
}
...
void CPUFreqMonitorDelegate::GetCPUIds(std::vector<unsigned int>* ids) const
{
    ...
    for (unsigned int i = 0; i <= kernel_max_cpu; i++) {
        ...
        for (auto& str_piece : base::SplitString(line, " ", base::WhitespaceHandling::TRIM_WHITESPACE, base::SplitResult::SPLIT_WANT_NONEMPTY)) {
            unsigned int cpu_id;
...
void CPUFreqMonitorDelegate::RecordFrequency(unsigned int cpu_id, unsigned int freq)
{
    TRACE_COUNTER_ID1(kTraceCategory, kEventTitle, cpu_id, freq);
}

scoped_refptr<SingleThreadTaskRunner> CPUFreqMonitorDelegate::CreateTaskRunner()
{
    return base::ThreadPool::CreateSingleThreadTaskRunner(
        { base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN, base::TaskPriority::BEST_EFFORT }, base::SingleThreadTaskRunnerThreadMode::SHARED);
}
...
{
    GetOrCreateTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&CPUFreqMonitor::Start, weak_ptr_factory_.GetWeakPtr()));
}
...
void CPUFreqMonitor::Start()
{
    ...
    if (is_enabled_.load(std::memory_order_relaxed) || !delegate_->IsTraceCategoryEnabled()) {
        return;
    }
    ...
    GetOrCreateTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&CPUFreqMonitor::Sample, weak_ptr_factory_.GetWeakPtr(), std::move(fds)));
}
...
void CPUFreqMonitor::Sample(std::vector<std::pair<unsigned int, base::ScopedFD>> fds)
{
    ...
    GetOrCreateTaskRunner()->PostDelayedTask(FROM_HERE, base::BindOnce(&CPUFreqMonitor::Sample, weak_ptr_factory_.GetWeakPtr(), std::move(fds)),
...
const scoped_refptr<SingleThreadTaskRunner>& CPUFreqMonitor::GetOrCreateTaskRunner()
{
    if (!task_runner_)
