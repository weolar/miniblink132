...
TraceEventETWExport::TraceEventETWExport()
{
    ...
    etw_provider_
        = std::make_unique<TlmProvider>("Google.Chrome", Chrome_GUID, base::BindRepeating(&TraceEventETWExport::OnETWEnableUpdate, base::Unretained(this)));
    ...
    // for keyword use.
    static_assert(std::size(kFilteredEventGroupNames) <= kMaxNumberOfGroupNames, "Exceeded max ETW keyword bits");
}
...
void TraceEventETWExport::AddEvent(
    char phase, const unsigned char* category_group_enabled, const char* name, unsigned long long id, TimeTicks timestamp, const TraceArguments* args)
{
    ...
    uint64_t keyword = instance->CategoryStateToETWKeyword(category_group_enabled);
    ...
    for (size_t i = 0; i < num_args; i++) {
        ...
        if (type == TRACE_VALUE_TYPE_CONVERTABLE || type == TRACE_VALUE_TYPE_PROTO) {
    ...
    if (num_args == 0) {
        instance->etw_provider_->WriteEvent(
            name, TlmEventDescriptor(0, keyword), TlmMbcsStringField("Phase", phase_string), TlmInt64Field("Timestamp", timestamp_ms));
    } else if (num_args == 1) {
        instance->etw_provider_->WriteEvent(name, TlmEventDescriptor(0, keyword), TlmMbcsStringField("Phase", phase_string),
            TlmInt64Field("Timestamp", timestamp_ms), TlmMbcsStringField((args->names()[0]), (arg_values_string[0].c_str())));
    } else if (num_args == 2) {
        instance->etw_provider_->WriteEvent(name, TlmEventDescriptor(0, keyword), TlmMbcsStringField("Phase", phase_string),
            TlmInt64Field("Timestamp", timestamp_ms), TlmMbcsStringField((args->names()[0]), (arg_values_string[0].c_str())),
...
// static
void TraceEventETWExport::AddCompleteEndEvent(const unsigned char* category_group_enabled, const char* name)
{
    auto* instance = GetInstance();
    if (!instance) {
        return;
    ...
    // Log the event and include the info needed to decode it via TraceLogging
    instance->etw_provider_->WriteEvent(name, TlmEventDescriptor(0, keyword), TlmMbcsStringField("Phase", "Complete End"));
}

// static
bool TraceEventETWExport::IsCategoryGroupEnabled(std::string_view category_group_name)
{
    ...
    StringViewTokenizer category_group_tokens(category_group_name.begin(), category_group_name.end(), ",");
...
{
    if (etw_match_any_keyword_ == (etw_provider_->keyword_any() & kCategoryKeywordMask)) {
        return false;
    }
...
bool TraceEventETWExport::IsCategoryEnabled(std::string_view category_name) const
{
    ...
    if (StartsWith(category_name, "disabled-by-default")) {
        DCHECK(categories_status_.find(kFilteredEventGroupNames[kDisabledOtherEventsGroupNameIndex]) != categories_status_.end());
        return categories_status_.find(kFilteredEventGroupNames[kDisabledOtherEventsGroupNameIndex])->second;
    } else {
        DCHECK(categories_status_.find(kFilteredEventGroupNames[kOtherEventsGroupNameIndex]) != categories_status_.end());
        return categories_status_.find(kFilteredEventGroupNames[kOtherEventsGroupNameIndex])->second;
    }
}

uint64_t TraceEventETWExport::CategoryStateToETWKeyword(const uint8_t* category_state)
{
...
void TraceEventETWExport::OnETWEnableUpdate(TlmProvider::EventControlCode enabled)
{
...
{
    return Singleton<TraceEventETWExport, StaticMemorySingletonTraits<TraceEventETWExport>>::get();
}

// static
TraceEventETWExport* TraceEventETWExport::GetInstanceIfExists()
{
    return Singleton<TraceEventETWExport, StaticMemorySingletonTraits<TraceEventETWExport>>::GetIfExists();
}

uint64_t CategoryGroupToETWKeyword(std::string_view category_group_name)
{
    static NoDestructor<base::flat_map<std::string_view, uint64_t>> categories_to_keyword([] {
        ...
        return base::flat_map<std::string_view, uint64_t>(base::sorted_unique, std::move(items));
    }());
...
perfetto::protos::gen::TrackEventConfig ETWKeywordToTrackEventConfig(uint64_t keyword)
{
    ...
    bool disabled_other_events_enables = (keyword & (1ULL << kDisabledOtherEventsGroupNameIndex));
