...
union BASE_EXPORT TraceValue {
 ...
 public:
...
#if 0 // BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
  template <typename T>
  struct TypeFor<T, std::enable_if_t<!HasHelperSupport<T> && perfetto::internal::has_traced_value_support<std::decay_t<T>>::value>> {
  ...
  template <
      typename T,
      class = std::enable_if_t<HasHelperSupport<T> || perfetto::internal::has_traced_value_support<std::decay_t<T>>::value>>
  struct TypeCheck {
