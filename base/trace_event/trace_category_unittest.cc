...
bool Initializer()
{
    return CategoryRegistry::kCategoryMetadata && CategoryRegistry::kCategoryMetadata->is_valid();
}
...
TEST_F(TraceCategoryTest, Basic)
{
    ...
    ASSERT_EQ(TraceCategory::ENABLED_FOR_RECORDING | TraceCategory::ENABLED_FOR_FILTERING, cat_1->state());
    ...
    ASSERT_TRUE(GetOrCreateCategoryByName("__test_basic_ab,__test_basic_a", &cat_3));
...
TEST_F(TraceCategoryTest, ThreadRaces)
{
    ...
    WaitableEvent sync_event(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    for (int i = 0; i < kNumThreads; i++) {
        threads[i]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestRaceThreadMain, Unretained(&sync_event)));
    }
    sync_event.Signal();
    for (int i = 0; i < kNumThreads; i++)
        threads[i]->Stop();
...
{
    static_assert(GetBuiltinCategoryByName("nonexistent") == nullptr, "nonexistent found");
#if BUILDFLAG(IS_WIN) && defined(COMPONENT_BUILD)
    static_assert(GetBuiltinCategoryByName("toplevel") == nullptr, "toplevel found");
#else
    static_assert(GetBuiltinCategoryByName("toplevel") != nullptr, "toplevel not found");
#endif
}
