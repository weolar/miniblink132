...
void MemoryDumpScheduler::Start(MemoryDumpScheduler::Config config, scoped_refptr<SequencedTaskRunner> task_runner)
{
    DCHECK(!task_runner_);
    task_runner_ = task_runner;
    task_runner->PostTask(FROM_HERE, BindOnce(&MemoryDumpScheduler::StartInternal, Unretained(this), config));
}
...
void MemoryDumpScheduler::Stop()
{
    if (!task_runner_)
        return;
    task_runner_->PostTask(FROM_HERE, BindOnce(&MemoryDumpScheduler::StopInternal, Unretained(this)));
...
void MemoryDumpScheduler::StartInternal(MemoryDumpScheduler::Config config)
{
    ...
    SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce(&MemoryDumpScheduler::Tick, Unretained(this), ++generation_), Milliseconds(200));
}
...
void MemoryDumpScheduler::Tick(uint32_t expected_generation)
{
    ...
    SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce(&MemoryDumpScheduler::Tick, Unretained(this), expected_generation), Milliseconds(period_ms_));
}

MemoryDumpScheduler::Config::Config() = default;
MemoryDumpScheduler::Config::~Config() = default;
MemoryDumpScheduler::Config::Config(const MemoryDumpScheduler::Config&) = default;
