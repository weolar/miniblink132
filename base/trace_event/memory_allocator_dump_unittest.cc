...
public:
    bool OnMemoryDump(const MemoryDumpArgs& args, ProcessMemoryDump* pmd) override
    {
        ...
        root_heap->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, 4096);
        root_heap->AddScalar(MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 42);
        ...
        sub_heap->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, 1);
        sub_heap->AddScalar(MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 3);
...
void CheckString(const MemoryAllocatorDump* dump, const std::string& name, const char* expected_units, const std::string& expected_value)
{
...
void CheckScalar(const MemoryAllocatorDump* dump, const std::string& name, const char* expected_units, uint64_t expected_value)
{
    MemoryAllocatorDump::Entry expected(name, expected_units, expected_value);
    EXPECT_THAT(dump->entries(), testing::Contains(Eq(ByRef(expected))));
}
...
{
    std::unique_ptr<MemoryAllocatorDump> mad(new MemoryAllocatorDump("foo", MemoryDumpLevelOfDetail::kFirst, MemoryAllocatorDumpGuid(0x42u)));
...
TEST(MemoryAllocatorDumpTest, DumpIntoProcessMemoryDump)
{
    ...
    CheckScalar(root_heap, MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, 4096);
    CheckScalar(root_heap, MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 42);
    ...
    CheckScalar(sub_heap, MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, 1);
    CheckScalar(sub_heap, MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 3);
...
TEST(MemoryAllocatorDumpTest, GetSize)
{
    ...
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, 1);
...
TEST(MemoryAllocatorDumpTest, ReadValues)
{
    ...
    EXPECT_THAT(dump->entries(), ElementsAre(Eq(ByRef(expected_scalar)), Eq(ByRef(expected_string))));
}
...
#if !defined(NDEBUG) && !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS) && !BUILDFLAG(IS_FUCHSIA)
