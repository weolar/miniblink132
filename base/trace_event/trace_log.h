...
public:
    ...
    //void SetEnabled(const TraceConfig& trace_config, const perfetto::TraceConfig& perfetto_config);
    ...
    // does.
    void AddOwnedEnabledStateObserver(std::unique_ptr<EnabledStateObserver> listener);
    ...
    void AddAsyncEnabledStateObserver(WeakPtr<AsyncEnabledStateObserver> listener);
    ...
    void SetArgumentFilterPredicate(const ArgumentFilterPredicate& argument_filter_predicate);
    ArgumentFilterPredicate GetArgumentFilterPredicate() const;

    void SetMetadataFilterPredicate(const MetadataFilterPredicate& metadata_filter_predicate);
    ...
    using OutputCallback = base::RepeatingCallback<void(const scoped_refptr<base::RefCountedString>&, bool has_more_events)>;
    void Flush(const OutputCallback& cb, bool use_worker_thread = false);

    // Cancels tracing and discards collected data.
    void CancelTracing(const OutputCallback& cb);

    using AddTraceEventOverrideFunction = void (*)(TraceEvent*, bool thread_will_flush, TraceEventHandle* handle);
    using OnFlushFunction = void (*)();
    using UpdateDurationFunction = void (*)(const unsigned char* category_group_enabled, const char* name, TraceEventHandle handle, PlatformThreadId thread_id,
    ...
    void SetAddTraceEventOverrides(const AddTraceEventOverrideFunction& add_event_override, const OnFlushFunction& on_flush_callback,
    ...
    static const char* GetCategoryGroupName(const unsigned char* category_group_enabled);
    static constexpr const unsigned char* GetBuiltinCategoryEnabled(const char* name)
    {
    ...
    bool ShouldAddAfterUpdatingState(char phase, const unsigned char* category_group_enabled, const char* name, uint64_t id, PlatformThreadId thread_id,
    ...
    TraceEventHandle AddTraceEventWithBindId(char phase, const unsigned char* category_group_enabled, const char* name, const char* scope, uint64_t id,
        uint64_t bind_id, TraceArguments* args, unsigned int flags);
    TraceEventHandle AddTraceEventWithProcessId(char phase, const unsigned char* category_group_enabled, const char* name, const char* scope, uint64_t id,
        ProcessId process_id, TraceArguments* args, unsigned int flags);
    TraceEventHandle AddTraceEventWithThreadIdAndTimestamp(char phase, const unsigned char* category_group_enabled, const char* name, const char* scope,
    ...
    TraceEventHandle AddTraceEventWithThreadIdAndTimestamps(char phase, const unsigned char* category_group_enabled, const char* name, const char* scope,
    ...
    void AddMetadataEvent(const unsigned char* category_group_enabled, const char* name, TraceArguments* args, unsigned int flags);

    void UpdateTraceEventDuration(const unsigned char* category_group_enabled, const char* name, TraceEventHandle handle);

    void UpdateTraceEventDurationExplicit(const unsigned char* category_group_enabled, const char* name, TraceEventHandle handle, PlatformThreadId thread_id,
        bool explicit_timestamps, const TimeTicks& now, const ThreadTicks& thread_now);
    ...
    //void SetEnabledImpl(const TraceConfig& trace_config, const perfetto::TraceConfig& perfetto_config);
...
private:
    ...
    InternalTraceOptions GetInternalOptionsFromTraceConfig(const TraceConfig& config);
    ...
    template <typename T>
    void AddMetadataEventWhileLocked(PlatformThreadId thread_id, const char* metadata_name, const char* arg_name, const T& value)
    ...
    std::string EventToConsoleMessage(char phase, const TimeTicks& timestamp, TraceEvent* trace_event);

    TraceEvent* AddEventToThreadSharedChunkWhileLocked(TraceEventHandle* handle, bool check_buffer_is_full) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    TraceEvent* GetEventByHandleInternal(TraceEventHandle handle, OptionalAutoLock* lock);

    void FlushInternal(const OutputCallback& cb, bool use_worker_thread, bool discard_events);
    ...
    static void ConvertTraceEventsToTraceFormat(std::unique_ptr<TraceBuffer> logged_events, const TraceLog::OutputCallback& flush_output_callback,
    ...
    std::vector<raw_ptr<EnabledStateObserver, VectorExperimental>> enabled_state_observers_ GUARDED_BY(observers_lock_);
    std::map<AsyncEnabledStateObserver*, RegisteredAsyncObserver> async_observers_ GUARDED_BY(observers_lock_);
    // Manages ownership of the owned observers. The owned observers will also be
    // added to |enabled_state_observers_|.
    std::vector<std::unique_ptr<EnabledStateObserver>> owned_enabled_state_observer_copy_ GUARDED_BY(observers_lock_);
    std::vector<raw_ptr<IncrementalStateObserver, VectorExperimental>> incremental_state_observers_ GUARDED_BY(observers_lock_);
    ...
    std::unordered_map<PlatformThreadId, std::string> thread_names_ GUARDED_BY(thread_info_lock_);

    // The following two maps are used only when ECHO_TO_CONSOLE.
    std::unordered_map<PlatformThreadId, base::stack<TimeTicks>> thread_event_start_times_ GUARDED_BY(thread_info_lock_);
    std::unordered_map<std::string, size_t> thread_colors_ GUARDED_BY(thread_info_lock_);
    ...
    std::unordered_map<PlatformThreadId, scoped_refptr<SingleThreadTaskRunner>> thread_task_runners_;
    ...
    std::vector<TrackEventSession> track_event_sessions_ GUARDED_BY(track_event_lock_);
