...
void CopyTraceEventParameter(char** buffer, const char** member, const char* end)
{
    if (*member) {
        ...
        size_t written = strlcpy(*buffer, *member, static_cast<size_t>(end - *buffer)) + 1;
...
void AppendValueDebugString(const TraceArguments& args, size_t idx, std::string* out)
{
...
bool StringStorage::Contains(const TraceArguments& args) const
{
    for (size_t n = 0; n < args.size(); ++n) {
        if (args.types()[n] == TRACE_VALUE_TYPE_COPY_STRING && !Contains(args.values()[n].as_string)) {
            return false;
        }
    }
    return true;
...
static_assert(std::is_trivial_v<TraceValue> && std::is_standard_layout_v<TraceValue>, "TraceValue must be plain-old-data type for performance reasons!");
...
void TraceValue::Append(unsigned char type, bool as_json, std::string* out) const
{
    ...
    case TRACE_VALUE_TYPE_POINTER: {
        ...
        // surrounding quotes.
        std::string value = StringPrintf("0x%" PRIx64, static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this->as_pointer)));
...
TraceArguments::TraceArguments(int num_args, const char* const* arg_names, const unsigned char* arg_types, const unsigned long long* arg_values)
{
...
void TraceArguments::CopyStringsTo(StringStorage* storage, bool copy_all_strings, const char** extra_string1, const char** extra_string2)
{
    // First, compute total allocation size.
    size_t alloc_size = 0;

    if (copy_all_strings) {
        alloc_size += GetAllocLength(*extra_string1) + GetAllocLength(*extra_string2);
        for (size_t n = 0; n < size_; ++n)
...
void ConvertableToTraceFormat::Add(perfetto::protos::pbzero::DebugAnnotation* annotation) const
{
