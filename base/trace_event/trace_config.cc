...
class ConvertableTraceConfigToTraceFormat : public base::trace_event::ConvertableToTraceFormat {
public:
...
std::set<MemoryDumpLevelOfDetail> GetDefaultAllowedMemoryDumpModes()
{
    ...
    for (uint32_t mode = static_cast<uint32_t>(MemoryDumpLevelOfDetail::kFirst); mode <= static_cast<uint32_t>(MemoryDumpLevelOfDetail::kLast); mode++) {
...
void TraceConfig::ResetMemoryDumpConfig(const TraceConfig::MemoryDumpConfig& memory_dump_config)
{
...
void TraceConfig::MemoryDumpConfig::Merge(const TraceConfig::MemoryDumpConfig& config)
{
    ...
    heap_profiler_options.breakdown_threshold_bytes
        = std::min(heap_profiler_options.breakdown_threshold_bytes, config.heap_profiler_options.breakdown_threshold_bytes);
}

TraceConfig::ProcessFilterConfig::ProcessFilterConfig() = default;

TraceConfig::ProcessFilterConfig::ProcessFilterConfig(const ProcessFilterConfig& other) = default;

TraceConfig::ProcessFilterConfig::ProcessFilterConfig(const std::unordered_set<base::ProcessId>& included_process_ids)
...
{
    included_process_ids_.insert(config.included_process_ids_.begin(), config.included_process_ids_.end());
}

void TraceConfig::ProcessFilterConfig::InitializeFromConfigDict(const Value::Dict& dict)
{
    included_process_ids_.clear();
    ...
    for (auto& pid_value : *value) {
        if (pid_value.is_int()) {
            included_process_ids_.insert(static_cast<base::ProcessId>(pid_value.GetInt()));
        }
    }
}
...
void TraceConfig::ProcessFilterConfig::ToDict(Value::Dict& dict) const
{
    ...
    std::set<base::ProcessId> ordered_set(included_process_ids_.begin(), included_process_ids_.end());
...
{
    return included_process_ids_.empty() || included_process_ids_.count(process_id);
}
...
TraceConfig::EventFilterConfig& TraceConfig::EventFilterConfig::operator=(const TraceConfig::EventFilterConfig& rhs)
{
    if (this == &rhs)
        return *this;
...
{
    return predicate_name_ == other.predicate_name_ && category_filter_.IsEquivalentTo(category_filter_) && args_ == other.args_;
}

void TraceConfig::EventFilterConfig::InitializeFromConfigDict(const Value::Dict& event_filter)
{
...
bool TraceConfig::EventFilterConfig::GetArgAsSet(const char* key, std::unordered_set<std::string>* out_set) const
{
...
{
    InitializeFromStrings(category_filter_string, TraceConfig::TraceRecordModeToStr(record_mode));
}
...
{
    if (enable_systrace_ != other.enable_systrace_ || enable_argument_filter_ != other.enable_argument_filter_
        || enable_event_package_name_filter_ != other.enable_event_package_name_filter_ || histogram_names_ != other.histogram_names_
        || systrace_events_ != other.systrace_events_ || process_filter_config_ != other.process_filter_config_
        || memory_dump_config_ != other.memory_dump_config_ || !category_filter_.IsEquivalentTo(other.category_filter_)) {
        return false;
    }
...
{
    if (record_mode_ != config.record_mode_ || enable_systrace_ != config.enable_systrace_ || enable_argument_filter_ != config.enable_argument_filter_
        || enable_event_package_name_filter_ != config.enable_event_package_name_filter_) {
        DLOG(ERROR) << "Attempting to merge trace config with a different "
                    << "set of options.";
    }
    DCHECK_EQ(trace_buffer_size_in_events_, config.trace_buffer_size_in_events_) << "Cannot change trace buffer size";
    ...
    event_filters_.insert(event_filters_.end(), config.event_filters().begin(), config.event_filters().end());
    histogram_names_.insert(config.histogram_names().begin(), config.histogram_names().end());
}
...
void TraceConfig::InitializeFromConfigDict(const Value::Dict& dict)
{
    ...
    trace_buffer_size_in_events_ = base::saturated_cast<size_t>(dict.FindInt(kTraceBufferSizeInEvents).value_or(0));
    trace_buffer_size_in_kb_ = base::saturated_cast<size_t>(dict.FindInt(kTraceBufferSizeInKb).value_or(0));

    enable_systrace_ = dict.FindBool(kEnableSystraceParam).value_or(false);
    enable_argument_filter_ = dict.FindBool(kEnableArgumentFilterParam).value_or(false);
    enable_event_package_name_filter_ = dict.FindBool(kEnableEventPackageNameFilterParam).value_or(false);
    ...
    if (category_filter_.IsCategoryEnabled(MemoryDumpManager::kTraceCategory)) {
        ...
        const Value::Dict* memory_dump_config = dict.FindDict(kMemoryDumpConfigParam);
...
void TraceConfig::InitializeFromStrings(std::string_view category_filter_string, std::string_view trace_options_string)
{
    ...
    if (!trace_options_string.empty()) {
        std::vector<std::string> split = SplitString(trace_options_string, ",", TRIM_WHITESPACE, SPLIT_WANT_ALL);
        for (const std::string& token : split) {
            if (token == kRecordUntilFull) {
                ...
                const auto system_events_not_trimmed = token.substr(kEnableSystraceLength);
                const auto system_events = TrimString(system_events_not_trimmed, kWhitespaceASCII, TRIM_ALL);
                if (system_events[0] != '=') {
                    LOG(ERROR) << "Failed to parse " << token;
                    continue;
                }
                enable_systrace_ = true;
                const std::vector<std::string> split_systrace_events = SplitString(system_events.substr(1), " ", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
void TraceConfig::SetMemoryDumpConfigFromConfigDict(const Value::Dict& memory_dump_config)
{
    // Set allowed dump modes.
    memory_dump_config_.allowed_dump_modes.clear();
    const Value::List* allowed_modes_list = memory_dump_config.FindList(kAllowedDumpModesParam);
    if (allowed_modes_list) {
        for (const Value& item : *allowed_modes_list) {
            DCHECK(item.is_string());
            memory_dump_config_.allowed_dump_modes.insert(StringToMemoryDumpLevelOfDetail(item.GetString()));
        }
    } else {
    ...
    if (trigger_list) {
        for (const Value& trigger : *trigger_list) {
            ...
            if (!interval) {
                ...
                const std::string* trigger_type_str = trigger_dict.FindString(kTriggerTypeParam);
            ...
            const std::string* level_of_detail_str = trigger_dict.FindString(kTriggerModeParam);
    ...
    const Value::Dict* heap_profiler_options = memory_dump_config.FindDict(kHeapProfilerOptions);
    if (heap_profiler_options) {
        std::optional<int> min_size_bytes = heap_profiler_options->FindInt(kBreakdownThresholdBytes);
        if (min_size_bytes && *min_size_bytes >= 0) {
            memory_dump_config_.heap_profiler_options.breakdown_threshold_bytes = static_cast<uint32_t>(*min_size_bytes);
        } else {
...
void TraceConfig::SetHistogramNamesFromConfigList(const Value::List& histogram_names)
{
    histogram_names_.clear();
...
void TraceConfig::SetEventFiltersFromConfigList(const Value::List& category_event_filters)
{
    event_filters_.clear();
    ...
    for (const Value& event_filter : category_event_filters) {
        ...
        const std::string* predicate_name = event_filter_dict.FindString(kFilterPredicateParam);
...
Value TraceConfig::ToValue() const
{
    ...
    if (trace_buffer_size_in_events_ > 0) {
        dict.Set(kTraceBufferSizeInEvents, base::checked_cast<int>(trace_buffer_size_in_events_));
    }
    if (trace_buffer_size_in_kb_ > 0) {
        dict.Set(kTraceBufferSizeInKb, base::checked_cast<int>(trace_buffer_size_in_kb_));
    }

    dict.Set(kEnableEventPackageNameFilterParam, enable_event_package_name_filter_);
    ...
    if (category_filter_.IsCategoryEnabled(MemoryDumpManager::kTraceCategory)) {
        ...
        for (const auto& config : memory_dump_config_.triggers) {
            ...
            trigger_dict.Set(kTriggerTypeParam, MemoryDumpTypeToString(config.trigger_type));
            trigger_dict.Set(kMinTimeBetweenDumps, static_cast<int>(config.min_time_between_dumps_ms));
            trigger_dict.Set(kTriggerModeParam, MemoryDumpLevelOfDetailToString(config.level_of_detail));
        ...
        if (memory_dump_config_.heap_profiler_options.breakdown_threshold_bytes != MemoryDumpConfig::HeapProfiler::kDefaultBreakdownThresholdBytes) {
            Value::Dict options;
            options.Set(kBreakdownThresholdBytes, base::checked_cast<int>(memory_dump_config_.heap_profiler_options.breakdown_threshold_bytes));
...
std::string TraceConfig::ToPerfettoTrackEventConfigRaw(bool privacy_filtering_enabled) const
{
    perfetto::protos::gen::TrackEventConfig te_cfg;
    if (!base::Contains(category_filter_.excluded_categories(), "*") && !base::Contains(category_filter_.included_categories(), "*")) {
