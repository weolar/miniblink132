...
{
    return base::ThreadTicks::IsSupported() ? base::subtle::ThreadTicksNowIgnoringOverride() : base::ThreadTicks();
}
...
base::trace_event::EmitEmptyTracePacketFunction g_empty_packet_callback = nullptr;

std::pair<char /*phase*/, unsigned long long /*id*/> GetPhaseAndIdForTraceLog(bool explicit_track, uint64_t track_uuid, char phase)
{
    if (!explicit_track)
...
void EnableTypedTraceEvents(
    PrepareTrackEventFunction typed_event_callback, PrepareTracePacketFunction trace_packet_callback, EmitEmptyTracePacketFunction empty_packet_callback)
{
...
TrackEventHandle::TrackEventHandle(TrackEvent* event, IncrementalState* incremental_state, CompletionListener* listener, bool filter_debug_annotations)
    : event_(event)
...
TracePacketHandle::TracePacketHandle(PerfettoPacketHandle packet, CompletionListener* listener)
...
base::trace_event::TrackEventHandle CreateTrackEvent(
    char phase, const unsigned char* category_group_enabled, perfetto::StaticString name, base::TimeTicks ts, uint64_t track_uuid, bool explicit_track)
{
    DCHECK(phase == TRACE_EVENT_PHASE_BEGIN || phase == TRACE_EVENT_PHASE_END || phase == TRACE_EVENT_PHASE_INSTANT);
    ...
    auto phase_and_id_for_trace_log = GetPhaseAndIdForTraceLog(explicit_track, track_uuid, phase);
    ...
    if (!trace_log->ShouldAddAfterUpdatingState(
            phase_and_id_for_trace_log.first, category_group_enabled, name.value, phase_and_id_for_trace_log.second, thread_id, ts, nullptr)) {
        return base::trace_event::TrackEventHandle();
    }
    ...
    base::trace_event::TraceEvent event(thread_id, ts, thread_now, phase, category_group_enabled, name.value, trace_event_internal::kGlobalScope,
...
bool ShouldEmitTrackDescriptor(uint64_t track_uuid, base::trace_event::TrackEventHandle::IncrementalState* incr_state)
{
