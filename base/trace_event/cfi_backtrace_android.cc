...
static_assert(sizeof(uintptr_t) == 4, "The unwind table format is only valid for 32 bit builds.");
...
struct CFIUnwindDataRow {
    ...
    uint16_t cfa_offset() const
    {
        return cfi_data & kCFAMask;
    }
};

static_assert(sizeof(CFIUnwindDataRow) == 4, "The CFIUnwindDataRow struct must be exactly 4 bytes for searching.");
...
void CFIBacktraceAndroid::ParseCFITables()
{
    ...
    unw_index_function_col_ = reinterpret_cast<const uintptr_t*>(cfi_mmap_->data()) + 1;
    unw_index_row_count_ = unw_index_size;
    unw_index_indices_col_ = reinterpret_cast<const uint16_t*>((unw_index_function_col_ + unw_index_row_count_).get());
...
size_t CFIBacktraceAndroid::Unwind(uintptr_t pc, uintptr_t sp, uintptr_t lr, const void** out_trace, size_t max_depth)
{
    if (!can_unwind_stack_frames())
        return 0;
    ...
    while (is_chrome_address(pc) && depth < max_depth) {
        ...
        memcpy(&pc, reinterpret_cast<uintptr_t*>(sp - cfi.ra_offset), sizeof(uintptr_t));
    }
    return depth;
}

bool CFIBacktraceAndroid::FindCFIRowForPC(uintptr_t func_addr, CFIBacktraceAndroid::CFIRow* cfi)
{
    if (!can_unwind_stack_frames())
        return false;
    ...
    *cfi = { 0 };
    ...
    static const uintptr_t* const unw_index_fn_end = unw_index_function_col_ + unw_index_row_count_;
    const uintptr_t* found = std::lower_bound(unw_index_function_col_.get(), unw_index_fn_end, func_addr);
    ...
    const CFIUnwindDataRow* function_data = reinterpret_cast<const CFIUnwindDataRow*>(unwind_data + 1);
