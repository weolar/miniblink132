...
TEST(CFIBacktraceAndroidTest, TestUnwinding)
{
    ...
    for (size_t i = 0; i < unwind_count; ++i) {
        EXPECT_GT(reinterpret_cast<uintptr_t>(frames[i]), unwinder->executable_start_addr());
        EXPECT_LT(reinterpret_cast<uintptr_t>(frames[i]), unwinder->executable_end_addr());
    }
}
...
TEST(CFIBacktraceAndroidTest, DISABLED_TestFindCFIRow)
{
  ...
  */
    uint16_t input[] = { // UNW_INDEX size
        ...
        0x1000, 0x0, 0x1502, 0x0, 0x2000, 0x0, 0x2024, 0x0, 0x2126, 0x0, 0x2200, 0x0, 0x2212, 0x0,

        // UNW_INDEX index column (2 byte rows).
        0x0, 0xffff, 0xb, 0x10, 0xffff, 0x15, 0xffff,

        // UNW_DATA table.
        0x5, 0x2, 0x111, 0x8, 0x220, 0x40, 0x330, 0x50, 0x332, 0x80, 0x220, 0x2, 0x4, 0x13, 0x8, 0x13, 0x2, 0xc, 0x33, 0xdc, 0x40, 0x1, 0x4, 0x2e
    };
    FilePath temp_path;
...
TEST(CFIBacktraceAndroidTest, TestCFICache)
{
    ...
    for (uintptr_t i = 1; i <= 2 * cache.kLimit; ++i) {
        CFIBacktraceAndroid::CFIRow val = { static_cast<uint16_t>(4 * i), static_cast<uint16_t>(2 * i) };
        cache.Add(i, val);
