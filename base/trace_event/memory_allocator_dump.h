...
public:
    ...
    MemoryAllocatorDump(const std::string& absolute_name, MemoryDumpLevelOfDetail, const MemoryAllocatorDumpGuid&);
    ...
    void AsProtoInto(perfetto::protos::pbzero::MemoryTrackerSnapshot_ProcessSnapshot_MemoryNode* memory_node) const;
    ...
    MemoryDumpLevelOfDetail level_of_detail() const
    {
    ...
    const MemoryAllocatorDumpGuid& guid() const LIFETIME_BOUND
    {
        return guid_;
    }

    const std::vector<Entry>& entries() const LIFETIME_BOUND
    {
        return entries_;
    }
