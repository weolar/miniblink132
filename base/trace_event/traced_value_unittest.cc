...
TEST(TraceEventArgumentTest, InitializerListCreatedContainers)
{
    ...
    EXPECT_EQ("{\"empty_array\":[],\"empty_dictionary\":{},"
              "\"nested_array\":[[],{},true],"
...
{
    std::string json;
    TracedValue::Build({ { "bool_var", true }, { "double_var", 3.14 }, { "int_var", 2020 }, { "literal_var", "literal" } })->AppendAsTraceFormat(&json);
    EXPECT_EQ("{\"bool_var\":true,\"double_var\":3.14,\"int_var\":2020,\""
              "literal_var\":\"literal\"}",
        json);
}
...
TEST(TraceEventArgumentTest, ArrayAndDictionaryScope)
{
    ...
    {
        auto surround_dictionary = value->BeginDictionaryScoped("outside_dictionary");
    ...
    {
        auto dictionary = value->BeginDictionaryScopedWithCopiedName(std::string("wonderful_") + std::string("world"));
    }
    {
        auto array = value->BeginArrayScopedWithCopiedName(std::string("wonderful_") + std::string("array"));
    }
    std::string json;
...
TEST(TraceEventArgumentTest, FlatDictionary)
{
    ...
    value->AppendAsTraceFormat(&json);
    EXPECT_EQ("PREFIX{\"bool\":true,\"double\":0.0,\"int\":2014,\"string\":\"string\","
              "\"ptr\":\"0x1234\"}",
        json);
}
...
TEST(TraceEventArgumentTest, NoDotPathExpansion)
{
    ...
    std::string json;
    value->AppendAsTraceFormat(&json);
    EXPECT_EQ("{\"bo.ol\":true,\"doub.le\":0.0,\"in.t\":2014,\"str.ing\":\"str.ing\"}", json);
}
...
TEST(TraceEventArgumentTest, Hierarchy)
{
    ...
    std::string json;
    value->AppendAsTraceFormat(&json);
    EXPECT_EQ("{\"a1\":[1,true,{\"i2\":3}],\"b0\":true,\"d0\":0.0,\"dict1\":{\"dict2\":"
...
TEST(TraceEventArgumentTest, LongStrings)
{
    ...
    std::string json;
    value->AppendAsTraceFormat(&json);
    EXPECT_EQ("{\"a\":\"short\",\"b\":\"" + kLongString + "\",\"c\":[\"" + kLongString2 + "\",\"\",{\"a\":\"" + kLongString3.data() + "\"}]}", json);
}
...
TEST(TraceEventArgumentTest, NanAndInfinityJSON)
{
    ...
    std::string json;
    value.AppendAsTraceFormat(&json);
    EXPECT_EQ("{\"nan\":\"NaN\",\"infinity\":\"Infinity\","
