...
public:
    struct BASE_EXPORT MemoryAllocatorDumpEdge {
        friend bool operator==(const MemoryAllocatorDumpEdge&, const MemoryAllocatorDumpEdge&) = default;
    ...
    using AllocatorDumpsMap = std::map<std::string, std::unique_ptr<MemoryAllocatorDump>>;

    // Stores allocator dump edges indexed by source allocator dump GUID.
    using AllocatorDumpEdgesMap = std::map<MemoryAllocatorDumpGuid, MemoryAllocatorDumpEdge>;
    ...
    static std::optional<size_t> CountResidentBytes(void* start_address, size_t mapped_size);

    // The same as above, but the given mapped range should belong to the
    // shared_memory's mapped region.
    static std::optional<size_t> CountResidentBytesInSharedMemory(void* start_address, size_t mapped_size);
#endif
    ...
    MemoryAllocatorDump* CreateAllocatorDump(const std::string& absolute_name, const MemoryAllocatorDumpGuid& guid);
    ...
    // size.
    MemoryAllocatorDump* GetOrCreateAllocatorDump(const std::string& absolute_name);
    ...
    MemoryAllocatorDump* CreateSharedGlobalAllocatorDump(const MemoryAllocatorDumpGuid& guid);
    ...
    MemoryAllocatorDump* CreateWeakSharedGlobalAllocatorDump(const MemoryAllocatorDumpGuid& guid);

    // Looks up a shared MemoryAllocatorDump given its guid.
    MemoryAllocatorDump* GetSharedGlobalAllocatorDump(const MemoryAllocatorDumpGuid& guid) const;
    ...
    void SetAllocatorDumpsForSerialization(std::vector<std::unique_ptr<MemoryAllocatorDump>>);
    ...
    void DumpHeapUsage(const std::unordered_map<base::trace_event::AllocationContext, base::trace_event::AllocationMetrics>& metrics_by_context,
    ...
    void AddOwnershipEdge(const MemoryAllocatorDumpGuid& source, const MemoryAllocatorDumpGuid& target, int importance);
    void AddOwnershipEdge(const MemoryAllocatorDumpGuid& source, const MemoryAllocatorDumpGuid& target);
    ...
    // |importance| value.
    void AddOverridableOwnershipEdge(const MemoryAllocatorDumpGuid& source, const MemoryAllocatorDumpGuid& target, int importance);
    ...
    void CreateSharedMemoryOwnershipEdge(const MemoryAllocatorDumpGuid& client_local_dump_guid, const UnguessableToken& shared_memory_guid, int importance);
    void CreateWeakSharedMemoryOwnershipEdge(const MemoryAllocatorDumpGuid& client_local_dump_guid, const UnguessableToken& shared_memory_guid, int importance);
    ...
    void AddSuballocation(const MemoryAllocatorDumpGuid& source, const std::string& target_node_name);
    ...
    void SerializeAllocatorDumpsInto(perfetto::protos::pbzero::MemoryTrackerSnapshot* memory_snapshot, const base::ProcessId pid) const;

    const MemoryDumpArgs& dump_args() const LIFETIME_BOUND
    {
        return dump_args_;
    }
...
private:
    ...
    MemoryAllocatorDump* AddAllocatorDumpInternal(std::unique_ptr<MemoryAllocatorDump> mad);
