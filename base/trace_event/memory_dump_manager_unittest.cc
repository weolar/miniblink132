...
void RegisterDumpProvider(
    MemoryDumpProvider* mdp, scoped_refptr<base::SingleThreadTaskRunner> task_runner, const MemoryDumpProvider::Options& options, const char* name = kMDPName)
{
...
void RegisterDumpProvider(MemoryDumpProvider* mdp, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    RegisterDumpProvider(mdp, task_runner, MemoryDumpProvider::Options());
}

void RegisterDumpProviderWithSequencedTaskRunner(
    MemoryDumpProvider* mdp, scoped_refptr<base::SequencedTaskRunner> task_runner, const MemoryDumpProvider::Options& options)
{
    MemoryDumpManager* mdm = MemoryDumpManager::GetInstance();
    mdm->set_dumper_registrations_ignored_for_testing(false);
    mdm->RegisterDumpProviderWithSequencedTaskRunner(mdp, kMDPName, task_runner, options);
    mdm->set_dumper_registrations_ignored_for_testing(true);
}

// Posts |task| to |task_runner| and blocks until it is executed.
void PostTaskAndWait(const Location& from_here, SequencedTaskRunner* task_runner, base::OnceClosure task)
{
    base::WaitableEvent event(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    task_runner->PostTask(from_here, std::move(task));
    task_runner->PostTask(FROM_HERE, base::BindOnce(&WaitableEvent::Signal, base::Unretained(&event)));
...
public:
    ...
    {
        ON_CALL(*this, OnMemoryDump(_, _)).WillByDefault(Invoke([](const MemoryDumpArgs&, ProcessMemoryDump* pmd) -> bool { return true; }));
    }
...
public:
    ...
    unsigned no_of_post_tasks() const
    {
        return num_of_post_tasks_;
    }
...
private:
    ...
    const scoped_refptr<SequencedTaskRunner> task_runner_ = ThreadPool::CreateSequencedTaskRunner({});
    bool enabled_ = true;
...
protected:
    ...
    bool RequestProcessDumpAndWait(MemoryDumpType dump_type, MemoryDumpLevelOfDetail level_of_detail, MemoryDumpDeterminism determinism)
    {
        RunLoop run_loop;
        bool success = false;
        static uint64_t test_guid = 1;
        test_guid++;
        MemoryDumpRequestArgs request_args { test_guid, dump_type, level_of_detail, determinism };
        ...
        ProcessMemoryDumpCallback callback = BindOnce(
            [](bool* curried_success, OnceClosure curried_quit_closure, uint64_t curried_expected_guid, bool success, uint64_t dump_guid,
                std::unique_ptr<ProcessMemoryDump> pmd) {
                *curried_success = success;
                EXPECT_EQ(curried_expected_guid, dump_guid);
                SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(curried_quit_closure));
            },
...
public:
    MemoryDumpManagerTestAsCoordinator()
        : MemoryDumpManagerTest(true)
    {
    }

    MemoryDumpManagerTestAsCoordinator(const MemoryDumpManagerTestAsCoordinator&) = delete;
    MemoryDumpManagerTestAsCoordinator& operator=(const MemoryDumpManagerTestAsCoordinator&) = delete;
};
...
TEST_F(MemoryDumpManagerTest, SingleDumper)
{
    ...
    for (int i = 0; i < 3; ++i) {
        EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kExplicitlyTriggered, MemoryDumpLevelOfDetail::kDetailed, MemoryDumpDeterminism::kNone));
    }
    DisableTracing();
...
TEST_F(MemoryDumpManagerTest, CheckMemoryDumpArgs)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kExplicitlyTriggered, MemoryDumpLevelOfDetail::kLight, MemoryDumpDeterminism::kNone));
    DisableTracing();
    mdm_->UnregisterDumpProvider(&mdp);
}
...
TEST_F(MemoryDumpManagerTest, CheckMemoryDumpArgsDeterministic)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kExplicitlyTriggered, MemoryDumpLevelOfDetail::kDetailed, MemoryDumpDeterminism::kForceGc));
    DisableTracing();
    mdm_->UnregisterDumpProvider(&mdp);
...
TEST_F(MemoryDumpManagerTest, RespectTaskRunnerAffinity)
{
    ...
    for (uint32_t i = kNumInitialThreads; i > 0; --i) {
        ...
        EXPECT_CALL(*mdp, OnMemoryDump(_, _)).Times(i).WillRepeatedly(Invoke([task_runner](const MemoryDumpArgs&, ProcessMemoryDump*) -> bool {
    ...
    while (!threads.empty()) {
        ...
        {
            RunLoop run_loop;
            OnceClosure unregistration = BindOnce(&MemoryDumpManager::UnregisterDumpProvider, Unretained(mdm_.get()), Unretained(mdps.back().get()));
            threads.back()->task_runner()->PostTaskAndReply(FROM_HERE, std::move(unregistration), run_loop.QuitClosure());
            run_loop.Run();
        }
        mdps.pop_back();
        threads.back()->Stop();
...
TEST_F(MemoryDumpManagerTest, PostTaskForSequencedTaskRunner)
{
    ...
    scoped_refptr<TestSequencedTaskRunner> task_runner1(MakeRefCounted<TestSequencedTaskRunner>());
    scoped_refptr<TestSequencedTaskRunner> task_runner2(MakeRefCounted<TestSequencedTaskRunner>());
    RegisterDumpProviderWithSequencedTaskRunner(&mdps[0], task_runner1, kDefaultOptions);
    RegisterDumpProviderWithSequencedTaskRunner(&mdps[1], task_runner2, kDefaultOptions);
    RegisterDumpProviderWithSequencedTaskRunner(&mdps[2], task_runner2, kDefaultOptions);
...
TEST_F(MemoryDumpManagerTest, DisableFailingDumpers)
{
    ...
    EXPECT_CALL(mdp1, OnMemoryDump(_, _)).Times(GetMaxConsecutiveFailuresCount()).WillRepeatedly(Return(false));
...
TEST_F(MemoryDumpManagerTest, RegisterDumperWhileDumping)
{
    ...
    EXPECT_CALL(mdp1, OnMemoryDump(_, _))
        .Times(4)
        .WillOnce(Return(true))
        .WillOnce(Invoke([&mdp2](const MemoryDumpArgs&, ProcessMemoryDump*) -> bool {
            RegisterDumpProvider(&mdp2, nullptr);
            return true;
        }))
...
TEST_F(MemoryDumpManagerTest, UnregisterDumperWhileDumping)
{
    ...
    RegisterDumpProvider(&mdp1, SingleThreadTaskRunner::GetCurrentDefault(), kDefaultOptions);
    RegisterDumpProvider(&mdp2, SingleThreadTaskRunner::GetCurrentDefault(), kDefaultOptions);
    EnableForTracing();
...
TEST_F(MemoryDumpManagerTest, UnregisterDumperFromThreadWhileDumping)
{
    ...
    for (const std::unique_ptr<MockMemoryDumpProvider>& mdp : mdps) {
        ...
        scoped_refptr<SingleThreadTaskRunner> other_runner = threads[other_idx]->task_runner();
        MockMemoryDumpProvider* other_mdp = mdps[other_idx].get();
        auto on_dump = [this, other_runner, other_mdp, &on_memory_dump_call_count](const MemoryDumpArgs& args, ProcessMemoryDump* pmd) {
            PostTaskAndWait(FROM_HERE, other_runner.get(), base::BindOnce(&MemoryDumpManager::UnregisterDumpProvider, base::Unretained(&*mdm_), other_mdp));
        ...
        EXPECT_CALL(*mdp, OnMemoryDump(_, _)).Times(AtMost(1)).WillOnce(Invoke(on_dump));
    }
...
TEST_F(MemoryDumpManagerTest, TearDownThreadWhileDumping)
{
    ...
    for (const std::unique_ptr<MockMemoryDumpProvider>& mdp : mdps) {
        ...
        scoped_refptr<SequencedTaskRunner> main_runner = SequencedTaskRunner::GetCurrentDefault();
        auto on_dump = [other_thread, main_runner, &on_memory_dump_call_count](const MemoryDumpArgs& args, ProcessMemoryDump* pmd) {
            PostTaskAndWait(FROM_HERE, main_runner.get(), base::BindOnce(&TestIOThread::Stop, base::Unretained(other_thread)));
            on_memory_dump_call_count++;
            return true;
        };
...
TEST_F(MemoryDumpManagerTest, BackgroundAllowlisting)
{
    ...
    RegisterDumpProvider(&backgroundMdp, nullptr, kDefaultOptions, kAllowlistedMDPName);

    EnableForTracing();

    EXPECT_CALL(backgroundMdp, OnMemoryDump(_, _)).Times(1);
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kSummaryOnly, MemoryDumpLevelOfDetail::kBackground, MemoryDumpDeterminism::kNone));
    DisableTracing();
}
...
TEST_F(MemoryDumpManagerTest, UnregisterAndDeleteDumpProviderSoon)
{
    ...
    for (int i = 0; i < kNumProviders; ++i) {
        ...
        EXPECT_CALL(*mdp, Destructor()).WillOnce(Invoke([&dtor_count] { dtor_count++; }));
...
TEST_F(MemoryDumpManagerTest, UnregisterAndDeleteDumpProviderSoonDuringDump)
{
    ...
    auto self_unregister_from_another_thread = [&mdp, &thread_ref](const MemoryDumpArgs&, ProcessMemoryDump*) -> bool {
        thread_ref = PlatformThread::CurrentRef();
        TestIOThread thread_for_unregistration(TestIOThread::kAutoStart);
        PostTaskAndWait(FROM_HERE, thread_for_unregistration.task_runner().get(),
            base::BindOnce(&MemoryDumpManager::UnregisterAndDeleteDumpProviderSoon, base::Unretained(MemoryDumpManager::GetInstance()), std::move(mdp)));
        thread_for_unregistration.Stop();
        return true;
    };
    EXPECT_CALL(*mdp, OnMemoryDump(_, _)).Times(1).WillOnce(Invoke(self_unregister_from_another_thread));
    EXPECT_CALL(*mdp, Destructor()).Times(1).WillOnce(Invoke([&thread_ref] { EXPECT_EQ(thread_ref, PlatformThread::CurrentRef()); }));
...
TEST_F(MemoryDumpManagerTest, NoStackOverflowWithTooManyMDPs)
{
    ...
    for (int i = 0; i < kMDPCount; ++i) {
        ...
        RegisterDumpProvider(mdps.back().get(), nullptr, kDefaultOptions, kAllowlistedMDPName);
    }
    ...
    for (int i = 0; i < kMDPCount; ++i) {
        ...
        RegisterDumpProvider(mdps.back().get(), stopped_thread->task_runner(), kDefaultOptions, kAllowlistedMDPName);
    }
    stopped_thread->Stop();

    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kExplicitlyTriggered, MemoryDumpLevelOfDetail::kDetailed, MemoryDumpDeterminism::kNone));
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kExplicitlyTriggered, MemoryDumpLevelOfDetail::kBackground, MemoryDumpDeterminism::kNone));
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::kSummaryOnly, MemoryDumpLevelOfDetail::kBackground, MemoryDumpDeterminism::kNone));
}
