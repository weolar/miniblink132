...
public:
    ...
    bool IsPickleWriter() const override
    {
        return true;
    }
    bool IsProtoWriter() const override
    {
        return false;
    }
    ...
    void SetStringWithCopiedName(std::string_view name, std::string_view value) override
    {
        pickle_.WriteBytes(byte_span_from_ref(kTypeString));
        pickle_.WriteString(value);
        WriteKeyNameWithCopy(pickle_, name);
    ...
    void AppendAsTraceFormat(std::string* out) const override
    {
        ...
        auto maybe_append_key_name = [](State current_state, PickleIterator* it, std::string* out) {
        ...
        while (it.ReadBytes(&type, 1)) {
            ...
            case kTypeStartDict: {
                if (cur_dict) {
                    ...
                    cur_dict = cur_dict->GetDict().Set(ReadKeyName(it), Value(Value::Type::DICT));
                } else {
            ...
            case kTypeStartArray: {
                ...
                if (cur_dict) {
                    stack.push_back(cur_dict);
                    cur_list = cur_dict->GetDict().Set(ReadKeyName(it), std::move(new_list));
                    cur_dict = nullptr;
                } else {
...
{
    TracedValue::WriterFactoryCallback callback = g_writer_factory_callback.load(std::memory_order_relaxed);
    if (callback) {
...
TracedValue::TracedValue(size_t capacity, bool forced_json)
{
    ...
    writer_ = forced_json ? std::make_unique<PickleWriter>(capacity) : CreateWriter(capacity);
}
...
{
    return base::StringPrintf("0x%" PRIx64, static_cast<uint64_t>(reinterpret_cast<uintptr_t>(value)));
}
...
void TracedValue::EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead)
{
...
TracedValue::Dictionary::Dictionary(const std::initializer_list<DictionaryItem> items)
{
    items_ = items;
}
...
TracedValue::ValueHolder::ValueHolder(TracedValue::ValueHolder&& other)
{
    ...
    case KeptValueType::kStdStringType: {
        new (&kept_value_.std_string_value) std::string(std::move(other.kept_value_.std_string_value));
        break;
    }
    ...
    case KeptValueType::kArrayType: {
        new (&kept_value_.array_value) TracedValue::Array(std::move(other.kept_value_.array_value));
        break;
    }
    case KeptValueType::kDictionaryType: {
        new (&kept_value_.dictionary_value) TracedValue::Dictionary(std::move(other.kept_value_.dictionary_value));
        break;
    }
    }
...
void TracedValue::ValueHolder::WriteToValue(const char* name, TracedValue* value) const
{
    switch (kept_value_type_) {
    case KeptValueType::kIntType: {
...
std::unique_ptr<TracedValue> TracedValue::Build(const std::initializer_list<DictionaryItem> items)
{
...
{
    std::string str;
    base::JSONWriter::WriteWithOptions(*ToBaseValue(), base::JSONWriter::OPTIONS_OMIT_DOUBLE_TYPE_PRESERVATION | base::JSONWriter::OPTIONS_PRETTY_PRINT, &str);
    return str;
}
...
TracedValue::ArrayScope TracedValue::BeginArrayScopedWithCopiedName(std::string_view name)
{
...
TracedValue::DictionaryScope TracedValue::BeginDictionaryScoped(const char* name)
{
    BeginDictionary(name);
    return TracedValue::DictionaryScope(this);
}

TracedValue::DictionaryScope TracedValue::BeginDictionaryScopedWithCopiedName(std::string_view name)
{
    BeginDictionaryWithCopiedName(name);
    return TracedValue::DictionaryScope(this);
}
