...
#define TRACE_HEAP_PROFILER_API_GET_CURRENT_TASK_CONTEXT trace_event_internal::HeapProfilerCurrentTaskContext
...
public:
    inline explicit HeapProfilerScopedTaskExecutionTracker(const char* task_context)
        : context_(task_context)
    {
        using base::trace_event::AllocationContextTracker;
        if (AllocationContextTracker::capture_mode() != AllocationContextTracker::CaptureMode::kDisabled) [[unlikely]] {
            AllocationContextTracker::GetInstanceForCurrentThread()->PushCurrentTaskContext(context_);
        }
    }
    ...
    inline ~HeapProfilerScopedTaskExecutionTracker()
    {
        using base::trace_event::AllocationContextTracker;
        if (AllocationContextTracker::capture_mode() != AllocationContextTracker::CaptureMode::kDisabled) [[unlikely]] {
            AllocationContextTracker::GetInstanceForCurrentThread()->PopCurrentTaskContext(context_);
        }
    }
...
{
    return base::trace_event::AllocationContextTracker::GetInstanceForCurrentThread()->TaskContext();
}
