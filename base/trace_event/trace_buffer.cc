...
public:
    ...
    void ReturnChunk(size_t index, std::unique_ptr<TraceBufferChunk> chunk) override
    {
    ...
    void EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead) override
    {
        ...
        for (size_t queue_index = queue_head_; queue_index != queue_tail_; queue_index = NextQueueIndex(queue_index)) {
...
private:
    ...
    {
        return queue_tail_ > queue_head_ ? queue_tail_ - queue_head_ : queue_tail_ + queue_capacity() - queue_head_;
    }
...
public:
    ...
    std::unique_ptr<TraceBufferChunk> GetChunk(size_t* index) override
    {
        ...
        *index = chunks_.size();
        ...
        return std::make_unique<TraceBufferChunk>(static_cast<uint32_t>(*index) + 1);
    }
    ...
    void EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead) override
    {
        ...
        overhead->Add(TraceEventMemoryOverhead::kTraceBuffer, chunks_ptr_vector_allocated_size, chunks_ptr_vector_resident_size);
...
void TraceBufferChunk::EstimateTraceMemoryOverhead(TraceEventMemoryOverhead* overhead)
{
    if (!cached_overhead_estimate_) {
        ...
        cached_overhead_estimate_->Add(TraceEventMemoryOverhead::kTraceBufferChunk, sizeof(*this) - sizeof(chunk_));
    }

    const size_t num_cached_estimated_events = cached_overhead_estimate_->GetCount(TraceEventMemoryOverhead::kTraceEvent);
    ...
    if (IsFull()) {
        ...
        overhead->Add(TraceEventMemoryOverhead::kUnusedTraceEvent, num_unused_trace_events * sizeof(TraceEvent));
    }

    overhead->Update(*cached_overhead_estimate_);
}

TraceResultBuffer::OutputCallback TraceResultBuffer::SimpleOutput::GetCallback()
{
    return BindRepeating(&SimpleOutput::Append, Unretained(this));
}

void TraceResultBuffer::SimpleOutput::Append(const std::string& json_trace_output)
{
