...
// the member function.
template <class T> auto EstimateMemoryUsage(const T& object) -> decltype(object.EstimateMemoryUsage());

// String

template <class C, class T, class A> size_t EstimateMemoryUsage(const std::basic_string<C, T, A>& string);

// Arrays

template <class T, size_t N> size_t EstimateMemoryUsage(const std::array<T, N>& array);

template <class T, size_t N> size_t EstimateMemoryUsage(T (&array)[N]);

template <class T> size_t EstimateMemoryUsage(const base::HeapArray<T>& array);

template <class T> size_t EstimateMemoryUsage(base::span<T> array);

// std::unique_ptr

template <class T, class D> size_t EstimateMemoryUsage(const std::unique_ptr<T, D>& ptr);

// std::shared_ptr

template <class T> size_t EstimateMemoryUsage(const std::shared_ptr<T>& ptr);

// Containers

template <class F, class S> size_t EstimateMemoryUsage(const std::pair<F, S>& pair);

template <class T, class A> size_t EstimateMemoryUsage(const std::vector<T, A>& vector);

template <class T, class A> size_t EstimateMemoryUsage(const std::list<T, A>& list);

template <class T> size_t EstimateMemoryUsage(const base::LinkedList<T>& list);

template <class T, class C, class A> size_t EstimateMemoryUsage(const std::set<T, C, A>& set);

template <class T, class C, class A> size_t EstimateMemoryUsage(const std::multiset<T, C, A>& set);

template <class K, class V, class C, class A> size_t EstimateMemoryUsage(const std::map<K, V, C, A>& map);

template <class K, class V, class C, class A> size_t EstimateMemoryUsage(const std::multimap<K, V, C, A>& map);

template <class T, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_set<T, H, KE, A>& set);

template <class T, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_multiset<T, H, KE, A>& set);

template <class K, class V, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_map<K, V, H, KE, A>& map);

template <class K, class V, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_multimap<K, V, H, KE, A>& map);

template <class T, class A> size_t EstimateMemoryUsage(const std::deque<T, A>& deque);

template <class T, class C> size_t EstimateMemoryUsage(const std::queue<T, C>& queue);

template <class T, class C> size_t EstimateMemoryUsage(const std::priority_queue<T, C>& queue);

template <class T, class C> size_t EstimateMemoryUsage(const std::stack<T, C>& stack);

template <class T> size_t EstimateMemoryUsage(const base::circular_deque<T>& deque);

template <class T, class C> size_t EstimateMemoryUsage(const base::flat_set<T, C>& set);

template <class K, class V, class C> size_t EstimateMemoryUsage(const base::flat_map<K, V, C>& map);

template <class K, class V, class C> size_t EstimateMemoryUsage(const base::LRUCache<K, V, C>& lru);

template <class K, class V, class C> size_t EstimateMemoryUsage(const base::HashingLRUCache<K, V, C>& lru);

template <class V, class C> size_t EstimateMemoryUsage(const base::LRUCacheSet<V, C>& lru);

template <class V, class C> size_t EstimateMemoryUsage(const base::HashingLRUCacheSet<V, C>& lru);
...
template <typename I> using IteratorValueType = typename std::iterator_traits<I>::value_type;

template <typename I, typename InstantiatedContainer>
concept IsIteratorOfInstantiatedContainer
    = (std::same_as<typename InstantiatedContainer::iterator, I> || std::same_as<typename InstantiatedContainer::const_iterator, I>
        || std::same_as<typename InstantiatedContainer::reverse_iterator, I> || std::same_as<typename InstantiatedContainer::const_reverse_iterator, I>);

template <typename I, template <typename...> typename Container>
concept IsIteratorOfContainer = !std::is_pointer_v<I> && IsIteratorOfInstantiatedContainer<I, Container<IteratorValueType<I>>>;

// std::array has an extra required template argument.
template <typename T> using array_test_helper = std::array<T, 1>;
...
concept IsIteratorOfStandardContainer
    = IsIteratorOfContainer<T, array_test_helper> || IsIteratorOfContainer<T, std::vector> || IsIteratorOfContainer<T, std::deque>
    || IsIteratorOfContainer<T, std::list> || IsIteratorOfContainer<T, std::set> || IsIteratorOfContainer<T, std::multiset>;

template <typename T>
concept IsKnownNonAllocatingType = std::is_trivially_destructible_v<T> || base::IsRawPtrV<T> || IsIteratorOfStandardContainer<T>;
...
template <class T> size_t EstimateItemMemoryUsage(const T& value)
{
...
template <class I> size_t EstimateIterableMemoryUsage(const I& iterable)
{
...
template <class T> auto EstimateMemoryUsage(const T& object) -> decltype(object.EstimateMemoryUsage())
{
    static_assert(std::same_as<decltype(object.EstimateMemoryUsage()), size_t>, "'T::EstimateMemoryUsage() const' must return size_t.");
...
template <class T> size_t EstimateMemoryUsage(const std::shared_ptr<T>& ptr)
{
    ...
    return sizeof(SharedPointer) + (EstimateItemMemoryUsage(*ptr) + (use_count - 1)) / use_count;
}
...
template <class F, class S> size_t EstimateMemoryUsage(const std::pair<F, S>& pair)
{
    return EstimateItemMemoryUsage(pair.first) + EstimateItemMemoryUsage(pair.second);
}
...
template <class T> size_t EstimateMemoryUsage(const base::LinkedList<T>& list)
{
    ...
    for (base::LinkNode<T>* node = list.head(); node != list.end(); node = node->next()) {
...
template <class V> size_t EstimateTreeMemoryUsage(size_t size)
{
...
template <class T, class C, class A> size_t EstimateMemoryUsage(const std::set<T, C, A>& set)
{
    ...
    return EstimateTreeMemoryUsage<value_type>(set.size()) + EstimateIterableMemoryUsage(set);
}
...
template <class K, class V, class C, class A> size_t EstimateMemoryUsage(const std::map<K, V, C, A>& map)
{
    ...
    return EstimateTreeMemoryUsage<value_type>(map.size()) + EstimateIterableMemoryUsage(map);
}
...
template <class V> size_t HashMapBucketCountForTesting(size_t bucket_count)
{
    return bucket_count;
}

template <class LruCacheType> size_t DoEstimateMemoryUsageForLruCache(const LruCacheType& lru_cache)
{
    return EstimateMemoryUsage(lru_cache.ordering_) + EstimateMemoryUsage(lru_cache.index_);
}

} // namespace internal

template <class V> size_t EstimateHashMapMemoryUsage(size_t bucket_count, size_t size)
{
...
template <class K, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_set<K, H, KE, A>& set)
{
    using value_type = typename std::unordered_set<K, H, KE, A>::value_type;
    return EstimateHashMapMemoryUsage<value_type>(set.bucket_count(), set.size()) + EstimateIterableMemoryUsage(set);
}

template <class K, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_multiset<K, H, KE, A>& set)
{
...
template <class K, class V, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_map<K, V, H, KE, A>& map)
{
    ...
    return EstimateHashMapMemoryUsage<value_type>(map.bucket_count(), map.size()) + EstimateIterableMemoryUsage(map);
}

template <class K, class V, class H, class KE, class A> size_t EstimateMemoryUsage(const std::unordered_multimap<K, V, H, KE, A>& map)
{
    using value_type = typename std::unordered_multimap<K, V, H, KE, A>::value_type;
    return EstimateHashMapMemoryUsage<value_type>(map.bucket_count(), map.size()) + EstimateIterableMemoryUsage(map);
}
...
//                   if sizeof(T) > kBlockSize
    ...
    size_t block_length = (sizeof(T) > kBlockSize) ? kMinBlockLength : kBlockSize / sizeof(T);
    ...
    return (blocks * block_length * sizeof(T)) + EstimateIterableMemoryUsage(deque);
}
...
template <class K, class V, class C> size_t EstimateMemoryUsage(const LRUCache<K, V, C>& lru_cache)
{
    return internal::DoEstimateMemoryUsageForLruCache(lru_cache);
}

template <class K, class V, class C> size_t EstimateMemoryUsage(const HashingLRUCache<K, V, C>& lru_cache)
{
    return internal::DoEstimateMemoryUsageForLruCache(lru_cache);
}

template <class V, class C> size_t EstimateMemoryUsage(const LRUCacheSet<V, C>& lru_cache)
{
    return internal::DoEstimateMemoryUsageForLruCache(lru_cache);
}

template <class V, class C> size_t EstimateMemoryUsage(const HashingLRUCacheSet<V, C>& lru_cache)
{
    return internal::DoEstimateMemoryUsageForLruCache(lru_cache);
}
