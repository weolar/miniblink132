...
bool AreStringListsEquivalent(const std::vector<std::string>& list1, const std::vector<std::string>& list2)
{
...
TraceConfigCategoryFilter::TraceConfigCategoryFilter(const TraceConfigCategoryFilter& other) = default;

TraceConfigCategoryFilter::~TraceConfigCategoryFilter() = default;

TraceConfigCategoryFilter& TraceConfigCategoryFilter::operator=(const TraceConfigCategoryFilter& rhs) = default;

bool TraceConfigCategoryFilter::IsEquivalentTo(const TraceConfigCategoryFilter& other) const
{
    return AreStringListsEquivalent(included_categories_, other.included_categories_)
        && AreStringListsEquivalent(disabled_categories_, other.disabled_categories_)
        && AreStringListsEquivalent(excluded_categories_, other.excluded_categories_);
}

void TraceConfigCategoryFilter::InitializeFromString(std::string_view category_filter_string)
{
    std::vector<std::string_view> split = SplitStringPiece(category_filter_string, ",", TRIM_WHITESPACE, SPLIT_WANT_ALL);
...
void TraceConfigCategoryFilter::InitializeFromConfigDict(const Value::Dict& dict)
{
    const Value::List* included_category_list = dict.FindList(kIncludedCategoriesParam);
    if (included_category_list)
        SetCategoriesFromIncludedList(*included_category_list);
    const Value::List* excluded_category_list = dict.FindList(kExcludedCategoriesParam);
    if (excluded_category_list)
        SetCategoriesFromExcludedList(*excluded_category_list);
}

bool TraceConfigCategoryFilter::IsCategoryGroupEnabled(std::string_view category_group_name) const
{
    ...
    while (category_group_tokens.GetNext()) {
        ...
        DCHECK(IsCategoryNameAllowed(category_group_token)) << "Disallowed category string";
    ...
    return !category_group_disabled && had_enabled_by_default && included_categories_.empty();
}

bool TraceConfigCategoryFilter::IsCategoryEnabled(std::string_view category_name) const
{
...
void TraceConfigCategoryFilter::Merge(const TraceConfigCategoryFilter& config)
{
    ...
    if (!included_categories_.empty() && !config.included_categories_.empty()) {
        included_categories_.insert(included_categories_.end(), config.included_categories_.begin(), config.included_categories_.end());
    } else {
        included_categories_.clear();
    }

    disabled_categories_.insert(disabled_categories_.end(), config.disabled_categories_.begin(), config.disabled_categories_.end());
    excluded_categories_.insert(excluded_categories_.end(), config.excluded_categories_.begin(), config.excluded_categories_.end());
}
...
void TraceConfigCategoryFilter::ToDict(Value::Dict& dict) const
{
    ...
    categories.insert(categories.end(), disabled_categories_.begin(), disabled_categories_.end());
...
void TraceConfigCategoryFilter::SetCategoriesFromIncludedList(const Value::List& included_list)
{
    included_categories_.clear();
    for (const Value& item : included_list) {
        ...
        if (category.compare(0, strlen(TRACE_DISABLED_BY_DEFAULT("")), TRACE_DISABLED_BY_DEFAULT("")) == 0) {
...
void TraceConfigCategoryFilter::SetCategoriesFromExcludedList(const Value::List& excluded_list)
{
    excluded_categories_.clear();
...
void TraceConfigCategoryFilter::AddCategoriesToDict(const StringList& categories, const char* param, Value::Dict& dict) const
{
    if (categories.empty())
        return;
...
void TraceConfigCategoryFilter::WriteCategoryFilterString(const StringList& values, std::string* out, bool included) const
{
