...
constexpr auto kDumpProviderAllowlist = base::MakeFixedFlatSet<std::string_view>({
// clang-format on
#if BUILDFLAG(IS_ANDROID)
    base::android::MeminfoDumpProvider::kDumpProviderName, "android::ResourceManagerImpl",
#endif
        "AutocompleteController", "AXPlatformNode", "BlinkGC", "BlinkObjectCounters", "BlobStorageContext", "Canvas", "ClientDiscardableSharedMemoryManager",
#if BUILDFLAG(IS_MAC)
        "CommandBuffer",
#endif
        "ContextProviderCommandBuffer", "DOMStorage", "DawnSharedContext", "DevTools", "DiscardableSharedMemoryManager", "DownloadService",
        "ExtensionFunctions", "FontCaches", "FrameEvictionManager", "GrShaderCache", "HistoryReport", "cc::ResourcePool", "gpu::BufferManager",
        "gpu::RenderbufferManager", "gpu::ServiceDiscardableManager", "gpu::ServiceTransferCache", "gpu::SharedImageStub", "gpu::TextureManager",
        "hibernated_canvas", "vulkan", "IPCChannel", "InMemoryURLIndex", "IndexedDBBackingStore", "IndexedDBFactoryImpl", "JavaHeap", "LevelDB",
        "LeveldbValueStore", "LocalStorage", "MadvFreeDiscardableMemoryAllocator", "Malloc", "ManualFillingCache", "MemoryCache", "MojoHandleTable",
        "MojoLevelDB", "MojoMessages", "PartitionAlloc", "PartitionAlloc.AddressSpace", "ProcessMemoryMetrics", "SharedContextState", "SharedImageManager",
        "SharedMemoryTracker", "Skia", "Sql", "TabRestoreServiceHelper",
        "TextureOwner"
        "URLRequestContext",
        "V8Isolate", "WebMediaPlayer_MainThread", "WebMediaPlayer_MediaThread",
    // clang-format on
});

// A list of string names that are allowed for the memory allocator dumps in
// background mode.
constexpr auto kAllocatorDumpNameAllowlist = base::MakeFixedFlatSet<std::string_view>({
// clang-format off
...
bool IsMemoryAllocatorDumpNameInAllowlist(const std::string& name)
{
    ...
    for (size_t i = 0; i < length; ++i) {
        if (parsing_hex && absl::ascii_isxdigit(static_cast<unsigned char>(name[i]))) {
            continue;
        }
        parsing_hex = false;
    ...
    if (g_allocator_dump_name_allowlist_for_testing.empty()) {
        ...
        return base::Contains(g_allocator_dump_name_allowlist_for_testing, stripped_str);
    }
}

void SetDumpProviderAllowlistForTesting(base::span<const std::string_view> list)
{
    g_dump_provider_allowlist_for_testing = list;
}

void SetAllocatorDumpNameAllowlistForTesting(base::span<const std::string_view> list)
{
