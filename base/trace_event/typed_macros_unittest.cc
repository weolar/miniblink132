...
void EnableTrace(bool filter_debug_annotations = false)
{
    ...
    if (filter_debug_annotations) {
        for (auto& data_source : *config.mutable_data_sources()) {
            ...
            data_source.mutable_config()->set_track_event_config_raw(track_event_config.SerializeAsString());
        }
    }
...
public:
    TestTrackEvent()
    {
        CHECK_EQ(g_test_track_event, nullptr) << "Another instance of TestTrackEvent is already active";
        g_test_track_event = this;
    }

    ~TestTrackEvent() override
    {
        g_test_track_event = nullptr;
    }
...
public:
    TestTracePacket()
    {
        CHECK_EQ(g_test_trace_packet, nullptr) << "Another instance of TestTracePacket is already active";
        g_test_trace_packet = this;
    }

    ~TestTracePacket() override
    {
        g_test_trace_packet = nullptr;
    }
...
TrackEventHandle PrepareTrackEvent(bool filter_debug_annotations)
{
    ...
    return TrackEventHandle(g_test_track_event->event.get(), &g_test_track_event->incremental_state, g_test_track_event, filter_debug_annotations);
}
...
TracePacketHandle PrepareTracePacket()
{
    ...
    return TracePacketHandle(TracePacketHandle::PerfettoPacketHandle(g_test_trace_packet->packet.get()), g_test_trace_packet);
}
...
public:
    TypedTraceEventTest()
    {
        ...
        EnableTypedTraceEvents(&PrepareTrackEventWithDebugAnnotations, &PrepareTracePacket, &EmitEmptyPacket);
    }

    ~TypedTraceEventTest() override
    {
        ResetTypedTraceEventsForTesting();
    }
    ...
    void FlushTrace()
    {
        ...
        EXPECT_TRUE(trace.ParseFromArray(serialized_data.data(), serialized_data.size()));
        for (const auto& packet : trace.packet()) {
            if (packet.has_track_event()) {
                std::string serialized_event = packet.track_event().SerializeAsString();
                event_.prepare_called = true;
                event_.event->AppendRawProtoBytes(serialized_event.data(), serialized_event.size());
                event_.OnTrackEventCompleted();

                std::string serialized_interned_data = packet.interned_data().SerializeAsString();
                event_.incremental_state.serialized_interned_data->AppendRawProtoBytes(serialized_interned_data.data(), serialized_interned_data.size());

                std::string serialized_packet = packet.SerializeAsString();
                packet_.prepare_called = true;
                packet_.packet->AppendRawProtoBytes(serialized_packet.data(), serialized_packet.size());
    ...
    perfetto::protos::TrackEvent ParseTrackEvent()
    {
        ...
        EXPECT_TRUE(track_event.ParseFromArray(serialized_data.data(), serialized_data.size()));
        return track_event;
    }
...
TEST_F(TypedTraceEventTest, InternedData)
{
    ...
    TRACE_EVENT("cat", "Name", [&location, &iid](perfetto::EventContext ctx) {
        ...
        base::Location base_location = base::Location::CreateForTesting("TestFunction", "test.cc", 123, dummy_pc);
    ...
    auto serialized_data = event_.incremental_state.serialized_interned_data.SerializeAsArray();
    perfetto::protos::InternedData interned_data;
    EXPECT_TRUE(interned_data.ParseFromArray(serialized_data.data(), serialized_data.size()));
    ...
    EXPECT_EQ("", event_.incremental_state.serialized_interned_data.SerializeAsString());
...
TEST_F(TypedTraceEventTest, DISABLED_InstantProcessEvent)
{
    ...
    TRACE_EVENT_INSTANT("cat", "ProcessEvent", perfetto::ProcessTrack::Current(), [](perfetto::EventContext) {});
    auto track_event = ParseTrackEvent();
...
TEST_F(TypedTraceEventTest, InstantGlobalEvent)
{
    ...
    TRACE_EVENT_INSTANT("cat", "GlobalEvent", perfetto::Track::Global(1234), [](perfetto::EventContext) {});
    auto track_event = ParseTrackEvent();
    EXPECT_TRUE(track_event.has_track_uuid());
...
TEST_F(TypedTraceEventTest, InstantGlobalDefaultEvent)
{
    ...
    TRACE_EVENT_INSTANT("cat", "GlobalDefaultEvent", perfetto::Track::Global(0), [](perfetto::EventContext) {});
    auto track_event = ParseTrackEvent();
    EXPECT_TRUE(track_event.has_track_uuid());
...
TEST_F(TypedTraceEventTest, ChromeMojoInterfaceTag)
{
    ...
    EXPECT_EQ(track_event.chrome_mojo_event_info().mojo_interface_tag(), "MojoInterface");
...
public:
    ...
    {
        EnableTypedTraceEvents(&PrepareTrackEventFilterDebugAnnotations, &PrepareTracePacket, &EmitEmptyPacket);
    }
};
