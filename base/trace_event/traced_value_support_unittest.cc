...
public:
    explicit RefCountedData(std::string data)
        : data_(data)
    {
    }
...
struct WeakData {
public:
    explicit WeakData(std::string data)
        : data_(data)
    {
    }
...
{
    EXPECT_EQ(perfetto::TracedValueToString(scoped_refptr<RefCountedData>(nullptr)), "0x0");
    scoped_refptr<RefCountedData> data = base::MakeRefCounted<RefCountedData>("refcounted");
...
TEST(TracedValueSupportTest, Time)
{
    ...
    EXPECT_EQ(perfetto::TracedValueToString(base::Time() + base::Microseconds(42)), "42");
    EXPECT_EQ(perfetto::TracedValueToString(base::TimeTicks() + base::Microseconds(42)), "42");
}
...
TEST(TracedValueSupportTest, UTF16String)
{
    ...
    EXPECT_EQ(perfetto::TracedValueToString(static_cast<const char16_t*>(u"utf-16")), "utf-16");
...
TEST(TracedValueSupportTest, WideString)
{
    ...
    EXPECT_EQ(perfetto::TracedValueToString(static_cast<const wchar_t*>(L"wide")), "wide");
    EXPECT_EQ(perfetto::TracedValueToString(std::wstring(L"wide")), "wide");
}

TEST(TracedValueSupportTest, StdString)
{
    EXPECT_EQ(perfetto::TracedValueToString(std::string_view("string")), "string");
    EXPECT_EQ(perfetto::TracedValueToString(std::u16string_view(u"utf-16")), "utf-16");
