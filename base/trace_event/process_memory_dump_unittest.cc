...
constexpr std::string_view kTestDumpNameAllowlist[] = { "Allowlisted/TestName", "Allowlisted/TestName_0x?", "Allowlisted/0x?/TestName", "Allowlisted/0x?" };
...
std::optional<size_t> CountResidentBytesInSharedMemory(WritableSharedMemoryMapping& mapping)
{
    ...
    auto mapped = UNSAFE_BUFFERS(base::span(mapping.data(), mapping.mapped_size()));
    return ProcessMemoryDump::CountResidentBytesInSharedMemory(mapped.data(), mapped.size());
}
...
TEST(ProcessMemoryDumpTest, MoveConstructor)
{
    ...
    pmd1.AddOwnershipEdge(MemoryAllocatorDumpGuid(42), MemoryAllocatorDumpGuid(4242));
    ...
    EXPECT_EQ(MemoryDumpLevelOfDetail::kDetailed, pmd2.dump_args().level_of_detail);
...
{
    std::unique_ptr<ProcessMemoryDump> pmd1(new ProcessMemoryDump(kDetailedDumpArgs));
    pmd1->CreateAllocatorDump("mad1");
    pmd1->CreateAllocatorDump("mad2");
...
TEST(ProcessMemoryDumpTest, TakeAllDumpsFrom)
{
    ...
    std::unique_ptr<ProcessMemoryDump> pmd2(new ProcessMemoryDump(kDetailedDumpArgs));
    ...
    pmd2->AddOwnershipEdge(MemoryAllocatorDumpGuid(42), MemoryAllocatorDumpGuid(4242));
...
{
    std::unique_ptr<ProcessMemoryDump> pmd(new ProcessMemoryDump(kDetailedDumpArgs));
    ...
    pmd->AddOverridableOwnershipEdge(child1_dump->guid(), shm_dump1->guid(), 0 /* importance */);
    auto* child2_dump = pmd->CreateAllocatorDump("shared_mem/child/seg2");
    pmd->AddOwnershipEdge(child2_dump->guid(), shm_dump2->guid(), 3 /* importance */);
    MemoryAllocatorDumpGuid shared_mad_guid(1);
    pmd->CreateSharedGlobalAllocatorDump(shared_mad_guid);
    pmd->AddOverridableOwnershipEdge(shm_dump3->guid(), shared_mad_guid, 0 /* importance */);
    auto* child4_dump = pmd->CreateAllocatorDump("shared_mem/child/seg4");
    pmd->AddOverridableOwnershipEdge(child4_dump->guid(), shm_dump4->guid(), 4 /* importance */);

    const ProcessMemoryDump::AllocatorDumpEdgesMap& edges = pmd->allocator_dumps_edges();
    ...
    pmd->AddOwnershipEdge(child1_dump->guid(), shm_dump1->guid(), 1 /* importance */);
    pmd->AddOwnershipEdge(shm_dump3->guid(), shared_mad_guid, 2 /* importance */);
    // This should not change the old edges.
    pmd->AddOverridableOwnershipEdge(child2_dump->guid(), shm_dump2->guid(), 0 /* importance */);
    pmd->AddOwnershipEdge(child4_dump->guid(), shm_dump4->guid(), 0 /* importance */);
...
TEST(ProcessMemoryDumpTest, Suballocations)
{
    ...
    auto* pic2_dump = pmd->CreateAllocatorDump("picturemanager/picture2", MemoryAllocatorDumpGuid(0x42));
    ...
    auto anon_node_1_it = pmd->allocator_dumps().find(allocator_dump_name + "/__" + pic1_dump->guid().ToString());
    ...
    for (const auto& e : pmd->allocator_dumps_edges()) {
        found_edge[0] |= (e.first == pic1_dump->guid() && e.second.target == anon_node_1_it->second->guid());
        found_edge[1] |= (e.first == pic2_dump->guid() && e.second.target == anon_node_2_it->second->guid());
    }
...
TEST(ProcessMemoryDumpTest, SharedMemoryOwnershipTest)
{
    ...
    MemoryAllocatorDumpGuid shm_local_guid2 = pmd->GetDumpId(SharedMemoryTracker::GetDumpNameForTracing(shm_token2));
    MemoryAllocatorDumpGuid shm_global_guid2 = SharedMemoryTracker::GetGlobalDumpIdForTracing(shm_token2);
    pmd->AddOverridableOwnershipEdge(shm_local_guid2, shm_global_guid2, 0 /* importance */);

    pmd->CreateSharedMemoryOwnershipEdge(client_dump2->guid(), shm_token2, 1 /* importance */);
...
TEST(ProcessMemoryDumpTest, BackgroundModeTest)
{
    ...
    std::unique_ptr<ProcessMemoryDump> pmd(new ProcessMemoryDump(background_args));
    ...
    // Invalid dump names.
    EXPECT_EQ(black_hole_mad, pmd->CreateAllocatorDump("NotAllowlisted/TestName"));
    ...
    EXPECT_EQ(black_hole_mad, pmd->CreateAllocatorDump("Not/Allowlisted/TestName"));
    EXPECT_EQ(black_hole_mad, pmd->CreateAllocatorDump("Allowlisted/TestName/Google"));
    EXPECT_EQ(black_hole_mad, pmd->CreateAllocatorDump("Allowlisted/TestName/0x1a2Google"));
    EXPECT_EQ(black_hole_mad, pmd->CreateAllocatorDump("Allowlisted/TestName/__12/Google"));
    ...
    EXPECT_NE(black_hole_mad, pmd->CreateAllocatorDump("Allowlisted/TestName_0xA1b2"));
    EXPECT_NE(black_hole_mad, pmd->CreateAllocatorDump("Allowlisted/0xaB/TestName"));
...
TEST(ProcessMemoryDumpTest, MAYBE_CountResidentBytes)
{
    ...
    std::optional<size_t> res1 = ProcessMemoryDump::CountResidentBytes(memory1, size1);
    ...
    std::optional<size_t> res2 = ProcessMemoryDump::CountResidentBytes(memory2, kVeryLargeMemorySize);
...
// TODO(crbug.com/42050620): Counting resident bytes is not supported on
// Fuchsia.
#define MAYBE_CountResidentBytesInSharedMemory DISABLED_CountResidentBytesInSharedMemory
#else
...
TEST(ProcessMemoryDumpTest, MAYBE_CountResidentBytesInSharedMemory)
{
    ...
    {
        const size_t kDirtyMemorySize = 5 * page_size;
        auto region = base::WritableSharedMemoryRegion::Create(kDirtyMemorySize + page_size);
        base::WritableSharedMemoryMapping mapping = region.MapAt(page_size / 2, kDirtyMemorySize);
        base::span<uint8_t> mapping_mem(mapping);
        std::ranges::fill(mapping_mem, 0u);
        std::optional<size_t> res1 = CountResidentBytesInSharedMemory(mapping);
        ASSERT_TRUE(res1.has_value());
    ...
    {
        const size_t kVeryLargeMemorySize = 15 * 1024 * 1024;
        auto region = base::WritableSharedMemoryRegion::Create(kVeryLargeMemorySize);
        base::WritableSharedMemoryMapping mapping = region.Map();
        base::span<uint8_t> mapping_mem(mapping);
        std::ranges::fill(mapping_mem, 0u);
