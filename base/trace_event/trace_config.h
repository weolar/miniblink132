...
public:
    ...
    struct BASE_EXPORT MemoryDumpConfig {
        ...
        struct HeapProfiler {
            ...
            friend bool operator==(const HeapProfiler&, const HeapProfiler&) = default;

            uint32_t breakdown_threshold_bytes;
        };

        friend bool operator==(const MemoryDumpConfig&, const MemoryDumpConfig&) = default;
    ...
    public:
        ProcessFilterConfig();
        explicit ProcessFilterConfig(const std::unordered_set<base::ProcessId>& included_process_ids);
        ...
        friend bool operator==(const ProcessFilterConfig&, const ProcessFilterConfig&) = default;
    ...
    public:
        ...
        const Value::Dict& filter_args() const LIFETIME_BOUND
        {
            return args_;
        }
    ...
    TraceConfig(std::string_view category_filter_string, std::string_view trace_options_string);
    ...
    size_t GetTraceBufferSizeInKb() const
    {
        return trace_buffer_size_in_kb_;
    }
    ...
    void SetTraceBufferSizeInKb(size_t size)
    {
        trace_buffer_size_in_kb_ = size;
    }
    void EnableSystrace()
    {
    ...
    // TraceConfig.
    std::string ToPerfettoTrackEventConfigRaw(bool privacy_filtering_enabled) const;
...
private:
    ...
    void InitializeFromStrings(std::string_view category_filter_string, std::string_view trace_options_string);
