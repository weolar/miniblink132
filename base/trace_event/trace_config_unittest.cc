...
void CheckDefaultTraceConfigBehavior(const TraceConfig& tc)
{
    ...
    EXPECT_FALSE(tc.IsCategoryGroupEnabled("disabled-by-default-cc,disabled-by-default-cc2"));
}
...
// word2.
std::string SwapWords(const std::string& in_str, const std::string& word1, const std::string& word2)
{
    size_t pos1 = in_str.find(word1);
    size_t len1 = word1.size();
    size_t pos2 = in_str.find(word2);
    size_t len2 = word2.size();
    return in_str.substr(0, pos1) + word2 + in_str.substr(pos1 + len1, pos2 - pos1 - len1) + word1 + in_str.substr(pos2 + len2);
}
...
TEST(TraceConfigTest, TraceConfigFromValidLegacyFormat)
{
    ...
    EXPECT_FALSE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    EXPECT_STREQ("record-as-much-as-possible", config.ToTraceOptionsString().c_str());
    ...
    EXPECT_FALSE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    EXPECT_STREQ("record-continuously,enable-systrace", config.ToTraceOptionsString().c_str());
    ...
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    EXPECT_STREQ("record-as-much-as-possible,enable-argument-filter", config.ToTraceOptionsString().c_str());

    config = TraceConfig("", "enable-systrace,trace-to-console,enable-argument-filter");
    EXPECT_EQ(ECHO_TO_CONSOLE, config.GetTraceRecordMode());
    EXPECT_TRUE(config.IsSystraceEnabled());
    EXPECT_TRUE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    EXPECT_STREQ("trace-to-console,enable-systrace,enable-argument-filter", config.ToTraceOptionsString().c_str());

    config = TraceConfig("", "record-continuously, record-until-full, trace-to-console");
    EXPECT_EQ(ECHO_TO_CONSOLE, config.GetTraceRecordMode());
    EXPECT_FALSE(config.IsSystraceEnabled());
    EXPECT_FALSE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    ...
    EXPECT_STREQ("included,inc_pattern*,-excluded,-exc_pattern*", config.ToCategoryFilterString().c_str());
    ...
    EXPECT_STREQ("disabled-by-default-cc,-excluded", config.ToCategoryFilterString().c_str());

    config = TraceConfig("disabled-by-default-cc,included", "");
    EXPECT_STREQ("included,disabled-by-default-cc", config.ToCategoryFilterString().c_str());
    ...
    config = TraceConfig("included,-excluded,inc_pattern*,-exc_pattern*", "enable-systrace, trace-to-console");
    EXPECT_EQ(ECHO_TO_CONSOLE, config.GetTraceRecordMode());
    EXPECT_TRUE(config.IsSystraceEnabled());
    EXPECT_FALSE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
    EXPECT_STREQ("included,inc_pattern*,-excluded,-exc_pattern*", config.ToCategoryFilterString().c_str());
    EXPECT_STREQ("trace-to-console,enable-systrace", config.ToTraceOptionsString().c_str());
    ...
    config = TraceConfig("included,-excluded,inc_pattern*,-exc_pattern*", RECORD_CONTINUOUSLY);
    EXPECT_EQ(RECORD_CONTINUOUSLY, config.GetTraceRecordMode());
    EXPECT_FALSE(config.IsSystraceEnabled());
    EXPECT_FALSE(config.IsArgumentFilterEnabled());
    EXPECT_FALSE(config.IsEventPackageNameFilterEnabled());
...
TEST(TraceConfigTest, TraceConfigFromInvalidLegacyStrings)
{
    ...
    EXPECT_STREQ("record-until-full,enable-systrace", config.ToTraceOptionsString().c_str());
}
...
TEST(TraceConfigTest, ConstructDefaultTraceConfig)
{
    ...
    EXPECT_STREQ(kDefaultTraceConfigString, tc_empty_category_filter.ToString().c_str());
    ...
    EXPECT_STREQ(kDefaultTraceConfigString, tc_empty_json_string.ToString().c_str());
...
TEST(TraceConfigTest, EmptyAndAsteriskCategoryFilterString)
{
    ...
    EXPECT_FALSE(tc_empty.category_filter().IsCategoryEnabled("not-excluded-category"));
    EXPECT_TRUE(tc_asterisk.category_filter().IsCategoryEnabled("Category1"));
    EXPECT_TRUE(tc_asterisk.category_filter().IsCategoryEnabled("not-excluded-category"));
}

TEST(TraceConfigTest, DisabledByDefaultCategoryFilterString)
{
    TraceConfig tc("foo,disabled-by-default-foo", "");
    EXPECT_STREQ("foo,disabled-by-default-foo", tc.ToCategoryFilterString().c_str());
...
TEST(TraceConfigTest, TraceConfigFromDict)
{
    ...
    std::optional<Value> default_value = JSONReader::Read(kDefaultTraceConfigString);
    ...
    std::optional<Value> custom_value = JSONReader::Read(kCustomTraceConfigString);
    ...
    EXPECT_TRUE(custom_tc_str == kCustomTraceConfigString || custom_tc_str == SwapWords(kCustomTraceConfigString, "uma1", "uma2"));
...
TEST(TraceConfigTest, TraceConfigFromValidString)
{
    ...
    EXPECT_FALSE(tc.category_filter().IsCategoryEnabled("disabled-by-default-others"));
    EXPECT_FALSE(tc.category_filter().IsCategoryEnabled("not-excluded-nor-included"));
    ...
    EXPECT_STREQ("event_whitelist_predicate", event_filter.predicate_name().c_str());
    EXPECT_EQ(1u, event_filter.category_filter().included_categories().size());
    EXPECT_STREQ("*", event_filter.category_filter().included_categories()[0].c_str());
    EXPECT_EQ(1u, event_filter.category_filter().excluded_categories().size());
    EXPECT_STREQ("unfiltered_cat", event_filter.category_filter().excluded_categories()[0].c_str());
    EXPECT_FALSE(event_filter.filter_args().empty());

    std::string json_out;
    base::JSONWriter::Write(event_filter.filter_args(), &json_out);
    EXPECT_STREQ(json_out.c_str(), "{\"event_name_allowlist\":[\"a snake\",\"a dog\"]}");
    ...
    EXPECT_TRUE(tc2.category_filter().IsCategoryEnabled("non-disabled-by-default-pattern"));
    EXPECT_FALSE(tc2.category_filter().IsCategoryEnabled("disabled-by-default-pattern"));
...
TEST(TraceConfigTest, TraceConfigFromInvalidString)
{
    ...
    EXPECT_TRUE(tc.category_filter().IsCategoryEnabled("disabled-by-default-pattern"));
...
TEST(TraceConfigTest, IsCategoryGroupEnabled)
{
    ...
    EXPECT_STREQ("disabled-by-default-cc,-excluded", tc.ToCategoryFilterString().c_str());
    EXPECT_TRUE(tc.IsCategoryGroupEnabled("disabled-by-default-cc"));
    ...
    EXPECT_STREQ("included,disabled-by-default-cc", tc.ToCategoryFilterString().c_str());
    EXPECT_TRUE(tc.IsCategoryGroupEnabled("disabled-by-default-cc"));
    EXPECT_TRUE(tc.IsCategoryGroupEnabled("included"));
...
TEST(TraceConfigTest, IsCategoryNameAllowed)
{
    ...
    EXPECT_FALSE(TraceConfigCategoryFilter::IsCategoryNameAllowed(" bad_category "));
    EXPECT_FALSE(TraceConfigCategoryFilter::IsCategoryNameAllowed(" bad_category"));
    EXPECT_FALSE(TraceConfigCategoryFilter::IsCategoryNameAllowed("bad_category "));
    EXPECT_FALSE(TraceConfigCategoryFilter::IsCategoryNameAllowed("   bad_category"));
    EXPECT_FALSE(TraceConfigCategoryFilter::IsCategoryNameAllowed("bad_category   "));
    ...
    EXPECT_TRUE(TraceConfigCategoryFilter::IsCategoryNameAllowed("good_category"));
}
...
TEST(TraceConfigTest, TraceConfigFromMemoryConfigString)
{
    ...
    TraceConfig tc2(TraceConfigMemoryTestUtil::GetTraceConfig_LegacyPeriodicTriggers(200, 2000));
    ...
    EXPECT_EQ(200u, tc1.memory_dump_config().triggers[0].min_time_between_dumps_ms);
    EXPECT_EQ(MemoryDumpLevelOfDetail::kLight, tc1.memory_dump_config().triggers[0].level_of_detail);

    EXPECT_EQ(2000u, tc1.memory_dump_config().triggers[1].min_time_between_dumps_ms);
    EXPECT_EQ(MemoryDumpLevelOfDetail::kDetailed, tc1.memory_dump_config().triggers[1].level_of_detail);
    EXPECT_EQ(2048u, tc1.memory_dump_config().heap_profiler_options.breakdown_threshold_bytes);

    std::string tc_str3 = TraceConfigMemoryTestUtil::GetTraceConfig_BackgroundTrigger(1 /* period_ms */);
    ...
    EXPECT_EQ(MemoryDumpLevelOfDetail::kBackground, tc3.memory_dump_config().triggers[0].level_of_detail);
}
...
TEST(TraceConfigTest, EmptyMemoryDumpConfigTest)
{
    ...
    EXPECT_EQ(TraceConfigMemoryTestUtil::GetTraceConfig_EmptyTriggers(), tc.ToString());
    EXPECT_EQ(0u, tc.memory_dump_config().triggers.size());
    EXPECT_EQ(static_cast<uint32_t>(TraceConfig::MemoryDumpConfig::HeapProfiler::kDefaultBreakdownThresholdBytes),
...
TEST(TraceConfigTest, SystraceEventsSerialization)
{
    ...
    const TraceConfig tc1(MemoryDumpManager::kTraceCategory, tc.ToTraceOptionsString());
...
TEST(TraceConfigTest, IsConfigEquivalent)
{
    ...
    TraceConfig tc3("{\"record_mode\":\"record-until-full\","
