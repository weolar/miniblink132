...
{
    EXPECT_TRUE(LoadNativeLibrary(FilePath("/pkg/lib/libtest_shared_library.so"), nullptr));
}

TEST(NativeLibraryTest, LoadAbsolutePath_OutsideLibraryRoot)
{
    NativeLibraryLoadError error;
    EXPECT_FALSE(LoadNativeLibrary(FilePath("/pkg/tmp/libtest_shared_library.so"), &error));
    std::string expected_error = "Absolute library paths must begin with /pkg/lib";
...
public:
        ...
        : library_(nullptr)
    {
        ...
        library_ = LoadNativeLibraryWithOptions(exe_path.AppendASCII(kTestLibraryName), options, nullptr);
        CHECK(library_);
    }
    ...
    template <typename ReturnType, typename... Args> ReturnType Call(const char* function_name, Args... args)
    {
        return reinterpret_cast<ReturnType (*)(Args...)>(GetFunctionPointerFromNativeLibrary(library_, function_name))(args...);
    }
...
#if !BUILDFLAG(IS_ANDROID) && !defined(THREAD_SANITIZER) && !defined(MEMORY_SANITIZER)
