...
public:
    ...
    void PerformTask()
    {
        did_perform_task_ = true;
    }
    bool did_perform_task() const
    {
        return did_perform_task_;
    }
...
{
    OneShotEvent event;
    scoped_refptr<base::TestSimpleTaskRunner> runner(new base::TestSimpleTaskRunner);
    int i = 0;
    ...
    EXPECT_NE(pending_tasks[0].location.line_number(), pending_tasks[1].location.line_number()) << "Make sure FROM_HERE is propagated.";
}
...
void CheckSignaledAndPostIncrement(OneShotEvent* event, const scoped_refptr<base::SingleThreadTaskRunner>& runner, int* i)
{
    EXPECT_TRUE(event->is_signaled());
    event->Post(FROM_HERE, base::BindOnce(&Increment, i), runner);
}
...
TEST(OneShotEventTest, IsSignaledAndPostsFromCallbackWork)
{
    ...
    event.Post(FROM_HERE, base::BindOnce(&CheckSignaledAndPostIncrement, &event, runner, &i), runner);
    EXPECT_EQ(0, i);
    event.Signal();
...
TEST(OneShotEventTest, DropsCallbackRefUponSignalled)
{
    ...
    {
        auto ref_counted_class = base::MakeRefCounted<RefCountedClass>(&did_delete_instance);
        event.Post(FROM_HERE, base::BindOnce(&RefCountedClass::PerformTask, ref_counted_class), runner);
        event.Signal();
        runner->RunPendingTasks();
