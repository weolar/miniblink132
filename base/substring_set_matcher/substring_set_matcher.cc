...
bool ComparePatterns(const MatcherStringPattern* a, const MatcherStringPattern* b)
{
    return a->pattern() < b->pattern();
}

std::vector<const MatcherStringPattern*> GetVectorOfPointers(const std::vector<MatcherStringPattern>& patterns)
{
...
bool SubstringSetMatcher::Build(const std::vector<MatcherStringPattern>& patterns)
{
    return Build(GetVectorOfPointers(patterns));
}

bool SubstringSetMatcher::Build(std::vector<const MatcherStringPattern*> patterns)
{
...
bool SubstringSetMatcher::Match(const std::string& text, std::set<MatcherStringPattern::ID>* matches) const
{
...
SubstringSetMatcher::NodeID SubstringSetMatcher::GetTreeSize(const std::vector<const MatcherStringPattern*>& patterns) const
{
    ...
    for (; current != patterns.end(); ++last, ++current) {
        ...
        while (common_prefix < prefix_bound && last_pattern[common_prefix] == current_pattern[common_prefix]) {
            ++common_prefix;
        }
...
void SubstringSetMatcher::BuildAhoCorasickTree(const SubstringPatternVector& patterns)
{
    DCHECK(tree_.empty());
...
void SubstringSetMatcher::InsertPatternIntoAhoCorasickTree(const MatcherStringPattern* pattern)
{
    ...
    while (i != text_end) {
        tree_.emplace_back();
        current_node->SetEdge(static_cast<unsigned char>(*i), static_cast<NodeID>(tree_.size() - 1));
...
void SubstringSetMatcher::CreateFailureAndOutputEdges()
{
    ...
    while (!queue.empty()) {
        ...
        // of the current node.
        for (unsigned edge_idx = 0; edge_idx < current_node->num_edges(); ++edge_idx) {
            ...
            const AhoCorasickNode* failure_candidate_parent = &tree_[current_node->failure()];
            NodeID failure_candidate_id = failure_candidate_parent->GetEdge(edge.label);
            while (failure_candidate_id == kInvalidNodeID && failure_candidate_parent != root) {
            ...
            child->SetOutputLink(failure_candidate->IsEndOfPattern() ? failure_candidate_id : failure_candidate->output_link());
...
void SubstringSetMatcher::AccumulateMatchesForNode(const AhoCorasickNode* node, std::set<MatcherStringPattern::ID>* matches) const
{
    DCHECK(matches);
...
SubstringSetMatcher::AhoCorasickNode& SubstringSetMatcher::AhoCorasickNode::operator=(AhoCorasickNode&& other)
{
    if (edges_capacity_ != 0) {
...
SubstringSetMatcher::NodeID SubstringSetMatcher::AhoCorasickNode::GetEdgeNoInline(uint32_t label) const
{
    ...
    for (unsigned edge_idx = 0; edge_idx < num_edges(); edge_idx += 4) {
        const __m128i four = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&edges_.edges[edge_idx]));
...
void SubstringSetMatcher::AhoCorasickNode::SetEdge(uint32_t label, NodeID node)
{
    ...
    if (num_free_edges_ == 0) {
        ...
        unsigned old_capacity = edges_capacity_ == 0 ? kNumInlineEdges : edges_capacity_;
        ...
        for (unsigned edge_idx = old_capacity; edge_idx < new_capacity; ++edge_idx) {
...
size_t SubstringSetMatcher::AhoCorasickNode::EstimateMemoryUsage() const
{
    if (edges_capacity_ == 0) {
        ...
        return base::trace_event::EstimateMemoryUsage(base::span<const AhoCorasickEdge>(edges_.edges, edges_capacity_));
    }
}
