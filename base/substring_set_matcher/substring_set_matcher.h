...
public:
    ...
    bool Match(const std::string& text, std::set<MatcherStringPattern::ID>* matches) const;
...
private:
    ...
    public:
        ...
        const AhoCorasickEdge* edges() const
        {
            ...
            DCHECK_EQ(0u, reinterpret_cast<uintptr_t>(edges_.inline_edges) % alignof(AhoCorasickEdge));
        ...
        NodeID output_link() const
        {
            return GetEdge(kOutputLinkLabel);
        }
    ...
    NodeID GetTreeSize(const std::vector<const MatcherStringPattern*>& patterns) const;
    ...
    // represented by |node|.
    void AccumulateMatchesForNode(const AhoCorasickNode* node, std::set<MatcherStringPattern::ID>* matches) const;
