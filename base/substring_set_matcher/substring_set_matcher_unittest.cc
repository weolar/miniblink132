...
void TestOnePattern(const std::string& test_string, const std::string& pattern, bool is_match)
{
    std::string test = "TestOnePattern(" + test_string + ", " + pattern + ", " + (is_match ? "1" : "0") + ")";
...
void TestTwoPatterns(const std::string& test_string, const std::string& pattern_1, const std::string& pattern_2, bool is_match_1, bool is_match_2)
{
    std::string test
        = "TestTwoPatterns(" + test_string + ", " + pattern_1 + ", " + pattern_2 + ", " + (is_match_1 ? "1" : "0") + ", " + (is_match_2 ? "1" : "0") + ")";
...
TEST(SubstringSetMatcherTest, TestMatcher2)
{
    ...
    std::vector<const MatcherStringPattern*> patterns = { &pattern_1, &pattern_2, &pattern_3 };
...
TEST(SubstringSetMatcherTest, TestMatcher3)
{
    ...
    for (const MatcherStringPattern& pattern : patterns) {
        EXPECT_TRUE(matches.find(pattern.id()) != matches.end()) << pattern.pattern();
    }
}
