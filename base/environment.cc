...
private:
    bool GetVarImpl(std::string_view variable_name, std::string* result)
    {
        ...
        DWORD value_length = ::GetEnvironmentVariable(wide_name.c_str(), value.data(), value.size());
        if (value_length == 0) {
            return false;
        }
        CHECK_LE(value_length, value.size() - 1) << "value should fit in the buffer (including the null terminator)";
    ...
    bool SetVarImpl(std::string_view variable_name, const std::string& new_value)
    {
#if BUILDFLAG(IS_WIN)
