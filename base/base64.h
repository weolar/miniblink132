...
BASE_EXPORT void Base64EncodeAppend(span<const uint8_t> input, std::string* output);
...
BASE_EXPORT bool Base64Decode(std::string_view input, std::string* output, Base64DecodePolicy policy = Base64DecodePolicy::kStrict);

// Decodes the base64 input string. Returns `std::nullopt` if unsuccessful.
BASE_EXPORT std::optional<std::vector<uint8_t>> Base64Decode(std::string_view input);
