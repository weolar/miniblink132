...
public:
    ...
    DiscardableMemoryAllocator& operator=(const DiscardableMemoryAllocator&) = delete;
    ...
    virtual std::unique_ptr<DiscardableMemory> AllocateLockedDiscardableMemory(size_t size) = 0;
    ...
    std::unique_ptr<DiscardableMemory> AllocateLockedDiscardableMemoryWithRetryOrDie(size_t size, OnceClosure on_no_memory);
