...
private:
    ...
    static_assert(std::is_unsigned_v<decltype(ref_count_)>, "ref_count_ must be an unsigned type.");
...
//    base::RefCounted.
//
#define REQUIRE_ADOPTION_FOR_REFCOUNTED_TYPE() using RefCountPreferenceTag = ::base::subtle::StartRefCountFromOneTag
...
template <class T, typename Traits = DefaultRefCountedTraits<T>> class RefCounted : public subtle::RefCountedBase {
public:
...
private:
    ...
    template <typename U> static void DeleteInternal(const U* x)
    {
        delete x;
    }
};
...
template <class T, typename Traits = DefaultRefCountedThreadSafeTraits<T>> class RefCountedThreadSafe : public subtle::RefCountedThreadSafeBase {
public:
    using RefCountPreferenceTag = subtle::StartRefCountFromZeroTag;
...
//
template <typename T> class RefCountedData : public base::RefCountedThreadSafe<base::RefCountedData<T>> {
public:
    RefCountedData()
        : data()
    {
...
template <typename T> bool operator==(const RefCountedData<T>& lhs, const RefCountedData<T>& rhs)
{
