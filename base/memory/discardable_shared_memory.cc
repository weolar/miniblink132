...
// serialized format.
template <int> Time TimeFromWireFormat(int64_t value);
template <int> int64_t TimeToWireFormat(Time time);

// Serialize to Unix time when using 4-byte wire format.
// Note: 19 January 2038, this will cease to work.
template <> [[maybe_unused]] Time TimeFromWireFormat<4>(int64_t value)
{
    return value ? Time::UnixEpoch() + Seconds(value) : Time();
}
template <> [[maybe_unused]] int64_t TimeToWireFormat<4>(Time time)
{
    return time > Time::UnixEpoch() ? (time - Time::UnixEpoch()).InSeconds() : 0;
}

// Standard serialization format when using 8-byte wire format.
template <> [[maybe_unused]] Time TimeFromWireFormat<8>(int64_t value)
{
    return Time::FromInternalValue(value);
}
template <> [[maybe_unused]] int64_t TimeToWireFormat<8>(Time time)
{
...
SharedState* SharedStateFromSharedMemory(const WritableSharedMemoryMapping& shared_memory)
{
...
bool UseAshmemUnpinningForDiscardableMemory()
{
    ...
    if (base::DiscardableMemoryBackingFieldTrialIsEnabled()) {
        return base::GetDiscardableMemoryBackingFieldTrialGroup() == base::DiscardableMemoryTrialGroup::kAshmem;
    }
    return true;
}
...
bool DiscardableSharedMemory::CreateAndMap(size_t size)
{
    ...
    locked_page_count_ = AlignToPageSize(mapped_memory().size()) / base::GetPageSize();
#if DCHECK_IS_ON()
    ...
    subtle::Release_Store(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i, new_state.value.i);
    return true;
}
...
bool DiscardableSharedMemory::Map(size_t size)
{
    ...
    shared_memory_mapping_ = shared_memory_region_.MapAt(0, AlignToPageSize(sizeof(SharedState)) + size);
    if (!shared_memory_mapping_.IsValid())
        return false;
...
DiscardableSharedMemory::LockResult DiscardableSharedMemory::Lock(size_t offset, size_t length)
{
    ...
    if (!locked_page_count_) {
        ...
        SharedState new_state(SharedState::LOCKED, Time());
        SharedState result(subtle::Acquire_CompareAndSwap(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i, old_state.value.i, new_state.value.i));
...
void DiscardableSharedMemory::Unlock(size_t offset, size_t length)
{
    ...
    DCHECK_EQ((new_state.GetTimestamp() - Time::UnixEpoch()).InSeconds(), (current_time - Time::UnixEpoch()).InSeconds());
    SharedState result(subtle::Release_CompareAndSwap(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i, old_state.value.i, new_state.value.i));
...
{
    return shared_memory_mapping_.GetMemoryAsSpan<uint8_t>().subspan(AlignToPageSize(sizeof(SharedState)));
}

span<uint8_t> DiscardableSharedMemory::mapped_memory() const
{
    return shared_memory_mapping_.mapped_memory().subspan(AlignToPageSize(sizeof(SharedState)));
}
...
bool DiscardableSharedMemory::Purge(Time current_time)
{
    ...
    // the memory again.
    if (result.value.u != old_state.value.u) {
        last_known_usage_ = result.GetLockState() == SharedState::LOCKED ? current_time : result.GetTimestamp();
        return false;
    }
...
#define MADV_PURGE_ARGUMENT MADV_FREE
    ...
    zx_status_t status = zx::vmar::root_self()->op_range(ZX_VMO_OP_DECOMMIT, address_int, AlignToPageSize(mapped.size()), nullptr, 0);
...
bool DiscardableSharedMemory::IsMemoryResident() const
{
    ...
    SharedState result(subtle::NoBarrier_Load(&SharedStateFromSharedMemory(shared_memory_mapping_)->value.i));
...
void DiscardableSharedMemory::CreateSharedMemoryOwnershipEdge(
    trace_event::MemoryAllocatorDump* local_segment_dump, trace_event::ProcessMemoryDump* pmd, bool is_owned) const
{
// Memory dumps are only supported when tracing support is enabled,.
#if BUILDFLAG(ENABLE_BASE_TRACING)
    auto* shared_memory_dump = SharedMemoryTracker::GetOrCreateSharedMemoryDump(shared_memory_mapping_, pmd);
    ...
    local_segment_dump->AddScalar(trace_event::MemoryAllocatorDump::kNameSize, trace_event::MemoryAllocatorDump::kUnitsBytes, resident_size);
    ...
    if (is_owned) {
        pmd->CreateWeakSharedMemoryOwnershipEdge(local_segment_dump->guid(), shared_memory_guid, kImportance);
    } else {
        pmd->CreateSharedMemoryOwnershipEdge(local_segment_dump->guid(), shared_memory_guid, kImportance);
    }
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
}

// static
DiscardableSharedMemory::LockResult DiscardableSharedMemory::LockPages(const UnsafeSharedMemoryRegion& region, size_t offset, size_t length)
{
#if BUILDFLAG(IS_ANDROID)
    if (region.IsValid()) {
...
// static
void DiscardableSharedMemory::UnlockPages(const UnsafeSharedMemoryRegion& region, size_t offset, size_t length)
{
#if BUILDFLAG(IS_ANDROID)
    if (region.IsValid()) {
        if (UseAshmemUnpinningForDiscardableMemory()) {
