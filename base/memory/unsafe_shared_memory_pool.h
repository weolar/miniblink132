...
class BASE_EXPORT UnsafeSharedMemoryPool : public RefCountedThreadSafe<UnsafeSharedMemoryPool> {
public:
...
private:
    ...
    void ReleaseBuffer(UnsafeSharedMemoryRegion region, WritableSharedMemoryMapping mapping);
    ...
    std::vector<std::pair<UnsafeSharedMemoryRegion, WritableSharedMemoryMapping>> regions_ GUARDED_BY(lock_);
