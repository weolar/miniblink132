...
struct ScopedPathUnlinkerTraits {
    static const FilePath* InvalidValue()
    {
        return nullptr;
    }
...
bool CheckFDAccessMode(int fd, int expected_mode)
{
    ...
    if (mode != expected_mode) {
        // TODO(crbug.com/40574272): convert to DLOG when bug fixed.
        LOG(ERROR) << "Descriptor access mode (" << mode << ") differs from expected (" << expected_mode << ")";
        return false;
    }
...
// static
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(ScopedFDPair handle, Mode mode, size_t size, const UnguessableToken& guid)
{
...
{
    return handle_.fd.is_valid() && (mode_ == Mode::kWritable ? handle_.readonly_fd.is_valid() : true);
}
...
)
{
    ...
    return PlatformSharedMemoryRegion({ ScopedFD(shm_file.TakePlatformFile()), std::move(readonly_fd) }, mode, size, UnguessableToken::Create());
#endif // !BUILDFLAG(IS_NACL)
}
...
PlatformSharedMemoryRegion::PlatformSharedMemoryRegion(ScopedFDPair handle, Mode mode, size_t size, const UnguessableToken& guid)
