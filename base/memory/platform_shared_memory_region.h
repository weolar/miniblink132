...
public:
    ...
    // are invalid.
    static PlatformSharedMemoryRegion Take(ScopedPlatformSharedMemoryHandle handle, Mode mode, size_t size, const UnguessableToken& guid);
    ...
    static PlatformSharedMemoryRegion Take(ScopedFD handle, Mode mode, size_t size, const UnguessableToken& guid);
#endif
    ...
    PlatformSharedMemoryRegion& operator=(const PlatformSharedMemoryRegion&) = delete;
    ...
    std::optional<span<uint8_t>> MapAt(uint64_t offset, size_t size, SharedMemoryMapper* mapper) const;
...
private:
    ...
    static bool CheckPlatformHandlePermissionsCorrespondToMode(PlatformSharedMemoryHandle handle, Mode mode, size_t size);

    PlatformSharedMemoryRegion(ScopedPlatformSharedMemoryHandle handle, Mode mode, size_t size, const UnguessableToken& guid);
