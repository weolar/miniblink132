...
__declspec(allocate("prot$a")) __declspec(selectany) char __start_protected_memory;
...
// boundary.
#if (PA_BUILDFLAG(IS_ANDROID) && PA_BUILDFLAG(PA_ARCH_CPU_64_BITS)) || (PA_BUILDFLAG(IS_LINUX) && PA_BUILDFLAG(PA_ARCH_CPU_ARM64))
...
inline constexpr int kProtectedMemoryAlignment = 65536;
#elif defined(_MIPS_ARCH_LOONGSON) || PA_BUILDFLAG(PA_ARCH_CPU_LOONGARCH64) || defined(ARCH_CPU_LOONGARCH64)
// 16kb page size
...
#define DECLARE_PROTECTED_DATA constinit
#define DEFINE_PROTECTED_DATA constinit __attribute__((section("protected_memory")))
#elif BUILDFLAG(IS_MAC)
// The segment the section is in is defined with a linker flag in
// build/config/mac/BUILD.gn
#define DECLARE_PROTECTED_DATA constinit
#define DEFINE_PROTECTED_DATA constinit __attribute__((section("PROTECTED_MEMORY, protected_memory")))

extern char __start_protected_memory __asm("section$start$PROTECTED_MEMORY$protected_memory");
extern char __stop_protected_memory __asm("section$end$PROTECTED_MEMORY$protected_memory");
#else
...
template <typename T> class AutoWritableMemory;
...
template <typename T> class ProtectedDataHolder {
public:
    consteval ProtectedDataHolder() = default;

    T& GetReference() LIFETIME_BOUND
    {
        return *GetPointer();
    }
    const T& GetReference() const LIFETIME_BOUND
    {
        return *GetPointer();
    }
...
template <typename T> class ProtectedMemory {
public:
    ...
    consteval explicit ProtectedMemory()
        : data_()
    {
...
protected:
    ...
    template <typename T> static bool IsObjectInProtectedSection(const T& object)
    {
        const T* const ptr = std::addressof(object);
        const T* const ptr_end = ptr + 1;
        return (ptr >= internal::kProtectedMemoryStart) && (ptr_end <= internal::kProtectedMemoryEnd);
    }

    template <typename T> static void CheckObjectReadOnly(const T& object)
    {
        internal::CheckMemoryReadOnly(std::addressof(object));
    }

    template <typename T> static bool SetObjectReadWrite(T& object)
    {
    ...
    {
        return SetMemoryReadOnly(internal::kProtectedMemoryStart, internal::kProtectedMemoryEnd);
    }

    static bool IsSectionStartPageAligned()
    {
        const uintptr_t protected_memory_start = reinterpret_cast<uintptr_t>(internal::kProtectedMemoryStart);
        const uintptr_t page_start = bits::AlignDown(protected_memory_start, GetPageSize());
    ...
    struct WriterData {
        ...
        constinit __attribute__((section("protected_memory"), aligned(kProtectedMemoryAlignment)))
#else
        DEFINE_PROTECTED_DATA
#endif
        static inline size_t writers GUARDED_BY(writers_lock())
            = 0;
        ...
        constinit __attribute__((section("protected_memory_buffer"), aligned(kProtectedMemoryAlignment))) static inline bool protected_memory_section_buffer
            = false;
...
class BASE_EXPORT AutoWritableMemoryInitializer : public AutoWritableMemoryBase {
public:
#if BUILDFLAG(IS_WIN)
...
template <typename T> class AutoWritableMemory : public AutoWritableMemoryBase {
public:
...
#endif
    {
        ...
        if (WriterData::writers == 0) {
            ...
            CHECK(SetProtectedSectionReadOnly());
            CheckObjectReadOnly(*static_cast<const char*>(internal::kProtectedMemoryStart));
            CheckObjectReadOnly(WriterData::writers);
        }
#endif // BUILDFLAG(PROTECTED_MEMORY_ENABLED)
    }
    ...
    T& GetProtectedData()
    {
        return protected_memory_->data_.GetReference();
    }
    T* GetProtectedDataPtr()
    {
        return protected_memory_->data_.GetPointer();
    }
...
public:
    template <typename T, typename... U> explicit ProtectedMemoryInitializer(ProtectedMemory<T>& protected_memory, U&&... args)
    {
    ...
    ProtectedMemoryInitializer& operator=(const ProtectedMemoryInitializer&) = delete;
