...
public:
    ...
    template <typename U> SafeRef& operator=(const SafeRef<U>& other)
    {
        ref_ = other.ref_;
        ptr_ = other.ptr_; // raw_ptr<U> converts to raw_ptr<T>.
        // Avoid use-after-move.
    ...
    template <typename U> SafeRef& operator=(SafeRef<U>&& other)
    {
        ref_ = std::move(other.ref_);
        ptr_ = std::move(other.ptr_); // raw_ptr<U> converts to raw_ptr<T>.
        // Avoid use-after-move.
    ...
    template <typename U> std::strong_ordering operator<=>(const SafeRef<U>& other) const
    {
...
private:
    template <typename U, SafeRefTraits PassedTraits> friend class SafeRef;
    template <typename U> friend SafeRef<U> internal::MakeSafeRefFromWeakPtrInternals(internal::WeakReference&& ref, U* ptr);
    ...
    static constexpr RawPtrTraits PtrTrait = Traits == SafeRefTraits::kEmpty ? RawPtrTraits::kEmpty : DanglingUntriaged;
...
namespace internal {
template <typename T> SafeRef<T> MakeSafeRefFromWeakPtrInternals(internal::WeakReference&& ref, T* ptr)
{
