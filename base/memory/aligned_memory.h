...
template <class T> using AlignedHeapArray = HeapArray<T, AlignedFreeDeleter>;
...
template <class T> AlignedHeapArray<T> AlignedUninit(size_t capacity, size_t alignment = alignof(T))
{
    ...
    return UNSAFE_BUFFERS(HeapArray<T, AlignedFreeDeleter>::FromOwningPointer(static_cast<T*>(AlignedAlloc(bytes, alignment)), capacity));
}
...
template <class T> auto AlignedUninitCharArray(size_t capacity, size_t alignment = alignof(T))
{
    alignment = std::max(alignment, alignof(void*));
    CHECK_GE(alignment, alignof(T));
    CHECK_LE(capacity, SIZE_MAX / sizeof(T));
    ...
    auto uninit_array = UNSAFE_BUFFERS(HeapArray<char, AlignedFreeDeleter>::FromOwningPointer(static_cast<char*>(AlignedAlloc(bytes, alignment)), bytes));
    // SAFETY: `uninit_array` holds `capacity * sizeof(T)` bytes, so it has room
    // for `capacity` many objects of type `T`.
    auto uninit_span = UNSAFE_BUFFERS(span(reinterpret_cast<T*>(uninit_array.data()), capacity));
