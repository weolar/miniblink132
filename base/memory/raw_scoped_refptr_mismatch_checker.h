...
template <typename T> struct NeedsScopedRefptrButGetsRawPtr {
    static_assert(!std::is_reference_v<T>, "NeedsScopedRefptrButGetsRawPtr requires non-reference type.");

    // TODO(danakj): Should ban native references and
    // std::reference_wrapper here too.
    static constexpr bool value
        = (base::IsRawRef<T>::value && IsRefCountedType<base::RemoveRawRefT<T>>) || (base::IsRawPointer<T> && IsRefCountedType<base::RemoveRawPointerT<T>>);
};
