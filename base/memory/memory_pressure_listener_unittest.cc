...
public:
    ...
    void SetUp() override
    {
        listener_ = std::make_unique<MemoryPressureListener>(FROM_HERE, BindRepeating(&MemoryPressureListenerTest::OnMemoryPressure, Unretained(this)));
    }
...
protected:
    void ExpectNotification(void (*notification_function)(MemoryPressureLevel), MemoryPressureLevel level)
    {
    ...
    void ExpectNoNotification(void (*notification_function)(MemoryPressureLevel), MemoryPressureLevel level)
    {
...
private:
    MOCK_METHOD1(OnMemoryPressure, void(MemoryPressureListener::MemoryPressureLevel));
...
TEST_F(MemoryPressureListenerTest, NotifyMemoryPressure)
{
    ...
    ExpectNotification(&MemoryPressureListener::NotifyMemoryPressure, MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE);
    ExpectNotification(&MemoryPressureListener::SimulatePressureNotification, MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE);
    ...
    ExpectNoNotification(&MemoryPressureListener::NotifyMemoryPressure, MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE);
    ExpectNotification(&MemoryPressureListener::SimulatePressureNotification, MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_MODERATE);

    // Disable suppressing memory pressure notifications.
    MemoryPressureListener::SetNotificationsSuppressed(false);
    EXPECT_FALSE(MemoryPressureListener::AreNotificationsSuppressed());
    ExpectNotification(&MemoryPressureListener::NotifyMemoryPressure, MemoryPressureLevel::MEMORY_PRESSURE_LEVEL_CRITICAL);
