...
bool IsFreedHeapPointer(uintptr_t address)
{
    ...
    void* region_ptr;
    size_t region_size;
    const char* allocation_type = __asan_locate_address(reinterpret_cast<void*>(address), nullptr, 0, &region_ptr, &region_size);

    auto region_base = reinterpret_cast<uintptr_t>(region_ptr);
    if (strcmp(allocation_type, "heap") != 0 || address < region_base || address >= region_base + region_size) { // We exclude pointers one past the end
...
NOINLINE NOT_TAIL_CALLED void CrashImmediatelyOnUseAfterFree(uintptr_t address)
{
    NO_CODE_FOLDING();
...
void WrapPtr(uintptr_t address)
{
    ...
    if (service.is_instantiation_check_enabled() && IsFreedHeapPointer(address)) {
        RawPtrAsanService::SetPendingReport(RawPtrAsanService::ReportType::kInstantiation, reinterpret_cast<void*>(address));
...
{
    if (RawPtrAsanService::GetInstance().is_dereference_check_enabled() && IsFreedHeapPointer(address)) {
        RawPtrAsanService::SetPendingReport(RawPtrAsanService::ReportType::kDereference, reinterpret_cast<void*>(address));
...
void SafelyUnwrapForExtraction(uintptr_t address)
{
    ...
    if ((service.is_extraction_check_enabled() || service.is_dereference_check_enabled()) && IsFreedHeapPointer(address)) {
        RawPtrAsanService::SetPendingReport(RawPtrAsanService::ReportType::kExtraction, reinterpret_cast<void*>(address));
