...
bool RefCountedMemory::Equals(const scoped_refptr<RefCountedMemory>& other) const
{
...
{
    return base::span<const uint8_t>(bytes_.data(), bytes_.size());
}
...
{
    //return base::as_byte_span(string_);
    return base::span<const uint8_t>((const uint8_t*)string_.data(), string_.size() * sizeof(char16_t));
}
...
// static
scoped_refptr<RefCountedSharedMemoryMapping> RefCountedSharedMemoryMapping::CreateFromWholeRegion(const ReadOnlySharedMemoryRegion& region)
{
