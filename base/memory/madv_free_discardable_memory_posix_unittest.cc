...
class MadvFreeDiscardableMemoryPosixTester : public MadvFreeDiscardableMemoryPosix {
public:
...
protected:
    ...
    std::unique_ptr<MadvFreeDiscardableMemoryPosixTester> AllocateLockedDiscardableMemoryPagesForTest(size_t size_in_pages)
    {
        return std::make_unique<MadvFreeDiscardableMemoryPosixTester>(size_in_pages * kPageSize);
    }
};

using MadvFreeDiscardableMemoryDeathTest = MadvFreeDiscardableMemoryTest;

constexpr char kTestPattern[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";

TEST_F(MadvFreeDiscardableMemoryTest, AllocateAndUse)
{
    SUCCEED_IF_MADV_FREE_UNSUPPORTED();

    std::unique_ptr<MadvFreeDiscardableMemoryPosixTester> mem = AllocateLockedDiscardableMemoryPagesForTest(1);
...
TEST_F(MadvFreeDiscardableMemoryTest, LockAndUnlock)
{
    ...
    std::unique_ptr<MadvFreeDiscardableMemoryPosixTester> mem = AllocateLockedDiscardableMemoryPagesForTest(kPageCount);
