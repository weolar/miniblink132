...
static_assert(DefaultSingletonTraits<int>::kRegisterAtExit == true, "object must be deleted on process exit");
...
template <typename Type> struct CallbackTrait : public DefaultSingletonTraits<Type> {
...
struct CallbackSingletonWithStaticTrait::Trait : public StaticMemorySingletonTraits<CallbackSingletonWithStaticTrait> {
    static void Delete(CallbackSingletonWithStaticTrait* instance)
    {
        if (instance->callback_)
            (instance->callback_)();
        StaticMemorySingletonTraits<CallbackSingletonWithStaticTrait>::Delete(instance);
    }
};

template <class Type> class AlignedTestSingleton {
public:
    AlignedTestSingleton() = default;
    ~AlignedTestSingleton() = default;
    static AlignedTestSingleton* GetInstance()
    {
        return Singleton<AlignedTestSingleton, StaticMemorySingletonTraits<AlignedTestSingleton>>::get();
    }
...
{
    DefaultSingletonTraits<CallbackSingletonWithLeakTrait>::Delete(CallbackSingletonWithLeakTrait::GetInstance());
}
...
{
    CallbackSingletonWithStaticTrait* instance = CallbackSingletonWithStaticTrait::GetInstance();
    if (instance == nullptr) {
        return nullptr;
    } else {
...
TEST_F(SingletonTest, Alignment)
{
    ...
    AlignedTestSingleton<AlignedData<32>>* align32 = AlignedTestSingleton<AlignedData<32>>::GetInstance();
    AlignedTestSingleton<AlignedData<128>>* align128 = AlignedTestSingleton<AlignedData<128>>::GetInstance();
    AlignedTestSingleton<AlignedData<4096>>* align4096 = AlignedTestSingleton<AlignedData<4096>>::GetInstance();
