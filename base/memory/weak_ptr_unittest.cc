...
public:
    template <typename T> static void BindToCurrentSequence(WeakPtrFactory<T>& factory)
    {
...
template <class T> class OffThreadObjectCreator {
public:
    static T* NewObject()
    {
        T* result;
        {
            ...
            creator_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(OffThreadObjectCreator::CreateObject, &result));
        }
...
struct DerivedTargetMultipleInheritance : public VirtualDestructor, public Target { };
...
public:
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoCreateArrowFromTarget, arrow, target, &completion));
        completion.Wait();
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoCreateArrowFromArrow, arrow, other, &completion));
        completion.Wait();
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoDeleteTarget, object, &completion));
        completion.Wait();
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoCopyAndAssignArrow, object, &completion));
        completion.Wait();
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoCopyAndAssignArrowBase, object, &completion));
        completion.Wait();
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoDeleteArrow, object, &completion));
        completion.Wait();
    }
    ...
    Target* DeRef(const Arrow* arrow)
    {
        ...
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoDeRef, arrow, &result, &completion));
        completion.Wait();
        return result;
    }
    ...
    {
        WaitableEvent completion(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&BackgroundThread::DoBindToCurrentSequence, target_with_factory, &completion));
        completion.Wait();
    }

protected:
    static void DoCreateArrowFromArrow(Arrow** arrow, const Arrow* other, WaitableEvent* completion)
    {
        *arrow = new Arrow;
        **arrow = *other;
        completion->Signal();
    ...
    static void DoCreateArrowFromTarget(Arrow** arrow, Target* target, WaitableEvent* completion)
    {
        *arrow = new Arrow;
    ...
    static void DoDeRef(const Arrow* arrow, Target** result, WaitableEvent* completion)
    {
    ...
    static void DoCopyAndAssignArrowBase(Arrow* object, WaitableEvent* completion)
    {
        // Copy constructor.
    ...
    static void DoBindToCurrentSequence(TargetWithFactory* target_with_factory, WaitableEvent* completion)
    {
        subtle::BindWeakPtrFactoryForTesting::BindToCurrentSequence(target_with_factory->factory);
        completion->Signal();
    }
};
...
TEST(WeakPtrTest, ConstGetWeakPtr)
{
    ...
    EXPECT_EQ(non_const_test_target.weak_ptr_factory.GetWeakPtr()->Method(), "non-const method");
    EXPECT_EQ(const_test_target.weak_ptr_factory.GetMutableWeakPtr()->Method(), "non-const method");
}
