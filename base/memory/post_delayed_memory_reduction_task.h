...
void BASE_EXPORT PostDelayedMemoryReductionTask(
    scoped_refptr<SequencedTaskRunner> task_runner, const Location& from_here, OnceClosure task, base::TimeDelta delay);
...
public:
    ...
    {
        Start(posted_from, delay, BindOnce([](OnceClosure task, MemoryReductionTaskContext called_from_prefreeze) { std::move(task).Run(); }, std::move(task)));
    }
    void Start(const Location& posted_from, TimeDelta delay, OnceCallback<void(MemoryReductionTaskContext)> task);
...
private:
    ...
    public:
        ...
        virtual void Stop() = 0;
        virtual void Start(const Location& posted_from, TimeDelta delay, OnceCallback<void(MemoryReductionTaskContext)> task) = 0;
        virtual bool IsRunning() const = 0;
        virtual void SetTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner) = 0;
    };
