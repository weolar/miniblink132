...
ScopedPlatformSharedMemoryHandle GetPlatformHandle(PlatformSharedMemoryRegion& shmem_region, [[maybe_unused]] bool make_read_only)
{
#if BUILDFLAG(IS_FUCHSIA)
    if (make_read_only) {
        ...
        zx_status_t status = shmem_region.GetPlatformHandle()->duplicate(
            ZX_RIGHT_READ | ZX_RIGHT_MAP | ZX_RIGHT_TRANSFER | ZX_RIGHT_GET_PROPERTY | ZX_RIGHT_DUPLICATE, &scoped_handle);
...
std::string Serialize(PlatformSharedMemoryRegion shmem_region, bool is_read_only,
#if BUILDFLAG(IS_APPLE)
    ...
    [[maybe_unused]] LaunchOptions* launch_options)
{
    ...
    StrAppend(&serialized, { NumberToString(win::HandleToUint32(handle)), (launch_options->elevated ? ",p," : ",i,") });
#elif BUILDFLAG(IS_APPLE)
    // In the receiving child, the handle is looked up using the rendezvous key.
    launch_options->mach_ports_for_rendezvous.emplace(rendezvous_key, MachRendezvousPort(std::move(shmem_handle)));
    ...
    uint32_t handle_id = LaunchOptions::AddHandleToTransfer(&launch_options->handles_to_transfer, shmem_handle.release());
...
#endif
    DVLOG(1) << "Sharing fd=" << descriptor_to_share.get() << " with child process as fd_key=" << descriptor_key;
...
#error "Unsupported OS"
    ...
    StrAppend(&serialized,
        { NumberToString(shmem_token.GetHighForSerialization()), ",", NumberToString(shmem_token.GetLowForSerialization()), ",", NumberToString(shmem_size) });
...
std::optional<UnguessableToken> DeserializeGUID(std::string_view hi_part, std::string_view lo_part)
{
    uint64_t hi = 0;
    uint64_t lo = 0;
...
expected<PlatformSharedMemoryRegion, SharedMemoryError> Deserialize(std::string_view switch_value, PlatformSharedMemoryRegion::Mode mode)
{
    std::vector<std::string_view> tokens = SplitStringPiece(switch_value, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    if (tokens.size() != 5) {
    ...
    if (tokens[1] == "p") {
        ...
        win::ScopedHandle parent_handle(OpenProcess(PROCESS_ALL_ACCESS, FALSE, GetParentProcessId(GetCurrentProcess())));
        DuplicateHandle(parent_handle.get(), handle, GetCurrentProcess(), &handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    } else if (tokens[1] != "i") {
    ...
    apple::ScopedMachSendRight scoped_handle = rendezvous->TakeSendRight(static_cast<MachPortsForRendezvous::key_type>(shmem_handle));
...
#elif BUILDFLAG(IS_POSIX)
    DCHECK_EQ(tokens[1], "i");
    const int fd = GlobalDescriptors::GetInstance()->MaybeGet(checked_cast<GlobalDescriptors::Key>(shmem_handle));
    if (fd == -1) {
        LOG(ERROR) << "Failed global descriptor lookup: " << shmem_handle;
        return unexpected(SharedMemoryError::kGetFDFailed);
    }
    DVLOG(1) << "Opening shared memory handle " << fd << " shared as " << shmem_handle;
...
#error Unsupported OS
    ...
    return PlatformSharedMemoryRegion::Take(std::move(scoped_handle), mode, checked_cast<size_t>(size), guid.value());
}

} // namespace

void AddToLaunchParameters(std::string_view switch_name, ReadOnlySharedMemoryRegion read_only_memory_region,
#if BUILDFLAG(IS_APPLE)
    MachPortsForRendezvous::key_type rendezvous_key,
#elif BUILDFLAG(IS_POSIX)
    GlobalDescriptors::Key descriptor_key, ScopedFD& out_descriptor_to_share,
#endif
    CommandLine* command_line, LaunchOptions* launch_options)
{
    std::string switch_value = Serialize(ReadOnlySharedMemoryRegion::TakeHandleForSerialization(std::move(read_only_memory_region)),
        /*is_read_only=*/true,
#if BUILDFLAG(IS_APPLE)
        rendezvous_key,
#elif BUILDFLAG(IS_POSIX)
...
void AddToLaunchParameters(std::string_view switch_name, UnsafeSharedMemoryRegion unsafe_memory_region,
#if BUILDFLAG(IS_APPLE)
    MachPortsForRendezvous::key_type rendezvous_key,
#elif BUILDFLAG(IS_POSIX)
    GlobalDescriptors::Key descriptor_key, ScopedFD& out_descriptor_to_share,
    ...
    CommandLine* command_line, LaunchOptions* launch_options)
{
    std::string switch_value = Serialize(UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(unsafe_memory_region)),
        /*is_read_only=*/false,
#if BUILDFLAG(IS_APPLE)
        rendezvous_key,
#elif BUILDFLAG(IS_POSIX)
...
expected<UnsafeSharedMemoryRegion, SharedMemoryError> UnsafeSharedMemoryRegionFrom(std::string_view switch_value)
{
    auto platform_handle = Deserialize(switch_value, PlatformSharedMemoryRegion::Mode::kUnsafe);
    if (!platform_handle.has_value()) {
        return unexpected(platform_handle.error());
    }
    auto shmem_region = UnsafeSharedMemoryRegion::Deserialize(std::move(platform_handle).value());
...
expected<ReadOnlySharedMemoryRegion, SharedMemoryError> ReadOnlySharedMemoryRegionFrom(std::string_view switch_value)
{
    auto platform_handle = Deserialize(switch_value, PlatformSharedMemoryRegion::Mode::kReadOnly);
    if (!platform_handle.has_value()) {
        return unexpected(platform_handle.error());
    }
    auto shmem_region = ReadOnlySharedMemoryRegion::Deserialize(std::move(platform_handle).value());
    if (!shmem_region.IsValid()) {
