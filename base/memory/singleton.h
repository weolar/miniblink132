...
// at exit.
template <typename Type> struct LeakySingletonTraits : public DefaultSingletonTraits<Type> {
...
template <typename Type> alignas(Type) char StaticMemorySingletonTraits<Type>::buffer_[sizeof(Type)];
template <typename Type> std::atomic<bool> StaticMemorySingletonTraits<Type>::dead_ = false;
...
template <typename Type, typename Traits = DefaultSingletonTraits<Type>, typename DifferentiatingType = Type> class Singleton {
private:
    ...
    static Type* get()
    {
        ...
        return subtle::GetOrCreateLazyPointer(instance_, &CreatorFunc, nullptr, Traits::kRegisterAtExit ? OnExit : nullptr, nullptr);
    }
...
template <typename Type, typename Traits, typename DifferentiatingType> std::atomic<uintptr_t> Singleton<Type, Traits, DifferentiatingType>::instance_ = 0;
