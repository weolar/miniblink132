...
public:
    ...
    static MappedReadOnlyRegion Create(size_t size, SharedMemoryMapper* mapper = nullptr);
    ...
    // boundaries.
    static ReadOnlySharedMemoryRegion Deserialize(subtle::PlatformSharedMemoryRegion handle);
    ...
    // another.
    static subtle::PlatformSharedMemoryRegion TakeHandleForSerialization(ReadOnlySharedMemoryRegion region);
    ...
    ReadOnlySharedMemoryMapping MapAt(uint64_t offset, size_t size, SharedMemoryMapper* mapper = nullptr) const;
