...
{
    {
        auto mem = MakeRefCounted<RefCountedStaticMemory>(byte_span_from_cstring("static mem"));

        EXPECT_THAT(span(*mem), ElementsAre('s', 't', 'a', 't', 'i', 'c', ' ', 'm', 'e', 'm'));
    }
}
...
TEST(RefCountedMemoryUnitTest, RefCountedBytes)
{
    ...
    data.push_back(45);
    data.push_back(99);
    scoped_refptr<RefCountedMemory> mem = MakeRefCounted<RefCountedBytes>(std::move(data));
...
{
    scoped_refptr<RefCountedMemory> mem = MakeRefCounted<RefCountedString>(std::string("destroy me"));
...
{
    scoped_refptr<RefCountedMemory> mem1 = MakeRefCounted<RefCountedString>(std::string("same"));

    std::vector<uint8_t> d2 = { 's', 'a', 'm', 'e' };
    scoped_refptr<RefCountedMemory> mem2 = MakeRefCounted<RefCountedBytes>(std::move(d2));

    EXPECT_TRUE(mem1->Equals(mem2));

    std::string s3("diff");
    scoped_refptr<RefCountedMemory> mem3 = MakeRefCounted<RefCountedString>(std::move(s3));
...
{
    std::string s("str");
    scoped_refptr<RefCountedMemory> mem = MakeRefCounted<RefCountedString>(std::move(s));
