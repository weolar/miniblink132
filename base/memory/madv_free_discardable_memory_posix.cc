...
void* AllocatePages(size_t size_in_pages)
{
    ...
    void* data = mmap(nullptr, length, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    PCHECK(data != MAP_FAILED);

#if BUILDFLAG(IS_ANDROID)
    prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, data, length, "madv-free-discardable");
#endif
...
MadvFreeDiscardableMemoryPosix::MadvFreeDiscardableMemoryPosix(size_t size_in_bytes, std::atomic<size_t>* allocator_byte_count)
...
bool MadvFreeDiscardableMemoryPosix::LockPage(size_t page_index)
{
    ...
    static_assert(sizeof(intptr_t) == sizeof(std::atomic<intptr_t>), "Incompatible layout of std::atomic.");
    DCHECK(std::atomic<intptr_t> {}.is_lock_free());
    std::atomic<intptr_t>* page_as_atomic = reinterpret_cast<std::atomic<intptr_t>*>(static_cast<uint8_t*>(data_) + page_index * base::GetPageSize());
    ...
    if (!std::atomic_compare_exchange_strong_explicit(
            page_as_atomic, &expected, static_cast<intptr_t>(page_first_word_[page_index]), std::memory_order_relaxed, std::memory_order_relaxed)) {
        return false;
    }
...
void MadvFreeDiscardableMemoryPosix::DiscardPage(size_t page_index)
{
    ...
    int retval = madvise(static_cast<uint8_t*>(data_) + base::GetPageSize() * page_index, base::GetPageSize(), MADV_DONTNEED);
    DPCHECK(!retval);
}
...
trace_event::MemoryAllocatorDump* MadvFreeDiscardableMemoryPosix::CreateMemoryAllocatorDump(const char* name, trace_event::ProcessMemoryDump* pmd) const
{
#if BUILDFLAG(ENABLE_BASE_TRACING)
    DFAKE_SCOPED_LOCK(thread_collision_warner_);

    using base::trace_event::MemoryAllocatorDump;
    std::string allocator_dump_name = base::StringPrintf("discardable/segment_0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this));
    ...
    // page-aligned.
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, is_discarded ? 0U : static_cast<uint64_t>(size_in_bytes_));

    allocator_dump->AddScalar(
        MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, is_discarded ? 0U : static_cast<uint64_t>(allocated_pages_ * base::GetPageSize()));
    allocator_dump->AddScalar(MemoryAllocatorDump::kNameObjectCount, MemoryAllocatorDump::kUnitsObjects, 1U);
    allocator_dump->AddScalar("wasted_size", MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(allocated_pages_ * base::GetPageSize() - size_in_bytes_));
    allocator_dump->AddScalar("locked_size", MemoryAllocatorDump::kUnitsBytes, is_locked_ ? size_in_bytes_ : 0U);
    allocator_dump->AddScalar("page_count", MemoryAllocatorDump::kUnitsObjects, static_cast<uint64_t>(allocated_pages_));
    ...
    allocator_dump->AddScalar("discarded_size", MemoryAllocatorDump::kUnitsBytes, is_discarded ? allocated_pages_ * base::GetPageSize() : 0U);
