...
protected:
    ...
    std::unique_ptr<test::ScopedFeatureList> GetScopedFeatureListForDiscardableMemoryTrialGroup(DiscardableMemoryTrialGroup group)
    {
        auto feature_list = std::make_unique<test::ScopedFeatureList>();
        feature_list->InitAndEnableFeatureWithParameters(base::features::kDiscardableMemoryBackingTrial,
            { { features::kDiscardableMemoryBackingParam.name, features::kDiscardableMemoryBackingParamOptions[group].name } });
        return feature_list;
    }
};
...
TEST_F(DiscardableMemoryBackingFieldTrialTest, TrialActiveOnlyIfCapable)
{
    std::unique_ptr<test::ScopedFeatureList> scoped_feature
        = GetScopedFeatureListForDiscardableMemoryTrialGroup(DiscardableMemoryTrialGroup::kEmulatedSharedMemory);
    FieldTrial* trial = FeatureList::GetFieldTrial(features::kDiscardableMemoryBackingTrial);
...
TEST_F(DiscardableMemoryBackingFieldTrialTest, EmulatedSharedMemoryBackingMatchesTrialGroup)
{
    std::unique_ptr<test::ScopedFeatureList> scoped_feature
        = GetScopedFeatureListForDiscardableMemoryTrialGroup(DiscardableMemoryTrialGroup::kEmulatedSharedMemory);
...
TEST_F(DiscardableMemoryBackingFieldTrialTest, MadvFreeBackingMatchesTrialGroup)
{
    std::unique_ptr<test::ScopedFeatureList> scoped_feature = GetScopedFeatureListForDiscardableMemoryTrialGroup(DiscardableMemoryTrialGroup::kMadvFree);
    if (!DiscardableMemoryBackingFieldTrialIsEnabled())
        return;
    DiscardableMemoryBacking backing = GetDiscardableMemoryBacking();
...
{
    std::unique_ptr<test::ScopedFeatureList> scoped_feature = GetScopedFeatureListForDiscardableMemoryTrialGroup(DiscardableMemoryTrialGroup::kAshmem);
    if (!DiscardableMemoryBackingFieldTrialIsEnabled())
        return;
    DiscardableMemoryBacking backing = GetDiscardableMemoryBacking();
    EXPECT_EQ(backing, DiscardableMemoryBacking::kSharedMemory);
