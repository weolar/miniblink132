...
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(0);
    EXPECT_FALSE(region.IsValid());

    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::CreateUnsafe(0);
...
{
    size_t too_large_region_size = static_cast<size_t>(std::numeric_limits<int>::max()) + 1;
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(too_large_region_size);
    EXPECT_FALSE(region.IsValid());

    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::CreateUnsafe(too_large_region_size);
    EXPECT_FALSE(region2.IsValid());
}
...
TEST_F(PlatformSharedMemoryRegionTest, CreateMaxSizeRegionIsInvalid)
{
    ...
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(max_region_size);
    EXPECT_FALSE(region.IsValid());

    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::CreateUnsafe(max_region_size);
    EXPECT_FALSE(region2.IsValid());
}
...
TEST_F(PlatformSharedMemoryRegionTest, ReportedSizeIsRequestedSize)
{
    ...
    for (size_t size : kTestSizes) {
        PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(size);
...
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
...
TEST_F(PlatformSharedMemoryRegionTest, ReadOnlyHandleIsNotWritable)
{
    ...
    EXPECT_EQ(region.GetMode(), PlatformSharedMemoryRegion::Mode::kReadOnly);
    EXPECT_TRUE(CheckReadOnlyPlatformSharedMemoryRegionForTesting(std::move(region)));
}
...
TEST_F(PlatformSharedMemoryRegionTest, TakeRegionOfZeroSizeIsInvalid)
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
    ASSERT_TRUE(region.IsValid());
    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::Take(region.PassPlatformHandle(), region.GetMode(), 0, region.GetGUID());
    EXPECT_FALSE(region2.IsValid());
}
...
TEST_F(PlatformSharedMemoryRegionTest, TakeTooLargeRegionIsInvalid)
{
    ...
    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::Take(
        region.PassPlatformHandle(), region.GetMode(), static_cast<size_t>(std::numeric_limits<int>::max()) + 1, region.GetGUID());
    EXPECT_FALSE(region2.IsValid());
}
...
TEST_F(PlatformSharedMemoryRegionTest, MapAtOutOfTheRegionLimitsTest)
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
    ASSERT_TRUE(region.IsValid());
    WritableSharedMemoryMapping mapping = MapAtForTesting(&region, 0, region.GetSize() + 1);
    EXPECT_FALSE(mapping.IsValid());
}
...
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(SysInfo::VMAllocationGranularity() * 2);
    ASSERT_TRUE(region.IsValid());
...
// POSIX.
TEST_F(PlatformSharedMemoryRegionTest, ConvertToReadOnlyInvalidatesSecondHandle)
{
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
    ASSERT_TRUE(region.IsValid());
    ASSERT_TRUE(region.ConvertToReadOnly());
...
void CheckReadOnlyMapProtection(void* addr)
{
    ...
    kern_return_t kr = vm_region_64(mach_task_self(), reinterpret_cast<vm_address_t*>(&addr), &dummy_size, VM_REGION_BASIC_INFO_64,
        reinterpret_cast<vm_region_info_t>(&basic_info), &info_count, &object_name);
    ...
    auto it = ranges::find_if(regions, [addr](const base::debug::MappedMemoryRegion& region) { return region.start == reinterpret_cast<uintptr_t>(addr); });
    ...
    size_t result = VirtualQueryEx(GetCurrentProcess(), addr, &memory_info, sizeof(memory_info));

    ASSERT_GT(result, 0ULL) << "Failed executing VirtualQueryEx " << logging::SystemErrorCodeToString(logging::GetLastSystemErrorCode());
...
#elif BUILDFLAG(IS_FUCHSIA)
    zx_status_t status = zx::vmar::root_self()->protect(ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, reinterpret_cast<uintptr_t>(addr), len);
    return status == ZX_OK;
#else
    return false;
#endif
...
TEST_F(PlatformSharedMemoryRegionTest, MappingProtectionSetCorrectly)
{
    ...
    auto full_map_mem = UNSAFE_BUFFERS(span(ro_mapping.data(), ro_mapping.mapped_size()));
    EXPECT_FALSE(TryToRestoreWritablePermissions(full_map_mem.data(), full_map_mem.size()));

    CheckReadOnlyMapProtection(ro_mapping.data());
}

// Tests that platform handle permissions are checked correctly.
TEST_F(PlatformSharedMemoryRegionTest, CheckPlatformHandlePermissionsCorrespondToMode)
{
    using Mode = PlatformSharedMemoryRegion::Mode;
    auto check = [](const PlatformSharedMemoryRegion& region, PlatformSharedMemoryRegion::Mode mode) {
        return PlatformSharedMemoryRegion::CheckPlatformHandlePermissionsCorrespondToMode(region.GetPlatformHandle(), mode, region.GetSize());
    };
    ...
    PlatformSharedMemoryRegion region2 = PlatformSharedMemoryRegion::CreateUnsafe(kRegionSize);
...
#else
    const char kErrorRegex[] = "Creating a region in read-only mode will lead to this region being "
                               "non-modifiable";
#endif
    EXPECT_DEATH_IF_SUPPORTED(PlatformSharedMemoryRegion::Create(PlatformSharedMemoryRegion::Mode::kReadOnly, kRegionSize), kErrorRegex);
}
...
TEST_F(PlatformSharedMemoryRegionTest, DuplicateWritableRegionDeathTest)
{
    ...
    const char kErrorRegex[] = "";
#else
    const char kErrorRegex[] = "Duplicating a writable shared memory region is prohibited";
#endif
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
    ASSERT_TRUE(region.IsValid());
...
TEST_F(PlatformSharedMemoryRegionTest, UnsafeRegionConvertToReadOnlyDeathTest)
{
    ...
    const char kErrorRegex[] = "";
#else
    const char kErrorRegex[] = "Only writable shared memory region can be converted to read-only";
#endif
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateUnsafe(kRegionSize);
    ASSERT_TRUE(region.IsValid());
...
TEST_F(PlatformSharedMemoryRegionTest, ReadOnlyRegionConvertToReadOnlyDeathTest)
{
#ifdef OFFICIAL_BUILD
    const char kErrorRegex[] = "";
#else
...
TEST_F(PlatformSharedMemoryRegionTest, ReadOnlyRegionConvertToUnsafeDeathTest)
{
    ...
    const char kErrorRegex[] = "";
#else
    const char kErrorRegex[] = "Only writable shared memory region can be converted to unsafe";
#endif
    PlatformSharedMemoryRegion region = PlatformSharedMemoryRegion::CreateWritable(kRegionSize);
    ASSERT_TRUE(region.IsValid());
    ASSERT_TRUE(region.ConvertToReadOnly());
