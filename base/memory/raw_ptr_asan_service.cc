...
{
    uint8_t* header = static_cast<uint8_t*>(const_cast<void*>(ptr)) - kChunkHeaderSize;
    *RawPtrAsanService::GetInstance().GetShadow(header) = kAsanUserPoisonedMemoryMagic;
}
...
void RawPtrAsanService::Configure(
    EnableDereferenceCheck enable_dereference_check, EnableExtractionCheck enable_extraction_check, EnableInstantiationCheck enable_instantiation_check)
{
    CHECK_EQ(mode_, Mode::kUninitialized);

    Mode new_mode = enable_dereference_check || enable_extraction_check || enable_instantiation_check ? Mode::kEnabled : Mode::kDisabled;
    if (new_mode == Mode::kEnabled) {
        ...
        CHECK_EQ(*GetShadow(dummy_alloc - kChunkHeaderSize), kAsanHeapLeftRedzoneMagic);
...
{
    return reinterpret_cast<uint8_t*>((reinterpret_cast<uintptr_t>(ptr) >> kShadowScale) + shadow_offset_);
}

// static
void RawPtrAsanService::SetPendingReport(ReportType type, const volatile void* ptr)
{
    // The actual ASan crash may occur at an offset from the pointer passed
    // here, so track the whole region.
    void* region_base;
    size_t region_size;
    __asan_locate_address(const_cast<void*>(ptr), nullptr, 0, &region_base, &region_size);
...
void RawPtrAsanService::ErrorReportCallback(const char* report, bool*)
{
    ...
    if (pending_report.allocation_base <= ptr && ptr < pending_report.allocation_base + pending_report.allocation_size) {
        bool is_supported_allocation = RawPtrAsanService::GetInstance().IsSupportedAllocation(reinterpret_cast<void*>(pending_report.allocation_base));
        ...
        case ReportType::kExtraction: {
            if (is_supported_allocation && bound_arg_ptr) {
                ...
                crash_info = { ProtectionStatus::kManualAnalysisRequired,
        ...
        bool is_supported_allocation = RawPtrAsanService::GetInstance().IsSupportedAllocation(reinterpret_cast<void*>(bound_arg_ptr));
        if (is_supported_allocation) {
            crash_info = { ProtectionStatus::kProtected,
        ...
        crash_info = { ProtectionStatus::kNotProtected, "No raw_ptr<T> access to this region was detected prior to this crash.",
            "This crash is still exploitable with MiraclePtr." };
    }
    ...
    if (crash_info.protection_status != ProtectionStatus::kNotProtected) {
        ...
        __asan_get_free_stack(reinterpret_cast<void*>(ptr), nullptr, 0, &free_thread_id);
        if (free_thread_id != GetCurrentThreadId()) {
            crash_info.protection_status = ProtectionStatus::kManualAnalysisRequired;
            crash_info.protection_details = "The \"use\" and \"free\" threads don't match. This crash is likely "
            ...
            crash_info.protection_status = ProtectionStatus::kManualAnalysisRequired;
            crash_info.protection_details = "This crash occurred in the thread pool. The sequence which invoked "
    ...
    debug::AsanService::GetInstance()->Log("\nMiraclePtr Status: %s\n%s\n%s\n"
        ...
        ProtectionStatusToString(crash_info.protection_status), crash_info.crash_details, crash_info.protection_details);
}
...
void Log(MessageLevel level, uintptr_t address, const char* type, const char* description)
{
#if __has_builtin(__builtin_extract_return_addr) && __has_builtin(__builtin_return_address)
    ...
    debug::AsanService::GetInstance()->Log("=================================================================\n"
        ...
        Process::Current().Pid(), LevelToString(level), type, address, pc, bp, sp);
    __sanitizer_print_stack_trace();
    __asan_describe_address(reinterpret_cast<void*>(address));
    debug::AsanService::GetInstance()->Log("%s\n"
                                           "=================================================================",
        description);
}
} // namespace

void RawPtrAsanService::WarnOnDanglingExtraction(const volatile void* ptr) const
{
    Log(MessageLevel::kWarning, reinterpret_cast<uintptr_t>(ptr), "dangling-pointer-extraction",
...
void RawPtrAsanService::CrashOnDanglingInstantiation(const volatile void* ptr) const
{
    Log(MessageLevel::kError, reinterpret_cast<uintptr_t>(ptr), "dangling-pointer-instantiation",
