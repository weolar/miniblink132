...
#define ASAN_BRP_MANUAL_ANALYSIS(x) "MiraclePtr Status: MANUAL ANALYSIS REQUIRED\\n.*" x
#define ASAN_BRP_NOT_PROTECTED(x) "MiraclePtr Status: NOT PROTECTED\\n.*" x

const char kAsanBrpProtected_Dereference[] = ASAN_BRP_PROTECTED("dangling pointer was being dereferenced");
const char kAsanBrpProtected_Callback[] = ASAN_BRP_PROTECTED("crash occurred inside a callback where a raw_ptr<T> pointing to the same "
                                                             "region");
const char kAsanBrpMaybeProtected_Extraction[] = ASAN_BRP_MANUAL_ANALYSIS("pointer to the same region was extracted from a raw_ptr<T>");
const char kAsanBrpNotProtected_EarlyAllocation[] = ASAN_BRP_NOT_PROTECTED("crash occurred while accessing a region that was allocated before "
                                                                           "MiraclePtr was activated");
const char kAsanBrpNotProtected_NoRawPtrAccess[] = ASAN_BRP_NOT_PROTECTED("No raw_ptr<T> access to this region was detected");
const char kAsanBrpMaybeProtected_Race[] = ASAN_BRP_MANUAL_ANALYSIS("\\nThe \"use\" and \"free\" threads don't match");
const char kAsanBrpMaybeProtected_ThreadPool[] = ASAN_BRP_MANUAL_ANALYSIS("\\nThis crash occurred in the thread pool");

// Instantiation failure message format is special.
const char kAsanBrp_Instantiation[] = "crash occurred due to an attempt to assign a dangling pointer";
    ...
    void SetUp() override
    {
        ...
        if (!RawPtrAsanService::GetInstance().IsEnabled()) {
            base::RawPtrAsanService::GetInstance().Configure(
                base::EnableDereferenceCheck(true), base::EnableExtractionCheck(true), base::EnableInstantiationCheck(true));
        } else {
            ASSERT_TRUE(base::RawPtrAsanService::GetInstance().is_dereference_check_enabled());
            ASSERT_TRUE(base::RawPtrAsanService::GetInstance().is_extraction_check_enabled());
            ASSERT_TRUE(base::RawPtrAsanService::GetInstance().is_instantiation_check_enabled());
        }
    }
...
TEST_F(AsanBackupRefPtrTest, Dereference)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED((*protected_ptr).x = 1, kAsanBrpProtected_Dereference);
    EXPECT_DEATH_IF_SUPPORTED((*protected_ptr).func(), kAsanBrpProtected_Dereference);
    EXPECT_DEATH_IF_SUPPORTED(++(protected_ptr->x), kAsanBrpProtected_Dereference);
    EXPECT_DEATH_IF_SUPPORTED(protected_ptr->func(), kAsanBrpProtected_Dereference);
...
TEST_F(AsanBackupRefPtrTest, Instantiation)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED({ [[maybe_unused]] raw_ptr<AsanStruct> protected_ptr2 = ptr; }, kAsanBrp_Instantiation);
}
...
TEST_F(AsanBackupRefPtrTest, InstantiationInvalidPointer)
{
    ...
    [[maybe_unused]] raw_ptr<void> protected_ptr2 = reinterpret_cast<void*>(shadow_offset); // Shouldn't crash.
}
...
TEST_F(AsanBackupRefPtrTest, UserPoisoned)
{
    ...
    [[maybe_unused]] raw_ptr<AsanStruct> protected_ptr1 = ptr; // Shouldn't crash.
...
TEST_F(AsanBackupRefPtrTest, EarlyAllocationDetection)
{
    ...
    EXPECT_FALSE(RawPtrAsanService::GetInstance().IsSupportedAllocation(early_allocation_ptr_.get()));
    EXPECT_TRUE(RawPtrAsanService::GetInstance().IsSupportedAllocation(late_allocation_ptr.get()));
    ...
    EXPECT_DEATH_IF_SUPPORTED({ early_allocation_ptr_->func(); }, kAsanBrpNotProtected_EarlyAllocation);
    EXPECT_DEATH_IF_SUPPORTED({ late_allocation_ptr->func(); }, kAsanBrpProtected_Dereference);
...
TEST_F(AsanBackupRefPtrTest, BoundRawPtr)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(std::move(ptr_callback).Run(), kAsanBrpProtected_Callback);
}
...
TEST_F(AsanBackupRefPtrTest, BoundArgumentsProtected)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(std::move(safe_callback).Run(), kAsanBrpProtected_Callback);
    EXPECT_DEATH_IF_SUPPORTED(std::move(safe_nested_inner_callback).Run(), kAsanBrpProtected_Callback);
    EXPECT_DEATH_IF_SUPPORTED(std::move(safe_nested_outer_callback).Run(), kAsanBrpProtected_Callback);
}
...
TEST_F(AsanBackupRefPtrTest, BoundArgumentsNotProtected)
{
    ...
    auto unsafe_nested_inner_callback = base::BindOnce(
        ...
        base::BindOnce(
            [](uintptr_t inner_address) {
                AsanStruct* inner_ptr = reinterpret_cast<AsanStruct*>(inner_address);
    ...
    EXPECT_DEATH_IF_SUPPORTED(std::move(unsafe_callback).Run(), kAsanBrpNotProtected_NoRawPtrAccess);
    EXPECT_DEATH_IF_SUPPORTED(std::move(unsafe_nested_inner_callback).Run(), kAsanBrpMaybeProtected_Extraction);
    EXPECT_DEATH_IF_SUPPORTED(std::move(unsafe_nested_outer_callback).Run(), kAsanBrpMaybeProtected_Extraction);
}
...
TEST_F(AsanBackupRefPtrTest, BoundReferences)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(std::move(callback).Run(), kAsanBrpProtected_Callback);
}
...
TEST_F(AsanBackupRefPtrTest, AccessOnThreadPoolThread)
{
    ...
    ThreadPool::PostTaskAndReply(FROM_HERE, {}, base::BindLambdaForTesting([&ptr, &protected_ptr] {
        ptr.reset();
        EXPECT_DEATH_IF_SUPPORTED(protected_ptr->func(), kAsanBrpMaybeProtected_ThreadPool);
    }),
