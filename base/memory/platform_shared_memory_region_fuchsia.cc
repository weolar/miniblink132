...
static constexpr int kNoWriteOrExec = ZX_DEFAULT_VMO_RIGHTS & ~(ZX_RIGHT_WRITE | ZX_RIGHT_EXECUTE | ZX_RIGHT_SET_PROPERTY);

// static
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(zx::vmo handle, Mode mode, size_t size, const UnguessableToken& guid)
{
    if (!handle.is_valid())
        return {};
    ...
    CHECK(CheckPlatformHandlePermissionsCorrespondToMode(zx::unowned_vmo(handle), mode, size));
...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Duplicate() const
{
    ...
    return PlatformSharedMemoryRegion(std::move(duped_handle), mode_, size_, guid_);
}
...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Create(Mode mode, size_t size)
{
    ...
    return PlatformSharedMemoryRegion(std::move(vmo), mode, size, UnguessableToken::Create());
}
...
bool PlatformSharedMemoryRegion::CheckPlatformHandlePermissionsCorrespondToMode(PlatformSharedMemoryHandle handle, Mode mode, size_t size)
{
    ...
    zx_status_t status = handle->get_info(ZX_INFO_HANDLE_BASIC, &basic, sizeof(basic), nullptr, nullptr);
    ...
    if (is_read_only != expected_read_only) {
        // TODO(crbug.com/40574272): convert to DLOG when bug fixed.
        LOG(ERROR) << "VMO object has wrong access rights: it is" << (is_read_only ? " " : " not ") << "read-only but it should"
...
PlatformSharedMemoryRegion::PlatformSharedMemoryRegion(zx::vmo handle, Mode mode, size_t size, const UnguessableToken& guid)
