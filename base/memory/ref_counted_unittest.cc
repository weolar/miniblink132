...
public:
    ...
    static void reset_was_destroyed()
    {
...
protected:
    ~ScopedRefPtrCountDerived() override
    {
        ++destructor_count_;
    }
...
class HasPrivateDestructorWithDeleter : public base::RefCounted<HasPrivateDestructorWithDeleter, Deleter> {
public:
...
public:
    ...
    void set_scoped_refptr(scoped_refptr<CheckRefptrNull>* ptr)
    {
        ptr_ = ptr;
    }
...
{
    scoped_refptr<InitialRefCountIsOne> obj = base::MakeRefCounted<InitialRefCountIsOne>();
    EXPECT_TRUE(obj->HasOneRef());
    obj.reset();

    scoped_refptr<InitialRefCountIsOne> obj2 = base::AdoptRef(new InitialRefCountIsOne);
...
TEST(RefCountedUnitTest, TestPrivateDestructorWithDeleter)
{
    ...
    scoped_refptr<HasPrivateDestructorWithDeleter> obj = base::MakeRefCounted<HasPrivateDestructorWithDeleter>();
}
...
TEST(RefCountedUnitTest, TestReset)
{
    ...
    scoped_refptr<ScopedRefPtrCountBase> obj1 = base::MakeRefCounted<ScopedRefPtrCountBase>();
...
public:
    ...
    static std::atomic_int& GetMutableRefCount(RefCountedThreadSafeBase* ref_counted)
    {
