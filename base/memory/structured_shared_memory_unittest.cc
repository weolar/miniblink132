...
public:
    std::optional<span<uint8_t>> Map(subtle::PlatformSharedMemoryHandle handle, bool write_allowed, uint64_t offset, size_t size) final
    {
        return std::nullopt;
    }
...
TEST(StructuredSharedMemoryTest, ReadWrite)
{
    ...
    auto read_only_memory = StructuredSharedMemory<double>::MapReadOnlyRegion(writable_memory->TakeReadOnlyRegion());
...
TEST(StructuredSharedMemoryTest, MapFailure)
{
    ...
    auto writable_memory = StructuredSharedMemory<double>::CreateWithCustomMapper(&failing_mapper);
    EXPECT_FALSE(writable_memory.has_value());

    // Initialize from same type, but fail.
    writable_memory = StructuredSharedMemory<double>::CreateWithCustomMapper(1.2, &failing_mapper);
    EXPECT_FALSE(writable_memory.has_value());

    // Initialize from compatible type, but fail.
    writable_memory = StructuredSharedMemory<double>::CreateWithCustomMapper(3, &failing_mapper);
    EXPECT_FALSE(writable_memory.has_value());

    // Fail to create read-only region (bad handle).
    ReadOnlySharedMemoryRegion region;
    EXPECT_FALSE(region.IsValid());
    auto read_only_memory = StructuredSharedMemory<double>::MapReadOnlyRegion(std::move(region));
    ...
    read_only_memory = StructuredSharedMemory<double>::MapReadOnlyRegion(std::move(region), &failing_mapper);
    EXPECT_FALSE(read_only_memory.has_value());
}
...
TEST(StructuredSharedMemoryDeathTest, DuplicateRegion)
{
    auto writable_memory = StructuredSharedMemory<double>::Create();
    ASSERT_TRUE(writable_memory.has_value());
    ReadOnlySharedMemoryRegion region = writable_memory->DuplicateReadOnlyRegion();
    ReadOnlySharedMemoryRegion region2 = writable_memory->DuplicateReadOnlyRegion();
...
TEST(StructuredSharedMemoryTest, AtomicReadWrite)
{
    ...
    auto read_only_memory = AtomicSharedMemory<int>::MapReadOnlyRegion(writable_memory->TakeReadOnlyRegion());
    ASSERT_TRUE(read_only_memory.has_value());
    EXPECT_EQ(read_only_memory->ReadOnlyPtr(), &read_only_memory->ReadOnlyRef());
