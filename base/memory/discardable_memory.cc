...
BASE_FEATURE(kMadvFreeDiscardableMemory, "MadvFreeDiscardableMemory", base::FEATURE_DISABLED_BY_DEFAULT);
#endif // BUILDFLAG(IS_POSIX)

#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
BASE_FEATURE(kDiscardableMemoryBackingTrial, "DiscardableMemoryBackingTrial", base::FEATURE_DISABLED_BY_DEFAULT);
...
#if BUILDFLAG(IS_POSIX)
    if (base::FeatureList::IsEnabled(base::features::kMadvFreeDiscardableMemory) && base::GetMadvFreeSupport() == base::MadvFreeSupport::kSupported) {
        return DiscardableMemoryBacking::kMadvFree;
    }
#endif // BUILDFLAG(IS_POSIX)
