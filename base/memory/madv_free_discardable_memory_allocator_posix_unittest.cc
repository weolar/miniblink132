...
#define SUCCEED_IF_MADV_FREE_UNSUPPORTED()                                                                                                                     \
    do {                                                                                                                                                       \
        if (GetMadvFreeSupport() != base::MadvFreeSupport::kSupported) {                                                                                       \
            SUCCEED() << "MADV_FREE is not supported (Linux 4.5+ required), vacuously "                                                                        \
                         "passing test";                                                                                                                       \
            return;                                                                                                                                            \
        }                                                                                                                                                      \
    } while (0)
...
protected:
    ...
    std::unique_ptr<MadvFreeDiscardableMemoryPosix> AllocateLockedMadvFreeDiscardableMemory(size_t size)
    {
        return std::unique_ptr<MadvFreeDiscardableMemoryPosix>(
            static_cast<MadvFreeDiscardableMemoryPosix*>(allocator_.AllocateLockedDiscardableMemory(size).release()));
    }

#if BUILDFLAG(ENABLE_BASE_TRACING)
    size_t GetDiscardableMemorySizeFromDump(const DiscardableMemory& mem, const std::string& dump_id)
    {
        return mem.CreateMemoryAllocatorDump(dump_id.c_str(), pmd_.get())->GetSizeInternal();
    }
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
...
#if BUILDFLAG(ENABLE_BASE_TRACING)
    EXPECT_EQ(GetDiscardableMemorySizeFromDump(*mem1, "dummy_dump_1"), kPageSize * 3 + 1);
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
...
#if BUILDFLAG(ENABLE_BASE_TRACING)
    EXPECT_EQ(GetDiscardableMemorySizeFromDump(*mem2, "dummy_dump_2"), kPageSize * 3);
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
