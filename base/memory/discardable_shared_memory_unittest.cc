...
TEST(DiscardableSharedMemoryTest, TracingOwnershipEdges)
{
    ...
    trace_event::MemoryAllocatorDump* client_dump = pmd.CreateAllocatorDump("discardable_manager/map1");
    const bool is_owned = false;
    memory1.CreateSharedMemoryOwnershipEdge(client_dump, &pmd, is_owned);
    const auto* shm_dump = pmd.GetAllocatorDump(SharedMemoryTracker::GetDumpNameForTracing(memory1.mapped_id()));
