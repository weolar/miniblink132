...
TEST_F(SharedMemoryMappingTest, TooBigSpanWithExplicitElementCount)
{
    ...
    // overflow.
    EXPECT_TRUE(write_mapping_.GetMemoryAsSpan<uint32_t>(std::numeric_limits<size_t>::max()).empty());

    EXPECT_TRUE(read_mapping_.GetMemoryAsSpan<uint32_t>(std::numeric_limits<size_t>::max()).empty());
}
