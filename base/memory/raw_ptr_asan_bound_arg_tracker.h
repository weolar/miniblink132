...
namespace base {
namespace internal {
template <typename, typename, typename> struct Invoker;

template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits> class UnretainedWrapper;

template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits> class UnretainedRefWrapper;
} // namespace internal
...
private:
    template <typename, typename, typename> friend struct internal::Invoker;
    ...
    template <typename T> void AddArg(const T& arg)
    {
    }
    ...
    template <typename T, typename UnretainedTrait, RawPtrTraits PtrTraits> void AddArg(const internal::UnretainedWrapper<T, UnretainedTrait, PtrTraits>& arg)
    {
    ...
    void AddArg(const internal::UnretainedRefWrapper<T, UnretainedTrait, PtrTraits>& arg)
    {
        if constexpr (raw_ptr_traits::IsSupportedType<T>::value) {
    ...
    template <typename... Args> void AddArgs(Args&&... args)
    {
        if (enabled_) {
