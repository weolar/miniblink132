...
protected:
    SharedMemoryMapping(span<uint8_t> mapped_span, size_t size, const UnguessableToken& guid, SharedMemoryMapper* mapper);
...
public:
    ...
    ReadOnlySharedMemoryMapping& operator=(ReadOnlySharedMemoryMapping&&) noexcept;
    ...
    span<const T> GetMemoryAsSpan(size_t count) const
    {
        ...
        return UNSAFE_BUFFERS(span(reinterpret_cast<const T*>(mapped_memory().data()), count));
    }

private:
    friend class ReadOnlySharedMemoryRegion;
    ReadOnlySharedMemoryMapping(span<uint8_t> mapped_span, size_t size, const UnguessableToken& guid, SharedMemoryMapper* mapper);
};
...
public:
    ...
    WritableSharedMemoryMapping& operator=(WritableSharedMemoryMapping&&) noexcept;
    ...
    {
        // SAFETY: As in the ReadOnly code above.
        return UNSAFE_BUFFERS(const_iterator(data(), data() + size(), data() + size()));
    }
    ...
    span<T> GetMemoryAsSpan(size_t count) const
    {
        ...
        // of type T that fit within size(), so the pointer given to span() points
        // to at least that many T objects.
        return UNSAFE_BUFFERS(span(reinterpret_cast<T*>(mapped_memory().data()), count));
    }

private:
    friend WritableSharedMemoryMapping MapAtForTesting(subtle::PlatformSharedMemoryRegion* region, uint64_t offset, size_t size);
    friend class ReadOnlySharedMemoryRegion;
    friend class WritableSharedMemoryRegion;
    friend class UnsafeSharedMemoryRegion;
    WritableSharedMemoryMapping(span<uint8_t> mapped_span, size_t size, const UnguessableToken& guid, SharedMemoryMapper* mapper);
