...
class BASE_EXPORT RefCountedMemory : public RefCountedThreadSafe<RefCountedMemory> {
public:
    ...
    // v[2] = uint8_t{4};
    // ```
    const uint8_t* data() const LIFETIME_BOUND
    {
        return AsSpan().data();
    }
    size_t size() const
    ...
    const uint8_t* front() const LIFETIME_BOUND
    {
        return AsSpan().data();
    }
...
public:
    ...
    const std::string& as_string() const LIFETIME_BOUND
    {
        return string_;
    }
    std::string& as_string() LIFETIME_BOUND
    {
        return string_;
    }
...
public:
    ...
    const std::u16string& as_string() const LIFETIME_BOUND
    {
        return string_;
    }
    std::u16string& as_string() LIFETIME_BOUND
    {
        return string_;
    }
...
public:
    ...
    RefCountedSharedMemoryMapping& operator=(const RefCountedSharedMemoryMapping&) = delete;
    ...
    static scoped_refptr<RefCountedSharedMemoryMapping> CreateFromWholeRegion(const ReadOnlySharedMemoryRegion& region);
