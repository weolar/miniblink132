...
constexpr MemorySafetyCheck operator|(MemorySafetyCheck a, MemorySafetyCheck b)
{
    return static_cast<MemorySafetyCheck>(static_cast<uint32_t>(a) | static_cast<uint32_t>(b));
}
...
constexpr auto kAdvancedMemorySafetyChecks
    = MemorySafetyCheck::kForcePartitionAlloc | MemorySafetyCheck::kSchedulerLoopQuarantine | MemorySafetyCheck::kZapOnFree;
...
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    return static_cast<bool>(checks & (MemorySafetyCheck::kForcePartitionAlloc | MemorySafetyCheck::kSchedulerLoopQuarantine | MemorySafetyCheck::kZapOnFree));
#else
    return false;
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
}
...
{
    return partition_alloc::AllocFlags::kReturnNull | partition_alloc::AllocFlags::kNoHooks;
}
...
template <typename T, MemorySafetyCheck c> inline constexpr bool is_memory_safety_checked = (get_memory_safety_checks<T> & c) == c;

// Allocator functions.
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
ALWAYS_INLINE partition_alloc::PartitionRoot* GetPartitionRootForMemorySafetyCheckedAllocation()
{
    return allocator_shim::internal::PartitionAllocMalloc::Allocator();
}
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)

template <MemorySafetyCheck checks> NOINLINE void* HandleMemorySafetyCheckedOperatorNew(std::size_t count)
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    if constexpr (ShouldUsePartitionAlloc(checks)) {
        return GetPartitionRootForMemorySafetyCheckedAllocation()->AllocInline<GetAllocFlags(checks)>(count);
    }
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    return ::operator new(count);
}

template <MemorySafetyCheck checks> NOINLINE void* HandleMemorySafetyCheckedOperatorNew(std::size_t count, std::align_val_t alignment)
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    if constexpr (ShouldUsePartitionAlloc(checks)) {
        return GetPartitionRootForMemorySafetyCheckedAllocation()->AlignedAlloc<GetAllocFlags(checks)>(static_cast<size_t>(alignment), count);
    }
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    return ::operator new(count, alignment);
}

template <MemorySafetyCheck checks> NOINLINE void HandleMemorySafetyCheckedOperatorDelete(void* ptr)
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    if constexpr (ShouldUsePartitionAlloc(checks)) {
        GetPartitionRootForMemorySafetyCheckedAllocation()->Free<GetFreeFlags(checks)>(ptr);
        return;
    }
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    ::operator delete(ptr);
...
template <MemorySafetyCheck checks> NOINLINE void HandleMemorySafetyCheckedOperatorDelete(void* ptr, std::align_val_t alignment)
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    if constexpr (ShouldUsePartitionAlloc(checks)) {
        GetPartitionRootForMemorySafetyCheckedAllocation()->Free<GetFreeFlags(checks)>(ptr);
...
//     int public_field;
//   };
#define MEMORY_SAFETY_CHECKS_INTERNAL(SPECIFIER, DEFAULT_CHECKS, ENABLED_CHECKS, DISABLED_CHECKS, ...)                                                         \
public:                                                                                                                                                        \
    ...
    {                                                                                                                                                          \
        return base::internal::HandleMemorySafetyCheckedOperatorNew<kMemorySafetyChecks>(count);                                                               \
    }                                                                                                                                                          \
    SPECIFIER static void* operator new(std::size_t count, std::align_val_t alignment)                                                                         \
    {                                                                                                                                                          \
        return base::internal::HandleMemorySafetyCheckedOperatorNew<kMemorySafetyChecks>(count, alignment);                                                    \
    }                                                                                                                                                          \
    ...
    {                                                                                                                                                          \
        base::internal::HandleMemorySafetyCheckedOperatorDelete<kMemorySafetyChecks>(ptr);                                                                     \
    }                                                                                                                                                          \
    SPECIFIER static void operator delete(void* ptr, std::align_val_t alignment) noexcept                                                                      \
    {                                                                                                                                                          \
        base::internal::HandleMemorySafetyCheckedOperatorDelete<kMemorySafetyChecks>(ptr, alignment);                                                          \
    }                                                                                                                                                          \
                                                                                                                                                               \
private:                                                                                                                                                       \
...
#define ADVANCED_MEMORY_SAFETY_CHECKS(...)                                                                                                                     \
    MEMORY_SAFETY_CHECKS_INTERNAL(NOINLINE NOT_TAIL_CALLED, base::internal::kAdvancedMemorySafetyChecks __VA_OPT__(, ) __VA_ARGS__, kNone, kNone)
#else
#define ADVANCED_MEMORY_SAFETY_CHECKS(...)                                                                                                                     \
    MEMORY_SAFETY_CHECKS_INTERNAL(ALWAYS_INLINE, base::internal::kAdvancedMemorySafetyChecks __VA_OPT__(, ) __VA_ARGS__, kNone, kNone)
#endif // DCHECK_IS_ON()
...
#define INHERIT_MEMORY_SAFETY_CHECKS(BASE_CLASS, ...)                                                                                                          \
    MEMORY_SAFETY_CHECKS_INTERNAL(ALWAYS_INLINE, BASE_CLASS::kMemorySafetyChecks __VA_OPT__(, ) __VA_ARGS__, kNone, kNone)

#define DEFAULT_MEMORY_SAFETY_CHECKS(...) MEMORY_SAFETY_CHECKS_INTERNAL(ALWAYS_INLINE, kNone __VA_OPT__(, ) __VA_ARGS__, kNone, kNone)
