...
MULTIPROCESS_TEST_MAIN(InitFromSwitchValue)
{
    ...
    GlobalDescriptors::GetInstance()->Set(kArbitraryDescriptorKey, kArbitraryDescriptorKey + GlobalDescriptors::kBaseDescriptor);
#endif
    const std::string shared_memory_data = command_line.GetSwitchValueASCII(kSharedMemoryData);
    const bool is_read_only = command_line.GetSwitchValueASCII(kIsReadOnly) == "true";
    const std::string guid_string = command_line.GetSwitchValueASCII(kSharedMemoryGUID);
...
INSTANTIATE_TEST_SUITE_P(All, SharedMemorySwitchTest, ::testing::Combine(testing::Bool(), testing::Bool()));
...
TEST_P(SharedMemorySwitchTest, PassViaSwitch)
{
    ...
    SCOPED_TRACE(base::StringPrintf("read_only=%d; elevated=%d", read_only, elevated));
    ...
    command_line.AppendSwitchASCII(kSharedMemoryGUID, (read_only ? read_only_region.region.GetGUID() : unsafe_region.GetGUID()).ToString());
    ...
    if (read_only) {
        AddToLaunchParameters(kSharedMemoryData, read_only_region.region.Duplicate(),
#if BUILDFLAG(IS_APPLE)
    ...
    launch_options.fds_to_remap.emplace_back(descriptor_to_share.get(), kArbitraryDescriptorKey);
    ...
    Process process = SpawnMultiProcessTestChild("InitFromSwitchValue", command_line, launch_options);
