...
{
    CHECK(end > start);
    const uintptr_t page_start = bits::AlignDown(reinterpret_cast<uintptr_t>(start), GetPageSize());
    return mprotect(reinterpret_cast<void*>(page_start), reinterpret_cast<uintptr_t>(end) - page_start, prot) == 0;
}
...
{
    const uintptr_t page_start = bits::AlignDown(reinterpret_cast<uintptr_t>(ptr), GetPageSize());
    ...
    int result = getrlimit(RLIMIT_NPROC, reinterpret_cast<struct rlimit*>(page_start));
...
void CheckMemoryReadOnly(const void* ptr)
{
    ...
    kern_return_t kr = mach_vm_region(mach_task_self(), reinterpret_cast<mach_vm_address_t*>(&ptr), &size, VM_REGION_BASIC_INFO_64,
        reinterpret_cast<vm_region_info_t>(&region_info), &count, &object_name);
