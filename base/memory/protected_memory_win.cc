...
bool SetMemory(void* start, void* end, DWORD prot)
{
    ...
    DWORD old_prot;
    return VirtualProtect(reinterpret_cast<void*>(page_start), reinterpret_cast<uintptr_t>(end) - page_start, prot, &old_prot) != 0;
}
...
void CheckMemoryReadOnly(const void* ptr)
{
    ...
    SIZE_T result = VirtualQuery(reinterpret_cast<LPCVOID>(page_start), &info, sizeof(info));
