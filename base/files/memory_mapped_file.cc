...
bool MemoryMappedFile::Initialize(const FilePath& file_name, Access access)
{
    ...
    case READ_CODE_IMAGE:
        flags |= File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_EXCLUSIVE_WRITE | File::FLAG_WIN_EXECUTE;
        break;
#endif
    }
...
bool MemoryMappedFile::Initialize(File file, const Region& region, Access access)
{
    switch (access) {
    case READ_WRITE_EXTEND:
...
// static
void MemoryMappedFile::CalculateVMAlignedBoundaries(int64_t start, size_t size, int64_t* aligned_start, size_t* aligned_size, int32_t* offset)
{
    ...
    *aligned_size = (size + static_cast<size_t>(*offset) + static_cast<size_t>(mask)) & ~mask;
}
