...
size_t FilePathWatcherKQueue::EventsForPath(FilePath path, EventVector* events)
{
    ...
    for (std::vector<FilePath::StringType>::iterator i = components.begin(); i != components.end(); ++i) {
        ...
        struct kevent event;
        EV_SET(&event, fd, EVFILT_VNODE, (EV_ADD | EV_CLEAR | EV_RECEIPT), (NOTE_DELETE | NOTE_WRITE | NOTE_ATTRIB | NOTE_RENAME | NOTE_REVOKE | NOTE_EXTEND),
            0, data);
...
// static
size_t FilePathWatcherKQueue::EventForItem(const FilePath& path, EventVector* events)
{
    // Make sure that we are working with a clean slate.
    DCHECK(events->empty());
    ...
    EV_SET(&event, FileDescriptorForPath(path), EVFILT_VNODE, (EV_ADD | EV_CLEAR | EV_RECEIPT),
        (NOTE_DELETE | NOTE_WRITE | NOTE_ATTRIB | NOTE_RENAME | NOTE_REVOKE | NOTE_EXTEND), 0, new EventData(path, /*subdir=*/FilePath::StringType()));
...
bool FilePathWatcherKQueue::AreKeventValuesValid(struct kevent* kevents, int count)
{
    if (count < 0) {
        DPLOG(ERROR) << "kevent";
        return false;
    ...
    for (int i = 0; i < count; ++i) {
        if (kevents[i].flags & EV_ERROR && kevents[i].data) {
            ...
            if (path_name.empty()) {
                path_name = base::StringPrintf("fd %ld", reinterpret_cast<long>(&kevents[i].ident));
            }
...
void FilePathWatcherKQueue::HandleAttributesChange(const EventVector::iterator& event, bool* target_file_affected, bool* update_watches)
{
...
void FilePathWatcherKQueue::HandleDeleteOrMoveChange(const EventVector::iterator& event, bool* target_file_affected, bool* update_watches)
{
    *target_file_affected = true;
    *update_watches = true;
    EventVector::iterator local_event(event);
...
void FilePathWatcherKQueue::HandleCreateItemChange(const EventVector::iterator& event, bool* target_file_affected, bool* update_watches)
{
...
bool FilePathWatcherKQueue::UpdateWatches(bool* target_file_affected)
{
    ...
    while (update_watches) {
        ...
        int count = HANDLE_EINTR(kevent(kqueue_, &events_[0], valid_int, &updates[0], valid_int, NULL));
...
bool FilePathWatcherKQueue::Watch(const FilePath& path, Type type, const FilePathWatcher::Callback& callback)
{
    ...
    size_t last_entry = type == Type::kNonRecursive ? EventsForPath(target_, &events_) : EventForItem(target_, &events_);
    if (!last_entry) {
    ...
    int count = HANDLE_EINTR(kevent(kqueue_, &events_[0], last_entry_int, &responses[0], last_entry_int, NULL));
    ...
    kqueue_watch_controller_ = FileDescriptorWatcher::WatchReadable(kqueue_, BindRepeating(&FilePathWatcherKQueue::OnKQueueReadable, Unretained(this)));
...
void FilePathWatcherKQueue::OnKQueueReadable()
{
    ...
    int count = HANDLE_EINTR(kevent(kqueue_, NULL, 0, &updates[0], checked_cast<int>(updates.size()), &timeout));
