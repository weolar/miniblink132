...
// drive.
BASE_EXPORT std::optional<DriveInfo> GetFileDriveInfo(const FilePath& file_path);

#if BUILDFLAG(IS_MAC)
// BSD name is the file found under `/dev`, not the full path including "/dev".
BASE_EXPORT std::optional<DriveInfo> GetBSDNameDriveInfo(const std::string_view bsd_name);
...
// `kIOMediaClass`.
BASE_EXPORT std::optional<DriveInfo> GetIOObjectDriveInfo(io_object_t io_object);
#endif
