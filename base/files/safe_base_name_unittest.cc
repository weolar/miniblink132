...
TEST(SafeBaseNameTest, Basic)
{
    ...
    std::optional<SafeBaseName> basename2(SafeBaseName::Create(FILE_PATH_LITERAL("bar")));
...
{
    std::optional<SafeBaseName> basename(SafeBaseName::Create(FILE_PATH_LITERAL("C:\\foo\\bar")));
    EXPECT_TRUE(basename.has_value());
...
{
    std::optional<SafeBaseName> basename(SafeBaseName::Create(FilePath(FILE_PATH_LITERAL("/"))));
    EXPECT_FALSE(basename.has_value());
}
...
{
    std::optional<SafeBaseName> basename(SafeBaseName::Create(FILE_PATH_LITERAL("/foo/bar")));
    EXPECT_TRUE(basename.has_value());
    EXPECT_EQ(basename->path(), FilePath(FILE_PATH_LITERAL("bar")));
