...
FilePath MakeShortFilePath(const FilePath& input)
{
    ...
    path_short_len = ::GetShortPathName(input.value().c_str(), WriteInto(&path_short_str, path_short_len), path_short_len);
    if (path_short_len == 0UL) {
        return FilePath();
    }
...
void ChangePosixFilePermissions(const FilePath& path, int mode_bits_to_set, int mode_bits_to_clear)
{
    ASSERT_FALSE(mode_bits_to_set & mode_bits_to_clear) << "Can't set and clear the same bits.";
...
void SetReadOnly(const FilePath& path, bool read_only)
{
    ...
    ASSERT_TRUE(SetFileAttributes(path.value().c_str(), read_only ? (attrs | FILE_ATTRIBUTE_READONLY) : (attrs & ~FILE_ATTRIBUTE_READONLY)));

    DWORD expected = read_only ? ((attrs & (FILE_ATTRIBUTE_ARCHIVE | FILE_ATTRIBUTE_DIRECTORY)) | FILE_ATTRIBUTE_READONLY)
    ...
    attrs = GetFileAttributes(path.value().c_str()) & ~(FILE_ATTRIBUTE_NOT_CONTENT_INDEXED | FILE_ATTRIBUTE_COMPRESSED);
    ASSERT_EQ(expected, attrs);
#else
    // On all other platforms, it involves removing/setting the write bit.
    mode_t mode = read_only ? S_IRUSR : (S_IRUSR | S_IWUSR);
    EXPECT_TRUE(SetPosixFilePermissions(path, DirectoryExists(path) ? (mode | S_IXUSR) : mode));
#endif // BUILDFLAG(IS_WIN)
}
    ...
    bool AreShortFilePathsEnabled()
    {
        static const bool enabled = [this] {
            ...
            AreShortNamesEnabledFunction short_names_func
                = reinterpret_cast<AreShortNamesEnabledFunction>(GetProcAddress(::GetModuleHandleW(L"kernel32.dll"), "AreShortNamesEnabled"));
            ...
            base::File temp_dir(temp_dir_.GetPath(), base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_WIN_BACKUP_SEMANTICS | base::File::FLAG_READ);
            BOOL enabled = false;
...
TEST_F(FileUtilTest, MakeAbsoluteFilePathNoResolveSymbolicLinks)
{
    ...
    for (auto& expected_result : kExpectedResults) {
        EXPECT_EQ(MakeAbsoluteFilePathNoResolveSymbolicLinks(expected_result.first), expected_result.second);
    }
    ...
    ASSERT_FALSE(MakeAbsoluteFilePathNoResolveSymbolicLinks(FilePath("relative_file_path")).has_value());
}
...
TEST_F(FileUtilTest, NormalizeFilePathBasic)
{
    ...
    ASSERT_FALSE(NormalizeFilePath(file_a_path, &normalized_file_a_path)) << "NormalizeFilePath() should fail on nonexistent paths.";
    ...
    ASSERT_TRUE(normalized_file_a_path.DirName().IsParent(normalized_file_b_path.DirName()));
}
...
TEST_F(FileUtilTest, NormalizeFileEmptyFile)
{
    ...
    FilePath normalized_file_a_path, normalized_file_b_path;
    ASSERT_FALSE(PathExists(file_a_path));
    EXPECT_FALSE(NormalizeFilePath(file_a_path, &normalized_file_a_path)) << "NormalizeFilePath() should fail on nonexistent paths.";
    ...
    // or junction.  So, the real path of the directory holding file a
    // must be the parent of the path holding file b.
    EXPECT_TRUE(normalized_file_a_path.DirName().IsParent(normalized_file_b_path.DirName()));
}
...
TEST_F(FileUtilTest, NormalizeFilePathReparsePoints)
{
    ...
    FilePath normalized_path;
    {
        ...
        auto reparse_to_base_b = test::FilePathReparsePoint::Create(to_base_b, base_b);
        ASSERT_TRUE(reparse_to_base_b.has_value());

        FilePath to_sub_long = base_b.Append(FPL("to_sub_long"));
        ASSERT_TRUE(CreateDirectory(to_sub_long));
        auto reparse_to_sub_long = test::FilePathReparsePoint::Create(to_sub_long, sub_long);
        ...
        ASSERT_TRUE(NormalizeFilePath(to_sub_a.Append(FPL("file.txt")), &normalized_path));
        ASSERT_EQ(file_txt.value(), normalized_path.value());
        ...
        ASSERT_TRUE(
            NormalizeFilePath(base_b.Append(FPL("to_base_b")).Append(FPL("to_base_b")).Append(FPL("to_sub_a")).Append(FPL("file.txt")), &normalized_path));
        ASSERT_EQ(file_txt.value(), normalized_path.value());
        ...
        ASSERT_TRUE(NormalizeFilePath(to_sub_long.Append(deep_txt), &normalized_path));
    ...
    ASSERT_FALSE(NormalizeFilePath(to_sub_a.Append(FPL("file.txt")), &normalized_path));
}
...
{
    // Get a drive letter.
    std::wstring real_drive_letter = AsWString(ToUpperASCII(AsStringPiece16(temp_dir_.GetPath().value().substr(0, 2))));
    ...
    ASSERT_TRUE(::QueryDosDevice(real_drive_letter.c_str(), device_path, MAX_PATH));
    ...
    ASSERT_TRUE(DevicePathToDriveLetterPath(actual_device_path.Append(kRelativePath), &win32_path));
    EXPECT_EQ(FilePath(real_drive_letter + FILE_PATH_LITERAL("\\")).Append(kRelativePath).value(), win32_path.value());
    ...
    FilePath prefix_of_real_device_path(actual_device_path.value().substr(0, new_length));
    ASSERT_FALSE(DevicePathToDriveLetterPath(prefix_of_real_device_path, &win32_path));

    ASSERT_FALSE(DevicePathToDriveLetterPath(prefix_of_real_device_path.Append(kRelativePath), &win32_path));
    ...
    FilePath real_device_path_plus_numbers(actual_device_path.value() + kExtraChars);

    ASSERT_FALSE(DevicePathToDriveLetterPath(real_device_path_plus_numbers, &win32_path));

    ASSERT_FALSE(DevicePathToDriveLetterPath(real_device_path_plus_numbers.Append(kRelativePath), &win32_path));
}
...
TEST_F(FileUtilTest, CreateTemporaryFileInDirLongPathTest)
{
    ...
    ASSERT_TRUE(CreateTemporaryFileInDir(short_test_dir.Append(kTestSubDirName), &temp_file));
    EXPECT_TRUE(PathExists(temp_file));
...
TEST_F(FileUtilTest, PreventExecuteMappingNewFile)
{
    ...
    {
        File new_file(file, File::FLAG_WRITE | File::FLAG_WIN_NO_EXECUTE | File::FLAG_CREATE_ALWAYS);
        ASSERT_TRUE(new_file.IsValid());
    }

    {
        File open_file(file, File::FLAG_READ | File::FLAG_WIN_EXECUTE | File::FLAG_OPEN_ALWAYS);
...
TEST_F(FileUtilTest, PreventExecuteMappingOpenFile)
{
    ...
    CreateTextFile(file, bogus_content);
    ASSERT_TRUE(PathExists(file));
    File open_file(file, File::FLAG_READ | File::FLAG_WRITE | File::FLAG_WIN_EXECUTE | File::FLAG_OPEN_ALWAYS);
    EXPECT_TRUE(open_file.IsValid());
    // Verify ACE can be set even on an open file.
    EXPECT_TRUE(PreventExecuteMapping(file));
    {
        File second_open_file(file, File::FLAG_READ | File::FLAG_WRITE | File::FLAG_OPEN_ALWAYS);
        EXPECT_TRUE(second_open_file.IsValid());
    }
    {
        File third_open_file(file, File::FLAG_READ | File::FLAG_WIN_EXECUTE | File::FLAG_OPEN_ALWAYS);
...
TEST(FileUtilDeathTest, DisallowNoExecuteOnUnsafeFile)
{
    ...
    ASSERT_TRUE(base::PathService::Get(base::DIR_LOCAL_APP_DATA, &local_app_data));

    base::FilePath file_path;
    EXPECT_DCHECK_DEATH_WITH(
        {
            {
                base::File temp_file = base::CreateAndOpenTemporaryFileInDir(local_app_data, &file_path);
            }
            File reopen_file(file_path, File::FLAG_READ | File::FLAG_WRITE | File::FLAG_WIN_NO_EXECUTE | File::FLAG_OPEN_ALWAYS | File::FLAG_DELETE_ON_CLOSE);
        },
...
MULTIPROCESS_TEST_MAIN(NoExecuteOnSafeFileMain)
{
    ...
    // permitted.
    File reopen_file(temp_file, File::FLAG_READ | File::FLAG_WRITE | File::FLAG_WIN_NO_EXECUTE | File::FLAG_OPEN_ALWAYS | File::FLAG_DELETE_ON_CLOSE);
    return 0;
}
...
{
    FilePath new_dir;
    ASSERT_TRUE(CreateTemporaryDirInDir(temp_dir_.GetPath(), FILE_PATH_LITERAL("NoExecuteOnSafeFileLongPath"), &new_dir));
    ...
    Process child_process = SpawnMultiProcessTestChild("NoExecuteOnSafeFileMain", child_command_line, options);
    ASSERT_TRUE(child_process.IsValid());
    int rv = -1;
    ASSERT_TRUE(WaitForMultiprocessTestChildExit(child_process, TestTimeouts::action_timeout(), &rv));
    ASSERT_EQ(0, rv);
}
...
TEST_F(FileUtilTest, CreateAndReadSymlinks)
{
    ...
    ASSERT_TRUE(CreateSymbolicLink(link_to, link_from)) << "Failed to create file symlink.";
    ...
    ASSERT_TRUE(CreateSymbolicLink(link_to, link_from)) << "Failed to create directory symlink.";
...
TEST_F(FileUtilTest, CreateAndReadRelativeSymlinks)
{
    ...
    FilePath link_from_in_subdir = temp_dir_.GetPath().Append(FPL("subdir")).Append(FPL("from_file"));
    ...
    ASSERT_TRUE(CreateSymbolicLink(filename_link_to, link_from)) << "Failed to create file symlink.";
    ...
    ASSERT_TRUE(CreateDirectory(link_to));
    ASSERT_TRUE(CreateSymbolicLink(filename_link_to, link_from)) << "Failed to create relative directory symlink.";
...
TEST_F(FileUtilTest, NormalizeFilePathSymlinks)
{
    ...
    EXPECT_TRUE(NormalizeFilePath(link_from, &normalized_path)) << "Links to directories should return true.";
    ...
    ASSERT_TRUE(CreateSymbolicLink(link_to, link_from)) << "Failed to create loop symlink a.";
    ASSERT_TRUE(CreateSymbolicLink(link_from, link_to)) << "Failed to create loop symlink b.";
...
TEST_F(FileUtilTest, DeleteSymlinkToExistentFile)
{
    ...
    ASSERT_TRUE(CreateSymbolicLink(file_name, file_link)) << "Failed to create symlink.";
...
TEST_F(FileUtilTest, DeleteSymlinkToNonExistentFile)
{
    ...
    FilePath non_existent = temp_dir_.GetPath().Append(FPL("Test DeleteFile 3.txt"));
    ...
    ASSERT_TRUE(CreateSymbolicLink(non_existent, file_link)) << "Failed to create symlink.";
...
{
    // Create a file path.
    FilePath file_name = temp_dir_.GetPath().Append(FPL("Test Readable File.txt"));
...
TEST_F(FileUtilTest, ChangeFilePermissionsAndWrite)
{
    ...
    EXPECT_TRUE(SetPosixFilePermissions(file_name, FILE_PERMISSION_WRITE_BY_USER));
    EXPECT_TRUE(GetPosixFilePermissions(file_name, &mode));
    EXPECT_TRUE(mode & FILE_PERMISSION_WRITE_BY_USER);
...
TEST_F(FileUtilTest, ExecutableExistsInPath)
{
    ...
    ScopedEnvironmentVariableOverride scoped_env("PATH", dir1.value() + ":" + dir2.value());
    ...
    ASSERT_TRUE(SetPosixFilePermissions(dir1.Append(kExeFileName), FILE_PERMISSION_EXECUTE_BY_USER));
...
{
    // Create a directory.
    FilePath dir_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_From_Subdir"));
    ...
    FilePath file_name_from = dir_name_from.Append(FILE_PATH_LITERAL("Reggy-1.txt"));
    ...
    FilePath file2_name_from = dir_name_from.Append(FILE_PATH_LITERAL("Reggy-2.txt"));
    ...
    FilePath file3_name_from = dir_name_from.Append(FILE_PATH_LITERAL("Reggy-3.txt"));
    ...
    FilePath dir_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_To_Subdir"));
...
{
    FilePath non_existent = temp_dir_.GetPath().AppendASCII("bogus_file_dne.foobar");
...
TEST_F(FileUtilTest, DeleteDeep)
{
    ...
    {
        ScopedFD fd(HANDLE_EINTR(open(dir_path.value().c_str(), O_DIRECTORY | O_CLOEXEC)));
        ASSERT_TRUE(fd.is_valid()) << strerror(errno);

        for (char c = 'a'; c <= 'z'; ++c) {
            const std::string name(NAME_MAX, c);
            ASSERT_EQ(HANDLE_EINTR(mkdirat(fd.get(), name.c_str(), 0777)), 0) << strerror(errno);

            fd = ScopedFD(HANDLE_EINTR(openat(fd.get(), name.c_str(), O_DIRECTORY | O_CLOEXEC)));
            ASSERT_TRUE(fd.is_valid()) << strerror(errno);
        }

#if !BUILDFLAG(IS_FUCHSIA)
        // Create a symlink at the bottom of the deep tree.
        ASSERT_EQ(HANDLE_EINTR(symlinkat("..", fd.get(), "up")), 0) << strerror(errno);
#endif // !BUILDFLAG(IS_FUCHSIA)

        // Create a file at the bottom of the deep tree.
        fd = ScopedFD(HANDLE_EINTR(openat(fd.get(), "file.txt", O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC, 0666)));
        ASSERT_TRUE(fd.is_valid()) << strerror(errno);

        const std::string_view s = "This is a deep file";
        ASSERT_EQ(HANDLE_EINTR(write(fd.get(), s.data(), s.size())), static_cast<ssize_t>(s.size()));
    }
...
TEST_F(FileUtilTest, ContentUriGetInfo)
{
    ...
    FilePath content_uri_file = *test::android::GetContentUriFromCacheDirFilePath(file);
    FilePath content_uri_dir = *test::android::GetContentUriFromCacheDirFilePath(dir);
    FilePath content_uri_file_in_memory = *test::android::GetInMemoryContentUriFromCacheDirFilePath(file);
    FilePath content_uri_dir_in_memory = *test::android::GetInMemoryContentUriFromCacheDirFilePath(dir);
    FilePath content_uri_dir_tree = *test::android::GetInMemoryContentTreeUriFromCacheDirDirectory(dir);
    ...
    EXPECT_TRUE(GetFileInfo(content_uri_file_in_memory, &content_uri_in_memory_info));
    ...
    EXPECT_FALSE(GetFileInfo(content_uri_dir_in_memory, &content_uri_in_memory_info));
    ...
    EXPECT_EQ(info.last_modified.ToTimeT(), content_uri_tree_info.last_modified.ToTimeT());
...
TEST_F(FileUtilTest, DeleteWildCard)
{
    ...
    FilePath file_name = temp_dir_.GetPath().Append(FPL("Test DeleteWildCard.txt"));
    CreateTextFile(file_name, bogus_content);
    ASSERT_TRUE(PathExists(file_name));
...
{
    // Create a file and a directory
    FilePath subdir_path = temp_dir_.GetPath().Append(FPL("DeleteNonExistantWildCard"));
    CreateDirectory(subdir_path);
    ASSERT_TRUE(PathExists(subdir_path));
...
TEST_F(FileUtilTest, DeleteDirNonRecursive)
{
    ...
    FilePath test_subdir = temp_dir_.GetPath().Append(FPL("DeleteDirNonRecursive"));
...
TEST_F(FileUtilTest, DeleteDirRecursiveWithOpenFile)
{
    ...
    File file1(file_name1, File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE);
    ...
    File file3(file_name3, File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE);
    ...
    int flags;
    bool file_attrs_supported = ioctl(file1.GetPlatformFile(), FS_IOC_GETFLAGS, &flags) == 0;
...
TEST_F(FileUtilTest, TestNonBlockingFileReadLinux)
{
    ...
    int res = mkfifo(fifo_path.MaybeAsASCII().c_str(), S_IWUSR | S_IRUSR | S_IWGRP | S_IWGRP);
    ASSERT_NE(res, -1);
...
{
    // Create a file
    FilePath file_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Move_Test_File.txt"));
    CreateTextFile(file_name_from, L"Gooooooooooooooooooooogle");
    ASSERT_TRUE(PathExists(file_name_from));

    // The destination.
    FilePath file_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Move_Test_File_Destination.txt"));
...
TEST_F(FileUtilTest, MoveFileDirExists)
{
    ...
    FilePath dir_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Destination"));
    CreateDirectory(dir_name_to);
    ASSERT_TRUE(PathExists(dir_name_to));
...
{
    // Create a directory
    FilePath dir_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Move_From_Subdir"));
    CreateDirectory(dir_name_from);
    ASSERT_TRUE(PathExists(dir_name_from));
    ...
    // Move the directory.
    FilePath dir_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Move_To_Subdir"));
...
TEST_F(FileUtilTest, MoveExist)
{
    ...
    // Move the directory
    FilePath dir_name_exists = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Destination"));
...
{
    // Create a file
    FilePath file_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_Test_File.txt"));
    CreateTextFile(file_name_from, L"Gooooooooooooooooooooogle");
    ASSERT_TRUE(PathExists(file_name_from));

    // The destination name
    FilePath file_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_Test_File_Destination.txt"));
    ASSERT_FALSE(PathExists(file_name_to));
...
#if BUILDFLAG(IS_WIN)
    FilePath from_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_From_Subdir\\\\\\"));
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
    FilePath from_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("Copy_From_Subdir///"));
#endif
...
TEST_F(FileUtilTest, CopyDirectoryWithNonRegularFiles)
{
    ...
    FilePath symlink_name_from = dir_name_from.Append(FILE_PATH_LITERAL("Symlink"));
...
TEST_F(FileUtilTest, CopyDirectoryExclFileOverSymlink)
{
    ...
    FilePath symlink_target = dir_name_to.Append(FILE_PATH_LITERAL("Symlink_Target.txt"));
    CreateTextFile(symlink_target, L"asdf");
    ASSERT_TRUE(PathExists(symlink_target));

    FilePath symlink_name_to = dir_name_to.Append(FILE_PATH_LITERAL("Copy_Test_File.txt"));
...
TEST_F(FileUtilTest, CopyDirectoryExclFileOverFifo)
{
    ...
    FilePath fifo_name_to = dir_name_to.Append(FILE_PATH_LITERAL("Copy_Test_File.txt"));
...
TEST_F(ReadOnlyFileUtilTest, ContentsEqual)
{
    ...
    EXPECT_FALSE(ContentsEqual(FilePath(FILE_PATH_LITERAL("bogusname")), FilePath(FILE_PATH_LITERAL("bogusname"))));
...
{
    // Create a directory
    FilePath dir_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("CopyAndDelete_From_Subdir"));
    CreateDirectory(dir_name_from);
    ASSERT_TRUE(PathExists(dir_name_from));
    ...
    FilePath dir_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("CopyAndDelete_To_Subdir"));
...
TEST_F(FileUtilTest, GetTempDirTest)
{
    ...
    std::array<const TCHAR*, 5> kTmpValues = { _T(""), _T("C:"), _T("C:\\"), _T("C:\\tmp"), _T("C:\\tmp\\") };
    ...
    for (const TCHAR* val : kTmpValues) {
        ...
        GetTempDir(&path);
        EXPECT_TRUE(path.IsAbsolute()) << "$TMP=" << val << " result=" << path.value();
    }
...
TEST_F(FileUtilTest, CreateAndOpenTemporaryFileInDir)
{
    ...
    File file2(path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_SHARE_DELETE);
#if BUILDFLAG(IS_WIN)
...
TEST_F(FileUtilTest, GetUniquePathTooManyFiles)
{
    ...
    for (int i = 1; i <= kMaxUniqueFiles; ++i) {
        FilePath path = temp_dir_.GetPath().AppendASCII(StringPrintf("SomeFile (%d).txt", i));
...
TEST_F(FileUtilTest, GetUniquePathWithSuffixFormat)
{
    ...
    EXPECT_EQ(temp_dir_.GetPath().Append(FPL("Unique_Base_Name (1).txt")), GetUniquePathWithSuffixFormat(base_path, " (%d)"));
}
...
{
    FilePath new_dir;
    ASSERT_TRUE(CreateTemporaryDirInDir(temp_dir_.GetPath(), FILE_PATH_LITERAL("CreateNewTemporaryDirInDirTest"), &new_dir));
...
TEST_F(FileUtilTest, AllocateFileRegionTest_ZeroOffset)
{
    ...
    FilePath file_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("allocate_file_region_test_zero_offset"));
    WriteFile(file_path, test_data);

    File file(file_path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE);
    ASSERT_TRUE(file.IsValid());
...
{
    std::string_view test_data = "test_data";
    FilePath file_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("allocate_file_region_test_non_zero_offset"));
    WriteFile(file_path, test_data);
...
{
    std::string_view test_data = "test_data";
    FilePath file_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("allocate_file_region_test_dont_truncate"));
    WriteFile(file_path, test_data);
...
{
    FilePath test_root = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("create_directory_test"));
#if BUILDFLAG(IS_WIN)
...
{
    // Check a directory
    FilePath test_root = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("detect_directory_test"));
    EXPECT_FALSE(PathExists(test_root));
...
TEST_F(FileUtilTest, FileEnumeratorTest)
{
    ...
    FileEnumerator f0_dotdot(temp_dir_.GetPath(), false, FILES_AND_DIRECTORIES | FileEnumerator::INCLUDE_DOT_DOT);
    EXPECT_EQ(temp_dir_.GetPath().Append(FPL("..")).value(), f0_dotdot.Next().value());
    ...
    FilePath file2_rel = dir2.Append(FilePath::kParentDirectory).Append(FPL("file2.txt"));
    ...
    FileEnumerator f2_non_recursive(temp_dir_.GetPath(), false, FileEnumerator::DIRECTORIES);
    ...
    FileEnumerator f2_dotdot(temp_dir_.GetPath(), false, FileEnumerator::DIRECTORIES | FileEnumerator::INCLUDE_DOT_DOT);
    ...
    FileEnumerator f5(temp_dir_.GetPath(), true, FILES_AND_DIRECTORIES, FPL("dir*"));
...
{
    FilePath data_dir = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("FilePathTest"));
...
TEST_F(FileUtilTest, ReadFile)
{
    ...
    FilePath file_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("ReadFileTest"));
    ...
    EXPECT_EQ(kTestData.substr(0, small_buffer.size()), std::string(small_buffer.begin(), small_buffer.end()));
    ...
    FilePath file_path_not_exist = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("ReadFileNotExistTest"));
...
TEST_F(FileUtilTest, ReadFileToBytes)
{
    ...
    FilePath file_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("ReadFileToStringTest"));
    FilePath file_path_dangerous
        = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("..")).Append(temp_dir_.GetPath().BaseName()).Append(FILE_PATH_LITERAL("ReadFileToStringTest"));
...
#if !BUILDFLAG(IS_WIN) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_IOS)
...
#define MoreThanBufferSizeChildMainString "WriteToPipeMoreThanBufferSizeChildMain"
...
{
    FilePath pipe_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("test_pipe"));
    ...
    {
        Process child_process = SpawnMultiProcessTestChild(ChildMainString, child_command_line, LaunchOptions());
        ASSERT_TRUE(child_process.IsValid());
    ...
    {
        Process child_process = SpawnMultiProcessTestChild(MoreThanBufferSizeChildMainString, child_command_line, LaunchOptions());
        ASSERT_TRUE(child_process.IsValid());
    ...
    }
    {
        ...
        std::string data = "temp";
        EXPECT_FALSE(ReadFileToStringWithMaxSize(pipe_path, &data, kLargeFileSize - 1));
    ...
    }
    {
        ...
        std::string data = "temp";
        EXPECT_TRUE(ReadFileToStringWithMaxSize(pipe_path, &data, kLargeFileSize * 5));
        EXPECT_EQ(std::string(kLargeFileSize, 'c'), data);
...
MULTIPROCESS_TEST_MAIN(ChildMain)
{
    ...
    HANDLE ph = CreateNamedPipe(pipe_path.value().c_str(), PIPE_ACCESS_OUTBOUND, PIPE_WAIT, 1, 0, 0, 0, NULL);
...
MULTIPROCESS_TEST_MAIN(MoreThanBufferSizeChildMain)
{
    ...
    HANDLE ph = CreateNamedPipe(pipe_path.value().c_str(), PIPE_ACCESS_OUTBOUND, PIPE_WAIT, 1, data.size(), data.size(), 0, NULL);
    EXPECT_NE(ph, INVALID_HANDLE_VALUE);
    EXPECT_TRUE(SetEvent(sync_event.get()));
    if (!::ConnectNamedPipe(ph, /*lpOverlapped=*/nullptr)) {
        // ERROR_PIPE_CONNECTED means that the other side has already connected.
...
TEST_F(FileUtilTest, ReadFileToStringWithNamedPipe)
{
    ...
    CommandLine child_command_line(GetMultiProcessTestChildBaseCommandLine());
    child_command_line.AppendSwitchPath("pipe-path", pipe_path);
    child_command_line.AppendSwitchASCII("sync_event", NumberToString(win::HandleToUint32(sync_event.get())));

    LaunchOptions options;
    options.handles_to_inherit.push_back(sync_event.get());

    {
        Process child_process = SpawnMultiProcessTestChild(ChildMainString, child_command_line, options);
        ASSERT_TRUE(child_process.IsValid());
    ...
    {
        Process child_process = SpawnMultiProcessTestChild(MoreThanBufferSizeChildMainString, child_command_line, options);
        ASSERT_TRUE(child_process.IsValid());
        // Wait for pipe creation in child process.
        EXPECT_EQ(WAIT_OBJECT_0, WaitForSingleObject(sync_event.get(), INFINITE));
...
TEST_F(FileUtilTest, ReadFileToStringWithLargeFile)
{
    ...
    // Read more than buffer size.
    actual_data = "temp";
    EXPECT_FALSE(ReadFileToStringWithMaxSize(file_path, &actual_data, kLargeFileSize - 1));
    EXPECT_EQ(std::string(kLargeFileSize - 1, 'c'), actual_data);
}

TEST_F(FileUtilTest, ReadStreamToString)
{
    ScopedFILE stream(OpenFile(temp_dir_.GetPath().Append(FPL("hello.txt")), "wb+"));
    ASSERT_TRUE(stream);
    File file = FILEToFile(stream.get());
    ASSERT_TRUE(file.IsValid());
    ASSERT_EQ(fprintf(stream.get(), "there"), 5);
...
TEST_F(FileUtilTest, ReadStreamToString_ZeroLengthFile)
{
    ...
    for (size_t size : kSizes) {
        ...
        write_thread.task_runner()->PostTask(FROM_HERE,
            BindLambdaForTesting([random_data, write_fd = std::move(write_fd)]() { ASSERT_TRUE(WriteFileDescriptor(write_fd.get(), random_data)); }));
...
TEST_F(FileUtilTest, TouchFile)
{
    ...
    ASSERT_TRUE(Time::FromString("Tue, 15 Nov 1994, 12:45:26 GMT", &modification_time));
    ...
    EXPECT_EQ(access_time.ToInternalValue(), file_info.last_accessed.ToInternalValue());
#endif
    EXPECT_EQ(modification_time.ToInternalValue(), file_info.last_modified.ToInternalValue());
}

TEST_F(FileUtilTest, WriteFileSpanVariant)
{
    FilePath empty_file = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("empty_file"));
    ...
    FilePath write_span_file = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("write_span_file"));
...
TEST_F(FileUtilTest, WriteFileStringVariant)
{
    ...
    FilePath write_span_file = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("write_string_file"));
    ASSERT_FALSE(PathExists(write_span_file));
...
{
    FilePath empty_dir = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("EmptyDir"));
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_NO_FATAL_FAILURE(ChangePosixFilePermissions(base_dir_, enabled_permissions, disabled_permissions));
        ASSERT_NO_FATAL_FAILURE(ChangePosixFilePermissions(sub_dir_, enabled_permissions, disabled_permissions));
    }
...
{
    // File does not exist.
    FilePath does_not_exist = base_dir_.AppendASCII("does").AppendASCII("not").AppendASCII("exist");
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, does_not_exist, uid_, ok_gids_));
...
TEST_F(VerifyPathControlledByUserTest, Symlinks)
{
    ...
    ASSERT_TRUE(CreateSymbolicLink(text_file_, file_link)) << "Failed to create symlink.";

    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, file_link, uid_, ok_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(file_link, file_link, uid_, ok_gids_));

    // Symlink from one directory to another within the path.
    FilePath link_to_sub_dir = base_dir_.AppendASCII("link_to_sub_dir");
    ASSERT_TRUE(CreateSymbolicLink(sub_dir_, link_to_sub_dir)) << "Failed to create symlink.";

    FilePath file_path_with_link = link_to_sub_dir.AppendASCII("file.txt");
    ASSERT_TRUE(PathExists(file_path_with_link));

    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, file_path_with_link, uid_, ok_gids_));

    EXPECT_FALSE(VerifyPathControlledByUser(link_to_sub_dir, file_path_with_link, uid_, ok_gids_));

    // Symlinks in parents of base path are allowed.
    EXPECT_TRUE(VerifyPathControlledByUser(file_path_with_link, file_path_with_link, uid_, ok_gids_));
}
...
TEST_F(VerifyPathControlledByUserTest, OwnershipChecks)
{
    ...
    EXPECT_TRUE(VerifyPathControlledByUser(base_dir_, sub_dir_, uid_, ok_gids_));
    EXPECT_TRUE(VerifyPathControlledByUser(base_dir_, text_file_, uid_, ok_gids_));
    EXPECT_TRUE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, ok_gids_));

    // Another user does not control these paths.
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, sub_dir_, bad_uid, ok_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, text_file_, bad_uid, ok_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(sub_dir_, text_file_, bad_uid, ok_gids_));

    // Another group does not control the paths.
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, sub_dir_, uid_, bad_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, text_file_, uid_, bad_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, bad_gids_));
}

TEST_F(VerifyPathControlledByUserTest, GroupWriteTest)
{
    // Make all files and directories writable only by their owner.
    ASSERT_NO_FATAL_FAILURE(ChangePosixFilePermissions(base_dir_, 0u, S_IWOTH | S_IWGRP));
    ASSERT_NO_FATAL_FAILURE(ChangePosixFilePermissions(sub_dir_, 0u, S_IWOTH | S_IWGRP));
    ASSERT_NO_FATAL_FAILURE(ChangePosixFilePermissions(text_file_, 0u, S_IWOTH | S_IWGRP));
    ...
    EXPECT_TRUE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, bad_gids_));
    ...
    std::set_union(ok_gids_.begin(), ok_gids_.end(), bad_gids_.begin(), bad_gids_.end(), std::inserter(multiple_gids, multiple_gids.begin()));

    EXPECT_TRUE(VerifyPathControlledByUser(base_dir_, sub_dir_, uid_, multiple_gids));
    EXPECT_TRUE(VerifyPathControlledByUser(base_dir_, text_file_, uid_, multiple_gids));
    EXPECT_TRUE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, multiple_gids));
}
...
TEST_F(VerifyPathControlledByUserTest, WriteBitChecks)
{
    ...
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, text_file_, uid_, ok_gids_));
    EXPECT_TRUE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, ok_gids_));
    ...
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, sub_dir_, uid_, ok_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(base_dir_, text_file_, uid_, ok_gids_));
    EXPECT_FALSE(VerifyPathControlledByUser(sub_dir_, text_file_, uid_, ok_gids_));
...
TEST_F(FileUtilTest, ValidContentUriTest)
{
    ...
    EXPECT_TRUE(UNSAFE_BUFFERS(file.ReadAtCurrentPos(buffer.get(), image_size.value())));
}
...
TEST_F(FileUtilTest, WriteContentUri)
{
    ...
    FilePath content_uri = *test::android::GetContentUriFromCacheDirFilePath(path);
...
#if BUILDFLAG(IS_WIN) && BUILDFLAG(GOOGLE_CHROME_BRANDING) && defined(ARCH_CPU_32_BITS)
...
#define MAYBE_PreReadFileExistingFileNoSize DISABLED_PreReadFileExistingFileNoSize
#else
...
TEST_F(FileUtilTest, MAYBE_PreReadFileExistingFileNoSize)
{
    ...
    EXPECT_TRUE(PreReadFile(text_file, /*is_executable=*/false, /*sequential=*/false));
}

#if defined(FLAKY_327582285)
#define MAYBE_PreReadFileExistingFileExactSize DISABLED_PreReadFileExistingFileExactSize
#else
...
#if defined(FLAKY_327582285)
#define MAYBE_PreReadFileExistingFileOverSized DISABLED_PreReadFileExistingFileOverSized
#else
...
#if defined(FLAKY_327582285)
#define MAYBE_PreReadFileExistingFileUnderSized DISABLED_PreReadFileExistingFileUnderSized
#else
#define MAYBE_PreReadFileExistingFileUnderSized PreReadFileExistingFileUnderSized
#endif
...
TEST_F(FileUtilTest, MAYBE_PreReadFileExecutable)
{
    ...
    EXPECT_TRUE(PreReadFile(test_exe, /*is_executable=*/true, /*sequential=*/false));
}

#if defined(FLAKY_327582285)
#define MAYBE_PreReadFileWithSequentialAccess DISABLED_PreReadFileWithSequentialAccess
#else
...
TEST_F(FileUtilTest, MAYBE_PreReadFileWithSequentialAccess)
{
    ...
    EXPECT_TRUE(PreReadFile(text_file, /*is_executable=*/false, /*sequential=*/true));
}
...
TEST(FileUtilMultiThreadedTest, MultiThreadedTempFiles)
{
    ...
    const RepeatingClosure open_write_close_read = BindRepeating([] {
...
#if BUILDFLAG(IS_WIN)
        HANDLE handle = reinterpret_cast<HANDLE>(_get_osfhandle(_fileno(output_file.get())));
        DWORD bytes_written = 0;
        ::WriteFile(handle, content.c_str(), content.length(), &bytes_written, NULL);
#else
        size_t bytes_written = ::write(::fileno(output_file.get()), content.c_str(), content.length());
#endif
        ...
        EXPECT_TRUE(ReadFileToString(output_filename, &output_file_contents)) << output_filename;
...
TEST_F(FileUtilTest, CopyFileContentsWithSendfile)
{
    ...
    FilePath file_name_from = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("copy_contents_file_in.txt"));
    FilePath file_name_to = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("copy_contents_file_out.txt"));
...
TEST_F(FileUtilTest, CopyFileContentsWithSendfilePipe)
{
    ...
    File to(file_name_to, File::FLAG_OPEN | File::FLAG_WRITE | File::FLAG_CREATE_ALWAYS);
    ASSERT_TRUE(to.IsValid());
    ...
    ASSERT_FALSE(internal::CopyFileContentsWithSendfile(pipe_read, to, retry_slow));
    ASSERT_TRUE(retry_slow);
}
...
TEST_F(FileUtilTest, CopyFileContentsWithSendfileSocket)
{
    ...
    bool retry_slow = false;
    ASSERT_TRUE(internal::CopyFileContentsWithSendfile(from, to_sock, retry_slow));
    ...
    File to(file_name_to, File::FLAG_OPEN | File::FLAG_WRITE | File::FLAG_CREATE_ALWAYS);
    ASSERT_TRUE(to.IsValid());
    ASSERT_FALSE(internal::CopyFileContentsWithSendfile(from_sock, to, retry_slow));
    ASSERT_TRUE(retry_slow);
}
...
TEST_F(FileUtilTest, CopyFileContentsWithSendfileSeqFile)
{
    ...
    for (auto* const file : { "/proc/meminfo", "/proc/self/cmdline", "/proc/self/environ", "/proc/self/auxv" }) {
        ...
        bool retry_slow = false;
        ASSERT_FALSE(internal::CopyFileContentsWithSendfile(from, to, retry_slow)) << proc_file_from << " should have failed";
        ASSERT_TRUE(retry_slow) << "retry slow for " << proc_file_from << " should be set";
