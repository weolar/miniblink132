...
class BackgroundDataSerializer : public ImportantFileWriter::BackgroundDataSerializer {
public:
    explicit BackgroundDataSerializer(ImportantFileWriter::BackgroundDataProducerCallback data_producer_callback)
        : data_producer_callback_(std::move(data_producer_callback))
    {
        DCHECK(data_producer_callback_);
    }

    ImportantFileWriter::BackgroundDataProducerCallback GetSerializedDataProducerForBackgroundSequence() override
    {
        EXPECT_TRUE(sequence_checker_.CalledOnValidSequence());
...
public:
    ...
    // OnAfterWrite().
    void ObserveNextWriteCallbacks(ImportantFileWriter* writer, base::OnceClosure after_write_closure = base::DoNothing());
...
private:
    ...
    void OnAfterWrite(bool success)
    {
        ...
        after_write_observation_state_ = success ? CALLED_WITH_SUCCESS : CALLED_WITH_ERROR;
...
void WriteCallbacksObserver::ObserveNextWriteCallbacks(ImportantFileWriter* writer, base::OnceClosure after_write_closure)
{
    after_write_closure_ = std::move(after_write_closure);
    writer->RegisterOnNextWriteCallbacks(base::BindOnce(&WriteCallbacksObserver::OnBeforeWrite, base::Unretained(this)),
        base::BindOnce(&WriteCallbacksObserver::OnAfterWrite, base::Unretained(this)));
}

WriteCallbackObservationState WriteCallbacksObserver::GetAndResetObservationState()
{
    EXPECT_EQ(after_write_observation_state_ != NOT_CALLED, before_write_called_) << "The before-write callback should always be called before the "
...
TEST_F(ImportantFileWriterTest, WriteWithObserver)
{
    ...
    EXPECT_EQ(CALLED_WITH_SUCCESS, write_callback_observer_.GetAndResetObservationState());
    ASSERT_TRUE(PathExists(writer.path()));
    EXPECT_EQ("foo", GetFileContent(writer.path()));
...
TEST_F(ImportantFileWriterTest, FailedWriteWithObserver)
{
    ...
    ImportantFileWriter writer(FilePath().AppendASCII("bad/../path"), SingleThreadTaskRunner::GetCurrentDefault());
    EXPECT_FALSE(PathExists(writer.path()));
    EXPECT_EQ(NOT_CALLED, write_callback_observer_.GetAndResetObservationState());
    write_callback_observer_.ObserveNextWriteCallbacks(&writer);
    writer.WriteNow("foo");
    ...
    // to false.
    EXPECT_EQ(CALLED_WITH_ERROR, write_callback_observer_.GetAndResetObservationState());
    EXPECT_FALSE(PathExists(writer.path()));
}
...
TEST_F(ImportantFileWriterTest, CallbackRunsOnWriterThread)
{
    ...
    // executed on it.
    base::WaitableEvent wait_helper(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    file_writer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Wait, base::Unretained(&wait_helper)));
...
TEST_F(ImportantFileWriterTest, ScheduleWrite)
{
    ...
    ImportantFileWriter writer(file_, SingleThreadTaskRunner::GetCurrentDefault(), kCommitInterval);
...
TEST_F(ImportantFileWriterTest, DoScheduledWrite_FailToSerialize)
{
    ...
    histogram_tester.ExpectTotalCount("ImportantFile.SerializationDuration.All", 0);
...
TEST_F(ImportantFileWriterTest, ScheduleWriteWithBackgroundDataSerializer)
{
    ...
    constexpr TimeDelta kCommitInterval = Seconds(12345);
    MockOneShotTimer timer;
    ImportantFileWriter writer(file_, file_writer_thread.task_runner(), kCommitInterval);
    EXPECT_EQ(0u, writer.previous_data_size());
    writer.SetTimerForTesting(&timer);
    EXPECT_FALSE(writer.HasPendingWrite());
    ASSERT_FALSE(file_writer_thread.task_runner()->RunsTasksInCurrentSequence());
    BackgroundDataSerializer serializer(base::BindLambdaForTesting([&]() -> std::optional<std::string> {
        EXPECT_TRUE(file_writer_thread.task_runner()->RunsTasksInCurrentSequence());
        return "foo";
    }));
    ...
    histogram_tester.ExpectTotalCount("ImportantFile.SerializationDuration.All", 1);
...
TEST_F(ImportantFileWriterTest, ScheduleWriteWithBackgroundDataSerializer_FailToSerialize)
{
    base::HistogramTester histogram_tester;
    base::Thread file_writer_thread("ImportantFileWriter test thread");
    file_writer_thread.Start();
...
{
    base::HistogramTester histogram_tester;
    ImportantFileWriter writer(file_, SingleThreadTaskRunner::GetCurrentDefault(), "Foo");
    DataSerializer serializer("foo");
    writer.ScheduleWrite(&serializer);
    writer.DoScheduledWrite();
    RunLoop().RunUntilIdle();
    histogram_tester.ExpectTotalCount("ImportantFile.SerializationDuration.Foo", 1);
...
TEST_F(ImportantFileWriterTest, ReplaceFileSuccess)
{
    ...
    writer.SetReplaceFileCallbackForTesting(base::BindRepeating([](const FilePath&, const FilePath&, File::Error* error) {
        *error = File::FILE_OK;
        return true;
    }));
    ...
    write_callback_observer_.ObserveNextWriteCallbacks(&writer, run_loop.QuitClosure());
    writer.WriteNow("foo");
    run_loop.Run();
    ...
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount", 0, 1);
    ...
    histogram_tester.ExpectTotalCount("ImportantFile.FileReplaceRetryCount2.All", 0);

    // 0 means no retries were needed.
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult", 0, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult.All", 0, 1);
}
...
TEST_F(ImportantFileWriterTest, ReplaceFileRetry)
{
    ...
    writer.SetReplaceFileCallbackForTesting(base::BindLambdaForTesting([&retry_count](const FilePath&, const FilePath&, File::Error* error) {
        if (retry_count < 2) {
            retry_count += 1;
    ...
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount", 2, 1);

    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2", 2, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2.All", 2, 1);

    // 1 means succeeded with retries.
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult", 1, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult.All", 1, 1);
}
...
TEST_F(ImportantFileWriterTest, ReplaceFileFails)
{
    ...
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount", 10, 1);

    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2", 5, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2.All", 5, 1);

    // 2 means ReplaceFile never succeeded.
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult", 2, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult.All", 2, 1);
}
...
TEST_F(ImportantFileWriterTest, ReplaceFileFailsWithSuffix)
{
    ...
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2.Foo", 5, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceRetryCount2.All", 5, 1);

    // 2 means ReplaceFile never succeeded.
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult.Foo", 2, 1);
    histogram_tester.ExpectUniqueSample("ImportantFile.FileReplaceResult.All", 2, 1);
}
#endif
