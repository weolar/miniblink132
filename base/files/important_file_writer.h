...
public:
    ...
    using BackgroundDataProducerCallback = base::OnceCallback<std::optional<std::string>()>;
    ...
    public:
        ...
        virtual BackgroundDataProducerCallback GetSerializedDataProducerForBackgroundSequence() = 0;
    ...
    static bool WriteFileAtomically(const FilePath& path, std::string_view data, std::string_view histogram_suffix = std::string_view());
    ...
    ImportantFileWriter(const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, std::string_view histogram_suffix = std::string_view());

    // Same as above, but with a custom commit interval.
    ImportantFileWriter(
        const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, TimeDelta interval, std::string_view histogram_suffix = std::string_view());
    ...
    const FilePath& path() const LIFETIME_BOUND
    {
        return path_;
    }
    ...
    void ScheduleWriteWithBackgroundDataSerializer(BackgroundDataSerializer* serializer);
    ...
    void RegisterOnNextWriteCallbacks(OnceClosure before_next_write_callback, OnceCallback<void(bool success)> after_next_write_callback);
    ...
    size_t previous_data_size() const
    {
    ...
    using ReplaceFileCallback = RepeatingCallback<bool(const FilePath&, const FilePath&, File::Error*)>;
...
private:
    ...
    void WriteNowWithBackgroundDataProducer(BackgroundDataProducerCallback background_producer);

    // Helper function to call WriteFileAtomically() with a promise-like callback
    // producing a std::string.
    static void ProduceAndWriteStringToFileAtomically(const FilePath& path, BackgroundDataProducerCallback data_producer_for_background_sequence,
