...
FilePath BuildSearchFilter(FileEnumerator::FolderSearchPolicy policy, const FilePath& root_path, const FilePath::StringType& pattern)
{
    ...
    while (has_find_data_ || !pending_paths_.empty()) {
        if (!has_find_data_) {
            ...
            find_handle_ = FindFirstFileEx(src.value().c_str(),
                ...
                ChromeToWindowsType(&find_data_), FindExSearchNameMatch, nullptr, FIND_FIRST_EX_LARGE_FETCH);
            has_find_data_ = true;
        } else {
