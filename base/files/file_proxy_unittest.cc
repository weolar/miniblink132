...
public:
    ...
    void DidCreateTemporary(base::RepeatingClosure continuation, File::Error error, const FilePath& path)
    {
        error_ = error;
        path_ = path;
        continuation.Run();
    ...
    void DidGetFileInfo(base::RepeatingClosure continuation, File::Error error, const File::Info& file_info)
    {
        error_ = error;
    ...
    void DidRead(base::RepeatingClosure continuation, File::Error error, base::span<const char> data)
    {
        error_ = error;
    ...
    void DidWrite(base::RepeatingClosure continuation, File::Error error, int bytes_written)
    {
        error_ = error;
...
protected:
    ...
    {
        RunLoop run_loop;
        proxy->CreateOrOpen(TestPath(), flags, BindOnce(&FileProxyTest::DidCreateOrOpen, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
        run_loop.Run();
    ...
    const FilePath& TestDirPath() const
    {
        return dir_.GetPath();
    }
    const FilePath TestPath() const
    {
...
TEST_F(FileProxyTest, CreateOrOpen_Create)
{
    ...
    RunLoop run_loop;
    proxy.CreateOrOpen(
        TestPath(), File::FLAG_CREATE | File::FLAG_READ, BindOnce(&FileProxyTest::DidCreateOrOpen, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
...
TEST_F(FileProxyTest, CreateOrOpen_Open)
{
    ...
    RunLoop run_loop;
    proxy.CreateOrOpen(
        TestPath(), File::FLAG_OPEN | File::FLAG_READ, BindOnce(&FileProxyTest::DidCreateOrOpen, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
...
{
    {
        ...
        {
            FileProxy proxy(file_task_runner());
            proxy.CreateOrOpen(TestPath(), File::FLAG_CREATE | File::FLAG_READ,
                BindOnce(&FileProxyTest::DidCreateOrOpen, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
        }
        run_loop.Run();
    }
...
TEST_F(FileProxyTest, Close)
{
    ...
    RunLoop run_loop;
    proxy.Close(BindOnce(&FileProxyTest::DidFinish, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
    EXPECT_EQ(File::FILE_OK, error_);
    EXPECT_FALSE(proxy.IsValid());
...
{
    {
        FileProxy proxy(file_task_runner());
        {
            ...
            proxy.CreateTemporary(
                0 /* additional_file_flags */, BindOnce(&FileProxyTest::DidCreateTemporary, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
            run_loop.Run();
        }
        ...
        // The file should be writable.
        {
            ...
            proxy.Write(0, base::as_byte_span(std::string_view("test")),
                BindOnce(&FileProxyTest::DidWrite, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
            run_loop.Run();
        }
        EXPECT_EQ(File::FILE_OK, error_);
...
TEST_F(FileProxyTest, GetInfo)
{
    ...
    RunLoop run_loop;
    proxy.GetInfo(BindOnce(&FileProxyTest::DidGetFileInfo, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
...
TEST_F(FileProxyTest, Read)
{
    ...
    RunLoop run_loop;
    proxy.Read(0, 128, BindOnce(&FileProxyTest::DidRead, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
...
TEST_F(FileProxyTest, WriteAndFlush)
{
    ...
    {
        RunLoop run_loop;
        proxy.Write(0, write_span, BindOnce(&FileProxyTest::DidWrite, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
        run_loop.Run();
    }
    EXPECT_EQ(File::FILE_OK, error_);
    ...
    {
        RunLoop run_loop;
        proxy.Flush(BindOnce(&FileProxyTest::DidFinish, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
        run_loop.Run();
    }
    EXPECT_EQ(File::FILE_OK, error_);
...
{
    FileProxy proxy(file_task_runner());
    CreateProxy(File::FLAG_CREATE | File::FLAG_WRITE | File::FLAG_WRITE_ATTRIBUTES, &proxy);

    Time last_accessed_time = Time::Now() - Days(12345);
    Time last_modified_time = Time::Now() - Hours(98765);

    RunLoop run_loop;
    proxy.SetTimes(last_accessed_time, last_modified_time, BindOnce(&FileProxyTest::DidFinish, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
    EXPECT_EQ(File::FILE_OK, error_);
    ...
    EXPECT_EQ(static_cast<int>(last_modified_time.InSecondsFSinceUnixEpoch()), static_cast<int>(info.last_modified.InSecondsFSinceUnixEpoch()));

#if !BUILDFLAG(IS_FUCHSIA)
    // On Fuchsia, /tmp is noatime
    EXPECT_EQ(static_cast<int>(last_accessed_time.InSecondsFSinceUnixEpoch()), static_cast<int>(info.last_accessed.InSecondsFSinceUnixEpoch()));
...
TEST_F(FileProxyTest, SetLength_Shrink)
{
    ...
    RunLoop run_loop;
    proxy.SetLength(7, BindOnce(&FileProxyTest::DidFinish, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
...
TEST_F(FileProxyTest, SetLength_Expand)
{
    ...
    CreateProxy(File::FLAG_OPEN | File::FLAG_WRITE, &proxy);
    RunLoop run_loop;
    proxy.SetLength(53, BindOnce(&FileProxyTest::DidFinish, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
    run_loop.Run();
