...
const char* const kCommonDoubleExtensionSuffixes[] = { "bz", "bz2", "gz", "lz", "lzma", "lzo", "xz", "z", "zst" };
...
StringType::size_type ExtensionSeparatorPosition(const StringType& path)
{
    ...
    const StringType::size_type last_separator = path.find_last_of(FilePath::kSeparators, last_dot - 1, FilePath::kSeparatorsLength - 1);
...
bool FilePath::AppendRelativePath(const FilePath& child, FilePath* path) const
{
    ...
    if (IsNetwork() && parent_components.size() > 1) {
        if (*parent_comp++ != *child_comp++ || !base::EqualsCaseInsensitiveASCII(*parent_comp++, *child_comp++)) {
            return false;
        }
    }
...
FilePath FilePath::DirName() const
{
    ...
    StringType::size_type last_separator = new_path.path_.find_last_of(kSeparators, StringType::npos, kSeparatorsLength - 1);
...
inline base_icu::UChar32 HFSReadNextNonIgnorableCodepoint(const char* string, size_t length, size_t* index)
{
    ...
    while (*index < length && codepoint == 0) {
        ...
        CBU8_NEXT(reinterpret_cast<const uint8_t*>(string), *index, length, codepoint);
...
int FilePath::HFSFastUnicodeCompare(StringPieceType string1, StringPieceType string2)
{
...
StringType FilePath::GetHFSDecomposedForm(StringPieceType string)
{
    apple::ScopedCFTypeRef<CFStringRef> cfstring(CFStringCreateWithBytesNoCopy(
        nullptr, reinterpret_cast<const UInt8*>(string.data()), checked_cast<CFIndex>(string.length()), kCFStringEncodingUTF8, false, kCFAllocatorNull));
...
int FilePath::CompareIgnoreCase(StringPieceType string1, StringPieceType string2)
{
    ...
    if (hfs1.empty() || hfs2.empty()) {
        apple::ScopedCFTypeRef<CFStringRef> cfstring1(CFStringCreateWithBytesNoCopy(
            NULL, reinterpret_cast<const UInt8*>(string1.data()), checked_cast<CFIndex>(string1.length()), kCFStringEncodingUTF8, false, kCFAllocatorNull));
        apple::ScopedCFTypeRef<CFStringRef> cfstring2(CFStringCreateWithBytesNoCopy(
            NULL, reinterpret_cast<const UInt8*>(string2.data()), checked_cast<CFIndex>(string2.length()), kCFStringEncodingUTF8, false, kCFAllocatorNull));
        ...
        if (!cfstring1 || !cfstring2) {
            int comparison = memcmp(string1.data(), string2.data(), std::min(string1.length(), string2.length()));
            if (comparison < 0)
                return -1;
            if (comparison > 0)
                return 1;
...
FilePath FilePath::NormalizePathSeparatorsTo(CharType separator) const
{
    ...
    DCHECK_NE(kSeparators + kSeparatorsLength, std::find(kSeparators, kSeparators + kSeparatorsLength, separator));
