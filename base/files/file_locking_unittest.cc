...
bool WaitForEventWithTimeout(const FilePath& signal_dir, const char* signal_file, const base::TimeDelta& timeout)
{
...
{
    return WaitForEventWithTimeout(signal_dir, signal_file, base::TimeDelta::Max());
}
...
MULTIPROCESS_TEST_MAIN(ChildMain)
{
    ...
    const File::LockMode mode = use_exclusive_lock ? File::LockMode::kExclusive : File::LockMode::kShared;

    // Immediately lock the file.
    File file(temp_path.AppendASCII(kLockFile), File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE);
    CHECK(file.IsValid());
...
protected:
    void SetUp() override
    {
        ...
        lock_file_.Initialize(temp_dir_.GetPath().AppendASCII(kLockFile), File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE);
    ...
    {
        return ::WaitForEventWithTimeout(temp_dir_.GetPath(), signal_file, TestTimeouts::action_timeout());
    }

    // Start a child process set to use the specified locking mode and unlock
    // action, and wait for it to lock the file.
    void StartChildAndSignalLock(File::LockMode lock_mode, const char* unlock_action)
    {
        // Create a temporary dir and spin up a ChildLockExit subprocess against it.
        const FilePath temp_path = temp_dir_.GetPath();
        base::CommandLine child_command_line(base::GetMultiProcessTestChildBaseCommandLine());
        ...
        lock_child_ = base::SpawnMultiProcessTestChild(ChildMainString, child_command_line, base::LaunchOptions());
    ...
    void ExitChildCleanly()
    {
        ...
        int rv = -1;
        ASSERT_TRUE(WaitForMultiprocessTestChildExit(lock_child_, TestTimeouts::action_timeout(), &rv));
        ASSERT_EQ(0, rv);
    }
