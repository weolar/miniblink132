...
public:
    BlockTestsWritingToSpecialDirsTest()
    {
        ...
        if (prev_block_tests.has_value()) {
            save_block_tests_.emplace(std::move(prev_block_tests->blocked_dirs_), prev_block_tests->failure_callback_);
    ...
    ~BlockTestsWritingToSpecialDirsTest() override
    {
        if (save_block_tests_.has_value()) {
            Get().emplace(std::move(save_block_tests_->blocked_dirs_), save_block_tests_->failure_callback_);
        }
    }
...
{
    EXPECT_TRUE(BlockTestsWritingToSpecialDirs::CanWriteToPath(PathService::CheckedGet(DIR_SRC_TEST_DATA_ROOT).AppendASCII("file")));
}
...
TEST_F(BlockTestsWritingToSpecialDirsTest, SpecialDirWriteBlocker)
{
    ...
    EXPECT_FALSE(BlockTestsWritingToSpecialDirs::CanWriteToPath(PathService::CheckedGet(DIR_SRC_TEST_DATA_ROOT).AppendASCII("file")));
}
