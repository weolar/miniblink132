...
void ImportantFileWriterCleaner::AddDirectory(const FilePath& directory)
{
    ...
    if (task_runner->RunsTasksInCurrentSequence()) {
        ...
        task_runner->PostTask(FROM_HERE, BindOnce(&ImportantFileWriterCleaner::AddDirectoryImpl, Unretained(&instance), directory));
    }
}
...
void ImportantFileWriterCleaner::Initialize()
{
    ...
    DCHECK(!task_runner_ || task_runner_ == SequencedTaskRunner::GetCurrentDefault());
...
void ImportantFileWriterCleaner::ScheduleTask()
{
    ...
    running_ = ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { TaskPriority::BEST_EFFORT, TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN, MayBlock() },
        BindOnce(&ImportantFileWriterCleaner::CleanInBackground, upper_bound_time_, std::move(pending_directories_), std::ref(stop_flag_)),
        // Unretained is safe here since the cleaner instance is never destroyed.
        BindOnce(&ImportantFileWriterCleaner::OnBackgroundTaskFinished, Unretained(this)));
}

// static
bool ImportantFileWriterCleaner::CleanInBackground(Time upper_bound_time, std::vector<FilePath> directories, std::atomic_bool& stop_flag)
{
    DCHECK(!directories.empty());
    for (auto scan = directories.begin(), end = directories.end(); scan != end; ++scan) {
        const auto& directory = *scan;
        FileEnumerator file_enum(directory, /*recursive=*/false, FileEnumerator::FILES, FormatTemporaryFileName(FILE_PATH_LITERAL("*")).value());
...
void ImportantFileWriterCleaner::OnBackgroundTaskFinished(bool processing_completed)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
