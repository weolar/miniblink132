...
TEST_F(FilePathTest, IsAbsoluteOrNetwork)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].expected_is_absolute, observed_is_absolute) << "i: " << i << ", input: " << input.value();
        bool observed_is_network = input.IsNetwork();
        EXPECT_EQ(cases[i].expected_is_network, observed_is_network) << "i: " << i << ", input: " << input.value();
    }
}
...
TEST_F(FilePathTest, AppendRelativePathTest)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        FilePath child(cases[i].inputs[1]);
        {
            ...
            EXPECT_EQ(!cases[i].expected.empty(), success) << "i: " << i << ", parent: " << parent.value() << ", child: " << child.value();
            EXPECT_EQ(cases[i].expected, result.value()) << "i: " << i << ", parent: " << parent.value() << ", child: " << child.value();
        }
        {
...
TEST_F(FilePathTest, Extension2)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].expected, extension) << "i: " << i << ", path: " << path.value();
        EXPECT_EQ(cases[i].expected, final_extension) << "i: " << i << ", path: " << path.value();
    }

    for (size_t i = 0; i < std::size(double_extension_cases); ++i) {
        FilePath path(double_extension_cases[i].input);
        FilePath::StringType extension = path.Extension();
        EXPECT_EQ(double_extension_cases[i].expected, extension) << "i: " << i << ", path: " << path.value();
    }
}
...
TEST_F(FilePathTest, InsertBeforeExtension)
{
    const struct BinaryTestData cases[]
        = { { { FPL(""), FPL("") }, FPL("") },
              ...
              { { FPL("C:\\bar.baz\\foo.dll.exe"), FPL("txt") }, FPL("C:\\bar.baz\\foo.dlltxt.exe") },
...
TEST_F(FilePathTest, RemoveExtension)
{
    ...
    const FilePath::StringPieceType tarballs[] = { FPL("foo.tar.gz"), FPL("foo.tar.xz"), FPL("foo.tar.bz2"), FPL("foo.tar.Z"), FPL("foo.tar.bz") };
    for (size_t i = 0; i < std::size(tarballs); ++i) {
        FilePath path(FPL("foo.tar.gz"));
        FilePath removed = path.RemoveExtension();
        FilePath removed_final = path.RemoveFinalExtension();
        EXPECT_EQ(FPL("foo"), removed.value()) << "i: " << i << ", path: " << path.value();
        EXPECT_EQ(FPL("foo.tar"), removed_final.value()) << "i: " << i << ", path: " << path.value();
    }
}
...
TEST_F(FilePathTest, MatchesFinalExtension)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].expected, path.MatchesFinalExtension(ext)) << "i: " << i << ", path: " << path.value() << ", ext: " << ext;
    }
}
...
TEST_F(FilePathTest, FromASCII)
{
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(FilePath::StringType(cases[i].native), from_ascii.value()) << "i: " << i << ", input: " << cases[i].utf8;
    }
}
...
TEST_F(FilePathTest, FromUTF8Unsafe_And_AsUTF8Unsafe)
{
...
#if !defined(SYSTEM_NATIVE_UTF8) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
    ...
    for (size_t i = 0; i < std::size(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].native, from_utf8.value()) << "i: " << i << ", input: " << cases[i].native;
        // Test AsUTF8Unsafe() works.
        FilePath from_native = FilePath(cases[i].native);
        EXPECT_EQ(cases[i].utf8, from_native.AsUTF8Unsafe()) << "i: " << i << ", input: " << cases[i].native;
...
{
    const UnaryTestData cases[] = { { FPL(""), FPL("") }, { FPL("/"), FPL("/") }, { FPL("foo"), FPL("foo/") }, { FPL("foo/"), FPL("foo/") } };
    for (const auto& i : cases) {
        FilePath input = FilePath(i.input).NormalizePathSeparators();
...
TEST_F(FilePathTest, GetHFSDecomposedFormWithInvalidInput)
{
    ...
    for (auto* invalid_input : cases) {
        FilePath::StringType observed = FilePath::GetHFSDecomposedForm(invalid_input);
