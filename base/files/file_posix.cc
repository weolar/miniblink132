...
static_assert(File::FROM_BEGIN == SEEK_SET && File::FROM_CURRENT == SEEK_CUR && File::FROM_END == SEEK_END, "whence mapping must match the system headers");
...
BASE_FEATURE(kMacEfficientFileFlush, "MacEfficientFileFlush", base::FEATURE_DISABLED_BY_DEFAULT);

const FeatureParam<bool> kMacEfficientFileFlushUseBarrier { &kMacEfficientFileFlush, "MacEfficientFileFlushUseBarrier", true };
...
std::atomic<MacFileFlushMechanism> g_mac_file_flush_mechanism { MacFileFlushMechanism::kFullFsync };
...
int CallFtruncate(PlatformFile file, int64_t length)
{
    ...
    static_assert(sizeof(off_t) >= sizeof(int64_t), "off_t is not a 64-bit integer");
...
File::Error CallFcntlFlock(PlatformFile file, std::optional<File::LockMode> mode)
{
    struct flock lock;
...
void File::Info::FromStat(const stat_wrapper_t& stat_info)
{
    ...
    last_modified = Time::FromTimeT(last_modified_sec) + Microseconds(last_modified_nsec / Time::kNanosecondsPerMicrosecond);

    last_accessed = Time::FromTimeT(last_accessed_sec) + Microseconds(last_accessed_nsec / Time::kNanosecondsPerMicrosecond);

    creation_time = Time::FromTimeT(creation_time_sec) + Microseconds(creation_time_nsec / Time::kNanosecondsPerMicrosecond);
}
...
int File::Read(int64_t offset, char* data, int size)
{
    ...
    long rv;
    do {
        rv = HANDLE_EINTR(pread(file_.get(), data + bytes_read, static_cast<size_t>(size - bytes_read), static_cast<off_t>(offset + bytes_read)));
        if (rv <= 0)
            break;
...
int File::ReadAtCurrentPos(char* data, int size)
{
    ...
    long rv;
    do {
        rv = HANDLE_EINTR(read(file_.get(), data + bytes_read, static_cast<size_t>(size - bytes_read)));
        if (rv <= 0)
            break;
...
int File::ReadNoBestEffort(int64_t offset, char* data, int size)
{
    ...
    return checked_cast<int>(HANDLE_EINTR(pread(file_.get(), data, static_cast<size_t>(size), static_cast<off_t>(offset))));
}
...
int File::ReadAtCurrentPosNoBestEffort(char* data, int size)
{
    ...
    return checked_cast<int>(HANDLE_EINTR(read(file_.get(), data, static_cast<size_t>(size))));
}
...
int File::Write(int64_t offset, const char* data, int size)
{
    ...
    do {
        ...
        // instead of pwrite().
        static_assert(sizeof(int64_t) == sizeof(off64_t), "off64_t must be 64 bits");
        rv = HANDLE_EINTR(pwrite64(file_.get(), data + bytes_written, static_cast<size_t>(size - bytes_written), offset + bytes_written));
#else
        rv = HANDLE_EINTR(pwrite(file_.get(), data + bytes_written, static_cast<size_t>(size - bytes_written), offset + bytes_written));
#endif
        if (rv <= 0)
            break;
...
int File::WriteAtCurrentPos(const char* data, int size)
{
    ...
    long rv;
    do {
        rv = HANDLE_EINTR(write(file_.get(), data + bytes_written, static_cast<size_t>(size - bytes_written)));
        if (rv <= 0)
            break;
...
int File::WriteAtCurrentPosNoBestEffort(const char* data, int size)
{
    ...
    return checked_cast<int>(HANDLE_EINTR(write(file_.get(), data, static_cast<size_t>(size))));
}
...
bool File::GetInfo(Info* info) const
{
    ...
    if (path_.IsContentUri()) {
        ...
        return (success && info->size > 0 && !internal::IsDocumentUri(path_)) || GetContentUriInfo(path_, info);
    }
#endif
    return success;
}
...
void File::InitializeFeatures()
{
    if (FeatureList::IsEnabled(kMacEfficientFileFlush)) {
        ...
        if (kMacEfficientFileFlushUseBarrier.Get()) {
            g_mac_file_flush_mechanism.store(MacFileFlushMechanism::kBarrierFsync, std::memory_order_relaxed);
        } else {
            g_mac_file_flush_mechanism.store(MacFileFlushMechanism::kFlush, std::memory_order_relaxed);
        }
    }
}
#endif // BUILDFLAG(IS_APPLE)
...
bool File::Flush()
{
    ...
    return true;
#elif BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_LINUX)
...
int File::Stat(const FilePath& path, stat_wrapper_t* sb)
{
    ...
    if (path.IsContentUri()) {
        ...
        Info info;
        if ((file.IsValid() && file.GetInfo(&info)) || GetContentUriInfo(path, &info)) {
            ...
            sb->st_mtime_nsec
                = (info.last_modified.ToDeltaSinceWindowsEpoch().InMicroseconds() % Time::kMicrosecondsPerSecond) * Time::kNanosecondsPerMicrosecond;
            return 0;
        }
    }
#endif
