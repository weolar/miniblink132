...
public:
    ...
    enum Flags : uint32_t {
        ...
        FLAG_WIN_NO_EXECUTE = 1 << 21, // Windows only. Marks the file with a deny ACE that prevents
    ...
    UNSAFE_BUFFER_USAGE int ReadNoBestEffort(int64_t offset, char* data, int size);
    std::optional<size_t> ReadNoBestEffort(int64_t offset, base::span<uint8_t> data);
    ...
    // on error.
    UNSAFE_BUFFER_USAGE int WriteAtCurrentPosNoBestEffort(const char* data, int size);
    std::optional<size_t> WriteAtCurrentPosNoBestEffort(base::span<const uint8_t> data);
...
#if !BUILDFLAG(IS_FUCHSIA) // Fuchsia's POSIX API does not support file locking.
    enum class LockMode {
        kShared,
        kExclusive,
    };
    ...
    static constexpr uint32_t AddFlagsForPassingToUntrustedProcess(uint32_t flags)
    {
        if (flags & File::FLAG_WRITE || flags & File::FLAG_APPEND || flags & File::FLAG_WRITE_ATTRIBUTES) {
