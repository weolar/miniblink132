...
const std::vector<FileEnumerator::FolderSearchPolicy> kFolderSearchPolicies { FileEnumerator::FolderSearchPolicy::MATCH_ONLY,
...
struct TestFile {
    ...
    TestFile(const FilePath::CharType* directory, const FilePath::CharType* file_name, const char* c)
        : path(FilePath(directory).Append(file_name))
        , contents(c)
    {
    }
...
struct TestDirectory {
    explicit TestDirectory(const FilePath::CharType* n)
        : name(n)
    {
    }
    const FilePath name;
...
void CheckModificationTime(const FileEnumerator::FileInfo& actual, Time expected_last_modified_time)
{
...
void CheckFileAgainstInfo(const FileEnumerator::FileInfo& actual, TestFile& expected)
{
    EXPECT_FALSE(expected.found) << "Got " << expected.path.BaseName().value() << " twice";
    expected.found = true;
    EXPECT_EQ(actual.GetSize(), int64_t(expected.contents.size()));
    CheckModificationTime(actual, expected.info.last_modified);
}

void CheckDirectoryAgainstInfo(const FileEnumerator::FileInfo& actual, TestDirectory& expected)
{
...
circular_deque<FilePath> RunEnumerator(
    const FilePath& root_path, bool recursive, int file_type, const FilePath::StringType& pattern, FileEnumerator::FolderSearchPolicy folder_search_policy)
{
    circular_deque<FilePath> rv;
    FileEnumerator enumerator(root_path, recursive, file_type, pattern, folder_search_policy, FileEnumerator::ErrorPolicy::IGNORE_ERRORS);
...
bool GetFileInfo(const FilePath& file_path, File::Info& info)
{
    ...
    File f(file_path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_BACKUP_SEMANTICS);
    if (!f.IsValid()) {
        LOG(ERROR) << "Could not open " << file_path.value() << ": " << File::ErrorToString(f.error_details());
        return false;
    }
    if (!f.GetInfo(&info)) {
        std::string last_error = File::ErrorToString(File::GetLastFileError());
        LOG(ERROR) << "Could not get info about " << file_path.value() << ": " << last_error;
        return false;
    }
...
void SetUpTestFiles(const ScopedTempDir& temp_dir, std::vector<TestFile>& files)
{
...
TEST(FileEnumerator, NotExistingPath)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        const auto files = RunEnumerator(path, true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, FILE_PATH_LITERAL(""), policy);
...
TEST(FileEnumerator, EmptyFolder)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        const auto files = RunEnumerator(temp_dir.GetPath(), true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, kEmptyPattern, policy);
        EXPECT_THAT(files, IsEmpty());
    }
}
...
TEST(FileEnumerator, SingleFileInFolderForFileSearch)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        const auto files = RunEnumerator(temp_dir.GetPath(), true, FileEnumerator::FILES, kEmptyPattern, policy);
...
TEST(FileEnumerator, SingleFileInFolderForDirSearch)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        const auto files = RunEnumerator(path, true, FileEnumerator::DIRECTORIES, kEmptyPattern, policy);
        EXPECT_THAT(files, IsEmpty());
    }
}
...
TEST(FileEnumerator, SingleFileInFolderWithFiltering)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        auto files = RunEnumerator(path, true, FileEnumerator::FILES, FILE_PATH_LITERAL("*.txt"), policy);
        EXPECT_THAT(files, ElementsAre(file));

        files = RunEnumerator(path, true, FileEnumerator::FILES, FILE_PATH_LITERAL("*.pdf"), policy);
        EXPECT_THAT(files, IsEmpty());
    }
}
...
TEST(FileEnumerator, TwoFilesInFolder)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        ...
        files = RunEnumerator(path, true, FileEnumerator::FILES, FILE_PATH_LITERAL("foo*"), policy);
        EXPECT_THAT(files, ElementsAre(foo_txt));

        files = RunEnumerator(path, true, FileEnumerator::FILES, FILE_PATH_LITERAL("*.pdf"), policy);
        EXPECT_THAT(files, IsEmpty());

        files = RunEnumerator(path, true, FileEnumerator::FILES, kEmptyPattern, policy);
        EXPECT_THAT(files, UnorderedElementsAre(foo_txt, bar_txt));
    }
}
...
TEST(FileEnumerator, SingleFolderInFolderForFileSearch)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        const auto files = RunEnumerator(path, true, FileEnumerator::FILES, kEmptyPattern, policy);
        EXPECT_THAT(files, IsEmpty());
    }
}
...
TEST(FileEnumerator, TwoFoldersInFolder)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        auto files = RunEnumerator(path, true, FileEnumerator::DIRECTORIES, kEmptyPattern, policy);
        EXPECT_THAT(files, UnorderedElementsAre(subdir_foo, subdir_bar));

        files = RunEnumerator(path, true, FileEnumerator::DIRECTORIES, FILE_PATH_LITERAL("foo"), policy);
...
TEST(FileEnumerator, FolderAndFileInFolder)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        auto files = RunEnumerator(path, true, FileEnumerator::FILES, kEmptyPattern, policy);
        EXPECT_THAT(files, ElementsAre(file));

        files = RunEnumerator(path, true, FileEnumerator::DIRECTORIES, kEmptyPattern, policy);
        EXPECT_THAT(files, ElementsAre(temp_subdir.GetPath()));

        files = RunEnumerator(path, true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, kEmptyPattern, policy);
...
TEST(FileEnumerator, FileInSubfolder)
{
    ...
    for (auto policy : kFolderSearchPolicies) {
        auto files = RunEnumerator(temp_dir.GetPath(), true, FileEnumerator::FILES, kEmptyPattern, policy);
        EXPECT_THAT(files, ElementsAre(file));

        files = RunEnumerator(temp_dir.GetPath(), false, FileEnumerator::FILES, kEmptyPattern, policy);
        EXPECT_THAT(files, IsEmpty());
    }
}
...
TEST(FileEnumerator, FilesInSubfoldersWithFiltering)
{
    ...
    auto files = RunEnumerator(temp_dir.GetPath(), true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, FILE_PATH_LITERAL("foo*"),
        FileEnumerator::FolderSearchPolicy::MATCH_ONLY);
    EXPECT_THAT(files, UnorderedElementsAre(subdir_foo, foo_test, foo_foo, foo_bar));

    files = RunEnumerator(
        temp_dir.GetPath(), true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, FILE_PATH_LITERAL("foo*"), FileEnumerator::FolderSearchPolicy::ALL);
...
TEST(FileEnumerator, InvalidDirectory)
{
    ...
    FileEnumerator enumerator(test_file, /*recursive=*/true, FileEnumerator::FILES, kEmptyPattern, FileEnumerator::FolderSearchPolicy::ALL,
...
TEST(FileEnumerator, SymLinkLoops)
{
    ...
    auto files = RunEnumerator(
        temp_dir.GetPath(), true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES, kEmptyPattern, FileEnumerator::FolderSearchPolicy::MATCH_ONLY);

    EXPECT_THAT(files, UnorderedElementsAre(subdir, link, file));

    files = RunEnumerator(subdir, true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES | FileEnumerator::SHOW_SYM_LINKS, kEmptyPattern,
        FileEnumerator::FolderSearchPolicy::MATCH_ONLY);
...
TEST(FileEnumerator, GetInfo)
{
    ...
    std::vector<TestFile> files = { TestFile(FILE_PATH_LITERAL("file1"), "First"), TestFile(FILE_PATH_LITERAL("file2"), "Second"),
        TestFile(FILE_PATH_LITERAL("file3"), "Third-third-third") };
    SetUpTestFiles(temp_dir, files);

#if BUILDFLAG(IS_ANDROID)
    FilePath root_dir = *base::test::android::GetInMemoryContentTreeUriFromCacheDirDirectory(temp_dir.GetPath());
#else
    FilePath root_dir = temp_dir.GetPath();
#endif
    FileEnumerator file_enumerator(root_dir, false, FileEnumerator::FILES | FileEnumerator::DIRECTORIES);
    ...
    for (const TestFile& file : files) {
        EXPECT_TRUE(file.found) << "File " << file.path.value() << " was not returned";
    }
}
...
TEST(FileEnumerator, GetInfoRecursive)
{
    ...
    TestDirectory directories[] = { TestDirectory(FILE_PATH_LITERAL("dir1")), TestDirectory(FILE_PATH_LITERAL("dir2-empty")),
        TestDirectory(FILE_PATH_LITERAL("dir3")), TestDirectory(FILE_PATH_LITERAL("dir4")) };
    ...
    std::vector<TestFile> files
        = { TestFile(FILE_PATH_LITERAL("dir1"), FILE_PATH_LITERAL("file1"), "First"), TestFile(FILE_PATH_LITERAL("dir1"), FILE_PATH_LITERAL("file2"), "Second"),
              TestFile(FILE_PATH_LITERAL("dir3"), FILE_PATH_LITERAL("fileA"), "Third-third-3"),
    ...
    FilePath root_dir = temp_dir.GetPath();
#endif
    FileEnumerator file_enumerator(root_dir, true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES);
    while (!file_enumerator.Next().empty()) {
        auto info = file_enumerator.GetInfo();
        bool found = false;
        if (info.IsDirectory()) {
            ...
            for (TestFile& file : files) {
                if (info.GetName() == file.path.BaseName()) {
                    CheckFileAgainstInfo(info, file);
#if BUILDFLAG(IS_ANDROID)
                    std::string expected = temp_dir.GetPath().BaseName().Append(file.path.DirName()).value();
    ...
    for (const TestDirectory& dir : directories) {
        EXPECT_TRUE(dir.found) << "Directory " << dir.name.value() << " was not returned";
    }
    for (const TestFile& file : files) {
        EXPECT_TRUE(file.found) << "File " << file.path.value() << " was not returned";
    }
...
TEST(FileEnumerator, GetInfoDotDot)
{
    ...
    std::vector<TestFile> files = { TestFile(kSubdir, FILE_PATH_LITERAL("file1"), "First"), TestFile(kSubdir, FILE_PATH_LITERAL("file2"), "Second"),
    ...
    FileEnumerator file_enumerator(subdir_path, false, FileEnumerator::FILES | FileEnumerator::DIRECTORIES | FileEnumerator::INCLUDE_DOT_DOT);
    while (!file_enumerator.Next().empty()) {
        auto info = file_enumerator.GetInfo();
        bool found = false;
        if (info.IsDirectory()) {
...
TEST(FileEnumerator, OnlyName)
{
    ...
    auto found_paths = RunEnumerator(
        path, /*recursive=*/false, FileEnumerator::FileType::NAMES_ONLY, FilePath::StringType(), FileEnumerator::FolderSearchPolicy::MATCH_ONLY);
...
class FileEnumeratorForEachTest : public ::testing::TestWithParam<FileEnumeratorForEachTestCase> { };

INSTANTIATE_TEST_SUITE_P(FileEnumeratorForEachTestCases, FileEnumeratorForEachTest,
...
TEST_P(FileEnumeratorForEachTest, TestCases)
{
    ...
    // each directory.
    for (const FilePath& path : { mock_path, mock_path.Append(FILE_PATH_LITERAL("1.2.3.4")), mock_path.Append(FILE_PATH_LITERAL("Download")),
             mock_path.Append(FILE_PATH_LITERAL("Install")) }) {
        ASSERT_TRUE(CreateDirectory(path));
        for (const FilePath::StringType& file_name : { FILE_PATH_LITERAL("mock.executable"), FILE_PATH_LITERAL("mock.text") }) {
            ASSERT_TRUE(File(path.Append(file_name), File::FLAG_CREATE | File::FLAG_WRITE).IsValid());
        }
    }

    int invocation_count = 0;

    FileEnumerator(mock_path, GetParam().recursive, GetParam().file_type).ForEach([&invocation_count](const FilePath& item) {
        ++invocation_count;
        if (invocation_count > GetParam().expected_invocation_count) {
            ADD_FAILURE() << "Unexpected file/directory found: " << item << ": " << invocation_count << ": " << GetParam().expected_invocation_count;
        }
    });
