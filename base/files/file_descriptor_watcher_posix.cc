...
class FileDescriptorWatcher::Controller::Watcher : public MessagePumpForIO::FdWatcher, public CurrentThread::DestructionObserver {
public:
    Watcher(WeakPtr<Controller> controller, base::WaitableEvent& on_destroyed, MessagePumpForIO::Mode mode, int fd);
...
private:
    ...
    const scoped_refptr<SequencedTaskRunner> callback_task_runner_ = SequencedTaskRunner::GetCurrentDefault();
...
FileDescriptorWatcher::Controller::Watcher::Watcher(WeakPtr<Controller> controller, base::WaitableEvent& on_destroyed, MessagePumpForIO::Mode mode, int fd)
...
void FileDescriptorWatcher::Controller::Watcher::StartWatching()
{
    ...
    const bool watch_success = CurrentIOThread::Get()->WatchFileDescriptor(fd_, false, mode_, &fd_watch_controller_, this);
...
void FileDescriptorWatcher::Controller::Watcher::OnFileCanReadWithoutBlocking(int fd)
{
    DCHECK_EQ(fd_, fd);
...
void FileDescriptorWatcher::Controller::Watcher::WillDestroyCurrentMessageLoop()
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
FileDescriptorWatcher::Controller::Controller(MessagePumpForIO::Mode mode, int fd, const RepeatingClosure& callback)
    : callback_(callback)
...
void FileDescriptorWatcher::Controller::StartWatching()
{
    ...
    if (io_thread_task_runner_->BelongsToCurrentThread()) {
        ...
        io_thread_task_runner_->PostTask(FROM_HERE, BindOnce(&Watcher::StartWatching, Unretained(watcher_)));
    }
}
...
FileDescriptorWatcher::FileDescriptorWatcher(scoped_refptr<SingleThreadTaskRunner> io_thread_task_runner)
...
std::unique_ptr<FileDescriptorWatcher::Controller> FileDescriptorWatcher::WatchReadable(int fd, const RepeatingClosure& callback)
{
    return WrapUnique(new Controller(MessagePumpForIO::WATCH_READ, fd, callback));
}

std::unique_ptr<FileDescriptorWatcher::Controller> FileDescriptorWatcher::WatchWritable(int fd, const RepeatingClosure& callback)
{
