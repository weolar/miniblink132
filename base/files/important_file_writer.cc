...
void UmaHistogramRetryCountWithSuffix(std::string_view histogram_suffix, int retry_count, bool success)
{
    ...
    auto result = success ? (retry_count > 0 ? ReplaceResult::kSuccessWithRetry : ReplaceResult::kSuccessWithoutRetry) : ReplaceResult::kFailure;
    ...
    if (histogram_suffix.empty()) {
        ...
        UmaHistogramEnumeration(base::JoinString({ kResultHistogramName, histogram_suffix }, "."), result);
    }
    UmaHistogramEnumeration(base::JoinString({ kResultHistogramName, "All" }, "."), result);
    if (retry_count > 0) {
        if (histogram_suffix.empty()) {
            UmaHistogramExactLinear(kCountHistogramName, retry_count, kReplaceRetries + 1);
        } else {
            UmaHistogramExactLinear(base::JoinString({ kCountHistogramName, histogram_suffix }, "."), retry_count, kReplaceRetries + 1);
        }
        UmaHistogramExactLinear(base::JoinString({ kCountHistogramName, "All" }, "."), retry_count, kReplaceRetries + 1);
    }
}
#endif

void UmaHistogramTimesWithSuffix(const char* histogram_name, std::string_view histogram_suffix, base::TimeDelta sample)
{
    ...
    if (histogram_suffix.empty()) {
        ...
        UmaHistogramTimes(base::JoinString({ histogram_name, histogram_suffix }, "."), sample);
    }
...
void DeleteTmpFileWithRetry(File tmp_file, const FilePath& tmp_file_path, int attempt = 0)
{
#if BUILDFLAG(IS_WIN)
    ...
    if (!DeleteFile(tmp_file_path) && ++attempt < kMaxDeleteAttempts && SequencedTaskRunner::HasCurrentDefault()) {
        SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, BindOnce(&DeleteTmpFileWithRetry, base::File(), tmp_file_path, attempt), kDeleteFileRetryDelay);
    }
}
...
// static
bool ImportantFileWriter::WriteFileAtomically(const FilePath& path, std::string_view data, std::string_view histogram_suffix)
{
    ...
    return WriteFileAtomicallyImpl(path, data, histogram_suffix,
        /*from_instance=*/false, BindRepeating(&ReplaceFile));
}

// static
void ImportantFileWriter::ProduceAndWriteStringToFileAtomically(const FilePath& path, BackgroundDataProducerCallback data_producer_for_background_sequence,
    OnceClosure before_write_callback, OnceCallback<void(bool success)> after_write_callback, ReplaceFileCallback replace_file_callback,
    const std::string& histogram_suffix)
{
    // Produce the actual data string on the background sequence.
    std::optional<std::string> data = std::move(data_producer_for_background_sequence).Run();
    if (!data) {
    ...
    const bool result = WriteFileAtomicallyImpl(path, *data, histogram_suffix,
        /*from_instance=*/true, std::move(replace_file_callback));
...
bool ImportantFileWriter::WriteFileAtomicallyImpl(
    const FilePath& path, std::string_view data, std::string_view histogram_suffix, bool from_instance, ReplaceFileCallback replace_file_callback)
{
    ...
    int bytes_written = 0;
    for (const char *scan = data.data(), *const end = scan + data.length(); scan < end; scan += bytes_written) {
        const int write_amount = static_cast<int>(std::min(kMaxWriteAmount, end - scan));
        ...
        if (bytes_written != write_amount) {
            DPLOG(WARNING) << "Failed to write " << write_amount << " bytes to temp "
                           << "file to update " << path << " (bytes_written=" << bytes_written << ")";
    ...
    UmaHistogramExactLinear("ImportantFile.FileReplaceRetryCount", retry_count, kReplaceRetryFailure);
#else
    tmp_file.Close();
    result = replace_file_callback.Run(tmp_file_path, path, &replace_file_error);
    ...
    UmaHistogramTimesWithSuffix("ImportantFile.WriteDuration", histogram_suffix, write_duration);

    return result;
}

ImportantFileWriter::ImportantFileWriter(const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, std::string_view histogram_suffix)
    : ImportantFileWriter(path, std::move(task_runner), kDefaultCommitInterval, histogram_suffix)
{
}

ImportantFileWriter::ImportantFileWriter(
    const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, TimeDelta interval, std::string_view histogram_suffix)
    : path_(path)
...
void ImportantFileWriter::WriteNow(std::string data)
{
    ...
    WriteNowWithBackgroundDataProducer(base::BindOnce([](std::string data) { return std::make_optional(std::move(data)); }, std::move(data)));
}

void ImportantFileWriter::SetReplaceFileCallbackForTesting(ReplaceFileCallback callback)
{
    replace_file_callback_ = std::move(callback);
}

void ImportantFileWriter::WriteNowWithBackgroundDataProducer(BackgroundDataProducerCallback background_data_producer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    auto split_task = SplitOnceCallback(BindOnce(&ProduceAndWriteStringToFileAtomically, path_, std::move(background_data_producer),
        std::move(before_next_write_callback_), std::move(after_next_write_callback_), replace_file_callback_, histogram_suffix_));

    if (!task_runner_->PostTask(FROM_HERE,
            MakeCriticalClosure("ImportantFileWriter::WriteNow", std::move(split_task.first),
                /*is_immediate=*/true))) {
...
void ImportantFileWriter::ScheduleWrite(DataSerializer* serializer)
{
    ...
    if (!timer().IsRunning()) {
        timer().Start(FROM_HERE, commit_interval_, BindOnce(&ImportantFileWriter::DoScheduledWrite, Unretained(this)));
    }
}

void ImportantFileWriter::ScheduleWriteWithBackgroundDataSerializer(BackgroundDataSerializer* serializer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ImportantFileWriter::DoScheduledWrite()
{
    ...
    if (absl::holds_alternative<DataSerializer*>(serializer_)) {
        ...
        data_producer_for_background_sequence = base::BindOnce([](std::string data) { return std::make_optional(std::move(data)); }, std::move(data).value());
    } else {
        data_producer_for_background_sequence = absl::get<BackgroundDataSerializer*>(serializer_)->GetSerializedDataProducerForBackgroundSequence();
    ...
    UmaHistogramTimesWithSuffix("ImportantFile.SerializationDuration", histogram_suffix_, serialization_duration);

    WriteNowWithBackgroundDataProducer(std::move(data_producer_for_background_sequence));
    DCHECK(!HasPendingWrite());
}

void ImportantFileWriter::RegisterOnNextWriteCallbacks(OnceClosure before_next_write_callback, OnceCallback<void(bool success)> after_next_write_callback)
{
