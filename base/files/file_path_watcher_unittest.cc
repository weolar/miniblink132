...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)
...
struct Event {
    ...
    {
        return error == other.error && path == other.path && change_info.file_path_type == other.change_info.file_path_type
            && change_info.change_type == other.change_info.change_type &&
            // Don't compare the values of the cookies.
            change_info.cookie.has_value() == other.change_info.cookie.has_value();
    }
};
using EventListMatcher = testing::Matcher<std::list<Event>>;

Event ToEvent(const FilePathWatcher::ChangeInfo& change_info, const FilePath& path, bool error)
{
    return Event { .error = error, .path = path, .change_info = change_info };
}

std::ostream& operator<<(std::ostream& os, const FilePathWatcher::ChangeType& change_type)
{
    switch (change_type) {
...
std::ostream& operator<<(std::ostream& os, const FilePathWatcher::FilePathType& file_path_type)
{
...
std::ostream& operator<<(std::ostream& os, const FilePathWatcher::ChangeInfo& change_info)
{
    return os << "ChangeInfo{ file_path_type: " << change_info.file_path_type << ", change_type: " << change_info.change_type
...
std::ostream& operator<<(std::ostream& os, const Event& event)
{
    ...
    return os << "Event{ path: " << event.path << ", change_info: " << event.change_info << " }";
}

void SpinEventLoopForABit()
{
    base::RunLoop loop;
    SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, loop.QuitClosure(), TestTimeouts::tiny_timeout());
    loop.Run();
}

// Returns the reason why `value` matches, or doesn't match, `matcher`.
template <typename MatcherType, typename Value> std::string Explain(const MatcherType& matcher, const Value& value)
{
...
inline constexpr auto HasPath = [](const FilePath& path) { return testing::Field(&Event::path, path); };
inline constexpr auto HasErrored = [] { return testing::Field(&Event::error, testing::IsTrue()); };
inline constexpr auto HasCookie = [] { return testing::Field(&Event::change_info, testing::Field(&FilePathWatcher::ChangeInfo::cookie, testing::IsTrue())); };
inline constexpr auto IsType = [](const FilePathWatcher::ChangeType& change_type) {
    return testing::Field(&Event::change_info, testing::Field(&FilePathWatcher::ChangeInfo::change_type, change_type));
};
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)
inline constexpr auto IsFile
    = [] { return testing::Field(&Event::change_info, testing::Field(&FilePathWatcher::ChangeInfo::file_path_type, FilePathWatcher::FilePathType::kFile)); };
inline constexpr auto IsDirectory = [] {
    return testing::Field(&Event::change_info, testing::Field(&FilePathWatcher::ChangeInfo::file_path_type, FilePathWatcher::FilePathType::kDirectory));
};
#else
inline constexpr auto IsUnknownPathType
    = [] { return testing::Field(&Event::change_info, testing::Field(&FilePathWatcher::ChangeInfo::file_path_type, FilePathWatcher::FilePathType::kUnknown)); };
...
private:
    ...
    ExpectedEventsSinceLastWait expected_events_since_last_wait_ = ExpectedEventsSinceLastWait::kNone;
};
...
public:
    ...
    virtual void OnFileChangedWithInfo(const FilePathWatcher::ChangeInfo& change_info, const FilePath& path, bool error) = 0;
...
public:
    TestDelegate()
        : id_(g_next_delegate_id.GetNext())
    {
    }
    ...
    void OnFileChangedWithInfo(const FilePathWatcher::ChangeInfo& change_info, const FilePath& path, bool error) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // out.
    void RunUntilEventsMatch(const EventListMatcher& matcher, ExpectedEventsSinceLastWait expected_events_since_last_wait, const Location& location = FROM_HERE)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    void RunUntilEventsMatch(const EventListMatcher& matcher, const Location& location = FROM_HERE)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        return RunUntilEventsMatch(matcher, ExpectedEventsSinceLastWait::kSome, location);
    }
    // Convenience method for above.
    void RunUntilEventsMatch(AccumulatingEventExpecter& event_expecter, const Location& location = FROM_HERE)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

        return RunUntilEventsMatch(event_expecter.GetMatcher(), event_expecter.GetAndResetExpectedEventsSinceLastWait(), location);
    }
    ...
    void SpinAndExpectNoEvents(const Location& location = FROM_HERE)
    {
        ...
        return RunUntilEventsMatch(testing::IsEmpty(), ExpectedEventsSinceLastWait::kNone, location);
    }
...
protected:
    ...
    bool SetupWatch(const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::Type watch_type);

    bool SetupWatchWithOptions(const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::WatchOptions watch_options);

    bool SetupWatchWithChangeInfo(const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::WatchOptions watch_options);
...
bool FilePathWatcherTest::SetupWatch(const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::Type watch_type)
{
    return watcher->Watch(target, watch_type, BindRepeating(&TestDelegateBase::OnFileChanged, delegate->AsWeakPtr()));
}

bool FilePathWatcherTest::SetupWatchWithOptions(
    const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::WatchOptions watch_options)
{
    return watcher->WatchWithOptions(target, watch_options, BindRepeating(&TestDelegateBase::OnFileChanged, delegate->AsWeakPtr()));
}
    ...
    const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::WatchOptions watch_options)
{
    return watcher->WatchWithChangeInfo(
        target, watch_options, BindPostTaskToCurrentDefault(BindRepeating(&TestDelegateBase::OnFileChangedWithInfo, delegate->AsWeakPtr())));
}
...
TEST_F(FilePathWatcherTest, NewFile)
{
    ...
    ASSERT_TRUE(SetupWatch(test_file(), &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, NewDirectoryRecursiveWatch)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatch(test_file(), &watcher, &delegate, FilePathWatcher::Type::kRecursive));
...
TEST_F(FilePathWatcherTest, CreateParentDirectory)
{
    ...
    ASSERT_TRUE(SetupWatch(child, &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, WindowsBufferOverflow)
{
...

    {
        ...
        // it's twice that.
        const size_t kWriteFileEventSize = (sizeof(FILE_NOTIFY_INFORMATION) + test_file().AsUTF8Unsafe().size()) * 2;
        ...
        for (size_t bytes_in_buffer = 0; bytes_in_buffer < kMaxBufferSize; bytes_in_buffer += kWriteFileEventSize) {
...
TEST_F(FilePathWatcherTest, DeleteDuringNotify)
{
    ...
    ASSERT_TRUE(SetupWatch(test_file(), deleter.watcher(), &deleter, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MultipleWatchersSingleFile)
{
    ...
    ASSERT_TRUE(SetupWatch(test_file(), &watcher1, &delegate1, FilePathWatcher::Type::kNonRecursive));
    ASSERT_TRUE(SetupWatch(test_file(), &watcher2, &delegate2, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, NonExistentDirectory)
{
    ...
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatch(file, &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
    ...
    delegate.RunUntilEventsMatch(testing::Not(testing::IsEmpty()), ExpectedEventsSinceLastWait::kSome);
...
TEST_F(FilePathWatcherTest, WatchDirectory)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatch(dir, &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MoveParent)
{
    ...
    ASSERT_TRUE(SetupWatch(file, &file_watcher, &file_delegate, FilePathWatcher::Type::kNonRecursive));
    ASSERT_TRUE(SetupWatch(subdir, &subdir_watcher, &subdir_delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, RecursiveWatch)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    bool setup_result = SetupWatch(dir, &watcher, &delegate, FilePathWatcher::Type::kRecursive);
    if (!FilePathWatcher::RecursiveWatchAvailable()) {
...
TEST_F(FilePathWatcherTest, RecursiveWithSymLink)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatch(symlink, &watcher, &delegate, FilePathWatcher::Type::kRecursive));
...
TEST_F(FilePathWatcherTest, MoveChild)
{
    ...
    ASSERT_TRUE(SetupWatch(dest_file, &file_watcher, &file_delegate, FilePathWatcher::Type::kNonRecursive));
    ASSERT_TRUE(SetupWatch(dest_subdir, &subdir_watcher, &subdir_delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, CreateLink)
{
    ...
    ASSERT_TRUE(SetupWatch(test_link(), &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, LinkedDirectoryPart1)
{
    ...
    ASSERT_TRUE(SetupWatch(linkfile, &watcher, &delegate, FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, RacyRecursiveWatch)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto watcher_callback = BindLambdaForTesting([&](const FilePath& path, bool error) {
        ...
        bool setup_result = watcher->Watch(dir, FilePathWatcher::Type::kRecursive, watcher_callback);
        ASSERT_TRUE(setup_result);
...
TEST_F(FilePathWatcherTest, InotifyLimitInWatch)
{
    ...
    ASSERT_FALSE(
        watcher->Watch(test_file(), FilePathWatcher::Type::kNonRecursive, BindLambdaForTesting([&](const FilePath& path, bool error) { ADD_FAILURE(); })));
...
TEST_F(FilePathWatcherTest, InotifyLimitInUpdate)
{
    ...
    for (auto callback_type : { kDestroyWatcher, kDoNothing }) {
        ...
        ASSERT_TRUE(watcher->Watch(test_file(), FilePathWatcher::Type::kNonRecursive, watcher_callback));
...
TEST_F(FilePathWatcherTest, InotifyLimitInUpdateRecursive)
{
    ...
    for (auto callback_type : { kDestroyWatcher, kDoNothing }) {
        ...
        ASSERT_TRUE(watcher->Watch(dir, FilePathWatcher::Type::kRecursive, watcher_callback));

        constexpr size_t kMaxLimit = 10u;
        ScopedMaxNumberOfInotifyWatchesOverrideForTest max_inotify_watches(kMaxLimit);
...
TEST_F(FilePathWatcherTest, ReturnFullPath_RecursiveInRootFolder)
{
    ...
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(
        SetupWatchWithOptions(watched_folder, &directory_watcher, &delegate, { .type = FilePathWatcher::Type::kRecursive, .report_modified_path = true }));
...
TEST_F(FilePathWatcherTest, ReturnFullPath_NonRecursiveInRootFolder)
{
    ...
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(
        SetupWatchWithOptions(watched_folder, &directory_watcher, &delegate, { .type = FilePathWatcher::Type::kNonRecursive, .report_modified_path = true }));
...
TEST_F(FilePathWatcherTest, ReturnWatchedPath_RecursiveInRootFolder)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatchWithOptions(watched_folder, &directory_watcher, &delegate, { .type = FilePathWatcher::Type::kRecursive }));
...
TEST_F(FilePathWatcherTest, ReturnWatchedPath_NonRecursiveInRootFolder)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatchWithOptions(watched_folder, &directory_watcher, &delegate, { .type = FilePathWatcher::Type::kNonRecursive }));
...
{
    FilePath test_dir1(temp_dir_.GetPath().AppendASCII("DirAttributesChangedDir1"));
...
TEST_F(FilePathWatcherTest, TrivialNoDir)
{
    ...
    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_FALSE(SetupWatch(non_existent, &watcher, &delegate, FilePathWatcher::Type::kTrivial));
}
...
TEST_F(FilePathWatcherTest, TrivialDirStart)
{
    ...
    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatch(tmp_dir, &watcher, &delegate, FilePathWatcher::Type::kTrivial));
}
...
TEST_F(FilePathWatcherTest, TrivialParentDirChange)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatch(sub_dir2, &watcher, &delegate, FilePathWatcher::Type::kTrivial));
...
TEST_F(FilePathWatcherTest, TrivialDirMove)
{
    ...
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatch(sub_dir, &watcher, &delegate, FilePathWatcher::Type::kTrivial));
...
class FilePathWatcherWithChangeInfoTest : public FilePathWatcherTest, public testing::WithParamInterface<std::tuple<FilePathWatcher::Type, bool>> {
public:
    void SetUp() override
    {
        FilePathWatcherTest::SetUp();
    }

protected:
    FilePathWatcher::Type type() const
    {
        return std::get<0>(GetParam());
    }
    bool report_modified_path() const
    {
    ...
    {
        return FilePathWatcher::WatchOptions { .type = type(), .report_modified_path = report_modified_path() };
    }
};
...
TEST_P(FilePathWatcherWithChangeInfoTest, NewFile)
{
    ...
    const auto each_event_matcher = testing::Each(testing::AllOf(HasPath(test_file()), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    ...
    const auto sequence_matcher = testing::ElementsAre(IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified));
    // Put it all together.
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);

    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(test_file(), &watcher, &delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, NewDirectory)
{
    const auto matcher = testing::ElementsAre(testing::AllOf(
        HasPath(test_file()), testing::Not(HasErrored()), IsDirectory(), IsType(FilePathWatcher::ChangeType::kCreated), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, ModifiedFile)
{
    ...
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_file()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kModified), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, MovedFile)
{
    ...
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_file()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kMoved), HasCookie()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, MatchCookies)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kMoved), HasCookie()));
    // TODO(crbug.com/40260973): Some platforms will not provide separate
    // events for "moved from" and "moved to". Update this matcher once support
    // for those platforms is added.
    const auto sequence_matcher = testing::UnorderedElementsAre(
        testing::AllOf(HasPath(report_modified_path() ? source_file : test_file()), IsType(FilePathWatcher::ChangeType::kMoved)),
        testing::AllOf(HasPath(report_modified_path() ? dest_file : test_file()), IsType(FilePathWatcher::ChangeType::kMoved)));
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
    ...
    EXPECT_EQ(events.front().change_info.cookie, events.back().change_info.cookie);
}

TEST_P(FilePathWatcherWithChangeInfoTest, DeletedFile)
{
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_file()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasCookie())));
...
{
    const auto matcher = testing::ElementsAre(testing::AllOf(
        HasPath(test_file()), testing::Not(HasErrored()), IsDirectory(), IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, MultipleWatchersSingleFile)
{
    ...
    FilePathWatcher watcher1, watcher2;
    TestDelegate delegate1, delegate2;
    ASSERT_TRUE(SetupWatchWithChangeInfo(test_file(), &watcher1, &delegate1, GetWatchOptions()));
    ASSERT_TRUE(SetupWatchWithChangeInfo(test_file(), &watcher2, &delegate2, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, NonExistentDirectory)
{
    FilePath dir(temp_dir_.GetPath().AppendASCII("dir"));
    FilePath file(dir.AppendASCII("file"));
    const auto each_event_matcher = testing::Each(testing::AllOf(HasPath(file), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    const auto sequence_matcher = testing::IsSupersetOf(
        { IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified), IsType(FilePathWatcher::ChangeType::kDeleted) });
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);

    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(file, &watcher, &delegate, GetWatchOptions()));
    ...
    // trigger an event.
    delegate.RunUntilEventsMatch(testing::IsEmpty(), ExpectedEventsSinceLastWait::kNone);
...
TEST_P(FilePathWatcherWithChangeInfoTest, DirectoryChain)
{
    ...
    const auto each_event_matcher = testing::Each(testing::AllOf(HasPath(file), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    const auto sequence_matcher = testing::IsSupersetOf({ IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified) });
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
...
TEST_P(FilePathWatcherWithChangeInfoTest, DisappearingDirectory)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(HasPath(file), testing::Not(HasErrored()), IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, DeleteAndRecreate)
{
    ...
    static_assert(kExpectedEventsForNewFileWrite == 2);
    const auto sequence_matcher = testing::ElementsAre(
        IsType(FilePathWatcher::ChangeType::kDeleted), IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified));
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
...
TEST_P(FilePathWatcherWithChangeInfoTest, WatchDirectory)
{
    ...
    const auto each_event_matcher = testing::Each(testing::AllOf(testing::Not(HasErrored()), testing::Not(HasCookie())));
    const auto sequence_matcher
        = testing::IsSupersetOf({ testing::AllOf(HasPath(report_modified_path() ? file1 : dir), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated)),
            testing::AllOf(HasPath(report_modified_path() ? file1 : dir), IsFile(), IsType(FilePathWatcher::ChangeType::kModified)),
            testing::AllOf(HasPath(report_modified_path() ? file1 : dir), IsFile(), IsType(FilePathWatcher::ChangeType::kDeleted)),
            testing::AllOf(HasPath(report_modified_path() ? file2 : dir), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated)) });
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
    ...
    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(dir, &watcher, &delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, MoveParent)
{
    ...
    const auto file_delegate_sequence_matcher = testing::IsSupersetOf({ testing::AllOf(HasPath(file), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated)),
        testing::AllOf(HasPath(file), IsDirectory(), IsType(FilePathWatcher::ChangeType::kMoved)) });
    const auto subdir_delegate_sequence_matcher
        = testing::IsSupersetOf({ testing::AllOf(HasPath(subdir), IsDirectory(), IsType(FilePathWatcher::ChangeType::kCreated)),
            testing::AllOf(HasPath(report_modified_path() ? file : subdir), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated)),
            testing::AllOf(HasPath(subdir), IsDirectory(), IsType(FilePathWatcher::ChangeType::kMoved)) });
    const auto file_delegate_matcher = testing::AllOf(each_event_matcher, file_delegate_sequence_matcher);
    const auto subdir_delegate_matcher = testing::AllOf(each_event_matcher, subdir_delegate_sequence_matcher);

    FilePathWatcher file_watcher, subdir_watcher;
    TestDelegate file_delegate, subdir_delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(file, &file_watcher, &file_delegate, GetWatchOptions()));
    ASSERT_TRUE(SetupWatchWithChangeInfo(subdir, &subdir_watcher, &subdir_delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, MoveChild)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(testing::Not(HasErrored()), IsDirectory(), IsType(FilePathWatcher::ChangeType::kMoved), HasCookie()));
    const auto file_delegate_sequence_matcher = testing::ElementsAre(HasPath(dest_file));
    const auto subdir_delegate_sequence_matcher = testing::ElementsAre(HasPath(dest_subdir));
    const auto file_delegate_matcher = testing::AllOf(each_event_matcher, file_delegate_sequence_matcher);
    const auto subdir_delegate_matcher = testing::AllOf(each_event_matcher, subdir_delegate_sequence_matcher);
    ...
    FilePathWatcher file_watcher, subdir_watcher;
    TestDelegate file_delegate, subdir_delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(dest_file, &file_watcher, &file_delegate, GetWatchOptions()));
    ASSERT_TRUE(SetupWatchWithChangeInfo(dest_subdir, &subdir_watcher, &subdir_delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, CreateLink)
{
    ...
    // support it.
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_link()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated), testing::Not(HasCookie())));

    FilePathWatcher watcher;
    TestDelegate delegate;
    AccumulatingEventExpecter event_expecter;
    ASSERT_TRUE(SetupWatchWithChangeInfo(test_link(), &watcher, &delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, DeleteLink)
{
    ...
    // support it.
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_link()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, ModifiedLinkedFile)
{
    ...
    // support it.
    const auto matcher = testing::ElementsAre(
        testing::AllOf(HasPath(test_link()), testing::Not(HasErrored()), IsFile(), IsType(FilePathWatcher::ChangeType::kModified), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, CreateTargetLinkedFile)
{
    // TODO(crbug.com/40260973): Check for symlink-ness on platforms which
    // support it.
    const auto each_event_matcher = testing::Each(testing::AllOf(HasPath(test_link()), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    // TODO(crbug.com/40260973): Update this when change types are
    // supported on on more platforms.
    static_assert(kExpectedEventsForNewFileWrite == 2);
    const auto sequence_matcher = testing::ElementsAre(IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified));
...
TEST_P(FilePathWatcherWithChangeInfoTest, LinkedDirectoryPart1)
{
    ...
    // TODO(crbug.com/40260973): Check for symlink-ness on platforms which
    // support it.
    const auto each_event_matcher = testing::Each(testing::AllOf(HasPath(linkfile), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    const auto sequence_matcher = testing::IsSupersetOf(
        { IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified), IsType(FilePathWatcher::ChangeType::kDeleted) });
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
    ...
    TestDelegate delegate;
    // Note that we are watching dir.lnk/file which doesn't exist yet.
    ASSERT_TRUE(SetupWatchWithChangeInfo(linkfile, &watcher, &delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, CreatedFileInDirectory)
{
    ...
    const auto matcher = testing::IsSupersetOf({ testing::AllOf(HasPath(report_modified_path() ? child : parent), IsFile(),
        IsType(FilePathWatcher::ChangeType::kCreated), testing::Not(HasErrored()), testing::Not(HasCookie())) });

    ASSERT_TRUE(CreateDirectory(parent));

    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(parent, &watcher, &delegate, GetWatchOptions()));
...
TEST_P(FilePathWatcherWithChangeInfoTest, ModifiedFileInDirectory)
{
    ...
    const auto matcher = testing::ElementsAre(testing::AllOf(HasPath(report_modified_path() ? child : parent), IsFile(),
        IsType(FilePathWatcher::ChangeType::kModified), testing::Not(HasErrored()), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, DeletedFileInDirectory)
{
    ...
    const auto matcher = testing::ElementsAre(testing::AllOf(HasPath(report_modified_path() ? child : parent), IsFile(),
        IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasErrored()), testing::Not(HasCookie())));
...
TEST_P(FilePathWatcherWithChangeInfoTest, FileInDirectory)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(HasPath(report_modified_path() ? child : parent), testing::Not(HasErrored()), IsFile(), testing::Not(HasCookie())));
    const auto sequence_matcher = testing::IsSupersetOf(
        { IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kModified), IsType(FilePathWatcher::ChangeType::kDeleted) });
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
...
TEST_P(FilePathWatcherWithChangeInfoTest, DirectoryInDirectory)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(HasPath(report_modified_path() ? child : parent), testing::Not(HasErrored()), IsDirectory(), testing::Not(HasCookie())));
    const auto sequence_matcher = testing::ElementsAre(IsType(FilePathWatcher::ChangeType::kCreated), IsType(FilePathWatcher::ChangeType::kDeleted));
    const auto matcher = testing::AllOf(each_event_matcher, sequence_matcher);
...
TEST_P(FilePathWatcherWithChangeInfoTest, NestedDirectoryInDirectory)
{
    ...
    if (type() == FilePathWatcher::Type::kRecursive) {
        sequence_matcher = testing::IsSupersetOf(
            { testing::AllOf(HasPath(report_modified_path() ? child : parent), IsDirectory(), IsType(FilePathWatcher::ChangeType::kCreated)),
                testing::AllOf(HasPath(report_modified_path() ? grandchild : parent), IsFile(), IsType(FilePathWatcher::ChangeType::kCreated)),
                testing::AllOf(HasPath(report_modified_path() ? grandchild : parent), IsFile(), IsType(FilePathWatcher::ChangeType::kModified)),
                testing::AllOf(HasPath(report_modified_path() ? grandchild : parent), IsFile(), IsType(FilePathWatcher::ChangeType::kDeleted)),
                testing::AllOf(HasPath(report_modified_path() ? child : parent), IsDirectory(), IsType(FilePathWatcher::ChangeType::kDeleted)) });
    } else {
...
TEST_P(FilePathWatcherWithChangeInfoTest, DeleteDirectoryRecursively)
{
    ...
    const auto each_event_matcher
        = testing::Each(testing::AllOf(testing::Not(HasErrored()), IsType(FilePathWatcher::ChangeType::kDeleted), testing::Not(HasCookie())));
    ...
    if (type() == FilePathWatcher::Type::kRecursive) {
        sequence_matcher = testing::IsSupersetOf(
            { testing::AllOf(HasPath(parent), IsDirectory()), testing::AllOf(HasPath(report_modified_path() ? child : parent), IsDirectory()),
                ...
                testing::AllOf(HasPath(report_modified_path() ? grandchild.BaseName() : parent), IsFile()) });
    } else {
        // Do not expect changes to `grandchild` when watching `parent`
        // non-recursively.
        sequence_matcher = testing::IsSupersetOf(
...
INSTANTIATE_TEST_SUITE_P(
    /* no prefix */, FilePathWatcherWithChangeInfoTest,
    ::testing::Combine(::testing::Values(FilePathWatcher::Type::kNonRecursive, FilePathWatcher::Type::kRecursive),
...
{
    const auto matcher = testing::ElementsAre(testing::AllOf(
        HasPath(test_file()), testing::Not(HasErrored()), IsUnknownPathType(), IsType(FilePathWatcher::ChangeType::kUnknown), testing::Not(HasCookie())));

    FilePathWatcher watcher;
    TestDelegate delegate;
    ASSERT_TRUE(SetupWatchWithChangeInfo(test_file(), &watcher, &delegate, { .type = FilePathWatcher::Type::kNonRecursive }));
