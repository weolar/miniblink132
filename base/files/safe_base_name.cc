...
std::optional<SafeBaseName> SafeBaseName::Create(const FilePath& path)
{
    ...
    if (!basename.IsAbsolute() && !basename.ReferencesParent() && !basename.EndsWithSeparator()) {
...
// static
std::optional<SafeBaseName> SafeBaseName::Create(FilePath::StringPieceType path)
{
    return Create(FilePath(path));
}

SafeBaseName::SafeBaseName(const FilePath& path)
    : path_(path)
{
}
