...
DWORD DeleteFileRecursive(const FilePath& path, const FilePath::StringType& pattern, bool recursive)
{
    ...
    for (FilePath current = traversal.Next(); !current.empty(); current = traversal.Next()) {
        ...
        if ((info.find_data().dwFileAttributes & FILE_ATTRIBUTE_READONLY) && (recursive || !info.IsDirectory())) {
            ::SetFileAttributes(current.value().c_str(), info.find_data().dwFileAttributes & ~DWORD { FILE_ATTRIBUTE_READONLY });
        }
...
bool DoCopyDirectory(const FilePath& from_path, const FilePath& to_path, bool recursive, bool fail_if_exists)
{
    ...
    if (!PathExists(from_path)) {
        DLOG(ERROR) << "CopyDirectory() couldn't stat source directory: " << from_path.value().c_str();
        return false;
    }
    ...
    while (success && !current.empty()) {
        ...
        if (from_is_dir) {
            if (!DirectoryExists(target_path) && !::CreateDirectory(target_path.value().c_str(), NULL)) {
                DLOG(ERROR) << "CopyDirectory() couldn't create directory: " << target_path.value().c_str();
                success = false;
            }
...
DWORD DoDeleteFile(const FilePath& path, bool recursive)
{
    ...
    if (path.BaseName().value().find_first_of(FILE_PATH_LITERAL("*?")) != FilePath::StringType::npos) {
        const DWORD error_code = DeleteFileRecursive(path.DirName(), path.BaseName().value(), recursive);
    ...
    if ((attr & FILE_ATTRIBUTE_READONLY) && !::SetFileAttributes(path.value().c_str(), attr & ~DWORD { FILE_ATTRIBUTE_READONLY })) {
    ...
    if (!(attr & FILE_ATTRIBUTE_DIRECTORY)) {
        return ::DeleteFile(path.value().c_str()) ? ERROR_SUCCESS : ReturnLastErrorOrSuccessOnNotFound();
    }
...
void DeleteFileWithRetry(const FilePath& path, bool recursive, int attempt, OnceCallback<void(bool)> reply_callback)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, { TaskPriority::BEST_EFFORT, MayBlock() },
        BindOnce(&DeleteFileWithRetry, path, recursive, attempt, std::move(reply_callback)), kDeleteFileRetryDelay);
}

OnceClosure GetDeleteFileCallbackInternal(const FilePath& path, bool recursive, OnceCallback<void(bool)> reply_callback)
{
    OnceCallback<void(bool)> bound_callback;
    if (!reply_callback.is_null()) {
        bound_callback = BindPostTask(SequencedTaskRunner::GetCurrentDefault(), std::move(reply_callback));
    }
    return BindOnce(&DeleteFileWithRetry, path, recursive, /*attempt=*/0, std::move(bound_callback));
}
...
bool IsPathSafeToSetAclOn(const FilePath& path)
{
    ...
    if (::IsUserAnAdmin() && PathService::Get(DIR_SYSTEM_TEMP, &secure_system_temp)) {
...
{
    return GetDeleteFileCallbackInternal(path, /*recursive=*/false, std::move(reply_callback));
}

OnceClosure GetDeletePathRecursivelyCallback(const FilePath& path, OnceCallback<void(bool)> reply_callback)
{
    return GetDeleteFileCallbackInternal(path, /*recursive=*/true, std::move(reply_callback));
}
...
bool ReplaceFile(const FilePath& from_path, const FilePath& to_path, File::Error* error)
{
    ...
    if (::ReplaceFile(to_path.value().c_str(), from_path.value().c_str(), NULL, REPLACEFILE_IGNORE_MERGE_ERRORS, NULL, NULL)) {
        return true;
    }
    ...
    if (error) {
        *error = replace_error == File::FILE_ERROR_NOT_FOUND ? File::GetLastFileError() : replace_error;
    }
    return false;
}
...
bool CopyDirectoryExcl(const FilePath& from_path, const FilePath& to_path, bool recursive)
{
...
bool PathHasAccess(const FilePath& path, DWORD dir_desired_access, DWORD file_desired_access)
{
    ...
    win::ScopedHandle file(CreateFile(path_str, desired_access, kFileShareAll, nullptr, OPEN_EXISTING, flags_and_attrs, nullptr));
...
File CreateAndOpenTemporaryFileInDir(const FilePath& dir, FilePath* temp_file)
{
    ...
    constexpr uint32_t kFlags = File::FLAG_CREATE | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_WIN_EXCLUSIVE_READ | File::FLAG_WIN_EXCLUSIVE_WRITE
    ...
    for (int i = 0; i < 100; ++i) {
        temp_name = dir.Append(FormatTemporaryFileName(UTF8ToWide(Uuid::GenerateRandomV4().AsLowercaseString())));
    ...
    const DWORD long_name_len = GetLongPathName(temp_name.value().c_str(), long_temp_name, MAX_PATH);
...
ScopedFILE CreateAndOpenTemporaryStreamInDir(const FilePath& dir, FilePath* path)
{
...
bool CreateTemporaryDirInDir(const FilePath& base_dir, FilePath::StringPieceType prefix, FilePath* new_dir)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    for (int count = 0; count < 50; ++count) {
        ...
        new_dir_name.append(AsWString(NumberToString16(RandInt(0, std::numeric_limits<int32_t>::max()))));
...
bool CreateNewTempDirectory(const FilePath::StringType& prefix, FilePath* new_temp_path)
{
    ...
    if (::IsUserAnAdmin() && PathService::Get(DIR_SYSTEM_TEMP, &parent_dir)
        && CreateTemporaryDirInDir(parent_dir, prefix.empty() ? kDefaultTempDirPrefix : prefix, new_temp_path)) {
        return true;
    }
...
bool NormalizeFilePath(const FilePath& path, FilePath* real_path)
{
    ...
    File file(path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_SHARE_DELETE | File::FLAG_WIN_BACKUP_SEMANTICS);
    if (!file.IsValid()) {
        return false;
    }
    ...
    // >= kMaxPathLength.
    DWORD used_wchars = ::GetFinalPathNameByHandle(file.GetPlatformFile(), native_file_path, kMaxPathLength, FILE_NAME_NORMALIZED | VOLUME_NAME_NT);
    ...
    if (!DevicePathToDriveLetterPath(FilePath(FilePath::StringPieceType(native_file_path, used_wchars)), real_path)) {
        return false;
    }
...
bool DevicePathToDriveLetterPath(const FilePath& nt_device_path, FilePath* out_drive_letter_path)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    DWORD count = ::GetLogicalDriveStrings(drive_strings_buffer.size() - 1u, drive_strings_buffer.data());
    ...
    for (std::wstring_view drive_string :
        base::SplitStringPiece(drive_strings, base::MakeStringViewWithNulChars(L"\0"), base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
        wchar_t drive[] = L" :";
        drive[0u] = drive_string[0u]; // Copy the drive letter.

        wchar_t device_path_as_string[MAX_PATH];
        if (::QueryDosDevice(drive, device_path_as_string, std::size(device_path_as_string))) {
...
FilePath MakeLongFilePath(const FilePath& input)
{
    ...
    path_long_len = ::GetLongPathName(input.value().c_str(), WriteInto(&path_long_str, path_long_len), path_long_len);
    if (path_long_len == 0UL)
        return FilePath();
...
FILE* OpenFile(const FilePath& filename, const char* mode)
{
    ...
    DCHECK(strchr(mode, 'N') == nullptr || (strchr(mode, ',') != nullptr && strchr(mode, 'N') > strchr(mode, ',')));
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
File FILEToFile(FILE* file_stream)
{
    ...
    if (!::DuplicateHandle(
            /*hSourceProcessHandle=*/GetCurrentProcess(), reinterpret_cast<HANDLE>(file_handle),
...
std::optional<uint64_t> ReadFile(const FilePath& filename, span<char> buffer)
{
    ...
    win::ScopedHandle file(
        CreateFile(filename.value().c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL));
    if (!file.is_valid()) {
        return std::nullopt;
    }
    ...
    DWORD bytes_read;
    if (!::ReadFile(file.get(), buffer.data(), bytes_to_read, &bytes_read, nullptr)) {
        return std::nullopt;
    }
    return bytes_read;
}
...
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    win::ScopedHandle file(CreateFile(filename.value().c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL));
    if (!file.is_valid()) {
    ...
    if (!result) {
        ...
        DLOG(WARNING) << "wrote" << written << " bytes to " << filename.value() << " expected " << size;
    }
    return false;
}
...
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    win::ScopedHandle file(CreateFile(filename.value().c_str(), FILE_APPEND_DATA, 0, nullptr, OPEN_EXISTING, 0, nullptr));
    if (!file.is_valid()) {
    ...
    if (!result) {
        ...
        // Didn't write all the bytes.
        VPLOG(1) << "Only wrote " << written << " out of " << size << " byte(s) to " << filename.value();
    }
    return false;
}
...
bool AppendToFile(const FilePath& filename, std::string_view data)
{
    return AppendToFile(filename, 
        //as_bytes(make_span(data))
        span<const uint8_t>((const uint8_t*)data.data(), data.size())
    );
}
...
int GetMaximumPathComponentLength(const FilePath& path)
{
    ...
    if (!GetVolumePathNameW(path.NormalizePathSeparators().value().c_str(), volume_path, std::size(volume_path))) {
        return -1;
    }
...
bool SetNonBlocking(int fd)
{
    ...
    if (ioctlsocket(static_cast<SOCKET>(fd), static_cast<long>(FIONBIO), &nonblocking) == 0)
        return true;
    return false;
}

bool PreReadFile(const FilePath& file_path, bool is_executable, bool sequential, int64_t max_bytes)
{
    ...
    MemoryMappedFile::Access access = is_executable ? MemoryMappedFile::READ_CODE_IMAGE : MemoryMappedFile::READ_ONLY;
    MemoryMappedFile mapped_file;
    if (!mapped_file.Initialize(file_path, access)) {
        return false;
    }

    const ::SIZE_T length = std::min(base::saturated_cast<::SIZE_T>(max_bytes), base::saturated_cast<::SIZE_T>(mapped_file.length()));
...
bool PreventExecuteMappingUnchecked(const FilePath& path, base::PassKey<PreventExecuteMappingClasses> passkey)
{
...
{
    DCHECK(!g_extra_allowed_path_for_no_execute || g_extra_allowed_path_for_no_execute == path_key);
    g_extra_allowed_path_for_no_execute = path_key;
    base::FilePath valid_path;
    DCHECK(base::PathService::Get(g_extra_allowed_path_for_no_execute, &valid_path));
}
