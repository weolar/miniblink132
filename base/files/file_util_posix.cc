...
bool VerifySpecificPathControlledByUser(const FilePath& path, uid_t owner_uid, const std::set<gid_t>& group_gids)
{
...
bool AdvanceEnumeratorWithStat(FileEnumerator* traversal, FilePath* out_next_path, stat_wrapper_t* out_next_stat)
{
...
bool DoCopyDirectory(const FilePath& from_path, const FilePath& to_path, bool recursive, bool open_exclusive)
{
...
bool DoDeleteFile(const PlatformFile at_fd, const char* const path, const bool recursive)
{
    ...
    if (recursive) {
        ...
        // Open the directory.
        const PlatformFile fd = HANDLE_EINTR(openat(at_fd, path, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC));
        if (fd < 0) {
        ...
        while (true) {
            ...
            if (const std::string_view s = entry->d_name; s != "." && s != ".." && !DoDeleteFile(fd, entry->d_name, true)) {
                return false;
            }
        }
...
#if !BUILDFLAG(IS_LINUX) && !BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_APPLE) && !(BUILDFLAG(IS_ANDROID) && __ANDROID_API__ >= 21)
bool PreReadFileSlow(const FilePath& file_path, int64_t max_bytes)
{
    ...
    while (max_bytes > 0) {
        const size_t read_size = base::checked_cast<size_t>(std::min<uint64_t>(static_cast<uint64_t>(max_bytes), buffer.size()));
        std::optional<size_t> read_bytes = file.ReadAtCurrentPos(buffer.first(read_size));
...
std::optional<FilePath> MakeAbsoluteFilePathNoResolveSymbolicLinks(const FilePath& input)
{
    if (input.empty()) {
        return std::nullopt;
    }
...
ScopedFD CreateAndOpenFdForTemporaryFileInDir(const FilePath& directory, FilePath* path)
{
...
bool CreateSymbolicLink(const FilePath& target_path, const FilePath& symlink_path)
{
...
bool ExecutableExistsInPath(Environment* env, const FilePath::StringType& executable)
{
    std::string path;
    ...
    for (std::string_view cur_path : SplitStringPiece(path, ":", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
...
static bool CreateTemporaryDirInDirImpl(const FilePath& base_dir, const FilePath& name_tmpl, FilePath* new_dir)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK); // For call to mkdtemp().
    DCHECK(EndsWith(name_tmpl.value(), "XXXXXX")) << "Directory name template must end with \"XXXXXX\".";
...
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK); // For call to mkdir().
    std::vector<FilePath> subpaths;

    // Collect a list of all parent directories.
    FilePath last_path = full_path;
    subpaths.push_back(full_path);
    for (FilePath path = full_path.DirName(); path.value() != last_path.value(); path = path.DirName()) {
...
FILE* OpenFile(const FilePath& filename, const char* mode)
{
    ...
    DCHECK(strchr(mode, 'e') == nullptr || (strchr(mode, ',') != nullptr && strchr(mode, 'e') > strchr(mode, ',')));
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    FILE* result = nullptr;
#if BUILDFLAG(IS_APPLE)
...
{
    while (!data.empty()) {
        ssize_t bytes_written_partial = HANDLE_EINTR(write(fd, data.data(), data.size()));
...
bool AllocateFileRegion(File* file, int64_t offset, size_t size)
{
    ...
    // (though not UB).
    if (!IsValueInRangeForNumericType<int64_t>(size) || !IsValueInRangeForNumericType<off_t>(size) || !IsValueInRangeForNumericType<off_t>(new_file_len)
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    if (HANDLE_EINTR(fallocate(file->GetPlatformFile(), 0, offset, static_cast<off_t>(size))) != -1) {
        return true;
    }
    ...
    const int64_t extension_start = checked_cast<int64_t>(base::bits::AlignUp(static_cast<size_t>(original_file_len), static_cast<size_t>(block_size)));
    for (int64_t i = extension_start; i < new_file_len; i += block_size) {
        char existing_byte;
        if (HANDLE_EINTR(pread(file->GetPlatformFile(), &existing_byte, 1, static_cast<off_t>(i))) != 1) {
            return false; // Can't read? Not viable.
        }
        if (existing_byte != 0) {
            continue; // Block has data so must already exist.
        }
        if (HANDLE_EINTR(pwrite(file->GetPlatformFile(), &existing_byte, 1, static_cast<off_t>(i))) != 1) {
...
#if BUILDFLAG(IS_MAC)
bool VerifyPathControlledByUser(const FilePath& base, const FilePath& path, uid_t owner_uid, const std::set<gid_t>& group_gids)
{
    if (base != path && !base.IsParent(path)) {
        DLOG(ERROR) << "|base| must be a subdirectory of |path|.  base = \"" << base.value() << "\", path = \"" << path.value() << "\"";
        return false;
    }
    ...
    for (ib = base_components.begin(), ip = path_components.begin(); ib != base_components.end(); ++ib, ++ip) {
...
bool PreReadFile(const FilePath& file_path, bool is_executable, bool sequential, int64_t max_bytes)
{
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || (BUILDFLAG(IS_ANDROID) && __ANDROID_API__ >= 21)
    File file(file_path, File::FLAG_OPEN | File::FLAG_READ);
    if (!file.IsValid()) {
        return false;
    }
    ...
    const PlatformFile fd = file.GetPlatformFile();
    ::radvisory read_advise_data = { .ra_offset = 0, .ra_count = base::saturated_cast<int>(max_bytes) };
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)
bool CopyFileContentsWithSendfile(File& infile, File& outfile, bool& retry_slow)
{
    ...
    do {
        ...
        res = HANDLE_EINTR(sendfile(outfile.GetPlatformFile(), infile.GetPlatformFile(), /*offset=*/nullptr,
