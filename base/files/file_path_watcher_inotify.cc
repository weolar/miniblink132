...
public:
    ...
    InotifyReaderThreadDelegate& operator=(const InotifyReaderThreadDelegate&) = delete;
...
private:
    ...
    std::unordered_map<Watch, std::map<FilePathWatcherImpl*, WatcherEntry>> watchers_ GUARDED_BY(lock_);
...
public:
    ...
    void OnFilePathChanged(
        InotifyReader::Watch fired_watch, const FilePath::StringType& child, FilePathWatcher::ChangeInfo change_info, bool created, bool deleted);
...
private:
    ...
    [[nodiscard]] bool UpdateRecursiveWatches(InotifyReader::Watch fired_watch, bool is_dir);
    ...
    // Returns true if watch limit is not hit. Otherwise, returns false.
    [[nodiscard]] bool AddWatchForBrokenSymlink(const FilePath& path, WatchEntry* watch_entry);
...
void InotifyReaderThreadDelegate::ThreadMain()
{
    ...
    while (true) {
        ...
        ssize_t bytes_read = HANDLE_EINTR(read(inotify_fd_, buffer.data(), static_cast<size_t>(buffer_size)));
...
InotifyReader::Watch InotifyReader::AddWatch(const FilePath& path, FilePathWatcherImpl* watcher)
{
    if (!valid_)
    ...
    const int watch_int = inotify_add_watch(inotify_fd_, path.value().c_str(), IN_ATTRIB | IN_CREATE | IN_DELETE | IN_CLOSE_WRITE | IN_MOVE | IN_ONLYDIR);
    if (watch_int == -1)
        return kInvalidWatch;
...
void InotifyReader::OnInotifyEvent(const inotify_event* event)
{
    ...
    for (const auto& entry : watcher_map) {
        ...
        FilePathWatcher::ChangeInfo change_info {
            .file_path_type = event->mask & IN_ISDIR ? FilePathWatcher::FilePathType::kDirectory : FilePathWatcher::FilePathType::kFile,
            .change_type = ToChangeType(event),
            .cookie = event->cookie ? std::make_optional(event->cookie) : std::nullopt,
        };
        ...
        watcher_entry.task_runner->PostTask(FROM_HERE,
            BindOnce(&FilePathWatcherImpl::OnFilePathChanged, watcher_entry.watcher, static_cast<Watch>(event->wd), child, std::move(change_info), created,
                deleted));
    }
}
...
{
    DUMP_WILL_BE_CHECK(!task_runner() || task_runner()->RunsTasksInCurrentSequence());
}
    ...
    InotifyReader::Watch fired_watch, const FilePath::StringType& child, FilePathWatcher::ChangeInfo change_info, bool created, bool deleted)
{
    ...
    for (size_t i = 0; i < watches_.size(); ++i) {
        ...
        if (target_changed || (change_on_target_path && deleted) || (change_on_target_path && created && PathExists(target_))) {
            if (!did_update) {
                if (!UpdateRecursiveWatches(fired_watch, change_info.file_path_type == FilePathWatcher::FilePathType::kDirectory)) {
                    exceeded_limit = true;
                    break;
                }
                did_update = true;
            }
            FilePath modified_path = report_modified_path_ && !change_on_target_path ? target_.Append(child) : target_;
    ...
    if (!exceeded_limit && Contains(recursive_paths_by_watch_, fired_watch)) {
        ...
        if (!exceeded_limit) {
            FilePath modified_path = report_modified_path_ ? recursive_paths_by_watch_[fired_watch].Append(child) : target_;
            callback_.Run(std::move(change_info), modified_path,
                /*error=*/false); // `this` may be deleted.
            return;
        }
...
bool FilePathWatcherImpl::UpdateRecursiveWatches(InotifyReader::Watch fired_watch, bool is_dir)
{
    ...
    const FilePath& changed_dir = Contains(recursive_paths_by_watch_, fired_watch) ? recursive_paths_by_watch_[fired_watch] : target_;
...
bool FilePathWatcherImpl::UpdateRecursiveWatchesForPath(const FilePath& path)
{
    ...
    FileEnumerator enumerator(path, true /* recursive enumeration */, FileEnumerator::DIRECTORIES | FileEnumerator::SHOW_SYM_LINKS);
...
void FilePathWatcherImpl::TrackWatchForRecursion(InotifyReader::Watch watch, const FilePath& path)
{
    DUMP_WILL_BE_CHECK_EQ(type_, Type::kRecursive);
    DUMP_WILL_BE_CHECK(!path.empty());
...
bool FilePathWatcherImpl::AddWatchForBrokenSymlink(const FilePath& path, WatchEntry* watch_entry)
{
#if BUILDFLAG(IS_FUCHSIA)
...
ScopedMaxNumberOfInotifyWatchesOverrideForTest::ScopedMaxNumberOfInotifyWatchesOverrideForTest(size_t override_max)
{
