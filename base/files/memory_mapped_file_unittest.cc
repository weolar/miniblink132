...
protected:
    ...
    {
        EXPECT_TRUE(DeleteFile(temp_file_path_));
    }

    void CreateTemporaryTestFile(size_t size)
    {
        File file(temp_file_path_, File::FLAG_CREATE_ALWAYS | File::FLAG_READ | File::FLAG_WRITE);
...
TEST_F(MemoryMappedFileTest, MapWholeFileByFD)
{
    ...
    CreateTemporaryTestFile(kFileSize);
    MemoryMappedFile map;
    ASSERT_TRUE(map.Initialize(File(temp_file_path(), File::FLAG_OPEN | File::FLAG_READ)));
    ASSERT_EQ(kFileSize, map.length());
    ASSERT_TRUE(map.data() != nullptr);
    EXPECT_TRUE(map.IsValid());
    ASSERT_TRUE(CheckBufferContents(map.bytes(), 0));
...
TEST_F(MemoryMappedFileTest, MapWholeFileUsingRegion)
{
    ...
    ASSERT_TRUE(map.Initialize(std::move(file), MemoryMappedFile::Region::kWholeFile));
    ASSERT_EQ(kFileSize, map.length());
    ASSERT_TRUE(map.data() != nullptr);
    EXPECT_TRUE(map.IsValid());
    ASSERT_TRUE(CheckBufferContents(map.bytes(), 0));
...
TEST_F(MemoryMappedFileTest, WriteableFile)
{
...

    {
        ...
        EXPECT_TRUE(CheckBufferContents(map.bytes().first(kFileSize - 1).subspan(3), 3));
    }
...
TEST_F(MemoryMappedFileTest, CopyOnWrite)
{
    ...
    {
        MemoryMappedFile map;
        ASSERT_TRUE(map.Initialize(temp_file_path(), MemoryMappedFile::READ_WRITE_COPY));
        ASSERT_EQ(kFileSize, map.length());
        ASSERT_TRUE(map.data() != nullptr);
        EXPECT_TRUE(map.IsValid());
        ASSERT_TRUE(CheckBufferContents(map.bytes(), 0));
...
TEST_F(MemoryMappedFileTest, ExtendableFile)
{
    ...
    {
        File file(temp_file_path(), File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WRITE);
        MemoryMappedFile::Region region = { 0, kFileSize + kFileExtend };
        MemoryMappedFile map;
        ASSERT_TRUE(map.Initialize(std::move(file), region, MemoryMappedFile::READ_WRITE_EXTEND));
...
TEST_F(MemoryMappedFileTest, ReadCodeImage)
{
    ...
    File file(exe_path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_SHARE_DELETE);
    ASSERT_TRUE(file.IsValid());
    MemoryMappedFile map;
    ASSERT_TRUE(map.Initialize(std::move(file), MemoryMappedFile::READ_CODE_IMAGE));
}
#endif // BUILDFLAG(IS_WIN)
