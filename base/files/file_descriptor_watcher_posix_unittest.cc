...
public:
    FileDescriptorWatcherTest()
        : task_environment_(std::make_unique<test::TaskEnvironment>(GetParam() == FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_MAIN_THREAD
    ...
    FileDescriptorWatcherTest& operator=(const FileDescriptorWatcherTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        if (GetParam() == FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_OTHER_THREAD) {
            ...
            file_descriptor_watcher_ = std::make_unique<FileDescriptorWatcher>(other_thread_.task_runner());
        }
    }

    void TearDown() override
    {
        if (GetParam() == FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_MAIN_THREAD && task_environment_) {
...
protected:
    int read_file_descriptor() const
    {
        return pipe_fds_[0];
    }
    int write_file_descriptor() const
    {
        return pipe_fds_[1];
    }

    int read_file_descriptor2() const
    {
        return pipe_fds2_[0];
    }
    int write_file_descriptor2() const
    {
        return pipe_fds2_[1];
    }
    int& read_file_descriptor2_ref()
    {
        return pipe_fds2_[0];
    }
    int& write_file_descriptor2_ref()
    {
        return pipe_fds2_[1];
    }
    ...
    std::unique_ptr<FileDescriptorWatcher::Controller> WatchReadable()
    {
        std::unique_ptr<FileDescriptorWatcher::Controller> controller
            = FileDescriptorWatcher::WatchReadable(read_file_descriptor(), BindRepeating(&Mock::ReadableCallback, Unretained(&mock_)));
    ...
    {
        std::unique_ptr<FileDescriptorWatcher::Controller> controller
            = FileDescriptorWatcher::WatchWritable(write_file_descriptor(), BindRepeating(&Mock::WritableCallback, Unretained(&mock_)));
        EXPECT_TRUE(controller);
        return controller;
    }
    ...
    {
        std::unique_ptr<FileDescriptorWatcher::Controller> controller
            = FileDescriptorWatcher::WatchReadable(read_file_descriptor2(), BindRepeating(&Mock::ReadableCallback2, Unretained(&mock_)));
        EXPECT_TRUE(controller);
    ...
    {
        std::unique_ptr<FileDescriptorWatcher::Controller> controller
            = FileDescriptorWatcher::WatchWritable(write_file_descriptor2(), BindRepeating(&Mock::WritableCallback2, Unretained(&mock_)));
        EXPECT_TRUE(controller);
        return controller;
    }
    ...
    void WriteByte()
    {
        ...
        ASSERT_TRUE(WriteFileDescriptor(write_file_descriptor(), byte_span_from_ref(kByte)));
    }
    ...
    {
        constexpr char kByte = '!';
        ASSERT_TRUE(WriteFileDescriptor(write_file_descriptor2(), byte_span_from_ref(kByte)));
    }
...
TEST_P(FileDescriptorWatcherTest, WatchWritable)
{
    ...
    RunLoop run_loop;
    EXPECT_CALL(mock_, WritableCallback()).WillOnce(testing::Invoke(&run_loop, &RunLoop::Quit));
    run_loop.Run();
}
...
TEST_P(FileDescriptorWatcherTest, WatchReadableOneByte)
{
    ...
    WriteByte();
    RunLoop run_loop;
    EXPECT_CALL(mock_, ReadableCallback()).WillOnce(testing::Invoke([this, &run_loop] {
        ReadByte();
        run_loop.Quit();
    }));
    run_loop.Run();
...
TEST_P(FileDescriptorWatcherTest, WatchReadableTwoBytes)
{
    ...
    WriteByte();
    RunLoop run_loop;
    EXPECT_CALL(mock_, ReadableCallback()).WillOnce(testing::Invoke([this] { ReadByte(); })).WillOnce(testing::Invoke([this, &run_loop] {
        ReadByte();
        run_loop.Quit();
    }));
    run_loop.Run();
...
TEST_P(FileDescriptorWatcherTest, DeleteControllerFromCallback)
{
    ...
    WriteByte();
    RunLoop run_loop;
    EXPECT_CALL(mock_, ReadableCallback()).WillOnce(testing::Invoke([&run_loop, &controller] {
        controller = nullptr;
        run_loop.Quit();
    }));
    run_loop.Run();
...
TEST_P(FileDescriptorWatcherTest, DeleteControllerBeforeFileDescriptorReadable)
{
    auto controller = WatchReadable();
...
TEST_P(FileDescriptorWatcherTest, DeleteControllerAfterDeleteMessagePumpForIO)
{
    ...
    if (GetParam() == FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_MAIN_THREAD) {
...
TEST_P(FileDescriptorWatcherTest, WatchReadableOneByteAndWatchReadableTwoBytesOnFd2ClosedInTheMiddle)
{
    auto controller = WatchReadable();
    ...
    RunLoop run_loop;
    EXPECT_CALL(mock_, ReadableCallback()).WillOnce(testing::Invoke([this, &controller] {
        ReadByte();
        CloseWriteFd2();
        controller.reset();
    }));
    EXPECT_CALL(mock_, ReadableCallback2()).WillOnce(testing::Invoke([this] { ReadByte2(); })).WillOnce(testing::Invoke([this, &controller2, &run_loop] {
        ReadByte2();
        controller2.reset();
        run_loop.Quit();
    }));
...
INSTANTIATE_TEST_SUITE_P(
    MessagePumpForIOOnMainThread, FileDescriptorWatcherTest, ::testing::Values(FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_MAIN_THREAD));
INSTANTIATE_TEST_SUITE_P(
    MessagePumpForIOOnOtherThread, FileDescriptorWatcherTest, ::testing::Values(FileDescriptorWatcherTestType::MESSAGE_PUMP_FOR_IO_ON_OTHER_THREAD));
