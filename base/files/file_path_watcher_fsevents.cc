    ...
    while (resolve_count < kMaxLinksToResolve && !components.empty()) {
        ...
        if (ReadSymbolicLink(current, &target)) {
            if (target.IsAbsolute())
                result.clear();
            std::vector<FilePath::StringType> target_components = target.GetComponents();
            components.insert(components.begin(), target_components.begin(), target_components.end());
            resolve_count++;
        } else {
            result = current;
        }
...
FilePathWatcherFSEvents::FilePathWatcherFSEvents()
    : queue_(dispatch_queue_create(base::StringPrintf("org.chromium.base.FilePathWatcher.%p", this).c_str(), DISPATCH_QUEUE_SERIAL))
{
}
...
FilePathWatcherFSEvents::~FilePathWatcherFSEvents()
{
    ...
    DCHECK(callback_.is_null()) << "Cancel() must be called before FilePathWatcher is destroyed.";
}

bool FilePathWatcherFSEvents::Watch(const FilePath& path, Type type, const FilePathWatcher::Callback& callback)
{
...
// static
void FilePathWatcherFSEvents::FSEventsCallback(ConstFSEventStreamRef stream, void* event_watcher, size_t num_events, void* event_paths,
    const FSEventStreamEventFlags flags[], const FSEventStreamEventId event_ids[])
{
    ...
    for (size_t i = 0; i < num_events; i++) {
        ...
        paths.push_back(FilePath(reinterpret_cast<char**>(event_paths)[i]).StripTrailingSeparators());
    }
    ...
    if (root_changed) {
        ...
        watcher->task_runner()->PostTask(FROM_HERE,
            BindOnce(
                [](WeakPtr<FilePathWatcherFSEvents> weak_watcher, FSEventStreamEventId root_change_at) {
                    if (!weak_watcher)
                        return;
...
void FilePathWatcherFSEvents::OnFilePathsChanged(const std::vector<FilePath>& paths)
{
    DCHECK(!resolved_target_.empty());
    task_runner()->PostTask(FROM_HERE, BindOnce(&FilePathWatcherFSEvents::DispatchEvents, weak_factory_.GetWeakPtr(), paths, target_, resolved_target_));
}

void FilePathWatcherFSEvents::DispatchEvents(const std::vector<FilePath>& paths, const FilePath& target, const FilePath& resolved_target)
{
...
void FilePathWatcherFSEvents::UpdateEventStream(FSEventStreamEventId start_event)
{
    ...
    apple::ScopedCFTypeRef<CFStringRef> cf_path = apple::FilePathToCFString(resolved_target_);
    apple::ScopedCFTypeRef<CFStringRef> cf_dir_path = apple::FilePathToCFString(resolved_target_.DirName());
    CFStringRef paths_array[] = { cf_path.get(), cf_dir_path.get() };
    apple::ScopedCFTypeRef<CFArrayRef> watched_paths(
        CFArrayCreate(NULL, reinterpret_cast<const void**>(paths_array), std::size(paths_array), &kCFTypeArrayCallBacks));
    ...
    fsevent_stream_
        = FSEventStreamCreate(NULL, &FSEventsCallback, &context, watched_paths.get(), start_event, kEventLatencySeconds, kFSEventStreamCreateFlagWatchRoot);
    FSEventStreamSetDispatchQueue(fsevent_stream_, queue_.get());

    if (!FSEventStreamStart(fsevent_stream_)) {
        task_runner()->PostTask(FROM_HERE, BindOnce(&FilePathWatcherFSEvents::ReportError, weak_factory_.GetWeakPtr(), target_));
    }
}
...
void FilePathWatcherFSEvents::StartEventStream(FSEventStreamEventId start_event, const FilePath& path)
{
    DCHECK(resolved_target_.empty());
