...
public:
    ...
    using ReadCallback = OnceCallback<void(File::Error, base::span<const char> data)>;
    ...
    bool CreateOrOpen(const FilePath& file_path, uint32_t file_flags, StatusCallback callback);
    ...
    //
    // This returns false if task posting to |task_runner| has failed.
    bool CreateTemporary(uint32_t additional_file_flags, CreateTemporaryCallback callback);
    ...
    bool Write(int64_t offset, base::span<const uint8_t> data, WriteCallback callback);

    // Proxies File::SetTimes. The callback can be null.
    // This returns false if task posting to |task_runner| has failed.
    bool SetTimes(Time last_access_time, Time last_modified_time, StatusCallback callback);
