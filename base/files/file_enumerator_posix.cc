...
bool GetStat(const FilePath& path, bool show_links, stat_wrapper_t* st)
{
    ...
    if (res < 0) {
        ...
        // symlinks.
        DPLOG_IF(ERROR, errno != ENOENT || show_links) << "Cannot stat '" << path << "'";
...
#if BUILDFLAG(IS_ANDROID)
FileEnumerator::FileInfo::FileInfo(base::FilePath content_uri, base::FilePath filename, bool is_directory, off_t size, Time time)
    : content_uri_(std::move(content_uri))
...
FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive, int file_type)
    : FileEnumerator(root_path, recursive, file_type, FilePath::StringType(), FolderSearchPolicy::MATCH_ONLY)
{
}

FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive, int file_type, const FilePath::StringType& pattern)
...
FileEnumerator::FileEnumerator(
    ...
    : FileEnumerator(root_path, recursive, file_type, pattern, folder_search_policy, ErrorPolicy::IGNORE_ERRORS)
{
}

FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive, int file_type, const FilePath::StringType& pattern,
    FolderSearchPolicy folder_search_policy, ErrorPolicy error_policy)
    ...
    , error_policy_(error_policy)
{
    ...
    if (root_path.IsContentUri()) {
        ...
        pending_subdirs_.push(std::vector<std::string>({ root_info.GetName().value() }));
    }
#endif

    if (file_type_ & FileType::NAMES_ONLY) {
        DCHECK(!recursive_);
        DCHECK_EQ(file_type_ & ~(FileType::NAMES_ONLY | FileType::INCLUDE_DOT_DOT), 0);
...
FilePath FileEnumerator::Next()
{
    ...
    while (current_directory_entry_ >= directory_entries_.size()) {
        ...
        while (errno = 0, dent = readdir(dir)) {
            ...
            if (recursive_ && is_dir && (!ShouldTrackVisitedDirectories(file_type_) || MarkVisited(info.stat_))) {
