...
bool MemoryMappedFile::MapFileRegionToMemory(const MemoryMappedFile::Region& region, Access access)
{
    ...
    if (region == MemoryMappedFile::Region::kWholeFile) {
        ...
        CalculateVMAlignedBoundaries(region.offset, region.size, &aligned_start, &aligned_size, &data_offset);
    ...
    auto* ptr = static_cast<uint8_t*>(mmap(nullptr, map_size, prot, flags, file_.GetPlatformFile(), map_start));
    if (ptr == MAP_FAILED) {
