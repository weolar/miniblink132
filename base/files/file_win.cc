...
static_assert(
    File::FROM_BEGIN == FILE_BEGIN && File::FROM_CURRENT == FILE_CURRENT && File::FROM_END == FILE_END, "whence mapping must match the system headers");
...
int File::Read(int64_t offset, char* data, int size)
{
    ...
    DWORD bytes_read;
    if (::ReadFile(file_.get(), data, static_cast<DWORD>(size), &bytes_read, &overlapped)) {
...
int File::ReadAtCurrentPos(char* data, int size)
{
    ...
    DWORD bytes_read;
    if (::ReadFile(file_.get(), data, static_cast<DWORD>(size), &bytes_read, NULL)) {
        // TODO(crbug.com/40227936): Change to return some type with a uint64_t size
        // and eliminate this cast.
        return checked_cast<int>(bytes_read);
    }
...
int File::WriteAtCurrentPos(const char* data, int size)
{
    ...
    DWORD bytes_written;
    if (::WriteFile(file_.get(), data, static_cast<DWORD>(size), &bytes_written, NULL))
        return static_cast<int>(bytes_written);
...
bool File::SetTimes(Time last_access_time, Time last_modified_time)
{
    ...
    return (::SetFileTime(file_.get(), NULL, &last_access_filetime, &last_modified_filetime) != FALSE);
}
...
bool File::DeleteOnClose(bool delete_on_close)
{
    ...
    return ::SetFileInformationByHandle(GetPlatformFile(), FileDispositionInfo, &disposition, sizeof(disposition)) != 0;
}
...
void File::DoInitialize(const FilePath& path, uint32_t flags)
{
    ...
    file_.Set(CreateFile(path.value().c_str(), access, sharing, NULL, disposition, create_flags, NULL));
    ...
    if (file_.is_valid()) {
        ...
        if (flags & FLAG_WIN_NO_EXECUTE) {
            ...
            // passed to renderers.
            DCHECK(!base::FilePath::CompareEqualIgnoreCase(FILE_PATH_LITERAL(".exe"), path.Extension()));
            DCHECK(!base::FilePath::CompareEqualIgnoreCase(FILE_PATH_LITERAL(".dll"), path.Extension()));
