...
TEST(FileTest, Create)
{
...

    {
        ...
        File file(file_path, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
        EXPECT_TRUE(file.IsValid());
        EXPECT_TRUE(file.created());
        EXPECT_EQ(base::File::FILE_OK, file.error_details());
    }

    {
        ...
        File file(file_path, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_READ | base::File::FLAG_DELETE_ON_CLOSE);
        EXPECT_TRUE(file.IsValid());
        EXPECT_TRUE(file.created());
        EXPECT_EQ(base::File::FILE_OK, file.error_details());
    }
...
TEST(FileTest, SelfSwap)
{
    ...
    ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
    FilePath file_path = temp_dir.GetPath().AppendASCII("create_file_1");
    File file(file_path, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_DELETE_ON_CLOSE);
    std::swap(file, file);
    EXPECT_TRUE(file.IsValid());
}
...
TEST(FileTest, DeleteOpenFile)
{
    ...
    // Create a file.
    File file(file_path, base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_READ | base::File::FLAG_WIN_SHARE_DELETE);
    EXPECT_TRUE(file.IsValid());
    EXPECT_TRUE(file.created());
    EXPECT_EQ(base::File::FILE_OK, file.error_details());

    // Open an existing file and mark it as delete on close.
    File same_file(file_path, base::File::FLAG_OPEN | base::File::FLAG_DELETE_ON_CLOSE | base::File::FLAG_READ);
    EXPECT_TRUE(file.IsValid());
...
TEST(FileTest, ReadWrite)
{
    ...
    std::optional<size_t> maybe_bytes_read = file.ReadNoBestEffort(0, base::as_writable_byte_span(data_read_1).first(static_cast<size_t>(kTestDataSize)));
    ...
    bytes_written = file.Write(kOffsetBeyondEndOfFile, data_to_write, kPartialWriteLength);
...
TEST(FileTest, ReadWriteSpans)
{
    ...
    std::optional<size_t> bytes_read = file.Read(bytes_written.value(), data_read_1);
    ...
    bytes_written = file.Write(kOffsetBeyondEndOfFile, base::as_byte_span(data_to_write).first(kPartialWriteLength));
    ASSERT_TRUE(bytes_written.has_value());
    ...
    std::optional<int64_t> file_size = GetFileSize(file_path);
    ASSERT_TRUE(file_size.has_value());
    EXPECT_EQ(static_cast<int64_t>(kOffsetBeyondEndOfFile + kPartialWriteLength), file_size.value());
...
TEST(FileTest, Append)
{
    ...
    file.Close();
    File file2(file_path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_APPEND);
...
TEST(FileTest, Length)
{
    ...
    file.Close();
    file.Initialize(file_path, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    EXPECT_EQ(0, file.GetLength());
}
...
TEST(FileTest, DISABLED_TouchGetInfo)
{
    ...
    base::ScopedTempDir temp_dir;
    ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
    File file(temp_dir.GetPath().AppendASCII("touch_get_info_file"), base::File::FLAG_CREATE | base::File::FLAG_WRITE | base::File::FLAG_WRITE_ATTRIBUTES);
    ASSERT_TRUE(file.IsValid());
    ...
    EXPECT_EQ(info.last_accessed.ToTimeVal().tv_sec, new_last_accessed.ToTimeVal().tv_sec);
    EXPECT_EQ(info.last_modified.ToTimeVal().tv_sec, new_last_modified.ToTimeVal().tv_sec);
#else
    EXPECT_EQ(info.last_accessed.ToInternalValue(), new_last_accessed.ToInternalValue());
    EXPECT_EQ(info.last_modified.ToInternalValue(), new_last_modified.ToInternalValue());
#endif

    EXPECT_EQ(info.creation_time.ToInternalValue(), creation_time.ToInternalValue());
}

// Test we can retrieve the file's creation time through File::GetInfo().
TEST(FileTest, GetInfoForCreationTime)
{
    int64_t before_creation_time_s = base::Time::Now().ToDeltaSinceWindowsEpoch().InSeconds();
    ...
    int64_t after_creation_time_s = base::Time::Now().ToDeltaSinceWindowsEpoch().InSeconds();

    base::File::Info info;
    EXPECT_TRUE(file.GetInfo(&info));
    EXPECT_GE(info.creation_time.ToDeltaSinceWindowsEpoch().InSeconds(), before_creation_time_s);
    EXPECT_LE(info.creation_time.ToDeltaSinceWindowsEpoch().InSeconds(), after_creation_time_s);
}
...
TEST(FileTest, ReadAtCurrentPosition)
{
    base::ScopedTempDir temp_dir;
    ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
    FilePath file_path = temp_dir.GetPath().AppendASCII("read_at_current_position");
    File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE);
    EXPECT_TRUE(file.IsValid());
    ...
    EXPECT_EQ(kDataSize - first_chunk_size, file.ReadAtCurrentPos(buffer + first_chunk_size, kDataSize - first_chunk_size));
...
TEST(FileTest, ReadAtCurrentPositionSpans)
{
    ...
    result = file.ReadAtCurrentPos(base::make_span(buffer).first(first_chunk_size));
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(first_chunk_size, result.value());

    result = file.ReadAtCurrentPos(base::make_span(buffer).subspan(first_chunk_size));
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(first_chunk_size, result.value());
...
TEST(FileTest, WriteAtCurrentPosition)
{
    base::ScopedTempDir temp_dir;
    ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
    FilePath file_path = temp_dir.GetPath().AppendASCII("write_at_current_position");
    File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE);
    EXPECT_TRUE(file.IsValid());
    ...
    EXPECT_EQ(kDataSize - first_chunk_size, file.WriteAtCurrentPos(kData + first_chunk_size, kDataSize - first_chunk_size));
...
TEST(FileTest, WriteAtCurrentPositionSpans)
{
    ...
    std::optional<size_t> result = file.WriteAtCurrentPos(base::as_byte_span(data).first(first_chunk_size));
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(first_chunk_size, result.value());

    result = file.WriteAtCurrentPos(base::as_byte_span(data).subspan(first_chunk_size));
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(first_chunk_size, result.value());
...
TEST(FileTest, TracedValueSupport)
{
    ...
    EXPECT_EQ(perfetto::TracedValueToString(file), "{is_valid:true,created:true,async:false,error_details:FILE_OK}");
}
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
...
TEST(FileTest, AddFlagsForPassingToUntrustedProcess)
{
    ...
    }
    {
        ...
        flags = base::File::AddFlagsForPassingToUntrustedProcess(flags);
        EXPECT_EQ(flags, base::File::FLAG_OPEN | base::File::FLAG_WRITE | base::File::FLAG_WIN_NO_EXECUTE);
    }
}
...
TEST(FileTest, GetInfoForDirectory)
{
    base::ScopedTempDir temp_dir;
    ASSERT_TRUE(temp_dir.CreateUniqueTempDir());
    FilePath empty_dir = temp_dir.GetPath().Append(FILE_PATH_LITERAL("gpfi_test"));
    ASSERT_TRUE(CreateDirectory(empty_dir));

    base::File dir(
        ::CreateFile(empty_dir.value().c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
...
TEST(FileTest, DeleteNoop)
{
    ...
    File file(file_path, (base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_CAN_DELETE_ON_CLOSE));
    ASSERT_TRUE(file.IsValid());
    file.Close();
...
TEST(FileTest, IrrevokableDeleteOnClose)
{
    ...
    File file(file_path,
        (base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_DELETE_ON_CLOSE | base::File::FLAG_WIN_SHARE_DELETE
...
TEST(FileTest, IrrevokableDeleteOnCloseOther)
{
    ...
    File file2(file_path,
        (base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_WIN_SHARE_DELETE | base::File::FLAG_CAN_DELETE_ON_CLOSE));
    ASSERT_TRUE(file2.IsValid());
...
TEST(FileTest, UnsharedDeleteOnClose)
{
    ...
    ASSERT_TRUE(file.IsValid());
    File file2(file_path,
        (base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_DELETE_ON_CLOSE | base::File::FLAG_WIN_SHARE_DELETE));
...
TEST(FileTest, UseSyncApiWithAsyncFile)
{
    ...
    File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_WRITE | base::File::FLAG_ASYNC);
    ...
    EXPECT_CHECK_DEATH_WITH(
        {
            ...
            base::ScopedTempDir temp_dir;
            ASSERT_TRUE(temp_dir.CreateUniqueTempDirUnderPath(base::FilePath(base::UTF8ToWide(tmp_folder))));
            FilePath file_path = temp_dir.GetPath().AppendASCII("file");

            File file(file_path, base::File::FLAG_CREATE | base::File::FLAG_WIN_EXECUTE | base::File::FLAG_READ | base::File::FLAG_WIN_NO_EXECUTE);
        },
        "FLAG_WIN_NO_EXECUTE");
}
