...
[[nodiscard]] BASE_EXPORT std::optional<FilePath> MakeAbsoluteFilePathNoResolveSymbolicLinks(const FilePath& input);
#endif
...
BASE_EXPORT OnceClosure GetDeleteFileCallback(const FilePath& path, OnceCallback<void(bool)> reply_callback = {});
BASE_EXPORT OnceClosure GetDeletePathRecursivelyCallback(const FilePath& path, OnceCallback<void(bool)> reply_callback = {});
...
BASE_EXPORT bool PreventExecuteMappingUnchecked(const FilePath& path, base::PassKey<PreventExecuteMappingClasses> passkey);
...
BASE_EXPORT bool CopyDirectory(const FilePath& from_path, const FilePath& to_path, bool recursive);

// Like CopyDirectory() except trying to overwrite an existing file will not
// work and will return false.
BASE_EXPORT bool CopyDirectoryExcl(const FilePath& from_path, const FilePath& to_path, bool recursive);
...
BASE_EXPORT std::optional<std::vector<uint8_t>> ReadFileToBytes(const FilePath& path);
...
// |contents| may be NULL, in which case this function is useful for its side
// effect of priming the disk cache (could be used for unit tests).
BASE_EXPORT bool ReadFileToStringWithMaxSize(const FilePath& path, std::string* contents, size_t max_size);
...
BASE_EXPORT bool ReadStreamToStringWithMaxSize(FILE* stream, size_t max_size, std::string* contents);
...
// appropriate.
BASE_EXPORT ScopedFD CreateAndOpenFdForTemporaryFileInDir(const FilePath& dir, FilePath* path);
...
// EWOULDBLOCK.
BASE_EXPORT bool ReadFileToStringNonBlocking(const base::FilePath& file, std::string* ret);

// Creates a symbolic link at |symlink| pointing to |target|.  Returns
// false on failure.
BASE_EXPORT bool CreateSymbolicLink(const FilePath& target, const FilePath& symlink);
...
// path.
BASE_EXPORT std::optional<FilePath> ReadSymbolicLinkAbsolute(const FilePath& symlink);
...
BASE_EXPORT bool ExecutableExistsInPath(Environment* env, const FilePath::StringType& executable);
...
BASE_EXPORT File CreateAndOpenTemporaryFileInDir(const FilePath& dir, FilePath* temp_file);
...
BASE_EXPORT FilePath FormatTemporaryFileName(FilePath::StringPieceType identifier);
...
BASE_EXPORT ScopedFILE CreateAndOpenTemporaryStreamInDir(const FilePath& dir, FilePath* path);
...
BASE_EXPORT bool CreateTemporaryDirInDir(const FilePath& base_dir, FilePath::StringPieceType prefix, FilePath* new_dir);
...
// Returns true on success, leaving *error unchanged.
// Returns false on failure and sets *error appropriately, if it is non-NULL.
BASE_EXPORT bool CreateDirectoryAndGetError(const FilePath& full_path, File::Error* error);
...
BASE_EXPORT OnceCallback<std::optional<int64_t>()> GetFileSizeCallback(const FilePath& path);
...
BASE_EXPORT bool DevicePathToDriveLetterPath(const FilePath& device_path, FilePath* drive_letter_path);
...
BASE_EXPORT bool CreateWinHardLink(const FilePath& to_file, const FilePath& from_file);
#endif
...
BASE_EXPORT std::optional<uint64_t> ReadFile(const FilePath& filename, span<char> buffer);
BASE_EXPORT std::optional<uint64_t> ReadFile(const FilePath& filename, span<uint8_t> buffer);
...
// |filename|.
BASE_EXPORT bool AppendToFile(const FilePath& filename, span<const uint8_t> data);
...
BASE_EXPORT FilePath GetUniquePathWithSuffixFormat(const FilePath& path, base::cstring_view suffix_format);
...
BASE_EXPORT bool PreReadFile(const FilePath& file_path, bool is_executable, bool sequential, int64_t max_bytes = std::numeric_limits<int64_t>::max());
...
BASE_EXPORT bool CreatePipe(ScopedFD* read_fd, ScopedFD* write_fd, bool non_blocking = false);
...
BASE_EXPORT bool VerifyPathControlledByUser(const base::FilePath& base, const base::FilePath& path, uid_t owner_uid, const std::set<gid_t>& group_gids);
...
BASE_EXPORT bool CopyAndDeleteDirectory(const FilePath& from_path, const FilePath& to_path);
#endif // BUILDFLAG(IS_WIN)
...
BASE_EXPORT bool CopyFileContentsWithSendfile(File& infile, File& outfile, bool& retry_slow);
