...
bool absl::str_format_internal::FormatUntyped(
...
void RunAndReply(OnceCallback<bool()> action_callback, OnceCallback<void(bool)> reply_callback)
{
...
bool ReadStreamToSpanWithMaxSize(FILE* stream, size_t max_size, FunctionRef<span<uint8_t>(size_t)> resize_span)
{
    if (!stream) {
        return false;
    }
    ...
    if (::GetFileInformationByHandle(reinterpret_cast<HANDLE>(_get_osfhandle(_fileno(stream))), &file_info)) {
        LARGE_INTEGER size;
    ...
    while ((bytes_read_this_pass = fread(bytes_span.data() + bytes_read_so_far, 1, chunk_size, stream)) > 0) {
...
OnceClosure GetDeleteFileCallback(const FilePath& path, OnceCallback<void(bool)> reply_callback)
{
    return BindOnce(&RunAndReply, BindOnce(&DeleteFile, path),
        reply_callback.is_null() ? std::move(reply_callback) : BindPostTask(SequencedTaskRunner::GetCurrentDefault(), std::move(reply_callback)));
}
...
bool CopyFileContents(File& infile, File& outfile)
{
    ...
    for (;;) {
        int bytes_read = infile.ReadAtCurrentPos(buffer.data(), static_cast<int>(buffer.size()));
        if (bytes_read < 0) {
            return false;
        }
        if (bytes_read == 0) {
        ...
        do {
            int bytes_written_partial = outfile.WriteAtCurrentPos(&buffer[static_cast<size_t>(bytes_written_per_read)], bytes_read - bytes_written_per_read);
...
bool ReadStreamToStringWithMaxSize(FILE* stream, size_t max_size, std::string* contents)
{
    if (contents) {
        contents->clear();
    }
    ...
    bool read_successs = ReadStreamToSpanWithMaxSize(stream, max_size, [&content_string](size_t size) {
        content_string.resize(size);
        //return as_writable_bytes(make_span(content_string));
        return span<uint8_t>((uint8_t*)content_string.data(), content_string.size());
    });
...
std::optional<std::vector<uint8_t>> ReadFileToBytes(const FilePath& path)
{
    ...
    if (!ReadStreamToSpanWithMaxSize(file_stream.get(), std::numeric_limits<size_t>::max(), [&bytes](size_t size) {
            bytes.resize(size);
            return span<uint8_t>(bytes.data(), bytes.size());
        })) {
        return std::nullopt;
    }
    return bytes;
...
bool ReadFileToStringWithMaxSize(const FilePath& path, std::string* contents, size_t max_size)
{
    if (contents) {
        contents->clear();
    }
...
OnceCallback<std::optional<int64_t>()> GetFileSizeCallback(const FilePath& path)
{
    return BindOnce([](const FilePath& path) { return GetFileSize(path); }, path);
}

bool TouchFile(const FilePath& path, const Time& last_accessed, const Time& last_modified)
{
...
std::optional<uint64_t> ReadFile(const FilePath& filename, span<uint8_t> buffer)
{
...
int ReadFile(const FilePath& filename, char* data, int max_size)
{
    ...
    std::optional<uint64_t> result = ReadFile(filename, make_span(data, static_cast<uint32_t>(max_size)));
    if (!result) {
        return -1;
    }
...
FilePath GetUniquePathWithSuffixFormat(const FilePath& path, base::cstring_view suffix_format)
{
    DCHECK(!path.empty());
    ...
    for (int count = 1; count <= kMaxUniqueFiles; ++count) {
        FilePath candidate_path = path.InsertBeforeExtensionASCII(/*StringPrintfNonConstexpr*/StringPrintf(suffix_format.data(), count));
