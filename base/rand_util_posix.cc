...
#if (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)) && !BUILDFLAG(IS_NACL)
...
bool GetRandomSyscall(void* output, size_t output_length)
{
    ...
    // some bots).
    const ssize_t r = HANDLE_EINTR(syscall(__NR_getrandom, output, output_length, 0));
...
{
    g_use_boringssl.store(FeatureList::IsEnabled(kUseBoringSSLForRandBytes), std::memory_order_relaxed);
}
