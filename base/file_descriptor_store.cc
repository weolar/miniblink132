...
FileDescriptorStore::Descriptor::Descriptor(const std::string& key, base::ScopedFD fd, base::MemoryMappedFile::Region region)
    : key(key)
    , fd(std::move(fd))
    , region(region)
{
...
base::ScopedFD FileDescriptorStore::TakeFD(const std::string& key, base::MemoryMappedFile::Region* region)
{
...
base::ScopedFD FileDescriptorStore::MaybeTakeFD(const std::string& key, base::MemoryMappedFile::Region* region)
{
...
void FileDescriptorStore::Set(const std::string& key, base::ScopedFD fd, base::MemoryMappedFile::Region region)
{
