...
static_assert(std::is_trivially_destructible_v<base::NoDestructor<std::string>>);
...
struct UncopyableUnmovable {
    ...
    explicit UncopyableUnmovable(int value)
        : value(value)
    {
    }
...
TEST(NoDestructorTest, PriorityInversionAtStaticInitializationResolves)
{
    ...
    BlockingConstructorThread background_getter(ThreadType::kDefault, OnceClosure());
    ...
    RepeatingClosure foreground_thread_ready_callback = BarrierClosure(kNumForegroundThreads, BindOnce(&BlockingConstructor::CompleteConstructionNow));
    for (int i = 0; i < kNumForegroundThreads; ++i) {
        ...
        foreground_threads.push_back(std::make_unique<BlockingConstructorThread>(ThreadType::kDisplayCritical, foreground_thread_ready_callback));
