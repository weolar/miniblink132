...
public:
    explicit FileVersionInfoForModuleFactory(const FilePath& path)
        ...
        // be executed.
        : library_(::LoadLibraryEx(path.value().c_str(), nullptr, LOAD_LIBRARY_AS_IMAGE_RESOURCE))
    {
        EXPECT_TRUE(library_.is_valid());
    }
    FileVersionInfoForModuleFactory(const FileVersionInfoForModuleFactory&) = delete;
    FileVersionInfoForModuleFactory& operator=(const FileVersionInfoForModuleFactory&) = delete;
...
template <typename T> class FileVersionInfoTest : public testing::Test { };
...
{
    const base::FilePath::CharType kDLLName[] = FILE_PATH_LITERAL("FileVersionInfoTest1.dll");
    ...
    int j = 0;
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->company_name()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->company_short_name()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->product_name()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->product_short_name()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->internal_name()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->product_version()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->special_build()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->original_filename()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->file_description()));
    EXPECT_EQ(kExpectedValues[j++], base::AsWStringView(version_info->file_version()));
}
...
TYPED_TEST(FileVersionInfoTest, CustomProperties)
{
    ...
    EXPECT_EQ(u"1600 Amphitheatre Parkway Mountain View, CA 94043", version_info_win->GetStringValue(u"Custom prop 3"));

    // Test an non-existing property.
    EXPECT_FALSE(version_info_win->GetValue(u"Unknown property", &str));
    EXPECT_EQ(std::u16string(), version_info_win->GetStringValue(u"Unknown property"));

    EXPECT_EQ(base::Version(std::vector<uint32_t> { 1, 0, 0, 1 }), version_info_win->GetFileVersion());
}
