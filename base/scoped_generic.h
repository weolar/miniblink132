...
template <typename T, typename Traits> class ScopedGeneric {
private:
...
public:
    ...
    {
        element_type old_generic = std::exchange(data_.generic, traits_type::InvalidValue());
    ...
    class Receiver {
    public:
        explicit Receiver(ScopedGeneric& parent)
            : scoped_generic_(&parent)
        {
    ...
    Traits& get_traits() LIFETIME_BOUND
    {
        return data_;
    }
    const Traits& get_traits() const LIFETIME_BOUND
    {
        return data_;
    }
...
private:
    ...
    template <typename T2, typename Traits2> bool operator==(const ScopedGeneric<T2, Traits2>& p2) const;
    template <typename T2, typename Traits2> bool operator!=(const ScopedGeneric<T2, Traits2>& p2) const;
...
template <class T, class Traits> void swap(const ScopedGeneric<T, Traits>& a, const ScopedGeneric<T, Traits>& b)
{
    a.swap(b);
}

template <class T, class Traits> bool operator==(const T& value, const ScopedGeneric<T, Traits>& scoped)
{
