...
void GetTestFunctionInstructions(std::vector<Instruction>* body)
{
...
#endif
    helper_library_path = helper_library_path.AppendASCII(GetNativeLibraryName("immediate_crash_test_helper"));
    ScopedNativeLibrary helper_library(helper_library_path);
    ASSERT_TRUE(helper_library.is_valid()) << "shared library load failed: " << helper_library.GetError()->ToString();
    ...
    ASSERT_TRUE(reinterpret_cast<uintptr_t>(a) & 0x1) << "Expected T32 opcodes but found A32 opcodes instead.";
    ASSERT_TRUE(reinterpret_cast<uintptr_t>(b) & 0x1) << "Expected T32 opcodes but found A32 opcodes instead.";
...
std::optional<std::vector<Instruction>> ExpectImmediateCrashInvocation(std::vector<Instruction> instructions)
{
...
std::vector<Instruction> MaybeSkipOptionalFooter(std::vector<Instruction> instructions)
{
    auto iter = instructions.begin();
...
bool MatchPrefix(const std::vector<Instruction>& haystack, const base::span<const Instruction>& needle)
{
...
std::vector<Instruction> DropUntilMatch(std::vector<Instruction> haystack, const base::span<const Instruction>& needle)
{
...
std::vector<Instruction> MaybeSkipCoverageHook(std::vector<Instruction> instructions)
{
#if BUILDFLAG(USE_CLANG_COVERAGE) || BUILDFLAG(CLANG_PROFILING)
