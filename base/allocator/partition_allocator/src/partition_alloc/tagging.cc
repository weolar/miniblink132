...
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
using ChangeMemoryTaggingModeForCurrentThreadInternalFn = void(TagViolationReportingMode);

void ChangeMemoryTaggingModeForCurrentThreadForMte(TagViolationReportingMode m)
{
    ...
    if (m == TagViolationReportingMode::kSynchronous) {
        change_memory_tagging_mode(PR_TAGGED_ADDR_ENABLE | PR_MTE_TCF_SYNC | (0xfffe << PR_MTE_TAG_SHIFT));
    } else if (m == TagViolationReportingMode::kAsynchronous) {
        change_memory_tagging_mode(PR_TAGGED_ADDR_ENABLE | PR_MTE_TCF_ASYNC | (0xfffe << PR_MTE_TAG_SHIFT));
    } else {
        change_memory_tagging_mode(PR_TAGGED_ADDR_ENABLE | PR_MTE_TCF_NONE);
    }
}

extern "C" ChangeMemoryTaggingModeForCurrentThreadInternalFn* ResolveChangeMemoryTaggingModeForCurrentThread(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
    return internal::IsMteEnabled(hwcap, hw) ? &ChangeMemoryTaggingModeForCurrentThreadForMte : &ChangeMemoryTaggingModeForCurrentThreadNoOp;
}
...
#if PA_BUILDFLAG(IS_ANDROID)
bool ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode m)
{
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
    ...
    static MalloptSignature mallopt_fnptr = [] {
        ...
        PA_CHECK(library);
        void* func_ptr = base::GetFunctionPointerFromNativeLibrary(library, "mallopt");
        PA_CHECK(func_ptr);
    ...
    int status = 0;
    if (m == TagViolationReportingMode::kSynchronous) {
        status = mallopt_fnptr(M_BIONIC_SET_HEAP_TAGGING_LEVEL, M_HEAP_TAGGING_LEVEL_SYNC);
    } else if (m == TagViolationReportingMode::kAsynchronous) {
        status = mallopt_fnptr(M_BIONIC_SET_HEAP_TAGGING_LEVEL, M_HEAP_TAGGING_LEVEL_ASYNC);
    } else {
        status = mallopt_fnptr(M_BIONIC_SET_HEAP_TAGGING_LEVEL, M_HEAP_TAGGING_LEVEL_NONE);
    }
    return status != 0;
#else
    return false;
...
[[maybe_unused]] static bool CheckTagRegionParameters(void* ptr, size_t sz)
{
    ...
    bool ret = (ptr_as_uint % kMemTagGranuleSize == 0) && (sz % kMemTagGranuleSize == 0) && sz;
    return ret;
}
...
void* TagRegionRandomlyForMTE(void* ptr, size_t sz, uint64_t mask)
{
    ...
    for (size_t i = 0; i < sz; i += kMemTagGranuleSize) {
        ...
        __arm_mte_set_tag(nptr + i); // Tag is taken from the top bits of the argument.
    }
    return nptr;
}
...
using TagMemoryRangeRandomlyInternalFn = void*(void* ptr, size_t size, uint64_t mask);

extern "C" TagMemoryRangeIncrementInternalFn* ResolveTagMemoryRangeIncrement(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
...
extern "C" TagMemoryRangeRandomlyInternalFn* ResolveTagMemoryRandomly(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
    if (IsMteEnabled(hwcap, hw)) {
...
extern "C" RemaskPtrInternalFn* ResolveRemaskPointer(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
    if (IsMteEnabled(hwcap, hw)) {
...
void* TagMemoryRangeIncrementInternal(void* ptr, size_t size) __attribute__((ifunc("ResolveTagMemoryRangeIncrement")));
void* TagMemoryRangeRandomlyInternal(void* ptr, size_t size, uint64_t mask) __attribute__((ifunc("ResolveTagMemoryRandomly")));
void* RemaskPointerInternal(void* ptr) __attribute__((ifunc("ResolveRemaskPointer")));
#endif // PA_BUILDFLAG(HAS_MEMORY_TAGGING)
...
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
using GetMemoryTaggingModeForCurrentThreadInternalFn = TagViolationReportingMode();
...
extern "C" GetMemoryTaggingModeForCurrentThreadInternalFn* ResolveGetMemoryTaggingModeForCurrentThread(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
    return IsMteEnabled(hwcap, hw) ? &GetMemoryTaggingModeForCurrentThreadMte : &GetMemoryTaggingModeForCurrentThreadNoMte;
}
#endif // PA_BUILDFLAG(HAS_MEMORY_TAGGING)
...
// static
bool PermissiveMte::HandleCrash(int signo, siginfo_t* siginfo, ucontext_t* context)
{
    if (siginfo->si_signo == SIGSEGV && (siginfo->si_code == SEGV_MTESERR || siginfo->si_code == SEGV_MTEAERR) && PermissiveMte::enabled_) {
        ...
        internal::ChangeMemoryTaggingModeForAllThreadsPerProcess(partition_alloc::TagViolationReportingMode::kDisabled);
        return true;
    }
    return false;
}
