...
#if PA_BUILDFLAG(USE_FREESLOT_BITMAP) && !defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
...
protected:
    ...
    static constexpr FreeSlotBitmapCellType kAllFree = std::numeric_limits<FreeSlotBitmapCellType>::max();
    ...
    void SetUp() override
    {
        ...
        allocated_ptr_ = reinterpret_cast<uintptr_t>(allocator_.root()->Alloc(2 * kSuperPageSize));
        super_page_ = (allocated_ptr_ + kSuperPageSize) & kSuperPageBaseMask;

        // Checks that the whole superpage is in the allocated region.
        PA_DCHECK(super_page_ + kSuperPageSize <= allocated_ptr_ + 2 * kSuperPageSize);
    }
...
TEST_F(PartitionAllocFreeSlotBitmapTest, MarkAllBitsInCellAsUsed)
{
    ...
    auto [cell_first_slot, bit_index_first_slot] = GetFreeSlotBitmapCellPtrAndBitIndex(kFirstSlotAddr);
    auto [cell_last_slot, bit_index_last_slot] = GetFreeSlotBitmapCellPtrAndBitIndex(kLastSlotAddr);
    ...
    for (size_t slot_addr = kFirstSlotAddr; slot_addr < kLastSlotAddr; slot_addr += kSmallestBucket) {
        FreeSlotBitmapMarkSlotAsFree(slot_addr);
    }
...
TEST_F(PartitionAllocFreeSlotBitmapTest, ResetBitmap)
{
    ...
    auto [cell_first_slot, bit_index_first_slot] = GetFreeSlotBitmapCellPtrAndBitIndex(SlotAddr(0));
    EXPECT_EQ(0u, bit_index_first_slot);
    EXPECT_EQ(kAllFree, *cell_first_slot);
    EXPECT_EQ(kAllFree, *(cell_first_slot + 1));
    EXPECT_EQ(kAllFree, *(cell_first_slot + 2));

    FreeSlotBitmapReset(SlotAddr(kFreeSlotBitmapBitsPerCell), SlotAddr(2 * kFreeSlotBitmapBitsPerCell), kSmallestBucket);
    EXPECT_EQ(kAllFree, *cell_first_slot);
...
TEST_F(PartitionAllocFreeSlotBitmapTest, ResetBitmapWithZeroLengthRange)
{
    ...
    auto [cell_aligned, bit_index_aligned] = GetFreeSlotBitmapCellPtrAndBitIndex(aligned_addr);
    ...
    auto [cell_non_aligned, bit_index_non_aligned] = GetFreeSlotBitmapCellPtrAndBitIndex(non_aligned_addr);
...
TEST_F(PartitionAllocFreeSlotBitmapTest, ResetSingleBitInMiddleOfCell)
{
    ...
    auto [cell_mid, bit_index_mid] = GetFreeSlotBitmapCellPtrAndBitIndex(mid_cell_slot_addr);
    EXPECT_NE(0u, bit_index_mid);
    EXPECT_TRUE(*cell_mid & CellWithAOne(bit_index_mid));

    // Reset the single bit in the middle of the cell.
    FreeSlotBitmapReset(mid_cell_slot_addr, mid_cell_slot_addr + kSmallestBucket, kSmallestBucket);
