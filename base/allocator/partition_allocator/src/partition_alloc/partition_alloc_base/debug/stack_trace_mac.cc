...
// libc.
#if PA_BUILDFLAG(IS_APPLE) || (defined(__GLIBC__) && !defined(__UCLIBC__) && !defined(__AIX))
...
size_t CollectStackTrace(const void** trace, size_t count)
{
    ...
    return base::saturated_cast<size_t>(backtrace(const_cast<void**>(trace), base::saturated_cast<int>(count)));
#else
