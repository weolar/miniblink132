...
void OutputStackTrace(unsigned index, uintptr_t address, uintptr_t base_address, const char* module_name, uintptr_t offset)
{
    size_t module_name_len = strlen(module_name);

    char buffer[256];
    if (module_name_len > 4 && !strcmp(module_name + module_name_len - 4, ".apk")) {
        strings::SafeSPrintf(buffer, "#%02d pc 0x%0x %s (offset 0x%0x)\n", index, address - base_address, module_name, offset);
    } else {
        strings::SafeSPrintf(buffer, "#%02d pc 0x%0x %s\n", index, address - base_address, module_name);
    }
