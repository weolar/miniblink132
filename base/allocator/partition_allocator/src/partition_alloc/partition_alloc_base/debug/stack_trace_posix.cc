...
bool ParseAddress(const char** ptr, const char* end, uintptr_t* address_return)
{
...
bool ParsePermissions(const char** ptr, const char* end, unsigned* permission_return)
{
...
bool ParseMapsLine(const char* line_start, const char* line_end, uintptr_t* start_address_return, uintptr_t* end_address_return, unsigned* permission_return,
    uintptr_t* offset_return, const char** module_name)
{
...
ssize_t ReadFromOffset(const int fd, void* buf, const size_t count, const size_t offset)
{
    ...
    while (num_bytes < count) {
        ssize_t len;
        len = WrapEINTR(pread)(fd, buf0 + num_bytes, count - num_bytes, static_cast<off_t>(offset + num_bytes));
...
void UpdateBaseAddress(unsigned permissions, uintptr_t start_address, uintptr_t* base_address)
{
    ...
    ElfW(Ehdr) ehdr;
    ssize_t len = ReadFromOffset(mem_fd, &ehdr, sizeof(ElfW(Ehdr)), start_address);
            ...
            case ET_DYN:
                ...
                *base_address = start_address;
                for (unsigned i = 0; i != ehdr.e_phnum; ++i) {
                    ElfW(Phdr) phdr;
                    len = ReadFromOffset(mem_fd, &phdr, sizeof(ElfW(Phdr)), start_address + ehdr.e_phoff + i * sizeof(phdr));
                    if (len == sizeof(ElfW(Phdr)) && phdr.p_type == PT_LOAD && phdr.p_offset == 0) {
        ...
        for (char* line_end = buffer; line_end < read_end; ++line_end) {
            if (*line_end == '\n') {
                ...
                *line_end = '\0';
                ...
                bool ok = ParseMapsLine(line_start, line_end, &start_address, &end_address, &permissions, &offset, &module_name);
                if (ok) {
#if !PA_BUILDFLAG(IS_ANDROID)
...
void PrintStackTraceInternal(const void* const* trace, size_t size)
{
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        if (dl_info_found) {
            ...
            OutputStackTrace(i, reinterpret_cast<uintptr_t>(trace[i]), reinterpret_cast<uintptr_t>(dl_info.dli_fbase), basename, 0u);
        } else {
...
#if !PA_BUILDFLAG(IS_ANDROID)
void OutputStackTrace(unsigned index, uintptr_t address, uintptr_t base_address, const char* module_name, uintptr_t)
{
    char buffer[256];
    strings::SafeSPrintf(buffer, "#%02d 0x%0x  (%s+0x%0x)\n", index, address, module_name, address - base_address);
