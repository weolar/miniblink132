...
{
    PA_BASE_DCHECK(CanConvertToFileTime(us)) << "Out-of-range: Cannot convert " << us << " microseconds to FILETIME units.";
...
TimeTicks RolloverProtectedNow()
{
    ...
    while (true) {
        ...
        int32_t check = g_last_time_and_rollovers.compare_exchange_strong(original, state.as_opaque_32, std::memory_order_release);
        if (check == original) {
            break;
        }
