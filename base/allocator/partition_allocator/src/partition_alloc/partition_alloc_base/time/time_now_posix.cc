...
#if (PA_BUILDFLAG(IS_POSIX) && defined(_POSIX_MONOTONIC_CLOCK) && _POSIX_MONOTONIC_CLOCK >= 0) || PA_BUILDFLAG(IS_BSD) || PA_BUILDFLAG(IS_ANDROID)
...
Time TimeNowIgnoringOverride()
{
    ...
    return Time() + Microseconds((tv.tv_sec * Time::kMicrosecondsPerSecond + tv.tv_usec) + Time::kTimeTToMicrosecondsOffset);
}
...
{
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || PA_BUILDFLAG(IS_ANDROID)
