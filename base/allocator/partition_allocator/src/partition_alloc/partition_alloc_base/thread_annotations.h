...
#define PA_ACQUIRED_AFTER(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(acquired_after(__VA_ARGS__))

#define PA_ACQUIRED_BEFORE(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(acquired_before(__VA_ARGS__))
...
#define PA_EXCLUSIVE_LOCKS_REQUIRED(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(exclusive_locks_required(__VA_ARGS__))

#define PA_SHARED_LOCKS_REQUIRED(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(shared_locks_required(__VA_ARGS__))
...
// non-reentrant).
#define PA_LOCKS_EXCLUDED(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(locks_excluded(__VA_ARGS__))
...
// not release it.
#define PA_EXCLUSIVE_LOCK_FUNCTION(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(exclusive_lock_function(__VA_ARGS__))

// PA_SHARED_LOCK_FUNCTION()
//
// Documents functions that acquire a shared (reader) lock in the body of a
// function, and do not release it.
#define PA_SHARED_LOCK_FUNCTION(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(shared_lock_function(__VA_ARGS__))

// PA_UNLOCK_FUNCTION()
//
// Documents functions that expect a lock to be held on entry to the function,
// and release it in the body of the function.
#define PA_UNLOCK_FUNCTION(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(unlock_function(__VA_ARGS__))
...
#define PA_EXCLUSIVE_TRYLOCK_FUNCTION(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(exclusive_trylock_function(__VA_ARGS__))

#define PA_SHARED_TRYLOCK_FUNCTION(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(shared_trylock_function(__VA_ARGS__))

// PA_ASSERT_EXCLUSIVE_LOCK() / PA_ASSERT_SHARED_LOCK()
//
// Documents functions that dynamically check to see if a lock is held, and fail
// if it is not held.
#define PA_ASSERT_EXCLUSIVE_LOCK(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(assert_exclusive_lock(__VA_ARGS__))

#define PA_ASSERT_SHARED_LOCK(...) PA_THREAD_ANNOTATION_ATTRIBUTE__(assert_shared_lock(__VA_ARGS__))
...
#define PA_NO_THREAD_SAFETY_ANALYSIS PA_THREAD_ANNOTATION_ATTRIBUTE__(no_thread_safety_analysis)
...
#define PA_TS_UNCHECKED_READ(x) partition_alloc::internal::thread_safety_analysis::ts_unchecked_read(x)

namespace partition_alloc::internal::thread_safety_analysis {

// Takes a reference to a guarded data member, and returns an unguarded
// reference.
template <typename T> inline const T& ts_unchecked_read(const T& v) PA_NO_THREAD_SAFETY_ANALYSIS
{
    return v;
}

template <typename T> inline T& ts_unchecked_read(T& v) PA_NO_THREAD_SAFETY_ANALYSIS
{
    return v;
}
