...
private:
    ...
    public:
        template <typename... Args> explicit PlacementStorage(Args&&... args)
        {
        ...
        const T* get() const
        {
            return const_cast<PlacementStorage*>(this)->get();
        }
        T* get()
        {
    ...
    // tracking at all.
    std::conditional_t<std::is_trivially_destructible_v<T>, DirectStorage, PlacementStorage> storage_;
};
