...
// Visual C 32 bit and ClangCL 32/64 bit test.
#elif PA_BUILDFLAG(PA_COMPILER_MSVC) && (PA_BUILDFLAG(PA_ARCH_CPU_32_BITS) || (PA_BUILDFLAG(PA_ARCH_CPU_64_BITS) && defined(__clang__)))
    ...
    if (cpu.has_mte()) {
        ...
        GTEST_FAIL() << "MTE support detected (but base/ built without MTE support)";
#else
        char ptr[32];
        uint64_t val;
