...
TEST(PartitionAllocBaseRandUtilTest, RandGeneratorForRandomShuffle)
{
    ...
    EXPECT_LE(std::numeric_limits<ptrdiff_t>::max(), std::numeric_limits<int64_t>::max());
}
...
TEST(PartitionAllocBaseRandUtilTest, RandGeneratorIsUniform)
{
    ...
    constexpr uint64_t kTopOfRange = (std::numeric_limits<uint64_t>::max() / 4ULL) * 3ULL;
...
TEST(PartitionAllocBaseRandUtilTest, DISABLED_RandBytesPerf)
{
    ...
    PA_LOG(INFO) << "RandBytes(" << kTestBufferSize << ") took: " << (end - now).InMicroseconds() << "µs";
}

TEST(PartitionAllocBaseRandUtilTest, InsecureRandomGeneratorProducesBothValuesOfAllBits)
{
    // This tests to see that our underlying random generator is good
    // enough, for some value of good enough.
    uint64_t kAllZeros = 0ULL;
    ...
    InsecureRandomGenerator generator = InsecureRandomGenerator::ConstructForTesting();
...
bool ChiSquaredTest(InsecureRandomGenerator& gen, size_t n, int from_bit, int num_bits)
{
    const int range = 1 << num_bits;
    PA_BASE_CHECK(static_cast<int>(n % range) == 0) << "Makes computations simpler";
    ...
    return chi_squared > ChiSquaredCriticalValue(range - 1, kXp1Percent) && chi_squared < ChiSquaredCriticalValue(range - 1, kXp99Percent);
}
...
TEST(PartitionAllocBaseRandUtilTest, InsecureRandomGeneratorChiSquared)
{
    ...
    for (int start_bit : { 1, 2, 3, 8, 12, 20, 32, 48, 54 }) {
        ...
        for (int i = 0; i < kIterations; i++) {
            size_t samples = 1 << 16;
            InsecureRandomGenerator gen = InsecureRandomGenerator::ConstructForTesting();
        ...
        EXPECT_GE(pass_count, expected_pass_count - ((kIterations * 2) / 100)) << "For start_bit = " << start_bit;
    }
}
