...
template <typename Dst, typename Src, typename Enable = void> struct IsValueInRangeFastOp {
    static constexpr bool is_supported = false;
...
struct IsValueInRangeFastOp<Dst, Src,
    typename std::enable_if<std::is_integral_v<Dst> && std::is_integral_v<Src> && std::is_signed_v<Dst> && std::is_signed_v<Src>
...
template <typename Dst, typename Src> constexpr bool IsValueInRangeForNumericType(Src value)
{
    using SrcType = typename internal::UnderlyingType<Src>::type;
    return internal::IsValueInRangeFastOp<Dst, SrcType>::is_supported ? internal::IsValueInRangeFastOp<Dst, SrcType>::Do(static_cast<SrcType>(value))
...
template <typename Dst, class CheckHandler = internal::CheckOnFailure, typename Src> constexpr Dst checked_cast(Src value)
{
...
template <typename T> struct SaturationDefaultLimits : public std::numeric_limits<T> {
    ...
    {
        return std::numeric_limits<T>::has_infinity ? std::numeric_limits<T>::infinity() * -1 : std::numeric_limits<T>::lowest();
    }
};

template <typename Dst, template <typename> class S, typename Src> constexpr Dst saturated_cast_impl(Src value, RangeCheck constraint)
{
...
template <typename Dst, typename Src, typename Enable = void> struct SaturateFastOp {
    static constexpr bool is_supported = false;
...
struct SaturateFastOp<Dst, Src,
    typename std::enable_if<std::is_integral_v<Src> && std::is_integral_v<Dst> && SaturateFastAsmOp<Dst, Src>::is_supported>::type> {
    static constexpr bool is_supported = true;
    static constexpr Dst Do(Src value)
    {
...
struct SaturateFastOp<Dst, Src,
    ...
    static constexpr Dst Do(Src value)
    {
        ...
        const Dst saturated
            = CommonMaxOrMin<Dst, Src>(IsMaxInRangeForNumericType<Dst, Src>() || (!IsMinInRangeForNumericType<Dst, Src>() && IsValueNegative(value)));
        if (IsValueInRangeForNumericType<Dst>(value)) [[likely]] {
...
template <typename Dst, template <typename> class SaturationHandler = SaturationDefaultLimits, typename Src> constexpr Dst saturated_cast(Src value)
{
    using SrcType = typename UnderlyingType<Src>::type;
    return !PA_IsConstantEvaluated() && SaturateFastOp<Dst, SrcType>::is_supported && std::is_same_v<SaturationHandler<Dst>, SaturationDefaultLimits<Dst>>
        ? SaturateFastOp<Dst, SrcType>::Do(static_cast<SrcType>(value))
        : saturated_cast_impl<Dst, SaturationHandler, SrcType>(
            static_cast<SrcType>(value), DstRangeRelationToSrcRange<Dst, SaturationHandler, SrcType>(static_cast<SrcType>(value)));
}

// strict_cast<> is analogous to static_cast<> for numeric types, except that
// it will cause a compile failure if the destination type is not large enough
// to contain any value in the source type. It performs no runtime checking.
template <typename Dst, typename Src> constexpr Dst strict_cast(Src value)
{
    using SrcType = typename UnderlyingType<Src>::type;
...
template <typename Dst, typename Src, class Enable = void> struct IsNumericRangeContained {
...
public:
    ...
    // saturated_cast<> template functions for your particular use case.
    template <typename Dst, typename std::enable_if<IsNumericRangeContained<Dst, T>::value>::type* = nullptr> constexpr operator Dst() const
    {
...
// type.
template <typename T> constexpr StrictNumeric<typename UnderlyingType<T>::type> MakeStrictNum(const T value)
{
    return value;
}
...
// versions.
template <typename Dst = int, typename Src, typename = std::enable_if_t<std::is_integral_v<Dst> && std::is_floating_point_v<Src>>> Dst ClampFloor(Src value)
{
    return saturated_cast<Dst>(std::floor(value));
}
template <typename Dst = int, typename Src, typename = std::enable_if_t<std::is_integral_v<Dst> && std::is_floating_point_v<Src>>> Dst ClampCeil(Src value)
{
    return saturated_cast<Dst>(std::ceil(value));
}
template <typename Dst = int, typename Src, typename = std::enable_if_t<std::is_integral_v<Dst> && std::is_floating_point_v<Src>>> Dst ClampRound(Src value)
{
