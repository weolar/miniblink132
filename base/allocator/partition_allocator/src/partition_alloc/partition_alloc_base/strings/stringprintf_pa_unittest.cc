...
#if PA_BUILDFLAG(IS_WIN) && defined(COMPONENT_BUILD) && PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
...
// crashes.
#define MAYBE_PartitionAllocStringPrintfTest DISABLED_PartitionAllocStringPrintfTest
#else
...
{
    EXPECT_EQ("123hello w", TruncatingStringPrintf("%3d%2s %1c", 123, "hello", 'w'));
}
...
TEST(MAYBE_PartitionAllocStringPrintfTest, TruncatingStringPrintfTruncatesResult)
{
