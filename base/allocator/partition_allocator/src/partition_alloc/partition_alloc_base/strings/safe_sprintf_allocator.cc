...
// complain, but clang does.
        ...
        static_assert(kSSizeMaxConst == static_cast<size_t>(std::numeric_limits<ssize_t>::max()), "kSSizeMaxConst should be the max value of an ssize_t");
#endif
        DEBUG_CHECK(size > 0);
    ...
    bool IToASCII(bool sign, bool upcase, int64_t i, size_t base, char pad, size_t padding, const char* prefix);
...
bool Buffer::IToASCII(bool sign, bool upcase, int64_t i, size_t base, char pad, size_t padding, const char* prefix)
{
...
ssize_t SafeSNPrintf(char* buf, size_t sz, const char* fmt, const Arg* args, const size_t max_args)
{
        ...
        format_character_found:
            ...
            case '9':
                ...
                for (;;) {
                    ...
                    if (padding > max_padding / 10 || 10 * padding > max_padding - digit) {
                        DEBUG_CHECK(padding <= max_padding / 10 && 10 * padding <= max_padding - digit);
            ...
            case 'p': { // Output a pointer value.
                ...
                buffer.IToASCII(ch == 'd' && arg.type == Arg::INT, ch != 'x', i, ch == 'o' ? 8 : ch == 'd' ? 10 : 16, pad, padding, prefix);
                break;
            }
            case 's': {
                // Check that there are arguments left to be inserted.
