...
TEST(SafeSPrintfTestPA, NoArguments)
{
    ...
    // unchanged.
    EXPECT_EQ(static_cast<ssize_t>(sizeof(text)) - 1, SafeSNPrintf(buf, sizeof(buf), text));
    EXPECT_EQ(std::string(text), std::string(buf));
    EXPECT_TRUE(!memcmp(buf + sizeof(text), ref + sizeof(text), sizeof(buf) - sizeof(text)));
    memcpy(buf, ref, sizeof(buf));
...
TEST(SafeSPrintfTestPA, OneArgument)
{
    ...
    // A one-byte buffer should always print a single NUL byte.
    EXPECT_EQ(static_cast<ssize_t>(sizeof(text)) - 1, SafeSNPrintf(buf, 1, fmt, ' '));
    EXPECT_EQ(0, buf[0]);
    EXPECT_TRUE(!memcmp(buf + 1, ref + 1, sizeof(buf) - 1));
    memcpy(buf, ref, sizeof(buf));
    ...
    // A larger (but limited) buffer should always leave the trailing bytes
    // unchanged.
    EXPECT_EQ(static_cast<ssize_t>(sizeof(text)) - 1, SafeSNPrintf(buf, 2, fmt, ' '));
    EXPECT_EQ(text[0], buf[0]);
    EXPECT_EQ(0, buf[1]);
    EXPECT_TRUE(!memcmp(buf + 2, ref + 2, sizeof(buf) - 2));
    memcpy(buf, ref, sizeof(buf));
    ...
    // A unrestricted buffer length should always leave the trailing bytes
    // unchanged.
    EXPECT_EQ(static_cast<ssize_t>(sizeof(text)) - 1, SafeSNPrintf(buf, sizeof(buf), fmt, ' '));
    EXPECT_EQ(std::string(text), std::string(buf));
    EXPECT_TRUE(!memcmp(buf + sizeof(text), ref + sizeof(text), sizeof(buf) - sizeof(text)));
    memcpy(buf, ref, sizeof(buf));
...
TEST(SafeSPrintfTestPA, ASANFriendlyBufferTest)
{
    ...
    EXPECT_EQ(static_cast<ssize_t>(sizeof(kTestString) - 1), SafeSNPrintf(buf.data(), sizeof(kTestString), kTestString));
    EXPECT_EQ(std::string(kTestString), std::string(buf.data()));
    EXPECT_EQ(static_cast<ssize_t>(sizeof(kTestString) - 1), SafeSNPrintf(buf.data(), sizeof(kTestString), "%s", kTestString));
    EXPECT_EQ(std::string(kTestString), std::string(buf.data()));
}
...
TEST(SafeSPrintfTestPA, DataTypes)
{
    ...
    EXPECT_EQ(std::string(addr), std::string(buf));
    snprintf(addr, sizeof(addr), "0x%llX", (unsigned long long)(uintptr_t)snprintf);
    ...
    EXPECT_EQ(std::string(addr), std::string(buf));
    snprintf(addr, sizeof(addr), "0x%llX", (unsigned long long)(uintptr_t)buf);
    memset(addr, ' ', (char*)memmove(addr + sizeof(addr) - strlen(addr) - 1, addr, strlen(addr) + 1) - addr);
...
void PrintLongString(char* buf, size_t sz)
{
    ...
    for (std::vector<char> perfect_buf;;) {
        size_t needed = SafeSNPrintf(out, out_sz,
                            ...
                            "A%2cong %s: %%d %010X %d %p%7s", 'l', "string",
#endif
                            0xDEADBEEF, std::numeric_limits<intptr_t>::min(), PrintLongString, static_cast<char*>(nullptr))
            + 1;
    ...
    snprintf(ref, sizeof(ref), "A long string: %%d 00DEADBEEF %lld 0x%llX <NULL>", static_cast<long long>(std::numeric_limits<intptr_t>::min()),
        static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(PrintLongString)));
    ref[sz - 1] = '\000';
...
public:
    ...
    ScopedSafeSPrintfSSizeMaxSetter(const ScopedSafeSPrintfSSizeMaxSetter&) = delete;
    ScopedSafeSPrintfSSizeMaxSetter& operator=(const ScopedSafeSPrintfSSizeMaxSetter&) = delete;
...
TEST(SafeSPrintfTestPA, Padding)
{
    ...
    SafeSPrintf(fmt, "%%%dc", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    ...
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, 1));
    EXPECT_EQ("   ", std::string(buf));
    SafeSPrintf(fmt, "%%0%do", std::numeric_limits<ssize_t>::max() - 1);
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, 1));
    EXPECT_EQ("000", std::string(buf));
    SafeSPrintf(fmt, "%%%do", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    ...
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, 1));
    EXPECT_EQ("000", std::string(buf));
    SafeSPrintf(fmt, "%%%dd", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    EXPECT_EQ(2, SafeSPrintf(buf, fmt, 1));
    ...
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, 1));
    EXPECT_EQ("000", std::string(buf));
    SafeSPrintf(fmt, "%%%dX", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    EXPECT_EQ(2, SafeSPrintf(buf, fmt, 1));
    ...
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, (void*)1));
    EXPECT_EQ("   ", std::string(buf));
    SafeSPrintf(fmt, "%%0%dp", std::numeric_limits<ssize_t>::max() - 1);
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, (void*)1));
    EXPECT_EQ("0x0", std::string(buf));
    SafeSPrintf(fmt, "%%%dp", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    EXPECT_EQ(2, SafeSPrintf(buf, fmt, 1));
    ...
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, "A"));
    EXPECT_EQ("   ", std::string(buf));
    SafeSPrintf(fmt, "%%0%ds", std::numeric_limits<ssize_t>::max() - 1);
    EXPECT_EQ(std::numeric_limits<ssize_t>::max() - 1, SafeSNPrintf(buf, 4, fmt, "A"));
    EXPECT_EQ("   ", std::string(buf));
    SafeSPrintf(fmt, "%%%ds", static_cast<size_t>(std::numeric_limits<ssize_t>::max()));
#if defined(NDEBUG)
    EXPECT_EQ(2, SafeSPrintf(buf, fmt, "A"));
