...
template <typename T> void CStringBuilder::PutInteger(T value)
{
...
void CStringBuilder::PutFloatingPoint(double value, unsigned num_digits10)
{
    ...
    case FP_SUBNORMAL:
        ...
        PutNormalFloatingPoint(value > 0 ? std::numeric_limits<double>::min() : -std::numeric_limits<double>::min(), num_digits10);
        break;
    case FP_NORMAL:
    default:
...
void CStringBuilder::PutNormalFloatingPoint(double value, unsigned num_digits10)
{
    if (value < 0) {
        PutText("-", 1);
        value = -value;
    ...
    char buffer[64];
    ssize_t n = base::strings::SafeSPrintf(buffer, "%d", lrint(significand * GetDigits10(num_digits10)));
    PA_RAW_DCHECK(n > 0);
    PA_RAW_DCHECK(static_cast<size_t>(n) < sizeof(buffer));
    PutText(buffer, 1);
    if (n > 1) {
    ...
    if (exponent != 0) {
        n = base::strings::SafeSPrintf(buffer, "e%s%d", exponent > 0 ? "+" : "", exponent);
        PA_RAW_DCHECK(n > 0);
        PA_RAW_DCHECK(static_cast<size_t>(n) < sizeof(buffer));
        PutText(buffer, n);
    }
