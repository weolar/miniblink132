...
#if defined(BASE_ALLOCATOR_PARTITION_ALLOCATOR_SRC_PARTITION_ALLOC_PARTITION_ALLOC_BASE_CHECK_H_) || defined(BASE_CHECK_H_)                                    \
    || defined(BASE_ALLOCATOR_PARTITION_ALLOCATOR_SRC_PARTITION_ALLOC_PARTITION_ALLOC_CHECK_H_)
...
LogMessage::~LogMessage()
{
...
#if !defined(OFFICIAL_BUILD) && (PA_BUILDFLAG(IS_POSIX) || PA_BUILDFLAG(IS_WIN)) && !defined(__UCLIBC__) && !PA_BUILDFLAG(IS_AIX)
    ...
    if (severity_ == LOGGING_FATAL) {
        ...
        size_t num_traces = base::debug::CollectStackTrace(traces, kMaxTracesOfLoggingFatal);
...
void SystemErrorCodeToStream(base::strings::CStringBuilder& os, SystemErrorCode error_code)
{
    char buffer[256];
#if PA_BUILDFLAG(IS_WIN)
    ...
    DWORD len = FormatMessageA(flags, nullptr, error_code, 0, msgbuf, std::size(msgbuf), nullptr);
    if (len) {
    ...
    base::strings::SafeSPrintf(buffer, "Error (0x%x) while retrieving error. (0x%x)", GetLastError(), error_code);
    os << buffer;
#elif PA_BUILDFLAG(IS_POSIX) || PA_BUILDFLAG(IS_FUCHSIA)
