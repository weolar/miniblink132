...
#define PA_COMPACT_GOOGLE_LOG_EX_INFO(ClassName)                                                                                                               \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_INFO)
#define PA_COMPACT_GOOGLE_PLOG_EX_INFO(ClassName, error_code)                                                                                                  \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_INFO, error_code)
#define PA_COMPACT_GOOGLE_LOG_EX_WARNING(ClassName)                                                                                                            \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_WARNING)
#define PA_COMPACT_GOOGLE_PLOG_EX_WARNING(ClassName, error_code)                                                                                               \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_WARNING)
#define PA_COMPACT_GOOGLE_LOG_EX_ERROR(ClassName)                                                                                                              \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_ERROR)
#define PA_COMPACT_GOOGLE_PLOG_EX_ERROR(ClassName, error_code)                                                                                                 \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_ERROR, error_code)
#define PA_COMPACT_GOOGLE_LOG_EX_FATAL(ClassName)                                                                                                              \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_FATAL)
#define PA_COMPACT_GOOGLE_PLOG_EX_FATAL(ClassName, error_code)                                                                                                 \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_FATAL, error_code)
#define PA_COMPACT_GOOGLE_LOG_EX_DFATAL(ClassName)                                                                                                             \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_DFATAL)
#define PA_COMPACT_GOOGLE_PLOG_EX_DFATAL(ClassName, error_code)                                                                                                \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_DFATAL, error_code)
#define PA_COMPACT_GOOGLE_LOG_EX_DCHECK(ClassName)                                                                                                             \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_DCHECK)
#define PA_COMPACT_GOOGLE_PLOG_EX_DCHECK(ClassName, error_code)                                                                                                \
    ::partition_alloc::internal::logging::ClassName(__FILE__, __LINE__, ::partition_alloc::internal::logging::LOGGING_DCHECK, error_code)

#define PA_COMPACT_GOOGLE_LOG_INFO PA_COMPACT_GOOGLE_LOG_EX_INFO(LogMessage)
#define PA_COMPACT_GOOGLE_LOG_WARNING PA_COMPACT_GOOGLE_LOG_EX_WARNING(LogMessage)
...
#define PA_ERROR 0
#define PA_COMPACT_GOOGLE_LOG_EX_0(ClassName) PA_COMPACT_GOOGLE_LOG_EX_ERROR(ClassName)
...
#define PA_LOG_IS_ON(severity) (::partition_alloc::internal::logging::ShouldCreateLogMessage(::partition_alloc::internal::logging::LOGGING_##severity))
...
// may be slow.
#define PA_VLOG_IS_ON(verboselevel) ((verboselevel) <= ::partition_alloc::internal::logging::GetVlogVerbosity())

// Helper macro which avoids evaluating the arguments to a stream if
// the condition doesn't hold. Condition is evaluated once and only once.
#define PA_LAZY_STREAM(stream, condition) !(condition) ? (void)0 : ::partition_alloc::internal::logging::LogMessageVoidify() & (stream)
...
#define PA_LOG(severity) PA_LAZY_STREAM(PA_LOG_STREAM(severity), PA_LOG_IS_ON(severity))
#define PA_LOG_IF(severity, condition) PA_LAZY_STREAM(PA_LOG_STREAM(severity), PA_LOG_IS_ON(severity) && (condition))

// The VLOG macros log with negative verbosities.
#define PA_VLOG_STREAM(verbose_level) ::partition_alloc::internal::logging::LogMessage(__FILE__, __LINE__, -(verbose_level)).stream()

#define PA_VLOG(verbose_level) PA_LAZY_STREAM(PA_VLOG_STREAM(verbose_level), PA_VLOG_IS_ON(verbose_level))

#define PA_VLOG_IF(verbose_level, condition) PA_LAZY_STREAM(PA_VLOG_STREAM(verbose_level), PA_VLOG_IS_ON(verbose_level) && (condition))

#if PA_BUILDFLAG(IS_WIN)
#define PA_VPLOG_STREAM(verbose_level)                                                                                                                         \
    ::partition_alloc::internal::logging::Win32ErrorLogMessage(                                                                                                \
        __FILE__, __LINE__, -(verbose_level), ::partition_alloc::internal::logging::GetLastSystemErrorCode())                                                  \
        .stream()
...
#define PA_VPLOG(verbose_level) PA_LAZY_STREAM(PA_VPLOG_STREAM(verbose_level), PA_VLOG_IS_ON(verbose_level))

#define PA_VPLOG_IF(verbose_level, condition) PA_LAZY_STREAM(PA_VPLOG_STREAM(verbose_level), PA_VLOG_IS_ON(verbose_level) && (condition))

// TODO(akalin): Add more VLOG variants, e.g. VPLOG.

#define PA_LOG_ASSERT(condition) PA_LOG_IF(FATAL, !(PA_ANALYZER_ASSUME_TRUE(condition))) << "Assert failed: " #condition ". "

#if PA_BUILDFLAG(IS_WIN)
#define PA_PLOG_STREAM(severity)                                                                                                                               \
    PA_COMPACT_GOOGLE_PLOG_EX_##severity(Win32ErrorLogMessage, ::partition_alloc::internal::logging::GetLastSystemErrorCode()).stream()
#elif PA_BUILDFLAG(IS_POSIX) || PA_BUILDFLAG(IS_FUCHSIA)
#define PA_PLOG_STREAM(severity) PA_COMPACT_GOOGLE_PLOG_EX_##severity(ErrnoLogMessage, ::partition_alloc::internal::logging::GetLastSystemErrorCode()).stream()
#endif

#define PA_PLOG(severity) PA_LAZY_STREAM(PA_PLOG_STREAM(severity), PA_LOG_IS_ON(severity))

#define PA_PLOG_IF(severity, condition) PA_LAZY_STREAM(PA_PLOG_STREAM(severity), PA_LOG_IS_ON(severity) && (condition))
...
#define PA_EAT_STREAM_PARAMETERS                                                                                                                               \
    true ? (void)0 : ::partition_alloc::internal::logging::LogMessageVoidify() & (*::partition_alloc::internal::logging::g_swallow_stream)
...
#define PA_DVPLOG_IF(verboselevel, condition) PA_VPLOG_IF(verboselevel, condition)
...
#define PA_DLOG(severity) PA_LAZY_STREAM(PA_LOG_STREAM(severity), PA_DLOG_IS_ON(severity))

#define PA_DPLOG(severity) PA_LAZY_STREAM(PA_PLOG_STREAM(severity), PA_DLOG_IS_ON(severity))
...
#define PA_RAW_LOG(level, message) ::partition_alloc::internal::logging::RawLog(::partition_alloc::internal::logging::LOGGING_##level, message)
