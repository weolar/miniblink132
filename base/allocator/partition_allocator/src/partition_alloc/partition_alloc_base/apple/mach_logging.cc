...
std::string FormatMachErrorNumber(mach_error_t mach_err)
{
    ...
    if (mach_err >= 0 && mach_err < KERN_RETURN_MAX) {
        return partition_alloc::internal::base::TruncatingStringPrintf(" (%d)", mach_err);
    }
    return partition_alloc::internal::base::TruncatingStringPrintf(" (0x%08x)", mach_err);
}
...
MachLogMessage::MachLogMessage(const char* file_path, int line, LogSeverity severity, mach_error_t mach_err)
...
{
    stream() << ": " << mach_error_string(mach_err_) << FormatMachErrorNumber(mach_err_).c_str();
}
