...
template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>> constexpr bool HasSingleBit(T value)
{
...
template <typename T> inline constexpr T AlignDown(T size, T alignment)
{
...
template <typename T> inline T* AlignDown(T* ptr, size_t alignment)
{
    return reinterpret_cast<T*>(AlignDown(reinterpret_cast<uintptr_t>(ptr), alignment));
}

// Round up |size| to a multiple of alignment, which must be a power of two.
template <typename T> inline constexpr T AlignUp(T size, T alignment)
{
    static_assert(std::is_unsigned_v<T>);
    PA_BASE_DCHECK(HasSingleBit(alignment));
...
template <typename T> inline T* AlignUp(T* ptr, size_t alignment)
{
...
PA_ALWAYS_INLINE constexpr typename std::enable_if<std::is_unsigned_v<T> && sizeof(T) <= 8, int>::type CountlZero(T value)
{
...
#else
        return bits == 64 ? __builtin_clzll(static_cast<uint64_t>(value)) : __builtin_clz(static_cast<uint32_t>(value)) - (32 - bits);
#endif
    }
    return bits;
}
...
// one.
template <typename T, int bits = sizeof(T) * 8>
PA_ALWAYS_INLINE constexpr typename std::enable_if<std::is_unsigned_v<T> && sizeof(T) <= 8, int>::type CountrZero(T value)
{
    if (value) [[likely]] {
#if PA_BUILDFLAG(PA_COMPILER_MSVC) && !defined(__clang__)
