...
// condition is false.
#define PA_LAZY_CHECK_STREAM(stream, condition) !(condition) ? (void)0 : ::partition_alloc::internal::logging::VoidifyStream() & (stream)

// Macro which uses but does not evaluate expr and any stream parameters.
#define PA_EAT_CHECK_STREAM_PARAMS(expr)                                                                                                                       \
    true ? (void)0 : ::partition_alloc::internal::logging::VoidifyStream(expr) & (*::partition_alloc::internal::logging::g_swallow_stream)
...
protected:
    CheckError(const char* file, int line, LogSeverity severity, const char* condition);
    CheckError(const char* file, int line, LogSeverity severity);
    CheckError(const char* file, int line, LogSeverity severity, const char* condition, SystemErrorCode err_code);
...
class PA_COMPONENT_EXPORT(PARTITION_ALLOC_BASE) NotImplemented : public CheckError {
public:
...
#define PA_BASE_CHECK(cond) PA_BASE_INTERNAL_EXPECT_FALSE(cond) ? PA_IMMEDIATE_CRASH() : PA_EAT_CHECK_STREAM_PARAMS()

#define PA_BASE_CHECK_WILL_STREAM() false

#define PA_BASE_PCHECK(cond)                                                                                                                                   \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::check_error::PCheck(__FILE__, __LINE__).stream(), PA_BASE_INTERNAL_EXPECT_FALSE(cond))

#else

#define PA_BASE_CHECK(condition)                                                                                                                               \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::check_error::Check(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))

#define PA_BASE_CHECK_WILL_STREAM() true

#define PA_BASE_PCHECK(condition)                                                                                                                              \
    PA_LAZY_CHECK_STREAM(                                                                                                                                      \
        ::partition_alloc::internal::logging::check_error::PCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))
...
#define PA_BASE_DCHECK(condition)                                                                                                                              \
    PA_LAZY_CHECK_STREAM(                                                                                                                                      \
        ::partition_alloc::internal::logging::check_error::DCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))

#define PA_BASE_DPCHECK(condition)                                                                                                                             \
    PA_LAZY_CHECK_STREAM(                                                                                                                                      \
        ::partition_alloc::internal::logging::check_error::DPCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))
...
[[noreturn]] PA_COMPONENT_EXPORT(PARTITION_ALLOC_BASE) void RawCheckFailure(const char* message);
#define PA_RAW_CHECK(condition)                                                                                                                                \
    do {                                                                                                                                                       \
        if (!(condition))                                                                                                                                      \
            ::partition_alloc::internal::logging::RawCheckFailure("Check failed: " #condition "\n");                                                           \
    } while (0)
