...
{
    return !std::is_same_v<std::decay_t<decltype(T::kRefCountPreference)>, std::decay_t<decltype(U::kRefCountPreference)>>;
}
...
{
    static_assert(std::is_base_of_v<U, T>, "T implements RefCountedThreadSafe<U>, but U is not a base of T.");
}
...
template <typename T> scoped_refptr<T> AdoptRef(T* obj)
{
    ...
    static_assert(std::is_same_v<subtle::StartRefCountFromOneTag, Tag>, "Use AdoptRef only if the reference count starts from one.");
...
template <class T> class PA_TRIVIAL_ABI scoped_refptr {
public:
...
template <typename T, typename U> bool operator==(const scoped_refptr<T>& lhs, const U* rhs)
{
    return lhs.get() == rhs;
}

template <typename T, typename U> bool operator==(const T* lhs, const scoped_refptr<U>& rhs)
{
    return lhs == rhs.get();
}

template <typename T> bool operator==(const scoped_refptr<T>& lhs, std::nullptr_t null)
{
    return !static_cast<bool>(lhs);
}

template <typename T> bool operator==(std::nullptr_t null, const scoped_refptr<T>& rhs)
{
