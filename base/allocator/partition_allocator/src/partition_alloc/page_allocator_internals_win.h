...
void* VirtualAllocWithRetry(void* address, size_t size, DWORD type_flags, DWORD access_flags)
{
    void* ret = nullptr;
    ...
    bool should_retry = GetRetryOnCommitFailure() && (type_flags & MEM_COMMIT) && (access_flags != PAGE_NOACCESS);
    ...
    uintptr_t hint, size_t length, PageAccessibilityConfiguration accessibility, PageTag page_tag, [[maybe_unused]] int file_descriptor_for_shared_alloc)
{
    ...
    const DWORD type_flags = (access_flag == PAGE_NOACCESS) ? MEM_RESERVE : (MEM_RESERVE | MEM_COMMIT);
    void* ret = VirtualAllocWithRetry(reinterpret_cast<void*>(hint), length, type_flags, access_flag);
    if (ret == nullptr) {
...
bool TrySetSystemPagesAccessInternal(uintptr_t address, size_t length, PageAccessibilityConfiguration accessibility)
{
    ...
    return nullptr != VirtualAllocWithRetry(ptr, length, MEM_COMMIT, GetAccessFlags(accessibility));
}
...
void SetSystemPagesAccessInternal(uintptr_t address, size_t length, PageAccessibilityConfiguration accessibility)
{
    ...
    if (access_flag == PAGE_NOACCESS) {
        ...
        if (!VirtualAllocWithRetry(ptr, length, MEM_COMMIT, access_flag)) {
            ...
            if (error == ERROR_COMMITMENT_LIMIT || error == ERROR_COMMITMENT_MINIMUM) {
                OOM_CRASH(length);
            }
            // We check `GetLastError` for `ERROR_SUCCESS` here so that in a crash
            // report we get the error number.
