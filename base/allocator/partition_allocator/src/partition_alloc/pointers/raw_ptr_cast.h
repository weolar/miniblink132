...
template <typename Dest, typename Source> inline constexpr Dest unsafe_raw_ptr_static_cast(Source&& source) noexcept
{
    return static_cast<Dest>(source);
}

// Wrapper for |reinterpret_cast<T>(src)|.
template <typename Dest, typename Source> inline constexpr Dest unsafe_raw_ptr_reinterpret_cast(Source&& source) noexcept
{
...
template <typename Dest, typename Source> inline constexpr Dest unsafe_raw_ptr_bit_cast(const Source& source) noexcept
{
    ...
    static_assert(!std::is_reference_v<Source>, "bit_cast must not be used on reference types");
