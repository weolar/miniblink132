...
template <int IGNORE> struct CountingRawPtrExpectationTemplate {
...
#define REPORT_UNEQUAL_RAW_PTR_COUNTER(member_name)                                                                                                            \
    {                                                                                                                                                          \
        if (arg.member_name.has_value() && arg.member_name.value() != base::test::RawPtrCountingImplForTest::member_name) {                                    \
            *result_listener << "Expected `" #member_name "` to be " << arg.member_name.value() << " but got "                                                 \
...
// from the `MATCHER`.
inline bool CountersMatchImpl(const CountingRawPtrExpectations& arg, testing::MatchResultListener* result_listener)
{
    bool result = true;
