...
#if PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) || PA_BUILDFLAG(USE_ASAN_BACKUP_REF_PTR)
...
enum class RawPtrTraits : unsigned {
    ...
    kAllMask = kMayDangle | kDisableHooks | kAllowPtrArithmetic | kDisableBRP | kAllowUninitialized | kUseCountingImplForTest | kDummyForTest,
};
// Template specialization to use |PA_DEFINE_OPERATORS_FOR_FLAGS| without
// |kMaxValue| declaration.
template <> constexpr inline RawPtrTraits kAllFlags<RawPtrTraits> = RawPtrTraits::kAllMask;
...
template <typename T, typename SFINAE = void> struct IsSupportedType {
...
template <typename T> struct IsSupportedType<T, std::enable_if_t<std::is_function_v<T>>> {
...
template <> struct IsSupportedType<cc::Scheduler> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<base::internal::DelayTimerBase> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<content::responsiveness::Calculator> {
    static constexpr bool value = false;
};
// The ones below were identified from speedometer3. See crbug.com/335556942 for
// more info.
template <> struct IsSupportedType<v8::JobTask> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<blink::scheduler::MainThreadTaskQueue> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<base::sequence_manager::internal::TaskQueueImpl> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<base::internal::JobTaskSource> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<mojo::Connector> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<blink::scheduler::NonMainThreadTaskQueue> {
    static constexpr bool value = false;
};
// The ones below were identified from MotionMark. See crbug.com/335556942 for
// more info.
template <> struct IsSupportedType<cc::ImageDecodeCache> {
    static constexpr bool value = false;
};
template <> struct IsSupportedType<cc::TextureLayerImpl> {
    static constexpr bool value = false;
};
...
template <typename T> struct IsSupportedType<T, std::enable_if_t<std::is_convertible_v<T*, id>>> {
    static constexpr bool value = false;
};
#endif // __OBJC__
...
#define PA_WINDOWS_HANDLE_TYPE(name)                                                                                                                           \
    template <> struct IsSupportedType<name##__, void> {                                                                                                       \
        static constexpr bool value = false;                                                                                                                   \
    };
...
using UnderlyingImplForTraits = internal::RawPtrBackupRefImpl<
    /*AllowDangling=*/partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kMayDangle),
    /*DisableBRP=*/partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kDisableBRP)>;

#elif PA_BUILDFLAG(USE_RAW_PTR_ASAN_UNOWNED_IMPL)
template <RawPtrTraits Traits>
using UnderlyingImplForTraits = internal::RawPtrAsanUnownedImpl<partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kAllowPtrArithmetic),
...
using UnderlyingImplForTraits = internal::RawPtrHookableImpl<
    /*EnableHooks=*/!partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kDisableHooks)>;

#else
template <RawPtrTraits Traits> using UnderlyingImplForTraits = internal::RawPtrNoOpImpl;
#endif
...
constexpr bool IsPtrArithmeticAllowed([[maybe_unused]] RawPtrTraits Traits)
{
    ...
    return partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kAllowPtrArithmetic);
#else
    return true;
#endif
}
...
// test impl instead.
template <RawPtrTraits Traits>
using ImplForTraits = std::conditional_t<partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kUseCountingImplForTest),
    test::RawPtrCountingImplForTest, UnderlyingImplForTraits<Traits>>;
...
template <typename T, typename SFINAE = void> constexpr inline auto kTypeTraits = RawPtrTraits::kEmpty;
...
template <typename T, RawPtrTraits PointerTraits = RawPtrTraits::kEmpty> class PA_TRIVIAL_ABI PA_GSL_POINTER raw_ptr {
public:
    ...
    static_assert(partition_alloc::internal::AreValidFlags(Traits), "Unknown raw_ptr trait(s)");
    static_assert(raw_ptr_traits::IsSupportedType<T>::value, "raw_ptr<T> doesn't work with this kind of pointee type T");

    static constexpr bool kZeroOnConstruct = Impl::kMustZeroOnConstruct
        || (PA_BUILDFLAG(RAW_PTR_ZERO_ON_CONSTRUCT) && !partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kAllowUninitialized));
    static constexpr bool kZeroOnMove = Impl::kMustZeroOnMove || PA_BUILDFLAG(RAW_PTR_ZERO_ON_MOVE);
    static constexpr bool kZeroOnDestruct = Impl::kMustZeroOnDestruct || PA_BUILDFLAG(RAW_PTR_ZERO_ON_DESTRUCT);
...
// BackupRefPtr), or even for NoOpImpl when zeroing is requested.
    ...
    template <RawPtrTraits PassedTraits, typename = std::enable_if_t<Traits != PassedTraits>>
    PA_ALWAYS_INLINE constexpr explicit raw_ptr(const raw_ptr<T, PassedTraits>& p) noexcept
        : wrapped_ptr_(
            Impl::WrapRawPtrForDuplication(raw_ptr_traits::ImplForTraits<raw_ptr<T, PassedTraits>::Traits>::UnsafelyUnwrapPtrForDuplication(p.wrapped_ptr_)))
    {
        Impl::Trace(tracer_.owner_id(), wrapped_ptr_);
        ...
        static_assert(Traits == (raw_ptr<T, PassedTraits>::Traits | RawPtrTraits::kMayDangle));
    }
    ...
    // let move operations degrade to copy, which handles it well.
    template <RawPtrTraits PassedTraits, typename = std::enable_if_t<Traits != PassedTraits>>
    PA_ALWAYS_INLINE constexpr raw_ptr& operator=(const raw_ptr<T, PassedTraits>& p) noexcept
    {
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*> && !std::is_void_v<typename std::remove_cv<T>::type>>>
    // NOLINTNEXTLINE(google-explicit-constructor)
    PA_ALWAYS_INLINE constexpr raw_ptr(const raw_ptr<U, Traits>& ptr) noexcept
        : wrapped_ptr_(Impl::Duplicate(Impl::template Upcast<T, U>(ptr.wrapped_ptr_)))
    {
        Impl::Trace(tracer_.owner_id(), wrapped_ptr_);
    }
    // Deliberately implicit in order to support implicit upcast.
    ...
    // Upcast assignment
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*> && !std::is_void_v<typename std::remove_cv<T>::type>>>
    PA_ALWAYS_INLINE constexpr raw_ptr& operator=(const raw_ptr<U, Traits>& ptr) noexcept
    {
        // If it was the same type, another overload would've been used.
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible_v<U*, T*> && !std::is_void_v<typename std::remove_cv<T>::type>>>
    PA_ALWAYS_INLINE constexpr raw_ptr& operator=(raw_ptr<U, Traits>&& ptr) noexcept
    {
        // If it was the same type, another overload would've been used.
        static_assert(!std::is_same_v<raw_ptr, std::decay_t<decltype(ptr)>>);
    ...
    public:
        ...
        PA_ALWAYS_INLINE PA_CONSTEXPR_DTOR ~EphemeralRawAddr()
        {
            original = copy;
        }
    ...
    template <typename U = T, typename = std::enable_if_t<!std::is_void_v<typename std::remove_cv<U>::type>>> PA_ALWAYS_INLINE constexpr U& operator*() const
    {
    ...
    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    // NOLINTNEXTLINE(google-explicit-constructor)
    PA_ALWAYS_INLINE constexpr operator T*() const
    {
        return GetForExtraction();
    }
    template <typename U> PA_ALWAYS_INLINE constexpr explicit operator U*() const
    {
    ...
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot increment raw_ptr unless AllowPtrArithmetic trait is present.");
        wrapped_ptr_ = Impl::Advance(wrapped_ptr_, 1, true);
        return *this;
    }
    PA_ALWAYS_INLINE constexpr raw_ptr& operator--()
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot decrement raw_ptr unless AllowPtrArithmetic trait is present.");
    ...
    template <typename Z, typename U = T,
        typename = std::enable_if_t<!std::is_void_v<typename std::remove_cv<U>::type> && partition_alloc::internal::is_offset_type<Z>>>
    PA_ALWAYS_INLINE constexpr U& operator[](Z delta_elems) const
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot index raw_ptr unless AllowPtrArithmetic trait is present.");
        // Call SafelyUnwrapPtrForDereference() to simulate what GetForDereference()
        // does, but without creating a temporary.
        return *Impl::SafelyUnwrapPtrForDereference(Impl::Advance(wrapped_ptr_, delta_elems, false));
    }
    ...
    template <typename Z> PA_ALWAYS_INLINE friend constexpr raw_ptr operator+(const raw_ptr& p, Z delta_elems)
    {
        // Don't check `is_offset_type<Z>` here, as existence of `Advance` is
        // already gated on that, and we'd get double errors.
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot add to raw_ptr unless AllowPtrArithmetic trait is present.");
        raw_ptr result = Impl::Advance(p.wrapped_ptr_, delta_elems, false);
        return result;
    }
    template <typename Z> PA_ALWAYS_INLINE friend constexpr raw_ptr operator+(Z delta_elems, const raw_ptr& p)
    {
    ...
    PA_ALWAYS_INLINE friend constexpr ptrdiff_t operator-(const raw_ptr& p1, const raw_ptr& p2)
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot subtract raw_ptrs unless AllowPtrArithmetic trait is present.");
        return Impl::GetDeltaElems(p1.wrapped_ptr_, p2.wrapped_ptr_);
    }
    PA_ALWAYS_INLINE friend constexpr ptrdiff_t operator-(T* p1, const raw_ptr& p2)
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot subtract raw_ptrs unless AllowPtrArithmetic trait is present.");
        return Impl::GetDeltaElems(p1, p2.wrapped_ptr_);
    }
    PA_ALWAYS_INLINE friend constexpr ptrdiff_t operator-(const raw_ptr& p1, T* p2)
    {
        static_assert(raw_ptr_traits::IsPtrArithmeticAllowed(Traits), "cannot subtract raw_ptrs unless AllowPtrArithmetic trait is present.");
    ...
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr bool operator==(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr bool operator!=(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
#if PA_HAVE_SPACESHIP_OPERATOR
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr auto operator<=>(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
#else
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr bool operator<(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr bool operator>(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
    template <typename U, typename V, RawPtrTraits R1, RawPtrTraits R2> friend constexpr bool operator<=(const raw_ptr<U, R1>& lhs, const raw_ptr<V, R2>& rhs);
    ...
    template <typename U> PA_ALWAYS_INLINE friend constexpr bool operator==(const raw_ptr& lhs, U* rhs)
    {
    ...
    template <typename U> PA_ALWAYS_INLINE friend constexpr bool operator==(U* lhs, const raw_ptr& rhs)
    {
...
#if PA_HAVE_SPACESHIP_OPERATOR
    template <typename U> PA_ALWAYS_INLINE friend constexpr auto operator<=>(const raw_ptr& lhs, U* rhs)
    {
        return lhs.GetForComparison() <=> rhs;
    }
    template <typename U> PA_ALWAYS_INLINE friend constexpr auto operator<=>(U* lhs, const raw_ptr& rhs)
    {
    ...
    PA_ALWAYS_INLINE friend constexpr bool operator==(const raw_ptr& lhs, std::nullptr_t)
    {
        return !lhs;
    }
    PA_ALWAYS_INLINE friend constexpr bool operator!=(const raw_ptr& lhs, std::nullptr_t)
    ...
    PA_ALWAYS_INLINE friend constexpr bool operator==(std::nullptr_t, const raw_ptr& rhs)
    {
        return !rhs;
    }
    PA_ALWAYS_INLINE friend constexpr bool operator!=(std::nullptr_t, const raw_ptr& rhs)
    ...
    PA_ALWAYS_INLINE friend constexpr void swap(raw_ptr& lhs, raw_ptr& rhs) noexcept
    {
...
private:
    ...
    template <typename U, base::RawPtrTraits R> friend class raw_ptr;
};

template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
PA_ALWAYS_INLINE constexpr bool operator==(const raw_ptr<U, Traits1>& lhs, const raw_ptr<V, Traits2>& rhs)
{
...
#if PA_HAVE_SPACESHIP_OPERATOR
template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
PA_ALWAYS_INLINE constexpr auto operator<=>(const raw_ptr<U, Traits1>& lhs, const raw_ptr<V, Traits2>& rhs)
{
    return lhs.GetForComparison() <=> rhs.GetForComparison();
}
#else
...
template <typename T, RawPtrTraits Traits> struct IsRawPtr<raw_ptr<T, Traits>> : std::true_type { };

template <typename T> inline constexpr bool IsRawPtrV = IsRawPtr<T>::value;

template <typename T> inline constexpr bool IsRawPtrMayDangleV = false;

template <typename T, RawPtrTraits Traits>
inline constexpr bool IsRawPtrMayDangleV<raw_ptr<T, Traits>> = partition_alloc::internal::ContainsFlags(Traits, RawPtrTraits::kMayDangle);

// Template helpers for working with T* or raw_ptr<T>.
template <typename T> struct IsRawPointerHelper : std::false_type { };

template <typename T> struct IsRawPointerHelper<T*> : std::true_type { };

template <typename T, RawPtrTraits Traits> struct IsRawPointerHelper<raw_ptr<T, Traits>> : std::true_type { };

template <typename T> inline constexpr bool IsRawPointer = IsRawPointerHelper<T>::value;

template <typename T> struct RemoveRawPointer {
    using type = T;
};

template <typename T> struct RemoveRawPointer<T*> {
    using type = T;
};

template <typename T, RawPtrTraits Traits> struct RemoveRawPointer<raw_ptr<T, Traits>> {
    using type = T;
};

template <typename T> using RemoveRawPointerT = typename RemoveRawPointer<T>::type;
...
constexpr inline auto DisableDanglingPtrDetection = base::RawPtrTraits::kMayDangle;
...
//
// This is not meant to be added manually. You can ignore this flag.
constexpr inline auto AcrossTasksDanglingUntriaged = base::RawPtrTraits::kMayDangle;
...
// silently enforce it.
constexpr inline auto AllowUninitialized = base::RawPtrTraits::kAllowUninitialized;
...
template <typename T, base::RawPtrTraits Traits = base::RawPtrTraits::kEmpty> using MayBeDangling = base::raw_ptr<T, Traits | base::RawPtrTraits::kMayDangle>;

namespace std {

// Override so set/map lookups do not create extra raw_ptr. This also allows
// dangling pointers to be used for lookup.
template <typename T, base::RawPtrTraits Traits> struct less<raw_ptr<T, Traits>> {
...
template <typename T, base::RawPtrTraits Traits> struct hash<raw_ptr<T, Traits>> {
...
template <typename T, base::RawPtrTraits Traits> struct iterator_traits<raw_ptr<T, Traits>> {
...
template <typename T, ::base::RawPtrTraits Traits> struct pointer_traits<::raw_ptr<T, Traits>> {
