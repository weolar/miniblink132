...
static_assert(sizeof(raw_ptr<void>) == sizeof(void*), "raw_ptr shouldn't add memory overhead");
static_assert(sizeof(raw_ptr<int>) == sizeof(int*), "raw_ptr shouldn't add memory overhead");
static_assert(sizeof(raw_ptr<std::string>) == sizeof(std::string*), "raw_ptr shouldn't add memory overhead");
#endif

#if !PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) && !PA_BUILDFLAG(USE_RAW_PTR_ASAN_UNOWNED_IMPL) && !PA_BUILDFLAG(USE_RAW_PTR_HOOKABLE_IMPL)                     \
    && !PA_BUILDFLAG(RAW_PTR_ZERO_ON_MOVE) && !PA_BUILDFLAG(RAW_PTR_ZERO_ON_DESTRUCT)
// |is_trivially_copyable| assertion means that arrays/vectors of raw_ptr can
// be copied by memcpy.
static_assert(std::is_trivially_copyable_v<raw_ptr<void>>, "raw_ptr should be trivially copyable");
static_assert(std::is_trivially_copyable_v<raw_ptr<int>>, "raw_ptr should be trivially copyable");
static_assert(std::is_trivially_copyable_v<raw_ptr<std::string>>, "raw_ptr should be trivially copyable");
...
#if !PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) && !PA_BUILDFLAG(USE_RAW_PTR_ASAN_UNOWNED_IMPL) && !PA_BUILDFLAG(USE_RAW_PTR_HOOKABLE_IMPL)                     \
    && !PA_BUILDFLAG(RAW_PTR_ZERO_ON_CONSTRUCT) && !PA_BUILDFLAG(RAW_PTR_ZERO_ON_DESTRUCT)
...
static_assert(std::is_trivially_default_constructible_v<raw_ptr<void>>, "raw_ptr should be trivially default constructible");
static_assert(std::is_trivially_default_constructible_v<raw_ptr<int>>, "raw_ptr should be trivially default constructible");
static_assert(std::is_trivially_default_constructible_v<raw_ptr<std::string>>, "raw_ptr should be trivially default constructible");
#endif // !PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) &&                                                                                                        \
    // !PA_BUILDFLAG(USE_RAW_PTR_ASAN_UNOWNED_IMPL) &&                                                                                                         \
    // !PA_BUILDFLAG(USE_RAW_PTR_HOOKABLE_IMPL) &&                                                                                                             \
...
static_assert([]() constexpr {
    ...
    Int* i = new Int();
    {
        ...
        raw_ptr<Int, base::RawPtrTraits::kMayDangle> r4(r); // raw_ptr(const raw_ptr<DifferentTraits>&)
        r4 = r2; // operator=(const raw_ptr<DifferentTraits>&)
        // (There is no move-version of DifferentTraits.)
        [[maybe_unused]] raw_ptr<IntBase> r5(r2); // raw_ptr(const raw_ptr<Convertible>&)
        [[maybe_unused]] raw_ptr<IntBase> r6(std::move(r2)); // raw_ptr(raw_ptr<Convertible>&&)
...
template <typename T> constexpr auto kTypeTraits<T, std::enable_if_t<std::is_base_of_v<BaseWithTypeBasedTraits, T>>> = RawPtrTraits::kDummyForTest;
} // namespace base::raw_ptr_traits

// `raw_ptr<T>` should have traits based on specialization of `kTypeTraits<T>`.
static_assert(!ContainsFlags(raw_ptr<StructWithoutTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
static_assert(ContainsFlags(raw_ptr<BaseWithTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
static_assert(ContainsFlags(raw_ptr<DerivedWithTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
...
template <typename T> using CountingRawPtr = raw_ptr<T, base::RawPtrTraits::kUseCountingImplForTest | base::RawPtrTraits::kAllowPtrArithmetic>;
...
using CountingRawPtrMayDangle
    = raw_ptr<T, base::RawPtrTraits::kMayDangle | base::RawPtrTraits::kUseCountingImplForTest | base::RawPtrTraits::kAllowPtrArithmetic>;

// Ensure that the `kUseCountingImplForTest` flag selects the test impl.
static_assert(std::is_same_v<CountingRawPtrMayDangle<int>::Impl, RawPtrCountingImpl>);

template <typename T> using CountingRawPtrUninitialized = raw_ptr<T, base::RawPtrTraits::kUseCountingImplForTest | base::RawPtrTraits::kAllowUninitialized>;

// Ensure that the `kUseCountingImplForTest` flag selects the test impl.
static_assert(std::is_same_v<CountingRawPtrUninitialized<int>::Impl, RawPtrCountingImpl>);
...
TEST_F(RawPtrTest, NullExtractNoDereference)
{
    ...
    int* raw = ptr;
    std::ignore = raw;
    EXPECT_THAT((CountingRawPtrExpectations { .get_for_dereference_cnt = 0, .get_for_extraction_cnt = 1, .get_for_comparison_cnt = 0 }), CountersMatch());
}
...
TEST_F(RawPtrTest, OperatorEQCastHierarchy)
{
    ...
    ASSERT_NE(reinterpret_cast<uintptr_t>(raw_base2_ptr), reinterpret_cast<uintptr_t>(raw_derived_ptr));
    ...
    EXPECT_NE(reinterpret_cast<uintptr_t>(checked_base2_ptr.get()), reinterpret_cast<uintptr_t>(checked_derived_ptr.get()));
    EXPECT_NE(reinterpret_cast<uintptr_t>(raw_base2_ptr), reinterpret_cast<uintptr_t>(checked_derived_ptr.get()));
    EXPECT_NE(reinterpret_cast<uintptr_t>(checked_base2_ptr.get()), reinterpret_cast<uintptr_t>(raw_derived_ptr));
...
TEST_F(RawPtrTest, Cast)
{
    ...
    raw_ptr<Derived> checked_derived_ptr2 = static_cast<Derived*>(checked_base1_ptr);
    ...
    raw_ptr<volatile Derived> checked_volatile_derived_ptr = raw_volatile_derived_ptr;
    ...
    raw_ptr<Derived> checked_derived_ptr4 = static_cast<Derived*>(checked_void_ptr);
...
{
    int x = 123;
    FunctionWithRawPtrParameter(raw_ptr<int>(&x), // Temporary that will be moved into the function.
        &x);
}
...
#if PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) || PA_BUILDFLAG(USE_RAW_PTR_ASAN_UNOWNED_IMPL) || PA_BUILDFLAG(USE_RAW_PTR_HOOKABLE_IMPL)                        \
    || PA_BUILDFLAG(RAW_PTR_ZERO_ON_DESTRUCT)
    EXPECT_EQ((int)number_of_cleared_elements, RawPtrCountingImpl::release_wrapped_ptr_cnt);
#else
...
struct DerivedType1 : public BaseStruct {
    explicit DerivedType1(int a, int b)
        : BaseStruct(a)
        , b(b)
    {
    }
...
struct DerivedType2 : public BaseStruct {
    explicit DerivedType2(int a, int c)
        : BaseStruct(a)
        , c(c)
    {
    }
...
TEST_F(RawPtrTest, DerivedStructsComparison)
{
    ...
    EXPECT_NE(checked_derived1_ptr, static_cast<BaseStruct*>(checked_derived2_ptr.get()));
    EXPECT_NE(static_cast<BaseStruct*>(checked_derived1_ptr.get()), checked_derived2_ptr);
}

class PmfTestBase {
public:
    int MemFunc(char, double) const
    {
        return 11;
    }
};
...
TEST_F(RawPtrTest, PointerToMemberFunction)
{
    ...
    int (PmfTestDerived::*pmf_derived_base)(char, double) const = &PmfTestDerived::MemFunc;
    int (PmfTestDerived::*pmf_derived_derived)(float, double) = &PmfTestDerived::MemFunc;
...
TEST_F(RawPtrTest, CrossKindConversion)
{
    ...
    EXPECT_THAT(
        (CountingRawPtrExpectations {
            .wrap_raw_ptr_cnt = 0, .get_for_dereference_cnt = 0, .get_for_extraction_cnt = 0, .wrap_raw_ptr_for_dup_cnt = 2, .get_for_duplication_cnt = 2 }),
        CountersMatch());
}
...
TEST_F(RawPtrTest, ToAddressDoesNotDereference)
{
    ...
    std::ignore = raw;
    EXPECT_THAT(
        (CountingRawPtrExpectations { .get_for_dereference_cnt = 0, .get_for_extraction_cnt = 1, .get_for_comparison_cnt = 0, .get_for_duplication_cnt = 0 }),
        CountersMatch());
}
...
TEST_F(RawPtrTest, EphemeralRawAddrPointerPointer)
{
    ...
    EXPECT_NE(&ptr.AsEphemeralRawAddr(), reinterpret_cast<int**>(std::addressof(ptr)));
...
TEST_F(RawPtrTest, EphemeralRawAddrPointerReference)
{
    ...
    EXPECT_NE(&static_cast<int*&>(ptr.AsEphemeralRawAddr()), reinterpret_cast<int**>(std::addressof(ptr)));
    // But inner pointer should point to the same address.
...
#if PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL) && !defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
...
protected:
    ...
    size_t GetRequestSizeThatFills512BSlot()
    {
        ...
        size_t requested_size = allocator_.root()->AdjustSizeForExtrasSubtract(slot_size);
        // Verify that we're indeed filling up the slot.
        // (ASSERT_EQ is more appropriate here, because it verifies test setup, but
        // it doesn't compile.)
        EXPECT_EQ(requested_size, allocator_.root()->AllocationCapacityFromRequestedSize(requested_size));
        return requested_size;
    }

    partition_alloc::PartitionAllocator allocator_ = partition_alloc::PartitionAllocator([] {
        partition_alloc::PartitionOptions opts;
        opts.backup_ref_ptr = partition_alloc::PartitionOptions::kEnabled;
        opts.memory_tagging = { .enabled
            = base::CPU::GetInstanceNoAllocation().has_mte() ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled };
        return opts;
    }());
};
...
TEST_F(BackupRefPtrTest, Basic)
{
    ...
    int* raw_ptr1 = reinterpret_cast<int*>(allocator_.root()->Alloc(sizeof(int), ""));
    ...
    *raw_ptr1 = 42;
    ...
    EXPECT_NE(partition_alloc::UntagPtr(raw_ptr1), partition_alloc::UntagPtr(raw_ptr2));
    ...
    EXPECT_EQ(partition_alloc::UntagPtr(raw_ptr1), partition_alloc::UntagPtr(raw_ptr3));
...
{
    uint64_t* raw_ptr1 = reinterpret_cast<uint64_t*>(allocator_.root()->Alloc(sizeof(uint64_t), ""));
    raw_ptr<uint64_t, DisableDanglingPtrDetection> wrapped_ptr1 = raw_ptr1;
    EXPECT_EQ(allocator_.root()->total_size_of_brp_quarantined_bytes.load(std::memory_order_relaxed), 0U);
    EXPECT_EQ(allocator_.root()->total_count_of_brp_quarantined_slots.load(std::memory_order_relaxed), 0U);

    // Memory should get quarantined.
    allocator_.root()->Free(raw_ptr1);
    EXPECT_GT(allocator_.root()->total_size_of_brp_quarantined_bytes.load(std::memory_order_relaxed), 0U);
    EXPECT_EQ(allocator_.root()->total_count_of_brp_quarantined_slots.load(std::memory_order_relaxed), 1U);
...
{
    uint64_t* ptr = reinterpret_cast<uint64_t*>(allocator_.root()->Alloc(sizeof(uint64_t), ""));
    raw_ptr<uint64_t, DisableDanglingPtrDetection> wrapped_ptr = ptr;
    ASSERT_EQ(allocator_.root()->total_count_of_brp_quarantined_slots.load(std::memory_order_relaxed), 0U);

    // Make the pointer dangle. Memory will get quarantined.
    allocator_.root()->Free(ptr);
    ASSERT_EQ(allocator_.root()->total_count_of_brp_quarantined_slots.load(std::memory_order_relaxed), 1U);
...
void RunBackupRefPtrImplAdvanceTest(partition_alloc::PartitionAllocator& allocator, size_t requested_size)
{
    ...
    *protected_ptr = ' ';
    ...
    raw_ptr<FunkyArray, AllowPtrArithmetic> protected_arr_ptr = reinterpret_cast<FunkyArray*>(ptr);
...
TEST_F(BackupRefPtrTest, GetDeltaElems)
{
    ...
    ASSERT_LT(partition_alloc::UntagPtr(ptr1),
        partition_alloc::UntagPtr(ptr2)); // There should be a ref-count between slots.
    raw_ptr<char, AllowPtrArithmetic> protected_ptr1 = ptr1;
    raw_ptr<char, AllowPtrArithmetic> protected_ptr1_2 = ptr1 + 1;
    raw_ptr<char, AllowPtrArithmetic> protected_ptr1_3 = ptr1 + requested_size - 1;
    ...
    EXPECT_EQ(protected_ptr1_3 - protected_ptr1, checked_cast<ptrdiff_t>(requested_size) - 1);
    EXPECT_EQ(protected_ptr1 - protected_ptr1_3, -checked_cast<ptrdiff_t>(requested_size) + 1);
    EXPECT_EQ(protected_ptr1_4 - protected_ptr1, checked_cast<ptrdiff_t>(requested_size));
    EXPECT_EQ(protected_ptr1 - protected_ptr1_4, -checked_cast<ptrdiff_t>(requested_size));
...
{
    return allocator.root()->total_size_of_brp_quarantined_bytes.load(std::memory_order_relaxed) == 0;
}

struct BoundRawPtrTestHelper {
    static BoundRawPtrTestHelper* Create(partition_alloc::PartitionAllocator& allocator)
    {
        return new (allocator.root()->Alloc(sizeof(BoundRawPtrTestHelper), "")) BoundRawPtrTestHelper(allocator);
    }

    explicit BoundRawPtrTestHelper(partition_alloc::PartitionAllocator& allocator)
        : owning_allocator(allocator)
        , once_callback(BindOnce(&BoundRawPtrTestHelper::DeleteItselfAndCheckIfInQuarantine, Unretained(this)))
        , repeating_callback(BindRepeating(&BoundRawPtrTestHelper::DeleteItselfAndCheckIfInQuarantine, Unretained(this)))
    {
    }
...
TEST_F(BackupRefPtrTest, Bind)
{
    ...
    auto* object_for_repeating_callback1 = BoundRawPtrTestHelper::Create(allocator_);
    ...
    auto* object_for_repeating_callback2 = BoundRawPtrTestHelper::Create(allocator_);
...
{
    void* ptr = allocator_.root()->Alloc(16);
    auto* ref_count = allocator_.root()->InSlotMetadataPointerFromObjectForTesting(ptr);
...
public:
    ...
    {
        enabled_feature_list_.InitWithFeaturesAndParameters({ { features::kPartitionAllocDanglingPtr, { { "mode", "crash" } } } }, { /* disabled_features */ });
...
TEST_F(BackupRefPtrTest, RawPtrNotDangling)
{
    ...
    BASE_EXPECT_DEATH(
        ...
        AllOf(HasSubstr("Detected dangling raw_ptr"), HasSubstr("The memory was freed at:"), HasSubstr("The dangling raw_ptr was released at:")));
#else
    allocator_.root()->Free(ptr);
...
TEST_F(BackupRefPtrTest, RawPtrExtractAsDangling)
{
    ...
    raw_ptr<int> ptr = static_cast<int*>(allocator_.root()->Alloc(sizeof(int), ""));
    allocator_.root()->Free(ptr.ExtractAsDangling()); // No dangling raw_ptr reported.
...
TEST_F(BackupRefPtrTest, SpatialAlgoCompat)
{
    ...
    uint32_t* ptr = reinterpret_cast<uint32_t*>(allocator_.root()->Alloc(requested_size));
    ...
    for (CountingRawPtr<uint32_t> counting_ptr_i = counting_ptr; counting_ptr_i < counting_ptr_end; counting_ptr_i++) {
    ...
    for (CountingRawPtr<uint32_t> counting_ptr_i = counting_ptr; counting_ptr_i < ptr_end; counting_ptr_i++) {
        *counting_ptr_i ^= *counting_ptr_i + 1;
    }
    ...
    size_t iter_cnt = 0;
    for (uint32_t *ptr_i = counting_ptr, *ptr_i_end = counting_ptr_end; ptr_i < ptr_i_end; ptr_i++) {
        *ptr_i ^= *ptr_i + 1;
        iter_cnt++;
    }
...
TEST_F(BackupRefPtrTest, WriteAfterFree)
{
    ...
    raw_ptr<uint64_t, DisableDanglingPtrDetection> ptr = static_cast<uint64_t*>(allocator_.root()->Alloc(sizeof(uint64_t), ""));
...
static_assert(kCustomQuarantineByte != partition_alloc::internal::kQuarantinedByte);
...
{
    partition_alloc::PartitionAllocHooks::SetQuarantineOverrideHook(CustomQuarantineHook);
    uint8_t* native_ptr = static_cast<uint8_t*>(allocator_.root()->Alloc(sizeof(uint8_t), ""));
    *native_ptr = 0;
    {
        ...
        EXPECT_EQ(*partition_alloc::internal::TagPtr(native_ptr), kCustomQuarantineByte);
...
TEST_F(BackupRefPtrTest, RawPtrTraits_DisableBRP)
{
    ...
    constexpr uint32_t kQuarantined2Bytes = partition_alloc::internal::kQuarantinedByte | (partition_alloc::internal::kQuarantinedByte << 8);
    constexpr uint32_t kQuarantined4Bytes = kQuarantined2Bytes | (kQuarantined2Bytes << 16);

    {
        raw_ptr<unsigned int, DanglingUntriaged> ptr = static_cast<unsigned int*>(allocator_.root()->Alloc(sizeof(unsigned int), ""));
        *ptr = 0;
        // Freeing would  update the MTE tag so use |TagPtr()| to dereference it
        // below.
        allocator_.root()->Free(ptr);
#if PA_BUILDFLAG(DCHECKS_ARE_ON) || PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)
        ...
        raw_ptr<unsigned int, DanglingUntriaged> dangling_ptr = partition_alloc::internal::TagPtr(ptr.get());
        EXPECT_DEATH_IF_SUPPORTED(*dangling_ptr = 0, "");
#else
        EXPECT_EQ(kQuarantined4Bytes, *partition_alloc::internal::TagPtr(ptr.get()));
#endif
    }
    // raw_ptr with DisableBRP, BRP is expected to be off.
    {
        raw_ptr<unsigned int, DanglingUntriaged | RawPtrTraits::kDisableBRP> ptr
            = static_cast<unsigned int*>(allocator_.root()->Alloc(sizeof(unsigned int), ""));
        *ptr = 0;
        allocator_.root()->Free(ptr);
        ...
        // the MTE tag.
        EXPECT_NE(kQuarantined4Bytes, *partition_alloc::internal::TagPtr(ptr.get()));
    }
...
#define F(name)                                                                                                                                                \
    template <typename... T> static void name##_adapter(T...)                                                                                                  \
    {                                                                                                                                                          \
        Get()->name##_count++;                                                                                                                                 \
    }                                                                                                                                                          \
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_INSTANCE_TRACER) && PA_BUILDFLAG(USE_RAW_PTR_BACKUP_REF_IMPL)
TEST(RawPtrInstanceTracerTest, CreateAndDestroy)
{
    auto owned = std::make_unique<int>(8);

    EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned.get()), IsEmpty());

    {
        raw_ptr<int> ptr1 = owned.get();
        const auto stacks = InstanceTracer::GetStackTracesForAddressForTest(owned.get());
        EXPECT_THAT(stacks, SizeIs(1));
        {
            // A second raw_ptr to the same object should result in an additional
            // stack trace.
            raw_ptr<int> ptr2 = owned.get();
            EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned.get()), SizeIs(2));
        }
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned.get()), Eq(stacks));
    }
    EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned.get()), IsEmpty());
}
...
TEST(RawPtrInstanceTracerTest, CopyConstruction)
{
    ...
    {
        raw_ptr<int> ptr1 = owned.get();
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned.get()), SizeIs(1));
        {
...
TEST(RawPtrInstanceTracerTest, CopyAssignment)
{
    ...
    EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned1.get()), IsEmpty());
    EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned2.get()), IsEmpty());

    {
        raw_ptr<int> ptr1 = owned1.get();
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned1.get()), SizeIs(1));
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned2.get()), IsEmpty());

        raw_ptr<int> ptr2 = owned2.get();
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned1.get()), SizeIs(1));
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned2.get()), SizeIs(1));

        ptr2 = ptr1;
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned1.get()), SizeIs(2));
        EXPECT_THAT(InstanceTracer::GetStackTracesForAddressForTest(owned2.get()), IsEmpty());
    }
