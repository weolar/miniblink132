...
template <class T, RawPtrTraits Traits> class raw_ref;

namespace internal {

template <class T> struct is_raw_ref : std::false_type { };

template <class T, RawPtrTraits Traits> struct is_raw_ref<::base::raw_ref<T, Traits>> : std::true_type { };

template <class T> constexpr inline bool is_raw_ref_v = is_raw_ref<T>::value;
...
template <class T, RawPtrTraits ReferenceTraits = RawPtrTraits::kEmpty> class PA_TRIVIAL_ABI PA_GSL_POINTER raw_ref {
public:
    ...
    constexpr static auto Traits = ReferenceTraits | raw_ptr_traits::kTypeTraits<T>;
...
public:
    ...
    // allow it.
    template <class U, RawPtrTraits PassedTraits, class = std::enable_if_t<std::is_convertible_v<U&, T&>>>
    ...
    // allow it.
    template <class U, RawPtrTraits PassedTraits, class = std::enable_if_t<std::is_convertible_v<U&, T&>>>
    PA_ALWAYS_INLINE constexpr raw_ref& operator=(const raw_ref<U, PassedTraits>& p) noexcept
    {
        PA_RAW_PTR_CHECK(p.inner_); // Catch use-after-move.
        inner_.operator=(p.inner_);
        return *this;
    ...
    // allow it.
    template <class U, RawPtrTraits PassedTraits, class = std::enable_if_t<std::is_convertible_v<U&, T&>>>
    PA_ALWAYS_INLINE constexpr raw_ref& operator=(raw_ref<U, PassedTraits>&& p) noexcept
    {
        PA_RAW_PTR_CHECK(p.inner_); // Catch use-after-move.
        inner_.operator=(std::move(p.inner_));
        if constexpr (kNeedClearAfterMove) {
    ...
    PA_ALWAYS_INLINE constexpr T* operator->() const PA_ATTRIBUTE_RETURNS_NONNULL
    {
        PA_RAW_PTR_CHECK(inner_); // Catch use-after-move.
        return inner_.operator->();
    }
    ...
    PA_ALWAYS_INLINE friend constexpr void swap(raw_ref& lhs, raw_ref& rhs) noexcept
    {
    ...
    friend constexpr bool operator==(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs);
    template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
    friend constexpr bool operator!=(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs);
#if PA_HAVE_SPACESHIP_OPERATOR
    template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
    friend constexpr auto operator<=>(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs);
#else
    template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
    friend constexpr bool operator<(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs);
    template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
    ...
    PA_ALWAYS_INLINE friend constexpr bool operator==(const raw_ref& lhs, const U& rhs)
    {
        PA_RAW_PTR_CHECK(lhs.inner_); // Catch use-after-move.
    ...
    template <class U, class = std::enable_if_t<!internal::is_raw_ref_v<U>, void>>
    PA_ALWAYS_INLINE friend constexpr bool operator==(const U& lhs, const raw_ref& rhs)
    {
        PA_RAW_PTR_CHECK(rhs.inner_); // Catch use-after-move.
...
#if PA_HAVE_SPACESHIP_OPERATOR
    template <class U, class = std::enable_if_t<!internal::is_raw_ref_v<U>, void>>
    PA_ALWAYS_INLINE friend constexpr auto operator<=>(const raw_ref& lhs, const U& rhs)
    {
        PA_RAW_PTR_CHECK(lhs.inner_); // Catch use-after-move.
        return lhs.inner_ <=> &rhs;
    }
    template <class U, class = std::enable_if_t<!internal::is_raw_ref_v<U>, void>>
    PA_ALWAYS_INLINE friend constexpr auto operator<=>(const U& lhs, const raw_ref& rhs)
    {
        PA_RAW_PTR_CHECK(rhs.inner_); // Catch use-after-move.
        return &lhs <=> rhs.inner_;
    }
...
private:
    template <class U, RawPtrTraits R> friend class raw_ref;

    Inner inner_;
};

template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
PA_ALWAYS_INLINE constexpr bool operator==(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs)
{
    PA_RAW_PTR_CHECK(lhs.inner_); // Catch use-after-move.
    PA_RAW_PTR_CHECK(rhs.inner_); // Catch use-after-move.
...
#if PA_HAVE_SPACESHIP_OPERATOR
template <typename U, typename V, RawPtrTraits Traits1, RawPtrTraits Traits2>
PA_ALWAYS_INLINE constexpr auto operator<=>(const raw_ref<U, Traits1>& lhs, const raw_ref<V, Traits2>& rhs)
{
    PA_RAW_PTR_CHECK(lhs.inner_); // Catch use-after-move.
    PA_RAW_PTR_CHECK(rhs.inner_); // Catch use-after-move.
    return lhs.inner_ <=> rhs.inner_;
...
template <class T> raw_ref(const T&) -> raw_ref<const T>;

// Template helpers for working with raw_ref<T>.
template <typename T> struct IsRawRef : std::false_type { };

template <typename T, RawPtrTraits Traits> struct IsRawRef<raw_ref<T, Traits>> : std::true_type { };

template <typename T> inline constexpr bool IsRawRefV = IsRawRef<T>::value;

template <typename T> struct RemoveRawRef {
    using type = T;
};

template <typename T, RawPtrTraits Traits> struct RemoveRawRef<raw_ref<T, Traits>> {
    using type = T;
};

template <typename T> using RemoveRawRefT = typename RemoveRawRef<T>::type;

} // namespace base

using base::raw_ref;

template <base::RawPtrTraits Traits = base::RawPtrTraits::kEmpty, typename T> auto ToRawRef(T& ref)
{
...
template <typename T, base::RawPtrTraits Traits> struct less<raw_ref<T, Traits>> {
...
template <typename T, ::base::RawPtrTraits Traits> struct pointer_traits<::raw_ref<T, Traits>> {
    using pointer = ::raw_ref<T, Traits>;
    using element_type = T;
    using difference_type = ptrdiff_t;

    template <typename U> using rebind = ::raw_ref<U, Traits>;
