...
public:
    constexpr uint64_t owner_id() const
    {
        return 0;
    }
...
public:
    constexpr InstanceTracer()
        : owner_id_(CreateOwnerId())
    {
    }
    ...
    constexpr InstanceTracer(const InstanceTracer&)
        : InstanceTracer()
    {
    }
    ...
    constexpr InstanceTracer& operator=(const InstanceTracer&)
    {
        return *this;
    }

    constexpr InstanceTracer(InstanceTracer&&)
        : InstanceTracer()
    {
    }
    constexpr InstanceTracer& operator=(InstanceTracer&&)
    {
        return *this;
    }
    ...
    constexpr static void Trace(uint64_t owner_id, bool may_dangle, uintptr_t address)
    {
        if (partition_alloc::internal::base::is_constant_evaluated() || owner_id == 0) {
            return;
        }
    ...
    static std::vector<std::array<const void*, 32>> GetStackTracesForDanglingRefs(uintptr_t allocation);

    PA_COMPONENT_EXPORT(RAW_PTR)
    static std::vector<std::array<const void*, 32>> GetStackTracesForAddressForTest(const void* address);
