...
// will occur.
template <bool AllowDangling = false, bool DisableBRP = false> struct RawPtrBackupRefImpl {
...
private:
    ...
    template <typename T> PA_ALWAYS_INLINE static T* UnpoisonPtr(T* ptr)
    {
        return reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(ptr) & ~OOB_POISON_BIT);
    }

    template <typename T> PA_ALWAYS_INLINE static bool IsPtrOOB(T* ptr)
    {
        return (reinterpret_cast<uintptr_t>(ptr) & OOB_POISON_BIT) == OOB_POISON_BIT;
    }

    template <typename T> PA_ALWAYS_INLINE static T* PoisonOOBPtr(T* ptr)
    {
        return reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(ptr) | OOB_POISON_BIT);
    }
...
public:
    // Wraps a pointer.
    template <typename T> PA_ALWAYS_INLINE static constexpr T* WrapRawPtr(T* ptr)
    {
        ...
        if (IsSupportedAndNotNull(address)) {
            ...
            bool known_constant_zero = __builtin_constant_p(address == 0) && (address == 0);
#else // PA_HAS_BUILTIN(__builtin_constant_p)
            bool known_constant_zero = false;
#endif // PA_HAS_BUILTIN(__builtin_constant_p)

            if (!known_constant_zero) {
                partition_alloc::internal::AddressPoolManagerBitmap::BanSuperPageFromBRPPool(address);
            }
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr void ReleaseWrappedPtr(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return;
        }
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr T* SafelyUnwrapPtrForDereference(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr;
        }
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr T* SafelyUnwrapPtrForExtraction(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr;
        }
    ...
    // freed or not.
    template <typename T> PA_ALWAYS_INLINE static constexpr T* UnsafelyUnwrapPtrForComparison(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr;
        }
    ...
    template <typename To, typename From> PA_ALWAYS_INLINE static constexpr To* Upcast(From* wrapped_ptr)
    {
    ...
    template <typename T> PA_ALWAYS_INLINE static T* VerifyAndPoisonPointerAfterAdvanceOrRetreat(T* unpoisoned_ptr, T* new_ptr)
    {
        ...
        if (IsSupportedAndNotNull(before_addr)) {
            ...
            [[maybe_unused]] const bool is_end = CheckPointerWithinSameAlloc(before_addr, after_addr, size);
#if PA_BUILDFLAG(BACKUP_REF_PTR_POISON_OOB_PTR)
            if (is_end) {
    ...
    template <typename T, typename Z, typename = std::enable_if_t<partition_alloc::internal::is_offset_type<Z>, void>>
    PA_ALWAYS_INLINE static constexpr T* Advance(T* wrapped_ptr, Z delta_elems, bool is_in_pointer_modification)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
        ...
        if (PA_BUILDFLAG(BACKUP_REF_PTR_EXTRA_OOB_CHECKS) || is_in_pointer_modification) {
            return VerifyAndPoisonPointerAfterAdvanceOrRetreat(unpoisoned_ptr, unpoisoned_ptr + delta_elems);
        }
    ...
    // the pointer (as opposed to creating a new one).
    template <typename T, typename Z, typename = std::enable_if_t<partition_alloc::internal::is_offset_type<Z>, void>>
    PA_ALWAYS_INLINE static constexpr T* Retreat(T* wrapped_ptr, Z delta_elems, bool is_in_pointer_modification)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr - delta_elems;
        }
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr ptrdiff_t GetDeltaElems(T* wrapped_ptr1, T* wrapped_ptr2)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
        ...
        if (IsSupportedAndNotNull(address1)) {
            ...
            PA_BASE_CHECK(
                partition_alloc::internal::IsPtrWithinSameAlloc(address2, address1, sizeof(T)) != partition_alloc::internal::PtrPosWithinAlloc::kFarOOB);
        } else {
            PA_BASE_CHECK(!IsSupportedAndNotNull(address2));
        }
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr T* Duplicate(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr;
        }
    ...
    template <typename T> PA_ALWAYS_INLINE static void ReportIfDangling(T* wrapped_ptr)
    {
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr T* WrapRawPtrForDuplication(T* ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return ptr;
        } else {
    ...
    template <typename T> PA_ALWAYS_INLINE static constexpr T* UnsafelyUnwrapPtrForDuplication(T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return wrapped_ptr;
        } else {
    ...
    template <typename T> static constexpr void Trace(uint64_t owner_id, T* wrapped_ptr)
    {
        if (partition_alloc::internal::base::is_constant_evaluated()) {
            return;
        }
...
private:
    ...
    PA_NOINLINE static PA_COMPONENT_EXPORT(RAW_PTR) void AcquireInternal(uintptr_t address);
    PA_NOINLINE static PA_COMPONENT_EXPORT(RAW_PTR) void ReleaseInternal(uintptr_t address);
    PA_NOINLINE static PA_COMPONENT_EXPORT(RAW_PTR) bool IsPointeeAlive(uintptr_t address);
    PA_NOINLINE static PA_COMPONENT_EXPORT(RAW_PTR) void ReportIfDanglingInternal(uintptr_t address);
    ...
    PA_NOINLINE static PA_COMPONENT_EXPORT(RAW_PTR) bool CheckPointerWithinSameAlloc(uintptr_t before_addr, uintptr_t after_addr, size_t type_size);
};
