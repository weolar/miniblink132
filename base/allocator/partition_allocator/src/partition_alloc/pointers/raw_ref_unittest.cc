...
static_assert(std::is_trivially_constructible_v<raw_ref<int>, const int&> == std::is_trivially_constructible_v<raw_ptr<int>, const int&>);
static_assert(std::is_trivially_destructible_v<raw_ref<int>> == std::is_trivially_destructible_v<raw_ptr<int>>);
...
static_assert(std::is_constructible_v<raw_ref<BaseClass>, const raw_ref<SubClass>&>);
static_assert(std::is_constructible_v<raw_ref<BaseClass>, raw_ref<SubClass>&&>);
static_assert(std::is_assignable_v<raw_ref<BaseClass>, raw_ref<SubClass>>);
static_assert(std::is_assignable_v<raw_ref<BaseClass>, const raw_ref<SubClass>&>);
static_assert(std::is_assignable_v<raw_ref<BaseClass>, raw_ref<SubClass>&&>);
// A BaseClass can't be implicitly downcasted.
static_assert(!std::is_constructible_v<raw_ref<SubClass>, raw_ref<BaseClass>>);
static_assert(!std::is_constructible_v<raw_ref<SubClass>, const raw_ref<BaseClass>&>);
static_assert(!std::is_constructible_v<raw_ref<SubClass>, raw_ref<BaseClass>&&>);
static_assert(!std::is_assignable_v<raw_ref<SubClass>, raw_ref<BaseClass>>);
static_assert(!std::is_assignable_v<raw_ref<SubClass>, const raw_ref<BaseClass>&>);
static_assert(!std::is_assignable_v<raw_ref<SubClass>, raw_ref<BaseClass>&&>);
...
static_assert(std::is_constructible_v<raw_ref<const BaseClass>, const SubClass&>);
...
static_assert(!std::is_constructible_v<raw_ref<const SubClass>, const BaseClass&>);
...
static_assert(std::is_same_v<int&, decltype(*std::declval<const raw_ref<int>>())>);
static_assert(std::is_same_v<int&, decltype(*std::declval<raw_ref<int>&>())>);
static_assert(std::is_same_v<int&, decltype(*std::declval<const raw_ref<int>&>())>);
static_assert(std::is_same_v<int&, decltype(*std::declval<raw_ref<int>&&>())>);
static_assert(std::is_same_v<int&, decltype(*std::declval<const raw_ref<int>&&>())>);
// A const T is always returned as const.
static_assert(std::is_same_v<const int&, decltype(*std::declval<raw_ref<const int>>())>);

// The arrow operator gives a (non-null) pointer to the internal reference.
static_assert(std::is_same_v<int*, decltype(std::declval<raw_ref<int>>().operator->())>);
static_assert(std::is_same_v<const int*, decltype(std::declval<raw_ref<const int>>().operator->())>);
...
static_assert([]() constexpr {
    ...
    Int* i = new Int();
    {
        ...
        [[maybe_unused]] raw_ref<IntBase> r5(r2); // raw_ref(const raw_ref<Convertible>&)
        [[maybe_unused]] raw_ref<IntBase> r6(std::move(r2)); // raw_ref(raw_ref<Convertible>&&)
        r2 = r; // Reset after move.
...
static_assert(!ContainsFlags(raw_ref<StructWithoutTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
static_assert(ContainsFlags(raw_ref<BaseWithTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
static_assert(ContainsFlags(raw_ref<DerivedWithTypeBasedTraits>::Traits, base::RawPtrTraits::kDummyForTest));
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) || PA_BUILDFLAG(USE_ASAN_BACKUP_REF_PTR) || PA_BUILDFLAG(DCHECKS_ARE_ON)
...
TEST(RawRefDeathTest, MoveConstructAfterMoveUpCast)
{
    ...
    auto r = raw_ref<SubClass>(s);
    auto moved = std::move(r);
    EXPECT_CHECK_DEATH({ [[maybe_unused]] auto r2 = raw_ref<BaseClass>(std::move(r)); });
}
...
template <typename T> using CountingRawRef = raw_ref<T, base::RawPtrTraits::kUseCountingImplForTest>;
...
template <typename T> using CountingRawRefMayDangle = raw_ref<T, base::RawPtrTraits::kMayDangle | base::RawPtrTraits::kUseCountingImplForTest>;

// Ensure that the `kUseCountingImplForTest` flag selects the test impl.
static_assert(std::is_same_v<CountingRawRefMayDangle<int>::Impl, RawPtrCountingImpl>);
...
TEST(RawRef, OperatorsUseGetForComparison)
{
    ...
    EXPECT_THAT((CountingRawPtrExpectations { .get_for_comparison_cnt = 4 }), CountersMatch());
