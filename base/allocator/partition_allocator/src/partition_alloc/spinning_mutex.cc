...
#if !PA_CONFIG(HAS_LINUX_KERNEL) && !PA_BUILDFLAG(IS_WIN) && !PA_BUILDFLAG(IS_APPLE) && !PA_BUILDFLAG(IS_POSIX) && !PA_BUILDFLAG(IS_FUCHSIA)
...
void SpinningMutex::FutexWait()
{
    ...
    int err = syscall(SYS_futex, &state_, FUTEX_WAIT | FUTEX_PRIVATE_FLAG, kLockedContended, nullptr, nullptr, 0);
...
{
    int saved_errno = errno;
    long retval = syscall(SYS_futex, &state_, FUTEX_WAKE | FUTEX_PRIVATE_FLAG, 1 /* wake up a single waiter */, nullptr, nullptr, 0);
    PA_CHECK(retval != -1);
    errno = saved_errno;
}
