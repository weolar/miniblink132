...
// this gating.
PA_ALWAYS_INLINE constexpr size_t AlignUpInSlotMetadataSizeForApple(size_t in_slot_metadata_size)
{
...
template <typename CountType> static constexpr CountType SafeShift(CountType lhs, int rhs)
{
    return rhs >= std::numeric_limits<CountType>::digits ? 0 : lhs << rhs;
}
template <typename CountType> struct BitField {
    static constexpr CountType None()
    {
        return CountType(0);
    }
    ...
    {
        return (SafeShift<CountType>(1, hi + 1) - 1) & ~(SafeShift<CountType>(1, lo) - 1);
    }
};
...
public:
    ...
    static constexpr CountType kMemoryHeldByAllocatorBit = BitField<CountType>::Bit(0);
    ...
    static constexpr CountType kRequestQuarantineBit = BitField<CountType>::Bit(30);
    static constexpr CountType kNeedsMac11MallocSizeHackBit = BitField<CountType>::Bit(31);
    static constexpr CountType kDanglingRawPtrDetectedBit = BitField<CountType>::None();
    static constexpr CountType kUnprotectedPtrCountMask = BitField<CountType>::None();
    ...
    static constexpr auto kDanglingRawPtrDetectedBit = BitField<CountType>::Bit(32);
    static constexpr auto kNeedsMac11MallocSizeHackBit = BitField<CountType>::Bit(33);
    static constexpr CountType kRequestQuarantineBit = BitField<CountType>::Bit(34);
    static constexpr auto kUnprotectedPtrCountMask = BitField<CountType>::Mask(35, 63);
    // The most significant bit of the refcount is reserved to prevent races with
    // overflow detection.
    static constexpr auto kMaxUnprotectedPtrCount = BitField<CountType>::Mask(35, 62);
#endif // !PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS)

    // Quick check to assert these masks do not overlap.
    static_assert((kMemoryHeldByAllocatorBit + kPtrCountMask + kUnprotectedPtrCountMask + kDanglingRawPtrDetectedBit + kRequestQuarantineBit
                      + kNeedsMac11MallocSizeHackBit)
        == std::numeric_limits<CountType>::max());

    static constexpr auto kPtrInc = SafeShift<CountType>(1, base::bits::CountrZero(kPtrCountMask));
    static constexpr auto kUnprotectedPtrInc = SafeShift<CountType>(1, base::bits::CountrZero(kUnprotectedPtrCountMask));
    ...
    PA_ALWAYS_INLINE bool Release()
    {
        ...
        if ((old_count & kDanglingRawPtrDetectedBit) == kDanglingRawPtrDetectedBit) [[unlikely]] {
            partition_alloc::internal::DanglingRawPtrReleased(reinterpret_cast<uintptr_t>(this));
        }
#endif
    ...
    PA_ALWAYS_INLINE bool ReleaseFromUnprotectedPtr()
    {
        ...
        CountType old_count = count_.fetch_sub(kUnprotectedPtrInc, std::memory_order_release);
        // Check underflow.
    ...
    PA_ALWAYS_INLINE bool ReleaseFromAllocator()
    {
        ...
        CountType old_count = count_.fetch_and(~kMemoryHeldByAllocatorBit, std::memory_order_release);
    ...
    {
        CheckCookieIfSupported();
        static constexpr CountType mask = kMemoryHeldByAllocatorBit | kPtrCountMask | kUnprotectedPtrCountMask;
        return (count_.load(std::memory_order_acquire) & mask) == kMemoryHeldByAllocatorBit;
    }

    PA_ALWAYS_INLINE bool IsAlive()
    {
        bool alive = count_.load(std::memory_order_relaxed) & kMemoryHeldByAllocatorBit;
        if (alive) {
            CheckCookieIfSupported();
        }
        return alive;
    ...
    {
        if (!IsAlive()) {
            partition_alloc::internal::UnretainedDanglingRawPtrDetected(reinterpret_cast<uintptr_t>(this));
        }
    }
    ...
    {
        CountType old_count = count_.fetch_or(kRequestQuarantineBit, std::memory_order_relaxed);
    ...
    {
        CountType old_count = count_.fetch_and(~kRequestQuarantineBit, std::memory_order_acq_rel);
        // This bit cannot be used after the memory is freed.
        PA_DCHECK(old_count & kMemoryHeldByAllocatorBit);
...
#endif
        count_.store(kPtrInc | kMemoryHeldByAllocatorBit, std::memory_order_release);
    }
    ...
    {
        return count_.load(std::memory_order_relaxed) & kNeedsMac11MallocSizeHackBit;
    }
    ...
    PA_ALWAYS_INLINE uint32_t requested_size() const
    {
...
private:
    ...
    PA_ALWAYS_INLINE void CheckDanglingPointersOnFree(CountType count)
    {
        ...
        // `Release().
        CountType old_count = count_.fetch_or(kDanglingRawPtrDetectedBit, std::memory_order_relaxed);
        ...
        partition_alloc::internal::DanglingRawPtrDetected(reinterpret_cast<uintptr_t>(this));
#endif // PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS)
    }
    ...
    PA_ALWAYS_INLINE bool ReleaseCommon(CountType count)
    {
        ...
        if ((count & (kMemoryHeldByAllocatorBit | kPtrCountMask | kUnprotectedPtrCountMask))) [[likely]] {
    ...
    {
        return static_cast<uint32_t>(reinterpret_cast<uintptr_t>(this)) ^ kCookieSalt;
    }
#endif // PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE)

    [[noreturn]] PA_NOINLINE PA_NOT_TAIL_CALLED void DoubleFreeOrCorruptionDetected(CountType count)
    {
...
PA_ALWAYS_INLINE InSlotMetadata::InSlotMetadata(bool needs_mac11_malloc_size_hack)
    : count_(kMemoryHeldByAllocatorBit | (needs_mac11_malloc_size_hack ? kNeedsMac11MallocSizeHackBit : 0))
#if PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE)
    , brp_cookie_(CalculateCookie())
#endif
{
...
static_assert(kAlignment % alignof(InSlotMetadata) == 0, "kAlignment must be multiples of alignof(InSlotMetadata).");

#if PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS)

#if PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE) || PA_CONFIG(IN_SLOT_METADATA_STORE_REQUESTED_SIZE)
...
#if PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE) && PA_CONFIG(IN_SLOT_METADATA_STORE_REQUESTED_SIZE)
static constexpr size_t kInSlotMetadataSizeShift = 4;
#elif PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE) || PA_CONFIG(IN_SLOT_METADATA_STORE_REQUESTED_SIZE)
static constexpr size_t kInSlotMetadataSizeShift = 3;
#else
...
PA_ALWAYS_INLINE static PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR size_t GetInSlotMetadataIndexMultiplierShift()
{
    return SystemPageShift() * 2 - kSuperPageShift - kInSlotMetadataSizeShift;
}

PA_ALWAYS_INLINE InSlotMetadata* InSlotMetadataPointer(uintptr_t slot_start, size_t slot_size)
{
    ...
    if (slot_start & SystemPageOffsetMask()) [[likely]] {
        uintptr_t refcount_address = slot_start + slot_size - sizeof(InSlotMetadata);
        ...
        InSlotMetadata* table_base = reinterpret_cast<InSlotMetadata*>((slot_start & kSuperPageBaseMask) + SystemPageSize() * 2);
        size_t index = ((slot_start & kSuperPageOffsetMask) >> SystemPageShift()) << GetInSlotMetadataIndexMultiplierShift();
#if PA_BUILDFLAG(DCHECKS_ARE_ON) || PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)
