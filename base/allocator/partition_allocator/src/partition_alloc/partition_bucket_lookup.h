...
constexpr size_t OrderSubIndexMask(uint8_t order)
{
    ...
    return ((static_cast<size_t>(1) << order) - 1) >> (kNumBucketsPerOrderBits + 1);
}
...
constexpr auto MakeOrderArray(SizeClass (*order_function)(uint8_t), std::integer_sequence<SizeClass, Index...> seq)
{
    return std::array { order_function(Index)... };
}

inline constexpr auto kOrderIndexShift = MakeOrderArray(OrderIndexShift, std::make_integer_sequence<uint8_t, kNumOrders> {});

inline constexpr auto kOrderSubIndexMask = MakeOrderArray(OrderSubIndexMask, std::make_integer_sequence<size_t, kNumOrders> {});

// The class used to generate the bucket lookup table at compile-time.
class BucketIndexLookup final {
public:
    PA_ALWAYS_INLINE static constexpr uint16_t GetIndexForNeutralBuckets(size_t size);
    PA_ALWAYS_INLINE static constexpr uint16_t GetIndexForDenserBuckets(size_t size);
    ...
    constexpr BucketIndexLookup()
    {
        ...
        // Normal buckets.
        for (uint8_t order = kMinBucketedOrder; order <= kMaxBucketedOrder; ++order) {
        ...
        for (uint8_t order = kMaxBucketedOrder + 1; order <= kBitsPerSizeT; ++order) {
            for (uint16_t j = 0; j < kNumBucketsPerOrder; ++j) {
        ...
        PA_DCHECK(bucket_index_ptr == bucket_index_lookup_ + ((kBitsPerSizeT + 1) * kNumBucketsPerOrder));
        // And there's one last bucket lookup that will be hit for e.g. malloc(-1),
        // which tries to overflow to a non-existent order.
        *bucket_index_ptr = sentinel_bucket_index;
    }
    constexpr const size_t* bucket_sizes() const
    {
        return &bucket_sizes_[0];
    }
...
private:
    ...
    // behavior.
    uint16_t bucket_index_lookup_[((kBitsPerSizeT + 1) * kNumBucketsPerOrder) + 1] {};
};
...
// static
PA_ALWAYS_INLINE constexpr uint16_t BucketIndexLookup::GetIndexForDenserBuckets(size_t size)
{
    ...
    const size_t order = kBitsPerSizeT - static_cast<size_t>(base::bits::CountlZero(size));
    // The order index is simply the next few bits after the most significant
    // bit.
    const size_t order_index = (size >> kOrderIndexShift[order]) & (kNumBucketsPerOrder - 1);
    // And if the remaining bits are non-zero we must bump the bucket up.
    const size_t sub_order_index = size & kOrderSubIndexMask[order];
    const uint16_t index = lookup.bucket_index_lookup_[(order << kNumBucketsPerOrderBits) + order_index + !!sub_order_index];
    PA_DCHECK(index <= kNumBuckets); // Last one is the sentinel bucket.
    return index;
}

// static
PA_ALWAYS_INLINE constexpr uint16_t BucketIndexLookup::GetIndexForNeutralBuckets(size_t size)
{
    ...
    if (size > kAlignment * kNumBucketsPerOrder && index < GetIndexForDenserBuckets(kMaxBucketed)) {
