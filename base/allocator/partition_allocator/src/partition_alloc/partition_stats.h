...
struct PartitionMemoryStats {
    ...
    size_t total_brp_quarantined_bytes; // Total bytes that are quarantined by BRP.
...
public:
    ...
    virtual void PartitionDumpTotals(const char* partition_name, const PartitionMemoryStats*) = 0;

    // Called to dump stats about buckets, for each bucket.
    virtual void PartitionsDumpBucketStats(const char* partition_name, const PartitionBucketMemoryStats*) = 0;
};

// Simple version of PartitionStatsDumper, storing the returned stats in stats_.
// Does not handle per-bucket stats.
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) SimplePartitionStatsDumper : public PartitionStatsDumper {
public:
    SimplePartitionStatsDumper();
    ~SimplePartitionStatsDumper() override = default;

    void PartitionDumpTotals(const char* partition_name, const PartitionMemoryStats* memory_stats) override;

    void PartitionsDumpBucketStats(const char* partition_name, const PartitionBucketMemoryStats*) override
    {
    }

    const PartitionMemoryStats& stats() const
    {
        return stats_;
    }
