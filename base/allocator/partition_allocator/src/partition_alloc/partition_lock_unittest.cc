...
class ThreadDelegateForContended : public base::PlatformThreadForTesting::Delegate {
public:
    explicit ThreadDelegateForContended(Lock& start_lock, Lock& lock, int iterations, int& counter)
...
TEST(PartitionAllocLockTest, Contended)
{
    ...
    ThreadDelegateForContended delegate(start_lock, lock, iterations_per_thread, counter);
...
class ThreadDelegateForSlowThreads : public base::PlatformThreadForTesting::Delegate {
public:
    explicit ThreadDelegateForSlowThreads(Lock& start_lock, Lock& lock, int iterations, int& counter)
        : start_lock_(start_lock)
        , lock_(lock)
        , iterations_(iterations)
        , counter_(counter)
...
TEST(PartitionAllocLockTest, SlowThreads)
{
    ...
    ThreadDelegateForSlowThreads delegate(start_lock, lock, iterations_per_thread, counter);
...
#if defined(GTEST_HAS_DEATH_TEST) && (PA_BUILDFLAG(DCHECKS_ARE_ON) || PA_BUILDFLAG(ENABLE_PARTITION_LOCK_REENTRANCY_CHECK))
...
#if defined(GTEST_HAS_DEATH_TEST) && PA_BUILDFLAG(DCHECKS_ARE_ON) && (!defined(OFFICIAL_BUILD) || !defined(NDEBUG))
...
class ThreadDelegateForAssertAcquiredAnotherThreadHoldsTheLock : public base::PlatformThreadForTesting::Delegate {
public:
...
class ThreadDelegateForReinitInOtherThread : public base::PlatformThreadForTesting::Delegate {
public:
    explicit ThreadDelegateForReinitInOtherThread(Lock& lock)
        : lock_(lock)
    {
    }
