...
template <MetadataKind kind> struct PartitionSuperPageExtentEntry;
...
template <MetadataKind kind> struct PartitionSuperPageExtentEntryBase {
    ...
    MaybeConstT<kind, PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>*> next;
...
template <> struct PartitionSuperPageExtentEntry<MetadataKind::kReadOnly> : public PartitionSuperPageExtentEntryBase<MetadataKind::kReadOnly> {
    PartitionSuperPageExtentEntry<MetadataKind::kWritable>* ToWritable(const PartitionRoot* partition_root)
    {
...
private:
    ...
    // ToWritable() uses it.
    template <typename T> PartitionSuperPageExtentEntry<MetadataKind::kWritable>* ToWritableInternal([[maybe_unused]] T* partition_root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kWritable>*>(reinterpret_cast<intptr_t>(this) + partition_root->ShadowPoolOffset());
#else
        return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kWritable>*>(this);
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)
    }
};

template <> struct PartitionSuperPageExtentEntry<MetadataKind::kWritable> : public PartitionSuperPageExtentEntryBase<MetadataKind::kWritable> {
    PA_ALWAYS_INLINE void IncrementNumberOfNonemptySlotSpans()
    {
        DCheckNumberOfPartitionPagesInSuperPagePayload(this, root, number_of_nonempty_slot_spans);
    ...
    PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* ToReadOnly(const PartitionRoot* partition_root)
    {
...
private:
    ...
    // ToReadOnly() uses it.
    template <typename T> PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* ToReadOnlyInternal([[maybe_unused]] T* partition_root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>*>(reinterpret_cast<intptr_t>(this) - partition_root->ShadowPoolOffset());
#else
        return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>*>(this);
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)
    }
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON) || !PA_CONFIG(ENABLE_SHADOW_METADATA)
};

static_assert(sizeof(PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>) == sizeof(PartitionSuperPageExtentEntry<MetadataKind::kWritable>),
    ...
    "size");
static_assert(sizeof(PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>) <= kPageMetadataSize,
    "PartitionSuperPageExtentEntry must be able to fit in a metadata slot");
static_assert(kMaxSuperPagesInPool / kSuperPageSize
        <= std::numeric_limits<decltype(PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>::number_of_consecutive_super_pages)>::max(),
...
PA_ALWAYS_INLINE uintptr_t SuperPagesBeginFromExtent(const PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* extent)
{
...
// CAUTION! |extent| must point to the extent of the first super page in the
// range of consecutive super pages.
PA_ALWAYS_INLINE uintptr_t SuperPagesEndFromExtent(const PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* extent)
{
    return SuperPagesBeginFromExtent(extent) + (extent->number_of_consecutive_super_pages * kSuperPageSize);
}
