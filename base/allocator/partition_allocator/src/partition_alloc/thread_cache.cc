...
namespace tools {
uintptr_t kThreadCacheNeedleArray[kThreadCacheNeedleArraySize] = { kNeedle1, reinterpret_cast<uintptr_t>(&g_instance),
...
void OnDllProcessDetach()
{
    ...
    g_thread_cache_root.load(std::memory_order_relaxed)->settings.with_thread_cache = false;
}
#endif
...
uint16_t ThreadCache::largest_active_bucket_index_ = internal::BucketIndexLookup::GetIndex(ThreadCache::kDefaultSizeThreshold);
...
const internal::PartitionFreelistDispatcher* ThreadCache::get_freelist_dispatcher_from_root()
{
...
void ThreadCacheRegistry::DumpStats(bool my_thread_only, ThreadCacheStats* stats)
{
...
void ThreadCacheRegistry::SetLargestActiveBucketIndex(uint16_t largest_active_bucket_index)
{
    ...
    // - Enumerate all thread caches and set the limit to the global one.
    {
        ...
        while (tcache) {
            ...
            for (int index = 0; index < ThreadCache::kBucketCount; index++) {
                ...
                tcache->buckets_[index].limit.store(ThreadCache::global_limits_[index], std::memory_order_relaxed);
            }
...
void ThreadCacheRegistry::SetPurgingConfiguration(const internal::base::TimeDelta min_purge_interval, const internal::base::TimeDelta max_purge_interval,
    const internal::base::TimeDelta default_purge_interval, size_t min_cached_memory_for_purging_bytes)
{
...
void ThreadCacheRegistry::RunPeriodicPurge()
{
    ...
    if (cached_memory_approx > 10 * min_cached_memory_for_purging_bytes_) {
        periodic_purge_next_interval_ = std::min(default_purge_interval_, periodic_purge_next_interval_ / 2);
    } else if (cached_memory_approx > 2 * min_cached_memory_for_purging_bytes_) {
        periodic_purge_next_interval_ = std::max(min_purge_interval_, periodic_purge_next_interval_ / 2);
    } else if (cached_memory_approx < min_cached_memory_for_purging_bytes_) {
        periodic_purge_next_interval_ = std::min(max_purge_interval_, periodic_purge_next_interval_ * 2);
    }
    ...
    // allocation).
    periodic_purge_next_interval_ = std::clamp(periodic_purge_next_interval_, min_purge_interval_, max_purge_interval_);

    PurgeAll();
}

int64_t ThreadCacheRegistry::GetPeriodicPurgeNextIntervalInMicroseconds() const
{
...
void ThreadCache::Init(PartitionRoot* root)
{
...
#endif
    PA_CHECK(root->buckets[kBucketCount - 1].slot_size == ThreadCache::kLargeSizeThreshold);
    PA_CHECK(root->buckets[largest_active_bucket_index_].slot_size == ThreadCache::kDefaultSizeThreshold);
    ...
    if (!g_thread_cache_root.compare_exchange_strong(expected, root, std::memory_order_seq_cst, std::memory_order_seq_cst)) {
        PA_CHECK(false) << "Only one PartitionRoot is allowed to have a thread cache";
    }
...
{
    size_t initial_value = static_cast<size_t>(kSmallBucketBaseCount) * multiplier;
    ...
    for (int index = 0; index < kBucketCount; index++) {
        ...
        global_limits_[index] = static_cast<uint8_t>(std::clamp(value, kMinLimit, kMaxLimit));
...
void ThreadCache::SetLargestCachedSize(size_t size)
{
    ...
    largest_active_bucket_index_ = PartitionRoot::SizeToBucketIndex(size, PartitionRoot::BucketDistribution::kNeutral);
    PA_CHECK(largest_active_bucket_index_ < kBucketCount);
    ThreadCacheRegistry::Instance().SetLargestActiveBucketIndex(largest_active_bucket_index_);
}
    ...
    , prev_(nullptr)
{
    ...
    for (int index = 0; index < kBucketCount; index++) {
        ...
        tcache_bucket->limit.store(global_limits_[index], std::memory_order_relaxed);
    ...
    if (root_->settings.scheduler_loop_quarantine) {
        internal::LightweightQuarantineBranchConfig per_thread_config = {
            .lock_required = false,
            .branch_capacity_in_bytes = root_->scheduler_loop_quarantine_branch_capacity_in_bytes,
        };
        scheduler_loop_quarantine_branch_.emplace(root_->GetSchedulerLoopQuarantineRoot().CreateBranch(per_thread_config));
    }
}
...
void ThreadCache::Delete(void* tcache_ptr)
{
    ...
    internal::PartitionTlsSet(internal::g_thread_cache_key, reinterpret_cast<void*>(kTombstone));
#if PA_CONFIG(THREAD_CACHE_FAST_TLS)
...
void ThreadCache::FillBucket(size_t bucket_index)
{
    ...
    int count = std::max(1, bucket.limit.load(std::memory_order_relaxed) / kBatchFillRatio);
    ...
    for (int i = 0; i < count; i++) {
        ...
        uintptr_t slot_start = root_->AllocFromBucket<AllocFlags::kFastPathOrReturnNull | AllocFlags::kReturnNull>(&root_->buckets[bucket_index],
            root_->buckets[bucket_index].slot_size /* raw_size */, internal::PartitionPageSize(), &usable_size, &ret_slot_size, &is_already_zeroed);
...
template <bool crash_on_corruption> void ThreadCache::ClearBucketHelper(Bucket& bucket, size_t limit)
{
    ...
    const internal::PartitionFreelistDispatcher* freelist_dispatcher = root_->get_freelist_dispatcher();

    if constexpr (crash_on_corruption) {
        freelist_dispatcher->CheckFreeListForThreadCache(bucket.freelist_head, bucket.slot_size);
    }
    ...
    if (limit == 0) {
        ...
        while (items < limit) {
            ...
            head = freelist_dispatcher->GetNextForThreadCacheBool(head, crash_on_corruption, bucket.slot_size);
#else
            head = freelist_dispatcher->GetNextForThreadCache<crash_on_corruption>(head, bucket.slot_size);
#endif // PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
            items++;
        }

#if PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
        FreeAfter<crash_on_corruption>(freelist_dispatcher->GetNextForThreadCacheBool(head, crash_on_corruption, bucket.slot_size), bucket.slot_size);
#else
        FreeAfter<crash_on_corruption>(freelist_dispatcher->GetNextForThreadCache<crash_on_corruption>(head, bucket.slot_size), bucket.slot_size);
#endif // PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
...
template <bool crash_on_corruption> void ThreadCache::FreeAfter(internal::PartitionFreelistEntry* head, size_t slot_size)
{
    ...
    while (head) {
        ...
        const internal::PartitionFreelistDispatcher* freelist_dispatcher = root_->get_freelist_dispatcher();
#if PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
        head = freelist_dispatcher->GetNextForThreadCacheBool(head, crash_on_corruption, slot_size);
#else
        head = freelist_dispatcher->GetNextForThreadCache<crash_on_corruption>(head, slot_size);
#endif // PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
...
template <bool crash_on_corruption> void ThreadCache::PurgeInternalHelper()
{
    should_purge_.store(false, std::memory_order_relaxed);
