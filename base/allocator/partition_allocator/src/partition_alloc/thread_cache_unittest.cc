...
#if !defined(MEMORY_TOOL_REPLACES_ALLOCATOR) && PA_CONFIG(THREAD_CACHE_SUPPORTED)
...
const std::vector<ThreadCacheTestParam> params
    = { { ThreadCacheTestParam { BucketDistribution::kNeutral, internal::PartitionFreelistEncoding::kPoolOffsetFreeList } },
          { ThreadCacheTestParam { BucketDistribution::kDenser, internal::PartitionFreelistEncoding::kEncodedFreeList } },
          { ThreadCacheTestParam { BucketDistribution::kNeutral, internal::PartitionFreelistEncoding::kPoolOffsetFreeList } },
          { ThreadCacheTestParam { BucketDistribution::kDenser, internal::PartitionFreelistEncoding::kEncodedFreeList } } };
...
constexpr size_t kDefaultCountForSmallBucket = ThreadCache::kSmallBucketBaseCount * ThreadCache::kDefaultMultiplier;
constexpr size_t kFillCountForSmallBucket = kDefaultCountForSmallBucket / ThreadCache::kBatchFillRatio;

constexpr size_t kMediumSize = 200;
constexpr size_t kDefaultCountForMediumBucket = kDefaultCountForSmallBucket / 2;
constexpr size_t kFillCountForMediumBucket = kDefaultCountForMediumBucket / ThreadCache::kBatchFillRatio;
...
std::unique_ptr<PartitionAllocatorForTesting> CreateAllocator(
    internal::PartitionFreelistEncoding encoding = internal::PartitionFreelistEncoding::kEncodedFreeList)
{
    ...
    opts.use_pool_offset_freelists
        = (encoding == internal::PartitionFreelistEncoding::kPoolOffsetFreeList) ? PartitionOptions::kEnabled : PartitionOptions::kDisabled;
    std::unique_ptr<PartitionAllocatorForTesting> allocator = std::make_unique<PartitionAllocatorForTesting>(opts);
...
class PartitionAllocThreadCacheTest : public ::testing::TestWithParam<ThreadCacheTestParam> {
public:
...
protected:
    void SetUp() override
    {
        ...
        ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier);
        ThreadCacheRegistry::Instance().SetPurgingConfiguration(kMinPurgeInterval, kMaxPurgeInterval, kDefaultPurgeInterval, kMinCachedMemoryForPurgingBytes);
    ...
    {
        size_t tc_bucket_index = PartitionRoot::SizeToBucketIndex(sizeof(ThreadCache), PartitionRoot::BucketDistribution::kNeutral);
    ...
    {
        return PartitionRoot::SizeToBucketIndex(size, GetParam().bucket_distribution);
    }
    ...
    size_t FillThreadCacheAndReturnIndex(size_t raw_size, size_t count = 1)
    {
        ...
        for (size_t i = 0; i < count; ++i) {
            allocated_data.push_back(root()->Alloc(root()->AdjustSizeForExtrasSubtract(raw_size), ""));
        }
    ...
    void FillThreadCacheWithMemory(size_t target_cached_memory)
    {
        for (int batch : { 1, 2, 4, 8, 16 }) {
            for (size_t raw_size = root()->AdjustSizeForExtrasAdd(1); raw_size <= ThreadCache::kLargeSizeThreshold; raw_size++) {
...
INSTANTIATE_TEST_SUITE_P(AlternateBucketDistributionAndPartitionFreeList, PartitionAllocThreadCacheTest, testing::ValuesIn(params));
...
TEST_P(PartitionAllocThreadCacheTest, Simple)
{
    ...
    void* ptr = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kSmallSize), "");
    ASSERT_TRUE(ptr);

    uint16_t index = SizeToIndex(kSmallSize);
    EXPECT_EQ(kFillCountForSmallBucket - 1, tcache->bucket_count_for_testing(index));

    root()->Free(ptr);
    // Freeing fills the thread cache.
    EXPECT_EQ(kFillCountForSmallBucket, tcache->bucket_count_for_testing(index));

    void* ptr2 = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kSmallSize), "");
...
TEST_P(PartitionAllocThreadCacheTest, InexactSizeMatch)
{
    ...
    void* ptr2 = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kSmallSize + 1), "");
    // MTE-untag, because Free() changes tag.
    EXPECT_EQ(UntagPtr(ptr), UntagPtr(ptr2));
    // Allocated from the thread cache.
    EXPECT_EQ(kFillCountForSmallBucket - 1, tcache->bucket_count_for_testing(index));
...
TEST_P(PartitionAllocThreadCacheTest, MultipleObjectsCachedPerBucket)
{
    auto* tcache = root()->thread_cache_for_testing();
    DeltaCounter batch_fill_counter { tcache->stats_for_testing().batch_fill_count };
    size_t bucket_index = FillThreadCacheAndReturnIndex(kMediumSize, kFillCountForMediumBucket + 2);
    EXPECT_EQ(2 * kFillCountForMediumBucket, tcache->bucket_count_for_testing(bucket_index));
...
TEST_P(PartitionAllocThreadCacheTest, Purge)
{
    ...
    auto* tcache = root()->thread_cache_for_testing();
    EXPECT_EQ((1 + allocations / kFillCountForMediumBucket) * kFillCountForMediumBucket, tcache->bucket_count_for_testing(bucket_index));
    tcache->Purge();
...
TEST_P(PartitionAllocThreadCacheTest, NoCrossPartitionCache)
{
    ...
    void* ptr = allocator.root()->Alloc(allocator.root()->AdjustSizeForExtrasSubtract(kSmallSize), "");
    ASSERT_TRUE(ptr);

    auto* tcache = root()->thread_cache_for_testing();
    EXPECT_EQ(kFillCountForSmallBucket, tcache->bucket_count_for_testing(bucket_index));
...
TEST_P(PartitionAllocThreadCacheTest, LargeAllocationsAreNotCached)
{
    ...
    DeltaCounter alloc_miss_too_large_counter { tcache->stats_for_testing().alloc_miss_too_large };
    DeltaCounter cache_fill_counter { tcache->stats_for_testing().cache_fill_count };
    DeltaCounter cache_fill_misses_counter { tcache->stats_for_testing().cache_fill_misses };
...
TEST_P(PartitionAllocThreadCacheTest, DirectMappedReallocMetrics)
{
    ...
    EXPECT_EQ(expected_allocated_size, root()->get_total_size_of_allocated_bytes());
    EXPECT_EQ(expected_allocated_size, root()->get_max_size_of_allocated_bytes());

    void* ptr = root()->Alloc(root()->AdjustSizeForExtrasSubtract(10 * internal::kMaxBucketed), "");

    EXPECT_EQ(expected_allocated_size + 10 * internal::kMaxBucketed, root()->get_total_size_of_allocated_bytes());

    void* ptr2 = root()->Realloc(ptr, root()->AdjustSizeForExtrasSubtract(9 * internal::kMaxBucketed), "");

    ASSERT_EQ(ptr, ptr2);
    EXPECT_EQ(expected_allocated_size + 9 * internal::kMaxBucketed, root()->get_total_size_of_allocated_bytes());

    ptr2 = root()->Realloc(ptr, root()->AdjustSizeForExtrasSubtract(10 * internal::kMaxBucketed), "");
...
size_t FillThreadCacheAndReturnIndex(PartitionRoot* root, size_t size, BucketDistribution bucket_distribution, size_t count = 1)
{
    uint16_t bucket_index = PartitionRoot::SizeToBucketIndex(size, bucket_distribution);
    std::vector<void*> allocated_data;

    for (size_t i = 0; i < count; ++i) {
        allocated_data.push_back(root->Alloc(root->AdjustSizeForExtrasSubtract(size), ""));
    }
    for (void* ptr : allocated_data) {
        root->Free(ptr);
    }
...
class ThreadDelegateForMultipleThreadCaches : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForMultipleThreadCaches(ThreadCache* parent_thread_cache, PartitionRoot* root, BucketDistribution bucket_distribution)
...
TEST_P(PartitionAllocThreadCacheTest, MultipleThreadCaches)
{
    ...
    ThreadDelegateForMultipleThreadCaches delegate(parent_thread_tcache, root(), GetParam().bucket_distribution);

    internal::base::PlatformThreadHandle thread_handle;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle);
    internal::base::PlatformThreadForTesting::Join(thread_handle);
}

namespace {

class ThreadDelegateForThreadCacheReclaimedWhenThreadExits : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForThreadCacheReclaimedWhenThreadExits(PartitionRoot* root, void*& other_thread_ptr)
        : root_(root)
        , other_thread_ptr_(other_thread_ptr)
    {
    }
    ...
    {
        EXPECT_FALSE(root_->thread_cache_for_testing()); // No allocations yet.
        other_thread_ptr_ = root_->Alloc(root_->AdjustSizeForExtrasSubtract(kMediumSize), "");
...
TEST_P(PartitionAllocThreadCacheTest, ThreadCacheReclaimedWhenThreadExits)
{
    ...
    for (size_t i = 0; i < kDefaultCountForMediumBucket / 4; i++) {
        tmp.push_back(root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), ""));
    }

    void* other_thread_ptr = nullptr;
    ThreadDelegateForThreadCacheReclaimedWhenThreadExits delegate(root(), other_thread_ptr);

    internal::base::PlatformThreadHandle thread_handle;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle);
    internal::base::PlatformThreadForTesting::Join(thread_handle);

    void* this_thread_ptr = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), "");
...
class ThreadDelegateForThreadCacheRegistry : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForThreadCacheRegistry(ThreadCache* parent_thread_cache, PartitionRoot* root, BucketDistribution bucket_distribution)
        : parent_thread_tcache_(parent_thread_cache)
        , root_(root)
        , bucket_distribution_(bucket_distribution)
    {
...
TEST_P(PartitionAllocThreadCacheTest, ThreadCacheRegistry)
{
...
#if !(PA_BUILDFLAG(IS_APPLE) || PA_BUILDFLAG(IS_ANDROID) || PA_BUILDFLAG(IS_CHROMEOS) || PA_BUILDFLAG(IS_LINUX)) && PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    ...
    ThreadDelegateForThreadCacheRegistry delegate(parent_thread_tcache, root(), GetParam().bucket_distribution);
...
TEST_P(PartitionAllocThreadCacheTest, RecordStats)
{
    ...
    DeltaCounter alloc_miss_empty_counter { tcache->stats_for_testing().alloc_miss_empty };

    DeltaCounter cache_fill_counter { tcache->stats_for_testing().cache_fill_count };
    DeltaCounter cache_fill_hits_counter { tcache->stats_for_testing().cache_fill_hits };
    DeltaCounter cache_fill_misses_counter { tcache->stats_for_testing().cache_fill_misses };

    // Cache has been purged, first allocation is a miss.
    void* data = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), "");
    ...
    size_t allocations = 10;
    size_t bucket_index = FillThreadCacheAndReturnIndex(kMediumSize, kDefaultCountForMediumBucket + allocations);
    EXPECT_EQ(kDefaultCountForMediumBucket + allocations, cache_fill_counter.Delta());
    EXPECT_EQ(0u, cache_fill_misses_counter.Delta());
    ...
    size_t expected_count = kDefaultCountForMediumBucket / 2 - 1 + (1 + allocations / kFillCountForMediumBucket) * kFillCountForMediumBucket;
    EXPECT_EQ(root()->buckets[bucket_index].slot_size * expected_count, stats.bucket_total_memory);
    EXPECT_EQ(sizeof(ThreadCache), stats.metadata_overhead);
}

namespace {

class ThreadDelegateForMultipleThreadCachesAccounting : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForMultipleThreadCachesAccounting(
        PartitionRoot* root, const ThreadCacheStats& wqthread_stats, int alloc_count, BucketDistribution bucket_distribution)
        : root_(root)
        , bucket_distribution_(bucket_distribution)
    ...
    {
        EXPECT_FALSE(root_->thread_cache_for_testing()); // No allocations yet.
        size_t bucket_index = FillThreadCacheAndReturnIndex(root_, kMediumSize, bucket_distribution_);

        ThreadCacheStats stats;
        ThreadCacheRegistry::Instance().DumpStats(false, &stats);
        // 2* for this thread and the parent one.
        EXPECT_EQ(2 * root_->buckets[bucket_index].slot_size * kFillCountForMediumBucket, stats.bucket_total_memory - wqthread_stats_.bucket_total_memory);
        EXPECT_EQ(2 * sizeof(ThreadCache), stats.metadata_overhead - wqthread_stats_.metadata_overhead);

        ThreadCacheStats this_thread_cache_stats {};
        root_->thread_cache_for_testing()->AccumulateStats(&this_thread_cache_stats);
        EXPECT_EQ(alloc_count_ + this_thread_cache_stats.alloc_count, stats.alloc_count - wqthread_stats_.alloc_count);
    }
...
#endif
    FillThreadCacheAndReturnIndex(kMediumSize);
    uint64_t alloc_count = root()->thread_cache_for_testing()->stats_for_testing().alloc_count;

    ThreadDelegateForMultipleThreadCachesAccounting delegate(root(), wqthread_stats, alloc_count, GetParam().bucket_distribution);
...
class ThreadDelegateForPurgeAll : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForPurgeAll(PartitionRoot* root, ThreadCache*& other_thread_tcache, std::atomic<bool>& other_thread_started, std::atomic<bool>& purge_called,
        int bucket_index, BucketDistribution bucket_distribution)
        : root_(root)
    ...
    void ThreadMain() override PA_NO_THREAD_SAFETY_ANALYSIS
    {
        ...
        EXPECT_EQ(kFillCountForSmallBucket, other_thread_tcache_->bucket_count_for_testing(bucket_index_));
        // Allocations do not trigger Purge().
        void* data = root_->Alloc(root_->AdjustSizeForExtrasSubtract(kSmallSize), "");
        EXPECT_EQ(kFillCountForSmallBucket - 1, other_thread_tcache_->bucket_count_for_testing(bucket_index_));
        // But deallocations do.
        root_->Free(data);
        EXPECT_EQ(0u, other_thread_tcache_->bucket_count_for_testing(bucket_index_));
    }
...
PA_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    ThreadDelegateForPurgeAll delegate(root(), other_thread_tcache, other_thread_started, purge_called, bucket_index, GetParam().bucket_distribution);
    internal::base::PlatformThreadHandle thread_handle;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle);
    ...
    EXPECT_EQ(kFillCountForSmallBucket, this_thread_tcache->bucket_count_for_testing(bucket_index));
    EXPECT_EQ(kFillCountForSmallBucket, other_thread_tcache->bucket_count_for_testing(bucket_index));
...
TEST_P(PartitionAllocThreadCacheTest, PeriodicPurge)
{
    ...
    auto NextInterval = [&registry] { return internal::base::Microseconds(registry.GetPeriodicPurgeNextIntervalInMicroseconds()); };

    EXPECT_EQ(NextInterval(), registry.default_purge_interval());

    // Small amount of memory, the period gets longer.
    auto* tcache = ThreadCache::Get();
    ASSERT_LT(tcache->CachedMemory(), registry.min_cached_memory_for_purging_bytes());
    ...
    FillThreadCacheWithMemory(2 * registry.min_cached_memory_for_purging_bytes() + 1);
    registry.RunPeriodicPurge();
...
void FillThreadCacheWithMemory(PartitionRoot* root, size_t target_cached_memory, BucketDistribution bucket_distribution)
{
    for (int batch : { 1, 2, 4, 8, 16 }) {
        for (size_t allocation_size = 1; allocation_size <= ThreadCache::kLargeSizeThreshold; allocation_size++) {
            FillThreadCacheAndReturnIndex(root, root->AdjustSizeForExtrasAdd(allocation_size), bucket_distribution, batch);
...
class ThreadDelegateForPeriodicPurgeSumsOverAllThreads : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForPeriodicPurgeSumsOverAllThreads(
        PartitionRoot* root, std::atomic<int>& allocations_done, std::atomic<bool>& can_finish, BucketDistribution bucket_distribution)
        : root_(root)
    ...
    void ThreadMain() override
    {
        FillThreadCacheWithMemory(root_, 5 * ThreadCacheRegistry::Instance().min_cached_memory_for_purging_bytes(), bucket_distribution_);
...
TEST_P(PartitionAllocThreadCacheTest, DISABLED_PeriodicPurgeSumsOverAllThreads)
{
    ...
    ThreadDelegateForPeriodicPurgeSumsOverAllThreads delegate(root(), allocations_done, can_finish, GetParam().bucket_distribution);

    internal::base::PlatformThreadHandle thread_handle;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle);
    internal::base::PlatformThreadHandle thread_handle_2;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle_2);
...
{
    auto* tcache = root()->thread_cache_for_testing();
    size_t bucket_index = FillThreadCacheAndReturnIndex(kMediumSize, kDefaultCountForMediumBucket);

    EXPECT_EQ(kDefaultCountForMediumBucket, tcache->bucket_for_testing(bucket_index).count);

    ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier / 2);
    // No immediate batch deallocation.
    EXPECT_EQ(kDefaultCountForMediumBucket, tcache->bucket_for_testing(bucket_index).count);
    void* data = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), "");
    // Not triggered by allocations.
    EXPECT_EQ(kDefaultCountForMediumBucket - 1, tcache->bucket_for_testing(bucket_index).count);

    // Free() triggers the purge within limits.
    root()->Free(data);
    EXPECT_LE(tcache->bucket_for_testing(bucket_index).count, kDefaultCountForMediumBucket / 2);
    ...
    ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier * 2);
    FillThreadCacheAndReturnIndex(kMediumSize, 1000);
    EXPECT_GT(tcache->bucket_for_testing(bucket_index).count, kDefaultCountForMediumBucket / 2);
}
...
TEST_P(PartitionAllocThreadCacheTest, DynamicCountPerBucketClamping)
{
    ...
    ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier / 1000.);
    for (size_t i = 0; i < ThreadCache::kBucketCount; i++) {
        ...
        EXPECT_GE(tcache->bucket_for_testing(i).limit.load(std::memory_order_relaxed), 1u);
    }
    ...
    for (size_t i = 0; i < ThreadCache::kBucketCount; i++) {
        ...
        EXPECT_LT(tcache->bucket_for_testing(i).limit.load(std::memory_order_relaxed), 0xff);
    }
}

// TODO(crbug.com/40816487): Flaky on IOS.
#if PA_BUILDFLAG(IS_IOS)
#define MAYBE_DynamicCountPerBucketMultipleThreads DISABLED_DynamicCountPerBucketMultipleThreads
#else
#define MAYBE_DynamicCountPerBucketMultipleThreads DynamicCountPerBucketMultipleThreads
#endif

namespace {

class ThreadDelegateForDynamicCountPerBucketMultipleThreads : public internal::base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForDynamicCountPerBucketMultipleThreads(PartitionRoot* root, std::atomic<bool>& other_thread_started, std::atomic<bool>& threshold_changed,
        int bucket_index, BucketDistribution bucket_distribution)
        : root_(root)
        , other_thread_started_(other_thread_started)
    ...
    void ThreadMain() override
    {
        FillThreadCacheAndReturnIndex(root_, kSmallSize, bucket_distribution_, kDefaultCountForSmallBucket + 10);
        auto* this_thread_tcache = root_->thread_cache_for_testing();
        // More than the default since the multiplier has changed.
        EXPECT_GT(this_thread_tcache->bucket_count_for_testing(bucket_index_), kDefaultCountForSmallBucket + 10);
        ...
        EXPECT_EQ(static_cast<uint8_t>(ThreadCache::kSmallBucketBaseCount / 2), this_thread_tcache->bucket_count_for_testing(bucket_index_));
    }
...
TEST_P(PartitionAllocThreadCacheTest, MAYBE_DynamicCountPerBucketMultipleThreads)
{
    std::atomic<bool> other_thread_started { false };
    ...
    EXPECT_EQ(kDefaultCountForSmallBucket, tcache->bucket_for_testing(bucket_index).count);

    // Change the ratio before starting the threads, checking that it will applied
    // to newly-created threads.
    ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier + 1);

    ThreadDelegateForDynamicCountPerBucketMultipleThreads delegate(
        root(), other_thread_started, threshold_changed, bucket_index, GetParam().bucket_distribution);
...
TEST_P(PartitionAllocThreadCacheTest, ClearFromTail)
{
    auto count_items = [this](ThreadCache* tcache, size_t index) {
        const internal::PartitionFreelistDispatcher* freelist_dispatcher = this->root()->get_freelist_dispatcher();
        uint8_t count = 0;
        auto* head = tcache->bucket_for_testing(index).freelist_head;
        while (head) {
#if PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
            head = freelist_dispatcher->GetNextForThreadCacheTrue(head, tcache->bucket_for_testing(index).slot_size);
#else
            head = freelist_dispatcher->GetNextForThreadCache<true>(head, tcache->bucket_for_testing(index).slot_size);
    ...
    for (size_t limit : { 8, 3, 1 }) {
        ...
        EXPECT_EQ(head, static_cast<void*>(tcache->bucket_for_testing(index).freelist_head));
        EXPECT_EQ(count_items(tcache, index), limit);
    }
    tcache->ClearBucketForTesting(tcache->bucket_for_testing(index), 0);
    EXPECT_EQ(nullptr, static_cast<void*>(tcache->bucket_for_testing(index).freelist_head));
}
...
TEST_P(PartitionAllocThreadCacheTest, MAYBE_Bookkeeping)
{
    ...
    root()->PurgeMemory(PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages);
    root()->ResetBookkeepingForTesting();
    ...
    void* ptr = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), "");
    ...
    expected_committed_size += kUseLazyCommit ? internal::SystemPageSize() : medium_bucket->get_bytes_per_span();
    ...
    for (size_t i = 0; i < kFillCountForMediumBucket; i++) {
        arr[i] = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kMediumSize), "");
        EXPECT_EQ(expected_committed_size, root()->total_size_of_committed_pages);
        EXPECT_EQ(expected_committed_size, root()->max_size_of_committed_pages);
        EXPECT_EQ(expected_allocated_size, root()->get_total_size_of_allocated_bytes());
        EXPECT_EQ(expected_allocated_size, root()->get_max_size_of_allocated_bytes());
        EXPECT_EQ((kFillCountForMediumBucket - 1 - i) * medium_alloc_size, tcache->CachedMemory());
    }
    ...
    EXPECT_EQ(root()->get_total_size_of_allocated_bytes(), expected_allocated_size);
    tcache->Purge();
}
...
TEST_P(PartitionAllocThreadCacheTest, TryPurgeMultipleCorrupted)
{
    ...
    const internal::PartitionFreelistDispatcher* freelist_dispatcher = root()->get_freelist_dispatcher();
#if PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
...
TEST(AlternateBucketDistributionTest, SizeToIndex)
{
    ...
    for (size_t i = 1 << 0; i < 1 << 8; i <<= 1) {
        for (size_t offset = 0; offset < 4; offset++) {
            ...
            EXPECT_EQ(BucketIndexLookup::GetIndex(n), BucketIndexLookup::GetIndexForNeutralBuckets(n));
        }
    }
    ...
    for (size_t i = 1 << 8; i < internal::kHighThresholdForAlternateDistribution; i <<= 1) {
        ...
        for (size_t offset = 2; offset < 4; offset++) {
            ...
            EXPECT_GT(BucketIndexLookup::GetIndex(n), BucketIndexLookup::GetIndexForNeutralBuckets(n));
    ...
    // bucket distribution.
    for (size_t i = internal::kHighThresholdForAlternateDistribution; i < internal::kMaxBucketed; i <<= 1) {
        for (size_t offset = 0; offset < 4; offset++) {
            size_t n = i * (4 + offset) / 4;
            EXPECT_EQ(BucketIndexLookup::GetIndex(n), BucketIndexLookup::GetIndexForNeutralBuckets(n));
        }
...
TEST_P(PartitionAllocThreadCacheTest, AllocationRecording)
{
    ...
    void* ptr2 = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kBucketedNotCached), "");
    ASSERT_TRUE(ptr2);
    expected_total_size += PartitionRoot::GetUsableSize(ptr2);
    void* ptr3 = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kDirectMapped), "");
    ASSERT_TRUE(ptr3);
    expected_total_size += PartitionRoot::GetUsableSize(ptr3);
    void* ptr4 = root()->Alloc(root()->AdjustSizeForExtrasSubtract(kSingleSlot), "");
    ASSERT_TRUE(ptr4);
    ...
    EXPECT_EQ(expected_total_size, tcache->thread_alloc_stats().dealloc_total_size);
...
TEST_P(PartitionAllocThreadCacheTest, AllocationRecordingAligned)
{
    ...
    size_t total_size = 0;
    size_t size_alignments[][2]
        = { { 128, 4 }, { 128, 128 }, { 1024, 128 }, { 128, 1024 }, { 128, 2 * internal::PartitionPageSize() }, { (4 << 20) + 1, 1 << 19 } };
    ...
    EXPECT_EQ(tcache->thread_alloc_stats().alloc_total_size, tcache->thread_alloc_stats().dealloc_total_size);
...
TEST_P(PartitionAllocThreadCacheTest, AllocationRecordingRealloc)
{
    ...
    size_t size_new_sizes[][2] = { { 16, 15 }, { 16, 64 }, { 16, internal::PartitionPageSize() + 1 }, { 4 << 20, 8 << 20 }, { 8 << 20, 4 << 20 },
    ...
    for (auto [size, new_size] : size_new_sizes) {
        ...
        EXPECT_EQ(total_dealloc_size, tcache->thread_alloc_stats().dealloc_total_size) << new_size;
        ...
        EXPECT_EQ(total_alloc_size, tcache->thread_alloc_stats().alloc_total_size);
        EXPECT_EQ(dealloc_count, tcache->thread_alloc_stats().dealloc_count);
        EXPECT_EQ(total_dealloc_size, tcache->thread_alloc_stats().dealloc_total_size);
    }
    EXPECT_EQ(tcache->thread_alloc_stats().alloc_total_size, tcache->thread_alloc_stats().dealloc_total_size);
}
...
TEST(AlternateBucketDistributionTest, SwitchBeforeAlloc)
{
    ...
    EXPECT_NE(internal::BucketIndexLookup::GetIndex(n), internal::BucketIndexLookup::GetIndexForNeutralBuckets(n));
