...
TEST(PartitionAllocAddressSpaceRandomizationTest, Alignment)
{
    ...
    for (size_t i = 0; i < kSamples; ++i) {
        ...
        EXPECT_EQ(0ULL, (address & internal::PageAllocationGranularityOffsetMask()));
    }
}
...
#define TEST_RANDOM_BIT(BIT)                                                                                                                                   \
    TEST(PartitionAllocAddressSpaceRandomizationTest, RandomBitCorrelations##BIT)                                                                              \
    {                                                                                                                                                          \
...
TEST(PartitionAllocAddressSpaceRandomizationTest, CanMapInAslrRange)
{
    ...
    for (tries = 0; tries < kMaxTries; tries++) {
        ...
        uintptr_t address = AllocPages(requested_address, size, internal::PageAllocationGranularity(),
            PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageTag::kPartitionAlloc);
