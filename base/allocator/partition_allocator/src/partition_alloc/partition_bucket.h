...
struct PartitionBucket {
    ...
    static_assert(kMaxBucketed < (1 << (kReciprocalShift / 2)),
    ...
    PA_NOINLINE PA_COMPONENT_EXPORT(PARTITION_ALLOC) uintptr_t SlowPathAlloc(PartitionRoot* root, AllocFlags flags, size_t raw_size, size_t slot_span_alignment,
        SlotSpanMetadata<MetadataKind::kReadOnly>** slot_span, bool* is_already_zeroed) PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));

    PA_ALWAYS_INLINE bool CanStoreRawSize() const
    {
        return can_store_raw_size;
    }
    ...
    PA_ALWAYS_INLINE size_t get_pages_per_slot_span() const
    {
        ...
        return (num_system_pages_per_slot_span + (NumSystemPagesPerPartitionPage() - 1)) / NumSystemPagesPerPartitionPage();
    }
    ...
    PA_ALWAYS_INLINE size_t GetSlotNumber(size_t offset_in_slot_span) const
    {
        ...
        const size_t offset_in_slot = ((offset_in_slot_span * slot_size_reciprocal) >> kReciprocalShift);
    ...
    // compilation unit.
    uintptr_t AllocNewSuperPageSpanForGwpAsan(PartitionRoot* root, size_t super_page_count, AllocFlags flags)
        PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    void InitializeSlotSpanForGwpAsan(SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span, PartitionRoot* root);
...
private:
    ...
    // super page.
    PA_ALWAYS_INLINE uintptr_t AllocNewSuperPageSpan(PartitionRoot* root, size_t super_page_count, AllocFlags flags)
        PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    // Allocates a new slot span with size |num_partition_pages| from the
    // current extent. Metadata within this slot span will be initialized.
    // Returns nullptr on error.
    PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* AllocNewSlotSpan(PartitionRoot* root, AllocFlags flags, size_t slot_span_alignment)
        PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    ...
    PA_ALWAYS_INLINE uintptr_t AllocNewSuperPage(PartitionRoot* root, AllocFlags flags) PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    ...
    // freelist.
    PA_ALWAYS_INLINE void InitializeSlotSpan(SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span, PartitionRoot* root);

    // Initializes a super page. Returns the address of the super page's payload.
    PA_ALWAYS_INLINE uintptr_t InitializeSuperPage(PartitionRoot* root, uintptr_t super_page, uintptr_t requested_address)
        PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    ...
    PA_ALWAYS_INLINE uintptr_t ProvisionMoreSlotsAndAllocOne(PartitionRoot* root, AllocFlags flags, SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span)
        PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
};
