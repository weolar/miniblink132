...
extern "C" void PAPushAllRegistersAndIterateStack(const Stack*, StackVisitor*, IterateStackCallback);

void PAPushAllRegistersAndIterateStack(const Stack*, StackVisitor*, IterateStackCallback)
{
    *(int*)1 = 1;
}
...
[[maybe_unused]] void IterateSafeStackIfNecessary(StackVisitor* visitor)
{
    ...
    PA_CHECK(0u == (reinterpret_cast<uintptr_t>(stack_ptr) & (kSafeStackAlignmentBytes - 1)));
    PA_CHECK(0u == (reinterpret_cast<uintptr_t>(stack_top) & (kSafeStackAlignmentBytes - 1)));
    visitor->VisitStack(reinterpret_cast<uintptr_t*>(stack_ptr), reinterpret_cast<uintptr_t*>(stack_top));
...
[[maybe_unused]] PA_NOINLINE PA_NO_SANITIZE("address") void IteratePointersImpl(const Stack* stack, StackVisitor* visitor, uintptr_t* stack_ptr)
{
    PA_DCHECK(stack);
    PA_DCHECK(visitor);
    ...
    PA_CHECK(0u == (reinterpret_cast<uintptr_t>(stack_ptr) & (kMinStackAlignment - 1)));
    visitor->VisitStack(stack_ptr, reinterpret_cast<uintptr_t*>(stack->stack_top()));
}
