...
template <typename T> typename InternalAllocator<T>::value_type* InternalAllocator<T>::allocate(std::size_t count)
{
    PA_CHECK(count <= std::numeric_limits<std::size_t>::max() / sizeof(value_type));
    return static_cast<value_type*>(InternalAllocatorRoot().Alloc<AllocFlags::kNoHooks>(count * sizeof(value_type)));
}
template <typename T> void InternalAllocator<T>::deallocate(value_type* ptr, std::size_t)
{
    InternalAllocatorRoot().Free<FreeFlags::kNoHooks>(ptr);
}

// Create an object on heap in the internal partition.
template <typename T, typename... Args> T* ConstructAtInternalPartition(Args&&... args)
{
    auto* memory = static_cast<T*>(InternalAllocatorRoot().Alloc<AllocFlags::kNoHooks>(sizeof(T)));
...
template <typename T> void DestroyAtInternalPartition(T* ptr)
{
