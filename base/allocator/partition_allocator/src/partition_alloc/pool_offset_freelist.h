...
class EncodedPoolOffset {
    ...
    PA_ALWAYS_INLINE constexpr uintptr_t Inverted() const
    {
        return ~encoded_;
    }
    ...
    PA_ALWAYS_INLINE static uintptr_t Encode(void* ptr)
    {
        ...
        uintptr_t tagged_offset = reinterpret_cast<uintptr_t>(ptr) & (kPtrTagMask | ~pool_info.base_mask);
    ...
    PA_ALWAYS_INLINE PoolOffsetFreelistEntry* Decode(PoolInfo& pool_info) const
    {
        ...
        return reinterpret_cast<PoolOffsetFreelistEntry*>(pool_info.base | tagged_offset);
    }
...
public:
    ...
    PA_ALWAYS_INLINE static PoolOffsetFreelistEntry* EmplaceAndInitNull(void* slot_start_tagged)
    {
        // |slot_start_tagged| is MTE-tagged.
        auto* entry = new (slot_start_tagged) PoolOffsetFreelistEntry(nullptr);
        return entry;
    }
    PA_ALWAYS_INLINE static PoolOffsetFreelistEntry* EmplaceAndInitNull(uintptr_t slot_start)
    {
    ...
    PA_ALWAYS_INLINE static PoolOffsetFreelistEntry* EmplaceAndInitForThreadCache(uintptr_t slot_start, PoolOffsetFreelistEntry* next)
    {
        auto* entry = new (SlotStartAddr2Ptr(slot_start)) PoolOffsetFreelistEntry(next);
        return entry;
    }
    ...
    PA_ALWAYS_INLINE static void EmplaceAndInitForTest(uintptr_t slot_start, void* next, bool make_shadow_match)
    {
        new (SlotStartAddr2Ptr(slot_start)) PoolOffsetFreelistEntry(next, make_shadow_match);
    }
    ...
    template <bool crash_on_corruption> PA_ALWAYS_INLINE PoolOffsetFreelistEntry* GetNextForThreadCache(size_t slot_size) const
    {
        return GetNextInternal<crash_on_corruption, /*for_thread_cache=*/true>(slot_size);
    }
    ...
    {
        for (auto* entry = this; entry; entry = entry->GetNextForThreadCache<true>(slot_size)) {
    ...
    PA_ALWAYS_INLINE void SetNext(PoolOffsetFreelistEntry* entry)
    {
        ...
        if (entry && (SlotStartPtr2Addr(this) & kSuperPageBaseMask) != (SlotStartPtr2Addr(entry) & kSuperPageBaseMask)) [[unlikely]] {
...
private:
    template <bool crash_on_corruption, bool for_thread_cache> PA_ALWAYS_INLINE PoolOffsetFreelistEntry* GetNextInternal(size_t slot_size) const
    {
        ...
        if (!IsWellFormed<for_thread_cache>(pool_info, this, ret)) [[unlikely]] {
            if constexpr (crash_on_corruption) {
                ...
                PA_DEBUG_DATA_ON_STACK("first", static_cast<size_t>(encoded_next_.encoded_));
#if PA_CONFIG(HAS_FREELIST_SHADOW_ENTRY)
    ...
    PA_ALWAYS_INLINE static bool IsWellFormed(const PoolInfo& pool_info, const PoolOffsetFreelistEntry* here, const PoolOffsetFreelistEntry* next)
    {
        ...
        // tag.
        const bool pool_base_mask_matches = (next_address & pool_info.base_mask) == pool_info.base;
        ...
        const bool not_in_metadata = (next_address & kSuperPageOffsetMask) >= PartitionPageSize();

        if constexpr (for_thread_cache) {
            return pool_base_mask_matches & shadow_ptr_ok & not_in_metadata;
        }

        const bool same_super_page = (here_address & kSuperPageBaseMask) == (next_address & kSuperPageBaseMask);
        ...
        return pool_base_mask_matches & shadow_ptr_ok & same_super_page & marked_as_free_in_bitmap & not_in_metadata;
    }
