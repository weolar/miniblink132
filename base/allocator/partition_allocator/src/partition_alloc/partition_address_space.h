...
public:
    ...
    PA_ALWAYS_INLINE static PoolInfo GetPoolInfo(uintptr_t address)
    {
        ...
        return PoolInfo { .handle = pool, .base = base, .base_mask = base_mask, .offset = address - base };
    }
    ...
    {
        return setup_.configurable_pool_base_address_ != kUninitializedPoolBaseAddress;
    }

#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
    PA_ALWAYS_INLINE static bool IsThreadIsolatedPoolInitialized()
    {
        return setup_.thread_isolated_pool_base_address_ != kUninitializedPoolBaseAddress;
    }
#endif
...
#endif
        return (address & regular_pool_base_mask) == setup_.regular_pool_base_address_;
    }
    ...
    PA_ALWAYS_INLINE static bool IsInCorePools(uintptr_t address)
    {
        ...
        bool ret = (address & core_pools_base_mask) == setup_.regular_pool_base_address_;
    ...
    {
        return (address & setup_.configurable_pool_base_mask_) == setup_.configurable_pool_base_address_;
    }
    ...
    {
        return (address & kThreadIsolatedPoolBaseMask) == setup_.thread_isolated_pool_base_address_;
    }
#endif
  ...
  //      (i+kSystemPageOffsetOfRegularPoolShadow)-th
    ...
    static_assert(!PA_BUILDFLAG(IS_IOS),
    ...
    PA_ALWAYS_INLINE static bool IsInPoolShadow(const void* ptr)
    {
        ...
        return (pool_shadow_address_ <= ptr_as_uintptr
            && (ptr_as_uintptr < pool_shadow_address_ + CorePoolSize() || ptr_as_uintptr < pool_shadow_address_ + kConfigurablePoolMaxSize));
    }
...
#error iOS is only supported with a dynamically sized GigaCase.
    ...
    static constexpr uintptr_t kCorePoolOffsetMask = static_cast<uintptr_t>(kCorePoolSize) - 1;
    static constexpr uintptr_t kCorePoolBaseMask = ~kCorePoolOffsetMask;
#endif // !PA_CONFIG(DYNAMICALLY_SELECT_POOL_SIZE)

#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
    static constexpr uintptr_t kThreadIsolatedPoolOffsetMask = static_cast<uintptr_t>(kThreadIsolatedPoolSize) - 1;
    static constexpr uintptr_t kThreadIsolatedPoolBaseMask = ~kThreadIsolatedPoolOffsetMask;
#endif

    // This must be set to such a value that IsIn*Pool() always returns false when
    // the pool isn't initialized.
    static constexpr uintptr_t kUninitializedPoolBaseAddress = static_cast<uintptr_t>(-1);
...
#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
        uintptr_t thread_isolated_pool_base_address_ = kUninitializedPoolBaseAddress;
#endif
#if PA_CONFIG(DYNAMICALLY_SELECT_POOL_SIZE)
...
#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
    static_assert(sizeof(PoolSetup) % SystemPageSize() == 0, "PoolSetup has to fill a page(s)");
#else
    static_assert(sizeof(PoolSetup) % kPartitionCachelineSize == 0, "PoolSetup has to fill a cacheline(s)");
#endif
...
PA_ALWAYS_INLINE PartitionAddressSpace::PoolInfo GetPoolInfo(uintptr_t address)
{
...
// Returns false for nullptr.
PA_ALWAYS_INLINE bool IsManagedByPartitionAllocConfigurablePool(uintptr_t address)
{
...
#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
// Returns false for nullptr.
PA_ALWAYS_INLINE bool IsManagedByPartitionAllocThreadIsolatedPool(uintptr_t address)
{
