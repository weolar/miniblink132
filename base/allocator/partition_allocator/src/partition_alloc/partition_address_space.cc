...
PA_NOINLINE void HandlePoolAllocFailure()
{
    ...
    } else if (alloc_page_error_code == ERROR_COMMITMENT_LIMIT || alloc_page_error_code == ERROR_COMMITMENT_MINIMUM) {
...
uintptr_t PartitionAddressSpace::pool_shadow_address_ = PartitionAddressSpace::kUninitializedPoolBaseAddress;
...
bool PartitionAddressSpace::IsIOSTestProcess()
{
    ...
    PA_CHECK(!rv);
    size_t executable_path_length = std::char_traits<char>::length(executable_path);
    ...
    auto has_suffix = [&](const char* suffix) -> bool {
        ...
        return std::char_traits<char>::compare(executable_path + (executable_path_length - suffix_length), suffix, suffix_length) == 0;
    };
...
void PartitionAddressSpace::Init()
{
    ...
    setup_.regular_pool_base_address_ = AllocPages(
        glued_pool_sizes, glued_pool_sizes, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc, pools_fd);
    ...
    if (setup_.regular_pool_base_address_ == 0x400000000) {
        uintptr_t new_base_address = AllocPages(glued_pool_sizes, glued_pool_sizes,
            PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc, pools_fd);
    ...
    setup_.brp_pool_base_address_ = setup_.regular_pool_base_address_ + core_pool_size;
    ...
    AddressPoolManager::GetInstance().Add(kRegularPoolHandle, setup_.regular_pool_base_address_, core_pool_size);
    AddressPoolManager::GetInstance().Add(kBRPPoolHandle, setup_.brp_pool_base_address_, core_pool_size);
    ...
    PA_DCHECK(IsInRegularPool(setup_.regular_pool_base_address_ + core_pool_size - 1));
    PA_DCHECK(!IsInRegularPool(setup_.regular_pool_base_address_ + core_pool_size));
    ...
    PA_DCHECK(IsInCorePools(setup_.regular_pool_base_address_ + core_pool_size - 1));
...
void PartitionAddressSpace::InitConfigurablePool(uintptr_t pool_base, size_t size)
{
    ...
    AddressPoolManager::GetInstance().Add(kConfigurablePoolHandle, setup_.configurable_pool_base_address_, size);
...
#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
void PartitionAddressSpace::InitThreadIsolatedPool(ThreadIsolationOption thread_isolation)
{
    ...
    AddressPoolManager::GetInstance().Add(kThreadIsolatedPoolHandle, setup_.thread_isolated_pool_base_address_, pool_size);

    PA_DCHECK(!IsInThreadIsolatedPool(setup_.thread_isolated_pool_base_address_ - 1));
    PA_DCHECK(IsInThreadIsolatedPool(setup_.thread_isolated_pool_base_address_));
    PA_DCHECK(IsInThreadIsolatedPool(setup_.thread_isolated_pool_base_address_ + pool_size - 1));
    PA_DCHECK(!IsInThreadIsolatedPool(setup_.thread_isolated_pool_base_address_ + pool_size));
...
void PartitionAddressSpace::UninitThreadIsolatedPoolForTesting()
{
    if (IsThreadIsolatedPoolInitialized()) {
        ...
        FreePages(setup_.thread_isolated_pool_base_address_, ThreadIsolatedPoolSize());
...
int CreateAnonymousFileForMapping([[maybe_unused]] const char* name, [[maybe_unused]] size_t size)
{
    int fd = -1;
...
void PartitionAddressSpace::InitShadowMetadata(PoolHandleMask mask)
{
    ...
    if (pool_shadow_address_ == kUninitializedPoolBaseAddress) {
        ...
        const size_t shadow_pool_size = std::max(ConfigurablePoolShadowSize(), std::max(CorePoolShadowSize(), CorePoolShadowSize()));

        // Reserve virtual address space for the shadow pool.
        uintptr_t pool_shadow_address = AllocPages(shadow_pool_size, PageAllocationGranularity(),
    ...
    if (ContainsFlags(mask, PoolHandleMask::kConfigurable)) {
        if (configurable_pool_fd_ == -1) {
            ...
            configurable_pool_fd_ = CreateAnonymousFileForMapping("configurable_pool_shadow", ConfigurablePoolShadowSize());
            configurable_pool_shadow_offset_ = pool_shadow_address_ - ConfigurablePoolBase() + SystemPageSize() * kSystemPageOffsetOfConfigurablePoolShadow;
        }
    }
    if (ContainsFlags(mask, PoolHandleMask::kBRP)) {
        if (brp_pool_fd_ == -1) {
            ...
            brp_pool_fd_ = CreateAnonymousFileForMapping("brp_pool_shadow", CorePoolShadowSize());
            brp_pool_shadow_offset_ = pool_shadow_address_ - BRPPoolBase() + SystemPageSize() * kSystemPageOffsetOfBRPPoolShadow;
        }
    }
    if (ContainsFlags(mask, PoolHandleMask::kRegular)) {
        if (regular_pool_fd_ == -1) {
            ...
            regular_pool_fd_ = CreateAnonymousFileForMapping("regular_pool_shadow", CorePoolShadowSize());
            regular_pool_shadow_offset_ = pool_shadow_address_ - RegularPoolBase() + SystemPageSize() * kSystemPageOffsetOfRegularPoolShadow;
        }
    }
}
...
// metadata.
void PartitionAddressSpace::MapMetadata(uintptr_t super_page, bool copy_metadata)
{
    PA_DCHECK(pool_shadow_address_);
    ...
    size_t file_offset = (super_page - base_address) >> kSuperPageShift << SystemPageShift();

#if PA_BUILDFLAG(IS_POSIX)
    uintptr_t writable_metadata = metadata + offset;
    void* ptr = mmap(reinterpret_cast<void*>(writable_metadata), SystemPageSize(), PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, pool_fd, file_offset);
    ...
    if (copy_metadata) [[unlikely]] {
        ...
        memcpy(reinterpret_cast<void*>(writable_metadata), reinterpret_cast<void*>(metadata), SystemPageSize());
    }

    ptr = mmap(reinterpret_cast<void*>(metadata), SystemPageSize(), PROT_READ, MAP_FIXED | MAP_SHARED, pool_fd, file_offset);
    PA_CHECK(ptr != MAP_FAILED);
...
void PartitionAddressSpace::UnmapShadowMetadata(uintptr_t super_page, pool_handle pool)
{
    PA_DCHECK(0u == (super_page & kSuperPageOffsetMask));
    std::ptrdiff_t offset;
...
#if PA_BUILDFLAG(IS_POSIX)
    void* ret = mmap(ptr, SystemPageSize(), PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
