...
{
    return std::max(::partition_alloc::internal::ASLRMask(), std::size_t { 2 } * ::partition_alloc::internal::ASLRMask());
}
...
TEST(PartitionAllocPageAllocatorTest, Rounding)
{
    ...
    EXPECT_EQ(PageAllocationGranularity(), RoundUpToPageAllocationGranularity(PageAllocationGranularity() - 1));
    EXPECT_EQ(PageAllocationGranularity(), RoundUpToPageAllocationGranularity(PageAllocationGranularity()));
    EXPECT_EQ(2 * PageAllocationGranularity(), RoundUpToPageAllocationGranularity(PageAllocationGranularity() + 1));
    EXPECT_EQ(0u, RoundDownToPageAllocationGranularity(0u));
    EXPECT_EQ(0u, RoundDownToPageAllocationGranularity(PageAllocationGranularity() - 1));
    EXPECT_EQ(PageAllocationGranularity(), RoundDownToPageAllocationGranularity(PageAllocationGranularity()));
    EXPECT_EQ(PageAllocationGranularity(), RoundDownToPageAllocationGranularity(PageAllocationGranularity() + 1));
    EXPECT_EQ(PageAllocationGranularity(), RoundDownToPageAllocationGranularity(2 * PageAllocationGranularity() - 1));
}
...
TEST(PartitionAllocPageAllocatorTest, AllocFailure)
{
    ...
    uintptr_t result
        = AllocPages(size, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kChromium);
    if (!result) {
...
TEST(PartitionAllocPageAllocatorTest, AllocPagesAligned)
{
    ...
    size_t sizes[]
        = { PageAllocationGranularity(), alignment - PageAllocationGranularity(), alignment, alignment + PageAllocationGranularity(), alignment * 4 };
    ...
    for (size_t size : sizes) {
        for (size_t offset : offsets) {
            uintptr_t buffer = AllocPagesWithAlignOffset(
                0, size, alignment, offset, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageTag::kChromium);
            EXPECT_TRUE(buffer);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadWriteTagged)
{
    ...
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWriteTagged), PageTag::kChromium);
    EXPECT_TRUE(buffer);
    int* buffer0 = reinterpret_cast<int*>(buffer);
    *buffer0 = 42;
    EXPECT_EQ(42, *buffer0);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadExecuteConfirmCFI)
{
    ...
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageTag::kChromium);
    ptrdiff_t function_range = reinterpret_cast<char*>(arm_bti_test_function_end) - reinterpret_cast<char*>(arm_bti_test_function);
    ...
    memcpy(reinterpret_cast<void*>(buffer), reinterpret_cast<void*>(arm_bti_test_function), function_range);

    // Next re-protect the page.
    SetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadExecuteProtected));
    ...
    BTITestFunction bti_invalid_fn = reinterpret_cast<BTITestFunction>(buffer + invalid_offset);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadWriteTaggedSynchronous)
{
    // This test checks that a page allocated with
    ...
    TagViolationReportingMode parent_tagging_mode = GetMemoryTaggingModeForCurrentThread();
    EXPECT_EXIT(
        {
    // Switch to synchronous mode.
#if PA_BUILDFLAG(IS_ANDROID)
            bool success = ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode::kSynchronous);
            EXPECT_TRUE(success);
#else
            ChangeMemoryTaggingModeForCurrentThread(TagViolationReportingMode::kSynchronous);
#endif // PA_BUILDFLAG(IS_ANDROID)
            EXPECT_EQ(GetMemoryTaggingModeForCurrentThread(), TagViolationReportingMode::kSynchronous);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadWriteTaggedAsynchronous)
{
    // This test checks that a page allocated with
    // PageAccessibilityConfiguration::kReadWriteTagged generates tag violations
...
#if PA_BUILDFLAG(IS_ANDROID)
            bool success = ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode::kAsynchronous);
            EXPECT_TRUE(success);
#else
            ChangeMemoryTaggingModeForCurrentThread(TagViolationReportingMode::kAsynchronous);
#endif // PA_BUILDFLAG(IS_ANDROID)
            EXPECT_EQ(GetMemoryTaggingModeForCurrentThread(), TagViolationReportingMode::kAsynchronous);
...
TEST(PartitionAllocPageAllocatorTest, DecommitErasesMemory)
{
    ...
    size_t size = PageAllocationGranularity();
    uintptr_t buffer
        = AllocPages(size, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageTag::kChromium);
    ASSERT_TRUE(buffer);

    memset(reinterpret_cast<void*>(buffer), 42, size);

    DecommitSystemPages(buffer, size, PageAccessibilityDisposition::kAllowKeepForPerf);
    RecommitSystemPages(
        buffer, size, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageAccessibilityDisposition::kAllowKeepForPerf);
...
// Test permission setting on POSIX, where we can set a trap handler.
    ...
    SetSystemPagesAccess(buffer, size, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite));
...
TEST(PartitionAllocPageAllocatorTest, MappedPagesAccounting)
{
    ...
    for (size_t offset : offsets) {
        uintptr_t data = AllocPagesWithAlignOffset(
            0, size, alignment, offset, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kChromium);
        ASSERT_TRUE(data);

        EXPECT_EQ(mapped_size_before + size, GetTotalMappedSize());

        DecommitSystemPages(data, size, PageAccessibilityDisposition::kAllowKeepForPerf);
        EXPECT_EQ(mapped_size_before + size, GetTotalMappedSize());
...
TEST(PartitionAllocPageAllocatorTest, AllocInaccessibleWillJitLater)
{
    ...
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessibleWillJitLater), PageTag::kChromium);
    EXPECT_TRUE(TrySetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite)));
    EXPECT_TRUE(TrySetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadExecute)));
    FreePages(buffer, PageAllocationGranularity());
}
...
TEST(PartitionAllocPageAllocatorTest, MAYBE_AllocReadWriteExecute)
{
    ...
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWriteExecute), PageTag::kChromium);
    EXPECT_TRUE(TrySetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite)));
    EXPECT_TRUE(TrySetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadExecute)));
    FreePages(buffer, PageAllocationGranularity());
}
