...
public:
    ...
    PA_ALWAYS_INLINE static EncodedNextFreelistEntry* EmplaceAndInitNull(void* slot_start_tagged)
    {
        // |slot_start_tagged| is MTE-tagged.
        auto* entry = new (slot_start_tagged) EncodedNextFreelistEntry(nullptr);
        return entry;
    }
    PA_ALWAYS_INLINE static EncodedNextFreelistEntry* EmplaceAndInitNull(uintptr_t slot_start)
    {
    ...
    PA_ALWAYS_INLINE static EncodedNextFreelistEntry* EmplaceAndInitForThreadCache(uintptr_t slot_start, EncodedNextFreelistEntry* next)
    {
        auto* entry = new (SlotStartAddr2Ptr(slot_start)) EncodedNextFreelistEntry(next);
        return entry;
    }
    ...
    {
        new (SlotStartAddr2Ptr(slot_start)) EncodedNextFreelistEntry(next, make_shadow_match);
    }
    ...
    template <bool crash_on_corruption> PA_ALWAYS_INLINE EncodedNextFreelistEntry* GetNextForThreadCache(size_t slot_size) const
    {
...
private:
    template <bool crash_on_corruption, bool for_thread_cache> PA_ALWAYS_INLINE EncodedNextFreelistEntry* GetNextInternal(size_t slot_size) const
    {
    ...
    template <bool for_thread_cache> PA_ALWAYS_INLINE static bool IsWellFormed(const EncodedNextFreelistEntry* here, const EncodedNextFreelistEntry* next)
    {
        ...
        return shadow_ptr_ok & same_super_page & marked_as_free_in_bitmap & not_in_metadata;
    }
