...
#if PA_BUILDFLAG(IS_ANDROID) || PA_BUILDFLAG(PA_ARCH_CPU_32_BITS) || PA_BUILDFLAG(IS_FUCHSIA)
...
{
    perf_test::PerfResultReporter reporter(kMetricPrefixMemoryAllocation, story_name);
...
public:
    ...
    ~PartitionAllocator() override
    {
        alloc_.DestructForTesting();
    }
    ...
    void Free(void* data) override
    {
        ...
        PartitionRoot::FreeInlineInUnknownRoot<partition_alloc::FreeFlags::kNoHooks>(data);
    }
...
public:
    explicit PartitionAllocatorWithAllocationStackTraceRecorder(bool register_hooks)
    ...
    void* Alloc(size_t size) override
    {
        return alloc_.AllocInline(size);
    }
...
private:
    ...
    PartitionRoot alloc_ { PartitionOptions {} };
    ::base::allocator::dispatcher::Dispatcher& dispatcher_ = ::base::allocator::dispatcher::Dispatcher::GetInstance();
...
public:
    ...
    {
        laps_per_second_ = test_fn_(allocator_);
    }
...
void DisplayResults(const std::string& story_name, float iterations_per_second)
{
    auto reporter = SetUpReporter(story_name);
    reporter.AddResult(kMetricThroughput, iterations_per_second);
    reporter.AddResult(kMetricTimePerAllocation, static_cast<size_t>(1e9 / iterations_per_second));
}

class MemoryAllocationPerfNode {
public:
    MemoryAllocationPerfNode* GetNext() const
    {
        return next_;
    }
    void SetNext(MemoryAllocationPerfNode* p)
    {
        next_ = p;
    }
...
float SingleBucket(Allocator* allocator)
{
    ...
    MemoryAllocationPerfNode* cur = first;
    do {
        auto* next = reinterpret_cast<MemoryAllocationPerfNode*>(allocator->Alloc(kAllocSize));
...
float MultiBucket(Allocator* allocator)
{
    ...
    do {
        for (int i = 0; i < kMultiBucketRounds; i++) {
            ...
            auto* next = reinterpret_cast<MemoryAllocationPerfNode*>(allocator->Alloc(size));
            PA_CHECK(next != nullptr);
            cur->SetNext(next);
            cur = next;
...
float MultiBucketWithFree(Allocator* allocator)
{
    ...
    for (int i = 0; i < kMultiBucketRounds; i++) {
        void* cur = allocator->Alloc(kMultiBucketMinimumSize + (i * kMultiBucketIncrement));
        PA_CHECK(cur != nullptr);
        elems.push_back(cur);
    }
...
std::unique_ptr<Allocator> CreateAllocator(AllocatorType type, bool use_alternate_bucket_dist)
{
    switch (type) {
    ...
    case AllocatorType::kPartitionAllocWithThreadCache:
        return std::make_unique<PartitionAllocatorWithThreadCache>(use_alternate_bucket_dist);
#if BUILDFLAG(ENABLE_ALLOCATION_STACK_TRACE_RECORDER)
    case AllocatorType::kPartitionAllocWithAllocationStackTraceRecorder:
        return std::make_unique<PartitionAllocatorWithAllocationStackTraceRecorder>(true);
#endif
    }
}

void LogResults(int thread_count, AllocatorType alloc_type, uint64_t total_laps_per_second, uint64_t min_laps_per_second)
{
    PA_LOG(INFO) << "RESULTSCSV: " << thread_count << "," << static_cast<int>(alloc_type) << "," << total_laps_per_second << "," << min_laps_per_second;
}

void RunTest(int thread_count, bool use_alternate_bucket_dist, AllocatorType alloc_type, float (*test_fn)(Allocator*), float (*noisy_neighbor_fn)(Allocator*),
    const char* story_base_name)
{
    ...
    if (noisy_neighbor_fn) {
        noisy_neighbor_thread = std::make_unique<TestLoopThread>(noisy_neighbor_fn, alloc.get());
    }
    ...
    std::string name = base::TruncatingStringPrintf("%s%s_%s_%d", kMetricPrefixMemoryAllocation, story_base_name, alloc_type_str, thread_count);

    DisplayResults(name + "_total", total_laps_per_second);
    DisplayResults(name + "_worst", min_laps_per_second);
    LogResults(thread_count, alloc_type, total_laps_per_second, min_laps_per_second);
}
...
class PartitionAllocMemoryAllocationPerfTest : public testing::TestWithParam<std::tuple<int, bool, AllocatorType>> {
    ...
    void SetUp() override
    {
        PartitionRoot::EnableShadowMetadata(partition_alloc::internal::PoolHandleMask::kRegular | partition_alloc::internal::PoolHandleMask::kBRP);
    }
#endif
};
...
INSTANTIATE_TEST_SUITE_P(, PartitionAllocMemoryAllocationPerfTest,
    ::testing::Combine(::testing::Values(1, 2, 3, 4), ::testing::Values(false, true),
        ::testing::Values(AllocatorType::kSystem, AllocatorType::kPartitionAlloc, AllocatorType::kPartitionAllocWithThreadCache
#if BUILDFLAG(ENABLE_ALLOCATION_STACK_TRACE_RECORDER)
            ,
...
TEST_P(PartitionAllocMemoryAllocationPerfTest, SingleBucket)
{
    ...
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), SingleBucket, nullptr, "SingleBucket");
}
#endif // defined(MEMORY_CONSTRAINED)
...
{
    auto params = GetParam();
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), SingleBucketWithFree, nullptr, "SingleBucketWithFree");
}
...
{
    auto params = GetParam();
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), MultiBucket, nullptr, "MultiBucket");
}
#endif // defined(MEMORY_CONSTRAINED)
...
{
    auto params = GetParam();
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), MultiBucketWithFree, nullptr, "MultiBucketWithFree");
}

TEST_P(PartitionAllocMemoryAllocationPerfTest, DirectMapped)
{
    auto params = GetParam();
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), DirectMapped, nullptr, "DirectMapped");
}

#if !defined(MEMORY_CONSTRAINED)
TEST_P(PartitionAllocMemoryAllocationPerfTest, DISABLED_MultiBucketWithNoisyNeighbor)
{
    auto params = GetParam();
    RunTest(std::get<int>(params), std::get<bool>(params), std::get<AllocatorType>(params), MultiBucket, DirectMapped, "MultiBucketWithNoisyNeighbor");
}
#endif // !defined(MEMORY_CONSTRAINED)
