...
template <typename T, typename... Args> std::unique_ptr<T, PADeleter> make_pa_unique(PartitionAllocator& alloc, Args&&... args)
{
    T* result = new (alloc.root()->Alloc(sizeof(T), nullptr)) T(std::forward<Args>(args)...);
    return std::unique_ptr<T, PADeleter>(result, PADeleter { alloc });
}

template <typename T> std::unique_ptr<T[], PADeleter> make_pa_array_unique(PartitionAllocator& alloc, size_t num)
{
...
static_assert(std::is_trivially_default_constructible_v<CompressedPointer<Base>>);
...
static_assert(std::is_trivially_default_constructible_v<UncompressedPointer<Base>>);
static_assert(std::is_trivially_copy_constructible_v<UncompressedPointer<Base>>);
static_assert(std::is_trivially_move_constructible_v<UncompressedPointer<Base>>);
...
template <typename TagType> class CompressedPointerTest : public ::testing::Test {
public:
#if PA_BUILDFLAG(ENABLE_POINTER_COMPRESSION)
    template <typename T> using PointerType = std::conditional_t<std::is_same_v<TagType, CompressedTypeTag>, CompressedPointer<T>, UncompressedPointer<T>>;
#else // PA_BUILDFLAG(ENABLE_POINTER_COMPRESSION)
    template <typename T> using PointerType = UncompressedPointer<T>;
#endif // PA_BUILDFLAG(ENABLE_POINTER_COMPRESSION)
...
TYPED_TEST(CompressedPointerTest, HeterogeneousValueConstructionSamePointerValue)
{
...
TYPED_TEST(CompressedPointerTest, HeterogeneousValueAssignmentSamePointerValue)
{
    using BasePointer = typename TestFixture::template PointerType<Base>;
    auto d = make_pa_unique<Derived>(this->allocator_);
    {
...
TYPED_TEST(CompressedPointerTest, HeterogeneousValueConstructionDifferentPointerValues)
{
...
TYPED_TEST(CompressedPointerTest, HeterogeneousValueAssignmentDifferentPointerValue)
{
    using MixinPointer = typename TestFixture::template PointerType<Mixin>;
    auto d = make_pa_unique<DerivedWithMixin>(this->allocator_);
    {
...
template <template <typename> class PointerType, typename T1, typename T2, typename U> void EqualityTest(U* raw)
{
...
template <template <typename> class PointerType, typename T1, typename T2, typename U> void CompareTest(U* array)
{
...
TYPED_TEST(CompressedPointerTest, EqualityDifferentPointerValues)
{
    ...
    EqualityTest<TestFixture::template PointerType, Mixin, DerivedWithMixin>(d.get());
    EqualityTest<TestFixture::template PointerType, DerivedWithMixin, Mixin>(d.get());
    EqualityTest<TestFixture::template PointerType, DerivedWithMixin, DerivedWithMixin>(d.get());
}
...
TYPED_TEST(CompressedPointerTest, CompareDifferentPointerValues)
{
    ...
    CompareTest<TestFixture::template PointerType, Mixin, DerivedWithMixin>(d.get());
    CompareTest<TestFixture::template PointerType, DerivedWithMixin, Mixin>(d.get());
    CompareTest<TestFixture::template PointerType, DerivedWithMixin, DerivedWithMixin>(d.get());
}
