...
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING) || (defined(__ARM_FEATURE_BTI_DEFAULT) && (__ARM_FEATURE_BTI_DEFAULT == 1) && __has_include(<sys/ifunc.h>))
...
int GetAccessFlags(PageAccessibilityConfiguration accessibility, bool mte_enabled, bool bti_enabled)
{
    ...
    case PageAccessibilityConfiguration::kReadWriteExecuteProtected:
        return PROT_READ | PROT_WRITE | PROT_EXEC | (bti_enabled ? PA_PROT_BTI : 0);
...
template <bool MteEnabled, bool BtiEnabled> int GetAccessFlags(PageAccessibilityConfiguration accessibility)
{
...
extern "C" GetAccessFlagsInternalFn* ResolveGetAccessFlags(uint64_t hwcap, struct __ifunc_arg_t* hw)
{
...
bool UseMapJit()
{
    ...
    base::apple::ScopedCFTypeRef<SecTaskRef> task(SecTaskCreateFromSelf(kCFAllocatorDefault));
    if (!task) {
        return true;
    }
    ...
    base::apple::ScopedCFTypeRef<CFTypeRef> jit_entitlement(SecTaskCopyValueForEntitlement(task.get(), CFSTR("com.apple.security.cs.allow-jit"), nullptr));
    if (!jit_entitlement) {
        return false;
    }

    return base::apple::CFCast<CFBooleanRef>(jit_entitlement.get()) == kCFBooleanTrue;
}
