...
TEST(HardeningTest, PartialCorruption)
{
    ...
    // encoded_next_.
    root.get_freelist_dispatcher()->EmplaceAndInitForTest(root.ObjectToSlotStart(data), to_corrupt, false);
...
TEST(HardeningTest, OffHeapPointerCrashing)
{
    ...
    // consistent.
    root.get_freelist_dispatcher()->EmplaceAndInitForTest(root.ObjectToSlotStart(data), to_corrupt, true);
...
TEST(HardeningTest, MetadataPointerCrashing)
{
    ...
    auto* metadata = SlotSpanMetadata<MetadataKind::kReadOnly>::FromSlotStart(slot_start);

    root.get_freelist_dispatcher()->EmplaceAndInitForTest(slot_start, metadata, true);
...
TEST(HardeningTest, SuccessfulCorruption)
{
    ...
    uintptr_t* zero_vector = reinterpret_cast<uintptr_t*>(root.Alloc<AllocFlags::kZeroFill>(100 * sizeof(uintptr_t), ""));
    ...
    root.get_freelist_dispatcher()->EmplaceAndInitForTest(root.ObjectToSlotStartUnchecked(data), to_corrupt, true);
...
TEST(HardeningTest, ConstructPoolOffsetFromStackPointerCrashing)
{
    ...
    EXPECT_DEATH(root.get_freelist_dispatcher()->EmplaceAndInitForTest(root.ObjectToSlotStart(data), to_corrupt, true), "");
}
