...
class PartitionAllocLightweightQuarantineTest : public testing::TestWithParam<QuarantineConfig> {
protected:
    void SetUp() override
    {
        allocator_ = std::make_unique<PartitionAllocatorForTesting>(PartitionOptions {});
    ...
    PartitionRoot* GetPartitionRoot() const
    {
        return allocator_->root();
    }

    QuarantineRoot* GetQuarantineRoot()
    {
        return &root_.value();
    }
    QuarantineBranch* GetQuarantineBranch()
    {
    ...
    {
        auto* slot_span = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::FromObject(object);
        ...
        return GetQuarantineBranch()->Quarantine(object, slot_span, slot_start, usable_size);
    }

    size_t GetObjectSize(void* object)
    {
        auto* entry_slot_span = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::FromObject(object);
...
constexpr QuarantineConfig kConfigSmall = { .lock_required = false, .branch_capacity_in_bytes = 256 };
constexpr QuarantineConfig kConfigLarge = { .lock_required = false, .branch_capacity_in_bytes = 256 };
constexpr QuarantineConfig kConfigSmallThreadSafe = { .lock_required = true, .branch_capacity_in_bytes = 2048 };
constexpr QuarantineConfig kConfigLargeThreadSafe = { .lock_required = true, .branch_capacity_in_bytes = 2048 };
INSTANTIATE_TEST_SUITE_P(PartitionAllocLightweightQuarantineTestInstantiation, PartitionAllocLightweightQuarantineTest,
    ::testing::Values(kConfigSmall, kConfigLarge, kConfigSmallThreadSafe, kConfigLargeThreadSafe));
