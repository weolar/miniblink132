...
uint64_t AmountOfPhysicalMemory()
{
    ...
    if (host_info(mach_host_self(), HOST_BASIC_INFO, reinterpret_cast<host_info_t>(&host_basic_info), &count) == KERN_SUCCESS) {
...
template <partition_alloc::AllocFlags alloc_flags, partition_alloc::FreeFlags free_flags = partition_alloc::FreeFlags::kNone>
void AllocateRandomly(partition_alloc::PartitionRoot* root, size_t count)
{
    std::vector<void*> allocations(count, nullptr);
    for (size_t i = 0; i < count; ++i) {
        const size_t size = kTestSizes[partition_alloc::internal::base::RandGenerator(kTestSizesCount)];
...
public:
    CountDanglingRawPtr()
    {
        ...
        partition_alloc::SetDanglingRawPtrDetectedFn(CountDanglingRawPtr::DanglingRawPtrDetected);
        partition_alloc::SetDanglingRawPtrReleasedFn(CountDanglingRawPtr::DanglingRawPtrReleased);
    }
    ~CountDanglingRawPtr()
    {
...
void SetDistributionForPartitionRoot(PartitionRoot* root, BucketDistribution distribution)
{
    switch (distribution) {
...
const std::vector<PartitionAllocTestParam> GetPartitionAllocTestParams()
{
    ...
    params.emplace_back(PartitionAllocTestParam { BucketDistribution::kNeutral, false });
    params.emplace_back(PartitionAllocTestParam { BucketDistribution::kDenser, false });
#if PA_BUILDFLAG(ENABLE_PKEYS)
    if (CPUHasPkeySupport()) {
        params.emplace_back(PartitionAllocTestParam { BucketDistribution::kNeutral, true });
        params.emplace_back(PartitionAllocTestParam { BucketDistribution::kDenser, true });
    }
#endif
    return params;
}

class PartitionAllocTest : public testing::TestWithParam<PartitionAllocTestParam> {
protected:
    class ScopedPageAllocation {
    public:
        ScopedPageAllocation(PartitionAllocator& allocator, base::CheckedNumeric<size_t> npages)
            : allocator_(allocator)
            , npages_(npages)
            , ptr_(static_cast<char*>(allocator_.root()->Alloc((npages * SystemPageSize() - ExtraAllocSize(allocator_)).ValueOrDie(), type_name)))
        {
        }

        ~ScopedPageAllocation()
        {
            allocator_.root()->Free(ptr_);
        }

        void TouchAllPages()
        {
            memset(ptr_, 'A', ((npages_ * SystemPageSize()) - ExtraAllocSize(allocator_)).ValueOrDie());
        }
    ...
    void InitializeTestRoot(PartitionRoot* root, PartitionOptions opts, PartitionTestOptions test_opts)
    {
        root->Init(opts);
    ...
    std::unique_ptr<PartitionRoot> CreateCustomTestRoot(PartitionOptions opts, PartitionTestOptions test_opts)
    {
    ...
    PartitionOptions GetCommonPartitionOptions()
    {
        ...
        opts.scheduler_loop_quarantine_branch_capacity_in_bytes = std::numeric_limits<size_t>::max();
        return opts;
    }
    ...
    void InitializeMainTestAllocators()
    {
        ...
        PartitionOptions::EnableToggle enable_backup_ref_ptr = PartitionOptions::kEnabled;
#endif
#if PA_BUILDFLAG(ENABLE_PKEYS)
        ...
        InitializeTestRoot(pkey_allocator.root(), pkey_opts, PartitionTestOptions { .use_memory_reclaimer = true });
        ...
        opts.memory_tagging = {
            .enabled = partition_alloc::internal::base::CPU::GetInstanceNoAllocation().has_mte() ? PartitionOptions::kEnabled : PartitionOptions::kDisabled,
        };
#endif // PA_BUILDFLAG(HAS_MEMORY_TAGGING)
        InitializeTestRoot(allocator.root(), opts,
            PartitionTestOptions { .use_memory_reclaimer = true, .uncap_empty_slot_span_memory = true, .set_bucket_distribution = true });
    }
    ...
    void SetUp() override
    {
        PartitionRoot::SetStraightenLargerSlotSpanFreeListsMode(StraightenLargerSlotSpanFreeListsMode::kOnlyWhenUnprovisioning);
        ...
        PartitionRoot::EnableShadowMetadata(internal::PoolHandleMask::kRegular | internal::PoolHandleMask::kBRP);
#endif
    ...
    {
        allocator.root()->PurgeMemory(PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages);
    ...
    size_t GetNumPagesPerSlotSpan(size_t size)
    {
        ...
        return (bucket->num_system_pages_per_slot_span + (NumSystemPagesPerPartitionPage() - 1)) / NumSystemPagesPerPartitionPage();
    }
    ...
    const SlotSpan* GetFullSlotSpan(size_t size)
    {
        ...
        size_t bucket_index = SizeToIndex(real_size);
        PartitionRoot::Bucket* bucket = &allocator.root()->buckets[bucket_index];
        size_t num_slots = (bucket->num_system_pages_per_slot_span * SystemPageSize()) / bucket->slot_size;
        uintptr_t first = 0;
        uintptr_t last = 0;
        size_t i;
        ...
        EXPECT_TRUE(bucket->active_slot_spans_head != SlotSpan::get_sentinel_slot_span());
    ...
    void DoReturnNullTest(size_t alloc_size, ReturnNullTestMode mode)
    {
        ...
        if (!IsLargeMemoryDevice()) {
            PA_LOG(WARNING) << "Skipping test on this device because of crbug.com/678782";
        ...
        void** ptrs = static_cast<void**>(allocator.root()->Alloc(num_allocations * sizeof(void*), type_name));
        int i;

        for (i = 0; i < num_allocations; ++i) {
            switch (mode) {
            case kPartitionAlloc: {
                ptrs[i] = allocator.root()->Alloc<AllocFlags::kReturnNull>(alloc_size, type_name);
                break;
            }
            case kPartitionRealloc: {
                ptrs[i] = allocator.root()->Alloc<AllocFlags::kReturnNull>(1, type_name);
                ptrs[i] = allocator.root()->Realloc<AllocFlags::kReturnNull>(ptrs[i], alloc_size, type_name);
                break;
            }
            }
    ...
    PA_NOINLINE void Free(void* ptr)
    {
    ...
    bool UseThreadIsolatedPool() const
    {
        return GetParam().use_pkey_pool;
    }
...
INSTANTIATE_TEST_SUITE_P(AlternateTestParams, PartitionAllocDeathTest, testing::ValuesIn(GetPartitionAllocTestParams()));
...
void FreeFullSlotSpan(PartitionRoot* root, const SlotSpan* slot_span)
{
    ...
    size_t num_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / size;
...
#define CHECK_PAGE_IN_CORE(ptr, in_core) EXPECT_TRUE(CheckPageInCore(ptr, in_core))
#else
...
public:
    ...
    void PartitionDumpTotals(const char* partition_name, const PartitionMemoryStats* stats) override
    {
    ...
    void PartitionsDumpBucketStats([[maybe_unused]] const char* partition_name, const PartitionBucketMemoryStats* stats) override
    {
...
{
    partition_alloc::internal::PartitionSuperPageExtentEntry<partition_alloc::internal::MetadataKind::kReadOnly>* extent = root->first_extent;
    while (extent != nullptr) {
        uintptr_t super_page = partition_alloc::internal::SuperPagesBeginFromExtent(extent);
        uintptr_t super_page_end = partition_alloc::internal::SuperPagesEndFromExtent(extent);
...
{
    ::partition_alloc::internal::ScopedGuard locker { partition_alloc::internal::PartitionRootLock(root) };

    partition_alloc::internal::PartitionDirectMapExtent<partition_alloc::internal::MetadataKind::kReadOnly>* extent = root->direct_map_list;
    while (extent != nullptr) {
        uintptr_t super_page = reinterpret_cast<uintptr_t>(extent) & kSuperPageBaseMask;
...
bool IsManagedByNormalBucketsForTesting(uintptr_t address, [[maybe_unused]] PartitionRoot* root)
{
...
bool IsManagedByDirectMapForTesting(uintptr_t address, [[maybe_unused]] PartitionRoot* root)
{
...
bool IsManagedByNormalBucketsOrDirectMapForTesting(uintptr_t address, [[maybe_unused]] PartitionRoot* root)
{
    return IsManagedByNormalBucketsOrDirectMap(address)
#if PA_BUILDFLAG(IS_APPLE)
        && (IsManagedByNormalBucketsForTesting(address, root) || IsManagedByDirectMapForTesting(address, root))
#endif // PA_BUILDFLAG(IS_APPLE)
        ;
}
...
INSTANTIATE_TEST_SUITE_P(AlternateTestParams, PartitionAllocTest, testing::ValuesIn(GetPartitionAllocTestParams()));

// Check that the most basic of allocate / free pairs work.
TEST_P(PartitionAllocTest, Basic)
{
    PartitionRoot::Bucket* bucket = &allocator.root()->buckets[test_bucket_index_];
    ...
    EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize() + kPointerOffset, UntagPtr(ptr) & kSuperPageOffsetMask);
...
TEST_P(PartitionAllocTest, MultiSlotSpans)
{
    ...
    EXPECT_EQ(SlotSpan::ToSlotSpanStart(slot_span) & kSuperPageBaseMask, SlotSpan::ToSlotSpanStart(slot_span2) & kSuperPageBaseMask);
...
TEST_P(PartitionAllocTest, SlotSpanTransitions)
{
    ...
    void* ptr = allocator.root()->SlotStartToObject(SlotSpan::ToSlotSpanStart(slot_span1));
    allocator.root()->Free(ptr);
    EXPECT_EQ(slot_span1, bucket->active_slot_spans_head);
    ...
    ptr = allocator.root()->SlotStartToObject(SlotSpan::ToSlotSpanStart(slot_span2));
    allocator.root()->Free(ptr);
    ...
    ptr = allocator.root()->SlotStartToObject(SlotSpan::ToSlotSpanStart(slot_span1));
    allocator.root()->Free(ptr);
...
TEST_P(PartitionAllocTest, ExtraAllocSize)
{
    ...
    size_t slot_size = 64;
    size_t bucket_index = PartitionRoot::SizeToBucketIndex(slot_size, GetBucketDistribution());
    PartitionRoot::Bucket* bucket = &allocator.root()->buckets[bucket_index];
    ...
    size_t capacity1 = allocator.root()->AllocationCapacityFromSlotStart(allocator.root()->ObjectToSlotStart(ptr1));
    size_t capacity2 = allocator.root()->AllocationCapacityFromSlotStart(allocator.root()->ObjectToSlotStart(ptr2));
...
TEST_P(PartitionAllocTest, PreferSlotSpansWithProvisionedEntries)
{
    ...
    size_t real_size = size + ExtraAllocSize(allocator);
    size_t bucket_index = PartitionRoot::SizeToBucketIndex(real_size, GetBucketDistribution());
    PartitionRoot::Bucket* bucket = &allocator.root()->buckets[bucket_index];
    ...
    for (int span_index = 0; span_index < kSpans; span_index++) {
        for (size_t i = 0; i < slots_per_span; i++) {
            allocated_memory_spans[span_index].push_back(allocator.root()->Alloc(size));
        }
    }
    ...
    for (auto* span = bucket->active_slot_spans_head; span; span = span->next_slot_span) {
...
TEST_P(PartitionAllocTest, FreeSlotSpanListSlotSpanTransitions)
{
    ...
    size_t num_to_fill_free_list_slot_span = PartitionPageSize() / (sizeof(SlotSpan) + ExtraAllocSize(allocator));
    ...
    auto slot_spans = std::make_unique<const SlotSpan*[]>(num_to_fill_free_list_slot_span);

    size_t i;
    for (i = 0; i < num_to_fill_free_list_slot_span; ++i) {
        slot_spans[i] = GetFullSlotSpan(kTestAllocSize);
    }
    EXPECT_EQ(slot_spans[num_to_fill_free_list_slot_span - 1], bucket->active_slot_spans_head);
    for (i = 0; i < num_to_fill_free_list_slot_span; ++i) {
...
TEST_P(PartitionAllocTest, MultiPageAllocs)
{
    ...
    size_t num_slot_spans_needed
        = (NumPartitionPagesPerSuperPage() - 2 - partition_alloc::internal::NumPartitionPagesPerFreeSlotBitmap()) / num_pages_per_slot_span;
    ...
    for (i = 0; i < num_slot_spans_needed; ++i) {
        ...
        if (i == num_slot_spans_needed - 1) {
            ...
            uintptr_t second_super_page_offset = slot_span_start & kSuperPageOffsetMask;
            ...
            // the second page.
            EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize(), second_super_page_offset);
        }
    }
    for (i = 0; i < num_slot_spans_needed; ++i) {
        FreeFullSlotSpan(allocator.root(), slot_spans[i]);
...
TEST_P(PartitionAllocTest, Alloc)
{
    ...
    size_t base_size = partition_alloc::internal::base::bits::AlignUp(ExtraAllocSize(allocator), kAlignment) - ExtraAllocSize(allocator);
    ...
    *char_ptr = 'A';
    ...
    PA_EXPECT_PTR_EQ(ptr, new_ptr);
    new_ptr = allocator.root()->Realloc(ptr, base_size + kSmallestBucket, type_name);
    PA_EXPECT_PTR_EQ(ptr, new_ptr);

    // Change the size of the realloc, switching buckets.
    new_ptr = allocator.root()->Realloc(ptr, base_size + kSmallestBucket + 1, type_name);
    ...
    EXPECT_EQ(kUninitializedByte, static_cast<unsigned char>(*(new_char_ptr + kSmallestBucket)));
#endif
    *new_char_ptr = 'B';
...
TEST_P(PartitionAllocTest, AllocSizes)
{
...

    {
        ...
        auto* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
...
#if !(PA_BUILDFLAG(IS_LINUX) && PA_BUILDFLAG(PA_ARCH_CPU_ARM64))
    {
        ...
        const size_t size = PartitionPageSize() * kMaxPartitionPagesPerRegularSlotSpan + 1;
        ...
        auto* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
        auto* slot_span2 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr3));
        ...
        EXPECT_EQ(kFreedByte, *(static_cast<unsigned char*>(new_ptr_1) + (size - 1)));
#endif
    }
...

    {
        ...
        *(char_ptr + (size - 1)) = 'A';
        ...
        EXPECT_EQ(nullptr, allocator.root()->Alloc<AllocFlags::kReturnNull>(3u * 1024 * 1024 * 1024, type_name));
    }
}
...
TEST_P(PartitionAllocTest, AllocGetSizeAndStart)
{
    ...
    predicted_capacity = allocator.root()->AllocationCapacityFromRequestedSize(requested_size);
    ptr = allocator.root()->Alloc(requested_size, type_name);
    EXPECT_TRUE(ptr);
    uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
    actual_capacity = allocator.root()->AllocationCapacityFromSlotStart(slot_start);
    ...
    if (UseBRPPool()) {
        ...
        for (size_t offset = 0; offset < requested_size; ++offset) {
            EXPECT_EQ(PartitionAllocGetSlotStartAndSizeInBRPPool(address + offset).slot_start, slot_start);
        }
    }
#endif // PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
    allocator.root()->Free(ptr);
    ...
    requested_size = num * SystemPageSize() - SystemPageSize() - ExtraAllocSize(allocator);
    predicted_capacity = allocator.root()->AllocationCapacityFromRequestedSize(requested_size);
    ptr = allocator.root()->Alloc(requested_size, type_name);
    EXPECT_TRUE(ptr);
    slot_start = allocator.root()->ObjectToSlotStart(ptr);
...
TEST_P(PartitionAllocTest, MTEProtectsFreedPtr)
{
    ...
    uint64_t* ptr1 = static_cast<uint64_t*>(allocator.root()->Alloc(alloc_size, type_name));
    EXPECT_TRUE(ptr1);
    ...
    uint64_t* ptr2 = static_cast<uint64_t*>(allocator.root()->Alloc(alloc_size, type_name));
    ...
    // ptr1 and ptr2.
    allocator.root()->Free(ptr2);
    uint64_t* ptr3 = static_cast<uint64_t*>(allocator.root()->Alloc(alloc_size, type_name));
...
TEST_P(PartitionAllocTest, IsPtrWithinSameAlloc)
{
    ...
    const size_t kMinReasonableTestSize = partition_alloc::internal::base::bits::AlignUp(ExtraAllocSize(allocator) + 1, kAlignment);
    ASSERT_GT(kMinReasonableTestSize, ExtraAllocSize(allocator));
    const size_t kSizes[] = { kMinReasonableTestSize, 256, SystemPageSize(), PartitionPageSize(), MaxRegularSlotSpanSize(), MaxRegularSlotSpanSize() + 1,
        MaxRegularSlotSpanSize() + SystemPageSize(), MaxRegularSlotSpanSize() + PartitionPageSize(), kMaxBucketed, kMaxBucketed + 1,
        kMaxBucketed + SystemPageSize(), kMaxBucketed + PartitionPageSize(), kSuperPageSize };
    ...
    for (size_t size : kSizes) {
        ...
        if (size <= MaxRegularSlotSpanSize()) {
            ASSERT_EQ(requested_size, allocator.root()->AllocationCapacityFromRequestedSize(requested_size));
        }
        ...
        for (void*& ptr : ptrs) {
            ...
            if (size <= MaxRegularSlotSpanSize()) {
                uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
                EXPECT_EQ(requested_size, allocator.root()->AllocationCapacityFromSlotStart(slot_start));
            }

            uintptr_t address = UntagPtr(ptr);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address - kFarFarAwayDelta, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address - kSuperPageSize, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address - 1, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address, 0u), PtrPosWithinAlloc::kInBounds);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size / 2, 0u), PtrPosWithinAlloc::kInBounds);
#if PA_BUILDFLAG(BACKUP_REF_PTR_POISON_OOB_PTR)
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size - 1, 1u), PtrPosWithinAlloc::kInBounds);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size, 1u), PtrPosWithinAlloc::kAllocEnd);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size - 4, 4u), PtrPosWithinAlloc::kInBounds);
            for (size_t subtrahend = 0; subtrahend < 4; subtrahend++) {
                EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size - subtrahend, 4u), PtrPosWithinAlloc::kAllocEnd);
            }
#else // PA_BUILDFLAG(BACKUP_REF_PTR_POISON_OOB_PTR)
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size, 0u), PtrPosWithinAlloc::kInBounds);
#endif
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size + 1, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size + kSuperPageSize, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address, address + requested_size + kFarFarAwayDelta, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size + kFarFarAwayDelta, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size + kSuperPageSize, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size + 1, 0u), PtrPosWithinAlloc::kFarOOB);
#if PA_BUILDFLAG(BACKUP_REF_PTR_POISON_OOB_PTR)
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size - 1, address + requested_size - 1, 1u), PtrPosWithinAlloc::kInBounds);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size - 1, address + requested_size, 1u), PtrPosWithinAlloc::kAllocEnd);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size, 1u), PtrPosWithinAlloc::kAllocEnd);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size - 4, address + requested_size - 4, 4u), PtrPosWithinAlloc::kInBounds);
            for (size_t addend = 1; addend < 4; addend++) {
                EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size - 4, address + requested_size - 4 + addend, 4u), PtrPosWithinAlloc::kAllocEnd);
            }
#else // PA_BUILDFLAG(BACKUP_REF_PTR_POISON_OOB_PTR)
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size, 0u), PtrPosWithinAlloc::kInBounds);
#endif
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address + requested_size - (requested_size / 2), 0u), PtrPosWithinAlloc::kInBounds);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address, 0u), PtrPosWithinAlloc::kInBounds);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address - 1, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address - kSuperPageSize, 0u), PtrPosWithinAlloc::kFarOOB);
            EXPECT_EQ(IsPtrWithinSameAlloc(address + requested_size, address - kFarFarAwayDelta, 0u), PtrPosWithinAlloc::kFarOOB);
        }
...
TEST_P(PartitionAllocTest, GetSlotStartMultiplePages)
{
    ...
    for (const auto& bucket : root->buckets) {
        if ((root->buckets + SizeToIndex(bucket.slot_size))->slot_size != bucket.slot_size) {
            continue;
        }
    ...
    EXPECT_GT(bucket->num_system_pages_per_slot_span, PartitionPageSize() / SystemPageSize());
    size_t num_slots = (bucket->num_system_pages_per_slot_span * SystemPageSize()) / real_size;
    ...
    for (void* ptr : ptrs) {
        uintptr_t address = UntagPtr(ptr);
        uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
        EXPECT_EQ(allocator.root()->AllocationCapacityFromSlotStart(slot_start), requested_size);
...
TEST_P(PartitionAllocTest, Realloc)
{
    ...
    EXPECT_EQ(allocator.root()->ObjectToSlotStart(ptr), UntagPtr(slot_span->get_freelist_head()));
    ...
    ptr = allocator.root()->Alloc(size, type_name);
    uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
    size_t actual_capacity = allocator.root()->AllocationCapacityFromSlotStart(slot_start);
    ...
    EXPECT_EQ(actual_capacity - SystemPageSize(), allocator.root()->AllocationCapacityFromSlotStart(slot_start2));
    void* ptr3 = allocator.root()->Realloc(ptr2, size - 32 * SystemPageSize(), type_name);
    uintptr_t slot_start3 = allocator.root()->ObjectToSlotStart(ptr3);
    EXPECT_EQ(slot_start2, slot_start3);
    EXPECT_EQ(actual_capacity - 32 * SystemPageSize(), allocator.root()->AllocationCapacityFromSlotStart(slot_start3));
    ...
    EXPECT_EQ(actual_capacity, allocator.root()->AllocationCapacityFromSlotStart(slot_start));
    ...
    EXPECT_EQ(slot_start, slot_start2);
    EXPECT_EQ(actual_capacity, allocator.root()->AllocationCapacityFromSlotStart(slot_start2));
...
TEST_P(PartitionAllocTest, ReallocDirectMapAligned)
{
    ...
    for (size_t alignment : alignments) {
        ...
        ASSERT_GT(size, kMaxBucketed);
        void* ptr = allocator.root()->AllocInternalForTesting(size, alignment, type_name);
        uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
        size_t actual_capacity = allocator.root()->AllocationCapacityFromSlotStart(slot_start);
        void* ptr2 = allocator.root()->Realloc(ptr, size - SystemPageSize(), type_name);
        uintptr_t slot_start2 = allocator.root()->ObjectToSlotStart(ptr2);
        EXPECT_EQ(slot_start, slot_start2);
        EXPECT_EQ(actual_capacity - SystemPageSize(), allocator.root()->AllocationCapacityFromSlotStart(slot_start2));
        void* ptr3 = allocator.root()->Realloc(ptr2, size - 32 * SystemPageSize(), type_name);
...
TEST_P(PartitionAllocTest, ReallocDirectMapAlignedRelocate)
{
    ...
    ASSERT_GT(size, kMaxBucketed);
    void* ptr = allocator.root()->AllocInternalForTesting(size, kMaxSupportedAlignment, type_name);
    ...
    ASSERT_GT(size, kMaxBucketed);
    ptr = allocator.root()->AllocInternalForTesting(size, kMaxSupportedAlignment, type_name);
...
TEST_P(PartitionAllocTest, PartialPageFreelists)
{
    ...
    auto* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    size_t total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (big_size + ExtraAllocSize(allocator));
    ...
    auto* slot_span2 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr5));
    ...
    size_t non_dividing_size = SystemPageSize() / 2 + 1 - ExtraAllocSize(allocator);
    ...
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / bucket->slot_size;
    ...
    slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (medium_size + ExtraAllocSize(allocator));
    size_t first_slot_span_slots = SystemPageSize() / (medium_size + ExtraAllocSize(allocator));
    EXPECT_EQ(2u, first_slot_span_slots);
    EXPECT_EQ(total_slots - first_slot_span_slots, slot_span->num_unprovisioned_slots);
    ...
    slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (small_size + ExtraAllocSize(allocator));
    first_slot_span_slots = SystemPageSize() / (small_size + ExtraAllocSize(allocator));
    EXPECT_EQ(total_slots - first_slot_span_slots, slot_span->num_unprovisioned_slots);
    ...
    size_t very_small_size = (ExtraAllocSize(allocator) <= 32) ? (32 - ExtraAllocSize(allocator)) : (64 - ExtraAllocSize(allocator));
    size_t very_small_adjusted_size = allocator.root()->AdjustSize0IfNeeded(very_small_size);
    bucket_index = SizeToIndex(very_small_adjusted_size + ExtraAllocSize(allocator));
    bucket = &allocator.root()->buckets[bucket_index];
    EXPECT_EQ(nullptr, bucket->empty_slot_spans_head);
    ...
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (very_small_actual_size + ExtraAllocSize(allocator));
    first_slot_span_slots = SystemPageSize() / (very_small_actual_size + ExtraAllocSize(allocator));
    EXPECT_EQ(total_slots - first_slot_span_slots, slot_span->num_unprovisioned_slots);
    ...
    size_t page_and_a_half_size = (SystemPageSize() + (SystemPageSize() / 2)) - ExtraAllocSize(allocator);
    ...
    EXPECT_TRUE(!slot_span->get_freelist_head());
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (page_and_a_half_size + ExtraAllocSize(allocator));
    EXPECT_EQ(total_slots - 1, slot_span->num_unprovisioned_slots);
    ...
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    EXPECT_TRUE(slot_span->get_freelist_head());
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (page_size + ExtraAllocSize(allocator));
    EXPECT_EQ(total_slots - 2, slot_span->num_unprovisioned_slots);
    allocator.root()->Free(ptr);
}
...
TEST_P(PartitionAllocTest, SlotSpanRefilling)
{
    ...
    // free it.
    void* ptr2 = allocator.root()->SlotStartToObject(SlotSpan::ToSlotSpanStart(slot_span1));
    allocator.root()->Free(ptr2);
    ptr2 = allocator.root()->SlotStartToObject(SlotSpan::ToSlotSpanStart(slot_span2));
    allocator.root()->Free(ptr2);
...
TEST_P(PartitionAllocTest, MappingCollision)
{
    ...
    size_t num_slot_span_needed
        = (NumPartitionPagesPerSuperPage() - 2 - partition_alloc::internal::NumPartitionPagesPerFreeSlotBitmap()) / num_pages_per_slot_span;
    size_t num_partition_pages_needed = num_slot_span_needed * num_pages_per_slot_span;

    auto first_super_page_pages = std::make_unique<const SlotSpan*[]>(num_partition_pages_needed);
    auto second_super_page_pages = std::make_unique<const SlotSpan*[]>(num_partition_pages_needed);
    ...
    uintptr_t slot_span_start = SlotSpan::ToSlotSpanStart(first_super_page_pages[0]);
    EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize(), slot_span_start & kSuperPageOffsetMask);
    uintptr_t super_page = slot_span_start - PartitionPageSize() - partition_alloc::internal::ReservedFreeSlotBitmapSize();
    // Map a single system page either side of the mapping for our allocations,
    // with the goal of tripping up alignment of the next mapping.
    uintptr_t map1 = AllocPages(super_page - PageAllocationGranularity(), PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc);
    EXPECT_TRUE(map1);
    uintptr_t map2 = AllocPages(super_page + kSuperPageSize, PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc);
    EXPECT_TRUE(map2);
    ...
    EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize(), super_page & kSuperPageOffsetMask);
    super_page -= PartitionPageSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize();
    // Map a single system page either side of the mapping for our allocations,
    // with the goal of tripping up alignment of the next mapping.
    map1 = AllocPages(super_page - PageAllocationGranularity(), PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWriteTagged), PageTag::kPartitionAlloc);
    EXPECT_TRUE(map1);
    map2 = AllocPages(super_page + kSuperPageSize, PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWriteTagged), PageTag::kPartitionAlloc);
    EXPECT_TRUE(map2);
    EXPECT_TRUE(TrySetSystemPagesAccess(map1, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible)));
    EXPECT_TRUE(TrySetSystemPagesAccess(map2, PageAllocationGranularity(), PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible)));

    auto* slot_span_in_third_super_page = GetFullSlotSpan(kTestAllocSize);
    FreePages(map1, PageAllocationGranularity());
    FreePages(map2, PageAllocationGranularity());

    EXPECT_EQ(0u, SlotSpan::ToSlotSpanStart(slot_span_in_third_super_page) & PartitionPageOffsetMask());

    // And make sure we really did get a page in a new superpage.
    EXPECT_NE(SlotSpan::ToSlotSpanStart(first_super_page_pages[0]) & kSuperPageBaseMask,
        SlotSpan::ToSlotSpanStart(slot_span_in_third_super_page) & kSuperPageBaseMask);
    EXPECT_NE(SlotSpan::ToSlotSpanStart(second_super_page_pages[0]) & kSuperPageBaseMask,
        SlotSpan::ToSlotSpanStart(slot_span_in_third_super_page) & kSuperPageBaseMask);
...
TEST_P(PartitionAllocTest, FreeCache)
{
    ...
    size_t expected_committed_size = kUseLazyCommit ? SystemPageSize() : PartitionPageSize();
    EXPECT_EQ(expected_committed_size, allocator.root()->get_total_size_of_committed_pages());
    allocator.root()->Free(ptr);
    EXPECT_EQ(0u, slot_span->num_allocated_slots);
    EXPECT_TRUE(slot_span->in_empty_cache());
    EXPECT_TRUE(slot_span->get_freelist_head());
...
TEST_P(PartitionAllocTest, LostFreeSlotSpansBug)
{
    ...
    const SlotSpan* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    const SlotSpan* slot_span2 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr2));
...
#else
#define MAYBE_RepeatedAllocReturnNullDirect DISABLED_RepeatedAllocReturnNullDirect
#define MAYBE_RepeatedReallocReturnNullDirect DISABLED_RepeatedReallocReturnNullDirect
#endif
...
TEST_P(PartitionAllocDeathTest, MAYBE_RepeatedAllocReturnNullDirect)
{
    ...
    EXPECT_DEATH(DoReturnNullTest(direct_map_size, kPartitionAlloc), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, MAYBE_RepeatedReallocReturnNullDirect)
{
    size_t direct_map_size = 32 * 1024 * 1024;
    ASSERT_GT(direct_map_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(direct_map_size, kPartitionRealloc), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, DISABLED_RepeatedAllocReturnNull)
{
    ...
    EXPECT_DEATH(DoReturnNullTest(single_slot_size, kPartitionAlloc), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, DISABLED_RepeatedReallocReturnNull)
{
    ...
    ASSERT_GT(single_slot_size, MaxRegularSlotSpanSize());
    ASSERT_LE(single_slot_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(single_slot_size, kPartitionRealloc), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, MTEProtectsFreedPtr)
{
    ...
    size_t alloc_size = 64 - ExtraAllocSize(allocator);
    uint64_t* ptr = static_cast<uint64_t*>(allocator.root()->Alloc(alloc_size, type_name));
    EXPECT_TRUE(ptr);
...
#if !PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) || (PA_BUILDFLAG(HAS_64_BIT_POINTERS) && PA_BUILDFLAG(PA_ARCH_CPU_LITTLE_ENDIAN))
...
{
    const size_t kSizes[] = { kMaxBucketed + ExtraAllocSize(allocator) + 1, kMaxBucketed + SystemPageSize(), kMaxBucketed + PartitionPageSize(),
        partition_alloc::internal::base::bits::AlignUp(kMaxBucketed + kSuperPageSize, kSuperPageSize)
            - PartitionRoot::GetDirectMapMetadataAndGuardPagesSize() };
    for (size_t size : kSizes) {
        ASSERT_GT(size, kMaxBucketed);
        size -= ExtraAllocSize(allocator);
        EXPECT_GT(size, kMaxBucketed) << "allocation not large enough for direct allocation";
        void* ptr = allocator.root()->Alloc(size, type_name);
        ...
        EXPECT_DEATH(*(char_ptr + partition_alloc::internal::base::bits::AlignUp(size, SystemPageSize())) = 'A', "");
...
#if !PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) && PA_CONFIG(HAS_FREELIST_SHADOW_ENTRY)
...
TEST_P(PartitionAllocDeathTest, FreelistCorruption)
{
    ...
    void** fake_freelist_entry = static_cast<void**>(allocator.root()->Alloc(alloc_size));
...
TEST_P(PartitionAllocDeathTest, OffByOneDetectionByCookie)
{
    ...
    auto* slot_span = PartitionRoot::ReadOnlySlotSpanMetadata::FromObjectInnerPtr(array);
...
TEST_P(PartitionAllocDeathTest, OffByOneDetectionByCookieWithRealisticData)
{
    ...
    auto* slot_span = PartitionRoot::ReadOnlySlotSpanMetadata::FromObjectInnerPtr(array);
    size_t usable_size = allocator.root()->GetSlotUsableSize(slot_span) / sizeof(void*);
...
{
    {
        ...
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &mock_stats_dumper);
    ...
    {
        {
            void* ptr = allocator.root()->Alloc(2048 - ExtraAllocSize(allocator), type_name);
            MockPartitionStatsDumper dumper;
            allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
    ...
    // This test checks for correct empty slot span list accounting.
    {
...

        {
            ...
            const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(PartitionPageSize());
            EXPECT_TRUE(stats);
            EXPECT_TRUE(stats->is_valid);
    ...
    // This test checks for correct direct mapped accounting.
    {
        ...
        size_t real_size_smaller = (size_smaller + SystemPageOffsetMask()) & SystemPageBaseMask();
...

        {
            ...
            const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(real_size_smaller);
            EXPECT_TRUE(stats);
            EXPECT_TRUE(stats->is_valid);
    ...
    // This test checks large-but-not-quite-direct allocations.
    {
...

        {
            ...
            const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(slot_size);
            ASSERT_TRUE(stats);
            EXPECT_TRUE(stats->is_valid);
            EXPECT_FALSE(stats->is_direct_map);
            EXPECT_EQ(slot_size, stats->bucket_slot_size);
            EXPECT_EQ(requested_size + ExtraAllocSize(allocator), stats->active_bytes);
            EXPECT_EQ(1u, stats->active_count);
            EXPECT_EQ(slot_size, stats->resident_bytes);
            EXPECT_EQ(0u, stats->decommittable_bytes);
            EXPECT_EQ(base::bits::AlignDown(slot_size - requested_size, SystemPageSize()), stats->discardable_bytes);
            EXPECT_EQ(1u, stats->num_full_slot_spans);
            EXPECT_EQ(0u, stats->num_active_slot_spans);
            EXPECT_EQ(0u, stats->num_empty_slot_spans);
            EXPECT_EQ(0u, stats->num_decommitted_slot_spans);
...
{
    char* ptr = static_cast<char*>(allocator.root()->Alloc(2048 - ExtraAllocSize(allocator), type_name));
    allocator.root()->Free(ptr);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
    ...
    ASSERT_GT(single_slot_size, MaxRegularSlotSpanSize());
    ASSERT_LE(single_slot_size, kMaxBucketed);
    char* big_ptr = static_cast<char*>(allocator.root()->Alloc(single_slot_size, type_name));
...
TEST_P(PartitionAllocTest, PreferActiveOverEmpty)
{
    ...
    const SlotSpan* slot_span1 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr1));
    const SlotSpan* slot_span2 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr3));
    const SlotSpan* slot_span3 = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr6));
...
TEST_P(PartitionAllocTest, PurgeDiscardableSecondPage)
{
    ...
    void* ptr1 = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    char* ptr2 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name));
    allocator.root()->Free(ptr2);
    const SlotSpan* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr1));
    EXPECT_EQ(2u, slot_span->num_unprovisioned_slots);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
        ...
        const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(SystemPageSize());
        EXPECT_TRUE(stats);
        EXPECT_TRUE(stats->is_valid);
        EXPECT_EQ(0u, stats->decommittable_bytes);
...
TEST_P(PartitionAllocTest, PurgeDiscardableFirstPage)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name));
    void* ptr2 = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    allocator.root()->Free(ptr1);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
...
TEST_P(PartitionAllocTest, PurgeDiscardableNonPageSizedAlloc)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(requested_size - ExtraAllocSize(allocator), type_name));
    void* ptr2 = allocator.root()->Alloc(requested_size - ExtraAllocSize(allocator), type_name);
    void* ptr3 = allocator.root()->Alloc(requested_size - ExtraAllocSize(allocator), type_name);
    void* ptr4 = allocator.root()->Alloc(requested_size - ExtraAllocSize(allocator), type_name);
    ...
    allocator.root()->Free(ptr2);
    {
        ...
        const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(requested_size);
        EXPECT_TRUE(stats);
        EXPECT_TRUE(stats->is_valid);
        EXPECT_EQ(0u, stats->decommittable_bytes);
#if PA_BUILDFLAG(IS_WIN)
...
TEST_P(PartitionAllocTest, PurgeDiscardableManyPages)
{
    ...
    const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(kFirstAllocPages * SystemPageSize());
    EXPECT_TRUE(stats);
    EXPECT_TRUE(stats->is_valid);
    EXPECT_EQ(0u, stats->decommittable_bytes);
...
TEST_P(PartitionAllocTest, PurgeDiscardableWithFreeListStraightening)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name));
    void* ptr2 = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    void* ptr3 = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    void* ptr4 = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    ptr1[0] = 'A';
    ...
    void* ptr1b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr1, ptr1b);
    void* ptr2b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr2, ptr2b);
    EXPECT_FALSE(slot_span->get_freelist_head()); // ptr4 was unprovisioned
    void* ptr4b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    ...
    ptr2b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr2, ptr2b);
    void* ptr3b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr3, ptr3b);
    ptr1b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr1, ptr1b);
    EXPECT_FALSE(slot_span->get_freelist_head());
    ...
    allocator.root()->Free(ptr3);
    allocator.root()->Free(ptr2);
    PartitionRoot::SetStraightenLargerSlotSpanFreeListsMode(StraightenLargerSlotSpanFreeListsMode::kAlways);
    allocator.root()->PurgeMemory(PurgeFlags::kDiscardUnusedSystemPages);
    ...
    ptr2b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr2, ptr2b);
    ptr3b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr3, ptr3b);
    EXPECT_FALSE(slot_span->get_freelist_head());
    ...
    allocator.root()->Free(ptr4);
    allocator.root()->Free(ptr2);
    PartitionRoot::SetStraightenLargerSlotSpanFreeListsMode(StraightenLargerSlotSpanFreeListsMode::kNever);
    allocator.root()->PurgeMemory(PurgeFlags::kDiscardUnusedSystemPages);
    ...
    PA_EXPECT_PTR_EQ(ptr1, ptr1b);
    EXPECT_FALSE(slot_span->get_freelist_head());
    ptr4b = allocator.root()->Alloc(SystemPageSize() - ExtraAllocSize(allocator), type_name);
    PA_EXPECT_PTR_EQ(ptr4, ptr4b);
    EXPECT_FALSE(slot_span->get_freelist_head());
...
{
    static constexpr size_t kAllZerosSentinel = std::numeric_limits<size_t>::max();
    for (size_t size : kTestSizes) {
        char* p = static_cast<char*>(allocator.root()->Alloc<AllocFlags::kZeroFill>(size));
        ...
        EXPECT_EQ(kAllZerosSentinel, non_zero_position) << "test allocation size: " << size;
...
{
    LightweightQuarantineBranch& branch = allocator.root()->GetSchedulerLoopQuarantineBranchForTesting();
    ...
    for (int i = 0; i < 10; ++i) {
        SCOPED_TRACE(i);
        AllocateRandomly<AllocFlags::kNone, FreeFlags::kSchedulerLoopQuarantine>(allocator.root(), 250);
    }
...
TEST_P(PartitionAllocTest, SchedulerLoopQuarantineDisabled)
{
    ...
    EXPECT_EQ(root->ObjectToSlotStart(ptr), UntagPtr(slot_span->get_freelist_head()));
...
TEST_P(PartitionAllocTest, ZapOnFree)
{
    ...
    ptr = TagPtr(ptr);
    EXPECT_NE('A', *static_cast<unsigned char*>(ptr));
    EXPECT_EQ(kFreedByte, *(static_cast<unsigned char*>(ptr) + 2 * sizeof(void*)));
...
TEST_P(PartitionAllocTest, Bug_897585)
{
    ...
    ptr = allocator.root()->Realloc<AllocFlags::kReturnNull>(ptr, kDesiredSize, nullptr);
    ASSERT_NE(nullptr, ptr);
...
TEST_P(PartitionAllocTest, OverrideHooks)
{
    ...
    PartitionAllocHooks::SetOverrideHooks(
        [](void** out, AllocFlags flags, size_t size, const char* type_name) -> bool {
    ...
    void* ptr = allocator.root()->Alloc<AllocFlags::kReturnNull>(kOverriddenSize, kOverriddenType);
...
TEST_P(PartitionAllocTest, Alignment)
{
    ...
    for (size_t size = 1; size <= PartitionPageSize(); size <<= 1) {
        ...
        for (int index = 0; index < 3; index++) {
            ...
            EXPECT_EQ(0u, allocator.root()->ObjectToSlotStart(ptr) % expected_alignment) << (index + 1) << "-th allocation of size=" << size;
        }
    }
...
TEST_P(PartitionAllocTest, FundamentalAlignment)
{
    ...
    for (size_t size = 0; size < SystemPageSize(); size++) {
        ...
        EXPECT_EQ(allocator.root()->AllocationCapacityFromSlotStart(slot_start) % fundamental_alignment, -ExtraAllocSize(allocator) % fundamental_alignment);
...
void VerifyAlignment(PartitionRoot* root, size_t size, size_t alignment)
{
    ...
    for (int index = 0; index < 3; index++) {
        ...
        ASSERT_TRUE(ptr);
        allocated_ptrs.push_back(ptr);
        EXPECT_EQ(0ull, UntagPtr(ptr) % alignment) << (index + 1) << "-th allocation of size=" << size << ", alignment=" << alignment;
    }
...
{
    size_t alloc_sizes[] = { 1, 10, 100, 1000, 10000, 60000, 70000, 130000, 500000, 900000, kMaxBucketed + 1, 2 * kMaxBucketed,
        kSuperPageSize - 2 * PartitionPageSize(), 4 * kMaxBucketed };
    for (size_t alloc_size : alloc_sizes) {
        for (size_t alignment = 1; alignment <= kMaxSupportedAlignment; alignment <<= 1) {
...
TEST_P(PartitionAllocTest, OptimizedGetSlotNumber)
{
    for (size_t i = 0; i < kNumBuckets; ++i) {
        ...
        for (size_t slot = 0, offset = 0; slot < bucket.get_slots_per_span(); ++slot, offset += bucket.slot_size) {
...
TEST_P(PartitionAllocTest, GetUsableSize)
{
    ...
    for (size_t size = 1; size <= kMinDirectMappedDownsize; size += delta) {
        ...
        size_t usable_size_with_hack = PartitionRoot::GetUsableSizeWithMac11MallocSizeHack(ptr);
#if PA_CONFIG(MAYBE_ENABLE_MAC11_MALLOC_SIZE_HACK)
...
TEST_P(PartitionAllocTest, GetUsableSizeWithMac11MallocSizeHack)
{
    ...
    size_t usable_size = PartitionRoot::GetUsableSize(ptr);
    size_t usable_size_with_hack = PartitionRoot::GetUsableSizeWithMac11MallocSizeHack(ptr);
    EXPECT_EQ(usable_size, allocator.root()->settings.mac11_malloc_size_hack_usable_size_);
...
TEST_P(PartitionAllocTest, Bookkeeping)
{
    ...
    expected_committed_size += kUseLazyCommit ? SystemPageSize() : PartitionPageSize();
    expected_max_committed_size = std::max(expected_max_committed_size, expected_committed_size);
    expected_max_allocated_size = std::max(expected_max_allocated_size, static_cast<size_t>(2048));
    EXPECT_EQ(expected_committed_size, root.total_size_of_committed_pages);
    EXPECT_EQ(expected_max_committed_size, root.max_size_of_committed_pages);
    EXPECT_EQ(expected_max_allocated_size, root.get_max_size_of_allocated_bytes());
    EXPECT_EQ(expected_super_pages_size, root.total_size_of_super_pages);
    ...
    expected_max_allocated_size = std::max(expected_max_allocated_size, expected_total_allocated_size);
    EXPECT_EQ(expected_committed_size, root.total_size_of_committed_pages);
    EXPECT_EQ(expected_max_committed_size, root.max_size_of_committed_pages);
    EXPECT_EQ(expected_total_allocated_size, root.get_total_size_of_allocated_bytes());
    EXPECT_EQ(expected_max_allocated_size, root.get_max_size_of_allocated_bytes());
    ...
    for (size_t huge_size : huge_sizes) {
        ...
        for (size_t alignment : alignments) {
            ...
            size_t aligned_size = partition_alloc::internal::base::bits::AlignUp(huge_size, SystemPageSize());
            ptr = root.AllocInternalForTesting(huge_size - ExtraAllocSize(allocator), alignment, type_name);
            ...
            size_t surrounding_pages_size = PartitionRoot::GetDirectMapMetadataAndGuardPagesSize() + alignment - PartitionPageSize();
            size_t expected_direct_map_size
                = partition_alloc::internal::base::bits::AlignUp(aligned_size + surrounding_pages_size, DirectMapAllocationGranularity());
            EXPECT_EQ(expected_committed_size, root.total_size_of_committed_pages);
            EXPECT_EQ(expected_max_committed_size, root.max_size_of_committed_pages);
            EXPECT_EQ(expected_total_allocated_size, root.get_total_size_of_allocated_bytes());
            EXPECT_EQ(expected_max_allocated_size, root.get_max_size_of_allocated_bytes());
...
TEST_P(PartitionAllocTest, RefCountBasic)
{
    ...
    *ptr1 = kCookie;
    ...
    auto* in_slot_metadata = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr1);
...
void PartitionAllocTest::RunRefCountReallocSubtest(size_t orig_size, size_t new_size)
{
    void* ptr1 = allocator.root()->Alloc(orig_size, type_name);
    EXPECT_TRUE(ptr1);

    auto* in_slot_metadata1 = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr1);
    ...
    auto* in_slot_metadata2 = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr2);
...
public:
    void SetUp() override
    {
        ...
        partition_alloc::SetUnretainedDanglingRawPtrDetectedFn(&UnretainedDanglingRawPtrTest::DanglingRawPtrDetected);
        old_unretained_dangling_ptr_enabled_ = partition_alloc::SetUnretainedDanglingRawPtrCheckEnabled(true);
    }
    void TearDown() override
    {
        partition_alloc::SetUnretainedDanglingRawPtrDetectedFn(old_detected_fn_);
        partition_alloc::SetUnretainedDanglingRawPtrCheckEnabled(old_unretained_dangling_ptr_enabled_);
...
INSTANTIATE_TEST_SUITE_P(AlternateTestParams, UnretainedDanglingRawPtrTest, testing::ValuesIn(GetPartitionAllocTestParams()));
...
TEST_P(UnretainedDanglingRawPtrTest, UnretainedDanglingPtrNoReport)
{
    ...
    void* ptr = allocator.root()->Alloc(kTestAllocSize, type_name);
    EXPECT_TRUE(ptr);
    auto* in_slot_metadata = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr);
    in_slot_metadata->Acquire();
...
TEST_P(PartitionAllocTest, BackupRefPtrGuardRegion)
{
    ...
    memset(&requested_address, internal::kQuarantinedByte, sizeof(requested_address));
    requested_address = RoundDownToPageAllocationGranularity(requested_address);

    uintptr_t allocated_address = AllocPages(
        requested_address, alignment, alignment, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kReadWrite), PageTag::kPartitionAlloc);
...
TEST_P(PartitionAllocTest, DanglingPtr)
{
    ...
    uint64_t* ptr = static_cast<uint64_t*>(allocator.root()->Alloc(64 - ExtraAllocSize(allocator), type_name));
    auto* in_slot_metadata = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr);
...
TEST_P(PartitionAllocTest, DanglingMixedReleaseDanglingPtrFirst)
{
    ...
    void* ptr = allocator.root()->Alloc(64 - ExtraAllocSize(allocator), type_name);
    auto* in_slot_metadata = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr);
    // Acquire a raw_ptr<T, DisableDanglingPtrDetection> and a raw_ptr<>.
    in_slot_metadata->AcquireFromUnprotectedPtr();
    in_slot_metadata->Acquire();
...
TEST_P(PartitionAllocTest, DanglingPtrReleaseBeforeSchedulerLoopQuarantineExit)
{
    if (!UseBRPPool()) {
        return;
    }
    ...
    // Allocate memory, and reference it from 3 raw_ptr.
    uint64_t* ptr = static_cast<uint64_t*>(allocator.root()->Alloc(64 - ExtraAllocSize(allocator), type_name));
    auto* ref_count = allocator.root()->InSlotMetadataPointerFromObjectForTesting(ptr);
...
TEST_P(PartitionAllocTest, ReservationOffset)
{
    ...
    EXPECT_EQ(kOffsetTagNotAllocated, *ReservationOffsetPointer(address + kSuperPageSize));
    EXPECT_EQ(kOffsetTagNotAllocated, *ReservationOffsetPointer(address + kSuperPageSize * 2));
    EXPECT_EQ(kOffsetTagNotAllocated, *ReservationOffsetPointer(address + kSuperPageSize * 3));
    EXPECT_EQ(kOffsetTagNotAllocated, *ReservationOffsetPointer(address + kSuperPageSize * 4));
    EXPECT_EQ(kOffsetTagNotAllocated, *ReservationOffsetPointer(address + kSuperPageSize * 5));
}
...
TEST_P(PartitionAllocTest, CheckReservationType)
{
    ...
    EXPECT_TRUE(IsManagedByNormalBucketsForTesting(address_to_check, allocator.root()));
    EXPECT_FALSE(IsManagedByDirectMapForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMapForTesting(address_to_check, allocator.root()));
    address_to_check = address + kTestAllocSize - 1;
    EXPECT_FALSE(IsReservationStart(address_to_check));
    EXPECT_TRUE(IsManagedByNormalBucketsForTesting(address_to_check, allocator.root()));
    EXPECT_FALSE(IsManagedByDirectMapForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMapForTesting(address_to_check, allocator.root()));
    address_to_check = partition_alloc::internal::base::bits::AlignDown(address, kSuperPageSize);
    ...
    EXPECT_FALSE(IsReservationStart(address_to_check));
    EXPECT_FALSE(IsManagedByNormalBucketsForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByDirectMapForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMapForTesting(address_to_check, allocator.root()));
    address_to_check = partition_alloc::internal::base::bits::AlignUp(address, kSuperPageSize);
    EXPECT_FALSE(IsReservationStart(address_to_check));
    EXPECT_FALSE(IsManagedByNormalBucketsForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByDirectMapForTesting(address_to_check, allocator.root()));
    EXPECT_TRUE(IsManagedByNormalBucketsOrDirectMapForTesting(address_to_check, allocator.root()));
...
#if PA_BUILDFLAG(DCHECKS_ARE_ON) && (!defined(OFFICIAL_BUILD) || !defined(NDEBUG))
    ...
    EXPECT_FALSE(IsManagedByNormalBucketsForTesting(address_to_check, allocator.root()));
    EXPECT_FALSE(IsManagedByDirectMapForTesting(address_to_check, allocator.root()));
    EXPECT_FALSE(IsManagedByNormalBucketsOrDirectMapForTesting(address_to_check, allocator.root()));
}
...
TEST_P(PartitionAllocTest, CrossPartitionRootRealloc)
{
    ...
    allocator.root()->PurgeMemory(PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages);
    std::unique_ptr<PartitionRoot> new_root = CreateCustomTestRoot(GetCommonPartitionOptions(), PartitionTestOptions { .set_bucket_distribution = true });

    // Realloc from |allocator.root()| into |new_root|.
    void* ptr2 = new_root->Realloc<AllocFlags::kReturnNull>(ptr, test_size + 1024, nullptr);
    EXPECT_TRUE(ptr2);
    PA_EXPECT_PTR_NE(ptr, ptr2);
}
...
TEST_P(PartitionAllocTest, FastPathOrReturnNull)
{
    ...
    EXPECT_FALSE(allocator.root()->Alloc<AllocFlags::kFastPathOrReturnNull>(allocation_size, ""));
    void* ptr = allocator.root()->Alloc(allocation_size);
    ASSERT_TRUE(ptr);

    // Next one is, since the partition page has been activated.
    void* ptr2 = allocator.root()->Alloc<AllocFlags::kFastPathOrReturnNull>(allocation_size, "");
    EXPECT_TRUE(ptr2);

    // First allocation of a different bucket is slow.
    EXPECT_FALSE(allocator.root()->Alloc<AllocFlags::kFastPathOrReturnNull>(2 * allocation_size, ""));

    size_t allocated_size = 2 * allocation_size;
    std::vector<void*> ptrs;
    while (void* new_ptr = allocator.root()->Alloc<AllocFlags::kFastPathOrReturnNull>(allocation_size, "")) {
        ptrs.push_back(new_ptr);
        allocated_size += allocation_size;
    }
    EXPECT_LE(allocated_size, PartitionPageSize() * kMaxPartitionPagesPerRegularSlotSpan);
...
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_USE_DEATH_TESTS() && !PA_BUILDFLAG(IS_CASTOS)
...
class ThreadDelegateForPreforkHandler : public base::PlatformThreadForTesting::Delegate {
public:
    ThreadDelegateForPreforkHandler(std::atomic<bool>& please_stop, std::atomic<int>& started_threads, const int alloc_size)
...
TEST_P(PartitionAllocTest, DISABLED_PreforkHandler)
{
    ...
    ThreadDelegateForPreforkHandler delegate(please_stop, started_threads, kAllocSize);
...
TEST_P(PartitionAllocTest, GetIndex)
{
    ...
    for (size_t size = (1 << (kMinBucketedOrder - 1)); size < kMaxBucketed; size <<= 1) {
        size_t index = BucketIndexLookup::GetIndex(size);
...
TEST_P(PartitionAllocTest, ConfigurablePool)
{
    ...
    for (size_t pool_size = max_pool_size; pool_size >= min_pool_size; pool_size /= 2) {
        PA_DCHECK(base::bits::HasSingleBit(pool_size));
        EXPECT_FALSE(IsConfigurablePoolAvailable());
        uintptr_t pool_base
            = AllocPages(pool_size, pool_size, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc);
        ...
        std::unique_ptr<PartitionRoot> root
            = CreateCustomTestRoot(opts, PartitionTestOptions { .uncap_empty_slot_span_memory = true, .set_bucket_distribution = true });
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            EXPECT_TRUE(allocation_base >= pool_base && allocation_base < pool_base + pool_size);
        }
...
TEST_P(PartitionAllocTest, EmptySlotSpanSizeIsCapped)
{
    ...
    // slot span size cap.
    std::unique_ptr<PartitionRoot> root = CreateCustomTestRoot(GetCommonPartitionOptions(), PartitionTestOptions { .set_bucket_distribution = true });
    ...
    ASSERT_GE(root->total_size_of_committed_pages.load(std::memory_order_relaxed), size * count);
    ...
    ASSERT_LT(MaxRegularSlotSpanSize() * 2, ((count * size) >> root->max_empty_slot_spans_dirty_bytes_shift));
    ...
    EXPECT_LT(PA_TS_UNCHECKED_READ(root->empty_slot_spans_dirty_bytes), single_slot_count * single_slot_size);
...
{
    std::unique_ptr<PartitionRoot> root
        = CreateCustomTestRoot(GetCommonPartitionOptions(), PartitionTestOptions { .uncap_empty_slot_span_memory = true, .set_bucket_distribution = true });

    std::vector<void*> single_slot_allocated_memory;
    constexpr size_t single_slot_count = kDefaultEmptySlotSpanRingSize + 10;
    const size_t single_slot_size = MaxRegularSlotSpanSize() + 1;
    const size_t bucket_size = root->buckets[SizeToIndex(single_slot_size)].slot_size;
    ...
    EXPECT_EQ(PA_TS_UNCHECKED_READ(root->empty_slot_spans_dirty_bytes), kDefaultEmptySlotSpanRingSize * bucket_size);
    ...
    EXPECT_EQ(PA_TS_UNCHECKED_READ(root->empty_slot_spans_dirty_bytes), single_slot_large_count * bucket_size);
    ...
    EXPECT_EQ(PA_TS_UNCHECKED_READ(root->empty_slot_spans_dirty_bytes), kMaxEmptySlotSpanRingSize * bucket_size);
#endif
}
...
TEST_P(PartitionAllocTest, SortFreelist)
{
    ...
    size_t bucket_index = SizeToIndex(allocation_size + ExtraAllocSize(allocator));
...
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_BUILDFLAG(IS_LINUX) && PA_BUILDFLAG(PA_ARCH_CPU_64_BITS)
...
public:
    ...
    void SetUp()
    {
        ...
        rv = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_CPU, 1, &device_id, nullptr);
        ...
        static_assert(std::size(sources) == std::size(source_lengths), "arrays must be parallel");

        program_ = clCreateProgramWithSource(context_, std::size(sources), sources, source_lengths, &rv);
        ASSERT_EQ(rv, CL_SUCCESS) << "clCreateProgramWithSource";

        rv = clBuildProgram(program_, 1, &device_id, "-cl-opt-disable", nullptr, nullptr);
...
TEST_P(PartitionAllocTest, SmallSlotSpanWaste)
{
    for (PartitionRoot::Bucket& bucket : allocator.root()->buckets) {
        ...
        size_t small_system_page_count = partition_alloc::internal::ComputeSystemPagesPerSlotSpan(bucket.slot_size, true);
        size_t small_waste = (small_system_page_count * SystemPageSize()) % slot_size;
...
TEST_P(PartitionAllocTest, SortActiveSlotSpans)
{
    auto run_test = [this](size_t count) {
        ...
        // SuperPage.
        const size_t num_slotspans_per_superpage = SystemPageSize() / sizeof(SlotSpan);
        ...
        while (count > 0) {
            ...
            uintptr_t super_page = partition_alloc::internal::base::bits::AlignUp(reinterpret_cast<uintptr_t>(ptr), kSuperPageSize);
            ...
            if (PartitionAddressSpace::IsShadowMetadataEnabled(allocator.root()->ChoosePool())) {
            ...
            for (size_t j = 0; j < loop_count; ++j) {
                ...
                SlotSpanMetadata<MetadataKind::kWritable>* writable_slot_span
                    = new (reinterpret_cast<void*>(writable_metadata + offset)) SlotSpanMetadata<MetadataKind::kWritable>(&bucket);

                SlotSpan* slot_span = reinterpret_cast<SlotSpan*>(metadata + offset);
                slot_spans.push_back(slot_span);

                writable_slot_span->num_unprovisioned_slots = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() / 2);
                writable_slot_span->num_allocated_slots
                    = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() - slot_span->num_unprovisioned_slots);
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            slot_span.ToWritable(nullptr)->num_unprovisioned_slots = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() / 2);
            slot_span.ToWritable(nullptr)->num_allocated_slots
                = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() - slot_span.num_unprovisioned_slots);
            slot_span.ToWritable(nullptr)->next_slot_span = bucket.active_slot_spans_head;
            bucket.active_slot_spans_head = &slot_span;
        }
        ...
        for (auto* slot_span = bucket.active_slot_spans_head; slot_span; slot_span = slot_span->next_slot_span) {
        ...
        size_t sorted_spans_count = std::min(PartitionBucket::kMaxSlotSpansToSort, count);
        EXPECT_TRUE(std::is_sorted(sorted_slot_spans.begin(), sorted_slot_spans.begin() + sorted_spans_count, partition_alloc::internal::CompareSlotSpans));

        // Slot spans with no freelist entries are at the end of the sorted run.
        auto has_empty_freelist = [](const SlotSpan* a) { return a->GetFreelistLength() == 0; };
        auto it = std::find_if(sorted_slot_spans.begin(), sorted_slot_spans.begin() + sorted_spans_count, has_empty_freelist);
        if (it != sorted_slot_spans.end()) {
            EXPECT_TRUE(std::all_of(it, sorted_slot_spans.begin() + sorted_spans_count, has_empty_freelist));
        }
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
...
TEST_P(PartitionAllocTest, GlobalEmptySlotSpanRingIndexResets)
{
    ...
    allocator.root()->SetGlobalEmptySlotSpanRingIndexForTesting(internal::kMaxEmptySlotSpanRingSize - 1);
    ...
    // not crash.
    EXPECT_EQ(0u, PA_TS_UNCHECKED_READ(allocator.root()->empty_slot_spans_dirty_bytes));
}
#endif
...
TEST_P(PartitionAllocTest, FastReclaim)
{
    ...
    constexpr int kFlags = PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages;
    ...
    EXPECT_EQ(PA_TS_UNCHECKED_READ(allocator.root()->purge_next_bucket_index), 0u);

    allocator.root()->PurgeMemory(kFlags | PurgeFlags::kLimitDuration);
    // Ran out of time.
    unsigned int next_bucket = PA_TS_UNCHECKED_READ(allocator.root()->purge_next_bucket_index);
    EXPECT_NE(next_bucket, 0u);
    allocator.root()->PurgeMemory(kFlags | PurgeFlags::kLimitDuration);
    // Make some progress, but not through all buckets yet.
    EXPECT_GT(PA_TS_UNCHECKED_READ(allocator.root()->purge_next_bucket_index), next_bucket);

    allocator.root()->PurgeMemory(kFlags | PurgeFlags::kLimitDuration);
    // Ran out of time.
    EXPECT_NE(PA_TS_UNCHECKED_READ(allocator.root()->purge_next_bucket_index), 0u);
