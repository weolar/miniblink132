...
PA_ALWAYS_INLINE bool SpinningMutex::Try()
{
    ...
    return (state_.load(std::memory_order_relaxed) == expected)
        && state_.compare_exchange_weak(expected, kLockedUncontended, std::memory_order_acquire, std::memory_order_relaxed);
}

PA_ALWAYS_INLINE void SpinningMutex::Release()
{
    if (state_.exchange(kUnlocked, std::memory_order_release) == kLockedContended) [[unlikely]] {
