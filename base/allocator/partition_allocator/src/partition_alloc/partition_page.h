...
template <MetadataKind kind> struct SlotSpanMetadata;
...
#pragma pack(push, 1)
template <MetadataKind kind> struct SlotSpanMetadataBase {
protected:
...
public:
    ...
    MaybeConstT<kind, SlotSpanMetadata<MetadataKind::kReadOnly>*> next_slot_span = nullptr;
    ...
    MaybeConstT<kind, uint32_t> num_allocated_slots : kMaxSlotsPerSlotSpanBits = 0u;
    MaybeConstT<kind, uint32_t> num_unprovisioned_slots : kMaxSlotsPerSlotSpanBits = 0u;
...
protected:
    ...
    MaybeConstT<kind, uint16_t> empty_cache_index_ : internal::base::bits::BitWidth(kMaxEmptySlotSpanRingSize - 1) = 0u;
...
public:
    ...
    {
        size_t num_provisioned_slots = bucket->get_slots_per_span() - num_unprovisioned_slots;
...
template <> struct SlotSpanMetadata<MetadataKind::kReadOnly> : public SlotSpanMetadataBase<MetadataKind::kReadOnly> {
    ...
    // sanitize this input.
    PA_ALWAYS_INLINE static uintptr_t ToSlotSpanStart(const SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span);
    PA_ALWAYS_INLINE static SlotSpanMetadata<MetadataKind::kReadOnly>* FromAddr(uintptr_t address);
    PA_ALWAYS_INLINE static SlotSpanMetadata<MetadataKind::kReadOnly>* FromSlotStart(uintptr_t slot_start);
    PA_ALWAYS_INLINE static SlotSpanMetadata<MetadataKind::kReadOnly>* FromObject(const void* object);
    PA_ALWAYS_INLINE static SlotSpanMetadata<MetadataKind::kReadOnly>* FromObjectInnerAddr(uintptr_t address);
    PA_ALWAYS_INLINE static SlotSpanMetadata<MetadataKind::kReadOnly>* FromObjectInnerPtr(const void* ptr);

    PA_ALWAYS_INLINE PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* ToSuperPageExtent() const;
    ...
    static const SlotSpanMetadata<MetadataKind::kReadOnly>* get_sentinel_slot_span();
    ...
    static SlotSpanMetadata<MetadataKind::kReadOnly>* get_sentinel_slot_span_non_const();
    ...
    PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kWritable>* ToWritable(const PartitionRoot* root)
    {
...
private:
    ...
    template <typename T> SlotSpanMetadata<MetadataKind::kWritable>* ToWritableInternal([[maybe_unused]] T* root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        // Must not make writable slot_span from sentinel slot_span.
        PA_DCHECK(this != get_sentinel_slot_span());
        return reinterpret_cast<SlotSpanMetadata<MetadataKind::kWritable>*>(reinterpret_cast<intptr_t>(this) + root->ShadowPoolOffset());
#else
    ...
    // For the sentinel.
    inline constexpr SlotSpanMetadata<MetadataKind::kReadOnly>() noexcept = default;
};

template <> struct SlotSpanMetadata<MetadataKind::kWritable> : public SlotSpanMetadataBase<MetadataKind::kWritable> {
    PA_COMPONENT_EXPORT(PARTITION_ALLOC)
    ...
    PA_ALWAYS_INLINE PartitionSuperPageExtentEntry<MetadataKind::kWritable>* ToSuperPageExtent();

    // Note the matching Alloc() functions are in PartitionPage.
    PA_NOINLINE PA_COMPONENT_EXPORT(PARTITION_ALLOC) void FreeSlowPath(size_t number_of_freed, PartitionRoot* root);
    // Note the matching Alloc() functions are in PartitionPage.
    PA_NOINLINE PA_COMPONENT_EXPORT(PARTITION_ALLOC) void FreeSlowPath(size_t number_of_freed);
    PA_ALWAYS_INLINE PartitionFreelistEntry* PopForAlloc(size_t size, const PartitionFreelistDispatcher* freelist_dispatcher);
    PA_ALWAYS_INLINE void Free(uintptr_t ptr, PartitionRoot* root, const PartitionFreelistDispatcher* freelist_dispatcher)
    ...
    PA_ALWAYS_INLINE void AppendFreeList(PartitionFreelistEntry* head, PartitionFreelistEntry* tail, size_t number_of_freed, PartitionRoot* root,
        const PartitionFreelistDispatcher* freelist_dispatcher) PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root));
    ...
    PA_ALWAYS_INLINE void SetFreelistHead(PartitionFreelistEntry* new_head, PartitionRoot* root);
    ...
    PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* ToReadOnly(const PartitionRoot* root)
    {
...
private:
    ...
    template <typename T> PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* ToReadOnlyInternal([[maybe_unused]] const T* root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        return reinterpret_cast<SlotSpanMetadata<MetadataKind::kReadOnly>*>(reinterpret_cast<intptr_t>(this) - root->ShadowPoolOffset());
#else
...
#pragma pack(pop)
static_assert(sizeof(SlotSpanMetadata<MetadataKind::kReadOnly>) <= kPageMetadataSize,
    "SlotSpanMetadata<MetadataKind::kReadOnly> must fit into a Page "
    "Metadata slot.");

// Metadata of a non-first partition page in a slot span.
template <MetadataKind kind> struct SubsequentPageMetadata {
...
#pragma pack(push, 1)
template <MetadataKind kind> struct PartitionPageMetadataBase {
    union {
        ...
        MaybeConstT<kind, char> optional_padding[kPageMetadataSize - sizeof(uint8_t) - sizeof(bool)];
    };
    ...
    static constexpr uint16_t kMaxSlotSpanMetadataOffset = (1 << kMaxSlotSpanMetadataBits) - 1;
    MaybeConstT<kind, uint8_t> slot_span_metadata_offset : kMaxSlotSpanMetadataBits;
...
template <MetadataKind kind> struct PartitionPageMetadata;

template <> struct PartitionPageMetadata<MetadataKind::kReadOnly> : public PartitionPageMetadataBase<MetadataKind::kReadOnly> {
    PA_ALWAYS_INLINE static PartitionPageMetadata<MetadataKind::kReadOnly>* FromAddr(uintptr_t address);

    PA_ALWAYS_INLINE PartitionPageMetadata<MetadataKind::kWritable>* ToWritable(PartitionRoot* root)
    {
        return ToWritableInternal(root);
    }
    ...
    // ToWritable() uses it.
    template <typename T> PartitionPageMetadata<MetadataKind::kWritable>* ToWritableInternal([[maybe_unused]] T* root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        return reinterpret_cast<PartitionPageMetadata<MetadataKind::kWritable>*>(reinterpret_cast<intptr_t>(this) + root->ShadowPoolOffset());
#else
        return reinterpret_cast<PartitionPageMetadata<MetadataKind::kWritable>*>(this);
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)
    }
};

template <> struct PartitionPageMetadata<MetadataKind::kWritable> : public PartitionPageMetadataBase<MetadataKind::kWritable> {
    PA_ALWAYS_INLINE PartitionPageMetadata<MetadataKind::kReadOnly>* ToReadOnly(PartitionRoot* root)
    {
        return ToReadOnlyInternal(root);
    }
    ...
    // ToReadOnly() uses it.
    template <typename T> PartitionPageMetadata<MetadataKind::kReadOnly>* ToReadOnlyInternal([[maybe_unused]] T* root)
    {
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        return reinterpret_cast<PartitionPageMetadata<MetadataKind::kReadOnly>*>(reinterpret_cast<intptr_t>(this) - root->ShadowPoolOffset());
#else
        return reinterpret_cast<PartitionPageMetadata<MetadataKind::kReadOnly>*>(this);
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)
    }
};
#pragma pack(pop)

static_assert(sizeof(PartitionPageMetadata<MetadataKind::kWritable>) == kPageMetadataSize, "PartitionPage must be able to fit in a metadata slot");
static_assert(sizeof(PartitionPageMetadata<MetadataKind::kReadOnly>) == sizeof(PartitionPageMetadata<MetadataKind::kWritable>),
...
#endif
static_assert(offsetof(PartitionPageMetadata<MetadataKind::kReadOnly>, slot_span_metadata) == 0,
    "slot_span_metadata must be placed at the beginning of "
    "PartitionPageMetadata<MetadataKind::kReadOnly>.");
static_assert(offsetof(PartitionPageMetadata<MetadataKind::kReadOnly>, subsequent_page_metadata) == 0,
    "subsequent_page_metadata must be placed at the beginning of "
    "PartitionPageMetadata<MetadataKind::kReadOnly>.");
static_assert(offsetof(PartitionPageMetadata<MetadataKind::kWritable>, slot_span_metadata) == 0,
    "slot_span_metadata must be placed at the beginning of "
    "PartitionPageMetadata<MetadataKind::kWritable>.");
static_assert(offsetof(PartitionPageMetadata<MetadataKind::kWritable>, subsequent_page_metadata) == 0,
    "subsequent_page_metadata must be placed at the beginning of "
    "PartitionPageMetadata<MetadataKind::kWritable>.");
#if defined(__clang__) || defined(__GNUC__)
...
PA_ALWAYS_INLINE PartitionPageMetadata<MetadataKind::kReadOnly>* PartitionSuperPageToMetadataArea(uintptr_t super_page)
{
    ...
    // super page.
    return reinterpret_cast<PartitionPageMetadata<MetadataKind::kReadOnly>*>(super_page + SystemPageSize());
}

PA_ALWAYS_INLINE const SubsequentPageMetadata<MetadataKind::kReadOnly>* GetSubsequentPageMetadata(
    const PartitionPageMetadata<MetadataKind::kReadOnly>* page_metadata)
{
    return &(page_metadata + 1)->subsequent_page_metadata;
}

PA_ALWAYS_INLINE SubsequentPageMetadata<MetadataKind::kWritable>* GetSubsequentPageMetadata(PartitionPageMetadata<MetadataKind::kWritable>* page_metadata)
{
    return &(page_metadata + 1)->subsequent_page_metadata;
}

PA_ALWAYS_INLINE PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* PartitionSuperPageToExtent(uintptr_t super_page)
{
    // The very first entry of the metadata is the super page extent entry.
    return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>*>(PartitionSuperPageToMetadataArea(super_page));
}

PA_ALWAYS_INLINE PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR size_t ReservedStateBitmapSize()
{
    return 0ull;
}

PA_ALWAYS_INLINE uintptr_t SuperPagePayloadStartOffset(bool is_managed_by_normal_buckets)
{
    return PartitionPageSize() + (is_managed_by_normal_buckets ? ReservedFreeSlotBitmapSize() : 0);
}

PA_ALWAYS_INLINE uintptr_t SuperPagePayloadBegin(uintptr_t super_page)
{
    PA_DCHECK(!(super_page % kSuperPageAlignment));
    return super_page + SuperPagePayloadStartOffset(IsManagedByNormalBuckets(super_page));
}
...
PA_ALWAYS_INLINE PartitionSuperPageExtentEntry<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::ToSuperPageExtent() const
{
    uintptr_t super_page = reinterpret_cast<uintptr_t>(this) & kSuperPageBaseMask;
    return PartitionSuperPageToExtent(super_page);
}

PA_ALWAYS_INLINE PartitionSuperPageExtentEntry<MetadataKind::kWritable>* SlotSpanMetadata<MetadataKind::kWritable>::ToSuperPageExtent()
{
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
    uintptr_t super_page_extent_entry = reinterpret_cast<uintptr_t>(this) & SystemPageBaseMask();
    return reinterpret_cast<PartitionSuperPageExtentEntry<MetadataKind::kWritable>*>(super_page_extent_entry);
#else
    // Must be no-op.
...
PA_ALWAYS_INLINE PartitionPageMetadata<MetadataKind::kReadOnly>* PartitionPageMetadata<MetadataKind::kReadOnly>::FromAddr(uintptr_t address)
{
    uintptr_t super_page = address & kSuperPageBaseMask;
    ...
    uintptr_t partition_page_index = (address & kSuperPageOffsetMask) >> PartitionPageShift();
...
PA_ALWAYS_INLINE uintptr_t SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(const SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span)
{
    ...
    PA_DCHECK(super_page_offset < SystemPageSize() + (NumPartitionPagesPerSuperPage() * kPageMetadataSize));
    uintptr_t partition_page_index = (super_page_offset - SystemPageSize()) >> kPageMetadataShift;
    // Index 0 is invalid because it is the super page extent metadata and the
    // last index is invalid because the whole PartitionPage is set as guard
    // pages.
    PA_DCHECK(partition_page_index);
...
// partition page.
PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::FromAddr(uintptr_t address)
{
    auto* page_metadata = PartitionPageMetadata<MetadataKind::kReadOnly>::FromAddr(address);
...
PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::FromSlotStart(uintptr_t slot_start)
{
...
//
// This works on direct maps too.
PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::FromObject(const void* object)
{
...
// CAUTION! For direct-mapped allocation, |address| has to be within the first
// partition page.
PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::FromObjectInnerAddr(uintptr_t address)
{
    auto* slot_span = FromAddr(address);
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
    // Checks that the address is within the expected object boundaries.
    uintptr_t slot_span_start = ToSlotSpanStart(slot_span);
    uintptr_t shift_from_slot_start = (address - slot_span_start) % slot_span->bucket->slot_size;
    DCheckIsValidShiftFromSlotStart(slot_span, shift_from_slot_start);
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON)
    return slot_span;
}

PA_ALWAYS_INLINE SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::FromObjectInnerPtr(const void* ptr)
{
    return FromObjectInnerAddr(ObjectInnerPtr2Addr(ptr));
}

PA_ALWAYS_INLINE void SlotSpanMetadata<MetadataKind::kWritable>::SetRawSize(size_t raw_size)
{
    PA_DCHECK(CanStoreRawSize());
    auto* subsequent_page_metadata = GetSubsequentPageMetadata(reinterpret_cast<PartitionPageMetadata<MetadataKind::kWritable>*>(this));
    subsequent_page_metadata->raw_size = raw_size;
}

PA_ALWAYS_INLINE size_t SlotSpanMetadata<MetadataKind::kReadOnly>::GetRawSize() const
{
    PA_DCHECK(CanStoreRawSize());
    const auto* subsequent_page_metadata = GetSubsequentPageMetadata(reinterpret_cast<const PartitionPageMetadata<MetadataKind::kReadOnly>*>(this));
    return subsequent_page_metadata->raw_size;
}

PA_ALWAYS_INLINE void SlotSpanMetadata<MetadataKind::kWritable>::SetFreelistHead(PartitionFreelistEntry* new_head, [[maybe_unused]] PartitionRoot* root)
{
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
    // |this| is in the metadata region, hence isn't MTE-tagged. Untag |new_head|
    // as well.
    uintptr_t new_head_untagged = UntagPtr(new_head);
    PA_DCHECK(!new_head || (reinterpret_cast<uintptr_t>(ToReadOnly(root)) & kSuperPageBaseMask) == (new_head_untagged & kSuperPageBaseMask));
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON)
...
PA_ALWAYS_INLINE PartitionFreelistEntry* SlotSpanMetadata<MetadataKind::kWritable>::PopForAlloc(
    size_t size, const PartitionFreelistDispatcher* freelist_dispatcher)
{
    ...
    PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root))
{
    ...
    auto* entry = static_cast<PartitionFreelistEntry*>(SlotStartAddr2Ptr(slot_start));
    // Catches an immediate double free.
    PA_CHECK(entry != freelist_head);

    // Look for double free one level deeper in debug.
    PA_DCHECK(!freelist_head || entry != freelist_dispatcher->GetNext(freelist_head, bucket->slot_size));
...
PA_ALWAYS_INLINE void SlotSpanMetadata<MetadataKind::kWritable>::AppendFreeList(PartitionFreelistEntry* head, PartitionFreelistEntry* tail,
    size_t number_of_freed, PartitionRoot* root, const PartitionFreelistDispatcher* freelist_dispatcher) PA_EXCLUSIVE_LOCKS_REQUIRED(PartitionRootLock(root))
{
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
    DCheckRootLockIsAcquired(root);
    ...
    }
    {
        ...
        for (auto* entry = head; entry; entry = freelist_dispatcher->GetNext(entry, bucket->slot_size), ++number_of_entries) {
            uintptr_t untagged_entry = UntagPtr(entry);
            // Check that all entries belong to this slot span.
            PA_DCHECK(SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(ToReadOnly(root)) <= untagged_entry);
            PA_DCHECK(untagged_entry < SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(ToReadOnly(root)) + bucket->get_bytes_per_span());
        }
...
PA_ALWAYS_INLINE bool SlotSpanMetadata<MetadataKind::kReadOnly>::is_active() const
{
    PA_DCHECK(this != get_sentinel_slot_span());
    bool ret = (num_allocated_slots > 0 && (freelist_head || num_unprovisioned_slots));
    if (ret) {
        PA_DCHECK(!marked_full);
...
PA_ALWAYS_INLINE bool SlotSpanMetadata<MetadataKind::kReadOnly>::is_full() const
{
    PA_DCHECK(this != get_sentinel_slot_span());
...
PA_ALWAYS_INLINE bool SlotSpanMetadata<MetadataKind::kReadOnly>::is_empty() const
{
    PA_DCHECK(this != get_sentinel_slot_span());
    return is_empty_internal();
}

PA_ALWAYS_INLINE bool SlotSpanMetadata<MetadataKind::kReadOnly>::is_decommitted() const
{
    PA_DCHECK(this != get_sentinel_slot_span());
...
template <typename Callback> void IterateSlotSpans(uintptr_t super_page, Callback callback)
{
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
    PA_DCHECK(!(super_page % kSuperPageAlignment));
    ...
    auto* const first_page_metadata = PartitionPageMetadata<MetadataKind::kReadOnly>::FromAddr(SuperPagePayloadBegin(super_page));
    auto* const last_page_metadata = PartitionPageMetadata<MetadataKind::kReadOnly>::FromAddr(SuperPagePayloadEnd(super_page) - PartitionPageSize());
    ...
    for (page_metadata = first_page_metadata; page_metadata <= last_page_metadata;) {
        PA_DCHECK(!page_metadata->slot_span_metadata_offset); // Ensure slot span beginning.
    ...
    PA_DCHECK(page_metadata == reinterpret_cast<PartitionPageMetadata<MetadataKind::kReadOnly>*>(slot_span) + slot_span->bucket->get_pages_per_slot_span());
}
...
public:
    template <bool enforce = PA_CONFIG(ENFORCE_SLOT_STARTS)> PA_ALWAYS_INLINE static SlotStart FromUntaggedAddr(uintptr_t untagged_slot_start)
    {
    ...
    template <bool enforce = PA_CONFIG(ENFORCE_SLOT_STARTS)> PA_ALWAYS_INLINE static SlotStart FromObject(void* tagged_object)
    {
        uintptr_t untagged_slot_start = internal::UntagAddr(reinterpret_cast<uintptr_t>(tagged_object));
    ...
    {
        auto* slot_span_metadata = SlotSpanMetadata<MetadataKind::kReadOnly>::FromAddr(untagged_slot_start_);
        uintptr_t slot_span = SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(slot_span_metadata);
        PA_CHECK(!((untagged_slot_start_ - slot_span) % slot_span_metadata->bucket->slot_size));
    }
