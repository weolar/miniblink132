...
#define STATIC_ASSERT_OR_PA_CHECK(condition, message) static_assert(condition, message)
...
#if PA_BUILDFLAG(IS_ANDROID) && PA_BUILDFLAG(PA_ARCH_CPU_X86_FAMILY) && PA_BUILDFLAG(PA_ARCH_CPU_32_BITS)
...
struct PA_DEBUGKV_ALIGN DebugKv {
    ...
    DebugKv(const char* key, uint64_t value)
        : v(value)
    {
...
#define PA_DEBUG_DATA_ON_STACK(name, value)                                                                                                                    \
    static_assert(sizeof name <= ::partition_alloc::internal::kDebugKeyMaxLength + 1);                                                                         \
