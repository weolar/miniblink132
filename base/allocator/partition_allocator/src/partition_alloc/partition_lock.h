...
public:
    ...
    void Acquire() PA_EXCLUSIVE_LOCK_FUNCTION()
    {
        ...
        if (!lock_.Try()) {
            ...
            // hangs on Linux.
            if (owning_thread_ref_.load(std::memory_order_acquire) == current_thread) [[unlikely]] {
    ...
    void Release() PA_UNLOCK_FUNCTION()
    {
        ...
        LiftThreadIsolationScope lift_thread_isolation_restrictions;
#endif
        owning_thread_ref_.store(base::PlatformThreadRef(), std::memory_order_release);
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON) ||                                                                                                                      \
    // PA_BUILDFLAG(ENABLE_PARTITION_LOCK_REENTRANCY_CHECK)
        lock_.Release();
    }
    void AssertAcquired() const PA_ASSERT_EXCLUSIVE_LOCK()
    {
        ...
        LiftThreadIsolationScope lift_thread_isolation_restrictions;
#endif
        PA_DCHECK(owning_thread_ref_.load(std ::memory_order_acquire) == base::PlatformThread::CurrentRef());
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON) ||                                                                                                                      \
    // PA_BUILDFLAG(ENABLE_PARTITION_LOCK_REENTRANCY_CHECK)
    }
...
private:
    ...
    std::atomic<base::PlatformThreadRef> owning_thread_ref_ = base::PlatformThreadRef();
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON) ||                                                                                                                      \
    // PA_BUILDFLAG(ENABLE_PARTITION_LOCK_REENTRANCY_CHECK)
};
...
public:
    ...
    ~ScopedGuard() PA_UNLOCK_FUNCTION()
    {
        lock_.Release();
    }
...
public:
    ...
    ~ScopedUnlockGuard() PA_EXCLUSIVE_LOCK_FUNCTION()
    {
        lock_.Acquire();
    }
