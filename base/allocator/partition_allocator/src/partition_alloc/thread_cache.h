...
extern PA_COMPONENT_EXPORT(PARTITION_ALLOC) thread_local ThreadCache* g_thread_cache;
#endif

} // namespace internal

constexpr internal::base::TimeDelta kMinPurgeInterval = internal::base::Seconds(1);
constexpr internal::base::TimeDelta kMaxPurgeInterval = internal::base::Minutes(1);
constexpr internal::base::TimeDelta kDefaultPurgeInterval = 2 * kMinPurgeInterval;
...
public:
    ...
    void SetPurgingConfiguration(const internal::base::TimeDelta min_purge_interval, const internal::base::TimeDelta max_purge_interval,
    ...
    bool is_purging_configured() const
    {
        return is_purging_configured_;
    }

    static internal::Lock& GetLock()
    {
...
private:
    ...
    uint16_t largest_active_bucket_index_ = internal::BucketIndexLookup::GetIndex(kThreadCacheDefaultSizeThreshold);
};
...
public:
    ...
    static ThreadCache* Get()
    {
        ...
        return reinterpret_cast<ThreadCache*>(internal::PartitionTlsGet(internal::g_thread_cache_key));
#endif
    }
    ...
    const internal::PartitionFreelistDispatcher* get_freelist_dispatcher_from_root();
    ...
    PA_ALWAYS_INLINE std::optional<size_t> MaybePutInCache(uintptr_t slot_start, size_t bucket_index);
    ...
    PA_ALWAYS_INLINE uintptr_t GetFromCache(size_t bucket_index, size_t* slot_size);
    ...
    internal::base::PlatformThreadId thread_id() const
    {
        return thread_id_;
    }
    ...
    static constexpr size_t kDefaultSizeThreshold = kThreadCacheDefaultSizeThreshold;
    static constexpr size_t kLargeSizeThreshold = kThreadCacheLargeSizeThreshold;
    static constexpr uint16_t kBucketCount = internal::BucketIndexLookup::GetIndex(kThreadCacheLargeSizeThreshold) + 1;
    static_assert(kBucketCount < internal::kNumBuckets, "Cannot have more cached buckets than what the allocator supports");

    const ThreadCache* prev_for_testing() const PA_EXCLUSIVE_LOCKS_REQUIRED(ThreadCacheRegistry::GetLock())
    {
        return prev_;
    }
    const ThreadCache* next_for_testing() const PA_EXCLUSIVE_LOCKS_REQUIRED(ThreadCacheRegistry::GetLock())
    {
        return next_;
    }

    ThreadCacheStats& stats_for_testing()
    {
        return stats_;
    }
...
private:
    ...
    void PurgeInternal();
    template <bool crash_on_corruption> void PurgeInternalHelper();
    ...
    template <bool crash_on_corruption> void ClearBucketHelper(Bucket& bucket, size_t limit);
    ...
    template <bool crash_on_corruption> void FreeAfter(internal::PartitionFreelistEntry* head, size_t slot_size);
    ...
    std::optional<internal::LightweightQuarantineBranch> scheduler_loop_quarantine_branch_;
...
PA_ALWAYS_INLINE std::optional<size_t> ThreadCache::MaybePutInCache(uintptr_t slot_start, size_t bucket_index)
{
...
PA_ALWAYS_INLINE uintptr_t ThreadCache::GetFromCache(size_t bucket_index, size_t* slot_size)
{
...
#if PA_BUILDFLAG(IS_CHROMEOS) && PA_BUILDFLAG(PA_ARCH_CPU_X86_64) && PA_BUILDFLAG(HAS_64_BIT_POINTERS)
    ...
    const internal::PartitionFreelistDispatcher* freelist_dispatcher = get_freelist_dispatcher_from_root();
#if PA_BUILDFLAG(USE_FREELIST_DISPATCHER)
    internal::PartitionFreelistEntry* next = freelist_dispatcher->GetNextForThreadCacheTrue(entry, bucket.slot_size);
#else
    internal::PartitionFreelistEntry* next = freelist_dispatcher->GetNextForThreadCache<true>(entry, bucket.slot_size);
...
PA_ALWAYS_INLINE void ThreadCache::PutInBucket(Bucket& bucket, uintptr_t slot_start)
{
#if PA_CONFIG(HAS_FREELIST_SHADOW_ENTRY) && PA_BUILDFLAG(PA_ARCH_CPU_X86_64) && PA_BUILDFLAG(HAS_64_BIT_POINTERS)
    ...
    static_assert(internal::kPartitionCachelineSize == 64, "The computation below assumes that cache lines are 64 bytes long.");
    int distance_to_next_cacheline_in_16_bytes = 4 - ((slot_start >> 4) & 3);
    int slot_size_remaining_in_16_bytes = bucket.slot_size / 16;
    slot_size_remaining_in_16_bytes = std::min(slot_size_remaining_in_16_bytes, distance_to_next_cacheline_in_16_bytes);

    static const uint32_t poison_16_bytes[4] = { 0xbadbad00, 0xbadbad00, 0xbadbad00, 0xbadbad00 };

#if !(PA_BUILDFLAG(IS_WIN) && defined(COMPONENT_BUILD)) && PA_HAS_BUILTIN(__builtin_assume_aligned)
    void* slot_start_tagged = __builtin_assume_aligned(internal::SlotStartAddr2Ptr(slot_start), internal::kAlignment);
#else
    ...
    auto* entry = get_freelist_dispatcher_from_root()->EmplaceAndInitForThreadCache(slot_start, bucket.freelist_head);
