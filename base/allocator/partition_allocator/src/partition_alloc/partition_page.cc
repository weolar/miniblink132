...
void UnmapNow(uintptr_t reservation_start, size_t reservation_size, pool_handle pool);

PA_ALWAYS_INLINE void PartitionDirectUnmap(SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span)
{
    auto* root = PartitionRoot::FromSlotSpanMetadata(slot_span);
    PartitionRootLock(root).AssertAcquired();
    auto* extent = PartitionDirectMapExtent<MetadataKind::kReadOnly>::FromSlotSpanMetadata(slot_span);
    ...
    uintptr_t reservation_start = SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(slot_span);
...
PA_ALWAYS_INLINE void SlotSpanMetadata<MetadataKind::kWritable>::RegisterEmpty()
{
    ...
    root->empty_slot_spans_dirty_bytes += base::bits::AlignUp(GetProvisionedSize(), SystemPageSize());
    ...
    PA_DCHECK(root->global_empty_slot_span_ring_index < root->global_empty_slot_span_ring_size);
    int16_t current_index = root->global_empty_slot_span_ring_index;
    SlotSpanMetadata<MetadataKind::kReadOnly>* slot_span_to_decommit = root->global_empty_slot_span_ring[current_index];
    ...
    PA_DCHECK(current_index < base::checked_cast<int16_t>(internal::kMaxEmptySlotSpanRingSize));
    ...
    size_t max_empty_dirty_bytes = root->total_size_of_committed_pages.load(std::memory_order_relaxed) >> root->max_empty_slot_spans_dirty_bytes_shift;
    if (root->empty_slot_spans_dirty_bytes > max_empty_dirty_bytes) {
        root->ShrinkEmptySlotSpansRing(std::min(root->empty_slot_spans_dirty_bytes / 2, max_empty_dirty_bytes));
    }
}
// static
const SlotSpanMetadata<MetadataKind::kReadOnly> SlotSpanMetadata<MetadataKind::kReadOnly>::sentinel_slot_span_;

// static
const SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::get_sentinel_slot_span()
{
    return &sentinel_slot_span_;
}

// static
SlotSpanMetadata<MetadataKind::kReadOnly>* SlotSpanMetadata<MetadataKind::kReadOnly>::get_sentinel_slot_span_non_const()
{
    return const_cast<SlotSpanMetadata<MetadataKind::kReadOnly>*>(&sentinel_slot_span_);
}

void SlotSpanMetadata<MetadataKind::kWritable>::FreeSlowPath(size_t number_of_freed, PartitionRoot* root)
{
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
    DCheckRootLockIsAcquired(root);
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON)
    SlotSpanMetadata<MetadataKind::kReadOnly>* readonly_this = ToReadOnly(root);
    PA_DCHECK(readonly_this != SlotSpanMetadata<MetadataKind::kReadOnly>::get_sentinel_slot_span());
    ...
    if (marked_full) {
        ...
        PA_DCHECK(num_allocated_slots == bucket->get_slots_per_span() - number_of_freed);
        marked_full = 0;
        ...
        if (bucket->active_slot_spans_head != SlotSpanMetadata<MetadataKind::kReadOnly>::get_sentinel_slot_span()) [[likely]] {
...
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
        const PartitionFreelistDispatcher* freelist_dispatcher = PartitionRoot::FromSlotSpanMetadata(this)->get_freelist_dispatcher();
...
void SlotSpanMetadata<MetadataKind::kWritable>::Decommit(PartitionRoot* root)
{
    ...
    PA_DCHECK(is_empty_internal());
    PA_DCHECK(!bucket->is_direct_mapped());
    uintptr_t slot_span_start = SlotSpanMetadata<MetadataKind::kReadOnly>::ToSlotSpanStart(ToReadOnly(root));
    // If lazy commit is enabled, only provisioned slots are committed.
    size_t dirty_size = base::bits::AlignUp(GetProvisionedSize(), SystemPageSize());
    size_t size_to_decommit = kUseLazyCommit ? dirty_size : bucket->get_bytes_per_span();
    ...
    root->DecommitSystemPagesForData(slot_span_start, size_to_decommit, PageAccessibilityDisposition::kAllowKeepForPerf);

#if PA_BUILDFLAG(USE_FREESLOT_BITMAP)
    FreeSlotBitmapReset(slot_span_start, slot_span_start + size_to_decommit, bucket->slot_size);
#endif
...
void SlotSpanMetadata<MetadataKind::kWritable>::DecommitIfPossible(PartitionRoot* root)
{
    PartitionRootLock(root).AssertAcquired();
    PA_DCHECK(in_empty_cache_);
    PA_DCHECK(empty_cache_index_ < kMaxEmptySlotSpanRingSize);
    PA_DCHECK(ToReadOnly(root) == root->global_empty_slot_span_ring[empty_cache_index_]);
    in_empty_cache_ = 0;
...
void SlotSpanMetadata<MetadataKind::kWritable>::SortFreelist([[maybe_unused]] PartitionRoot* root)
{
    ...
    for (PartitionFreelistEntry* head = freelist_head; head; head = freelist_dispatcher->GetNext(head, slot_size)) {
        ++num_free_slots;
    ...
    if (num_free_slots > 1) {
        ...
        for (size_t slot_number = 0; slot_number < num_provisioned_slots; slot_number++) {
...
void SlotSpanMetadata<MetadataKind::kWritable>::IncrementNumberOfNonemptySlotSpans()
{
    PartitionSuperPageExtentEntry<MetadataKind::kWritable>* extent = ToSuperPageExtent();
...
void UnmapNow(uintptr_t reservation_start, size_t reservation_size, pool_handle pool)
{
    ...
    if (pool == kBRPPoolHandle) {
        ...
        PA_DCHECK(IsManagedByPartitionAllocBRPPool(
            reservation_start + AddressPoolManagerBitmap::kBytesPer1BitOfBRPPoolBitmap * AddressPoolManagerBitmap::kGuardOffsetOfBRPPoolBitmap));
#endif // PA_BUILDFLAG(HAS_64_BIT_POINTERS)

    } else
#endif // PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
    {
        const bool received_expected_pool_handle = pool == kRegularPoolHandle
...
#if PA_BUILDFLAG(ENABLE_THREAD_ISOLATION)
        PA_DCHECK(IsManagedByPartitionAllocThreadIsolatedPool(reservation_start) || IsManagedByPartitionAllocRegularPool(reservation_start)
            || IsManagedByPartitionAllocConfigurablePool(reservation_start));
#else
        PA_DCHECK(IsManagedByPartitionAllocRegularPool(reservation_start) || IsManagedByPartitionAllocConfigurablePool(reservation_start));
#endif
    }
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON)
