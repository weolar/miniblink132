...
struct ThreadIsolationOption {
    ...
    explicit ThreadIsolationOption(bool enabled)
        : enabled(enabled)
    {
    }

#if PA_BUILDFLAG(ENABLE_PKEYS)
    explicit ThreadIsolationOption(int pkey)
        : pkey(pkey)
    {
...
[[nodiscard]] int MprotectWithThreadIsolation(void* addr, size_t len, int prot, ThreadIsolationOption thread_isolation);
