...
struct PA_THREAD_ISOLATED_ALIGN IsolatedGlobals {
    ...
    void* stack;
    partition_alloc::internal::base::NoDestructor<partition_alloc::PartitionAllocator> allocator {};
} isolated_globals;
...
int ProtectROSegments(struct dl_phdr_info* info, size_t info_size, void* data)
{
    ...
    for (int i = 0; i < info->dlpi_phnum; i++) {
        ...
        PA_PCHECK(PkeyMprotect(reinterpret_cast<void*>(start_page), size, ProtFromSegmentFlags(phdr->p_flags), isolated_globals.pkey) == 0);
    }
    return 0;
}
...
protected:
    static void PkeyProtectMemory()
    {
        ...
        PA_PCHECK(PkeyMprotect(&isolated_globals, sizeof(isolated_globals), PROT_READ | PROT_WRITE, isolated_globals.pkey) == 0);

        PA_PCHECK(PkeyMprotect(isolated_globals.stack, kIsolatedThreadStackSize, PROT_READ | PROT_WRITE, isolated_globals.pkey) == 0);
    }

    static void InitializeIsolatedThread()
    {
        isolated_globals.stack = mmap(nullptr, kIsolatedThreadStackSize, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE | MAP_STACK, -1, 0);
    ...
    static void TearDownTestSuite()
    {
        ...
        PA_PCHECK(PkeyMprotect(&isolated_globals, sizeof(isolated_globals), PROT_READ | PROT_WRITE, kDefaultPkey) == 0);
...
{
    char* buf = (char*)isolated_globals.allocator->root()->Alloc<partition_alloc::AllocFlags::kNoHooks>(1024);
    if (!buf) {
...
TEST_F(PkeyTest, AllocWithoutDefaultPkey)
{
    ...
    asm volatile(
        ...
        : "=r"(ret)
        : "a"(pkru_value), "c"(0), "d"(0), "r"(reinterpret_cast<uintptr_t>(isolated_globals.stack) + kIsolatedThreadStackSize - 8)
        : "memory", "cc", "r8", "r9", "r10", "r11", "xmm0", "xmm1", "xmm2", "xmm3", "xmm4", "xmm5", "xmm6", "xmm7", "xmm8", "xmm9", "xmm10", "xmm11", "xmm12",
        "xmm13", "xmm14", "xmm15", "flags", "fpsr", "st", "st(1)", "st(2)", "st(3)", "st(4)", "st(5)", "st(6)", "st(7)");
...
TEST_F(PkeyTest, DumpPkeyPoolStats)
{
    ...
    partition_alloc::internal::AddressPoolManager::GetInstance().DumpStats(&mock_stats_dumper);
}
