...
#define PA_CONFIG_HAS_LINUX_KERNEL() (PA_BUILDFLAG(IS_LINUX) || PA_BUILDFLAG(IS_CHROMEOS) || PA_BUILDFLAG(IS_ANDROID))
...
// Need TLS support.
#define PA_CONFIG_THREAD_CACHE_SUPPORTED() (PA_BUILDFLAG(IS_POSIX) || PA_BUILDFLAG(IS_WIN) || PA_BUILDFLAG(IS_FUCHSIA))
...
#if PA_BUILDFLAG(DCHECKS_ARE_ON) || PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) || PA_BUILDFLAG(USE_PARTITION_COOKIE)
...
#if !(PA_BUILDFLAG(IS_WIN) && defined(COMPONENT_BUILD)) && !PA_BUILDFLAG(IS_APPLE) && !PA_BUILDFLAG(IS_LINUX) && !PA_BUILDFLAG(IS_CHROMEOS)
...
#if PA_BUILDFLAG(DCHECKS_ARE_ON) && PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_CONFIG(THREAD_LOCAL_TLS) && !PA_BUILDFLAG(IS_ANDROID)
...
#define PA_CONFIG_HAS_ATFORK_HANDLER() (PA_BUILDFLAG(IS_APPLE) || PA_BUILDFLAG(IS_LINUX) || PA_BUILDFLAG(IS_CHROMEOS))
...
#if PA_BUILDFLAG(ENABLE_SHADOW_METADATA_FOR_64_BITS_POINTERS) && PA_BUILDFLAG(HAS_64_BIT_POINTERS)
...
#define PA_CONFIG_IN_SLOT_METADATA_CHECK_COOKIE()                                                                                                              \
    (!(PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS) && PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT))                                                            \
        && (PA_BUILDFLAG(DCHECKS_ARE_ON) || PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)))
...
#if !PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE) && !PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS)
...
#if PA_CONFIG(IN_SLOT_METADATA_STORE_REQUESTED_SIZE) && PA_CONFIG(IN_SLOT_METADATA_CHECK_COOKIE)
...
#if PA_BUILDFLAG(IS_LINUX) || (PA_BUILDFLAG(IS_APPLE) && PA_BUILDFLAG(PA_ARCH_CPU_ARM64))
...
static_assert(__cplusplus >= 202002L, "PartitionAlloc targets C++20 or higher.");
