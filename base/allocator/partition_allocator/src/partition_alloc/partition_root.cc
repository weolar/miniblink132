...
{
    g_allocs.allocs[g_allocs.index.fetch_add(1, std::memory_order_relaxed) % kAllocInfoSize] = { addr, size };
}
...
PtrPosWithinAlloc IsPtrWithinSameAlloc(uintptr_t orig_address, uintptr_t test_address, size_t type_size)
{
    ...
    auto [slot_start, _] = PartitionAllocGetSlotStartAndSizeInBRPPool(orig_address);
    ...
    auto* slot_span = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::FromSlotStart(slot_start);
...
public:
    template <typename T> using EnumerateCallback = void (*)(PartitionRoot* root, T param);
    enum EnumerateOrder {
        kNormal,
        kReverse,
    };
    ...
    template <typename T> void Enumerate(EnumerateCallback<T> callback, T param, EnumerateOrder order) PA_NO_THREAD_SAFETY_ANALYSIS
    {
        if (order == kNormal) {
            PartitionRoot* root;
            for (root = Head(partition_roots_); root != nullptr; root = root->next_root) {
                callback(root, param);
            }
        } else {
            PA_DCHECK(order == kReverse);
            PartitionRoot* root;
            for (root = Tail(partition_roots_); root != nullptr; root = root->prev_root) {
                callback(root, param);
            }
        }
    }
...
private:
    ...
    PartitionRoot* Head(PartitionRoot* roots)
    {
        return roots;
    }
    ...
    PartitionRoot* partition_roots_ PA_GUARDED_BY(PartitionRoot::GetEnumeratorLock()) = nullptr;
};
...
#if (PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && PA_CONFIG(HAS_ATFORK_HANDLER)) || PA_CONFIG(ENABLE_SHADOW_METADATA)
...
template <typename T> void UnlockOrReinit(T& lock, bool in_child) PA_NO_THREAD_SAFETY_ANALYSIS
{
...
void UnlockOrReinitRoot(PartitionRoot* root, bool in_child) PA_NO_THREAD_SAFETY_ANALYSIS
{
...
void BeforeForkInParent() PA_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    internal::PartitionRootEnumerator::Instance().Enumerate(LockRoot, false, internal::PartitionRootEnumerator::EnumerateOrder::kNormal);
...
void ReleaseLocks(bool in_child) PA_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    internal::PartitionRootEnumerator::Instance().Enumerate(UnlockOrReinitRoot, in_child, internal::PartitionRootEnumerator::EnumerateOrder::kReverse);
...
void PartitionAllocMallocInitOnce()
{
    ...
    int err = pthread_atfork(BeforeForkInParent, AfterForkInParent, AfterForkInChild);
    PA_CHECK(err == 0);
#endif // PA_BUILDFLAG(IS_LINUX) || PA_BUILDFLAG(IS_CHROMEOS)
}
...
void MakeSuperPageExtentEntriesShared(PartitionRoot* root, internal::PoolHandleMask mask) PA_NO_THREAD_SAFETY_ANALYSIS
{
    PA_DCHECK(root);
    ...
    case internal::kRegularPoolHandle:
        ...
        root->settings.shadow_pool_offset_ = internal::PartitionAddressSpace::RegularPoolShadowOffset();
        break;
    case internal::kBRPPoolHandle:
        if (!ContainsFlags(mask, internal::PoolHandleMask::kBRP)) {
            return;
        }
        root->settings.shadow_pool_offset_ = internal::PartitionAddressSpace::BRPPoolShadowOffset();
        break;
    case internal::kConfigurablePoolHandle:
        if (!ContainsFlags(mask, internal::PoolHandleMask::kConfigurable)) {
            return;
        }
        root->settings.shadow_pool_offset_ = internal::PartitionAddressSpace::ConfigurablePoolShadowOffset();
        break;
    default:
        return;
    }
    ...
    // For normal-bucketed.
    for (const internal::PartitionSuperPageExtentEntry<internal::MetadataKind::kReadOnly>* extent = root->first_extent; extent != nullptr;
         extent = extent->next) {
    ...
    // For direct-mapped.
    for (const internal::PartitionDirectMapExtent<internal::MetadataKind::kReadOnly>* extent = root->direct_map_list; extent != nullptr;
         extent = extent->next_extent) {
        internal::PartitionAddressSpace::MapMetadata(reinterpret_cast<uintptr_t>(extent) & internal::kSuperPageBaseMask,
            /*copy_metadata=*/true);
    }
}
...
PA_ALWAYS_INLINE PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR size_t MinPurgeableSlotSize()
{
    return SystemPageSize() / kMaxPurgeableSlotsPerSystemPage;
}

PA_ALWAYS_INLINE PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR size_t MinConservativePurgeableSlotSize()
{
...
PA_NOPROFILE
static size_t PartitionPurgeSlotSpan(PartitionRoot* root, internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span, bool accounting_only)
    PA_EXCLUSIVE_LOCKS_REQUIRED(internal::PartitionRootLock(root))
{
    ...
    if (slot_span->CanStoreRawSize()) {
        uint32_t utilized_slot_size = static_cast<uint32_t>(RoundUpToSystemPage(slot_span->GetUtilizedSlotSize()));
        discardable_bytes = bucket->slot_size - utilized_slot_size;
        if (discardable_bytes && !accounting_only) {
            uintptr_t slot_span_start = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::ToSlotSpanStart(slot_span);
    ...
    constexpr size_t kMaxSlotCount = (PartitionPageSize() * kMaxPartitionPagesPerRegularSlotSpan) / MinPurgeableSlotSize();
#elif PA_BUILDFLAG(IS_APPLE) || defined(PARTITION_ALLOCATOR_CONSTANTS_POSIX_NONCONST_PAGE_SIZE)
    // It's better for slot_usage to be stack-allocated and fixed-size, which
    // demands that its size be constexpr. On IS_APPLE and Linux on arm64,
    // PartitionPageSize() is always SystemPageSize() << 2, so regardless of
    // what the run time page size is, kMaxSlotCount can always be simplified
    // to this expression.
    constexpr size_t kMaxSlotCount = 4 * kMaxPurgeableSlotsPerSystemPage * internal::kMaxPartitionPagesPerRegularSlotSpan;
    PA_CHECK(kMaxSlotCount == (PartitionPageSize() * internal::kMaxPartitionPagesPerRegularSlotSpan) / MinPurgeableSlotSize());
#endif
    PA_DCHECK(bucket_num_slots <= kMaxSlotCount);
    PA_DCHECK(slot_span->num_unprovisioned_slots < bucket_num_slots);
    size_t num_provisioned_slots = bucket_num_slots - slot_span->num_unprovisioned_slots;
    ...
    const PartitionFreelistDispatcher* freelist_dispatcher = root->get_freelist_dispatcher();

    for (PartitionFreelistEntry* entry = slot_span->get_freelist_head(); entry; entry = freelist_dispatcher->GetNext(entry, slot_size)) {
        size_t slot_number = bucket->GetSlotNumber(SlotStartPtr2Addr(entry) - slot_span_start);
    ...
    if (truncated_slots) {
        ...
        uintptr_t rounded_up_truncatation_begin_addr = RoundUpToSystemPage(begin_addr);
    ...
    if (!accounting_only) {
        auto straighten_mode = PartitionRoot::GetStraightenLargerSlotSpanFreeListsMode();
        bool straighten = straighten_mode == StraightenLargerSlotSpanFreeListsMode::kAlways
            || (straighten_mode == StraightenLargerSlotSpanFreeListsMode::kOnlyWhenUnprovisioning && unprovisioned_bytes);

        PA_DCHECK((unprovisioned_bytes > 0) == (truncated_slots > 0));
        size_t new_unprovisioned_slots = truncated_slots + slot_span->num_unprovisioned_slots;
        PA_DCHECK(new_unprovisioned_slots <= bucket->get_slots_per_span());
        slot_span->ToWritable(root)->num_unprovisioned_slots = new_unprovisioned_slots;
        ...
        if (straighten) {
            ...
            // fragmentation.
            for (size_t slot_index = 0; slot_index < num_provisioned_slots; ++slot_index) {
                ...
                // anyway.
                auto* entry = static_cast<PartitionFreelistEntry*>(SlotStartAddr2Ptr(slot_span_start + (slot_size * slot_index)));
            ...
            uintptr_t first_unprovisioned_slot = slot_span_start + (num_provisioned_slots * slot_size);
            bool skipped = false;
            for (PartitionFreelistEntry* entry = slot_span->get_freelist_head(); entry; entry = freelist_dispatcher->GetNext(entry, slot_size)) {
        ...
        if (straighten || unprovisioned_bytes) {
            if (num_new_freelist_entries) {
                ...
                last_slot = bucket->GetSlotNumber(SlotStartPtr2Addr(back) - slot_span_start);
#endif
            } else {
                PA_DCHECK(!back);
                slot_span->ToWritable(root)->SetFreelistHead(nullptr, root);
            }
            PA_DCHECK(num_new_freelist_entries == num_provisioned_slots - slot_span->num_allocated_slots);
        }

#if PA_BUILDFLAG(USE_FREESLOT_BITMAP)
        FreeSlotBitmapReset(slot_span_start + (slot_size * num_provisioned_slots), end_addr, slot_size);
#endif
        ...
        if (unprovisioned_bytes) {
            if (!kUseLazyCommit) {
                ...
                root->DecommitSystemPagesForData(begin_addr, unprovisioned_bytes, PageAccessibilityDisposition::kAllowKeepForPerf);
            }
        }
    }
    ...
    for (size_t i = 0; i < num_provisioned_slots; ++i) {
        ...
        if (rounded_down_begin_addr < rounded_up_begin_addr && i != 0 && !slot_usage[i - 1] && can_discard_free_list_pointer) {
...
PA_NOPROFILE
static void PartitionPurgeBucket(PartitionRoot* root, internal::PartitionBucket* bucket) PA_EXCLUSIVE_LOCKS_REQUIRED(internal::PartitionRootLock(root))
{
    if (bucket->active_slot_spans_head != internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::get_sentinel_slot_span()) {
        for (internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span = bucket->active_slot_spans_head; slot_span;
             slot_span = slot_span->next_slot_span) {
            PA_DCHECK(slot_span != internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::get_sentinel_slot_span());
            PartitionPurgeSlotSpan(root, slot_span, false);
        }
    }
}

static void PartitionDumpSlotSpanStats(PartitionBucketMemoryStats* stats_out, PartitionRoot* root,
    internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span) PA_EXCLUSIVE_LOCKS_REQUIRED(internal::PartitionRootLock(root))
{
    ...
    if (slot_span->CanStoreRawSize()) {
        ...
        stats_out->active_bytes += (slot_span->num_allocated_slots * stats_out->bucket_slot_size);
    }
    stats_out->active_count += slot_span->num_allocated_slots;

    size_t slot_span_bytes_resident = RoundUpToSystemPage((bucket_num_slots - slot_span->num_unprovisioned_slots) * stats_out->bucket_slot_size);
...
static void PartitionDumpBucketStats(PartitionBucketMemoryStats* stats_out, PartitionRoot* root, const internal::PartitionBucket* bucket)
    PA_EXCLUSIVE_LOCKS_REQUIRED(internal::PartitionRootLock(root))
{
    ...
    if (bucket->active_slot_spans_head == internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::get_sentinel_slot_span()
        && !bucket->empty_slot_spans_head && !bucket->decommitted_slot_spans_head && !bucket->num_full_slot_spans) {
        return;
    }
    ...
    stats_out->num_full_slot_spans = static_cast<size_t>(bucket->num_full_slot_spans);
    ...
    stats_out->resident_bytes = bucket->num_full_slot_spans * stats_out->allocated_slot_span_size;

    for (internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span = bucket->empty_slot_spans_head; slot_span;
         slot_span = slot_span->next_slot_span) {
        PA_DCHECK(slot_span->is_empty() || slot_span->is_decommitted());
        PartitionDumpSlotSpanStats(stats_out, root, slot_span);
    }
    for (internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span = bucket->decommitted_slot_spans_head; slot_span;
         slot_span = slot_span->next_slot_span) {
...
[[noreturn]] PA_NOINLINE void PartitionRoot::OutOfMemory(size_t size)
{
    const size_t virtual_address_space_size
        = total_size_of_super_pages.load(std::memory_order_relaxed) + total_size_of_direct_mapped_pages.load(std::memory_order_relaxed);
    ...
    if (virtual_address_space_size > kReasonableVirtualSize) {
        internal::PartitionOutOfMemoryWithLargeVirtualSize(virtual_address_space_size);
    }
...
void PartitionRoot::DestructForTesting() PA_EXCLUSIVE_LOCKS_REQUIRED(internal::PartitionRootLock(this))
{
...

    {
        ...
        while (curr != nullptr) {
            ...
            size_t size = internal::kSuperPageSize * curr->number_of_consecutive_super_pages;
#if !PA_BUILDFLAG(HAS_64_BIT_POINTERS)
            internal::AddressPoolManager::GetInstance().MarkUnused(pool_handle, address, size);
#endif
            internal::AddressPoolManager::GetInstance().UnreserveAndDecommit(pool_handle, address, size);
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
            if (internal::PartitionAddressSpace::IsShadowMetadataEnabled(pool_handle)) {
                internal::PartitionAddressSpace::UnmapShadowMetadata(address, pool_handle);
            }
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)
            curr = next;
        }
    ...
    if (internal::PartitionAddressSpace::IsShadowMetadataEnabled(pool_handle)) {
        ...
        while (curr != nullptr) {
            ...
            uintptr_t reservation_start = internal::base::bits::AlignDown(reinterpret_cast<uintptr_t>(curr), kSuperPageSize);
            size_t reservation_size = curr->reservation_size;

            {
                uintptr_t reservation_end = reservation_start + reservation_size;
                auto* offset_ptr = internal::ReservationOffsetPointer(reservation_start);
                ...
                // race condition.
                uint16_t i = 0;
                for (uintptr_t address = reservation_start; address < reservation_end; address += kSuperPageSize) {
                    PA_DCHECK(offset_ptr < internal::GetReservationOffsetTableEnd(address));
                    PA_DCHECK(*offset_ptr == i++);
                    *offset_ptr++ = internal::kOffsetTagNotAllocated;
                }
            }
#if !PA_BUILDFLAG(HAS_64_BIT_POINTERS)
            internal::AddressPoolManager::GetInstance().MarkUnused(pool_handle, reservation_start, reservation_size);
#endif // !PA_BUILDFLAG(HAS_64_BIT_POINTERS)

            // After resetting the table entries, unreserve and decommit the memory.
            internal::AddressPoolManager::GetInstance().UnreserveAndDecommit(pool_handle, reservation_start, reservation_size);

            internal::PartitionAddressSpace::UnmapShadowMetadata(reservation_start, pool_handle);
            curr = next;
        }
...
void PartitionRoot::InitMac11MallocSizeHackUsableSize()
{
    ...
    settings.mac11_malloc_size_hack_usable_size_ = 48 - settings.in_slot_metadata_size;
}
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) && !PA_BUILDFLAG(HAS_64_BIT_POINTERS)
namespace {
...
void ReserveBackupRefPtrGuardRegionIfNeeded()
{
    ...
    // No need to block execution for potential concurrent initialization, merely
    // want to make sure this is only called once.
    if (!g_reserve_brp_guard_region_called.compare_exchange_strong(expected, true)) {
        return;
    }
    ...
    for (size_t i = 0; i < 4; ++i) {
        [[maybe_unused]] uintptr_t allocated_address = AllocPages(
            requested_address, alignment, alignment, PageAccessibilityConfiguration(PageAccessibilityConfiguration::kInaccessible), PageTag::kPartitionAlloc);
...
{
    {
        ...
        // apple OSes.
        PA_CHECK((internal::SystemPageSize() == (size_t { 1 } << 12)) || (internal::SystemPageSize() == (size_t { 1 } << 14)));
...
#endif // PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        settings.use_configurable_pool = (opts.use_configurable_pool == PartitionOptions::kAllowed) && IsConfigurablePoolAvailable();
        PA_DCHECK(!settings.use_configurable_pool || IsConfigurablePoolAvailable());
        settings.zapping_by_free_flags = opts.zapping_by_free_flags == PartitionOptions::kEnabled;
        settings.eventually_zero_freed_memory = opts.eventually_zero_freed_memory == PartitionOptions::kEnabled;

        settings.scheduler_loop_quarantine = opts.scheduler_loop_quarantine == PartitionOptions::kEnabled;
        if (settings.scheduler_loop_quarantine) {
            internal::LightweightQuarantineBranchConfig global_config = {
                .lock_required = true,
                .branch_capacity_in_bytes = opts.scheduler_loop_quarantine_branch_capacity_in_bytes,
            };
            scheduler_loop_quarantine_branch_capacity_in_bytes = opts.scheduler_loop_quarantine_branch_capacity_in_bytes;
            scheduler_loop_quarantine.emplace(scheduler_loop_quarantine_root.CreateBranch(global_config));
        } else {
        ...
        settings.memory_tagging_enabled_ = opts.memory_tagging.enabled == PartitionOptions::kEnabled;
        ...
        PA_CHECK(!settings.memory_tagging_enabled_ || !settings.use_configurable_pool);

        settings.use_random_memory_tagging_ = opts.memory_tagging.random_memory_tagging == PartitionOptions::kEnabled;

        settings.memory_tagging_reporting_mode_ = opts.memory_tagging.reporting_mode;
#endif // PA_BUILDFLAG(HAS_MEMORY_TAGGING)

        settings.use_pool_offset_freelists = opts.use_pool_offset_freelists == PartitionOptions::kEnabled;
        ...
        PA_CHECK(!opts.thread_isolation.enabled || opts.backup_ref_ptr == PartitionOptions::kDisabled);
        ...
        const bool use_small_single_slot_spans = opts.use_small_single_slot_spans == PartitionOptions::kEnabled;

        // Set up the actual usable buckets first.
        constexpr internal::BucketIndexLookup lookup {};
        size_t bucket_index = 0;
        while (lookup.bucket_sizes()[bucket_index] != internal::kInvalidBucketSize) {
            buckets[bucket_index].Init(lookup.bucket_sizes()[bucket_index], use_small_single_slot_spans);
            bucket_index++;
        }
        ...
        for (size_t index = bucket_index; index < internal::kNumBuckets; index++) {
            ...
            // bucket is invalid.
            buckets[index].Init(internal::kInvalidBucketSize, use_small_single_slot_spans);
        ...
        settings.with_thread_cache = (opts.thread_cache == PartitionOptions::kEnabled);
...
#if PA_CONFIG(ENABLE_SHADOW_METADATA)
        if (internal::PartitionAddressSpace::IsShadowMetadataEnabled(ChoosePool())) {
            switch (ChoosePool()) {
            case internal::kRegularPoolHandle:
                settings.shadow_pool_offset_ = internal::PartitionAddressSpace::RegularPoolShadowOffset();
                break;
            case internal::kBRPPoolHandle:
                settings.shadow_pool_offset_ = internal::PartitionAddressSpace::BRPPoolShadowOffset();
                break;
            case internal::kConfigurablePoolHandle:
                settings.shadow_pool_offset_ = internal::PartitionAddressSpace::ConfigurablePoolShadowOffset();
                break;
            default:
                break;
            }
...
{
#if PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    PA_CHECK(!settings.with_thread_cache) << "Must not destroy a partition with a thread cache";
#endif // PA_BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
...
void PartitionRoot::EnableThreadCacheIfSupported()
{
    ...
    int before = thread_caches_being_constructed_.fetch_add(1, std::memory_order_acquire);
    PA_CHECK(before == 0);
    ThreadCache::Init(this);
...
bool PartitionRoot::TryReallocInPlaceForDirectMap(internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span, size_t requested_size)
{
    PA_DCHECK(slot_span->bucket->is_direct_mapped());
    // Slot-span metadata isn't MTE-tagged.
    PA_DCHECK(internal::IsManagedByDirectMap(reinterpret_cast<uintptr_t>(slot_span)));
    ...
    // for alignment.
    if ((new_reservation_size >> internal::SystemPageShift()) * 5 < (current_reservation_size >> internal::SystemPageShift()) * 4) {
        return false;
    }
    ...
    uintptr_t slot_start = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::ToSlotSpanStart(slot_span);
    ...
    PA_DCHECK(slot_start + available_reservation_size
        == reservation_start + current_reservation_size - GetDirectMapMetadataAndGuardPagesSize() + internal::PartitionPageSize());
#endif // PA_BUILDFLAG(DCHECKS_ARE_ON)
    ...
    if (new_slot_size == current_slot_size) {
        ...
        DecommitSystemPagesForData(slot_start + new_slot_size, decommit_size, PageAccessibilityDisposition::kRequireUpdate);
        ...
        RecommitSystemPagesForData(slot_start + current_slot_size, recommit_slot_size_growth, PageAccessibilityDisposition::kRequireUpdate, false);
...
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
        memset(reinterpret_cast<void*>(slot_start + current_slot_size), internal::kUninitializedByte, recommit_slot_size_growth);
#endif
    } else {
    ...
    DecreaseTotalSizeOfAllocatedBytes(reinterpret_cast<uintptr_t>(slot_span), slot_span->bucket->slot_size);
    ...
    internal::PartitionBucket* writable_bucket
        = reinterpret_cast<internal::PartitionBucket*>(reinterpret_cast<intptr_t>(slot_span->bucket) + ShadowPoolOffset());
    writable_bucket->slot_size = new_slot_size;
#endif // !PA_CONFIG(ENABLE_SHADOW_METADATA)
    IncreaseTotalSizeOfAllocatedBytes(reinterpret_cast<uintptr_t>(slot_span), slot_span->bucket->slot_size, raw_size);
...
bool PartitionRoot::TryReallocInPlaceForNormalBuckets(void* object, internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span, size_t new_size)
{
    ...
    if (AllocationCapacityFromRequestedSize(new_size) != AllocationCapacityFromSlotStart(slot_start)) {
        return false;
    }
    size_t current_usable_size = GetSlotUsableSize(slot_span);
    ...
    if (slot_span->CanStoreRawSize()) {
        ...
        if (brp_enabled()) [[likely]] {
            old_ref_count = InSlotMetadataPointerFromSlotStartAndSize(slot_start, slot_span->bucket->slot_size);
        }
#endif // PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) &&                                                                                                       \
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) && PA_BUILDFLAG(DCHECKS_ARE_ON)
        if (brp_enabled()) [[likely]] {
            internal::InSlotMetadata* new_ref_count = InSlotMetadataPointerFromSlotStartAndSize(slot_start, slot_span->bucket->slot_size);
    ...
    // PA_BUILDFLAG(DCHECKS_ARE_ON)
        ...
        if (Settings::use_cookie) {
            internal::PartitionCookieWriteValue(static_cast<unsigned char*>(object) + GetSlotUsableSize(slot_span));
        }
    }
...
void PartitionRoot::PurgeMemory(int flags)
{
...

    {
        ...
        if (flags & PurgeFlags::kDecommitEmptySlotSpans) {
            ...
            if (flags & PurgeFlags::kLimitDuration && (now_maybe_overridden_for_testing() - start > kMaxPurgeDuration)) {
                return;
            }
        }
    }
    ...
    if (flags & PurgeFlags::kDiscardUnusedSystemPages) {
        ...
        size_t min_bucket_size_to_purge = internal::MinConservativePurgeableSlotSize();
        if (!(flags & PurgeFlags::kLimitDuration) || !local_purge_generation) {
            min_bucket_size_to_purge = internal::MinPurgeableSlotSize();
        }

        for (unsigned int bucket_index = local_purge_next_bucket_index; bucket_index < internal::kNumBuckets; bucket_index++) {
...
void PartitionRoot::ShrinkEmptySlotSpansRing(size_t limit)
{
    ...
    while (empty_slot_spans_dirty_bytes > limit) {
        internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span = global_empty_slot_span_ring[index];
...
void PartitionRoot::DumpStats(const char* partition_name, bool is_light_dump, PartitionStatsDumper* dumper)
{
    ...
    if (!is_light_dump) {
        direct_map_lengths = std::unique_ptr<uint32_t[]>(new uint32_t[kMaxReportableDirectMaps]);
    }
    ...
    // below.
    {
        ...
        stats.total_mmapped_bytes
            = total_size_of_super_pages.load(std::memory_order_relaxed) + total_size_of_direct_mapped_pages.load(std::memory_order_relaxed);
        stats.total_committed_bytes = total_size_of_committed_pages.load(std::memory_order_relaxed);
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        stats.total_brp_quarantined_bytes = total_size_of_brp_quarantined_bytes.load(std::memory_order_relaxed);
        stats.total_brp_quarantined_count = total_count_of_brp_quarantined_slots.load(std::memory_order_relaxed);
        stats.cumulative_brp_quarantined_bytes = cumulative_size_of_brp_quarantined_bytes.load(std::memory_order_relaxed);
        stats.cumulative_brp_quarantined_count = cumulative_count_of_brp_quarantined_slots.load(std::memory_order_relaxed);
#endif
        ...
        for (const ReadOnlyDirectMapExtent* extent = direct_map_list; extent && num_direct_mapped_allocations < kMaxReportableDirectMaps;
             extent = extent->next_extent, ++num_direct_mapped_allocations) {
            PA_DCHECK(!extent->next_extent || extent->next_extent->prev_extent == extent);
        ...
        if (stats.has_thread_cache) {
            ThreadCacheRegistry::Instance().DumpStats(true, &stats.current_thread_cache_stats);
            ThreadCacheRegistry::Instance().DumpStats(false, &stats.all_thread_caches_stats);
        }

        stats.has_scheduler_loop_quarantine = settings.scheduler_loop_quarantine;
        if (stats.has_scheduler_loop_quarantine) {
            memset(reinterpret_cast<void*>(&stats.scheduler_loop_quarantine_stats_total), 0, sizeof(LightweightQuarantineStats));
            scheduler_loop_quarantine_root.AccumulateStats(stats.scheduler_loop_quarantine_stats_total);
        }
    }
...
void PartitionRoot::DeleteForTesting(PartitionRoot* partition_root)
{
    ...
    {
        ::partition_alloc::internal::ScopedGuard guard { internal::PartitionRootLock(partition_root) };
...
void PartitionRoot::ResetForTesting(bool allow_leaks)
{
    ...
    if (!allow_leaks) {
        ...
        for (Bucket& bucket : buckets) {
            if (bucket.active_slot_spans_head != internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::get_sentinel_slot_span()) {
                for (const internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span = bucket.active_slot_spans_head; slot_span;
                     slot_span = slot_span->next_slot_span) {
            ...
            if (bucket.num_full_slot_spans) {
                num_allocated_slots += bucket.num_full_slot_spans * bucket.get_slots_per_span();
            }
        }
    ...
    for (Bucket& bucket : buckets) {
        bucket.active_slot_spans_head = internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>::get_sentinel_slot_span_non_const();
...
ThreadCache* PartitionRoot::MaybeInitThreadCache()
{
    ...
    // load.
    if (ThreadCache::IsTombstone(tcache) || thread_caches_being_constructed_.load(std::memory_order_acquire)) {
        // Two cases:
    ...
    int before = thread_caches_being_constructed_.fetch_add(1, std::memory_order_relaxed);
...
// static
void PartitionRoot::SetStraightenLargerSlotSpanFreeListsMode(StraightenLargerSlotSpanFreeListsMode new_value)
{
...
#if PA_BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
PA_NOINLINE void PartitionRoot::QuarantineForBrp(internal::SlotSpanMetadata<internal::MetadataKind::kReadOnly>* slot_span, void* object)
{
...
void PartitionRoot::EnableShadowMetadata(internal::PoolHandleMask mask)
{
    ...
    internal::PartitionRootEnumerator::Instance().Enumerate(LockRoot, false, internal::PartitionRootEnumerator::EnumerateOrder::kNormal);
    {
        ...
        internal::PartitionRootEnumerator::Instance().Enumerate(
            MakeSuperPageExtentEntriesShared, mask, internal::PartitionRootEnumerator::EnumerateOrder::kNormal);
    }
    internal::PartitionRootEnumerator::Instance().Enumerate(UnlockOrReinitRoot, false, internal::PartitionRootEnumerator::EnumerateOrder::kReverse);
}
#endif // PA_CONFIG(ENABLE_SHADOW_METADATA)

// Explicitly define common template instantiations to reduce compile time.
#define EXPORT_TEMPLATE template PA_EXPORT_TEMPLATE_DEFINE(PA_COMPONENT_EXPORT(PARTITION_ALLOC))
...
#endif
static_assert(offsetof(PartitionRoot, sentinel_bucket) == offsetof(PartitionRoot, buckets) + internal::kNumBuckets * sizeof(PartitionRoot::Bucket),
    "sentinel_bucket must be just after the regular buckets.");

static_assert(offsetof(PartitionRoot, lock_) >= internal::kPartitionCachelineSize, "The lock should not be on the same cacheline as the read-mostly flags");
#if defined(__clang__)
