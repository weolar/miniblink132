...
// the pools.
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) PA_THREAD_ISOLATED_ALIGN AddressPoolManager {
public:
    ...
    void GetPoolUsedSuperPages(pool_handle handle, std::bitset<kMaxSuperPagesInPool>& used);
    ...
    void UnreserveAndDecommit(pool_handle handle, uintptr_t address, size_t length);
...
#endif
    char pad_[PA_THREAD_ISOLATED_ARRAY_PAD_SZ_WITH_OFFSET(Pool, kNumPools, offsetof(Pool, alloc_bitset_))] = {};
#if defined(__clang__)
