...
static_assert(kPtrTagMask == (0xffULL << kPtrTagShift), "kPtrTagMask and kPtrTagShift must be consistent");
#else
...
// range is set to the same tag.
PA_ALWAYS_INLINE void* TagMemoryRangeRandomly(void* ptr, size_t size, uint64_t mask = 0u)
{
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
    return reinterpret_cast<void*>(TagMemoryRangeRandomlyInternal(ptr, size, mask));
#else
    return ptr;
#endif // PA_BUILDFLAG(HAS_MEMORY_TAGGING)
}

PA_ALWAYS_INLINE void* TagMemoryRangeRandomly(uintptr_t address, size_t size, uint64_t mask = 0u)
{
    return TagMemoryRangeRandomly(reinterpret_cast<void*>(address), size, mask);
}

// Gets a version of ptr that's safe to dereference.
template <typename T> PA_ALWAYS_INLINE T* TagPtr(T* ptr)
{
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
...
#if PA_BUILDFLAG(HAS_MEMORY_TAGGING)
template <typename T> inline uint8_t ExtractTagFromPtr(T* ptr)
{
...
template <typename T> PA_ALWAYS_INLINE uintptr_t UntagPtr(T* ptr)
{
