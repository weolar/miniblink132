...
public:
    ...
    static void* MockAllocZeroInit(size_t n, size_t size, void* context)
    {
        ...
        return g_mock_dispatch.next->alloc_zero_initialized_function(n, size, context);
    }
    ...
    static void* MockAllocAligned(size_t alignment, size_t size, void* context)
    {
        ...
        return g_mock_dispatch.next->alloc_aligned_function(alignment, size, context);
    }
    ...
    static void* MockReallocUnchecked(void* address, size_t size, void* context)
    {
        ...
        return g_mock_dispatch.next->realloc_unchecked_function(address, size, context);
    }
    ...
    static size_t MockGoodSize(size_t size, void* context)
    {
        return size;
    }

    static unsigned MockBatchMalloc(size_t size, void** results, unsigned num_requested, void* context)
    {
        if (instance_) {
            instance_->batch_mallocs_intercepted_by_size[size] = instance_->batch_mallocs_intercepted_by_size[size] + num_requested;
        }
        return g_mock_dispatch.next->batch_malloc_function(size, results, num_requested, context);
    }

    static void MockBatchFree(void** to_be_freed, unsigned num_to_be_freed, void* context)
    {
        if (instance_) {
        ...
        g_mock_dispatch.next->batch_free_function(to_be_freed, num_to_be_freed, context);
    }
    ...
    static void* MockAlignedMalloc(size_t size, size_t alignment, void* context)
    {
        ...
        return g_mock_dispatch.next->aligned_malloc_function(size, alignment, context);
    }

    static void* MockAlignedMallocUnchecked(size_t size, size_t alignment, void* context)
    {
        if (instance_ && size < MaxSizeTracked()) {
            ++instance_->aligned_mallocs_intercepted_by_size[size];
        }
        return g_mock_dispatch.next->aligned_malloc_unchecked_function(size, alignment, context);
    }

    static void* MockAlignedRealloc(void* address, size_t size, size_t alignment, void* context)
    {
        if (instance_) {
            if (size < MaxSizeTracked()) {
        ...
        return g_mock_dispatch.next->aligned_realloc_function(address, size, alignment, context);
    }

    static void* MockAlignedReallocUnchecked(void* address, size_t size, size_t alignment, void* context)
    {
        if (instance_) {
            if (size < MaxSizeTracked()) {
                ++instance_->aligned_reallocs_intercepted_by_size[size];
        ...
        return g_mock_dispatch.next->aligned_realloc_unchecked_function(address, size, alignment, context);
    }
...
TEST_F(AllocatorShimTest, InterceptLibcSymbolsBatchMallocFree)
{
    ...
    unsigned result_count = malloc_zone_batch_malloc(malloc_default_zone(), 99, results.data(), count);
...
TEST_F(AllocatorShimTest, InterceptLibcSymbolsFreeDefiniteSize)
{
    ...
    ChromeMallocZone* default_zone = reinterpret_cast<ChromeMallocZone*>(malloc_default_zone());
...
#elif PA_BUILDFLAG(IS_LINUX) || PA_BUILDFLAG(IS_CHROMEOS) || PA_BUILDFLAG(IS_ANDROID)
static size_t GetUsableSize(void* ptr)
{
...
{
    char* str = nullptr;
    const char* lorem_ipsum = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed non risus. "
...
TEST_F(AllocatorShimTest, BatchMalloc)
{
    ...
    EXPECT_EQ(kNumToAllocate, malloc_zone_batch_malloc(malloc_default_zone(), 10, pointers, kNumToAllocate));
...
TEST_F(AllocatorShimTest, OptimizeAllocatorDispatchTable)
{
    ...
    non_empty_dispatch.get_size_estimate_function = AllocatorShimTest::MockGetSizeEstimate;
    ...
    EXPECT_EQ(head->get_size_estimate_function, AllocatorShimTest::MockGetSizeEstimate);
...
void* MockAlignedReallocUncheckedWithAdvancedChecks(void*, size_t, size_t, void*);
...
AllocatorDispatch g_mock_dispatch_for_advanced_checks = {
    ...
    .alloc_zero_initialized_function = &MockAllocZeroInitializedWithAdvancedChecks,
    ...
    .aligned_malloc_unchecked_function = &MockAlignedMallocUncheckedWithAdvancedChecks,
    .aligned_realloc_function = &MockAlignedReallocWithAdvancedChecks,
    .aligned_realloc_unchecked_function = &MockAlignedReallocUncheckedWithAdvancedChecks,
...
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->alloc_function(size, context);
}

void* MockAllocUncheckedWithAdvancedChecks(size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->alloc_unchecked_function(size, context);
}

void* MockAllocZeroInitializedWithAdvancedChecks(size_t n, size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->alloc_zero_initialized_function(n, size, context);
}

void* MockAllocAlignedWithAdvancedChecks(size_t alignment, size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->alloc_aligned_function(alignment, size, context);
}

void* MockReallocWithAdvancedChecks(void* address, size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->realloc_function(address, size, context);
}

void* MockReallocUncheckedWithAdvancedChecks(void* address, size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->realloc_unchecked_function(address, size, context);
}
...
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->get_size_estimate_function(address, context);
}

size_t MockGoodSizeWithAdvancedChecks(size_t size, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->good_size_function(size, context);
}

bool MockClaimedAddressWithAdvancedChecks(void* address, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->claimed_address_function(address, context);
}

unsigned MockBatchMallocWithAdvancedChecks(size_t size, void** results, unsigned num_requested, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->batch_malloc_function(size, results, num_requested, context);
}

void MockBatchFreeWithAdvancedChecks(void** to_be_freed, unsigned num_to_be_freed, void* context)
{
    // no-op.
    g_mock_dispatch_for_advanced_checks.next->batch_free_function(to_be_freed, num_to_be_freed, context);
}

void MockFreeDefiniteSizeWithAdvancedChecks(void* address, size_t size, void* context)
{
    g_mock_free_with_advanced_checks_count++;
    g_mock_dispatch_for_advanced_checks.next->free_definite_size_function(address, size, context);
}

void MockTryFreeDefaultWithAdvancedChecks(void* address, void* context)
{
    // no-op.
    g_mock_dispatch_for_advanced_checks.next->try_free_default_function(address, context);
}

void* MockAlignedMallocWithAdvancedChecks(size_t size, size_t alignment, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->aligned_malloc_function(size, alignment, context);
}

void* MockAlignedMallocUncheckedWithAdvancedChecks(size_t size, size_t alignment, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->aligned_malloc_unchecked_function(size, alignment, context);
}

void* MockAlignedReallocWithAdvancedChecks(void* address, size_t size, size_t alignment, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->aligned_realloc_function(address, size, alignment, context);
}

void* MockAlignedReallocUncheckedWithAdvancedChecks(void* address, size_t size, size_t alignment, void* context)
{
    // no-op.
    return g_mock_dispatch_for_advanced_checks.next->aligned_realloc_unchecked_function(address, size, alignment, context);
}

void MockAlignedFreeWithAdvancedChecks(void* address, void* context)
{
    // no-op.
    g_mock_dispatch_for_advanced_checks.next->aligned_free_function(address, context);
}
