...
#if !defined(MEMORY_TOOL_REPLACES_ALLOCATOR) && PA_BUILDFLAG(USE_PARTITION_ALLOC)
...
TEST(PartitionAllocAsMalloc, Memalign)
{
    ...
    void* data = PartitionAllocFunctions::Memalign(alignment, alloc_size, nullptr);
    EXPECT_TRUE(data);
...
TEST(PartitionAllocAsMalloc, AlignedAlloc)
{
    for (size_t alloc_size : { 100, 100000, 10000000 }) {
        for (size_t alignment = 1; alignment <= partition_alloc::kMaxSupportedAlignment; alignment <<= 1) {
            void* data = PartitionAllocFunctions::AlignedAlloc(alloc_size, alignment, nullptr);
            EXPECT_TRUE(data);
            EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(data) % alignment);
            PartitionAllocFunctions::Free(data, nullptr);
        }
...
TEST(PartitionAllocAsMalloc, AlignedRealloc)
{
    for (size_t alloc_size : { 100, 100000, 10000000 }) {
        for (size_t alignment = 1; alignment <= partition_alloc::kMaxSupportedAlignment; alignment <<= 1) {
            ...
            void* data2 = PartitionAllocFunctions::AlignedRealloc(data, alloc_size, alignment, nullptr);
            EXPECT_TRUE(data2);

            // Aligned realloc always relocates.
            EXPECT_NE(reinterpret_cast<uintptr_t>(data), reinterpret_cast<uintptr_t>(data2));
...
TEST(PartitionAllocAsMalloc, Realloc)
{
    ...
    EXPECT_TRUE(data);
    void* data2 = PartitionAllocFunctions::Realloc(data, 2u * alloc_size, nullptr);
    EXPECT_TRUE(data2);
...
{
    EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(PartitionAllocMalloc::Allocator()) % alignof(partition_alloc::PartitionRoot));
    // This works fine even if nullptr is returned.
    EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(PartitionAllocMalloc::OriginalAllocator()) % alignof(partition_alloc::PartitionRoot));
}
