...
struct AlignedPrefix {
    ...
    static_assert(kMaxWindowsAllocation < std::numeric_limits<unsigned int>::max(), "original_allocation_offset must be able to fit into an unsigned int");
...
void* AlignAllocation(void* ptr, size_t alignment)
{
    ...
    address = partition_alloc::internal::base::bits::AlignUp(address + sizeof(AlignedPrefix), alignment);

    // Write the prefix.
    AlignedPrefix* prefix = reinterpret_cast<AlignedPrefix*>(address) - 1;
    prefix->original_allocation_offset = partition_alloc::internal::base::checked_cast<unsigned int>(address - reinterpret_cast<uintptr_t>(ptr));
#if PA_BUILDFLAG(DCHECKS_ARE_ON)
...
void* UnalignAllocation(void* ptr)
{
    ...
    PA_CHECK(reinterpret_cast<uintptr_t>(ptr) - reinterpret_cast<uintptr_t>(unaligned) <= kMaxWindowsAllocation);
    return unaligned;
}
...
void* WinHeapAlignedRealloc(void* ptr, size_t size, size_t alignment)
{
    ...
    if (HeapReAlloc(get_heap_handle(), HEAP_REALLOC_IN_PLACE_ONLY, unaligned, adjusted)) {
        return ptr;
    }
