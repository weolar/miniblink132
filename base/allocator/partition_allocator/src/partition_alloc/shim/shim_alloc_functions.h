...
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
...
PA_ALWAYS_INLINE void* ShimMalloc(size_t size, void* context)
{
    ...
    while (!ptr && allocator_shim::internal::g_call_new_handler_on_malloc_failure && allocator_shim::internal::CallNewHandler(size)) [[unlikely]] {
        ptr = chain_head->alloc_function(size, context);
    }
...
PA_ALWAYS_INLINE void* ShimRealloc(void* address, size_t size, void* context)
{
    ...
    while (!ptr && size != 0 && allocator_shim::internal::g_call_new_handler_on_malloc_failure && allocator_shim::internal::CallNewHandler(size)) [[unlikely]] {
...
PA_ALWAYS_INLINE void* ShimMemalign(size_t alignment, size_t size, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
    void* ptr = chain_head->alloc_aligned_function(alignment, size, context);
...
PA_ALWAYS_INLINE int ShimPosixMemalign(void** res, size_t alignment, size_t size)
{
    // posix_memalign is supposed to check the arguments. See tc_posix_memalign()
    // in tc_malloc.cc.
    if (((alignment % sizeof(void*)) != 0) || !partition_alloc::internal::base::bits::HasSingleBit(alignment)) [[unlikely]] {
        return EINVAL;
    }
...
PA_ALWAYS_INLINE size_t ShimGetSizeEstimate(const void* address, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
    return chain_head->get_size_estimate_function(const_cast<void*>(address), context);
}
...
PA_ALWAYS_INLINE unsigned ShimBatchMalloc(size_t size, void** results, unsigned num_requested, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
    return chain_head->batch_malloc_function(size, results, num_requested, context);
}

PA_ALWAYS_INLINE void ShimBatchFree(void** to_be_freed, unsigned num_to_be_freed, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
    return chain_head->batch_free_function(to_be_freed, num_to_be_freed, context);
}

PA_ALWAYS_INLINE void ShimFreeDefiniteSize(void* ptr, size_t size, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
...
PA_ALWAYS_INLINE void* ShimAlignedMalloc(size_t size, size_t alignment, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
...
PA_ALWAYS_INLINE void* ShimAlignedRealloc(void* address, size_t size, size_t alignment, void* context)
{
    const allocator_shim::AllocatorDispatch* const chain_head = allocator_shim::internal::GetChainHead();
    void* ptr = chain_head->aligned_realloc_function(address, size, alignment, context);
    ...
    while (!ptr && size != 0 && allocator_shim::internal::g_call_new_handler_on_malloc_failure && allocator_shim::internal::CallNewHandler(size)) [[unlikely]] {
        ptr = chain_head->aligned_realloc_function(address, size, alignment, context);
    }
