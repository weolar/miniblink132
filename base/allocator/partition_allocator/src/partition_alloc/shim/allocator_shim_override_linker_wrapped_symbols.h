...
SHIM_ALWAYS_EXPORT int __wrap_posix_memalign(void** res, size_t align, size_t size)
{
...
SHIM_ALWAYS_EXPORT int __wrap_vasprintf(char** strp, const char* fmt, va_list va_args)
{
    ...
    *strp = static_cast<char*>(malloc(kInitialSize)); // Our malloc() doesn't return nullptr.
    ...
    *strp = static_cast<char*>(realloc(*strp, static_cast<size_t>(actual_size + 1)));
    ...
    if (actual_size >= kInitialSize) {
        int ret = vsnprintf(*strp, static_cast<size_t>(actual_size + 1), fmt, va_args_copy);
        va_end(va_args_copy);
        return ret;
    }
