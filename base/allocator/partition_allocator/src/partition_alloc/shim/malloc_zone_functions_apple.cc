...
static_assert(std::is_trivial_v<MallocZoneFunctions> && std::is_standard_layout_v<MallocZoneFunctions>, "MallocZoneFunctions must be POD");
...
void StoreZoneFunctions(const ChromeMallocZone* zone, MallocZoneFunctions* functions)
{
    ...
    PA_BASE_CHECK(
        functions->malloc && functions->calloc && functions->valloc && functions->free && functions->realloc && functions->size && functions->good_size);
...
bool DoesMallocZoneNeedReplacing(ChromeMallocZone* zone, const MallocZoneFunctions* functions)
{
