...
public:
    explicit SimpleScopedSpinLocker(std::atomic<bool>& lock)
        : lock_(lock)
    {
        ...
        while (!lock_.compare_exchange_weak(expected, true, std::memory_order_acquire, std::memory_order_relaxed)) {
            expected = false;
        }
    }

    ~SimpleScopedSpinLocker()
    {
...
template <typename T, typename Constructor> class LeakySingleton {
public:
...
template <typename T, typename Constructor> T* LeakySingleton<T, Constructor>::GetSlowPath()
{
...
LeakySingleton<partition_alloc::PartitionRoot, MainPartitionConstructor> g_root PA_CONSTINIT = {};
...
template <partition_alloc::AllocFlags flags> void* AllocateAlignedMemory(size_t alignment, size_t size)
{
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::Malloc(size_t size, void* context)
{
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::MallocUnchecked(size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return Allocator()->AllocInline<base_alloc_flags | partition_alloc::AllocFlags::kReturnNull>(size);
}

// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::Calloc(size_t n, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    const size_t total = partition_alloc::internal::base::CheckMul(n, size).ValueOrDie();
    return Allocator()->AllocInline<base_alloc_flags | partition_alloc::AllocFlags::kZeroFill>(total);
}

// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::Memalign(size_t alignment, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::AlignedAlloc(size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return AllocateAlignedMemory<base_alloc_flags>(alignment, size);
}
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::AlignedAllocUnchecked(size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return AllocateAlignedMemory<base_alloc_flags | partition_alloc::AllocFlags::kReturnNull>(alignment, size);
}
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::AlignedRealloc(void* address, size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    ...
    if (size > 0) {
        ...
        if (address) {
            partition_alloc::PartitionRoot::FreeInlineInUnknownRoot<base_free_flags>(address);
        }
    }
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::AlignedReallocUnchecked(void* address, size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    void* new_ptr = nullptr;
    if (size > 0) {
        new_ptr = AllocateAlignedMemory<base_alloc_flags | partition_alloc::AllocFlags::kReturnNull>(alignment, size);
    } else {
        // size == 0 and address != null means just "free(address)".
        if (address) {
            partition_alloc::PartitionRoot::FreeInlineInUnknownRoot<base_free_flags>(address);
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::Realloc(void* address, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
#if PA_BUILDFLAG(IS_APPLE)
    if (!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address)) && address) [[unlikely]] {
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void* PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::ReallocUnchecked(void* address, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
#if PA_BUILDFLAG(IS_APPLE)
    if (!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address)) && address) [[unlikely]] {
    ...
    return Allocator()->Realloc<base_alloc_flags | partition_alloc::AllocFlags::kReturnNull>(address, size, "");
}
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
PA_ALWAYS_INLINE void PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::Free(void* object, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
#if PA_BUILDFLAG(IS_APPLE)
    // TODO(bartekn): Add MTE unmasking here (and below).
    if (!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(object)) && object) [[unlikely]] {
        // A memory region allocated by the system allocator is passed in this
    ...
    partition_alloc::PartitionRoot::FreeInlineInUnknownRoot<base_free_flags>(object);
}
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::FreeDefiniteSize(void* address, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
size_t PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::GetSizeEstimate(void* address, void* context)
{
...
#if PA_BUILDFLAG(IS_APPLE)
    if (!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address))) {
    ...
    const size_t size = partition_alloc::PartitionRoot::GetUsableSizeWithMac11MallocSizeHack(address);
#if PA_BUILDFLAG(IS_APPLE)
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
size_t PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::GoodSize(size_t size, void* context)
{
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
bool PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::ClaimedAddress(void* address, void* context)
{
    return partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address));
}
#endif // PA_BUILDFLAG(IS_APPLE)

// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
unsigned PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::BatchMalloc(size_t size, void** results, unsigned num_requested, void* context)
{
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::BatchFree(void** to_be_freed, unsigned num_to_be_freed, void* context)
{
    // No real batching: we could only acquire the lock once for instance, keep it
    // simple for now.
...
// static
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags>
void PartitionAllocFunctionsInternal<base_alloc_flags, base_free_flags>::TryFreeDefault(void* address, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};

    if (!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address))) [[unlikely]] {
        // The object pointed to by `address` is not allocated by the
...
template class PA_COMPONENT_EXPORT(ALLOCATOR_SHIM) PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks, partition_alloc::FreeFlags::kNoHooks>;
// Explicitly instantiate `PartitionAllocWithAdvancedChecksFunctions`.
template class PA_COMPONENT_EXPORT(ALLOCATOR_SHIM) PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks,
    partition_alloc::FreeFlags::kNoHooks | partition_alloc::FreeFlags::kZap | partition_alloc::FreeFlags::kSchedulerLoopQuarantine>;
...
void EnablePartitionAllocMemoryReclaimer()
{
    ...
    ::partition_alloc::MemoryReclaimer::Instance()->RegisterPartition(Allocator());
...
void ConfigurePartitions(EnableBrp enable_brp, EnableMemoryTagging enable_memory_tagging,
    ...
    UseSmallSingleSlotSpans use_small_single_slot_spans)
{
    ...
    static partition_alloc::internal::base::NoDestructor<partition_alloc::PartitionAllocator> new_main_allocator([&] {
        partition_alloc::PartitionOptions opts;
        ...
        opts.thread_cache = partition_alloc::PartitionOptions::kDisabled;
        opts.star_scan_quarantine = partition_alloc::PartitionOptions::kAllowed;
        opts.backup_ref_ptr = enable_brp ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        opts.zapping_by_free_flags = zapping_by_free_flags ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        opts.eventually_zero_freed_memory
            = eventually_zero_freed_memory ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        opts.scheduler_loop_quarantine = scheduler_loop_quarantine ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        opts.scheduler_loop_quarantine_branch_capacity_in_bytes = scheduler_loop_quarantine_branch_capacity_in_bytes;
        opts.memory_tagging = { .enabled = enable_memory_tagging ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled,
            .reporting_mode = memory_tagging_reporting_mode };
        opts.use_pool_offset_freelists = use_pool_offset_freelists ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        opts.use_small_single_slot_spans
            = use_small_single_slot_spans ? partition_alloc::PartitionOptions::kEnabled : partition_alloc::PartitionOptions::kDisabled;
        return opts;
    }());
    ...
    current_root->PurgeMemory(partition_alloc::PurgeFlags::kDecommitEmptySlotSpans | partition_alloc::PurgeFlags::kDiscardUnusedSystemPages);
...
const AllocatorDispatch AllocatorDispatch::default_dispatch = internal::kPartitionAllocDispatch;
...
SHIM_ALWAYS_EXPORT struct mallinfo mallinfo(void) __THROW
{
    ...
    info.hblks = partition_alloc::internal::base::checked_cast<decltype(info.hblks)>(allocator_dumper.stats().total_mmapped_bytes);
    // Resident bytes.
    info.hblkhd = partition_alloc::internal::base::checked_cast<decltype(info.hblkhd)>(allocator_dumper.stats().total_resident_bytes);
    // Allocated bytes.
    info.uordblks = partition_alloc::internal::base::checked_cast<decltype(info.uordblks)>(allocator_dumper.stats().total_active_bytes);
