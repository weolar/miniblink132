...
template <partition_alloc::AllocFlags base_alloc_flags, partition_alloc::FreeFlags base_free_flags> class PartitionAllocFunctionsInternal {
public:
    ...
    static void* AlignedAllocUnchecked(size_t size, size_t alignment, void* context);

    static void* AlignedRealloc(void* address, size_t size, size_t alignment, void* context);

    static void* AlignedReallocUnchecked(void* address, size_t size, size_t alignment, void* context);
    ...
    static unsigned BatchMalloc(size_t size, void** results, unsigned num_requested, void* context);

    static void BatchFree(void** to_be_freed, unsigned num_to_be_freed, void* context);
...
using PartitionAllocFunctions = PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks, partition_alloc::FreeFlags::kNoHooks>;
using PartitionAllocWithAdvancedChecksFunctions = PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks,
...
extern template class PA_COMPONENT_EXPORT(
    ALLOCATOR_SHIM) PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks, partition_alloc::FreeFlags::kNoHooks>;
// `PartitionAllocWithAdvancedChecksFunctions` in instantiated in cc file.
extern template class PA_COMPONENT_EXPORT(ALLOCATOR_SHIM) PartitionAllocFunctionsInternal<partition_alloc::AllocFlags::kNoHooks,
    partition_alloc::FreeFlags::kNoHooks | partition_alloc::FreeFlags::kZap | partition_alloc::FreeFlags::kSchedulerLoopQuarantine>;
...
PA_ALWAYS_INLINE void ConfigurePartitionsForTesting()
{
    ...
    auto enable_memory_tagging = allocator_shim::EnableMemoryTagging(PA_BUILDFLAG(HAS_MEMORY_TAGGING) && PA_BUILDFLAG(DCHECKS_ARE_ON));
    ...
    auto memory_tagging_reporting_mode
        = enable_memory_tagging ? partition_alloc::TagViolationReportingMode::kSynchronous : partition_alloc::TagViolationReportingMode::kDisabled;
    ...
    ConfigurePartitions(enable_brp, enable_memory_tagging, memory_tagging_reporting_mode, distribution, scheduler_loop_quarantine,
        scheduler_loop_quarantine_capacity_in_bytes, zapping_by_free_flags, eventually_zero_freed_memory, use_pool_offset_freelists,
