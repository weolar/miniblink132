...
std::atomic<const allocator_shim::AllocatorDispatch*> g_chain_head { &allocator_shim::AllocatorDispatch::default_dispatch };
...
{
    const AllocatorDispatch* const chain_head = internal::GetChainHead();
    return chain_head->aligned_realloc_unchecked_function(ptr, size, align, nullptr);
}
...
void InsertAllocatorDispatch(AllocatorDispatch* dispatch)
{
    ...
    for (size_t i = 0; i < kMaxRetries; ++i) {
        ...
        // retry.
        if (internal::g_chain_head.compare_exchange_strong(chain_head, dispatch, std::memory_order_relaxed, std::memory_order_relaxed)) {
            // Success.
            return;
        }
    }
...
AutoResetAllocatorDispatchChainForTesting::AutoResetAllocatorDispatchChainForTesting()
{
    original_dispatch_ = internal::g_chain_head.exchange(&allocator_shim::AllocatorDispatch::default_dispatch);
}
