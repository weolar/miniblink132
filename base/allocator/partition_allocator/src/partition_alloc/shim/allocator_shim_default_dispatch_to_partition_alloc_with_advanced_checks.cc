...
namespace {
std::atomic<const AllocatorDispatch*> g_delegate_dispatch = &internal::kPartitionAllocDispatch;
...
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->alloc_zero_initialized_function(n, size, context);
}
...
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->realloc_unchecked_function(address, size, context);
}
...
unsigned DelegatedBatchMallocFn(size_t size, void** results, unsigned num_requested, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->batch_malloc_function(size, results, num_requested, context);
}

void DelegatedBatchFreeFn(void** to_be_freed, unsigned num_to_be_freed, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->batch_free_function(to_be_freed, num_to_be_freed, context);
}

void DelegatedFreeDefiniteSizeFn(void* address, size_t size, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->free_definite_size_function(address, size, context);
}
...
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->aligned_malloc_function(size, alignment, context);
}

void* DelegatedAlignedMallocUncheckedFn(size_t size, size_t alignment, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->aligned_malloc_unchecked_function(size, alignment, context);
}

void* DelegatedAlignedReallocFn(void* address, size_t size, size_t alignment, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->aligned_realloc_function(address, size, alignment, context);
}

void* DelegatedAlignedReallocUncheckedFn(void* address, size_t size, size_t alignment, void* context)
{
    const AllocatorDispatch* delegate = GetDelegate();
    PA_MUSTTAIL return delegate->aligned_realloc_unchecked_function(address, size, alignment, context);
}
...
void InstallCustomDispatch(AllocatorDispatch* dispatch)
{
...
#else
    const AllocatorDispatch* previous_value = g_delegate_dispatch.exchange(dispatch, std::memory_order_relaxed);
    ...
    PA_DCHECK(previous_value == &internal::kPartitionAllocDispatch || previous_value == dispatch);
...
void InstallCustomDispatchForPartitionAllocWithAdvancedChecks()
{
    PA_CONSTINIT static AllocatorDispatch dispatch = []() constexpr {
        auto dispatch = internal::PartitionAllocWithAdvancedChecksFunctions::MakeDispatch();
        dispatch.next = &internal::kPartitionAllocDispatch;
        return dispatch;
    }();
    InstallCustomDispatch(&dispatch);
...
{
    g_delegate_dispatch.store(&internal::kPartitionAllocDispatch, std::memory_order_relaxed);
}
