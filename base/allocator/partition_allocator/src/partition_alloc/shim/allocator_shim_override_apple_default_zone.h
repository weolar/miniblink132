...
kern_return_t MallocIntrospectionEnumerator(
    task_t task, void*, unsigned type_mask, vm_address_t zone_address, memory_reader_t reader, vm_range_recorder_t recorder)
{
...
void MallocIntrospectionStatistics(malloc_zone_t* zone, malloc_statistics_t* stats)
{
...
void MallocIntrospectionEnumerateDischargedPointers(malloc_zone_t* zone, void (^report_discharged)(void* memory, void* info))
{
    // 'discharge' is not supported.
}
...
void MallocIntrospectionPrintTask(task_t task, unsigned level, vm_address_t zone_address, memory_reader_t reader, print_task_printer_t printer)
{
...
void MallocIntrospectionTaskStatistics(task_t task, vm_address_t zone_address, memory_reader_t reader, malloc_statistics_t* stats)
{
...
unsigned MallocZoneBatchMalloc(malloc_zone_t* zone, size_t size, void** results, unsigned num_requested)
{
    return ShimBatchMalloc(size, results, num_requested, nullptr);
}

void MallocZoneBatchFree(malloc_zone_t* zone, void** to_be_freed, unsigned num)
{
...
malloc_zone_t* GetDefaultMallocZone()
{
    ...
    kern_return_t result = malloc_get_all_zones(mach_task_self(), nullptr, &zones, &zone_count);
...
bool IsAlreadyRegistered()
{
    ...
    for (unsigned int i = 0; i < zone_count; i++) {
        ...
        if (zone->zone_name && (strcmp(zone->zone_name, kPartitionAllocZoneName) == 0)) {
...
void InitializeZone()
{
    ...
    g_mac_malloc_introspection.enable_discharge_checking = MallocIntrospectionEnableDischargeChecking;
    g_mac_malloc_introspection.disable_discharge_checking = MallocIntrospectionDisableDischargeChecking;
    g_mac_malloc_introspection.discharge = MallocIntrospectionDischarge;
    g_mac_malloc_introspection.enumerate_discharged_pointers = MallocIntrospectionEnumerateDischargedPointers;
    ...
    g_mac_malloc_introspection.task_statistics = MallocIntrospectionTaskStatistics;
...
__attribute__((constructor(0))) void InitializeDefaultMallocZoneWithPartitionAlloc()
{
