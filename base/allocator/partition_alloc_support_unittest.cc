...
TEST(PartitionAllocSupportTest, ProposeSyntheticFinchTrials_DanglingPointerDetector)
{
    std::string dpd_group = ProposeSyntheticFinchTrials()["DanglingPointerDetector"];
...
#if PA_BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS) && !BUILDFLAG(IS_ANDROID) && defined(GTEST_HAS_DEATH_TEST)
...
public:
    ...
    ScopedInstallDanglingRawPtrChecks(ConstructorParams params)
    {
        enabled_feature_list_.InitWithFeaturesAndParameters(
            { { features::kPartitionAllocDanglingPtr, { { "mode", params.mode }, { "type", params.type } } } }, { /* disabled_features */ });
...
TEST(PartitionAllocDanglingPtrChecks, Basic)
{
    EXPECT_DEATH(
        ...
        AllOf(HasSubstr("Detected dangling raw_ptr with id=0x000000000000002a:"), HasSubstr("[DanglingSignature]\t"), HasSubstr("The memory was freed at:"),
...
TEST(PartitionAllocDanglingPtrChecks, FreeNotRecorded)
{
    EXPECT_DEATH(
        ...
        AllOf(HasSubstr("Detected dangling raw_ptr with id=0x000000000000002a:"), HasSubstr("[DanglingSignature]\tmissing\tmissing\t"),
            HasSubstr("It was not recorded where the memory was freed."), HasSubstr("The dangling raw_ptr was released at:")));
}
    ...
    BASE_EXPECT_DEATH(
        {
            ...
            task_environment.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(partition_alloc::GetDanglingRawPtrDetectedFn(), 42));
            task_environment.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(partition_alloc::GetDanglingRawPtrReleasedFn(), 42));
...
TEST(PartitionAllocDanglingPtrChecks, CrossTaskIgnoresSameTask)
{
    ...
    base::test::TaskEnvironment task_environment;
    task_environment.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce([] {
...
TEST(PartitionAllocDanglingPtrChecks, ExtractDanglingPtrSignatureMacStackTrace)
{
    const std::string stack_trace_output = "0   lib_1  0x0000000115fdfa12 base::F1(**) + 18\r\n"
    ...
    EXPECT_EQ("base::F3(bar)", PartitionAllocSupport::ExtractDanglingPtrSignatureForTests(stack_trace_output));
}

TEST(PartitionAllocDanglingPtrChecks, ExtractDanglingPtrSignatureMacTaskTrace)
{
    const std::string task_trace_output = "Task trace:\r\n"
                                          "0   lib_1  0x00000001161fd431 base::F1() + 257\r\n"
                                          "1   lib_1  0x0000000115a49404 base::F2() + 68\r\n";
    EXPECT_EQ("base::F1()", PartitionAllocSupport::ExtractDanglingPtrSignatureForTests(task_trace_output));
}

TEST(PartitionAllocDanglingPtrChecks, ExtractDanglingPtrSignatureWindowsStackTrace)
{
    const std::string stack_trace_output = "\tbase::F1 [0x055643C3+19] (o:\\base\\F1.cc:329)\r\n"
    ...
    EXPECT_EQ("base::F2", PartitionAllocSupport::ExtractDanglingPtrSignatureForTests(stack_trace_output));
}

TEST(PartitionAllocDanglingPtrChecks, ExtractDanglingPtrSignatureWindowsTaskTrace)
{
    const std::string task_trace_output = "Task trace:\r\n"
...
TEST(PartitionAllocSupportTest, ProposeSyntheticFinchTrials_MemoryTaggingDogfood)
{
    {
...

    {
        ...
        std::string expectation = partition_alloc::internal::base::CPU::GetInstanceNoAllocation().has_mte() ? "Enabled" : "Disabled";
        auto group_iter = trials.find("MemoryTaggingDogfood");
...
public:
    void SetUp() override
    {
        feature_list_.InitWithFeatures({ base::features::kPartitionAllocMemoryReclaimer, base::allocator::kDisableMemoryReclaimerInBackground }, {});
...
{
    test::ScopedFeatureList feature_list { base::features::kPartitionAllocMemoryReclaimer };
...
TEST_F(MemoryReclaimerSupportTest, ForegroundToBackground)
{
    ...
    task_environment_.FastForwardBy(MemoryReclaimerSupport::kFirstPAPurgeOrReclaimDelay);
    // Task gets reposted.
    EXPECT_TRUE(instance.has_pending_task_for_testing());
    EXPECT_EQ(1u, task_environment_.GetPendingMainThreadTaskCount());
