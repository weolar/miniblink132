...
{
    void* const mmap_res = mmap(nullptr, size_in_bytes, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX)
    ...
    if (mmap_res != MAP_FAILED) {
        ...
        prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, mmap_res, size_in_bytes, "tls-mmap-allocator");
    }
#endif
#endif
...
bool MMapAllocator::FreeMemoryForTesting(void* pointer_to_allocated, size_t size_in_bytes)
{
...
bool PThreadTLSSystem::Setup(OnThreadTerminationFunction thread_termination_function, std::string_view instance_id)
{
#if DCHECK_IS_ON()
    ...
    auto const key_create_res = pthread_key_create(&data_access_key_, thread_termination_function);
    ...
    crash_key_ = base::debug::AllocateCrashKeyString(crash_key_name.c_str(), GetCrashKeySize(crash_key_name));
    base::debug::SetCrashKeyString(crash_key_, base::NumberToString(data_access_key_));
