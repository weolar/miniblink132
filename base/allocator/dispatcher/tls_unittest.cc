...
struct AllocatorMockBase {
    AllocatorMockBase()
    {
        ON_CALL(*this, AllocateMemory(_)).WillByDefault([](size_t size_in_bytes) { return malloc(size_in_bytes); });
        ON_CALL(*this, FreeMemoryForTesting(_, _)).WillByDefault([](void* pointer_to_allocated, size_t size_in_bytes) {
...
struct TLSSystemMockBase {
    ...
    MOCK_METHOD(bool, Setup, (internal::OnThreadTerminationFunction thread_termination_function, std::string_view instance_id), ());
...
struct CrashKeyImplementationMockBase : public base::debug::CrashKeyImplementation {
    MOCK_METHOD(base::debug::CrashKeyString*, Allocate, (const char name[], base::debug::CrashKeySize size), (override));
    MOCK_METHOD(void, Set, (base::debug::CrashKeyString * crash_key, std::string_view value), (override));
    MOCK_METHOD(void, Clear, (base::debug::CrashKeyString * crash_key), (override));
...
template <typename T, typename Allocator, typename TLSSystem> auto CreateThreadLocalStorage(Allocator& allocator, TLSSystem& tlsSystem)
{
    return ThreadLocalStorage<T, std::reference_wrapper<Allocator>, std::reference_wrapper<TLSSystem>, 0, true>(
        "ThreadLocalStorage", std::ref(allocator), std::ref(tlsSystem));
}

template <typename T> auto CreateThreadLocalStorage()
{
    return ThreadLocalStorage<T, internal::DefaultAllocator, internal::DefaultTLSSystem, 0, true>("ThreadLocalStorage");
}
...
TEST_F(BaseThreadLocalStorageTest, VerifyDataIsIndependentBetweenDifferentSUTs)
{
...
TEST_F(BaseThreadLocalStorageTest, VerifyDistinctEntriesForEachThread)
{
    ...
    for (auto& t : threads) {
        t = std::thread { [&] {
...

            {
                ...
                threads_can_finish_cv.wait(lock, [&] { return threads_can_finish.load(); });
            }
        } };
    }

    {
        ...
        thread_counter_cv.wait(lock, [&] { return thread_counter.load() == threads.size(); });
    }
...
TEST_F(BaseThreadLocalStorageTest, VerifyEntriesAreReusedForNewThreads)
{
    ...
    for (size_t thread_count = 0; thread_count < (2 * TLSType::ItemsPerChunk); ++thread_count) {
        auto thread = std::thread { [&] { stored_object_addresses.insert(sut.GetThreadLocalData()); } };
...
TEST_F(BaseThreadLocalStorageTest, VerifySetupTeardownSequence)
{
    ...
    EXPECT_CALL(allocator_mock, AllocateMemory(_)).WillOnce([](size_t size_in_bytes) { return malloc(size_in_bytes); });
    ...
    EXPECT_CALL(allocator_mock, FreeMemoryForTesting(_, _)).WillOnce([](void* pointer_to_allocated, size_t size_in_bytes) {
        free(pointer_to_allocated);
        return true;
    });

    auto sut = CreateThreadLocalStorage<DataToStore>(allocator_mock, tlsSystem_mock);
}
...
TEST_F(BaseThreadLocalStorageTest, VerifyAllocatorIsUsedForMultipleChunks)
{
    ...
    EXPECT_CALL(allocator_mock, AllocateMemory(_)).Times(number_of_chunks).WillRepeatedly([](size_t size_in_bytes) { return malloc(size_in_bytes); });

    EXPECT_CALL(allocator_mock, FreeMemoryForTesting(_, _)).Times(number_of_chunks).WillRepeatedly([](void* pointer_to_allocated, size_t size_in_bytes) {
        free(pointer_to_allocated);
        return true;
    });
    ...
    std::array<std::thread, (number_of_chunks - 1) * decltype(sut)::ItemsPerChunk + 1> threads;
    std::mutex thread_worker_mutex;
    std::condition_variable thread_counter_cv;
    std::atomic_uint32_t thread_counter { 0 };
    std::unordered_set<void*> stored_object_addresses;
...
TEST_F(BaseThreadLocalStorageTest, VerifyTLSSystemIsUsed)
{
    ...
    EXPECT_CALL(tlsSystem_mock, Setup(NotNull(), _)).WillOnce(Return(true));
    EXPECT_CALL(tlsSystem_mock, GetThreadSpecificData()).WillOnce(Return(nullptr));
    EXPECT_CALL(tlsSystem_mock, SetThreadSpecificData(NotNull()));
    EXPECT_CALL(tlsSystem_mock, TearDownForTesting()).Times(1).WillOnce(Return(true));
...
TEST_F(BaseThreadLocalStorageDeathTest, VerifyDeathIfFreeFails)
{
    auto f = [] {
        ...
        // Setup all expectations here. If we're setting them up in the parent
        // process, they will fail because the parent doesn't execute any test.
        EXPECT_CALL(allocator_mock, FreeMemoryForTesting(_, _)).WillOnce([](void* allocated_memory, size_t size_in_bytes) {
...
TEST_F(BaseThreadLocalStorageDeathTest, VerifyDeathIfStoringTLSDataFails)
{
    auto f = [] {
        ...
        // Setup all expectations here. If we're setting them up in the parent
        // process, they will fail because the parent doesn't execute any test.
        EXPECT_CALL(tlsSystem_mock, SetThreadSpecificData(_)).Times(1).WillOnce(Return(false));
        EXPECT_CALL(tlsSystem_mock, TearDownForTesting()).Times(0);

        CreateThreadLocalStorage<DataToStore>(allocator_mock, tlsSystem_mock).GetThreadLocalData();
    };
...
struct BasePThreadTLSSystemTest : public ::testing::Test {
    void SetUp() override
    {
        thread_termination_counter = 0;
    }

protected:
    static void ThreadTerminationFunction(void*)
    {
        ++thread_termination_counter;
    }
...
TEST_F(BasePThreadTLSSystemTest, VerifySetupNTeardownSequence)
{
    ...
    const std::vector<std::string> crash_key_values = { "tls_system-first", "tls_system-second" };
    std::list<base::debug::CrashKeyString> crash_keys;

    const auto handle_allocate = [&](const char* name, base::debug::CrashKeySize size) -> base::debug::CrashKeyString* {
        const auto it_crash_key_value = std::find(std::begin(crash_key_values), std::end(crash_key_values), name);
    ...
    const auto handle_set = [&](base::debug::CrashKeyString* crash_key, std::string_view value) {
        const auto it_crash_key
            = std::find_if(std::begin(crash_keys), std::end(crash_keys), [&](const base::debug::CrashKeyString& key) { return &key == crash_key; });
    ...
    auto crash_key_implementation = std::make_unique<CrashKeyImplementationMock>();

    InSequence execution_sequence;

    for (size_t instance_index = 0; instance_index < instance_ids.size(); ++instance_index) {
        EXPECT_CALL(*crash_key_implementation, Allocate(_, _)).WillOnce(handle_allocate);
