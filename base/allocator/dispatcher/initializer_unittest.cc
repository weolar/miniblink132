...
struct Dispatcher {
    ...
    template <typename... Observers> void Initialize(const std::tuple<Observers*...>& observers)
    {
        ++total_number_of_inits_;
        ++(GetInitCounterForObservers(observers));
    }

    size_t GetTotalInitCounter() const
    {
        return total_number_of_inits_;
    }

    template <typename... Observers> size_t& GetInitCounterForObservers(const std::tuple<Observers*...>& observers)
    {
        static std::map<std::tuple<Observers*...>, size_t> observer_init_counter_map;
        reseter_[&observer_init_counter_map] = [] { observer_init_counter_map.clear(); };
...
TEST_F(BaseAllocatorDispatcherInitializerTest, VerifySettingOptionalObservers)
{
    ...
    auto initializer_1 = CreateInitializer().SetOptionalObservers(&optional_observer_1, &optional_observer_2);
    EXPECT_EQ(initializer_1.GetOptionalObservers(), std::make_tuple(&optional_observer_1, &optional_observer_2));
    ...
    EXPECT_EQ(initializer_2.GetOptionalObservers(), std::make_tuple(&optional_observer_3));
...
TEST_F(BaseAllocatorDispatcherInitializerTest, VerifyAddingOptionalObservers)
{
    ...
    auto initializer_1 = CreateInitializer().AddOptionalObservers(&optional_observer_1, &optional_observer_2);
    EXPECT_EQ(initializer_1.GetOptionalObservers(), std::make_tuple(&optional_observer_1, &optional_observer_2));
    EXPECT_EQ(initializer_1.GetMandatoryObservers(), std::make_tuple());

    auto initializer_2 = initializer_1.AddOptionalObservers(&optional_observer_3);
    EXPECT_EQ(initializer_2.GetOptionalObservers(), std::make_tuple(&optional_observer_1, &optional_observer_2, &optional_observer_3));
    EXPECT_EQ(initializer_2.GetMandatoryObservers(), std::make_tuple());

    auto initializer_3 = initializer_2.AddOptionalObservers();
    EXPECT_EQ(initializer_3.GetOptionalObservers(), std::make_tuple(&optional_observer_1, &optional_observer_2, &optional_observer_3));
    EXPECT_EQ(initializer_3.GetMandatoryObservers(), std::make_tuple());
...
TEST_F(BaseAllocatorDispatcherInitializerTest, VerifySettingMandatoryObservers)
{
    ObserverMock<int> mandatory_observer_1;
    ObserverMock<float> mandatory_observer_2;
    ObserverMock<size_t> mandatory_observer_3;

    auto initializer_1 = CreateInitializer().SetMandatoryObservers(&mandatory_observer_1, &mandatory_observer_2);
    EXPECT_EQ(initializer_1.GetMandatoryObservers(), std::make_tuple(&mandatory_observer_1, &mandatory_observer_2));
    EXPECT_EQ(initializer_1.GetOptionalObservers(), std::make_tuple());

    auto initializer_2 = initializer_1.SetMandatoryObservers(&mandatory_observer_3);
    EXPECT_EQ(initializer_2.GetMandatoryObservers(), std::make_tuple(&mandatory_observer_3));
...
TEST_F(BaseAllocatorDispatcherInitializerTest, VerifyAddingMandatoryObservers)
{
    ...
    auto initializer_1 = CreateInitializer().AddMandatoryObservers(&mandatory_observer_1, &mandatory_observer_2);
    EXPECT_EQ(initializer_1.GetMandatoryObservers(), std::make_tuple(&mandatory_observer_1, &mandatory_observer_2));
    EXPECT_EQ(initializer_1.GetOptionalObservers(), std::make_tuple());

    auto initializer_2 = initializer_1.AddMandatoryObservers(&mandatory_observer_3);
    EXPECT_EQ(initializer_2.GetMandatoryObservers(), std::make_tuple(&mandatory_observer_1, &mandatory_observer_2, &mandatory_observer_3));
    EXPECT_EQ(initializer_2.GetOptionalObservers(), std::make_tuple());

    auto initializer_3 = initializer_2.AddMandatoryObservers();
    EXPECT_EQ(initializer_3.GetMandatoryObservers(), std::make_tuple(&mandatory_observer_1, &mandatory_observer_2, &mandatory_observer_3));
    EXPECT_EQ(initializer_3.GetOptionalObservers(), std::make_tuple());
...
TEST_F(BaseAllocatorDispatcherInitializerTest, VerifyBasicInitialization)
{
    ...
    const auto observer_ptrs = std::make_tuple(&mandatory_observer_1, &mandatory_observer_2, &optional_observer_1, &optional_observer_2);

    EXPECT_EQ(1ul, test_dispatcher.GetInitCounterForObservers(observer_ptrs));
}

TEST_F(BaseAllocatorDispatcherInitializerTest, VerifyInitializationWithMandatoryNullObservers)
{
    ObserverMock<int> optional_observer_1;
    ObserverMock<float> optional_observer_2;
    ...
    const auto valid_observer_ptrs = std::make_tuple(&mandatory_observer, mandatory_null_observer, &optional_observer_1, &optional_observer_2);

    EXPECT_EQ(1ul, test_dispatcher.GetTotalInitCounter());
    EXPECT_EQ(1ul, test_dispatcher.GetInitCounterForObservers(valid_observer_ptrs));
}

TEST_F(BaseAllocatorDispatcherInitializerTest, VerifyInitializationWithOptionalNullObservers)
{
    ...
    const auto valid_observer_ptrs = std::make_tuple(&mandatory_observer_1, &mandatory_observer_2, &optional_observer);
