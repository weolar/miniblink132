...
struct ObserverMock {
    ...
    size_t GetNumberOnAllocationCalls() const
    {
        return on_allocation_calls_;
    }
    size_t GetNumberOnFreeCalls() const
    {
        return on_free_calls_;
    }
...
struct DispatcherInitializerGuard {
    template <typename... Observers> explicit DispatcherInitializerGuard(std::tuple<Observers*...> observers)
    {
    ...
    {
        Dispatcher::GetInstance().ResetForTesting();
    }
};

struct BaseAllocatorDispatcherTest : public DispatcherTest { };

template <typename A> void DoBasicTest(A& allocator)
{
    ...
    {
        DispatcherInitializerGuard const g(testing::CreateTupleOfPointers(observers));
...
#if PA_BUILDFLAG(USE_PARTITION_ALLOC) && !defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
...
struct AllocatorShimAllocator {
    ...
    void Free(void* data)
    {
        allocator_shim::UncheckedFree(data);
    }
};
...
#define MAYBE_VerifyNotificationUsingAllocatorShim DISABLED_VerifyNotificationUsingAllocatorShim
#else
#define MAYBE_VerifyNotificationUsingAllocatorShim VerifyNotificationUsingAllocatorShim
#endif
