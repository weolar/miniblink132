...
#define TLS_RAW_CHECK(error_message, condition) TLS_RAW_CHECK_IMPL(error_message, condition, __FILE__, __LINE__)

#define TLS_RAW_CHECK_IMPL(error_message, condition, file, line)                                                                                               \
    do {                                                                                                                                                       \
        if (!(condition)) {                                                                                                                                    \
            constexpr const char* message = "TLS System: " error_message " Failed condition '" #condition "' in (" file "@" STR(line) ").\n";                  \
...
#if defined(PAGE_ALLOCATOR_CONSTANTS_ARE_CONSTEXPR) && PAGE_ALLOCATOR_CONSTANTS_ARE_CONSTEXPR
    constexpr static size_t AllocationChunkSize = partition_alloc::PartitionPageSize();
...
public:
    ...
    bool Setup(OnThreadTerminationFunction thread_termination_function, std::string_view instance_id);
...
// underlying value.
template <typename T> T& dereference(T& ref)
{
    return ref;
}

template <typename T> T& dereference(std::reference_wrapper<T>& ref)
{
...
template <typename PayloadType, typename AllocatorType, typename TLSSystemType, size_t AllocationChunkSize, bool IsDestructibleForTesting>
struct ThreadLocalStorage {
    ...
    // first chunk of data.
    ThreadLocalStorage(std::string_view instance_id, AllocatorType allocator, TLSSystemType tls_system)
    ...
    ~ThreadLocalStorage()
    {
        if constexpr (IsDestructibleForTesting) {
            ...
            static_assert(IsDestructibleForTesting, "ThreadLocalStorage cannot be destructed outside of test code.");
        }
    }
    ...
    PayloadType* GetThreadLocalData()
    {
        ...
        if (slot == nullptr) [[unlikely]] {
            ...
            TLS_RAW_CHECK("Failed to set thread specific data.", tls_system.SetThreadSpecificData(slot));
...
private:
    ...
    template <size_t NumberOfItems> struct ChunkT {
    ...
    template <size_t LowerNumberOfItems, size_t UpperNumberOfItems, size_t NumberOfBytes> static constexpr size_t CalculateEffectiveNumberOfItemsBinSearch()
    {
        if constexpr (LowerNumberOfItems == UpperNumberOfItems) {
            return LowerNumberOfItems;
        }

        constexpr size_t CurrentNumberOfItems = (UpperNumberOfItems - LowerNumberOfItems) / 2 + LowerNumberOfItems;

        if constexpr (sizeof(ChunkT<CurrentNumberOfItems>) > NumberOfBytes) {
            return CalculateEffectiveNumberOfItemsBinSearch<LowerNumberOfItems, CurrentNumberOfItems, NumberOfBytes>();
        }

        if constexpr (sizeof(ChunkT<CurrentNumberOfItems + 1>) < NumberOfBytes) {
            return CalculateEffectiveNumberOfItemsBinSearch<CurrentNumberOfItems + 1, UpperNumberOfItems, NumberOfBytes>();
        }
    ...
    // search.
    template <size_t MinimumNumberOfItems, size_t NumberOfBytes> static constexpr size_t CalculateEffectiveNumberOfItems()
    {
        if constexpr (sizeof(ChunkT<MinimumNumberOfItems>) < NumberOfBytes) {
            constexpr size_t LowerNumberOfItems = MinimumNumberOfItems;
            constexpr size_t UpperNumberOfItems = NumberOfBytes / sizeof(PayloadType) + 1;
            return CalculateEffectiveNumberOfItemsBinSearch<LowerNumberOfItems, UpperNumberOfItems, NumberOfBytes>();
        }
...
public:
    ...
    static constexpr size_t ItemsPerChunk = CalculateEffectiveNumberOfItems<MinimumNumberOfItemsPerChunk, AllocationChunkSize>();
...
private:
    ...
    static void MarkSlotAsFree(void* data)
    {
        ...
        // recursion.
        TLS_RAW_CHECK("Received an invalid slot.", slot && slot->is_used.test_and_set());
    ...
    {
        void* const uninitialized_memory = dereference(allocator_).AllocateMemory(sizeof(Chunk));

        // We might be called in the course of handling a memory allocation. We do
        // not use CHECK since they might allocate and cause a recursion.
        TLS_RAW_CHECK("Failed to allocate memory for new chunk.", uninitialized_memory != nullptr);
    ...
    void FreeAndDeallocateChunkForTesting(Chunk* chunk_to_erase)
    {
        ...
        CHECK(dereference(allocator_).FreeMemoryForTesting(chunk_to_erase, sizeof(Chunk)));
    }
    ...
    {
        SingleSlot* const slot = std::find_if_not(std::begin(chunk->slots), std::end(chunk->slots),
            [](SingleSlot& candidate_slot) { return candidate_slot.is_used.test_and_set(std::memory_order_relaxed); });
    ...
    template <bool IsDestructibleForTestingP = IsDestructibleForTesting> typename std::enable_if<IsDestructibleForTestingP>::type TearDownForTesting()
    {
    ...
    // instrumentation.
    DISABLE_TSAN_INSTRUMENTATION void Reset(PayloadType& item)
    {
        item = {};
    }
...
template <typename StorageType, typename AllocatorType = internal::DefaultAllocator, typename TLSSystemType = internal::DefaultTLSSystem,
    size_t AllocationChunkSize = AllocatorType::AllocationChunkSize, bool IsDestructibleForTesting = false>
using ThreadLocalStorage = internal::ThreadLocalStorage<StorageType, AllocatorType, TLSSystemType, AllocationChunkSize, IsDestructibleForTesting>;
