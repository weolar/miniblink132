...
BASE_EXPORT void StartMemoryReclaimer(scoped_refptr<SequencedTaskRunner> task_runner);
...
public:
    ...
    void ReconfigureAfterFeatureListInit(const std::string& process_type, bool configure_dangling_pointer_detector = true);
    ...
    static std::string ExtractDanglingPtrSignatureForTests(std::string stacktrace);
#endif
    ...
    static bool ShouldEnablePartitionAllocWithAdvancedChecks(const std::string& process_type);
...
private:
    ...
    size_t largest_cached_size_ = ::partition_alloc::kThreadCacheDefaultSizeThreshold;
#endif
};
...
public:
    ...
    bool has_pending_task_for_testing() const
    {
    ...
    // Visible for testing
    static constexpr base::TimeDelta kFirstPAPurgeOrReclaimDelay = base::Minutes(1);
