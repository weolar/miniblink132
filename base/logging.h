...
using LogAssertHandlerFunction = base::RepeatingCallback<void(const char* file, int line, std::string_view message, std::string_view stack_trace)>;
...
typedef bool (*LogMessageHandlerFunction)(int severity, const char* file, int line, size_t message_start, const std::string& str);
...
#define COMPACT_GOOGLE_LOG_EX_WARNING(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_WARNING, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_ERROR(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_ERROR, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_FATAL(ClassName, ...) ::logging::ClassName##Fatal(__FILE__, __LINE__, ::logging::LOGGING_FATAL, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_DFATAL(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_DFATAL, ##__VA_ARGS__)
...
#define LOG_IS_ON(severity) (::logging::LOGGING_##severity == ::logging::LOGGING_FATAL || ::logging::ShouldCreateLogMessage(::logging::LOGGING_##severity))
...
// may be slow.
#define VLOG_IS_ON(verboselevel) ((verboselevel) <= (ENABLED_VLOG_LEVEL) || (verboselevel) <= ::logging::GetVlogLevel(__FILE__))
...
#define VLOG_STREAM(verbose_level) ::logging::LogMessage(__FILE__, __LINE__, -(verbose_level)).stream()
...
#if BUILDFLAG(IS_WIN)
#define VPLOG_STREAM(verbose_level) ::logging::Win32ErrorLogMessage(__FILE__, __LINE__, -(verbose_level), ::logging::GetLastSystemErrorCode()).stream()
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
#define VPLOG_STREAM(verbose_level) ::logging::ErrnoLogMessage(__FILE__, __LINE__, -(verbose_level), ::logging::GetLastSystemErrorCode()).stream()
#endif

#define VPLOG(verbose_level) LAZY_STREAM(VPLOG_STREAM(verbose_level), VLOG_IS_ON(verbose_level))

#define VPLOG_IF(verbose_level, condition) LAZY_STREAM(VPLOG_STREAM(verbose_level), VLOG_IS_ON(verbose_level) && (condition))

// TODO(akalin): Add more VLOG variants, e.g. VPLOG.

#define LOG_ASSERT(condition) LOG_IF(FATAL, !(ANALYZER_ASSUME_TRUE(condition))) << "Assert failed: " #condition ". "

#if BUILDFLAG(IS_WIN)
#define PLOG_STREAM(severity) COMPACT_GOOGLE_LOG_EX_##severity(Win32ErrorLogMessage, ::logging::GetLastSystemErrorCode()).stream()
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
#define PLOG_STREAM(severity) COMPACT_GOOGLE_LOG_EX_##severity(ErrnoLogMessage, ::logging::GetLastSystemErrorCode()).stream()
#endif
...
// behavior.
#define EAT_STREAM_PARAMETERS true ? (void)0 : ::logging::LogMessageVoidify() & (*::logging::g_swallow_stream)
...
// but LOG(FATAL) is.
#define DLOG_IS_ON(severity) (::logging::ShouldCreateLogMessage(::logging::LOGGING_##severity))

#define DLOG(severity) LAZY_STREAM(LOG_STREAM(severity), DLOG_IS_ON(severity))
#define DLOG_IF(severity, condition) LAZY_STREAM(LOG_STREAM(severity), DLOG_IS_ON(severity) && (condition))
#define DPLOG(severity) LAZY_STREAM(PLOG_STREAM(severity), DLOG_IS_ON(severity))
#define DPLOG_IF(severity, condition) LAZY_STREAM(PLOG_STREAM(severity), DLOG_IS_ON(severity) && (condition))
#define DVLOG_IF(verboselevel, condition) VLOG_IF(verboselevel, condition)
#define DVPLOG_IF(verboselevel, condition) VPLOG_IF(verboselevel, condition)
#define DLOG_ASSERT(condition) DLOG_IF(FATAL, !(condition)) << "Assert failed: " #condition ". "
...
#if BUILDFLAG(IS_CHROMEOS)
    void InitWithSyslogPrefix(std::string_view filename, int line, uint64_t tick_count, const char* log_severity_name_c_str, const char* log_prefix,
        bool enable_process_id, bool enable_thread_id, bool enable_timestamp, bool enable_tickcount);
#endif
};
...
class BASE_EXPORT Win32ErrorLogMessageFatal final : public Win32ErrorLogMessage {
public:
