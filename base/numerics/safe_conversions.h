...
template <typename Dst, typename Src> struct IsValueInRangeFastOp {
    static constexpr bool is_supported = false;
...
template <typename Dst, typename Src>
    requires(std::signed_integral<Dst> && std::signed_integral<Src> && !IsTypeInRangeForNumericType<Dst, Src>::value)
...
template <typename Dst, typename Src>
    requires(std::unsigned_integral<Dst> && std::signed_integral<Src> && !IsTypeInRangeForNumericType<Dst, Src>::value)
struct IsValueInRangeFastOp<Dst, Src> {
    static constexpr bool is_supported = true;
...
template <typename Dst, typename Src> struct SaturateFastOp {
    static constexpr bool is_supported = false;
...
template <typename Dst, template <typename> class SaturationHandler = SaturationDefaultLimits, typename Src> constexpr Dst saturated_cast(Src value)
{
    ...
    return !IsConstantEvaluated() && SaturateFastOp<Dst, SrcType>::is_supported && std::is_same_v<SaturationHandler<Dst>, SaturationDefaultLimits<Dst>>
...
template <typename Dst, typename Src> struct IsNumericRangeContained {
    static constexpr bool value = false;
};

template <typename Dst, typename Src>
    requires(ArithmeticOrUnderlyingEnum<Dst>::value && ArithmeticOrUnderlyingEnum<Src>::value)
...
private:
    template <typename> friend class StrictNumeric;
