...
BASE_EXPORT void Base64UrlEncode(span<const uint8_t> input, Base64UrlEncodePolicy policy, std::string* output);

// Same as the previous function, but accepts an input string.
BASE_EXPORT void Base64UrlEncode(std::string_view input, Base64UrlEncodePolicy policy, std::string* output);
...
[[nodiscard]] BASE_EXPORT bool Base64UrlDecode(std::string_view input, Base64UrlDecodePolicy policy, std::string* output);

// Same as the previous function, but writing to a `std::vector`.
[[nodiscard]] BASE_EXPORT std::optional<std::vector<uint8_t>> Base64UrlDecode(std::string_view input, Base64UrlDecodePolicy policy);
