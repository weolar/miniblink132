...
bool IsSwitch(const CommandLine::StringType& string, CommandLine::StringType* switch_string, CommandLine::StringType* switch_value)
{
    switch_string->clear();
    switch_value->clear();
...
bool IsSwitchWithKey(CommandLine::StringViewType string, CommandLine::StringViewType switch_key_without_prefix)
{
    size_t prefix_length = GetSwitchPrefixLength(string);
    if (prefix_length == 0 || prefix_length == string.length())
        return false;
    ...
    const size_t equals_position = string.find(kSwitchValueSeparator);
    return string.substr(prefix_length, equals_position - prefix_length) == switch_key_without_prefix;
}
...
std::wstring QuoteForCommandLineToArgvWInternal(const std::wstring& arg, bool allow_unsafe_insert_sequences)
{
...
// static
void CommandLine::SetDuplicateSwitchHandler(std::unique_ptr<DuplicateSwitchHandler> new_duplicate_switch_handler)
{
...
CommandLine::CommandLine(CommandLine&& other) noexcept
    :
#if BUILDFLAG(IS_WIN)
    raw_command_line_string_(std::exchange(other.raw_command_line_string_, StringViewType()))
    , has_single_argument_switch_(std::exchange(other.has_single_argument_switch_, false))
    ,
#endif // BUILDFLAG(IS_WIN)
...
void CommandLine::set_slash_is_not_a_switch()
{
    ...
    static_assert(base::make_span(kSwitchPrefixes).back() == L"/", "Error: Last switch prefix is not a slash.");
...
CommandLine CommandLine::FromArgvWithoutProgram(const StringVector& argv)
{
    ...
    cmd.AppendSwitchesAndArguments(span<const CommandLine::StringType>(argv.data(), argv.size()));
    return cmd;
}
...
void CommandLine::InitFromArgv(const StringVector& argv)
{
    ...
    if (!argv.empty()) {
        AppendSwitchesAndArguments(span<const CommandLine::StringType>(argv.data(), argv.size()).subspan(1));
    }
}
...
void CommandLine::AppendSwitchPath(std::string_view switch_string, const FilePath& path)
{
    AppendSwitchNative(switch_string, path.value());
}

void CommandLine::AppendSwitchNative(std::string_view switch_string, CommandLine::StringViewType value)
{
#if BUILDFLAG(ENABLE_COMMANDLINE_SEQUENCE_CHECKS)
    sequence_checker_.Check();
#endif
...
void CommandLine::AppendSwitchASCII(std::string_view switch_string, std::string_view value_string)
{
#if BUILDFLAG(IS_WIN)
...
void CommandLine::RemoveSwitch(std::string_view switch_key_without_prefix)
{
    ...
    auto expell = std::remove_if(
        argv_switches_begin, argv_switches_end, [&switch_key_native](const StringType& arg) { return IsSwitchWithKey(arg, switch_key_native); });
...
void CommandLine::CopySwitchesFrom(const CommandLine& source, span<const char* const> switches)
{
...
void CommandLine::AppendArguments(const CommandLine& other, bool include_program)
{
    if (include_program)
        SetProgram(other.GetProgram());
    if (!other.argv().empty()) {
        AppendSwitchesAndArguments(span<const CommandLine::StringType>(other.argv().data(), other.argv().size()).subspan(1));
    }
}
...
void CommandLine::ParseFromString(StringViewType command_line)
{
    ...
    HMODULE downlevel_shell32_dll = ::LoadLibraryEx(L"api-ms-win-downlevel-shell32-l1-1-0.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (downlevel_shell32_dll) {
        auto command_line_to_argv_w_proc = reinterpret_cast<decltype(::CommandLineToArgvW)*>(::GetProcAddress(downlevel_shell32_dll, "CommandLineToArgvW"));
...
CommandLine::StringType CommandLine::GetArgumentsStringInternal(bool allow_unsafe_insert_sequences) const
{
    StringType params;
    ...
    for (size_t i = 1; i < argv_.size(); ++i) {
        ...
        if (parse_switches && IsSwitch(arg, &switch_string, &switch_value)) {
...
#if BUILDFLAG(IS_WIN)
                switch_value = QuoteForCommandLineToArgvWInternal(switch_value, allow_unsafe_insert_sequences);
#endif
            ...
            if (has_single_argument_switch_) {
                ...
                params.append(base::StrCat({ kSwitchPrefixes[0], kSingleArgument, FILE_PATH_LITERAL(" ") }));
            } else {
                arg = QuoteForCommandLineToArgvWInternal(arg, allow_unsafe_insert_sequences);
            }
#endif
            params.append(arg);
        }
...
#if BUILDFLAG(IS_WIN)
    string = QuoteForCommandLineToArgvWInternal(string,
...
{
    return QuoteForCommandLineToArgvWInternal(arg, /*allow_unsafe_insert_sequences=*/false);
}
...
CommandLine::StringType CommandLine::GetCommandLineStringForShell() const
{
    ...
    return command_line_string + FILE_PATH_LITERAL(" ") + StringType(kSwitchPrefixes[0]) + kSingleArgument + FILE_PATH_LITERAL(" %1");
}

CommandLine::StringType CommandLine::GetCommandLineStringWithUnsafeInsertSequences() const
{
    StringType string(argv_[0]);
    string = QuoteForCommandLineToArgvWInternal(string,
...
#if BUILDFLAG(IS_WIN)
void CommandLine::ParseAsSingleArgument(const CommandLine::StringType& single_arg_switch)
{
