...
public:
    ...
    static std::optional<HardwareModelNameSplit> SplitHardwareModelNameDoNotUse(std::string_view name);
#endif
    ...
    static void SetChromeOSVersionInfoForTest(const std::string& lsb_release, const Time& lsb_release_time);
    ...
    static bool IsLowEndDeviceOrPartialLowEndModeEnabled(const FeatureParam<bool>& param_for_exclusion);
...
private:
    ...
    static uint64_t AmountOfAvailablePhysicalMemory(const SystemMemoryInfoKB& meminfo);
#endif
    ...
    static std::optional<uint64_t> SetAmountOfPhysicalMemoryMbForTesting(uint64_t amount_of_memory_mb);
