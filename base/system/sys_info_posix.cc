...
base::LazyInstance<base::internal::LazySysInfoValue<uint64_t, AmountOfVirtualMemory>>::Leaky g_lazy_virtual_memory = LAZY_INSTANCE_INITIALIZER;
...
bool GetDiskSpaceInfo(const base::FilePath& path, int64_t* available_bytes, int64_t* total_bytes)
{
    ...
    if (available_bytes) {
        *available_bytes = zero_size_means_unlimited ? std::numeric_limits<int64_t>::max() : base::saturated_cast<int64_t>(stats.f_bavail * stats.f_frsize);
    }

    if (total_bytes) {
        *total_bytes = zero_size_means_unlimited ? std::numeric_limits<int64_t>::max() : base::saturated_cast<int64_t>(stats.f_blocks * stats.f_frsize);
    }
    return true;
}
...
{
#if BUILDFLAG(IS_MAC)
    std::optional<int> number_of_physical_cores = internal::NumberOfProcessorsWhenCpuSecurityMitigationEnabled();
...
int SysInfo::NumberOfEfficientProcessorsImpl()
{
    ...
    std::vector<uint32_t> max_core_frequencies_khz(static_cast<size_t>(num_cpus), 0);
    for (int core_index = 0; core_index < num_cpus; ++core_index) {
        std::string content;
        auto path = StringPrintf("/sys/devices/system/cpu/cpu%d/cpufreq/cpuinfo_max_freq", core_index);
        if (!ReadFileToStringNonBlocking(FilePath(path), &content))
            return 0;
        if (!StringToUint(content, &max_core_frequencies_khz[static_cast<size_t>(core_index)]))
            return 0;
    }

    auto [min_max_core_frequencies_khz_it, max_max_core_frequencies_khz_it]
        = std::minmax_element(max_core_frequencies_khz.begin(), max_core_frequencies_khz.end());

    if (*min_max_core_frequencies_khz_it == *max_max_core_frequencies_khz_it)
        return 0;

    return static_cast<int>(std::count(max_core_frequencies_khz.begin(), max_core_frequencies_khz.end(), *min_max_core_frequencies_khz_it));
#else
    NOTIMPLEMENTED();
    return 0;
#endif
