...
base::LazyInstance<base::internal::LazySysInfoValue<uint64_t, AmountOfPhysicalMemory>>::Leaky g_lazy_physical_memory = LAZY_INSTANCE_INITIALIZER;
...
uint64_t SysInfo::AmountOfAvailablePhysicalMemory(const SystemMemoryInfoKB& info)
{
    ...
    int res_kb = info.available != 0 ? std::max(info.available - info.active_file, 0) : info.free + info.reclaimable + info.inactive_file;
...
SysInfo::HardwareInfo SysInfo::GetHardwareInfoSync()
{
    ...
    HardwareInfo info;
    std::string data;
    if (ReadFileToStringWithMaxSize(FilePath("/sys/devices/virtual/dmi/id/sys_vendor"), &data, kMaxStringSize)) {
        TrimWhitespaceASCII(data, TrimPositions::TRIM_ALL, &info.manufacturer);
    }
    if (ReadFileToStringWithMaxSize(FilePath("/sys/devices/virtual/dmi/id/product_name"), &data, kMaxStringSize)) {
