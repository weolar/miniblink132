...
TEST_F(SysInfoTest, NumProcs)
{
    ...
    EXPECT_LT(SysInfo::NumberOfEfficientProcessors(), SysInfo::NumberOfProcessors());
}
...
TEST_F(SysInfoTest, NumProcsWithSecurityMitigationEnabled)
{
    ...
    EXPECT_EQ(SysInfo::NumberOfProcessors(), internal::NumberOfPhysicalProcessors());
...
#define MAYBE_AmountOfAvailablePhysicalMemory DISABLED_AmountOfAvailablePhysicalMemory
#else
...
TEST_F(SysInfoTest, NestedVolumesAmountOfTotalDiskSpace)
{
    ...
    EXPECT_EQ(SysInfo::AmountOfTotalDiskSpace(subdirectory_path), kInnerVolumeQuota);

    // Remove the inner directory quota setting and check again.
    SysInfo::SetAmountOfTotalDiskSpace(subdirectory_path, -1);
    EXPECT_EQ(SysInfo::AmountOfTotalDiskSpace(subdirectory_path), kOuterVolumeQuota);
}
#endif // BUILDFLAG(IS_FUCHSIA)

#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
...
TEST_F(SysInfoTest, OperatingSystemVersionNumbers)
{
    ...
    SysInfo::OperatingSystemVersionNumbers(&os_major_version, &os_minor_version, &os_bugfix_version);
...
TEST_F(SysInfoTest, HardwareModelNameFormatMacAndiOS)
{
    ...
    // a number.
    EXPECT_TRUE(base::MatchPattern(hardware_model, "iOS Simulator (*)")) << hardware_model;
    std::vector<std::string_view> mainPieces = SplitStringPiece(hardware_model, "()", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    ASSERT_EQ(3u, mainPieces.size()) << hardware_model;
    std::vector<std::string_view> modelPieces = SplitStringPiece(mainPieces[1], ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    ASSERT_GE(modelPieces.size(), 1u) << hardware_model;
    if (modelPieces.size() == 1u) {
        EXPECT_TRUE(modelPieces[0] == "Unknown" || modelPieces[0] == "iPhone" || modelPieces[0] == "iPad") << hardware_model;
    } else {
        int value;
    ...
    // a number.
    std::vector<std::string_view> pieces = SplitStringPiece(hardware_model, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
...
TEST_F(SysInfoTest, GetHardwareInfo)
{
    ...
    auto callback
        = base::BindOnce([](std::optional<SysInfo::HardwareInfo>* target_info, SysInfo::HardwareInfo info) { *target_info = std::move(info); }, &hardware_info);
...
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
        false;
#else
        true;
#endif
...
TEST_F(SysInfoTest, GetHardwareInfoWMIMatchRegistry)
{
    ...
    static constexpr wchar_t query_computer_system[] = L"SELECT Manufacturer,Model FROM Win32_ComputerSystem";

    Microsoft::WRL::ComPtr<IEnumWbemClassObject> enumerator_computer_system;
    HRESULT hr = wmi_services->ExecQuery(base::win::ScopedBstr(L"WQL").Get(), base::win::ScopedBstr(query_computer_system).Get(),
        WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr, &enumerator_computer_system);
    EXPECT_FALSE(FAILED(hr) || !enumerator_computer_system.Get());

    Microsoft::WRL::ComPtr<IWbemClassObject> class_object;
    ULONG items_returned = 0;
    hr = enumerator_computer_system->Next(WBEM_INFINITE, 1, &class_object, &items_returned);
    EXPECT_FALSE(FAILED(hr) || !items_returned);

    base::win::ScopedVariant manufacturerVar;
    std::wstring manufacturer;
    hr = class_object->Get(L"Manufacturer", 0, manufacturerVar.Receive(), nullptr, nullptr);
    if (SUCCEEDED(hr) && manufacturerVar.type() == VT_BSTR) {
        manufacturer.assign(V_BSTR(manufacturerVar.ptr()), ::SysStringLen(V_BSTR(manufacturerVar.ptr())));
    }
    ...
    if (SUCCEEDED(hr) && modelVar.type() == VT_BSTR) {
        model.assign(V_BSTR(modelVar.ptr()), ::SysStringLen(V_BSTR(modelVar.ptr())));
    }
...
TEST_F(SysInfoTest, GoogleChromeOSLsbReleaseTime)
{
    ...
    EXPECT_DOUBLE_EQ(lsb_release_time.InSecondsFSinceUnixEpoch(), parsed_lsb_release_time.InSecondsFSinceUnixEpoch());
}
...
TEST_F(SysInfoTest, ScopedChromeOSVersionInfoDoesNotChangeEnvironment)
{
    ...
    {
        const char kLsbRelease[] = "CHROMEOS_RELEASE_NAME=Chrome OS\n"
                                   "CHROMEOS_RELEASE_VERSION=1.2.3.4\n";
        test::ScopedChromeOSVersionInfo version(kLsbRelease, Time());
    }
