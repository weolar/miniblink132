...
{
    static const uint64_t threshold = base::saturated_cast<uint64_t>(features::kLowMemoryDeviceThresholdMB.Get());
    return threshold;
}
...
uint64_t SysInfo::AmountOfPhysicalMemory()
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableLowEndDeviceMode)) {
        ...
        return std::min(kSimulatedMemoryForEnableLowEndDeviceMode, AmountOfPhysicalMemoryImpl());
    }
...
uint64_t SysInfo::AmountOfAvailablePhysicalMemory()
{
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableLowEndDeviceMode)) {
        ...
        uint64_t memory_used = AmountOfPhysicalMemoryImpl() - AmountOfAvailablePhysicalMemoryImpl();
...
bool IsPartialLowEndModeOnMidRangeDevicesEnabled()
{
    ...
    return SysInfo::Is4GbOr6GbDevice() && base::FeatureList::IsEnabled(features::kPartialLowEndModeOnMidRangeDevices);
}

bool IsPartialLowEndModeOn3GbDevicesEnabled()
{
    return SysInfo::Is3GbDevice() && base::FeatureList::IsEnabled(features::kPartialLowEndModeOn3GbDevices);
}
...
{
    return GetCachedSystemRamBucketizedSize() == BucketizedSize::k4Gb || GetCachedSystemRamBucketizedSize() == BucketizedSize::k6Gb;
}
...
{
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS)
    return base::SysInfo::IsLowEndDevice() || IsPartialLowEndModeOnMidRangeDevicesEnabled() || IsPartialLowEndModeOn3GbDevicesEnabled();
#else
    return base::SysInfo::IsLowEndDevice();
#endif
}

bool SysInfo::IsLowEndDeviceOrPartialLowEndModeEnabled(const FeatureParam<bool>& param_for_exclusion)
{
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS)
    return base::SysInfo::IsLowEndDevice()
        || ((IsPartialLowEndModeOnMidRangeDevicesEnabled() || IsPartialLowEndModeOn3GbDevicesEnabled()) && !param_for_exclusion.Get());
#else
    return base::SysInfo::IsLowEndDevice();
#endif
}
...
bool DetectLowEndDevice()
{
    ...
    return ram_size_mb > 0 && static_cast<uint64_t>(ram_size_mb) <= GetLowMemoryDeviceThresholdMB();
}
...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_WIN) && !BUILDFLAG(IS_CHROMEOS)
...
void SysInfo::GetHardwareInfo(base::OnceCallback<void(HardwareInfo)> callback)
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, kTraits, base::BindOnce(&GetHardwareInfoSync), std::move(callback));
}
...
// static
std::optional<uint64_t> SysInfo::SetAmountOfPhysicalMemoryMbForTesting(const uint64_t amount_of_memory_mb)
{
