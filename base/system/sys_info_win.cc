...
{
    const DWORD kReservedSize = sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX) * 64;
    absl::InlinedVector<BYTE, kReservedSize> buffer;
    buffer.resize(kReservedSize);
    DWORD byte_length = kReservedSize;
    if (!GetLogicalProcessorInformationEx(RelationProcessorCore, reinterpret_cast<SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX*>(buffer.data()), &byte_length)) {
    ...
    while (byte_ptr < buffer.data() + byte_length) {
        const auto* structure_ptr = reinterpret_cast<SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX*>(byte_ptr);
        DCHECK_EQ(structure_ptr->Relationship, RelationProcessorCore);
        DCHECK_LE(&structure_ptr->Processor.EfficiencyClass + sizeof(structure_ptr->Processor.EfficiencyClass), buffer.data() + byte_length);
        efficiency_classes.push_back(structure_ptr->Processor.EfficiencyClass);
        DCHECK_GE(structure_ptr->Size, offsetof(std::remove_pointer_t<decltype(structure_ptr)>, Processor) + sizeof(structure_ptr->Processor));
...
std::vector<uint64_t> GetCoreProcessorMasks()
{
    ...
    absl::InlinedVector<SYSTEM_LOGICAL_PROCESSOR_INFORMATION, kReservedSize> buffer;
    buffer.resize(kReservedSize);
    DWORD byte_length = sizeof(buffer[0]) * kReservedSize;
    const BOOL result = GetLogicalProcessorInformation(buffer.data(), &byte_length);
...
bool GetDiskSpaceInfo(const base::FilePath& path, int64_t* available_bytes, int64_t* total_bytes)
{
...
int SysInfo::NumberOfEfficientProcessorsImpl()
{
    ...
    auto [min_efficiency_class_it, max_efficiency_class_it] = std::minmax_element(efficiency_classes.begin(), efficiency_classes.end());
...
std::string SysInfo::OperatingSystemVersion()
{
    ...
    std::string version(StringPrintf("%d.%d.%d", version_number.major, version_number.minor, version_number.build));
...
std::string ReadHardwareInfoFromRegistry(const wchar_t* reg_value_name)
{
    ...
    for (const wchar_t* system_info_reg_key_path : kSystemInfoRegKeyPaths) {
        ...
        if (system_information_key.Open(HKEY_LOCAL_MACHINE, system_info_reg_key_path, KEY_READ) == ERROR_SUCCESS) {
            if ((system_information_key.ReadValue(reg_value_name, &value) == ERROR_SUCCESS) && !value.empty()) {
                break;
            }
        }
    }
...
{
    HardwareInfo info = { ReadHardwareInfoFromRegistry(L"SystemManufacturer"), SysInfo::HardwareModelName() };
    return info;
}
