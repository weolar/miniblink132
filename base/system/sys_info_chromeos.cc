...
public:
    ...
    void GetVersionNumbers(int32_t* major_version, int32_t* minor_version, int32_t* bugfix_version)
    {
    ...
    void set_lsb_release_time(const Time& time)
    {
        lsb_release_time_ = time;
    }

    bool is_running_on_chromeos() const
    {
        return is_running_on_chromeos_;
    }
...
{
    return GetChromeOSVersionInfo().GetVersionNumbers(major_version, minor_version, bugfix_version);
}
...
// static
void SysInfo::SetChromeOSVersionInfoForTest(const std::string& lsb_release, const Time& lsb_release_time)
{
    DCHECK(!g_chromeos_version_info_for_test) << "Nesting is not allowed";
    g_chromeos_version_info_for_test = new ChromeOSVersionInfo(ChromeOSVersionInfo::FOR_TEST);
...
SysInfo::HardwareInfo SysInfo::GetHardwareInfoSync()
{
    ...
    if (IsRunningOnChromeOS()) {
        ...
        std::string data;
        if (ReadFileToStringWithMaxSize(FilePath(kModelNamePath), &data, kMaxStringSize)) {
