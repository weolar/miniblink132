...
bool GetDiskSpaceInfo(const FilePath& path, int64_t* available_bytes, int64_t* total_bytes)
{
...
int64_t GetAmountOfTotalDiskSpaceAndVolumePath(const FilePath& path, FilePath* volume_path)
{
    if (!path.IsAbsolute()) {
        return -1;
    }
...
int64_t SysInfo::AmountOfFreeDiskSpace(const FilePath& path)
{
    ...
    const int64_t total_space = GetAmountOfTotalDiskSpaceAndVolumePath(path, &volume_path);
    if (total_space >= 0) {
