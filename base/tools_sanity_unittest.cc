...
#define HARMFUL_ACCESS(action, error_regexp) EXPECT_DEATH_IF_SUPPORTED(action, error_regexp)
#elif BUILDFLAG(IS_HWASAN)
#define HARMFUL_ACCESS(action, error_regexp) EXPECT_DEATH(action, "tag-mismatch")
#else
...
TEST(ToolsSanityTest, AccessesToStack)
{
    ...
    HARMFUL_ACCESS(ReadValueOutOfArrayBoundsLeft(foo), "underflows this variable");
    HARMFUL_ACCESS(ReadValueOutOfArrayBoundsRight(foo, 16), "overflows this variable");
    HARMFUL_ACCESS(WriteValueOutOfArrayBoundsLeft(foo), "underflows this variable");
    HARMFUL_ACCESS(WriteValueOutOfArrayBoundsRight(foo, 16), "overflows this variable");
}
...
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_WIN) || BUILDFLAG(IS_FUCHSIA)
#define MAYBE_SingleElementDeletedWithBraces DISABLED_SingleElementDeletedWithBraces
...
{
    bool shared = false;
    ABSL_ANNOTATE_BENIGN_RACE(&shared, "Intentional race - make sure doesn't show up");
    TOOLS_SANITY_TEST_CONCURRENT_THREAD thread1(&shared), thread2(&shared);
    RunInParallel(&thread1, &thread2);
    EXPECT_TRUE(shared);
}
