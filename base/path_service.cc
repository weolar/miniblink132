...
#define ENABLE_BEHAVIOUR_OVERRIDE_PROVIDER ((BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_ANDROID)) || BUILDFLAG(IS_WIN))
...
bool LockedGetFromCache(int key, const PathData* path_data, FilePath* result) EXCLUSIVE_LOCKS_REQUIRED(path_data->lock)
{
...
bool LockedGetFromOverrides(int key, PathData* path_data, FilePath* result) EXCLUSIVE_LOCKS_REQUIRED(path_data->lock)
{
...
// static
bool PathService::OverrideAndCreateIfNeeded(int key, const FilePath& path, bool is_absolute, bool create)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
...
// static
void PathService::RegisterProvider(ProviderFunc func, int key_start, int key_end)
{
    PathData* path_data = GetPathData();
    DCHECK(path_data);
