...
std::string GetKeyValueFromOSReleaseFile(const std::string& input, const char* key)
{
...
BASE_EXPORT std::string GetKeyValueFromOSReleaseFileForTesting(const std::string& input, const char* key)
{
#if !BUILDFLAG(IS_CHROMEOS_ASH)
...
pid_t FindThreadIDWithSyscall(pid_t pid, const std::string& expected_data, bool* syscall_supported)
{
    ...
    for (pid_t tid : tids) {
        ...
        while (std::optional<std::string_view> token = tokenizer.GetNextTokenView()) {
            ...
            *ns_pid_supported = true;
            std::vector<std::string_view> split_value_str = SplitStringPiece(token.value(), "\t", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
