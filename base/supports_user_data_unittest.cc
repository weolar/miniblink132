...
TEST_F(SupportsUserDataTest, ClearWorksRecursively)
{
    ...
    supports_user_data.SetUserData(&key, std::make_unique<UsesItself>(&supports_user_data, &key));
...
TEST_F(SupportsUserDataTest, TakeUserData)
{
    ...
    std::unique_ptr<TestSupportsUserData::Data> data1 = supports_user_data.TakeUserData(&key1);
...
TEST_F(SupportsUserDataTest, ReentrantSetUserDataDuringRemoval)
{
    ...
    public:
        explicit ProblematicSet(const void* const key, TestSupportsUserData& supports_user_data)
            : key_(key)
            , supports_user_data_(supports_user_data)
        {
        }
        ...
        {
            supports_user_data_->SetUserData(key_, std::make_unique<ProblematicSet>(key_, *supports_user_data_));
        }
    ...
    };
    {
        ...
        // observe the crash.
        EXPECT_CHECK_DEATH([&] {
            supports_user_data->SetUserData(&kKey, std::make_unique<ProblematicSet>(&kKey, *supports_user_data));
