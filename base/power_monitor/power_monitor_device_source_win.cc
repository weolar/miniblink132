...
void PowerMonitorDeviceSource::PlatformInit()
{
    ...
    speed_limit_observer_ = std::make_unique<base::SequenceBound<SpeedLimitObserverWin>>(
        base::ThreadPool::CreateSequencedTaskRunner({}), BindRepeating(&PowerMonitorSource::ProcessSpeedLimitEvent));
}
...
PowerStateObserver::BatteryPowerStatus PowerMonitorDeviceSource::GetBatteryPowerStatus() const
{
    ...
    return (status.ACLineStatus == 0) ? PowerStateObserver::BatteryPowerStatus::kBatteryPower : PowerStateObserver::BatteryPowerStatus::kExternalPower;
}
...
PowerMonitorDeviceSource::PowerMessageWindow::PowerMessageWindow()
{
    ...
    base::win::InitializeWindowClass(kWindowClassName, &base::win::WrappedWindowProc<PowerMonitorDeviceSource::PowerMessageWindow::WndProcThunk>, 0, 0, 0,
        nullptr, nullptr, nullptr, nullptr, nullptr, &window_class);
    instance_ = window_class.hInstance;
    ATOM clazz = ::RegisterClassEx(&window_class);
    DCHECK(clazz);

    message_hwnd_ = ::CreateWindowEx(WS_EX_NOACTIVATE, kWindowClassName, nullptr, WS_POPUP, 0, 0, 0, 0, nullptr, nullptr, instance_, nullptr);
    if (message_hwnd_) {
        ...
        power_notify_handle_ = ::RegisterSuspendResumeNotification(message_hwnd_, DEVICE_NOTIFY_WINDOW_HANDLE);
    }
}
...
// static
LRESULT CALLBACK PowerMonitorDeviceSource::PowerMessageWindow::WndProcThunk(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    switch (message) {
