...
{
    return AddPowerStateObserverAndReturnBatteryPowerStatus(obs) == PowerStateObserver::BatteryPowerStatus::kBatteryPower;
}

PowerStateObserver::BatteryPowerStatus PowerMonitor::AddPowerStateObserverAndReturnBatteryPowerStatus(PowerStateObserver* obs)
{
...
{
    DCHECK(IsInitialized());
    return GetBatteryPowerStatus() == PowerStateObserver::BatteryPowerStatus::kBatteryPower;
}

PowerStateObserver::BatteryPowerStatus PowerMonitor::GetBatteryPowerStatus() const
{
    DCHECK(IsInitialized());
    AutoLock auto_lock(battery_power_status_lock_);
    return battery_power_status_;
...
{
    DCHECK(IsInitialized());
    NotifyPowerStateChange(on_battery_power ? PowerStateObserver::BatteryPowerStatus::kBatteryPower : PowerStateObserver::BatteryPowerStatus::kExternalPower);
}

void PowerMonitor::NotifyPowerStateChange(PowerStateObserver::BatteryPowerStatus battery_power_status)
{
    DCHECK(IsInitialized());

    if (battery_power_status == PowerStateObserver::BatteryPowerStatus::kUnknown) {
        DVLOG(1) << "PowerStateChange: with unknown value";
    } else {
        DVLOG(1) << "PowerStateChange: " << (battery_power_status == PowerStateObserver::BatteryPowerStatus::kBatteryPower ? "On" : "Off") << " battery";
        TRACE_EVENT_INSTANT("power",
            battery_power_status == PowerStateObserver::BatteryPowerStatus::kBatteryPower ? perfetto::StaticString("PowerMonitor::BatteryPower")
    ...
    if (battery_power_status_ != battery_power_status) {
        ...
        power_state_observers_->Notify(FROM_HERE, &PowerStateObserver::OnBatteryPowerStatusChange, battery_power_status);
    }
}
...
void PowerMonitor::NotifySuspend()
{
    ...
    if (!is_system_suspended_) {
        ...
        power_suspend_observers_->Notify(FROM_HERE, &PowerSuspendObserver::OnSuspend);
    }
}
...
void PowerMonitor::NotifyResume()
{
    ...
    if (is_system_suspended_) {
        ...
        power_suspend_observers_->Notify(FROM_HERE, &PowerSuspendObserver::OnResume);
    }
}
...
void PowerMonitor::NotifyThermalStateChange(PowerThermalObserver::DeviceThermalState new_state)
{
    ...
    if (power_thermal_state_ != new_state) {
        ...
        thermal_state_observers_->Notify(FROM_HERE, &PowerThermalObserver::OnThermalStateChange, new_state);
    }
}
...
void PowerMonitor::NotifySpeedLimitChange(int speed_limit)
{
    ...
    if (speed_limit_ != speed_limit) {
        ...
        thermal_state_observers_->Notify(FROM_HERE, &PowerThermalObserver::OnSpeedLimitChange, speed_limit);
    }
}
