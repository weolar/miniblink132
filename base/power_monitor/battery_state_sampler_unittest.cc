...
public:
    ...
    void GetBatteryState(OnceCallback<void(const std::optional<BatteryState>&)> callback) override
    {
...
public:
    ...
    void GetBatteryState(OnceCallback<void(const std::optional<BatteryState>&)> callback) override
    {
        ...
        SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
            BindLambdaForTesting([callback = std::move(callback), battery_state = next_battery_state]() mutable { std::move(callback).Run(battery_state); }));
    }
};
...
public:
    ...
    void OnBatteryStateSampled(const std::optional<BatteryLevelProvider::BatteryState>& battery_state) override
    {
...
// matter.
const std::optional<BatteryLevelProvider::BatteryState> kTestBatteryState1 = BatteryLevelProvider::BatteryState { .battery_count = 1,
...
const std::optional<BatteryLevelProvider::BatteryState> kTestBatteryState2 = BatteryLevelProvider::BatteryState { .battery_count = 1,
    .is_external_power_connected = false,
    .current_capacity = 5000,
    .full_charged_capacity = 10000,
    .charge_unit = BatteryLevelProvider::BatteryLevelUnit::kMWh };
const std::optional<BatteryLevelProvider::BatteryState> kTestBatteryState3 = BatteryLevelProvider::BatteryState { .battery_count = 1,
    .is_external_power_connected = true,
    .current_capacity = 2000,
    .full_charged_capacity = 10000,
    .charge_unit = BatteryLevelProvider::BatteryLevelUnit::kMWh };

bool operator==(const BatteryLevelProvider::BatteryState& lhs, const BatteryLevelProvider::BatteryState& rhs)
{
    return std::tie(lhs.battery_count, lhs.is_external_power_connected, lhs.current_capacity, lhs.full_charged_capacity, lhs.charge_unit)
        == std::tie(rhs.battery_count, rhs.is_external_power_connected, rhs.current_capacity, rhs.full_charged_capacity, rhs.charge_unit);
}
...
TEST(BatteryStateSamplerTest, GlobalInstance)
{
    ...
    // Create the sampler.
    auto battery_state_sampler = std::make_unique<BatteryStateSampler>(std::make_unique<test::TestSamplingEventSource>(), std::move(battery_level_provider));
    ...
    EXPECT_DCHECK_DEATH({
        BatteryStateSampler another_battery_state_sampler(std::make_unique<test::TestSamplingEventSource>(), std::make_unique<TestBatteryLevelProvider>());
    });
...
{
    auto sampling_event_source = std::make_unique<test::TestSamplingEventSource>();

    auto battery_level_provider = std::make_unique<TestBatteryLevelProvider>();
    // Push the initial battery state that will be queried by the sampler.
    battery_level_provider->PushBatteryState(kTestBatteryState1);

    BatteryStateSampler battery_state_sampler(std::move(sampling_event_source), std::move(battery_level_provider));
...
TEST(BatteryStateSamplerTest, InitialSample_Async)
{
    ...
    auto battery_level_provider = std::make_unique<TestBatteryLevelProviderAsync>();
