...
std::optional<CpuThroughputEstimationResult> EstimateCpuThroughput()
{
    ...
    for (int i = 0; i < kAmountOfIterations; ++i) {
        __asm__ __volatile__("addl  %%eax, %%eax\n"
                             "addl  %%eax, %%eax\n"
                             "addl  %%eax, %%eax\n"
                             "addl  %%eax, %%eax\n"
                             "addl  %%eax, %%eax\n"
    ...
    const double estimated_frequency = (kAmountOfIterations * kAmountOfInstructions) / elapsed.InSecondsF();
...
BASE_EXPORT CpuFrequencyInfo GetCpuFrequencyInfo()
{
    ...
    if (!NT_SUCCESS(CallNtPowerInformation(ProcessorInformation, nullptr, 0, &info[0], static_cast<ULONG>(sizeof(PROCESSOR_POWER_INFORMATION) * num_cpu)))) {
        return cpu_info;
    }
...
void GenerateCpuInfoForTracingMetadata(base::Value::Dict* metadata)
{
    ...
    for (const auto& i : info) {
        ...
        metadata->Set(base::StringPrintf("cpu-max-frequency-core%lu", cpu_number), static_cast<int>(i.MaxMhz));
        ...
        if (i.MhzLimit != i.MaxMhz) {
            metadata->Set(base::StringPrintf("cpu-limit-frequency-core%lu", cpu_number), static_cast<int>(i.MhzLimit));
        }

        // The MaxIdleState field contains the maximum supported C-state. The value
        // is zero when the C-State is not supported.
        if (i.MaxIdleState != 0) {
            metadata->Set(base::StringPrintf("cpu-max-idle-state-core%lu", cpu_number), static_cast<int>(i.MaxIdleState));
        }
    }
}
#endif
