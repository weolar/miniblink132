...
public:
    ...
    static BatteryState MakeBatteryState(const std::vector<BatteryDetails>& battery_details)
    {
...
{
    auto state = FakeBatteryLevelProvider::MakeBatteryState(
        { BatteryDetails({ .is_external_power_connected = true, .current_capacity = 42, .full_charged_capacity = 100 }) });
...
{
    auto state = FakeBatteryLevelProvider::MakeBatteryState(
        { BatteryDetails({ .is_external_power_connected = false, .current_capacity = 42, .full_charged_capacity = 100 }) });
    EXPECT_EQ(1, state.battery_count);
...
TEST(BatteryLevelProviderTest, MultipleBatteriesWithExternalPower)
{
    auto state = FakeBatteryLevelProvider::MakeBatteryState(
        { BatteryDetails({ .is_external_power_connected = false, .current_capacity = 42, .full_charged_capacity = 100 }),
            BatteryDetails({ .is_external_power_connected = true, .current_capacity = 10, .full_charged_capacity = 100 }) });
...
TEST(BatteryLevelProviderTest, MultipleBatteriesDischarging)
{
    auto state = FakeBatteryLevelProvider::MakeBatteryState(
        { BatteryDetails({ .is_external_power_connected = false, .current_capacity = 42, .full_charged_capacity = 100 }),
            BatteryDetails({ .is_external_power_connected = false, .current_capacity = 10, .full_charged_capacity = 100 }) });
    EXPECT_EQ(2, state.battery_count);
    EXPECT_FALSE(state.is_external_power_connected);
    EXPECT_EQ(std::nullopt, state.current_capacity);
    EXPECT_EQ(std::nullopt, state.full_charged_capacity);
...
{
    auto state = FakeBatteryLevelProvider::MakeBatteryState({ BatteryDetails({ .is_external_power_connected = false,
        .current_capacity = 42,
