...
bool GetCPUIdleness(int* idleness_percent)
{
    ...
    if (!NT_SUCCESS(CallNtPowerInformation(SystemPowerInformation, nullptr, 0, info.get(), sizeof(SYSTEM_POWER_INFORMATION)))) {
        *idleness_percent = 0;
        return false;
    }
...
SpeedLimitObserverWin::SpeedLimitObserverWin(SpeedLimitUpdateCallback speed_limit_update_callback)
    ...
    , moving_average_(kMovingAverageWindowSize)
{
    ...
    timer_.Start(FROM_HERE, kSampleInterval, this, &SpeedLimitObserverWin::OnTimerTick);
}
...
int SpeedLimitObserverWin::GetCurrentSpeedLimit() const
{
    ...
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(kPowerTraceCategory, &trace_events_enabled);
    if (trace_events_enabled) {
        TRACE_COUNTER(kPowerTraceCategory, "idleness", idleness_percent);
        TRACE_COUNTER(kPowerTraceCategory, "throttling_level", static_cast<unsigned int>(throttling_level * 100));

#if defined(ARCH_CPU_X86_FAMILY)
        double cpu_frequency = EstimateCpuFrequency();
        TRACE_COUNTER(kPowerTraceCategory, "frequency_mhz", static_cast<unsigned int>(cpu_frequency / 1'000'000));
#endif
    }
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
    ...
    int speed_limit = static_cast<int>(std::ceil(kSpeedLimitMax * speed_limit_factor));
...
#if BUILDFLAG(ENABLE_BASE_TRACING)
    TRACE_COUNTER(kPowerTraceCategory, "speed_limit", static_cast<unsigned int>(speed_limit));
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
}
...
float SpeedLimitObserverWin::EstimateThrottlingLevel() const
{
    ...
    if (!NT_SUCCESS(CallNtPowerInformation(ProcessorInformation, nullptr, 0, &info[0], static_cast<ULONG>(sizeof(PROCESSOR_POWER_INFORMATION) * num_cpus())))) {
    ...
    for (size_t i = 0; i < num_cpus(); ++i) {
        ...
        const float load_fraction = static_cast<float>(idle_diff) / info[i].MaxIdleState;
    ...
    DCHECK_LE(load_fraction_total, static_cast<float>(num_non_idle_cpus))
        << " load_fraction_total: " << load_fraction_total << " num_non_idle_cpus:" << num_non_idle_cpus;
