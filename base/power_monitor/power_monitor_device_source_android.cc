...
// A helper function which is a friend of PowerMonitorSource.
void ProcessThermalEventHelper(PowerThermalObserver::DeviceThermalState new_thermal_state)
{
...
PowerThermalObserver::DeviceThermalState MapToDeviceThermalState(int android_thermal_status)
{
...
PowerStateObserver::BatteryPowerStatus PowerMonitorDeviceSource::GetBatteryPowerStatus() const
{
    ...
    int battery_power = base::android::Java_PowerMonitor_getBatteryPowerStatus(env);
...
{
    JNIEnv* env = jni_zero::AttachCurrentThread();
    return android::MapToDeviceThermalState(android::Java_PowerMonitor_getCurrentThermalStatus(env));
}
