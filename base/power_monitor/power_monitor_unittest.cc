...
TEST_F(PowerMonitorTest, PowerNotifications)
{
    ...
    source().GeneratePowerStateEvent(PowerStateObserver::BatteryPowerStatus::kBatteryPower);
    EXPECT_EQ(observers[0].power_state_changes(), 1);
    EXPECT_EQ(observers[0].last_power_status(), PowerStateObserver::BatteryPowerStatus::kBatteryPower);
    ...
    source().GeneratePowerStateEvent(PowerStateObserver::BatteryPowerStatus::kExternalPower);
    EXPECT_EQ(observers[0].power_state_changes(), 2);
    EXPECT_EQ(observers[0].last_power_status(), PowerStateObserver::BatteryPowerStatus::kExternalPower);
    ...
    source().GenerateThermalThrottlingEvent(PowerThermalObserver::DeviceThermalState::kNominal);
    EXPECT_EQ(observers[0].thermal_state_changes(), 1);
    EXPECT_EQ(observers[0].last_thermal_state(), PowerThermalObserver::DeviceThermalState::kNominal);
    ...
    source().GenerateThermalThrottlingEvent(PowerThermalObserver::DeviceThermalState::kFair);
    EXPECT_EQ(observers[0].thermal_state_changes(), 2);
    EXPECT_EQ(observers[0].last_thermal_state(), PowerThermalObserver::DeviceThermalState::kFair);
...
TEST_F(PowerMonitorTest, ThermalThrottling)
{
    ...
    constexpr PowerThermalObserver::DeviceThermalState kThermalStates[] = { PowerThermalObserver::DeviceThermalState::kUnknown,
        PowerThermalObserver::DeviceThermalState::kNominal, PowerThermalObserver::DeviceThermalState::kFair, PowerThermalObserver::DeviceThermalState::kSerious,
...
TEST_F(PowerMonitorTest, SuspendStateReturnedFromAddObserver)
{
    ...
    EXPECT_FALSE(power_monitor->AddPowerSuspendObserverAndReturnSuspendedState(&observer1));

    source().GenerateSuspendEvent();

    EXPECT_TRUE(power_monitor->AddPowerSuspendObserverAndReturnSuspendedState(&observer2));
...
TEST_F(PowerMonitorTest, PowerStateReturnedFromAddObserver)
{
    ...
    EXPECT_NE(power_monitor->AddPowerStateObserverAndReturnBatteryPowerStatus(&observer1), PowerStateObserver::BatteryPowerStatus::kBatteryPower);

    source().GeneratePowerStateEvent(PowerStateObserver::BatteryPowerStatus::kBatteryPower);

    // An observer is added after the on-battery notification.
    EXPECT_EQ(power_monitor->AddPowerStateObserverAndReturnBatteryPowerStatus(&observer2), PowerStateObserver::BatteryPowerStatus::kBatteryPower);
