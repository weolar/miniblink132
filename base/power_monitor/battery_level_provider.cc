...
BatteryLevelProvider::BatteryState BatteryLevelProvider::MakeBatteryState(const std::vector<BatteryDetails>& battery_details)
{
    BatteryState state;

    state.battery_count = static_cast<int>(battery_details.size());
    state.is_external_power_connected = battery_details.size() == 0
        || base::ranges::any_of(battery_details, [](const BatteryDetails& details) { return details.is_external_power_connected; });
...
#if BUILDFLAG(IS_WIN)
        state.battery_discharge_granularity = battery_details.front().battery_discharge_granularity;
