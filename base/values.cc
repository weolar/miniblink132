...
constexpr auto kTypeNames = std::to_array<const char*>({ "null", "boolean", "integer", "double", "string", "binary", "dictionary", "list" });
static_assert(kTypeNames.size() == static_cast<size_t>(Value::Type::LIST) + 1, "kTypeNames Has Wrong Size");
...
class PathSplitter {
public:
    explicit PathSplitter(std::string_view path)
        : path_(path)
    {
    }
...
public:
    ...
    template <typename T> static const T& UnwrapReference(std::reference_wrapper<const T> value)
    {
        return value.get();
    }

    template <typename T> static const T& UnwrapReference(const T& value)
    {
        return value;
    }

    // Returns a new Value object using the contents of the |storage| variant.
    template <typename Storage> static Value Clone(const Storage& storage)
    {
        return absl::visit(
            [](const auto& member) {
                const auto& value = UnwrapReference(member);
                using T = std::decay_t<decltype(value)>;
                if constexpr (std::is_same_v<T, Value::Dict> || std::is_same_v<T, Value::List>) {
...
Value::Value(const std::vector<uint8_t>& value)
    : Value(base::span<const uint8_t>(value.data(), value.size()))
{
}
...
Value::DoubleStorage::DoubleStorage(double v)
    : v_(bit_cast<decltype(v_)>(v))
{
    if (!std::isfinite(v)) {
        DUMP_WILL_BE_NOTREACHED() << "Non-finite (i.e. NaN or positive/negative infinity) "
...
{
    return (is_int() || is_double()) ? std::make_optional(GetDouble()) : std::nullopt;
}
...
Value::Dict&& Value::Dict::Set(std::string_view key, std::string_view value) &&
{
    return std::move(*this).Set(key, Value(value));
}

Value::Dict&& Value::Dict::Set(std::string_view key, std::u16string_view value) &&
{
    return std::move(*this).Set(key, Value(value));
}
...
std::optional<bool> Value::Dict::FindBoolByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfBool() : std::nullopt;
}

std::optional<int> Value::Dict::FindIntByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfInt() : std::nullopt;
}

std::optional<double> Value::Dict::FindDoubleByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfDouble() : std::nullopt;
}

const std::string* Value::Dict::FindStringByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfString() : nullptr;
}
...
const Value::BlobStorage* Value::Dict::FindBlobByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfBlob() : nullptr;
}
...
const Value::Dict* Value::Dict::FindDictByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfDict() : nullptr;
}
...
const Value::List* Value::Dict::FindListByDottedPath(std::string_view path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfList() : nullptr;
}
...
Value* Value::Dict::SetByDottedPath(std::string_view path, std::string_view value) &
{
    return SetByDottedPath(path, Value(value));
}

Value* Value::Dict::SetByDottedPath(std::string_view path, std::u16string_view value) &
{
    return SetByDottedPath(path, Value(value));
}

Value* Value::Dict::SetByDottedPath(std::string_view path, const char* value) &
{
    return SetByDottedPath(path, Value(value));
}

Value* Value::Dict::SetByDottedPath(std::string_view path, const char16_t* value) &
{
    return SetByDottedPath(path, Value(value));
}

Value* Value::Dict::SetByDottedPath(std::string_view path, std::string&& value) &
{
    return SetByDottedPath(path, Value(std::move(value)));
}

Value* Value::Dict::SetByDottedPath(std::string_view path, BlobStorage&& value) &
{
    return SetByDottedPath(path, Value(std::move(value)));
}
...
Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, Value&& value) &&
{
...
Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, int value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, double value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, std::string_view value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, std::u16string_view value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, const char* value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, const char16_t* value) &&
{
    SetByDottedPath(path, Value(value));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, std::string&& value) &&
{
    SetByDottedPath(path, Value(std::move(value)));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, BlobStorage&& value) &&
{
    SetByDottedPath(path, Value(std::move(value)));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, Dict&& value) &&
{
    SetByDottedPath(path, Value(std::move(value)));
    return std::move(*this);
}

Value::Dict&& Value::Dict::SetByDottedPath(std::string_view path, List&& value) &&
{
    SetByDottedPath(path, Value(std::move(value)));
    return std::move(*this);
}
...
std::optional<Value> Value::Dict::ExtractByDottedPath(std::string_view path)
{
    ...
    std::optional<Value> extracted = next_dict->ExtractByDottedPath(path.substr(dot_index + 1));
...
Value::Dict::Dict(const flat_map<std::string, std::unique_ptr<Value>>& storage)
{
...
{
    auto deref_2nd = [](const auto& p) { return std::tie(p.first, *p.second); };
    return ranges::lexicographical_compare(lhs.storage_, rhs.storage_, {}, deref_2nd, deref_2nd);
}
...
Value::List::iterator Value::List::begin()
{
    ...
    return UNSAFE_BUFFERS(iterator(base::to_address(storage_.begin()), base::to_address(storage_.end())));
}

Value::List::const_iterator Value::List::begin() const
{
    // SAFETY: Both iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.begin()), base::to_address(storage_.end())));
}

Value::List::const_iterator Value::List::cbegin() const
{
    // SAFETY: Both iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.cbegin()), base::to_address(storage_.cend())));
}

Value::List::iterator Value::List::end()
{
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()), base::to_address(storage_.end())));
}

Value::List::const_iterator Value::List::end() const
{
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()), base::to_address(storage_.end())));
}

Value::List::const_iterator Value::List::cend() const
{
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.cbegin()), base::to_address(storage_.cend()), base::to_address(storage_.cend())));
}
...
Value::List::iterator Value::List::erase(iterator pos)
{
    ...
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end())));
}
...
Value::List::const_iterator Value::List::erase(const_iterator pos)
{
    auto next_it = storage_.erase(storage_.begin() + (pos - begin()));
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end())));
}

Value::List::iterator Value::List::erase(iterator first, iterator last)
{
    auto next_it = storage_.erase(storage_.begin() + (first - begin()), storage_.begin() + (last - begin()));
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end())));
}

Value::List::const_iterator Value::List::erase(const_iterator first, const_iterator last)
{
    auto next_it = storage_.erase(storage_.begin() + (first - begin()), storage_.begin() + (last - begin()));
    // SAFETY: All iterators point to a single allocation.
    return UNSAFE_BUFFERS(const_iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end())));
...
{
    auto inserted_it = storage_.insert(storage_.begin() + (pos - begin()), std::move(value));
    // SAFETY: All pointers point to a single allocation.
    return UNSAFE_BUFFERS(iterator(base::to_address(storage_.begin()), base::to_address(inserted_it), base::to_address(storage_.end())));
}
...
ValueView::ValueView(const Value& value)
    : data_view_(value.Visit([](const auto& member) { return ViewType(member); }))
{
}
...
{
    if (static_cast<int>(type) < 0 || static_cast<size_t>(type) >= std::size(kTypeNames)) {
