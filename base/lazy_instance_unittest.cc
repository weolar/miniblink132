...
public:
    ...
    ConstructAndDestructLogger& operator=(const ConstructAndDestructLogger&) = delete;
    ~ConstructAndDestructLogger()
    {
...
public:
    explicit SlowDelegate(base::LazyInstance<SlowConstructor>::DestructorAtExit* lazy)
        : lazy_(lazy)
    {
    }
...
base::LazyInstance<ConstructAndDestructLogger>::DestructorAtExit lazy_logger = LAZY_INSTANCE_INITIALIZER;
...
base::LazyInstance<SlowConstructor>::DestructorAtExit lazy_slow = LAZY_INSTANCE_INITIALIZER;
...
TEST(LazyInstanceTest, LeakyLazyInstance)
{
    ...
    {
        base::ShadowingAtExitManager shadow;
        static base::LazyInstance<DeleteLogger>::DestructorAtExit test = LAZY_INSTANCE_INITIALIZER;
    ...
    {
        base::ShadowingAtExitManager shadow;
        static base::LazyInstance<DeleteLogger>::Leaky test = LAZY_INSTANCE_INITIALIZER;
...
template <size_t alignment> class AlignedData {
public:
    AlignedData() = default;
    ~AlignedData() = default;
...
TEST(LazyInstanceTest, Alignment)
{
    ...
    static LazyInstance<AlignedData<4>>::DestructorAtExit align4 = LAZY_INSTANCE_INITIALIZER;
    static LazyInstance<AlignedData<32>>::DestructorAtExit align32 = LAZY_INSTANCE_INITIALIZER;
    static LazyInstance<AlignedData<4096>>::DestructorAtExit align4096 = LAZY_INSTANCE_INITIALIZER;
...
public:
    ...
    bool done_construction() const
    {
        return done_construction_;
    }
...
public:
    BlockingConstructorThread(base::ThreadType thread_type, base::LazyInstance<BlockingConstructor>::DestructorAtExit* lazy, base::OnceClosure before_get)
...
base::LazyInstance<BlockingConstructor>::DestructorAtExit lazy_blocking = LAZY_INSTANCE_INITIALIZER;
...
TEST(LazyInstanceTest, PriorityInversionAtInitializationResolves)
{
    ...
    BlockingConstructorThread background_getter(base::ThreadType::kBackground, &lazy_blocking, base::OnceClosure());
    ...
    base::RepeatingClosure foreground_thread_ready_callback
        = base::BarrierClosure(kNumForegroundThreads, base::BindOnce(&BlockingConstructor::CompleteConstructionNow));
    for (int i = 0; i < kNumForegroundThreads; ++i) {
        foreground_threads.push_back(std::make_unique<BlockingConstructorThread>(base::ThreadType::kDefault, &lazy_blocking, foreground_thread_ready_callback));
