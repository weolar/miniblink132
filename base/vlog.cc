...
// static
std::vector<VlogInfo::VmodulePattern> VlogInfo::ParseVmoduleLevels(const std::string& vmodule_switch)
{
    ...
    for (const auto& pair : kv_pairs) {
        ...
        if (!base::StringToInt(pair.second, &pattern.vlog_level)) {
            DLOG(WARNING) << "Parsed vlog level for \"" << pair.first << "=" << pair.second << "\" as " << pattern.vlog_level;
        }
...
VlogInfo::VlogInfo(const std::string& v_switch, const std::string& vmodule_switch, int* min_log_level)
...
int VlogInfo::GetVlogLevel(std::string_view file) const
{
    if (!vmodule_levels_.empty()) {
        ...
        for (const auto& it : vmodule_levels_) {
            std::string_view target((it.match_target == VmodulePattern::MATCH_FILE) ? file : module);
...
VlogInfo::VlogInfo(std::vector<VmodulePattern> vmodule_levels, int* min_log_level)
...
VlogInfo* VlogInfo::WithSwitches(const std::string& vmodule_switch) const
{
    ...
    std::vector<VmodulePattern> additional_vmodule_levels = ParseVmoduleLevels(vmodule_switch);
    vmodule_levels.insert(vmodule_levels.end(), additional_vmodule_levels.begin(), additional_vmodule_levels.end());
