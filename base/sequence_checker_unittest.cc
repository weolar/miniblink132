...
{
    SequenceCheckerImpl sequence_checker;
    RunCallbackThread thread(BindOnce(&ExpectNotCalledOnValidSequence, Unretained(&sequence_checker)));
}
...
TEST(SequenceCheckerTest, DetachFromSequenceNoSequenceToken)
{
    ...
    RunCallbackThread thread(BindOnce(&ExpectCalledOnValidSequence, Unretained(&sequence_checker)));
...
TEST(SequenceCheckerTest, Move)
{
    ...
    // sequence.
    RunCallbackThread thread1(BindOnce(&ExpectCalledOnValidSequence, Unretained(&initial)));
    RunCallbackThread thread2(BindOnce(&ExpectCalledOnValidSequence, Unretained(&move_constructed)));

    // But the latest one shouldn't be able to run on another sequence.
    RunCallbackThread thread(BindOnce(&ExpectNotCalledOnValidSequence, Unretained(&move_assigned)));
...
TEST(SequenceCheckerTest, MoveAssignIntoDetached)
{
    ...
    RunCallbackThread thread2(BindOnce(&ExpectNotCalledOnValidSequence, Unretained(&move_assigned)));
...
TEST(SequenceCheckerTest, MoveFromDetachedRebinds)
{
    ...
    RunCallbackThread thread2(BindOnce(&ExpectNotCalledOnValidSequence, Unretained(&moved_into)));
...
TEST(SequenceCheckerTest, MoveOffSequenceBanned)
{
    ...
    RunCallbackThread thread(BindOnce(&ExpectCalledOnValidSequence, Unretained(&other_sequence)));

    EXPECT_DCHECK_DEATH(SequenceCheckerImpl main_sequence(std::move(other_sequence)));
}
...
TEST(SequenceCheckerMacroTest, Macros)
{
    ...
    EXPECT_DCHECK_DEATH({ DCHECK_CALLED_ON_VALID_SEQUENCE(my_sequence_checker); });
#else
...
TEST(SequenceCheckerTest, FromThreadDestruction)
{
    ...
    ThreadLocalOwnedPointer<SequenceCheckerOwner> thread_local_owner;
    {
        ...
        task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&] { thread_local_owner.Set(std::make_unique<SequenceCheckerOwner>(&other_checker)); }));
        task_runner = nullptr;
...
TEST(SequenceCheckerTest, LockBasic)
{
    ...
    ReleasableAutoLock releasable_auto_lock(&lock, subtle::LockTracking::kEnabled);
    SequenceCheckerImpl sequence_checker;
...
TEST(SequenceCheckerTest, ManyLocks)
{
    ...
    ReleasableAutoLock releasable_auto_lock_a(&lock_a, subtle::LockTracking::kEnabled);
    ReleasableAutoLock releasable_auto_lock_b(&lock_b, subtle::LockTracking::kEnabled);
    ReleasableAutoLock releasable_auto_lock_c(&lock_c, subtle::LockTracking::kEnabled);
    SequenceCheckerImpl sequence_checker;
