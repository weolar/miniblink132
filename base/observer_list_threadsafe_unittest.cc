...
template <RemoveObserverPolicy RemovePolicy = RemoveObserverPolicy::kAnySequence> class AddRemoveThread : public Foo {
    using Self = AddRemoveThread<RemovePolicy>;
    using ObserverList = ObserverListThreadSafe<Foo, RemovePolicy>;

public:
    AddRemoveThread(ObserverList* list, bool notify, scoped_refptr<SingleThreadTaskRunner> removal_task_runner)
        : list_(list)
        , task_runner_(ThreadPool::CreateSingleThreadTaskRunner({}, SingleThreadTaskRunnerThreadMode::DEDICATED))
        , removal_task_runner_(std::move(removal_task_runner))
        , in_list_(false)
        , start_(Time::Now())
        , do_notifies_(notify)
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&Self::AddTask, weak_factory_.GetWeakPtr()));
    }
    ...
    void AddTask()
    {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&Self::AddTask, weak_factory_.GetWeakPtr()));
    }
    ...
    void Observe(int x) override
    {
        ...
        if (removal_task_runner_) {
            ...
            removal_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&Self::RemoveTask, base::Unretained(this)).Then(base::BindOnce(&base::TestWaitableEvent::Signal, base::Unretained(&event))));
            event.Wait();
        } else {
...
{
    test::TaskEnvironment task_environment;
    scoped_refptr<ObserverListThreadSafe<Foo>> observer_list(new ObserverListThreadSafe<Foo>);
...
template <RemoveObserverPolicy RemovePolicy = RemoveObserverPolicy::kAnySequence>
static void ThreadSafeObserverHarness(int num_threads, bool cross_thread_notifies = false, bool cross_thread_removes = false)
{
    test::TaskEnvironment task_environment;

    auto observer_list = base::MakeRefCounted<ObserverListThreadSafe<Foo, RemovePolicy>>();
    ...
    for (int index = 0; index < num_threads; index++) {
        auto add_remove_thread = std::make_unique<TestThread>(observer_list.get(), cross_thread_notifies, std::move(removal_task_runner));
...
TEST(ObserverListThreadSafeTest, CrossThreadRemovalRestricted)
{
    ...
    // restriction.
    ThreadSafeObserverHarness<RemoveObserverPolicy::kAddingSequenceOnly>(3, /*cross_thread_notifies=*/true, /*cross_thread_removes=*/false);
}
...
TEST(ObserverListThreadSafeDeathTest, CrossThreadRemovalRestricted)
{
    ...
    EXPECT_CHECK_DEATH(ThreadSafeObserverHarness<RemoveObserverPolicy::kAddingSequenceOnly>(3, /*cross_thread_notifies=*/true, /*cross_thread_removes=*/true));
}
...
{
    std::optional<test::TaskEnvironment> task_environment(std::in_place);
    auto observer_list = base::MakeRefCounted<ObserverListThreadSafe<Foo, RemoveObserverPolicy::kAddingSequenceOnly>>();
...
public:
    explicit SequenceVerificationObserver(scoped_refptr<SequencedTaskRunner> task_runner)
        : task_runner_(std::move(task_runner))
    {
    }
    SequenceVerificationObserver(const SequenceVerificationObserver&) = delete;
    SequenceVerificationObserver& operator=(const SequenceVerificationObserver&) = delete;
    ...
    bool called_on_valid_sequence() const
    {
        return called_on_valid_sequence_;
    }
...
TEST(ObserverListThreadSafeTest, NotificationOnValidSequence)
{
    ...
    task_runner_1->PostTask(FROM_HERE, BindOnce(base::IgnoreResult(&ObserverListThreadSafe<Foo>::AddObserver), observer_list, Unretained(&observer_1)));
    task_runner_2->PostTask(FROM_HERE, BindOnce(base::IgnoreResult(&ObserverListThreadSafe<Foo>::AddObserver), observer_list, Unretained(&observer_2)));
...
public:
    RemoveWhileNotificationIsRunningObserver()
        : notification_running_(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED)
        , barrier_(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    RemoveWhileNotificationIsRunningObserver(const RemoveWhileNotificationIsRunningObserver&) = delete;
    RemoveWhileNotificationIsRunningObserver& operator=(const RemoveWhileNotificationIsRunningObserver&) = delete;
    ...
    void WaitForNotificationRunning()
    {
        notification_running_.Wait();
    }
    void Unblock()
    {
        barrier_.Signal();
...
TEST(ObserverListThreadSafeTest, RemoveWhileNotificationIsRunning)
{
    ...
    WaitableEvent task_running(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    WaitableEvent barrier(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    ...
    ThreadPool::CreateSequencedTaskRunner({ MayBlock() })
        ->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&ObserverListThreadSafe<Foo>::AddObserver), observer_list, Unretained(&observer)));
    ThreadPoolInstance::Get()->FlushForTesting();
...
{
    test::TaskEnvironment task_environment;
    scoped_refptr<ObserverListThreadSafe<Foo>> observer_list(new ObserverListThreadSafe<Foo>(ObserverListPolicy::EXISTING_ONLY));
    Adder a(1);
