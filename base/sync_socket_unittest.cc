...
public:
    explicit HangingReceiveThread(SyncSocket* socket, bool with_timeout)
        ...
        , started_event_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
        , done_event_(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED)
    {
        thread_.Start();
    }
    ...
    void Run() override
    {
        ...
        if (with_timeout_) {
            ASSERT_EQ(0u, socket_->ReceiveWithTimeout(byte_span_from_ref(data), kReceiveTimeout));
        } else {
...
TEST_F(CancelableSyncSocketTest, ReceiveWithTimeoutAfterShutdown)
{
    ...
    int data = 0;
    EXPECT_EQ(0u, socket_a_.ReceiveWithTimeout(byte_span_from_ref(data), kReceiveTimeout));
