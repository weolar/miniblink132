...
template <typename Type> inline bool PickleIterator::ReadBuiltinType(Type* result)
{
    static_assert(std::is_integral_v<Type> && !std::is_same_v<Type, bool>, "This method is only safe with to use with types without padding bits.");
...
template <typename Type> inline const char* PickleIterator::GetReadPointerAndAdvance()
{
...
inline const char* PickleIterator::GetReadPointerAndAdvance(size_t num_elements, size_t size_element)
{
...
bool PickleIterator::ReadStringPiece16(std::u16string_view* result)
{
    ...
    *result = std::u16string_view(reinterpret_cast<const char16_t*>(read_from), len);
    return true;
}
...
Pickle::Pickle()
    ...
    , write_offset_(0)
{
    static_assert(std::has_single_bit(Pickle::kPayloadUnit), "Pickle::kPayloadUnit must be a power of two");
    Resize(kPayloadUnit);
...
bool Pickle::ReadAttachment(base::PickleIterator* iter, scoped_refptr<Attachment>* attachment) const
{
    return false;
}
...
// static
bool Pickle::PeekNext(size_t header_size, const char* start, const char* end, size_t* pickle_size)
{
