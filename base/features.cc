...
BASE_FEATURE(kFeatureParamWithCache, "FeatureParamWithCache", FEATURE_ENABLED_BY_DEFAULT);
...
BASE_FEATURE(kUseRustJsonParser, "UseRustJsonParser",
#if BUILDFLAG(IS_ANDROID)
...
// pool.
const base::FeatureParam<bool> kUseRustJsonParserInCurrentSequence { &kUseRustJsonParser, "UseRustJsonParserInCurrentSequence", false };
...
#endif
BASE_FEATURE(kLowEndMemoryExperiment, "LowEndMemoryExperiment", FEATURE_DISABLED_BY_DEFAULT);
const base::FeatureParam<int> kLowMemoryDeviceThresholdMB { &kLowEndMemoryExperiment, "LowMemoryDeviceThresholdMB", LOW_MEMORY_DEVICE_THRESHOLD_MB };

#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS)
// Force to enable LowEndDeviceMode partially on Android 3Gb devices.
// (see PartialLowEndModeOnMidRangeDevices below)
BASE_FEATURE(kPartialLowEndModeOn3GbDevices, "PartialLowEndModeOn3GbDevices", FEATURE_DISABLED_BY_DEFAULT);
...
BASE_FEATURE(kPartialLowEndModeOnMidRangeDevices, "PartialLowEndModeOnMidRangeDevices",
#if BUILDFLAG(IS_ANDROID)
    FEATURE_ENABLED_BY_DEFAULT);
...
BASE_FEATURE(kCollectAndroidFrameTimelineMetrics, "CollectAndroidFrameTimelineMetrics", FEATURE_DISABLED_BY_DEFAULT);

// If enabled, post registering PowerMonitor broadcast receiver to a background
// thread,
BASE_FEATURE(kPostPowerMonitorBroadcastReceiverInitToBackground, "PostPowerMonitorBroadcastReceiverInitToBackground", FEATURE_ENABLED_BY_DEFAULT);
// If enabled, getMyMemoryState IPC will be posted to background.
BASE_FEATURE(kPostGetMyMemoryStateToBackground, "PostGetMyMemoryStateToBackground", FEATURE_DISABLED_BY_DEFAULT);
#endif // BUILDFLAG(IS_ANDROID)

void Init(EmitThreadControllerProfilerMetadata emit_thread_controller_profiler_metadata)
{
    //InitializeCpuReductionExperiment();
    sequence_manager::internal::SequenceManagerImpl::InitializeFeatures();
    sequence_manager::internal::ThreadController::InitializeFeatures(emit_thread_controller_profiler_metadata);
...
#if BUILDFLAG(IS_WIN)
    sequence_manager::internal::ThreadControllerPowerMonitor::InitializeFeatures();
