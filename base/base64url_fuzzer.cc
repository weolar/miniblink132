...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    Base64UrlEncodePolicy encode_policy = provider.ConsumeBool() ? Base64UrlEncodePolicy::INCLUDE_PADDING : Base64UrlEncodePolicy::OMIT_PADDING;
    std::string encoded;
    ...
    CHECK(Base64UrlDecode(encoded,
        encode_policy == Base64UrlEncodePolicy::INCLUDE_PADDING ? Base64UrlDecodePolicy::REQUIRE_PADDING : Base64UrlDecodePolicy::DISALLOW_PADDING, &decoded));
    CHECK_EQ(decoded, plaintext);
    // Same result should be when ignoring padding.
    decoded.clear();
    CHECK(Base64UrlDecode(encoded, Base64UrlDecodePolicy::IGNORE_PADDING, &decoded));
    CHECK_EQ(decoded, plaintext);
