...
TEST(ValuesTest, SizeOfValue)
{
    ...
    static_assert(
        std::is_same_v<std::integral_constant<size_t, sizeof(Value)>, std::integral_constant<size_t, kExpectedSize>>, "base::Value has an unexpected size!");
}
#endif

TEST(ValuesTest, TestNothrow)
{
    static_assert(std::is_nothrow_move_constructible_v<Value>, "IsNothrowMoveConstructible");
    static_assert(std::is_nothrow_default_constructible_v<Value>, "IsNothrowDefaultConstructible");
    static_assert(std::is_nothrow_constructible_v<Value, std::string&&>, "IsNothrowMoveConstructibleFromString");
    static_assert(std::is_nothrow_constructible_v<Value, Value::BlobStorage&&>, "IsNothrowMoveConstructibleFromBlob");
    static_assert(std::is_nothrow_move_assignable_v<Value>, "IsNothrowMoveAssignable");
}
...
TEST(ValuesTest, UseTestingEachOnValueList)
{
    ...
    EXPECT_THAT(list, testing::Each(testing::ResultOf([](const Value& value) { return value.GetBool(); }, testing::Eq(true))));
}
...
TEST(ValuesTest, ConstructDictWithIterators)
{
    ...
    Value blank;
    blank = Value(Value::Dict(std::make_move_iterator(values.begin()), std::make_move_iterator(values.end())));
    EXPECT_EQ(Value::Type::DICT, blank.type());
    EXPECT_EQ(123, blank.GetDict().Find("Int")->GetInt());
}
...
TEST(ValuesTest, RvalueDictSetByDottedPath)
{
    Value::Dict dict = Value::Dict()
                           ...
                           .SetByDottedPath("nested.dictionary.std-string", std::string("std-value"))
                           .SetByDottedPath("nested.dictionary.blob", Value::BlobStorage({ 1, 2 }))
                           .SetByDottedPath("nested.dictionary.list", Value::List().Append("value in list"))
                           .SetByDottedPath("nested.dictionary.dict", Value::Dict().Set("key", "value"));

    Value::Dict expected = Value::Dict() //
                               .Set("nested",
                                   base::Value::Dict() //
                                       .Set("dictionary",
                                           base::Value::Dict()
...
TEST(ValuesTest, Basic)
{
    ...
    ASSERT_TRUE(settings.Find("global"));
    const std::string* homepage = settings.FindStringByDottedPath("global.homepage");
    ...
    settings.SetByDottedPath("global.toolbar.bookmarks", std::move(new_toolbar_bookmarks));
    Value::List* toolbar_bookmarks = settings.FindListByDottedPath("global.toolbar.bookmarks");
...
{
    Value::List list_with_capacity = Value::List::with_capacity(3).Append(true).Append(42).Append(88.8);
...
TEST(ValuesTest, BinaryValue)
{
    ...
    ASSERT_TRUE(binary.GetBlob().data());
    ASSERT_NE(stack_buffer, reinterpret_cast<const char*>(binary.GetBlob().data()));
    ASSERT_EQ(42U, binary.GetBlob().size());
    ASSERT_EQ(0, memcmp(stack_buffer, binary.GetBlob().data(), binary.GetBlob().size()));
}
...
TEST(ValuesTest, SpecializedEquals)
{
    ...
    for (const Value& outer_value : values) {
        for (const Value& inner_value : values) {
            SCOPED_TRACE(::testing::Message() << "Outer: " << outer_value << "Inner: " << inner_value);
            ...
            outer_value.Visit([&](const auto& outer_member) {
                ...
                if constexpr (!std::is_same_v<T, absl::monostate> && !std::is_same_v<T, Value::BlobStorage>) {
                    if (should_be_equal) {
...
TEST(ValuesTest, Merge)
{
    ...
    const std::string* sub_base_key_value = res_sub_dict->FindString("sub_base_key");
    ASSERT_TRUE(sub_base_key_value);
    EXPECT_EQ("sub_base_key_value_base", *sub_base_key_value); // Preserved.
    const std::string* sub_collide_key_value = res_sub_dict->FindString("sub_collide_key");
    ASSERT_TRUE(sub_collide_key_value);
    EXPECT_EQ("sub_collide_key_value_merge",
        *sub_collide_key_value); // Replaced.
    const std::string* sub_merge_key_value = res_sub_dict->FindString("sub_merge_key");
...
TEST(ValuesTest, FromToUniquePtrValue)
{
    ...
    std::unique_ptr<Value> val = Value::ToUniquePtrValue(std::move(dict_converted));
...
TEST(ValuesTest, TracingSupport)
{
    ...
    }
    {
        ...
        EXPECT_EQ(perfetto::TracedValueToString(Value(std::move(dict))), "{key:value}");
    }
}
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
...
TEST(ValueViewTest, BasicConstruction)
{
    ...
    {
        ValueView v = 3.14;
        EXPECT_DOUBLE_EQ(3.14, absl::get<ValueView::DoubleStorageForTest>(v.data_view_for_test()));
    }
    {
        ValueView v = std::string_view("hello world");
        EXPECT_EQ("hello world", absl::get<std::string_view>(v.data_view_for_test()));
    }
    {
    ...
    }
    {
        ...
        dict.Set("hello", "world");
        ValueView v = dict;
        EXPECT_EQ(dict, absl::get<std::reference_wrapper<const Value::Dict>>(v.data_view_for_test()));
    }
    {
        Value::List list;
        list.Append("hello");
        list.Append("world");
        ValueView v = list;
        EXPECT_EQ(list, absl::get<std::reference_wrapper<const Value::List>>(v.data_view_for_test()));
    }
}
