...
public:
    ...
    template <typename T> explicit Value(const T*) = delete;
    ...
    explicit Value(const std::vector<uint8_t>& value);
    ...
    public:
        ...
        template <class IteratorType> explicit Dict(std::move_iterator<IteratorType> first, std::move_iterator<IteratorType> last)
        {
            ...
            for (auto current = first; current != last; ++current) {
                ...
                auto value = *current;
                values.emplace_back(std::move(value.first), std::make_unique<Value>(std::move(value.second)));
            }
        ...
        template <typename T> Value* Set(std::string_view, const T*) & = delete;
        ...
        template <typename T> Dict&& Set(std::string_view, const T*) && = delete;
        ...
        template <typename T> Value* SetByDottedPath(std::string_view, const T*) & = delete;
        ...
        template <typename T> Dict&& SetByDottedPath(std::string_view, const T*) && = delete;
    ...
    public:
        ...
        template <typename T> void Append(const T*) & = delete;
        ...
        template <typename T> List&& Append(const T*) && = delete;
    ...
    template <typename T> friend bool operator==(const Value& lhs, const T* rhs) = delete;
    template <typename T> friend bool operator==(const T* lhs, const Value& rhs) = delete;
    template <typename T> friend bool operator!=(const Value& lhs, const T* rhs) = delete;
    template <typename T> friend bool operator!=(const T* lhs, const Value& rhs) = delete;
    BASE_EXPORT friend bool operator==(const Value& lhs, int rhs);
    friend bool operator==(int lhs, const Value& rhs)
    {
        return rhs == lhs;
    }
    friend bool operator!=(const Value& lhs, int rhs)
    {
        return !(lhs == rhs);
    }
    friend bool operator!=(int lhs, const Value& rhs)
    ...
    friend bool operator==(double lhs, const Value& rhs)
    {
        return rhs == lhs;
    }
...
private:
    ...
    public:
        ...
        {
            return base::bit_cast<double>(v_);
        }
    ...
    absl::variant<absl::monostate, bool, int, DoubleStorage, std::string, BlobStorage, Dict, List> data_;
};
...
public:
    ...
    template <typename T> ValueView(const T*) = delete;
    ValueView(int value)
        : data_view_(value)
    {
    }
    ...
    ValueView(std::string_view value)
        : data_view_(value)
    {
    }
    ...
    ValueView(const Value::BlobStorage& value)
        : data_view_(value)
    {
    }
    ValueView(const Value::Dict& value)
        : data_view_(value)
    {
    }
    ValueView(const Value::List& value)
        : data_view_(value)
    {
    }
...
private:
    using ViewType = absl::variant<absl::monostate, bool, int, Value::DoubleStorage, std::string_view, std::reference_wrapper<const Value::BlobStorage>,
        std::reference_wrapper<const Value::Dict>, std::reference_wrapper<const Value::List>>;

public:
    using DoubleStorageForTest = Value::DoubleStorage;
    const ViewType& data_view_for_test() const
    {
        return data_view_;
    }
...
public:
    ...
    virtual std::unique_ptr<Value> Deserialize(int* error_code, std::string* error_message) = 0;
    ...
    {
        return (kErrorCodeInvalidFormat <= error_code) && (error_code < kErrorCodeFirstMetadataError);
    }
};

// Stream operator so Values can be pretty printed by gtest.
BASE_EXPORT std::ostream& operator<<(std::ostream& out, const Value& value);
BASE_EXPORT std::ostream& operator<<(std::ostream& out, const Value::Dict& dict);
BASE_EXPORT std::ostream& operator<<(std::ostream& out, const Value::List& list);

// Stream operator so that enum class Types can be used in log statements.
BASE_EXPORT std::ostream& operator<<(std::ostream& out, const Value::Type& type);
