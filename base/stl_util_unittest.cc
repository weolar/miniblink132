...
{
    {
        ...
        static_assert(std::is_same_v<decltype(GetUnderlyingContainer(queue)), const std::deque<int>&>, "GetUnderlyingContainer(queue) should be of type deque");
        EXPECT_THAT(GetUnderlyingContainer(queue), testing::ElementsAre(1, 2, 3, 4, 5));
    }
...

    {
        ...
        static_assert(std::is_same_v<decltype(GetUnderlyingContainer(queue)), const base::circular_deque<int>&>,
...

    {
        ...
        static_assert(
            std::is_same_v<decltype(GetUnderlyingContainer(queue)), const std::vector<int>&>, "GetUnderlyingContainer(queue) should be of type vector");
        EXPECT_THAT(GetUnderlyingContainer(queue), testing::UnorderedElementsAre(1, 2, 3, 4, 5));
    }

    {
        std::stack<int> stack({ 1, 2, 3, 4, 5 });
        static_assert(std::is_same_v<decltype(GetUnderlyingContainer(stack)), const std::deque<int>&>, "GetUnderlyingContainer(stack) should be of type deque");
        EXPECT_THAT(GetUnderlyingContainer(stack), testing::ElementsAre(1, 2, 3, 4, 5));
    }
}
