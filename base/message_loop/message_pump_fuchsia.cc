...
MessagePumpFuchsia::ZxHandleWatchController::ZxHandleWatchController(const Location& from_here)
    : async_wait_t({})
...
bool MessagePumpFuchsia::ZxHandleWatchController::WaitBegin()
{
    ...
    zx_status_t status = async_begin_wait(weak_pump_->async_loop_->dispatcher(), this);
    if (status != ZX_OK) {
        ZX_DLOG(ERROR, status) << "async_begin_wait():" << created_from_location_.ToString();
...
bool MessagePumpFuchsia::ZxHandleWatchController::StopWatchingZxHandle()
{
    ...
    zx_status_t result = async_cancel_wait(weak_pump_->async_loop_->dispatcher(), this);
    ZX_DLOG_IF(ERROR, result != ZX_OK, result) << "async_cancel_wait(): " << created_from_location_.ToString();
    return result == ZX_OK;
}

// static
void MessagePumpFuchsia::ZxHandleWatchController::HandleSignal(
    async_dispatcher_t* async, async_wait_t* wait, zx_status_t status, const zx_packet_signal_t* signal)
{
    ZxHandleWatchController* controller = static_cast<ZxHandleWatchController*>(wait);
    ...
    if (controller->weak_pump_ && controller->weak_pump_->run_state_) {
        scoped_do_work_item = controller->weak_pump_->run_state_->delegate->BeginWorkItem();
    }

    TRACE_EVENT0("toplevel", "ZxHandleSignal");

    if (status != ZX_OK) {
        ZX_DLOG(WARNING, status) << "async wait failed: " << controller->created_from_location_.ToString();
        return;
    }
...
void MessagePumpFuchsia::FdWatchController::OnZxHandleSignalled(zx_handle_t handle, zx_signals_t signals)
{
    uint32_t events;
...
MessagePumpFuchsia::FdWatchController::FdWatchController(const Location& from_here)
...
bool MessagePumpFuchsia::FdWatchController::WaitBegin()
{
    ...
    if (async_wait_t::object == ZX_HANDLE_INVALID) {
        DLOG(ERROR) << "fdio_wait_begin failed: " << ZxHandleWatchController::created_from_location_.ToString();
        return false;
    }
...
bool MessagePumpFuchsia::WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* delegate)
{
    DCHECK_GE(fd, 0);
    DCHECK(controller);
    DCHECK(delegate);
    ...
    return WatchZxHandle(ZX_HANDLE_INVALID, persistent, 1, controller, controller);
}

bool MessagePumpFuchsia::WatchZxHandle(
    zx_handle_t handle, bool persistent, zx_signals_t signals, ZxHandleWatchController* controller, ZxHandleWatcher* delegate)
{
    ...
    // for the same handle.
    DCHECK(handle == ZX_HANDLE_INVALID || !controller->is_active() || handle == controller->async_wait_t::object);
...
void MessagePumpFuchsia::Run(Delegate* delegate)
{
    ...
    for (;;) {
        ...
        bool attempt_more_work = next_work_info.is_immediate() || did_handle_io_event;
        ...
        zx_time_t deadline = next_work_info.delayed_run_time.is_max() ? ZX_TIME_INFINITE : next_work_info.delayed_run_time.ToZxTime();
...
void MessagePumpFuchsia::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
