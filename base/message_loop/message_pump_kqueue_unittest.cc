...
public:
    MessagePumpKqueueTest()
        : pump_(new MessagePumpKqueue())
        , executor_(WrapUnique(pump_.get()))
    {
    }

    MessagePumpKqueue* pump()
    {
        return pump_;
    }

    static void CreatePortPair(apple::ScopedMachReceiveRight* receive, apple::ScopedMachSendRight* send)
    {
        mach_port_options_t options {};
        options.flags = MPO_INSERT_SEND_RIGHT;
        apple::ScopedMachReceiveRight port;
        kern_return_t kr = mach_port_construct(mach_task_self(), &options, 0, apple::ScopedMachReceiveRight::Receiver(*receive).get());
        ASSERT_EQ(kr, KERN_SUCCESS);
        *send = apple::ScopedMachSendRight(receive->get());
    }

    static mach_msg_return_t SendEmptyMessage(mach_port_t remote_port, mach_msg_id_t msgid)
    {
...
private:
    raw_ptr<MessagePumpKqueue, DanglingUntriaged> pump_; // Weak, owned by |executor_|.
    SingleThreadTaskExecutor executor_;
};

class PortWatcher : public MessagePumpKqueue::MachPortWatcher {
public:
    PortWatcher(RepeatingClosure callback)
    ...
    void OnMachMessageReceived(mach_port_t port) override
    {
        ...
        kern_return_t kr = mach_msg(&message.header, MACH_RCV_MSG, 0, sizeof(message), port, 0, MACH_PORT_NULL);
        ASSERT_EQ(kr, KERN_SUCCESS);
...
TEST_F(MessagePumpKqueueTest, MachPortStopWatching)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce([](MessagePumpKqueue::MachPortWatchController* controller) { controller->StopWatchingMachPort(); }, Unretained(&controller)));

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce([](mach_port_t port) { EXPECT_EQ(KERN_SUCCESS, SendEmptyMessage(port, 100)); }, port.get()));
...
TEST_F(MessagePumpKqueueTest, MultipleMachWatchers)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce([](mach_port_t port1) { ASSERT_EQ(KERN_SUCCESS, SendEmptyMessage(port1, 0xf000f)); }, port1.get()));
