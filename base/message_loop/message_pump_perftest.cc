...
public:
    ...
    void Schedule(int index)
    {
        ...
        target_message_loop_base()->GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ScheduleWorkTest::Increment, base::Unretained(this), schedule_calls));
    }
    ...
    void ScheduleWork(MessagePumpType target_type, int num_scheduling_threads)
    {
        ...
        scheduling_times_ = std::make_unique<base::TimeDelta[]>(num_scheduling_threads);
        scheduling_thread_times_ = std::make_unique<base::TimeDelta[]>(num_scheduling_threads);
        min_batch_times_ = std::make_unique<base::TimeDelta[]>(num_scheduling_threads);
        max_batch_times_ = std::make_unique<base::TimeDelta[]>(num_scheduling_threads);
        ...
        for (int i = 0; i < num_scheduling_threads; ++i) {
            scheduling_threads[i]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&ScheduleWorkTest::Schedule, base::Unretained(this), i));
        }
        ...
        std::string story_name = StringPrintf("%s_pump_from_%d_threads",
            target_type == MessagePumpType::IO ? "io" : (target_type == MessagePumpType::UI ? "ui" : "default"), num_scheduling_threads);
        auto reporter = SetUpReporter(story_name);
        reporter.AddResult(kMetricMinBatchTime, total_time.InMicroseconds() / static_cast<double>(counter_));
        reporter.AddResult(kMetricMaxBatchTime, max_batch_time.InMicroseconds() / static_cast<double>(kBatchSize));
        reporter.AddResult(kMetricTotalTime, total_time.InMicroseconds() / static_cast<double>(counter_));
        if (ThreadTicks::IsSupported()) {
            reporter.AddResult(kMetricThreadTime, total_thread_time.InMicroseconds() / static_cast<double>(counter_));
        }
    }
...
#if BUILDFLAG(IS_ANDROID)
        if (java_thread_) {
            return static_cast<sequence_manager::internal::SequenceManagerImpl*>(java_thread_->state()->sequence_manager.get());
        }
#endif
