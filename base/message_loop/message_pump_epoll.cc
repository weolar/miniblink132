...
BASE_FEATURE(kBatchNativeEventsInMessagePumpEpoll, "BatchNativeEventsInMessagePumpEpoll", base::FEATURE_DISABLED_BY_DEFAULT);
...
constexpr std::pair<uint32_t, short int> kEpollToPollEvents[]
    = { { EPOLLIN, POLLIN }, { EPOLLOUT, POLLOUT }, { EPOLLRDHUP, POLLRDHUP }, { EPOLLPRI, POLLPRI }, { EPOLLERR, POLLERR }, { EPOLLHUP, POLLHUP } };
...
struct MessagePumpEpoll::InterestParams {
    ...
    {
        return std::tie(fd, read, write, one_shot) == std::tie(rhs.fd, rhs.read, rhs.write, rhs.one_shot);
    }
};
...
public:
    ...
    const InterestParams& params() const
    {
        return params_;
    }
    ...
    bool was_controller_destroyed() const
    {
        return was_controller_destroyed_;
    }
    ...
    void StopWatchingForControllerDestruction()
    {
        --nested_controller_destruction_watchers_;
        DCHECK_GE(nested_controller_destruction_watchers_, 0);
        if (nested_controller_destruction_watchers_ == 0 && !was_controller_destroyed_) {
            DCHECK_EQ(controller_->was_destroyed_, &was_controller_destroyed_);
...
void MessagePumpEpoll::InitializeFeatures()
{
    ...
    g_use_batched_version.store(base::FeatureList::IsEnabled(kBatchNativeEventsInMessagePumpEpoll), std::memory_order_relaxed);
    g_use_poll.store(base::FeatureList::IsEnabled(kUsePollForMessagePumpEpoll), std::memory_order_relaxed);
}

bool MessagePumpEpoll::WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* watcher)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    TRACE_EVENT("base", "MessagePumpEpoll::WatchFileDescriptor", "fd", fd, "persistent", persistent, "watch_read", mode & WATCH_READ, "watch_write",
        mode & WATCH_WRITE);
...
void MessagePumpEpoll::Run(Delegate* delegate)
{
    ...
    for (;;) {
        ...
        const int max_iterations = g_use_batched_version.load(std::memory_order_relaxed) ? 16 : 1;
        ...
        TimeDelta next_metrics_delay = next_metrics_time_ - next_work_info.recent_now;
...
{
    UMA_HISTOGRAM_COUNTS_1000("MessagePumpEpoll.WatchedFileDescriptors", (int)entries_.size());
...
void MessagePumpEpoll::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
    ...
    if (rv != 0) {
        for (auto& history : entry.epoll_history_) {
            if (history.event) {
                ...
                LOG(ERROR) << "events=0x" << std::hex << std::setfill('0') << std::setw(8) << e.events;
                LOG(ERROR) << "data=0x" << std::hex << std::setfill('0') << std::setw(16) << e.data.u64;
            }
...
void MessagePumpEpoll::UnregisterInterest(const scoped_refptr<Interest>& interest)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
bool MessagePumpEpoll::WaitForEpollEvents(TimeDelta timeout)
{
    ...
    const int epoll_timeout = timeout.is_max() ? -1 : saturated_cast<int>(timeout.InMillisecondsRoundedUp());
    ...
    bool use_poll = g_use_poll.load(std::memory_order_relaxed) && entries_.size() < 500;
    ...
    if (use_poll) {
        ...
        const int epoll_result = epoll_wait(epoll_.get(), epoll_events, std::size(epoll_events), epoll_timeout);
        if (epoll_result < 0) {
            DPCHECK(errno == EINTR);
            return false;
        }
        ...
        ready_events = span(epoll_events).first(base::checked_cast<size_t>(epoll_result));
    }
...
{
    return std::find_if(pollfds_.begin(), pollfds_.end(), [fd](const struct pollfd poll_entry) { return poll_entry.fd == fd; });
}
...
bool MessagePumpEpoll::GetEventsPoll(int epoll_timeout, std::vector<epoll_event>* epoll_events)
{
    int retval = poll(&pollfds_[0], base::checked_cast<nfds_t>(pollfds_.size()), epoll_timeout);
    if (retval < 0) {
        DPCHECK(errno == EINTR);
        return false;
    }
...
void MessagePumpEpoll::OnEpollEvent(EpollEventEntry& entry, uint32_t events)
{
    ...
    for (const auto& interest : interests) {
        ...
        const bool can_write = (writable || disconnected) && interest->params().write;
...
void MessagePumpEpoll::HandleEvent(int fd, bool can_read, bool can_write, FdWatchController* controller)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    TRACE_EVENT("toplevel", "EpollEvent", "controller_created_from", controller->created_from_location(), "fd", fd, "can_read", can_read, "can_write",
        can_write, "context", static_cast<void*>(controller));
    TRACE_HEAP_PROFILER_API_SCOPED_TASK_EXECUTION heap_profiler_scope(controller->created_from_location().file_name());
    if (can_read && can_write) {
        bool controller_was_destroyed = false;
        bool* previous_was_destroyed_flag = std::exchange(controller->was_destroyed_, &controller_was_destroyed);
...
MessagePumpEpoll::FdWatchController::FdWatchController(const Location& from_here)
...
const scoped_refptr<MessagePumpEpoll::Interest>& MessagePumpEpoll::FdWatchController::AssignInterest(const InterestParams& params)
{
