...
public:
    MessagePumpEpollTest()
        : task_environment_(std::make_unique<test::SingleThreadTaskEnvironment>(test::SingleThreadTaskEnvironment::MainThreadType::UI))
...
protected:
    ...
    void SimulateIOEvent(MessagePumpEpoll* pump, MessagePumpEpoll::FdWatchController* controller)
    {
...
public:
    explicit DeleteWatcher(std::unique_ptr<MessagePumpEpoll::FdWatchController> controller)
...
{
    DeleteWatcher delegate(std::make_unique<MessagePumpEpoll::FdWatchController>(FROM_HERE));
    auto pump = std::make_unique<MessagePumpEpoll>();
    pump->WatchFileDescriptor(receiver(), false, MessagePumpEpoll::WATCH_READ_WRITE, delegate.controller(), &delegate);
...
TEST_F(MessagePumpEpollTest, StopWatcher)
{
    ...
    pump->WatchFileDescriptor(receiver(), false, MessagePumpEpoll::WATCH_READ_WRITE, &controller, &delegate);
...
TEST_F(MessagePumpEpollTest, NestedPumpWatcher)
{
    ...
    auto pump = std::make_unique<MessagePumpEpoll>();
    MessagePumpEpoll::FdWatchController controller(FROM_HERE);
    pump->WatchFileDescriptor(receiver(), false, MessagePumpEpoll::WATCH_READ, &controller, &delegate);
    SimulateIOEvent(pump.get(), &controller);
}
...
public:
    ...
    void OnFileCanReadWithoutBlocking(int /* fd */) override
    {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&FatalClosure));
...
TEST_F(MessagePumpEpollTest, QuitWatcher)
{
    ...
    MessagePumpEpoll::FdWatchController controller(FROM_HERE);
    WaitableEvent event(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    ...
    WaitableEventWatcher::EventCallback write_fd_task = BindOnce(&WriteFDWrapper, sender(), &null_byte_, 1);
    io_runner()->PostTask(
    ...
    io_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEventWatcher::StopWatching, Owned(watcher.release())));
}

class InnerNestedWatcher : public MessagePumpEpoll::FdWatcher {
public:
    InnerNestedWatcher(MessagePumpEpollTest& test, MessagePumpEpoll::FdWatchController& outer_controller, base::OnceClosure callback)
        : test_(test)
        , outer_controller_(outer_controller)
        , callback_(std::move(callback))
    {
        base::CurrentIOThread::Get().WatchFileDescriptor(test_->receiver(), false, MessagePumpEpoll::WATCH_READ, &controller_, this);
    }
...
public:
    ...
    {
        base::RunLoop loop;
        test_->io_runner()->PostTask(FROM_HERE, base::BindOnce(&OuterNestedWatcher::InitOnIOThread, base::Unretained(this), loop.QuitClosure()));
        loop.Run();
    }
    ...
    void OnFileCanReadWithoutBlocking(int) override
    {
        ...
        base::RunLoop loop;
        std::unique_ptr<InnerNestedWatcher> inner_watcher = std::make_unique<InnerNestedWatcher>(test_.get(), *controller_, loop.QuitClosure());
        test_->Notify();
        loop.Run();
...
private:
    ...
    {
        controller_ = std::make_unique<MessagePumpEpoll::FdWatchController>(FROM_HERE);
        base::CurrentIOThread::Get().WatchFileDescriptor(test_->receiver(), false, MessagePumpEpoll::WATCH_READ, controller_.get(), this);
...
public:
    RepeatWatcher(MessagePumpEpoll* pump, int fd, MessagePumpForIO::Mode mode, bool persistent, int repeat)
        : pump_(pump)
        , fd_(fd)
        , mode_(mode)
    ...
    void StartWatching()
    {
        const bool watch_success = pump_->WatchFileDescriptor(fd_, persistent_, mode_, &fd_watch_controller_, this);
...
void RepeatEventTest(bool persistent, int repeat, std::unique_ptr<MessagePumpEpoll> executor_pump, int sender, int receiver)
{
    MessagePumpEpoll* pump = executor_pump.get();
    SingleThreadTaskExecutor executor(std::move(executor_pump));
    RunLoop run_loop;
    RepeatWatcher delegate(pump, receiver, MessagePumpEpoll::WATCH_READ, persistent, repeat);
...
TEST_F(MessagePumpEpollTest, RepeatPersistentEvent)
{
    ...
    RepeatEventTest(/* persistent= */ true, /* repeat= */ 3, std::make_unique<MessagePumpEpoll>(), sender(), receiver());
}
...
TEST_F(MessagePumpEpollTest, RepeatOneShotEvent)
{
    ...
    RepeatEventTest(/* persistent= */ false, /* repeat= */ 3, std::make_unique<MessagePumpEpoll>(), sender(), receiver());
}
