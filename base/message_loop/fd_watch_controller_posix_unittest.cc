...
class FdWatchControllerPosixTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
    FdWatchControllerPosixTest() = default;

    FdWatchControllerPosixTest(const FdWatchControllerPosixTest&) = delete;
    FdWatchControllerPosixTest& operator=(const FdWatchControllerPosixTest&) = delete;

    // testing::Test interface.
    void SetUp() override
    {
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)
        features_.InitWithFeatureStates({ { kUsePollForMessagePumpEpoll, GetParam() } });
...
public:
    ...
    void set_run_loop(base::RunLoop* loop)
    {
        loop_ = loop;
    }
...
TEST_P(FdWatchControllerPosixTest, FileDescriptorWatcherOutlivesMessageLoop)
{
    ...
    MessagePumpForIO::FdWatchController watcher(FROM_HERE);
    {
        ...
        CurrentIOThread::Get()->WatchFileDescriptor(write_fd_.get(), true, MessagePumpForIO::WATCH_WRITE, &watcher, &handler);
...
TEST_P(FdWatchControllerPosixTest, FileDescriptorWatcherDeleteInCallback)
{
    ...
    handler.watcher_to_delete_ = std::make_unique<MessagePumpForIO::FdWatchController>(FROM_HERE);

    CurrentIOThread::Get()->WatchFileDescriptor(write_fd_.get(), true, MessagePumpForIO::WATCH_WRITE, handler.watcher_to_delete_.get(), &handler);
    loop.Run();
}
...
public:
    ...
    ReaderWriterHandler(Action action, ActWhen when, OnceClosure idle_quit_closure)
        : action_(action)
        , when_(when)
...
class MessageLoopForIoPosixReadAndWriteTest : public testing::TestWithParam<ReaderWriterHandler::Action> {
protected:
...
INSTANTIATE_TEST_SUITE_P(
    StopWatchingOrDelete, MessageLoopForIoPosixReadAndWriteTest, testing::Values(ReaderWriterHandler::kStopWatching, ReaderWriterHandler::kDelete));
...
TEST_P(MessageLoopForIoPosixReadAndWriteTest, AfterRead)
{
    ...
    RunLoop run_loop;
    ReaderWriterHandler* handler = new ReaderWriterHandler(GetParam(), ReaderWriterHandler::kOnReadEvent, run_loop.QuitWhenIdleClosure());
    ...
    CurrentIOThread::Get()->WatchFileDescriptor(one.get(), true, MessagePumpForIO::WATCH_READ_WRITE, handler->controller(), handler);
    run_loop.Run();
...
TEST_P(MessageLoopForIoPosixReadAndWriteTest, AfterWrite)
{
    ...
    RunLoop run_loop;
    ReaderWriterHandler* handler = new ReaderWriterHandler(GetParam(), ReaderWriterHandler::kOnWriteEvent, run_loop.QuitWhenIdleClosure());
...
TEST_P(FdWatchControllerPosixTest, WatchReadable)
{
    ...
    MessagePumpForIO::FdWatchController watcher(FROM_HERE);
    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(read_fd_.get(), /*persistent=*/false, MessagePumpForIO::WATCH_READ, &watcher, &handler));
...
TEST_P(FdWatchControllerPosixTest, WatchReadableTwiceSameWatcher)
{
    ...
    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(read_fd_.get(), /*persistent=*/false, MessagePumpForIO::WATCH_READ, &watcher, &handler2));
...
TEST_P(FdWatchControllerPosixTest, WatchWritable)
{
    ...
    MessagePumpForIO::FdWatchController watcher(FROM_HERE);
    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(write_fd_.get(), /*persistent=*/false, MessagePumpForIO::WATCH_WRITE, &watcher, &handler));
...
void StopWatching(MessagePumpForIO::FdWatchController* controller, RunLoop* run_loop)
{
...
TEST_P(FdWatchControllerPosixTest, StopFromHandler)
{
    ...
    RunLoop run_loop;
    MessagePumpForIO::FdWatchController watcher(FROM_HERE);
    CallClosureHandler handler(BindOnce(&StopWatching, &watcher, &run_loop), OnceClosure());

    // Create persistent watcher.
    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(read_fd_.get(), /*persistent=*/true, MessagePumpForIO::WATCH_READ, &watcher, &handler));
...
void StopWatchingAndWatchAgain(MessagePumpForIO::FdWatchController* controller, int fd, MessagePumpForIO::FdWatcher* new_handler, RunLoop* run_loop)
{
    controller->StopWatchingFileDescriptor();

    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(fd, /*persistent=*/true, MessagePumpForIO::WATCH_READ, controller, new_handler));
...
TEST_P(FdWatchControllerPosixTest, StopAndRestartFromHandler)
{
    ...
    CallClosureHandler handler1(BindOnce(&StopWatchingAndWatchAgain, &watcher, read_fd_.get(), &handler2, &run_loop1), OnceClosure());

    // Create persistent watcher.
    ASSERT_TRUE(CurrentIOThread::Get()->WatchFileDescriptor(read_fd_.get(), /*persistent=*/true, MessagePumpForIO::WATCH_READ, &watcher, &handler1));
...
TEST_P(FdWatchControllerPosixTest, IoEventThenTimer)
{
    ...
    env.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, timer_run_loop.QuitClosure(), base::Milliseconds(10));
...
TEST_P(FdWatchControllerPosixTest, TimerThenIoEvent)
{
    ...
    env.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, BindOnce(&FdWatchControllerPosixTest::TriggerReadEvent, Unretained(this)), Milliseconds(1));
