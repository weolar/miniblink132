...
public:
    explicit MockMessagePumpDelegate(MessagePumpType pump_type)
        ...
        , native_work_item_accounting_is_on_(!ChromeControlsNativeEventProcessing(pump_type))
    {
    }

    ~MockMessagePumpDelegate() override
    {
        ValidateNoOpenWorkItems();
    }
...
public:
    MessagePumpTest()
        : message_pump_(MessagePump::Create(GetParam()))
    {
    }
...
protected:
...
#endif
    void AddPreDoWorkExpectations(testing::StrictMock<MockMessagePumpDelegate>& delegate)
    {
#if BUILDFLAG(IS_WIN)
    ...
    void AddPostDoWorkExpectations(testing::StrictMock<MockMessagePumpDelegate>& delegate)
    {
#if defined(USE_GLIB)
        if (GetParam() == MessagePumpType::UI) {
...
TEST_P(MessagePumpTest, DetectingHasInputYieldsOnUi)
{
    ...
    android::InputHintChecker::ScopedOverrideInstance scoped_override_hint(&hint_checker_mock);
    ...
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([] {
        auto work_info = MessagePump::Delegate::NextWorkInfo { .delayed_run_time = TimeTicks() };
    ...
    if (pump_type == MessagePumpType::UI) {
        ...
        EXPECT_CALL(hint_checker_mock, HasInputImplWithThrottling()).WillOnce(Invoke([] { return true; }));
    }

    // Override the second DoWork() to quit the loop.
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([this] {
        message_pump_->Quit();
        return MessagePump::Delegate::NextWorkInfo { .delayed_run_time = TimeTicks::Max() };
    }));
    ...
    EXPECT_EQ(initial_work_enters + work_loop_entered, GetAndroidNonDelayedWorkEnterCount());
}
#endif
...
TEST_P(MessagePumpTest, YieldToNativeRequestedSmokeTest)
{
    ...
    AddPreDoWorkExpectations(delegate);
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([] {
        return MessagePump::Delegate::NextWorkInfo { TimeTicks(), TimeDelta(), TimeTicks(),
    ...
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([this] {
        ...
        return MessagePump::Delegate::NextWorkInfo { now + Milliseconds(1), TimeDelta(), now, true };
    }));
...
TEST_P(MessagePumpTest, LeewaySmokeTest)
{
    ...
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([this] {
        message_pump_->Quit();
        auto now = TimeTicks::Now();
        return MessagePump::Delegate::NextWorkInfo { now + Milliseconds(1), Milliseconds(8), now };
    }));
    EXPECT_CALL(delegate, DoIdleWork()).Times(AnyNumber());
...
INSTANTIATE_TEST_SUITE_P(All, MessagePumpTest, ::testing::Values(MessagePumpType::DEFAULT, MessagePumpType::UI, MessagePumpType::IO));
