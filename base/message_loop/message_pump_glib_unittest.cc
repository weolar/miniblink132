...
private:
    ...
    static gboolean Dispatch(GSource* source, GSourceFunc unused_func, gpointer unused_data)
    {
...
{
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(from_here, std::move(task));
}
...
private:
    test::SingleThreadTaskEnvironment task_environment_ { test::SingleThreadTaskEnvironment::MainThreadType::UI };
...
TEST_F(MessagePumpGLibTest, TestEventTaskInterleave)
{
    ...
    OnceClosure check_task = BindOnce(&ExpectProcessedEvents, Unretained(injector()), 2);
    OnceClosure posted_task = BindOnce(&PostMessageLoopTask, FROM_HERE, std::move(check_task));
    ...
    posted_task = BindOnce(&PostMessageLoopTask, FROM_HERE, std::move(check_task));
    injector()->AddEventAsTask(0, std::move(posted_task));
...
TEST_F(MessagePumpGLibTest, TestWorkWhileWaitingForEvents)
{
    ...
    for (int i = 0; i < 10; ++i) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&IncrementInt, &task_count));
    }
    // After all the previous tasks have executed, enqueue an event that will
    // quit.
    {
        RunLoop run_loop;
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&EventInjector::AddEvent, Unretained(injector()), 0, run_loop.QuitClosure()));
        run_loop.Run();
    }
    ...
    task_count = 0;
    for (int i = 0; i < 10; ++i) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce(&IncrementInt, &task_count), Milliseconds(10 * i));
    }
    // After all the previous tasks have executed, enqueue an event that will
    // quit.
    ...
    // That is verified in message_loop_unittest.cc.
    {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, BindOnce(&EventInjector::AddEvent, Unretained(injector()), 0, run_loop.QuitClosure()), Milliseconds(150));
        run_loop.Run();
    }
    ASSERT_EQ(10, task_count);
    EXPECT_EQ(1, injector()->processed_events());
...
TEST_F(MessagePumpGLibTest, TestEventsWhileWaitingForWork)
{
    ...
    OnceClosure check_task = BindOnce(&ExpectProcessedEvents, Unretained(injector()), 11);
    OnceClosure posted_task = BindOnce(&PostMessageLoopTask, FROM_HERE, std::move(check_task));
...
public:
    ...
    void FromTask()
    {
        ...
        if (task_count_ == 0 && event_count_ == 0) {
            ...
            SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, this));
        }
    }
    ...
    void FromEvent()
    {
        ...
        if (task_count_ == 0 && event_count_ == 0) {
            ...
            injector_->AddEventAsTask(0, BindOnce(&ConcurrentHelper::FromEvent, this));
        }
    }
...
TEST_F(MessagePumpGLibTest, TestConcurrentEventPostedTask)
{
    ...
    RunLoop run_loop;
    scoped_refptr<ConcurrentHelper> helper = new ConcurrentHelper(injector(), run_loop.QuitClosure());
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, helper));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, helper));
...
TEST_F(MessagePumpGLibTest, TestDrainingGLib)
{
    ...
    RunLoop run_loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&AddEventsAndDrainGLib, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
...
void TestGLibLoopInternal(EventInjector* injector, OnceClosure done)
{
    ...
    injector->AddDummyEvent(10);
    // Delayed work
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce(&IncrementInt, &task_count), Milliseconds(30));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce(&GLibLoopRunner::Quit, runner), Milliseconds(40));
...
TEST_F(MessagePumpGLibTest, TestGLibLoop)
{
    ...
    RunLoop run_loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&TestGLibLoopInternal, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
}
...
TEST_F(MessagePumpGLibTest, TestGtkLoop)
{
    ...
    // between events and posted tasks.
    RunLoop run_loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&TestGtkLoopInternal, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
}
...
public:
    ...
    size_t CountEvents()
    {
        std::unique_ptr<trace_analyzer::TraceAnalyzer> analyzer = trace_analyzer::Stop();
        trace_analyzer::TraceEventVector events;
        return analyzer->FindEvents(trace_analyzer::Query::EventName() == trace_analyzer::Query::String("Nested"), &events);
    }
};
...
TEST_F(MessagePumpGLibTest, TestNativeNestedLoopWithoutDoWork)
{
    ...
    base::CurrentThread::Get()->EnableMessagePumpTimeKeeperMetrics("GlibMainLoopTest");

    scoped_refptr<GLibLoopRunner> runner = base::MakeRefCounted<GLibLoopRunner>();
    injector()->AddEvent(0,
        BindOnce(
            [](EventInjector* injector, scoped_refptr<GLibLoopRunner> runner, OnceClosure done) {
                CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow;
                runner->RunLoop();
            },
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), Milliseconds(40));
...
protected:
    ...
    void SimulateEvent(MessagePumpGlib* pump, MessagePumpGlib::FdWatchController* controller)
    {
...
public:
    explicit DeleteWatcher(std::unique_ptr<MessagePumpGlib::FdWatchController> controller)
        : BaseWatcher(controller.get())
        , owned_controller_(std::move(controller))
    {
    }

    ~DeleteWatcher() override
    {
        DCHECK(!controller_);
    }
...
void QuitMessageLoopAndStart(OnceClosure quit_closure)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, runloop.QuitClosure());
    runloop.Run();
}
...
public:
    ...
    {
        RunLoop runloop;
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&QuitMessageLoopAndStart, runloop.QuitClosure()));
        runloop.Run();
    }
...
public:
    QuitWatcher(std::unique_ptr<MessagePumpGlib::FdWatchController> controller, base::OnceClosure quit_closure)
...
void WriteFDWrapper(const int fd, const char* buf, int size, WaitableEvent* event)
{
...
TEST_F(MessagePumpGLibFdWatchTest, DeleteWatcher)
{
    ...
    auto controller_ptr = std::make_unique<MessagePumpGlib::FdWatchController>(FROM_HERE);
    auto* controller = controller_ptr.get();

    DeleteWatcher watcher(std::move(controller_ptr));
    pump->WatchFileDescriptor(pipefds_[1], false, MessagePumpGlib::WATCH_READ_WRITE, controller, &watcher);
...
{
    test::SingleThreadTaskEnvironment task_environment(test::SingleThreadTaskEnvironment::MainThreadType::UI);
    std::unique_ptr<MessagePumpGlib> pump(new MessagePumpGlib);
    NestedPumpWatcher watcher;
    MessagePumpGlib::FdWatchController controller(FROM_HERE);
    pump->WatchFileDescriptor(pipefds_[1], false, MessagePumpGlib::WATCH_READ, &controller, &watcher);
...
TEST_F(MessagePumpGLibFdWatchTest, QuitWatcher)
{
    ...
    auto owned_controller = std::make_unique<MessagePumpGlib::FdWatchController>(FROM_HERE);
    ...
    pump->WatchFileDescriptor(pipefds_[0], false, MessagePumpGlib::WATCH_READ, controller, &delegate);

    // Make the IO thread wait for |event| before writing to pipefds[1].
    WaitableEvent event;
    auto watcher = std::make_unique<WaitableEventWatcher>();
    WaitableEventWatcher::EventCallback write_fd_task = BindOnce(&WriteFDWrapper, pipefds_[1], &null_byte_, 1);
    io_runner()->PostTask(
        FROM_HERE, BindOnce(IgnoreResult(&WaitableEventWatcher::StartWatching), Unretained(watcher.get()), &event, std::move(write_fd_task), io_runner()));

    // Queue |event| to signal on |CurrentUIThread::Get()|.
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&event)));

    // Now run the MessageLoop.
    run_loop.Run();

    // StartWatching can move |watcher| to IO thread. Release on IO thread.
    io_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEventWatcher::StopWatching, Owned(std::move(watcher))));
}
