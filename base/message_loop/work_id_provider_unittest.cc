...
template <class Func> void RunTest(const Func& func)
{
...
{
    RunTest([](WorkIdProvider* provider) { EXPECT_EQ(0u, provider->GetWorkId()); });
}
...
{
    RunTest([](WorkIdProvider* provider) {
        provider->SetCurrentWorkIdForTesting(std::numeric_limits<unsigned int>::max());
        provider->IncrementWorkIdForTesting();
        EXPECT_EQ(1u, provider->GetWorkId());
    });
}
