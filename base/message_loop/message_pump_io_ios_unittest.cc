...
protected:
    ...
    {
        MessagePumpIOSForIO::HandleFdIOEvent(watcher->fdref_.get(), kCFFileDescriptorReadCallBack | kCFFileDescriptorWriteCallBack, watcher);
    }
...
TEST_F(MessagePumpIOSForIOTest, DeleteWatcher)
{
    ...
    MessagePumpIOSForIO::FdWatchController* watcher = new MessagePumpIOSForIO::FdWatchController(FROM_HERE);
    DeleteWatcher delegate(watcher);
    pump->WatchFileDescriptor(pipefds_[1], false, MessagePumpIOSForIO::WATCH_READ_WRITE, watcher, &delegate);
...
public:
    StopWatcher(MessagePumpIOSForIO::FdWatchController* controller, MessagePumpIOSForIO* pump, int fd_to_start_watching = -1)
        : BaseWatcher(controller)
        , pump_(pump)
    ...
    void OnFileCanWriteWithoutBlocking(int /* fd */) override
    {
        ...
        if (fd_to_start_watching_ >= 0) {
            pump_->WatchFileDescriptor(fd_to_start_watching_, false, MessagePumpIOSForIO::WATCH_READ_WRITE, controller_, this);
        }
    }
