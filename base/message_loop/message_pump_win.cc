...
DWORD GetSleepTimeoutMs(TimeTicks next_task_time, TimeTicks recent_now = TimeTicks())
{
...
{
    bool succeeded = message_window_.Create(BindRepeating(&MessagePumpForUI::MessageCallback, Unretained(this)));
...
void MessagePumpForUI::ScheduleWork()
{
    ...
    if (g_ui_pump_improvements_win && !in_nested_native_loop_with_application_tasks_) {
    ...
    if (!native_msg_scheduled_.compare_exchange_strong(not_scheduled, true, std::memory_order_relaxed)) {
    ...
    TRACE_EVENT_INSTANT0("base", "Chrome.MessageLoopProblem.MESSAGE_POST_ERROR", TRACE_EVENT_SCOPE_THREAD);
}

void MessagePumpForUI::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
    DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
    ...
    if (in_nested_native_loop_with_application_tasks_ && !native_msg_scheduled_.load(std::memory_order_relaxed)) {
        ScheduleNativeTimer(next_work_info);
    }
}

bool MessagePumpForUI::HandleNestedNativeLoopWithApplicationTasks(bool application_tasks_desired)
{
...
bool MessagePumpForUI::MessageCallback(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result)
{
    DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
    switch (message) {
    case kMsgHaveWork:
...
void MessagePumpForUI::DoRunLoop()
{
    ...
    for (;;) {
        ...
        if (!g_ui_pump_improvements_win || wakeup_state_ != WakeupState::kApplicationTask) {
...
void MessagePumpForUI::WaitForWork(Delegate::NextWorkInfo next_work_info)
{
    ...
    for (DWORD delay = GetSleepTimeoutMs(next_work_info.delayed_run_time, next_work_info.recent_now); delay != 0;
        ...
        if (g_ui_pump_improvements_win) {
            ...
            result = MsgWaitForMultipleObjectsEx(1, &event_handle, delay, QS_ALLINPUT, wait_flags);
            ...
            result = MsgWaitForMultipleObjectsEx(0, nullptr, delay, QS_ALLINPUT, wait_flags);
            DPCHECK(WAIT_FAILED != result);
            if (result == WAIT_OBJECT_0) {
                wakeup_state_ = WakeupState::kNative;
            } else {
        ...
        if (wakeup_state_ == WakeupState::kApplicationTask) {
            ...
            {
                TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::WaitForWork GetQueueStatus");
                if (HIWORD(::GetQueueStatus(QS_SENDMESSAGE)) & QS_SENDMESSAGE)
                    return;
            }
            {
                MSG msg;
                TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::WaitForWork PeekMessage");
            ...
            TRACE_EVENT_INSTANT("base", "MessagePumpForUI::WaitForWork Spurious Wakeup", [&](perfetto::EventContext ctx) {
                ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_chrome_message_pump_for_ui()->set_wait_for_object_result(result);
            });
        }
    }
}
...
void MessagePumpForUI::ScheduleNativeTimer(Delegate::NextWorkInfo next_work_info)
{
    ...
    DCHECK(!g_ui_pump_improvements_win || in_nested_native_loop_with_application_tasks_);
    ...
    UINT delay_msec = strict_cast<UINT>(GetSleepTimeoutMs(next_work_info.delayed_run_time, next_work_info.recent_now));
    if (delay_msec == 0) {
        ScheduleWork();
    } else {
        // TODO(gab): ::SetTimer()'s documentation claims it does this for us.
        // Consider removing this safety net.
        delay_msec = std::clamp(delay_msec, static_cast<UINT>(USER_TIMER_MINIMUM), static_cast<UINT>(USER_TIMER_MAXIMUM));

        // Tell the optimizer to retain the delay to simplify analyzing hangs.
        base::debug::Alias(&delay_msec);
        const UINT_PTR ret = ::SetTimer(message_window_.hwnd(), reinterpret_cast<UINT_PTR>(this), delay_msec, nullptr);
        ...
        TRACE_EVENT_INSTANT0("base", "Chrome.MessageLoopProblem.SET_TIMER_ERROR", TRACE_EVENT_SCOPE_THREAD);
    }
}
...
bool MessagePumpForUI::ProcessNextWindowsMessage()
{
    ...
    bool more_work_is_plausible = false;
    {
...

        {
            ...
            // this frame).
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::ProcessNextWindowsMessage GetQueueStatus");
...

        {
            ...
            TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::ProcessNextWindowsMessage PeekMessage", [&](perfetto::EventContext ctx) {
                perfetto::protos::pbzero::ChromeMessagePump* msg_pump_data = ctx.event()->set_chrome_message_pump();
...
bool MessagePumpForUI::ProcessMessageHelper(const MSG& msg)
{
    ...
    if (msg.message == WM_QUIT) {
        ...
        // unexpected.
        TRACE_EVENT_INSTANT0("base", "Chrome.MessageLoopProblem.RECEIVED_WM_QUIT_ERROR", TRACE_EVENT_SCOPE_THREAD);
        return true;
    }
    ...
    TRACE_EVENT("base,toplevel", "MessagePumpForUI DispatchMessage", [&](perfetto::EventContext ctx) {
        ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_chrome_message_pump_for_ui()->set_message_id(msg.message);
    });
...
bool MessagePumpForUI::ProcessPumpReplacementMessage()
{
    ...
    bool have_message = false;
    {
        ...
        auto scoped_do_work_item = run_state_->delegate->BeginWorkItem();
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::ProcessPumpReplacementMessage PeekMessage");
...
{
    port_.Set(::CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, reinterpret_cast<ULONG_PTR>(nullptr), 1));
...
void MessagePumpForIO::ScheduleWork()
{
    ...
    const BOOL ret = ::PostQueuedCompletionStatus(port_.get(), 0, reinterpret_cast<ULONG_PTR>(this), reinterpret_cast<OVERLAPPED*>(this));
    if (ret)
        return; // Post worked perfectly.

    // See comment in MessagePumpForUI::ScheduleWork() for this error recovery.

    native_msg_scheduled_.store(false, std::memory_order_relaxed); // Clarify that we didn't succeed.
    TRACE_EVENT_INSTANT0("base", "Chrome.MessageLoopProblem.COMPLETION_POST_ERROR", TRACE_EVENT_SCOPE_THREAD);
}

void MessagePumpForIO::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
    DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
...
bool MessagePumpForIO::RegisterIOHandler(HANDLE file_handle, IOHandler* handler)
{
    ...
    HANDLE port = ::CreateIoCompletionPort(file_handle, port_.get(), reinterpret_cast<ULONG_PTR>(handler), 1);
    ...
    if (port == nullptr) {
        ...
        static auto* const crash_key = debug::AllocateCrashKeyString("RegisterIOHandlerError", debug::CrashKeySize::Size32);
...
bool MessagePumpForIO::RegisterJobObject(HANDLE job_handle, IOHandler* handler)
{
    ...
    return ::SetInformationJobObject(job_handle, JobObjectAssociateCompletionPortInformation, &info, sizeof(info)) != FALSE;
}
...
bool MessagePumpForIO::WaitForIOCompletion(DWORD timeout)
{
    ...
    TRACE_EVENT("base,toplevel", "IOHandler::OnIOCompleted", [&](perfetto::EventContext ctx) {
        ctx.event()->set_chrome_message_pump()->set_io_handler_location_iid(
            base::trace_event::InternedSourceLocation::Get(&ctx, base::trace_event::TraceSourceLocation(item.handler->io_handler_location())));
    });

    item.handler.ExtractAsDangling()->OnIOCompleted(item.context.ExtractAsDangling(), item.bytes_transfered, item.error);
...
bool MessagePumpForIO::ProcessInternalIOItem(const IOItem& item)
{
    ...
    if (reinterpret_cast<void*>(this) == reinterpret_cast<void*>(item.context.get())
        && reinterpret_cast<void*>(this) == reinterpret_cast<void*>(item.handler.get())) {
