...
class BASE_EXPORT MessagePumpFuchsia : public MessagePump, public WatchableIOMessagePumpPosix {
public:
    // Implemented by callers to receive notifications of handle & fd events.
    class ZxHandleWatcher {
    public:
        virtual void OnZxHandleSignalled(zx_handle_t handle, zx_signals_t signals) = 0;
    ...
    public:
        ...
        const Location& created_from_location()
        {
    ...
    protected:
        ...
        {
            return async_wait_t::handler != nullptr;
        }

        static void HandleSignal(async_dispatcher_t* async, async_wait_t* wait, zx_status_t status, const zx_packet_signal_t* signal);
    ...
    class FdWatchController : public FdWatchControllerInterface, public ZxHandleWatchController, public ZxHandleWatcher {
    public:
    ...
    bool WatchZxHandle(zx_handle_t handle, bool persistent, zx_signals_t signals, ZxHandleWatchController* controller, ZxHandleWatcher* delegate);
...
private:
    ...
    struct RunState {
        explicit RunState(Delegate* delegate_in)
