...
class BASE_EXPORT MessagePumpKqueue : public MessagePump, public WatchableIOMessagePumpPosix {
public:
    ...
    protected:
        ...
        void Init(WeakPtr<MessagePumpKqueue> pump, int fd, int mode, FdWatcher* watcher);
        void Reset();
    ...
    protected:
        ...
        void Init(WeakPtr<MessagePumpKqueue> pump, mach_port_t port, MachPortWatcher* watcher);
        void Reset();
    ...
    TimeTicks AdjustDelayedRunTime(TimeTicks earliest_time, TimeTicks run_time, TimeTicks latest_time) override;
    ...
    bool WatchMachReceivePort(mach_port_t port, MachPortWatchController* controller, MachPortWatcher* delegate);
...
private:
    ...
    // stored in |events_|.
    bool DoInternalWork(Delegate* delegate, Delegate::NextWorkInfo* next_work_info);
    ...
    void MaybeUpdateWakeupTimer(const base::TimeTicks& wakeup_time, base::TimeDelta leeway);

    void SetWakeupTimerEvent(const base::TimeTicks& wakeup_time, base::TimeDelta leeway, kevent64_s* timer_event);
