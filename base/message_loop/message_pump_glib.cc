...
static_assert(G_PRIORITY_DEFAULT < kPriorityFdWatch && kPriorityFdWatch < kPriorityWork, "Wrong priorities are set for event sources!");
...
int GetTimeIntervalMilliseconds(TimeTicks next_task_time)
{
    ...
    auto timeout_ms = (next_task_time - TimeTicks::Now()).InMillisecondsRoundedUp();
...
gboolean WorkSourceDispatch(GSource* source, GSourceFunc unused_func, gpointer unused_data)
{
...
GSourceFuncs g_work_source_funcs = { WorkSourcePrepare, WorkSourceCheck, WorkSourceDispatch, WorkSourceFinalize };
...
GSourceFuncs g_observer_funcs = { ObserverPrepare, ObserverCheck, nullptr, ObserverFinalize };
...
gboolean FdWatchSourceDispatch(GSource* gsource, GSourceFunc unused_func, gpointer unused_data)
{
    auto* source = static_cast<FdWatchSource*>(gsource);
...
GSourceFuncs g_fd_watch_source_funcs = { FdWatchSourcePrepare, FdWatchSourceCheck, FdWatchSourceDispatch, FdWatchSourceFinalize };

} // namespace

struct MessagePumpGlib::RunState {
    explicit RunState(Delegate* delegate)
        : delegate(delegate)
    {
        CHECK(delegate);
    }
...
MessagePumpGlib::MessagePumpGlib()
    : state_(nullptr)
    , wakeup_gpollfd_(std::make_unique<GPollFD>())
{
    DCHECK(!g_main_context_get_thread_default());
    if (RunningOnMainThread()) {
        context_ = g_main_context_default();
    } else {
        owned_context_ = std::unique_ptr<GMainContext, GMainContextDeleter>(g_main_context_new());
    ...
    observer_source_ = std::unique_ptr<GSource, GSourceDeleter>(g_source_new(&g_observer_funcs, sizeof(ObserverSource)));
    static_cast<ObserverSource*>(observer_source_.get())->pump = this;
    g_source_attach(observer_source_.get(), context_);

    work_source_ = std::unique_ptr<GSource, GSourceDeleter>(g_source_new(&g_work_source_funcs, sizeof(WorkSource)));
...
bool MessagePumpGlib::FdWatchController::InitOrUpdate(int fd, int mode, FdWatcher* watcher)
{
...
bool MessagePumpGlib::WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* watcher)
{
    DCHECK_GE(fd, 0);
    DCHECK(controller);
    DCHECK(watcher);
...
int MessagePumpGlib::HandlePrepare()
{
    ...
    const int next_wakeup_millis = GetTimeIntervalMilliseconds(state_->next_work_info.delayed_run_time);
...
bool MessagePumpGlib::HandleCheck()
{
    ...
    if (wakeup_gpollfd_->revents & G_IO_IN) {
        ...
        DCHECK((num_bytes == 1 && msg[0] == '!') || (num_bytes == 2 && msg[0] == '!' && msg[1] == '!'));
    ...
    if (GetTimeIntervalMilliseconds(state_->next_work_info.delayed_run_time) == 0) {
        return true;
    }
...
void MessagePumpGlib::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
...
void MessagePumpGlib::NestIfRequired()
{
    ...
    if (state_->native_loop_do_work_item.IsNull() && state_->g_depth_on_iteration.has_value() && g_main_depth() != state_->g_depth_on_iteration.value()) {
        RegisterNested();
    }
}
