...
BASE_FEATURE(kUsePollForMessagePumpEpoll, "UsePollForMessagePumpEpoll", base::FEATURE_DISABLED_BY_DEFAULT);

// A MessagePump implementation suitable for I/O message loops on Linux-based
// systems with epoll API support.
class BASE_EXPORT MessagePumpEpoll : public MessagePump, public WatchableIOMessagePumpPosix {
    class Interest;
...
public:
    ...
    private:
        ...
        void set_pump(WeakPtr<MessagePumpEpoll> pump)
        {
            pump_ = std::move(pump);
        }
        const scoped_refptr<Interest>& interest() const
        {
            return interest_;
        }
    ...
    bool WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* watcher);
...
private:
    ...
    struct EpollEventEntry {
        ...
        {
            EpollHistory info = { .stack_trace = base::debug::StackTrace(), .event = event };
    ...
    void HandleEvent(int fd, bool can_read, bool can_write, FdWatchController* controller);
    void HandleWakeUp();
