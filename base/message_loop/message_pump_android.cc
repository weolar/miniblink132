...
#define NO_INSTRUMENT_STACK_ALIGN __attribute__((force_align_arg_pointer, no_instrument_function))
#else
#define NO_INSTRUMENT_STACK_ALIGN __attribute__((no_instrument_function))
#endif

NO_INSTRUMENT_STACK_ALIGN int NonDelayedLooperCallback(int fd, int events, void* data)
{
...
NO_INSTRUMENT_STACK_ALIGN int DelayedLooperCallback(int fd, int events, void* data)
{
    if (events & ALOOPER_EVENT_HANGUP)
        return 0;
    ...
    : env_(base::android::AttachCurrentThread())
{
    ...
    delayed_fd_ = checked_cast<int>(timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC));
    ...
    ALooper_addFd(looper_, non_delayed_fd_, 0, ALOOPER_EVENT_INPUT, &NonDelayedLooperCallback, reinterpret_cast<void*>(this));
    ALooper_addFd(looper_, delayed_fd_, 0, ALOOPER_EVENT_INPUT, &DelayedLooperCallback, reinterpret_cast<void*>(this));
}
...
void MessagePumpAndroid::DoNonDelayedLooperWork(bool do_idle_work)
{
    ...
    do {
        ...
        if (is_type_ui_ && next_work_info.is_immediate() && InputHintChecker::HasInput()) {
...
void MessagePumpAndroid::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
    if (ShouldQuit())
        return;

    if (delayed_scheduled_time_ && *delayed_scheduled_time_ == next_work_info.delayed_run_time) {
        return;
    }

    DCHECK(!next_work_info.is_immediate());
    delayed_scheduled_time_ = next_work_info.delayed_run_time;
    int64_t nanos = next_work_info.delayed_run_time.since_origin().InNanoseconds();
    struct itimerspec ts;
    ts.it_interval.tv_sec = 0; // Don't repeat.
    ts.it_interval.tv_nsec = 0;
    ts.it_value.tv_sec = static_cast<time_t>(nanos / TimeTicks::kNanosecondsPerSecond);
