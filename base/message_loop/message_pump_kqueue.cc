...
// is resolved.
BASE_FEATURE(kBatchNativeEventsInMessagePumpKqueue, "BatchNativeEventsInMessagePumpKqueue", base::FEATURE_DISABLED_BY_DEFAULT);
...
MessagePumpKqueue::FdWatchController::FdWatchController(const Location& from_here)
...
void MessagePumpKqueue::FdWatchController::Init(WeakPtr<MessagePumpKqueue> pump, int fd, int mode, FdWatcher* watcher)
{
    DCHECK_NE(fd, -1);
    DCHECK(!watcher_);
    DCHECK(watcher);
...
MessagePumpKqueue::MachPortWatchController::MachPortWatchController(const Location& from_here)
...
void MessagePumpKqueue::MachPortWatchController::Init(WeakPtr<MessagePumpKqueue> pump, mach_port_t port, MachPortWatcher* watcher)
{
    DCHECK(!watcher_);
    DCHECK(watcher);
    DCHECK(pump);
    ...
    , weak_factory_(this)
{
    ...
    kern_return_t kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, base::apple::ScopedMachReceiveRight::Receiver(wakeup_).get());
...
{
    g_use_batched_version.store(base::FeatureList::IsEnabled(kBatchNativeEventsInMessagePumpKqueue), std::memory_order_relaxed);
...
void MessagePumpKqueue::ScheduleWork()
{
    ...
    if (kr != KERN_SUCCESS) {
        ...
        // to avoid leaking.
        MACH_DLOG_IF(ERROR, (kr & ~MACH_MSG_IPC_SPACE) != MACH_SEND_NO_BUFFER, kr) << "mach_msg_send";
        mach_msg_destroy(&message.header);
    }
}

void MessagePumpKqueue::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info)
{
    // Nothing to do. This MessagePump uses DoWork().
}

bool MessagePumpKqueue::WatchMachReceivePort(mach_port_t port, MachPortWatchController* controller, MachPortWatcher* delegate)
{
    ...
    if (controller->port() != MACH_PORT_NULL) {
        DLOG(ERROR) << "Cannot use the same MachPortWatchController while it is active";
        return false;
    }
...
TimeTicks MessagePumpKqueue::AdjustDelayedRunTime(TimeTicks earliest_time, TimeTicks run_time, TimeTicks latest_time)
{
...
bool MessagePumpKqueue::WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* delegate)
{
    DCHECK_GE(fd, 0);
    DCHECK(controller);
    DCHECK(delegate);
    ...
    int rv = HANDLE_EINTR(kevent64(kqueue_.get(), events.data(), checked_cast<int>(events.size()), nullptr, 0, 0, nullptr));
    if (rv < 0) {
...
void MessagePumpKqueue::SetWakeupTimerEvent(const base::TimeTicks& wakeup_time, base::TimeDelta leeway, kevent64_s* timer_event)
{
...
bool MessagePumpKqueue::StopWatchingMachPort(MachPortWatchController* controller)
{
...
bool MessagePumpKqueue::StopWatchingFileDescriptor(FdWatchController* controller)
{
    ...
    for (IDMap<FdWatchController*, uint64_t>::iterator it(&fd_controllers_); !it.IsAtEnd(); it.Advance()) {
...
bool MessagePumpKqueue::DoInternalWork(Delegate* delegate, Delegate::NextWorkInfo* next_work_info)
{
    ...
    if (!immediate) {
        MaybeUpdateWakeupTimer(next_work_info->delayed_run_time, next_work_info->leeway);
        DCHECK_EQ(scheduled_wakeup_time_, next_work_info->delayed_run_time);
        delegate->BeforeWait();
    }

    int rv = HANDLE_EINTR(kevent64(kqueue_.get(), nullptr, 0, events_.data(), checked_cast<int>(events_.size()), flags, nullptr));
    if (rv == 0) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (event->filter == EVFILT_READ || event->filter == EVFILT_WRITE) {
            ...
            if (event->filter == EVFILT_READ) {
                fd_watcher->OnFileCanReadWithoutBlocking(static_cast<int>(event->ident));
            } else if (event->filter == EVFILT_WRITE) {
                fd_watcher->OnFileCanWriteWithoutBlocking(static_cast<int>(event->ident));
            }
            ...
            if (!KqueueTimersSpuriouslyWakeUp() && !subtle::ScopedTimeClockOverrides::overrides_active()) {
...
void MessagePumpKqueue::MaybeUpdateWakeupTimer(const base::TimeTicks& wakeup_time, base::TimeDelta leeway)
{
