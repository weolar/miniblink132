...
TEST(VersionTest, Compare)
{
    ...
    for (const auto& i : cases) {
        ...
        EXPECT_EQ(lhs.CompareToWildcardString(i.rhs), i.expected) << i.lhs << " ? " << i.rhs;
        EXPECT_EQ(rhs.CompareToWildcardString(i.lhs), -i.expected) << i.lhs << " ? " << i.rhs;
...
TEST(VersionTest, IsValidWildcardString)
{
    ...
    for (const auto& i : cases) {
        EXPECT_EQ(base::Version::IsValidWildcardString(i.version), i.expected) << i.version << "?" << i.expected;
    }
}
