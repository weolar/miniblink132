...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    base::Pickle pickle = base::Pickle::WithUnownedBuffer(UNSAFE_BUFFERS(base::span(data, size)));
    ...
    for (int i = 0; i < kIterations; i++) {
        ...
        case 14: {
            const char* data_result = nullptr;
            int read_length = data_provider.ConsumeIntegralInRange(0, kMaxReadLength);
            std::ignore = iter.ReadBytes(&data_result, static_cast<size_t>(read_length));
            break;
        }
        case 15: {
            size_t result = 0;
        ...
        case 16: {
            std::ignore = iter.SkipBytes(static_cast<size_t>(data_provider.ConsumeIntegralInRange(0, kMaxSkipBytes)));
            break;
        }
        }
    }
