...
{
    if (d_name.size() >= NAME_MAX || !std::ranges::all_of(d_name, &IsAsciiDigit<char>)) {
        return 0;
    }
...
bool ReadProcFileToTrimmedStringPairs(pid_t pid, std::string_view filename, StringPairs* key_value_pairs)
{
...
size_t ReadProcStatusAndGetKbFieldAsSizeT(pid_t pid, std::string_view field)
{
    ...
    for (const auto& pair : pairs) {
        ...
        std::vector<std::string_view> split_value_str = SplitStringPiece(value_str, " ", TRIM_WHITESPACE, SPLIT_WANT_ALL);
...
bool ReadProcStatusAndGetFieldAsUint64(pid_t pid, std::string_view field, uint64_t* result)
{
    StringPairs pairs;
    if (!ReadProcFileToTrimmedStringPairs(pid, "status", &pairs)) {
        return false;
...
bool ParseProcStats(const std::string& stats_data, std::vector<std::string>* proc_stats)
{
    ...
    // Split the rest.
    std::vector<std::string> other_stats = SplitString(stats_data.substr(close_parens_idx + 2), " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
std::optional<int64_t> GetProcStatsFieldAsOptionalInt64(base::span<const std::string> proc_stats, ProcStatsFields field_num)
{
    int64_t value;
...
int64_t ReadStatFileAndGetFieldAsInt64(const FilePath& stat_file, ProcStatsFields field_num)
{
...
TimeDelta GetUserCpuTimeSinceBoot()
{
    ...
    std::vector<std::string> cpu = SplitString(cpu_it->second, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
