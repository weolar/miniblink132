...
{
    static_assert(__builtin_constant_p(PTHREAD_STACK_MIN), "Always constant on mac");
}
...
public:
    PosixSpawnAttr()
    {
        DPSXCHECK(posix_spawnattr_init(&attr_));
    }

    ~PosixSpawnAttr()
    {
        DPSXCHECK(posix_spawnattr_destroy(&attr_));
    }
...
bool GetAppOutputInternal(const std::vector<std::string>& argv, GetAppOutputOptions* gao_options)
{
    ...
    do {
        ...
        read_this_pass = HANDLE_EINTR(read(read_fd.get(), &(*output)[total_bytes_read], kBufferSize));
        if (read_this_pass >= 0) {
...
Process LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options)
{
    ...
    MachPortRendezvousServerMac::AddFeatureStateToEnvironment(new_environment_map);
#else
    ...
    char** new_environ = options.clear_environment ? &empty_environ : *_NSGetEnviron();
    if (!new_environment_map.empty()) {
        owned_environ = internal::AlterEnvironment(new_environ, new_environment_map);
        new_environ = owned_environ.data();
    }

    const char* executable_path = !options.real_path.empty() ? options.real_path.value().c_str() : argv_cstr[0];
    ...
    pid_t pid;
    {
        const bool has_mach_ports_for_rendezvous = !options.mach_ports_for_rendezvous.empty();
#if BUILDFLAG(IS_IOS)
        ...
        bool needs_rendezvous_lock = has_mach_ports_for_rendezvous || options.process_requirement;

        AutoLockMaybe rendezvous_lock(needs_rendezvous_lock ? &MachPortRendezvousServerMac::GetInstance()->GetLock() : nullptr);
#endif
        // Use posix_spawnp as some callers expect to have PATH consulted.
        rv = posix_spawnp(&pid, executable_path, file_actions.get(), attr.get(), &argv_cstr[0], new_environ);
        ...
        if (needs_rendezvous_lock) {
            if (rv == 0) {
                ...
                if (has_mach_ports_for_rendezvous) {
                    MachPortRendezvousServerMac::GetInstance()->RegisterPortsForPid(pid, options.mach_ports_for_rendezvous);
                }
                if (options.process_requirement) {
                    MachPortRendezvousServerMac::GetInstance()->SetProcessRequirementForPid(pid, *options.process_requirement);
                }
            } else {
    ...
    if (rv != 0) {
        DLOG(ERROR) << "posix_spawnp(" << executable_path << "): -" << rv << " " << strerror(rv);
        return Process();
    }
...
bool GetAppOutput(const std::vector<std::string>& argv, std::string* output)
{
    GetAppOutputOptions options;
    options.output = output;
    return GetAppOutputInternal(argv, &options) && options.exit_code == EXIT_SUCCESS;
}
