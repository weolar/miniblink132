...
TEST(SetProcTitleLinuxTest, Simple)
{
    ...
    EXPECT_TRUE(base::EndsWith(ReadCmdline(), " a cute cat", base::CompareCase::SENSITIVE)) << ReadCmdline();
...
TEST(SetProcTitleLinuxTest, Long)
{
    ...
    EXPECT_TRUE(base::StartsWith(ReadCmdline(), "long cat is l00000000", base::CompareCase::SENSITIVE)) << ReadCmdline();
}
...
TEST(SetProcTitleLinuxTest, GetModuleForAddressWorksWithSetProcTitle)
{
    ...
    const base::ModuleCache::Module* module = module_cache.GetModuleForAddress(reinterpret_cast<uintptr_t>(&ReadCmdline));
