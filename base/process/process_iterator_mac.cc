...
bool ProcessIterator::CheckForNextProcess()
{
    ...
    for (; index_of_kinfo_proc_ < kinfo_procs_.size(); ++index_of_kinfo_proc_) {
        ...
        entry_.cmd_line_args_ = SplitString(data, delimiters, KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
        ...
        if (last_slash == std::string::npos)
            ...
            entry_.exe_file_.assign(data, last_slash + 1, exec_name_end - last_slash - 1);
...
{
    const bool name_match = use_prefix_match_ ? base::StartsWith(entry().exe_file(), executable_name_) : executable_name_ == entry().exe_file();
