...
// failed.
base::expected<TimeDelta, ProcessCPUUsageError> ParseTotalCPUTimeFromStats(base::span<const std::string> proc_stats)
{
    const std::optional<int64_t> utime = internal::GetProcStatsFieldAsOptionalInt64(proc_stats, internal::VM_UTIME);
    if (utime.value_or(-1) < 0) {
        return base::unexpected(ProcessCPUUsageError::kSystemError);
    }
    const std::optional<int64_t> stime = internal::GetProcStatsFieldAsOptionalInt64(proc_stats, internal::VM_STIME);
    if (stime.value_or(-1) < 0) {
        return base::unexpected(ProcessCPUUsageError::kSystemError);
    }
    const TimeDelta cpu_time = internal::ClockTicksToTimeDelta(base::ClampAdd(utime.value(), stime.value()));
...
{
    return internal::ReadProcStatsAndGetFieldAsSizeT(process_, internal::VM_RSS) * checked_cast<size_t>(getpagesize());
}
...
base::expected<TimeDelta, ProcessCPUUsageError> ProcessMetrics::GetCumulativeCPUUsage()
{
    ...
    if (!internal::ReadProcStats(process_, &buffer) || !internal::ParseProcStats(buffer, &proc_stats)) {
        return base::unexpected(ProcessCPUUsageError::kSystemError);
    }
...
bool ProcessMetrics::GetCumulativeCPUUsagePerThread(CPUUsagePerThread& cpu_per_thread)
{
    ...
    internal::ForEachProcessTask(process_, [&cpu_per_thread](PlatformThreadId tid, const FilePath& task_path) {
        FilePath thread_stat_path = task_path.Append("stat");

        std::string buffer;
        std::vector<std::string> proc_stats;
        if (!internal::ReadProcFile(thread_stat_path, &buffer) || !internal::ParseProcStats(buffer, &proc_stats)) {
            return;
        }

        const base::expected<TimeDelta, ProcessCPUUsageError> thread_time = ParseTotalCPUTimeFromStats(proc_stats);
...
{
    return internal::ReadProcStatusAndGetKbFieldAsSizeT(process_, "VmSwap") * 1024;
}
...
int ProcessMetrics::GetOpenFdSoftLimit() const
{
    ...
    for (const auto& line : SplitStringPiece(limits_contents, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
...
size_t GetSystemCommitChargeFromMeminfo(const SystemMemoryInfoKB& meminfo)
{
    ...
    return ClampedNumeric<size_t>(meminfo.total) - meminfo.free - meminfo.buffers - meminfo.cached;
}
...
bool ParseProcMeminfo(std::string_view meminfo_data, SystemMemoryInfoKB* meminfo)
{
    ...
    for (std::string_view line : SplitStringPiece(meminfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
        std::vector<std::string_view> tokens = SplitStringPiece(line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
bool ParseProcVmstat(std::string_view vmstat_data, VmStatInfo* vmstat)
{
    ...
    for (std::string_view line : SplitStringPiece(vmstat_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
        std::vector<std::string_view> tokens = SplitStringPiece(line, " ", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
        if (tokens.size() != 2)
            continue;
...
bool GetSystemDiskInfo(SystemDiskInfo* diskinfo)
{
    ...
    std::vector<std::string_view> diskinfo_lines = SplitStringPiece(diskinfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
    ...
    for (std::string_view line : diskinfo_lines) {
        std::vector<std::string_view> disk_fields = SplitStringPiece(line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
Value::Dict SwapInfo::ToDict() const
{
    ...
    double ratio = compr_data_size ? static_cast<double>(orig_data_size) / static_cast<double>(compr_data_size) : 0;
...
bool ParseZramMmStat(std::string_view mm_stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<std::string_view> tokens = SplitStringPiece(mm_stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 7) {
        DLOG(WARNING) << "zram mm_stat: tokens: " << tokens.size() << " malformed line: " << mm_stat_data;
        return false;
    }
...
bool ParseZramStat(std::string_view stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<std::string_view> tokens = SplitStringPiece(stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 11) {
...
void GetFdInfoFromPid(pid_t pid, std::map<unsigned int, struct DrmFdInfo>& fdinfo_table)
{
    const FilePath pid_path = FilePath("/proc").AppendASCII(base::NumberToString(pid));
    ...
    for (; dir_reader.Next();) {
        ...
        const FilePath fdinfo_path = pid_path.AppendASCII("fdinfo").AppendASCII(name);
...
bool GetGraphicsMemoryInfo(GraphicsMemoryInfoKB* gpu_meminfo)
{
    ...
    if (ReadFileToStringNonBlocking(geminfo_path, &geminfo_data)) {
        ...
        int num_res = sscanf(geminfo_data.c_str(), "%d objects, %" SCNd64 " bytes", &gpu_objects, &gpu_memory_size);
        if (num_res == 2) {
...
int ProcessMetrics::GetIdleWakeupsPerSecond()
{
    ...
    return internal::ReadProcStatusAndGetFieldAsUint64(process_, kSwitchStat, &num_switches) ? CalculateIdleWakeupsPerSecond(num_switches) : 0;
}
