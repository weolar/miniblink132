...
// in most instances.
TerminationStatus WaitForChildTermination(ProcessHandle handle, int* exit_code)
{
    ...
    do {
        ...
        waited += kInterval;
    } while (status == TERMINATION_STATUS_STILL_RUNNING && waited < TestTimeouts::action_max_timeout());
...
TEST_F(ProcessUtilTest, SpawnChild)
{
    ...
    int exit_code;
    EXPECT_TRUE(process.WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), &exit_code));
}
...
TEST_F(ProcessUtilTest, MAYBE_GetTerminationStatusExit)
{
    ...
    int exit_code = 42;
    EXPECT_EQ(TERMINATION_STATUS_STILL_RUNNING, GetTerminationStatus(process.Handle(), &exit_code));
    EXPECT_EQ(kExpectedStillRunningExitCode, exit_code);

    SignalChildren(signal_file.c_str());
    exit_code = 42;
    TerminationStatus status = WaitForChildTermination(process.Handle(), &exit_code);
...
TEST_F(ProcessUtilTest, DISABLED_DuplicateTransferAndClonePaths_Fail)
{
    ...
    options.paths_to_transfer.push_back({ FilePath(kPersistedDataDirectoryPath), OpenDirectoryHandle(tmpdir.GetPath()).TakeChannel().release() });
...
TEST_F(ProcessUtilTest, DISABLED_OverlappingPaths_Fail)
{
    ...
    options.paths_to_transfer.push_back(
        { FilePath(kPersistedDataDirectoryPath).Append("staged"), OpenDirectoryHandle(tmpdir.GetPath()).TakeChannel().release() });
...
TEST_F(ProcessUtilTest, TransferHandleToPath)
{
    ...
    zx::channel tmp_channel = OpenDirectoryHandle(new_tmpdir.GetPath()).TakeChannel();

    ASSERT_TRUE(tmp_channel.is_valid());
    LaunchOptions options;
    options.paths_to_clone.push_back(FilePath("/tmp"));
    options.paths_to_transfer.push_back({ FilePath("/foo"), tmp_channel.release() });
    options.spawn_flags = FDIO_SPAWN_CLONE_STDIO;
...
int CheckOnlyOnePathExists(std::string_view expected_path)
{
    ...
    FileEnumerator enumerator(FilePath("/"), false, FileEnumerator::FILES | FileEnumerator::DIRECTORIES);
    for (auto path = enumerator.Next(); !path.empty(); path = enumerator.Next()) {
        paths.push_back(std::move(path));
    }

    EXPECT_EQ(paths.size(), is_expected_path_tmp ? 1u : 2u);
    EXPECT_EQ(paths[0], FilePath(expected_path)) << "Clone policy violation: found non-tmp directory " << paths[0].MaybeAsASCII();
...
{
    zx::handle handles[2];
    zx_status_t result = zx_channel_create(0, handles[0].reset_and_get_address(), handles[1].reset_and_get_address());
    ...
    CommandLine command_line(FilePath(FILE_PATH_LITERAL("💩magical_filename_that_will_never_exist_ever")));
    ...
    EXPECT_EQ(zx_object_wait_one(handles[1].get(), ZX_CHANNEL_PEER_CLOSED, 0, nullptr), ZX_OK);
...
TEST_F(ProcessUtilTest, HandlesToTransferClosedOnBadPathToMapFailure)
{
    ...
    options.paths_to_clone.emplace_back("💩magical_path_that_will_never_exist_ever");
...
TEST_F(ProcessUtilTest, FuchsiaProcessNameSuffix)
{
    ...
    zx_status_t status = zx_object_get_property(process.Handle(), ZX_PROP_NAME, name, name_size);
    ASSERT_EQ(status, ZX_OK);
    EXPECT_EQ(std::string(name), CommandLine::ForCurrentProcess()->GetProgram().BaseName().value() + "#test");
}
...
MULTIPROCESS_TEST_MAIN(CheckCwdProcess)
{
    ...
    RepeatingClosure signal_event = BindRepeating(&WaitableEvent::Signal, Unretained(&event));
    ...
    task_runner->PostTask(FROM_HERE, BindOnce(IgnoreResult(&SetCurrentDirectory), home_dir));
...
TEST_F(ProcessUtilTest, EnsureTerminationUndying)
{
    ...
    bool did_exit = child_process.WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), nullptr);
...
TEST_F(ProcessUtilTest, EnsureTerminationGracefulExit)
{
    ...
    child_process.Duplicate().WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), nullptr);

    EnsureProcessTerminated(child_process.Duplicate());

    // Verify that the process is really, truly gone.
    EXPECT_TRUE(child_process.WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), nullptr));
}
...
TEST_F(ProcessUtilTest, LaunchAsUser)
{
    ...
    EXPECT_TRUE(LaunchProcess(MakeCmdLine("SimpleChildProcess"), options).IsValid());
}
...
TEST_F(ProcessUtilTest, LaunchDisablingCetCompat)
{
    ...
    EXPECT_TRUE(LaunchProcess(MakeCmdLine("ChildVerifiesCetDisabled"), options).IsValid());
}
...
{
    std::string handle_value_string = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kEventToTriggerHandleSwitch);
    ...
    WaitableEvent event(win::ScopedHandle(reinterpret_cast<HANDLE>(handle_value_uint64)));
...
TEST_F(ProcessUtilTest, InheritSpecifiedHandles)
{
    ...
    WaitableEvent event(win::ScopedHandle(CreateEvent(&security_attributes, true, false, NULL)));
    LaunchOptions options;
    options.handles_to_inherit.emplace_back(event.handle());

    CommandLine cmd_line = MakeCmdLine("TriggerEventChildProcess");
    cmd_line.AppendSwitchASCII(kEventToTriggerHandleSwitch, NumberToString(reinterpret_cast<uint64_t>(event.handle())));
...
int change_fdguard_np(int fd, const guardid_t* guard, unsigned int guardflags, const guardid_t* nguard, unsigned int nguardflags, int* fdflagsp);
...
MULTIPROCESS_TEST_MAIN(ProcessUtilsLeakFDChildProcess)
{
    ...
    int written = HANDLE_EINTR(write(write_pipe, &num_open_files, sizeof(num_open_files)));
...
int ProcessUtilTest::CountOpenFDsInChild()
{
    ...
    Process process = SpawnChildWithOptions("ProcessUtilsLeakFDChildProcess", options);
...
TEST_F(ProcessUtilTest, FDRemappingIncludesStdio)
{
    ...
    File temp_file(temp_file_path, File::FLAG_CREATE_ALWAYS | File::FLAG_READ | File::FLAG_WRITE);
...
{
    zx_handle_t handle = zx_take_startup_handle(PA_HND(PA_USER0, kStartupHandleId));
    CHECK_NE(ZX_HANDLE_INVALID, handle);

    // Write to the pipe so the parent process can observe output.
    size_t bytes_written = 0;
    zx_status_t result = zx_socket_write(handle, 0, &kPipeValue, sizeof(kPipeValue), &bytes_written);
...
TEST_F(ProcessUtilTest, LaunchWithHandleTransfer)
{
    ...
    zx_status_t result = zx_socket_create(ZX_SOCKET_STREAM, &handles[0], &handles[1]);
    ASSERT_EQ(ZX_OK, result);

    // Launch the test process, and pass it one end of the pipe.
    LaunchOptions options;
    options.handles_to_transfer.push_back({ PA_HND(PA_USER0, kStartupHandleId), handles[0] });
    ...
    result
        = zx_object_wait_one(handles[1], ZX_SOCKET_READABLE | ZX_SOCKET_PEER_CLOSED, (TimeTicks::Now() + TestTimeouts::action_timeout()).ToZxTime(), &signals);
    ASSERT_EQ(ZX_OK, result);
...
public:
    explicit WriteToPipeDelegate(int fd)
        : fd_(fd)
    {
    }
...
std::string TestLaunchProcess(const CommandLine& cmdline, const EnvironmentMap& env_changes, const bool clear_environment, const int clone_flags)
{
    LaunchOptions options;
    options.wait = true;
...
TEST_F(ProcessUtilTest, LaunchProcess)
{
    ...
    const CommandLine kPrintEnvCommand(CommandLine::StringVector({ test_helper_path_.value(), FILE_PATH_LITERAL("-e"), kBaseTest }));
    std::unique_ptr<Environment> env = Environment::Create();

    EnvironmentMap env_changes;
    env_changes[kBaseTest] = FILE_PATH_LITERAL("bar");
    EXPECT_EQ("bar", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));
    env_changes.clear();

    EXPECT_TRUE(env->SetVar("BASE_TEST", "testing"));
    EXPECT_EQ("testing", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));

    env_changes[kBaseTest] = FilePath::StringType();
    EXPECT_EQ("", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));

    env_changes[kBaseTest] = FILE_PATH_LITERAL("foo");
    EXPECT_EQ("foo", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));

    env_changes.clear();
    EXPECT_TRUE(env->SetVar("BASE_TEST", kLargeString));
    EXPECT_EQ(std::string(kLargeString), TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));

    env_changes[kBaseTest] = FILE_PATH_LITERAL("wibble");
    EXPECT_EQ("wibble", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, no_clone_flags));

#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    // Test a non-trival value for clone_flags.
    EXPECT_EQ("wibble", TestLaunchProcess(kPrintEnvCommand, env_changes, no_clear_environ, CLONE_FS));
#endif // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
...
{
    if (!PathExists(FilePath("/proc/self/ns/user")) || !PathExists(FilePath("/proc/self/ns/pid"))) {
...
MULTIPROCESS_TEST_MAIN(ProcessUtilTest_ProcessRequirement_ChildFailedValidation)
{
...
TEST_F(ProcessUtilTest, ProcessRequirement)
{
    ...
    // default.
    test::ScopedFeatureList scoped_feature_list(kMachPortRendezvousEnforcePeerRequirements);

    apple::ScopedMachReceiveRight port;
    kern_return_t kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, apple::ScopedMachReceiveRight::Receiver(port).get());
    ...
    options.process_requirement = mac::ProcessRequirement::NeverMatchesForTesting();

    Process child = SpawnChildWithOptions("ProcessUtilTest_ProcessRequirement_ChildFailedValidation", options);

    int exit_code;
    ASSERT_TRUE(WaitForMultiprocessTestChildExit(child, TestTimeouts::action_timeout(), &exit_code));
