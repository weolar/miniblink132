...
#if BUILDFLAG(IS_CHROMEOS)
BASE_FEATURE(kOneGroupPerRenderer, "OneGroupPerRenderer",
...
struct CGroups {
    ...
    CGroups()
    {
        ...
        enabled = PathIsCGroupFileSystem(foreground_file) && PathIsCGroupFileSystem(background_file);
        ...
        group_prefix_token = StrCat({ kCgroupPrefix, UnguessableToken::Create().ToString(), "-" });
...
{
    int64_t start_ticks = is_current() ? internal::ReadProcSelfStatsAndGetFieldAsInt64(internal::VM_STARTTIME)
...
Process::Priority Process::GetPriority() const
{
    ...
    return GetOSPriority() == kBackgroundPriority ? Priority::kBestEffort : Priority::kUserBlocking;
}
...
bool Process::SetPriority(Priority priority)
{
    ...
    if (PlatformThreadChromeOS::IsThreadsBgFeatureEnabled()) {
        DCHECK_CALLED_ON_VALID_SEQUENCE(PlatformThreadChromeOS::GetCrossProcessThreadPrioritySequenceChecker());

        int process_id = process_;
        bool background = priority == Priority::kBestEffort;
        internal::ForEachProcessTask(process_, [process_id, background](PlatformThreadId tid, const FilePath& path) {
            PlatformThreadChromeOS::SetThreadBackgrounded(process_id, tid, background);
        });
    }

    if (CGroups::Get().enabled) {
        std::string pid = NumberToString(process_);
        const FilePath file = priority == Priority::kBestEffort ? CGroups::Get().background_file : CGroups::Get().GetForegroundCgroupFile(unique_token_);
    ...
    int priority_value = priority == Priority::kBestEffort ? kBackgroundPriority : kForegroundPriority;
    int result = setpriority(PRIO_PROCESS, static_cast<id_t>(process_), priority_value);
    DPCHECK(result == 0);
    return result == 0;
}
...
Process::Priority GetProcessPriorityCGroup(std::string_view cgroup_contents)
{
    ...
    std::vector<std::string_view> lines = SplitStringPiece(cgroup_contents, "\n", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    for (const auto& line : lines) {
        std::vector<std::string_view> fields = SplitStringPiece(line, ":", TRIM_WHITESPACE, SPLIT_WANT_ALL);
        if (fields.size() != 3U) {
            NOTREACHED();
        }
...
ProcessId Process::GetPidInNamespace() const
{
    ...
    for (const auto& pair : pairs) {
        ...
        if (key == "NSpid") {
            std::vector<std::string_view> split_value_str = SplitStringPiece(value_str, "\t", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
bool Process::IsSeccompSandboxed()
{
    ...
    if (!internal::ReadProcStatusAndGetFieldAsUint64(process_, "Seccomp", &seccomp_value)) {
        return false;
    }
...
void Process::InitializePriority()
{
    ...
    // cgroup cleanup.
    unique_token_ = StrCat({ CGroups::Get().group_prefix_token, UnguessableToken::Create().ToString() });
    ...
    if (!CreateDirectoryAndGetError(cgroup_path, nullptr)) {
        ...
        LOG(ERROR) << "Failed to create cgroup, falling back to foreground"
                   << ", cgroup=" << cgroup_path << ", errno=" << strerror(saved_errno);

        unique_token_.clear();
        return;
    }

    if (!CGroups::Get().uclamp_min.empty() && !WriteFile(cgroup_path.Append(kUclampMinFile), CGroups::Get().uclamp_min)) {
        LOG(ERROR) << "Failed to write uclamp min file, cgroup_path=" << cgroup_path;
    }
    if (!CGroups::Get().uclamp_min.empty() && !WriteFile(cgroup_path.Append(kUclampMaxFile), CGroups::Get().uclamp_max)) {
        LOG(ERROR) << "Failed to write uclamp max file, cgroup_path=" << cgroup_path;
    }
}
...
void Process::CleanUpProcessAsync() const
{
    ...
    ThreadPool::PostTask(FROM_HERE, { MayBlock(), TaskPriority::BEST_EFFORT }, BindOnce(&Process::CleanUpProcessScheduled, Duplicate(), kCgroupDeleteRetries));
}
...
void Process::CleanUpProcess(int remaining_retries) const
{
    ...
    if (!DeleteFile(cgroup)) {
        ...
        LOG(ERROR) << "Failed to delete cgroup " << cgroup << ", errno=" << strerror(saved_errno);
        ...
        if (remaining_retries > 0) {
            ...
            if (!WriteFile(CGroups::Get().background_file, pidstr)) {
                ...
                LOG(WARNING) << "Failed to move the process to background"
                             << ", pid=" << pidstr << ", errno=" << strerror(saved_errno);
            }
            ThreadPool::PostDelayedTask(FROM_HERE, { MayBlock(), TaskPriority::BEST_EFFORT },
                BindOnce(&Process::CleanUpProcessScheduled, Duplicate(), remaining_retries - 1), kCgroupDeleteRetryTime);
        }
    }
}
...
void Process::CleanUpStaleProcessStates()
{
    ...
    FileEnumerator traversal(FilePath(kFullRendererCgroupRoot), false, FileEnumerator::DIRECTORIES);
    for (FilePath path = traversal.Next(); !path.empty(); path = traversal.Next()) {
        std::string dirname = path.BaseName().value();
        if (dirname == FilePath(kForeground).BaseName().value() || dirname == FilePath(kBackground).BaseName().value()) {
            continue;
        }

        if (!StartsWith(dirname, kCgroupPrefix) || StartsWith(dirname, CGroups::Get().group_prefix_token)) {
            continue;
        }

        if (!DeleteFile(path)) {
            auto saved_errno = errno;
            LOG(ERROR) << "Failed to delete " << path << ", errno=" << strerror(saved_errno);
        }
    }
}
#endif // BUILDFLAG(IS_CHROMEOS)
