...
{
#if BUILDFLAG(ENABLE_BASE_TRACING)
    CurrentProcessType process = static_cast<CurrentProcessType>(process_type_.load(std::memory_order_relaxed));
    switch (process) {
    case CurrentProcessType::PROCESS_UNSPECIFIED:
...
void CurrentProcess::SetProcessType(CurrentProcessType process_type)
{
    ...
    CurrentProcess::GetInstance().SetProcessNameAndType(process_name, process_type);
}
...
{
    {
        ...
        process_type_.store(static_cast<CurrentProcessType>(process_type), std::memory_order_relaxed);
    }
#if BUILDFLAG(ENABLE_BASE_TRACING)
