...
bool GetAppOutputInternal(CommandLine::StringViewType cl, bool include_stderr, std::string* output, int* exit_code)
{
    ...
    if (!CreateProcess(nullptr, data(writable_command_line_string), nullptr, nullptr,
    ...
    return status != TERMINATION_STATUS_PROCESS_CRASHED && status != TERMINATION_STATUS_ABNORMAL_TERMINATION;
}

Process LaunchElevatedProcess(const CommandLine& cmdline, bool start_hidden, bool wait)
{
...
Process LaunchProcess(const CommandLine::StringType& cmdline, const LaunchOptions& options)
{
    ...
    if (options.elevated) {
        return LaunchElevatedProcess(base::CommandLine::FromString(cmdline), options.start_hidden, options.wait);
    }
    ...
    if (options.disable_cetcompat && base::win::GetVersion() >= base::win::Version::WIN10_20H1) {
        ...
        process_mitigations[1] |= PROCESS_CREATION_MITIGATION_POLICY2_CET_USER_SHADOW_STACKS_ALWAYS_OFF;
        if (!startup_info_wrapper.UpdateProcThreadAttribute(PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY, &process_mitigations[0], sizeof(process_mitigations))) {
            return Process();
        }
    }
    ...
    if (!options.handles_to_inherit.empty()) {
        ...
        if (!startup_info_wrapper.UpdateProcThreadAttribute(PROC_THREAD_ATTRIBUTE_HANDLE_LIST, const_cast<HANDLE*>(&options.handles_to_inherit[0]),
    ...
    LPCTSTR current_directory = options.current_directory.empty() ? nullptr : options.current_directory.value().c_str();

    auto writable_cmdline(cmdline);
    DCHECK(!(flags & CREATE_SUSPENDED)) << "Creating a suspended process can lead to hung processes if the "
    ...
    if (options.as_user) {
        ...
        BOOL launched = CreateProcessAsUser(options.as_user, nullptr, data(writable_cmdline), nullptr, nullptr, inherit_handles, flags, environment_block,
        ...
        if (!CreateProcess(nullptr, data(writable_cmdline), nullptr, nullptr, inherit_handles, flags, new_environment, current_directory, startup_info,
                &temp_process_info)) {
    ...
    if (options.job_handle && !AssignProcessToJobObject(options.job_handle, process_info.process_handle())) {
    ...
    if (options.grant_foreground_privilege && !AllowSetForegroundWindow(GetProcId(process_info.process_handle()))) {
