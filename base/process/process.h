...
public:
    ...
    WaitExitStatus WaitForExitOrEvent(const base::win::ScopedHandle& stop_event_handle, int* exit_code) const;
    ...
    using WaitForExitCallback = bool (*)(ProcessHandle handle, int* exit_code, base::TimeDelta timeout);
    // Function ptrs to implement termination without polluting //base with
    // BrowserEngineKit APIs.
    static void SetTerminationHooks(TerminateCallback terminate_callback, WaitForExitCallback wait_callback);
...
private:
    ...
    bool WaitForExitWithTimeoutImpl(base::ProcessHandle handle, int* exit_code, base::TimeDelta timeout) const;
#endif
...
BASE_EXPORT Process::Priority GetProcessPriorityCGroup(std::string_view cgroup_contents);
#endif // BUILDFLAG(IS_CHROMEOS)
