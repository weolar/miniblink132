...
TerminationStatus GetTerminationStatusImpl(ProcessHandle handle, bool can_block, int* exit_code)
{
    DCHECK(exit_code);
...
TerminationStatus GetKnownDeadTerminationStatus(ProcessHandle handle, int* exit_code)
{
...
bool WaitForProcessesToExit(const FilePath::StringType& executable_name, TimeDelta wait, const ProcessFilter* filter)
{
    bool result = false;
...
bool CleanupProcesses(const FilePath::StringType& executable_name, TimeDelta wait, int exit_code, const ProcessFilter* filter)
{
...
void EnsureProcessTerminated(Process process)
{
    ...
    PlatformThread::CreateNonJoinable(0, new BackgroundReaper(std::move(process), Seconds(2)));
}
