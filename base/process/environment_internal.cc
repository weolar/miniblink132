...
size_t ParseEnvLine(const NativeEnvironmentString::value_type* input, NativeEnvironmentString* key)
{
...
base::HeapArray<char*> AlterEnvironment(const char* const* const env, const EnvironmentMap& changes)
{
    ...
    if (!value_storage.empty()) {
        ...
        char* storage_data = reinterpret_cast<char*>(&result[result_indices.size() + 1]);
