...
{
    std::string proc;
    if (!base::ReadFileToString(base::FilePath(base::StringPrintf(kProcPath, process.Pid())), &proc)) {
        return std::string();
    }

    std::vector<std::string_view> lines = SplitStringPiece(proc, "\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    for (const auto& line : lines) {
        std::vector<std::string_view> fields = SplitStringPiece(line, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        if (fields.size() != 3U) {
            continue;
        }
...
bool AddProcessToCpuCgroup(const base::Process& process, const std::string& cgroup)
{
...
TEST_F(ProcessTest, Terminate)
{
    ...
    process.WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), &exit_code);

    EXPECT_NE(TERMINATION_STATUS_STILL_RUNNING, GetTerminationStatus(process.Handle(), &exit_code));
#if BUILDFLAG(IS_WIN)
...
TEST_F(ProcessTest, TerminateCurrentProcessImmediatelyWithZeroExitCode)
{
    ...
    ASSERT_TRUE(process.IsValid());
    int exit_code = 42;
    ASSERT_TRUE(process.WaitForExitWithTimeout(TestTimeouts::action_max_timeout(), &exit_code));
...
TEST_F(ProcessTest, WaitForExitOrEventWithProcessExit)
{
    ...
    base::win::ScopedHandle stop_watching_handle(CreateEvent(nullptr, TRUE, FALSE, nullptr));

    int exit_code = kDummyExitCode;
    EXPECT_EQ(process.WaitForExitOrEvent(stop_watching_handle, &exit_code), base::Process::WaitExitStatus::PROCESS_EXITED);
    EXPECT_EQ(0, exit_code);
}
...
TEST_F(ProcessTest, WaitForExitOrEventWithEventSet)
{
    ...
    base::win::ScopedHandle stop_watching_handle(CreateEvent(nullptr, TRUE, TRUE, nullptr));

    int exit_code = kDummyExitCode;
    EXPECT_EQ(process.WaitForExitOrEvent(stop_watching_handle, &exit_code), base::Process::WaitExitStatus::STOP_EVENT_SIGNALED);
    EXPECT_EQ(kDummyExitCode, exit_code);
...
bool IsThreadRT(PlatformThreadId thread_id)
{
    ...
    int sched = sched_getscheduler(PlatformThread::CurrentId() == thread_id ? 0 : thread_id);
    if (sched == -1) {
        // The thread may disappear for any reason so ignore ESRCH.
        DPLOG_IF(ERROR, errno != ESRCH) << "Failed to call sched_getscheduler on thread_id=" << thread_id;
        return false;
    }
...
{
    internal::ForEachProcessTask(process_id, [is_rt](PlatformThreadId tid, const FilePath& /* path */) { EXPECT_EQ(IsThreadRT(tid), is_rt); });
}

void AssertThreadsType(int process_id, ThreadType type)
{
    internal::ForEachProcessTask(process_id,
        [process_id, type](PlatformThreadId tid, const FilePath& path) { EXPECT_EQ(PlatformThread::GetThreadTypeFromThreadId(process_id, tid), type); });
}

void AssertThreadsBgState(int process_id, bool is_bg)
{
    internal::ForEachProcessTask(
        process_id, [is_bg](PlatformThreadId tid, const FilePath& path) { EXPECT_EQ(PlatformThreadLinux::IsThreadBackgroundedForTest(tid), is_bg); });
}
...
public:
    ...
    RTAudioFunctionTestThread& operator=(const RTAudioFunctionTestThread&) = delete;
...
public:
    ...
    RTDisplayFunctionTestThread& operator=(const RTDisplayFunctionTestThread&) = delete;
...
{
    if (!PlatformThread::CanChangeThreadType(ThreadType::kDefault, ThreadType::kDisplayCritical)) {
        return;
    }
...
MULTIPROCESS_TEST_MAIN(ProcessRTDisplayBgMain)
{
    ...
    scoped_feature_list.InitWithFeatures({ kSetThreadBgForBgProcess, kSetRtForDisplayThreads }, {});
    PlatformThreadChromeOS::InitializeFeatures();
...
{
    EXPECT_FALSE(Process::Current().WaitForExitWithTimeout(base::TimeDelta(), nullptr));
}
...
TEST_F(ProcessTest, PredefinedProcessIsRunning)
{
    ...
    EXPECT_FALSE(Process::Open(1).WaitForExitWithTimeout(base::TimeDelta(), nullptr));
}
#endif
...
{
    EXPECT_EXIT(base::debug::win::TerminateWithHeapCorruption(), ::testing::ExitedWithCode(STATUS_HEAP_CORRUPTION), "");
}
...
TEST_F(ProcessTest, MAYBE_ControlFlowViolation)
{
    ...
    EXPECT_EXIT(base::debug::win::TerminateWithControlFlowViolation(), ::testing::ExitedWithCode(STATUS_STACK_BUFFER_OVERRUN), "");
}
...
TEST_F(ProcessTest, TestGetProcessPriorityCGroup)
{
    ...
    EXPECT_EQ(GetProcessPriorityCGroup(kNotBackgroundedCGroup), Process::Priority::kUserBlocking);
    EXPECT_EQ(GetProcessPriorityCGroup(kBackgroundedCGroup), Process::Priority::kBestEffort);
}
...
TEST_F(ProcessTest, CleanUpBusyProcess)
{
    ...
    std::string foreground_path = std::string(kCgroupRoot) + std::string(kForeground);
...
TEST_F(ProcessTest, CleansUpStaleGroups)
{
    ...
    std::vector<std::string> tokens = base::SplitString(cgroup, "-", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    tokens[1] = "fake";
