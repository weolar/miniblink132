...
#define PTHREAD_STACK_MIN_CONST (__builtin_constant_p(PTHREAD_STACK_MIN) ? PTHREAD_STACK_MIN : 131072)
#else
#define PTHREAD_STACK_MIN_CONST (__builtin_constant_p(PTHREAD_STACK_MIN) ? PTHREAD_STACK_MIN : 16384)
#endif // defined(ARCH_CPU_ARM64)

static_assert(__builtin_constant_p(PTHREAD_STACK_MIN_CONST), "must be constant");
...
struct BASE_EXPORT LaunchOptions {
    ...
    static uint32_t AddHandleToTransfer(HandlesToTransferVector* handles_to_transfer, zx_handle_t handle);
    ...
    uint32_t spawn_flags = FDIO_SPAWN_CLONE_NAMESPACE | FDIO_SPAWN_CLONE_STDIO | FDIO_SPAWN_CLONE_JOB;
...
BASE_EXPORT Process LaunchProcess(const CommandLine& cmdline, const LaunchOptions& options);
...
BASE_EXPORT Process LaunchProcess(const CommandLine::StringType& cmdline, const LaunchOptions& options);
...
BASE_EXPORT Process LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options);
...
BASE_EXPORT bool GetAppOutput(CommandLine::StringViewType cl, std::string* output);
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
// A POSIX-specific version of GetAppOutput that takes an argv array
// instead of a CommandLine.  Useful for situations where you need to
// control the command line arguments directly.
BASE_EXPORT bool GetAppOutput(const std::vector<std::string>& argv, std::string* output);

// Like the above POSIX-specific version of GetAppOutput, but also includes
// stderr.
BASE_EXPORT bool GetAppOutputAndError(const std::vector<std::string>& argv, std::string* output);
#endif // BUILDFLAG(IS_WIN)
