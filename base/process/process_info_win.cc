...
IntegrityLevel GetProcessIntegrityLevelInternal(std::optional<win::AccessToken> token)
{
    if (!token) {
...
IntegrityLevel GetProcessIntegrityLevel(ProcessId process_id)
{
    ...
    return process.IsValid() ? GetProcessIntegrityLevelInternal(win::AccessToken::FromProcess(process.Handle(),
...
{
    return GetProcessIntegrityLevelInternal(win::AccessToken::FromCurrentProcess());
}

bool IsCurrentProcessElevated()
{
    std::optional<win::AccessToken> token = win::AccessToken::FromCurrentProcess();
    if (!token) {
