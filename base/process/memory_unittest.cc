...
#if !BUILDFLAG(IS_OPENBSD) && PA_BUILDFLAG(USE_ALLOCATOR_SHIM) && !defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
...
#define ASSERT_OOM_DEATH(statement) ASSERT_EXIT(statement, testing::ExitedWithCode(base::win::kOomExceptionCode), "")
...
public:
    OutOfMemoryTest()
        ...
        test_size_(std::numeric_limits<std::size_t>::max() - 3 * base::GetPageSize())
        ,
...
public:
    ...
    bool ShouldSkipTest()
    {
        ...
        return base::android::BuildInfo::GetInstance()->sdk_int() < base::android::SDK_VERSION_NOUGAT;
#else
        return false;
#endif
    }
...
TEST_F(OutOfMemoryDeathTest, AlignedRealloc)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = _aligned_realloc(nullptr, test_size_, 8);
    });
}
...
TEST_F(OutOfMemoryDeathTest, SecurityCalloc)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = calloc(1024, insecure_test_size_ / 1024L);
    });
}
...
TEST_F(OutOfMemoryDeathTest, SecurityAlignedAlloc)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = base::AlignedAlloc(insecure_test_size_, 8);
    });
}
...
TEST_F(OutOfMemoryDeathTest, SecurityAlignedRealloc)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = _aligned_realloc(nullptr, insecure_test_size_, 8);
    });
}
#endif // BUILDFLAG(IS_WIN)
#endif // !BUILDFLAG(IS_MAC)
...
TEST_F(OutOfMemoryDeathTest, ReallocPurgeable)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = malloc_zone_realloc(zone, nullptr, test_size_);
    });
}
...
TEST_F(OutOfMemoryDeathTest, CallocPurgeable)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = malloc_zone_calloc(zone, 1024, test_size_ / 1024L);
    });
}
...
TEST_F(OutOfMemoryDeathTest, PosixMemalignPurgeable)
{
    ...
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* volatile ptr = malloc_zone_memalign(zone, 8, test_size_);
    });
}
...
TEST_F(OutOfMemoryDeathTest, MAYBE_CFAllocatorSystemDefault)
{
    ASSERT_OOM_DEATH({
        SetUpInDeathAssert();
        [[maybe_unused]] void* ptr;
        while ((ptr = base::AllocateViaCFAllocatorSystemDefault(signed_test_size_))) {
        }
    });
}
...
DWORD HandleOutOfMemoryException(EXCEPTION_POINTERS* exception_ptrs, size_t expected_size)
{
    EXPECT_EQ(base::win::kOomExceptionCode, exception_ptrs->ExceptionRecord->ExceptionCode);
    EXPECT_LE(1U, exception_ptrs->ExceptionRecord->NumberParameters);
    EXPECT_EQ(expected_size, exception_ptrs->ExceptionRecord->ExceptionInformation[0]);
...
// size in the exception raised.
    ...
    } __except (HandleOutOfMemoryException(GetExceptionInformation(), kAttemptedAllocationSize)) {
    }
}
#endif // BUILDFLAG(IS_WIN)

#if defined(ARCH_CPU_32_BITS) && (BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))

void TestAllocationsReleaseReservation(void* (*alloc_fn)(size_t), void (*free_fn)(void*))
{
    ...
    EXPECT_GE(areas.size(), 2u) << "Should be able to allocate without releasing the reservation";
...
{
    TestAllocationsReleaseReservation([](size_t size) { return static_cast<void*>(new char[size]); }, [](void* ptr) { delete[] static_cast<char*>(ptr); });
}
