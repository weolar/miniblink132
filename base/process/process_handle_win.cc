...
ProcessId GetParentProcessId(ProcessHandle process)
{
    ...
    decltype(NtQueryInformationProcess)* nt_query_information_process
        = reinterpret_cast<decltype(NtQueryInformationProcess)*>(GetProcAddress(ntdll, "NtQueryInformationProcess"));
    ...
    ProcessId pid = 0u;
    if (NT_SUCCESS(nt_query_information_process(process, ProcessBasicInformation, &pbi, sizeof(pbi), nullptr))) {
