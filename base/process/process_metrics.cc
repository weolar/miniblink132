...
SystemMemoryInfoKB& SystemMemoryInfoKB::operator=(const SystemMemoryInfoKB&) = default;
...
double ProcessMetrics::GetPlatformIndependentCPUUsage(TimeDelta cumulative_cpu)
{
...
{
    return GetCumulativeCPUUsage().transform([this](base::TimeDelta cpu_usage) { return GetPlatformIndependentCPUUsage(cpu_usage); });
}
#endif

#if BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_AIX)
int ProcessMetrics::CalculateIdleWakeupsPerSecond(uint64_t absolute_idle_wakeups)
{
    return CalculateEventsPerSecond(absolute_idle_wakeups, &last_absolute_idle_wakeups_, &last_idle_wakeups_time_);
}
#else
...
int ProcessMetrics::CalculatePackageIdleWakeupsPerSecond(uint64_t absolute_package_idle_wakeups)
{
    return CalculateEventsPerSecond(absolute_package_idle_wakeups, &last_absolute_package_idle_wakeups_, &last_package_idle_wakeups_time_);
}
