...
zx::process FindProcessInJobTree(const zx::job& job, ProcessId pid)
{
    ...
    if (status == ZX_ERR_NOT_FOUND) {
        ...
        while (true) {
            ...
            size_t actual = 0u;
            size_t available = 0u;
            status = job.get_info(ZX_INFO_JOB_CHILDREN, job_koids.data(), job_koids.size() * sizeof(zx_koid_t), &actual, &available);
...
ProcessHandle Process::Release()
{
    if (is_current()) {
        ...
        zx::process handle;
        zx_status_t result = zx::process::self()->duplicate(ZX_RIGHT_SAME_RIGHTS, &handle);
        if (result != ZX_OK) {
...
Time Process::CreationTime() const
{
    ...
    zx_status_t status = zx_object_get_info(Handle(), ZX_INFO_PROCESS, &proc_info, sizeof(proc_info), nullptr, nullptr);
    if (status != ZX_OK) {
    ...
    return Time::Now() + (TimeTicks::FromZxTime(proc_info.start_time) - TimeTicks::Now());
}
...
bool Process::Terminate(int exit_code, bool wait) const
{
    ...
    if (status == ZX_OK && wait) {
        ...
        status = zx_object_wait_one(Handle(), ZX_TASK_TERMINATED, zx_deadline_after(ZX_SEC(60)), &signals);
        if (status != ZX_OK) {
...
bool Process::WaitForExitWithTimeout(TimeDelta timeout, int* exit_code) const
{
    ...
    zx::time deadline = timeout == TimeDelta::Max() ? zx::time::infinite() : zx::time((TimeTicks::Now() + timeout).ToZxTime());
    zx_signals_t signals_observed = 0;
    zx_status_t status = process_.wait_one(ZX_TASK_TERMINATED, deadline, &signals_observed);
    if (status != ZX_OK) {
        ZX_DLOG_IF(ERROR, status != ZX_ERR_TIMED_OUT, status) << "zx_object_wait_one";
        return false;
    }

    zx_info_process_t proc_info;
    status = process_.get_info(ZX_INFO_PROCESS, &proc_info, sizeof(proc_info), nullptr, nullptr);
    if (status != ZX_OK) {
