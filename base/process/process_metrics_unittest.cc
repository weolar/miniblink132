...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_CHROMEOS_LACROS) || BUILDFLAG(IS_WIN) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FUCHSIA)      \
    || BUILDFLAG(IS_APPLE)
...
{
    const base::expected<TimeDelta, ProcessCPUUsageError> current_cpu_usage = metrics->GetCumulativeCPUUsage();
...
AssertionResult TestChildLauncher::SpawnChildProcess(const std::string& procname)
{
    ...
    options.mach_ports_for_rendezvous.emplace(kTestChildRendezvousKey, MachRendezvousPort(receive_port.get(), MACH_MSG_TYPE_MAKE_SEND));
    child_process_ = SpawnMultiProcessTestChild(procname, command_line_, std::move(options));
    ...
    kern_return_t kr = mach_msg(
        &msg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0, sizeof(msg), receive_port.get(), TestTimeouts::action_timeout().InMilliseconds(), MACH_PORT_NULL);
    if (kr != KERN_SUCCESS) {
        return AssertionFailure() << "Failed to read mach_task_self from child process: " << mach_error_string(kr);
    }
    port_provider_ = std::make_unique<TestChildPortProvider>(child_process_.Handle(), apple::ScopedMachSendRight(msg.task_port.name));
...
{
#if BUILDFLAG(IS_MAC)
    return ProcessMetrics::CreateProcessMetrics(child_process_.Handle(), port_provider_.get());
#else
...
void TestChildLauncher::NotifyParent()
{
    ...
    ASSERT_TRUE(client);
    apple::ScopedMachSendRight send_port = client->TakeSendRight(kTestChildRendezvousKey);
    ...
    kern_return_t kr = mach_msg(&msg.header, MACH_SEND_MSG, msg.header.msgh_size, 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
...
AssertionResult TestChildLauncher::SpawnChildProcess(const std::string& procname)
{
    child_process_ = SpawnMultiProcessTestChild(procname, command_line_, LaunchOptionsForTest());
    return child_process_.IsValid() ? AssertionSuccess() : AssertionFailure() << "Failed to launch child process.";
}
...
bool TestChildLauncher::TerminateChildProcess()
{
    ...
    const auto stat_path = FilePath(FILE_PATH_LITERAL("/proc")).AppendASCII(NumberToString(child_handle)).Append(FILE_PATH_LITERAL("stat"));
...
TEST_F(SystemMetricsTest, ParseMeminfo)
{
    ...
    const char invalid_input3[] = "MemFree:         3913968 kB\n"
                                  "Buffers:         2348340 kB\n"
                                  "Cached:         49071596 kB\n"
                                  "SwapCached:           12 kB\n"
                                  "Active:         36393900 kB\n"
    ...
    const char valid_input1[] = "MemTotal:        3981504 kB\n"
                                "MemFree:          140764 kB\n"
    ...
    const char valid_input2[] = "MemTotal:       255908 kB\n"
                                "MemFree:         69936 kB\n"
                                "Buffers:         15812 kB\n"
                                "Cached:         115124 kB\n"
                                "SwapCached:          0 kB\n"
...
#endif
    EXPECT_EQ(355725u, base::SysInfo::AmountOfAvailablePhysicalMemory(meminfo) / 1024);
    // Simulate as if there is no MemAvailable.
    meminfo.available = 0;
    EXPECT_EQ(374448u, base::SysInfo::AmountOfAvailablePhysicalMemory(meminfo) / 1024);
    meminfo = {};
    ...
    EXPECT_EQ(69936u, base::SysInfo::AmountOfAvailablePhysicalMemory(meminfo) / 1024);
    ...
    const char large_cache_input[] = "MemTotal:       18025572 kB\n"
                                     "MemFree:        13150176 kB\n"
...
TEST_F(SystemMetricsTest, ParseVmstat)
{
    ...
    const char missing_pgmajfault_input[] = "pswpin 12\n"
                                            "pswpout 901\n";
...
{
    std::unique_ptr<ProcessMetrics> metrics = ProcessMetrics::CreateCurrentProcessMetrics();
...
TEST_F(SystemMetricsTest, MeasureChildCpuUsage)
{
    ...
    std::unique_ptr<ProcessMetrics> metrics = child_launcher.CreateChildProcessMetrics();
    ...
    const auto abort_time = base::TimeTicks::Now() + TestTimeouts::action_max_timeout();
    TimeDelta cpu_usage2;
    while (cpu_usage2.is_zero() && !HasFailure() && base::TimeTicks::Now() < abort_time) {
        PlatformThread::Sleep(TestTimeouts::tiny_timeout());
...
{
#if BUILDFLAG(IS_MAC)
    std::unique_ptr<ProcessMetrics> metrics = ProcessMetrics::CreateProcessMetrics(kNullProcessHandle, nullptr);
#else
    std::unique_ptr<ProcessMetrics> metrics = ProcessMetrics::CreateProcessMetrics(kNullProcessHandle);
#endif
    EXPECT_THAT(metrics->GetCumulativeCPUUsage(), ErrorIs(_));
    EXPECT_THAT(metrics->GetPlatformIndependentCPUUsage(), ErrorIs(_));
}
...
TEST_F(SystemMetricsTest, ParseZramMmStat)
{
    ...
    const char valid_input1[] = "17715200 5008166 566062  0 1225715712  127 183842";
...
TEST_F(SystemMetricsTest, ParseZramStat)
{
    ...
    const char valid_input1[] = "299    0    2392    0    1    0    8    0    0    0    0";
...
{
    File file(signal_dir.AppendASCII(signal_file), File::FLAG_CREATE | File::FLAG_WRITE);
    return file.IsValid();
}
...
{
    File file(signal_dir.AppendASCII(signal_file), File::FLAG_OPEN | File::FLAG_READ);
    return file.IsValid();
}
...
MULTIPROCESS_TEST_MAIN(ChildMain)
{
    ...
    for (int i = 0; i < kChildNumFilesToOpen; ++i) {
        files.emplace_back(temp_path.AppendASCII(StringPrintf("file.%d", i)), File::FLAG_CREATE | File::FLAG_WRITE);
    }
...
TEST(ProcessMetricsTest, GetOpenFdCount)
{
    ...
    File file(temp_dir.GetPath().AppendASCII("file"), File::FLAG_CREATE | File::FLAG_WRITE);
...
{
    std::unique_ptr<ProcessMetrics> process_metrics = ProcessMetrics::CreateCurrentProcessMetrics();
    ...
    // page fault count goes up, as memory is typically mapped lazily.
    {
        ...
        WritableSharedMemoryRegion region = WritableSharedMemoryRegion::Create(kMappedSize);
...
TEST(ProcessMetricsTestLinux, GetCumulativeCPUUsagePerThread)
{
    ...
    EXPECT_TRUE(ranges::any_of(
        prev_thread_times, [&thread1](const std::pair<PlatformThreadId, base::TimeDelta>& entry) { return entry.first == thread1.GetThreadId(); }));
    EXPECT_TRUE(ranges::any_of(
        prev_thread_times, [](const std::pair<PlatformThreadId, base::TimeDelta>& entry) { return entry.first == base::PlatformThread::CurrentId(); }));
    ...
    EXPECT_TRUE(ranges::any_of(
        current_thread_times, [](const std::pair<PlatformThreadId, base::TimeDelta>& entry) { return entry.first == base::PlatformThread::CurrentId(); }));

    // Reported times should not decrease.
    for (const auto& entry : current_thread_times) {
        auto prev_it = ranges::find_if(
            prev_thread_times, [&entry](const std::pair<PlatformThreadId, base::TimeDelta>& prev_entry) { return entry.first == prev_entry.first; });
