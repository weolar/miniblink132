...
bool WaitpidWithTimeout(base::ProcessHandle handle, int* status, base::TimeDelta wait)
{
    ...
    while (ret_pid == 0) {
        ...
        const uint32_t sleep_time_usecs
            = static_cast<uint32_t>(std::min(static_cast<uint64_t>((wakeup_time - now).InMicroseconds()), uint64_t { max_sleep_time_usecs }));
...
bool WaitForSingleNonChildProcess(base::ProcessHandle handle, base::TimeDelta wait)
{
    DCHECK_GT(handle, 0);
    ...
    if (event.filter != EVFILT_PROC || (event.fflags & NOTE_EXIT) == 0 || event.ident != static_cast<uintptr_t>(handle)) {
        DLOG(ERROR) << "kevent (wait " << handle << "): unexpected event: filter=" << event.filter << ", fflags=" << event.fflags << ", ident=" << event.ident;
        return false;
    }
...
#if !BUILDFLAG(IS_IOS) || (BUILDFLAG(USE_BLINK) && TARGET_OS_SIMULATOR)
bool Process::WaitForExitWithTimeoutImpl(base::ProcessHandle handle, int* exit_code, base::TimeDelta timeout) const
{
