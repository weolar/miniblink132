...
void setproctitle(const char* fmt, ...)
{
    ...
    const size_t avail_size = base::checked_cast<size_t>(g_envp_end - g_argv_start - 1);
    ...
    static const bool buggy_kernel = [avail_size] {
        ...
        std::string cmdline;
        if (!base::ReadFileToString(base::FilePath("/proc/self/cmdline"), &cmdline)) {
            return false;
        }
    ...
    va_start(ap, fmt);
    if (fmt[0] == '-') {
        size = base::checked_cast<size_t>(vsnprintf(g_argv_start, avail_size, &fmt[1], ap));
    } else {
        size = base::checked_cast<size_t>(snprintf(g_argv_start, avail_size, "%s ", g_orig_argv0));
        if (size < avail_size)
            size += base::checked_cast<size_t>(vsnprintf(&g_argv_start[size], avail_size - size, fmt, ap));
    }
    va_end(ap);
    ...
    // null characters.
    const size_t argv_size = base::checked_cast<size_t>(g_argv_end - g_argv_start - 1);
...
void setproctitle_init(const char** main_argv)
{
    ...
    // incrementally.
    static base::NoDestructor<std::vector<std::string>> environ_copy(environ_size);
