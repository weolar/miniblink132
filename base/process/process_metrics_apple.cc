...
base::expected<task_basic_info_64, ProcessCPUUsageError> GetTaskInfo(mach_port_t task)
{
...
bool GetPowerInfo(mach_port_t task, task_power_info* power_info_data)
{
    ...
    kern_return_t kr = task_info(task, TASK_POWER_INFO, reinterpret_cast<task_info_t>(power_info_data), &power_info_count);
    // Most likely cause for failure: |task| is a zombie.
...
base::expected<TimeDelta, ProcessCPUUsageError> ProcessMetrics::GetCumulativeCPUUsage()
{
    ...
    kern_return_t kr = task_info(task, TASK_THREAD_TIMES_INFO, reinterpret_cast<task_info_t>(&thread_info_data), &thread_info_count);
    if (kr != KERN_SUCCESS) {
        // Most likely cause: |task| is a zombie.
        return base::unexpected(ProcessCPUUsageError::kSystemError);
    }

    const base::expected<task_basic_info_64, ProcessCPUUsageError> task_info_data = GetTaskInfo(task);
    ...
    const TimeDelta measured_cpu = Microseconds(TimeValToMicroseconds(task_timeval));
...
int ProcessMetrics::GetPackageIdleWakeupsPerSecond()
{
    ...
    return CalculatePackageIdleWakeupsPerSecond(power_info_data.task_platform_idle_wakeups);
}
...
size_t GetSystemCommitCharge()
{
    ...
    kern_return_t kr = host_statistics(host.get(), HOST_VM_INFO, reinterpret_cast<host_info_t>(&data), &count);
    if (kr != KERN_SUCCESS) {
...
bool GetSystemMemoryInfo(SystemMemoryInfoKB* meminfo)
{
    ...
    base::apple::ScopedMachSendRight host(mach_host_self());
    int result = host_info(host.get(), HOST_BASIC_INFO, reinterpret_cast<host_info_t>(&hostinfo), &count);
    ...
    if (host_statistics64(host.get(), HOST_VM_INFO64, reinterpret_cast<host_info64_t>(&vm_info), &count) != KERN_SUCCESS) {
        return false;
    }
    ...
    if (vm_info.speculative_count <= vm_info.free_count) {
        meminfo->free = saturated_cast<int>(PAGE_SIZE / 1024 * (vm_info.free_count - vm_info.speculative_count));
    } else {
    ...
    meminfo->speculative = saturated_cast<int>(PAGE_SIZE / 1024 * vm_info.speculative_count);
    meminfo->file_backed = saturated_cast<int>(PAGE_SIZE / 1024 * vm_info.external_page_count);
    meminfo->purgeable = saturated_cast<int>(PAGE_SIZE / 1024 * vm_info.purgeable_count);
...
MachVMRegionResult GetTopInfo(mach_port_t task, mach_vm_size_t* size, mach_vm_address_t* address, vm_region_top_info_data_t* info)
{
    ...
    kern_return_t kr =
#if BUILDFLAG(IS_MAC)
        mach_vm_region(task, address, size, VM_REGION_TOP_INFO, reinterpret_cast<vm_region_info_t>(info), &info_count,
            apple::ScopedMachSendRight::Receiver(object_name).get());
#else
        vm_region_64(task, reinterpret_cast<vm_address_t*>(address), reinterpret_cast<vm_size_t*>(size), VM_REGION_TOP_INFO,
            reinterpret_cast<vm_region_info_t>(info), &info_count, apple::ScopedMachSendRight::Receiver(object_name).get());
#endif
    return ParseOutputFromMachVMRegion(kr);
}

MachVMRegionResult GetBasicInfo(mach_port_t task, mach_vm_size_t* size, mach_vm_address_t* address, vm_region_basic_info_64* info)
{
    ...
    kern_return_t kr =
#if BUILDFLAG(IS_MAC)
        mach_vm_region(task, address, size, VM_REGION_BASIC_INFO_64, reinterpret_cast<vm_region_info_t>(info), &info_count,
            apple::ScopedMachSendRight::Receiver(object_name).get());

#else
        vm_region_64(task, reinterpret_cast<vm_address_t*>(address), reinterpret_cast<vm_size_t*>(size), VM_REGION_BASIC_INFO_64,
            reinterpret_cast<vm_region_info_t>(info), &info_count, apple::ScopedMachSendRight::Receiver(object_name).get());
#endif
    return ParseOutputFromMachVMRegion(kr);
}
...
int ProcessMetrics::GetOpenFdCount() const
{
    ...
    base::HeapArray<char> buffer = base::HeapArray<char>::WithSize(static_cast<size_t>(rv));
