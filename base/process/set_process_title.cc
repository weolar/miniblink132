...
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_SOLARIS) && !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_NACL)
...
void SetProcessTitleFromCommandLine(const char** main_argv)
{
    ...
    if (base::ReadSymbolicLink(self_exe, &target)) {
        ...
        base::FilePath::StringType base_name = base::FilePath(title).BaseName().value();
