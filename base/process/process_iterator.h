...
struct ProcessEntry : public PROCESSENTRY32 {
    ...
    ProcessId parent_pid() const
    {
        return th32ParentProcessID;
    }
    const wchar_t* exe_file() const
    {
        return szExeFile;
    }
};
...
protected:
    ...
    const ProcessEntry& entry() const LIFETIME_BOUND
    {
        return entry_;
    }
...
public:
    NamedProcessIterator(const FilePath::StringType& executable_name, const ProcessFilter* filter, bool use_prefix_match = false);
