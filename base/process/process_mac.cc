...
BASE_FEATURE(kMacSetDefaultTaskRole, "MacSetDefaultTaskRole", FEATURE_ENABLED_BY_DEFAULT);
...
std::optional<task_role_t> GetTaskCategoryPolicyRole(mach_port_t task_port)
{
    ...
    kern_return_t result = task_policy_get(task_port, TASK_CATEGORY_POLICY, reinterpret_cast<task_policy_t>(&category_policy), &task_info_count, &get_default);
...
bool SetTaskCategoryPolicy(mach_port_t task_port, task_role_t task_role)
{
    ...
    kern_return_t result = task_policy_set(task_port, TASK_CATEGORY_POLICY, reinterpret_cast<task_policy_t>(&task_category_policy), TASK_CATEGORY_POLICY_COUNT);
    if (result != KERN_SUCCESS) {
...
bool SetTaskSuppressionPolicy(mach_port_t task_port, bool activate)
{
    task_suppression_policy suppression_policy = {
        .active = activate,
        .lowpri_cpu = activate,
        .timer_throttle = activate ? LATENCY_QOS_TIER_5 : LATENCY_QOS_TIER_UNSPECIFIED,
    ...
    kern_return_t result
        = task_policy_set(task_port, TASK_SUPPRESSION_POLICY, reinterpret_cast<task_policy_t>(&suppression_policy), TASK_SUPPRESSION_POLICY_COUNT);
    if (result != KERN_SUCCESS) {
...
bool IsTaskSuppressionPolicyActive(mach_port_t task_port)
{
    ...
    kern_return_t result
        = task_policy_get(task_port, TASK_SUPPRESSION_POLICY, reinterpret_cast<task_policy_t>(&suppression_policy), &task_info_count, &get_default);
    if (result != KERN_SUCCESS) {
...
bool SetPriorityImpl(mach_port_t task_port, task_role_t task_role, bool activate_suppression_policy)
{
...
Time Process::CreationTime() const
{
    ...
    std::unique_ptr<struct kinfo_proc, base::FreeDeleter> proc(static_cast<struct kinfo_proc*>(malloc(len)));
...
Process::Priority Process::GetPriority(PortProvider* port_provider) const
{
    ...
    if (*task_role == TASK_BACKGROUND_APPLICATION && is_suppression_policy_active) {
        return Priority::kBestEffort;
    } else if (*task_role == TASK_BACKGROUND_APPLICATION && !is_suppression_policy_active) {
        return Priority::kUserVisible;
    } else if (*task_role == TASK_FOREGROUND_APPLICATION && !is_suppression_policy_active) {
        return Priority::kUserBlocking;
    }
...
void Process::SetCurrentTaskDefaultRole()
{
    ...
    task_policy_set(mach_task_self(), TASK_BASE_QOS_POLICY, reinterpret_cast<task_policy_t>(&task_qos_policy), TASK_QOS_POLICY_COUNT);
}
