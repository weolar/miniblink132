...
bool Process::Terminate(int exit_code, bool wait) const
{
    ...
    DCHECK(IsValid());
    bool result = ::TerminateProcess(Handle(), static_cast<UINT>(exit_code)) != FALSE;
    if (result) {
        ...
        if (::WaitForSingleObject(Handle(), kWaitMs) == WAIT_OBJECT_0) {
            DWORD actual_exit;
            Exited(::GetExitCodeProcess(Handle(), &actual_exit) ? static_cast<int>(actual_exit) : exit_code);
            result = true;
        }
    }
    return result;
...
Process::WaitExitStatus Process::WaitForExitOrEvent(const base::win::ScopedHandle& stop_event_handle, int* exit_code) const
{
...
Process::Priority Process::GetPriority() const
{
    ...
    if ((priority == BELOW_NORMAL_PRIORITY_CLASS) || (priority == IDLE_PRIORITY_CLASS)) {
    ...
    const bool ret = ::GetProcessInformation(Handle(), ProcessPowerThrottling, &power_throttling, sizeof(power_throttling));

    // Return Priority::kUserVisible if EcoQoS read & write supported and level
    // set.
    if (ret != 0 && power_throttling.ControlMask == PROCESS_POWER_THROTTLING_EXECUTION_SPEED
...
bool Process::SetPriority(Priority priority)
{
    ...
    DCHECK(!is_current());
    const DWORD priority_class = priority == Priority::kBestEffort ? IDLE_PRIORITY_CLASS : NORMAL_PRIORITY_CLASS;
    ...
    if (os_info->version() >= win::Version::WIN11) {
        ...
        if (priority == Priority::kBestEffort || (priority == Priority::kUserVisible && os_info->version() >= win::Version::WIN11_22H2)) {
            // Sets Eco QoS level.
        ...
        bool ret = ::SetProcessInformation(Handle(), ProcessPowerThrottling, &power_throttling, sizeof(power_throttling));
        if (ret == 0) {
