...
#if (!BUILDFLAG(IS_LINUX) && !BUILDFLAG(IS_AIX) && !BUILDFLAG(IS_CHROMEOS)) || (!defined(__i386__) && !defined(__x86_64__) && !defined(__arm__))
...
struct kernel_sigaction {
    RAW_PTR_EXCLUSION void* k_sa_handler; // For this usage it only needs to be a generic pointer.
    unsigned long k_sa_flags;
    RAW_PTR_EXCLUSION void* k_sa_restorer; // For this usage it only needs to be a generic pointer.
    kernel_sigset_t k_sa_mask;
};

// glibc's sigaction() will prevent access to sa_restorer, so we need to roll
// our own.
long sys_rt_sigaction(int sig, const struct kernel_sigaction* act, struct kernel_sigaction* oact)
{
...
void CloseSuperfluousFds(const base::InjectiveMultimap& saved_mapping)
{
    ...
    for (; fd_dir.Next();) {
        ...
        if (fd_dir.name()[0] == 0 || *endptr || fd < 0 || errno || !IsValueInRangeForNumericType<int>(fd)) {
            continue;
        }
        if (fd == STDIN_FILENO || fd == STDOUT_FILENO || fd == STDERR_FILENO)
            continue;
...
Process LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options)
{
    ...
    if (options.clone_flags) {
        ...
        RAW_CHECK(!(options.clone_flags & (CLONE_CHILD_CLEARTID | CLONE_CHILD_SETTID | CLONE_PARENT_SETTID)));
...
static bool GetAppOutputInternal(
    const std::vector<std::string>& argv, char* const envp[], bool include_stderr, std::string* output, bool do_search_path, int* exit_code)
{
    ...
    *exit_code = EXIT_FAILURE;
    ...
    case 0: {
        ...
        fd_shuffle1.push_back(InjectionArc(include_stderr ? pipe_fd[1] : dev_null, STDERR_FILENO, true));
...
NOINLINE __attribute__((no_sanitize_address)) pid_t CloneAndLongjmpInChild(int flags, pid_t* ptid, pid_t* ctid, jmp_buf* env)
{
#else
NOINLINE pid_t CloneAndLongjmpInChild(int flags, pid_t* ptid, pid_t* ctid, jmp_buf* env)
{
#endif
...
#if defined(ARCH_CPU_X86_FAMILY) || defined(ARCH_CPU_ARM_FAMILY) || defined(ARCH_CPU_MIPS_FAMILY) || defined(ARCH_CPU_S390_FAMILY)                             \
    || defined(ARCH_CPU_PPC64_FAMILY) || defined(ARCH_CPU_LOONGARCH_FAMILY) || defined(ARCH_CPU_RISCV_FAMILY)
