...
bool KillProcesses(const FilePath::StringType& executable_name, int exit_code, const ProcessFilter* filter)
{
    bool result = true;
...
void EnsureProcessTerminated(Process process)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, { TaskPriority::BEST_EFFORT, TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        BindOnce(
            [](Process process) {
                if (process.WaitForExitWithTimeout(TimeDelta(), nullptr))
                    return;
