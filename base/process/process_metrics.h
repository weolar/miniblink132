...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_AIX)
#include <string>
#include <utility>
#include <vector>
...
public:
    ...
    static std::unique_ptr<ProcessMetrics> CreateProcessMetrics(ProcessHandle process, PortProvider* port_provider);
...
#if BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_AIX)
...
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_AIX) || BUILDFLAG(IS_FUCHSIA)
    int buffers = 0;
    int cached = 0;
    int active_anon = 0;
    int inactive_anon = 0;
...
// Exposed for testing.
BASE_EXPORT bool ParseProcMeminfo(std::string_view input, SystemMemoryInfoKB* meminfo);
...
// Exposed for testing.
BASE_EXPORT size_t GetSystemCommitChargeFromMeminfo(const SystemMemoryInfoKB& meminfo);
...
// Returns true on success or false for a parsing error.
// Exposed for testing.
BASE_EXPORT bool ParseZramMmStat(std::string_view mm_stat_data, SwapInfo* swap_info);
...
BASE_EXPORT MachVMRegionResult GetBasicInfo(mach_port_t task, mach_vm_size_t* size, mach_vm_address_t* address, vm_region_basic_info_64* info);
...
BASE_EXPORT MachVMRegionResult GetTopInfo(mach_port_t task, mach_vm_size_t* size, mach_vm_address_t* address, vm_region_top_info_data_t* info);
#endif // BUILDFLAG(IS_APPLE)
