...
template <base::RawPtrTraits ptr_traits = RawPtrTraits::kEmpty, bool use_raw_pointer = false> class BASE_EXPORT UncheckedObserverAdapter {
public:
    ...
    UncheckedObserverAdapter& operator=(UncheckedObserverAdapter&& other) = default;
    ...
    bool IsEqual(const void* rhs) const
    {
        return ptr_ == rhs;
    }

    template <class ObserverType> static ObserverType* Get(const UncheckedObserverAdapter& adapter)
    {
        static_assert(!std::is_base_of_v<CheckedObserver, ObserverType>, "CheckedObserver classes must not use ObserverList<T>::Unchecked.");
...
private:
    using StorageType = std::conditional_t<use_raw_pointer, void*, raw_ptr<void, ptr_traits>>;
    StorageType ptr_;
#if DCHECK_IS_ON()
...
public:
    ...
    {
        static_assert(std::is_base_of_v<CheckedObserver, ObserverType>,
...
// threading support.
template <class ObserverList> class WeakLinkNode : public base::LinkNode<WeakLinkNode<ObserverList>> {
public:
