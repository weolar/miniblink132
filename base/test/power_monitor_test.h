...
public:
    ...
    ScopedPowerMonitorTestSource& operator=(const ScopedPowerMonitorTestSource&) = delete;
...
private:
    ...
    raw_ptr<PowerMonitorTestSource, DanglingUntriaged> power_monitor_test_source_ = nullptr;
};

class PowerMonitorTestObserver : public PowerSuspendObserver, public PowerThermalObserver, public PowerStateObserver {
public:
    ...
    void OnThermalStateChange(PowerThermalObserver::DeviceThermalState new_state) override;
    void OnSpeedLimitChange(int speed_limit) override;

    // Test status counts.
    int power_state_changes() const
    {
        return power_state_changes_;
    }
    int suspends() const
    {
        return suspends_;
    ...
    int thermal_state_changes() const
    {
        return thermal_state_changes_;
    }
    int speed_limit_changes() const
    {
        return speed_limit_changes_;
    }
...
private:
    ...
    PowerStateObserver::BatteryPowerStatus last_power_status_ = PowerStateObserver::BatteryPowerStatus::kUnknown;
    // Last power thermal we were notified of.
    PowerThermalObserver::DeviceThermalState last_thermal_state_ = PowerThermalObserver::DeviceThermalState::kUnknown;
