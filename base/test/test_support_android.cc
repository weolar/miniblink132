...
public:
    ...
    {
        return base::Singleton<Waitable, base::LeakySingletonTraits<Waitable>>::get();
    }
...
private:
    ...
    Waitable()
        : waitable_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
...
void InitPathProvider(int key)
{
    ...
    if (GetTestProviderPath(key, &path) && !base::PathService::Override(key, path)) {
