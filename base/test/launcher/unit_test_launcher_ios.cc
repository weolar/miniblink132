...
int LaunchUnitTestsSerially(int argc, char** argv, RunTestSuiteCallback run_test_suite)
{
    ...
    bool only_write_tests = command_line->HasSwitch(switches::kTestLauncherListTests);
    bool write_and_run_tests = command_line->HasSwitch(switches::kWriteCompiledTestsJsonToWritablePath);
    if (only_write_tests || write_and_run_tests) {
        ...
        FilePath list_path = only_write_tests ? (command_line->GetSwitchValuePath(switches::kTestLauncherListTests))
                                              : apple::GetUserDocumentPath().Append("compiled_tests.json");
        int write_result = WriteCompiledInTestsToFileAndLog(list_path);
        if (only_write_tests) {
            return write_result;
        }
    } else if (command_line->HasSwitch(switches::kTestLauncherPrintWritablePath)) {
