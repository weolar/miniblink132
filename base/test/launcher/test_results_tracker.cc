...
const FilePath::CharType kDefaultOutputFile[] = FILE_PATH_LITERAL("test_detail.xml");
...
{
    return base::UnlocalizedTimeFormatWithPattern(time, "yyyy-MM-dd'T'HH:mm:ss", icu::TimeZone::getGMT());
}
...
TestResultsTracker::~TestResultsTracker()
{
    ...
    for (const PerIterationData::ResultsMap::value_type& i : per_iteration_data_[iteration_].results) {
    ...
    fprintf(out_.get(),
        ...
        all_tests_aggregator.tests, all_tests_aggregator.failures, all_tests_aggregator.disabled, all_tests_aggregator.errors,
        all_tests_aggregator.elapsed_time.InSecondsF(), FormatTimeAsIso8601(Time::Now()).c_str());
    ...
    for (const TestCaseMap::value_type& i : test_case_map) {
        ...
        fprintf(out_.get(),
            ...
            "timestamp=\"%s\">\n",
            testsuite_name.c_str(), aggregator.tests, aggregator.failures, aggregator.disabled, aggregator.errors, aggregator.elapsed_time.InSecondsF(),
        ...
        for (const TestResult& result : results) {
            fprintf(out_.get(),
                ...
                (result.timestamp ? StrCat({ " timestamp=\"", FormatTimeAsIso8601(*result.timestamp), "\"" }).c_str() : ""), result.GetTestCaseName().c_str());
            if (result.status != TestResult::TEST_SUCCESS) {
                ...
                fprintf(out_.get(), "      <failure message=\"\" type=\"\"></failure>\n");
            }
...
bool TestResultsTracker::Init(const CommandLine& command_line)
{
    ...
    print_temp_leaks_ = command_line.HasSwitch(switches::kTestLauncherPrintTempLeaks);
    ...
    if (colon_pos != std::string::npos) {
        ...
        if (path.EndsWithSeparator()) {
            ...
            path = path.Append(executable.ReplaceExtension(FilePath::StringType(FILE_PATH_LITERAL("xml"))));
        }
    }
    if (path.value().empty())
...
void TestResultsTracker::AddTestLocation(const std::string& test_name, const std::string& file, int line)
{
    test_locations_.insert(std::make_pair(TestNameWithoutDisabledPrefix(test_name), CodeLocation(file, line)));
}
...
void TestResultsTracker::AddTestResult(const TestResult& result)
{
    ...
    PerIterationData::ResultsMap& results_map = per_iteration_data_[iteration_].results;
    std::string test_name_without_disabled_prefix = TestNameWithoutDisabledPrefix(result.full_name);
    ...
    if (test_name_without_pre_prefix != test_name_without_disabled_prefix) {
        if (result.status != TestResult::TEST_SUCCESS) {
            ...
            if (!it->second.test_results.empty() && it->second.test_results.back().status == TestResult::TEST_NOT_RUN) {
    ...
    // have real results.
    if (!aggregate_test_result.test_results.empty() && aggregate_test_result.test_results.back().status == TestResult::TEST_NOT_RUN) {
...
void TestResultsTracker::AddLeakedItems(int count, const std::vector<std::string>& test_names)
{
    DCHECK(count);
...
void TestResultsTracker::GeneratePlaceholderIteration()
{
    ...
    for (auto& full_test_name : test_placeholders_) {
        ...
        // results.
        CHECK(per_iteration_data_[iteration_].results[test_name].test_results.empty()) << test_name;
        per_iteration_data_[iteration_].results[test_name].test_results.push_back(test_result);
    }
}
...
void TestResultsTracker::PrintSummaryOfCurrentIteration() const
{
    ...
    PrintTests(tests_by_status[TestResult::TEST_FAILURE].begin(), tests_by_status[TestResult::TEST_FAILURE].end(), "failed");
    PrintTests(tests_by_status[TestResult::TEST_FAILURE_ON_EXIT].begin(), tests_by_status[TestResult::TEST_FAILURE_ON_EXIT].end(), "failed on exit");
    PrintTests(
        tests_by_status[TestResult::TEST_EXCESSIVE_OUTPUT].begin(), tests_by_status[TestResult::TEST_EXCESSIVE_OUTPUT].end(), "produced excessive output");
    PrintTests(tests_by_status[TestResult::TEST_TIMEOUT].begin(), tests_by_status[TestResult::TEST_TIMEOUT].end(), "timed out");
    PrintTests(tests_by_status[TestResult::TEST_CRASH].begin(), tests_by_status[TestResult::TEST_CRASH].end(), "crashed");
    PrintTests(tests_by_status[TestResult::TEST_SKIPPED].begin(), tests_by_status[TestResult::TEST_SKIPPED].end(), "skipped");
    PrintTests(tests_by_status[TestResult::TEST_UNKNOWN].begin(), tests_by_status[TestResult::TEST_UNKNOWN].end(), "had unknown result");
    PrintTests(tests_by_status[TestResult::TEST_NOT_RUN].begin(), tests_by_status[TestResult::TEST_NOT_RUN].end(), "not run");

    if (print_temp_leaks_) {
        for (const auto& leaking_tests : per_iteration_data_.back().leaked_temp_items) {
...
bool TestResultsTracker::SaveSummaryAsJSON(const FilePath& path, const std::vector<std::string>& additional_tags) const
{
    ...
    for (int i = 0; i <= max_iteration; i++) {
        ...
        for (const auto& j : per_iteration_data_[i].results) {
            ...
            for (size_t k = 0; k < j.second.test_results.size(); k++) {
                ...
                test_result_value.Set("elapsed_time_ms", static_cast<int>(test_result.elapsed_time.InMilliseconds()));

                if (test_result.thread_id) {
                    test_result_value.Set("thread_id", static_cast<int>(*test_result.thread_id));
                }
                ...
                if (test_result.timestamp) {
                    ...
                    test_result_value.Set("timestamp", TimeFormatAsIso8601(*test_result.timestamp).c_str());
                }
                ...
                if (IsStringUTF8(test_result.output_snippet)) {
                    ...
                    test_result_value.Set("output_snippet", "<non-UTF-8 snippet, see output_snippet_base64>");
                }
                ...
                for (const TestResultPart& result_part : test_result.test_result_parts) {
                    ...
                    if (lossless_summary) {
                        ...
                        result_part_value.Set("summary", "<non-UTF-8 snippet, see summary_base64>");
                    }
                    ...
                    if (lossless_message) {
                        ...
                        result_part_value.Set("message", "<non-UTF-8 snippet, see message_base64>");
                    }
    ...
    while (flush_attempts_left-- > 0) {
        if (output.Flush())
            return true;
        LOG(ERROR) << "fsync() failed when saving test output summary. " << ((flush_attempts_left > 0) ? "Retrying." : " Giving up.");
    }
...
TestResultsTracker::TestStatusMap TestResultsTracker::GetTestStatusMapForCurrentIteration() const
{
...
TestResultsTracker::TestStatusMap TestResultsTracker::GetTestStatusMapForAllIterations() const
{
    TestStatusMap tests_by_status;
...
void TestResultsTracker::GetTestStatusForIteration(int iteration, TestStatusMap* map) const
{
...
template <typename InputIterator> void TestResultsTracker::PrintTests(InputIterator first, InputIterator last, const std::string& description) const
{
    size_t count = std::distance(first, last);
    if (count == 0)
        return;

    fprintf(stdout, "%" PRIuS " test%s %s:\n", count, count != 1 ? "s" : "", description.c_str());
    for (InputIterator it = first; it != last; ++it) {
        ...
        fprintf(stdout, "    %s (%s:%d)\n", test_name.c_str(), location.file.c_str(), location.line);
    }
    fflush(stdout);
}

void TestResultsTracker::PrintLeaks(int count, const std::vector<std::string>& test_names) const
{
    fprintf(stdout,
...
TestResultsTracker::AggregateTestResult::AggregateTestResult(const AggregateTestResult& other) = default;

TestResultsTracker::AggregateTestResult::~AggregateTestResult() = default;

TestResultsTracker::PerIterationData::PerIterationData() = default;

TestResultsTracker::PerIterationData::PerIterationData(const PerIterationData& other) = default;
