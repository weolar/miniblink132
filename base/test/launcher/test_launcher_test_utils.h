...
bool ValidateTestResult(
    const Value::Dict& iteration_data, const std::string& test_name, const std::string& status, size_t result_part_count, bool have_running_info = true);

// Validate test_locations contains all tests in |test_case_name|.
bool ValidateTestLocations(const Value::Dict& test_locations, const std::string& test_case_name);
