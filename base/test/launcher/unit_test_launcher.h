...
int LaunchUnitTests(int argc, char** argv, RunTestSuiteCallback run_test_suite, size_t retry_limit = 1U);
...
public:
    ...
    virtual bool CreateResultsFile(const base::FilePath& temp_dir, base::FilePath* path) = 0;

    // Called to create a new temporary file. The delegate must put the resulting
    // path in |path| and return true on success.
    virtual bool CreateTemporaryFile(const base::FilePath& temp_dir, base::FilePath* path) = 0;
...
public:
    ...
    DefaultUnitTestPlatformDelegate(const DefaultUnitTestPlatformDelegate&) = delete;
    DefaultUnitTestPlatformDelegate& operator=(const DefaultUnitTestPlatformDelegate&) = delete;
...
private:
    ...
    bool CreateResultsFile(const base::FilePath& temp_dir, base::FilePath* path) override;

    bool CreateTemporaryFile(const base::FilePath& temp_dir, base::FilePath* path) override;

    CommandLine GetCommandLineForChildGTestProcess(
        const std::vector<std::string>& test_names, const base::FilePath& output_file, const base::FilePath& flag_file) override;
...
public:
    UnitTestLauncherDelegate(UnitTestPlatformDelegate* delegate, size_t batch_limit, bool use_job_objects, RepeatingClosure timeout_callback);
...
private:
    ...
    CommandLine GetCommandLine(const std::vector<std::string>& test_names, const FilePath& temp_dir, FilePath* output_file) override;
