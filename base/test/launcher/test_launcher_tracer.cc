...
int TestLauncherTracer::RecordProcessExecution(TimeTicks start_time, TimeDelta duration)
{
    AutoLock lock(lock_);
...
bool TestLauncherTracer::Dump(const FilePath& path)
{
    ...
    for (const Event& event : events_) {
        ...
        json_event.Set("ph", "X");
        json_event.Set("ts", checked_cast<int>((event.timestamp - trace_start_time_).InMicroseconds()));
