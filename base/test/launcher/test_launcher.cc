...
ResultWatcher::ResultWatcher(FilePath result_file, size_t num_tests)
    : result_file_(std::move(result_file))
...
bool ResultWatcher::PollUntilDone(TimeDelta timeout_per_test)
{
    ...
    } while (TimeTicks::Now() < batch_deadline && time_to_next_check.is_positive());
...
TimeDelta ResultWatcher::PollOnce(TimeDelta timeout_per_test)
{
    ...
    if (time_since_latest_completion.is_negative() || time_since_latest_completion > kDaylightSavingsThreshold) {
...
Time ResultWatcher::LatestCompletionTimestamp(const std::vector<TestResult>& test_results)
{
...
{
    zx::time deadline = zx::deadline_after(zx::duration(kOutputTimeout.ToZxDuration()));
...
void KillSpawnedTestProcesses()
{
    ...
    fprintf(stdout, "Sending SIGTERM to %zu child processes... ", GetLiveProcesses()->size());
    fflush(stdout);
    ...
    fprintf(stdout, "Sending SIGKILL to %zu child processes... ", GetLiveProcesses()->size());
    fflush(stdout);
...
{
    std::unique_ptr<Environment> env(Environment::Create());
    return command_line->HasSwitch(switches::kTestLauncherBotMode) || env->HasVar("CHROMIUM_TEST_LAUNCHER_BOT_MODE");
}

// Returns command line command line after gtest-specific processing
// and applying |wrapper|.
CommandLine PrepareCommandLineForGTest(const CommandLine& command_line, const std::string& wrapper, const size_t retries_left)
{
    ...
    for (CommandLine::SwitchMap::const_iterator iter = switches.begin(); iter != switches.end(); ++iter) {
...
int LaunchChildTestProcessWithOptions(const CommandLine& command_line, const LaunchOptions& options, int flags, const FilePath& result_file,
    TimeDelta timeout_per_test, size_t num_tests, TestLauncherDelegate* delegate, bool* was_timeout)
{
#if BUILDFLAG(IS_POSIX)
    ...
    for (size_t i = 0; i < flat_namespace->count; ++i) {
        ...
        if (path == kDataPath || path == kCachePath) {
            ...
            new_options.paths_to_transfer.push_back({ path, flat_namespace->handle[i] });
        }
    }
    free(flat_namespace);
    ...
    const base::FilePath child_data_path
        = kDataPath.AppendASCII(base::StringPrintf("test-%zu-%d", base::Process::Current().Pid(), child_launch_index.GetNext()));
    ...
    // process' namespace.
    new_options.paths_to_transfer.push_back({ kDataPath, base::OpenDirectoryHandle(test_data_dir).TakeChannel().release() });
    new_options.paths_to_transfer.push_back({ kCachePath, base::OpenDirectoryHandle(test_cache_dir).TakeChannel().release() });
#endif // BUILDFLAG(IS_FUCHSIA)
...

    {
        ...
        if (new_options.stdout_handle) {
            ::SetHandleInformation(new_options.stdout_handle, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT);
        }
#endif
...
FilePath CreateChildTempDirIfSupported(const FilePath& task_temp_dir, int child_index)
{
...
void SetTemporaryDirectory(const FilePath& temp_dir, EnvironmentMap* environment)
{
#if BUILDFLAG(IS_WIN)
...
ChildProcessResults DoLaunchChildTestProcess(const CommandLine& command_line, const FilePath& process_temp_dir, const FilePath& result_file,
    TimeDelta timeout_per_test, size_t num_tests, const TestLauncher::LaunchOptions& test_launch_options, bool redirect_stdio, TestLauncherDelegate* delegate)
{
    ...
    if (redirect_stdio) {
        ...
        if (!FILEToFile(output_file.get()).DeleteOnClose(true)) {
            PLOG(WARNING) << "Failed to mark " << output_filename.AsUTF8Unsafe() << " for deletion on close";
        }
#endif
    }
    ...
    if (redirect_stdio) {
        ...
        // FILE_TYPE_CHAR.
        if (options.inherit_mode == base::LaunchOptions::Inherit::kSpecific && GetFileType(handle) != FILE_TYPE_CHAR) {
    ...
    if (redirect_stdio) {
        ...
        options.fds_to_remap.push_back(std::make_pair(output_file_fd, STDOUT_FILENO));
        options.fds_to_remap.push_back(std::make_pair(output_file_fd, STDERR_FILENO));
    }
    ...
    result.exit_code = LaunchChildTestProcessWithOptions(
        command_line, options, test_launch_options.flags, result_file, timeout_per_test, num_tests, delegate, &result.was_timeout);
    ...
    if (redirect_stdio) {
        ...
        CHECK(ReadStreamToString(output_file.get(), &result.output_file_contents) || result.exit_code != 0);
    ...
    result.process_num = GetTestLauncherTracer()->RecordProcessExecution(start_time, result.elapsed_time);
    return result;
}

std::vector<std::string> ExtractTestsFromFilter(const std::string& filter, bool double_colon_supported)
{
    std::vector<std::string> tests;
    if (double_colon_supported) {
        tests = SplitString(filter, "::", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    }
    if (tests.size() <= 1) {
        tests = SplitString(filter, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    }
...
class TestRunner {
public:
    explicit TestRunner(TestLauncher* launcher, size_t max_workers = 1u, size_t batch_size = 1u)
        : launcher_(launcher)
...
private:
    ...
    {
        return test_names.size() == 1u && test_names.front().find(kPreTestPrefix) != std::string::npos;
    }
    ...
    void WorkerTask(scoped_refptr<TaskRunner> main_task_runner, base::JobDelegate* delegate);
    ...
    size_t GetMaxConcurrency(size_t worker_count)
    {
        ...
        return std::min((tests_to_run_.size() + batch_size_ - 1) / batch_size_, max_workers_);
    }
    ...
    std::vector<std::string> GetNextBatch() EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
        ...
        std::vector<std::string> batch(tests_to_run_.rbegin(), tests_to_run_.rbegin() + batch_size);
...
void TestRunner::Run(const std::vector<std::string>& test_names)
{
    ...
    job_handle_ = base::PostJob(FROM_HERE, { TaskPriority::USER_BLOCKING, MayBlock() },
        BindRepeating(&TestRunner::WorkerTask, Unretained(this), SingleThreadTaskRunner::GetCurrentDefault()),
        BindRepeating(&TestRunner::GetMaxConcurrency, Unretained(this)));

    run_loop_.Run();
}

void TestRunner::WorkerTask(scoped_refptr<TaskRunner> main_task_runner, base::JobDelegate* delegate)
{
    bool done = false;
    while (!done && !delegate->ShouldYield()) {
        ...
        for (const auto& batch : batches) {
            launcher_->LaunchChildGTestProcess(
                main_task_runner, batch, task_temp_dir.GetPath(), CreateChildTempDirIfSupported(task_temp_dir.GetPath(), child_index++));
        }
        ...
        main_task_runner->PostTask(FROM_HERE, BindOnce(&TestRunner::CleanupTask, weak_ptr_factory_.GetWeakPtr(), std::move(task_temp_dir), done));
    }
}
...
void TestRunner::CleanupTask(base::ScopedTempDir task_temp_dir, bool done)
{
    ...
    if (!task_temp_dir.Delete()) {
        ...
        LOG(WARNING) << "Failed to delete " << task_temp_dir.GetPath().AsUTF8Unsafe();
    }
...
int CountItemsInDirectory(const FilePath& dir)
{
    if (dir.empty())
        return 0;
    int items = 0;
    FileEnumerator file_enumerator(dir, /*recursive=*/false, FileEnumerator::FILES | FileEnumerator::DIRECTORIES);
    for (FilePath name = file_enumerator.Next(); !name.empty(); name = file_enumerator.Next()) {
        ++items;
    }
    return items;
}
...
std::string TruncateSnippet(std::string_view snippet, size_t byte_limit)
{
    ...
    std::string truncation_message = StringPrintf("\n<truncated (%zu bytes)>\n", snippet.length());
    ...
    return base::StrCat({ snippet.substr(0, first_half), truncation_message, snippet.substr(snippet.length() - (remaining_limit - first_half)) });
}
...
const char kIsolatedScriptRunDisabledTestsFlag[] = "isolated-script-test-also-run-disabled-tests";
...
public:
    ...
    const std::string& test_name() const LIFETIME_BOUND
    {
        return test_name_;
    }
    const std::string& file() const LIFETIME_BOUND
    {
        return file_;
    }
    int line() const
...
std::string TestLauncher::TestInfo::GetDisabledStrippedName() const
{
    ...
    ReplaceSubstringsAfterOffset(&test_name, 0, kDisabledTestPrefix, std::string());
    return test_name;
}
...
std::string TestLauncher::TestInfo::GetPreName() const
{
    ...
    ReplaceSubstringsAfterOffset(&case_name, 0, kDisabledTestPrefix, std::string());
...
TestLauncher::LaunchOptions::LaunchOptions(const LaunchOptions& other) = default;
TestLauncher::LaunchOptions::~LaunchOptions() = default;

TestLauncher::TestLauncher(TestLauncherDelegate* launcher_delegate, size_t parallel_jobs, size_t retry_limit)
    ...
    , watchdog_timer_(FROM_HERE, kOutputTimeout, this, &TestLauncher::OnOutputTimeout)
...
bool TestLauncher::Run(CommandLine* command_line)
{
    ...
    if (!Init((command_line == nullptr) ? CommandLine::ForCurrentProcess() : command_line))
        return false;
    ...
    auto controller
        = base::FileDescriptorWatcher::WatchReadable(g_shutdown_pipe[0], base::BindRepeating(&TestLauncher::OnShutdownPipeReadable, Unretained(this)));
#endif // BUILDFLAG(IS_POSIX)
    ...
    while ((iterations > 0 || iterations == -1) && !(stop_on_failure_ && test_failed)) {
...
void TestLauncher::LaunchChildGTestProcess(
    scoped_refptr<TaskRunner> task_runner, const std::vector<std::string>& test_names, const FilePath& task_temp_dir, const FilePath& child_temp_dir)
{
    FilePath result_file;
    CommandLine cmd_line = launcher_delegate_->GetCommandLine(test_names, task_temp_dir, &result_file);

    // Record the exact command line used to launch the child.
    CommandLine new_command_line(PrepareCommandLineForGTest(cmd_line, launcher_delegate_->GetWrapper(), retries_left_));
    LaunchOptions options;
    ...
    ChildProcessResults process_results = DoLaunchChildTestProcess(
        new_command_line, child_temp_dir, result_file, launcher_delegate_->GetTimeout(), test_names.size(), options, redirect_stdio_, launcher_delegate_);

    // Invoke ProcessTestResults on the original thread, not
    // on a worker pool thread.
    task_runner->PostTask(FROM_HERE,
        BindOnce(&TestLauncher::ProcessTestResults, Unretained(this), test_names, result_file, process_results.output_file_contents,
            process_results.elapsed_time, process_results.exit_code, process_results.was_timeout, process_results.thread_id, process_results.process_num,
            CountItemsInDirectory(child_temp_dir)));
}

// Determines which result status will be assigned for missing test results.
TestResult::Status MissingResultStatus(size_t tests_to_run_count, bool was_timeout, bool exit_code)
{
...
void TestLauncher::ProcessTestResults(const std::vector<std::string>& test_names, const FilePath& result_file, const std::string& output,
    TimeDelta elapsed_time, int exit_code, bool was_timeout, PlatformThreadId thread_id, int process_num, int leaked_items)
{
    std::vector<TestResult> test_results;
    bool crashed = false;
    bool have_test_results = ProcessGTestOutput(result_file, &test_results, &crashed);
    ...
    TestResult::Status missing_result_status = MissingResultStatus(test_names.size(), was_timeout, exit_code);
...
void TestLauncher::OnTestFinished(const TestResult& original_result)
{
    ...
    if (result.output_snippet.length() > output_bytes_limit_) {
        ...
        result.output_snippet = TruncateSnippetFocused(result.output_snippet, output_bytes_limit_);
    }
    ...
    if (print_snippet) {
        std::vector<std::string_view> snippet_lines = SplitStringPiece(result.output_snippet, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
        if (snippet_lines.size() > kOutputSnippetLinesLimit) {
            size_t truncated_size = snippet_lines.size() - kOutputSnippetLinesLimit;
            snippet_lines.erase(snippet_lines.begin(), snippet_lines.begin() + truncated_size);
    ...
    std::string status_line(StringPrintf("[%zu/%zu] %s ", test_finished_count_, test_started_count_, result.full_name.c_str()));
    if (result.completed()) {
        status_line.append(StringPrintf("(%" PRId64 " ms)", result.elapsed_time.InMilliseconds()));
    ...
    if (CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestLauncherPrintTimestamps)) {
...
bool LoadFilterFile(const FilePath& file_path, std::vector<std::string>* positive_filter, std::vector<std::string>* negative_filter)
{
    ...
    std::vector<std::string> filter_lines = SplitString(file_content, "\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    int line_num = 0;
    for (const std::string& filter_line : filter_lines) {
        ...
        // unintentional.
        if (hash_pos != std::string::npos && hash_pos > 0 && filter_line[hash_pos - 1] != ' ') {
            LOG(WARNING) << "Content of line " << line_num << " in " << file_path << " after # is treated as a comment, " << filter_line;
        }

        // Strip comments and whitespace from each line.
        std::string trimmed_line(TrimWhitespaceASCII(filter_line.substr(0, hash_pos), TRIM_ALL));

        if (trimmed_line.substr(0, 2) == "//") {
            LOG(ERROR) << "Line " << line_num << " in " << file_path << " starts with //, use # for comments.";
            return false;
        }
...
bool TestLauncher::IsOnlyExactPositiveFilterFromFile(const CommandLine* command_line) const
{
...
bool TestLauncher::Init(CommandLine* command_line)
{
    ...
    if (command_line->HasSwitch(switches::kTestLauncherTotalShards) && command_line->HasSwitch(switches::kTestLauncherShardIndex)) {
        if (!StringToInt(command_line->GetSwitchValueASCII(switches::kTestLauncherTotalShards), &total_shards_)) {
            LOG(ERROR) << "Invalid value for " << switches::kTestLauncherTotalShards;
            return false;
        }
        if (!StringToInt(command_line->GetSwitchValueASCII(switches::kTestLauncherShardIndex), &shard_index_)) {
            LOG(ERROR) << "Invalid value for " << switches::kTestLauncherShardIndex;
            return false;
        }
        fprintf(stdout, "Using sharding settings from command line. This is shard %d/%d\n", shard_index_, total_shards_);
        fflush(stdout);
    } else {
        ...
        if (!TakeInt32FromEnvironment(kTestShardIndex, &shard_index_))
            return false;
        fprintf(stdout, "Using sharding settings from environment. This is shard %d/%d\n", shard_index_, total_shards_);
        fflush(stdout);
    }
    if (shard_index_ < 0 || total_shards_ < 0 || shard_index_ >= total_shards_) {
        LOG(ERROR) << "Invalid sharding settings: we require 0 <= " << kTestShardIndex << " < " << kTestTotalShards << ", but you have " << kTestShardIndex
    ...
    if (command_line->HasSwitch(kIsolatedScriptTestRepeatFlag) && !StringToInt(command_line->GetSwitchValueASCII(kIsolatedScriptTestRepeatFlag), &cycles_)) {
    ...
    if (command_line->HasSwitch(switches::kTestLauncherRetryLimit)) {
        int retry_limit = -1;
        if (!StringToInt(command_line->GetSwitchValueASCII(switches::kTestLauncherRetryLimit), &retry_limit) || retry_limit < 0) {
            LOG(ERROR) << "Invalid value for " << switches::kTestLauncherRetryLimit;
            return false;
        }

        retry_limit_ = retry_limit;
    } else if (command_line->HasSwitch(switches::kIsolatedScriptTestLauncherRetryLimit)) {
        int retry_limit = -1;
        if (!StringToInt(command_line->GetSwitchValueASCII(switches::kIsolatedScriptTestLauncherRetryLimit), &retry_limit) || retry_limit < 0) {
            LOG(ERROR) << "Invalid value for " << switches::kIsolatedScriptTestLauncherRetryLimit;
            return false;
        }

        retry_limit_ = retry_limit;
    } else if (command_line->HasSwitch(kGTestRepeatFlag) || command_line->HasSwitch(kGTestBreakOnFailure)) {
        // If we are repeating tests or waiting for the first test to fail, disable
        // retries.
        retry_limit_ = 0U;
    } else if (!BotModeEnabled(command_line) && (command_line->HasSwitch(kGTestFilterFlag) || command_line->HasSwitch(kIsolatedScriptTestFilterFlag))) {
    ...
    force_run_broken_tests_ = command_line->HasSwitch(switches::kTestLauncherForceRunBrokenTests);

    if (command_line->HasSwitch(switches::kTestLauncherOutputBytesLimit)) {
        int output_bytes_limit = -1;
        if (!StringToInt(command_line->GetSwitchValueASCII(switches::kTestLauncherOutputBytesLimit), &output_bytes_limit) || output_bytes_limit < 0) {
            LOG(ERROR) << "Invalid value for " << switches::kTestLauncherOutputBytesLimit;
            return false;
        }
    ...
    if (command_line->HasSwitch(switches::kTestLauncherFilterFile)) {
        auto filter = command_line->GetSwitchValueNative(switches::kTestLauncherFilterFile);
        for (auto filter_file : SplitStringPiece(filter, FILE_PATH_LITERAL(";"), base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
#if BUILDFLAG(IS_IOS)
...
#else
            base::FilePath filter_file_path = base::MakeAbsoluteFilePath(FilePath(filter_file));
#endif // BUILDFLAG(IS_IOS)

            if (!LoadFilterFile(filter_file_path, &positive_file_filter, &negative_test_filter_))
                return false;
        }
    }
    ...
    enforce_exact_postive_filter_ = command_line->HasSwitch(switches::kEnforceExactPositiveFilter);
    if (enforce_exact_postive_filter_ && !IsOnlyExactPositiveFilterFromFile(command_line)) {
        LOG(ERROR) << "With " << switches::kEnforceExactPositiveFilter << ", only accept exact positive filters via " << switches::kTestLauncherFilterFile;
        return false;
    }
    ...
    std::string filter = command_line->GetSwitchValueASCII(double_colon_supported ? kIsolatedScriptTestFilterFlag : kGTestFilterFlag);
    size_t dash_pos = filter.find('-');
    if (dash_pos == std::string::npos) {
        positive_gtest_filter = ExtractTestsFromFilter(filter, double_colon_supported);
    } else {
        // Everything up to the dash.
        positive_gtest_filter = ExtractTestsFromFilter(filter.substr(0, dash_pos), double_colon_supported);

        // Everything after the dash.
        for (std::string pattern : ExtractTestsFromFilter(filter.substr(dash_pos + 1), double_colon_supported)) {
            negative_test_filter_.push_back(pattern);
        }
    }

    skip_disabled_tests_ = !command_line->HasSwitch(kGTestRunDisabledTestsFlag) && !command_line->HasSwitch(kIsolatedScriptRunDisabledTestsFlag);
    ...
    if (command_line->HasSwitch(switches::kTestLauncherPrintTestStdio)) {
        std::string print_test_stdio = command_line->GetSwitchValueASCII(switches::kTestLauncherPrintTestStdio);
        if (print_test_stdio == "auto") {
            ...
            LOG(WARNING) << "Invalid value of " << switches::kTestLauncherPrintTestStdio << ": " << print_test_stdio;
            return false;
        }
    }
    ...
    if (command_line->HasSwitch(switches::kTestLauncherSummaryOutput)) {
        summary_path_ = FilePath(command_line->GetSwitchValuePath(switches::kTestLauncherSummaryOutput));
    }
    if (command_line->HasSwitch(switches::kTestLauncherTrace)) {
        trace_path_ = FilePath(command_line->GetSwitchValuePath(switches::kTestLauncherTrace));
    }
    ...
    CombinePositiveTestFilters(std::move(positive_gtest_filter), std::move(positive_file_filter));
...
bool TestLauncher::InitTests()
{
    ...
    for (auto& test : tests) {
        const std::string full_test_name = test.test_case_name + "." + test.test_name;
...
bool TestLauncher::ShuffleTests(CommandLine* command_line)
{
    if (command_line->HasSwitch(kGTestShuffleFlag)) {
        ...
        if (command_line->HasSwitch(kGTestRandomSeedFlag)) {
            const std::string custom_seed_str = command_line->GetSwitchValueASCII(kGTestRandomSeedFlag);
...
bool TestLauncher::ProcessAndValidateTests()
{
    ...
    for (const TestInfo& test_info : tests_) {
        ...
        if (!test_info.disabled() || !skip_disabled_tests_)
            tests_to_run.insert(tests_to_run.end(), test_sequence.rbegin(), test_sequence.rend());
    }
...
void TestLauncher::CombinePositiveTestFilters(std::vector<std::string> filter_a, std::vector<std::string> filter_b)
{
...
bool TestLauncher::ShouldRunInCurrentShard(std::string_view prefix_stripped_name) const
{
    CHECK(!StartsWith(prefix_stripped_name, kPreTestPrefix));
    CHECK(!StartsWith(prefix_stripped_name, kDisabledTestPrefix));
    return PersistentHash(prefix_stripped_name) % total_shards_ == static_cast<uint32_t>(shard_index_);
}
...
std::vector<std::string> TestLauncher::CollectTests()
{
    ...
    for (const TestInfo& test_info : tests_) {
        ...
        if (has_at_least_one_positive_filter_) {
            bool found = positive_exact_filter.find(test_name) != positive_exact_filter.end()
                || positive_exact_filter.find(prefix_stripped_name) != positive_exact_filter.end();
            ...
            if (!found) {
                for (std::string_view filter : positive_wildcards_filter) {
                    if (MatchPattern(test_name, filter) || MatchPattern(prefix_stripped_name, filter)) {
                        found = true;
                        break;
                    }
                }
        ...
        if (negative_exact_filter.find(test_name) != negative_exact_filter.end()
            || negative_exact_filter.find(prefix_stripped_name) != negative_exact_filter.end()) {
            continue;
        }
        ...
        results_tracker_.AddTestLocation(test_name, test_info.file(), test_info.line());
    ...
    if (enforce_exact_postive_filter_) {
        ...
        for (const auto& filter : positive_exact_filter) {
            if (!ShouldRunInCurrentShard(filter) || Contains(enforced_positive_tests, std::string(filter))) {
                continue;
            }
...
void TestLauncher::RunTests()
{
    ...
    for (int i = 0; i < repeats_per_iteration_; ++i) {
        test_names.insert(test_names.end(), original_test_names.begin(), original_test_names.end());
    }
    ...
    size_t batch_size = repeats_per_iteration_ > 1 ? 1U : launcher_delegate_->GetBatchSize();
...
void TestLauncher::PrintFuzzyMatchingTestNames()
{
    for (auto filter : positive_test_filter_) {
        ...
        for (const TestInfo& test_info : tests_) {
            std::string test_name = test_info.GetFullName();
            std::string prefix_stripped_name = test_info.GetPrefixStrippedName();
            if (MatchPattern(test_name, almost_filter) || MatchPattern(prefix_stripped_name, almost_filter)) {
...
bool TestLauncher::RunRetryTests()
{
    while (!tests_to_retry_.empty() && retries_left_ > 0) {
        ...
        fprintf(stdout, "Retrying %zu test%s (retry #%zu)\n", retry_started_count, retry_started_count > 1 ? "s" : "", retry_limit_ - retries_left_);
        fflush(stdout);
...
void TestLauncher::MaybeSaveSummaryAsJSON(const std::vector<std::string>& additional_tags)
{
...
{
    TestResultsTracker::TestStatusMap tests_by_status(results_tracker_.GetTestStatusMapForCurrentIteration());
...
size_t NumParallelJobs(unsigned int cores_per_job)
{
    ...
    if (command_line->HasSwitch(switches::kTestLauncherJobs)) {
        ...
        if (!StringToSizeT(command_line->GetSwitchValueASCII(switches::kTestLauncherJobs), &jobs) || !jobs) {
            LOG(ERROR) << "Invalid value for " << switches::kTestLauncherJobs;
            return 0U;
        }
        return jobs;
    }
    if (!BotModeEnabled(command_line) && (command_line->HasSwitch(kGTestFilterFlag) || command_line->HasSwitch(kIsolatedScriptTestFilterFlag))) {
    ...
    size_t cores = base::checked_cast<size_t>(::GetActiveProcessorCount(ALL_PROCESSOR_GROUPS));
#else
...
std::string TruncateSnippetFocused(std::string_view snippet, size_t byte_limit)
{
    ...
    size_t fatal_message_pos = std::min(snippet.find("FATAL:"), snippet.find("FATAL "));
    ...
    fatal_message_end = std::min(fatal_message_end, fatal_message_start + (byte_limit / 2));
    ...
    // the remaining bytes.
    size_t remaining_bytes = byte_limit - (fatal_message_end - fatal_message_start);
    ...
    if (fatal_message_start < remaining_bytes / 2) {
        ...
        start_split_bytes = remaining_bytes - (snippet.length() - fatal_message_end);
    ...
    return base::StrCat({ TruncateSnippet(snippet.substr(0, fatal_message_start), start_split_bytes),
        snippet.substr(fatal_message_start, fatal_message_end - fatal_message_start), TruncateSnippet(snippet.substr(fatal_message_end), end_split_bytes) });
}
