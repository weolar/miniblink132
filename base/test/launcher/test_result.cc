...
TestResultPart& TestResultPart::operator=(const TestResultPart& other) = default;
...
void TestResult::AddProperty(const std::string& name, const std::string& value)
{
    auto [it, inserted] = properties.insert({ name, value });
    DCHECK(inserted) << name << " is already used as a property name. Ignoring...";
}
