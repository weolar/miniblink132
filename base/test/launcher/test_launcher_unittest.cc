...
TestResult GenerateTestResult(
    const std::string& test_name, TestResult::Status status, TimeDelta elapsed_td = Milliseconds(30), const std::string& output_snippet = "output")
{
    TestResult result;
...
TestResultPart GenerateTestResultPart(
    TestResultPart::Type type, const std::string& file_name, int line_number, const std::string& summary, const std::string& message)
{
...
public:
    MockTestLauncher(TestLauncherDelegate* launcher_delegate, size_t parallel_jobs)
    ...
    MOCK_METHOD4(LaunchChildGTestProcess,
        void(scoped_refptr<TaskRunner> task_runner, const std::vector<std::string>& test_names, const FilePath& task_temp_dir, const FilePath& child_temp_dir));
};
...
public:
    ...
    MOCK_METHOD2(WillRunTest, bool(const std::string& test_case_name, const std::string& test_name));
    MOCK_METHOD2(ProcessTestResults, void(std::vector<TestResult>& test_names, TimeDelta elapsed_time));
    MOCK_METHOD3(GetCommandLine, CommandLine(const std::vector<std::string>& test_names, const FilePath& temp_dir_, FilePath* output_file_));
...
protected:
    ...
    void AddMockedTests(std::string test_case_name, const std::vector<std::string>& test_names)
    {
    ...
    {
        EXPECT_CALL(delegate, GetTests(_)).WillOnce(::testing::DoAll(testing::SetArgPointee<0>(tests_), testing::Return(true)));
        EXPECT_CALL(delegate, WillRunTest(_, _)).WillRepeatedly(testing::Return(true));
        EXPECT_CALL(delegate, ProcessTestResults(_, _)).Times(0);
        EXPECT_CALL(delegate, GetCommandLine(_, _, _)).WillRepeatedly(testing::Return(CommandLine(CommandLine::NO_PROGRAM)));
        EXPECT_CALL(delegate, GetWrapper()).WillRepeatedly(testing::Return(std::string()));
        EXPECT_CALL(delegate, IsPreTask(_)).WillRepeatedly(testing::Return(true));
        EXPECT_CALL(delegate, GetLaunchOptions()).WillRepeatedly(testing::Return(true));
        EXPECT_CALL(delegate, GetTimeout()).WillRepeatedly(testing::Return(TimeDelta()));
        EXPECT_CALL(delegate, GetBatchSize()).WillRepeatedly(testing::Return(batch_size));
    }

    std::unique_ptr<CommandLine> command_line;
    MockTestLauncher test_launcher;
    MockTestLauncherDelegate delegate;
    base::test::TaskEnvironment task_environment { base::test::TaskEnvironment::MainThreadType::IO };
    ScopedTempDir dir;
...
ACTION_P3(OnTestResult, launcher, full_name, status)
{
    ...
    arg0->PostTask(FROM_HERE, BindOnce(&TestLauncher::OnTestFinished, Unretained(launcher), result));
}
...
TEST_F(TestLauncherTest, FilterDisabledTestByDefault)
{
    ...
    AddMockedTests("Test", { "firstTest", "secondTest", "DISABLED_firstTestDisabled" });
    SetUpExpectCalls();
    std::vector<std::string> tests_names = { "Test.firstTest", "Test.secondTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS),
            OnTestResult(&test_launcher, "Test.secondTest", TestResult::TEST_SUCCESS)));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, ReorderPreTests)
{
    ...
    SetUpExpectCalls();
    std::vector<std::string> tests_names = { "Test.PRE_PRE_firstTest", "Test.PRE_firstTest", "Test.firstTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _)).Times(1);
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, UsingCommandLineFilter)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, FilterIncludePreTest)
{
    ...
    std::vector<std::string> tests_names = { "Test.PRE_firstTest", "Test.firstTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _)).Times(1);
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
{
    AddMockedTests("Test", { "firstTest", "PRE_firstTest", "secondTest", "PRE_secondTest", "thirdTest", "DISABLED_Disable1" });
    SetUpExpectCalls();
    command_line->AppendSwitchASCII("gtest_filter", "Test.*Test:-Test.secondTest");
...
TEST_F(TestLauncherTest, RepeatTest)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _))
        .Times(2)
        .WillRepeatedly(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS)));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, RunningMultipleIterationsUntilFailure)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS)))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS)))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_FAILURE)));
    EXPECT_FALSE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, SuccessOnRetryTests)
{
    ...
    std::vector<std::string> tests_names = { "Test.firstTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_FAILURE))
        .WillOnce(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, FailOnRetryTests)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .Times(3)
        .WillRepeatedly(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_FAILURE));
    EXPECT_FALSE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, RetryPreTests)
{
    ...
    SetUpExpectCalls();
    command_line->AppendSwitchASCII("test-launcher-retry-limit", "2");
    std::vector<TestResult> results = { GenerateTestResult("Test.PRE_PRE_firstTest", TestResult::TEST_SUCCESS),
        GenerateTestResult("Test.PRE_firstTest", TestResult::TEST_FAILURE), GenerateTestResult("Test.firstTest", TestResult::TEST_SUCCESS) };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.PRE_PRE_firstTest", TestResult::TEST_SUCCESS),
            OnTestResult(&test_launcher, "Test.PRE_firstTest", TestResult::TEST_FAILURE),
            OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS)));
    std::vector<std::string> tests_names = { "Test.PRE_PRE_firstTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.PRE_PRE_firstTest", TestResult::TEST_SUCCESS));
    tests_names = { "Test.PRE_firstTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.PRE_firstTest", TestResult::TEST_SUCCESS));
...
TEST_F(TestLauncherTest, PreTestFailure)
{
    ...
    std::vector<TestResult> results
        = { GenerateTestResult("Test.PRE_FirstTest", TestResult::TEST_FAILURE), GenerateTestResult("Test.FirstTest", TestResult::TEST_SUCCESS) };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.PRE_FirstTest", TestResult::TEST_FAILURE),
            OnTestResult(&test_launcher, "Test.FirstTest", TestResult::TEST_SUCCESS)));
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAre("Test.PRE_FirstTest"), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.PRE_FirstTest", TestResult::TEST_FAILURE));
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAre("Test.FirstTest"), _, _))
        .WillOnce(OnTestResult(&test_launcher, "Test.FirstTest", TestResult::TEST_SUCCESS));
    EXPECT_FALSE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, RunDisabledTests)
{
    ...
    command_line->AppendSwitchASCII("gtest_filter", "Test*.first*");
    std::vector<std::string> tests_names = { "DISABLED_TestDisabled.firstTest", "Test.firstTest", "Test.DISABLED_firstTestDisabled" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS),
            OnTestResult(&test_launcher, "DISABLED_TestDisabled.firstTest", TestResult::TEST_SUCCESS),
            OnTestResult(&test_launcher, "Test.DISABLED_firstTestDisabled", TestResult::TEST_SUCCESS)));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, DoesRunFilteredTests)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _))
        .WillOnce(::testing::DoAll(OnTestResult(&test_launcher, "Test.secondTest", TestResult::TEST_SUCCESS)));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
{
    AddMockedTests("Test", { "DISABLED_firstTest", "PRE_PRE_firstTest", "PRE_firstTest", "secondTest" });
    SetUpExpectCalls();
    std::vector<std::string> tests_names = { "Test.secondTest" };
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, testing::ElementsAreArray(tests_names.cbegin(), tests_names.cend()), _, _)).Times(1);
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
...
TEST_F(TestLauncherTest, EnforceRunTestsInExactPositiveFailWithGtestFilterFlag)
{
    command_line->AppendSwitch("enforce-exact-positive-filter");
...
TEST_F(TestLauncherTest, EnforceRunTestsInExactPositiveFailWithWildcardPositiveFilter)
{
    command_line->AppendSwitch("enforce-exact-positive-filter");
    ASSERT_TRUE(dir.CreateUniqueTempDir());
    FilePath path = dir.GetPath().AppendASCII("test.filter");
...
TEST_F(TestLauncherTest, ExcessiveOutput)
{
    ...
    TestResult test_result = GenerateTestResult("Test.firstTest", TestResult::TEST_SUCCESS, Milliseconds(30), std::string(500000, 'a'));
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _)).WillOnce(OnTestResult(&test_launcher, test_result));
    EXPECT_FALSE(test_launcher.Run(command_line.get()));
}
...
TEST_F(TestLauncherTest, RedirectStdio)
{
    ...
    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _)).WillOnce(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));
}
...
bool ValidateTestResultObject(const Value::Dict& iteration_data, TestResult& test_result)
{
    ...
    bool result = ValidateKeyValue(*dict, "elapsed_time_ms", test_result.elapsed_time.InMilliseconds());
    ...
    result &= ValidateKeyValue(*dict, "output_snippet", test_result.output_snippet);

    std::string base64_output_snippet = base::Base64Encode(test_result.output_snippet);
    result &= ValidateKeyValue(*dict, "output_snippet_base64", base64_output_snippet);
...
// at |key| value.
bool ValidateStringList(const std::optional<Value::Dict>& root, const std::string& key, std::vector<const char*> values)
{
    ...
    if (values.size() != list->size()) {
        ADD_FAILURE() << "expected size: " << values.size() << ", actual size:" << list->size();
        return false;
    }

    for (unsigned i = 0; i < values.size(); i++) {
        if (!(*list)[i].is_string() && (*list)[i].GetString().compare(values.at(i))) {
...
TEST_F(TestLauncherTest, JsonSummary)
{
    ...
    TestResult first_result = GenerateTestResult("Test.firstTest", TestResult::TEST_SUCCESS, Milliseconds(30), "output_first");
    first_result.test_result_parts.push_back(GenerateTestResultPart(TestResultPart::kSuccess, "TestFile", 110, "summary", "message"));
    TestResult second_result = GenerateTestResult("Test.secondTest", TestResult::TEST_SUCCESS, Milliseconds(50), "output_second");

    EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _))
        .Times(2)
        .WillRepeatedly(::testing::DoAll(OnTestResult(&test_launcher, first_result), OnTestResult(&test_launcher, second_result)));
    EXPECT_TRUE(test_launcher.Run(command_line.get()));

    // Validate the resulting JSON file is the expected output.
    std::optional<Value::Dict> root = test_launcher_utils::ReadSummary(path);
    ASSERT_TRUE(root);
    EXPECT_TRUE(ValidateStringList(root, "all_tests", { "Test.firstTest", "Test.firstTestDisabled", "Test.secondTest", "TestDisabled.firstTest" }));
    EXPECT_TRUE(ValidateStringList(root, "disabled_tests", { "Test.firstTestDisabled", "TestDisabled.firstTest" }));

    const Value::Dict* dict = root->FindDict("test_locations");
    ASSERT_TRUE(dict);
    EXPECT_EQ(2u, dict->size());
    ASSERT_TRUE(test_launcher_utils::ValidateTestLocation(*dict, "Test.firstTest", "File", 100));
    ASSERT_TRUE(test_launcher_utils::ValidateTestLocation(*dict, "Test.secondTest", "File", 100));
...
TEST_F(TestLauncherTest, JsonSummaryWithDisabledTests)
{
    ...
    // Setup results to be returned by the test launcher delegate.
    TestResult test_result = GenerateTestResult("Test.DISABLED_Test", TestResult::TEST_SUCCESS, Milliseconds(50), "output_second");
    ...
    EXPECT_TRUE(test_launcher_utils::ValidateTestLocation(*dict, "Test.DISABLED_Test", "File", 100));
...
TEST_F(TestLauncherTest, TestChildTempDir)
{
    AddMockedTests("Test", { "firstTest" });
    SetUpExpectCalls();
    ON_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, _)).WillByDefault(OnTestResult(&test_launcher, "Test.firstTest", TestResult::TEST_SUCCESS));
    ...
    if (TestLauncher::SupportsPerChildTempDirs()) {
        ...
        EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, ::testing::ResultOf(DirectoryExists, ::testing::IsTrue())))
        ...
        // |child_temp_dir| arg.
        EXPECT_CALL(test_launcher, LaunchChildGTestProcess(_, _, _, FilePath())).WillOnce(::testing::SaveArg<2>(&task_temp));
    }
...
{
    UnitTestLauncherDelegate launcher_delegate(&defaultPlatform, 10u, true, DoNothing());
    ...
    CommandLine cmd_line = delegate_ptr->GetCommandLine(test_names, temp_dir, &result_file);
    ...
    const int size = 2048;
    std::string content;
    ASSERT_TRUE(ReadFileToStringWithMaxSize(cmd_line.GetSwitchValuePath("gtest_flagfile"), &content, size));
    EXPECT_EQ(content.find("--gtest_filter="), 0u);
    base::ReplaceSubstringsAfterOffset(&content, 0, "--gtest_filter=", "");
    std::vector<std::string> gtest_filter_tests = SplitString(content, ":", TRIM_WHITESPACE, SPLIT_WANT_ALL);
...
TEST_F(ResultWatcherTest, PollCompletesQuickly)
{
    ...
    ASSERT_TRUE(AppendToFile(result_file,
        StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeFormatAsIso8601(Time::Now()).c_str(), "\" />\n",
    ...
    EXPECT_CALL(result_watcher, WaitWithTimeout(_))
        .WillOnce(DoAll(InvokeWithoutArgs([&] { task_environment.AdvanceClock(Milliseconds(1500)); }), Return(true)));
...
TEST_F(ResultWatcherTest, PollCompletesSlowly)
{
    ...
    ASSERT_TRUE(
        AppendToFile(result_file, StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeFormatAsIso8601(start).c_str(), "\" />\n" })));
...
TEST_F(ResultWatcherTest, PollTimeout)
{
    ...
    FilePath result_file = CreateResultFile();
    ASSERT_TRUE(
        AppendToFile(result_file, StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeFormatAsIso8601(Time::Now()).c_str(), "\" />\n" })));

    MockResultWatcher result_watcher(result_file, 10);
    EXPECT_CALL(result_watcher, WaitWithTimeout(_))
        .Times(2)
        .WillRepeatedly(DoAll(Invoke(&task_environment, &test::TaskEnvironment::AdvanceClock), Return(false)));
...
TEST_F(ResultWatcherTest, RetryIncompleteResultRead)
{
    ...
    Time start = Time::Now();
    ASSERT_TRUE(result_watcher.PollUntilDone(Seconds(45)));
    ASSERT_EQ(Time::Now() - start, Seconds(45) + 4 * TestTimeouts::tiny_timeout());
}
...
TEST_F(ResultWatcherTest, PollWithClockJumpBackward)
{
    ...
    ASSERT_TRUE(AppendToFile(result_file,
        StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeFormatAsIso8601(time_before_change).c_str(), "\" />\n",
            "    <testcase name=\"B\" status=\"run\" time=\"0.500\" "
            "classname=\"A\" timestamp=\"",
    ...
    MockResultWatcher result_watcher(result_file, 2);
    EXPECT_CALL(result_watcher, WaitWithTimeout(_))
        .WillOnce(DoAll(Invoke(&task_environment, &test::TaskEnvironment::AdvanceClock), Return(false)))
        .WillOnce(DoAll(Invoke(&task_environment, &test::TaskEnvironment::AdvanceClock), Return(true)));
...
{
    UnitTestLauncherDelegate launcher_delegate(&defaultPlatform, 15u, true, DoNothing());
    TestLauncherDelegate* delegate_ptr = &launcher_delegate;
...
TEST_F(UnitTestLauncherDelegateTester, RunMockTests)
{
    ...
    EXPECT_TRUE(test_launcher_utils::ValidateTestLocations(*dict, "MockUnitTests"));
    ...
    // We expect the result to be stripped of disabled prefix.
    EXPECT_TRUE(test_launcher_utils::ValidateTestResult(*iteration_dict, "MockUnitTests.PassTest", "SUCCESS", 0u));
    EXPECT_TRUE(test_launcher_utils::ValidateTestResult(*iteration_dict, "MockUnitTests.FailTest", "FAILURE", 1u));
    EXPECT_TRUE(test_launcher_utils::ValidateTestResult(*iteration_dict, "MockUnitTests.CrashTest", "CRASH", 0u));
    EXPECT_TRUE(test_launcher_utils::ValidateTestResult(*iteration_dict, "MockUnitTests.NoRunTest", "NOTRUN", 0u,
...
TEST(ProcessGTestOutputTest, FoundTestCaseNotEnforced)
{
    ...
    std::vector<std::string> lines = base::SplitString(output, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        if (banner_has_printed && tests_not_enforced.size() < 2) {
            ...
            std::vector<std::string> line_vec = base::SplitString(lines[i], "]", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
// the test case cannot catch the "Check failed" line.
    ...
    // to compare.
    EXPECT_THAT(tests_not_enforced, testing::UnorderedElementsAre("Test.firstTest", "Test.secondTest"));
}
#endif // !BUILDFLAG(IS_FUCHSIA)
...
{
    Process child_process = SpawnMultiProcessTestChild("LeakChildProcess", GetMultiProcessTestChildBaseCommandLine(), LaunchOptions());
...
{
    CommandLine command_line(CommandLine::ForCurrentProcess()->GetProgram());
    command_line.AppendSwitchASCII("gtest_filter", "LeakedChildProcessTest.DISABLED_LeakChildProcess");
    ...
    EXPECT_TRUE(test_launcher_utils::ValidateTestLocations(*dict, "LeakedChildProcessTest"));
...
{
    const std::string output = "[ RUN      ] TestCase.FirstTest\n"
...
MATCHER(CheckTruncationPreservesMessage, "")
{
    ...
    const std::string snippet = base::StrCat({ "[ RUN      ] SampleTestSuite.SampleTestName\n"
...
void MatchesFatalMessagesTest()
{
    ...
    // the presence/absence of an extra logging prefix.
    {
        ...
        EXPECT_DEATH_IF_SUPPORTED(LOG(FATAL) << "message", CheckTruncationPreservesMessage());
    }
    {
...
TEST(TestLauncherTools, TruncateSnippetFocusedTest)
{
    ...
    const std::string snippet_two = "[ RUN      ] NetworkingPrivateApiTest.CreateSharedNetwork\n"
    ...
    const std::string snippet_three = "[ RUN      ] All/PDFExtensionAccessibilityTreeDumpTest.Highlights/"
                                      "linux\n"
    ...
    const std::string snippet_four = "[ RUN      ] All/PassingTest/linux\n"
