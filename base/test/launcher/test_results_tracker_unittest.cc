...
TEST(TestResultsTrackerTest, SaveSummaryAsJSONWithLinkInResult)
{
    ...
    tracker.per_iteration_data_.emplace_back(TestResultsTracker::PerIterationData());
...
TEST(TestResultsTrackerTest, RepeatDisabledTests)
{
    ...
    TestResultsTracker::TestStatusMap results = tracker.GetTestStatusMapForAllIterations();
    ASSERT_TRUE(results[TestResult::TEST_SUCCESS].find(TEST_NAME) == results[TestResult::TEST_SUCCESS].end());
    ASSERT_TRUE(results[TestResult::TEST_SUCCESS].find(TEST_NAME_WITHOUT_PREFIX) != results[TestResult::TEST_SUCCESS].end());
}
