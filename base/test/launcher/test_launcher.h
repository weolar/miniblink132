...
public:
    ...
    virtual void ProcessTestResults(std::vector<TestResult>& test_results, TimeDelta elapsed_time)
    {
    }
    ...
    // be inside |temp_dir|.
    virtual CommandLine GetCommandLine(const std::vector<std::string>& test_names, const FilePath& temp_dir, FilePath* output_file) = 0;
...
public:
...
#if BUILDFLAG(IS_WIN)
        base::LaunchOptions::Inherit inherit_mode = base::LaunchOptions::Inherit::kSpecific;
    ...
    TestLauncher(TestLauncherDelegate* launcher_delegate, size_t parallel_jobs, size_t retry_limit = 1U);
...
private:
    ...
    void CombinePositiveTestFilters(std::vector<std::string> filter_a, std::vector<std::string> filter_b);
    ...
    void ProcessTestResults(const std::vector<std::string>& test_names, const FilePath& result_file, const std::string& output, TimeDelta elapsed_time,
        int exit_code, bool was_timeout, PlatformThreadId thread_id, int process_num, int leaked_items);
