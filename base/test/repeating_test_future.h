...
template <typename... Types> class RepeatingTestFuture {
public:
    ...
    //
    template <typename... CallbackArgumentsTypes> base::RepeatingCallback<void(CallbackArgumentsTypes...)> GetCallback()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        return base::BindRepeating(
            [](WeakPtr<RepeatingTestFuture<Types...>> future, CallbackArgumentsTypes... values) {
                if (future) {
...
// callback.
template <> class RepeatingTestFuture<void> {
public:
    void AddValue()
    {
        implementation_.AddValue(true);
    }
    ...
    bool IsEmpty() const
    {
        return implementation_.IsEmpty();
    }
