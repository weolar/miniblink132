...
bool RebindableTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    return task_runner_->PostDelayedTask(from_here, std::move(task), delay);
}

bool RebindableTaskRunner::PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    return task_runner_->PostNonNestableDelayedTask(from_here, std::move(task), delay);
}
...
{
    static base::NoDestructor<scoped_refptr<RebindableTaskRunner>> task_runner(MakeRefCounted<RebindableTaskRunner>());
...
TracingEnvironment::TracingEnvironment(TaskEnvironment& task_environment, scoped_refptr<SequencedTaskRunner> task_runner)
