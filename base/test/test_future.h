...
//
template <typename... Types> class TestFuture {
public:
    using TupleType = std::tuple<std::decay_t<Types>...>;

    static_assert(std::tuple_size_v<TupleType> > 0, "Don't use TestFuture<> but use TestFuture<void> instead");
    ...
    //
    template <typename Type> const auto& Get()
    {
    ...
    //
    template <typename... CallbackArgumentsTypes> OnceCallback<void(CallbackArgumentsTypes...)> GetCallback()
    {
        return GetRepeatingCallback<CallbackArgumentsTypes...>();
    }

    OnceCallback<void(Types...)> GetCallback()
    {
    ...
    //
    template <typename... CallbackArgumentsTypes> RepeatingCallback<void(CallbackArgumentsTypes...)> GetRepeatingCallback()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        return BindRepeating(
            [](WeakPtr<TestFuture<Types...>> future, CallbackArgumentsTypes... values) {
                if (future) {
    ...
    //   future.Wait();
    //
    template <typename... CallbackArgumentsTypes> OnceCallback<void(CallbackArgumentsTypes...)> GetSequenceBoundCallback()
    {
    ...
    //   EXPECT_EQ(future.Take(), "expected-third-value");
    //
    template <typename... CallbackArgumentsTypes> RepeatingCallback<void(CallbackArgumentsTypes...)> GetSequenceBoundRepeatingCallback()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        return BindPostTask(base::SequencedTaskRunner::GetCurrentDefault(), GetRepeatingCallback<CallbackArgumentsTypes...>());
    }
    ...
    void SetValue(Types... values)
    {
        ...
        EXPECT_FALSE(values_.has_value()) << "Received new value " << ToString(new_values) //
                                          << " before old value " << ToString(GetTuple()) << " was consumed through Take() or Clear().";
...
private:
    ...
    base::RepeatingClosure ready_signal_ GUARDED_BY_CONTEXT(sequence_checker_) = base::DoNothing();
...
template <> class TestFuture<void> {
public:
    ...
    void Get()
    {
        std::ignore = implementation_.Get();
    }

    // Returns true if the callback or `SetValue()` was invoked.
    bool IsReady() const
    {
        return implementation_.IsReady();
    }
    ...
    {
        return BindRepeating(implementation_.GetSequenceBoundRepeatingCallback(), true);
    }

    // Indicates this `TestFuture` is ready, and unblocks any waiters.
    void SetValue()
    {
        implementation_.SetValue(true);
    }
...
// and .WillRepeatedly().
template <typename... Types> class InvokeFuture {
public:
...
template <> class InvokeFuture<void> {
public:
...
// InvokeFuture.
template <typename... Types> InvokeFuture(TestFuture<Types...>&) -> InvokeFuture<Types...>;
