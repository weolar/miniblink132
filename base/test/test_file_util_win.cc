...
void* GetPermissionInfo(const FilePath& path, size_t* length)
{
    ...
    *length = 0;
    PACL dacl = nullptr;
    PSECURITY_DESCRIPTOR security_descriptor;
    if (GetNamedSecurityInfo(path.value().c_str(), SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, &dacl, nullptr, &security_descriptor)
        != ERROR_SUCCESS) {
        return nullptr;
    }
    DCHECK(dacl);
...
bool RestorePermissionInfo(const FilePath& path, void* info, size_t length)
{
    ...
    DWORD rc
        = SetNamedSecurityInfo(const_cast<wchar_t*>(path.value().c_str()), SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, &perm->dacl, nullptr);
...
bool EvictFileFromSystemCache(const FilePath& file)
{
    ...
    win::ScopedHandle file_handle(CreateFile(file_value.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_FLAG_NO_BUFFERING, nullptr));
    ...
    CHECK(::SetFileTime(file_handle.get(), &bhi.ftCreationTime, &bhi.ftLastAccessTime, &bhi.ftLastWriteTime));
    return true;
}
...
bool DenyFilePermission(const FilePath& path, DWORD permission)
{
    ...
    if (GetNamedSecurityInfo(path_ptr.get(), SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, &old_dacl, nullptr, &security_descriptor)
        != ERROR_SUCCESS) {
        return false;
    }

    std::unique_ptr<TCHAR[]> current_user = ToCStr(std::wstring(L"CURRENT_USER"));
    EXPLICIT_ACCESS new_access = { permission, DENY_ACCESS, 0, { nullptr, NO_MULTIPLE_TRUSTEE, TRUSTEE_IS_NAME, TRUSTEE_IS_USER, current_user.get() } };
    ...
    DWORD rc = SetNamedSecurityInfo(path_ptr.get(), SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, new_dacl, nullptr);
    LocalFree(security_descriptor);
    LocalFree(new_dacl);
...
std::wstring GetFileDacl(const FilePath& path)
{
    ...
    if (::GetNamedSecurityInfo(path.value().c_str(), SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, nullptr, nullptr, nullptr, nullptr, &sd) != ERROR_SUCCESS) {
        return std::wstring();
    }
    auto sd_ptr = win::TakeLocalAlloc(sd);
    LPWSTR sddl;
    if (!::ConvertSecurityDescriptorToStringSecurityDescriptor(sd_ptr.get(), SDDL_REVISION_1, DACL_SECURITY_INFORMATION, &sddl, nullptr)) {
        return std::wstring();
    }
...
bool CreateWithDacl(const FilePath& path, wcstring_view sddl, bool directory)
{
    ...
    return win::ScopedHandle(::CreateFile(path.value().c_str(), GENERIC_ALL, 0, &security_attr, CREATE_ALWAYS, 0, nullptr)).is_valid();
}
