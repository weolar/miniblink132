...
class TestMockTimeTaskRunner : public SingleThreadTaskRunner, public RunLoop::Delegate {
public:
    ...
    private:
        SingleThreadTaskRunner::CurrentHandleOverrideForTesting single_thread_task_runner_current_default_handle_override_;
    };
    ...
    TestMockTimeTaskRunner(Time start_time, TimeTicks start_ticks, Type type = Type::kStandalone);
    ...
    bool PostDelayedTaskAt(
        subtle::PostDelayedTaskPassKey, const Location& from_here, OnceClosure task, TimeTicks delayed_run_time, subtle::DelayPolicy deadline_policy) override;
...
private:
    ...
    class TemporalOrder {
    public:
        bool operator()(const TestOrderedPendingTask& first_task, const TestOrderedPendingTask& second_task) const;
    };

    typedef std::priority_queue<TestOrderedPendingTask, std::vector<TestOrderedPendingTask>, TemporalOrder> TaskPriorityQueue;
    ...
    bool DequeueNextTask(const TimeTicks& reference, const TimeDelta& max_delta, TestPendingTask* next_task);
