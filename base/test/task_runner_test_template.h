...
template <typename TaskRunnerTestDelegate> class TaskRunnerTest : public testing::Test {
protected:
    TaskRunnerTest()
        : task_tracker_(base::MakeRefCounted<test::TaskTracker>())
    {
    }
...
TYPED_TEST_P(TaskRunnerTest, Basic)
{
    ...
    for (int i = 0; i < 20; ++i) {
        RepeatingClosure ith_task = this->task_tracker_->WrapTask(RepeatingClosure(), i);
        for (int j = 0; j < i + 1; ++j) {
