...
template <typename Signature, typename R, typename F, typename... Args> static constexpr bool kHasConstCallOperator = false;
template <typename R, typename F, typename... Args> static constexpr bool kHasConstCallOperator<R (F::*)(Args...) const, R, F, Args...> = true;

// Implementation of `BindLambdaForTesting()`, which checks preconditions before
// handing off to `Bind{Once,Repeating}()`.
template <typename Lambda, typename = ExtractCallableRunType<std::decay_t<Lambda>>> struct BindLambdaForTestingHelper;

template <typename Lambda, typename R, typename... Args> struct BindLambdaForTestingHelper<Lambda, R(Args...)> {
private:
    using F = std::decay_t<Lambda>;

    // For context on this "templated struct with a lambda that asserts" pattern,
    // see comments in `Invoker<>`.
    template <bool v = std::is_rvalue_reference_v<Lambda&&> && !std::is_const_v<std::remove_reference_t<Lambda>>> struct IsNonConstRvalueRef {
...
public:
    ...
    {
        if constexpr (kHasConstCallOperator<decltype(&F::operator()), R, F, Args...>) {
...
// can do.
template <typename Lambda> auto BindLambdaForTesting(Lambda&& lambda)
{
    return internal::BindLambdaForTestingHelper<Lambda>::BindLambdaForTesting(std::forward<Lambda>(lambda));
}

// Returns a closure that fails on destruction if it hasn't been run.
OnceClosure MakeExpectedRunClosure(const Location& location, std::string_view message = std::string_view());
RepeatingClosure MakeExpectedRunAtLeastOnceClosure(const Location& location, std::string_view message = std::string_view());

// Returns a closure that fails the test if run.
RepeatingClosure MakeExpectedNotRunClosure(const Location& location, std::string_view message = std::string_view());
