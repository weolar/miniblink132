...
TEST_F(TestTraceProcessorExample, Basic)
{
    ...
    EXPECT_THAT(result.value(), ::testing::ElementsAre(std::vector<std::string> { "name" }, std::vector<std::string> { "test_event" }));
}
...
TEST_F(TestTraceProcessorExample, BasicTraceConfig)
{
    ...
    test_trace_processor.StartTrace(base::test::DefaultTraceConfig("test_category", /*privacy_filtering=*/false));
