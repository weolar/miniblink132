...
public:
    ...
    explicit MoveOnlyValue(int data)
        : data(data)
    {
    }
...
public:
    ...
    template <typename Lambda> void RunLater(Lambda lambda, scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunner::GetCurrentDefault())
    {
        task_runner->PostTask(FROM_HERE, BindLambdaForTesting(lambda));
    }

    void RunLater(OnceClosure callable, scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunner::GetCurrentDefault())
    {
        task_runner->PostTask(FROM_HERE, std::move(callable));
    }

    void RunLater(RepeatingClosure callable, scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunner::GetCurrentDefault())
    {
        task_runner->PostTask(FROM_HERE, std::move(callable));
    }

    void PostDelayedTask(OnceClosure callable, base::TimeDelta delay, scoped_refptr<SequencedTaskRunner> task_runner = SequencedTaskRunner::GetCurrentDefault())
    {
...
TEST_F(TestFutureTest, WaitShouldBlockUntilValueArrives)
{
    ...
    PostDelayedTask(BindOnce(future.GetCallback(), expected_value), Milliseconds(1));
...
TEST_F(TestFutureTest, WaitShouldBlockUntilValueArrivesOnOtherSequence)
{
    ...
    PostDelayedTask(BindOnce(future.GetSequenceBoundCallback(), expected_value), Milliseconds(1), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, WaitShouldReturnTrueWhenValueArrivesOnOtherSequence)
{
    ...
    PostDelayedTask(BindOnce(future.GetSequenceBoundCallback(), kAnyValue), Milliseconds(1), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, TakeShouldWorkWithMoveOnlyValueOnOtherSequence)
{
    ...
    RunLater(BindOnce(future.GetSequenceBoundCallback(), MoveOnlyValue(expected_data)), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, ShouldAllowReusingCallback)
{
    ...
    RepeatingCallback<void(std::string)> sequence_bound_callback = future.GetSequenceBoundRepeatingCallback();
    ...
    RunLater(BindOnce(sequence_bound_callback, "fourth value"), other_task_runner);
...
TEST_F(TestFutureTest, ShouldAllowReferenceArgumentsForCallbackOnOtherSequence)
{
    const int expected_value = 222;
    TestFuture<int> future;

    OnceCallback<void(const int&)> callback = future.GetSequenceBoundCallback<const int&>();
    RunLater(BindOnce(std::move(callback), expected_value), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, ShouldAllowInvokingCallbackOnOtherSequenceAfterFutureIsDestroyed)
{
    OnceCallback<void(int)> callback;
    ...
    ThreadPool::PostTask(FROM_HERE, BindOnce(std::move(callback), 1).Then(run_loop.QuitClosure()));
    run_loop.Run();
}
...
TEST_F(TestFutureTest, ShouldReturnTupleValue)
{
    ...
    RunLater(BindOnce(future.GetCallback(), expected_int_value, expected_string_value));
...
TEST_F(TestFutureTest, ShouldReturnTupleValueOnOtherSequence)
{
    ...
    RunLater(BindOnce(future.GetSequenceBoundCallback(), expected_int_value, expected_string_value), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, ShouldAllowReferenceArgumentsForMultiArgumentCallback)
{
    ...
    OnceCallback<void(int, const std::string&)> callback = future.GetCallback<int, const std::string&>();
    RunLater(BindOnce(std::move(callback), expected_int_value, expected_string_value));
...
TEST_F(TestFutureTest, ShouldAllowReferenceArgumentsForMultiArgumentCallbackOnOtherSequence)
{
    const int expected_int_value = 5;
    const std::string expected_string_value = "value";
    ...
    OnceCallback<void(int, const std::string&)> callback = future.GetSequenceBoundCallback<int, const std::string&>();
    RunLater(BindOnce(std::move(callback), expected_int_value, expected_string_value), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureTest, SetValueShouldAllowMultipleArguments)
{
    ...
    RunLater([&future, expected_string_value]() { future.SetValue(expected_int_value, expected_string_value); });
...
TEST_F(TestFutureTest, ShouldSupportMultipleCvRefTypes)
{
    ...
    OnceCallback<void(const int, std::string&, const long&)> callback = future.GetCallback();
    std::move(callback).Run(expected_first_value, expected_second_value, expected_third_value);
...
TEST_F(TestFutureTest, ShouldPrintCurrentValueIfItIsOverwritten)
{
    ...
    EXPECT_NONFATAL_FAILURE(future.SetValue("second-value", 2222, UnprintableValue()), "old value <first-value, 1111, [4-byte object at 0x");
}
...
TEST_F(TestFutureTest, ShouldPrintNewValueIfItOverwritesOldValue)
{
    ...
    EXPECT_NONFATAL_FAILURE(future.SetValue("second-value", 2222, UnprintableValue()), "new value <second-value, 2222, [4-byte object at 0x");
}
...
TEST_F(TestFutureWithoutValuesTest, WaitShouldUnblockWhenCallbackIsInvokedOnOtherSequence)
{
    TestFuture<void> future;

    RunLater(future.GetSequenceBoundCallback(), ThreadPool::CreateSequencedTaskRunner({}));
...
TEST_F(TestFutureWithoutValuesTest, GetShouldUnblockWhenCallbackIsInvokedOnOtherSequence)
{
    TestFuture<void> future;
...
TEST(TestFutureWithoutTaskEnvironmentTest, CanCreateTestFutureBeforeTaskEnvironment)
{
    TestFuture<AnyType> future;
...
TEST(TestFutureWithoutTaskEnvironmentDeathTest, WaitShouldDcheckWithoutTaskEnvironment)
{
    TestFuture<AnyType> future;

    EXPECT_CHECK_DEATH_WITH((void)future.Wait(), "requires a single-threaded context");
}
