...
{
    static NoDestructor<ObserverList<TaskEnvironment::DestructionObserver>> instance;
    return *instance;
}
...
base::MessagePumpType GetMessagePumpTypeForMainThreadType(TaskEnvironment::MainThreadType main_thread_type)
{
...
std::unique_ptr<sequence_manager::SequenceManager> CreateSequenceManagerForMainThreadType(
    TaskEnvironment::MainThreadType main_thread_type, sequence_manager::SequenceManager::PrioritySettings priority_settings)
{
    auto type = GetMessagePumpTypeForMainThreadType(main_thread_type);
    return sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(MessagePump::Create(type),
        base::sequence_manager::SequenceManager::Settings::Builder().SetMessagePumpType(type).SetPrioritySettings(std::move(priority_settings)).Build());
}
...
class TaskEnvironment::TestTaskTracker : public internal::ThreadPoolImpl::TaskTrackerImpl {
public:
    TestTaskTracker();
...
private:
    ...
    void RunTask(internal::Task task, internal::TaskSource* sequence, const TaskTraits& traits) override;
...
public:
    explicit MockTimeDomain(sequence_manager::internal::SequenceManagerImpl* sequence_manager)
    ...
    static Time GetTime()
    {
        return Time::UnixEpoch() + (current_mock_time_domain_->NowTicks() - TimeTicks());
    }
    ...
    void SetThreadPool(internal::ThreadPoolImpl* thread_pool, const TestTaskTracker* thread_pool_task_tracker)
    {
        DCHECK(!thread_pool_);
    ...
    bool MaybeFastForwardToWakeUp(std::optional<sequence_manager::WakeUp> next_wake_up, bool quit_when_idle_requested) override
    {
    ...
    void AdvanceTimesToNextTaskTimeOrCap(TimeTicks next_task_time, bool advance_live_ticks)
    {
        AutoLock lock(now_ticks_lock_);
    ...
    NextTaskSource FastForwardToNextTaskOrCap(
        std::optional<sequence_manager::WakeUp> next_main_thread_wake_up, TimeTicks fast_forward_cap, bool advance_live_ticks)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        ...
        if (thread_pool_ && thread_pool_task_tracker_->TasksAllowedToRun()) {
            next_thread_pool_task_time = thread_pool_->NextScheduledRunTimeForTesting();
        }
        ...
        if (!next_main_thread_wake_up) {
            ...
            next_task_time = std::min(next_main_thread_wake_up->time, *next_thread_pool_task_time);
        }
        ...
        if (next_task_time && *next_task_time <= fast_forward_cap) {
            ...
            if (next_main_thread_wake_up && next_task_time == next_main_thread_wake_up->time) {
            ...
            thread_pool_->FlushAsyncForTesting(BindOnce(&sequence_manager::internal::SequenceManagerImpl::ScheduleWork, Unretained(sequence_manager_)));
...
private:
    ...
    raw_ptr<const TestTaskTracker, DanglingUntriaged> thread_pool_task_tracker_ = nullptr;

    const raw_ptr<sequence_manager::internal::SequenceManagerImpl, DanglingUntriaged> sequence_manager_;
    ...
    TimeTicks now_ticks_ GUARDED_BY(now_ticks_lock_) { base::subtle::TimeTicksNowIgnoringOverride().SnappedToNextTick(TimeTicks(), Milliseconds(1)) };

    // Only ever written to from the main sequence. Start from real Now() instead
    // of zero to give a more realistic view to tests.
    LiveTicks live_ticks_ GUARDED_BY(now_ticks_lock_) { base::subtle::LiveTicksNowIgnoringOverride() };
};

TaskEnvironment::MockTimeDomain* TaskEnvironment::MockTimeDomain::current_mock_time_domain_ = nullptr;

TaskEnvironment::TaskEnvironment(sequence_manager::SequenceManager::PrioritySettings priority_settings, TimeSource time_source, MainThreadType main_thread_type,
    ...
    bool subclass_creates_default_taskrunner, trait_helpers::NotATraitTag)
    ...
    , sequence_manager_(CreateSequenceManagerForMainThreadType(main_thread_type, std::move(priority_settings)))
    , mock_time_domain_(time_source != TimeSource::SYSTEM_TIME
              ? std::make_unique<TaskEnvironment::MockTimeDomain>(static_cast<sequence_manager::internal::SequenceManagerImpl*>(sequence_manager_.get()))
              : nullptr)
    , time_overrides_(time_source == TimeSource::MOCK_TIME ? std::make_unique<subtle::ScopedTimeClockOverrides>(
                          &MockTimeDomain::GetTime, &MockTimeDomain::GetTimeTicks, nullptr, &MockTimeDomain::GetLiveTicks)
                                                           : nullptr)
    , mock_clock_(mock_time_domain_ ? std::make_unique<TickClockBasedClock>(mock_time_domain_.get()) : nullptr)
    , scoped_lazy_task_runner_list_for_testing_(std::make_unique<internal::ScopedLazyTaskRunnerListForTesting>())
    ,
    // TODO(crbug.com/41435712): Enable Run() timeouts even for
    // instances created with TimeSource::MOCK_TIME.
    run_loop_timeout_(mock_time_domain_ ? nullptr
                                        : std::make_unique<ScopedRunLoopTimeout>(FROM_HERE, TestTimeouts::action_timeout(),
                                            BindRepeating(&sequence_manager::SequenceManager::DescribeAllPendingTasks, Unretained(sequence_manager_.get()))))
{
    ...
    if (!subclass_creates_default_taskrunner) {
        task_queue_ = sequence_manager_->CreateTaskQueue(sequence_manager::TaskQueue::Spec(sequence_manager::QueueName::TASK_ENVIRONMENT_DEFAULT_TQ));
        ...
        CHECK(base::SingleThreadTaskRunner::HasCurrentDefault()) << "SingleThreadTaskRunner::CurrentDefaultHandle should've been set "
                                                                    "now.";
    ...
    if (thread_pool_execution_mode_ == ThreadPoolExecutionMode::QUEUED && task_tracker_) {
...
{
    CHECK(!ThreadPoolInstance::Get()) << "Someone has already installed a ThreadPoolInstance. If nothing in "
    ...
    auto thread_pool = std::make_unique<internal::ThreadPoolImpl>(std::string(), std::move(task_tracker),
...
void TaskEnvironment::InitializeThreadPool()
{
    ...
    if (mock_time_domain_) {
        mock_time_domain_->SetThreadPool(static_cast<internal::ThreadPoolImpl*>(ThreadPoolInstance::Get()), task_tracker_);
    }
    ...
    if (thread_pool_com_environment_ == ThreadPoolCOMEnvironment::COM_MTA) {
        init_params.common_thread_pool_environment = ThreadPoolInstance::InitParams::CommonThreadPoolEnvironment::COM_MTA;
    }
#endif
...
void TaskEnvironment::CompleteInitialization()
{
    ...
    if (main_thread_type() == MainThreadType::IO) {
        file_descriptor_watcher_ = std::make_unique<FileDescriptorWatcher>(GetMainThreadTaskRunner());
    }
...
void TaskEnvironment::DeferredInitFromSubclass(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
...
scoped_refptr<base::SingleThreadTaskRunner> TaskEnvironment::GetMainThreadTaskRunner()
{
    DCHECK(task_runner_);
    return task_runner_;
}
...
bool TaskEnvironment::MainThreadIsIdle() const
{
    ...
    sequence_manager::internal::SequenceManagerImpl* sequence_manager_impl
        = static_cast<sequence_manager::internal::SequenceManagerImpl*>(sequence_manager_.get());
...
RepeatingClosure TaskEnvironment::QuitClosure()
{
    ...
    if (!run_until_quit_loop_) {
        run_until_quit_loop_ = std::make_unique<RunLoop>(RunLoop::Type::kNestableTasksAllowed);
    }
...
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    DCHECK(run_until_quit_loop_) << "QuitClosure() not called before RunUntilQuit()";
    ...
    if (!could_run_tasks) {
        EXPECT_TRUE(task_tracker_->DisallowRunTasks(TestTimeouts::action_max_timeout()))
...
void TaskEnvironment::FastForwardByInternal(TimeDelta delta, bool advance_live_ticks)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    ...
    } while (mock_time_domain_->FastForwardToNextTaskOrCap(sequence_manager_->GetNextDelayedWakeUp(), fast_forward_until, advance_live_ticks)
...
TimeDelta TaskEnvironment::NextMainThreadPendingTaskDelay() const
{
    ...
    std::optional<sequence_manager::WakeUp> wake_up = sequence_manager_->GetNextDelayedWakeUp();
...
TaskEnvironment::ParallelExecutionFence::ParallelExecutionFence(const char* error_message)
{
    CHECK(!g_task_tracker || g_task_tracker->OnControllerThread()) << error_message;
    if (g_task_tracker) {
        ...
        previously_allowed_to_run_ = g_task_tracker->TasksAllowedToRun() && !g_task_tracker->IsShutdownComplete();
        ...
        while (previously_allowed_to_run_ && !g_task_tracker->DisallowRunTasks(Seconds(5))) {
        ...
        LOG(WARNING) << "ParallelExecutionFence is ineffective when ThreadPoolInstance is "
...
bool TaskEnvironment::TestTaskTracker::DisallowRunTasks(TimeDelta timeout)
{
    ...
    for (TimeTicks now = subtle::TimeTicksNowIgnoringOverride(), end = now + timeout; !running_tasks_.empty() && now < end;
...
void TaskEnvironment::TestTaskTracker::RunTask(internal::Task task, internal::TaskSource* sequence, const TaskTraits& traits)
{
    ...
    internal::ThreadPoolImpl::TaskTrackerImpl::RunTask(std::move(task), sequence, traits);
    ...
    if ((after - before) > kTimeout) {
        ADD_FAILURE() << "TaskEnvironment: RunTask took more than " << kTimeout.InSeconds() << " seconds. Posted from " << posted_from.ToString();
    }
...
void TaskEnvironment::TestTaskTracker::BeginCompleteShutdown(base::WaitableEvent& shutdown_event)
{
    const TimeDelta kTimeout = TestTimeouts::action_max_timeout();
    ...
    ADD_FAILURE() << "TaskEnvironment: CompleteShutdown took more than " << kTimeout.InSeconds() << " seconds.\n" << failure_tasks;
...
{
    AutoLock auto_lock(lock_);
    ASSERT_TRUE(can_run_tasks_) << "FlushForTesting() requires ThreadPool tasks to be allowed to run or "
