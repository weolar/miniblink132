...
MATCHER_P(EqualsProto, message, "Match a proto Message equal to the matcher's argument.")
{
...
MATCHER(EqualsProto, "Matches if the tuple's proto Message arguments are equal.")
{
    return ::testing::Matcher<decltype(std::get<0>(arg))>(EqualsProto(std::get<1>(arg))).MatchAndExplain(std::get<0>(arg), result_listener);
}
