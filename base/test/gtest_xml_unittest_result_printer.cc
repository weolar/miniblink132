...
const char kTestPartLesultsLimitExceeded[] = "Test part results limit exceeded. Use --test-launcher-test-part-limit to "
...
void XmlUnitTestResultPrinter::AddLink(const std::string& name, const std::string& url)
{
    DCHECK(output_file_);
    DCHECK(!open_failed_);
    ...
    fprintf(output_file_.get(),
        ...
        "link_name=\"%s\">%s</link>\n",
        info->name(), info->test_suite_name(), name.c_str(), escaped_url.c_str());
    fflush(output_file_);
}

void XmlUnitTestResultPrinter::AddTag(const std::string& name, const std::string& value)
{
    DCHECK(output_file_);
    DCHECK(!open_failed_);
    ...
    fprintf(output_file_.get(),
        ...
        "tag_name=\"%s\">%s</tag>\n",
        info->name(), info->test_suite_name(), name.c_str(), escaped_value.c_str());
    fflush(output_file_);
}
...
bool XmlUnitTestResultPrinter::Initialize(const FilePath& output_file_path)
{
    ...
    fprintf(output_file_.get(), "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<testsuites>\n");
    fflush(output_file_);

    return true;
}

void XmlUnitTestResultPrinter::OnAssert(const char* file, int line, const std::string& summary, const std::string& message)
{
    WriteTestPartResult(file, line, testing::TestPartResult::kFatalFailure, summary, message);
}

void XmlUnitTestResultPrinter::OnTestSuiteStart(const testing::TestSuite& test_suite)
{
...
void XmlUnitTestResultPrinter::OnTestStart(const testing::TestInfo& test_info)
{
    ...
    fprintf(output_file_.get(), "    <x-teststart name=\"%s\" classname=\"%s\" timestamp=\"%s\" />\n", test_info.name(), test_info.test_suite_name(),
...
void XmlUnitTestResultPrinter::OnTestEnd(const testing::TestInfo& test_info)
{
    ...
    fprintf(output_file_.get(),
        ...
        test_info.name(), static_cast<double>(test_info.result()->elapsed_time()) / Time::kMillisecondsPerSecond, test_info.test_suite_name(),
        TimeFormatAsIso8601(Time::FromMillisecondsSinceUnixEpoch(test_info.result()->start_timestamp())).c_str());
    if (test_info.result()->Failed()) {
        fprintf(output_file_.get(), "      <failure message=\"\" type=\"\"></failure>\n");
    }

    int limit = test_info.result()->total_part_count();
    if (CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestLauncherTestPartResultsLimit)) {
        std::string limit_str = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTestLauncherTestPartResultsLimit);
    ...
    for (int i = 0; i < limit; ++i) {
        ...
        WriteTestPartResult(
            test_part_result.file_name(), test_part_result.line_number(), test_part_result.type(), test_part_result.summary(), test_part_result.message());
    }

    if (test_info.result()->total_part_count() > limit) {
        WriteTestPartResult("unknown", 0, testing::TestPartResult::kNonFatalFailure, kTestPartLesultsLimitExceeded, kTestPartLesultsLimitExceeded);
    }
...
void XmlUnitTestResultPrinter::OnTestSuiteEnd(const testing::TestSuite& test_suite)
{
    fprintf(output_file_.get(), "  </testsuite>\n");
    fflush(output_file_);
}
...
void XmlUnitTestResultPrinter::WriteTestPartResult(
    const char* file, int line, testing::TestPartResult::Type result_type, const std::string& summary, const std::string& message)
{
