...
void ValidatePathsAreEqual(const FilePath& expected_path, const FilePath& actual_path)
{
    ...
    EXPECT_NE(0U, ::GetLongPathName(actual_path.value().c_str(), long_actual_path_chars, MAX_PATH)) << "Failed to get LongPathName of " << actual_path.value();
    ...
    EXPECT_TRUE(base::FilePath::CompareEqualIgnoreCase(long_expected_path.value(), long_actual_path.value()));
}

void ValidateShortcut(const FilePath& shortcut_path, const ShortcutProperties& properties)
{
    ...
    EXPECT_TRUE(SUCCEEDED(hr = ::CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&i_shell_link))));
    if (FAILED(hr))
        return;
    ...
    // Load the shortcut.
    EXPECT_TRUE(SUCCEEDED(hr = i_persist_file->Load(shortcut_path.value().c_str(), 0))) << "Failed to load shortcut at " << shortcut_path.value();
    if (FAILED(hr))
        return;

    if (properties.options & ShortcutProperties::PROPERTIES_TARGET) {
        EXPECT_TRUE(SUCCEEDED(i_shell_link->GetPath(read_target, MAX_PATH, NULL, SLGP_SHORTPATH)));
    ...
    if (properties.options & ShortcutProperties::PROPERTIES_WORKING_DIR) {
        EXPECT_TRUE(SUCCEEDED(i_shell_link->GetWorkingDirectory(read_working_dir, MAX_PATH)));
        ValidatePathsAreEqual(properties.working_dir, FilePath(read_working_dir));
    }

    if (properties.options & ShortcutProperties::PROPERTIES_ARGUMENTS) {
        EXPECT_TRUE(SUCCEEDED(i_shell_link->GetArguments(read_arguments, MAX_PATH)));
        EXPECT_EQ(properties.arguments, read_arguments);
    }

    if (properties.options & ShortcutProperties::PROPERTIES_DESCRIPTION) {
        EXPECT_TRUE(SUCCEEDED(i_shell_link->GetDescription(read_description, MAX_PATH)));
        EXPECT_EQ(properties.description, read_description);
    }

    if (properties.options & ShortcutProperties::PROPERTIES_ICON) {
        EXPECT_TRUE(SUCCEEDED(i_shell_link->GetIconLocation(read_icon, MAX_PATH, &read_icon_index)));
    ...
    if (properties.options & ShortcutProperties::PROPERTIES_DUAL_MODE) {
        ...
        case VT_EMPTY:
            ...
            break;
        case VT_BOOL:
            EXPECT_EQ(properties.dual_mode, static_cast<bool>(pv_dual_mode.get().boolVal));
            break;
        default:
