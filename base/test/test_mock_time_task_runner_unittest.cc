...
{
    static constexpr TestMockTimeTaskRunner::Type kTestCases[] = { TestMockTimeTaskRunner::Type::kStandalone, TestMockTimeTaskRunner::Type::kBoundToThread };
    ...
    for (auto type : kTestCases) {
        ...
        mock_time_task_runner->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
        mock_time_task_runner->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));
        mock_time_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 256; }, Unretained(&counter)), Seconds(3));
        mock_time_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 64; }, Unretained(&counter)), Seconds(1));
        mock_time_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1024; }, Unretained(&counter)), Minutes(20));
        mock_time_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 4096; }, Unretained(&counter)), Days(20));
...
{
    auto bound_mock_time_task_runner = MakeRefCounted<TestMockTimeTaskRunner>(TestMockTimeTaskRunner::Type::kBoundToThread);
...
TEST(TestMockTimeTaskRunnerTest, AvoidCaptureWhenBound)
{
    ...
    {
        auto task_runner = MakeRefCounted<TestMockTimeTaskRunner>(TestMockTimeTaskRunner::Type::kBoundToThread);

        task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { captured = SingleThreadTaskRunner::GetCurrentDefault(); }));
        task_runner->RunUntilIdle();
    }

    {
        // This should not complain about RunLoop::Delegate already existing.
        auto task_runner2 = MakeRefCounted<TestMockTimeTaskRunner>(TestMockTimeTaskRunner::Type::kBoundToThread);
    }
}
...
TEST(TestMockTimeTaskRunnerTest, RunLoopQuitFromIdle)
{
    ...
    RunLoop run_loop;
    quitting_thread.task_runner()->PostDelayedTask(FROM_HERE, run_loop.QuitWhenIdleClosure(), TestTimeouts::tiny_timeout());
    run_loop.Run();
}
...
TEST(TestMockTimeTaskRunnerTest, AdvanceMockTickClockDoesNotRunTasks)
{
    ...
    task_runner->PostDelayedTask(FROM_HERE, BindOnce([] { ADD_FAILURE(); }), Seconds(1));
...
{
    auto mock_time_task_runner = MakeRefCounted<TestMockTimeTaskRunner>(TestMockTimeTaskRunner::Type::kStandalone);
    int counter = 0;

    mock_time_task_runner->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
    mock_time_task_runner->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));

    CancelableOnceClosure task1(base::BindOnce([](int* counter) { *counter += 16; }, Unretained(&counter)));
