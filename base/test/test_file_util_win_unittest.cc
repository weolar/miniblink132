...
public:
    ...
    {
        win::ScopedHandle handle(::CreateFile(long_path_.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, nullptr));
...
TEST(TestFileUtil, EvictFileWithLongName)
{
    ...
    FilePath subdir = temp_dir.GetPath().Append(FilePath(std::wstring(100, L'a')));
