...
TEST(GmockMoveSupportTest, MoveArgsTwiceByRef)
{
    ...
    MockFoo foo;
    EXPECT_CALL(foo, TwiceByRef).WillOnce(DoAll(MoveArg<0>(&result1), MoveArg<1>(&result2)));
...
TEST(GmockMoveSupportTest, MoveArgAndReturn)
{
    ...
    MockFoo foo;
    EXPECT_CALL(foo, ByValWithReturnValue).WillOnce(MoveArgAndReturn(&result, true));
