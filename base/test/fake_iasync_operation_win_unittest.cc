...
template <typename T> class FakeIAsyncOperationTest : public ::testing::Test { };
...
TYPED_TEST_P(FakeIAsyncOperationTest, MultipleCompletedHandlers)
{
    ...
    auto handler = Callback<IAsyncOperationCompletedHandler<TypeParam>>([](auto async_operation, AsyncStatus async_status) { return S_OK; });
    ASSERT_HRESULT_SUCCEEDED(operation->put_Completed(handler.Get()));
    EXPECT_NONFATAL_FAILURE(ASSERT_HRESULT_SUCCEEDED(operation->put_Completed(handler.Get()));, "put_Completed");
}
...
TYPED_TEST_P(FakeIAsyncOperationTest, MultipleCompletions)
{
    ...
    ASSERT_NO_FATAL_FAILURE(operation->CompleteWithResults(test_values.GetTestValue_T()));
    ...
    EXPECT_FATAL_FAILURE(static_operation->CompleteWithResults(test_value_t), "already completed");
    ...
    EXPECT_FATAL_FAILURE(static_operation->CompleteWithError(E_FAIL), "already completed");
...
TYPED_TEST_P(FakeIAsyncOperationTest, CompleteWithResults_WithHandler)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(operation->put_Completed(
        Callback<IAsyncOperationCompletedHandler<TypeParam>>([operation, &completed_handler_called](auto async_operation, AsyncStatus async_status) {
    ...
    EXPECT_NONFATAL_FAILURE(ASSERT_HRESULT_FAILED(operation->GetResults(&results)), "GetResults");
...
REGISTER_TYPED_TEST_SUITE_P(FakeIAsyncOperationTest, MultipleCompletedHandlers, MultipleCompletions, CompleteWithResults_WithHandler,
    CompleteWithResults_WithoutHandler, CompleteWithError_WithHandler, CompleteWithError_WithoutHandler);

INSTANTIATE_TYPED_TEST_SUITE_P(Win, FakeIAsyncOperationTest, base::test::AsyncResultsTestValuesTypes);
