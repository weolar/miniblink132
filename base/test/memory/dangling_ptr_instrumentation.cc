...
// static
base::expected<DanglingPtrInstrumentation, std::string_view> DanglingPtrInstrumentation::Create()
{
    if (!FeatureList::IsEnabled(features::kPartitionAllocBackupRefPtr)) {
        return base::unexpected("DanglingPtrInstrumentation requires the feature flag "
    ...
    return base::unexpected("DanglingPtrInstrumentation requires the binary flag "
...
DanglingPtrInstrumentation::DanglingPtrInstrumentation(DanglingPtrInstrumentation&& old)
{
    operator=(std::move(old));
}

DanglingPtrInstrumentation& DanglingPtrInstrumentation::operator=(DanglingPtrInstrumentation&& old)
{
    old.Unregister();
    Register();
    return *this;
...
raw_ptr<DanglingPtrInstrumentation> DanglingPtrInstrumentation::g_observer = nullptr;
