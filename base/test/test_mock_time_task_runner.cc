...
//    task runner.
class TestMockTimeTaskRunner::NonOwningProxyTaskRunner : public SingleThreadTaskRunner {
public:
    ...
    bool PostNonNestableDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay) override
    {
        AutoLock scoped_lock(lock_);
        if (target_) {
            return target_->PostNonNestableDelayedTask(from_here, std::move(task), delay);
        }
...
struct TestMockTimeTaskRunner::TestOrderedPendingTask : public base::TestPendingTask {
    TestOrderedPendingTask();
    TestOrderedPendingTask(const Location& location, OnceClosure task, TimeTicks post_time, TimeDelta delay, size_t ordinal, TestNestability nestability);
...
TestMockTimeTaskRunner::TestOrderedPendingTask::TestOrderedPendingTask(TestOrderedPendingTask&&) = default;

TestMockTimeTaskRunner::TestOrderedPendingTask::TestOrderedPendingTask(
    const Location& location, OnceClosure task, TimeTicks post_time, TimeDelta delay, size_t ordinal, TestNestability nestability)
    : base::TestPendingTask(location, std::move(task), post_time, delay, nestability)
    , ordinal(ordinal)
{
}

TestMockTimeTaskRunner::TestOrderedPendingTask::~TestOrderedPendingTask() = default;

TestMockTimeTaskRunner::TestOrderedPendingTask& TestMockTimeTaskRunner::TestOrderedPendingTask::operator=(TestOrderedPendingTask&&) = default;
...
TestMockTimeTaskRunner::ScopedContext::ScopedContext(scoped_refptr<TestMockTimeTaskRunner> scope)
...
bool TestMockTimeTaskRunner::TemporalOrder::operator()(const TestOrderedPendingTask& first_task, const TestOrderedPendingTask& second_task) const
{
...
TestMockTimeTaskRunner::TestMockTimeTaskRunner(Time start_time, TimeTicks start_ticks, Type type)
    : now_(start_time)
    ...
    , mock_clock_(this)
{
    if (type == Type::kBoundToThread) {
        ...
        thread_task_runner_handle_ = std::make_unique<SingleThreadTaskRunner::CurrentDefaultHandle>(proxy_task_runner_);
    }
}
...
base::circular_deque<TestPendingTask> TestMockTimeTaskRunner::TakePendingTasks()
{
    AutoLock scoped_lock(tasks_lock_);
    ...
    while (!tasks_.empty()) {
        ...
        if (!tasks_.top().task.IsCancelled()) {
            tasks.push_back(std::move(const_cast<TestOrderedPendingTask&>(tasks_.top())));
        }
        tasks_.pop();
    }
    return tasks;
...
size_t TestMockTimeTaskRunner::GetPendingTaskCount()
{
    ...
    while (!tasks_.empty()) {
        if (!tasks_.top().task.IsCancelled()) {
            preserved_tasks.push(std::move(const_cast<TestOrderedPendingTask&>(tasks_.top())));
        }
        tasks_.pop();
    }
...
TimeDelta TestMockTimeTaskRunner::NextPendingTaskDelay()
{
    ...
    while (!tasks_.empty() && tasks_.top().task.IsCancelled())
        tasks_.pop();
    return tasks_.empty() ? TimeDelta::Max() : tasks_.top().GetTimeToRun() - now_ticks_;
}
...
bool TestMockTimeTaskRunner::PostDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay)
{
    AutoLock scoped_lock(tasks_lock_);
    tasks_.push(TestOrderedPendingTask(from_here, std::move(task), now_ticks_, delay, next_task_ordinal_++, TestPendingTask::NESTABLE));
    tasks_lock_cv_.Signal();
    return true;
}

bool TestMockTimeTaskRunner::PostDelayedTaskAt(
    subtle::PostDelayedTaskPassKey, const Location& from_here, OnceClosure task, TimeTicks delayed_run_time, subtle::DelayPolicy deadline_policy)
{
    return PostDelayedTask(from_here, std::move(task), delayed_run_time.is_null() ? TimeDelta() : delayed_run_time - now_ticks_);
}

bool TestMockTimeTaskRunner::PostNonNestableDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay)
{
...
void TestMockTimeTaskRunner::ProcessTasksNoLaterThan(TimeDelta max_delta, int limit)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    std::optional<SingleThreadTaskRunner::CurrentHandleOverrideForTesting> ttrh_override;
    if (!SingleThreadTaskRunner::HasCurrentDefault() || SingleThreadTaskRunner::GetCurrentDefault() != proxy_task_runner_.get()) {
...
bool TestMockTimeTaskRunner::DequeueNextTask(const TimeTicks& reference, const TimeDelta& max_delta, TestPendingTask* next_task)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    AutoLock scoped_lock(tasks_lock_);
...
void TestMockTimeTaskRunner::Run(bool application_tasks_allowed, TimeDelta timeout)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    DCHECK(application_tasks_allowed) << "This is a nested RunLoop instance and needs to be of "
    ...
    while (!quit_run_loop_ && now_ticks_ < run_until) {
        ...
        TimeDelta auto_fast_forward_by;
        {
            ...
            auto_fast_forward_by = std::min(run_until, tasks_.top().GetTimeToRun()) - now_ticks_;
        }
