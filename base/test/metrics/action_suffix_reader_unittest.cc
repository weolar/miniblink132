...
// base_unittests.
extern std::vector<ActionSuffixEntryMap> ReadActionSuffixesForActionForTesting(const std::string& xml_string, const std::string& affected_action);

TEST(ActionSuffixReaderTest, NoSuffixesFound)
{
    const auto results = ReadActionSuffixesForActionForTesting(kTestActionXml, "Action.DoesNotExist");
    EXPECT_TRUE(results.empty());
}

TEST(ActionSuffixReaderTest, OneResult)
{
    const auto results = ReadActionSuffixesForActionForTesting(kTestActionXml, "Action.WithSuffix3");
...
{
    const auto results = ReadActionSuffixesForActionForTesting(kTestActionXml, "Action.WithSuffix2");
    EXPECT_EQ(1U, results.size());
...
{
    const auto results = ReadActionSuffixesForActionForTesting(kTestActionXml, "Action.WithSuffix1");
