...
TEST_F(HistogramTesterTest, Scope)
{
    ...
    std::unique_ptr<HistogramSamples> samples(tester.GetHistogramSamplesSinceCreation(kHistogram1));
    EXPECT_TRUE(samples);
...
TEST_F(HistogramTesterTest, TestUniqueSample_TooManySamplesInActualBucket)
{
    ...
    EXPECT_NONFATAL_FAILURE(failing_code(), "Histogram \"Test2\" did not meet its expectations.");
}
...
TEST_F(HistogramTesterTest, TestGetAllSamples)
{
    ...
    EXPECT_THAT(tester.GetAllSamples(kHistogram5), ElementsAre(Bucket(2, 1), Bucket(3, 2), Bucket(5, 1)));
}
...
TEST_F(HistogramTesterTest, TestGetAllChangedHistograms)
{
    ...
    EXPECT_NE(std::string::npos, results.find("Histogram: Test4 recorded 1 new samples"));
    EXPECT_NE(std::string::npos, results.find("Histogram: Test5 recorded 4 new samples"));
    EXPECT_NE(std::string::npos, results.find("Histogram: Test1.Test2.Test3 recorded 1 new samples"));
}
...
TEST_F(HistogramTesterTest, MissingHistogramMeansEmptyBuckets)
{
    ...
    EXPECT_EQ(0, tester.GetHistogramSamplesSinceCreation(kHistogram)->TotalCount());
}
...
TEST_F(HistogramTesterTest, BucketsAre)
{
    ...
    auto b = [](base::Histogram::Sample min, base::Histogram::Count count) { return Bucket(min, count); };
    using ::testing::Not;
    ...
    EXPECT_THAT(a({ b(1, 1), b(2, 2) }), Not(BucketsAre(b(0, 0), b(1, 0), b(2, 0))));
}
...
TEST_F(HistogramTesterTest, BucketsInclude)
{
    ...
    EXPECT_THAT(a({ b(1, 1), b(2, 2) }), Not(BucketsInclude(b(0, 0), b(1, 0), b(2, 0))));
}
