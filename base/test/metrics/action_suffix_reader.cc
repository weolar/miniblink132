...
std::optional<ActionSuffixEntryMap> ParseActionSuffixesFromActionsXml(const std::string& affected_action, XmlReader& reader)
{
    ...
    while (true) {
        ...
        if (node_name == "affected-action" && reader.NodeAttribute("name", &name) && name == affected_action) {
            action_found = true;
        }
        ...
        if (node_name == "suffix") {
            ...
            if (!has_name) {
                failures.emplace_back(StringPrintf("Bad suffix entry with label \"%s\"; no 'name' attribute.", label.c_str()));
            }

            if (!has_label) {
                failures.emplace_back(StringPrintf("Bad suffix entry with name \"%s\"; no 'label' attribute.", name.c_str()));
            }
            ...
            if (has_name) {
                ...
                if (!insert_result.second) {
                    failures.emplace_back(StringPrintf("Duplicate suffix name \"%s\"", name.c_str()));
                }
            }
        }
...
std::vector<ActionSuffixEntryMap> ReadActionSuffixesForActionImpl(XmlReader& reader, const std::string& affected_action)
{
    ...
    while (true) {
        ...
        if (node_name == "action-suffix") {
            ...
            if (reader.Read()) {
                auto suffixes = ParseActionSuffixesFromActionsXml(affected_action, reader);
                if (suffixes) {
...
std::vector<ActionSuffixEntryMap> ReadActionSuffixesForActionForTesting(const std::string& xml_string, const std::string& affected_action)
{
    XmlReader reader;
...
std::vector<ActionSuffixEntryMap> ReadActionSuffixesForAction(const std::string& affected_action)
{
    FilePath src_root;
    ...
    const FilePath path = src_root.AppendASCII("tools").AppendASCII("metrics").AppendASCII("actions").AppendASCII("actions.xml");
