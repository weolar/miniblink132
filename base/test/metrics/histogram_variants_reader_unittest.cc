...
TEST(HistogramVariantsReaderTest, SanityChecks)
{
    ...
    std::optional<HistogramVariantsEntryMap> results = ReadVariantsFromHistogramsXml("TestToken", "test_data",
        /*from_metadata=*/false);
    ASSERT_TRUE(results);
    EXPECT_THAT(*results, testing::UnorderedElementsAre(std::make_pair("Variant1", "Label1"), std::make_pair("Variant2", "Label2")));

    results = ReadVariantsFromHistogramsXml("TheWorstNameForVariants", "test_data", /*from_metadata=*/false);
