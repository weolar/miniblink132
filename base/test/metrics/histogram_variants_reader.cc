...
std::optional<HistogramVariantsEntryMap> ParseVariantsFromHistogramsXml(const std::string& variants_name, XmlReader& reader)
{
    ...
    while (true) {
        ...
        if (node_name == "variant") {
            ...
            if (!has_name) {
                ADD_FAILURE() << "Bad " << variants_name << " variant entry, summary='" << summary << "'): No 'name' attribute.";
                success = false;
            }

            if (!has_summary) {
                ADD_FAILURE() << "Bad " << variants_name << " variant entry, name='" << name << "'): No 'summary' attribute.";
                success = false;
            }
            ...
            if (has_name) {
                ...
                if (!insert_result.second) {
                    ADD_FAILURE() << "Duplicate entry in " << variants_name << " variant entry, name='" << name << ')';
                    success = false;
                }
            }
        }
...
std::optional<HistogramVariantsEntryMap> ReadVariantsFromHistogramsXml(const std::string& variants_name, const std::string& subdirectory, bool from_metadata)
{
    ...
    base::FilePath path = src_root.AppendASCII("tools").AppendASCII("metrics").AppendASCII("histograms");
    if (from_metadata) {
    ...
    while (true) {
        ...
        if (node_name == "variants") {
            ...
            if (reader.NodeAttribute("name", &name) && name == variants_name) {
                if (result.has_value()) {
                    ADD_FAILURE() << "Duplicate variant '" << variants_name << "' found in " << path;
                    return std::nullopt;
                }

                const bool got_into_variant = reader.Read();
                if (!got_into_variant) {
                    ADD_FAILURE() << "Bad variant '" << variants_name << "' (looks empty) found in " << path;
                    return std::nullopt;
                }

                result = ParseVariantsFromHistogramsXml(variants_name, reader);
                if (!result.has_value()) {
                    ADD_FAILURE() << "Bad variant '" << variants_name << "' found in " << path << " (format error).";
                    return std::nullopt;
                }
            }
        }
