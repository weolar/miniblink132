...
public:
    void SetUp() override
    {
        if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestLauncherOutput)) {
...
TEST_F(XmlUnitTestResultPrinterTest, LinkInXmlFile)
{
    ...
    std::string file_path = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTestLauncherOutput);
    std::string content;
    ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
    std::string expected_content = base::StrCat({ "<link name=\"LinkInXmlFile\" "
                                                  "classname=\"XmlUnitTestResultPrinterTest\" "
                                                  "link_name=\"unique_link\">",
        "http://google.com", "</link>" });
    EXPECT_TRUE(content.find(expected_content) != std::string::npos) << expected_content << " not found in " << content;
}

TEST_F(XmlUnitTestResultPrinterTest, EscapedLinkInXmlFile)
{
    XmlUnitTestResultPrinter::Get()->AddLink("unique_link", "http://google.com/path?id=\"'<>&\"");
    std::string file_path = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTestLauncherOutput);
    std::string content;
    ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
    std::string expected_content = base::StrCat({ "<link name=\"EscapedLinkInXmlFile\" "
                                                  "classname=\"XmlUnitTestResultPrinterTest\" "
                                                  "link_name=\"unique_link\">",
...
TEST_F(XmlUnitTestResultPrinterTest, TagInXmlFile)
{
    ...
    std::string content;
    ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
    std::string expected_content = base::StrCat({ "<tag name=\"TagInXmlFile\" "
                                                  "classname=\"XmlUnitTestResultPrinterTest\" "
                                                  "tag_name=\"tag_name\">",
        "tag_value", "</tag>" });
    EXPECT_TRUE(content.find(expected_content) != std::string::npos) << expected_content << " not found in " << content;
...
TEST_F(XmlUnitTestResultPrinterTest, MultiTagsInXmlFile)
{
    ...
    std::string content;
    ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
    std::string expected_content_1 = "<tag name=\"MultiTagsInXmlFile\" "
                                     "classname=\"XmlUnitTestResultPrinterTest\" "
                                     "tag_name=\"tag_name1\">tag_value1</tag>";
    EXPECT_TRUE(content.find(expected_content_1) != std::string::npos) << expected_content_1 << " not found in " << content;

    std::string expected_content_2 = "<tag name=\"MultiTagsInXmlFile\" "
                                     "classname=\"XmlUnitTestResultPrinterTest\" "
                                     "tag_name=\"tag_name2\">tag_value2</tag>";
    EXPECT_TRUE(content.find(expected_content_2) != std::string::npos) << expected_content_2 << " not found in " << content;
}
...
TEST_F(XmlUnitTestResultPrinterTest, MultiTagsWithSameNameInXmlFile)
{
    ...
    std::string content;
    ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
    std::string expected_content_1 = "<tag name=\"MultiTagsWithSameNameInXmlFile\" "
                                     "classname=\"XmlUnitTestResultPrinterTest\" "
                                     "tag_name=\"tag_name\">tag_value1</tag>";
    EXPECT_TRUE(content.find(expected_content_1) != std::string::npos) << expected_content_1 << " not found in " << content;

    std::string expected_content_2 = "<tag name=\"MultiTagsWithSameNameInXmlFile\" "
                                     "classname=\"XmlUnitTestResultPrinterTest\" "
...
public:
    static void TearDownTestSuite()
    {
        ...
        std::string file_path = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTestLauncherOutput);
        if (file_path.empty()) {
            GTEST_SKIP() << "Test has to run with --" << switches::kTestLauncherOutput << " switch.";
        }
        std::string content;
        ASSERT_TRUE(base::ReadFileToString(FilePath::FromUTF8Unsafe(file_path), &content));
