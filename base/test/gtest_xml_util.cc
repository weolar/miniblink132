...
bool ProcessGTestOutput(const base::FilePath& output_file, std::vector<TestResult>* results, bool* crashed)
{
    DCHECK(results);
    ...
    while (xml_reader.Read()) {
        ...
        case STATE_TESTCASE:
            if (node_name == "testsuite" && xml_reader.IsClosingElement()) {
                ...
                Time test_timestamp;
                if (xml_reader.NodeAttribute("timestamp", &test_timestamp_str) && Time::FromString(test_timestamp_str.c_str(), &test_timestamp)) {
                ...
                if (!xml_reader.NodeAttribute("time", &test_time_str))
                    return false;
                result.elapsed_time = Microseconds(static_cast<int64_t>(strtod(test_time_str.c_str(), nullptr) * Time::kMicrosecondsPerSecond));
                ...
                if (!results->empty() && results->back().full_name == result.full_name && results->back().status == TestResult::TEST_CRASH) {
                ...
                // Deliberately empty.
            } else if (node_name == "x-test-result-part" && !xml_reader.IsClosingElement()) {
                ...
                test_result_part.file_name = file_name, test_result_part.line_number = line_number;
                DCHECK(!results->empty());
