...
#if DCHECK_IS_ON() && defined(GTEST_HAS_DEATH_TEST) && !BUILDFLAG(DCHECK_IS_CONFIGURABLE) && !BUILDFLAG(IS_ANDROID)
...
#define EXPECT_DCHECK_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "Check failed", )
#define EXPECT_DCHECK_DEATH_WITH(statement, msg) GTEST_UNSUPPORTED_DEATH_TEST(statement, msg, )
#define ASSERT_DCHECK_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "Check failed", return)
#define ASSERT_DCHECK_DEATH_WITH(statement, msg) GTEST_UNSUPPORTED_DEATH_TEST(statement, msg, return)
...
#define EXPECT_NOTREACHED_DEATH(statement) EXPECT_DEATH(statement, "NOTREACHED hit")
#define ASSERT_NOTREACHED_DEATH(statement) ASSERT_DEATH(statement, "NOTREACHED hit")
#else
...
#define EXPECT_CHECK_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", )
#define EXPECT_CHECK_DEATH_WITH(statement, msg) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", )
#define ASSERT_CHECK_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", return)
#define EXPECT_NOTREACHED_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", )
#define ASSERT_NOTREACHED_DEATH(statement) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", return)
...
#define BASE_EXPECT_DEATH(statement, matcher) GTEST_UNSUPPORTED_DEATH_TEST(statement, "", )
...
std::string FormatFullTestName(const std::string& test_case_name, const std::string& test_name);
...
[[nodiscard]] bool ReadTestNamesFromFile(const FilePath& path, std::vector<TestIdentifier>* output);
