...
void VerifyRunUntilIdleDidNotReturnAndSetFlag(AtomicFlag* run_until_idle_returned, AtomicFlag* task_ran)
{
    EXPECT_FALSE(run_until_idle_returned->IsSet());
    task_ran->Set();
}

void RunUntilIdleTest(TaskEnvironment::ThreadPoolExecutionMode thread_pool_execution_mode)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce(&VerifyRunUntilIdleDidNotReturnAndSetFlag, Unretained(&run_until_idle_returned), Unretained(&first_main_thread_task_ran)));

    AtomicFlag first_thread_pool_task_ran;
    ThreadPool::PostTask(
        FROM_HERE, BindOnce(&VerifyRunUntilIdleDidNotReturnAndSetFlag, Unretained(&run_until_idle_returned), Unretained(&first_thread_pool_task_ran)));

    AtomicFlag second_thread_pool_task_ran;
    AtomicFlag second_main_thread_task_ran;
    ThreadPool::PostTaskAndReply(FROM_HERE,
        BindOnce(&VerifyRunUntilIdleDidNotReturnAndSetFlag, Unretained(&run_until_idle_returned), Unretained(&second_thread_pool_task_ran)),
        BindOnce(&VerifyRunUntilIdleDidNotReturnAndSetFlag, Unretained(&run_until_idle_returned), Unretained(&second_main_thread_task_ran)));
...
{
    TaskEnvironment task_environment(TaskEnvironment::ThreadPoolExecutionMode::QUEUED);

    AtomicFlag run_until_idle_called;
    ThreadPool::PostTask(
        FROM_HERE, BindOnce([](AtomicFlag* run_until_idle_called) { EXPECT_TRUE(run_until_idle_called->IsSet()); }, Unretained(&run_until_idle_called)));
    ...
    ThreadPool::PostTask(FROM_HERE,
        BindOnce([](AtomicFlag* other_run_until_idle_called) { EXPECT_TRUE(other_run_until_idle_called->IsSet()); }, Unretained(&other_run_until_idle_called)));
    PlatformThread::Sleep(TestTimeouts::tiny_timeout());
...
{
    TaskEnvironment task_environment(TaskEnvironment::ThreadPoolExecutionMode::ASYNC);

    WaitableEvent task_ran;
    ThreadPool::PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&task_ran)));
    task_ran.Wait();
}
...
void DelayedTasksTest(TaskEnvironment::TimeSource time_source)
{
    ...
    TaskEnvironment task_environment(time_source, TaskEnvironment::ThreadPoolExecutionMode::QUEUED);
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 4); }, Unretained(&counter)), kShortTaskDelay);
    ThreadPool::PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 128); }, Unretained(&counter)), kShortTaskDelay);

    constexpr base::TimeDelta kLongTaskDelay = Days(7);
    // Same as first task, longer delays to exercise
    // FastForwardUntilNoTasksRemain().
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 8); }, Unretained(&counter)), Days(5));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 16); }, Unretained(&counter)), kLongTaskDelay);
    ThreadPool::PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 256); }, Unretained(&counter)), kLongTaskDelay * 2);
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 512); }, Unretained(&counter)), kLongTaskDelay * 3);
    ThreadPool::PostDelayedTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 1024); }, Unretained(&counter)), kLongTaskDelay * 4);

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 1); }, Unretained(&counter)));
    ThreadPool::PostTask(FROM_HERE, BindOnce([](subtle::Atomic32* counter) { subtle::NoBarrier_AtomicIncrement(counter, 2); }, Unretained(&counter)));
    ...
    if (time_source == TaskEnvironment::TimeSource::MOCK_TIME) {
        ...
        EXPECT_EQ(task_environment.NowLiveTicks() - live_start_time, kInferiorTaskDelay);
        ...
        EXPECT_EQ(task_environment.NowLiveTicks() - live_start_time, kShortTaskDelay);
        ...
        EXPECT_EQ(task_environment.NowLiveTicks() - live_start_time, kLongTaskDelay * 4);
    }
}
...
void SupportsSequenceLocalStorageOnMainThreadTest(TaskEnvironment::TimeSource time_source)
{
    TaskEnvironment task_environment(time_source, TaskEnvironment::ThreadPoolExecutionMode::ASYNC);
...
{
    SupportsSequenceLocalStorageOnMainThreadTest(TaskEnvironment::TimeSource::SYSTEM_TIME);
}

TEST_F(TaskEnvironmentTest, SupportsSequenceLocalStorageOnMainThreadWithMockTime)
{
    SupportsSequenceLocalStorageOnMainThreadTest(TaskEnvironment::TimeSource::MOCK_TIME);
}
...
TEST_F(TaskEnvironmentTest, SupportsFileDescriptorWatcherOnIOMainThread)
{
    ...
    auto controller = FileDescriptorWatcher::WatchWritable(pipe_fds_[1], run_loop.QuitClosure());

    // This will hang if the notification doesn't occur as expected.
    run_loop.Run();
}

TEST_F(TaskEnvironmentTest, SupportsFileDescriptorWatcherOnIOMockTimeMainThread)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&] {
        int64_t x = 1;
    ...
    auto controller = FileDescriptorWatcher::WatchReadable(pipe_fds_[0], run_loop.QuitClosure());
...
TEST_F(TaskEnvironmentTest, MockTimeStartsWithWholeMilliseconds)
{
    ...
    EXPECT_TRUE((mock_tick_clock->NowTicks().since_origin() % Milliseconds(1)).is_zero());
...
TEST_F(TaskEnvironmentTest, FastForwardAdvancesTickClock)
{
    // Use a QUEUED execution-mode environment, so that no tasks are actually
    // executed until RunUntilIdle()/FastForwardBy() are invoked.
    TaskEnvironment task_environment(TaskEnvironment::TimeSource::MOCK_TIME, TaskEnvironment::ThreadPoolExecutionMode::QUEUED);

    constexpr base::TimeDelta kShortTaskDelay = Days(1);
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, base::DoNothing(), kShortTaskDelay);

    constexpr base::TimeDelta kLongTaskDelay = Days(7);
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, base::DoNothing(), kLongTaskDelay);
...
TEST_F(TaskEnvironmentTest, AdvanceClockDoesNotRunTasks)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, base::DoNothing(), kTaskDelay);
...
TEST_F(TaskEnvironmentTest, AdvanceClockSchedulesRipeDelayedTasks)
{
    ...
    constexpr base::TimeDelta kTaskDelay = Days(1);
    ThreadPool::PostDelayedTask(FROM_HERE, base::BindLambdaForTesting([&] { ran = true; }), kTaskDelay);
...
TEST_F(TaskEnvironmentTest, FastForwardOnlyAdvancesWhenIdle)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { EXPECT_EQ(start_time, base::TimeTicks::Now()); }));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindLambdaForTesting([&] { EXPECT_EQ(start_time + kDelay, base::TimeTicks::Now()); }), kDelay);
...
TEST_F(TaskEnvironmentTest, SuspendedFastForwardOnlyAdvancesWhenIdle)
{
    ...
    constexpr base::TimeDelta kDelay = Seconds(42);
    constexpr base::TimeDelta kFastForwardUntil = Seconds(100);
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
...
TEST_F(TaskEnvironmentTest, FastForwardZero)
{
    ...
    for (int i = 0; i < 1000; ++i) {
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { run_count.fetch_add(1, std::memory_order_relaxed); }));
        ThreadPool::PostTask(FROM_HERE, BindLambdaForTesting([&] { run_count.fetch_add(1, std::memory_order_relaxed); }));
    }
...
TEST_F(TaskEnvironmentTest, NestedFastForwardBy)
{
    ...
    post_fast_forwarding_task = BindLambdaForTesting([&] {
        if (max_nesting_level < 5) {
            ++max_nesting_level;
            SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_fast_forwarding_task, kDelayPerTask);
    ...
    EXPECT_EQ(task_environment.NowLiveTicks(), live_start_time + kDelayPerTask * 5);
}
...
TEST_F(TaskEnvironmentTest, NestedRunInFastForwardBy)
{
    ...
    post_and_runloop_task = BindLambdaForTesting([&] {
        ...
        if (run_loops.size() < 4U) {
            SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_and_runloop_task, kDelayPerTask);
...
TEST_F(TaskEnvironmentTest, CrossThreadImmediateTaskPostingDoesntAffectMockTime)
{
    TaskEnvironment task_environment(TaskEnvironment::TimeSource::MOCK_TIME);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
            BindOnce(
    ...
    for (int i = 0; i < 1000; ++i) {
        ThreadPool::PostTaskAndReply(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&first_reply_is_incoming)), DoNothing());
    }
...
TEST_F(TaskEnvironmentTest, MultiThreadedMockTime)
{
    ...
    post_main_thread_delayed_task = BindLambdaForTesting([&] {
        ...
        // the end!).
        if (last_main_thread_ticks < task_environment.NowTicks() && task_environment.NowTicks() < end_time) {
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
        }
    ...
    post_thread_pool_delayed_task = BindLambdaForTesting([&] {
        ...
        // first task at its runtime (otherwise we end up with 4^10'000 tasks by
        // the end!).
        if (last_thread_pool_ticks < task_environment.NowTicks() && task_environment.NowTicks() < end_time) {
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
    ThreadPool::CreateSequencedTaskRunner({})->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
...
TEST_F(TaskEnvironmentTest, MultiThreadedFastForwardBy)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Seconds(5));
    ThreadPool::PostDelayedTask(FROM_HERE, {}, MakeExpectedRunClosure(FROM_HERE), Seconds(1));
...
TEST_F(TaskEnvironmentTest, MultiThreadedMockTimeAndThreadPoolQueuedMode)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, {}, BindLambdaForTesting([&] { count += 256; }), Seconds(5));

    // Time should auto-advance to +500s in RunLoop::Run() without having to run
    // the above forcefully QUEUED tasks.
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { count += 1; }));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&] {
        count += 2;
        run_loop.Quit();
    }),
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, BindLambdaForTesting([&] { count += 512; }), Seconds(5));
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { count += 2048; }));
...
{
    TaskEnvironment task_environment;
    ThreadPool::PostTask(FROM_HERE, BindOnce(&win::AssertComApartmentType, win::ComApartmentType::MTA));
    task_environment.RunUntilIdle();
}
...
{
    const RunLoop::RunLoopTimeout* old_run_timeout = ScopedRunLoopTimeout::GetTimeoutForCurrentThread();

    {
        ...
        const RunLoop::RunLoopTimeout* run_timeout = ScopedRunLoopTimeout::GetTimeoutForCurrentThread();
        ...
        EXPECT_NONFATAL_FAILURE(static_on_timeout_cb.Run(FROM_HERE),
    ...
    EXPECT_EQ(ScopedRunLoopTimeout::GetTimeoutForCurrentThread(), old_run_timeout);
}
...
TEST_F(TaskEnvironmentTest, DescribeCurrentTasksHasPendingMainThreadTasks)
{
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOGGING_INFO, _, _, _, HasSubstr("ThreadPool currently running tasks"))).WillOnce(Return(true));
    // The pending task posted above to the main thread is logged.
    EXPECT_CALL(mock_log, Log(::logging::LOGGING_INFO, _, _, _, HasSubstr("task_environment_unittest.cc"))).WillOnce(Return(true));
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOGGING_INFO, _, _, _, HasSubstr("\"immediate_work_queue_size\":0"))).WillOnce(Return(true));
    task_environment.DescribeCurrentTasks();
}
...
TEST_F(TaskEnvironmentTest, DescribeCurrentTasksHasThreadPoolTasks)
{
    ...
    scoped_refptr<SequencedTaskRunner> thread_pool_task_runner
        = base::ThreadPool::CreateSequencedTaskRunner({ WithBaseSyncPrimitives(), TaskShutdownBehavior::SKIP_ON_SHUTDOWN });
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOGGING_INFO, _, _, _, Not(HasSubstr("task_environment_unittest.cc")))).WillOnce(Return(true));
...
TEST_F(TaskEnvironmentTest, Basic)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 256; }, Unretained(&counter)), Seconds(3));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 64; }, Unretained(&counter)), Seconds(1));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, BindOnce([](int* counter) { *counter += 1024; }, Unretained(&counter)), Minutes(20));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 4096; }, Unretained(&counter)), Days(20));
...
TEST_F(TaskEnvironmentTest, RunLoopDriveable)
{
    ...
    int counter = 0;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce([](int* counter) { *counter += 256; }, Unretained(&counter)), Seconds(3));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce([](int* counter) { *counter += 64; }, Unretained(&counter)), Seconds(1));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce([](int* counter) { *counter += 1024; }, Unretained(&counter)), Minutes(20));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce([](int* counter) { *counter += 4096; }, Unretained(&counter)), Days(20));
    ...
    {
        RunLoop run_loop;
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), Seconds(1));
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce([](int* counter) { *counter += 8192; }, Unretained(&counter)), Seconds(1));
    ...
    {
        RunLoop run_loop;
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitWhenIdleClosure(), Seconds(5));
        SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce([](int* counter) { *counter += 16384; }, Unretained(&counter)), Seconds(5));
    ...
    RunLoop run_loop;
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitWhenIdleClosure(), Days(50));
...
TEST_F(TaskEnvironmentTest, RunLoopGetsTurnAfterYieldingToPool)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Seconds(1));
...
TEST_F(TaskEnvironmentTest, ThreadPoolAdvancesTimeUnderIdleMainThread)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Seconds(2));
...
TEST_F(TaskEnvironmentTest, MainThreadCanContributeWhileFlushingPool)
{
    ...
    ThreadPool::PostTask(FROM_HERE, BindLambdaForTesting([&] {
        task_environment.GetMainThreadTaskRunner()->PostTask(FROM_HERE, BindOnce(&TestWaitableEvent::Signal, Unretained(&wait_for_collaboration)));
...
TEST_F(TaskEnvironmentTest, CancelPendingTask)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, task1.callback(), Seconds(1));
    ...
    task1.Cancel();
    EXPECT_TRUE(task_environment.MainThreadIsIdle());
    EXPECT_EQ(TimeDelta::Max(), task_environment.NextMainThreadPendingTaskDelay());

    CancelableRepeatingClosure task2(BindRepeating([] {}));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, task2.callback(), Seconds(1));
    task2.Cancel();
    EXPECT_EQ(0u, task_environment.GetPendingMainThreadTaskCount());

    CancelableRepeatingClosure task3(BindRepeating([] {}));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, task3.callback(), Seconds(1));
    task3.Cancel();
    EXPECT_EQ(TimeDelta::Max(), task_environment.NextMainThreadPendingTaskDelay());

    CancelableRepeatingClosure task4(BindRepeating([] {}));
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, task4.callback(), Seconds(1));
    task4.Cancel();
    EXPECT_TRUE(task_environment.MainThreadIsIdle());
}
...
TEST_F(TaskEnvironmentTest, CancelPendingImmediateTask)
{
    ...
    CancelableOnceClosure task1(BindOnce([] {}));
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, task1.callback());
...
TEST_F(TaskEnvironmentTest, NoFastForwardToCancelledTask)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, task.callback(), Seconds(1));
    EXPECT_EQ(Seconds(1), task_environment.NextMainThreadPendingTaskDelay());
    task.Cancel();
    task_environment.FastForwardUntilNoTasksRemain();
...
TEST_F(TaskEnvironmentTest, NextTaskIsDelayed)
{
    ...
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, BindOnce([] {}), Seconds(2));
    EXPECT_TRUE(task_environment.NextTaskIsDelayed());
    task_environment.FastForwardUntilNoTasksRemain();
    EXPECT_FALSE(task_environment.NextTaskIsDelayed());

    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce([] {}));
    EXPECT_FALSE(task_environment.NextTaskIsDelayed());
}
...
TEST_F(TaskEnvironmentTest, SingleThread)
{
    ...
    bool ran = false;
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { ran = true; }));
    RunLoop().RunUntilIdle();
    EXPECT_TRUE(ran);
...
TEST_F(TaskEnvironmentTest, SingleThreadMockTime)
{
    ...
    int counter = 0;
    SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, base::BindLambdaForTesting([&] { counter += 1; }), kDelay);
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { counter += 2; }));
...
{
    base::win::ScopedCOMInitializer initializer(base::win::ScopedCOMInitializer::kMTA);
    EXPECT_TRUE(initializer.Succeeded());
}

void InitializeCOMOnWorker(TaskEnvironment::ThreadPoolCOMEnvironment com_environment, ApartmentType apartment_type)
{
    TaskEnvironment task_environment(com_environment);
    ThreadPool::PostTask(FROM_HERE, BindOnce(apartment_type == ApartmentType::kSTA ? &InitializeSTAApartment : &InitializeMTAApartment));
    task_environment.RunUntilIdle();
}
...
TEST_F(TaskEnvironmentTest, DefaultCOMEnvironment)
{
    ...
    InitializeCOMOnWorker(TaskEnvironment::ThreadPoolCOMEnvironment::DEFAULT, ApartmentType::kMTA);

    // Attempt to initialize an STA COM apartment. Expect this to fail since the
    // thread is already in an MTA apartment.
    EXPECT_DCHECK_DEATH(InitializeCOMOnWorker(TaskEnvironment::ThreadPoolCOMEnvironment::DEFAULT, ApartmentType::kSTA));
}
...
TEST_F(TaskEnvironmentTest, NoCOMEnvironment)
{
    ...
    InitializeCOMOnWorker(TaskEnvironment::ThreadPoolCOMEnvironment::NONE, ApartmentType::kMTA);
    InitializeCOMOnWorker(TaskEnvironment::ThreadPoolCOMEnvironment::NONE, ApartmentType::kSTA);
}
#endif // BUILDFLAG(IS_WIN)
...
TEST_F(TaskEnvironmentTest, MAYBE_ParallelExecutionFence)
{
    ...
    constexpr int kNumParallelTasks = TaskEnvironment::kNumForegroundThreadPoolThreads;
...
TEST_F(TaskEnvironmentTest, ParallelExecutionFenceWithSingleThreadTaskEnvironment)
{
    SingleThreadTaskEnvironment task_environment;
...
#endif
        EXPECT_DEATH_IF_SUPPORTED({ TaskEnvironment::ParallelExecutionFence fence(kFailureLog); }, kFailureLog);
    }));
...
namespace {
bool FailOnTaskEnvironmentLog(int severity, const char* file, int line, size_t message_start, const std::string& str)
{
...
TEST_F(TaskEnvironmentTest, DisallowRunTasksRetriesForFullTimeout)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, BindOnce(&TestWaitableEvent::Signal, Unretained(&resume_worker_task)), Seconds(1));
...
TEST_F(TaskEnvironmentTest, RunUntilQuit_RunsMainThread)
{
    ...
    SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        task_run = true;
        quit.Run();
    }));
...
public:
    ...
    void CountDown(int n, OnceClosure done)
    {
        ...
        SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindOnce(&TestLogger::CountDown, Unretained(this), n - 1, std::move(done)));
    }
...
TEST_F(TaskEnvironmentTest, RunUntilQuit_QueuedExecution)
{
    ...
    logger.AsyncCall(&TestLogger::CountDown).WithArgs(5, task_environment.QuitClosure());
    ...
    auto quit = task_environment.QuitClosure();
    logger.AsyncCall(&TestLogger::GetLog).Then(BindLambdaForTesting([&](std::vector<std::string> log) {
        actual_log = log;
        quit.Run();
    }));
    task_environment.RunUntilQuit();

    ASSERT_THAT(actual_log, testing::ElementsAre("5", "Test", "4", "3", "2", "1"));
}
...
TEST_F(TaskEnvironmentTest, RunUntilQuit_ThreadPoolStaysQueued)
{
    ...
    bool task_run = false;
    ThreadPool::PostTask(FROM_HERE, BindLambdaForTesting([&] { task_run = true; }));
...
TEST_F(TaskEnvironmentTest, RunUntilQuit_QuitClosureInvalidatedByRun)
{
    ...
    SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { log.push_back("after quit2"); }));
    // `quit3` will terminate execution.
    SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, quit3);
    // This line will *not* be logged.
    SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, BindLambdaForTesting([&] { log.push_back("after quit3"); }));
    task_environment.RunUntilQuit();
...
{
    TaskEnvironment task_environment;
    EXPECT_DCHECK_DEATH_WITH(task_environment.RunUntilQuit(), R"(QuitClosure\(\) not called before RunUntilQuit\(\))");
}
