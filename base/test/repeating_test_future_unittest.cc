...
public:
    ...
    explicit MoveOnlyValue(std::string data)
        : data(std::move(data))
    {
    }
...
public:
    ...
    {
        SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(callable));
    }
...
TEST_F(RepeatingTestFutureTest, ShouldNotBeEmptyIfTakingOneElementFromAfutureWith2Elements)
{
    RepeatingTestFuture<std::string> future;
...
TEST_F(RepeatingTestFutureTest, WaitShouldReturnTrueImmediatelyWhenValueIsAlreadyPresent)
{
    RepeatingTestFuture<std::string> future;
...
TEST_F(RepeatingTestFutureTest, TakeShouldBlockUntilAnElementArrives)
{
    ...
    RunLater(BindLambdaForTesting([&future] { future.AddValue("value pushed delayed"); }));
...
TEST_F(RepeatingTestFutureTest, TakeShouldWorkWithMoveOnlyValue)
{
    ...
    RunLater(BindLambdaForTesting([&future] { future.AddValue(MoveOnlyValue("move only value")); }));
...
TEST_F(RepeatingTestFutureTest, ShouldAllowReferenceArgumentsForCallback)
{
    ...
    RepeatingCallback<void(const std::string&)> callback = future.GetCallback<const std::string&>();
...
TEST_F(RepeatingTestFutureTest, ShouldStoreMultipleValuesInATuple)
{
    ...
    RunLater(BindLambdaForTesting([&] { future.AddValue(expected_int_value, expected_string_value); }));
...
TEST_F(RepeatingTestFutureTest, ShouldAllowCallbackWithMultipleReferenceValues)
{
    const int expected_int_value = 5;
    const std::string expected_string_value = "value";
    ...
    RepeatingCallback<void(const int&, std::string)> callback = future.GetCallback<const int&, std::string>();
...
TEST_F(RepeatingTestFutureTest, ShouldSupportMultipleCvRefType)
{
    ...
    base::OnceCallback<void(const int, std::string&, const long&)> callback = future.GetCallback();
