...
void TestBatteryLevelProvider::GetBatteryState(base::OnceCallback<void(const std::optional<base::BatteryLevelProvider::BatteryState>&)> callback)
{
    std::move(callback).Run(battery_state_);
}

void TestBatteryLevelProvider::SetBatteryState(std::optional<base::BatteryLevelProvider::BatteryState> battery_state)
{
    battery_state_ = battery_state;
}

// static
base::BatteryLevelProvider::BatteryState TestBatteryLevelProvider::CreateBatteryState(int battery_count, bool is_external_power_connected, int charge_percent)
{
    return {
