...
void EmitChromeTrackEventDescriptor()
{
    base::TrackEvent::Trace([&](base::TrackEvent::TraceContext ctx) {
        protozero::MessageHandle<perfetto::protos::pbzero::TracePacket> handle = ctx.NewTracePacket();
        auto* extension_descriptor = handle->BeginNestedMessage<protozero::Message>(perfetto::protos::pbzero::TracePacket::kExtensionDescriptorFieldNumber);
        extension_descriptor->AppendBytes(perfetto::protos::pbzero::ExtensionDescriptor::kExtensionSetFieldNumber,
            base::testing::kChromeTrackEventDescriptor.data(), base::testing::kChromeTrackEventDescriptor.size());
        handle->Finalize();
    });
}
...
TestTraceProcessorImpl::PerfettoSQLModule GetChromeStdlib()
{
    ...
    for (const auto& file_to_sql : perfetto::trace_processor::chrome_stdlib::kFileToSql) {
        ...
        stdlib.emplace_back(kChromeSqlModuleName + "." + include_key, file_to_sql.sql);
    }
    return stdlib;
}
} // namespace

TraceConfig DefaultTraceConfig(std::string_view category_filter_string, bool privacy_filtering)
{
    ...
    source_config->set_track_event_config_raw(track_event_config.SerializeAsString());
...
{
    auto status = test_trace_processor_.OverrideSqlModule(kChromeSqlModuleName, GetChromeStdlib());
    CHECK(status.ok());
}

TestTraceProcessor::~TestTraceProcessor() = default;

void TestTraceProcessor::StartTrace(std::string_view category_filter_string, bool privacy_filtering)
{
    StartTrace(DefaultTraceConfig(category_filter_string, privacy_filtering));
}

void TestTraceProcessor::StartTrace(const TraceConfig& config, perfetto::BackendType backend)
{
    ...
    if (backend == perfetto::kUnspecifiedBackend) {
        if (base::trace_event::TraceLog::GetInstance()->IsPerfettoInitializedByTraceLog()) {
...
base::expected<TestTraceProcessor::QueryResult, std::string> TestTraceProcessor::RunQuery(const std::string& query)
{
