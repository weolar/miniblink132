...
template <size_t I = 0, typename T> auto MoveArg(T* out)
{
    return [out](auto&&... args) { *out = std::move(std::get<I>(std::tie(args...))); };
}
...
//   EXPECT_THAT(int_result, Pointee(Eq(123)));
//
template <size_t I = 0, typename T1, typename T2> auto MoveArgAndReturn(T1* out, T2&& return_value)
{
