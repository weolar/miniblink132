...
TEST(TestPendingTaskTest, GTestPrettyPrint)
{
    ...
    EXPECT_THAT(::testing::PrintToString(task), ::testing::StartsWith("TestPendingTask("));
...
TEST(TestPendingTaskTest, ShouldRunBefore)
{
    ...
    EXPECT_FALSE(task_after.ShouldRunBefore(task_first)) << task_after << ".ShouldRunBefore(" << task_first << ")\n";
