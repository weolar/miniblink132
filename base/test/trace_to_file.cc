...
void TraceToFile::BeginTracingFromCommandLineOptions()
{
    ...
    // TraceEventImpl
    std::string filter = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTraceToFile);

    FilePath path;
    if (CommandLine::ForCurrentProcess()->HasSwitch(switches::kTraceToFileName)) {
        path = FilePath(CommandLine::ForCurrentProcess()->GetSwitchValuePath(switches::kTraceToFileName));
    } else {
...
void TraceToFile::BeginTracing(const FilePath& path, const std::string& categories)
{
    DCHECK(!started_);
    started_ = true;
    path_ = path;
    ...
    trace_event::TraceLog::GetInstance()->SetEnabled(
        trace_event::TraceConfig(categories, trace_event::RECORD_UNTIL_FULL), trace_event::TraceLog::RECORDING_MODE);
}
...
static void OnTraceDataCollected(
    OnceClosure quit_closure, trace_event::TraceResultBuffer* buffer, const scoped_refptr<RefCountedString>& json_events_str, bool has_more_events)
{
...
void TraceToFile::EndTracingIfNeeded()
{
    ...
    buffer.SetOutputCallback(BindRepeating(&TraceToFile::TraceOutputCallback, Unretained(this)));
    ...
    RunLoop run_loop;
    trace_event::TraceLog::GetInstance()->Flush(BindRepeating(&OnTraceDataCollected, run_loop.QuitClosure(), Unretained(&buffer)));
    run_loop.Run();
