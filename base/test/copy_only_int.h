...
class CopyOnlyInt {
public:
    explicit CopyOnlyInt(int data = 1)
        : data_(data)
    {
    }
    ...
    friend bool operator==(const CopyOnlyInt& lhs, const CopyOnlyInt& rhs) = default;
    friend auto operator<=>(const CopyOnlyInt& lhs, const CopyOnlyInt& rhs) = default;
    ...
    static auto SetScopedDestructionCallback(RepeatingCallback<void(int)> callback)
    {
