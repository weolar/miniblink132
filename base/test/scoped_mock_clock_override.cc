...
ScopedMockClockOverride::ScopedMockClockOverride()
    ...
    offset_(Days(365))
{
    DCHECK(!scoped_mock_clock_) << "Nested ScopedMockClockOverrides are not supported.";

    scoped_mock_clock_ = this;

    time_clock_overrides_ = std::make_unique<subtle::ScopedTimeClockOverrides>(
        &ScopedMockClockOverride::Now, &ScopedMockClockOverride::NowTicks, &ScopedMockClockOverride::NowThreadTicks);
}
...
{
    DCHECK_GT(delta, base::TimeDelta()) << "Monotonically increasing time may not go backwards";
    offset_ += delta;
}
