...
void InitScopedFeatureListForTesting(ScopedFeatureList& scoped_feature_list)
{
    ...
    std::string enabled = command_line->GetSwitchValueASCII(switches::kEnableFeatures);
    std::string disabled = command_line->GetSwitchValueASCII(switches::kDisableFeatures);
