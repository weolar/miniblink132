...
ScopedPathOverride::ScopedPathOverride(int key, const FilePath& path, bool is_absolute, bool create)
    : key_(key)
{
    SaveOriginal();
...
ScopedPathOverride::~ScopedPathOverride()
{
    ...
    if (original_override_) {
        ...
        // disallow blocking.
        result = PathService::OverrideAndCreateIfNeeded(key_, *original_override_, /*is_absolute=*/true, /*create=*/false);
        CHECK(result);
    }
}
