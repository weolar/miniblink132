...
BASE_FEATURE_PARAM(bool, kTestFeatureParam1, &kTestFeature1, "TestFeatureParam1", false);
...
void ExpectFeatures(const std::string& enabled_features, const std::string& disabled_features)
{
    ...
    list->GetFeatureOverrides(&actual_enabled_features, &actual_disabled_features);
...
{
    const std::map<std::string, std::string> feature_params1 = { { "x", "uma" }, { "y", "ukm" } };
    const std::map<std::string, std::string> feature_params2 = { { "x", "ukm" }, { "y", "uma" } };
    ...
    FieldTrial* original_field_trial = FieldTrialList::Find(active_groups[0].trial_name);
...

    {
        ...
        feature_list2.InitAndEnableFeatureWithParameters(kTestFeature1, feature_params2);
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
...
{
    scoped_refptr<FieldTrial> trial = FieldTrialList::CreateFieldTrial("foo", "bar");
    ...
    test::ScopedFeatureList feature_list1;
    feature_list1.InitFromCommandLine("TestFeature1<foo.bar:param_1/value_0,TestFeature2", std::string());
    ...
    FieldTrial* trial_for_test_feature1 = FeatureList::GetFieldTrial(kTestFeature1);
...

    {
        ...
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature2, kParam));
        EXPECT_NE(trial.get(), FeatureList::GetFieldTrial(kTestFeature1));
        EXPECT_NE(trial_for_test_feature1, FeatureList::GetFieldTrial(kTestFeature1));
        EXPECT_NE(nullptr, FeatureList::GetFieldTrial(kTestFeature1));
        EXPECT_EQ(nullptr, FeatureList::GetFieldTrial(kTestFeature2));
    }

    {
        ...
        feature_list2.InitFromCommandLine("TestFeature1<foo.bar2:param_1/value_1", std::string());
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature2));
        EXPECT_EQ(kValue, GetFieldTrialParamValueByFeature(kTestFeature1, kParam));
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature2, kParam));
...

    {
        ...
        EXPECT_EQ(trial_for_test_feature1->trial_name(), FeatureList::GetFieldTrial(kTestFeature1)->trial_name());
        EXPECT_EQ(trial_for_test_feature1->group_name(), FeatureList::GetFieldTrial(kTestFeature1)->group_name());
...
TEST_F(ScopedFeatureListTest, OverrideWithFeatureMultipleParameters)
{
    ...
    test::ScopedFeatureList feature_list1;
    feature_list1.InitFromCommandLine("TestFeature1<foo.bar:param_1/value_1/param_2/"
                                      "value_2,TestFeature2:param_1/value_2",
        std::string());

    // Check initial state.
    ExpectFeatures("TestFeature1<foo,TestFeature2<StudyTestFeature2", std::string());
    EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
    EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature2));
    EXPECT_NE(nullptr, FeatureList::GetFieldTrial(kTestFeature1));
    EXPECT_NE(nullptr, FeatureList::GetFieldTrial(kTestFeature2));
    ...
    EXPECT_EQ("GroupTestFeature2", GetActiveFieldTrialGroupName("StudyTestFeature2"));

    {
        ...
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature2));
        EXPECT_EQ(kValue3, GetFieldTrialParamValueByFeature(kTestFeature1, kParam1));
        // param_2 is not set.
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature1, kParam2));
        EXPECT_EQ(kValue2, GetFieldTrialParamValueByFeature(kTestFeature2, kParam1));
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature2, kParam2));
...

    {
        ...
        feature_list2.InitFromCommandLine("TestFeature1<foo.bar2:param_2/value_3", std::string());
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
        EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature2));
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature1, kParam1));
        EXPECT_EQ(kValue3, GetFieldTrialParamValueByFeature(kTestFeature1, kParam2));
        EXPECT_EQ(kValue2, GetFieldTrialParamValueByFeature(kTestFeature2, kParam1));
        EXPECT_EQ("", GetFieldTrialParamValueByFeature(kTestFeature2, kParam2));
        EXPECT_NE(trial, FeatureList::GetFieldTrial(kTestFeature1));
        EXPECT_NE(nullptr, FeatureList::GetFieldTrial(kTestFeature1));
...
{
    scoped_refptr<FieldTrial> trial1 = FieldTrialList::CreateFieldTrial("foo1", "bar1");
    const char kParam[] = "param_1";
    const char kValue0[] = "value_0";
    const char kValue1[] = "value_1";
    const char kValue2[] = "value_2";
    ...
    test::ScopedFeatureList feature_list1;
    feature_list1.InitFromCommandLine("TestFeature1<foo1:param_1/value_0,TestFeature2", std::string());
...

    {
        ...
        test::ScopedFeatureList feature_list2;
        feature_list2.InitWithFeaturesAndParameters({ { kTestFeature1, parameters1 }, { kTestFeature2, parameters2 } }, {});
...

    {
        ...
        test::ScopedFeatureList feature_list2;
        feature_list2.InitWithFeaturesAndParameters({ { kTestFeature1, parameters2 } }, { kTestFeature2 });
...
TEST_F(ScopedFeatureListTest, ParamsWithEmptyValue)
{
    ...
    EXPECT_EQ(kEmptyValue, GetFieldTrialParamValueByFeature(kTestFeature1, kParam));
    {
        const char kValue1[] = "normal";
        FieldTrialParams params1 = { { kParam, kValue1 } };
        test::ScopedFeatureList feature_list1;
...
TEST_F(ScopedFeatureListTest, FeatureOverrideFeatureWithEnabledFieldTrial)
{
    ...
    feature_list->RegisterFieldTrialOverride(kTestFeature1.name, FeatureList::OVERRIDE_ENABLE_FEATURE, trial);
...
TEST_F(ScopedFeatureListTest, FeatureOverrideFeatureWithDisabledFieldTrial)
{
    ...
    std::unique_ptr<FeatureList> feature_list(new FeatureList);
    FieldTrial* trial = FieldTrialList::CreateFieldTrial("TrialExample", "A");
    feature_list->RegisterFieldTrialOverride(kTestFeature1.name, FeatureList::OVERRIDE_DISABLE_FEATURE, trial);
    feature_list1.InitWithFeatureList(std::move(feature_list));
...
TEST_F(ScopedFeatureListTest, RestoreFieldTrialParamsCorrectlyWhenLeakedFieldTrialCreated)
{
    test::ScopedFeatureList feature_list1;
    feature_list1.InitFromCommandLine("TestFeature1:TestParam/TestValue1", "");
    EXPECT_TRUE(FeatureList::IsEnabled(kTestFeature1));
    EXPECT_EQ("TestValue1", GetFieldTrialParamValueByFeature(kTestFeature1, "TestParam"));
    ...
    {
        FieldTrialList* backup_field_trial = FieldTrialList::BackupInstanceForTesting();
...
{
    test::ScopedFeatureList feature_list1;
    feature_list1.InitWithFeatureStates({ { kTestFeature1, true }, { kTestFeature2, false } });
    ExpectFeatures(/*enabled_features=*/"TestFeature1",
        /*disabled_features=*/"TestFeature2");

    {
        test::ScopedFeatureList feature_list2;
        feature_list2.InitWithFeatureStates({ { kTestFeature1, false }, { kTestFeature2, true } });
...
TEST_F(ScopedFeatureListTest, FeatureParameterCache)
{
    ...
    feature_list_to_override_cached_parameter.InitAndEnableFeatureWithParameters(kTestFeature1, { { kTestFeatureParam1.name, "true" } });
