...
void OnTraceDataCollected(
    base::OnceClosure quit_closure, base::trace_event::TraceResultBuffer* buffer, const scoped_refptr<base::RefCountedString>& json, bool has_more_events)
{
...
bool TraceEvent::SetFromJSON(const base::Value* event_value)
{
    ...
    bool require_id = (phase == TRACE_EVENT_PHASE_ASYNC_BEGIN || phase == TRACE_EVENT_PHASE_ASYNC_STEP_INTO || phase == TRACE_EVENT_PHASE_ASYNC_STEP_PAST
        || phase == TRACE_EVENT_PHASE_MEMORY_DUMP || phase == TRACE_EVENT_PHASE_CREATE_OBJECT || phase == TRACE_EVENT_PHASE_DELETE_OBJECT
        || phase == TRACE_EVENT_PHASE_SNAPSHOT_OBJECT || phase == TRACE_EVENT_PHASE_ASYNC_END || phase == TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN
    ...
    if (require_id) {
        ...
        if (!maybe_id && !maybe_id2) {
            LOG(ERROR) << "id/id2 is missing from ASYNC_BEGIN/ASYNC_END TraceEvent JSON";
            return false;
        }
        if (maybe_id)
            id = *maybe_id;
...
bool TraceEvent::GetArgAsString(const std::string& arg_name, std::string* arg) const
{
...
bool TraceEvent::GetArgAsNumber(const std::string& arg_name, double* arg) const
{
...
bool TraceEvent::GetArgAsDict(const std::string& arg_name, base::Value::Dict* arg) const
{
...
base::Value::Dict TraceEvent::GetKnownArgAsDict(const std::string& arg_name) const
{
...
bool Query::Evaluate(const TraceEvent& event) const
{
    ...
    DCHECK_EQ(QUERY_BOOLEAN_OPERATOR, type_) << "Invalid query: missing boolean expression";
    DCHECK(left_.get());
    DCHECK(right_.get() || is_unary_operator());

    if (is_comparison_operator()) {
        DCHECK(left().is_value() && right().is_value()) << "Invalid query: comparison operator used between event member and "
                                                           "value.";
...
bool Query::EvaluateArithmeticOperator(const TraceEvent& event, double* num) const
{
...
const TraceEvent* Query::SelectTargetEvent(const TraceEvent* event, TraceEventMember member)
{
...
bool Query::GetMemberValueAsString(const TraceEvent& event, std::string* str) const
{
    DCHECK_EQ(QUERY_EVENT_MEMBER, type_);
...
size_t FindMatchingEvents(const std::vector<TraceEvent>& events, const Query& query, TraceEventVector* output, bool ignore_metadata_events)
{
...
bool ParseEventsFromJson(const std::string& json, std::vector<TraceEvent>* output)
{
...
TraceAnalyzer::TraceAnalyzer()
    ...
    , allow_association_changes_(true)
{
}
...
// static
std::unique_ptr<TraceAnalyzer> TraceAnalyzer::Create(const std::string& json_events)
{
...
void TraceAnalyzer::AssociateBeginEndEvents()
{
    ...
    Query match(Query::EventName() == Query::OtherName() && Query::EventCategory() == Query::OtherCategory() && Query::EventTid() == Query::OtherTid()
...
void TraceAnalyzer::AssociateAsyncBeginEndEvents(bool match_pid)
{
    ...
    Query begin(Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_BEGIN) || Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_STEP_INTO)
        || Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_STEP_PAST) || Query::EventPhaseIs(TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN));
    Query end(Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_END) || Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_STEP_INTO)
        || Query::EventPhaseIs(TRACE_EVENT_PHASE_ASYNC_STEP_PAST) || Query::EventPhaseIs(TRACE_EVENT_PHASE_NESTABLE_ASYNC_END));
    Query match(Query::EventCategory() == Query::OtherCategory() && Query::EventId() == Query::OtherId());
...
void TraceAnalyzer::AssociateEvents(const Query& first, const Query& second, const Query& match)
{
    DCHECK(allow_association_changes_) << "AssociateEvents not allowed after FindEvents";
    ...
    for (auto& this_event : raw_events_) {
        if (second.Evaluate(this_event)) {
            ...
            for (int stack_index = static_cast<int>(begin_stack.size()) - 1; stack_index >= 0; --stack_index) {
...
void TraceAnalyzer::MergeAssociatedEventArgs()
{
    for (auto& i : raw_events_) {
        ...
        while (other && encounters.find(other) == encounters.end()) {
            ...
            i.arg_numbers.insert(other->arg_numbers.begin(), other->arg_numbers.end());
            i.arg_strings.insert(other->arg_strings.begin(), other->arg_strings.end());
...
size_t TraceAnalyzer::FindEvents(const Query& query, TraceEventVector* output)
{
    ...
    output->clear();
    return FindMatchingEvents(raw_events_, query, output, ignore_metadata_events_);
}
...
const std::string& TraceAnalyzer::GetThreadName(const TraceEvent::ProcessThreadID& thread)
{
...
void Start(const std::string& category_filter_string)
{
    ...
    base::trace_event::TraceLog::GetInstance()->SetEnabled(
        base::trace_event::TraceConfig(category_filter_string, ""), base::trace_event::TraceLog::RECORDING_MODE);
}
...
std::unique_ptr<TraceAnalyzer> Stop()
{
    ...
    buffer.Start();
    base::trace_event::TraceLog::GetInstance()->Flush(base::BindRepeating(&OnTraceDataCollected, run_loop.QuitClosure(), base::Unretained(&buffer)));
    run_loop.Run();
    buffer.Finish();
...
bool GetRateStats(const TraceEventVector& events, RateStats* stats, const RateStatsOptions* options)
{
    DCHECK(stats);
...
bool FindFirstOf(const TraceEventVector& events, const Query& query, size_t position, size_t* return_index)
{
    DCHECK(return_index);
...
bool FindLastOf(const TraceEventVector& events, const Query& query, size_t position, size_t* return_index)
{
    DCHECK(return_index);
...
bool FindClosest(const TraceEventVector& events, const Query& query, size_t position, size_t* return_closest, size_t* return_second_closest)
{
...
size_t CountMatches(const TraceEventVector& events, const Query& query, size_t begin_position, size_t end_position)
{
