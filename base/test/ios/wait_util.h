...
[[nodiscard]] bool WaitUntilConditionOrTimeout(TimeDelta timeout, bool run_message_loop, ConditionBlock condition);

// Same as above but `run_message_loop` is false.
[[nodiscard]] bool WaitUntilConditionOrTimeout(TimeDelta timeout, ConditionBlock condition);
