...
#define SPIN_FOR_1_SECOND_OR_UNTIL_TRUE(expression) SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(base::Seconds(1), (expression))
    ...
    do {                                                                                                                                                       \
        ...
        while (!(expression)) {                                                                                                                                \
            if (kSpinWaitTimeout < base::TimeTicks::Now() - spin_wait_start) {                                                                                 \
                EXPECT_LE((base::TimeTicks::Now() - spin_wait_start).InMilliseconds(), kSpinWaitTimeout.InMilliseconds()) << "Timed out";                      \
                break;                                                                                                                                         \
            }                                                                                                                                                  \
