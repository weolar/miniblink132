...
public:
    ...
    Server(fidl::InterfaceRequest<fuchsia::io::Directory> directory_request, OnceCallback<void(zx_status_t status)> on_initialized);
...
ScopedDevZero::Server::Server(fidl::InterfaceRequest<fuchsia::io::Directory> directory_request, OnceCallback<void(zx_status_t status)> on_initialized)
{
    ...
    if (status == ZX_OK) {
        status = dev_dir_.AddEntry("zero", std::make_unique<vfs::VmoFile>(std::move(vmo), /*length=*/UINT32_MAX));
        ZX_LOG_IF(ERROR, status != ZX_OK, status);
    }

    if (status == ZX_OK) {
        status = dev_dir_.Serve(fuchsia::io::OpenFlags::RIGHT_READABLE, directory_request.TakeChannel());
        ZX_LOG_IF(ERROR, status != ZX_OK, status);
    }
...
bool ScopedDevZero::Initialize()
{
    ...
    RunLoop run_loop;
    server_ = SequenceBound<Server>(io_thread_.task_runner(), fidl::InterfaceRequest<fuchsia::io::Directory>(std::move(request)),
        base::BindOnce(
            [](base::OnceClosure quit_loop, zx_status_t& status, zx_status_t init_status) {
                status = init_status;
