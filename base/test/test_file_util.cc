...
constexpr FilePath::CharType kDirPrefix[] = FILE_PATH_LITERAL("test_scoped_temp_dir");
...
public:
    ...
    static PathDeleterOnTestEnd* GetInstance()
    {
        return instance_;
    }
    ...
    void OnTestEnd(const testing::TestInfo& test_info) override
    {
        ...
        for (const FilePath& file_path : file_paths_to_delete_) {
            if (!DieFileDie(file_path, /*recurse=*/true)) {
                ADD_FAILURE() << "Failed to delete temporary directory for testing: " << file_path;
            }
        }
...
FilePath CreateUniqueTempDirectoryScopedToTest()
{
    ...
    if (!PathDeleterOnTestEnd::GetInstance()) {
        ...
        testing::UnitTest::GetInstance()->listeners().Append(new PathDeleterOnTestEnd());
