...
#if (!defined(NDEBUG) || defined(MEMORY_SANITIZER) || defined(ADDRESS_SANITIZER)) && BUILDFLAG(IS_CHROMEOS_ASH)
...
void InitializeTimeout(const char* switch_name, base::TimeDelta min_value, base::TimeDelta* value)
{
    DCHECK(value);
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switch_name)) {
        ...
        if (!base::StringToInt(string_value, &command_line_timeout_ms)) {
            LOG(FATAL) << "Timeout value \"" << string_value << "\" was parsed as " << command_line_timeout_ms;
        }
    ...
    *value = std::max(std::max(*value, command_line_timeout) * kTimeoutMultiplier, min_value);
}
...
void TestTimeouts::Initialize()
{
    ...
    if (being_debugged) {
        fprintf(stdout, "Detected presence of a debugger, running without test timeouts.\n");
    }

    // Note that these timeouts MUST be initialized in the correct order as
    // per the CHECKS below.

    InitializeTimeout(switches::kTestTinyTimeout, base::TimeDelta(), &tiny_timeout_);
    ...
    if (being_debugged || base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestLauncherInteractive)) {
        min_ui_test_action_timeout = base::Days(1);
    }

    InitializeTimeout(switches::kUiTestActionTimeout, min_ui_test_action_timeout, &action_timeout_);
    InitializeTimeout(switches::kUiTestActionMaxTimeout, action_timeout_, &action_max_timeout_);
