...
template <typename T>
concept HasVoidValueType = base::internal::IsExpected<T> && std::is_void_v<typename std::remove_cvref_t<T>::value_type>;
...
private:
    ...
    public:
        ...
        bool MatchAndExplain(T actual_value, ::testing::MatchResultListener* listener) const override
        {
...
template <typename T> class ValueIsMatcher {
public:
    explicit ValueIsMatcher(T matcher)
        : matcher_(std::move(matcher))
    {
    }

    template <typename U> operator ::testing::Matcher<U>() const
    { // NOLINT
...
private:
    ...
    public:
        ...
        bool MatchAndExplain(U actual_value, ::testing::MatchResultListener* listener) const override
        {
            if (!actual_value.has_value()) {
                *listener << "which has the error " << ToString(actual_value.error());
                return false;
            ...
            const bool match = matcher_.MatchAndExplain(actual_value.value(), &inner_listener);
            const std::string explanation = inner_listener.str();
            if (!explanation.empty()) {
                *listener << "which has the value " << ToString(actual_value.value()) << ", " << explanation;
            }
            return match;
        }
...
template <typename T> class ErrorIsMatcher {
public:
    explicit ErrorIsMatcher(T matcher)
        : matcher_(std::move(matcher))
    {
    }
...
private:
    ...
    public:
        ...
        bool MatchAndExplain(U actual_value, ::testing::MatchResultListener* listener) const override
        {
            ...
            ::testing::StringMatchResultListener inner_listener;
            const bool match = matcher_.MatchAndExplain(actual_value.error(), &inner_listener);
            const std::string explanation = inner_listener.str();
            if (!explanation.empty()) {
                *listener << "which has the error " << ToString(actual_value.error()) << ", " << explanation;
            }
            return match;
        }
...
template <typename T> inline internal::ValueIsMatcher<typename std::decay_t<T>> ValueIs(T&& matcher)
{
    return internal::ValueIsMatcher<typename std::decay_t<T>>(std::forward<T>(matcher));
}

// Returns a gMock matcher that matches an `expected<T, E>` which has an error
// which matches the inner matcher.
template <typename T> inline internal::ErrorIsMatcher<typename std::decay_t<T>> ErrorIs(T&& matcher)
{
    return internal::ErrorIsMatcher<typename std::decay_t<T>>(std::forward<T>(matcher));
}
...
#define ASSERT_OK_AND_ASSIGN(lhs, rexpr)                                                                                                                       \
    ASSIGN_OR_RETURN(lhs, rexpr, [](const auto& e) {                                                                                                           \
        return GTEST_MESSAGE_(base::StrCat({ #rexpr, " returned error: ", base::ToString(e) }).c_str(), ::testing::TestPartResult::kFatalFailure);             \
    })

namespace base {
template <typename T, typename E> void PrintTo(const expected<T, E>& expected, ::std::ostream* os)
{
    *os << expected.ToString();
}

template <typename T> void PrintTo(const ok<T>& a, ::std::ostream* os)
{
    *os << a.ToString();
}

template <typename T> void PrintTo(const unexpected<T>& a, ::std::ostream* os)
{
    *os << a.ToString();
}
} // namespace base
