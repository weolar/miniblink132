...
template <typename Lambda> void RunLater(Lambda lambda)
{
    SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindLambdaForTesting(lambda));
}

void PostDelayedTask(base::OnceClosure closure, base::TimeDelta delay)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, std::move(closure), delay);
}
...
TEST_F(RunUntilTest, ShouldNotSimplyActivelyInvokePredicateInALoop)
{
    ...
    PostDelayedTask(base::BindLambdaForTesting([&done] { done = true; }), base::Milliseconds(50));
...
TEST_F(RunUntilTest, ShouldNotSimplyReturnOnFirstIdle)
{
    ...
    PostDelayedTask(base::BindLambdaForTesting([&done] { done = true; }), base::Milliseconds(10));

    EXPECT_TRUE(RunUntil([&] { return done; }));
}

TEST_F(RunUntilTest, ShouldAlwaysLetOtherTasksRunFirstEvenIfPredicateIsAlreadyFulfilled)
{
...
TEST_F(RunUntilTest, ShouldReturnFalseIfTimeoutHappens)
{
    ...
    EXPECT_NONFATAL_FAILURE({ success = RunUntil([] { return false; }); }, "timed out");
...
TEST(RunUntilTestWithThreadPool, SupportsMockTime)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, base::BindOnce([](bool* flag) { *flag = true; }, &done), base::Days(1));
...
TEST(RunUntilTestWithThreadPool, TimesOutWhenMainThreadSleepsForever)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, BindLambdaForTesting([&]() { done.Set(); }), Milliseconds(1));
...
TEST(RunUntilTestWithMockTime, ConditionOnlyObservedIfWorkIsDone)
{
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, BindLambdaForTesting([&done]() { done.Set(); }), Milliseconds(1));
    PostDelayedTask(base::DoNothing(), TestTimeouts::tiny_timeout());
    EXPECT_TRUE(RunUntil([&]() { return done.IsSet(); }));
