...
void TestPredicateOrRegisterOnNextIdleCallback(
    base::FunctionRef<bool(void)> condition, CallbackListSubscription* on_idle_callback_subscription, OnceClosure ready_callback)
{
    if (condition()) {
        ...
        *on_idle_callback_subscription = CurrentThread::Get().RegisterOnNextIdleCallback(
            {}, BindOnce(TestPredicateOrRegisterOnNextIdleCallback, condition, on_idle_callback_subscription, std::move(ready_callback)));
    }
}
...
bool RunUntil(base::FunctionRef<bool(void)> condition)
{
    ...
    // timeout.
    CHECK(test::ScopedRunLoopTimeout::ExistsForCurrentThread() || subtle::ScopedTimeClockOverrides::overrides_active())
    ...
    on_idle_callback_subscription = CurrentThread::Get().RegisterOnNextIdleCallback(
        {}, BindOnce(TestPredicateOrRegisterOnNextIdleCallback, condition, &on_idle_callback_subscription, ready_signal.GetCallback()));
