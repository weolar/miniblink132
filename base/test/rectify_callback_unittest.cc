...
#define CALLBACK_TESTS_VOID(TestNameSuffix, Signature, LambdaToRectify, ...)                                                                                   \
    ...
    {                                                                                                                                                          \
        auto cb = RectifyCallback<RepeatingCallback<Signature>>(BindRepeating(LambdaToRectify));                                                               \
        cb.Run(__VA_ARGS__);                                                                                                                                   \
    }                                                                                                                                                          \
    TEST(RectifyCallbackTest, RepeatingToOnceVoid##TestNameSuffix)                                                                                             \
    {                                                                                                                                                          \
        auto cb = RectifyCallback<OnceCallback<Signature>>(BindRepeating(LambdaToRectify));                                                                    \
    ...
    {                                                                                                                                                          \
        auto cb = RectifyCallback<OnceCallback<Signature>>(BindOnce(LambdaToRectify));                                                                         \
        std::move(cb).Run(__VA_ARGS__);                                                                                                                        \
    }
...
CALLBACK_TESTS_VOID(
    OneArgRemoveOneArg, void(int), [] {}, 1)

CALLBACK_TESTS_VOID(
    TwoArgsRemoveTwoArgs, void(float, int), [] {}, 0.25f, 1)

CALLBACK_TESTS_VOID(
    OneArgSameSignature, void(int), [](int x) { EXPECT_EQ(1, x); }, 1)

CALLBACK_TESTS_VOID(
    TwoArgsRemoveOneArg, void(float, int), [](int x) { EXPECT_EQ(1, x); }, 0.25f, 1)

CALLBACK_TESTS_VOID(
    ThreeArgsRemoveTwoArgs, void(bool, float, int), [](int x) { EXPECT_EQ(1, x); }, true, 0.25f, 1)

// Test rectifying functions that do return a value.

#define CALLBACK_TESTS_RETURN(TestNameSuffix, Signature, LambdaToRectify, ExpectedReturn, ...)                                                                 \
    TEST(RectifyCallbackTest, RepeatingCallbackSignatureReturn##TestNameSuffix)                                                                                \
    {                                                                                                                                                          \
        auto cb = RectifyCallback<Signature>(BindRepeating(LambdaToRectify));                                                                                  \
...
CALLBACK_TESTS_RETURN(
    NoArgsSameSignature, int(), [] { return 2; }, 2)

CALLBACK_TESTS_RETURN(
    OneArgRemoveOneArg, int(int), [] { return 2; }, 2, 1)

CALLBACK_TESTS_RETURN(
    TwoArgsRemoveTwoArgs, int(float, int), [] { return 2; }, 2, 0.25f, 1)

CALLBACK_TESTS_RETURN(
    OneArgSameSignature, int(int), [](int x) { return x; }, 2, 2)

CALLBACK_TESTS_RETURN(
    TwoArgsRemoveOneArg, int(float, int), [](int x) { return x; }, 2, 0.25f, 2)

CALLBACK_TESTS_RETURN(
    ThreeArgsRemoveTwoArgs, int(bool, float, int), [](int x) { return x; }, 2, true, 0.25f, 2)

// Test proper forwarding of move-only arguments.

CALLBACK_TESTS_RETURN(
    DiscardsMoveOnlyArgs, bool(int, std::unique_ptr<int>), [] { return true; }, true, 2, std::make_unique<int>(3))

CALLBACK_TESTS_RETURN(
    UsesMoveOnlyArg, int(float, int, std::unique_ptr<int>), [](std::unique_ptr<int> p) { return *p; }, 3, 0.25f, 2, std::make_unique<int>(3))
...
template <typename T> void ExampleFunctionRepeatingCallback(T&& callback)
{
    ExampleFunctionRepeatingCallback(RectifyCallback<ExampleRepeatingTargetType>(std::forward<T>(callback)));
}
...
// If the rectify goes awry (e.g. the wrong kind of callback is generated),
// this can result in an infinite loop.
template <typename T> void ExampleFunctionOnceCallback(T&& callback)
{
    ExampleFunctionOnceCallback(RectifyCallback<ExampleOnceTargetType>(std::forward<T>(callback)));
}
...
TEST(RectifyCallbackTest, TemplateOverloadRectifiesRepeatingCallback)
{
    ...
    ExampleFunctionOnceCallback(BindRepeating([](double d, int i) { return d && i; }));
...
TEST(RectifyCallbackTest, TemplateOverloadCoerceRepeatingTarget)
{
    ...
    ExampleFunctionRepeatingCallback(BindRepeating([](double d, int i) { return d && i; }));
    auto cb = BindRepeating(&ExampleTargetFunction);
    ExampleFunctionRepeatingCallback(cb);
    bool result = true;
    ExampleFunctionRepeatingCallback(BindLambdaForTesting([&] { return result; }));
}
