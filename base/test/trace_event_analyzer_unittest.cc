...
public:
    void ManualSetUp();
    void OnTraceDataCollected(base::WaitableEvent* flush_complete_event, const scoped_refptr<base::RefCountedString>& json_events_str, bool has_more_events);
    void BeginTracing();
    void EndTracing();
...
void TraceEventAnalyzerTest::OnTraceDataCollected(
    base::WaitableEvent* flush_complete_event, const scoped_refptr<base::RefCountedString>& json_events_str, bool has_more_events)
{
...
void TraceEventAnalyzerTest::BeginTracing()
{
    output_.json_output.clear();
    buffer_.Start();
    base::trace_event::TraceLog::GetInstance()->SetEnabled(base::trace_event::TraceConfig("*", ""), base::trace_event::TraceLog::RECORDING_MODE);
}

void TraceEventAnalyzerTest::EndTracing()
{
    base::trace_event::TraceLog::GetInstance()->SetDisabled();
    base::WaitableEvent flush_complete_event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
    base::trace_event::TraceLog::GetInstance()->Flush(
        base::BindRepeating(&TraceEventAnalyzerTest::OnTraceDataCollected, base::Unretained(this), base::Unretained(&flush_complete_event)));
...
TEST_F(TraceEventAnalyzerTest, NoEvents)
{
    ...
    std::unique_ptr<TraceAnalyzer> analyzer(TraceAnalyzer::Create(output_.json_output));
...
TEST_F(TraceEventAnalyzerTest, ArithmeticOperators)
{
    ...
    BeginTracing();
    {
        ...
        TRACE_EVENT_INSTANT2("cat1", "math1", TRACE_EVENT_SCOPE_THREAD, "a", 10, "b", 5);
        TRACE_EVENT_INSTANT2("cat1", "math2", TRACE_EVENT_SCOPE_THREAD, "a", 10, "b", 10);
        // Extra events that never match, for noise:
        TRACE_EVENT_INSTANT2("noise", "math3", TRACE_EVENT_SCOPE_THREAD, "a", 1, "b", 3);
        TRACE_EVENT_INSTANT2("noise", "math4", TRACE_EVENT_SCOPE_THREAD, "c", 10, "d", 5);
    }
    EndTracing();
...
TEST_F(TraceEventAnalyzerTest, CompleteDuration)
{
    ...
    TraceEventVector found;
    analyzer->FindEvents(Query::EventCompleteDuration() > Query::Int(static_cast<int>(duration_cutoff_us))
            && (Query::EventCategory() == Query::String("cat1") || Query::EventCategory() == Query::String("cat2")
...
TEST_F(TraceEventAnalyzerTest, AsyncBeginEndAssocationsWithSteps)
{
    ...
    EXPECT_EQ(TRACE_EVENT_PHASE_ASYNC_END, found[0]->other_event->other_event->phase);
    EXPECT_EQ(found[0]->other_event, found[0]->other_event->other_event->prev_event);
    ...
    EXPECT_EQ(TRACE_EVENT_PHASE_ASYNC_STEP_INTO, found[1]->other_event->other_event->phase);
    EXPECT_EQ(found[1]->other_event, found[1]->other_event->other_event->prev_event);
    double arg_actual = 0;
    EXPECT_TRUE(found[1]->other_event->other_event->GetArgAsNumber("a", &arg_actual));
    EXPECT_EQ(1.0, arg_actual);
    EXPECT_TRUE(found[1]->other_event->other_event->other_event);
    EXPECT_EQ(TRACE_EVENT_PHASE_ASYNC_END, found[1]->other_event->other_event->other_event->phase);
...
TEST_F(TraceEventAnalyzerTest, FindOf)
{
    ...
    EXPECT_TRUE(FindLastOf(event_ptrs, Query::Bool(true), num_events - 5, &index));
...
TEST_F(TraceEventAnalyzerTest, FindClosest)
{
    ...
    size_t index_2 = 0;
    TraceEventVector event_ptrs;
    EXPECT_FALSE(FindClosest(event_ptrs, Query::Bool(true), 0, &index_1, &index_2));
...
TEST_F(TraceEventAnalyzerTest, ComplexArgument)
{
    ...
    EXPECT_EQ(std::optional<std::string>("value"), base::OptionalFromPtr(arg.FindString("property")));
}
...
TEST_F(TraceEventAnalyzerTest, AssociateNestableAsyncEvents)
{
    ...
    BeginTracing();
    {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0("cat", "name", 0xA, base::TimeTicks() + base::Milliseconds(100));
        TRACE_EVENT_BEGIN0("noise", "name2"); // not searched for, just noise
        TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0("cat", "name", 0xA, base::TimeTicks() + base::Milliseconds(200));
    }
    EndTracing();
    ...
    TraceEventVector found;
    analyzer->FindEvents(Query::EventName() == Query::String("name") && Query::EventPhaseIs(TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN), &found);
    ASSERT_EQ(1u, found.size());
