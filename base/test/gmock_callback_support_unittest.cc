...
using TestOnceCallbackMove = base::OnceCallback<void(std::unique_ptr<int>, int* dst)>;
...
TEST(GmockCallbackSupportTest, RunCallback0)
{
    MockFunction<void(const TestCallback&)> check;
    bool dst = false;
    EXPECT_CALL(check, Call(IsNotNullCallback())).WillOnce(RunCallback<0>(true, &dst));
    check.Call(base::BindRepeating(&SetBool));
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunCallback1)
{
    ...
    bool dst = false;
    EXPECT_CALL(check, Call(0, IsNotNullCallback())).WillOnce(RunCallback<1>(true, &dst));
...
TEST(GmockCallbackSupportTest, RunCallbackPassByRef)
{
    ...
    bool dst = false;
    bool src = false;
    EXPECT_CALL(check, Call(IsNotNullCallback())).WillOnce(RunCallback<0>(ByRef(src), &dst));
    src = true;
    check.Call(base::BindRepeating(&SetBool));
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunCallbackPassByValue)
{
    ...
    bool dst = false;
    bool src = true;
    EXPECT_CALL(check, Call(IsNotNullCallback())).WillOnce(RunCallback<0>(src, &dst));
    src = false;
    check.Call(base::BindRepeating(&SetBool));
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunOnceCallback0)
{
    ...
    bool dst = false;
    bool src = true;
    EXPECT_CALL(check, Call(IsNotNullCallback())).WillOnce(RunOnceCallback<0>(src, &dst));
    src = false;
...
TEST(GmockCallbackSupportTest, RunOnceCallbackTwice)
{
    ...
    EXPECT_CALL(check, Call).WillRepeatedly(RunOnceCallbackRepeatedly<0>(std::ref(src), &dst));
...
TEST(GmockCallbackSupportTest, RunClosureValue)
{
    ...
    bool dst = false;
    EXPECT_CALL(check, Call()).WillOnce(RunClosure(base::BindRepeating(&SetBool, true, &dst)));
    check.Call();
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunClosureValueWithArgs)
{
    ...
    bool dst = false;
    EXPECT_CALL(check, Call(true, 42)).WillOnce(RunClosure(base::BindRepeating(&SetBool, true, &dst)));
    check.Call(true, 42);
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunOnceClosureValue)
{
    MockFunction<void()> check;
    bool dst = false;
    EXPECT_CALL(check, Call()).WillOnce(RunOnceClosure(base::BindOnce(&SetBool, true, &dst)));
    check.Call();
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunOnceClosureValueWithArgs)
{
    MockFunction<void(bool, int)> check;
    bool dst = false;
    EXPECT_CALL(check, Call(true, 42)).WillOnce(RunOnceClosure(base::BindOnce(&SetBool, true, &dst)));
    check.Call(true, 42);
    EXPECT_TRUE(dst);
}
...
TEST(GmockCallbackSupportTest, RunOnceClosureValueMultipleCall)
{
    MockFunction<void()> check;
    bool dst = false;
    EXPECT_CALL(check, Call()).WillRepeatedly(RunOnceClosure(base::BindOnce(&SetBool, true, &dst)));
    check.Call();
    EXPECT_TRUE(dst);
...
TEST(GmockCallbackSupportTest, RunOnceCallbackReturnsValueMoveOnly)
{
    ...
    EXPECT_CALL(check, Call).WillOnce(RunOnceCallback<0>(std::make_unique<int>(42)));
    EXPECT_EQ(43, check.Call(base::BindOnce([](std::unique_ptr<int> i) { return *i + 1; })));
}
...
TEST(GmockCallbackSupportTest, VectorOfMoveOnlyArgs)
{
    ...
    EXPECT_CALL(check, Call).WillOnce(RunOnceCallback<0>([] { return VectorOfMoveOnly(); }()));
