...
protected:
    ...
    {
        base::win::RegKey key;
        ASSERT_EQ(ERROR_SUCCESS, key.Open(HKEY_CURRENT_USER, key_path.c_str(), KEY_READ)) << key_path << " does not exist.";
    }

    void AssertKeyAbsent(const std::wstring& key_path)
    {
        base::win::RegKey key;
        ASSERT_NE(ERROR_SUCCESS, key.Open(HKEY_CURRENT_USER, key_path.c_str(), KEY_READ)) << key_path << " exists but it should not.";
    }

    void CreateKey(const std::wstring& key_path)
    {
        base::win::RegKey key;
        ASSERT_EQ(ERROR_SUCCESS, key.Create(HKEY_CURRENT_USER, key_path.c_str(), KEY_ALL_ACCESS));
    }

    std::wstring FakeOverrideManagerPath(const base::Time& time)
    {
        return fake_test_key_root_ + L"\\" + base::AsWString(base::NumberToString16(time.ToInternalValue()));
    }
...
TEST_F(RegistryOverrideManagerTest, Basic)
{
    ...
    EXPECT_EQ(ERROR_SUCCESS, create_key.Create(HKEY_CURRENT_USER, kTestKeyPath, KEY_ALL_ACCESS));
    ...
    EXPECT_EQ(ERROR_SUCCESS, read_key.Open(HKEY_CURRENT_USER, kTestKeyPath, KEY_READ));
...
{
    static constexpr base::Time::Exploded kTestTimeExploded = { .year = 2013, .month = 11, .day_of_week = 1, .day_of_month = 4 };
    base::Time kTestTime;
    EXPECT_TRUE(base::Time::FromUTCExploded(kTestTimeExploded, &kTestTime));

    std::wstring path_garbage = fake_test_key_root_ + L"\\Blah";
    std::wstring path_very_stale = FakeOverrideManagerPath(kTestTime - base::Days(100));
    std::wstring path_stale = FakeOverrideManagerPath(kTestTime - base::Days(5));
    std::wstring path_current = FakeOverrideManagerPath(kTestTime - base::Minutes(1));
    std::wstring path_future = FakeOverrideManagerPath(kTestTime + base::Minutes(1));
...
TEST_F(RegistryOverrideManagerTest, DoesNotUseMockTime)
{
    ...
    std::wstring mock_time_path_stale = FakeOverrideManagerPath(kTestTime - base::Days(5));
    std::wstring mock_time_path_current = FakeOverrideManagerPath(kTestTime - base::Minutes(1));
    ...
    std::wstring system_time_path_stale = FakeOverrideManagerPath(kTestTime_new - base::Days(5));
    std::wstring system_time_path_current = FakeOverrideManagerPath(kTestTime_new - base::Minutes(1));
