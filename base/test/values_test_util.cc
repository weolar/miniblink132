...
void ExpectDictBooleanValue(bool expected_value, const Value::Dict& dict, std::string_view path)
{
    EXPECT_EQ(dict.FindBoolByDottedPath(path), std::make_optional(expected_value)) << path;
}

void ExpectDictIntegerValue(int expected_value, const Value::Dict& dict, std::string_view path)
{
    EXPECT_EQ(dict.FindIntByDottedPath(path), std::make_optional(expected_value)) << path;
}

void ExpectDictStringValue(std::string_view expected_value, const Value::Dict& dict, std::string_view path)
{
    EXPECT_EQ(OptionalFromPtr(dict.FindStringByDottedPath(path)), std::make_optional(expected_value)) << path;
}

void ExpectDictValue(const Value::Dict& expected_value, const Value::Dict& dict, std::string_view path)
{
...
void ExpectDictValue(const Value& expected_value, const Value::Dict& dict, std::string_view path)
{
    const Value* found_value = dict.FindByDottedPath(path);
    ASSERT_TRUE(found_value) << path;
    EXPECT_EQ(*found_value, expected_value) << path;
...
class DictionaryHasValueMatcher : public testing::MatcherInterface<const base::Value::Dict&> {
public:
    DictionaryHasValueMatcher(const std::string& key, const base::Value& expected_value)
        : key_(key)
        , expected_value_(expected_value.Clone())
    {
    }

    DictionaryHasValueMatcher(const DictionaryHasValueMatcher& other) = delete;
    DictionaryHasValueMatcher& operator=(const DictionaryHasValueMatcher& other) = delete;

    bool MatchAndExplain(const base::Value::Dict& value, testing::MatchResultListener* listener) const override
    {
        const base::Value* sub_value = value.Find(key_);
        if (!sub_value) {
            *listener << "Dictionary '" << FormatAsJSON(value) << "' does not have key '" << key_ << "'";
            return false;
        }
        if (*sub_value != expected_value_) {
            *listener << "Dictionary value under key '" << key_ << "' is '" << FormatAsJSON(*sub_value) << "', expected '" << FormatAsJSON(expected_value_)
                      << "'";
            return false;
        }
        return true;
    ...
    {
        *os << "has key '" << key_ << "' with value '" << FormatAsJSON(expected_value_) << "'";
    }

    void DescribeNegationTo(std::ostream* os) const override
    {
        *os << "does not have key '" << key_ << "' with value '" << FormatAsJSON(expected_value_) << "'";
    }
...
class DictionaryHasValuesMatcher : public testing::MatcherInterface<const base::Value::Dict&> {
public:
    ...
    DictionaryHasValuesMatcher& operator=(const DictionaryHasValuesMatcher& other) = delete;
    ...
    bool MatchAndExplain(const base::Value::Dict& value, testing::MatchResultListener* listener) const override
    {
        ...
        for (auto template_dict_item : template_value_) {
            ...
            if (!sub_value) {
                *listener << "\nDictionary does not have key '" << template_dict_item.first << "'";
                ok = false;
                continue;
            }
            if (*sub_value != template_dict_item.second) {
                *listener << "\nDictionary value under key '" << template_dict_item.first << "' is '" << FormatAsJSON(*sub_value) << "', expected '"
...
//
std::optional<Value> ParseJsonHelper(std::string_view json, std::optional<Value::Type> expected_type)
{
    auto result = JSONReader::ReadAndReturnValueWithError(json, JSON_PARSE_CHROMIUM_EXTENSIONS | JSON_ALLOW_TRAILING_COMMAS);
    if (!result.has_value()) {
        ADD_FAILURE() << "Failed to parse \"" << json << "\": " << result.error().message;
        return std::nullopt;
    }
...
testing::Matcher<const base::Value::Dict&> DictionaryHasValue(const std::string& key, const base::Value& expected_value)
{
    return testing::MakeMatcher(new DictionaryHasValueMatcher(key, expected_value));
}

testing::Matcher<const base::Value::Dict&> DictionaryHasValues(const base::Value::Dict& template_value)
{
...
bool IsJsonMatcher::MatchAndExplain(std::string_view json, testing::MatchResultListener* listener) const
{
...
bool IsJsonMatcher::MatchAndExplain(const base::Value& value, testing::MatchResultListener* /* listener */) const
{
    return expected_value_ == value;
}

bool IsJsonMatcher::MatchAndExplain(const base::Value::Dict& dict, testing::MatchResultListener* /* listener */) const
{
    return expected_value_.is_dict() && expected_value_.GetDict() == dict;
}

bool IsJsonMatcher::MatchAndExplain(const base::Value::List& list, testing::MatchResultListener* /* listener */) const
{
...
{
    std::optional<Value> result = ParseJsonHelper(json, /*expected_type=*/std::nullopt);
    return result.has_value() ? std::move(*result) : Value();
}

Value::Dict ParseJsonDict(std::string_view json)
{
    std::optional<Value> result = ParseJsonHelper(json, /*expected_type=*/Value::Type::DICT);
    return result.has_value() ? std::move(*result).TakeDict() : Value::Dict();
}

Value::List ParseJsonList(std::string_view json)
{
    std::optional<Value> result = ParseJsonHelper(json, /*expected_type=*/Value::Type::LIST);
...
Value::Dict ParseJsonDictFromFile(const FilePath& json_file_path)
{
    ...
    if (!ReadFileToString(json_file_path, &json)) {
        ADD_FAILURE() << "Failed to load json file for parsing. path=" << json_file_path;
        return {};
    }
    return ParseJsonDict(json);
}

expected<void, WriteJsonError> WriteJsonFile(const FilePath& json_file_path, ValueView root)
{
    std::string json;
