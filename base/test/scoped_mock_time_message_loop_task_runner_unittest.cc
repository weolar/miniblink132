...
public:
    ...
    ScopedMockTimeMessageLoopTaskRunnerTest(const ScopedMockTimeMessageLoopTaskRunnerTest&) = delete;
    ScopedMockTimeMessageLoopTaskRunnerTest& operator=(const ScopedMockTimeMessageLoopTaskRunnerTest&) = delete;
...
{
    auto scoped_task_runner_ = std::make_unique<ScopedMockTimeMessageLoopTaskRunner>();
...
TEST_F(ScopedMockTimeMessageLoopTaskRunnerTest, IncompleteTasksAreCopiedToPreviousTaskRunnerAfterDestruction)
{
    auto scoped_task_runner_ = std::make_unique<ScopedMockTimeMessageLoopTaskRunner>();
    ...
    GetCurrentTaskRunner()->PostDelayedTask(FROM_HERE, std::move(task_1), task_1_delay);
    GetCurrentTaskRunner()->PostDelayedTask(FROM_HERE, std::move(task_2), task_2_delay);
    GetCurrentTaskRunner()->PostDelayedTask(FROM_HERE, std::move(task_10), task_10_delay);
    GetCurrentTaskRunner()->PostDelayedTask(FROM_HERE, std::move(task_11), task_11_delay);

    scoped_task_runner_->task_runner()->FastForwardBy(step_time_by);

    scoped_task_runner_.reset();

    base::circular_deque<TestPendingTask> pending_tasks = original_task_runner()->TakePendingTasks();
