...
void SequencedTaskTracker::PostWrappedNonNestableTask(SequencedTaskRunner* task_runner, OnceClosure task)
{
    AutoLock event_lock(lock_);
    const int post_i = next_post_i_++;
    auto wrapped_task = BindOnce(&SequencedTaskTracker::RunTask, this, std::move(task), post_i);
    task_runner->PostNonNestableTask(FROM_HERE, std::move(wrapped_task));
    TaskPosted(post_i);
}

void SequencedTaskTracker::PostWrappedNestableTask(SequencedTaskRunner* task_runner, OnceClosure task)
{
    AutoLock event_lock(lock_);
    const int post_i = next_post_i_++;
    auto wrapped_task = BindOnce(&SequencedTaskTracker::RunTask, this, std::move(task), post_i);
...
void SequencedTaskTracker::PostWrappedDelayedNonNestableTask(SequencedTaskRunner* task_runner, OnceClosure task, TimeDelta delay)
{
    AutoLock event_lock(lock_);
    const int post_i = next_post_i_++;
    auto wrapped_task = BindOnce(&SequencedTaskTracker::RunTask, this, std::move(task), post_i);
    task_runner->PostNonNestableDelayedTask(FROM_HERE, std::move(wrapped_task), delay);
    TaskPosted(post_i);
}

void SequencedTaskTracker::PostNonNestableTasks(SequencedTaskRunner* task_runner, int task_count)
{
...
// they were recorded.
std::vector<int> GetEventTypeOrder(const std::vector<TaskEvent>& events, TaskEvent::Type type)
{
...
std::vector<TaskEvent::Type> GetEventsForTask(const std::vector<TaskEvent>& events, int task_i)
{
...
// per task.
::testing::AssertionResult CheckEventOrdersForEachTask(const std::vector<TaskEvent>& events, int task_count)
{
    ...
    for (int i = 0; i < task_count; ++i) {
        ...
        if (task_events != expected_order) {
            return ::testing::AssertionFailure() << "Events for task " << i << " are out of order; expected: " << ::testing::PrintToString(expected_order)
...
// tasks.
::testing::AssertionResult CheckNoTaskRunsOverlap(const std::vector<TaskEvent>& events)
{
    ...
    for (event = events.begin(); event != events.end(); ++event) {
        ...
        if (current_task_i == -1) { // Not inside a START, END pair.
            ...
            if (interleaved_task_detected) {
                return ::testing::AssertionFailure() << "Found event " << ::testing::PrintToString(*event) << " between START and END events for task "
                                                     << current_task_i << "; event dump: " << ::testing::PrintToString(events);
            }
        }

        if (spurious_event_found) {
            const int event_i = event - events.begin();
            return ::testing::AssertionFailure() << "Spurious event " << ::testing::PrintToString(*event) << " at position " << event_i
...
::testing::AssertionResult CheckNonNestableInvariants(const std::vector<TaskEvent>& events, int task_count)
{
    ...
    if (start_order != post_order) {
        return ::testing::AssertionFailure() << "Expected START order (which equals actual POST order): \n"
                                             << ::testing::PrintToString(post_order) << "\n Actual START order:\n"
                                             << ::testing::PrintToString(start_order);
    }

    if (end_order != post_order) {
        return ::testing::AssertionFailure() << "Expected END order (which equals actual POST order): \n"
                                             << ::testing::PrintToString(post_order) << "\n Actual END order:\n"
