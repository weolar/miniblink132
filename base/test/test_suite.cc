...
public:
    ...
    {
        ASSERT_FALSE(IsMarkedMaybe(test_info)) << "Probably the OS #ifdefs don't include all of the necessary "
...
public:
    ...
    ResetCommandLineBetweenTests& operator=(const ResetCommandLineBetweenTests&) = delete;
...
public:
    ...
    {
        DCHECK_EQ(feature_list_set_before_test_, FeatureList::GetInstance()) << " in test " << test.test_suite_name() << "." << test.name();
        DCHECK_EQ(thread_pool_set_before_test_, ThreadPoolInstance::Get()) << " in test " << test.test_suite_name() << "." << test.name();
    ...
    {
        DCHECK_EQ(feature_list_set_before_suite_, FeatureList::GetInstance()) << " in suite " << test_suite.name();
        DCHECK_EQ(thread_pool_set_before_suite_, ThreadPoolInstance::Get()) << " in suite " << test_suite.name();
        feature_list_set_before_suite_ = nullptr;
        thread_pool_set_before_suite_ = nullptr;
    }

private:
    raw_ptr<FeatureList, DanglingUntriaged> feature_list_set_before_test_ = nullptr;
...
#else
    constexpr auto kLoggingDest = logging::LOG_TO_SYSTEM_DEBUG_LOG | logging::LOG_TO_STDERR;
#endif
...
void InvalidParameter(const wchar_t* expression, const wchar_t* function, const wchar_t* file, unsigned int line, uintptr_t reserved)
{
...
std::string GetStackTraceMessage()
{
    ...
    // value of interest.
    auto filter_switch = CommandLine::ForCurrentProcess()->GetSwitchValueNative("gtest_filter");
    return StrCat({
        "Stack trace suppressed; retry with `--", switches::kWithDeathTestStackTraces, " --gtest_filter=",
#if BUILDFLAG(IS_WIN)
...
int RunUnitTestsUsingBaseTestSuite(int argc, char** argv)
{
    ...
    return LaunchUnitTests(argc, argv, BindOnce(&TestSuite::Run, Unretained(&test_suite)));
}
...
int TestSuite::Run()
{
    ...
    std::string client_func = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kTestChildProcess);
    ...
    ::partition_alloc::ChangeMemoryTaggingModeForCurrentThread(::partition_alloc::TagViolationReportingMode::kSynchronous);
...
void TestSuite::UnitTestAssertHandler(const char* file, int line, std::string_view summary, std::string_view stack_trace)
{
#if BUILDFLAG(IS_ANDROID)
    ...
    if (test_info) {
        LOG(ERROR) << "Currently running: " << test_info->test_suite_name() << "." << test_info->name();
        fflush(stderr);
    }
#endif // BUILDFLAG(IS_ANDROID)
...
void TestSuite::Initialize()
{
...
#if GTEST_HAS_DEATH_TEST
    if (::testing::internal::InDeathTestChild() && !CommandLine::ForCurrentProcess()->HasSwitch(switches::kWithDeathTestStackTraces)) {
        ...
        debug::StackTrace::SuppressStackTracesWithMessageForTesting(GetStackTraceMessage());
    }
#endif
    ...
    if (!debug::BeingDebugged() && !command_line->HasSwitch("show-error-dialogs")) {
        SuppressErrorDialogs();
        debug::SetSuppressDebugUI(true);
        assert_handler_ = std::make_unique<logging::ScopedLogAssertHandler>(BindRepeating(&TestSuite::UnitTestAssertHandler, Unretained(this)));
    }
...
void TestSuite::AddTestLauncherResultPrinter()
{
    ...
    if (!CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestLauncherOutput)) {
        return;
    }
    ...
    if (PathExists(output_path)) {
        LOG(WARNING) << "Test launcher output path " << output_path.AsUTF8Unsafe() << " exists. Not adding test launcher result printer.";
        return;
    }

    printer_ = new XmlUnitTestResultPrinter;
    CHECK(printer_->Initialize(output_path)) << "Output path is " << output_path.AsUTF8Unsafe() << " and PathExists(output_path) is "
