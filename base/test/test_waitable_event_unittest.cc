...
TEST(TestWaitableEvent, NoBlockingCall)
{
    ...
    ThreadPool::PostTask(FROM_HERE, {}, BindOnce(&WaitableEvent::Signal, Unretained(&test_waitable_event)));
...
TEST(TestWaitableEvent, WaitingInPoolDoesntRequireAllowance)
{
    ...
    ThreadPool::PostTask(FROM_HERE, {}, BindOnce(&WaitableEvent::Wait, Unretained(&test_waitable_event)));
...
TEST(TestWaitableEvent, CanBindEitherType)
{
    ...
    ThreadPool::PostTask(FROM_HERE, {}, BindOnce(&TestWaitableEvent::Signal, Unretained(&test_waitable_event)));
    test_waitable_event.Wait();
}
