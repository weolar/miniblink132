...
TestTraceProcessorImpl::TestTraceProcessorImpl()
{
    ...
    trace_processor_ = perfetto::trace_processor::TraceProcessor::CreateInstance(*config_);
}

TestTraceProcessorImpl::~TestTraceProcessorImpl() = default;

TestTraceProcessorImpl::TestTraceProcessorImpl(TestTraceProcessorImpl&& other) = default;
TestTraceProcessorImpl& TestTraceProcessorImpl::operator=(TestTraceProcessorImpl&& other) = default;

QueryResultOrError TestTraceProcessorImpl::ExecuteQuery(const std::string& sql) const
{
...
absl::Status TestTraceProcessorImpl::ParseTrace(const std::vector<char>& raw_trace)
{
    auto status
        = trace_processor_->Parse(perfetto::trace_processor::TraceBlobView(perfetto::trace_processor::TraceBlob::CopyFrom(raw_trace.data(), raw_trace.size())));
...
absl::Status TestTraceProcessorImpl::OverrideSqlModule(const std::string& module_name, const TestTraceProcessorImpl::PerfettoSQLModule& module)
{
    auto status = trace_processor_->RegisterSqlModule({ module_name, module, true });
    return status.ok() ? absl::OkStatus() : absl::UnknownError(status.message());
}
