...
std::optional<FilePath> GetInMemoryContentUriFromCacheDirFilePath(const FilePath& path)
{
    base::FilePath cache_dir;
    if (!base::android::GetCacheDirectory(&cache_dir)) {
        return std::nullopt;
...
std::optional<FilePath> GetInMemoryContentTreeUriFromCacheDirDirectory(const FilePath& path)
{
    base::FilePath cache_dir;
    if (!base::android::GetCacheDirectory(&cache_dir)) {
        return std::nullopt;
    ...
    base::FilePath uri("content://org.chromium.native_test.docprov/tree/" + base::EscapeAllExceptUnreserved(document_id.value()));
    return uri;
}
