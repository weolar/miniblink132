...
{
    return std::make_unique<JavaHandlerThread>(nullptr, Java_JavaHandlerThreadHelpers_testAndGetJavaHandlerThread(jni_zero::AttachCurrentThread()));
}
...
// static
bool JavaHandlerThreadHelpers::IsExceptionTestException(ScopedJavaLocalRef<jthrowable> exception)
{
    JNIEnv* env = jni_zero::AttachCurrentThread();
