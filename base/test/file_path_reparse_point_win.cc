...
std::optional<base::test::FilePathReparsePoint> FilePathReparsePoint::Create(const FilePath& source, const FilePath& target)
{
...
FilePathReparsePoint& FilePathReparsePoint::operator=(FilePathReparsePoint&& other)
{
    dir_ = std::move(other.dir_);
    created_ = std::exchange(other.created_, false);
    return *this;
...
FilePathReparsePoint::FilePathReparsePoint(const FilePath& source, const FilePath& target)
{
    dir_.Set(::CreateFile(source.value().c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
...
bool FilePathReparsePoint::SetReparsePoint(HANDLE source, const FilePath& target_path)
{
...
bool FilePathReparsePoint::DeleteReparsePoint(HANDLE source)
{
    ...
    data.ReparseTag = 0xa0000003;
    if (!DeviceIoControl(source, FSCTL_DELETE_REPARSE_POINT, &data, 8, NULL, 0, &returned, NULL)) {
        return false;
    }
    return true;
}
