...
void DeleteStaleTestKeys(const base::Time& now, const std::wstring& test_key_root)
{
    base::win::RegKey test_root_key;
    if (test_root_key.Open(HKEY_CURRENT_USER, test_key_root.c_str(), KEY_ALL_ACCESS) != ERROR_SUCCESS) {
        // This will occur on first-run, but is harmless.
        return;
    }

    base::win::RegistryKeyIterator iterator_test_root_key(HKEY_CURRENT_USER, test_key_root.c_str());
    for (; iterator_test_root_key.Valid(); ++iterator_test_root_key) {
        ...
        std::vector<std::u16string_view> tokens
            = base::SplitStringPiece(base::AsStringPiece16(key_name), kTimestampDelimiter, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (tokens.empty())
            continue;
...
std::wstring GenerateTempKeyPath(const std::wstring& test_key_root, const base::Time& timestamp)
{
    return base::AsWString(base::StrCat({ base::AsStringPiece16(test_key_root), u"\\", base::NumberToString16(timestamp.ToInternalValue()), kTimestampDelimiter,
        base::ASCIIToUTF16(base::Uuid::GenerateRandomV4().AsLowercaseString()) }));
}

} // namespace

RegistryOverrideManager::ScopedRegistryKeyOverride::ScopedRegistryKeyOverride(HKEY override, const std::wstring& key_path)
    : override_(override)
    , key_path_(key_path)
{
}
...
RegistryOverrideManager::ScopedRegistryKeyOverride::~ScopedRegistryKeyOverride()
{
    ...
    base::win::RegKey(HKEY_CURRENT_USER, L"", KEY_QUERY_VALUE).DeleteKey(key_path_.c_str());
}
...
RegistryOverrideManager::RegistryOverrideManager(const base::Time& timestamp, const std::wstring& test_key_root)
...
void RegistryOverrideManager::OverrideRegistry(HKEY override, std::wstring* override_path)
{
    CHECK(override != HKEY_LOCAL_MACHINE || g_hklm_override_allowed) << "Use of RegistryOverrideManager to override HKLM is not permitted in "
                                                                        "this environment.";

    std::wstring key_path = GenerateTempKeyPath(test_key_root_, timestamp_);

    base::win::RegKey temp_key;
    ASSERT_EQ(ERROR_SUCCESS, temp_key.Create(HKEY_CURRENT_USER, key_path.c_str(), KEY_ALL_ACCESS));
    ASSERT_EQ(ERROR_SUCCESS, ::RegOverridePredefKey(override, temp_key.Handle()));

    overrides_.push_back(std::make_unique<ScopedRegistryKeyOverride>(override, key_path));
    if (override_path)
        override_path->assign(key_path);
}

void RegistryOverrideManager::SetAllowHKLMRegistryOverrideForIntegrationTests(bool allow)
{
...
{
    return GenerateTempKeyPath(kTempTestKeyPath, base::subtle::TimeNowIgnoringOverride());
}
