...
{
    int min_log_level = 0;
    EXPECT_EQ(0, VlogInfo(std::string(), std::string(), &min_log_level).GetVlogLevel("test1"));
    EXPECT_EQ(0, VlogInfo("0", std::string(), &min_log_level).GetVlogLevel("test2"));
    EXPECT_EQ(0, VlogInfo("blah", std::string(), &min_log_level).GetVlogLevel("test3"));
    EXPECT_EQ(0, VlogInfo("0blah1", std::string(), &min_log_level).GetVlogLevel("test4"));
    EXPECT_EQ(1, VlogInfo("1", std::string(), &min_log_level).GetVlogLevel("test5"));
    EXPECT_EQ(5, VlogInfo("5", std::string(), &min_log_level).GetVlogLevel("test6"));
}
...
TEST(VlogTest, VmoduleOrderSample)
{
    ...
    const char kVModuleSwitch[] = "profile=2,icon_loader=1,browser_*=3,*/chromeos/*=4";
    int min_log_level = 0;
    VlogInfo vlog_info(std::string(), kVModuleSwitch, &min_log_level);
...
TEST(VlogTest, WithSwitches)
{
    ...
    std::unique_ptr<VlogInfo> vlog_info_with_switches(vlog_info.WithSwitches("bar=2"));
