...
public:
    ...
    static Location CreateForTesting(const char* function_name, const char* file_name, int line_number, const void* program_counter)
    {
    ...
    friend std::weak_ordering operator<=>(const Location& lhs, const Location& rhs)
    {
        return lhs.program_counter_ <=> rhs.program_counter_;
    }
