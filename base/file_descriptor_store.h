...
public:
    ...
    struct Descriptor {
        ...
        Descriptor(const std::string& key, base::ScopedFD fd, base::MemoryMappedFile::Region region);
    ...
    base::ScopedFD TakeFD(const std::string& key, base::MemoryMappedFile::Region* region);

    // Gets a descriptor given a key. Returns an empty ScopedFD on error.
    base::ScopedFD MaybeTakeFD(const std::string& key, base::MemoryMappedFile::Region* region);
    ...
    void Set(const std::string& key, base::ScopedFD fd, base::MemoryMappedFile::Region region);
