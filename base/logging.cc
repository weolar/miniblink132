...
std::unique_ptr<VlogInfo> VlogInfoFromCommandLine()
{
    ...
    return std::make_unique<VlogInfo>(command_line->GetSwitchValueASCII(switches::kV), command_line->GetSwitchValueASCII(switches::kVModule), &g_min_log_level);
}
...
static_assert(LOGGING_NUM_SEVERITIES == std::size(log_severity_names), "Incorrect number of log_severity_names");
...
uint64_t TickCount()
{
    ...
    return static_cast<uint64_t>(zx_clock_get_monotonic() / static_cast<zx_time_t>(base::Time::kNanosecondsPerMicrosecond));
    ...
    uint64_t absolute_micro = static_cast<uint64_t>(ts.tv_sec) * 1000000 + static_cast<uint64_t>(ts.tv_nsec) / 1000;
...
bool InitializeLogFileHandle()
{
    ...
    g_log_file
        = CreateFile(g_log_file_name->c_str(), FILE_APPEND_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
...
void SetLogFatalCrashKey(LogMessage* log_message)
{
    ...
    static auto* const crash_key = base::debug::AllocateCrashKeyString("LOG_FATAL", base::debug::CrashKeySize::Size1024);
    base::debug::SetCrashKeyString(crash_key, log_message->BuildCrashString());

#endif // !BUILDFLAG(IS_NACL)
}

std::string BuildCrashString(const char* file, int line, const char* message_without_prefix)
{
...
void TraceLogMessage(const char* file, int line, const std::string& message)
{
    TRACE_EVENT_INSTANT("log", "LogMessage", [&](perfetto::EventContext ctx) {
        ...
        log->set_source_location_iid(
            base::trace_event::InternedSourceLocation::Get(&ctx, base::trace_event::TraceSourceLocation(/*function_name=*/nullptr, file, line)));
        log->set_body_iid(base::trace_event::InternedLogMessage::Get(&ctx, message));
    });
}
...
bool BaseInitLoggingImpl(const LoggingSettings& settings)
{
    ...
    CHECK(!settings.log_file_path.empty(), base::NotFatalUntil::M127) << "LOG_TO_FILE set but no log_file_path!";
...
int GetVlogLevelHelper(const char* file, size_t N)
{
    ...
    return vlog_info ? vlog_info->GetVlogLevel(std::string_view(file, N - 1)) : GetVlogVerbosity();
}

void SetLogItems(bool enable_process_id, bool enable_thread_id, bool enable_timestamp, bool enable_tickcount)
{
...
[[noreturn]] void AbslAbortHook(const char* file, int line, const char* buf_start, const char* prefix_end, const char* buf_end)
{
    ...
    LogMessageFatal(file, line, LOGGING_FATAL).stream() << "Check failed: false. " << prefix_end;
}
...
void DisplayDebugMessageInDialog(const std::string& str)
{
    ...
    if (base::win::IsUser32AndGdi32Available()) {
        MessageBoxW(nullptr, base::as_wcstr(base::UTF8ToUTF16(str)), L"Fatal error", MB_OK | MB_ICONHAND | MB_TOPMOST);
    } else {
...
void LogMessage::Flush()
{
...
#if !defined(OFFICIAL_BUILD) && !BUILDFLAG(IS_NACL) && !defined(__UCLIBC__) && !BUILDFLAG(IS_AIX)
    ...
    if (severity_ == LOGGING_FATAL && !base::debug::BeingDebugged()) {
        ...
        if (task && task->ipc_hash) {
            stream_ << "IPC message handler context: " << base::StringPrintf("0x%08X", task->ipc_hash) << std::endl;
        }
    ...
    if (g_log_message_handler && g_log_message_handler(severity_, file_, line_, message_start_, str_newline)) {
    ...
    if ((g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0) {
        ...
        if (log_to_system) {
            ...
            std::string main_bundle_id = main_bundle_id_cf ? base::SysCFStringRefToUTF8(main_bundle_id_cf) : std::string("");

            const class OSLog {
            public:
                explicit OSLog(const char* subsystem)
                    : os_log_(subsystem ? os_log_create(subsystem, "chromium_logging") : OS_LOG_DEFAULT)
                {
                }
        ...
        // truncating the log.
        std::vector<std::string> lines = base::SplitString(str_newline, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
        ...
        GetScopedFxLogger().LogMessage(file_, static_cast<uint32_t>(line_), message.substr(0, message.size() - 1), severity_);
...
#if BUILDFLAG(IS_WIN)
            DWORD num_written;
            WriteFile(g_log_file, static_cast<const void*>(str_newline.c_str()), static_cast<DWORD>(str_newline.length()), &num_written, nullptr);
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
            std::ignore = fwrite(str_newline.data(), str_newline.size(), 1, g_log_file);
            fflush(g_log_file);
#else
#error Unsupported platform
#endif
...
{
    return logging::BuildCrashString(file(), line(), str().c_str() + message_start_);
}
...
void LogMessage::Init(const char* file, int line)
{
    ...
    if (g_log_format == LogFormat::LOG_FORMAT_SYSLOG) {
        InitWithSyslogPrefix(
            filename, line, TickCount(), log_severity_name(severity_), g_log_prefix, g_log_process_id, g_log_thread_id, g_log_timestamp, g_log_tickcount);
    } else
#endif // BUILDFLAG(IS_CHROMEOS)
    {
        ...
        if (g_log_timestamp) {
            ...
            stream_ << std::setfill('0') << std::setw(2) << local_time.wMonth << std::setw(2) << local_time.wDay << '/' << std::setw(2) << local_time.wHour
                    << std::setw(2) << local_time.wMinute << std::setw(2) << local_time.wSecond << '.' << std::setw(3) << local_time.wMilliseconds << ':';
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
            timeval tv;
            ...
            stream_ << std::setfill('0') << std::setw(2) << 1 + tm_time->tm_mon << std::setw(2) << tm_time->tm_mday << '/' << std::setw(2) << tm_time->tm_hour
                    << std::setw(2) << tm_time->tm_min << std::setw(2) << tm_time->tm_sec << '.' << std::setw(6) << tv.tv_usec << ':';
#else
#error Unsupported platform
#endif
        }
...
void LogMessage::HandleFatal(size_t stack_start, const std::string& str_newline) const
{
    ...
    if (!GetLogAssertHandlerStack().empty()) {
        ...
        if (log_assert_handler) {
            ...
            log_assert_handler.Run(file_, line_, newline_view.substr(message_start_, stack_start - message_start_), newline_view.substr(stack_start));
        }
    } else {
...
BASE_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    DWORD len = ::FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, error_code, 0,
    ...
    if (len) {
        ...
        return base::UTF16ToUTF8(base::CollapseWhitespace(message, true)) + base::StringPrintf(" (0x%lX)", error_code);
    }
    return base::StringPrintf("Error (0x%lX) while retrieving error. (0x%lX)", GetLastError(), error_code);
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
...
HANDLE DuplicateLogFileHandle()
{
    ...
    if (!(g_logging_destination & LOG_TO_FILE) || !g_log_file || g_log_file == INVALID_HANDLE_VALUE) {
        return nullptr;
    }
    HANDLE duplicate = nullptr;
    if (!::DuplicateHandle(::GetCurrentProcess(), g_log_file, ::GetCurrentProcess(), &duplicate, 0,
...
ScopedLoggingSettings::~ScopedLoggingSettings()
{
    ...
    CHECK(InitLogging({
        .logging_dest = logging_destination_, .log_file_path = log_file_name_,
#if BUILDFLAG(IS_CHROMEOS)
    ...
    SetLogItems(enable_process_id_, enable_thread_id_, enable_timestamp_, enable_tickcount_);
...
VlogInfo* ScopedVmoduleSwitches::CreateVlogInfoWithSwitches(const std::string& vmodule_switch)
{
...
void ScopedVmoduleSwitches::InitWithSwitches(const std::string& vmodule_switch)
{
