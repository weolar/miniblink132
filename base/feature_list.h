...
#define BASE_DECLARE_FEATURE(kFeature) extern constinit const base::Feature kFeature
...
// header files.
#define BASE_FEATURE(feature, name, default_state) constinit const base::Feature feature(name, default_state, base::internal::FeatureMacroHandshake::kSecret)
...
// types directly.
#define BASE_DECLARE_FEATURE_PARAM(T, feature_object_name) extern constinit const base::FeatureParam<T> feature_object_name
...
// files.
#define BASE_FEATURE_PARAM(T, feature_object_name, feature, name, default_value)                                                                               \
    namespace field_trial_params_internal {                                                                                                                    \
    T GetFeatureParamWithCacheFor##feature_object_name(const base::FeatureParam<T>* feature_param)                                                             \
    {                                                                                                                                                          \
    ...
    constinit const base::FeatureParam<T> feature_object_name(                                                                                                 \
        feature, name, default_value, &field_trial_params_internal::GetFeatureParamWithCacheFor##feature_object_name)
...
#define BASE_FEATURE_ENUM_PARAM(T, feature_object_name, feature, name, default_value, options)                                                                 \
    namespace field_trial_params_internal {                                                                                                                    \
    T GetFeatureParamWithCacheFor##feature_object_name(const base::FeatureParam<T>* feature_param)                                                             \
    {                                                                                                                                                          \
        static const T param = feature_param->GetWithoutCache();                                                                                               \
    ...
    constinit const base::FeatureParam<T> feature_object_name(                                                                                                 \
        feature, name, default_value, options, &field_trial_params_internal::GetFeatureParamWithCacheFor##feature_object_name)
...
struct BASE_EXPORT LOGICALLY_CONST Feature {
    constexpr Feature(const char* name, FeatureState default_state, internal::FeatureMacroHandshake)
        : name(name)
        , default_state(default_state)
    {
#if BUILDFLAG(ENABLE_BANNED_BASE_FEATURE_PREFIX)
        if (std::string_view(name).find(BUILDFLAG(BANNED_BASE_FEATURE_PREFIX)) == 0) {
            LOG(FATAL) << "Invalid feature name " << name << " starts with " << BUILDFLAG(BANNED_BASE_FEATURE_PREFIX);
        }
...
public:
    ...
    public:
        ...
        FeatureList::OverrideState GetOverrideStateByFeatureName(std::string_view feature_name);

        // Look up the feature, and, if present, populate |params|.
        // See GetFieldTrialParams in field_trial_params.h for more documentation.
        bool GetParamsByFeatureName(std::string_view feature_name, std::map<std::string, std::string>* params);
    ...
    using FeatureOverrideInfo = std::pair<const std::reference_wrapper<const Feature>, OverrideState>;
    ...
    void InitFromCommandLine(const std::string& enable_features, const std::string& disable_features);
    ...
    bool IsFeatureOverriddenFromCommandLine(const std::string& feature_name) const;

    // Returns true if the state |feature_name| has been overridden by
    // |InitFromCommandLine()| and the state matches |state|.
    bool IsFeatureOverriddenFromCommandLine(const std::string& feature_name, OverrideState state) const;
    ...
    void AssociateReportingFieldTrial(const std::string& feature_name, OverrideState for_overridden_state, FieldTrial* field_trial);
    ...
    void RegisterFieldTrialOverride(const std::string& feature_name, OverrideState override_state, FieldTrial* field_trial);
    ...
    void RegisterExtraFeatureOverrides(const std::vector<FeatureOverrideInfo>& extra_overrides);
    ...
    void GetFeatureOverrides(std::string* enable_overrides, std::string* disable_overrides, bool include_group_names = false) const;
    ...
    void GetCommandLineFeatureOverrides(std::string* enable_overrides, std::string* disable_overrides) const;
    ...
    base::FieldTrial* GetAssociatedFieldTrialByFeatureName(std::string_view name) const;
    ...
    static std::vector<std::string_view> SplitFeatureListString(std::string_view input);
    ...
    static bool ParseEnableFeatureString(
    ...
    static bool InitInstance(const std::string& enable_features, const std::string& disable_features);
    ...
    static bool InitInstance(const std::string& enable_features, const std::string& disable_features, const std::vector<FeatureOverrideInfo>& extra_overrides);
    ...
    static void SetEarlyAccessInstance(std::unique_ptr<FeatureList> instance, base::flat_set<std::string> allowed_feature_names);
    ...
    static void VisitFeaturesAndParams(FeatureVisitor& visitor, std::string_view filter_prefix = "");
...
private:
    ...
    const base::FeatureList::OverrideEntry* GetOverrideEntryByFeatureName(std::string_view name) const;
    ...
    std::optional<bool> IsFeatureEnabledIfOverridden(const Feature& feature) const;
    ...
    OverrideState GetOverrideStateByFeatureName(std::string_view feature_name) const;
    ...
    void RegisterOverridesFromCommandLine(const std::string& feature_list, OverrideState overridden_state);
    ...
    // will not be changed.
    void RegisterOverride(std::string_view feature_name, OverrideState overridden_state, FieldTrial* field_trial);
    ...
    void GetFeatureOverridesImpl(std::string* enable_overrides, std::string* disable_overrides, bool command_line_only, bool include_group_name = false) const;
    ...
    mutable std::map<std::string, const Feature*, std::less<>> feature_identity_tracker_ GUARDED_BY(feature_identity_tracker_lock_);
