...
namespace base {
template <class Char> class basic_cstring_view;
}
...
template <typename T>
    requires(base::internal::SupportsOstreamOperator<const T&> && !std::is_function_v<T> && !std::is_pointer_v<T>)
...
#else
    const void* vp = const_cast<const void*>(reinterpret_cast<const volatile void*>(v));
#endif
...
template <typename T>
    requires(!base::internal::SupportsOstreamOperator<const T&> && base::internal::SupportsToString<const T&>)
inline char* CheckOpValueStr(const T& v)
{
...
template <typename T>
    requires(!base::internal::SupportsOstreamOperator<const T&> && std::is_enum_v<T>)
inline char* CheckOpValueStr(const T& v)
{
...
BASE_EXPORT char* CreateCheckOpLogMessageString(const char* expr_str, char* v1_str, char* v2_str);
...
#define CHECK_OP_FUNCTION_IMPL(check_failure_function, name, op, val1, val2, ...)                                                                              \
    ...
    case 0:                                                                                                                                                    \
    default:                                                                                                                                                   \
        if (char* const message_on_fail = ::logging::Check##name##Impl((val1), (val2), #val1 " " #op " " #val2); !message_on_fail)                             \
            ;                                                                                                                                                  \
        else                                                                                                                                                   \
...
#define CHECK_OP(name, op, val1, val2, ...)                                                                                                                    \
    BASE_IF(BASE_IS_EMPTY(__VA_ARGS__), CHECK((val1)op(val2)), CHECK_OP_FUNCTION_IMPL(::logging::CheckError::CheckOp, name, op, val1, val2, __VA_ARGS__))

#else

#define CHECK_OP(name, op, val1, val2, ...) CHECK_OP_FUNCTION_IMPL(::logging::CheckError::CheckOp, name, op, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                                                                                                         \
    ...
    constexpr char* Check##name##Impl(const T& v1, const U& v2, const char* expr_str)                                                                          \
    {                                                                                                                                                          \
        if (ANALYZER_ASSUME_TRUE(v1 op v2)) [[likely]]                                                                                                         \
            return nullptr;                                                                                                                                    \
        return CreateCheckOpLogMessageString(expr_str, CheckOpValueStr(v1), CheckOpValueStr(v2));                                                              \
    }                                                                                                                                                          \
    template <typename T, typename U>                                                                                                                          \
        requires(std::is_fundamental_v<T> && std::is_fundamental_v<U>)                                                                                         \
...
#define CHECK_EQ(val1, val2, ...) CHECK_OP(EQ, ==, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
#define CHECK_NE(val1, val2, ...) CHECK_OP(NE, !=, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
#define CHECK_LE(val1, val2, ...) CHECK_OP(LE, <=, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
#define CHECK_LT(val1, val2, ...) CHECK_OP(LT, <, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
#define CHECK_GE(val1, val2, ...) CHECK_OP(GE, >=, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
#define CHECK_GT(val1, val2, ...) CHECK_OP(GT, >, val1, val2 __VA_OPT__(, ) __VA_ARGS__)
// clang-format on

#if DCHECK_IS_ON()

#define DCHECK_OP(name, op, val1, val2) CHECK_OP_FUNCTION_IMPL(::logging::CheckError::DCheckOp, name, op, val1, val2)

#else

// Don't do any evaluation but still reference the same stuff as when enabled.
#define DCHECK_OP(name, op, val1, val2) EAT_CHECK_STREAM_PARAMS((::logging::CheckOpValueStr(val1), ::logging::CheckOpValueStr(val2), (val1)op(val2)))
...
#define DUMP_WILL_BE_CHECK_OP(name, op, val1, val2) CHECK_OP_FUNCTION_IMPL(::logging::CheckError::DumpWillBeCheckOp, name, op, val1, val2)

#define DUMP_WILL_BE_CHECK_EQ(val1, val2) DUMP_WILL_BE_CHECK_OP(EQ, ==, val1, val2)
#define DUMP_WILL_BE_CHECK_NE(val1, val2) DUMP_WILL_BE_CHECK_OP(NE, !=, val1, val2)
#define DUMP_WILL_BE_CHECK_LE(val1, val2) DUMP_WILL_BE_CHECK_OP(LE, <=, val1, val2)
#define DUMP_WILL_BE_CHECK_LT(val1, val2) DUMP_WILL_BE_CHECK_OP(LT, <, val1, val2)
#define DUMP_WILL_BE_CHECK_GE(val1, val2) DUMP_WILL_BE_CHECK_OP(GE, >=, val1, val2)
#define DUMP_WILL_BE_CHECK_GT(val1, val2) DUMP_WILL_BE_CHECK_OP(GT, >, val1, val2)
