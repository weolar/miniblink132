...
#define DCHECK_CALLED_ON_VALID_THREAD(name, ...) base::ScopedValidateThreadChecker BASE_UNIQUIFY(scoped_validate_thread_checker_)(name, ##__VA_ARGS__);
...
public:
    ...
    [[nodiscard]] bool CalledOnValidThread(std::unique_ptr<void*> = nullptr) const
    {
        return true;
    }
...
public:
    explicit ScopedValidateThreadChecker(const ThreadChecker& checker) EXCLUSIVE_LOCK_FUNCTION(checker);
    ScopedValidateThreadChecker(const ThreadChecker& checker, std::string_view msg) EXCLUSIVE_LOCK_FUNCTION(checker);

    ScopedValidateThreadChecker(const ScopedValidateThreadChecker&) = delete;
    ScopedValidateThreadChecker& operator=(const ScopedValidateThreadChecker&) = delete;
