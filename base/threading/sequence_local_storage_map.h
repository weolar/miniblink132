...
public:
    ...
    struct BASE_EXPORT ExternalValue {
        ...
        template <class T> void emplace(T* ptr)
        {
            value = static_cast<void*>(ptr);
        }

        template <class T, class Deleter> void Destroy()
        {
            Deleter()(std::addressof(value_as<T>()));
        }

        template <typename T> T& value_as() LIFETIME_BOUND
        {
            return *static_cast<T*>(value);
        }

        template <typename T> const T& value_as() const LIFETIME_BOUND
        {
    ...
    struct BASE_EXPORT alignas(sizeof(void*)) InlineValue {
        ...
        {
            static_assert(sizeof(T) <= sizeof(void*), "Type T is too big for storage inline.");
    ...
    template <class T, class Deleter> static DestructorFunc* MakeExternalDestructor()
    {
        return [](Value* value) { value->external_value.Destroy<T, Deleter>(); };
    }
    template <class T> static DestructorFunc* MakeInlineDestructor()
    {
...
class BASE_EXPORT [[maybe_unused, nodiscard]] ScopedSetSequenceLocalStorageMapForCurrentThread
{
public:
    ScopedSetSequenceLocalStorageMapForCurrentThread(SequenceLocalStorageMap * sequence_local_storage);

    ScopedSetSequenceLocalStorageMapForCurrentThread(const ScopedSetSequenceLocalStorageMapForCurrentThread&) = delete;
    ScopedSetSequenceLocalStorageMapForCurrentThread& operator=(const ScopedSetSequenceLocalStorageMapForCurrentThread&) = delete;
