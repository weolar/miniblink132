...
public:
    ...
    explicit ObjectToDelete(bool* delete_flag)
...
public:
    MockRunsTasksInCurrentSequenceTaskRunner(TestMockTimeTaskRunner::Type type = TestMockTimeTaskRunner::Type::kStandalone)
        : TestMockTimeTaskRunner(type)
    {
    }

    MockRunsTasksInCurrentSequenceTaskRunner(const MockRunsTasksInCurrentSequenceTaskRunner&) = delete;
    MockRunsTasksInCurrentSequenceTaskRunner& operator=(const MockRunsTasksInCurrentSequenceTaskRunner&) = delete;

    void StopAcceptingTasks()
    {
        accepts_tasks_ = false;
    }
    ...
    bool PostDelayedTask(const Location& from_here, OnceClosure task, TimeDelta delay) override
    {
        ...
        return TestMockTimeTaskRunner::PostDelayedTask(from_here, std::move(task), delay);
    }
...
protected:
    ...
    bool PostTaskAndReplyToMockObject(bool task_uses_weak_ptr = false)
    {
        ...
        if (task_uses_weak_ptr) {
            task = BindOnce(&MockObject::Task, mock_object_.GetWeakPtr(), MakeRefCounted<ObjectToDelete>(&delete_task_flag_));
        } else {
            task = BindOnce(&MockObject::Task, Unretained(&mock_object_), MakeRefCounted<ObjectToDelete>(&delete_task_flag_));
        }

        return PostTaskAndReplyImpl([this](const Location& location, OnceClosure task) { return post_runner_->PostTask(location, std::move(task)); }, FROM_HERE,
            std::move(task), BindOnce(&MockObject::Reply, Unretained(&mock_object_), MakeRefCounted<ObjectToDelete>(&delete_reply_flag_)));
    }

    void ExpectPostTaskAndReplyToMockObjectSucceeds(bool task_uses_weak_ptr = false)
    {
    ...
    scoped_refptr<MockRunsTasksInCurrentSequenceTaskRunner> post_runner_ = MakeRefCounted<MockRunsTasksInCurrentSequenceTaskRunner>();
    scoped_refptr<MockRunsTasksInCurrentSequenceTaskRunner> reply_runner_
        = MakeRefCounted<MockRunsTasksInCurrentSequenceTaskRunner>(TestMockTimeTaskRunner::Type::kBoundToThread);
...
TEST_F(PostTaskAndReplyImplTest, PostTaskToStoppedTaskRunnerWithoutSequencedContext)
{
    reply_runner_.reset();
