...
class BASE_EXPORT [[nodiscard]] ScopedBlockingCall : public internal::UncheckedScopedBlockingCall {
public:
...
class BASE_EXPORT [[nodiscard]] ScopedBlockingCallWithBaseSyncPrimitives : public UncheckedScopedBlockingCall {
public:
...
using IOJankReportingCallback = RepeatingCallback<void(int janky_intervals_per_minute, int total_janks_per_minute)>;
...
void BASE_EXPORT EnableIOJankMonitoringForProcess(
    IOJankReportingCallback reporting_callback, OnlyObservedThreadsForTest only_observed_threads = OnlyObservedThreadsForTest(false));
