...
SimpleThread::~SimpleThread()
{
    ...
    DCHECK(!options_.joinable || HasBeenJoined()) << "Joinable SimpleThread destroyed without being Join()ed.";
}
...
void SimpleThread::StartAsync()
{
    ...
    start_called_ = true;
    BeforeStart();
    bool success = options_.joinable ? PlatformThread::CreateWithType(options_.stack_size, this, &thread_, options_.thread_type)
                                     : PlatformThread::CreateNonJoinableWithType(options_.stack_size, this, options_.thread_type);
    CHECK(success);
}
...
DelegateSimpleThread::DelegateSimpleThread(Delegate* delegate, const std::string& name, const Options& options)
    : SimpleThread(name, options)
    , delegate_(delegate)
{
    DCHECK(delegate_);
...
DelegateSimpleThreadPool::DelegateSimpleThreadPool(const std::string& name_prefix, size_t num_threads)
...
void DelegateSimpleThreadPool::AddWork(Delegate* delegate, size_t repeat_count)
{
