...
public:
    ...
    [[nodiscard]] bool CalledOnValidThread(std::unique_ptr<debug::StackTrace>* out_bound_at = nullptr) const LOCKS_EXCLUDED(lock_);
...
private:
    ...
    // the time.
    std::unique_ptr<debug::StackTrace> GetBoundAt() const EXCLUSIVE_LOCKS_REQUIRED(lock_);
