...
public:
    ...
    enum class ProcessType { kUnknownProcess = 0, kBrowserProcess = 1, kGPUProcess = 2, kRendererProcess = 3, kUtilityProcess = 4, kMax = kUtilityProcess };

    // Describes the type of a thread for logging purposes.
    enum class ThreadType { kIOThread = 0, kMainThread = 1, kThreadPoolThread = 2, kMax = kThreadPoolThread };
    ...
    static void InitializeOnMainThread(ProcessType process_type, bool emit_crashes);
    ...
    // thread with.
    [[nodiscard]] static ScopedClosureRunner RegisterThread(ThreadType thread_type);
    ...
    // only for testing.
    void SetAfterWaitCallbackForTesting(RepeatingCallback<void(TimeTicks)> callback);
...
private:
    ...
    [[nodiscard]] ScopedClosureRunner RegisterThreadInternal(ThreadType thread_type) LOCKS_EXCLUDED(watch_state_lock_);
    ...
    public:
        ...
        // to Clear().
        void Init(const HangWatchStates& watch_states, base::TimeTicks deadline_ignore_threshold, base::TimeDelta monitoring_period);
    ...
    // locked.
    WatchStateSnapShot GrabWatchStateSnapshotForTesting() const NO_THREAD_SAFETY_ANALYSIS;
    ...
    // and after.
    void DoDumpWithoutCrashing(const WatchStateSnapShot& watch_state_snapshot) EXCLUSIVE_LOCKS_REQUIRED(watch_state_lock_) LOCKS_EXCLUDED(capture_lock_);
    ...
    std::vector<std::unique_ptr<internal::HangWatchState>> watch_states_ GUARDED_BY(watch_state_lock_);

    // Snapshot to be reused across hang captures. The point of keeping it
    // around is reducing allocations during capture.
    WatchStateSnapShot watch_state_snapshot_ GUARDED_BY_CONTEXT(hang_watcher_thread_checker_);
...
public:
    ...
    // only for testing.
    void SetSwitchBitsClosureForTesting(RepeatingCallback<uint64_t(void)> closure);
...
private:
    using TimeTicksInternalRepresentation = std::invoke_result<decltype(&TimeTicks::ToInternalValue), TimeTicks>::type;
...
public:
    ...
    static std::unique_ptr<HangWatchState> CreateHangWatchStateForCurrentThread(HangWatcher::ThreadType thread_type);
    ...
    HangWatcher::ThreadType thread_type() const
    {
        return thread_type_;
    }
