...
TEST(ThreadLocalTest, ThreadLocalOwnedPointerBasic)
{
    ...
    tls_owned_pointer.Set(std::make_unique<SetTrueOnDestruction>(&was_destroyed1));
    EXPECT_FALSE(was_destroyed1);
    EXPECT_TRUE(tls_owned_pointer.Get());

    bool was_destroyed2 = false;
    tls_owned_pointer.Set(std::make_unique<SetTrueOnDestruction>(&was_destroyed2));
...
TEST(ThreadLocalTest, ThreadLocalOwnedPointerFreedOnThreadExit)
{
    ...
    thread.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        tls_owned_pointer.Set(std::make_unique<SetTrueOnDestruction>(&tls_was_destroyed));
        tls_set.Signal();
    }));
...
{
    std::optional<ThreadLocalOwnedPointer<SetTrueOnDestruction>> tls_owned_pointer(std::in_place);
    ...
    thread.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&] {
        tls_owned_pointer->Set(std::make_unique<SetTrueOnDestruction>(&tls_was_destroyed_other));
        tls_set.Signal();
    }));
    ...
    tls_owned_pointer->Set(std::make_unique<SetTrueOnDestruction>(&tls_was_destroyed_main));
...
TEST(ThreadLocalTest, ThreadLocalOwnedPointerMultiThreadedAndStaticStorage)
{
    ...
    for (int i = 0; i < kNumThreads; ++i) {
        threads[i]->task_runner()->PostTask(
            FROM_HERE, BindOnce([](bool* was_destroyed) { tls_owned_pointer.Set(std::make_unique<SetTrueOnDestruction>(was_destroyed)); }, &were_destroyed[i]));
    }
    ...
    tls_owned_pointer.Set(std::make_unique<SetTrueOnDestruction>(&main_thread_was_destroyed));
