...
void ThreadIdNameManager::RegisterThread(PlatformThreadHandle::Handle handle, PlatformThreadId id)
{
    AutoLock locked(lock_);
...
void ThreadIdNameManager::SetName(const std::string& name)
{
    ...
    trace_event::AllocationContextTracker::SetCurrentThreadName(leaked_str->c_str());
}
...
void ThreadIdNameManager::RemoveName(PlatformThreadHandle::Handle handle, PlatformThreadId id)
{
    AutoLock locked(lock_);
    auto handle_to_name_iter = thread_handle_to_interned_name_.find(handle);

    CHECK(handle_to_name_iter != thread_handle_to_interned_name_.end(), base::NotFatalUntil::M125);
    thread_handle_to_interned_name_.erase(handle_to_name_iter);

    auto id_to_handle_iter = thread_id_to_handle_.find(id);
    CHECK(id_to_handle_iter != thread_id_to_handle_.end(), base::NotFatalUntil::M125);
