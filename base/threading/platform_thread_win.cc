...
BASE_FEATURE(kUseThreadPriorityLowest, "UseThreadPriorityLowest", base::FEATURE_DISABLED_BY_DEFAULT);
BASE_FEATURE(kAboveNormalCompositingBrowserWin, "AboveNormalCompositingBrowserWin", base::FEATURE_ENABLED_BY_DEFAULT);
...
void SetNameInternal(PlatformThreadId thread_id, const char* name)
{
    ...
    __try {
        RaiseException(kVCThreadNameException, 0, sizeof(info) / sizeof(ULONG_PTR), reinterpret_cast<ULONG_PTR*>(&info));
...
DWORD __stdcall ThreadFunc(void* params)
{
    ...
    if (thread_params->thread_type != ThreadType::kDefault)
        internal::SetCurrentThreadType(thread_params->thread_type, thread_params->message_pump_type);

    // Retrieve a copy of the thread handle to use as the key in the
    // thread name mapping.
    PlatformThreadHandle::Handle platform_handle;
    BOOL did_dup = DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &platform_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    ...
    if (did_dup) {
        ...
        ThreadIdNameManager::GetInstance()->RegisterThread(scoped_platform_handle.get(), PlatformThread::CurrentId());
    }
    ...
    if (did_dup) {
        ThreadIdNameManager::GetInstance()->RemoveName(scoped_platform_handle.get(), PlatformThread::CurrentId());
    }
...
// is created.
bool CreateThreadInternal(
    size_t stack_size, PlatformThread::Delegate* delegate, PlatformThreadHandle* out_thread_handle, ThreadType thread_type, MessagePumpType message_pump_type)
{
    ...
    if (!thread_handle) {
        ...
        default:
            static auto* last_error_crash_key = debug::AllocateCrashKeyString("create_thread_last_error", debug::CrashKeySize::Size32);
...
void AssertMemoryPriority(HANDLE thread, int memory_priority)
{
    ...
    static const auto get_thread_information_fn
        = reinterpret_cast<decltype(&::GetThreadInformation)>(::GetProcAddress(::GetModuleHandle(L"Kernel32.dll"), "GetThreadInformation"));

    DCHECK(get_thread_information_fn);

    MEMORY_PRIORITY_INFORMATION memory_priority_information = {};
    DCHECK(get_thread_information_fn(thread, ::ThreadMemoryPriority, &memory_priority_information, sizeof(memory_priority_information)));
...
void PlatformThread::Sleep(TimeDelta duration)
{
    ...
    for (TimeTicks now = subtle::TimeTicksNowIgnoringOverride(); now < end; now = subtle::TimeTicksNowIgnoringOverride()) {
...
// static
bool PlatformThread::CreateWithType(
    size_t stack_size, Delegate* delegate, PlatformThreadHandle* thread_handle, ThreadType thread_type, MessagePumpType pump_type_hint)
{
    DCHECK(thread_handle);
    return CreateThreadInternal(stack_size, delegate, thread_handle, thread_type, pump_type_hint);
}
...
// static
bool PlatformThread::CreateNonJoinableWithType(size_t stack_size, Delegate* delegate, ThreadType thread_type, MessagePumpType pump_type_hint)
{
    return CreateThreadInternal(stack_size, delegate, nullptr /* non-joinable */, thread_type, pump_type_hint);
}
...
void PlatformThread::Join(PlatformThreadHandle thread_handle)
{
    ...
    base::internal::ScopedBlockingCallWithBaseSyncPrimitives scoped_blocking_call(FROM_HERE, base::BlockingType::MAY_BLOCK);
...
void SetCurrentThreadPriority(ThreadType thread_type, MessagePumpType pump_type_hint)
{
    if (thread_type == ThreadType::kDisplayCritical && pump_type_hint == MessagePumpType::UI && !g_above_normal_compositing_browser) {
    ...
    case ThreadType::kBackground:
        ...
        // CPU priority.
        desired_priority = g_use_thread_priority_lowest.load(std::memory_order_relaxed) ? THREAD_PRIORITY_LOWEST : THREAD_MODE_BACKGROUND_BEGIN;
        break;
    ...
    [[maybe_unused]] const BOOL cpu_priority_success = ::SetThreadPriority(thread_handle, desired_priority);
    DPLOG_IF(ERROR, !cpu_priority_success) << "Failed to set thread priority to " << desired_priority;

    if (desired_priority == THREAD_MODE_BACKGROUND_BEGIN) {
        // Override the memory priority.
        MEMORY_PRIORITY_INFORMATION memory_priority { .MemoryPriority = MEMORY_PRIORITY_NORMAL };
        [[maybe_unused]] const BOOL memory_priority_success
            = SetThreadInformation(thread_handle, ::ThreadMemoryPriority, &memory_priority, sizeof(memory_priority));
        DPLOG_IF(ERROR, !memory_priority_success) << "Set thread memory priority failed.";
    }
    ...
    if (!g_use_thread_priority_lowest && thread_type == ThreadType::kBackground) {
        ...
        if (PlatformThread::GetCurrentThreadPriorityForTest() != ThreadPriorityForTest::kBackground) {
...
void SetCurrentThreadQualityOfService(ThreadType thread_type)
{
    ...
    THREAD_POWER_THROTTLING_STATE thread_power_throttling_state {
        ...
        .ControlMask = desire_ecoqos ? THREAD_POWER_THROTTLING_EXECUTION_SPEED : 0ul,
        .StateMask = desire_ecoqos ? THREAD_POWER_THROTTLING_EXECUTION_SPEED : 0ul,
    };
    [[maybe_unused]] const BOOL success
        = ::SetThreadInformation(::GetCurrentThread(), ::ThreadPowerThrottling, &thread_power_throttling_state, sizeof(thread_power_throttling_state));
    // Failure is expected on versions of Windows prior to RS3.
    DPLOG_IF(ERROR, !success && win::GetVersion() >= win::Version::WIN10_RS3) << "Failed to set EcoQoS to " << std::boolalpha << desire_ecoqos;
}
...
void SetCurrentThreadTypeImpl(ThreadType thread_type, MessagePumpType pump_type_hint)
{
...
{
    static_assert(THREAD_PRIORITY_IDLE < 0, "THREAD_PRIORITY_IDLE is >= 0 and will incorrectly cause errors.");
    static_assert(THREAD_PRIORITY_LOWEST < 0, "THREAD_PRIORITY_LOWEST is >= 0 and will incorrectly cause errors.");
...
{
    g_use_thread_priority_lowest.store(FeatureList::IsEnabled(kUseThreadPriorityLowest), std::memory_order_relaxed);
    g_above_normal_compositing_browser.store(FeatureList::IsEnabled(kAboveNormalCompositingBrowserWin), std::memory_order_relaxed);
}
