...
std::atomic<HangWatcher::ProcessType> g_hang_watcher_process_type { HangWatcher::ProcessType::kBrowserProcess };
...
void LogStatusHistogram(HangWatcher::ThreadType thread_type, int count, TimeTicks sample_ticks, TimeDelta monitoring_period)
{
    ...
    const HangWatcher::ProcessType process_type = g_hang_watcher_process_type.load(std::memory_order_relaxed);
    switch (process_type) {
    ...
    case HangWatcher::ProcessType::kBrowserProcess:
        ...
        case HangWatcher::ThreadType::kIOThread:
            if (shutting_down) {
                UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.BrowserProcess.IOThread.Shutdown", any_thread_hung);
            } else {
                UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.BrowserProcess.IOThread.Normal", any_thread_hung);
            }
            break;
        case HangWatcher::ThreadType::kMainThread:
            if (shutting_down) {
                UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Shutdown", any_thread_hung);
            } else {
                UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Normal", any_thread_hung);
            }
            break;
    ...
    case HangWatcher::ProcessType::kRendererProcess:
        ...
        case HangWatcher::ThreadType::kIOThread:
            UMA_HISTOGRAM_SPLIT_BY_PROCESS_PRIORITY(
                UMA_HISTOGRAM_BOOLEAN, sample_ticks, monitoring_period, "HangWatcher.IsThreadHung.RendererProcess.IOThread", any_thread_hung);
            break;
        case HangWatcher::ThreadType::kMainThread:
            UMA_HISTOGRAM_SPLIT_BY_PROCESS_PRIORITY(
                UMA_HISTOGRAM_BOOLEAN, sample_ticks, monitoring_period, "HangWatcher.IsThreadHung.RendererProcess.MainThread", any_thread_hung);
            break;
        case HangWatcher::ThreadType::kThreadPoolThread:
            // Not recorded for now.
            break;
    ...
    case HangWatcher::ProcessType::kUtilityProcess:
        ...
        switch (thread_type) {
        case HangWatcher::ThreadType::kIOThread:
            UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.UtilityProcess.IOThread", any_thread_hung);
            break;
        case HangWatcher::ThreadType::kMainThread:
            UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.UtilityProcess.MainThread", any_thread_hung);
            break;
        case HangWatcher::ThreadType::kThreadPoolThread:
            // Not recorded for now.
            break;
...
bool ThreadTypeLoggingLevelGreaterOrEqual(HangWatcher::ThreadType thread_type, LoggingLevel logging_level)
{
    switch (thread_type) {
    case HangWatcher::ThreadType::kIOThread:
        return g_io_thread_log_level.load(std::memory_order_relaxed) >= logging_level;
    case HangWatcher::ThreadType::kMainThread:
        return g_main_thread_log_level.load(std::memory_order_relaxed) >= logging_level;
    case HangWatcher::ThreadType::kThreadPoolThread:
...
BASE_FEATURE(kEnableHangWatcher, "EnableHangWatcher", FEATURE_ENABLED_BY_DEFAULT);
...
constexpr base::FeatureParam<int> kThreadPoolLogLevel { &kEnableHangWatcher, "threadpool_log_level", static_cast<int>(LoggingLevel::kUmaOnly) };

// GPU process.
constexpr base::FeatureParam<int> kGPUProcessIOThreadLogLevel { &kEnableHangWatcher, "gpu_process_io_thread_log_level", static_cast<int>(LoggingLevel::kNone) };
constexpr base::FeatureParam<int> kGPUProcessMainThreadLogLevel { &kEnableHangWatcher, "gpu_process_main_thread_log_level",
    static_cast<int>(LoggingLevel::kNone) };
constexpr base::FeatureParam<int> kGPUProcessThreadPoolLogLevel { &kEnableHangWatcher, "gpu_process_threadpool_log_level",
    static_cast<int>(LoggingLevel::kNone) };

// Renderer process.
constexpr base::FeatureParam<int> kRendererProcessIOThreadLogLevel { &kEnableHangWatcher, "renderer_process_io_thread_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };
constexpr base::FeatureParam<int> kRendererProcessMainThreadLogLevel { &kEnableHangWatcher, "renderer_process_main_thread_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };
constexpr base::FeatureParam<int> kRendererProcessThreadPoolLogLevel { &kEnableHangWatcher, "renderer_process_threadpool_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };

// Utility process.
constexpr base::FeatureParam<int> kUtilityProcessIOThreadLogLevel { &kEnableHangWatcher, "utility_process_io_thread_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };
constexpr base::FeatureParam<int> kUtilityProcessMainThreadLogLevel { &kEnableHangWatcher, "utility_process_main_thread_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };
constexpr base::FeatureParam<int> kUtilityProcessThreadPoolLogLevel { &kEnableHangWatcher, "utility_process_threadpool_log_level",
    static_cast<int>(LoggingLevel::kUmaOnly) };
...
{
    internal::HangWatchState* current_hang_watch_state = HangWatcher::IsEnabled() ? internal::HangWatchState::GetHangWatchStateForCurrentThread() : nullptr;
    ...
    auto [old_flags, old_deadline] = current_hang_watch_state->GetFlagsAndDeadline();
    ...
    const bool hangs_ignored_for_current_scope
        = internal::HangWatchDeadline::IsFlagSet(internal::HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, old_flags);
...
WatchHangsInScope::~WatchHangsInScope()
{
    ...
    // do.
    auto* const state = internal::HangWatchState::GetHangWatchStateForCurrentThread();
    if (!state) {
        return;
    }
...
// static
void HangWatcher::InitializeOnMainThread(ProcessType process_type, bool emit_crashes)
{
    ...
    if (process_type == HangWatcher::ProcessType::kBrowserProcess) {
        ...
        if (emit_crashes) {
            g_io_thread_log_level.store(static_cast<LoggingLevel>(LoggingLevel::kUmaAndCrash), std::memory_order_relaxed);
            g_main_thread_log_level.store(static_cast<LoggingLevel>(LoggingLevel::kUmaAndCrash), std::memory_order_relaxed);
        } else {
            g_io_thread_log_level.store(static_cast<LoggingLevel>(kIOThreadLogLevel.Get()), std::memory_order_relaxed);
            g_main_thread_log_level.store(static_cast<LoggingLevel>(kUIThreadLogLevel.Get()), std::memory_order_relaxed);
        }

        g_threadpool_log_level.store(static_cast<LoggingLevel>(kThreadPoolLogLevel.Get()), std::memory_order_relaxed);
    } else if (process_type == HangWatcher::ProcessType::kGPUProcess) {
        g_threadpool_log_level.store(static_cast<LoggingLevel>(kGPUProcessThreadPoolLogLevel.Get()), std::memory_order_relaxed);
        g_io_thread_log_level.store(static_cast<LoggingLevel>(kGPUProcessIOThreadLogLevel.Get()), std::memory_order_relaxed);
        g_main_thread_log_level.store(static_cast<LoggingLevel>(kGPUProcessMainThreadLogLevel.Get()), std::memory_order_relaxed);
    } else if (process_type == HangWatcher::ProcessType::kRendererProcess) {
        g_threadpool_log_level.store(static_cast<LoggingLevel>(kRendererProcessThreadPoolLogLevel.Get()), std::memory_order_relaxed);
        g_io_thread_log_level.store(static_cast<LoggingLevel>(kRendererProcessIOThreadLogLevel.Get()), std::memory_order_relaxed);
        g_main_thread_log_level.store(static_cast<LoggingLevel>(kRendererProcessMainThreadLogLevel.Get()), std::memory_order_relaxed);
    } else if (process_type == HangWatcher::ProcessType::kUtilityProcess) {
        g_threadpool_log_level.store(static_cast<LoggingLevel>(kUtilityProcessThreadPoolLogLevel.Get()), std::memory_order_relaxed);
        g_io_thread_log_level.store(static_cast<LoggingLevel>(kUtilityProcessIOThreadLogLevel.Get()), std::memory_order_relaxed);
        g_main_thread_log_level.store(static_cast<LoggingLevel>(kUtilityProcessMainThreadLogLevel.Get()), std::memory_order_relaxed);
    }
}
...
{
    if (g_main_thread_log_level.load(std::memory_order_relaxed) == LoggingLevel::kUmaAndCrash) {
        return true;
    }
...
void HangWatcher::SetShuttingDown()
{
    ...
    bool was_shutting_down = g_shutting_down.exchange(true, std::memory_order_relaxed);
...
HangWatcher::HangWatcher()
    ...
    , memory_pressure_listener_(FROM_HERE, base::BindRepeating(&HangWatcher::OnMemoryPressure, base::Unretained(this)))
{
...
debug::ScopedCrashKeyString HangWatcher::GetTimeSinceLastCriticalMemoryPressureCrashKey()
{
    ...
    DCHECK_GE(static_cast<uint64_t>(kCrashKeyContentSize), base::NumberToString(std::numeric_limits<int64_t>::max()).size());

    static debug::CrashKeyString* crash_key = AllocateCrashKeyString("seconds-since-last-memory-pressure", kCrashKeyContentSize);

    const base::TimeTicks last_critical_memory_pressure_time = last_critical_memory_pressure_.load(std::memory_order_relaxed);
    if (last_critical_memory_pressure_time.is_null()) {
        constexpr char kNoMemoryPressureMsg[] = "No critical memory pressure";
        static_assert(std::size(kNoMemoryPressureMsg) <= static_cast<uint64_t>(kCrashKeyContentSize),
        ...
        base::TimeDelta time_since_last_critical_memory_pressure = base::TimeTicks::Now() - last_critical_memory_pressure_time;
        return debug::ScopedCrashKeyString(crash_key, base::NumberToString(time_since_last_critical_memory_pressure.InSeconds()));
    }
}
#endif

std::string HangWatcher::GetTimeSinceLastSystemPowerResumeCrashKeyValue() const
{
    DCHECK_CALLED_ON_VALID_THREAD(hang_watcher_thread_checker_);

    const TimeTicks last_system_power_resume_time = PowerMonitor::GetInstance()->GetLastSystemResumeTime();
    ...
    const TimeDelta time_since_last_system_resume = TimeTicks::Now() - last_system_power_resume_time;
    return NumberToString(time_since_last_system_resume.InSeconds());
}

void HangWatcher::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
...
void HangWatcher::Wait()
{
    while (true) {
        ...
        const bool wait_was_normal = wait_time <= (monitoring_period_ + kWaitDriftTolerance);
...
void HangWatcher::Run()
{
    ...
    while (g_keep_monitoring.load(std::memory_order_relaxed)) {
        ...
        if (!IsWatchListEmpty() && g_keep_monitoring.load(std::memory_order_relaxed)) {
            Monitor();
...
ScopedClosureRunner HangWatcher::RegisterThreadInternal(ThreadType thread_type)
{
    AutoLock auto_lock(watch_state_lock_);
    CHECK(base::FeatureList::GetInstance());

    // Do not install a WatchState if the results would never be observable.
    if (!ThreadTypeLoggingLevelGreaterOrEqual(thread_type, LoggingLevel::kUmaOnly)) {
        return ScopedClosureRunner(base::DoNothing());
    }

    watch_states_.push_back(internal::HangWatchState::CreateHangWatchStateForCurrentThread(thread_type));
...
void HangWatcher::WatchStateSnapShot::Init(const HangWatchStates& watch_states, base::TimeTicks deadline_ignore_threshold, base::TimeDelta monitoring_period)
{
    DCHECK(!initialized_);
    ...
    // of the type found.
    constexpr size_t kHangCountArraySize = static_cast<std::size_t>(base::HangWatcher::ThreadType::kMax) + 1;
    ...
    for (const auto& watch_state : watch_states) {
        ...
        if (internal::HangWatchDeadline::IsFlagSet(internal::HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, flags)) {
            continue;
        }
        ...
        if (deadline <= now) {
            ...
            if (ThreadTypeLoggingLevelGreaterOrEqual(watch_state.get()->thread_type(), LoggingLevel::kUmaAndCrash)) {
                any_hung_thread_has_dumping_enabled = true;
            }

#if BUILDFLAG(ENABLE_BASE_TRACING)
            // Emit trace events for monitored threads.
            if (ThreadTypeLoggingLevelGreaterOrEqual(watch_state.get()->thread_type(), LoggingLevel::kUmaOnly)) {
                const PlatformThreadId thread_id = watch_state.get()->GetThreadID();
                const auto track = perfetto::Track::FromPointer(this, perfetto::ThreadTrack::ForThread(thread_id));
                TRACE_EVENT_BEGIN("latency", "HangWatcher::ThreadHung", track, deadline);
            ...
            if (thread_marked && all_threads_marked) {
                hung_watch_state_copies_.push_back(WatchStateCopy { deadline, watch_state.get()->GetThreadID() });
            } else {
    ...
    for (size_t i = 0; i < kHangCountArraySize; ++i) {
        ...
        if (hang_count != kInvalidHangCount && ThreadTypeLoggingLevelGreaterOrEqual(thread_type, LoggingLevel::kUmaOnly)) {
    ...
    //
    if (!all_threads_marked || found_deadline_before_ignore_threshold || !any_hung_thread_has_dumping_enabled) {
    ...
    ranges::sort(hung_watch_state_copies_, [](const WatchStateCopy& lhs, const WatchStateCopy& rhs) { return lhs.deadline < rhs.deadline; });
}
...
std::string HangWatcher::WatchStateSnapShot::PrepareHungThreadListCrashKey() const
{
    DCHECK(IsActionable());
    ...
    for (const WatchStateCopy& copy : hung_watch_state_copies_) {
        ...
        if (list_of_hung_thread_ids.size() + fragment.size() < static_cast<std::size_t>(debug::CrashKeySize::Size256)) {
...
HangWatcher::WatchStateSnapShot HangWatcher::GrabWatchStateSnapshotForTesting() const
{
...
void HangWatcher::Monitor()
{
    ...
    watch_state_snapshot_.Init(watch_states_, deadline_ignore_threshold_, monitoring_period_);
...
void HangWatcher::DoDumpWithoutCrashing(const WatchStateSnapShot& watch_state_snapshot)
{
...
#if !BUILDFLAG(IS_NACL)
    const std::string list_of_hung_thread_ids = watch_state_snapshot.PrepareHungThreadListCrashKey();

    static debug::CrashKeyString* crash_key = AllocateCrashKeyString("list-of-hung-threads", debug::CrashKeySize::Size256);

    const debug::ScopedCrashKeyString list_of_hung_threads_crash_key_string(crash_key, list_of_hung_thread_ids);

    const debug::ScopedCrashKeyString time_since_last_critical_memory_pressure_crash_key_string = GetTimeSinceLastCriticalMemoryPressureCrashKey();

    SCOPED_CRASH_KEY_STRING32("HangWatcher", "seconds-since-last-resume", GetTimeSinceLastSystemPowerResumeCrashKeyValue());

    SCOPED_CRASH_KEY_BOOL("HangWatcher", "shutting-down", g_shutting_down.load(std::memory_order_relaxed));
#endif
...
void HangWatcher::SetAfterMonitorClosureForTesting(base::RepeatingClosure closure)
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
...
void HangWatcher::SetAfterWaitCallbackForTesting(RepeatingCallback<void(TimeTicks)> callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(constructing_thread_checker_);
...
void HangWatcher::UnregisterThread()
{
    ...
    auto it = ranges::find(watch_states_, internal::HangWatchState::GetHangWatchStateForCurrentThread(), &std::unique_ptr<internal::HangWatchState>::get);
...
constexpr uint64_t kPersistentFlagsAndDeadlineMask = kOnlyDeadlineMask | static_cast<uint64_t>(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope);
} // namespace

// Flag binary representation assertions.
static_assert(
    static_cast<uint64_t>(HangWatchDeadline::Flag::kMinValue) > kOnlyDeadlineMask, "Invalid numerical value for flag. Would interfere with bits of data.");
static_assert(static_cast<uint64_t>(HangWatchDeadline::Flag::kMaxValue) <= kMaximumFlag, "A flag can only set a single bit.");
...
void HangWatchDeadline::SetDeadline(TimeTicks new_deadline)
{
    ...
    bits_.store(new_flags | ExtractDeadline(static_cast<uint64_t>(new_deadline.ToInternalValue())), std::memory_order_relaxed);
}

// TODO(crbug.com/40132796): Add flag DCHECKs here.
bool HangWatchDeadline::SetShouldBlockOnHang(uint64_t old_flags, TimeTicks old_deadline)
{
    ...
    return bits_.compare_exchange_weak(old_bits, desired_bits, std::memory_order_relaxed, std::memory_order_relaxed);
}
...
TimeTicks HangWatchDeadline::DeadlineFromBits(uint64_t bits)
{
    ...
    // representable value.
    DCHECK(bits <= kOnlyDeadlineMask) << "Flags bits are set. Remove them before returning deadline.";
...
void HangWatchDeadline::SetSwitchBitsClosureForTesting(RepeatingCallback<uint64_t(void)> closure)
{
...
HangWatchState::HangWatchState(HangWatcher::ThreadType thread_type)
    : resetter_(&hang_watch_state, this, nullptr)
...
// static
std::unique_ptr<HangWatchState> HangWatchState::CreateHangWatchStateForCurrentThread(HangWatcher::ThreadType thread_type)
{
...
bool HangWatchState::SetShouldBlockOnHang(uint64_t old_flags, TimeTicks old_deadline)
{
...
#if DCHECK_IS_ON()
void HangWatchState::SetCurrentWatchHangsInScope(WatchHangsInScope* current_hang_watch_scope_enable)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
