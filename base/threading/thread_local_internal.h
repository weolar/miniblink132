...
// hook).
template <typename T> class CheckedThreadLocalOwnedPointer {
public:
    ...
    ~CheckedThreadLocalOwnedPointer()
    {
        ...
        DCHECK_EQ(num_assigned_threads_.load(std::memory_order_relaxed), 0) << "Memory leak: Must join all threads or release all associated "
...
private:
    ...
    static void DeleteTlsPtr(void* ptr)
    {
