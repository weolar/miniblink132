...
public:
    ...
    UseTLSDuringDestructionRunner& operator=(const UseTLSDuringDestructionRunner&) = delete;
    ...
    {
        return teardown_works_correctly_;
    }
...
private:
    ...
    void CreateTlsKeyWithDestructor(size_t index)
    {
        ...
        int result = pthread_key_create(&(tls_states_[index].key), UseTLSDuringDestructionRunner::ThreadLocalDestructor);
        ASSERT_EQ(result, 0);
...
{
    UseTLSDuringDestructionRunner* runner = static_cast<UseTLSDuringDestructionRunner*>(input);
    runner->Run();
    return nullptr;
}
...
private:
    ...
    {
        NoDestructor<ThreadLocalStorage::Slot> slot(&TlsDestructionOrderRunner::Destructor);
...
public:
    void Run() override
    {
        ...
        NoDestructor<ThreadLocalStorage::Slot> slot(&CreateDuringDestructionRunner::FirstDestructor);
...
private:
    ...
    {
        NoDestructor<ThreadLocalStorage::Slot> slot(&CreateDuringDestructionRunner::SecondDestructor);
        slot->Set(reinterpret_cast<void*>(234));
    }

    static void SecondDestructor(void*)
    {
        second_destructor_called = true;
    }
};
...
TEST(ThreadLocalStorageTest, MAYBE_TLSDestructors)
{
    ...
    for (int index = 0; index < kNumThreads; index++) {
        ...
        threads[index] = new DelegateSimpleThread(thread_delegates[index], "tls thread");
        threads[index]->Start();
    }
