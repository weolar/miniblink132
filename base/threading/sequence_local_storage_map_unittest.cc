...
template <typename T, typename... Args> SequenceLocalStorageMap::ValueDestructorPair CreateExternalValueDestructorPair(Args... args)
{
    internal::SequenceLocalStorageMap::ExternalValue value;
    value.emplace(new T(args...));
    auto* destructor = SequenceLocalStorageMap::MakeExternalDestructor<T, std::default_delete<T>>();

    SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair { std::move(value), destructor };

    return value_destructor_pair;
}

template <typename T, typename... Args> SequenceLocalStorageMap::ValueDestructorPair CreateInlineValueDestructorPair(Args... args)
{
...
TEST(SequenceLocalStorageMapTest, SetGetExternal)
{
    ...
    ScopedSetSequenceLocalStorageMapForCurrentThread scoped_sequence_local_storage_map(&sequence_local_storage_map);

    SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateExternalValueDestructorPair<int>(5);

    sequence_local_storage_map.Set(kSlotId, std::move(value_destructor_pair));

    EXPECT_EQ(sequence_local_storage_map.Get(kSlotId)->external_value.value_as<int>(), 5);
}
...
TEST(SequenceLocalStorageMapTest, SetGetInline)
{
    ...
    SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateInlineValueDestructorPair<int>(5);

    sequence_local_storage_map.Set(kSlotId, std::move(value_destructor_pair));

    EXPECT_EQ(sequence_local_storage_map.Get(kSlotId)->inline_value.value_as<int>(), 5);
}
...
TEST(SequenceLocalStorageMapTest, DestructorExternal)
{
...

    {
        ...
        SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateExternalValueDestructorPair<SetOnDestroy>(&set_on_destruction);
...
TEST(SequenceLocalStorageMapTest, DestructorCalledOnSetOverwriteExternal)
{
    ...
    bool set_on_destruction2 = false;
    {
        ...
        SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateExternalValueDestructorPair<SetOnDestroy>(&set_on_destruction);
        SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair2 = CreateExternalValueDestructorPair<SetOnDestroy>(&set_on_destruction2);
...
template <typename T> void DestructorInline()
{
    if constexpr (!absl::is_trivially_relocatable<T>()) {
...

        {
            ...
            SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateInlineValueDestructorPair<T>(&set_on_destruction);
...
template <typename T> void DestructorCalledOnSetOverwriteInline()
{
    if constexpr (!absl::is_trivially_relocatable<T>()) {
        // Test disabled because there is no reliable way to detect SetOnDestroy
        // is trivially relocatble.
        ...
        bool set_on_destruction2 = false;
        {
            ...
            SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair = CreateInlineValueDestructorPair<T>(&set_on_destruction);
            SequenceLocalStorageMap::ValueDestructorPair value_destructor_pair2 = CreateInlineValueDestructorPair<T>(&set_on_destruction2);
