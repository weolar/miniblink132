...
ScopedBlockingCall::ScopedBlockingCall(const Location& from_here, BlockingType blocking_type)
    : UncheckedScopedBlockingCall(blocking_type, UncheckedScopedBlockingCall::BlockingCallType::kRegular)
{
#if DCHECK_IS_ON()
    ...
    TRACE_EVENT_BEGIN("base", "ScopedBlockingCall",
...
ScopedBlockingCallWithBaseSyncPrimitives::ScopedBlockingCallWithBaseSyncPrimitives(const Location& from_here, BlockingType blocking_type)
    : UncheckedScopedBlockingCall(blocking_type, UncheckedScopedBlockingCall::BlockingCallType::kBaseSyncPrimitives)
{
#if DCHECK_IS_ON()
    const AutoReset<bool> resetter(&construction_in_progress, true, false);
#endif
    ...
    TRACE_EVENT_BEGIN("base", "ScopedBlockingCallWithBaseSyncPrimitives", [&](perfetto::EventContext ctx) {
        perfetto::protos::pbzero::SourceLocation* source_location_data = ctx.event()->set_source_location();
