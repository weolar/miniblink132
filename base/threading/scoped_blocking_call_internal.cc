...
constinit thread_local UncheckedScopedBlockingCall* last_scoped_blocking_call = nullptr;
...
UncheckedScopedBlockingCall* GetLastScopedBlockingCall()
{
    ...
    // thread_local storage for loaded libraries:
    // https://github.com/google/sanitizers/issues/1265
    MSAN_UNPOISON(&last_scoped_blocking_call, sizeof(UncheckedScopedBlockingCall*));
...
{
    return GetTaskPriorityForCurrentThread() == TaskPriority::BEST_EFFORT && CanUseBackgroundThreadTypeForWorkerThread();
}

} // namespace

void SetBlockingObserverForCurrentThread(BlockingObserver* new_blocking_observer)
{
...
{
    if (assigned_jank_window_ && call_start_ < assigned_jank_window_->start_time_) {
...
// static
scoped_refptr<IOJankMonitoringWindow> IOJankMonitoringWindow::MonitorNextJankWindowIfNecessary(TimeTicks recent_now)
{
...

    {
        ...
        scoped_refptr<IOJankMonitoringWindow>& current_jank_window_ref = current_jank_window_storage();
        ...
        TimeTicks next_window_start_time = current_jank_window_ref ? current_jank_window_ref->start_time_ + kMonitoringWindow : recent_now;
    ...
    ThreadPool::PostDelayedTask(FROM_HERE, BindOnce([] { IOJankMonitoringWindow::MonitorNextJankWindowIfNecessary(TimeTicks::Now()); }),
...
void IOJankMonitoringWindow::OnBlockingCallCompleted(TimeTicks call_start, TimeTicks call_end)
{
...
void IOJankMonitoringWindow::AddJank(int local_jank_start_index, int num_janky_intervals)
{
...
// static
scoped_refptr<IOJankMonitoringWindow>& IOJankMonitoringWindow::current_jank_window_storage()
{
...
UncheckedScopedBlockingCall::UncheckedScopedBlockingCall(BlockingType blocking_type, BlockingCallType blocking_call_type)
    ...
    , is_will_block_(blocking_type == BlockingType::WILL_BLOCK || (previous_scoped_blocking_call_ && previous_scoped_blocking_call_->is_will_block_))
{
    ...
    if (!IsBackgroundPriorityWorker() && (!g_only_monitor_observed_threads || blocking_observer_)) {
        const bool is_monitored_type = blocking_call_type == BlockingCallType::kRegular && !is_will_block_;
        if (is_monitored_type && !previous_scoped_blocking_call_) {
            monitored_call_.emplace();
        } else if (!is_monitored_type && previous_scoped_blocking_call_ && previous_scoped_blocking_call_->monitored_call_) {
    ...
    if (blocking_observer_) {
        ...
        } else if (blocking_type == BlockingType::WILL_BLOCK && !previous_scoped_blocking_call_->is_will_block_) {
...
void EnableIOJankMonitoringForProcess(IOJankReportingCallback reporting_callback, OnlyObservedThreadsForTest only_observed_threads)
{
    {
        ...
        internal::IOJankMonitoringWindow::reporting_callback_storage() = std::move(reporting_callback);
    }
