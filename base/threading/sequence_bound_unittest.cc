...
struct DirectVariation {
    ...
    template <typename T> class Wrapper : public SequenceBound<T> {
    public:
        template <typename... Args>
        explicit Wrapper(scoped_refptr<SequencedTaskRunner> task_runner, Args&&... args)
            : SequenceBound<T>(std::move(task_runner), std::forward<Args>(args)...)
        {
        }

        template <typename... Args> void WrappedEmplace(scoped_refptr<SequencedTaskRunner> task_runner, Args&&... args)
        {
...
struct UniquePtrVariation {
    ...
    template <typename T> struct Wrapper : public SequenceBound<std::unique_ptr<T>> {
    public:
        template <typename... Args>
        explicit Wrapper(scoped_refptr<SequencedTaskRunner> task_runner, Args&&... args)
            : SequenceBound<std::unique_ptr<T>>(std::move(task_runner), std::make_unique<T>(std::forward<Args>(args)...))
        {
        }

        template <typename... Args> void WrappedEmplace(scoped_refptr<SequencedTaskRunner> task_runner, Args&&... args)
        {
            this->emplace(std::move(task_runner), std::make_unique<T>(std::forward<Args>(args)...));
        }
...
template <typename Variation> class SequenceBoundTest : public ::testing::Test {
public:
    ...
    scoped_refptr<SequencedTaskRunner> background_task_runner_ = ThreadPool::CreateSequencedTaskRunner({});
...
class Base {
public:
    explicit Base(EventLogger& logger)
        : logger_(logger)
    {
    ...
    virtual ~Base()
    {
        logger_->AddEvent("destroyed Base");
    }
...
public:
    explicit Derived(EventLogger& logger)
        : Base(logger)
    {
    ...
    ~Derived() override
    {
        GetLogger().AddEvent("destroyed Derived");
    }
...
class Leftmost {
public:
    explicit Leftmost(EventLogger& logger)
        : logger_(logger)
    {
    ...
    virtual ~Leftmost()
    {
        logger_->AddEvent("destroyed Leftmost");
    }
...
public:
    explicit Rightmost(EventLogger& logger)
        : Base(logger)
    {
    ...
    ~Rightmost() override
    {
        GetLogger().AddEvent("destroyed Rightmost");
    }
...
public:
    ...
    {
        EXPECT_TRUE(sequence_checker_.CalledOnValidSequence());
        AddEventIfNeeded(StringPrintf("updated BoxedValue from %d to %d", value_, value));
        value_ = value;
    }
...
public:
    explicit SequenceValidator(scoped_refptr<SequencedTaskRunner> task_runner, bool constructs_on_managing_task_runner)
...
{
    SEQUENCE_BOUND_T<SequenceValidator> validator(this->background_task_runner_, this->background_task_runner_, TypeParam::kManagingTaskRunnerConstructsT);
    ...
    validator.AsyncCall(&SequenceValidator::ReturnsIntMutable).Then(BindOnce([](int) {}));
    ...
    validator.WrappedEmplace(this->background_task_runner_, this->background_task_runner_, TypeParam::kManagingTaskRunnerConstructsT);
    validator.PostTaskWithThisObject(BindLambdaForTesting([](const SequenceValidator& v) { v.ReturnsVoid(); }));
    validator.PostTaskWithThisObject(BindLambdaForTesting([](SequenceValidator* v) { v->ReturnsVoidMutable(); }));
    validator.Reset();
...
{
    SEQUENCE_BOUND_T<BoxedValue> value(this->background_task_runner_, 0, &this->logger_);
    ...
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0"));

    value.AsyncCall(&BoxedValue::set_value).WithArgs(66);
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("updated BoxedValue from 0 to 66"));
    ...
    EXPECT_FALSE(value);
    this->FlushPostedTasks();
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("destroyed BoxedValue = 66"));
}
...
TYPED_TEST(SequenceBoundTest, ConstructAndImmediateAsyncCall)
{
    ...
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0", "updated BoxedValue from 0 to 8"));
}
...
TYPED_TEST(SequenceBoundTest, MoveConstruction)
{
    ...
    SEQUENCE_BOUND_T<Derived> derived_old(this->background_task_runner_, std::ref(this->logger_));
    ...
    derived_new.Reset();
    this->FlushPostedTasks();
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("constructed Base", "constructed Derived", "destroyed Derived", "destroyed Base"));
}

TYPED_TEST(SequenceBoundTest, MoveConstructionUpcastsToBase)
{
    SEQUENCE_BOUND_T<Derived> derived(this->background_task_runner_, std::ref(this->logger_));
...
{
    SEQUENCE_BOUND_T<MultiplyDerived> multiply_derived(this->background_task_runner_, std::ref(this->logger_));
    ...
    EXPECT_THAT(this->logger_.TakeEvents(),
        ::testing::ElementsAre("constructed Leftmost", "constructed Base", "constructed Rightmost", "constructed MultiplyDerived", "destroyed MultiplyDerived",
...
TYPED_TEST(SequenceBoundTest, AsyncCallLeftmost)
{
    ...
    EXPECT_THAT(this->logger_.TakeEvents(),
        ::testing::ElementsAre("constructed Leftmost", "constructed Base", "constructed Rightmost", "constructed MultiplyDerived", "set Leftmost to 3"));
}
...
TYPED_TEST(SequenceBoundTest, AsyncCallRightmost)
{
    ...
    multiply_derived.FlushPostedTasksForTesting();
    EXPECT_THAT(this->logger_.TakeEvents(),
        ::testing::ElementsAre("constructed Leftmost", "constructed Base", "constructed Rightmost", "constructed MultiplyDerived", "set Rightmost to 3"));
}
...
{
    int lvalue = 99;
    SEQUENCE_BOUND_T<BoxedValue> value(this->background_task_runner_, lvalue, &this->logger_);
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 99"));
}
...
TYPED_TEST(SequenceBoundTest, PostTaskWithThisObject)
{
    ...
    SEQUENCE_BOUND_T<BoxedValue> value(this->background_task_runner_, kTestValue1);
    value.PostTaskWithThisObject(BindLambdaForTesting([&](const BoxedValue& v) { EXPECT_EQ(kTestValue1, v.value()); }));
    value.PostTaskWithThisObject(BindLambdaForTesting([&](BoxedValue* v) { v->set_value(kTestValue2); }));
    value.PostTaskWithThisObject(BindLambdaForTesting([&](const BoxedValue& v) { EXPECT_EQ(kTestValue2, v.value()); }));
    value.FlushPostedTasksForTesting();
}
...
TYPED_TEST(SequenceBoundTest, SynchronouslyResetForTest)
{
    ...
    value.AsyncCall(&BoxedValue::set_destruction_callback).WithArgs(BindLambdaForTesting([&] { destroyed = true; }));
...
TYPED_TEST(SequenceBoundTest, FlushPostedTasksForTesting)
{
    ...
    EXPECT_THAT(this->logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0", "updated BoxedValue from 0 to 42"));
}
...
TYPED_TEST(SequenceBoundTest, Emplace)
{
    ...
    value.AsyncCall(&BoxedValue::value).Then(BindLambdaForTesting([&](int actual_value) { EXPECT_EQ(8, actual_value); }));
    value.FlushPostedTasksForTesting();
}

TYPED_TEST(SequenceBoundTest, EmplaceOverExisting)
{
    SEQUENCE_BOUND_T<BoxedValue> value(this->background_task_runner_, 8, &this->logger_);
    EXPECT_FALSE(value.is_null());
    value.WrappedEmplace(this->background_task_runner_, 9, &this->logger_);
    value.AsyncCall(&BoxedValue::value).Then(BindLambdaForTesting([&](int actual_value) { EXPECT_EQ(9, actual_value); }));
    value.FlushPostedTasksForTesting();
    ...
    if constexpr (TypeParam::kManagingTaskRunnerConstructsT) {
        ...
        EXPECT_THAT(this->logger_.TakeEvents(),
            ::testing::ElementsAre("constructed BoxedValue = 8", "destroyed BoxedValue = 8", "constructed BoxedValue = 9", "accessed BoxedValue = 9"));
    } else {
        ...
        EXPECT_THAT(this->logger_.TakeEvents(),
            ::testing::ElementsAre("constructed BoxedValue = 8", "constructed BoxedValue = 9", "destroyed BoxedValue = 8", "accessed BoxedValue = 9"));
    }
}

TYPED_TEST(SequenceBoundTest, EmplaceOverExistingWithTaskRunnerSwap)
{
    scoped_refptr<SequencedTaskRunner> another_task_runner = ThreadPool::CreateSequencedTaskRunner({});
    ...
    value.WrappedEmplace(this->background_task_runner_, 9);
    {
        value.PostTaskWithThisObject(
            BindLambdaForTesting([another_task_runner, background_task_runner = this->background_task_runner_](const BoxedValue& boxed_value) {
...
TYPED_TEST(SequenceBoundTest, AsyncCallIntArgNoThen)
{
    ...
    SEQUENCE_BOUND_T<IntArgVoidReturn> s(this->background_task_runner_, &method_called_with, &const_method_called_with);
...
TYPED_TEST(SequenceBoundTest, AsyncCallNoArgsVoidThen)
{
    ...
    {
        RunLoop loop;
        s.AsyncCall(&NoArgsVoidReturn::Method).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
    }

    {
        RunLoop loop;
        s.AsyncCall(&NoArgsVoidReturn::ConstMethod).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
    }
}
...
TYPED_TEST(SequenceBoundTest, AsyncCallNoArgsIntThen)
{
    ...
    {
        RunLoop loop;
        s.AsyncCall(&NoArgsIntReturn::Method).Then(BindLambdaForTesting([&](int result) {
    ...
    {
        RunLoop loop;
        s.AsyncCall(&NoArgsIntReturn::ConstMethod).Then(BindLambdaForTesting([&](int result) {
...
TYPED_TEST(SequenceBoundTest, AsyncCallWithArgsVoidThen)
{
    ...
    {
        RunLoop loop;
        s.AsyncCall(&IntArgVoidReturn::Method).WithArgs(123).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_EQ(123, method_called_with);
    }
    ...
    {
        RunLoop loop;
        s.AsyncCall(&IntArgVoidReturn::ConstMethod).WithArgs(456).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_EQ(456, const_method_called_with);
    }
}
...
TYPED_TEST(SequenceBoundTest, AsyncCallWithArgsIntThen)
{
    ...
    {
        RunLoop loop;
        s.AsyncCall(&IntArgIntReturn::Method).WithArgs(123).Then(BindLambdaForTesting([&](int result) {
            EXPECT_EQ(-123, result);
            loop.Quit();
        }));
        loop.Run();
    ...
    {
        RunLoop loop;
        s.AsyncCall(&IntArgIntReturn::ConstMethod).WithArgs(456).Then(BindLambdaForTesting([&](int result) {
            EXPECT_EQ(-456, result);
            loop.Quit();
        }));
        loop.Run();
...
TYPED_TEST(SequenceBoundTest, AsyncCallIgnoreResultNoArgs)
{
    {
        RunLoop loop;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithNoArgs> s(this->background_task_runner_, &loop, nullptr);
...
{
    {
        RunLoop loop;
        bool called = false;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithNoArgs> s(this->background_task_runner_, nullptr, &called);
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithNoArgs::ConstMethod)).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_TRUE(called);
    }

    {
        ...
        bool called = false;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithNoArgs> s(this->background_task_runner_, nullptr, &called);
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithNoArgs::Method)).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_TRUE(called);
    }
}
...
{
    {
        RunLoop loop;
        int result = 0;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithArgs> s(this->background_task_runner_, &loop, std::ref(result));
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithArgs::ConstMethod)).WithArgs(60);
        loop.Run();
        EXPECT_EQ(60, result);
    }

    {
        ...
        int result = 0;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithArgs> s(this->background_task_runner_, &loop, std::ref(result));
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithArgs::Method)).WithArgs(06);
        loop.Run();
        EXPECT_EQ(06, result);
    }
}
...
{
    {
        RunLoop loop;
        int result = 0;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithArgs> s(this->background_task_runner_, nullptr, std::ref(result));
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithArgs::ConstMethod)).WithArgs(60).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_EQ(60, result);
    }

    {
        ...
        int result = 0;
        SEQUENCE_BOUND_T<IgnoreResultTestHelperWithArgs> s(this->background_task_runner_, nullptr, std::ref(result));
        s.AsyncCall(IgnoreResult(&IgnoreResultTestHelperWithArgs::Method)).WithArgs(06).Then(BindLambdaForTesting([&] { loop.Quit(); }));
        loop.Run();
        EXPECT_EQ(06, result);
    }
}
...
protected:
    ...
    scoped_refptr<SequencedTaskRunner> task_runner_ = SequencedTaskRunner::GetCurrentDefault();
};
