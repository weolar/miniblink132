...
bool ThreadCheckerImpl::CalledOnValidThread(std::unique_ptr<debug::StackTrace>* out_bound_at) const
{
    AutoLock auto_lock(lock_);
    ...
    if (thread_ref_ == PlatformThread::CurrentRef()) {
        ...
        if (sequence_token_ == internal::SequenceToken::GetForCurrentThread() && internal::CurrentTaskIsThreadBound()) {
            return true;
        }
    }
