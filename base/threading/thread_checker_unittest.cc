...
void ExpectNotCalledOnValidThreadWithSequenceTokenThreadBound(ThreadCheckerImpl* thread_checker, SequenceToken sequence_token)
{
...
TEST(ThreadCheckerTest, DisallowedSameThreadAndSequenceDifferentTasksNotThreadBound)
{
    std::unique_ptr<ThreadCheckerImpl> thread_checker;
...
{
    ThreadCheckerImpl thread_checker;
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectNotCalledOnValidThread, Unretained(&thread_checker)));
}

TEST(ThreadCheckerTest, DisallowedDifferentThreadsSameSequence)
{
    SingleThreadTaskRunner::CurrentDefaultHandle single_thread_task_runner_current_default_handle(MakeRefCounted<TestSimpleTaskRunner>());
    const SequenceToken sequence_token(SequenceToken::Create());
    ...
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectNotCalledOnValidThreadWithSequenceTokenThreadBound, Unretained(&thread_checker), sequence_token));
}
...
TEST(ThreadCheckerTest, DetachFromThread)
{
    ...
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectCalledOnValidThread, Unretained(&thread_checker)));
...
public:
    ...
    ~ThreadCheckerOwner()
    {
        EXPECT_TRUE(checker_.CalledOnValidThread());
    }
...
TEST(ThreadCheckerTest, CalledOnValidThreadFromThreadDestruction)
{
    ...
    RunCallbackOnNewThreadSynchronously(BindLambdaForTesting([&] { thread_local_owner.Set(std::make_unique<ThreadCheckerOwner>(false)); }));
}
...
{
    ThreadLocalOwnedPointer<ThreadCheckerOwner> thread_local_owner;
    RunCallbackOnNewThreadSynchronously(BindLambdaForTesting([&] { thread_local_owner.Set(std::make_unique<ThreadCheckerOwner>(true)); }));
}
...
TEST(ThreadCheckerTest, Move)
{
    ...
    // thread.
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectCalledOnValidThread, Unretained(&initial)));
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectCalledOnValidThread, Unretained(&move_constructed)));

    // But the latest one shouldn't be able to run on another thread.
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectNotCalledOnValidThread, Unretained(&move_assigned)));
...
TEST(ThreadCheckerTest, MoveFromDetachedRebinds)
{
    ...
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectNotCalledOnValidThread, Unretained(&moved_into)));
...
TEST(ThreadCheckerTest, MoveOffThreadBanned)
{
    ...
    RunCallbackOnNewThreadSynchronously(BindOnce(&ExpectCalledOnValidThread, Unretained(&other_thread)));
...
TEST_F(ThreadCheckerMacroTest, Macros)
{
    ...
    RunCallbackOnNewThreadSynchronously(BindOnce(&ThreadCheckerMacroTest::ExpectDeathOnOtherThread, Unretained(this)));

    DETACH_FROM_THREAD(thread_checker_);

    RunCallbackOnNewThreadSynchronously(BindOnce(&ThreadCheckerMacroTest::ExpectNoDeathOnOtherThreadAfterDetach, Unretained(this)));
}
