...
static ThreadType kAllThreadTypes[] = { ThreadType::kRealtimeAudio, ThreadType::kDisplayCritical, ThreadType::kDefault, ThreadType::kBackground };

static_assert(static_cast<int>(ThreadType::kBackground) == 0, "kBackground isn't lowest");
static_assert(ThreadType::kRealtimeAudio == ThreadType::kMaxValue, "kRealtimeAudio isn't highest");
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_EQ(ThreadPriorityForTest::kNormal, PlatformThread::GetCurrentThreadPriorityForTest());
    }
};
...
#if BUILDFLAG(IS_WIN)
void FunctionThatBoostsPriorityOnFirstInvoke(ThreadPriorityForTest expected_priority)
{
    SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY();
    EXPECT_EQ(expected_priority, PlatformThread::GetCurrentThreadPriorityForTest());
}

void FunctionThatBoostsPriorityOnEveryInvoke()
{
    SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY_REPEATEDLY();
    EXPECT_EQ(base::ThreadPriorityForTest::kNormal, PlatformThread::GetCurrentThreadPriorityForTest());
}
                    ...
                    [](ThreadType from, ThreadType to) {
                        EXPECT_EQ(PlatformThread::GetCurrentThreadType(), from);
                        {
                            ...
                            bool will_boost_priority
                                = from < to && PlatformThread::CanChangeThreadType(from, to) && PlatformThread::CanChangeThreadType(to, from);
                            EXPECT_EQ(PlatformThread::GetCurrentThreadType(), will_boost_priority ? to : from);
                        }
                        EXPECT_EQ(PlatformThread::GetCurrentThreadType(), from);
                    },
                    from, to));
...
{
    EXPECT_DCHECK_DEATH({ ScopedBoostPriority scoped_boost_priority(ThreadType::kRealtimeAudio); });
}
...
TEST_F(ScopedThreadPriorityTest, WithoutPriorityBoost)
{
    ...
    {
        SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY();
        EXPECT_EQ(ThreadPriorityForTest::kNormal, PlatformThread::GetCurrentThreadPriorityForTest());
    }
    EXPECT_EQ(ThreadPriorityForTest::kNormal, PlatformThread::GetCurrentThreadPriorityForTest());
}
...
TEST_F(ScopedThreadPriorityTest, WithPriorityBoost)
{
    ...
    EXPECT_EQ(ThreadPriorityForTest::kBackground, PlatformThread::GetCurrentThreadPriorityForTest());
...
TEST_F(ScopedThreadPriorityTest, FunctionThatBoostsPriorityOnFirstInvoke)
{
    ...
    FunctionThatBoostsPriorityOnFirstInvoke(base::ThreadPriorityForTest::kBackground);
