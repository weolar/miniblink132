...
{
    ABSL_ANNOTATE_BENIGN_RACE(value, "Test-only data race on boolean in base/thread_unittest");
    *value = !*value;
}
...
public:
    ...
    {
        init_called_ = false;
        ABSL_ANNOTATE_BENIGN_RACE(this, "Benign test-only data race on vptr - http://crbug.com/98219");
    }
    ...
    ~SleepInsideInitThread() override
    {
        Stop();
    }
...
public:
    ...
    CapturingDestructionObserver& operator=(const CapturingDestructionObserver&) = delete;
...
// signal |event|.
void ReturnThreadId(Thread* thread, PlatformThreadId* id, WaitableEvent* event)
{
...
TEST_F(ThreadTest, StartWithOptions_StackSize)
{
    ...
    a.task_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&event)));
    event.Wait();
}
...
TEST_F(ThreadTest, StartWithOptions_NonJoinable)
{
    ...
    WaitableEvent block_event(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    a->task_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Wait, Unretained(&block_event)));
...
TEST_F(ThreadTest, TwoTasksOnJoinableThread)
{
    bool was_invoked = false;
    {
        ...
        a.task_runner()->PostTask(FROM_HERE, BindOnce(static_cast<void (*)(TimeDelta)>(&PlatformThread::Sleep), Milliseconds(20)));
    ...
    EXPECT_DCHECK_DEATH_WITH(
        {
            ...
            b.task_runner()->PostTask(FROM_HERE, BindOnce(&Thread::Stop, Unretained(&a)));
...
{
    std::unique_ptr<Thread> a = std::make_unique<Thread>("TransferOwnershipAndStop");
    EXPECT_TRUE(a->StartAndWaitForTesting());
    EXPECT_TRUE(a->IsRunning());
    ...
    b.task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](std::unique_ptr<Thread> thread_to_stop, WaitableEvent* event_to_signal) {
                thread_to_stop->Stop();
...
TEST_F(ThreadTest, StartTwiceNonJoinableNotAllowed)
{
    ...
    WaitableEvent last_task_event(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    a->task_runner()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&last_task_event)));
...
TEST_F(ThreadTest, ThreadId)
{
    ...
    a.task_runner()->PostTask(FROM_HERE, BindOnce(ReturnThreadId, &a, &id_from_new_thread, &event));
...
TEST_F(ThreadTest, CleanUp)
{
...

    {
        ...
        t.task_runner()->PostTask(FROM_HERE, BindOnce(&RegisterDestructionObserver, Unretained(&loop_destruction_observer)));
...
TEST_F(ThreadTest, FlushForTesting)
{
    ...
    for (size_t i = 0; i < kNumSleepTasks; ++i) {
        a.task_runner()->PostTask(FROM_HERE, BindOnce(&PlatformThread::Sleep, kSleepPerTestTask));
    }

    // All tasks should have executed, as reflected by the elapsed time.
    a.FlushForTesting();
    EXPECT_GE(TimeTicks::Now() - ticks_before_post, kNumSleepTasks * kSleepPerTestTask);
...
public:
    SequenceManagerThreadDelegate()
        ...
        , task_queue_(sequence_manager_->CreateTaskQueue(TaskQueue::Spec(sequence_manager::QueueName::DEFAULT_TQ)))
    {
    ...
    SequenceManagerThreadDelegate& operator=(const SequenceManagerThreadDelegate&) = delete;
    ...
    {
        sequence_manager_->BindToMessagePump(MessagePump::Create(MessagePumpType::DEFAULT));
    }
...
TEST_F(ThreadTest, ProvidedThreadDelegate)
{
    ...
    scoped_refptr<SingleThreadTaskRunner> task_runner = options.delegate->GetDefaultTaskRunner();
