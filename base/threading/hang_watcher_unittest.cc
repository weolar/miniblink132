...
// threadpool threads.
const std::vector<base::test::FeatureRefAndParams> kFeatureAndParams { { base::kEnableHangWatcher, { { "ui_thread_log_level", "2" } } } };
...
public:
    ...
    {
        std::vector<base::test::FeatureRefAndParams> enabled_features = kFeatureAndParams;
        feature_list_.InitWithFeaturesAndParameters(enabled_features, {});
        HangWatcher::InitializeOnMainThread(HangWatcher::ProcessType::kUtilityProcess,
            /*emit_crashes=*/true);
    }
    ...
    HangWatcherEnabledInZygoteChildTest(const HangWatcherEnabledInZygoteChildTest& other) = delete;
    HangWatcherEnabledInZygoteChildTest& operator=(const HangWatcherEnabledInZygoteChildTest& other) = delete;
...
public:
    explicit BlockingThread(base::WaitableEvent* unblock_thread, base::TimeDelta timeout)
    ...
    void Run() override
    {
        ...
        base::ScopedClosureRunner unregister_closure = base::HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kMainThread);
...
public:
    ...
    HangWatcherTest()
    {
        ...
        HangWatcher::InitializeOnMainThread(HangWatcher::ProcessType::kBrowserProcess, /*emit_crashes=*/true);

        hang_watcher_.SetAfterMonitorClosureForTesting(base::BindRepeating(&WaitableEvent::Signal, base::Unretained(&monitor_event_)));

        hang_watcher_.SetOnHangClosureForTesting(base::BindRepeating(&WaitableEvent::Signal, base::Unretained(&hang_event_)));
...
public:
    ...
    HangWatcherBlockingThreadTest(const HangWatcherBlockingThreadTest& other) = delete;
    HangWatcherBlockingThreadTest& operator=(const HangWatcherBlockingThreadTest& other) = delete;
...
TEST_F(HangWatcherTest, InvalidatingExpectationsPreventsCapture)
{
    ...
    auto unregister_thread_closure = HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kMainThread);
...
TEST_F(HangWatcherTest, NewSeparateWatchHangsInScopeAfterInvalidationDetectsHang)
{
    // Register the main test thread for hang watching.
    auto unregister_thread_closure = HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kMainThread);
...
TEST_F(HangWatcherTest, NestedScopes)
{
    ...
    // threaded.
    auto current_hang_watch_state = base::internal::HangWatchState::CreateHangWatchStateForCurrentThread(HangWatcher::ThreadType::kMainThread);
...
TEST_F(HangWatcherBlockingThreadTest, HistogramsLoggedOnHang)
{
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Shutdown"), IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.IOThread.Shutdown"), IsEmpty());
...
TEST_F(HangWatcherBlockingThreadTest, HistogramsLoggedWithShutdownFlag)
{
    ...
    // First monitoring catches the hang and emits the histogram.
    MonitorHangs();
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Shutdown"), ElementsAre(base::Bucket(true, /*count=*/1)));
    ...
    // Hang is logged again even if it would not trigger a crash dump.
    MonitorHangs();
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Shutdown"), ElementsAre(base::Bucket(true, /*count=*/2)));

    // Thread types that are not monitored should not get any samples.
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.IOThread.Shutdown"), IsEmpty());

    // No normal hangs.
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.UIThread.Normal"), IsEmpty());
    EXPECT_THAT(histogram_tester.GetAllSamples("HangWatcher.IsThreadHung.BrowserProcess.IOThread.Normal"), IsEmpty());
...
public:
    ...
    HangWatcherSnapshotTest& operator=(const HangWatcherSnapshotTest& other) = delete;
...
protected:
    ...
    std::string ConcatenateThreadIds(const std::vector<base::PlatformThreadId>& ids) const
    {
        std::string result;
...
{
    hang_watcher_.SetOnHangClosureForTesting(base::BindLambdaForTesting([this] { ++hang_capture_count_; }));
    hang_watcher_.SetAfterMonitorClosureForTesting(base::BindLambdaForTesting([this] { monitor_event_.Signal(); }));
    ...
    auto unregister_thread_closure = HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kMainThread);
    {
        ...
        internal::HangWatchState* current_hang_watch_state = internal::HangWatchState::GetHangWatchStateForCurrentThread();
        ...
        ASSERT_NE(current_hang_watch_state->GetDeadline(), base::TimeTicks::FromInternalValue(kArbitraryDeadline));
        current_hang_watch_state->GetHangWatchDeadlineForTesting()->SetSwitchBitsClosureForTesting(
            base::BindLambdaForTesting([] { return kArbitraryDeadline; }));

        ExpectNoCapture();

        // Marking failed.
        ASSERT_FALSE(current_hang_watch_state->IsFlagSet(internal::HangWatchDeadline::Flag::kShouldBlockOnHang));

        current_hang_watch_state->GetHangWatchDeadlineForTesting()->ResetSwitchBitsClosureForTesting();
    }
}
...
TEST_F(HangWatcherSnapshotTest, MAYBE_HungThreadIDs)
{
    ...
    hang_watcher_.SetOnHangClosureForTesting(base::BindLambdaForTesting([this] {
        EXPECT_EQ(hang_watcher_.GrabWatchStateSnapshotForTesting().PrepareHungThreadListCrashKey(), list_of_hung_thread_ids_during_capture_);
        ++hang_capture_count_;
    }));
    ...
    blocking_thread.StartAndWaitForScopeEntered();
    {
        ...
        // deadline.
        TestIDList(ConcatenateThreadIds({ blocking_thread.GetId(), test_thread_id_ }));
...
TEST_F(HangWatcherSnapshotTest, TimeSinceLastSystemPowerResumeCrashKey)
{
    ...
    hang_watcher_.SetOnHangClosureForTesting(base::BindLambdaForTesting([this] {
        ++hang_capture_count_;
        seconds_since_last_power_resume_crash_key_ = hang_watcher_.GetTimeSinceLastSystemPowerResumeCrashKeyValue();
    }));
...

    {
...

        {
            ...
            EXPECT_EQ(base::NumberToString(kAfterResumeTime.InSeconds()), seconds_since_last_power_resume_crash_key_);
        }
    }
}
...
public:
    ...
    {
        hang_watcher_.InitializeOnMainThread(HangWatcher::ProcessType::kBrowserProcess, /*emit_crashes=*/true);
    ...
    HangWatcherPeriodicMonitoringTest(const HangWatcherPeriodicMonitoringTest& other) = delete;
    HangWatcherPeriodicMonitoringTest& operator=(const HangWatcherPeriodicMonitoringTest& other) = delete;
...
protected:
    ...
    {
        hang_watcher_.SetAfterWaitCallbackForTesting(
            base::BindLambdaForTesting([this, time_delta](base::TimeTicks time_before_wait) { test_clock_.Advance(time_delta); }));
    }
...
TEST_F(HangWatcherPeriodicMonitoringTest, NoPeriodicMonitoringWithoutRegisteredThreads)
{
    RunLoop run_loop;

    // If a call to HangWatcher::Monitor() takes place the test will instantly
    // fail.
    hang_watcher_.SetAfterMonitorClosureForTesting(base::BindLambdaForTesting([&run_loop] {
    ...
    // place.
    task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), TestTimeouts::tiny_timeout());
    run_loop.Run();
...
TEST_F(HangWatcherPeriodicMonitoringTest, PeriodicCallsTakePlace)
{
    ...
    hang_watcher_.SetAfterMonitorClosureForTesting(BarrierClosure(kMinimumMonitorCount, base::BindLambdaForTesting([&run_loop] {
    ...
    // Register a thread,
    unregister_thread_closure_ = HangWatcher::RegisterThread(base::HangWatcher::ThreadType::kMainThread);
...
public:
    WatchHangsInScopeBlockingTest()
    {
        ...
        hang_watcher_.SetAfterMonitorClosureForTesting(base::BindLambdaForTesting([&] {
    ...
    WatchHangsInScopeBlockingTest(const WatchHangsInScopeBlockingTest& other) = delete;
    WatchHangsInScopeBlockingTest& operator=(const WatchHangsInScopeBlockingTest& other) = delete;
...
#define MAYBE_NewScopeDoesNotBlockDuringCapture DISABLED_NewScopeDoesNotBlockDuringCapture
#else
#define MAYBE_NewScopeDoesNotBlockDuringCapture NewScopeDoesNotBlockDuringCapture
#endif
...
constexpr std::array<HangWatchDeadline::Flag, 3> kAllFlags { { HangWatchDeadline::Flag::kMinValue, HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope,
...
TEST_F(HangWatchDeadlineTest, BitsPreservedThroughExtract)
{
    for (auto bits : { kAllOnes, kAllZeros, kOnesThenZeroes, kZeroesThenOnes }) {
        ASSERT_TRUE((HangWatchDeadline::ExtractFlags(bits) | HangWatchDeadline::ExtractDeadline(bits)) == bits);
    }
}
...
TEST_F(HangWatchDeadlineTest, SetAndClearPersistentFlag)
{
    ...
    ASSERT_TRUE(HangWatchDeadline::IsFlagSet(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, new_flags));

    // No side-effect on deadline.
    ASSERT_EQ(new_deadline, old_deadline);

    // No side-effect on other flags.
    ASSERT_EQ(FlagsMinus(new_flags, HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope), old_flags);
...
TEST_F(HangWatchDeadlineTest, SetShouldBlockOnHangDeadlineChanged)
{
    ...
    const base::TimeTicks new_deadline = base::TimeTicks::FromInternalValue(kArbitraryDeadline);
    ASSERT_NE(deadline, new_deadline);
    deadline_.SetSwitchBitsClosureForTesting(base::BindLambdaForTesting([] { return kArbitraryDeadline; }));
    ...
    ASSERT_FALSE(deadline_.IsFlagSet(HangWatchDeadline::Flag::kShouldBlockOnHang));
...
TEST_F(HangWatchDeadlineTest, ClearIgnoreHangsDeadlineChanged)
{
    ...
    ASSERT_TRUE(HangWatchDeadline::IsFlagSet(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope, flags));

    // Simulate deadline change. Flags are constant.
    const base::TimeTicks new_deadline = base::TimeTicks::FromInternalValue(kArbitraryDeadline);
    ASSERT_NE(deadline, new_deadline);
    deadline_.SetSwitchBitsClosureForTesting(
        base::BindLambdaForTesting([] { return static_cast<uint64_t>(HangWatchDeadline::Flag::kShouldBlockOnHang) | kArbitraryDeadline; }));

    // Clearing kIgnoreHang is unaffected by deadline or flags change.
    deadline_.UnsetIgnoreCurrentWatchHangsInScope();
    ASSERT_FALSE(deadline_.IsFlagSet(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope));
...
TEST_F(HangWatchDeadlineTest, SetIgnoreCurrentHangWatchScopeEnableDeadlineChangedd)
{
    AssertNoFlagsSet();
    ...
    deadline_.SetIgnoreCurrentWatchHangsInScope();
    ASSERT_TRUE(deadline_.IsFlagSet(HangWatchDeadline::Flag::kIgnoreCurrentWatchHangsInScope));
