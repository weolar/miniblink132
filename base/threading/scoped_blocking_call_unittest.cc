...
TEST_F(ScopedBlockingCallTest, MayBlockWillBlock)
{
...

    {
        ...
        ScopedBlockingCall scoped_blocking_call_b(FROM_HERE, BlockingType::WILL_BLOCK);
        testing::Mock::VerifyAndClear(&observer_);
    }
...
{
    EXPECT_CALL(observer_, BlockingStarted(BlockingType::WILL_BLOCK));
    ScopedBlockingCall scoped_blocking_call_a(FROM_HERE, BlockingType::WILL_BLOCK);
    testing::Mock::VerifyAndClear(&observer_);

    {
        ScopedBlockingCall scoped_blocking_call_b(FROM_HERE, BlockingType::MAY_BLOCK);
    }
...
TEST_F(ScopedBlockingCallTest, MayBlockWillBlockTwice)
{
...

    {
        ...
        {
            ScopedBlockingCall scoped_blocking_call_c(FROM_HERE, BlockingType::MAY_BLOCK);
            ScopedBlockingCall scoped_blocking_call_d(FROM_HERE, BlockingType::WILL_BLOCK);
        }
    }
...
{
    auto scoped_blocking_call_a = std::make_unique<ScopedBlockingCall>(FROM_HERE, BlockingType::WILL_BLOCK);
    auto scoped_blocking_call_b = std::make_unique<ScopedBlockingCall>(FROM_HERE, BlockingType::WILL_BLOCK);
...
public:
    explicit ScopedBlockingCallIOJankMonitoringTest(test::TaskEnvironment::TimeSource time_source = test::TaskEnvironment::TimeSource::MOCK_TIME)
    ...
    void SetUp() override
    {
        ...
        EnableIOJankMonitoringForProcess(BindLambdaForTesting([&](int janky_intervals_per_minute, int total_janks_per_minute) {
            reports_.emplace_back(janky_intervals_per_minute, total_janks_per_minute);
        }),
...
{
    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;
    {
    ...
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow - kJankTiming);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, NestedDoesntMatter)
{
    ...
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, ManyInAWindow)
{
    ...
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow - (kJankTiming + kIdleTiming) * 3);
...
{
    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kMonitoringWindow * 3 + internal::IOJankMonitoringWindow::kIOJankInterval * 5;
    ...
    EXPECT_THAT(reports_, ElementsAre(std::make_pair(60, 60), std::make_pair(60, 60), std::make_pair(60, 60), std::make_pair(5, 5)));
}
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, InstantUnblockReportsZero)
{
    ...
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow);
    EXPECT_THAT(reports_, ElementsAre(std::make_pair(0, 0), std::make_pair(0, 0)));
}
...
{
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kIOJankInterval / 3);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, Jank1sMidInterval)
{
    ...
    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kIOJankInterval;
    {
...
{
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kIOJankInterval * 0.9);

    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 1.3;
    {
        ScopedBlockingCall blocked_for_1s(FROM_HERE, BlockingType::MAY_BLOCK);
        task_environment_->FastForwardBy(kJankTiming);
    }
...
{
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kIOJankInterval * 0.5);

    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 1.7;
    {
        ScopedBlockingCall blocked_for_1s(FROM_HERE, BlockingType::MAY_BLOCK);
        task_environment_->FastForwardBy(kJankTiming);
    }
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, NoJankMidInterval)
{
...

    {
        ...
        task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kIOJankInterval - Milliseconds(1));
    }
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, MultiThreaded)
{
    ...
    const int kNumJankyTasks = test::TaskEnvironment::kNumForegroundThreadPoolThreads;

    TestWaitableEvent all_threads_blocked;
    auto on_thread_blocked = BarrierClosure(kNumJankyTasks, BindOnce(&TestWaitableEvent::Signal, Unretained(&all_threads_blocked)));

    TestWaitableEvent resume_all_threads;

    for (int i = 0; i < kNumJankyTasks; ++i) {
        base::ThreadPool::PostTask(FROM_HERE, { MayBlock() }, BindLambdaForTesting([&] {
            ScopedBlockingCall blocked_until_signal(FROM_HERE, BlockingType::MAY_BLOCK);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, MultiThreadedOverlapped)
{
    ...
    static_assert(kNumJankyTasks <= test::TaskEnvironment::kNumForegroundThreadPoolThreads, "");
    ...
    // [0-1]s
    base::ThreadPool::PostTask(FROM_HERE, { MayBlock() }, BindOnce(blocking_task, 0));
    next_task_is_blocked.Wait();
    task_environment_->AdvanceClock(internal::IOJankMonitoringWindow::kIOJankInterval);

    // [1-2]s
    base::ThreadPool::PostTask(FROM_HERE, { MayBlock() }, BindOnce(blocking_task, 1));
    next_task_is_blocked.Wait();
    task_environment_->AdvanceClock(internal::IOJankMonitoringWindow::kIOJankInterval);

    // [2-3]s
    base::ThreadPool::PostTask(FROM_HERE, { MayBlock() }, BindOnce(blocking_task, 2));
    next_task_is_blocked.Wait();
    task_environment_->AdvanceClock(internal::IOJankMonitoringWindow::kIOJankInterval);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, MultiThreadedOverlappedWindows)
{
    ...
    task_environment_->AdvanceClock(internal::IOJankMonitoringWindow::kTimeDiscrepancyTimeout - Milliseconds(1));

    // [10-70]s
    base::ThreadPool::PostTask(FROM_HERE, { MayBlock() }, BindOnce(blocking_task, 0));
    next_task_is_blocked.Wait();
    task_environment_->AdvanceClock(internal::IOJankMonitoringWindow::kMonitoringWindow);
    ...
    EXPECT_THAT(reports_,
        ElementsAre(
            std::make_pair(51, 51), std::make_pair(60, 111), std::make_pair(60, 171), std::make_pair(60, 129), std::make_pair(60, 69), std::make_pair(9, 9)));
}
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, CancellationAcrossSleep)
{
    ...
    task_environment_->AdvanceClock(
        internal::IOJankMonitoringWindow::kMonitoringWindow + internal::IOJankMonitoringWindow::kTimeDiscrepancyTimeout - kJankTiming);
    task_environment_->RunUntilIdle();
    ...
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow - Seconds(1));
    EXPECT_THAT(reports_, ElementsAre());
...
{
    {
        ScopedBlockingCall blocked_through_sleep(FROM_HERE, BlockingType::MAY_BLOCK);

        // Fast-forward 2 full windows and almost to the end of the 3rd.
        task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow * 3 - Seconds(1));

        // Simulate a "sleep" over the timeout threshold.
        task_environment_->AdvanceClock(Seconds(1) + internal::IOJankMonitoringWindow::kTimeDiscrepancyTimeout);
    }

    // Two full jank windows are reported when the ScopedBlokcingCall unwinds but
    // the 3rd is canceled.
    EXPECT_THAT(reports_, ElementsAre(std::make_pair(60, 60), std::make_pair(60, 60)));
    ...
    task_environment_->FastForwardBy(Seconds(1));
    EXPECT_THAT(reports_, ElementsAre(std::make_pair(60, 60), std::make_pair(60, 60), std::make_pair(0, 0)));
}
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, BackgroundBlockingCallsIgnored)
{
    ...
    base::ThreadPool::PostTask(FROM_HERE, { TaskPriority::BEST_EFFORT, MayBlock() }, BindLambdaForTesting([&] {
        ScopedBlockingCall blocked_for_7s(FROM_HERE, BlockingType::MAY_BLOCK);
        task_running.Signal();
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, BackgroundAndForegroundCallsMixed)
{
    constexpr auto kJankTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;

    TestWaitableEvent tasks_running;
    auto on_task_running = BarrierClosure(2, BindOnce(&TestWaitableEvent::Signal, Unretained(&tasks_running)));
    ...
    base::ThreadPool::PostTask(FROM_HERE, { TaskPriority::USER_BLOCKING, MayBlock() }, BindLambdaForTesting([&] {
        ScopedBlockingCall blocked_for_7s(FROM_HERE, BlockingType::MAY_BLOCK);
        on_task_running.Run();
...
{
    constexpr auto kBlockedTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;
    {
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, NestedWillBlockCancelsMonitoring)
{
    constexpr auto kBlockedTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;
    {
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, BaseSyncPrimitivesNotMonitored)
{
    constexpr auto kBlockedTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;
    {
        ...
        internal::ScopedBlockingCallWithBaseSyncPrimitives base_sync_primitives_for_7s(FROM_HERE, BlockingType::MAY_BLOCK);
        task_environment_->FastForwardBy(kBlockedTiming);
    }
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, NestedBaseSyncPrimitivesCancels)
{
    constexpr auto kBlockedTiming = internal::IOJankMonitoringWindow::kIOJankInterval * 7;
    {
        ScopedBlockingCall blocked_for_14s(FROM_HERE, BlockingType::MAY_BLOCK);
...
TEST_F(ScopedBlockingCallIOJankMonitoringTest, RacySampleNearMonitoringWindowBoundary)
{
    constexpr auto kDeltaFromBoundary = Milliseconds(1);
    const int kNumBlockedIntervals = 7;
    constexpr auto kBlockedTiming = internal::IOJankMonitoringWindow::kIOJankInterval * kNumBlockedIntervals;
    ...
    static_assert(kBlockedTiming <= internal::IOJankMonitoringWindow::kTimeDiscrepancyTimeout, "");

    // Start this test near an IOJankMonitoringWindow boundary.
    task_environment_->FastForwardBy(internal::IOJankMonitoringWindow::kMonitoringWindow - kDeltaFromBoundary);

    const int kNumRacingThreads = test::TaskEnvironment::kNumForegroundThreadPoolThreads;

    TestWaitableEvent all_threads_blocked;
    auto on_thread_blocked = BarrierClosure(kNumRacingThreads, BindOnce(&TestWaitableEvent::Signal, Unretained(&all_threads_blocked)));
    ...
    on_thread_blocked = BarrierClosure(kNumRacingThreads, BindOnce(&TestWaitableEvent::Signal, Unretained(&all_threads_blocked)));
