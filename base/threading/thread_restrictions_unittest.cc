...
TEST_F(ThreadRestrictionsTest, ScopedAllowBaseSyncPrimitivesWithBlockingDisallowed)
{
    ScopedDisallowBlocking scoped_disallow_blocking;
    DisallowBaseSyncPrimitives();
    ...
    EXPECT_DCHECK_DEATH({ ScopedAllowBaseSyncPrimitives scoped_allow_base_sync_primitives; });
}

TEST_F(ThreadRestrictionsTest, ScopedAllowBaseSyncPrimitivesOutsideBlockingScope)
{
    ScopedDisallowBlocking scoped_disallow_blocking;
    DisallowBaseSyncPrimitives();
    ScopedAllowBaseSyncPrimitivesOutsideBlockingScope scoped_allow_base_sync_primitives;
    internal::AssertBaseSyncPrimitivesAllowed();
}

TEST_F(ThreadRestrictionsTest, ScopedAllowBaseSyncPrimitivesOutsideBlockingScopeResetsState)
{
    DisallowBaseSyncPrimitives();
    {
        ScopedAllowBaseSyncPrimitivesOutsideBlockingScope scoped_allow_base_sync_primitives;
...
{
    DisallowBaseSyncPrimitives();
    ScopedAllowBaseSyncPrimitivesForTesting scoped_allow_base_sync_primitives_for_testing;
    internal::AssertBaseSyncPrimitivesAllowed();
}

TEST_F(ThreadRestrictionsTest, ScopedAllowBaseSyncPrimitivesForTestingResetsState)
{
    DisallowBaseSyncPrimitives();
    {
        ScopedAllowBaseSyncPrimitivesForTesting scoped_allow_base_sync_primitives_for_testing;
...
TEST_F(ThreadRestrictionsTest, ScopedAllowBaseSyncPrimitivesForTestingWithBlockingDisallowed)
{
    ScopedDisallowBlocking scoped_disallow_blocking;
    DisallowBaseSyncPrimitives();
...
#if !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_ANDROID) && DCHECK_IS_ON() && defined(GTEST_HAS_DEATH_TEST)

TEST_F(ThreadRestrictionsTest, BlockingCheckEmitsStack)
{
    debug::OverrideStackTraceOutputForTesting enable_stacks_in_death_tests(debug::OverrideStackTraceOutputForTesting::Mode::kForceOutput);
    ScopedDisallowBlocking scoped_disallow_blocking;
    // The above ScopedDisallowBlocking should be on the blame list for who set
    // the ban.
    EXPECT_DEATH(
        { AssertBlockingAllowed(); }, EXPENSIVE_DCHECKS_ARE_ON() && debug::StackTrace::WillSymbolizeToStreamForTesting() ? "ScopedDisallowBlocking" : "");
    // And the stack should mention this test body as source.
    EXPECT_DEATH(
        { AssertBlockingAllowed(); }, EXPENSIVE_DCHECKS_ARE_ON() && debug::StackTrace::WillSymbolizeToStreamForTesting() ? "BlockingCheckEmitsStack" : "");
}

class TestCustomDisallow {
public:
    NOINLINE TestCustomDisallow()
    {
        DisallowBlocking();
    }
    NOINLINE ~TestCustomDisallow()
    {
        PermanentThreadAllowance::AllowBlocking();
    }
};
...
TEST_F(ThreadRestrictionsTest, NestedAllowRestoresPreviousStack)
{
    ...
    // no longer relevant).
    EXPECT_DEATH({ AssertBlockingAllowed(); }, EXPENSIVE_DCHECKS_ARE_ON() && debug::StackTrace::WillSymbolizeToStreamForTesting() ? "TestCustomDisallow" : "");
    // And the stack should mention this test body as source.
    EXPECT_DEATH({ AssertBlockingAllowed(); },
        EXPENSIVE_DCHECKS_ARE_ON() && debug::StackTrace::WillSymbolizeToStreamForTesting() ? "NestedAllowRestoresPreviousStack" : "");
}
