...
TEST(PlatformThreadWinTest, SetBackgroundThreadModeFailsInIdlePriorityProcess)
{
    ...
    EXPECT_EQ(::GetPriorityClass(Process::Current().Handle()), static_cast<DWORD>(NORMAL_PRIORITY_CLASS));
    ::SetPriorityClass(Process::Current().Handle(), IDLE_PRIORITY_CLASS);
    EXPECT_EQ(Process::Current().GetOSPriority(), static_cast<int>(IDLE_PRIORITY_CLASS));
    ...
    // *not* be the case.
    const int priority_after_thread_mode_background_begin = ::GetThreadPriority(thread_handle);
    EXPECT_EQ(priority_after_thread_mode_background_begin, THREAD_PRIORITY_NORMAL);
    ...
    EXPECT_EQ(::GetThreadPriority(thread_handle), priority_after_thread_mode_background_begin);
    internal::AssertMemoryPriority(thread_handle, MEMORY_PRIORITY_VERY_LOW);
...
namespace {
class MemoryPriorityAssertingThreadDelegate : public base::PlatformThread::Delegate {
public:
