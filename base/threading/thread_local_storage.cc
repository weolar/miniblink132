...
std::atomic<PlatformThreadLocalStorage::TLSKey> g_native_tls_key { PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES };
...
static_assert(static_cast<int>(TlsVectorState::kMaxValue) <= kVectorStateBitMask, "number of states must fit in header");
...
void SetTlsVectorValue(PlatformThreadLocalStorage::TLSKey key, TlsVectorEntry* tls_data, TlsVectorState state)
{
    DCHECK(tls_data || (state == TlsVectorState::kUninitialized) || (state == TlsVectorState::kDestroyed));
    PlatformThreadLocalStorage::SetTLSValue(key, reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(tls_data) | static_cast<uintptr_t>(state)));
}

// Returns the tls vector and current state from the raw tls value.
TlsVectorState GetTlsVectorStateAndValue(void* tls_value, TlsVectorEntry** entry = nullptr)
{
    if (entry) {
        *entry = reinterpret_cast<TlsVectorEntry*>(reinterpret_cast<uintptr_t>(tls_value) & ~kVectorStateBitMask);
    }
    return static_cast<TlsVectorState>(reinterpret_cast<uintptr_t>(tls_value) & kVectorStateBitMask);
}

// Returns the tls vector and state using the tls key.
TlsVectorState GetTlsVectorStateAndValue(PlatformThreadLocalStorage::TLSKey key, TlsVectorEntry** entry = nullptr)
{
...
// (_os_tsd_get_direct() is used by pthread_getspecific() internally).
    ...
    static_assert(std::is_same_v<PlatformThreadLocalStorage::TLSKey, pthread_key_t>,
    ...
    return GetTlsVectorStateAndValue(reinterpret_cast<void*>(platform_tls_value), entry);
#else
...
TlsVectorEntry* ConstructTlsVector()
{
    ...
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
        ...
        PlatformThreadLocalStorage::TLSKey old_key = PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES;
        if (!g_native_tls_key.compare_exchange_strong(old_key, key, std::memory_order_relaxed, std::memory_order_relaxed)) {
...
void OnThreadExitInternal(TlsVectorEntry* tls_data)
{
    ...
    while (need_to_scan_destructors) {
        ...
        std::sort(std::begin(slot_destruction_order), std::end(slot_destruction_order),
            [](const OrderedSlot& s1, const OrderedSlot& s2) { return s1.sequence_num > s2.sequence_num; });
        ...
        for (const auto& ordered_slot : slot_destruction_order) {
            ...
            if (!tls_value || tls_metadata[slot].status == TlsStatus::FREE || stack_allocated_tls_data[slot].version != tls_metadata[slot].version)
                continue;
...
{
    PlatformThreadLocalStorage::TLSKey key = g_native_tls_key.load(std::memory_order_relaxed);
    if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES || GetTlsVectorStateAndValue(key) == TlsVectorState::kUninitialized) {
        ConstructTlsVector();
    }
...
void* ThreadLocalStorage::Slot::Get() const
{
    ...
    const TlsVectorState state = GetTlsVectorStateAndValue(g_native_tls_key.load(std::memory_order_relaxed), &tls_data);
    DCHECK_NE(state, TlsVectorState::kDestroyed);
    if (!tls_data)
        return nullptr;
    DCHECK_LT(slot_, kThreadLocalStorageSize);
