...
template <typename CounterType> class IncrementThread : public SimpleThread {
public:
    ...
    explicit IncrementThread(WaitableEvent* start_event, CounterType* counter, OnceClosure done_closure)
...
template <typename CounterType> void RunIncrementPerfTest(const std::string& story_name, int num_threads)
{
    WaitableEvent start_event;
    WaitableEvent end_event;
    CounterType counter;
    RepeatingClosure done_closure = BarrierClosure(num_threads, BindOnce(&WaitableEvent::Signal, Unretained(&end_event)));

    std::vector<std::unique_ptr<IncrementThread<CounterType>>> threads;
    for (int i = 0; i < num_threads; ++i) {
        threads.push_back(std::make_unique<IncrementThread<CounterType>>(&start_event, &counter, done_closure));
        threads.back()->Start();
    }
    ...
    reporter.AddResult(kMetricOperationThroughput, kNumIterations / (end_time - start_time).InMillisecondsF());
...
{
    RunIncrementPerfTest<AtomicUint64_NoBarrier>("AtomicUint64_NoBarrier_1Thread", 1);
}

TEST(CounterPerfTest, AtomicUint64_NoBarrier_4Threads)
{
    RunIncrementPerfTest<AtomicUint64_NoBarrier>("AtomicUint64_NoBarrier_4Threads", 4);
}
...
{
    RunIncrementPerfTest<AtomicUint64_Barrier>("AtomicUint64_Barrier_4Threads", 4);
}
