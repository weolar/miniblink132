...
public:
    ThreadPerfTest()
        : done_(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    ...
    {
        base::WaitableEvent done(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
        base::ThreadTicks ticks;
        thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&ThreadPerfTest::TimeOnThread, base::Unretained(this), &ticks, &done));
        done.Wait();
        return ticks;
    }
...
class TaskPerfTest : public ThreadPerfTest {
    ...
    void PingPong(int hops) override
    {
        ...
        NextThread(hops)->task_runner()->PostTask(FROM_HERE, base::BindOnce(&ThreadPerfTest::PingPong, base::Unretained(this), hops - 1));
    }
};
...
public:
    void Init() override
    {
        TaskPerfTest::Init();
        for (auto& i : threads_) {
            i->task_runner()->PostTask(FROM_HERE,
                BindOnce([](MessageLoopObserver* observer) { CurrentThread::Get()->AddTaskObserver(observer); }, Unretained(&message_loop_observer)));
        }
    }
};
...
{
    RunPingPongTest(std::string(kStoryBaseTaskWithObserver) + kStorySuffixOneThread, 1);
    RunPingPongTest(std::string(kStoryBaseTaskWithObserver) + kStorySuffixFourThreads, 4);
}
...
template <typename WaitableEventType> class EventPerfTest : public ThreadPerfTest {
public:
    void Init() override
    {
        for (size_t i = 0; i < threads_.size(); i++) {
            events_.push_back(std::make_unique<WaitableEventType>(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED));
        }
    }
    ...
    void PingPong(int hops) override
    {
        ...
        for (size_t i = 0; i < threads_.size(); i++) {
            threads_[i]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&EventPerfTest::WaitAndSignalOnThread, base::Unretained(this), i));
        }
...
{
    RunPingPongTest(std::string(kStoryBaseWaitableEvent) + kStorySuffixFourThreads, 4);
}

// Build a minimal event using ConditionVariable.
class ConditionVariableEvent {
public:
    ConditionVariableEvent(WaitableEvent::ResetPolicy reset_policy, WaitableEvent::InitialState initial_state)
        : cond_(&lock_)
        , signaled_(false)
    {
...
class PthreadEvent {
public:
    PthreadEvent(WaitableEvent::ResetPolicy reset_policy, WaitableEvent::InitialState initial_state)
    {
        DCHECK_EQ(WaitableEvent::ResetPolicy::AUTOMATIC, reset_policy);
        DCHECK_EQ(WaitableEvent::InitialState::NOT_SIGNALED, initial_state);
...
{
    RunPingPongTest(std::string(kStoryBasePthreadCondVar) + kStorySuffixFourThreads, 4);
}
