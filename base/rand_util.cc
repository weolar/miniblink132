...
std::string RandBytesAsString(size_t length)
{
    ...
    RandBytes(
        //as_writable_byte_span(result)
        span<uint8_t>((uint8_t*)result.data(), result.size())
    );
    return result;
}
...
std::vector<uint8_t> RandBytesAsVector(size_t length)
{
    ...
    RandBytes(span<uint8_t>((uint8_t*)result.data(), result.size()));
    return result;
}
...
// uint64_t RandGenerator(uint64_t range)
// {
//     return ::base::RandGenerator(range);
// }
// 
// void RandBytes(void* output, size_t output_length)
// {
//     ::base::RandBytes(::base::span<uint8_t>((uint8_t*)output, output_length));
// }
// 
// using InsecureRandomGenerator = ::base::InsecureRandomGenerator;
// 
// }
