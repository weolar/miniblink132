...
public:
    explicit StringViewOrString(std::string_view piece)
        : piece_(piece)
    {
    }
    explicit StringViewOrString(std::string str)
        : str_(std::move(str))
    {
    }
...
std::optional<StringViewOrString> Base64UrlToBase64(std::string_view input, Base64UrlDecodePolicy policy)
{
...
void Base64UrlEncode(span<const uint8_t> input, Base64UrlEncodePolicy policy, std::string* output)
{
...
void Base64UrlEncode(std::string_view input, Base64UrlEncodePolicy policy, std::string* output)
{
    Base64UrlEncode(base::as_byte_span(input), policy, output);
}

bool Base64UrlDecode(std::string_view input, Base64UrlDecodePolicy policy, std::string* output)
{
    std::optional<StringViewOrString> base64_input = Base64UrlToBase64(input, policy);
    if (!base64_input) {
        return false;
    }
...
std::optional<std::vector<uint8_t>> Base64UrlDecode(std::string_view input, Base64UrlDecodePolicy policy)
{
    std::optional<StringViewOrString> base64_input = Base64UrlToBase64(input, policy);
    if (!base64_input) {
        return std::nullopt;
