    ...
    : policy_(reset_policy)
{
    ...
    mach_port_t name;
    kern_return_t kr = mach_port_construct(mach_task_self(), &options, /*context=*/0, &name);
...
void WaitableEvent::SignalImpl()
{
    ...
    // has a length of one.
    kern_return_t kr = mach_msg(&msg.header, MACH_SEND_MSG | MACH_SEND_TIMEOUT, sizeof(msg),
...
bool WaitableEvent::TimedWaitImpl(TimeDelta wait_delta)
{
    ...
    const TimeTicks end_time = wait_delta.is_max() ? TimeTicks::Max() : subtle::TimeTicksNowIgnoringOverride() + wait_delta;
    // Fake |kr| value to bootstrap the for loop.
    kern_return_t kr = MACH_RCV_INTERRUPTED;
    for (mach_msg_timeout_t timeout = wait_delta.is_max() ? MACH_MSG_TIMEOUT_NONE : saturated_cast<mach_msg_timeout_t>(wait_delta.InMillisecondsRoundedUp());
         ...
         timeout = end_time.is_max() ? MACH_MSG_TIMEOUT_NONE
                                     : std::max(mach_msg_timeout_t { 0 },
                                         saturated_cast<mach_msg_timeout_t>((end_time - subtle::TimeTicksNowIgnoringOverride()).InMillisecondsRoundedUp()))) {
        kr = mach_msg(&msg.header, options, /*send_size=*/0, rcv_size, receive_right_->Name(), timeout, /*notify=*/MACH_PORT_NULL);
    }
...
// static
size_t WaitableEvent::WaitManyImpl(WaitableEvent** raw_waitables, size_t count)
{
    ...
    if (kPrimitive == KQUEUE) {
        ...
        for (size_t i = 0; i < count; ++i) {
            EV_SET64(&events[i], raw_waitables[i]->receive_right_->Name(), EVFILT_MACHPORT, EV_ADD, 0, 0, i, 0, 0);
        }
        ...
        int rv = HANDLE_EINTR(kevent64(wait_many.get(), events.data(), count_int, out_events.data(), count_int, /*flags=*/0,
            /*timeout=*/nullptr));
        ...
        for (size_t i = 0; i < count; ++i) {
            kr = mach_port_insert_member(mach_task_self(), raw_waitables[i]->receive_right_->Name(), port_set.get());
        ...
        kr = mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_LARGE | MACH_RCV_LARGE_IDENTITY,
...
bool WaitableEvent::PeekPort(mach_port_t port, bool dequeue)
{
    if (dequeue) {
        ...
        kern_return_t kr = mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, /*send_size=*/0, sizeof(msg), port, /*timeout=*/0, /*notify=*/MACH_PORT_NULL);
        if (kr == KERN_SUCCESS) {
            return true;
        } else {
            MACH_CHECK(kr == MACH_RCV_TIMED_OUT, kr) << "mach_msg";
        ...
        kern_return_t kr = mach_port_peek(mach_task_self(), port, MACH_RCV_TRAILER_TYPE(MACH_RCV_TRAILER_NULL), &seqno, &size, &id,
            reinterpret_cast<mach_msg_trailer_info_t>(&trailer), &trailer_size);
        if (kr == KERN_SUCCESS) {
            return true;
        } else {
