...
bool WaitableEvent::TimedWaitImpl(TimeDelta wait_delta)
{
    ...
    for (TimeDelta remaining = wait_delta; remaining.is_positive() && !sw.fired();
         remaining = end_time.is_max() ? TimeDelta::Max() : end_time - subtle::TimeTicksNowIgnoringOverride()) {
        if (end_time.is_max())
            sw.cv()->Wait();
        else
...
cmp_fst_addr(const std::pair<WaitableEvent*, unsigned>& a, const std::pair<WaitableEvent*, unsigned>& b)
{
...
size_t WaitableEvent::WaitManyImpl(WaitableEvent** raw_waitables, size_t count) NO_THREAD_SAFETY_ANALYSIS
{
...
// static
// NO_THREAD_SAFETY_ANALYSIS: Complex control flow.
size_t WaitableEvent::EnqueueMany(std::pair<WaitableEvent*, size_t>* waitables, size_t count, Waiter* waiter) NO_THREAD_SAFETY_ANALYSIS
{
    size_t winner = count;
...
WaitableEvent::WaitableEventKernel::WaitableEventKernel(ResetPolicy reset_policy, InitialState initial_state)
