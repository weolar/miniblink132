...
template <class LockType> class SCOPED_LOCKABLE BasicAutoLock {
    STACK_ALLOCATED();

public:
    struct AlreadyAcquired { };

    explicit BasicAutoLock(LockType& lock, subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_LOCK_FUNCTION(lock)
        : lock_(lock)
    {
...
// type.
template <class LockType> class SCOPED_LOCKABLE BasicMovableAutoLock {
public:
    explicit BasicMovableAutoLock(LockType& lock, subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_LOCK_FUNCTION(lock)
        : lock_(&lock)
    {
        lock_->Acquire(tracking);
    }
...
template <class LockType> class SCOPED_LOCKABLE BasicAutoTryLock {
    STACK_ALLOCATED();
...
public:
    ...
    explicit BasicAutoTryLock(LockType& lock, subtle::LockTracking tracking = subtle::LockTracking::kDisabled) LOCKS_EXCLUDED(lock)
        : lock_(lock)
        , is_acquired_(lock_.Try(tracking))
    {
    }
...
template <class LockType> class BasicAutoUnlock {
    STACK_ALLOCATED();
...
template <class LockType> class SCOPED_LOCKABLE BasicAutoLockMaybe {
    STACK_ALLOCATED();

public:
    explicit BasicAutoLockMaybe(LockType* lock, subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_LOCK_FUNCTION(lock)
        : lock_(lock)
    {
        if (lock_)
            lock_->Acquire(tracking);
...
// type.
template <class LockType> class SCOPED_LOCKABLE BasicReleasableAutoLock {
    STACK_ALLOCATED();

public:
    explicit BasicReleasableAutoLock(LockType* lock, subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_LOCK_FUNCTION(lock)
        : lock_(lock)
    {
        DCHECK(lock_);
        lock_->Acquire(tracking);
