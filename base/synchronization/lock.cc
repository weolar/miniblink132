...
thread_local std::array<uintptr_t, kHeldLocksCapacity> g_tracked_locks_held_by_thread;
...
void Lock::AddToLocksHeldOnCurrentThread()
{
    ...
    if (g_num_tracked_locks_held_by_thread >= kHeldLocksCapacity) {
        CHECK(false) << "This thread holds more than " << kHeldLocksCapacity
    ...
    g_tracked_locks_held_by_thread[g_num_tracked_locks_held_by_thread] = reinterpret_cast<uintptr_t>(this);
...
void Lock::RemoveFromLocksHeldOnCurrentThread()
{
    ...
    for (size_t i = 0; i < g_num_tracked_locks_held_by_thread; ++i) {
        ...
        if (g_tracked_locks_held_by_thread[index] == reinterpret_cast<uintptr_t>(this)) {
            g_tracked_locks_held_by_thread[index] = g_tracked_locks_held_by_thread[g_num_tracked_locks_held_by_thread - 1];
            g_tracked_locks_held_by_thread[g_num_tracked_locks_held_by_thread - 1] = reinterpret_cast<uintptr_t>(nullptr);
            --g_num_tracked_locks_held_by_thread;
            break;
        }
    }
...
{
    return span<const uintptr_t>(g_tracked_locks_held_by_thread.begin(), g_num_tracked_locks_held_by_thread);
}
