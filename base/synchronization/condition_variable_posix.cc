...
BASE_FEATURE(kSkipConditionVariableWakeupHack, "SkipConditionVariableWakeupHack", base::FEATURE_ENABLED_BY_DEFAULT);
...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !defined(HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC)
...
{
    g_skip_wakeup_hack.store(base::FeatureList::IsEnabled(kSkipConditionVariableWakeupHack), std::memory_order_relaxed);
}
#endif
...
void ConditionVariable::TimedWait(const TimeDelta& max_time)
{
    ...
    relative_time.tv_sec = static_cast<time_t>(usecs / Time::kMicrosecondsPerSecond);
