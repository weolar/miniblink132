...
public:
    AsyncWaiter(scoped_refptr<SequencedTaskRunner> task_runner, base::OnceClosure callback, Flag* flag)
...
// -----------------------------------------------------------------------------
void AsyncCallbackHelper(Flag* flag, WaitableEventWatcher::EventCallback callback, WaitableEvent* event)
{
...
bool WaitableEventWatcher::StartWatching(WaitableEvent* event, EventCallback callback, scoped_refptr<SequencedTaskRunner> task_runner)
{
    ...
    OnceClosure internal_callback
        = base::BindOnce(&AsyncCallbackHelper, base::RetainedRef(cancel_flag_), std::move(callback), base::UnsafeDanglingUntriaged(event));
    ...
    kernel_ = kernel;
    waiter_ = new AsyncWaiter(std::move(task_runner), std::move(internal_callback), cancel_flag_.get());
