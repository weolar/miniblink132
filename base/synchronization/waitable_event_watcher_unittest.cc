...
class WaitableEventWatcherTest : public testing::TestWithParam<test::TaskEnvironment::MainThreadType> { };
...
TEST_P(WaitableEventWatcherTest, BasicSignalManual)
{
    ...
    WaitableEvent event(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);

    WaitableEventWatcher watcher;
    watcher.StartWatching(&event, BindOnce(&QuitWhenSignaled, loop.QuitWhenIdleClosure()), SequencedTaskRunner::GetCurrentDefault());
...
TEST_P(WaitableEventWatcherTest, BasicCancel)
{
    ...
    WaitableEventWatcher watcher;
    watcher.StartWatching(&event, DoNothing(), SequencedTaskRunner::GetCurrentDefault());
...
TEST_P(WaitableEventWatcherTest, CancelAfterSet)
{
    ...
    WaitableEventWatcher::EventCallback callback = BindOnce(&DecrementCountContainer::OnWaitableEventSignaled, Unretained(&delegate));
    watcher.StartWatching(&event, std::move(callback), SequencedTaskRunner::GetCurrentDefault());
...
TEST_P(WaitableEventWatcherTest, SignaledAtStartManual)
{
    test::TaskEnvironment task_environment(GetParam());
    base::RunLoop loop;
    WaitableEvent event(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::SIGNALED);
...
TEST_P(WaitableEventWatcherTest, SignaledAtStartAutomatic)
{
    test::TaskEnvironment task_environment(GetParam());
    base::RunLoop loop;
    WaitableEvent event(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::SIGNALED);
...
TEST_P(WaitableEventWatcherTest, MultipleWatchersManual)
{
    ...
    auto callback = [&run_loop, &total_counter](int* watcher_counter, WaitableEvent*) {
        ++(*watcher_counter);
        if (++total_counter == 2) {
            run_loop.Quit();
        }
    ...
    watcher1.StartWatching(&event, BindOnce(BindLambdaForTesting(callback), Unretained(&watcher1_counter)), SequencedTaskRunner::GetCurrentDefault());

    WaitableEventWatcher watcher2;
    watcher2.StartWatching(&event, BindOnce(BindLambdaForTesting(callback), Unretained(&watcher2_counter)), SequencedTaskRunner::GetCurrentDefault());
...
TEST_P(WaitableEventWatcherTest, MultipleWatchersAutomatic)
{
    ...
    WaitableEventWatcher watcher1;
    watcher1.StartWatching(&event, BindOnce(callback, Unretained(&current_run_loop), Unretained(&counter1)), SequencedTaskRunner::GetCurrentDefault());

    WaitableEventWatcher watcher2;
    watcher2.StartWatching(&event, BindOnce(callback, Unretained(&current_run_loop), Unretained(&counter2)), SequencedTaskRunner::GetCurrentDefault());
...
class WaitableEventWatcherDeletionTest : public testing::TestWithParam<std::tuple<test::TaskEnvironment::MainThreadType, bool>> { };
...
TEST_P(WaitableEventWatcherDeletionTest, DeleteUnder)
{
...

    {
        ...
        auto* event = new WaitableEvent(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
...
TEST_P(WaitableEventWatcherDeletionTest, SignalAndDelete)
{
...

    {
        ...
        auto event = std::make_unique<WaitableEvent>(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);

        watcher.StartWatching(event.get(), BindOnce(&QuitWhenSignaled, loop.QuitWhenIdleClosure()), SequencedTaskRunner::GetCurrentDefault());
        event->Signal();
        event.reset();
...
TEST_P(WaitableEventWatcherDeletionTest, DeleteWatcherBeforeCallback)
{
    ...
    test::TaskEnvironment task_environment(main_thread_type);
    scoped_refptr<SingleThreadTaskRunner> task_runner = SingleThreadTaskRunner::GetCurrentDefault();
    ...
    WaitableEventWatcher::EventCallback watcher_callback
        = BindOnce([](bool* did_callback, WaitableEvent*) { *did_callback = true; }, Unretained(&did_callback));

    task_runner->PostTask(FROM_HERE,
        BindOnce(IgnoreResult(&WaitableEventWatcher::StartWatching), Unretained(watcher.get()), Unretained(&event), std::move(watcher_callback), task_runner));
    task_runner->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&event)));
    task_runner->DeleteSoon(FROM_HERE, std::move(watcher));
    if (delay_after_delete) {
        task_runner->PostTask(FROM_HERE, BindOnce(&PlatformThread::Sleep, Milliseconds(30)));
    }
...
INSTANTIATE_TEST_SUITE_P(All, WaitableEventWatcherTest, testing::ValuesIn(testing_main_threads));

INSTANTIATE_TEST_SUITE_P(All, WaitableEventWatcherDeletionTest, testing::Combine(testing::ValuesIn(testing_main_threads), testing::Bool()));
