...
WaitableEvent::~WaitableEvent()
{
    ...
    if (!only_used_while_idle_) {
        ...
        static const uint8_t* flow_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED("wakeup.flow,toplevel.flow");
        if (*flow_enabled && IsSignaled()) {
            TRACE_EVENT_INSTANT("wakeup.flow,toplevel.flow", "~WaitableEvent while Signaled", perfetto::TerminatingFlow::FromPointer(this));
        }
    }
...
void WaitableEvent::Signal()
{
    ...
    if (!only_used_while_idle_) {
        TRACE_EVENT_INSTANT("wakeup.flow,toplevel.flow", "WaitableEvent::Signal", perfetto::Flow::FromPointer(this));
    }
    SignalImpl();
}
...
bool WaitableEvent::TimedWait(TimeDelta wait_delta)
{
    ...
    std::optional<internal::ScopedBlockingCallWithBaseSyncPrimitives> scoped_blocking_call;
    if (!only_used_while_idle_) {
    ...
    if (result && !only_used_while_idle_) {
        TRACE_EVENT_INSTANT("wakeup.flow,toplevel.flow", "WaitableEvent::Wait Complete", perfetto::TerminatingFlow::FromPointer(this));
    }
...
size_t WaitableEvent::WaitMany(WaitableEvent** events, size_t count)
{
    ...
    internal::ScopedBlockingCallWithBaseSyncPrimitives scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    if (!signaled_event->only_used_while_idle_) {
        TRACE_EVENT_INSTANT("wakeup.flow,toplevel.flow", "WaitableEvent::WaitMany Complete", perfetto::TerminatingFlow::FromPointer(signaled_event));
    }
    return signaled_id;
}
