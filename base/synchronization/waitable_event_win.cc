...
[[nodiscard]] debug::ScopedCrashKeyString SetLastErrorCrashKey(DWORD last_error)
{
    static auto* const key = debug::AllocateCrashKeyString("WaitableEvent-last_error", debug::CrashKeySize::Size32);
...
WaitableEvent::WaitableEvent(ResetPolicy reset_policy, InitialState initial_state)
    : handle_(CreateEvent(nullptr, reset_policy == ResetPolicy::MANUAL, initial_state == InitialState::SIGNALED, nullptr))
{
...
bool WaitableEvent::TimedWaitImpl(TimeDelta wait_delta)
{
    ...
    for (TimeDelta remaining = wait_delta; remaining.is_positive(); remaining = end_time - subtle::TimeTicksNowIgnoringOverride()) {
        ...
        const DWORD timeout_ms = remaining.is_max() ? INFINITE : saturated_cast<DWORD>(remaining.InMillisecondsRoundedUp());
        ...
        if (result == WAIT_FAILED) {
            debug::ScopedCrashKeyString last_error_key = SetLastErrorCrashKey(::GetLastError());
            NOTREACHED();
        }
...
size_t WaitableEvent::WaitManyImpl(WaitableEvent** events, size_t count)
{
    ...
    CHECK_LE(count, static_cast<size_t>(MAXIMUM_WAIT_OBJECTS)) << "Can only wait on " << MAXIMUM_WAIT_OBJECTS << " with WaitMany";

    for (size_t i = 0; i < count; ++i)
        handles[i] = events[i]->handle();

    // The cast is safe because count is small - see the CHECK above.
    DWORD result = WaitForMultipleObjects(static_cast<DWORD>(count), handles,
