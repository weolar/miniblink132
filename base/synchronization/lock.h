...
public:
    ...
    void Acquire(subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_LOCK_FUNCTION()
    {
        lock_.Lock();
    }
    ...
    // assertion may fail).
    bool Try(subtle::LockTracking tracking = subtle::LockTracking::kDisabled) EXCLUSIVE_TRYLOCK_FUNCTION(true)
    {
        return lock_.Try();
    }
