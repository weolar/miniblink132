...
bool WaitableEventWatcher::StartWatching(WaitableEvent* event, EventCallback callback, scoped_refptr<SequencedTaskRunner> task_runner)
{
    DCHECK(task_runner->RunsTasksInCurrentSequence());
    DCHECK(!storage_->dispatch_source || dispatch_source_testcancel(storage_->dispatch_source.get()));
    ...
    callback_ = BindOnce(std::move(callback), base::UnsafeDanglingUntriaged(event));
    ...
    storage_->dispatch_source.reset(
        dispatch_source_create(DISPATCH_SOURCE_TYPE_MACH_RECV, receive_right_->Name(), 0, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)));
    ...
    const bool auto_reset = event->policy_ == WaitableEvent::ResetPolicy::AUTOMATIC;
    ...
    dispatch_source_set_event_handler(storage_->dispatch_source.get(), ^{
        ...
        task_runner->PostTask(FROM_HERE, BindOnce(&WaitableEventWatcher::InvokeCallback, weak_this));
    });
