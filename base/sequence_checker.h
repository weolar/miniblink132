...
#define DCHECK_CALLED_ON_VALID_SEQUENCE(name, ...) base::ScopedValidateSequenceChecker BASE_UNIQUIFY(scoped_validate_sequence_checker_)(name, ##__VA_ARGS__)
...
public:
    ...
    SequenceCheckerDoNothing& operator=(SequenceCheckerDoNothing&& other) = default;
...
public:
    explicit ScopedValidateSequenceChecker(const SequenceChecker& checker) EXCLUSIVE_LOCK_FUNCTION(checker);
