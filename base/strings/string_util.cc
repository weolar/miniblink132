        ...
        if (*position == '%') {
            ...
            while (in_specification) {
                ...
                if (*position == 'l') {
                    ...
                    modifier_l = true;
                } else if (((*position == 's' || *position == 'c') && !modifier_l) || *position == 'S' || *position == 'C' || *position == 'F'
...
bool ReplaceChars(std::u16string_view input, std::u16string_view replace_chars, std::u16string_view replace_with, std::u16string* output)
{
    return internal::ReplaceCharsT(input, replace_chars, replace_with, output);
}

bool ReplaceChars(std::string_view input, std::string_view replace_chars, std::string_view replace_with, std::string* output)
{
    return internal::ReplaceCharsT(input, replace_chars, replace_with, output);
}

bool RemoveChars(std::u16string_view input, std::u16string_view remove_chars, std::u16string* output)
{
    return internal::ReplaceCharsT(input, remove_chars, std::u16string_view(), output);
}

bool RemoveChars(std::string_view input, std::string_view remove_chars, std::string* output)
{
    return internal::ReplaceCharsT(input, remove_chars, std::string_view(), output);
}
...
std::u16string_view TrimString(std::u16string_view input, std::u16string_view trim_chars, TrimPositions positions)
{
...
void TruncateUTF8ToByteSize(const std::string& input, const size_t byte_size, std::string* output)
{
    ...
    while (char_index >= 0) {
        ...
        CBU8_NEXT(reinterpret_cast<const uint8_t*>(data), char_index, truncation_length, code_point);
...
TrimPositions TrimWhitespace(std::u16string_view input, TrimPositions positions, std::u16string* output)
{
    return internal::TrimStringT(input, std::u16string_view(kWhitespaceUTF16), positions, output);
}

std::u16string_view TrimWhitespace(std::u16string_view input, TrimPositions positions)
{
    return internal::TrimStringPieceT(input, std::u16string_view(kWhitespaceUTF16), positions);
}

TrimPositions TrimWhitespaceASCII(std::string_view input, TrimPositions positions, std::string* output)
{
    return internal::TrimStringT(input, std::string_view(kWhitespaceASCII), positions, output);
}

std::string_view TrimWhitespaceASCII(std::string_view input, TrimPositions positions)
{
    return internal::TrimStringPieceT(input, std::string_view(kWhitespaceASCII), positions);
}
...
bool ContainsOnlyChars(std::u16string_view input, std::u16string_view characters)
{
...
bool StartsWith(std::u16string_view str, std::u16string_view search_for, CompareCase case_sensitivity)
{
    return internal::StartsWithT(str, search_for, case_sensitivity);
}
...
bool EndsWith(std::u16string_view str, std::u16string_view search_for, CompareCase case_sensitivity)
{
    return internal::EndsWithT(str, search_for, case_sensitivity);
}
...
char HexDigitToInt(char c)
{
    ...
    return (c >= 'A' && c <= 'F') ? static_cast<char>(c - 'A' + 10) : static_cast<char>(c - 'a' + 10);
}
...
std::u16string FormatBytesUnlocalized(int64_t bytes)
{
    ...
    if (bytes != 0 && dimension > 0 && unit_amount < 100) {
        base::snprintf(buf, std::size(buf), "%.1lf%s", unit_amount, kByteStringsUnlocalized[dimension]);
    } else {
        base::snprintf(buf, std::size(buf), "%.0lf%s", unit_amount, kByteStringsUnlocalized[dimension]);
    }

    return ASCIIToUTF16(buf);
}

void ReplaceFirstSubstringAfterOffset(std::u16string* str, size_t start_offset, std::u16string_view find_this, std::u16string_view replace_with)
{
    internal::DoReplaceMatchesAfterOffset(str, start_offset, internal::MakeSubstringMatcher(find_this), replace_with, internal::ReplaceType::REPLACE_FIRST);
}
...
void ReplaceSubstringsAfterOffset(std::u16string* str, size_t start_offset, std::u16string_view find_this, std::u16string_view replace_with)
{
    internal::DoReplaceMatchesAfterOffset(str, start_offset, internal::MakeSubstringMatcher(find_this), replace_with, internal::ReplaceType::REPLACE_ALL);
}
...
std::string JoinString(span<const std::string> parts, std::string_view separator)
{
    return internal::JoinStringT(parts, separator);
}

std::string JoinString(std::vector<std::string_view> parts, std::string_view separator)
{
    std::vector<std::string> parts_copy;
    for (size_t i = 0 ; i < parts.size(); ++i) {
        parts_copy.push_back(std::string(parts[i]));
    }
    JoinString(parts_copy, separator);
}

std::u16string JoinString(span<const std::u16string> parts, std::u16string_view separator)
{
    return internal::JoinStringT(parts, separator);
}

std::string JoinString(span<const std::string_view> parts, std::string_view separator)
{
    return internal::JoinStringT(parts, separator);
}

std::u16string JoinString(span<const std::u16string_view> parts, std::u16string_view separator)
{
    return internal::JoinStringT(parts, separator);
}
...
std::u16string JoinString(std::initializer_list<std::u16string_view> parts, std::u16string_view separator)
{
    return internal::JoinStringT(parts, separator);
}

std::u16string ReplaceStringPlaceholders(std::u16string_view format_string, const std::vector<std::u16string>& subst, std::vector<size_t>* offsets)
{
    std::optional<std::u16string> replacement = internal::DoReplaceStringPlaceholders(format_string, subst,
...
{
    std::optional<std::string> replacement = internal::DoReplaceStringPlaceholders(format_string, subst,
        /*placeholder_prefix*/ '$',
        /*should_escape_multiple_placeholder_prefixes*/ true,
        /*is_strict_mode*/ false, offsets);
...
std::u16string ReplaceStringPlaceholders(const std::u16string& format_string, const std::u16string& a, size_t* offset)
{
...
{
    return internal::lcpyT(UNSAFE_TODO(base::span(dst, dst_size), std::string_view(src)));
}

size_t u16cstrlcpy(char16_t* dst, const char16_t* src, size_t dst_size)
{
    return internal::lcpyT(UNSAFE_TODO(base::span(dst, dst_size)), std::u16string_view(src));
}

size_t wcslcpy(wchar_t* dst, const wchar_t* src, size_t dst_size)
{
    return internal::lcpyT(UNSAFE_TODO(base::span(dst, dst_size)), std::wstring_view(src));
}
