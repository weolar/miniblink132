...
template <typename STR, typename INT> static STR IntToStringT(INT value)
{
...
template <int BASE, typename CHAR> std::optional<uint8_t> CharToDigit(CHAR c)
{
...
template <typename Number, int kBase> class StringToNumberParser {
public:
    struct Result {
        Number value = 0;
        bool valid = false;
    ...
    template <typename Sign> class Base {
    public:
        template <typename Iter> static Result Invoke(Iter begin, Iter end)
        {
            Number value = 0;
    ...
    public:
        ...
        {
            if (value > static_cast<Number>(kMax / kBase) || (value == static_cast<Number>(kMax / kBase) && new_digit > kMax % kBase)) {
                return { kMax, false };
            }
            return { value, true };
        }
        static Number Increment(Number lhs, uint8_t rhs)
        {
            return lhs + rhs;
        }
    };
...
template <typename Number, int kBase, typename CharT> auto StringToNumber(std::basic_string_view<CharT> input)
{
...
template <typename T, typename VALUE, typename CharT = typename T::value_type> bool StringToIntImpl(T input, VALUE& output)
{
...
template <typename T, typename VALUE, typename CharT = typename T::value_type> bool HexStringToIntImpl(T input, VALUE& output)
{
...
template <typename StringT> StringT ToString(const typename StringT::value_type* data, size_t size)
{
    return StringT(data, size);
}

template <typename StringT, typename CharT> StringT ToString(const CharT* data, size_t size)
{
    return StringT(data, data + size);
}

template <typename StringT> StringT DoubleToStringT(double value)
{
    char buffer[32];
...
template <typename STRING, typename CHAR> bool StringToDoubleImpl(STRING input, const CHAR* data, double& output)
{
    static double_conversion::StringToDoubleConverter converter(
        double_conversion::StringToDoubleConverter::ALLOW_LEADING_SPACES | double_conversion::StringToDoubleConverter::ALLOW_TRAILING_JUNK, 0.0, 0, nullptr,
        nullptr);
    ...
    //    algorithm.
    return !input.empty() && output != HUGE_VAL && output != -HUGE_VAL && static_cast<size_t>(processed_characters_count) == input.size()
        && !IsWhitespace(input[0]);
}

template <typename Char, typename OutIter> static bool HexStringToByteContainer(std::string_view input, OutIter output)
{
