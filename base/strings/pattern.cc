...
constexpr bool SearchForChars(const CHAR** pattern, const CHAR* pattern_end, const CHAR** string, const CHAR* string_end, int maximum_distance, NEXT next)
{
...
// least one *).
template <typename CHAR, typename NEXT> constexpr int EatWildcards(const CHAR** pattern, const CHAR* end, NEXT next)
{
...
template <typename CHAR, typename NEXT>
constexpr bool MatchPatternT(const CHAR* eval, const CHAR* eval_end, const CHAR* pattern, const CHAR* pattern_end, NEXT next)
{
    do {
...
{
    return MatchPatternT(eval.data(), eval.data() + eval.size(), pattern.data(), pattern.data() + pattern.size(), NextCharUTF8());
}

bool MatchPattern(std::u16string_view eval, std::u16string_view pattern)
{
    return MatchPatternT(eval.data(), eval.data() + eval.size(), pattern.data(), pattern.data() + pattern.size(), NextCharUTF16());
}
