...
BASE_EXPORT size_t u16cstrlcpy(char16_t* dst, const char16_t* src, size_t dst_size);
...
template <typename CharT, typename Iter> constexpr std::basic_string_view<CharT> MakeBasicStringPiece(Iter begin, Iter end)
{
...
template <typename Iter> constexpr std::string_view MakeStringPiece(Iter begin, Iter end)
{
    return MakeBasicStringPiece<char>(begin, end);
}

template <typename Iter> constexpr std::u16string_view MakeStringPiece16(Iter begin, Iter end)
{
...
BASE_EXPORT constexpr int CompareCaseInsensitiveASCII(std::string_view a, std::string_view b)
{
    return internal::CompareCaseInsensitiveASCIIT(a, b);
}
BASE_EXPORT constexpr int CompareCaseInsensitiveASCII(std::u16string_view a, std::u16string_view b)
{
    return internal::CompareCaseInsensitiveASCIIT(a, b);
}
...
inline bool EqualsCaseInsensitiveASCII(std::u16string_view a, std::u16string_view b)
{
    return internal::EqualsCaseInsensitiveASCIIT(a, b);
}
inline bool EqualsCaseInsensitiveASCII(std::u16string_view a, std::string_view b)
{
    return internal::EqualsCaseInsensitiveASCIIT(a, b);
}
inline bool EqualsCaseInsensitiveASCII(std::string_view a, std::u16string_view b)
{
    return internal::EqualsCaseInsensitiveASCIIT(a, b);
}
...
BASE_EXPORT extern const char16_t kWhitespaceNoCrLfUTF16[]; // Unicode w/o CR/LF.
...
BASE_EXPORT bool RemoveChars(std::u16string_view input, std::u16string_view remove_chars, std::u16string* output);
BASE_EXPORT bool RemoveChars(std::string_view input, std::string_view remove_chars, std::string* output);
...
// NOTE: Safe to use the same variable for both |input| and |output|.
BASE_EXPORT bool ReplaceChars(std::u16string_view input, std::u16string_view replace_chars, std::u16string_view replace_with, std::u16string* output);
BASE_EXPORT bool ReplaceChars(std::string_view input, std::string_view replace_chars, std::string_view replace_with, std::string* output);
...
BASE_EXPORT bool TrimString(std::u16string_view input, std::u16string_view trim_chars, std::u16string* output);
BASE_EXPORT bool TrimString(std::string_view input, std::string_view trim_chars, std::string* output);

// std::string_view versions of the above. The returned pieces refer to the
// original buffer.
BASE_EXPORT std::u16string_view TrimString(std::u16string_view input, std::u16string_view trim_chars, TrimPositions positions);
...
BASE_EXPORT void TruncateUTF8ToByteSize(const std::string& input, const size_t byte_size, std::string* output);
...
// NOTE: Safe to use the same variable for both input and output.
BASE_EXPORT TrimPositions TrimWhitespace(std::u16string_view input, TrimPositions positions, std::u16string* output);
BASE_EXPORT std::u16string_view TrimWhitespace(std::u16string_view input, TrimPositions positions);
BASE_EXPORT TrimPositions TrimWhitespaceASCII(std::string_view input, TrimPositions positions, std::string* output);
...
BASE_EXPORT std::u16string CollapseWhitespace(std::u16string_view text, bool trim_sequences_with_line_breaks);
BASE_EXPORT std::string CollapseWhitespaceASCII(std::string_view text, bool trim_sequences_with_line_breaks);

// Returns true if |input| is empty or contains only characters found in
// |characters|.
BASE_EXPORT bool ContainsOnlyChars(std::string_view input, std::string_view characters);
BASE_EXPORT bool ContainsOnlyChars(std::u16string_view input, std::u16string_view characters);
...
BASE_EXPORT bool StartsWith(std::string_view str, std::string_view search_for, CompareCase case_sensitivity = CompareCase::SENSITIVE);
BASE_EXPORT bool StartsWith(std::u16string_view str, std::u16string_view search_for, CompareCase case_sensitivity = CompareCase::SENSITIVE);
BASE_EXPORT bool EndsWith(std::string_view str, std::string_view search_for, CompareCase case_sensitivity = CompareCase::SENSITIVE);
BASE_EXPORT bool EndsWith(std::u16string_view str, std::u16string_view search_for, CompareCase case_sensitivity = CompareCase::SENSITIVE);
...
constexpr bool IsUnicodeWhitespace(Char c)
{
    ...
    for (const auto* cur = kWhitespaceWide; *cur; ++cur) {
        if (static_cast<typename std::make_unsigned_t<wchar_t>>(*cur) == static_cast<typename std::make_unsigned_t<Char>>(c))
            return true;
    }
    return false;
}
...
// instead!
template <typename Char> constexpr bool IsWhitespace(Char c)
{
...
BASE_EXPORT void ReplaceFirstSubstringAfterOffset(std::u16string* str, size_t start_offset, std::u16string_view find_this, std::u16string_view replace_with);
BASE_EXPORT void ReplaceFirstSubstringAfterOffset(std::string* str, size_t start_offset, std::string_view find_this, std::string_view replace_with);
...
BASE_EXPORT void ReplaceSubstringsAfterOffset(std::u16string* str, size_t start_offset, std::u16string_view find_this, std::u16string_view replace_with);
...
BASE_EXPORT std::string JoinString(span<const std::string> parts, std::string_view separator);
BASE_EXPORT std::string JoinString(std::vector<std::string_view> parts, std::string_view separator);
BASE_EXPORT std::u16string JoinString(span<const std::u16string> parts, std::u16string_view separator);
BASE_EXPORT std::string JoinString(span<const std::string_view> parts, std::string_view separator);
BASE_EXPORT std::u16string JoinString(span<const std::u16string_view> parts, std::u16string_view separator);
...
BASE_EXPORT std::u16string JoinString(std::initializer_list<std::u16string_view> parts, std::u16string_view separator);

// Replace $1-$2-$3..$9 in the format string with values from |subst|.
// Additionally, any number of consecutive '$' characters is replaced by that
// number less one. Eg $$->$, $$$->$$, etc. The offsets parameter here can be
// NULL. This only allows you to use up to nine replacements.
BASE_EXPORT std::u16string ReplaceStringPlaceholders(std::u16string_view format_string, const std::vector<std::u16string>& subst, std::vector<size_t>* offsets);

BASE_EXPORT std::string ReplaceStringPlaceholders(std::string_view format_string, const std::vector<std::string>& subst, std::vector<size_t>* offsets);

// Single-string shortcut for ReplaceStringHolders. |offset| may be NULL.
BASE_EXPORT std::u16string ReplaceStringPlaceholders(const std::u16string& format_string, const std::u16string& a, size_t* offset);

// Helper function for creating a std::string_view from a string literal that
// preserves internal NUL characters.
template <class CharT, size_t N>
std::basic_string_view<CharT> MakeStringViewWithNulChars(const CharT (&lit LIFETIME_BOUND)[N])
