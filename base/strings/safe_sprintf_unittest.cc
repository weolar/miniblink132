...
TEST(SafeSPrintfTest, ASANFriendlyBufferTest)
{
    ...
    EXPECT_EQ(static_cast<ssize_t>(sizeof(kTestString) - 1), SafeSNPrintf(buf.data(), buf.size(), kTestString));
    EXPECT_EQ(std::string(kTestString), std::string(buf.data()));
    EXPECT_EQ(static_cast<ssize_t>(buf.size() - 1), SafeSNPrintf(buf.data(), buf.size(), "%s", kTestString));
    EXPECT_EQ(std::string(kTestString), std::string(buf.data()));
}
