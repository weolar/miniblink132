...
void CheckRoundtripsT(const uint8_t* data, const size_t size, StringType (*num_to_string)(NumberType), bool (*string_to_num)(StringPieceType, NumberType*))
{
...
template <class NumberType> void CheckRoundtrips(const uint8_t* data, const size_t size, bool (*string_to_num)(std::string_view, NumberType*))
{
    return CheckRoundtripsT<NumberType, std::string_view, std::string>(data, size, &base::NumberToString, string_to_num);
}

template <class NumberType> void CheckRoundtrips16(const uint8_t* data, const size_t size, bool (*string_to_num)(std::u16string_view, NumberType*))
{
    return CheckRoundtripsT<NumberType, std::u16string_view, std::u16string>(data, size, &base::NumberToString16, string_to_num);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::string_view string_piece_input(reinterpret_cast<const char*>(data), size);
    ...
    if (size % 2 == 0) {
        std::u16string_view string_piece_input16(reinterpret_cast<const char16_t*>(data), size / 2);
