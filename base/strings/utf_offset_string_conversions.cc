...
OffsetAdjuster::Adjustment::Adjustment(size_t original_offset, size_t original_length, size_t output_length)
...
// static
void OffsetAdjuster::AdjustOffsets(const Adjustments& adjustments, std::vector<size_t>* offsets_for_adjustment, size_t limit)
{
...
// static
void OffsetAdjuster::AdjustOffset(const Adjustments& adjustments, size_t* offset, size_t limit)
{
    DCHECK(offset);
...
// static
void OffsetAdjuster::UnadjustOffsets(const Adjustments& adjustments, std::vector<size_t>* offsets_for_unadjustment)
{
...
// static
void OffsetAdjuster::UnadjustOffset(const Adjustments& adjustments, size_t* offset)
{
    if (*offset == std::u16string::npos)
        return;
    size_t original_lengths = 0;
    ...
    for (const auto& i : adjustments) {
        ...
        original_lengths += i.original_length;
        output_lengths += i.output_length;
        if ((*offset + original_lengths - output_lengths) < (i.original_offset + i.original_length)) {
            *offset = std::u16string::npos;
            return;
        }
    }
...
// static
void OffsetAdjuster::MergeSequentialAdjustments(const Adjustments& first_adjustments, Adjustments* adjustments_on_adjusted_string)
{
    ...
    while (adjusted_iter != adjustments_on_adjusted_string->end()) {
        if ((first_iter == first_adjustments.end())
            || ((adjusted_iter->original_offset + shift + adjusted_iter->original_length) <= first_iter->original_offset)) {
            ...
            // adjustments.)
            DCHECK_LE(first_iter->original_offset + first_iter->output_length, adjusted_iter->original_offset + shift);
            ...
            const size_t collapse = first_iter->original_length - first_iter->output_length;
...
bool ConvertUnicode(const SrcChar* src, size_t src_len, DestStdString* output, OffsetAdjuster::Adjustments* adjustments)
{
    if (adjustments)
        adjustments->clear();
    bool success = true;
    for (size_t i = 0; i < src_len; i++) {
        ...
        if (adjustments && ((i - original_i + 1) != chars_written)) {
            adjustments->push_back(OffsetAdjuster::Adjustment(original_i, i - original_i + 1, chars_written));
        }
    }
    return success;
}

bool UTF8ToUTF16WithAdjustments(const char* src, size_t src_len, std::u16string* output, base::OffsetAdjuster::Adjustments* adjustments)
{
    PrepareForUTF16Or32Output(src, src_len, output);
    return ConvertUnicode(src, src_len, output, adjustments);
}

std::u16string UTF8ToUTF16WithAdjustments(std::string_view utf8, base::OffsetAdjuster::Adjustments* adjustments)
{
...
std::u16string UTF8ToUTF16AndAdjustOffsets(std::string_view utf8, std::vector<size_t>* offsets_for_adjustment)
{
...
std::string UTF16ToUTF8AndAdjustOffsets(std::u16string_view utf16, std::vector<size_t>* offsets_for_adjustment)
{
    for (size_t& offset : *offsets_for_adjustment) {
