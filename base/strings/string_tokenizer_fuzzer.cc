...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    size_t pattern_size = *reinterpret_cast<const size_t*>(data) % (size - size_t_bytes);

    std::string pattern(reinterpret_cast<const char*>(data + size_t_bytes), pattern_size);
    std::string input(reinterpret_cast<const char*>(data + size_t_bytes + pattern_size), size - pattern_size - size_t_bytes);
