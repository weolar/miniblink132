...
template <typename T, typename CharT = typename T::value_type> std::basic_string<CharT> ToLowerASCIIImpl(T str)
{
...
template <typename T, typename CharT = typename T::value_type> std::basic_string<CharT> ToUpperASCIIImpl(T str)
{
    std::basic_string<CharT> ret;
    ret.reserve(str.size());
    for (size_t i = 0; i < str.size(); i++)
...
TrimPositions TrimStringT(T input, T trim_chars, TrimPositions positions, std::basic_string<CharT>* output)
{
    ...
    if (input.empty() || first_good_char == std::basic_string<CharT>::npos || last_good_char == std::basic_string<CharT>::npos) {
...
template <typename T, typename CharT = typename T::value_type> T TrimStringPieceT(T input, T trim_chars, TrimPositions positions)
{
...
template <typename T, typename CharT = typename T::value_type> std::basic_string<CharT> CollapseWhitespaceT(T text, bool trim_sequences_with_line_breaks)
{
...
template <class Char> bool DoIsStringASCII(const Char* characters, size_t length)
{
...
template <bool (*Validator)(base_icu::UChar32)> inline bool DoIsStringUTF8(std::string_view str)
{
...
template <typename T, typename CharT = typename T::value_type> bool StartsWithT(T str, T search_for, CompareCase case_sensitivity)
{
    ...
    case CompareCase::INSENSITIVE_ASCII:
        return std::equal(search_for.begin(), search_for.end(), source.begin(), CaseInsensitiveCompareASCII<CharT>());
    }
}

template <typename T, typename CharT = typename T::value_type> bool EndsWithT(T str, T search_for, CompareCase case_sensitivity)
{
    if (search_for.size() > str.size())
        return false;

    std::basic_string_view<CharT> source = str.substr(str.size() - search_for.size(), search_for.size());
    ...
    case CompareCase::INSENSITIVE_ASCII:
        return std::equal(source.begin(), source.end(), search_for.begin(), CaseInsensitiveCompareASCII<CharT>());
    }
}

// A Matcher for DoReplaceMatchesAfterOffset() that matches substrings.
template <class CharT> struct SubstringMatcher {
...
template <typename T, typename CharT = typename T::value_type> auto MakeSubstringMatcher(T find_this)
{
    return SubstringMatcher<CharT> { find_this };
}

// A Matcher for DoReplaceMatchesAfterOffset() that matches single characters.
template <class CharT> struct CharacterMatcher {
...
template <typename T, typename CharT = typename T::value_type> auto MakeCharacterMatcher(T find_any_of_these)
{
...
bool DoReplaceMatchesAfterOffset(std::basic_string<CharT>* str, size_t initial_offset, Matcher matcher, T replace_with, ReplaceType replace_type)
{
    ...
    if (find_length == replace_length) {
        auto* buffer = &((*str)[0]);
        for (size_t offset = first_match; offset != std::basic_string<CharT>::npos; offset = matcher.Find(*str, offset + replace_length)) {
    ...
    if (replace_length > find_length) {
        ...
        for (size_t match = first_match; match != std::basic_string<CharT>::npos; match = matcher.Find(*str, match + find_length)) {
...
template <typename T, typename CharT = typename T::value_type>
bool ReplaceCharsT(T input, T find_any_of_these, T replace_with, std::basic_string<CharT>* output)
{
    ...
    return DoReplaceMatchesAfterOffset(output, 0, MakeCharacterMatcher(find_any_of_these), replace_with, ReplaceType::REPLACE_ALL);
}

template <class string_type> inline typename string_type::value_type* WriteIntoT(string_type* str, size_t length_with_null)
{
...
// char or char16_t.
template <typename list_type, typename T, typename CharT = typename T::value_type>
...
template <typename T, typename CharT = typename T::value_type>
std::optional<std::basic_string<CharT>> DoReplaceStringPlaceholders(T format_string, const std::vector<std::basic_string<CharT>>& subst,
    const CharT placeholder_prefix, const bool should_escape_multiple_placeholder_prefixes, const bool is_strict_mode, std::vector<size_t>* offsets)
{
            ...
            if (i + 1 != format_string.end()) {
                ...
                if (placeholder_prefix == *i) {
                    do {
                        ...
                        ++i;
                    } while (should_escape_multiple_placeholder_prefixes && i != format_string.end() && placeholder_prefix == *i);
                    --i;
                } else {
                    if (*i < '1' || *i > '9') {
                        if (is_strict_mode) {
                            DLOG(ERROR) << "Invalid placeholder after placeholder prefix: " << std::basic_string<CharT>(1, placeholder_prefix)
                    ...
                    if (offsets) {
                        ...
                        r_offsets.insert(ranges::upper_bound(r_offsets, r_offset, &CompareParameter), r_offset);
                    }
                    if (index < substitutions) {
                        formatted.append(subst.at(index));
                    } else if (is_strict_mode) {
                        DLOG(ERROR) << "index out of range: " << index << ": " << substitutions;
                        return std::nullopt;
                    }
                }
            } else if (is_strict_mode) {
...
template <typename CHAR> size_t lcpyT(span<CHAR> dst, std::basic_string_view<CHAR> src)
{
    size_t i = 0;
