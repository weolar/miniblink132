...
BASE_EXPORT ssize_t SafeSNPrintf(char* buf, size_t sz, const char* fmt, const Arg* args, size_t max_args);
...
template <typename... Args> ssize_t SafeSNPrintf(char* buf, size_t N, const char* fmt, Args... args)
{
...
template <size_t N, typename... Args> ssize_t SafeSPrintf(char (&buf)[N], const char* fmt, Args... args)
{
    // Use Arg() object to record type information and then copy arguments to an
    // array to make it easier to iterate over them.
    const internal::Arg arg_array[] = { args... };
...
template <size_t N> inline ssize_t SafeSPrintf(char (&buf)[N], const char* fmt)
{
