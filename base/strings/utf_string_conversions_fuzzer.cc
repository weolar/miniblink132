...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    base::UTF8ToWide(reinterpret_cast<const char*>(data), size, &output_std_wstring);
    std::ignore = base::UTF8ToUTF16(string_piece_input);
    base::UTF8ToUTF16(reinterpret_cast<const char*>(data), size, &output_string16);
    ...
    if (size % 2 == 0) {
        ...
        base::UTF16ToWide(reinterpret_cast<const char16_t*>(data), size / 2, &output_std_wstring);
        std::ignore = base::UTF16ToUTF8(string_piece_input16);
        base::UTF16ToUTF8(reinterpret_cast<const char16_t*>(data), size / 2, &output_std_string);
    }
    ...
    if (size % wchar_t_size == 0) {
        ...
        base::WideToUTF8(reinterpret_cast<const wchar_t*>(data), size / wchar_t_size, &output_std_string);
        std::ignore = base::WideToUTF16(output_std_wstring);
        base::WideToUTF16(reinterpret_cast<const wchar_t*>(data), size / wchar_t_size, &output_string16);
    }
