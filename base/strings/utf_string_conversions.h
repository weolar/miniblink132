...
// time.
template <size_t N> [[noreturn]] std::u16string WideToUTF16(const wchar_t (&str)[N])
{
    static_assert(AlwaysFalse<decltype(N)>, "Error: Use u\"...\" to create a std::u16string literal.");
}

template <size_t N> [[noreturn]] std::u16string UTF8ToUTF16(const char (&str)[N])
{
    static_assert(AlwaysFalse<decltype(N)>, "Error: Use u\"...\" to create a std::u16string literal.");
}

template <size_t N> [[noreturn]] std::u16string ASCIIToUTF16(const char (&str)[N])
{
    static_assert(AlwaysFalse<decltype(N)>, "Error: Use u\"...\" to create a std::u16string literal.");
}
...
template <size_t N> std::u16string ASCIIToUTF16(char (&str)[N])
{
