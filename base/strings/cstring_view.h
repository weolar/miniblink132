...
template <class Char> class basic_cstring_view final {
...
public:
    ...
    constexpr basic_cstring_view(const Char (&lit LIFETIME_BOUND)[M]) noexcept ENABLE_IF_ATTR(lit[M - 1u] == Char { 0 }, "requires string literal as input")
        : ptr_(lit)
        , len_(std::char_traits<Char>::length(lit))
    {
    ...
    template <class String>
        requires(std::same_as<std::remove_cvref_t<String>, Char*> || std::same_as<std::remove_cvref_t<String>, const Char*>)
    UNSAFE_BUFFER_USAGE explicit constexpr basic_cstring_view(String&& ptr LIFETIME_BOUND) noexcept
        : ptr_(ptr)
        , len_(std::char_traits<Char>::length(ptr))
    {
    }
    ...
    UNSAFE_BUFFER_USAGE explicit constexpr basic_cstring_view(const Char* ptr LIFETIME_BOUND, size_t len)
        : ptr_(ptr)
        , len_(len)
    {
    ...
    //
    // This is never null.
    PURE_FUNCTION constexpr const Char* c_str() const noexcept
    {
        return ptr_;
    }
    ...
    PURE_FUNCTION constexpr const Char* data() const noexcept
    {
        return ptr_;
    }
    ...
    // terminating NUL.
    PURE_FUNCTION constexpr size_t size() const noexcept
    {
        return len_;
    }
    // An alias for `size()`, returning the number of characters in the string.
    PURE_FUNCTION constexpr size_t length() const noexcept
    {
        return len_;
    }
    ...
    PURE_FUNCTION constexpr bool empty() const noexcept
    {
        return len_ == 0u;
    }
    ...
    // Produces an iterator over the cstring view, excluding the terminating NUL.
    PURE_FUNCTION constexpr const_iterator cend() const noexcept
    {
        return end();
    }
    ...
    PURE_FUNCTION constexpr std::basic_string_view<Char> substr(size_t pos, size_t count = npos) const noexcept
    {
        ...
        return UNSAFE_BUFFERS(std::basic_string_view<Char>(ptr_ + pos, std::min(count, len_ - pos)));
    }
    ...
    constexpr bool starts_with(std::basic_string_view<Char> prefix) const noexcept
    {
    ...
    // `pos` is out of range.
    constexpr size_t find(std::basic_string_view<Char> search, size_t pos = 0u) const noexcept
    {
    ...
    // not found.
    constexpr size_t rfind(std::basic_string_view<Char> search, size_t pos = npos) const noexcept
    {
    ...
    constexpr size_t find_first_of(std::basic_string_view<Char> search, size_t pos = 0u) const noexcept
    {
    ...
    constexpr size_t find_last_of(std::basic_string_view<Char> search, size_t pos = npos) const noexcept
    {
    ...
    constexpr size_t find_first_not_of(std::basic_string_view<Char> search, size_t pos = 0u) const noexcept
    {
        return std::basic_string_view<Char>(*this).find_first_not_of(search, pos);
    }
    constexpr size_t find_first_not_of(Char search, size_t pos = 0u) const noexcept
    {
        return std::basic_string_view<Char>(*this).find_first_not_of(search, pos);
    }
    ...
    constexpr size_t find_last_not_of(std::basic_string_view<Char> search, size_t pos = npos) const noexcept
    {
        return std::basic_string_view<Char>(*this).find_last_not_of(search, pos);
    }
    constexpr size_t find_last_not_of(Char search, size_t pos = npos) const noexcept
    {
        return std::basic_string_view<Char>(*this).find_last_not_of(search, pos);
    }
    ...
    friend constexpr std::weak_ordering operator<=>(basic_cstring_view l, basic_cstring_view r)
    {
    ...
    friend constexpr std::basic_string<Char, Traits, Alloc> operator+(basic_cstring_view lhs, const std::basic_string<Char, Traits, Alloc>& rhs)
    {
        return lhs.c_str() + rhs;
    }
    template <class Traits, class Alloc>
    friend constexpr std::basic_string<Char, Traits, Alloc> operator+(basic_cstring_view lhs, std::basic_string<Char, Traits, Alloc>&& rhs)
    {
        return lhs.c_str() + std::move(rhs);
    }
    template <class Traits, class Alloc>
    friend constexpr std::basic_string<Char, Traits, Alloc> operator+(const std::basic_string<Char, Traits, Alloc>& lhs, basic_cstring_view rhs)
    {
        return lhs + rhs.c_str();
    }
    template <class Traits, class Alloc>
    friend constexpr std::basic_string<Char, Traits, Alloc> operator+(std::basic_string<Char, Traits, Alloc>&& lhs, basic_cstring_view rhs)
    {
...
template <class Char, class Traits> std::basic_ostream<Char, Traits>& operator<<(std::basic_ostream<Char, Traits>& os, basic_cstring_view<Char> view)
{
...
template <class Char> struct std::hash<base::basic_cstring_view<Char>> {
...
template <class Char> inline constexpr bool std::ranges::enable_borrowed_range<base::basic_cstring_view<Char>> = true;

template <class Char> inline constexpr bool std::ranges::enable_view<base::basic_cstring_view<Char>> = true;
