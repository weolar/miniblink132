...
template <typename CharT> std::basic_string_view<CharT> WhitespaceForType();

template <> inline std::u16string_view WhitespaceForType<char16_t>()
{
    return kWhitespaceUTF16;
}
template <> inline std::string_view WhitespaceForType<char>()
{
...
template <typename OutputStringType, typename T, typename CharT = typename T::value_type>
static std::vector<OutputStringType> SplitStringT(T str, T delimiter, WhitespaceHandling whitespace, SplitResult result_type)
{
...
template <typename OutputStringType, typename T, typename CharT = typename T::value_type>
std::vector<OutputStringType> SplitStringUsingSubstrT(T input, T delimiter, WhitespaceHandling whitespace, SplitResult result_type)
{
    ...
    for (size_type begin_index = 0, end_index = 0; end_index != Piece::npos; begin_index = end_index + delimiter.size()) {
        ...
        Piece term = end_index == Piece::npos ? input.substr(begin_index) : input.substr(begin_index, end_index - begin_index);
