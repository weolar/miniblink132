...
template <typename... Ts> std::string ToString(const Ts&... values);
...
// pointers.
template <typename T> constexpr bool IsIomanip = false;
template <typename T>
...
template <typename T>
concept WillBeIncorrectlyStreamedAsBool = std::is_function_v<std::remove_pointer_t<T>> && !IsIomanip<std::remove_pointer_t<T>>;
...
template <typename T> struct ToStringHelper {
    static void Stringify(const T& v, std::ostringstream& ss)
    {
        ...
        // char-like types.
        ss << "[" << sizeof(v) << "-byte object at 0x" << static_cast<const void*>(std::addressof(v)) << "]";
    }
};

// Most streamables.
template <typename T>
    requires(SupportsOstreamOperator<const T&> && !WillBeIncorrectlyStreamedAsBool<T>)
...
struct ToStringHelper<T> {
    static void Stringify(const T& v, std::ostringstream& ss)
    {
        ToStringHelper<const void*>::Stringify(reinterpret_cast<const void*>(v), ss);
    }
};
...
template <typename... T> struct ToStringHelper<std::tuple<T...>> {
    template <size_t... I> static void StringifyHelper(const std::tuple<T...>& values, std::index_sequence<I...>, std::ostringstream& ss)
    {
        ss << "<";
...
template <typename... Ts> std::string ToString(const Ts&... values)
{
    ...
    (..., internal::ToStringHelper<std::remove_cvref_t<decltype(values)>>::Stringify(values, ss));
    return ss.str();
}
