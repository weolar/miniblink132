...
template <typename INT> struct NumberToStringTest {
    INT num;
...
TEST(StringNumberConversionsTest, NumberToString)
{
    ...
    for (const auto& test : int_tests) {
        ...
        EXPECT_EQ(NumberToString16(static_cast<unsigned>(test.num)), UTF8ToUTF16(test.uexpected));
    }
    for (const auto& test : int64_tests) {
        EXPECT_EQ(NumberToString(test.num), test.sexpected);
        EXPECT_EQ(NumberToString16(test.num), UTF8ToUTF16(test.sexpected));
        EXPECT_EQ(NumberToString(static_cast<uint64_t>(test.num)), test.uexpected);
        EXPECT_EQ(NumberToString16(static_cast<uint64_t>(test.num)), UTF8ToUTF16(test.uexpected));
    }
}
...
TEST(StringNumberConversionsTest, HexStringToUInt)
{
    static const struct {
        ...
        { "0x7fffffffffffffff", std::numeric_limits<uint32_t>::max(), false }, // Overflow test.
        { "-0x8000000000000000", 0, false },
        { "0x8000000000000000", std::numeric_limits<uint32_t>::max(), false }, // Overflow test.
        { "-0x8000000000000001", 0, false },
        { "0xFFFFFFFFFFFFFFFF", std::numeric_limits<uint32_t>::max(), false }, // Overflow test.
        { "FFFFFFFFFFFFFFFF", std::numeric_limits<uint32_t>::max(), false }, // Overflow test.
        { "0x0000000000000000", 0, true },
        { "0000000000000000", 0, true },
        { "1FFFFFFFFFFFFFFFF", std::numeric_limits<uint32_t>::max(), false }, // Overflow test.
        { "0x0f", 0x0f, true },
        { "0f", 0x0f, true },
        { " 45", 0x45, false },
        { "\t\n\v\f\r 0x45", 0x45, false },
...
TEST(StringNumberConversionsTest, HexStringToInt64)
{
    static const struct {
        ...
        { "0x8000000000000000", std::numeric_limits<int64_t>::max(), false }, // Overflow test.
        { "-0x8000000000000001", std::numeric_limits<int64_t>::min(), false }, // Underflow test.
        { "0x0f", 15, true },
        { "0f", 15, true },
        { " 45", 0x45, false },
        { "\t\n\v\f\r 0x45", 0x45, false },
...
TEST(StringNumberConversionsTest, HexStringToUInt64)
{
    static const struct {
        ...
        { "0x0000000000000000", 0, true },
        { "0000000000000000", 0, true },
        { "1FFFFFFFFFFFFFFFF", std::numeric_limits<uint64_t>::max(), false }, // Overflow test.
        { "0x0f", 15, true },
        { "0f", 15, true },
        { " 45", 0x45, false },
        { "\t\n\v\f\r 0x45", 0x45, false },
...
TEST(StringNumberConversionsTest, HexStringToBytesStringSpan)
{
    static const struct {
        ...
        { "0123456789ABCDEF012345", "\x01\x23\x45\x67\x89\xAB\xCD\xEF\x01\x23\x45", 11, true },
    };
    ...
    for (size_t test_i = 0; test_i < std::size(cases); ++test_i) {
        ...
        // Test HexStringToBytes().
        {
            ...
            EXPECT_EQ(test.success, HexStringToBytes(test.input, &output)) << test_i << ": " << test.input;
        ...
        {
            std::string output;
            EXPECT_EQ(test.success, HexStringToString(test.input, &output)) << test_i << ": " << test.input;
        ...
        // Test HexStringToSpan() with a properly sized output.
        {
            ...
            EXPECT_EQ(test.success, HexStringToSpan(test.input, output)) << test_i << ": " << test.input;
            ...
            for (size_t i = 0; i < test.output_len; ++i) {
                EXPECT_EQ(test.output[i], static_cast<char>(output[i])) << test_i << ": " << test.input;
            }
            for (size_t i = test.output_len; i < output.size(); ++i) {
                EXPECT_EQ('\0', static_cast<char>(output[i])) << test_i << ": " << test.input;
            }
        }
        ...
        // Test HexStringToSpan() with an output that is 1 byte too small.
        {
            ...
            EXPECT_FALSE(HexStringToSpan(test.input, output)) << test_i << ": " << test.input;
        }
...
TEST(StringNumberConversionsTest, StringToDouble)
{
    static const struct {
        ...
        // crbug.org/588726
        { "-0.0010000000000000000000000000000000000000001e-256", -1.0000000000000001e-259, true },
    };

    for (size_t i = 0; i < std::size(cases); ++i) {
        SCOPED_TRACE(StringPrintf("case %" PRIuS " \"%s\"", i, cases[i].input.c_str()));
        double output;
        errno = 1;
...
TEST(StringNumberConversionsTest, StrtodFailures)
{
    static const struct {
        ...
        { "1.50000000000000011102230246251565404236316680908203125", 0x3ff8000000000000ULL },
        { "9007199254740991.4999999999999999999999999999999995", 0x433fffffffffffffULL },
