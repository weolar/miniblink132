...
namespace base {
namespace {
std::string SafeConvertJavaStringToUTF8(JNIEnv* env, const JavaRef<jstring>& str)
{
    if (str.is_null()) {
        return std::string();
    }
...
bool ContentUriGetFileInfo(const FilePath& content_uri, FileEnumerator::FileInfo* info)
{
    JNIEnv* env = android::AttachCurrentThread();
    std::vector<FileEnumerator::FileInfo> list;
    Java_ContentUriUtils_getFileInfo(env, content_uri.value(), reinterpret_cast<jlong>(&list));
...
std::vector<FileEnumerator::FileInfo> ListContentUriDirectory(const FilePath& content_uri)
{
    JNIEnv* env = android::AttachCurrentThread();
    std::vector<FileEnumerator::FileInfo> result;
    Java_ContentUriUtils_listDirectory(env, content_uri.value(), reinterpret_cast<jlong>(&result));
...
void JNI_ContentUriUtils_AddFileInfoToVector(JNIEnv* env, jlong vector_pointer, const JavaParamRef<jstring>& uri, const JavaParamRef<jstring>& display_name,
    jboolean is_directory, jlong size, jlong last_modified)
{
    auto* result = reinterpret_cast<std::vector<FileEnumerator::FileInfo>*>(vector_pointer);
    result->emplace_back(FilePath(SafeConvertJavaStringToUTF8(env, uri)), FilePath(SafeConvertJavaStringToUTF8(env, display_name)), is_directory, size,
...
{
    JNIEnv* env = android::AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_mime = Java_ContentUriUtils_getMimeType(env, content_uri.value());
    return SafeConvertJavaStringToUTF8(env, j_mime);
}

bool MaybeGetFileDisplayName(const FilePath& content_uri, std::u16string* file_display_name)
{
    ...
    JNIEnv* env = android::AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_display_name = Java_ContentUriUtils_maybeGetDisplayName(env, content_uri.value());
...
FilePath ContentUriBuildDocumentUriUsingTree(const FilePath& tree_uri, const std::string& encoded_document_id)
{
    JNIEnv* env = android::AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_uri = Java_ContentUriUtils_buildDocumentUriUsingTree(env, tree_uri.value(), encoded_document_id);
    return FilePath(SafeConvertJavaStringToUTF8(env, j_uri));
}

FilePath ContentUriGetChildDocumentOrQuery(
    const FilePath& parent, const std::string& display_name, const std::string& mime_type, bool is_directory, bool create)
{
    JNIEnv* env = android::AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_uri = Java_ContentUriUtils_getChildDocumentOrQuery(env, parent.value(), display_name, mime_type, is_directory, create);
    return FilePath(SafeConvertJavaStringToUTF8(env, j_uri));
}
...
{
    JNIEnv* env = android::AttachCurrentThread();
    return Java_ContentUriUtils_isCreateChildDocumentQuery(env, content_uri.value());
}

FilePath ContentUriGetDocumentFromQuery(const FilePath& content_uri, bool create)
{
    JNIEnv* env = android::AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_uri = Java_ContentUriUtils_getDocumentFromQuery(env, content_uri.value(), create);
    return FilePath(SafeConvertJavaStringToUTF8(env, j_uri));
}
