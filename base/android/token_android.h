...
public:
    ...
    static ScopedJavaLocalRef<jobject> Create(JNIEnv* env, const base::Token& token);

    // Creates a Token from `j_token`.
    static base::Token FromJavaToken(JNIEnv* env, const JavaRef<jobject>& j_token);
...
namespace jni_zero {
template <> inline base::Token FromJniType<base::Token>(JNIEnv* env, const JavaRef<jobject>& j_object)
{
    return base::android::TokenAndroid::FromJavaToken(env, j_object);
}
template <> inline ScopedJavaLocalRef<jobject> ToJniType<base::Token>(JNIEnv* env, const base::Token& token)
{
