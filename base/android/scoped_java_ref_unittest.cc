...
#define EXPECT_SAME_OBJECT(a, b) EXPECT_TRUE(env->IsSameObject((a).obj(), (b).obj()))
...
TEST_F(ScopedJavaRefTest, Conversions)
{
    ...
    // but not implicit conversions.
    {
        ...
        new_global = str;
        EXPECT_SAME_OBJECT(new_local, str);
        static_assert(!std::is_convertible_v<ScopedJavaLocalRef<jobject>, ScopedJavaGlobalRef<jobject>>, "");
        static_assert(!std::is_convertible_v<ScopedJavaGlobalRef<jobject>, ScopedJavaLocalRef<jobject>>, "");
    }
...
protected:
    void SetUp() override
    {
        ...
        int_constructor_ = MethodID::Get<MethodID::TYPE_INSTANCE>(env, int_class_.obj(), "<init>", "(I)V");
...
TEST_F(JavaObjectArrayReaderTest, InputIteratorRequirements)
{
    ...
    bool same_type = std::is_same_v<std::iterator_traits<It>::iterator_category, std::input_iterator_tag>;
