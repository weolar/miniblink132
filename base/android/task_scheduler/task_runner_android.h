...
public:
    explicit TaskRunnerAndroid(scoped_refptr<TaskRunner> task_runner, TaskRunnerType type);
    ...
    void PostDelayedTask(JNIEnv* env, const base::android::JavaRef<jobject>& task, jlong delay, std::string& runnable_class_name);

    bool BelongsToCurrentThread(JNIEnv* env);

    static std::unique_ptr<TaskRunnerAndroid> Create(jint task_runner_type, jint j_task_traits);

    using UiThreadTaskRunnerCallback = RepeatingCallback<scoped_refptr<base::SingleThreadTaskRunner>(::TaskTraits)>;

    static void SetUiThreadTaskRunnerCallback(UiThreadTaskRunnerCallback callback);
