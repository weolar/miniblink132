...
// refcounts.
BASE_EXPORT ScopedJavaLocalRef<jobject> PackBinderBox(JNIEnv* env, std::vector<BinderRef> binders);

// Retrieves a collection of binders stashed in a binder box.
BASE_EXPORT BinderStatusOr<std::vector<BinderRef>> UnpackBinderBox(JNIEnv* env, const JavaRef<jobject>& box);
