...
using JavaExceptionFilter = base::RepeatingCallback<bool(const JavaRef<jthrowable>&)>;

LazyInstance<JavaExceptionFilter>::Leaky g_java_exception_filter = LAZY_INSTANCE_INITIALIZER;
...
void InitJavaExceptionReporter()
{
    ...
    SetJavaExceptionFilter(base::BindRepeating([](const JavaRef<jthrowable>&) { return true; }));
...
void JNI_JavaExceptionReporter_ReportJavaException(JNIEnv* env, jboolean crash_after_report, const JavaParamRef<jthrowable>& e)
{
...
void JNI_JavaExceptionReporter_ReportJavaStackTrace(JNIEnv* env, std::string& stack_trace)
{
