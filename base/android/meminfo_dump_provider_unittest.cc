...
namespace {
std::map<std::string, uint64_t> GetEntries(base::trace_event::MemoryAllocatorDump* dump)
{
    std::map<std::string, uint64_t> entries;
    for (const auto& entry : dump->entries()) {
        EXPECT_EQ(entry.entry_type, base::trace_event::MemoryAllocatorDump::Entry::kUint64);
...
TEST(MeminfoDumpProviderTest, Simple)
{
    ...
    ASSERT_TRUE(success);
    base::trace_event::MemoryAllocatorDump* first_dump = first_pmd.GetAllocatorDump(MeminfoDumpProvider::kDumpName);
    ASSERT_TRUE(first_dump);

    std::map<std::string, uint64_t> first_entries = GetEntries(first_dump);

    EXPECT_TRUE(first_entries.contains(MeminfoDumpProvider::kIsStaleName)); // Cannot assert on the value.
    // Expect the values to not be 0, that would indicate that the values are not
    // reported.
    ASSERT_TRUE(first_entries.contains(MeminfoDumpProvider::kPrivateDirtyMetricName));
    ...
    base::trace_event::MemoryAllocatorDump* second_dump = second_pmd.GetAllocatorDump(MeminfoDumpProvider::kDumpName);
    ...
    EXPECT_TRUE(second_entries[MeminfoDumpProvider::kIsStaleName]); // Entries are stale
        // this time.
    ASSERT_TRUE(second_entries.contains(MeminfoDumpProvider::kPrivateDirtyMetricName));
    ...
    if (BuildInfo::GetInstance()->sdk_int() >= SdkVersion::SDK_VERSION_Q) {
        ...
        EXPECT_EQ(first_entries[MeminfoDumpProvider::kPrivateDirtyMetricName], second_entries[MeminfoDumpProvider::kPrivateDirtyMetricName]);
        EXPECT_EQ(first_entries[MeminfoDumpProvider::kPssMetricName], second_entries[MeminfoDumpProvider::kPssMetricName]);
    }
}
...
TEST(MeminfoDumpProviderTest, NoStaleReportsInBackgroundDumps)
{
    ...
    // Second one, stale data, should not report.
    {
        ...
        base::trace_event::ProcessMemoryDump pmd { args };
        ASSERT_TRUE(instance.OnMemoryDump(args, &pmd));
        base::trace_event::MemoryAllocatorDump* dump = pmd.GetAllocatorDump(MeminfoDumpProvider::kDumpName);
        EXPECT_FALSE(dump);
    }
}
