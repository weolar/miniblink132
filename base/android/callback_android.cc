...
void RunObjectCallbackAndroid(const JavaRef<jobject>& callback, const JavaRef<jobject>& arg)
{
...
{
    Java_Helper_onBooleanResultFromNative(AttachCurrentThread(), callback, static_cast<jboolean>(arg));
}
...
{
    Java_Helper_onTimeResultFromNative(AttachCurrentThread(), callback, time.InMillisecondsSinceUnixEpoch());
}

void RunStringCallbackAndroid(const JavaRef<jobject>& callback, const std::string& arg)
{
...
void RunOptionalStringCallbackAndroid(const JavaRef<jobject>& callback, base::optional_ref<const std::string> optional_string_arg)
{
    JNIEnv* env = AttachCurrentThread();
    if (optional_string_arg.has_value()) {
        Java_Helper_onOptionalStringResultFromNative(env, callback, true, ConvertUTF8ToJavaString(env, optional_string_arg.value()));
    } else {
        Java_Helper_onOptionalStringResultFromNative(env, callback, false, ConvertUTF8ToJavaString(env, std::string()));
    }
}

void RunByteArrayCallbackAndroid(const JavaRef<jobject>& callback, const std::vector<uint8_t>& arg)
{
    JNIEnv* env = AttachCurrentThread();
