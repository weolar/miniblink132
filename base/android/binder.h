...
#define DEFINE_BINDER_CLASS(name)                                                                                                                              \
    struct name : public base::android::internal::BinderClassBase {                                                                                            \
        ...
        {                                                                                                                                                      \
            static AIBinder_Class* const clazz = RegisterBinderClass(#name ":" __FILE__ ":" _BINDER_CLASS_LINE(__LINE__));                                     \
            return clazz;                                                                                                                                      \
        }                                                                                                                                                      \
        static inline base::android::TypedBinderRef<name> AdoptBinderRef(base::android::BinderRef binder)                                                      \
        {                                                                                                                                                      \
...
template <typename T> using BinderStatusOr = expected<T, binder_status_t>;
...
public:
    ...
    template <typename Allocator> BinderStatusOr<void> ReadByteArray(Allocator allocator) const
    {
...
private:
    BinderStatusOr<void> ReadByteArrayImpl(AParcel_byteArrayAllocator allocator, void* context) const;
...
public:
    ...
    {
        return parcel_ != nullptr;
    }

    const AParcel* get() const
    {
        return parcel_;
    }
    AParcel* get()
    ...
    [[nodiscard]] AParcel* release()
    {
        return std::exchange(parcel_, nullptr);
    }

    void reset();

    ParcelReader reader() const
    {
        return ParcelReader(*this);
    }
    ParcelWriter writer()
    {
        return ParcelWriter(*this);
    }
...
public:
    ...
    [[nodiscard]] AIBinder* release()
    {
        return std::exchange(binder_, nullptr);
    }
...
protected:
    ...
    BinderStatusOr<Parcel> TransactImpl(transaction_code_t code, Parcel parcel, binder_flags_t flags);
...
class BASE_EXPORT SupportsBinderBase : public RefCountedThreadSafe<SupportsBinderBase> {
public:
    ...
    // thread-safe.
    virtual BinderStatusOr<void> OnBinderTransaction(transaction_code_t code, const ParcelReader& in, const ParcelWriter& out) = 0;
...
private:
    ...
    static binder_status_t OnIBinderTransact(AIBinder* binder, transaction_code_t code, const AParcel* in, AParcel* out);
...
template <typename T> class TypedBinderRef : public BinderRef {
public:
    static_assert(std::is_base_of_v<android::internal::BinderClassBase, T>, "Invalid binder class type");
    ...
    template <typename Fn> BinderStatusOr<Parcel> Transact(transaction_code_t code, Fn fn)
    {
    ...
    // transacted.
    template <typename Fn> BinderStatusOr<void> TransactOneWay(transaction_code_t code, Fn fn)
    {
        ASSIGN_OR_RETURN(auto parcel, PrepareTransaction());
        RETURN_IF_ERROR(fn(ParcelWriter(parcel.get())));
...
//   }
template <typename T> class BASE_EXPORT SupportsBinder : public internal::SupportsBinderBase {
public:
    static_assert(std::is_base_of_v<android::internal::BinderClassBase, T>, "Invalid binder class type");

    SupportsBinder()
        : SupportsBinderBase(T::GetBinderClass())
    {
    }
