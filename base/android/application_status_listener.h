...
public:
    using ApplicationStateChangeCallback = base::RepeatingCallback<void(ApplicationState)>;

    ApplicationStatusListener(const ApplicationStatusListener&) = delete;
    ApplicationStatusListener& operator=(const ApplicationStatusListener&) = delete;
    ...
    static std::unique_ptr<ApplicationStatusListener> New(const ApplicationStateChangeCallback& callback);
