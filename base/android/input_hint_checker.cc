...
enum class InputHintChecker::InitState { kNotStarted, kInProgress, kInitialized, kFailedToInitialize };
...
BASE_EXPORT
BASE_FEATURE(kYieldWithInputHint, "YieldWithInputHint", base::FEATURE_DISABLED_BY_DEFAULT);
...
// the hint.
const base::FeatureParam<int> kPollIntervalMillisParam { &kYieldWithInputHint, "poll_interval_ms", 3 };
...
InputHintChecker::InputHintChecker()
    : init_state_(InitState::kNotStarted)
{
}
...
void InputHintChecker::SetView(JNIEnv* env, const jni_zero::JavaParamRef<jobject>& root_view)
{
    ...
    if (state == InitState::kNotStarted) {
        ...
        view_class_ = ScopedJavaGlobalRef<jobject>(env, env->GetObjectClass(root_view.obj()));
        pthread_t new_thread;
        if (pthread_create(&new_thread, nullptr, OffThreadInitInvoker::Run, nullptr) != 0) {
...
{
    auto has_input_result = ScopedJavaLocalRef<jobject>::Adopt(env, env->CallObjectMethod(reflect_method_for_has_input_.obj(), invoke_id_, o, nullptr));
    ...
    bool value = static_cast<bool>(env->CallBooleanMethod(has_input_result.obj(), boolean_value_id_));
    if (ClearException(env)) {
...
void InputHintChecker::TransitionToState(InitState new_state)
{
    ...
    if (new_state == InitState::kInitialized || new_state == InitState::kFailedToInitialize) {
        InitializationResult r = (new_state == InitState::kInitialized) ? InitializationResult::kSuccess : InitializationResult::kFailure;
...
void InputHintChecker::InitGlobalRefsAndMethodIds(JNIEnv* env)
{
    ...
    jmethodID get_method_id = env->GetMethodID(view_class, "getMethod", "(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;");
    if (ClearException(env)) {
        LOG(ERROR) << "exception when looking for method getMethod()";
        TransitionToState(InitState::kFailedToInitialize);
        return;
    }
    ScopedJavaLocalRef<jstring> has_input_string = ConvertUTF8ToJavaString(env, "probablyHasInput");
    auto method = ScopedJavaLocalRef<jobject>::Adopt(env, env->CallObjectMethod(view_class_.obj(), get_method_id, has_input_string.obj(), nullptr));
    if (ClearException(env)) {
    ...
    jclass method_class = env->GetObjectClass(reflect_method_for_has_input_.obj());
    if (ClearException(env) || !method_class) {
        LOG(ERROR) << "exception on GetObjectClass(getMethod) or null returned";
        TransitionToState(InitState::kFailedToInitialize);
        return;
    }
    invoke_id_ = env->GetMethodID(method_class, "invoke", "(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;");
    if (ClearException(env)) {
...
InputHintChecker::ScopedOverrideInstance::ScopedOverrideInstance(InputHintChecker* checker)
{
...
void JNI_InputHintChecker_SetView(_JNIEnv* env, const jni_zero::JavaParamRef<jobject>& v)
{
...
void JNI_InputHintChecker_OnCompositorViewHolderTouchEvent(_JNIEnv* env)
{
    ...
    if (checker.is_after_input_yield()) {
        InputHintChecker::RecordInputHintResult(InputHintResult::kCompositorViewTouchEvent);
    }
...
{
    return InputHintChecker::GetInstance().FailedToInitializeForTesting(); // IN-TEST
}
