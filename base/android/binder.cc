...
#define DEFINE_BINDER_API_ENTRY(name)                                                                                                                          \
    ...
    decltype(::name)* const name = library ? (decltype(::name)*)dlsym(library, "" #name) : nullptr
...
ParcelReader::ParcelReader(const AParcel* parcel)
    : parcel_(parcel)
{
}

ParcelReader::ParcelReader(const Parcel& parcel)
    : parcel_(parcel.get())
{
}
...
BinderStatusOr<void> ParcelReader::ReadByteArrayImpl(AParcel_byteArrayAllocator allocator, void* context) const
{
    WITH_BINDER_API(api)
    {
        const auto status = api.AParcel_readByteArray(parcel_.get(), context, allocator);
        if (status != STATUS_OK) {
            return unexpected(status);
        }
        return ok();
...
BinderStatusOr<ScopedFD> ParcelReader::ReadFileDescriptor() const
{
    ...
    {
        int fd;
        const auto status = api.AParcel_readParcelFileDescriptor(parcel_.get(), &fd);
        if (status != STATUS_OK) {
            return unexpected(status);
        }
        return ScopedFD(fd);
...
ParcelWriter::ParcelWriter(AParcel* parcel)
    : parcel_(parcel)
{
}

ParcelWriter::ParcelWriter(Parcel& parcel)
    : parcel_(parcel.get())
{
}
...
BinderStatusOr<void> ParcelWriter::WriteByteArray(span<const uint8_t> bytes) const
{
    binder_status_t status = STATUS_UNEXPECTED_NULL;
    WITH_BINDER_API(api)
    {
        status = api.AParcel_writeByteArray(parcel_.get(), reinterpret_cast<const int8_t*>(bytes.data()), checked_cast<int32_t>(bytes.size()));
        if (status == STATUS_OK) {
            return ok();
        }
    }
...
BinderRef::BinderRef(AIBinder* binder)
    : binder_(binder)
{
}

BinderRef::BinderRef(const BinderRef& other)
...
BinderRef::BinderRef(BinderRef&& other)
...
ScopedJavaLocalRef<jobject> BinderRef::ToJavaBinder(JNIEnv* env) const
{
    ...
    if (binder_) {
        WITH_BINDER_API(api)
        {
            object = ScopedJavaLocalRef<jobject>::Adopt(env, api.AIBinder_toJavaBinder(env, binder_.get()));
        }
    }
    return object;
}
...
BinderStatusOr<Parcel> BinderRef::PrepareTransaction()
{
    if (binder_) {
        ...
        {
            AParcel* parcel;
            const auto status = api.AIBinder_prepareTransaction(binder_.get(), &parcel);
            if (status != STATUS_OK) {
                return unexpected(status);
            }
...
BinderStatusOr<Parcel> BinderRef::TransactImpl(transaction_code_t code, Parcel parcel, binder_flags_t flags)
{
    if (binder_) {
        WITH_BINDER_API(api)
        {
            ...
            AParcel* out;
            const auto status = api.AIBinder_transact(binder_.get(), code, &in, &out, flags);
            if (status != STATUS_OK) {
                return unexpected(status);
            }
            return Parcel(out);
...
AIBinder_Class* BinderClassBase::RegisterBinderClass(const char* name)
{
    WITH_BINDER_API(api)
    {
        return api.AIBinder_Class_define(
            name, &SupportsBinderBase::OnIBinderCreate, &SupportsBinderBase::OnIBinderDestroy, &SupportsBinderBase::OnIBinderTransact);
    }
    return nullptr;
}
...
binder_status_t SupportsBinderBase::OnIBinderTransact(AIBinder* binder, transaction_code_t code, const AParcel* in, AParcel* out)
{
    WITH_BINDER_API(api)
    {
        ...
        const auto result = target->OnBinderTransaction(code, ParcelReader(in), ParcelWriter(out));
...
void SetBindersFromParent(std::vector<BinderRef> binders)
{
    ...
    BindersFromParent() = std::make_unique<std::vector<BinderRef>>(std::move(binders));
}
