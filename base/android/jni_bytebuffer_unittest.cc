...
TEST(JniByteBuffer, ConversionDoesNotCopy)
{
    ...
    ScopedJavaLocalRef<jobject> jbuffer(env, env->NewDirectByteBuffer(bytes, sizeof(bytes)));
    ASSERT_TRUE(jbuffer);
...
TEST(JniByteBuffer, DISABLED_ConversionFromNonBuffer)
{
    ...
    jmethodID init = base::android::MethodID::Get<base::android::MethodID::TYPE_INSTANCE>(env, cls, "<init>", "()V");

    ScopedJavaLocalRef<jobject> jnonbuffer(env, env->NewObject(cls, init));

    std::optional<base::span<const uint8_t>> maybe_span = MaybeJavaByteBufferToSpan(env, jnonbuffer.obj());
...
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> jbuffer(env, env->NewDirectByteBuffer(nullptr, 0));
    ASSERT_TRUE(jbuffer);
