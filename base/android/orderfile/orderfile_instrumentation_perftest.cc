...
void RunBenchmark(int iterations, int addresses_count, int threads)
{
    ...
    auto iterate = [iterations, addresses_count]() {
        for (int i = 0; i < iterations; i++) {
            for (size_t addr = kStartOfTextForTesting; addr < static_cast<size_t>(addresses_count); addr += sizeof(int)) {
    ...
    auto ns_per_call = nanos / (iterations * static_cast<double>(addresses_count));
    auto modifier = base::StringPrintf("_%d_%d_%d", iterations, addresses_count, threads);
    perf_test::PrintResult("RecordAddressCostPerCall", modifier, "", ns_per_call, "ns", true);
}
