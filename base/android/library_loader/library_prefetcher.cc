...
bool Mincore(size_t start, size_t end, std::vector<unsigned char>* residency)
{
    ...
    int err = HANDLE_EINTR(mincore(reinterpret_cast<void*>(start), size, &(*residency)[0]));
...
struct TimestampAndResidency {
    ...
    TimestampAndResidency(uint64_t timestamp_nanos, std::vector<unsigned char>&& residency)
...
// Returns true for success.
bool CollectResidency(size_t start, size_t end, std::vector<TimestampAndResidency>* data)
{
    ...
    uint64_t now = static_cast<uint64_t>(ts.tv_sec) * 1000 * 1000 * 1000 + static_cast<uint64_t>(ts.tv_nsec);
    std::vector<unsigned char> residency;
...
void DumpResidency(size_t start, size_t end, std::unique_ptr<std::vector<TimestampAndResidency>> data)
{
    LOG(WARNING) << "Dumping native library residency";
    auto path = FilePath(StringPrintf("/data/local/tmp/chrome/residency-%d.txt", getpid()));
    auto file = File(path, File::FLAG_CREATE_ALWAYS | File::FLAG_WRITE);
    if (!file.IsValid()) {
        PLOG(ERROR) << "Cannot open file to dump the residency data " << path.value();
        return;
    }
...
void NativeLibraryPrefetcher::ForkAndPrefetchNativeLibrary(bool ordered_only)
{
    ...
    if (status != PrefetchStatus::kSuccess) {
        LOG(WARNING) << "Cannot prefetch the library. status = " << static_cast<int>(status);
    }
#endif // BUILDFLAG(ORDERFILE_INSTRUMENTATION)
}

// static
int NativeLibraryPrefetcher::PercentageOfResidentCode(size_t start, size_t end)
{
    ...
    resident_pages += static_cast<size_t>(ranges::count_if(residency, [](unsigned char x) { return x & 1; }));
    if (total_pages == 0)
        return -1;
