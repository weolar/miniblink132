...
public:
    ...
    template <typename WriteFn, typename ReadFn> void Reflect(WriteFn writer, ReadFn reader)
    {
...
private:
    ...
    BinderStatusOr<void> OnBinderTransaction(transaction_code_t code, const ParcelReader& reader, const ParcelWriter& out) override
    {
...
TEST_F(BinderTest, ReadWriteInt32)
{
    ...
    reflector->Reflect(
        [](const ParcelWriter& writer) {
            ...
            EXPECT_TRUE(writer.WriteInt32(-42).has_value());
            EXPECT_TRUE(writer.WriteInt32(std::numeric_limits<int32_t>::min()).has_value());
            EXPECT_TRUE(writer.WriteInt32(std::numeric_limits<int32_t>::max()).has_value());
        },
...
TEST_F(BinderTest, ReadWriteUint32)
{
    ...
    reflector->Reflect(
        [](const ParcelWriter& writer) {
            ...
            EXPECT_TRUE(writer.WriteUint32(std::numeric_limits<uint32_t>::min()).has_value());
            EXPECT_TRUE(writer.WriteUint32(std::numeric_limits<uint32_t>::max()).has_value());
        },
        [](const ParcelReader& reader) {
...
TEST_F(BinderTest, ReadWriteUint64)
{
    ...
    reflector->Reflect(
        [](const ParcelWriter& writer) {
            ...
            EXPECT_TRUE(writer.WriteUint64(std::numeric_limits<uint64_t>::min()).has_value());
            EXPECT_TRUE(writer.WriteUint64(std::numeric_limits<uint64_t>::max()).has_value());
        },
        [](const ParcelReader& reader) {
...
TEST_F(BinderTest, ReadWriteByteArray)
{
    ...
    reflector->Reflect([&](const ParcelWriter& writer) { EXPECT_TRUE(writer.WriteByteArray(kPrimeData).has_value()); },
        [&](const ParcelReader& reader) {
            ...
            EXPECT_TRUE(reader
                            ...
                            .has_value());
            EXPECT_TRUE(std::equal(buffer.begin(), buffer.end(), std::begin(kPrimeData), std::end(kPrimeData)));
        });
}
...
{
    auto reflector = MakeRefCounted<Reflector>();
    reflector->Reflect([&](const ParcelWriter& writer) { EXPECT_TRUE(writer.WriteByteArray({}).has_value()); },
        [](const ParcelReader& reader) {
            EXPECT_TRUE(reader
...
TEST_F(BinderTest, ReadWriteFileDescriptor)
{
    ...
    File file(temp_dir.GetPath().AppendASCII("test_file"), File::Flags::FLAG_CREATE | File::Flags::FLAG_WRITE);
    auto reflector = MakeRefCounted<Reflector>();
    reflector->Reflect([&](const ParcelWriter& writer) { EXPECT_TRUE(writer.WriteFileDescriptor(ScopedFD(file.TakePlatformFile())).has_value()); },
        [](const ParcelReader& reader) {
...
public:
    ...
    public:
        explicit Proxy(BinderRef binder)
            : Class::BinderRef(std::move(binder))
        {
        }

        int32_t Add(int32_t n)
        {
            const Parcel sum = *Transact(kAdd, [n](const auto& p) { return p.WriteInt32(n); });
...
public:
    explicit AddService(int32_t offset)
        : offset_(offset)
    {
    }
    ...
    BinderStatusOr<void> OnBinderTransaction(transaction_code_t code, const ParcelReader& in, const ParcelWriter& out) override
    {
...
public:
    ...
    public:
        ...
        {
            const Parcel product = *Transact(kMultiply, [n](const auto& p) { return p.WriteInt32(n); });
...
public:
    explicit MultiplyService(int32_t scale)
        : scale_(scale)
    {
    }
...
public:
    ...
    public:
        ...
        {
            auto reply = *Transact(kGetAdd, [offset](const auto& p) { return p.WriteInt32(offset); });
            return AddInterface::Proxy(*reply.reader().ReadBinder());
        }

        MultiplyInterface::Proxy GetMultiply(int32_t scale)
        {
            auto reply = *Transact(kGetMultiply, [scale](const auto& p) { return p.WriteInt32(scale); });
...
public:
    ...
    void WaitForAllServicesToBeDestroyed()
    {
        all_services_destroyed_.Wait();
    }
...
public:
    template <typename... Binders> Process LaunchChild(const char* name, Binders&&... binders)
    {
        CommandLine cmd = GetMultiProcessTestChildBaseCommandLine();
        LaunchOptions options;
        options.binders = std::vector<BinderRef>({ std::forward<Binders>(binders)... });
    ...
    bool JoinChild(const Process& child)
    {
        ...
        WaitForMultiprocessTestChildExit(child, TestTimeouts::action_timeout(), &child_exit_code);
...
TEST_F(BinderMultiprocessTest, PassMultipleBinders)
{
    ...
    Process child = LaunchChild("PassMultipleBinders_Child", add100->GetBinder(), multiply7->GetBinder());
    EXPECT_TRUE(JoinChild(child));
}
...
public:
    ...
    explicit BinderSink(BinderMultiprocessTest& test)
        : test_(test)
    {
    }

    template <typename Fn> Process LaunchChildAndWaitForBinder(const char* child_name, Fn fn)
    {
...
TEST_F(BinderMultiprocessTest, AssociateValid)
{
    ...
    Process child = sink->LaunchChildAndWaitForBinder(
        "Associate_Child", [](BinderRef binder) { EXPECT_TRUE(binder.AssociateWithClass(AddInterface::Class::GetBinderClass())); });
    EXPECT_TRUE(JoinChild(child));
}
...
TEST_F(BinderMultiprocessTest, DISABLED_AssociateInvalid)
{
    auto sink = base::MakeRefCounted<BinderSink>(*this);
    Process child = sink->LaunchChildAndWaitForBinder(
        "Associate_Child", [](BinderRef binder) { EXPECT_FALSE(binder.AssociateWithClass(MultiplyInterface::Class::GetBinderClass())); });
    EXPECT_TRUE(JoinChild(child));
}
...
BINDER_TEST_CHILD_MAIN(Associate_Child)
{
    ...
    auto service = base::MakeRefCounted<AddService>(15);
    std::ignore = sink.Transact(42, [&service](ParcelWriter in) { return in.WriteBinder(service->GetBinder()); });
}
...
TEST_F(BinderMultiprocessTest, AssociateDestroyed)
{
    ...
    Process child = sink->LaunchChildAndWaitForBinder("AssociateDestroyed_Child", [&](BinderRef binder) { child_binder = std::move(binder); });
    ...
    EXPECT_FALSE(child_binder.AssociateWithClass(AddInterface::Class::GetBinderClass()));
}
...
BINDER_TEST_CHILD_MAIN(AssociateDestroyed_Child)
{
    auto sink = BinderSinkClass::AdoptBinderRef(TakeBinderFromParent(0));
    auto service = base::MakeRefCounted<AddService>(15);
    std::ignore = sink.Transact(42, [&](ParcelWriter in) { return in.WriteBinder(service->GetBinder()); });
