...
public:
    ...
    static BinderStatusOr<std::vector<BinderRef>> Unpack(JNIEnv* env, const JavaRef<jobject>& box)
    {
        auto proxy = TypedBinderRef<BinderBoxInterface>::Adopt(BinderRef::FromJavaBinder(env, box.obj()));
        if (!proxy) {
            return unexpected(STATUS_BAD_TYPE);
        }
        ASSIGN_OR_RETURN(auto parcel, proxy.PrepareTransaction());
        ASSIGN_OR_RETURN(const auto reply, proxy.Transact(kUnpack, std::move(parcel)));
...
ScopedJavaLocalRef<jobject> PackBinderBox(JNIEnv* env, std::vector<BinderRef> binders)
{
    if (binders.empty()) {
        return nullptr;
    }
...
BinderStatusOr<std::vector<BinderRef>> UnpackBinderBox(JNIEnv* env, const JavaRef<jobject>& box)
{
