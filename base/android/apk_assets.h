...
BASE_EXPORT int OpenApkAsset(const std::string& file_path, const std::string& split_name, base::MemoryMappedFile::Region* region);
BASE_EXPORT int OpenApkAsset(const std::string& file_path, base::MemoryMappedFile::Region* region);

// Registers an uncompressed asset from within the apk in the
// FileDescriptorStore.
// Returns: true in case of success, false otherwise.
BASE_EXPORT bool RegisterApkAssetWithFileDescriptorStore(const std::string& key, const base::FilePath& file_path);
