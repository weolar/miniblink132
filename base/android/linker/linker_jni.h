...
#if DEBUG
#define LOG_INFO(FORMAT, ...) __android_log_print(ANDROID_LOG_INFO, TAG, "%s: " FORMAT, __FUNCTION__, ##__VA_ARGS__)
#else
#define LOG_INFO(FORMAT, ...) ((void)0)
#endif
#define LOG_ERROR(FORMAT, ...) __android_log_print(ANDROID_LOG_ERROR, TAG, "%s: " FORMAT, __FUNCTION__, ##__VA_ARGS__)
#define PLOG_ERROR(FORMAT, ...) LOG_ERROR(FORMAT ": %s", ##__VA_ARGS__, strerror(errno))
...
// |env| is the current JNI environment handle.
// On success, return true and set |*field_id|.
bool InitStaticFieldId(JNIEnv* env, jclass clazz, const char* field_name, const char* field_sig, jfieldID* field_id);
...
struct LibInfo_class {
    ...
    {
        jclass clazz;
        if (!InitClassReference(env, "org/chromium/base/library_loader/Linker$LibInfo", &clazz)) {
            return false;
        }

        return InitFieldId(env, clazz, "mLoadAddress", "J", &load_address_id) && InitFieldId(env, clazz, "mLoadSize", "J", &load_size_id)
            && InitFieldId(env, clazz, "mRelroStart", "J", &relro_start_id) && InitFieldId(env, clazz, "mRelroSize", "J", &relro_size_id)
            && InitFieldId(env, clazz, "mRelroFd", "I", &relro_fd_id);
    }

    void SetLoadInfo(JNIEnv* env, jobject library_info_obj, uintptr_t load_address, size_t load_size)
    {
        env->SetLongField(library_info_obj, load_address_id, load_address);
        env->SetLongField(library_info_obj, load_size_id, load_size);
    }

    void SetRelroInfo(JNIEnv* env, jobject library_info_obj, uintptr_t relro_start, size_t relro_size, int relro_fd)
    {
    ...
    bool GetLoadInfo(JNIEnv* env, jobject library_info_obj, uintptr_t* load_address, size_t* load_size)
    {
        if (load_address) {
        ...
        if (load_size) {
            *load_size = static_cast<uintptr_t>(env->GetLongField(library_info_obj, load_size_id));
        }
        return true;
    }

    void GetRelroInfo(JNIEnv* env, jobject library_info_obj, uintptr_t* relro_start, size_t* relro_size, int* relro_fd)
    {
        if (relro_start) {
            *relro_start = static_cast<uintptr_t>(env->GetLongField(library_info_obj, relro_start_id));
        }

        if (relro_size) {
            *relro_size = static_cast<size_t>(env->GetLongField(library_info_obj, relro_size_id));
        }
...
public:
    ...
    void set_load_address(uintptr_t a)
    {
        load_address_ = a;
    }
    ...
    void set_relro_fd_for_testing(int fd)
    {
        relro_fd_ = fd;
    }
    int get_relro_fd_for_testing() const
    {
        return relro_fd_;
    }
    size_t get_relro_start_for_testing() const
    {
        return relro_start_;
    }
    size_t get_load_size_for_testing() const
    {
        return load_size_;
    }
...
private:
    ...
    // equal among the two.
    bool RelroIsIdentical(const NativeLibInfo& external_lib_info, const SharedMemoryFunctions& functions) const;
