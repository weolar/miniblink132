...
int dl_iterate_phdr(int (*cb)(dl_phdr_info* info, size_t size, void* data), void* data) __attribute__((weak_import));
...
public:
    explicit LibraryRangeFinder(uintptr_t address)
    ...
    uintptr_t relro_start() const
    {
        return relro_start_;
    }
    size_t relro_size() const
    {
        return relro_size_;
    }

    static int VisitLibraryPhdrs(dl_phdr_info* info, [[maybe_unused]] size_t size, void* data);
...
// static
int LibraryRangeFinder::VisitLibraryPhdrs(dl_phdr_info* info, [[maybe_unused]] size_t size, void* data)
{
    ...
    if (is_matching) {
        finder->load_size_ = PageEnd(kPageSize, max_vaddr) - PageStart(kPageSize, min_vaddr);
        finder->relro_size_ = PageEnd(kPageSize, max_relro_vaddr) - PageStart(kPageSize, min_relro_vaddr);
        finder->relro_start_ = info->dlpi_addr + PageStart(kPageSize, min_relro_vaddr);
...
TEST_F(LinkerTest, CreatedRegionIsSealed)
{
    ...
    void* relro_address = mmap(nullptr, kRelroSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ASSERT_NE(MAP_FAILED, relro_address);
    NativeLibInfo lib_info = { 0, 0 };
    lib_info.set_relro_info_for_testing(reinterpret_cast<uintptr_t>(relro_address), kRelroSize);
    ...
    void* ro_address = mmap(nullptr, kRelroSize, PROT_READ, MAP_SHARED, relro_fd, 0);
    ...
    EXPECT_EQ(MAP_FAILED, mmap(nullptr, kRelroSize, PROT_READ | PROT_WRITE, MAP_SHARED, relro_fd, 0));
    EXPECT_EQ(MAP_FAILED, mmap(nullptr, kRelroSize, PROT_READ | PROT_WRITE, MAP_PRIVATE, relro_fd, 0));
    EXPECT_EQ(MAP_FAILED, mmap(nullptr, kRelroSize, PROT_WRITE, MAP_SHARED, relro_fd, 0));
    EXPECT_EQ(MAP_FAILED, mmap(nullptr, kRelroSize, PROT_WRITE, MAP_PRIVATE, relro_fd, 0));
}
...
TEST_F(LinkerTest, FindReservedMemoryRegion)
{
    ...
    void* synthetic_region_start = mmap(nullptr, kSize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ASSERT_NE(MAP_FAILED, synthetic_region_start);
    prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, synthetic_region_start, kSize, "[anon:libwebview reservation]");
...
TEST_F(LinkerTest, FindLibraryRanges)
{
    ...
    EXPECT_LE(inside_library, lib_info.load_address() + lib_info.get_load_size_for_testing());

    EXPECT_LE(lib_info.load_address(), lib_info.get_relro_start_for_testing());
    EXPECT_LE(lib_info.get_relro_start_for_testing(), lib_info.load_address() + lib_info.get_load_size_for_testing());
}
...
TEST_F(LinkerTest, LibraryRangesViaIteratePhdr)
{
    ...
    ASSERT_EQ(1, dl_iterate_phdr(&LibraryRangeFinder::VisitLibraryPhdrs, reinterpret_cast<void*>(&finder)));
    ASSERT_LE(finder.relro_start() + finder.relro_size(), finder.load_address() + finder.load_size());
