...
void ReserveAddressWithHint(uintptr_t hint, uintptr_t* address, size_t* size)
{
    ...
    void* new_ptr = mmap(ptr, kAddressSpaceReservationSize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (new_ptr == MAP_FAILED) {
        ...
        *address = 0;
        ...
        LOG_ERROR("Address range starting at 0x%" PRIxPTR " was not free to use", hint);
        munmap(new_ptr, kAddressSpaceReservationSize);
        *address = 0;
    } else {
        *address = reinterpret_cast<uintptr_t>(new_ptr);
        *size = kAddressSpaceReservationSize;
        LOG_INFO("Reserved region at address: 0x%" PRIxPTR ", size: 0x%zu", *address, *size);
    }
}

bool ScanRegionInBuffer(const char* buf, size_t length, uintptr_t* out_address, size_t* out_size)
{
    ...
    if (sscanf(line_start, "%" SCNxPTR "-%" SCNxPTR " %4c", &vma_start, &vma_end, permissions) < 3) {
        return false;
    }
...
#pragma clang diagnostic pop
    ...
    while (true) {
        ...
        size_t bytes_read = 0;
        do {
            ssize_t rv = HANDLE_EINTR(read(fd, buf + pos + bytes_read, bytes_requested - bytes_read));
            if (rv == 0) {
                reached_end = true;
            } else if (rv < 0) {
...
bool AndroidDlopenExt(void* mapping_start, size_t mapping_size, const char* filename, void** handle)
{
...
bool InitFieldId(JNIEnv* env, jclass clazz, const char* field_name, const char* field_sig, jfieldID* field_id)
{
...
struct SharedMemoryFunctions {
    SharedMemoryFunctions()
    {
        ...
        create = reinterpret_cast<CreateFunction>(dlsym(library_handle, "ASharedMemory_create"));
        set_protection = reinterpret_cast<SetProtectionFunction>(dlsym(library_handle, "ASharedMemory_setProt"));
    }
...
void NativeLibInfo::ExportRelroInfoToJava() const
{
    if (!env_)
        return;
    s_lib_info_fields.SetRelroInfo(env_, java_object_, relro_start_, relro_size_, relro_fd_);
}
...
bool NativeLibInfo::FindRelroAndLibraryRangesInElf()
{
    ...
    const auto* phdrs = reinterpret_cast<const ElfW(Phdr)*>(load_address_ + ehdr->e_phoff);
    const size_t kPageSize = GetPageSize();
    ...
    relro_size_ = PageEnd(kPageSize, max_relro_vaddr) - PageStart(kPageSize, min_relro_vaddr);
...
bool NativeLibInfo::LoadWithDlopenExt(const String& path, void** handle)
{
    ...
    if (!AndroidDlopenExt(address, reservation_size, path.c_str(), &local_handle)) {
...
bool NativeLibInfo::CreateSharedRelroFd(const SharedMemoryFunctions& functions)
{
    LOG_INFO("Entering");
    ...
    void* relro_copy_addr = mmap(nullptr, relro_size_, rw_flags, MAP_SHARED, shared_mem_fd, 0);
...
bool NativeLibInfo::ReplaceRelroWithSharedOne(const SharedMemoryFunctions& functions) const
{
    LOG_INFO("Entering");
    ...
    void* new_addr = mmap(reinterpret_cast<void*>(relro_start_), relro_size_, PROT_READ, MAP_FIXED | MAP_SHARED, relro_fd_, 0);
...
bool NativeLibInfo::CopyFromJavaObject()
{
    ...
    if (!s_lib_info_fields.GetLoadInfo(env_, java_object_, &load_address_, &load_size_)) {
        return false;
    }
    s_lib_info_fields.GetRelroInfo(env_, java_object_, &relro_start_, &relro_size_, &relro_fd_);
    return true;
}

bool NativeLibInfo::LoadLibrary(const String& library_path, bool spawn_relro_region)
{
    // Load the library.
    void* handle = nullptr;
    ...
    LOG_INFO("Created and converted RELRO to shared memory: relro_fd=%d, "
...
bool NativeLibInfo::RelroIsIdentical(const NativeLibInfo& other_lib_info, const SharedMemoryFunctions& functions) const
{
    ...
    // the field.
    if (other_lib_info.relro_start_ != relro_start_ || other_lib_info.relro_size_ != relro_size_ || other_lib_info.load_size_ != load_size_) {
        LOG_ERROR("Incoming RELRO size does not match RELRO of the loaded library");
        return false;
    }
    void* shared_relro_address = mmap(nullptr, other_lib_info.relro_size_, PROT_READ, MAP_SHARED, other_lib_info.relro_fd_, 0);
    ...
    int not_equal = memcmp(shared_relro_address, current_relro_address, relro_size_);
...
bool NativeLibInfo::CompareRelroAndReplaceItBy(const NativeLibInfo& other_lib_info)
{
    ...
    if (!FindRelroAndLibraryRangesInElf()) {
        LOG_ERROR("Could not find RELRO from externally provided address: 0x%p", reinterpret_cast<void*>(other_lib_info.load_address_));
...
JNI_BOUNDARY_EXPORT void Java_org_chromium_base_library_1loader_LinkerJni_nativeFindMemoryRegionAtRandomAddress(JNIEnv* env, jclass clazz, jobject lib_info_obj)
{
    LOG_INFO("Entering");
    uintptr_t address;
    size_t size;
...
JNI_BOUNDARY_EXPORT void Java_org_chromium_base_library_1loader_LinkerJni_nativeReserveMemoryForLibrary(JNIEnv* env, jclass clazz, jobject lib_info_obj)
{
    LOG_INFO("Entering");
    uintptr_t address;
    size_t size;
...
JNI_BOUNDARY_EXPORT jboolean Java_org_chromium_base_library_1loader_LinkerJni_nativeFindRegionReservedByWebViewZygote(
    JNIEnv* env, jclass clazz, jobject lib_info_obj)
{
    LOG_INFO("Entering");
    uintptr_t address;
    size_t size;
...
JNI_BOUNDARY_EXPORT jboolean Java_org_chromium_base_library_1loader_LinkerJni_nativeLoadLibrary(
    JNIEnv* env, jclass clazz, jstring jdlopen_ext_path, jobject lib_info_obj, jboolean spawn_relro_region)
{
    LOG_INFO("Entering");
...
JNI_BOUNDARY_EXPORT jboolean Java_org_chromium_base_library_1loader_LinkerJni_nativeUseRelros(
    JNIEnv* env, jclass clazz, jlong local_load_address, jobject remote_lib_info_obj)
{
    LOG_INFO("Entering");
    // Copy the contents from the Java-side LibInfo object.
...
JNI_BOUNDARY_EXPORT jint Java_org_chromium_base_library_1loader_LinkerJni_nativeGetRelroSharingResult(JNIEnv* env, jclass clazz)
{
