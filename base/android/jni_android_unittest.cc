...
private:
    ...
    const JavaExceptionCallback prev_exception_callback_ = GetJavaExceptionCallback();
...
JniAndroidExceptionTestContext* JniAndroidExceptionTestContext::instance = nullptr;

std::atomic<jmethodID> g_atomic_id(nullptr);
int LazyMethodIDCall(JNIEnv* env, jclass clazz, int p)
{
    jmethodID id = base::android::MethodID::LazyGet<base::android::MethodID::TYPE_STATIC>(env, clazz, "abs", "(I)I", &g_atomic_id);
...
TEST(JniAndroidExceptionTest, HandleExceptionInNative)
{
    ...
    EXPECT_THAT(ctx.last_java_exception, Optional(StartsWith("java.lang.RuntimeException")));
...
TEST(JniAndroidExceptionTest, HandleExceptionInJava_NoOpHandler)
{
    ...
    EXPECT_THAT(ctx.last_java_exception, Optional(StartsWith("java.lang.RuntimeException")));
    EXPECT_THAT(ctx.assertion_message, Optional(Eq(kUncaughtExceptionHandlerFailedMessage)));
}
...
TEST(JniAndroidExceptionTest, HandleExceptionInJava_ThrowingHandler)
{
    ...
    EXPECT_THAT(ctx.last_java_exception, Optional(StartsWith("java.lang.IllegalStateException")));
    EXPECT_THAT(ctx.assertion_message, Optional(Eq(kUncaughtExceptionHandlerFailedMessage)));
}
...
TEST(JniAndroidExceptionTest, HandleExceptionInJava_OomInGetJavaExceptionInfo)
{
    ...
    EXPECT_THAT(ctx.last_java_exception, Optional(Eq(kOomInGetJavaExceptionInfoMessage)));
    EXPECT_THAT(ctx.assertion_message, Optional(Eq(kUncaughtExceptionHandlerFailedMessage)));
}
...
TEST(JniAndroidExceptionTest, HandleExceptionInJava_ReentrantOom)
{
    ...
    EXPECT_THAT(ctx.last_java_exception, Optional(Eq(kReetrantOutOfMemoryMessage)));
