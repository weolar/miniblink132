...
BASE_EXPORT std::optional<std::string> TranslateOpenFlagsToJavaMode(uint32_t open_flags);
...
bool ContentUriGetFileInfo(const FilePath& content_uri, FileEnumerator::FileInfo* results);
...
BASE_EXPORT bool MaybeGetFileDisplayName(const FilePath& content_uri, std::u16string* file_display_name);

// Build URI using tree_uri and encoded_document_id.
BASE_EXPORT FilePath ContentUriBuildDocumentUriUsingTree(const FilePath& tree_uri, const std::string& encoded_document_id);
...
BASE_EXPORT FilePath ContentUriGetChildDocumentOrQuery(
...
BASE_EXPORT bool ContentUriIsCreateChildDocumentQuery(const FilePath& content_uri);
...
BASE_EXPORT FilePath ContentUriGetDocumentFromQuery(const FilePath& document_query, bool create);
