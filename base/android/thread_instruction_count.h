...
public:
    ...
    explicit constexpr ThreadInstructionDelta(int64_t delta)
        : delta_(delta)
    {
    }
...
public:
    ...
    constexpr ThreadInstructionDelta operator-(ThreadInstructionCount other) const
    {
        return ThreadInstructionDelta(static_cast<int64_t>(value_ - other.value_));
    }

    constexpr uint64_t ToInternalValue() const
    {
        return value_;
    }
