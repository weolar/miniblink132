...
TEST(ContentUriUtilsTest, TranslateOpenFlagsToJavaMode)
{
    ...
    for (const auto open_or_create :
        std::vector<uint32_t>({ 0u, File::FLAG_OPEN, File::FLAG_CREATE, File::FLAG_OPEN_ALWAYS, File::FLAG_CREATE_ALWAYS, File::FLAG_OPEN_TRUNCATED })) {
        for (const auto read_write_append :
            std::vector<uint32_t>({ 0u, File::FLAG_READ, File::FLAG_WRITE, File::FLAG_APPEND, File::FLAG_READ | File::FLAG_WRITE })) {
            for (const auto other : std::vector<uint32_t>({ 0u, File::FLAG_DELETE_ON_CLOSE, File::FLAG_TERMINAL_DEVICE })) {
...
TEST(ContentUriUtilsTest, GetFileInfo)
{
    ...
    FilePath content_uri_not_exists = *test::android::GetContentUriFromCacheDirFilePath(not_exists);
...
TEST(ContentUriUtilsTest, ContentUriBuildDocumentUriUsingTree)
{
    ...
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc:bar").value(), "content://authority/tree/foo/document/doc%3Abar");

    // `%` should not get encoded again to `%25` when it is a valid encoding, but
    // chars are upper-cased.
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc%3Abar").value(), "content://authority/tree/foo/document/doc%3Abar");
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc%3abar").value(), "content://authority/tree/foo/document/doc%3Abar");

    // Strange stuff happens if the encoding is invalid.
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc%").value(), "content://authority/tree/foo/document/doc%EF%BF%BD");
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc%3").value(), "content://authority/tree/foo/document/doc%EF%BF%BD");
    EXPECT_EQ(ContentUriBuildDocumentUriUsingTree(tree_uri, "doc%xy").value(), "content://authority/tree/foo/document/doc%EF%BF%BD%00y");
}
...
TEST(ContentUriUtilsTest, GetOrCreateByDisplayName)
{
    ...
    FilePath parent = *test::android::GetInMemoryContentTreeUriFromCacheDirDirectory(dir);

    // If there is a match, the result should be the valid tree URI.
    bool is_directory = false;
    bool create = false;
    FilePath child = ContentUriGetChildDocumentOrQuery(parent, "child1.txt", "text/plain", is_directory, create);
    EXPECT_EQ(child.value(),
        "content://org.chromium.native_test.docprov/tree/" + temp_dir.GetPath().BaseName().value() + "%2Fdir/document/" + temp_dir.GetPath().BaseName().value()
            + "%2Fdir%2Fchild1.txt");
    ...
    child = ContentUriGetChildDocumentOrQuery(parent, "child2.txt", "text/plain", is_directory, create);
    ...
    create = true;
    FilePath query = ContentUriGetChildDocumentOrQuery(parent, "child2.txt", "text/plain", is_directory, create);
    EXPECT_EQ(query.value(),
        "content://org.chromium.native_test.docprov/create-child-document/"
        "tree/"
            + temp_dir.GetPath().BaseName().value() + "%2Fdir/document/" + temp_dir.GetPath().BaseName().value()
