...
public:
    ...
    static jni_zero::ScopedJavaLocalRef<jobject> Create(JNIEnv* env, const base::UnguessableToken& token);

    // Create a native UnguessableToken from Java UnguessableToken |token|.
    static base::UnguessableToken FromJavaUnguessableToken(JNIEnv* env, const jni_zero::JavaRef<jobject>& token);
    ...
    // in a clone of |token|.
    static jni_zero::ScopedJavaLocalRef<jobject> ParcelAndUnparcelForTesting(JNIEnv* env, const jni_zero::JavaRef<jobject>& token);
...
namespace jni_zero {
template <> inline base::UnguessableToken FromJniType<base::UnguessableToken>(JNIEnv* env, const JavaRef<jobject>& j_object)
{
    return base::android::UnguessableTokenAndroid::FromJavaUnguessableToken(env, j_object);
}

template <> inline ScopedJavaLocalRef<jobject> ToJniType<base::UnguessableToken>(JNIEnv* env, const base::UnguessableToken& token)
{
