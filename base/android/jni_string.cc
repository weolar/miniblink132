...
{
    jstring result = env->NewString(reinterpret_cast<const jchar*>(str.data()), base::checked_cast<jsize>(str.length()));
...
void ConvertJavaStringToUTF8(JNIEnv* env, jstring str, std::string* result)
{
    ...
    if (length <= BUFFER_SIZE) {
        ...
        UTF16ToUTF8(reinterpret_cast<const char16_t*>(chars.data()), static_cast<size_t>(length), result);
    } else {
        // slow path
        ...
        DCHECK(chars);
        UTF16ToUTF8(reinterpret_cast<const char16_t*>(chars), static_cast<size_t>(length), result);
...
ScopedJavaLocalRef<jstring> ConvertUTF8ToJavaString(JNIEnv* env, std::string_view str)
{
...
void ConvertJavaStringToUTF16(JNIEnv* env, jstring str, std::u16string* result)
{
    DCHECK(str);
    if (!str) {
    ...
    if (length <= BUFFER_SIZE) {
        ...
        result->assign(reinterpret_cast<const char16_t*>(chars.data()), static_cast<size_t>(length));
    } else {
        // slow path
        const jchar* chars = env->GetStringChars(str, NULL);
        DCHECK(chars);
        ...
        result->assign(reinterpret_cast<const char16_t*>(chars), static_cast<size_t>(length));
        env->ReleaseStringChars(str, chars);
    }
    CheckException(env);
}
...
std::u16string ConvertJavaStringToUTF16(JNIEnv* env, const JavaRef<jstring>& str)
{
    return ConvertJavaStringToUTF16(env, str.obj());
}

ScopedJavaLocalRef<jstring> ConvertUTF16ToJavaString(JNIEnv* env, std::u16string_view str)
{
    return ScopedJavaLocalRef<jstring>(env, ConvertUTF16ToJavaStringImpl(env, str));
}
