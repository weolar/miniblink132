...
public:
    ...
    void Run()
    {
        ...
        thread_.Start();
        thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&MultiThreadedTest::RegisterThreadForEvents, base::Unretained(this)));
        ...
        // Again
        ApplicationStatusListener::NotifyApplicationStateChange(APPLICATION_STATE_HAS_DESTROYED_ACTIVITIES);
        event_.Wait();
...
private:
    ...
    {
        ExpectOnThread();
        listener_ = ApplicationStatusListener::New(base::BindRepeating(&MultiThreadedTest::StoreStateAndSignal, base::Unretained(this)));
...
TEST(ApplicationStatusListenerTest, SingleThread)
{
    ...
    // state change.
    auto listener = ApplicationStatusListener::New(base::BindRepeating(&StoreStateTo, base::Unretained(&result)));
