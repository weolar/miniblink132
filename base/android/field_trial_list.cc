...
public:
    ...
    void OnFieldTrialGroupFinalized(const base::FieldTrial& trial, const std::string& group_name) override
    {
        Log(trial.trial_name(), group_name);
    }

    static void Log(const std::string& trial_name, const std::string& group_name)
    {
        ...
        // in the logcat.
        LOG(INFO) << "Active field trial \"" << trial_name << "\" in group \"" << group_name << '"';
    }
...
base::LazyInstance<TrialLogger>::Leaky g_trial_logger = LAZY_INSTANCE_INITIALIZER;

} // namespace

static std::string JNI_FieldTrialList_FindFullName(JNIEnv* env, std::string& trial_name)
{
    return base::FieldTrialList::FindFullName(trial_name);
}

static jboolean JNI_FieldTrialList_TrialExists(JNIEnv* env, std::string& trial_name)
{
    return base::FieldTrialList::TrialExists(trial_name);
}

static std::string JNI_FieldTrialList_GetVariationParameter(JNIEnv* env, std::string& trial_name, std::string& parameter_key)
{
...
private:
    ...
    static void GetActiveFieldTrialGroups(base::FieldTrial::ActiveGroups* active_groups)
    {
        base::FieldTrialListIncludingLowAnonymity::GetActiveFieldTrialGroups(active_groups);
    }
};
...
static void JNI_FieldTrialList_LogActiveTrials(JNIEnv* env)
{
    ...
    AndroidFieldTrialListLogActiveTrialsFriendHelper::AddObserver(&g_trial_logger.Get());

    // Log any trials that were already active before adding the observer.
    std::vector<base::FieldTrial::ActiveGroup> active_groups;
    AndroidFieldTrialListLogActiveTrialsFriendHelper::GetActiveFieldTrialGroups(&active_groups);
    for (const base::FieldTrial::ActiveGroup& group : active_groups) {
        TrialLogger::Log(group.trial_name, group.group_name);
    }
}

static jboolean JNI_FieldTrialList_CreateFieldTrial(JNIEnv* env, std::string& trial_name, std::string& group_name)
{
    return base::FieldTrialList::CreateFieldTrial(trial_name, group_name) != nullptr;
}
