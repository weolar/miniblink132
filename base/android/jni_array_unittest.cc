...
TEST(JniArray, GetLength)
{
    ...
    ScopedJavaLocalRef<jbyteArray> j_empty_bytes = ToJavaByteArray(env, base::span<uint8_t>());
...
TEST(JniArray, BasicConversions)
{
    ...
    ScopedJavaLocalRef<jbyteArray> bytes_from_string = ToJavaByteArray(env, input_string);
    ASSERT_TRUE(bytes_from_string);
    ASSERT_EQ(4U, SafeGetArrayLength(env, bytes_from_string));

    ScopedJavaLocalRef<jbyteArray> bytes_from_ptr = UNSAFE_BUFFERS(ToJavaByteArray(env, bytes.data(), bytes.size()));
...
void CheckBoolConversion(JNIEnv* env, span<const bool> bool_array, const ScopedJavaLocalRef<jbooleanArray>& booleans)
{
    ...
    for (size_t i = 0; i < bool_array.size(); ++i) {
        env->GetBooleanArrayRegion(booleans.obj(), checked_cast<jsize>(i), jsize { 1 }, &value);
...
void CheckIntConversion(JNIEnv* env, span<const int> in, const ScopedJavaLocalRef<jintArray>& ints)
{
    ASSERT_TRUE(ints);
...
{
    const int kInts[] = { 0, 1, -1, std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::max() };

    JNIEnv* env = AttachCurrentThread();
    CheckIntConversion(env, kInts, ToJavaIntArray(env, kInts));
}

void CheckLongConversion(JNIEnv* env, span<const int64_t> in, const ScopedJavaLocalRef<jlongArray>& longs)
{
    ASSERT_TRUE(longs);
...
{
    const int64_t kLongs[] = { 0, 1, -1, std::numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max() };

    JNIEnv* env = AttachCurrentThread();
    CheckLongConversion(env, kLongs, ToJavaLongArray(env, kLongs));
}

void CheckFloatConversion(JNIEnv* env, span<const float> in, const ScopedJavaLocalRef<jfloatArray>& floats)
{
    ASSERT_TRUE(floats);
...
void CheckDoubleConversion(JNIEnv* env, span<const double> in, const ScopedJavaLocalRef<jdoubleArray>& doubles)
{
    ASSERT_TRUE(doubles);
...
{
    std::vector<std::vector<std::string>> kArrays = { { "a", "f" }, { "a", "" }, {}, { "" }, { "今日は" } };
...
{
    std::vector<std::vector<std::u16string>> kArrays = { { u"a", u"f" }, { u"a", u"" }, {}, { u"" } };
...
void CheckBoolArrayConversion(JNIEnv* env, ScopedJavaLocalRef<jbooleanArray> jbooleans, std::vector<bool> bool_vector)
{
    jboolean value;
...
TEST(JniArray, JavaBooleanArrayToBoolVector)
{
    ...
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jbooleanArray> jbooleans(env, env->NewBooleanArray(kBools.size()));
    ...
    ASSERT_EQ(checked_cast<jsize>(bools.size()), env->GetArrayLength(jbooleans.obj()));
    ASSERT_EQ(bools.size(), kBools.size());

    CheckBoolArrayConversion(env, jbooleans, bools);
}

void CheckIntArrayConversion(JNIEnv* env, ScopedJavaLocalRef<jintArray> jints, std::vector<int> int_vector)
{
    jint value;
...
TEST(JniArray, JavaLongArrayToInt64Vector)
{
    ...
    ASSERT_EQ(checked_cast<jsize>(int64s.size()), env->GetArrayLength(jlongs.obj()));
...
TEST(JniArray, JavaLongArrayToLongVector)
{
    ...
    ASSERT_EQ(checked_cast<jsize>(jlongs_vector.size()), env->GetArrayLength(jlongs.obj()));
...
TEST(JniArray, JavaFloatArrayToFloatVector)
{
    ...
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jfloatArray> jfloats(env, env->NewFloatArray(kFloats.size()));
    ASSERT_TRUE(jfloats);
    ...
    ASSERT_EQ(checked_cast<jsize>(floats.size()), env->GetArrayLength(jfloats.obj()));
...
{
    const auto kDoubles = std::to_array<double>({ 0.0, 0.5, -0.5, std::numeric_limits<double>::min() });
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jdoubleArray> jdoubles(env, env->NewDoubleArray(kDoubles.size()));
    ASSERT_TRUE(jdoubles);

    env->SetDoubleArrayRegion(jdoubles.obj(), 0, kDoubles.size(), reinterpret_cast<const jdouble*>(kDoubles.data()));
    ASSERT_FALSE(HasException(env));
...
TEST(JniArray, JavaArrayOfByteArrayToStringVector)
{
    ...
    ScopedJavaLocalRef<jobjectArray> array(env, env->NewObjectArray(kMaxItems, byte_array_clazz.obj(), NULL));
    ASSERT_TRUE(array);
    ...
    for (int i = 0; i < kMaxItems; ++i) {
        ...
        ScopedJavaLocalRef<jbyteArray> byte_array = ToJavaByteArray(env, base::as_byte_span(text));
...
TEST(JniArray, JavaArrayOfByteArrayToBytesVector)
{
    ...
    ScopedJavaLocalRef<jobjectArray> array(env, env->NewObjectArray(kMaxItems, byte_array_clazz.obj(), nullptr));
    ASSERT_TRUE(array);
...
{
    const std::vector<std::vector<std::u16string>> kArrays = { { u"a", u"f" }, { u"a", u"" }, {}, { u"" } };

    JNIEnv* env = AttachCurrentThread();

    ScopedJavaLocalRef<jobjectArray> array(env, env->NewObjectArray(kArrays.size(), env->FindClass("[Ljava/lang/String;"), NULL));
    ASSERT_TRUE(array);

    ScopedJavaLocalRef<jclass> string_clazz(env, env->FindClass("java/lang/String"));
    ASSERT_TRUE(string_clazz);

    for (size_t i = 0; i < kArrays.size(); ++i) {
        const std::vector<std::u16string>& child_data = kArrays[i];

        ScopedJavaLocalRef<jobjectArray> child_array(env, env->NewObjectArray(child_data.size(), string_clazz.obj(), NULL));
        ASSERT_TRUE(child_array);

        for (size_t j = 0; j < child_data.size(); ++j) {
            ScopedJavaLocalRef<jstring> item = base::android::ConvertUTF16ToJavaString(env, child_data[j]);
...
TEST(JniArray, JavaArrayOfIntArrayToIntVector)
{
    ...
    ScopedJavaLocalRef<jobjectArray> array(env, env->NewObjectArray(kNumItems, int_array_clazz.obj(), nullptr));
    ASSERT_TRUE(array);

    // Populate int[][] object.
    const auto kInts0 = std::to_array<int>({ 0, 1, -1, std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::max() });
...
TEST(JniArray, ToJavaArrayOfObjectsOfClass)
{
    ...
    ScopedJavaLocalRef<jobjectArray> j_array = ToJavaArrayOfObjects(env, jni_zero::g_string_class, objects);
    ASSERT_TRUE(j_array);

    EXPECT_EQ("one", ConvertJavaStringToUTF8(env, ScopedJavaLocalRef<jstring>(env, static_cast<jstring>(env->GetObjectArrayElement(j_array.obj(), 0)))));
    EXPECT_EQ("two", ConvertJavaStringToUTF8(env, ScopedJavaLocalRef<jstring>(env, static_cast<jstring>(env->GetObjectArrayElement(j_array.obj(), 1)))));
    EXPECT_EQ("three", ConvertJavaStringToUTF8(env, ScopedJavaLocalRef<jstring>(env, static_cast<jstring>(env->GetObjectArrayElement(j_array.obj(), 2)))));
}
