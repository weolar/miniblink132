...
JavaHandlerThread::JavaHandlerThread(const char* name, base::ThreadType thread_type)
    : JavaHandlerThread(name, Java_JavaHandlerThread_create(AttachCurrentThread(), name, base::internal::ThreadTypeToNiceValue(thread_type)))
{
}

JavaHandlerThread::JavaHandlerThread(const char* name, const base::android::ScopedJavaLocalRef<jobject>& obj)
    : name_(name)
    , java_thread_(obj)
{
}
...
void JavaHandlerThread::Start()
{
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    base::WaitableEvent initialize_event(WaitableEvent::ResetPolicy::AUTOMATIC, WaitableEvent::InitialState::NOT_SIGNALED);
    Java_JavaHandlerThread_startAndInitialize(env, java_thread_, reinterpret_cast<intptr_t>(this), reinterpret_cast<intptr_t>(&initialize_event));
...
void JavaHandlerThread::Stop()
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&JavaHandlerThread::StopOnThread, base::Unretained(this)));
    JNIEnv* env = base::android::AttachCurrentThread();
...
{
    base::ThreadIdNameManager::GetInstance()->RegisterThread(base::PlatformThread::CurrentHandle().platform_handle(), base::PlatformThread::CurrentId());
...
void JavaHandlerThread::OnLooperStopped(JNIEnv* env)
{
    ...
    base::ThreadIdNameManager::GetInstance()->RemoveName(base::PlatformThread::CurrentHandle().platform_handle(), base::PlatformThread::CurrentId());
}
...
void JavaHandlerThread::ListenForUncaughtExceptionsForTesting()
{
    DCHECK(!task_runner()->BelongsToCurrentThread());
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_JavaHandlerThread_listenForUncaughtExceptionsForTesting(env, java_thread_);
}
...
void JavaHandlerThread::StopOnThread()
{
    DCHECK(task_runner()->BelongsToCurrentThread());
    DCHECK(state_);
    state_->pump->QuitWhenIdle(base::BindOnce(&JavaHandlerThread::QuitThreadSafely, base::Unretained(this)));
}
...
void JavaHandlerThread::QuitThreadSafely()
{
    DCHECK(task_runner()->BelongsToCurrentThread());
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_JavaHandlerThread_quitThreadSafely(env, java_thread_, reinterpret_cast<intptr_t>(this));
}

JavaHandlerThread::State::State()
    : sequence_manager(sequence_manager::CreateUnboundSequenceManager(
        sequence_manager::SequenceManager::Settings::Builder().SetMessagePumpType(base::MessagePumpType::JAVA).Build()))
    , default_task_queue(sequence_manager->CreateTaskQueue(sequence_manager::TaskQueue::Spec(sequence_manager::QueueName::DEFAULT_TQ)))
{
    // TYPE_JAVA to get the Android java style message loop.
    std::unique_ptr<MessagePump> message_pump = MessagePump::Create(base::MessagePumpType::JAVA);
    ...
    static_cast<sequence_manager::internal::SequenceManagerImpl*>(sequence_manager.get())->SetTaskRunner(default_task_queue->task_runner());
