...
const char* GetPerScrollHistogramName(JankScenario scenario, int num_frames, PerScrollHistogramType type, bool with_scroll_size_suffix)
{
    ...
    if (scenario == JankScenario::WEBVIEW_SCROLLING) {
        if (type == PerScrollHistogramType::kPercentage) {
            ...
            if (num_frames <= 16) {
                ...
                return HISTOGRAM_NAME(WebviewScrolling, DelayedFramesPercentage, .Medium);
            } else {
...
void EmitMetrics(JankScenario scenario, int janky_frame_count, int missed_vsyncs_max, int missed_vsyncs_sum, int num_presented_frames,
    int64_t reporting_interval_start_time, int64_t reporting_interval_duration)
{
    DCHECK_GT(num_presented_frames, 0);
    int delayed_frames_percentage = (100 * janky_frame_count) / num_presented_frames;
    if (reporting_interval_start_time > 0) {
        // The following code does nothing if base tracing is disabled.
        [[maybe_unused]] int non_janky_frame_count = num_presented_frames - janky_frame_count;
        [[maybe_unused]] auto t = perfetto::Track(static_cast<uint64_t>(reporting_interval_start_time + static_cast<int>(scenario)));
        TRACE_EVENT_BEGIN("android_webview.timeline,android.ui.jank", "JankMetricsReportingInterval", t,
            base::TimeTicks::FromUptimeMillis(reporting_interval_start_time), "janky_frames", janky_frame_count, "non_janky_frames", non_janky_frame_count,
            "scenario", static_cast<int>(scenario), "delayed_frames_percentage", delayed_frames_percentage, "missed_vsyns_max", missed_vsyncs_max,
            "missed_vsyncs_sum", missed_vsyncs_sum);
        TRACE_EVENT_END(
            "android_webview.timeline,android.ui.jank", t, base::TimeTicks::FromUptimeMillis((reporting_interval_start_time + reporting_interval_duration)));
    }

    if (scenario != JankScenario::WEBVIEW_SCROLLING && scenario != JankScenario::FEED_SCROLLING) {
        return;
    }
    // Emit non-bucketed per scroll metrics.
    base::UmaHistogramPercentage(GetPerScrollHistogramName(scenario, num_presented_frames, PerScrollHistogramType::kPercentage,
                                     /*with_scroll_size_suffix=*/false),
        delayed_frames_percentage);
    base::UmaHistogramCustomCounts(GetPerScrollHistogramName(scenario, num_presented_frames, PerScrollHistogramType::kMax,
                                       /*with_scroll_size_suffix=*/false),
        missed_vsyncs_max, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    base::UmaHistogramCustomCounts(GetPerScrollHistogramName(scenario, num_presented_frames, PerScrollHistogramType::kSum,
                                       /*with_scroll_size_suffix=*/false),
...
const char* GetAndroidFrameTimelineDurationHistogramName(JankScenario scenario)
{
...
// testing.
void JNI_JankMetricUMARecorder_RecordJankMetrics(JNIEnv* env, const base::android::JavaParamRef<jlongArray>& java_durations_ns,
    const base::android::JavaParamRef<jintArray>& java_missed_vsyncs, jlong java_reporting_interval_start_time, jlong java_reporting_interval_duration,
    jint java_scenario_enum)
{
    RecordJankMetrics(env, java_durations_ns, java_missed_vsyncs, java_reporting_interval_start_time, java_reporting_interval_duration, java_scenario_enum);
}

void RecordJankMetrics(JNIEnv* env, const base::android::JavaParamRef<jlongArray>& java_durations_ns,
    const base::android::JavaParamRef<jintArray>& java_missed_vsyncs, jlong java_reporting_interval_start_time, jlong java_reporting_interval_duration,
    jint java_scenario_enum)
{
    ...
    const char* frame_duration_histogram_name = GetAndroidFrameTimelineDurationHistogramName(scenario);
    const char* janky_frames_per_scenario_histogram_name = GetAndroidFrameTimelineJankHistogramName(scenario);
    ...
    // overlapping scrolls.
    const bool emit_per_frame_metrics = scenario != JankScenario::WEBVIEW_SCROLLING;

    if (emit_per_frame_metrics) {
        for (const int64_t frame_duration_ns : durations_ns) {
            base::UmaHistogramTimes(frame_duration_histogram_name, base::Nanoseconds(frame_duration_ns));
        }
    }
    ...
    for (int curr_frame_missed_vsyncs : missed_vsyncs) {
        ...
        if (emit_per_frame_metrics) {
            base::UmaHistogramEnumeration(janky_frames_per_scenario_histogram_name, is_janky ? FrameJankStatus::kJanky : FrameJankStatus::kNonJanky);
        }
        if (is_janky) {
            ++janky_frame_count;
        }
    ...
    if (num_presented_frames > 0) {
        EmitMetrics(scenario, janky_frame_count, missed_vsyncs_max, missed_vsyncs_sum, num_presented_frames, java_reporting_interval_start_time,
