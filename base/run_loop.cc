...
constinit thread_local const RunLoop::RunLoopTimeout* run_loop_timeout = nullptr;

// Runs |closure| immediately if this is called on |task_runner|, otherwise
// forwards |closure| to it.
void ProxyToTaskRunner(scoped_refptr<SequencedTaskRunner> task_runner, OnceClosure closure)
{
...
void OnRunLoopTimeout(RunLoop* run_loop, const Location& location, OnceCallback<void(const Location&)> on_timeout)
{
    run_loop->Quit();
...
bool RunLoop::Delegate::ShouldQuitWhenIdle()
{
    ...
    if (top_loop->quit_when_idle_) {
        TRACE_EVENT_WITH_FLOW0("toplevel.flow", "RunLoop_ExitedOnIdle", TRACE_ID_LOCAL(top_loop), TRACE_EVENT_FLAG_FLOW_IN);
        return true;
    }
    return false;
}
...
void RunLoop::RegisterDelegateForCurrentThread(Delegate* new_delegate)
{
    ...
    DCHECK(!delegate) << "Error: Multiple RunLoop::Delegates registered on the same thread.\n\n"
...
void RunLoop::Run(const Location& location)
{
    ...
    if (run_timeout) {
        cancelable_timeout.Reset(BindOnce(&OnRunLoopTimeout, Unretained(this), location, run_timeout->on_timeout));
        origin_task_runner_->PostDelayedTask(FROM_HERE, cancelable_timeout.callback(), run_timeout->timeout);
    }

    DCHECK_EQ(this, delegate_->active_run_loops_.top());
    const bool application_tasks_allowed = delegate_->active_run_loops_.size() == 1U || type_ == Type::kNestableTasksAllowed;
...
void RunLoop::Quit()
{
    ...
    TRACE_EVENT_WITH_FLOW0("toplevel.flow", "RunLoop::Quit", TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_OUT | TRACE_EVENT_FLAG_FLOW_IN);
...
void RunLoop::QuitWhenIdle()
{
    ...
    if (!origin_task_runner_->RunsTasksInCurrentSequence()) {
        origin_task_runner_->PostTask(FROM_HERE, BindOnce(&RunLoop::QuitWhenIdle, Unretained(this)));
        return;
    }

    // OUT|IN as in Quit() to link all Quit*() together should there be multiple.
    TRACE_EVENT_WITH_FLOW0("toplevel.flow", "RunLoop::QuitWhenIdle", TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_OUT | TRACE_EVENT_FLAG_FLOW_IN);
...
RepeatingClosure RunLoop::QuitClosure() &
{
    ...
    return BindRepeating(&ProxyToTaskRunner, origin_task_runner_, BindRepeating(&RunLoop::Quit, weak_factory_.GetWeakPtr()));
}
...
RepeatingClosure RunLoop::QuitWhenIdleClosure() &
{
    ...
    return BindRepeating(&ProxyToTaskRunner, origin_task_runner_, BindRepeating(&RunLoop::QuitWhenIdle, weak_factory_.GetWeakPtr()));
}
...
bool RunLoop::BeforeRun()
{
...
#if DCHECK_IS_ON()
    DCHECK(delegate_->allow_running_for_testing_) << "RunLoop::Run() isn't allowed in the scope of a "
    ...
    if (quit_called_) {
        TRACE_EVENT_WITH_FLOW0("toplevel.flow", "RunLoop_ExitedEarly", TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_IN);
        return false;
    }
...
void RunLoop::AfterRun()
{
    ...
    TRACE_EVENT_WITH_FLOW0("toplevel.flow", "RunLoop_Exited", TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_IN);
