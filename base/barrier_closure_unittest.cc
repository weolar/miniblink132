...
{
    int count = 0;
    base::RepeatingClosure barrier_closure = base::BarrierClosure(0, base::BindLambdaForTesting([&count] { ++count; }));
    EXPECT_EQ(1, count);
}

TEST(BarrierClosureTest, ChecksIfCalledForZeroClosures)
{
    base::RepeatingClosure barrier_closure = base::BarrierClosure(0, base::DoNothing());
...
{
    int count = 0;
    base::RepeatingClosure barrier_closure = base::BarrierClosure(2, base::BindLambdaForTesting([&count] { ++count; }));
    EXPECT_EQ(0, count);
...
public:
    ...
    DestructionIndicator(bool* destructed)
...
TEST(BarrierClosureTest, ReleasesDoneClosureWhenDone)
{
    ...
    base::RepeatingClosure barrier_closure
        = base::BarrierClosure(1, base::BindOnce(&DestructionIndicator::DoNothing, base::Owned(new DestructionIndicator(&done_destructed))));
...
{
    base::RepeatingClosure barrier_closure;
    barrier_closure = base::BarrierClosure(1, base::BindLambdaForTesting([&barrier_closure] { barrier_closure = base::RepeatingClosure(); }));
    barrier_closure.Run();
