...
struct BASE_EXPORT TaskMetadata {
    TaskMetadata();
    explicit TaskMetadata(const Location& posted_from, TimeTicks queue_time = TimeTicks(), TimeTicks delayed_run_time = TimeTicks(),
        TimeDelta leeway = TimeDelta(), subtle::DelayPolicy delay_policy = subtle::DelayPolicy::kFlexibleNoSooner);
...
struct BASE_EXPORT PendingTask : public TaskMetadata {
    PendingTask();
    PendingTask(const Location& posted_from, OnceClosure task, TimeTicks queue_time = TimeTicks(), TimeTicks delayed_run_time = TimeTicks(),
        TimeDelta leeway = TimeDelta(), subtle::DelayPolicy delay_policy = subtle::DelayPolicy::kFlexibleNoSooner);
