...
template <typename T> inline void WriteBigEndianCommon(::benchmark::State& state, span<uint8_t, kSize> buffer)
{
    size_t offset = 0u;
    auto value = T { 0 };
    for (auto _ : state) {
        if constexpr (sizeof(T) == 1) {
            ...
            buffer.subspan(offset).first<sizeof(T)>().copy_from(U16ToBigEndian(value));
        } else if constexpr (sizeof(T) == 4) {
            buffer.subspan(offset).first<sizeof(T)>().copy_from(U32ToBigEndian(value));
        } else {
            static_assert(sizeof(T) == 8);
            buffer.subspan(offset).first<sizeof(T)>().copy_from(U64ToBigEndian(value));
        }
        offset += sizeof(T);
...
template <typename T> void BM_WriteBigEndianAligned(::benchmark::State& state)
{
...
template <typename T> void BM_WriteBigEndianMisaligned(::benchmark::State& state)
{
...
template <typename T> inline void ReadBigEndianCommon(::benchmark::State& state, span<const uint8_t, kSize> buffer)
{
    size_t offset = 0;
    for (auto _ : state) {
        T value;
...
template <typename T> void BM_ReadBigEndianAligned(::benchmark::State& state)
{
...
template <typename T> void BM_ReadBigEndianMisaligned(::benchmark::State& state)
{
