...
void PerformanceEvaluator::WriteMetricsToFile() const
{
    ...
    metrics.SetKey("FramesDecoded", base::Value(base::checked_cast<int>(perf_metrics_.frames_decoded_)));
    metrics.SetKey("TotalDurationMs", base::Value(perf_metrics_.total_duration_.InMillisecondsF()));
    metrics.SetKey("FPS", base::Value(perf_metrics_.frames_per_second_));
    metrics.SetKey("FramesDropped", base::Value(base::checked_cast<int>(perf_metrics_.frames_dropped_)));
    metrics.SetKey("DroppedFramePercentage", base::Value(perf_metrics_.dropped_frame_percentage_));
    metrics.SetKey("FrameDeliveryTimeAverage", base::Value(perf_metrics_.delivery_time_stats_.avg_ms_));
    metrics.SetKey("FrameDeliveryTimePercentile25", base::Value(perf_metrics_.delivery_time_stats_.percentile_25_ms_));
    metrics.SetKey("FrameDeliveryTimePercentile50", base::Value(perf_metrics_.delivery_time_stats_.percentile_50_ms_));
    metrics.SetKey("FrameDeliveryTimePercentile75", base::Value(perf_metrics_.delivery_time_stats_.percentile_75_ms_));
    metrics.SetKey("FrameDecodeTimeAverage", base::Value(perf_metrics_.decode_time_stats_.avg_ms_));
    metrics.SetKey("FrameDecodeTimePercentile25", base::Value(perf_metrics_.decode_time_stats_.percentile_25_ms_));
    metrics.SetKey("FrameDecodeTimePercentile50", base::Value(perf_metrics_.decode_time_stats_.percentile_50_ms_));
    metrics.SetKey("FrameDecodeTimePercentile75", base::Value(perf_metrics_.decode_time_stats_.percentile_75_ms_));
...
public:
    ...
    // is disabled.
    std::unique_ptr<DecoderListener> CreateDecoderListener(const Video* video, uint32_t render_frame_rate = 0, uint32_t vsync_rate = 0)
    {
        LOG_ASSERT(video);
        ...
        video_player->SetEventWaitTimeout(std::max(kDefaultEventWaitTimeout, g_env->Video()->GetDuration()));
        return video_player;
    }
