...
void PerformanceMetrics::WriteToFile() const
{
    ...
    metrics.SetKey("BitstreamsEncoded", base::Value(base::checked_cast<int>(bitstreams_encoded_)));
    metrics.SetKey("TotalDurationMs", base::Value(total_duration_.InMillisecondsF()));
    metrics.SetKey("FPS", base::Value(bitstreams_per_second_));
    metrics.SetKey("BitstreamDeliveryTimeAverage", base::Value(bitstream_delivery_stats_.avg_ms_));
    metrics.SetKey("BitstreamDeliveryTimePercentile25", base::Value(bitstream_delivery_stats_.percentile_25_ms_));
    metrics.SetKey("BitstreamDeliveryTimePercentile50", base::Value(bitstream_delivery_stats_.percentile_50_ms_));
    metrics.SetKey("BitstreamDeliveryTimePercentile75", base::Value(bitstream_delivery_stats_.percentile_75_ms_));
    metrics.SetKey("BitstreamEncodeTimeAverage", base::Value(bitstream_encode_stats_.avg_ms_));
    metrics.SetKey("BitstreamEncodeTimePercentile25", base::Value(bitstream_encode_stats_.percentile_25_ms_));
    metrics.SetKey("BitstreamEncodeTimePercentile50", base::Value(bitstream_encode_stats_.percentile_50_ms_));
    metrics.SetKey("BitstreamEncodeTimePercentile75", base::Value(bitstream_encode_stats_.percentile_75_ms_));
...
struct BitstreamQualityMetrics {
    BitstreamQualityMetrics(const PSNRVideoFrameValidator* const psnr_validator, const SSIMVideoFrameValidator* const ssim_validator,
        const absl::optional<size_t>& spatial_idx, const absl::optional<size_t>& temporal_idx);
...
private:
    ...
    void WriteToConsole(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats,
        const BitstreamQualityMetrics::QualityStats& ssim_stats, uint32_t target_bitrate, uint32_t actual_bitrate) const;
...
void BitstreamQualityMetrics::Output(uint32_t target_bitrate, uint32_t actual_bitrate)
{
    ...
    WriteToConsole(svc_text, psnr_stats, ssim_stats, target_bitrate, actual_bitrate);
    ...
    const BitstreamQualityMetrics::QualityStats& ssim_stats, uint32_t target_bitrate, uint32_t actual_bitrate) const
{
    ...
    metrics.SetKey("BitrateDeviation", base::Value((actual_bitrate * 100.0 / target_bitrate) - 100.0));
    ...
    base::FilePath metrics_file_path = output_folder_path.Append(
        g_env->GetTestOutputFilePath().AddExtension(svc_text.empty() ? "" : "." + svc_text).AddExtension(FILE_PATH_LITERAL(".json")));
    // Make sure that the directory into which json is saved is created.
    LOG_ASSERT(base::CreateDirectory(metrics_file_path.DirName()));
    base::File metrics_output_file(base::FilePath(metrics_file_path), base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    int bytes_written = metrics_output_file.WriteAtCurrentPos(metrics_str.data(), metrics_str.length());
...
public:
    ...
    std::unique_ptr<VideoEncoder> CreateVideoEncoder(absl::optional<uint32_t> encode_rate, bool measure_quality)
    {
        ...
        if (measure_quality) {
            bitstream_processors = CreateBitstreamProcessorsForQualityPerformance(video, profile, spatial_layers);
        } else {
        ...
        VideoEncoderClientConfig config(video, profile, spatial_layers, bitrate, g_env->Reverse());
...
private:
    std::unique_ptr<BitstreamValidator> CreateBitstreamValidator(const VideoCodecProfile profile, const gfx::Rect& visible_rect,
        const absl::optional<size_t>& spatial_layer_index_to_decode, const absl::optional<size_t>& temporal_layer_index_to_decode,
        const std::vector<gfx::Size>& spatial_layer_resolutions)
    {
        ...
        quality_metrics_.push_back(
            BitstreamQualityMetrics(psnr_validator.get(), ssim_validator.get(), spatial_layer_index_to_decode, temporal_layer_index_to_decode));
        ...
        return BitstreamValidator::Create(decoder_config, kNumFramesToEncodeForPerformance - 1, std::move(video_frame_processors),
    ...
    std::vector<std::unique_ptr<BitstreamProcessor>> CreateBitstreamProcessorsForQualityPerformance(
        Video* video, VideoCodecProfile profile, const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers)
    {
        std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors;
        ...
        if (spatial_layers.empty()) {
            ...
            bitstream_processors.push_back(CreateBitstreamValidator(profile, gfx::Rect(video->Resolution()),
            ...
            for (size_t sid = 0; sid < spatial_layers.size(); ++sid) {
                for (size_t tid = 0; tid < spatial_layers[sid].num_of_temporal_layers; ++tid) {
                    bitstream_processors.push_back(
                        CreateBitstreamValidator(profile, gfx::Rect(spatial_layer_resolutions[sid]), sid, tid, spatial_layer_resolutions));
                    LOG_ASSERT(!!bitstream_processors.back());
                }
            }
        }
...
int main(int argc, char** argv)
{
    ...
    media::test::VideoEncoderTestEnvironment* test_environment
        = media::test::VideoEncoderTestEnvironment::Create(video_path, video_metadata_path, false, base::FilePath(output_folder), codec, num_temporal_layers,
            num_spatial_layers, false /* output_bitstream */, encode_bitrate, bitrate_mode, reverse, media::test::FrameOutputConfig(),
