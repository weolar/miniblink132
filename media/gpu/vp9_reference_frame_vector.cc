...
void Vp9ReferenceFrameVector::Refresh(scoped_refptr<VP9Picture> pic)
{
    ...
    DCHECK(pic);
    const std::bitset<kVp9NumRefFrames> refresh_frame_flags(pic->frame_hdr->refresh_frame_flags);
...
scoped_refptr<VP9Picture> Vp9ReferenceFrameVector::GetFrame(size_t index) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
