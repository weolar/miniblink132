...
public:
    ...
    static std::unique_ptr<H264RateCtrlRTC> Create(const H264RateControlConfigRTC& config);
    ...
    void PostEncodeUpdate(uint64_t encoded_frame_size, const H264FrameParamsRTC& frame_params);

    // The `buffer_fullness` array is filled with the HRD buffer fullness values
    // for each layer at the given `timestamp`.
    void GetBufferFullness(base::span<int> buffer_fullness, base::TimeDelta timestamp);
