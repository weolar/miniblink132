...
public:
    ...
    void SetUp() override
    {
        ...
        rate_control_config_rtc_.content_type = VideoEncodeAccelerator::Config::ContentType::kCamera;
        rate_control_config_rtc_.frame_size.SetSize(kCommonFrameWidth, kCommonFrameHeight);
        ...
        rate_control_config_rtc_.layer_settings[0].avg_bitrate = kCommonAvgBitrate * 2 / 3;
        rate_control_config_rtc_.layer_settings[0].peak_bitrate = kCommonPeakBitrate * 2 / 3;
        rate_control_config_rtc_.layer_settings[0].hrd_buffer_size = kCommonHRDBufferSize * 2 / 3;
        ...
        rate_control_config_rtc_.layer_settings[1].peak_bitrate = kCommonPeakBitrate;
        rate_control_config_rtc_.layer_settings[1].hrd_buffer_size = kCommonHRDBufferSize;
        ...
        EXPECT_TRUE((rate_control_config_rtc <=> rate_control_config_rtc_ == std::partial_ordering::greater));

        EXPECT_EQ(kExpectedLoopFilterLevel, rate_ctrl_rtc_->GetLoopfilterLevel());
    }

protected:
    int RunTestSequence(int fps, int start_frame_index, int& last_intra_frame_qp, int& last_inter_frame_qp, int& drop_frames)
    {
        constexpr size_t kFirstIntraFrameIndex = 0;
        std::vector<size_t> frames { 12500, 3000, 4000, 3000, 4000, 3000, 8000, 6000, 8000, 6000, 10000, 8000, 10000, 8000, 8000, 0 };
        ...
        for (size_t encoded_size : frames) {
            ...
            H264RateCtrlRTC::FrameDropDecision frame_drop_decision = rate_ctrl_rtc_->ComputeQP(frame_params);
            if (keyframe) {
...
TEST_F(H264RateCtrlRTCTest, RunBasicRateCtrlRTCTest)
{
    ...
    int drop_frames = 0;
    int last_frame_index = RunTestSequence(kCommonFps, start_frame_index, last_intra_frame_qp, last_inter_frame_qp, drop_frames);
    ...
    rate_control_config_rtc_.layer_settings[1].avg_bitrate = kCommonAvgBitrate * 4 / 3;
    rate_control_config_rtc_.layer_settings[1].peak_bitrate = kCommonPeakBitrate * 4 / 3;

    rate_ctrl_rtc_->UpdateRateControl(rate_control_config_rtc_);

    start_frame_index = last_frame_index;
    last_frame_index = RunTestSequence(kCommonFps, start_frame_index, last_intra_frame_qp, last_inter_frame_qp, drop_frames);
