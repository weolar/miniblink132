...
public:
    ...
    scoped_refptr<const FrameResource> AccessFrame(const base::UnguessableToken& token) const;
...
private:
    ...
    // elements at a time.
    base::flat_map<base::UnguessableToken, scoped_refptr<const FrameResource>> map_ GUARDED_BY(lock_);
};
