...
// anywhere else.
class MEDIA_GPU_EXPORT VdVideoDecodeAccelerator : public VideoDecodeAccelerator, public VdaVideoFramePool::VdaDelegate {
public:
    // Callback for creating VideoDecoder instance.
    using CreateVideoDecoderCb = base::RepeatingCallback<decltype(VideoDecoderPipeline::CreateForARC)>;
    ...
    void RequestFrames(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect, size_t max_num_frames,
        NotifyLayoutChangedCb notify_layout_changed_cb, ImportFrameCb import_frame_cb) override;
    VideoFrame::StorageType GetFrameStorageType() const override;

private:
    VdVideoDecodeAccelerator(CreateVideoDecoderCb create_vd_cb, scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    static void OnFrameReleasedThunk(std::optional<base::WeakPtr<VdVideoDecodeAccelerator>> weak_this, scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    NotifyLayoutChangedCb notify_layout_changed_cb_ GUARDED_BY_CONTEXT(client_sequence_checker_);
    ...
    std::map<base::UnguessableToken, int32_t /* picture_buffer_id */> frame_token_to_picture_id_;
    ...
    std::map<int32_t /* picture_buffer_id */, std::pair<scoped_refptr<VideoFrame>, size_t /* num_sent */>> picture_at_client_;
