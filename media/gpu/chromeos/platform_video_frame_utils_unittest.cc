...
scoped_refptr<VideoFrame> CreateMockDmaBufVideoFrame(
    VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    ...
    return VideoFrame::WrapExternalDmabufs(*layout, visible_rect, natural_size, std::move(dmabuf_fds), base::TimeDelta());
}
} // namespace
...
TEST(PlatformVideoFrameUtilsTest, CreateNativePixmapDmaBuf)
{
    ...
    const std::optional<gfx::BufferFormat> gfx_format = VideoPixelFormatToGfxBufferFormat(kPixelFormat);
    ASSERT_TRUE(gfx_format) << "Invalid pixel format: " << kPixelFormat;

    scoped_refptr<VideoFrame> video_frame = CreateMockDmaBufVideoFrame(kPixelFormat, kCodedSize, gfx::Rect(kCodedSize), kCodedSize);
    ASSERT_TRUE(video_frame);

    // Create a native pixmap and verify its metadata.
    scoped_refptr<gfx::NativePixmapDmaBuf> native_pixmap = CreateNativePixmapDmaBuf(video_frame.get());
    ASSERT_TRUE(native_pixmap);
    EXPECT_EQ(native_pixmap->GetBufferFormat(), *gfx_format);
    EXPECT_EQ(native_pixmap->GetBufferFormatModifier(), video_frame->layout().modifier());
    ...
    for (size_t i = 0; i < num_planes; i++) {
        ...
        EXPECT_EQ(native_pixmap->GetDmaBufPitch(i), base::checked_cast<uint32_t>(plane.stride));
...
TEST(PlatformVideoFrameUtilsTest, CreateVideoFrame)
{
    ...
    constexpr gfx::BufferUsage kBufferUsage = gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE;
    ...
    for (const auto& storage_type : storage_types) {
        ...
        case VideoFrame::STORAGE_DMABUFS:
            frame = CreatePlatformVideoFrame(kPixelFormat, kCodedSize, kVisibleRect, kNaturalSize, kTimeStamp, kBufferUsage);
            break;
        case VideoFrame::STORAGE_GPU_MEMORY_BUFFER:
            frame = CreateGpuMemoryBufferVideoFrame(kPixelFormat, kCodedSize, kVisibleRect, kNaturalSize, kTimeStamp, kBufferUsage);
            break;
        default:
            NOTREACHED();
        };
