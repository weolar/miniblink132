...
std::vector<base::ScopedFD> CreateMockDMABufs(size_t num_planes)
{
    ...
    for (size_t i = 0; i < num_planes; i++) {
        ...
        base::File file(base::FilePath("/dev/null"), base::File::FLAG_OPEN | base::File::FLAG_READ);
        if (!file.IsValid()) {
...
scoped_refptr<const gfx::NativePixmapDmaBuf> CreateMockNativePixmapDmaBuf(VideoPixelFormat pixel_format, const gfx::Size& coded_size)
{
    ...
    const std::optional<VideoFrameLayout> layout = VideoFrameLayout::Create(pixel_format, coded_size);
    if (!layout.has_value()) {
    ...
    if (!buffer_format) {
        LOG(ERROR) << "Unable to convert pixel format " << pixel_format << " to BufferFormat";
        return nullptr;
    }
    ...
    for (size_t i = 0; i < num_planes; ++i) {
        ...
        handle.planes.emplace_back(plane.stride, plane.offset, plane.size, base::ScopedFD(file.TakePlatformFile()));
    }
    handle.modifier = layout->modifier();

    return base::MakeRefCounted<gfx::NativePixmapDmaBuf>(coded_size, *buffer_format, std::move(handle));
}

} // namespace

class CreateTest : public ::testing::Test, public ::testing::WithParamInterface<std::tuple<gfx::Size, gfx::Rect, gfx::Size, bool>> {
public:
    void SetUp() override
    {
    }
...
TEST_P(CreateTest, CreateFromDMABufs)
{
    ...
    const std::optional<VideoFrameLayout> layout = VideoFrameLayout::Create(kPixelFormat, coded_size);
    ...
    auto frame = NativePixmapFrameResource::Create(*layout, visible_rect, natural_size, std::move(dmabuf_fds), kTimestamp);
    if (should_succeed) {
        EXPECT_TRUE(!!frame);
    } else {
...
TEST_P(CreateTest, CreateFromNativePixmapDmabuf)
{
    ...
    scoped_refptr<const gfx::NativePixmapDmaBuf> pixmap = CreateMockNativePixmapDmaBuf(kPixelFormat, coded_size);
    ASSERT_TRUE(!!pixmap);
    auto frame = NativePixmapFrameResource::Create(visible_rect, natural_size, kTimestamp, kBufferUsage, std::move(pixmap));
    if (should_succeed) {
        EXPECT_TRUE(!!frame);
    } else {
        EXPECT_EQ(frame, nullptr);
...
INSTANTIATE_TEST_SUITE_P(
    NativePixmapFrameResourceValidSize, CreateTest, ::testing::Values(std::make_tuple(gfx::Size(320, 240), gfx::Rect(320, 240), gfx::Size(320, 240), true)));

INSTANTIATE_TEST_SUITE_P(NativePixmapFrameResourceInvalidSize, CreateTest,
    ::testing::Values(std::make_tuple(gfx::Size(), gfx::Rect(), gfx::Size(), false),
        std::make_tuple(gfx::Size(320, 240), gfx::Rect(), gfx::Size(320, 240), false),
        std::make_tuple(gfx::Size(320, 240), gfx::Rect(320, 240), gfx::Size(), false)));
