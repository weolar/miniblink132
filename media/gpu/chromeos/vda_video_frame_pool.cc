...
VdaVideoFramePool::VdaVideoFramePool(base::WeakPtr<VdaDelegate> vda, scoped_refptr<base::SequencedTaskRunner> vda_task_runner)
    : vda_(std::move(vda))
...
CroStatus::Or<GpuBufferLayout> VdaVideoFramePool::Initialize(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, size_t max_num_frames, bool use_protected, bool use_linear_buffers)
{
    ...
    if (layout_ && max_num_frames_ == max_num_frames && fourcc_ && *fourcc_ == fourcc && coded_size_ == coded_size) {
    ...
    CroStatus::Or<GpuBufferLayout> status_or_layout = CroStatus::Codes::kResetRequired;
    base::WaitableEvent done;
    vda_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VdaDelegate::RequestFrames, vda_, fourcc, coded_size, visible_rect, max_num_frames,
            base::BindOnce(&VdaVideoFramePool::OnRequestFramesDone, &done, &status_or_layout),
            base::BindRepeating(&VdaVideoFramePool::ImportFrameThunk, parent_task_runner_, weak_this_)));
    done.Wait();
    ...
    if (layout.fourcc() != fourcc || layout.size().height() < coded_size.height() || layout.size().width() < coded_size.width()) {
...
// static
void VdaVideoFramePool::OnRequestFramesDone(base::WaitableEvent* done, CroStatus::Or<GpuBufferLayout>* layout, CroStatus::Or<GpuBufferLayout> layout_value)
{
    DVLOGF(3);
...
void VdaVideoFramePool::ImportFrameThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, std::optional<base::WeakPtr<VdaVideoFramePool>> weak_this, scoped_refptr<FrameResource> frame)
{
    DVLOGF(3);
    DCHECK(weak_this);

    task_runner->PostTask(FROM_HERE, base::BindOnce(&VdaVideoFramePool::ImportFrame, *weak_this, std::move(frame)));
}
...
void VdaVideoFramePool::ImportFrame(scoped_refptr<FrameResource> frame)
{
    ...
    if (!layout_ || layout_->fourcc().ToVideoPixelFormat() != frame->format() || layout_->size() != frame->coded_size()
...
scoped_refptr<FrameResource> VdaVideoFramePool::GetFrame()
{
    ...
    wrapped_frame->AddDestructionObserver(base::BindOnce(&VdaVideoFramePool::ImportFrameThunk, parent_task_runner_, weak_this_, std::move(origin_frame)));
