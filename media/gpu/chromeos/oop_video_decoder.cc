...
scoped_refptr<FrameResource> MojoVideoFrameToFrameResource(stable::mojom::VideoFramePtr mojo_frame)
{
    if (mojo_frame->metadata.protected_video && mojo_frame->metadata.needs_detiling && mojo_frame->format == PIXEL_FORMAT_P010LE) {
        ...
        if (mojo_frame->gpu_memory_buffer_handle.type != gfx::NATIVE_PIXMAP) {
            VLOGF(1) << "Unexpected GpuMemoryBufferType: " << mojo_frame->gpu_memory_buffer_handle.type;
            return nullptr;
        }
        if (!media::VideoFrame::IsValidCodedSize(mojo_frame->coded_size)) {
            VLOG(1) << "Coded size is beyond allowed dimensions: " << mojo_frame->coded_size.ToString();
            return nullptr;
        }
        constexpr size_t kNumP010Planes = 2;
        if (kNumP010Planes != mojo_frame->gpu_memory_buffer_handle.native_pixmap_handle.planes.size()) {
            VLOGF(1) << "Invalid number of dmabuf planes passed: " << mojo_frame->gpu_memory_buffer_handle.native_pixmap_handle.planes.size()
                     << ", expected: 2";
            return nullptr;
        }
    } else {
        if (!VerifyGpuMemoryBufferHandle(mojo_frame->format, mojo_frame->coded_size, mojo_frame->gpu_memory_buffer_handle)) {
            VLOGF(2) << "Received an invalid GpuMemoryBufferHandle";
            return nullptr;
        }
    }

    std::optional<gfx::BufferFormat> buffer_format = VideoPixelFormatToGfxBufferFormat(mojo_frame->format);
    if (!buffer_format) {
    ...
    scoped_refptr<media::NativePixmapFrameResource> native_pixmap_frame
        = NativePixmapFrameResource::Create(mojo_frame->visible_rect, mojo_frame->natural_size, mojo_frame->timestamp, gfx::BufferUsage::SCANOUT_VDA_WRITE,
            base::MakeRefCounted<gfx::NativePixmapDmaBuf>(
                mojo_frame->coded_size, *buffer_format, std::move(mojo_frame->gpu_memory_buffer_handle.native_pixmap_handle)));
...
public:
    ...
    void NotifySupportKnown(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
        base::OnceCallback<void(mojo::PendingRemote<stable::mojom::StableVideoDecoder>)> cb)
    {
        ...
        if ((configs_ && interface_version_) || disconnected_) {
            ...
            waiting_callbacks_.emplace(std::move(oop_video_decoder), std::move(cb), base::SequencedTaskRunner::GetCurrentDefault());
            return;
        }
        ...
        oop_video_decoder_.set_disconnect_handler(base::BindOnce(&OOPVideoDecoderSupportedConfigsManager::OnDecoderDisconnected, base::Unretained(this)));
        oop_video_decoder_.QueryVersion(base::BindOnce(&OOPVideoDecoderSupportedConfigsManager::OnGetInterfaceVersion, base::Unretained(this)));
        oop_video_decoder_->GetSupportedConfigs(base::BindOnce(&OOPVideoDecoderSupportedConfigsManager::OnGetSupportedConfigs, base::Unretained(this)));
        ...
        waiting_callbacks_.emplace(mojo::PendingRemote<stable::mojom::StableVideoDecoder>(), std::move(cb), base::SequencedTaskRunner::GetCurrentDefault());
    }
...
private:
    ...
    void OnGetSupportedConfigs(const SupportedVideoDecoderConfigs& configs, VideoDecoderType decoder_type)
    {
        base::AutoLock lock(lock_);
        DCHECK(!configs_);
        ...
        if (decoder_type == VideoDecoderType::kVda || decoder_type == VideoDecoderType::kVaapi || decoder_type == VideoDecoderType::kV4L2) {
            if (configs.empty() && config_retry_count_ < kMaxConfigRetries) {
                ...
                base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
                    FROM_HERE, base::BindOnce(&OOPVideoDecoderSupportedConfigsManager::GetSupportedConfigs, base::Unretained(this)), base::Milliseconds(250));
    ...
    {
        if (!disconnected_ && (!configs_.has_value() || !interface_version_.has_value())) {
        ...
        while (!waiting_callbacks_.empty()) {
            WaitingCallbackContext waiting_callback = std::move(waiting_callbacks_.front());
            waiting_callbacks_.pop();

            mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder
                = waiting_callback.oop_video_decoder ? std::move(waiting_callback.oop_video_decoder) : oop_video_decoder_.Unbind();
            ...
            if (waiting_callback.cb_task_runner->RunsTasksInCurrentSequence()) {
                ...
                waiting_callback.cb_task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(waiting_callback.cb), std::move(oop_video_decoder)));
            }
        }
    }
    ...
    struct WaitingCallbackContext {
        WaitingCallbackContext(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
            base::OnceCallback<void(mojo::PendingRemote<stable::mojom::StableVideoDecoder>)> cb, scoped_refptr<base::SequencedTaskRunner> cb_task_runner)
...
// static
std::unique_ptr<VideoDecoderMixin> OOPVideoDecoder::Create(mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder,
    std::unique_ptr<media::MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
{
    ...
    return base::WrapUnique<VideoDecoderMixin>(
        new OOPVideoDecoder(std::move(media_log), std::move(decoder_task_runner), std::move(client), std::move(pending_remote_decoder)));
}

// static
void OOPVideoDecoder::NotifySupportKnown(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
    base::OnceCallback<void(mojo::PendingRemote<stable::mojom::StableVideoDecoder>)> cb)
{
    OOPVideoDecoderSupportedConfigsManager::Instance().NotifySupportKnown(std::move(oop_video_decoder), std::move(cb));
}

// static
std::optional<SupportedVideoDecoderConfigs> OOPVideoDecoder::GetSupportedConfigs()
{
...
{
    OOPVideoDecoderSupportedConfigsManager::Instance().ResetForTesting(); // IN-TEST
}

OOPVideoDecoder::OOPVideoDecoder(std::unique_ptr<media::MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner,
    base::WeakPtr<VideoDecoderMixin::Client> client, mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder)
    ...
    , weak_this_factory_(this)
{
    ...
    remote_decoder_.set_disconnect_handler(base::BindOnce(&OOPVideoDecoder::Stop, base::Unretained(this)));
    ...
    mojo_decoder_buffer_writer_ = MojoDecoderBufferWriter::Create(GetDefaultDecoderBufferConverterCapacity(DemuxerStream::VIDEO), &remote_consumer_handle);
    ...
    mojo::PendingReceiver<stable::mojom::VideoFrameHandleReleaser> stable_video_frame_handle_releaser_receiver
        = stable_video_frame_handle_releaser_remote_.BindNewPipeAndPassReceiver();

    // base::Unretained() is safe because `this` owns the `mojo::Remote`.
    stable_video_frame_handle_releaser_remote_.set_disconnect_handler(base::BindOnce(&OOPVideoDecoder::Stop, base::Unretained(this)));

    DCHECK(!stable_media_log_receiver_.is_bound());

    CHECK(!has_error_);
    // TODO(b/171813538): plumb the remaining parameters.
    remote_decoder_->Construct(client_receiver_.BindNewEndpointAndPassRemote(), stable_media_log_receiver_.BindNewPipeAndPassRemote(),
        std::move(stable_video_frame_handle_releaser_receiver), std::move(remote_consumer_handle), gfx::ColorSpace());
}
...
OOPVideoDecoder::~OOPVideoDecoder()
{
    ...
    for (auto& pending_decode : pending_decodes_) {
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(pending_decode.second), DecoderStatus::Codes::kAborted));
    }
}

void OOPVideoDecoder::Initialize(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const PipelineOutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    mojo::PendingRemote<stable::mojom::StableCdmContext> pending_remote_stable_cdm_context;
    if (config.is_encrypted()) {
        ...
        DCHECK(!stable_cdm_context_ || cdm_context == stable_cdm_context_->cdm_context());
        if (!stable_cdm_context_) {
            if (!cdm_context || !cdm_context->GetChromeOsCdmContext()) {
                std::move(init_cb).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
                return;
            }
            stable_cdm_context_ = std::make_unique<chromeos::StableCdmContextImpl>(cdm_context);
            stable_cdm_context_receiver_ = std::make_unique<mojo::Receiver<stable::mojom::StableCdmContext>>(
                stable_cdm_context_.get(), pending_remote_stable_cdm_context.InitWithNewPipeAndPassReceiver());

            // base::Unretained() is safe because |this| owns the mojo::Receiver.
            stable_cdm_context_receiver_->set_disconnect_handler(base::BindOnce(&OOPVideoDecoder::Stop, base::Unretained(this)));
        }
#else
        std::move(init_cb).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
        return;
    ...
    remote_decoder_->Initialize(
        config, low_delay, std::move(pending_remote_stable_cdm_context), base::BindOnce(&OOPVideoDecoder::OnInitializeDone, weak_this_factory_.GetWeakPtr()));
}

void OOPVideoDecoder::OnInitializeDone(
    const DecoderStatus& status, bool needs_bitstream_conversion, int32_t max_decode_requests, VideoDecoderType decoder_type, bool needs_transcryption)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const VideoDecoderType expected_decoder_type = OOPVideoDecoderSupportedConfigsManager::Instance().GetDecoderType();

    if (!status.is_ok() || decoder_type != expected_decoder_type
        || (remote_decoder_type_ != VideoDecoderType::kUnknown && remote_decoder_type_ != decoder_type)) {
        Stop();
        return;
    }
    ...
    if (OOPVideoDecoderSupportedConfigsManager::Instance().GetInterfaceVersion() >= 1u) {
        // Starting on version 1, the remote decoder tells us if we need to do
        // transcryption before sending the encoded data.
        needs_transcryption_ = initialized_for_protected_content_ && needs_transcryption;
    } else {
        ...
        // version 1).
#if BUILDFLAG(USE_VAAPI)
        needs_transcryption_ = initialized_for_protected_content_ && (VaapiWrapper::GetImplementationType() == VAImplementation::kMesaGallium);
#endif // BUILDFLAG(USE_VAAPI)
    }
...
void OOPVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (has_error_ || remote_decoder_type_ == VideoDecoderType::kUnknown) {
        DeferDecodeCallback(std::move(decode_cb), DecoderStatus::Codes::kNotInitialized);
        return;
    }
    ...
    if (!buffer->end_of_stream()) {
        const base::TimeDelta next_fake_timestamp = current_fake_timestamp_ + base::Microseconds(1u);
        ...
        DCHECK(fake_timestamp_to_real_timestamp_cache_.Peek(current_fake_timestamp_) == fake_timestamp_to_real_timestamp_cache_.end());
        fake_timestamp_to_real_timestamp_cache_.Put(current_fake_timestamp_, buffer->timestamp());
        buffer_timestamp_restorer.ReplaceClosure(base::BindOnce(
            [](scoped_refptr<DecoderBuffer> decoder_buffer, base::TimeDelta original_timestamp) { decoder_buffer->set_timestamp(original_timestamp); }, buffer,
            buffer->timestamp()));
    ...
    remote_decoder_->Decode(std::move(buffer), base::BindOnce(&OOPVideoDecoder::OnDecodeDone, weak_this_factory_.GetWeakPtr(), decode_id, is_flushing_));
}

void OOPVideoDecoder::OnDecodeDone(uint64_t decode_id, bool is_flush_cb, const DecoderStatus& status)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void OOPVideoDecoder::DeferDecodeCallback(DecodeCB decode_cb, const DecoderStatus& status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    decoder_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OOPVideoDecoder::CallDeferredDecodeCallback, weak_this_factory_.GetWeakPtr(), std::move(decode_cb), status));
}

void OOPVideoDecoder::CallDeferredDecodeCallback(DecodeCB decode_cb, const DecoderStatus& status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    std::move(decode_cb).Run(status);
    num_deferred_decode_cbs_--;
...
void OOPVideoDecoder::Reset(base::OnceClosure reset_cb)
{
    ...
    if (has_error_ || remote_decoder_type_ == VideoDecoderType::kUnknown) {
        ...
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&OOPVideoDecoder::CallResetCallback, weak_this_factory_.GetWeakPtr()));
        return;
    }

    remote_decoder_->Reset(base::BindOnce(&OOPVideoDecoder::OnResetDone, weak_this_factory_.GetWeakPtr()));
}
...
void OOPVideoDecoder::Stop()
{
    ...
    for (auto& pending_decode : pending_decodes_) {
        ...
        DeferDecodeCallback(std::move(pending_decode.second), DecoderStatus::Codes::kFailed);
    }
...
void OOPVideoDecoder::ReleaseVideoFrame(const base::UnguessableToken& release_token)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void OOPVideoDecoder::OnVideoFrameDecoded(stable::mojom::VideoFramePtr frame, bool can_read_without_stalling, const base::UnguessableToken& release_token)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!initialized_for_protected_content_ && (frame->metadata.protected_video || frame->metadata.hw_protected)) {
    ...
    const gfx::GpuMemoryBufferId received_gmb_id = frame->gpu_memory_buffer_handle.id;
    if (!received_id_to_decoded_frame_map_.empty()) {
        ...
        const VideoPixelFormat current_format = received_id_to_decoded_frame_map_.cbegin()->second->format();
        const gfx::Size& current_coded_size = received_id_to_decoded_frame_map_.cbegin()->second->coded_size();
        const gfx::Size& current_visible_rect_size_from_origin = GetRectSizeFromOrigin(received_id_to_decoded_frame_map_.cbegin()->second->visible_rect());
        const bool currently_uses_protected = received_id_to_decoded_frame_map_.cbegin()->second->metadata().hw_protected;

        if (format != current_format || coded_size != current_coded_size || GetRectSizeFromOrigin(visible_rect) != current_visible_rect_size_from_origin
    ...
    auto decoded_frame_it = received_id_to_decoded_frame_map_.find(received_gmb_id);
    if (decoded_frame_it != received_id_to_decoded_frame_map_.end()) {
        ...
        CHECK_EQ(GetRectSizeFromOrigin(frame_to_wrap->visible_rect()), GetRectSizeFromOrigin(visible_rect));
        CHECK_EQ(frame_to_wrap->metadata().hw_protected, metadata.hw_protected);
    } else {
        scoped_refptr<FrameResource> native_pixmap_frame = MojoVideoFrameToFrameResource(std::move(frame));
        if (!native_pixmap_frame) {
            Stop();
            return;
        }
        received_id_to_decoded_frame_map_[received_gmb_id] = native_pixmap_frame;
        generated_id_to_decoded_frame_map_[native_pixmap_frame->GetSharedMemoryId()] = native_pixmap_frame.get();
    ...
    // |natural_size|.
    scoped_refptr<FrameResource> wrapped_frame = frame_to_wrap->CreateWrappingFrame(visible_rect, natural_size);
    if (!wrapped_frame) {
    ...
    wrapped_frame->AddDestructionObserver(
        base::BindPostTaskToCurrentDefault(base::BindOnce(&OOPVideoDecoder::ReleaseVideoFrame, weak_this_factory_.GetWeakPtr(), release_token)));
...
FrameResource* OOPVideoDecoder::GetOriginalFrame(gfx::GenericSharedMemoryId frame_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    CHECK(frame_id.is_valid());
    auto it = generated_id_to_decoded_frame_map_.find(frame_id);
    return (it == generated_id_to_decoded_frame_map_.end()) ? nullptr : it->second;
}
