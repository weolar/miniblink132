...
FrameResource::ID GetNextID()
{
    ...
    return FrameResource::ID::FromUnsafeValue(counter.fetch_add(1u, std::memory_order_relaxed));
}
...
const NativePixmapFrameResource* FrameResource::AsNativePixmapFrameResource() const
{
    return nullptr;
}
