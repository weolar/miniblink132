...
std::vector<ColorPlaneLayout> ExtractColorPlaneLayout(const gfx::GpuMemoryBufferHandle& gmb_handle)
{
...
template <class T> std::string VectorToString(const std::vector<T>& vec)
{
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
scoped_refptr<DecoderBuffer> DecryptBitstreamBuffer(BitstreamBuffer bitstream_buffer)
{
    ...
    const size_t available_size = mem_region.GetSize() - base::checked_cast<size_t>(bitstream_buffer.offset());
    ...
    if (available_size <= cdm_oemcrypto::kSecureBufferHeaderSize || memcmp(data, cdm_oemcrypto::kSecureBufferTag, cdm_oemcrypto::kSecureBufferTagLen)) {
    ...
    memcpy(&proto_size, data + cdm_oemcrypto::kSecureBufferTagLen, sizeof(uint32_t));
    if (proto_size > cdm_oemcrypto::kSecureBufferHeaderSize - cdm_oemcrypto::kSecureBufferProtoOffset) {
        DVLOG(2) << "Proto size goes beyond header size";
        return nullptr;
    }
    // Read the serialized proto.
    std::string serialized_proto(data + cdm_oemcrypto::kSecureBufferProtoOffset, data + cdm_oemcrypto::kSecureBufferProtoOffset + proto_size);
    ...
    if (buffer_proto.has_pattern()) {
        pattern.emplace(buffer_proto.pattern().cypher_bytes(), buffer_proto.pattern().clear_bytes());
    }
    // Now create the DecryptConfig and set it in the decoder buffer.
    scoped_refptr<DecoderBuffer> buffer = bitstream_buffer.ToDecoderBuffer(cdm_oemcrypto::kSecureBufferHeaderSize, buffer_size);
    if (!buffer) {
        DVLOG(2) << "Secure buffer data goes beyond shared memory size";
        return nullptr;
    }
    if (buffer_proto.encryption_scheme() != chromeos::cdm::ArcSecureBufferForChrome::NONE) {
        buffer->set_decrypt_config(std::make_unique<DecryptConfig>(
            buffer_proto.encryption_scheme() == chromeos::cdm::ArcSecureBufferForChrome::CBCS ? EncryptionScheme::kCbcs : EncryptionScheme::kCenc,
            buffer_proto.key_id(), buffer_proto.iv(), std::move(subsamples), std::move(pattern)));
    }
    return buffer;
}
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
...
std::unique_ptr<VideoDecodeAccelerator> VdVideoDecodeAccelerator::Create(
    CreateVideoDecoderCb create_vd_cb, Client* client, const Config& config, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    std::unique_ptr<VdVideoDecodeAccelerator, std::default_delete<VideoDecodeAccelerator>> vda(
        new VdVideoDecodeAccelerator(std::move(create_vd_cb), std::move(task_runner)));
    if (!vda->Initialize(config, client, /*low_delay=*/true))
        return nullptr;
    return vda;
}

VdVideoDecodeAccelerator::VdVideoDecodeAccelerator(CreateVideoDecoderCb create_vd_cb, scoped_refptr<base::SequencedTaskRunner> client_task_runner)
...
void VdVideoDecodeAccelerator::Destroy()
{
    ...
    if (notify_layout_changed_cb_)
        std::move(notify_layout_changed_cb_).Run(CroStatus::Codes::kFailedToGetFrameLayout);
    client_ = nullptr;
    vd_.reset();
...
bool VdVideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
...
bool VdVideoDecodeAccelerator::Initialize(const Config& config, Client* client, bool low_delay)
{
    ...
    if (!vd_) {
        std::unique_ptr<VdaVideoFramePool> frame_pool = std::make_unique<VdaVideoFramePool>(weak_this_, client_task_runner_);
        // TODO(b/238684141): Wire a meaningful GpuDriverBugWorkarounds or remove
        // its use.
        vd_ = create_vd_cb_.Run(gpu::GpuDriverBugWorkarounds(), client_task_runner_, std::move(frame_pool),
            VideoDecoderPipeline::DefaultPreferredRenderableFourccs(), std::make_unique<NullMediaLog>());
        if (!vd_)
            return false;
...
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    VideoDecoderConfig vd_config(VideoCodecProfileToVideoCodec(config.profile), config.profile, VideoDecoderConfig::AlphaMode::kIsOpaque,
        config.container_color_space, VideoTransformation(), config.initial_expected_coded_size, gfx::Rect(config.initial_expected_coded_size),
        config.initial_expected_coded_size, std::vector<uint8_t>(), config.encryption_scheme);
    auto init_cb = base::BindOnce(&VdVideoDecodeAccelerator::OnInitializeDone, weak_this_);
    auto output_cb = base::BindRepeating(&VdVideoDecodeAccelerator::OnFrameReady, weak_this_);
    vd_->Initialize(std::move(vd_config), low_delay, cdm_context, std::move(init_cb), std::move(output_cb), base::DoNothing());
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
    if (is_encrypted_) {
        scoped_refptr<DecoderBuffer> buffer = DecryptBitstreamBuffer(std::move(bitstream_buffer));
...
void VdVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    vd_->Decode(std::move(buffer), base::BindOnce(&VdVideoDecodeAccelerator::OnDecodeDone, weak_this_, bitstream_id));
}

void VdVideoDecodeAccelerator::OnDecodeDone(int32_t bitstream_buffer_id, DecoderStatus status)
{
    DVLOGF(4) << "status: " << status.group() << ":" << static_cast<int>(status.code());
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    DCHECK(client_);
...
void VdVideoDecodeAccelerator::OnFrameReady(scoped_refptr<VideoFrame> frame)
{
    ...
    if (it == picture_at_client_.end()) {
        ...
        picture_at_client_.emplace(picture->picture_buffer_id(), std::make_pair(std::move(frame), 1));
    } else {
...
void VdVideoDecodeAccelerator::Flush()
{
    ...
    vd_->Decode(DecoderBuffer::CreateEOSBuffer(), base::BindOnce(&VdVideoDecodeAccelerator::OnFlushDone, weak_this_));
}
...
void VdVideoDecodeAccelerator::Reset()
{
    ...
    vd_->Reset(base::BindOnce(&VdVideoDecodeAccelerator::OnResetDone, weak_this_));
}
...
void VdVideoDecodeAccelerator::RequestFrames(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect, size_t max_num_frames,
    NotifyLayoutChangedCb notify_layout_changed_cb, ImportFrameCb import_frame_cb)
{
    VLOGF(2);
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    DCHECK(client_);
    ...
    client_->ProvidePictureBuffersWithVisibleRect(max_num_frames, fourcc.ToVideoPixelFormat(), coded_size, visible_rect);
}
...
void VdVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    VLOGF(2);
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
...
void VdVideoDecodeAccelerator::ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gmb_handle)
{
    ...
    if (notify_layout_changed_cb_) {
        ...
        if (!fourcc) {
            ...
            import_frame_cb_.Reset();
            std::move(notify_layout_changed_cb_).Run(CroStatus::Codes::kFailedToChangeResolution);
            return;
        }

        CHECK(media::VerifyGpuMemoryBufferHandle(pixel_format, coded_size_, gmb_handle));
        const uint64_t modifier = gmb_handle.type == gfx::NATIVE_PIXMAP ? gmb_handle.native_pixmap_handle.modifier : gfx::NativePixmapHandle::kNoModifier;

        std::vector<ColorPlaneLayout> planes = ExtractColorPlaneLayout(gmb_handle);
        layout_ = VideoFrameLayout::CreateWithPlanes(pixel_format, coded_size_, planes, VideoFrameLayout::kBufferAddressAlignment, modifier);
        if (!layout_) {
            VLOGF(1) << "Failed to create VideoFrameLayout. format: " << VideoPixelFormatToString(pixel_format) << ", coded_size: " << coded_size_.ToString()
                     << ", planes: " << VectorToString(planes) << ", modifier: " << std::hex << modifier;
            import_frame_cb_.Reset();
            std::move(notify_layout_changed_cb_).Run(CroStatus::Codes::kFailedToChangeResolution);
            return;
        }

        auto gb_layout = GpuBufferLayout::Create(*fourcc, coded_size_, planes, modifier);
        if (!gb_layout) {
            VLOGF(1) << "Failed to create GpuBufferLayout. fourcc: " << fourcc->ToString() << ", coded_size: " << coded_size_.ToString()
                     << ", planes: " << VectorToString(planes) << ", modifier: " << std::hex << modifier;
            layout_ = std::nullopt;
            import_frame_cb_.Reset();
            std::move(notify_layout_changed_cb_).Run(CroStatus::Codes::kFailedToChangeResolution);
    ...
    CHECK(media::VerifyGpuMemoryBufferHandle(pixel_format, layout_->coded_size(), gmb_handle));
    ...
    scoped_refptr<media::FrameResource> origin_frame = NativePixmapFrameResource::Create(gfx::Rect(layout_->coded_size()), layout_->coded_size(),
        base::TimeDelta(), gfx::BufferUsage::SCANOUT_CPU_READ_WRITE,
        base::MakeRefCounted<gfx::NativePixmapDmaBuf>(layout_->coded_size(), *buffer_format, std::move(gmb_handle.native_pixmap_handle)));
    ...
    auto res = frame_token_to_picture_id_.emplace(origin_frame->tracking_token(), picture_buffer_id);
    CHECK(res.second);
    ...
    // OnFrameReleased().
    scoped_refptr<FrameResource> wrapped_frame = origin_frame->CreateWrappingFrame();
    wrapped_frame->AddDestructionObserver(
        base::BindOnce(&VdVideoDecodeAccelerator::OnFrameReleasedThunk, weak_this_, client_task_runner_, std::move(origin_frame)));
    ...
    if (picture_at_client_.erase(picture_buffer_id) > 0) {
        VLOGF(1) << "Picture " << picture_buffer_id << " still referenced, dropping it.";
    }

    import_frame_cb_.Run(std::move(wrapped_frame));
}

std::optional<Picture> VdVideoDecodeAccelerator::GetPicture(const VideoFrame& frame)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    ...
    return std::make_optional(Picture(picture_buffer_id, bitstream_id, frame.visible_rect()));
}

// static
void VdVideoDecodeAccelerator::OnFrameReleasedThunk(std::optional<base::WeakPtr<VdVideoDecodeAccelerator>> weak_this,
    scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<FrameResource> origin_frame)
{
    DVLOGF(4);
    DCHECK(weak_this);

    task_runner->PostTask(FROM_HERE, base::BindOnce(&VdVideoDecodeAccelerator::OnFrameReleased, *weak_this, std::move(origin_frame)));
}

void VdVideoDecodeAccelerator::OnFrameReleased(scoped_refptr<FrameResource> origin_frame)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
...
{
    LOG(ERROR) << "Failed at " << location.ToString() << ", error code: " << static_cast<int>(error);
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
