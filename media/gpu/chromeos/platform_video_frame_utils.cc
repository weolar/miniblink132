...
static std::unique_ptr<ui::GbmDevice> CreateGbmDevice(
    std::string_view drm_node_file_prefix, int first_drm_file_index, const char* only_supported_driver_name_for_testing = nullptr)
{
    constexpr int kMaximumNumberOfDrmNodes = 100;

    for (int i = first_drm_file_index; i < first_drm_file_index + kMaximumNumberOfDrmNodes; ++i) {
        const base::FilePath dev_path(FILE_PATH_LITERAL(base::StrCat({ drm_node_file_prefix, base::NumberToString(i) })));
        ...
        const uint32_t kDrmNodeFileFlags = base::File::FLAG_OPEN | base::File::FLAG_READ | (is_render_node ? base::File::FLAG_WRITE : 0);
#else
        const uint32_t kDrmNodeFileFlags = base::File::FLAG_OPEN | base::File::FLAG_READ;
#endif
        ...
        const std::string driver_name(version->name, base::checked_cast<std::string::size_type>(version->name_len));
        ...
        if (only_supported_driver_name_for_testing == nullptr || (driver_name == only_supported_driver_name_for_testing)) {
...
public:
    ...
    gfx::GpuMemoryBufferHandle CreateGpuMemoryBuffer(gfx::BufferFormat format, const gfx::Size& size, gfx::BufferUsage buffer_usage)
    {
        ...
        std::unique_ptr<ui::GbmBuffer> buffer = CreateGbmBuffer(fourcc_format, size, buffer_usage);
        if (!buffer)
            return gfx::GpuMemoryBufferHandle();
    ...
    std::unique_ptr<ui::GbmBuffer> ImportGpuMemoryBuffer(gfx::BufferFormat format, const gfx::Size& size, gfx::NativePixmapHandle handle)
    {
        ...
        if (fourcc_format == DRM_FORMAT_INVALID)
            return nullptr;
        return gbm_device_->CreateBufferFromHandle(fourcc_format, size, std::move(handle));
    }
...
private:
    GbmDeviceWrapper()
    {
        ...
        auto gbm_device_from_render_node = CreateGbmDevice(kRenderNodeFilePrefix, kMinRenderNodeNum);
        ...
        if (cmd_line->HasSwitch(switches::kEnablePrimaryNodeAccessForVkmsTesting)) {
            ...
            auto gbm_device_from_primary_node = CreateGbmDevice(kPrimaryNodeFilePrefix, kMinPrimaryNodeNum, "vkms");
    ...
    std::unique_ptr<ui::GbmBuffer> CreateGbmBuffer(int fourcc_format, const gfx::Size& size, gfx::BufferUsage buffer_usage) EXCLUSIVE_LOCKS_REQUIRED(lock_)
    {
        uint32_t flags = ui::BufferUsageToGbmFlags(buffer_usage);
        std::unique_ptr<ui::GbmBuffer> buffer = gbm_device_->CreateBuffer(fourcc_format, size, flags);
        if (buffer)
            return buffer;
        ...
        constexpr auto kScanoutUsages = base::MakeFixedFlatSet<gfx::BufferUsage>({
            gfx::BufferUsage::SCANOUT, gfx::BufferUsage::PROTECTED_SCANOUT,
#if !BUILDFLAG(USE_V4L2_CODEC)
...
gfx::GpuMemoryBufferHandle AllocateGpuMemoryBufferHandle(VideoPixelFormat pixel_format, const gfx::Size& coded_size, gfx::BufferUsage buffer_usage)
{
    gfx::GpuMemoryBufferHandle gmb_handle;
    auto buffer_format = VideoPixelFormatToGfxBufferFormat(pixel_format);
    if (!buffer_format)
        return gmb_handle;
    return GbmDeviceWrapper::Get()->CreateGpuMemoryBuffer(*buffer_format, coded_size, buffer_usage);
}
...
void UniqueTrackingTokenHelper::ClearToken(const base::UnguessableToken& token)
{
...
void UniqueTrackingTokenHelper::SetUniqueTrackingToken(VideoFrameMetadata& metadata)
{
    CHECK(tokens_.size() < kMaxNumberOfTokens);

    if (metadata.tracking_token.has_value()) {
        if (auto res = tokens_.insert(*metadata.tracking_token); true == res.second) {
...
scoped_refptr<VideoFrame> CreateGpuMemoryBufferVideoFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, base::TimeDelta timestamp, gfx::BufferUsage buffer_usage)
{
    auto gmb_handle = AllocateGpuMemoryBufferHandle(pixel_format, coded_size, buffer_usage);
    if (gmb_handle.is_null() || gmb_handle.type != gfx::NATIVE_PIXMAP)
        return nullptr;

    return CreateVideoFrameFromGpuMemoryBufferHandle(std::move(gmb_handle), pixel_format, coded_size, visible_rect, natural_size, timestamp, buffer_usage);
}

scoped_refptr<VideoFrame> CreateVideoFrameFromGpuMemoryBufferHandle(gfx::GpuMemoryBufferHandle gmb_handle, VideoPixelFormat pixel_format,
    const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp, gfx::BufferUsage buffer_usage)
{
    const bool supports_zero_copy_webgpu_import = gmb_handle.native_pixmap_handle.supports_zero_copy_webgpu_import;

    auto buffer_format = VideoPixelFormatToGfxBufferFormat(pixel_format);
    DCHECK(buffer_format);
    gpu::GpuMemoryBufferSupport support;
    std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer
        = support.CreateGpuMemoryBufferImplFromHandle(std::move(gmb_handle), coded_size, *buffer_format, buffer_usage, base::NullCallback());
    if (!gpu_memory_buffer)
        return nullptr;

    // It is not necessary to pass a SharedImage because this VideoFrame is not
    // rendered.
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect, natural_size, std::move(gpu_memory_buffer), timestamp);
    if (!frame)
        return nullptr;
...
scoped_refptr<VideoFrame> CreatePlatformVideoFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, base::TimeDelta timestamp, gfx::BufferUsage buffer_usage)
{
    auto gmb_handle = AllocateGpuMemoryBufferHandle(pixel_format, coded_size, buffer_usage);
    if (gmb_handle.is_null() || gmb_handle.type != gfx::NATIVE_PIXMAP)
    ...
    auto layout = VideoFrameLayout::CreateWithPlanes(
        pixel_format, coded_size, std::move(planes), VideoFrameLayout::kBufferAddressAlignment, gmb_handle.native_pixmap_handle.modifier);
    ...
    auto frame = VideoFrame::WrapExternalDmabufs(*layout, visible_rect, natural_size, std::move(dmabuf_fds), timestamp);
    if (!frame)
        return nullptr;
...
std::optional<VideoFrameLayout> GetPlatformVideoFrameLayout(VideoPixelFormat pixel_format, const gfx::Size& coded_size, gfx::BufferUsage buffer_usage)
{
    // |visible_rect| and |natural_size| do not matter here. |coded_size| is set
    // as a dummy variable.
    auto frame = CreatePlatformVideoFrame(pixel_format, coded_size, gfx::Rect(coded_size), coded_size, base::TimeDelta(), buffer_usage);
    return frame ? std::make_optional<VideoFrameLayout>(frame->layout()) : std::nullopt;
}

gfx::GpuMemoryBufferHandle CreateGpuMemoryBufferHandle(const VideoFrame* video_frame)
{
    DCHECK(video_frame);
    ...
    case VideoFrame::STORAGE_GPU_MEMORY_BUFFER:
        ...
        CHECK_EQ(handle.type, gfx::NATIVE_PIXMAP) << "The cloned handle has an unexpected type: " << handle.type;
        CHECK(!handle.native_pixmap_handle.planes.empty()) << "The cloned handle has no planes";
        break;
    case VideoFrame::STORAGE_DMABUFS: {
        ...
        for (size_t i = 0; i < num_planes; ++i) {
            ...
            int source_fd = (i < video_frame->NumDmabufFds()) ? video_frame->GetDmabufFd(i) : duped_fds.back().get();
        ...
        for (size_t i = 0; i < num_planes; ++i) {
            ...
            handle.native_pixmap_handle.planes.emplace_back(plane.stride, plane.offset, plane.size, std::move(duped_fds[i]));
        }
    } break;
    default:
        NOTREACHED() << "Unsupported storage type: " << video_frame->storage_type();
    }
    CHECK_EQ(handle.type, gfx::NATIVE_PIXMAP);
    if (video_frame->format() == PIXEL_FORMAT_MJPEG)
        return handle;
#if DCHECK_IS_ON()
    const bool is_handle_valid = !handle.is_null() && VerifyGpuMemoryBufferHandle(video_frame->format(), video_frame->coded_size(), handle);
    DLOG_IF(WARNING, !is_handle_valid) << __func__ << "(): Created GpuMemoryBufferHandle is invalid";
#endif // DCHECK_IS_ON()
    return handle;
}

scoped_refptr<gfx::NativePixmapDmaBuf> CreateNativePixmapDmaBuf(const VideoFrame* video_frame)
{
    DCHECK(video_frame);

    // Create a native pixmap from the frame's memory buffer handle.
    gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle = CreateGpuMemoryBufferHandle(video_frame);
    ...
    auto buffer_format = VideoPixelFormatToGfxBufferFormat(video_frame->layout().format());
    if (!buffer_format) {
    ...
    auto native_pixmap
        = base::MakeRefCounted<gfx::NativePixmapDmaBuf>(video_frame->coded_size(), *buffer_format, std::move(gpu_memory_buffer_handle.native_pixmap_handle));
...
bool CanImportGpuMemoryBufferHandle(const gfx::Size& size, gfx::BufferFormat format, const gfx::GpuMemoryBufferHandle& gmb_handle)
{
    ...
    if (!pixel_format) {
        VLOGF(1) << "Unsupported buffer format: " << gfx::BufferFormatToString(format);
        return false;
    }
    ...
    gfx::NativePixmapHandle native_pixmap_handle = gfx::CloneHandleForIPC(gmb_handle.native_pixmap_handle);
    if (native_pixmap_handle.planes.empty()) {
        VLOGF(1) << "Could not duplicate the NativePixmapHandle";
        return false;
    }
    return !!GbmDeviceWrapper::Get()->ImportGpuMemoryBuffer(format, size, std::move(native_pixmap_handle));
}
