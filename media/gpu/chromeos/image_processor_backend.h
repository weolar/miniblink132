...
public:
    ...
    using FrameResourceReadyCB = base::OnceCallback<void(scoped_refptr<FrameResource>)>;
    // Callback for returning a processed image to the client.
    ...
    // buffer.
    using LegacyFrameResourceReadyCB = base::OnceCallback<void(size_t, scoped_refptr<FrameResource>)>;
    ...
    struct MEDIA_GPU_EXPORT PortConfig {
        ...
        PortConfig(Fourcc fourcc, const gfx::Size& size, const std::vector<ColorPlaneLayout>& planes, const gfx::Rect& visible_rect,
            const VideoFrame::StorageType storage_type);
        ~PortConfig();

        bool operator==(const PortConfig& other) const
        {
            return fourcc == other.fourcc && size == other.size && planes == other.planes && visible_rect == other.visible_rect
    ...
    // actual work.
    void Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb);

    // Process |input_frame| and store in |output_frame|. Only used when output
    // mode is IMPORT. After processing, call |cb| with |output_frame|.
    virtual void ProcessFrame(scoped_refptr<FrameResource> input_frame, scoped_refptr<FrameResource> output_frame, FrameResourceReadyCB cb) = 0;
    ...
    // panic.
    virtual void ProcessLegacyFrame(scoped_refptr<FrameResource> frame, LegacyFrameResourceReadyCB cb);
    ...
    const PortConfig& input_config() const
    {
        return input_config_;
    }
    const PortConfig& output_config() const
    {
...
protected:
    ...
    ImageProcessorBackend(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, ErrorCB error_cb,
...
template <> struct MEDIA_GPU_EXPORT default_delete<media::ImageProcessorBackend> {
    constexpr default_delete() = default;

    template <typename U, typename = typename std::enable_if<std::is_convertible<U*, media::ImageProcessorBackend*>::value>::type>
