...
constexpr VideoFrame::StorageType kDefaultFrameStorageType = VideoFrame::STORAGE_DMABUFS;

// The default method to create frames.
CroStatus::Or<scoped_refptr<FrameResource>> DefaultCreateFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers, bool needs_detiling, base::TimeDelta timestamp)
{
    ...
    scoped_refptr<FrameResource> frame = NativePixmapFrameResource::Create(format, coded_size, visible_rect, natural_size, timestamp,
        use_protected ? gfx::BufferUsage::PROTECTED_SCANOUT_VDA_WRITE
                      : (use_linear_buffers ? gfx::BufferUsage::SCANOUT_CPU_READ_WRITE : gfx::BufferUsage::SCANOUT_VDA_WRITE));
...
scoped_refptr<FrameResource> PlatformVideoFramePool::GetFrame()
{
    ...
    if (free_frames_.empty()) {
        ...
        CroStatus::Or<scoped_refptr<FrameResource>> new_frame
            = create_frame_cb_.Run(format, coded_size, gfx::Rect(GetRectSizeFromOrigin(visible_rect_)), coded_size, use_protected_, *use_linear_buffers_,
                frame_layout_->fourcc() == Fourcc(Fourcc::MM21) || frame_layout_->fourcc() == Fourcc(Fourcc::MT2T), base::TimeDelta());
    ...
    scoped_refptr<FrameResource> wrapped_frame = origin_frame->CreateWrappingFrame(visible_rect_, natural_size_);
    DCHECK(wrapped_frame);
    frames_in_use_.emplace(wrapped_frame->GetSharedMemoryId(), origin_frame.get());
    wrapped_frame->AddDestructionObserver(
        base::BindOnce(&PlatformVideoFramePool::OnFrameReleasedThunk, weak_this_, parent_task_runner_, std::move(origin_frame)));
...
CroStatus::Or<GpuBufferLayout> PlatformVideoFramePool::Initialize(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, size_t max_num_frames, bool use_protected, bool use_linear_buffers)
{
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
    ...
    if (!IsSameFormat_Locked(format, coded_size, visible_rect, use_protected)) {
        ...
        free_frames_.clear();
        auto maybe_frame = create_frame_cb_.Run(format, coded_size, visible_rect, natural_size, use_protected, *use_linear_buffers_,
            fourcc == Fourcc(Fourcc::MM21) || fourcc == Fourcc(Fourcc::MT2T), base::TimeDelta());
        if (!maybe_frame.has_value())
            return std::move(maybe_frame).error();
        auto frame = std::move(maybe_frame).value();
        frame_layout_ = GpuBufferLayout::Create(fourcc, frame->coded_size(), frame->layout().planes(), frame->layout().modifier());
        if (!frame_layout_)
...
void PlatformVideoFramePool::SetCustomFrameAllocator(DmabufVideoFramePool::CreateFrameCB allocator, VideoFrame::StorageType frame_storage_type)
{
    base::AutoLock auto_lock(lock_);
...
FrameResource* PlatformVideoFramePool::GetOriginalFrame(gfx::GenericSharedMemoryId frame_id)
{
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
...
void PlatformVideoFramePool::OnFrameReleasedThunk(
    std::optional<base::WeakPtr<PlatformVideoFramePool>> pool, scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<FrameResource> origin_frame)
{
    TRACE_EVENT2(
        "media", "PlatformVideoFramePool::OnFrameReleasedThunk", "frame_id", origin_frame->unique_id(), "frame", origin_frame->AsHumanReadableString());
    DCHECK(pool);
    DVLOGF(4);

    task_runner->PostTask(FROM_HERE, base::BindOnce(&PlatformVideoFramePool::OnFrameReleased, *pool, std::move(origin_frame)));
}

void PlatformVideoFramePool::OnFrameReleased(scoped_refptr<FrameResource> origin_frame)
{
    TRACE_EVENT2("media", "PlatformVideoFramePool::OnFrameReleased", "frame_id", origin_frame->unique_id(), "frame", origin_frame->AsHumanReadableString());
    DCHECK(parent_task_runner_->RunsTasksInCurrentSequence());
    DVLOGF(4);
    base::AutoLock auto_lock(lock_);
    ...
    if (IsSameFormat_Locked(origin_frame->format(), origin_frame->coded_size(), origin_frame->visible_rect(), origin_frame->metadata().hw_protected)) {
...
void PlatformVideoFramePool::InsertFreeFrame_Locked(scoped_refptr<FrameResource> frame)
{
    DCHECK(frame);
    DVLOGF(4);
    lock_.AssertAcquired();
...
bool PlatformVideoFramePool::IsSameFormat_Locked(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, bool use_protected) const
{
    DVLOGF(4);
    lock_.AssertAcquired();

    return frame_layout_ && frame_layout_->fourcc().ToVideoPixelFormat() == format && frame_layout_->size() == coded_size
        && GetRectSizeFromOrigin(visible_rect_) == GetRectSizeFromOrigin(visible_rect) && use_protected_ == use_protected;
}
