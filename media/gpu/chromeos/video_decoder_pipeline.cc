...
std::optional<Fourcc> PickRenderableFourcc(
    const std::vector<Fourcc>& renderable_fourccs, const std::vector<Fourcc>& candidates, std::optional<Fourcc> preferred_fourcc)
{
    if (preferred_fourcc && base::Contains(candidates, *preferred_fourcc) && base::Contains(renderable_fourccs, *preferred_fourcc)) {
...
size_t EstimateRequiredRendererPipelineBuffers(bool low_delay, bool use_protected)
{
...
#endif
    static_assert(kExpectedNonLatencyPipelineDepth > limits::kMaxVideoFrames, "kMaxVideoFrames is expected to be relatively small");
...
scoped_refptr<base::SequencedTaskRunner> GetDecoderTaskRunner(bool in_video_decoder_process)
{
    ...
    if (in_video_decoder_process) {
        return base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::MayBlock() });
    }
    return base::ThreadPool::CreateSingleThreadTaskRunner(
        { base::WithBaseSyncPrimitives(), base::TaskPriority::USER_VISIBLE, base::MayBlock() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED);
}
...
private:
    ...
    void ConvertFrameImpl(scoped_refptr<FrameResource> frame) override
    {
        ...
        LOG_ASSERT(frame->AsVideoFrameResource() || frame->AsNativePixmapFrameResource()) << "|frame| is expected to be a VideoFrameResource or "
                                                                                             "NativePixmapFrameResource";
        scoped_refptr<VideoFrame> video_frame
            = frame->AsVideoFrameResource() ? frame->AsVideoFrameResource()->GetMutableVideoFrame() : frame->AsNativePixmapFrameResource()->CreateVideoFrame();
        if (!video_frame) {
...
VideoDecoderMixin::VideoDecoderMixin(
    std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
...
CroStatus VideoDecoderMixin::AttachSecureBuffer(scoped_refptr<DecoderBuffer>& buffer)
{
    return CroStatus::Codes::kOk;
}

void VideoDecoderMixin::Initialize(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
...
VideoDecoderPipeline::ClientFlushCBState::ClientFlushCBState(DecodeCB flush_cb, DecoderStatus decoder_decode_status)
...
// static
std::unique_ptr<VideoDecoder> VideoDecoderPipeline::Create(const gpu::GpuDriverBugWorkarounds& workarounds,
    scoped_refptr<base::SequencedTaskRunner> client_task_runner, std::unique_ptr<DmabufVideoFramePool> frame_pool,
    std::unique_ptr<FrameResourceConverter> frame_converter, std::vector<Fourcc> renderable_fourccs, std::unique_ptr<MediaLog> media_log,
    mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder, bool in_video_decoder_process)
{
    ...
    if (oop_video_decoder) {
        create_decoder_function_cb = base::BindOnce(&OOPVideoDecoder::Create, std::move(oop_video_decoder));
        ...
        if (IsV4L2DecoderStateful()) {
            create_decoder_function_cb = base::BindOnce(&V4L2StatefulVideoDecoder::Create);
        } else {
    ...
    auto* pipeline = new VideoDecoderPipeline(workarounds, std::move(client_task_runner), std::move(frame_pool), std::move(frame_converter),
        std::move(renderable_fourccs), std::move(media_log), std::move(create_decoder_function_cb), uses_oop_video_decoder, in_video_decoder_process);
    return std::make_unique<AsyncDestroyVideoDecoder<VideoDecoderPipeline>>(base::WrapUnique(pipeline));
}

// static
std::unique_ptr<VideoDecoder> VideoDecoderPipeline::CreateForARC(const gpu::GpuDriverBugWorkarounds& workarounds,
    scoped_refptr<base::SequencedTaskRunner> client_task_runner, std::unique_ptr<DmabufVideoFramePool> frame_pool, std::vector<Fourcc> renderable_fourccs,
    std::unique_ptr<MediaLog> media_log)
{
    ...
    auto* pipeline = new VideoDecoderPipeline(workarounds, std::move(client_task_runner), std::move(frame_pool),
        RegisteredFrameConverter::Create(base::MakeRefCounted<FrameRegistry>()), std::move(renderable_fourccs), std::move(media_log),
...
std::unique_ptr<VideoDecoder> VideoDecoderPipeline::CreateForTesting(
    scoped_refptr<base::SequencedTaskRunner> client_task_runner, std::unique_ptr<MediaLog> media_log, bool ignore_resolution_changes_to_smaller_for_testing)
{
    CreateDecoderFunctionCB create_decoder_function_cb;
#if BUILDFLAG(USE_VAAPI)
    create_decoder_function_cb = base::BindOnce(&VaapiVideoDecoder::Create);
    ...
    auto* pipeline = new VideoDecoderPipeline(gpu::GpuDriverBugWorkarounds(), std::move(client_task_runner), std::make_unique<PlatformVideoFramePool>(),
        /*frame_converter=*/nullptr, VideoDecoderPipeline::DefaultPreferredRenderableFourccs(), std::move(media_log), std::move(create_decoder_function_cb),
        /*uses_oop_video_decoder=*/false,
...
// static
void VideoDecoderPipeline::NotifySupportKnown(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
    base::OnceCallback<void(mojo::PendingRemote<stable::mojom::StableVideoDecoder>)> cb)
{
    if (oop_video_decoder) {
        OOPVideoDecoder::NotifySupportKnown(std::move(oop_video_decoder), std::move(cb));
        return;
    }
...
// static
std::optional<SupportedVideoDecoderConfigs> VideoDecoderPipeline::GetSupportedConfigs(
    VideoDecoderType decoder_type, const gpu::GpuDriverBugWorkarounds& workarounds)
{
    ...
    if (workarounds.disable_accelerated_vp8_decode) {
        std::erase_if(configs.value(), [](const auto& config) { return config.profile_min >= VP8PROFILE_MIN && config.profile_max <= VP8PROFILE_MAX; });
    }

    if (workarounds.disable_accelerated_vp9_decode) {
        std::erase_if(
            configs.value(), [](const auto& config) { return config.profile_min >= VP9PROFILE_PROFILE0 && config.profile_max <= VP9PROFILE_PROFILE0; });
    }

    if (workarounds.disable_accelerated_vp9_profile2_decode) {
        std::erase_if(
            configs.value(), [](const auto& config) { return config.profile_min >= VP9PROFILE_PROFILE2 && config.profile_max <= VP9PROFILE_PROFILE2; });
    }

    if (workarounds.disable_accelerated_h264_decode) {
        std::erase_if(configs.value(), [](const auto& config) { return config.profile_min >= H264PROFILE_MIN && config.profile_max <= H264PROFILE_MAX; });
    }

    if (workarounds.disable_accelerated_hevc_decode) {
        std::erase_if(configs.value(), [](const auto& config) { return config.profile_min >= HEVCPROFILE_MIN && config.profile_max <= HEVCPROFILE_MAX; });
    }

    return configs;
}

VideoDecoderPipeline::VideoDecoderPipeline(const gpu::GpuDriverBugWorkarounds& gpu_workarounds, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
    std::unique_ptr<DmabufVideoFramePool> frame_pool, std::unique_ptr<FrameResourceConverter> frame_converter, std::vector<Fourcc> renderable_fourccs,
    std::unique_ptr<MediaLog> media_log, CreateDecoderFunctionCB create_decoder_function_cb, bool uses_oop_video_decoder, bool in_video_decoder_process)
    : gpu_workarounds_(gpu_workarounds)
    , client_task_runner_(std::move(client_task_runner))
    , decoder_task_runner_(uses_oop_video_decoder ? client_task_runner_ : GetDecoderTaskRunner(in_video_decoder_process))
    , main_frame_pool_(std::move(frame_pool))
    , frame_converter_(frame_converter ? std::move(frame_converter) : DefaultFrameConverter::Create())
    ...
    , uses_oop_video_decoder_(uses_oop_video_decoder)
{
    ...
    frame_converter_->Initialize(decoder_task_runner_, base::BindRepeating(&VideoDecoderPipeline::OnFrameConverted, decoder_weak_this_));
}
...
// static
void VideoDecoderPipeline::DestroyAsync(std::unique_ptr<VideoDecoderPipeline> pipeline)
{
    DVLOGF(2);
    DCHECK(pipeline);
...
bool VideoDecoderPipeline::CanReadWithoutStalling() const
{
    ...
    if (uses_oop_video_decoder_) {
        return oop_decoder_can_read_without_stalling_.load(std::memory_order_seq_cst);
    }
...
size_t VideoDecoderPipeline::GetDecoderMaxOutputFramePoolSize() const
{
    ...
    return decoder_ ? decoder_->GetMaxOutputFramePoolSize() : std::numeric_limits<size_t>::max();
}

void VideoDecoderPipeline::Initialize(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    ...
    const auto supported_configs = supported_configs_for_testing_.empty() ? VideoDecoderPipeline::GetSupportedConfigs(GetDecoderType(), gpu_workarounds_)
    ...
    if (!IsVideoDecoderConfigSupported(supported_configs.value(), config)) {
        VLOGF(1) << "Video configuration is not supported: " << config.AsHumanReadableString();
    ...
    base::UmaHistogramEnumeration("Media.PlatformVideoDecoding.VideoCodecProfile", config.profile(), static_cast<VideoCodecProfile>(kVideoCodecProfileCount));

    needs_bitstream_conversion_ = (config.codec() == VideoCodec::kH264) || (config.codec() == VideoCodec::kHEVC);

    decoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoDecoderPipeline::InitializeTask, decoder_weak_this_, config, low_delay, cdm_context, std::move(init_cb), std::move(output_cb),
            std::move(waiting_cb)));
}

void VideoDecoderPipeline::InitializeTask(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    DVLOGF(3);
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    if (!decoder_ && !create_decoder_function_cb_.is_null()) {
        ...
        decoder_ = std::move(create_decoder_function_cb_).Run(media_log_->Clone(), decoder_task_runner_, decoder_weak_this_);
    }
    // Note: |decoder_| might fail to be created, e.g. on V4L2 platforms.
    if (!decoder_) {
        OnError("|decoder_| creation failed.");
        client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), DecoderStatus::Codes::kFailedToCreateDecoder));
        return;
    }
    ...
    if (frame_converter_->UsesGetOriginalFrameCB()) {
        ...
        if (uses_oop_video_decoder_) {
            ...
            get_original_frame_cb = base::BindRepeating(&OOPVideoDecoder::GetOriginalFrame, base::Unretained(static_cast<OOPVideoDecoder*>(decoder_.get())));
        } else {
            CHECK(main_frame_pool_);
            PlatformVideoFramePool* platform_video_frame_pool = main_frame_pool_->AsPlatformVideoFramePool();
            ...
            get_original_frame_cb = base::BindRepeating(&PlatformVideoFramePool::GetOriginalFrame, base::Unretained(platform_video_frame_pool));
        }

        frame_converter_->set_get_original_frame_cb(std::move(get_original_frame_cb));
    }

    estimated_num_buffers_for_renderer_ = EstimateRequiredRendererPipelineBuffers(low_delay, config.is_encrypted());

#if BUILDFLAG(USE_V4L2_CODEC)
    decryption_needs_vp9_superframe_splitting_ = config.codec() == VideoCodec::kVP9;
#endif
    ...
    decoder_->Initialize(config, /* low_delay=*/false, cdm_context,
        base::BindOnce(&VideoDecoderPipeline::OnInitializeDone, decoder_weak_this_, std::move(init_cb), cdm_context),
        base::BindRepeating(&VideoDecoderPipeline::OnFrameDecoded, decoder_weak_this_),
        base::BindRepeating(&VideoDecoderPipeline::OnDecoderWaiting, decoder_weak_this_));
}

void VideoDecoderPipeline::OnInitializeDone(InitCB init_cb, CdmContext* cdm_context, DecoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(4) << "Initialization status = " << static_cast<int>(status.code());

    if (!status.is_ok()) {
        MEDIA_LOG(ERROR, media_log_) << "VideoDecoderPipeline |decoder_| Initialize() failed, status: " << static_cast<int>(status.code());
        ...
        MEDIA_LOG(INFO, media_log_) << "VideoDecoderPipeline |decoder_| Initialize() successful";
    }
    ...
    if (decoder_ && decoder_->NeedsTranscryption()) {
        if (!cdm_context) {
            ...
            buffer_transcryptor_ = std::make_unique<DecoderBufferTranscryptor>(cdm_context, *decoder_, decryption_needs_vp9_superframe_splitting_,
                base::BindRepeating(&VideoDecoderPipeline::OnBufferTranscrypted, decoder_weak_this_),
                base::BindRepeating(&VideoDecoderPipeline::OnDecoderWaiting, decoder_weak_this_));
        }
    } else {
    ...
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), status));
}
...
void VideoDecoderPipeline::Reset(base::OnceClosure reset_cb)
{
    ...
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoDecoderPipeline::ResetTask, decoder_weak_this_, std::move(reset_cb)));
}
...
void VideoDecoderPipeline::ResetTask(base::OnceClosure reset_cb)
{
    ...
    decoder_->Reset(base::BindOnce(&VideoDecoderPipeline::OnResetDone, decoder_weak_this_, std::move(reset_cb)));
}
...
void VideoDecoderPipeline::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    CHECK(buffer);
    DVLOGF(4);
    TRACE_EVENT1("media,gpu", "VideoDecoderPipeline::Decode", "timestamp", (buffer->end_of_stream() ? 0 : buffer->timestamp().InMicroseconds()));
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoDecoderPipeline::DecodeTask, decoder_weak_this_, std::move(buffer), std::move(decode_cb)));
}

void VideoDecoderPipeline::DecodeTask(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DCHECK(decoder_);
    CHECK(buffer);
    DVLOGF(4);
    TRACE_EVENT1("media,gpu", "VideoDecoderPipeline::DecodeTask", "timestamp", (buffer->end_of_stream() ? 0 : buffer->timestamp().InMicroseconds()));
    if (has_error_) {
        client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
        return;
    }
    ...
    if (buffer_transcryptor_) {
        buffer_transcryptor_->EnqueueBuffer(
            std::move(buffer), base::BindOnce(&VideoDecoderPipeline::OnDecodeDone, decoder_weak_this_, is_flush, std::move(decode_cb)));
        return;
    }
#endif // BUILDFLAG(IS_CHROMEOS)

    decoder_->Decode(std::move(buffer), base::BindOnce(&VideoDecoderPipeline::OnDecodeDone, decoder_weak_this_, is_flush, std::move(decode_cb)));
}

void VideoDecoderPipeline::OnDecodeDone(bool is_flush, DecodeCB decode_cb, DecoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(4) << "is_flush: " << is_flush << ", status: " << static_cast<int>(status.code());
    ...
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), std::move(status)));
}
...
void VideoDecoderPipeline::OnFrameDecoded(scoped_refptr<FrameResource> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(4);
    TRACE_EVENT1("media,gpu", "VideoDecoderPipeline::OnFrameDecoded", "timestamp", (frame ? frame->timestamp().InMicroseconds() : 0));
    ...
    if (uses_oop_video_decoder_) {
        oop_decoder_can_read_without_stalling_.store(decoder_->CanReadWithoutStalling(), std::memory_order_seq_cst);
    }

    if (image_processor_) {
        image_processor_->Process(std::move(frame), base::BindOnce(&VideoDecoderPipeline::OnFrameProcessed, decoder_weak_this_));
        return;
    }
...
void VideoDecoderPipeline::OnFrameProcessed(scoped_refptr<FrameResource> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(4);
    TRACE_EVENT1("media,gpu", "VideoDecoderPipeline::OnFrameProcessed", "timestamp", (frame ? frame->timestamp().InMicroseconds() : 0));
    frame_converter_->ConvertFrame(std::move(frame));
}

void VideoDecoderPipeline::OnFrameConverted(scoped_refptr<VideoFrame> video_frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(4);
    TRACE_EVENT1("media,gpu", "VideoDecoderPipeline::OnFrameConverted", "timestamp", (video_frame ? video_frame->timestamp().InMicroseconds() : 0));
    if (!video_frame) {
    ...
    // |client_task_runner_|.
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(client_output_cb_, std::move(video_frame)));
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    const bool frame_converter_has_pending_frames_ = frame_converter_->HasPendingFrames();
    const bool image_processor_has_pending_frames_ = image_processor_ && image_processor_->HasPendingFrames();

    DVLOGF(3) << "|frame_converter_|: " << (frame_converter_has_pending_frames_ ? "yes" : "no")
              << ", |image_processor_|: " << (image_processor_has_pending_frames_ ? "yes" : "no");
    return frame_converter_has_pending_frames_ || image_processor_has_pending_frames_;
}
...
void VideoDecoderPipeline::CallFlushCbIfNeeded(std::optional<DecoderStatus> override_status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    const DecoderStatus status = override_status.value_or(client_flush_cb_state_->decoder_decode_status);
    DVLOGF(3) << "status: " << static_cast<int>(status.code());
    client_flush_cb_state_.reset();
    client_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(flush_cb), status));
}
...
CroStatus::Or<PixelLayoutCandidate> VideoDecoderPipeline::PickDecoderOutputFormat(const std::vector<PixelLayoutCandidate>& candidates,
    const gfx::Rect& decoder_visible_rect, const gfx::Size& decoder_natural_size, std::optional<gfx::Size> output_size, size_t num_codec_reference_frames,
    bool use_protected, bool need_aux_frame_pool, std::optional<DmabufVideoFramePool::CreateFrameCB> allocator)
{
    DVLOGF(3);
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    if (use_protected) {
        for (const auto& candidate : candidates) {
            if (candidate.fourcc == Fourcc(Fourcc::MM21) || candidate.fourcc == Fourcc(Fourcc::MT2T)) {
    ...
    main_frame_pool_->AsPlatformVideoFramePool()->SetCustomFrameAllocator(*allocator, VideoFrame::STORAGE_DMABUFS);
    ...
    CHECK(!allocator.has_value());
    CHECK(main_frame_pool_->AsPlatformVideoFramePool() || main_frame_pool_->IsFakeVideoFramePool());
...
#error "Unsupported platform"
    ...
    if (viable_candidate) {
        ...
        // being decoded.
        const size_t num_pictures = std::min(GetDecoderMaxOutputFramePoolSize(), num_codec_reference_frames + 1 + estimated_num_buffers_for_renderer_);
        VLOGF(1) << "Initializing frame pool with up to " << num_pictures << " VideoFrames. No ImageProcessor needed.";
        ...
        if (use_protected) {
            ...
            // Resolution
            size_t mem_required = viable_candidate->size.width() * viable_candidate->size.height();
            // YUV 4:2:0
            ...
            VLOGF(1) << "V4L2 secure memory requirement is " << (mem_required / (1024 * 1024)) << "MB";
            if (mem_required > kMaxV4L2ProtectedMemory) {
                LOG(ERROR) << "Exceeded max memory for secure V4L2: " << (mem_required / (1024 * 1024)) << "MB of " << (kMaxV4L2ProtectedMemory / (1024 * 1024))
                           << "MB";
        ...
        CroStatus::Or<GpuBufferLayout> status_or_layout = main_frame_pool_->Initialize(
            viable_candidate->fourcc, viable_candidate->size, decoder_visible_rect, decoder_natural_size, num_pictures, use_protected);
        ...
        if (layout.modifier() == viable_candidate->modifier) {
            ...
            DVLOGF(2) << "Unsupported modifier, " << std::hex << viable_candidate->modifier << ", passed in";
    ...
    if (create_image_processor_cb_for_testing_) {
        image_processor = create_image_processor_cb_for_testing_.Run(candidates,
            /*input_visible_rect=*/decoder_visible_rect, output_size ? *output_size : decoder_visible_rect.size(), estimated_num_buffers_for_renderer_);
    } else {
        VLOGF(2) << "Initializing ImageProcessor; max buffers: " << estimated_num_buffers_for_renderer_;
        ...
        // completes.
        const VideoFrame::StorageType output_storage_type = main_frame_pool_->GetFrameStorageType();
        image_processor = ImageProcessorFactory::CreateWithInputCandidates(candidates, /*input_visible_rect=*/decoder_visible_rect,
            output_size ? *output_size : decoder_visible_rect.size(), output_storage_type, estimated_num_buffers_for_renderer_, decoder_task_runner_,
            base::BindRepeating(&PickRenderableFourcc, renderable_fourccs_),
            base::BindPostTaskToCurrentDefault(base::BindRepeating(&VideoDecoderPipeline::OnError, decoder_weak_this_, "ImageProcessor error")));
    }
    ...
    if (need_aux_frame_pool) {
        ...
        VLOGF(2) << "Initializing auxiliary frame pool with up to " << num_pictures << " VideoFrames";
        auxiliary_frame_pool_->set_parent_task_runner(decoder_task_runner_);

#if BUILDFLAG(IS_LINUX)
        // The custom allocator creates frames backed by NativePixmap, which uses a
        // VideoFrame::StorageType of VideoFrame::STORAGE_DMABUFS.
        auxiliary_frame_pool_->AsPlatformVideoFramePool()->SetCustomFrameAllocator(*allocator, VideoFrame::STORAGE_DMABUFS);
#endif

        CroStatus::Or<GpuBufferLayout> status_or_layout = auxiliary_frame_pool_->Initialize(image_processor->input_config().fourcc,
            image_processor->input_config().size, decoder_visible_rect, decoder_natural_size, num_pictures, use_protected);
        if (!status_or_layout.has_value()) {
    ...
    num_buffers = std::max<size_t>(estimated_num_buffers_for_renderer_, kMinImageProcessorOutputFramePoolSize);
#else
    ...
    VLOGF(2) << "Initializing Image Processor frame pool with up to " << num_buffers << " VideoFrames";
    auto status_or_image_processor
        = ImageProcessorWithPool::Create(std::move(image_processor), main_frame_pool_.get(), num_buffers, use_protected, decoder_task_runner_);
    ...
    return PixelLayoutCandidate { fourcc, size, gfx::NativePixmapHandle::kNoModifier };
}

#if BUILDFLAG(IS_CHROMEOS)
void VideoDecoderPipeline::OnBufferTranscrypted(scoped_refptr<DecoderBuffer> transcrypted_buffer, DecodeCB decode_callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DCHECK(!has_error_);
