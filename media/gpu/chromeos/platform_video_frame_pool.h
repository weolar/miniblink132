...
public:
    ...
    CroStatus::Or<GpuBufferLayout> Initialize(const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
        size_t max_num_frames, bool use_protected, bool use_linear_buffers) override;
    ...
    void SetCustomFrameAllocator(DmabufVideoFramePool::CreateFrameCB allocator, VideoFrame::StorageType frame_storage_type);
...
private:
    ...
    static void OnFrameReleasedThunk(std::optional<base::WeakPtr<PlatformVideoFramePool>> pool, scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    void InsertFreeFrame_Locked(scoped_refptr<FrameResource> frame) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    size_t GetTotalNumFrames_Locked() const EXCLUSIVE_LOCKS_REQUIRED(lock_);
    bool IsSameFormat_Locked(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, bool use_protected) const
    ...
    media::UniqueTrackingTokenHelper frame_tracking_token_helper_ GUARDED_BY(lock_);
    ...
    base::circular_deque<scoped_refptr<FrameResource>> free_frames_ GUARDED_BY(lock_);
    // Mapping from the frame's shared memory ID to the original frame.
    std::map<gfx::GenericSharedMemoryId, raw_ptr<FrameResource, CtnExperimental>> frames_in_use_ GUARDED_BY(lock_);
