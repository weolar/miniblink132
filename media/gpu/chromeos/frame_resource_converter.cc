...
void FrameResourceConverter::Initialize(scoped_refptr<base::SequencedTaskRunner> parent_task_runner, OutputCB output_cb)
{
...
void FrameResourceConverter::set_get_original_frame_cb(GetOriginalFrameCB get_original_frame_cb)
{
    DCHECK(parent_task_runner_->RunsTasksInCurrentSequence());
    get_original_frame_cb_ = std::move(get_original_frame_cb);
}

void FrameResourceConverter::OnError(const base::Location& location, const std::string& msg)
{
...
FrameResource* FrameResourceConverter::GetOriginalFrame(FrameResource& frame) const
{
    DCHECK(parent_task_runner_->RunsTasksInCurrentSequence());
...
const scoped_refptr<base::SequencedTaskRunner>& FrameResourceConverter::parent_task_runner()
{
...
{
    DCHECK(!parent_task_runner_ || parent_task_runner_->RunsTasksInCurrentSequence());
    delete this;
}
...
void default_delete<media::FrameResourceConverter>::operator()(media::FrameResourceConverter* ptr) const
{
    ptr->Destroy();
}
