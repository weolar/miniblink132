...
public:
    ...
    UniqueTrackingTokenHelper& operator=(const UniqueTrackingTokenHelper&) = delete;
    ~UniqueTrackingTokenHelper();
...
MEDIA_GPU_EXPORT scoped_refptr<VideoFrame> CreateGpuMemoryBufferVideoFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size,
...
MEDIA_GPU_EXPORT scoped_refptr<VideoFrame> CreatePlatformVideoFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
...
MEDIA_GPU_EXPORT std::optional<VideoFrameLayout> GetPlatformVideoFrameLayout(
    VideoPixelFormat pixel_format, const gfx::Size& coded_size, gfx::BufferUsage buffer_usage);

// Create a shared GPU memory handle to the |video_frame|'s data.
MEDIA_GPU_EXPORT gfx::GpuMemoryBufferHandle CreateGpuMemoryBufferHandle(const VideoFrame* video_frame);
...
// compositing/scanout.
MEDIA_GPU_EXPORT scoped_refptr<gfx::NativePixmapDmaBuf> CreateNativePixmapDmaBuf(const VideoFrame* video_frame);

// Returns either the GPU MemoryBuffer ID or the FD of the first file
// descriptor depending on the storage type.
MEDIA_GPU_EXPORT gfx::GenericSharedMemoryId GetSharedMemoryId(const VideoFrame& frame);
