...
public:
    struct PixelLayoutCandidate {
        ...
        {
            return this->fourcc == candidate.fourcc && this->size == candidate.size && this->modifier == candidate.modifier;
        }
    };
    ...
    using LegacyFrameResourceReadyCB = ImageProcessorBackend::LegacyFrameResourceReadyCB;
    ...
    using CreateBackendCB = base::RepeatingCallback<std::unique_ptr<ImageProcessorBackend>(
        const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, ErrorCB error_cb)>;

    static std::unique_ptr<ImageProcessor> Create(CreateBackendCB create_backend_cb, const PortConfig& input_config, const PortConfig& output_config,
    ...
    bool Process(scoped_refptr<FrameResource> frame, LegacyFrameResourceReadyCB cb);
    ...
    // Process() must be called on |client_task_runner_|. This should not be
    // blocking function.
    bool Process(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, FrameReadyCB cb);

    // FrameResource version of Process().
    bool Process(scoped_refptr<FrameResource> input_frame, scoped_refptr<FrameResource> output_frame, FrameResourceReadyCB cb);
...
protected:
    ...
    ImageProcessor(std::unique_ptr<ImageProcessorBackend> backend, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
        scoped_refptr<base::SequencedTaskRunner> backend_task_runner);

    // Callbacks of processing frames.
    static void OnProcessDoneThunk(scoped_refptr<base::SequencedTaskRunner> task_runner, std::optional<base::WeakPtr<ImageProcessor>> weak_this, int cb_index,
        scoped_refptr<VideoFrame> frame);
    static void OnProcessFrameResourceDoneThunk(scoped_refptr<base::SequencedTaskRunner> task_runner, std::optional<base::WeakPtr<ImageProcessor>> weak_this,
        int cb_index, scoped_refptr<FrameResource> frame);
    static void OnProcessFrameResourceLegacyDoneThunk(scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    void OnProcessFrameResourceDone(int cb_index, scoped_refptr<FrameResource> frame);
    void OnProcessFrameResourceLegacyDone(int cb_index, size_t buffer_id, scoped_refptr<FrameResource> frame);
