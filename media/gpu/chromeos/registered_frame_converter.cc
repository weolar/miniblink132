...
// static
std::unique_ptr<FrameResourceConverter> RegisteredFrameConverter::Create(scoped_refptr<FrameRegistry> registry)
{
    return base::WrapUnique<FrameResourceConverter>(new RegisteredFrameConverter(std::move(registry)));
}

RegisteredFrameConverter::RegisteredFrameConverter(scoped_refptr<FrameRegistry> registry)
...
void RegisteredFrameConverter::ConvertFrameImpl(scoped_refptr<FrameResource> frame)
{
    DVLOGF(4);
    ...
    scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapTrackingToken(
        frame->format(), frame->tracking_token(), frame->coded_size(), frame->visible_rect(), frame->natural_size(), frame->timestamp());
    if (!video_frame) {
    ...
    video_frame->AddDestructionObserver(
        base::BindOnce([](scoped_refptr<FrameRegistry> registry, const base::UnguessableToken& token) { registry->UnregisterFrame(token); }, registry_,
            frame->tracking_token()));
