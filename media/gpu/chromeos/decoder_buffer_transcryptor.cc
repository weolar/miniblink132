...
namespace media {
DecoderBufferTranscryptor::TranscryptTask::TranscryptTask(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_done_cb)
    : buffer(std::move(buffer))
    , decode_done_cb(std::move(decode_done_cb))
{
}

DecoderBufferTranscryptor::TranscryptTask::~TranscryptTask() = default;

DecoderBufferTranscryptor::TranscryptTask::TranscryptTask(TranscryptTask&&) = default;

DecoderBufferTranscryptor::DecoderBufferTranscryptor(CdmContext* cdm_context, VideoDecoderMixin& decoder, bool needs_vp9_superframe_splitting,
    OnBufferTranscryptedCB transcrypt_callback, WaitingCB waiting_callback)
    : decoder_(decoder)
    ...
    , needs_vp9_superframe_splitting_(needs_vp9_superframe_splitting)
{
    ...
    DCHECK(cdm_context);
    cdm_event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&DecoderBufferTranscryptor::OnCdmContextEvent, weak_this_));
...
void DecoderBufferTranscryptor::SecureBuffersMayBeAvailable()
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&DecoderBufferTranscryptor::DecryptPendingBuffer, weak_this_));
}

void DecoderBufferTranscryptor::EnqueueBuffer(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void DecoderBufferTranscryptor::DecryptPendingBuffer()
{
    ...
    if (needs_vp9_superframe_splitting_
        && Vp9Parser::IsSuperframe(curr_buffer->data(), base::checked_cast<off_t>(curr_buffer->size()), curr_buffer->decrypt_config())) {
        base::circular_deque<Vp9Parser::FrameInfo> frames
            = Vp9Parser::ExtractFrames(curr_buffer->data(), base::checked_cast<off_t>(curr_buffer->size()), curr_buffer->decrypt_config());
        if (frames.empty()) {
        ...
        scoped_refptr<DecoderBuffer> superframe = std::move(current_transcrypt_task_->buffer);
        ...
        current_transcrypt_task_->buffer
            = DecoderBuffer::CopyFrom(UNSAFE_TODO(base::span(frames.front().ptr.get(), base::checked_cast<size_t>(frames.front().size))));
        ...
        current_transcrypt_task_->buffer->set_is_key_frame(superframe->is_key_frame());
        if (superframe->has_side_data()) {
            current_transcrypt_task_->buffer->set_side_data(superframe->side_data()->Clone());
        }
        if (frames.front().decrypt_config) {
            current_transcrypt_task_->buffer->set_decrypt_config(std::move(frames.front().decrypt_config));
        }
        frames.pop_front();
        ...
        while (!frames.empty()) {
            ...
            scoped_refptr<DecoderBuffer> buffer
                = DecoderBuffer::CopyFrom(UNSAFE_TODO(base::span(frames.back().ptr.get(), base::checked_cast<size_t>(frames.back().size))));
            ...
            frames.pop_back();
            transcrypt_task_queue_.emplace_front(std::move(buffer), std::move(next_decode_done_cb));
            next_decode_done_cb = base::DoNothing();
        }
    }

    // If we've already attached a secure buffer, don't do it again.
    if (!curr_buffer->has_side_data() || !curr_buffer->side_data()->secure_handle) {
        auto status = decoder_->AttachSecureBuffer(current_transcrypt_task_->buffer);
        ...
        if (curr_buffer->has_side_data() && curr_buffer->side_data()->secure_handle) {
            ...
            // done.
            current_transcrypt_task_->decode_done_cb = base::BindOnce(&DecoderBufferTranscryptor::OnSecureBufferRelease, weak_this_,
                curr_buffer->side_data()->secure_handle, std::move(current_transcrypt_task_->decode_done_cb));
        }
    }
    transcrypt_pending_ = true;
    cdm_context_ref_->GetCdmContext()->GetDecryptor()->Decrypt(Decryptor::kVideo, current_transcrypt_task_->buffer,
        base::BindPostTaskToCurrentDefault(base::BindOnce(&DecoderBufferTranscryptor::OnBufferTranscrypted, weak_this_)));
}

void DecoderBufferTranscryptor::OnBufferTranscrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> transcrypted_buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    std::optional<TranscryptTask> temp_task = std::move(current_transcrypt_task_);
    current_transcrypt_task_ = std::nullopt;
    transcrypt_callback_.Run(std::move(transcrypted_buffer), std::move(temp_task->decode_done_cb));
...
void DecoderBufferTranscryptor::OnSecureBufferRelease(uint64_t secure_handle, VideoDecoder::DecodeCB decode_cb, DecoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
