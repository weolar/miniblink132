...
public:
    static std::optional<GpuBufferLayout> Create(const Fourcc& fourcc, const gfx::Size& size, const std::vector<ColorPlaneLayout>& planes, uint64_t modifier);
...
private:
    GpuBufferLayout(const Fourcc& fourcc, const gfx::Size& size, const std::vector<ColorPlaneLayout>& planes, uint64_t modifier);
...
MEDIA_GPU_EXPORT std::ostream& operator<<(std::ostream& ostream, const GpuBufferLayout& layout);
