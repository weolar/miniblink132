...
class MEDIA_GPU_EXPORT GpuMemoryBufferVideoFrameMapper : public VideoFrameMapper {
public:
    static std::unique_ptr<GpuMemoryBufferVideoFrameMapper> Create(VideoPixelFormat format);

    GpuMemoryBufferVideoFrameMapper(const GpuMemoryBufferVideoFrameMapper&) = delete;
    GpuMemoryBufferVideoFrameMapper& operator=(const GpuMemoryBufferVideoFrameMapper&) = delete;

    ~GpuMemoryBufferVideoFrameMapper() override = default;

    // VideoFrameMapper implementation.
    scoped_refptr<VideoFrame> MapFrame(scoped_refptr<const FrameResource> video_frame, int permissions) override;
