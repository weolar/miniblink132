...
CroStatus::Or<scoped_refptr<FrameResource>> CreateGpuMemoryBufferFrameResource(VideoPixelFormat format, const gfx::Size& coded_size,
    const gfx::Rect& visible_rect, const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers, bool needs_detiling, base::TimeDelta timestamp)
{
    std::optional<gfx::BufferFormat> gfx_format = VideoPixelFormatToGfxBufferFormat(format);
    DCHECK(gfx_format);
    return static_cast<scoped_refptr<FrameResource>>(VideoFrameResource::Create(VideoFrame::WrapExternalGpuMemoryBuffer(
        visible_rect, natural_size, std::make_unique<FakeGpuMemoryBuffer>(coded_size, *gfx_format, modifier), timestamp)));
}
...
public:
        ...
        , pool_(new PlatformVideoFramePool())
    {
        pool_->SetCustomFrameAllocator(
            base::BindRepeating(&CreateGpuMemoryBufferFrameResource<gfx::NativePixmapHandle::kNoModifier>), VideoFrame::STORAGE_GPU_MEMORY_BUFFER);
        pool_->set_parent_task_runner(base::SingleThreadTaskRunner::GetCurrentDefault());
    }

    bool Initialize(const Fourcc& fourcc)
    {
        constexpr gfx::Size kCodedSize(320, 240);
        return Initialize(kCodedSize, /*visible_rect=*/gfx::Rect(kCodedSize), fourcc);
    }

    bool Initialize(const gfx::Size& coded_size, const gfx::Rect& visible_rect, const Fourcc& fourcc)
    {
        constexpr size_t kNumFrames = 10;
        visible_rect_ = visible_rect;
        natural_size_ = visible_rect.size();
        auto status_or_layout = pool_->Initialize(fourcc, coded_size, visible_rect_, natural_size_, kNumFrames,
            /*use_protected=*/false,
...
class PlatformVideoFramePoolTest : public PlatformVideoFramePoolTestBase, public testing::WithParamInterface<VideoPixelFormat> { };

constexpr VideoPixelFormat kPixelFormats[] = { PIXEL_FORMAT_YV12, PIXEL_FORMAT_NV12, PIXEL_FORMAT_P010LE };
INSTANTIATE_TEST_SUITE_P(All, PlatformVideoFramePoolTest, testing::ValuesIn(kPixelFormats),
    [](const ::testing::TestParamInfo<VideoPixelFormat>& info) { return VideoPixelFormatToString(info.param); });
...
TEST_P(PlatformVideoFramePoolTest, InitializeWithDifferentFourcc)
{
    ...
    // to get drained.
    const Fourcc different_fourcc(*fourcc != Fourcc(Fourcc::NV12) ? Fourcc::NV12 : Fourcc::P010);
...
TEST_P(PlatformVideoFramePoolTest, InitializeWithDifferentUsableArea)
{
    ...
    ASSERT_NE(GetRectSizeFromOrigin(kInitialVisibleRect), GetRectSizeFromOrigin(kDifferentVisibleRect));
...
TEST_P(PlatformVideoFramePoolTest, GetOriginalFrame)
{
    ...
    EXPECT_EQ(pool_->GetOriginalFrame(frame_1->GetSharedMemoryId()), pool_->GetOriginalFrame(frame_2->GetSharedMemoryId()));
    EXPECT_EQ(frame_1->GetSharedMemoryId().id, frame_2->GetSharedMemoryId().id);

    scoped_refptr<FrameResource> frame_3 = GetFrame(20);
    EXPECT_NE(pool_->GetOriginalFrame(frame_1->GetSharedMemoryId()), pool_->GetOriginalFrame(frame_3->GetSharedMemoryId()));
    EXPECT_NE(frame_1->GetSharedMemoryId().id, frame_3->GetSharedMemoryId().id);
}

TEST_P(PlatformVideoFramePoolTest, InitializeWithSameFourccUsableAreaAndCodedSize)
{
    const auto fourcc = Fourcc::FromVideoPixelFormat(GetParam());
    ASSERT_TRUE(fourcc.has_value());
    ...
    ASSERT_EQ(GetRectSizeFromOrigin(kInitialVisibleRect), GetRectSizeFromOrigin(kDifferentVisibleRect));
    ASSERT_TRUE(Initialize(kCodedSize, kDifferentVisibleRect, fourcc.value()));
...
TEST_P(PlatformVideoFramePoolTest, InitializeFail)
{
    ...
    ASSERT_TRUE(fourcc.has_value());
    pool_->SetCustomFrameAllocator(
        base::BindRepeating([](VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
                                bool use_protected, bool use_linear_buffers, bool needs_detiling, base::TimeDelta timestamp) {
            return CroStatus::Or<scoped_refptr<FrameResource>>(CroStatus::Codes::kFailedToCreateVideoFrame);
        }),
...
TEST_P(PlatformVideoFramePoolTest, ModifierIsPassed)
{
    ...
    const auto fourcc = Fourcc::FromVideoPixelFormat(GetParam());
    ASSERT_TRUE(fourcc.has_value());
    pool_->SetCustomFrameAllocator(base::BindRepeating(&CreateGpuMemoryBufferFrameResource<kSampleModifier>), VideoFrame::STORAGE_GPU_MEMORY_BUFFER);
    ASSERT_TRUE(Initialize(fourcc.value()));
