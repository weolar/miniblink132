...
public:
    ...
    NativePixmapFrameResource& operator=(const NativePixmapFrameResource&) = delete;
    ...
    static scoped_refptr<NativePixmapFrameResource> Create(const media::VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
        std::vector<base::ScopedFD> dmabuf_fds, base::TimeDelta timestamp);

    // Uses MiniGBM to allocate a NativePixmapFrameResource.
    static scoped_refptr<NativePixmapFrameResource> Create(media::VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    ...
    static scoped_refptr<NativePixmapFrameResource> Create(const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp,
        gfx::BufferUsage buffer_usage, scoped_refptr<const gfx::NativePixmapDmaBuf> pixmap);
    ...
    scoped_refptr<const gfx::NativePixmapDmaBuf> GetNativePixmapDmaBuf() const override;
    ...
    std::unique_ptr<VideoFrame::ScopedMapping> MapGMBOrSharedImage() const override;
    ...
    void set_hdr_metadata(const std::optional<gfx::HDRMetadata>& hdr_metadata) override;
    base::TimeDelta timestamp() const override;
    void set_timestamp(base::TimeDelta timestamp) override;
    void AddDestructionObserver(base::OnceClosure callback) override;
    scoped_refptr<FrameResource> CreateWrappingFrame(const gfx::Rect& visible_rect, const gfx::Size& natural_size) override;
    std::string AsHumanReadableString() const override;
    // Always returns empty handle.
    gfx::GpuMemoryBufferHandle GetGpuMemoryBufferHandleForTesting() const override;
...
private:
    ...
    NativePixmapFrameResource(const media::VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp,
        gfx::BufferFormat buffer_format, gfx::GenericSharedMemoryId id, const base::UnguessableToken& token, std::optional<gfx::BufferUsage> buffer_usage,
        gfx::NativePixmapHandle handle);

    NativePixmapFrameResource(const media::VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp,
        gfx::GenericSharedMemoryId id, const base::UnguessableToken& token, std::optional<gfx::BufferUsage> buffer_usage,
