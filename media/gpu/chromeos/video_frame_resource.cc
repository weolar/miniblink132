...
scoped_refptr<VideoFrameResource> VideoFrameResource::Create(scoped_refptr<VideoFrame> frame)
{
    if (!frame) {
        return nullptr;
    }
    ...
    // constructor.
    return base::WrapRefCounted<VideoFrameResource>(new VideoFrameResource(std::move(frame)));
}

scoped_refptr<const VideoFrameResource> VideoFrameResource::CreateConst(scoped_refptr<const VideoFrame> frame)
{
    if (!frame) {
        return nullptr;
    }
    // Uses WrapRefCounted since MakeRefCounted cannot access a private
    // constructor.
    return base::WrapRefCounted<const VideoFrameResource>(new VideoFrameResource(std::move(frame)));
}
...
scoped_refptr<const gfx::NativePixmapDmaBuf> VideoFrameResource::GetNativePixmapDmaBuf() const
{
    return media::CreateNativePixmapDmaBuf(frame_.get());
}

gfx::GpuMemoryBufferHandle VideoFrameResource::CreateGpuMemoryBufferHandle() const
{
    return media::CreateGpuMemoryBufferHandle(frame_.get());
}

std::unique_ptr<VideoFrame::ScopedMapping> VideoFrameResource::MapGMBOrSharedImage() const
{
...
const std::optional<gfx::HDRMetadata>& VideoFrameResource::hdr_metadata() const
{
    return frame_->hdr_metadata();
}

void VideoFrameResource::set_hdr_metadata(const std::optional<gfx::HDRMetadata>& hdr_metadata)
{
...
scoped_refptr<FrameResource> VideoFrameResource::CreateWrappingFrame(const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    auto wrapping_frame = Create(VideoFrame::WrapVideoFrame(GetMutableVideoFrame(), format(), visible_rect, natural_size));
    if (!wrapping_frame) {
        return nullptr;
    }
    ...
    wrapping_frame->AddDestructionObserver(base::DoNothingWithBoundArgs(base::WrapRefCounted<VideoFrameResource>(this)));
...
gfx::GpuMemoryBufferHandle VideoFrameResource::GetGpuMemoryBufferHandleForTesting() const
{
