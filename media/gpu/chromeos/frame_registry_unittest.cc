...
scoped_refptr<FrameResource> GenerateFrame(base::TimeDelta timestamp)
{
    ...
    auto video_frame = VideoFrame::CreateFrame(kPixelFormat, kCodedSize, kVisibleRect, kNaturalSize, timestamp);
...
public:
    FrameRegistryTest()
        : registry_(base::MakeRefCounted<FrameRegistry>())
    {
    }
...
TEST_F(FrameRegistryTest, RegisterAccessUnregister)
{
    ...
    scoped_refptr<const FrameResource> retrieved_frame = registry_->AccessFrame(token);
    ...
    scoped_refptr<const FrameResource> retrieved_frame_two = registry_->AccessFrame(token);
    ...
    // at least two.
    EXPECT_TRUE(retrieved_frame->HasAtLeastOneRef() && !retrieved_frame->HasOneRef());
