...
public:
    ...
    using GetOriginalFrameCB = base::RepeatingCallback<FrameResource*(gfx::GenericSharedMemoryId frame_id)>;
    ...
    // nullptr).
    void Initialize(scoped_refptr<base::SequencedTaskRunner> parent_task_runner, OutputCB output_cb);
...
template <> struct MEDIA_GPU_EXPORT default_delete<media::FrameResourceConverter> {
    constexpr default_delete() = default;

    template <typename U, typename = typename std::enable_if<std::is_convertible<U*, media::FrameResourceConverter*>::value>::type>
