...
class MEDIA_GPU_EXPORT LibYUVImageProcessorBackend : public ImageProcessorBackend {
public:
    ...
    // ImageProcessor.
    static std::unique_ptr<ImageProcessorBackend> CreateWithTaskRunner(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode,
    ...
    LibYUVImageProcessorBackend& operator=(const LibYUVImageProcessorBackend&) = delete;
...
private:
    LibYUVImageProcessorBackend(std::unique_ptr<VideoFrameMapper> input_frame_mapper, std::unique_ptr<VideoFrameMapper> output_frame_mapper,
        scoped_refptr<FrameResource> intermediate_frame, scoped_refptr<FrameResource> crop_intermediate_frame, const PortConfig& input_config,
        const PortConfig& output_config, OutputMode output_mode, ErrorCB error_cb, scoped_refptr<base::SequencedTaskRunner> backend_task_runner);
    ~LibYUVImageProcessorBackend() override;

    void NotifyError();

    // Execute Libyuv function for the conversion from |input| to |output|.
    int DoConversion(const FrameResource* const input, FrameResource* const output);
