...
bool HardwareVideoEncodingPreSandboxHook(sandbox::policy::SandboxLinux::Options options)
{
    ...
    if (options.use_amd_specific_policies) {
        ...
        for (const char* item : kDevices) {
            ...
            permissions.push_back(BrokerFilePermission::StatOnlyWithIntermediateDirs(path));
    ...
    for (int i = 128; i <= 137; ++i) {
        ...
        if (stat(path.c_str(), &st) == 0) {
            permissions.push_back(options.use_amd_specific_policies ? BrokerFilePermission::ReadWrite(path) : BrokerFilePermission::ReadOnly(path));
