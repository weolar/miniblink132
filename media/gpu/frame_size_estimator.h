...
public:
    FrameSizeEstimator(base::TimeDelta max_window_size, float initial_qp_size, float initial_size_correction);
    ~FrameSizeEstimator();
    ...
    float qp_size_mean() const
    {
        return qp_size_stats_.mean();
    }
    float size_correction_mean() const
    {
        return size_correction_stats_.mean();
    }
    ...
    void Update(size_t frame_bytes, uint32_t qp, uint32_t qp_prev, base::TimeDelta elapsed_time);
