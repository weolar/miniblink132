...
public:
    ...
    void SetUp() override
    {
        ...
        float qp_size_init = h264_rate_control_util::QP2QStepSize(24) * bytes_per_frame_avg;
        float size_correction_init = 0.3f * bytes_per_frame_avg;

        frame_size_estimator_ = std::make_unique<FrameSizeEstimator>(base::Milliseconds(300), qp_size_init, size_correction_init);
...
protected:
    ...
    int RunTestSequence(uint32_t avg_bitrate, int fps, int frame_count, int start_frame_index)
    {
        ...
        for (int i = 0; i < frame_count; ++i) {
            ...
            size_t encoded_size = 0.625 * 16 * common_frame_size / h264_rate_control_util::QP2QStepSize(qp);
...
TEST_F(FrameSizeEstimatorTest, RunBasicFrameSizeEstimatorTest)
{
    ...
    int last_frame_index = RunTestSequence(kCommonAvgBitrate, kCommonFps, 30, start_frame_index);
    ...
    EXPECT_LT(kExpectedSizeCorrectionMeanMin1, frame_size_estimator_->size_correction_mean());
    EXPECT_GT(kExpectedSizeCorrectionMeanMax1, frame_size_estimator_->size_correction_mean());
    ...
    EXPECT_LT(kExpectedSizeCorrectionMeanMin2, frame_size_estimator_->size_correction_mean());
    EXPECT_GT(kExpectedSizeCorrectionMeanMax2, frame_size_estimator_->size_correction_mean());
}
...
{
    constexpr size_t kEstimatedFrameSizeValues[] = { 4556, 3874, 2663, 5259, 4916, 5540, 2352, 3095, 2655, 5250 };
    constexpr uint32_t kQpValues[] = { 22, 24, 26, 28, 26, 24 };
