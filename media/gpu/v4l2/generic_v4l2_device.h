...
public:
    ...
    void* Mmap(void* addr, unsigned int len, int prot, int flags, unsigned int offset) override;
    void Munmap(void* addr, unsigned int len) override;

    std::vector<base::ScopedFD> GetDmabufsForV4L2Buffer(int index, size_t num_planes, enum v4l2_buf_type buf_type) override;

    bool CanCreateEGLImageFrom(const Fourcc fourcc) const override;
    EGLImageKHR CreateEGLImage(EGLDisplay egl_display, EGLContext egl_context, GLuint texture_id, const gfx::Size& size, unsigned int buffer_index,
        const Fourcc fourcc, gfx::NativePixmapHandle handle) const override;

    scoped_refptr<gl::GLImage> CreateGLImage(const gfx::Size& size, const Fourcc fourcc, gfx::NativePixmapHandle handle) const override;

    EGLBoolean DestroyEGLImage(EGLDisplay egl_display, EGLImageKHR egl_image) const override;
    GLenum GetTextureTarget() const override;
    std::vector<uint32_t> PreferredInputFormat(Type type) const override;

    std::vector<uint32_t> GetSupportedImageProcessorPixelformats(v4l2_buf_type buf_type) override;

    VideoDecodeAccelerator::SupportedProfiles GetSupportedDecodeProfiles(const size_t num_formats, const uint32_t pixelformats[]) override;

    VideoEncodeAccelerator::SupportedProfiles GetSupportedEncodeProfiles() override;
