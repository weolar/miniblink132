...
constexpr std::array<const char*, static_cast<size_t>(V4l2VideoDecoderFunctions::kMaxValue) + 1> kV4l2VideoDecoderFunctionNames
    = { "InitializeBackend", "SetupInputFormat", "SetupOutputFormat", "StartStreamV4L2Queue", "StopStreamV4L2Queue", "Other V4l2VideoDecoder functions" };
...
void LogAndRecordUMA(const base::Location& location, V4l2VideoDecoderFunctions function, const std::string& message = "")
{
    LOG(ERROR) << V4l2VideoDecoderFunctionName(function) << " failed at " << location.ToString() << (message.empty() ? " " : " : ") << message;
    ...
    std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
{
    ...
    return base::WrapUnique<VideoDecoderMixin>(new V4L2VideoDecoder(std::move(media_log), std::move(decoder_task_runner), std::move(client), new V4L2Device()));
}

// static
std::optional<SupportedVideoDecoderConfigs> V4L2VideoDecoder::GetSupportedConfigs()
{
...
V4L2VideoDecoder::V4L2VideoDecoder(std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner,
    base::WeakPtr<VideoDecoderMixin::Client> client, scoped_refptr<V4L2Device> device)
    : VideoDecoderMixin(std::move(media_log), std::move(decoder_task_runner), std::move(client))
...
void V4L2VideoDecoder::Initialize(const VideoDecoderConfig& config, bool /*low_delay*/, CdmContext* cdm_context, InitCB init_cb,
    const PipelineOutputCB& output_cb, const WaitingCB& /*waiting_cb*/)
{
    ...
    case State::kError:
        VLOGF(1) << "V4L2 decoder should not be initialized at state: " << static_cast<int>(state_);
    ...
    if (config.is_encrypted()) {
        ...
        if (config.codec() != VideoCodec::kH264 && config.codec() != VideoCodec::kVP9 && config.codec() != VideoCodec::kAV1
            && config.codec() != VideoCodec::kHEVC) {
            VLOGF(1) << GetCodecName(config.codec()) << " is not supported for encrypted content";
            std::move(init_cb).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
            return;
        }
    ...
    if (state_ == State::kDecoding) {
        if (!StopStreamV4L2Queue(true)) {
            ...
            std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kNotInitialized).AddCause(V4L2Status(V4L2Status::Codes::kFailedToStopStreamQueue)));
            return;
        }
    }
    ...
    if (state_ == State::kDecoding || state_ == State::kInitialized) {
        if (!input_queue_->DeallocateBuffers() || !output_queue_->DeallocateBuffers()) {
            VLOGF(1) << "Failed to deallocate V4L2 buffers";
            std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kNotInitialized).AddCause(V4L2Status(V4L2Status::Codes::kFailedToDestroyQueueBuffers)));
            return;
        }
        input_queue_ = nullptr;
        output_queue_ = nullptr;
    ...
    if (config.is_encrypted()) {
        device_->set_secure_allocate_cb(base::BindRepeating(&V4L2VideoDecoder::AllocateSecureBuffer, weak_this_for_callbacks_.GetWeakPtr()));
    } else {
    ...
    if (profile_ == VIDEO_CODEC_PROFILE_UNKNOWN) {
        ...
        std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kNotInitialized).AddCause(V4L2Status(V4L2Status::Codes::kNoProfile)));
        return;
    }
    if (VideoCodecProfileToVideoCodec(profile_) == VideoCodec::kAV1 && !base::FeatureList::IsEnabled(kChromeOSHWAV1Decoder)) {
    ...
    if (status != V4L2Status::Codes::kOk) {
        ...
        SetState(State::kError);
        std::move(init_cb).Run(DecoderStatus(DecoderStatus::Codes::kNotInitialized).AddCause(std::move(status)));
        return;
    }
...
V4L2Status V4L2VideoDecoder::InitializeBackend()
{
    ...
    can_use_decoder_ = num_instances_.Increment() < kMaxNumOfInstances || !base::FeatureList::IsEnabled(media::kLimitConcurrentDecoderInstances);
    if (!can_use_decoder_) {
        VLOGF(1) << "Reached maximum number of decoder instances (" << kMaxNumOfInstances << ")";
    ...
    for (const auto api : { kStateful, kStateless }) {
        ...
        if (fourcc == V4L2_PIX_FMT_INVALID || !device_->Open(V4L2Device::Type::kDecoder, fourcc)) {
            continue;
        }
    ...
    if (device_->Ioctl(VIDIOC_QUERYCAP, &caps) || (caps.capabilities & kCapsRequired) != kCapsRequired) {
    ...
    if (preferred_api_and_format.first == kStateful) {
        VLOGF(1) << "Using a stateful API for profile: " << GetProfileName(profile_) << " and fourcc: " << FourccToString(input_format_fourcc_);
        backend_ = std::make_unique<V4L2StatefulVideoDecoderBackend>(this, device_, profile_, color_space_, decoder_task_runner_);
    } else {
        DCHECK_EQ(preferred_api_and_format.first, kStateless);
        VLOGF(1) << "Using a stateless API for profile: " << GetProfileName(profile_) << " and fourcc: " << FourccToString(input_format_fourcc_);
        backend_ = std::make_unique<V4L2StatelessVideoDecoderBackend>(
            this, device_, profile_, color_space_, decoder_task_runner_, cdm_context_ref_ ? cdm_context_ref_->GetCdmContext() : nullptr);
    }
...
void V4L2VideoDecoder::AllocateSecureBuffer(uint32_t size, SecureBufferAllocatedCB callback)
{
#if BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    cdm_context_ref_->GetCdmContext()->GetChromeOsCdmContext()->AllocateSecureBuffer(size,
        mojo::WrapCallbackWithDefaultInvokeIfNotRun(base::BindPostTaskToCurrentDefault(base::BindOnce(
                                                        &V4L2VideoDecoder::AllocateSecureBufferCB, weak_this_for_callbacks_.GetWeakPtr(), std::move(callback))),
            mojo::PlatformHandle()));
#else
    NOTREACHED();
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
...
void V4L2VideoDecoder::AllocateSecureBufferCB(SecureBufferAllocatedCB callback, mojo::PlatformHandle mojo_fd)
{
    ...
    if (!mojo_fd.is_valid()) {
        ...
        SetState(State::kError);
        std::move(pending_init_cb_).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
        return;
    }
    ...
    if (device_->Ioctl(VIDIOC_S_EXT_CTRLS, &ctrls)) {
        ...
        PLOG(ERROR) << "Failed getting secure buffer identifier for FD " << secure_fd.get();
        SetState(State::kError);
        std::move(pending_init_cb_).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
        return;
    }
...
bool V4L2VideoDecoder::SetupInputFormat()
{
    ...
    const auto v4l2_codecs_as_pix_fmts = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, device_), V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
    if (!base::Contains(v4l2_codecs_as_pix_fmts, input_format_fourcc_)) {
        DVLOGF(1) << FourccToString(input_format_fourcc_) << " not recognised, skipping...";
        return false;
    }
    VLOGF(1) << "Input (OUTPUT queue) Fourcc: " << FourccToString(input_format_fourcc_);
    ...
    if (backend_->StopInputQueueOnResChange()) {
        ...
        GetSupportedResolution(base::BindRepeating(&V4L2Device::Ioctl, device_), input_format_fourcc_, &min_size, &max_size);
        input_size = GetInputBufferSizeForResolution(max_size);
    }

    // Setup the input format.
    auto format = input_queue_->SetFormat(input_format_fourcc_, gfx::Size(), input_size);
    if (!format) {
        VPLOGF(1) << "Failed to call IOCTL to set input format.";
        return false;
    }
    DCHECK_EQ(format->fmt.pix_mp.pixelformat, input_format_fourcc_)
        << "The input (OUTPUT) queue must accept the requested pixel format " << FourccToString(input_format_fourcc_)
        << ", but it returned instead: " << FourccToString(format->fmt.pix_mp.pixelformat);
...
{
    const size_t num_OUTPUT_buffers = backend_->GetNumOUTPUTQueueBuffers(!!cdm_context_ref_);
    // Secure playback uses dmabufs for the OUTPUT queue, otherwise we use mmap
    // buffers.
    v4l2_memory input_queue_memory = !!cdm_context_ref_ ? V4L2_MEMORY_DMABUF : V4L2_MEMORY_MMAP;
    VLOGF(1) << "Requesting: " << num_OUTPUT_buffers << " OUTPUT buffers of type "
             << (input_queue_memory == V4L2_MEMORY_MMAP ? "V4L2_MEMORY_MMAP" : "V4L2_MEMORY_DMABUF");
    return input_queue_->AllocateBuffers(num_OUTPUT_buffers, input_queue_memory, incoherent_) != 0;
}

CroStatus V4L2VideoDecoder::SetupOutputFormat(const gfx::Size& size, const gfx::Rect& visible_rect, size_t num_codec_reference_frames, uint8_t bit_depth)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3) << "size: " << size.ToString() << ", visible_rect: " << visible_rect.ToString();
    ...
    const auto v4l2_pix_fmts = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, device_), V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    ...
    for (const uint32_t& pixfmt : v4l2_pix_fmts) {
        ...
        if (candidate_bit_depth != bit_depth) {
            DVLOGF(1) << "Enumerated format " << candidate->ToString() << " with a bit depth of " << candidate_bit_depth
                      << " removed from consideration because it does not match"
                      << " the bit depth returned by the backend of " << base::strict_cast<size_t>(bit_depth);
            continue;
        }

        std::optional<struct v4l2_format> format = output_queue_->TryFormat(pixfmt, size, 0);
        if (!format)
            continue;

        gfx::Size adjusted_size(format->fmt.pix_mp.width, format->fmt.pix_mp.height);

        candidates.emplace_back(PixelLayoutCandidate { .fourcc = *candidate, .size = adjusted_size });
    }

    // Ask the pipeline to pick the output format.
    CroStatus::Or<PixelLayoutCandidate> status_or_output_format
        = client_->PickDecoderOutputFormat(candidates, visible_rect, aspect_ratio_.GetNaturalSize(visible_rect),
            /*output_size=*/std::nullopt, num_codec_reference_frames,
            /*use_protected=*/!!cdm_context_ref_, /*need_aux_frame_pool=*/false, std::nullopt);
    ...
    const PixelLayoutCandidate output_format = std::move(status_or_output_format).value();
    ...
    std::optional<struct v4l2_format> format = output_queue_->SetFormat(fourcc.ToV4L2PixFmt(), picked_size, 0);
    DCHECK(format);
    gfx::Size adjusted_size(format->fmt.pix_mp.width, format->fmt.pix_mp.height);
    if (!gfx::Rect(adjusted_size).Contains(gfx::Rect(picked_size))) {
        VLOGF(1) << "The adjusted coded size (" << adjusted_size.ToString() << ") should contains the original coded size(" << picked_size.ToString() << ").";
    ...
    if (pool) {
        ...
        if (layout->size() != adjusted_size) {
            VLOGF(1) << "The size adjusted by VFPool is different from one "
                     << "adjusted by a video driver. fourcc: " << fourcc.ToString() << ", (video driver v.s. VFPool) " << adjusted_size.ToString()
                     << " != " << layout->size().ToString();
            return CroStatus::Codes::kFailedToChangeResolution;
        }

        VLOGF(1) << "buffer modifier: " << std::hex << layout->modifier();
        if (layout->modifier() != DRM_FORMAT_MOD_LINEAR && layout->modifier() != gfx::NativePixmapHandle::kNoModifier) {
            std::optional<struct v4l2_format> modifier_format = output_queue_->SetModifierFormat(layout->modifier(), picked_size);
            if (!modifier_format)
                return CroStatus::Codes::kFailedToChangeResolution;

            gfx::Size size_for_modifier_format(format->fmt.pix_mp.width, format->fmt.pix_mp.height);
            if (size_for_modifier_format != adjusted_size) {
                VLOGF(1) << "Buffers were allocated for " << adjusted_size.ToString() << " but modifier format is expecting buffers to be allocated for "
...
CroStatus V4L2VideoDecoder::SetExtCtrls10Bit(const gfx::Size& size)
{
    ...
    if (input_format_fourcc_ == V4L2_PIX_FMT_HEVC_SLICE) {
        ...
        v4l2_vp9_frame.flags = V4L2_VP9_FRAME_FLAG_X_SUBSAMPLING | V4L2_VP9_FRAME_FLAG_Y_SUBSAMPLING;
...
void V4L2VideoDecoder::Reset(base::OnceClosure closure)
{
    ...
    auto trampoline_reset_cb
        = base::BindOnce(&base::SequencedTaskRunner::PostTask, base::SequencedTaskRunner::GetCurrentDefault(), FROM_HERE, std::move(closure));
...
void V4L2VideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DCHECK_NE(state_, State::kUninitialized);

    // VideoDecoder interface: |decode_cb| can't be called from within Decode().
    auto trampoline_decode_cb
        = base::BindOnce([](const scoped_refptr<base::SequencedTaskRunner>& this_sequence_runner, DecodeCB decode_cb,
                             DecoderStatus status) { this_sequence_runner->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), status)); },
    ...
    if (state_ == State::kInitialized) {
        ...
        if (!StartStreamV4L2Queue(false)) {
            LogAndRecordUMA(FROM_HERE, V4l2VideoDecoderFunctions::kStartStreamV4L2Queue);
            SetState(State::kError);
            std::move(trampoline_decode_cb)
                .Run(DecoderStatus(DecoderStatus::Codes::kFailed).AddCause(V4L2Status(V4L2Status::Codes::kFailedToStartStreamQueue)));
            return;
        }
        SetState(State::kDecoding);
    }

    backend_->EnqueueDecodeTask(std::move(buffer), std::move(trampoline_decode_cb));
}
...
bool V4L2VideoDecoder::StartStreamV4L2Queue(bool start_output_queue)
{
    ...
    if (!input_queue_->Streamon() || (start_output_queue && !output_queue_->Streamon())) {
        VLOGF(1) << "Failed to streamon V4L2 queue.";
        SetState(State::kError);
        return false;
    }

    if (!device_->StartPolling(base::BindRepeating(&V4L2VideoDecoder::ServiceDeviceTask, weak_this_for_polling_),
            base::BindRepeating(&V4L2VideoDecoder::SetState, weak_this_for_polling_, State::kError))) {
        SetState(State::kError);
        return false;
    }
...
void V4L2VideoDecoder::CompleteFlush()
{
    ...
    if (state_ != State::kFlushing) {
        VLOGF(1) << "Completed flush in the wrong state: " << static_cast<int>(state_);
        SetState(State::kError);
    } else {
        SetState(State::kDecoding);
    }
...
void V4L2VideoDecoder::ChangeResolution(gfx::Size pic_size, gfx::Rect visible_rect, size_t num_codec_reference_frames, uint8_t bit_depth)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3);
    ...
    continue_change_resolution_cb_
        = base::BindOnce(&V4L2VideoDecoder::ContinueChangeResolution, base::Unretained(this), pic_size, visible_rect, num_codec_reference_frames, bit_depth)
              .Then(base::BindOnce(&V4L2VideoDecoder::OnChangeResolutionDone, base::Unretained(this)));
...
CroStatus V4L2VideoDecoder::AttachSecureBuffer(scoped_refptr<DecoderBuffer>& buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    if (!cdm_context_ref_) {
        return CroStatus::Codes::kOk;
    ...
    buffer->WritableSideData().secure_handle = std::move(secure_handle_or_error).value();
    return CroStatus::Codes::kOk;
}
...
CroStatus V4L2VideoDecoder::ContinueChangeResolution(
    const gfx::Size& pic_size, const gfx::Rect& visible_rect, const size_t num_codec_reference_frames, const uint8_t bit_depth)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3);
    ...
    if (backend_->StopInputQueueOnResChange()) {
        size_t curr_buffer_size = input_queue_->GetMemoryUsage() / input_queue_->AllocatedBuffersCount();
        if (curr_buffer_size != GetInputBufferSizeForResolution(current_resolution_)) {
            if (!input_queue_->DeallocateBuffers()) {
                SetState(State::kError);
                return CroStatus::Codes::kFailedToChangeResolution;
            }
    ...
    const CroStatus status = SetupOutputFormat(pic_size, visible_rect, num_codec_reference_frames, bit_depth);
    if (status == CroStatus::Codes::kResetRequired) {
        LogAndRecordUMA(FROM_HERE, V4l2VideoDecoderFunctions::kSetupOutputFormat, "SetupOutputFormat is aborted");

        return CroStatus::Codes::kResetRequired;
    }
    if (status != CroStatus::Codes::kOk) {
        LogAndRecordUMA(FROM_HERE, V4l2VideoDecoderFunctions::kSetupOutputFormat,
            "Failed to setup output format, status= " + base::NumberToString(static_cast<int>(status.code())));
    ...
    const v4l2_memory type = use_v4l2_allocated_buffers ? V4L2_MEMORY_MMAP : V4L2_MEMORY_DMABUF;
    ...
    VLOGF(1) << "Requesting: " << v4l2_num_buffers << " CAPTURE buffers of type " << (use_v4l2_allocated_buffers ? "V4L2_MEMORY_MMAP" : "V4L2_MEMORY_DMABUF");

    const auto allocated_buffers = output_queue_->AllocateBuffers(v4l2_num_buffers, type, incoherent_);

    if (allocated_buffers < v4l2_num_buffers) {
        LOGF(ERROR) << "Failed to allocated enough CAPTURE buffers, requested: " << v4l2_num_buffers << " and got: " << allocated_buffers;
        SetState(State::kError);
        return CroStatus::Codes::kFailedToChangeResolution;
    }
...
void V4L2VideoDecoder::ServiceDeviceTask(bool event)
{
    ...
    if (input_queue_ && output_queue_) {
        DVLOGF(3) << "Number of queued input buffers: " << input_queue_->QueuedBuffersCount()
                  << ", Number of queued output buffers: " << output_queue_->QueuedBuffersCount();
        TRACE_COUNTER_ID2("media,gpu", "V4L2 queue sizes", this, "input (OUTPUT_queue)", input_queue_->QueuedBuffersCount(), "output (CAPTURE_queue)",
...
void V4L2VideoDecoder::OutputFrame(
    scoped_refptr<FrameResource> frame, const gfx::Rect& visible_rect, const VideoColorSpace& color_space, base::TimeDelta timestamp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    if (frame->visible_rect() != visible_rect || frame->timestamp() != timestamp) {
        gfx::Size natural_size = aspect_ratio_.GetNaturalSize(visible_rect);
        scoped_refptr<FrameResource> wrapped_frame = frame->CreateWrappingFrame(visible_rect, natural_size);
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DVLOGF(3) << "Change state from " << static_cast<int>(state_) << " to " << static_cast<int>(new_state);
