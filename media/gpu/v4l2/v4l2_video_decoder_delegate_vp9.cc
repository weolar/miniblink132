...
scoped_refptr<V4L2DecodeSurface> VP9PictureToV4L2DecodeSurface(VP9Picture* pic)
{
...
{
#define SET_FLAG_IF(cond, flag) v4l2_lf->flags |= ((vp9_lf_params.cond) ? (flag) : 0)
...
{
#define SET_FLAG_IF(cond, flag) v4l2_seg->flags |= ((vp9_seg_params.cond) ? (flag) : 0)
    ...
    SET_FLAG_IF(abs_or_delta_update, V4L2_VP9_SEGMENTATION_FLAG_ABS_OR_DELTA_UPDATE);
#undef SET_FLAG_IF
    ...
    constexpr size_t kV4L2VP9SegmentationFeaturesLength = std::extent<decltype(v4l2_seg->feature_enabled), 0>::value;
...
void FillV4L2VP9MvProbsParams(const Vp9FrameContext& vp9_ctx, struct v4l2_vp9_mv_probs* v4l2_mv_probs)
{
...
void FillV4L2VP9ProbsParams(const Vp9FrameContext& vp9_ctx, struct v4l2_ctrl_vp9_compressed_hdr* v4l2_probs)
{
...
V4L2VideoDecoderDelegateVP9::V4L2VideoDecoderDelegateVP9(V4L2DecodeSurfaceHandler* surface_handler, V4L2Device* device)
    : surface_handler_(surface_handler)
    , device_(device)
    , supports_compressed_header_(device->IsCtrlExposed(V4L2_CID_STATELESS_VP9_COMPRESSED_HDR))
{
    VLOGF(1);
...
scoped_refptr<VP9Picture> V4L2VideoDecoderDelegateVP9::CreateVP9PictureSecure(uint64_t secure_handle)
{
...
DecodeStatus V4L2VideoDecoderDelegateVP9::SubmitDecode(
    scoped_refptr<VP9Picture> pic, const Vp9SegmentationParams& segm_params, const Vp9LoopFilterParams& lf_params, const Vp9ReferenceFrameVector& ref_frames)
{
...
#define SET_FLAG_IF(cond, flag) v4l2_frame_params.flags |= ((frame_hdr->cond) ? (flag) : 0)

    SET_FLAG_IF(frame_type == Vp9FrameHeader::KEYFRAME, V4L2_VP9_FRAME_FLAG_KEY_FRAME);
    ...
    SET_FLAG_IF(frame_parallel_decoding_mode, V4L2_VP9_FRAME_FLAG_PARALLEL_DEC_MODE);
...
#undef SET_FLAG_IF
    ...
    std::vector<struct v4l2_ext_control> ext_ctrls = {
        { .id = V4L2_CID_STATELESS_VP9_FRAME, .size = sizeof(v4l2_frame_params), .ptr = &v4l2_frame_params },
    };
    ...
    if (supports_compressed_header_) {
        ...
        ext_ctrls.push_back({ .id = V4L2_CID_STATELESS_VP9_COMPRESSED_HDR, .size = sizeof(v4l2_compressed_hdr_probs), .ptr = &v4l2_compressed_hdr_probs });
    }

    const __u32 ext_ctrls_size = base::checked_cast<__u32>(ext_ctrls.size());
    struct v4l2_ext_controls ctrls = { .count = ext_ctrls_size, .controls = ext_ctrls.data() };
    ...
    if (!surface_handler_->SubmitSlice(dec_surface.get(), dec_surface->secure_handle() ? nullptr : frame_hdr->data.data(), frame_hdr->data.size())) {
        return DecodeStatus::kFail;
    }
