...
void AppendSideData(DecoderBuffer& decoder_buffer, const std::vector<uint32_t>& frame_sizes)
{
    const uint8_t* side_data = reinterpret_cast<const uint8_t*>(frame_sizes.data());
    size_t side_data_size = frame_sizes.size() * sizeof(uint32_t) / sizeof(uint8_t);
...
TEST(V4L2StatefulWorkaroundTest, CheckSuperFrameIndexSize)
{
    ...
    for (size_t mask = 1; mask < (1 << kNumFrames) - 1; mask++) {
        ...
        auto decoder_buffer = DecoderBuffer::CopyFrom(tmp_buffer.data(), tmp_buffer.size());
        ...
        EXPECT_EQ(superframe_index_size, 2 + expected_bytes_per_framesize * frame_sizes.size());
    }
}
...
TEST(V4L2StatefulWorkaroundTest, ParseAppendedSuperFrameIndex)
{
    ...
    ASSERT_TRUE(ivf_parser.Initialize(stream->data(), stream->length(), &ivf_file_header));
    ...
    for (size_t i = 0; i < kNumBuffers; ++i) {
        ...
        auto decoder_buffer = DecoderBuffer::CopyFrom(merged_buffer.data(), merged_buffer.size());
        AppendSideData(*decoder_buffer, frame_sizes);
        ...
        for (size_t j = 0; j <= i; j++) {
            ...
            EXPECT_EQ(vp9_parser.ParseNextFrame(&frame_header, &allocate_size, &frame_decrypt_config), Vp9Parser::Result::kOk);
