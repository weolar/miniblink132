...
void FillV4L2SegmentationHeader(const Vp8SegmentationHeader& vp8_sgmnt_hdr, struct v4l2_vp8_sgmnt_hdr* v4l2_sgmnt_hdr)
{
#define SET_V4L2_SGMNT_HDR_FLAG_IF(cond, flag) v4l2_sgmnt_hdr->flags |= ((vp8_sgmnt_hdr.cond) ? (flag) : 0)
    SET_V4L2_SGMNT_HDR_FLAG_IF(segmentation_enabled, V4L2_VP8_SEGMNT_HDR_FLAG_ENABLED);
    SET_V4L2_SGMNT_HDR_FLAG_IF(update_mb_segmentation_map, V4L2_VP8_SEGMNT_HDR_FLAG_UPDATE_MAP);
    SET_V4L2_SGMNT_HDR_FLAG_IF(update_segment_feature_data, V4L2_VP8_SEGMNT_HDR_FLAG_UPDATE_FEATURE_DATA);
...
void FillV4L2LoopfilterHeader(const Vp8LoopFilterHeader& vp8_loopfilter_hdr, struct v4l2_vp8_loopfilter_hdr* v4l2_lf_hdr)
{
#define SET_V4L2_LF_HDR_FLAG_IF(cond, flag) v4l2_lf_hdr->flags |= ((vp8_loopfilter_hdr.cond) ? (flag) : 0)
    ...
    SET_V4L2_LF_HDR_FLAG_IF(mode_ref_lf_delta_update, V4L2_VP8_LF_HDR_DELTA_UPDATE);
...
#undef LF_HDR_TO_V4L2_LF_HDR
    ...
    SafeArrayMemcpy(v4l2_lf_hdr->ref_frm_delta_magnitude, vp8_loopfilter_hdr.ref_frame_delta);
    SafeArrayMemcpy(v4l2_lf_hdr->mb_mode_delta_magnitude, vp8_loopfilter_hdr.mb_mode_delta);
}

void FillV4L2QuantizationHeader(const Vp8QuantizationHeader& vp8_quant_hdr, struct v4l2_vp8_quantization_hdr* v4l2_quant_hdr)
{
...
void FillV4L2Vp8EntropyHeader(const Vp8EntropyHeader& vp8_entropy_hdr, struct v4l2_vp8_entropy_hdr* v4l2_entropy_hdr)
{
...
V4L2VideoDecoderDelegateVP8Legacy::V4L2VideoDecoderDelegateVP8Legacy(V4L2DecodeSurfaceHandler* surface_handler, V4L2Device* device)
...
scoped_refptr<VP8Picture> V4L2VideoDecoderDelegateVP8Legacy::CreateVP8Picture()
{
...
bool V4L2VideoDecoderDelegateVP8Legacy::SubmitDecode(scoped_refptr<VP8Picture> pic, const Vp8ReferenceFrameVector& reference_frames)
{
...
#define SET_V4L2_FRM_HDR_FLAG_IF(cond, flag) v4l2_frame_hdr.flags |= ((frame_hdr->cond) ? (flag) : 0)
    SET_V4L2_FRM_HDR_FLAG_IF(is_experimental, V4L2_VP8_FRAME_HDR_FLAG_EXPERIMENTAL);
    SET_V4L2_FRM_HDR_FLAG_IF(show_frame, V4L2_VP8_FRAME_HDR_FLAG_SHOW_FRAME);
    SET_V4L2_FRM_HDR_FLAG_IF(mb_no_skip_coeff, V4L2_VP8_FRAME_HDR_FLAG_MB_NO_SKIP_COEFF);
#undef SET_V4L2_FRM_HDR_FLAG_IF

    FillV4L2SegmentationHeader(frame_hdr->segmentation_hdr, &v4l2_frame_hdr.sgmnt_hdr);

    FillV4L2LoopfilterHeader(frame_hdr->loopfilter_hdr, &v4l2_frame_hdr.lf_hdr);

    FillV4L2QuantizationHeader(frame_hdr->quantization_hdr, &v4l2_frame_hdr.quant_hdr);
...
bool V4L2VideoDecoderDelegateVP8Legacy::OutputPicture(scoped_refptr<VP8Picture> pic)
{
    surface_handler_->SurfaceReady(VP8PictureToV4L2DecodeSurface(pic.get()), pic->bitstream_id(), pic->visible_rect(), pic->get_colorspace());
    return true;
}

scoped_refptr<V4L2DecodeSurface> V4L2VideoDecoderDelegateVP8Legacy::VP8PictureToV4L2DecodeSurface(VP8Picture* pic)
{
