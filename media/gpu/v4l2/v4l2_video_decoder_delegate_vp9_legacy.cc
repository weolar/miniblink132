...
void FillV4L2VP9LoopFilterParams(const Vp9LoopFilterParams& vp9_lf_params, struct v4l2_vp9_loop_filter_params* v4l2_lf_params)
{
#define SET_LF_PARAMS_FLAG_IF(cond, flag) v4l2_lf_params->flags |= ((vp9_lf_params.cond) ? (flag) : 0)
...
void FillV4L2VP9QuantizationParams(const Vp9QuantizationParams& vp9_quant_params, struct v4l2_vp9_quantization_params* v4l2_q_params)
{
#define SET_Q_PARAMS_FLAG_IF(cond, flag) v4l2_q_params->flags |= ((vp9_quant_params.cond) ? (flag) : 0)
...
void FillV4L2VP9SegmentationParams(const Vp9SegmentationParams& vp9_segm_params, struct v4l2_vp9_segmentation_params* v4l2_segm_params)
{
#define SET_SEG_PARAMS_FLAG_IF(cond, flag) v4l2_segm_params->flags |= ((vp9_segm_params.cond) ? (flag) : 0)
    ...
    SET_SEG_PARAMS_FLAG_IF(temporal_update, V4L2_VP9_SGMNT_PARAM_FLAG_TEMPORAL_UPDATE);
    SET_SEG_PARAMS_FLAG_IF(update_data, V4L2_VP9_SGMNT_PARAM_FLAG_UPDATE_DATA);
    SET_SEG_PARAMS_FLAG_IF(abs_or_delta_update, V4L2_VP9_SGMNT_PARAM_FLAG_ABS_OR_DELTA_UPDATE);
#undef SET_SEG_PARAMS_FLAG_IF
    ...
    static_assert(std::extent<decltype(v4l2_segm_params->feature_enabled)>() == std::extent<decltype(vp9_segm_params.feature_enabled)>()
            && std::extent<decltype(v4l2_segm_params->feature_enabled[0])>() == std::extent<decltype(vp9_segm_params.feature_enabled[0])>(),
        "feature_enabled arrays must be of same size");
    for (size_t i = 0; i < std::size(v4l2_segm_params->feature_enabled); ++i) {
        for (size_t j = 0; j < std::size(v4l2_segm_params->feature_enabled[i]); ++j) {
            v4l2_segm_params->feature_enabled[i][j] = vp9_segm_params.feature_enabled[i][j];
        }
    }
}

void FillV4L2Vp9EntropyContext(const Vp9FrameContext& vp9_frame_ctx, struct v4l2_vp9_entropy_ctx* v4l2_entropy_ctx)
{
#define ARRAY_MEMCPY_CHECKED_FRM_CTX_TO_V4L2_ENTR(a) SafeArrayMemcpy(v4l2_entropy_ctx->a, vp9_frame_ctx.a)
...
void FillVp9FrameContext(struct v4l2_vp9_entropy_ctx& v4l2_entropy_ctx, Vp9FrameContext* vp9_frame_ctx)
{
#define ARRAY_MEMCPY_CHECKED_V4L2_ENTR_TO_FRM_CTX(a) SafeArrayMemcpy(vp9_frame_ctx->a, v4l2_entropy_ctx.a)
...
V4L2VideoDecoderDelegateVP9Legacy::V4L2VideoDecoderDelegateVP9Legacy(V4L2DecodeSurfaceHandler* surface_handler, V4L2Device* device)
    : surface_handler_(surface_handler)
    , device_(device)
    , device_needs_compressed_header_parsed_(device->IsCtrlExposed(V4L2_CID_MPEG_VIDEO_VP9_ENTROPY))
{
    DCHECK(surface_handler_);

    DVLOG_IF(1, device_needs_compressed_header_parsed_) << "Device requires frame context parsing";
}
...
scoped_refptr<VP9Picture> V4L2VideoDecoderDelegateVP9Legacy::CreateVP9Picture()
{
...
DecodeStatus V4L2VideoDecoderDelegateVP9Legacy::SubmitDecode(scoped_refptr<VP9Picture> pic, const Vp9SegmentationParams& segm_params,
    const Vp9LoopFilterParams& lf_params, const Vp9ReferenceFrameVector& ref_frames, base::OnceClosure done_cb)
{
...
#undef FHDR_TO_V4L2_FHDR
    ...
    FillV4L2VP9QuantizationParams(frame_hdr->quant_params, &v4l2_frame_hdr.quant_params);

#define SET_V4L2_FRM_HDR_FLAG_IF(cond, flag) v4l2_frame_hdr.flags |= ((frame_hdr->cond) ? (flag) : 0)
    SET_V4L2_FRM_HDR_FLAG_IF(show_frame, V4L2_VP9_FRAME_HDR_FLAG_SHOW_FRAME);
    SET_V4L2_FRM_HDR_FLAG_IF(error_resilient_mode, V4L2_VP9_FRAME_HDR_FLAG_ERR_RES);
    SET_V4L2_FRM_HDR_FLAG_IF(intra_only, V4L2_VP9_FRAME_HDR_FLAG_FRAME_INTRA);
    SET_V4L2_FRM_HDR_FLAG_IF(allow_high_precision_mv, V4L2_VP9_FRAME_HDR_ALLOW_HIGH_PREC_MV);
    SET_V4L2_FRM_HDR_FLAG_IF(refresh_frame_context, V4L2_VP9_FRAME_HDR_REFRESH_FRAME_CTX);
...
#undef SET_V4L2_FRM_HDR_FLAG_IF
    ...
    for (size_t i = 0; i < kVp9NumRefFrames; ++i) {
        ...
        if (ref_pic) {
            scoped_refptr<V4L2DecodeSurface> ref_surface = VP9PictureToV4L2DecodeSurface(ref_pic.get());
    ...
    static_assert(std::extent<decltype(v4l2_decode_param.active_ref_frames)>() == std::extent<decltype(frame_hdr->ref_frame_idx)>(),
...
#undef REF_TO_V4L2_REF
    ...
    if (device_needs_compressed_header_parsed_) {
        ...
        FillV4L2Vp9EntropyContext(frame_hdr->initial_frame_context, &v4l2_entropy.initial_entropy_ctx);
        FillV4L2Vp9EntropyContext(frame_hdr->frame_context, &v4l2_entropy.current_entropy_ctx);
    ...
    scoped_refptr<V4L2DecodeSurface> dec_surface = VP9PictureToV4L2DecodeSurface(pic.get());
    ...
    if (!surface_handler_->SubmitSlice(dec_surface.get(), frame_hdr->data, frame_hdr->frame_size))
        return DecodeStatus::kFail;
...
bool V4L2VideoDecoderDelegateVP9Legacy::OutputPicture(scoped_refptr<VP9Picture> pic)
{
    surface_handler_->SurfaceReady(VP9PictureToV4L2DecodeSurface(pic.get()), pic->bitstream_id(), pic->visible_rect(), pic->get_colorspace());
    return true;
}

bool V4L2VideoDecoderDelegateVP9Legacy::GetFrameContext(scoped_refptr<VP9Picture> pic, Vp9FrameContext* frame_ctx)
{
    struct v4l2_ctrl_vp9_entropy v4l2_entropy;
    memset(&v4l2_entropy, 0, sizeof(v4l2_entropy));
...
bool V4L2VideoDecoderDelegateVP9Legacy::SupportsContextProbabilityReadback() const
{
    return true;
}

scoped_refptr<V4L2DecodeSurface> V4L2VideoDecoderDelegateVP9Legacy::VP9PictureToV4L2DecodeSurface(VP9Picture* pic)
{
