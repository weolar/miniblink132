...
void* GenericV4L2Device::Mmap(void* addr, unsigned int len, int prot, int flags, unsigned int offset)
{
    DCHECK(device_fd_.is_valid());
...
std::vector<base::ScopedFD> GenericV4L2Device::GetDmabufsForV4L2Buffer(int index, size_t num_planes, enum v4l2_buf_type buf_type)
{
    DVLOGF(3);
...
EGLImageKHR GenericV4L2Device::CreateEGLImage(EGLDisplay egl_display, EGLContext /* egl_context */, GLuint texture_id, const gfx::Size& size,
    unsigned int buffer_index, const Fourcc fourcc, gfx::NativePixmapHandle handle) const
{
    DVLOGF(3);
    ...
    EGLImageKHR egl_image = eglCreateImageKHR(egl_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, &attrs[0]);
...
scoped_refptr<gl::GLImage> GenericV4L2Device::CreateGLImage(const gfx::Size& size, const Fourcc fourcc, gfx::NativePixmapHandle handle) const
{
    DVLOGF(3);
    ...
    scoped_refptr<gfx::NativePixmap> pixmap
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmapFromHandle(0, size, buffer_format, std::move(handle));

    DCHECK(pixmap);

    // TODO(b/220336463): plumb the right color space.
    auto image = base::MakeRefCounted<gl::GLImageNativePixmap>(size, buffer_format);
    bool ret = image->Initialize(std::move(pixmap));
    DCHECK(ret);
    return image;
}

EGLBoolean GenericV4L2Device::DestroyEGLImage(EGLDisplay egl_display, EGLImageKHR egl_image) const
{
    DVLOGF(3);
...
std::vector<uint32_t> GenericV4L2Device::GetSupportedImageProcessorPixelformats(v4l2_buf_type buf_type)
{
...
VideoDecodeAccelerator::SupportedProfiles GenericV4L2Device::GetSupportedDecodeProfiles(const size_t num_formats, const uint32_t pixelformats[])
{
...
VideoEncodeAccelerator::SupportedProfiles GenericV4L2Device::GetSupportedEncodeProfiles()
{
...
const GenericV4L2Device::Devices& GenericV4L2Device::GetDevicesForType(Type type)
{
