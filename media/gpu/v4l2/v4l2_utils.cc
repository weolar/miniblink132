...
#define MAKE_V4L2_CODEC_PAIR(codec, suffix) std::make_pair(codec##_##suffix, codec)
...
{
    base::UmaHistogramEnumeration("Media.V4l2VideoDecoder.MediaIoctlError", function);
}

void RecordVidiocIoctlErrorUMA(VidiocIoctlRequests function)
{
    base::UmaHistogramEnumeration("Media.V4l2VideoDecoder.VidiocIoctlError", function);
}
...
std::string V4L2FormatToString(const struct v4l2_format& format)
{
    ...
    if (format.type == V4L2_BUF_TYPE_VIDEO_CAPTURE || format.type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
        ...
        s << ", width_height: " << gfx::Size(pix.width, pix.height).ToString() << ", pixelformat: " << FourccToString(pix.pixelformat)
          << ", field: " << pix.field << ", bytesperline: " << pix.bytesperline << ", sizeimage: " << pix.sizeimage;
        ...
        s << ", width_height: " << gfx::Size(pix_mp.width, pix_mp.height).ToString() << ", pixelformat: " << FourccToString(pix_mp.pixelformat)
          << ", field: " << pix_mp.field << ", num_planes: " << static_cast<unsigned int>(pix_mp.num_planes);
        for (size_t i = 0; i < pix_mp.num_planes; ++i) {
            const struct v4l2_plane_pix_format& plane_fmt = pix_mp.plane_fmt[i];
            s << ", plane_fmt[" << i << "].sizeimage: " << plane_fmt.sizeimage << ", plane_fmt[" << i << "].bytesperline: " << plane_fmt.bytesperline;
        }
    } else {
        s << " unsupported yet.";
    }
...
{
    std::ostringstream s;
    s << "v4l2_buffer type: " << buffer.type << ", memory: " << buffer.memory << ", index: " << buffer.index << " bytesused: " << buffer.bytesused
    ...
    if (buffer.type == V4L2_BUF_TYPE_VIDEO_CAPTURE || buffer.type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
        ...
        for (size_t i = 0; i < buffer.length; ++i) {
            ...
            s << ", m.planes[" << i << "](bytesused: " << plane.bytesused << ", length: " << plane.length << ", data_offset: " << plane.data_offset;
            if (buffer.memory == V4L2_MEMORY_MMAP) {
...
VideoCodecProfile V4L2ProfileToVideoCodecProfile(uint32_t v4l2_codec, uint32_t v4l2_profile)
{
    switch (v4l2_codec) {
...
std::optional<VideoFrameLayout> V4L2FormatToVideoFrameLayout(const struct v4l2_format& format)
{
    if (!V4L2_TYPE_IS_MULTIPLANAR(format.type)) {
        VLOGF(1) << "v4l2_buf_type is not multiplanar: " << std::hex << "0x" << format.type;
        return std::nullopt;
    }
    ...
    if (!video_fourcc) {
        VLOGF(1) << "Failed to convert pixel format to VideoPixelFormat: " << FourccToString(pix_fmt);
        return std::nullopt;
    }
    ...
    if (num_color_planes == 0) {
        VLOGF(1) << "Unsupported video format for NumPlanes(): " << VideoPixelFormatToString(video_format);
        return std::nullopt;
    }
    if (num_buffers > num_color_planes) {
        VLOGF(1) << "pix_mp.num_planes: " << num_buffers << " should not be larger than NumPlanes(" << VideoPixelFormatToString(video_format)
                 << "): " << num_color_planes;
        return std::nullopt;
    }
    ...
    for (size_t i = 0; i < num_buffers; ++i) {
        ...
        planes.emplace_back(static_cast<int32_t>(plane_format.bytesperline), 0u, plane_format.sizeimage);
    }
    ...
    if (num_color_planes > num_buffers) {
        ...
        case V4L2_PIX_FMT_NV12:
            ...
            planes.emplace_back(y_stride, y_stride_abs * pix_mp.height, y_stride_abs * pix_mp.height / 2);
        ...
        case V4L2_PIX_FMT_YVU420: {
            ...
            if (y_stride % 2 != 0 || pix_mp.height % 2 != 0) {
                VLOGF(1) << "Plane-Y stride and height should be even; stride: " << y_stride << ", height: " << pix_mp.height;
                return std::nullopt;
            }
            ...
            planes.emplace_back(half_stride, plane_0_area + plane_1_area, plane_1_area);
            DCHECK_EQ(3u, planes.size());
            break;
        }
        default:
            VLOGF(1) << "Cannot derive stride for each plane for pixel format " << FourccToString(pix_fmt);
            return std::nullopt;
        }
    }
    ...
    if (num_buffers == 1) {
        return VideoFrameLayout::CreateWithPlanes(video_format, gfx::Size(pix_mp.width, pix_mp.height), std::move(planes), buffer_alignment);
    } else {
        return VideoFrameLayout::CreateMultiPlanar(video_format, gfx::Size(pix_mp.width, pix_mp.height), std::move(planes), buffer_alignment);
    }
}
...
static const std::map<v4l2_enum_type, v4l2_enum_type> kV4L2CodecPixFmtToProfileCID = {
...
static const std::map<v4l2_enum_type, std::vector<VideoCodecProfile>> kDefaultVideoCodecProfilesForProfileCID = {
...
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    { V4L2_CID_MPEG_VIDEO_HEVC_PROFILE, { HEVCPROFILE_MAIN, HEVCPROFILE_MAIN10 } },
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
...
// as a pixel format.
static const std::map<VideoCodecProfile, std::pair<v4l2_enum_type, v4l2_enum_type>> kVideoCodecProfileToV4L2CodecPixFmt = {
...
#if BUILDFLAG(IS_CHROMEOS)
    { AV1PROFILE_PROFILE_MAIN, MAKE_V4L2_CODEC_PAIR(V4L2_PIX_FMT_AV1, FRAME) },
#endif
};

} // namespace

std::vector<SVCScalabilityMode> GetSupportedScalabilityModesForV4L2Codec(const IoctlAsCallback& ioctl_cb, VideoCodecProfile media_profile)
{
    std::vector<SVCScalabilityMode> scalability_modes;
    scalability_modes.push_back(SVCScalabilityMode::kL1T1);

    if (base::FeatureList::IsEnabled(kV4L2H264TemporalLayerHWEncoding) && media_profile >= H264PROFILE_MIN && media_profile <= H264PROFILE_MAX) {
        ...
        struct v4l2_querymenu query_menu = { .id = query_ctrl.id, .index = static_cast<__u32>(query_ctrl.minimum) };
        for (; static_cast<int>(query_menu.index) <= query_ctrl.maximum; query_menu.index++) {
...
std::vector<VideoCodecProfile> EnumerateSupportedProfilesForV4L2Codec(const IoctlAsCallback& ioctl_cb, uint32_t codec_as_pix_fmt)
{
    ...
    if (ioctl_cb.Run(VIDIOC_QUERYCTRL, &query_ctrl) != kIoctlOk) {
        ...
        DVLOGF(4) << "Driver doesn't support enumerating " << FourccToString(codec_as_pix_fmt) << " profiles, using default ones.";
        DCHECK(base::Contains(kDefaultVideoCodecProfilesForProfileCID, profile_cid));
        return kDefaultVideoCodecProfilesForProfileCID.at(profile_cid);
    }

    std::vector<VideoCodecProfile> profiles;

    v4l2_querymenu query_menu = { .id = query_ctrl.id, .index = static_cast<__u32>(query_ctrl.minimum) };
    for (; static_cast<int>(query_menu.index) <= query_ctrl.maximum; query_menu.index++) {
        if (ioctl_cb.Run(VIDIOC_QUERYMENU, &query_menu) != kIoctlOk) {
            continue;
        }
        const VideoCodecProfile profile = V4L2ProfileToVideoCodecProfile(profile_cid, query_menu.index);
        DVLOGF_IF(4, profile == VIDEO_CODEC_PROFILE_UNKNOWN) << "Profile: " << query_menu.name << " not supported by Chrome, skipping.";
...
std::vector<uint32_t> EnumerateSupportedPixFmts(const IoctlAsCallback& ioctl_cb, v4l2_buf_type buf_type)
{
    DCHECK(buf_type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE || buf_type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    ...
    for (; ioctl_cb.Run(VIDIOC_ENUM_FMT, &fmtdesc) == kIoctlOk; ++fmtdesc.index) {
        DVLOGF(4) << "Enumerated " << (buf_type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE ? "codec: " : "pixel format: ") << FourccToString(fmtdesc.pixelformat)
                  << " (" << fmtdesc.description << ")";
...
void GetSupportedResolution(const IoctlAsCallback& ioctl_cb, uint32_t pixelformat, gfx::Size* min_resolution, gfx::Size* max_resolution)
{
    ...
    *min_resolution = kDefaultMinCodedSize;
    ...
    if (ioctl_cb.Run(VIDIOC_ENUM_FRAMESIZES, &frame_size) == kIoctlOk) {
        if (frame_size.type == V4L2_FRMSIZE_TYPE_STEPWISE) {
            max_resolution->SetSize(frame_size.stepwise.max_width, frame_size.stepwise.max_height);
            min_resolution->SetSize(frame_size.stepwise.min_width, frame_size.stepwise.min_height);
        } else {
#if BUILDFLAG(IS_CHROMEOS)
...
uint32_t VideoCodecProfileToV4L2PixFmt(VideoCodecProfile profile, bool slice_based)
{
    CHECK(base::Contains(kVideoCodecProfileToV4L2CodecPixFmt, profile)) << "Unsupported profile: " << GetProfileName(profile);
...
struct timeval TimeDeltaToTimeVal(base::TimeDelta time_delta)
{
    ...
    return { .tv_sec = base::checked_cast<__time_t>(time_delta_linear / kMicrosecondsPerSecond),
        .tv_usec = base::checked_cast<__suseconds_t>(time_delta_linear % kMicrosecondsPerSecond) };
}
...
std::optional<SupportedVideoDecoderConfigs> GetSupportedV4L2DecoderConfigs()
{
    ...
    for (int i = 0; i < kMaxDevices; ++i) {
        candidate_paths.push_back(base::StringPrintf("%s%d", kVideoDevicePattern, i));
    }
#endif

    for (const auto& path : candidate_paths) {
        base::ScopedFD device_fd(HANDLE_EINTR(open(path.c_str(), O_RDWR | O_NONBLOCK | O_CLOEXEC)));
        if (!device_fd.is_valid()) {
            PLOG(WARNING) << "Could not open " << path;
            continue;
        }

        std::vector<uint32_t> v4l2_codecs = EnumerateSupportedPixFmts(base::BindRepeating(&HandledIoctl, device_fd.get()), V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);

        for (const uint32_t v4l2_codec : v4l2_codecs) {
            const std::vector<VideoCodecProfile> media_codec_profiles
                = EnumerateSupportedProfilesForV4L2Codec(base::BindRepeating(&HandledIoctl, device_fd.get()), v4l2_codec);

            gfx::Size min_coded_size;
            gfx::Size max_coded_size;
            GetSupportedResolution(base::BindRepeating(&HandledIoctl, device_fd.get()), v4l2_codec, &min_coded_size, &max_coded_size);

            for (const auto& profile : media_codec_profiles) {
                supported_media_configs.emplace_back(SupportedVideoDecoderConfig(profile, profile, min_coded_size, max_coded_size,
    ...
    for (const auto& config : supported_media_configs) {
        DVLOGF(3) << "Enumerated " << GetProfileName(config.profile_min) << " (" << config.coded_size_min.ToString() << "-" << config.coded_size_max.ToString()
                  << ")";
    }
#endif
...
bool IsV4L2DecoderStateful()
{
    ...
    return std::find_first_of(v4l2_codecs.begin(), v4l2_codecs.end(), kSupportedStatefulInputCodecs.begin(), kSupportedStatefulInputCodecs.end())
        != v4l2_codecs.end();
}
...
bool IsVislDriver()
{
    ...
    std::string v4l2_driver_name = GetDriverName(base::BindRepeating(&HandledIoctl, device_fd.get()));

    return v4l2_driver_name.compare("visl") == 0;
}

#ifndef NDEBUG
template <class T> std::string ControlToString(const T& a, const std::string control)
{
    std::ostringstream s;
...
template <typename T, size_t N> std::string ControlToString(const T (&arr)[N], const std::string control)
{
    std::ostringstream s;
...
template <typename T, size_t N, size_t M> std::string ControlToString(const T (&arr)[N][M], const std::string control)
{
    std::ostringstream s;
...
static std::string PrintStatelessAV1Control(const struct v4l2_ext_control* ext_ctrls)
{
    std::ostringstream s;
    s << "av1" << std::endl;
    switch (ext_ctrls->id) {
    case V4L2_CID_STATELESS_AV1_SEQUENCE: {
        ...
        const struct v4l2_ctrl_av1_sequence* ctrl = static_cast<const struct v4l2_ctrl_av1_sequence*>(ext_ctrls->ptr);
        CONTROL(ctrl, flags);
    ...
    case V4L2_CID_STATELESS_AV1_TILE_GROUP_ENTRY: {
        ...
        const struct v4l2_ctrl_av1_tile_group_entry* group_entry = static_cast<const struct v4l2_ctrl_av1_tile_group_entry*>(ext_ctrls->ptr);
    ...
    case V4L2_CID_STATELESS_AV1_FRAME: {
        ...
        const struct v4l2_ctrl_av1_frame* frame = static_cast<const struct v4l2_ctrl_av1_frame*>(ext_ctrls->ptr);

        const struct v4l2_av1_tile_info* tile_info = static_cast<const struct v4l2_av1_tile_info*>(&frame->tile_info);
        ...
        const struct v4l2_av1_quantization* quantization = static_cast<const struct v4l2_av1_quantization*>(&frame->quantization);
        ...
        const struct v4l2_av1_segmentation* segmentation = static_cast<const struct v4l2_av1_segmentation*>(&frame->segmentation);
        ...
        const struct v4l2_av1_loop_filter* loop_filter = static_cast<const struct v4l2_av1_loop_filter*>(&frame->loop_filter);
        ...
        const struct v4l2_av1_cdef* cdef = static_cast<const struct v4l2_av1_cdef*>(&frame->cdef);
        ...
        const struct v4l2_av1_loop_restoration* loop_restoration = static_cast<const struct v4l2_av1_loop_restoration*>(&frame->loop_restoration);
        ...
        const struct v4l2_av1_global_motion* global_motion = static_cast<const struct v4l2_av1_global_motion*>(&frame->global_motion);
...
static std::string PrintStatelessVP8Control(const struct v4l2_ext_control* ext_ctrls)
{
    ...
    const struct v4l2_ctrl_vp8_frame* ctrl = static_cast<const struct v4l2_ctrl_vp8_frame*>(ext_ctrls->ptr);

    const struct v4l2_vp8_segment* segment = static_cast<const struct v4l2_vp8_segment*>(&ctrl->segment);
    ...
    const struct v4l2_vp8_loop_filter* lf = static_cast<const struct v4l2_vp8_loop_filter*>(&ctrl->lf);
    ...
    const struct v4l2_vp8_quantization* quant = static_cast<const struct v4l2_vp8_quantization*>(&ctrl->quant);
    ...
    const struct v4l2_vp8_entropy* entropy = static_cast<const struct v4l2_vp8_entropy*>(&ctrl->entropy);
    ...
    const struct v4l2_vp8_entropy_coder_state* coder_state = static_cast<const struct v4l2_vp8_entropy_coder_state*>(&ctrl->coder_state);
...
static std::string PrintStatelessVP9Control(const struct v4l2_ext_control* ext_ctrls)
{
    ...
    const struct v4l2_ctrl_vp9_frame* ctrl = static_cast<const struct v4l2_ctrl_vp9_frame*>(ext_ctrls->ptr);

    const struct v4l2_vp9_loop_filter* lf = static_cast<const struct v4l2_vp9_loop_filter*>(&ctrl->lf);
    ...
    const struct v4l2_vp9_quantization* quant = static_cast<const struct v4l2_vp9_quantization*>(&ctrl->quant);
    ...
    const struct v4l2_vp9_segmentation* seg = static_cast<const struct v4l2_vp9_segmentation*>(&ctrl->seg);
...
static std::string PrintStatelessH264Control(const struct v4l2_ext_control* ext_ctrls)
{
    std::ostringstream s;
    s << "h.264" << std::endl;
    switch (ext_ctrls->id) {
    case V4L2_CID_STATELESS_H264_SPS: {
        ...
        const struct v4l2_ctrl_h264_sps* sps = static_cast<const struct v4l2_ctrl_h264_sps*>(ext_ctrls->ptr);
    ...
    case V4L2_CID_STATELESS_H264_PPS: {
        ...
        const struct v4l2_ctrl_h264_pps* pps = static_cast<const struct v4l2_ctrl_h264_pps*>(ext_ctrls->ptr);
    ...
    case V4L2_CID_STATELESS_H264_SCALING_MATRIX: {
        ...
        const struct v4l2_ctrl_h264_scaling_matrix* sm = static_cast<const struct v4l2_ctrl_h264_scaling_matrix*>(ext_ctrls->ptr);
        CONTROL(sm, scaling_list_4x4);
        CONTROL(sm, scaling_list_8x8);
    } break;
    case V4L2_CID_STATELESS_H264_DECODE_PARAMS: {
        s << "V4L2_CID_STATELESS_H264_DECODE_PARAMS" << std::endl;
        const struct v4l2_ctrl_h264_decode_params* dp = static_cast<const struct v4l2_ctrl_h264_decode_params*>(ext_ctrls->ptr);
        for (uint32_t i = 0; i < 16; ++i) {
            s << "dbp entry " << +i << std::endl;
            const struct v4l2_h264_dpb_entry* dpb = static_cast<const struct v4l2_h264_dpb_entry*>(&dp->dpb[i]);
    ...
    case V4L2_CID_STATELESS_H264_DECODE_MODE: {
        const enum v4l2_stateless_h264_decode_mode dm = static_cast<const enum v4l2_stateless_h264_decode_mode>(ext_ctrls->value);
        s << "V4L2_CID_STATELESS_H264_DECODE_MODE: "
          << (dm == V4L2_STATELESS_H264_DECODE_MODE_SLICE_BASED ? "V4L2_STATELESS_H264_DECODE_MODE_SLICE_BASED" : "V4L2_STATELESS_H264_DECODE_MODE_FRAME_BASED")
          << std::endl;
    } break;
    }
...
static std::string PrintStatelessHEVCControl(const struct v4l2_ext_control* ext_ctrls)
{
    std::ostringstream s;
    s << "hevc" << std::endl;
    switch (ext_ctrls->id) {
    case V4L2_CID_STATELESS_HEVC_SPS: {
        const struct v4l2_ctrl_hevc_sps* sps = static_cast<const struct v4l2_ctrl_hevc_sps*>(ext_ctrls->ptr);
    ...
    case V4L2_CID_STATELESS_HEVC_PPS: {
        const struct v4l2_ctrl_hevc_pps* pps = static_cast<const struct v4l2_ctrl_hevc_pps*>(ext_ctrls->ptr);
        CONTROL(pps, pic_parameter_set_id);
    ...
    case V4L2_CID_STATELESS_HEVC_SCALING_MATRIX: {
        const struct v4l2_ctrl_hevc_scaling_matrix* sm = static_cast<const struct v4l2_ctrl_hevc_scaling_matrix*>(ext_ctrls->ptr);
        CONTROL(sm, scaling_list_4x4);
        CONTROL(sm, scaling_list_8x8);
    ...
    case V4L2_CID_STATELESS_HEVC_DECODE_PARAMS: {
        const struct v4l2_ctrl_hevc_decode_params* dp = static_cast<const struct v4l2_ctrl_hevc_decode_params*>(ext_ctrls->ptr);
        ...
        for (uint32_t i = 0; i < V4L2_HEVC_DPB_ENTRIES_NUM_MAX; ++i) {
            s << "dbp entry " << +i << std::endl;
            const struct v4l2_hevc_dpb_entry* dpb = static_cast<const struct v4l2_hevc_dpb_entry*>(&dp->dpb[i]);
