...
public:
    explicit V4L2VideoDecoderDelegateVP9Legacy(V4L2DecodeSurfaceHandler* surface_handler, V4L2Device* device);

    V4L2VideoDecoderDelegateVP9Legacy(const V4L2VideoDecoderDelegateVP9Legacy&) = delete;
    V4L2VideoDecoderDelegateVP9Legacy& operator=(const V4L2VideoDecoderDelegateVP9Legacy&) = delete;
    ...
    Status SubmitDecode(scoped_refptr<VP9Picture> pic, const Vp9SegmentationParams& segm_params, const Vp9LoopFilterParams& lf_params,
        const Vp9ReferenceFrameVector& reference_frames, base::OnceClosure done_cb) override;

    bool OutputPicture(scoped_refptr<VP9Picture> pic) override;

    bool GetFrameContext(scoped_refptr<VP9Picture> pic, Vp9FrameContext* frame_ctx) override;
