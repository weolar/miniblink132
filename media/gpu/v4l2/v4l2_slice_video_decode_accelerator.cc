...
#define IOCTL_OR_ERROR_RETURN(type, arg) IOCTL_OR_ERROR_RETURN_VALUE(type, arg, ((void)0), #type)

#define IOCTL_OR_ERROR_RETURN_FALSE(type, arg) IOCTL_OR_ERROR_RETURN_VALUE(type, arg, false, #type)
...
V4L2SliceVideoDecodeAccelerator::OutputRecord::OutputRecord(OutputRecord&&) = default;
...
struct V4L2SliceVideoDecodeAccelerator::BitstreamBufferRef {
    BitstreamBufferRef(base::WeakPtr<VideoDecodeAccelerator::Client>& client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner,
        scoped_refptr<DecoderBuffer> buffer, int32_t input_id);
    ~BitstreamBufferRef();
...
V4L2SliceVideoDecodeAccelerator::BitstreamBufferRef::BitstreamBufferRef(base::WeakPtr<VideoDecodeAccelerator::Client>& client,
    scoped_refptr<base::SingleThreadTaskRunner> client_task_runner, scoped_refptr<DecoderBuffer> buffer, int32_t input_id)
    : client(client)
...
V4L2SliceVideoDecodeAccelerator::BitstreamBufferRef::~BitstreamBufferRef()
{
    if (input_id >= 0) {
        ...
        client_task_runner->PostTask(FROM_HERE, base::BindOnce(&VideoDecodeAccelerator::Client::NotifyEndOfBitstreamBuffer, client, input_id));
    }
}

V4L2SliceVideoDecodeAccelerator::PictureRecord::PictureRecord(bool cleared, const Picture& picture)
    : cleared(cleared)
    , picture(picture)
{
}
...
V4L2SliceVideoDecodeAccelerator::V4L2SliceVideoDecodeAccelerator(scoped_refptr<V4L2Device> device, EGLDisplay egl_display,
    const BindGLImageCallback& bind_image_cb, const MakeGLContextCurrentCallback& make_context_current_cb)
...
void V4L2SliceVideoDecodeAccelerator::NotifyError(Error error)
{
    ...
    if (!child_task_runner_->BelongsToCurrentThread()) {
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::NotifyError, weak_this_, error));
        return;
    }
...
bool V4L2SliceVideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    ...
    client_ptr_factory_.reset(new base::WeakPtrFactory<VideoDecodeAccelerator::Client>(client));
    ...
    if (!input_format_fourcc_ || !device_->Open(V4L2Device::Type::kDecoder, input_format_fourcc_)) {
        VLOGF(1) << "Failed to open device for profile: " << config.profile << " fourcc: " << FourccToString(input_format_fourcc_);
        return false;
    }
    ...
    if (video_profile_ >= H264PROFILE_MIN && video_profile_ <= H264PROFILE_MAX) {
        if (supports_requests_) {
            decoder_ = std::make_unique<H264Decoder>(
                std::make_unique<V4L2VideoDecoderDelegateH264>(this, device_.get()), video_profile_, config.container_color_space);
        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<H264Decoder>(
                std::make_unique<V4L2VideoDecoderDelegateH264Legacy>(this, device_.get()), video_profile_, config.container_color_space);
#else
        ...
        if (supports_requests_) {
            decoder_ = std::make_unique<VP8Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP8>(this, device_.get()), config.container_color_space);
        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<VP8Decoder>(std::make_unique<V4L2VideoDecoderDelegateVP8Legacy>(this, device_.get()), config.container_color_space);
#else
            NOTREACHED() << "Unsupported profile " << GetProfileName(video_profile_);
            return false;
#endif
...
#define V4L2_CID_STATELESS_VP9_FRAME (0x00a40900 + 300)
#endif
            const bool supports_stable_api = device_->IsCtrlExposed(V4L2_CID_STATELESS_VP9_FRAME);
            CHECK(supports_stable_api);
            decoder_ = std::make_unique<VP9Decoder>(
                std::make_unique<V4L2VideoDecoderDelegateVP9>(this, device_.get()), video_profile_, config.container_color_space);

        } else {
#if BUILDFLAG(IS_CHROMEOS)
            decoder_ = std::make_unique<VP9Decoder>(
                std::make_unique<V4L2VideoDecoderDelegateVP9Legacy>(this, device_.get()), video_profile_, config.container_color_space);
#else
            NOTREACHED() << "Unsupported profile " << GetProfileName(video_profile_);
            return false;
#endif
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "media::V4l2SliceVideoDecodeAccelerator", decoder_thread_task_runner_);

    state_ = kInitialized;
    output_mode_ = config.output_mode;

    // InitializeTask will NOTIFY_ERROR on failure.
    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::InitializeTask, base::Unretained(this)));
        ...
        decoder_thread_task_runner_->PostTask(FROM_HERE,
            ...
            base::BindOnce(
                [](V4L2SliceVideoDecodeAccelerator* vda) {
                    ...
                    vda->surfaces_at_ip_ = {};
                    vda->decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DestroyTask, base::Unretained(vda)));
                },
...
bool V4L2SliceVideoDecodeAccelerator::SetupFormats()
{
    ...
    device_->GetSupportedResolution(input_format_fourcc_, &min_resolution, &max_resolution);
    ...
    if (!is_format_supported) {
        DVLOGF(1) << "Input fourcc " << input_format_fourcc_ << " not supported by device.";
        return false;
    }
    ...
    while (device_->Ioctl(VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
        ...
        if (fourcc && device_->CanCreateEGLImageFrom(*fourcc)) {
            ...
            output_planes_count_ = V4L2Device::GetNumPlanesOfV4L2PixFmt(output_format_fourcc_->ToV4L2PixFmt());
            break;
        }
        ++fmtdesc.index;
    }
    ...
    if (!output_format_fourcc_) {
        ...
        output_format_fourcc_ = v4l2_vda_helpers::FindImageProcessorInputFormat(device_.get());
        if (!output_format_fourcc_) {
        ...
        gl_image_format_fourcc_ = v4l2_vda_helpers::FindImageProcessorOutputFormat(image_processor_device_.get());
        if (!gl_image_format_fourcc_) {
            VLOGF(1) << "Can't find a usable output format from image processor";
            return false;
        }
        gl_image_planes_count_ = V4L2Device::GetNumPlanesOfV4L2PixFmt(gl_image_format_fourcc_->ToV4L2PixFmt());
    } else {
    ...
    format.fmt.pix_mp.num_planes = V4L2Device::GetNumPlanesOfV4L2PixFmt(output_format_fourcc_->ToV4L2PixFmt());
    IOCTL_OR_ERROR_RETURN_FALSE(VIDIOC_S_FMT, &format);
    DCHECK_EQ(format.fmt.pix_mp.pixelformat, output_format_fourcc_->ToV4L2PixFmt());

    DCHECK_EQ(static_cast<size_t>(format.fmt.pix_mp.num_planes), output_planes_count_);
...
bool V4L2SliceVideoDecodeAccelerator::CreateImageProcessor()
{
    ...
    const ImageProcessor::OutputMode image_processor_output_mode
        = (output_mode_ == Config::OutputMode::ALLOCATE ? ImageProcessor::OutputMode::ALLOCATE : ImageProcessor::OutputMode::IMPORT);
    ...
    image_processor_ = v4l2_vda_helpers::CreateImageProcessor(*output_format_fourcc_, *gl_image_format_fourcc_, coded_size_, coded_size_,
        decoder_->GetVisibleRect(), VideoFrame::StorageType::STORAGE_DMABUFS, output_buffer_map_.size(), image_processor_device_, image_processor_output_mode,
        ...
        // ErrorCB is executed.
        decoder_thread_.task_runner(), base::BindRepeating(&V4L2SliceVideoDecodeAccelerator::ImageProcessorError, base::Unretained(this)));
...
bool V4L2SliceVideoDecodeAccelerator::CreateOutputBuffers()
{
    ...
    coded_size_.SetSize(base::checked_cast<int>(format.fmt.pix_mp.width), base::checked_cast<int>(format.fmt.pix_mp.height));
    ...
    if (image_processor_device_) {
        ...
        if (!V4L2ImageProcessorBackend::TryOutputFormat(
                output_format_fourcc_->ToV4L2PixFmt(), gl_image_format_fourcc_->ToV4L2PixFmt(), coded_size_, &output_size, &planes_count)) {
        ...
        if (gl_image_planes_count_ != planes_count) {
            VLOGF(1) << "IP buffers planes count returned by V4L2 (" << planes_count << ") doesn't match the computed number (" << gl_image_planes_count_
                     << ")";
            return false;
        }
    } else {
    ...
    DVLOGF(3) << "buffer_count=" << num_pictures << ", pic size=" << pic_size.ToString() << ", coded size=" << coded_size_.ToString();

    VideoPixelFormat pixel_format = gl_image_format_fourcc_->ToVideoPixelFormat();
    child_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoDecodeAccelerator::Client::ProvidePictureBuffersWithVisibleRect, client_, num_pictures, pixel_format, 1, gl_image_size_,
...
void V4L2SliceVideoDecodeAccelerator::DismissPictures(const std::vector<int32_t>& picture_buffer_ids, base::WaitableEvent* done)
{
    DVLOGF(3);
    DCHECK(child_task_runner_->BelongsToCurrentThread());
...
void V4L2SliceVideoDecodeAccelerator::ServiceDeviceTask(bool event)
{
    ...
    DVLOGF(3) << "buffer counts: "
              ...
              << " => DEVICE[" << input_queue_->FreeBuffersCount() << "+" << input_queue_->QueuedBuffersCount() << "/" << input_queue_->AllocatedBuffersCount()
              << "]->[" << output_queue_->FreeBuffersCount() << "+" << output_queue_->QueuedBuffersCount() << "/" << output_buffer_map_.size() << "]"
...
void V4L2SliceVideoDecodeAccelerator::Dequeue()
{
    ...
    while (input_queue_->QueuedBuffersCount() > 0) {
        ...
        DVLOGF(4) << "Dequeued input=" << ret.second->BufferId() << " count: " << input_queue_->QueuedBuffersCount();
    }
    ...
    while (output_queue_->QueuedBuffersCount() > 0) {
        ...
        DVLOGF(4) << "Dequeued output=" << buffer_id << " count " << output_queue_->QueuedBuffersCount();
        ...
        if (image_processor_) {
            ...
            surface->SetReleaseCallback(base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ReuseOutputBuffer, base::Unretained(this), std::move(ret.second)));
        }
    }
...
void V4L2SliceVideoDecodeAccelerator::ReuseOutputBuffer(V4L2ReadableBufferRef buffer)
{
...
bool V4L2SliceVideoDecodeAccelerator::StartDevicePoll()
{
    ...
    return device_->StartPolling(base::BindRepeating(&V4L2SliceVideoDecodeAccelerator::ServiceDeviceTask, base::Unretained(this)),
        base::BindRepeating(&V4L2SliceVideoDecodeAccelerator::OnPollError, base::Unretained(this)));
}
...
void V4L2SliceVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    DVLOGF(4) << "input_id=" << bitstream_id << ", size=" << (buffer ? buffer->data_size() : 0);
    ...
    decoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DecodeTask, base::Unretained(this), std::move(buffer), bitstream_id));
}

void V4L2SliceVideoDecodeAccelerator::DecodeTask(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    DVLOGF(4) << "input_id=" << bitstream_id << " size=" << (buffer ? buffer->data_size() : 0);
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    ...
    std::unique_ptr<BitstreamBufferRef> bitstream_record(new BitstreamBufferRef(decode_client_, decode_task_runner_, std::move(buffer), bitstream_id));
    ...
    TRACE_COUNTER_ID1("media,gpu", "V4L2SVDA decoder input BitstreamBuffers", this, decoder_input_queue_.size());
...
bool V4L2SliceVideoDecodeAccelerator::TrySetNewBistreamBuffer()
{
    ...
    decoder_->SetStream(decoder_current_bitstream_buffer_->input_id, *decoder_current_bitstream_buffer_->buffer);
    return true;
}
...
void V4L2SliceVideoDecodeAccelerator::ScheduleDecodeBufferTaskIfNeeded()
{
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    if (state_ == kDecoding) {
        decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DecodeBufferTask, base::Unretained(this)));
    }
}
    ...
    while (true) {
        ...
        case AcceleratedVideoDecoder::kConfigChange:
            if (decoder_->GetBitDepth() != 8u) {
                LOG(ERROR) << "Unsupported bit depth: " << base::strict_cast<int>(decoder_->GetBitDepth());
                NOTIFY_ERROR(PLATFORM_FAILURE);
                return;
            }
...
void V4L2SliceVideoDecodeAccelerator::InitiateSurfaceSetChange()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "V4L2SVDA Resolution Change", TRACE_ID_LOCAL(this));
...
bool V4L2SliceVideoDecodeAccelerator::FinishSurfaceSetChange()
{
    ...
    const size_t num_imported_buffers = std::count_if(
        output_buffer_map_.begin(), output_buffer_map_.end(), [](const OutputRecord& output_record) { return output_record.output_frame != nullptr; });
    DCHECK_EQ(output_queue_->FreeBuffersCount() + surfaces_at_display_.size(), num_imported_buffers);
#endif // DCHECK_IS_ON()
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("media,gpu", "V4L2SVDA Resolution Change", TRACE_ID_LOCAL(this));
    return true;
}
...
bool V4L2SliceVideoDecodeAccelerator::DestroyOutputs(bool dismiss)
{
    ...
    for (const auto& [output_frame, picture_id, client_texture_id, texture_id, cleared, num_times_sent_to_client] : output_buffer_map_) {
    ...
    if (dismiss) {
        ...
        child_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DismissPictures, weak_this_, picture_buffers_to_dismiss, &done));
        done.Wait();
    }
...
void V4L2SliceVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    VLOGF(2);
    DCHECK(child_task_runner_->BelongsToCurrentThread());

    decoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::AssignPictureBuffersTask, base::Unretained(this), buffers));
}

void V4L2SliceVideoDecodeAccelerator::AssignPictureBuffersTask(const std::vector<PictureBuffer>& buffers)
{
    VLOGF(2);
    DCHECK(!output_queue_->IsStreaming());
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    DCHECK_EQ(state_, kAwaitingPictureBuffers);
    TRACE_EVENT1("media,gpu", "V4L2SVDA::AssignPictureBuffersTask", "buffers_size", buffers.size());
    ...
    const gfx::Size pic_size_expected_from_client
        = output_mode_ == Config::OutputMode::ALLOCATE ? GetRectSizeFromOrigin(decoder_->GetVisibleRect()) : coded_size_;
    if (output_mode_ == Config::OutputMode::ALLOCATE && pic_size_expected_from_client != pic_size_received_from_client) {
        ...
        // here (eventually).
        LOG(ERROR) << "The client supplied a picture buffer with an unexpected size (Got " << pic_size_received_from_client.ToString() << ", expected "
                   << pic_size_expected_from_client.ToString() << ")";
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    } else if (output_mode_ == Config::OutputMode::IMPORT && !image_processor_device_ && pic_size_expected_from_client != pic_size_received_from_client) {
        ...
        if (device_->Ioctl(VIDIOC_S_FMT, &format) != 0) {
            PLOG(ERROR) << "Failed with frame size adjusted by client" << pic_size_received_from_client.ToString();
            NOTIFY_ERROR(PLATFORM_FAILURE);
            return;
        }
        ...
        if (coded_size_ != pic_size_received_from_client) {
            LOG(ERROR) << "The size of PictureBuffer is invalid."
                       << " size adjusted by the client = " << pic_size_received_from_client.ToString()
    ...
    const v4l2_memory memory = (image_processor_device_ || output_mode_ == Config::OutputMode::ALLOCATE ? V4L2_MEMORY_MMAP : V4L2_MEMORY_DMABUF);
    ...
    for (auto&& buffer : v4l2_buffers) {
        ...
        output_record.texture_id = buffers[i].service_texture_ids().empty() ? 0 : buffers[i].service_texture_ids()[0];

        output_record.client_texture_id = buffers[i].client_texture_ids().empty() ? 0 : buffers[i].client_texture_ids()[0];
        ...
        if (output_mode_ == Config::OutputMode::ALLOCATE && !image_processor_device_) {
        ...
        if (output_mode_ == Config::OutputMode::ALLOCATE) {
            ...
            if (!image_processor_device_) {
                native_pixmap = CreateGpuMemoryBufferHandle(video_frame.get()).native_pixmap_handle;
            }

            ImportBufferForPictureTask(output_record.picture_id, std::move(native_pixmap));
...
void V4L2SliceVideoDecodeAccelerator::CreateGLImageFor(scoped_refptr<V4L2Device> gl_device, size_t buffer_index, int32_t picture_buffer_id,
    gfx::NativePixmapHandle handle, GLuint client_texture_id, GLuint texture_id, const gfx::Size& visible_size, const Fourcc fourcc)
{
    DVLOGF(3) << "index=" << buffer_index;
    DCHECK(child_task_runner_->BelongsToCurrentThread());
    DCHECK_NE(texture_id, 0u);
    TRACE_EVENT1("media,gpu", "V4L2SVDA::CreateGLImageFor", "picture_buffer_id", picture_buffer_id);
    ...
    scoped_refptr<gl::GLImage> gl_image = gl_device->CreateGLImage(visible_size, fourcc, std::move(handle));
    if (!gl_image) {
    ...
    gl::ScopedTextureBinder bind_restore(gl_device->GetTextureTarget(), texture_id);
    ...
    ret = bind_image_cb_.Run(client_texture_id, gl_device->GetTextureTarget(), gl_image, true);
    if (!ret) {
...
void V4L2SliceVideoDecodeAccelerator::ImportBufferForPicture(
    int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
    ...
    decoder_thread_.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ImportBufferForPictureForImportTask, base::Unretained(this), picture_buffer_id, pixel_format,
            std::move(gpu_memory_buffer_handle.native_pixmap_handle)));
}

void V4L2SliceVideoDecodeAccelerator::ImportBufferForPictureForImportTask(
    int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::NativePixmapHandle handle)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());

    if (pixel_format != gl_image_format_fourcc_->ToVideoPixelFormat()) {
        LOG(ERROR) << "Unsupported import format: " << VideoPixelFormatToString(pixel_format) << ", expected "
                   << VideoPixelFormatToString(gl_image_format_fourcc_->ToVideoPixelFormat());
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    }
...
void V4L2SliceVideoDecodeAccelerator::ImportBufferForPictureTask(int32_t picture_buffer_id, gfx::NativePixmapHandle handle)
{
    DVLOGF(3) << "picture_buffer_id=" << picture_buffer_id;
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    ...
    const auto iter = base::ranges::find(output_buffer_map_, picture_buffer_id, &OutputRecord::picture_id);
    if (iter == output_buffer_map_.end()) {
        ...
        DVLOGF(3) << "got picture id=" << picture_buffer_id << " not in use (anymore?).";
        return;
    }
    ...
    if (output_mode_ == Config::OutputMode::IMPORT) {
        ...
        const gfx::Size handle_size = v4l2_vda_helpers::NativePixmapSizeFromHandle(handle, *gl_image_format_fourcc_, gl_image_size_);
        ...
        if (state_ == kAwaitingPictureBuffers) {
            ...
            DVLOGF(3) << "Original gl_image_size=" << gl_image_size_.ToString() << ", adjusted buffer size=" << handle_size.ToString();
    ...
    if (state_ == kAwaitingPictureBuffers) {
        ...
        decoder_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ProcessPendingEventsIfNeeded, base::Unretained(this)));
    }
    ...
    if (output_mode_ == Config::OutputMode::IMPORT) {
        ...
        for (const gfx::NativePixmapPlane& plane : handle.planes) {
            ...
            color_planes.push_back(
                ColorPlaneLayout(base::checked_cast<int32_t>(plane.stride), base::checked_cast<size_t>(plane.offset), base::checked_cast<size_t>(plane.size)));
        }
        auto layout = VideoFrameLayout::CreateWithPlanes(gl_image_format_fourcc_->ToVideoPixelFormat(), gl_image_size_, std::move(color_planes));
        if (!layout) {
        ...
        iter->output_frame = VideoFrame::WrapExternalDmabufs(*layout, visible_rect, visible_rect.size(), std::move(duped_fds), base::TimeDelta());
    }
    ...
    if (iter->texture_id != 0 && !image_processor_) {
        ...
        child_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&V4L2SliceVideoDecodeAccelerator::CreateGLImageFor, weak_this_, device_, index, picture_buffer_id, std::move(handle),
                iter->client_texture_id, iter->texture_id, GetRectSizeFromOrigin(decoder_->GetVisibleRect()), *gl_image_format_fourcc_));
    }
...
void V4L2SliceVideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_buffer_id)
{
    DCHECK(child_task_runner_->BelongsToCurrentThread());
    ...
    decoder_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ReusePictureBufferTask, base::Unretained(this), picture_buffer_id, std::move(egl_fence)));
}

void V4L2SliceVideoDecodeAccelerator::ReusePictureBufferTask(int32_t picture_buffer_id, std::unique_ptr<gl::GLFenceEGL> egl_fence)
{
    DVLOGF(4) << "picture_buffer_id=" << picture_buffer_id;
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    ...
    const size_t output_map_index = decoded_buffer_map_[it->second->output_record()];
    ...
    if (!output_record.at_client()) {
        ...
        if (egl_fence)
            surfaces_awaiting_fence_.push(std::make_pair(std::move(egl_fence), std::move(it->second)));
...
void V4L2SliceVideoDecodeAccelerator::Flush()
{
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::FlushTask, base::Unretained(this)));
}
...
void V4L2SliceVideoDecodeAccelerator::FlushTask()
{
    ...
    decoder_input_queue_.push_back(std::make_unique<BitstreamBufferRef>(decode_client_, decode_task_runner_, nullptr, kFlushBufferId));
...
void V4L2SliceVideoDecodeAccelerator::InitiateFlush()
{
    VLOGF(2);
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "V4L2SVDA Flush", TRACE_ID_LOCAL(this));
...
bool V4L2SliceVideoDecodeAccelerator::FinishFlush()
{
    ...
    DCHECK_EQ(input_queue_->FreeBuffersCount(), input_queue_->AllocatedBuffersCount());
    ...
    child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyFlushDone, client_));

    TRACE_EVENT_NESTABLE_ASYNC_END0("media,gpu", "V4L2SVDA Flush", TRACE_ID_LOCAL(this));
    return true;
}
...
void V4L2SliceVideoDecodeAccelerator::Reset()
{
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ResetTask, base::Unretained(this)));
}
...
void V4L2SliceVideoDecodeAccelerator::ResetTask()
{
    VLOGF(2);
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "V4L2SVDA Reset", TRACE_ID_LOCAL(this));
...
bool V4L2SliceVideoDecodeAccelerator::FinishReset()
{
    ...
    child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyResetDone, client_));

    TRACE_EVENT_NESTABLE_ASYNC_END0("media,gpu", "V4L2SVDA Reset", TRACE_ID_LOCAL(this));
    return true;
}
...
void V4L2SliceVideoDecodeAccelerator::SetErrorState(Error error)
{
    ...
    if (decoder_thread_.IsRunning() && !decoder_thread_task_runner_->BelongsToCurrentThread()) {
        decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::SetErrorState, base::Unretained(this), error));
        return;
    }
...
bool V4L2SliceVideoDecodeAccelerator::SubmitSlice(V4L2DecodeSurface* dec_surface, const uint8_t* data, size_t size)
{
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void V4L2SliceVideoDecodeAccelerator::DecodeSurface(scoped_refptr<V4L2DecodeSurface> dec_surface)
{
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void V4L2SliceVideoDecodeAccelerator::SurfaceReady(
    scoped_refptr<V4L2DecodeSurface> dec_surface, int32_t bitstream_id, const gfx::Rect& visible_rect, const VideoColorSpace& color_space)
{
    DVLOGF(4);
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
{
    while (!decoder_display_queue_.empty()) {
        scoped_refptr<V4L2DecodeSurface> dec_surface = decoder_display_queue_.front().second;
...
void V4L2SliceVideoDecodeAccelerator::OutputSurface(int32_t bitstream_id, scoped_refptr<V4L2DecodeSurface> dec_surface)
{
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());

    DCHECK_EQ(decoded_buffer_map_.count(dec_surface->output_record()), 1u);
    const size_t output_map_index = decoded_buffer_map_[dec_surface->output_record()];
    DCHECK_LT(output_map_index, output_buffer_map_.size());
    OutputRecord& output_record = output_buffer_map_[output_map_index];

    if (!output_record.at_client()) {
        bool inserted = surfaces_at_display_.insert(std::make_pair(output_record.picture_id, dec_surface)).second;
        DCHECK(inserted);
    } else {
        // The surface is already sent to client, and not returned back yet.
        DCHECK(surfaces_at_display_.find(output_record.picture_id) != surfaces_at_display_.end());
        CHECK(surfaces_at_display_[output_record.picture_id].get() == dec_surface.get());
    }
    ...
    Picture picture(
        output_record.picture_id, bitstream_id, dec_surface->visible_rect(), dec_surface->color_space().ToGfxColorSpace(), true /* allow_overlay */);
    DVLOGF(4) << dec_surface->ToString() << ", bitstream_id: " << picture.bitstream_buffer_id() << ", picture_id: " << picture.picture_buffer_id()
...
void V4L2SliceVideoDecodeAccelerator::CheckGLFences()
{
    ...
    while (!surfaces_awaiting_fence_.empty() && surfaces_awaiting_fence_.front().first->HasCompleted()) {
    ...
    if (output_queue_->FreeBuffersCount() == 0) {
        ...
        decoder_thread_.task_runner()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&V4L2SliceVideoDecodeAccelerator::DecodeBufferTask, base::Unretained(this)), base::Milliseconds(kRescheduleDelayMs));
    }
}

scoped_refptr<V4L2DecodeSurface> V4L2SliceVideoDecodeAccelerator::CreateSurface()
{
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    DCHECK_EQ(state_, kDecoding);
    TRACE_COUNTER_ID2("media,gpu", "V4L2 input buffers", this, "free", input_queue_->FreeBuffersCount(), "in use",
        input_queue_->AllocatedBuffersCount() - input_queue_->FreeBuffersCount());
    TRACE_COUNTER_ID2("media,gpu", "V4L2 output buffers", this, "free", output_queue_->FreeBuffersCount(), "in use",
        output_queue_->AllocatedBuffersCount() - output_queue_->AllocatedBuffersCount());
    TRACE_COUNTER_ID2("media,gpu", "V4L2 output buffers", this, "at client", GetNumOfOutputRecordsAtClient(), "at device", GetNumOfOutputRecordsAtDevice());
    ...
    if (supports_requests_) {
        ...
        absl::optional<V4L2RequestRef> request_ref = requests_queue_->GetFreeRequest();
        if (!request_ref) {
            LOG(ERROR) << "Failed getting a request";
            NOTIFY_ERROR(PLATFORM_FAILURE);
            return nullptr;
        }
        dec_surface = new V4L2RequestDecodeSurface(std::move(*input_buffer), std::move(*output_buffer), output_record.output_frame, std::move(*request_ref));
    } else {
// ConfigStore is ChromeOS-specific legacy stuff
#if BUILDFLAG(IS_CHROMEOS)
        dec_surface = new V4L2ConfigStoreDecodeSurface(std::move(*input_buffer), std::move(*output_buffer), output_record.output_frame);
#else
...
void V4L2SliceVideoDecodeAccelerator::SendPictureReady()
{
    DVLOGF(4);
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    bool send_now = (decoder_resetting_ || decoder_flushing_ || surface_set_change_pending_);
    while (!pending_picture_ready_.empty()) {
        bool cleared = pending_picture_ready_.front().cleared;
        const Picture& picture = pending_picture_ready_.front().picture;
        if (cleared && picture_clearing_count_ == 0) {
            DVLOGF(4) << "Posting picture ready to decode task runner for: " << picture.picture_buffer_id();
            ...
            decode_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::PictureReady, decode_client_, picture));
            pending_picture_ready_.pop();
        } else if (!cleared || send_now) {
            DVLOGF(4) << "cleared=" << pending_picture_ready_.front().cleared << ", decoder_resetting_=" << decoder_resetting_
                      << ", decoder_flushing_=" << decoder_flushing_ << ", surface_set_change_pending_=" << surface_set_change_pending_
                      << ", picture_clearing_count_=" << picture_clearing_count_;
            DVLOGF(4) << "Posting picture ready to GPU for: " << picture.picture_buffer_id();
            ...
            child_task_runner_->PostTaskAndReply(FROM_HERE, base::BindOnce(&Client::PictureReady, client_, picture),
                // Unretained is safe. If Client::PictureReady gets to run, |this| is
                // alive. Destroy() will wait the decode thread to finish.
                base::BindOnce(&V4L2SliceVideoDecodeAccelerator::PictureCleared, base::Unretained(this)));
...
// static
VideoDecodeAccelerator::SupportedProfiles V4L2SliceVideoDecodeAccelerator::GetSupportedProfiles()
{
    scoped_refptr<V4L2Device> device = V4L2Device::Create();
    if (!device)
        return SupportedProfiles();

    return device->GetSupportedDecodeProfiles(std::size(supported_input_fourccs_), supported_input_fourccs_);
}

bool V4L2SliceVideoDecodeAccelerator::IsSupportedProfile(VideoCodecProfile profile)
{
    DCHECK(device_);
...
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    return std::count_if(output_buffer_map_.begin(), output_buffer_map_.end(), [](const auto& r) { return r.at_client(); });
}
...
bool V4L2SliceVideoDecodeAccelerator::ProcessFrame(V4L2ReadableBufferRef buffer, scoped_refptr<V4L2DecodeSurface> surface)
{
    DVLOGF(4);
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    if (!gfx::Rect(input_frame->coded_size()).Contains(visible_rect) || !input_frame->visible_rect().Contains(visible_rect)) {
        VLOGF(1) << "The visible rectangle is invalid!";
        return false;
    }
    if (!gfx::Rect(input_frame->natural_size()).Contains(gfx::Rect(natural_size))) {
        VLOGF(1) << "The natural size is too large!";
        return false;
    }
    scoped_refptr<VideoFrame> cropped_input_frame = VideoFrame::WrapVideoFrame(input_frame, input_frame->format(), visible_rect, natural_size);
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        ...
        scoped_refptr<VideoFrame> wrapped_frame
            = VideoFrame::WrapVideoFrame(output_frame, output_frame->format(), output_frame->visible_rect(), output_frame->visible_rect().size());
        DCHECK(wrapped_frame);

        image_processor_->Process(std::move(cropped_input_frame), std::move(wrapped_frame),
            base::BindOnce(&V4L2SliceVideoDecodeAccelerator::FrameProcessed, base::Unretained(this), surface, buffer->BufferId()));
    } else {
        ...
        if (!image_processor_->Process(
                std::move(cropped_input_frame), base::BindOnce(&V4L2SliceVideoDecodeAccelerator::FrameProcessed, base::Unretained(this), surface)))
            return false;
    }
...
void V4L2SliceVideoDecodeAccelerator::FrameProcessed(scoped_refptr<V4L2DecodeSurface> surface, size_t ip_buffer_index, scoped_refptr<VideoFrame> frame)
{
    DVLOGF(4);
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    // implemented.
    if (surfaces_at_ip_.empty() || surfaces_at_ip_.front().first != surface || output_buffer_map_.empty()) {
    ...
    if (ip_output_record.texture_id != 0 && !ip_output_record.cleared) {
        ...
        child_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&V4L2SliceVideoDecodeAccelerator::CreateGLImageFor, weak_this_, image_processor_device_, ip_buffer_index,
                ip_output_record.picture_id, CreateGpuMemoryBufferHandle(frame.get()).native_pixmap_handle, ip_output_record.client_texture_id,
                ip_output_record.texture_id, GetRectSizeFromOrigin(decoder_->GetVisibleRect()), *gl_image_format_fourcc_));
    }

    DCHECK(!surfaces_at_ip_.empty());
    DCHECK_EQ(surfaces_at_ip_.front().first, surface);
    V4L2ReadableBufferRef decoded_buffer = std::move(surfaces_at_ip_.front().second);
    surfaces_at_ip_.pop();
    DCHECK_EQ(decoded_buffer->BufferId(), static_cast<size_t>(surface->output_record()));
    ...
    frame->AddDestructionObserver(base::BindOnce(&V4L2SliceVideoDecodeAccelerator::ReuseOutputBuffer, base::Unretained(this), decoded_buffer));

    // This holds the IP video frame until everyone is done with it
    surface->SetReleaseCallback(base::BindOnce([](scoped_refptr<VideoFrame> frame) {}, frame));
...
bool V4L2SliceVideoDecodeAccelerator::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    // OnMemoryDump() must be performed on |decoder_thread_|.
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());

    // VIDEO_OUTPUT queue's memory usage.
    const size_t input_queue_buffers_count = input_queue_->AllocatedBuffersCount();
    size_t input_queue_memory_usage = 0;
    std::string input_queue_buffers_memory_type = V4L2MemoryToString(input_queue_->GetMemoryType());
    ...
    std::string output_queue_buffers_memory_type = V4L2MemoryToString(output_queue_->GetMemoryType());
    if (output_mode_ == Config::OutputMode::ALLOCATE) {
        ...
        for (uint32_t index = 0; index < output_buffer_map_.size(); ++index) {
            ...
            v4l2_buffer.length = std::min(output_planes_count_, std::size(v4l2_planes));
    ...
    const size_t total_usage = input_queue_memory_usage + output_queue_memory_usage;

    using ::base::trace_event::MemoryAllocatorDump;

    auto dump_name = base::StringPrintf("gpu/v4l2/slice_decoder/0x%" PRIxPTR, reinterpret_cast<uintptr_t>(this));

    MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
    dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(total_usage));
    dump->AddScalar("input_queue_memory_usage", MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(input_queue_memory_usage));
    dump->AddScalar("input_queue_buffers_count", MemoryAllocatorDump::kUnitsObjects, static_cast<uint64_t>(input_queue_buffers_count));
    dump->AddString("input_queue_buffers_memory_type", "", input_queue_buffers_memory_type);
    dump->AddScalar("output_queue_memory_usage", MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(output_queue_memory_usage));
    dump->AddScalar("output_queue_buffers_count", MemoryAllocatorDump::kUnitsObjects, static_cast<uint64_t>(output_queue_buffers_count));
    dump->AddString("output_queue_buffers_memory_type", "", output_queue_buffers_memory_type);
    return true;
}
