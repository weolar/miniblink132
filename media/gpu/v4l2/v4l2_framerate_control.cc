...
double GetUserFrameRate()
{
    ...
    const std::string framerate_str(cmd_line->GetSwitchValueASCII(switches::kHardwareVideoDecodeFrameRate));

    constexpr double kMaxFramerate = 120.0;
    double framerate = 0;
    if (base::StringToDouble(framerate_str, &framerate) && framerate >= 0.0 && framerate <= kMaxFramerate) {
        return framerate;
    }
    VLOG(1) << "Requested framerate is unreasonable: " << framerate << "fps, not overriding.";
    return 0.0;
}

bool FrameRateControlPresent(const media::V4L2FrameRateControl::IoctlAsCallback& ioctl_cb)
{
    ...
    if (user_framerate > 0.0) {
        ...
        VLOG(1) << "Overriding playback framerate. Set at " << user_framerate << " fps.";
    }
    ...
    return (user_framerate == 0) && (parms.parm.output.capability & V4L2_CAP_TIMEPERFRAME);
}
...
static constexpr base::TimeDelta kFrameIntervalFor120fps = base::Milliseconds(8);
static constexpr base::TimeDelta kFrameIntervalFor24fps = base::Milliseconds(41);

V4L2FrameRateControl::V4L2FrameRateControl(const IoctlAsCallback& ioctl_cb, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : ioctl_cb_(ioctl_cb)
...
void V4L2FrameRateControl::UpdateFrameRate()
{
    ...
    const base::TimeDelta frame_duration_avg = frame_duration_moving_average_.Mean();

    if (frame_duration_avg > kFrameIntervalFor120fps && frame_duration_avg < kFrameIntervalFor24fps
        ...
        TRACE_COUNTER_ID1("media,gpu", "V4L2 time per frame (ms)", this, current_frame_duration_avg_ms_);
        TRACE_COUNTER_ID1("media,gpu", "V4L2 estimated frame rate (Hz)", this, std::round(frame_duration_avg.ToHz()));
...
// static
void V4L2FrameRateControl::RecordFrameDurationThunk(base::WeakPtr<V4L2FrameRateControl> weak_this, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    if (task_runner->RunsTasksInCurrentSequence()) {
        ...
        task_runner->PostTask(FROM_HERE, base::BindOnce(&V4L2FrameRateControl::RecordFrameDuration, weak_this));
    }
}
...
void V4L2FrameRateControl::RecordFrameDuration()
{
    ...
    const base::TimeDelta duration = frame_display_time - last_frame_display_time_;
...
void V4L2FrameRateControl::AttachToVideoFrame(scoped_refptr<VideoFrame>& video_frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!framerate_control_present_)
        return;

    video_frame->AddDestructionObserver(base::BindOnce(&V4L2FrameRateControl::RecordFrameDurationThunk, weak_this_factory_.GetWeakPtr(), task_runner_));
}

void V4L2FrameRateControl::AttachToFrameResource(scoped_refptr<FrameResource>& frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!framerate_control_present_) {
        return;
    ...
    frame->AddDestructionObserver(base::BindOnce(&V4L2FrameRateControl::RecordFrameDurationThunk, weak_this_factory_.GetWeakPtr(), task_runner_));
}
