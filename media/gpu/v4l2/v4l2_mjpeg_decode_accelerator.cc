...
#define READ_U8_OR_RETURN_FALSE(reader, out)                                                                                                                   \
    do {                                                                                                                                                       \
        ...
        if (!reader.ReadU8BigEndian(_out)) {                                                                                                                   \
            DVLOGF(1) << "Error in stream: unexpected EOS while trying to read " #out;                                                                         \
            return false;                                                                                                                                      \
        }                                                                                                                                                      \
        out = _out;                                                                                                                                            \
    } while (0)
...
// can only be 1.
static_assert(kMaxInputPlanes == 1, "kMaxInputPlanes must be 1 as input must be V4L2_PIX_FMT_JPEG");
} // namespace
...
const uint8_t kDefaultDhtSeg[] = { ,
...
public:
    JobRecordBitstreamBuffer(BitstreamBuffer bitstream_buffer, scoped_refptr<VideoFrame> video_frame)
    ...
    int32_t task_id() const override
    {
        return task_id_;
    }
    size_t size() const override
    {
        return shm_region_.GetSize();
    }
    ...
    {
        return shm_mapping_.memory();
    }

    const scoped_refptr<VideoFrame>& out_frame() override
    {
        return out_frame_;
    }
...
public:
    JobRecordDmaBuf(int32_t task_id, base::ScopedFD src_dmabuf_fd, size_t src_size, off_t src_offset, scoped_refptr<VideoFrame> dst_frame)
        : task_id_(task_id)
    ...
    bool map() override
    {
        ...
        DCHECK_GT(size(), 0u);
        void* addr = mmap(nullptr, size(), PROT_READ, MAP_SHARED, dmabuf_fd_.get(), base::checked_cast<off_t>(offset()));
        if (addr == MAP_FAILED)
            return false;
        mapped_addr_ = addr;
        return true;
...
V4L2MjpegDecodeAccelerator::V4L2MjpegDecodeAccelerator(
    const scoped_refptr<V4L2Device>& device, const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner)
...
V4L2MjpegDecodeAccelerator::~V4L2MjpegDecodeAccelerator()
{
    ...
    if (decoder_task_runner_) {
        ...
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::DestroyTask, base::Unretained(this), &waiter));
        waiter.Wait();
    }
...
{
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::NotifyError, weak_ptr_, task_id, error));
}

void V4L2MjpegDecodeAccelerator::InitializeOnDecoderTaskRunner(
    chromeos_camera::MjpegDecodeAccelerator::Client* client, chromeos_camera::MjpegDecodeAccelerator::InitCB init_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_);
    ...
    if ((caps.capabilities & kCapsRequired) != kCapsRequired) {
        VLOGF(1) << "VIDIOC_QUERYCAP, caps check failed: 0x" << std::hex << caps.capabilities;
        std::move(init_cb).Run(false);
        return;
    }
    ...
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::StartDevicePoll, weak_ptr_for_decoder_));
    ...
    chromeos_camera::MjpegDecodeAccelerator::Client* client, chromeos_camera::MjpegDecodeAccelerator::InitCB init_cb)
{
    ...
    // destructor.
    decoder_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::WithBaseSyncPrimitives(), base::MayBlock() });
    ...
    decoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &V4L2MjpegDecodeAccelerator::InitializeOnDecoderTaskRunner, weak_ptr_for_decoder_, client, base::BindPostTaskToCurrentDefault(std::move(init_cb))));
}

void V4L2MjpegDecodeAccelerator::Decode(BitstreamBuffer bitstream_buffer, scoped_refptr<VideoFrame> video_frame)
{
    DVLOGF(4) << "input_id=" << bitstream_buffer.id() << ", size=" << bitstream_buffer.size();
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    ...
    if ((video_frame->visible_rect().width() & 1) || (video_frame->visible_rect().height() & 1)) {
        VLOGF(1) << "Output frame visible size has odd dimension";
        PostNotifyError(bitstream_buffer.id(), PLATFORM_FAILURE);
        return;
    }

    std::unique_ptr<JobRecord> job_record(new JobRecordBitstreamBuffer(std::move(bitstream_buffer), std::move(video_frame)));

    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::DecodeTask, weak_ptr_for_decoder_, std::move(job_record)));
}

void V4L2MjpegDecodeAccelerator::Decode(
    int32_t task_id, base::ScopedFD src_dmabuf_fd, size_t src_size, off_t src_offset, scoped_refptr<media::VideoFrame> dst_frame)
{
    ...
    if (src_offset < 0 || src_offset % page_size != 0) {
        VLOGF(1) << "Input buffer offset (" << src_offset << ") should be non-negative and aligned to page size (" << page_size << ")";
        PostNotifyError(task_id, INVALID_ARGUMENT);
        return;
    }
    ...
    if ((dst_frame->visible_rect().width() & 1) || (dst_frame->visible_rect().height() & 1)) {
        VLOGF(1) << "Output frame visible size has odd dimension";
        PostNotifyError(task_id, PLATFORM_FAILURE);
        return;
    }

    std::unique_ptr<JobRecord> job_record(new JobRecordDmaBuf(task_id, std::move(src_dmabuf_fd), src_size, src_offset, std::move(dst_frame)));
...
void V4L2MjpegDecodeAccelerator::DecodeTask(std::unique_ptr<JobRecord> job_record)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_);
    if (!job_record->map()) {
...
bool V4L2MjpegDecodeAccelerator::ShouldRecreateInputBuffers()
{
    ...
    return (input_buffer_map_.empty() || (job_record->size() + sizeof(kDefaultDhtSeg)) > input_buffer_map_.front().length[0]);
}
...
bool V4L2MjpegDecodeAccelerator::CreateInputBuffers()
{
    ...
    for (size_t i = 0; i < input_buffer_map_.size(); ++i) {
        ...
        for (size_t j = 0; j < buffer.length; ++j) {
            void* address = device_->Mmap(NULL, planes[j].length, PROT_READ | PROT_WRITE, MAP_SHARED, planes[j].m.mem_offset);
...
bool V4L2MjpegDecodeAccelerator::CreateOutputBuffers()
{
    ...
    size_t frame_size = VideoFrame::AllocationSize(PIXEL_FORMAT_I420, job_record->out_frame()->coded_size());
    struct v4l2_format format;
    std::ranges::fill(base::byte_span_from_ref(format), 0u);
    ...
    output_buffer_coded_size_.SetSize(format.fmt.pix_mp.width, format.fmt.pix_mp.height);
    ...
    if (!output_format) {
        VLOGF(1) << "unknown V4L2 pixel format: " << FourccToString(output_buffer_pixelformat_);
        PostNotifyError(kInvalidTaskId, PLATFORM_FAILURE);
        return false;
    }
    ...
    for (size_t i = 0; i < output_buffer_map_.size(); ++i) {
        ...
        for (size_t j = 0; j < buffer.length; ++j) {
            if (base::checked_cast<int64_t>(planes[j].length)
                < VideoFrame::PlaneSize(output_format->ToVideoPixelFormat(), j, gfx::Size(format.fmt.pix_mp.width, format.fmt.pix_mp.height)).GetArea()) {
                return false;
            }
            void* address = device_->Mmap(NULL, planes[j].length, PROT_READ | PROT_WRITE, MAP_SHARED, planes[j].m.mem_offset);
            if (address == MAP_FAILED) {
...
void V4L2MjpegDecodeAccelerator::DevicePollTask()
{
    ...
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::ServiceDeviceTask, weak_ptr_for_decoder_, event_pending));
}
...
void V4L2MjpegDecodeAccelerator::ServiceDeviceTask(bool event_pending)
{
    ...
    if (!running_jobs_.empty()) {
        device_poll_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::DevicePollTask, base::Unretained(this)));
    }

    DVLOGF(3) << "buffer counts: INPUT[" << input_jobs_.size() << "] => DEVICE[" << free_input_buffers_.size() << "/" << input_buffer_map_.size() << "->"
              << free_output_buffers_.size() << "/" << output_buffer_map_.size() << "]";
}
...
bool V4L2MjpegDecodeAccelerator::ConvertOutputImage(const BufferRecord& output_buffer, scoped_refptr<VideoFrame> dst_frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_);
    ...
    if (dst_frame->HasDmaBufs()) {
        std::unique_ptr<VideoFrameMapper> frame_mapper = VideoFrameMapperFactory::CreateMapper(dst_frame->format(), VideoFrame::STORAGE_DMABUFS);
        if (!frame_mapper) {
    ...
    if (output_buffer_num_planes_ == 1 && dst_frame->format() == PIXEL_FORMAT_I420) {
        DCHECK_EQ(dst_frame->layout().num_planes(), 3u);
        const auto format = Fourcc::FromV4L2PixFmt(output_buffer_pixelformat_);
        if (!format) {
            VLOGF(1) << "Unknown V4L2 format: " << FourccToString(output_buffer_pixelformat_);
            return false;
        }
        const size_t src_size = VideoFrame::AllocationSize(format->ToVideoPixelFormat(), output_buffer_coded_size_);
        if (libyuv::ConvertToI420(static_cast<uint8_t*>(output_buffer.address[0]), src_size, dst_ptrs[0], dst_strides[0], dst_ptrs[1], dst_strides[1],
                dst_ptrs[2], dst_strides[2], 0 /*x*/, 0 /*y*/, output_buffer_coded_size_.width(), output_buffer_coded_size_.height(), dst_width, dst_height,
                libyuv::kRotate0, output_buffer_pixelformat_)) {
            VLOGF(1) << "ConvertToI420 failed. Source format: " << FourccToString(output_buffer_pixelformat_);
            return false;
        }
        return true;
    }
    ...
    if (output_buffer_pixelformat_ == V4L2_PIX_FMT_YUV420M) {
        ...
        case PIXEL_FORMAT_I420:
            DCHECK_EQ(dst_frame->layout().num_planes(), 3u);
            if (libyuv::I420Copy(src_ptrs[0], src_strides[0], src_ptrs[1], src_strides[1], src_ptrs[2], src_strides[2], dst_ptrs[0], dst_strides[0],
                    dst_ptrs[1], dst_strides[1], dst_ptrs[2], dst_strides[2], dst_width, dst_height)) {
        ...
        case PIXEL_FORMAT_YV12:
            DCHECK_EQ(dst_frame->layout().num_planes(), 3u);
            if (libyuv::I420Copy(src_ptrs[0], src_strides[0], src_ptrs[1], src_strides[1], src_ptrs[2], src_strides[2], dst_ptrs[0], dst_strides[0],
                    dst_ptrs[2], dst_strides[2], dst_ptrs[1], dst_strides[1], dst_width, dst_height)) {
                VLOGF(1) << "I420Copy failed";
                return false;
            }
            break;
        case PIXEL_FORMAT_NV12:
            ...
            if (libyuv::I420ToNV12(src_ptrs[0], src_strides[0], src_ptrs[1], src_strides[1], src_ptrs[2], src_strides[2], dst_ptrs[0], dst_strides[0],
                    dst_ptrs[1], dst_strides[1], dst_width, dst_height)) {
        ...
        case PIXEL_FORMAT_I420:
            DCHECK_EQ(dst_frame->layout().num_planes(), 3u);
            if (libyuv::I422ToI420(src_ptrs[0], src_strides[0], src_ptrs[1], src_strides[1], src_ptrs[2], src_strides[2], dst_ptrs[0], dst_strides[0],
                    dst_ptrs[1], dst_strides[1], dst_ptrs[2], dst_strides[2], dst_width, dst_height)) {
        ...
        case PIXEL_FORMAT_NV12:
            DCHECK_EQ(dst_frame->layout().num_planes(), 2u);
            if (libyuv::I422ToNV21(src_ptrs[0], src_strides[0], src_ptrs[2], src_strides[2], src_ptrs[1], src_strides[1], dst_ptrs[0], dst_strides[0],
                    dst_ptrs[1], dst_strides[1], dst_width, dst_height)) {
        ...
        VLOGF(1) << "Unsupported source buffer format: " << FourccToString(output_buffer_pixelformat_);
        return false;
    }
    return true;
}
...
void V4L2MjpegDecodeAccelerator::Dequeue()
{
    ...
    while (!running_jobs_.empty() && OutputBufferQueuedCount() > 0) {
        ...
        if (dqbuf.flags & V4L2_BUF_FLAG_ERROR) {
            ...
            DVLOGF(4) << "Decoding finished, returning bitstream buffer, id=" << job_record->task_id();
            ...
            job_record.reset();
            io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2MjpegDecodeAccelerator::VideoFrameReady, weak_ptr_, task_id));
        }
    }
}

static bool AddHuffmanTable(base::span<const uint8_t> input, base::span<uint8_t> output)
{
    ...
    while (!has_marker_sos && !has_marker_dht) {
        ...
        if (size < sizeof(size)) {
            DVLOGF(1) << ": Ill-formed JPEG. Segment size (" << size << ") is smaller than size field (" << sizeof(size) << ")";
            return false;
        }
        size -= sizeof(size);
        ...
        if (!reader.Skip(size)) {
            DVLOGF(1) << "Ill-formed JPEG. Remaining size (" << reader.remaining() << ") is smaller than header specified (" << size << ")";
            return false;
        }
...
bool V4L2MjpegDecodeAccelerator::EnqueueInputRecord()
{
    ...
    if (!AddHuffmanTable(
            ...
            // a pointer.
            UNSAFE_TODO(base::span(static_cast<const uint8_t*>(job_record->memory()), job_record->size())),
            // SAFETY: BufferRecord has an array of pointer + length pairs. The
            // length is the number of elements at the matching pointer.
            UNSAFE_BUFFERS(base::span(static_cast<uint8_t*>(input_record.address[0]), input_record.length[0])))) {
