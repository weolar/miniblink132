...
TEST(MT21UtilTest, TestFastReadGolombRiceSymbol)
{
    ...
    ASSERT_TRUE(FastReadGolombRiceSymbol(reader, k, cache + (k - 1) * kGolombRiceTableSize) == 0);
    ASSERT_TRUE(FastReadGolombRiceSymbol(reader, k, cache + (k - 1) * kGolombRiceTableSize) == 1);
    ASSERT_TRUE(FastReadGolombRiceSymbol(reader, k, cache + (k - 1) * kGolombRiceTableSize) == -1);
    ASSERT_TRUE(FastReadGolombRiceSymbol(reader, k, cache + (k - 1) * kGolombRiceTableSize) == 2);
    ASSERT_TRUE(FastReadGolombRiceSymbol(reader, k, cache + (k - 1) * kGolombRiceTableSize) == -18);
}
...
TEST(MT21UtilTest, TestVectorReadGolombRiceSymbol)
{
    ...
    uint8x16_t symbol;
    symbol = VectorReadGolombRiceSymbol(accumulator, outstanding_reads, escape_codes, escape_binary_len_diff, k_vals, dword_solid_color_mask, compressed_ptr);
    for (int i = 0; i < 16; i++) {
...
TEST(MT21UtilTest, TestVectorPredictionMethods)
{
    ...
    pred = VectorFirstColPrediction(vdupq_n_u8(0x80), vdupq_n_u8(0x7E), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x72);
    }
    pred = VectorFirstColPrediction(vdupq_n_u8(0x80), vdupq_n_u8(0x83), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x80);
    }
    pred = VectorFirstColPrediction(vdupq_n_u8(0x80), vdupq_n_u8(0x6F), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x70);
    }

    pred = VectorBodyPrediction(vdupq_n_u8(0x81), vdupq_n_u8(0x80), vdupq_n_u8(0x7E), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x72);
    }
    pred = VectorBodyPrediction(vdupq_n_u8(0x7F), vdupq_n_u8(0x80), vdupq_n_u8(0x83), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x71);
    }
    pred = VectorBodyPrediction(vdupq_n_u8(0x82), vdupq_n_u8(0x80), vdupq_n_u8(0x83), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x80);
    }
    pred = VectorBodyPrediction(vdupq_n_u8(0x6F), vdupq_n_u8(0x80), vdupq_n_u8(0x83), vdupq_n_u8(0x70));
    for (int i = 0; i < 16; i++) {
        ASSERT_TRUE(pred[i] == 0x70);
    }
}
...
{
    uint8_t footer[kMT21ScratchMemorySize] __attribute__((aligned(kMT21YFooterAlignment)));
    footer[0] = 0b00000111;
    std::vector<MT21YSubblock> bins[2];

    ASSERT_TRUE(ComputeFooterOffset(512, sizeof(footer), kMT21YFooterAlignment) == 0);
