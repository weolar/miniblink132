...
void PopulateGolombRiceCache(GolombRiceTableEntry* cache)
{
    ...
    for (size_t k = 1; k < kMaxKValue; k++) {
        ...
        for (size_t lookahead_val = 0; lookahead_val < (1 << kGolombRiceTableLookaheadLen); lookahead_val++) {
            ...
            // 2 bytes.
            tmp_buf[kMT21SubblockWidth - 1] = lookahead_val >> (kGolombRiceTableLookaheadLen - kBitsInByte);
            tmp_buf[kMT21SubblockWidth - 2] = (lookahead_val << (kBitsInByte - (kGolombRiceTableLookaheadLen - kBitsInByte))) & 0xFF;
...
int FastReadGolombRiceSymbol(MT21BitstreamReader& reader, int k, const GolombRiceTableEntry* table)
{
...
// values.
uint8_t BodyPrediction(uint8_t up_left, uint8_t up, uint8_t up_right, uint8_t right)
{
    int max_up_right = up > right ? up : right;
    int min_up_right = up > right ? right : up;
    ...
    int idx = (use_right_grad << 1 | use_right_grad) | (up_left > max_up_right) << 1 | up_left < min_up_right;
    uint8_t ret[4];
    ret[0b00] = left_grad;
    ret[0b01] = min_up_right;
    ret[0b10] = max_up_right;
...
template <int width> void DecompressSubblock(MT21BitstreamReader& reader, uint8_t* dest, const GolombRiceTableEntry* symbol_cache)
{
    ...
    // manually.
    const GolombRiceTableEntry* symbol_table = symbol_cache + (k - 1) * kGolombRiceTableSize;
    ...
    for (int x = width - 2; x >= 0; x--) {
        curr = FirstRowPrediction(right) + FastReadGolombRiceSymbol(reader, k, symbol_table);
        right = curr;
        dest[x] = curr;
    }
    for (size_t y = 1; y < kMT21SubblockHeight; y++) {
        up = dest[y * width - 1];
        curr = LastColPrediction(up) + FastReadGolombRiceSymbol(reader, k, symbol_table);
        ...
        for (size_t x = width - 2; x >= 1; x--) {
            ...
            curr = BodyPrediction(up_left, up, up_right, right) + FastReadGolombRiceSymbol(reader, k, symbol_table);
            dest[y * width + x] = curr;
            right = curr;
            up_right = up;
            up = up_left;
        }
        dest[y * width] = FirstColPrediction(up, up_right, right) + FastReadGolombRiceSymbol(reader, k, symbol_table);
    }
}
...
template <typename T> void DecompressSubblockHelper(T subblock, const GolombRiceTableEntry* symbol_cache);
...
void InterleaveUVSubblock(const uint8_t* src_u, const uint8_t* src_v, uint8_t* dest_uv)
{
...
template <> void DecompressSubblockHelper(MT21UVSubblock subblock, const GolombRiceTableEntry* symbol_cache)
{
    MT21BitstreamReader reader(subblock.src);
...
__attribute__((always_inline)) uint8x16_t NarrowToU8(uint32x4_t& vec1, uint32x4_t& vec2, uint32x4_t& vec3, uint32x4_t& vec4)
{
    return vcombine_u8(vmovn_u16(vcombine_u16(vmovn_u32(vec1), vmovn_u32(vec2))), vmovn_u16(vcombine_u16(vmovn_u32(vec3), vmovn_u32(vec4))));
}
...
__attribute__((always_inline)) void VectorManageAccumulator(
    uint32x4_t* accumulator, uint32x4_t* outstanding_reads, const uint32x4_t& discard_size, int i, uint8_t** compressed_ptr)
{
    ...
    accumulator[i] = vshlq_u32(accumulator[i], vreinterpretq_s32_u32(outstanding_reads[i]));
}
...
__attribute__((always_inline)) uint8x16_t VectorReadGolombRiceSymbol(uint32x4_t* accumulator, uint32x4_t* outstanding_reads, uint32x4_t* escape_codes,
    uint32x4_t* escape_binary_len_diff, uint32x4_t* k_vals, uint32x4_t* dword_solid_color_mask, uint8_t** compressed_ptr)
{
    ...
    LOOPN(
        {
            leading_ones[i] = vminq_u32(vclzq_u32(vmvnq_u32(accumulator[i])), escape_codes[i]);
    ...
    LOOPN(
        {
            binary_len[i] = vaddq_u32(k_vals[i], vandq_u32(escape_lanes[i], escape_binary_len_diff[i]));
            unary_len[i] = vaddq_u32(leading_ones[i], vandq_u32(dword_literal_1, vmvnq_u32(escape_lanes[i])));
        },
        4)
    ...
    LOOPN(
        {
            dword_output[i] = vshlq_u32(leading_ones[i], vreinterpretq_s32_u32(k_vals[i]));
            dword_output[i] = vaddq_u32(dword_output[i],
                vshlq_u32(vshlq_u32(accumulator[i], vreinterpretq_s32_u32(unary_len[i])), vsubq_s32(vreinterpretq_s32_u32(binary_len[i]), dword_literal_32)));
        },
        4)
    ...
    LOOPN(
        {
            total_len[i] = vsubq_u32(vaddq_u32(unary_len[i], binary_len[i]), vandq_u32(dword_literal_1, vcleq_u32(dword_output[i], dword_literal_1)));
            total_len[i] = vandq_u32(total_len[i], dword_solid_color_mask[i]);
        },
        4)

    // Handle accumulator.
    LOOPN({ VectorManageAccumulator(accumulator, outstanding_reads, total_len[i], i, compressed_ptr); }, 4)

    // output = (output / 2) * (output % 2 ? -1 : 1)
    // This is a hack that relies on how two's complement arithmetic works.
    uint8x16_t output = NarrowToU8(dword_output[0], dword_output[1], dword_output[2], dword_output[3]);
    uint8x16_t negative_lanes = vandq_u8(output, byte_literal_1);
    output = vaddq_u8(veorq_u8(vshrq_n_u8(output, 1), vtstq_u8(negative_lanes, negative_lanes)), negative_lanes);

    return output;
}

// Initializes the accumulator with the first 4 bytes of compressed data.
__attribute__((always_inline)) void VectorInitializeAccumulator(uint32x4_t* accumulator, uint8_t** compressed_ptr)
{
    LOOPN(
        {
            accumulator[i][0] = LoadUnalignedDword((uint32_t*)compressed_ptr[i * 4]);
...
// vectors.
__attribute__((always_inline)) uint8x16_t VectorReadCompressedHeader(uint32x4_t* accumulator, uint32x4_t* outstanding_reads, uint32x4_t* escape_codes,
    uint32x4_t* escape_binary_len_diff, uint32x4_t* k_vals, uint32x4_t* dword_solid_color_mask, uint8x16_t& solid_color_mask, uint8_t** compressed_ptr)
{
    // Parse out our K value.
    // k = (accumulator >> 29) + 1
    // 29 comes from 32 - 3, since 32 is the size of the accumulator and 3 is the
    // size of k.
    LOOPN({ k_vals[i] = vaddq_u32(vshrq_n_u32(accumulator[i], 32 - 3), dword_literal_1); }, 4)
    ...
    LOOPN({ escape_binary_len_diff[i] = vaddq_u32(vsubq_u32(dword_literal_8, k_vals[i]), vcgeq_u32(k_vals[i], dword_literal_4)); }, 4)
    ...
    LOOPN({ dword_solid_color_mask[i] = vcltq_u32(k_vals[i], dword_literal_8); }, 4)
    solid_color_mask = NarrowToU8(dword_solid_color_mask[0], dword_solid_color_mask[1], dword_solid_color_mask[2], dword_solid_color_mask[3]);
    ...
    LOOPN({ VectorManageAccumulator(accumulator, outstanding_reads, dword_literal_11, i, compressed_ptr); }, 4)
...
__attribute__((always_inline)) uint8x16_t VectorFirstRowPrediction(const uint8x16_t& right)
{
    return right;
}

__attribute__((always_inline)) uint8x16_t VectorLastColPrediction(const uint8x16_t& up)
{
    return up;
}

__attribute__((always_inline)) uint8x16_t VectorFirstColPrediction(const uint8x16_t& up, const uint8x16_t& up_right, const uint8x16_t& right)
{
    const uint8x16_t min_pred = vminq_u8(up, right);
    const uint8x16_t max_pred = vmaxq_u8(up, right);
    const uint8x16_t right_grad = vreinterpretq_u8_s8(vaddq_s8(right, vsubq_s8(vreinterpretq_s8_u8(up), vreinterpretq_s8_u8(up_right))));
    ...
    pred = vbslq_u8(vorrq_u8(up_right_above_max, up_right_below_min), pred, right_grad);
    return pred;
}

__attribute__((always_inline)) uint8x16_t VectorBodyPrediction(
    const uint8x16_t& up_left, const uint8x16_t& up, const uint8x16_t& up_right, const uint8x16_t& right)
{
    uint8x16_t min_pred = vminq_u8(up, right);
    uint8x16_t max_pred = vmaxq_u8(up, right);
    uint8x16_t right_grad = vreinterpretq_u8_s8(vaddq_s8(vreinterpretq_s8_u8(right), vsubq_s8(vreinterpretq_s8_u8(up), vreinterpretq_s8_u8(up_right))));
    uint8x16_t left_grad = vreinterpretq_u8_s8(vaddq_s8(vreinterpretq_s8_u8(right), vsubq_s8(vreinterpretq_s8_u8(up), vreinterpretq_s8_u8(up_left))));
    uint8x16_t up_left_above_max = vcgtq_u8(up_left, max_pred);
    uint8x16_t up_left_below_min = vcltq_u8(up_left, min_pred);
    uint8x16_t use_right_grad = vandq_u8(vcleq_u8(up_right, max_pred), vcgeq_u8(up_right, min_pred));
    uint8x16_t pred = vbslq_u8(up_left_above_max, max_pred, min_pred);
    pred = vbslq_u8(vorrq_u8(up_left_above_max, up_left_below_min), pred, left_grad);
...
template <class T> void SubblockGather(const std::vector<T>& subblock_list, int start_idx, uint8_t* aligned_scratch_memory, uint8_t** compressed_ptr)
{
    ...
    for (size_t i = 0; i < kNumOutputLanes; i++) {
        ...
        for (size_t j = 0; j < subblock_list[i + start_idx].len; j += kMT21SubblockWidth) {
            memcpy(compressed_ptr[i] + 3 * kMT21SubblockWidth - j, subblock_list[i + start_idx].src + j, kMT21SubblockWidth);
        }
...
void SubblockTransposeScatter(uint8_t*& src, uint8_t** decompressed_ptr)
{
    ...
    LOOPN(
        {
            uint8x8x2_t table;
            table.val[0] = vget_low_u8(vreinterpretq_u8_u32(load_regs[i / 4].val[i % 4]));
            table.val[1] = vget_high_u8(vreinterpretq_u8_u32(load_regs[i / 4].val[i % 4]));
            load_regs[i / 4].val[i % 4] = vcombine_u32(
                vreinterpret_u32_u8(vtbl2_u8(table, kTableTranspose4x4UpperIndices)), vreinterpret_u32_u8(vtbl2_u8(table, kTableTranspose4x4LowerIndices)));
        },
        16)
...
void VectorDecompressSubblock(
    const std::vector<T>& subblock_list, int start_idx, uint8_t** compressed_ptr, uint8_t* output_buf, uint32x4_t* outstanding_reads, uint32x4_t* accumulator)
{
    ...
    output = VectorReadCompressedHeader(
        accumulator, outstanding_reads, escape_codes, escape_binary_len_diff, k_vals, dword_solid_color_mask, solid_color_mask, compressed_ptr);
    right = output;
    ...
    for (int i = 0; i < width - 1; i++) {
        // Handle first row
        output
            = VectorReadGolombRiceSymbol(accumulator, outstanding_reads, escape_codes, escape_binary_len_diff, k_vals, dword_solid_color_mask, compressed_ptr);
    ...
    for (size_t y = 1; y < kMT21SubblockHeight; y++) {
        ...
        for (int x = width - 2; x >= 1; x--) {
            // Handle body
            up_left = vld1q_u8(output_buf + (stride - pixel_distance) * kNumOutputLanes);

            output = VectorReadGolombRiceSymbol(
                accumulator, outstanding_reads, escape_codes, escape_binary_len_diff, k_vals, dword_solid_color_mask, compressed_ptr);

            output = vandq_u8(output, solid_color_mask);
            output = vaddq_u8(output, VectorBodyPrediction(up_left, up, up_right, right));
...
template <class T> void VectorDecompressSubblockHelper(const std::vector<T>& subblock_list, int start_idx, uint8_t* aligned_scratch)
{
}
template <> void VectorDecompressSubblockHelper(const std::vector<MT21YSubblock>& subblock_list, int start_idx, uint8_t* aligned_scratch)
{
    uint8_t* compressed_ptr[kNumOutputLanes];
    uint8_t* decompressed_ptr[kNumOutputLanes];
    uint8_t* output_buf = aligned_scratch + 2 * kMT21RedZoneSize + kNumOutputLanes * kMT21SubblockSize - kNumOutputLanes;
    uint32x4_t outstanding_reads[4] = { { 0 } };
    uint32x4_t accumulator[4];

    SubblockGather<MT21YSubblock>(subblock_list, start_idx, aligned_scratch, compressed_ptr);
    for (size_t i = 0; i < kNumOutputLanes; i++) {
    ...
    VectorDecompressSubblock<MT21YSubblock, kMT21SubblockWidth, kMT21SubblockWidth>(
        subblock_list, start_idx, compressed_ptr, output_buf, outstanding_reads, accumulator);
...
template <> void VectorDecompressSubblockHelper(const std::vector<MT21UVSubblock>& subblock_list, int start_idx, uint8_t* aligned_scratch)
{
    ...
    SubblockGather<MT21UVSubblock>(subblock_list, start_idx, aligned_scratch, compressed_ptr);
    for (int i = 0; i < 16; i++) {
        decompressed_ptr[i] = subblock_list[start_idx + i].dest;
    }
    ...
    VectorDecompressSubblock<MT21UVSubblock, kMT21SubblockWidth / 2, kMT21SubblockWidth>(
        subblock_list, start_idx, compressed_ptr, output_buf, outstanding_reads, accumulator);
    VectorDecompressSubblock<MT21UVSubblock, kMT21SubblockWidth / 2, kMT21SubblockWidth>(
        subblock_list, start_idx, compressed_ptr, output_buf - 16, outstanding_reads, accumulator);
...
size_t ComputeFooterOffset(size_t plane_size, size_t buf_size, size_t alignment)
{
...
void ParseBlockMetadata(const uint8_t* footer, size_t block_offset, size_t& subblock1_len, size_t& subblock2_len)
{
    ...
    subblock1_len = kMT21BlockWidth * (((footer[block_idx / 2] >> ((block_idx % 2) * 4)) & 0x3) + 1);
    subblock2_len = kMT21BlockWidth * (((footer[block_idx / 2] >> ((block_idx % 2) * 4 + 2)) & 0x3) + 1);
}
...
// routines.
template <class T> void BinSubblocks(const uint8_t* src, const uint8_t* footer, uint8_t* dest, size_t block_offset, std::vector<T>* subblock_bins)
{
    ...
    T subblock2 = { src + block_offset + subblock1_len, dest + block_offset + kMT21SubblockSize, subblock2_len };
