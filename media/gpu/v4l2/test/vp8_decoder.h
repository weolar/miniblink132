...
public:
    ...
    static std::unique_ptr<Vp8Decoder> Create(const base::MemoryMappedFile& stream);
...
private:
    Vp8Decoder(std::unique_ptr<IvfParser> ivf_parser, std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution);
    ...
    std::set<int> RefreshReferenceSlots(const Vp8FrameHeader& frame_hdr, MmappedBuffer* buffer, std::set<uint32_t> queued_buffer_ids);
    ...
    void UpdateReusableReferenceBufferSlots(const Vp8FrameHeader& frame_hdr, const size_t curr_ref_frame_index, std::set<int>& reusable_buffer_slots);
