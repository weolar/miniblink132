...
public:
    ...
    static std::unique_ptr<Vp9Decoder> Create(const base::MemoryMappedFile& stream);
...
private:
    Vp9Decoder(std::unique_ptr<IvfParser> ivf_parser, std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution);

    // Reads next frame from IVF stream and its size into |vp9_frame_header|
    // and |size| respectively.
    Vp9Parser::Result ReadNextFrame(Vp9FrameHeader& vp9_frame_header, gfx::Size& size);

    // Copies the frame data into the V4L2 buffer of OUTPUT |queue|.
    void CopyFrameData(const Vp9FrameHeader& frame_hdr, std::unique_ptr<V4L2Queue>& queue);

    // Sets up per frame parameters |v4l2_frame_params| needed for VP9 decoding
    // with VIDIOC_S_EXT_CTRLS ioctl call.
    void SetupFrameParams(const Vp9FrameHeader& frame_hdr, struct v4l2_ctrl_vp9_frame* v4l2_frame_params);
    ...
    std::set<int> RefreshReferenceSlots(uint8_t refresh_frame_flags, scoped_refptr<MmappedBuffer> buffer, uint32_t last_queued_buffer_id);
