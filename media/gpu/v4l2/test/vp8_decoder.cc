...
static_assert(kVp8FrameLast == base::strict_cast<TypeOfVp8RefType>(media::VP8_FRAME_LAST), "Invalid index value for Last reference frame");

static_assert(kVp8FrameGolden == base::strict_cast<TypeOfVp8RefType>(media::VP8_FRAME_GOLDEN), "Invalid index value for Golden reference frame");

static_assert(kVp8FrameAltref == base::strict_cast<TypeOfVp8RefType>(media::VP8_FRAME_ALTREF), "Invalid index value for Altref reference frame");
...
const gfx::Size GetResolutionFromBitstream(const base::MemoryMappedFile& stream)
{
    ...
    vp8_parser.ParseFrame(ivf_frame_data, ivf_frame_header.frame_size, &vp8_frame_header);
...
struct v4l2_vp8_loop_filter FillV4L2VP8LoopFilterHeader(const media::Vp8LoopFilterHeader& vp8_lf_hdr)
{
    ...
    static_assert(std::size(decltype(v4l2_lf.ref_frm_delta) {}) == media::kNumBlockContexts, "Invalid size of ref_frm_delta");

    static_assert(std::size(decltype(v4l2_lf.mb_mode_delta) {}) == media::kNumBlockContexts, "Invalid size of mb_mode_delta");
...
struct v4l2_vp8_quantization FillV4L2Vp8QuantizationHeader(const media::Vp8QuantizationHeader& vp8_quantization_hdr)
{
    struct v4l2_vp8_quantization v4l2_quant = {};

    v4l2_quant.y_ac_qi = base::checked_cast<__u8>(vp8_quantization_hdr.y_ac_qi);
    v4l2_quant.y_dc_delta = base::checked_cast<__s8>(vp8_quantization_hdr.y_dc_delta);
    v4l2_quant.y2_dc_delta = base::checked_cast<__s8>(vp8_quantization_hdr.y2_dc_delta);
    v4l2_quant.y2_ac_delta = base::checked_cast<__s8>(vp8_quantization_hdr.y2_ac_delta);
    v4l2_quant.uv_dc_delta = base::checked_cast<__s8>(vp8_quantization_hdr.uv_dc_delta);
    v4l2_quant.uv_ac_delta = base::checked_cast<__s8>(vp8_quantization_hdr.uv_ac_delta);

    return v4l2_quant;
}

// Section 9.9.  DCT Coefficient Probability Update
struct v4l2_vp8_entropy FillV4L2VP8EntropyHeader(const media::Vp8EntropyHeader& vp8_entropy_hdr)
{
    struct v4l2_vp8_entropy v4l2_entr = {};

    static_assert(std::size(decltype(v4l2_entr.coeff_probs) {}) == media::kNumBlockTypes, "Invalid size of coeff_probs");

    static_assert(std::size(decltype(v4l2_entr.y_mode_probs) {}) == media::kNumYModeProbs, "Invalid size of y_mode_probs");

    static_assert(std::size(decltype(v4l2_entr.uv_mode_probs) {}) == media::kNumUVModeProbs, "Invalid size of uv_mode_probs");

    static_assert(std::size(decltype(v4l2_entr.mv_probs) {}) == media::kNumMVContexts, "Invalid size of mv_probs");
    ...
    media::SafeArrayMemcpy(v4l2_entr.uv_mode_probs, vp8_entropy_hdr.uv_mode_probs);
    media::SafeArrayMemcpy(v4l2_entr.mv_probs, vp8_entropy_hdr.mv_probs);
    return v4l2_entr;
}

// Section 9.3. Segment-Based Adjustments
struct v4l2_vp8_segment FillV4L2VP8SegmentationHeader(const media::Vp8SegmentationHeader& vp8_segmentation_hdr)
{
    ...
    if (vp8_segmentation_hdr.segment_feature_mode == media::Vp8SegmentationHeader::FEATURE_MODE_DELTA) {
        v4l2_segment.flags |= V4L2_VP8_SEGMENT_FLAG_DELTA_VALUE_MODE;
    }

    static_assert(std::size(decltype(v4l2_segment.quant_update) {}) == media::kMaxMBSegments, "Invalid size of quant_update");

    static_assert(std::size(decltype(v4l2_segment.lf_update) {}) == media::kMaxMBSegments, "Invalid size of lf_update");

    static_assert(std::size(decltype(v4l2_segment.segment_probs) {}) == media::kNumMBFeatureTreeProbs, "Invalid size of segment_probs");

    media::SafeArrayMemcpy(v4l2_segment.quant_update, vp8_segmentation_hdr.quantizer_update_value);
    media::SafeArrayMemcpy(v4l2_segment.lf_update, vp8_segmentation_hdr.lf_update_value);
    media::SafeArrayMemcpy(v4l2_segment.segment_probs, vp8_segmentation_hdr.segment_prob);
...
// frames
bool IsBufferSlotInUse(const media::Vp8FrameHeader& frame_hdr,
    const std::array<scoped_refptr<media::v4l2_test::MmappedBuffer>, media::kNumVp8ReferenceBuffers>& ref_frames, size_t curr_ref_frame_index)
{
    for (size_t i = 0; i < media::kNumVp8ReferenceBuffers; i++) {
        ...
        case kVp8FrameAltref:
            is_frame_not_refreshed = !frame_hdr.refresh_alternate_frame && frame_hdr.copy_buffer_to_alternate == media::Vp8FrameHeader::NO_ALT_REFRESH;
            break;
        case kVp8FrameGolden:
            is_frame_not_refreshed = !frame_hdr.refresh_golden_frame && frame_hdr.copy_buffer_to_golden == media::Vp8FrameHeader::NO_GOLDEN_REFRESH;
            break;
        case kVp8FrameLast:
        ...
        const bool is_candidate_in_use = (ref_frames[i]->buffer_id() == ref_frames[curr_ref_frame_index]->buffer_id());
...
Vp8Decoder::Vp8Decoder(std::unique_ptr<IvfParser> ivf_parser, std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution)
...
// static
std::unique_ptr<Vp8Decoder> Vp8Decoder::Create(const base::MemoryMappedFile& stream)
{
    ...
    LOG(INFO) << "Ivf file header: " << gfx::Size(file_header.width, file_header.height).ToString();

    return base::WrapUnique(new Vp8Decoder(std::move(ivf_parser), std::move(v4l2_ioctl), bitstream_coded_size));
}

struct v4l2_ctrl_vp8_frame Vp8Decoder::SetupFrameHeaders(const Vp8FrameHeader& frame_hdr)
{
    struct v4l2_ctrl_vp8_frame v4l2_frame_headers = {};

    v4l2_frame_headers.lf = FillV4L2VP8LoopFilterHeader(frame_hdr.loopfilter_hdr);
    v4l2_frame_headers.quant = FillV4L2Vp8QuantizationHeader(frame_hdr.quantization_hdr);
    ...
    static_assert(std::size(decltype(v4l2_frame_headers.dct_part_sizes) {}) == media::kMaxDCTPartitions, "Invalid size of dct_part_sizes");

    for (size_t i = 0; i < frame_hdr.num_of_dct_partitions && i < std::size(v4l2_frame_headers.dct_part_sizes); ++i) {
        v4l2_frame_headers.dct_part_sizes[i] = static_cast<size_t>(frame_hdr.dct_partition_sizes[i]);
    }

    v4l2_frame_headers.entropy = FillV4L2VP8EntropyHeader(frame_hdr.entropy_hdr);
    v4l2_frame_headers.segment = FillV4L2VP8SegmentationHeader(frame_hdr.segmentation_hdr);
    ...
    v4l2_frame_headers.last_frame_ts = ref_frames_[kVp8FrameLast] ? (ref_frames_[kVp8FrameLast]->frame_number() * kTimestampToNanoSecs) : kInvalidSurface;
    v4l2_frame_headers.golden_frame_ts = ref_frames_[kVp8FrameGolden] ? (ref_frames_[kVp8FrameGolden]->frame_number() * kTimestampToNanoSecs) : kInvalidSurface;
    v4l2_frame_headers.alt_frame_ts = ref_frames_[kVp8FrameAltref] ? (ref_frames_[kVp8FrameAltref]->frame_number() * kTimestampToNanoSecs) : kInvalidSurface;

    return v4l2_frame_headers;
}

void Vp8Decoder::UpdateReusableReferenceBufferSlots(const Vp8FrameHeader& frame_hdr, const size_t curr_ref_frame_index, std::set<int>& reusable_buffer_slots)
{
    const auto reusable_candidate_buffer_id = ref_frames_[curr_ref_frame_index]->buffer_id();
    reusable_buffer_slots.insert(base::checked_cast<int>(reusable_candidate_buffer_id));

    bool is_buffer_slot_copied = false;
    switch (curr_ref_frame_index) {
    case kVp8FrameAltref:
        is_buffer_slot_copied = frame_hdr.copy_buffer_to_golden == Vp8FrameHeader::COPY_ALT_TO_GOLDEN;
        break;
    case kVp8FrameGolden:
        is_buffer_slot_copied = frame_hdr.copy_buffer_to_alternate == Vp8FrameHeader::COPY_GOLDEN_TO_ALT;
        break;
    case kVp8FrameLast:
        is_buffer_slot_copied = (frame_hdr.copy_buffer_to_alternate == Vp8FrameHeader::COPY_LAST_TO_ALT)
            || (frame_hdr.copy_buffer_to_golden == Vp8FrameHeader::COPY_LAST_TO_GOLDEN);
        break;
    default:
        NOTREACHED() << "Invalid reference frame index";
    }
    const bool is_buffer_slot_in_use = IsBufferSlotInUse(frame_hdr, ref_frames_, curr_ref_frame_index);

    if (is_buffer_slot_copied || is_buffer_slot_in_use)
        reusable_buffer_slots.erase(base::checked_cast<int>(reusable_candidate_buffer_id));
}

std::set<int> Vp8Decoder::RefreshReferenceSlots(const Vp8FrameHeader& frame_hdr, MmappedBuffer* buffer, std::set<uint32_t> queued_buffer_ids)
{
    ...
    if (frame_hdr.refresh_alternate_frame) {
        UpdateReusableReferenceBufferSlots(frame_hdr, kVp8FrameAltref, reusable_buffer_slots);
        ...
        case Vp8FrameHeader::NO_ALT_REFRESH:
            ...
            break;
        default:
            NOTREACHED() << "Invalid flag to refresh altenate frame: " << frame_hdr.copy_buffer_to_alternate;
        }
    }

    if (frame_hdr.refresh_golden_frame) {
        UpdateReusableReferenceBufferSlots(frame_hdr, kVp8FrameGolden, reusable_buffer_slots);
        ...
        case Vp8FrameHeader::NO_GOLDEN_REFRESH:
            ...
            break;
        default:
            NOTREACHED() << "Invalid flag to refresh golden frame: " << frame_hdr.copy_buffer_to_golden;
        }
    }

    if (frame_hdr.refresh_last) {
        UpdateReusableReferenceBufferSlots(frame_hdr, kVp8FrameLast, reusable_buffer_slots);
...
Vp8Decoder::ParseResult Vp8Decoder::ReadNextFrame(Vp8FrameHeader& vp8_frame_header)
{
    ...
    const bool result = vp8_parser_->ParseFrame(ivf_frame_data, ivf_frame_header.frame_size, &vp8_frame_header);
    ...
    const int frame_number, std::vector<uint8_t>& y_plane, std::vector<uint8_t>& u_plane, std::vector<uint8_t>& v_plane, gfx::Size& size, BitDepth& bit_depth)
{
    ...
    const bool resolution_changed = frame_hdr.width != OUTPUT_queue_->resolution().width() || frame_hdr.height != OUTPUT_queue_->resolution().height();
    if (frame_hdr.IsKeyframe() && resolution_changed) {
        const gfx::Size new_resolution(frame_hdr.width, frame_hdr.height);
        LOG_ASSERT(!new_resolution.IsEmpty()) << "New key frame resolution is empty.";
    ...
    scoped_refptr<MmappedBuffer> OUTPUT_queue_buffer = OUTPUT_queue_->GetBuffer(buffer_id);
    OUTPUT_queue_buffer->mmapped_planes()[0].CopyIn(frame_hdr.data, frame_hdr.frame_size);
    ...
    struct v4l2_ext_control ext_ctrl = { .id = V4L2_CID_STATELESS_VP8_FRAME, .size = sizeof(v4l2_frame_headers), .ptr = &v4l2_frame_headers };
    ...
    const std::set<int> reusable_buffer_slots
        = RefreshReferenceSlots(frame_hdr, CAPTURE_queue_->GetBuffer(buffer_id).get(), CAPTURE_queue_->queued_buffer_ids());
    ...
    CHECK_EQ(buffer_id, uint32_t(0)) << "Buffer ID of the buffer in OUTPUT queue is greater than size";
