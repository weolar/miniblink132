...
void H264DPB::UnmarkLowestFrameNumWrapShortRefPic()
{
    ...
    for (auto& i : *this) {
        ...
        if (pic.ref && !pic.long_term_reference_flag && (key < 0 || pic.frame_num_wrap < this->at(key).frame_num_wrap)) {
            key = i.first;
        }
    }
...
{
    for (auto& i : *this) {
        if (i.second.pic_num == pic_num && i.second.ref && !i.second.long_term_reference_flag) {
            i.second.ref = false;
            break;
        }
    }
...
{
    for (auto& i : *this) {
        if (i.second.long_term_pic_num == pic_num && i.second.ref && i.second.long_term_reference_flag) {
            i.second.ref = false;
            break;
        }
    }
...
{
    for (auto& i : *this) {
        if (i.second.ref && !i.second.long_term_reference_flag && i.second.pic_num == pic_num) {
            return &i.second;
        }
    }
    return nullptr;
...
{
    for (auto& i : *this) {
        if (i.second.ref && i.second.long_term_reference_flag && i.second.long_term_frame_idx == frame_index) {
            return &i.second;
        }
    }
    return nullptr;
...
{
    for (auto& i : *this) {
        if (i.second.long_term_reference_flag && i.second.long_term_frame_idx > index) {
            i.second.ref = false;
        }
    }
}
