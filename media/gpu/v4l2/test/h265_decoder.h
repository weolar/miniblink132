...
public:
    ...
    static std::unique_ptr<H265Decoder> Create(const base::MemoryMappedFile& stream);
...
private:
    H265Decoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution, const base::MemoryMappedFile& data_stream);
    ...
    std::set<uint32_t> GetReusableReferenceSlots(const MmappedBuffer& buffer, const std::set<uint32_t>& queued_buffer_ids);
