...
public:
    ...
    MmappedBuffer(const base::PlatformFile decode_fd, const struct v4l2_buffer& v4l2_buffer);
    ...
    public:
        ...
        void CopyInSlice(const uint8_t* frame_data, size_t frame_size, bool is_first_slice)
        {
            if (is_first_slice) {
                bytes_used = 0;
            }

            LOG_ASSERT((bytes_used + frame_size) < length) << "Not enough memory allocated to copy into.";

            memcpy(static_cast<uint8_t*>(start_addr) + bytes_used, frame_data, frame_size);
    ...
    MmappedPlanes& mmapped_planes()
    {
        return mmapped_planes_;
    }
...
class V4L2Queue {
public:
    V4L2Queue(enum v4l2_buf_type type, const gfx::Size& resolution, enum v4l2_memory memory);
    ...
    void set_fourcc(uint32_t fourcc)
    {
        fourcc_ = fourcc;
    }
    ...
    void set_resolution(gfx::Size resolution)
    {
    ...
    void set_buffers(MmappedBuffers& buffers)
    {
        buffers_ = buffers;
    }
    ...
    void set_num_planes(uint32_t num_planes)
    {
        num_planes_ = num_planes;
    }

    uint32_t last_queued_buffer_id() const
    {
        return last_queued_buffer_id_;
    }
    ...
    int media_request_fd() const
    {
        return media_request_fd_;
    }
    ...
    std::set<uint32_t> queued_buffer_ids() const
    {
        return queued_buffer_ids_;
    }
    ...
    void DequeueAllBufferIds()
    {
        queued_buffer_ids_.clear();
    }
...
public:
    ...
    [[nodiscard]] bool QBuf(const std::unique_ptr<V4L2Queue>& queue, const uint32_t buffer_id) const;

    // Dequeues a filled (capturing) or decoded (output) buffer
    // from the driver’s outgoing |queue|.
    void DQBuf(const std::unique_ptr<V4L2Queue>& queue, uint32_t* buffer_id) const;
    ...
    void SetExtCtrls(const std::unique_ptr<V4L2Queue>& queue, v4l2_ext_controls* ext_ctrls, bool immediate = false) const;
...
private:
    ...
    [[nodiscard]] bool QueryFormat(enum v4l2_buf_type type, uint32_t fourcc) const;
    ...
    // for said ioctl().
    template <typename T> [[nodiscard]] bool Ioctl(int request_code, T arg) const;
