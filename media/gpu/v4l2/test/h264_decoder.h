...
public:
    ...
    static std::unique_ptr<H264Decoder> Create(const base::MemoryMappedFile& stream);
...
private:
    H264Decoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution, const base::MemoryMappedFile& data_stream);
    ...
    VideoDecoder::Result StartNewFrame(bool is_OUTPUT_queue_new, H264SliceMetadata* slice_metadata);
    ...
    VideoDecoder::Result InitializeSliceMetadata(const H264SliceHeader& slice_hdr, const H264SPS* sps, H264SliceMetadata* slice_metadata) const;

    // Returns all CAPTURE buffer indexes that can be reused for a
    // VIDIOC_QBUF ioctl call.
    std::set<uint32_t> GetReusableReferenceSlots(const MmappedBuffer& buffer, std::set<uint32_t> queued_buffer_ids);
