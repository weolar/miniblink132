...
constexpr uint8_t zigzag_4x4[] = { 0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15 };
...
struct H264PicOrderCompare {
    bool operator()(const H264SliceMetadata* a, const H264SliceMetadata* b) const
    {
...
bool ParseBitDepth(const H264SPS& sps, uint8_t& bit_depth)
{
    ...
    if (sps.bit_depth_luma_minus8 != sps.bit_depth_chroma_minus8) {
        VLOGF(4) << "H264Decoder doesn't support different bit depths between luma"
                 << "and chroma, bit_depth_luma_minus8=" << sps.bit_depth_luma_minus8 << ", bit_depth_chroma_minus8=" << sps.bit_depth_chroma_minus8;
        return false;
    }
    ...
    case 6:
        ...
        break;
    default:
        VLOGF(4) << "Invalid bit depth: " << base::checked_cast<int>(sps.bit_depth_luma_minus8 + 8);
        return false;
    }
    return true;
}
...
v4l2_ctrl_h264_sps SetupSPSCtrl(const H264SPS* sps)
{
    ...
    v4l2_sps.log2_max_pic_order_cnt_lsb_minus4 = sps->log2_max_pic_order_cnt_lsb_minus4;
    v4l2_sps.max_num_ref_frames = sps->max_num_ref_frames;
    v4l2_sps.num_ref_frames_in_pic_order_cnt_cycle = sps->num_ref_frames_in_pic_order_cnt_cycle;
...
v4l2_ctrl_h264_pps SetupPPSCtrl(const H264PPS* pps)
{
    ...
    v4l2_pps.num_ref_idx_l0_default_active_minus1 = pps->num_ref_idx_l0_default_active_minus1;
    v4l2_pps.num_ref_idx_l1_default_active_minus1 = pps->num_ref_idx_l1_default_active_minus1;
    ...
    if (pps->bottom_field_pic_order_in_frame_present_flag)
        v4l2_pps.flags |= V4L2_H264_PPS_FLAG_BOTTOM_FIELD_PIC_ORDER_IN_FRAME_PRESENT;
...
v4l2_ctrl_h264_scaling_matrix SetupScalingMatrix(const H264SPS* sps, const H264PPS* pps)
{
    ...
    static_assert(std::extent<decltype(matrix.scaling_list_4x4)>() <= std::extent<decltype(pps->scaling_list4x4)>()
            && std::extent<decltype(matrix.scaling_list_4x4[0])>() <= std::extent<decltype(pps->scaling_list4x4[0])>()
            && std::extent<decltype(matrix.scaling_list_8x8)>() <= std::extent<decltype(pps->scaling_list8x8)>()
            && std::extent<decltype(matrix.scaling_list_8x8[0])>() <= std::extent<decltype(pps->scaling_list8x8[0])>(),
        "PPS scaling_lists must be of correct size");

    // Makes sure that the size of the matrix scaling lists correspond
    // to the SPS scaling matrix sizes.
    static_assert(std::extent<decltype(matrix.scaling_list_4x4)>() <= std::extent<decltype(sps->scaling_list4x4)>()
            && std::extent<decltype(matrix.scaling_list_4x4[0])>() <= std::extent<decltype(sps->scaling_list4x4[0])>()
            && std::extent<decltype(matrix.scaling_list_8x8)>() <= std::extent<decltype(sps->scaling_list8x8)>()
            && std::extent<decltype(matrix.scaling_list_8x8[0])>() <= std::extent<decltype(sps->scaling_list8x8[0])>(),
    ...
    static_assert(std::extent<decltype(matrix.scaling_list_4x4), 1>() == std::extent<decltype(zigzag_4x4)>());
    ...
    static_assert(std::extent<decltype(matrix.scaling_list_8x8), 1>() == std::extent<decltype(zigzag_8x8)>());
...
v4l2_ctrl_h264_decode_params SetupDecodeParams(const H264SliceHeader& slice, const H264SliceMetadata& slice_metadata, const H264DPB& dpb)
{
    ...
    v4l2_decode_params.delta_pic_order_cnt_bottom = slice.delta_pic_order_cnt_bottom;
    ...
    v4l2_decode_params.dec_ref_pic_marking_bit_size = slice.dec_ref_pic_marking_bit_size;
    ...
    v4l2_decode_params.bottom_field_order_cnt = slice_metadata.bottom_field_order_cnt;
    ...
    for (const auto& element : dpb) {
        ...
        entry = { .reference_ts = element.second.ref_ts_nsec * kTimestampToNanoSecs,
            ...
            .flags = static_cast<uint32_t>(V4L2_H264_DPB_ENTRY_FLAG_VALID | (element.second.ref ? V4L2_H264_DPB_ENTRY_FLAG_ACTIVE : 0)
                | (element.second.long_term_reference_flag ? V4L2_H264_DPB_ENTRY_FLAG_LONG_TERM : 0)) };
    }
...
bool IsNewFrame(const H264SliceMetadata& curr_picture, const H264SliceHeader& curr_slice_hdr, const H264SPS& sps)
{
    bool nalu_size_error = curr_picture.slice_header.nalu_size < 1;

    bool slice_changed = curr_slice_hdr.frame_num != curr_picture.slice_header.frame_num
        || curr_slice_hdr.pic_parameter_set_id != curr_picture.slice_header.pic_parameter_set_id
        || curr_slice_hdr.nal_ref_idc != curr_picture.slice_header.nal_ref_idc || curr_slice_hdr.idr_pic_flag != curr_picture.slice_header.idr_pic_flag
        || curr_slice_hdr.idr_pic_id != curr_picture.slice_header.idr_pic_id;

    bool slice_pic_order_changed = false;

    if (sps.pic_order_cnt_type == 0) {
        slice_pic_order_changed = curr_slice_hdr.pic_order_cnt_lsb != curr_picture.slice_header.pic_order_cnt_lsb
            || curr_slice_hdr.delta_pic_order_cnt_bottom != curr_picture.slice_header.delta_pic_order_cnt_bottom;

    } else if (sps.pic_order_cnt_type == 1) {
        slice_pic_order_changed = curr_slice_hdr.delta_pic_order_cnt0 != curr_picture.slice_header.delta_pic_order_cnt0
            || curr_slice_hdr.delta_pic_order_cnt1 != curr_picture.slice_header.delta_pic_order_cnt1;
    }
...
void H264Decoder::ProcessSPS(const int sps_id)
{
    ...
    if ((sps->profile_idc == H264SPS::kProfileIDCBaseline || sps->profile_idc == H264SPS::kProfileIDCConstrainedBaseline
    ...
    size_t max_dpb_size = std::max(static_cast<int>(max_dpb_frames), std::max(sps->max_num_ref_frames, sps->max_dec_frame_buffering));

    VideoCodecProfile new_profile = H264Parser::ProfileIDCToVideoCodecProfile(sps->profile_idc);
    ...
    if (pic_size_ != new_pic_size || dpb_.max_dpb_size_ != max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth) {
        FlushDPB();
...
{
    std::vector<H264SliceMetadata*> transmittable_slices = dpb_.GetNotOutputtedPicsAppending();
    std::sort(transmittable_slices.begin(), transmittable_slices.end(), H264PicOrderCompare());
...
void H264Decoder::InitializeDecoderLogic()
{
    ...
    while (true) {
        ...
        if (curr_nalu->nal_unit_type == H264NALU::kIDRSlice || curr_nalu->nal_unit_type == H264NALU::kNonIDRSlice) {
            break;
    ...
    CHECK(parser_->ParseSliceHeader(*curr_nalu, curr_slice_hdr_.get()) == H264Parser::kOk);
}

VideoDecoder::Result H264Decoder::SubmitSlice()
{
    std::vector<uint8_t> slice_data(sizeof(V4L2_STATELESS_H264_START_CODE_ANNEX_B) - 1);
    slice_data[2] = V4L2_STATELESS_H264_START_CODE_ANNEX_B;
    slice_data.insert(
        slice_data.end(), (curr_slice_hdr_->nalu_data).get(), (curr_slice_hdr_->nalu_data + base::checked_cast<size_t>(curr_slice_hdr_->nalu_size)).get());
...
VideoDecoder::Result H264Decoder::InitializeSliceMetadata(const H264SliceHeader& slice_hdr, const H264SPS* sps, H264SliceMetadata* slice_metadata) const
{
    if (!sps) {
        return VideoDecoder::kError;
    }
    ...
    if (slice_hdr.adaptive_ref_pic_marking_mode_flag) {
        static_assert(sizeof(slice_metadata->ref_pic_marking) == sizeof(slice_hdr.ref_pic_marking), "Array sizes of ref pic marking do not match.");
        memcpy(slice_metadata->ref_pic_marking, slice_hdr.ref_pic_marking, sizeof(slice_metadata->ref_pic_marking));
    }
    ...
    case 0: {
        ...
        if ((slice_metadata->pic_order_cnt_lsb < prev_pic_order_cnt_lsb)
            && (prev_pic_order_cnt_lsb - slice_metadata->pic_order_cnt_lsb >= max_pic_order_cnt_lsb / 2)) {
            slice_metadata->pic_order_cnt_msb = prev_pic_order_cnt_msb + max_pic_order_cnt_lsb;
        } else if ((slice_metadata->pic_order_cnt_lsb > prev_pic_order_cnt_lsb)
            && (slice_metadata->pic_order_cnt_lsb - prev_pic_order_cnt_lsb > max_pic_order_cnt_lsb / 2)) {
            slice_metadata->pic_order_cnt_msb = prev_pic_order_cnt_msb - max_pic_order_cnt_lsb;
        } else {
            slice_metadata->pic_order_cnt_msb = prev_pic_order_cnt_msb;
        }

        slice_metadata->top_field_order_cnt = slice_metadata->pic_order_cnt_msb + slice_metadata->pic_order_cnt_lsb;
        slice_metadata->bottom_field_order_cnt = slice_metadata->top_field_order_cnt + slice_hdr.delta_pic_order_cnt_bottom;
        break;
    }
    case 1: {
    ...
    case 2: {
        ...
        if (slice_metadata->slice_header.idr_pic_flag) {
            ...
            slice_metadata->frame_num_offset = prev_frame_num_offset_ + (1 << (sps->log2_max_frame_num_minus4 + 4));
        } else {
        ...
        if (slice_metadata->slice_header.idr_pic_flag) {
            ...
            temp_pic_order_cnt = 2 * (slice_metadata->frame_num_offset + slice_metadata->frame_num) - 1;
        } else {
            temp_pic_order_cnt = 2 * (slice_metadata->frame_num_offset + slice_metadata->frame_num);
        }
    ...
    slice_metadata->pic_order_cnt = std::min(slice_metadata->top_field_order_cnt, slice_metadata->bottom_field_order_cnt);

    return VideoDecoder::kOk;
}

VideoDecoder::Result H264Decoder::StartNewFrame(bool is_OUTPUT_queue_new, H264SliceMetadata* slice_metadata)
{
    ...
    if (InitializeSliceMetadata(*(curr_slice_hdr_.get()), sps, slice_metadata) == VideoDecoder::kError) {
        return VideoDecoder::kError;
    }
    ...
    struct v4l2_ctrl_h264_scaling_matrix v4l2_matrix = SetupScalingMatrix(sps, pps);

    struct v4l2_ext_control ctrls[] = { { .id = V4L2_CID_STATELESS_H264_SPS, .size = sizeof(v4l2_sps), .ptr = &v4l2_sps },
        { .id = V4L2_CID_STATELESS_H264_PPS, .size = sizeof(v4l2_pps), .ptr = &v4l2_pps },
        { .id = V4L2_CID_STATELESS_H264_SCALING_MATRIX, .size = sizeof(v4l2_matrix), .ptr = &v4l2_matrix } };
...
void H264Decoder::ProcessNextFrame()
{
    ...
    v4l2_ctrl_h264_decode_params v4l2_decode_params = SetupDecodeParams(*curr_slice_hdr_, slice_metadata, dpb_);

    const int pps_id = curr_slice_hdr_->pic_parameter_set_id;
    const int sps_id = parser_->GetPPS(pps_id)->seq_parameter_set_id;

    struct v4l2_ext_control ctrls[] = { { .id = V4L2_CID_STATELESS_H264_DECODE_PARAMS, .size = sizeof(v4l2_decode_params), .ptr = &v4l2_decode_params },
        { .id = V4L2_CID_STATELESS_H264_DECODE_MODE, .value = V4L2_STATELESS_H264_DECODE_MODE_FRAME_BASED } };
    struct v4l2_ext_controls ext_ctrls = { .count = (sizeof(ctrls) / sizeof(ctrls[0])), .controls = ctrls };
    ...
    while (true) {
        ...
        if (curr_nalu->nal_unit_type == H264NALU::kNonIDRSlice || curr_nalu->nal_unit_type == H264NALU::kIDRSlice) {
            curr_slice_hdr_ = std::make_unique<H264SliceHeader>();
            CHECK(parser_->ParseSliceHeader(*curr_nalu, curr_slice_hdr_.get()) == H264Parser::kOk);
...
void H264Decoder::FinishPicture(H264SliceMetadata picture, const int sps_id)
{
    ...
    if (picture.ref) {
        ...
        if (picture.slice_header.idr_pic_flag) {
            ...
            for (size_t i = 0; i < std::size(picture.ref_pic_marking); ++i) {
                ...
                case 1: {
                    const int pic_num_x = picture.pic_num - (ref_pic_marking->difference_of_pic_nums_minus1 + 1);
                ...
                case 3: {
                    ...
                    const int pic_num_x = picture.pic_num - (ref_pic_marking->difference_of_pic_nums_minus1 + 1);
                    H264SliceMetadata* short_pic = dpb_.GetShortRefPicByPicNum(pic_num_x);
                    if (short_pic) {
                        H264SliceMetadata* long_term_mark = dpb_.GetLongRefPicByFrameIdx(ref_pic_marking->long_term_frame_idx);
                        ...
                        short_pic->long_term_frame_idx = ref_pic_marking->long_term_frame_idx;
                    }
                    break;
                }

                case 4: {
                    const int max_long_term_frame_idx = ref_pic_marking->max_long_term_frame_idx_plus1 - 1;
                    dpb_.UnmarkLongTermPicsGreaterThanFrameIndex(max_long_term_frame_idx);
                    break;
                }
    ...
    while (output_candidate != transmittable_slices.end()
        && (slices_remaining > max_num_reorder_frames_ ||
            ...
            (dpb_.size() == dpb_.max_dpb_size_ && ((!(*output_candidate)->outputted || (*output_candidate)->ref)) && slices_remaining))) {
        DVLOG_IF(1, slices_remaining <= max_num_reorder_frames_) << "Invalid stream: max_num_reorder_frames not preserved";
...
// static
std::unique_ptr<H264Decoder> H264Decoder::Create(const base::MemoryMappedFile& stream)
{
    ...
    return base::WrapUnique(new H264Decoder(std::move(v4l2_ioctl), coded_size.value(), stream));
}

H264Decoder::H264Decoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution, const base::MemoryMappedFile& data_stream)
...
std::set<uint32_t> H264Decoder::GetReusableReferenceSlots(const MmappedBuffer& buffer, std::set<uint32_t> queued_buffer_ids)
{
    ...
    const int frame_number, std::vector<uint8_t>& y_plane, std::vector<uint8_t>& u_plane, std::vector<uint8_t>& v_plane, gfx::Size& size, BitDepth& bit_depth)
{
    ...
    scoped_refptr<MmappedBuffer> buffer = CAPTURE_queue_->GetBuffer(picture.capture_queue_buffer_id);
    ...
    bit_depth = ConvertToYUV(y_plane, u_plane, v_plane, size, buffer->mmapped_planes(), CAPTURE_queue_->resolution(), CAPTURE_queue_->fourcc());
