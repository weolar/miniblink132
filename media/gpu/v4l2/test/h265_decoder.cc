...
struct POCAscCompare {
    bool operator()(const scoped_refptr<media::v4l2_test::H265Picture>& a, const scoped_refptr<media::v4l2_test::H265Picture>& b) const
    {
...
bool IsValidBitDepth(uint8_t bit_depth, VideoCodecProfile profile)
{
    ...
    case HEVCPROFILE_REXT:
        return bit_depth == 8u || bit_depth == 10u || bit_depth == 12u || bit_depth == 14u || bit_depth == 16u;
    // Spec A.3.6
    case HEVCPROFILE_HIGH_THROUGHPUT:
        return bit_depth == 8u || bit_depth == 10u || bit_depth == 14u || bit_depth == 16u;
    // Spec G.11.1.1
...
#define SET_V4L2_SPS_FLAG_IF(cond, flag) v4l2_sps.flags |= ((sps->cond) ? (flag) : 0)
    SET_V4L2_SPS_FLAG_IF(separate_colour_plane_flag, V4L2_HEVC_SPS_FLAG_SEPARATE_COLOUR_PLANE);
    SET_V4L2_SPS_FLAG_IF(scaling_list_enabled_flag, V4L2_HEVC_SPS_FLAG_SCALING_LIST_ENABLED);
    SET_V4L2_SPS_FLAG_IF(amp_enabled_flag, V4L2_HEVC_SPS_FLAG_AMP_ENABLED);
    SET_V4L2_SPS_FLAG_IF(sample_adaptive_offset_enabled_flag, V4L2_HEVC_SPS_FLAG_SAMPLE_ADAPTIVE_OFFSET);
    SET_V4L2_SPS_FLAG_IF(pcm_enabled_flag, V4L2_HEVC_SPS_FLAG_PCM_ENABLED);
    SET_V4L2_SPS_FLAG_IF(pcm_loop_filter_disabled_flag, V4L2_HEVC_SPS_FLAG_PCM_LOOP_FILTER_DISABLED);
    SET_V4L2_SPS_FLAG_IF(long_term_ref_pics_present_flag, V4L2_HEVC_SPS_FLAG_LONG_TERM_REF_PICS_PRESENT);
    SET_V4L2_SPS_FLAG_IF(sps_temporal_mvp_enabled_flag, V4L2_HEVC_SPS_FLAG_SPS_TEMPORAL_MVP_ENABLED);
    SET_V4L2_SPS_FLAG_IF(strong_intra_smoothing_enabled_flag, V4L2_HEVC_SPS_FLAG_STRONG_INTRA_SMOOTHING_ENABLED);
...
#define PPS_TO_V4L2PPS(a) v4l2_pps.a = pps->a
    ...
    if (pps->tiles_enabled_flag) {
        ...
        if (!pps->uniform_spacing_flag) {
            static_assert(
                std::size(v4l2_pps.column_width_minus1) >= std::extent<decltype(pps->column_width_minus1)>(), "column_width_minus1 arrays must be same size");
            for (int i = 0; i <= pps->num_tile_columns_minus1; ++i) {
                v4l2_pps.column_width_minus1[i] = pps->column_width_minus1[i];
            }

            static_assert(
                std::size(v4l2_pps.row_height_minus1) >= std::extent<decltype(pps->row_height_minus1)>(), "row_height_minus1 arrays must be same size");
...
#define SET_V4L2_PPS_FLAG_IF(cond, flag) v4l2_pps.flags |= ((pps->cond) ? (flag) : 0)
    SET_V4L2_PPS_FLAG_IF(dependent_slice_segments_enabled_flag, V4L2_HEVC_PPS_FLAG_DEPENDENT_SLICE_SEGMENT_ENABLED);
    SET_V4L2_PPS_FLAG_IF(output_flag_present_flag, V4L2_HEVC_PPS_FLAG_OUTPUT_FLAG_PRESENT);
    SET_V4L2_PPS_FLAG_IF(sign_data_hiding_enabled_flag, V4L2_HEVC_PPS_FLAG_SIGN_DATA_HIDING_ENABLED);
    SET_V4L2_PPS_FLAG_IF(cabac_init_present_flag, V4L2_HEVC_PPS_FLAG_CABAC_INIT_PRESENT);
    SET_V4L2_PPS_FLAG_IF(constrained_intra_pred_flag, V4L2_HEVC_PPS_FLAG_CONSTRAINED_INTRA_PRED);
    SET_V4L2_PPS_FLAG_IF(transform_skip_enabled_flag, V4L2_HEVC_PPS_FLAG_TRANSFORM_SKIP_ENABLED);
    SET_V4L2_PPS_FLAG_IF(cu_qp_delta_enabled_flag, V4L2_HEVC_PPS_FLAG_CU_QP_DELTA_ENABLED);
    SET_V4L2_PPS_FLAG_IF(pps_slice_chroma_qp_offsets_present_flag, V4L2_HEVC_PPS_FLAG_PPS_SLICE_CHROMA_QP_OFFSETS_PRESENT);
    SET_V4L2_PPS_FLAG_IF(weighted_pred_flag, V4L2_HEVC_PPS_FLAG_WEIGHTED_PRED);
    SET_V4L2_PPS_FLAG_IF(weighted_bipred_flag, V4L2_HEVC_PPS_FLAG_WEIGHTED_BIPRED);
    SET_V4L2_PPS_FLAG_IF(transquant_bypass_enabled_flag, V4L2_HEVC_PPS_FLAG_TRANSQUANT_BYPASS_ENABLED);
    SET_V4L2_PPS_FLAG_IF(tiles_enabled_flag, V4L2_HEVC_PPS_FLAG_TILES_ENABLED);
    SET_V4L2_PPS_FLAG_IF(entropy_coding_sync_enabled_flag, V4L2_HEVC_PPS_FLAG_ENTROPY_CODING_SYNC_ENABLED);
    SET_V4L2_PPS_FLAG_IF(loop_filter_across_tiles_enabled_flag, V4L2_HEVC_PPS_FLAG_LOOP_FILTER_ACROSS_TILES_ENABLED);
    SET_V4L2_PPS_FLAG_IF(pps_loop_filter_across_slices_enabled_flag, V4L2_HEVC_PPS_FLAG_PPS_LOOP_FILTER_ACROSS_SLICES_ENABLED);
    SET_V4L2_PPS_FLAG_IF(deblocking_filter_override_enabled_flag, V4L2_HEVC_PPS_FLAG_DEBLOCKING_FILTER_OVERRIDE_ENABLED);
    SET_V4L2_PPS_FLAG_IF(pps_deblocking_filter_disabled_flag, V4L2_HEVC_PPS_FLAG_PPS_DISABLE_DEBLOCKING_FILTER);
    SET_V4L2_PPS_FLAG_IF(lists_modification_present_flag, V4L2_HEVC_PPS_FLAG_LISTS_MODIFICATION_PRESENT);
    SET_V4L2_PPS_FLAG_IF(slice_segment_header_extension_present_flag, V4L2_HEVC_PPS_FLAG_SLICE_SEGMENT_HEADER_EXTENSION_PRESENT);
    SET_V4L2_PPS_FLAG_IF(deblocking_filter_control_present_flag, V4L2_HEVC_PPS_FLAG_DEBLOCKING_FILTER_CONTROL_PRESENT);
    SET_V4L2_PPS_FLAG_IF(uniform_spacing_flag, V4L2_HEVC_PPS_FLAG_UNIFORM_SPACING);
...
v4l2_ctrl_hevc_scaling_matrix SetupScalingMatrix(const H265SPS* sps, const H265PPS* pps)
{
    ...
    static_assert(std::size(checker.scaling_list_dc_coef_16x16) == std::size(v4l2_scaling_matrix.scaling_list_dc_coef_16x16)
            && std::size(checker.scaling_list_dc_coef_32x32) / 3 == std::size(v4l2_scaling_matrix.scaling_list_dc_coef_32x32)
            && std::size(checker.scaling_list_4x4) == std::size(v4l2_scaling_matrix.scaling_list_4x4)
            && std::size(checker.scaling_list_4x4[0]) == std::size(v4l2_scaling_matrix.scaling_list_4x4[0])
            && std::size(checker.scaling_list_8x8) == std::size(v4l2_scaling_matrix.scaling_list_8x8)
            && std::size(checker.scaling_list_8x8[0]) == std::size(v4l2_scaling_matrix.scaling_list_8x8[0])
            && std::size(checker.scaling_list_16x16) == std::size(v4l2_scaling_matrix.scaling_list_16x16)
            && std::size(checker.scaling_list_16x16[0]) == std::size(v4l2_scaling_matrix.scaling_list_16x16[0])
            && std::size(checker.scaling_list_32x32) / 3 == std::size(v4l2_scaling_matrix.scaling_list_32x32)
            && std::size(checker.scaling_list_32x32[0]) == std::size(v4l2_scaling_matrix.scaling_list_32x32[0]),
    ...
    if (sps->scaling_list_enabled_flag) {
        ...
        const auto& scaling_list = pps->pps_scaling_list_data_present_flag ? pps->scaling_list_data : sps->scaling_list_data;
        ...
        for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; ++i) {
            for (size_t j = 0; j < H265ScalingListData::kScalingListSizeId0Count; ++j) {
                v4l2_scaling_matrix.scaling_list_4x4[i][j] = scaling_list.GetScalingList4x4EntryInRasterOrder(/*matrix_id=*/i,
                    /*raster_idx=*/j);
            }
        }

        for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; ++i) {
            for (size_t j = 0; j < H265ScalingListData::kScalingListSizeId1To3Count; ++j) {
                v4l2_scaling_matrix.scaling_list_8x8[i][j] = scaling_list.GetScalingList8x8EntryInRasterOrder(/*matrix_id=*/i,
                    /*raster_idx=*/j);
            }
        }
        ...
        for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; ++i) {
            for (size_t j = 0; j < H265ScalingListData::kScalingListSizeId1To3Count; ++j) {
                v4l2_scaling_matrix.scaling_list_16x16[i][j] = scaling_list.GetScalingList16x16EntryInRasterOrder(
                    /*matrix_id=*/i,
                    /*raster_idx=*/j);
            }
        }
        ...
        for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; i += 3) {
            for (size_t j = 0; j < H265ScalingListData::kScalingListSizeId1To3Count; ++j) {
                v4l2_scaling_matrix.scaling_list_32x32[i / 3][j] = scaling_list.GetScalingList32x32EntryInRasterOrder(
                    /*matrix_id=*/i,
                    /*raster_idx=*/j);
            }
        }

        memcpy(v4l2_scaling_matrix.scaling_list_dc_coef_16x16, scaling_list.scaling_list_dc_coef_16x16, sizeof(v4l2_scaling_matrix.scaling_list_dc_coef_16x16));
        v4l2_scaling_matrix.scaling_list_dc_coef_32x32[0] = scaling_list.scaling_list_dc_coef_32x32[0];
        v4l2_scaling_matrix.scaling_list_dc_coef_32x32[1] = scaling_list.scaling_list_dc_coef_32x32[3];
    }

    return v4l2_scaling_matrix;
}

struct v4l2_ctrl_hevc_decode_params SetupDecodeParams(const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list,
    const H265Picture::Vector& ref_pic_set_lt_curr, const H265Picture::Vector& ref_pic_set_st_curr_after, const H265Picture::Vector& ref_pic_set_st_curr_before,
    scoped_refptr<H265Picture> curr_pic)
{
    ...
    v4l2_decode_params.short_term_ref_pic_set_size = static_cast<__u16>(slice_hdr->st_rps_bits),
    v4l2_decode_params.long_term_ref_pic_set_size = static_cast<__u16>(slice_hdr->lt_rps_bits),
    ...
    // .num_delta_pocs_of_ref_rps_idx is upstream but not yet pulled
        ...
        v4l2_decode_params.num_delta_pocs_of_ref_rps_idx = static_cast<__u8>(slice_hdr->st_ref_pic_set.rps_idx_num_delta_pocs),
#endif
    v4l2_decode_params.flags = static_cast<__u64>((curr_pic->irap_pic_ ? V4L2_HEVC_DECODE_PARAM_FLAG_IRAP_PIC : 0)
        | ((curr_pic->nal_unit_type_ >= H265NALU::IDR_W_RADL && curr_pic->nal_unit_type_ <= H265NALU::IDR_N_LP) ? V4L2_HEVC_DECODE_PARAM_FLAG_IDR_PIC : 0)
        | (curr_pic->no_output_of_prior_pics_flag_ ? V4L2_HEVC_DECODE_PARAM_FLAG_NO_OUTPUT_OF_PRIOR : 0)),
    ...
    for (const auto& pic : ref_pic_list) {
        ...
        entry = {
            ...
            .flags = static_cast<__u8>(pic->IsLongTermRef() ? V4L2_HEVC_DPB_ENTRY_LONG_TERM_REFERENCE : 0),
    ...
    // Set defaults
    std::fill_n(v4l2_decode_params.poc_st_curr_before, std::size(v4l2_decode_params.poc_st_curr_before), 0xff);
    std::fill_n(v4l2_decode_params.poc_st_curr_after, std::size(v4l2_decode_params.poc_st_curr_after), 0xff);
    std::fill_n(v4l2_decode_params.poc_lt_curr, std::size(v4l2_decode_params.poc_lt_curr), 0xff);
    ...
    for (const auto& pic : ref_pic_set_st_curr_before) {
        ...
        for (unsigned int j = 0; j < v4l2_decode_params.num_active_dpb_entries; j++) {
            if (pic->pic_order_cnt_val_ == v4l2_decode_params.dpb[j].pic_order_cnt_val) {
...
H265Decoder::H265Decoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution, const base::MemoryMappedFile& data_stream)
...
// static
std::unique_ptr<H265Decoder> H265Decoder::Create(const base::MemoryMappedFile& stream)
{
    ...
    return base::WrapUnique(new H265Decoder(std::move(v4l2_ioctl), coded_size.value(), stream));
}
...
bool H265Decoder::ProcessPPS(int pps_id, bool* need_new_buffers)
{
    ...
    // Equation 7-8
    max_pic_order_cnt_lsb_ = std::pow(2, sps->log2_max_pic_order_cnt_lsb_minus4 + 4);

    VideoCodecProfile new_profile = H265Parser::ProfileIDCToVideoCodecProfile(sps->profile_tier_level.general_profile_idc);
    ...
    if (!IsValidBitDepth(new_bit_depth, new_profile)) {
        LOG(ERROR) << "Invalid bit depth=" << base::strict_cast<int>(new_bit_depth) << ", profile=" << GetProfileName(new_profile);
        return false;
    }

    if (pic_size_ != new_pic_size || dpb_.MaxNumPics() != sps->max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth
        || chroma_sampling_ != new_chroma_sampling) {
        CHECK(Flush()) << "Failed to flush the decoder.";

        LOG(INFO) << "Codec profile: " << GetProfileName(new_profile) << ", level(x30): " << sps->profile_tier_level.general_level_idc
                  << ", DPB size: " << sps->max_dpb_size << ", Picture size: " << new_pic_size.ToString()
                  << ", bit_depth: " << base::strict_cast<int>(new_bit_depth)
                  << ", chroma_sampling_format: " << VideoChromaSamplingToString(new_chroma_sampling);
...
bool H265Decoder::ProcessCurrentSlice()
{
    ...
    slice_data.insert(slice_data.end(), curr_slice_hdr_->nalu_data.get(), (curr_slice_hdr_->nalu_data + curr_slice_hdr_->nalu_size).get());

    scoped_refptr<MmappedBuffer> OUTPUT_buffer = OUTPUT_queue_->GetBuffer(0);
    OUTPUT_buffer->mmapped_planes()[0].CopyInSlice(&slice_data[0], slice_data.size(), curr_slice_hdr_->first_slice_segment_in_pic_flag);
...
void H265Decoder::CalcPicOutputFlags(const H265SliceHeader* slice_hdr)
{
    if (slice_hdr->irap_pic) {
        ...
        curr_pic_->no_rasl_output_flag_
            = (curr_nalu_->nal_unit_type >= H265NALU::BLA_W_LP && curr_nalu_->nal_unit_type <= H265NALU::IDR_N_LP) || curr_pic_->first_picture_;
    } else {
        curr_pic_->no_rasl_output_flag_ = false;
    }

    // C.5.2.2
    if (slice_hdr->irap_pic && curr_pic_->no_rasl_output_flag_ && !curr_pic_->first_picture_) {
        curr_pic_->no_output_of_prior_pics_flag_ = (slice_hdr->nal_unit_type == H265NALU::CRA_NUT) || slice_hdr->no_output_of_prior_pics_flag;
    } else {
        curr_pic_->no_output_of_prior_pics_flag_ = false;
    }

    if ((slice_hdr->nal_unit_type == H265NALU::RASL_N || slice_hdr->nal_unit_type == H265NALU::RASL_R) && curr_pic_->no_rasl_output_flag_) {
...
void H265Decoder::CalcPictureOrderCount(const H265PPS* pps, const H265SliceHeader* slice_hdr)
{
    // 8.3.1 Decoding process for picture order count.
    curr_pic_->valid_for_prev_tid0_pic_
        = !slice_hdr->temporal_id && (slice_hdr->nal_unit_type < H265NALU::RADL_N || slice_hdr->nal_unit_type > H265NALU::RSV_VCL_N14);
    curr_pic_->slice_pic_order_cnt_lsb_ = slice_hdr->slice_pic_order_cnt_lsb;

    // Calculate POC for current picture.
    if ((!slice_hdr->irap_pic || !curr_pic_->no_rasl_output_flag_) && prev_tid0_pic_) {
        ...
        if ((slice_hdr->slice_pic_order_cnt_lsb < prev_pic_order_cnt_lsb)
            && ((prev_pic_order_cnt_lsb - slice_hdr->slice_pic_order_cnt_lsb) >= (max_pic_order_cnt_lsb_ / 2))) {
            curr_pic_->pic_order_cnt_msb_ = prev_pic_order_cnt_msb + max_pic_order_cnt_lsb_;
        } else if ((slice_hdr->slice_pic_order_cnt_lsb > prev_pic_order_cnt_lsb)
            && ((slice_hdr->slice_pic_order_cnt_lsb - prev_pic_order_cnt_lsb) > (max_pic_order_cnt_lsb_ / 2))) {
            curr_pic_->pic_order_cnt_msb_ = prev_pic_order_cnt_msb - max_pic_order_cnt_lsb_;
        } else {
    ...
    curr_pic_->pic_order_cnt_val_ = curr_pic_->pic_order_cnt_msb_ + slice_hdr->slice_pic_order_cnt_lsb;
}

bool H265Decoder::CalcRefPicPocs(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr)
{
    if (slice_hdr->nal_unit_type == H265NALU::IDR_W_RADL || slice_hdr->nal_unit_type == H265NALU::IDR_N_LP) {
        num_poc_st_curr_before_ = num_poc_st_curr_after_ = num_poc_st_foll_ = num_poc_lt_curr_ = num_poc_lt_foll_ = 0;
        return true;
    }
    ...
    num_poc_st_foll_ = k;
    for (i = 0, j = 0, k = 0; i < slice_hdr->num_long_term_sps + slice_hdr->num_long_term_pics; ++i) {
        ...
        if (slice_hdr->delta_poc_msb_present_flag[i]) {
            ...
            base::CheckedNumeric<int> poc_delta = slice_hdr->delta_poc_msb_cycle_lt[i];
        ...
        if (slice_hdr->used_by_curr_pic_lt[i]) {
            ...
            curr_delta_poc_msb_present_flag_[j++] = slice_hdr->delta_poc_msb_present_flag[i];
        } else {
            poc_lt_foll_[k] = poc_lt.ValueOrDefault(0);
            foll_delta_poc_msb_present_flag_[k++] = slice_hdr->delta_poc_msb_present_flag[i];
        }
    }
    num_poc_lt_curr_ = j;
    num_poc_lt_foll_ = k;
    ...
    if (slice_hdr->nal_unit_type == H265NALU::CRA_NUT || (slice_hdr->nal_unit_type >= H265NALU::BLA_W_LP && slice_hdr->nal_unit_type <= H265NALU::BLA_N_LP)) {
        if (slice_hdr->num_pic_total_curr) {
            LOG(ERROR) << "Invalid value for num_pic_total_curr";
            return false;
        }
    } else if ((slice_hdr->IsBSlice() || slice_hdr->IsPSlice()) && !slice_hdr->num_pic_total_curr) {
        LOG(ERROR) << "Invalid value for num_pic_total_curr";
        return false;
    }
...
bool H265Decoder::BuildRefPicLists(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr)
{
    ...
    for (int i = 0; i < num_poc_lt_curr_; ++i) {
        if (!curr_delta_poc_msb_present_flag_[i]) {
            ref_pic_set_lt_curr_[i] = dpb_.GetPicByPocMaskedAndMark(poc_lt_curr_[i], sps->max_pic_order_cnt_lsb - 1, H265Picture::kLongTermCurr);
        } else {
            ref_pic_set_lt_curr_[i] = dpb_.GetPicByPocAndMark(poc_lt_curr_[i], H265Picture::kLongTermCurr);
        }
    ...
    for (int i = 0; i < num_poc_lt_foll_; ++i) {
        if (!foll_delta_poc_msb_present_flag_[i]) {
            ref_pic_set_lt_foll[i] = dpb_.GetPicByPocMaskedAndMark(poc_lt_foll_[i], sps->max_pic_order_cnt_lsb - 1, H265Picture::kLongTermFoll);
        } else {
            ref_pic_set_lt_foll[i] = dpb_.GetPicByPocAndMark(poc_lt_foll_[i], H265Picture::kLongTermFoll);
        }
    ...
    for (int i = 0; i < num_poc_st_curr_before_; ++i) {
        ref_pic_set_st_curr_before_[i] = dpb_.GetPicByPocAndMark(poc_st_curr_before_[i], H265Picture::kShortTermCurrBefore);
    ...
    for (int i = 0; i < num_poc_st_curr_after_; ++i) {
        ref_pic_set_st_curr_after_[i] = dpb_.GetPicByPocAndMark(poc_st_curr_after_[i], H265Picture::kShortTermCurrAfter);
        if (ref_pic_set_st_curr_after_[i]) {
            total_ref_pics++;
        }
    }
    for (int i = 0; i < num_poc_st_foll_; ++i) {
        ref_pic_set_st_foll[i] = dpb_.GetPicByPocAndMark(poc_st_foll_[i], H265Picture::kShortTermFoll);
    ...
    if (slice_hdr->IsPSlice() || slice_hdr->IsBSlice()) {
        ...
        int num_rps_curr_temp_list0 = std::max(slice_hdr->num_ref_idx_l0_active_minus1 + 1, slice_hdr->num_pic_total_curr);
        scoped_refptr<H265Picture> ref_pic_list_temp0[kMaxDpbSize];

        // Equation 8-8.
        int r_idx = 0;
        while (r_idx < num_rps_curr_temp_list0) {
            for (int i = 0; i < num_poc_st_curr_before_ && r_idx < num_rps_curr_temp_list0; ++i, ++r_idx) {
                ref_pic_list_temp0[r_idx] = ref_pic_set_st_curr_before_[i];
            }
            for (int i = 0; i < num_poc_st_curr_after_ && r_idx < num_rps_curr_temp_list0; ++i, ++r_idx) {
                ref_pic_list_temp0[r_idx] = ref_pic_set_st_curr_after_[i];
            }
            for (int i = 0; i < num_poc_lt_curr_ && r_idx < num_rps_curr_temp_list0; ++i, ++r_idx) {
        ...
        for (r_idx = 0; r_idx <= slice_hdr->num_ref_idx_l0_active_minus1; ++r_idx) {
            ref_pic_list0_.push_back(slice_hdr->ref_pic_lists_modification.ref_pic_list_modification_flag_l0
                    ? ref_pic_list_temp0[slice_hdr->ref_pic_lists_modification.list_entry_l0[r_idx]]
                    : ref_pic_list_temp0[r_idx]);
        }

        if (slice_hdr->IsBSlice()) {
            int num_rps_curr_temp_list1 = std::max(slice_hdr->num_ref_idx_l1_active_minus1 + 1, slice_hdr->num_pic_total_curr);
            scoped_refptr<H265Picture> ref_pic_list_temp1[kMaxDpbSize];

            // Equation 8-10.
            r_idx = 0;
            while (r_idx < num_rps_curr_temp_list1) {
                for (int i = 0; i < num_poc_st_curr_after_ && r_idx < num_rps_curr_temp_list1; ++i, r_idx++) {
                    ref_pic_list_temp1[r_idx] = ref_pic_set_st_curr_after_[i];
                }
                for (int i = 0; i < num_poc_st_curr_before_ && r_idx < num_rps_curr_temp_list1; ++i, r_idx++) {
                    ref_pic_list_temp1[r_idx] = ref_pic_set_st_curr_before_[i];
                }
                for (int i = 0; i < num_poc_lt_curr_ && r_idx < num_rps_curr_temp_list1; ++i, r_idx++) {
                    ref_pic_list_temp1[r_idx] = ref_pic_set_lt_curr_[i];
                }
            }

            // Equation 8-11.
            for (r_idx = 0; r_idx <= slice_hdr->num_ref_idx_l1_active_minus1; ++r_idx) {
                ref_pic_list1_.push_back(slice_hdr->ref_pic_lists_modification.ref_pic_list_modification_flag_l1
                        ? ref_pic_list_temp1[slice_hdr->ref_pic_lists_modification.list_entry_l1[r_idx]]
...
{
    // C.5.2.2
    if (curr_pic_->irap_pic_ && curr_pic_->no_rasl_output_flag_ && !curr_pic_->first_picture_) {
        ...
        do {
            ...
            num_to_output = 0;
            int highest_tid = sps->sps_max_sub_layers_minus1;
            num_to_output = std::max(num_to_output, static_cast<int>(not_outputted.size()) - sps->sps_max_num_reorder_pics[highest_tid]);
            num_to_output = std::max(num_to_output, static_cast<int>(dpb_.Size()) - sps->sps_max_dec_pic_buffering_minus1[highest_tid]);

            num_to_output = std::min(num_to_output, static_cast<int>(not_outputted.size()));
...
bool H265Decoder::StartNewFrame(const H265SliceHeader* slice_hdr)
{
    ...
    struct v4l2_ctrl_hevc_scaling_matrix v4l2_matrix = SetupScalingMatrix(sps, pps);
    struct v4l2_ctrl_hevc_decode_params v4l2_decode_params
        = SetupDecodeParams(slice_hdr, ref_pic_list_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_, ref_pic_set_st_curr_before_, curr_pic_);

    struct v4l2_ext_control ctrls[] = { { .id = V4L2_CID_STATELESS_HEVC_SPS, .size = sizeof(v4l2_sps), .ptr = &v4l2_sps },
        { .id = V4L2_CID_STATELESS_HEVC_PPS, .size = sizeof(v4l2_pps), .ptr = &v4l2_pps },
        { .id = V4L2_CID_STATELESS_HEVC_SCALING_MATRIX, .size = sizeof(v4l2_matrix), .ptr = &v4l2_matrix },
        { .id = V4L2_CID_STATELESS_HEVC_DECODE_PARAMS, .size = sizeof(v4l2_decode_params), .ptr = &v4l2_decode_params } };

    struct v4l2_ext_controls ext_ctrls = { .count = (sizeof(ctrls) / sizeof(ctrls[0])), .controls = ctrls };
...
std::set<uint32_t> H265Decoder::GetReusableReferenceSlots(const MmappedBuffer& buffer, const std::set<uint32_t>& queued_buffer_ids)
{
...
bool H265Decoder::DecodePicture()
{
    ...
    const std::set<uint32_t> reusable_buffer_slots
        = GetReusableReferenceSlots(*CAPTURE_queue_->GetBuffer(CAPTURE_id).get(), CAPTURE_queue_->queued_buffer_ids());
...
H265Decoder::DecodeResult H265Decoder::Decode()
{
    ...
    while (frames_ready_to_be_outputted_.empty()) {
        if (!curr_nalu_) {
            ...
            const H265Parser::Result parse_result = parser_->AdvanceToNextNALU(curr_nalu_.get());
        ...
        if (curr_nalu_->nuh_layer_id) {
            VLOGF(4) << "Skipping NALU with nuh_layer_id=" << curr_nalu_->nuh_layer_id;
            curr_nalu_.reset();
            continue;
        }
        ...
        case H265NALU::CRA_NUT: {
            if (!curr_slice_hdr_) {
                ...
                const H265Parser::Result parse_result = parser_->ParseSliceHeader(*curr_nalu_, curr_slice_hdr_.get(), last_slice_hdr_.get());
                ...
                if (curr_slice_hdr_->irap_pic) {
                    ...
                    const bool success = ProcessPPS(curr_slice_hdr_->slice_pic_parameter_set_id, &need_new_buffers);
        ...
        case H265NALU::SPS_NUT: {
            ...
            CHECK_EQ(parse_result, H265Parser::kOk) << "Parser Failed to parse SPS.";
        ...
        case H265NALU::PPS_NUT: {
            ...
            const H265Parser::Result parse_result = parser_->ParsePPS(*curr_nalu_, &pps_id);
            CHECK_EQ(parse_result, H265Parser::kOk) << "Parser Failed to parse PPS.";

            if (curr_pps_id_ == -1) {
                bool need_new_buffers = false;

                const bool success_process_pps = ProcessPPS(pps_id, &need_new_buffers);
        ...
        VLOGF(4) << "Finished with current NALU type: " << static_cast<int>(curr_nalu_->nal_unit_type);
        curr_nalu_.reset();
    }
    ...
    const int frame_number, std::vector<uint8_t>& y_plane, std::vector<uint8_t>& u_plane, std::vector<uint8_t>& v_plane, gfx::Size& size, BitDepth& bit_depth)
{
    ...
    scoped_refptr<MmappedBuffer> buffer = CAPTURE_queue_->GetBuffer(picture->capture_queue_buffer_id_);
