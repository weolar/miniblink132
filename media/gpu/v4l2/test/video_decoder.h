...
public:
    ...
    VideoDecoder(std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution);
    ...
    virtual Result DecodeNextFrame(const int frame_number, std::vector<uint8_t>& y_plane, std::vector<uint8_t>& u_plane, std::vector<uint8_t>& v_plane,
        gfx::Size& size, BitDepth& bit_depth)
        = 0;
    ...
    bool LastDecodedFrameVisible() const
    {
        return last_decoded_frame_visible_;
    }

    // Converts raw YUV of decoded frame data to PNG.
    static std::vector<uint8_t> ConvertYUVToPNG(uint8_t* y_plane, uint8_t* u_plane, uint8_t* v_plane, const gfx::Size& size, BitDepth bit_depth);
...
protected:
    ...
    // the converted frame.
    static BitDepth ConvertToYUV(std::vector<uint8_t>& dest_y, std::vector<uint8_t>& dest_u, std::vector<uint8_t>& dest_v, const gfx::Size& dest_size,
        const MmappedBuffer::MmappedPlanes& planes, const gfx::Size& src_size, uint32_t fourcc);
