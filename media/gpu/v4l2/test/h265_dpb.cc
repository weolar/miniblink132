...
void H265DPB::StorePicture(scoped_refptr<H265Picture> pic)
{
    ...
    LOG(INFO) << "Adding PicNum: " << pic->pic_order_cnt_val_ << " reference type: " << static_cast<int>(pic->reference_type_);
...
void H265DPB::DeleteUnused()
{
    ...
    for (auto it = pics_.begin(); it != pics_.end();) {
        auto& pic = *it;
        if ((!pic->pic_output_flag_ || pic->outputted_) && (pic->reference_type_ == H265Picture::kUnused)) {
