...
constexpr char kUsageMsg[] = "usage: v4l2_stateless_decoder\n"
                             "           --video=<video path>\n"
...
constexpr char kHelpMsg[] = "This binary decodes the IVF video in <video> path with specified \n"
...
void ComputeAndPrintMD5hash(const std::vector<uint8_t>& yuv_plane, const base::FilePath md5_log_location)
{
...
std::unique_ptr<VideoDecoder> CreateVideoDecoder(const base::MemoryMappedFile& stream)
{
...
int main(int argc, char** argv)
{
    ...
    const std::string output_file_prefix = cmd->GetSwitchValueASCII("output_path_prefix");

    std::string output_format = "yuv";
    if (has_output_file && cmd->HasSwitch("output_format")) {
        output_format = base::ToLowerASCII(cmd->GetSwitchValueASCII("output_format"));
    ...
    for (int i = 0; i < n_frames || n_frames == 0; i++) {
        ...
        const VideoDecoder::Result res = dec->DecodeNextFrame(i, y_plane, u_plane, v_plane, size, bit_depth);
        ...
        base::FilePath filename(base::StringPrintf("%s%.6d.%s", output_file_prefix.c_str(), i, output_format.c_str()));
        base::File output_file(filename, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);

        if (output_format == "yuv") {
            output_file.Write(0, reinterpret_cast<const char*>(yuv_plane.data()), yuv_plane.size());
        } else {
            std::vector<uint8_t> image_buffer = dec->ConvertYUVToPNG(y_plane.data(), u_plane.data(), v_plane.data(), size, bit_depth);
            output_file.Write(0, reinterpret_cast<char*>(image_buffer.data()), image_buffer.size());
        }
    }
