...
public:
    ...
    static std::unique_ptr<Av1Decoder> Create(const base::MemoryMappedFile& stream);
    ...
    VideoDecoder::Result DecodeNextFrame(const int frame_number, std::vector<uint8_t>& y_plane, std::vector<uint8_t>& u_plane, std::vector<uint8_t>& v_plane,
        gfx::Size& size, BitDepth& bit_depth) override;
...
private:
    ...
    Av1Decoder(std::unique_ptr<IvfParser> ivf_parser, std::unique_ptr<V4L2IoctlShim> v4l2_ioctl, gfx::Size display_resolution);
    ...
    void CopyFrameData(const libgav1::ObuFrameHeader& frame_hdr, std::unique_ptr<V4L2Queue>& queue);
    ...
    std::set<int> RefreshReferenceSlots(const libgav1::ObuFrameHeader& frame_hdr, const libgav1::RefCountedBufferPtr current_frame,
    ...
    void QueueReusableBuffersInCaptureQueue(const std::set<int> reusable_buffer_ids, const bool is_inter_frame);
