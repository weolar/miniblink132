...
std::optional<gfx::GpuMemoryBufferHandle> CreateHandle(const FrameResource* frame)
{
...
void FillV4L2BufferByGpuMemoryBufferHandle(
    const Fourcc& fourcc, const gfx::Size& coded_size, const gfx::GpuMemoryBufferHandle& gmb_handle, V4L2WritableBufferRef* buffer)
{
...
bool AllocateV4L2Buffers(V4L2Queue* queue, const size_t num_buffers, v4l2_memory memory_type)
{
    DCHECK(queue);
    ...
    if (queue->AllocatedBuffersCount() < num_buffers) {
        VLOGF(1) << "Failed to allocate buffers. Allocated number=" << queue->AllocatedBuffersCount() << ", Requested number=" << num_buffers;
        return false;
    }
...
V4L2ImageProcessorBackend::V4L2ImageProcessorBackend(scoped_refptr<V4L2Device> device, const PortConfig& input_config, const PortConfig& output_config,
    v4l2_memory input_memory_type, v4l2_memory output_memory_type, OutputMode output_mode, ErrorCB error_cb)
    : ImageProcessorBackend(input_config, output_config, output_mode, std::move(error_cb),
    ...
    poll_task_runner_(base::ThreadPool::CreateSingleThreadTaskRunner({}, base::SingleThreadTaskRunnerThreadMode::DEDICATED))
{
    DVLOGF(2);
    DETACH_FROM_SEQUENCE(poll_sequence_checker_);
    DCHECK_NE(output_memory_type_, V4L2_MEMORY_USERPTR);

    VLOGF(2) << "V4L2ImageProcessorBackend constructed with input: " << input_config_.ToString() << ", output: " << output_config_.ToString();
...
void V4L2ImageProcessorBackend::Destroy()
{
    ...
    poll_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::DestroyOnPollSequence, poll_weak_this_));
}
...
// static
std::unique_ptr<ImageProcessorBackend> V4L2ImageProcessorBackend::Create(scoped_refptr<V4L2Device> device, size_t num_buffers, const PortConfig& input_config,
    const PortConfig& output_config, OutputMode output_mode, ErrorCB error_cb)
{
    VLOGF(2);
    ...
    if (input_config.size != output_config.size || input_config.visible_rect != output_config.visible_rect) {
        VLOGF(2) << "V4L2ImageProcessor cannot adapt size/visible_rects, input " << input_config.ToString() << ", output " << output_config.ToString();
        return nullptr;
    }
    ...
    const v4l2_memory input_memory_type = InputStorageTypeToV4L2Memory(input_config.storage_type);
    if (input_memory_type != V4L2_MEMORY_USERPTR && input_memory_type != V4L2_MEMORY_DMABUF) {
    ...
    const v4l2_memory output_memory_type = output_mode == OutputMode::ALLOCATE ? V4L2_MEMORY_MMAP : InputStorageTypeToV4L2Memory(output_config.storage_type);
    if (output_memory_type != V4L2_MEMORY_MMAP && output_memory_type != V4L2_MEMORY_DMABUF) {
    ...
    if (!device->Open(V4L2Device::Type::kImageProcessor, input_config.fourcc.ToV4L2PixFmt())) {
        VLOGF(1) << "Failed to open device with input fourcc: " << input_config.fourcc.ToString();
        return nullptr;
    }
    ...
    if (device->Ioctl(VIDIOC_S_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != input_config.fourcc.ToV4L2PixFmt()) {
    ...
    if (!gfx::Rect(negotiated_input_size).Contains(input_config.visible_rect)) {
        VLOGF(1) << "Negotiated input allocated size: " << negotiated_input_size.ToString()
                 << " should contain visible size: " << input_config.visible_rect.size().ToString();
        return nullptr;
    }
    ...
    if (device->Ioctl(VIDIOC_S_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != output_config.fourcc.ToV4L2PixFmt()) {
    ...
    if (!gfx::Rect(negotiated_output_size).Contains(gfx::Rect(output_config.size))) {
        VLOGF(1) << "Negotiated output allocated size: " << negotiated_output_size.ToString()
                 << " should contain original output allocated size: " << output_config.size.ToString();
        return nullptr;
    }
    ...
    std::unique_ptr<V4L2ImageProcessorBackend> image_processor(
        new V4L2ImageProcessorBackend(std::move(device), input_config, output_config, input_memory_type, output_memory_type, output_mode, std::move(error_cb)));

    if (!image_processor->CreateInputBuffers(num_buffers) || !image_processor->CreateOutputBuffers(num_buffers)) {
        return nullptr;
    }
...
{
    auto device = base::MakeRefCounted<V4L2Device>();
    return device->GetSupportedImageProcessorPixelformats(V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
}
...
{
    auto device = base::MakeRefCounted<V4L2Device>();
    return device->GetSupportedImageProcessorPixelformats(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
}

// static
bool V4L2ImageProcessorBackend::TryOutputFormat(
    uint32_t input_pixelformat, uint32_t output_pixelformat, const gfx::Size& input_size, gfx::Size* output_size, size_t* num_planes)
{
    DVLOGF(3) << "input_format=" << FourccToString(input_pixelformat) << " input_size=" << input_size.ToString()
              << " output_format=" << FourccToString(output_pixelformat) << " output_size=" << output_size->ToString();
    auto device = base::MakeRefCounted<V4L2Device>();
    ...
    if (device->Ioctl(VIDIOC_S_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != input_pixelformat) {
        DVLOGF(4) << "Failed to set image processor input format: " << V4L2FormatToString(format);
        return false;
    }
    ...
    if (device->Ioctl(VIDIOC_TRY_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != output_pixelformat) {
        return false;
    }

    *num_planes = format.fmt.pix_mp.num_planes;
    *output_size = V4L2Device::AllocatedSizeFromV4L2Format(format);
    DVLOGF(3) << "Adjusted output_size=" << output_size->ToString() << ", num_planes=" << *num_planes;
    return true;
}

void V4L2ImageProcessorBackend::ProcessLegacyFrame(scoped_refptr<FrameResource> frame, LegacyFrameResourceReadyCB cb)
{
...
void V4L2ImageProcessorBackend::ProcessFrame(scoped_refptr<FrameResource> input_frame, scoped_refptr<FrameResource> output_frame, FrameResourceReadyCB cb)
{
...
void V4L2ImageProcessorBackend::ProcessJobs()
{
    ...
    while (!input_job_queue_.empty()) {
        if (!input_queue_->IsStreaming()) {
            const FrameResource& input_frame = *(input_job_queue_.front()->input_frame.get());
            const gfx::Size input_buffer_size(input_frame.stride(0) / VideoFrame::BytesPerElement(input_frame.format(), 0), input_frame.coded_size().height());
            if (!ReconfigureV4L2Format(input_buffer_size, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE)) {
                NotifyError();
                return;
            }
        }

        if (input_job_queue_.front()->output_frame && // output_frame is nullptr in ALLOCATE mode.
            !output_queue_->IsStreaming()) {
            const FrameResource& output_frame = *(input_job_queue_.front()->output_frame.get());
            const gfx::Size output_buffer_size(
                output_frame.stride(0) / VideoFrame::BytesPerElement(output_frame.format(), 0), output_frame.coded_size().height());
            if (!ReconfigureV4L2Format(output_buffer_size, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE)) {
                NotifyError();
                return;
            }
        }
        ...
        if (input_memory_type_ == V4L2_MEMORY_DMABUF) {
            const FrameResource& input_frame = *(input_job_queue_.front()->input_frame.get());
            input_buffer = input_queue_->GetFreeBufferForFrame(input_frame.GetSharedMemoryId());
        }
        if (!input_buffer)
        ...
        if (output_memory_type_ == V4L2_MEMORY_DMABUF) {
            const FrameResource& output_frame = *(input_job_queue_.front()->output_frame.get());
            output_buffer = output_queue_->GetFreeBufferForFrame(output_frame.GetSharedMemoryId());
        }
        if (!output_buffer)
...
bool V4L2ImageProcessorBackend::ReconfigureV4L2Format(const gfx::Size& size, enum v4l2_buf_type type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    struct v4l2_format format;
    memset(&format, 0, sizeof(format));
    ...
    if (static_cast<int>(format.fmt.pix_mp.width) == size.width() && static_cast<int>(format.fmt.pix_mp.height) == size.height()) {
        return true;
    }
    ...
    DCHECK_GT(num_buffers, 0u);
    return queue->DeallocateBuffers() && AllocateV4L2Buffers(queue.get(), num_buffers, memory_type);
}
...
bool V4L2ImageProcessorBackend::CreateInputBuffers(size_t num_buffers)
{
    ...
    return input_queue_ && AllocateV4L2Buffers(input_queue_.get(), num_buffers, input_memory_type_);
}
...
bool V4L2ImageProcessorBackend::CreateOutputBuffers(size_t num_buffers)
{
    ...
    return output_queue_ && AllocateV4L2Buffers(output_queue_.get(), num_buffers, output_memory_type_);
}
...
void V4L2ImageProcessorBackend::TriggerPoll(bool poll_device)
{
    ...
    poll_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::DevicePollTask, poll_weak_this_, poll_device));
}
...
void V4L2ImageProcessorBackend::DevicePollTask(bool poll_device)
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::ServiceDevice, weak_this_));
}
...
void V4L2ImageProcessorBackend::ServiceDevice()
{
    ...
    const bool poll_device = (input_queue_->QueuedBuffersCount() > 0 || output_queue_->QueuedBuffersCount() > 0);
    TriggerPoll(poll_device);

    DVLOGF(3) << __func__ << ": buffer counts: INPUT[" << input_job_queue_.size() << "] => DEVICE[" << input_queue_->FreeBuffersCount() << "+"
              << input_queue_->QueuedBuffersCount() << "/" << input_queue_->AllocatedBuffersCount() << "->"
              << output_queue_->AllocatedBuffersCount() - output_queue_->QueuedBuffersCount() << "+" << output_queue_->QueuedBuffersCount() << "/"
              << output_queue_->AllocatedBuffersCount() << "]";
}

void V4L2ImageProcessorBackend::EnqueueInput(const JobRecord* job_record, V4L2WritableBufferRef buffer)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(input_queue_);
...
void V4L2ImageProcessorBackend::EnqueueOutput(JobRecord* job_record, V4L2WritableBufferRef buffer)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void V4L2ImageProcessorBackend::V4L2VFRecycleThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, std::optional<base::WeakPtr<V4L2ImageProcessorBackend>> image_processor, V4L2ReadableBufferRef buf)
{
    DVLOGF(4);
    DCHECK(image_processor);

    task_runner->PostTask(FROM_HERE, base::BindOnce(&V4L2ImageProcessorBackend::V4L2VFRecycleTask, *image_processor, std::move(buf)));
}
...
void V4L2ImageProcessorBackend::Dequeue()
{
    ...
    while (output_queue_->QueuedBuffersCount() > 0) {
        ...
        if (running_jobs_.empty() || running_jobs_.front()->output_buffer_id != buffer->BufferId()) {
        ...
        case V4L2_MEMORY_MMAP:
            ...
            // we can reuse the MMAP buffer once the client is done with it.
            {
                ...
                // runner.
                output_frame->AddDestructionObserver(base::BindOnce(&V4L2ImageProcessorBackend::V4L2VFRecycleThunk, task_runner(), weak_this_, buffer));
                break;
            }
        ...
        if (job_record->start_time) {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0(
                "media", "V4L2ImageProcessorBackend::Process", TRACE_ID_LOCAL(this), job_record->start_time.value());
            TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP1(
                "media", "V4L2ImageProcessorBackend::Process", TRACE_ID_LOCAL(this), base::TimeTicks::Now(), "timestamp", timestamp.InMilliseconds());
        }

        if (!job_record->legacy_ready_cb.is_null()) {
            std::move(job_record->legacy_ready_cb).Run(buffer->BufferId(), std::move(output_frame));
        } else {
...
bool V4L2ImageProcessorBackend::EnqueueInputRecord(const JobRecord* job_record, V4L2WritableBufferRef buffer)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(input_queue_);
    ...
    case V4L2_MEMORY_USERPTR: {
        const size_t num_planes = GetNumPlanesOfV4L2PixFmt(input_config_.fourcc.ToV4L2PixFmt());
        std::vector<void*> user_ptrs(num_planes);
        for (size_t i = 0; i < num_planes; ++i) {
            int bytes_used = VideoFrame::PlaneSize(job_record->input_frame->format(), i, input_config_.size).GetArea();
    ...
    case V4L2_MEMORY_DMABUF: {
        ...
        FillV4L2BufferByGpuMemoryBufferHandle(input_config_.fourcc, input_config_.size, *input_handle, &buffer);
        if (!std::move(buffer).QueueDMABuf(input_handle->native_pixmap_handle.planes)) {
            VPLOGF(1) << "Failed to queue a DMABUF buffer to input queue";
            NotifyError();
            return false;
        }
    ...
    DVLOGF(4) << "enqueued frame ts=" << job_record->input_frame->timestamp().InMilliseconds() << " to device.";

    return true;
}

bool V4L2ImageProcessorBackend::EnqueueOutputRecord(JobRecord* job_record, V4L2WritableBufferRef buffer)
{
    DVLOGF(4);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    case V4L2_MEMORY_DMABUF: {
        ...
        FillV4L2BufferByGpuMemoryBufferHandle(output_config_.fourcc, output_config_.size, *output_handle, &buffer);
        return std::move(buffer).QueueDMABuf(output_handle->native_pixmap_handle.planes);
    }
    default:
        NOTREACHED();
    }
