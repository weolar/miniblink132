...
{
    return VideoCodecProfileToVideoCodec(profile) == VideoCodec::kVP9 && decoder_buffer.side_data_size() > 0;
}
...
V4L2StatefulVideoDecoderBackend::DecodeRequest::DecodeRequest(scoped_refptr<DecoderBuffer> buf, VideoDecoder::DecodeCB cb, int32_t id)
    : buffer(std::move(buf))
...
bool V4L2StatefulVideoDecoderBackend::Initialize()
{
    ...
    framerate_control_ = std::make_unique<V4L2FrameRateControl>(device_, task_runner_);

    return true;
}

void V4L2StatefulVideoDecoderBackend::EnqueueDecodeTask(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb, int32_t bitstream_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
...
void V4L2StatefulVideoDecoderBackend::DoDecodeWork()
{
    ...
    const size_t data_size = current_buffer->data_size() - current_decode_request_->bytes_used;
...
// static
void V4L2StatefulVideoDecoderBackend::ReuseOutputBufferThunk(scoped_refptr<base::SequencedTaskRunner> task_runner,
    absl::optional<base::WeakPtr<V4L2StatefulVideoDecoderBackend>> weak_this, V4L2ReadableBufferRef buffer)
{
    DVLOGF(3);
    DCHECK(weak_this);
...
void V4L2StatefulVideoDecoderBackend::OnOutputBufferDequeued(V4L2ReadableBufferRef buffer)
{
    ...
    if (buffer->GetPlaneBytesUsed(0) > 0) {
        ...
        case V4L2_MEMORY_MMAP: {
            ...
            frame = VideoFrame::WrapVideoFrame(origin_frame, origin_frame->format(), origin_frame->visible_rect(), origin_frame->natural_size());
...
void V4L2StatefulVideoDecoderBackend::ChangeResolution()
{
    ...
    const size_t num_output_buffers = (ctrl ? ctrl->value : kDefaultNumOutputBuffers) + kPicsInPipeline;
    if (!ctrl)
    ...
    resolution_change_cb_ = base::BindOnce(&V4L2StatefulVideoDecoderBackend::ContinueChangeResolution, weak_this_, pic_size, *visible_rect, num_output_buffers);
...
void V4L2StatefulVideoDecoderBackend::ContinueChangeResolution(const gfx::Size& pic_size, const gfx::Rect& visible_rect, const size_t num_output_buffers)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
...
bool V4L2StatefulVideoDecoderBackend::ApplyResolution(const gfx::Size& pic_size, const gfx::Rect& visible_rect, const size_t num_output_frames)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
...
bool V4L2StatefulVideoDecoderBackend::IsSupportedProfile(VideoCodecProfile profile)
{
    ...
    if (supported_profiles_.empty()) {
        ...
        VideoDecodeAccelerator::SupportedProfiles profiles = device->GetSupportedDecodeProfiles(std::size(kSupportedInputFourccs), kSupportedInputFourccs);
