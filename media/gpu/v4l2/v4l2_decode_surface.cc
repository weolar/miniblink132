...
V4L2DecodeSurface::V4L2DecodeSurface(
    V4L2WritableBufferRef input_buffer, V4L2WritableBufferRef output_buffer, scoped_refptr<FrameResource> frame, uint64_t secure_handle)
...
void V4L2DecodeSurface::SetReferenceSurfaces(std::vector<scoped_refptr<V4L2DecodeSurface>> ref_surfaces)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
std::string V4L2DecodeSurface::ToString() const
{
    ...
    std::string out;
    base::StringAppendF(&out, "Buffer %zu -> %d. ", input_buffer_.BufferId(), output_record_);
...
void V4L2RequestDecodeSurface::PrepareSetCtrls(struct v4l2_ext_controls* ctrls) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool V4L2RequestDecodeSurface::Submit()
{
    ...
    struct timeval timestamp = { .tv_sec = 0, .tv_usec = output_record() };
    input_buffer().SetTimeStamp(timestamp);

    if (input_buffer().Memory() == V4L2_MEMORY_DMABUF) {
        if (!std::move(input_buffer()).QueueDMABuf(secure_handle(), &request_ref_)) {
            return false;
        }
