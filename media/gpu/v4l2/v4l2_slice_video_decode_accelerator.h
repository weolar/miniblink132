...
class MEDIA_GPU_EXPORT V4L2SliceVideoDecodeAccelerator : public VideoDecodeAccelerator,
                                                         public V4L2DecodeSurfaceHandler,
                                                         public base::trace_event::MemoryDumpProvider {
public:
    V4L2SliceVideoDecodeAccelerator(scoped_refptr<V4L2Device> device, EGLDisplay egl_display, const BindGLImageCallback& bind_image_cb,
        const MakeGLContextCurrentCallback& make_context_current_cb);

    V4L2SliceVideoDecodeAccelerator(const V4L2SliceVideoDecodeAccelerator&) = delete;
    V4L2SliceVideoDecodeAccelerator& operator=(const V4L2SliceVideoDecodeAccelerator&) = delete;
    ...
    void ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle) override;
...
private:
    ...
    struct OutputRecord {
        ...
        bool at_client() const
        {
            return num_times_sent_to_client > 0;
        }
    };
    ...
    void SurfaceReady(
        scoped_refptr<V4L2DecodeSurface> dec_surface, int32_t bitstream_id, const gfx::Rect& visible_rect, const VideoColorSpace& /* color_space */) override;
    bool SubmitSlice(V4L2DecodeSurface* dec_surface, const uint8_t* data, size_t size) override;
    ...
    void DismissPictures(const std::vector<int32_t>& picture_buffer_ids, base::WaitableEvent* done);
    ...
    void ImportBufferForPictureTask(int32_t picture_buffer_id, gfx::NativePixmapHandle handle);

    // Check that |planes| and |dmabuf_fds| are valid in import mode and call
    // ImportBufferForPictureTask.
    void ImportBufferForPictureForImportTask(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::NativePixmapHandle handle);
    ...
    // will be able to see.
    void CreateGLImageFor(scoped_refptr<V4L2Device> gl_device, size_t buffer_index, int32_t picture_buffer_id, gfx::NativePixmapHandle handle,
        GLuint client_texture_id, GLuint texture_id, const gfx::Size& visible_size, const Fourcc fourcc);
    ...
    void ReusePictureBufferTask(int32_t picture_buffer_id, std::unique_ptr<gl::GLFenceEGL> egl_fence);
    ...
    void OutputSurface(int32_t bitstream_id, scoped_refptr<V4L2DecodeSurface> dec_surface);
    ...
    bool ProcessFrame(V4L2ReadableBufferRef buffer, scoped_refptr<V4L2DecodeSurface>);
    void FrameProcessed(scoped_refptr<V4L2DecodeSurface> surface, size_t ip_buffer_index, scoped_refptr<VideoFrame> frame);
    ...
    // child_task_runner_.
    std::unique_ptr<base::WeakPtrFactory<VideoDecodeAccelerator::Client>> client_ptr_factory_;
    ...
    base::circular_deque<std::unique_ptr<BitstreamBufferRef>> decoder_input_queue_;
    ...
    base::queue<std::pair<int32_t, scoped_refptr<V4L2DecodeSurface>>> decoder_display_queue_;
    ...
    std::queue<std::pair<scoped_refptr<V4L2DecodeSurface>, V4L2ReadableBufferRef>> surfaces_at_ip_;
    ...
    std::queue<std::pair<std::unique_ptr<gl::GLFenceEGL>, scoped_refptr<V4L2DecodeSurface>>> surfaces_awaiting_fence_;
