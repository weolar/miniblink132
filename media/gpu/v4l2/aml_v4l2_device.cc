...
typedef int32_t (*AmlV4L2Poll)(void* context, int32_t fd, bool poll_device, bool* event_pending);
...
typedef void* (*AmlV4L2Mmap)(void* context, void* addr, size_t length, int prot, int flags, int fd, unsigned int offset);
...
int AmlV4L2Device::Ioctl(int flags, void* arg)
{
    ...
    return HANDLE_EINTR(AmlV4L2_Ioctl(context_, device_fd_.get(), static_cast<unsigned long>(flags), arg));
}
...
bool AmlV4L2Device::Poll(bool poll_device, bool* event_pending)
{
    ...
    if (HANDLE_EINTR(AmlV4L2_Poll(context_, device_fd_.get(), poll_device, event_pending)) == -1) {
...
void* AmlV4L2Device::Mmap(void* addr, unsigned int len, int prot, int flags, unsigned int offset)
{
    if (type_ != Type::kEncoder)
        return GenericV4L2Device::Mmap(addr, len, prot, flags, offset);

    return AmlV4L2_Mmap(context_, addr, len, prot, flags, device_fd_.get(), offset);
}
...
bool AmlV4L2Device::SetDevicePollInterrupt()
{
    ...
    if (HANDLE_EINTR(AmlV4L2_SetDevicePollInterrupt(context_, device_fd_.get())) == -1) {
...
bool AmlV4L2Device::ClearDevicePollInterrupt()
{
    ...
    if (HANDLE_EINTR(AmlV4L2_ClearDevicePollInterrupt(context_, device_fd_.get())) == -1) {
...
bool AmlV4L2Device::Initialize()
{
    ...
    static bool initialized = []() {
        libvpcodec = dlopen("/system/lib/libvpcodec.so", RTLD_NOW | RTLD_GLOBAL | RTLD_NODELETE | RTLD_LAZY);
...
#define AMLV4L2_DLSYM_OR_RETURN_ON_ERROR(lib, name)                                                                                                            \
    do {                                                                                                                                                       \
        AmlV4L2_##name = reinterpret_cast<AmlV4L2##name>(dlsym(lib, "AmlV4L2_" #name));                                                                        \
...
VideoEncodeAccelerator::SupportedProfiles AmlV4L2Device::GetSupportedEncodeProfiles()
{
    if (!OpenDevice())
