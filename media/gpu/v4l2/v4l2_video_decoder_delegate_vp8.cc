...
void FillV4L2SegmentationHeader(const Vp8SegmentationHeader& vp8_sgmnt_hdr, struct v4l2_vp8_segment* v4l2_sgmnt_hdr)
{
#define SET_V4L2_SGMNT_HDR_FLAG_IF(cond, flag) v4l2_sgmnt_hdr->flags |= ((vp8_sgmnt_hdr.cond) ? (flag) : 0)
    SET_V4L2_SGMNT_HDR_FLAG_IF(segmentation_enabled, V4L2_VP8_SEGMENT_FLAG_ENABLED);
    SET_V4L2_SGMNT_HDR_FLAG_IF(update_mb_segmentation_map, V4L2_VP8_SEGMENT_FLAG_UPDATE_MAP);
    SET_V4L2_SGMNT_HDR_FLAG_IF(update_segment_feature_data, V4L2_VP8_SEGMENT_FLAG_UPDATE_FEATURE_DATA);
    SET_V4L2_SGMNT_HDR_FLAG_IF(segment_feature_mode == Vp8SegmentationHeader::FEATURE_MODE_DELTA, V4L2_VP8_SEGMENT_FLAG_DELTA_VALUE_MODE);
#undef SET_V4L2_SGMNT_HDR_FLAG_IF

    SafeArrayMemcpy(v4l2_sgmnt_hdr->quant_update, vp8_sgmnt_hdr.quantizer_update_value);
...
void FillV4L2LoopFilterHeader(const Vp8LoopFilterHeader& vp8_loopfilter_hdr, struct v4l2_vp8_loop_filter* v4l2_lf_hdr)
{
#define SET_V4L2_LF_HDR_FLAG_IF(cond, flag) v4l2_lf_hdr->flags |= ((vp8_loopfilter_hdr.cond) ? (flag) : 0)
    ...
    SET_V4L2_LF_HDR_FLAG_IF(type == Vp8LoopFilterHeader::LOOP_FILTER_TYPE_SIMPLE, V4L2_VP8_LF_FILTER_TYPE_SIMPLE);
...
#undef LF_HDR_TO_V4L2_LF_HDR
    ...
    SafeArrayMemcpy(v4l2_lf_hdr->ref_frm_delta, vp8_loopfilter_hdr.ref_frame_delta);
    SafeArrayMemcpy(v4l2_lf_hdr->mb_mode_delta, vp8_loopfilter_hdr.mb_mode_delta);
}

void FillV4L2QuantizationHeader(const Vp8QuantizationHeader& vp8_quant_hdr, struct v4l2_vp8_quantization* v4l2_quant_hdr)
{
...
void FillV4L2Vp8EntropyHeader(const Vp8EntropyHeader& vp8_entropy_hdr, struct v4l2_vp8_entropy* v4l2_entropy_hdr)
{
    ...
    SafeArrayMemcpy(v4l2_entropy_hdr->uv_mode_probs, vp8_entropy_hdr.uv_mode_probs);
...
public:
    ...
    V4L2VP8Picture* AsV4L2VP8Picture() override
    {
        return this;
    }
...
V4L2VideoDecoderDelegateVP8::V4L2VideoDecoderDelegateVP8(V4L2DecodeSurfaceHandler* surface_handler, V4L2Device* device)
...
bool V4L2VideoDecoderDelegateVP8::SubmitDecode(scoped_refptr<VP8Picture> pic, const Vp8ReferenceFrameVector& reference_frames)
{
...
#define SET_V4L2_FRM_HDR_FLAG_IF(cond, flag) v4l2_frame_hdr.flags |= ((frame_hdr->cond) ? (flag) : 0)
    SET_V4L2_FRM_HDR_FLAG_IF(frame_type == Vp8FrameHeader::KEYFRAME, V4L2_VP8_FRAME_FLAG_KEY_FRAME);
    SET_V4L2_FRM_HDR_FLAG_IF(sign_bias_golden, V4L2_VP8_FRAME_FLAG_SIGN_BIAS_GOLDEN);
    SET_V4L2_FRM_HDR_FLAG_IF(sign_bias_alternate, V4L2_VP8_FRAME_FLAG_SIGN_BIAS_ALT);
    ...
    SET_V4L2_FRM_HDR_FLAG_IF(mb_no_skip_coeff, V4L2_VP8_FRAME_FLAG_MB_NO_SKIP_COEFF);
#undef SET_V4L2_FRM_HDR_FLAG_IF

    FillV4L2SegmentationHeader(frame_hdr->segmentation_hdr, &v4l2_frame_hdr.segment);

    FillV4L2LoopFilterHeader(frame_hdr->loopfilter_hdr, &v4l2_frame_hdr.lf);

    FillV4L2QuantizationHeader(frame_hdr->quantization_hdr, &v4l2_frame_hdr.quant);
    ...
    v4l2_frame_hdr.first_part_header_bits = base::checked_cast<__u32>(frame_hdr->macroblock_bit_offset);
    v4l2_frame_hdr.num_dct_parts = frame_hdr->num_of_dct_partitions;

    static_assert(std::extent<decltype(v4l2_frame_hdr.dct_part_sizes)>() == std::extent<decltype(frame_hdr->dct_partition_sizes)>(),
        "DCT partition size arrays must have equal number of elements");
    for (size_t i = 0; i < frame_hdr->num_of_dct_partitions && i < std::size(v4l2_frame_hdr.dct_part_sizes); ++i)
        v4l2_frame_hdr.dct_part_sizes[i] = frame_hdr->dct_partition_sizes[i];

    scoped_refptr<V4L2DecodeSurface> dec_surface = VP8PictureToV4L2DecodeSurface(pic.get());
    std::vector<scoped_refptr<V4L2DecodeSurface>> ref_surfaces;

    const auto last_frame = reference_frames.GetFrame(Vp8RefType::VP8_FRAME_LAST);
    if (last_frame) {
        scoped_refptr<V4L2DecodeSurface> last_frame_surface = VP8PictureToV4L2DecodeSurface(last_frame.get());
    ...
    if (golden_frame) {
        scoped_refptr<V4L2DecodeSurface> golden_frame_surface = VP8PictureToV4L2DecodeSurface(golden_frame.get());
    ...
    if (alt_frame) {
        scoped_refptr<V4L2DecodeSurface> alt_frame_surface = VP8PictureToV4L2DecodeSurface(alt_frame.get());
...
{
    surface_handler_->SurfaceReady(VP8PictureToV4L2DecodeSurface(pic.get()), pic->bitstream_id(), pic->visible_rect(), pic->get_colorspace());
    return true;
}

scoped_refptr<V4L2DecodeSurface> V4L2VideoDecoderDelegateVP8::VP8PictureToV4L2DecodeSurface(VP8Picture* pic)
{
