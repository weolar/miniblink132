...
class MEDIA_GPU_EXPORT V4L2Device : public base::RefCountedThreadSafe<V4L2Device> {
public:
    // Utility format conversion functions
    // Calculates the largest plane's allocation size requested by a V4L2 device.
    static gfx::Size AllocatedSizeFromV4L2Format(const struct v4l2_format& format);
    ...
    VideoEncodeAccelerator::SupportedRateControlMode GetSupportedRateControlMode();
    ...
    std::vector<uint32_t> GetSupportedImageProcessorPixelformats(v4l2_buf_type buf_type);

    // Return supported profiles for decoder, including only profiles for given
    // fourcc |pixelformats|.
    VideoDecodeAccelerator::SupportedProfiles GetSupportedDecodeProfiles(const std::vector<uint32_t>& pixelformats);
    ...
    [[nodiscard]] bool StartPolling(V4L2DevicePoller::EventCallback event_callback, base::RepeatingClosure error_callback);
    ...
    bool SetExtCtrls(uint32_t ctrl_class, std::vector<V4L2ExtCtrl> ctrls, V4L2RequestRef* request_ref = nullptr);
    ...
    void set_secure_allocate_cb(AllocateSecureBufferAsCallback secure_allocate_cb)
    {
...
private:
    ...
    VideoDecodeAccelerator::SupportedProfiles EnumerateSupportedDecodeProfiles(const std::vector<uint32_t>& pixelformats);
