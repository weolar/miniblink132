...
public:
    static std::unique_ptr<InputBufferFragmentSplitter> CreateFromProfile(media::VideoCodecProfile profile);
    ...
    virtual bool AdvanceFrameFragment(const uint8_t* data, size_t size, size_t* endpos);
...
public:
    ...
    bool AdvanceFrameFragment(const uint8_t* data, size_t size, size_t* endpos) override;
    void Reset() override;
