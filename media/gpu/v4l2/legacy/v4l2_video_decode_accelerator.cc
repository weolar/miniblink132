...
{
    return input_format_fourcc == V4L2_PIX_FMT_VP9 && decoder_buffer.has_side_data() && !decoder_buffer.side_data()->spatial_layers.empty();
}
...
struct V4L2VideoDecodeAccelerator::BitstreamBufferRef {
    BitstreamBufferRef(
        base::WeakPtr<Client>& client, scoped_refptr<base::SequencedTaskRunner>& client_task_runner, scoped_refptr<DecoderBuffer> buffer, int32_t input_id);
    ~BitstreamBufferRef();
...
V4L2VideoDecodeAccelerator::V4L2VideoDecodeAccelerator(scoped_refptr<V4L2Device> device)
...
bool V4L2VideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    ...
    if (config.output_mode != Config::OutputMode::kAllocate && config.output_mode != Config::OutputMode::kImport) {
...
bool V4L2VideoDecodeAccelerator::CheckConfig(const Config& config)
{
    ...
    if (input_format_fourcc_ == V4L2_PIX_FMT_INVALID || !device_->Open(V4L2Device::Type::kDecoder, input_format_fourcc_)) {
...
{
    DVLOGF(4) << "input_id=" << bitstream_id << ", size=" << (buffer ? buffer->size() : 0);
...
void V4L2VideoDecodeAccelerator::AssignPictureBuffersTask(const std::vector<PictureBuffer>& buffers)
{
    ...
    if (!image_processor_device_ && !prefer_software_mt21 && output_mode_ == Config::OutputMode::kImport) {
        memory = V4L2_MEMORY_DMABUF;
    } else {
        memory = V4L2_MEMORY_MMAP;
    }

    if (output_queue_->AllocateBuffers(buffers.size(), memory, prefer_software_mt21) == 0) {
    ...
    for (auto&& buffer : v4l2_buffers) {
        ...
        if (output_mode_ == Config::OutputMode::kAllocate && !image_processor_device_)
        ...
        if (output_mode_ == Config::OutputMode::kAllocate) {
            ...
            if (!image_processor_device_) {
                ...
                // available.
                native_pixmap = frame->CreateGpuMemoryBufferHandle().native_pixmap_handle;
            }
...
void V4L2VideoDecodeAccelerator::ImportBufferForPictureTask(int32_t picture_buffer_id, gfx::NativePixmapHandle handle)
{
...
#ifdef SUPPORT_MT21_PIXEL_FORMAT_SOFTWARE_DECOMPRESSION
        if (base::FeatureList::IsEnabled(media::kPreferSoftwareMT21) && !mt21_decompressor_) {
    ...
    if (output_mode_ == Config::OutputMode::kImport) {
        ...
        iter->output_frame = NativePixmapFrameResource::Create(*layout, gfx::Rect(visible_size_), visible_size_, std::move(duped_fds), base::TimeDelta());
    }
...
void V4L2VideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_buffer_id)
{
    ...
    decoder_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::ReusePictureBufferTask, base::Unretained(this), picture_buffer_id));
}
...
bool V4L2VideoDecodeAccelerator::TryToSetupDecodeOnSeparateSequence(
    const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SequencedTaskRunner>& decode_task_runner)
{
    VLOGF(2);
...
void V4L2VideoDecodeAccelerator::DecodeBufferTask()
{
    ...
    if (decoder_current_bitstream_buffer_ == NULL) {
        ...
        if (buffer) {
            DVLOGF(4) << "reading input_id=" << decoder_current_bitstream_buffer_->input_id << ", addr=" << buffer->data() << ", size=" << buffer->size();
        } else {
    ...
    if (!buffer) {
        ...
        const size_t data_size = buffer->size() - decoder_current_bitstream_buffer_->bytes_used;
    ...
    if (schedule_task) {
        ...
        if ((buffer ? buffer->size() : 0) == decoder_current_bitstream_buffer_->bytes_used) {
...
bool V4L2VideoDecodeAccelerator::SetupFormats()
{
    ...
    GetSupportedResolution(base::BindRepeating(&V4L2Device::Ioctl, device_), input_format_fourcc_, &min_resolution, &max_resolution);
...
bool V4L2VideoDecodeAccelerator::CreateImageProcessor()
{
    ...
    const ImageProcessor::OutputMode image_processor_output_mode
        = (output_mode_ == Config::OutputMode::kAllocate ? ImageProcessor::OutputMode::ALLOCATE : ImageProcessor::OutputMode::IMPORT);
...
bool V4L2VideoDecodeAccelerator::ProcessFrame(int32_t bitstream_buffer_id, V4L2ReadableBufferRef buf)
{
    ...
    if (base::FeatureList::IsEnabled(media::kPreferSoftwareMT21)) {
        ...
        output_frame_mapper = VideoFrameMapperFactory::CreateMapper(PIXEL_FORMAT_NV12, VideoFrame::STORAGE_DMABUFS,
            /*force_linear_buffer_mapper=*/true);
        if (!output_frame_mapper) {
            output_frame_mapper = VideoFrameMapperFactory::CreateMapper(PIXEL_FORMAT_NV12, VideoFrame::STORAGE_GPU_MEMORY_BUFFER,
                /*force_linear_buffer_mapper=*/true);
        }
        if (!output_frame_mapper) {
        ...
        scoped_refptr<FrameResource> mapped_output_frame
            = VideoFrameResource::Create(output_frame_mapper->MapFrame(output_record.output_frame, PROT_READ | PROT_WRITE));
...

        {
            ...
            mt21_decompressor_->MT21ToNV12(static_cast<const uint8_t*>(buf->GetPlaneMapping(0)), static_cast<const uint8_t*>(buf->GetPlaneMapping(1)),
                buf->GetPlaneBytesUsed(0), buf->GetPlaneBytesUsed(1), mapped_output_frame->GetWritableVisibleData(VideoFrame::Plane::kY),
    ...
    scoped_refptr<FrameResource> cropped_input_frame = input_frame->CreateWrappingFrame(visible_rect, natural_size);
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        image_processor_->Process(std::move(cropped_input_frame), output_record.output_frame,
            base::BindOnce(&V4L2VideoDecodeAccelerator::FrameProcessed, base::Unretained(this), bitstream_buffer_id, buf->BufferId()));
    } else {
        image_processor_->Process(
            std::move(cropped_input_frame), base::BindOnce(&V4L2VideoDecodeAccelerator::FrameProcessed, base::Unretained(this), bitstream_buffer_id));
    }
    return true;
}
...
bool V4L2VideoDecodeAccelerator::CreateOutputBuffers()
{
    ...
    VideoPixelFormat pixel_format = (output_mode_ == Config::OutputMode::kImport) ? egl_image_format_fourcc_->ToVideoPixelFormat() : PIXEL_FORMAT_UNKNOWN;

    child_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&Client::ProvidePictureBuffersWithVisibleRect, client_, buffer_count, pixel_format, egl_image_size_, gfx::Rect(visible_size_)));
...
void V4L2VideoDecodeAccelerator::SendBufferToClient(
    size_t output_buffer_index, int32_t bitstream_buffer_id, V4L2ReadableBufferRef vda_buffer, scoped_refptr<FrameResource> frame)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    DCHECK_GE(bitstream_buffer_id, 0);
    ...
    const Picture picture(output_record.picture_id, bitstream_buffer_id, gfx::Rect(visible_size_));
...
void V4L2VideoDecodeAccelerator::FrameProcessed(int32_t bitstream_buffer_id, size_t ip_buffer_index, scoped_refptr<FrameResource> frame)
{
