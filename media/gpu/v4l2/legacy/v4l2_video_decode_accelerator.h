...
public:
    ...
    bool TryToSetupDecodeOnSeparateSequence(
        const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SequencedTaskRunner>& decode_task_runner) override;
...
private:
    ...
    void SendBufferToClient(size_t buffer_index, int32_t bitstream_buffer_id, V4L2ReadableBufferRef vda_buffer, scoped_refptr<FrameResource> frame = nullptr);
    ...
    // image processor.
    void FrameProcessed(int32_t bitstream_buffer_id, size_t output_buffer_index, scoped_refptr<FrameResource> frame);
    ...
    std::map<int32_t, std::pair<V4L2ReadableBufferRef, scoped_refptr<FrameResource>>> buffers_at_client_;
