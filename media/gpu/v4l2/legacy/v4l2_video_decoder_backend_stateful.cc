...
bool IsVp9KSVCStream(VideoCodecProfile profile, const DecoderBuffer& decoder_buffer)
{
    return VideoCodecProfileToVideoCodec(profile) == VideoCodec::kVP9 && decoder_buffer.has_side_data() && !decoder_buffer.side_data()->spatial_layers.empty();
}
...
V4L2StatefulVideoDecoderBackend::DecodeRequest::DecodeRequest(scoped_refptr<DecoderBuffer> buf, VideoDecoder::DecodeCB cb)
    : buffer(std::move(buf))
    , decode_cb(std::move(cb))
{
}

V4L2StatefulVideoDecoderBackend::DecodeRequest::DecodeRequest(DecodeRequest&&) = default;
V4L2StatefulVideoDecoderBackend::DecodeRequest& V4L2StatefulVideoDecoderBackend::DecodeRequest::operator=(DecodeRequest&&) = default;
...
V4L2StatefulVideoDecoderBackend::V4L2StatefulVideoDecoderBackend(Client* const client, scoped_refptr<V4L2Device> device, VideoCodecProfile profile,
    const VideoColorSpace& color_space, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
bool V4L2StatefulVideoDecoderBackend::Initialize()
{
    ...
    frame_splitter_ = v4l2_vda_helpers::InputBufferFragmentSplitter::CreateFromProfile(profile_);
    if (!frame_splitter_) {
    ...
    framerate_control_ = std::make_unique<V4L2FrameRateControl>(base::BindRepeating(&V4L2Device::Ioctl, device_), task_runner_);

    return true;
}

void V4L2StatefulVideoDecoderBackend::EnqueueDecodeTask(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
    ...
    decode_request_queue_.push(DecodeRequest(std::move(buffer), std::move(decode_cb)));
...
void V4L2StatefulVideoDecoderBackend::DoDecodeWork()
{
    ...
    if (!current_input_buffer_) {
        ...
        const struct timespec timespec = current_decode_request_->buffer->timestamp().ToTimeSpec();
        ...
        const int64_t flat_timespec = base::TimeDelta::FromTimeSpec(timespec).InMilliseconds();
    ...
    const uint8_t* const data = current_buffer->data() + current_decode_request_->bytes_used;
    const size_t data_size = current_buffer->size() - current_decode_request_->bytes_used;
    size_t bytes_to_copy = 0;

    if (!frame_splitter_->AdvanceFrameFragment(data, data_size, &bytes_to_copy)) {
        LOG(ERROR) << "Invalid bitstream detected.";
        std::move(current_decode_request_->decode_cb).Run(DecoderStatus::Codes::kFailed);
    ...
    uint8_t* dst = static_cast<uint8_t*>(current_input_buffer_->GetPlaneMapping(0)) + bytes_used;
...
void V4L2StatefulVideoDecoderBackend::ScheduleDecodeWork()
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2StatefulVideoDecoderBackend::DoDecodeWork, weak_this_));
}

void V4L2StatefulVideoDecoderBackend::ProcessEventQueue()
{
    while (std::optional<struct v4l2_event> ev = device_->DequeueEvent()) {
        if (ev->type == V4L2_EVENT_SOURCE_CHANGE && (ev->u.src_change.changes & V4L2_EVENT_SRC_CH_RESOLUTION)) {
            ChangeResolution();
        }
    }
}
...
void V4L2StatefulVideoDecoderBackend::EnqueueOutputBuffers()
{
    ...
    while (true) {
        ...
        case V4L2_MEMORY_DMABUF: {
            ...
            buffer = output_queue_->GetFreeBufferForFrame(frame->GetSharedMemoryId());
            if (!buffer) {
                no_buffer = true;
                break;
            }
    ...
    DVLOGF(3) << output_queue_->QueuedBuffersCount() << "/" << output_queue_->AllocatedBuffersCount() << " output buffers queued";
}

scoped_refptr<FrameResource> V4L2StatefulVideoDecoderBackend::GetPoolVideoFrame()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
    ...
    if (!frame) {
        ...
        pool->NotifyWhenFrameAvailable(base::BindOnce(base::IgnoreResult(&base::SequencedTaskRunner::PostTask), task_runner_, FROM_HERE,
            base::BindOnce(base::IgnoreResult(&V4L2StatefulVideoDecoderBackend::EnqueueOutputBuffers), weak_this_)));
    }
    return frame;
}
...
void V4L2StatefulVideoDecoderBackend::ReuseOutputBufferThunk(
    scoped_refptr<base::SequencedTaskRunner> task_runner, std::optional<base::WeakPtr<V4L2StatefulVideoDecoderBackend>> weak_this, V4L2ReadableBufferRef buffer)
{
    DVLOGF(3);
    DCHECK(weak_this);
    ...
    if (task_runner->RunsTasksInCurrentSequence()) {
        ...
        task_runner->PostTask(FROM_HERE, base::BindOnce(&V4L2StatefulVideoDecoderBackend::ReuseOutputBuffer, *weak_this, std::move(buffer)));
    }
}

void V4L2StatefulVideoDecoderBackend::ReuseOutputBuffer(V4L2ReadableBufferRef buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void V4L2StatefulVideoDecoderBackend::OnOutputBufferDequeued(V4L2ReadableBufferRef buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
    ...
    if (buffer->GetPlaneBytesUsed(0) > 0) {
        ...
        case V4L2_MEMORY_MMAP: {
            ...
            frame->AddDestructionObserver(base::BindOnce(&V4L2StatefulVideoDecoderBackend::ReuseOutputBufferThunk, task_runner_, weak_this_, buffer));
            break;
        }
        case V4L2_MEMORY_DMABUF:
        ...
        client_->OutputFrame(std::move(frame), *visible_rect_, color_space_, timestamp);
    }
...
bool V4L2StatefulVideoDecoderBackend::InitiateFlush(VideoDecoder::DecodeCB flush_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
    DCHECK(!flush_cb_);
...
void V4L2StatefulVideoDecoderBackend::ChangeResolution()
{
    ...
    if (!gfx::Rect(pic_size).Contains(*visible_rect)) {
        LOG(ERROR) << "Visible rectangle (" << visible_rect->ToString() << ") is not contained by the picture rectangle (" << gfx::Rect(pic_size).ToString()
                   << ").";
        client_->OnBackendError();
        return;
    }

    const auto bit_depth = V4L2PixelFormatToBitDepth(format->fmt.pix_mp.pixelformat);
    if (!bit_depth) {
    ...
    resolution_change_cb_ = base::BindOnce(
        &V4L2StatefulVideoDecoderBackend::ContinueChangeResolution, weak_this_, pic_size, *visible_rect, num_codec_reference_frames, *bit_depth);
...
bool V4L2StatefulVideoDecoderBackend::ApplyResolution(const gfx::Size& pic_size, const gfx::Rect& visible_rect)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
...
void V4L2StatefulVideoDecoderBackend::ClearPendingRequests(DecoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(3);
...
bool V4L2StatefulVideoDecoderBackend::IsSupportedProfile(VideoCodecProfile profile)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(device_);
...
size_t V4L2StatefulVideoDecoderBackend::GetNumOUTPUTQueueBuffers(bool secure_mode) const
{
    CHECK(!secure_mode);
