...
private:
    ...
    // an empty vector.
    std::vector<ImageProcessor::PixelLayoutCandidate> EnumeratePixelLayoutCandidates(const gfx::Size& coded_size);
    ...
    SupportedVideoDecoderConfigs supported_configs_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    base::queue<std::pair<scoped_refptr<DecoderBuffer>, DecodeCB>> decoder_buffer_and_callbacks_;
    ...
    base::CancelableTaskTracker cancelable_task_tracker_ GUARDED_BY_CONTEXT(sequence_checker_);

    // Optional helper class to reassemble full H.264 frames out of NALUs.
    std::unique_ptr<H264FrameReassembler> h264_frame_reassembler_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    base::WeakPtrFactory<V4L2StatefulVideoDecoder> weak_ptr_factory_for_CAPTURE_availability_;
};
