...
class V4L2MinigbmTest : public testing::TestWithParam<std::tuple<VideoCodecProfile, gfx::Size>> {
public:
    ...
    struct PrintToStringParamName {
        ...
        {
            return base::StringPrintf("%s__%s", VideoCodecProfileToString(std::get<0>(info.param)), std::get<1>(info.param).ToString().c_str());
        }
    };
};

void TestStatefulDecoderAllocations(uint32_t codec_fourcc, scoped_refptr<V4L2Device> device, uint32_t chosen_v4l2_pixel_format, gfx::Size resolution)
{
    scoped_refptr<V4L2Queue> OUTPUT_queue = device->GetQueue(V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
    ASSERT_NE(OUTPUT_queue.get(), nullptr);
    const std::optional<struct v4l2_format> input_v4l2_format = OUTPUT_queue->SetFormat(codec_fourcc, gfx::Size(), /*buffer_size=*/1E6);
    ...
    ASSERT_EQ(codec_fourcc, input_v4l2_format.value().fmt.pix_mp.pixelformat) << "The driver should never changed the codec :)";
    LOG(INFO) << " Chosen codec: " << FourccToString(codec_fourcc);
    constexpr size_t kNumInputBuffers = 8;
    ASSERT_NE(OUTPUT_queue->AllocateBuffers(kNumInputBuffers, V4L2_MEMORY_MMAP, false), 0u);

    scoped_refptr<V4L2Queue> CAPTURE_queue = device->GetQueue(V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    ASSERT_NE(CAPTURE_queue.get(), nullptr);
    std::optional<struct v4l2_format> output_v4l2_format = CAPTURE_queue->SetFormat(chosen_v4l2_pixel_format, resolution,
        /*buffer_size=*/0);
    ASSERT_TRUE(output_v4l2_format.has_value());
    const gfx::Size coded_size(output_v4l2_format->fmt.pix_mp.width, output_v4l2_format->fmt.pix_mp.height);
    LOG_IF(INFO, resolution != coded_size) << "Device adjusted " << resolution.ToString() << " to " << coded_size.ToString();
    constexpr size_t kNumOutputBuffers = VIDEO_MAX_FRAME;
    ASSERT_NE(CAPTURE_queue->AllocateBuffers(kNumOutputBuffers, V4L2_MEMORY_MMAP, false), 0u);

    // Determines proper device driver to use for setting up GBM.
    base::FilePath drm_path;
    base::FileEnumerator fe(kDecoderDevicePrefix, true, base::FileEnumerator::FILES);
    ...
    base::File drm_fd(drm_path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE);
    ...
    struct gbm_bo* bo
        = gbm_bo_create(gbm, coded_size.width(), coded_size.height(), gbm_format, GBM_BO_USE_SCANOUT | GBM_BO_USE_TEXTURING | GBM_BO_USE_HW_VIDEO_DECODER);
    ASSERT_TRUE(bo);

    EXPECT_EQ(coded_size, gfx::Size(base::checked_cast<int>(gbm_bo_get_width(bo)), base::checked_cast<int>(gbm_bo_get_height(bo))));
    ...
    std::vector<int32_t> strides = VideoFrame::ComputeStrides(PIXEL_FORMAT_NV12, coded_size);
...
TEST_P(V4L2MinigbmTest, AllocateAndCompareWithMinigbm)
{
    ...
    const auto fourcc_stateful = VideoCodecProfileToV4L2PixFmt(video_codec_profile, /*slice_based=*/false);
    const bool is_stateful = device->Open(V4L2Device::Type::kDecoder, fourcc_stateful);

    constexpr auto kCapsRequired = V4L2_CAP_VIDEO_M2M_MPLANE | V4L2_CAP_STREAMING;
    struct v4l2_capability caps;
    if (device->Ioctl(VIDIOC_QUERYCAP, &caps) || (caps.capabilities & kCapsRequired) != kCapsRequired) {
        GTEST_SKIP() << "Device doesn't support expected capabilities";
    }

    constexpr uint32_t desired_v4l2_pixel_formats[] = { V4L2_PIX_FMT_NV12, V4L2_PIX_FMT_NV12M };
    std::vector<uint32_t> supported_v4l2_pixel_formats
        = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, device), V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    int32_t chosen_v4l2_pixel_format = 0;
    for (const auto supported_v4l2_pixel_format : supported_v4l2_pixel_formats) {
        if (base::Contains(desired_v4l2_pixel_formats, supported_v4l2_pixel_format)) {
    ...
    if (is_stateful) {
        TestStatefulDecoderAllocations(fourcc_stateful, device, chosen_v4l2_pixel_format, resolution);
    }
}

constexpr VideoCodecProfile kVideoCodecProfiles[] = { H264PROFILE_BASELINE };
constexpr gfx::Size kResolutions[] = { gfx::Size(127, 128), gfx::Size(128, 128), gfx::Size(323, 243), gfx::Size(640, 360), gfx::Size(1280, 720) };

INSTANTIATE_TEST_SUITE_P(, V4L2MinigbmTest, ::testing::Combine(::testing::ValuesIn(kVideoCodecProfiles), ::testing::ValuesIn(kResolutions)),
    V4L2MinigbmTest::PrintToStringParamName());

class MockVideoDecoderMixinClient : public VideoDecoderMixin::Client {
public:
    MockVideoDecoderMixinClient()
...
TEST_F(V4L2FlatVideoDecoderTest, UnsupportedVideoCodec)
{
    ...
    auto decoder = V4L2StatefulVideoDecoder::Create(
        std::make_unique<MockMediaLog>(), base::SequencedTaskRunner::GetCurrentDefault(), mock_client.weak_ptr_factory_.GetWeakPtr());

    const auto unsupported_config = TestVideoConfig::Normal(VideoCodec::kMPEG2);
    EXPECT_CALL(mock_client, InitCallback(DecoderStatus(DecoderStatus::Codes::kUnsupportedConfig)));
    static_cast<V4L2StatefulVideoDecoder*>(decoder.get())
        ->Initialize(unsupported_config, /*low_delay=*/false,
            /*cdm_context=*/nullptr, base::BindOnce(&MockVideoDecoderMixinClient::InitCallback, mock_client.weak_ptr_factory_.GetWeakPtr()),
...
TEST_F(V4L2FlatVideoDecoderTest, TooManyDecoderInstances)
{
    ...
    const int kMaxNumOfInstances = V4L2StatefulVideoDecoder::GetMaxNumDecoderInstancesForTesting();

    ::testing::InSequence s;
    EXPECT_CALL(mock_client, InitCallback(DecoderStatus(DecoderStatus::Codes::kOk))).Times(::testing::Exactly(kMaxNumOfInstances));
    ...
    for (auto& decoder : decoders) {
        ...
        static_cast<V4L2StatefulVideoDecoder*>(decoder.get())
            ->Initialize(supported_config,
                ...
                base::BindOnce(&MockVideoDecoderMixinClient::InitCallback, mock_client.weak_ptr_factory_.GetWeakPtr()),
                /*output_cb=*/base::DoNothing(),
                /*waiting_cb*/ base::DoNothing());
    }
    ...
    // Next one fails:
    EXPECT_CALL(mock_client, InitCallback(DecoderStatus(DecoderStatus::Codes::kTooManyDecoders)));
    auto decoder = V4L2StatefulVideoDecoder::Create(
        std::make_unique<MockMediaLog>(), base::SequencedTaskRunner::GetCurrentDefault(), mock_client.weak_ptr_factory_.GetWeakPtr());
    static_cast<V4L2StatefulVideoDecoder*>(decoder.get())
        ->Initialize(supported_config,
