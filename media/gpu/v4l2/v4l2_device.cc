...
public:
    static scoped_refptr<V4L2Queue> CreateQueue(scoped_refptr<V4L2Device> dev, enum v4l2_buf_type type, base::OnceClosure destroy_cb)
    {
        return new V4L2Queue(base::BindRepeating(&V4L2Device::Ioctl, dev), base::BindRepeating(&V4L2Device::SchedulePoll, dev),
            base::BindRepeating(&V4L2Device::Mmap, dev), dev->get_secure_allocate_cb(), type, std::move(destroy_cb));
    }
};
...
scoped_refptr<V4L2Queue> V4L2Device::GetQueue(enum v4l2_buf_type type)
{
    ...
    scoped_refptr<V4L2Queue> queue = V4L2QueueFactory::CreateQueue(this, type, base::BindOnce(&V4L2Device::OnQueueDestroyed, this, type));
...
bool V4L2Device::Open(Type type, uint32_t v4l2_pixfmt)
{
    ...
    if (path.empty()) {
        VLOGF(1) << "No devices supporting " << FourccToString(v4l2_pixfmt) << " for type: " << static_cast<int>(type);
        return false;
    }
...
// static
int32_t V4L2Device::VideoCodecProfileToV4L2H264Profile(VideoCodecProfile profile)
{
    switch (profile) {
...
// static
gfx::Size V4L2Device::AllocatedSizeFromV4L2Format(const struct v4l2_format& format)
{
    gfx::Size coded_size;
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(format.type)) {
        ...
        visible_size.SetSize(base::checked_cast<int>(format.fmt.pix.width), base::checked_cast<int>(format.fmt.pix.height));
        ...
        frame_format = frame_fourcc ? frame_fourcc->ToVideoPixelFormat() : PIXEL_FORMAT_UNKNOWN;
    }
...
bool V4L2Device::CanCreateEGLImageFrom(const Fourcc fourcc) const
{
    ...
    return base::Contains(kEGLImageDrmFmtsSupported, V4L2PixFmtToDrmFormat(fourcc.ToV4L2PixFmt()));
}
...
VideoEncodeAccelerator::SupportedRateControlMode V4L2Device::GetSupportedRateControlMode()
{
    ...
    for (query_menu.index = query_ctrl.minimum; base::checked_cast<int>(query_menu.index) <= query_ctrl.maximum; query_menu.index++) {
...
std::vector<uint32_t> V4L2Device::GetSupportedImageProcessorPixelformats(v4l2_buf_type buf_type)
{
    ...
    for (const auto& device : devices) {
        ...
        const auto pixelformats = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, this), buf_type);

        supported_pixelformats.insert(supported_pixelformats.end(), pixelformats.begin(), pixelformats.end());
        CloseDevice();
    }
...
VideoDecodeAccelerator::SupportedProfiles V4L2Device::GetSupportedDecodeProfiles(const std::vector<uint32_t>& pixelformats)
{
    ...
    for (const auto& device : devices) {
        ...
        supported_profiles.insert(supported_profiles.end(), profiles.begin(), profiles.end());
        CloseDevice();
    }
...
VideoEncodeAccelerator::SupportedProfiles V4L2Device::GetSupportedEncodeProfiles()
{
...
VideoDecodeAccelerator::SupportedProfiles V4L2Device::EnumerateSupportedDecodeProfiles(const std::vector<uint32_t>& pixelformats)
{
    VideoDecodeAccelerator::SupportedProfiles profiles;

    const auto v4l2_codecs_as_pix_fmts = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, this), V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
    ...
    for (uint32_t pixelformat : v4l2_codecs_as_pix_fmts) {
        ...
        if ((pixelformat == V4L2_PIX_FMT_AV1 || pixelformat == V4L2_PIX_FMT_AV1_FRAME) && !base::FeatureList::IsEnabled(kChromeOSHWAV1Decoder)) {
            continue;
        }

        VideoDecodeAccelerator::SupportedProfile profile;
        GetSupportedResolution(base::BindRepeating(&V4L2Device::Ioctl, this), pixelformat, &profile.min_resolution, &profile.max_resolution);

        const auto video_codec_profiles = EnumerateSupportedProfilesForV4L2Codec(base::BindRepeating(&V4L2Device::Ioctl, this), pixelformat);

        for (const auto& video_codec_profile : video_codec_profiles) {
            profile.profile = video_codec_profile;
            profiles.push_back(profile);

            DVLOGF(3) << "Found decoder profile " << GetProfileName(profile.profile) << ", resolutions: " << profile.min_resolution.ToString() << " "
...
VideoEncodeAccelerator::SupportedProfiles V4L2Device::EnumerateSupportedEncodeProfiles()
{
    VideoEncodeAccelerator::SupportedProfiles profiles;

    const auto v4l2_codecs_as_pix_fmts = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, this), V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    ...
    for (const auto& pixelformat : v4l2_codecs_as_pix_fmts) {
        ...
        if (profile.rate_control_modes == VideoEncodeAccelerator::kNoMode) {
            DLOG(ERROR) << "Skipped because no bitrate mode is supported for " << FourccToString(pixelformat);
            continue;
        }
        gfx::Size min_resolution;
        GetSupportedResolution(base::BindRepeating(&V4L2Device::Ioctl, this), pixelformat, &min_resolution, &profile.max_resolution);
        const auto video_codec_profiles = EnumerateSupportedProfilesForV4L2Codec(base::BindRepeating(&V4L2Device::Ioctl, this), pixelformat);

        for (const auto& video_codec_profile : video_codec_profiles) {
            profile.profile = video_codec_profile;

            profile.scalability_modes = GetSupportedScalabilityModesForV4L2Codec(base::BindRepeating(&V4L2Device::Ioctl, this), video_codec_profile);

            profiles.push_back(profile);

            DVLOGF(3) << "Found encoder profile " << GetProfileName(profile.profile) << ", max resolution: " << profile.max_resolution.ToString();
        }
    }
...
bool V4L2Device::StartPolling(V4L2DevicePoller::EventCallback event_callback, base::RepeatingClosure error_callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);

    if (!device_poller_) {
        device_poller_ = std::make_unique<V4L2DevicePoller>(this, "V4L2DevicePollerThread");
    }

    bool ret = device_poller_->StartPolling(std::move(event_callback), std::move(error_callback));
...
V4L2RequestsQueue* V4L2Device::GetRequestsQueue()
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        base::ScopedFD candidate_media_fd(HANDLE_EINTR(open(path.c_str(), O_RDWR, 0)));
        ...
        if (HANDLE_EINTR(ioctl(candidate_media_fd.get(), MEDIA_IOC_DEVICE_INFO, &media_info)) < 0) {
        ...
        if (strlen(reinterpret_cast<const char*>(caps.bus_info)) > 0 && strlen(reinterpret_cast<const char*>(media_info.bus_info)) > 0
            && strncmp(reinterpret_cast<const char*>(caps.bus_info), reinterpret_cast<const char*>(media_info.bus_info), sizeof(caps.bus_info))) {
            continue;
        }
        ...
        if (strncmp(reinterpret_cast<const char*>(caps.driver), reinterpret_cast<const char*>(media_info.driver), sizeof(caps.driver))) {
            continue;
        }
    ...
    std::unique_ptr<V4L2RequestsQueue> requests_queue(new V4L2RequestsQueue(std::move(media_fd)));
...
bool V4L2Device::SetExtCtrls(uint32_t ctrl_class, std::vector<V4L2ExtCtrl> ctrls, V4L2RequestRef* request_ref)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(client_sequence_checker_);
    ...
    ext_ctrls.count = 0;
    const bool use_modern_s_ext_ctrls = Ioctl(VIDIOC_S_EXT_CTRLS, &ext_ctrls) == 0;

    ext_ctrls.which = use_modern_s_ext_ctrls ? V4L2_CTRL_WHICH_CUR_VAL : ctrl_class;
    ...
    if (result < 0) {
        ...
        if (ext_ctrls.error_idx == ext_ctrls.count)
            ...
            VPLOGF(1) << "VIDIOC_S_EXT_CTRLS: unable to set control (0x" << std::hex << ctrls[ext_ctrls.error_idx].ctrl.id << ") at index ("
                      << ext_ctrls.error_idx << ")  to 0x" << ctrls[ext_ctrls.error_idx].ctrl.value;
    }
...
{
    if (!SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_GOP_SIZE, gop_length) })) {
        ...
        if (gop_length == 0) {
            ...
            if (Ioctl(VIDIOC_QUERY_EXT_CTRL, &queryctrl) == 0) {
                VPLOGF(3) << "Unable to set GOP to 0, instead using max : " << queryctrl.maximum;
                return SetExtCtrls(V4L2_CTRL_CLASS_MPEG, { V4L2ExtCtrl(V4L2_CID_MPEG_VIDEO_GOP_SIZE, queryctrl.maximum) });
            }
        }
        return false;
    }
...
bool V4L2Device::OpenDevicePath(const std::string& path)
{
    ...
    device_fd_.reset(HANDLE_EINTR(open(path.c_str(), O_RDWR | O_NONBLOCK | O_CLOEXEC)));
...
void V4L2Device::EnumerateDevicesForType(Type type)
{
    ...
    for (int i = 0; i < kMaxDevices; ++i) {
        candidate_paths.push_back(base::StringPrintf("%s%d", device_pattern.c_str(), i));
    }
    ...
    for (const auto& path : candidate_paths) {
        ...
        const auto supported_pixelformats = EnumerateSupportedPixFmts(base::BindRepeating(&V4L2Device::Ioctl, this), buf_type);
