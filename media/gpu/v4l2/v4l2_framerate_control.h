...
public:
    ...
    V4L2FrameRateControl(const IoctlAsCallback& ioctl_cb, scoped_refptr<base::SequencedTaskRunner> task_runner);
    ~V4L2FrameRateControl();

    // Trampoline method for frame destructor callbacks to be directed to this
    // class' task runner.
    static void RecordFrameDurationThunk(base::WeakPtr<V4L2FrameRateControl> weak_this, scoped_refptr<base::SequencedTaskRunner> task_runner);
...
private:
    ...
    base::MovingAverage<base::TimeDelta, base::TimeDelta> frame_duration_moving_average_;
