...
public:
    ...
    // In case of error, false is returned and the buffer is returned to the free
    // list.
    [[nodiscard]] bool QueueUserPtr(const std::vector<void*>& ptrs, V4L2RequestRef* request_ref = nullptr) &&;
    ...
    // In case of error, false is returned and the buffer is returned to the free
    // list.
    [[nodiscard]] bool QueueDMABuf(const std::vector<base::ScopedFD>& fds, V4L2RequestRef* request_ref = nullptr) &&;
    ...
    // In case of error, false is returned and the buffer is returned to the free
    // list.
    [[nodiscard]] bool QueueDMABuf(const std::vector<gfx::NativePixmapPlane>& planes, V4L2RequestRef* request_ref = nullptr) &&;
    ...
    [[nodiscard]] bool QueueDMABuf(scoped_refptr<FrameResource> frame_resource, V4L2RequestRef* request_ref = nullptr) &&;
    ...
    [[nodiscard]] bool QueueDMABuf(uint64_t secure_handle, V4L2RequestRef* request_ref) &&;
...
private:
    ...
    [[nodiscard]] bool DoQueue(V4L2RequestRef* request_ref, scoped_refptr<FrameResource> frame_resource) &&;

    V4L2WritableBufferRef(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue);
...
class MEDIA_GPU_EXPORT V4L2ReadableBuffer : public base::RefCountedThreadSafe<V4L2ReadableBuffer> {
public:
...
private:
    ...
    V4L2ReadableBuffer(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<FrameResource> frame);
...
private:
    ...
    V4L2RequestRef(V4L2Request* request)
        : V4L2RequestRefBase(request)
    {
    }
};
...
private:
    ...
    V4L2SubmittedRequestRef(V4L2Request* request)
        : V4L2RequestRefBase(request)
    {
    }
};
...
//    described in 1).
class MEDIA_GPU_EXPORT V4L2Queue : public base::RefCountedThreadSafe<V4L2Queue> {
public:
    ...
    [[nodiscard]] std::optional<struct v4l2_format> SetFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size);
    ...
    [[nodiscard]] std::optional<struct v4l2_format> TryFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size);
    ...
    [[nodiscard]] size_t AllocateBuffers(size_t count, enum v4l2_memory memory, bool incoherent);
    ...
    // return |std::nullopt|.
    [[nodiscard]] std::optional<V4L2WritableBufferRef> GetFreeBuffer(size_t requested_buffer_id);
    ...
    [[nodiscard]] std::optional<V4L2WritableBufferRef> GetFreeBufferForFrame(const gfx::GenericSharedMemoryId& id);
    ...
    [[nodiscard]] std::optional<struct v4l2_format> SetModifierFormat(uint64_t modifier, const gfx::Size& size);
    ...
    // active.
    bool SetBufferFdForSecureHandle(uint64_t secure_handle, struct v4l2_buffer* v4l2_buffer);

private:
    ~V4L2Queue();

    // Called when clients request a buffer to be queued.
    [[nodiscard]] bool QueueBuffer(struct v4l2_buffer* v4l2_buffer, scoped_refptr<FrameResource> frame);
    ...
    base::small_map<std::map<size_t, scoped_refptr<FrameResource>>> queued_buffers_;
    ...
    std::map<gfx::GenericSharedMemoryId, size_t> free_buffers_indexes_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    const base::RepeatingClosure schedule_poll_cb_ GUARDED_BY_CONTEXT(sequence_checker_);
    const MmapAsCallback mmap_cb_ GUARDED_BY_CONTEXT(sequence_checker_);
    const AllocateSecureBufferAsCallback allocate_secure_cb_ GUARDED_BY_CONTEXT(sequence_checker_);

    // Callback to call in this queue's destructor.
    base::OnceClosure destroy_cb_;

    V4L2Queue(const IoctlAsCallback& ioctl_cb, const base::RepeatingClosure& schedule_poll_cb, const MmapAsCallback& mmap_cb,
