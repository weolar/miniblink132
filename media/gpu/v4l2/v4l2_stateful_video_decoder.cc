...
void* Mmap(int fd, void* addr, unsigned int len, int prot, int flags, unsigned int offset)
{
...
void WaitOnceForEvents(int device_fd, int wake_event, base::OnceClosure dequeue_callback, base::OnceClosure resolution_change_callback)
{
    ...
    struct pollfd pollfds[] = { { .fd = device_fd, .events = POLLIN | POLLPRI }, { .fd = wake_event, .events = POLLIN } };
    constexpr int kInfiniteTimeout = -1;
    if (HANDLE_EINTR(poll(pollfds, std::size(pollfds), kInfiniteTimeout)) < kIoctlOk) {
    ...
    CHECK((events_from_device & (POLLERR | POLLHUP | POLLNVAL)) || (other_events & (POLLERR | POLLHUP | POLLNVAL)));
    VLOG(2) << "Unhandled |events_from_device|: 0x" << std::hex << events_from_device << ", or |other_events|: 0x" << other_events;
}
...
bool IsNewH264Frame(
    const media::H264SPS* sps, const media::H264PPS* pps, const media::H264SliceHeader* prev_slice_header, const media::H264SliceHeader* curr_slice_header)
{
    if (curr_slice_header->frame_num != prev_slice_header->frame_num || curr_slice_header->pic_parameter_set_id != pps->pic_parameter_set_id
        || curr_slice_header->nal_ref_idc != prev_slice_header->nal_ref_idc || curr_slice_header->idr_pic_flag != prev_slice_header->idr_pic_flag
        || (curr_slice_header->idr_pic_flag && (curr_slice_header->idr_pic_id != prev_slice_header->idr_pic_id || curr_slice_header->first_mb_in_slice == 0))) {
        return true;
    }

    if (sps->pic_order_cnt_type == 0) {
        if (curr_slice_header->pic_order_cnt_lsb != prev_slice_header->pic_order_cnt_lsb
            || curr_slice_header->delta_pic_order_cnt_bottom != prev_slice_header->delta_pic_order_cnt_bottom) {
            return true;
        }
    } else if (sps->pic_order_cnt_type == 1) {
        if (curr_slice_header->delta_pic_order_cnt0 != prev_slice_header->delta_pic_order_cnt0
            || curr_slice_header->delta_pic_order_cnt1 != prev_slice_header->delta_pic_order_cnt1) {
            return true;
        }
    }
...
scoped_refptr<media::DecoderBuffer> ReassembleFragments(std::vector<scoped_refptr<media::DecoderBuffer>>& fragments)
{
    size_t frame_size = 0;
    ...
    auto reassembled_frame = media::DecoderBuffer::FromArray(std::move(temp_buffer));
...
public:
    ...
    // will returned.
    std::vector<std::pair<scoped_refptr<DecoderBuffer>, VideoDecoder::DecodeCB>> Process(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb);
...
private:
    ...
    std::optional<struct FrameBoundaryInfo> FindH264FrameBoundary(const uint8_t* const data, size_t size);
...
std::unique_ptr<VideoDecoderMixin> V4L2StatefulVideoDecoder::Create(
    std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
{
    DCHECK(task_runner->RunsTasksInCurrentSequence());
    DCHECK(client);

    return base::WrapUnique<VideoDecoderMixin>(new V4L2StatefulVideoDecoder(std::move(media_log), std::move(task_runner), std::move(client)));
}

void V4L2StatefulVideoDecoder::Initialize(const VideoDecoderConfig& config, bool /*low_delay*/, CdmContext* cdm_context, InitCB init_cb,
    const PipelineOutputCB& output_cb, const WaitingCB& /*waiting_cb*/)
{
    ...
    static const auto decoder_instances_limit = V4L2StatefulVideoDecoder::GetMaxNumDecoderInstances();
    const bool can_create_decoder = num_decoder_instances_.Increment() < decoder_instances_limit;
    ...
    if (supported_configs_.empty()) {
        supported_configs_ = GetSupportedV4L2DecoderConfigs().value_or(SupportedVideoDecoderConfigs());
    ...
    if (!device_fd_.is_valid()) {
        ...
        device_fd_.reset(HANDLE_EINTR(open(kVideoDeviceDriverPath, O_RDWR | O_NONBLOCK | O_CLOEXEC)));
        if (!device_fd_.is_valid()) {
        ...
        is_mtk8173_ = base::Contains(std::string(reinterpret_cast<const char*>(caps.card)), "8173");
    ...
    framerate_control_
        = std::make_unique<V4L2FrameRateControl>(base::BindRepeating(&HandledIoctl, device_fd_.get()), base::SequencedTaskRunner::GetCurrentDefault());
    ...
    OUTPUT_queue_ = base::WrapRefCounted(new V4L2Queue(base::BindRepeating(&HandledIoctl, device_fd_.get()),
        /*schedule_poll_cb=*/base::DoNothing(),
        /*mmap_cb=*/base::BindRepeating(&Mmap, device_fd_.get()), AllocateSecureBufferAsCallback(), V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE,
        /*destroy_cb=*/base::DoNothing()));

    const auto profile_as_v4l2_fourcc = VideoCodecProfileToV4L2PixFmt(config.profile(), /*slice_based=*/false);
    ...
    const size_t kInputBufferInMBs = (config.coded_size().GetArea() <= kFullHDNumPixels) ? 2 : 4;
    const auto v4l2_format = OUTPUT_queue_->SetFormat(profile_as_v4l2_fourcc, gfx::Size(), kInputBufferInMBs * kMiB);
    if (!v4l2_format) {
        std::move(init_cb).Run(DecoderStatus::Codes::kFailedToCreateDecoder);
        return;
    }
    ...
    const bool is_h264 = VideoCodecProfileToVideoCodec(config.profile()) == VideoCodec::kH264;
    constexpr size_t kNumInputBuffersH264 = 16;
    constexpr size_t kNumInputBuffersVPx = 2;
    const auto num_input_buffers = is_h264 ? kNumInputBuffersH264 : kNumInputBuffersVPx;
    ...
    client_->NotifyEstimatedMaxDecodeRequests(base::checked_cast<int>(std::min(static_cast<size_t>(4), num_input_buffers)));
    ...
    if (HandledIoctl(device_fd_.get(), VIDIOC_SUBSCRIBE_EVENT, &sub) != kIoctlOk) {
...
void V4L2StatefulVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    VLOGF(3) << buffer->AsHumanReadableString(/*verbose=*/false);
    if (!IsInitialized()) {
        DecoderStatus init_result;
        Initialize(config_, /*low_delay=*/false, /*cdm_context=*/nullptr, base::BindOnce([](DecoderStatus* out, DecoderStatus in) { *out = in; }, &init_result),
            output_cb_,
    ...
    if (buffer->end_of_stream()) {
        ...
        if (h264_frame_reassembler_ && h264_frame_reassembler_->HasFragments()) {
            decoder_buffer_and_callbacks_.emplace(h264_frame_reassembler_->AssembleAndFlushFragments(), base::DoNothing());
        ...
        if (is_pending_work || !decoding) {
            ...
            // later processing.
            decoder_buffer_and_callbacks_.emplace(std::move(buffer), std::move(decode_cb));
            return;
        }
    ...
    if (VideoCodecProfileToVideoCodec(config_.profile()) == VideoCodec::kH264) {
        auto processed_buffer_and_decode_cbs = h264_frame_reassembler_->Process(std::move(buffer), std::move(decode_cb));
    ...
    } else if (VideoCodecProfileToVideoCodec(config_.profile()) == VideoCodec::kHEVC) {
        NOTIMPLEMENTED();
    ...
    if (!event_task_runner_) {
        ...
        event_task_runner_ = base::ThreadPool::CreateSingleThreadTaskRunner(
            { base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::SingleThreadTaskRunnerThreadMode::DEDICATED);
...
void V4L2StatefulVideoDecoder::Reset(base::OnceClosure closure)
{
    ...
    absl::Cleanup scoped_trampoline_reset
        = [closure = std::move(closure)]() mutable { base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(closure)); };
    ...
    while (!decoder_buffer_and_callbacks_.empty()) {
        auto media_decode_cb = std::move(decoder_buffer_and_callbacks_.front().second);
...
V4L2StatefulVideoDecoder::V4L2StatefulVideoDecoder(
    std::unique_ptr<MediaLog> media_log, scoped_refptr<base::SequencedTaskRunner> task_runner, base::WeakPtr<VideoDecoderMixin::Client> client)
    : VideoDecoderMixin(std::move(media_log), std::move(task_runner), std::move(client))
...
V4L2StatefulVideoDecoder::~V4L2StatefulVideoDecoder()
{
    ...
    if (event_task_runner_) {
        ...
        event_task_runner_->PostTask(FROM_HERE, base::BindOnce([](base::ScopedFD fd) {}, std::move(device_fd_)));
        event_task_runner_->PostTask(FROM_HERE, base::BindOnce([](base::ScopedFD fd) {}, std::move(wake_event_)));
    }
}
...
bool V4L2StatefulVideoDecoder::InitializeCAPTUREQueue()
{
    ...
    CAPTURE_queue_ = base::WrapRefCounted(new V4L2Queue(base::BindRepeating(&HandledIoctl, device_fd_.get()),
        /*schedule_poll_cb=*/base::DoNothing(),
        /*mmap_cb=*/base::BindRepeating(&Mmap, device_fd_.get()), AllocateSecureBufferAsCallback(), V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE,
        /*destroy_cb=*/base::DoNothing()));
    ...
    VLOG(3) << "Out-of-the-box |CAPTURE_queue_| configuration: " << V4L2FormatToString(v4l2_format);

    const gfx::Size coded_size(v4l2_format.fmt.pix_mp.width, v4l2_format.fmt.pix_mp.height);
    std::vector<ImageProcessor::PixelLayoutCandidate> candidates = EnumeratePixelLayoutCandidates(coded_size);
    ...
    CroStatus::Or<ImageProcessor::PixelLayoutCandidate> status_or_output_format
        = client_->PickDecoderOutputFormat(candidates, *visible_rect, config_.aspect_ratio().GetNaturalSize(*visible_rect),
    ...
    const ImageProcessor::PixelLayoutCandidate output_format = std::move(status_or_output_format).value();
    ...
    const v4l2_memory buffer_type = use_v4l2_allocated_buffers ? V4L2_MEMORY_MMAP : V4L2_MEMORY_DMABUF;
    ...
    const size_t v4l2_num_buffers = use_v4l2_allocated_buffers ? num_codec_reference_frames + 2 : VIDEO_MAX_FRAME;

    if (!use_v4l2_allocated_buffers) {
        std::optional<GpuBufferLayout> layout = client_->GetVideoFramePool()->GetGpuBufferLayout();
        if (!layout.has_value()) {
            return false;
        }
        if (layout->modifier() == DRM_FORMAT_MOD_QCOM_COMPRESSED) {
            ...
            if (!CAPTURE_queue_->SetFormat(V4L2_PIX_FMT_QC08C, chosen_size, /*buffer_size=*/0).has_value()) {
                return false;
            }
            chosen_fourcc = Fourcc::FromV4L2PixFmt(V4L2_PIX_FMT_QC08C).value();
        }
    }
    VLOG(2) << "Chosen |CAPTURE_queue_| format: " << chosen_fourcc.ToString() << " " << chosen_size.ToString() << " (modifier: 0x" << std::hex
            << chosen_modifier << std::dec << "). Using " << v4l2_num_buffers << " |CAPTURE_queue_| slots.";

    const auto allocated_buffers = CAPTURE_queue_->AllocateBuffers(v4l2_num_buffers, buffer_type, /*incoherent=*/false);
    if (allocated_buffers < v4l2_num_buffers) {
        LOGF(ERROR) << "Failed to allocate enough CAPTURE buffers, requested= " << v4l2_num_buffers << " actual= " << allocated_buffers;
        return false;
    }
...
std::vector<ImageProcessor::PixelLayoutCandidate> V4L2StatefulVideoDecoder::EnumeratePixelLayoutCandidates(const gfx::Size& coded_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(CAPTURE_queue_) << "|CAPTURE_queue_| must be created at this point";

    const auto v4l2_pix_fmts = EnumerateSupportedPixFmts(base::BindRepeating(&HandledIoctl, device_fd_.get()), V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    ...
    for (const uint32_t& pixfmt : v4l2_pix_fmts) {
        ...
        candidates.emplace_back(ImageProcessor::PixelLayoutCandidate { .fourcc = *candidate_fourcc, .size = coded_size });
        VLOG(2) << "CAPTURE queue candidate format: " << candidate_fourcc->ToString() << ", " << coded_size.ToString();
    }
    return candidates;
}
...
size_t V4L2StatefulVideoDecoder::GetNumberOfReferenceFrames()
{
    ...
    size_t num_codec_reference_frames = is_mtk8173_ ? kDefaultNumReferenceFramesMTK8173 : kDefaultNumReferenceFrames;

    struct v4l2_ext_control ctrl = { .id = V4L2_CID_MIN_BUFFERS_FOR_CAPTURE };
    struct v4l2_ext_controls ext_ctrls = { .count = 1, .controls = &ctrl };
    if (HandledIoctl(device_fd_.get(), VIDIOC_G_EXT_CTRLS, &ext_ctrls) == kIoctlOk) {
        num_codec_reference_frames = std::max(base::checked_cast<size_t>(ctrl.value), num_codec_reference_frames);
    }
    VLOG(2) << "Driver wants: " << ctrl.value << " CAPTURE buffers. We'll use: " << num_codec_reference_frames;
...
void V4L2StatefulVideoDecoder::RearmCAPTUREQueueMonitoring()
{
    ...
    auto dequeue_callback = base::BindPostTaskToCurrentDefault(
        base::BindOnce(&V4L2StatefulVideoDecoder::TryAndDequeueCAPTUREQueueBuffers, weak_ptr_factory_for_events_.GetWeakPtr()));
    ...
    // by a Reset()).
    auto resolution_change_callback = base::BindPostTaskToCurrentDefault(base::BindOnce(
        [](base::WeakPtr<VideoDecoderMixin::Client> client, base::WeakPtr<V4L2StatefulVideoDecoder> weak_this) {
            if (weak_this && client) {
    ...
    //   behavior.
    cancelable_task_tracker_.PostTask(event_task_runner_.get(), FROM_HERE,
        base::BindOnce(&WaitOnceForEvents, device_fd_.get(), wake_event_.get(), std::move(dequeue_callback), std::move(resolution_change_callback)));
}
...
void V4L2StatefulVideoDecoder::TryAndDequeueCAPTUREQueueBuffers()
{
    ...
    const bool use_v4l2_allocated_buffers = !client_->GetVideoFramePool();
    DCHECK((queue_type == V4L2_MEMORY_MMAP && use_v4l2_allocated_buffers) || (queue_type == V4L2_MEMORY_DMABUF && !use_v4l2_allocated_buffers));

    bool success;
    scoped_refptr<V4L2ReadableBuffer> dequeued_buffer;
    for (std::tie(success, dequeued_buffer) = CAPTURE_queue_->DequeueBuffer(); success && dequeued_buffer;
         std::tie(success, dequeued_buffer) = CAPTURE_queue_->DequeueBuffer()) {
        PrintAndTraceQueueStates(FROM_HERE);

        const int64_t flat_timespec = TimeValToTimeDelta(dequeued_buffer->GetTimeStamp()).InMilliseconds();
        if (base::Contains(encoding_timestamps_, flat_timespec)) {
            UMA_HISTOGRAM_TIMES("Media.PlatformVideoDecoding.Decode", base::TimeTicks::Now() - encoding_timestamps_[flat_timespec]);
        ...
        if (dequeued_buffer->IsLast()) {
            ...
            // [1] https://www.kernel.org/doc/html/v5.15/userspace-api/media/v4l/dev-decoder.html#drain
            const bool has_pending_OUTPUT_queue_work = OUTPUT_queue_->QueuedBuffersCount();
            ...
            if (queue_type == V4L2_MEMORY_MMAP) {
                ...
                auto wrapped_frame = frame->CreateWrappingFrame(visible_rect_,
                ...
                wrapped_frame->AddDestructionObserver(base::BindPostTaskToCurrentDefault(base::BindOnce(
                    [](scoped_refptr<V4L2ReadableBuffer> buffer, base::WeakPtr<V4L2StatefulVideoDecoder> weak_this) {
                    ...
                    std::move(dequeued_buffer), weak_ptr_factory_for_CAPTURE_availability_.GetWeakPtr())));
                CHECK(wrapped_frame);
    ...
    if (queue_type == V4L2_MEMORY_MMAP) {
        ...
        while (true) {
            ...
            if (client_->GetVideoFramePool()->IsExhausted()) {
                ...
                client_->GetVideoFramePool()->NotifyWhenFrameAvailable(
                    base::BindOnce(base::IgnoreResult(&base::SequencedTaskRunner::PostTask), base::SequencedTaskRunner::GetCurrentDefault(), FROM_HERE,
                        base::BindOnce(&V4L2StatefulVideoDecoder::TryAndEnqueueCAPTUREQueueBuffers, weak_ptr_factory_for_CAPTURE_availability_.GetWeakPtr())));
                return;
            }
...
bool V4L2StatefulVideoDecoder::DrainOUTPUTQueue()
{
    ...
    bool success;
    scoped_refptr<V4L2ReadableBuffer> dequeued_buffer;
    for (std::tie(success, dequeued_buffer) = OUTPUT_queue_->DequeueBuffer(); success && dequeued_buffer;
...
bool V4L2StatefulVideoDecoder::TryAndEnqueueOUTPUTQueueBuffers()
{
    ...
    for (std::optional<V4L2WritableBufferRef> v4l2_buffer = OUTPUT_queue_->GetFreeBuffer(); v4l2_buffer && !decoder_buffer_and_callbacks_.empty();
        ...
        if (media_buffer) {
            ...
            if (!std::move(*v4l2_buffer).QueueMMap()) {
                ...
                std::move(media_decode_cb).Run(DecoderStatus::Codes::kPlatformDecodeFailure);
                return false;
            }
        }
...
void V4L2StatefulVideoDecoder::PrintAndTraceQueueStates(const base::Location& from_here)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(IsInitialized()) << "V4L2StatefulVideoDecoder must be Initialize()d";
    VLOG(4) << from_here.function_name() << "(): |OUTPUT_queue_| " << OUTPUT_queue_->QueuedBuffersCount() << "/" << OUTPUT_queue_->AllocatedBuffersCount()
            << ", |CAPTURE_queue_| " << (CAPTURE_queue_ ? CAPTURE_queue_->QueuedBuffersCount() : 0) << "/"
            << (CAPTURE_queue_ ? CAPTURE_queue_->AllocatedBuffersCount() : 0);

    TRACE_COUNTER_ID1("media,gpu", "V4L2 OUTPUT Q used buffers", this, base::checked_cast<int32_t>(OUTPUT_queue_->QueuedBuffersCount()));
    TRACE_COUNTER_ID1(
        "media,gpu", "V4L2 CAPTURE Q free buffers", this, (CAPTURE_queue_ ? base::checked_cast<int32_t>(CAPTURE_queue_->QueuedBuffersCount()) : 0));
}
...
int V4L2StatefulVideoDecoder::GetMaxNumDecoderInstances()
{
    ...
    constexpr char kVideoDeviceDriverPath[] = "/dev/video-dec0";
    base::ScopedFD device_fd(HANDLE_EINTR(open(kVideoDeviceDriverPath, O_RDWR | O_NONBLOCK | O_CLOEXEC)));
    ...
    const bool is_mtk8173 = base::Contains(std::string(reinterpret_cast<const char*>(caps.card)), "8173");
...
std::vector<std::pair<scoped_refptr<DecoderBuffer>, VideoDecoder::DecodeCB>> H264FrameReassembler::Process(
    scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb)
{
    std::vector<std::pair<scoped_refptr<DecoderBuffer>, VideoDecoder::DecodeCB>> frames;

    auto remaining = base::span(*buffer);

    do {
        const auto nalu_info = FindH264FrameBoundary(remaining.data(), remaining.size());
        if (!nalu_info.has_value()) {
            LOG(ERROR) << "Failed parsing H.264 DecoderBuffer";
            std::move(decode_cb).Run(DecoderStatus::Codes::kFailed);
            return {};
        }
        const size_t found_nalu_size = base::checked_cast<size_t>(nalu_info->nalu_size);

        if (nalu_info->is_start_of_new_frame && HasFragments()) {
            VLOGF(4) << frame_fragments_.size() << " currently stored frame fragment(s) can be reassembled.";
            frames.emplace_back(ReassembleFragments(frame_fragments_), base::DoNothing());
        }

        if (nalu_info->is_whole_frame) {
            VLOGF(3) << "Found a whole frame, size=" << found_nalu_size << " bytes";
            frames.emplace_back(DecoderBuffer::CopyFrom(remaining.first(found_nalu_size)), base::DoNothing());
        ...
        frame_fragments_.emplace_back(DecoderBuffer::CopyFrom(remaining.first(found_nalu_size)));
...
std::optional<struct H264FrameReassembler::FrameBoundaryInfo> H264FrameReassembler::FindH264FrameBoundary(const uint8_t* const data, size_t data_size)
{
    ...
    while (true) {
        ...
        if (result == H264Parser::kEOStream) {
            ...
            return FrameBoundaryInfo { .is_whole_frame = true, .is_start_of_new_frame = true, .nalu_size = nalu.size };
        }
        ...
        VLOGF(4) << "H264NALU type " << kKnownNALUNames[nalu.nal_unit_type] << ", NALU size=" << nalu_size << " bytes, payload size=" << nalu.size << " bytes";
        ...
        case H264NALU::kSPS:
            ...
            return FrameBoundaryInfo { .is_whole_frame = true, .is_start_of_new_frame = true, .nalu_size = nalu_size };
        case H264NALU::kPPS:
        ...
        case H264NALU::kIDRSlice: {
            ...
            if (result != H264Parser::kOk) {
                ...
                return FrameBoundaryInfo { .is_whole_frame = true, .is_start_of_new_frame = false, .nalu_size = nalu_size };
            }
            const bool is_new_frame = previous_slice_header_
                && IsNewH264Frame(h264_parser_.GetSPS(sps_id_), h264_parser_.GetPPS(pps_id_), previous_slice_header_.get(), &curr_slice_header);
            previous_slice_header_ = std::make_unique<H264SliceHeader>(curr_slice_header);
            return FrameBoundaryInfo { .is_whole_frame = false, .is_start_of_new_frame = is_new_frame, .nalu_size = nalu_size };
        }
