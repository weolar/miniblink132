...
bool IsVp9KSVCStream(uint32_t input_format_fourcc, const DecoderBuffer& decoder_buffer)
{
    return input_format_fourcc == V4L2_PIX_FMT_VP9 && decoder_buffer.side_data_size() > 0;
}
...
struct V4L2VideoDecodeAccelerator::BitstreamBufferRef {
    BitstreamBufferRef(
        base::WeakPtr<Client>& client, scoped_refptr<base::SingleThreadTaskRunner>& client_task_runner, scoped_refptr<DecoderBuffer> buffer, int32_t input_id);
    ~BitstreamBufferRef();
...
{
    if (input_id >= 0) {
        client_task_runner->PostTask(FROM_HERE, base::BindOnce(&Client::NotifyEndOfBitstreamBuffer, client, input_id));
    }
}
...
V4L2VideoDecodeAccelerator::OutputRecord::OutputRecord(OutputRecord&&) = default;

V4L2VideoDecodeAccelerator::OutputRecord::~OutputRecord()
{
}

V4L2VideoDecodeAccelerator::PictureRecord::PictureRecord(bool cleared, const Picture& picture)
    : cleared(cleared)
    , picture(picture)
{
}
...
V4L2VideoDecodeAccelerator::V4L2VideoDecodeAccelerator(EGLDisplay egl_display, const GetGLContextCallback& get_gl_context_cb,
    const MakeGLContextCurrentCallback& make_context_current_cb, scoped_refptr<V4L2Device> device)
...
bool V4L2VideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    VLOGF(2) << "profile: " << config.profile << ", output_mode=" << static_cast<int>(config.output_mode);
...
// TODO(posciak): https://crbug.com/450898.
    ...
    decoder_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::InitializeTask, base::Unretained(this), config, &result, &done));
    done.Wait();

    return result;
}

void V4L2VideoDecodeAccelerator::InitializeTask(const Config& config, bool* result, base::WaitableEvent* done)
{
    DVLOGF(3);
    ...
    *result = config_result;
    ...
    frame_splitter_ = v4l2_vda_helpers::InputBufferFragmentSplitter::CreateFromProfile(config.profile);
    if (!frame_splitter_) {
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    }

    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "media::V4l2VideoDecodeAccelerator", decoder_thread_.task_runner());
...
bool V4L2VideoDecodeAccelerator::CheckConfig(const Config& config)
{
    ...
    input_format_fourcc_ = V4L2Device::VideoCodecProfileToV4L2PixFmt(config.profile, false);
    ...
    workarounds_ = CreateV4L2StatefulWorkarounds(V4L2Device::Type::kDecoder, config.profile);
...
void V4L2VideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    decoder_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DecodeTask, base::Unretained(this), std::move(buffer), bitstream_id));
}

void V4L2VideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    VLOGF(2) << "buffer_count=" << buffers.size();
    DCHECK(child_task_runner_->BelongsToCurrentThread());

    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::AssignPictureBuffersTask, base::Unretained(this), buffers));
}

void V4L2VideoDecodeAccelerator::AssignPictureBuffersTask(const std::vector<PictureBuffer>& buffers)
{
    VLOGF(2);
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    DCHECK_EQ(decoder_state_, kAwaitingPictureBuffers);
    DCHECK(output_queue_);
    TRACE_EVENT1("media,gpu", "V4L2VDA::AssignPictureBuffersTask", "buffers_size", buffers.size());
    ...
    if (buffers.size() < req_buffer_count) {
        LOG(ERROR) << "Failed to provide requested picture buffers. (Got " << buffers.size() << ", requested " << req_buffer_count << ")";
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    }
...
void V4L2VideoDecodeAccelerator::CreateEGLImageFor(scoped_refptr<V4L2Device> egl_device, size_t buffer_index, int32_t picture_buffer_id,
    gfx::NativePixmapHandle handle, GLuint texture_id, const gfx::Size& visible_size, const Fourcc fourcc)
{
    ...
    EGLImageKHR egl_image
        = egl_device->CreateEGLImage(egl_display_, gl_context->GetHandle(), texture_id, visible_size, buffer_index, fourcc, std::move(handle));
    ...
    decoder_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::AssignEGLImage, base::Unretained(this), buffer_index, picture_buffer_id, egl_image));
}

void V4L2VideoDecodeAccelerator::AssignEGLImage(size_t buffer_index, int32_t picture_buffer_id, EGLImageKHR egl_image)
{
    ...
    if (buffer_index >= output_buffer_map_.size() || output_buffer_map_[buffer_index].picture_id != picture_buffer_id) {
        ...
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&V4L2Device::DestroyEGLImage), device_, egl_display_, egl_image));
        return;
    }
    ...
    int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
    ...
    decoder_thread_.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&V4L2VideoDecodeAccelerator::ImportBufferForPictureForImportTask, base::Unretained(this), picture_buffer_id, pixel_format,
            std::move(gpu_memory_buffer_handle.native_pixmap_handle)));
}

void V4L2VideoDecodeAccelerator::ImportBufferForPictureForImportTask(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::NativePixmapHandle handle)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    if (pixel_format != egl_image_format_fourcc_->ToVideoPixelFormat()) {
        LOG(ERROR) << "Unsupported import format: " << pixel_format << ", expected "
                   << VideoPixelFormatToString(egl_image_format_fourcc_->ToVideoPixelFormat());
        NOTIFY_ERROR(INVALID_ARGUMENT);
        return;
    }
...
void V4L2VideoDecodeAccelerator::ImportBufferForPictureTask(int32_t picture_buffer_id, gfx::NativePixmapHandle handle)
{
    DVLOGF(3) << "picture_buffer_id=" << picture_buffer_id << ", handle.planes.size()=" << handle.planes.size();
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    TRACE_EVENT2("media,gpu", "V4L2VDA::ImportBufferForPictureTask", "picture_buffer_id", picture_buffer_id, "handle.planes", handle.planes.size());
    ...
    if (output_mode_ == Config::OutputMode::IMPORT) {
        ...
        const gfx::Size handle_size = v4l2_vda_helpers::NativePixmapSizeFromHandle(handle, *egl_image_format_fourcc_, egl_image_size_);
        ...
        if (decoder_state_ == kAwaitingPictureBuffers) {
            ...
            DVLOGF(3) << "Original egl_image_size=" << egl_image_size_.ToString() << ", adjusted buffer size=" << handle_size.ToString();
    ...
    if (output_mode_ == Config::OutputMode::IMPORT) {
        ...
        auto layout = VideoFrameLayout::CreateWithPlanes(egl_image_format_fourcc_->ToVideoPixelFormat(), egl_image_size_, std::move(color_planes));
        if (!layout) {
        ...
        iter->output_frame = VideoFrame::WrapExternalDmabufs(*layout, gfx::Rect(visible_size_), visible_size_, DuplicateFDs(duped_fds), base::TimeDelta());
    }

    if (iter->texture_id != 0) {
        if (iter->egl_image != EGL_NO_IMAGE_KHR) {
            child_task_runner_->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&V4L2Device::DestroyEGLImage), device_, egl_display_, iter->egl_image));
        }
        ...
        if (!image_processor_) {
            ...
            child_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&V4L2VideoDecodeAccelerator::CreateEGLImageFor, weak_this_, device_, index, picture_buffer_id, std::move(handle),
                    iter->texture_id, visible_size_, *egl_image_format_fourcc_));
...
// TODO(posciak): https://crbug.com/450898.
    ...
    decoder_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::ReusePictureBufferTask, base::Unretained(this), picture_buffer_id, std::move(egl_fence)));
}
...
void V4L2VideoDecodeAccelerator::Flush()
{
    VLOGF(2);
    DCHECK(child_task_runner_->BelongsToCurrentThread());
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::FlushTask, base::Unretained(this)));
}
...
void V4L2VideoDecodeAccelerator::Reset()
{
    VLOGF(2);
    DCHECK(child_task_runner_->BelongsToCurrentThread());
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::ResetTask, base::Unretained(this)));
}
...
void V4L2VideoDecodeAccelerator::Destroy()
{
    ...
    if (decoder_thread_.IsRunning()) {
        decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DestroyTask, base::Unretained(this)));
...
// static
VideoDecodeAccelerator::SupportedProfiles V4L2VideoDecodeAccelerator::GetSupportedProfiles()
{
...
void V4L2VideoDecodeAccelerator::DecodeTask(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
...
void V4L2VideoDecodeAccelerator::DecodeBufferTask()
{
    ...
    if (decoder_current_bitstream_buffer_ == NULL) {
        ...
        if (decoder_delay_bitstream_buffer_id_ == decoder_input_queue_.front()->input_id) {
        ...
        if (buffer) {
            DVLOGF(4) << "reading input_id=" << decoder_current_bitstream_buffer_->input_id << ", addr=" << buffer->data() << ", size=" << buffer->data_size();
        } else {
    ...
    if (!buffer) {
        ...
        schedule_task = true;
        if (current_input_buffer_ && current_input_buffer_->GetTimeStamp().tv_sec != kFlushBufferId)
        ...
        const uint8_t* const data = buffer->data() + decoder_current_bitstream_buffer_->bytes_used;
        const size_t data_size = buffer->data_size() - decoder_current_bitstream_buffer_->bytes_used;
        ...
        if (!frame_splitter_->AdvanceFrameFragment(data, data_size, &decoded_size)) {
    ...
    if (schedule_task) {
        ...
        if ((buffer ? buffer->data_size() : 0) == decoder_current_bitstream_buffer_->bytes_used) {
...
void V4L2VideoDecodeAccelerator::ScheduleDecodeBufferTaskIfNeeded()
{
    ...
    if (decoder_decode_buffer_tasks_scheduled_ < buffers_to_decode) {
        decoder_decode_buffer_tasks_scheduled_++;
        decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DecodeBufferTask, base::Unretained(this)));
    }
}

bool V4L2VideoDecodeAccelerator::DecodeBufferInitial(const void* data, size_t size, size_t* endpos)
{
...
bool V4L2VideoDecodeAccelerator::DecodeBufferContinue(const void* data, size_t size)
{
    ...
    return (AppendToInputFrame(data, size) && (frame_splitter_->IsPartialFramePending() || FlushInputFrame()));
}

bool V4L2VideoDecodeAccelerator::AppendToInputFrame(const void* data, size_t size)
{
    DVLOGF(4);
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    DCHECK_NE(decoder_state_, kUninitialized);
    ...
    if (!current_input_buffer_) {
        ...
        struct timeval timestamp = { .tv_sec = decoder_current_bitstream_buffer_->input_id };
...
bool V4L2VideoDecodeAccelerator::FlushInputFrame()
{
    ...
    DCHECK(input_buffer_id != kFlushBufferId || current_input_buffer_->GetPlaneBytesUsed(0) == 0);
    ...
    TRACE_COUNTER_ID1("media,gpu", "V4L2VDA input ready buffers", this, input_ready_queue_.size());
...
void V4L2VideoDecodeAccelerator::ServiceDeviceTask(bool event_pending)
{
    ...
    if (input_queue_->QueuedBuffersCount() + output_queue_->QueuedBuffersCount() > 0)
        poll_device = true;
    ...
    device_poll_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DevicePollTask, base::Unretained(this), poll_device));

    DVLOGF(3) << "ServiceDeviceTask(): buffer counts: DEC[" << decoder_input_queue_.size() << "->" << input_ready_queue_.size() << "] => DEVICE["
              << input_queue_->FreeBuffersCount() << "+" << input_queue_->QueuedBuffersCount() << "/" << input_queue_->AllocatedBuffersCount() << "->"
              << output_queue_->FreeBuffersCount() << "+" << output_queue_->QueuedBuffersCount() << "/" << output_buffer_map_.size() << "] => PROCESSOR["
              << buffers_at_ip_.size() << "] => CLIENT[" << buffers_at_client_.size() << "]";
        ...
        if (buffers_awaiting_fence_.front().first->HasCompleted()) {
            ...
            if (output_queue_->FreeBuffersCount() == 0) {
                ...
                decoder_thread_.task_runner()->PostDelayedTask(
                    FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::Enqueue, base::Unretained(this)), base::Milliseconds(resched_delay));
            }
            break;
        }
    }
...
bool V4L2VideoDecodeAccelerator::DequeueResolutionChangeEvent()
{
    ...
    while (absl::optional<struct v4l2_event> event = device_->DequeueEvent()) {
        if (event->type == V4L2_EVENT_SOURCE_CHANGE) {
            ...
            VLOGF(1) << "got an event (" << event->type << ") we haven't subscribed to.";
        }
    }
    return false;
}
...
bool V4L2VideoDecodeAccelerator::DequeueOutputBuffer()
{
    ...
    if (buf->GetPlaneBytesUsed(0) > 0) {
        ...
        DVLOGF(4) << "Dequeue output buffer: dqbuf index=" << buf->BufferId() << " bitstream input_id=" << bitstream_buffer_id;
        if (image_processor_device_) {
    ...
    if (buf->IsLast()) {
        DVLOGF(3) << "Got last output buffer. Waiting last buffer=" << flush_awaiting_last_output_buffer_;
...
bool V4L2VideoDecodeAccelerator::EnqueueInputRecord(V4L2WritableBufferRef buffer)
{
    DVLOGF(4);
...
bool V4L2VideoDecodeAccelerator::EnqueueOutputRecord(V4L2WritableBufferRef buffer)
{
...
void V4L2VideoDecodeAccelerator::ReusePictureBufferTask(int32_t picture_buffer_id, std::unique_ptr<gl::GLFenceEGL> egl_fence)
{
    DVLOGF(4) << "picture_buffer_id=" << picture_buffer_id;
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    // the fence signals.
    if (egl_fence)
        buffers_awaiting_fence_.emplace(std::make_pair(std::move(egl_fence), std::move(iter->second)));
    ...
    TRACE_COUNTER_ID2(
        "media,gpu", "V4L2 output buffers", this, "in client", buffers_at_client_.size(), "in vda", output_buffer_map_.size() - buffers_at_client_.size());
    TRACE_COUNTER_ID2("media,gpu", "V4L2 output buffers in vda", this, "free", output_queue_->FreeBuffersCount(), "in device or IP",
...
void V4L2VideoDecodeAccelerator::FlushTask()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "V4L2VDA::FlushTask", TRACE_ID_LOCAL(this));
...
{
    TRACE_EVENT_NESTABLE_ASYNC_END0("media,gpu", "V4L2VDA::FlushTask", TRACE_ID_LOCAL(this));
    decoder_delay_bitstream_buffer_id_ = -1;
    decoder_flushing_ = false;
...
void V4L2VideoDecodeAccelerator::ResetTask()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "V4L2VDA::ResetTask", TRACE_ID_LOCAL(this));
...
void V4L2VideoDecodeAccelerator::FinishReset()
{
    ...
    SendPictureReady(); // Send all pending PictureReady.
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::ResetDoneTask, base::Unretained(this)));
}
...
void V4L2VideoDecodeAccelerator::ResetDoneTask()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("media,gpu", "V4L2VDA::ResetTask", TRACE_ID_LOCAL(this));
...
bool V4L2VideoDecodeAccelerator::StartDevicePoll()
{
    ...
    cancelable_service_device_task_.Reset(base::BindRepeating(&V4L2VideoDecodeAccelerator::ServiceDeviceTask, base::Unretained(this)));
    cancelable_service_device_task_callback_ = cancelable_service_device_task_.callback();
    device_poll_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::DevicePollTask, base::Unretained(this), 0));
...
void V4L2VideoDecodeAccelerator::DevicePollTask(bool poll_device)
{
    ...
    decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(cancelable_service_device_task_callback_, event_pending));
}
...
void V4L2VideoDecodeAccelerator::NotifyError(Error error)
{
    ...
    if (!child_task_runner_->BelongsToCurrentThread()) {
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::NotifyError, weak_this_, error));
        return;
    }
...
void V4L2VideoDecodeAccelerator::SetErrorState(Error error)
{
    ...
    if (decoder_thread_.task_runner() && !decoder_thread_.task_runner()->BelongsToCurrentThread()) {
        decoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&V4L2VideoDecodeAccelerator::SetErrorState, base::Unretained(this), error));
        return;
    }
    ...
    if (decoder_state_ != kError && decoder_state_ != kUninitialized && decoder_state_ != kDestroying)
        NotifyError(error);

    decoder_state_ = kError;
}

bool V4L2VideoDecodeAccelerator::GetFormatInfo(struct v4l2_format* format, gfx::Size* visible_size, bool* again)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
...
bool V4L2VideoDecodeAccelerator::CreateBuffersForFormat(const struct v4l2_format& format, const gfx::Size& visible_size)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    if (image_processor_device_) {
        ...
        if (!V4L2ImageProcessorBackend::TryOutputFormat(
                output_format_fourcc_->ToV4L2PixFmt(), egl_image_format_fourcc_->ToV4L2PixFmt(), coded_size_, &output_size, &egl_image_planes_count)) {
    ...
    VLOGF(2) << "new resolution: " << coded_size_.ToString() << ", visible size: " << visible_size_.ToString()
             << ", decoder output planes count: " << format.fmt.pix_mp.num_planes << ", EGLImage size: " << egl_image_size_.ToString()
             << ", EGLImage plane count: " << egl_image_planes_count;

    return CreateOutputBuffers();
}

gfx::Size V4L2VideoDecodeAccelerator::GetVisibleSize(const gfx::Size& coded_size)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    if (!gfx::Rect(coded_size).Contains(rect)) {
        DVLOGF(3) << "visible rectangle " << rect.ToString() << " is not inside coded size " << coded_size.ToString();
        return coded_size;
    }
    if (rect.IsEmpty()) {
    ...
    if (!rect.origin().IsOrigin()) {
        VLOGF(1) << "Unexpected visible rectangle " << rect.ToString() << ", top-left is not origin";
        return coded_size;
    }
...
bool V4L2VideoDecodeAccelerator::SetupFormats()
{
    ...
    if (!is_format_supported) {
        VLOGF(1) << "Input fourcc " << input_format_fourcc_ << " not supported by device.";
        return false;
    }
    ...
    if (!output_format_fourcc_) {
        ...
        egl_image_format_fourcc_ = v4l2_vda_helpers::FindImageProcessorOutputFormat(device_.get());
...
bool V4L2VideoDecodeAccelerator::CreateImageProcessor()
{
    ...
    image_processor_ = v4l2_vda_helpers::CreateImageProcessor(*output_format_fourcc_, *egl_image_format_fourcc_, coded_size_, coded_size_,
        gfx::Rect(visible_size_), VideoFrame::StorageType::STORAGE_DMABUFS, output_buffer_map_.size(), image_processor_device_, image_processor_output_mode,
        ...
        // ErrorCB is executed.
        base::BindRepeating(&V4L2VideoDecodeAccelerator::ImageProcessorError, base::Unretained(this)));
...
bool V4L2VideoDecodeAccelerator::ProcessFrame(int32_t bitstream_buffer_id, V4L2ReadableBufferRef buf)
{
    DVLOGF(4);
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    ...
    if (image_processor_->output_mode() == ImageProcessor::OutputMode::IMPORT) {
        image_processor_->Process(cropped_input_frame, output_record.output_frame,
            base::BindOnce(&V4L2VideoDecodeAccelerator::FrameProcessed, base::Unretained(this), bitstream_buffer_id, buf->BufferId()));
    } else {
        image_processor_->Process(
            cropped_input_frame, base::BindOnce(&V4L2VideoDecodeAccelerator::FrameProcessed, base::Unretained(this), bitstream_buffer_id));
    }
    return true;
}
...
bool V4L2VideoDecodeAccelerator::CreateOutputBuffers()
{
    ...
    DVLOGF(3) << "buffer_count=" << buffer_count << ", coded_size=" << coded_size_.ToString();

    // With ALLOCATE mode the client can sample it as RGB and doesn't need to
    // know the precise format.
    VideoPixelFormat pixel_format = (output_mode_ == Config::OutputMode::IMPORT) ? egl_image_format_fourcc_->ToVideoPixelFormat() : PIXEL_FORMAT_UNKNOWN;

    child_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&Client::ProvidePictureBuffersWithVisibleRect, client_, buffer_count, pixel_format, 1, egl_image_size_, gfx::Rect(visible_size_),
    ...
    for (size_t i = 0; i < output_buffer_map_.size(); ++i) {
        ...
        if (output_record.egl_image != EGL_NO_IMAGE_KHR) {
            child_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(base::IgnoreResult(&V4L2Device::DestroyEGLImage), device_, egl_display_, output_record.egl_image));
        }

        DVLOGF(3) << "dismissing PictureBuffer id=" << output_record.picture_id;
        child_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Client::DismissPictureBuffer, client_, output_record.picture_id));
    }
...
void V4L2VideoDecodeAccelerator::SendBufferToClient(
    size_t output_buffer_index, int32_t bitstream_buffer_id, V4L2ReadableBufferRef vda_buffer, scoped_refptr<VideoFrame> frame)
{
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    DCHECK_GE(bitstream_buffer_id, 0);
    ...
    buffers_at_client_.emplace(output_record.picture_id, std::make_pair(std::move(vda_buffer), std::move(frame)));
    // TODO(b/214190092): Get color space from the v4l2 buffer.
    const Picture picture(output_record.picture_id, bitstream_buffer_id, gfx::Rect(visible_size_), container_color_space_.ToGfxColorSpace(), false);
...
void V4L2VideoDecodeAccelerator::SendPictureReady()
{
    ...
    while (pending_picture_ready_.size() > 0) {
        ...
        if (cleared && picture_clearing_count_ == 0) {
            ...
            DVLOGF(4) << "cleared=" << pending_picture_ready_.front().cleared << ", decoder_state_=" << decoder_state_
                      << ", decoder_flushing_=" << decoder_flushing_ << ", picture_clearing_count_=" << picture_clearing_count_;
            ...
            child_task_runner_->PostTaskAndReply(FROM_HERE, base::BindOnce(&Client::PictureReady, client_, picture),
                ...
                base::BindOnce(&V4L2VideoDecodeAccelerator::PictureCleared, base::Unretained(this)));
...
void V4L2VideoDecodeAccelerator::FrameProcessed(int32_t bitstream_buffer_id, size_t ip_buffer_index, scoped_refptr<VideoFrame> frame)
{
    DVLOGF(4) << "ip_buffer_index=" << ip_buffer_index << ", bitstream_buffer_id=" << bitstream_buffer_id;
    DCHECK(decoder_thread_.task_runner()->BelongsToCurrentThread());
    // TODO(crbug.com/921825): Remove this workaround once reset callback is
    // implemented.
    if (buffers_at_ip_.empty() || buffers_at_ip_.front().first != bitstream_buffer_id || output_buffer_map_.empty()) {
        ...
        // Reset() case.
        DVLOGF(4) << "Ignore processed frame for bitstream_buffer_id=" << bitstream_buffer_id;
        return;
    }
    ...
    if (ip_output_record.texture_id != 0 && !ip_output_record.cleared) {
        ...
        child_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&V4L2VideoDecodeAccelerator::CreateEGLImageFor, weak_this_, image_processor_device_, ip_buffer_index, ip_output_record.picture_id,
                CreateGpuMemoryBufferHandle(frame.get()).native_pixmap_handle, ip_output_record.texture_id, visible_size_, *egl_image_format_fourcc_));
    }
    ...
    SendBufferToClient(ip_buffer_index, bitstream_buffer_id, std::move(vda_buffer), std::move(frame));
...
bool V4L2VideoDecodeAccelerator::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    if (input_queue_) {
        ...
        input_queue_buffers_memory_type = V4L2MemoryToString(input_queue_->GetMemoryType());
    ...
    if (output_queue_) {
        ...
        output_queue_buffers_memory_type = V4L2MemoryToString(output_queue_->GetMemoryType());
        if (output_queue_->GetMemoryType() == V4L2_MEMORY_MMAP)
    ...
    auto dump_name = base::StringPrintf("gpu/v4l2/decoder/0x%" PRIxPTR, reinterpret_cast<uintptr_t>(this));
