...
struct v4l2_format BuildV4L2Format(const enum v4l2_buf_type type, uint32_t fourcc, const gfx::Size& size, size_t buffer_size)
{
...
int64_t V4L2BufferTimestampInMilliseconds(const struct v4l2_buffer* v4l2_buffer)
{
    struct timespec ts;
...
void V4L2ProcessingTrace(const struct v4l2_buffer* v4l2_buffer, bool start)
{
    ...
    TRACE_EVENT_INSTANT1(kTracingCategory, name, TRACE_EVENT_SCOPE_THREAD, "type", v4l2_buffer->type);
    ...
    if (start && v4l2_buffer->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(kTracingCategory, kVideoDecoding, TRACE_ID_LOCAL(timestamp), "timestamp", timestamp);
    } else if (!start && v4l2_buffer->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
        TRACE_EVENT_NESTABLE_ASYNC_END1(kTracingCategory, kVideoDecoding, TRACE_ID_LOCAL(timestamp), "timestamp", timestamp);
    }
}
...
std::vector<base::ScopedFD> GetDmabufsForV4L2Buffer(const IoctlAsCallback& ioctl_cb, int index, size_t num_planes, enum v4l2_buf_type buf_type)
{
    DVLOGF(3);
...
V4L2ExtCtrl::V4L2ExtCtrl(uint32_t id, int32_t val)
    : V4L2ExtCtrl(id)
{
    ctrl.value = val;
}
...
class V4L2Buffer {
public:
    static std::unique_ptr<V4L2Buffer> Create(const IoctlAsCallback& ioctl_cb, const MmapAsCallback& mmap_cb, enum v4l2_buf_type type, enum v4l2_memory memory,
        const struct v4l2_format& format, size_t buffer_id);
    ...
    const struct v4l2_buffer& v4l2_buffer() const
    {
        return v4l2_buffer_;
    }
...
private:
    V4L2Buffer(const IoctlAsCallback& ioctl_cb, const MmapAsCallback& mmap_cb, enum v4l2_buf_type type, enum v4l2_memory memory,
        const struct v4l2_format& format, size_t buffer_id);
    bool Query();
...
std::unique_ptr<V4L2Buffer> V4L2Buffer::Create(const IoctlAsCallback& ioctl_cb, const MmapAsCallback& mmap_cb, enum v4l2_buf_type type, enum v4l2_memory memory,
    const struct v4l2_format& format, size_t buffer_id)
{
    // Not using std::make_unique because constructor is private.
    std::unique_ptr<V4L2Buffer> buffer(new V4L2Buffer(std::move(ioctl_cb), std::move(mmap_cb), type, memory, format, buffer_id));
    if (!buffer->Query()) {
        return nullptr;
    }
...
V4L2Buffer::V4L2Buffer(const IoctlAsCallback& ioctl_cb, const MmapAsCallback& mmap_cb, enum v4l2_buf_type type, enum v4l2_memory memory,
    const struct v4l2_format& format, size_t buffer_id)
    : ioctl_cb_(ioctl_cb)
    , mmap_cb_(mmap_cb)
    , format_(format)
{
    ...
    v4l2_buffer_.length = std::min(static_cast<size_t>(format.fmt.pix_mp.num_planes), std::size(v4l2_planes_));
...
void* V4L2Buffer::GetPlaneMapping(const size_t plane)
{
    ...
    p = mmap_cb_.Run(nullptr, v4l2_buffer_.m.planes[plane].length, PROT_READ | PROT_WRITE, MAP_SHARED, v4l2_buffer_.m.planes[plane].m.mem_offset);
    if (p == MAP_FAILED) {
...
scoped_refptr<FrameResource> V4L2Buffer::CreateFrame()
{
    ...
    std::vector<base::ScopedFD> dmabuf_fds
        = GetDmabufsForV4L2Buffer(ioctl_cb_, v4l2_buffer_.index, v4l2_buffer_.length, static_cast<enum v4l2_buf_type>(v4l2_buffer_.type));
    if (dmabuf_fds.empty()) {
    ...
    return NativePixmapFrameResource::Create(*layout, gfx::Rect(size), size, std::move(dmabuf_fds), base::TimeDelta());
}
...
std::optional<size_t> V4L2BuffersList::GetFreeBuffer(size_t requested_buffer_id)
{
    base::AutoLock auto_lock(lock_);

    return (free_buffers_.erase(requested_buffer_id) > 0) ? std::make_optional(requested_buffer_id) : std::nullopt;
}
...
public:
    V4L2BufferRefBase(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue);
...
V4L2BufferRefBase::V4L2BufferRefBase(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue)
    : queue_(std::move(queue))
    , return_to_(queue_->free_buffers_)
{
    ...
    memcpy(v4l2_planes_, v4l2_buffer.m.planes, sizeof(struct v4l2_plane) * v4l2_buffer.length);
    v4l2_buffer_.m.planes = v4l2_planes_;
}
...
bool V4L2BufferRefBase::CheckNumFDsForFormat(const size_t num_fds) const
{
    ...
    if (num_fds < required_fds) {
        VLOGF(1) << "Insufficient number of FDs given for the current format. " << num_fds << " provided, " << required_fds << " required.";
        return false;
    }
...
V4L2WritableBufferRef::V4L2WritableBufferRef(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue)
    : buffer_data_(std::make_unique<V4L2BufferRefBase>(v4l2_buffer, std::move(queue)))
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
bool V4L2WritableBufferRef::DoQueue(V4L2RequestRef* request_ref, scoped_refptr<FrameResource> frame) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);

    if (request_ref && buffer_data_->queue_->SupportsRequests() && !request_ref->ApplyQueueBuffer(&(buffer_data_->v4l2_buffer_))) {
        return false;
    }
...
bool V4L2WritableBufferRef::QueueUserPtr(const std::vector<void*>& ptrs, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
    ...
    if (ptrs.size() != self.PlanesCount()) {
        VLOGF(1) << "Provided " << ptrs.size() << " pointers while we require " << self.buffer_data_->v4l2_buffer_.length << ".";
        return false;
    }

    for (size_t i = 0; i < ptrs.size(); i++) {
        self.buffer_data_->v4l2_buffer_.m.planes[i].m.userptr = reinterpret_cast<unsigned long>(ptrs[i]);
    }

    return std::move(self).DoQueue(request_ref, nullptr);
}

bool V4L2WritableBufferRef::QueueDMABuf(const std::vector<base::ScopedFD>& fds, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
...
bool V4L2WritableBufferRef::QueueDMABuf(scoped_refptr<FrameResource> frame, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
    ...
    if (frame->storage_type() != VideoFrame::STORAGE_GPU_MEMORY_BUFFER && frame->storage_type() != VideoFrame::STORAGE_DMABUFS) {
...
bool V4L2WritableBufferRef::QueueDMABuf(const std::vector<gfx::NativePixmapPlane>& planes, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
...
bool V4L2WritableBufferRef::QueueDMABuf(uint64_t secure_handle, V4L2RequestRef* request_ref) &&
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
    ...
    bool set_fd = self.buffer_data_->queue_->SetBufferFdForSecureHandle(secure_handle, &self.buffer_data_->v4l2_buffer_);
    if (!set_fd) {
        return false;
    }
...
void V4L2WritableBufferRef::SetPlaneBytesUsed(const size_t plane, const size_t bytes_used)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
    ...
    if (bytes_used > GetPlaneSize(plane)) {
        VLOGF(1) << "Set bytes used " << bytes_used << " larger than plane size " << GetPlaneSize(plane) << ".";
        return;
    }
...
void V4L2WritableBufferRef::SetPlaneDataOffset(const size_t plane, const size_t data_offset)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer_data_);
...
V4L2ReadableBuffer::V4L2ReadableBuffer(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<FrameResource> frame)
    : buffer_data_(std::make_unique<V4L2BufferRefBase>(v4l2_buffer, std::move(queue)))
...
struct SecureBufferData {
    SecureBufferData(uint64_t in_secure_handle, base::ScopedFD in_fd)
        : secure_handle(in_secure_handle)
        , fd(std::move(in_fd))
    {
    }
...
#define VPQLOGF(level) VPLOGF(level) << "(" << V4L2BufferTypeToString(type_) << ") "
#define VQLOGF(level) VLOGF(level) << "(" << V4L2BufferTypeToString(type_) << ") "
#define DVQLOGF(level) DVLOGF(level) << "(" << V4L2BufferTypeToString(type_) << ") "

V4L2Queue::V4L2Queue(const IoctlAsCallback& ioctl_cb, const base::RepeatingClosure& schedule_poll_cb, const MmapAsCallback& mmap_cb,
    const AllocateSecureBufferAsCallback& allocate_secure_cb, enum v4l2_buf_type type, base::OnceClosure destroy_cb)
    : type_(type)
    , ioctl_cb_(ioctl_cb)
    ...
    , weak_this_factory_(this)
{
    ...
    struct v4l2_requestbuffers reqbufs = { .count = 0, .type = type_, .memory = V4L2_MEMORY_MMAP };
    supports_requests_ = (ioctl_cb_.Run(VIDIOC_REQBUFS, &reqbufs) == kIoctlOk) && (reqbufs.capabilities & V4L2_BUF_CAP_SUPPORTS_REQUESTS);
...
std::optional<struct v4l2_format> V4L2Queue::SetFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    struct v4l2_format format = BuildV4L2Format(type_, fourcc, size, buffer_size);
    if (ioctl_cb_.Run(VIDIOC_S_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != fourcc) {
...
std::optional<struct v4l2_format> V4L2Queue::TryFormat(uint32_t fourcc, const gfx::Size& size, size_t buffer_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    struct v4l2_format format = BuildV4L2Format(type_, fourcc, size, buffer_size);
    if (ioctl_cb_.Run(VIDIOC_TRY_FMT, &format) != 0 || format.fmt.pix_mp.pixelformat != fourcc) {
...
std::optional<gfx::Rect> V4L2Queue::GetVisibleRect()
{
    ...
    struct v4l2_selection selection = { .type = type_, .target = V4L2_SEL_TGT_COMPOSE };
...
size_t V4L2Queue::AllocateBuffers(size_t count, enum v4l2_memory memory, bool incoherent)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (buffers_.size() != 0) {
        VQLOGF(1) << "Cannot allocate new buffers while others are still allocated.";
        return 0;
    }
    ...
    struct v4l2_requestbuffers reqbufs
        = { .count = base::checked_cast<decltype(v4l2_requestbuffers::count)>(count), .type = type_, .memory = memory, .flags = flags };
    DVQLOGF(3) << "Requesting " << count << " buffers (" << (incoherent ? "incoherent" : "coherent") << ")";
    ...
    for (size_t i = 0; i < reqbufs.count; i++) {
        auto buffer = V4L2Buffer::Create(ioctl_cb_, mmap_cb_, type_, memory_, *format, i);
        ...
        if (type_ == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE && allocate_secure_cb_) {
            ...
            allocate_secure_cb_.Run(
                buffer->v4l2_buffer().m.planes[0].length, base::BindOnce(&V4L2Queue::SecureBufferAllocated, weak_this_factory_.GetWeakPtr()));
        }
...
bool V4L2Queue::DeallocateBuffers()
{
    ...
    struct v4l2_requestbuffers reqbufs = { .count = 0, .type = type_, .memory = memory_, .flags = flags };
...
public:
    static V4L2WritableBufferRef CreateWritableRef(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue)
    {
        return V4L2WritableBufferRef(v4l2_buffer, std::move(queue));
    }

    static V4L2ReadableBufferRef CreateReadableRef(const struct v4l2_buffer& v4l2_buffer, base::WeakPtr<V4L2Queue> queue, scoped_refptr<FrameResource> frame)
    {
        return new V4L2ReadableBuffer(v4l2_buffer, std::move(queue), std::move(frame));
    }
};
...
std::optional<V4L2WritableBufferRef> V4L2Queue::GetFreeBuffer()
{
    ...
    return V4L2BufferRefFactory::CreateWritableRef(buffers_[buffer_id.value()]->v4l2_buffer(), weak_this_factory_.GetWeakPtr());
}

std::optional<V4L2WritableBufferRef> V4L2Queue::GetFreeBuffer(size_t requested_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
std::optional<V4L2WritableBufferRef> V4L2Queue::GetFreeBufferForFrame(const gfx::GenericSharedMemoryId& id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool V4L2Queue::QueueBuffer(struct v4l2_buffer* v4l2_buffer, scoped_refptr<FrameResource> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const auto inserted = queued_buffers_.emplace(v4l2_buffer->index, std::move(frame));
    DCHECK(inserted.second);
...
std::pair<bool, V4L2ReadableBufferRef> V4L2Queue::DequeueBuffer()
{
    ...
    for (auto& buf : secure_buffers_) {
        std::erase_if(buf.queued_buffer_indexes, [v4l2_buffer](size_t idx) { return idx == v4l2_buffer.index; });
    }

    DCHECK(free_buffers_);
    return std::make_pair(true, V4L2BufferRefFactory::CreateReadableRef(v4l2_buffer, weak_this_factory_.GetWeakPtr(), std::move(queued_frame)));
}
...
std::optional<struct v4l2_format> V4L2Queue::SetModifierFormat(uint64_t modifier, const gfx::Size& size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool V4L2Queue::SetBufferFdForSecureHandle(uint64_t secure_handle, struct v4l2_buffer* v4l2_buffer)
{
    for (auto& buf : secure_buffers_) {
        if (buf.secure_handle == secure_handle) {
...
bool V4L2Queue::SendCommand(__u32 command)
{
    ...
    PLOG_IF(ERROR, !success) << "Failed to issue command " << command << " (V4L2_DEC_CMD_START: " << V4L2_DEC_CMD_START
                             << ", V4L2_DEC_CMD_STOP: " << V4L2_DEC_CMD_STOP << ")";
    return success;
}

void V4L2Queue::SecureBufferAllocated(base::ScopedFD secure_fd, uint64_t secure_handle)
{
...
private:
    ...
    V4L2Request(base::ScopedFD&& request_fd, V4L2RequestsQueue* request_queue)
        ...
        , request_fd_(std::move(request_fd))
    {
    }
...
std::optional<base::ScopedFD> V4L2RequestsQueue::CreateRequestFD()
{
    ...
    int request_fd;
    int ret = HANDLE_EINTR(ioctl(media_fd_.get(), MEDIA_IOC_REQUEST_ALLOC, &request_fd));
    if (ret < 0) {
...
std::optional<V4L2RequestRef> V4L2RequestsQueue::GetFreeRequest()
{
    ...
    V4L2Request* request_ptr = free_requests_.empty() ? nullptr : free_requests_.front();
    if (request_ptr && request_ptr->IsCompleted()) {
        ...
        // Not using std::make_unique because constructor is private.
        std::unique_ptr<V4L2Request> request(new V4L2Request(std::move(*request_fd), this));
