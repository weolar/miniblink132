...
class MEDIA_GPU_EXPORT V4L2VideoDecodeAccelerator : public VideoDecodeAccelerator, public base::trace_event::MemoryDumpProvider {
public:
    V4L2VideoDecodeAccelerator(EGLDisplay egl_display, const GetGLContextCallback& get_gl_context_cb,
    ...
    V4L2VideoDecodeAccelerator& operator=(const V4L2VideoDecodeAccelerator&) = delete;
    ...
    void ImportBufferForPicture(int32_t picture_buffer_id, VideoPixelFormat pixel_format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handles) override;
...
private:
    ...
    void InitializeTask(const Config& config, bool* result, base::WaitableEvent* done);
    ...
    // able to see.
    void CreateEGLImageFor(scoped_refptr<V4L2Device> egl_device, size_t buffer_index, int32_t picture_buffer_id, gfx::NativePixmapHandle handle,
        GLuint texture_id, const gfx::Size& visible_size, const Fourcc fourcc);

    // Take the EGLImage |egl_image|, created for |picture_buffer_id|, and use it
    // for OutputRecord at |buffer_index|.
    void AssignEGLImage(size_t buffer_index, int32_t picture_buffer_id, EGLImageKHR egl_image);
    ...
    bool GetFormatInfo(struct v4l2_format* format, gfx::Size* visible_size, bool* again);
    // Create output buffers for the given |format| and |visible_size|.
    bool CreateBuffersForFormat(const struct v4l2_format& format, const gfx::Size& visible_size);
    ...
    void SendBufferToClient(size_t buffer_index, int32_t bitstream_buffer_id, V4L2ReadableBufferRef vda_buffer, scoped_refptr<VideoFrame> frame = nullptr);
    ...
    // image processor.
    void FrameProcessed(int32_t bitstream_buffer_id, size_t output_buffer_index, scoped_refptr<VideoFrame> frame);
    ...
    // for H.264 streams.
    std::unique_ptr<v4l2_vda_helpers::InputBufferFragmentSplitter> frame_splitter_;
    ...
    std::map<int32_t, std::pair<V4L2ReadableBufferRef, scoped_refptr<VideoFrame>>> buffers_at_client_;
    ...
    std::queue<std::pair<std::unique_ptr<gl::GLFenceEGL>, std::pair<V4L2ReadableBufferRef, scoped_refptr<VideoFrame>>>> buffers_awaiting_fence_;
