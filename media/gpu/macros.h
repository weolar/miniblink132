...
#define DVLOGF_IF(level, condition) DVLOG_IF(level, condition) << __func__ << "(): "
#define VLOGF(level) VLOG(level) << __func__ << "(): "
#define VPLOGF(level) VPLOG(level) << __func__ << "(): "
#define LOGF(severity) LOG(severity) << __func__ << "(): "
#define DLOGF(severity) DLOG(severity) << __func__ << "(): "
#define DLOGF_IF(severity, condition) DLOG_IF(severity, condition) << __func__ << "(): "

namespace media {

// Copy the memory between arrays with checking the array size.
template <typename T, size_t N> inline void SafeArrayMemcpy(T (&to)[N], const T (&from)[N])
{
