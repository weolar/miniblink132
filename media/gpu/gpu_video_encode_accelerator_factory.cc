...
std::unique_ptr<VideoEncodeAccelerator> CreateV4L2VEA()
{
    ...
    return base::WrapUnique<VideoEncodeAccelerator>(new V4L2VideoEncodeAccelerator(new V4L2Device()));
#else
    return nullptr;
#endif
}
...
{
    return base::WrapUnique<VideoEncodeAccelerator>(new VaapiVideoEncodeAccelerator());
}
#endif
...
std::unique_ptr<VideoEncodeAccelerator> CreateAndroidVEA()
{
    if (__builtin_available(android NDK_MEDIA_CODEC_MIN_API, *)) {
        return base::WrapUnique<VideoEncodeAccelerator>(new NdkVideoEncodeAccelerator(base::SequencedTaskRunner::GetCurrentDefault()));
    } else {
        return base::WrapUnique<VideoEncodeAccelerator>(new AndroidVideoEncodeAccelerator());
    }
}
#endif
...
{
    return base::WrapUnique<VideoEncodeAccelerator>(new VTVideoEncodeAccelerator());
}
#endif
...
{
    return base::WrapUnique<VideoEncodeAccelerator>(new MediaFoundationVideoEncodeAccelerator(gpu_preferences, gpu_workarounds, gpu_device.luid));
}
#endif
...
{
    return base::WrapUnique<VideoEncodeAccelerator>(new FuchsiaVideoEncodeAccelerator());
}
#endif

using VEAFactoryFunction = base::RepeatingCallback<std::unique_ptr<VideoEncodeAccelerator>()>;
...
#endif
#if BUILDFLAG(IS_WIN)
    vea_factory_functions.push_back(base::BindRepeating(&CreateMediaFoundationVEA, gpu_preferences, gpu_workarounds, gpu_device));
#endif
#if BUILDFLAG(IS_FUCHSIA)
    ...
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, const gpu::GPUInfo::GPUDevice& gpu_device)
{
    ...
    for (const auto& create_vea : GetVEAFactoryFunctions(gpu_preferences, gpu_workarounds, gpu_device)) {
        auto vea = std::move(create_vea).Run();
        if (!vea)
            continue;
        auto vea_profiles = vea->GetSupportedProfiles();
        GpuVideoAcceleratorUtil::InsertUniqueEncodeProfiles(vea_profiles, &profiles);
    }
    return profiles;
}
...
// static
MEDIA_GPU_EXPORT std::unique_ptr<VideoEncodeAccelerator> GpuVideoEncodeAcceleratorFactory::CreateVEA(const VideoEncodeAccelerator::Config& config,
    ...
    const gpu::GPUInfo::GPUDevice& gpu_device, std::unique_ptr<MediaLog> media_log, GetCommandBufferHelperCB get_command_buffer_helper_cb,
    scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner)
{
    ...
    for (const auto& create_vea : GetVEAFactoryFunctions(gpu_preferences, gpu_workarounds, gpu_device)) {
        ...
        if (!get_command_buffer_helper_cb.is_null()) {
            vea->SetCommandBufferHelperCB(get_command_buffer_helper_cb, gpu_task_runner);
        }
...
// static
MEDIA_GPU_EXPORT VideoEncodeAccelerator::SupportedProfiles GpuVideoEncodeAcceleratorFactory::GetSupportedProfiles(
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, const gpu::GPUInfo::GPUDevice& gpu_device)
{
    ...
    // between calls.
    static auto profiles = GetSupportedProfilesInternal(gpu_preferences, gpu_workarounds, gpu_device);
    ...
    if (gpu_workarounds.disable_accelerated_av1_encode) {
        std::erase_if(
            profiles, [](const auto& vea_profile) { return vea_profile.profile >= AV1PROFILE_PROFILE_MAIN && vea_profile.profile <= AV1PROFILE_PROFILE_PRO; });
    }

    if (gpu_workarounds.disable_accelerated_vp8_encode) {
        std::erase_if(profiles, [](const auto& vea_profile) { return vea_profile.profile == VP8PROFILE_ANY; });
    }

    if (gpu_workarounds.disable_accelerated_vp9_encode) {
        std::erase_if(
            profiles, [](const auto& vea_profile) { return vea_profile.profile >= VP9PROFILE_PROFILE0 && vea_profile.profile <= VP9PROFILE_PROFILE3; });
    }

    if (gpu_workarounds.disable_accelerated_h264_encode) {
        std::erase_if(profiles, [](const auto& vea_profile) { return vea_profile.profile >= H264PROFILE_MIN && vea_profile.profile <= H264PROFILE_MAX; });
    }
