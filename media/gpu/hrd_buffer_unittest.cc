...
protected:
    ...
    int GetBufferSizeFromDelay(uint32_t avg_bitrate, base::TimeDelta buffer_delay) const
    {
    ...
    {
        return 100 * hrd_buffer_->GetBytesAtTime(timestamp) / hrd_buffer_->buffer_size();
    }
...
TEST_F(HRDBufferTest, RunBasicBufferTest)
{
    ...
    size_t buffer_size = GetBufferSizeFromDelay(kCommonAvgBitrate, kCommonBufferDelay);

    hrd_buffer_->SetParameters(buffer_size, kCommonAvgBitrate, kCommonPeakBitrate, false);
    EXPECT_EQ(buffer_size, hrd_buffer_->buffer_size());
    EXPECT_EQ(kCommonAvgBitrate, hrd_buffer_->average_bitrate());

    int start_frame_index = 0;
    int last_frame_index = RunTestSequence(kCommonAvgBitrate, kCommonFps, start_frame_index);
    ...
    EXPECT_EQ(kExpectedBufferBytesRemaining, hrd_buffer_->GetBytesRemainingAtTime(timestamp));
    EXPECT_EQ(kExpectedLastFrameBufferBytes, hrd_buffer_->last_frame_buffer_bytes());
    EXPECT_EQ(kExpectedFrameOvershooting, hrd_buffer_->frame_overshooting());

    // Check behavior when invalid timestamp is provided.
    timestamp = base::Microseconds(last_frame_index * base::Time::kMicrosecondsPerSecond / kCommonFps) - base::Microseconds(60000);
...
TEST_F(HRDBufferTest, CheckBufferParameterChange)
{
    ...
    EXPECT_EQ(kExpectedBufferBytesRemaining1, hrd_buffer_->GetBytesRemainingAtTime(timestamp));
    EXPECT_EQ(kExpectedLastFrameBufferBytes1, hrd_buffer_->last_frame_buffer_bytes());

    // Increase average bitrate 50%.
    hrd_buffer_->SetParameters(buffer_size, kCommonAvgBitrate * 3 / 2, kCommonPeakBitrate * 3 / 2, false);
    EXPECT_EQ(buffer_size, hrd_buffer_->buffer_size());
    EXPECT_EQ(kCommonAvgBitrate * 3 / 2, hrd_buffer_->average_bitrate());

    start_frame_index = last_frame_index;
    last_frame_index = RunTestSequence(kCommonAvgBitrate, kCommonFps, start_frame_index);
    ...
    EXPECT_EQ(kExpectedBufferBytesRemaining2, hrd_buffer_->GetBytesRemainingAtTime(timestamp));
    EXPECT_EQ(kExpectedLastFrameBufferBytes2, hrd_buffer_->last_frame_buffer_bytes());

    // Decrease average bitrate 33%.
    hrd_buffer_->SetParameters(buffer_size, kCommonAvgBitrate * 2 / 3, kCommonPeakBitrate * 2 / 3, false);
    EXPECT_EQ(buffer_size, hrd_buffer_->buffer_size());
    ...
    EXPECT_EQ(kExpectedBufferBytesRemaining3, hrd_buffer_->GetBytesRemainingAtTime(timestamp));
    EXPECT_EQ(kExpectedLastFrameBufferBytes3, hrd_buffer_->last_frame_buffer_bytes());
}
...
TEST_F(HRDBufferTest, CheckSettingBufferState)
{
    ...
    constexpr base::TimeDelta kExpectedLastFrameTimestamp1 = base::Microseconds(-1);
    ...
    constexpr base::TimeDelta kExpectedLastFrameTimestamp2 = base::Microseconds(99000);
    ...
    hrd_buffer_ = std::make_unique<HRDBuffer>(buffer_size, kCommonAvgBitrate, kLastFrameBufferBytes2, kLastFrameTimestamp2);
...
{
    constexpr int kFrameSequenceValues[] = { 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000 };
    constexpr size_t kBufferShrinkingValues[] = { 122917, 120834, 118751, 116668, 114585, 112502, 110419, 108336, 106253, 104170 };

    const size_t buffer_size = GetBufferSizeFromDelay(kCommonAvgBitrate, kCommonBufferDelay);
    ...
    hrd_buffer_->SetParameters(buffer_size / 2, kCommonAvgBitrate, kCommonPeakBitrate, true);
    ...
    hrd_buffer_->SetParameters(buffer_size / 3, kCommonAvgBitrate, static_cast<uint32_t>(kCommonAvgBitrate * 1.2f), true);
...
{
    constexpr int kFrameSequenceValues[] = { 30000, 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000 };
    constexpr int kExpectedFrameOvershootingIndex = 6;

    const size_t buffer_size = GetBufferSizeFromDelay(kCommonAvgBitrate, kCommonBufferDelay / 2);
