...
bool ParseBitDepth(const H264SPS& sps, uint8_t& bit_depth)
{
    ...
    case 6:
        ...
        break;
    default:
        DVLOG(1) << "Invalid bit depth: " << base::checked_cast<int>(sps.bit_depth_luma_minus8 + 8);
        return false;
    }
    return true;
}
...
bool IsValidBitDepth(uint8_t bit_depth, VideoCodecProfile profile)
{
    ...
    case H264PROFILE_HIGH444PREDICTIVEPROFILE:
        return bit_depth == 8u || bit_depth == 10u || bit_depth == 12u || bit_depth == 14u;
...
scoped_refptr<H264Picture> H264Decoder::H264Accelerator::CreateH264PictureSecure(uint64_t secure_handle)
{
    return nullptr;
}

void H264Decoder::H264Accelerator::ProcessSPS(const H264SPS* sps, base::span<const uint8_t> sps_nalu_data)
{
}

void H264Decoder::H264Accelerator::ProcessPPS(const H264PPS* pps, base::span<const uint8_t> pps_nalu_data)
{
}

H264Decoder::H264Accelerator::Status H264Decoder::H264Accelerator::ParseEncryptedSliceHeader(const std::vector<base::span<const uint8_t>>& data,
...
H264Decoder::H264Accelerator::Status H264Decoder::H264Accelerator::SetStream(base::span<const uint8_t> stream, const DecryptConfig* decrypt_config)
{
    return H264Decoder::H264Accelerator::Status::kNotSupported;
}
...
H264Decoder::H264Decoder(std::unique_ptr<H264Accelerator> accelerator, VideoCodecProfile profile, const VideoColorSpace& container_color_space)
...
bool H264Decoder::InitNonexistingPicture(scoped_refptr<H264Picture> pic, int frame_num)
{
...
{
    if (!FillH264PictureFromSliceHeader(parser_.GetSPS(curr_sps_id_), *slice_hdr, curr_pic_.get())) {
        return false;
    }
    ...
    if (slice_hdr->adaptive_ref_pic_marking_mode_flag) {
        static_assert(sizeof(curr_pic_->ref_pic_marking) == sizeof(slice_hdr->ref_pic_marking), "Array sizes of ref pic marking do not match.");
        memcpy(curr_pic_->ref_pic_marking, slice_hdr->ref_pic_marking, sizeof(curr_pic_->ref_pic_marking));
    }
...
bool H264Decoder::CalculatePicOrderCounts(scoped_refptr<H264Picture> pic)
{
    ...
    case 1: {
        ...
        if (abs_frame_num > 0) {
            ...
            expected_pic_order_cnt = base::CheckedNumeric<int>(pic_order_cnt_cycle_cnt) * sps->expected_delta_per_pic_order_cnt_cycle;
        ...
        base::CheckedNumeric<int> top_field_order_cnt = expected_pic_order_cnt + pic->delta_pic_order_cnt0;
        base::CheckedNumeric<int> bottom_field_order_cnt = top_field_order_cnt + sps->offset_for_top_to_bottom_field + pic->delta_pic_order_cnt1;
...
struct PicNumDescCompare {
    bool operator()(const scoped_refptr<H264Picture>& a, const scoped_refptr<H264Picture>& b) const
    {
...
void H264Decoder::ConstructReferencePicListsP()
{
    ...
    std::sort(ref_pic_list_p0_.begin(), ref_pic_list_p0_.end(), PicNumDescCompare());

    // Now get long term pics and sort them by long_term_pic_num to get [2].
    dpb_.GetLongTermRefPicsAppending(&ref_pic_list_p0_);
    std::sort(ref_pic_list_p0_.begin() + num_short_refs, ref_pic_list_p0_.end(), LongTermPicNumAscCompare());
}
...
void H264Decoder::ConstructReferencePicListsB()
{
    ...
    iter = std::upper_bound(ref_pic_list_b0_.begin(), ref_pic_list_b0_.end(), curr_pic_.get(), POCAscCompare());
    ...
    std::sort(ref_pic_list_b0_.begin() + num_short_refs, ref_pic_list_b0_.end(), LongTermPicNumAscCompare());
    ...
    // Find first with POC < curr_pic's POC to get first element in [2]...
    iter = std::upper_bound(ref_pic_list_b1_.begin(), ref_pic_list_b1_.end(), curr_pic_.get(), POCDescCompare());
    ...
    std::sort(ref_pic_list_b1_.begin() + num_short_refs, ref_pic_list_b1_.end(), LongTermPicNumAscCompare());

    // If lists identical, swap first two entries in RefPicList1 (spec 8.2.4.2.3)
    if (ref_pic_list_b1_.size() > 1 && base::ranges::equal(ref_pic_list_b0_, ref_pic_list_b1_))
...
static void ShiftRightAndInsert(H264Picture::Vector* v, int from, int to, scoped_refptr<H264Picture> pic)
{
...
bool H264Decoder::ModifyReferencePicList(const H264SliceHeader* slice_hdr, int list, H264Picture::Vector* ref_pic_listx)
{
    ...
    for (int i = 0; i < H264SliceHeader::kRefListModSize && !done; ++i) {
        ...
        case 1:
            ...
            DCHECK_LT(num_ref_idx_lX_active_minus1 + 1, H264SliceHeader::kRefListModSize);
            ...
            if (ref_idx_lx > num_ref_idx_lX_active_minus1) {
                DVLOG(1) << "Bounds mismatch: expected " << ref_idx_lx << " <= " << num_ref_idx_lX_active_minus1;
                return false;
            }

            ShiftRightAndInsert(ref_pic_listx, ref_idx_lx, num_ref_idx_lX_active_minus1, pic);
            ref_idx_lx++;

            for (int src = ref_idx_lx, dst = ref_idx_lx; src <= num_ref_idx_lX_active_minus1 + 1; ++src) {
        ...
        case 2:
            ...
            if (!pic) {
                DVLOG(1) << "Malformed stream, no pic num " << list_mod->long_term_pic_num;
                return false;
            }
            ShiftRightAndInsert(ref_pic_listx, ref_idx_lx, num_ref_idx_lX_active_minus1, pic);
            ref_idx_lx++;
        ...
        default:
            // May be recoverable.
            DVLOG(1) << "Invalid modification_of_pic_nums_idc=" << list_mod->modification_of_pic_nums_idc << " in position " << i;
            break;
        }
...
bool H264Decoder::OutputPic(scoped_refptr<H264Picture> pic)
{
    ...
    DVLOG_IF(1, pic->pic_order_cnt < last_output_poc_)
        << "Outputting out of order, likely a broken stream: " << last_output_poc_ << " -> " << pic->pic_order_cnt;
...
H264Decoder::H264Accelerator::Status H264Decoder::StartNewFrame(const H264SliceHeader* slice_hdr)
{
    ...
    return accelerator_->SubmitFrameMetadata(sps, pps, dpb_, ref_pic_list_p0_, ref_pic_list_b0_, ref_pic_list_b1_, curr_pic_.get());
}
...
bool H264Decoder::HandleMemoryManagementOps(scoped_refptr<H264Picture> pic)
{
    ...
    for (size_t i = 0; i < std::size(pic->ref_pic_marking); ++i) {
        ...
        case 2:
            ...
            // if outputted.
            to_mark = dpb_.GetLongRefPicByLongTermPicNum(ref_pic_marking->long_term_pic_num);
            if (to_mark) {
                to_mark->ref = false;
            } else {
        ...
        case 3:
            ...
            if (to_mark) {
                ...
                scoped_refptr<H264Picture> long_term_mark = dpb_.GetLongRefPicByLongTermIdx(ref_pic_marking->long_term_frame_idx);
                if (long_term_mark) {
...
bool H264Decoder::FinishPicture(scoped_refptr<H264Picture> pic)
{
    ...
    DVLOG(4) << "Finishing picture frame_num: " << pic->frame_num << ", entries in DPB: " << dpb_.size();
    if (recovery_frame_cnt_) {
        ...
        if (*recovery_frame_cnt_ >= max_frame_num_) {
            DVLOG(1) << "Invalid recovery_frame_cnt=" << *recovery_frame_cnt_ << " (must be less than or equal to max_frame_num-1=" << (max_frame_num_ - 1)
                     << ")";
            return false;
        }

        // Compute the frame_num of the first frame that should be output (D.2.8).
        recovery_frame_num_ = (*recovery_frame_cnt_ + pic->frame_num) % max_frame_num_;
    ...
    while (num_remaining > max_num_reorder_frames_ ||
        ...
        DVLOG_IF(1, num_remaining <= max_num_reorder_frames_) << "Invalid stream: max_num_reorder_frames not preserved";
...
bool H264Decoder::UpdateMaxNumReorderFrames(const H264SPS* sps)
{
    if (sps->vui_parameters_present_flag && sps->bitstream_restriction_flag) {
        ...
        if (max_num_reorder_frames_ > dpb_.max_num_pics()) {
            DVLOG(1) << "max_num_reorder_frames present, but larger than MaxDpbFrames (" << max_num_reorder_frames_ << " > " << dpb_.max_num_pics() << ")";
            max_num_reorder_frames_ = 0;
            return false;
        }
        return true;
...
bool H264Decoder::ProcessSPS(int sps_id, bool* need_new_buffers)
{
    ...
    *need_new_buffers = false;
    ...
    size_t max_dpb_frames = std::min(max_dpb_mbs / (width_mb * height_mb), static_cast<int>(H264DPB::kDPBMaxSize));
    DVLOG(1) << "MaxDpbFrames: " << max_dpb_frames << ", max_num_ref_frames: " << sps->max_num_ref_frames
    ...
    DVLOG_IF(1, max_dpb_size > max_dpb_frames) << "Invalid stream, DPB size > MaxDpbFrames";
    ...
    if (!IsValidBitDepth(new_bit_depth, new_profile)) {
        DVLOG(1) << "Invalid bit depth=" << base::strict_cast<int>(new_bit_depth) << ", profile=" << GetProfileName(new_profile);
        return false;
    }
    ...
    if (base::FeatureList::IsEnabled(kAVDColorSpaceChanges)) {
        is_color_space_change = new_color_space.IsSpecified() && new_color_space != picture_color_space_;
    }

    if (pic_size_ != new_pic_size || dpb_.max_num_pics() != max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth || is_color_space_change) {
        if (!Flush()) {
            return false;
        }
        DVLOG(1) << "Codec profile: " << GetProfileName(new_profile) << ", level: " << base::strict_cast<int>(level) << ", DPB size: " << max_dpb_size
                 << ", Picture size: " << new_pic_size.ToString() << ", bit depth: " << base::strict_cast<int>(new_bit_depth)
...
bool H264Decoder::HandleFrameNumGap(int frame_num)
{
    ...
    DVLOG(2) << "Handling frame_num gap: " << prev_ref_frame_num_ << "->" << frame_num;
...
H264Decoder::H264Accelerator::Status H264Decoder::ProcessEncryptedSliceHeader(const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK(curr_nalu_);
    DCHECK(curr_slice_hdr_);
    std::vector<base::span<const uint8_t>> spans(prior_cencv1_nalus_.begin(), prior_cencv1_nalus_.end());
    spans.emplace_back(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size));
    std::vector<SubsampleEntry> all_subsamples(prior_cencv1_subsamples_.begin(), prior_cencv1_subsamples_.end());
    all_subsamples.insert(all_subsamples.end(), subsamples.begin(), subsamples.end());
    auto rv = accelerator_->ParseEncryptedSliceHeader(spans, all_subsamples, secure_handle_, curr_slice_hdr_.get());
    ...
    // picture.
    prior_cencv1_nalus_.emplace_back(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size));
    prior_cencv1_subsamples_.insert(prior_cencv1_subsamples_.end(), subsamples.begin(), subsamples.end());
    return rv;
}
...
H264Decoder::H264Accelerator::Status H264Decoder::PreprocessCurrentSlice()
{
    ...
    if (IsNewPrimaryCodedPicture(curr_pic_.get(), curr_pps_id_, parser_.GetSPS(curr_sps_id_), *slice_hdr)) {
        ...
        if (slice_hdr->first_mb_in_slice != 0) {
            DVLOG(1) << "ASO/invalid stream, first_mb_in_slice: " << slice_hdr->first_mb_in_slice;
            return H264Accelerator::Status::kFail;
        }
...
H264Decoder::H264Accelerator::Status H264Decoder::ProcessCurrentSlice()
{
    ...
    if (!slice_hdr->full_sample_encryption && requires_ref_lists_ && !ModifyReferencePicLists(slice_hdr, &ref_pic_list0, &ref_pic_list1)) {
        return H264Accelerator::Status::kFail;
    }
    ...
    return accelerator_->SubmitSlice(
        pps, slice_hdr, ref_pic_list0, ref_pic_list1, curr_pic_.get(), slice_hdr->nalu_data, slice_hdr->nalu_size, parser_.GetCurrentSubsamples());
}
...
void H264Decoder::SetStream(int32_t id, const DecoderBuffer& decoder_buffer)
{
    ...
    DCHECK(ptr);
    DCHECK(size);
    DVLOG(4) << "New input stream id: " << id << " at: " << (void*)ptr << " size: " << size;
    stream_id_ = id;
    current_stream_ = ptr;
    ...
    if (decoder_buffer.has_side_data() && decoder_buffer.side_data()->secure_handle) {
...
H264Decoder::DecodeResult H264Decoder::Decode()
{
    ...
    if (current_stream_has_been_changed_) {
        ...
        H264Accelerator::Status result
            = accelerator_->SetStream(base::span<const uint8_t>(current_stream_.get(), current_stream_size_), current_decrypt_config_.get());
        switch (result) {
        case H264Accelerator::Status::kOk:
        case H264Accelerator::Status::kNotSupported:
    ...
    while (true) {
        ...
        case H264NALU::kNonIDRSlice:
            ...
            // process is going.
            if (state_ == State::kError || (state_ == State::kAfterReset && !recovery_frame_cnt_))
                break;
        ...
        case H264NALU::kIDRSlice: {
            ...
            if (state_ == State::kParseSliceHeader) {
                ...
                if (current_decrypt_config_) {
                    const std::vector<SubsampleEntry>& subsamples = parser_.GetCurrentSubsamples();
        ...
        case H264NALU::kSPS: {
            ...
            accelerator_->ProcessSPS(parser_.GetSPS(sps_id), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
        ...
        case H264NALU::kPPS: {
            ...
            accelerator_->ProcessPPS(
                parser_.GetPPS(last_parsed_pps_id_), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
            break;
        }
        ...
        case H264NALU::kSEIMessage: {
            ...
            for (auto& sei_msg : sei.msgs) {
                ...
                case H264SEIMessage::kSEIRecoveryPoint:
                    ...
                    if (state_ == State::kAfterReset && !recovery_frame_cnt_ && !recovery_frame_num_) {
                        recovery_frame_cnt_ = sei_msg.recovery_point.recovery_frame_cnt;

                        if (0 > *recovery_frame_cnt_) {
                            DVLOG(1) << "Invalid recovery_frame_cnt=" << *recovery_frame_cnt_ << " (it must not be less then 0)";
                            SET_ERROR_AND_RETURN();
                        }
                        DVLOG(3) << "Recovery point SEI is found, recovery_frame_cnt_=" << *recovery_frame_cnt_;
                    }
                    break;
                case H264SEIMessage::kSEIContentLightLevelInfo:
                    ...
                    hdr_metadata_->cta_861_3 = sei_msg.content_light_level_info.ToGfx();
                    break;
                case H264SEIMessage::kSEIMasteringDisplayInfo:
                    if (!hdr_metadata_.has_value()) {
                        hdr_metadata_.emplace();
                    }
                    hdr_metadata_->smpte_st_2086 = sei_msg.mastering_display_info.ToGfx();
                    break;
                default:
                    break;
                }
...
// static
bool H264Decoder::FillH264PictureFromSliceHeader(const H264SPS* sps, const H264SliceHeader& slice_hdr, H264Picture* pic)
{
    DCHECK(pic);
...
// static
bool H264Decoder::IsNewPrimaryCodedPicture(const H264Picture* curr_pic, int curr_pps_id, const H264SPS* sps, const H264SliceHeader& slice_hdr)
{
    if (!curr_pic)
        return true;

    // 7.4.1.2.4, assumes non-interlaced.
    if (slice_hdr.frame_num != curr_pic->frame_num || slice_hdr.pic_parameter_set_id != curr_pps_id || slice_hdr.nal_ref_idc != curr_pic->nal_ref_idc
        ...
        || (slice_hdr.idr_pic_flag
            && (slice_hdr.idr_pic_id != curr_pic->idr_pic_id ||
                ...
                (slice_hdr.first_mb_in_slice == 0 && !slice_hdr.full_sample_encryption))))
        return true;
    ...
    if (sps->pic_order_cnt_type == curr_pic->pic_order_cnt_type) {
        if (curr_pic->pic_order_cnt_type == 0) {
            if (slice_hdr.pic_order_cnt_lsb != curr_pic->pic_order_cnt_lsb || slice_hdr.delta_pic_order_cnt_bottom != curr_pic->delta_pic_order_cnt_bottom)
                return true;
        } else if (curr_pic->pic_order_cnt_type == 1) {
            if (slice_hdr.delta_pic_order_cnt0 != curr_pic->delta_pic_order_cnt0 || slice_hdr.delta_pic_order_cnt1 != curr_pic->delta_pic_order_cnt1)
                return true;
        }
    }
