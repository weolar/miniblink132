...
public:
    ...
    struct MEDIA_GPU_EXPORT Config {
        Config(const std::vector<gfx::Size>& spatial_layer_resolutions, size_t begin_active_layer, size_t end_active_layer, size_t num_temporal_layers,
    ...
    void GetPictureParamAndMetadata(PictureParam& picture_param, Vp9Metadata& metadata) const;
...
private:
    ...
    void FillMetadataForFirstFrame(Vp9Metadata& metadata, bool& key_frame, uint8_t& refresh_frame_flags, std::vector<uint8_t>& reference_frame_indices) const;
    // Fill metadata for the second and later frames, i.e. frame_num != 0.
    void FillMetadataForNonFirstFrame(Vp9Metadata& metadata, uint8_t& refresh_frame_flags, std::vector<uint8_t>& reference_frame_indices) const;
