...
base::TimeDelta ClampedTimestampDiff(base::TimeDelta ts_new, base::TimeDelta ts_old)
{
    base::TimeDelta elapsed_time = ts_new - ts_old;
    base::TimeDelta clamped_elapsed_time = std::clamp(elapsed_time, base::Seconds(0), base::Minutes(5));
...
float ClampedLinearInterpolation(float x, float x0, float x1, float y0, float y1)
{
    return std::clamp(y0 + (x - x0) * (y1 - y0) / (x1 - x0), std::min(y0, y1), std::max(y0, y1));
}
