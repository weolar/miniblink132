...
public:
    ...
    MOCK_METHOD2(SubmitDecode, bool(scoped_refptr<VP8Picture> pic, const Vp8ReferenceFrameVector& reference_frames));
...
AcceleratedVideoDecoder::DecodeResult VP8DecoderTest::Decode(std::string input_frame_file)
{
    ...
    EXPECT_TRUE(result == AcceleratedVideoDecoder::DecodeResult::kRanOutOfStreamData || result == AcceleratedVideoDecoder::DecodeResult::kConfigChange
...
TEST_F(VP8DecoderTest, DecodeMultipleIAndPFrames)
{
    ...
    for (size_t i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(AcceleratedVideoDecoder::kRanOutOfStreamData, Decode((i % 3) == 0 ? kIFrame : kPFrame));
    }
...
TEST_F(VP8DecoderTest, HaveSkippedFramesAtMaxNumOfSizeChangeFailures)
{
    ...
    SkipFrame();
    for (size_t i = 0; i < AcceleratedVideoDecoder::kVPxMaxNumOfSizeChangeFailures; i++) {
        ASSERT_EQ(AcceleratedVideoDecoder::kRanOutOfStreamData, Decode(kPFrame));
    }
