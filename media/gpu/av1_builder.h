...
public:
    ...
    static AV1BitstreamBuilder BuildSequenceHeaderOBU(const SequenceHeader& seq_hdr);
    // Pack frame OBU. Spec 5.9.
    static AV1BitstreamBuilder BuildFrameHeaderOBU(const SequenceHeader& seq_hdr, const FrameHeader& pic_hdr);

    void Write(uint64_t val, int num_bits);
    void WriteBool(bool val);
    // Spec 5.3.2.
    void WriteOBUHeader(libgav1::ObuType type, bool extension_flag, bool has_size);
    // Writes a value encoded in LEB128. Spec 4.10.5.
    void WriteValueInLeb128(uint32_t value, std::optional<int> fixed_size = std::nullopt);
    std::vector<uint8_t> Flush() &&;
    size_t OutstandingBits() const
    {
        return total_outstanding_bits_;
    }
...
struct AV1BitstreamBuilder::FrameHeader {
    ...
    uint32_t feature_data[8 /*libgav1::kMaxSegments*/][8 /*libgav1::kSegmentFeatureMax*/];
};
