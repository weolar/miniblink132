...
x11::Pixmap CreatePixmap(const gfx::Size& size)
{
    ...
    if (!base::CheckedNumeric<int>(size.width()).AssignIfValid(&pixmap_width) || !base::CheckedNumeric<int>(size.height()).AssignIfValid(&pixmap_height)) {
        return x11::Pixmap::None;
    }
    auto req = connection->CreatePixmap({ depth, pixmap, root, pixmap_width, pixmap_height });
    if (req.Sync().error)
...
VaapiPictureNativePixmapAngle::VaapiPictureNativePixmapAngle(scoped_refptr<VaapiWrapper> vaapi_wrapper,
    ...
    const gfx::Size& visible_size, uint32_t service_texture_id, uint32_t client_texture_id, uint32_t texture_target)
    : VaapiPictureNativePixmap(std::move(vaapi_wrapper), make_context_current_cb, bind_image_cb, picture_buffer_id, size, visible_size, service_texture_id,
        client_texture_id, texture_target)
{
...
VaapiStatus VaapiPictureNativePixmapAngle::Allocate(gfx::BufferFormat format)
{
    ...
    auto image = base::MakeRefCounted<gl::GLImageEGLPixmap>(visible_size_, format);
    if (!image)
...
bool VaapiPictureNativePixmapAngle::ImportGpuMemoryBufferHandle(gfx::BufferFormat format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
    NOTREACHED();
    return false;
}

bool VaapiPictureNativePixmapAngle::DownloadFromSurface(scoped_refptr<VASurface> va_surface)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!vaapi_wrapper_->PutSurfaceIntoPixmap(va_surface->id(), x_pixmap_, visible_size_)) {
        return false;
    }
