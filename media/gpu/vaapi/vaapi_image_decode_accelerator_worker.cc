...
VaapiImageDecoder* VaapiImageDecodeAcceleratorWorker::GetInitializedDecoder(const std::vector<uint8_t>& encoded_data)
{
    ...
    const auto uma_cb = base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiImageDecodeAcceleratorWorker.VAAPIError");
    if (IsJpegImage(encoded_data) && base::Contains(decoders_, gpu::ImageDecodeAcceleratorType::kJpeg)) {
        CHECK(base::FeatureList::IsEnabled(features::kVaapiJpegImageDecodeAcceleration));
        if (!decoders_[gpu::ImageDecodeAcceleratorType::kJpeg]->Initialize(uma_cb)) {
            return nullptr;
        }
        return decoders_[gpu::ImageDecodeAcceleratorType::kJpeg].get();
    } else if (IsLossyWebPImage(encoded_data) && base::Contains(decoders_, gpu::ImageDecodeAcceleratorType::kWebP)) {
        CHECK(base::FeatureList::IsEnabled(features::kVaapiWebPImageDecodeAcceleration));
        if (!decoders_[gpu::ImageDecodeAcceleratorType::kWebP]->Initialize(uma_cb)) {
            return nullptr;
        }
...
// static
std::unique_ptr<VaapiImageDecodeAcceleratorWorker> VaapiImageDecodeAcceleratorWorker::Create()
{
    VaapiImageDecoderVector decoders;
    gpu::ImageDecodeAcceleratorSupportedProfiles supported_profiles;

    std::optional<gpu::ImageDecodeAcceleratorSupportedProfile> jpeg_decoder_supported_profile = VaapiJpegDecoder::GetSupportedProfile();
    ...
    std::optional<gpu::ImageDecodeAcceleratorSupportedProfile> webp_decoder_supported_profile = VaapiWebPDecoder::GetSupportedProfile();
    ...
    return base::WrapUnique(new VaapiImageDecodeAcceleratorWorker(std::move(decoders), std::move(supported_profiles)));
}

VaapiImageDecodeAcceleratorWorker::VaapiImageDecodeAcceleratorWorker(
    VaapiImageDecoderVector decoders, gpu::ImageDecodeAcceleratorSupportedProfiles supported_profiles)
...
gpu::ImageDecodeAcceleratorSupportedProfiles VaapiImageDecodeAcceleratorWorker::GetSupportedProfiles()
{
    return supported_profiles_;
}

void VaapiImageDecodeAcceleratorWorker::DecodeTask(
    std::vector<uint8_t> encoded_data, const gfx::Size& output_size_for_tracing, gpu::ImageDecodeAcceleratorWorker::CompletedDecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    TRACE_EVENT2("jpeg", "VaapiImageDecodeAcceleratorWorker::DecodeTask", "encoded_bytes", encoded_data.size(), "output_size_for_tracing",
        output_size_for_tracing.ToString());
    gpu::ImageDecodeAcceleratorWorker::CompletedDecodeCB scoped_decode_callback = mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(decode_cb), nullptr);
    ...
    if (status != VaapiImageDecodeStatus::kSuccess) {
        DVLOGF(1) << "Failed to decode - status = " << static_cast<uint32_t>(status);
        return;
    }

    // Export the decode result as a NativePixmap.
    std::unique_ptr<NativePixmapAndSizeInfo> exported_pixmap = decoder->ExportAsNativePixmapDmaBuf(&status);
    if (status != VaapiImageDecodeStatus::kSuccess) {
        DVLOGF(1) << "Failed to export surface - status = " << static_cast<uint32_t>(status);
        return;
    }
    ...
    gfx::NativePixmapHandle pixmap_handle = exported_pixmap->pixmap->ExportHandle();
    ...
    auto result = std::make_unique<gpu::ImageDecodeAcceleratorWorker::DecodeResult>();
...
void VaapiImageDecodeAcceleratorWorker::Decode(std::vector<uint8_t> encoded_data, const gfx::Size& output_size, CompletedDecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(io_sequence_checker_);
    CHECK(decoder_task_runner_);

    decoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VaapiImageDecodeAcceleratorWorker::DecodeTask, decoder_weak_this_factory_.GetWeakPtr(), std::move(encoded_data), output_size,
            std::move(decode_cb)));
}
...
void default_delete<media::VaapiImageDecodeAcceleratorWorker>::operator()(media::VaapiImageDecodeAcceleratorWorker* ptr) const
{
    ptr->Destroy();
}
