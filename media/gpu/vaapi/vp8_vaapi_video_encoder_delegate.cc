...
libvpx::VP8RateControlRtcConfig CreateRateControlConfig(const gfx::Size encode_size, const VP8VaapiVideoEncoderDelegate::EncodeParams& encode_params,
    const VideoBitrateAllocation& bitrate_allocation, size_t num_temporal_layers)
{
...
scoped_refptr<VP8Picture> GetVP8Picture(const VaapiVideoEncoderDelegate::EncodeJob& job)
{
    return base::WrapRefCounted(reinterpret_cast<VP8Picture*>(job.picture().get()));
}

Vp8FrameHeader GetDefaultVp8FrameHeader(bool keyframe, const gfx::Size& visible_size)
{
    Vp8FrameHeader hdr;
    ...
    hdr.show_frame = true;
    hdr.frame_type = keyframe ? Vp8FrameHeader::KEYFRAME : Vp8FrameHeader::INTERFRAME;
...
bool UpdateFrameHeaderForTemporalLayerEncoding(const size_t num_layers, const size_t frame_num, Vp8FrameHeader& frame_hdr, Vp8Metadata& metadata,
    std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
    ...
    if (keyframe) {
        ...
        constexpr std::pair<Vp8Metadata, std::array<BufferFlags, kNumVp8ReferenceBuffers>> kFrameConfigs[][kTemporalLayerCycle] = {
            {
                // For two temporal layers.
                { { .non_reference = false, .temporal_idx = 0, .layer_sync = false }, { kReferenceAndUpdate, kNone, kNone } },
                { { .non_reference = true, .temporal_idx = 1, .layer_sync = true }, { kReference, kNone, kNone } },
                { { .non_reference = false, .temporal_idx = 0, .layer_sync = false }, { kReferenceAndUpdate, kNone, kNone } },
                { { .non_reference = true, .temporal_idx = 1, .layer_sync = true }, { kReference, kNone, kNone } },
            },
            {
                ...
                { { .non_reference = false, .temporal_idx = 0, .layer_sync = false }, { kReferenceAndUpdate, kNone, kNone } },
                { { .non_reference = true, .temporal_idx = 2, .layer_sync = true }, { kReference, kNone, kNone } },
                { { .non_reference = false, .temporal_idx = 1, .layer_sync = true }, { kReference, kUpdate, kNone } },
                { { .non_reference = true, .temporal_idx = 2, .layer_sync = false }, { kNone, kReference, kNone } },
            },
        };

        std::tie(metadata, buffer_flags) = kFrameConfigs[num_layers - kMinSupportedVP8TemporalLayers][frame_num % kTemporalLayerCycle];
    }

    frame_hdr.frame_type = keyframe ? Vp8FrameHeader::KEYFRAME : Vp8FrameHeader::INTERFRAME;
...
size_t GetActiveTemporalLayers(const VideoBitrateAllocation& bitrate_allocation)
{
    size_t temporal_id = 0;
    while (temporal_id < VideoBitrateAllocation::kMaxTemporalLayers && bitrate_allocation.GetBitrateBps(0, temporal_id) != 0) {
        temporal_id++;
    }
    return temporal_id;
}
...
VP8VaapiVideoEncoderDelegate::VP8VaapiVideoEncoderDelegate(scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure error_cb)
...
bool VP8VaapiVideoEncoderDelegate::Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    if (VideoCodecProfileToVideoCodec(config.output_profile) != VideoCodec::kVP8) {
    ...
    if (config.HasTemporalLayer()) {
        ...
        if (VP8TLEncodingIsEnabled()) {
            ...
            if (num_temporal_layers_ > kMaxSupportedVP8TemporalLayers || num_temporal_layers_ < kMinSupportedVP8TemporalLayers) {
                VLOGF(1) << "Unsupported number of temporal layers: " << base::strict_cast<size_t>(num_temporal_layers_);
                return false;
            }
        } else {
    ...
    if (!rate_ctrl_) {
        rate_ctrl_
            = libvpx::VP8RateControlRTC::Create(CreateRateControlConfig(visible_size_, current_params_, initial_bitrate_allocation, num_temporal_layers_));
        if (!rate_ctrl_)
            return false;
    }
...
VaapiVideoEncoderDelegate::PrepareEncodeJobResult VP8VaapiVideoEncoderDelegate::PrepareEncodeJob(EncodeJob& encode_job)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    std::array<bool, kNumVp8ReferenceBuffers> ref_frames_used;
    if (auto result = SetFrameHeader(frame_num_, *picture, ref_frames_used); result != PrepareEncodeJobResult::kSuccess) {
        return result;
    }

    DCHECK(!picture->frame_hdr->IsKeyframe() || !base::Contains(ref_frames_used, true));
...
BitstreamBufferMetadata VP8VaapiVideoEncoderDelegate::GetMetadata(const EncodeJob& encode_job, size_t payload_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    metadata.qp = base::strict_cast<int32_t>(picture->frame_hdr->quantization_hdr.y_ac_qi);
    return metadata;
}

void VP8VaapiVideoEncoderDelegate::BitrateControlUpdate(const BitstreamBufferMetadata& metadata)
{
    if (!rate_ctrl_) {
    ...
    DVLOGF(4) << "temporal_idx=" << (metadata.vp8 ? metadata.vp8->temporal_idx : 0) << ", encoded chunk size=" << metadata.payload_size_bytes;
    CHECK_NE(metadata.payload_size_bytes, 0u);
    rate_ctrl_->PostEncodeUpdate(metadata.payload_size_bytes);
}

bool VP8VaapiVideoEncoderDelegate::UpdateRates(const VideoBitrateAllocation& bitrate_allocation, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (current_params_.bitrate_allocation == bitrate_allocation && current_params_.framerate == framerate) {
        return true;
    }
    DVLOGF(2) << "New bitrate: " << bitrate_allocation.ToString() << ", new framerate: " << framerate;

    current_params_.bitrate_allocation = bitrate_allocation;
    current_params_.framerate = framerate;

    if (VP8TLEncodingIsEnabled()) {
        const size_t new_num_temporal_layers = GetActiveTemporalLayers(bitrate_allocation);
        if (new_num_temporal_layers != num_temporal_layers_) {
            VLOGF(2) << "The number of temporal layers is changed, from " << base::strict_cast<int>(num_temporal_layers_) << " to " << new_num_temporal_layers;
            num_temporal_layers_ = base::checked_cast<uint8_t>(new_num_temporal_layers);
            static_assert(std::has_single_bit(kTemporalLayerCycle), "temporal layer cycle must be power of two");
    ...
    rate_ctrl_->UpdateRateControl(CreateRateControlConfig(visible_size_, current_params_, bitrate_allocation, num_temporal_layers_));
    return true;
}

VaapiVideoEncoderDelegate::PrepareEncodeJobResult VP8VaapiVideoEncoderDelegate::SetFrameHeader(
    size_t frame_num, VP8Picture& picture, std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    const bool keyframe = frame_num == 0;
    ...
    *picture.frame_hdr = GetDefaultVp8FrameHeader(keyframe, visible_size_);
    if (num_temporal_layers_ > 1) {
        UpdateFrameHeaderForTemporalLayerEncoding(
            num_temporal_layers_, frame_num, *picture.frame_hdr, picture.metadata_for_encoding.emplace(), ref_frames_used);
    } else {
        ...
        if (keyframe) {
            ...
            picture.frame_hdr->copy_buffer_to_golden = Vp8FrameHeader::NO_GOLDEN_REFRESH;
            picture.frame_hdr->copy_buffer_to_alternate = Vp8FrameHeader::NO_ALT_REFRESH;
            ...
            picture.frame_hdr->copy_buffer_to_golden = Vp8FrameHeader::COPY_LAST_TO_GOLDEN;
            picture.frame_hdr->copy_buffer_to_alternate = Vp8FrameHeader::COPY_GOLDEN_TO_ALT;
    ...
    frame_params.temporal_layer_id = picture.metadata_for_encoding.has_value() ? picture.metadata_for_encoding->temporal_idx : 0;
    ...
    picture.frame_hdr->quantization_hdr.y_ac_qi = base::checked_cast<uint8_t>(rate_ctrl_->GetQP());
    libvpx::UVDeltaQP uv_delta_qp = rate_ctrl_->GetUVDeltaQP();
    picture.frame_hdr->quantization_hdr.uv_dc_delta = base::checked_cast<int8_t>(uv_delta_qp.uvdc_delta_q);
    picture.frame_hdr->quantization_hdr.uv_ac_delta = base::checked_cast<int8_t>(uv_delta_qp.uvac_delta_q);
    picture.frame_hdr->loopfilter_hdr.level = base::checked_cast<uint8_t>(rate_ctrl_->GetLoopfilterLevel());

    DVLOGF(4) << "qp=" << static_cast<int>(picture.frame_hdr->quantization_hdr.y_ac_qi)
              << ", uv_dc_delta=" << static_cast<int>(picture.frame_hdr->quantization_hdr.uv_dc_delta)
              << ", uv_ac_delta=" << static_cast<int>(picture.frame_hdr->quantization_hdr.uv_ac_delta)
              << ", filter_level=" << static_cast<int>(picture.frame_hdr->loopfilter_hdr.level) << (keyframe ? " (keyframe)" : "")
              << (picture.metadata_for_encoding ? " temporal id=" + base::NumberToString(frame_params.temporal_layer_id) : "");
    return PrepareEncodeJobResult::kSuccess;
}

void VP8VaapiVideoEncoderDelegate::UpdateReferenceFrames(scoped_refptr<VP8Picture> picture)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool VP8VaapiVideoEncoderDelegate::SubmitFrameParameters(EncodeJob& job, const EncodeParams& encode_params, scoped_refptr<VP8Picture> pic,
    const Vp8ReferenceFrameVector& ref_frames, const std::array<bool, kNumVp8ReferenceBuffers>& ref_frames_used)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    pic_param.ref_last_frame = last_frame ? last_frame->AsVaapiVP8Picture()->va_surface_id() : VA_INVALID_ID;
    auto golden_frame = ref_frames.GetFrame(Vp8RefType::VP8_FRAME_GOLDEN);
    pic_param.ref_gf_frame = golden_frame ? golden_frame->AsVaapiVP8Picture()->va_surface_id() : VA_INVALID_ID;
    auto alt_frame = ref_frames.GetFrame(Vp8RefType::VP8_FRAME_ALTREF);
    pic_param.ref_arf_frame = alt_frame ? alt_frame->AsVaapiVP8Picture()->va_surface_id() : VA_INVALID_ID;
    pic_param.coded_buf = job.coded_buffer_id();
    DCHECK_NE(pic_param.coded_buf, VA_INVALID_ID);
    pic_param.ref_flags.bits.no_ref_last = !ref_frames_used[Vp8RefType::VP8_FRAME_LAST];
    pic_param.ref_flags.bits.no_ref_gf = !ref_frames_used[Vp8RefType::VP8_FRAME_GOLDEN];
    pic_param.ref_flags.bits.no_ref_arf = !ref_frames_used[Vp8RefType::VP8_FRAME_ALTREF];
    ...
    pic_param.pic_flags.bits.num_token_partitions = frame_header->num_of_dct_partitions;
    pic_param.pic_flags.bits.segmentation_enabled = frame_header->segmentation_hdr.segmentation_enabled;
    pic_param.pic_flags.bits.update_mb_segmentation_map = frame_header->segmentation_hdr.update_mb_segmentation_map;
    pic_param.pic_flags.bits.update_segment_feature_data = frame_header->segmentation_hdr.update_segment_feature_data;

    pic_param.pic_flags.bits.loop_filter_adj_enable = frame_header->loopfilter_hdr.loop_filter_adj_enable;

    pic_param.pic_flags.bits.refresh_entropy_probs = !encode_params.error_resilient_mode;
    pic_param.pic_flags.bits.refresh_golden_frame = frame_header->refresh_golden_frame;
    pic_param.pic_flags.bits.refresh_alternate_frame = frame_header->refresh_alternate_frame;
    pic_param.pic_flags.bits.refresh_last = frame_header->refresh_last;
    pic_param.pic_flags.bits.copy_buffer_to_golden = frame_header->copy_buffer_to_golden;
    pic_param.pic_flags.bits.copy_buffer_to_alternate = frame_header->copy_buffer_to_alternate;
    pic_param.pic_flags.bits.sign_bias_golden = frame_header->sign_bias_golden;
    pic_param.pic_flags.bits.sign_bias_alternate = frame_header->sign_bias_alternate;
    ...
    static_assert(std::extent<decltype(pic_param.loop_filter_level)>() == std::extent<decltype(pic_param.ref_lf_delta)>()
            && std::extent<decltype(pic_param.ref_lf_delta)>() == std::extent<decltype(pic_param.mode_lf_delta)>()
            && std::extent<decltype(pic_param.ref_lf_delta)>() == std::extent<decltype(frame_header->loopfilter_hdr.ref_frame_delta)>()
            && std::extent<decltype(pic_param.mode_lf_delta)>() == std::extent<decltype(frame_header->loopfilter_hdr.mb_mode_delta)>(),
    ...
    qmatrix_buf.quantization_index_delta[0] = frame_header->quantization_hdr.y_dc_delta;
    qmatrix_buf.quantization_index_delta[1] = frame_header->quantization_hdr.y2_dc_delta;
    qmatrix_buf.quantization_index_delta[2] = frame_header->quantization_hdr.y2_ac_delta;
    qmatrix_buf.quantization_index_delta[3] = frame_header->quantization_hdr.uv_dc_delta;
    qmatrix_buf.quantization_index_delta[4] = frame_header->quantization_hdr.uv_ac_delta;

    return vaapi_wrapper_->SubmitBuffers({ { VAEncSequenceParameterBufferType, sizeof(seq_param), &seq_param },
        { VAEncPictureParameterBufferType, sizeof(pic_param), &pic_param }, { VAQMatrixBufferType, sizeof(qmatrix_buf), &qmatrix_buf } });
}
} // namespace media
