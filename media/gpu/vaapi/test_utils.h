...
static_assert(std::size(VAImage().pitches) == std::size(VAImage().offsets), "The number of VAImage pitches is not equal to the number of offsets");
...
// I420 format.
bool CompareImages(const DecodedImage& reference_image, const DecodedImage& hw_decoded_image, double min_ssim = 0.995);
