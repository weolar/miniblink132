...
std::optional<VAProfile> ConvertToVAProfile(VideoCodecProfile profile)
{
    ...
    return it != kProfileMap.end() ? std::make_optional<VAProfile>(it->second) : std::nullopt;
}
...
std::optional<VAProfile> StringToVAProfile(const std::string& va_profile)
{
    ...
    return it != kStringToVAProfile.end() ? std::make_optional<VAProfile>(it->second) : std::nullopt;
}

// Converts the given string to VAEntrypoint
std::optional<VAEntrypoint> StringToVAEntrypoint(const std::string& va_entrypoint)
{
    ...
    return it != kStringToVAEntrypoint.end() ? std::make_optional<VAEntrypoint>(it->second) : std::nullopt;
}
...
bool IsDisplayVer14()
{
    ...
    return cpuid.family() == kPentiumAndLaterFamily && (cpuid.model() == kMeteorLakeModelId || cpuid.model() == kMeteorLake_LModelId);
}
...
std::map<VAProfile, std::vector<VAEntrypoint>> ParseVainfo(const std::string& output)
{
    const std::vector<std::string> lines = base::SplitString(output, "\n", base::WhitespaceHandling::TRIM_WHITESPACE, base::SplitResult::SPLIT_WANT_ALL);
    std::map<VAProfile, std::vector<VAEntrypoint>> info;
    for (const std::string& line : lines) {
        if (!base::StartsWith(line, "VAProfile", base::CompareCase::INSENSITIVE_ASCII)) {
            continue;
        }
        std::vector<std::string> res = base::SplitString(line, ":", base::WhitespaceHandling::TRIM_WHITESPACE, base::SplitResult::SPLIT_WANT_ALL);
        if (res.size() != 2) {
...
TEST_F(VaapiTest, GetSupportedDecodeProfiles)
{
    ...
    for (const auto& profile : VaapiWrapper::GetSupportedDecodeProfiles()) {
        ...
        EXPECT_TRUE(base::Contains(va_info.at(*va_profile), VAEntrypointVLD))
            << " profile: " << GetProfileName(profile.profile) << ", va profile: " << vaProfileStr(*va_profile);
...
TEST_F(VaapiTest, GetSupportedEncodeProfiles)
{
    ...
    for (const auto& profile : VaapiWrapper::GetSupportedEncodeProfiles()) {
        ...
        EXPECT_TRUE(base::Contains(va_info.at(*va_profile), VAEntrypointEncSlice) || base::Contains(va_info.at(*va_profile), VAEntrypointEncSliceLP))
            << " profile: " << GetProfileName(profile.profile) << ", va profile: " << vaProfileStr(*va_profile);
    }
}
...
TEST_F(VaapiTest, VbrAndCbrResolutionsMatch)
{
    ...
    for (const VaapiWrapper::CodecMode codec_mode :
        { VaapiWrapper::kEncodeConstantBitrate, VaapiWrapper::kEncodeConstantQuantizationParameter, VaapiWrapper::kEncodeVariableBitrate }) {
        const std::map<VAProfile, std::vector<VAEntrypoint>> configurations = VaapiWrapper::GetSupportedConfigurationsForCodecModeForTesting(codec_mode);
        for (const auto& configuration : configurations) {
            const VAProfile va_profile = configuration.first;
            ResolutionInfo res_info { .mode = codec_mode };
            ASSERT_TRUE(VaapiWrapper::GetSupportedResolutions(va_profile, codec_mode, res_info.min, res_info.max))
        ...
        for (size_t i = 0; i < resolution_info.size(); ++i) {
            for (size_t j = i + 1; j < resolution_info.size(); ++j) {
                EXPECT_EQ(resolution_info[i].min, resolution_info[j].min)
                    << " Minimum supported resolution mismatch for profile=" << VAProfileToString(va_profile) << ": " << resolution_info[i].mode << " ("
                    << resolution_info[i].min.ToString() << ") and " << resolution_info[j].mode << " (" << resolution_info[j].min.ToString() << ")";
                EXPECT_EQ(resolution_info[i].max, resolution_info[j].max)
                    << " Maximum supported resolution mismatch for profile=" << VAProfileToString(va_profile) << ": " << resolution_info[i].mode << " ("
                    << resolution_info[i].max.ToString() << ") and " << resolution_info[j].mode << " (" << resolution_info[j].max.ToString() << ")";
            }
        }
    }
}
...
TEST_F(VaapiTest, VaapiProfileProtected)
{
    ...
    if (impl == VAImplementation::kIntelIHD) {
        ...
        EXPECT_TRUE(base::Contains(va_info.at(VAProfileProtected), VAEntrypointProtectedContent)) << ", va profile: " << vaProfileStr(VAProfileProtected);
    } else {
...
TEST_F(VaapiTest, VaapiProfilesJPEG)
{
    ...
    EXPECT_EQ(VaapiWrapper::IsDecodeSupported(VAProfileJPEGBaseline), base::Contains(va_info.at(VAProfileJPEGBaseline), VAEntrypointVLD));
    EXPECT_EQ(VaapiWrapper::IsJpegEncodeSupported(), base::Contains(va_info.at(VAProfileJPEGBaseline), VAEntrypointEncPicture));
}
...
TEST_F(VaapiTest, DefaultEntrypointIsSupported)
{
    for (size_t i = 0; i < VaapiWrapper::kCodecModeMax; ++i) {
        ...
        std::map<VAProfile, std::vector<VAEntrypoint>> configurations = VaapiWrapper::GetSupportedConfigurationsForCodecModeForTesting(wrapper_mode);
        for (const auto& profile_and_entrypoints : configurations) {
            const VAEntrypoint default_entrypoint = VaapiWrapper::GetDefaultVaEntryPoint(wrapper_mode, profile_and_entrypoints.first);
            ...
            EXPECT_TRUE(base::Contains(supported_entrypoints, default_entrypoint))
                << "Default VAEntrypoint " << vaEntrypointStr(default_entrypoint) << " (VaapiWrapper mode = " << wrapper_mode << ") is not supported for "
...
{
    std::map<VAProfile, std::vector<VAEntrypoint>> configurations = VaapiWrapper::GetSupportedConfigurationsForCodecModeForTesting(VaapiWrapper::kDecode);
    ...
    auto wrapper_or_error = VaapiWrapper::Create(VaapiWrapper::kDecode, kUnsupportedVAProfile, EncryptionScheme::kUnencrypted, base::DoNothing());
    ASSERT_FALSE(wrapper_or_error.has_value());
    ASSERT_EQ(wrapper_or_error.error(), DecoderStatus::Codes::kUnsupportedProfile);
}
...
TEST_F(VaapiTest, TooManyDecoderInstances)
{
    ...
    for (auto& wrapper : vaapi_wrappers) {
        auto wrapper_or_error = VaapiWrapper::Create(VaapiWrapper::kDecode, kVAProfile, EncryptionScheme::kUnencrypted, base::DoNothing());
...
TEST_F(VaapiTest, EncryptionSchemeNeedsCodecMode)
{
    ...
    auto wrapper_or_error = VaapiWrapper::Create(VaapiWrapper::kDecode, kVAProfile, EncryptionScheme::kCenc, base::DoNothing());
    ASSERT_FALSE(wrapper_or_error.has_value());
...
TEST_F(VaapiTest, LowQualityEncodingSetting)
{
    ...
    const bool is_low_power_intel = cpuid.family() == kPentiumAndLaterFamily
    ...
    for (const auto& codec_mode : { VaapiWrapper::kEncodeConstantBitrate, VaapiWrapper::kEncodeConstantQuantizationParameter }) {
        std::map<VAProfile, std::vector<VAEntrypoint>> configurations = VaapiWrapper::GetSupportedConfigurationsForCodecModeForTesting(codec_mode);

        for (const auto& profile_and_entrypoints : configurations) {
            const VAProfile va_profile = profile_and_entrypoints.first;
            scoped_refptr<VaapiWrapper> wrapper
                = VaapiWrapper::Create(VaapiWrapper::kEncodeConstantBitrate, va_profile, EncryptionScheme::kUnencrypted, base::DoNothing()).value_or(nullptr);
            ...
            const VAEntrypoint entrypoint = VaapiWrapper::GetDefaultVaEntryPoint(codec_mode, va_profile);
            ...
            attrib.type = VAConfigAttribEncQualityRange;
            {
                ...
                VAStatus va_res = vaGetConfigAttributes(wrapper->va_display_, va_profile, entrypoint, &attrib, 1);
            ...
            if (quality_level == VA_ATTRIB_NOT_SUPPORTED || quality_level <= 1u)
                continue;
            DLOG(INFO) << vaProfileStr(va_profile) << " supports encoding quality setting, with max value " << quality_level;
            ...
            ASSERT_EQ(wrapper->pending_va_buffers_.size(), 1u);
            {
                ...
                auto mapping = ScopedVABufferMapping::Create(wrapper->va_lock_, wrapper->va_display_, wrapper->pending_va_buffers_.front());
                ASSERT_TRUE(mapping);
                auto* const va_buffer = reinterpret_cast<VAEncMiscParameterBuffer*>(mapping->data());
                ...
                EXPECT_EQ(enc_quality->quality_level, quality_level) << vaProfileStr(va_profile) << " " << vaEntrypointStr(entrypoint);
            }
        }
    }
}
...
{
    const std::vector<SVCScalabilityMode> kSupportedL1T1 = { SVCScalabilityMode::kL1T1 };
#if BUILDFLAG(IS_CHROMEOS)
    const std::vector<SVCScalabilityMode> kSupportedTemporalSVC = { SVCScalabilityMode::kL1T1, SVCScalabilityMode::kL1T2, SVCScalabilityMode::kL1T3 };
    const std::vector<SVCScalabilityMode> kSupportedTemporalAndKeySVC = { SVCScalabilityMode::kL1T1, SVCScalabilityMode::kL1T2, SVCScalabilityMode::kL1T3,
        SVCScalabilityMode::kL2T2Key, SVCScalabilityMode::kL2T3Key, SVCScalabilityMode::kL3T2Key, SVCScalabilityMode::kL3T3Key, SVCScalabilityMode::kS2T1,
        SVCScalabilityMode::kS2T2, SVCScalabilityMode::kS2T3, SVCScalabilityMode::kS3T1, SVCScalabilityMode::kS3T2, SVCScalabilityMode::kS3T3 };
#endif

    const auto scalability_modes_vp9_profile0 = VaapiWrapper::GetSupportedScalabilityModes(VP9PROFILE_PROFILE0, VAProfileVP9Profile0);
#if BUILDFLAG(IS_CHROMEOS)
    const VAEntrypoint vp9_cqp_enc_va_entry_point
        = VaapiWrapper::GetDefaultVaEntryPoint(VaapiWrapper::kEncodeConstantQuantizationParameter, VAProfileVP9Profile0);
    if (vp9_cqp_enc_va_entry_point == VAEntrypointEncSliceLP || vp9_cqp_enc_va_entry_point == VAEntrypointEncSlice) {
    ...
    const auto scalability_modes_vp9_profile2 = VaapiWrapper::GetSupportedScalabilityModes(VP9PROFILE_PROFILE2, VAProfileVP9Profile2);
    EXPECT_EQ(scalability_modes_vp9_profile2, kSupportedL1T1);

    const auto scalability_modes_vp8 = VaapiWrapper::GetSupportedScalabilityModes(VP8PROFILE_ANY, VAProfileVP8Version0_3);
#if BUILDFLAG(IS_CHROMEOS)
    ...
    const auto scalability_modes_h264_baseline = VaapiWrapper::GetSupportedScalabilityModes(H264PROFILE_BASELINE, VAProfileH264ConstrainedBaseline);
#if BUILDFLAG(IS_CHROMEOS)
...
class VaapiVppTest : public VaapiTest, public testing::WithParamInterface<std::tuple<uint32_t, uint32_t>> {
public:
    ...
    struct PrintToStringParamName {
        ...
        {
            std::stringstream ss;
            ss << FourccToString(std::get<0>(info.param)) << "_to_" << FourccToString(std::get<1>(info.param));
            return ss.str();
        }
    };
};
...
TEST_P(VaapiVppTest, BlitWithVAAllocatedSurfaces)
{
    ...
    if (!VaapiWrapper::IsVppFormatSupported(va_fourcc_in) || !VaapiWrapper::IsVppFormatSupported(va_fourcc_out)) {
        GTEST_SKIP() << FourccToString(va_fourcc_in) << " -> " << FourccToString(va_fourcc_out) << " not supported";
    }
    ...
    auto wrapper = VaapiWrapper::Create(VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted, base::DoNothing()).value_or(nullptr);
    ASSERT_TRUE(!!wrapper);
    ...
    auto scoped_surfaces = wrapper->CreateScopedVASurfaces(
        va_rt_format_in, kInputSize, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u, /*visible_size=*/std::nullopt, /*va_fourcc=*/std::nullopt);
    ASSERT_FALSE(scoped_surfaces.empty());
    std::unique_ptr<ScopedVASurface> scoped_surface_in = std::move(scoped_surfaces[0]);

    scoped_surfaces = wrapper->CreateScopedVASurfaces(
        va_rt_format_out, kOutputSize, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u, /*visible_size=*/std::nullopt, /*va_fourcc=*/std::nullopt);
    ASSERT_FALSE(scoped_surfaces.empty());
    std::unique_ptr<ScopedVASurface> scoped_surface_out = std::move(scoped_surfaces[0]);

    ASSERT_TRUE(
        wrapper->BlitSurface(scoped_surface_in->id(), kInputSize, scoped_surface_out->id(), kOutputSize, gfx::Rect(kInputSize), gfx::Rect(kOutputSize)));
...
constexpr uint32_t kVAFourCCs[] = { VA_FOURCC_I420, VA_FOURCC_YUY2, VA_FOURCC_RGBA, VA_FOURCC_P010 };

INSTANTIATE_TEST_SUITE_P(
    , VaapiVppTest, ::testing::Combine(::testing::ValuesIn(kVAFourCCs), ::testing::ValuesIn(kVAFourCCs)), VaapiVppTest::PrintToStringParamName());

class VaapiMinigbmTest : public VaapiTest, public testing::WithParamInterface<std::tuple<VAProfile, unsigned int /*va_rt_format*/, gfx::Size>> {
public:
    ...
    struct PrintToStringParamName {
        template <class ParamType> std::string operator()(const testing::TestParamInfo<ParamType>& info) const
        {
            ...
            return base::StringPrintf("%s__%s__%s", VAProfileToString(std::get<0>(info.param)), VARTFormatToString(std::get<1>(info.param)).c_str(),
...
TEST_P(VaapiMinigbmTest, AllocateAndCompareWithMinigbm)
{
    ...
    if (!(backend == VAImplementation::kIntelIHD || backend == VAImplementation::kMesaGallium)) {
        GTEST_SKIP() << "backend not supported";
    }
    ...
    if (!VaapiWrapper::IsDecodingSupportedForInternalFormat(va_profile, va_rt_format)) {
        GTEST_SKIP() << VARTFormatToString(va_rt_format) << " not supported.";
    }
    // TODO(b/200817282): Fix high-bit depth formats on AMD Gallium impl.
    if (backend == VAImplementation::kMesaGallium && va_rt_format == VA_RT_FORMAT_YUV420_10) {
        GTEST_SKIP() << vaProfileStr(va_profile) << " fails on AMD, skipping.";
    }

    gfx::Size minimum_supported_size;
    gfx::Size maximum_supported_size;
    ASSERT_TRUE(VaapiWrapper::GetSupportedResolutions(va_profile, VaapiWrapper::CodecMode::kDecode, minimum_supported_size, maximum_supported_size));
    if (resolution.width() < minimum_supported_size.width() || resolution.height() < minimum_supported_size.height()
        || resolution.width() > maximum_supported_size.width() || resolution.height() > maximum_supported_size.height()) {
        GTEST_SKIP() << resolution.ToString() << " not supported (min: " << minimum_supported_size.ToString() << ", max: " << maximum_supported_size.ToString()
                     << ")";
    }

    auto wrapper = VaapiWrapper::Create(VaapiWrapper::kDecode, va_profile, EncryptionScheme::kUnencrypted, base::DoNothing()).value_or(nullptr);
    ASSERT_TRUE(!!wrapper);
    ASSERT_TRUE(wrapper->CreateContext(resolution));

    auto scoped_surfaces = wrapper->CreateScopedVASurfaces(va_rt_format, resolution, { VaapiWrapper::SurfaceUsageHint::kVideoDecoder }, 1u,
    ...
    VADRMPRIMESurfaceDescriptor va_descriptor {};
    {
        VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(wrapper->sequence_checker_);
        base::AutoLockMaybe auto_lock(wrapper->va_lock_.get());
        VAStatus va_res = vaSyncSurface(wrapper->va_display_, scoped_va_surface->id());
        ASSERT_EQ(va_res, VA_STATUS_SUCCESS);
        va_res = vaExportSurfaceHandle(wrapper->va_display_, scoped_va_surface->id(), VA_SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
            VA_EXPORT_SURFACE_READ_ONLY | VA_EXPORT_SURFACE_SEPARATE_LAYERS, &va_descriptor);
        ASSERT_EQ(va_res, VA_STATUS_SUCCESS);
    }
    ...
    EXPECT_EQ(scoped_va_surface->size(), gfx::Size(base::checked_cast<int>(va_descriptor.width), base::checked_cast<int>(va_descriptor.height)));

    const auto va_fourcc = ToVaFourcc(va_rt_format);
    ASSERT_NE(va_fourcc, base::checked_cast<unsigned int>(DRM_FORMAT_INVALID));
    EXPECT_EQ(va_descriptor.fourcc, va_fourcc) << FourccToString(va_descriptor.fourcc) << " != " << FourccToString(va_fourcc);
    ...
    EXPECT_GE(va_descriptor.objects[0].size, base::checked_cast<uint32_t>(scoped_va_surface->size().GetArea()));
    if (va_descriptor.num_objects == 2) {
        ...
        EXPECT_GE(va_descriptor.objects[1].size, base::checked_cast<uint32_t>(2 * uv_width * uv_height));
    }

    VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(wrapper->sequence_checker_);
    base::AutoLockMaybe auto_lock(wrapper->va_lock_.get());
    const std::string va_vendor_string = vaQueryVendorString(wrapper->va_display_);
    uint64_t expected_drm_modifier = DRM_FORMAT_MOD_LINEAR;

    if (backend == VAImplementation::kIntelIHD) {
        expected_drm_modifier = IsDisplayVer14() ? I915_FORMAT_MOD_4_TILED : I915_FORMAT_MOD_Y_TILED;
    ...
    EXPECT_EQ(va_descriptor.objects[0].drm_format_modifier, expected_drm_modifier);
    ...
    for (uint32_t i = 0; i < va_descriptor.num_layers; ++i) {
        ...
        const uint32_t expected_object_index = (va_descriptor.num_objects == 1) ? 0 : i;
        EXPECT_EQ(va_descriptor.layers[i].object_index[0], expected_object_index);

        DVLOG(2) << "plane " << i << ", pitch: " << va_descriptor.layers[i].pitch[0];
        // Luma and chroma planes have different |pitch| expectations.
        // TODO(mcasas): consider bitdepth for pitch lower thresholds.
        if (i == 0) {
            EXPECT_GE(va_descriptor.layers[i].pitch[0], base::checked_cast<uint32_t>(scoped_va_surface->size().width()));
        } else {
            const auto expected_rounded_up_pitch = base::bits::AlignUpDeprecatedDoNotUse(scoped_va_surface->size().width(), 2);
            EXPECT_GE(va_descriptor.layers[i].pitch[0], base::checked_cast<uint32_t>(expected_rounded_up_pitch));
        }
    }
    ...
    for (int i = 0;; i++) {
        base::FilePath dev_path(FILE_PATH_LITERAL(base::StringPrintf(kPrimaryNodeFilePattern, i).c_str()));
        drm_fd = base::File(dev_path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WRITE);
    ...
    struct gbm_bo* bo = gbm_bo_create(gbm, resolution.width(), resolution.height(), gbm_format, bo_use_flags | GBM_BO_USE_SCANOUT);
    if (!bo) {
        // Try again without the scanout flag. This reproduces Chrome's behaviour.
        bo = gbm_bo_create(gbm, resolution.width(), resolution.height(), gbm_format, bo_use_flags);
    }
    ASSERT_TRUE(bo);
    EXPECT_EQ(scoped_va_surface->size(), gfx::Size(base::checked_cast<int>(gbm_bo_get_width(bo)), base::checked_cast<int>(gbm_bo_get_height(bo))));

    const int bo_num_planes = gbm_bo_get_plane_count(bo);
    ASSERT_EQ(va_descriptor.num_layers, base::checked_cast<uint32_t>(bo_num_planes));
    for (int i = 0; i < bo_num_planes; ++i) {
        EXPECT_EQ(va_descriptor.layers[i].pitch[0], gbm_bo_get_stride_for_plane(bo, i));
    }
...
constexpr VAProfile kVACodecProfiles[]
    = { VAProfileVP8Version0_3, VAProfileH264ConstrainedBaseline, VAProfileVP9Profile0, VAProfileVP9Profile2, VAProfileAV1Profile0, VAProfileJPEGBaseline };
constexpr uint32_t kVARTFormatsForGBM[] = { VA_RT_FORMAT_YUV420, VA_RT_FORMAT_YUV420_10 };
...
INSTANTIATE_TEST_SUITE_P(, VaapiMinigbmTest,
    ::testing::Combine(::testing::ValuesIn(kVACodecProfiles), ::testing::ValuesIn(kVARTFormatsForGBM), ::testing::ValuesIn(kResolutions)),
...
int main(int argc, char** argv)
{
    ...
    return base::LaunchUnitTests(argc, argv, base::BindOnce(&base::TestSuite::Run, base::Unretained(&test_suite)));
}
