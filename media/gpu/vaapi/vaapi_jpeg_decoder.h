...
public:
    ...
    std::unique_ptr<ScopedVAImage> GetImage(uint32_t preferred_image_fourcc, VaapiImageDecodeStatus* status);

    // Returns the supported image profile.
    static std::optional<gpu::ImageDecodeAcceleratorSupportedProfile> GetSupportedProfile();

private:
    friend class fuzzing::VaapiJpegDecoderWrapper;

    // VaapiImageDecoder implementation.
    VaapiImageDecodeStatus AllocateVASurfaceAndSubmitVABuffers(base::span<const uint8_t> encoded_image) override;
