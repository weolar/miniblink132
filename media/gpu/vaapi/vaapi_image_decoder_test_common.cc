...
VaapiImageDecoderTestCommon::VaapiImageDecoderTestCommon(std::unique_ptr<VaapiImageDecoder> decoder)
...
{
    ASSERT_TRUE(decoder_->Initialize(base::BindRepeating([](VaapiFunctions function) { LOG(FATAL) << "Oh noes! Decoder failed"; })));
}

base::FilePath VaapiImageDecoderTestCommon::FindTestDataFilePath(const std::string& file_name) const
{
