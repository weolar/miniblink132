...
VaapiTFPPicture::VaapiTFPPicture(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb,
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (format != gfx::BufferFormat::BGRX_8888 && format != gfx::BufferFormat::BGRA_8888 && format != gfx::BufferFormat::RGBX_8888) {
    ...
    if (!base::CheckedNumeric<int>(size_.width()).AssignIfValid(&pixmap_width) || !base::CheckedNumeric<int>(size_.height()).AssignIfValid(&pixmap_height)) {
        return VaapiStatus::Codes::kNoPixmap;
    }
    auto req = connection_->CreatePixmap({ depth, pixmap, root, pixmap_width, pixmap_height });
    if (req.Sync().error) {
...
bool VaapiTFPPicture::ImportGpuMemoryBufferHandle(gfx::BufferFormat format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return vaapi_wrapper_->PutSurfaceIntoPixmap(va_surface->id(), x_pixmap_, va_surface->size());
}
