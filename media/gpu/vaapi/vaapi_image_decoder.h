...
using ScopedVAContextAndSurface = std::unique_ptr<ScopedVASurface, VAContextAndScopedVASurfaceDeleter>;
...
public:
    ...
    virtual VaapiImageDecodeStatus Decode(base::span<const uint8_t> encoded_image);
    ...
    virtual std::unique_ptr<NativePixmapAndSizeInfo> ExportAsNativePixmapDmaBuf(VaapiImageDecodeStatus* status);
...
protected:
    ...
    ScopedVAContextAndSurface scoped_va_context_and_surface_ GUARDED_BY_CONTEXT(decoder_sequence_checker_);

    scoped_refptr<VaapiWrapper> vaapi_wrapper_ GUARDED_BY_CONTEXT(decoder_sequence_checker_);
...
private:
    ...
    virtual VaapiImageDecodeStatus AllocateVASurfaceAndSubmitVABuffers(base::span<const uint8_t> encoded_image) = 0;
