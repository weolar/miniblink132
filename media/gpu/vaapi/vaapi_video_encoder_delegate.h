...
public:
    ...
    class EncodeResult {
    public:
        EncodeResult(std::unique_ptr<ScopedVABuffer> coded_buffer, const BitstreamBufferMetadata& metadata);
        ~EncodeResult();
        ...
        bool IsFrameDropped() const
        {
            return !coded_buffer_;
        }
    ...
    public:
        ...
        EncodeJob(bool keyframe, base::TimeDelta timestamp, uint8_t spatial_index, bool end_of_picture, VASurfaceID input_surface_id,
            scoped_refptr<CodecPicture> picture, std::unique_ptr<ScopedVABuffer> coded_buffer);
    ...
    VaapiVideoEncoderDelegate(scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure error_cb);
    ...
    virtual bool Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config) = 0;

    // Updates current framerate and/or bitrate to |framerate| in FPS
    // and the specified video bitrate allocation.
    virtual bool UpdateRates(const VideoBitrateAllocation& bitrate_allocation, uint32_t framerate) = 0;
    ...
    // on failure.
    std::optional<EncodeResult> GetEncodeResult(std::unique_ptr<EncodeJob> encode_job);
...
private:
    virtual BitstreamBufferMetadata GetMetadata(const EncodeJob& encode_job, size_t payload_size) = 0;
    ...
    // and AV1.
    virtual void BitrateControlUpdate(const BitstreamBufferMetadata& metadata) = 0;
};
} // namespace media
