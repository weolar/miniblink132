...
VaapiVideoEncoderDelegate::EncodeJob::EncodeJob(bool keyframe, base::TimeDelta timestamp, uint8_t spatial_index, bool end_of_picture,
    VASurfaceID input_surface_id, scoped_refptr<CodecPicture> picture, std::unique_ptr<ScopedVABuffer> coded_buffer)
    : keyframe_(keyframe)
...
VaapiVideoEncoderDelegate::EncodeResult VaapiVideoEncoderDelegate::EncodeJob::CreateEncodeResult(const BitstreamBufferMetadata& metadata) &&
{
...
VaapiVideoEncoderDelegate::EncodeResult::EncodeResult(std::unique_ptr<ScopedVABuffer> coded_buffer, const BitstreamBufferMetadata& metadata)
    : coded_buffer_(std::move(coded_buffer))
    , metadata_(metadata)
{
}
...
VaapiVideoEncoderDelegate::EncodeResult& VaapiVideoEncoderDelegate::EncodeResult::operator=(EncodeResult&&) = default;
...
VaapiVideoEncoderDelegate::VaapiVideoEncoderDelegate(scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure error_cb)
...
void VaapiVideoEncoderDelegate::BitrateControlUpdate(const BitstreamBufferMetadata& metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
bool VaapiVideoEncoderDelegate::Encode(EncodeJob& encode_job)
{
    ...
    if (!vaapi_wrapper_->ExecuteAndDestroyPendingBuffers(encode_job.input_surface_id())) {
...
std::optional<VaapiVideoEncoderDelegate::EncodeResult> VaapiVideoEncoderDelegate::GetEncodeResult(std::unique_ptr<EncodeJob> encode_job)
{
    TRACE_EVENT0("media,gpu", "VAVEDelegate::GetEncodeResult");
    if (encode_job->IsFrameDropped()) {
        return std::make_optional<EncodeResult>(
            nullptr, BitstreamBufferMetadata::CreateForDropFrame(encode_job->timestamp(), encode_job->spatial_index(), encode_job->end_of_picture()));
    }

    const VASurfaceID va_surface_id = encode_job->input_surface_id();
    const uint64_t encoded_chunk_size = vaapi_wrapper_->GetEncodedChunkSize(encode_job->coded_buffer_id(), va_surface_id);
    ...
    return std::make_optional<EncodeResult>(std::move(*encode_job).CreateEncodeResult(metadata));
}
