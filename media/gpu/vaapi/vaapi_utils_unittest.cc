...
protected:
    ...
    void SetUp() override
    {
        ...
        vaapi_wrapper_ = VaapiWrapper::Create(
            VaapiWrapper::kDecode, VAProfileJPEGBaseline, EncryptionScheme::kUnencrypted, base::BindRepeating([](VaapiFunctions function) {
...
TEST_F(VaapiUtilsTest, ScopedVABuffer)
{
    ...
    for (size_t i = 0; i < std::size(kBufferParameters); i++) {
        ...
        auto scoped_va_buffer = vaapi_wrapper_->CreateVABuffer(buffer_type, buffer_size);
...
TEST_F(VaapiUtilsTest, ScopedVAImage)
{
    ...
    ASSERT_TRUE(vaapi_wrapper_->CreateContextAndSurfaces(
        VA_RT_FORMAT_YUV420, coded_size, std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1, &va_surfaces));
    ...
    std::unique_ptr<ScopedVAImage> scoped_image;
    {
        ...
        scoped_image = ScopedVAImage::Create(vaapi_wrapper_->va_lock_, vaapi_wrapper_->va_display_, va_surfaces[0], va_image_format, coded_size);
...
TEST_F(VaapiUtilsTest, BadScopedVAImage)
{
    ...
    std::unique_ptr<ScopedVAImage> scoped_image;
    {
        ...
        VAImageFormat va_image_format = kImageFormatI420;
        base::AutoLockMaybe auto_lock(vaapi_wrapper_->va_lock_.get());
        EXPECT_DCHECK_DEATH(ScopedVAImage::Create(vaapi_wrapper_->va_lock_, vaapi_wrapper_->va_display_, va_surfaces[0], va_image_format, coded_size));

        // This should not hit any DCHECK() but will create an invalid
        // ScopedVAImage.
        scoped_image = ScopedVAImage::Create(vaapi_wrapper_->va_lock_, vaapi_wrapper_->va_display_, va_surfaces[0] - 1, va_image_format, coded_size);
...
TEST_F(VaapiUtilsTest, BadScopedVABufferMapping)
{
    ...
    EXPECT_DCHECK_DEATH(ScopedVABufferMapping::Create(vaapi_wrapper_->va_lock_, vaapi_wrapper_->va_display_, VA_INVALID_ID));

    // This should not hit any DCHECK() but will create an invalid
    // ScopedVABufferMapping.
    auto scoped_buffer = ScopedVABufferMapping::Create(vaapi_wrapper_->va_lock_, vaapi_wrapper_->va_display_, VA_INVALID_ID - 1);
...
TEST_F(VaapiUtilsTest, ScopedVASurface)
{
    ...
    auto scoped_va_surfaces
        = vaapi_wrapper_->CreateContextAndScopedVASurfaces(VA_RT_FORMAT_YUV420, coded_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u,
    ...
    EXPECT_EQ(VA_RT_FORMAT_YUV420, base::checked_cast<int>(scoped_va_surface->format()));
...
TEST_F(VaapiUtilsTest, ScopedVASurfaceWithVisibleSize)
{
    ...
    auto scoped_va_surfaces
        = vaapi_wrapper_->CreateContextAndScopedVASurfaces(VA_RT_FORMAT_YUV420, coded_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u, visible_size);
    ASSERT_FALSE(scoped_va_surfaces.empty());
...
TEST_F(VaapiUtilsTest, ScopedVASurfaceInvalidSizeRequest)
{
    ...
    EXPECT_TRUE(vaapi_wrapper_
                    ->CreateContextAndScopedVASurfaces(VA_RT_FORMAT_YUV420, invalid_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u,
                        /*visible_size=*/std::nullopt)
                    .empty());
}
...
TEST_F(VaapiUtilsTest, ScopedVASurfaceInvalidRTFormatRequest)
{
    const gfx::Size coded_size(64, 64);
    EXPECT_TRUE(vaapi_wrapper_
                    ->CreateContextAndScopedVASurfaces(kInvalidVaRtFormat, coded_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u,
                        /*visible_size=*/std::nullopt)
                    .empty());
}
