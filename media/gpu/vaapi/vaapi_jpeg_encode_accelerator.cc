...
VaapiJpegEncodeAccelerator::EncodeRequest::EncodeRequest(int32_t task_id, scoped_refptr<VideoFrame> video_frame, base::WritableSharedMemoryMapping exif_mapping,
    base::WritableSharedMemoryMapping output_mapping, int quality)
    : task_id(task_id)
...
public:
    ...
    void Initialize(base::RepeatingCallback<void(int32_t, size_t)> video_frame_ready_cb, base::RepeatingCallback<void(int32_t, Status)> notify_error_cb,
        chromeos_camera::JpegEncodeAccelerator::InitCB init_cb);

    // Processes one encode task with DMA-buf.
    void EncodeWithDmaBufTask(scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int32_t task_id, int quality,
...
private:
    std::unique_ptr<VaapiJpegEncoder> jpeg_encoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    scoped_refptr<VaapiWrapper> vaapi_wrapper_ GUARDED_BY_CONTEXT(sequence_checker_);
    scoped_refptr<VaapiWrapper> vpp_vaapi_wrapper_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::unique_ptr<gpu::GpuMemoryBufferSupport> gpu_memory_buffer_support_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    std::unique_ptr<ScopedVABuffer> cached_output_buffer_ GUARDED_BY_CONTEXT(sequence_checker_);

    base::RepeatingCallback<void(int32_t, size_t)> video_frame_ready_cb_ GUARDED_BY_CONTEXT(sequence_checker_);
    base::RepeatingCallback<void(int32_t, Status)> notify_error_cb_ GUARDED_BY_CONTEXT(sequence_checker_);

    // The current VA surface ID used for encoding. Only used for Non-DMA-buf use
    // case.
    VASurfaceID va_surface_id_ GUARDED_BY_CONTEXT(sequence_checker_) { VA_INVALID_SURFACE };
...
void VaapiJpegEncodeAccelerator::Encoder::Initialize(base::RepeatingCallback<void(int32_t, size_t)> video_frame_ready_cb,
    base::RepeatingCallback<void(int32_t, Status)> notify_error_cb, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    vaapi_wrapper_ = VaapiWrapper::Create(VaapiWrapper::kEncodeConstantBitrate, VAProfileJPEGBaseline, EncryptionScheme::kUnencrypted,
        base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiJpegEncodeAccelerator.VAAPIError"))
                         .value_or(nullptr);
    if (!vaapi_wrapper_) {
    ...
    vpp_vaapi_wrapper_ = VaapiWrapper::Create(VaapiWrapper::kVideoProcess, VAProfileNone, EncryptionScheme::kUnencrypted,
        base::BindRepeating(&ReportVaapiErrorToUMA, "Media.VaapiJpegEncodeAccelerator.Vpp.VAAPIError"))
                             .value_or(nullptr);
...
void VaapiJpegEncodeAccelerator::Encoder::EncodeWithDmaBufTask(
    scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int32_t task_id, int quality, base::WritableSharedMemoryMapping exif_mapping)
{
    DVLOGF(4);
    ...
    if (context_changed) {
        vaapi_wrapper_->DestroyContextAndSurfaces(std::vector<VASurfaceID>({ va_surface_id_ }));
        va_surface_id_ = VA_INVALID_SURFACE;
        va_format_ = 0;
        input_size_ = gfx::Size();

        std::vector<VASurfaceID> va_surfaces;
        if (!vaapi_wrapper_->CreateContextAndSurfaces(va_format, input_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1, &va_surfaces)) {
    ...
    DCHECK(input_frame);
    scoped_refptr<gfx::NativePixmap> pixmap = CreateNativePixmapDmaBuf(input_frame.get());
    if (!pixmap) {
    ...
    auto input_surface = vpp_vaapi_wrapper_->CreateVASurfaceForPixmap(std::move(pixmap));
    ...
    if (!vpp_vaapi_wrapper_->BlitSurface(input_surface->id(), input_surface->size(), va_surface_id_, input_size)) {
    ...
    size_t max_coded_buffer_size = VaapiJpegEncoder::GetMaxCodedBufferSize(input_size);
    if (context_changed || !cached_output_buffer_ || cached_output_buffer_->size() < max_coded_buffer_size) {
        cached_output_buffer_.reset();

        auto output_buffer = vaapi_wrapper_->CreateVABuffer(VAEncCodedBufferType, max_coded_buffer_size);
        if (!output_buffer) {
    ...
    if (!jpeg_encoder_->Encode(input_size, /*exif_buffer=*/nullptr,
            /*exif_buffer_size=*/0u, quality, va_surface_id_, cached_output_buffer_->id(),
            /*exif_offset=*/nullptr)) {
    ...
    absl::Cleanup output_gmb_buffer_unmapper = [&output_gmb_buffer] { output_gmb_buffer->Unmap(); };
    ...
    if (exif_buffer_size > 0 && (exif_buffer_size < kApp0DataSize || exif_buffer_size > kMaxExifSize)) {
        VLOGF(1) << "Unexpected EXIF data size (" << exif_buffer_size << ")";
        notify_error_cb_.Run(task_id, PLATFORM_FAILURE);
        return;
    }
    const size_t output_offset = exif_buffer_size > 0 ? exif_buffer_size - kApp0DataSize : 0;
    const size_t output_size = base::checked_cast<size_t>(output_gmb_buffer->GetSize().GetArea());
    ...
    if (!vaapi_wrapper_->DownloadFromVABuffer(cached_output_buffer_->id(), va_surface_id_, frame_content, max_frame_size, &encoded_size)) {
    ...
    if (exif_buffer_size > 0) {
        ...
        memcpy(output_memory, jpeg_soi_and_app1_header, std::size(jpeg_soi_and_app1_header));
        memcpy(output_memory + std::size(jpeg_soi_and_app1_header), exif_buffer, exif_buffer_size);
...
void VaapiJpegEncodeAccelerator::Encoder::EncodeTask(std::unique_ptr<EncodeRequest> request)
{
    DVLOGF(4);
    ...
    bool context_changed = input_size != input_size_ || va_surface_id_ == VA_INVALID_SURFACE;
    if (context_changed) {
        vaapi_wrapper_->DestroyContextAndSurfaces(std::vector<VASurfaceID>({ va_surface_id_ }));
        va_surface_id_ = VA_INVALID_SURFACE;
        input_size_ = gfx::Size();
        ...
        std::vector<VASurfaceID> va_surfaces;
        if (!vaapi_wrapper_->CreateContextAndSurfaces(VA_RT_FORMAT_YUV420, input_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1, &va_surfaces)) {
            VLOGF(1) << "Failed to create VA surface";
            notify_error_cb_.Run(task_id, PLATFORM_FAILURE);
            return;
        }
    ...
    if (!vaapi_wrapper_->UploadVideoFrameToSurface(*request->video_frame, va_surface_id_, input_size_)) {
    ...
    if (!jpeg_encoder_->Encode(
            input_size, exif_buffer_dummy.data(), exif_buffer_size, request->quality, va_surface_id_, cached_output_buffer_->id(), &exif_offset)) {
        VLOGF(1) << "Encode JPEG failed";
        notify_error_cb_.Run(task_id, PLATFORM_FAILURE);
        return;
    }
    ...
    if (!vaapi_wrapper_->DownloadFromVABuffer(
            cached_output_buffer_->id(), va_surface_id_, request->output_mapping.GetMemoryAs<uint8_t>(), request->output_mapping.size(), &encoded_size)) {
        VLOGF(1) << "Failed to retrieve output image from VA coded buffer";
        notify_error_cb_.Run(task_id, PLATFORM_FAILURE);
        return;
    }
    ...
    memcpy(request->output_mapping.GetMemoryAs<uint8_t>() + exif_offset, exif_buffer, exif_buffer_size);

    video_frame_ready_cb_.Run(task_id, encoded_size);
}

VaapiJpegEncodeAccelerator::VaapiJpegEncodeAccelerator(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
...
void VaapiJpegEncodeAccelerator::VideoFrameReady(int32_t task_id, size_t encoded_picture_size)
{
...
void VaapiJpegEncodeAccelerator::InitializeAsync(chromeos_camera::JpegEncodeAccelerator::Client* client, chromeos_camera::JpegEncodeAccelerator::InitCB init_cb)
{
    VLOGF(2);
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    client_ = client;

    encoder_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::BEST_EFFORT });
    ...
    encoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VaapiJpegEncodeAccelerator::Encoder::Initialize, base::Unretained(encoder_.get()),
            BindPostTask(io_task_runner_, base::BindRepeating(&VaapiJpegEncodeAccelerator::VideoFrameReady, weak_this_)),
            BindPostTask(io_task_runner_, base::BindRepeating(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_)),
            base::BindPostTaskToCurrentDefault(std::move(init_cb))));
}

size_t VaapiJpegEncodeAccelerator::GetMaxCodedBufferSize(const gfx::Size& picture_size)
{
    return VaapiJpegEncoder::GetMaxCodedBufferSize(picture_size);
}

void VaapiJpegEncodeAccelerator::Encode(scoped_refptr<VideoFrame> video_frame, int quality, BitstreamBuffer* exif_buffer, BitstreamBuffer output_buffer)
{
    DVLOGF(4);
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    ...
    if (video_frame->format() != VideoPixelFormat::PIXEL_FORMAT_I420) {
        ...
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, INVALID_ARGUMENT));
        return;
    }
    ...
    if (exif_buffer) {
        ...
        if (!exif_mapping.IsValid()) {
            ...
            io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, PLATFORM_FAILURE));
            return;
        }
    ...
    base::WritableSharedMemoryMapping output_mapping = output_region.MapAt(output_buffer.offset(), output_buffer.size());
    if (!output_mapping.IsValid()) {
        VLOGF(1) << "Failed to map output buffer";
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::NotifyError, weak_this_, task_id, INACCESSIBLE_OUTPUT_BUFFER));
        return;
    }

    auto request = std::make_unique<EncodeRequest>(task_id, std::move(video_frame), std::move(exif_mapping), std::move(output_mapping), quality);

    // base::Unretained(encoder_) is safe because |encoder_| is passed to
    // and destroyed on |encoder_task_runner_| in destructor. Thus |encoder_| is
    // outlive any task that has been posted by |this|.
    encoder_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VaapiJpegEncodeAccelerator::Encoder::EncodeTask, base::Unretained(encoder_.get()), std::move(request)));
}
    ...
    scoped_refptr<VideoFrame> input_frame, scoped_refptr<VideoFrame> output_frame, int quality, int32_t task_id, BitstreamBuffer* exif_buffer)
{
    ...
    // outlive any task that has been posted by |this|.
    encoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VaapiJpegEncodeAccelerator::Encoder::EncodeWithDmaBufTask, base::Unretained(encoder_.get()), input_frame, output_frame, task_id,
            quality, std::move(exif_mapping)));
}
