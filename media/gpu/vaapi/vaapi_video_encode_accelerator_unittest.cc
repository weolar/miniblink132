...
constexpr Bitrate kDefaultBitrate = Bitrate::ConstantBitrate(kDefaultBitrateBps);
...
{
    VideoEncodeAccelerator::Config vea_config(PIXEL_FORMAT_I420, kDefaultEncodeSize, VP9PROFILE_PROFILE0, kDefaultBitrate, kDefaultFramerate,
        VideoEncodeAccelerator::Config::StorageType::kShmem, VideoEncodeAccelerator::Config::ContentType::kCamera);

    return vea_config;
}

std::vector<VideoEncodeAccelerator::Config::SpatialLayer> GetDefaultSVCLayers(size_t num_spatial_layers, size_t num_temporal_layers)
{
...
std::vector<gfx::Size> GetDefaultSVCResolutions(size_t num_spatial_layers)
{
    ...
    for (size_t i = 0; i < num_spatial_layers; ++i) {
        const int denom = kSpatialLayersResolutionDenom[num_spatial_layers - 1][i];
        spatial_layer_resolutions.emplace_back(gfx::Size(kDefaultEncodeSize.width() / denom, kDefaultEncodeSize.height() / denom));
    }
...
bool IsSVCSupported(const VideoEncodeAccelerator::Config& config)
{
    ...
    return config.input_format == PIXEL_FORMAT_NV12 && config.storage_type == VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer
...
MATCHER_P3(MatchesBitstreamBufferMetadata, payload_size_bytes, key_frame, has_vp9_metadata, "")
{
    return arg.payload_size_bytes == payload_size_bytes && arg.key_frame == key_frame && arg.vp9.has_value() == has_vp9_metadata;
}

MATCHER_P2(MatchesEncoderInfo, num_of_spatial_layers, num_of_temporal_layers, "")
{
    for (size_t i = 0; i < num_of_spatial_layers; ++i) {
        ...
        if (fps_allocation.back() != kFullFramerate)
            return false;
        if (fps_allocation.size() != 1 && fps_allocation != GetFpsAllocation(num_of_temporal_layers)) {
            return false;
        }
    }
    return arg.implementation_name == "VaapiVideoEncodeAccelerator" && arg.supports_native_handle && !arg.has_trusted_rate_controller
...
public:
    ...
    MOCK_METHOD3(RequireBitstreamBuffers, void(unsigned int, const gfx::Size&, size_t));
    MOCK_METHOD2(BitstreamBufferReady, void(int32_t, const BitstreamBufferMetadata&));
...
public:
    ...
    MOCK_METHOD6(CreateScopedVASurfaces,
        std::vector<std::unique_ptr<ScopedVASurface>>(
            unsigned int, const gfx::Size&, const std::vector<SurfaceUsageHint>&, size_t, const std::optional<gfx::Size>&, const std::optional<uint32_t>&));
    MOCK_METHOD2(CreateVABuffer, std::unique_ptr<ScopedVABuffer>(VABufferType, size_t));
    MOCK_METHOD2(CreateVASurfaceForPixmap, std::unique_ptr<ScopedVASurface>(scoped_refptr<const gfx::NativePixmap>, bool));
    MOCK_METHOD2(GetEncodedChunkSize, uint64_t(VABufferID, VASurfaceID));
    MOCK_METHOD5(DownloadFromVABuffer, bool(VABufferID, std::optional<VASurfaceID>, uint8_t*, size_t, size_t*));
    MOCK_METHOD3(UploadVideoFrameToSurface, bool(const VideoFrame&, VASurfaceID, const gfx::Size&));
    ...
    MOCK_METHOD2(DoBlitSurface, bool(std::optional<gfx::Rect>, std::optional<gfx::Rect>));
    bool BlitSurface(VASurfaceID va_surface_src_id, const gfx::Size& va_surface_src_size, VASurfaceID va_surface_dst_id, const gfx::Size& va_surface_dst_size,
        std::optional<gfx::Rect> src_rect = std::nullopt, std::optional<gfx::Rect> dest_rect = std::nullopt
...
public:
    MockVaapiVideoEncoderDelegate(scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure error_cb)
        : VaapiVideoEncoderDelegate(vaapi_wrapper, error_cb)
    {
    }
    MOCK_METHOD2(Initialize, bool(const VideoEncodeAccelerator::Config&, const VaapiVideoEncoderDelegate::Config&));
...
public:
    MockVP9VaapiVideoEncoderDelegate(const scoped_refptr<VaapiWrapper>& vaapi_wrapper, base::RepeatingClosure error_cb)
...
class VaapiVideoEncodeAcceleratorTest : public ::testing::TestWithParam<VaapiVideoEncodeAcceleratorTestParam> {
public:
    ...
    struct PrintToStringParamName {
        template <class ParamType> std::string operator()(const testing::TestParamInfo<ParamType>& info) const
        {
            ...
            return base::StringPrintf("L%dT%d%s", info.param.num_of_spatial_layers, info.param.num_of_temporal_layers,
                (info.param.inter_layer_pred == SVCInterLayerPredMode::kOnKeyPic ? "_KEY" : ""));
        }
    };
...
protected:
    ...
    void SetUp() override
    {
        mock_vaapi_wrapper_ = base::MakeRefCounted<MockVaapiWrapper>(VaapiWrapper::kEncodeConstantBitrate);
    ...
    void ResetEncoder()
    {
        ...
        auto* vaapi_encoder = reinterpret_cast<VaapiVideoEncodeAccelerator*>(encoder_.get());
        base::WaitableEvent event;
        auto on_error_cb = base::BindRepeating(&VaapiVideoEncodeAcceleratorTest::OnError, base::Unretained(this));
        // Set |encoder_| and |vaapi_wrapper_| of |vaapi_encoder| in the encoder
        // sequence.
        vaapi_encoder->encoder_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                [](VaapiVideoEncodeAccelerator* vaapi_encoder, scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure on_error_cb,
                    raw_ptr<MockVaapiVideoEncoderDelegate, AcrossTasksDanglingUntriaged>* mock_encoder_delegate, base::WaitableEvent* event) {
                    DCHECK_CALLED_ON_VALID_SEQUENCE(vaapi_encoder->encoder_sequence_checker_);
                    vaapi_encoder->vaapi_wrapper_ = vaapi_wrapper;
                    vaapi_encoder->encoder_ = std::make_unique<MockVaapiVideoEncoderDelegate>(vaapi_wrapper, std::move(on_error_cb));
                    *mock_encoder_delegate = reinterpret_cast<MockVaapiVideoEncoderDelegate*>(vaapi_encoder->encoder_.get());
                    event->Signal();
                },
                base::Unretained(vaapi_encoder), mock_vaapi_wrapper_, on_error_cb, base::Unretained(&mock_encoder_delegate_), base::Unretained(&event)));
        event.Wait();
    ...
    void ResetVp9Encoder()
    {
        ...
        vaapi_encoder->encoder_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                [](VaapiVideoEncodeAccelerator* vaapi_encoder, scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure on_error_cb,
                    raw_ptr<MockVP9VaapiVideoEncoderDelegate, AcrossTasksDanglingUntriaged>* mock_encoder, base::WaitableEvent* event) {
                    DCHECK_CALLED_ON_VALID_SEQUENCE(vaapi_encoder->encoder_sequence_checker_);
                    vaapi_encoder->vaapi_wrapper_ = vaapi_wrapper;
                    vaapi_encoder->encoder_ = std::make_unique<MockVP9VaapiVideoEncoderDelegate>(vaapi_wrapper, std::move(on_error_cb));
                    *mock_encoder = reinterpret_cast<MockVP9VaapiVideoEncoderDelegate*>(vaapi_encoder->encoder_.get());
                    event->Signal();
                },
                base::Unretained(vaapi_encoder), mock_vaapi_wrapper_, on_error_cb, base::Unretained(&mock_encoder_), base::Unretained(&event)));
        event.Wait();
        EXPECT_CALL(*this, OnError()).Times(0);
    }
    ...
    void SetDefaultMocksBehavior(const VideoEncodeAccelerator::Config& config)
    {
        ...
        ON_CALL(*mock_vaapi_wrapper_, GetVAEncMaxNumOfRefFrames).WillByDefault(WithArgs<1>([](size_t* max_ref_frames) {
            *max_ref_frames = kMaxNumOfRefFrames;
            return true;
        }));

        ON_CALL(*mock_encoder_, GetBitstreamBufferSize).WillByDefault(Return(config.input_visible_size.GetArea()));
        ON_CALL(*mock_encoder_, GetCodedSize()).WillByDefault(Return(config.input_visible_size));
        ON_CALL(*mock_encoder_, GetMaxNumOfRefFrames()).WillByDefault(Return(kMaxNumOfRefFrames));
        ...
        ON_CALL(*mock_encoder_, GetSVCLayerResolutions()).WillByDefault(Return(svc_resolutions));
    }

    bool InitializeVideoEncodeAccelerator(const VideoEncodeAccelerator::Config& config)
    {
        VideoEncodeAccelerator::SupportedProfile profile(config.output_profile, config.input_visible_size);
        auto* vaapi_encoder = reinterpret_cast<VaapiVideoEncodeAccelerator*>(encoder_.get());
        vaapi_encoder->supported_profiles_for_testing_.push_back(profile);
        if (config.input_visible_size.IsEmpty())
            return false;
        return encoder_->Initialize(config, &client_, std::make_unique<media::NullMediaLog>());
    }
    ...
    void InitializeSequenceForVP9(const VideoEncodeAccelerator::Config& config) NO_THREAD_SAFETY_ANALYSIS
    {
        ...
        mock_vpp_vaapi_wrapper_ = base::MakeRefCounted<MockVaapiWrapper>(VaapiWrapper::kVideoProcess);
        // In real usage, the VaapiWrapper expects to be constructed, used, and
        // destroyed on the same sequence. For testing, however, we create it in the
        // main thread of the test and inject it into the VaapiVideoEncodeAccelerator
        // where it will be used and destroyed on the encoder thread. Therefore, we
        ...
        EXPECT_CALL(*mock_encoder_, Initialize(_, testing::Field(&VaapiVideoEncoderDelegate::Config::max_num_ref_frames, kMaxNumOfRefFrames)))
            .WillOnce(Return(true));
        EXPECT_CALL(*mock_vaapi_wrapper_, CreateContext(kDefaultEncodeSize)).WillOnce(Return(true));
        EXPECT_CALL(client_, RequireBitstreamBuffers(_, kDefaultEncodeSize, _)).WillOnce(WithArgs<2>([this](size_t output_buffer_size) {
            this->output_buffer_size_ = output_buffer_size;
        }));
        EXPECT_CALL(client_, NotifyEncoderInfoChange(MatchesEncoderInfo(num_spatial_layers, config.spatial_layers[0].num_of_temporal_layers)))
            .WillOnce(RunClosure(run_loop.QuitClosure()));
        ASSERT_TRUE(InitializeVideoEncodeAccelerator(config));
        run_loop.Run();
    }

    void EncodeSequenceForVP9SingleSpatialLayer(bool use_temporal_layer_encoding)
    {
        ::testing::InSequence s;

        constexpr VASurfaceID kEncodeSurfaceId = 1234;
        EXPECT_CALL(*mock_vaapi_wrapper_,
            CreateScopedVASurfaces(VA_RT_FORMAT_YUV420, kDefaultEncodeSize,
                std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kVideoEncoder }, _, std::optional<gfx::Size>(),
                std::optional<uint32_t>()))
            .WillOnce(WithArgs<0, 1, 3>([&surface_ids = this->va_encode_surface_ids_[0], &vaapi_wrapper = this->mock_vaapi_wrapper_](
                                            unsigned int format, const gfx::Size& size, size_t num_surfaces) {
                ...
                for (const VASurfaceID id : surface_ids) {
                    va_surfaces.push_back(std::make_unique<ScopedVASurface>(vaapi_wrapper, id, size, format));
                }
                return va_surfaces;
            }));
        ...
        EXPECT_CALL(*mock_vaapi_wrapper_,
            CreateScopedVASurfaces(VA_RT_FORMAT_YUV420, kDefaultEncodeSize,
                std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kVideoEncoder }, 1, std::optional<gfx::Size>(),
                std::optional<uint32_t>()))
            .WillOnce(WithArgs<0, 1>([&vaapi_wrapper = this->mock_vaapi_wrapper_, surface_id = kInputSurfaceId](unsigned int format, const gfx::Size& size) {
                std::vector<std::unique_ptr<ScopedVASurface>> va_surfaces;
                va_surfaces.push_back(std::make_unique<ScopedVASurface>(vaapi_wrapper, surface_id, size, format));
                return va_surfaces;
            }));

        EXPECT_CALL(*mock_vaapi_wrapper_, UploadVideoFrameToSurface(_, kInputSurfaceId, kDefaultEncodeSize)).WillOnce(Return(true));

        constexpr VABufferID kCodedBufferId = 123;
        EXPECT_CALL(*mock_vaapi_wrapper_, CreateVABuffer(VAEncCodedBufferType, output_buffer_size_)).WillOnce(WithArgs<1>([](size_t buffer_size) {
            return ScopedVABuffer::CreateForTesting(kCodedBufferId, VAEncCodedBufferType, buffer_size);
        }));

        EXPECT_CALL(*mock_encoder_, PrepareEncodeJob(_)).WillOnce(WithArgs<0>([use_temporal_layer_encoding](VaapiVideoEncoderDelegate::EncodeJob& job) {
            if (use_temporal_layer_encoding) {
                // Set Vp9Metadata on temporal layer encoding.
                CodecPicture* picture = job.picture().get();
                reinterpret_cast<VP9Picture*>(picture)->metadata_for_encoding = Vp9Metadata();
            }
            return VaapiVideoEncoderDelegate::PrepareEncodeJobResult::kSuccess;
        }));
        EXPECT_CALL(*mock_vaapi_wrapper_, ExecuteAndDestroyPendingBuffers(kInputSurfaceId)).WillOnce(Return(true));

        constexpr uint64_t kEncodedChunkSize = 1234;
        ASSERT_LE(kEncodedChunkSize, output_buffer_size_);
        EXPECT_CALL(*mock_vaapi_wrapper_, GetEncodedChunkSize(kCodedBufferId, kInputSurfaceId)).WillOnce(Return(kEncodedChunkSize));
        EXPECT_CALL(*mock_encoder_, GetMetadata(_, _)).WillOnce(WithArgs<0, 1>([](const VaapiVideoEncoderDelegate::EncodeJob& job, size_t payload_size) {
            // Same implementation in VP9VaapiVideoEncoderDelegate.
            BitstreamBufferMetadata metadata(payload_size, job.IsKeyframeRequested(), job.timestamp());
            CodecPicture* picture = job.picture().get();
            metadata.vp9 = reinterpret_cast<VP9Picture*>(picture)->metadata_for_encoding;
            return metadata;
        }));
        EXPECT_CALL(*mock_encoder_, BitrateControlUpdate(CheckEncodeData(kEncodedChunkSize))).WillOnce(Return());
        EXPECT_CALL(*mock_vaapi_wrapper_, DownloadFromVABuffer(kCodedBufferId, Eq(std::nullopt), _, output_buffer_size_, _))
        ...
        EXPECT_CALL(client_, BitstreamBufferReady(kBitstreamId, MatchesBitstreamBufferMetadata(kEncodedChunkSize, false, use_temporal_layer_encoding)))
            .WillOnce(RunClosure(run_loop.QuitClosure()));

        auto region = base::UnsafeSharedMemoryRegion::Create(output_buffer_size_);
        ASSERT_TRUE(region.IsValid());
        encoder_->UseOutputBitstreamBuffer(BitstreamBuffer(kBitstreamId, std::move(region), output_buffer_size_));

        auto frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, kDefaultEncodeSize, gfx::Rect(kDefaultEncodeSize), kDefaultEncodeSize, base::TimeDelta());
        ASSERT_TRUE(frame);
    ...
    void EncodeSequenceForVP9MultipleSpatialLayers(size_t num_spatial_layers)
    {
        ...
        for (size_t i = 0; i < num_spatial_layers; ++i) {
            ...
            if (i < num_spatial_layers - 1) {
                EXPECT_CALL(client_, BitstreamBufferReady(kBitstreamId, MatchesBitstreamBufferMetadata(kEncodedChunkSize, false, true)));
            } else {
                EXPECT_CALL(client_, BitstreamBufferReady(kBitstreamId, MatchesBitstreamBufferMetadata(kEncodedChunkSize, false, true)))
                    .WillOnce(RunClosure(run_loop.QuitClosure()));
            }
        ...
        std::vector<gfx::Size> svc_resolutions = GetDefaultSVCResolutions(num_spatial_layers);
        ...
        for (size_t i = 0; i < num_spatial_layers; i++) {
            ...
            if (va_encode_surface_ids_[i].empty()) {
                EXPECT_CALL(*mock_vaapi_wrapper_,
                    CreateScopedVASurfaces(VA_RT_FORMAT_YUV420, svc_resolutions[i],
                        std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kVideoEncoder }, _, std::optional<gfx::Size>(),
                        std::optional<uint32_t>()))
                    .WillOnce(
                        WithArgs<0, 1, 3>([&surface_ids = this->va_encode_surface_ids_[i], &vaapi_wrapper = this->mock_vaapi_wrapper_,
                                              va_encode_surface_id = kEncodeSurfaceIds[i]](unsigned int format, const gfx::Size& size, size_t num_surfaces) {
                            surface_ids.resize(num_surfaces);
                            std::iota(surface_ids.begin(), surface_ids.end(), 1);
        ...
        EXPECT_CALL(*mock_vaapi_wrapper_, CreateVASurfaceForPixmap(_, _))
            .WillOnce(Return(std::make_unique<ScopedVASurface>(mock_vaapi_wrapper_, kSourceSurfaceId, kDefaultEncodeSize, VA_RT_FORMAT_YUV420)));
        ...
        for (size_t i = 0; i < num_spatial_layers - 1; ++i) {
                ...
                EXPECT_CALL(*mock_vpp_vaapi_wrapper_,
                    CreateScopedVASurfaces(VA_RT_FORMAT_YUV420, svc_resolutions[i],
                        std::vector<VaapiWrapper::SurfaceUsageHint> {
                            VaapiWrapper::SurfaceUsageHint::kVideoProcessWrite, VaapiWrapper::SurfaceUsageHint::kVideoEncoder },
                        1, std::optional<gfx::Size>(), std::optional<uint32_t>()))
                    .WillOnce(WithArgs<0, 1>([&surface_id = this->va_vpp_dest_surface_ids_[i], &vaapi_wrapper = this->mock_vpp_vaapi_wrapper_,
                                                 vpp_dest_surface_id = kVppDestSurfaceIds[i]](unsigned int format, const gfx::Size& size) {
                        surface_id = vpp_dest_surface_id;
                        std::vector<std::unique_ptr<ScopedVASurface>> va_surfaces;
                        va_surfaces.push_back(std::make_unique<ScopedVASurface>(vaapi_wrapper, vpp_dest_surface_id, size, format));
                        return va_surfaces;
                    }));
            }
            ...
            EXPECT_CALL(*mock_vpp_vaapi_wrapper_, DoBlitSurface(default_rect, layer_rect)).WillOnce(Return(true));
        }
        ...
        for (size_t i = 0; i < num_spatial_layers; ++i) {
            ...
            EXPECT_CALL(*mock_vaapi_wrapper_, CreateVABuffer(VAEncCodedBufferType, output_buffer_size_))
                .WillOnce(WithArgs<1>(
                    [kCodedBufferId](size_t buffer_size) { return ScopedVABuffer::CreateForTesting(kCodedBufferId, VAEncCodedBufferType, buffer_size); }));
        }

        for (size_t i = 0; i < num_spatial_layers; ++i) {
            EXPECT_CALL(*mock_encoder_, PrepareEncodeJob(_)).WillOnce(WithArgs<0>([](VaapiVideoEncoderDelegate::EncodeJob& job) {
                // Set Vp9Metadata on spatial layer encoding.
                CodecPicture* picture = job.picture().get();
                reinterpret_cast<VP9Picture*>(picture)->metadata_for_encoding = Vp9Metadata();
                return VaapiVideoEncoderDelegate::PrepareEncodeJobResult::kSuccess;
            }));
            EXPECT_CALL(*mock_vaapi_wrapper_, ExecuteAndDestroyPendingBuffers(_)).WillOnce(Return(true));
        }
        for (size_t i = 0; i < num_spatial_layers; ++i) {
            const VABufferID kCodedBufferId = kCodedBufferIds[i];
            const uint64_t kEncodedChunkSize = kEncodedChunkSizes[i];
            ASSERT_LE(kEncodedChunkSize, output_buffer_size_);
            EXPECT_CALL(*mock_vaapi_wrapper_, GetEncodedChunkSize(kCodedBufferId, _)).WillOnce(Return(kEncodedChunkSize));
            EXPECT_CALL(*mock_encoder_, GetMetadata(_, _)).WillOnce(WithArgs<0, 1>([](const VaapiVideoEncoderDelegate::EncodeJob& job, size_t payload_size) {
                // Same implementation in VP9VaapiVideoEncoderDelegate.
                BitstreamBufferMetadata metadata(payload_size, job.IsKeyframeRequested(), job.timestamp());
                CodecPicture* picture = job.picture().get();
        ...
        std::unique_ptr<gfx::GpuMemoryBuffer> gmb = std::make_unique<FakeGpuMemoryBuffer>(kDefaultEncodeSize, gfx::BufferFormat::YUV_420_BIPLANAR);
        auto frame = VideoFrame::WrapExternalGpuMemoryBuffer(gfx::Rect(kDefaultEncodeSize), kDefaultEncodeSize, std::move(gmb), base::TimeDelta());
        ASSERT_TRUE(frame);
    ...
    raw_ptr<MockVP9VaapiVideoEncoderDelegate, AcrossTasksDanglingUntriaged> mock_encoder_ = nullptr;
    raw_ptr<MockVaapiVideoEncoderDelegate, AcrossTasksDanglingUntriaged> mock_encoder_delegate_ = nullptr;
};
...
TEST_P(VaapiVideoEncodeAcceleratorTest, Initialize)
{
    ...
    config.spatial_layers = GetDefaultSVCLayers(num_of_spatial_layers, num_of_temporal_layers);

    for (const VideoPixelFormat format : { PIXEL_FORMAT_I420, PIXEL_FORMAT_NV12 }) {
        for (const Config::StorageType storage_type : { Config::StorageType::kShmem, Config::StorageType::kGpuMemoryBuffer }) {
            for (const VideoCodecProfile profile : { H264PROFILE_MAIN, VP9PROFILE_PROFILE0 }) {
                config.input_format = format;
                config.storage_type = storage_type;
                config.output_profile = profile;
                if (storage_type == Config::StorageType::kGpuMemoryBuffer && format != PIXEL_FORMAT_NV12) {
                    // VaapiVEA doesn't support native input mode for non-NV12 format.
                    EXPECT_EQ(InitializeVideoEncodeAccelerator(config), false);
                } else {
                    EXPECT_EQ(InitializeVideoEncodeAccelerator(config), !config.HasSpatialLayer() || IsSVCSupported(config));
                }
...
TEST_P(VaapiVideoEncodeAcceleratorTest, EncodeVP9WithSingleSpatialLayer)
{
    ...
    EncodeSequenceForVP9SingleSpatialLayer(spatial_layer.num_of_temporal_layers > 1u);
}
...
TEST_F(VaapiVideoEncodeAcceleratorTest, TooManyEncoderInstances)
{
    ...
    std::vector<std::unique_ptr<VaapiVideoEncodeAccelerator>> encoders(kMaxNumOfInstances);
...
TEST_F(VaapiVideoEncodeAcceleratorTest, InitializeWithUnsupportedConfig)
{
    ...
    const Config unsupported_configs[] = { // VaapiVEA does not support HEVC encoding.
        Config(PIXEL_FORMAT_NV12, kDefaultEncodeSize, HEVCPROFILE_MAIN, kDefaultBitrate, kDefaultFramerate, Config::StorageType::kShmem,
            Config::ContentType::kCamera),
        // VaapiVEA only supports variable bitrate with H264 encoding.
        Config(PIXEL_FORMAT_NV12, kDefaultEncodeSize, VP9PROFILE_PROFILE0, kVariableBitrate, kDefaultFramerate, Config::StorageType::kShmem,
            Config::ContentType::kCamera),
        // VaapiVEA does not support PIXEL_FORMAT_YV12.
        Config(PIXEL_FORMAT_YV12, kDefaultEncodeSize, VP9PROFILE_PROFILE0, kDefaultBitrate, kDefaultFramerate, Config::StorageType::kShmem,
            Config::ContentType::kCamera)
    };
...
TEST_F(VaapiVideoEncodeAcceleratorTest, EncodingParametersChange)
{
    ...
    const Bitrate kVariableBitrate = Bitrate::VariableBitrate(kNewBitrate, 2 * kNewBitrate);
    ...
    for (const Bitrate bitrate : { kConstantBitrate, kVariableBitrate }) {
        ...
        EXPECT_CALL(*mock_encoder_delegate_, UpdateRates(expected_bitrate_allocation, kNewFramerate)).WillOnce(Return(true));
        encoder_->RequestEncodingParametersChange(bitrate, kNewFramerate, std::nullopt);
        task_environment_.RunUntilIdle();
    }
}
...
TEST_F(VaapiVideoEncodeAcceleratorTest, EncodingParametersChangeWithBitrateAllocation)
{
    ResetEncoder();
    Config config = DefaultVideoEncodeAcceleratorConfig();
    ASSERT_TRUE(InitializeVideoEncodeAccelerator(config));
    ...
    for (size_t si = 0; si < VideoBitrateAllocation::kMaxSpatialLayers; si++) {
        for (size_t ti = 0; ti < VideoBitrateAllocation::kMaxTemporalLayers; ti++) {
            uint32_t layer_bitrate = std::max(si * ti * 1000, static_cast<size_t>(100));
            bitrate_allocation.SetBitrate(si, ti, layer_bitrate);
            EXPECT_CALL(*mock_encoder_delegate_, UpdateRates(bitrate_allocation, kNewFramerate)).WillOnce(Return(true));
            encoder_->RequestEncodingParametersChange(bitrate_allocation, kNewFramerate, std::nullopt);
            task_environment_.RunUntilIdle();
        }
    }
}

INSTANTIATE_TEST_SUITE_P(, VaapiVideoEncodeAcceleratorTest, ::testing::ValuesIn(kTestCases), VaapiVideoEncodeAcceleratorTest::PrintToStringParamName());
} // namespace media
