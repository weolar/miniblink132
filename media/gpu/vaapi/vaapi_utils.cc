...
template <typename To, typename From> void CheckedMemcpy(To& to, From& from)
{
    static_assert(std::is_array<To>::value, "First parameter must be an array");
    static_assert(std::is_array<From>::value, "Second parameter must be an array");
...
// static
std::unique_ptr<ScopedVABufferMapping> ScopedVABufferMapping::Create(const base::Lock* lock, VADisplay va_display, VABufferID buffer_id)
{
    DCHECK(va_display);
    ...
    CHECK(va_buffer_data) << "va_buffer_data must not be null if vaMapBuffer() succeeds";

    return base::WrapUnique(new ScopedVABufferMapping(lock, va_display, buffer_id, va_buffer_data));
}

ScopedVABufferMapping::ScopedVABufferMapping(const base::Lock* lock, VADisplay va_display, VABufferID buffer_id, void* va_buffer_data)
    : lock_(lock)
...
ScopedVABufferMapping::~ScopedVABufferMapping()
{
    ...
    if (VAStatus result = vaUnmapBuffer(va_display_, buffer_id_); result != VA_STATUS_SUCCESS) {
...
std::unique_ptr<ScopedVABuffer> ScopedVABuffer::Create(
    base::Lock* lock, VADisplay va_display, VAContextID va_context_id, VABufferType va_buffer_type, size_t size)
{
    DCHECK(va_display);
    ...
    const VAStatus va_res = vaCreateBuffer(va_display, va_context_id, va_buffer_type, va_buffer_size, 1, nullptr, &buffer_id);
    if (va_res != VA_STATUS_SUCCESS) {
        LOG(ERROR) << "Failed to create a VA buffer: " << vaErrorStr(va_res);
        return nullptr;
    }
    DCHECK_NE(buffer_id, VA_INVALID_ID);
    return base::WrapUnique(new ScopedVABuffer(lock, va_display, buffer_id, va_buffer_type, size));
}

// static
std::unique_ptr<ScopedVABuffer> ScopedVABuffer::CreateForTesting(VABufferID va_buffer_id, VABufferType va_buffer_type, size_t size)
{
    return base::WrapUnique(new ScopedVABuffer(nullptr, nullptr, va_buffer_id, va_buffer_type, size));
}

ScopedVABuffer::ScopedVABuffer(base::Lock* lock, VADisplay va_display, VABufferID va_buffer_id, VABufferType va_buffer_type, size_t size)
    : lock_(lock)
    , va_display_(va_display)
...
ScopedVABuffer::~ScopedVABuffer()
{
    ...
    LOG_IF(ERROR, va_res != VA_STATUS_SUCCESS) << "Failed to destroy a VA buffer: " << vaErrorStr(va_res);
}

// static
std::unique_ptr<ScopedVAImage> ScopedVAImage::Create(
    base::Lock* lock, VADisplay va_display, VASurfaceID va_surface_id, const VAImageFormat& format, const gfx::Size& size)
{
    DCHECK(va_display);
    ...
    VAStatus result = vaCreateImage(va_display, &format_copy, size.width(), size.height(), &image);
    if (result != VA_STATUS_SUCCESS) {
    ...
    result = vaGetImage(va_display, va_surface_id, 0, 0, size.width(), size.height(), image.image_id);
    if (result != VA_STATUS_SUCCESS) {
        LOG(ERROR) << "vaGetImage failed: " << vaErrorStr(result);
        result = vaDestroyImage(va_display, image.image_id);
        LOG_IF(ERROR, result != VA_STATUS_SUCCESS) << "vaDestroyImage failed: " << vaErrorStr(result);
        return nullptr;
    }
    std::unique_ptr<ScopedVABufferMapping> va_buffer = ScopedVABufferMapping::Create(lock, va_display, image.buf);
    if (!va_buffer) {
        result = vaDestroyImage(va_display, image.image_id);
        LOG_IF(ERROR, result != VA_STATUS_SUCCESS) << "vaDestroyImage failed: " << vaErrorStr(result);
        return nullptr;
    ...
    return base::WrapUnique(new ScopedVAImage(lock, va_display, image, std::move(va_buffer)));
}

ScopedVAImage::ScopedVAImage(base::Lock* lock, VADisplay va_display, const VAImage& image, std::unique_ptr<ScopedVABufferMapping> va_buffer)
    : lock_(lock)
    , va_display_(va_display)
    , image_(image)
...
ScopedVASurface::ScopedVASurface(scoped_refptr<VaapiWrapper> vaapi_wrapper, VASurfaceID va_surface_id, const gfx::Size& size, unsigned int va_rt_format)
...
{
    return va_surface_id_ != VA_INVALID_ID && !size_.IsEmpty() && va_rt_format_ != kInvalidVaRtFormat;
}

void FillVP8DataStructures(const Vp8FrameHeader& frame_header, const Vp8ReferenceFrameVector& reference_frames, VAIQMatrixBufferVP8* iq_matrix_buf,
    VAProbabilityDataBufferVP8* prob_buf, VAPictureParameterBufferVP8* pic_param, VASliceParameterBufferVP8* slice_param)
{
    ...
    static_assert(std::size(decltype(iq_matrix_buf->quantization_index) {}) == kMaxMBSegments, "incorrect quantization matrix segment size");
    static_assert(std::size(decltype(iq_matrix_buf->quantization_index) {}[0]) == 6, "incorrect quantization matrix Q index size");
...
#undef CLAMP_Q
    ...
    if (last_frame) {
        pic_param->last_ref_frame = last_frame->AsVaapiVP8Picture()->va_surface_id();
    } else {
        pic_param->last_ref_frame = VA_INVALID_SURFACE;
    }

    const auto golden_frame = reference_frames.GetFrame(Vp8RefType::VP8_FRAME_GOLDEN);
    if (golden_frame) {
        pic_param->golden_ref_frame = golden_frame->AsVaapiVP8Picture()->va_surface_id();
    } else {
        pic_param->golden_ref_frame = VA_INVALID_SURFACE;
    }

    const auto alt_frame = reference_frames.GetFrame(Vp8RefType::VP8_FRAME_ALTREF);
    if (alt_frame)
...
#define FHDR_TO_PP_PF(a, b) pic_param->pic_fields.bits.a = (b)
    ...
    FHDR_TO_PP_PF(update_mb_segmentation_map, sgmnt_hdr.update_mb_segmentation_map);
    FHDR_TO_PP_PF(update_segment_feature_data, sgmnt_hdr.update_segment_feature_data);
...
#undef FHDR_TO_PP_PF
    ...
    static_assert(
        std::extent<decltype(sgmnt_hdr.lf_update_value)>() == std::extent<decltype(pic_param->loop_filter_level)>(), "loop filter level arrays mismatch");
    ...
    static_assert(std::extent<decltype(lf_hdr.ref_frame_delta)>() == std::extent<decltype(pic_param->loop_filter_deltas_ref_frame)>(),
        "loop filter deltas arrays size mismatch");
    static_assert(std::extent<decltype(lf_hdr.mb_mode_delta)>() == std::extent<decltype(pic_param->loop_filter_deltas_mode)>(),
        "loop filter deltas arrays size mismatch");
    static_assert(std::extent<decltype(lf_hdr.ref_frame_delta)>() == std::extent<decltype(lf_hdr.mb_mode_delta)>(), "loop filter deltas arrays size mismatch");
...
#undef FHDR_TO_PP
    ...
    slice_param->partition_size[0] = frame_header.first_part_size - ((frame_header.macroblock_bit_offset + 7) / 8);
