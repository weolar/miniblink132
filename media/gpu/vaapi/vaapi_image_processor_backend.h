...
public:
    ...
    VaapiImageProcessorBackend& operator=(const VaapiImageProcessorBackend&) = delete;
...
private:
    VaapiImageProcessorBackend(const PortConfig& input_config, const PortConfig& output_config, OutputMode output_mode, ErrorCB error_cb);
    ...
    const ScopedVASurface* GetOrCreateSurfaceForFrame(const FrameResource& frame, bool use_protected);

    scoped_refptr<VaapiWrapper> vaapi_wrapper_ GUARDED_BY_CONTEXT(backend_sequence_checker_);
    ...
    base::IDMap<std::unique_ptr<ScopedVASurface>, decltype(gfx::GenericSharedMemoryId::id)> allocated_va_surfaces_ GUARDED_BY_CONTEXT(
        backend_sequence_checker_);
};
