...
VaapiVideoDecoderDelegate::VaapiVideoDecoderDelegate(VaapiDecodeSurfaceHandler* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
    ...
    , on_protected_session_update_cb_(std::move(on_protected_session_update_cb))
...
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    transcryption_ = cdm_context && VaapiWrapper::GetImplementationType() == VAImplementation::kMesaGallium;
}
...
void VaapiVideoDecoderDelegate::set_vaapi_wrapper(scoped_refptr<VaapiWrapper> vaapi_wrapper)
{
    DETACH_FROM_SEQUENCE(sequence_checker_);
    vaapi_wrapper_ = std::move(vaapi_wrapper);
    protected_session_state_ = ProtectedSessionState::kNotCreated;
...
bool VaapiVideoDecoderDelegate::SetDecryptConfig(std::unique_ptr<DecryptConfig> decrypt_config)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
VaapiVideoDecoderDelegate::ProtectedSessionState VaapiVideoDecoderDelegate::SetupDecryptDecode(bool full_sample, size_t size,
    VAEncryptionParameters* crypto_params, std::vector<VAEncryptionSegmentInfo>* segments, const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(crypto_params);
    DCHECK(segments);
    if (protected_session_state_ == ProtectedSessionState::kInProcess || protected_session_state_ == ProtectedSessionState::kFailed) {
    ...
    if (protected_session_state_ == ProtectedSessionState::kNotCreated) {
        ...
        chromeos_cdm_context_->GetHwConfigData(
            base::BindPostTaskToCurrentDefault(base::BindOnce(&VaapiVideoDecoderDelegate::OnGetHwConfigData, weak_factory_.GetWeakPtr())));
    ...
    if (ctr) {
        crypto_params->encryption_type = full_sample ? VA_ENCRYPTION_TYPE_FULLSAMPLE_CTR : VA_ENCRYPTION_TYPE_SUBSAMPLE_CTR;
    } else {
        crypto_params->encryption_type = full_sample ? VA_ENCRYPTION_TYPE_FULLSAMPLE_CBC : VA_ENCRYPTION_TYPE_SUBSAMPLE_CBC;
    }
    ...
    if (subsamples.empty() || (subsamples.size() == 1 && subsamples[0].cypher_bytes == 0)) {
        ...
        if (decrypt_config_) {
            ...
            memcpy(segment_info.aes_cbc_iv_or_ctr, decrypt_config_->iv().data(), DecryptConfig::kDecryptionKeySize);
        }
    ...
    if (!IsTranscrypted() && last_used_encryption_scheme_ != EncryptionScheme::kUnencrypted && last_used_encryption_scheme_ != encryption_scheme_) {
    ...
    if (!base::Contains(hw_key_data_map_, decrypt_config_->key_id())) {
        ...
        chromeos_cdm_context_->GetHwKeyData(decrypt_config_.get(), hw_identifier_,
            base::BindPostTaskToCurrentDefault(
                base::BindOnce(&VaapiVideoDecoderDelegate::OnGetHwKeyData, weak_factory_.GetWeakPtr(), decrypt_config_->key_id())));
    ...
    if (decrypt_config_->HasPattern() && decrypt_config_->encryption_pattern()->skip_byte_block()) {
        crypto_params->blocks_stripe_encrypted = decrypt_config_->encryption_pattern()->crypt_byte_block();
        crypto_params->blocks_stripe_clear = decrypt_config_->encryption_pattern()->skip_byte_block();
    }
    ...
    for (const auto& entry : subsamples) {
        ...
        memcpy(segment_info.aes_cbc_iv_or_ctr, iv.data(), DecryptConfig::kDecryptionKeySize);
        if (ctr) {
            size_t partial_block_size
                = (DecryptConfig::kDecryptionKeySize - (total_cypher_size % DecryptConfig::kDecryptionKeySize)) % DecryptConfig::kDecryptionKeySize;
            ...
            if (entry.cypher_bytes > partial_block_size) {
                ...
                for (size_t block = 0; block < (entry.cypher_bytes - partial_block_size) / DecryptConfig::kDecryptionKeySize; ++block)
                    ctr128_inc64(iv.data());
            }
    ...
    memcpy(crypto_params->wrapped_decrypt_blob, hw_key_data_map_[decrypt_config_->key_id()].data(), DecryptConfig::kDecryptionKeySize);
...
{
    if (!IsEncryptedSession() || !vaapi_wrapper_->IsProtectedSessionDead() || performing_recovery_) {
        return false;
    }
...
void VaapiVideoDecoderDelegate::OnGetHwConfigData(bool success, const std::vector<uint8_t>& config_data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!success) {
        protected_session_state_ = ProtectedSessionState::kFailed;
    ...
    hw_identifier_.clear();
    if (!vaapi_wrapper_->CreateProtectedSession(encryption_scheme_, config_data, &hw_identifier_)) {
...
void VaapiVideoDecoderDelegate::OnGetHwKeyData(const std::string& key_id, Decryptor::Status status, const std::vector<uint8_t>& key_data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void VaapiVideoDecoderDelegate::RecoverProtectedSession()
{
    ...
    if (chromeos_cdm_context_->UsingArcCdm()) {
        ...
        // running again.
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindRepeating(on_protected_session_update_cb_, true));
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
}
