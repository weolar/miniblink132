...
scoped_refptr<H265Picture> H265VaapiWrapper::CreateH265Picture(const H265SPS* sps)
{
    const VAProfile profile = GetProfile(sps);
    const gfx::Size size = sps->GetVisibleRect().size();

    if (!va_config_) {
        va_config_ = std::make_unique<ScopedVAConfig>(*va_device_, profile, GetFormatForProfile(profile));
    }
    if (!va_context_) {
    ...
    scoped_refptr<SharedVASurface> surface = SharedVASurface::Create(*va_device_, va_config_->va_rt_format(), size, attribute);
    return base::WrapRefCounted(new vaapi_test::H265Picture(surface));
}

bool H265VaapiWrapper::IsChromaSamplingSupported(VideoChromaSampling chroma_sampling)
{
    return chroma_sampling == VideoChromaSampling::k420;
}

bool H265VaapiWrapper::SubmitFrameMetadata(
    const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list, scoped_refptr<H265Picture> pic)
{
...
#undef FROM_PPS_TO_PP_SPF2
    ...
    for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; ++i) {
        iq_matrix_buf.ScalingListDC16x16[i] = scaling_list.scaling_list_dc_coef_16x16[i];
    }

    for (size_t i = 0; i < H265ScalingListData::kNumScalingListMatrices; i += 3) {
        iq_matrix_buf.ScalingListDC32x32[i / 3] = scaling_list.scaling_list_dc_coef_32x32[i];
    }

    return SubmitBuffer(VAIQMatrixBufferType, &iq_matrix_buf);
}

bool H265VaapiWrapper::SubmitSlice(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader* slice_hdr, const H265Picture::Vector& ref_pic_list0,
    const H265Picture::Vector& ref_pic_list1, const H265Picture::Vector& ref_pic_set_lt_curr, const H265Picture::Vector& ref_pic_set_st_curr_after,
    const H265Picture::Vector& ref_pic_set_st_curr_before, scoped_refptr<H265Picture> pic, const uint8_t* data, size_t size,
...
void H265VaapiWrapper::FillVAPicture(VAPictureHEVC* va_pic, scoped_refptr<H265Picture> pic)
{
...
void H265VaapiWrapper::FillVARefFramesFromRefList(const H265Picture::Vector& ref_pic_list, VAPictureHEVC* va_pics)
{
    ref_pic_list_pocs_.clear();
...
bool H265VaapiWrapper::SubmitPriorSliceDataIfPresent(bool last_slice)
{
    ...
    success = SubmitBuffers(
        { { VASliceParameterBufferType, sizeof(slice_param_), &slice_param_ }, { VASliceDataBufferType, last_slice_data_.size(), last_slice_data_.data() } });
    last_slice_data_.clear();
    return success;
}

bool H265VaapiWrapper::SubmitBuffer(VABufferType va_buffer_type, size_t size, const void* data)
{
    ...
    {
        const VAStatus va_res = vaCreateBuffer(va_device_->display(), va_context_->id(), va_buffer_type, size, 1, const_cast<void*>(data), &buffer_id);
...
bool H265VaapiWrapper::SubmitBuffers(const std::vector<VABufferDescriptor>& va_buffers)
{
...
bool H265VaapiWrapper::ExecuteAndDestroyPendingBuffers(VASurfaceID va_surface_id)
{
    ...
    VAStatus va_res = vaBeginPicture(va_device_->display(), va_context_->id(), va_surface_id);
    VA_LOG_ASSERT(va_res, "vaBeginPicture");

    if (!pending_buffers_.empty()) {
        // vaRenderPicture() needs a non-const pointer, possibly unnecessarily.
        VABufferID* va_buffers_data = const_cast<VABufferID*>(pending_buffers_.data());
        va_res = vaRenderPicture(va_device_->display(), va_context_->id(), va_buffers_data, base::checked_cast<int>(pending_buffers_.size()));
...
VAProfile H265VaapiWrapper::GetProfile(const H265SPS* sps)
{
    ...
    case H265ProfileTierLevel::H265ProfileIdc::kProfileIdcScalableRangeExtensions:
    case H265ProfileTierLevel::H265ProfileIdc::kProfileIdcHighThroughputScreenContentCoding:
    default:
        LOG(FATAL) << "Invalid IDC profile " << sps->profile_tier_level.general_profile_idc;
    }
}
