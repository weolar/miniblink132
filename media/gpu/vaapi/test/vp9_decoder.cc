...
VAProfile GetProfile(Vp9FrameHeader frame_hdr)
{
    ...
    case 1:
        break;
    case 2:
        LOG_ASSERT(frame_hdr.bit_depth == 10) << "Only 10-bit streams are supported for VP9 profile 2";
...
Vp9Decoder::Vp9Decoder(std::unique_ptr<IvfParser> ivf_parser, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy)
    : VideoDecoder::VideoDecoder(va_device, fetch_policy)
    , vp9_parser_(std::make_unique<Vp9Parser>(/*parsing_compressed_header=*/false))
...
void Vp9Decoder::RefreshReferenceSlots(uint8_t refresh_frame_flags, scoped_refptr<SharedVASurface> surface)
{
...
VideoDecoder::Result Vp9Decoder::DecodeNextFrame()
{
    ...
    LOG_ASSERT(parser_res == Vp9Parser::kOk) << "Failed to parse next frame, got " << parser_res;
    ...
    if (frame_hdr.show_existing_frame) {
        ...
        LOG_ASSERT(last_decoded_surface_) << "got invalid surface as existing frame to decode";
    ...
    if (!va_context_ || (frame_hdr.IsKeyframe() && (va_context_->size().width() < size.width() || va_context_->size().height() < size.height()))) {
    ...
    for (size_t i = 0; i < std::size(pic_param.reference_frames); ++i) {
        pic_param.reference_frames[i] = ref_frames_[i] ? ref_frames_[i]->id() : VA_INVALID_SURFACE;
    }
...
#undef FHDR_TO_PP_PF1
    ...
    VAStatus res = vaCreateBuffer(
        va_device_->display(), va_context_->id(), VAPictureParameterBufferType, sizeof(VADecPictureParameterBufferVP9), 1u, &pic_param, &buffer_id);
...
#undef SEG_TO_SP_SF
    ...
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceParameterBufferType, sizeof(VASliceParameterBufferVP9), 1u, &slice_param, &buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    buffers.push_back(buffer_id);

    // Set up buffer for frame header.
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceDataBufferType, base::checked_cast<int>(frame_hdr.data.size()), 1u,
        reinterpret_cast<void*>(const_cast<uint8_t*>(frame_hdr.data.data())), &buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    buffers.push_back(buffer_id);
