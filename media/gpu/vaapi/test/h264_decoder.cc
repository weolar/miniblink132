...
bool FillH264PictureFromSliceHeader(const H264SPS* sps, const H264SliceHeader& slice_hdr, H264Picture* pic)
{
    ...
    if (slice_hdr.field_pic_flag) {
        pic->field = slice_hdr.bottom_field_flag ? H264Picture::FIELD_BOTTOM : H264Picture::FIELD_TOP;
    } else {
...
H264Decoder::H264Decoder(const uint8_t* stream_data, size_t stream_len, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy)
...
void H264Decoder::DecodeNextFrameInStream()
{
    ...
    while (true) {
        ...
        H264Parser::Result parser_result = parser_->AdvanceToNextNALU(curr_nalu_.get());
        if (parser_result == H264Parser::kEOStream) {
            is_stream_over_ = true;
            break;
        }

        if (curr_nalu_->nal_unit_type == H264NALU::kNonIDRSlice || curr_nalu_->nal_unit_type == H264NALU::kIDRSlice) {
            ExtractSliceHeader();
...
void H264Decoder::ProcessSlice()
{
    ...
    LOG_ASSERT(ModifyReferencePicLists(curr_slice_hdr_.get(), &ref_pic_list0, &ref_pic_list1)) << "Error modify reference pic lists!";

    const H264PPS* pic_params = parser_->GetPPS(curr_pps_id_);
    LOG_ASSERT(pic_params) << "No picture params for ID " << curr_pps_id_;

    va_wrapper_.SubmitSlice(pic_params, curr_slice_hdr_.get(), ref_pic_list0, ref_pic_list1, curr_picture_.get(), curr_slice_hdr_->nalu_data,
        curr_slice_hdr_->nalu_size, parser_->GetCurrentSubsamples());
}

void H264Decoder::UpdateSequenceParams()
{
    LOG_ASSERT(parser_->ParseSPS(&curr_sps_id_) == H264Parser::kOk) << "Error parsing SPS NALU";

    const H264SPS* sps = parser_->GetSPS(curr_sps_id_);
    LOG_ASSERT(sps) << "SPS not present for ID " << curr_sps_id_;

    const gfx::Rect new_visible_rect = sps->GetVisibleRect().value_or(gfx::Rect());
    ...
    LOG_ASSERT(std::numeric_limits<int>::max() / width_mb > height_mb) << "Picture size is too big: " << new_pic_size.ToString();
    ...
    VLOG(1) << "MaxDpbFrames: " << max_dpb_frames << ", max_num_ref_frames: " << sps->max_num_ref_frames
    ...
    LOG_ASSERT(max_dpb_size <= max_dpb_frames) << "Invalid stream, DPB size > MaxDpbFrames";
    LOG_ASSERT(max_dpb_size != 0 && max_dpb_size <= H264DPB::kDPBMaxSize) << "Invalid DPB size: " << max_dpb_size;
...
{
    LOG_ASSERT(parser_->ParsePPS(&curr_pps_id_) == H264Parser::kOk) << "Error parsing PPS NALU";
}
...
void H264Decoder::FinishPicture(scoped_refptr<H264Picture> pic)
{
    ...
    VLOG(4) << "Finishing picture frame_num: " << pic->frame_num << ", poc: " << pic->pic_order_cnt << ", entries in DPB: " << dpb_.size();
    ...
    while (num_remaining > max_num_reorder_frames_ ||
        ...
        VLOG_IF(1, num_remaining <= max_num_reorder_frames_) << "Invalid stream: max_num_reorder_frames not preserved";
...
{
    if (curr_slice_hdr_->frame_num != curr_picture_->frame_num || curr_slice_hdr_->pic_parameter_set_id != curr_pps_id_
        || curr_slice_hdr_->nal_ref_idc != curr_picture_->nal_ref_idc || curr_slice_hdr_->idr_pic_flag != curr_picture_->idr
        || (curr_slice_hdr_->idr_pic_flag && (curr_slice_hdr_->idr_pic_id != curr_picture_->idr_pic_id || curr_slice_hdr_->first_mb_in_slice == 0))) {
        return true;
    }
    ...
    if (sequence_params->pic_order_cnt_type == curr_picture_->pic_order_cnt_type) {
        if (curr_picture_->pic_order_cnt_type == 0) {
            if (curr_slice_hdr_->pic_order_cnt_lsb != curr_picture_->pic_order_cnt_lsb
                || curr_slice_hdr_->delta_pic_order_cnt_bottom != curr_picture_->delta_pic_order_cnt_bottom) {
                return true;
            }
        } else if (curr_picture_->pic_order_cnt_type == 1) {
            if (curr_slice_hdr_->delta_pic_order_cnt0 != curr_picture_->delta_pic_order_cnt0
                || curr_slice_hdr_->delta_pic_order_cnt1 != curr_picture_->delta_pic_order_cnt1) {
                return true;
            }
        }
    }
...
void H264Decoder::ExtractSliceHeader()
{
    ...
    H264Parser::Result result = parser_->ParseSliceHeader(*curr_nalu_, curr_slice_hdr_.get());
    LOG_ASSERT(result == H264Parser::kOk) << "Error parsing slice header! Parser returned " << result;
}
...
void H264Decoder::StartNewFrame()
{
    ...
    LOG_ASSERT(picture_params) << "Error extracting picture parameters for ID " << curr_pps_id_;

    curr_sps_id_ = picture_params->seq_parameter_set_id;
    const H264SPS* sequence_params = parser_->GetSPS(curr_sps_id_);
    LOG_ASSERT(sequence_params) << "Error extracting sequence parameters for ID" << curr_sps_id_;
    ...
    va_wrapper_.SubmitFrameMetadata(sequence_params, picture_params, dpb_, ref_pic_list_p0_, ref_pic_list_b0_, ref_pic_list_b1_, curr_picture_);
}

bool H264Decoder::InitCurrPicture(const H264SliceHeader* slice_hdr)
{
    if (!FillH264PictureFromSliceHeader(parser_->GetSPS(curr_sps_id_), *slice_hdr, curr_picture_.get())) {
        return false;
    }
    ...
    curr_picture_->adaptive_ref_pic_marking_mode_flag = slice_hdr->adaptive_ref_pic_marking_mode_flag;

    // If the slice header indicates we will have to perform reference marking
    // process after this picture is decoded, store required data for that
    // purpose.
    if (slice_hdr->adaptive_ref_pic_marking_mode_flag) {
        static_assert(sizeof(curr_picture_->ref_pic_marking) == sizeof(slice_hdr->ref_pic_marking), "Array sizes of ref pic marking do not match.");
        memcpy(curr_picture_->ref_pic_marking, slice_hdr->ref_pic_marking, sizeof(curr_picture_->ref_pic_marking));
    }
...
void H264Decoder::ConstructReferencePicListsB()
{
    ...
    iter = std::upper_bound(ref_pic_list_b0_.begin(), ref_pic_list_b0_.end(), curr_picture_.get(), POCAscCompare());
    ...
    // Find first with POC < curr_pic's POC to get first element in [2]...
    iter = std::upper_bound(ref_pic_list_b1_.begin(), ref_pic_list_b1_.end(), curr_picture_.get(), POCDescCompare());
...
bool H264Decoder::InitNonexistingPicture(scoped_refptr<H264Picture> pic, int frame_num, bool ref)
{
...
bool H264Decoder::HandleFrameNumGap(int frame_num)
{
    ...
    VLOG(2) << "Handling frame_num gap: " << prev_ref_frame_num_ << "->" << frame_num;
...
bool H264Decoder::ModifyReferencePicList(const H264SliceHeader* slice_hdr, int list, H264Picture::Vector* ref_pic_listx)
{
    ...
    for (int i = original_size; i < num_ref_idx_lX_active_minus1 + 1; i++) {
        scoped_refptr<H264Picture> nonref_pic = base::WrapRefCounted(new H264Picture(nullptr));
    ...
    for (int i = 0; i < H264SliceHeader::kRefListModSize && !done; ++i) {
        ...
        case 1:
            ...
            if (ref_idx_lx > num_ref_idx_lX_active_minus1) {
                VLOG(1) << "Bounds mismatch: expected " << ref_idx_lx << " <= " << num_ref_idx_lX_active_minus1;
                return false;
            }
        ...
        case 2:
            ...
            if (!pic) {
                VLOG(1) << "Malformed stream, no pic num " << list_mod->long_term_pic_num;
                return false;
            }
            ShiftRightAndInsert(ref_pic_listx, ref_idx_lx, num_ref_idx_lX_active_minus1, pic);
            ref_idx_lx++;
            ...
            for (int src = ref_idx_lx, dst = ref_idx_lx; src <= num_ref_idx_lX_active_minus1 + 1; ++src) {
                if ((*ref_pic_listx)[src] && LongTermPicNumF(*(*ref_pic_listx)[src]) != static_cast<int>(list_mod->long_term_pic_num))
                    (*ref_pic_listx)[dst++] = (*ref_pic_listx)[src];
            }
            break;
        ...
        default:
            // May be recoverable.
            VLOG(1) << "Invalid modification_of_pic_nums_idc=" << list_mod->modification_of_pic_nums_idc << " in position " << i;
            break;
        }
...
void H264Decoder::ShiftRightAndInsert(H264Picture::Vector* v, int from, int to, scoped_refptr<H264Picture> pic)
{
...
void H264Decoder::FlushDPB()
{
    ...
    std::sort(not_outputted_vec.begin(), not_outputted_vec.end(), POCDescCompare());
