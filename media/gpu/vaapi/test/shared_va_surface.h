...
public:
    ...
    static scoped_refptr<SharedVASurface> Create(const VaapiDevice& va_device, unsigned int va_rt_format, const gfx::Size& size, VASurfaceAttrib attribute);

    SharedVASurface(base::PassKey<SharedVASurface>, const VaapiDevice& va_device, VASurfaceID id, const gfx::Size& size, unsigned int format);
    ...
    unsigned int va_rt_format() const
    {
...
private:
    ...
    void FetchData(FetchPolicy fetch_policy, const VAImageFormat& format, VAImage* image, uint8_t** image_data) const;
