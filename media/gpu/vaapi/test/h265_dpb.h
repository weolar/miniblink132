...
#define LOG_PIC_INFO(func, pic, fetch_policy)                                                                                                                  \
    do {                                                                                                                                                       \
        VLOG(3) << func << "," << pic->pic_order_cnt_val_ << "," << pic->nal_unit_type_ << "," << pic->surface->GetMD5Sum(fetch_policy) << ","                 \
                << pic->pic_output_flag_ << "," << pic->no_output_of_prior_pics_flag_ << "," << pic->outputted_;                                               \
    } while (0)
...
public:
    ...
    {
        return ref_ == kShortTermCurrBefore || ref_ == kShortTermCurrAfter || ref_ == kShortTermFoll;
    }
    bool IsUnused() const
    {
        return ref_ == kUnused;
    ...
    void set_visible_rect(const gfx::Rect& rect)
    {
        visible_rect_ = rect;
    }
...
public:
    ...
    size_t max_num_pics() const
    {
        return max_num_pics_;
    }
    ...
    // term reference.
    void StorePicture(scoped_refptr<H265Picture> pic, H265Picture::ReferenceType ref);
    ...
    scoped_refptr<H265Picture> GetPicByPocAndMark(int poc, H265Picture::ReferenceType ref);
    ...
    scoped_refptr<H265Picture> GetPicByPocMaskedAndMark(int poc, int mask, H265Picture::ReferenceType ref);
    ...
    bool IsFull() const
    {
        return pics_.size() >= max_num_pics_;
    }
