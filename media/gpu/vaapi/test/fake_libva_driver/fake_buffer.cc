...
{
    base::CheckedNumeric<size_t> data_size(base::strict_cast<size_t>(size_per_element));
...
FakeBuffer::FakeBuffer(IdType id, VAContextID context, VABufferType type, unsigned int size_per_element, unsigned int num_elements, const void* data)
    : id_(id)
    , context_(context)
    , type_(type)
