...
#if defined(MINIGBM)
        PCHECK(HANDLE_EINTR(ioctl(plane.prime_fd.get(), DMA_BUF_IOCTL_SYNC, &sync_start)) == 0);
#else
...
#if defined(MINIGBM)
        PCHECK(HANDLE_EINTR(ioctl(plane.prime_fd.get(), DMA_BUF_IOCTL_SYNC, &sync_end)) == 0);
#else
...
ScopedBOMapping::ScopedBOMapping()
    : scoped_bo_mapping_factory_(nullptr)
    , bo_import_(nullptr)
{
}

ScopedBOMapping::ScopedBOMapping(ScopedBOMappingFactory* scoped_bo_mapping_factory, std::vector<Plane> planes, struct gbm_bo* bo_import)
...
ScopedBOMapping::~ScopedBOMapping()
{
    if (IsValid()) {
        ...
        scoped_bo_mapping_factory_->UnmapAndDestroyBufferObject(std::move(planes_), bo_import_);
    }
}

ScopedBOMapping::Plane::Plane(uint32_t stride, void* addr, void* mmap_data, int prime_fd)
    : stride(stride)
    , addr(addr)
    , mmap_data(mmap_data)
    , prime_fd(prime_fd)
...
ScopedBOMapping::Plane& ScopedBOMapping::Plane::operator=(ScopedBOMapping::Plane&& other)
{
    stride = other.stride;
    other.stride = 0u;
...
ScopedBOMapping ScopedBOMappingFactory::Create(gbm_import_fd_modifier_data import_data)
{
    base::AutoLock lock(lock_);
    struct gbm_bo* bo_import = gbm_bo_import(gbm_device_.get(), GBM_BO_IMPORT_FD_MODIFIER, &import_data, GBM_BO_USE_SW_READ_OFTEN | GBM_BO_USE_SW_WRITE_OFTEN);
    CHECK(bo_import);
    ...
    for (int plane = 0; plane < gbm_bo_get_plane_count(bo_import); plane++) {
        uint32_t stride;
        void* mmap_data;
        void* addr = gbm_bo_map2(
            bo_import, /*x=*/0, /*y=*/0, gbm_bo_get_width(bo_import), gbm_bo_get_height(bo_import), GBM_BO_TRANSFER_READ_WRITE, &stride, &mmap_data, plane);
...
void ScopedBOMappingFactory::UnmapAndDestroyBufferObject(std::vector<ScopedBOMapping::Plane> planes, struct gbm_bo* bo_import)
{
    base::AutoLock lock(lock_);
    CHECK(bo_import);
    CHECK(gbm_device_);
