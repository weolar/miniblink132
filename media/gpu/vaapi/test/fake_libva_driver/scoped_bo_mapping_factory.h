...
public:
    ...
    bool IsValid() const
    {
        return !!scoped_bo_mapping_factory_;
    }
...
private:
    ...
    ScopedBOMapping(ScopedBOMappingFactory* scoped_bo_mapping_factory, std::vector<Plane> planes, struct gbm_bo* bo_import);
...
private:
    ...
    void UnmapAndDestroyBufferObject(std::vector<ScopedBOMapping::Plane> planes, struct gbm_bo* bo_import);
