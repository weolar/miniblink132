...
std::unique_ptr<FakeImage> FakeImage::Create(IdType id, const VAImageFormat& format, int width, int height, FakeDriver& fake_driver, VAImage* va_image)
{
    ...
    base::CheckedNumeric<unsigned int> data_size(uv_offset.ValueOrDie<unsigned int>());
    ...
    return base::WrapUnique(new FakeImage(id, format, width, height, std::move(planes), fake_driver.GetBuffer(buf), fake_driver));
}

FakeImage::FakeImage(
    FakeImage::IdType id, const VAImageFormat& format, int width, int height, std::vector<Plane> planes, const FakeBuffer& buffer, FakeDriver& driver)
    : id_(id)
    , format_(format)
    , width_(width)
    , height_(height)
