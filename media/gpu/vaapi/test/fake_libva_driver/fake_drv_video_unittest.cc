...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(VA_STATUS_SUCCESS, vaInitialize(display_, &major_version, &minor_version));
    }
...
TEST_F(FakeDriverTest, VerifyQueryConfigProfiles)
{
    ...
    std::vector<VAProfile> va_profiles(base::checked_cast<size_t>(vaMaxNumProfiles(display_)));
    int num_va_profiles;

    const VAStatus va_res = vaQueryConfigProfiles(display_, va_profiles.data(), &num_va_profiles);
...
TEST_F(FakeDriverTest, CanCreateConfig)
{
    ...
    const VAStatus va_res = vaCreateConfig(display_, VAProfileVP8Version0_3, VAEntrypointVLD,
        /*attrib_list=*/nullptr,
...
TEST_F(FakeDriverTest, QueryConfigAttributesForValidConfigID)
{
    ...
    base::FixedArray<VAConfigAttrib> config_attribs(base::checked_cast<size_t>(vaMaxNumConfigAttributes(display_)));
    memset(config_attribs.data(), 0, config_attribs.memsize());
    int num_attribs = 0;
    const VAStatus va_res = vaQueryConfigAttributes(display_, config_id, &profile, &entrypoint, config_attribs.data(), &num_attribs);
    ASSERT_EQ(VA_STATUS_SUCCESS, va_res);
...
TEST_F(FakeDriverTest, QueryConfigAttributesCrashesForInvalidConfigID)
{
    ...
    memset(config_attribs.data(), 0, config_attribs.memsize());
    int num_attribs;
    EXPECT_DEATH(vaQueryConfigAttributes(display_, /*config_id=*/0, &profile, &entrypoint, config_attribs.data(), &num_attribs);, "");
}
...
TEST_F(FakeDriverTest, CreateContextForValidConfigID)
{
    ...
    const VAStatus va_res = vaCreateContext(display_, config_id, /*picture_width=*/1280, /*picture_height=*/720,
...
{
    VAContextID context_id = VA_INVALID_ID;
    EXPECT_DEATH(vaCreateContext(display_, /*config_id=*/0, /*picture_width=*/1280,
...
TEST_F(FakeDriverTest, QuerySurfaceAttributesForValidConfigID)
{
    ...
    const VAStatus va_res = vaQuerySurfaceAttributes(display_, config_id, surface_attribs, &num_attribs);
    ASSERT_EQ(VA_STATUS_SUCCESS, va_res);
...
TEST_F(FakeDriverTest, CanCreateSurfaces)
{
    ...
    const VAStatus va_res = vaCreateSurfaces(display_, /*format=*/VA_RT_FORMAT_YUV420, /*width=*/1280,
...
TEST_F(FakeDriverTest, CanBeginPictureForValidSurfaceID)
{
    ...
    ASSERT_EQ(VA_STATUS_SUCCESS,
        vaCreateContext(display_, config_id, /*picture_width=*/1280,
            /*picture_height=*/720, /*flag=*/0, surfaces, kNumSurfaces, &context_id));
...
TEST_F(FakeDriverTest, CanGetImageForValidSurfaceID)
{
    ...
    VAImageFormat image_format_nv12 { .fourcc = VA_FOURCC_NV12, .byte_order = VA_LSB_FIRST, .bits_per_pixel = 12 };
...
TEST_F(FakeDriverTest, CanPutImageForValidSurfaceID)
{
    ...
    for (unsigned int i = 0; i < kNumSurfaces; i++) {
        // TODO(b/258275488): Provide a valid Image ID once that functionality is
        // implemented.
        const VAStatus va_res = vaPutImage(display_, surfaces[i], /*image=*/0, /*src_x=*/0, /*src_y=*/0,
...
TEST_F(FakeDriverTest, CanCreateBufferForValidContextID)
{
    ...
    ASSERT_EQ(VA_STATUS_SUCCESS,
        vaCreateSurfaces(display_, /*format=*/VA_RT_FORMAT_YUV420,
            /*width=*/1280, /*height=*/720, surfaces, kNumSurfaces, /*surface_attribs=*/nullptr,
            /*num_attribs=*/0));
...
TEST_F(FakeDriverTest, CanRenderPictureForValidContextID)
{
    ...
    // implemented.
    const VAStatus va_res = vaRenderPicture(display_, context_id, /*buf_id=*/nullptr, /*num_buffers=*/0);
    EXPECT_EQ(VA_STATUS_SUCCESS, va_res);
}
