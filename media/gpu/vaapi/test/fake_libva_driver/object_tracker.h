...
public:
    ...
    template <class... Args> typename T::IdType CreateObject(Args&&... args)
    {
        ...
        do {
            ...
            next_id_++;
        } while (std::find_if(objects_.begin(), objects_.end(), [this](const std::unique_ptr<T>& it) EXCLUSIVE_LOCKS_REQUIRED(lock_) {
    ...
    bool ObjectExists(typename T::IdType id)
    {
        ...
        return std::find_if(objects_.begin(), objects_.end(), [id](const std::unique_ptr<T>& it) { return it->GetID() == id; }) != objects_.end();
    }
    ...
    const T& GetObject(typename T::IdType id)
    {
        ...
        auto object = std::find_if(objects_.begin(), objects_.end(), [id](const std::unique_ptr<T>& it) { return it->GetID() == id; });
...
private:
    ...
    // and |args|.
    template <class... Args, typename std::enable_if<std::is_constructible_v<T, typename T::IdType, Args...>, bool>::type = true>
    ...
    typename T::IdType GUARDED_BY(lock_) next_id_ = std::numeric_limits<typename T::IdType>::min();
};
