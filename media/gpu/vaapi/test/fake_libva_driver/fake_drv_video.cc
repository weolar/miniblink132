...
const VAImageFormat kSupportedImageFormats[] = { { .fourcc = VA_FOURCC_NV12, .byte_order = VA_LSB_FIRST, .bits_per_pixel = 12 } };
...
const struct Capability kCapabilities[] = { { VAProfileAV1Profile0, VAEntrypointVLD, 1,
    ...
    { VAProfileVP8Version0_3, VAEntrypointEncSlice, 3,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 },
    ...
    { VAProfileVP9Profile0, VAEntrypointEncSlice, 3,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 },
            { VAConfigAttribRateControl, VA_RC_CQP | VA_RC_CBR },
            { VAConfigAttribEncMaxRefFrames, 1 },
    ...
    { VAProfileVP9Profile2, VAEntrypointEncSlice, 3,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 | VA_RT_FORMAT_YUV420_10BPP },
            { VAConfigAttribRateControl, VA_RC_CQP | VA_RC_CBR },
            { VAConfigAttribEncMaxRefFrames, 1 },
    ...
    { VAProfileH264ConstrainedBaseline, VAEntrypointEncSlice, 4,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 },
            { VAConfigAttribRateControl, VA_RC_CQP | VA_RC_CBR },
    ...
    { VAProfileH264Main, VAEntrypointEncSlice, 4,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 },
            { VAConfigAttribRateControl, VA_RC_CQP | VA_RC_CBR },
            { VAConfigAttribEncPackedHeaders, VA_ENC_PACKED_HEADER_NONE },
    ...
    { VAProfileH264High, VAEntrypointEncSlice, 4,
        {
            { VAConfigAttribRTFormat, VA_RT_FORMAT_YUV420 },
            { VAConfigAttribRateControl, VA_RC_CQP | VA_RC_CBR },
            { VAConfigAttribEncPackedHeaders, VA_ENC_PACKED_HEADER_NONE },
 ...
 */
VAStatus FakeQueryConfigProfiles(VADriverContextP ctx, VAProfile* profile_list, int* num_profiles)
{
    int i = 0;
 ...
 */
VAStatus FakeQueryConfigEntrypoints(VADriverContextP ctx, VAProfile profile, VAEntrypoint* entrypoint_list, int* num_entrypoints)
{
    *num_entrypoints = 0;
 ...
 */
VAStatus FakeGetConfigAttributes(VADriverContextP ctx, VAProfile profile, VAEntrypoint entrypoint, VAConfigAttrib* attrib_list, int num_attribs)
{
    ...
    for (const auto& capability : kCapabilities) {
        ...
        if (!(capability.profile == profile && capability.entry_point == entrypoint)) {
            continue;
        }
    ...
    return profile_found ? VA_STATUS_ERROR_UNSUPPORTED_ENTRYPOINT : VA_STATUS_ERROR_UNSUPPORTED_PROFILE;
}

VAStatus FakeCreateConfig(VADriverContextP ctx, VAProfile profile, VAEntrypoint entrypoint, VAConfigAttrib* attrib_list, int num_attribs, VAConfigID* config_id)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
    ...
    for (size_t i = 0; i < kCapabilitiesSize; ++i) {
        ...
        for (int k = 0; k < num_attribs; k++) {
            ...
            for (int j = 0; j < kCapabilities[i].num_attribs; j++) {
                ...
                attrib_supported
                    = (kCapabilities[i].attrib_list[j].value & attrib_list[k].value) || (kCapabilities[i].attrib_list[j].value == attrib_list[k].value);
        ...
        for (const auto& capability_attrib : kCapabilities[i].attrib_list) {
            if (std::find_if(
                    attribs.begin(), attribs.end(), [&capability_attrib](const VAConfigAttrib& attrib) { return attrib.type == capability_attrib.type; })
                == attribs.end()) {
...
VAStatus FakeQueryConfigAttributes(
    VADriverContextP ctx, VAConfigID config_id, VAProfile* profile, VAEntrypoint* entrypoint, VAConfigAttrib* attrib_list, int* num_attribs)
{
    ...
    *entrypoint = fconfig.GetEntrypoint();
    const size_t fconfig_attribs_size_in_bytes
        = base::CheckMul(sizeof(VAConfigAttrib), base::strict_cast<size_t>(fconfig.GetConfigAttribs().size())).ValueOrDie<size_t>();
...
VAStatus FakeCreateSurfaces(VADriverContextP ctx, int width, int height, int format, int num_surfaces, VASurfaceID* surfaces)
{
    CHECK(false);
...
VAStatus FakeDestroySurfaces(VADriverContextP ctx, VASurfaceID* surface_list, int num_surfaces)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeCreateContext(VADriverContextP ctx, VAConfigID config_id, int picture_width, int picture_height, int flag, VASurfaceID* render_targets,
    int num_render_targets, VAContextID* context)
{
    ...
    *context = fdrv->CreateContext(config_id, picture_width, picture_height, flag,
        std::vector<VASurfaceID>(render_targets, render_targets + base::checked_cast<size_t>(num_render_targets)));
...
VAStatus FakeBufferSetNumElements(VADriverContextP ctx, VABufferID buf_id, unsigned int num_elements)
{
    return VA_STATUS_SUCCESS;
}
...
VAStatus FakeBeginPicture(VADriverContextP ctx, VAContextID context, VASurfaceID render_target)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeRenderPicture(VADriverContextP ctx, VAContextID context, VABufferID* buffers, int num_buffers)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeQuerySurfaceStatus(VADriverContextP ctx, VASurfaceID render_target, VASurfaceStatus* status)
{
    CHECK(false);
...
VAStatus FakePutSurface(VADriverContextP ctx, VASurfaceID surface, void* draw, short srcx, short srcy, unsigned short srcw, unsigned short srch, short destx,
    short desty, unsigned short destw, unsigned short desth, VARectangle* cliprects, unsigned int number_cliprects, unsigned int flags)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeQueryImageFormats(VADriverContextP ctx, VAImageFormat* format_list, int* num_formats)
{
    int i = 0;
...
VAStatus FakeCreateImage(VADriverContextP ctx, VAImageFormat* format, int width, int height, VAImage* image)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeSetImagePalette(VADriverContextP ctx, VAImageID image, unsigned char* palette)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeGetImage(VADriverContextP ctx, VASurfaceID surface, int x, int y, unsigned int width, unsigned int height, VAImageID image)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
    ...
    const media::internal::ScopedBOMapping::ScopedAccess mapped_bo = fake_surface.GetMappedBO().BeginAccess();
    ...
    uint8_t* const dst_y_addr = static_cast<uint8_t*>(fake_image.GetBuffer().GetData()) + fake_image.GetPlaneOffset(0);
    const int dst_y_stride = base::checked_cast<int>(fake_image.GetPlaneStride(0));

    uint8_t* const dst_uv_addr = static_cast<uint8_t*>(fake_image.GetBuffer().GetData()) + fake_image.GetPlaneOffset(1);
    const int dst_uv_stride = base::checked_cast<int>(fake_image.GetPlaneStride(1));
...
VAStatus FakePutImage(VADriverContextP ctx, VASurfaceID surface, VAImageID image, int src_x, int src_y, unsigned int src_width, unsigned int src_height,
...
VAStatus FakeDeriveImage(VADriverContextP ctx, VASurfaceID surface, VAImage* image)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeQuerySubpictureFormats(VADriverContextP ctx, VAImageFormat* format_list, unsigned int* flags, unsigned int* num_formats)
{
    *num_formats = 0;
    return VA_STATUS_SUCCESS;
}

VAStatus FakeCreateSubpicture(VADriverContextP ctx, VAImageID image, VASubpictureID* subpicture)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeDestroySubpicture(VADriverContextP ctx, VASubpictureID subpicture)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeSetSubpictureImage(VADriverContextP ctx, VASubpictureID subpicture, VAImageID image)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeSetSubpictureChromakey(
    VADriverContextP ctx, VASubpictureID subpicture, unsigned int chromakey_min, unsigned int chromakey_max, unsigned int chromakey_mask)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeSetSubpictureGlobalAlpha(VADriverContextP ctx, VASubpictureID subpicture, float global_alpha)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeAssociateSubpicture(VADriverContextP ctx, VASubpictureID subpicture, VASurfaceID* target_surfaces, int num_surfaces, int16_t src_x, int16_t src_y,
    uint16_t src_width, uint16_t src_height, int16_t dest_x, int16_t dest_y, uint16_t dest_width, uint16_t dest_height, uint32_t flags)
{
    CHECK(false);
...
VAStatus FakeDeassociateSubpicture(VADriverContextP ctx, VASubpictureID subpicture, VASurfaceID* target_surfaces, int num_surfaces)
{
    CHECK(false);
...
VAStatus FakeQueryDisplayAttributes(VADriverContextP ctx, VADisplayAttribute* attr_list, int* num_attributes)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeGetDisplayAttributes(VADriverContextP ctx, VADisplayAttribute* attr_list, int num_attributes)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeSetDisplayAttributes(VADriverContextP ctx, VADisplayAttribute* attr_list, int num_attributes)
{
    return VA_STATUS_SUCCESS;
}

VAStatus FakeQuerySurfaceAttributes(VADriverContextP ctx, VAConfigID config, VASurfaceAttrib* attribs, unsigned int* num_attribs)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);
...
VAStatus FakeCreateSurfaces2(VADriverContextP ctx, unsigned int format, unsigned int width, unsigned int height, VASurfaceID* surfaces,
    unsigned int num_surfaces, VASurfaceAttrib* attrib_list, unsigned int num_attribs)
{
    media::internal::FakeDriver* fdrv = static_cast<media::internal::FakeDriver*>(ctx->pDriverData);

    for (unsigned int i = 0; i < num_surfaces; i++) {
        surfaces[i] = fdrv->CreateSurface(format, width, height, std::vector<VASurfaceAttrib>(attrib_list, attrib_list + num_attribs));
    }
...
extern "C" VAStatus DLL_EXPORT __vaDriverInit_1_0(VADriverContextP ctx)
{
    ...
    ctx->pDriverData = new media::internal::FakeDriver((static_cast<drm_state*>(ctx->drm_state))->fd);
