...
{
    return bo->meta.strides[plane] * ALIGN(bo->meta.height, get_y_subsample(bo, plane)) / get_y_subsample(bo, plane);
}
...
extern "C" GBM_EXPORT struct gbm_bo* gbm_bo_create(struct gbm_device* gbm, uint32_t width, uint32_t height, uint32_t format, uint32_t flags)
{
    ...
    for (int i = 0; i < gbm_bo_get_plane_count(bo); i++) {
        ...
        bo->meta.strides[i] = ALIGN(width, get_x_subsample(bo, i)) / get_x_subsample(bo, i) * gbm_bo_get_bpp(bo);
...
extern "C" GBM_EXPORT void* gbm_bo_map2(
    struct gbm_bo* bo, uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t transfer_flags, uint32_t* stride, void** map_data, int plane)
{
    CHECK(bo);
    ...
    void* addr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, bo->meta.fds[0], bo->meta.offsets[plane]);
    CHECK(addr);
    CHECK(addr != MAP_FAILED);

    struct gbm_bo_mapping** _map_data = reinterpret_cast<struct gbm_bo_mapping**>(map_data);
    *_map_data = new struct gbm_bo_mapping;
    (*_map_data)->addr = addr;
    (*_map_data)->size = size;
    *stride = bo->meta.strides[plane];

    size_t offset = y / get_y_subsample(bo, plane) * bo->meta.strides[plane] + x / get_x_subsample(bo, plane) * gbm_bo_get_bpp(bo);
...
extern "C" GBM_EXPORT struct gbm_bo* gbm_bo_import(struct gbm_device* gbm, uint32_t type, void* buffer, uint32_t usage)
{
    CHECK(buffer);
...
extern "C" GBM_EXPORT int gbm_bo_get_fd_for_plane(struct gbm_bo* bo, size_t plane)
{
    CHECK(bo);
    CHECK(static_cast<int>(plane) < gbm_bo_get_plane_count(bo));
...
extern "C" GBM_EXPORT uint32_t gbm_bo_get_offset(struct gbm_bo* bo, size_t plane)
{
    CHECK(bo);
    CHECK(static_cast<int>(plane) < gbm_bo_get_plane_count(bo));
...
extern "C" GBM_EXPORT uint32_t gbm_bo_get_stride_for_plane(struct gbm_bo* bo, size_t plane)
{
    CHECK(bo);
    CHECK(static_cast<int>(plane) < gbm_bo_get_plane_count(bo));
