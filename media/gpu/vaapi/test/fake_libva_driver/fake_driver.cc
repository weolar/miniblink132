...
FakeDriver::FakeDriver(int drm_fd)
    : scoped_bo_mapping_factory_(drm_fd)
{
}

FakeDriver::~FakeDriver() = default;

FakeConfig::IdType FakeDriver::CreateConfig(VAProfile profile, VAEntrypoint entrypoint, std::vector<VAConfigAttrib> attrib_list)
{
...
FakeSurface::IdType FakeDriver::CreateSurface(unsigned int format, unsigned int width, unsigned int height, std::vector<VASurfaceAttrib> attrib_list)
{
    return surface_.CreateObject(format, width, height, std::move(attrib_list), scoped_bo_mapping_factory_);
}
...
FakeContext::IdType FakeDriver::CreateContext(VAConfigID config_id, int picture_width, int picture_height, int flag, std::vector<VASurfaceID> render_targets)
{
    return context_.CreateObject(GetConfig(config_id), picture_width, picture_height, flag, std::move(render_targets));
}
...
FakeBuffer::IdType FakeDriver::CreateBuffer(VAContextID context, VABufferType type, unsigned int size_per_element, unsigned int num_elements, const void* data)
{
...
void FakeDriver::CreateImage(const VAImageFormat& format, int width, int height, VAImage* va_image)
{
