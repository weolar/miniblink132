...
VpxDecoderDelegate::VpxDecoderDelegate(int picture_width_hint, int picture_height_hint, VAProfile profile)
{
    ...
    const vpx_codec_err_t status
        = vpx_codec_dec_init(vpx_codec_.get(), (profile == VAProfileVP8Version0_3) ? vpx_codec_vp8_dx() : vpx_codec_vp9_dx(), &vpx_config, /*flags=*/0);
...
void VpxDecoderDelegate::EnqueueWork(const std::vector<raw_ptr<const FakeBuffer>>& buffers)
{
...
void VpxDecoderDelegate::Run()
{
    CHECK(render_target_);
    CHECK(encoded_data_buffer_);
    const vpx_codec_err_t status = vpx_codec_decode(vpx_codec_.get(), static_cast<uint8_t*>(encoded_data_buffer_->GetData()),
    ...
    CHECK_EQ(render_target_->GetVAFourCC(), static_cast<uint32_t>(VA_FOURCC_NV12));
