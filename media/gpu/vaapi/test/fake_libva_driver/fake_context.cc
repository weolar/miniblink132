...
std::unique_ptr<media::internal::ContextDelegate> CreateDelegate(const media::internal::FakeConfig& config, int picture_width, int picture_height)
{
    std::unique_ptr<base::Environment> env = base::Environment::Create();
    CHECK(env);
    std::string no_op_flag;
    if (env->GetVar("USE_NO_OP_CONTEXT_DELEGATE", &no_op_flag) && no_op_flag == "1") {
    ...
    case VAProfileVP9Profile0:
        return std::make_unique<media::internal::VpxDecoderDelegate>(picture_width, picture_height, config.GetProfile());
    case VAProfileAV1Profile0:
        return std::make_unique<media::internal::Av1DecoderDelegate>(config.GetProfile());
    default:
        break;
    }
...
FakeContext::FakeContext(
    FakeContext::IdType id, const FakeConfig& config, int picture_width, int picture_height, int flag, std::vector<VASurfaceID> render_targets)
    : id_(id)
    , config_(config)
...
void FakeContext::RenderPicture(const std::vector<raw_ptr<const FakeBuffer>>& buffers) const
{
    CHECK(delegate_);
