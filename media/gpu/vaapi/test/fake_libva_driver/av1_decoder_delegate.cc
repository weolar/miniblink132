...
Av1DecoderDelegate::Av1DecoderDelegate(VAProfile profile)
{
    ...
    dav1d_context_ = std::unique_ptr<Dav1dContext, Dav1dContextDeleter>(temp_david_context_pointer);
}
...
void Av1DecoderDelegate::EnqueueWork(const std::vector<raw_ptr<const FakeBuffer>>& buffers)
{
...
void Av1DecoderDelegate::Run()
{
    ...
    CHECK_EQ(dav1d_data_wrap(input_buffer.get(), static_cast<uint8_t*>(encoded_data_buffer_->GetData()), encoded_data_buffer_->GetDataSize(),
