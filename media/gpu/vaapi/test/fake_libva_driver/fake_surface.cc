...
FakeSurface::FakeSurface(FakeSurface::IdType id, unsigned int format, uint32_t va_fourcc, unsigned int width, unsigned int height,
    std::vector<VASurfaceAttrib> attrib_list, ScopedBOMapping mapped_bo)
    : id_(id)
    , format_(format)
...
std::unique_ptr<FakeSurface> FakeSurface::Create(IdType id, unsigned int format, unsigned int width, unsigned int height,
    std::vector<VASurfaceAttrib> attrib_list, ScopedBOMappingFactory& scoped_bo_mapping_factory)
{
    // There are no specified attributes to this surface
    if (attrib_list.empty()) {
        NOTIMPLEMENTED();
        return base::WrapUnique(new FakeSurface(id, format, /*va_fourcc=*/0u, width, height, std::move(attrib_list),
            /*mapped_bo=*/ {}));
    }
    ...
    const unsigned int expected_rt_format = surf_desc->fourcc == VA_FOURCC_NV12 ? VA_RT_FORMAT_YUV420 : VA_RT_FORMAT_YUV420_10;

    const uint32_t expected_drm_format = surf_desc->fourcc == VA_FOURCC_NV12 ? DRM_FORMAT_NV12 : DRM_FORMAT_P010;
    ...
    CHECK_LE(surf_desc->num_objects, base::checked_cast<uint32_t>(std::size(surf_desc->objects)));
    ...
    for (uint32_t plane = 0u; plane < expected_num_planes; ++plane) {
        ...
        fd_data.strides[plane] = base::checked_cast<int>(surf_desc->layers[0].pitch[plane]);
        fd_data.offsets[plane] = base::checked_cast<int>(surf_desc->layers[0].offset[plane]);
    }

    ScopedBOMapping mapped_bo = scoped_bo_mapping_factory.Create(fd_data);
    CHECK(!!mapped_bo);
    return base::WrapUnique(new FakeSurface(id, format, surf_desc->fourcc, width, height, std::move(attrib_list), std::move(mapped_bo)));
}
