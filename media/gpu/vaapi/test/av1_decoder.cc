...
{
    LOG_ASSERT(!color_config.is_monochrome) << "Monochrome color config is not supported.";
    ...
    if (color_config.subsampling_x == 1u && color_config.subsampling_y == 1u) {
        ...
        if (color_config.bitdepth == 8) {
            ...
            NOTREACHED() << "Unsupported color config with chroma subsampling of bitdepth %d" << color_config.bitdepth;
        }
    }
...
Av1Decoder::Av1Decoder(std::unique_ptr<IvfParser> ivf_parser, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy)
...
void Av1Decoder::RefreshReferenceSlots(const uint8_t refresh_frame_flags, scoped_refptr<SharedVASurface> surface, libgav1::RefCountedBufferPtr current_frame,
    scoped_refptr<SharedVASurface> display_surface)
{
    ...
    state_->UpdateReferenceFrames(current_frame, base::strict_cast<int>(refresh_frame_flags));
}
...
VideoDecoder::Result Av1Decoder::DecodeNextFrame()
{
    ...
    if (parser_res != ParsingResult::kOk) {
        LOG_ASSERT(parser_res == ParsingResult::kEOStream) << "Failed to parse next frame, got " << static_cast<int>(parser_res);
    ...
    if (obu_parser_->sequence_header_changed()) {
        if (current_frame_header.frame_type != libgav1::kFrameKey || !current_frame_header.show_frame || current_frame_header.show_existing_frame
            || current_frame->temporal_id() != 0) {
            // Section 7.5.
            LOG(FATAL) << "The first frame successive to sequence header OBU must be a "
        ...
        LOG_ASSERT(current_sequence_header_->profile == libgav1::BitstreamProfile::kProfile0) << "Unsupported profile.";
        ...
        LOG_ASSERT(current_sequence_header_->operating_point_idc[0] == 0) << "Either temporal or spatial layer decoding is not supported.";

        if (!va_config_ || va_config_->profile() != new_profile) {
            va_context_.reset();
            libgav1::ColorConfig color_config = current_sequence_header_.value().color_config;
            va_config_ = std::make_unique<ScopedVAConfig>(*va_device_, new_profile, GetFormatForColorConfig(color_config));
        }
        ...
        const gfx::Size new_max_frame_size(
            base::strict_cast<int>(current_sequence_header_->max_frame_width), base::strict_cast<int>(current_sequence_header_->max_frame_height));
        if (!va_context_ || va_context_->size() != new_max_frame_size) {
            VLOG(1) << "New context size needed";
            VLOG_IF(1, va_context_) << "Previous context size: " << va_context_->size().ToString();
            VLOG(1) << "New context size: " << new_max_frame_size.ToString();
            va_context_ = std::make_unique<ScopedVAContext>(*va_device_, *va_config_, new_max_frame_size);
        }
    }
    ...
    if (current_frame_header.show_existing_frame) {
        last_decoded_surface_ = display_surfaces_[current_frame_header.frame_to_show];
        RefreshReferenceSlots(current_frame_header.refresh_frame_flags, last_decoded_surface_, current_frame, last_decoded_surface_);
    ...
    const gfx::Size visible_size(base::strict_cast<int>(current_frame->frame_width()), base::strict_cast<int>(current_frame->frame_height()));
    ...
    scoped_refptr<SharedVASurface> surface = SharedVASurface::Create(*va_device_, va_config_->va_rt_format(), visible_size, attribute);
    ...
    pic_parameters.profile = base::strict_cast<uint8_t>(current_sequence_header_->profile);

    if (current_sequence_header_->enable_order_hint) {
        pic_parameters.order_hint_bits_minus_1 = current_sequence_header_->order_hint_bits - 1;
    }
    ...
    default:
        ...
        NOTREACHED() << "Invalid color bit depth: " << current_sequence_header_->color_config.bitdepth;
    }
    pic_parameters.matrix_coefficients = base::checked_cast<uint8_t>(current_sequence_header_->color_config.matrix_coefficients);
#define COPY_SEQ_FIELD(a) pic_parameters.seq_info_fields.fields.a = current_sequence_header_->a
#define COPY_SEQ_FIELD2(a, b) pic_parameters.seq_info_fields.fields.a = b
    ...
    COPY_SEQ_FIELD2(mono_chrome, current_sequence_header_->color_config.is_monochrome);
    COPY_SEQ_FIELD2(subsampling_x, current_sequence_header_->color_config.subsampling_x);
    COPY_SEQ_FIELD2(subsampling_y, current_sequence_header_->color_config.subsampling_y);
    COPY_SEQ_FIELD(film_grain_params_present);
#undef COPY_SEQ_FIELD
    COPY_SEQ_FIELD2(color_range, base::strict_cast<uint32_t>(current_sequence_header_->color_config.color_range));
#undef COPY_SEQ_FIELD2
    ...
    if (current_frame_header.film_grain_params.apply_grain) {
        ...
        film_grain_surface = SharedVASurface::Create(*va_device_, va_config_->va_rt_format(), visible_size, attribute);
    ...
    for (size_t i = 0; i < kAv1NumRefFrames; ++i) {
        pic_parameters.ref_frame_map[i] = ref_frames_[i] ? ref_frames_[i]->id() : VA_INVALID_SURFACE;
    }
    ...
    pic_parameters.primary_ref_frame = base::checked_cast<uint8_t>(current_frame_header.primary_reference_frame);
    pic_parameters.order_hint = current_frame_header.order_hint;

    LOG_ASSERT(!current_frame_header.use_superres) << "Upscaling (use_superres=1) is not supported";

    FillSegmentInfo(pic_parameters.seg_info, current_frame_header.segmentation);
    FillFilmGrainInfo(pic_parameters.film_grain_info, current_frame_header.film_grain_params);

    const bool tile_info_success = FillTileInfo(pic_parameters, current_frame_header.tile_info);
    LOG_ASSERT(tile_info_success) << "Failed to fill tile info for current frame.";

    auto& info_fields = pic_parameters.pic_info_fields.bits;
    info_fields.uniform_tile_spacing_flag = current_frame_header.tile_info.uniform_spacing;
...
#undef COPY_PIC_FIELD
    info_fields.frame_type = base::strict_cast<uint32_t>(current_frame_header.frame_type);
    info_fields.disable_cdf_update = !current_frame_header.enable_cdf_update;
    info_fields.disable_frame_end_update_cdf = !current_frame_header.enable_frame_end_update_cdf;

    pic_parameters.superres_scale_denominator = current_frame_header.superres_scale_denominator;
    pic_parameters.interp_filter = base::strict_cast<uint8_t>(current_frame_header.interpolation_filter);
    ...
    FillLoopRestorationInfo(pic_parameters, current_frame_header.loop_restoration);
    ...
    VAStatus res = vaCreateBuffer(
        va_device_->display(), va_context_->id(), VAPictureParameterBufferType, sizeof(VADecPictureParameterBufferAV1), 1u, &pic_parameters, &buffer_id);
    ...
    const bool slice_parameters_success
        = FillAV1SliceParameters(obu_parser_->tile_buffers(), tile_columns, base::make_span(ivf_frame_data_, ivf_frame_header_.frame_size), slice_params);
    LOG_ASSERT(slice_parameters_success) << "Failed to fill slice parameters for current frame.";

    // Set up a buffer for the slice parameters for each slice.
    for (auto& slice_param : slice_params) {
        res = vaCreateBuffer(
            va_device_->display(), va_context_->id(), VASliceParameterBufferType, sizeof(VASliceParameterBufferAV1), 1u, &slice_param, &buffer_id);
        VA_LOG_ASSERT(res, "vaCreateBuffer");
        buffers.push_back(buffer_id);
    }
    ...
    res = vaCreateBuffer(
        va_device_->display(), va_context_->id(), VASliceDataBufferType, ivf_frame_header_.frame_size, 1u, const_cast<uint8_t*>(ivf_frame_data_), &buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    buffers.push_back(buffer_id);
    ...
    res = vaRenderPicture(va_device_->display(), va_context_->id(), buffers.data(), buffers.size());
    ...
    RefreshReferenceSlots(current_frame_header.refresh_frame_flags, surface, current_frame, last_decoded_surface_);
