...
ScopedVAContext::ScopedVAContext(const VaapiDevice& device, const ScopedVAConfig& config, const gfx::Size& size)
    : device_(device)
    , config_(config)
    , context_id_(VA_INVALID_ID)
    , size_(size)
{
    const VAStatus res = vaCreateContext(device_->display(), config_->id(), size_.width(), size_.height(), VA_PROGRESSIVE,
        /*render_targets=*/nullptr,
        /*num_render_targets=*/0, &context_id_);
    VA_LOG_ASSERT(res, "vaCreateContext");
    LOG_ASSERT(context_id_ != VA_INVALID_ID) << "vaCreateContext created invalid context ID";
