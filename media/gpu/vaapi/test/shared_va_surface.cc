...
bool DeriveImage(VADisplay display, VASurfaceID surface_id, VAImage* image, uint8_t** image_data)
{
...
VAImageFormat GetImageFormat(unsigned int va_rt_format)
{
    ...
    constexpr VAImageFormat kImageFormatP010 { .fourcc = VA_FOURCC_P010, .byte_order = VA_LSB_FIRST, .bits_per_pixel = 16 };
    switch (va_rt_format) {
...
void GetSurfaceImage(VADisplay display, VASurfaceID surface_id, VAImageFormat format, const gfx::Size size, VAImage* image, uint8_t** image_data)
{
    VAStatus res = vaCreateImage(display, &format, size.width(), size.height(), image);
    VA_LOG_ASSERT(res, "vaCreateImage");

    res = vaGetImage(display, surface_id, 0, 0, size.width(), size.height(), image->image_id);
...
SharedVASurface::SharedVASurface(base::PassKey<SharedVASurface>, const VaapiDevice& va_device, VASurfaceID id, const gfx::Size& size, unsigned int format)
    : va_device_(va_device)
    , id_(id)
    , size_(size)
    , va_rt_format_(format)
{
}

// static
scoped_refptr<SharedVASurface> SharedVASurface::Create(const VaapiDevice& va_device, unsigned int va_rt_format, const gfx::Size& size, VASurfaceAttrib attrib)
{
    VASurfaceID surface_id;
    VAStatus res = vaCreateSurfaces(va_device.display(), va_rt_format, base::checked_cast<unsigned int>(size.width()),
        base::checked_cast<unsigned int>(size.height()), &surface_id, 1u, &attrib, 1u);
    VA_LOG_ASSERT(res, "vaCreateSurfaces");
    VLOG(1) << "created surface: " << surface_id;
    return base::MakeRefCounted<SharedVASurface>(base::PassKey<SharedVASurface>(), va_device, surface_id, size, va_rt_format);
}

SharedVASurface::~SharedVASurface()
{
    VAStatus res = vaDestroySurfaces(va_device_->display(), const_cast<VASurfaceID*>(&id_), 1u);
    VA_LOG_ASSERT(res, "vaDestroySurfaces");
    VLOG(1) << "destroyed surface " << id_;
}

void SharedVASurface::FetchData(FetchPolicy fetch_policy, const VAImageFormat& format, VAImage* image, uint8_t** image_data) const
{
    if (fetch_policy == FetchPolicy::kDeriveImage || fetch_policy == FetchPolicy::kAny) {
...
void SharedVASurface::SaveAsPNG(FetchPolicy fetch_policy, const std::string& path) const
{
    VAImage image;
    uint8_t* image_data;
    ...
    if (fourcc == VA_FOURCC_NV12) {
        convert_res = libyuv::NV12ToARGB(image_data + image.offsets[0], base::checked_cast<int>(image.pitches[0]), image_data + image.offsets[1],
            base::checked_cast<int>(image.pitches[1]), argb_data.get(), base::checked_cast<int>(argb_stride), base::strict_cast<int>(image.width),
        ...
        convert_res = libyuv::H010ToARGB(y_plane.data(), base::strict_cast<int>(image.width), u_plane.data(), base::checked_cast<int>((image.width + 1) / 2),
            v_plane.data(), base::checked_cast<int>((image.width + 1) / 2), argb_data.get(), base::checked_cast<int>(argb_stride),
            base::strict_cast<int>(image.width), base::strict_cast<int>(image.height));
    }
    LOG_ASSERT(convert_res == 0) << "Failed to convert to ARGB";

    std::optional<std::vector<uint8_t>> image_buffer = gfx::PNGCodec::Encode(
        argb_data.get(), gfx::PNGCodec::FORMAT_BGRA, size_, argb_stride, true /* discard_transparency */, std::vector<gfx::PNGCodec::Comment>());
...
std::string SharedVASurface::GetMD5Sum(FetchPolicy fetch_policy) const
{
    VAImage image;
    uint8_t* image_data;
    constexpr VAImageFormat kImageFormatI420 { .fourcc = VA_FOURCC_I420, .byte_order = VA_LSB_FIRST, .bits_per_pixel = 12 };
    ...
    uint32_t luma_plane_size = base::checked_cast<uint32_t>(image.height * image.width);
    uint32_t chroma_plane_size = base::checked_cast<uint32_t>(((image.height + 1) / 2) * ((image.width + 1) / 2));
    ...
    if (fourcc == VA_FOURCC_I420) {
        ...
        convert_res = libyuv::I420Copy(image_data + image.offsets[0], base::checked_cast<int>(image.pitches[0]), image_data + image.offsets[1],
            base::checked_cast<int>(image.pitches[1]), image_data + image.offsets[2], base::checked_cast<int>(image.pitches[2]), i420_data.data(),
            base::strict_cast<int>(image.width), i420_data.data() + luma_plane_size, base::checked_cast<int>((image.width + 1) / 2),
            i420_data.data() + luma_plane_size + chroma_plane_size, base::checked_cast<int>((image.width + 1) / 2), base::strict_cast<int>(image.width),
            base::strict_cast<int>(image.height));
    } else if (fourcc == VA_FOURCC_NV12) {
        LOG_ASSERT(image.num_planes == 2u);
        convert_res = libyuv::NV12ToI420(image_data + image.offsets[0], base::checked_cast<int>(image.pitches[0]), image_data + image.offsets[1],
            base::checked_cast<int>(image.pitches[1]), i420_data.data(), base::strict_cast<int>(image.width), i420_data.data() + luma_plane_size,
            base::checked_cast<int>((image.width + 1) / 2), i420_data.data() + luma_plane_size + chroma_plane_size,
            base::checked_cast<int>((image.width + 1) / 2), base::strict_cast<int>(image.width), base::strict_cast<int>(image.height));
    }
    LOG_ASSERT(convert_res == 0) << "Failed to convert " << media::FourccToString(fourcc) << " to packed I420.";
