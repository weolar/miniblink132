...
void H264VaapiWrapper::SubmitFrameMetadata(const H264SPS* sps, const H264PPS* pps, const H264DPB& dpb, const H264Picture::Vector& ref_pic_listp0,
    const H264Picture::Vector& ref_pic_listb0, const H264Picture::Vector& ref_pic_listb1, scoped_refptr<H264Picture> pic)
{
...
#undef FROM_PPS_TO_PP_PF2
    ...
    VAStatus va_res = vaCreateBuffer(va_device_->display(), va_context_->id(), VAPictureParameterBufferType, sizeof(pic_param), 1, &pic_param, &buffer_id);
    VA_LOG_ASSERT(va_res, "vaCreateBuffer");
    buffers_.push_back(buffer_id);
    va_res = vaCreateBuffer(va_device_->display(), va_context_->id(), VAIQMatrixBufferType, sizeof(iq_matrix_buf), 1, &iq_matrix_buf, &buffer_id);
    VA_LOG_ASSERT(va_res, "vaCreateBuffer");
    buffers_.push_back(buffer_id);
}

void H264VaapiWrapper::SubmitSlice(const H264PPS* pps, const H264SliceHeader* slice_hdr, const H264Picture::Vector& ref_pic_list0,
...
#define SHDRToSP(a) slice_param.a = slice_hdr->a
    ...
    VABufferID buffer_id;
    VAStatus va_res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceParameterBufferType, sizeof(slice_param), 1, &slice_param, &buffer_id);
    VA_LOG_ASSERT(va_res, "vaCreateBuffer");
    buffers_.push_back(buffer_id);
    va_res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceDataBufferType, size, 1, pic->slice_data_buffers.back().get(), &buffer_id);
    VA_LOG_ASSERT(va_res, "vaCreateBuffer");
    buffers_.push_back(buffer_id);
}
...
void H264VaapiWrapper::SubmitDecode(scoped_refptr<H264Picture> pic)
{
    ...
    VAStatus va_res = vaBeginPicture(va_device_->display(), va_context_->id(), pic->surface->id());
    VA_LOG_ASSERT(va_res, "vaBeginPicture");

    va_res = vaRenderPicture(va_device_->display(), va_context_->id(), buffers_.data(), buffers_.size());
