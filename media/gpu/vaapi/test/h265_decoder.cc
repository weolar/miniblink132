...
struct POCAscCompare {
    bool operator()(const scoped_refptr<media::vaapi_test::H265Picture>& a, const scoped_refptr<media::vaapi_test::H265Picture>& b) const
    {
...
H265Decoder::H265Decoder(const uint8_t* stream_data, size_t stream_len, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy)
...
H265Decoder::DecodeResult H265Decoder::DecodeNALUs()
{
    ...
    while (output_queue.empty()) {
        ...
        case H265NALU::CRA_NUT:
            if (!curr_slice_hdr_) {
                ...
                if (curr_slice_hdr_->irap_pic) {
                    ...
                    CHECK(ProcessPPS(curr_slice_hdr_->slice_pic_parameter_set_id, &need_new_buffers)) << "Processing PPS Unit Failed.";
                    if (need_new_buffers) {
                        curr_pic_ = nullptr;
        ...
        DVLOG(4) << "Finished with current NALU: " << static_cast<int>(curr_nalu_->nal_unit_type);
        curr_nalu_.reset();
    }
...
bool H265Decoder::ProcessCurrentSlice()
{
    ...
    DCHECK(pps);
    return wrapper_.SubmitSlice(sps, pps, slice_hdr, ref_pic_list0_, ref_pic_list1_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_,
...
bool H265Decoder::StartNewFrame(const H265SliceHeader* slice_hdr)
{
    ...
    return wrapper_.SubmitFrameMetadata(sps, pps, slice_hdr, ref_pic_list_, curr_pic_);
}
...
VideoDecoder::Result H265Decoder::DecodeNextFrame()
{
    ...
    DVLOG(3) << "Outputting frame poc: " << output_queue.front()->pic_order_cnt_val_;
