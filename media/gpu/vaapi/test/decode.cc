...
constexpr char kUsageMsg[] = "usage: decode_test\n"
                             "           --video=<video path>\n"
                             "           --codec=<codec name>\n"
...
constexpr char kHelpMsg[] = "This binary decodes the IVF video in <video> path with specified video\n"
...
std::unique_ptr<VideoDecoder> CreateDecoder(
    const std::string& codec, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy, const uint8_t* stream_data, size_t stream_len)
{
    if (codec == "H264")
        return std::make_unique<H264Decoder>(stream_data, stream_len, va_device, fetch_policy);
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    if (codec == "H265")
        return std::make_unique<H265Decoder>(stream_data, stream_len, va_device, fetch_policy);
#endif
    ...
    if (codec == "AV1")
        return std::make_unique<Av1Decoder>(std::move(ivf_parser), va_device, fetch_policy);
    if (codec == "VP8")
        return std::make_unique<Vp8Decoder>(std::move(ivf_parser), va_device, fetch_policy);
    if (codec == "VP9")
        return std::make_unique<Vp9Decoder>(std::move(ivf_parser), va_device, fetch_policy);

    LOG(ERROR) << "Invalid codec requested: " << codec;
    return nullptr;
}

std::optional<SharedVASurface::FetchPolicy> GetFetchPolicy(const VaapiDevice& va_device, const std::string& fetch_policy)
{
...
int main(int argc, char** argv)
{
    ...
    const std::string codec = base::ToUpperASCII(cmd->GetSwitchValueASCII("codec"));
    if (codec.empty()) {
    ...
    if (frames.empty()) {
        ...
        LOG(ERROR) << "Number of frames to decode must be positive integer, got " << frames;
        return EXIT_FAILURE;
    }
    ...
    const auto fetch_policy = GetFetchPolicy(va_device, cmd->GetSwitchValueASCII("fetch"));
    if (!fetch_policy) {
        return EXIT_FAILURE;
    }
    ...
    do {
        const std::unique_ptr<VideoDecoder> dec = CreateDecoder(codec, va_device, *fetch_policy, stream.data(), stream.length());
        if (!dec) {
        ...
        for (int i = 0; i < n_frames || n_frames == 0; i++) {
            ...
            if (!output_prefix.empty() && first_loop) {
                dec->LastDecodedFrameToPNG(base::StringPrintf("%s_%d.png", output_prefix.c_str(), i));
            }
