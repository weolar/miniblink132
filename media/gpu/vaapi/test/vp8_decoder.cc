...
Vp8Decoder::Vp8Decoder(std::unique_ptr<IvfParser> ivf_parser, const VaapiDevice& va_device, SharedVASurface::FetchPolicy fetch_policy)
    : VideoDecoder(va_device, fetch_policy)
    , va_config_(std::make_unique<ScopedVAConfig>(*va_device_, VAProfile::VAProfileVP8Version0_3, VA_RT_FORMAT_YUV420))
...
void Vp8Decoder::FillVp8DataStructures(const Vp8FrameHeader& frame_hdr, VAIQMatrixBufferVP8& iq_matrix_buf, VAProbabilityDataBufferVP8& prob_buf,
    VAPictureParameterBufferVP8& pic_param, VASliceParameterBufferVP8& slice_param)
{
...
#undef CLAMP_Q
    ...
    pic_param.last_ref_frame = ref_frames_[VP8_FRAME_LAST] ? ref_frames_[VP8_FRAME_LAST]->id() : VA_INVALID_SURFACE;
    pic_param.golden_ref_frame = ref_frames_[VP8_FRAME_GOLDEN] ? ref_frames_[VP8_FRAME_GOLDEN]->id() : VA_INVALID_SURFACE;
    pic_param.alt_ref_frame = ref_frames_[VP8_FRAME_ALTREF] ? ref_frames_[VP8_FRAME_ALTREF]->id() : VA_INVALID_SURFACE;
...
#undef FHDR_TO_PP_PF
    ...
    static_assert(
        std::extent<decltype(lf_hdr.mb_mode_delta)>() == std::extent<decltype(pic_param.loop_filter_deltas_mode)>(), "loop filter deltas arrays size mismatch");
...
void Vp8Decoder::RefreshReferenceSlots(Vp8FrameHeader& frame_hdr, scoped_refptr<SharedVASurface> surface)
{
...
VideoDecoder::Result Vp8Decoder::DecodeNextFrame()
{
    ...
    if (frame_hdr.IsKeyframe()) {
        ...
        if (!va_context_ || new_size != va_context_->size()) {
            va_context_ = std::make_unique<ScopedVAContext>(*va_device_, *va_config_, new_size);
        }
    } else {
        frame_hdr.height = va_context_->size().height();
        frame_hdr.width = va_context_->size().width();
    }
    LOG_ASSERT(va_context_ != nullptr) << "VA Context not set. First frame was not a key frame.";
    ...
    scoped_refptr<SharedVASurface> surface = SharedVASurface::Create(*va_device_, va_config_->va_rt_format(), va_context_->size(), attribute);
    ...
    FillVp8DataStructures(frame_hdr, iq_matrix_buf, prob_buf, pic_param, slice_param);

    // Populate the VA API buffers.
    std::vector<VABufferID> buffers;

    VABufferID iq_matrix_id;
    VAStatus res = vaCreateBuffer(va_device_->display(), va_context_->id(), VAIQMatrixBufferType, sizeof(iq_matrix_buf), 1u, nullptr, &iq_matrix_id);
    ...
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VAProbabilityBufferType, sizeof(prob_buf), 1u, nullptr, &prob_buffer_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    ...
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VAPictureParameterBufferType, sizeof(pic_param), 1u, nullptr, &picture_params_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    void* picture_params_data;
    res = vaMapBuffer(va_device_->display(), picture_params_id, &picture_params_data);
    VA_LOG_ASSERT(res, "vaMapBuffer");
    memcpy(picture_params_data, &pic_param, sizeof(pic_param));
    buffers.push_back(picture_params_id);

    VABufferID slice_params_id;
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceParameterBufferType, sizeof(slice_param), 1u, nullptr, &slice_params_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    ...
    res = vaCreateBuffer(va_device_->display(), va_context_->id(), VASliceDataBufferType, frame_hdr.frame_size, 1u, nullptr, &encoded_data_id);
    VA_LOG_ASSERT(res, "vaCreateBuffer");
    void* encoded_data;
    ...
    res = vaRenderPicture(va_device_->display(), va_context_->id(), buffers.data(), base::checked_cast<int>(buffers.size()));
