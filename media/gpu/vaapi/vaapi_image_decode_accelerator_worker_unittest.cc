...
constexpr base::span<const uint8_t, kWebPFileAndVp8ChunkHeaderSizeInBytes> kLossyWebPEncodedData = kLossyWebPFileHeader;

class MockNativePixmapDmaBuf : public gfx::NativePixmapDmaBuf {
public:
    MockNativePixmapDmaBuf(const gfx::Size& size)
        : gfx::NativePixmapDmaBuf(size, kFormatForDecodes, gfx::NativePixmapHandle())
    {
    }
...
public:
    MockVaapiImageDecoder(gpu::ImageDecodeAcceleratorType type)
        : VaapiImageDecoder(VAProfileNone)
        , type_(type)
    {
    }
    ...
    MOCK_METHOD1(ExportAsNativePixmapDmaBuf, std::unique_ptr<NativePixmapAndSizeInfo>(VaapiImageDecodeStatus*));
    MOCK_METHOD1(AllocateVASurfaceAndSubmitVABuffers, VaapiImageDecodeStatus(base::span<const uint8_t>));
...
public:
    VaapiImageDecodeAcceleratorWorkerTest()
    {
        feature_list_.InitWithFeatures(
            { features::kVaapiJpegImageDecodeAcceleration, features::kVaapiWebPImageDecodeAcceleration } /* enabled_features */, {} /* disabled_features */);
        VaapiImageDecoderVector decoders;
        gpu::ImageDecodeAcceleratorSupportedProfiles supported_profiles;

        auto fake_jpeg_profile = GetFakeSupportedProfile(gpu::ImageDecodeAcceleratorType::kJpeg);
        supported_profiles.push_back(fake_jpeg_profile);
        auto fake_webp_profile = GetFakeSupportedProfile(gpu::ImageDecodeAcceleratorType::kWebP);
        supported_profiles.push_back(fake_webp_profile);

        auto vaapi_jpeg_decoder = std::make_unique<StrictMock<MockVaapiImageDecoder>>(gpu::ImageDecodeAcceleratorType::kJpeg);
        vaapi_jpeg_decoder_ = vaapi_jpeg_decoder.get();
        decoders.push_back(std::move(vaapi_jpeg_decoder));

        auto vaapi_webp_decoder = std::make_unique<StrictMock<MockVaapiImageDecoder>>(gpu::ImageDecodeAcceleratorType::kWebP);
        vaapi_webp_decoder_ = vaapi_webp_decoder.get();
        decoders.push_back(std::move(vaapi_webp_decoder));

        worker_ = base::WrapUnique(new VaapiImageDecodeAcceleratorWorker(std::move(decoders), std::move(supported_profiles)));
    }

    VaapiImageDecodeAcceleratorWorkerTest(const VaapiImageDecodeAcceleratorWorkerTest&) = delete;
    VaapiImageDecodeAcceleratorWorkerTest& operator=(const VaapiImageDecodeAcceleratorWorkerTest&) = delete;

    gpu::ImageDecodeAcceleratorSupportedProfile GetFakeSupportedProfile(gpu::ImageDecodeAcceleratorType type)
    {
    ...
    MOCK_METHOD1(OnDecodeCompleted, void(std::unique_ptr<gpu::ImageDecodeAcceleratorWorker::DecodeResult>));
...
ACTION_P2(ExportAsNativePixmapDmaBufSuccessfully, va_surface_resolution, visible_size)
{
    *arg0 = VaapiImageDecodeStatus::kSuccess;
    ...
    exported_pixmap->pixmap = base::MakeRefCounted<MockNativePixmapDmaBuf>(visible_size);
    return exported_pixmap;
}

TEST_F(VaapiImageDecodeAcceleratorWorkerTest, ImageDecodeSucceeds)
{
    std::vector<uint8_t> jpeg_encoded_data(kJpegEncodedData.begin(), kJpegEncodedData.end());
    std::vector<uint8_t> webp_encoded_data(kLossyWebPEncodedData.begin(), kLossyWebPEncodedData.end());
    {
        InSequence sequence;
        ...
        EXPECT_CALL(*jpeg_decoder,
            Decode(AllOf(Property(&base::span<const uint8_t>::data, jpeg_encoded_data.data()),
                Property(&base::span<const uint8_t>::size, jpeg_encoded_data.size())) /* encoded_data */))
            .WillOnce(Return(VaapiImageDecodeStatus::kSuccess));
        EXPECT_CALL(*jpeg_decoder, ExportAsNativePixmapDmaBuf(NotNull() /* status */))
            .WillOnce(ExportAsNativePixmapDmaBufSuccessfully(kVaSurfaceResolution, kVisibleSize));
        ...
        EXPECT_CALL(*webp_decoder,
            Decode(AllOf(Property(&base::span<const uint8_t>::data, webp_encoded_data.data()),
                Property(&base::span<const uint8_t>::size, webp_encoded_data.size())) /* encoded_data */))
            .WillOnce(Return(VaapiImageDecodeStatus::kSuccess));
        EXPECT_CALL(*webp_decoder, ExportAsNativePixmapDmaBuf(NotNull() /* status */))
            .WillOnce(ExportAsNativePixmapDmaBufSuccessfully(kVaSurfaceResolution, kVisibleSize));
        EXPECT_CALL(*this, OnDecodeCompleted(NotNull()));
    }

    worker_->Decode(
        std::move(jpeg_encoded_data), kVisibleSize, base::BindOnce(&VaapiImageDecodeAcceleratorWorkerTest::OnDecodeCompleted, base::Unretained(this)));

    worker_->Decode(
        std::move(webp_encoded_data), kVisibleSize, base::BindOnce(&VaapiImageDecodeAcceleratorWorkerTest::OnDecodeCompleted, base::Unretained(this)));
...
TEST_F(VaapiImageDecodeAcceleratorWorkerTest, UnknownImageDecodeFails)
{
    ...
    EXPECT_CALL(*this, OnDecodeCompleted(IsNull()));
    worker_->Decode(std::move(encoded_data), kVisibleSize, base::BindOnce(&VaapiImageDecodeAcceleratorWorkerTest::OnDecodeCompleted, base::Unretained(this)));
    task_environment_.RunUntilIdle();
}
