...
VaapiPictureNativePixmapEgl::VaapiPictureNativePixmapEgl(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb,
    const BindGLImageCallback& bind_image_cb, int32_t picture_buffer_id, const gfx::Size& visible_size, const gfx::Size& size, uint32_t texture_id,
...
VaapiStatus VaapiPictureNativePixmapEgl::Initialize(scoped_refptr<gfx::NativePixmap> pixmap)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(pixmap);
...
VaapiStatus VaapiPictureNativePixmapEgl::Allocate(gfx::BufferFormat format)
{
    ...
    if (size_.width() > static_cast<int>(native_pixmap_handle.planes[0].stride) || size_.GetArea() > static_cast<int>(native_pixmap_handle.planes[0].size)) {
        DLOG(ERROR) << "EGLImage (stride=" << native_pixmap_handle.planes[0].stride << ", size=" << native_pixmap_handle.planes[0].size
    ...
    scoped_refptr<gfx::NativePixmap> native_pixmap_dmabuf(new gfx::NativePixmapDmaBuf(size_, format, std::move(native_pixmap_handle)));
...
bool VaapiPictureNativePixmapEgl::ImportGpuMemoryBufferHandle(gfx::BufferFormat format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    NOTIMPLEMENTED();
    return false;
