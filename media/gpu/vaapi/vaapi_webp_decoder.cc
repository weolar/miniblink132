...
{
    if (!VaapiWrapper::IsDecodingSupportedForInternalFormat(kWebPVAProfile, kWebPVARtFormat)) {
    ...
    const gfx::Size webp_size(base::strict_cast<int>(webp_header.width), base::strict_cast<int>(webp_header.height));
    ...
    if (!VaapiWrapper::GetSupportedResolutions(kWebPVAProfile, VaapiWrapper::CodecMode::kDecode, min_webp_resolution, max_webp_resolution)) {
        DLOG(ERROR) << "Could not get the minimum and maximum resolutions";
        return false;
    }
    if (webp_size.width() < min_webp_resolution.width() || webp_size.height() < min_webp_resolution.height()) {
        DLOG(ERROR) << "VAAPI doesn't support size " << webp_size.ToString() << ": under minimum resolution " << min_webp_resolution.ToString();
        return false;
    }
    if (webp_size.width() > max_webp_resolution.width() || webp_size.height() > max_webp_resolution.height()) {
        DLOG(ERROR) << "VAAPI doesn't support size " << webp_size.ToString() << ": above maximum resolution " << max_webp_resolution.ToString();
        return false;
    }
    return true;
}
...
VaapiWebPDecoder::VaapiWebPDecoder()
    : VaapiImageDecoder(kWebPVAProfile)
{
}
...
// static
std::optional<gpu::ImageDecodeAcceleratorSupportedProfile> VaapiWebPDecoder::GetSupportedProfile()
{
    if (!base::FeatureList::IsEnabled(features::kVaapiWebPImageDecodeAcceleration)) {
        return std::nullopt;
    }
    ...
    const bool got_supported_resolutions = VaapiWrapper::GetSupportedResolutions(
        kWebPVAProfile, VaapiWrapper::CodecMode::kDecode, profile.min_encoded_dimensions, profile.max_encoded_dimensions);
...
VaapiImageDecodeStatus VaapiWebPDecoder::AllocateVASurfaceAndSubmitVABuffers(base::span<const uint8_t> encoded_image)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    ...
    const gfx::Size new_visible_size(base::strict_cast<int>(parse_result->width), base::strict_cast<int>(parse_result->height));
    DCHECK(!scoped_va_context_and_surface_ || scoped_va_context_and_surface_->IsValid());
    DCHECK(!scoped_va_context_and_surface_ || (scoped_va_context_and_surface_->format() == kWebPVARtFormat));
    if (!scoped_va_context_and_surface_ || new_visible_size != scoped_va_context_and_surface_->size()) {
        ...
        auto scoped_va_surfaces
            = vaapi_wrapper_->CreateContextAndScopedVASurfaces(kWebPVARtFormat, new_visible_size, { VaapiWrapper::SurfaceUsageHint::kGeneric }, 1u,
        ...
        scoped_va_context_and_surface_ = ScopedVAContextAndSurface(scoped_va_surfaces[0].release());
    ...
    FillVP8DataStructures(*parse_result, Vp8ReferenceFrameVector(), &iq_matrix_buf, &prob_buf, &pic_param, &slice_param);

    const bool success = vaapi_wrapper_->SubmitBuffers({ { VAIQMatrixBufferType, sizeof(iq_matrix_buf), &iq_matrix_buf },
        { VAProbabilityBufferType, sizeof(prob_buf), &prob_buf }, { VAPictureParameterBufferType, sizeof(pic_param), &pic_param },
        { VASliceParameterBufferType, sizeof(slice_param), &slice_param }, { VASliceDataBufferType, parse_result->frame_size, parse_result->data.get() } });

    return success ? VaapiImageDecodeStatus::kSuccess : VaapiImageDecodeStatus::kSubmitVABuffersFailed;
}
