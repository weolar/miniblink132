...
VaapiWrapper::VABufferDescriptor CreateVABufferDescriptor()
{
    ...
    return VaapiWrapper::VABufferDescriptor { VAProcPipelineParameterBufferType, sizeof(kData), kData };
}

class MockVaapiWrapper : public VaapiWrapper {
public:
    MockVaapiWrapper()
        : VaapiWrapper(VADisplayStateHandle(), kVideoProcess)
    {
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        ON_CALL(*mock_vaapi_wrapper_, SubmitBuffer_Locked).WillByDefault(Invoke(this, &VaapiWrapperTest::DefaultSubmitBuffer_Locked));
        ON_CALL(*mock_vaapi_wrapper_, DestroyPendingBuffers_Locked).WillByDefault(Invoke(this, &VaapiWrapperTest::DefaultDestroyPendingBuffers_Locked));
    }
    void TearDown() override
    {
        VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(mock_vaapi_wrapper_->sequence_checker_);
    ...
    bool DefaultSubmitBuffer_Locked(const VaapiWrapper::VABufferDescriptor& va_buffer) EXCLUSIVE_LOCKS_REQUIRED(mock_vaapi_wrapper_->va_lock_)
    {
        VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(mock_vaapi_wrapper_->sequence_checker_);
        if (va_buffer.data) {
        ...
        const bool submit_buffer_res = (*mock_vaapi_wrapper_).VaapiWrapper::SubmitBuffer_Locked(va_buffer);
        if (submit_buffer_res)
            ADD_FAILURE();
        return false;
    }

    void DefaultDestroyPendingBuffers_Locked() EXCLUSIVE_LOCKS_REQUIRED(mock_vaapi_wrapper_->va_lock_)
    {
        VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(mock_vaapi_wrapper_->sequence_checker_);
        mock_vaapi_wrapper_->pending_va_buffers_.clear();
    }
...
TEST_F(VaapiWrapperTest, SubmitBuffer)
{
    ...
    for (size_t i = 0; i < kNumBuffers; ++i) {
        EXPECT_TRUE(mock_vaapi_wrapper_->SubmitBuffer(va_buffer.type, va_buffer.size, va_buffer.data));
    }
...
TEST_F(VaapiWrapperTest, FailOnSubmitBuffers)
{
    ...
    ::testing::InSequence s;
    EXPECT_CALL(*mock_vaapi_wrapper_, SubmitBuffer_Locked(_)).Times(kNumBuffers - 1);
    EXPECT_CALL(*mock_vaapi_wrapper_, DestroyPendingBuffers_Locked);
