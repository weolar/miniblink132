...
public:
    MockVaapiWrapper(CodecMode mode)
        : VaapiWrapper(mode)
    {
    }
    MOCK_METHOD5(CreateContextAndSurfaces, bool(unsigned int, const gfx::Size&, const std::vector<SurfaceUsageHint>&, size_t, std::vector<VASurfaceID>*));
...
public:
    MockVaapiPicture(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb,
        const BindGLImageCallback& bind_image_cb, int32_t picture_buffer_id, const gfx::Size& size, const gfx::Size& visible_size, uint32_t texture_id,
        uint32_t client_texture_id, uint32_t texture_target)
        : VaapiPicture(std::move(vaapi_wrapper), make_context_current_cb, bind_image_cb, picture_buffer_id, size, visible_size, texture_id, client_texture_id,
            texture_target)
    {
    }
    ...
    bool ImportGpuMemoryBufferHandle(gfx::BufferFormat format, gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle) override
    {
        return true;
    }
...
public:
    ...
    MOCK_METHOD3(MockCreateVaapiPicture, void(VaapiWrapper*, const gfx::Size&, const gfx::Size&));
    std::unique_ptr<VaapiPicture> Create(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb,
        const BindGLImageCallback& bind_image_cb, const PictureBuffer& picture_buffer, const gfx::Size& visible_size) override
    {
        ...
        MockCreateVaapiPicture(vaapi_wrapper.get(), picture_buffer.size(), visible_size);
        return std::make_unique<MockVaapiPicture>(std::move(vaapi_wrapper), make_context_current_cb, bind_image_cb, picture_buffer.id(), picture_buffer.size(),
            visible_size, service_texture_id, client_texture_id, picture_buffer.texture_target());
    }
};

class VaapiVideoDecodeAcceleratorTest : public TestWithParam<TestParams>, public VideoDecodeAccelerator::Client {
public:
    VaapiVideoDecodeAcceleratorTest()
        : vda_(base::BindRepeating([] { return true; }),
            base::BindRepeating(
                [](uint32_t client_texture_id, uint32_t texture_target, const scoped_refptr<gl::GLImage>& image, bool can_bind_to_sampler) { return true; }))
        ...
        , weak_ptr_factory_(this)
    {
        ...
        decoder_delegate_ = std::make_unique<VaapiVideoDecoderDelegate>(&vda_, mock_vaapi_wrapper_, base::DoNothing(), nullptr);
        ...
        vda_.buffer_allocation_mode_ = GetParam().decode_using_client_picture_buffers ? VaapiVideoDecodeAccelerator::BufferAllocationMode::kNone
                                                                                      : VaapiVideoDecodeAccelerator::BufferAllocationMode::kSuperReduced;

        vda_.state_ = VaapiVideoDecodeAccelerator::kIdle;
    }

    VaapiVideoDecodeAcceleratorTest(const VaapiVideoDecodeAcceleratorTest&) = delete;
    VaapiVideoDecodeAcceleratorTest& operator=(const VaapiVideoDecodeAcceleratorTest&) = delete;
    ...
    void ResetSequence()
    {
        ...
        EXPECT_CALL(*this, NotifyResetDone()).WillOnce(RunClosure(run_loop.QuitClosure()));
        vda_.Reset();
        run_loop.Run();
    }
    ...
    // calls.
    void QueueInputBufferSequence(size_t num_pictures, const gfx::Size& picture_size, int32_t bitstream_id, bool expect_dismiss_picture_buffers = false,
        size_t num_picture_buffers_to_dismiss = 0)
    {
        ::testing::InSequence s;
        EXPECT_CALL(*mock_decoder_, SetStream(_, IsExpectedDecoderBuffer(kInputSize, nullptr))).WillOnce(Return());
        EXPECT_CALL(*mock_decoder_, Decode()).WillOnce(Return(AcceleratedVideoDecoder::kConfigChange));
        ...
        EXPECT_CALL(*mock_decoder_, GetVisibleRect()).WillOnce(Return(gfx::Rect(picture_size)));
        EXPECT_CALL(*mock_decoder_, GetRequiredNumOfPictures()).WillOnce(Return(num_pictures));
        const size_t kNumReferenceFrames = num_pictures / 2;
        EXPECT_CALL(*mock_decoder_, GetNumReferenceFrames()).WillOnce(Return(kNumReferenceFrames));
        EXPECT_CALL(*mock_decoder_, GetProfile()).WillOnce(Return(GetParam().video_codec));
        EXPECT_CALL(*mock_vaapi_wrapper_, DestroyContext());

        if (expect_dismiss_picture_buffers) {
            EXPECT_CALL(*this, DismissPictureBuffer(_)).Times(num_picture_buffers_to_dismiss);
        }

        const size_t expected_num_picture_buffers_requested = vda_.buffer_allocation_mode_ == VaapiVideoDecodeAccelerator::BufferAllocationMode::kSuperReduced
            ? num_pictures - kNumReferenceFrames
            : num_pictures;

        base::RunLoop run_loop;

        EXPECT_CALL(*this, ProvidePictureBuffers(expected_num_picture_buffers_requested, _, 1, picture_size, _)).WillOnce(RunClosure(run_loop.QuitClosure()));

        BitstreamBuffer bitstream_buffer(bitstream_id, in_shm_.Duplicate(), kInputSize);
    ...
    void AssignPictureBuffersSequence(size_t num_pictures, const gfx::Size& picture_size, int32_t bitstream_id)
    {
        ASSERT_TRUE(vda_.curr_input_buffer_) << "QueueInputBuffer() should have been called";
        ...
        if (GetParam().decode_using_client_picture_buffers) {
            EXPECT_CALL(*mock_vaapi_wrapper_, CreateContext(picture_size)).WillOnce(Return(true));
            EXPECT_CALL(*mock_decoder_, GetVisibleRect()).WillRepeatedly(Return(gfx::Rect(picture_size)));
            EXPECT_CALL(*mock_vaapi_picture_factory_, MockCreateVaapiPicture(mock_vaapi_wrapper_.get(), picture_size, picture_size)).Times(num_pictures);
        } else {
            EXPECT_EQ(vda_.buffer_allocation_mode_, VaapiVideoDecodeAccelerator::BufferAllocationMode::kSuperReduced);
            const size_t kNumReferenceFrames = 1 + num_pictures / 2;
            EXPECT_CALL(*mock_vaapi_wrapper_,
                CreateContextAndSurfaces(
                    _, picture_size, std::vector<VaapiWrapper::SurfaceUsageHint> { VaapiWrapper::SurfaceUsageHint::kVideoDecoder }, kNumReferenceFrames, _))
                .WillOnce(
                    DoAll(WithArg<4>(Invoke([kNumReferenceFrames](std::vector<VASurfaceID>* va_surface_ids) { va_surface_ids->resize(kNumReferenceFrames); })),
                        Return(true)));
            EXPECT_CALL(*mock_vaapi_wrapper_, DestroySurface(_)).Times(kNumReferenceFrames);
            EXPECT_CALL(*mock_decoder_, GetVisibleRect()).WillRepeatedly(Return(gfx::Rect(picture_size)));
            EXPECT_CALL(*mock_vaapi_picture_factory_, MockCreateVaapiPicture(_, picture_size, picture_size)).Times(num_pictures);
        }

        ::testing::InSequence s;
        base::RunLoop run_loop;

        EXPECT_CALL(*mock_decoder_, Decode()).WillOnce(Return(AcceleratedVideoDecoder::kRanOutOfStreamData));
        EXPECT_CALL(*this, NotifyEndOfBitstreamBuffer(bitstream_id)).WillOnce(RunClosure(run_loop.QuitClosure()));
        ...
        for (size_t picture = 0; picture < num_pictures; ++picture) {
            ...
            // arbitrarily chosen.
            picture_buffers.push_back({ irrelevant_id++, picture_size, PictureBuffer::TextureIds { static_cast<uint32_t>(irrelevant_id++) },
                PictureBuffer::TextureIds { static_cast<uint32_t>(irrelevant_id++) }, tex_target, PIXEL_FORMAT_XRGB });
        }
    ...
    void DecodeOneFrameFast(int32_t bitstream_id)
    {
        ...
        QueueInputBuffer(BitstreamBuffer(bitstream_id, in_shm_.Duplicate(), kInputSize));
    ...
    MOCK_METHOD5(ProvidePictureBuffers, void(uint32_t, VideoPixelFormat, uint32_t, const gfx::Size&, uint32_t));
...
{
    EXPECT_EQ(VaapiPictureFactory::kVaapiImplementationNone, mock_vaapi_picture_factory_->GetVaapiImplementation(gl::kGLImplementationNone));
    EXPECT_EQ(VaapiPictureFactory::kVaapiImplementationDrm, mock_vaapi_picture_factory_->GetVaapiImplementation(gl::kGLImplementationEGLGLES2));

#if BUILDFLAG(USE_VAAPI_X11)
    EXPECT_EQ(VaapiPictureFactory::kVaapiImplementationAngle, mock_vaapi_picture_factory_->GetVaapiImplementation(gl::kGLImplementationEGLANGLE));
    EXPECT_EQ(VaapiPictureFactory::kVaapiImplementationX11, mock_vaapi_picture_factory_->GetVaapiImplementation(gl::kGLImplementationDesktopGL));
#elif defined(USE_OZONE)
    EXPECT_EQ(VaapiPictureFactory::kVaapiImplementationDrm, mock_vaapi_picture_factory_->GetVaapiImplementation(gl::kGLImplementationEGLANGLE));
#endif
}

// This test checks that QueueInputBuffer() fails when state is kUnitialized.
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBufferAndErrorWhenVDAUninitialized)
{
    SetVdaStateToUnitialized();

    BitstreamBuffer bitstream_buffer(kBitstreamId, in_shm_.Duplicate(), kInputSize);
...
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBufferAndDecodeError)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*mock_decoder_, SetStream(_, IsExpectedDecoderBuffer(kInputSize, nullptr))).WillOnce(Return());
    EXPECT_CALL(*mock_decoder_, Decode()).WillOnce(Return(AcceleratedVideoDecoder::kDecodeError));
    EXPECT_CALL(*this, NotifyError(VaapiVideoDecodeAccelerator::PLATFORM_FAILURE)).WillOnce(RunClosure(run_loop.QuitClosure()));
...
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBuffersAndAssignPictureBuffers)
{
    if (GetParam().video_codec == VP9PROFILE_PROFILE2)
        GTEST_SKIP() << "Decoding profile 2 is not supported";
...
TEST_P(VaapiVideoDecodeAcceleratorTest, QueueInputBuffersAndAssignPictureBuffersAndReallocate)
{
    if (GetParam().video_codec == VP9PROFILE_PROFILE2)
        GTEST_SKIP() << "Decoding profile 2 is not supported";
    QueueInputBufferSequence(kNumPictures, kPictureSize, kBitstreamId);
    ...
    QueueInputBufferSequence(
        kNewNumPictures, kNewPictureSize, kBitstreamId, true /* expect_dismiss_picture_buffers */, kNumPictures /* num_picture_buffers_to_dismiss */);
...
INSTANTIATE_TEST_SUITE_P(All, VaapiVideoDecodeAcceleratorTest, ValuesIn(kTestCases));
