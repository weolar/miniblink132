...
class VaapiVideoDecoder : public VideoDecoderMixin, public VaapiDecodeSurfaceHandler {
public:
    ...
    void SurfaceReady(VASurfaceID va_surface_id, int32_t buffer_id, const gfx::Rect& visible_rect, const VideoColorSpace& color_space) override;
...
private:
    ...
    struct DecodeTask {
        DecodeTask(scoped_refptr<DecoderBuffer> buffer, int32_t buffer_id, DecodeCB decode_done_cb);
    ...
    void ApplyResolutionChangeWithScreenSizes(const std::vector<gfx::Size>& screen_resolution);

    // Private static helper to allow using weak ptr instead of an unretained ptr.
    static CroStatus::Or<scoped_refptr<FrameResource>> AllocateCustomFrameProxy(base::WeakPtr<VaapiVideoDecoder> decoder, VideoPixelFormat format,
        const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, bool use_protected, bool use_linear_buffers,
        bool needs_detiling, base::TimeDelta timestamp);
    ...
    // frame it creates.
    CroStatus::Or<scoped_refptr<FrameResource>> AllocateCustomFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    ...
    VideoCodecProfile profile_ GUARDED_BY_CONTEXT(sequence_checker_) = VIDEO_CODEC_PROFILE_UNKNOWN;
    VideoColorSpace color_space_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::optional<gfx::HDRMetadata> hdr_metadata_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    base::LRUCache<int32_t, base::TimeDelta> buffer_id_to_timestamp_ GUARDED_BY_CONTEXT(sequence_checker_);

    // Queue containing all requested decode tasks.
    base::queue<DecodeTask> decode_task_queue_ GUARDED_BY_CONTEXT(sequence_checker_);
    // The decode task we're currently trying to execute.
    std::optional<DecodeTask> current_decode_task_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    std::map<VASurfaceID, scoped_refptr<FrameResource>> output_frames_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    base::IDMap<std::unique_ptr<ScopedVASurface>, decltype(gfx::GpuMemoryBufferId::id)> allocated_va_surfaces_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    std::unique_ptr<CdmContextRef> cdm_context_ref_ GUARDED_BY_CONTEXT(sequence_checker_);

    EncryptionScheme encryption_scheme_ GUARDED_BY_CONTEXT(sequence_checker_);

#if BUILDFLAG(IS_CHROMEOS_ASH)
    // To keep the CdmContext event callback registered.
    std::unique_ptr<CallbackRegistration> cdm_event_cb_registration_ GUARDED_BY_CONTEXT(sequence_checker_);
#endif

    // Platform and codec specific video decoder.
    std::unique_ptr<AcceleratedVideoDecoder> decoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    raw_ptr<VaapiVideoDecoderDelegate, DanglingUntriaged> decoder_delegate_ GUARDED_BY_CONTEXT(sequence_checker_) = nullptr;
    ...
    bool ignore_resolution_changes_to_smaller_for_testing_ GUARDED_BY_CONTEXT(sequence_checker_) = false;
