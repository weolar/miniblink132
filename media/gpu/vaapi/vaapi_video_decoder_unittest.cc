...
{
    const VideoDecoderConfig config(media::VideoCodec::kVP8, VP8PROFILE_ANY, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
        kCodedSize, gfx::Rect(kCodedSize), kCodedSize, EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
scoped_refptr<DecoderBuffer> CreateDecoderBuffer(const base::span<const uint8_t>& bitstream)
{
...
public:
    ...
    MOCK_METHOD(CroStatus::Or<ImageProcessor::PixelLayoutCandidate>, PickDecoderOutputFormat,
        (const std::vector<ImageProcessor::PixelLayoutCandidate>&, const gfx::Rect&, const gfx::Size&, std::optional<gfx::Size>, size_t, bool, bool,
...
public:
    ...
    MOCK_METHOD(std::optional<gfx::HDRMetadata>, GetHDRMetadata, (), (const, override));
...
public:
    ...
    void SetUp() override
    {
        mock_vaapi_wrapper_ = base::MakeRefCounted<MockVaapiWrapper>(VaapiWrapper::kDecode);
    ...
    void ResetDecoder()
    {
        auto mock_accelerated_video_decoder = std::make_unique<StrictMock<MockAcceleratedVideoDecoder>>();
        mock_accelerated_video_decoder_ = mock_accelerated_video_decoder.get();
        decoder_ = VaapiVideoDecoder::Create(
            std::make_unique<media::NullMediaLog>(), base::SequencedTaskRunner::GetCurrentDefault(), client_.weak_ptr_factory_.GetWeakPtr());
    ...
    void InitializeVaapiVideoDecoder(
        const DecoderStatus::Codes& status_code, const VideoDecoderConfig& config = DefaultVideoDecoderConfig(), CdmContext* cdm_context = nullptr)
    {
        base::RunLoop run_loop;
        EXPECT_CALL(client_, InitCallback(MatchesStatusCode(status_code))).WillOnce(RunClosure(run_loop.QuitClosure()));
        vaapi_decoder()->Initialize(config, /*low_delay=*/false, /*cdm_context=*/cdm_context,
            base::BindOnce(&MockVideoDecoderMixinClient::InitCallback, client_.weak_ptr_factory_.GetWeakPtr()),
    ...
    void Decode(scoped_refptr<DecoderBuffer> buffer, AcceleratedVideoDecoder::DecodeResult mock_decoder_result, DecoderStatus::Codes vaapi_decoder_status)
    {
        ASSERT_TRUE(mock_accelerated_video_decoder_);
        {
            InSequence sequence;
            EXPECT_CALL(*mock_accelerated_video_decoder_, SetStream(_, _));
            EXPECT_CALL(*mock_accelerated_video_decoder_, Decode()).WillOnce(Return(mock_decoder_result));
            EXPECT_CALL(*this, OnDecodeCompleted(MatchesStatusCode(vaapi_decoder_status)));
        }
        vaapi_decoder()->Decode(std::move(buffer), base::BindOnce(&VaapiVideoDecoderTest::OnDecodeCompleted, base::Unretained(this)));
    ...
    raw_ptr<StrictMock<MockAcceleratedVideoDecoder>> mock_accelerated_video_decoder_ = nullptr;
};
...
TEST_F(VaapiVideoDecoderTest, InitializeFailsDueToMissingCdmContextForEncryptedContent)
{
    InitializeVaapiVideoDecoder(DecoderStatus::Codes::kUnsupportedEncryptionMode,
        VideoDecoderConfig(VideoCodec::kVP8, VP8PROFILE_ANY, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, kCodedSize,
            gfx::Rect(kCodedSize), kCodedSize, EmptyExtraData(), EncryptionScheme::kCenc));
}
...
TEST_F(VaapiVideoDecoderTest, InitializeForVP9EncryptedContent)
{
    ...
    InitializeVaapiVideoDecoder(DecoderStatus::Codes::kOk,
        VideoDecoderConfig(VideoCodec::kVP9, VP9PROFILE_PROFILE0, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, kCodedSize,
            gfx::Rect(kCodedSize), kCodedSize, EmptyExtraData(), EncryptionScheme::kCenc),
        &cdm_context_);
    EXPECT_FALSE(vaapi_decoder()->NeedsTranscryption());
    testing::Mock::VerifyAndClearExpectations(&chromeos_cdm_context_);
...
TEST_F(VaapiVideoDecoderTest, DecodeSucceeds)
{
    ...
    Decode(buffer, AcceleratedVideoDecoder::DecodeResult::kRanOutOfStreamData, DecoderStatus::Codes::kOk);
...
TEST_F(VaapiVideoDecoderTest, DecodeFails)
{
    ...
    Decode(buffer, AcceleratedVideoDecoder::DecodeResult::kDecodeError, DecoderStatus::Codes::kFailed);
    testing::Mock::VerifyAndClearExpectations(mock_accelerated_video_decoder_);
}
...
TEST_F(VaapiVideoDecoderTest, ConfigChange)
{
    ...
    Decode(buffer, AcceleratedVideoDecoder::DecodeResult::kConfigChange, DecoderStatus::Codes::kAborted);
    testing::Mock::VerifyAndClearExpectations(mock_accelerated_video_decoder_);
}
...
TEST_F(VaapiVideoDecoderTest, Reset)
{
    ...
    vaapi_decoder()->Reset(base::BindOnce(&VaapiVideoDecoderTest::OnResetDone, base::Unretained(this)));
    task_environment_.RunUntilIdle();
}
