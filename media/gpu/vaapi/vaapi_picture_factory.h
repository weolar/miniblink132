...
using CreatePictureCB = base::RepeatingCallback<std::unique_ptr<VaapiPicture>(
    scoped_refptr<VaapiWrapper>, const MakeGLContextCurrentCallback&, const BindGLImageCallback&, const PictureBuffer&, const gfx::Size&, uint32_t, uint32_t)>;
...
public:
    ...
    // picture_buffer.id().
    virtual std::unique_ptr<VaapiPicture> Create(scoped_refptr<VaapiWrapper> vaapi_wrapper, const MakeGLContextCurrentCallback& make_context_current_cb,
        const BindGLImageCallback& bind_image_cb, const PictureBuffer& picture_buffer, const gfx::Size& visible_size);
    ...
    std::unique_ptr<VaapiPicture> CreateVaapiPictureNative(scoped_refptr<VaapiWrapper> vaapi_wrapper,
    ...
    std::map<gl::GLImplementation, VaapiPictureFactory::VaapiImplementation> vaapi_impl_pairs_;
