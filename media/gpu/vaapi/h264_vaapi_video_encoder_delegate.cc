...
template <typename VAEncMiscParam> VAEncMiscParam& AllocateMiscParameterBuffer(std::vector<uint8_t>& misc_buffer, VAEncMiscParameterType misc_param_type)
{
    constexpr size_t buffer_size = sizeof(VAEncMiscParameterBuffer) + sizeof(VAEncMiscParam);
    misc_buffer.resize(buffer_size);
    auto* va_buffer = reinterpret_cast<VAEncMiscParameterBuffer*>(misc_buffer.data());
    va_buffer->type = misc_param_type;
    return *reinterpret_cast<VAEncMiscParam*>(va_buffer->data);
}

void CreateVAEncRateControlParams(uint32_t bps, uint32_t target_percentage, uint32_t window_size, uint32_t initial_qp, uint32_t min_qp, uint32_t max_qp,
    uint32_t framerate, uint32_t buffer_size, std::vector<uint8_t> misc_buffers[3])
{
    auto& rate_control_param = AllocateMiscParameterBuffer<VAEncMiscParameterRateControl>(misc_buffers[0], VAEncMiscParameterTypeRateControl);
    ...
    auto& framerate_param = AllocateMiscParameterBuffer<VAEncMiscParameterFrameRate>(misc_buffers[1], VAEncMiscParameterTypeFrameRate);
    framerate_param.framerate = framerate;

    auto& hrd_param = AllocateMiscParameterBuffer<VAEncMiscParameterHRD>(misc_buffers[2], VAEncMiscParameterTypeHRD);
...
// referenced by |pic|.
void UpdatePictureForTemporalLayerEncoding(const size_t num_layers, H264Picture& pic, unsigned int& frame_num, std::optional<size_t>& ref_frame_idx,
    const unsigned int num_encoded_frames, const base::circular_deque<scoped_refptr<H264Picture>>& ref_pic_list0)
{
    ...
    std::tie(pic.metadata_for_encoding.emplace(), pic.ref) = kFrameMetadata[num_layers - 2][num_encoded_frames % kTemporalLayerCycle];
...
{
    return base::WrapRefCounted(reinterpret_cast<H264Picture*>(job.picture().get()));
}

std::optional<H264RateControlConfigRTC> CreateRateControlConfig(const gfx::Size encode_size, const H264VaapiVideoEncoderDelegate::EncodeParams& encode_params,
    const VideoBitrateAllocation& bitrate_allocation, const size_t& num_temporal_layers)
{
    ...
    for (size_t tid = 0; tid < num_temporal_layers; ++tid) {
        ...
        if (rc_cfg.content_type == VideoEncodeAccelerator::Config::ContentType::kDisplay) {
        ...
        layer_setting.frame_rate = static_cast<float>(encode_params.framerate / (1u << (num_temporal_layers - tid - 1)));
    }
    return std::make_optional<H264RateControlConfigRTC>(rc_cfg);
}
} // namespace

std::unique_ptr<H264RateControlWrapper> H264RateControlWrapper::Create(const H264RateControlConfigRTC& config)
{
...
H264RateControlWrapper::H264RateControlWrapper(std::unique_ptr<H264RateCtrlRTC> impl)
    : impl_(std::move(impl))
{
}
...
void H264RateControlWrapper::UpdateRateControl(const H264RateControlConfigRTC& config)
{
    DCHECK(impl_);
    impl_->UpdateRateControl(config);
}

H264RateCtrlRTC::FrameDropDecision H264RateControlWrapper::ComputeQP(const H264FrameParamsRTC& frame_params)
{
    DCHECK(impl_);
...
void H264RateControlWrapper::PostEncodeUpdate(uint64_t encoded_frame_size, const H264FrameParamsRTC& frame_params)
{
...
H264VaapiVideoEncoderDelegate::H264VaapiVideoEncoderDelegate(scoped_refptr<VaapiWrapper> vaapi_wrapper, base::RepeatingClosure error_cb)
...
void H264VaapiVideoEncoderDelegate::set_rate_ctrl_for_testing(std::unique_ptr<H264RateControlWrapper> rate_ctrl)
{
    rate_ctrl_ = std::move(rate_ctrl);
}

bool H264VaapiVideoEncoderDelegate::Initialize(const VideoEncodeAccelerator::Config& config, const VaapiVideoEncoderDelegate::Config& ave_config)
{
    ...
    coded_size_ = gfx::Size(base::bits::AlignUpDeprecatedDoNotUse(visible_size_.width(), kH264MacroblockSizeInPixels),
        base::bits::AlignUpDeprecatedDoNotUse(visible_size_.height(), kH264MacroblockSizeInPixels));
    ...
    if (!CheckH264LevelLimits(profile_, level_, config.bitrate.target_bps(), framerate, mb_width_ * mb_height_)) {
        std::optional<uint8_t> valid_level = FindValidH264Level(profile_, config.bitrate.target_bps(), framerate, mb_width_ * mb_height_);
        if (!valid_level) {
            VLOGF(1) << "Could not find a valid h264 level for"
                     << " profile=" << profile_ << " bitrate=" << config.bitrate.target_bps() << " framerate=" << framerate
    ...
    if (config.content_type == VideoEncodeAccelerator::Config::ContentType::kDisplay) {
    ...
    if (config.HasTemporalLayer()) {
        ...
        if (num_temporal_layers_ > kMaxSupportedH264TemporalLayers || num_temporal_layers_ < kMinSupportedH264TemporalLayers) {
            DVLOGF(1) << "Unsupported number of temporal layers: " << base::strict_cast<size_t>(num_temporal_layers_);
            return false;
        }
    }

    curr_params_.max_ref_pic_list0_size
        = num_temporal_layers_ > 1u ? num_temporal_layers_ - 1 : std::min(kMaxRefIdxL0Size, ave_config.max_num_ref_frames & 0xffff);
    ...
    if (!vaapi_wrapper_->GetSupportedPackedHeaders(config.output_profile, submit_packed_sps, submit_packed_pps, submit_packed_slice)) {
    ...
    curr_params_.bitrate_allocation = VideoBitrateAllocation(config.bitrate.mode());
    ...
    if (UseSoftwareRateController(config)) {
        if (!rate_ctrl_) {
            auto rc_config = CreateRateControlConfig(visible_size_, curr_params_, initial_bitrate_allocation, num_temporal_layers_);
            if (!rc_config) {
...
bool H264VaapiVideoEncoderDelegate::UseSoftwareRateController(const VideoEncodeAccelerator::Config& config)
{
    // Software bitrate controller is not supported on AMD backend,
    // crbug.com/365106092.
    const bool is_sw_bitrate_controller_supported = VaapiWrapper::GetImplementationType() != VAImplementation::kMesaGallium;
    ...
    const bool is_constant_bitrate_mode = config.bitrate.mode() == Bitrate::Mode::kConstant;

    return is_sw_bitrate_controller_supported && num_temporal_layers <= kMaxSupportedH264TemporalLayersBySWBRC && is_sw_bitrate_controller_enabled
        && is_constant_bitrate_mode;
}

BitstreamBufferMetadata H264VaapiVideoEncoderDelegate::GetMetadata(const EncodeJob& encode_job, size_t payload_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
VaapiVideoEncoderDelegate::PrepareEncodeJobResult H264VaapiVideoEncoderDelegate::PrepareEncodeJob(EncodeJob& encode_job)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (num_temporal_layers_ > 1u) {
        UpdatePictureForTemporalLayerEncoding(num_temporal_layers_, *pic, frame_num_, ref_frame_index, num_encoded_frames_, ref_pic_list0_);
    } else {
        pic->ref = true;
    ...
    DVLOGF(4) << "Starting a new frame, type: " << pic->type << (encode_job.IsKeyframeRequested() ? " (keyframe)" : "") << " frame_num: " << pic->frame_num
              << " POC: " << pic->pic_order_cnt;

    std::optional<int> qp;
    if (rate_ctrl_) {
        H264FrameParamsRTC frame_params {};
        frame_params.temporal_layer_id = pic->metadata_for_encoding ? base::strict_cast<int>(pic->metadata_for_encoding->temporal_idx) : 0;
        frame_params.keyframe = encode_job.IsKeyframeRequested();
        frame_params.timestamp = encode_job.timestamp();
        if (rate_ctrl_->ComputeQP(frame_params) == H264RateCtrlRTC::FrameDropDecision::kDrop) {
    ...
    if (!SubmitFrameParameters(encode_job, curr_params_, current_sps_, current_pps_, pic, ref_pic_list0_, ref_frame_index, qp)) {
...
bool H264VaapiVideoEncoderDelegate::UpdateRates(const VideoBitrateAllocation& bitrate_allocation, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    if (bitrate_allocation.GetMode() != curr_params_.bitrate_allocation.GetMode()) {
        DVLOGF(1) << "Unexpected bitrate mode, requested rate " << bitrate_allocation.GetSumBitrate().ToString() << ", expected mode to match "
    ...
    if (curr_params_.bitrate_allocation == bitrate_allocation && curr_params_.framerate == framerate) {
        return true;
    }
    VLOGF(2) << "New bitrate allocation: " << bitrate_allocation.ToString() << ", New framerate: " << framerate;
    ...
    if (rate_ctrl_) {
        auto rc_config = CreateRateControlConfig(visible_size_, curr_params_, bitrate_allocation, num_temporal_layers_);
        if (!rc_config) {
            DVLOGF(1) << "Failed creating rate control config";
            return false;
        }
...
void H264VaapiVideoEncoderDelegate::UpdateSPS()
{
    ...
    H264SPS::GetLevelConfigFromProfileLevel(profile_, level_, &current_sps_.level_idc, &current_sps_.constraint_set3_flag);
    ...
    current_sps_.bit_rate_value_minus1[0] = (curr_params_.bitrate_allocation.GetSumBps() >> (kBitRateScale + H264SPS::kBitRateScaleConstantTerm)) - 1;
    current_sps_.cpb_size_value_minus1[0] = (curr_params_.cpb_size_bits >> (kCPBSizeScale + H264SPS::kCPBSizeScaleConstantTerm)) - 1;
...
void H264VaapiVideoEncoderDelegate::UpdatePPS()
{
    ...
    current_pps_.num_ref_idx_l0_default_active_minus1 = curr_params_.max_ref_pic_list0_size - 1;
...
void H264VaapiVideoEncoderDelegate::GeneratePackedSliceHeader(H26xAnnexBBitstreamBuilder& packed_slice_header, const VAEncPictureParameterBufferH264& pic_param,
    const VAEncSliceParameterBufferH264& slice_param, const H264Picture& pic)
{
    ...
    packed_slice_header.AppendBits(current_sps_.log2_max_pic_order_cnt_lsb_minus4 + 4, pic_param.CurrPic.TopFieldOrderCnt);
    ...
    DCHECK(!pic_param.pic_fields.bits.weighted_pred_flag || !(slice_param.slice_type == H264SliceHeader::kPSlice));
    ...
    if (pic_param.pic_fields.bits.entropy_coding_mode_flag && slice_param.slice_type != H264SliceHeader::kISlice) {
...
bool H264VaapiVideoEncoderDelegate::SubmitFrameParameters(EncodeJob& job, const H264VaapiVideoEncoderDelegate::EncodeParams& encode_params, const H264SPS& sps,
    const H264PPS& pps, scoped_refptr<H264Picture> pic, const base::circular_deque<scoped_refptr<H264Picture>>& ref_pic_list0,
    const std::optional<size_t>& ref_frame_index, const std::optional<int>& qp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (bitrate.mode() == Bitrate::Mode::kVariable) {
        ...
        base::CheckedNumeric<uint32_t> checked_percentage = base::CheckDiv(base::CheckMul<uint32_t>(bitrate.target_bps(), 100u), bitrate.peak_bps());
        if (!checked_percentage.AssignIfValid(&target_percentage)) {
            DVLOGF(1) << "Integer overflow while computing target percentage for bitrate.";
            return false;
        }
...
#undef SPS_TO_SP
    ...
    pic_param.num_ref_idx_l0_active_minus1 = pps.num_ref_idx_l0_default_active_minus1;
...
#undef PPS_TO_PP_PF
    ...
    if (slice_param.slice_type == H264SliceHeader::kPSlice) {
        slice_param.num_ref_idx_l0_active_minus1 = ref_frame_index.has_value() ? 0 : ref_pic_list0.size() - 1;
    } else {
    ...
    std::vector<VaapiWrapper::VABufferDescriptor> va_buffers = { { VAEncSequenceParameterBufferType, sizeof(seq_param), &seq_param },
        { VAEncPictureParameterBufferType, sizeof(pic_param), &pic_param }, { VAEncSliceParameterBufferType, sizeof(slice_param), &slice_param } };

    std::vector<uint8_t> misc_buffers[3];
    if (!qp.has_value()) {
        CHECK(!rate_ctrl_);
        CreateVAEncRateControlParams(bitrate_bps, target_percentage, encode_params.cpb_window_size_ms, base::strict_cast<uint32_t>(pic_param.pic_init_qp),
            base::strict_cast<uint32_t>(encode_params.min_qp), base::strict_cast<uint32_t>(encode_params.max_qp), encode_params.framerate,
            base::strict_cast<uint32_t>(encode_params.cpb_size_bits), misc_buffers);
        va_buffers.push_back({ VAEncMiscParameterBufferType, misc_buffers[0].size(), misc_buffers[0].data() });
        va_buffers.push_back({ VAEncMiscParameterBufferType, misc_buffers[1].size(), misc_buffers[1].data() });
        va_buffers.push_back({ VAEncMiscParameterBufferType, misc_buffers[2].size(), misc_buffers[2].data() });
    }
    ...
    if (submit_packed_headers_) {
        ...
        va_buffers.push_back({ VAEncPackedHeaderParameterBufferType, sizeof(packed_slice_param_buffer), &packed_slice_param_buffer });
        va_buffers.push_back({ VAEncPackedHeaderDataBufferType, packed_slice_header.BytesInBuffer(), packed_slice_header.data() });
    }

    return vaapi_wrapper_->SubmitBuffers(va_buffers);
}

bool H264VaapiVideoEncoderDelegate::SubmitPackedHeaders(const H26xAnnexBBitstreamBuilder& packed_sps, const H26xAnnexBBitstreamBuilder& packed_pps)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    return vaapi_wrapper_->SubmitBuffers({ { VAEncPackedHeaderParameterBufferType, sizeof(packed_sps_param), &packed_sps_param },
        { VAEncPackedHeaderDataBufferType, packed_sps.BytesInBuffer(), packed_sps.data() },
        { VAEncPackedHeaderParameterBufferType, sizeof(packed_pps_param), &packed_pps_param },
        { VAEncPackedHeaderDataBufferType, packed_pps.BytesInBuffer(), packed_pps.data() } });
}

void H264VaapiVideoEncoderDelegate::BitrateControlUpdate(const BitstreamBufferMetadata& metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!rate_ctrl_) {
        return;
    ...
    H264FrameParamsRTC frame_params {};
    if (metadata.h264) {
        frame_params.temporal_layer_id = static_cast<int>(metadata.h264->temporal_idx);
    } else {
    ...
    DVLOGF(4) << "temporal_idx=" << (metadata.h264 ? metadata.h264->temporal_idx : 0) << ", encoded chunk size=" << metadata.payload_size_bytes
              << ", timestamp=" << metadata.timestamp << ", keyframe=" << metadata.key_frame;
