...
template <typename T> class ScopedID;
...
class MEDIA_GPU_EXPORT VaapiVideoDecodeAccelerator : public VideoDecodeAccelerator,
                                                     public DecodeSurfaceHandler<VASurface>,
                                                     public base::trace_event::MemoryDumpProvider {
public:
    VaapiVideoDecodeAccelerator(const MakeGLContextCurrentCallback& make_context_current_cb, const BindGLImageCallback& bind_image_cb);

    VaapiVideoDecodeAccelerator(const VaapiVideoDecodeAccelerator&) = delete;
    VaapiVideoDecodeAccelerator& operator=(const VaapiVideoDecodeAccelerator&) = delete;
    ...
    void SurfaceReady(scoped_refptr<VASurface> va_surface, int32_t bitstream_id, const gfx::Rect& visible_rect, const VideoColorSpace& color_space) override;
...
private:
    ...
    void QueueInputBuffer(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id);
    ...
    void OutputPicture(scoped_refptr<VASurface> va_surface, int32_t input_id, gfx::Rect visible_rect, const VideoColorSpace& picture_color_space);
    ...
    void RecycleVASurface(std::unique_ptr<ScopedVASurfaceID> va_surface, VASurfaceID va_surface_id);
    ...
    // by |decoder_|.
    void InitiateSurfaceSetChange(size_t num_pics, gfx::Size size, size_t num_reference_frames, const gfx::Rect& visible_rect);
    ...
    base::small_map<std::map<int32_t, std::unique_ptr<VaapiPicture>>> pictures_ GUARDED_BY(lock_);
    ...
    std::list<std::unique_ptr<ScopedVASurfaceID>> available_va_surfaces_ GUARDED_BY(lock_);
    ...
    base::RepeatingCallback<void(std::unique_ptr<ScopedVASurfaceID>, VASurfaceID)> va_surface_recycle_cb_;
