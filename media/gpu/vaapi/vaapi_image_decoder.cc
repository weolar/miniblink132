...
void VAContextAndScopedVASurfaceDeleter::operator()(ScopedVASurface* scoped_va_surface) const
{
...
bool VaapiImageDecoder::Initialize(const ReportErrorToUMACB& error_uma_cb)
{
    ...
    vaapi_wrapper_ = VaapiWrapper::Create(VaapiWrapper::kDecode, va_profile_, EncryptionScheme::kUnencrypted, error_uma_cb).value_or(nullptr);
    return !!vaapi_wrapper_;
}

VaapiImageDecodeStatus VaapiImageDecoder::Decode(base::span<const uint8_t> encoded_image)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    if (!vaapi_wrapper_) {
    ...
    const VaapiImageDecodeStatus status = AllocateVASurfaceAndSubmitVABuffers(encoded_image);
    if (status != VaapiImageDecodeStatus::kSuccess) {
        scoped_va_context_and_surface_.reset();
        return status;
    }

    if (!vaapi_wrapper_->ExecuteAndDestroyPendingBuffers(scoped_va_context_and_surface_->id())) {
...
std::unique_ptr<NativePixmapAndSizeInfo> VaapiImageDecoder::ExportAsNativePixmapDmaBuf(VaapiImageDecodeStatus* status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
    DCHECK(status);
    ...
    ScopedVAContextAndSurface temp_scoped_va_surface = std::move(scoped_va_context_and_surface_);
    ...
    std::unique_ptr<NativePixmapAndSizeInfo> exported_pixmap = vaapi_wrapper_->ExportVASurfaceAsNativePixmapDmaBuf(*temp_scoped_va_surface);
    if (!exported_pixmap) {
        *status = VaapiImageDecodeStatus::kCannotExportSurface;
        return nullptr;
    }

    DCHECK_EQ(temp_scoped_va_surface->size(), exported_pixmap->pixmap->GetBufferSize());
