...
public:
    ...
    VaapiVideoDecoderDelegate(VaapiDecodeSurfaceHandler* const vaapi_dec, scoped_refptr<VaapiWrapper> vaapi_wrapper,
        ProtectedSessionUpdateCB on_protected_session_update_cb, CdmContext* cdm_context, EncryptionScheme encryption_scheme = EncryptionScheme::kUnencrypted);
...
protected:
    ...
    enum class ProtectedSessionState { kNotCreated, kInProcess, kCreated, kNeedsRecovery, kFailed };
...
#if BUILDFLAG(IS_CHROMEOS_ASH)
    ProtectedSessionState SetupDecryptDecode(bool full_sample, size_t size, VAEncryptionParameters* crypto_params,
        std::vector<VAEncryptionSegmentInfo>* segments, const std::vector<SubsampleEntry>& subsamples);
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    {
        return (encryption_scheme_ != EncryptionScheme::kUnencrypted) && !transcryption_;
    }
...
private:
    ...
    void OnGetHwKeyData(const std::string& key_id, Decryptor::Status status, const std::vector<uint8_t>& key_data);
