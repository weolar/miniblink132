...
#if DCHECK_IS_ON()
#define VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker) DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker)
#else
#define VAAPI_CHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker) CHECK(sequence_checker.CalledOnValidSequence())
#endif
...
public:
    ...
    VADisplayStateSingleton* operator->()
    {
        return va_display_state_;
    }
...
class MEDIA_GPU_EXPORT VaapiWrapper : public base::RefCountedThreadSafe<VaapiWrapper> {
public:
    ...
    // TODO(mcasas): Add and use a VaapiWrapperStatus instead of reusing here
    // DecoderStatus.
    static base::expected<scoped_refptr<VaapiWrapper>, DecoderStatus> CreateForVideoCodec(
    ...
    static std::vector<SVCScalabilityMode> GetSupportedScalabilityModes(VideoCodecProfile media_profile, VAProfile va_profile);
    ...
    static InternalFormats GetDecodeSupportedInternalFormats(VAProfile va_profile);
    ...
    static bool IsDecodingSupportedForInternalFormat(VAProfile va_profile, unsigned int rt_format);
    ...
    static bool GetSupportedResolutions(VAProfile va_profile, CodecMode codec_mode, gfx::Size& min_size, gfx::Size& max_size);
    ...
    static bool GetJpegDecodeSuitableImageFourCC(unsigned int rt_format, uint32_t preferred_fourcc, uint32_t* suitable_fourcc);
    ...
    static bool IsVppSupportedForJpegDecodedSurfaceToFourCC(unsigned int rt_format, uint32_t fourcc);
    ...
    static std::map<VAProfile, std::vector<VAEntrypoint>> GetSupportedConfigurationsForCodecModeForTesting(CodecMode mode);
    ...
    [[nodiscard]] virtual bool CreateContextAndSurfaces(unsigned int va_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& surface_usage_hints,
    ...
    virtual std::vector<std::unique_ptr<ScopedVASurface>> CreateContextAndScopedVASurfaces(unsigned int va_format, const gfx::Size& size,
    ...
    bool CreateProtectedSession(media::EncryptionScheme encryption, const std::vector<uint8_t>& hw_config, std::vector<uint8_t>* hw_identifier_out);
    ...
    virtual std::vector<std::unique_ptr<ScopedVASurface>> CreateScopedVASurfaces(unsigned int va_rt_format, const gfx::Size& size,
        const std::vector<SurfaceUsageHint>& usage_hints, size_t num_surfaces, const std::optional<gfx::Size>& visible_size,
    ...
    // only true for AMD).
    std::unique_ptr<ScopedVASurface> CreateVASurfaceForFrameResource(const FrameResource& frame, bool protected_content);
    ...
    virtual std::unique_ptr<ScopedVASurface> CreateVASurfaceForPixmap(scoped_refptr<const gfx::NativePixmap> pixmap, bool protected_content = false);
    ...
    // complete.
    std::unique_ptr<ScopedVASurface> CreateVASurfaceForUserPtr(const gfx::Size& size, uintptr_t* buffers, size_t buffer_size);
    ...
    // documentation.
    std::unique_ptr<NativePixmapAndSizeInfo> ExportVASurfaceAsNativePixmapDmaBuf(const ScopedVASurface& scoped_va_surface);
    ...
    // are destroyed.
    [[nodiscard]] bool SubmitBuffer(VABufferType va_buffer_type, size_t size, const void* data);
    // Convenient templatized version of SubmitBuffer() where |size| is deduced to
    // be the size of the type of |*data|.
    template <typename T> [[nodiscard]] bool SubmitBuffer(VABufferType va_buffer_type, const T* data)
    {
    ...
    [[nodiscard]] bool SubmitBuffers(const std::vector<VABufferDescriptor>& va_buffers);
    ...
    [[nodiscard]] virtual bool ExecuteAndDestroyPendingBuffers(VASurfaceID va_surface_id);
    ...
    [[nodiscard]] bool MapAndCopyAndExecute(VASurfaceID va_surface_id, const std::vector<std::pair<VABufferID, VABufferDescriptor>>& va_buffers);
    ...
    std::unique_ptr<ScopedVAImage> CreateVaImage(VASurfaceID va_surface_id, const VAImageFormat& format, const gfx::Size& size);

    // Uploads contents of |frame| into |va_surface_id| for encode.
    [[nodiscard]] virtual bool UploadVideoFrameToSurface(const VideoFrame& frame, VASurfaceID va_surface_id, const gfx::Size& va_surface_size);

    // Creates a buffer of |size| bytes to be used as encode output.
    virtual std::unique_ptr<ScopedVABuffer> CreateVABuffer(VABufferType type, size_t size);
    ...
    // reason.
    [[nodiscard]] virtual uint64_t GetEncodedChunkSize(VABufferID buffer_id, VASurfaceID sync_surface_id);
    ...
    [[nodiscard]] virtual bool DownloadFromVABuffer(
        VABufferID buffer_id, std::optional<VASurfaceID> sync_surface_id, uint8_t* target_ptr, size_t target_size, size_t* coded_data_size);
    ...
    [[nodiscard]] virtual bool GetVAEncMaxNumOfRefFrames(VideoCodecProfile profile, size_t* max_ref_frames);
    ...
    [[nodiscard]] virtual bool GetSupportedPackedHeaders(VideoCodecProfile profile, bool& packed_sps, bool& packed_pps, bool& packed_slice);

    // Gets the minimum segment block size supported for AV1 encoding.
    [[nodiscard]] bool GetMinAV1SegmentSize(VideoCodecProfile profile, uint32_t& min_seg_size);
    ...
    [[nodiscard]] virtual bool BlitSurface(VASurfaceID va_surface_src_id, const gfx::Size& va_surface_src_size, VASurfaceID va_surface_dst_id,
        const gfx::Size& va_surface_dst_size, std::optional<gfx::Rect> src_rect = std::nullopt, std::optional<gfx::Rect> dest_rect = std::nullopt
#if BUILDFLAG(IS_CHROMEOS_ASH)
        ,
    ...
    static void PreSandboxInitialization(bool allow_disabling_global_lock = false);
...
private:
    ...
    [[nodiscard]] bool Initialize(VAProfile va_profile, EncryptionScheme encryption_scheme);
    void Deinitialize();
    ...
    [[nodiscard]] bool CreateSurfaces(unsigned int va_format, const gfx::Size& size, const std::vector<SurfaceUsageHint>& usage_hints, size_t num_surfaces,
    ...
    // |va_surface_size|.
    std::unique_ptr<NativePixmapAndSizeInfo> ExportVASurfaceAsNativePixmapDmaBufUnwrapped(VASurfaceID va_surface_id, const gfx::Size& va_surface_size);
    ...
    [[nodiscard]] bool Execute_Locked(VASurfaceID va_surface_id, const std::vector<VABufferID>& va_buffers) EXCLUSIVE_LOCKS_REQUIRED(va_lock_);

    virtual void DestroyPendingBuffers_Locked() EXCLUSIVE_LOCKS_REQUIRED(va_lock_);
    ...
    [[nodiscard]] virtual bool SubmitBuffer_Locked(const VABufferDescriptor& va_buffer) EXCLUSIVE_LOCKS_REQUIRED(va_lock_);

    // Maps |va_buffer_id| and, if successful, copies the contents of |va_buffer|
    // into it.
    [[nodiscard]] bool MapAndCopy_Locked(VABufferID va_buffer_id, const VABufferDescriptor& va_buffer) EXCLUSIVE_LOCKS_REQUIRED(va_lock_);
    ...
    [[nodiscard]] bool MaybeAttachProtectedSession_Locked() EXCLUSIVE_LOCKS_REQUIRED(va_lock_);
    ...
    VADisplayStateHandle va_display_state_handle_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    VADisplay va_display_ GUARDED_BY(va_lock_) GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    VAContextID va_context_id_ GUARDED_BY_CONTEXT(sequence_checker_) { VA_INVALID_ID };
    ...
    std::vector<VABufferID> pending_va_buffers_ GUARDED_BY_CONTEXT(sequence_checker_);

    // VA buffer to be used for kVideoProcess. Allocated the first time around,
    // and reused afterwards.
    std::unique_ptr<ScopedVABuffer> va_buffer_for_vpp_ GUARDED_BY_CONTEXT(sequence_checker_);

#if BUILDFLAG(IS_CHROMEOS_ASH)
    // For protected decode mode.
    VAConfigID va_protected_config_id_ GUARDED_BY_CONTEXT(sequence_checker_) { VA_INVALID_ID };
    VAProtectedSessionID va_protected_session_id_ GUARDED_BY_CONTEXT(sequence_checker_) { VA_INVALID_ID };
#endif

    // Called to report codec errors to UMA. Errors to clients are reported via
    // return values from public methods.
    ReportErrorToUMACB report_error_to_uma_cb_ GUARDED_BY_CONTEXT(sequence_checker_);
};
