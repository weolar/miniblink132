...
TEST(BufferValidationTest, VerifyGmbHandlePasses)
{
    ...
    EXPECT_TRUE(VerifyGpuMemoryBufferHandle(PIXEL_FORMAT_I420, size, gmb_handle, base::BindRepeating(FileSizeForTesting, buffer_size)));
}
...
TEST(BufferValidationTest, VerifyGmbHandleInvalidType)
{
    ...
    EXPECT_FALSE(VerifyGpuMemoryBufferHandle(PIXEL_FORMAT_I420, size, gmb_handle, base::BindRepeating(FileSizeForTesting, size.GetArea() * 2)));
}
...
TEST(BufferValidationTest, VerifyGmbHandlePlanesCountMatches)
{
    ...
    EXPECT_FALSE(VerifyGpuMemoryBufferHandle(PIXEL_FORMAT_UYVY, size, gmb_handle, base::BindRepeating(FileSizeForTesting, buffer_size)));
}
...
TEST(BufferValidationTest, VerifyGmbHandleStridesMonotonicallyDecrease)
{
    ...
    EXPECT_FALSE(VerifyGpuMemoryBufferHandle(PIXEL_FORMAT_I420, size, gmb_handle, base::BindRepeating(FileSizeForTesting, buffer_size)));
}
...
TEST(BufferValidationTest, VerifyGmbHandleOffsetValid)
{
    ...
    EXPECT_FALSE(VerifyGpuMemoryBufferHandle(PIXEL_FORMAT_I420, size, gmb_handle, base::BindRepeating(FileSizeForTesting, 2499)));
}
