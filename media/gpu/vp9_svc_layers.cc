...
constexpr static size_t kMaxNumUsedRefFramesEachSpatialLayer = kVp9NumRefFrames / VP9SVCLayers::kMaxSpatialLayers;
static_assert(kMaxNumUsedRefFramesEachSpatialLayer == 2u, "VP9SVCLayers uses two reference frames for each spatial layer");
constexpr static size_t kMaxNumUsedReferenceFrames = kMaxNumUsedRefFramesEachSpatialLayer * VP9SVCLayers::kMaxSpatialLayers;
static_assert(kMaxNumUsedReferenceFrames == 6u, "VP9SVCLayers uses six reference frames");
...
struct FrameConfig {
    constexpr FrameConfig(size_t layer_index, FrameFlags first, FrameFlags second, bool temporal_up_switch)
    ...
    std::vector<uint8_t> GetRefFrameIndices(size_t spatial_idx) const
    {
        ...
        for (size_t i = 0; i < kMaxNumUsedRefFramesEachSpatialLayer; ++i) {
            if (buffer_flags_[i] & FrameFlags::kReference) {
                indices.push_back(i + kMaxNumUsedRefFramesEachSpatialLayer * spatial_idx);
            }
        }
        return indices;
    }
...
VP9SVCLayers::Config::Config(const std::vector<gfx::Size>& spatial_layer_resolutions, size_t begin_active_layer, size_t end_active_layer,
    size_t num_temporal_layers, SVCInterLayerPredMode inter_layer_pred)
    ...
    , active_spatial_layer_resolutions(spatial_layer_resolutions.begin() + begin_active_layer, spatial_layer_resolutions.begin() + end_active_layer)
...
bool VP9SVCLayers::IsKeyFrame() const
{
    ...
    CHECK(config_.active_spatial_layer_resolutions.size() == 1 || config_.inter_layer_pred == SVCInterLayerPredMode::kOff);
    return true;
}

void VP9SVCLayers::GetPictureParamAndMetadata(PictureParam& picture_param, Vp9Metadata& metadata) const
{
    picture_param.frame_size = config_.active_spatial_layer_resolutions[spatial_idx_];

    if (frame_num_ == 0) {
        FillMetadataForFirstFrame(metadata, picture_param.key_frame, picture_param.refresh_frame_flags, picture_param.reference_frame_indices);
        return;
    }

    picture_param.key_frame = false;
    FillMetadataForNonFirstFrame(metadata, picture_param.refresh_frame_flags, picture_param.reference_frame_indices);
}

void VP9SVCLayers::FillMetadataForFirstFrame(
    Vp9Metadata& metadata, bool& key_frame, uint8_t& refresh_frame_flags, std::vector<uint8_t>& reference_frame_indices) const
{
    ...
    metadata.end_of_picture = spatial_idx_ == config_.active_spatial_layer_resolutions.size() - 1;
    ...
    if (spatial_idx_ == 0) {
        ...
        refresh_frame_flags = 1 << (spatial_idx_ * kMaxNumUsedRefFramesEachSpatialLayer);
        reference_frame_indices = {};
        if (config_.inter_layer_pred == SVCInterLayerPredMode::kOnKeyPic) {
            reference_frame_indices = { base::checked_cast<uint8_t>((spatial_idx_ - 1) * kMaxNumUsedRefFramesEachSpatialLayer) };
        }
    }

    if (key_frame) {
        metadata.spatial_layer_resolutions = config_.active_spatial_layer_resolutions;
        metadata.begin_active_spatial_layer_index = base::checked_cast<uint8_t>(config_.begin_active_layer);
        metadata.end_active_spatial_layer_index = base::checked_cast<uint8_t>(config_.end_active_layer);
    }
}

void VP9SVCLayers::FillMetadataForNonFirstFrame(Vp9Metadata& metadata, uint8_t& refresh_frame_flags, std::vector<uint8_t>& reference_frame_indices) const
{
    CHECK_NE(frame_num_, 0u);

    const FrameConfig frame_config = GetFrameConfig(config_.num_temporal_layers, frame_num_);
    ...
    for (const uint8_t i : reference_frame_indices) {
        const uint8_t p_diff = base::checked_cast<uint8_t>(frame_num_ - frame_num_ref_frames_[i]);
