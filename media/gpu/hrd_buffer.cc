...
HRDBuffer::HRDBuffer(size_t buffer_size, uint32_t avg_bitrate, int last_frame_buffer_bytes, base::TimeDelta last_frame_timestamp)
...
{
    const base::TimeDelta elapsed_time = h264_rate_control_util::ClampedTimestampDiff(timestamp, last_frame_timestamp_);
    const float max_rate_bytes_per_sec = avg_bitrate_ / 8.0f;
    const float max_flow_in_bytes_since_last_frame = max_rate_bytes_per_sec / base::Time::kMicrosecondsPerSecond * elapsed_time.InMicroseconds();
    return std::max(static_cast<int>(last_frame_buffer_bytes_ - max_flow_in_bytes_since_last_frame), 0);
}
...
void HRDBuffer::SetParameters(size_t buffer_size, uint32_t avg_bitrate, uint32_t peak_bitrate, bool ease_hrd_reduction)
{
    ...
    constexpr base::TimeDelta kMinShrinkingBucketWaitTime = base::Milliseconds(500);

    base::TimeDelta wait_time = base::TimeDelta();
    // Check if HRD becomes smaller. If so, gradually converge to the smaller
    // size.
    if (last_frame_buffer_bytes_ > 0 && ease_hrd_reduction && old_buffer_size > new_buffer_size_) {
        float peak_avg_ratio = 1;
        if (avg_bitrate > 0) {
            peak_avg_ratio = static_cast<float>(peak_bitrate) / static_cast<float>(avg_bitrate);
        }

        // Shrink HRD buffer size over time - the duration depends on the peak to
        // average bitrate ratio.
        wait_time = std::max(wait_time, base::Microseconds((peak_avg_ratio - 1.0f) * base::Time::kMicrosecondsPerSecond));
...
{
    if (new_buffer_size_ >= buffer_size_ || shrinking_bucket_wait_time_ <= base::TimeDelta()) {
        return;
    }

    base::TimeDelta elapsed_time = h264_rate_control_util::ClampedTimestampDiff(timestamp, last_frame_timestamp_);
    size_t target_buffer_size
        = buffer_size_ - static_cast<int>((elapsed_time.InMillisecondsF() / shrinking_bucket_wait_time_.InMillisecondsF()) * buffer_size_delta_rate_);
    buffer_size_ = std::max(target_buffer_size, new_buffer_size_);
}

void HRDBuffer::AddFrameBytes(size_t frame_bytes, base::TimeDelta frame_timestamp)
{
