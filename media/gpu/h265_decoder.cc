...
struct POCAscCompare {
    bool operator()(const scoped_refptr<H265Picture>& a, const scoped_refptr<H265Picture>& b) const
    {
...
scoped_refptr<H265Picture> H265Decoder::H265Accelerator::CreateH265PictureSecure(uint64_t secure_handle)
{
    return nullptr;
}

void H265Decoder::H265Accelerator::ProcessVPS(const H265VPS* vps, base::span<const uint8_t> vps_nalu_data)
{
}

void H265Decoder::H265Accelerator::ProcessSPS(const H265SPS* sps, base::span<const uint8_t> sps_nalu_data)
{
}

void H265Decoder::H265Accelerator::ProcessPPS(const H265PPS* pps, base::span<const uint8_t> pps_nalu_data)
{
}

H265Decoder::H265Accelerator::Status H265Decoder::H265Accelerator::SetStream(base::span<const uint8_t> stream, const DecryptConfig* decrypt_config)
{
...
H265Decoder::H265Decoder(std::unique_ptr<H265Accelerator> accelerator, VideoCodecProfile profile, const VideoColorSpace& container_color_space)
    : state_(kAfterReset)
...
H265Decoder::DecodeResult H265Decoder::Decode()
{
    ...
    while (true) {
        ...
        if (curr_nalu_->nuh_layer_id) {
            ...
            if (aux_alpha_layer_id_ == curr_nalu_->nuh_layer_id && accelerator_->IsAlphaLayerSupported()) {
                ...
                case H265NALU::CRA_NUT: {
                    if (!curr_slice_hdr_) {
                        ...
                        par_res = parser_.ParseSliceHeader(*curr_nalu_, curr_slice_hdr_.get(), last_slice_hdr_.get());
                    ...
                    const H265PPS* pps = parser_.GetPPS(curr_slice_hdr_->slice_pic_parameter_set_id);
                    const H265SPS* sps = parser_.GetSPS(pps->pps_seq_parameter_set_id);
                    H265Picture::Vector empty;
                    CHECK_ACCELERATOR_RESULT(accelerator_->SubmitSlice(sps, pps, curr_slice_hdr_.get(), empty, empty, empty, empty, empty, curr_pic_.get(),
                        curr_slice_hdr_->nalu_data, curr_slice_hdr_->nalu_size, parser_.GetCurrentSubsamples()));
                ...
                case H265NALU::SPS_NUT: {
                    ...
                    accelerator_->ProcessSPS(
                        parser_.GetSPS(sps_id), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
                    break;
                }
                case H265NALU::PPS_NUT: {
                    ...
                    accelerator_->ProcessPPS(
                        parser_.GetPPS(pps_id), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
                    break;
                }
                default:
                    break;
                ...
                DVLOG(4) << "Skipping NALU with nuh_layer_id=" << curr_nalu_->nuh_layer_id;
            }
            curr_nalu_.reset();
            continue;
        }
        ...
        case H265NALU::VPS_NUT:
            ...
            accelerator_->ProcessVPS(parser_.GetVPS(vps_id), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
            break;
        case H265NALU::SPS_NUT:
            CHECK_ACCELERATOR_RESULT(FinishPrevFrameIfPresent());
            int sps_id;
        ...
        case H265NALU::PPS_NUT:
            ...
            if (par_res != H265Parser::kOk)
                SET_ERROR_AND_RETURN();
            accelerator_->ProcessPPS(parser_.GetPPS(pps_id), base::span<const uint8_t>(curr_nalu_->data.get(), base::checked_cast<size_t>(curr_nalu_->size)));
...
bool H265Decoder::ProcessPPS(int pps_id, bool* need_new_buffers)
{
    ...
    if (new_color_space.matrix == VideoColorSpace::MatrixID::RGB && new_chroma_sampling != VideoChromaSampling::k444) {
    ...
    if (pic_size_ != new_pic_size || dpb_.max_num_pics() != sps->max_dpb_size || profile_ != new_profile || bit_depth_ != new_bit_depth
        || chroma_sampling_ != new_chroma_sampling || is_color_space_change) {
        if (!Flush())
            return false;
        DVLOG(1) << "Codec profile: " << GetProfileName(new_profile) << ", level(x30): " << sps->profile_tier_level.general_level_idc
...
H265Decoder::H265Accelerator::Status H265Decoder::ProcessCurrentSlice()
{
    ...
    return accelerator_->SubmitSlice(sps, pps, slice_hdr, ref_pic_list0_, ref_pic_list1_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_,
        ref_pic_set_st_curr_before_, curr_pic_.get(), slice_hdr->nalu_data, slice_hdr->nalu_size, parser_.GetCurrentSubsamples());
}
...
H265Decoder::H265Accelerator::Status H265Decoder::StartNewFrame(const H265SliceHeader* slice_hdr)
{
    CHECK(curr_pic_.get());
    DCHECK(slice_hdr);
    ...
    return accelerator_->SubmitFrameMetadata(
        sps, pps, slice_hdr, ref_pic_list_, ref_pic_set_lt_curr_, ref_pic_set_st_curr_after_, ref_pic_set_st_curr_before_, curr_pic_);
}
        ...
        do {
            ...
            if (sps->sps_max_latency_increase_plus1[highest_tid] != 0) {
                for (const auto& pic : not_outputted) {
                    if (pic->pic_latency_count_ >= sps->sps_max_latency_pictures[highest_tid]) {
...
bool H265Decoder::FinishPicture(scoped_refptr<H265Picture> pic, std::unique_ptr<H265SliceHeader> slice_hdr)
{
    // 8.3.1
    ...
    if (sps->sps_max_latency_increase_plus1[highest_tid] != 0) {
        for (auto& pending_output_pic : not_outputted) {
            if (pending_output_pic->pic_latency_count_ >= sps->sps_max_latency_pictures[highest_tid]) {
                ++pic_latency_output_count;
            }
        }
    }
