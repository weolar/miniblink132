...
std::optional<base::FilePath> ResolveFilePath(const base::FilePath& file_path)
{
    ...
    if (!resolved_path.IsAbsolute()) {
        resolved_path = base::MakeAbsoluteFilePath(PathExists(resolved_path) ? resolved_path : media::GetTestDataPath().Append(resolved_path));
    }

    return PathExists(resolved_path) ? std::optional<base::FilePath>(resolved_path) : std::nullopt;
}
...
bool Image::LoadMetadata()
{
    ...
    if (!metadata_result.has_value()) {
        VLOGF(1) << "Failed to parse image metadata: " << json_path << ": " << metadata_result.error().message;
        return false;
    }
    ...
    const base::Value::List* visible_rect_info = metadata.FindList("visible_rect");
    if (visible_rect_info) {
        ...
        visible_rect_ = gfx::Rect(origin_x, origin_y, visible_width, visible_height);
    }
