...
VideoFrameFileWriter::VideoFrameFileWriter(
    const base::FilePath& output_folder, OutputFormat output_format, size_t output_limit, const base::FilePath::StringType& output_file_prefix)
...
VideoFrameFileWriter::~VideoFrameFileWriter()
{
    ...
    if (frame_writer_thread_.task_runner()) {
        ...
        frame_writer_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoFrameFileWriter::CleanUpOnWriterThread, base::Unretained(this)));
    }
    ...
    const base::FilePath& output_folder, OutputFormat output_format, size_t output_limit, const base::FilePath::StringType& output_file_prefix)
{
    ...
    if (!resolved_output_folder.IsAbsolute()) {
        resolved_output_folder = base::MakeAbsoluteFilePath(base::FilePath(base::FilePath::kCurrentDirectory)).Append(resolved_output_folder);
    }

    // Create the directory tree if it doesn't exist yet.
    if (!DirectoryExists(resolved_output_folder) && !base::CreateDirectory(resolved_output_folder)) {
        LOG(ERROR) << "Failed to create a output directory: " << resolved_output_folder;
        return nullptr;
    }

    auto frame_file_writer = base::WrapUnique(new VideoFrameFileWriter(resolved_output_folder, output_format, output_limit, output_file_prefix));
...
void VideoFrameFileWriter::ProcessVideoFrame(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    ...
    if (video_frame->visible_rect().IsEmpty()) {
        ...
        DLOG(WARNING) << "Skipping writing, frame_index=" << frame_index << " because visible_rect is empty";
        return;
    }
    ...
    frame_writer_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&VideoFrameFileWriter::ProcessVideoFrameTask, base::Unretained(this), video_frame, frame_index));
}
...
void VideoFrameFileWriter::ProcessVideoFrameTask(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(writer_thread_sequence_checker_);
    ...
    if (!output_file_prefix_.empty()) {
        file_path = base::FilePath(output_file_prefix_)
                        .AppendASCII("_")
                        .Append(base::FilePath::FromASCII(base::StringPrintf("frame_%04zu_%dx%d", frame_index, visible_size.width(), visible_size.height())));
    } else {
        file_path = base::FilePath::FromASCII(base::StringPrintf("frame_%04zu_%dx%d", frame_index, visible_size.width(), visible_size.height()));
    }
    ...
    if ((frame->storage_type() == VideoFrame::STORAGE_DMABUFS) && !video_frame_mapper_) {
...
void VideoFrameFileWriter::WriteVideoFramePNG(scoped_refptr<const VideoFrame> video_frame, const base::FilePath& filename)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(writer_thread_sequence_checker_);
    ...
    if (argb_out_frame->format() != PIXEL_FORMAT_ARGB) {
        argb_out_frame = ConvertVideoFrame(argb_out_frame.get(), VideoPixelFormat::PIXEL_FORMAT_ARGB);
    }

    // Convert the ARGB frame to PNG.
    std::optional<std::vector<uint8_t>> png_output = gfx::PNGCodec::Encode(argb_out_frame->visible_data(VideoFrame::Plane::kARGB), gfx::PNGCodec::FORMAT_BGRA,
        argb_out_frame->visible_rect().size(), argb_out_frame->stride(VideoFrame::Plane::kARGB),
        /*discard_transparency=*/true, std::vector<gfx::PNGCodec::Comment>());

    // Write the PNG data to file.
    base::FilePath file_path(output_folder_.Append(filename).AddExtension(FILE_PATH_LITERAL(".png")));
    ASSERT_TRUE(base::WriteFile(file_path, png_output.value()));
}

void VideoFrameFileWriter::WriteVideoFrameYUV(scoped_refptr<const VideoFrame> video_frame, const base::FilePath& filename)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(writer_thread_sequence_checker_);
    ...
    const VideoPixelFormat yuv_out_format = VideoFrame::BytesPerElement(mapped_frame->format(), 0) > 1 ? PIXEL_FORMAT_YUV420P10 : PIXEL_FORMAT_I420;
    scoped_refptr<const VideoFrame> out_frame = mapped_frame;
    if (out_frame->format() != PIXEL_FORMAT_I420 && out_frame->format() != PIXEL_FORMAT_YUV420P10) {
        out_frame = ConvertVideoFrame(out_frame.get(), yuv_out_format);
    }

    // Write the YUV data to file.
    base::FilePath file_path(output_folder_.Append(filename)
                                 .AddExtension(FILE_PATH_LITERAL(".yuv"))
                                 .InsertBeforeExtension(yuv_out_format == PIXEL_FORMAT_I420 ? FILE_PATH_LITERAL("_I420") : FILE_PATH_LITERAL("_I420P10")));
    base::File yuv_file(file_path, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    ...
    for (size_t i = 0; i < num_planes; i++) {
        ...
        const size_t rows = VideoFrame::Rows(i, pixel_format, visible_size.height());
        const int row_bytes = VideoFrame::RowBytes(i, pixel_format, visible_size.width());
        ASSERT_TRUE(stride > 0);
        for (size_t row = 0; row < rows; ++row) {
            if (yuv_file.WriteAtCurrentPos(reinterpret_cast<const char*>(data + (stride * row)), row_bytes) != row_bytes) {
                LOG(ERROR) << "Failed to write plane #" << i << " to file: " << base::File::ErrorToString(base::File::GetLastFileError());
            }
        }
    }
}
