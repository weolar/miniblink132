...
public:
    ...
    // thread.
    virtual void ProcessVideoFrame(scoped_refptr<const VideoFrame> video_frame, size_t frame_index) = 0;
...
scoped_refptr<VideoFrame> CloneVideoFrame(const VideoFrame* const src_frame, const VideoFrameLayout& dst_layout,
    VideoFrame::StorageType dst_storage_type = VideoFrame::STORAGE_OWNED_MEMORY, std::optional<gfx::BufferUsage> dst_buffer_usage = std::nullopt);
...
scoped_refptr<VideoFrame> CreateDmabufVideoFrame(const VideoFrame* const src_frame);
...
std::optional<VideoFrameLayout> CreateVideoFrameLayout(VideoPixelFormat pixel_format, const gfx::Size& dimension,
    const uint32_t alignment = VideoFrame::kFrameAddressAlignment, std::vector<size_t>* plane_rows = nullptr);
} // namespace test
} // namespace media
