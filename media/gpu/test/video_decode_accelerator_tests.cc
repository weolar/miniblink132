...
public:
    std::unique_ptr<DecoderListener> CreateDecoderListener(const VideoBitstream* video, DecoderWrapperConfig config = DecoderWrapperConfig(),
        std::unique_ptr<FrameRendererDummy> frame_renderer = FrameRendererDummy::Create())
    {
        LOG_ASSERT(video);
        ...
        if (g_env->GetFrameOutputMode() == FrameOutputMode::kAll) {
            frame_processors.push_back(VideoFrameFileWriter::Create(output_folder, g_env->GetFrameOutputFormat(), g_env->GetFrameOutputLimit()));
        ...
        if (g_env->IsValidatorEnabled()) {
            ...
            if (g_env->GetFrameOutputMode() == FrameOutputMode::kCorrupt) {
                frame_writer = VideoFrameFileWriter::Create(output_folder, g_env->GetFrameOutputFormat(), g_env->GetFrameOutputLimit());
            }
            if (g_env->Video()->BitDepth() != 8u && g_env->Video()->BitDepth() != 10u) {
                LOG(ERROR) << "Unsupported bit depth: " << base::strict_cast<int>(g_env->Video()->BitDepth());
                ADD_FAILURE();
            }
            const VideoPixelFormat validation_format = g_env->Video()->BitDepth() == 10 ? PIXEL_FORMAT_YUV420P10 : PIXEL_FORMAT_I420;
            if (g_env->GetValidatorType() == VideoPlayerTestEnvironment::ValidatorType::kMD5) {
                frame_processors.push_back(media::test::MD5VideoFrameValidator::Create(video->FrameChecksums(), validation_format, std::move(frame_writer)));
            } else {
                DCHECK_EQ(g_env->GetValidatorType(), VideoPlayerTestEnvironment::ValidatorType::kSSIM);
                ...
                frame_processors.push_back(
                    media::test::SSIMVideoFrameValidator::Create(base::BindRepeating(&VideoDecoderTest::GetModelFrame, base::Unretained(this)),
                        std::move(frame_writer), VideoFrameValidator::ValidationMode::kThreshold, kSSIMTolerance));
            }
        }
...
// VideoDecoderPipeline to work.
        ...
        command_line.AppendSwitchASCII(switches::kHardwareVideoDecodeFrameRate, base::NumberToString(g_env->Video()->FrameRate()));
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        ...
        config.ignore_resolution_changes_to_smaller_vp9 = g_env->Video()->HasKeyFrameLessResolutionChange();
#endif
        ...
        if (g_env->GetFrameOutputMode() != FrameOutputMode::kNone || IsSlowMappingDevice() || g_env->IsV4L2VirtualDriver()) {
            video_player->SetEventWaitTimeout(std::max(kDefaultEventWaitTimeout, g_env->Video()->Duration() * 10));
        }
    ...
    bool InitializeDecoderWithConfig(VideoDecoderConfig& decoder_config)
    {
        ...
        std::unique_ptr<VideoDecoder> decoder
            = VideoDecoderPipeline::Create(gpu::GpuDriverBugWorkarounds(), base::SingleThreadTaskRunner::GetCurrentDefault(), std::move(frame_pool),
        ...
        VideoDecoder::InitCB init_cb = base::BindLambdaForTesting([&init_result](DecoderStatus result) { init_result = result.is_ok(); });
        decoder->Initialize(decoder_config, /*low_delay=*/false,
            /*cdm_context=*/nullptr, std::move(init_cb), base::BindRepeating(&VideoDecoderTest::AddModelFrame, base::Unretained(this)),
...
private:
    bool IsSlowMappingDevice() const
    {
        static const bool is_slow_mapping_device = []() {
            ...
            const bool is_glk_device = cpuid.family() == kPentiumAndLaterFamily && cpuid.model() == kGeminiLakeModelId;
            const bool is_apl_device = cpuid.family() == kPentiumAndLaterFamily && cpuid.model() == kApolloLakeModelId;
    ...
    bool CreateModelFrames(const VideoBitstream* video)
    {
        ...
        Dav1dVideoDecoder decoder(std::make_unique<NullMediaLog>(), OffloadableVideoDecoder::OffloadState::kOffloaded);
        VideoDecoderConfig decoder_config(video->Codec(), video->Profile(), VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
            video->Resolution(), gfx::Rect(video->Resolution()), video->Resolution(), EmptyExtraData(), EncryptionScheme::kUnencrypted);

        bool init_success = false;
        VideoDecoder::InitCB init_cb = base::BindOnce([](bool* init_success, DecoderStatus result) { *init_success = result.is_ok(); }, &init_success);
        decoder.Initialize(decoder_config, /*low_delay=*/false,
            /*cdm_context=*/nullptr, std::move(init_cb), base::BindRepeating(&VideoDecoderTest::AddModelFrame, base::Unretained(this)),
            /*waiting_cb=*/base::NullCallback());
        if (!init_success)
            return false;
        auto encoded_data_helper = EncodedDataHelper::Create(video->Data(), video->Codec());
        DCHECK(encoded_data_helper);
        while (!encoded_data_helper->ReachEndOfStream()) {
            bool decode_success = false;
            media::VideoDecoder::DecodeCB decode_cb
                = base::BindOnce([](bool* decode_success, DecoderStatus status) { *decode_success = status.is_ok(); }, &decode_success);
            scoped_refptr<DecoderBuffer> bitstream_buffer = encoded_data_helper->GetNextBuffer();
            if (!bitstream_buffer) {
        ...
        media::VideoDecoder::DecodeCB flush_cb
            = base::BindOnce([](bool* flush_success, DecoderStatus status) { *flush_success = status.is_ok(); }, &flush_success);
...
#endif
    const auto supported_configs = VideoDecoderPipeline::GetSupportedConfigs(decoder_type, gpu::GpuDriverBugWorkarounds());
    ASSERT_FALSE(supported_configs->empty());

    const bool contains_h264 = std::find_if(supported_configs->begin(), supported_configs->end(), [](SupportedVideoDecoderConfig config) {
        return config.profile_min >= H264PROFILE_MIN && config.profile_max <= H264PROFILE_MAX;
...
TEST_F(VideoDecoderTest, DISABLED_FlushMidStream)
{
    ...
    tvp->Play();
    const size_t flush_location_in_frames = std::min(static_cast<size_t>(10), g_env->Video()->NumFrames() / 2);
...
TEST_F(VideoDecoderTest, ResetMidStream)
{
    ...
    EXPECT_EQ(tvp->GetFrameDecodedCount(), numFramesDecoded + g_env->Video()->NumFrames());
    EXPECT_TRUE(tvp->WaitForFrameProcessors());
}
...
TEST_F(VideoDecoderTest, ResetAfterFirstConfigInfo)
{
    ...
    if (g_env->Video()->Codec() != media::VideoCodec::kH264 && g_env->Video()->Codec() != media::VideoCodec::kHEVC)
        GTEST_SKIP();
...
TEST_F(VideoDecoderTest, FlushAtEndOfStream_MultipleConcurrentDecodes)
{
    ...
    std::vector<std::unique_ptr<DecoderListener>> tvps(kMinSupportedConcurrentDecoders);
    ...
    for (size_t i = 0; i < kMinSupportedConcurrentDecoders; ++i) {
        ...
        EXPECT_EQ(tvps[i]->GetFrameDecodedCount(), g_env->Video()->NumFrames()) << "Decoder #" << i;
...
TEST_F(VideoDecoderTest, InitializeWithNonSupportedConfig)
{
    ...
    constexpr VideoCodecProfile kProfileNotSupportedByChromeOS = THEORAPROFILE_ANY;
    VideoDecoderConfig non_supported_decoder_config(video->Codec(), kProfileNotSupportedByChromeOS, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(),
        kNoTransformation, video->Resolution(), gfx::Rect(video->Resolution()), video->Resolution(), EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
{
    media::test::g_env = media::test::VideoDecodeAcceleratorTestSuite::Create(argc, argv);
    if (!media::test::g_env || !media::test::g_env->ValidVideoTestEnv()) {
        LOG(ERROR) << "Invalid video test environment";
        return EXIT_FAILURE;
    }

    return base::LaunchUnitTests(argc, argv, base::BindOnce(&media::test::VideoDecodeAcceleratorTestSuite::Run, base::Unretained(media::test::g_env)));
}
