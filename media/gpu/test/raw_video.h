...
public:
    ...
    struct FrameData {
        FrameData(const std::vector<const uint8_t*>& plane_addrs, const std::vector<size_t>& strides, std::vector<uint8_t> buffer);
    ...
    static std::unique_ptr<RawVideo> Create(const base::FilePath& file_path, const base::FilePath& metadata_file_path, bool read_all_frames);
    ...
    std::unique_ptr<RawVideo> CreateExpandedVideo(const gfx::Size& resolution, const gfx::Rect& visible_rect) const;
    ...
    {
        return metadata_.frame_rate;
    }
    size_t NumFrames() const
    {
        return metadata_.num_frames;
    }
...
private:
    ...
    RawVideo(std::unique_ptr<VP9Decoder> vp9_decoder, const Metadata& metadata, size_t video_frame_size);
    RawVideo(std::unique_ptr<base::MemoryMappedFile> memory_mapped_file, const Metadata& metadata, size_t video_frame_size);

    static base::FilePath ResolveFilePath(const base::FilePath& file_path);
    static bool LoadMetadata(const base::FilePath& json_file_path, Metadata& metadata, bool& compressed_data);
