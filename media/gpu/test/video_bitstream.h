...
public:
    ...
    static std::unique_ptr<VideoBitstream> Create(const base::FilePath& file_path, const base::FilePath& metadata_file_path);
    ...
    {
        return metadata_.profile;
    }
    VideoCodec Codec() const
    {
        return metadata_.codec;
    }
    uint8_t BitDepth() const
    {
        return metadata_.bit_depth;
    }
    ...
    {
        return base::Seconds(static_cast<double>(metadata_.num_frames) / static_cast<double>(metadata_.frame_rate));
    }
...
private:
    ...
    VideoBitstream(std::unique_ptr<base::MemoryMappedFile> memory_mapped_file, const Metadata& metadata);

    static base::FilePath ResolveFilePath(const base::FilePath& file_path);
    static bool LoadMetadata(const base::FilePath& json_file_path, Metadata& metadata);
