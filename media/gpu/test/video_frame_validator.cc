...
VideoFrameValidator::VideoFrameValidator(std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, CropHelper crop_helper)
...
void VideoFrameValidator::Destroy()
{
    if (frame_validator_thread_.task_runner()) {
        ...
        frame_validator_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VideoFrameValidator::CleanUpOnValidatorThread, base::Unretained(this)));
    }
...
void VideoFrameValidator::ProcessVideoFrame(scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    ...
    if (video_frame->visible_rect().IsEmpty()) {
        ...
        DLOG(WARNING) << "Skipping validation, frame_index=" << frame_index << " because visible_rect is empty";
        return;
    }
    ...
    frame_validator_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&VideoFrameValidator::ProcessVideoFrameTask, base::Unretained(this), video_frame, frame_index));
}
...
void VideoFrameValidator::ProcessVideoFrameTask(const scoped_refptr<const VideoFrame> video_frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    ...
    if (frame->storage_type() == VideoFrame::STORAGE_DMABUFS) {
        ...
        if (!video_frame_mapper_) {
            video_frame_mapper_ = VideoFrameMapperFactory::CreateMapper(frame->format(), frame->storage_type());
...
scoped_refptr<VideoFrame> VideoFrameValidator::CloneAndCropFrame(scoped_refptr<const VideoFrame> frame) const
{
    ...
    if (!visible.Contains(crop)) {
        LOG(ERROR) << "Crop " << crop.ToString() << "must be contained by the visible area of the input frame: " << visible.ToString() << ".";
        return cloned_frame;
    }
    return VideoFrame::WrapVideoFrame(cloned_frame, frame->format(), crop, crop.size());
}
...
gfx::Rect BottomRowCrop(int row_height, const VideoFrame& frame)
{
    ...
    if (visible.size().height() < row_height) {
        LOG(ERROR) << "Could not get a row of height " << row_height << " from a visible area of height " << visible.size().height();
        return visible;
    }

    return gfx::Rect { visible.x(), visible.bottom() - row_height, visible.width(), row_height };
}

struct MD5VideoFrameValidator::MD5MismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    MD5MismatchedFrameInfo(size_t frame_index, const std::string& computed_md5, const std::string& expected_md5)
    ...
    {
        LOG(ERROR) << "frame_index: " << frame_index << ", computed_md5: " << computed_md5 << ", expected_md5: " << expected_md5;
    }
...
// static
std::unique_ptr<MD5VideoFrameValidator> MD5VideoFrameValidator::Create(const std::vector<std::string>& expected_frame_checksums,
    VideoPixelFormat validation_format, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, CropHelper crop_helper)
{
    auto video_frame_validator
        = base::WrapUnique(new MD5VideoFrameValidator(expected_frame_checksums, validation_format, std::move(corrupt_frame_processor), std::move(crop_helper)));
...
MD5VideoFrameValidator::MD5VideoFrameValidator(const std::vector<std::string>& expected_frame_checksums, VideoPixelFormat validation_format,
    std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, CropHelper crop_helper)
    : VideoFrameValidator(std::move(corrupt_frame_processor), std::move(crop_helper))
...
std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> MD5VideoFrameValidator::Validate(scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    ...
    if (base::StartsWith(kernel_version, "3.18")) {
        static const bool is_skylake = []() {
            ...
            return cpuid.family() == kPentiumAndLaterFamily && (cpuid.model() == kSkyLakeModelId || cpuid.model() == kSkyLake_LModelId);
        }();
    ...
    if (expected_frame_checksums_.size() > 0) {
        LOG_IF(FATAL, frame_index >= expected_frame_checksums_.size()) << "Frame number is over than the number of read md5 values in file.";
        const auto& expected_md5 = expected_frame_checksums_[frame_index];
        if (computed_md5 != expected_md5)
            return std::make_unique<MD5MismatchedFrameInfo>(frame_index, computed_md5, expected_md5);
    }
    return nullptr;
}

std::string MD5VideoFrameValidator::ComputeMD5FromVideoFrame(const VideoFrame& video_frame) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    ...
    for (size_t i = 0; i < VideoFrame::NumPlanes(format); ++i) {
        const size_t visible_row_bytes = VideoFrame::RowBytes(i, format, visible_rect.width());
        const int visible_rows = VideoFrame::Rows(i, format, visible_rect.height());
        const size_t stride = video_frame.stride(i);
        for (int row = 0; row < visible_rows; ++row) {
            base::MD5Update(&context, base::span<const uint8_t>(video_frame.data(i) + (stride * row), visible_row_bytes));
        }
    }
...
struct RawVideoFrameValidator::RawMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
...
std::unique_ptr<RawVideoFrameValidator> RawVideoFrameValidator::Create(
    const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, uint8_t tolerance, CropHelper crop_helper)
{
    auto video_frame_validator
        = base::WrapUnique(new RawVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), tolerance, std::move(crop_helper)));
    if (!video_frame_validator->Initialize()) {
...
std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> RawVideoFrameValidator::Validate(scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    ...
    size_t diff_cnt = CompareFramesWithErrorDiff(*frame, *model_frame, tolerance_);
    if (diff_cnt > 0)
        return std::make_unique<RawMismatchedFrameInfo>(frame_index, diff_cnt);
    return nullptr;
}

struct PSNRVideoFrameValidator::PSNRMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
...
// static
std::unique_ptr<PSNRVideoFrameValidator> PSNRVideoFrameValidator::Create(const GetModelFrameCB& get_model_frame_cb,
    std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
{
    auto video_frame_validator = base::WrapUnique(
        new PSNRVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), validation_mode, tolerance, std::move(crop_helper)));
    if (!video_frame_validator->Initialize()) {
...
PSNRVideoFrameValidator::PSNRVideoFrameValidator(const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor,
    ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
    : VideoFrameValidator(std::move(corrupt_frame_processor), std::move(crop_helper))
    , get_model_frame_cb_(get_model_frame_cb)
    , tolerance_(tolerance)
...
std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> PSNRVideoFrameValidator::Validate(scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    auto model_frame = get_model_frame_cb_.Run(frame_index);
    CHECK(model_frame);
...
struct SSIMVideoFrameValidator::SSIMMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
...
// static
std::unique_ptr<SSIMVideoFrameValidator> SSIMVideoFrameValidator::Create(const GetModelFrameCB& get_model_frame_cb,
    std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
{
    auto video_frame_validator = base::WrapUnique(
        new SSIMVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), validation_mode, tolerance, std::move(crop_helper)));
    if (!video_frame_validator->Initialize()) {
...
SSIMVideoFrameValidator::SSIMVideoFrameValidator(const GetModelFrameCB& get_model_frame_cb, std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor,
    ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
    : VideoFrameValidator(std::move(corrupt_frame_processor), std::move(crop_helper))
    , get_model_frame_cb_(get_model_frame_cb)
    , tolerance_(tolerance)
...
std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> SSIMVideoFrameValidator::Validate(scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    auto model_frame = get_model_frame_cb_.Run(frame_index);
    CHECK(model_frame);
...
struct LogLikelihoodRatioVideoFrameValidator::LogLikelihoodRatioMismatchedFrameInfo : public VideoFrameValidator::MismatchedFrameInfo {
    ...
    void Print() const override
    {
        LOG(ERROR) << "frame_index: " << frame_index << ", log likelihood ratio: " << ratio;
    }
...
// static
std::unique_ptr<LogLikelihoodRatioVideoFrameValidator> LogLikelihoodRatioVideoFrameValidator::Create(const GetModelFrameCB& get_model_frame_cb,
    std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
{
    auto video_frame_validator = base::WrapUnique(
        new LogLikelihoodRatioVideoFrameValidator(get_model_frame_cb, std::move(corrupt_frame_processor), validation_mode, tolerance, std::move(crop_helper)));
    if (!video_frame_validator->Initialize()) {
...
LogLikelihoodRatioVideoFrameValidator::LogLikelihoodRatioVideoFrameValidator(const GetModelFrameCB& get_model_frame_cb,
    std::unique_ptr<VideoFrameProcessor> corrupt_frame_processor, ValidationMode validation_mode, double tolerance, CropHelper crop_helper)
    : VideoFrameValidator(std::move(corrupt_frame_processor), std::move(crop_helper))
    , get_model_frame_cb_(get_model_frame_cb)
    , tolerance_(tolerance)
...
LogLikelihoodRatioVideoFrameValidator::~LogLikelihoodRatioVideoFrameValidator() = default;

std::unique_ptr<VideoFrameValidator::MismatchedFrameInfo> LogLikelihoodRatioVideoFrameValidator::Validate(
    scoped_refptr<const VideoFrame> frame, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(validator_thread_sequence_checker_);
    auto model_frame = get_model_frame_cb_.Run(frame_index);
    ...
    DVLOGF(4) << "frame_index: " << frame_index << ", log likelihood ratio: " << ratio;
    log_likelihood_ratios_[frame_index] = ratio;
    if (ratio >= tolerance_) {
        return std::make_unique<LogLikelihoodRatioMismatchedFrameInfo>(frame_index, ratio);
    }
