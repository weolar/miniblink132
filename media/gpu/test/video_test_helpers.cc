...
uint32_t GetReadFrameIndex(uint32_t frame_index, bool reverse, uint32_t num_frames)
{
...
{
    output_file_ = base::File(output_filepath, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    LOG_ASSERT(output_file_.IsValid());
}

bool IvfWriter::WriteFileHeader(VideoCodec codec, const gfx::Size& resolution, uint32_t frame_rate, uint32_t num_frames)
{
...
bool IvfWriter::WriteFrame(uint32_t data_size, uint64_t timestamp, const uint8_t* data)
{
    ...
    if (!output_file_.WriteAtCurrentPosAndCheck(base::as_byte_span(ivf_frame_header))) {
        return false;
    }
...
// static
std::unique_ptr<EncodedDataHelper> EncodedDataHelper::Create(base::span<const uint8_t> stream, VideoCodec codec)
{
    ...
    if (codec == VideoCodec::kVP8 || codec == VideoCodec::kVP9 || codec == VideoCodec::kAV1) {
...
// static
bool EncodedDataHelper::HasConfigInfo(const uint8_t* data, size_t size, VideoCodec codec)
{
    CHECK(codec == media::VideoCodec::kH264 || codec == media::VideoCodec::kHEVC) << "Unsupported codec " << GetCodecName(codec);
    return EncodedDataHelperH26x::HasConfigInfo(data, size, codec);
}

EncodedDataHelper::EncodedDataHelper(base::span<const uint8_t> stream, VideoCodec codec)
    : data_(std::string(reinterpret_cast<const char*>(stream.data()), stream.size()))
    , codec_(codec)
{
}
...
EncodedDataHelperH26x::EncodedDataHelperH26x(base::span<const uint8_t> stream, VideoCodec codec)
    : EncodedDataHelper(std::move(stream), codec)
{
}

// static
bool EncodedDataHelperH26x::HasConfigInfo(const uint8_t* data, size_t size, VideoCodec codec)
{
    ...
    if (codec == media::VideoCodec::kH264) {
        return (size > kNALUReducedHeaderSize && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x1 && (data[kNALUReducedHeaderSize] & 0x1f) == 0x7)
            || (size > kNALUHeaderSize && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x0 && data[3] == 0x1 && (data[kNALUHeaderSize] & 0x1f) == 0x7);
    }
    CHECK_EQ(codec, media::VideoCodec::kHEVC);
    return (size > kNALUReducedHeaderSize && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x1 && (data[kNALUReducedHeaderSize] & 0x7e) == 0x42)
        || (size > kNALUHeaderSize && data[0] == 0x0 && data[1] == 0x0 && data[2] == 0x0 && data[3] == 0x1 && (data[kNALUHeaderSize] & 0x7e) == 0x42);
}
...
scoped_refptr<DecoderBuffer> EncodedDataHelperH26x::GetNextBuffer()
{
    ...
    return DecoderBuffer::CopyFrom(base::as_byte_span(data_).subspan(start_pos, next_nalu_pos - start_pos));
}
...
bool EncodedDataHelperH26x::LookForSPS()
{
    while (next_pos_to_parse_ + kNALUHeaderSize < data_.size()) {
        if (codec_ == VideoCodec::kH264 && (data_[next_pos_to_parse_ + kNALUHeaderSize] & 0x1f) == 0x7) {
            return true;
        } else if (codec_ == VideoCodec::kHEVC && (data_[next_pos_to_parse_ + kNALUHeaderSize] & 0x7e) == 0x42) {
            return true;
        }
...
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
EncodedDataHelperH265::EncodedDataHelperH265(base::span<const uint8_t> stream, VideoCodec codec)
    : EncodedDataHelper(std::move(stream), codec)
    , h265_parser_(std::make_unique<H265Parser>())
{
    h265_parser_->SetStream(reinterpret_cast<uint8_t*>(data_.data()), data_.size());
}
...
scoped_refptr<DecoderBuffer> EncodedDataHelperH265::GetNextBuffer()
{
    ...
    while (true) {
        H265NALU nalu;
        {
            ...
            if (result == H265Parser::kInvalidStream || result == H265Parser::kUnsupportedStream) {
                LOG(ERROR) << __func__ << " Invalid or unsupported bitstream";
                return nullptr;
            }
            CHECK_EQ(result, H265Parser::kOk);
        }
        CHECK_LE(nalu.data, reinterpret_cast<uint8_t*>(data_.data()) + data_.size());
        CHECK_LE(nalu.data + nalu.size, reinterpret_cast<uint8_t*>(data_.data()) + data_.size());

        struct NALUMetadata nalu_metadata;
        nalu_metadata.start_pointer = reinterpret_cast<uint8_t*>(data_.data()) + next_pos_to_parse_;
        ...
        VLOG(2) << "NALU (" << nalu.nal_unit_type << ") found " << nalu_metadata << " next_pos_to_parse_=" << next_pos_to_parse_;
        ...
        case H265NALU::CRA_NUT: {
            ...
            const auto result = h265_parser_->ParseSliceHeader(nalu, current_slice_header.get(), previous_slice_header_.get());
            if (result != H265Parser::kOk) {
                LOG(ERROR) << __func__ << "Error parsing slice header";
                return nullptr;
            }

            is_new_frame_boundary = current_slice_header->first_slice_segment_in_pic_flag;
        ...
        if (previous_nalus_.empty() && is_stand_alone_NALU) {
            ...
            return DecoderBuffer::CopyFrom(base::as_byte_span(data_).subspan(nalu_metadata.start_index, nalu_metadata.size_with_header));
        }
...
scoped_refptr<DecoderBuffer> EncodedDataHelperH265::ReassembleNALUs(const std::vector<struct NALUMetadata>& nalus)
{
    if (nalus.empty()) {
        return nullptr;
    }
    const size_t total_size
        = std::accumulate(nalus.begin(), nalus.end(), 0, [](size_t total, NALUMetadata metadata) { return total + metadata.size_with_header; });
    VLOG(4) << "Reassembling " << nalus.size() << " NALUs, " << total_size << "B";
    return DecoderBuffer::CopyFrom(base::as_byte_span(data_).subspan(nalus.begin()->start_index, total_size));
}
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)

EncodedDataHelperIVF::EncodedDataHelperIVF(base::span<const uint8_t> stream, VideoCodec codec)
    : EncodedDataHelper(std::move(stream), codec)
{
}
...
scoped_refptr<DecoderBuffer> EncodedDataHelperIVF::GetNextBuffer()
{
    ...
    if (next_pos_to_parse_ == 0) {
        ...
        auto ivf_header = GetIvfFileHeader(base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&data_[0]), kIvfFileHeaderSize));
    ...
    if (ivf_frames.size() == 1) {
        return DecoderBuffer::CopyFrom(
            ...
            UNSAFE_TODO(base::span(ivf_frames[0].data.get(), ivf_frames[0].header.frame_size)));
    }
    ...
    for (const IvfFrame& ivf : ivf_frames) {
        data.append(reinterpret_cast<const char*>(ivf.data.get()), ivf.header.frame_size);
...
std::optional<IvfFrameHeader> EncodedDataHelperIVF::GetNextIvfFrameHeader() const
{
    ...
    return GetIvfFrameHeader(base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(&data_[pos]), kIvfFrameHeaderSize));
}
...
AlignedDataHelper::AlignedDataHelper(const RawVideo* video, uint32_t num_read_frames, bool reverse, const gfx::Size& aligned_coded_size,
    const gfx::Size& natural_size, uint32_t frame_rate, VideoFrame::StorageType storage_type)
    : video_(video)
    , num_frames_(video_->NumFrames())
    , num_read_frames_(num_read_frames)
    , reverse_(reverse)
    , create_frame_mode_(num_frames_ > RawVideo::kLimitedReadFrames ? CreateFrameMode::kOnDemand : CreateFrameMode::kAllAtOnce)
    ...
    , elapsed_frame_time_(base::Seconds(/*secs=*/0u))
{
...
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        layout_ = GetPlatformVideoFrameLayout(video_->PixelFormat(), aligned_coded_size, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    } else {
        layout_ = CreateVideoFrameLayout(video_->PixelFormat(), aligned_coded_size, kPlatformBufferAlignment);
    }
    ...
    for (size_t i = 0; i < num_frames_; i++) {
        video_frame_data_[i] = CreateVideoFrameData(storage_type_, video->GetFrame(i), video_->FrameLayout(), *layout_);
    }

    LOG_ASSERT(video_frame_data_.size() == num_frames_) << "Failed to initialize VideoFrames";
}
...
scoped_refptr<VideoFrame> AlignedDataHelper::GetNextFrame()
{
    ...
    const uint32_t read_frame_index = GetReadFrameIndex(frame_index_++, reverse_, num_frames_);
    if (create_frame_mode_ == CreateFrameMode::kOnDemand) {
        auto frame_data = video_->GetFrame(read_frame_index);
        VideoFrameData video_frame_data = CreateVideoFrameData(storage_type_, frame_data, video_->FrameLayout(), *layout_);
        return CreateVideoFrameFromVideoFrameData(video_frame_data, frame_timestamp);
    } else {
        return CreateVideoFrameFromVideoFrameData(video_frame_data_[read_frame_index], frame_timestamp);
    }
}

scoped_refptr<VideoFrame> AlignedDataHelper::CreateVideoFrameFromVideoFrameData(const VideoFrameData& video_frame_data, base::TimeDelta frame_timestamp) const
{
    if (storage_type_ == VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        ...
        std::optional<gfx::BufferFormat> buffer_format = VideoPixelFormatToGfxBufferFormat(layout_->format());
        if (!buffer_format) {
        ...
        auto gpu_memory_buffer = support.CreateGpuMemoryBufferImplFromHandle(
            std::move(dup_handle), layout_->coded_size(), *buffer_format, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE, base::DoNothing());
        ...
        return media::VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect_, natural_size_, std::move(gpu_memory_buffer), frame_timestamp);
    } else {
        ...
        auto frame = media::VideoFrame::WrapExternalYuvDataWithLayout(*layout_, visible_rect_, natural_size_, data[0], data[1], data[2], frame_timestamp);
        DCHECK(frame);
...
AlignedDataHelper::VideoFrameData AlignedDataHelper::CreateVideoFrameData(
    VideoFrame::StorageType storage_type, const RawVideo::FrameData& src_frame, const VideoFrameLayout& src_layout, const VideoFrameLayout& dst_layout)
{
    LOG_ASSERT(gfx::Rect(dst_layout.coded_size()).Contains(gfx::Rect(src_layout.coded_size())))
    ...
    if (storage_type == VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        ...
        auto memory_frame = VideoFrame::CreateFrame(pixel_format, resolution, gfx::Rect(resolution), resolution, base::TimeDelta());
        LOG_ASSERT(!!memory_frame) << "Failed creating VideoFrame";
        for (size_t i = 0; i < src_layout.planes().size(); i++) {
            libyuv::CopyPlane(src_frame.plane_addrs[i], src_frame.strides[i], memory_frame->writable_data(i), memory_frame->stride(i),
                VideoFrame::RowBytes(i, pixel_format, resolution.width()), VideoFrame::Rows(i, pixel_format, resolution.height()));
        }
        // Create GpuMemoryBuffer VideoFrame from the on-memory VideoFrame.
        auto frame
            = CloneVideoFrame(memory_frame.get(), dst_layout, VideoFrame::STORAGE_GPU_MEMORY_BUFFER, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
        LOG_ASSERT(!!frame) << "Failed creating GpuMemoryBuffer VideoFrame";

        auto gmb_handle = CreateGpuMemoryBufferHandle(frame.get());
        LOG_ASSERT(!gmb_handle.is_null()) << "Failed creating GpuMemoryBufferHandle";
        return VideoFrameData(std::move(gmb_handle));
#else
        NOTREACHED();
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    } else {
        const size_t dst_video_frame_size = dst_layout.planes().back().offset + dst_layout.planes().back().size;
        auto mapped_region = base::ReadOnlySharedMemoryRegion::Create(dst_video_frame_size);
        ...
        for (size_t i = 0; i < src_layout.planes().size(); i++) {
            ...
            libyuv::CopyPlane(src_frame.plane_addrs[i], src_frame.strides[i], dst_ptr, dst_plane_layout.stride,
                VideoFrame::RowBytes(i, pixel_format, resolution.width()), VideoFrame::Rows(i, pixel_format, resolution.height()));
        }
...
{
    uint32_t read_frame_index = GetReadFrameIndex(index, reverse_, video_->NumFrames());
    ...
    scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalYuvDataWithLayout(video_->FrameLayout(), video_->VisibleRect(),
        video_->VisibleRect().size(), frame_data[0], frame_data[1], frame_data[2], base::TimeTicks::Now().since_origin());
    video_frame->AddDestructionObserver(base::DoNothingWithBoundArgs(std::move(src_frame)));
    return video_frame;
}
