...
std::unique_ptr<base::test::TaskEnvironment> VideoDecodeAcceleratorTestSuite::task_environment_;

// static
VideoDecodeAcceleratorTestSuite* VideoDecodeAcceleratorTestSuite::Create(int argc, char** argv)
{
    ...
    for (base::CommandLine::SwitchMap::const_iterator it = switches.begin(); it != switches.end(); ++it) {
        if (it->first.find("gtest_") == 0 || // Handled by GoogleTest
            ...
            it->first == "use-gl" || it->first == "v" || it->first == "vmodule" || it->first == "enable-features" || it->first == "disable-features"
            || it->first == "test-launcher-shard-index" || it->first == "test-launcher-summary-output" || it->first == "test-launcher-total-shards"
            || it->first == "enable-primary-node-access-for-vkms-testing" || it->first == "single-process-tests" || it->first == "test-launcher-output"
            || it->first == "test-launcher-retries-left" || it->first == "enable-clear-hevc-for-testing") {
            continue;
        }
        ...
        if (it->first == "validator_type") {
            ...
            if (validator_type_str == "none") {
                ...
                std::cout << "unknown validator type \"" << validator_type_str << "\", possible values are \"none|md5|ssim\"\n";
                return nullptr;
            }
            ...
            if (output_frames_str == "all") {
                ...
                std::cout << "unknown frame output mode \"" << output_frames_str << "\", possible values are \"all|corrupt\"\n";
                return nullptr;
            }
    ...
    return new VideoDecodeAcceleratorTestSuite(argc, argv, video_path, video_metadata_path, validator_type, implementation, linear_output,
        base::FilePath(output_folder), frame_output_config, enabled_features, disabled_features);
}

VideoDecodeAcceleratorTestSuite::VideoDecodeAcceleratorTestSuite(int argc, char** argv, const base::FilePath& video_path,
    const base::FilePath& video_metadata_path, VideoPlayerTestEnvironment::ValidatorType validator_type, const DecoderImplementation implementation,
    bool linear_output, const base::FilePath& output_folder, const FrameOutputConfig& frame_output_config,
    const std::vector<base::test::FeatureRef>& enabled_features, const std::vector<base::test::FeatureRef>& disabled_features)
    : base::TestSuite(argc, argv)
    , video_test_env_(std::move(media::test::VideoPlayerTestEnvironment::Create(video_path, video_metadata_path, validator_type, implementation, linear_output,
          base::FilePath(output_folder), frame_output_config, enabled_features, disabled_features, false)))
{
}

VideoDecodeAcceleratorTestSuite::~VideoDecodeAcceleratorTestSuite() = default;

const media::test::VideoBitstream* VideoDecodeAcceleratorTestSuite::Video() const
{
...
VideoPlayerTestEnvironment::ValidatorType VideoDecodeAcceleratorTestSuite::GetValidatorType() const
{
    return video_test_env_->GetValidatorType();
}

DecoderImplementation VideoDecodeAcceleratorTestSuite::GetDecoderImplementation() const
{
...
VideoFrameFileWriter::OutputFormat VideoDecodeAcceleratorTestSuite::GetFrameOutputFormat() const
{
