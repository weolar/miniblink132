...
public:
    struct BitstreamRef : public base::RefCountedThreadSafe<BitstreamRef> {
        static scoped_refptr<BitstreamRef> Create(scoped_refptr<DecoderBuffer> buffer, const BitstreamBufferMetadata& metadata, int32_t id,
            base::TimeTicks source_timestamp, base::OnceClosure release_cb);
        BitstreamRef() = delete;
    ...
    private:
        ...
        BitstreamRef(scoped_refptr<DecoderBuffer> buffer, const BitstreamBufferMetadata& metadata, int32_t id, base::TimeTicks source_timestamp,
    ...
    virtual void ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index) = 0;
