...
template <typename StateEnum> class ClientStateNotification {
public:
...
template <typename StateEnum> ClientStateNotification<StateEnum>::~ClientStateNotification()
{
}

template <typename StateEnum> void ClientStateNotification<StateEnum>::Notify(StateEnum state)
{
...
template <typename StateEnum> StateEnum ClientStateNotification<StateEnum>::Wait()
{
    base::AutoLock auto_lock(lock_);
...
public:
    ...
    bool WriteFileHeader(VideoCodec codec, const gfx::Size& resolution, uint32_t frame_rate, uint32_t num_frames);
...
public:
    static std::unique_ptr<EncodedDataHelper> Create(base::span<const uint8_t> stream, VideoCodec codec);
...
private:
    ...
    struct NALUMetadata {
        ...
        {
            return os << "start_index=" << m.start_index << ", header_size=" << m.header_size << ", size_with_header=" << m.size_with_header;
        }
    };

    scoped_refptr<DecoderBuffer> ReassembleNALUs(const std::vector<struct NALUMetadata>& nalus);
...
public:
    AlignedDataHelper(const RawVideo* video, uint32_t num_read_frames, bool reverse, const gfx::Size& aligned_coded_size, const gfx::Size& natural_size,
        uint32_t frame_rate, VideoFrame::StorageType storage_type);
    ~AlignedDataHelper();
...
private:
    ...
    scoped_refptr<VideoFrame> CreateVideoFrameFromVideoFrameData(const VideoFrameData& video_frame_data, base::TimeDelta frame_timestamp) const;
