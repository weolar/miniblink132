...
public:
    ...
    static std::unique_ptr<DecoderListener> Create(const DecoderWrapperConfig& config, std::unique_ptr<FrameRendererDummy> frame_renderer,
...
private:
    DecoderListener(const DecoderWrapperConfig& config, std::unique_ptr<FrameRendererDummy> frame_renderer,
        std::vector<std::unique_ptr<VideoFrameProcessor>> frame_processors);
    ...
    size_t video_player_event_counts_[static_cast<size_t>(Event::kNumEvents)] GUARDED_BY(event_lock_) {};
