...
public:
    ...
    static std::unique_ptr<FrameRendererDummy> Create(
        base::TimeDelta frame_duration = base::TimeDelta(), base::TimeDelta vsync_interval_duration = base::TimeDelta());
    ...
    scoped_refptr<VideoFrame> CreateVideoFrame(VideoPixelFormat pixel_format, const gfx::Size& size, uint32_t texture_target, uint32_t* texture_id);

    // Get the number of frames dropped due to the decoder running behind.
    uint64_t FramesDropped() const;

private:
    FrameRendererDummy(base::TimeDelta frame_duration, base::TimeDelta vsync_interval_duration);
    ...
    base::queue<scoped_refptr<VideoFrame>> pending_frames_ GUARDED_BY(renderer_lock_);
