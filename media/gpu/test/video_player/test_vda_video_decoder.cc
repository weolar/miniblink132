...
TestVDAVideoDecoder::TestVDAVideoDecoder(bool use_vd_vda, OnProvidePictureBuffersCB on_provide_picture_buffers_cb, const gfx::ColorSpace& target_color_space,
    FrameRendererDummy* const frame_renderer, bool linear_output)
...
void TestVDAVideoDecoder::Initialize(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (use_vd_vda_) {
        ...
        decoder_ = media::VdVideoDecodeAccelerator::Create(
            base::BindRepeating(&media::VideoDecoderPipeline::CreateForARC), this, vda_config, base::SequencedTaskRunner::GetCurrentDefault());
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    } else {
        DVLOGF(2) << "Use original VDA";
        decoder_ = GpuVideoDecodeAcceleratorFactory::CreateVDA(this, vda_config, gpu::GpuPreferences());
    }
...
void TestVDAVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(vda_wrapper_sequence_checker_);
...
void TestVDAVideoDecoder::ProvidePictureBuffersWithVisibleRect(
    uint32_t requested_num_of_buffers, VideoPixelFormat format, const gfx::Size& dimensions, const gfx::Rect& visible_rect)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(vda_wrapper_sequence_checker_);
    DVLOGF(4) << "Requested " << requested_num_of_buffers << " picture buffers with size " << dimensions.width() << "x" << dimensions.height();
    ...
    for (const PictureBuffer& picture_buffer : picture_buffers) {
        scoped_refptr<VideoFrame> video_frame = CreateGpuMemoryBufferVideoFrame(format, dimensions, visible_rect, visible_rect.size(), base::TimeDelta(),
            linear_output_ ? gfx::BufferUsage::SCANOUT_CPU_READ_WRITE : gfx::BufferUsage::SCANOUT_VDA_WRITE);

        ASSERT_TRUE(video_frame) << "Failed to create video frame";
        video_frames_.emplace(picture_buffer.id(), video_frame);

        gfx::GpuMemoryBufferHandle handle = CreateGpuMemoryBufferHandle(video_frame.get());

        ASSERT_TRUE(!handle.is_null()) << "Failed to create GPU memory handle";
        decoder_->ImportBufferForPicture(picture_buffer.id(), format, std::move(handle));
    }
#else
    FAIL();
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
...
void TestVDAVideoDecoder::PictureReady(const Picture& picture)
{
    ...
    auto timestamp_it = decode_start_timestamps_.Peek(picture.bitstream_buffer_id());
    ...
    if (!picture.visible_rect().IsEmpty()) {
        wrapped_video_frame = VideoFrame::WrapVideoFrame(video_frame, video_frame->format(), picture.visible_rect(), picture.visible_rect().size());
    } else {
        ...
        wrapped_video_frame = VideoFrame::CreateFrame(PIXEL_FORMAT_UNKNOWN, gfx::Size(), gfx::Rect(), gfx::Size(), video_frame->timestamp());
    }
    ...
    base::OnceClosure reuse_cb
        = base::BindOnce(&TestVDAVideoDecoder::ReusePictureBufferThunk, weak_this_, vda_wrapper_task_runner_, picture.picture_buffer_id());
...
void TestVDAVideoDecoder::ReusePictureBufferThunk(
    std::optional<base::WeakPtr<TestVDAVideoDecoder>> vda_video_decoder, scoped_refptr<base::SequencedTaskRunner> task_runner, int32_t picture_buffer_id)
{
    DCHECK(vda_video_decoder);
    task_runner->PostTask(FROM_HERE, base::BindOnce(&TestVDAVideoDecoder::ReusePictureBufferTask, *vda_video_decoder, picture_buffer_id));
}
...
void TestVDAVideoDecoder::NotifyEndOfBitstreamBuffer(int32_t bitstream_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(vda_wrapper_sequence_checker_);

    auto it = decode_cbs_.find(bitstream_buffer_id);
    ASSERT_TRUE(it != decode_cbs_.end()) << "Couldn't find decode callback for picture buffer with id " << bitstream_buffer_id;
