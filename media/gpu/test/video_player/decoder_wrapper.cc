...
void CallbackThunk(std::optional<base::WeakPtr<DecoderWrapper>> decoder_client, scoped_refptr<base::SequencedTaskRunner> task_runner, F f, Args... args)
{
...
DecoderWrapper::DecoderWrapper(const DecoderListener::EventCallback& event_cb, std::unique_ptr<FrameRendererDummy> renderer,
    std::vector<std::unique_ptr<VideoFrameProcessor>> frame_processors, const DecoderWrapperConfig& config)
    : event_cb_(event_cb)
    ...
    , worker_task_runner_(base::ThreadPool::CreateSingleThreadTaskRunner(
          { base::TaskPriority::USER_BLOCKING, base::MayBlock(), base::WithBaseSyncPrimitives() }, base::SingleThreadTaskRunnerThreadMode::DEDICATED))
...
DecoderWrapper::~DecoderWrapper()
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::DestroyDecoderTask, weak_this_, &done));
    done.Wait();
...
// static
std::unique_ptr<DecoderWrapper> DecoderWrapper::Create(const DecoderListener::EventCallback& event_cb, std::unique_ptr<FrameRendererDummy> frame_renderer,
    std::vector<std::unique_ptr<VideoFrameProcessor>> frame_processors, const DecoderWrapperConfig& config)
{
    auto wrapper = base::WrapUnique(new DecoderWrapper(event_cb, std::move(frame_renderer), std::move(frame_processors), config));
...
void DecoderWrapper::CreateDecoder()
{
    ...
    base::WaitableEvent done;
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::CreateDecoderTask, weak_this_, &done));
    done.Wait();
}
...
void DecoderWrapper::Initialize(const VideoBitstream* video)
{
    ...
    base::WaitableEvent done;
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::InitializeTask, weak_this_, video, &done));
    done.Wait();
}
...
void DecoderWrapper::Play()
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::PlayTask, weak_this_));
}
...
void DecoderWrapper::Flush()
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::FlushTask, weak_this_));
}
...
void DecoderWrapper::Reset()
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DecoderWrapper::ResetTask, weak_this_));
}
...
void DecoderWrapper::CreateDecoderTask(base::WaitableEvent* done)
{
...
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
        decoder_ = VideoDecoderPipeline::CreateForTesting(base::SingleThreadTaskRunner::GetCurrentDefault(), std::make_unique<NullMediaLog>(),
    ...
    case DecoderImplementation::kVDVDA:
        ...
        // decoders.
        decoder_ = std::make_unique<TestVDAVideoDecoder>(decoder_wrapper_config_.implementation == DecoderImplementation::kVDVDA,
            // base::Unretained(this) is safe because |decoder_| is owned by
            // |*this|. The lifetime of |decoder_| must be shorter than |*this|.
            base::BindRepeating(&DecoderWrapper::OnResolutionChangedTask, base::Unretained(this)), gfx::ColorSpace(), frame_renderer_.get(),
...
void DecoderWrapper::InitializeTask(const VideoBitstream* video, base::WaitableEvent* done)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(worker_sequence_checker_);
    DCHECK(state_ == DecoderWrapperState::kUninitialized || state_ == DecoderWrapperState::kIdle);
    ASSERT_TRUE(decoder_) << "Can't initialize decoder: not created yet";
    ASSERT_TRUE(video);

    encoded_data_helper_ = EncodedDataHelper::Create(video->Data(), video->Codec());

    // (Re-)initialize the decoder.
    VideoDecoderConfig config(video->Codec(), video->Profile(), VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
        video->Resolution(), gfx::Rect(video->Resolution()), video->Resolution(), std::vector<uint8_t>(0), EncryptionScheme());
    input_video_codec_ = video->Codec();
    input_video_profile_ = video->Profile();

    VideoDecoder::InitCB init_cb = base::BindOnce(CallbackThunk<decltype(&DecoderWrapper::OnDecoderInitializedTask), DecoderStatus>, weak_this_,
        worker_task_runner_, &DecoderWrapper::OnDecoderInitializedTask);
    VideoDecoder::OutputCB output_cb = base::BindRepeating(CallbackThunk<decltype(&DecoderWrapper::OnFrameReadyTask), scoped_refptr<VideoFrame>>, weak_this_,
        worker_task_runner_, &DecoderWrapper::OnFrameReadyTask);

    decoder_->Initialize(config, false, nullptr, std::move(init_cb), output_cb, WaitingCB());

    DCHECK_LE(decoder_wrapper_config_.max_outstanding_decode_requests, static_cast<size_t>(decoder_->GetMaxDecodeRequests()));
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(worker_sequence_checker_);
    LOG_IF(WARNING, 0u != num_outstanding_decode_requests_) << "There is/are " << num_outstanding_decode_requests_
...
void DecoderWrapper::PlayTask()
{
    ...
    for (size_t i = 0; i < decoder_wrapper_config_.max_outstanding_decode_requests; ++i) {
...
void DecoderWrapper::DecodeNextFragmentTask()
{
    ...
    scoped_refptr<DecoderBuffer> bitstream_buffer = encoded_data_helper_->GetNextBuffer();
    if (!bitstream_buffer) {
    ...
    if (input_video_codec_ == media::VideoCodec::kH264 || input_video_codec_ == media::VideoCodec::kHEVC) {
        has_config_info = media::test::EncodedDataHelper::HasConfigInfo(bitstream_buffer->data(), bitstream_buffer->size(), input_video_codec_);
    }

    VideoDecoder::DecodeCB decode_cb = base::BindOnce(
        CallbackThunk<decltype(&DecoderWrapper::OnDecodeDoneTask), DecoderStatus>, weak_this_, worker_task_runner_, &DecoderWrapper::OnDecodeDoneTask);
...
void DecoderWrapper::FlushTask()
{
    ...
    VideoDecoder::DecodeCB flush_done_cb = base::BindOnce(
        CallbackThunk<decltype(&DecoderWrapper::OnFlushDoneTask), DecoderStatus>, weak_this_, worker_task_runner_, &DecoderWrapper::OnFlushDoneTask);
...
void DecoderWrapper::ResetTask()
{
    ...
    decoder_->Reset(
        base::BindOnce(CallbackThunk<decltype(&DecoderWrapper::OnResetDoneTask)>, weak_this_, worker_task_runner_, &DecoderWrapper::OnResetDoneTask));
...
void DecoderWrapper::OnDecodeDoneTask(DecoderStatus status)
{
    ...
    ASSERT_TRUE(status != DecoderStatus::Codes::kAborted || state_ == DecoderWrapperState::kResetting);
    DVLOGF(4);
    ...
    worker_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&DecoderWrapper::DecodeNextFragmentTask, weak_this_), delay);
