...
std::unique_ptr<DecoderListener> DecoderListener::Create(
    const DecoderWrapperConfig& config, std::unique_ptr<FrameRendererDummy> frame_renderer, std::vector<std::unique_ptr<VideoFrameProcessor>> frame_processors)
{
    return base::WrapUnique(new DecoderListener(config, std::move(frame_renderer), std::move(frame_processors)));
}
    ...
    while (true) {
        while (!video_player_events_.empty()) {
            ...
            if (received_event == DecoderListener::Event::kFailure) {
                LOG(ERROR) << "Failed waiting for '" << EventName(sought_event) << "' event.";
                return false;
            }
        }
        ...
        if (time_waiting >= event_timeout_) {
            LOG(ERROR) << "Timeout while waiting for '" << EventName(sought_event) << "' event";
            return false;
        }
...
DecoderListener::DecoderListener(
    ...
    : decoder_wrapper_(DecoderWrapper::Create(
        ...
        // owned.
        base::BindRepeating(&DecoderListener::NotifyEvent, base::Unretained(this)), std::move(frame_renderer), std::move(frame_processors), config))
