...
public:
    ...
    static std::unique_ptr<DecoderWrapper> Create(const DecoderListener::EventCallback& event_cb, std::unique_ptr<FrameRendererDummy> frame_renderer,
...
private:
    ...
    DecoderWrapper(const DecoderListener::EventCallback& event_cb, std::unique_ptr<FrameRendererDummy> renderer,
        std::vector<std::unique_ptr<VideoFrameProcessor>> frame_processors, const DecoderWrapperConfig& config);
    ...
    std::unique_ptr<media::VideoDecoder> decoder_ GUARDED_BY_CONTEXT(worker_sequence_checker_);
