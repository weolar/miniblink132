...
class TestVDAVideoDecoder : public media::VideoDecoder, public VideoDecodeAccelerator::Client {
public:
    using OnProvidePictureBuffersCB = base::RepeatingCallback<bool(void)>;
    // Constructor for the TestVDAVideoDecoder.
    TestVDAVideoDecoder(bool use_vd_vda, OnProvidePictureBuffersCB on_provide_picture_buffers_cb, const gfx::ColorSpace& target_color_space,
        FrameRendererDummy* const frame_renderer, bool linear_output = false);
...
private:
    ...
    void ProvidePictureBuffersWithVisibleRect(
        uint32_t requested_num_of_buffers, VideoPixelFormat format, const gfx::Size& dimensions, const gfx::Rect& visible_rect) override;
    ...
    static void ReusePictureBufferThunk(
        std::optional<base::WeakPtr<TestVDAVideoDecoder>> decoder_client, scoped_refptr<base::SequencedTaskRunner> task_runner, int32_t picture_buffer_id);
