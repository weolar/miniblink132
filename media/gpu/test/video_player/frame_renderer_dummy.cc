...
FrameRendererDummy::FrameRendererDummy(base::TimeDelta frame_duration, base::TimeDelta vsync_interval_duration)
...
// static
std::unique_ptr<FrameRendererDummy> FrameRendererDummy::Create(base::TimeDelta frame_duration, base::TimeDelta vsync_interval_duration)
{
    auto frame_renderer = base::WrapUnique(new FrameRendererDummy(frame_duration, vsync_interval_duration));
...
bool FrameRendererDummy::Initialize()
{
    ...
    renderer_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&FrameRendererDummy::InitializeTask, base::Unretained(this), &done));
    done.Wait();
...
{
    base::WaitableEvent done;
    renderer_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&FrameRendererDummy::DestroyTask, base::Unretained(this), &done));
    done.Wait();
...
void FrameRendererDummy::RenderFrame(scoped_refptr<VideoFrame> video_frame)
{
    ...
    if (next_frame_time_.is_null()) {
        ...
        renderer_thread_.task_runner()->PostTask(FROM_HERE, render_task_.callback());
    }
}
...
scoped_refptr<VideoFrame> FrameRendererDummy::CreateVideoFrame(
    VideoPixelFormat pixel_format, const gfx::Size& size, uint32_t texture_target, uint32_t* texture_id)
{
    *texture_id = 0;
    ...
    std::optional<VideoFrameLayout> layout = CreateVideoFrameLayout(pixel_format, size);
    DCHECK(layout);
    return VideoFrame::CreateFrameWithLayout(*layout, gfx::Rect(size), size, base::TimeDelta(),
...
void FrameRendererDummy::InitializeTask(base::WaitableEvent* done)
{
    ...
    render_task_.Reset(base::BindRepeating(&FrameRendererDummy::RenderFrameTask, base::Unretained(this)));
    done->Signal();
}
...
void FrameRendererDummy::ScheduleNextRenderFrameTask()
{
    ...
    if (vsync_interval_duration_.is_zero()) {
        ...
        next_render_time = std::max(now + vsync_interval_duration_, next_frame_time_);
        next_render_time -= (next_render_time - vsync_timebase_) % vsync_interval_duration_;
    }
    ...
    renderer_thread_.task_runner()->PostDelayedTask(FROM_HERE, render_task_.callback(), (next_render_time - now));
}
