...
constexpr base::FilePath::CharType kDefaultTestVideoPath[] = FILE_PATH_LITERAL("bear_320x192_40frames.yuv.webm");
...
constexpr const base::FilePath::CharType* kDefaultOutputFolder = FILE_PATH_LITERAL("perf_metrics");
...
public:
    ...
    void ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index) override;
...
void PerformanceEvaluator::ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    base::TimeTicks now = base::TimeTicks::Now();

    base::TimeDelta delivery_time = (now - prev_bitstream_delivery_time_);
    perf_metrics_.bitstream_delivery_times_.push_back(delivery_time.InMillisecondsF());
    ...
    base::TimeDelta encode_time = base::TimeTicks::Now() - bitstream->source_timestamp;
...
{
    DCHECK_EQ(perf_metrics_.bitstream_delivery_times_.size(), perf_metrics_.bitstream_encode_times_.size());

    end_time_ = base::TimeTicks::Now();
    perf_metrics_.total_duration_ = end_time_ - start_time_;
    perf_metrics_.bitstreams_encoded_ = perf_metrics_.bitstream_encode_times_.size();
    perf_metrics_.bitstreams_per_second_ = perf_metrics_.bitstreams_encoded_ / perf_metrics_.total_duration_.InSecondsF();

    // Calculate delivery and encode time metrics.
    perf_metrics_.bitstream_delivery_stats_ = PerformanceTimeStats(perf_metrics_.bitstream_delivery_times_);
    perf_metrics_.bitstream_encode_stats_ = PerformanceTimeStats(perf_metrics_.bitstream_encode_times_);
}

void PerformanceMetrics::WriteToConsole() const
{
    std::cout << "Bitstreams encoded:     " << bitstreams_encoded_ << std::endl;
    std::cout << "Total duration:         " << total_duration_.InMillisecondsF() << "ms" << std::endl;
    std::cout << "FPS:                    " << bitstreams_per_second_ << std::endl;
    std::cout << "Bitstream delivery time - average:       " << bitstream_delivery_stats_.avg_ms_ << "ms" << std::endl;
    std::cout << "Bitstream delivery time - percentile 25: " << bitstream_delivery_stats_.percentile_25_ms_ << "ms" << std::endl;
    std::cout << "Bitstream delivery time - percentile 50: " << bitstream_delivery_stats_.percentile_50_ms_ << "ms" << std::endl;
    std::cout << "Bitstream delivery time - percentile 75: " << bitstream_delivery_stats_.percentile_75_ms_ << "ms" << std::endl;
    std::cout << "Bitstream encode time - average:       " << bitstream_encode_stats_.avg_ms_ << "ms" << std::endl;
    std::cout << "Bitstream encode time - percentile 25: " << bitstream_encode_stats_.percentile_25_ms_ << "ms" << std::endl;
    std::cout << "Bitstream encode time - percentile 50: " << bitstream_encode_stats_.percentile_50_ms_ << "ms" << std::endl;
    std::cout << "Bitstream encode time - percentile 75: " << bitstream_encode_stats_.percentile_75_ms_ << "ms" << std::endl;
}
...
void PerformanceMetrics::WriteToFile() const
{
    ...
    metrics.Set("BitstreamsEncoded", base::checked_cast<int>(bitstreams_encoded_));
    metrics.Set("TotalDurationMs", total_duration_.InMillisecondsF());
    metrics.Set("FPS", bitstreams_per_second_);
    metrics.Set("BitstreamDeliveryTimeAverage", bitstream_delivery_stats_.avg_ms_);
    metrics.Set("BitstreamDeliveryTimePercentile25", bitstream_delivery_stats_.percentile_25_ms_);
    metrics.Set("BitstreamDeliveryTimePercentile50", bitstream_delivery_stats_.percentile_50_ms_);
    metrics.Set("BitstreamDeliveryTimePercentile75", bitstream_delivery_stats_.percentile_75_ms_);
    metrics.Set("BitstreamEncodeTimeAverage", bitstream_encode_stats_.avg_ms_);
    metrics.Set("BitstreamEncodeTimePercentile25", bitstream_encode_stats_.percentile_25_ms_);
    metrics.Set("BitstreamEncodeTimePercentile50", bitstream_encode_stats_.percentile_50_ms_);
    metrics.Set("BitstreamEncodeTimePercentile75", bitstream_encode_stats_.percentile_75_ms_);
    ...
    ASSERT_TRUE(base::JSONWriter::WriteWithOptions(metrics, base::JSONWriter::OPTIONS_PRETTY_PRINT, &metrics_str));
    base::FilePath metrics_file_path = output_folder_path.Append(g_env->GetTestOutputFilePath().AddExtension(FILE_PATH_LITERAL(".json")));
    ...
    base::File metrics_output_file(base::FilePath(metrics_file_path), base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    int bytes_written = metrics_output_file.WriteAtCurrentPos(metrics_str.data(), metrics_str.length());
...
struct BitstreamQualityMetrics {
    BitstreamQualityMetrics(const PSNRVideoFrameValidator* const psnr_validator, const SSIMVideoFrameValidator* const ssim_validator,
        const PSNRVideoFrameValidator* const bottom_row_psnr_validator, const LogLikelihoodRatioVideoFrameValidator* const log_likelihood_validator,
        const DecoderBufferValidator* const decoder_buffer_validator, const std::optional<size_t>& spatial_idx, const std::optional<size_t>& temporal_idx,
        size_t num_spatial_layers, SVCInterLayerPredMode inter_layer_pred_mode);
...
private:
    ...
    static QualityStats ComputeQualityStats(const std::map<size_t, double>& values);
    static QualityStats ComputeQualityStats(const std::vector<int>& values);
    void WriteToConsole(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats,
        const BitstreamQualityMetrics::QualityStats& ssim_stats, const BitstreamQualityMetrics::QualityStats& bottom_row_psnr_stats,
        const BitstreamQualityMetrics::QualityStats& log_likelihood_stats, const BitstreamQualityMetrics::QualityStats& qp_stats, uint32_t target_bitrate,
        uint32_t actual_bitrate) const;
    void WriteToFile(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats,
        const BitstreamQualityMetrics::QualityStats& ssim_stats, const BitstreamQualityMetrics::QualityStats& bottom_row_psnr_stats,
        const BitstreamQualityMetrics::QualityStats& log_likelihood_stats, const BitstreamQualityMetrics::QualityStats& qp_stats, uint32_t target_bitrate,
        uint32_t actual_bitrate) const;
    ...
    const raw_ptr<const LogLikelihoodRatioVideoFrameValidator> log_likelihood_validator;
    const raw_ptr<const DecoderBufferValidator> decoder_buffer_validator;
};

BitstreamQualityMetrics::BitstreamQualityMetrics(const PSNRVideoFrameValidator* const psnr_validator, const SSIMVideoFrameValidator* const ssim_validator,
    const PSNRVideoFrameValidator* const bottom_row_psnr_validator, const LogLikelihoodRatioVideoFrameValidator* const log_likelihood_validator,
    const DecoderBufferValidator* const decoder_buffer_validator, const std::optional<size_t>& spatial_idx, const std::optional<size_t>& temporal_idx,
    size_t num_spatial_layers, SVCInterLayerPredMode inter_layer_pred_mode)
...
// static
BitstreamQualityMetrics::QualityStats BitstreamQualityMetrics::ComputeQualityStats(const std::map<size_t, double>& values)
{
    if (values.empty())
        return QualityStats();
    ...
    QualityStats stats;
    stats.avg = std::accumulate(sorted_values.begin(), sorted_values.end(), 0.0) / sorted_values.size();
...
// static
BitstreamQualityMetrics::QualityStats BitstreamQualityMetrics::ComputeQualityStats(const std::vector<int>& values)
{
    if (values.empty()) {
        return QualityStats();
    }
...
void BitstreamQualityMetrics::Output(uint32_t target_bitrate, uint32_t actual_bitrate)
{
    std::string svc_text;
    if (spatial_idx) {
        svc_text += (inter_layer_pred_mode == SVCInterLayerPredMode::kOff && num_spatial_layers_ > 1 ? "S" : "L") + base::NumberToString(*spatial_idx + 1);
    }
    if (temporal_idx)
    ...
    auto bottom_row_psnr_stats = ComputeQualityStats(bottom_row_psnr_validator->GetPSNRValues());
    auto log_likelihood_stats = ComputeQualityStats(log_likelihood_validator->get_log_likelihood_ratio_values());
    auto qp_stats = ComputeQualityStats(decoder_buffer_validator->GetQPValues(spatial_idx.value_or(0), temporal_idx.value_or(0)));
    WriteToConsole(svc_text, psnr_stats, ssim_stats, bottom_row_psnr_stats, log_likelihood_stats, qp_stats, target_bitrate, actual_bitrate);
    WriteToFile(svc_text, psnr_stats, ssim_stats, bottom_row_psnr_stats, log_likelihood_stats, qp_stats, target_bitrate, actual_bitrate);
}

void BitstreamQualityMetrics::WriteToConsole(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats,
    const BitstreamQualityMetrics::QualityStats& ssim_stats, const BitstreamQualityMetrics::QualityStats& bottom_row_psnr_stats,
    const BitstreamQualityMetrics::QualityStats& log_likelihood_stats, const BitstreamQualityMetrics::QualityStats& qp_stats, uint32_t target_bitrate,
    uint32_t actual_bitrate) const
{
    ...
    std::cout << "Bitrate: " << actual_bitrate << " (target:  " << target_bitrate << ")" << std::endl;
    std::cout << "Bitrate deviation: " << std::fixed << std::setprecision(2) << (actual_bitrate * 100.0 / target_bitrate) - 100.0 << " %" << std::endl;

    std::cout << std::fixed << std::setprecision(4);
    std::cout << "SSIM - average:       " << ssim_stats.avg << std::endl;
    std::cout << "SSIM - percentile 25: " << ssim_stats.percentile_25 << std::endl;
    std::cout << "SSIM - percentile 50: " << ssim_stats.percentile_50 << std::endl;
    std::cout << "SSIM - percentile 75: " << ssim_stats.percentile_75 << std::endl;
    std::cout << "PSNR - average:       " << psnr_stats.avg << std::endl;
    std::cout << "PSNR - percentile 25: " << psnr_stats.percentile_25 << std::endl;
    std::cout << "PSNR - percentile 50: " << psnr_stats.percentile_50 << std::endl;
    std::cout << "PSNR - percentile 75: " << psnr_stats.percentile_75 << std::endl;
    std::cout << "Bottom row PSNR - average:       " << bottom_row_psnr_stats.avg << std::endl;
    std::cout << "Bottom row PSNR - percentile 25: " << bottom_row_psnr_stats.percentile_25 << std::endl;
    std::cout << "Bottom row PSNR - percentile 50: " << bottom_row_psnr_stats.percentile_50 << std::endl;
    std::cout << "Bottom row PSNR - percentile 75: " << bottom_row_psnr_stats.percentile_75 << std::endl;
    std::cout << "Log likelihood ratio - average:       " << log_likelihood_stats.avg << std::endl;
    std::cout << "Log likelihood ratio - percentile 25: " << log_likelihood_stats.percentile_25 << std::endl;
    std::cout << "Log likelihood ratio - percentile 50: " << log_likelihood_stats.percentile_50 << std::endl;
    std::cout << "Log likelihood ratio - percentile 75: " << log_likelihood_stats.percentile_75 << std::endl;
...
void BitstreamQualityMetrics::WriteToFile(const std::string& svc_text, const BitstreamQualityMetrics::QualityStats& psnr_stats,
    const BitstreamQualityMetrics::QualityStats& ssim_stats, const BitstreamQualityMetrics::QualityStats& bottom_row_psnr_stats,
    const BitstreamQualityMetrics::QualityStats& log_likelihood_stats, const BitstreamQualityMetrics::QualityStats& qp_stats, uint32_t target_bitrate,
    uint32_t actual_bitrate) const
{
    ...
    metrics.Set("BitrateDeviation", (actual_bitrate * 100.0 / target_bitrate) - 100.0);
    ...
    std::string metrics_str;
    ASSERT_TRUE(base::JSONWriter::WriteWithOptions(metrics, base::JSONWriter::OPTIONS_PRETTY_PRINT, &metrics_str));
    constexpr const base::FilePath::CharType* kMetrixFileSuffix = FILE_PATH_LITERAL(".json");
    const std::string svc_text_ext = svc_text.empty() ? "" : "." + svc_text;
    base::FilePath metrics_file_path
        = output_folder_path.Append(g_env->GetTestOutputFilePath().AddExtensionASCII(svc_text_ext).AddExtension(kMetrixFileSuffix));
    // Make sure that the directory into which json is saved is created.
    LOG_ASSERT(base::CreateDirectory(metrics_file_path.DirName()));
    base::File metrics_output_file(base::FilePath(metrics_file_path), base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
    int bytes_written = metrics_output_file.WriteAtCurrentPos(metrics_str.data(), metrics_str.length());
...
public:
    ...
    std::unique_ptr<VideoEncoder> CreateVideoEncoder(
        std::optional<uint32_t> encode_rate = std::nullopt, bool measure_quality = false, size_t num_encode_frames = kNumEncodeFramesForSpeedPerformance)
    {
        ...
        const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers = g_env->SpatialLayers();
        SVCInterLayerPredMode inter_layer_pred_mode = g_env->InterLayerPredMode();
        std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors;
        if (measure_quality) {
            bitstream_processors = CreateBitstreamProcessorsForQualityPerformance(video, profile, num_encode_frames, spatial_layers, inter_layer_pred_mode);
        } else {
        ...
        LOG_ASSERT(!bitstream_processors.empty()) << "Failed to create bitstream processors";

        VideoEncoderClientConfig config(video, profile, spatial_layers, g_env->InterLayerPredMode(), g_env->ContentType(), bitrate, g_env->Reverse());
#if BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_LINUX)
        config.input_storage_type = VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer;
#else
        // TODO(https://crbugs.com/350994517, b/211783271): Enable GMB for
        // Windows/Linux.
        config.input_storage_type = VideoEncodeAccelerator::Config::StorageType::kShmem;
#endif // BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_LINUX)
        ...
        auto video_encoder = VideoEncoder::Create(config, std::move(bitstream_processors));
...
private:
    std::unique_ptr<BitstreamValidator> CreateBitstreamValidator(const VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t num_encode_frames,
        const std::optional<size_t>& spatial_layer_index_to_decode, const std::optional<size_t>& temporal_layer_index_to_decode,
        const std::vector<gfx::Size>& spatial_layer_resolutions, DecoderBufferValidator* const decoder_buffer_validator,
        SVCInterLayerPredMode inter_layer_pred_mode)
    {
        std::vector<std::unique_ptr<VideoFrameProcessor>> video_frame_processors;
        VideoFrameValidator::GetModelFrameCB get_model_frame_cb = base::BindRepeating(&VideoEncoderTest::GetModelFrame, base::Unretained(this), visible_rect);
        auto ssim_validator
            = SSIMVideoFrameValidator::Create(get_model_frame_cb, /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
                /*tolerance=*/0.0);
        LOG_ASSERT(ssim_validator);
        auto psnr_validator
            = PSNRVideoFrameValidator::Create(get_model_frame_cb, /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
                /*tolerance=*/0.0);
        LOG_ASSERT(psnr_validator);
        auto bottom_row_psnr_validator = PSNRVideoFrameValidator::Create(get_model_frame_cb,
            /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
            /*tolerance=*/0.0, base::BindRepeating(&BottomRowCrop, kDefaultBottomRowCropHeight));
        LOG_ASSERT(bottom_row_psnr_validator);
        auto log_likelihood_validator = LogLikelihoodRatioVideoFrameValidator::Create(get_model_frame_cb,
            /*corrupt_frame_processor=*/nullptr, VideoFrameValidator::ValidationMode::kAverage,
            /*tolerance=*/100.0);
        LOG_ASSERT(log_likelihood_validator);
        quality_metrics_.push_back(BitstreamQualityMetrics(psnr_validator.get(), ssim_validator.get(), bottom_row_psnr_validator.get(),
            log_likelihood_validator.get(), decoder_buffer_validator, spatial_layer_index_to_decode, temporal_layer_index_to_decode,
            spatial_layer_resolutions.size(), inter_layer_pred_mode));
        ...
        VideoDecoderConfig decoder_config(VideoCodecProfileToVideoCodec(profile), profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(),
            kNoTransformation, visible_rect.size(), visible_rect, visible_rect.size(), EmptyExtraData(), EncryptionScheme::kUnencrypted);

        return BitstreamValidator::Create(decoder_config, num_encode_frames - 1, std::move(video_frame_processors), spatial_layer_index_to_decode,
            temporal_layer_index_to_decode, spatial_layer_resolutions);
    }

    // Create bitstream processors for quality performance tests.
    std::vector<std::unique_ptr<BitstreamProcessor>> CreateBitstreamProcessorsForQualityPerformance(RawVideo* video, VideoCodecProfile profile,
        size_t num_encode_frames, const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers, SVCInterLayerPredMode inter_layer_pred_mode)
    {
        std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors;

        raw_data_helper_ = std::make_unique<RawDataHelper>(video, g_env->Reverse());

        auto decoder_buffer_validator = DecoderBufferValidator::Create(profile, gfx::Rect(video->Resolution()),
            spatial_layers.empty() ? 1u : spatial_layers.size(), spatial_layers.empty() ? 1u : spatial_layers[0].num_of_temporal_layers, inter_layer_pred_mode);
        ...
        if (spatial_layers.empty()) {
            ...
            bitstream_processors.push_back(CreateBitstreamValidator(profile, gfx::Rect(video->Resolution()), num_encode_frames,
                /*spatial_layer_index_to_decode=*/std::nullopt,
                /*temporal_layer_index_to_decode=*/std::nullopt,
                /*spatial_layer_resolutions=*/ {}, decoder_buffer_validator_, SVCInterLayerPredMode::kOff));
            if (g_env->SaveOutputBitstream()) {
                bitstream_processors.emplace_back(BitstreamFileWriter::Create(g_env->OutputFilePath(VideoCodecProfileToVideoCodec(profile)),
                    VideoCodecProfileToVideoCodec(profile), video->Resolution(), video->FrameRate(), video->NumFrames()));
            }
        } else {
            ...
            for (size_t sid = 0; sid < spatial_layers.size(); ++sid) {
                for (size_t tid = 0; tid < spatial_layers[sid].num_of_temporal_layers; ++tid) {
                    bitstream_processors.push_back(CreateBitstreamValidator(profile, gfx::Rect(spatial_layer_resolutions[sid]), num_encode_frames, sid, tid,
                        spatial_layer_resolutions, decoder_buffer_validator_, inter_layer_pred_mode));
                    if (g_env->SaveOutputBitstream()) {
                        bitstream_processors.emplace_back(BitstreamFileWriter::Create(
                            g_env->OutputFilePath(VideoCodecProfileToVideoCodec(profile), true, sid, tid), VideoCodecProfileToVideoCodec(profile),
                            spatial_layer_resolutions[sid], video->FrameRate(), video->NumFrames(), sid, tid, spatial_layer_resolutions));
                    }
                }
            }
        }
    ...
    scoped_refptr<const VideoFrame> GetModelFrame(const gfx::Rect& visible_rect, size_t frame_index)
    {
...
{
    if (g_env->RunTestType() != media::test::VideoEncoderTestEnvironment::TestType::kSpeedPerformance) {
        GTEST_SKIP() << "Skip because this test case is to measure speed performance";
    }
    ...
    EXPECT_EQ(encoder->GetFrameReleasedCount(), kNumEncodeFramesForSpeedPerformance);
}
...
TEST_F(VideoEncoderTest, MeasureUncappedPerformance_MultipleConcurrentEncoders)
{
    if (g_env->RunTestType() != media::test::VideoEncoderTestEnvironment::TestType::kSpeedPerformance) {
        GTEST_SKIP() << "Skip because this test case is to measure speed performance";
    }
    ...
    const size_t kMinSupportedConcurrentEncoders = g_env->Video()->Resolution().GetArea() >= k1080p.GetArea() ? 2 : 3;

    std::vector<std::unique_ptr<VideoEncoder>> encoders(kMinSupportedConcurrentEncoders);
...
{
    if (g_env->RunTestType() != media::test::VideoEncoderTestEnvironment::TestType::kQualityPerformance) {
        GTEST_SKIP() << "Skip because this test case is to measure quality performance";
    }
    ...
    for (auto& metrics : quality_metrics_) {
        ...
        if (!spatial_idx && !temporal_idx) {
            ...
            for (size_t tid = 0; tid <= *temporal_idx; ++tid) {
                target_bitrate += g_env->BitrateAllocation().GetBitrateBps(*spatial_idx, tid);
...
int main(int argc, char** argv)
{
    ...
    media::test::VideoEncoderTestEnvironment::TestType test_type = media::test::VideoEncoderTestEnvironment::TestType::kValidation;
    base::FilePath video_path = (args.size() >= 1) ? base::FilePath(args[0]) : base::FilePath(media::test::kDefaultTestVideoPath);
    base::FilePath video_metadata_path = (args.size() >= 2) ? base::FilePath(args[1]) : base::FilePath();
    std::string codec = "h264";
    media::VideoEncodeAccelerator::Config::ContentType content_type = media::VideoEncodeAccelerator::Config::ContentType::kCamera;
    ...
    for (base::CommandLine::SwitchMap::const_iterator it = switches.begin(); it != switches.end(); ++it) {
        ...
        if (it->first == "num_temporal_layers" || it->first == "num_spatial_layers") {
        ...
        if (it->first == "output_folder") {
            ...
            if (content_type_str == "display") {
                content_type = media::VideoEncodeAccelerator::Config::ContentType::kDisplay;
            } else if (content_type_str != "camera") {
                std::cout << "unknown content type \"" << it->second << "\", possible values are \"camera|display\"\n";
                return EXIT_FAILURE;
            }
            ...
            if (brc_mode_str == "vbr") {
                ...
                std::cout << "unknown bitrate mode \"" << it->second << "\", possible values are \"cbr|vbr\"\n";
                return EXIT_FAILURE;
            }
            ...
            test_type = media::test::VideoEncoderTestEnvironment::TestType::kSpeedPerformance;
        } else if (it->first == "quality") {
            test_type = media::test::VideoEncoderTestEnvironment::TestType::kQualityPerformance;
        } else {
    ...
    if (test_type == media::test::VideoEncoderTestEnvironment::TestType::kValidation) {
    ...
    media::test::VideoEncoderTestEnvironment* test_environment
        = media::test::VideoEncoderTestEnvironment::Create(test_type, video_path, video_metadata_path, base::FilePath(output_folder), codec, svc_mode,
            content_type, output_bitstream, encode_bitrate, bitrate_mode, reverse, media::test::FrameOutputConfig(), enabled_features, disabled_features);
    if (!test_environment)
        return EXIT_FAILURE;

    media::test::g_env = static_cast<media::test::VideoEncoderTestEnvironment*>(testing::AddGlobalTestEnvironment(test_environment));
