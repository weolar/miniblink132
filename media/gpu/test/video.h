...
class Video {
public:
    Video(const base::FilePath& file_path, const base::FilePath& metadata_file_path);
    ...
    std::unique_ptr<Video> Expand(const gfx::Size& resolution, const gfx::Rect& visible_rect) const;
...
private:
    ...
    static absl::optional<VideoCodecProfile> ConvertStringtoProfile(const std::string& profile);
    // Return the codec associated with the |profile|.
    static absl::optional<VideoCodec> ConvertProfileToCodec(VideoCodecProfile profile);
    // Return the pixel format associated with the |pixel_format| string.
    static absl::optional<VideoPixelFormat> ConvertStringtoPixelFormat(const std::string& pixel_format);
    ...
    absl::optional<base::FilePath> ResolveFilePath(const base::FilePath& file_path);
    ...
    static void DecodeTask(const std::vector<uint8_t> data, const gfx::Size& resolution, const size_t num_frames, std::vector<uint8_t>* decompressed_data,
    ...
    // |data|.
    static void OnFrameDecoded(const gfx::Size& resolution, std::vector<uint8_t>* data, scoped_refptr<VideoFrame> frame);
