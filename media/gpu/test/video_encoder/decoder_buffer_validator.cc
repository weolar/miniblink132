...
std::unique_ptr<DecoderBufferValidator> DecoderBufferValidator::Create(
    VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t num_spatial_layers, size_t num_temporal_layers, SVCInterLayerPredMode inter_layer_pred)
{
    ...
    case VideoCodec::kH264:
        return std::make_unique<H264Validator>(profile, visible_rect, num_temporal_layers);
    ...
    case VideoCodec::kVP9:
        ...
        return std::make_unique<VP9Validator>(profile, visible_rect, num_spatial_layers, num_temporal_layers, inter_layer_pred);
...
DecoderBufferValidator::DecoderBufferValidator(const gfx::Rect& visible_rect, size_t num_temporal_layers)
...
void DecoderBufferValidator::ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    CHECK(bitstream);
...
H264Validator::H264Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t num_temporal_layers, std::optional<uint8_t> level)
...
bool H264Validator::Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata)
{
    ...
    if (num_temporal_layers_ > 1) {
        ...
        if (metadata.h264->temporal_idx >= num_temporal_layers_) {
            LOG(ERROR) << "Invalid temporal_idx: " << base::strict_cast<int32_t>(metadata.h264->temporal_idx);
            return false;
        }
    }
    ...
    while ((result = parser_.AdvanceToNextNALU(&nalu)) != H264Parser::kEOStream) {
        ...
        case H264NALU::kIDRSlice:
            ...
            if (metadata.h264 && (metadata.h264->temporal_idx != 0 || metadata.h264->layer_sync)) {
                LOG(ERROR) << "temporal_idx=" << base::strict_cast<int>(metadata.h264->temporal_idx)
                           << " or layer_sync=" << base::strict_cast<int>(metadata.h264->layer_sync) << " is unexpected";
                return false;
            }
        ...
        case H264NALU::kNonIDRSlice: {
            ...
            const int qp = slice_hdr.slice_qp_delta + parser_.GetPPS(cur_pps_id_)->pic_init_qp_minus26 + 26;
            DVLOGF(4) << "qp=" << qp;
            const int temporal_idx = metadata.h264 ? metadata.h264->temporal_idx : 0;
        ...
        case H264NALU::kSPS: {
            ...
            if (visible_rect != visible_rect_) {
                LOG(ERROR) << "Visible rectangle mismatched. Actual visible_rect: " << visible_rect.ToString()
                           << ", expected visible_rect: " << visible_rect_.ToString();
                return false;
            }
            if (profile_ != sps->profile_idc) {
                LOG(ERROR) << "Profile mismatched. Actual profile: " << sps->profile_idc << ", expected profile: " << profile_;
                return false;
            }
            if (level_ && sps->GetIndicatedLevel() != *level_) {
                LOG(ERROR) << "Level mismatched. Actual profile: " << static_cast<int>(sps->GetIndicatedLevel())
        ...
        case H264NALU::kPPS: {
            ...
            if ((profile_ == H264SPS::kProfileIDCMain || profile_ == H264SPS::kProfileIDCHigh) && !pps->entropy_coding_mode_flag) {
...
bool H264Validator::IsNewPicture(const H264SliceHeader& slice_hdr)
{
    if (!cur_pic_)
        return true;
    return H264Decoder::IsNewPrimaryCodedPicture(cur_pic_.get(), cur_pps_id_, parser_.GetSPS(cur_sps_id_), slice_hdr);
}
...
bool H264Validator::UpdateCurrentPicture(const H264SliceHeader& slice_hdr)
{
    ...
    if (!H264Decoder::FillH264PictureFromSliceHeader(sps, slice_hdr, cur_pic_.get())) {
...
VP8Validator::VP8Validator(const gfx::Rect& visible_rect, size_t num_temporal_layers)
    : DecoderBufferValidator(visible_rect, num_temporal_layers)
{
}
...
bool VP8Validator::Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata)
{
    if (metadata.dropped_frame()) {
        if (metadata.key_frame) {
            LOG(ERROR) << "Don't drop key frame";
    ...
    if (!parser_.ParseFrame(decoder_buffer.data(), decoder_buffer.size(), &header)) {
        LOG(ERROR) << "Failed parsing";
        return false;
    }
    ...
    if (header.IsKeyframe()) {
        ...
        if (gfx::Rect(header.width, header.height) != visible_rect_) {
            LOG(ERROR) << "Visible rectangle mismatched. Actual visible_rect: " << gfx::Rect(header.width, header.height).ToString()
                       << ", expected visible_rect: " << visible_rect_.ToString();
            return false;
        }
    }
    ...
    if (num_temporal_layers_ == 1) {
        ...
        LOG(ERROR) << "refereh_entropy_probs must be false in temporal layer encoding";
        return false;
    }
    ...
    if (temporal_idx >= num_temporal_layers_) {
        LOG(ERROR) << "Invalid temporal id: " << base::strict_cast<int>(temporal_idx);
        return false;
    }
    qp_values_[0][temporal_idx].push_back(qp);
    if (header.IsKeyframe()) {
    ...
    if (header.copy_buffer_to_golden != Vp8FrameHeader::NO_GOLDEN_REFRESH || header.copy_buffer_to_alternate != Vp8FrameHeader::NO_ALT_REFRESH) {
        LOG(ERROR) << "Each reference frame is either updated by the current "
                   << "frame or unchanged in temporal layer encoding.";
        return false;
    }

    const bool update_reference = header.refresh_last || header.refresh_golden_frame || header.refresh_alternate_frame;
    ...
    if (num_temporal_layers_ == temporal_idx + 1) {
        ...
        if (temporal_idx == 0) {
            if (!header.refresh_last || header.refresh_golden_frame || header.refresh_alternate_frame) {
...
VP9Validator::VP9Validator(
    VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t max_num_spatial_layers, size_t num_temporal_layers, SVCInterLayerPredMode inter_layer_pred)
    : DecoderBufferValidator(visible_rect, num_temporal_layers)
    , profile_(VideoCodecProfileToVP9Profile(profile))
    , max_num_spatial_layers_(max_num_spatial_layers)
    , s_mode_(max_num_spatial_layers > 1 && inter_layer_pred == SVCInterLayerPredMode::kOff)
    ...
    , next_picture_id_(0)
{
    ...
    for (size_t i = 0; i < num_parsed_streams; ++i) {
        parsers_.push_back(std::make_unique<Vp9Parser>(/*parsing_compressed_header=*/false));
    }
...
bool VP9Validator::Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata)
{
    if (metadata.dropped_frame()) {
        if (metadata.key_frame) {
            LOG(ERROR) << "Don't drop key frame";
        ...
        if (metadata.vp9.has_value()) {
            LOG(ERROR) << "BitstreamBufferMetadata has Vp9Metadata on a dropped frame";
            return false;
        }
        if (metadata.end_of_picture()) {
    ...
    if (dropped_superframe_timestamp_ && *dropped_superframe_timestamp_ == metadata.timestamp) {
    ...
    if ((base::span(decoder_buffer).back() & kSuperFrameMarkerMask) == kSuperFrameMarker) {
        LOG(ERROR) << "Support for super-frames not yet implemented.";
        return false;
    }

    const bool svc_encoding = max_num_spatial_layers_ > 1 || num_temporal_layers_ > 1;
    if (metadata.vp9.has_value() != svc_encoding) {
        LOG(ERROR) << "VP9 specific metadata must exist only for SVC encodings";
        return false;
    }

    const size_t parser_index = s_mode_ ? begin_active_spatial_layer_index_ + metadata.vp9->spatial_idx : 0;
    ...
    if (parser.ParseNextFrame(&header, &allocate_size, nullptr) == Vp9Parser::kInvalidStream) {
        LOG(ERROR) << "Failed parsing";
        return false;
    }

    if (metadata.key_frame != header.IsKeyframe()) {
        LOG(ERROR) << "Keyframe info in metadata is wrong, metadata.keyframe=" << metadata.key_frame;
        return false;
    }
    if (header.profile != static_cast<uint8_t>(profile_)) {
        LOG(ERROR) << "Profile mismatched. Actual profile: " << static_cast<int>(header.profile) << ", expected profile: " << profile_;
        return false;
    }
...
bool VP9Validator::ValidateVanillaStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header)
{
    ...
    if (header.show_existing_frame && !reference_buffers_[0][header.frame_to_show_map_idx]) {
    ...
    if (!header.IsIntra()) {
        for (uint8_t ref_frame_index : header.ref_frame_idx) {
            if (ref_frame_index >= static_cast<uint8_t>(kVp9NumRefFrames)) {
                LOG(ERROR) << "Invalid reference frame index: " << static_cast<int>(ref_frame_index);
                return false;
            }
...
bool VP9Validator::ValidateSVCStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header)
{
    ...
    if (vp9.spatial_idx >= cur_num_spatial_layers_ || vp9.temporal_idx >= num_temporal_layers_) {
        LOG(ERROR) << "Invalid spatial_idx=" << base::strict_cast<int>(vp9.spatial_idx) << ", temporal_idx=" << base::strict_cast<int>(vp9.temporal_idx);
        return false;
    }
    ...
    if (metadata.key_frame) {
        ...
        int expected_diff = next_picture_id_ - reference_buffers_[0][header.frame_to_show_map_idx]->picture_id;
    ...
    if (end_of_picture != metadata.end_of_picture()) {
        LOG(ERROR) << "end_of_picture mismatches: end_of_picture=" << end_of_picture << ", metadata.end_of_picture=" << metadata.end_of_picture();
        return false;
    }
    ...
    if (visible_rect.size() != spatial_layer_resolutions_[vp9.spatial_idx]) {
        LOG(ERROR) << "Resolution mismatched. Actual resolution: " << visible_rect.size().ToString()
                   << ", expected resolution: " << spatial_layer_resolutions_[vp9.spatial_idx].ToString();
        return false;
    }
    ...
    if (header.IsIntra()) {
        ...
        for (uint8_t ref_frame_index : header.ref_frame_idx) {
            ...
            if (ref.spatial_id > new_buffer_state.spatial_id) {
                LOG(ERROR) << "Frame is trying to reference buffer from higher spatial layer.";
                return false;
            }
        ...
        for (uint8_t p_diff : vp9.p_diffs) {
            if (!std::erase(expected_pdiffs, p_diff)) {
                LOG(ERROR) << "Frame is referencing buffer not contained in the p_diff.";
                return false;
            }
        }
...
bool VP9Validator::ValidateSmodeStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header)
{
    const Vp9Metadata& vp9 = *metadata.vp9;
    if (next_picture_id_ == 0 && !metadata.key_frame) {
    ...
    if (metadata.key_frame) {
        ...
        const size_t stream_index = vp9.spatial_idx + begin_active_spatial_layer_index_;
        if (!reference_buffers_[stream_index][header.frame_to_show_map_idx]) {
            LOG(ERROR) << "Attempting to show an existing frame, but the selected "
                          "reference buffer is invalid.";
            return false;
        }
        int expected_diff = next_picture_id_ - reference_buffers_[stream_index][header.frame_to_show_map_idx]->picture_id;
        if (vp9.p_diffs.size() != 1 || vp9.p_diffs[0] != expected_diff) {
            LOG(ERROR) << "Inconsistency between p_diff and existing frame to show.";
            return false;
        }
    ...
    if (header.IsIntra()) {
        ...
        for (uint8_t ref_frame_index : header.ref_frame_idx) {
            ...
            const BufferState& ref = *reference_buffers_[vp9.spatial_idx][ref_frame_index];
            if (ref.spatial_id > new_buffer_state.spatial_id) {
                LOG(ERROR) << "Frame is trying to reference buffer from higher spatial layer.";
                return false;
            }
...
AV1Validator::AV1Validator(const gfx::Rect& visible_rect)
    ...
    , buffer_pool_(libgav1::OnInternalFrameBufferSizeChanged, libgav1::GetInternalFrameBuffer, libgav1::ReleaseInternalFrameBuffer, &buffer_list_)
{
}
...
// right dimensions.
bool AV1Validator::Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata)
{
    if (metadata.dropped_frame()) {
        if (metadata.key_frame) {
            LOG(ERROR) << "Don't drop key frame";
    ...
    CHECK(buffer);
    const DecoderBuffer& decoder_buffer = *buffer;
    libgav1::ObuParser av1_parser(decoder_buffer.data(), decoder_buffer.size(), 0, &buffer_pool_, &decoder_state_);
    ...
    if (gfx::Size(frame_header.render_width, frame_header.render_height) != visible_rect_.size()) {
        LOG(ERROR) << "Mismatched visible rectangle dimensions.";
        LOG(ERROR) << "Got render_width=" << frame_header.render_width << " render_height=" << frame_header.render_height;
        LOG(ERROR) << "Expected visible_width=" << visible_rect_.width() << " visible_height=" << visible_rect_.height();
        return false;
    }

    if (frame_header.frame_type != libgav1::FrameType::kFrameKey && frame_num_ == 0) {
    ...
    decoder_state_.UpdateReferenceFrames(curr_frame, av1_parser.frame_header().refresh_frame_flags);
