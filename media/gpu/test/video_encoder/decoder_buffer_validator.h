...
public:
    static std::unique_ptr<DecoderBufferValidator> Create(VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t num_spatial_layers,
    ...
    const std::vector<int>& GetQPValues(size_t spatial_idx, size_t temporal_idx) const
    {
...
protected:
    ...
    DecoderBufferValidator(const gfx::Rect& visible_rect, size_t num_temporal_layers);

    // Returns true if decoder_buffer is valid and expected, otherwise false.
    virtual bool Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata) = 0;
...
public:
    H264Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, const size_t num_temporal_layers, std::optional<uint8_t> level = std::nullopt);
    ~H264Validator() override;

private:
    bool Validate(const DecoderBuffer* buffer, const BitstreamBufferMetadata& metadata) override;
...
public:
    VP9Validator(VideoCodecProfile profile, const gfx::Rect& visible_rect, size_t max_num_spatial_layers, size_t num_temporal_layers,
...
private:
    ...
    bool ValidateVanillaStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header);
    // Validate DecoderBuffer for a temporal or spatial layer stream.
    bool ValidateSVCStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header);
    // Validate DecoderBuffer for S-mode stream.
    bool ValidateSmodeStream(const DecoderBuffer& decoder_buffer, const BitstreamBufferMetadata& metadata, const Vp9FrameHeader& header);
    ...
    std::vector<std::array<std::optional<BufferState>, kVp9NumRefFrames>> reference_buffers_;
