...
struct VideoEncoderClientConfig {
    ...
    VideoEncoderClientConfig(const RawVideo* video, VideoCodecProfile output_profile,
        const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers, SVCInterLayerPredMode inter_layer_pred_mode,
        VideoEncodeAccelerator::Config::ContentType content_type, const media::VideoBitrateAllocation& bitrate, bool reverse);
    ...
    VideoEncodeAccelerator::Config::StorageType input_storage_type = VideoEncodeAccelerator::Config::StorageType::kShmem;
...
public:
    ...
    ~VideoEncoderStats();
    VideoEncoderStats(uint32_t framerate, size_t num_temporal_layers, size_t num_spatial_layers);
...
public:
    ...
    static std::unique_ptr<VideoEncoderClient> Create(const VideoEncoder::EventCallback& event_cb,
        std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors, const VideoEncoderClientConfig& config);
...
private:
    ...
    VideoEncoderClient(const VideoEncoder::EventCallback& event_cb, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors,
    ...
    void CreateEncoderTask(const RawVideo* video, bool* success, base::WaitableEvent* done);
    ...
    void FlushTask();
    void UpdateBitrateTask(const VideoBitrateAllocation& bitrate, uint32_t framerate);
    ...
    scoped_refptr<BitstreamProcessor::BitstreamRef> CreateBitstreamRef(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata);
