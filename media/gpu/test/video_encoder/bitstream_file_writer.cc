...
public:
    ...
    bool WriteFrame(uint32_t data_size, uint64_t timestamp, const uint8_t* data)
    {
        ...
        return output_file_.WriteAtCurrentPos(reinterpret_cast<const char*>(data), data_size) == static_cast<int>(data_size);
    }
...
BitstreamFileWriter::BitstreamFileWriter(std::unique_ptr<FrameFileWriter> frame_file_writer, std::optional<size_t> spatial_layer_index_to_write,
    std::optional<size_t> temporal_layer_index_to_write, const std::vector<gfx::Size>& spatial_layer_resolutions)
...
// static
std::unique_ptr<BitstreamFileWriter> BitstreamFileWriter::Create(const base::FilePath& output_filepath, VideoCodec codec, const gfx::Size& resolution,
    uint32_t frame_rate, uint32_t num_frames, std::optional<size_t> spatial_layer_index_to_write, std::optional<size_t> temporal_layer_index_to_write,
    const std::vector<gfx::Size>& spatial_layer_resolutions)
{
    ...
    if (codec == VideoCodec::kH264) {
        base::File output_file(output_filepath, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
        LOG_ASSERT(output_file.IsValid());
        frame_file_writer = std::make_unique<FrameFileWriter>(std::move(output_file));
    } else {
        auto ivf_writer = std::make_unique<IvfWriter>(output_filepath);
        if (!ivf_writer->WriteFileHeader(codec, resolution, frame_rate, num_frames)) {
            LOG(ERROR) << "Failed writing ivf header";
            return nullptr;
        }

        frame_file_writer = std::make_unique<FrameFileWriter>(std::move(ivf_writer));
    }

    auto bitstream_file_writer = base::WrapUnique(
        new BitstreamFileWriter(std::move(frame_file_writer), spatial_layer_index_to_write, temporal_layer_index_to_write, spatial_layer_resolutions));
...
void BitstreamFileWriter::ProcessBitstream(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    ...
    if (spatial_layer_index_to_write_ && bitstream->metadata.vp9) {
        ...
        if (bitstream->metadata.key_frame) {
            begin_active_spatial_layer_index_ = metadata.begin_active_spatial_layer_index;
        }

        const uint8_t spatial_idx = begin_active_spatial_layer_index_ + metadata.spatial_idx;
        if (spatial_idx > *spatial_layer_index_to_write_ || (spatial_idx < *spatial_layer_index_to_write_ && !metadata.referenced_by_upper_spatial_layers)) {
    ...
    writer_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&BitstreamFileWriter::WriteBitstreamTask, base::Unretained(this), std::move(bitstream), frame_index));
}

void BitstreamFileWriter::WriteBitstreamTask(scoped_refptr<BitstreamRef> bitstream, size_t frame_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(writer_thread_sequence_checker_);
    const DecoderBuffer& buffer = *bitstream->buffer.get();
    bool success = frame_file_writer_->WriteFrame(static_cast<uint32_t>(buffer.size()), static_cast<uint64_t>(frame_index), buffer.data());
