...
void CallbackThunk(std::optional<base::WeakPtr<VideoEncoderClient>> encoder_client, scoped_refptr<base::SequencedTaskRunner> task_runner, CallbackFunc func,
    CallbackArgs... args)
{
    DCHECK(encoder_client);
    task_runner->PostTask(FROM_HERE, base::BindOnce(func, *encoder_client, args...));
}

} // namespace

VideoEncoderClientConfig::VideoEncoderClientConfig(const RawVideo* video, VideoCodecProfile output_profile,
    const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers, SVCInterLayerPredMode inter_layer_pred_mode,
    VideoEncodeAccelerator::Config::ContentType content_type, const VideoBitrateAllocation& bitrate_allocation, bool reverse)
    : output_profile(output_profile)
    , output_resolution(video->Resolution())
    , spatial_layers(spatial_layers)
    , num_temporal_layers(spatial_layers.empty() ? 1 : spatial_layers[0].num_of_temporal_layers)
    , num_spatial_layers(std::max(spatial_layers.size(), static_cast<size_t>(1u)))
    ...
    , reverse(reverse)
{
    CHECK(inter_layer_pred_mode == SVCInterLayerPredMode::kOff || inter_layer_pred_mode == SVCInterLayerPredMode::kOnKeyPic);
}

VideoEncoderClientConfig::VideoEncoderClientConfig(const VideoEncoderClientConfig&) = default;
...
VideoEncoderStats::VideoEncoderStats(uint32_t framerate, size_t num_temporal_layers, size_t num_spatial_layers)
    : framerate(framerate)
    , num_encoded_frames_per_layer(num_spatial_layers, std::vector<size_t>(num_temporal_layers, 0))
    , encoded_frames_size_per_layer(num_spatial_layers, std::vector<size_t>(num_temporal_layers, 0))
...
{
    const size_t average_frame_size_in_bits = total_encoded_frames_size * 8 / total_num_encoded_frames;
    const uint32_t average_bitrate = base::checked_cast<uint32_t>(average_frame_size_in_bits * framerate * num_spatial_layers);
    VLOGF(2) << " [Total] encoded_frames=" << total_num_encoded_frames << ", framerate=" << framerate << ", num_spatial_layers=" << num_spatial_layers
             << ", total_encoded_frames_size=" << total_encoded_frames_size << ", average_frame_size_in_bits=" << average_frame_size_in_bits
             << ", average bitrate=" << average_bitrate;

    return average_bitrate;
}

uint32_t VideoEncoderStats::LayerBitrate(size_t spatial_idx, size_t temporal_idx) const
{
    const size_t num_frames = num_encoded_frames_per_layer[spatial_idx][temporal_idx];
    const size_t frames_size = encoded_frames_size_per_layer[spatial_idx][temporal_idx];
    ...
    const double layer_framerate = static_cast<double>(framerate) / kFramerateDenom[num_temporal_layers - 1][temporal_idx];
    const double average_frame_size_in_bits = frames_size * 8 / num_frames;
    const uint32_t average_bitrate = base::checked_cast<uint32_t>(average_frame_size_in_bits * layer_framerate);

    std::string prefix;
    if (num_spatial_layers > 1) {
        prefix = "[SL#" + base::NumberToString(spatial_idx) + " TL#" + base::NumberToString(temporal_idx) + "] ";
    } else {
        DCHECK_NE(num_temporal_layers, 1u);
        prefix = "[TL#" + base::NumberToString(temporal_idx) + "] ";
    }

    VLOGF(2) << prefix << "encoded_frames=" << num_frames << ", framerate=" << layer_framerate << ", total_encoded_frames_size=" << frames_size
             << ", average_frame_size_in_bits=" << average_frame_size_in_bits << ", average bitrate=" << average_bitrate;
...
void VideoEncoderStats::Reset()
{
    ...
    std::fill(num_encoded_frames_per_layer.begin(), num_encoded_frames_per_layer.end(), std::vector<size_t>(num_temporal_layers, 0u));
    std::fill(encoded_frames_size_per_layer.begin(), encoded_frames_size_per_layer.end(), std::vector<size_t>(num_temporal_layers, 0u));
}

VideoEncoderClient::VideoEncoderClient(
    const VideoEncoder::EventCallback& event_cb, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors, const VideoEncoderClientConfig& config)
    : event_cb_(event_cb)
    ...
    , current_stats_(encoder_client_config_.framerate, config.num_temporal_layers, config.num_spatial_layers)
{
...
std::unique_ptr<VideoEncoderClient> VideoEncoderClient::Create(
    const VideoEncoder::EventCallback& event_cb, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors, const VideoEncoderClientConfig& config)
{
    return base::WrapUnique(new VideoEncoderClient(event_cb, std::move(bitstream_processors), config));
}
...
bool VideoEncoderClient::Initialize(const RawVideo* video)
{
    ...
    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::CreateEncoderTask, weak_this_, video, &success, &done));
    done.Wait();
...
void VideoEncoderClient::Destroy()
{
    ...
    base::WaitableEvent done;
    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::DestroyEncoderTask, weak_this_, &done));
    done.Wait();
...
void VideoEncoderClient::Encode()
{
    ...
    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::EncodeTask, weak_this_));
}
...
void VideoEncoderClient::Flush()
{
    ...
    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::FlushTask, weak_this_));
}

void VideoEncoderClient::UpdateBitrate(const VideoBitrateAllocation& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(test_sequence_checker_);

    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::UpdateBitrateTask, weak_this_, bitrate, framerate));
}
...
void VideoEncoderClient::ForceKeyFrame()
{
    ...
    encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::ForceKeyFrameTask, weak_this_));
}
...
void VideoEncoderClient::RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size)
{
    ...
    if (video_->Resolution() != encoder_client_config_.output_resolution) {
        ...
        EXPECT_EQ(encoder_client_config_.input_storage_type, VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer);
    ...
    const uint32_t frame_rate = encoder_client_config_.encode_interval ? 0 : video_->FrameRate();

    // Follow the behavior of the chrome capture stack; |natural_size| is the
    // dimension to be encoded.
    aligned_data_helper_ = std::make_unique<AlignedDataHelper>(video_, encoder_client_config_.num_frames_to_encode, encoder_client_config_.reverse,
        /*dst_coded_size=*/coded_size,
        /*natural_size=*/encoder_client_config_.output_resolution, frame_rate,
        encoder_client_config_.input_storage_type == VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer ? VideoFrame::STORAGE_GPU_MEMORY_BUFFER
    ...
    for (unsigned int i = 0; i < input_count; ++i) {
        ...
        BitstreamBuffer bitstream_buffer(GetNextBitstreamBufferId(), shm.Duplicate(), output_buffer_size_);

        bitstream_buffers_.insert(std::make_pair(bitstream_buffer.id(), std::move(shm)));
...
scoped_refptr<BitstreamProcessor::BitstreamRef> VideoEncoderClient::CreateBitstreamRef(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    ...
    if (!metadata.dropped_frame()) {
        decoder_buffer = DecoderBuffer::FromSharedMemoryRegion(it->second.Duplicate(), 0u /* offset */, metadata.payload_size_bytes);
        if (!decoder_buffer) {
            return nullptr;
        }
    ...
    return BitstreamProcessor::BitstreamRef::Create(std::move(decoder_buffer), metadata, bitstream_buffer_id, source_timestamp_it->second,
        base::BindPostTaskToCurrentDefault(base::BindOnce(&VideoEncoderClient::BitstreamBufferProcessed, weak_this_, bitstream_buffer_id)));
}

void VideoEncoderClient::BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    DVLOGF(4) << "frame_index=" << frame_index_ << ", encoded image size=" << metadata.payload_size_bytes << (metadata.dropped_frame() ? " (Drop Frame)" : "");
    {
        ...
        if (metadata.dropped_frame()) {
            ...
            if (metadata.vp9.has_value()) {
                ...
                current_stats_.encoded_frames_size_per_layer[spatial_id][temporal_id] += metadata.payload_size_bytes;
                ...
                current_stats_.encoded_frames_size_per_layer[0][temporal_id] += metadata.payload_size_bytes;
...
void VideoEncoderClient::FlushDoneTaskIfNeeded()
{
    ...
    if (!encoder_->IsFlushSupported() && encoder_client_state_ == VideoEncoderClientState::kFlushing
        && frame_index_ == encoder_client_config_.num_frames_to_encode && num_outstanding_encode_requests_ == 0) {
        FlushDoneTask(true);
    }
}
...
void VideoEncoderClient::BitstreamBufferProcessed(int32_t bitstream_buffer_id)
{
    ...
    BitstreamBuffer bitstream_buffer(bitstream_buffer_id, it->second.Duplicate(), output_buffer_size_);
    encoder_->UseOutputBitstreamBuffer(std::move(bitstream_buffer));
}
...
void VideoEncoderClient::NotifyErrorStatus(const EncoderStatus& status)
{
    ...
    LOG(ERROR) << "NotifyErrorStatus() is called, code=" << static_cast<int>(status.code()) << ", message=" << status.message();
...
void VideoEncoderClient::CreateEncoderTask(const RawVideo* video, bool* success, base::WaitableEvent* done)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    ...
    VideoEncodeAccelerator::Config config(video_->PixelFormat(), encoder_client_config_.output_resolution, encoder_client_config_.output_profile,
        encoder_client_config_.bitrate_allocation.GetSumBitrate(), encoder_client_config_.framerate, encoder_client_config_.input_storage_type,
        encoder_client_config_.content_type);

    config.drop_frame_thresh_percentage = encoder_client_config_.drop_frame_thresh;
    ...
    encoder_ = GpuVideoEncodeAcceleratorFactory::CreateVEA(config, this, gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds(), gpu::GPUInfo::GPUDevice());
...
void VideoEncoderClient::EncodeTask()
{
    ...
    for (size_t i = 0; i < encoder_client_config_.max_outstanding_encode_requests; ++i) {
        EncodeNextFrameTask();
    }
}
...
void VideoEncoderClient::EncodeNextFrameTask()
{
    ...
    video_frame->AddDestructionObserver(base::BindOnce(CallbackThunk<decltype(&VideoEncoderClient::EncodeDoneTask), base::TimeDelta>, weak_this_,
        encoder_client_task_runner_, &VideoEncoderClient::EncodeDoneTask, video_frame->timestamp()));
    ...
    if (encoder_client_config_.encode_interval) {
        ...
        encoder_client_task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&VideoEncoderClient::EncodeNextFrameTask, weak_this_), *encoder_client_config_.encode_interval);
    }
}
...
void VideoEncoderClient::FlushTask()
{
    ...
    auto flush_done_cb = base::BindOnce(
        CallbackThunk<decltype(&VideoEncoderClient::FlushDoneTask), bool>, weak_this_, encoder_client_task_runner_, &VideoEncoderClient::FlushDoneTask);
    encoder_->Flush(std::move(flush_done_cb));

    FireEvent(VideoEncoder::EncoderEvent::kFlushing);
}

void VideoEncoderClient::UpdateBitrateTask(const VideoBitrateAllocation& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(encoder_client_sequence_checker_);
    DVLOGF(4);
...
void VideoEncoderClient::EncodeDoneTask(base::TimeDelta timestamp)
{
    ...
    if (!encoder_client_config_.encode_interval) {
        ...
        encoder_client_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoEncoderClient::EncodeNextFrameTask, weak_this_));
    }
}
