...
uint32_t GetDefaultTargetBitrate(const VideoCodec codec, const gfx::Size& resolution, const uint32_t framerate, bool validation)
{
    ...
    const double low_bitrate = kBitrateTable[index - 1].bitrate[validation][codec_index];
    const int up_area = kBitrateTable[index].area;
    const double up_bitrate = kBitrateTable[index].bitrate[validation][codec_index];

    const double bitrate_in_30fps_in_kbps = (up_bitrate - low_bitrate) / (up_area - low_area) * (area - low_area) + low_bitrate;
    // This is selected as 1 in 30fps and 1.8 in 60fps.
    const double framerate_multiplier = 0.27 * (framerate * framerate / 30.0 / 30.0) + 0.73;
...
VideoBitrateAllocation CreateBitrateAllocation(const VideoCodec codec, const gfx::Size& resolution, uint32_t frame_rate, std::optional<uint32_t> encode_bitrate,
    size_t num_spatial_layers, size_t num_temporal_layers, bool is_vbr, bool validation)
{
    ...
    if ((num_spatial_layers == 1 && num_temporal_layers == 1) || encode_bitrate) {
        const uint32_t target_bitrate = encode_bitrate.value_or(GetDefaultTargetBitrate(codec, resolution, frame_rate, validation));
        // TODO(b/181797390): Reconsider if this peak bitrate is reasonable.
        const Bitrate bitrate
            = is_vbr ? media::Bitrate::VariableBitrate(target_bitrate, /*peak_bps=*/target_bitrate * 2) : media::Bitrate::ConstantBitrate(target_bitrate);
        return AllocateDefaultBitrateForTesting(num_spatial_layers, num_temporal_layers, bitrate);
    }
    std::vector<uint32_t> spatial_layer_bitrates(num_spatial_layers);
    for (size_t sid = 0; sid < num_spatial_layers; ++sid) {
        const int resolution_denom = kSpatialLayersResolutionScaleDenom[num_spatial_layers - 1][sid];
        LOG_IF(WARNING, resolution.width() % resolution_denom != 0) << "width of SL#" << sid << " is not dividable by " << resolution_denom;
        LOG_IF(WARNING, resolution.height() % resolution_denom != 0) << "height of SL#" << sid << " is not dividable by " << resolution_denom;
        const gfx::Size spatial_layer_resolution(resolution.width() / resolution_denom, resolution.height() / resolution_denom);
        spatial_layer_bitrates[sid] = GetDefaultTargetBitrate(codec, spatial_layer_resolution, frame_rate, validation);
    }
    return AllocateBitrateForDefaultEncodingWithBitrates(spatial_layer_bitrates, num_temporal_layers, is_vbr);
}

std::vector<VideoEncodeAccelerator::Config::SpatialLayer> GetDefaultSpatialLayers(
    const VideoBitrateAllocation& bitrate, const gfx::Size& resolution, uint32_t frame_rate, size_t num_spatial_layers, size_t num_temporal_layers)
{
...
// static
VideoEncoderTestEnvironment* VideoEncoderTestEnvironment::Create(TestType test_type, const base::FilePath& video_path,
    const base::FilePath& video_metadata_path, const base::FilePath& output_folder, const std::string& codec, const std::string& svc_mode,
    VideoEncodeAccelerator::Config::ContentType content_type, bool save_output_bitstream, std::optional<uint32_t> encode_bitrate, Bitrate::Mode bitrate_mode,
    bool reverse, const FrameOutputConfig& frame_output_config, const std::vector<base::test::FeatureRef>& enabled_features,
    const std::vector<base::test::FeatureRef>& disabled_features)
{
    ...
    const auto* codec_it = base::ranges::find(kCodecParamToProfile, codec, &CodecParamToProfile::codec);
    ...
    const auto* svc_it = base::ranges::find(kSVCModeParamToSVCConfig, svc_mode, &SVCConfig::svc_mode);
    ...
    const VideoBitrateAllocation bitrate_allocation = CreateBitrateAllocation(video_codec, video->Resolution(), video->FrameRate(), encode_bitrate,
        num_spatial_layers, num_temporal_layers, is_vbr, test_type == TestType::kValidation);
    ...
    std::vector<base::test::FeatureRef> combined_enabled_features(enabled_features);
    std::vector<base::test::FeatureRef> combined_disabled_features(disabled_features);
    ...
    combined_disabled_features.push_back(media::kVaapiEnforceVideoMinMaxResolution);
#endif
    ...
    return new VideoEncoderTestEnvironment(test_type, std::move(video), output_folder, video_path.BaseName(), profile, inter_layer_pred_mode,
        num_spatial_layers, num_temporal_layers, content_type, bitrate_allocation, save_output_bitstream, reverse, frame_output_config,
        combined_enabled_features, combined_disabled_features);
}

VideoEncoderTestEnvironment::VideoEncoderTestEnvironment(TestType test_type, std::unique_ptr<media::test::RawVideo> video, const base::FilePath& output_folder,
    const base::FilePath& output_bitstream_file_base_name, VideoCodecProfile profile, SVCInterLayerPredMode inter_layer_pred_mode, size_t num_spatial_layers,
    size_t num_temporal_layers, VideoEncodeAccelerator::Config::ContentType content_type, const VideoBitrateAllocation& bitrate, bool save_output_bitstream,
    bool reverse, const FrameOutputConfig& frame_output_config, const std::vector<base::test::FeatureRef>& enabled_features,
    const std::vector<base::test::FeatureRef>& disabled_features)
    ...
    , bitrate_(bitrate)
    , spatial_layers_(GetDefaultSpatialLayers(bitrate_, video_->Resolution(), video_->FrameRate(), num_spatial_layers, num_temporal_layers))
    , content_type_(content_type)
    , save_output_bitstream_(save_output_bitstream)
    , reverse_(reverse)
    , frame_output_config_(frame_output_config)
    , gpu_memory_buffer_factory_(gpu::GpuMemoryBufferFactory::CreateNativeType(nullptr))
{
}
...
VideoEncoderTestEnvironment::TestType VideoEncoderTestEnvironment::RunTestType() const
{
    return test_type_;
}
...
const std::vector<VideoEncodeAccelerator::Config::SpatialLayer>& VideoEncoderTestEnvironment::SpatialLayers() const
{
...
VideoEncodeAccelerator::Config::ContentType VideoEncoderTestEnvironment::ContentType() const
{
    return content_type_;
}

const VideoBitrateAllocation& VideoEncoderTestEnvironment::BitrateAllocation() const
{
    return bitrate_;
}
...
base::FilePath VideoEncoderTestEnvironment::OutputFilePath(const VideoCodec& codec, bool svc_enable, int spatial_idx, int temporal_idx) const
{
    base::FilePath::StringPieceType extension = codec == VideoCodec::kH264 ? FILE_PATH_LITERAL("h264") : FILE_PATH_LITERAL("ivf");
    auto output_bitstream_filepath = OutputFolder().Append(GetTestOutputFilePath()).Append(output_bitstream_file_base_name_.ReplaceExtension(extension));
    if (svc_enable) {
        auto file_name_suffix = base::StringPrintf(".SL%d.TL%d", spatial_idx, temporal_idx);
        output_bitstream_filepath = output_bitstream_filepath.InsertBeforeExtensionASCII(file_name_suffix);
    }
...
const FrameOutputConfig& VideoEncoderTestEnvironment::ImageOutputConfig() const
{
    return frame_output_config_;
}

gpu::GpuMemoryBufferFactory* VideoEncoderTestEnvironment::GetGpuMemoryBufferFactory() const
{
