...
public:
    ...
    static std::unique_ptr<BitstreamValidator> Create(const VideoDecoderConfig& decoder_config, size_t last_frame_index,
        std::vector<std::unique_ptr<VideoFrameProcessor>> video_frame_processors = {}, std::optional<size_t> spatial_layer_index_to_decode_ = std::nullopt,
        std::optional<size_t> temporal_layer_index_to_decode = std::nullopt, const std::vector<gfx::Size>& spatial_layer_resolutions = {});
...
private:
    BitstreamValidator(std::unique_ptr<VideoDecoder> decoder, std::unique_ptr<MediaLog> media_log, size_t last_frame_index, const gfx::Rect& decoding_rect,
        std::optional<size_t> spatial_layer_index_to_decode_, std::optional<size_t> temporal_layer_index_to_decode,
    ...
    void InitializeVideoDecoder(const VideoDecoderConfig& decoder_config, VideoDecoder::InitCB init_cb);
    void ProcessBitstreamTask(scoped_refptr<BitstreamRef> decoder_buffer, size_t frame_index);
    ...
    void ConstructSpatialIndices(const std::vector<gfx::Size>& spatial_layer_resolutions);
    ...
    const std::vector<std::unique_ptr<VideoFrameProcessor>> video_frame_processors_;
    ...
    base::LRUCache<int64_t, std::pair<size_t, scoped_refptr<BitstreamRef>>> decoding_buffers_ { kDecoderBufferMapSize };
