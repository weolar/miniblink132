...
constexpr std::pair<VideoEncoder::EncoderEvent, size_t> kInvalidEncodeUntil { VideoEncoder::kNumEvents, std::numeric_limits<size_t>::max() };
} // namespace

// static
std::unique_ptr<VideoEncoder> VideoEncoder::Create(
    const VideoEncoderClientConfig& config, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors)
{
    auto video_encoder = base::WrapUnique(new VideoEncoder());
    if (!video_encoder->CreateEncoderClient(config, std::move(bitstream_processors))) {
        return nullptr;
    }
...
bool VideoEncoder::CreateEncoderClient(const VideoEncoderClientConfig& config, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // destroyed first.
    EventCallback event_cb = base::BindRepeating(&VideoEncoder::NotifyEvent, base::Unretained(this));

    encoder_client_ = VideoEncoderClient::Create(event_cb, std::move(bitstream_processors), config);
    if (!encoder_client_) {
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(video_encoder_state_ == EncoderState::kUninitialized || video_encoder_state_ == EncoderState::kIdle);
    DCHECK(video);
    DVLOGF(4);
...
void VideoEncoder::EncodeUntil(EncoderEvent event, size_t event_count)
{
    ...
    if (video_encoder_state_.load() != EncoderState::kIdle) {
        LOG(ERROR) << "VideoEncoder state is not idle: " << static_cast<int>(video_encoder_state_.load());
        return;
    }
...
void VideoEncoder::UpdateBitrate(const VideoBitrateAllocation& bitrate, uint32_t framerate)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOGF(4);
...
bool VideoEncoder::WaitForEvent(EncoderEvent event, size_t times)
{
    ...
    while (true) {
        ...
        if (time_waiting >= event_timeout_) {
            LOG(ERROR) << "Timeout while waiting for '" << EventName(event) << "' event";
            return false;
        }
...
bool VideoEncoder::NotifyEvent(EncoderEvent event)
{
    ...
    if (encode_until_.first == event && encode_until_.second == video_encoder_event_counts_[event]) {
        video_encoder_state_ = EncoderState::kIdle;
        encode_until_ = kInvalidEncodeUntil;
