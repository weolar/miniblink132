...
private:
    ...
    bool CreateEncoderClient(const VideoEncoderClientConfig& config, std::vector<std::unique_ptr<BitstreamProcessor>> bitstream_processors);
    ...
    size_t video_encoder_event_counts_[EncoderEvent::kNumEvents] GUARDED_BY(event_lock_);
