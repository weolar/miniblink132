...
std::optional<VideoCodecProfile> ConvertStringtoProfile(const std::string& profile)
{
...
std::unique_ptr<base::MemoryMappedFile> LoadData(const base::FilePath& data_file_path)
{
    auto memory_mapped_file = std::make_unique<base::MemoryMappedFile>();
    if (!memory_mapped_file->Initialize(data_file_path, base::MemoryMappedFile::READ_ONLY)) {
...
// |metadata.|
// static
bool VideoBitstream::LoadMetadata(const base::FilePath& json_file_path, Metadata& metadata)
{
    ...
    metadata.resolution = gfx::Size(static_cast<uint32_t>(*width), static_cast<uint32_t>(*height));

    const base::Value::List* md5_checksums = metadata_dict.FindList("md5_checksums");
...
VideoBitstream::VideoBitstream(std::unique_ptr<base::MemoryMappedFile> memory_mapped_file, const Metadata& metadata)
...
VideoBitstream::Metadata& VideoBitstream::Metadata::operator=(const Metadata&) = default;

std::unique_ptr<VideoBitstream> VideoBitstream::Create(const base::FilePath& file_path, const base::FilePath& metadata_file_path)
{
    ...
    metadata.has_keyframeless_resolution_change = std::find_if(std::cbegin(kKeyFrameLessResolutionChangeFiles), std::cend(kKeyFrameLessResolutionChangeFiles),
        [filepath = data_file_path.value()](
            base::FilePath substr) { return base::Contains(base::ToLowerASCII(filepath), base::ToLowerASCII(substr.value())); });
    return base::WrapUnique(new VideoBitstream(std::move(memory_mapped_file), metadata));
}

base::span<const uint8_t> VideoBitstream::Data() const
{
    CHECK(memory_mapped_file_ && memory_mapped_file_->IsValid());
    return base::span<const uint8_t>(memory_mapped_file_->data(), memory_mapped_file_->length());
}
// static
...
// static
base::FilePath VideoBitstream::ResolveFilePath(const base::FilePath& file_path)
{
