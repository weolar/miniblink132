...
Video::Video(const base::FilePath& file_path, const base::FilePath& metadata_file_path)
    : file_path_(file_path)
    , metadata_file_path_(metadata_file_path)
{
}

Video::~Video() = default;

std::unique_ptr<Video> Video::Expand(const gfx::Size& resolution, const gfx::Rect& visible_rect) const
{
    LOG_ASSERT(IsLoaded()) << "The source video is not loaded";
    LOG_ASSERT(pixel_format_ == VideoPixelFormat::PIXEL_FORMAT_NV12) << "The pixel format of source video is not NV12";
    LOG_ASSERT(visible_rect.size() == resolution_) << "The resolution is different from the copied-into area of visible "
                                                   << "rectangle";
    ...
    const auto src_layout = CreateVideoFrameLayout(PIXEL_FORMAT_NV12, resolution_, 1u /* alignment*/);
    const auto dst_layout = CreateVideoFrameLayout(PIXEL_FORMAT_NV12, resolution, 1u /* alignment*/);
    const size_t src_frame_size = src_layout->planes().back().offset + src_layout->planes().back().size;
    const size_t dst_frame_size = dst_layout->planes().back().offset + dst_layout->planes().back().size;
    LOG_ASSERT(src_frame_size * num_frames_ == data_.size()) << "Unexpected data size";
    ...
    for (size_t i = 0; i < num_frames_; i++) {
        ...
        uint8_t* const dst_y_plane_visible_data = dst_plane + dst_layout->planes()[0].offset + dst_y_visible_offset;
        uint8_t* const dst_uv_plane_visible_data = dst_plane + dst_layout->planes()[1].offset + dst_uv_visible_offset;
        ...
        libyuv::NV12Scale(src_plane + src_layout->planes()[0].offset, src_layout->planes()[0].stride, src_plane + src_layout->planes()[1].offset,
            src_layout->planes()[1].stride, resolution_.width(), resolution_.height(), dst_y_plane_visible_data, dst_layout->planes()[0].stride,
            dst_uv_plane_visible_data, dst_layout->planes()[1].stride, visible_rect_.width(), visible_rect_.height(), libyuv::FilterMode::kFilterBilinear);
    }
...
{
    LOG_ASSERT(IsLoaded()) << "The source video is not loaded";
    LOG_ASSERT(pixel_format_ == VideoPixelFormat::PIXEL_FORMAT_I420) << "The pixel format of source video is not I420";
    auto new_video = std::make_unique<Video>(file_path_, metadata_file_path_);
    new_video->frame_checksums_ = frame_checksums_;
    new_video->profile_ = profile_;
    new_video->codec_ = codec_;
    ...
    const auto i420_layout = CreateVideoFrameLayout(PIXEL_FORMAT_I420, resolution_, 1u /* alignment */);
    const auto nv12_layout = CreateVideoFrameLayout(PIXEL_FORMAT_NV12, resolution_, 1u /* alignment*/);
    LOG_ASSERT(i420_layout && nv12_layout) << "Failed creating VideoFrameLayout";
    const size_t i420_frame_size = i420_layout->planes().back().offset + i420_layout->planes().back().size;
    const size_t nv12_frame_size = nv12_layout->planes().back().offset + nv12_layout->planes().back().size;
    LOG_ASSERT(i420_frame_size * num_frames_ == data_.size()) << "Unexpected data size";
    ...
    for (size_t i = 0; i < num_frames_; i++) {
        ...
        libyuv::I420ToNV12(src_plane + i420_layout->planes()[0].offset, i420_layout->planes()[0].stride, src_plane + i420_layout->planes()[1].offset,
            i420_layout->planes()[1].stride, src_plane + i420_layout->planes()[2].offset, i420_layout->planes()[2].stride,
            dst_plane + nv12_layout->planes()[0].offset, nv12_layout->planes()[0].stride, dst_plane + nv12_layout->planes()[1].offset,
            nv12_layout->planes()[1].stride, resolution_.width(), resolution_.height());
    }
    new_video->data_ = std::move(new_data);
    return new_video;
}
...
bool Video::Load(const size_t max_frames)
{
    ...
    if (base::ReadFile(file_path_, reinterpret_cast<char*>(data.data()), base::checked_cast<int>(file_size)) != file_size) {
    ...
    for (size_t i = 0; i < VideoFrame::NumPlanes(pixel_format_); ++i) {
        video_frame_size += VideoFrame::RowBytes(i, pixel_format_, resolution_.width()) * VideoFrame::Rows(i, pixel_format_, resolution_.height());
    }
    if (video_frame_size * num_frames_ != static_cast<size_t>(file_size)) {
        LOG(ERROR) << "Invalid file. file_size=" << file_size << ", expected file size=" << video_frame_size * num_frames_
                   << ", video_frame_size=" << video_frame_size << ", num_frames_=" << num_frames_;
    }
    num_frames_ = max_frames;
...
bool Video::Decode()
{
    ...
    decode_thread.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&Video::DecodeTask, std::move(data_), resolution_, num_frames_, &decompressed_data, &success, &done));
    done.Wait();
    decode_thread.Stop();
...
{
    return base::Seconds(static_cast<double>(num_frames_) / static_cast<double>(frame_rate_));
}
...
bool Video::LoadMetadata()
{
    ...
    absl::optional<base::FilePath> resolved_path = ResolveFilePath(metadata_file_path_);
    if (!resolved_path) {
    ...
    if (!metadata_result.has_value()) {
        LOG(ERROR) << "Failed to parse video metadata: " << metadata_file_path_ << ": " << metadata_result.error().message;
        return false;
    }
    ...
    profile_ = VIDEO_CODEC_PROFILE_UNKNOWN;
    const base::Value* profile = metadata.FindKeyOfType("profile", base::Value::Type::STRING);
    if (profile) {
    ...
    // video streams.
    const base::Value* bit_depth = metadata.FindKeyOfType("bit_depth", base::Value::Type::INTEGER);
    if (bit_depth) {
    ...
    const base::Value* pixel_format = metadata.FindKeyOfType("pixel_format", base::Value::Type::STRING);
    if (pixel_format) {
        auto converted_pixel_format = ConvertStringtoPixelFormat(pixel_format->GetString());
    ...
    if (profile_ == VIDEO_CODEC_PROFILE_UNKNOWN && pixel_format_ == VideoPixelFormat::PIXEL_FORMAT_UNKNOWN) {
    ...
    const base::Value* frame_rate = metadata.FindKeyOfType("frame_rate", base::Value::Type::INTEGER);
    if (!frame_rate) {
    ...
    const base::Value* num_frames = metadata.FindKeyOfType("num_frames", base::Value::Type::INTEGER);
    if (!num_frames) {
    ...
    if ((profile_ >= H264PROFILE_MIN && profile_ <= H264PROFILE_MAX) || (profile_ >= HEVCPROFILE_MIN && profile_ <= HEVCPROFILE_MAX)) {
        const base::Value* num_fragments = metadata.FindKeyOfType("num_fragments", base::Value::Type::INTEGER);
        if (!num_fragments) {
    ...
    const base::Value* width = metadata.FindKeyOfType("width", base::Value::Type::INTEGER);
    if (!width) {
        LOG(ERROR) << "Key \"width\" is not found in " << metadata_file_path_;
        return false;
    }
    const base::Value* height = metadata.FindKeyOfType("height", base::Value::Type::INTEGER);
    if (!height) {
        LOG(ERROR) << "Key \"height\" is not found in " << metadata_file_path_;
        return false;
    }
    resolution_ = gfx::Size(static_cast<uint32_t>(width->GetInt()), static_cast<uint32_t>(height->GetInt()));
    ...
    // validator.
    const base::Value* md5_checksums = metadata.FindKeyOfType("md5_checksums", base::Value::Type::LIST);
    if (md5_checksums) {
...
absl::optional<base::FilePath> Video::ResolveFilePath(const base::FilePath& file_path)
{
    ...
    return PathExists(resolved_path) ? absl::optional<base::FilePath>(resolved_path) : absl::optional<base::FilePath>();
}

// static
void Video::DecodeTask(const std::vector<uint8_t> data, const gfx::Size& resolution, const size_t num_frames, std::vector<uint8_t>* decompressed_data,
    ...
    *success = false;
    ...
    for (size_t i = 0; i < glue.format_context()->nb_streams; ++i) {
        ...
        if (codec_type == AVMEDIA_TYPE_VIDEO && codec_id == AV_CODEC_ID_VP9) {
            *success = AVStreamToVideoDecoderConfig(stream, &config) && config.IsValidConfig();
            stream_index = i;
            break;
        }
    }
    ...
    VpxVideoDecoder decoder(media::OffloadableVideoDecoder::OffloadState::kOffloaded);
    media::VideoDecoder::InitCB init_cb = base::BindOnce([](DecoderStatus* save_to, DecoderStatus save_from) { *save_to = save_from; }, &init_result);
    decoder.Initialize(
        config, false, nullptr, std::move(init_cb), base::BindRepeating(&Video::OnFrameDecoded, resolution, decompressed_data), base::NullCallback());
    ...
    while (av_read_frame(glue.format_context(), &packet) >= 0 && num_decoded_frames < num_frames) {
        if (packet.stream_index == stream_index) {
            media::VideoDecoder::DecodeCB decode_cb = base::BindOnce([](bool* success, DecoderStatus status) { *success = (status.is_ok()); }, success);
...
// static
void Video::OnFrameDecoded(const gfx::Size& resolution, std::vector<uint8_t>* data, scoped_refptr<VideoFrame> frame)
{
    ...
    for (size_t plane = 0; plane < num_planes; ++plane) {
        ...
        const int rows = VideoFrame::Rows(plane, frame->format(), resolution.height());
        const int row_bytes = VideoFrame::RowBytes(plane, frame->format(), resolution.width());
...
// static
absl::optional<VideoCodecProfile> Video::ConvertStringtoProfile(const std::string& profile)
{
...
// static
absl::optional<VideoCodec> Video::ConvertProfileToCodec(VideoCodecProfile profile)
{
...
// static
absl::optional<VideoPixelFormat> Video::ConvertStringtoPixelFormat(const std::string& pixel_format)
{
