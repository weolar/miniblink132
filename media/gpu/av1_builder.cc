...
AV1BitstreamBuilder AV1BitstreamBuilder::BuildSequenceHeaderOBU(const SequenceHeader& seq_hdr)
{
...
AV1BitstreamBuilder AV1BitstreamBuilder::BuildFrameHeaderOBU(const SequenceHeader& seq_hdr, const FrameHeader& pic_hdr)
{
    AV1BitstreamBuilder ret;
    ...
    if (pic_hdr.segmentation_enabled) {
        ...
        for (uint32_t i = 0; i < libgav1::kMaxSegments; i++) {
            for (uint32_t j = 0; j < libgav1::kSegmentFeatureMax; j++) {
                bool feature_enabled = (i < pic_hdr.segment_number && (pic_hdr.feature_mask[i] & (1u << j)));
    ...
    for (auto queued_write : queued_writes_) {
        ...
        while (outstanding_bits) {
            if (rem_bits_in_byte >= outstanding_bits) {
                ...
                curr_byte |= (val >> (outstanding_bits - rem_bits_in_byte)) & ((1 << rem_bits_in_byte) - 1);
...
void AV1BitstreamBuilder::WriteOBUHeader(libgav1::ObuType type, bool extension_flag, bool has_size)
{
    DCHECK_LE(1, type);
    DCHECK_LE(type, 8);
...
void AV1BitstreamBuilder::WriteValueInLeb128(uint32_t value, std::optional<int> fixed_size)
{
...
{
    queued_writes_.insert(queued_writes_.end(), std::make_move_iterator(buffer.queued_writes_.begin()), std::make_move_iterator(buffer.queued_writes_.end()));
