...
public:
    ...
    HRDBuffer(size_t buffer_size, uint32_t avg_bitrate, int last_frame_buffer_bytes, base::TimeDelta last_frame_timestamp);
    ~HRDBuffer();
    ...
    bool frame_overshooting() const
    {
        return frame_overshooting_;
    }

    // Number of bytes in the buffer after the last encoded frame has been added.
    int last_frame_buffer_bytes() const
    {
        return last_frame_buffer_bytes_;
    }

    // The timestamp of the last added frame.
    base::TimeDelta last_frame_timestamp() const
    {
    ...
    void SetParameters(size_t buffer_size, uint32_t avg_bitrate, uint32_t peak_bitrate, bool ease_hrd_reduction);
