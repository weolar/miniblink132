...
namespace {
bool GetSpatialLayerFrameSize(const DecoderBuffer& decoder_buffer, std::vector<uint32_t>& frame_sizes)
{
    frame_sizes.clear();

    if (!decoder_buffer.has_side_data() || decoder_buffer.side_data()->spatial_layers.empty()) {
        return true;
    }
...
scoped_refptr<VP9Picture> VP9Decoder::VP9Accelerator::CreateVP9PictureSecure(uint64_t secure_handle)
{
    return nullptr;
}

VP9Decoder::VP9Decoder(std::unique_ptr<VP9Accelerator> accelerator, VideoCodecProfile profile, const VideoColorSpace& container_color_space)
...
void VP9Decoder::SetStream(int32_t id, const DecoderBuffer& decoder_buffer)
{
    ...
    parser_.SetStream(ptr, size, frame_sizes, decrypt_config ? decrypt_config->Clone() : nullptr);
}
...
VP9Decoder::DecodeResult VP9Decoder::Decode()
{
    while (true) {
        ...
        if (pending_pic_) {
            VP9Accelerator::Status status = DecodeAndOutputPicture(std::move(pending_pic_));
        ...
        if (!curr_frame_hdr_) {
            ...
            Vp9Parser::Result res = parser_.ParseNextFrame(hdr.get(), &allocate_size, &decrypt_config_);
            switch (res) {
            case Vp9Parser::kOk:
        ...
        if (state_ != kDecoding) {
            ...
            // frame.
            if (curr_frame_hdr_->IsKeyframe() || (curr_frame_hdr_->IsIntra() && pic_size_.IsEmpty())) {
                state_ = kDecoding;
            } else {
        ...
        if (curr_frame_hdr_->show_existing_frame) {
            ...
            if (frame_to_show >= kVp9NumRefFrames || !ref_frames_.GetFrame(frame_to_show)) {
            ...
            // correct timestamp.
            scoped_refptr<VP9Picture> pic = ref_frames_.GetFrame(frame_to_show)->Duplicate();
        ...
        if (!gfx::Rect(new_pic_size).Contains(new_render_rect)) {
            DVLOG(1) << "Render size exceeds picture size. render size: " << new_render_rect.ToString() << ", picture size: " << new_pic_size.ToString();
            new_render_rect = gfx::Rect(new_pic_size);
        }
        VideoCodecProfile new_profile = VP9ProfileToVideoCodecProfile(curr_frame_hdr_->profile);
        ...
        if (!IsValidBitDepth(curr_frame_hdr_->bit_depth, new_profile)) {
            DVLOG(1) << "Invalid bit depth=" << base::strict_cast<int>(curr_frame_hdr_->bit_depth) << ", profile=" << GetProfileName(new_profile);
            return kDecodeError;
        }
        VideoChromaSampling new_chroma_sampling = GetVP9ChromaSampling(*curr_frame_hdr_);
        ...
        const bool is_pic_size_larger = new_pic_size.width() > pic_size_.width() || new_pic_size.height() > pic_size_.height();
        const bool is_new_configuration_different_enough = (ignore_resolution_changes_to_smaller_for_testing_ ? is_pic_size_larger : is_pic_size_different)
            || new_profile != profile_ || curr_frame_hdr_->bit_depth != bit_depth_ || is_color_space_change;

        if (is_new_configuration_different_enough) {
            DVLOG(1) << "New profile: " << GetProfileName(new_profile) << ", new resolution: " << new_pic_size.ToString()
                     << ", new bit depth: " << base::strict_cast<int>(curr_frame_hdr_->bit_depth) << ", new color space: " << new_color_space.ToString();
...
VP9Decoder::VP9Accelerator::Status VP9Decoder::DecodeAndOutputPicture(scoped_refptr<VP9Picture> pic)
{
    DCHECK(!pic_size_.IsEmpty());
    DCHECK(pic->frame_hdr);

    const Vp9Parser::Context& context = parser_.context();
    VP9Accelerator::Status status = accelerator_->SubmitDecode(pic, context.segmentation(), context.loop_filter(), ref_frames_);
