...
VP8Decoder::VP8Decoder(std::unique_ptr<VP8Accelerator> accelerator, const VideoColorSpace& container_color_space)
...
VP8Decoder::DecodeResult VP8Decoder::Decode()
{
    ...
    if (!curr_frame_hdr_) {
        ...
        if (!parser_.ParseFrame(curr_frame_start_, frame_size_, curr_frame_hdr_.get())) {
    ...
    const bool have_skipped_frame = last_decoded_stream_id_ + 1 != stream_id_ && last_decoded_stream_id_ != kInvalidId;
    if (curr_frame_hdr_->IsKeyframe()) {
        const gfx::Size new_picture_size(curr_frame_hdr_->width, curr_frame_hdr_->height);
        ...
        if (have_skipped_frame && ++size_change_failure_counter_ > kVPxMaxNumOfSizeChangeFailures) {
            state_ = kError;
            return kDecodeError;
        }
