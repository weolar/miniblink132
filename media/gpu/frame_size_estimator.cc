...
namespace media {
namespace {
void CalculateQSteps(uint32_t qp, uint32_t qp_prev, float& q_step, float& q_step_prev, float& delta_q_step_factor)
{
...
FrameSizeEstimator::FrameSizeEstimator(base::TimeDelta max_window_size, float initial_qp_size, float initial_size_correction)
    : qp_size_stats_(max_window_size)
    , size_correction_stats_(max_window_size)
{
    ...
    size_correction_stats_.AddValue(initial_size_correction, base::Milliseconds(1));
}
...
size_t FrameSizeEstimator::Estimate(uint32_t qp, uint32_t qp_prev) const
{
    ...
    float pred_frame_byte = qp_size_stats_.mean() * delta_q_step_factor / q_step + size_correction_stats_.mean();
    return static_cast<size_t>(std::max(pred_frame_byte, 0.0f));
}

uint32_t FrameSizeEstimator::InverseEstimate(size_t target_frame_bytes, uint32_t qp_prev) const
{
    ...
    frame_bytes = std::max(frame_bytes, target_frame_bytes * kMinTargetBytesMultiplier);
...
void FrameSizeEstimator::Update(size_t frame_bytes, uint32_t qp, uint32_t qp_prev, base::TimeDelta elapsed_time)
{
    float q_step, q_step_prev, delta_q_step_factor;
    CalculateQSteps(qp, qp_prev, q_step, q_step_prev, delta_q_step_factor);
    ...
    float corr = frame_bytes - qp_size_stats_.mean() * delta_q_step_factor / q_step;
