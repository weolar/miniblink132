...
class MEDIA_GPU_EXPORT CodecOutputBufferRenderer : public gpu::RefCountedLockHelperDrDc {
public:
    CodecOutputBufferRenderer(std::unique_ptr<CodecOutputBuffer> output_buffer, scoped_refptr<CodecBufferWaitCoordinator> codec_buffer_wait_coordinator,
        scoped_refptr<gpu::RefCountedLock> drdc_lock);
    ~CodecOutputBufferRenderer();

    CodecOutputBufferRenderer(const CodecOutputBufferRenderer&) = delete;
    CodecOutputBufferRenderer& operator=(const CodecOutputBufferRenderer&) = delete;
    ...
    using FrameInfoCallback = base::OnceCallback<void(std::optional<gfx::Size> coded_size, std::optional<gfx::Rect> visible_rect)>;
    ...
    {
        return output_buffer_->CanGuessCodedSize();
    }
    gfx::Size GuessCodedSize() const
    {
        return output_buffer_->GuessCodedSize();
    }
    ...
    {
        return codec_buffer_wait_coordinator_ ? codec_buffer_wait_coordinator_->texture_owner() : nullptr;
    }
