...
class MEDIA_GPU_EXPORT AndroidVideoSurfaceChooserImpl : public AndroidVideoSurfaceChooser {
public:
    ...
    AndroidVideoSurfaceChooserImpl(bool allow_dynamic, const base::TickClock* tick_clock = nullptr);

    AndroidVideoSurfaceChooserImpl(const AndroidVideoSurfaceChooserImpl&) = delete;
    AndroidVideoSurfaceChooserImpl& operator=(const AndroidVideoSurfaceChooserImpl&) = delete;

    ~AndroidVideoSurfaceChooserImpl() override;

    // AndroidVideoSurfaceChooser
    void SetClientCallbacks(UseOverlayCB use_overlay_cb, UseTextureOwnerCB use_texture_owner_cb) override;
    void UpdateState(std::optional<AndroidOverlayFactoryCB> new_factory, const State& new_state) override;
