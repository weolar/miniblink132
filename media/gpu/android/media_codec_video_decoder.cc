...
{
    return info.status_values[gpu::GPU_FEATURE_TYPE_ANDROID_SURFACE_CONTROL] == gpu::kGpuFeatureStatusEnabled;
}

std::vector<SupportedVideoDecoderConfig> GenerateSupportedConfigs(DeviceInfo* device_info, bool allow_media_codec_sw_decoder)
{
    ...
    for (const auto& info : GetDecoderInfoCache()) {
        ...
        if ((codec == VideoCodec::kVP8 && device_info->IsVp8DecoderAvailable()) || (codec == VideoCodec::kVP9 && device_info->IsVp9DecoderAvailable())
            || (codec == VideoCodec::kAV1 && device_info->IsAv1DecoderAvailable()) || (codec == VideoCodec::kH264 && IsDecoderBuiltInVideoCodec(codec))) {
            // Don't allow OS software decoding for bundled software decoders unless
            // the content is encrypted.
            const bool can_use_builtin_software_decoder
                = info.profile != VP9PROFILE_PROFILE2 && info.profile != VP9PROFILE_PROFILE3 && info.secure_codec_capability == SecureCodecCapability::kClear;
            const bool is_os_software_decoder_allowed = !can_use_builtin_software_decoder || allow_media_codec_sw_decoder;
            if (info.is_software_codec && !is_os_software_decoder_allowed) {
                supported_configs.emplace_back(info.profile, info.profile, info.coded_size_min, info.coded_size_max, kAllowEncrypted,
                    /*require_encrypted=*/true);
                continue;
            }

            supported_configs.emplace_back(info.profile, info.profile, info.coded_size_min, info.coded_size_max, kAllowEncrypted,
                /*require_encrypted=*/info.secure_codec_capability == SecureCodecCapability::kEncrypted);
            continue;
        }
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
        if (codec == VideoCodec::kH264
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
            || (codec == VideoCodec::kHEVC && base::FeatureList::IsEnabled(kPlatformHEVCDecoderSupport))
...
void SelectMediaCodec(const VideoDecoderConfig& config, bool requires_secure_codec, std::string* out_codec_name)
{
    *out_codec_name = "";
    ...
    for (const auto& info : GetDecoderInfoCache()) {
        ...
        if (config.profile() != VIDEO_CODEC_PROFILE_UNKNOWN && config.profile() != info.profile) {
            continue;
        }

        if (config.level() != kNoVideoCodecLevel && info.level != kNoVideoCodecLevel && config.level() > info.level) {
            continue;
        }

        if (config.coded_size().width() < info.coded_size_min.width() || config.coded_size().height() < info.coded_size_min.height()
            || config.coded_size().width() > info.coded_size_max.width() || config.coded_size().height() > info.coded_size_max.height()) {
            continue;
        }

        if (!requires_secure_codec && info.secure_codec_capability == SecureCodecCapability::kEncrypted) {
            continue;
        }

        if (requires_secure_codec && info.secure_codec_capability == SecureCodecCapability::kClear) {
            continue;
        }
...
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
            || (config.codec() == VideoCodec::kHEVC && base::FeatureList::IsEnabled(kPlatformHEVCDecoderSupport))
#endif // BUILDFLAG(ENABLE_PLATFORM_HEVC)
#if BUILDFLAG(ENABLE_PLATFORM_DOLBY_VISION)
    ...
    // TODO(b/376720494): Allow this only libvpx doesn't supports HRD.
            || config.profile() == VP9PROFILE_PROFILE2 || config.profile() == VP9PROFILE_PROFILE3)) {
...
PendingDecode::PendingDecode(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb)
    : buffer(std::move(buffer))
    , decode_cb(std::move(decode_cb))
{
}
...
// static
std::vector<SupportedVideoDecoderConfig> MediaCodecVideoDecoder::GetSupportedConfigs()
{
    static const auto configs = GenerateSupportedConfigs(DeviceInfo::GetInstance(), base::FeatureList::IsEnabled(media::kAllowMediaCodecSoftwareDecoder));
    return configs;
}

MediaCodecVideoDecoder::MediaCodecVideoDecoder(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuFeatureInfo& gpu_feature_info,
    std::unique_ptr<MediaLog> media_log, DeviceInfo* device_info, CodecAllocator* codec_allocator, std::unique_ptr<AndroidVideoSurfaceChooser> surface_chooser,
    AndroidOverlayMojoFactoryCB overlay_factory_cb, RequestOverlayInfoCB request_overlay_info_cb, std::unique_ptr<VideoFrameFactory> video_frame_factory,
    ...
    , surface_chooser_helper_(std::move(surface_chooser), base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceVideoOverlays),
          base::FeatureList::IsEnabled(media::kUseAndroidOverlayForSecureOnly), is_surface_control_enabled_)
    , video_frame_factory_(std::move(video_frame_factory))
    , overlay_factory_cb_(std::move(overlay_factory_cb))
    , device_info_(device_info)
    , enable_threaded_texture_mailboxes_(gpu_preferences.enable_threaded_texture_mailboxes)
    , allow_nonsecure_overlays_(base::FeatureList::IsEnabled(media::kAllowNonSecureOverlays))
    , use_block_model_(base::FeatureList::IsEnabled(kMediaCodecBlockModel) && device_info_->SdkVersion() >= base::android::SDK_VERSION_R)
{
    DVLOG(2) << __func__;
    surface_chooser_helper_.chooser()->SetClientCallbacks(base::BindRepeating(&MediaCodecVideoDecoder::OnSurfaceChosen, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&MediaCodecVideoDecoder::OnSurfaceChosen, weak_factory_.GetWeakPtr(), nullptr));
}

std::unique_ptr<VideoDecoder> MediaCodecVideoDecoder::Create(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuFeatureInfo& gpu_feature_info,
    std::unique_ptr<MediaLog> media_log, DeviceInfo* device_info, CodecAllocator* codec_allocator, std::unique_ptr<AndroidVideoSurfaceChooser> surface_chooser,
    AndroidOverlayMojoFactoryCB overlay_factory_cb, RequestOverlayInfoCB request_overlay_info_cb, std::unique_ptr<VideoFrameFactory> video_frame_factory,
    scoped_refptr<gpu::RefCountedLock> drdc_lock)
{
    auto* decoder = new MediaCodecVideoDecoder(gpu_preferences, gpu_feature_info, std::move(media_log), device_info, codec_allocator,
        std::move(surface_chooser), std::move(overlay_factory_cb), std::move(request_overlay_info_cb), std::move(video_frame_factory), std::move(drdc_lock));
    return std::make_unique<AsyncDestroyVideoDecoder<MediaCodecVideoDecoder>>(base::WrapUnique(decoder));
}
...
void MediaCodecVideoDecoder::DestroyAsync(std::unique_ptr<MediaCodecVideoDecoder> decoder)
{
    DVLOG(1) << __func__;
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    DVLOG(1) << (first_init ? "Initializing" : "Reinitializing") << " MCVD with config: " << config.AsHumanReadableString()
             << ", cdm_context = " << cdm_context;

    if (!config.IsValidConfig()) {
        MEDIA_LOG(INFO, media_log_) << "Video configuration is not valid: " << config.AsHumanReadableString();
        DVLOG(1) << "Invalid configuration.";
        base::BindPostTaskToCurrentDefault(std::move(init_cb)).Run(DecoderStatus::Codes::kUnsupportedConfig);
        return;
    }

    // If we don't have support support for a given codec, try to initialize
    // anyways -- otherwise we're certain to fail playback.
    if (!IsVideoDecoderConfigSupported(GetSupportedConfigsInternal(), config) && IsDecoderBuiltInVideoCodec(config.codec())) {
        MEDIA_LOG(INFO, media_log_) << "Video configuration is not valid: " << config.AsHumanReadableString();
        base::BindPostTaskToCurrentDefault(std::move(init_cb)).Run(DecoderStatus::Codes::kUnsupportedConfig);
        return;
    }
    ...
    if (!first_init && decoder_config_.codec() != config.codec()) {
        ...
        MEDIA_LOG(INFO, media_log_) << "Cannot change codec during re-init: " << decoder_config_.AsHumanReadableString() << " -> "
                                    << config.AsHumanReadableString();
        base::BindPostTaskToCurrentDefault(std::move(init_cb)).Run(DecoderStatus::Codes::kCantChangeCodec);
        return;
    }
    const auto old_size = decoder_config_.coded_size();
    decoder_config_ = config;

    surface_chooser_helper_.SetVideoRotation(decoder_config_.video_transformation().rotation);
    ...
    if (config.is_encrypted() && media_crypto_.is_null()) {
        ...
        base::BindPostTaskToCurrentDefault(std::move(init_cb)).Run(DecoderStatus::Codes::kUnsupportedEncryptionMode);
        return;
    }
    ...
    if (first_init) {
        ...
        MEDIA_LOG(INFO, media_log_) << "Queuing deferred codec reallocation for resolution change from " << old_size.ToString() << " to "
        ...
        MEDIA_LOG(INFO, media_log_) << "Reusing codec without reallocation for resolution change from " << old_size.ToString() << " to "
                                    << decoder_config_.coded_size().ToString();
    }
}
...
void MediaCodecVideoDecoder::SetCdm(CdmContext* cdm_context, InitCB init_cb)
{
    ...
    event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&MediaCodecVideoDecoder::OnCdmContextEvent, weak_factory_.GetWeakPtr()));

    // The callback will be posted back to this thread via
    // base::BindPostTaskToCurrentDefault.
    media_crypto_context_->SetMediaCryptoReadyCB(
        base::BindPostTaskToCurrentDefault(base::BindOnce(&MediaCodecVideoDecoder::OnMediaCryptoReady, weak_factory_.GetWeakPtr(), std::move(init_cb))));
}

void MediaCodecVideoDecoder::OnMediaCryptoReady(InitCB init_cb, JavaObjectPtr media_crypto, bool requires_secure_video_codec)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(1) << __func__ << ": requires_secure_video_codec = " << requires_secure_video_codec;
    ...
    if (media_crypto->is_null()) {
        ...
        if (decoder_config_.is_encrypted()) {
            ...
            EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "MediaCrypto is not available" });
    ...
    surface_chooser_helper_.SetSecureSurfaceMode(
        requires_secure_video_codec ? SurfaceChooserHelper::SecureSurfaceMode::kRequired : SurfaceChooserHelper::SecureSurfaceMode::kRequested);
...
void MediaCodecVideoDecoder::StartLazyInit()
{
    ...
    const bool want_promotion_hints = device_info_->IsSetOutputSurfaceSupported() && !enable_threaded_texture_mailboxes_;

    VideoFrameFactory::OverlayMode overlay_mode = VideoFrameFactory::OverlayMode::kDontRequestPromotionHints;
    if (is_surface_control_enabled_) {
        overlay_mode = requires_secure_codec_ ? VideoFrameFactory::OverlayMode::kSurfaceControlSecure : VideoFrameFactory::OverlayMode::kSurfaceControlInsecure;
    ...
    video_frame_factory_->Initialize(overlay_mode, base::BindRepeating(&MediaCodecVideoDecoder::OnVideoFrameFactoryInitialized, weak_factory_.GetWeakPtr()));
}

void MediaCodecVideoDecoder::OnVideoFrameFactoryInitialized(scoped_refptr<gpu::TextureOwner> texture_owner)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(2) << __func__;
    TRACE_EVENT0("media", "MediaCodecVideoDecoder::OnVideoFrameFactoryInitialized");
    if (!texture_owner) {
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "Could not allocated TextureOwner" });
        return;
    }
    texture_owner_bundle_ = new CodecSurfaceBundle(std::move(texture_owner), GetDrDcLock());
    ...
    const bool allowed_for_experiment = requires_secure_codec_ || allow_nonsecure_overlays_;

    // Overlays are disabled when |enable_threaded_texture_mailboxes| is true
    // (http://crbug.com/582170).
    if (enable_threaded_texture_mailboxes_ || !device_info_->SupportsOverlaySurfaces() || !allowed_for_experiment) {
    ...
    std::move(request_overlay_info_cb_)
        .Run(restart_for_transitions, base::BindRepeating(&MediaCodecVideoDecoder::OnOverlayInfoChanged, weak_factory_.GetWeakPtr()));
}

void MediaCodecVideoDecoder::OnOverlayInfoChanged(const OverlayInfo& overlay_info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(2) << __func__;
    ...
    surface_chooser_helper_.SetIsPersistentVideo(overlay_info_.is_persistent_video);
    surface_chooser_helper_.UpdateChooserState(overlay_changed ? std::make_optional(CreateOverlayFactoryCb()) : std::nullopt);
}

void MediaCodecVideoDecoder::OnSurfaceChosen(std::unique_ptr<AndroidOverlay> overlay)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(2) << __func__;
    DCHECK(state_ == State::kInitializing || device_info_->IsSetOutputSurfaceSupported());
    TRACE_EVENT1("media", "MediaCodecVideoDecoder::OnSurfaceChosen", "overlay", overlay ? "yes" : "no");

    if (overlay) {
        overlay->AddSurfaceDestroyedCallback(base::BindOnce(&MediaCodecVideoDecoder::OnSurfaceDestroyed, weak_factory_.GetWeakPtr()));
...
void MediaCodecVideoDecoder::OnSurfaceDestroyed(AndroidOverlay* overlay)
{
    ...
    if (!device_info_->IsSetOutputSurfaceSupported()) {
        EnterTerminalState(State::kSurfaceDestroyed, { DecoderStatus::Codes::kFailed, "Surface destroyed" });
        return;
    }
    ...
    if (requires_secure_codec_ && target_surface_bundle_ == texture_owner_bundle_) {
...
void MediaCodecVideoDecoder::TransitionToTargetSurface()
{
    ...
    if (!codec_->SetSurface(target_surface_bundle_)) {
        video_frame_factory_->SetSurfaceBundle(nullptr);
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "Could not switch codec output surface" });
        return;
    }
...
void MediaCodecVideoDecoder::CreateCodec()
{
    ...
    config->on_buffers_available_cb = base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaCodecVideoDecoder::PumpCodec, weak_factory_.GetWeakPtr()));
    ...
    video_frame_factory_->SetSurfaceBundle(target_surface_bundle_);
    codec_allocator_->CreateMediaCodecAsync(base::BindOnce(&MediaCodecVideoDecoder::OnCodecConfiguredInternal, codec_allocator_weak_factory_.GetWeakPtr(),
                                                codec_allocator_, target_surface_bundle_),
        std::move(config));
}

// static
void MediaCodecVideoDecoder::OnCodecConfiguredInternal(base::WeakPtr<MediaCodecVideoDecoder> weak_this, CodecAllocator* codec_allocator,
    scoped_refptr<CodecSurfaceBundle> surface_bundle, std::unique_ptr<MediaCodecBridge> codec)
{
    if (!weak_this) {
        if (codec) {
            codec_allocator->ReleaseMediaCodec(std::move(codec),
                base::BindOnce(&base::SequencedTaskRunner::ReleaseSoon<CodecSurfaceBundle>, base::SequencedTaskRunner::GetCurrentDefault(), FROM_HERE,
...
void MediaCodecVideoDecoder::OnCodecConfigured(scoped_refptr<CodecSurfaceBundle> surface_bundle, std::unique_ptr<MediaCodecBridge> codec)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!codec_);
    DCHECK_EQ(state_, State::kRunning);
    ...
    if (!codec && should_retry_codec_allocation && device_info_->SdkVersion() >= base::android::SDK_VERSION_R
    ...
    if (!codec) {
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "Unable to allocate codec" });
        return;
    }

    codec_name_ = codec->GetName();
    MEDIA_LOG(INFO, media_log_) << "Created MediaCodec " << codec_name_ << ", is_software_codec=" << codec->IsSoftwareCodec();
    ...
    if (base::FeatureList::IsEnabled(kMediaCodecCodedSizeGuessing)) {
        coded_size_alignment = MediaCodecUtil::LookupCodedSizeAlignment(codec_name_);
        if (coded_size_alignment) {
            MEDIA_LOG(INFO, media_log_) << "Using a coded size alignment of " << coded_size_alignment->ToString();
        } else {
            // TODO(crbug.com/40917948): If the known cases work well, we can try
            // guessing generically since we get a glitch either way.
            MEDIA_LOG(WARNING, media_log_) << "Unable to lookup coded size alignment for codec " << codec_name_;
        }
    }

    max_input_size_ = codec->GetMaxInputSize();
    codec_ = std::make_unique<CodecWrapper>(CodecSurfacePair(std::move(codec), std::move(surface_bundle)),
        base::BindRepeating(
            &OutputBufferReleased, base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaCodecVideoDecoder::PumpCodec, weak_factory_.GetWeakPtr()))),
        decoder_config_.coded_size(), decoder_config_.color_space_info().ToGfxColorSpace(), coded_size_alignment, use_block_model_);
...
void MediaCodecVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MediaCodecVideoDecoder::FlushCodec()
{
    ...
    if (!codec_->Flush())
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "Codec flush failed" });
}
...
bool MediaCodecVideoDecoder::QueueInput()
{
    ...
    if (!use_block_model_ && !pending_buffer->end_of_stream() && pending_buffer->is_key_frame() && pending_buffer->size() > max_input_size_) {
        ...
        if (decoder_config_.coded_size().width() == last_width_) {
            ...
            const size_t compression_ratio = (decoder_config_.codec() == VideoCodec::kH264 || decoder_config_.codec() == VideoCodec::kVP8) ? 2 : 4;
            const size_t max_pixels = (pending_buffer->size() * compression_ratio * 2) / 3;
    ...
    if (base::FeatureList::IsEnabled(kMediaCodecElideEOS) && pending_buffer->end_of_stream() && pending_buffer->next_config()) {
        const auto new_config = absl::get<VideoDecoderConfig>(*pending_buffer->next_config());
        ...
        const bool can_reuse_codec = [&]() {
            ...
            return !codec_name_.empty() && codec_name == codec_name_ && !CodecNeedsReallocation(new_config.coded_size().width());
        }();

        if (can_reuse_codec) {
            MEDIA_LOG(INFO, media_log_) << "Eliding EOS buffer and flush for resolution change from " << decoder_config_.coded_size().ToString() << " to "
                                        << new_config.coded_size().ToString();
            std::move(pending_decodes_.front().decode_cb).Run(DecoderStatus::Codes::kElidedEndOfStreamForConfigChange);
    ...
    DVLOG((status.code() == CodecWrapper::QueueStatus::Codes::kTryAgainLater || status.is_ok() ? 3 : 2))
        << "QueueInput(" << pending_buffer->AsHumanReadableString() << ") status=" << status.message();
    ...
    case CodecWrapper::QueueStatus::Codes::kError:
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "QueueInputBuffer failed", std::move(status) });
        return false;
    }
    ...
    if (pending_buffer->end_of_stream()) {
        ...
        std::move(pending_decodes_.front().decode_cb).Run(DecoderStatus::Codes::kOk);
    }
    pending_decodes_.pop_front();
    return true;
}
...
bool MediaCodecVideoDecoder::DequeueOutput()
{
    ...
    auto status = codec_->DequeueOutputBuffer(&presentation_time, &eos, &output_buffer);
    switch (status.code()) {
    ...
    case CodecWrapper::DequeueStatus::Codes::kError:
        ...
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "DequeueOutputBuffer failed", std::move(status) });
        return false;
    }
    DVLOG(3) << "DequeueOutputBuffer(): pts=" << (eos ? "EOS" : base::NumberToString(presentation_time.InMilliseconds()));

    if (eos) {
        if (eos_decode_cb_) {
            // Schedule the EOS DecodeCB to run after all previous frames.
            video_frame_factory_->RunAfterPendingVideoFrames(
                base::BindOnce(&MediaCodecVideoDecoder::RunEosDecodeCb, weak_factory_.GetWeakPtr(), reset_generation_));
        }
        if (drain_type_)
            OnCodecDrained();
    ...
    std::unique_ptr<ScopedAsyncTrace> async_trace = ScopedAsyncTrace::CreateIfEnabled("MediaCodecVideoDecoder::CreateVideoFrame");
    ...
    // transition anyway.
    if (!is_surface_control_enabled_) {
        output_buffer->set_render_cb(base::BindPostTaskToCurrentDefault(base::BindOnce(&MediaCodecVideoDecoder::PumpCodec, weak_factory_.GetWeakPtr())));
    }
    video_frame_factory_->CreateVideoFrame(std::move(output_buffer), presentation_time, decoder_config_.aspect_ratio().GetNaturalSize(visible_rect),
        CreatePromotionHintCB(),
        base::BindOnce(
            &MediaCodecVideoDecoder::ForwardVideoFrame, weak_factory_.GetWeakPtr(), reset_generation_, std::move(async_trace), base::TimeTicks::Now()));
    return true;
}
...
void MediaCodecVideoDecoder::ForwardVideoFrame(
    int reset_generation, std::unique_ptr<ScopedAsyncTrace> async_trace, base::TimeTicks started_at, scoped_refptr<VideoFrame> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(3) << __func__ << " : " << (frame ? frame->AsHumanReadableString() : "null");

    // No |frame| indicates an error creating it.
    if (!frame) {
        DLOG(ERROR) << __func__ << " |frame| is null";
        EnterTerminalState(State::kError, { DecoderStatus::Codes::kFailed, "Could not create VideoFrame" });
        return;
    }
...
void MediaCodecVideoDecoder::StartDrainingCodec(DrainType drain_type)
{
    ...
    deferred_flush_pending_ = codec_ && !codec_->IsDrained() && !codec_->IsFlushed();
    OnCodecDrained();
}
...
void MediaCodecVideoDecoder::OnCodecDrained()
{
    ...
    if (codec_ && !codec_->IsFlushed() && !codec_->IsDrained() && !deferred_flush_pending_) {
        FlushCodec();
    }
}

void MediaCodecVideoDecoder::EnterTerminalState(State state, DecoderStatus reason)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    // Any nested error inside `reason` not displayed as the error is propagated
    // to the caller.
    DVLOG(2) << __func__ << " " << static_cast<int>(state) << " " << reason.message();
    ...
    if (state == State::kError) {
        ...
        MEDIA_LOG(INFO, media_log_) << "Entering Terminal State: " << reason.message();
    }
    if (drain_type_) {
        OnCodecDrained();
    }
...
void MediaCodecVideoDecoder::ReleaseCodec()
{
    ...
    codec_allocator_->ReleaseMediaCodec(std::move(pair.first),
        base::BindOnce(
            &base::SequencedTaskRunner::ReleaseSoon<CodecSurfaceBundle>, base::SequencedTaskRunner::GetCurrentDefault(), FROM_HERE, std::move(pair.second)));
}
...
bool MediaCodecVideoDecoder::CanReadWithoutStalling() const
{
    ...
    // rendering to happen.
    const auto buffer_count = codec_ ? codec_->GetUnreleasedOutputBufferCount() : 0;
...
PromotionHintAggregator::NotifyPromotionHintCB MediaCodecVideoDecoder::CreatePromotionHintCB()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    return base::BindPostTaskToCurrentDefault(base::BindRepeating(
        [](base::WeakPtr<MediaCodecVideoDecoder> mcvd, CodecSurfaceBundle::ScheduleLayoutCB layout_cb, PromotionHintAggregator::Hint hint) {
        ...
        weak_factory_.GetWeakPtr(), codec_->SurfaceBundle()->GetScheduleLayoutCB()));
}
...
void MediaCodecVideoDecoder::NotifyPromotionHint(PromotionHintAggregator::Hint hint)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    surface_chooser_helper_.NotifyPromotionHintAndUpdateChooser(hint, IsUsingOverlay());
}

void MediaCodecVideoDecoder::CacheFrameInformation()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    cached_frame_information_ = surface_chooser_helper_.ComputeFrameInformation(IsUsingOverlay());
}

bool MediaCodecVideoDecoder::CodecNeedsReallocation(int new_width)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return !use_block_model_ && new_width > last_width_ * kReallocateThreshold && device_info_ && device_info_->SdkVersion() > base::android::SDK_VERSION_P;
}

std::vector<SupportedVideoDecoderConfig> MediaCodecVideoDecoder::GetSupportedConfigsInternal()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
