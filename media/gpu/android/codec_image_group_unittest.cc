...
public:
    CodecImageGroupWithDestructionHook(scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<CodecSurfaceBundle> surface_bundle)
...
public:
    ...
    {
        std::unique_ptr<MockAndroidOverlay> overlay = std::make_unique<MockAndroidOverlay>();
        EXPECT_CALL(*overlay.get(), MockAddSurfaceDestroyedCallback());
        Record rec;
        rec.surface_bundle = base::MakeRefCounted<CodecSurfaceBundle>(std::move(overlay));
        rec.image_group = base::MakeRefCounted<CodecImageGroupWithDestructionHook>(gpu_task_runner_, rec.surface_bundle);
...
TEST_F(CodecImageGroupTest, SurfaceBundleWithoutOverlayDoesntCrash)
{
    ...
    scoped_refptr<CodecSurfaceBundle> surface_bundle = base::MakeRefCounted<CodecSurfaceBundle>();
    scoped_refptr<CodecImageGroup> image_group = base::MakeRefCounted<CodecImageGroup>(
        gpu_task_runner_, surface_bundle, features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
...
TEST_F(CodecImageGroupTest, ImagesRetainRefToGroup)
{
    ...
    rec.image_group->SetDestructionCallback(base::BindOnce([](bool* flag) -> void { *flag = true; }, &was_destroyed));
