...
class MEDIA_GPU_EXPORT DirectSharedImageVideoProvider : public SharedImageVideoProvider, public gpu::RefCountedLockHelperDrDc {
public:
    ...
    DirectSharedImageVideoProvider(const DirectSharedImageVideoProvider&) = delete;
    DirectSharedImageVideoProvider& operator=(const DirectSharedImageVideoProvider&) = delete;
...
// the pool.
class GpuSharedImageVideoFactory : public gpu::CommandBufferStub::DestructionObserver {
public:
    explicit GpuSharedImageVideoFactory(SharedImageVideoProvider::GetStubCB get_stub_cb);

    GpuSharedImageVideoFactory(const GpuSharedImageVideoFactory&) = delete;
    GpuSharedImageVideoFactory& operator=(const GpuSharedImageVideoFactory&) = delete;
    ...
    void CreateImage(FactoryImageReadyCB cb, const SharedImageVideoProvider::ImageSpec& spec, scoped_refptr<gpu::RefCountedLock> drdc_lock);

private:
    // Creates a SharedImage for |mailbox|, and returns success or failure.
    bool CreateImageInternal(
        const SharedImageVideoProvider::ImageSpec& spec, gpu::Mailbox mailbox, scoped_refptr<CodecImage> image, scoped_refptr<gpu::RefCountedLock>);
