...
public:
    ...
    static CodecAllocator* GetInstance(scoped_refptr<base::SequencedTaskRunner> task_runner);

    CodecAllocator(const CodecAllocator&) = delete;
    CodecAllocator& operator=(const CodecAllocator&) = delete;

    using CodecFactoryCB = base::RepeatingCallback<std::unique_ptr<MediaCodecBridge>(const VideoCodecConfig& config)>;
    ...
    using CodecCreatedCB = base::OnceCallback<void(std::unique_ptr<MediaCodecBridge>)>;
    virtual void CreateMediaCodecAsync(CodecCreatedCB codec_created_cb, std::unique_ptr<VideoCodecConfig> codec_config);

    // Asynchronously release |codec|, |codec_released_cb| will be called after
    // the release has been completed.
    virtual void ReleaseMediaCodec(std::unique_ptr<MediaCodecBridge> codec, base::OnceClosure codec_released_cb);

protected:
    friend class base::NoDestructor<CodecAllocator>;

    CodecAllocator(CodecFactoryCB factory_cb, scoped_refptr<base::SequencedTaskRunner> task_runner);
...
private:
    ...
    void OnCodecCreated(base::TimeTicks start_time, CodecCreatedCB codec_created_cb, std::unique_ptr<MediaCodecBridge> codec);

    // Called on |task_runner_| after a codec is freed.
    void OnCodecReleased(base::TimeTicks start_time, base::OnceClosure codec_released_cb);
