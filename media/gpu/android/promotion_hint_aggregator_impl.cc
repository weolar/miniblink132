...
// promote new ones.
constexpr base::TimeDelta MinimumUnpromotableFrameTime = base::Milliseconds(2000);

PromotionHintAggregatorImpl::PromotionHintAggregatorImpl(const base::TickClock* tick_clock)
{
    if (!tick_clock)
...
void PromotionHintAggregatorImpl::NotifyPromotionHint(const Hint& hint)
{
    ...
    } else if (!IsSafeToPromote() && now - most_recent_update_ > MaximumInterFrameTime) {
...
bool PromotionHintAggregatorImpl::IsSafeToPromote()
{
    ...
    return consecutive_promotable_frames_ >= MinimumPromotableFrames && since_last_unpromotable >= MinimumUnpromotableFrameTime;
}
