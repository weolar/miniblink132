...
public:
    ...
    void ReplaceHelper(bool is_overlay_required, bool promote_secure_only, bool always_use_texture_owner = false)
    {
        // Advance the clock so that time 0 isn't recent.
        tick_clock_.Advance(base::Seconds(10000));

        std::unique_ptr<MockAndroidVideoSurfaceChooser> chooser = std::make_unique<MockAndroidVideoSurfaceChooser>();
        chooser_ = chooser.get();
        std::unique_ptr<MockPromotionHintAggregator> aggregator = std::make_unique<MockPromotionHintAggregator>();
        aggregator_ = aggregator.get();
        helper_ = std::make_unique<SurfaceChooserHelper>(
            std::move(chooser), is_overlay_required, promote_secure_only, always_use_texture_owner, std::move(aggregator), &tick_clock_);
    }
...
{
    helper_->SetSecureSurfaceMode(SurfaceChooserHelper::SecureSurfaceMode::kInsecure);
    UpdateChooserState();
...
{
    helper_->SetSecureSurfaceMode(SurfaceChooserHelper::SecureSurfaceMode::kRequested);
    UpdateChooserState();
...
{
    helper_->SetSecureSurfaceMode(SurfaceChooserHelper::SecureSurfaceMode::kRequired);
    UpdateChooserState();
    ASSERT_TRUE(chooser_->current_state_.is_secure);
    ASSERT_TRUE(chooser_->current_state_.is_required);
...
TEST_F(SurfaceChooserHelperTest, PromotionHintsRelayPosition)
{
    ...
    helper_->NotifyPromotionHintAndUpdateChooser(PromotionHintAggregator::Hint(rect, true), false);
...
TEST_F(SurfaceChooserHelperTest, FrameInformationIsCorrectForL1)
{
    ...
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::OVERLAY_L1, helper_->ComputeFrameInformation(true));
...
TEST_F(SurfaceChooserHelperTest, FrameInformationIsCorrectForL3)
{
    ...
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::OVERLAY_L3, helper_->ComputeFrameInformation(true));
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::NON_OVERLAY_L3, helper_->ComputeFrameInformation(false));
}
...
TEST_F(SurfaceChooserHelperTest, FrameInformationIsCorrectForInsecure)
{
    ...
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::NON_OVERLAY_INSECURE, helper_->ComputeFrameInformation(false));
    ...
    helper_->SetIsFullscreen(true);
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::OVERLAY_INSECURE_PLAYER_ELEMENT_FULLSCREEN, helper_->ComputeFrameInformation(true));
    helper_->SetIsFullscreen(false);
    ASSERT_EQ(SurfaceChooserHelper::FrameInformation::OVERLAY_INSECURE_NON_PLAYER_ELEMENT_FULLSCREEN, helper_->ComputeFrameInformation(true));
}
