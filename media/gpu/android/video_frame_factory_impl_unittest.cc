...
class MockFrameInfoHelper : public FrameInfoHelper, public DestructionObservable {
public:
    void GetFrameInfo(std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, FrameInfoReadyCB cb) override
    {
        FrameInfo info;
...
public:
        ...
        : task_runner_(base::SingleThreadTaskRunner::GetCurrentDefault())
    {
        ...
        impl_ = std::make_unique<VideoFrameFactoryImpl>(task_runner_, gpu_preferences_, std::move(image_provider), std::move(mre_manager),
            std::move(info_helper), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
        auto texture_owner = base::MakeRefCounted<NiceMock<gpu::MockTextureOwner>>(0, nullptr, nullptr, true);
        auto codec_buffer_wait_coordinator = base::MakeRefCounted<CodecBufferWaitCoordinator>(
            std::move(texture_owner), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);

        // Provide a non-null |codec_buffer_wait_coordinator| to |impl_|.
        impl_->SetCodecBufferWaitCorrdinatorForTesting(std::move(codec_buffer_wait_coordinator));
    }
    ...
    {
        auto output_buffer = CodecOutputBuffer::CreateForTesting(0, video_frame_params_.coded_size, video_frame_params_.color_space, std::nullopt);
        ASSERT_TRUE(VideoFrame::IsValidConfig(
            PIXEL_FORMAT_ARGB, VideoFrame::STORAGE_OPAQUE, video_frame_params_.coded_size, video_frame_params_.visible_rect, video_frame_params_.natural_size));
        ...
        impl_->CreateVideoFrame(std::move(output_buffer), base::TimeDelta(), video_frame_params_.natural_size, base::NullCallback(), output_cb_.Get());
    ...
    SharedImageVideoProvider::ImageRecord MakeImageRecord(bool* release_cb_called_flag = nullptr)
    {
        ...
        auto codec_image = base::MakeRefCounted<MockCodecImage>(gfx::Size(100, 100));
        record.codec_image_holder = base::MakeRefCounted<CodecImageHolder>(
            task_runner_, codec_image, features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
        return record;
    }
...
TEST_F(VideoFrameFactoryImplTest, ImageProviderInitFailure)
{
    ...
    // no TextureOwner.
    EXPECT_CALL(*image_provider_raw_, Initialize_(_)).Times(1).WillOnce(RunOnceCallback<0>(nullptr));
    base::MockCallback<VideoFrameFactory::InitCB> init_cb;
    EXPECT_CALL(init_cb, Run(scoped_refptr<gpu::TextureOwner>(nullptr)));
    impl_->Initialize(VideoFrameFactory::OverlayMode::kDontRequestPromotionHints, init_cb.Get());
    base::RunLoop().RunUntilIdle();
...
TEST_F(VideoFrameFactoryImplTest, SetSurfaceBundleForwardsToMaybeRenderEarlyManager)
{
    ...
    scoped_refptr<CodecSurfaceBundle> surface_bundle
        = base::MakeRefCounted<CodecSurfaceBundle>(base::MakeRefCounted<NiceMock<gpu::MockTextureOwner>>(0, nullptr, nullptr, true),
...
TEST_F(VideoFrameFactoryImplTest, CreateVideoFrameFailsIfUnsupportedFormat)
{
    ...
    auto output_buffer = CodecOutputBuffer::CreateForTesting(0, coded_size, gfx::ColorSpace(), std::nullopt);
    ASSERT_FALSE(VideoFrame::IsValidConfig(PIXEL_FORMAT_ARGB, VideoFrame::STORAGE_OPAQUE, coded_size, visible_rect, natural_size));
    ...
    impl_->CreateVideoFrame(std::move(output_buffer), base::TimeDelta(), natural_size, base::NullCallback(), output_cb.Get());
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(VideoFrameFactoryImplTest, CreateVideoFrameSucceeds)
{
    ...
    scoped_refptr<CodecImage> codec_image(record.codec_image_holder->codec_image_raw());
    image_provider_raw_->ProvideOneRequestedImage(&record);
    base::RunLoop().RunUntilIdle();
    EXPECT_NE(frame, nullptr);

    // Make sure that it set the output buffer properly.
    EXPECT_EQ(codec_image->get_codec_output_buffer_for_testing(), output_buffer_raw_);
...
TEST_F(VideoFrameFactoryImplTest, DestroyingFactoryDuringVideoFrameCreationDoesntCrash)
{
