...
std::unique_ptr<NdkMediaCodecWrapper> NdkMediaCodecWrapper::CreateByCodecName(
    std::string_view codec_name, Client* client, scoped_refptr<base::SequencedTaskRunner> runner)
{
    ...
    return base::WrapUnique(new NdkMediaCodecWrapper(std::move(codec), client, std::move(runner)));
}

std::unique_ptr<NdkMediaCodecWrapper> NdkMediaCodecWrapper::CreateByMimeType(
    std::string_view mime_type, Client* client, scoped_refptr<base::SequencedTaskRunner> runner)
{
...
NdkMediaCodecWrapper::NdkMediaCodecWrapper(MediaCodecPtr codec, Client* client, scoped_refptr<base::SequencedTaskRunner> runner)
...
media_status_t NdkMediaCodecWrapper::Start()
{
    ...
    media_status_t status = AMediaCodec_setAsyncNotifyCallback(media_codec_.get(), callbacks, this);
...
void NdkMediaCodecWrapper::OnAsyncInputAvailable(AMediaCodec* codec, void* userdata, int32_t index)
{
    auto* self = reinterpret_cast<NdkMediaCodecWrapper*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkMediaCodecWrapper::OnInputAvailable, self->weak_this_, index));
}
...
void NdkMediaCodecWrapper::OnAsyncOutputAvailable(AMediaCodec* codec, void* userdata, int32_t index, AMediaCodecBufferInfo* bufferInfo)
{
    auto* self = reinterpret_cast<NdkMediaCodecWrapper*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkMediaCodecWrapper::OnOutputAvailable, self->weak_this_, index, *bufferInfo));
}

void NdkMediaCodecWrapper::OnOutputAvailable(int32_t index, AMediaCodecBufferInfo info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NdkMediaCodecWrapper::OnAsyncError(AMediaCodec* codec, void* userdata, media_status_t error, int32_t actionCode, const char* detail)
{
    auto* self = reinterpret_cast<NdkMediaCodecWrapper*>(userdata);
    DCHECK(self);

    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&NdkMediaCodecWrapper::OnError, self->weak_this_, error));
}
