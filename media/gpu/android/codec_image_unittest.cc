...
public:
    ...
    void SetUp() override
    {
        ...
        codec_ = codec.get();
        wrapper_ = std::make_unique<CodecWrapper>(CodecSurfacePair(std::move(codec), new CodecSurfaceBundle()), base::DoNothing(), kFrameSize,
            gfx::ColorSpace::CreateREC709(), std::nullopt,
            /*use_block_model=*/false);
        ON_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillByDefault(Return(OkStatus()));

        gl::init::InitializeStaticGLBindingsImplementation(gl::GLImplementationParts(gl::kGLImplementationEGLGLES2));
        display_ = gl::init::InitializeGLOneOffPlatformImplementation(
            /*disable_gl_drawing=*/false,
            /*init_extensions=*/false,
            /*gpu_preference=*/gl::GpuPreference::kDefault);

        scoped_refptr<gl::GLSurface> surface(new gl::PbufferGLSurfaceEGL(gl::GLSurfaceEGL::GetGLDisplayEGL(), gfx::Size(320, 240)));
        surface->Initialize();
        ...
        auto texture_owner
            = base::MakeRefCounted<NiceMock<gpu::MockTextureOwner>>(texture_id_, context_.get(), context_->default_surface(), BindsTextureOnUpdate());
        ON_CALL(*texture_owner, GetCodedSizeAndVisibleRect(_, _, _))
            .WillByDefault(Invoke([](gfx::Size rotated_visible_size, gfx::Size* coded_size, gfx::Rect* visible_rect) {
                *visible_rect = gfx::Rect(rotated_visible_size);
                *coded_size = visible_rect->size();
                return true;
            }));

        codec_buffer_wait_coordinator_ = base::MakeRefCounted<NiceMock<MockCodecBufferWaitCoordinator>>(std::move(texture_owner));
    }
    ...
    scoped_refptr<CodecImage> NewImage(ImageKind kind, CodecImage::UnusedCB unused_cb = base::DoNothing())
    {
        std::unique_ptr<CodecOutputBuffer> buffer;
        wrapper_->DequeueOutputBuffer(nullptr, nullptr, &buffer);

        auto codec_buffer_wait_coordinator = kind == kTextureOwner ? codec_buffer_wait_coordinator_ : nullptr;
        auto buffer_renderer = std::make_unique<CodecOutputBufferRenderer>(std::move(buffer), codec_buffer_wait_coordinator,
            features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);

        buffer_renderer->set_frame_info_callback(base::BindOnce(&CodecImageTest::OnFrameInfoReady, base::Unretained(this)));

        scoped_refptr<CodecImage> image
            = new CodecImage(buffer_renderer->size(), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
        image->Initialize(std::move(buffer_renderer), kind == kTextureOwner,
            base::BindRepeating(&PromotionHintReceiver::OnPromotionHint, base::Unretained(&promotion_hint_receiver_)));

        image->AddUnusedCB(std::move(unused_cb));
        return image;
    }

    MOCK_METHOD(void, OnFrameInfoReady, (std::optional<gfx::Size> coded_size, std::optional<gfx::Rect> visible_rect), ());

    virtual bool BindsTextureOnUpdate()
    {
        return true;
    }
    ...
    scoped_refptr<NiceMock<MockCodecBufferWaitCoordinator>> codec_buffer_wait_coordinator_;
...
class CodecImageTestExplicitBind : public CodecImageTest {
    bool BindsTextureOnUpdate() override
    {
        return false;
    }
};
...
TEST_F(CodecImageTest, RenderToBackBufferDoesntWait)
{
    ...
    EXPECT_CALL(*codec_buffer_wait_coordinator_, WaitForFrameAvailable()).Times(0);
    ASSERT_TRUE(i->RenderToTextureOwnerBackBuffer());
    EXPECT_CALL(*this, OnFrameInfoReady(Eq(std::nullopt), Eq(std::nullopt)));
}
...
TEST_F(CodecImageTest, PromotingTheBackBufferWaits)
{
    ...
    EXPECT_CALL(*this, OnFrameInfoReady(Eq(kFrameSize), Eq(gfx::Rect(kFrameSize))));
...
TEST_F(CodecImageTest, RenderToFrontBufferRestoresTextureBindings)
{
    ...
    auto i = NewImage(kTextureOwner);
    EXPECT_CALL(*codec_buffer_wait_coordinator_->texture_owner(), UpdateTexImage());
    EXPECT_CALL(*this, OnFrameInfoReady(Eq(kFrameSize), Eq(gfx::Rect(kFrameSize))));
...
{
    auto i = NewImage(kTextureOwner);
    EXPECT_EQ(codec_buffer_wait_coordinator_->texture_owner()->get_a_hardware_buffer_count, 0);
    EXPECT_FALSE(i->was_rendered_to_front_buffer());

    EXPECT_CALL(*codec_buffer_wait_coordinator_->texture_owner(), UpdateTexImage());
    EXPECT_CALL(*this, OnFrameInfoReady(Eq(kFrameSize), Eq(gfx::Rect(kFrameSize))));
    i->GetAHardwareBuffer();
    EXPECT_EQ(codec_buffer_wait_coordinator_->texture_owner()->get_a_hardware_buffer_count, 1);
