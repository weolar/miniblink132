...
class REQUIRES_ANDROID_API(NDK_MEDIA_CODEC_MIN_API) MEDIA_GPU_EXPORT NdkVideoEncodeAccelerator final : public VideoEncodeAccelerator,
...
public:
    ...
    explicit NdkVideoEncodeAccelerator(scoped_refptr<base::SequencedTaskRunner> runner);

    NdkVideoEncodeAccelerator(const NdkVideoEncodeAccelerator&) = delete;
    NdkVideoEncodeAccelerator& operator=(const NdkVideoEncodeAccelerator&) = delete;
...
private:
    ...
    void NotifyMediaCodecError(EncoderStatus encoder_status, media_status_t media_codec_status, std::string message);
    ...
    // error triggers.
    std::unique_ptr<base::WeakPtrFactory<VideoEncodeAccelerator::Client>> client_ptr_factory_;
    ...
    base::flat_map<base::TimeDelta, base::TimeDelta> generated_to_real_timestamp_map_;
