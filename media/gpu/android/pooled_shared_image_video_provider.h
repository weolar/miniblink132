...
class MEDIA_GPU_EXPORT PooledSharedImageVideoProvider : public SharedImageVideoProvider, public gpu::RefCountedLockHelperDrDc {
public:
    ...
    public:
        ...
        virtual void OnImageReturned(const gpu::SyncToken& sync_token, scoped_refptr<CodecImageHolder> codec_image_holder, base::OnceClosure cb,
    ...
    static std::unique_ptr<PooledSharedImageVideoProvider> Create(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetStubCB get_stub_cb,
        std::unique_ptr<SharedImageVideoProvider> provider, scoped_refptr<gpu::RefCountedLock> drdc_lock);

    PooledSharedImageVideoProvider(const PooledSharedImageVideoProvider&) = delete;
    PooledSharedImageVideoProvider& operator=(const PooledSharedImageVideoProvider&) = delete;
...
private:
    ...
    private:
        void OnSyncTokenCleared(scoped_refptr<CodecImageHolder> codec_image_holder, base::OnceClosure cb, scoped_refptr<gpu::RefCountedLock> drdc_lock);
    ...
    void OnImageReturned(scoped_refptr<PooledImage> pooled_image, const gpu::SyncToken& sync_token);
