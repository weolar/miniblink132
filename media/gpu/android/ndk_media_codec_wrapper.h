...
class REQUIRES_ANDROID_API(NDK_MEDIA_CODEC_MIN_API) MEDIA_GPU_EXPORT NdkMediaCodecWrapper {
public:
    class Client {
    public:
    ...
    static std::unique_ptr<NdkMediaCodecWrapper> CreateByMimeType(std::string_view mime_type, Client* client, scoped_refptr<base::SequencedTaskRunner> runner);
...
private:
    ...
    NdkMediaCodecWrapper(MediaCodecPtr codec, Client* client, scoped_refptr<base::SequencedTaskRunner> runner);
    ...
    base::circular_deque<BufferIndex> input_buffers_ GUARDED_BY_CONTEXT(sequence_checker_);

    base::circular_deque<OutputInfo> output_buffers_ GUARDED_BY_CONTEXT(sequence_checker_);
