...
class REQUIRES_ANDROID_API(NDK_MEDIA_CODEC_MIN_API) NdkMediaCodecWrapperTest : public ::testing::Test, public NdkMediaCodecWrapper::Client {
public:
...
protected:
    ...
    void FlushMainThread()
    {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, run_loop.QuitClosure());
        run_loop.Run();
    }

    void CreateMediaCodecWrapper()
    {
        wrapper_ = NdkMediaCodecWrapper::CreateByMimeType(kMimeType, this, base::SequencedTaskRunner::GetCurrentDefault());
    }
    ...
    void SimulateAsyncCodecMessage(base::OnceClosure closure)
    {
        ...
        base::RunLoop run_loop;
        codec_task_runner_->PostTask(FROM_HERE, std::move(closure).Then(run_loop.QuitClosure()));
    ...
    void SimulateInputAvailable(NdkMediaCodecWrapper::BufferIndex index)
    {
        ...
        SimulateAsyncCodecMessage(base::BindLambdaForTesting([&]() { NdkMediaCodecWrapper::OnAsyncInputAvailable(nullptr, wrapper_.get(), index); }));
    }
    ...
    void SimulateOutputAvailable(NdkMediaCodecWrapper::BufferIndex index)
    {
        ...
        SimulateAsyncCodecMessage(base::BindLambdaForTesting([&]() {
            ...
            NdkMediaCodecWrapper::OnAsyncOutputAvailable(nullptr, wrapper_.get(), index, &buffer_info);
        }));
    }
    ...
    void SimulateError(media_status_t error)
    {
        ...
        SimulateAsyncCodecMessage(base::BindLambdaForTesting([&]() { NdkMediaCodecWrapper::OnAsyncError(nullptr, wrapper_.get(), error, 0, "Fake Error"); }));
    }
...
{
    auto wrapper = NdkMediaCodecWrapper::CreateByMimeType(kMimeType, this, base::SequencedTaskRunner::GetCurrentDefault());
