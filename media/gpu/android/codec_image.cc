...
CodecImage::CodecImage(const gfx::Size& coded_size, scoped_refptr<gpu::RefCountedLock> drdc_lock)
...
void CodecImage::Initialize(std::unique_ptr<CodecOutputBufferRenderer> output_buffer_renderer, bool is_texture_owner_backed,
...
void CodecImage::NotifyOverlayPromotion(bool promotion, const gfx::Rect& bounds)
{
    AssertAcquiredDrDcLock();
...
std::unique_ptr<base::android::ScopedHardwareBufferFenceSync> CodecImage::GetAHardwareBuffer()
{
    AssertAcquiredDrDcLock();
...
CodecImageHolder::CodecImageHolder(
    scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<CodecImage> codec_image, scoped_refptr<gpu::RefCountedLock> drdc_lock)
    : base::RefCountedDeleteOnSequence<CodecImageHolder>(std::move(task_runner))
