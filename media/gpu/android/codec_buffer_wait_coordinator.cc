...
struct FrameAvailableEvent : public base::RefCountedThreadSafe<FrameAvailableEvent> {
    FrameAvailableEvent()
        : event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    void Signal()
    {
...
CodecBufferWaitCoordinator::CodecBufferWaitCoordinator(scoped_refptr<gpu::TextureOwner> texture_owner, scoped_refptr<gpu::RefCountedLock> drdc_lock)
    ...
    , task_runner_(base::SingleThreadTaskRunner::GetCurrentDefault())
{
    ...
    texture_owner_->SetFrameAvailableCallback(base::BindRepeating(&FrameAvailableEvent::Signal, frame_available_event_));
}
...
void CodecBufferWaitCoordinator::WaitForFrameAvailable()
{
    ...
    if (remaining <= base::TimeDelta()) {
        if (!frame_available_event_->event.IsSignaled()) {
            DVLOG(1) << "Deferred WaitForFrameAvailable() timed out, elapsed: " << elapsed.InMillisecondsF() << "ms";
        }
    } else {
        DCHECK_LE(remaining, max_wait_.value());
        if (!frame_available_event_->event.TimedWait(remaining)) {
            DVLOG(1) << "WaitForFrameAvailable() timed out, elapsed: " << elapsed.InMillisecondsF()
                     << "ms, additionally waited: " << remaining.InMillisecondsF() << "ms, total: " << (elapsed + remaining).InMillisecondsF() << "ms";
        }
    }
}
