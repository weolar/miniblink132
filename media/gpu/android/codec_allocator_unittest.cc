...
public:
        ...
        : allocator_thread_("AllocatorThread")
    {
        ...
        allocator_ = new CodecAllocator(base::BindRepeating(&MockMediaCodecBridge::CreateVideoDecoder), base::SequencedTaskRunner::GetCurrentDefault());
    ...
    ~CodecAllocatorTest() override
    {
        if (allocator_thread_.IsRunning()) {
            ...
            allocator_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce([](CodecAllocator* allocator) { delete allocator; }, allocator_));
    ...
    void CreateAllocatorOnAnotherThread()
    {
        ...
        allocator_ = new CodecAllocator(base::BindRepeating(&MockMediaCodecBridge::CreateVideoDecoder), allocator_thread_.task_runner());
        allocator_->tick_clock_ = &tick_clock_;
    }

    void OnCodecCreatedInternal(base::OnceClosure quit_closure, std::unique_ptr<MediaCodecBridge> codec)
    {
        // This should always be called on the main thread, despite whatever thread
        // the allocator happens to be running on.
        ASSERT_TRUE(task_environment_.GetMainThreadTaskRunner()->BelongsToCurrentThread());

        last_created_codec_.reset(reinterpret_cast<MockMediaCodecBridge*>(codec.release()));
    ...
    {
        ASSERT_TRUE(allocator_->secondary_task_runner_->RunsTasksInCurrentSequence());
    }
    ...
    std::unique_ptr<VideoCodecConfig> CreateConfig()
    {
        ...
        config->initial_expected_coded_size = CodecAllocator::kMinHardwareResolution;
        return config;
    }
...
TEST_F(CodecAllocatorTest, NormalCreation)
{
    ...
    allocator_->CreateMediaCodecAsync(
        base::BindOnce(&CodecAllocatorTest::OnCodecCreatedInternal, base::Unretained(this), run_loop.QuitClosure()), std::move(config));
...
TEST_F(CodecAllocatorTest, MultipleCreation)
{
    ...
    base::RunLoop run_loop;
    allocator_->CreateMediaCodecAsync(
        base::BindOnce(&CodecAllocatorTest::OnCodecCreatedInternal, base::Unretained(this), base::DoNothing()), std::move(config));
    ...
    allocator_->CreateMediaCodecAsync(
        base::BindOnce(&CodecAllocatorTest::OnCodecCreatedInternal, base::Unretained(this), run_loop.QuitClosure()), std::move(config_secure));
...
TEST_F(CodecAllocatorTest, MultipleRelease)
{
    ...
    allocator_->ReleaseMediaCodec(
        std::make_unique<MockMediaCodecBridge>(), base::BindOnce(&CodecAllocatorTest::OnCodecReleasedInternal, base::Unretained(this), base::DoNothing()));
    ...
    allocator_->ReleaseMediaCodec(
        std::make_unique<MockMediaCodecBridge>(), base::BindOnce(&CodecAllocatorTest::OnCodecReleasedInternal, base::Unretained(this), run_loop.QuitClosure()));
...
TEST_F(CodecAllocatorTest, StalledReleaseCountsAsHung)
{
    ...
    allocator_->ReleaseMediaCodec(std::make_unique<MockMediaCodecBridge>(), base::DoNothing());
    tick_clock_.Advance(base::Seconds(1));
...
TEST_F(CodecAllocatorTest, SecureCreationFailsWhenHung)
{
    ...
    config->codec_type = CodecType::kSecure;
    base::RunLoop run_loop;
    allocator_->CreateMediaCodecAsync(base::BindOnce(
                                          [](base::OnceClosure quit_closure, std::unique_ptr<MediaCodecBridge> codec) {
                                              ASSERT_FALSE(codec);
                                              std::move(quit_closure).Run();
                                          },
...
TEST_F(CodecAllocatorTest, CodecReleasedOnRightTaskRunnerWhenHung)
{
    ...
    reinterpret_cast<MockMediaCodecBridge*>(sw_codec.get())
        ->destruction_cb.ReplaceClosure(base::BindOnce(&CodecAllocatorTest::VerifyOnSecondaryTaskRunner, base::Unretained(this)));
    ...
    reinterpret_cast<MockMediaCodecBridge*>(hw_codec.get())
        ->destruction_cb.ReplaceClosure(base::BindOnce(&CodecAllocatorTest::VerifyOnPrimaryTaskRunner, base::Unretained(this)));
...
TEST_F(CodecAllocatorTest, AllocateAndDestroyCodecOnAllocatorThread)
{
...

    {
        base::RunLoop run_loop;
        allocator_->ReleaseMediaCodec(
            std::move(last_created_codec_), base::BindOnce(&CodecAllocatorTest::OnCodecReleasedInternal, base::Unretained(this), run_loop.QuitClosure()));
...
{
    auto config = CreateConfig();
    config->initial_expected_coded_size = CodecAllocator::kMinHardwareResolution - gfx::Size(1, 1);
    base::RunLoop run_loop;
    allocator_->CreateMediaCodecAsync(
        base::BindOnce(&CodecAllocatorTest::OnCodecCreatedInternal, base::Unretained(this), run_loop.QuitClosure()), std::move(config));
