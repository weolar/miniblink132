...
AndroidVideoSurfaceChooserImpl::AndroidVideoSurfaceChooserImpl(bool allow_dynamic, const base::TickClock* tick_clock)
...
void AndroidVideoSurfaceChooserImpl::SetClientCallbacks(UseOverlayCB use_overlay_cb, UseTextureOwnerCB use_texture_owner_cb)
{
...
void AndroidVideoSurfaceChooserImpl::UpdateState(std::optional<AndroidOverlayFactoryCB> new_factory, const State& new_state)
{
    DCHECK(use_overlay_cb_);
    bool entered_fullscreen = !current_state_.is_fullscreen && new_state.is_fullscreen;
    ...
    if (!allow_dynamic_) {
        if (!initial_state_received_) {
            ...
            if (overlay_factory_
                && ((current_state_.is_fullscreen && !current_state_.promote_secure_only) || current_state_.is_secure || current_state_.is_required)
...
void AndroidVideoSurfaceChooserImpl::Choose()
{
    ...
    OverlayState new_overlay_state = current_state_.promote_secure_only ? kUsingTextureOwner : kUsingOverlay;
    ...
    if (current_state_.is_expecting_relayout && client_overlay_state_ != kUsingOverlay)
        new_overlay_state = kUsingTextureOwner;
    ...
    if (new_overlay_state == kUsingOverlay) {
        base::TimeDelta time_since_last_failure = tick_clock_->NowTicks() - most_recent_overlay_failure_;
...
void AndroidVideoSurfaceChooserImpl::SwitchToOverlay(bool needs_power_efficient)
{
    ...
    config.ready_cb = base::BindOnce(&AndroidVideoSurfaceChooserImpl::OnOverlayReady, weak_factory_.GetWeakPtr());
    config.failed_cb = base::BindOnce(&AndroidVideoSurfaceChooserImpl::OnOverlayFailed, weak_factory_.GetWeakPtr());
    ...
    config.power_cb = base::BindRepeating(&AndroidVideoSurfaceChooserImpl::OnPowerEfficientState, weak_factory_.GetWeakPtr());
...
void AndroidVideoSurfaceChooserImpl::OnOverlayReady(AndroidOverlay* overlay)
{
    ...
    overlay_->AddOverlayDeletedCallback(base::BindOnce(&AndroidVideoSurfaceChooserImpl::OnOverlayDeleted, weak_factory_.GetWeakPtr()));
...
void AndroidVideoSurfaceChooserImpl::OnPowerEfficientState(AndroidOverlay* overlay, bool is_power_efficient)
{
