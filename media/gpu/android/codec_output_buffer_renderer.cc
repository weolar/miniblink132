...
CodecOutputBufferRenderer::CodecOutputBufferRenderer(std::unique_ptr<CodecOutputBuffer> output_buffer,
    scoped_refptr<CodecBufferWaitCoordinator> codec_buffer_wait_coordinator, scoped_refptr<gpu::RefCountedLock> drdc_lock)
...
bool CodecOutputBufferRenderer::RenderToTextureOwnerFrontBuffer()
{
    ...
    if (frame_info_callback_) {
        ...
        if (texture_owner() && texture_owner()->GetCodedSizeAndVisibleRect(size(), &coded_size, &visible_rect)) {
...
bool CodecOutputBufferRenderer::RenderToFrontBuffer()
{
    ...
    return codec_buffer_wait_coordinator_ ? RenderToTextureOwnerFrontBuffer() : RenderToOverlay();
}
