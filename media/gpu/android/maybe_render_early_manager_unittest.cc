...
struct MockImage {
    ...
    {
        ON_CALL(*this, is_texture_owner_backed()).WillByDefault(Return(kind == kTextureOwner));

        ON_CALL(*this, was_rendered_to_front_buffer()).WillByDefault(Return(phase == kInFrontBuffer));

        if (expectation == kRenderToFrontBuffer) {
            EXPECT_CALL(*this, RenderToFrontBuffer()).WillOnce(Return(phase != kInvalidated));
        } else {
            EXPECT_CALL(*this, RenderToFrontBuffer()).Times(0);
        }

        if (expectation == kRenderToBackBuffer) {
            EXPECT_CALL(*this, RenderToTextureOwnerBackBuffer()).WillOnce(Return(phase != kInvalidated));
        } else {
...
public:
    ...
    {
        owned_images_.push_back(std::make_unique<NiceMock<MockImage>>(kind, phase, expectation));
...
TEST_F(MaybeRenderEarlyTest, ImageFollowingLatestFrontBufferIsBackBufferRendered)
{
