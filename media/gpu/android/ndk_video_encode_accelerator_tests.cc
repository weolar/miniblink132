...
class NdkVideoEncoderAcceleratorTest : public ::testing::TestWithParam<VideoParams>, public VideoEncodeAccelerator::Client {
public:
    void SetUp() override
    {
        ...
        bool codec_supported = base::Contains(profiles, profile_, &VideoEncodeAccelerator::SupportedProfile::profile);

        if (!codec_supported) {
            GTEST_SKIP() << "Device doesn't have hw encoder for: " << GetProfileName(profile_);
        }
    }
    ...
    void RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size) override
    {
        ...
        input_buffer_size_ = VideoFrame::AllocationSize(PIXEL_FORMAT_I420, input_coded_size);
        SendNewBuffer();
        if (!OnRequireBuffer())
            loop_.Quit();
    }
...
protected:
    ...
    scoped_refptr<VideoFrame> CreateI420Frame(gfx::Size size, uint32_t color, base::TimeDelta timestamp)
    {
    ...
    scoped_refptr<VideoFrame> CreateNV12Frame(gfx::Size size, uint32_t color, base::TimeDelta timestamp)
    {
        auto i420_frame = CreateI420Frame(size, color, timestamp);
        auto nv12_frame = VideoFrame::CreateFrame(PIXEL_FORMAT_NV12, size, gfx::Rect(size), size, timestamp);
        auto status = frame_converter_.ConvertAndScale(*i420_frame, *nv12_frame);
        EXPECT_TRUE(status.is_ok());
        return nv12_frame;
    }

    scoped_refptr<VideoFrame> CreateRGBFrame(gfx::Size size, uint32_t color, base::TimeDelta timestamp)
    {
    ...
    scoped_refptr<VideoFrame> CreateFrame(gfx::Size size, VideoPixelFormat format, base::TimeDelta timestamp, uint32_t color = 0x964050)
    {
        switch (format) {
    ...
    VideoEncodeAccelerator::Config GetDefaultConfig()
    {
        ...
        auto config = VideoEncodeAccelerator::Config(pixel_format_, frame_size, profile_, bitrate, framerate,
            VideoEncodeAccelerator::Config::StorageType::kShmem, VideoEncodeAccelerator::Config::ContentType::kCamera);
        config.gop_length = 1000;
        config.required_encoder_type = VideoEncodeAccelerator::Config::EncoderType::kNoPreference;
        return config;
    }
    ...
    std::unique_ptr<VideoEncodeAccelerator> MakeNdkAccelerator()
    {
        ...
        return base::WrapUnique<VideoEncodeAccelerator>(new NdkVideoEncodeAccelerator(runner));
    }
    ...
    void ValidateStream(base::span<uint8_t> data)
    {
        ...
        case VideoCodec::kH264: {
            ...
            while (true) {
                ...
                case H264NALU::kSPS: {
                    ...
                    VideoCodecProfile profile = H264Parser::ProfileIDCToVideoCodecProfile(sps->profile_idc);
        ...
        case VideoCodec::kVP9: {
            ...
            while (true) {
                ...
                Vp9Parser::Result res = parser.ParseNextFrame(&frame, &size, &frame_decrypt_config);
        ...
        default: {
            EXPECT_TRUE(base::ranges::any_of(data, [](uint8_t x) { return x != 0; }));
        }
        }
    }
    ...
    scoped_refptr<base::UnsafeSharedMemoryPool> output_pool_ = base::MakeRefCounted<base::UnsafeSharedMemoryPool>();
    std::map<int32_t, std::unique_ptr<base::UnsafeSharedMemoryPool::Handle>> id_to_buffer_;
    struct Output {
        int32_t id;
...
TEST_P(NdkVideoEncoderAcceleratorTest, HandleEncodingError)
{
    ...
    auto frame = VideoFrame::CreateFrame(PIXEL_FORMAT_NV21, size, gfx::Rect(size), size, {});
...
TEST_P(NdkVideoEncoderAcceleratorTest, EncodeSeveralFrames)
{
    ...
    for (auto frame_index = 0u; frame_index < total_frames_count; frame_index++) {
        ...
        auto frame = CreateFrame(config.input_visible_size, pixel_format_, timestamp, color);
    ...
    for (auto& output : outputs_) {
        ...
        auto span = mapping.GetMemoryAsSpan<uint8_t>().first(output.md.payload_size_bytes);
...
{
    auto result = GetProfileName(info.param.profile) + "__" + VideoPixelFormatToString(info.param.pixel_format);
...
INSTANTIATE_TEST_SUITE_P(AllNdkEncoderTests, NdkVideoEncoderAcceleratorTest, ::testing::ValuesIn(kParams), PrintTestParams);
