...
// ChildThread).
class MEDIA_GPU_EXPORT AndroidVideoEncodeAccelerator : public VideoEncodeAccelerator {
public:
    AndroidVideoEncodeAccelerator();

    AndroidVideoEncodeAccelerator(const AndroidVideoEncodeAccelerator&) = delete;
    AndroidVideoEncodeAccelerator& operator=(const AndroidVideoEncodeAccelerator&) = delete;
...
private:
    ...
    using PendingFrames = base::queue<std::tuple<scoped_refptr<VideoFrame>, bool, base::Time>>;
    ...
    std::map<base::TimeDelta /* presentation_timestamp */, base::TimeDelta /* frame_timestamp */> frame_timestamp_map_;
