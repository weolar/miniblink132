...
std::unique_ptr<FrameInfoHelper> CreateHelper()
{
    ...
    auto get_stub_cb = base::BindRepeating([]() -> gpu::CommandBufferStub* { return nullptr; });
    return FrameInfoHelper::Create(
        std::move(task_runner), std::move(get_stub_cb), features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
}
} // namespace
...
protected:
    void GetFrameInfo(std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer)
    {
        ...
        bool called = false;
        auto callback = base::BindLambdaForTesting([&](std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, FrameInfoHelper::FrameInfo info) {
    ...
    std::unique_ptr<CodecOutputBufferRenderer> CreateBufferRenderer(
        gfx::Size size, scoped_refptr<gpu::TextureOwner> texture_owner, std::optional<gfx::Size> coded_size_alignment = std::nullopt)
    {
        auto codec_buffer_wait_coordinator = texture_owner ? base::MakeRefCounted<CodecBufferWaitCoordinator>(texture_owner,
                                                 features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr)
                                                           : nullptr;
        auto buffer = CodecOutputBuffer::CreateForTesting(0, size, gfx::ColorSpace::CreateSRGB(), coded_size_alignment);
...
TEST_F(FrameInfoHelperTest, TextureOwner)
{
    ...
    ON_CALL(*texture_owner, GetCodedSizeAndVisibleRect(_, _, _)).WillByDefault(DoAll(SetArgPointee<1>(kTestCodedSize), Return(true)));
...
TEST_F(FrameInfoHelperTest, TextureOwnerGuessCodedSize)
{
    ...
    EXPECT_CALL(*texture_owner, GetCodedSizeAndVisibleRect(_, _, _)).Times(1);
    GetFrameInfo(CreateBufferRenderer(kTestVisibleSize, texture_owner, kTestAlignment));
    EXPECT_EQ(last_frame_info_.coded_size, kTestCodedSize);
    Mock::VerifyAndClearExpectations(texture_owner.get());
    ...
    auto buffer_renderer = CreateBufferRenderer(kTestVisibleSizeBig, texture_owner, kTestAlignment);
    helper_->GetFrameInfo(
        std::move(buffer_renderer), base::BindLambdaForTesting([&](std::unique_ptr<CodecOutputBufferRenderer> br, FrameInfoHelper::FrameInfo info) {
...
TEST_F(FrameInfoHelperTest, TextureOwnerGuessCodedSizeFailRender)
{
    ...
    // previous one.
    auto buffer_renderer2 = CreateBufferRenderer(kTestVisibleSizeBig, texture_owner, kTestAlignment);
    bool called = false;
    helper_->GetFrameInfo(
        std::move(buffer_renderer2), base::BindLambdaForTesting([&](std::unique_ptr<CodecOutputBufferRenderer> br, FrameInfoHelper::FrameInfo info) {
...
TEST_F(FrameInfoHelperTest, TextureOwnerGuessedWrongCodedSize)
{
    ...
    // Return CodedSize when GetCodedSizeAndVisibleRect is called.
    ON_CALL(*texture_owner, GetCodedSizeAndVisibleRect(_, _, _)).WillByDefault(DoAll(SetArgPointee<1>(kTestVisibleSize2), Return(true)));
...
TEST_F(FrameInfoHelperTest, OrderingTest)
{
    ...
    auto run_after_callback = base::BindLambdaForTesting([&](std::unique_ptr<CodecOutputBufferRenderer> buffer_renderer, FrameInfoHelper::FrameInfo info) {
...
TEST_F(FrameInfoHelperTest, FailedGetCodedSize)
{
    ...
    EXPECT_CALL(*texture_owner, GetCodedSizeAndVisibleRect(_, _, _)).WillOnce(DoAll(SetArgPointee<1>(gfx::Size()), Return(false)));
    GetFrameInfo(CreateBufferRenderer(kTestVisibleSize, texture_owner));
    EXPECT_EQ(last_frame_info_.coded_size, kTestVisibleSize);
    Mock::VerifyAndClearExpectations(texture_owner.get());
...
TEST_F(FrameInfoHelperTest, TextureOwnerBufferNotAvailable)
{
    ...
    EXPECT_CALL(*texture_owner, RunWhenBufferIsAvailable(_)).WillOnce(Invoke([&buffer_available_cb](base::OnceClosure cb) {
