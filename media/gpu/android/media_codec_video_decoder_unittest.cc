...
void OutputCb(scoped_refptr<VideoFrame>* output, scoped_refptr<VideoFrame> frame)
{
    *output = std::move(frame);
}

std::unique_ptr<AndroidOverlay> CreateAndroidOverlayCb(const base::UnguessableToken&, AndroidOverlayConfig)
{
    return nullptr;
}
...
public:
    ...
    MOCK_METHOD5(MockCreateVideoFrame,
        void(CodecOutputBuffer* raw_output_buffer, scoped_refptr<gpu::TextureOwner> texture_owner, base::TimeDelta timestamp, gfx::Size natural_size,
            PromotionHintAggregator::NotifyPromotionHintCB promotion_hint_cb));
    MOCK_METHOD1(MockRunAfterPendingVideoFrames, void(base::OnceClosure* closure));
    MOCK_METHOD0(CancelPendingCallbacks, void());
    MOCK_CONST_METHOD0(IsStalled, bool());

    void SetSurfaceBundle(scoped_refptr<CodecSurfaceBundle> surface_bundle) override
    {
        MockSetSurfaceBundle(surface_bundle);
        if (!surface_bundle) {
            texture_owner_ = nullptr;
        } else {
            texture_owner_ = surface_bundle->overlay() ? nullptr : surface_bundle->codec_buffer_wait_coordinator()->texture_owner();
        }
    }

    void CreateVideoFrame(std::unique_ptr<CodecOutputBuffer> output_buffer, base::TimeDelta timestamp, gfx::Size natural_size,
        PromotionHintAggregator::NotifyPromotionHintCB promotion_hint_cb, VideoFrameFactory::OnceOutputCB output_cb) override
    {
        MockCreateVideoFrame(output_buffer.get(), texture_owner_, timestamp, natural_size, promotion_hint_cb);
...
public:
    ...
    void SetUp() override
    {
        ...
        codec_allocator_ = std::make_unique<FakeCodecAllocator>(base::SingleThreadTaskRunner::GetCurrentDefault());
    ...
    void TearDown() override
    {
        ...
        // to finish the drain.
        if (mcvd_ && codec_ == VideoCodec::kVP8 && codec_allocator_->most_recent_codec)
            DoReset();
    ...
    void CreateMcvd()
    {
        auto surface_chooser = std::make_unique<NiceMock<MockAndroidVideoSurfaceChooser>>();
        surface_chooser_ = surface_chooser.get();

        auto texture_owner = base::MakeRefCounted<NiceMock<gpu::MockTextureOwner>>(0, nullptr, nullptr);
        texture_owner_ = texture_owner.get();

        auto video_frame_factory = std::make_unique<NiceMock<MockVideoFrameFactory>>();
        video_frame_factory_ = video_frame_factory.get();
        // Set up VFF to pass |texture_owner_| via its InitCB.
        ON_CALL(*video_frame_factory_, Initialize(ExpectedOverlayMode(), _)).WillByDefault(RunCallback<1>(texture_owner));

        auto* mcvd = new MediaCodecVideoDecoder(gpu_preferences_, gpu_feature_info_, std::make_unique<NullMediaLog>(), device_info_.get(),
            codec_allocator_.get(), std::move(surface_chooser), base::BindRepeating(&CreateAndroidOverlayCb),
            base::BindRepeating(&MediaCodecVideoDecoderTest::RequestOverlayInfoCb, base::Unretained(this)), std::move(video_frame_factory),
            features::NeedThreadSafeAndroidMedia() ? base::MakeRefCounted<gpu::RefCountedLockForTest>() : nullptr);
        mcvd_ = std::make_unique<AsyncDestroyVideoDecoder<MediaCodecVideoDecoder>>(base::WrapUnique(mcvd));
        mcvd_raw_ = mcvd;
    }
    ...
    VideoFrameFactory::OverlayMode ExpectedOverlayMode() const
    {
        ...
        return want_promotion_hint ? VideoFrameFactory::OverlayMode::kRequestPromotionHints : VideoFrameFactory::OverlayMode::kDontRequestPromotionHints;
    }

    void CreateCdm(bool has_media_crypto_context, bool require_secure_video_decoder)
    {
        ...
        // it's not null.
        media_crypto_ = base::android::ScopedJavaGlobalRef<jobject>(gl::SurfaceTexture::Create(0)->j_surface_texture());
    }
    ...
    bool Initialize(VideoDecoderConfig config)
    {
        if (!mcvd_)
            CreateMcvd();
        bool result = false;
        auto init_cb = [](bool* result_out, DecoderStatus result) { *result_out = result.is_ok(); };
        mcvd_->Initialize(config, false, cdm_.get(), base::BindOnce(init_cb, &result), base::BindRepeating(&OutputCb, &most_recent_frame_), base::DoNothing());
        base::RunLoop().RunUntilIdle();
        ...
        if (cdm_ && cdm_->media_crypto_ready_cb_) {
            std::move(cdm_->media_crypto_ready_cb_)
                .Run(std::make_unique<base::android::ScopedJavaGlobalRef<jobject>>(media_crypto_), require_secure_video_decoder_);
    ...
    MockAndroidOverlay* InitializeWithOverlay_OneDecodePending(VideoDecoderConfig config)
    {
        ...
        if (!java_surface_) {
            java_surface_ = base::android::ScopedJavaGlobalRef<jobject>(gl::SurfaceTexture::Create(0)->j_surface_texture());
        }
        EXPECT_CALL(*overlay, GetJavaSurface()).WillRepeatedly(ReturnRef(java_surface_));
    ...
    MockMediaCodecBridge* InitializeFully_OneDecodePending(VideoDecoderConfig config)
    {
    ...
    void DoReset()
    {
        ...
        mcvd_->Reset(base::BindOnce([](bool* reset_complete) { *reset_complete = true; }, &reset_complete));
        base::RunLoop().RunUntilIdle();
        if (!reset_complete) {
            // Note that there might be more pending decodes, and this will arrive
            // out of order.  We assume that MCVD doesn't care.
            codec_allocator_->most_recent_codec->ProduceOneOutput(MockMediaCodecBridge::kEos);
            PumpCodec();
            EXPECT_TRUE(reset_complete);
        }
    }

    void RequestOverlayInfoCb(bool restart_for_transitions, ProvideOverlayInfoCB provide_overlay_info_cb)
    {
...
TEST_P(MediaCodecVideoDecoderAV1Test, Av1IsSupported)
{
    ...
    EXPECT_CALL(*device_info_, IsAv1DecoderAvailable()).WillRepeatedly(Return(true));
...
{
    CreateMcvd();
    EXPECT_CALL(*video_frame_factory_, Initialize(ExpectedOverlayMode(), _)).Times(0);
...
TEST_P(MediaCodecVideoDecoderTest, FirstDecodeTriggersOverlayInfoRequestIfSupported)
{
    ASSERT_TRUE(Initialize(TestVideoConfig::Large(codec_)));
...
TEST_P(MediaCodecVideoDecoderTest, OverlayInfoIsNotRequestedIfOverlaysNotSupported)
{
    ASSERT_TRUE(Initialize(TestVideoConfig::Large(codec_)));
    ON_CALL(*device_info_, SupportsOverlaySurfaces()).WillByDefault(Return(false));
    mcvd_->Decode(fake_decoder_buffer_, decode_cb_.Get());
    ASSERT_FALSE(provide_overlay_info_cb_);
}
...
{
    ON_CALL(*device_info_, IsSetOutputSurfaceSupported()).WillByDefault(Return(true));
    ASSERT_TRUE(Initialize(TestVideoConfig::Large(codec_)));
    mcvd_->Decode(fake_decoder_buffer_, decode_cb_.Get());
    ASSERT_FALSE(restart_for_transitions_);
}

TEST_P(MediaCodecVideoDecoderTest, OverlayInfoIsNotRequestedIfThreadedTextureMailboxesEnabled)
{
...
{
    ASSERT_TRUE(InitializeWithTextureOwner_OneDecodePending(TestVideoConfig::Large(codec_)));
...
{
    ASSERT_TRUE(Initialize(TestVideoConfig::Large(codec_)));
    ON_CALL(*video_frame_factory_, Initialize(ExpectedOverlayMode(), _)).WillByDefault(RunCallback<1>(nullptr));
...
{
    auto* codec = InitializeFully_OneDecodePending(TestVideoConfig::Large(codec_));
    ASSERT_TRUE(codec);
    EXPECT_CALL(*codec, DequeueInputBuffer(_, _)).WillOnce(Return(MediaCodecResult::Codes::kError));
...
TEST_P(MediaCodecVideoDecoderTest, AfterInitCompletesTheCodecIsPolled)
{
    ...
    // input buffer.
    base::RunLoop loop;
    EXPECT_CALL(*codec, DequeueInputBuffer(_, _)).WillOnce(InvokeWithoutArgs([&loop]() {
        loop.Quit();
...
{
    EXPECT_CALL(*codec_allocator_, MockCreateMediaCodecAsync());
    ASSERT_TRUE(InitializeWithOverlay_OneDecodePending(TestVideoConfig::Large(codec_)));
    EXPECT_TRUE(base::android::AttachCurrentThread()->IsSameObject(java_surface_.obj(), codec_allocator_->most_recent_config->surface.obj()));
}

TEST_P(MediaCodecVideoDecoderTest, CodecCreationWeakPtrIsInvalidatedBySurfaceDestroyed)
{
    ON_CALL(*device_info_, IsSetOutputSurfaceSupported()).WillByDefault(Return(false));
    auto* overlay = InitializeWithOverlay_OneDecodePending(TestVideoConfig::Large(codec_));
    ASSERT_TRUE(overlay);
...
TEST_P(MediaCodecVideoDecoderTest, SurfaceDestroyedReleasesCodecIfSetSurfaceIsNotSupported)
{
    ON_CALL(*device_info_, IsSetOutputSurfaceSupported()).WillByDefault(Return(false));
    auto* overlay = InitializeWithOverlay_OneDecodePending(TestVideoConfig::Large(codec_));
    ASSERT_TRUE(overlay);
...
TEST_P(MediaCodecVideoDecoderTest, SetSurfaceFailureReleasesTheCodecAndSignalsError)
{
    ASSERT_TRUE(InitializeWithOverlay_OneDecodePending(TestVideoConfig::Large(codec_)));
    auto* codec = codec_allocator_->ProvideMockCodecAsync();
...
TEST_P(MediaCodecVideoDecoderTest, ResetBeforeCodecInitializedSucceedsImmediately)
{
    ASSERT_TRUE(InitializeWithTextureOwner_OneDecodePending(TestVideoConfig::Large(codec_)));
...
TEST_P(MediaCodecVideoDecoderTest, ResetAbortsPendingEosDecode)
{
    ...
    EXPECT_CALL(eos_decode_cb, Run(HasStatusCode(DecoderStatus::Codes::kAborted)));
    DoReset();
...
TEST_P(MediaCodecVideoDecoderTest, ElidedEOSForConfigChange)
{
    ...
    auto* codec = InitializeFully_OneDecodePending(TestVideoConfig::Large(codec_));
    ASSERT_TRUE(codec);
    mcvd_->Decode(DecoderBuffer::CreateEOSBuffer(TestVideoConfig::Normal(codec_)), decode_cb_.Get());
...
TEST_P(MediaCodecVideoDecoderTest, ElidedEOSSkippedForCodecChange)
{
    ...
    auto* codec = InitializeFully_OneDecodePending(TestVideoConfig::Large(codec_));
    ASSERT_TRUE(codec);
    auto different_codec = codec_ == VideoCodec::kAV1 ? VideoCodec::kVP9 : VideoCodec::kAV1;
    mcvd_->Decode(DecoderBuffer::CreateEOSBuffer(TestVideoConfig::Normal(different_codec)), decode_cb_.Get());
...
TEST_P(MediaCodecVideoDecoderTest, TeardownInvalidatesCodecCreationWeakPtrButDoesNotCallReleaseMediaCodec)
{
    ASSERT_TRUE(InitializeWithTextureOwner_OneDecodePending(TestVideoConfig::Large(codec_)));
    mcvd_.reset();
...
TEST_P(MediaCodecVideoDecoderTest, CdmInitializationWorksForL3)
{
    ...
    ASSERT_TRUE(InitializeWithOverlay_OneDecodePending(TestVideoConfig::NormalEncrypted(codec_)));
...
TEST_P(MediaCodecVideoDecoderTest, CdmInitializationWorksForL1)
{
    ...
    ASSERT_EQ(codec_allocator_->most_recent_config->codec_type, CodecType::kSecure);
    ASSERT_TRUE(codec_allocator_->most_recent_config->media_crypto);
}
...
TEST_P(MediaCodecVideoDecoderTest, CdmIsSetEvenForClearStream)
{
    ...
    ASSERT_EQ(surface_chooser_->current_state_.is_secure, true);
    ASSERT_EQ(surface_chooser_->current_state_.is_required, false);
    ASSERT_NE(codec_allocator_->most_recent_config->codec_type, CodecType::kSecure);
    // We can't check for equality safely, but verify that something was provided.
    ASSERT_TRUE(codec_allocator_->most_recent_config->media_crypto);
}
...
TEST_P(MediaCodecVideoDecoderTest, NoMediaCryptoContext_ClearStream)
{
    ...
    ASSERT_TRUE(InitializeWithOverlay_OneDecodePending(TestVideoConfig::Normal(codec_)));
...
TEST_P(MediaCodecVideoDecoderH264Test, CsdIsIncludedInCodecConfig)
{
    ...
    std::vector<uint8_t> csd0 = { 103, 77, 64, 30, 232, 128, 80, 23, 252, 184, 8, 128, 0, 0, 3, 0, 128, 0, 0, 30, 7, 139, 22, 137 };
    ...
    extra_data.insert(extra_data.end(), extra_data_separator.begin(), extra_data_separator.end());
...
TEST_P(MediaCodecVideoDecoderVp9Test, ColorSpaceIsIncludedInCodecConfig)
{
    VideoColorSpace color_space(
        VideoColorSpace::PrimaryID::BT2020, VideoColorSpace::TransferID::SMPTEST2084, VideoColorSpace::MatrixID::BT2020_CL, gfx::ColorSpace::RangeID::LIMITED);
    VideoDecoderConfig config = TestVideoConfig::NormalWithColorSpace(VideoCodec::kVP9, color_space);
    EXPECT_TRUE(InitializeFully_OneDecodePending(config));

    EXPECT_EQ(color_space, codec_allocator_->most_recent_config->container_color_space);
}
...
TEST_P(MediaCodecVideoDecoderVp9Test, HdrMetadataIsIncludedInCodecConfig)
{
    ...
    hdr_metadata.smpte_st_2086 = gfx::HdrMetadataSmpteSt2086({ 0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f },
...
{
    return HasAv1Decoder() ? std::vector<VideoCodec>(1, VideoCodec::kAV1) : std::vector<VideoCodec>();
}

INSTANTIATE_TEST_SUITE_P(MediaCodecVideoDecoderTest, MediaCodecVideoDecoderTest, testing::ValuesIn(GetTestList()));

#if BUILDFLAG(USE_PROPRIETARY_CODECS)
INSTANTIATE_TEST_SUITE_P(MediaCodecVideoDecoderH264Test, MediaCodecVideoDecoderH264Test, testing::ValuesIn(GetH264()));
#endif
...
INSTANTIATE_TEST_SUITE_P(MediaCodecVideoDecoderAV1Test, MediaCodecVideoDecoderAV1Test, testing::ValuesIn(GetAv1IfAvailable()));
