...
public:
    CodecWrapperImpl(CodecSurfacePair codec_surface_pair, CodecWrapper::OutputReleasedCB output_buffer_release_cb, const gfx::Size& initial_expected_size,
        const gfx::ColorSpace& config_color_space, std::optional<gfx::Size> coded_size_alignment, bool use_block_model);
    ...
    DequeueStatus DequeueOutputBuffer(base::TimeDelta* presentation_time, bool* end_of_stream, std::unique_ptr<CodecOutputBuffer>* codec_buffer);
...
CodecOutputBuffer::CodecOutputBuffer(
    scoped_refptr<CodecWrapperImpl> codec, int64_t id, const gfx::Size& size, const gfx::ColorSpace& color_space, std::optional<gfx::Size> coded_size_alignment)
...
// For testing.
CodecOutputBuffer::CodecOutputBuffer(int64_t id, const gfx::Size& size, const gfx::ColorSpace& color_space, std::optional<gfx::Size> coded_size_alignment)
    : id_(id)
    , size_(size)
    , color_space_(color_space)
    , coded_size_alignment_(coded_size_alignment)
...
gfx::Size CodecOutputBuffer::GuessCodedSize() const
{
    ...
    return gfx::Size(base::bits::AlignUpDeprecatedDoNotUse(size_.width(), coded_size_alignment_->width()),
        base::bits::AlignUpDeprecatedDoNotUse(size_.height(), coded_size_alignment_->height()));
}

CodecWrapperImpl::CodecWrapperImpl(CodecSurfacePair codec_surface_pair, CodecWrapper::OutputReleasedCB output_buffer_release_cb,
    const gfx::Size& initial_expected_size, const gfx::ColorSpace& config_color_space, std::optional<gfx::Size> coded_size_alignment, bool use_block_model)
...
CodecWrapperImpl::QueueStatus CodecWrapperImpl::QueueInputBuffer(const DecoderBuffer& buffer)
{
    DVLOG(4) << __func__;
    base::AutoLock l(lock_);
    DCHECK(codec_ && state_ != State::kError);
    ...
    if (owned_input_buffer_) {
        ...
        MediaCodecResult result = codec_->DequeueInputBuffer(base::TimeDelta(), &input_buffer);
        switch (result.code()) {
    ...
    if (buffer.end_of_stream()) {
        ...
        if (state_ == State::kFlushed || state_ == State::kDrained) {
            ...
            if (use_block_model_) {
                codec_->QueueInputBlock(input_buffer, base::span<const uint8_t>(), base::TimeDelta(), true);
            } else {
    ...
    if (decrypt_config) {
        result = codec_->QueueSecureInputBuffer(input_buffer, buffer, decrypt_config->key_id(), decrypt_config->iv(), decrypt_config->subsamples(),
            decrypt_config->encryption_scheme(), decrypt_config->encryption_pattern(), buffer.timestamp());
    } else {
        if (use_block_model_) {
            result = codec_->QueueInputBlock(input_buffer, buffer, buffer.timestamp(), false);
        } else {
...
CodecWrapperImpl::DequeueStatus CodecWrapperImpl::DequeueOutputBuffer(
    base::TimeDelta* presentation_time, bool* end_of_stream, std::unique_ptr<CodecOutputBuffer>* codec_buffer)
{
    DVLOG(4) << __func__;
    base::AutoLock l(lock_);
    DCHECK(codec_ && state_ != State::kError);
    ...
    for (int attempt = 0; attempt < 3; ++attempt) {
        ...
        size_t unused;
        bool eos = false;
        MediaCodecResult result = codec_->DequeueOutputBuffer(base::TimeDelta(), &index, &unused, &unused, presentation_time, &eos, nullptr);
        switch (result.code()) {
        case MediaCodecResult::Codes::kOk: {
            if (eos) {
                state_ = State::kDrained;
            ...
            *codec_buffer = base::WrapUnique(new CodecOutputBuffer(this, buffer_id, size_, color_space_, coded_size_alignment_));
            return DequeueStatus::Codes::kOk;
        }
        case MediaCodecResult::Codes::kTryAgainLater: {
        ...
        case MediaCodecResult::Codes::kOutputFormatChanged: {
            ...
            if (result.code() == MediaCodecResult::Codes::kError) {
                state_ = State::kError;
                return { DequeueStatus::Codes::kError, "Output Size changed to an unusable size.", std::move(result) };
            }
            ...
            bool error = codec_->GetOutputColorSpace(&color_space_) == MediaCodecResult::Codes::kError;
            ...
            if (error && !size_.IsEmpty()) {
                if (config_color_space_.IsValid()) {
                    ...
                    color_space_ = size_.width() >= 1280 ? gfx::ColorSpace::CreateREC709() : gfx::ColorSpace::CreateSRGB();
                }
            }
            continue;
        }
    ...
    state_ = State::kError;
    return { DequeueStatus::Codes::kError, "Failed to dequeue after multiple attempts." };
}

bool CodecWrapperImpl::SetSurface(scoped_refptr<CodecSurfaceBundle> surface_bundle)
{
    DVLOG(2) << __func__;
    base::AutoLock l(lock_);
...
bool CodecWrapperImpl::ReleaseCodecOutputBuffer(int64_t id, bool render)
{
    ...
    DVLOG(2) << __func__ << " id=" << id << " render=" << render << " valid=" << valid;
    if (!valid) {
        return false;
    }
    ...
    if (output_buffer_release_cb_) {
        output_buffer_release_cb_.Run(state_ == State::kDrained || state_ == State::kDraining || buffer_ids_.empty());
    }
    return true;
}

CodecWrapper::CodecWrapper(CodecSurfacePair codec_surface_pair, OutputReleasedCB output_buffer_release_cb, const gfx::Size& initial_expected_size,
    const gfx::ColorSpace& config_color_space, std::optional<gfx::Size> coded_size_alignment, bool use_block_model)
    : impl_(new CodecWrapperImpl(
        std::move(codec_surface_pair), std::move(output_buffer_release_cb), initial_expected_size, config_color_space, coded_size_alignment, use_block_model))
{
}
...
CodecWrapper::QueueStatus CodecWrapper::QueueInputBuffer(const DecoderBuffer& buffer)
{
...
CodecWrapper::DequeueStatus CodecWrapper::DequeueOutputBuffer(
    base::TimeDelta* presentation_time, bool* end_of_stream, std::unique_ptr<CodecOutputBuffer>* codec_buffer)
{
    return impl_->DequeueOutputBuffer(presentation_time, end_of_stream, codec_buffer);
}

bool CodecWrapper::SetSurface(scoped_refptr<CodecSurfaceBundle> surface_bundle)
{
