...
public:
    ...
    void AddCodecImage(scoped_refptr<CodecImageHolder> codec_image_holder)
    {
        ...
        codec_image_holder->codec_image_raw()->AddUnusedCB(base::BindOnce(&GpuMaybeRenderEarlyImpl::OnImageUnused, weak_factory_.GetWeakPtr()));
...
class MaybeRenderEarlyManagerImpl : public MaybeRenderEarlyManager, public gpu::RefCountedLockHelperDrDc {
public:
    MaybeRenderEarlyManagerImpl(scoped_refptr<base::SequencedTaskRunner> gpu_task_runner, scoped_refptr<gpu::RefCountedLock> drdc_lock)
    ...
    MaybeRenderEarlyManagerImpl& operator=(const MaybeRenderEarlyManagerImpl&) = delete;
    ...
    void SetSurfaceBundle(scoped_refptr<CodecSurfaceBundle> surface_bundle) override
    {
        ...
        auto image_group = base::MakeRefCounted<CodecImageGroup>(gpu_task_runner_, std::move(surface_bundle), GetDrDcLock());
        ...
        gpu_impl_.AsyncCall(&GpuMaybeRenderEarlyImpl::SetCodecImageGroup).WithArgs(std::move(image_group));
    }

    void AddCodecImage(scoped_refptr<CodecImageHolder> codec_image_holder) override
    {
        gpu_impl_.AsyncCall(&GpuMaybeRenderEarlyImpl::AddCodecImage).WithArgs(std::move(codec_image_holder));
    }

    void MaybeRenderEarly() override
    {
        gpu_impl_.AsyncCall(&GpuMaybeRenderEarlyImpl::MaybeRenderEarly).WithArgs(GetDrDcLock());
    }
...
std::unique_ptr<MaybeRenderEarlyManager> MaybeRenderEarlyManager::Create(
    scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<gpu::RefCountedLock> lock)
{
    return std::make_unique<MaybeRenderEarlyManagerImpl>(std::move(task_runner), std::move(lock));
}
