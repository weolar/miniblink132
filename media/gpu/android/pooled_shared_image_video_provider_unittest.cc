...
public:
    ...
    public:
        ...
        void OnImageReturned(const gpu::SyncToken& sync_token, scoped_refptr<CodecImageHolder> codec_image_holder, base::OnceClosure cb,
    ...
    void SetUp() override
    {
        ...
        base::SequenceBound<MockGpuHelper> mock_gpu_helper(task_runner_, &sync_token_);

        std::unique_ptr<MockSharedImageVideoProvider> mock_provider = std::make_unique<MockSharedImageVideoProvider>();
        mock_provider_raw_ = mock_provider.get();

        provider_ = base::WrapUnique(new PooledSharedImageVideoProvider(std::move(mock_gpu_helper), std::move(mock_provider),
    ...
    {
        return base::BindOnce([](std::list<SharedImageVideoProvider::ImageRecord>* output_list,
                                  SharedImageVideoProvider::ImageRecord record) { output_list->push_back(std::move(record)); },
            &image_records_);
    }
...
TEST_F(PooledSharedImageVideoProviderTest, InitializeForwardsGpuCallback)
{
    ...
    auto gpu_init_cb = base::BindOnce([](bool* flag, scoped_refptr<gpu::SharedContextState>) { *flag = true; }, &was_called);
...
TEST_F(PooledSharedImageVideoProviderTest, ReleasingAnImageForwardsSyncToken)
{
    ...
    gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferIdFromChannelAndRoute(2, 3), 4);
    std::move(image_records_.back().release_cb).Run(sync_token);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(sync_token, sync_token_);
}

TEST_F(PooledSharedImageVideoProviderTest, ReleasingAnImageDoesntRunUnderlyingReleaseCallback)
{
...
TEST_F(PooledSharedImageVideoProviderTest, DeletingProviderWithOutstandingImagesDoesntCrash)
{
...
TEST_F(PooledSharedImageVideoProviderTest, ReturnedImagesAreReleasedAfterSpecChange)
{
