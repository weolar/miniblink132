...
public:
    CodecWrapperTest()
        : other_thread_("Other thread")
    {
        auto codec = std::make_unique<NiceMock<MockMediaCodecBridge>>();
        codec_ = codec.get();
        surface_bundle_ = base::MakeRefCounted<CodecSurfaceBundle>();
        wrapper_ = std::make_unique<CodecWrapper>(CodecSurfacePair(std::move(codec), surface_bundle_), output_buffer_release_cb_.Get(), kInitialCodedSize,
        ...
        ON_CALL(*codec_, DequeueInputBuffer(_, _)).WillByDefault(DoAll(SetArgPointee<1>(12), Return(OkStatus())));
        ON_CALL(*codec_, QueueInputBuffer(_, _, _)).WillByDefault(Return(OkStatus()));
    ...
    NiceMock<base::MockCallback<CodecWrapper::OutputReleasedCB>> output_buffer_release_cb_;
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MediaCodecResult::Codes::kError));
...
TEST_F(CodecWrapperTest, CodecOutputBufferReleaseUsesCorrectIndex)
{
    ...
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(DoAll(SetArgPointee<1>(42), Return(OkStatus())));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillRepeatedly(Return(MediaCodecResult::Codes::kOutputFormatChanged));
    std::unique_ptr<CodecOutputBuffer> codec_buffer;
    auto status = wrapper_->DequeueOutputBuffer(nullptr, nullptr, &codec_buffer);
...
{
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MediaCodecResult::Codes::kOutputFormatChanged)).WillOnce(Return(OkStatus()));
    EXPECT_CALL(*codec_, GetOutputSize(_)).WillOnce(DoAll(SetArgPointee<0>(gfx::Size(42, 42)), Return(OkStatus())));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
TEST_F(CodecWrapperTest, CodecOutputBuffersGuessCodedSizeNoAlignment)
{
    ...
    wrapper_ = std::make_unique<CodecWrapper>(
        std::move(surface_pair), output_buffer_release_cb_.Get(), kInitialCodedSize, gfx::ColorSpace::CreateREC709(), std::nullopt, false);
...
TEST_F(CodecWrapperTest, CodecOutputBuffersGuessCodedSizeWeirdAlignment)
{
    auto surface_pair = wrapper_->TakeCodecSurfacePair();
    wrapper_ = std::make_unique<CodecWrapper>(
        std::move(surface_pair), output_buffer_release_cb_.Get(), kInitialCodedSize, gfx::ColorSpace::CreateREC709(), gfx::Size(128, 1), false);
...
TEST_F(CodecWrapperTest, RejectedInputBuffersAreReused)
{
    ...
    EXPECT_CALL(*codec_, DequeueInputBuffer(_, _)).WillOnce(DoAll(SetArgPointee<1>(666), Return(OkStatus())));
    EXPECT_CALL(*codec_, QueueInputBuffer(666, _, _)).WillOnce(Return(MediaCodecResult::Codes::kNoKey)).WillOnce(Return(OkStatus()));
...
TEST_F(CodecWrapperTest, RenderCallbackCalledIfRendered)
{
    auto codec_buffer = DequeueCodecOutputBuffer();
    bool flag = false;
    codec_buffer->set_render_cb(base::BindOnce([](bool* flag) { *flag = true; }, base::Unretained(&flag)));
    codec_buffer->ReleaseToSurface();
    EXPECT_TRUE(flag);
}
...
TEST_F(CodecWrapperTest, CodecWrapperGetsColorSpaceFromCodec)
{
    ...
    EXPECT_CALL(*codec_, GetOutputColorSpace(_)).WillOnce(DoAll(SetArgPointee<0>(color_space), Return(OkStatus())));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
TEST_F(CodecWrapperTest, CodecWrapperDefaultsToSRGB)
{
    auto surface_pair = wrapper_->TakeCodecSurfacePair();
    wrapper_
        = std::make_unique<CodecWrapper>(std::move(surface_pair), output_buffer_release_cb_.Get(), kInitialCodedSize, gfx::ColorSpace(), std::nullopt, false);
    ...
    // CodecWrapper should provide the color space that's reported by the bridge.
    EXPECT_CALL(*codec_, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(MediaCodecResult::Codes::kOutputFormatChanged)).WillOnce(Return(OkStatus()));
    EXPECT_CALL(*codec_, GetOutputColorSpace(_)).WillOnce(Return(MediaCodecResult::Codes::kError));
    auto codec_buffer = DequeueCodecOutputBuffer();
...
TEST_F(CodecWrapperTest, CodecWrapperUseConfigColorSpace)
{
    auto surface_pair = wrapper_->TakeCodecSurfacePair();
    wrapper_ = std::make_unique<CodecWrapper>(
        std::move(surface_pair), output_buffer_release_cb_.Get(), kInitialCodedSize, gfx::ColorSpace::CreateJpeg(), std::nullopt, false);
