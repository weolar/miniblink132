...
std::unique_ptr<MediaCodecBridge> CreateMediaCodecInternal(CodecAllocator::CodecFactoryCB factory_cb, std::unique_ptr<VideoCodecConfig> codec_config)
{
    TRACE_EVENT0("media", "CodecAllocator::CreateMediaCodec");
    base::ScopedBlockingCall scoped_block(FROM_HERE, base::BlockingType::MAY_BLOCK);
...
{
    return base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN });
}
...
// static
CodecAllocator* CodecAllocator::GetInstance(scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    static base::NoDestructor<CodecAllocator> allocator(base::BindRepeating(&MediaCodecBridgeImpl::CreateVideoDecoder), task_runner);
...
void CodecAllocator::CreateMediaCodecAsync(CodecCreatedCB codec_created_cb, std::unique_ptr<VideoCodecConfig> codec_config)
{
    ...
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&CodecAllocator::CreateMediaCodecAsync, base::Unretained(this), base::BindPostTaskToCurrentDefault(std::move(codec_created_cb)),
    ...
    if (codec_config->codec_type == CodecType::kAny
        && (codec_config->initial_expected_coded_size.width() < kMinHardwareResolution.width()
            || codec_config->initial_expected_coded_size.height() < kMinHardwareResolution.height())) {
        codec_config->codec_type = CodecType::kSoftware;
    }
    ...
    std::move(task_runner)
        ->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&CreateMediaCodecInternal, factory_cb_, std::move(codec_config)),
            base::BindOnce(&CodecAllocator::OnCodecCreated, base::Unretained(this), start_time, std::move(codec_created_cb)));
}

void CodecAllocator::ReleaseMediaCodec(std::unique_ptr<MediaCodecBridge> codec, base::OnceClosure codec_released_cb)
{
    DCHECK(codec);
    ...
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&CodecAllocator::ReleaseMediaCodec, base::Unretained(this), std::move(codec),
    ...
    task_runner->PostTaskAndReply(FROM_HERE, base::BindOnce(&ReleaseMediaCodecInternal, std::move(codec)),
        base::BindOnce(&CodecAllocator::OnCodecReleased, base::Unretained(this), start_time, std::move(codec_released_cb)));
}

CodecAllocator::CodecAllocator(CodecFactoryCB factory_cb, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void CodecAllocator::OnCodecCreated(base::TimeTicks start_time, CodecCreatedCB codec_created_cb, std::unique_ptr<MediaCodecBridge> codec)
{
...
void CodecAllocator::OnCodecReleased(base::TimeTicks start_time, base::OnceClosure codec_released_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    CompletePendingOperation(start_time);
...
bool CodecAllocator::IsPrimaryTaskRunnerLikelyHung() const
{
    ...
    return !pending_operations_.empty() && tick_clock_->NowTicks() - *pending_operations_.begin() > kHungTaskDetectionTimeout;
}
...
void CodecAllocator::CompletePendingOperation(base::TimeTicks start_time)
{
    ...
    pending_operations_.erase(base::ranges::find(pending_operations_, start_time));
}
