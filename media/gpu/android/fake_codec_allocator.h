...
public:
    explicit FakeCodecAllocator(scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    void CreateMediaCodecAsync(CodecCreatedCB codec_created_cb, std::unique_ptr<VideoCodecConfig> config) override;
    void ReleaseMediaCodec(std::unique_ptr<MediaCodecBridge> media_codec, base::OnceClosure codec_released_cb) override;
    ...
    MockMediaCodecBridge* ProvideMockCodecAsync(std::unique_ptr<MockMediaCodecBridge> codec = nullptr);
