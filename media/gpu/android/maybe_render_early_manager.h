...
public:
    ...
    virtual void SetSurfaceBundle(scoped_refptr<CodecSurfaceBundle> surface_bundle) = 0;
    ...
    virtual void AddCodecImage(scoped_refptr<CodecImageHolder> codec_image_holder) = 0;
    ...
    static std::unique_ptr<MaybeRenderEarlyManager> Create(
        scoped_refptr<base::SequencedTaskRunner> gpu_task_runner, scoped_refptr<gpu::RefCountedLock> drdc_lock);
};
...
template <typename Image> void MEDIA_GPU_EXPORT MaybeRenderEarly(std::vector<raw_ptr<Image, VectorExperimental>>* image_vector_ptr)
{
    ...
    if (back_buffer_index < images.size() && images[back_buffer_index]->is_texture_owner_backed()) {
