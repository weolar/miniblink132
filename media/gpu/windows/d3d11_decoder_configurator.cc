...
GUID GetD3D11DecoderGUID(const VideoCodecProfile& profile, uint8_t bit_depth, VideoChromaSampling chroma_sampling)
{
    switch (profile) {
...
D3D11DecoderConfigurator::D3D11DecoderConfigurator(
    DXGI_FORMAT decoder_output_dxgifmt, GUID decoder_guid, gfx::Size coded_size, bool is_encrypted, bool supports_swap_chain)
...
// static
std::unique_ptr<D3D11DecoderConfigurator> D3D11DecoderConfigurator::Create(const gpu::GpuPreferences& gpu_preferences,
    const gpu::GpuDriverBugWorkarounds& workarounds, const VideoDecoderConfig& config, uint8_t bit_depth, VideoChromaSampling chroma_sampling,
    MediaLog* media_log, bool use_shared_handle)
{
    ...
    const bool supports_nv12_decode_swap_chain = gl::DirectCompositionDecodeSwapChainSupported() && !use_shared_handle;

    DXGI_FORMAT decoder_dxgi_format = GetOutputDXGIFormat(bit_depth, chroma_sampling);
    if (decoder_dxgi_format == DXGI_FORMAT_UNKNOWN) {
        MEDIA_LOG(WARNING, media_log) << "D3D11VideoDecoder does not support bit depth " << base::strict_cast<int>(bit_depth)
                                      << " with chroma subsampling format " << VideoChromaSamplingToString(chroma_sampling);
        return nullptr;
    }

    GUID decoder_guid = GetD3D11DecoderGUID(config.profile(), bit_depth, chroma_sampling);
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    ...
    if (decoder_guid == GUID()) {
        if (config.profile() == HEVCPROFILE_REXT) {
            MEDIA_LOG(INFO, media_log) << "D3D11VideoDecoder does not support HEVC range extension " << config.codec() << " with chroma subsampling format "
                                       << VideoChromaSamplingToString(chroma_sampling) << " and bit depth " << base::strict_cast<int>(bit_depth);
        } else {
            MEDIA_LOG(INFO, media_log) << "D3D11VideoDecoder does not support codec " << config.codec();
        }
        return nullptr;
    }

    MEDIA_LOG(INFO, media_log) << "D3D11VideoDecoder is using " << GetProfileName(config.profile()) << " / " << VideoChromaSamplingToString(chroma_sampling);

    return std::make_unique<D3D11DecoderConfigurator>(
        decoder_dxgi_format, decoder_guid, config.coded_size(), config.is_encrypted(), supports_nv12_decode_swap_chain);
}

bool D3D11DecoderConfigurator::SupportsDevice(ComD3D11VideoDevice video_device)
{
...
D3D11Status::Or<ComD3D11Texture2D> D3D11DecoderConfigurator::CreateOutputTexture(
    ComD3D11Device device, gfx::Size size, uint32_t array_size, bool use_shared_handle)
{
    ...
    if (use_shared_handle) {
        ...
        output_texture_desc_.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED;
    ...
    HRESULT hr = device->CreateTexture2D(&output_texture_desc_, nullptr, &texture);
    if (FAILED(hr))
