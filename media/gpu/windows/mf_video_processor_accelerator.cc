...
MediaFoundationVideoProcessorAccelerator::MediaFoundationVideoProcessorAccelerator(
...
bool MediaFoundationVideoProcessorAccelerator::Initialize(
    const Config& config, scoped_refptr<DXGIDeviceManager> dxgi_device_manager, std::unique_ptr<MediaLog> media_log)
{
...
bool MediaFoundationVideoProcessorAccelerator::InitializeVideoProcessor(const Config& config)
{
    HRESULT hr = CoCreateInstance(CLSID_VideoProcessorMFT, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&video_processor_));
    ...
    if (dxgi_device_manager_) {
        ...
        hr = video_processor_->ProcessMessage(MFT_MESSAGE_SET_D3D_MANAGER, reinterpret_cast<ULONG_PTR>(mf_dxgi_device_manager.Get()));
    ...
    hr = output_attributes->SetUINT32(MF_SA_D3D11_BINDFLAGS, D3D11_BIND_VIDEO_ENCODER);
    ...
    hr = input_media_type_->SetGUID(MF_MT_SUBTYPE, VideoPixelFormatToMFSubtype(config.input_format));
    RETURN_ON_HR_FAILURE(hr, "Couldn't set subtype", false);
    hr = MFSetAttributeSize(input_media_type_.Get(), MF_MT_FRAME_SIZE, config.input_visible_size.width(), config.input_visible_size.height());
    ...
    hr = input_media_type_->SetUINT32(MF_MT_SAMPLE_SIZE, VideoFrame::AllocationSize(config.input_format, config.input_visible_size));
    RETURN_ON_HR_FAILURE(hr, "Couldn't set sample size", false);
    hr = MFSetAttributeRatio(input_media_type_.Get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1);
    RETURN_ON_HR_FAILURE(hr, "Couldn't set pixel aspect ratio", false);
    hr = input_media_type_->SetUINT32(MF_MT_VIDEO_PRIMARIES, VideoPrimariesToMFVideoPrimaries(config.input_color_space.GetPrimaryID()));
    RETURN_ON_HR_FAILURE(hr, "Couldn't set video primaries", false);
    if (config.input_format == PIXEL_FORMAT_XRGB || config.input_format == PIXEL_FORMAT_ARGB) {
        ...
        hr = input_media_type_->SetUINT32(MF_MT_DEFAULT_STRIDE, config.input_visible_size.width() * 4);
    ...
    hr = input_media_type_->SetUINT32(MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive);
    ...
    hr = output_media_type->SetGUID(MF_MT_SUBTYPE, VideoPixelFormatToMFSubtype(config.output_format));
    RETURN_ON_HR_FAILURE(hr, L"Couldn't set subtype", false);
    hr = MFSetAttributeSize(output_media_type.Get(), MF_MT_FRAME_SIZE, config.output_visible_size.width(), config.output_visible_size.height());
    ...
    hr = output_media_type->SetUINT32(MF_MT_VIDEO_PRIMARIES, VideoPrimariesToMFVideoPrimaries(config.output_color_space.GetPrimaryID()));
...
HRESULT MediaFoundationVideoProcessorAccelerator::Convert(scoped_refptr<VideoFrame> frame, IMFSample** sample_out)
{
    DCHECK(video_processor_ != nullptr);

    ComMFSample sample;
    HRESULT hr = GenerateSampleFromVideoFrame(frame.get(), dxgi_device_manager_.get(), true, nullptr, 0, &sample);
...
HRESULT MediaFoundationVideoProcessorAccelerator::Convert(IMFSample* sample, IMFSample** sample_out)
{
    ...
    if (!(stream_info.dwFlags & MFT_OUTPUT_STREAM_PROVIDES_SAMPLES) && !(stream_info.dwFlags & MFT_OUTPUT_STREAM_CAN_PROVIDE_SAMPLES)) {
...
HRESULT MediaFoundationVideoProcessorAccelerator::AdjustInputSizeIfNeeded(IMFSample* sample)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        (void)MFGetAttributeSize(input_media_type_.Get(), MF_MT_FRAME_SIZE, &mt_width, &mt_height);
        if (input_desc.Width != mt_width || input_desc.Height != mt_height) {
            ...
            hr = MFSetAttributeSize(new_input_media_type.Get(), MF_MT_FRAME_SIZE, input_desc.Width, input_desc.Height);
            RETURN_ON_HR_FAILURE(hr, "Couldn't set new frame size", hr);
            hr = video_processor_->SetInputType(0, new_input_media_type.Get(), 0);
            RETURN_ON_HR_FAILURE(hr, "Couldn't set new input type on video processor", hr);
