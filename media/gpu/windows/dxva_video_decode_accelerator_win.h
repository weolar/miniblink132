...
typedef HRESULT(WINAPI* CreateDXGIDeviceManager)(UINT* reset_token, IMFDXGIDeviceManager** device_manager);
...
public:
    ...
    virtual gfx::Rect current_visible_rect(const gfx::Rect& container_visible_rect) const = 0;
    virtual VideoColorSpace current_color_space(const VideoColorSpace& container_color_space) const = 0;
...
// from any other.
class MEDIA_GPU_EXPORT DXVAVideoDecodeAccelerator : public VideoDecodeAccelerator {
public:
    enum State {
    ...
    DXVAVideoDecodeAccelerator(const GetGLContextCallback& get_gl_context_cb, const MakeGLContextCurrentCallback& make_context_current_cb,
        const BindGLImageCallback& bind_image_cb, const gpu::GpuDriverBugWorkarounds& workarounds, const gpu::GpuPreferences& gpu_preferences,
        MediaLog* media_log);

    DXVAVideoDecodeAccelerator(const DXVAVideoDecodeAccelerator&) = delete;
    DXVAVideoDecodeAccelerator& operator=(const DXVAVideoDecodeAccelerator&) = delete;
    ...
    void Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id) override;
    ...
    bool TryToSetupDecodeOnSeparateThread(
        const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SingleThreadTaskRunner>& decode_task_runner) override;
...
private:
    ...
    void DoDecode(const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space);
    ...
    bool ProcessOutputSample(Microsoft::WRL::ComPtr<IMFSample> sample, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space);
    ...
    void NotifyPictureReady(int picture_buffer_id, int input_buffer_id, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space, bool allow_overlay,
    ...
    void CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id);

    void BindPictureBufferToSample(Microsoft::WRL::ComPtr<IMFSample> sample, int picture_buffer_id, int input_buffer_id);
    ...
    bool CopyTexture(ID3D11Texture2D* src_texture, ID3D11Texture2D* dest_texture, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex,
        uint64_t keyed_mutex_value, int picture_buffer_id, int input_buffer_id, const gfx::ColorSpace& color_space);

    // Copies the |video_frame| to the destination |dest_texture|.
    void CopyTextureOnDecoderThread(ID3D11Texture2D* dest_texture, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex, uint64_t keyed_mutex_value,
        Microsoft::WRL::ComPtr<IMFSample> input_sample, int picture_buffer_id, int input_buffer_id);
    ...
    void FlushDecoder(int iterations, IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id);
    ...
    bool InitializeID3D11VideoProcessor(int width, int height, const gfx::ColorSpace& color_space);
    ...
    // Returns true on success.
    bool SetTransformOutputType(IMFTransform* transform, const GUID& output_type, int width, int height);
    ...
    std::vector<scoped_refptr<Picture::ScopedSharedImage>> GetSharedImagesFromPictureBuffer(DXVAPictureBuffer* picture_buffer);
    ...
    Microsoft::WRL::ComPtr<IDirectXVideoProcessorService> video_processor_service_;
    ...
    struct PendingSampleInfo {
        PendingSampleInfo(int32_t buffer_id, Microsoft::WRL::ComPtr<IMFSample> sample, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space);
