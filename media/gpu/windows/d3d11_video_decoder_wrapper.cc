...
template <typename D3D11VideoContext, typename D3D11VideoDecoderBufferDesc> class ScopedD3D11DecoderBuffer;

template <typename D3D11VideoContext, typename D3D11VideoDecoderBufferDesc> class D3D11VideoDecoderWrapperImpl : public D3D11VideoDecoderWrapper {
public:
    D3D11VideoDecoderWrapperImpl(
        MediaLog* media_log, ComD3D11VideoDevice video_device, Microsoft::WRL::ComPtr<D3D11VideoContext> video_context, ComD3D11VideoDecoder video_decoder)
    ...
    std::optional<bool> UseSingleTexture() const override
    {
        ...
        if (FAILED(hr)) {
            MEDIA_PLOG(ERROR, hr, media_log_) << "D3D11VideoDecoder GetCreationParameters failed";
            return std::nullopt;
        }
    ...
    bool WaitForFrameBegins(D3D11PictureBuffer* output_picture) override
    {
        ...
        HRESULT hr;
        do {
            hr = video_context_->DecoderBeginFrame(video_decoder_.Get(), output_view, 0, nullptr);
    ...
    {
        D3D11_VIDEO_DECODER_BUFFER_TYPE d3d_buffer_type = BufferTypeToD3D11BufferType(type);
...
private:
    friend class ScopedD3D11DecoderBuffer<D3D11VideoContext, D3D11VideoDecoderBufferDesc>;

    std::unique_ptr<ScopedD3DBuffer> GetBuffer(BufferType type, uint32_t desired_size) override
    {
        return std::make_unique<ScopedD3D11DecoderBuffer<D3D11VideoContext, D3D11VideoDecoderBufferDesc>>(
...
template <> bool D3D11VideoDecoderWrapperImpl<ID3D11VideoContext, D3D11_VIDEO_DECODER_BUFFER_DESC>::SubmitDecoderBuffers()
{
    DCHECK_LE(video_buffers_.size(), 4ull);
    HRESULT hr = video_context_->SubmitDecoderBuffers(video_decoder_.Get(), video_buffers_.size(), video_buffers_.data());
...
template <> bool D3D11VideoDecoderWrapperImpl<ID3D11VideoContext1, D3D11_VIDEO_DECODER_BUFFER_DESC1>::SubmitDecoderBuffers()
{
    DCHECK_LE(video_buffers_.size(), 4ull);
    HRESULT hr = video_context_->SubmitDecoderBuffers1(video_decoder_.Get(), video_buffers_.size(), video_buffers_.data());
    video_buffers_.clear();
    if (FAILED(hr)) {
        MEDIA_PLOG(ERROR, hr, media_log_) << "SubmitDecoderBuffers failed";
        return false;
...
template <typename D3D11VideoContext, typename D3D11VideoDecoderBufferDesc> class ScopedD3D11DecoderBuffer : public ScopedD3DBuffer {
public:
    ScopedD3D11DecoderBuffer(D3D11VideoDecoderWrapperImpl<D3D11VideoContext, D3D11VideoDecoderBufferDesc>* decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type,
        uint32_t desired_size, MediaLog* media_log)
        : decoder_(decoder)
        , type_(type)
        ...
        , media_log_(media_log->Clone())
    {
        UINT size;
        uint8_t* buffer;
        HRESULT hr = decoder_->video_context_->GetDecoderBuffer(decoder_->video_decoder_.Get(), type_, &size, reinterpret_cast<void**>(&buffer));
        if (FAILED(hr)) {
            ...
            media_log_->NotifyError(D3D11Status { status_code, "D3D11 GetDecoderBuffer failed", hr });
            return;
        }
    ...
    ~ScopedD3D11DecoderBuffer() override
    {
        Commit();
    }
    ...
    bool Commit(uint32_t written_size) override
    {
        ...
        HRESULT hr = decoder_->video_context_->ReleaseDecoderBuffer(decoder_->video_decoder_.Get(), type_);
        if (FAILED(hr)) {
            D3D11StatusCode status_code = D3D11StatusCode::kOk;
            switch (type_) {
            case D3D11_VIDEO_DECODER_BUFFER_PICTURE_PARAMETERS:
            ...
            media_log_->NotifyError(D3D11Status { status_code, "D3D11 ReleaseDecoderBuffer failed", hr });
            return false;
        }
...
private:
    const raw_ptr<D3D11VideoDecoderWrapperImpl<D3D11VideoContext, D3D11VideoDecoderBufferDesc>> decoder_;
...
// static
std::unique_ptr<D3D11VideoDecoderWrapper> D3D11VideoDecoderWrapper::Create(MediaLog* media_log, ComD3D11VideoDevice video_device,
    ComD3D11VideoContext video_context, const D3D11DecoderConfigurator* decoder_configurator, D3D_FEATURE_LEVEL supported_d3d11_version,
    VideoDecoderConfig config)
{
    UINT config_count = 0;
    HRESULT hr = video_device->GetVideoDecoderConfigCount(decoder_configurator->DecoderDescriptor(), &config_count);
    ...
    for (UINT i = 0; i < config_count; i++) {
        hr = video_device->GetVideoDecoderConfig(decoder_configurator->DecoderDescriptor(), i, &dec_config);
        if (FAILED(hr)) {
        ...
        if (dec_config.ConfigBitstreamRaw == 1
            && (config.codec() == VideoCodec::kVP9 || config.codec() == VideoCodec::kAV1 || config.codec() == VideoCodec::kHEVC)) {
            found = true;
            break;
        }

        // ConfigBitstreamRaw == 2 means the decoder uses DXVA_Slice_H264_Short.
        if (dec_config.ConfigBitstreamRaw == 2 && config.codec() == VideoCodec::kH264) {
            found = true;
            break;
        }
    }
    ...
    hr = video_device->CreateVideoDecoder(decoder_configurator->DecoderDescriptor(), &dec_config, &video_decoder);
    if (FAILED(hr)) {
    ...
    if (supported_d3d11_version == D3D_FEATURE_LEVEL_11_0) {
        return std::make_unique<D3D11VideoDecoderWrapperImpl<ID3D11VideoContext, D3D11_VIDEO_DECODER_BUFFER_DESC>>(
            media_log, std::move(video_device), std::move(video_context), std::move(video_decoder));
    }
    ...
    if (supported_d3d11_version == D3D_FEATURE_LEVEL_11_1) {
        ...
        CHECK_EQ(hr, S_OK);
        return std::make_unique<D3D11VideoDecoderWrapperImpl<ID3D11VideoContext1, D3D11_VIDEO_DECODER_BUFFER_DESC1>>(
            media_log, std::move(video_device), std::move(video_context1), std::move(video_decoder));
    }
