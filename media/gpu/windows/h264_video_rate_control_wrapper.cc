...
template <> int H264RateControl::GetLoopfilterLevel() const
{
    DCHECK(impl_);
    return impl_->GetLoopfilterLevel();
}

template <> void H264RateControl::PostEncodeUpdate(uint64_t encoded_frame_size, const FrameParams& frame_params)
{
    DCHECK(impl_);
    return impl_->PostEncodeUpdate(encoded_frame_size, ConvertFrameParams(frame_params));
}

template <> H264RateControlConfigRTC H264RateControl::ConvertControlConfig(const RateControlConfig& config)
{
    ...
    for (int tid = 0; tid < config.ts_number_layers; ++tid) {
        ...
        rc_config.layer_settings[tid].avg_bitrate = config.layer_target_bitrate[tid] * 1000;
        // Peak bitrate is set to 1.5 times the average bitrate.
        rc_config.layer_settings[tid].peak_bitrate = config.layer_target_bitrate[tid] * 1000 * 3 / 2;
        base::TimeDelta buffer_delay;
        if (config.content_type == VideoEncodeAccelerator::Config::ContentType::kCamera) {
            buffer_delay = kHRDBufferDelayCamera;
        } else {
            buffer_delay = kHRDBufferDelayDisplay;
        }
        size_t buffer_size
            = static_cast<size_t>(rc_config.layer_settings[tid].avg_bitrate * buffer_delay.InMilliseconds() / base::Time::kMillisecondsPerSecond / 8);
        ...
        rc_config.layer_settings[tid].frame_rate = static_cast<float>(config.framerate / (1 << (config.ts_number_layers - tid - 1)));

        if (tid > 0) {
            DCHECK_GT(rc_config.layer_settings[tid].avg_bitrate, rc_config.layer_settings[tid - 1].avg_bitrate);
        }
    }
    return rc_config;
}

template <> H264FrameParamsRTC H264RateControl::ConvertFrameParams(const FrameParams& frame_params)
{
    H264FrameParamsRTC rc_params;
    rc_params.temporal_layer_id = frame_params.temporal_layer_id;
    rc_params.keyframe = frame_params.frame_type == FrameParams::FrameType::kKeyFrame;
