...
uint32_t MaxSizeOfKeyframeAsPercentage(uint32_t optimal_buffer_size, uint32_t max_framerate)
{
    ...
    return kMinIntraSizePercentage > target_size_kbyte_as_percent ? kMinIntraSizePercentage : target_size_kbyte_as_percent;
}
...
template <> int VP9RateControl::GetLoopfilterLevel() const
{
    DCHECK(impl_);
    return impl_->GetLoopfilterLevel();
}

template <> void VP9RateControl::PostEncodeUpdate(uint64_t encoded_frame_size, const FrameParams& frame_params)
{
    DCHECK(impl_);
    return impl_->PostEncodeUpdate(encoded_frame_size, ConvertFrameParams(frame_params));
}

template <> libvpx::VP9RateControlRtcConfig VP9RateControl::ConvertControlConfig(const RateControlConfig& config)
{
    ...
    rc_config.max_intra_bitrate_pct = MaxSizeOfKeyframeAsPercentage(rc_config.buf_optimal_sz, rc_config.framerate);
...
template <> libvpx::VP9FrameParamsQpRTC VP9RateControl::ConvertFrameParams(const FrameParams& frame_params)
{
    ...
    rc_params.frame_type = frame_params.frame_type == FrameParams::FrameType::kKeyFrame ? libvpx::RcFrameType::kKeyFrame : libvpx::RcFrameType::kInterFrame;
    return rc_params;
}
