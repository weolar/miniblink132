...
class MEDIA_GPU_EXPORT D3D11VideoDecoder : public VideoDecoder, public D3D11VideoDecoderClient {
public:
    ...
    static std::unique_ptr<VideoDecoder> Create(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log,
        const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds,
        base::RepeatingCallback<gpu::CommandBufferStub*()> get_stub_cb, GetD3DDeviceCB get_d3d_device_cb, SupportedConfigs supported_configs);
    ...
    bool OutputResult(const CodecPicture* picture, D3D11PictureBuffer* picture_buffer) override;
    D3DVideoDecoderWrapper* GetWrapper() override;

    bool ResetD3DVideoDecoder();

    static bool GetD3D11FeatureLevel(ComD3D11Device dev, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, D3D_FEATURE_LEVEL* feature_level);

    // Return the set of video decoder configs that we support.
    static std::vector<SupportedVideoDecoderConfig> GetSupportedVideoDecoderConfigs(
...
private:
    ...
    D3D11VideoDecoder(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log,
        const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds,
    ...
    void PictureBufferGPUResourceInitDone(scoped_refptr<D3D11PictureBuffer> buffer);

    // Called when the gpu side of initialization is complete.
    void OnGpuInitComplete(bool success, D3D11VideoFrameMailboxReleaseHelper::ReleaseMailboxCB release_mailbox_cb);
    ...
    std::unique_ptr<D3DVideoDecoderWrapper> CreateD3DVideoDecoderWrapper(D3D11DecoderConfigurator* decoder_configurator, uint8_t bit_depth);
    ...
    // just kFailed.
    void NotifyError(D3D11Status reason, DecoderStatus::Codes opt_decoder_code = DecoderStatus::Codes::kFailed);
    ...
    std::list<std::pair<scoped_refptr<DecoderBuffer>, DecodeCB>> input_buffer_queue_;
