...
#include "patch_code/dxgi_xp.h" // SUPPORT_XP_CODE
#include "patch_code/mfapi_xp.h"
...
const CLSID MEDIASUBTYPE_VP80 = { 0x30385056, 0x0000, 0x0010, { 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71 } };

const CLSID MEDIASUBTYPE_VP90 = { 0x30395056, 0x0000, 0x0010, { 0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71 } };
...
static const CLSID CLSID_CAV1DecoderMFT = { 0xC843981A, 0x3359, 0x4721, { 0xB3, 0xF5, 0xD4, 0x84, 0xD8, 0x56, 0x1E, 0x47 } };
...
HRESULT CreateAV1Decoder(const IID& iid, void** object)
{
    ...
    UINT32 acts_num;
    HRESULT hr = ::MFTEnumExXp(MFT_CATEGORY_VIDEO_DECODER, MFT_ENUM_FLAG_SORTANDFILTER, &type_info, nullptr, &acts, &acts_num);
    if (FAILED(hr))
        return hr;
...
#define RETURN_AND_NOTIFY_ON_HR_FAILURE(result, log, error_code, ret)                                                                                          \
    RETURN_AND_NOTIFY_ON_FAILURE(SUCCEEDED(result), log << ", HRESULT: 0x" << std::hex << result, error_code, ret);

namespace media {

bool VideoPixelFormatToVizFormat(
    VideoPixelFormat pixel_format, size_t textures_per_picture, std::array<viz::ResourceFormat, VideoFrame::kMaxPlanes>& texture_formats)
{
    switch (pixel_format) {
    ...
    case PIXEL_FORMAT_BGRA:
        ...
        texture_formats[0] = (pixel_format == PIXEL_FORMAT_ABGR) ? viz::RGBA_8888 : viz::BGRA_8888;
        return true;
...
constexpr VideoCodecProfile kSupportedProfiles[] = { H264PROFILE_BASELINE, H264PROFILE_MAIN, H264PROFILE_HIGH, VP8PROFILE_ANY, VP9PROFILE_PROFILE0,
    VP9PROFILE_PROFILE2, AV1PROFILE_PROFILE_MAIN, AV1PROFILE_PROFILE_HIGH, AV1PROFILE_PROFILE_PRO };

CreateDXGIDeviceManager DXVAVideoDecodeAccelerator::create_dxgi_device_manager_ = NULL;
...
static Microsoft::WRL::ComPtr<IMFSample> CreateInputSample(const uint8_t* stream, uint32_t size, uint32_t min_size, int alignment)
{
    CHECK(stream);
    CHECK_GT(size, 0U);
    Microsoft::WRL::ComPtr<IMFSample> sample;
    sample = CreateEmptySampleWithBuffer(std::max(min_size, size), alignment);
    RETURN_ON_FAILURE(sample.Get(), "Failed to create empty sample", Microsoft::WRL::ComPtr<IMFSample>());

    Microsoft::WRL::ComPtr<IMFMediaBuffer> buffer;
    HRESULT hr = sample->GetBufferByIndex(0, &buffer);
    RETURN_ON_HR_FAILURE(hr, "Failed to get buffer from sample", Microsoft::WRL::ComPtr<IMFSample>());

    DWORD max_length = 0;
    DWORD current_length = 0;
    uint8_t* destination = NULL;
    hr = buffer->Lock(&destination, &max_length, &current_length);
    RETURN_ON_HR_FAILURE(hr, "Failed to lock buffer", Microsoft::WRL::ComPtr<IMFSample>());
    ...
    RETURN_ON_HR_FAILURE(hr, "Failed to set buffer length", Microsoft::WRL::ComPtr<IMFSample>());

    hr = buffer->Unlock();
    RETURN_ON_HR_FAILURE(hr, "Failed to unlock buffer", Microsoft::WRL::ComPtr<IMFSample>());
...
HRESULT CreateCOMObjectFromDll(HMODULE dll, const CLSID& clsid, const IID& iid, void** object)
{
    if (!dll || !object)
        return E_INVALIDARG;
    ...
    GetClassObject get_class_object = reinterpret_cast<GetClassObject>(GetProcAddress(dll, "DllGetClassObject"));
    RETURN_ON_FAILURE(get_class_object, "Failed to get DllGetClassObject pointer", E_FAIL);
...
public:
    ...
    gfx::Rect current_visible_rect(const gfx::Rect& container_visible_rect) const override;
    VideoColorSpace current_color_space(const VideoColorSpace& container_color_space) const override;
...
bool H264ConfigChangeDetector::DetectConfig(const uint8_t* stream, unsigned int size)
{
...
gfx::Rect H264ConfigChangeDetector::current_visible_rect(const gfx::Rect& container_visible_rect) const
{
    if (!parser_)
...
VideoColorSpace H264ConfigChangeDetector::current_color_space(const VideoColorSpace& container_color_space) const
{
    if (!parser_)
...
public:
    ...
    bool DetectConfig(const uint8_t* stream, unsigned int size) override
    {
        ...
        while (parser_.ParseNextFrame(&fhdr, &allocate_size, &null_config) == Vp9Parser::kOk) {
            ...
            if (!size_.IsEmpty() && !pending_config_changed_ && !config_changed_ && size_ != new_size) {
                pending_config_changed_ = true;
                DVLOG(1) << "Configuration changed from " << size_.ToString() << " to " << new_size.ToString();
            }
            size_ = new_size;
    ...
    VideoColorSpace current_color_space(const VideoColorSpace& container_color_space) const override
    {
        return container_color_space.IsSpecified() ? container_color_space : color_space_;
    }

    bool is_vp9_resilient_mode() const override
    {
        return is_resilient_mode_;
    }
...
DXVAVideoDecodeAccelerator::PendingSampleInfo::PendingSampleInfo(
    int32_t buffer_id, Microsoft::WRL::ComPtr<IMFSample> sample, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space)
...
DXVAVideoDecodeAccelerator::PendingSampleInfo::PendingSampleInfo(const PendingSampleInfo& other) = default;

DXVAVideoDecodeAccelerator::PendingSampleInfo::~PendingSampleInfo()
{
}

DXVAVideoDecodeAccelerator::DXVAVideoDecodeAccelerator(const GetGLContextCallback& get_gl_context_cb,
    const MakeGLContextCurrentCallback& make_context_current_cb, const BindGLImageCallback& bind_image_cb, const gpu::GpuDriverBugWorkarounds& workarounds,
    const gpu::GpuPreferences& gpu_preferences, MediaLog* media_log)
    : client_(nullptr)
    ...
    , support_share_nv12_textures_(gpu_preferences.enable_zero_copy_dxgi_video && !workarounds.disable_dxgi_zero_copy_video)
    , num_picture_buffers_requested_(support_share_nv12_textures_ ? kNumPictureBuffersForZeroCopy : kNumPictureBuffers)
    , support_copy_nv12_textures_(gpu_preferences.enable_nv12_dxgi_video && !workarounds.disable_nv12_dxgi_video)
    , support_delayed_copy_nv12_textures_(base::FeatureList::IsEnabled(kDelayCopyNV12Textures) && !workarounds.disable_delayed_copy_nv12)
    , use_dx11_(false)
    , use_keyed_mutex_(false)
    , using_angle_device_(false)
    , using_debug_device_(false)
    , enable_accelerated_av1_decode_(!workarounds.disable_accelerated_av1_decode)
    , enable_accelerated_vp8_decode_(!workarounds.disable_accelerated_vp8_decode)
    , enable_accelerated_vp9_decode_(!workarounds.disable_accelerated_vp9_decode)
    , disallow_vp9_resilient_dxva_decoding_(workarounds.disallow_vp9_resilient_dxva_decoding)
...
bool DXVAVideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    if (media_log_)
    ...
    if (!config.supported_output_formats.empty() && !base::Contains(config.supported_output_formats, PIXEL_FORMAT_NV12)) {
    ...
    RETURN_ON_FAILURE(profile_supported, "Unsupported h.264, vp8, or vp9 profile", false);

    if (config.profile == VP9PROFILE_PROFILE2 || config.profile == VP9PROFILE_PROFILE3 || config.profile == H264PROFILE_HIGH10PROFILE) {
    ...
    if (config.container_color_space.ToGfxColorSpace().IsHDR()) {
        use_fp16_ = true;
        if (config.profile == AV1PROFILE_PROFILE_PRO || config.profile == AV1PROFILE_PROFILE_MAIN || config.profile == AV1PROFILE_PROFILE_HIGH) {
            decoder_output_p010_or_p016_ = true;
        }
    }
...
// and enable the code below.
    ...
    if (dxgi_manager_dll && !create_dxgi_device_manager_) {
        create_dxgi_device_manager_ = reinterpret_cast<CreateDXGIDeviceManager>(::GetProcAddress(dxgi_manager_dll, "MFCreateDXGIDeviceManager"));
    }

    RETURN_ON_FAILURE(make_context_current_cb_.Run(), "Failed to make context current", false);

    gl::GLDisplayEGL* display = gl::GLDisplayEGL::GetDisplayForCurrentContext();
    RETURN_ON_FAILURE(
        display && display->ext->b_EGL_ANGLE_surface_d3d_texture_2d_share_handle, "EGL_ANGLE_surface_d3d_texture_2d_share_handle unavailable", false);

    RETURN_ON_FAILURE(gl::GLFence::IsSupported(), "GL fences are unsupported", false);

    State state = GetState();
    RETURN_ON_FAILURE((state == kUninitialized), "Initialize: invalid state: " << state, false);

    RETURN_ON_FAILURE(InitializeMediaFoundation(), "Could not initialize Media Foundation", false);

    config_ = config;

    RETURN_ON_FAILURE(InitDecoder(config.profile), "Failed to initialize decoder", false);
    // Record this after we see if it works.
    UMA_HISTOGRAM_BOOLEAN("Media.DXVAVDA.UseD3D11", use_dx11_);

    RETURN_ON_FAILURE(GetStreamsInfoAndBufferReqs(), "Failed to get input/output stream info.", false);

    RETURN_ON_FAILURE(SendMFTMessage(MFT_MESSAGE_NOTIFY_BEGIN_STREAMING, 0), "Send MFT_MESSAGE_NOTIFY_BEGIN_STREAMING notification failed", false);

    RETURN_ON_FAILURE(SendMFTMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0), "Send MFT_MESSAGE_NOTIFY_START_OF_STREAM notification failed", false);
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.DXVAVDA.PictureBufferMechanism", GetPictureBufferMechanism());
...
bool DXVAVideoDecodeAccelerator::CreateD3DDevManager()
{
    ...
    hr = Direct3DCreate9ExXp(D3D_SDK_VERSION, &d3d9_);
    RETURN_ON_HR_FAILURE(hr, "Direct3DCreate9Ex failed", false);

    hr = d3d9_->CheckDeviceFormatConversion(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, static_cast<D3DFORMAT>(MAKEFOURCC('N', 'V', '1', '2')), D3DFMT_X8R8G8B8);
    RETURN_ON_HR_FAILURE(hr, "D3D9 driver does not support H/W format conversion", false);

    if (auto angle_device = gl::QueryD3D9DeviceObjectFromANGLE()) {
        using_angle_device_ = true;
        hr = angle_device.As(&d3d9_device_ex_);
        RETURN_ON_HR_FAILURE(hr, "QueryInterface for IDirect3DDevice9Ex from angle device failed", false);
    } else {
        ...
        hr = d3d9_->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, NULL,
            D3DCREATE_FPU_PRESERVE | D3DCREATE_MIXED_VERTEXPROCESSING | D3DCREATE_MULTITHREADED, &present_params, NULL, &d3d9_device_ex_);
        RETURN_ON_HR_FAILURE(hr, "Failed to create D3D device", false);
    }

    hr = DXVA2CreateDirect3DDeviceManager9Xp(&dev_manager_reset_token_, &device_manager_);
    RETURN_ON_HR_FAILURE(hr, "DXVA2CreateDirect3DDeviceManager9 failed", false);

    hr = device_manager_->ResetDevice(d3d9_device_ex_.Get(), dev_manager_reset_token_);
...
bool DXVAVideoDecodeAccelerator::CreateVideoProcessor()
{
    ...
    if (video_processor_service_.Get())
        return true;
    HRESULT hr = DXVA2CreateVideoServiceXp(d3d9_device_ex_.Get(), IID_PPV_ARGS(&video_processor_service_));
    ...
    hr = video_processor_service_->GetVideoProcessorDeviceGuids(&inputDesc, &guid_count, &guids);
    RETURN_ON_HR_FAILURE(hr, "GetVideoProcessorDeviceGuids failed", false);

    for (UINT g = 0; g < guid_count; g++) {
        DXVA2_VideoProcessorCaps caps;
        hr = video_processor_service_->GetVideoProcessorCaps(guids[g], &inputDesc, D3DFMT_X8R8G8B8, &caps);
        if (hr)
            continue;
        ...
        UINT format_count = 0;
        hr = video_processor_service_->GetVideoProcessorRenderTargets(guids[g], &inputDesc, &format_count, &formats);
        if (hr)
            continue;
        ...
        hr = video_processor_service_->CreateVideoProcessor(guids[g], &inputDesc, D3DFMT_X8R8G8B8, 0, &processor_);
        if (hr)
            continue;
...
bool DXVAVideoDecodeAccelerator::CreateDX11DevManager()
{
    ...
    HRESULT hr = create_dxgi_device_manager_(&dx11_dev_manager_reset_token_, &d3d11_device_manager_);
    ...
    if (ShouldUseANGLEDevice()) {
        ...
        hr = D3D11CreateDeviceXp(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, feature_levels, std::size(feature_levels), D3D11_SDK_VERSION, &d3d11_device_,
            &feature_level_out, &d3d11_device_context_);
        if (hr == DXGI_ERROR_SDK_COMPONENT_MISSING) {
            LOG(ERROR) << "Debug DXGI device creation failed, falling back to release.";
            flags &= ~D3D11_CREATE_DEVICE_DEBUG;
        } else {
        ...
        RETURN_ON_HR_FAILURE(hr, media::SetDebugName(d3d11_device_.Get(), "DXVA_DecodeAccelerator"), false);
    ...
    hr = D3D11Device()->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS, &options, sizeof(options));
    ...
    FormatSupportChecker checker(ShouldUseANGLEDevice() ? angle_device_ : d3d11_device_);
    RETURN_ON_FAILURE(checker.Initialize(), "Failed to check format supports!", false);
    ...
    hr = d3d11_device_manager_->ResetDevice(D3D11Device(), dx11_dev_manager_reset_token_);
    RETURN_ON_HR_FAILURE(hr, "Failed to reset device", false);
...
void DXVAVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    TRACE_EVENT0("media", "DXVAVideoDecodeAccelerator::Decode");
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());

    RETURN_AND_NOTIFY_ON_FAILURE(bitstream_id >= 0, "Invalid bitstream, id: " << bitstream_id, INVALID_ARGUMENT, );
    ...
    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state == kNormal || state == kStopped || state == kFlushing), "Invalid state: " << state, ILLEGAL_STATE, );

    Microsoft::WRL::ComPtr<IMFSample> sample;
    sample = CreateInputSample(
        buffer->data(), buffer->data_size(), std::min<uint32_t>(buffer->data_size(), input_stream_info_.cbSize), input_stream_info_.cbAlignment);
    RETURN_AND_NOTIFY_ON_FAILURE(sample.Get(), "Failed to create input sample", PLATFORM_FAILURE, );

    RETURN_AND_NOTIFY_ON_HR_FAILURE(sample->SetSampleTime(bitstream_id), "Failed to associate input buffer id with sample", PLATFORM_FAILURE, );

    decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::DecodeInternal, base::Unretained(this), sample));
}

void DXVAVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());

    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state != kUninitialized), "Invalid state: " << state, ILLEGAL_STATE, );
    RETURN_AND_NOTIFY_ON_FAILURE((num_picture_buffers_requested_ <= static_cast<int>(buffers.size())),
        "Failed to provide requested picture buffers. (Got " << buffers.size() << ", requested " << num_picture_buffers_requested_ << ")", INVALID_ARGUMENT, );

    RETURN_AND_NOTIFY_ON_FAILURE(make_context_current_cb_.Run(), "Failed to make context current", PLATFORM_FAILURE, );
    ...
    for (size_t buffer_index = 0; buffer_index < buffers.size(); ++buffer_index) {
        std::unique_ptr<DXVAPictureBuffer> picture_buffer = DXVAPictureBuffer::Create(*this, buffers[buffer_index], egl_config_);
        RETURN_AND_NOTIFY_ON_FAILURE(picture_buffer.get(), "Failed to allocate picture buffer", PLATFORM_FAILURE, );
        if (bind_image_cb_) {
            for (uint32_t client_id : buffers[buffer_index].client_texture_ids()) {
                ...
                bind_image_cb_.Run(client_id, GetTextureTarget(), picture_buffer->gl_image(), false);
            }
        }

        bool inserted = output_picture_buffers_.insert(std::make_pair(buffers[buffer_index].id(), std::move(picture_buffer))).second;
        DCHECK(inserted);
    }

    ProcessPendingSamples();
    if (pending_flush_ || processing_config_changed_) {
        decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::FlushInternal, base::Unretained(this)));
    }
}
...
void DXVAVideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_buffer_id)
{
    ...
    if (it == output_picture_buffers_.end()) {
        if (!stale_output_picture_buffers_.empty()) {
            ...
            RETURN_AND_NOTIFY_ON_FAILURE(it != stale_output_picture_buffers_.end(), "Invalid picture id: " << picture_buffer_id, INVALID_ARGUMENT, );
            main_thread_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::DeferredDismissStaleBuffer, weak_ptr_, picture_buffer_id));
        }
        return;
    }
    ...
    if (use_keyed_mutex_ || using_angle_device_) {
        RETURN_AND_NOTIFY_ON_FAILURE(it->second->ReusePictureBuffer(), "Failed to reuse picture buffer", PLATFORM_FAILURE, );
        if (bind_image_cb_ && (GetPictureBufferMechanism() == PictureBufferMechanism::DELAYED_COPY_TO_NV12)) {
            // Unbind the image to ensure it will be copied again the next time it's
            // needed.
            for (uint32_t client_id : it->second->picture_buffer().client_texture_ids()) {
                bind_image_cb_.Run(client_id, GetTextureTarget(), it->second->gl_image(), false);
            }
        }
...
void DXVAVideoDecodeAccelerator::WaitForOutputBuffer(int32_t picture_buffer_id, int count)
{
    ...
    if (count <= kMaxIterationsForANGLEReuseFlush && !fence->HasCompleted()) {
        main_thread_task_runner_->PostDelayedTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::WaitForOutputBuffer, weak_ptr_, picture_buffer_id, count + 1),
            base::Milliseconds(kFlushDecoderSurfaceTimeoutMs));
        return;
    }
    RETURN_AND_NOTIFY_ON_FAILURE(picture_buffer->ReusePictureBuffer(), "Failed to reuse picture buffer", PLATFORM_FAILURE, );
...
void DXVAVideoDecodeAccelerator::Flush()
{
    ...
    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state == kNormal || state == kStopped), "Unexpected decoder state: " << state, ILLEGAL_STATE, );
...
void DXVAVideoDecodeAccelerator::Reset()
{
    ...
    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE((state == kNormal || state == kStopped || state == kFlushing), "Reset: invalid state: " << state, ILLEGAL_STATE, );
    ...
    for (index = pending_output_samples_.begin(); index != pending_output_samples_.end(); ++index) {
    ...
    RETURN_AND_NOTIFY_ON_FAILURE(SendMFTMessage(MFT_MESSAGE_COMMAND_FLUSH, 0), "Reset: Failed to send message.", PLATFORM_FAILURE, );

    main_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::NotifyInputBuffersDropped, weak_ptr_, std::move(pending_input_buffers_)));
    pending_input_buffers_.clear();
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::NotifyResetDone, weak_ptr_));

    RETURN_AND_NOTIFY_ON_FAILURE(StartDecoderThread(), "Failed to start decoder thread.", PLATFORM_FAILURE, );
    SetState(kNormal);
}
...
bool DXVAVideoDecodeAccelerator::TryToSetupDecodeOnSeparateThread(
    const base::WeakPtr<Client>& decode_client, const scoped_refptr<base::SingleThreadTaskRunner>& decode_task_runner)
{
    return false;
}
...
// static
VideoDecodeAccelerator::SupportedProfiles DXVAVideoDecodeAccelerator::GetSupportedProfiles(
    const gpu::GpuPreferences& preferences, const gpu::GpuDriverBugWorkarounds& workarounds)
{
    TRACE_EVENT0("gpu,startup", "DXVAVideoDecodeAccelerator::GetSupportedProfiles");
    ...
    const auto supported_resolutions = GetSupportedD3D11VideoDecoderResolutions(gl::QueryD3D11DeviceObjectFromANGLE(), workarounds);
    for (const auto& kv : supported_resolutions) {
        ...
        if (!resolution_range.max_portrait_resolution.IsEmpty() && resolution_range.max_portrait_resolution != resolution_range.max_landscape_resolution) {
            SupportedProfile profile;
            profile.profile = kv.first;
            profile.min_resolution = resolution_range.min_resolution;
...
void DXVAVideoDecodeAccelerator::PreSandboxInitialization()
{
    ...
    if (base::win::GetVersion() >= base::win::Version::WIN8) {
        ...
        if (!LoadLibrary(L"mshtmlmedia.dll"))
            PLOG(ERROR) << "DXVAVDA fatal error: could not LoadLibrary: mshtmlmedia.dll";
#endif
    }
}
...
bool DXVAVideoDecodeAccelerator::InitDecoder(VideoCodecProfile profile)
{
    ...
    if (profile >= H264PROFILE_MIN && profile <= H264PROFILE_MAX) {
        ...
        std::unique_ptr<FileVersionInfo> version_info(FileVersionInfo::CreateFileVersionInfoForModule(decoder_dll));
        RETURN_ON_FAILURE(version_info, "unable to get version of msmpeg2vdec.dll", false);
        std::u16string file_version = version_info->file_version();
        RETURN_ON_FAILURE(file_version.find(u"6.1.7140") == std::u16string::npos, "blocked version of msmpeg2vdec.dll 6.1.7140", false);
        codec_ = VideoCodec::kH264;
        clsid = __uuidof(CMSH264DecoderMFT);
    } else if ((profile >= VP9PROFILE_PROFILE0 && profile <= VP9PROFILE_PROFILE3) || profile == VP8PROFILE_ANY) {
        codec_ = profile == VP8PROFILE_ANY ? VideoCodec::kVP8 : VideoCodec::kVP9;
        if ((codec_ == VideoCodec::kVP8 && enable_accelerated_vp8_decode_) || (codec_ == VideoCodec::kVP9 && enable_accelerated_vp9_decode_)) {
    ...
    if (enable_accelerated_av1_decode_ && profile >= AV1PROFILE_MIN && profile <= AV1PROFILE_MAX) {
        ...
        HRESULT hr = CreateCOMObjectFromDll(decoder_dll, clsid, IID_PPV_ARGS(&decoder_));
        RETURN_ON_HR_FAILURE(hr, "Failed to create decoder instance", false);
    }

    RETURN_ON_FAILURE(CheckDecoderDxvaSupport(), "Failed to check decoder DXVA support", false);
    ...
    if (use_dx11_) {
        ...
        RETURN_ON_FAILURE(CreateDX11DevManager(), "Failed to initialize DX11 device and manager", false);
        ...
        RETURN_ON_FAILURE(CreateD3DDevManager(), "Failed to initialize D3D device and manager", false);
        device_manager_to_use = reinterpret_cast<ULONG_PTR>(device_manager_.Get());
    }

    HRESULT hr = decoder_->ProcessMessage(MFT_MESSAGE_SET_D3D_MANAGER, device_manager_to_use);
    if (use_dx11_) {
    ...
    while (true) {
        std::vector<EGLint> config_attribs = { EGL_BUFFER_SIZE, 32, EGL_RED_SIZE, use_fp16_ ? 16 : 8, EGL_GREEN_SIZE, use_fp16_ ? 16 : 8, EGL_BLUE_SIZE,
            use_fp16_ ? 16 : 8, EGL_SURFACE_TYPE, EGL_PBUFFER_BIT, EGL_ALPHA_SIZE, 0 };
        if (use_fp16_) {
        ...
        if (eglChooseConfig(egl_display, config_attribs.data(), NULL, 0, &num_configs) && num_configs > 0) {
            std::vector<EGLConfig> configs(num_configs);
            if (eglChooseConfig(egl_display, config_attribs.data(), configs.data(), num_configs, &num_configs)) {
...
bool DXVAVideoDecodeAccelerator::CheckDecoderDxvaSupport()
{
    ...
    attributes->SetUINT32(MF_SA_MINIMUM_OUTPUT_SAMPLE_COUNT_PROGRESSIVE, kMaxOutputSamples);
    ...
    use_keyed_mutex_ = use_dx11_ && display && display->ext->b_EGL_ANGLE_keyed_mutex;

    if (!use_dx11_ || !display || !display->ext->b_EGL_ANGLE_stream_producer_d3d_texture || !display->ext->b_EGL_KHR_stream
        || !display->ext->b_EGL_KHR_stream_consumer_gltexture || !display->ext->b_EGL_NV_stream_consumer_gltexture_yuv) {
        DisableSharedTextureSupport();
        support_copy_nv12_textures_ = false;
    }
...
{
    RETURN_ON_FAILURE(SetDecoderInputMediaType(), "Failed to set decoder input media type", false);
    return SetDecoderOutputMediaType(MFVideoFormat_NV12) || SetDecoderOutputMediaType(MFVideoFormat_P010) || SetDecoderOutputMediaType(MFVideoFormat_P016);
}

bool DXVAVideoDecodeAccelerator::SetDecoderInputMediaType()
{
    Microsoft::WRL::ComPtr<IMFMediaType> media_type;
    HRESULT hr = MFCreateMediaTypeXp(&media_type);
    ...
    if (using_ms_vpx_mft_) {
        hr = MFSetAttributeSize(media_type.Get(), MF_MT_FRAME_SIZE, config_.initial_expected_coded_size.width(), config_.initial_expected_coded_size.height());
    ...
    if (GetPictureBufferMechanism() == PictureBufferMechanism::BIND) {
        out_attributes->SetUINT32(MF_SA_D3D11_BINDFLAGS, D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_DECODER);
...
bool DXVAVideoDecodeAccelerator::SetDecoderOutputMediaType(const GUID& subtype)
{
    return SetTransformOutputType(decoder_.Get(), subtype, 0, 0);
}

bool DXVAVideoDecodeAccelerator::SendMFTMessage(MFT_MESSAGE_TYPE msg, int32_t param)
{
...
bool DXVAVideoDecodeAccelerator::GetStreamsInfoAndBufferReqs()
{
    ...
    DVLOG(1) << "Flags: " << std::hex << std::showbase << output_stream_info_.dwFlags;
...
void DXVAVideoDecodeAccelerator::DoDecode(const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space)
{
    ...
    RETURN_AND_NOTIFY_ON_FAILURE(
        (state == kNormal || state == kFlushing || state == kStopped), "DoDecode: not in normal/flushing/stopped state", ILLEGAL_STATE, );
            ...
            if (hr == MF_E_TRANSFORM_STREAM_CHANGE) {
                if (!SetDecoderOutputMediaType(MFVideoFormat_NV12) && !SetDecoderOutputMediaType(MFVideoFormat_P010)
                    ...
                    if (retries-- > 0) {
                        ...
                        RETURN_AND_NOTIFY_ON_FAILURE(false, "Received too many format changes from decoder.", PLATFORM_FAILURE, );
                    }
                }
                return;
                ...
                SetState(kStopped);
                RETURN_AND_NOTIFY_ON_FAILURE(hr, "Unhandled error in DoDecode()", PLATFORM_FAILURE, );
            }
        }

        break; // No more retries needed.
    }
    TRACE_EVENT_NESTABLE_ASYNC_END0("gpu", "DXVAVideoDecodeAccelerator.Decoding", TRACE_ID_LOCAL(this));

    TRACE_COUNTER1("DXVA_Decoding", "TotalPacketsBeforeDecode", inputs_before_decode_);

    inputs_before_decode_ = 0;

    RETURN_AND_NOTIFY_ON_FAILURE(ProcessOutputSample(output_sample, visible_rect, color_space), "Failed to process output sample.", PLATFORM_FAILURE, );
}

bool DXVAVideoDecodeAccelerator::ProcessOutputSample(
    Microsoft::WRL::ComPtr<IMFSample> sample, const gfx::Rect& visible_rect, const gfx::ColorSpace& color_space)
{
    RETURN_ON_FAILURE(sample, "Decode succeeded with NULL output sample", false);

    LONGLONG input_buffer_id = 0;
    RETURN_ON_HR_FAILURE(sample->GetSampleTime(&input_buffer_id), "Failed to get input buffer id associated with sample", false);

    {
        base::AutoLock lock(decoder_lock_);
        DCHECK(pending_output_samples_.empty());
        pending_output_samples_.push_back(PendingSampleInfo(input_buffer_id, sample, visible_rect, color_space));
    }

    if (pictures_requested_) {
        DVLOG(1) << "Waiting for picture slots from the client.";
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::ProcessPendingSamples, weak_ptr_));
        return true;
    }
    ...
    if (!GetVideoFrameDimensions(sample.Get(), &width, &height)) {
        RETURN_ON_FAILURE(false, "Failed to get D3D surface from output sample", false);
    }

    // Go ahead and request picture buffers.
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::RequestPictureBuffers, weak_ptr_, width, height));
...
void DXVAVideoDecodeAccelerator::ProcessPendingSamples()
{
    ...
    for (index = output_picture_buffers_.begin(); index != output_picture_buffers_.end() && OutputSamplesPresent(); ++index) {
        if (index->second->available()) {
            ...
            int width = 0;
            int height = 0;
            if (!GetVideoFrameDimensions(pending_sample->output_sample.Get(), &width, &height)) {
                RETURN_AND_NOTIFY_ON_FAILURE(false, "Failed to get D3D surface from output sample", PLATFORM_FAILURE, );
            }
            ...
            if (index->second->CanBindSamples()) {
                main_thread_task_runner_->PostTask(FROM_HERE,
                    base::BindOnce(&DXVAVideoDecodeAccelerator::BindPictureBufferToSample, weak_ptr_, pending_sample->output_sample,
                        pending_sample->picture_buffer_id, pending_sample->input_buffer_id));
                continue;
            }

            Microsoft::WRL::ComPtr<IMFMediaBuffer> output_buffer;
            HRESULT hr = pending_sample->output_sample->GetBufferByIndex(0, &output_buffer);
            RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get buffer from output sample", PLATFORM_FAILURE, );
            ...
            if (use_dx11_) {
                ...
                RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get DXGIBuffer from output sample", PLATFORM_FAILURE, );
                hr = dxgi_buffer->GetResource(__uuidof(ID3D11Texture2D), &d3d11_texture);
            } else {
                hr = MFGetServiceXp(output_buffer.Get(), MR_BUFFER_SERVICE, IID_PPV_ARGS(&surface));
            }
            RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get surface from output sample", PLATFORM_FAILURE, );

            RETURN_AND_NOTIFY_ON_FAILURE(
                index->second->CopyOutputSampleDataToPictureBuffer(this, surface.Get(), d3d11_texture.Get(), pending_sample->input_buffer_id),
                "Failed to copy output sample", PLATFORM_FAILURE, );
        }
    }
}

void DXVAVideoDecodeAccelerator::StopOnError(VideoDecodeAccelerator::Error error)
{
    if (!main_thread_task_runner_->BelongsToCurrentThread()) {
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::StopOnError, weak_ptr_, error));
        return;
    }
...
void DXVAVideoDecodeAccelerator::RequestPictureBuffers(int width, int height)
{
    ...
    if (GetState() != kUninitialized && client_) {
        ...
        bool provide_nv12_textures = GetPictureBufferMechanism() != PictureBufferMechanism::COPY_TO_RGB;
        client_->ProvidePictureBuffers(num_picture_buffers_requested_, provide_nv12_textures ? PIXEL_FORMAT_NV12 : PIXEL_FORMAT_UNKNOWN,
            provide_nv12_textures ? 2 : 1, gfx::Size(width, height), GetTextureTarget());
    }
}

void DXVAVideoDecodeAccelerator::NotifyPictureReady(int picture_buffer_id, int input_buffer_id, const gfx::Rect& visible_rect,
    const gfx::ColorSpace& color_space, bool allow_overlay, std::vector<scoped_refptr<Picture::ScopedSharedImage>> shared_images)
{
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());
    // This task could execute after the decoder has been torn down.
    if (GetState() != kUninitialized && client_) {
        Picture picture(picture_buffer_id, input_buffer_id, visible_rect, color_space, allow_overlay);
...
void DXVAVideoDecodeAccelerator::NotifyInputBuffersDropped(const PendingInputs& pending_buffers)
{
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());
    if (!client_)
        return;
    ...
    for (const auto& buffer : pending_buffers) {
        LONGLONG input_buffer_id = 0;
        RETURN_ON_HR_FAILURE(buffer->GetSampleTime(&input_buffer_id), "Failed to get buffer id associated with sample", );
        client_->NotifyEndOfBitstreamBuffer(input_buffer_id);
    }
}
...
{
    TRACE_EVENT0("media", "DXVAVideoDecodeAccelerator::DecodePendingInputBuffers");
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
...
void DXVAVideoDecodeAccelerator::FlushInternal()
{
    ...
    if (!processing_config_changed_ && !pending_input_buffers_.empty()) {
        decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::DecodePendingInputBuffers, base::Unretained(this)));
        decoder_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::FlushInternal, base::Unretained(this)));
        return;
    }

    {
        ...
        if (!sent_drain_message_) {
            RETURN_AND_NOTIFY_ON_FAILURE(SendMFTMessage(MFT_MESSAGE_COMMAND_DRAIN, 0), "Failed to send drain message", PLATFORM_FAILURE, );
    ...
    if (!processing_config_changed_) {
        ...
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::NotifyFlushDone, weak_ptr_));
    } else {
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::ConfigChanged, weak_ptr_, config_));
    }

    SetState(kNormal);
}

void DXVAVideoDecodeAccelerator::DecodeInternal(const Microsoft::WRL::ComPtr<IMFSample>& sample)
{
    ...
    if (OutputSamplesPresent() || !pending_input_buffers_.empty() || processing_config_changed_) {
    ...
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to check video stream config", PLATFORM_FAILURE, );

    if (disallow_vp9_resilient_dxva_decoding_ && config_change_detector_ && config_change_detector_->is_vp9_resilient_mode()) {
        RETURN_AND_NOTIFY_ON_HR_FAILURE(E_FAIL, "Incompatible GPU for VP9 resilient mode", PLATFORM_FAILURE, );
    }

    // https://crbug.com/1160623 -- non 4:2:0 content hangs the decoder.
    RETURN_AND_NOTIFY_ON_FAILURE(
        codec_ != VideoCodec::kH264 || config_change_detector_->IsYUV420(), "Only 4:2:0 H.264 content is supported", PLATFORM_FAILURE, );
    ...
    if (!inputs_before_decode_) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("gpu", "DXVAVideoDecodeAccelerator.Decoding", TRACE_ID_LOCAL(this));
    }
    ...
    if (hr == MF_E_NOTACCEPTING) {
        ...
        if (!OutputSamplesPresent()) {
            State state = GetState();
            RETURN_AND_NOTIFY_ON_FAILURE((state == kStopped || state == kNormal || state == kFlushing),
                "Failed to process output. Unexpected decoder state: " << state, PLATFORM_FAILURE, );
            hr = decoder_->ProcessInput(0, sample.Get(), 0);
        }
        ...
        if (hr == MF_E_NOTACCEPTING) {
            ...
            if (inputs_before_decode_ >= 1000) {
                ...
                RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Input processing appears stuck in MF_E_NOTACCEPTING loop.", PLATFORM_FAILURE, );
            }
    ...
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to process input sample", PLATFORM_FAILURE, );

    DoDecode(visible_rect, color_space.ToGfxColorSpace());

    State state = GetState();
    RETURN_AND_NOTIFY_ON_FAILURE(
        (state == kStopped || state == kNormal || state == kFlushing), "Failed to process output. Unexpected decoder state: " << state, ILLEGAL_STATE, );

    LONGLONG input_buffer_id = 0;
    RETURN_ON_HR_FAILURE(sample->GetSampleTime(&input_buffer_id), "Failed to get input buffer id associated with sample", );
    ...
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::NotifyInputBufferRead, weak_ptr_, input_buffer_id));
}

void DXVAVideoDecodeAccelerator::HandleResolutionChanged(int width, int height)
{
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::DismissStaleBuffers, weak_ptr_, false));
...
void DXVAVideoDecodeAccelerator::DismissStaleBuffers(bool force)
{
    ...
    for (index = output_picture_buffers_.begin(); index != output_picture_buffers_.end(); ++index) {
        if (force || index->second->available()) {
            ...
            stale_output_picture_buffers_.insert(std::make_pair(index->first, std::move(index->second)));
        }
    }
...
void DXVAVideoDecodeAccelerator::DeferredDismissStaleBuffer(int32_t picture_buffer_id)
{
    RETURN_AND_NOTIFY_ON_FAILURE(make_context_current_cb_.Run(), "Failed to make context current", PLATFORM_FAILURE, );
...
DXVAVideoDecodeAccelerator::State DXVAVideoDecodeAccelerator::GetState()
{
    ...
    State state = static_cast<State>(InterlockedAdd(reinterpret_cast<volatile long*>(&state_), 0));
    return state;
}
...
{
    if (!main_thread_task_runner_->BelongsToCurrentThread()) {
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::SetState, weak_ptr_, new_state));
        return;
    }
...
void DXVAVideoDecodeAccelerator::CopySurface(
    IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id, const gfx::ColorSpace& color_space)
{
    ...
    if (!decoder_thread_task_runner_->BelongsToCurrentThread()) {
        decoder_thread_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::CopySurface, base::Unretained(this), base::Unretained(src_surface), base::Unretained(dest_surface),
    ...
    if (processor_) {
        ...
        if (hr != S_OK) {
            LOG(ERROR) << "VideoProcessBlt failed with code " << hr << "  E_INVALIDARG= " << E_INVALIDARG;
    ...
    if (!processor_) {
        hr = d3d9_device_ex_->StretchRect(src_surface, NULL, dest_surface, NULL, D3DTEXF_NONE);
    ...
    if (using_angle_device_) {
        main_thread_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::CopySurfaceComplete, weak_ptr_, base::Unretained(src_surface), base::Unretained(dest_surface),
    ...
    // target surface.
    decoder_thread_task_runner_->PostDelayedTask(FROM_HERE,
        base::BindOnce(&DXVAVideoDecodeAccelerator::FlushDecoder, base::Unretained(this), 0, base::Unretained(src_surface), base::Unretained(dest_surface),
            picture_buffer_id, input_buffer_id),
        base::Milliseconds(kFlushDecoderSurfaceTimeoutMs));
}

void DXVAVideoDecodeAccelerator::CopySurfaceComplete(
    IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id)
{
    ...
    RETURN_AND_NOTIFY_ON_FAILURE(result, "Failed to complete copying surface", PLATFORM_FAILURE, );

    std::vector<scoped_refptr<Picture::ScopedSharedImage>> scoped_shared_images = GetSharedImagesFromPictureBuffer(picture_buffer);

    NotifyPictureReady(picture_buffer->id(), input_buffer_id, picture_buffer->visible_rect(), picture_buffer->color_space(), picture_buffer->AllowOverlay(),
...
void DXVAVideoDecodeAccelerator::BindPictureBufferToSample(Microsoft::WRL::ComPtr<IMFSample> sample, int picture_buffer_id, int input_buffer_id)
{
    TRACE_EVENT0("media", "DXVAVideoDecodeAccelerator::BindPictureBufferToSample");
    DCHECK(main_thread_task_runner_->BelongsToCurrentThread());
    ...
    RETURN_AND_NOTIFY_ON_FAILURE(result, "Failed to bind sample to texture", PLATFORM_FAILURE, );
...
bool DXVAVideoDecodeAccelerator::CopyTexture(ID3D11Texture2D* src_texture, ID3D11Texture2D* dest_texture,
    Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex, uint64_t keyed_mutex_value, int picture_buffer_id, int input_buffer_id,
    const gfx::ColorSpace& color_space)
{
    ...
    if (!InitializeID3D11VideoProcessor(source_desc.Width, source_desc.Height, color_space)) {
        RETURN_AND_NOTIFY_ON_FAILURE(false, "Failed to initialize D3D11 video processor.", PLATFORM_FAILURE, false);
    }
    ...
    decoder_thread_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&DXVAVideoDecodeAccelerator::CopyTextureOnDecoderThread, base::Unretained(this), base::Unretained(dest_texture), dest_keyed_mutex,
            keyed_mutex_value, input_sample_for_conversion, picture_buffer_id, input_buffer_id));
    return true;
}

void DXVAVideoDecodeAccelerator::CopyTextureOnDecoderThread(ID3D11Texture2D* dest_texture, Microsoft::WRL::ComPtr<IDXGIKeyedMutex> dest_keyed_mutex,
    uint64_t keyed_mutex_value, Microsoft::WRL::ComPtr<IMFSample> input_sample, int picture_buffer_id, int input_buffer_id)
{
    TRACE_EVENT0("media", "DXVAVideoDecodeAccelerator::CopyTextureOnDecoderThread");
    DCHECK(decoder_thread_task_runner_->BelongsToCurrentThread());
    HRESULT hr = E_FAIL;
    ...
    if (dest_keyed_mutex) {
        ...
        RETURN_AND_NOTIFY_ON_FAILURE(hr == S_OK, "D3D11 failed to acquire keyed mutex for texture.", PLATFORM_FAILURE, );
    }
    ...
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get resource index", PLATFORM_FAILURE, );

    ComD3D11Texture2D dx11_decoding_texture;
    hr = dxgi_buffer->GetResource(IID_PPV_ARGS(&dx11_decoding_texture));
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get resource from output sample", PLATFORM_FAILURE, );

    D3D11_VIDEO_PROCESSOR_OUTPUT_VIEW_DESC output_view_desc = { D3D11_VPOV_DIMENSION_TEXTURE2D };
    output_view_desc.Texture2D.MipSlice = 0;
    ComD3D11VideoProcessorOutputView output_view;
    hr = video_device_->CreateVideoProcessorOutputView(dest_texture, enumerator_.Get(), &output_view_desc, &output_view);
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get output view", PLATFORM_FAILURE, );
    ...
    hr = video_device_->CreateVideoProcessorInputView(dx11_decoding_texture.Get(), enumerator_.Get(), &input_view_desc, &input_view);
    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "Failed to get input view", PLATFORM_FAILURE, );

    D3D11_VIDEO_PROCESSOR_STREAM streams = { 0 };
    streams.Enable = TRUE;
    streams.pInputSurface = input_view.Get();

    hr = video_context_->VideoProcessorBlt(d3d11_processor_.Get(), output_view.Get(), 0, 1, &streams);

    RETURN_AND_NOTIFY_ON_HR_FAILURE(hr, "VideoProcessBlit failed", PLATFORM_FAILURE, );

    if (dest_keyed_mutex) {
        hr = dest_keyed_mutex->ReleaseSync(keyed_mutex_value + 1);
        RETURN_AND_NOTIFY_ON_FAILURE(hr == S_OK, "Failed to release keyed mutex.", PLATFORM_FAILURE, );

        main_thread_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&DXVAVideoDecodeAccelerator::CopySurfaceComplete, weak_ptr_, nullptr, nullptr, picture_buffer_id, input_buffer_id));
    } else {
        d3d11_device_context_->Flush();
        d3d11_device_context_->End(d3d11_query_.Get());

        decoder_thread_task_runner_->PostDelayedTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::FlushDecoder, base::Unretained(this), 0, nullptr, nullptr, picture_buffer_id, input_buffer_id),
            base::Milliseconds(kFlushDecoderSurfaceTimeoutMs));
    }
}

void DXVAVideoDecodeAccelerator::FlushDecoder(
    int iterations, IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface, int picture_buffer_id, int input_buffer_id)
{
    ...
    if (use_dx11_) {
        BOOL query_data = 0;
        hr = d3d11_device_context_->GetData(d3d11_query_.Get(), &query_data, sizeof(BOOL), 0);
        if (FAILED(hr))
            DCHECK(false);
    } else {
    ...
    if ((hr == S_FALSE) && (++iterations < kMaxIterationsForD3DFlush)) {
        decoder_thread_task_runner_->PostDelayedTask(FROM_HERE,
            base::BindOnce(&DXVAVideoDecodeAccelerator::FlushDecoder, base::Unretained(this), iterations, base::Unretained(src_surface),
...
bool DXVAVideoDecodeAccelerator::InitializeID3D11VideoProcessor(int width, int height, const gfx::ColorSpace& color_space)
{
    ...
    if (width != processor_width_ || height != processor_height_) {
        ...
        HRESULT hr = video_device_->CreateVideoProcessorEnumerator(&desc, &enumerator_);
        RETURN_ON_HR_FAILURE(hr, "Failed to enumerate video processors", false);

        // TODO(Hubbe): Find correct index
        hr = video_device_->CreateVideoProcessor(enumerator_.Get(), 0, &d3d11_processor_);
        RETURN_ON_HR_FAILURE(hr, "Failed to create video processor.", false);
        processor_width_ = width;
        processor_height_ = height;

        video_context_->VideoProcessorSetStreamAutoProcessingMode(d3d11_processor_.Get(), 0, false);
    ...
    if ((!use_color_info_ && !use_fp16_) || GetPictureBufferMechanism() == PictureBufferMechanism::COPY_TO_NV12
        || GetPictureBufferMechanism() == PictureBufferMechanism::DELAYED_COPY_TO_NV12) {
        const auto d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(color_space);
        video_context_->VideoProcessorSetOutputColorSpace(d3d11_processor_.Get(), &d3d11_color_space);
        video_context_->VideoProcessorSetStreamColorSpace(d3d11_processor_.Get(), 0, &d3d11_color_space);
    ...
    if (FAILED(video_context_.As(&video_context1))) {
        auto d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(color_space);
        video_context_->VideoProcessorSetStreamColorSpace(d3d11_processor_.Get(), 0, &d3d11_color_space);

        // Since older platforms won't have HDR, just use SRGB.
        dx11_converter_output_color_space_ = gfx::ColorSpace::CreateSRGB();
        d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(dx11_converter_output_color_space_);
        video_context_->VideoProcessorSetOutputColorSpace(d3d11_processor_.Get(), &d3d11_color_space);
        return true;
    }
    ...
    if (color_space.GetTransferID() == gfx::ColorSpace::TransferID::HLG) {
        video_context1->VideoProcessorSetStreamColorSpace1(d3d11_processor_.Get(), 0, DXGI_COLOR_SPACE_YCBCR_STUDIO_G2084_LEFT_P2020);
        video_context1->VideoProcessorSetOutputColorSpace1(d3d11_processor_.Get(), DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020);
    ...
    if (use_fp16_ && config_.target_color_space.IsHDR() && color_space.IsHDR()) {
        ...
        dx11_converter_output_color_space_ = gfx::ColorSpace::CreateSCRGBLinear80Nits();
    } else {
        dx11_converter_output_color_space_ = gfx::ColorSpace::CreateSRGB();
    }

    DVLOG(2) << "input color space: " << color_space << " DXGIColorSpace: " << gfx::ColorSpaceWin::GetDXGIColorSpace(color_space);
    DVLOG(2) << "output color space:" << dx11_converter_output_color_space_
             << " DXGIColorSpace: " << gfx::ColorSpaceWin::GetDXGIColorSpace(dx11_converter_output_color_space_);

    video_context1->VideoProcessorSetStreamColorSpace1(d3d11_processor_.Get(), 0, gfx::ColorSpaceWin::GetDXGIColorSpace(color_space));
    video_context1->VideoProcessorSetOutputColorSpace1(d3d11_processor_.Get(), gfx::ColorSpaceWin::GetDXGIColorSpace(dx11_converter_output_color_space_));
...
void DXVAVideoDecodeAccelerator::SetDX11ProcessorHDRMetadataIfNeeded()
{
    ...
    if (config_.hdr_metadata || use_empty_video_hdr_metadata_) {
        ...
        DXGI_HDR_METADATA_HDR10 dxgi_stream_metadata = gl::HDRMetadataHelperWin::HDRMetadataToDXGI(stream_metadata);

        video_context2->VideoProcessorSetStreamHDRMetaData(
            d3d11_processor_.Get(), 0, DXGI_HDR_METADATA_TYPE_HDR10, sizeof(dxgi_stream_metadata), &dxgi_stream_metadata);
    }

    video_context2->VideoProcessorSetOutputHDRMetaData(
        d3d11_processor_.Get(), DXGI_HDR_METADATA_TYPE_HDR10, sizeof(*dxgi_display_metadata), &(*dxgi_display_metadata));
}

bool DXVAVideoDecodeAccelerator::GetVideoFrameDimensions(IMFSample* sample, int* width, int* height)
{
    Microsoft::WRL::ComPtr<IMFMediaBuffer> output_buffer;
    ...
    if (use_dx11_) {
        ...
        ComD3D11Texture2D d3d11_texture;
        hr = output_buffer.As(&dxgi_buffer);
        RETURN_ON_HR_FAILURE(hr, "Failed to get DXGIBuffer from output sample", false);
        hr = dxgi_buffer->GetResource(__uuidof(ID3D11Texture2D), &d3d11_texture);
        RETURN_ON_HR_FAILURE(hr, "Failed to get D3D11Texture from output buffer", false);
        ...
        *height = d3d11_texture_desc.Height;
        ...
        Microsoft::WRL::ComPtr<IDirect3DSurface9> surface;
        hr = MFGetServiceXp(output_buffer.Get(), MR_BUFFER_SERVICE, IID_PPV_ARGS(&surface));
        RETURN_ON_HR_FAILURE(hr, "Failed to get D3D surface from output sample", false);
...
bool DXVAVideoDecodeAccelerator::SetTransformOutputType(IMFTransform* transform, const GUID& output_type, int width, int height)
{
    HRESULT hr = E_FAIL;
    Microsoft::WRL::ComPtr<IMFMediaType> media_type;

    for (uint32_t i = 0; SUCCEEDED(transform->GetOutputAvailableType(0, i, &media_type)); ++i) {
        GUID out_subtype = { 0 };
...
HRESULT DXVAVideoDecodeAccelerator::CheckConfigChanged(IMFSample* sample, bool* config_changed)
{
    if (!config_change_detector_) {
    ...
    if (!config_change_detector_->DetectConfig(scoped_media_buffer.get(), scoped_media_buffer.current_length())) {
        RETURN_ON_HR_FAILURE(E_FAIL, "Failed to detect H.264 stream config", E_FAIL);
    }
...
std::vector<scoped_refptr<Picture::ScopedSharedImage>> DXVAVideoDecodeAccelerator::GetSharedImagesFromPictureBuffer(DXVAPictureBuffer* picture_buffer)
{
    const size_t textures_per_picture = picture_buffer->service_texture_ids().size();

    // Get the viz resource format per texture.
    std::array<viz::ResourceFormat, VideoFrame::kMaxPlanes> viz_formats;
    {
        bool result = VideoPixelFormatToVizFormat(picture_buffer->pixel_format(), textures_per_picture, viz_formats);
        RETURN_AND_NOTIFY_ON_FAILURE(result, "Could not convert pixel format to viz format", PLATFORM_FAILURE, {});
    }
    ...
    for (uint32_t texture_idx = 0; texture_idx < textures_per_picture; texture_idx++) {
        // Usage flags to allow the display compositor to draw from it, video
        // to decode, and allow webgl/canvas access.
        constexpr uint32_t shared_image_usage = gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_RASTER
        ...
        scoped_refptr<gpu::gles2::TexturePassthrough> gl_texture
            = gpu::gles2::TexturePassthrough::CheckedCast(helper->GetTexture(picture_buffer->service_texture_ids()[texture_idx]));
        ...
        gl::GLImageDXGI* gl_image_dxgi = gl::GLImageDXGI::FromGLImage(picture_buffer->gl_image().get());
        if (gl_image_dxgi) {
            shared_image = gpu::D3DImageBacking::CreateFromGLTexture(mailbox, viz_formats[texture_idx], picture_buffer->texture_size(texture_idx),
                picture_buffer->color_space(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage, gl_image_dxgi->texture(),
                std::move(gl_texture));
        } else {
            shared_image = gpu::GLImageBacking::CreateFromGLTexture(picture_buffer->gl_image(), mailbox, viz_formats[texture_idx], picture_buffer->size(),
                picture_buffer->color_space(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage, GetTextureTarget(), std::move(gl_texture));
        }
        ...
        const bool success = shared_image_stub->factory()->RegisterBacking(std::move(shared_image));
        if (!success) {
            RETURN_AND_NOTIFY_ON_FAILURE(false, "Failed to register shared image", PLATFORM_FAILURE, {});
        }

        auto destroy_shared_image_callback
            = base::BindPostTask(main_thread_task_runner_, base::BindOnce(shared_image_stub->GetSharedImageDestructionCallback(mailbox), gpu::SyncToken()));

        // Wrap the factory ref with a scoped shared image. The factory ref
        // is used instead of requiring a destruction call-back.
        auto scoped_shared_image = base::MakeRefCounted<Picture::ScopedSharedImage>(mailbox, GetTextureTarget(), std::move(destroy_shared_image_callback));
...
DXVAVideoDecodeAccelerator::PictureBufferMechanism DXVAVideoDecodeAccelerator::GetPictureBufferMechanism() const
{
    if (use_fp16_)
