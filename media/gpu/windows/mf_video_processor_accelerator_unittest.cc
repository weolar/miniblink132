...
protected:
    ...
    HRESULT CreateTexture(UINT width, UINT height, DXGI_FORMAT format, BYTE* image_buffer, ID3D11Texture2D** texture_out)
    {
    ...
    std::vector<BYTE> CreateRGBCheckerboard(UINT width, UINT height, RGBQUAD color0, RGBQUAD color1)
    {
        std::vector<BYTE> image(width * height * 4);
        for (UINT y = 0; y < height; y++) {
            for (UINT x = 0; x < width; x++) {
                bool use_color0 = ((x % 4) < 2 && (y % 4) < 2) || ((x % 4) >= 2 && (y % 4) >= 2);
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> TextureToGpuMemoryBuffer(ID3D11Texture2D* texture, int width, int height)
    {
        ...
        std::unique_ptr<gfx::GpuMemoryBuffer> gmb = gpu::GpuMemoryBufferImplDXGI::CreateFromHandle(
            std::move(gmb_handle), { width, height }, gfx::BufferFormat::BGRA_8888, gfx::BufferUsage::GPU_READ, base::NullCallback(), nullptr, nullptr);
        return gmb;
    }

    template <typename F> void ValidateResult(ID3D11Texture2D* texture, UINT width, UINT height, F validation_func)
    {
        D3D11_TEXTURE2D_DESC desc;
        ...
        ASSERT_HRESULT_SUCCEEDED(d3d11_device_->CreateTexture2D(&desc, nullptr, &staging_texture));
        ...
        ASSERT_HRESULT_SUCCEEDED(d3d11_context->Map(staging_texture.Get(), 0, D3D11_MAP_READ, 0, &mapped_subresource));
    ...
    template <typename F> void ValidateResult(IMFMediaBuffer* buffer, UINT size, F validation_func)
    {
...
#define CheckForVideoDevice()                                                                                                                                  \
    ...
    if (FAILED(hr_has_video_device)) {                                                                                                                         \
        GTEST_SKIP() << " gpu accelerated video processing not available on this platform";                                                                    \
    }
...
TEST_F(MFVideoProcessorAcceleratorTest, RGBToNV12)
{
    ...
    video_processor = std::make_unique<MediaFoundationVideoProcessorAccelerator>(gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds());
    ...
    ASSERT_TRUE(video_processor->Initialize(config, dxgi_device_man_, std::make_unique<NullMediaLog>()));

    std::vector<BYTE> image = CreateRGBCheckerboard(kWidth, kHeight, kGreen, kMagenta);
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture;
    ASSERT_HRESULT_SUCCEEDED(CreateTexture(kWidth, kHeight, DXGI_FORMAT_B8G8R8A8_UNORM, image.data(), &texture));
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = TextureToGpuMemoryBuffer(texture.Get(), kWidth, kHeight);
    ASSERT_TRUE(gmb);
    auto timestamp = base::Milliseconds(0);
    auto frame = VideoFrame::WrapExternalGpuMemoryBuffer({ kWidth, kHeight }, { kWidth, kHeight }, std::move(gmb), timestamp);
    ...
    ASSERT_HRESULT_SUCCEEDED(dxgi_buffer->GetResource(IID_PPV_ARGS(&output_texture)));
...
TEST_F(MFVideoProcessorAcceleratorTest, RGBResize)
{
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> output_texture;
    ASSERT_HRESULT_SUCCEEDED(dxgi_buffer->GetResource(IID_PPV_ARGS(&output_texture)));
    ValidateResult(output_texture.Get(), kWidth / 2, kHeight / 2, [](BYTE* image) {
        EXPECT_EQ(image[0], 0);
...
TEST_F(MFVideoProcessorAcceleratorTest, RGBToNV12SizeChange)
{
    ...
    if (dxgi_adapter_desc.VendorId == kVendorID_Qualcomm || dxgi_adapter_desc.VendorId == kVendorID_Qualcomm_ACPI) {
        ...
        GTEST_SKIP() << " Test crashes on win11-arm64-dbg-tests run on older drivers";
    }
    ...
    std::vector<BYTE> image = CreateRGBCheckerboard(kWidth * 2, kHeight * 2, kGreen, kMagenta);
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture;
    ASSERT_HRESULT_SUCCEEDED(CreateTexture(kWidth * 2, kHeight * 2, DXGI_FORMAT_B8G8R8A8_UNORM, image.data(), &texture));
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = TextureToGpuMemoryBuffer(texture.Get(), kWidth * 2, kHeight * 2);
    ASSERT_TRUE(gmb);
    auto timestamp = base::Milliseconds(0);
    auto frame = VideoFrame::WrapExternalGpuMemoryBuffer({ kWidth * 2, kHeight * 2 }, { kWidth * 2, kHeight * 2 }, std::move(gmb), timestamp);
...
TEST_F(MFVideoProcessorAcceleratorTest, RGBToNV12CPU)
{
    ...
    config.output_visible_size = { kWidth, kHeight };
    config.output_color_space = gfx::ColorSpace::CreateREC709();
    ASSERT_TRUE(video_processor->Initialize(config, nullptr, std::make_unique<NullMediaLog>()));

    std::vector<BYTE> image = CreateRGBCheckerboard(kWidth, kHeight, kGreen, kMagenta);
    auto timestamp = base::Milliseconds(0);
    auto frame = VideoFrame::WrapExternalData(
        VideoPixelFormat::PIXEL_FORMAT_XRGB, { kWidth, kHeight }, gfx::Rect(0, 0, kWidth, kHeight), { kWidth, kHeight }, image.data(), image.size(), timestamp);
...
public:
    ...
    void BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata) override
    {
    }
...
TEST_F(MFVideoProcessorAcceleratorTest, RGBToH264)
{
    ...
    video_encoder
        = base::WrapUnique<VideoEncodeAccelerator>(new MediaFoundationVideoEncodeAccelerator(gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds(), { 0 }));
    VideoEncodeAccelerator::Config config(VideoPixelFormat::PIXEL_FORMAT_XRGB, { kWidth, kHeight }, H264PROFILE_MAIN, Bitrate::ConstantBitrate(4000000u), 30,
        VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer, VideoEncodeAccelerator::Config::ContentType::kDisplay);
    MockEncoderClient client;
    ASSERT_TRUE(video_encoder->Initialize(config, &client, std::make_unique<NullMediaLog>()));
    ...
    BitstreamBuffer buffer(kBitstreamBufferId, std::move(shmem), kShMemSize, 0 /* offset */, base::TimeDelta());
