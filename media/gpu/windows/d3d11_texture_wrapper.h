...
using GetCommandBufferHelperCB = base::RepeatingCallback<CommandBufferHelperPtr()>;
...
public:
    using PictureBufferGPUResourceInitDoneCB = base::OnceCallback<void(scoped_refptr<media::D3D11PictureBuffer>)>;

    Texture2DWrapper();
    virtual ~Texture2DWrapper();

    // Initialize the wrapper.
    virtual D3D11Status Init(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D texture,
        size_t array_size, scoped_refptr<media::D3D11PictureBuffer> picture_buffer, PictureBufferGPUResourceInitDoneCB picture_buffer_gpu_resource_init_done_cb)
        = 0;
    ...
    // used to refer to it.
    virtual D3D11Status ProcessTexture(const gfx::ColorSpace& input_color_space, scoped_refptr<gpu::ClientSharedImage>& shared_image_dest_out) = 0;
};
...
public:
    ...
    using GPUResourceInitCB = base::OnceCallback<void(
        scoped_refptr<media::D3D11PictureBuffer>, std::unique_ptr<gpu::VideoImageRepresentation>, scoped_refptr<gpu::ClientSharedImage>)>;
    ...
    DefaultTexture2DWrapper(const gfx::Size& size, const gfx::ColorSpace& color_space, DXGI_FORMAT dxgi_format, ComD3D11Device device);
    ...
    D3D11Status Init(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D in_texture,
        ...
        Texture2DWrapper::PictureBufferGPUResourceInitDoneCB picture_buffer_gpu_resource_init_done_cb) override;
    ...
    void OnGPUResourceInitDone(scoped_refptr<media::D3D11PictureBuffer> picture_buffer, std::unique_ptr<gpu::VideoImageRepresentation> shared_image_rep,
        scoped_refptr<gpu::ClientSharedImage> client_shared_image);

    ComD3D11Device GetVideoDevice()
    {
...
private:
    ...
    class GpuResources {
    public:
        GpuResources(OnErrorCB on_error_cb, GetCommandBufferHelperCB get_helper_cb, const gfx::Size& size, const gfx::ColorSpace& color_space,
            DXGI_FORMAT dxgi_format, ComD3D11Device video_device, ComD3D11Texture2D texture, size_t array_slice,
    ...
    std::unique_ptr<gpu::VideoImageRepresentation::ScopedWriteAccess> shared_image_access_;

    ComD3D11Device video_device_;

    Texture2DWrapper::PictureBufferGPUResourceInitDoneCB picture_buffer_gpu_resource_init_done_cb_;
