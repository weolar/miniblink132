...
template <typename CTX> struct BufferSubmitter;

template <> struct BufferSubmitter<ID3D11VideoContext1> {
    static HRESULT SubmitDecoderBuffers(
        ComD3D11VideoContext1 context, ID3D11VideoDecoder* decoder, const UINT num_buffers, const VideoContextWrapper::VideoBufferWrapper* src)
    {
...
template <> struct BufferSubmitter<ID3D11VideoContext> {
    static HRESULT SubmitDecoderBuffers(
        ComD3D11VideoContext context, ID3D11VideoDecoder* decoder, const UINT num_buffers, const VideoContextWrapper::VideoBufferWrapper* src)
    {
        constexpr UINT max_buffers = 4;
        DCHECK_LE(num_buffers, max_buffers);
...
template <typename CTX> class VideoContextWrapperImpl : public VideoContextWrapper {
public:
    ...
    HRESULT DecoderBeginFrame(
        ID3D11VideoDecoder* video_decoder, ID3D11VideoDecoderOutputView* output_view, UINT content_key_size, const void* content_key) override
    {
        return video_context_->DecoderBeginFrame(video_decoder, output_view, content_key_size, content_key);
    }

    HRESULT GetDecoderBuffer(ID3D11VideoDecoder* video_decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type, UINT* buffer_size, void** buffer) override
    {
        return video_context_->GetDecoderBuffer(video_decoder, type, buffer_size, buffer);
    }

    HRESULT ReleaseDecoderBuffer(ID3D11VideoDecoder* video_decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type) override
    {
    ...
    HRESULT SubmitDecoderBuffers(ID3D11VideoDecoder* video_decoder, UINT num_buffers, const VideoBufferWrapper* buffers) override
    {
        return BufferSubmitter<CTX>::SubmitDecoderBuffers(video_context_, video_decoder, num_buffers, buffers);
    }
...
std::unique_ptr<VideoContextWrapper> VideoContextWrapper::CreateWrapper(
    D3D_FEATURE_LEVEL supported_d3d11_version, ComD3D11DeviceContext device_context, HRESULT* status)
{
    if (supported_d3d11_version == D3D_FEATURE_LEVEL_11_0) {
        ...
        *status = device_context.As(&video_context);
        return std::make_unique<VideoContextWrapperImpl<ID3D11VideoContext>>(video_context);
    }

    if (supported_d3d11_version == D3D_FEATURE_LEVEL_11_1) {
        ComD3D11VideoContext1 video_context;
        *status = device_context.As(&video_context);
        return std::make_unique<VideoContextWrapperImpl<ID3D11VideoContext1>>(video_context);
    }
