...
EncoderStatus::Codes ValidateInputOptions(const AudioEncoder::Options& options, ChannelLayout* channel_layout, int* bitrate)
{
...
HRESULT CreateMFEncoder(const IID& iid, void** out_encoder)
{
    ...
    RETURN_IF_FAILED(MFTEnumEx(MFT_CATEGORY_AUDIO_ENCODER, flags, &input_type, &output_type, &activates, &num_activates));
...
HRESULT CreateInputMediaType(const int sample_rate, const int channels, ComMFMediaType* input_media_type)
{
    ...
    RETURN_IF_FAILED(media_type->SetUINT32(MF_MT_AUDIO_BITS_PER_SAMPLE, kBitsPerSample));
    RETURN_IF_FAILED(media_type->SetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND, sample_rate));
...
HRESULT CreateOutputMediaType(
    const int sample_rate, const int channels, const int bitrate, media::AudioEncoder::AacOutputFormat format, ComMFMediaType* output_media_type)
{
    ...
    RETURN_IF_FAILED(media_type->SetUINT32(MF_MT_AUDIO_AVG_BYTES_PER_SECOND, adjusted_bitrate / 8));
...
HRESULT GetInputBufferRequirements(
    const ComMFTransform& mf_encoder, const ComMFMediaType& input_media_type, const int channels, int* input_buffer_alignment, size_t* min_input_buffer_size)
{
    MFT_INPUT_STREAM_INFO input_stream_info = {};
    RETURN_IF_FAILED(mf_encoder->GetInputStreamInfo(kStreamId, &input_stream_info));
    ...
    if (input_stream_info.cbAlignment != 0) {
        ...
        *input_buffer_alignment = std::max(channels * kBytesPerSample, kMinimumRecommendedBlockAlignment);
    }
    ...
    if (input_stream_info.cbSize != 0) {
        *min_input_buffer_size = input_stream_info.cbSize;
        ...
        RETURN_IF_FAILED(input_media_type->GetUINT32(MF_MT_AUDIO_BLOCK_ALIGNMENT, &input_block_attribute));

        *min_input_buffer_size = input_block_attribute;
    }

    // The buffer must be large enough to hold at least one sample.
    CHECK_GE(*min_input_buffer_size, static_cast<size_t>(channels * kBytesPerSample));
    return S_OK;
}

HRESULT GetOutputBufferRequirements(const ComMFTransform& mf_encoder, const ComMFMediaType& output_media_type, const int channels, int* output_buffer_alignment)
{
    MFT_OUTPUT_STREAM_INFO output_stream_info = {};
    RETURN_IF_FAILED(mf_encoder->GetOutputStreamInfo(kStreamId, &output_stream_info));
    ...
    if (output_stream_info.cbAlignment != 0) {
        *output_buffer_alignment = output_stream_info.cbAlignment;
        ...
        // audio channels multiplied by the number of bytes per audio sample.
        // https://docs.microsoft.com/en-us/windows/win32/medfound/mf-mt-audio-block-alignment-attribute
        *output_buffer_alignment = std::max(channels * kBytesPerSample, kMinimumRecommendedBlockAlignment);
    }
...
HRESULT CreateMFSampleFromAudioBus(
    const AudioBus& audio_bus, const int buffer_alignment, const LONGLONG duration, const LONGLONG timestamp, ComMFSample* output_sample)
{
    ...
    size_t source_data_size = audio_bus.channels() * audio_bus.frames() * kBytesPerSample;
    ...
    RETURN_IF_FAILED(MFCreateAlignedMemoryBuffer(source_data_size, buffer_alignment - 1, &dest_buffer));

    // Copy data from `audio_bus` into `dest_buffer`.
    BYTE* dest_buffer_ptr = nullptr;
    DWORD max_buffer_size = 0;
    DWORD current_buffer_size = 0;
    RETURN_IF_FAILED(dest_buffer->Lock(&dest_buffer_ptr, &max_buffer_size, &current_buffer_size));
    ...
    audio_bus.ToInterleaved<SignedInt16SampleTypeTraits>(audio_bus.frames(), reinterpret_cast<int16_t*>(dest_buffer_ptr));
...
HRESULT GetSampleBuffer(const DWORD required_size, const int buffer_alignment, ComMFSample& sample, ComMFMediaBuffer& buffer)
{
    if (!sample)
        RETURN_IF_FAILED(MFCreateSample(&sample));
    ...
    if (need_buffer_allocation) {
        RETURN_IF_FAILED(MFCreateAlignedMemoryBuffer(required_size, buffer_alignment, &buffer));
...
MFAudioEncoder::InputData::InputData(ComMFSample&& sample, const int sample_count, EncoderStatusCB&& done_cb)
...
MFAudioEncoder::PendingData::PendingData(std::unique_ptr<AudioBus>&& audio_bus, const base::TimeTicks capture_time, EncoderStatusCB&& done_cb)
...
MFAudioEncoder::MFAudioEncoder(scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void MFAudioEncoder::Initialize(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    EncoderStatus::Codes code = ValidateInputOptions(options_, &channel_layout, &bitrate);
    ...
    if (options_.bitrate_mode.has_value() && options_.bitrate_mode.value() == AudioEncoder::BitrateMode::kVariable && options.codec == AudioCodec::kAAC) {
        ComCodecAPI codec_api;
        hr = mf_encoder_.As(&codec_api);

        if (SUCCEEDED(hr) && codec_api->IsSupported(&CODECAPI_AVEncAACEnableVBR) == S_OK) {
            VARIANT var;
            var.vt = VT_UI4;
            var.ulVal = TRUE;
    ...
    hr = CreateInputMediaType(options_.sample_rate, options_.channels, &input_media_type);
    if (FAILED(hr) || !input_media_type) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializationError);
        return;
    }

    hr = mf_encoder_->SetInputType(kStreamId, input_media_type.Get(), /*flags=*/0);
    if (FAILED(hr)) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializationError);
        return;
    }
    ...
    hr = CreateOutputMediaType(options_.sample_rate, options_.channels, bitrate, format, &output_media_type);
    ...
    hr = GetInputBufferRequirements(mf_encoder_, input_media_type, options_.channels, &input_buffer_alignment_, &min_input_buffer_size_);
    if (FAILED(hr)) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializationError);
        return;
    }

    hr = GetOutputBufferRequirements(mf_encoder_, output_media_type, options_.channels, &output_buffer_alignment_);
  ...
  */
    UINT32 desc_size = 0;
    if (output_media_type->GetBlobSize(MF_MT_USER_DATA, &desc_size) == S_OK && desc_size > 0 && format == media::AudioEncoder::AacOutputFormat::AAC) {
        codec_desc_.resize(desc_size);
        size_t aac_config_offset = sizeof(HEAACWAVEINFO) - offsetof(HEAACWAVEINFO, wPayloadType);
        hr = output_media_type->GetBlob(MF_MT_USER_DATA, codec_desc_.data(), desc_size, nullptr);
        if (FAILED(hr) || aac_config_offset > codec_desc_.size()) {
            std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializationError);
            return;
        }
        codec_desc_.erase(codec_desc_.begin(), codec_desc_.begin() + aac_config_offset);
    }
    ...
    audio_params_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, { channel_layout, channel_count_ }, options_.sample_rate, kSamplesPerFrame);
    input_timestamp_tracker_ = std::make_unique<AudioTimestampHelper>(options_.sample_rate);
    output_timestamp_tracker_ = std::make_unique<AudioTimestampHelper>(options_.sample_rate);
    output_cb_ = BindCallbackToCurrentLoopIfNeeded(std::move(output_cb));
    initialized_ = true;
    std::move(done_cb).Run(EncoderStatus::Codes::kOk);
}

void MFAudioEncoder::Encode(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(done_cb);
    ...
    if (state_ == EncoderState::kFlushing || state_ == EncoderState::kDraining) {
        pending_inputs_.emplace_back(std::move(audio_bus), capture_time, std::move(done_cb));
        return;
    }
...
void MFAudioEncoder::Flush(EncoderStatusCB done_cb)
{
    ...
    if (state_ == EncoderState::kError || state_ == EncoderState::kFlushing || state_ == EncoderState::kDraining || !can_flush_) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderFailedFlush);
        return;
    }

    DCHECK(state_ == EncoderState::kIdle || state_ == EncoderState::kProcessing) << "state_ == " << static_cast<int>(state_);

    have_queued_input_task_ = false;
    have_queued_output_task_ = false;
    state_ = EncoderState::kFlushing;
    TryProcessOutput(base::BindOnce(&MFAudioEncoder::OnFlushComplete, weak_ptr_factory_.GetWeakPtr(), std::move(done_cb)));
}

void MFAudioEncoder::EnqueueInput(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(initialized_);
    DCHECK(audio_bus);
    ...
    if (!input_timestamp_tracker_->base_timestamp()) {
        ...
        input_timestamp_tracker_->SetBaseTimestamp(capture_time - base::TimeTicks());
        output_timestamp_tracker_->SetBaseTimestamp(capture_time - base::TimeTicks());
    }
    ...
    if (static_cast<size_t>(audio_bus->frames() * channel_count_ * kBytesPerSample) < min_input_buffer_size_) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderFailedEncode);
        return;
    }

    // MF requires the duration and timestamp to be in 100 nanosecond units.
    LONGLONG duration = input_timestamp_tracker_->GetFrameDuration(audio_bus->frames()).InNanoseconds() / 100LL;
    LONGLONG timestamp = input_timestamp_tracker_->GetTimestamp().InNanoseconds() / 100LL;
    input_timestamp_tracker_->AddFrames(audio_bus->frames());

    ComMFSample input_sample;
    hr = CreateMFSampleFromAudioBus(*audio_bus, input_buffer_alignment_, duration, timestamp, &input_sample);
    if (FAILED(hr)) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderFailedEncode);
        return;
    }

    input_queue_.emplace_back(std::move(input_sample), audio_bus->frames(), std::move(done_cb));
...
void MFAudioEncoder::TryProcessInput(FlushCB flush_cb)
{
    ...
    // from this one.
    if ((state_ == EncoderState::kFlushing || state_ == EncoderState::kDraining) && !flush_cb) {
        return;
    }
    ...
    DCHECK(state_ == EncoderState::kProcessing || state_ == EncoderState::kFlushing) << "state_ == " << static_cast<int>(state_);
    ...
    if (not_accepting || can_produce_output_ || state_ == EncoderState::kFlushing) {
        TryProcessOutput(std::move(flush_cb));
        return;
    }
...
void MFAudioEncoder::TryProcessOutput(FlushCB flush_cb)
{
    ...
    DCHECK(state_ == EncoderState::kProcessing || state_ == EncoderState::kFlushing || state_ == EncoderState::kDraining)
    ...
    while (SUCCEEDED(hr) && (status == MFT_OUTPUT_STATUS_SAMPLE_READY || state_ == EncoderState::kDraining)) {
    ...
    if (!input_queue_.empty()) {
        ...
        if (!have_queued_input_task_ || flush_cb) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&MFAudioEncoder::RunTryProcessInput, weak_ptr_factory_.GetWeakPtr(), std::move(flush_cb)));
        }
    ...
    if (samples_in_encoder_ >= kMinSamplesForOutput) {
        ...
        if (!have_queued_output_task_ || flush_cb) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&MFAudioEncoder::RunTryProcessOutput, weak_ptr_factory_.GetWeakPtr(), std::move(flush_cb)));
        }
...
HRESULT MFAudioEncoder::ProcessOutput(EncodedAudioBuffer& encoded_audio)
{
    ...
    // https://docs.microsoft.com/en-us/windows/win32/medfound/basic-mft-processing-model#get-buffer-requirements
    MFT_OUTPUT_STREAM_INFO output_stream_info = {};
    RETURN_IF_FAILED(mf_encoder_->GetOutputStreamInfo(kStreamId, &output_stream_info));
    ...
    RETURN_IF_FAILED(GetSampleBuffer(output_stream_info.cbSize, output_buffer_alignment_, output_sample_, output_buffer));
    ...
    DWORD status;
    HRESULT hr = mf_encoder_->ProcessOutput(/*flags=*/0, /*buffer_count=*/1, &output_data_container, &status);
    ...
    auto encoded_data = base::HeapArray<uint8_t>::CopiedFrom({ output_buffer_ptr, total_length });
    ...
    base::TimeTicks timestamp = output_timestamp_tracker_->GetTimestamp() + base::TimeTicks();
    output_timestamp_tracker_->AddFrames(kSamplesPerFrame);

    encoded_audio = EncodedAudioBuffer(audio_params_, std::move(encoded_data), timestamp, duration);
    return S_OK;
}
...
void MFAudioEncoder::OnFlushComplete(EncoderStatusCB done_cb)
{
    ...
    HRESULT hr = mf_encoder_->ProcessMessage(MFT_MESSAGE_COMMAND_FLUSH, /*param=*/0);
    if (FAILED(hr)) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderFailedFlush);
        return;
    }
    ...
    if (!pending_inputs_.empty()) {
        for (auto& pending_data : pending_inputs_) {
            EnqueueInput(std::move(pending_data.audio_bus), pending_data.capture_time, std::move(pending_data.done_cb));
        }
...
void MFAudioEncoder::OnError()
{
    ...
    while (!input_queue_.empty()) {
        InputData& input_data = input_queue_.front();
        std::move(input_data.done_cb).Run(EncoderStatus::Codes::kEncoderFailedEncode);
        input_queue_.pop_front();
    }

    while (!pending_inputs_.empty()) {
        PendingData& pending_data = pending_inputs_.front();
        std::move(pending_data.done_cb).Run(EncoderStatus::Codes::kEncoderFailedEncode);
...
uint32_t MFAudioEncoder::ClampAccCodecBitrate(uint32_t bitrate)
{
    ...
    auto it = std::lower_bound(std::begin(kSupportedBitrates), std::end(kSupportedBitrates), bitrate);
    if (it != std::end(kSupportedBitrates)) {
        return *it;
    }

    return kSupportedBitrates[sizeof(kSupportedBitrates) / sizeof(kSupportedBitrates[0]) - 1];
}
