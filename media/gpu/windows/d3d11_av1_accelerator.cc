...
public:
    explicit D3D11AV1Picture(D3D11PictureBuffer* d3d11_picture, D3D11VideoDecoderClient* client, bool apply_grain)
    ...
    bool apply_grain() const
    {
        return apply_grain_;
    }
    D3D11PictureBuffer* picture_buffer() const
    {
...
D3D11AV1Accelerator::D3D11AV1Accelerator(D3D11VideoDecoderClient* client, MediaLog* media_log, bool disable_invalid_ref)
...
scoped_refptr<AV1Picture> D3D11AV1Accelerator::CreateAV1Picture(bool apply_grain)
{
    D3D11PictureBuffer* picture_buffer = client_->GetPicture();
    return picture_buffer ? base::MakeRefCounted<D3D11AV1Picture>(picture_buffer, client_, apply_grain) : nullptr;
}

bool D3D11AV1Accelerator::SubmitDecoderBuffer(const DXVA_PicParams_AV1& pic_params, const libgav1::Vector<libgav1::TileBuffer>& tile_buffers)
{
    // Buffer #1 - AV1 specific picture parameters.
    auto params_buffer = client_->GetWrapper()->GetPictureParametersBuffer(sizeof(pic_params));
    ...
    const size_t bitstream_size
        = std::accumulate(tile_buffers.begin(), tile_buffers.end(), 0, [](size_t acc, const auto& buffer) { return acc + buffer.size; });
    auto& bitstream_buffer = client_->GetWrapper()->GetBitstreamBuffer(bitstream_size);
    ...
    return params_buffer.Commit() && tile_buffer.Commit() && client_->GetWrapper()->SubmitSlice();
}

DecodeStatus D3D11AV1Accelerator::SubmitDecode(const AV1Picture& pic, const libgav1::ObuSequenceHeader& seq_header, const AV1ReferenceFrameVector& ref_frames,
    const libgav1::Vector<libgav1::TileBuffer>& tile_buffers, base::span<const uint8_t> data)
{
    ...
    FillPicParams(pic_ptr->picture_buffer()->picture_index(), pic_ptr->apply_grain(), pic.frame_header, seq_header, ref_frames, &pic_params);
    ...
    return client_->GetWrapper()->SubmitDecode() ? DecodeStatus::kOk : DecodeStatus::kFail;
}
...
void D3D11AV1Accelerator::FillPicParams(size_t picture_index, bool apply_grain, const libgav1::ObuFrameHeader& frame_header,
    const libgav1::ObuSequenceHeader& seq_header, const AV1ReferenceFrameVector& ref_frames, DXVA_PicParams_AV1* pp)
{
    ...
    pp->superres_denom = frame_header.use_superres ? frame_header.superres_scale_denominator : libgav1::kSuperResScaleNumerator;
    ...
    if (tile_info.uniform_spacing) {
        ...
        const auto tile_width_sb = (tile_info.sb_columns + (1 << tile_info.tile_columns_log2) - 1) >> tile_info.tile_columns_log2;
        const int last_width_idx = tile_info.tile_columns - 1;
        for (int i = 0; i < last_width_idx; ++i)
            pp->tiles.widths[i] = tile_width_sb;
        pp->tiles.widths[last_width_idx] = tile_info.sb_columns - last_width_idx * tile_width_sb;

        const auto tile_height_sb = (tile_info.sb_rows + (1 << tile_info.tile_rows_log2) - 1) >> tile_info.tile_rows_log2;
        const int last_height_idx = tile_info.tile_rows - 1;
        for (int i = 0; i < last_height_idx; ++i)
            pp->tiles.heights[i] = tile_height_sb;
        pp->tiles.heights[last_height_idx] = tile_info.sb_rows - last_height_idx * tile_height_sb;
    } else {
        for (int i = 0; i < pp->tiles.cols; ++i) {
            pp->tiles.widths[i] = frame_header.tile_info.tile_column_width_in_superblocks[i];
        }
        for (int i = 0; i < pp->tiles.rows; ++i) {
            pp->tiles.heights[i] = frame_header.tile_info.tile_row_height_in_superblocks[i];
        }
    }
    ...
    pp->coding.disable_frame_end_update_cdf = !frame_header.enable_frame_end_update_cdf;
    ...
    pp->coding.reference_frame_update = !(frame_header.show_existing_frame && frame_header.frame_type == libgav1::kFrameKey);
    ...
    for (size_t i = 0; i < libgav1::kNumReferenceFrameTypes - 1; ++i) {
        ...
        const auto* rp = static_cast<const D3D11AV1Picture*>(ref_frames[ref_idx].get());
        if (!rp) {
        ...
        const auto& gm = frame_header.global_motion[libgav1::kReferenceFrameLast + i];
        for (size_t j = 0; j < 6; ++j)
            pp->frame_refs[i].wmmat[j] = gm.params[j];
        pp->frame_refs[i].wminvalid = gm.type == libgav1::kGlobalMotionTransformationTypeIdentity;
    ...
    for (size_t i = 0; i < libgav1::kNumReferenceFrameTypes; ++i) {
        ...
        pp->RefFrameMapTextureIndex[i] = rp ? rp->picture_buffer()->picture_index() : 0xFF;
    }
    ...
    pp->loop_filter.mode_ref_delta_enabled = frame_header.loop_filter.delta_enabled;
    ...
    for (size_t i = 0; i < libgav1::kMaxPlanes; ++i) {
        ...
        pp->loop_filter.frame_restoration_type[i] = kD3D11LoopRestorationMapping[frame_header.loop_restoration.type[i]];
        pp->loop_filter.log2_restoration_unit_size[i] = frame_header.loop_restoration.unit_size_log2[i];
    }
    ...
    pp->quantization.qm_y = frame_header.quantizer.use_matrix ? frame_header.quantizer.matrix_level[0] : 0xFF;
    pp->quantization.qm_u = frame_header.quantizer.use_matrix ? frame_header.quantizer.matrix_level[1] : 0xFF;
    pp->quantization.qm_v = frame_header.quantizer.use_matrix ? frame_header.quantizer.matrix_level[2] : 0xFF;
    ...
    for (size_t i = 0; i < libgav1::kMaxCdefStrengths; ++i) {
        ...
        pp->cdef.y_strengths[i].primary = frame_header.cdef.y_primary_strength[i] >> coeff_shift;
        pp->cdef.y_strengths[i].secondary = y_str;
        pp->cdef.uv_strengths[i].primary = frame_header.cdef.uv_primary_strength[i] >> coeff_shift;
    ...
    for (size_t i = 0; i < libgav1::kMaxSegments; ++i) {
        for (size_t j = 0; j < libgav1::kSegmentFeatureMax; ++j) {
            pp->segmentation.feature_mask[i].mask |= frame_header.segmentation.feature_enabled[i][j] << j;
            pp->segmentation.feature_data[i][j] = frame_header.segmentation.feature_data[i][j];
        }
    }
    ...
    if (apply_grain) {
        ...
        pp->film_grain.matrix_coeff_is_identity = seq_header.color_config.matrix_coefficients == libgav1::kMatrixCoefficientsIdentity;
