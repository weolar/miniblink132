...
BASE_FEATURE(kExpandMediaFoundationEncodingResolutions, "ExpandMediaFoundationEncodingResolutions", base::FEATURE_ENABLED_BY_DEFAULT);
...
// formats.
constexpr auto kSupportedPixelFormats = base::MakeFixedFlatSet<VideoPixelFormat>({ PIXEL_FORMAT_I420, PIXEL_FORMAT_NV12 });
constexpr auto kSupportedPixelFormatsD3DVideoProcessing = base::MakeFixedFlatSet<VideoPixelFormat>(
    { PIXEL_FORMAT_I420, PIXEL_FORMAT_NV12, PIXEL_FORMAT_YV12, PIXEL_FORMAT_NV21, PIXEL_FORMAT_ARGB, PIXEL_FORMAT_XRGB });

// The default supported max framerate and resolution.
constexpr FramerateAndResolution kDefaultMaxFramerateAndResolution = { kDefaultFrameRateNumerator / kDefaultFrameRateDenominator, gfx::Size(1920, 1080) };
...
constexpr FramerateAndResolution kLegacy2KMaxFramerateAndResolution = { 172, gfx::Size(1920, 1080) };
constexpr FramerateAndResolution kLegacy4KMaxFramerateAndResolution = { 66, gfx::Size(3840, 2160) };
...
constexpr FramerateAndResolution kModern2KMaxFramerateAndResolution = { 300, gfx::Size(1920, 1080) };
constexpr FramerateAndResolution kModern4KMaxFramerateAndResolution = { 300, gfx::Size(3840, 2160) };
constexpr FramerateAndResolution kModern8KMaxFramerateAndResolution = { 128, gfx::Size(7680, 4320) };

constexpr gfx::Size kMinResolution(32, 32);

constexpr CLSID kIntelAV1HybridEncoderCLSID = { 0x62c053ce, 0x5357, 0x4794, { 0x8c, 0x5a, 0xfb, 0xef, 0xfe, 0xff, 0xb8, 0x2d } };
...
BASE_FEATURE(kMediaFoundationVP9L1T2Support, "MediaFoundationVP9L1T2Support", base::FEATURE_DISABLED_BY_DEFAULT);
// Up to 3 temporal layers, i.e. this enables both L1T2 and L1T3.
BASE_FEATURE(kMediaFoundationVP9L1T3Support, "MediaFoundationVP9L1T3Support", base::FEATURE_DISABLED_BY_DEFAULT);
BASE_FEATURE(kMediaFoundationAV1L1T2Support, "MediaFoundationAV1L1T2Support", base::FEATURE_DISABLED_BY_DEFAULT);
// Up to 3 temporal layers, i.e. this enables both L1T2 and L1T3.
BASE_FEATURE(kMediaFoundationAV1L1T3Support, "MediaFoundationAV1L1T3Support", base::FEATURE_DISABLED_BY_DEFAULT);
#endif // !defined(ARCH_CPU_X86)

BASE_FEATURE(kMediaFoundationUseSWBRCForH264Camera, "MediaFoundationUseSWBRCForH264Camera", base::FEATURE_DISABLED_BY_DEFAULT);
BASE_FEATURE(kMediaFoundationUseSWBRCForH264Desktop, "MediaFoundationUseSWBRCForH264Desktop", base::FEATURE_DISABLED_BY_DEFAULT);

#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
// For H.265 encoding at L1T1/L1T2 we may use SW bitrate controller when
// constant bitrate encoding is requested.
BASE_FEATURE(kMediaFoundationUseSWBRCForH265, "MediaFoundationUseSWBRCForH265", base::FEATURE_DISABLED_BY_DEFAULT);
#endif // BUILDFLAG(ENABLE_PLATFORM_HEVC)

eAVEncH264VProfile GetH264VProfile(VideoCodecProfile profile, bool is_constrained_h264)
{
    switch (profile) {
    case H264PROFILE_BASELINE:
        return is_constrained_h264 ? eAVEncH264VProfile_ConstrainedBase : eAVEncH264VProfile_Base;
...
uint32_t GetDistanceToNextTemporalBaseLayer(uint32_t frame_number, uint32_t temporal_layer_count)
{
    DCHECK(temporal_layer_count >= 1 && temporal_layer_count <= 3);
    uint32_t pattern_count = 1 << (temporal_layer_count - 1);
    return (frame_number % pattern_count == 0) ? 0 : pattern_count - (frame_number % pattern_count);
}

MediaFoundationVideoEncodeAccelerator::DriverVendor GetDriverVendor(IMFActivate* encoder)
{
    using DriverVendor = MediaFoundationVideoEncodeAccelerator::DriverVendor;
    base::win::ScopedCoMem<WCHAR> vendor_id;
    UINT32 id_length;
    encoder->GetAllocatedString(MFT_ENUM_HARDWARE_VENDOR_ID_Attribute, &vendor_id, &id_length);
...
int GetMaxTemporalLayerVendorLimit(
    MediaFoundationVideoEncodeAccelerator::DriverVendor vendor, VideoCodec codec, const gpu::GpuDriverBugWorkarounds& workarounds)
{
    ...
    if ((codec == VideoCodec::kHEVC || codec == VideoCodec::kAV1) && vendor == DriverVendor::kQualcomm) {
        return 1;
    }
    ...
    // for that at present.
    if (codec == VideoCodec::kHEVC && vendor == DriverVendor::kIntel && workarounds.disable_hevc_hmft_cbr_encoding) {
        return 2;
    }
    ...
    if (codec == VideoCodec::kVP9) {
        if (vendor == DriverVendor::kIntel && workarounds.disable_vp9_hmft_temporal_encoding) {
            return 1;
        }
...
int GetNumSupportedTemporalLayers(IMFActivate* activate, VideoCodec codec, const gpu::GpuDriverBugWorkarounds& workarounds)
{
    auto vendor = GetDriverVendor(activate);
    int max_temporal_layer_vendor_limit = GetMaxTemporalLayerVendorLimit(vendor, codec, workarounds);
    ...
    if (FAILED(codec_api->GetParameterRange(&CODECAPI_AVEncVideoTemporalLayerCount, min.AsInput(), max.AsInput(), step.AsInput()))) {
        return 1;
    }
...
{
    if (GetDriverVendor(activate) == MediaFoundationVideoEncodeAccelerator::DriverVendor::kIntel) {
...
MFTEnum2Type GetMFTEnum2Function()
{
    static const MFTEnum2Type kMFTEnum2Func = []() {
        ...
        return mf_dll ? reinterpret_cast<MFTEnum2Type>(base::GetFunctionPointerFromNativeLibrary(mf_dll, "MFTEnum2")) : nullptr;
    }();
...
std::vector<ComPtr<IMFActivate>> EnumerateHardwareEncodersLegacy(VideoCodec codec)
{
    ...
    for (UINT adapter_idx = 0; SUCCEEDED(factory->EnumAdapters(adapter_idx, &temp_adapter)); adapter_idx++) {
        ++num_adapters;
    ...
    uint32_t count = 0;
    hr = MFTEnumEx(MFT_CATEGORY_VIDEO_ENCODER, flags, &input_info, &output_info, &pp_activates, &count);

    if (FAILED(hr)) {
        // Log to VLOG since errors are expected as part of
        // GetSupportedProfiles().
        DVLOG(2) << "Failed to enumerate hardware encoders for " << GetCodecName(codec) << " : " << PrintHr(hr);
        return encoders;
    }
    ...
    for (UINT32 i = 0; i < count; i++) {
        ...
        // in the system.
        if (num_adapters == 1) {
            pp_activates[i]->SetBlob(MFT_ENUM_ADAPTER_LUID, reinterpret_cast<BYTE*>(&single_adapter_luid), sizeof(LUID));
        }
...
std::vector<ComPtr<IMFActivate>> EnumerateHardwareEncoders(VideoCodec codec)
{
    ...
    for (UINT adapter_idx = 0; SUCCEEDED(factory->EnumAdapters(adapter_idx, &temp_adapter)); adapter_idx++) {
        ...
        if (FAILED(hr = attributes->SetBlob(MFT_ENUM_ADAPTER_LUID, reinterpret_cast<BYTE*>(&desc.AdapterLuid), sizeof(LUID)))) {
            continue;
        }
        ...
        hr = mftenum2_func(MFT_CATEGORY_VIDEO_ENCODER, flags, &input_info, &output_info, attributes.Get(), &pp_activates, &count);

        if (FAILED(hr)) {
            // Log to VLOG since errors are expected as part of
            // GetSupportedProfiles().
            DVLOG(2) << "Failed to enumerate hardware encoders for " << GetCodecName(codec) << " at a adapter #" << adapter_idx << " : " << PrintHr(hr);
            continue;
        }
        ...
        for (UINT32 i = 0; i < count; i++) {
            ...
            pp_activates[i]->SetBlob(MFT_ENUM_ADAPTER_LUID, reinterpret_cast<BYTE*>(&desc.AdapterLuid), sizeof(LUID));
            encoders.push_back(pp_activates[i]);
        }
...
uint32_t CalculateMaxFramerateFromMacroBlocksPerSecond(const FramerateAndResolution& max_framerate_and_resolution, uint32_t max_macroblocks_per_second)
{
    constexpr uint64_t kMacroBlockWidth = 16u;
    constexpr uint64_t kMacroBlockHeight = 16u;

    uint64_t max_possible_framerate
        = std::floor((max_macroblocks_per_second * kMacroBlockWidth * kMacroBlockHeight) / max_framerate_and_resolution.resoluion.Area64());

    return std::clamp(static_cast<uint32_t>(max_possible_framerate), 1u, max_framerate_and_resolution.frame_rate);
}

std::vector<FramerateAndResolution> GetMaxFramerateAndResolutionsFromMFT(VideoCodec codec, IMFTransform* encoder)
{
    ComPtr<IMFMediaType> media_type;
    std::vector<FramerateAndResolution> framerate_and_resolutions = { kDefaultMaxFramerateAndResolution };
    RETURN_ON_HR_FAILURE(MFCreateMediaType(&media_type), "Create media type failed", framerate_and_resolutions);
    RETURN_ON_HR_FAILURE(media_type->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video), "Set major type failed", framerate_and_resolutions);
    RETURN_ON_HR_FAILURE(media_type->SetGUID(MF_MT_SUBTYPE, VideoCodecToMFSubtype(codec)), "Set guid for sub type failed", framerate_and_resolutions);
    RETURN_ON_HR_FAILURE(MFSetAttributeSize(media_type.Get(), MF_MT_FRAME_SIZE, kDefaultMaxFramerateAndResolution.resoluion.width(),
    ...
    RETURN_ON_HR_FAILURE(media_type->SetUINT32(MF_MT_AVG_BITRATE, 9000000), "Set avg bitrate failed", framerate_and_resolutions);
    RETURN_ON_HR_FAILURE(media_type->SetUINT32(MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive), "Set interlace mode failed", framerate_and_resolutions);
    ...
    if (codec != VideoCodec::kVP9) {
        ...
        RETURN_ON_HR_FAILURE(media_type->SetUINT32(MF_MT_VIDEO_LEVEL, max_level), "Set video level failed", framerate_and_resolutions);
    }

    RETURN_ON_HR_FAILURE(encoder->SetOutputType(/*stream_id=*/0, media_type.Get(), 0), "Set output type failed", framerate_and_resolutions);

    ComPtr<IMFAttributes> attributes;
    RETURN_ON_HR_FAILURE(encoder->GetAttributes(&attributes), "Get attributes failed", framerate_and_resolutions);
    uint32_t max_macroblocks_per_second = MFGetAttributeUINT32(attributes.Get(), MF_VIDEO_MAX_MB_PER_SEC, 0);
    max_macroblocks_per_second &= 0x0fffffff; // Only lower 28 bits are supported.
    ...
    for (auto& max_framerate_and_resolution : max_framerate_and_resolutions) {
        FramerateAndResolution framerate_and_resolution
            = { CalculateMaxFramerateFromMacroBlocksPerSecond(max_framerate_and_resolution, max_macroblocks_per_second),
        ...
        if (framerate_and_resolution.frame_rate >= (kDefaultFrameRateNumerator / kDefaultFrameRateDenominator)) {
...
int GetMaxTemporalLayer(VideoCodec codec, std::vector<ComPtr<IMFActivate>>& activates, const gpu::GpuDriverBugWorkarounds& workarounds)
{
    int num_temporal_layers = 1;

    for (size_t i = 0; i < activates.size(); i++) {
        num_temporal_layers = std::max(GetNumSupportedTemporalLayers(activates[i].Get(), codec, workarounds), num_temporal_layers);
    }
...
uint32_t AdjustBitrateToFrameRate(uint32_t bitrate, uint32_t configured_framerate, uint32_t requested_framerate)
{
...
VideoRateControlWrapper::RateControlConfig CreateRateControllerConfig(const VideoBitrateAllocation& bitrate_allocation, gfx::Size size, uint32_t frame_rate,
    int num_temporal_layers, VideoCodec codec, VideoEncodeAccelerator::Config::ContentType content_type)
{
...
struct MediaFoundationVideoEncodeAccelerator::BitstreamBufferRef {
    ...
    BitstreamBufferRef(int32_t id, base::WritableSharedMemoryMapping mapping, size_t size)
        : id(id)
...
MediaFoundationVideoEncodeAccelerator::MediaFoundationVideoEncodeAccelerator(
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, CHROME_LUID luid)
...
VideoEncodeAccelerator::SupportedProfiles MediaFoundationVideoEncodeAccelerator::GetSupportedProfiles()
{
    TRACE_EVENT0("gpu,startup", "MediaFoundationVideoEncodeAccelerator::GetSupportedProfiles");
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    std::vector<VideoCodec> supported_codecs({ VideoCodec::kH264, VideoCodec::kVP9, VideoCodec::kAV1 });
    ...
    for (auto codec : supported_codecs) {
        ...
        if (activates.empty()) {
            DVLOG(1) << "Hardware encode acceleration is not available for " << GetCodecName(codec);
            continue;
        }

        int num_temporal_layers = GetMaxTemporalLayer(codec, activates, workarounds_);
        auto bitrate_mode = VideoEncodeAccelerator::kConstantMode | VideoEncodeAccelerator::kVariableMode;
        if (codec == VideoCodec::kH264) {
            bitrate_mode |= VideoEncodeAccelerator::kExternalMode;
        }

        std::vector<FramerateAndResolution> max_framerate_and_resolutions = { kDefaultMaxFramerateAndResolution };
        ...
        if (base::FeatureList::IsEnabled(kExpandMediaFoundationEncodingResolutions)) {
            ...
            CHECK(encoder);
            max_framerate_and_resolutions = GetMaxFramerateAndResolutionsFromMFT(codec, encoder.Get());
            activate->ShutdownObject();
        }

        for (auto& max_framerate_and_resolution : max_framerate_and_resolutions) {
            DVLOG(3) << __func__ << ": " << codec << " codec, max resolution width: " << max_framerate_and_resolution.resoluion.width()
                     << ", height: " << max_framerate_and_resolution.resoluion.height() << ", framerate: " << max_framerate_and_resolution.frame_rate;

            SupportedProfile profile(VIDEO_CODEC_PROFILE_UNKNOWN, max_framerate_and_resolution.resoluion,
                max_framerate_and_resolution.frame_rate * kDefaultFrameRateDenominator, kDefaultFrameRateDenominator, bitrate_mode,
            ...
            if (base::FeatureList::IsEnabled(kMediaFoundationD3DVideoProcessing)) {
                base::ranges::copy(kSupportedPixelFormatsD3DVideoProcessing, profile.gpu_supported_pixel_formats.begin());
            }
            ...
            if (codec == VideoCodec::kH264) {
                codec_profiles = { H264PROFILE_BASELINE, H264PROFILE_MAIN, H264PROFILE_HIGH };
...
bool MediaFoundationVideoEncodeAccelerator::Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log)
{
    ...
    if (base::FeatureList::IsEnabled(kMediaFoundationD3DVideoProcessing)) {
        is_supported_format = base::ranges::find(kSupportedPixelFormatsD3DVideoProcessing, config.input_format) != kSupportedPixelFormats.end();
    } else {
        is_supported_format = base::ranges::find(kSupportedPixelFormats, config.input_format) != kSupportedPixelFormats.end();
    }

    if (!is_supported_format) {
        MEDIA_LOG(ERROR, media_log_) << "Input format not supported= " << VideoPixelFormatToString(config.input_format);
        return false;
    }

    if (config.output_profile >= H264PROFILE_MIN && config.output_profile <= H264PROFILE_MAX) {
        if (GetH264VProfile(config.output_profile, config.is_constrained_h264) == eAVEncH264VProfile_unknown) {
            MEDIA_LOG(ERROR, media_log_) << "Output profile not supported = " << config.output_profile;
            return false;
        }
        codec_ = VideoCodec::kH264;
    } else if (config.output_profile >= VP9PROFILE_MIN && config.output_profile <= VP9PROFILE_MAX) {
    ...
    bitstream_buffer_size_ = EstimateBitstreamBufferSize(bitrate_allocation_.GetSumBitrate(), frame_rate_, config.input_visible_size);
    ...
    svc_parser_ = std::make_unique<TemporalScalabilityIdExtractor>(codec_, num_temporal_layers_);

    SetState(kInitializing);

    std::vector<ComPtr<IMFActivate>> activates = EnumerateHardwareEncoders(codec_);

    if (activates.empty()) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed finding a hardware encoder MFT" });
        return false;
    }
    ...
    if (!activated) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed activating an async hardware encoder MFT" });
        return false;
    }
    ...
    if (!SetEncoderModes()) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed to set encoder modes" });
        return false;
    }

    if (!InitializeInputOutputParameters(config.output_profile, config.is_constrained_h264)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed to set input/output param." });
        return false;
    }

    HRESULT hr = S_OK;
    if (IsMediaFoundationD3D11VideoCaptureEnabled()) {
        MEDIA_LOG(INFO, media_log_) << "Preferred DXGI device " << luid_.HighPart << ":" << luid_.LowPart;
        dxgi_device_manager_ = DXGIDeviceManager::Create(luid_);
        if (!dxgi_device_manager_) {
            NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed to create DXGIDeviceManager" });
            return false;
        }

        LUID mft_luid { 0, 0 };
        UINT32 out_size = 0;
        activate_->GetBlob(MFT_ENUM_ADAPTER_LUID, reinterpret_cast<BYTE*>(&mft_luid), sizeof(LUID), &out_size);

        hr = E_FAIL;
        if (out_size == sizeof(LUID) && mft_luid.HighPart == luid_.HighPart && mft_luid.LowPart == luid_.LowPart) {
            // Only try to set the device manager for MFTs on the correct adapter.
            // Don't rely on MFT rejecting the device manager.
            auto mf_dxgi_device_manager = dxgi_device_manager_->GetMFDXGIDeviceManager();
            hr = encoder_->ProcessMessage(MFT_MESSAGE_SET_D3D_MANAGER, reinterpret_cast<ULONG_PTR>(mf_dxgi_device_manager.Get()));
        }
        ...
        if (FAILED(hr)) {
            ...
            MEDIA_LOG(INFO, media_log_) << "Couldn't set DXGIDeviceManager, fallback to non-D3D11 encoding";
        }
    }

    hr = encoder_->QueryInterface(IID_PPV_ARGS(&event_generator_));
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Couldn't get event generator: " + PrintHr(hr) });
        return false;
    }
    ...
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Couldn't set ProcessMessage MFT_MESSAGE_COMMAND_FLUSH: " + PrintHr(hr) });
        return false;
    }
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_BEGIN_STREAMING, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus(
            { EncoderStatus::Codes::kEncoderInitializationError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_BEGIN_STREAMING: " + PrintHr(hr) });
        return false;
    }
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus(
            { EncoderStatus::Codes::kEncoderInitializationError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_START_OF_STREAM: " + PrintHr(hr) });
        return false;
    }
    ...
    encoder_info_.supports_frame_size_change = !workarounds_.disable_media_foundation_frame_size_change;
    ...
    if (!base::FeatureList::IsEnabled(kMediaFoundationD3DVideoProcessing) || config.input_format == PIXEL_FORMAT_NV12) {
        return true;
    }

    mf_video_processor_ = std::make_unique<MediaFoundationVideoProcessorAccelerator>(gpu_preferences_, workarounds_);
    ...
    if (dxgi_resource_mapping_required_) {
        hr = mf_video_processor_->Initialize(vp_config, nullptr, media_log_->Clone());
    } else {
        hr = mf_video_processor_->Initialize(vp_config, dxgi_device_manager_, media_log_->Clone());
    }
    ...
    MEDIA_LOG(INFO, media_log_) << "Using video processor to convert from " << config.input_format << " to encoder accepted " << vp_config.output_format;

    return true;
}

void MediaFoundationVideoEncodeAccelerator::Encode(scoped_refptr<VideoFrame> frame, bool force_keyframe)
{
    Encode(std::move(frame), EncodeOptions(force_keyframe));
}

void MediaFoundationVideoEncodeAccelerator::Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& options)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (codec_ == VideoCodec::kVP9 && workarounds_.avoid_consecutive_keyframes_for_vp9 && last_frame_was_keyframe_request_ && options.key_frame) {
    ...
    bool force_key_frame = (input_since_keyframe_count_ + pending_input_queue_.size()) % gop_length_ == 0;

    bool discard_high_layer_frames = (((codec_ == VideoCodec::kVP9 || codec_ == VideoCodec::kAV1) && vendor_ == DriverVendor::kIntel)
                                         || (codec_ == VideoCodec::kH264 && (vendor_ == DriverVendor::kIntel || vendor_ == DriverVendor::kNvidia)))
    ...
    if (discard_high_layer_frames) {
        ...
        uint32_t distance_to_base_layer = GetDistanceToNextTemporalBaseLayer(input_since_keyframe_count_ + pending_input_queue_.size(), num_temporal_layers_);
...
void MediaFoundationVideoEncodeAccelerator::QueueInput(scoped_refptr<media::VideoFrame> frame, const VideoEncoder::EncodeOptions& options, bool discard_output)
{
    PendingInput result;
    auto hr = MFCreateSample(&result.input_sample);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderInitializationError, "Failed to create sample" });
        return;
    }
    ...
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderFailedEncode, "Failed to populate input sample buffer" });
        return;
    }
...
void MediaFoundationVideoEncodeAccelerator::EncodeInternal(scoped_refptr<VideoFrame> frame, const EncodeOptions& options, bool discard_output)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    default:
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderFailedEncode, "Unexpected encoder state" });
        DVLOG(3) << "Abandon input frame for video encoder."
                 << " State: " << static_cast<int>(state_);
    }
}

void MediaFoundationVideoEncodeAccelerator::UseOutputBitstreamBuffer(BitstreamBuffer buffer)
{
    ...
    if (buffer.size() < bitstream_buffer_size_) {
        NotifyErrorStatus({ EncoderStatus::Codes::kInvalidOutputBuffer,
            "Output BitstreamBuffer isn't big enough: " + base::NumberToString(buffer.size()) + " vs. " + base::NumberToString(bitstream_buffer_size_) });
        return;
    }
    ...
    if (!mapping.IsValid()) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Failed mapping shared memory" });
        return;
    }
    auto buffer_ref = std::make_unique<BitstreamBufferRef>(buffer.id(), std::move(mapping), buffer.size());
    ...
    memcpy(buffer_ref->mapping.memory(), encode_output->memory(), encode_output->size());
...
void MediaFoundationVideoEncodeAccelerator::RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate, const std::optional<gfx::Size>& size)
{
    DVLOG(3) << __func__ << ": bitrate=" << bitrate.ToString() << ": framerate=" << framerate << ": size=" << (size.has_value() ? size->ToString() : "nullopt");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MediaFoundationVideoEncodeAccelerator::RequestEncodingParametersChange(
    const VideoBitrateAllocation& bitrate_allocation, uint32_t framerate, const std::optional<gfx::Size>& size)
{
    DVLOG(3) << __func__ << ": bitrate=" << bitrate_allocation.GetSumBps() << ": framerate=" << framerate
    ...
    if (bitrate_allocation.GetMode() != bitrate_allocation_.GetMode()) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderUnsupportedConfig, "Can't change bitrate mode after Initialize()" });
        return;
    }

    framerate = std::clamp(framerate, 1u, static_cast<uint32_t>(kDefaultFrameRateNumerator / kDefaultFrameRateDenominator));

    if (framerate == frame_rate_ && bitrate_allocation == bitrate_allocation_ && !size.has_value()) {
        return;
    }
    ...
    if (rate_ctrl_) {
        rate_ctrl_->UpdateRateControl(
            CreateRateControllerConfig(bitrate_allocation_, size.value_or(input_visible_size_), frame_rate_, num_temporal_layers_, codec_, content_type_));
    } else {
        VARIANT var;
        var.vt = VT_UI4;
        HRESULT hr;
        ...
        case Bitrate::Mode::kVariable:
            var.ulVal = AdjustBitrateToFrameRate(bitrate_allocation_.GetPeakBps(), configured_frame_rate_, framerate);
            DVLOG(3) << "bitrate_allocation_.GetPeakBps() is " << bitrate_allocation_.GetPeakBps();
            ...
            if (FAILED(hr)) {
                NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set max bitrate" + PrintHr(hr) });
                return;
            }
            [[fallthrough]];
        case Bitrate::Mode::kConstant:
            var.ulVal = AdjustBitrateToFrameRate(bitrate_allocation_.GetSumBps(), configured_frame_rate_, framerate);
            DVLOG(3) << "bitrate_allocation_.GetSumBps() is " << bitrate_allocation_.GetSumBps();
            DVLOG(3) << "configured_frame_rate_ is " << configured_frame_rate_;
            DVLOG(3) << "framerate is " << framerate;
            DVLOG(3) << "Setting CODECAPI_AVEncCommonMeanBitRate to " << var.ulVal;
            hr = codec_api_->SetValue(&CODECAPI_AVEncCommonMeanBitRate, &var);
            if (FAILED(hr)) {
                NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set mean bitrate" + PrintHr(hr) });
                return;
            }
            break;
        case Bitrate::Mode::kExternal:
            DVLOG(3) << "RequestEncodingParametersChange for Bitrate::Mode::kExternal";
            break;
        }
    }
...
bool MediaFoundationVideoEncodeAccelerator::IsFrameSizeAllowed(gfx::Size size)
{
    if (max_framerate_and_resolutions_.empty()) {
        DCHECK(encoder_);
        max_framerate_and_resolutions_ = GetMaxFramerateAndResolutionsFromMFT(codec_, encoder_.Get());
    }

    for (auto& [frame_rate, resolution] : max_framerate_and_resolutions_) {
        if (size.width() >= kMinResolution.width() && size.height() >= kMinResolution.height() && size.width() <= resolution.width()
...
void MediaFoundationVideoEncodeAccelerator::UpdateFrameSize(const gfx::Size& frame_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(imf_output_media_type_);
    DCHECK(imf_input_media_type_);
    ...
    if (!IsFrameSizeAllowed(frame_size)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderUnsupportedConfig, "Unsupported frame size" });
        return;
    }
    ...
    if (vendor_ == DriverVendor::kIntel || vendor_ == DriverVendor::kQualcomm) {
        hr = encoder_->ProcessMessage(MFT_MESSAGE_COMMAND_FLUSH, 0);
        if (FAILED(hr)) {
            NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set ProcessMessage MFT_MESSAGE_COMMAND_FLUSH: " + PrintHr(hr) });
            return;
        }
    }
    ...
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_END_OF_STREAM: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_END_STREAMING, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_END_STREAMING: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->SetInputType(input_stream_id_, nullptr, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set input stream type to nullptr: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->SetOutputType(output_stream_id_, nullptr, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set output stream type to nullptr: " + PrintHr(hr) });
        return;
    }
    hr = MFSetAttributeSize(imf_output_media_type_.Get(), MF_MT_FRAME_SIZE, input_visible_size_.width(), input_visible_size_.height());
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set output frame size: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->SetOutputType(output_stream_id_, imf_output_media_type_.Get(), 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set output media type: " + PrintHr(hr) });
        return;
    }
    hr = MFSetAttributeSize(imf_input_media_type_.Get(), MF_MT_FRAME_SIZE, input_visible_size_.width(), input_visible_size_.height());
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set input frame size: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->SetInputType(input_stream_id_, imf_input_media_type_.Get(), 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set input media type: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_BEGIN_STREAMING, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_BEGIN_STREAMING: " + PrintHr(hr) });
        return;
    }
    hr = encoder_->ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0);
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Couldn't set ProcessMessage MFT_MESSAGE_NOTIFY_START_OF_STREAM: " + PrintHr(hr) });
        return;
    }

    bitstream_buffer_size_ = EstimateBitstreamBufferSize(bitrate_allocation_.GetSumBitrate(), frame_rate_, input_visible_size_);
    ...
    input_since_keyframe_count_ = 0;
    client_->RequireBitstreamBuffers(kNumInputBuffers, input_visible_size_, bitstream_buffer_size_);
}
...
void MediaFoundationVideoEncodeAccelerator::Flush(FlushCallback flush_callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool MediaFoundationVideoEncodeAccelerator::ActivateAsyncEncoder(std::vector<ComPtr<IMFActivate>>& activates, bool is_constrained_h264)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    for (auto& activate : activates) {
        ...
        // to merit value.
        if (codec_ == VideoCodec::kH264 && is_constrained_h264 && vendor == DriverVendor::kNvidia) {
            DLOG(WARNING) << "Skipped NVIDIA GPU due to https://crbug.com/1088650";
            continue;
        }

        if (num_temporal_layers_ > GetMaxTemporalLayerVendorLimit(vendor, codec_, workarounds_)) {
        ...
        if (encoder_.Get() != nullptr) {
            ...
            UINT32 name_length;
            activate_->GetAllocatedString(MFT_FRIENDLY_NAME_Attribute, &friendly_name, &name_length);
            DVLOG(3) << "Selected asynchronous hardware encoder's friendly name: " << friendly_name;
    ...
    RETURN_ON_HR_FAILURE(hr, "Couldn't activate asynchronous hardware encoder", false);
    RETURN_ON_FAILURE((encoder_.Get() != nullptr), "No asynchronous hardware encoder instance created", false);
...
bool MediaFoundationVideoEncodeAccelerator::InitializeInputOutputParameters(VideoCodecProfile output_profile, bool is_constrained_h264)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(encoder_);
    ...
    if (input_count < 1 || output_count < 1) {
        DLOG(ERROR) << "Stream count too few: input " << input_count << ", output " << output_count;
        return false;
    }

    std::vector<DWORD> input_ids(input_count, 0);
    std::vector<DWORD> output_ids(output_count, 0);
    hr = encoder_->GetStreamIDs(input_count, input_ids.data(), output_count, output_ids.data());
    if (hr == S_OK) {
    ...
    hr = imf_output_media_type_->SetGUID(MF_MT_SUBTYPE, VideoCodecToMFSubtype(codec_));
    RETURN_ON_HR_FAILURE(hr, "Couldn't set video format", false);

    if (!rate_ctrl_) {
        UINT32 bitrate = AdjustBitrateToFrameRate(bitrate_allocation_.GetSumBps(), frame_rate_, frame_rate_);
    ...
    hr = MFSetAttributeRatio(imf_output_media_type_.Get(), MF_MT_FRAME_RATE, configured_frame_rate_, 1);
    RETURN_ON_HR_FAILURE(hr, "Couldn't set frame rate", false);
    hr = MFSetAttributeSize(imf_output_media_type_.Get(), MF_MT_FRAME_SIZE, input_visible_size_.width(), input_visible_size_.height());
    RETURN_ON_HR_FAILURE(hr, "Couldn't set frame size", false);
    hr = imf_output_media_type_->SetUINT32(MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive);
    RETURN_ON_HR_FAILURE(hr, "Couldn't set interlace mode", false);
    if (codec_ == VideoCodec::kH264) {
        hr = imf_output_media_type_->SetUINT32(MF_MT_MPEG2_PROFILE, GetH264VProfile(output_profile, is_constrained_h264));
    } else if (codec_ == VideoCodec::kVP9) {
        hr = imf_output_media_type_->SetUINT32(MF_MT_MPEG2_PROFILE, GetVP9VProfile(output_profile));
    } else if (codec_ == VideoCodec::kHEVC) {
        hr = imf_output_media_type_->SetUINT32(MF_MT_MPEG2_PROFILE, GetHEVCProfile(output_profile));
    }
    ...
    hr = MFSetAttributeRatio(imf_input_media_type_.Get(), MF_MT_FRAME_RATE, configured_frame_rate_, 1);
    RETURN_ON_HR_FAILURE(hr, "Couldn't set frame rate", false);
    DVLOG(3) << "MF_MT_FRAME_SIZE is " << input_visible_size_.width() << "x" << input_visible_size_.height();
    hr = MFSetAttributeSize(imf_input_media_type_.Get(), MF_MT_FRAME_SIZE, input_visible_size_.width(), input_visible_size_.height());
    RETURN_ON_HR_FAILURE(hr, "Couldn't set frame size", false);
    hr = imf_input_media_type_->SetUINT32(MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive);
    RETURN_ON_HR_FAILURE(hr, "Couldn't set interlace mode", false);
    hr = encoder_->SetInputType(input_stream_id_, imf_input_media_type_.Get(), 0);
...
void MediaFoundationVideoEncodeAccelerator::SetSWRateControl()
{
    ...
    // more than 3.
    if (bitrate_allocation_.GetMode() != Bitrate::Mode::kConstant || !base::FeatureList::IsEnabled(kMediaFoundationUseSoftwareRateCtrl)
    ...
    if (codec_ == VideoCodec::kH264) {
        ...
        if (content_type_ == VideoEncodeAccelerator::Config::ContentType::kCamera && !base::FeatureList::IsEnabled(kMediaFoundationUseSWBRCForH264Camera)) {
            return;
        }

        // Check feature flag for the desktop source.
        if (content_type_ == VideoEncodeAccelerator::Config::ContentType::kDisplay && !base::FeatureList::IsEnabled(kMediaFoundationUseSWBRCForH264Desktop)) {
            return;
        }
    }
    ...
    if (codec_ == VideoCodec::kHEVC) {
        ...
        // Check feature flag.
        if ((vendor_ != DriverVendor::kIntel || !workarounds_.disable_hevc_hmft_cbr_encoding)
    ...
    VideoRateControlWrapper::RateControlConfig rate_config
        = CreateRateControllerConfig(bitrate_allocation_, input_visible_size_, frame_rate_, num_temporal_layers_, codec_, content_type_);
...
bool MediaFoundationVideoEncodeAccelerator::SetEncoderModes()
{
    ...
    const bool set_svc_layer_count
        = (num_temporal_layers_ > 1) || (vendor_ == DriverVendor::kIntel && (codec_ == VideoCodec::kH264 || codec_ == VideoCodec::kHEVC));
    if (set_svc_layer_count) {
        var.ulVal = num_temporal_layers_;
        DVLOG(3) << "Setting CODECAPI_AVEncVideoTemporalLayerCount to " << var.ulVal;
        ...
        if (vendor_ == DriverVendor::kIntel && num_temporal_layers_ == 2) {
            if (S_OK == codec_api_->IsModifiable(&CODECAPI_AVEncVideoMaxNumRefFrame)) {
                var.ulVal = 1;
                DVLOG(3) << "Setting CODECAPI_AVEncVideoMaxNumRefFrame to " << var.ulVal;
    ...
    if (!rate_ctrl_ && bitrate_allocation_.GetMode() != Bitrate::Mode::kExternal) {
        var.ulVal = AdjustBitrateToFrameRate(bitrate_allocation_.GetSumBps(), configured_frame_rate_, frame_rate_);
        DVLOG(3) << "bitrate_allocation_.GetSumBps() is " << bitrate_allocation_.GetSumBps();
        DVLOG(3) << "configured_frame_rate_ is " << configured_frame_rate_;
    ...
    if (bitrate_allocation_.GetMode() == Bitrate::Mode::kVariable) {
        var.ulVal = AdjustBitrateToFrameRate(bitrate_allocation_.GetPeakBps(), configured_frame_rate_, frame_rate_);
        DVLOG(3) << "bitrate_allocation_.GetPeakBps() is " << bitrate_allocation_.GetPeakBps();
        DVLOG(3) << "configured_frame_rate_ is " << configured_frame_rate_;
        DVLOG(3) << "framerate is " << frame_rate_;
    ...
    if (S_OK == codec_api_->IsModifiable(&CODECAPI_AVScenarioInfo) && vendor_ == DriverVendor::kIntel && content_type_ == Config::ContentType::kDisplay) {
        var.vt = VT_UI4;
...
void MediaFoundationVideoEncodeAccelerator::NotifyErrorStatus(EncoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(!status.is_ok());
    CHECK(media_log_);
    ...
    DLOG(ERROR) << "Call NotifyErrorStatus(): code=" << static_cast<int>(status.code()) << ", message=" << status.message();
    CHECK(client_);
...
void MediaFoundationVideoEncodeAccelerator::FeedInputs()
{
    ...
    if (FAILED(hr)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Failed to encode pending frame: " + PrintHr(hr) });
        return;
    }
...
HRESULT MediaFoundationVideoEncodeAccelerator::ProcessInput(const PendingInput& input)
{
    DVLOG(3) << __func__;
    CHECK(input.input_sample);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(encoder_needs_input_counter_ > 0);
    TRACE_EVENT2("media", "MediaFoundationVideoEncodeAccelerator::ProcessInput", "timestamp", input.timestamp, "discard_output", input.discard_output);
    ...
    if (input.options.quantizer.has_value()) {
        ...
        quantizer = std::clamp(static_cast<int>(input.options.quantizer.value()), 1, kH26xMaxQp);
    } else if (rate_ctrl_ && !input.discard_output) {
        VideoRateControlWrapper::FrameParams frame_params {};
        frame_params.frame_type = input.options.key_frame || force_key_frame ? VideoRateControlWrapper::FrameParams::FrameType::kKeyFrame
        ...
        temporal_id = svc_parser_->AssignTemporalIdBySvcSpec(input_since_keyframe_count_);
    ...
    if (quantizer.has_value()) {
        ...
        hr = input.input_sample->SetUINT64(MFSampleExtension_VideoEncodeQP, var.ullVal);
    ...
    sample_metadata_queue_.push_back(OutOfBandMetadata {
        .color_space = input.color_space, .discard_output = input.discard_output, .qp = quantizer, .frame_id = input_since_keyframe_count_ });

    {
        TRACE_EVENT1("media", "IMFTransform::ProcessInput", "timestamp", input.timestamp);
...
HRESULT MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBuffer(const PendingInput& input, scoped_refptr<VideoFrame> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    auto input_sample = input.input_sample;
    if (frame->storage_type() != VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER && !frame->IsMappable()) {
        LOG(ERROR) << "Unsupported video frame storage type";
        return MF_E_INVALID_STREAM_DATA;
    }

    TRACE_EVENT1("media", "MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBuffer", "timestamp", frame->timestamp());

    if (frame->format() != PIXEL_FORMAT_NV12 && frame->format() != PIXEL_FORMAT_I420) {
        LOG(ERROR) << "Unsupported video frame format";
        return MF_E_INVALID_STREAM_DATA;
    }

    auto hr = input_sample->SetSampleTime(frame->timestamp().InMicroseconds() * kOneMicrosecondInMFSampleTimeUnits);
    ...
    if (FAILED(hr)) {
        ...
        hr = MFCreateAlignedMemoryBuffer(
            input_stream_info.cbSize ? input_stream_info.cbSize : VideoFrame::AllocationSize(kTargetPixelFormat, input_visible_size_),
            input_stream_info.cbAlignment == 0 ? input_stream_info.cbAlignment : input_stream_info.cbAlignment - 1, &input_buffer);
    ...
    size_t dst_y_stride = VideoFrame::RowBytes(VideoFrame::Plane::kY, kTargetPixelFormat, input_visible_size_.width());
    uint8_t* dst_uv = scoped_buffer.get() + dst_y_stride * VideoFrame::Rows(VideoFrame::Plane::kY, kTargetPixelFormat, input_visible_size_.height());
    size_t dst_uv_stride = VideoFrame::RowBytes(VideoFrame::Plane::kUV, kTargetPixelFormat, input_visible_size_.width());
    uint8_t* end = dst_uv + dst_uv_stride * VideoFrame::Rows(VideoFrame::Plane::kUV, kTargetPixelFormat, input_visible_size_.height());
    DCHECK_GE(static_cast<ptrdiff_t>(scoped_buffer.max_length()), end - scoped_buffer.get());
    ...
    auto frame_in_buffer = VideoFrame::WrapExternalYuvData(kTargetPixelFormat, input_visible_size_, gfx::Rect(input_visible_size_), input_visible_size_,
        dst_y_stride, dst_uv_stride, dst_y, dst_uv, frame->timestamp());

    auto status = frame_converter_.ConvertAndScale(*frame, *frame_in_buffer);
    if (!status.is_ok()) {
        LOG(ERROR) << "ConvertAndScale failed with error code: " << static_cast<uint32_t>(status.code());
        return E_FAIL;
    }
    return S_OK;
}
...
HRESULT MediaFoundationVideoEncodeAccelerator::CopyInputSampleBufferFromGpu(scoped_refptr<VideoFrame> frame, ComMFSample input_sample)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(frame->storage_type(), VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER);
    ...
    hr = device1->OpenSharedResource1(buffer_handle.dxgi_handle.Get(), IID_PPV_ARGS(&input_texture));
    ...
    if (texture_size != input_visible_size_ || frame->visible_rect().size() != input_visible_size_ || !frame->visible_rect().origin().IsOrigin()) {
    ...
    hr = encoder_->GetInputStreamInfo(input_stream_id_, &input_stream_info);
    RETURN_ON_HR_FAILURE(hr, "Couldn't get input stream info", hr);
    hr = MFCreateAlignedMemoryBuffer(input_stream_info.cbSize ? input_stream_info.cbSize : VideoFrame::AllocationSize(kTargetPixelFormat, input_visible_size_),
        input_stream_info.cbAlignment == 0 ? input_stream_info.cbAlignment : input_stream_info.cbAlignment - 1, &input_buffer);
    RETURN_ON_HR_FAILURE(hr, "Failed to create memory buffer for input sample", hr);

    MediaBufferScopedPointer scoped_buffer(input_buffer.Get());
    bool copy_succeeded = gpu::CopyD3D11TexToMem(sample_texture.Get(), scoped_buffer.get(), scoped_buffer.max_length(), d3d_device.Get(), &staging_texture_);
    if (!copy_succeeded) {
        LOG(ERROR) << "Failed to copy sample to memory.";
        return E_FAIL;
    }
    size_t copied_bytes = input_visible_size_.width() * input_visible_size_.height() * 3 / 2;
...
HRESULT MediaFoundationVideoEncodeAccelerator::PopulateInputSampleBufferGpu(scoped_refptr<VideoFrame> frame, ComMFSample input_sample)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(frame->storage_type(), VideoFrame::StorageType::STORAGE_GPU_MEMORY_BUFFER);
    DCHECK(dxgi_device_manager_);
    ...
    ComMFMediaBuffer input_buffer;
    hr = MFCreateDXGISurfaceBuffer(__uuidof(ID3D11Texture2D), sample_texture.Get(), 0, FALSE, &input_buffer);
...
void MediaFoundationVideoEncodeAccelerator::ProcessOutput()
{
    ...
    if (hr == MF_E_TRANSFORM_STREAM_CHANGE) {
        ...
        for (DWORD type_index = 0; SUCCEEDED(hr); ++type_index) {
            hr = encoder_->GetOutputAvailableType(output_stream_id_, type_index, &media_type);
            if (SUCCEEDED(hr)) {
                break;
            }
        }
    ...
    if (SUCCEEDED(hr)) {
        timestamp = base::Microseconds(sample_time / kOneMicrosecondInMFSampleTimeUnits);
    }
    ...
    if (metadata.qp.has_value()) {
        ...
        hr = output_data_buffer.pSample->GetUINT64(MFSampleExtension_VideoEncodeQP, &frame_qp_from_sample);
        if (vendor_ == DriverVendor::kIntel) {
            if ((FAILED(hr) || !IsValidQp(codec_, frame_qp_from_sample)) && encoder_info_.reports_average_qp) {
    ...
    const bool keyframe = MFGetAttributeUINT32(output_data_buffer.pSample, MFSampleExtension_CleanPoint, false);
    DWORD size = 0;
    ...
    if (IsTemporalScalabilityCoding()) {
        ...
        if (!svc_parser_->ParseChunk(base::span(scoped_buffer.get(), size), metadata.frame_id, bits_md)) {
            NotifyErrorStatus({ EncoderStatus::Codes::kEncoderHardwareDriverError, "Parse bitstream failed" });
            return;
        }
        ...
        if (codec_ == VideoCodec::kH264) {
            ...
            if (keyframe) {
                ...
                vp9.end_active_spatial_layer_index = 1 /*vp9.spatial_layer_resolutions.size()*/;
            } else {
                    ...
                    if (temporal_id == 0) {
                        ...
                        for (const auto ref : bits_md.ref_frame_list) {
                            if (ref.temporal_id >= temporal_id) {
                                NotifyErrorStatus({ EncoderStatus::Codes::kEncoderHardwareDriverError, "VP9 referenced frames check failed " });
                                return;
                            }
                        }
                    }
    ...
    if (rate_ctrl_) {
        ...
        frame_params.frame_type
            = keyframe ? VideoRateControlWrapper::FrameParams::FrameType::kKeyFrame : VideoRateControlWrapper::FrameParams::FrameType::kInterFrame;
        frame_params.temporal_layer_id = temporal_id;
...
void MediaFoundationVideoEncodeAccelerator::MediaEventHandler(MediaEventType event_type, HRESULT status)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (FAILED(status)) {
        NotifyErrorStatus({ EncoderStatus::Codes::kSystemAPICallError, "Media Foundation async error: " + PrintHr(status) });
        return;
    }
    ...
    case MEError: {
        NotifyErrorStatus({ EncoderStatus::Codes::kEncoderHardwareDriverError, "Media Foundation encountered a critical failure." });
        break;
    }
    default:
        break;
...
HRESULT MediaFoundationVideoEncodeAccelerator::InitializeD3DVideoProcessing(ID3D11Texture2D* input_texture)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    D3D11_TEXTURE2D_DESC input_desc = {};
    input_texture->GetDesc(&input_desc);
    if (vp_desc_.InputWidth == input_desc.Width && vp_desc_.InputHeight == input_desc.Height
        && scaled_d3d11_texture_desc_.Width == static_cast<UINT>(input_visible_size_.width())
        && scaled_d3d11_texture_desc_.Height == static_cast<UINT>(input_visible_size_.height())) {
        return S_OK;
    }

    // Input/output framerates are dummy values for passthrough.
    D3D11_VIDEO_PROCESSOR_CONTENT_DESC vp_desc = { .InputFrameFormat = D3D11_VIDEO_FRAME_FORMAT_PROGRESSIVE,
        .InputFrameRate = { 60, 1 },
    ...
    hr = video_device->CreateVideoProcessorEnumerator(&vp_desc, &video_processor_enumerator);
    RETURN_ON_HR_FAILURE(hr, "CreateVideoProcessorEnumerator failed", hr);

    ComD3D11VideoProcessor video_processor;
    hr = video_device->CreateVideoProcessor(video_processor_enumerator.Get(), 0, &video_processor);
    ...
    video_context->VideoProcessorSetStreamAutoProcessingMode(video_processor.Get(), 0, FALSE);

    D3D11_TEXTURE2D_DESC scaled_desc = { .Width = static_cast<UINT>(input_visible_size_.width()),
    ...
    hr = texture_device->CreateTexture2D(&scaled_desc, nullptr, &scaled_d3d11_texture);
    RETURN_ON_HR_FAILURE(hr, "Failed to create texture", hr);

    hr = SetDebugName(scaled_d3d11_texture.Get(), "MFVideoEncodeAccelerator_ScaledTexture");
    ...
    hr = video_device->CreateVideoProcessorOutputView(scaled_d3d11_texture.Get(), video_processor_enumerator.Get(), &output_desc, &vp_output_view);
...
HRESULT MediaFoundationVideoEncodeAccelerator::PerformD3DScaling(ID3D11Texture2D* input_texture, const gfx::Rect& visible_rect)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    D3D11_VIDEO_PROCESSOR_COLOR_SPACE src_d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(src_color_space);
    video_context_->VideoProcessorSetStreamColorSpace(video_processor_.Get(), 0, &src_d3d11_color_space);
    D3D11_VIDEO_PROCESSOR_COLOR_SPACE output_d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(output_color_space);
    video_context_->VideoProcessorSetOutputColorSpace(video_processor_.Get(), &output_d3d11_color_space);

    {
        ...
        hr = video_device_->CreateVideoProcessorInputView(input_texture, video_processor_enumerator_.Get(), &input_desc, &input_view);
        RETURN_ON_HR_FAILURE(hr, "CreateVideoProcessorInputView failed", hr);

        D3D11_VIDEO_PROCESSOR_STREAM stream
            = { .Enable = true, .OutputIndex = 0, .InputFrameOrField = 0, .PastFrames = 0, .FutureFrames = 0, .pInputSurface = input_view.Get() };

        D3D11_TEXTURE2D_DESC input_texture_desc = {};
        input_texture->GetDesc(&input_texture_desc);
        RECT source_rect = { static_cast<LONG>(visible_rect.x()), static_cast<LONG>(visible_rect.y()), static_cast<LONG>(visible_rect.right()),
        ...
        RECT dest_rect = { 0, 0, static_cast<LONG>(output_texture_desc.Width), static_cast<LONG>(output_texture_desc.Height) };
        video_context_->VideoProcessorSetOutputTargetRect(video_processor_.Get(), TRUE, &dest_rect);
        video_context_->VideoProcessorSetStreamDestRect(video_processor_.Get(), 0, TRUE, &dest_rect);

        hr = video_context_->VideoProcessorBlt(video_processor_.Get(), vp_output_view_.Get(), 0, 1, &stream);
...
HRESULT MediaFoundationVideoEncodeAccelerator::InitializeD3DCopying(ID3D11Texture2D* input_texture)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    D3D11_TEXTURE2D_DESC input_desc = {};
    input_texture->GetDesc(&input_desc);
    ...
    if (copied_d3d11_texture_) {
        ...
        if (input_desc.Width == copy_desc.Width && input_desc.Height == copy_desc.Height) {
            return S_OK;
        }
    }
    ComD3D11Device texture_device;
    input_texture->GetDevice(&texture_device);
    D3D11_TEXTURE2D_DESC copy_desc = { .Width = input_desc.Width,
    ...
    HRESULT hr = texture_device->CreateTexture2D(&copy_desc, nullptr, &copied_d3d11_texture);
    RETURN_ON_HR_FAILURE(hr, "Failed to create texture", hr);
    hr = SetDebugName(copied_d3d11_texture.Get(), "MFVideoEncodeAccelerator_CopiedTexture");
    RETURN_ON_HR_FAILURE(hr, "Failed to set debug name", hr);
    copied_d3d11_texture_ = std::move(copied_d3d11_texture);
    return S_OK;
}

HRESULT MediaFoundationVideoEncodeAccelerator::PerformD3DCopy(ID3D11Texture2D* input_texture, const gfx::Rect& visible_rect)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...

    {
        ...
        D3D11_BOX src_box = { static_cast<UINT>(visible_rect.x()), static_cast<UINT>(visible_rect.y()), 0, static_cast<UINT>(visible_rect.right()),
            static_cast<UINT>(visible_rect.bottom()), 1 };
        device_context->CopySubresourceRegion(copied_d3d11_texture_.Get(), 0, 0, 0, 0, input_texture, 0, &src_box);
    }
    return S_OK;
}

HRESULT MediaFoundationVideoEncodeAccelerator::GetParameters(DWORD* pdwFlags, DWORD* pdwQueue)
{
...
HRESULT MediaFoundationVideoEncodeAccelerator::Invoke(IMFAsyncResult* pAsyncResult)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&MediaFoundationVideoEncodeAccelerator::MediaEventHandler, weak_ptr_, event_type, status));
    return status;
}
...
HRESULT MediaFoundationVideoEncodeAccelerator::QueryInterface(REFIID riid, void** ppv)
{
    static const QITAB kQI[] = { QITABENT(MediaFoundationVideoEncodeAccelerator, IMFAsyncCallback), { 0 } };
