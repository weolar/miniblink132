...
CopyingTexture2DWrapper::CopyingTexture2DWrapper(
    const gfx::Size& size, std::unique_ptr<Texture2DWrapper> output_wrapper, scoped_refptr<VideoProcessorProxy> processor, ComD3D11Texture2D output_texture)
    : size_(size)
...
D3D11Status CopyingTexture2DWrapper::ProcessTexture(const gfx::ColorSpace& input_color_space, scoped_refptr<gpu::ClientSharedImage>& shared_image_dest)
{
    ...
    HRESULT hr = video_processor_->CreateVideoProcessorOutputView(output_texture_.Get(), &output_view_desc, &output_view);
    if (!SUCCEEDED(hr))
    ...
    hr = video_processor_->CreateVideoProcessorInputView(texture_.Get(), &input_view_desc, &input_view);
    if (!SUCCEEDED(hr))
    ...
    if (!previous_input_color_space_ || *previous_input_color_space_ != input_color_space) {
    ...
    return output_texture_wrapper_->ProcessTexture(input_color_space, shared_image_dest);
}

D3D11Status CopyingTexture2DWrapper::Init(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb,
    ...
    PictureBufferGPUResourceInitDoneCB picture_buffer_gpu_resource_init_done_cb)
{
    ...
    return output_texture_wrapper_->Init(std::move(gpu_task_runner), std::move(get_helper_cb), output_texture_,
        /*array_size=*/0, std::move(picture_buffer), std::move(picture_buffer_gpu_resource_init_done_cb));
}
