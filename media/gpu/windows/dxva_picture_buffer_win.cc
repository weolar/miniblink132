...
public:
    ...
    bool CopyTexSubImage(unsigned target, const gfx::Point& offset, const gfx::Rect& rect) override
    {
        return false;
    }
    ...
    void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, uint64_t process_tracing_id, const std::string& dump_name) override
    {
    }
...
// static
std::unique_ptr<DXVAPictureBuffer> DXVAPictureBuffer::Create(const DXVAVideoDecodeAccelerator& decoder, const PictureBuffer& buffer, EGLConfig egl_config)
{
    ...
    case DXVAVideoDecodeAccelerator::PictureBufferMechanism::DELAYED_COPY_TO_NV12: {
        auto picture_buffer = std::make_unique<EGLStreamDelayedCopyPictureBuffer>(buffer);
    ...
    case DXVAVideoDecodeAccelerator::PictureBufferMechanism::COPY_TO_NV12: {
        auto picture_buffer = std::make_unique<EGLStreamCopyPictureBuffer>(buffer);
        if (!picture_buffer->Initialize(decoder))
            return nullptr;
...
bool DXVAPictureBuffer::CopyOutputSampleDataToPictureBuffer(
    DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id)
{
    NOTREACHED();
    return false;
}
...
bool DXVAPictureBuffer::CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface)
{
    NOTREACHED();
    return false;
}
...
bool DXVAPictureBuffer::BindSampleToTexture(DXVAVideoDecodeAccelerator* decoder, Microsoft::WRL::ComPtr<IMFSample> sample)
{
    NOTREACHED();
    return false;
}

bool PbufferPictureBuffer::Initialize(const DXVAVideoDecodeAccelerator& decoder, EGLConfig egl_config)
{
    RETURN_ON_FAILURE(!picture_buffer_.service_texture_ids().empty(), "No service texture ids provided", false);

    EGLDisplay egl_display = gl::GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay();
    EGLint use_rgb = 1;
    eglGetConfigAttrib(egl_display, egl_config, EGL_BIND_TO_TEXTURE_RGB, &use_rgb);
    ...
    EGLint attrib_list[] = { EGL_WIDTH, size().width(), EGL_HEIGHT, size().height(), EGL_TEXTURE_FORMAT, use_rgb ? EGL_TEXTURE_RGB : EGL_TEXTURE_RGBA,
        EGL_TEXTURE_TARGET, EGL_TEXTURE_2D, EGL_NONE };

    decoding_surface_ = eglCreatePbufferFromClientBuffer(egl_display, EGL_D3D_TEXTURE_2D_SHARE_HANDLE_ANGLE, texture_share_handle_, egl_config, attrib_list);
    ...
    if (decoder.d3d11_device_ && decoder.use_keyed_mutex_) {
        ...
        EGLBoolean ret = eglQuerySurfacePointerANGLE(egl_display, decoding_surface_, EGL_DXGI_KEYED_MUTEX_ANGLE, &keyed_mutex);
        RETURN_ON_FAILURE(keyed_mutex && ret == EGL_TRUE, "Failed to query ANGLE keyed mutex", false);
        egl_keyed_mutex_ = Microsoft::WRL::ComPtr<IDXGIKeyedMutex>(static_cast<IDXGIKeyedMutex*>(keyed_mutex));
    }
    use_rgb_ = !!use_rgb;
    return true;
}

bool PbufferPictureBuffer::InitializeTexture(const DXVAVideoDecodeAccelerator& decoder, bool use_rgb, bool use_fp16)
{
    ...
    if (decoder.d3d11_device_) {
        ...
        desc.MiscFlags = decoder.use_keyed_mutex_ ? D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX : D3D11_RESOURCE_MISC_SHARED;

        HRESULT hr = decoder.d3d11_device_->CreateTexture2D(&desc, nullptr, &dx11_decoding_texture_);
        RETURN_ON_HR_FAILURE(hr, "Failed to create texture", false);
        RETURN_ON_HR_FAILURE(SetDebugName(dx11_decoding_texture_.Get(), "DXVADecoder_PictureBuffer"), "SetDebugNameFail", false);
        ...
        RETURN_ON_FAILURE(SUCCEEDED(hr) && texture_share_handle_, "Failed to query shared handle", false);

    } else {
        HRESULT hr = E_FAIL;
        hr = decoder.d3d9_device_ex_->CreateTexture(picture_buffer_.size().width(), picture_buffer_.size().height(), 1, D3DUSAGE_RENDERTARGET,
            use_rgb ? D3DFMT_X8R8G8B8 : D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &decoding_texture_, &texture_share_handle_);
        RETURN_ON_HR_FAILURE(hr, "Failed to create texture", false);
        RETURN_ON_FAILURE(texture_share_handle_, "Failed to query shared handle", false);
    }
    return true;
}
    ...
    DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id)
{
    ...
    if (dx11_texture) {
        ...
        if (!decoder->CopyTexture(dx11_texture, dx11_decoding_texture_.Get(), dx11_keyed_mutex_, keyed_mutex_value_, id(), input_buffer_id, color_space_)) {
    ...
    hr = decoder->d3d9_->CheckDeviceFormatConversion(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, surface_desc.Format, use_rgb_ ? D3DFMT_X8R8G8B8 : D3DFMT_A8R8G8B8);
    ...
    decoder->CopySurface(decoder_surface_.Get(), target_surface_.Get(), id(), input_buffer_id, color_space_);
...
bool PbufferPictureBuffer::CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface)
{
...
{
    RETURN_ON_FAILURE(picture_buffer_.service_texture_ids().size() >= 2, "Not enough texture ids provided", false);
    ...
    gl::ScopedTextureBinder texture0_binder(GL_TEXTURE_EXTERNAL_OES, picture_buffer_.service_texture_ids()[0]);
    gl::ScopedActiveTexture texture1(GL_TEXTURE1);
    gl::ScopedTextureBinder texture1_binder(GL_TEXTURE_EXTERNAL_OES, picture_buffer_.service_texture_ids()[1]);
    ...
    EGLBoolean result = eglStreamConsumerGLTextureExternalAttribsNV(egl_display, stream_, consumer_attributes);
    ...
    result = eglCreateStreamProducerD3DTextureANGLE(egl_display, stream_, producer_attributes);
...
bool EGLStreamPictureBuffer::BindSampleToTexture(DXVAVideoDecodeAccelerator* decoder, Microsoft::WRL::ComPtr<IMFSample> sample)
{
    DCHECK_EQ(BOUND, state_);
    state_ = IN_CLIENT;
    ...
    EGLBoolean result = eglStreamPostD3DTextureANGLE(egl_display, stream_, static_cast<void*>(dx11_decoding_texture_.Get()), frame_attributes);
    ...
    gl::GLImageDXGI* gl_image_dxgi = static_cast<gl::GLImageDXGI*>(gl_image_.get());
...
EGLStreamDelayedCopyPictureBuffer::EGLStreamDelayedCopyPictureBuffer(const PictureBuffer& buffer)
    : DXVAPictureBuffer(buffer)
    , stream_(nullptr)
{
}
...
bool EGLStreamDelayedCopyPictureBuffer::Initialize(const DXVAVideoDecodeAccelerator& decoder)
{
    RETURN_ON_FAILURE(picture_buffer_.service_texture_ids().size() >= 2, "Not enough texture ids provided", false);
    ...
    result = eglCreateStreamProducerD3DTextureANGLE(egl_display, stream_, producer_attributes);
    RETURN_ON_FAILURE(result, "Could not create stream producer", false);
    scoped_refptr<gl::CopyingGLImageDXGI> copying_image_ = base::MakeRefCounted<gl::CopyingGLImageDXGI>(ComD3D11Device(decoder.D3D11Device()), size(), stream_);
...
bool EGLStreamDelayedCopyPictureBuffer::BindSampleToTexture(DXVAVideoDecodeAccelerator* decoder, Microsoft::WRL::ComPtr<IMFSample> sample)
{
    DCHECK_EQ(BOUND, state_);
    state_ = IN_CLIENT;
    ...
    UINT subresource;
    dxgi_buffer->GetSubresourceIndex(&subresource);
    if (!decoder->InitializeID3D11VideoProcessor(size().width(), size().height(), color_space_))
        return false;

    DCHECK(decoder->d3d11_processor_);
    DCHECK(decoder->enumerator_);

    gl::CopyingGLImageDXGI* gl_image_dxgi = static_cast<gl::CopyingGLImageDXGI*>(gl_image_.get());
    DCHECK(gl_image_dxgi);

    gl_image_dxgi->SetTexture(dx11_decoding_texture_, subresource);
    return gl_image_dxgi->InitializeVideoProcessor(decoder->d3d11_processor_, decoder->enumerator_);
}
...
EGLStreamCopyPictureBuffer::EGLStreamCopyPictureBuffer(const PictureBuffer& buffer)
    : DXVAPictureBuffer(buffer)
    , stream_(nullptr)
{
}
...
bool EGLStreamCopyPictureBuffer::Initialize(const DXVAVideoDecodeAccelerator& decoder)
{
    RETURN_ON_FAILURE(picture_buffer_.service_texture_ids().size() >= 2, "Not enough texture ids provided", false);
    ...
    HRESULT hr = decoder.d3d11_device_->CreateTexture2D(&desc, nullptr, &decoder_copy_texture_);
    RETURN_ON_HR_FAILURE(hr, "Failed to create texture", false);
    RETURN_ON_HR_FAILURE(SetDebugName(decoder_copy_texture_.Get(), "DXVADecoder_EGLStreamCopyPictureBuffer"), "SetDebugNameFail", false);
    DCHECK(decoder.use_keyed_mutex_);
    ...
    hr = decoder.angle_device_->OpenSharedResource(texture_share_handle_, IID_PPV_ARGS(&angle_copy_texture_));
    ...
    DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id)
{
    ...
    if (!decoder->CopyTexture(dx11_texture, decoder_copy_texture_.Get(), dx11_keyed_mutex_, keyed_mutex_value_, id(), input_buffer_id, color_space_)) {
        // |this| might be destroyed
        return false;
    }
...
bool EGLStreamCopyPictureBuffer::CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface)
{
    DCHECK(!src_surface);
    DCHECK(!dest_surface);
    DCHECK_EQ(COPYING, state_);
    ...
    EGLBoolean result = eglStreamPostD3DTextureANGLE(egl_display, stream_, static_cast<void*>(angle_copy_texture_.Get()), frame_attributes);
