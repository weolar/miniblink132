...
using MockD3D11VideoProcessorEnumerator = Microsoft::WRL::ComPtr<D3D11VideoProcessorEnumeratorMock>;
...
public:
    ...
    scoped_refptr<VideoProcessorProxy> CreateProxy()
    {
        ...
        EXPECT_CALL(*dev_.Get(), CreateVideoProcessorEnumerator(_, _)).WillOnce(SetComPointeeAndReturnOk<1>(enumerator_.Get()));

        EXPECT_CALL(*dev_.Get(), CreateVideoProcessor(_, _, _)).WillOnce(SetComPointeeAndReturnOk<2>(proc_.Get()));

        EXPECT_CALL(*ctx_.Get(), QueryInterface(_, _)).WillOnce(SetComPointeeAndReturnOk<1>(vctx_.Get()));
    ...
    template <typename T> T* CreateGarbagePtr()
    {
        int foo;
        void* local = &foo;
...
TEST_F(D3D11VideoProcessorProxyUnittest, EnsureMethodPassthrough)
{
    ...
    EXPECT_CALL(*dev_.Get(), CreateVideoProcessorOutputView(texture, enumerator_.Get(), out_desc, nullptr));

    EXPECT_CALL(*dev_.Get(), CreateVideoProcessorInputView(texture, enumerator_.Get(), in_desc, nullptr));

    EXPECT_CALL(*vctx_.Get(), VideoProcessorBlt(proc_.Get(), out_view, 6, 7, streams));
