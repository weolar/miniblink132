...
private:
    ...
    static constexpr size_t kMaxSize = std::max({ static_cast<size_t>(H264DPB::kDPBMaxSize), static_cast<size_t>(/*H265*/ kMaxDpbSize),
                                           static_cast<size_t>(kVp9NumRefFrames), static_cast<size_t>(libgav1::kNumReferenceFrameTypes) })
...
MEDIA_GPU_EXPORT absl::InlinedVector<D3D12_RESOURCE_BARRIER, 2> CreateD3D12TransitionBarriersForAllPlanes(
...
// HEVCPROFILE_REXT.
MEDIA_GPU_EXPORT GUID GetD3D12VideoDecodeGUID(VideoCodecProfile profile, uint8_t bitdepth, VideoChromaSampling chroma_sampling);
