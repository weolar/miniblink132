...
public:
    D3D12VideoDecoderWrapperImpl(MediaLog* media_log, ComD3D12Device device, ComD3D12CommandQueue command_queue, ComD3D12CommandAllocator command_allocator,
        ComD3D12VideoDevice video_device, ComD3D12VideoDecoder video_decoder, ComD3D12VideoDecoderHeap video_decoder_heap,
        ComD3D12VideoDecodeCommandList command_list, scoped_refptr<D3D12Fence> fence)
    ...
    bool WaitForFrameBegins(D3D11PictureBuffer* output_picture) override
    {
        ...
        RETURN_IF_FAILED("Failed to reset command allocator", D3D11Status::Codes::kDecoderBeginFrameFailed, hr);
        hr = command_list_->Reset(command_allocator_.Get());
        RETURN_IF_FAILED("Failed to reset command list", D3D11Status::Codes::kDecoderBeginFrameFailed, hr);

        // Previous output texture will be read as a reference frame now.
        if (output_stream_arguments_.pOutputTexture2D) {
            auto barriers = CreateD3D12TransitionBarriersForAllPlanes(output_stream_arguments_.pOutputTexture2D, output_stream_arguments_.OutputSubresource,
        ...
        CHECK(desc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_NTHANDLE) << "Cannot create shared handle for d3d11 texture2d without "
                                                                       "D3D11_RESOURCE_MISC_SHARED_NTHANDLE flag.";

        auto d3d12_resource_or_error = output_picture->ToD3D12Resource(device_.Get());
        if (!d3d12_resource_or_error.has_value()) {
            return false;
        }
        ComD3D12Resource d3d12_resource = std::move(d3d12_resource_or_error).value();

        output_stream_arguments_ = { .pOutputTexture2D = d3d12_resource.Get(), .OutputSubresource = output_picture->array_slice() };
        reference_frame_list_.emplace(output_picture->picture_index(), d3d12_resource.Get(), output_picture->array_slice());
    ...
    bool SubmitSlice() override
    {
        if (!slice_info_bytes_.empty()) {
            ...
            CHECK_LT(input_stream_arguments_.NumFrameArguments, std::size(input_stream_arguments_.FrameArguments));
            input_stream_arguments_.FrameArguments[input_stream_arguments_.NumFrameArguments++] = {
    ...
    bool SubmitDecode() override
    {
        ...
        command_list_->DecodeFrame(video_decoder_.Get(), &output_stream_arguments_, &input_stream_arguments_);
        ...
        RETURN_IF_FAILED("Failed to close command list", D3D11Status::Codes::kSubmitDecoderBuffersFailed, hr);
        ...
        if (!fence_value_or_error.has_value()) {
            media_log_->NotifyError<D3D11StatusTraits>(std::move(fence_value_or_error).error());
            return false;
        }
...
private:
    ...
    std::unique_ptr<ScopedD3DBuffer> GetBuffer(BufferType type, uint32_t desired_size) override;
...
public:
    ScopedD3D12MemoryBuffer(D3D12VideoDecoderWrapperImpl* decoder, D3D12_VIDEO_DECODE_ARGUMENT_TYPE type, base::span<uint8_t> data)
        : ScopedD3DBuffer(data)
        , decoder_(*decoder)
        , type_(type)
    {
    }
    ~ScopedD3D12MemoryBuffer() override
    {
        ScopedD3D12MemoryBuffer::Commit();
    }
    ...
    bool Commit(uint32_t written_size) override
    {
        ...
        CHECK_LT(decoder_->input_stream_arguments_.NumFrameArguments, std::size(decoder_->input_stream_arguments_.FrameArguments));
        decoder_->input_stream_arguments_.FrameArguments[decoder_->input_stream_arguments_.NumFrameArguments++] = {
            .Type = type_,
            .Size = written_size,
            .pData = data_.data(),
        };
...
public:
    ScopedD3D12ResourceBuffer(D3D12VideoDecoderWrapperImpl* decoder, ID3D12Resource* resource, MediaLog* media_log)
        : resource_(resource)
        , media_log_(media_log->Clone())
    {
        ...
        if (FAILED(hr)) {
            MEDIA_PLOG(ERROR, hr, media_log_) << "Failed to map data of ID3D12Resource";
            return;
        }
        data_ = base::span(reinterpret_cast<uint8_t*>(mapped_data), static_cast<size_t>(resource_->GetDesc().Width));
    }
    ~ScopedD3D12ResourceBuffer() override
    {
        ScopedD3D12ResourceBuffer::Commit();
    }
    ...
    bool Commit() override
    {
        return Commit(resource_->GetDesc().Width);
    }
...
std::unique_ptr<ScopedD3DBuffer> D3D12VideoDecoderWrapperImpl::GetBuffer(BufferType type, uint32_t desired_size)
{
    switch (type) {
    case BufferType::kPictureParameters:
        picture_parameters_buffer_.resize(desired_size);
        return std::make_unique<ScopedD3D12MemoryBuffer>(this, D3D12_VIDEO_DECODE_ARGUMENT_TYPE_PICTURE_PARAMETERS, picture_parameters_buffer_);
    case BufferType::kInverseQuantizationMatrix:
        inverse_quantization_matrix_buffer_.resize(desired_size);
        return std::make_unique<ScopedD3D12MemoryBuffer>(
            this, D3D12_VIDEO_DECODE_ARGUMENT_TYPE_INVERSE_QUANTIZATION_MATRIX, inverse_quantization_matrix_buffer_);
    case BufferType::kSliceControl:
        slice_control_buffer_.resize(desired_size);
        return std::make_unique<ScopedD3D12MemoryBuffer>(this, D3D12_VIDEO_DECODE_ARGUMENT_TYPE_SLICE_CONTROL, slice_control_buffer_);
    case BufferType::kBitstream:
        ...
        HRESULT hr
            = device_->CreateCommittedResource(&heap_properties, {}, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&compressed_bitstream_));
        if (FAILED(hr)) {
            MEDIA_PLOG(ERROR, hr, media_log_) << "Failed to CreateCommittedResource";
            return std::make_unique<ScopedD3D12MemoryBuffer>(this, D3D12_VIDEO_DECODE_ARGUMENT_TYPE {}, base::span<uint8_t> {});
        }
        return std::make_unique<ScopedD3D12ResourceBuffer>(this, compressed_bitstream_.Get(), media_log_);
    }
    NOTREACHED();
}
...
std::unique_ptr<D3D12VideoDecoderWrapper> D3D12VideoDecoderWrapper::Create(
    MediaLog* media_log, ComD3D12VideoDevice video_device, VideoDecoderConfig config, uint8_t bit_depth, VideoChromaSampling chroma_sampling)
{
    GUID guid = GetD3D12VideoDecodeGUID(config.profile(), bit_depth, chroma_sampling);
    ...
    HRESULT hr = video_device->CheckFeatureSupport(D3D12_FEATURE_VIDEO_DECODE_SUPPORT, &feature, sizeof(feature));
    ...
    if (feature.SupportFlags != D3D12_VIDEO_DECODE_SUPPORT_FLAG_SUPPORTED) {
        MEDIA_LOG(ERROR, media_log) << "D3D12VideoDecoder does not support profile " << GetProfileName(config.profile()) << " with bit depth "
                                    << base::strict_cast<int>(bit_depth) << ", chroma subsampling format " << VideoChromaSamplingToString(chroma_sampling)
                                    << ", coded size " << config.coded_size().ToString() << ", and output dxgi format " << decode_format;
        return nullptr;
    }
    ...
    hr = video_device->CreateVideoDecoderHeap(&heap_desc, IID_PPV_ARGS(&video_decoder_heap));
    ...
    D3D12_COMMAND_QUEUE_DESC command_queue_desc { D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE };
    ...
    hr = device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, IID_PPV_ARGS(&command_allocator));
    STATIC_RETURN_IF_FAILED(hr, "D3D12Device CreateCommandAllocator failed");

    ComD3D12VideoDecodeCommandList command_list;
    hr = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, command_allocator.Get(), nullptr, IID_PPV_ARGS(&command_list));
    ...
    return std::make_unique<D3D12VideoDecoderWrapperImpl>(media_log, std::move(device), std::move(command_queue), std::move(command_allocator),
        std::move(video_device), std::move(video_decoder), std::move(video_decoder_heap), std::move(command_list), std::move(fence));
}
