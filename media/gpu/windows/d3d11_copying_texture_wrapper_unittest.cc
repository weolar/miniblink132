...
public:
    MockVideoProcessorProxy()
        : VideoProcessorProxy(nullptr, nullptr)
    {
    }
    ...
    HRESULT CreateVideoProcessorOutputView(
        ID3D11Texture2D* output_texture, D3D11_VIDEO_PROCESSOR_OUTPUT_VIEW_DESC* output_view_descriptor, ID3D11VideoProcessorOutputView** output_view) override
    {
        return MockCreateVideoProcessorOutputView();
    }

    HRESULT CreateVideoProcessorInputView(
        ID3D11Texture2D* input_texture, D3D11_VIDEO_PROCESSOR_INPUT_VIEW_DESC* input_view_descriptor, ID3D11VideoProcessorInputView** input_view) override
    {
    ...
    HRESULT VideoProcessorBlt(
        ID3D11VideoProcessorOutputView* output_view, UINT output_frameno, UINT stream_count, D3D11_VIDEO_PROCESSOR_STREAM* streams) override
    {
...
public:
    ...
    D3D11Status ProcessTexture(const gfx::ColorSpace& input_color_space, scoped_refptr<gpu::ClientSharedImage>& shared_image_dest) override
    {
        return MockProcessTexture();
    }

    D3D11Status Init(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, GetCommandBufferHelperCB get_helper_cb, ComD3D11Texture2D in_texture,
        size_t array_slice, scoped_refptr<media::D3D11PictureBuffer> picture_buffer,
        PictureBufferGPUResourceInitDoneCB picture_buffer_gpu_resource_init_done_cb) override
    {
...
class D3D11CopyingTexture2DWrapperTest : public ::testing::TestWithParam<std::tuple<HRESULT, HRESULT, HRESULT, bool, bool, bool, bool>> {
public:
#define FIELD(TYPE, NAME, INDEX)                                                                                                                               \
    TYPE Get##NAME()                                                                                                                                           \
    {                                                                                                                                                          \
        return std::get<INDEX>(GetParam());                                                                                                                    \
    }
...
#undef FIELD
    ...
    scoped_refptr<MockVideoProcessorProxy> ExpectProcessorProxy()
    {
        ...
        ON_CALL(*result.get(), MockInit(_, _))
            .WillByDefault(Return(GetProcessorProxyInit() ? D3D11Status::Codes::kOk : D3D11Status::Codes::kCreateVideoProcessorFailed));

        ON_CALL(*result.get(), MockCreateVideoProcessorOutputView()).WillByDefault(Return(GetCreateVideoProcessorOutputView()));

        ON_CALL(*result.get(), MockCreateVideoProcessorInputView()).WillByDefault(Return(GetCreateVideoProcessorInputView()));

        ON_CALL(*result.get(), MockVideoProcessorBlt()).WillByDefault(Return(GetVideoProcessorBlt()));
    ...
    std::unique_ptr<MockTexture2DWrapper> ExpectTextureWrapper()
    {
        ...
        ON_CALL(*result.get(), MockInit())
            .WillByDefault(Return(GetTextureWrapperInit() ? D3D11Status::Codes::kOk : D3D11Status::Codes::kCreateVideoProcessorFailed));

        ON_CALL(*result.get(), MockProcessTexture())
            .WillByDefault(Return(GetProcessTexture() ? D3D11Status::Codes::kOk : D3D11Status::Codes::kCreateVideoProcessorOutputViewFailed));
    ...
    {
        return GetProcessTexture() && SUCCEEDED(GetCreateVideoProcessorOutputView()) && SUCCEEDED(GetCreateVideoProcessorInputView())
...
INSTANTIATE_TEST_SUITE_P(CopyingTexture2DWrapperTest, D3D11CopyingTexture2DWrapperTest,
    Combine(Values(S_OK, E_FAIL), Values(S_OK, E_FAIL), Values(S_OK, E_FAIL), Bool(), Bool(), Bool(), Bool()));
...
TEST_P(D3D11CopyingTexture2DWrapperTest, CopyingTextureWrapperProcessesCorrectly)
{
    gfx::Size size;
    ...
    auto wrapper = std::make_unique<CopyingTexture2DWrapper>(size, std::move(texture_wrapper), processor, nullptr);
    ...
    EXPECT_EQ(wrapper->ProcessTexture(input_color_space, shared_image).is_ok(), ProcessTextureSucceeds());
