...
public:
    ...
    struct VideoBufferWrapper {
        ...
        raw_ptr<D3D11_VIDEO_DECODER_SUB_SAMPLE_MAPPING_BLOCK> pSubSampleMappingBlock;
        UINT SubSampleMappingCount;
    };

    static std::unique_ptr<VideoContextWrapper> CreateWrapper(D3D_FEATURE_LEVEL supported_d3d11_version, ComD3D11DeviceContext device_context, HRESULT* status);
    ...
    virtual HRESULT DecoderBeginFrame(
        ID3D11VideoDecoder* video_decoder, ID3D11VideoDecoderOutputView* output_view, UINT content_key_size, const void* content_key)
        = 0;

    // This method signiture is defined to match exactly that of
    // |ID3D11VideoContext::GetDecoderBuffer|.
    virtual HRESULT GetDecoderBuffer(ID3D11VideoDecoder* video_decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type, UINT* buffer_size, void** buffer) = 0;

    // This method signiture is defined to match exactly that of
    // |ID3D11VideoContext::ReleaseDecoderBuffer|.
    virtual HRESULT ReleaseDecoderBuffer(ID3D11VideoDecoder* video_decoder, D3D11_VIDEO_DECODER_BUFFER_TYPE type) = 0;
    ...
    virtual HRESULT SubmitDecoderBuffers(ID3D11VideoDecoder* video_decoder, UINT num_buffers, const VideoBufferWrapper* buffers) = 0;
}; // VideoContextWrapper
