...
void D3D12ReferenceFrameList::WriteTo(D3D12_VIDEO_DECODE_REFERENCE_FRAMES* dest)
{
...
void D3D12ReferenceFrameList::emplace(size_t index, ID3D12Resource* resource, UINT subresource)
{
    if (index >= size_) {
        size_ = index + 1;
...
ComD3D12Device CreateD3D12Device(IDXGIAdapter* adapter)
{
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "D3D12CreateDevice failed: " << logging::SystemErrorCodeToString(hr);
        return nullptr;
    }
...
absl::InlinedVector<D3D12_RESOURCE_BARRIER, 2> CreateD3D12TransitionBarriersForAllPlanes(
    ID3D12Resource* resource, UINT subresource, D3D12_RESOURCE_STATES state_before, D3D12_RESOURCE_STATES state_after)
{
    CHECK(resource);
    ...
    for (size_t i = 0; i < GetFormatPlaneCount(desc.Format); i++) {
        barriers.push_back({ .Transition = { .pResource = resource,
                                 .Subresource = D3D12CalcSubresource(subresource, 0, i, desc.MipLevels, desc.DepthOrArraySize),
...
GUID GetD3D12VideoDecodeGUID(VideoCodecProfile profile, uint8_t bitdepth, VideoChromaSampling chroma_sampling)
{
    switch (profile) {
