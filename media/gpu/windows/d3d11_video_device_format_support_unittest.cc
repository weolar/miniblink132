...
TEST_F(FormatSupportCheckerUnittest, CheckInitializationCantCast)
{
    ...
    ON_CALL(*device.Get(), QueryInterface(IID_ID3D11VideoDevice, _)).WillByDefault(SetComPointeeAndReturnOk<1>(vdevice.Get()));

    EXPECT_CALL(*vdevice.Get(), CreateVideoProcessorEnumerator(_, _)).WillOnce(SetComPointeeAndReturnOk<1>(enumerator.Get()));

    EXPECT_CALL(*device.Get(), CheckFormatSupport(_, _)).WillOnce(Return(S_OK));

    EXPECT_CALL(*enumerator.Get(), CheckVideoProcessorFormat(_, _)).WillOnce(Return(S_OK));
...
TEST_F(FormatSupportCheckerUnittest, CheckFormatSupportWorks)
{
    ...
    EXPECT_CALL(*enumerator.Get(), CheckVideoProcessorFormat(_, _)).WillOnce(DoAll(SetArgPointee<1>(enumerator_outcome), Return(S_OK)));
    EXPECT_CALL(*device.Get(), CheckFormatSupport(_, _)).WillOnce(DoAll(SetArgPointee<1>(device_outcome), Return(S_OK)));
