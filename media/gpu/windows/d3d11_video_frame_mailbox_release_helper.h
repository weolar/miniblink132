...
class MEDIA_GPU_EXPORT D3D11VideoFrameMailboxReleaseHelper : public base::RefCountedThreadSafe<D3D11VideoFrameMailboxReleaseHelper> {
public:
    // May be constructed on any thread.
    D3D11VideoFrameMailboxReleaseHelper(std::unique_ptr<MediaLog> media_log, base::OnceCallback<scoped_refptr<CommandBufferHelper>()> get_helper_cb);

    D3D11VideoFrameMailboxReleaseHelper(const D3D11VideoFrameMailboxReleaseHelper&) = delete;
    D3D11VideoFrameMailboxReleaseHelper& operator=(const D3D11VideoFrameMailboxReleaseHelper&) = delete;

    // Callback to us to wait for a sync token, then call a closure.
    using ReleaseMailboxCB = base::RepeatingCallback<void(base::OnceClosure, const gpu::SyncToken&)>;
...
private:
    ...
    void OnMailboxReleased(base::OnceClosure wait_complete_cb, const gpu::SyncToken& sync_token);
