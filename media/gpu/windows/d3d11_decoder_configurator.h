...
public:
    D3D11DecoderConfigurator(DXGI_FORMAT decoder_output_dxgifmt, GUID decoder_guid, gfx::Size coded_size, bool is_encrypted, bool supports_swap_chain);
    virtual ~D3D11DecoderConfigurator() = default;

    static std::unique_ptr<D3D11DecoderConfigurator> Create(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& workarounds,
        const VideoDecoderConfig& config, uint8_t bit_depth, VideoChromaSampling chroma_sampling, MediaLog* media_log, bool use_shared_handle);

    bool SupportsDevice(ComD3D11VideoDevice video_device);

    // Create the decoder's output texture.
    D3D11Status::Or<ComD3D11Texture2D> CreateOutputTexture(ComD3D11Device device, gfx::Size size, uint32_t array_size, bool use_shared_handle);

    const D3D11_VIDEO_DECODER_DESC* DecoderDescriptor() const
    {
        return &decoder_desc_;
    }
    const GUID DecoderGuid() const
    {
        return decoder_guid_;
    }
