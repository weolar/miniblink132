...
public:
    ...
    static std::unique_ptr<DXVAPictureBuffer> Create(const DXVAVideoDecodeAccelerator& decoder, const PictureBuffer& buffer, EGLConfig egl_config);
    ...
    {
        return state_ == UNUSED;
    }
    State state() const
    {
        return state_;
    }
    const PictureBuffer& picture_buffer() const
    {
    ...
    const std::vector<scoped_refptr<Picture::ScopedSharedImage>>& shared_images() const
    {
        return shared_images_;
    }

    void set_shared_image(size_t plane, scoped_refptr<Picture::ScopedSharedImage> shared_image)
    {
    ...
    bool waiting_to_reuse() const
    {
        return state_ == WAITING_TO_REUSE;
    }
    ...
    virtual bool BindSampleToTexture(DXVAVideoDecodeAccelerator* decoder, Microsoft::WRL::ComPtr<IMFSample> sample);
...
public:
    ...
    bool Initialize(const DXVAVideoDecodeAccelerator& decoder, EGLConfig egl_config);
    bool InitializeTexture(const DXVAVideoDecodeAccelerator& decoder, bool use_rgb, bool use_fp16);

    bool ReusePictureBuffer() override;
    void ResetReuseFence() override;
    bool CopyOutputSampleDataToPictureBuffer(
        DXVAVideoDecodeAccelerator* decoder, IDirect3DSurface9* dest_surface, ID3D11Texture2D* dx11_texture, int input_buffer_id) override;
    gl::GLFence* reuse_fence() override;
    bool CopySurfaceComplete(IDirect3DSurface9* src_surface, IDirect3DSurface9* dest_surface) override;
...
public:
    ...
    bool Initialize();
    bool ReusePictureBuffer() override;
    bool BindSampleToTexture(DXVAVideoDecodeAccelerator* decoder, Microsoft::WRL::ComPtr<IMFSample> sample) override;
    bool AllowOverlay() const override;
    bool CanBindSamples() const override;
