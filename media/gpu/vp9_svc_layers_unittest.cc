...
gfx::Size GetDefaultSVCResolution(size_t spatial_index)
{
    ...
    return gfx::Size(kDefaultEncodeSize.width() / denom, kDefaultEncodeSize.height() / denom);
}
...
VP9SVCLayers::Config GetDefaultSVCLayers2Config(size_t num_spatial_layers, size_t num_temporal_layers, SVCInterLayerPredMode inter_layer_pred)
{
    const auto& spatial_layer_resolutions = GetDefaultSVCResolutions(num_spatial_layers);
    return VP9SVCLayers::Config(spatial_layer_resolutions,
        /*begin_active_layer=*/0, spatial_layer_resolutions.size(), num_temporal_layers, inter_layer_pred);
}
...
struct Vp9MetadataAndFrameNum {
    ...
    Vp9MetadataAndFrameNum()
        : frame_num(kInvalidFrameNum)
    {
    }
    ...
    bool is_valid() const
    {
        return frame_num != kInvalidFrameNum;
    }
...
void VerifyReferenceFrame(const Vp9Metadata& metadata, size_t frame_num, const Vp9MetadataAndFrameNum& ref_frame, SVCInterLayerPredMode inter_layer_pred)
{
    ...
    if (frame_num == 0 && inter_layer_pred == SVCInterLayerPredMode::kOnKeyPic) {
        ...
        const std::vector<uint8_t> expected_p_diffs = { base::checked_cast<uint8_t>(frame_num - ref_frame.frame_num) };
...
void VerifykSVCFrame(const VP9SVCLayers::PictureParam& picture_param, const Vp9Metadata& metadata,
    const std::array<Vp9MetadataAndFrameNum, kVp9NumRefFrames>& ref_frames, size_t frame_num, size_t num_spatial_layers, size_t expected_begin_active_layer,
    size_t expected_end_active_layer)
{
    ...
    if (picture_param.key_frame) {
        ...
        EXPECT_EQ(metadata.referenced_by_upper_spatial_layers, num_spatial_layers > 1);
        EXPECT_FALSE(metadata.reference_lower_spatial_layers);
        EXPECT_EQ(metadata.spatial_layer_resolutions, GetDefaultSVCResolutions(num_spatial_layers));
        EXPECT_EQ(metadata.begin_active_spatial_layer_index, expected_begin_active_layer);
        EXPECT_EQ(metadata.end_active_spatial_layer_index, expected_end_active_layer);
        EXPECT_TRUE(metadata.p_diffs.empty());
    ...
    if (frame_num == 0) {
        ...
        EXPECT_EQ(metadata.referenced_by_upper_spatial_layers, spatial_index + 1 != num_spatial_layers);
    } else {
    ...
    VerifyReferenceFrame(metadata, frame_num, ref_frames[picture_param.reference_frame_indices[0]], SVCInterLayerPredMode::kOnKeyPic);
}

void VerifySmodeFrame(const VP9SVCLayers::PictureParam& picture_param, const Vp9Metadata& metadata,
    const std::array<Vp9MetadataAndFrameNum, kVp9NumRefFrames>& ref_frames, size_t frame_num, size_t num_spatial_layers, size_t expected_begin_active_layer,
    size_t expected_end_active_layer)
{
    const uint8_t temporal_index = metadata.temporal_idx;
    ...
    if (picture_param.key_frame) {
        ...
        if (spatial_index == 0) {
            ...
            EXPECT_EQ(picture_param.refresh_frame_flags, (0x1 << (spatial_index * 2)));
        }
        EXPECT_TRUE(picture_param.reference_frame_indices.empty());
        return;
    }
    ...
    // Check that the current frame doesn't reference upper layer frames.
    VerifyReferenceFrame(metadata, frame_num, ref_frames[picture_param.reference_frame_indices[0]], SVCInterLayerPredMode::kOff);
}
} // namespace

class VP9SVCLayersTest : public ::testing::TestWithParam<::testing::tuple<size_t, size_t, SVCInterLayerPredMode>> { };
...
TEST_P(VP9SVCLayersTest, VerifyMetadata)
{
    ...
    const SVCInterLayerPredMode inter_layer_pred_mode = ::testing::get<2>(GetParam());

    const VP9SVCLayers::Config config = GetDefaultSVCLayers2Config(num_spatial_layers, num_temporal_layers, inter_layer_pred_mode);
    ...
    for (size_t i = 0; i < kNumFramesToEncode; ++i) {
        ...
        for (size_t sid = 0; sid < num_spatial_layers; ++sid) {
            ...
            EXPECT_EQ(GetTemporalIndex(num_temporal_layers, frame_num), metadata.temporal_idx);

            if (inter_layer_pred_mode == SVCInterLayerPredMode::kOnKeyPic) {
                VerifykSVCFrame(picture_param, metadata, ref_frames, frame_num, config.active_spatial_layer_resolutions.size(), config.begin_active_layer,
                    config.end_active_layer);
            } else {
                VerifySmodeFrame(picture_param, metadata, ref_frames, frame_num, config.active_spatial_layer_resolutions.size(), config.begin_active_layer,
                    config.end_active_layer);
            }
...
INSTANTIATE_TEST_SUITE_P(, VP9SVCLayersTest,
    ::testing::Values(std::make_tuple(1, 2, SVCInterLayerPredMode::kOff), std::make_tuple(1, 3, SVCInterLayerPredMode::kOff),
        std::make_tuple(1, 2, SVCInterLayerPredMode::kOn), std::make_tuple(1, 3, SVCInterLayerPredMode::kOn),
        std::make_tuple(1, 2, SVCInterLayerPredMode::kOnKeyPic), std::make_tuple(1, 3, SVCInterLayerPredMode::kOnKeyPic),
        std::make_tuple(2, 1, SVCInterLayerPredMode::kOnKeyPic), std::make_tuple(2, 2, SVCInterLayerPredMode::kOnKeyPic),
        std::make_tuple(2, 3, SVCInterLayerPredMode::kOnKeyPic), std::make_tuple(3, 1, SVCInterLayerPredMode::kOnKeyPic),
        std::make_tuple(3, 2, SVCInterLayerPredMode::kOnKeyPic), std::make_tuple(3, 3, SVCInterLayerPredMode::kOnKeyPic),
        std::make_tuple(2, 1, SVCInterLayerPredMode::kOff), std::make_tuple(2, 2, SVCInterLayerPredMode::kOff),
        std::make_tuple(2, 3, SVCInterLayerPredMode::kOff), std::make_tuple(3, 1, SVCInterLayerPredMode::kOff),
        std::make_tuple(3, 2, SVCInterLayerPredMode::kOff), std::make_tuple(3, 3, SVCInterLayerPredMode::kOff)));
} // namespace media
