...
public:
    ...
    VideoEncoderClientConfig GetDefaultConfig()
    {
        ...
        return VideoEncoderClientConfig(g_env->Video(), g_env->Profile(), spatial_layers, g_env->BitrateAllocation(), g_env->Reverse());
    }

    std::unique_ptr<VideoEncoder> CreateVideoEncoder(Video* video, const VideoEncoderClientConfig& config)
    {
        LOG_ASSERT(video);

        auto video_encoder = VideoEncoder::Create(config, CreateBitstreamProcessors(video, config));
...
private:
    std::unique_ptr<BitstreamProcessor> CreateBitstreamValidator(const Video* video, const VideoDecoderConfig& decoder_config, const size_t last_frame_index,
        VideoFrameValidator::GetModelFrameCB get_model_frame_cb, absl::optional<size_t> spatial_layer_index_to_decode,
        absl::optional<size_t> temporal_layer_index_to_decode, const std::vector<gfx::Size>& spatial_layer_resolutions)
    {
        ...
        if (frame_output_config.output_mode != FrameOutputMode::kNone) {
            ...
            if (spatial_layer_index_to_decode) {
                output_file_prefix += FILE_PATH_LITERAL("SL") + base::NumberToString(*spatial_layer_index_to_decode);
            }
            if (temporal_layer_index_to_decode) {
                output_file_prefix += FILE_PATH_LITERAL("TL") + base::NumberToString(*temporal_layer_index_to_decode);
            }
        ...
        const double ssim_tolerance
            = encode_resolution.GetArea() < k360p.GetArea() ? kSSIMToleranceForLowerResolution : SSIMVideoFrameValidator::kDefaultTolerance;

        auto ssim_validator
            = SSIMVideoFrameValidator::Create(get_model_frame_cb, std::move(image_writer), VideoFrameValidator::ValidationMode::kAverage, ssim_tolerance);
    ...
    std::vector<std::unique_ptr<BitstreamProcessor>> CreateBitstreamProcessors(Video* video, const VideoEncoderClientConfig& config)
    {
        ...
        if (g_env->SaveOutputBitstream()) {
            base::FilePath::StringPieceType extension = codec == VideoCodec::kH264 ? FILE_PATH_LITERAL("h264") : FILE_PATH_LITERAL("ivf");
            auto output_bitstream_filepath
                = g_env->OutputFolder().Append(g_env->GetTestOutputFilePath()).Append(video->FilePath().BaseName().ReplaceExtension(extension));
            if (!spatial_layer_resolutions.empty()) {
                ...
                for (size_t spatial_layer_index_to_write = 0; spatial_layer_index_to_write < config.num_spatial_layers; ++spatial_layer_index_to_write) {
                    ...
                    for (size_t temporal_layer_index_to_write = 0; temporal_layer_index_to_write < config.num_temporal_layers;
                        ...
                        bitstream_processors.emplace_back(BitstreamFileWriter::Create(
                            output_bitstream_filepath.InsertBeforeExtensionASCII(FILE_PATH_LITERAL(".SL") + base::NumberToString(spatial_layer_index_to_write)
                                + FILE_PATH_LITERAL(".TL") + base::NumberToString(temporal_layer_index_to_write)),
                            codec, layer_size, config.framerate, config.num_frames_to_encode, spatial_layer_index_to_write, temporal_layer_index_to_write,
                ...
                bitstream_processors.emplace_back(
                    BitstreamFileWriter::Create(output_bitstream_filepath, codec, visible_rect.size(), config.framerate, config.num_frames_to_encode));
                LOG_ASSERT(bitstream_processors.back());
            }
        }
        ...
        case VideoCodec::kH264:
            bitstream_processors.emplace_back(new H264Validator(config.output_profile, visible_rect, config.num_temporal_layers));
            break;
        case VideoCodec::kVP8:
            bitstream_processors.emplace_back(new VP8Validator(visible_rect, config.num_temporal_layers));
            break;
        case VideoCodec::kVP9:
            bitstream_processors.emplace_back(new VP9Validator(config.output_profile, visible_rect, config.num_spatial_layers, config.num_temporal_layers));
            break;
        default:
            LOG(ERROR) << "Unsupported profile: " << GetProfileName(config.output_profile);
            break;
        }
        ...
        if (!spatial_layer_resolutions.empty()) {
            ...
            for (size_t spatial_layer_index_to_decode = 0; spatial_layer_index_to_decode < config.num_spatial_layers; ++spatial_layer_index_to_decode) {
                ...
                for (size_t temporal_layer_index_to_decode = 0; temporal_layer_index_to_decode < config.num_temporal_layers; ++temporal_layer_index_to_decode) {
                    bitstream_processors.emplace_back(CreateBitstreamValidator(video, decoder_config, config.num_frames_to_encode - 1, get_model_frame_cb,
                        spatial_layer_index_to_decode, temporal_layer_index_to_decode, spatial_layer_resolutions));
                    LOG_ASSERT(bitstream_processors.back());
                }
            }
        } else {
            ...
            bitstream_processors.emplace_back(
                CreateBitstreamValidator(video, decoder_config, config.num_frames_to_encode - 1, get_model_frame_cb, absl::nullopt, absl::nullopt,
                    /*spatial_layer_resolutions=*/ {}));
            LOG_ASSERT(bitstream_processors.back());
        }
        return bitstream_processors;
...
{
    return g_env->IsKeplerUsed() ? absl::make_optional<std::string>("The rate controller in the kepler firmware doesn't handle "
...
{
    return g_env->IsKeplerUsed() ? absl::make_optional<std::string>("Encoding with dmabuf input frames is not "
                                                                    "supported in kepler.")
                                 : absl::nullopt;
}
...
TEST_F(VideoEncoderTest, BitrateCheck)
{
    ...
    // on some boards.
    const double tolerance = config.bitrate_allocation.GetMode() == Bitrate::Mode::kConstant ? kBitrateTolerance : kVariableBitrateTolerance;
    EXPECT_NEAR(encoder->GetStats().Bitrate(), config.bitrate_allocation.GetSumBps(), tolerance * config.bitrate_allocation.GetSumBps());
}
...
TEST_F(VideoEncoderTest, BitrateCheck_DynamicBitrate)
{
    ...
    if (g_env->BitrateAllocation().GetMode() != Bitrate::Mode::kConstant) {
        GTEST_SKIP() << "Skip Dynamic bitrate change checks for non-CBR bitrate mode";
    }
    ...
    EXPECT_TRUE(encoder->WaitUntilIdle());
    EXPECT_NEAR(encoder->GetStats().Bitrate(), first_bitrate, kBitrateTolerance * first_bitrate);

    // Encode the video with the second bitrate.
    const uint32_t second_bitrate = first_bitrate * 3 / 2;
    encoder->ResetStats();
    encoder->UpdateBitrate(
        AllocateDefaultBitrateForTesting(config.num_spatial_layers, config.num_temporal_layers, Bitrate::ConstantBitrate(second_bitrate)), config.framerate);
    encoder->Encode();
    EXPECT_TRUE(encoder->WaitForFlushDone());
    EXPECT_NEAR(encoder->GetStats().Bitrate(), second_bitrate, kBitrateTolerance * second_bitrate);
...
TEST_F(VideoEncoderTest, BitrateCheck_DynamicFramerate)
{
    if (g_env->SpatialLayers().size() > 1)
        GTEST_SKIP() << "Skip SHMEM input test cases in spatial SVC encoding";
    if (g_env->BitrateAllocation().GetMode() != Bitrate::Mode::kConstant) {
        GTEST_SKIP() << "Skip dynamic framerate change checks for non-CBR bitrate mode";
    }
    ...
    encoder->EncodeUntil(VideoEncoder::kFrameReleased, kNumFramesToEncodeForBitrateCheck);
    EXPECT_TRUE(encoder->WaitUntilIdle());
    EXPECT_NEAR(encoder->GetStats().Bitrate(), config.bitrate_allocation.GetSumBps(), kBitrateTolerance * config.bitrate_allocation.GetSumBps());
...
TEST_F(VideoEncoderTest, FlushAtEndOfStream_NV12Dmabuf)
{
    ...
    VideoEncoderClientConfig config(nv12_video, g_env->Profile(), g_env->SpatialLayers(), g_env->BitrateAllocation(), g_env->Reverse());
...
TEST_F(VideoEncoderTest, FlushAtEndOfStream_NV12DmabufScaling)
{
    ...
    VideoEncoderClientConfig config(nv12_video, g_env->Profile(), spatial_layers,
...
TEST_F(VideoEncoderTest, FlushAtEndOfStream_NV12DmabufCroppingTopAndBottom)
{
    ...
    auto nv12_expanded_video = g_env->GenerateNV12Video()->Expand(expanded_resolution, expanded_visible_rect);
    ASSERT_TRUE(nv12_expanded_video);
    VideoEncoderClientConfig config(nv12_expanded_video.get(), g_env->Profile(), g_env->SpatialLayers(), g_env->BitrateAllocation(), g_env->Reverse());
...
int main(int argc, char** argv)
{
    ...
    for (base::CommandLine::SwitchMap::const_iterator it = switches.begin(); it != switches.end(); ++it) {
        ...
        if (it->first == "codec") {
            ...
            if (it->second == "all") {
                ...
                std::cout << "unknown image output mode \"" << it->second << "\", possible values are \"all|corrupt\"\n";
                return EXIT_FAILURE;
            }
            ...
            if (it->second == "png") {
                ...
                std::cout << "unknown frame output format \"" << it->second << "\", possible values are \"png|yuv\"\n";
                return EXIT_FAILURE;
            }
    ...
    media::test::VideoEncoderTestEnvironment* test_environment = media::test::VideoEncoderTestEnvironment::Create(video_path, video_metadata_path,
        enable_bitstream_validator, output_folder, codec, num_temporal_layers, num_spatial_layers, output_bitstream,
        /*output_bitrate=*/absl::nullopt, bitrate_mode, reverse, frame_output_config,
