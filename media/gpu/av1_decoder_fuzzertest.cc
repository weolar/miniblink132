...
public:
    ...
    Status SubmitDecode(const media::AV1Picture& pic, const libgav1::ObuSequenceHeader& sequence_header, const media::AV1ReferenceFrameVector& ref_frames,
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    media::AV1Decoder decoder(std::make_unique<FakeAV1Accelerator>(), media::AV1PROFILE_PROFILE_MAIN);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        std::vector<uint8_t> decoder_buffer_data = fuzzed_data_provider.ConsumeBytes<uint8_t>(size_to_consume);
        ...
        scoped_refptr<media::DecoderBuffer> decoder_buffer = media::DecoderBuffer::CopyFrom(decoder_buffer_data);
