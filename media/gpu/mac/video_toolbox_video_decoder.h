...
public:
    ...
    VideoToolboxVideoDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log,
    ...
    static std::vector<SupportedVideoDecoderConfig> GetSupportedVideoDecoderConfigs(const gpu::GpuDriverBugWorkarounds& gpu_workarounds);
...
private:
    ...
    void OnVideoToolboxOutput(base::apple::ScopedCFTypeRef<CVImageBufferRef> image, std::unique_ptr<VideoToolboxDecodeMetadata> metadata);
    void OnVideoToolboxError(DecoderStatus status);

    // |converter_| callbacks.
    void OnConverterOutput(scoped_refptr<VideoFrame> frame, std::unique_ptr<VideoToolboxDecodeMetadata> metadata);
    ...
    base::WeakPtrFactory<VideoToolboxVideoDecoder> converter_weak_this_factory_ { this };
};
