...
VideoToolboxVP9Accelerator::Status VideoToolboxVP9Accelerator::SubmitDecode(scoped_refptr<VP9Picture> pic, const Vp9SegmentationParams& segm_params,
    const Vp9LoopFilterParams& lf_params, const Vp9ReferenceFrameVector& reference_frames)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool VideoToolboxVP9Accelerator::ProcessFrame(scoped_refptr<VP9Picture> pic)
{
    ...
    AppendData(frame_data_.get(), pic->frame_hdr->data.data(), pic->frame_hdr->data.size());
...
bool VideoToolboxVP9Accelerator::ProcessFormat(scoped_refptr<VP9Picture> pic, bool* format_changed)
{
    DVLOG(4) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    gfx::Size coded_size(static_cast<int>(pic->frame_hdr->frame_width), static_cast<int>(pic->frame_hdr->frame_height));
    ...
    if (color_space != active_color_space_ || profile != active_profile_ || hdr_metadata != active_hdr_metadata_ || coded_size != active_coded_size_) {
        ...
        base::apple::ScopedCFTypeRef<CFDictionaryRef> format_config
            = CreateFormatExtensions(kCMVideoCodecType_VP9, profile, pic->frame_hdr->bit_depth, color_space, hdr_metadata, std::nullopt);
        if (!format_config) {
        ...
        OSStatus status = CMVideoFormatDescriptionCreate(
            kCFAllocatorDefault, kCMVideoCodecType_VP9, coded_size.width(), coded_size.height(), format_config.get(), active_format_.InitializeInto());
        if (status != noErr) {
...
bool VideoToolboxVP9Accelerator::SubmitFrames(scoped_refptr<VP9Picture> output_pic)
{
    DVLOG(4) << __func__;
    ...
    if (frame_sizes.size() > 1) {
        ...
        uint8_t header = (kSuperframeMarker << 5) | ((kBytesPerFrameSize - 1) << 3) | (frame_sizes.size() - 1);

        size_t trailer_size = kSuperframeHeaderSize + kBytesPerFrameSize * frame_sizes.size() + kSuperframeHeaderSize;
...
bool VideoToolboxVP9Accelerator::AppendData(CMBlockBufferRef dest, const uint8_t* data, size_t data_size)
{
    DVLOG(4) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
