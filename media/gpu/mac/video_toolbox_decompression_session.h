...
public:
    ...
    virtual bool Create(CMFormatDescriptionRef format, CFDictionaryRef decoder_config, CFDictionaryRef image_config) = 0;
...
// |task_runner|.
class MEDIA_GPU_EXPORT VideoToolboxDecompressionSessionImpl : public VideoToolboxDecompressionSession {
public:
    using OutputCB = base::RepeatingCallback<void(uintptr_t, OSStatus, VTDecodeInfoFlags, base::apple::ScopedCFTypeRef<CVImageBufferRef>)>;

    VideoToolboxDecompressionSessionImpl(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log, OutputCB output_cb);
    ...
    void OnOutputOnAnyThread(uintptr_t context, OSStatus status, VTDecodeInfoFlags flags, base::apple::ScopedCFTypeRef<CVImageBufferRef> image);
...
private:
    ...
    base::WeakPtrFactory<VideoToolboxDecompressionSessionImpl> weak_this_factory_ { this };
};
