...
void ReleaseDecoderBuffer(void* refcon, void* doomed_memory_block, size_t size_in_bytes)
{
    if (refcon)
...
bool VP9SuperFrameBitstreamFilter::EnqueueBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    ...
    const bool is_superframe = (buffer->data()[buffer->data_size() - 1] & 0xE0) == kSuperFrameMarker;
...
bool VP9SuperFrameBitstreamFilter::PreparePassthroughBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    ...
    OSStatus status = CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault, static_cast<void*>(buffer->writable_data()), buffer->data_size(),
        kCFAllocatorDefault, &source, 0, buffer->data_size(), 0, data_.InitializeInto());
    if (status != noErr) {
        OSSTATUS_DLOG(ERROR, status) << "CMBlockBufferCreateWithMemoryBlock failed.";
        return false;
    }
...
bool VP9SuperFrameBitstreamFilter::AllocateCombinedBlock(size_t total_size)
{
    ...
    OSStatus status
        = CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault, nullptr, total_size, kCFAllocatorDefault, nullptr, 0, total_size, 0, data_.InitializeInto());
    if (status != noErr) {
        OSSTATUS_DLOG(ERROR, status) << "CMBlockBufferCreateWithMemoryBlock failed.";
        return false;
    }
...
bool VP9SuperFrameBitstreamFilter::MergeBuffer(const DecoderBuffer& buffer, size_t offset)
{
    OSStatus status = CMBlockBufferReplaceDataBytes(buffer.data(), data_, offset, buffer.data_size());
    if (status != noErr) {
...
bool VP9SuperFrameBitstreamFilter::BuildSuperFrame()
{
    ...
    const uint8_t bytes_per_frame_size = base::bits::AlignUp(base::bits::Log2Ceiling(base::checked_cast<uint32_t>(max_size)), 8) / 8;
    ...
    const uint8_t marker = kSuperFrameMarker + ((bytes_per_frame_size - 1) << 3) + (partial_buffers_.size() - 1);
    ...
    OSStatus status = CMBlockBufferReplaceDataBytes(trailer.get(), data_, offset, trailer_size);
    if (status != noErr) {
        OSSTATUS_DLOG(ERROR, status) << "CMBlockBufferReplaceDataBytes failed.";
        return false;
    }
