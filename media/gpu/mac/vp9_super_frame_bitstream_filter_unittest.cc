...
public:
    ...
    void LoadTestData(const char* file_name)
    {
        ...
        protocol_ = std::make_unique<InMemoryUrlProtocol>(buffer_->data(), buffer_->data_size(), false);
...
TEST_F(VP9SuperFrameBitstreamFilterTest, Passthrough)
{
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        ASSERT_EQ(noErr, CMBlockBufferCopyDataBytes(cm_block, 0, buffer->data_size(), block_data.get()));
...
TEST_F(VP9SuperFrameBitstreamFilterTest, Superframe)
{
    ...
    ASSERT_EQ(noErr, CMBlockBufferCopyDataBytes(cm_block, 0, kExpectedTotalSize, block_data.get()));
