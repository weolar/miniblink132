...
VideoToolboxOutputQueue::VideoToolboxOutputQueue(scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void VideoToolboxOutputQueue::SetOutputCB(const VideoDecoder::OutputCB output_cb)
{
    DVLOG(2) << __func__;
    output_cb_ = std::move(output_cb);
}

void VideoToolboxOutputQueue::SchedulePicture(scoped_refptr<CodecPicture> picture)
{
    DVLOG(3) << __func__;
...
void VideoToolboxOutputQueue::FulfillPicture(scoped_refptr<CodecPicture> picture, scoped_refptr<VideoFrame> frame)
{
    DVLOG(3) << __func__;
...
void VideoToolboxOutputQueue::Reset(DecoderStatus status)
{
    ...
    if (flush_cb_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(flush_cb_), status));
    }
...
void VideoToolboxOutputQueue::Process()
{
    ...
    while (!scheduled_pictures_.empty()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(output_cb_, std::move(fulfilled_it->second)));
    ...
    if (flush_cb_ && scheduled_pictures_.empty()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(flush_cb_), DecoderStatus::Codes::kOk));
    }
}
