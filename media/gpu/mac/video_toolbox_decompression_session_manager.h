...
public:
    using OutputCB = base::RepeatingCallback<void(base::apple::ScopedCFTypeRef<CVImageBufferRef>, std::unique_ptr<VideoToolboxDecodeMetadata> metadata)>;
    using ErrorCB = base::OnceCallback<void(DecoderStatus)>;

    VideoToolboxDecompressionSessionManager(
        scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log, OutputCB output_cb, ErrorCB error_cb);

    ~VideoToolboxDecompressionSessionManager();

    // Decode |sample|, tagged with |context|.
    void Decode(base::apple::ScopedCFTypeRef<CMSampleBufferRef> sample, std::unique_ptr<VideoToolboxDecodeMetadata> metadata);
    ...
    // Public for testing.
    void SetDecompressionSessionForTesting(std::unique_ptr<VideoToolboxDecompressionSession> decompression_session);

    // Public for testing.
    void OnOutput(uintptr_t context, OSStatus status, VTDecodeInfoFlags flags, base::apple::ScopedCFTypeRef<CVImageBufferRef> image);
...
private:
    ...
    [[nodiscard]] bool CreateSession(CMFormatDescriptionRef format, const VideoToolboxDecompressionSessionMetadata& session_metadata);
    ...
    base::queue<std::pair<base::apple::ScopedCFTypeRef<CMSampleBufferRef>, std::unique_ptr<VideoToolboxDecodeMetadata>>> pending_decodes_;
    ...
    base::flat_map<uintptr_t, std::unique_ptr<VideoToolboxDecodeMetadata>> active_decodes_;
    ...
    base::WeakPtrFactory<VideoToolboxDecompressionSessionManager> weak_this_factory_ { this };
};
