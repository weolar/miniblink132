...
// interface for macOS.
class VTVideoDecodeAccelerator : public VideoDecodeAccelerator, public base::trace_event::MemoryDumpProvider {
public:
    VTVideoDecodeAccelerator(const GpuVideoDecodeGLClient& gl_client_, const gpu::GpuDriverBugWorkarounds& workarounds, MediaLog* media_log);
    ...
    void AssignPictureBuffers(const std::vector<PictureBuffer>& pictures) override;
    ...
    void Output(void* source_frame_refcon, OSStatus status, CVImageBufferRef image_buffer);

    static VideoDecodeAccelerator::SupportedProfiles GetSupportedProfiles(const gpu::GpuDriverBugWorkarounds& workarounds);
...
private:
    ...
    struct FrameOrder {
        bool operator()(const std::unique_ptr<Frame>& lhs, const std::unique_ptr<Frame>& rhs) const;
    };
    ...
    //
    void NotifyError(Error vda_error_type, VTVDASessionFailureType session_failure_type);
    ...
    std::priority_queue<std::unique_ptr<Frame>, std::vector<std::unique_ptr<Frame>>, FrameOrder> reorder_queue_;
