...
base::apple::ScopedCFTypeRef<CMFormatDescriptionRef> CreateFormat()
{
    ...
    OSStatus status = CMFormatDescriptionCreate(kCFAllocatorDefault, kCMMediaType_Video, 'test', nullptr, format.InitializeInto());
    CHECK_EQ(status, noErr);
    return format;
}

base::apple::ScopedCFTypeRef<CMSampleBufferRef> CreateSample(CMFormatDescriptionRef format)
{
    base::apple::ScopedCFTypeRef<CMSampleBufferRef> sample;
    OSStatus status = CMSampleBufferCreate(kCFAllocatorDefault, nullptr, true, nullptr, nullptr, format, 0, 0, nullptr, 0, nullptr, sample.InitializeInto());
    CHECK_EQ(status, noErr);
    return sample;
}
...
base::apple::ScopedCFTypeRef<CVImageBufferRef> CreateImage()
{
    ...
    OSStatus status = CVPixelBufferCreate(
        kCFAllocatorDefault, /*width=*/16, /*height=*/16, kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange, nullptr, image.InitializeInto());
    CHECK_EQ(status, noErr);
    return image;
}
...
public:
    explicit FakeDecompressionSession(VideoToolboxDecompressionSessionImpl::OutputCB output_cb)
    ...
    bool Create(CMFormatDescriptionRef format, CFDictionaryRef decoder_config, CFDictionaryRef image_config) override
    {
        CHECK(!IsValid());
        ++creations;
        if (can_create) {
...
public:
    ...
    {
        video_toolbox_.SetDecompressionSessionForTesting(base::WrapUnique(decompression_session_.get()));
    }
...
protected:
    ...
    MOCK_METHOD2(OnOutput, void(base::apple::ScopedCFTypeRef<CVImageBufferRef>, std::unique_ptr<VideoToolboxDecodeMetadata>));

    base::test::TaskEnvironment task_environment_;
    VideoToolboxDecompressionSessionManager video_toolbox_ { task_environment_.GetMainThreadTaskRunner(), std::make_unique<NullMediaLog>(),
        base::BindRepeating(&VideoToolboxDecompressionSessionManagerTest::OnOutput, base::Unretained(this)),
        base::BindOnce(&VideoToolboxDecompressionSessionManagerTest::OnError, base::Unretained(this)) };
    raw_ptr<FakeDecompressionSession> decompression_session_ { new FakeDecompressionSession(
        base::BindRepeating(&VideoToolboxDecompressionSessionManager::OnOutput, base::Unretained(&video_toolbox_))) };
};
