...
constexpr VideoCodecProfile kSupportedProfiles[] = {
    H264PROFILE_BASELINE, H264PROFILE_EXTENDED, H264PROFILE_MAIN, H264PROFILE_HIGH,
...
base::ScopedCFTypeRef<CFMutableDictionaryRef> BuildImageConfig(CMVideoDimensions coded_dimensions, bool is_hbd, bool has_alpha)
{
    ...
    int32_t pixel_format = is_hbd ? kCVPixelFormatType_420YpCbCr10BiPlanarVideoRange : kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange;
...
#undef CFINT
    ...
    image_config.reset(CFDictionaryCreateMutable(kCFAllocatorDefault,
        3, // capacity
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
    if (!image_config.get())
        return image_config;

    CFDictionarySetValue(image_config, kCVPixelBufferPixelFormatTypeKey, cf_pixel_format);
...
base::ScopedCFTypeRef<CMFormatDescriptionRef> CreateVideoFormatHEVC(ParameterSets param_sets)
{
    ...
    if (__builtin_available(macOS 11.0, *)) {
        ...
        OSSTATUS_DLOG_IF(WARNING, status != noErr, status) << "CMVideoFormatDescriptionCreateFromHEVCParameterSets()";
    }
    return format;
}
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
...
base::ScopedCFTypeRef<CMFormatDescriptionRef> CreateVideoFormatH264(
    const std::vector<uint8_t>& sps, const std::vector<uint8_t>& spsext, const std::vector<uint8_t>& pps)
{
    DCHECK(!sps.empty());
    DCHECK(!pps.empty());
    ...
    OSSTATUS_DLOG_IF(WARNING, status != noErr, status) << "CMVideoFormatDescriptionCreateFromH264ParameterSets()";
    return format;
}

base::ScopedCFTypeRef<CMFormatDescriptionRef> CreateVideoFormatVP9(
    media::VideoColorSpace color_space, media::VideoCodecProfile profile, absl::optional<gfx::HDRMetadata> hdr_metadata, const gfx::Size& coded_size)
{
    base::ScopedCFTypeRef<CFMutableDictionaryRef> format_config(CreateFormatExtensions(kCMVideoCodecType_VP9, profile, color_space, hdr_metadata));
    ...
    OSStatus status = CMVideoFormatDescriptionCreate(
        kCFAllocatorDefault, kCMVideoCodecType_VP9, coded_size.width(), coded_size.height(), format_config, format.InitializeInto());
    OSSTATUS_DLOG_IF(WARNING, status != noErr, status) << "CMVideoFormatDescriptionCreate()";
    return format;
}
...
bool CreateVideoToolboxSession(const CMFormatDescriptionRef format, bool require_hardware, bool is_hbd, bool has_alpha,
    const VTDecompressionOutputCallbackRecord* callback, base::ScopedCFTypeRef<VTDecompressionSessionRef>* session, gfx::Size* configured_size)
{
    // Prepare VideoToolbox configuration dictionaries.
    base::ScopedCFTypeRef<CFMutableDictionaryRef> decoder_config(CFDictionaryCreateMutable(kCFAllocatorDefault,
        1, // capacity
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
    if (!decoder_config) {
    ...
    CFDictionarySetValue(decoder_config, kVTVideoDecoderSpecification_EnableHardwareAcceleratedVideoDecoder, kCFBooleanTrue);
    CFDictionarySetValue(
        decoder_config, kVTVideoDecoderSpecification_RequireHardwareAcceleratedVideoDecoder, require_hardware ? kCFBooleanTrue : kCFBooleanFalse);
    ...
    CMVideoDimensions visible_dimensions = { base::ClampFloor(visible_rect.size.width), base::ClampFloor(visible_rect.size.height) };
    base::ScopedCFTypeRef<CFMutableDictionaryRef> image_config(BuildImageConfig(visible_dimensions, is_hbd, has_alpha));
    if (!image_config) {
    ...
    *configured_size = gfx::Size(visible_rect.size.width, visible_rect.size.height);
...
bool InitializeVideoToolboxInternal()
{
    ...
    const std::vector<uint8_t> sps_h264_normal
        = { 0x67, 0x64, 0x00, 0x1e, 0xac, 0xd9, 0x80, 0xd4, 0x3d, 0xa1, 0x00, 0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x03, 0x00, 0x30, 0x8f, 0x16, 0x2d, 0x9a };
    const std::vector<uint8_t> pps_h264_normal = { 0x68, 0xe9, 0x7b, 0xcb };
    if (!CreateVideoToolboxSession(CreateVideoFormatH264(sps_h264_normal, std::vector<uint8_t>(), pps_h264_normal),
            /*require_hardware=*/true, /*is_hbd=*/false, /*has_alpha=*/false, &callback, &session, &configured_size)) {
    ...
    const std::vector<uint8_t> sps_h264_small
        = { 0x67, 0x64, 0x00, 0x0a, 0xac, 0xd9, 0x89, 0x7e, 0x22, 0x10, 0x00, 0x00, 0x3e, 0x90, 0x00, 0x0e, 0xa6, 0x08, 0xf1, 0x22, 0x59, 0xa0 };
    const std::vector<uint8_t> pps_h264_small = { 0x68, 0xe9, 0x79, 0x72, 0xc0 };
    if (!CreateVideoToolboxSession(CreateVideoFormatH264(sps_h264_small, std::vector<uint8_t>(), pps_h264_small),
            /*require_hardware=*/false, /*is_hbd=*/false, /*has_alpha=*/false, &callback, &session, &configured_size)) {
    ...
    if (__builtin_available(macOS 11.0, *)) {
        ...
        if (!CreateVideoToolboxSession(CreateVideoFormatVP9(VideoColorSpace::REC709(), VP9PROFILE_PROFILE0, absl::nullopt, gfx::Size(720, 480)),
                /*require_hardware=*/true, /*is_hbd=*/false, /*has_alpha=*/false, &callback, &session, &configured_size)) {
    ...
    if (base::FeatureList::IsEnabled(media::kPlatformHEVCDecoderSupport)) {
        ...
        if (__builtin_available(macOS 11.0, *)) {
            ...
            const std::vector<uint8_t> vps_hevc_normal = { 0x40, 0x01, 0x0c, 0x01, 0xff, 0xff, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00,
                0x00, 0x03, 0x00, 0x5d, 0x95, 0x98, 0x09 };

            const std::vector<uint8_t> sps_hevc_normal
                = { 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x5d, 0xa0, 0x02, 0x80, 0x80, 0x2d,
                      0x16, 0x59, 0x59, 0xa4, 0x93, 0x2b, 0xc0, 0x5a, 0x70, 0x80, 0x00, 0x01, 0xf4, 0x80, 0x00, 0x3a, 0x98, 0x04 };

            const std::vector<uint8_t> pps_hevc_normal = { 0x44, 0x01, 0xc1, 0x72, 0xb4, 0x62, 0x40 };

            if (!CreateVideoToolboxSession(CreateVideoFormatHEVC(ParameterSets({ vps_hevc_normal, sps_hevc_normal, pps_hevc_normal })),
                    /*require_hardware=*/true, /*is_hbd=*/false, /*has_alpha=*/false, &callback, &session, &configured_size)) {
            ...
            const std::vector<uint8_t> vps_hevc_small = { 0x40, 0x01, 0x0c, 0x01, 0xff, 0xff, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00,
                0x00, 0x03, 0x00, 0x3c, 0x95, 0x98, 0x09 };

            const std::vector<uint8_t> sps_hevc_small = { 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
                0x3c, 0xa0, 0x0a, 0x08, 0x0f, 0x16, 0x59, 0x59, 0xa4, 0x93, 0x2b, 0xc0, 0x40, 0x40, 0x00, 0x00, 0xfa, 0x40, 0x00, 0x1d, 0x4c, 0x02 };

            const std::vector<uint8_t> pps_hevc_small = { 0x44, 0x01, 0xc1, 0x72, 0xb4, 0x62, 0x40 };

            if (!CreateVideoToolboxSession(CreateVideoFormatHEVC(ParameterSets({ vps_hevc_small, sps_hevc_small, pps_hevc_small })),
                    /*require_hardware=*/false, /*is_hbd=*/false, /*has_alpha=*/false, &callback, &session, &configured_size)) {
...
int32_t ComputeH264ReorderWindow(const H264SPS* sps)
{
    ...
    int max_dpb_frames = max_dpb_mbs / ((sps->pic_width_in_mbs_minus1 + 1) * (sps->pic_height_in_map_units_minus1 + 1));
...
void OutputThunk(void* decompression_output_refcon, void* source_frame_refcon, OSStatus status, VTDecodeInfoFlags info_flags, CVImageBufferRef image_buffer,
...
public:
    ...
    void DetectConfig(const uint8_t* stream, unsigned int size)
    {
        ...
        while (vp9_parser_.ParseNextFrame(&fhdr, &allocate_size, &null_config) == Vp9Parser::kOk) {
...
VTVideoDecodeAccelerator::PictureInfo::PictureInfo(uint32_t client_texture_id, uint32_t service_texture_id)
...
bool VTVideoDecodeAccelerator::FrameOrder::operator()(const std::unique_ptr<Frame>& lhs, const std::unique_ptr<Frame>& rhs) const
{
...
VTVideoDecodeAccelerator::VTVideoDecodeAccelerator(
    const GpuVideoDecodeGLClient& gl_client, const gpu::GpuDriverBugWorkarounds& workarounds, MediaLog* media_log)
    ...
    , decoder_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE }))
    ...
    , weak_this_factory_(this)
{
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "VTVideoDecodeAccelerator", gpu_task_runner_);
}
...
bool VTVideoDecodeAccelerator::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    ...
    for (const auto& [texture_id, picture_info] : picture_info_map_) {
        for (const auto& gl_image : picture_info->gl_images) {
            std::string dump_name = base::StringPrintf("media/vt_video_decode_accelerator_%d/picture_%d", memory_dump_id_, picture_info->bitstream_id);
    ...
    // NotifyEndOfBitstreamBuffer() has not been called yet).
    {
        ...
        base::trace_event::MemoryAllocatorDump* dump
            = pmd->CreateAllocatorDump(base::StringPrintf("media/vt_video_decode_accelerator_%d/output_queue", memory_dump_id_));
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, total_count);
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_size);
    }
    ...
    // NotifyEndOfBitstreamBuffer() has been called already).
    {
        ...
        base::trace_event::MemoryAllocatorDump* dump
            = pmd->CreateAllocatorDump(base::StringPrintf("media/vt_video_decode_accelerator_%d/reorder_queue", memory_dump_id_));
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, total_count);
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, total_size);
    }
...
bool VTVideoDecodeAccelerator::Initialize(const Config& config, Client* client)
{
    DVLOG(1) << __func__;
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    ...
    static const base::NoDestructor<VideoDecodeAccelerator::SupportedProfiles> kActualSupportedProfiles(GetSupportedProfiles(workarounds_));
    if (!base::Contains(*kActualSupportedProfiles, config.profile, &VideoDecodeAccelerator::SupportedProfile::profile)) {
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.VTVDA.SessionFailureReason", SFT_SUCCESSFULLY_INITIALIZED, SFT_MAX + 1);
    return true;
}
...
bool VTVideoDecodeAccelerator::FinishDelayedFrames()
{
    ...
    if (session_) {
        ...
        if (status) {
            NOTIFY_STATUS("VTDecompressionSessionWaitForAsynchronousFrames()", status, SFT_PLATFORM_ERROR);
            return false;
        }
    }
    return true;
...
bool VTVideoDecodeAccelerator::ConfigureDecoder()
{
    ...
    case VideoCodec::kVP9:
        format = CreateVideoFormatVP9(cc_detector_->GetColorSpace(config_.container_color_space), config_.profile, config_.hdr_metadata,
    ...
    const bool require_hardware = config_.profile == VP9PROFILE_PROFILE0 || config_.profile == VP9PROFILE_PROFILE2;
    const bool is_hbd = config_.profile == VP9PROFILE_PROFILE2 || config_.profile == HEVCPROFILE_MAIN10 || config_.profile == HEVCPROFILE_REXT;
    if (!CreateVideoToolboxSession(format_, require_hardware, is_hbd, has_alpha_, &callback_, &session_, &configured_size_)) {
        NotifyError(PLATFORM_FAILURE, SFT_PLATFORM_ERROR);
        return false;
    }
    ...
    if (VTSessionCopyProperty(session_,
            ...
            CFSTR("UsingHardwareAcceleratedVideoDecoder"), kCFAllocatorDefault, cf_using_hardware.InitializeInto())
        == 0) {
...
void VTVideoDecodeAccelerator::DecodeTaskVp9(scoped_refptr<DecoderBuffer> buffer, Frame* frame)
{
    DVLOG(2) << __func__ << ": bit_stream=" << frame->bitstream_id << ", buffer=" << buffer->AsHumanReadableString();
    DCHECK(decoder_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (!data) {
        gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::DecodeDone, weak_this_, frame));
        return;
    }
    ...
    status = VTDecompressionSessionDecodeFrame(session_,
        sample, // sample_buffer
        decode_flags, // decode_flags
        reinterpret_cast<void*>(frame), // source_frame_refcon
        nullptr); // &info_flags_out
    if (status) {
        NOTIFY_STATUS("VTDecompressionSessionDecodeFrame()", status, SFT_DECODE_ERROR);
        return;
    }
}

void VTVideoDecodeAccelerator::DecodeTaskH264(scoped_refptr<DecoderBuffer> buffer, Frame* frame)
{
    ...
    while (true) {
        ...
        if (result != H264Parser::kOk) {
            WriteToMediaLog(MediaLogMessageLevel::kERROR, "Failed to parse H.264 stream");
            NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
            return;
        }
        ...
        case H264NALU::kSPSExt: {
            ...
            if (result != H264Parser::kOk) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Could not parse SPS extension");
                NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                return;
            }
        ...
        case H264NALU::kSEIMessage: {
            ...
            if (result == H264Parser::kOk && sei_msg.type == H264SEIMessage::kSEIRecoveryPoint && sei_msg.recovery_point.recovery_frame_cnt == 0) {
        ...
        case H264NALU::kIDRSlice:
            ...
            if (!frame->has_slice) {
                ...
                if (result == H264Parser::kUnsupportedStream) {
                    WriteToMediaLog(MediaLogMessageLevel::kERROR, "Unsupported slice header");
                    NotifyError(PLATFORM_FAILURE, SFT_UNSUPPORTED_STREAM);
                    return;
                }
                if (result != H264Parser::kOk) {
                    WriteToMediaLog(MediaLogMessageLevel::kERROR, "Could not parse slice header");
                    NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                    return;
                }

                // Lookup SPS and PPS.
                const H264PPS* pps = h264_parser_.GetPPS(slice_hdr.pic_parameter_set_id);
                if (!pps) {
                    WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing PPS referenced by slice");
                    NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                    return;
                }
                ...
                if (!sps) {
                    WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing SPS referenced by PPS");
                    NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                    return;
                }
                ...
                absl::optional<int32_t> pic_order_cnt = h264_poc_.ComputePicOrderCnt(sps, slice_hdr);
                if (!pic_order_cnt.has_value()) {
                    WriteToMediaLog(MediaLogMessageLevel::kERROR, "Unable to compute POC");
                    NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                    return;
                }
    ...
    // best.
    if (configured_sps_ != active_sps_ || configured_spsext_ != active_spsext_ || configured_pps_ != active_pps_) {
        if (frame->is_idr) {
            if (active_sps_.empty()) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Invalid configuration (no SPS)");
                NotifyError(INVALID_ARGUMENT, SFT_INVALID_STREAM);
                return;
            }
            if (active_pps_.empty()) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Invalid configuration (no PPS)");
                NotifyError(INVALID_ARGUMENT, SFT_INVALID_STREAM);
                return;
            }
    ...
    if (!session_) {
        WriteToMediaLog(MediaLogMessageLevel::kERROR, "Cannot decode without configuration");
        NotifyError(INVALID_ARGUMENT, SFT_INVALID_STREAM);
        return;
    }
    ...
    OSStatus status = CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault,
        ...
        data.InitializeInto());
    if (status) {
        NOTIFY_STATUS("CMBlockBufferCreateWithMemoryBlock()", status, SFT_PLATFORM_ERROR);
        return;
    }
    ...
    if (status) {
        NOTIFY_STATUS("CMBlockBufferAssureBlockMemory()", status, SFT_PLATFORM_ERROR);
        return;
    }
    ...
    for (size_t i = 0; i < nalus.size(); i++) {
        ...
        if (status) {
            NOTIFY_STATUS("CMBlockBufferReplaceDataBytes()", status, SFT_PLATFORM_ERROR);
            return;
        }
        offset += kNALUHeaderLength;
        status = CMBlockBufferReplaceDataBytes(nalu_ref.data, data, offset, nalu_ref.size);
        if (status) {
            NOTIFY_STATUS("CMBlockBufferReplaceDataBytes()", status, SFT_PLATFORM_ERROR);
            return;
        }
...
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
void VTVideoDecodeAccelerator::DecodeTaskHEVC(scoped_refptr<DecoderBuffer> buffer, Frame* frame)
{
    DCHECK(decoder_task_runner_->RunsTasksInCurrentSequence());
    ...
    while (true) {
        ...
        if (result != H265Parser::kOk) {
            WriteToMediaLog(MediaLogMessageLevel::kERROR, "Failed to parse H.265 stream");
            NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
            return;
        }
        ...
        case H265NALU::PPS_NUT: {
            ...
            if (result == H265Parser::kMissingParameterSet) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing SPS from what was parsed");
        ...
        case H265NALU::PREFIX_SEI_NUT: {
            ...
            if (result == H265Parser::kOk && sei_msg.type == H265SEIMessage::kSEIAlphaChannelInfo
        ...
        case H265NALU::CRA_NUT: {
            ...
            if (output_count_for_cra_rasl_workaround_ < kMinOutputsBeforeRASL
                && (nalu.nal_unit_type == H265NALU::RASL_N || nalu.nal_unit_type == H265NALU::RASL_R)) {
                continue;
            }
            ...
            result = hevc_parser_.ParseSliceHeader(nalu, curr_slice_hdr.get(), last_slice_hdr.get());

            if (result == H265Parser::kMissingParameterSet) {
                curr_slice_hdr.reset();
                last_slice_hdr.reset();
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing PPS when parsing slice header");
                continue;
            }
            ...
            const H265PPS* pps = hevc_parser_.GetPPS(curr_slice_hdr->slice_pic_parameter_set_id);
            if (!pps) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing PPS referenced by slice");
                NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                return;
            ...
            const H265VPS* vps = hevc_parser_.GetVPS(sps->sps_video_parameter_set_id);
            if (!vps) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Missing VPS referenced by SPS");
                NotifyError(UNREADABLE_INPUT, SFT_INVALID_STREAM);
                return;
            }
            ...
            // Compute and store frame properties. |image_size| gets filled in
            // later, since it comes from the decoder configuration.
            int32_t pic_order_cnt = hevc_poc_.ComputePicOrderCnt(sps, pps, *curr_slice_hdr.get());

            frame->has_slice = true;
            frame->is_idr = nalu.nal_unit_type >= H265NALU::BLA_W_LP && nalu.nal_unit_type <= H265NALU::RSV_IRAP_VCL23;
            frame->pic_order_cnt = pic_order_cnt;
    ...
    if (seen_vps_ != configured_vpss_ || seen_sps_ != configured_spss_ || seen_pps_ != configured_ppss_) {
        if (frame->is_idr) {
            if (seen_vps_.empty()) {
                WriteToMediaLog(MediaLogMessageLevel::kERROR, "Invalid configuration (no VPS)");
                NotifyError(INVALID_ARGUMENT, SFT_INVALID_STREAM);
                return;
            }
            if (seen_sps_.empty()) {
            ...
            // parameter sets.
            configured_vpss_[active_vps_id].assign(active_vps_.data(), active_vps_.data() + active_vps_.size());
            configured_spss_[active_sps_id].assign(active_sps_.data(), active_sps_.data() + active_sps_.size());
            configured_ppss_[active_pps_id].assign(active_pps_.data(), active_pps_.data() + active_pps_.size());
        }
    }
...
void VTVideoDecodeAccelerator::Output(void* source_frame_refcon, OSStatus status, CVImageBufferRef image_buffer)
{
    if (status) {
...
void VTVideoDecodeAccelerator::FlushTask(TaskType type)
{
    ...
    gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::FlushDone, weak_this_, type));
}
...
void VTVideoDecodeAccelerator::Decode(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    ...
    if (codec_ == VideoCodec::kVP9) {
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::DecodeTaskVp9, decoder_weak_this_, std::move(buffer), frame));
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    } else if (codec_ == VideoCodec::kHEVC) {
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::DecodeTaskHEVC, decoder_weak_this_, std::move(buffer), frame));
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    } else {
        decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::DecodeTaskH264, decoder_weak_this_, std::move(buffer), frame));
    }
}

void VTVideoDecodeAccelerator::AssignPictureBuffers(const std::vector<PictureBuffer>& pictures)
{
    DVLOG(1) << __func__;
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    ...
    for (const PictureBuffer& picture : pictures) {
        ...
        if (picture.client_texture_ids().empty() && picture.service_texture_ids().empty()) {
            picture_info_map_.insert(std::make_pair(picture.id(), std::make_unique<PictureInfo>()));
        } else {
            DCHECK_LE(1u, picture.client_texture_ids().size());
            DCHECK_LE(1u, picture.service_texture_ids().size());
            picture_info_map_.insert(
                std::make_pair(picture.id(), std::make_unique<PictureInfo>(picture.client_texture_ids()[0], picture.service_texture_ids()[0])));
        }
    }
    ...
    gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::ProcessWorkQueues, weak_this_));
}
...
void VTVideoDecodeAccelerator::ReusePictureBuffer(int32_t picture_id)
{
    ...
    if (picture_info->uses_shared_images) {
        ...
        gl_client_.bind_image.Run(picture_info->client_texture_id, gpu::GetPlatformSpecificTextureTarget(), nullptr, false);
    }
...
bool VTVideoDecodeAccelerator::ProcessTaskQueue()
{
    ...
    case TASK_FRAME: {
        ...
        bool reorder_queue_has_space = reorder_queue_.size() < kMaxReorderQueueSize;
        bool reorder_queue_flush_needed = task.frame->is_idr || task.frame->has_mmco5;
        bool reorder_queue_flush_done = reorder_queue_.empty();
        if (reorder_queue_has_space && (!reorder_queue_flush_needed || reorder_queue_flush_done)) {
    ...
    case TASK_FLUSH:
        ...
        if ((codec_ != VideoCodec::kVP9 && reorder_queue_.size() == 0) || (codec_ == VideoCodec::kVP9 && output_queue_.empty())) {
...
bool VTVideoDecodeAccelerator::ProcessReorderQueue()
{
    ...
    // the next frame.
    bool flushing
        = !task_queue_.empty() && (task_queue_.front().type != TASK_FRAME || task_queue_.front().frame->is_idr || task_queue_.front().frame->has_mmco5);

    size_t reorder_window = std::max(0, reorder_queue_.top()->reorder_window);
    DVLOG(3) << __func__ << " size=" << reorder_queue_.size() << " window=" << reorder_window << " flushing=" << flushing;
...
bool VTVideoDecodeAccelerator::ProcessFrame(const Frame& frame)
{
    ...
    if (picture_size_ != frame.image_size) {
        ...
        } else if (config_.profile == VP9PROFILE_PROFILE2 || config_.profile == HEVCPROFILE_MAIN10 || config_.profile == HEVCPROFILE_REXT) {
        ...
        DVLOG(3) << "ProvidePictureBuffers(" << kNumPictureBuffers << frame.image_size.ToString() << ")";
        client_->ProvidePictureBuffers(kNumPictureBuffers, picture_format_, 1, frame.image_size, gpu::GetPlatformSpecificTextureTarget());
        return false;
    }
...
bool VTVideoDecodeAccelerator::SendFrame(const Frame& frame)
{
    ...
    for (size_t plane = 0; plane < planes.size(); ++plane) {
        const gfx::Size plane_size(CVPixelBufferGetWidthOfPlane(frame.image.get(), plane), CVPixelBufferGetHeightOfPlane(frame.image.get(), plane));
        gfx::BufferFormat plane_buffer_format = gpu::GetPlaneBufferFormat(planes[plane], buffer_format_);
        const viz::ResourceFormat viz_resource_format = viz::GetResourceFormat(plane_buffer_format);
        const GLenum gl_format = viz::GLDataFormat(viz_resource_format);

        scoped_refptr<gl::GLImageIOSurface> gl_image(gl::GLImageIOSurface::Create(plane_size));
        if (!gl_image->InitializeWithCVPixelBuffer(
                frame.image.get(), plane, gfx::GenericSharedMemoryId(g_cv_pixel_buffer_ids.GetNext()), plane_buffer_format, color_space)) {
            NOTIFY_STATUS("Failed to initialize GLImageIOSurface", PLATFORM_FAILURE, SFT_PLATFORM_ERROR);
        }

        if (picture_info->uses_shared_images) {
            gpu::SharedImageStub* shared_image_stub = client_->GetSharedImageStub();
            DCHECK(shared_image_stub);
            const uint32_t shared_image_usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
            ...
            gl_params.target = gl_client_.supports_arb_texture_rectangle ? GL_TEXTURE_RECTANGLE_ARB : GL_TEXTURE_2D;
            ...
            auto shared_image = std::make_unique<gpu::GLImageBacking>(gl_image, mailbox, viz::SharedImageFormat::SinglePlane(viz_resource_format), plane_size,
                color_space, kTopLeft_GrSurfaceOrigin, kOpaque_SkAlphaType, shared_image_usage, gl_params, gl_client_.is_passthrough);
            ...
            auto destroy_shared_image_lambda
                = [](gpu::SharedImageStub::SharedImageDestructionCallback callback, scoped_refptr<base::SingleThreadTaskRunner> task_runner) {
                      task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), gpu::SyncToken()));
                  };
            auto destroy_shared_image_callback
                = base::BindOnce(destroy_shared_image_lambda, shared_image_stub->GetSharedImageDestructionCallback(mailbox), gpu_task_runner_);
            picture_info->scoped_shared_images.push_back(
                scoped_refptr<Picture::ScopedSharedImage>(new Picture::ScopedSharedImage(mailbox, gl_params.target, std::move(destroy_shared_image_callback))));
        } else {
            if (!gl_client_.bind_image.Run(picture_info->client_texture_id, gpu::GetPlatformSpecificTextureTarget(), gl_image, false)) {
    ...
    Picture picture(picture_id, frame.bitstream_id, gfx::Rect(frame.image_size), color_space, true);
    ...
    for (size_t plane = 0; plane < planes.size(); ++plane) {
        picture.set_scoped_shared_image(picture_info->scoped_shared_images[plane], plane);
    }
...
void VTVideoDecodeAccelerator::NotifyError(Error vda_error_type, VTVDASessionFailureType session_failure_type)
{
    DCHECK_LT(session_failure_type, SFT_MAX + 1);
    if (!gpu_task_runner_->BelongsToCurrentThread()) {
        gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::NotifyError, weak_this_, vda_error_type, session_failure_type));
    } else if (state_ == STATE_DECODING) {
        state_ = STATE_ERROR;
        UMA_HISTOGRAM_ENUMERATION("Media.VTVDA.SessionFailureReason", session_failure_type, SFT_MAX + 1);
        client_->NotifyError(vda_error_type);
    }
}

void VTVideoDecodeAccelerator::WriteToMediaLog(MediaLogMessageLevel level, const std::string& message)
{
    if (!gpu_task_runner_->BelongsToCurrentThread()) {
        gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::WriteToMediaLog, weak_this_, level, message));
        return;
    }
...
void VTVideoDecodeAccelerator::QueueFlush(TaskType type)
{
    ...
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VTVideoDecodeAccelerator::FlushTask, decoder_weak_this_, type));
...
VideoDecodeAccelerator::TextureAllocationMode VTVideoDecodeAccelerator::GetSharedImageTextureAllocationMode() const
{
    return VideoDecodeAccelerator::TextureAllocationMode::kDoNotAllocateGLTextures;
}

// static
VideoDecodeAccelerator::SupportedProfiles VTVideoDecodeAccelerator::GetSupportedProfiles(const gpu::GpuDriverBugWorkarounds& workarounds)
{
    ...
    for (const auto& supported_profile : kSupportedProfiles) {
        if (supported_profile == VP9PROFILE_PROFILE0 || supported_profile == VP9PROFILE_PROFILE2) {
        ...
        if (supported_profile == HEVCPROFILE_MAIN || supported_profile == HEVCPROFILE_MAIN10 || supported_profile == HEVCPROFILE_MAIN_STILL_PICTURE
            || supported_profile == HEVCPROFILE_REXT) {
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
            if (!workarounds.disable_accelerated_hevc_decode && base::FeatureList::IsEnabled(kPlatformHEVCDecoderSupport)) {
                if (__builtin_available(macOS 11.0, *)) {
