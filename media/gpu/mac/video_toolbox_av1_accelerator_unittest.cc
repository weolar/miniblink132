...
protected:
    ...
    std::unique_ptr<VideoToolboxAV1Accelerator> accelerator_ { std::make_unique<VideoToolboxAV1Accelerator>(std::make_unique<NullMediaLog>(), std::nullopt,
        base::BindRepeating(&VideoToolboxAV1AcceleratorTest::OnDecode, base::Unretained(this)),
        base::BindRepeating(&VideoToolboxAV1AcceleratorTest::OnOutput, base::Unretained(this))) };
};
...
TEST_F(VideoToolboxAV1AcceleratorTest, DecodeRaw)
{
    ...
    constexpr uint8_t frame_data[] = { 0x0a, 0x0b, 0x00, 0x00, 0x00, 0x04, 0x3c, 0xff, 0xbc, 0xfb, 0xf9, 0x80, 0x40 };
    ...
    accelerator_->SubmitDecode(*pic, sequence_header, ref_frames, tile_buffers, base::make_span(frame_data));
...
TEST_F(VideoToolboxAV1AcceleratorTest, DecodeSuperframe)
{
    // Sequence Header OBU from bear-av1.webm.
    // A valid sequence header is required to extract the av1c.
    constexpr uint8_t superframe_data[] = { 0x0a, 0x0b, 0x00, 0x00, 0x00, 0x04, 0x3c, 0xff, 0xbc, 0xfb, 0xf9, 0x80, 0x40 };
    ...
    accelerator_->SubmitDecode(*pic1, sequence_header, ref_frames, tile_buffers, base::make_span(superframe_data));
    accelerator_->SubmitDecode(*pic2, sequence_header, ref_frames, tile_buffers, base::make_span(superframe_data));
    ...
    CMBlockBufferCopyDataBytes(buf2, 0, CMBlockBufferGetDataLength(buf2), data2.data());
