...
constexpr struct {
    float fps;
    float qp;
} kFPS2QPTradeoffs[] = { { 0.0f, 51.0f }, { 5.0f, 42.0f }, { 10.0f, 41.0f }, { 15.0f, 40.0f }, { 30.0f, 37.0f }, { kFpsMax, 37.0f },
    ...
    for (size_t i = 0; i < num_elems - 1; ++i) {
        if (fps >= kFPS2QPTradeoffs[i].fps && fps < kFPS2QPTradeoffs[i + 1].fps) {
            return h264_rate_control_util::ClampedLinearInterpolation(
                fps, kFPS2QPTradeoffs[i].fps, kFPS2QPTradeoffs[i + 1].fps, kFPS2QPTradeoffs[i].qp, kFPS2QPTradeoffs[i + 1].qp);
        }
    }
    NOTREACHED();
}
...
float MaxQP2Fps(int max_qp)
{
    size_t num_elems = sizeof(kFPS2QPTradeoffs) / sizeof(kFPS2QPTradeoffs[0]);
    for (size_t i = 0; i < num_elems - 1; ++i) {
        if (max_qp <= kFPS2QPTradeoffs[i].qp && max_qp > kFPS2QPTradeoffs[i + 1].qp) {
...
H264RateControllerSettings::~H264RateControllerSettings() = default;
H264RateControllerSettings::H264RateControllerSettings(const H264RateControllerSettings&) = default;
H264RateControllerSettings& H264RateControllerSettings::operator=(const H264RateControllerSettings&) = default;

std::partial_ordering H264RateControllerSettings::operator<=>(const H264RateControllerSettings& other) const
{
    ...
    if (auto res = gop_max_duration.InMilliseconds() <=> other.gop_max_duration.InMilliseconds(); res != 0) {
        return res;
    }
    return std::lexicographical_compare_three_way(layer_settings.begin(), layer_settings.end(), other.layer_settings.begin(), other.layer_settings.end());
}

H264RateController::Layer::Layer(
    H264RateControllerLayerSettings settings, float expected_fps, base::TimeDelta short_term_window_size, base::TimeDelta long_term_window_size)
    ...
    , short_term_estimator_(short_term_window_size, kInitQPSize, GetInitialSizeCorrection(settings))
    , long_term_estimator_(long_term_window_size, kInitQPSize, GetInitialSizeCorrection(settings))
...
void H264RateController::Layer::AddFrameBytes(size_t frame_bytes, base::TimeDelta frame_timestamp)
{
    hrd_buffer_.AddFrameBytes(frame_bytes, frame_timestamp);
}

void H264RateController::Layer::AddFrameTimestamp(base::TimeDelta frame_timestamp)
{
    src_frame_rate_.AddSample(frame_timestamp);
}

void H264RateController::Layer::SetBufferParameters(size_t buffer_size, uint32_t avg_bitrate, uint32_t peak_bitrate, bool ease_hrd_reduction)
{
    hrd_buffer_.SetParameters(buffer_size, avg_bitrate, peak_bitrate, ease_hrd_reduction);
}

size_t H264RateController::Layer::GetBufferBytesAtTime(base::TimeDelta timestamp) const
{
    return static_cast<size_t>(hrd_buffer_.GetBytesAtTime(timestamp));
}

size_t H264RateController::Layer::GetBufferBytesRemainingAtTime(base::TimeDelta timestamp) const
{
...
size_t H264RateController::Layer::EstimateShortTermFrameSize(uint32_t qp, uint32_t qp_prev) const
{
    return short_term_estimator_.Estimate(qp, qp_prev);
}

size_t H264RateController::Layer::EstimateLongTermFrameSize(uint32_t qp, uint32_t qp_prev) const
{
    return long_term_estimator_.Estimate(qp, qp_prev);
}

uint32_t H264RateController::Layer::EstimateShortTermQP(size_t target_frame_bytes, uint32_t qp_prev) const
{
    return short_term_estimator_.InverseEstimate(target_frame_bytes, qp_prev);
}

uint32_t H264RateController::Layer::EstimateLongTermQP(size_t target_frame_bytes, uint32_t qp_prev) const
{
...
void H264RateController::Layer::UpdateFrameSizeEstimator(size_t frame_bytes, uint32_t qp, uint32_t qp_prev, base::TimeDelta elapsed_time)
{
    ...
    estimator_error_.AddValue(pred_p_frame_size_ - static_cast<float>(frame_bytes), elapsed_time);
}

float H264RateController::Layer::GetInitialSizeCorrection(H264RateControllerLayerSettings settings) const
{
    ...
    , content_type_(settings.content_type)
{
    ...
    DCHECK_LE(settings.num_temporal_layers, h264_rate_control_util::kMaxNumTemporalLayers);
    ...
    base::TimeDelta short_term_window_size = base::Milliseconds(std::max(static_cast<int>(5.0f * base::Time::kMillisecondsPerSecond / target_fps_), 300));
    // Set long-term window to 3 x HRD buffer size. Use uint64_t, as it might
    // overflow uint32_t.
    base::TimeDelta long_term_window_size = base::Milliseconds(3 * static_cast<uint64_t>(settings.layer_settings[kBaseLayerIndex].hrd_buffer_size * 8)
        * base::Time::kMillisecondsPerSecond / settings.layer_settings[kBaseLayerIndex].avg_bitrate);
    for (auto& tls : settings.layer_settings) {
        temporal_layers_.emplace_back(std::make_unique<Layer>(tls, target_fps_, short_term_window_size, long_term_window_size));
    }
}
...
void H264RateController::EstimateIntraFrameQP(base::TimeDelta frame_timestamp)
{
    ...
    const float bpp = target_bytes_frame * 8.0f / (frame_size_.width() * frame_size_.height());
    const float q_step = kIntraFrameMAD / (std::pow(bpp / std::pow(2, kRDYIntercept), 1 / kRDSlope));

    uint32_t curr_qp = std::clamp(h264_rate_control_util::QStepSize2QP(q_step), h264_rate_control_util::kQPMin, h264_rate_control_util::kQPMax);
    ...
    if (0 == frame_number_) {
        ...
        if (base_layer.is_buffer_full()) {
        ...
        } else if (base_layer.last_frame_type() == H264RateController::FrameType::kPFrame) {
            ...
            float max_qp_offset_for_idr = kFrameRateToMaxQPSlope * base_layer.GetFrameRateMean() + kFrameRateToMaxQPYIntercept;
            max_qp_offset_for_idr = std::clamp(max_qp_offset_for_idr, kMaxQPOffsetForIDRLowerLimit, kMaxQPOffsetForIDRUpperLimit);
            const float last_qp = std::max(base_layer.long_term_qp(), base_layer.last_frame_qp());
            curr_qp = static_cast<uint32_t>(std::clamp(static_cast<float>(curr_qp), last_qp + kMinQPOffsetForIDR, last_qp + max_qp_offset_for_idr));
        } else if (base_layer.last_frame_type() == H264RateController::FrameType::kIFrame) {
            curr_qp = std::clamp(curr_qp, base_layer.last_frame_qp() - 1, base_layer.last_frame_qp() + 3);
        }
    }

    // Limit highest possible quality.
    base_layer.update_curr_frame_qp(std::clamp(curr_qp, base_layer.min_qp(), base_layer.max_qp()));

    base_layer.update_long_term_qp(std::clamp(base_layer.long_term_qp(), base_layer.min_qp(), h264_rate_control_util::kQPMax));

    last_idr_timestamp_ = frame_timestamp;
}

void H264RateController::EstimateInterFrameQP(size_t temporal_id, base::TimeDelta frame_timestamp)
{
    ...
    curr_layer.update_long_term_qp(std::clamp(base_layer.long_term_qp(), curr_layer.min_qp(), h264_rate_control_util::kQPMax));
    ...
    if (fixed_delta_qp_ && temporal_id > kBaseLayerIndex && !curr_layer.is_buffer_full()) {
    ...
    const size_t buffer_layer_id = fixed_delta_qp_ ? num_temporal_layers_ - 1 : temporal_id;
    uint32_t max_rate_bytes_per_sec = temporal_layers_[buffer_layer_id]->average_bitrate() / 8;
    size_t buffer_size = temporal_layers_[buffer_layer_id]->buffer_size();
    int buffer_level_current = temporal_layers_[buffer_layer_id]->GetBufferBytesAtTime(frame_timestamp);

    size_t frame_size_target = GetTargetBytesForInterFrame(temporal_id, max_rate_bytes_per_sec, buffer_size, buffer_level_current, frame_timestamp);
    ...
    const bool hrd_buffer_is_full = buffer_level_current >= static_cast<int>(buffer_size);
    ...
    // Limit the quality.
    curr_layer.update_curr_frame_qp(std::clamp(curr_qp, curr_layer.min_qp(), curr_layer.max_qp()));

    curr_layer.update_pred_p_frame_size(curr_layer.EstimateShortTermFrameSize(curr_layer.curr_frame_qp(), curr_layer.last_frame_qp()));
}

void H264RateController::FinishIntraFrame(size_t access_unit_bytes, base::TimeDelta frame_timestamp)
{
    FinishLayerData(kBaseLayerIndex, FrameType::kIFrame, access_unit_bytes, frame_timestamp);
    ...
    if (0 == frame_number_) {
        ...
        float buffer_level_norm
            = static_cast<float>(temporal_layers_[kBaseLayerIndex]->last_frame_buffer_bytes()) / temporal_layers_[kBaseLayerIndex]->buffer_size();
        ...
        if (0.5f < buffer_level_norm) {
            ...
            if (temporal_layers_[kBaseLayerIndex]->long_term_qp() > max_qp_from_fps) {
                target_fps_ = MaxQP2Fps(static_cast<int>(temporal_layers_[kBaseLayerIndex]->long_term_qp()));
            }
        }
    }
...
void H264RateController::FinishInterFrame(size_t temporal_id, size_t access_unit_bytes, base::TimeDelta frame_timestamp)
{
    FinishLayerData(temporal_id, FrameType::kPFrame, access_unit_bytes, frame_timestamp);

    H264RateController::Layer& curr_layer = *temporal_layers_[temporal_id];

    const base::TimeDelta elapsed_time = h264_rate_control_util::ClampedTimestampDiff(frame_timestamp, curr_layer.previous_frame_timestamp());

    curr_layer.UpdateFrameSizeEstimator(access_unit_bytes, curr_layer.curr_frame_qp(), curr_layer.last_frame_qp(), elapsed_time);
...
void H264RateController::GetHRDBufferFullness(base::span<int> buffer_fullness, base::TimeDelta frame_timestamp) const
{
    for (size_t tl = kBaseLayerIndex; tl < std::min(buffer_fullness.size(), num_temporal_layers_); ++tl) {
        buffer_fullness[tl] = (100 * temporal_layers_[tl]->GetBufferBytesAtTime(frame_timestamp)) / static_cast<int>(temporal_layers_[tl]->buffer_size());
    }
}

void H264RateController::FinishLayerData(size_t temporal_id, FrameType frame_type, size_t frame_bytes, base::TimeDelta frame_timestamp)
{
    ...
    for (size_t tl = temporal_id; tl < num_temporal_layers_; ++tl) {
        ...
        temporal_layers_[tl]->update_last_frame_qp(temporal_layers_[tl]->curr_frame_qp());
        temporal_layers_[tl]->update_last_frame_type(frame_type);
    }
}

void H264RateController::FinishLayerPreviousFrameTimestamp(size_t temporal_id, base::TimeDelta frame_timestamp)
{
...
void H264RateController::SetLastTsOvershootingFrame(size_t temporal_id, base::TimeDelta frame_timestamp)
{
    for (size_t tl = temporal_id; tl < num_temporal_layers_; ++tl) {
...
size_t H264RateController::GetTargetBytesForIntraFrame(base::TimeDelta frame_timestamp) const
{
    ...
    const size_t starting_layer_id = fixed_delta_qp_ ? num_temporal_layers_ - 1 : kBaseLayerIndex;
    ...
    for (size_t tl = starting_layer_id; tl < num_temporal_layers_; ++tl) {
        int bytes_remaining_tl = temporal_layers_[tl]->GetBufferBytesRemainingAtTime(frame_timestamp);
    ...
    const size_t buffer_bytes = temporal_layers_[min_bytes_remaining_layer_id]->GetBufferBytesAtTime(frame_timestamp);
    const size_t hrd_buffer_size = temporal_layers_[min_bytes_remaining_layer_id]->buffer_size();
    ...
    float intra_frame_multiplier = (content_type_ == VideoEncodeAccelerator::Config::ContentType::kDisplay)
        ? 4.0f
        : std::clamp(temporal_layers_[starting_layer_id]->GetFrameRateMean() / 3.75f, 1.0f, 4.0f);
    size_t bytes_target = std::max(min_bytes_target,
        static_cast<size_t>(GetRateBudget(temporal_layers_[starting_layer_id]->GetFrameRateMean(), temporal_layers_[starting_layer_id]->average_bitrate())
...
size_t H264RateController::GetTargetBytesForInterFrame(
    size_t temporal_id, uint32_t max_rate_bytes_per_sec, size_t buffer_size, int buffer_level_current, base::TimeDelta frame_timestamp) const
{
    // The long-term frame size is calculated based on short-term stats and
    // long-term QP parameters.
    size_t frame_size_long_term = temporal_layers_[temporal_id]->EstimateShortTermFrameSize(
        temporal_layers_[temporal_id]->long_term_qp(), temporal_layers_[temporal_id]->last_frame_qp());
    ...
    if (fixed_delta_qp_ && num_temporal_layers_ > 1) {
        DCHECK_EQ(num_temporal_layers_, h264_rate_control_util::kMaxNumTemporalLayers);
        curr_layer_bitrate = static_cast<uint32_t>(temporal_layers_[kBaseLayerIndex + 1]->average_bitrate() * kLayerRateRatio);
    } else {
        uint32_t lower_layer_bitrate = temporal_id == kBaseLayerIndex ? 0u : temporal_layers_[temporal_id - 1]->average_bitrate();
        curr_layer_bitrate = temporal_layers_[temporal_id]->average_bitrate() - lower_layer_bitrate;
    }

    float frame_rate = std::clamp(temporal_layers_[temporal_id]->GetFrameRateMean(), 1.0f, target_fps_);

    size_t frame_size_budget = static_cast<size_t>(curr_layer_bitrate / 8 / frame_rate);

    float frame_size_deviation = static_cast<float>(fabs(static_cast<int>(frame_size_long_term) - static_cast<int>(frame_size_budget))) / frame_size_budget;
    float frame_size_compress = h264_rate_control_util::ClampedLinearInterpolation(frame_size_deviation, 0.5f, 3.0f, 0.1f, 0.9f);

    int frame_size_target = static_cast<int>(
        static_cast<int>(frame_size_budget) + (static_cast<int>(frame_size_long_term) - static_cast<int>(frame_size_budget)) * (1 - frame_size_compress));
    ...
    if (gop_max_duration_ > base::TimeDelta() && buffer_target_high > buffer_target_low) {
        frame_remaining_gop = last_idr_timestamp_ - frame_timestamp + gop_max_duration_;
    }

    // Size correction window is a linear transformation of the remaining time in
    // GOP.
    uint32_t size_correction_window = static_cast<uint32_t>(
        h264_rate_control_util::ClampedLinearInterpolation(static_cast<float>(frame_remaining_gop.InMilliseconds()), 0.0f, 2000.0f, 200.0f, 800.0f));

    base::TimeDelta buffer_duration = base::Milliseconds(static_cast<float>(buffer_size) / max_rate_bytes_per_sec * base::Time::kMillisecondsPerSecond);

    int size_correction = 0;
    if (buffer_level_current + frame_size_target > static_cast<int>(buffer_target_high)) {
        ...
        size_correction = static_cast<int>(-(static_cast<int>(buffer_level_current) + frame_size_target - static_cast<int>(buffer_target_high))
            / static_cast<float>(size_correction_window) / frame_rate * base::Time::kMillisecondsPerSecond);
    } else if (buffer_level_current + frame_size_target < static_cast<int>(buffer_target_low)) {
        // Windowed undershoot prevention.
        uint32_t win = buffer_duration.InMilliseconds();
        size_correction_window = std::min(size_correction_window, win);
        size_correction = static_cast<int>((static_cast<int>(buffer_target_low) - buffer_level_current - frame_size_target)
            / static_cast<float>(size_correction_window) / frame_rate * base::Time::kMillisecondsPerSecond);
    }

    frame_size_target = std::clamp(frame_size_target + size_correction, frame_size_target / 5, frame_size_target * 5);

    size_t frame_size_error = temporal_layers_[temporal_id]->GetFrameSizeEstimatorError();

    // Instantaneous undershoot prevention (buffer should not be empty after
    // the frame is removed).
    int buf_level_pre_fill_next_frame = buffer_level_current + frame_size_target - static_cast<int>(frame_size_budget);
    ...
    frame_size_target = std::max(frame_size_target, static_cast<int>(frame_size_budget / 5));

    return static_cast<size_t>(frame_size_target);
}

uint32_t H264RateController::GetInterFrameShortTermQP(size_t temporal_id, size_t frame_size_target)
{
    uint32_t curr_qp = temporal_layers_[temporal_id]->EstimateShortTermQP(frame_size_target, temporal_layers_[temporal_id]->last_frame_qp());
    curr_qp = std::clamp(curr_qp, h264_rate_control_util::kQPMin, h264_rate_control_util::kQPMax);
    if (fixed_delta_qp_) {
        temporal_layers_[temporal_id]->update_undershoot_delta_qp(static_cast<int>(temporal_layers_[temporal_id]->min_qp()) - static_cast<int>(curr_qp));
    }
...
{
    float target_rate_bytes_per_sec = static_cast<float>(temporal_layers_[kBaseLayerIndex]->average_bitrate() / 8);
    float frame_rate = temporal_layers_[kBaseLayerIndex]->GetFrameRateMean();
    size_t target_frame_bytes = static_cast<uint32_t>(target_rate_bytes_per_sec / frame_rate);
    uint32_t long_term_qp = temporal_layers_[temporal_id]->EstimateLongTermQP(target_frame_bytes, temporal_layers_[temporal_id]->last_frame_qp());
    ...
    for (int i = 0; i < kMaxQPIter; i++) {
        size_t layer_bytes = temporal_layers_[temporal_id]->EstimateLongTermFrameSize(long_term_qp, temporal_layers_[temporal_id]->last_frame_qp());
        float bitrate = 8 * layer_bytes * frame_rate;
        if (bitrate > temporal_layers_[temporal_id]->average_bitrate() * kBitrateThreshold) {
            long_term_qp += 1;
        } else {
            break;
        }
    ...
    return std::clamp(long_term_qp, h264_rate_control_util::kQPMin, h264_rate_control_util::kQPMax);
}

uint32_t H264RateController::ClipInterFrameQP(uint32_t curr_qp, size_t temporal_id, base::TimeDelta frame_timestamp)
{
    ...
    uint32_t min_qp = h264_rate_control_util::kQPMin, max_qp = h264_rate_control_util::kQPMax;
    ...
    if (temporal_id == kBaseLayerIndex) {
        if (temporal_layers_[kBaseLayerIndex]->last_frame_qp() > 0) {
            min_qp = temporal_layers_[kBaseLayerIndex]->last_frame_qp()
                - (temporal_layers_[kBaseLayerIndex]->GetFrameRateMean() < kMinQPFrameRateThreshold ? 2 : 1);
            max_qp = std::max(temporal_layers_[kBaseLayerIndex]->last_frame_qp() + 3,
                (temporal_layers_[kBaseLayerIndex]->min_qp() + temporal_layers_[kBaseLayerIndex]->max_qp()) / 2);
        }
    } else {
    ...
    if (!fixed_delta_qp_ && num_temporal_layers_ > 1) {
        ...
        if (limit_base_qp_ && temporal_id == kBaseLayerIndex) {
            uint32_t enhance_qp = temporal_layers_[kBaseLayerIndex + 1]->curr_frame_qp();
            ...
            int layer_delta = static_cast<int>(curr_qp) - static_cast<int>(temporal_layers_[kBaseLayerIndex]->curr_frame_qp());
            int qp_trend = static_cast<int>(curr_qp) - static_cast<int>(temporal_layers_[temporal_id]->last_frame_qp());
        ...
        if (temporal_layers_[kBaseLayerIndex + 1]->curr_frame_qp() > temporal_layers_[kBaseLayerIndex]->curr_frame_qp() + kFixedLayerDeltaQP) {
            ...
            min_qp = std::max(min_qp, temporal_layers_[kBaseLayerIndex + 1]->curr_frame_qp() - kFixedLayerDeltaQP);
        }
    }
    ...
    if (temporal_layers_[temporal_id]->is_buffer_full()) {
        ...
        uint32_t lower_bound = std::min(temporal_layers_[temporal_id]->curr_frame_qp() + 2, h264_rate_control_util::kQPMax);
    ...
    max_qp = std::clamp(max_qp, kMaxQPLowestValue, h264_rate_control_util::kQPMax);

    // QP range continues growing as long as frames overshoot. Out of order
    // timestamps are ignored.
    constexpr int kQPStepDuration = 33;
    if (last_ts_overshooting_frame_ != base::TimeDelta::Max() && frame_timestamp > last_ts_overshooting_frame_) {
        base::TimeDelta delta_ts_overshooting_frame = frame_timestamp - last_ts_overshooting_frame_;
        uint32_t delta_qp = std::max(delta_ts_overshooting_frame.InMilliseconds() / kQPStepDuration,
            delta_ts_overshooting_frame.InMilliseconds() * delta_ts_overshooting_frame.InMilliseconds() / (kQPStepDuration * kQPStepDuration));
        max_qp += delta_qp;
        min_qp += delta_qp;
    }
...
float H264RateController::GetTargetFps(H264RateControllerSettings settings) const
{
