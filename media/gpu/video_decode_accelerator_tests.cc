...
public:
    std::unique_ptr<DecoderListener> CreateDecoderListener(const Video* video, DecoderWrapperConfig config = DecoderWrapperConfig(),
        std::unique_ptr<FrameRendererDummy> frame_renderer = FrameRendererDummy::Create())
    {
        ...
        if (g_env->GetFrameOutputMode() != FrameOutputMode::kNone) {
            video_player->SetEventWaitTimeout(std::max(kDefaultEventWaitTimeout, g_env->Video()->GetDuration() * 10));
        }
        return video_player;
    }
...
private:
    ...
    bool CreateModelFrames(const Video* video)
    {
...
#endif
            /*media_log=*/nullptr, OffloadableVideoDecoder::OffloadState::kOffloaded);
        VideoDecoderConfig decoder_config(video->Codec(), video->Profile(), VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
            video->Resolution(), video->VisibleRect(), video->VisibleRect().size(), EmptyExtraData(), EncryptionScheme::kUnencrypted);
        ...
        if (!init_success)
            return false;
        auto encoded_data_helper = std::make_unique<EncodedDataHelper>(video->Data(), video->Codec());
...
int main(int argc, char** argv)
{
    ...
    auto validator_type = media::test::VideoPlayerTestEnvironment::ValidatorType::kMD5;
    ...
    for (base::CommandLine::SwitchMap::const_iterator it = switches.begin(); it != switches.end(); ++it) {
        ...
        if (it->first == "validator_type") {
            if (it->second == "none") {
                validator_type = media::test::VideoPlayerTestEnvironment::ValidatorType::kNone;
            } else if (it->second == "md5") {
                validator_type = media::test::VideoPlayerTestEnvironment::ValidatorType::kMD5;
            } else if (it->second == "ssim") {
                validator_type = media::test::VideoPlayerTestEnvironment::ValidatorType::kSSIM;
            } else {
                std::cout << "unknown validator type \"" << it->second << "\", possible values are \"none|md5|ssim\"\n";
                return EXIT_FAILURE;
            }
            ...
            if (it->second == "all") {
                ...
                std::cout << "unknown frame output mode \"" << it->second << "\", possible values are \"all|corrupt\"\n";
                return EXIT_FAILURE;
            }
    ...
    media::test::VideoPlayerTestEnvironment* test_environment = media::test::VideoPlayerTestEnvironment::Create(video_path, video_metadata_path, validator_type,
        implementation, linear_output, base::FilePath(output_folder), frame_output_config, enabled_features, disabled_features);
    if (!test_environment)
        return EXIT_FAILURE;
