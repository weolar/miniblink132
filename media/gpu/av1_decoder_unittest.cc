...
MATCHER_P2(MatchesFrameSizeAndRenderSize, frame_size, render_size, "")
{
    ...
    return base::strict_cast<int>(frame_header.width) == frame_size.width() && base::strict_cast<int>(frame_header.height) == frame_size.height()
        && base::strict_cast<int>(frame_header.render_width) == render_size.width()
        && base::strict_cast<int>(frame_header.render_height) == render_size.height();
}

MATCHER_P4(MatchesFrameHeader, frame_size, render_size, show_existing_frame, show_frame, "")
{
    const auto& frame_header = arg.frame_header;
    return base::strict_cast<int>(frame_header.width) == frame_size.width() && base::strict_cast<int>(frame_header.height) == frame_size.height()
        && base::strict_cast<int>(frame_header.render_width) == render_size.width()
        && base::strict_cast<int>(frame_header.render_height) == render_size.height() && frame_header.show_existing_frame == show_existing_frame
        && frame_header.show_frame == show_frame;
}

MATCHER_P4(MatchesYUV420SequenceHeader, profile, bitdepth, max_frame_size, film_grain_params_present, "")
{
    return arg.profile == profile && arg.color_config.bitdepth == bitdepth && base::strict_cast<int>(arg.max_frame_width) == max_frame_size.width()
        && base::strict_cast<int>(arg.max_frame_height) == max_frame_size.height() && arg.film_grain_params_present == film_grain_params_present
        && IsYUV420(arg.color_config.subsampling_x, arg.color_config.subsampling_y, arg.color_config.is_monochrome);
}
...
{
    return arg.data() == decoder_buffer->data() && arg.size() == decoder_buffer->size();
}
...
public:
    ...
    MOCK_METHOD5(SubmitDecode,
        Status(const AV1Picture&, const libgav1::ObuSequenceHeader&, const AV1ReferenceFrameVector&, const libgav1::Vector<libgav1::TileBuffer>&,
...
protected:
    ...
    {
        base::FilePath file_path(base::FilePath(base::FilePath::kCurrentDirectory).AppendASCII(fname));
...
void AV1DecoderTest::SetUp()
{
    ...
    decoder_ = std::make_unique<AV1Decoder>(std::move(accelerator), VIDEO_CODEC_PROFILE_UNKNOWN);
}

std::vector<AcceleratedVideoDecoder::DecodeResult> AV1DecoderTest::Decode(scoped_refptr<DecoderBuffer> buffer)
{
    if (buffer)
    ...
    } while (res != DecodeResult::kDecodeError && res != DecodeResult::kRanOutOfStreamData && res != DecodeResult::kTryAgain);
    return results;
}
...
void AV1DecoderTest::Reset()
{
    ...
    EXPECT_LT(base::checked_cast<AV1ReferenceFrameVector::size_type>(base::ranges::count(decoder_->ref_frames_, nullptr)), decoder_->ref_frames_.size());
    ...
    EXPECT_EQ(decoder_->accelerator_.get(), mock_accelerator_);
    EXPECT_EQ(base::checked_cast<AV1ReferenceFrameVector::size_type>(base::ranges::count(decoder_->ref_frames_, nullptr)), decoder_->ref_frames_.size());
    EXPECT_FALSE(decoder_->current_frame_header_);
    EXPECT_FALSE(decoder_->current_frame_);
...
scoped_refptr<DecoderBuffer> AV1DecoderTest::ReadDecoderBuffer(const std::string& fname)
{
...
std::vector<scoped_refptr<DecoderBuffer>> AV1DecoderTest::ReadIVF(const std::string& fname)
{
    std::string ivf_data;
    auto input_file = GetTestFilePath(fname);
    ...
    EXPECT_TRUE(ivf_parser.Initialize(reinterpret_cast<const uint8_t*>(ivf_data.data()), ivf_data.size(), &ivf_header));
...
std::vector<scoped_refptr<DecoderBuffer>> AV1DecoderTest::ReadWebm(const std::string& fname)
{
    std::string webm_data;
    auto input_file = GetTestFilePath(fname);
    EXPECT_TRUE(base::ReadFileToString(input_file, &webm_data));

    InMemoryUrlProtocol protocol(reinterpret_cast<const uint8_t*>(webm_data.data()), webm_data.size(), false);
...
TEST_F(AV1DecoderTest, DecodeInvalidOBU)
{
    ...
    auto kInvalidBuffer = DecoderBuffer::CopyFrom(base::as_byte_span(kInvalidData));
...
TEST_F(AV1DecoderTest, DecodeOneIFrame)
{
    ...
    EXPECT_CALL(*mock_accelerator_, CreateAV1Picture(/*apply_grain=*/false)).WillOnce(Return(av1_picture));
    EXPECT_CALL(*mock_accelerator_,
        SubmitDecode(MatchesFrameHeader(kFrameSize, kRenderSize,
    ...
    EXPECT_CALL(*mock_accelerator_, OutputPicture(SameAV1PictureInstance(av1_picture))).WillOnce(Return(true));
    std::vector<DecodeResult> results = Decode(i_frame_buffer);
    std::vector<DecodeResult> expected = { DecodeResult::kConfigChange, DecodeResult::kRanOutOfStreamData };
    EXPECT_EQ(results, expected);
}
...
TEST_F(AV1DecoderTest, DecodeShowExistingPictureStream)
{
    ...
    std::vector<scoped_refptr<DecoderBuffer>> buffers = ReadIVF(kShowExistingFrameStream);
    ASSERT_FALSE(buffers.empty());
    ...
    EXPECT_CALL(*mock_accelerator_,
        SubmitDecode(MatchesFrameSizeAndRenderSize(kFrameSize, kRenderSize),
            MatchesYUV420SequenceHeader(kProfile, /*bitdepth=*/8, kFrameSize,
                /*film_grain_params_present=*/false),
            _, NonEmptyTileBuffers(), _))
        .Times(kDecodedFrames)
        .WillRepeatedly(Return(AV1Decoder::AV1Accelerator::Status::kOk));
    EXPECT_CALL(*mock_accelerator_, OutputPicture(_)).Times(kOutputFrames).WillRepeatedly(Return(true));
...
TEST_F(AV1DecoderTest, DecodeFilmGrain)
{
    ...
    EXPECT_CALL(*mock_accelerator_, CreateAV1Picture(_)).Times(kDecodedFrames).WillRepeatedly(Return(base::MakeRefCounted<FakeAV1Picture>()));
    EXPECT_CALL(*mock_accelerator_,
        SubmitDecode(MatchesFrameSizeAndRenderSize(kFrameSize, kRenderSize),
            MatchesYUV420SequenceHeader(kProfile, /*bitdepth=*/8, kFrameSize,
...
TEST_F(AV1DecoderTest, ConfigChange)
{
    ...
    const std::string kSimpleStreams[] = { "bear-av1.webm", "bear-av1-480x360.webm" };
    ...
    for (size_t i = 0; i < std::size(kSimpleStreams); ++i) {
        std::vector<scoped_refptr<DecoderBuffer>> buffers = ReadWebm(kSimpleStreams[i]);
        ASSERT_FALSE(buffers.empty());
        ...
        for (auto buffer : buffers) {
            ...
            EXPECT_CALL(*mock_accelerator_,
                SubmitDecode(MatchesFrameHeader(kFrameSizes[i], kRenderSizes[i],
                                 /*show_existing_frame=*/false,
                                 /*show_frame=*/true),
                    MatchesYUV420SequenceHeader(kProfile, /*bitdepth=*/8, kFrameSizes[i],
                        /*film_grain_params_present=*/false),
                    _, NonEmptyTileBuffers(), MatchesFrameData(buffer)))
                .WillOnce(Return(AV1Decoder::AV1Accelerator::Status::kOk));
            EXPECT_CALL(*mock_accelerator_, OutputPicture(SameAV1PictureInstance(av1_picture))).WillOnce(Return(true));
...
TEST_F(AV1DecoderTest, InconsistentReferenceFrameState)
{
    ...
    // reference frame state.
    {
        ...
        EXPECT_CALL(*mock_accelerator_, SubmitDecode(SameAV1PictureInstance(av1_picture), _, _, _, _))
            .WillOnce(DoAll(SaveArg<2>(&ref_frames), Return(AV1Decoder::AV1Accelerator::Status::kOk)));
        EXPECT_CALL(*mock_accelerator_, OutputPicture(SameAV1PictureInstance(av1_picture))).WillOnce(Return(true));
        ...
        EXPECT_EQ(base::ranges::count(decoder_state->reference_frame, nullptr), base::checked_cast<long>(decoder_state->reference_frame.size()));
        ...
        EXPECT_EQ(base::ranges::count(internal_ref_frames, nullptr), base::checked_cast<long>(internal_ref_frames.size()));
        ...
        EXPECT_EQ(base::ranges::count(ref_frames, nullptr), base::checked_cast<long>(ref_frames.size()));
        ...
        EXPECT_TRUE(base::ranges::all_of(
            internal_ref_frames, [&av1_picture](const scoped_refptr<AV1Picture>& ref_frame) { return ref_frame.get() == av1_picture.get(); }));
        testing::Mock::VerifyAndClearExpectations(mock_accelerator_);
    }
...
TEST_F(AV1DecoderTest, TryAgainSubmitDecode)
{
    ...
    std::vector<DecodeResult> results = Decode(i_frame_buffer);
    std::vector<DecodeResult> expected = { DecodeResult::kConfigChange, DecodeResult::kTryAgain };
    EXPECT_EQ(results, expected);
...
TEST_F(AV1DecoderTest, DecodeWithFrameSizeChange)
{
    ...
    std::vector<scoped_refptr<DecoderBuffer>> buffers = ReadIVF("reference-frame-scaling-test.ivf");
    EXPECT_EQ(buffers.size(), kExpectedBuffers);

    auto av1_picture = base::MakeRefCounted<AV1Picture>();

    EXPECT_CALL(*mock_accelerator_, CreateAV1Picture(_)).Times(buffers.size()).WillRepeatedly(Return(av1_picture));

    // Set up three sets of expectations for the expected frame and render sizes
    // as defined above.
    EXPECT_CALL(*mock_accelerator_, SubmitDecode(MatchesFrameSizeAndRenderSize(kFrameSize, kRenderSize), _, _, _, _))
        .Times(kOriginalFrameSizeCount)
        .WillRepeatedly(Return(AV1Decoder::AV1Accelerator::Status::kOk));

    EXPECT_CALL(*mock_accelerator_, SubmitDecode(MatchesFrameSizeAndRenderSize(kNewFrameSize1, kNewRenderSize1), _, _, _, _))
        .Times(kNewFrameSize1Count)
        .WillRepeatedly(Return(AV1Decoder::AV1Accelerator::Status::kOk));

    EXPECT_CALL(*mock_accelerator_, SubmitDecode(MatchesFrameSizeAndRenderSize(kNewFrameSize2, kNewRenderSize2), _, _, _, _))
        .Times(kNewFrameSize2Count)
        .WillRepeatedly(Return(AV1Decoder::AV1Accelerator::Status::kOk));

    EXPECT_CALL(*mock_accelerator_, OutputPicture(MatchesFrameSizeAndRenderSize(kFrameSize, kRenderSize)))
        .Times(kOriginalFrameSizeCount)
        .WillRepeatedly(Return(true));

    EXPECT_CALL(*mock_accelerator_, OutputPicture(MatchesFrameSizeAndRenderSize(kNewFrameSize1, kNewRenderSize1)))
        .Times(kNewFrameSize1Count)
        .WillRepeatedly(Return(true));

    EXPECT_CALL(*mock_accelerator_, OutputPicture(MatchesFrameSizeAndRenderSize(kNewFrameSize2, kNewRenderSize2)))
        .Times(kNewFrameSize2Count)
        .WillRepeatedly(Return(true));
    ...
    EXPECT_THAT(results, testing::Not(testing::Contains(DecodeResult::kDecodeError)));
}
