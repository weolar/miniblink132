...
GpuVideoDecodeAcceleratorHost::GpuVideoDecodeAcceleratorHost(gpu::CommandBufferProxyImpl* impl)
    : client_(nullptr)
    , impl_(impl)
...
bool GpuVideoDecodeAcceleratorHost::Initialize(const Config& config, Client* client)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    client_ = client;
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> io_task_runner = impl_->channel()->io_task_runner();
    bool succeeded = false;
    mojo::SharedAssociatedRemote<mojom::GpuAcceleratedVideoDecoderProvider> provider;
    impl_->BindMediaReceiver(provider.BindNewEndpointAndPassReceiver(io_task_runner));
    provider->CreateAcceleratedVideoDecoder(
        config, decoder_.BindNewEndpointAndPassReceiver(io_task_runner), client_receiver_.BindNewEndpointAndPassRemote(), &succeeded);
    if (!succeeded) {
    ...
    decoder_.set_disconnect_handler(
        base::BindOnce(&GpuVideoDecodeAcceleratorHost::OnDisconnectedFromGpuProcess, weak_this_), base::SequencedTaskRunnerHandle::Get());
    return true;
}
...
void GpuVideoDecodeAcceleratorHost::AssignPictureBuffers(const std::vector<PictureBuffer>& buffers)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!decoder_)
        return;
    ...
    for (const auto& buffer : buffers) {
        if (buffer.size() != picture_buffer_dimensions_) {
            DLOG(ERROR) << "buffer.size() invalid: expected " << picture_buffer_dimensions_.ToString() << ", got " << buffer.size().ToString();
            PostNotifyError(INVALID_ARGUMENT);
            return;
        }
        assignments.push_back(mojom::PictureBufferAssignment::New(buffer.id(), buffer.client_texture_ids()));
    }
    decoder_->AssignPictureBuffers(std::move(assignments));
}

void GpuVideoDecodeAcceleratorHost::ReusePictureBuffer(int32_t picture_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!decoder_)
        return;
...
void GpuVideoDecodeAcceleratorHost::Flush()
{
    ...
    if (!decoder_)
        return;
    decoder_->Flush(base::BindOnce(&GpuVideoDecodeAcceleratorHost::OnFlushDone, base::Unretained(this)));
}
...
void GpuVideoDecodeAcceleratorHost::Reset()
{
    ...
    if (!decoder_)
        return;
    decoder_->Flush(base::BindOnce(&GpuVideoDecodeAcceleratorHost::OnResetDone, base::Unretained(this)));
}

void GpuVideoDecodeAcceleratorHost::SetOverlayInfo(const OverlayInfo& overlay_info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!decoder_)
        return;
...
void GpuVideoDecodeAcceleratorHost::OnWillDeleteImpl()
{
    ...
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuVideoDecodeAcceleratorHost::OnDisconnectedFromGpuProcess, weak_this_));
}
...
void GpuVideoDecodeAcceleratorHost::PostNotifyError(Error error)
{
    ...
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuVideoDecodeAcceleratorHost::OnError, weak_this_, error));
}
...
void GpuVideoDecodeAcceleratorHost::OnInitializationComplete(bool success)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (client_) {
        client_->NotifyInitializationComplete(success ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed);
    }
}

void GpuVideoDecodeAcceleratorHost::OnBitstreamBufferProcessed(int32_t bitstream_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (client_)
...
void GpuVideoDecodeAcceleratorHost::OnProvidePictureBuffers(
    uint32_t num_requested_buffers, VideoPixelFormat format, uint32_t textures_per_buffer, const gfx::Size& dimensions, uint32_t texture_target)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (client_) {
        client_->ProvidePictureBuffers(num_requested_buffers, format, textures_per_buffer, dimensions, texture_target);
    }
}

void GpuVideoDecodeAcceleratorHost::OnDismissPictureBuffer(int32_t picture_buffer_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (client_)
...
void GpuVideoDecodeAcceleratorHost::OnPictureReady(mojom::PictureReadyParamsPtr params)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!client_)
        return;
    Picture picture(params->picture_buffer_id, params->bitstream_buffer_id, params->visible_rect, params->color_space, params->allow_overlay);
