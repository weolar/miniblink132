...
scoped_refptr<CommandBufferHelper> CreateCommandBufferHelper(VdaVideoDecoder::GetStubCB get_stub_cb)
{
...
bool BindImage(scoped_refptr<CommandBufferHelper> command_buffer_helper, uint32_t client_texture_id, uint32_t texture_target,
    const scoped_refptr<gl::GLImage>& image, bool can_bind_to_sampler)
{
    return command_buffer_helper->BindImage(client_texture_id, image.get(), can_bind_to_sampler);
}

std::unique_ptr<VideoDecodeAccelerator> CreateAndInitializeVda(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds,
    scoped_refptr<CommandBufferHelper> command_buffer_helper, VideoDecodeAccelerator::Client* client, MediaLog* media_log,
    const VideoDecodeAccelerator::Config& config)
{
    ...
    if (command_buffer_helper) {
        gl_client.get_context = base::BindRepeating(&CommandBufferHelper::GetGLContext, command_buffer_helper);
        gl_client.make_context_current = base::BindRepeating(&CommandBufferHelper::MakeContextCurrent, command_buffer_helper);
        ...
        gl_client.supports_arb_texture_rectangle = command_buffer_helper->SupportsTextureRectangle();
    }

    std::unique_ptr<GpuVideoDecodeAcceleratorFactory> factory = GpuVideoDecodeAcceleratorFactory::Create(gl_client);
    ...
    // from Initialize().
    return factory->CreateVDA(client, config, gpu_workarounds, gpu_preferences, media_log);
}

bool IsProfileSupported(const VideoDecodeAccelerator::SupportedProfiles& supported_profiles, VideoCodecProfile profile, gfx::Size coded_size)
{
    for (const auto& supported_profile : supported_profiles) {
        if (supported_profile.profile == profile && !supported_profile.encrypted_only
            && gfx::Rect(supported_profile.max_resolution).Contains(gfx::Rect(coded_size))
            && gfx::Rect(coded_size).Contains(gfx::Rect(supported_profile.min_resolution))) {
            return true;
        }
    }
    return false;
...
// static
std::unique_ptr<VideoDecoder> VdaVideoDecoder::Create(scoped_refptr<base::SingleThreadTaskRunner> parent_task_runner,
    scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, std::unique_ptr<MediaLog> media_log, const gfx::ColorSpace& target_color_space,
    const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds, GetStubCB get_stub_cb,
    VideoDecodeAccelerator::Config::OutputMode output_mode)
{
    auto* decoder = new VdaVideoDecoder(std::move(parent_task_runner), std::move(gpu_task_runner), std::move(media_log), target_color_space,
        base::BindOnce(&PictureBufferManager::Create,
            /*allocate_gpu_memory_buffers=*/output_mode == VideoDecodeAccelerator::Config::OutputMode::IMPORT),
        output_mode == VideoDecodeAccelerator::Config::OutputMode::ALLOCATE ? base::BindOnce(&CreateCommandBufferHelper, std::move(get_stub_cb))
                                                                            : base::NullCallback(),
        base::BindRepeating(&CreateAndInitializeVda, gpu_preferences, gpu_workarounds),
        GpuVideoAcceleratorUtil::ConvertGpuToMediaDecodeCapabilities(
            GpuVideoDecodeAcceleratorFactory::GetDecoderCapabilities(gpu_preferences, gpu_workarounds)),
        output_mode);

    return std::make_unique<AsyncDestroyVideoDecoder<VdaVideoDecoder>>(base::WrapUnique(decoder));
}

VdaVideoDecoder::VdaVideoDecoder(scoped_refptr<base::SingleThreadTaskRunner> parent_task_runner, scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner,
    std::unique_ptr<MediaLog> media_log, const gfx::ColorSpace& target_color_space, CreatePictureBufferManagerCB create_picture_buffer_manager_cb,
    CreateCommandBufferHelperCB create_command_buffer_helper_cb, CreateAndInitializeVdaCB create_and_initialize_vda_cb,
    const VideoDecodeAccelerator::Capabilities& vda_capabilities, VideoDecodeAccelerator::Config::OutputMode output_mode)
    : parent_task_runner_(std::move(parent_task_runner))
    , gpu_task_runner_(std::move(gpu_task_runner))
    , media_log_(std::move(media_log))
    , target_color_space_(target_color_space)
    , create_command_buffer_helper_cb_(std::move(create_command_buffer_helper_cb))
    ...
    , timestamps_(128)
{
    ...
    picture_buffer_manager_ = std::move(create_picture_buffer_manager_cb).Run(base::BindRepeating(&VdaVideoDecoder::ReusePictureBuffer, gpu_weak_this_));
}
...
void VdaVideoDecoder::DestroyAsync(std::unique_ptr<VdaVideoDecoder> decoder)
{
    ...
    gpu_task_runner->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::CleanupOnGpuThread, std::move(decoder)));
}

void VdaVideoDecoder::CleanupOnGpuThread(std::unique_ptr<VdaVideoDecoder> decoder)
{
    DVLOG(2) << __func__;
    DCHECK(decoder);
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (has_error_) {
        parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), DecoderStatus::Codes::kFailed));
        return;
    }
    ...
    if (!IsProfileSupported(vda_capabilities_.supported_profiles, config.profile(), config.coded_size())) {
    ...
    if (config.alpha_mode() != VideoDecoderConfig::AlphaMode::kIsOpaque && config.codec() != VideoCodec::kHEVC) {
    ...
    // correct HDR output.
    const bool is_hdr_color_space_change = config_.profile() == media::VP9PROFILE_PROFILE2 && config_.color_space_info() != config.color_space_info();
    ...
    if (reinitializing) {
        if (is_profile_change || is_hdr_color_space_change) {
            MEDIA_LOG(INFO, media_log_) << "Reinitializing video decode accelerator "
                                        << "for profile change";
            gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::ReinitializeOnGpuThread, gpu_weak_this_));
        } else {
            parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::InitializeDone, parent_weak_this_, DecoderStatus::Codes::kOk));
        }
        return;
    }

    gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::InitializeOnGpuThread, gpu_weak_this_));
}
...
void VdaVideoDecoder::InitializeOnGpuThread()
{
    ...
    if (!reinitializing_) {
        if (output_mode_ == VideoDecodeAccelerator::Config::OutputMode::ALLOCATE) {
            ...
            if (!command_buffer_helper_) {
                parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::InitializeDone, parent_weak_this_, DecoderStatus::Codes::kFailed));
                return;
            }
        }
    ...
    vda_ = create_and_initialize_vda_cb_.Run(command_buffer_helper_, this, media_log_.get(), vda_config);
    if (!vda_) {
        parent_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&VdaVideoDecoder::InitializeDone, parent_weak_this_, DecoderStatus::Codes::kFailedToCreateDecoder));
        return;
    }

    gpu_weak_vda_factory_ = std::make_unique<base::WeakPtrFactory<VideoDecodeAccelerator>>(vda_.get());
    gpu_weak_vda_ = gpu_weak_vda_factory_->GetWeakPtr();

    vda_initialized_ = true;
    decode_on_parent_thread_ = vda_->TryToSetupDecodeOnSeparateThread(parent_weak_this_, parent_task_runner_);
...
void VdaVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    if (has_error_) {
        parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
        return;
    }
    ...
    if (buffer->end_of_stream()) {
        ...
        gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoDecodeAccelerator::Flush, gpu_weak_vda_));
        return;
    }
    ...
    gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::DecodeOnGpuThread, gpu_weak_this_, std::move(buffer), bitstream_buffer_id));
}

void VdaVideoDecoder::DecodeOnGpuThread(scoped_refptr<DecoderBuffer> buffer, int32_t bitstream_id)
{
    DVLOG(3) << __func__;
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
...
void VdaVideoDecoder::Reset(base::OnceClosure reset_cb)
{
    ...
    gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoDecodeAccelerator::Reset, gpu_weak_vda_));
}
...
bool VdaVideoDecoder::NeedsBitstreamConversion() const
{
    ...
    // always return false?
    return config_.codec() == VideoCodec::kH264 || config_.codec() == VideoCodec::kHEVC;
}
...
void VdaVideoDecoder::ProvidePictureBuffers(
    uint32_t requested_num_of_buffers, VideoPixelFormat format, uint32_t textures_per_buffer, const gfx::Size& dimensions, uint32_t texture_target)
{
    DVLOG(2) << __func__ << "(" << requested_num_of_buffers << ", " << format << ", " << textures_per_buffer << ", " << dimensions.ToString() << ", "
             << texture_target << ")";
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    DCHECK(vda_initialized_);
    ...
    gpu_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &VdaVideoDecoder::ProvidePictureBuffersAsync, gpu_weak_this_, requested_num_of_buffers, format, textures_per_buffer, dimensions, texture_target));
}

void VdaVideoDecoder::ProvidePictureBuffersWithVisibleRect(uint32_t requested_num_of_buffers, VideoPixelFormat format, uint32_t textures_per_buffer,
    const gfx::Size& dimensions, const gfx::Rect& visible_rect, uint32_t texture_target)
{
    if (output_mode_ == VideoDecodeAccelerator::Config::OutputMode::IMPORT) {
        ...
        ProvidePictureBuffers(requested_num_of_buffers, format, textures_per_buffer, dimensions, texture_target);
        return;
    }
    VideoDecodeAccelerator::Client::ProvidePictureBuffersWithVisibleRect(
        requested_num_of_buffers, format, textures_per_buffer, dimensions, visible_rect, texture_target);
}

void VdaVideoDecoder::ProvidePictureBuffersAsync(uint32_t count, VideoPixelFormat pixel_format, uint32_t planes, gfx::Size texture_size, GLenum texture_target)
{
    DVLOG(2) << __func__;
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    DCHECK_GT(count, 0U);
    ...
    std::vector<std::pair<PictureBuffer, gfx::GpuMemoryBufferHandle>> picture_buffers_and_gmbs
        = picture_buffer_manager_->CreatePictureBuffers(count, pixel_format, planes, texture_size, texture_target,
            output_mode_ == VideoDecodeAccelerator::Config::OutputMode::IMPORT ? VideoDecodeAccelerator::TextureAllocationMode::kDoNotAllocateGLTextures
                                                                               : vda_->GetSharedImageTextureAllocationMode());
    if (picture_buffers_and_gmbs.empty()) {
        parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::EnterErrorState, parent_weak_this_));
        return;
    }
    ...
    if (output_mode_ == VideoDecodeAccelerator::Config::OutputMode::IMPORT) {
        for (auto& picture_buffer_and_gmb : picture_buffers_and_gmbs) {
            vda_->ImportBufferForPicture(picture_buffer_and_gmb.first.id(), pixel_format, std::move(picture_buffer_and_gmb.second));
        }
    }
}
...
void VdaVideoDecoder::DismissPictureBuffer(int32_t picture_buffer_id)
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::DismissPictureBufferOnParentThread, parent_weak_this_, picture_buffer_id));
}

void VdaVideoDecoder::DismissPictureBufferOnParentThread(int32_t picture_buffer_id)
{
    DVLOG(2) << __func__ << "(" << picture_buffer_id << ")";
    DCHECK(parent_task_runner_->BelongsToCurrentThread());
...
void VdaVideoDecoder::PictureReady(const Picture& picture)
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::PictureReadyOnParentThread, parent_weak_this_, picture));
}
...
void VdaVideoDecoder::PictureReadyOnParentThread(Picture picture)
{
    ...
    scoped_refptr<VideoFrame> frame
        = picture_buffer_manager_->CreateVideoFrame(picture, timestamp, visible_rect, config_.aspect_ratio().GetNaturalSize(visible_rect));
    if (!frame) {
        EnterErrorState();
        return;
    }
...
void VdaVideoDecoder::NotifyEndOfBitstreamBuffer(int32_t bitstream_buffer_id)
{
    ...
    parent_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VdaVideoDecoder::NotifyEndOfBitstreamBufferOnParentThread, parent_weak_this_, bitstream_buffer_id));
}

void VdaVideoDecoder::NotifyEndOfBitstreamBufferOnParentThread(int32_t bitstream_buffer_id)
{
    DVLOG(3) << __func__ << "(" << bitstream_buffer_id << ")";
    DCHECK(parent_task_runner_->BelongsToCurrentThread());
...
void VdaVideoDecoder::NotifyFlushDone()
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::NotifyFlushDoneOnParentThread, parent_weak_this_));
}
...
void VdaVideoDecoder::NotifyResetDone()
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::NotifyResetDoneOnParentThread, parent_weak_this_));
}
...
void VdaVideoDecoder::NotifyError(VideoDecodeAccelerator::Error error)
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::NotifyErrorOnParentThread, parent_weak_this_, error));
}
...
void VdaVideoDecoder::NotifyErrorOnParentThread(VideoDecodeAccelerator::Error error)
{
    DVLOG(1) << __func__ << "(" << error << ")";
    DCHECK(parent_task_runner_->BelongsToCurrentThread());
...
void VdaVideoDecoder::EnterErrorState()
{
    ...
    parent_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VdaVideoDecoder::DestroyCallbacks, parent_weak_this_));
}
