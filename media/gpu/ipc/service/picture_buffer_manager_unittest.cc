...
public:
    explicit PictureBufferManagerImplTest()
    {
        ...
        cbh_ = base::MakeRefCounted<FakeCommandBufferHelper>(environment_.GetMainThreadTaskRunner());
        pbm_ = PictureBufferManager::Create(/*allocate_gpu_memory_buffers=*/false, reuse_cb_.Get());
    }

    PictureBufferManagerImplTest(const PictureBufferManagerImplTest&) = delete;
    PictureBufferManagerImplTest& operator=(const PictureBufferManagerImplTest&) = delete;
...
protected:
    ...
    std::vector<PictureBuffer> CreateARGBPictureBuffers(
        uint32_t count, VideoDecodeAccelerator::TextureAllocationMode mode = VideoDecodeAccelerator::TextureAllocationMode::kAllocateGLTextures)
    {
        std::vector<std::pair<PictureBuffer, gfx::GpuMemoryBufferHandle>> picture_buffers_and_gmbs
            = pbm_->CreatePictureBuffers(count, PIXEL_FORMAT_ARGB, 1, gfx::Size(320, 240), GL_TEXTURE_2D, mode);
    ...
    PictureBuffer CreateARGBPictureBuffer(
        VideoDecodeAccelerator::TextureAllocationMode mode = VideoDecodeAccelerator::TextureAllocationMode::kAllocateGLTextures)
    {
        std::vector<PictureBuffer> picture_buffers = CreateARGBPictureBuffers(1, mode);
    ...
    {
        return pbm_->CreateVideoFrame(Picture(picture_buffer_id, // picture_buffer_id
    ...
    testing::StrictMock<base::MockCallback<PictureBufferManager::ReusePictureBufferCB>> reuse_cb_;
...
{
    Initialize();
    PictureBuffer pb1 = CreateARGBPictureBuffer(VideoDecodeAccelerator::TextureAllocationMode::kDoNotAllocateGLTextures);
    EXPECT_EQ(pb1.client_texture_ids().size(), 0u);

    PictureBuffer pb2 = CreateARGBPictureBuffer(VideoDecodeAccelerator::TextureAllocationMode::kAllocateGLTextures);
