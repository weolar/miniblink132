...
public:
    PictureBufferManagerImpl(bool allocate_gpu_memory_buffers, ReusePictureBufferCB reuse_picture_buffer_cb)
    ...
    void Initialize(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, scoped_refptr<CommandBufferHelper> command_buffer_helper) override
    {
        DVLOG(1) << __func__;
    ...
    std::vector<std::pair<PictureBuffer, gfx::GpuMemoryBufferHandle>> CreatePictureBuffers(uint32_t count, VideoPixelFormat pixel_format, uint32_t planes,
        gfx::Size texture_size, uint32_t texture_target, VideoDecodeAccelerator::TextureAllocationMode mode) override
    {
        DVLOG(2) << __func__;
        DCHECK(gpu_task_runner_);
        ...
        DCHECK(!allocate_gpu_memory_buffers_ || mode == VideoDecodeAccelerator::TextureAllocationMode::kDoNotAllocateGLTextures);

        // TODO(sandersd): Consider requiring that CreatePictureBuffers() is
        // called with the context current.
        if (mode == VideoDecodeAccelerator::TextureAllocationMode::kAllocateGLTextures) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (mode == VideoDecodeAccelerator::TextureAllocationMode::kAllocateGLTextures) {
                for (uint32_t j = 0; j < planes; j++) {
                    ...
                    const size_t width = VideoFrame::Columns(j, pixel_format, texture_size.width());
                    const size_t height = VideoFrame::Rows(j, pixel_format, texture_size.height());
                    ...
                    GLuint service_id = command_buffer_helper_->CreateTexture(texture_target, GL_RGBA, width, height, GL_RGBA, GL_UNSIGNED_BYTE);
                    DCHECK(service_id);
                    ...
                    picture_data.mailbox_holders[j] = gpu::MailboxHolder(command_buffer_helper_->CreateMailbox(service_id), gpu::SyncToken(), texture_target);
                }
            }
            ...
            if (allocate_gpu_memory_buffers_) {
#if BUILDFLAG(USE_VAAPI) || BUILDFLAG(USE_V4L2_CODEC)
                scoped_refptr<VideoFrame> gpu_memory_buffer_video_frame = CreateGpuMemoryBufferVideoFrame(
                    pixel_format, texture_size, gfx::Rect(texture_size), texture_size, base::TimeDelta(), gfx::BufferUsage::SCANOUT_VDA_WRITE);
                ...
                gfx::GpuMemoryBuffer* gmb = gpu_memory_buffer_video_frame->GetGpuMemoryBuffer();
                DCHECK(gmb);
                gmb_handle = gmb->CloneHandle();
                if (gmb_handle.type != gfx::NATIVE_PIXMAP || gmb_handle.native_pixmap_handle.planes.empty()) {
                    return {};
                }
                picture_data.gpu_memory_buffer_video_frame = std::move(gpu_memory_buffer_video_frame);
#else
                NOTREACHED();
                return {};
#endif // BUILDFLAG(USE_VAAPI) || BUILDFLAG(USE_V4L2_CODEC)
            ...
            picture_buffers_and_gmbs.emplace_back(PictureBuffer { picture_buffer_id, texture_size, picture_data.texture_sizes, picture_data.service_ids,
    ...
    bool DismissPictureBuffer(int32_t picture_buffer_id) override
    {
        ...
        if (!it->second.IsInUse()) {
            gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PictureBufferManagerImpl::DestroyPictureBuffer, this, picture_buffer_id));
        }
    ...
    scoped_refptr<VideoFrame> CreateVideoFrame(Picture picture, base::TimeDelta timestamp, gfx::Rect visible_rect, gfx::Size natural_size) override
    {
        ...
        if (!gfx::Rect(picture_buffer_data.texture_size).Contains(visible_rect)) {
            DLOG(WARNING) << "visible_rect " << visible_rect.ToString() << " exceeds coded_size " << picture_buffer_data.texture_size.ToString();
        ...
        if (picture_buffer_data.gpu_memory_buffer_video_frame) {
            frame = VideoFrame::WrapVideoFrame(
                picture_buffer_data.gpu_memory_buffer_video_frame, picture_buffer_data.gpu_memory_buffer_video_frame->format(), visible_rect, natural_size);
            if (!frame) {
                DLOG(ERROR) << "Failed to create VideoFrame for picture.";
                return nullptr;
            }
            frame->set_timestamp(timestamp);
            frame->AddDestructionObserver(base::BindOnce(&PictureBufferManagerImpl::OnVideoFrameDestroyed, this, picture_buffer_id, gpu::SyncToken()));
        } else {
            frame = VideoFrame::WrapNativeTextures(picture_buffer_data.pixel_format, picture_buffer_data.mailbox_holders,
                base::BindOnce(&PictureBufferManagerImpl::OnVideoFrameDestroyed, this, picture_buffer_id), picture_buffer_data.texture_size, visible_rect,
        ...
        frame->metadata().read_lock_fences_enabled = picture.read_lock_fences_enabled();
...
private:
    ...
    {
        DVLOG(1) << __func__;
        DCHECK(picture_buffers_.empty() || (!command_buffer_helper_ || !command_buffer_helper_->HasStub()));
    }

    void OnVideoFrameDestroyed(int32_t picture_buffer_id, const gpu::SyncToken& sync_token)
    {
        ...
        if (command_buffer_helper_) {
            ...
            gpu_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&CommandBufferHelper::WaitForSyncToken, command_buffer_helper_, sync_token,
                    base::BindOnce(&PictureBufferManagerImpl::OnSyncTokenReleased, this, picture_buffer_id)));
        } else {
            gpu_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PictureBufferManagerImpl::OnSyncTokenReleased, this, picture_buffer_id));
        }
    }
    ...
    void DestroyPictureBuffer(int32_t picture_buffer_id)
    {
        ...
        std::array<scoped_refptr<Picture::ScopedSharedImage>, VideoFrame::kMaxPlanes> scoped_shared_images;
        {
            base::AutoLock lock(picture_buffers_lock_);
            const auto& it = picture_buffers_.find(picture_buffer_id);
            DCHECK(it != picture_buffers_.end());
    ...
    std::map<int32_t, PictureBufferData> picture_buffers_ GUARDED_BY(picture_buffers_lock_);
};

} // namespace

// static
scoped_refptr<PictureBufferManager> PictureBufferManager::Create(bool allocate_gpu_memory_buffers, ReusePictureBufferCB reuse_picture_buffer_cb)
{
    return base::MakeRefCounted<PictureBufferManagerImpl>(allocate_gpu_memory_buffers, std::move(reuse_picture_buffer_cb));
}
