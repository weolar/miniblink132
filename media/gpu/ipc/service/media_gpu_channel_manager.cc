...
MediaGpuChannelManager::MediaGpuChannelManager(gpu::GpuChannelManager* channel_manager)
...
void MediaGpuChannelManager::AddChannel(int32_t client_id, const base::UnguessableToken& channel_token)
{
    gpu::GpuChannel* gpu_channel = channel_manager_->LookupChannel(client_id);
    DCHECK(gpu_channel);
    auto media_gpu_channel = std::make_unique<MediaGpuChannel>(gpu_channel, overlay_factory_cb_);
...
gpu::GpuChannel* MediaGpuChannelManager::LookupChannel(const base::UnguessableToken& channel_token)
{
...
void MediaGpuChannelManager::SetOverlayFactory(AndroidOverlayMojoFactoryCB overlay_factory_cb)
{
...
scoped_refptr<gpu::SharedContextState> MediaGpuChannelManager::GetSharedContextState()
{
