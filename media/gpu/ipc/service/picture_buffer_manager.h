...
class PictureBufferManager : public base::RefCountedThreadSafe<PictureBufferManager> {
public:
    ...
    static scoped_refptr<PictureBufferManager> Create(bool allocate_gpu_memory_buffers, ReusePictureBufferCB reuse_picture_buffer_cb);
    ...
    virtual void Initialize(scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner, scoped_refptr<CommandBufferHelper> command_buffer_helper) = 0;
    ...
    virtual std::vector<std::pair<PictureBuffer, gfx::GpuMemoryBufferHandle>> CreatePictureBuffers(uint32_t count, VideoPixelFormat pixel_format,
        uint32_t planes, gfx::Size texture_size, uint32_t texture_target, VideoDecodeAccelerator::TextureAllocationMode mode)
        = 0;
    ...
    virtual scoped_refptr<VideoFrame> CreateVideoFrame(Picture picture, base::TimeDelta timestamp, gfx::Rect visible_rect, gfx::Size natural_size) = 0;
