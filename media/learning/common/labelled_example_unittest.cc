...
TEST_F(LearnerLabelledExampleTest, InitListWorks)
{
    ...
    std::vector<FeatureValue> features = { FeatureValue(kFeature1), FeatureValue(kFeature2) };
    TargetValue target(789);
    LabelledExample example({ FeatureValue(kFeature1), FeatureValue(kFeature2) }, target);
...
{
    LabelledExample example_1({ FeatureValue(123), FeatureValue(456) }, TargetValue(789));
...
TEST_F(LearnerLabelledExampleTest, EqualExamplesCompareAsEqual)
{
    ...
    const int kFeature2 = 456;
    TargetValue target(789);
    LabelledExample example_1({ FeatureValue(kFeature1), FeatureValue(kFeature2) }, target);
    LabelledExample example_2({ FeatureValue(kFeature1), FeatureValue(kFeature2) }, target);
...
TEST_F(LearnerLabelledExampleTest, UnequalFeaturesCompareAsUnequal)
{
    ...
    const int kFeature2 = 456;
    TargetValue target(789);
    LabelledExample example_1({ FeatureValue(kFeature1), FeatureValue(kFeature1) }, target);
    LabelledExample example_2({ FeatureValue(kFeature2), FeatureValue(kFeature2) }, target);
...
TEST_F(LearnerLabelledExampleTest, UnequalTargetsCompareAsUnequal)
{
    const int kFeature1 = 123;
    const int kFeature2 = 456;
    LabelledExample example_1({ FeatureValue(kFeature1), FeatureValue(kFeature1) }, TargetValue(789));
    LabelledExample example_2({ FeatureValue(kFeature2), FeatureValue(kFeature2) }, TargetValue(987));
    EXPECT_TRUE(example_1 != example_2);
    EXPECT_FALSE(example_1 == example_2);
