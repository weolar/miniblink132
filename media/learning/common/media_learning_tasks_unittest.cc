...
{
    LearningTask task = MediaLearningTasks::Get(tasknames::kConsecutiveBadWindows);
    // Make sure the name is correct, mostly to reduce cut-and-paste errors.
...
{
    int count = 0;
    auto cb = base::BindRepeating([](int* count, const LearningTask& task) { (*count)++; }, base::Unretained(&count));
