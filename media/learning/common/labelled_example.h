...
struct COMPONENT_EXPORT(LEARNING_COMMON) LabelledExample {
    ...
    LabelledExample(std::initializer_list<FeatureValue> init_list, TargetValue target);
...
public:
    ...
    {
        return examples_.size() == total_weight_;
    }

    // Provide the |i|-th example, over [0, size()).
    const LabelledExample& operator[](size_t i) const
    {
        return examples_[i];
    }
    LabelledExample& operator[](size_t i)
    {
        return examples_[i];
    }
