...
void PushWMPIFeatures(LearningTask& task)
{
    ...
    // it memorizes them.
    task.feature_descriptions.push_back({ "codec", LearningTask::Ordering::kUnordered });
    task.feature_descriptions.push_back({ "profile", LearningTask::Ordering::kUnordered });
    task.feature_descriptions.push_back({ "width", LearningTask::Ordering::kNumeric });
    task.feature_descriptions.push_back({ "fps", LearningTask::Ordering::kNumeric });
}
...
const LearningTask& GetConsecutiveBadWindowsTask()
{
    ...
    if (!task->feature_descriptions.size()) {
        ...
        task->target_description = { "max_bad_windows", LearningTask::Ordering::kNumeric };
...
const LearningTask& GetConsecutiveNNRsTask()
{
    ...
    if (!task->feature_descriptions.size()) {
        ...
        // Target is max number of consecutive bad windows.
        task->target_description = { "total_playback_nnrs", LearningTask::Ordering::kNumeric };
...
// static
void MediaLearningTasks::Register(base::RepeatingCallback<void(const LearningTask&)> cb)
{
