...
TargetHistogram& TargetHistogram::operator=(const TargetHistogram& rhs) = default;
...
bool TargetHistogram::FindSingularMax(TargetValue* value_out, double* counts_out) const
{
    if (!counts_.size())
        return false;
...
std::ostream& operator<<(std::ostream& out, const media::learning::TargetHistogram& dist)
{
