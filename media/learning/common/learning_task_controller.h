...
struct ObservationCompletion {
    ...
    /* implicit */ ObservationCompletion(const TargetValue& target, WeightType w = 1.)
        : target_value(target)
        , weight(w)
    {
    }
...
public:
    using PredictionCB = base::OnceCallback<void(const std::optional<TargetHistogram>& predicted)>;
    ...
    // benefit from it.
    virtual void BeginObservation(base::UnguessableToken id, const FeatureVector& features, const std::optional<TargetValue>& default_target = std::nullopt,
        const std::optional<ukm::SourceId>& source_id = std::nullopt)
        = 0;

    // Complete an observation by sending a completion.
    virtual void CompleteObservation(base::UnguessableToken id, const ObservationCompletion& completion) = 0;
    ...
    virtual void UpdateDefaultTarget(base::UnguessableToken id, const std::optional<TargetValue>& default_target) = 0;
    ...
    virtual void PredictDistribution(const FeatureVector& features, PredictionCB callback) = 0;
};
