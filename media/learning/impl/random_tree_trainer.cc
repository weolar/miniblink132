...
RandomTreeTrainer::Split::Split(int index)
    : split_index(index)
{
}
...
RandomTreeTrainer::Split& RandomTreeTrainer::Split::operator=(Split&& rhs) = default;
...
struct InteriorNode : public Model {
    InteriorNode(const LearningTask& task, int split_index, FeatureValue split_point)
    ...
    TargetHistogram PredictDistributionWithMissingValues(const FeatureVector& features)
    {
        TargetHistogram total;
        for (auto& child_pair : children_) {
            TargetHistogram predicted = child_pair.second->PredictDistribution(features);
...
struct LeafNode : public Model {
    LeafNode(const TrainingData& training_data, const std::vector<size_t>& training_idx, LearningTask::Ordering ordering)
    {
...
void RandomTreeTrainer::Train(const LearningTask& task, const TrainingData& training_data, TrainedModelCB model_cb)
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(model_cb), std::move(model)));
}

std::unique_ptr<Model> RandomTreeTrainer::Train(const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx)
{
    if (training_data.empty()) {
        return std::make_unique<LeafNode>(training_data, std::vector<size_t>(), LearningTask::Ordering::kUnordered);
    }
...
std::unique_ptr<Model> RandomTreeTrainer::Build(
    const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, const FeatureSet& unused_set)
{
    ...
    // the Optional.
    std::optional<TargetValue> target_value(training_data[training_idx[0]].target_value);
    ...
    for (size_t feature_idx : unused_set) {
        feature_values[feature_idx] = training_data[training_idx[0]].features[feature_idx];
    }
    for (size_t idx : training_idx) {
    ...
    if (target_value) {
        return std::make_unique<LeafNode>(training_data, training_idx, task.target_description.ordering);
    }
    ...
    const size_t features_per_split = std::max(static_cast<int>(sqrt(feature_candidates.size())), 3);
    ...
    for (int i : feature_candidates) {
        Split potential_split = ConstructSplit(task, training_data, training_idx, i);
    ...
    std::unique_ptr<InteriorNode> node = std::make_unique<InteriorNode>(task, best_potential_split.split_index, best_potential_split.split_point);

    for (auto& branch_iter : best_potential_split.branch_infos) {
        node->AddChild(branch_iter.first, Build(task, training_data, branch_iter.second.training_idx, new_unused_set));
    }
...
RandomTreeTrainer::Split RandomTreeTrainer::ConstructSplit(
    const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, int split_index)
{
    ...
    bool is_numeric = task.feature_descriptions[split_index].ordering == LearningTask::Ordering::kNumeric;
    ...
    if (is_numeric) {
        split.split_point = FindSplitPoint_Numeric(split.split_index, training_data, training_idx);
    } else {
        split.split_point = FindSplitPoint_Nominal(split.split_index, training_data, training_idx);
    }
    ...
    for (size_t idx : training_idx) {
        ...
        auto result = split.branch_infos.emplace(split_feature, Split::BranchInfo());
...
void RandomTreeTrainer::ComputeSplitScore_Nominal(Split* split, double total_incoming_weight)
{
    ...
    for (auto& info_iter : split->branch_infos) {
        ...
        const double weight_along_branch = branch_info.target_histogram.total_counts();
...
void RandomTreeTrainer::ComputeSplitScore_Numeric(Split* split, double total_incoming_weight)
{
    // Compute the nats given that we're at this node.
    split->nats_remaining = 0;
    for (auto& info_iter : split->branch_infos) {
        ...
        for (auto& iter : branch_info.target_histogram) {
            ...
            double sq_err = (iter.first.value() - average) * (iter.first.value() - average) * iter.second;
            split->nats_remaining += sq_err * p_branch;
        }
    }
}

FeatureValue RandomTreeTrainer::FindSplitPoint_Numeric(size_t split_index, const TrainingData& training_data, const std::vector<size_t>& training_idx)
{
    // We should not be given a training set of size 0, since there's no need to
    // check an empty split.
    DCHECK_GT(training_idx.size(), 0u);
    ...
    if (v_max == v_min) {
        ...
        v_split = FeatureValue(rng()->GenerateDouble(v_max.value() - v_min.value()) + v_min.value());
    }

    return v_split;
}

FeatureValue RandomTreeTrainer::FindSplitPoint_Nominal(size_t split_index, const TrainingData& training_data, const std::vector<size_t>& training_idx)
{
    // We should not be given a training set of size 0, since there's no need to
    // check an empty split.
    DCHECK_GT(training_idx.size(), 0u);
