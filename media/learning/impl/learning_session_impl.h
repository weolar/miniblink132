...
// learning tasks.
class COMPONENT_EXPORT(LEARNING_IMPL) LearningSessionImpl : public LearningSession {
public:
    // We will create LearningTaskControllers that run on |task_runner|.
    explicit LearningSessionImpl(scoped_refptr<base::SequencedTaskRunner> task_runner);
    ~LearningSessionImpl() override;

    // Create a SequenceBound controller for |task| on |task_runner|.
    using CreateTaskControllerCB = base::RepeatingCallback<base::SequenceBound<LearningTaskController>(
        scoped_refptr<base::SequencedTaskRunner>, const LearningTask&, SequenceBoundFeatureProvider)>;

    void SetTaskControllerFactoryCBForTesting(CreateTaskControllerCB cb);

    // LearningSession
    std::unique_ptr<LearningTaskController> GetController(const std::string& task_name) override;

    // Registers |task|, so that calls to AddExample with |task.name| will work.
    // This will create a new controller for the task.
    void RegisterTask(const LearningTask& task, SequenceBoundFeatureProvider feature_provider = SequenceBoundFeatureProvider());
...
private:
    ...
    using LearningTaskControllerMap = std::map<std::string, base::SequenceBound<LearningTaskController>>;
