...
TEST_P(ExtraTreesTest, FisherIrisDataset)
{
    ...
    for (const LabelledExample& example : training_data) {
        ...
        if (distribution.FindSingularMax(&predicted_value) && predicted_value == example.target_value) {
...
TEST_P(ExtraTreesTest, RegressionWorks)
{
    ...
    // weight.  See if that one wins.
    SetupFeatures(2);
    LabelledExample example_1({ FeatureValue(1), FeatureValue(123) }, TargetValue(1));
    LabelledExample example_1_a({ FeatureValue(1), FeatureValue(123) }, TargetValue(5));
    LabelledExample example_2({ FeatureValue(1), FeatureValue(456) }, TargetValue(20));
    LabelledExample example_2_a({ FeatureValue(1), FeatureValue(456) }, TargetValue(25));
    TrainingData training_data;
...
INSTANTIATE_TEST_SUITE_P(ExtraTreesTest, ExtraTreesTest, testing::ValuesIn({ LearningTask::Ordering::kUnordered, LearningTask::Ordering::kNumeric }));
