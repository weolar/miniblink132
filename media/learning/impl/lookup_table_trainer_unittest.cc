        ...
        for (int f2 = 0; f2 < 2; f2++) {
            for (int f3 = 0; f3 < 2; f3++) {
                for (int f4 = 0; f4 < 2; f4++) {
                    ...
                    training_data.push_back(LabelledExample(
...
TEST_F(LookupTableTrainerTest, UnknownFeatureValueHandling)
{
    ...
    std::unique_ptr<Model> model = Train(task_, training_data);
    TargetHistogram distribution = model->PredictDistribution(FeatureVector({ FeatureValue(789) }));
...
TEST_F(LookupTableTrainerTest, RegressionWithWeightedExamplesWorks)
{
    ...
    std::unique_ptr<Model> model = Train(task_, training_data);
    TargetHistogram distribution = model->PredictDistribution(FeatureVector({ FeatureValue(123) }));
    double avg = distribution.Average();
    const double expected = static_cast<double>(((example_1.target_value.value() * example_1.weight) + (example_2.target_value.value() * example_2.weight)))
