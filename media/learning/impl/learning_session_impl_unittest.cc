...
public:
    ...
    using ControllerVector = std::vector<raw_ptr<FakeLearningTaskController, VectorExperimental>>;
    using TaskRunnerVector = std::vector<raw_ptr<base::SequencedTaskRunner, VectorExperimental>>;
    ...
    public:
        ...
        FakeLearningTaskController(void* controllers, const LearningTask& task, SequenceBoundFeatureProvider feature_provider)
            : feature_provider_(std::move(feature_provider))
        {
            ...
            if (!feature_provider_.is_null()) {
                feature_provider_.AsyncCall(&FeatureProvider::AddFeatures).WithArgs(FeatureVector(), FeatureProvider::FeatureVectorCB());
            }
        }

        void BeginObservation(base::UnguessableToken id, const FeatureVector& features, const std::optional<TargetValue>& default_target,
        ...
        void CompleteObservation(base::UnguessableToken id, const ObservationCompletion& completion) override
        {
            EXPECT_EQ(id_, id);
        ...
        void UpdateDefaultTarget(base::UnguessableToken id, const std::optional<TargetValue>& default_target) override
        {
        ...
        void PredictDistribution(const FeatureVector& features, PredictionCB callback) override
        {
    ...
    public:
        FakeFeatureProvider(bool* flag_ptr)
            : flag_ptr_(flag_ptr)
        {
        }

        // Do nothing, except note that we were called.
        void AddFeatures(FeatureVector features, FeatureProvider::FeatureVectorCB cb) override
        {
            *flag_ptr_ = true;
        }
    ...
    LearningSessionImplTest()
    {
        ...
        session_->SetTaskControllerFactoryCBForTesting(base::BindRepeating(
            [](ControllerVector* controllers, TaskRunnerVector* task_runners, scoped_refptr<base::SequencedTaskRunner> task_runner, const LearningTask& task,
                SequenceBoundFeatureProvider feature_provider) -> base::SequenceBound<LearningTaskController> {
                task_runners->push_back(task_runner.get());
                return base::SequenceBound<FakeLearningTaskController>(task_runner, static_cast<void*>(controllers), task, std::move(feature_provider));
            },
...
TEST_F(LearningSessionImplTest, RegisteringTasksCreatesControllers)
{
    ...
    std::unique_ptr<LearningTaskController> ltc_0 = session_->GetController(task_0_.name);
    EXPECT_EQ(ltc_0->GetLearningTask().name, task_0_.name);

    std::unique_ptr<LearningTaskController> ltc_1 = session_->GetController(task_1_.name);
...
TEST_F(LearningSessionImplTest, ExamplesAreForwardedToCorrectTask)
{
    ...
    LabelledExample example_0({ FeatureValue(123), FeatureValue(456) }, TargetValue(1234));
    std::unique_ptr<LearningTaskController> ltc_0 = session_->GetController(task_0_.name);
    ukm::SourceId source_id(123);
    ltc_0->BeginObservation(id, example_0.features, std::nullopt, source_id);
    ltc_0->CompleteObservation(id, ObservationCompletion(example_0.target_value, example_0.weight));

    LabelledExample example_1({ FeatureValue(321), FeatureValue(654) }, TargetValue(4321));

    std::unique_ptr<LearningTaskController> ltc_1 = session_->GetController(task_1_.name);
    ltc_1->BeginObservation(id, example_1.features);
    ltc_1->CompleteObservation(id, ObservationCompletion(example_1.target_value, example_1.weight));
...
TEST_F(LearningSessionImplTest, ControllerLifetimeScopedToSession)
{
    ...
    std::unique_ptr<LearningTaskController> controller = session_->GetController(task_0_.name);
    ...
    // Should not crash.
    controller->BeginObservation(base::UnguessableToken::Create(), FeatureVector());
}
...
TEST_F(LearningSessionImplTest, FeatureProviderIsForwarded)
{
    ...
    bool flag = false;
    session_->RegisterTask(task_0_, base::SequenceBound<FakeFeatureProvider>(task_runner_, &flag));
    task_environment_.RunUntilIdle();
...
TEST_F(LearningSessionImplTest, DestroyingControllerCompletesObservationsWithDefaultValues)
{
...
TEST_F(LearningSessionImplTest, PredictDistribution)
{
    ...
    controller->PredictDistribution(features,
        base::BindOnce(
            [](TargetHistogram* test_storage, const std::optional<TargetHistogram>& predicted) { *test_storage = *predicted; }, &observed_prediction));
    task_environment_.RunUntilIdle();
