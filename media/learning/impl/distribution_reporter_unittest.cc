...
TEST_F(DistributionReporterTest, CallbackRecordsRegressionPredictions)
{
    ...
    std::vector<raw_ptr<const ukm::mojom::UkmEntry, VectorExperimental>> entries = ukm_recorder_->GetEntriesByName("Media.Learning.PredictionRecord");
    EXPECT_EQ(entries.size(), 1u);
    ukm::TestUkmRecorder::ExpectEntryMetric(entries[0], "LearningTask", task_.GetId());
...
TEST_F(DistributionReporterTest, DistributionReporterHackyConfusionMatrixNeedsRegression)
{
