...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    while (provider.remaining_bytes() > 0) {
        ...
        controller.BeginObservation(id, ConsumeFeatureVector(&provider), default_target, std::nullopt);
        controller.CompleteObservation(id, ObservationCompletion(TargetValue(ConsumeDouble(&provider)), ConsumeDouble(&provider)));
