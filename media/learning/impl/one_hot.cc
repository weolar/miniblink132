...
OneHotConverter::OneHotConverter(const LearningTask& task, const TrainingData& training_data)
    : converted_task_(task)
{
    ...
    for (size_t i = 0; i < task.feature_descriptions.size(); i++) {
        const LearningTask::ValueDescription& feature = task.feature_descriptions[i];
...
FeatureVector OneHotConverter::Convert(const FeatureVector& feature_vector) const
{
...
void OneHotConverter::ProcessOneFeature(size_t index, const LearningTask::ValueDescription& original_description, const TrainingData& training_data)
{
...
ConvertingModel::ConvertingModel(std::unique_ptr<OneHotConverter> converter, std::unique_ptr<Model> model)
...
TargetHistogram ConvertingModel::PredictDistribution(const FeatureVector& instance)
{
