...
public:
    ...
    void GenerateAndVerify(base::RepeatingCallback<int64_t()> generate, int64_t lower_inclusive, int64_t upper_inclusive)
    {
...
TEST_F(RandomNumberGeneratorTest, ExclusiveUpTo)
{
    ...
    GenerateAndVerify(
        base::BindRepeating([](RandomNumberGenerator* rng, uint64_t range) { return static_cast<int64_t>(rng->Generate(range)); }, &rng_, range), 0, range - 1);
}
