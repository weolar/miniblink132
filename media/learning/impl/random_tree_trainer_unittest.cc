...
public:
    ...
    std::unique_ptr<Model> Train(const LearningTask& task, const TrainingData& data)
    {
        std::unique_ptr<Model> model;
        trainer_.Train(
            task_, data, base::BindOnce([](std::unique_ptr<Model>* model_out, std::unique_ptr<Model> model) { *model_out = std::move(model); }, &model));
...
{
    SetupFeatures(2);
    LabelledExample example({ FeatureValue(123), FeatureValue(456) }, TargetValue(789));
            ...
            for (int f3 = 0; f3 < 2; f3++) {
                for (int f4 = 0; f4 < 2; f4++) {
                    LabelledExample example(
                        { FeatureValue(f1), FeatureValue(f2), FeatureValue(f3), FeatureValue(f4) }, TargetValue(f1 * 1 + f2 * 2 + f3 * 4 + f4 * 8));
...
TEST_P(RandomTreeTest, UnknownFeatureValueHandling)
{
    ...
    auto distribution = model->PredictDistribution(FeatureVector({ FeatureValue(789) }));
    if (ordering_ == LearningTask::Ordering::kUnordered) {
        ...
        EXPECT_EQ(distribution.size(), 1u);
        EXPECT_EQ(distribution[example_1.target_value] + distribution[example_2.target_value], 1u);
    } else {
...
TEST_P(RandomTreeTest, NumericFeaturesSplitMultipleTimes)
{
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        TargetHistogram distribution = model->PredictDistribution(FeatureVector({ FeatureValue(i * feature_mult) }));
...
INSTANTIATE_TEST_SUITE_P(RandomTreeTest, RandomTreeTest, testing::ValuesIn({ LearningTask::Ordering::kUnordered, LearningTask::Ordering::kNumeric }));
