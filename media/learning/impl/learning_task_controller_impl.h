...
class COMPONENT_EXPORT(LEARNING_IMPL) LearningTaskControllerImpl final : public LearningTaskController, public HasRandomNumberGenerator {
public:
    LearningTaskControllerImpl(const LearningTask& task, std::unique_ptr<DistributionReporter> reporter = nullptr,
        SequenceBoundFeatureProvider feature_provider = SequenceBoundFeatureProvider());
...
private:
    ...
    void OnModelTrained(double training_weight, int training_size, std::unique_ptr<Model> model);
