...
public:
    ...
    const LearningTask& converted_task() const
    {
        return converted_task_;
    }
...
private:
    ...
    void ProcessOneFeature(size_t index, const LearningTask::ValueDescription& original_description, const TrainingData& training_data);
...
public:
    ConvertingModel(std::unique_ptr<OneHotConverter> converter, std::unique_ptr<Model> model);
