...
class COMPONENT_EXPORT(LEARNING_IMPL) RandomTreeTrainer : public TrainingAlgorithm, public HasRandomNumberGenerator {
public:
    ...
    void Train(const LearningTask& task, const TrainingData& examples, TrainedModelCB model_cb) override;

private:
    // Train on the subset |training_idx|.
    std::unique_ptr<Model> Train(const LearningTask& task, const TrainingData& examples, const std::vector<size_t>& training_idx);
    ...
    std::unique_ptr<Model> Build(
        const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, const FeatureSet& used_set);

    // Compute and return a split of |training_data| on the |index|-th feature.
    Split ConstructSplit(const LearningTask& task, const TrainingData& training_data, const std::vector<size_t>& training_idx, int index);
    ...
    FeatureValue FindSplitPoint_Nominal(size_t index, const TrainingData& training_data, const std::vector<size_t>& training_idx);

    // Compute the split point for |training_data| for a numeric feature.
    FeatureValue FindSplitPoint_Numeric(size_t index, const TrainingData& training_data, const std::vector<size_t>& training_idx);
};
