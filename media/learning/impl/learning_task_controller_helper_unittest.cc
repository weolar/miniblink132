...
public:
    ...
    public:
        FakeFeatureProvider(FeatureVector* features_out, FeatureProvider::FeatureVectorCB* cb_out)
    ...
    void CreateClient(bool include_fp)
    {
        ...
        if (include_fp) {
            sb_fp = base::SequenceBound<FakeFeatureProvider>(task_runner_, &fp_features_, &fp_cb_);
            task_environment_.RunUntilIdle();
        }

        // TODO(liberato): make sure this works without a fp.
        helper_ = std::make_unique<LearningTaskControllerHelper>(
            task_, base::BindRepeating(&LearningTaskControllerHelperTest::OnLabelledExample, base::Unretained(this)), std::move(sb_fp));
    }
...
TEST_F(LearningTaskControllerHelperTest, AddingAnExampleWithoutFPWorks)
{
    ...
    helper_->CompleteObservation(id_, ObservationCompletion(example_.target_value, example_.weight));
