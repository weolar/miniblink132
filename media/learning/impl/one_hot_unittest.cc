...
{
    LearningTask empty_task("EmptyTask", LearningTask::Model::kExtraTrees, {}, LearningTask::ValueDescription({ "target" }));
...
{
    LearningTask task(
        "SimpleTask", LearningTask::Model::kExtraTrees, { { "feature1", LearningTask::Ordering::kUnordered } }, LearningTask::ValueDescription({ "target" }));
    ...
    EXPECT_EQ(one_hot.converted_task().feature_descriptions.size(), adjusted_feature_size);
    EXPECT_EQ(one_hot.converted_task().feature_descriptions[0].ordering, LearningTask::Ordering::kNumeric);
    EXPECT_EQ(one_hot.converted_task().feature_descriptions[1].ordering, LearningTask::Ordering::kNumeric);
    EXPECT_EQ(one_hot.converted_task().feature_descriptions[2].ordering, LearningTask::Ordering::kNumeric);
    ...
    for (size_t i = 0; i < converted_training_data.size(); i++) {
        EXPECT_EQ(
            converted_training_data[i].features[0].value() + converted_training_data[i].features[1].value() + converted_training_data[i].features[2].value(),
            1);
    }
    ...
    for (size_t f = 0; f < adjusted_feature_size; f++) {
        EXPECT_EQ(converted_training_data[1].features[f], converted_training_data[3].features[f]);
    }

    // Converting each feature vector should result in the same one as before.
    for (size_t f = 0; f < adjusted_feature_size; f++) {
        FeatureVector converted_feature_vector = one_hot.Convert(training_data[f].features);
...
{
    LearningTask task(
        "SimpleTask", LearningTask::Model::kExtraTrees, { { "feature1", LearningTask::Ordering::kNumeric } }, LearningTask::ValueDescription({ "target" }));
    ...
    FeatureVector converted_feature_vector = one_hot.Convert(training_data[0].features);
...
TEST_F(OneHotTest, UnknownValuesAreZeroHot)
{
    ...
    FeatureVector converted_feature_vector = one_hot.Convert(FeatureVector({ FeatureValue("jkl") }));
