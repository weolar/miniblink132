...
void ExtraTreesTrainer::Train(const LearningTask& task, const TrainingData& training_data, TrainedModelCB model_cb)
{
...
void ExtraTreesTrainer::OnRandomTreeModel(TrainedModelCB model_cb, std::unique_ptr<Model> model)
{
    ...
    if (trees_.size() == task_.rf_number_of_trees) {
        std::unique_ptr<Model> finished_model = std::make_unique<VotingEnsemble>(std::move(trees_));
        // If we have a converter, then wrap everything in a ConvertingModel.
        if (converter_) {
            finished_model = std::make_unique<ConvertingModel>(std::move(converter_), std::move(finished_model));
        }
    ...
    // Train the next tree.
    auto cb = base::BindOnce(&ExtraTreesTrainer::OnRandomTreeModel, weak_ptr_factory_.GetWeakPtr(), std::move(model_cb));
