...
public:
    ...
    {
        DCHECK_EQ(task().target_description.ordering, LearningTask::Ordering::kNumeric);
        ...
        if (predicted.total_counts() != 0) {
            ...
            DVLOG(2) << "Learning: " << task().name << ": predicted: " << predicted_smooth << " (" << predicted.Average() << ") observed: " << observed_smooth
                     << " (" << info.observed.value() << ")";
        } else {
            DVLOG(2) << "Learning: " << task().name << ": predicted: N/A observed: " << observed_smooth << " (" << info.observed.value() << ")";
        }

        constexpr int matrix_size = static_cast<int>(ConfusionMatrix::kMaxValue) + 1;
        ...
        if (feature_indices() && feature_indices()->size() != task().feature_descriptions.size()) {
            return;
        }
    }
};
...
public:
    ...
    int Bucketize(double value)
    {
        ...
        double scaled_value
            = ((output_max - output_min + 1) * (value - task().ukm_min_input_value)) / (task().ukm_max_input_value - task().ukm_min_input_value) + output_min;
        // Clip to [0, 100] and truncate to an integer.
        return std::clamp(static_cast<int>(scaled_value), output_min, output_max);
    }
};

std::unique_ptr<DistributionReporter> DistributionReporter::Create(const LearningTask& task)
{
    ...
    if (task.uma_hacky_aggregate_confusion_matrix || task.uma_hacky_by_training_weight_confusion_matrix || task.uma_hacky_by_feature_subset_confusion_matrix) {
...
Model::PredictionCB DistributionReporter::GetPredictionCallback(const PredictionInfo& info)
{
    return base::BindOnce(&DistributionReporter::OnPrediction, weak_factory_.GetWeakPtr(), info);
}

void DistributionReporter::SetFeatureSubset(const std::set<int>& feature_indices)
{
