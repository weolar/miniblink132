...
void LookupTableTrainer::Train(const LearningTask& task, const TrainingData& training_data, TrainedModelCB model_cb)
{
    std::unique_ptr<LookupTable> lookup_table = std::make_unique<LookupTable>(training_data);
