...
LearningTaskControllerHelper::LearningTaskControllerHelper(const LearningTask& task, AddExampleCB add_example_cb, SequenceBoundFeatureProvider feature_provider)
    : task_(task)
...
void LearningTaskControllerHelper::BeginObservation(base::UnguessableToken id, FeatureVector features, std::optional<ukm::SourceId> source_id)
{
    ...
    if (!feature_provider_.is_null()) {
        ...
        feature_provider_.AsyncCall(&FeatureProvider::AddFeatures)
            .WithArgs(std::move(features),
                base::BindOnce(&LearningTaskControllerHelper::OnFeaturesReadyTrampoline, task_runner_, weak_ptr_factory_.GetWeakPtr(), id));
    } else {
...
void LearningTaskControllerHelper::CompleteObservation(base::UnguessableToken id, const ObservationCompletion& completion)
{
...
void LearningTaskControllerHelper::CancelObservation(base::UnguessableToken id)
{
    auto iter = pending_examples_.find(id);
    if (iter == pending_examples_.end())
        return;
...
// static
void LearningTaskControllerHelper::OnFeaturesReadyTrampoline(scoped_refptr<base::SequencedTaskRunner> task_runner,
    base::WeakPtr<LearningTaskControllerHelper> weak_this, base::UnguessableToken id, FeatureVector features)
{
    // TODO(liberato): this would benefit from promises / deferred data.
    auto cb = base::BindOnce(&LearningTaskControllerHelper::OnFeaturesReady, std::move(weak_this), id, std::move(features));
...
void LearningTaskControllerHelper::OnFeaturesReady(base::UnguessableToken id, FeatureVector features)
{
...
void LearningTaskControllerHelper::ProcessExampleIfFinished(PendingExampleMap::iterator iter)
{
