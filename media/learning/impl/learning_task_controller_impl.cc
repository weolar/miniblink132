...
LearningTaskControllerImpl::LearningTaskControllerImpl(
    const LearningTask& task, std::unique_ptr<DistributionReporter> reporter, SequenceBoundFeatureProvider feature_provider)
    : task_(task)
    ...
    , expected_feature_count_(task_.feature_descriptions.size())
{
    ...
    helper_ = std::make_unique<LearningTaskControllerHelper>(
        task, base::BindRepeating(&LearningTaskControllerImpl::AddFinishedExample, weak_ptr_factory_.GetWeakPtr()), std::move(feature_provider));
...
void LearningTaskControllerImpl::BeginObservation(
    base::UnguessableToken id, const FeatureVector& features, const std::optional<TargetValue>& default_target, const std::optional<ukm::SourceId>& source_id)
{
...
void LearningTaskControllerImpl::CompleteObservation(base::UnguessableToken id, const ObservationCompletion& completion)
{
    if (!trainer_)
        return;
...
void LearningTaskControllerImpl::UpdateDefaultTarget(base::UnguessableToken id, const std::optional<TargetValue>& default_target)
{
    NOTREACHED();
}
...
void LearningTaskControllerImpl::PredictDistribution(const FeatureVector& features, PredictionCB callback)
{
    if (model_)
...
void LearningTaskControllerImpl::AddFinishedExample(LabelledExample example, ukm::SourceId source_id)
{
    ...
    TrainedModelCB model_cb
        = base::BindOnce(&LearningTaskControllerImpl::OnModelTrained, weak_ptr_factory_.GetWeakPtr(), training_data_->total_weight(), training_data_->size());
...
void LearningTaskControllerImpl::OnModelTrained(double training_weight, int training_size, std::unique_ptr<Model> model)
{
...
void LearningTaskControllerImpl::SetTrainerForTesting(std::unique_ptr<TrainingAlgorithm> trainer)
{
