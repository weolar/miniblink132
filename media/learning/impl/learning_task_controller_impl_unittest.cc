...
public:
    ...
    protected:
        void OnPrediction(const PredictionInfo& info, TargetHistogram predicted) override
        {
            num_reported_++;
    ...
    public:
        ...
        void Train(const LearningTask& task, const TrainingData& training_data, TrainedModelCB model_cb) override
        {
            task_ = task;
            (*num_models_)++;
        ...
        const LearningTask& task() const
        {
            return task_;
        }

        const TrainingData& training_data() const
        {
    ...
    LearningTaskControllerImplTest()
        : predicted_target_(123)
        , not_predicted_target_(456)
    {
    ...
    void CreateController(SequenceBoundFeatureProvider feature_provider = SequenceBoundFeatureProvider())
    {
        std::unique_ptr<FakeDistributionReporter> reporter = std::make_unique<FakeDistributionReporter>(task_);
        reporter_raw_ = reporter.get();

        controller_ = std::make_unique<LearningTaskControllerImpl>(task_, std::move(reporter), std::move(feature_provider));

        auto fake_trainer = std::make_unique<FakeTrainer>(&num_models_, predicted_target_);
        trainer_raw_ = fake_trainer.get();
        controller_->SetTrainerForTesting(std::move(fake_trainer));
    }

    void AddExample(const LabelledExample& example, std::optional<ukm::SourceId> source_id = std::nullopt)
    {
        base::UnguessableToken id = base::UnguessableToken::Create();
        controller_->BeginObservation(id, example.features, std::nullopt, source_id);
        controller_->CompleteObservation(id, ObservationCompletion(example.target_value, example.weight));
    }

    void VerifyPrediction(const FeatureVector& features, std::optional<TargetHistogram> expectation)
    {
        std::optional<TargetHistogram> observed_prediction;
        controller_->PredictDistribution(features,
            base::BindOnce([](std::optional<TargetHistogram>* test_storage, const std::optional<TargetHistogram>& predicted) { *test_storage = predicted; },
                &observed_prediction));
        task_environment_.RunUntilIdle();
...
TEST_F(LearningTaskControllerImplTest, FeatureProviderIsUsed)
{
    ...
    SequenceBoundFeatureProvider feature_provider = base::SequenceBound<FakeFeatureProvider>(base::SequencedTaskRunner::GetCurrentDefault());
...
TEST_F(LearningTaskControllerImplTest, FeatureSubsetsWork)
{
    ...
    for (auto& iter : subset) {
        ...
        for (size_t i = 0; i < subset_size; i++) {
            if (trainer_raw_->task().feature_descriptions[i].name == feature_names[iter]) {
...
TEST_F(LearningTaskControllerImplTest, SourceIdIsProvidedToDistributionReporter)
{
    CreateController();
    LabelledExample example;
