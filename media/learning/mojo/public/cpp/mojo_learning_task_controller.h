...
class COMPONENT_EXPORT(MEDIA_LEARNING_MOJO) MojoLearningTaskController : public LearningTaskController {
public:
    // |task| will be provided by GetLearningTask().  Hopefully, it matches
    // whatever |controller| uses.
    MojoLearningTaskController(const LearningTask& task, mojo::Remote<mojom::LearningTaskController> controller);

    MojoLearningTaskController(const MojoLearningTaskController&) = delete;
    MojoLearningTaskController& operator=(const MojoLearningTaskController&) = delete;
