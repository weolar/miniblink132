...
public:
    ...
    public:
        void BeginObservation(const base::UnguessableToken& id, const FeatureVector& features, const std::optional<TargetValue>& default_target) override
        {
            begin_args_.id_ = id;
        ...
        void CompleteObservation(const base::UnguessableToken& id, const ObservationCompletion& completion) override
        {
        ...
        void UpdateDefaultTarget(const base::UnguessableToken& id, const std::optional<TargetValue>& default_target) override
        {
            update_default_args_.id_ = id;
            update_default_args_.default_target_ = default_target;
        }

        void PredictDistribution(const FeatureVector& features, PredictDistributionCallback callback) override
        {
...
public:
    ...
    void SetUp() override
    {
        ...
        learning_controller_ = std::make_unique<MojoLearningTaskController>(task_, std::move(remote));
    }
...
TEST_F(MojoLearningTaskControllerTest, BeginWithoutDefaultTarget)
{
    ...
    learning_controller_->BeginObservation(id, features, std::nullopt, std::nullopt);
...
TEST_F(MojoLearningTaskControllerTest, BeginWithDefaultTarget)
{
    ...
    FeatureVector features = { FeatureValue(123), FeatureValue(456) };
    learning_controller_->BeginObservation(id, features, default_target, std::nullopt);
    task_environment_.RunUntilIdle();
    EXPECT_EQ(id, fake_learning_controller_.begin_args_.id_);
    EXPECT_EQ(features, fake_learning_controller_.begin_args_.features_);
    EXPECT_EQ(default_target, fake_learning_controller_.begin_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerTest, UpdateDefaultTargetToValue)
{
    ...
    EXPECT_EQ(features, fake_learning_controller_.begin_args_.features_);
    EXPECT_EQ(default_target, fake_learning_controller_.update_default_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerTest, UpdateDefaultTargetToNoValue)
{
    ...
    EXPECT_EQ(id, fake_learning_controller_.update_default_args_.id_);
    EXPECT_EQ(features, fake_learning_controller_.begin_args_.features_);
    EXPECT_EQ(std::nullopt, fake_learning_controller_.update_default_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerTest, Complete)
{
    ...
    EXPECT_EQ(completion.target_value, fake_learning_controller_.complete_args_.completion_.target_value);
}
...
TEST_F(MojoLearningTaskControllerTest, PredictDistribution)
{
    ...
    learning_controller_->PredictDistribution(features,
        base::BindOnce(
    ...
    std::move(fake_learning_controller_.predict_args_.callback_).Run(expected_prediction);
    task_environment_.RunUntilIdle();
