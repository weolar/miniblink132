...
MojoLearningTaskControllerService::~MojoLearningTaskControllerService() = default;

void MojoLearningTaskControllerService::BeginObservation(
    const base::UnguessableToken& id, const FeatureVector& features, const std::optional<TargetValue>& default_target)
{
...
void MojoLearningTaskControllerService::CompleteObservation(const base::UnguessableToken& id, const ObservationCompletion& completion)
{
...
void MojoLearningTaskControllerService::CancelObservation(const base::UnguessableToken& id)
{
    auto iter = in_flight_observations_.find(id);
    if (iter == in_flight_observations_.end())
        return;
...
void MojoLearningTaskControllerService::UpdateDefaultTarget(const base::UnguessableToken& id, const std::optional<TargetValue>& default_target)
{
    auto iter = in_flight_observations_.find(id);
    if (iter == in_flight_observations_.end())
        return;
...
void MojoLearningTaskControllerService::PredictDistribution(const FeatureVector& features, PredictDistributionCallback callback)
{
