...
public:
    ...
    void SetUp() override
    {
        std::unique_ptr<FakeLearningTaskController> controller = std::make_unique<FakeLearningTaskController>();
        ...
        service_ = std::make_unique<MojoLearningTaskControllerService>(task_, kSourceId, std::move(controller));
    }
...
TEST_F(MojoLearningTaskControllerServiceTest, BeginComplete)
{
    ...
    EXPECT_EQ(completion.target_value, controller_raw_->complete_args_.completion_.target_value);
}
...
TEST_F(MojoLearningTaskControllerServiceTest, TooManyFeaturesIsIgnored)
{
    ...
    base::UnguessableToken id = base::UnguessableToken::Create();
    FeatureVector long_features = { FeatureValue(123), FeatureValue(456), FeatureValue(789) };
...
TEST_F(MojoLearningTaskControllerServiceTest, UpdateDefaultTargetToValue)
{
    ...
    EXPECT_EQ(default_target, controller_raw_->update_default_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerServiceTest, UpdateDefaultTargetToNoValue)
{
    ...
    EXPECT_EQ(id, controller_raw_->update_default_args_.id_);
    EXPECT_EQ(std::nullopt, controller_raw_->update_default_args_.default_target_);
}
...
TEST_F(MojoLearningTaskControllerServiceTest, PredictDistribution)
{
    ...
    service_->PredictDistribution(features,
        base::BindOnce(
    ...
    std::move(controller_raw_->predict_distribution_args_.callback_).Run(expected_prediction);
