...
#define YUV_ORDER(y, y_stride, u, u_stride, v, v_stride) (y), (y_stride), (u), (u_stride), (v), (v_stride)
#define GBR_TO_RGB_ORDER(y, y_stride, u, u_stride, v, v_stride) (v), (v_stride), (y), (y_stride), (u), (u_stride)
...
#define YUV_ORDER(y, y_stride, u, u_stride, v, v_stride) (y), (y_stride), (v), (v_stride), (u), (u_stride)
#define GBR_TO_RGB_ORDER(y, y_stride, u, u_stride, v, v_stride) (u), (u_stride), (y), (y_stride), (v), (v_stride)
...
public:
    ...
    ScopedSharedImageAccess(gpu::raster::RasterInterface* ri, GLuint texture, GLenum access = GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM)
        : ri(ri)
        , texture(texture)
    {
...
sk_sp<SkImage> WrapGLTexture(GLuint texture_id, const gfx::Size& size, viz::RasterContextProvider* raster_context_provider)
{
    ...
    auto backend_texture = GrBackendTextures::MakeGL(size.width(), size.height(), skgpu::Mipmapped::kNo, texture_info);
    return SkImages::AdoptTextureFrom(
        raster_context_provider->GrContext(), backend_texture, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
}

void BindAndTexImage2D(gpu::gles2::GLES2Interface* gl, unsigned int target, unsigned int texture, unsigned int internal_format, unsigned int format,
    unsigned int type, int level, const gfx::Size& size)
{
    gl->BindTexture(target, texture);
    gl->TexImage2D(target, level, internal_format, size.width(), size.height(), 0, format, type, nullptr);
}

gpu::SyncToken CopySharedImageToTexture(gpu::gles2::GLES2Interface* gl, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    gpu::ClientSharedImage* source_shared_image, const gpu::SyncToken& source_sync_token, unsigned int target, unsigned int texture,
    unsigned int internal_format, unsigned int format, unsigned int type, int level, bool premultiply_alpha, bool flip_y)
{
    auto si_texture = source_shared_image->CreateGLTexture(gl);
    auto scoped_si_access = si_texture->BeginAccess(source_sync_token, /*readonly=*/true);
    ...
    if (visible_rect != gfx::Rect(coded_size)) {
        ...
        BindAndTexImage2D(gl, target, texture, internal_format, format, type, level, visible_rect.size());
        gl->CopySubTextureCHROMIUM(scoped_si_access->texture_id(), 0, target, texture, level, 0, 0, visible_rect.x(), visible_rect.y(), visible_rect.width(),
            visible_rect.height(), flip_y, premultiply_alpha, false);

    } else {
        gl->CopyTextureCHROMIUM(scoped_si_access->texture_id(), 0, target, texture, level, internal_format, type, flip_y, premultiply_alpha, false);
    }
    return gpu::SharedImageTexture::ScopedAccess::EndAccess(std::move(scoped_si_access));
}
...
void SynchronizeVideoFrameRead(scoped_refptr<VideoFrame> video_frame, gpu::raster::RasterInterface* ri, gpu::ContextSupport* context_support)
{
    ...
    auto on_query_done_cb
        = base::BindOnce([](scoped_refptr<VideoFrame> video_frame, gpu::raster::RasterInterface* ri, unsigned query_id) { ri->DeleteQueriesEXT(1, &query_id); },
            video_frame, ri, query_id);
    context_support->SignalQuery(query_id, std::move(on_query_done_cb));
}

void SynchronizeVideoFrameRead(scoped_refptr<VideoFrame> video_frame, gpu::gles2::GLES2Interface* gl, gpu::ContextSupport* context_support)
{
...
libyuv::FilterMode ToLibyuvFilterMode(PaintCanvasVideoRenderer::FilterMode filter)
{
    switch (filter) {
...
size_t NumConvertVideoFrameToRGBPixelsTasks(const VideoFrame* video_frame)
{
    ...
    const size_t frame_size = VideoFrame::AllocationSize(video_frame->format(), video_frame->visible_rect().size());
    const size_t n_tasks = std::max<size_t>(1, frame_size / kTaskBytes);
    return std::min<size_t>(n_tasks, base::SysInfo::NumberOfProcessors());
}

void ConvertVideoFrameToRGBPixelsTask(const VideoFrame* video_frame, void* rgb_pixels, size_t row_bytes, bool premultiply_alpha, libyuv::FilterMode filter,
    size_t task_index, size_t n_tasks, base::RepeatingClosure* done)
{
    ...
    for (size_t plane = 0; plane < VideoFrame::kMaxPlanes; ++plane) {
        if (VideoFrame::IsValidPlane(format, plane)) {
            rows_per_chunk = std::lcm(rows_per_chunk, VideoFrame::SampleSize(format, plane).height());
        }
    }
    ...
    for (size_t plane = 0; plane < VideoFrame::kMaxPlanes; ++plane) {
        if (VideoFrame::IsValidPlane(format, plane)) {
            plane_meta[plane] = { // Note: Unlike |data|, stride does not need to be adjusted by the
                ...
                .data = video_frame->visible_data(plane)
                    + video_frame->stride(plane) * (chunk_start * rows_per_chunk) / VideoFrame::SampleSize(format, plane).height()
            };
        } else {
    ...
    uint8_t* pixels = static_cast<uint8_t*>(rgb_pixels) + row_bytes * chunk_start * rows_per_chunk;

    if (format == PIXEL_FORMAT_ARGB || format == PIXEL_FORMAT_XRGB || format == PIXEL_FORMAT_ABGR || format == PIXEL_FORMAT_XBGR) {
        ...
        if ((OUTPUT_ARGB && (format == PIXEL_FORMAT_ARGB || format == PIXEL_FORMAT_XRGB))
            || (!OUTPUT_ARGB && (format == PIXEL_FORMAT_ABGR || format == PIXEL_FORMAT_XBGR))) {
            uint8_t* dest = pixels;
            ...
            LIBYUV_ABGR_TO_ARGB(plane_meta[VideoFrame::Plane::kARGB].data, plane_meta[VideoFrame::Plane::kARGB].stride, pixels, row_bytes, width, rows);
        }
        ...
        // swap is performed).
        if (premultiply_alpha && (format == PIXEL_FORMAT_ARGB || format == PIXEL_FORMAT_ABGR)) {
    ...
    const libyuv::YuvConstants* matrix = GetYuvContantsForColorSpace(yuv_cs, OUTPUT_ARGB);

    if (!video_frame->data(VideoFrame::Plane::kU) && !video_frame->data(VideoFrame::Plane::kV)) {
        DCHECK_EQ(format, PIXEL_FORMAT_I420);
        // For monochrome content ARGB and ABGR are interchangeable.
        libyuv::I400ToARGBMatrix(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, pixels, row_bytes, matrix, width, rows);
        done->Run();
        return;
    }

    auto convert_yuv = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kU].data,
                 plane_meta[VideoFrame::Plane::kU].stride, plane_meta[VideoFrame::Plane::kV].data, plane_meta[VideoFrame::Plane::kV].stride),
            pixels, row_bytes, matrix, width, rows);
    };

    auto convert_yuv_with_filter = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kU].data,
                 plane_meta[VideoFrame::Plane::kU].stride, plane_meta[VideoFrame::Plane::kV].data, plane_meta[VideoFrame::Plane::kV].stride),
    ...
    auto convert_yuva = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kU].data,
                 plane_meta[VideoFrame::Plane::kU].stride, plane_meta[VideoFrame::Plane::kV].data, plane_meta[VideoFrame::Plane::kV].stride),
            plane_meta[VideoFrame::Plane::kA].data, plane_meta[VideoFrame::Plane::kA].stride, pixels, row_bytes, matrix, width, rows, premultiply_alpha);
    };

    auto convert_yuva_with_filter = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kU].data,
                 plane_meta[VideoFrame::Plane::kU].stride, plane_meta[VideoFrame::Plane::kV].data, plane_meta[VideoFrame::Plane::kV].stride),
            plane_meta[VideoFrame::Plane::kA].data, plane_meta[VideoFrame::Plane::kA].stride, pixels, row_bytes, matrix, width, rows, premultiply_alpha,
            filter);
    ...
    auto convert_yuv16 = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kY].data.get()), plane_meta[VideoFrame::Plane::kY].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kU].data.get()), plane_meta[VideoFrame::Plane::kU].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kV].data.get()), plane_meta[VideoFrame::Plane::kV].stride / 2),
            pixels, row_bytes, matrix, width, rows);
    };

    auto convert_yuv16_with_filter = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kY].data.get()), plane_meta[VideoFrame::Plane::kY].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kU].data.get()), plane_meta[VideoFrame::Plane::kU].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kV].data.get()), plane_meta[VideoFrame::Plane::kV].stride / 2),
            pixels, row_bytes, matrix, width, rows, filter);
    ...
    auto convert_yuva16 = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kY].data.get()), plane_meta[VideoFrame::Plane::kY].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kU].data.get()), plane_meta[VideoFrame::Plane::kU].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kV].data.get()), plane_meta[VideoFrame::Plane::kV].stride / 2),
            reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kA].data.get()), plane_meta[VideoFrame::Plane::kA].stride / 2, pixels, row_bytes,
            matrix, width, rows, premultiply_alpha);
    };

    auto convert_yuva16_with_filter = [&](const libyuv::YuvConstants* matrix, auto&& func) {
        func(YUV_ORDER(reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kY].data.get()), plane_meta[VideoFrame::Plane::kY].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kU].data.get()), plane_meta[VideoFrame::Plane::kU].stride / 2,
                 reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kV].data.get()), plane_meta[VideoFrame::Plane::kV].stride / 2),
            reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kA].data.get()), plane_meta[VideoFrame::Plane::kA].stride / 2, pixels, row_bytes,
    ...
    case PIXEL_FORMAT_I444:
        ...
        if (video_frame->ColorSpace().GetMatrixID() == gfx::ColorSpace::MatrixID::GBR) {
            libyuv::MergeARGBPlane(
                GBR_TO_RGB_ORDER(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kU].data,
                    plane_meta[VideoFrame::Plane::kU].stride, plane_meta[VideoFrame::Plane::kV].data, plane_meta[VideoFrame::Plane::kV].stride),
                plane_meta[VideoFrame::Plane::kA].data, plane_meta[VideoFrame::Plane::kA].stride, pixels, row_bytes, width, rows);
        } else {
    ...
    case PIXEL_FORMAT_NV12:
        LIBYUV_NV12_TO_ARGB_MATRIX(plane_meta[VideoFrame::Plane::kY].data, plane_meta[VideoFrame::Plane::kY].stride, plane_meta[VideoFrame::Plane::kUV].data,
            plane_meta[VideoFrame::Plane::kUV].stride, pixels, row_bytes, matrix, width, rows);
        break;
    case PIXEL_FORMAT_P010LE:
        libyuv::P010ToARGBMatrix(reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kY].data.get()), plane_meta[VideoFrame::Plane::kY].stride,
            reinterpret_cast<const uint16_t*>(plane_meta[VideoFrame::Plane::kUV].data.get()), plane_meta[VideoFrame::Plane::kUV].stride, pixels, row_bytes,
            matrix, width, rows);
        if (!OUTPUT_ARGB)
    ...
    case PIXEL_FORMAT_Y16:
        NOTREACHED() << "These cases should be handled in ConvertVideoFrameToRGBPixels";
    ...
    case PIXEL_FORMAT_UNKNOWN:
        NOTREACHED() << "Only YUV formats and Y16 are supported, got: " << media::VideoPixelFormatToString(format);
    }
    done->Run();
}
...
BASE_FEATURE(kOneCopyUploadOfVideoFrameToGLTexture, "OneCopyUploadOfVideoFrameToGLTexture", base::FEATURE_ENABLED_BY_DEFAULT);
#endif // BUILDFLAG(IS_ANDROID)

std::tuple<SkYUVAInfo::PlaneConfig, SkYUVAInfo::Subsampling> VideoPixelFormatAsSkYUVAInfoValues(VideoPixelFormat format)
{
    ...
    default:
        return { SkYUVAInfo::PlaneConfig::kUnknown, SkYUVAInfo::Subsampling::kUnknown };
    }
}

// Checks support before attempting one copy upload to GL texture.
bool SupportsOneCopyUploadToGLTexture(VideoPixelFormat video_frame_format, uint32_t shared_image_target, unsigned int dst_target,
    unsigned int dst_internal_format, unsigned int dst_type, int dst_level, bool premultiply_alpha)
{
    ...
    bool use_one_copy_upload = base::FeatureList::IsEnabled(kOneCopyUploadOfVideoFrameToGLTexture);
    bool supports_one_copy_format = ValidFormatForDirectUploading(static_cast<GLenum>(dst_internal_format), dst_type);
    // dst texture mipLevel must be 0.
    // TODO(crbug.com/40141173): Support more texture target, e.g.
    // 2d array, 3d etc.
    return si_usable_by_gles2_interface && dst_level == 0 && is_premul && use_one_copy_upload && dst_target == GL_TEXTURE_2D && supports_one_copy_format;
#endif // BUILDFLAG(IS_ANDROID)
}
...
public:
    ...
    VideoImageGenerator(scoped_refptr<VideoFrame> frame)
        : cc::PaintImageGenerator(
            SkImageInfo::MakeN32Premul(frame->visible_rect().width(), frame->visible_rect().height(), frame->CompatRGBColorSpace().ToSkColorSpace()))
    ...
    bool GetPixels(SkPixmap dst_pixmap, size_t frame_index, cc::PaintImage::GeneratorClientId client_id, uint32_t lazy_pixel_ref) override
    {
        DCHECK_EQ(frame_index, 0u);

        // If skia couldn't do the YUV conversion on GPU, we will on CPU.
        PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(frame_.get(), dst_pixmap.writable_addr(), dst_pixmap.rowBytes());

        if (!SkColorSpace::Equals(GetSkImageInfo().colorSpace(), dst_pixmap.colorSpace())) {
            SkPixmap src(GetSkImageInfo(), dst_pixmap.writable_addr(), dst_pixmap.rowBytes());
    ...
    bool QueryYUVA(const SkYUVAPixmapInfo::SupportedDataTypes&, SkYUVAPixmapInfo* info) const override
    {
        SkYUVAInfo::PlaneConfig plane_config;
        SkYUVAInfo::Subsampling subsampling;
        std::tie(plane_config, subsampling) = VideoPixelFormatAsSkYUVAInfoValues(frame_->format());
        if (plane_config == SkYUVAInfo::PlaneConfig::kUnknown)
            return false;

        // Don't use the YUV conversion path for multi-plane RGB frames.
        if (frame_->format() == PIXEL_FORMAT_I444 && frame_->ColorSpace().GetMatrixID() == gfx::ColorSpace::MatrixID::GBR) {
            return false;
        }
        ...
        if (!frame_->ColorSpace().ToSkYUVColorSpace(frame_->BitDepth(), &yuv_color_space)) {
            // TODO(crbug.com/41380578): This should default to BT.709 color space.
            yuv_color_space = kRec601_SkYUVColorSpace;
        }

        auto yuva_info = SkYUVAInfo({ frame_->visible_rect().width(), frame_->visible_rect().height() }, plane_config, subsampling, yuv_color_space);
        *info = SkYUVAPixmapInfo(yuva_info, SkYUVAPixmapInfo::DataType::kUnorm8,
            /*rowBytes=*/nullptr);
        return true;
    }

    bool GetYUVAPlanes(const SkYUVAPixmaps& pixmaps, size_t frame_index, uint32_t lazy_pixel_ref, cc::PaintImage::GeneratorClientId client_id) override
    {
        DCHECK_EQ(frame_index, 0u);
        ...
        for (int plane = VideoFrame::Plane::kY; plane <= VideoFrame::Plane::kV; ++plane) {
            const auto plane_size = VideoFrame::PlaneSizeInSamples(frame_->format(), plane, frame_->visible_rect().size());
            if (plane_size.width() != pixmaps.plane(plane).width() || plane_size.height() != pixmaps.plane(plane).height()) {
                return false;
            }
            ...
            libyuv::CopyPlane(frame_->visible_data(plane), frame_->stride(plane), reinterpret_cast<uint8_t*>(out_plane.writable_addr()), out_plane.rowBytes(),
...
public:
    explicit VideoTextureBacking(sk_sp<SkImage> sk_image, scoped_refptr<gpu::ClientSharedImage> shared_image,
        scoped_refptr<viz::RasterContextProvider> raster_context_provider, std::unique_ptr<ScopedSharedImageAccess> access)
    ...
    explicit VideoTextureBacking(
        scoped_refptr<gpu::ClientSharedImage> shared_image, const SkImageInfo& info, scoped_refptr<viz::RasterContextProvider> raster_context_provider)
        : sk_image_info_(info)
        , shared_image_(std::move(shared_image))
    {
        CHECK(shared_image_);
    ...
    const scoped_refptr<viz::RasterContextProvider>& raster_context_provider() const
    {
    ...
    void ReplaceAcceleratedSkImage(sk_sp<SkImage> sk_image, std::unique_ptr<ScopedSharedImageAccess> access)
    {
    ...
    {
        sk_sp<SkData> image_pixels = SkData::MakeUninitialized(sk_image_info_.computeMinByteSize());
        if (!readPixels(sk_image_info_, image_pixels->writable_data(), sk_image_info_.minRowBytes(), 0, 0)) {
            DLOG(ERROR) << "VideoTextureBacking::GetSkImageViaReadback failed.";
            return nullptr;
        }
        return SkImages::RasterFromData(sk_image_info_, std::move(image_pixels), sk_image_info_.minRowBytes());
    }

    bool readPixels(const SkImageInfo& dst_info, void* dst_pixels, size_t dst_row_bytes, int src_x, int src_y) override
    {
        gpu::raster::RasterInterface* ri = raster_context_provider_->RasterInterface();
        if (sk_image_) {
            GrGLTextureInfo texture_info;
            GrBackendTexture texture;
            if (!SkImages::GetBackendTextureFromImage(sk_image_, &texture,
            ...
            return sk_image_->readPixels(dst_info, dst_pixels, dst_row_bytes, src_x, src_y);
        }
        return ri->ReadbackImagePixels(shared_image_->mailbox(), dst_info, dst_info.minRowBytes(), src_x, src_y,
...
PaintCanvasVideoRenderer::PaintCanvasVideoRenderer()
    : cache_deleting_timer_(FROM_HERE, base::Seconds(kTemporaryResourceDeletionDelay), this, &PaintCanvasVideoRenderer::ResetCache)
...
void PaintCanvasVideoRenderer::Paint(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, cc::PaintFlags& flags, const PaintParams& params,
    viz::RasterContextProvider* raster_context_provider)
{
    ...
    if (video_frame && video_frame->HasSharedImage()) {
        if (!raster_context_provider) {
            DLOG(ERROR) << "Can't render textured frames w/o viz::RasterContextProvider";
            return; // Unable to get/create a shared main thread context.
        }
        if (!raster_context_provider->GrContext() && !raster_context_provider->ContextCapabilities().gpu_rasterization) {
            DLOG(ERROR) << "Can't render textured frames w/o valid GrContext or GPU raster.";
            return; // The context has been lost.
        }
    }

    gfx::RectF dest_rect = params.dest_rect.value_or(video_frame ? gfx::RectF(gfx::SizeF(video_frame->visible_rect().size())) : gfx::RectF());
    SkRect dest;
    dest.setLTRB(dest_rect.x(), dest_rect.y(), dest_rect.right(), dest_rect.bottom());

    // Paint black rectangle if there isn't a frame available or the
    // frame has an unexpected format.
    if (!video_frame.get() || video_frame->natural_size().IsEmpty()
        || !(media::IsYuvPlanar(video_frame->format()) || video_frame->format() == PIXEL_FORMAT_Y16 || video_frame->format() == PIXEL_FORMAT_ARGB
            || video_frame->format() == PIXEL_FORMAT_XRGB || video_frame->format() == PIXEL_FORMAT_ABGR || video_frame->format() == PIXEL_FORMAT_XBGR
    ...
    if (params.reinterpret_as_srgb) {
        image = cc::PaintImageBuilder::WithCopy(image).set_reinterpret_as_srgb(true).TakePaintImage();
    }
    DCHECK(image);
    ...
    const bool need_scaling = dest_rect.size() != gfx::SizeF(video_frame->visible_rect().size());
    const bool need_translation = !dest_rect.origin().IsOrigin();
    const bool needs_mirror = params.transformation.mirrored;
    bool need_transform = need_rotation || need_scaling || need_translation || needs_mirror;
    if (need_transform) {
        canvas->save();
        canvas->translate(SkFloatToScalar(dest_rect.x() + (dest_rect.width() * 0.5f)), SkFloatToScalar(dest_rect.y() + (dest_rect.height() * 0.5f)));
        ...
        const bool has_flipped_size = params.transformation.rotation == VIDEO_ROTATION_90 || params.transformation.rotation == VIDEO_ROTATION_270;
        if (has_flipped_size) {
            rotated_dest_size = gfx::SizeF(rotated_dest_size.height(), rotated_dest_size.width());
        }
        auto sx = SkFloatToScalar(rotated_dest_size.width() / video_frame->visible_rect().width());
        auto sy = SkFloatToScalar(rotated_dest_size.height() / video_frame->visible_rect().height());
        if (needs_mirror) {
            if (has_flipped_size)
                sy *= -1;
            else
        ...
        canvas->scale(sx, sy);
        canvas->translate(-SkFloatToScalar(video_frame->visible_rect().width() * 0.5f), -SkFloatToScalar(video_frame->visible_rect().height() * 0.5f));
    }
    ...
    if (!need_transform && !params.reinterpret_as_srgb && video_frame->IsMappable() && flags.isOpaque() && flags.getBlendMode() == SkBlendMode::kSrc
        && flags.getFilterQuality() == cc::PaintFlags::FilterQuality::kLow && (pixels = canvas->accessTopLayerPixels(&info, &row_bytes, &origin))
        ...
        DCHECK_EQ(video_frame->coded_size(), gfx::Size(image.width(), image.height()));
        canvas->drawImageRect(image, gfx::RectToSkRect(video_frame->visible_rect()),
            SkRect::MakeWH(video_frame->visible_rect().width(), video_frame->visible_rect().height()),
            cc::PaintFlags::FilterQualityToSkSamplingOptions(flags.getFilterQuality()), &video_flags, SkCanvas::kStrict_SrcRectConstraint);
    } else {
        DCHECK_EQ(video_frame->visible_rect().size(), gfx::Size(image.width(), image.height()));
        canvas->drawImage(image, 0, 0, cc::PaintFlags::FilterQualityToSkSamplingOptions(flags.getFilterQuality()), &video_flags);
    }
...
void PaintCanvasVideoRenderer::Copy(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, viz::RasterContextProvider* raster_context_provider)
{
    cc::PaintFlags flags;
    flags.setBlendMode(SkBlendMode::kSrc);
    flags.setFilterQuality(cc::PaintFlags::FilterQuality::kLow);

    Paint(std::move(video_frame), canvas, flags, PaintParams(), raster_context_provider);
}
...
scoped_refptr<VideoFrame> DownShiftHighbitVideoFrame(const VideoFrame* video_frame)
{
    ...
    scoped_refptr<VideoFrame> ret
        = VideoFrame::CreateFrame(format, video_frame->coded_size(), video_frame->visible_rect(), video_frame->natural_size(), video_frame->timestamp());
    ...
    for (int plane = VideoFrame::Plane::kY; plane <= VideoFrame::Plane::kV; ++plane) {
        int width = VideoFrame::Columns(plane, video_frame->format(), video_frame->visible_rect().width());
        int height = VideoFrame::Rows(plane, video_frame->format(), video_frame->visible_rect().height());
        const uint16_t* src = reinterpret_cast<const uint16_t*>(video_frame->visible_data(plane));
        ...
        libyuv::Convert16To8Plane(src, video_frame->stride(plane) / 2, dst, ret->stride(plane), scale, width, height);
    }
    return ret;
}
...
void FlipAndConvertY16(const VideoFrame* video_frame, uint8_t* out, unsigned format, unsigned type, bool flip_y, size_t output_row_bytes)
{
    ...
    for (int i = 0; i < height; ++i, row_head += stride) {
        uint8_t* out_row_head = flip_y ? out + output_row_bytes * (height - i - 1) : out + output_row_bytes * i;
        ...
        NOTREACHED() << "Unsupported Y16 conversion for format: 0x" << std::hex << format << " and type: 0x" << std::hex << type;
    }
}
...
bool TexImageHelper(VideoFrame* frame, unsigned format, unsigned type, bool flip_y, scoped_refptr<DataBuffer>* temp_buffer)
{
    ...
    *temp_buffer = new DataBuffer(output_row_bytes * frame->visible_rect().height());
    FlipAndConvertY16(frame, (*temp_buffer)->writable_data(), format, type, flip_y, output_row_bytes);
    return true;
}
...
void TextureSubImageUsingIntermediate(unsigned target, unsigned texture, gpu::gles2::GLES2Interface* gl, VideoFrame* frame, int temp_internalformat,
    unsigned temp_format, unsigned temp_type, int level, int xoffset, int yoffset, bool flip_y, bool premultiply_alpha)
{
    ...
    gl->TexImage2D(
        target, 0, temp_internalformat, frame->visible_rect().width(), frame->visible_rect().height(), 0, temp_format, temp_type, frame->visible_data(0));
    gl->BindTexture(target, texture);
    gl->CopySubTextureCHROMIUM(temp_texture, 0, target, texture, level, 0, 0, xoffset, yoffset, frame->visible_rect().width(), frame->visible_rect().height(),
...
void PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(
    const VideoFrame* video_frame, void* rgb_pixels, size_t row_bytes, bool premultiply_alpha, FilterMode filter, bool disable_threading)
{
    ...
    base::RepeatingClosure barrier = base::BarrierClosure(n_tasks, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));

    const libyuv::FilterMode libyuv_filter = ToLibyuvFilterMode(filter);
    for (size_t i = 1; i < n_tasks; ++i) {
        base::ThreadPool::PostTask(FROM_HERE,
            base::BindOnce(ConvertVideoFrameToRGBPixelsTask, base::Unretained(video_frame), rgb_pixels, row_bytes, premultiply_alpha, libyuv_filter, i, n_tasks,
                &barrier));
    }
...
bool PaintCanvasVideoRenderer::CopyVideoFrameTexturesToGLTexture(viz::RasterContextProvider* raster_context_provider,
    gpu::gles2::GLES2Interface* destination_gl, scoped_refptr<VideoFrame> video_frame, unsigned int target, unsigned int texture, unsigned int internal_format,
    unsigned int format, unsigned int type, int level, bool premultiply_alpha, bool flip_y)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(video_frame);
    ...
    const bool si_format_has_single_texture = si_format.is_single_plane() || si_format.PrefersExternalSampler();
    const bool si_usable_by_gles2_interface = shared_image->GetTextureTarget() != 0;
    ...
    if (si_format_has_single_texture && si_usable_by_gles2_interface) {
        ...
        CopySharedImageToTexture(destination_gl, video_frame->coded_size(), video_frame->visible_rect(), shared_image.get(), video_frame->acquire_sync_token(),
            target, texture, internal_format, format, type, level, premultiply_alpha, flip_y);
        destination_gl->ShallowFlushCHROMIUM();

        SynchronizeVideoFrameRead(std::move(video_frame), destination_gl, raster_context_provider->ContextSupport());
        return true;
    }
    ...
    if (SupportsOneCopyUploadToGLTexture(video_frame->format(), shared_image->GetTextureTarget(), target, internal_format, type, level, premultiply_alpha)) {
        // Trigger resource allocation for dst texture to back SkSurface.
        // Dst texture size should equal to video frame visible rect.
        BindAndTexImage2D(destination_gl, target, texture, internal_format, format, type, /*level=*/0, video_frame->visible_rect().size());

        destination_gl->WaitSyncTokenCHROMIUM(video_frame->acquire_sync_token().GetConstData());

        // Copy shared image to gl texture for hardware video decode with
        // multiplanar shared image formats.
        destination_gl->CopySharedImageToTextureINTERNAL(texture, target, internal_format, type, video_frame->visible_rect().x(),
            video_frame->visible_rect().y(), video_frame->visible_rect().width(), video_frame->visible_rect().height(), flip_y, shared_image->mailbox().name);
    ...
    if (!gr_context && !raster_context_provider->ContextCapabilities().gpu_rasterization) {
        return false;
    }
    ...
    gpu::raster::RasterInterface* canvas_ri = raster_context_provider->RasterInterface();
    ...
    gpu::SyncToken dest_sync_token = CopySharedImageToTexture(destination_gl, cache_->coded_size, cache_->visible_rect,
        cache_->texture_backing->GetSharedImage().get(), sync_token, target, texture, internal_format, format, type, level, premultiply_alpha, flip_y);
...
bool PaintCanvasVideoRenderer::CopyVideoFrameYUVDataToGLTexture(viz::RasterContextProvider* raster_context_provider, gpu::gles2::GLES2Interface* destination_gl,
    scoped_refptr<VideoFrame> video_frame, unsigned int target, unsigned int texture, unsigned int internal_format, unsigned int format, unsigned int type,
    int level, bool premultiply_alpha, bool flip_y)
{
    if (!raster_context_provider)
        return false;
#if BUILDFLAG(IS_ANDROID)
    ...
    if (format_enum == GL_RGB10_A2 || format_enum == GL_RGB565 || format_enum == GL_RGB5_A1 || format_enum == GL_RGBA4) {
        return false;
    }
#endif
    ...
    gpu::raster::RasterInterface* source_ri = raster_context_provider->RasterInterface();

    // We need a shared image to receive the intermediate RGB result. Try to reuse
    // one if compatible, otherwise create a new one.
    gpu::SyncToken token;
    if (yuv_cache_.shared_image && yuv_cache_.size == video_frame->coded_size() && yuv_cache_.raster_context_provider == raster_context_provider) {
        ...
        gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_GLES2_READ;
        ...
        yuv_cache_.shared_image = sii->CreateSharedImage(
            { SHARED_IMAGE_FORMAT, video_frame->coded_size(), video_frame->CompatRGBColorSpace(), usage, "PaintCanvasVideoRenderer" }, gpu::kNullSurfaceHandle);
    ...
    yuv_cache_.yuv_converter.ConvertYUVVideoFrame(video_frame.get(), raster_context_provider, dest_holder);

    gpu::SyncToken post_conversion_sync_token;
    source_ri->GenUnverifiedSyncTokenCHROMIUM(post_conversion_sync_token.GetData());

    // On the destination GL context, do a copy (with cropping) into the
    // destination texture.
    yuv_cache_.sync_token = CopySharedImageToTexture(destination_gl, video_frame->coded_size(), video_frame->visible_rect(), yuv_cache_.shared_image.get(),
        post_conversion_sync_token, target, texture, internal_format, format, type, level, premultiply_alpha, flip_y);
...
bool PaintCanvasVideoRenderer::TexImage2D(unsigned target, unsigned texture, gpu::gles2::GLES2Interface* gl, const gpu::Capabilities& gpu_capabilities,
    VideoFrame* frame, int level, int internalformat, unsigned format, unsigned type, bool flip_y, bool premultiply_alpha)
{
    DCHECK(frame);
    ...
    GLint precision = 0;
    GLint range[2] = { 0, 0 };
    gl->GetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_MEDIUM_FLOAT, range, &precision);
    ...
    if (gpu_capabilities.texture_norm16 && precision > 15 && target == GL_TEXTURE_2D && (type == GL_FLOAT || type == GL_UNSIGNED_BYTE)) {
        ...
        gl->TexImage2D(target, level, internalformat, frame->visible_rect().width(), frame->visible_rect().height(), 0, format, type, nullptr);
        // We use sized internal format GL_R16_EXT instead of unsized GL_RED.
        // See angleproject:1952
        TextureSubImageUsingIntermediate(target, texture, gl, frame, GL_R16_EXT, GL_RED, GL_UNSIGNED_SHORT, level, 0, 0, flip_y, premultiply_alpha);
        return true;
    }
    ...
    gl->TexImage2D(target, level, internalformat, frame->visible_rect().width(), frame->visible_rect().height(), 0, format, type, temp_buffer->data());
    return true;
}

bool PaintCanvasVideoRenderer::TexSubImage2D(unsigned target, gpu::gles2::GLES2Interface* gl, VideoFrame* frame, int level, unsigned format, unsigned type,
    int xoffset, int yoffset, bool flip_y, bool premultiply_alpha)
{
    DCHECK(frame);
    DCHECK(!frame->HasSharedImage());
    DCHECK(frame->metadata().texture_origin_is_top_left);
    ...
    gl->TexSubImage2D(target, level, xoffset, yoffset, frame->visible_rect().width(), frame->visible_rect().height(), format, type, temp_buffer->data());
    return true;
}
...
bool PaintCanvasVideoRenderer::UpdateLastImage(scoped_refptr<VideoFrame> video_frame, viz::RasterContextProvider* raster_context_provider)
{
    // Check for a cache hit.
    if (cache_ && video_frame->unique_id() == cache_->frame_id && cache_->paint_image) {
        cache_deleting_timer_.Reset();
        return true;
    }

    auto paint_image_builder = cc::PaintImageBuilder::WithDefault()
    ...
    if (video_frame->HasSharedImage()) {
        ...
        bool gpu_rasterization = raster_context_provider->ContextCapabilities().gpu_rasterization;
        ...
        if (cache_ && cache_->texture_backing && cache_->texture_backing->raster_context_provider() == raster_context_provider
            ...
            client_shared_image = sii->CreateSharedImage(
                { SHARED_IMAGE_FORMAT, video_frame->coded_size(), video_frame->CompatRGBColorSpace(), flags, "PaintCanvasVideoRenderer" },
        ...
        ri->CopySharedImage(
            video_frame_si->mailbox(), client_shared_image->mailbox(), 0, 0, 0, 0, video_frame->coded_size().width(), video_frame->coded_size().height());
        ...
        // completed.
        SynchronizeVideoFrameRead(video_frame, ri, raster_context_provider->ContextSupport());
        ...
        if (!gpu_rasterization) {
            cache_->source_texture = ri->CreateAndConsumeForGpuRaster(client_shared_image->mailbox());

            auto access = std::make_unique<ScopedSharedImageAccess>(ri, cache_->source_texture);
            auto source_image = WrapGLTexture(cache_->source_texture, video_frame->coded_size(), raster_context_provider);
            if (!source_image) {
            ...
            if (!cache_->texture_backing) {
                cache_->texture_backing
                    = sk_make_sp<VideoTextureBacking>(std::move(source_image), std::move(client_shared_image), raster_context_provider, std::move(access));
            } else {
                cache_->texture_backing->ReplaceAcceleratedSkImage(std::move(source_image), std::move(access));
            }
        } else if (!cache_->texture_backing) {
            SkImageInfo sk_image_info = SkImageInfo::Make(
                gfx::SizeToSkISize(cache_->coded_size), kRGBA_8888_SkColorType, kPremul_SkAlphaType, video_frame->CompatRGBColorSpace().ToSkColorSpace());
            cache_->texture_backing = sk_make_sp<VideoTextureBacking>(std::move(client_shared_image), sk_image_info, raster_context_provider);
        }
        paint_image_builder.set_texture_backing(cache_->texture_backing, cc::PaintImage::GetNextContentId());
    } else {
        cache_.emplace(video_frame->unique_id());
        paint_image_builder.set_paint_image_generator(sk_make_sp<VideoImageGenerator>(video_frame));
    }
...
bool PaintCanvasVideoRenderer::CanUseCopyVideoFrameToSharedImage(const VideoFrame& video_frame)
{
    return video_frame.HasSharedImage() || VideoFrameYUVConverter::IsVideoFrameFormatSupported(video_frame);
}

gpu::SyncToken PaintCanvasVideoRenderer::CopyVideoFrameToSharedImage(
    viz::RasterContextProvider* raster_context_provider, scoped_refptr<VideoFrame> video_frame, const gpu::MailboxHolder& destination, bool use_visible_rect)
{
    auto* ri = raster_context_provider->RasterInterface();

    // If we have single source shared image, just use CopySharedImage().
    if (video_frame->HasSharedImage()) {
        auto source_rect = use_visible_rect ? video_frame->visible_rect() : gfx::Rect(video_frame->coded_size());
        ri->WaitSyncTokenCHROMIUM(video_frame->acquire_sync_token().GetConstData());
        ri->WaitSyncTokenCHROMIUM(destination.sync_token.GetConstData());
        ri->CopySharedImage(
            video_frame->shared_image()->mailbox(), destination.mailbox, 0, 0, source_rect.x(), source_rect.y(), source_rect.width(), source_rect.height());
    } else {
        // TODO(vasilyt): Add caching support
        VideoFrameYUVConverter converter;
        converter.ConvertYUVVideoFrame(video_frame.get(), raster_context_provider, destination, use_visible_rect);
    }
    ...
    if (video_frame->HasSharedImage()) {
        SynchronizeVideoFrameRead(std::move(video_frame), ri, raster_context_provider->ContextSupport());
    }
    return sync_token;
}
