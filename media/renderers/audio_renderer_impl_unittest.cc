...
constexpr double kOutputMicrosPerFrame = static_cast<double>(base::Time::kMicrosecondsPerSecond) / kOutputSamplesPerSecond;
...
class AudioRendererImplTest : public ::testing::Test, public RendererClient, public SpeechRecognitionClient {
public:
    std::vector<std::unique_ptr<AudioDecoder>> CreateAudioDecoderForTest()
    {
        ...
        if (!enter_pending_decoder_init_) {
            EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _))
                .WillOnce(
                    DoAll(SaveArg<3>(&output_cb_), RunOnceCallback<2>(expected_init_result_ ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed)));
        } else {
            EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _)).WillOnce(EnterPendingDecoderInitStateAction(this));
        }
        EXPECT_CALL(*decoder, Decode(_, _)).WillRepeatedly(Invoke(this, &AudioRendererImplTest::DecodeDecoder));
        EXPECT_CALL(*decoder, Reset_(_)).WillRepeatedly(Invoke(this, &AudioRendererImplTest::ResetDecoder));
    ...
    AudioRendererImplTest()
        : hardware_params_(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kOutputSamplesPerSecond, 512)
        ...
        , ended_(false)
    {
        AudioDecoderConfig audio_config(kCodec, kSampleFormat, kChannelLayout, kInputSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted);
        demuxer_stream_.set_audio_decoder_config(audio_config);

        ConfigureDemuxerStream(true);

        AudioParameters out_params(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kOutputSamplesPerSecond, 512);
        renderer_ = std::make_unique<AudioRendererImpl>(main_thread_task_runner_, sink_.get(),
            base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, this);
    ...
    {
        EXPECT_CALL(demuxer_stream_, OnRead(_)).WillRepeatedly(Invoke(this, &AudioRendererImplTest::OnDemuxerRead));
        EXPECT_CALL(demuxer_stream_, SupportsConfigChanges()).WillRepeatedly(Return(supports_config_changes));
    }
    ...
    bool IsDemuxerStalled()
    {
        return !!stalled_demixer_read_cb_;
    }
    ...
    void ConfigureBasicRenderer(const AudioParameters& params)
    {
        ...
        renderer_ = std::make_unique<AudioRendererImpl>(main_thread_task_runner_, sink_.get(),
            base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, nullptr);
    ...
    void ConfigureConfigChangeRenderer(const AudioParameters& params, const AudioParameters& hardware_params)
    {
    ...
    void ConfigureWithMockSink(const AudioParameters& params)
    {
        ...
        renderer_ = std::make_unique<AudioRendererImpl>(main_thread_task_runner_, mock_sink_.get(),
            base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, nullptr);
        testing::Mock::VerifyAndClearExpectations(&demuxer_stream_);
        ConfigureDemuxerStream(true);
    }
    ...
    void ConfigureWithMockMediaLog()
    {
        sink_ = base::MakeRefCounted<FakeAudioRendererSink>(hardware_params_);
        renderer_ = std::make_unique<AudioRendererImpl>(main_thread_task_runner_, sink_.get(),
            base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &mock_media_log_, 0, nullptr);
        testing::Mock::VerifyAndClearExpectations(&demuxer_stream_);
        ConfigureDemuxerStream(true);
    }
    ...
    {
        renderer_->EnableSpeechRecognition();
    }
    ...
    MOCK_METHOD2(OnBufferingStateChange, void(BufferingState, BufferingStateChangeReason));
    ...
    MOCK_METHOD3(AddAudioBusOnMainSequence, void(std::unique_ptr<AudioBus>, int, ChannelLayout));
    ...
    void InitializeRenderer(DemuxerStream* demuxer_stream, PipelineStatusCallback pipeline_status_cb)
    {
        ...
        renderer_->Initialize(demuxer_stream, nullptr, this, std::move(pipeline_status_cb));
    }

    void Initialize()
    {
        InitializeWithStatus(PIPELINE_OK);
        next_timestamp_ = std::make_unique<AudioTimestampHelper>(kInputSamplesPerSecond);
    }

    void InitializeBitstreamFormat()
    {
        EXPECT_CALL(media_client_, IsSupportedBitstreamAudioCodec(_)).WillRepeatedly(Return(true));
        SetMediaClient(&media_client_);

        hardware_params_.Reset(AudioParameters::AUDIO_BITSTREAM_EAC3, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kOutputSamplesPerSecond, 512);
        sink_ = base::MakeRefCounted<FakeAudioRendererSink>(hardware_params_);
        AudioDecoderConfig audio_config(
            AudioCodec::kAC3, kSampleFormatEac3, kChannelLayout, kInputSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted);
        demuxer_stream_.set_audio_decoder_config(audio_config);
    ...
    {
        SCOPED_TRACE(base::StringPrintf("InitializeWithStatus(%d)", expected.code()));
    ...
    void Preroll(base::TimeDelta start_timestamp, base::TimeDelta first_timestamp, PipelineStatus expected)
    {
        SCOPED_TRACE(base::StringPrintf("Preroll(%" PRId64 ", %d)", first_timestamp.InMilliseconds(), expected.code()));
        ...
        WaitForPendingRead();
        EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    ...
    void SatisfyPendingRead(InputFrames frames)
    {
        ...
        if (hardware_params_.IsBitstreamFormat()) {
            buffer = MakeBitstreamAudioBuffer(
                kSampleFormatEac3, kChannelLayout, kChannels, kInputSamplesPerSecond, 1, 0, frames.value, frames.value / 2, next_timestamp_->GetTimestamp());
        } else {
            buffer = MakeAudioBuffer<float>(
                kSampleFormat, kChannelLayout, kChannels, kInputSamplesPerSecond, 1.0f, 0.0f, frames.value, next_timestamp_->GetTimestamp());
        }
    ...
    void DeliverEndOfStream()
    {
        ...
        EXPECT_CALL(demuxer_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kOk, buffers));

        // Satisfy pending |decode_cb_| to trigger a new DemuxerStream::Read().
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb_), DecoderStatus::Codes::kOk));
        ...
        base::RunLoop().RunUntilIdle();
        EXPECT_EQ(last_statistics_.audio_memory_usage, renderer_->algorithm_->GetMemoryUsage());
    }
    ...
    // to be consumed.
    bool ConsumeBufferedData(OutputFrames requested_frames, base::TimeDelta delay)
    {
    ...
    bool ConsumeBitstreamBufferedData(OutputFrames requested_frames, base::TimeDelta delay = base::TimeDelta())
    {
        std::unique_ptr<AudioBus> bus = AudioBus::Create(kChannels, requested_frames.value);
    ...
    base::TimeTicks ConvertMediaTime(base::TimeDelta timestamp, bool* is_time_moving)
    {
        std::vector<base::TimeTicks> wall_clock_times;
        *is_time_moving = renderer_->GetWallClockTimes(std::vector<base::TimeDelta>(1, timestamp), &wall_clock_times);
    ...
    {
        std::vector<base::TimeTicks> wall_clock_times;
        *is_time_moving = renderer_->GetWallClockTimes(std::vector<base::TimeDelta>(), &wall_clock_times);
        return wall_clock_times[0];
    }
    ...
    bool is_buffer_full()
    {
        return renderer_->algorithm_->IsQueueFull();
    }
    ...
    {
        return InputFrames(renderer_->buffer_converter_->input_frames_left_for_testing());
    }
    ...
    void DecodeDecoder(scoped_refptr<DecoderBuffer> buffer, AudioDecoder::DecodeCB decode_cb)
    {
        // TODO(scherkus): Make this a DCHECK after threading semantics are fixed.
        if (!main_thread_task_runner_->BelongsToCurrentThread()) {
            main_thread_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&AudioRendererImplTest::DecodeDecoder, base::Unretained(this), buffer, std::move(decode_cb)));
            return;
        }
...
TEST_F(AudioRendererImplTest, SignalConfigChange)
{
    ...
    const AudioDecoderConfig kValidAudioConfig(
        AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 44100, EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
TEST_F(AudioRendererImplTest, DecoderUnderflow)
{
    ...
    // pending read.
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, DECODER_UNDERFLOW));
    EXPECT_FALSE(ConsumeBufferedData(OutputFrames(1)));
...
TEST_F(AudioRendererImplTest, DemuxerUnderflow)
{
    ...
    // pending read.
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, DEMUXER_UNDERFLOW));
    EXPECT_FALSE(ConsumeBufferedData(OutputFrames(1)));
...
TEST_F(AudioRendererImplTest, Underflow_OneCapacityIncreasePerUnderflow)
{
    ...
    EXPECT_EQ(0, frames_buffered().value);
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).Times(0);
    EXPECT_FALSE(ConsumeBufferedData(OutputFrames(1)));
...
{
    AudioParameters hw_params(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<CHANNEL_LAYOUT_7_1>(), kOutputSamplesPerSecond, 1024);
    ConfigureConfigChangeRenderer(
        AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kOutputSamplesPerSecond, 1024), hw_params);
    Initialize();
    ...
    scoped_refptr<AudioBuffer> buffer = MakeAudioBuffer<float>(
        kSampleFormat, hw_params.channel_layout(), hw_params.channels(), kInputSamplesPerSecond, 1.0f, 0.0f, kInputFramesChunk, base::TimeDelta());
    DeliverBuffer(DecoderStatus::Codes::kOk, std::move(buffer));
...
TEST_F(AudioRendererImplTest, ChannelMask_DownmixDiscreteLayout)
{
    ...
    AudioDecoderConfig audio_config(
        AudioCodec::kOpus, kSampleFormat, CHANNEL_LAYOUT_DISCRETE, kInputSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted);
...
TEST_F(AudioRendererImplTest, CurrentMediaTimeBehavior)
{
    ...
    tick_clock_.Advance(kConsumptionDuration);
    EXPECT_EQ(timestamp_helper.GetTimestamp() + kConsumptionDuration, CurrentMediaTime());
...
TEST_F(AudioRendererImplTest, RenderingDelayedForEarlyStartTime)
{
    ...
    const base::TimeDelta first_timestamp = base::Seconds(hardware_params_.frames_per_buffer() * kBuffers / hardware_params_.sample_rate());
...
TEST_F(AudioRendererImplTest, AbsurdRenderingDelayLog)
{
    ...
    EXPECT_MEDIA_LOG_ON(mock_media_log_, testing::HasSubstr("Large rendering delay"));
...
TEST_F(AudioRendererImplTest, ImmediateEndOfStream)
{
    ...
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN)).WillOnce(AssertNotYetEnded(this));
    DeliverEndOfStream();
...
TEST_F(AudioRendererImplTest, TimeSourceBehavior)
{
    ...
    EXPECT_EQ(base::TimeTicks(), ConvertMediaTime(base::TimeDelta(), &is_time_moving));
    ...
    // Time shouldn't change just yet because we've only sent the initial audio
    // data to the hardware.
    EXPECT_EQ(tick_clock_.NowTicks(), ConvertMediaTime(base::TimeDelta(), &is_time_moving));
    ...
    EXPECT_EQ(wall_clock_time_zero, ConvertMediaTime(base::TimeDelta(), &is_time_moving));
    EXPECT_TRUE(is_time_moving);
    ...
    for (int i = 0; i < kSteps; ++i) {
        ...
        EXPECT_EQ(tick_clock_.NowTicks(), CurrentMediaWallClockTime(&is_time_moving));
        EXPECT_TRUE(is_time_moving);
    }

    // Converting the current media time should be relative to wall clock zero.
    EXPECT_EQ(wall_clock_time_zero + kSteps * kAdvanceDelta, ConvertMediaTime(current_media_time, &is_time_moving));
    EXPECT_TRUE(is_time_moving);

    // Advancing once more will exceed the amount of played out frames finally.
    const base::TimeDelta kOneSample = base::Seconds(1.0 / kOutputSamplesPerSecond);
    ...
    base::TimeDelta delay_time = base::Microseconds(std::round(delay_frames * kOutputMicrosPerFrame));
    ...
    EXPECT_EQ(current_time, CurrentMediaWallClockTime(&is_time_moving));
    EXPECT_TRUE(is_time_moving);
    EXPECT_EQ(current_time, ConvertMediaTime(renderer_->CurrentMediaTime(), &is_time_moving));
    EXPECT_TRUE(is_time_moving);
    ...
    EXPECT_EQ(current_time, CurrentMediaWallClockTime(&is_time_moving));
    EXPECT_TRUE(is_time_moving);
    EXPECT_EQ(current_time + kOneSample * 2, ConvertMediaTime(renderer_->CurrentMediaTime(), &is_time_moving));
    EXPECT_TRUE(is_time_moving);

    // Advance far enough that we shouldn't be clamped to current time (tested
    // already above).
    tick_clock_.Advance(kOneSecond);
    EXPECT_EQ(current_time + timestamp_helper.GetFrameDuration(frames_to_consume.value), CurrentMediaWallClockTime(&is_time_moving));
    EXPECT_TRUE(is_time_moving);
}
...
TEST_F(AudioRendererImplTest, BitstreamEndOfStream)
{
    ...
    EXPECT_FALSE(ConsumeBitstreamBufferedData(OutputFrames(frames_buffered().value + kOutputFramesChunk)));
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _));
    WaitForPendingRead();
...
TEST_F(AudioRendererImplTest, MutedPlaybackBadDeviceInfo)
{
    ...
    mock_sink_ = base::MakeRefCounted<MockAudioRendererSink>(std::string(), OUTPUT_DEVICE_STATUS_ERROR_NOT_AUTHORIZED, AudioParameters());
    renderer_ = std::make_unique<AudioRendererImpl>(main_thread_task_runner_, mock_sink_.get(),
        base::BindRepeating(&AudioRendererImplTest::CreateAudioDecoderForTest, base::Unretained(this)), &media_log_, 0, nullptr);
    testing::Mock::VerifyAndClearExpectations(&demuxer_stream_);
    ConfigureDemuxerStream(true);
...
TEST_F(AudioRendererImplTest, SinkIsFlushed)
{
    ConfigureWithMockSink(
        AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kOutputSamplesPerSecond, 1024 * 15));
    Initialize();
    Preroll();
    StartTicking();
    StopTicking();
...
TEST_F(AudioRendererImplTest, LowLatencyHint)
{
    ...
    ConfigureBasicRenderer(
        AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), kInputSamplesPerSecond, kFramesPerBuffer));
    Initialize();
    ...
    const int default_buffer_playback_threshold = buffer_playback_threshold().value;
    ...
    base::TimeDelta default_buffering_latency = AudioTimestampHelper::FramesToTime(default_buffer_playback_threshold, kInputSamplesPerSecond);
    ...
    EXPECT_EQ(AudioTimestampHelper::FramesToTime(low_latency_playback_threshold, kInputSamplesPerSecond), low_latency);
...
TEST_F(AudioRendererImplTest, HighLatencyHint)
{
    ...
    EXPECT_EQ(AudioTimestampHelper::FramesToTime(high_latency_playback_threshold, kInputSamplesPerSecond), high_latency);
...
TEST_F(AudioRendererImplTest, DecodeAudioReadyPreemptsFlush)
{
    ...
    renderer_->Flush(base::BindOnce(&AudioRendererImpl::StartPlaying, base::Unretained(renderer_.get())));
...
TEST_F(AudioRendererImplTest, TranscribeAudioCallback_SpeechRecognitionDisabled)
{
    EXPECT_CALL(*this, SetOnReadyCallback(_));
    Initialize();

    EXPECT_CALL(*this, AddAudio(testing::An<scoped_refptr<AudioBuffer>>())).Times(0);
    Preroll();

    StartTicking();
}

TEST_F(AudioRendererImplTest, TranscribeAudioCallback_Muted_WithoutUserActivation)
{
...
TEST_F(AudioRendererImplTest, TranscribeAudioCallback_Unmuted_WithoutUserActivation)
{
    EnableSpeechRecognition();
    ...
    EXPECT_CALL(*this, AddAudio(testing::An<scoped_refptr<AudioBuffer>>())).Times(3);
    next_timestamp_->SetBaseTimestamp(base::TimeDelta());
    renderer_->SetMediaTime(base::TimeDelta());
    renderer_->StartPlaying();
    renderer_->SetVolume(1);
...
TEST_F(AudioRendererImplTest, TranscribeAudioCallback_Muted_WithUserActivation)
{
    EnableSpeechRecognition();
