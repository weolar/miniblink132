...
public:
    DecryptingRendererTest()
    {
        ...
        decrypting_renderer_ = std::make_unique<DecryptingRenderer>(std::move(renderer), &null_media_log_, task_environment_.GetMainThreadTaskRunner());
        ...
        EXPECT_CALL(decryptor_, CanAlwaysDecrypt()).WillRepeatedly(ReturnPointee(&use_aes_decryptor_));
        EXPECT_CALL(decryptor_, CancelDecrypt(_)).Times(AnyNumber());
        EXPECT_CALL(media_resource_, GetAllStreams()).WillRepeatedly(Invoke(this, &DecryptingRendererTest::GetAllStreams));
...
TEST_F(DecryptingRendererTest, ClearStreams_NoCdm)
{
    ...
    EXPECT_CALL(*renderer_, OnInitialize(_, _, _)).WillOnce(RunOnceCallback<2>(PIPELINE_OK));
    EXPECT_CALL(renderer_init_cb_, Run(HasStatusCode(PIPELINE_OK)));

    decrypting_renderer_->Initialize(&media_resource_, &renderer_client_, renderer_init_cb_.Get());
...
TEST_F(DecryptingRendererTest, DecryptingMediaResourceInitFails)
{
    ...
    EXPECT_CALL(renderer_init_cb_, Run(HasStatusCode(PIPELINE_ERROR_INITIALIZATION_FAILED)));
