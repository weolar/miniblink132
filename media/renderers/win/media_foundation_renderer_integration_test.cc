...
{
    static const base::NoDestructor<VideoCodecMap> AllVideoCodecsMap({ { VideoCodec::kVP9, MFVideoFormat_VP90 }, { VideoCodec::kHEVC, MFVideoFormat_HEVC } });
...
bool CanDecodeVideoCodec(VideoCodec codec)
{
    ...
    if (FAILED(MFTEnumEx(MFT_CATEGORY_VIDEO_DECODER, MFT_ENUM_FLAG_SYNCMFT | MFT_ENUM_FLAG_ASYNCMFT | MFT_ENUM_FLAG_HARDWARE, &input_type,
            /*output_type=*/nullptr, &activates, &count))) {
        return false;
    }
...
class MediaFoundationRendererIntegrationTest : public testing::Test, public PipelineIntegrationTestBase {
public:
    MediaFoundationRendererIntegrationTest()
    {
        SetCreateRendererCB(base::BindRepeating(&MediaFoundationRendererIntegrationTest::CreateMediaFoundationRenderer, base::Unretained(this)));
    }

    MediaFoundationRendererIntegrationTest(const MediaFoundationRendererIntegrationTest&) = delete;
    MediaFoundationRendererIntegrationTest& operator=(const MediaFoundationRendererIntegrationTest&) = delete;

private:
    std::unique_ptr<Renderer> CreateMediaFoundationRenderer(std::optional<RendererType> /*renderer_type*/)
    {
        LUID empty_luid { 0, 0 };
        auto renderer = std::make_unique<MediaFoundationRenderer>(task_environment_.GetMainThreadTaskRunner(), std::make_unique<NullMediaLog>(), empty_luid,
...
TEST_F(MediaFoundationRendererIntegrationTest, BasicPlayback)
{
    ...
    if (base::win::OSInfo::GetInstance()->version() >= base::win::Version::WIN10_20H2) {
...
TEST_F(MediaFoundationRendererIntegrationTest, HEVCPlayback_with_FFMpegDemuxer)
{
    ...
    media::UpdateDefaultDecoderSupportedVideoProfiles({ media::VideoCodecProfile::HEVCPROFILE_MAIN });
