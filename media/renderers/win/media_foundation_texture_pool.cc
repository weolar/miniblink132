...
MediaFoundationFrameInfo::~MediaFoundationFrameInfo() = default;
MediaFoundationFrameInfo::MediaFoundationFrameInfo(MediaFoundationFrameInfo&& other) = default;
...
MediaFoundationTexturePool::TextureInfo::TextureInfo(const TextureInfo& other) = default;
MediaFoundationTexturePool::TextureInfo& MediaFoundationTexturePool::TextureInfo::operator=(const MediaFoundationTexturePool::TextureInfo& other) = default;
...
HRESULT MediaFoundationTexturePool::Initialize(ID3D11Device* device, FramePoolInitializedCallback frame_pool_cb, const gfx::Size& frame_size)
{
    D3D11_TEXTURE2D_DESC desc { static_cast<UINT>(frame_size.width()), static_cast<UINT>(frame_size.height()), 1, 1,
        // TODO(crbug.com/40808700): Need to handle higher bit-depths like HDR.
        DXGI_FORMAT_B8G8R8A8_UNORM, { 1, 0 }, D3D11_USAGE_DEFAULT, D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE, 0,
    ...
    for (int i = 0; i < kTexturePoolCount; ++i) {
        ...
        RETURN_IF_FAILED(device->CreateTexture2D(&desc, nullptr, &d3d11_video_frame));
        ...
        RETURN_IF_FAILED(
            d3d11_video_frame_resource->CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ | DXGI_SHARED_RESOURCE_WRITE, nullptr, &shared_texture_handle));
...
ComPtr<ID3D11Texture2D> MediaFoundationTexturePool::AcquireTexture(base::UnguessableToken* texture_token)
{
...
void MediaFoundationTexturePool::ReleaseTexture(const base::UnguessableToken& texture_token)
{
