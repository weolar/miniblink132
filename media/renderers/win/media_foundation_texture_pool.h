...
using FramePoolInitializedCallback = base::RepeatingCallback<void(std::vector<MediaFoundationFrameInfo> frame_textures, const gfx::Size& texture_size)>;
...
public:
    ...
    MediaFoundationTexturePool& operator=(const MediaFoundationTexturePool& other) = delete;
    ...
    HRESULT Initialize(ID3D11Device* device, FramePoolInitializedCallback frame_pool_cb, const gfx::Size& frame_size);
    Microsoft::WRL::ComPtr<ID3D11Texture2D> AcquireTexture(base::UnguessableToken* texture_token);
