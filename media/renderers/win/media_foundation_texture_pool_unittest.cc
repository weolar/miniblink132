...
public:
    ...
    HRESULT STDMETHODCALLTYPE SetEvictionPriority(UINT eviction_priority) override;
    HRESULT STDMETHODCALLTYPE GetEvictionPriority(UINT* eviction_priority) override;
...
private:
    ...
    {
        memcpy(&texture_description_, texture_description, sizeof(D3D11_TEXTURE2D_DESC));
    }

public:
    static HRESULT CreateInstance(const D3D11_TEXTURE2D_DESC* texture_description, ID3D11Texture2D** texture2D);
    // IUnknown
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void** ppvObject) override
    {
    ...
    HRESULT STDMETHODCALLTYPE QueryInterfaceInternal(REFIID riid, void** ppvObject)
    {
        if (ppvObject == nullptr) {
            return E_POINTER;
        }
    ...
    // ID3D11Resource
    void STDMETHODCALLTYPE GetType(D3D11_RESOURCE_DIMENSION* resource_dimension) override
    {
        *resource_dimension = D3D11_RESOURCE_DIMENSION::D3D11_RESOURCE_DIMENSION_TEXTURE2D;
    }
...
public:
    ...
    HRESULT STDMETHODCALLTYPE CreateTexture1D(
        const D3D11_TEXTURE1D_DESC* pDesc, const D3D11_SUBRESOURCE_DATA* pInitialData, ID3D11Texture1D** ppTexture1D) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateTexture2D(
        const D3D11_TEXTURE2D_DESC* pDesc, const D3D11_SUBRESOURCE_DATA* pInitialData, ID3D11Texture2D** ppTexture2D) override;
    HRESULT STDMETHODCALLTYPE CreateTexture3D(
        const D3D11_TEXTURE3D_DESC* pDesc, const D3D11_SUBRESOURCE_DATA* pInitialData, ID3D11Texture3D** ppTexture3D) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateShaderResourceView(
        ID3D11Resource* pResource, const D3D11_SHADER_RESOURCE_VIEW_DESC* pDesc, ID3D11ShaderResourceView** ppSRView) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateUnorderedAccessView(
        ID3D11Resource* pResource, const D3D11_UNORDERED_ACCESS_VIEW_DESC* pDesc, ID3D11UnorderedAccessView** ppUAView) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateRenderTargetView(
        ID3D11Resource* pResource, const D3D11_RENDER_TARGET_VIEW_DESC* pDesc, ID3D11RenderTargetView** ppRTView) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateDepthStencilView(
        ID3D11Resource* pResource, const D3D11_DEPTH_STENCIL_VIEW_DESC* pDesc, ID3D11DepthStencilView** ppDepthStencilView) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateInputLayout(const D3D11_INPUT_ELEMENT_DESC* pInputElementDescs, UINT NumElements,
        const void* pShaderBytecodeWithInputSignature, SIZE_T BytecodeLength, ID3D11InputLayout** ppInputLayout) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateVertexShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11VertexShader** ppVertexShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateGeometryShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11GeometryShader** ppGeometryShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateGeometryShaderWithStreamOutput(const void* pShaderBytecode, SIZE_T BytecodeLength,
        const D3D11_SO_DECLARATION_ENTRY* pSODeclaration, UINT NumEntries, const UINT* pBufferStrides, UINT NumStrides, UINT RasterizedStream,
        ID3D11ClassLinkage* pClassLinkage, ID3D11GeometryShader** ppGeometryShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreatePixelShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11PixelShader** ppPixelShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateHullShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11HullShader** ppHullShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateDomainShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11DomainShader** ppDomainShader) override
    {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE CreateComputeShader(
        const void* pShaderBytecode, SIZE_T BytecodeLength, ID3D11ClassLinkage* pClassLinkage, ID3D11ComputeShader** ppComputeShader) override
    {
        return E_NOTIMPL;
    }
    ...
    HRESULT STDMETHODCALLTYPE CheckCounter(const D3D11_COUNTER_DESC* pDesc, D3D11_COUNTER_TYPE* pType, UINT* pActiveCounters, LPSTR szName, UINT* pNameLength,
        LPSTR szUnits, UINT* pUnitsLength, LPSTR szDescription, UINT* pDescriptionLength) override
    {
        return E_NOTIMPL;
    }
...
// MockD3D11Resource
HRESULT STDMETHODCALLTYPE MockD3D11Resource::QueryInterface(REFIID riid, void** ppvObject)
{
    if (ppvObject == nullptr) {
        return E_POINTER;
    }
...
HRESULT STDMETHODCALLTYPE MockD3D11Resource::SetEvictionPriority(UINT eviction_priority)
{
    parent_->SetEvictionPriority(eviction_priority);
    return S_OK;
}
HRESULT STDMETHODCALLTYPE MockD3D11Resource::GetEvictionPriority(UINT* eviction_priority)
{
    *eviction_priority = parent_->GetEvictionPriority();
    return S_OK;
}

HRESULT STDMETHODCALLTYPE MockD3D11Resource::SetPrivateDataInterface(REFGUID guid, const IUnknown* pData)
{
    return parent_->SetPrivateDataInterface(guid, pData);
}

HRESULT STDMETHODCALLTYPE MockD3D11Resource::CreateSharedHandle(const SECURITY_ATTRIBUTES* pAttributes, DWORD dwAccess, LPCWSTR lpName, HANDLE* pHandle)
{
...
HRESULT MockD3D11Texture2D::CreateInstance(const D3D11_TEXTURE2D_DESC* texture_description, ID3D11Texture2D** texture2D)
{
    MockD3D11Texture2D* mock_texture = new MockD3D11Texture2D(texture_description);
    if (!mock_texture) {
...
HRESULT STDMETHODCALLTYPE MockD3D11Device::CreateTexture2D(
...
TEST_F(MediaFoundationTexturePoolTest, VerifyTextureInitialization)
{
    ...
    public:
        ...
        void Invoke(std::vector<media::MediaFoundationFrameInfo> frame_textures, const gfx::Size& texture_size)
        {
    ...
    EXPECT_HRESULT_SUCCEEDED(test.Initialize(&mock_d3d_device, base::BindRepeating(&SpecialCallback::Invoke, base::Unretained(&callback)), frame_size));
    wait_event.Wait();
}
