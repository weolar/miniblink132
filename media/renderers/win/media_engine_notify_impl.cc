...
PipelineStatus MediaEngineErrorToPipelineStatus(MF_MEDIA_ENGINE_ERR media_engine_error)
{
...
HRESULT MediaEngineNotifyImpl::RuntimeClassInitialize(ErrorCB error_cb, EndedCB ended_cb, FormatChangeCB format_change_cb, LoadedDataCB loaded_data_cb,
    CanPlayThroughCB can_play_through_cb, PlayingCB playing_cb, WaitingCB waiting_cb, FrameStepCompletedCB frame_step_completed_cb, TimeUpdateCB time_update_cb,
    std::optional<VideoDecoderConfig> video_decoder_config, std::optional<AudioDecoderConfig> audio_decoder_config)
{
    DVLOG_FUNC(1);
...
HRESULT MediaEngineNotifyImpl::EventNotify(DWORD event_code, DWORD_PTR param1, DWORD param2)
{
    ...
    case MF_MEDIA_ENGINE_EVENT_ERROR: {
        ...
        base::UmaHistogramSparse(base::StrCat({ "Media.MediaFoundation.MediaEngineError.", MediaEngineErrorToString(error), ".Hresult" }), hr);

        // Report the Video and Audio codec used when encountering the HRESULT
        // MF_E_TOPO_CODEC_NOT_FOUND.
        // TODO(crbug.com/315860185): Remove this after the investigation is done.
        if (hr == MF_E_TOPO_CODEC_NOT_FOUND && audio_decoder_config_.has_value() && video_decoder_config_.has_value()) {
            base::UmaHistogramEnumeration(
                base::StrCat({ "Media.MediaFoundation.MF_E_TOPO_CODEC_NOT_FOUND.", media::GetCodecNameForUMA(video_decoder_config_->codec()) }),
