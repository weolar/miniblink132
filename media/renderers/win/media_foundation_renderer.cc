...
bool InitializeVideoWindowClass()
{
    ...
    base::win::InitializeWindowClass(L"VirtualMediaFoundationCdmVideoWindow", &base::win::WrappedWindowProc<::DefWindowProc>, CS_OWNDC, 0, 0, nullptr,
        reinterpret_cast<HBRUSH>(GetStockObject(BLACK_BRUSH)), nullptr, nullptr, nullptr, &intermediate_class);
...
const std::string GetErrorReasonString(const MediaFoundationRenderer::ErrorReason& reason)
{
...
{
    base::UmaHistogramEnumeration("Media.MediaFoundationRenderer.ErrorReason", reason);
}

MediaFoundationRenderer::MediaFoundationRenderer(
    scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log, LUID gpu_process_adapter_luid, bool force_dcomp_mode_for_testing)
...
void MediaFoundationRenderer::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
    DVLOG_FUNC(1);
    ...
    rendering_mode_ = rendering_strategy == MediaFoundationClearRenderingStrategy::kDirectComposition ? MediaFoundationRenderingMode::DirectComposition
                                                                                                      : MediaFoundationRenderingMode::FrameServer;
    for (DemuxerStream* stream : media_resource->GetAllStreams()) {
        if (stream->type() == DemuxerStream::Type::VIDEO && stream->video_decoder_config().is_encrypted()) {
    ...
    MEDIA_LOG(INFO, media_log_) << "Starting MediaFoundationRenderingMode: " << rendering_mode_;

    HRESULT hr = CreateMediaEngine(media_resource);
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to create media engine: " << PrintHr(hr);
        base::UmaHistogramSparse("Media.MediaFoundationRenderer.CreateMediaEngineError", hr);
        OnError(PIPELINE_ERROR_INITIALIZATION_FAILED, ErrorReason::kFailedToCreateMediaEngine, hr, std::move(init_cb));
        return;
    }
...
HRESULT MediaFoundationRenderer::CreateMediaEngine(MediaResource* media_resource)
{
    DVLOG_FUNC(1);
    ...
    RETURN_IF_FAILED(MakeAndInitialize<MediaEngineNotifyImpl>(&mf_media_engine_notify_,
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnPlaybackError, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnPlaybackEnded, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnFormatChange, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnLoadedData, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnCanPlayThrough, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnPlaying, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnWaiting, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnFrameStepCompleted, weak_this)),
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaFoundationRenderer::OnTimeUpdate, weak_this)), video_decoder_config,
        audio_decoder_config));

    ComPtr<IMFAttributes> creation_attributes;
    RETURN_IF_FAILED(MFCreateAttributes(&creation_attributes, 6));
    RETURN_IF_FAILED(creation_attributes->SetUnknown(MF_MEDIA_ENGINE_CALLBACK, mf_media_engine_notify_.Get()));
    RETURN_IF_FAILED(creation_attributes->SetUINT32(MF_MEDIA_ENGINE_CONTENT_PROTECTION_FLAGS, MF_MEDIA_ENGINE_ENABLE_PROTECTED_CONTENT));
    RETURN_IF_FAILED(creation_attributes->SetUINT32(MF_MEDIA_ENGINE_AUDIO_CATEGORY, AudioCategory_Media));
    if (virtual_video_window_) {
        RETURN_IF_FAILED(creation_attributes->SetUINT64(MF_MEDIA_ENGINE_OPM_HWND, reinterpret_cast<uint64_t>(virtual_video_window_)));
    }

    if (dxgi_device_manager_) {
        RETURN_IF_FAILED(creation_attributes->SetUnknown(MF_MEDIA_ENGINE_DXGI_MANAGER, dxgi_device_manager_.Get()));
    ...
    RETURN_IF_FAILED(MakeAndInitialize<MediaEngineExtension>(&mf_media_engine_extension_));
    RETURN_IF_FAILED(creation_attributes->SetUnknown(MF_MEDIA_ENGINE_EXTENSION, mf_media_engine_extension_.Get()));

    ComPtr<IMFMediaEngineClassFactory> class_factory;
    RETURN_IF_FAILED(CoCreateInstance(CLSID_MFMediaEngineClassFactory, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&class_factory)));
    ...
    RETURN_IF_FAILED(class_factory->CreateInstance(creation_flags, creation_attributes.Get(), &mf_media_engine_));
    ...
    RETURN_IF_FAILED(MakeAndInitialize<MediaFoundationSourceWrapper>(&mf_source_, media_resource, media_log_.get(), task_runner_));
    ...
    RETURN_IF_FAILED(MakeAndInitialize<MediaFoundationProtectionManager>(
        &content_protection_manager_, task_runner_, base::BindRepeating(&MediaFoundationRenderer::OnProtectionManagerWaiting, weak_factory_.GetWeakPtr())));
    ComPtr<IMFMediaEngineProtectedContent> protected_media_engine;
    RETURN_IF_FAILED(mf_media_engine_.As(&protected_media_engine));
    RETURN_IF_FAILED(protected_media_engine->SetContentProtectionManager(content_protection_manager_.Get()));
...
HRESULT MediaFoundationRenderer::SetSourceOnMediaEngine()
{
    ...
    RETURN_IF_FAILED(mf_media_engine_extension_->SetMediaSource(source_unknown.Get()));

    DVLOG(2) << "Set MFRendererSrc scheme as the source for MFMediaEngine.";
    base::win::ScopedBstr mf_renderer_source_scheme(base::ASCIIToWide("MFRendererSrc"));
    // We need to set our source scheme first in order for the MFMediaEngine to
    // load of our custom MFMediaSource.
    RETURN_IF_FAILED(mf_media_engine_->SetSource(mf_renderer_source_scheme.Get()));
...
HRESULT MediaFoundationRenderer::InitializeDXGIDeviceManager()
{
    ...
    RETURN_IF_FAILED(MFLockDXGIDeviceManager(&device_reset_token, &dxgi_device_manager_));
    ...
    UINT creation_flags = (D3D11_CREATE_DEVICE_VIDEO_SUPPORT | D3D11_CREATE_DEVICE_BGRA_SUPPORT | D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS);
    static const D3D_FEATURE_LEVEL feature_levels[] = { D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_10_0,
    ...
    if (gpu_process_adapter_luid_.LowPart || gpu_process_adapter_luid_.HighPart) {
        ...
        for (UINT i = 0; SUCCEEDED(factory->EnumAdapters(i, &temp_adapter)); i++) {
            ...
            if (desc.AdapterLuid.LowPart == gpu_process_adapter_luid_.LowPart && desc.AdapterLuid.HighPart == gpu_process_adapter_luid_.HighPart) {
    ...
    HRESULT hr = D3D11CreateDevice(adapter_to_use.Get(), adapter_to_use ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, 0, creation_flags, feature_levels,
        std::size(feature_levels), D3D11_SDK_VERSION, &d3d11_device, nullptr, nullptr);
    if (FAILED(hr)) {
        base::UmaHistogramSparse("Media.MediaFoundationRenderer.D3D11CreateDeviceFailed", hr);
        if (hr == DXGI_ERROR_UNSUPPORTED) {
            ...
            // apply.
            RETURN_IF_FAILED(D3D11CreateDevice(adapter_to_use.Get(), adapter_to_use ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, 0,
                /*creation_flags=*/0, feature_levels, std::size(feature_levels), D3D11_SDK_VERSION, &d3d11_device, nullptr, nullptr));
        } else {
            RETURN_IF_FAILED(hr);
        }
    }
    ...
    return dxgi_device_manager_->ResetDevice(d3d11_device.Get(), device_reset_token);
}
...
HRESULT MediaFoundationRenderer::InitializeVirtualVideoWindow()
{
    ...
    virtual_video_window_ = CreateWindowEx(WS_EX_NOPARENTNOTIFY | WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOREDIRECTIONBITMAP,
        reinterpret_cast<wchar_t*>(g_video_window_class), L"", WS_POPUP | WS_DISABLED | WS_CLIPSIBLINGS, 0, 0, 1, 1, nullptr, nullptr, nullptr, nullptr);
    if (!virtual_video_window_) {
...
void MediaFoundationRenderer::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    DVLOG_FUNC(1);
...
void MediaFoundationRenderer::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
    DVLOG_FUNC(1);

    if (latency_hint.has_value()) {
        DLOG_IF(WARNING, mf_media_engine_) << "Latency hint is not utilized after MF media engine creation.";
...
void MediaFoundationRenderer::OnCdmProxyReceived()
{
    ...
    if (!waiting_for_mf_cdm_ || !content_protection_manager_) {
        OnError(PIPELINE_ERROR_INVALID_STATE, ErrorReason::kCdmProxyReceivedInInvalidState, kErrorCdmProxyReceivedInInvalidState);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToSetSourceOnMediaEngine, hr);
        return;
    }
}
...
void MediaFoundationRenderer::Flush(base::OnceClosure flush_cb)
{
    ...
    DVLOG_IF(1, FAILED(hr)) << "Failed to pause playback on flush: " << PrintHr(hr);

    mf_source_->FlushStreams();
    std::move(flush_cb).Run();
}

void MediaFoundationRenderer::SetMediaFoundationRenderingMode(MediaFoundationRenderingMode render_mode)
{
    ...
    if (mf_media_engine_->HasVideo()) {
        if (render_mode == MediaFoundationRenderingMode::FrameServer) {
            ...
            DVLOG(1) << "Rendering mode: " << static_cast<int>(render_mode) << " is unsupported";
            MEDIA_LOG(ERROR, media_log_) << "MediaFoundationRenderer SetMediaFoundationRenderingMode: " << static_cast<int>(render_mode)
                                         << " is not defined. No change to the rendering mode.";
            hr = E_NOT_SET;
        }

        if (SUCCEEDED(hr)) {
            hr = mf_media_engine_ex->EnableWindowlessSwapchainMode(render_mode == MediaFoundationRenderingMode::DirectComposition);
            if (SUCCEEDED(hr)) {
                rendering_mode_ = render_mode;
                MEDIA_LOG(INFO, media_log_) << "Set MediaFoundationRenderingMode: " << rendering_mode_;
            }
        }
    }
}
...
void MediaFoundationRenderer::StartPlayingFrom(base::TimeDelta time)
{
    ...
    if (FAILED(hr)) {
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToSetCurrentTime, hr);
        return;
    }
...
void MediaFoundationRenderer::SetPlaybackRate(double playback_rate)
{
    ...
    if (FAILED(hr)) {
        ...
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToSetPlaybackRate, hr);
        return;
    }
...
void MediaFoundationRenderer::GetDCompSurface(GetDCompSurfaceCB callback)
{
    ...
    if (FAILED(hr)) {
        base::UmaHistogramSparse("Media.MediaFoundationRenderer.FailedToSetDCompMode", hr);
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToSetDCompMode, hr);
    ...
    if (FAILED(hr) || IsInvalidHandle(surface_handle)) {
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToGetDCompSurface, hr);
        std::move(callback).Run(base::win::ScopedHandle(), PrintHr(hr));
        return;
    }
    ...
    const BOOL result = ::DuplicateHandle(process, surface_handle, process, &duplicated_handle, GENERIC_READ | GENERIC_EXECUTE, false, DUPLICATE_CLOSE_SOURCE);
    if (!result || IsInvalidHandle(surface_handle)) {
        hr = ::GetLastError();
        OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToDuplicateHandle, hr);
        std::move(callback).Run(base::win::ScopedHandle(), PrintHr(hr));
        return;
    }
...
void MediaFoundationRenderer::SetOutputRect(const gfx::Rect& output_rect, SetOutputRectCB callback)
{
    DVLOG_FUNC(2);

    // Call SetWindowPos to reposition the video from output_rect.
    if (virtual_video_window_
        && !::SetWindowPos(virtual_video_window_, HWND_BOTTOM, output_rect.x(), output_rect.y(), output_rect.width(), output_rect.height(), SWP_NOACTIVATE)) {
        DLOG(ERROR) << "Failed to SetWindowPos: " << PrintHr(HRESULT_FROM_WIN32(GetLastError()));
...
HRESULT MediaFoundationRenderer::InitializeTexturePool(const gfx::Size& size)
{
    ...
    RETURN_IF_FAILED(texture_pool_.Initialize(d3d11_device.Get(), initialized_frame_pool_cb_, size));
...
HRESULT MediaFoundationRenderer::GetDCompSurfaceInternal(HANDLE* surface_handle)
{
    DVLOG_FUNC(1);
...
HRESULT MediaFoundationRenderer::PopulateStatistics(PipelineStatistics& statistics)
{
    ComPtr<IMFMediaEngineEx> media_engine_ex;
    RETURN_IF_FAILED(mf_media_engine_.As(&media_engine_ex));

    base::win::ScopedPropVariant frames_rendered;
    RETURN_IF_FAILED(media_engine_ex->GetStatistics(MF_MEDIA_ENGINE_STATISTIC_FRAMES_RENDERED, frames_rendered.Receive()));
    base::win::ScopedPropVariant frames_dropped;
    RETURN_IF_FAILED(media_engine_ex->GetStatistics(MF_MEDIA_ENGINE_STATISTIC_FRAMES_DROPPED, frames_dropped.Receive()));

    statistics.video_frames_decoded = frames_rendered.get().ulVal + frames_dropped.get().ulVal;
    statistics.video_frames_dropped = frames_dropped.get().ulVal;
    DVLOG_FUNC(3) << "video_frames_decoded=" << statistics.video_frames_decoded << ", video_frames_dropped=" << statistics.video_frames_dropped;
    return S_OK;
}
...
void MediaFoundationRenderer::SendStatistics()
{
    ...
    if (FAILED(hr)) {
        LIMITED_MEDIA_LOG(INFO, media_log_, populate_statistics_failure_count_, 3) << "MediaFoundationRenderer failed to populate stats: " + PrintHr(hr);
        return;
    }

    const int kSignificantPlaybackFrames = 5400; // About 30 fps for 3 minutes.
    if (!has_reported_significant_playback_ && cdm_proxy_ && new_stats.video_frames_decoded >= kSignificantPlaybackFrames) {
    ...
    if (statistics_ != new_stats) {
        ...
        delta.video_frames_decoded = base::ClampSub(new_stats.video_frames_decoded, statistics_.video_frames_decoded);
        delta.video_frames_dropped = base::ClampSub(new_stats.video_frames_dropped, statistics_.video_frames_dropped);
...
void MediaFoundationRenderer::StartSendingStatistics()
{
    ...
    statistics_timer_.Start(FROM_HERE, kPipelineStatsPollingPeriod, this, &MediaFoundationRenderer::SendStatistics);
}
...
void MediaFoundationRenderer::SetFrameReturnCallbacks(FrameReturnCallback frame_available_cb, FramePoolInitializedCallback initialized_frame_pool_cb)
{
    frame_available_cb_ = std::move(frame_available_cb);
    initialized_frame_pool_cb_ = std::move(initialized_frame_pool_cb);
}

void MediaFoundationRenderer::SetGpuProcessAdapterLuid(LUID gpu_process_adapter_luid)
{
...
void MediaFoundationRenderer::OnPlaybackError(PipelineStatus status, HRESULT hr)
{
...
void MediaFoundationRenderer::OnCanPlayThrough()
{
    ...
    if (playback_rate_ == 0) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            OnError(PIPELINE_ERROR_COULD_NOT_RENDER, ErrorReason::kFailedToGetMediaEngineEx, hr);
            return;
        }
    }
    ...
    OnBufferingStateChange(BufferingState::BUFFERING_HAVE_ENOUGH, BufferingStateChangeReason::BUFFERING_CHANGE_REASON_UNKNOWN);
}
...
{
    DVLOG_FUNC(2);
    OnBufferingStateChange(BufferingState::BUFFERING_HAVE_NOTHING, BufferingStateChangeReason::BUFFERING_CHANGE_REASON_UNKNOWN);
}
...
void MediaFoundationRenderer::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
    DVLOG_FUNC(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    if (state == BufferingState::BUFFERING_HAVE_NOTHING && max_buffering_state_ != BufferingState::BUFFERING_HAVE_ENOUGH) {
...
void MediaFoundationRenderer::OnVideoNaturalSizeChange()
{
    ...
    DWORD native_width;
    DWORD native_height;
    HRESULT hr = mf_media_engine_->GetNativeVideoSize(&native_width, &native_height);
    if (FAILED(hr)) {
        ...
        native_video_size_ = { base::checked_cast<int>(native_width), base::checked_cast<int>(native_height) };
    }
...
void MediaFoundationRenderer::OnError(PipelineStatus status, ErrorReason reason, HRESULT hresult, PipelineStatusCallback status_cb)
{
    const std::string error = "MediaFoundationRenderer error: " + GetErrorReasonString(reason) + " (" + PrintHr(hresult) + ")";
    ...
    if (hresult == DRM_E_TEE_INVALID_HWDRM_STATE) {
        ...
        base::UmaHistogramBoolean("Media.MediaFoundationRenderer.InvalidHwdrmState.HasReportedPlaying", has_reported_playing_);
        base::UmaHistogramCounts10000("Media.MediaFoundationRenderer.InvalidHwdrmState.VideoFrameDecoded", statistics_.video_frames_decoded);
...
void MediaFoundationRenderer::RequestNextFrame()
{
    ...
    if (dxgi_device_manager_ == nullptr || mf_media_engine_->OnVideoStreamTick(&presentation_timestamp_in_hns) != S_OK) {
        return;
    }

    if (native_video_size_.IsEmpty()) {
        MEDIA_LOG(WARNING, media_log_) << "RequestNextFrame ignores empty native_video_size_";
        return;
    }
    ...
    if (FAILED(mf_media_engine_->TransferVideoFrame(d3d11_video_frame.Get(), nullptr, &destination_frame_size, nullptr))) {
...
void MediaFoundationRenderer::NotifyFrameReleased(const base::UnguessableToken& frame_token)
{
    texture_pool_.ReleaseTexture(frame_token);
}
