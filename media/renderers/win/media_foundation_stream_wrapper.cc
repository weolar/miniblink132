...
PendingInputBuffer::PendingInputBuffer(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
    : status(status)
...
PendingInputBuffer::PendingInputBuffer(const PendingInputBuffer& other) = default;
...
/*static*/
HRESULT MediaFoundationStreamWrapper::Create(int stream_id, IMFMediaSource* parent_source, DemuxerStream* demuxer_stream, std::unique_ptr<MediaLog> media_log,
    scoped_refptr<base::SequencedTaskRunner> task_runner, MediaFoundationStreamWrapper** stream_out)
{
    ...
    case DemuxerStream::Type::VIDEO:
        RETURN_IF_FAILED(MediaFoundationVideoStream::Create(stream_id, parent_source, demuxer_stream, std::move(media_log), &stream));
        break;
    case DemuxerStream::Type::AUDIO:
        RETURN_IF_FAILED(MediaFoundationAudioStream::Create(stream_id, parent_source, demuxer_stream, std::move(media_log), &stream));
        break;
    default:
        DLOG(ERROR) << "Unsupported demuxer stream type: " << demuxer_stream->type();
        return E_INVALIDARG;
    }
...
HRESULT MediaFoundationStreamWrapper::RuntimeClassInitialize(
    int stream_id, IMFMediaSource* parent_source, DemuxerStream* demuxer_stream, std::unique_ptr<MediaLog> media_log)
{
    {
    ...
    DVLOG_FUNC(1) << "stream_id=" << stream_id << ", stream_type=" << DemuxerStream::GetTypeName(stream_type_);

    media_log_ = std::move(media_log);
    if (base::FeatureList::IsEnabled(kMediaFoundationBatchRead)) {
        if (kBatchReadCount.Get() < 1 || kBatchReadCount.Get() > 500) {
            DLOG(WARNING) << "batch_read_count_=" << kBatchReadCount.Get() << " is out of range [1,500], "
                          << (kBatchReadCount.Get() < 1 ? "it shouldn't be negative or 0"
...
void MediaFoundationStreamWrapper::SetTaskRunner(scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    DVLOG_FUNC(1);
...
void MediaFoundationStreamWrapper::SetLastStartPosition(const PROPVARIANT* start_position)
{
...
HRESULT MediaFoundationStreamWrapper::QueueStartedEvent(const PROPVARIANT* start_position)
{
    DVLOG_FUNC(2);
    ...
    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamVar(MEStreamStarted, GUID_NULL, S_OK, start_position));
    return S_OK;
}

HRESULT MediaFoundationStreamWrapper::QueueSeekedEvent(const PROPVARIANT* start_position)
{
    DVLOG_FUNC(2);
    ...
    state_ = State::kStarted;
    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamVar(MEStreamSeeked, GUID_NULL, S_OK, start_position));
    return S_OK;
}
...
HRESULT MediaFoundationStreamWrapper::QueueStoppedEvent()
{
    ...
    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamVar(MEStreamStopped, GUID_NULL, S_OK, nullptr));
    return S_OK;
}
...
HRESULT MediaFoundationStreamWrapper::QueuePausedEvent()
{
    ...
    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamVar(MEStreamPaused, GUID_NULL, S_OK, nullptr));
    return S_OK;
}
...
void MediaFoundationStreamWrapper::ProcessRequestsIfPossible()
{
    ...
    if (!buffer_queue_.empty()) {
        ...
        OnDemuxerStreamRead(buffer_queue_.front().status, std::move(buffer_queue_.front().buffer));
    ...
    if (!pending_stream_read_) {
        ...
        TRACE_EVENT2(
            "media", "MFGetBuffersFromRendererByIPC", "StreamType:", DemuxerStream::GetTypeName(stream_type_), "batch_read_count_:", batch_read_count_);
        pending_stream_read_ = true;
        demuxer_stream_->Read(batch_read_count_, base::BindOnce(&MediaFoundationStreamWrapper::OnDemuxerStreamReadBuffers, weak_factory_.GetWeakPtr()));
    }
}

void MediaFoundationStreamWrapper::OnDemuxerStreamReadBuffers(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DVLOG_FUNC(3) << "receive data, status=" << DemuxerStream::GetStatusName(status) << ", buffer count= " << buffers.size()
                  << ", stream type=" << DemuxerStream::GetTypeName(stream_type_);
    {
        ...
        DemuxerStream::DecoderBufferVector pending_buffers
            = (status == DemuxerStream::Status::kOk) ? std::move(buffers) : DemuxerStream::DecoderBufferVector { nullptr };
        for (auto& buffer : pending_buffers) {
            DVLOG_FUNC(3) << "push buffer to buffer_queue_, status=" << DemuxerStream::GetStatusName(status)
                          << ", buffer=" << (buffer ? buffer->AsHumanReadableString(false) : "null");
...
HRESULT MediaFoundationStreamWrapper::ServiceSampleRequest(IUnknown* token, DecoderBuffer* buffer)
{
    DVLOG_FUNC(3);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    const auto timestamp_us = is_end_of_stream ? 0 : buffer->timestamp().InMicroseconds();
    TRACE_EVENT2("media", "MediaFoundationStreamWrapper::ServiceSampleRequest", "is_encrypted", is_encrypted, "timestamp_us", timestamp_us);
    ...
    if (is_end_of_stream) {
        ...
        RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamUnk(MEEndOfStream, GUID_NULL, S_OK, nullptr));
        stream_ended_ = true;
        if (parent_source_) {
            static_cast<MediaFoundationSourceWrapper*>(parent_source_.Get())->CheckForEndOfPresentation();
        }
    } else {
        DVLOG_FUNC(3) << "buffer ts=" << buffer->timestamp() << ", is_key_frame=" << buffer->is_key_frame();
        ComPtr<IMFSample> mf_sample;
        RETURN_IF_FAILED(GenerateSampleFromDecoderBuffer(
            buffer, &mf_sample, &last_key_id_, base::BindOnce(&MediaFoundationStreamWrapper::TransformSample, base::Unretained(this))));
        if (token) {
            RETURN_IF_FAILED(mf_sample->SetUnknown(MFSampleExtension_Token, token));
        }

        RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamUnk(MEMediaSample, GUID_NULL, S_OK, mf_sample.Get()));
    }
...
bool MediaFoundationStreamWrapper::ServicePostFlushSampleRequest()
{
    ...
    base::AutoLock auto_lock(lock_);
    if (flushed_ && state_ == State::kStarted && last_start_time_ != kInvalidTime) {
        ...
        hr = mf_media_event_queue_->QueueEventParamVar(MEStreamTick, GUID_NULL, S_OK, &var);
        if (FAILED(hr)) {
    ...
    } else if ((flushed_ && state_ != State::kStarted) || post_flush_buffers_.empty()) {
        return false;
    }

    DCHECK(!pending_sample_request_tokens_.empty());
    ComPtr<IUnknown> request_token = pending_sample_request_tokens_.front();
    hr = ServiceSampleRequest(request_token.Get(), post_flush_buffers_.front().get());
    if (FAILED(hr)) {
...
HRESULT MediaFoundationStreamWrapper::QueueFormatChangedEvent()
{
    ...
    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamUnk(MEStreamFormatChanged, GUID_NULL, S_OK, media_type.Get()));
    return S_OK;
}

void MediaFoundationStreamWrapper::OnDemuxerStreamRead(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    DVLOG_FUNC(3) << "status=" << status << (buffer ? " buffer=" + buffer->AsHumanReadableString(true) : "");
    {
        lock_.AssertAcquired();
        ...
        if (status == DemuxerStream::Status::kOk) {
            ...
            if (has_clear_lead_ && !switched_clear_to_encrypted_ && !buffer->end_of_stream() && buffer->is_encrypted()) {
                MEDIA_LOG(INFO, media_log_) << "Stream switched from clear to encrypted buffers.";
            ...
            if (flushed_ || !post_flush_buffers_.empty()) {
                ...
                if (FAILED(hr)) {
                    DLOG(ERROR) << __func__ << ": ServiceSampleRequest failed: " << PrintHr(hr);
                    return;
                }
            }
        } else if (status == DemuxerStream::Status::kConfigChanged) {
            DVLOG_FUNC(2) << "Stream config changed, AreFormatChangesEnabled=" << AreFormatChangesEnabled();
            if (AreFormatChangesEnabled()) {
                hr = QueueFormatChangedEvent();
                if (FAILED(hr)) {
                    DLOG(ERROR) << __func__ << ": QueueFormatChangedEvent failed: " << PrintHr(hr);
                    return;
                }
            } else {
            ...
            mf_media_event_queue_->QueueEventParamVar(MEError, GUID_NULL, MF_E_INVALID_STREAM_DATA, nullptr);
            return;
            ...
            NOTREACHED() << "Unexpected demuxer stream status. status=" << status << ", this=" << this;
        }
    }
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&MediaFoundationStreamWrapper::ProcessRequestsIfPossible, weak_factory_.GetWeakPtr()));
}

HRESULT MediaFoundationStreamWrapper::TransformSample(Microsoft::WRL::ComPtr<IMFSample>& sample)
{
    DVLOG_FUNC(3);
...
HRESULT MediaFoundationStreamWrapper::GetMediaSource(IMFMediaSource** media_source_out)
{
    DVLOG_FUNC(2);
    DCHECK(!task_runner_->RunsTasksInCurrentSequence());
...
HRESULT MediaFoundationStreamWrapper::GetStreamDescriptor(IMFStreamDescriptor** stream_descriptor_out)
{
    DVLOG_FUNC(2);
...
HRESULT MediaFoundationStreamWrapper::GetEvent(DWORD flags, IMFMediaEvent** event_out)
{
    DVLOG_FUNC(3);
...
HRESULT MediaFoundationStreamWrapper::EndGetEvent(IMFAsyncResult* result, IMFMediaEvent** event_out)
{
    DVLOG_FUNC(3);
    DCHECK(mf_media_event_queue_);
...
HRESULT MediaFoundationStreamWrapper::QueueEvent(MediaEventType type, REFGUID extended_type, HRESULT status, const PROPVARIANT* value)
{
    DVLOG_FUNC(3);
    DCHECK(mf_media_event_queue_);

    RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamVar(type, extended_type, status, value));
    return S_OK;
}
...
HRESULT MediaFoundationStreamWrapper::GenerateStreamDescriptor()
{
    ...
    RETURN_IF_FAILED(GetMediaType(&media_type));
    RETURN_IF_FAILED(MFCreateStreamDescriptor(stream_id_, 1, mediaTypes, &mf_stream_descriptor_));
...
void MediaFoundationStreamWrapper::ReportEncryptionType(const scoped_refptr<DecoderBuffer>& buffer)
{
    ...
    if (encryption_type == EncryptionType::kEncryptedWithClearLead) {
        MEDIA_LOG(INFO, media_log_) << "MediaFoundationStreamWrapper: " << DemuxerStream::GetTypeName(stream_type_) << " stream is encrypted with clear lead";
