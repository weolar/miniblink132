...
public:
    ...
    MOCK_METHOD6(GetInputTrustAuthority,
        HRESULT(uint32_t stream_id, uint32_t stream_count, const uint8_t* content_init_data, uint32_t content_init_data_size, REFIID riid,
    ...
    MOCK_METHOD2(ProcessContentEnabler, HRESULT(IUnknown* request, IMFAsyncResult* result));
...
class MockMediaProtectionPMPServer
    : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::WinRt | Microsoft::WRL::InhibitRoOriginateError>,
          IMediaProtectionPMPServer> {
public:
    MockMediaProtectionPMPServer() = default;
    ~MockMediaProtectionPMPServer() override = default;

    static HRESULT MakeMockMediaProtectionPMPServer(IMediaProtectionPMPServer** pmp_server)
    {
    ...
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void** object_result) override
    {
        return E_NOINTERFACE;
    }

    // ABI::Windows::Media::Protection::IMediaProtectionPMPServer.
    MOCK_STDCALL_METHOD1(get_Properties, HRESULT(ABI::Windows::Foundation::Collections::IPropertySet** properties));
};
...
public:
    ...
    {
        mf_cdm_proxy_ = base::MakeRefCounted<NiceMock<MockMediaFoundationCdmProxy>>();
        ...
        MockMediaProtectionPMPServer::MakeMockMediaProtectionPMPServer(&pmp_server_);

        LUID empty_luid { 0, 0 };
        mf_renderer_ = std::make_unique<MediaFoundationRenderer>(task_environment_.GetMainThreadTaskRunner(), std::make_unique<NullMediaLog>(), empty_luid);

        // Some default actions.
        ON_CALL(cdm_context_, GetMediaFoundationCdmProxy()).WillByDefault(Return(mf_cdm_proxy_));
        ON_CALL(*mf_cdm_proxy_, GetPMPServer(_, _)).WillByDefault(Invoke(this, &MediaFoundationRendererTest::GetPMPServer));

        // Some expected calls with return values.
        EXPECT_CALL(media_resource_, GetAllStreams()).WillRepeatedly(Invoke(this, &MediaFoundationRendererTest::GetAllStreams));
        EXPECT_CALL(media_resource_, GetType()).WillRepeatedly(Return(MediaResource::Type::kStream));
    }

    ~MediaFoundationRendererTest() override
    {
        mf_renderer_.reset();
    }
    ...
    HRESULT GetPMPServer(REFIID riid, LPVOID* object_result)
    {
        ...
        return pmp_server_.CopyTo(reinterpret_cast<IMediaProtectionPMPServer**>(object_result));
    }
...
TEST_F(MediaFoundationRendererTest, VerifyInitWithoutSetCdm)
{
    ...
    mf_renderer_->Initialize(&media_resource_, &renderer_client_, renderer_init_cb_.Get());
...
{
    base::MockCallback<MediaFoundationRendererExtension::GetDCompSurfaceCB> get_dcomp_surface_cb;
...
TEST_F(MediaFoundationRendererTest, ClearStartsInFrameServer)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(media::kMediaFoundationClearRendering, { { "strategy", "dynamic" } });
