...
std::ostream& operator<<(std::ostream& os, const MediaFoundationRenderingMode& render_mode)
{
    std::string mode;
    switch (render_mode) {
    ...
    case (MediaFoundationRenderingMode::DirectComposition):
        ...
        break;
    default:
        mode = "UNEXPECTED RENDERING MODE " + base::NumberToString(static_cast<int>(render_mode));
    }
