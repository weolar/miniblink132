...
HRESULT MediaEngineExtension::CanPlayType(BOOL is_audio_only, BSTR mime_type, MF_MEDIA_ENGINE_CANPLAY* result)
{
...
HRESULT MediaEngineExtension::BeginCreateObject(
    BSTR url_bstr, IMFByteStream* byte_stream, MF_OBJECT_TYPE type, IUnknown** cancel_cookie, IMFAsyncCallback* callback, IUnknown* state)
{
    ...
    if (type == MF_OBJECT_MEDIASOURCE) {
        ...
        RETURN_IF_FAILED(MFCreateAsyncResult(local_source.Get(), callback, state, &async_result));
...
HRESULT MediaEngineExtension::CancelObjectCreation(__in IUnknown* cancel_cookie)
{
    DVLOG_FUNC(1);
...
HRESULT MediaEngineExtension::EndCreateObject(__in IMFAsyncResult* result, __deref_out IUnknown** ret_obj)
{
    DVLOG_FUNC(1);
