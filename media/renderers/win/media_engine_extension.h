...
class MediaEngineExtension
    : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::RuntimeClassType::ClassicCom>, IMFMediaEngineExtension> {
public:
    ...
    IFACEMETHODIMP CanPlayType(BOOL is_audio_only, BSTR mime_type, MF_MEDIA_ENGINE_CANPLAY* result) override;
    IFACEMETHODIMP BeginCreateObject(
        BSTR url_bstr, IMFByteStream* byte_stream, MF_OBJECT_TYPE type, IUnknown** cancel_cookie, IMFAsyncCallback* callback, IUnknown* state) override;
    IFACEMETHODIMP CancelObjectCreation(IUnknown* cancel_cookie) override;
    IFACEMETHODIMP EndCreateObject(IMFAsyncResult* result, IUnknown** ret_obj) override;
