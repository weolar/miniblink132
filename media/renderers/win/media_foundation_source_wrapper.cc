...
MediaFoundationSourceWrapper::~MediaFoundationSourceWrapper()
{
    ...
    for (uint32_t stream_id = 0; stream_id < StreamCount(); stream_id++) {
        HRESULT hr = cdm_proxy_->SetLastKeyId(stream_id, media_streams_[stream_id]->GetLastKeyId());
        DLOG_IF(ERROR, FAILED(hr)) << "Failed to notify CDM proxy of last Key IDs: " << PrintHr(hr);
    }
}

HRESULT MediaFoundationSourceWrapper::RuntimeClassInitialize(
    MediaResource* media_resource, MediaLog* media_log, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    DVLOG_FUNC(1);
    ...
    for (DemuxerStream* demuxer_stream : demuxer_streams) {
        ...
        // device).
        if (has_video_stream && demuxer_stream->type() == DemuxerStream::Type::AUDIO) {
            auto default_audio_output_device_id = media::CoreAudioUtil::GetDefaultOutputDeviceID();
            DVLOG_FUNC(3) << "default_audio_output_device_id=" << default_audio_output_device_id;
        ...
        RETURN_IF_FAILED(MediaFoundationStreamWrapper::Create(stream_id++, this, demuxer_stream, media_log->Clone(), task_runner, &mf_stream));
...
HRESULT MediaFoundationSourceWrapper::SelectDefaultStreams(const DWORD stream_desc_count, IMFPresentationDescriptor* presentation_descriptor)
{
    ...
    for (DWORD idx = 0; idx < stream_desc_count; idx++) {
        ...
        BOOL selected;
        RETURN_IF_FAILED(presentation_descriptor->GetStreamDescriptorByIndex(idx, &selected, &stream_descriptor));
        if (selected)
            continue;
        DWORD stream_id;
        RETURN_IF_FAILED(stream_descriptor->GetStreamIdentifier(&stream_id));
        if (media_streams_[stream_id]->StreamType() == DemuxerStream::Type::AUDIO && !audio_stream_selected) {
            audio_stream_selected = true;
            RETURN_IF_FAILED(presentation_descriptor->SelectStream(idx));
        } else if (media_streams_[stream_id]->StreamType() == DemuxerStream::Type::VIDEO && !video_stream_selected) {
...
HRESULT MediaFoundationSourceWrapper::CreatePresentationDescriptor(IMFPresentationDescriptor** presentation_descriptor_out)
{
    DVLOG_FUNC(2);
    ...
    RETURN_IF_FAILED(
        MFCreatePresentationDescriptor(stream_desc_count, reinterpret_cast<IMFStreamDescriptor**>(stream_descriptors.data()), &presentation_descriptor));
    RETURN_IF_FAILED(SelectDefaultStreams(stream_desc_count, presentation_descriptor.Get()));
...
HRESULT MediaFoundationSourceWrapper::Start(IMFPresentationDescriptor* presentation_descriptor, const GUID* guid_time_format, const PROPVARIANT* start_position)
{
    DVLOG_FUNC(2);
    ...
    RETURN_IF_FAILED(presentation_descriptor->GetStreamDescriptorCount(&stream_desc_count));

    for (DWORD i = 0; i < stream_desc_count; i++) {
        ComPtr<IMFStreamDescriptor> stream_descriptor;
        BOOL selected;
        RETURN_IF_FAILED(presentation_descriptor->GetStreamDescriptorByIndex(i, &selected, &stream_descriptor));
        ...
        if (selected) {
            ...
            RETURN_IF_FAILED(mf_media_event_queue_->QueueEventParamUnk(event_type, GUID_NULL, S_OK, unknown_stream.Get()));
    ...
    if (started) {
        ...
        RETURN_IF_FAILED(MFCreateMediaEvent(MESourceStarted, GUID_NULL, S_OK, start_position, &media_event));
        RETURN_IF_FAILED(mf_media_event_queue_->QueueEvent(media_event.Get()));
    } else if (seeked) {
        RETURN_IF_FAILED(QueueEvent(MESourceSeeked, GUID_NULL, S_OK, start_position));
    }
...
//
HRESULT MediaFoundationSourceWrapper::GetEvent(DWORD flags, IMFMediaEvent** event_out)
{
    DVLOG_FUNC(3);
...
HRESULT MediaFoundationSourceWrapper::EndGetEvent(IMFAsyncResult* result, IMFMediaEvent** event_out)
{
    DVLOG_FUNC(3);
    DCHECK(mf_media_event_queue_);
...
HRESULT MediaFoundationSourceWrapper::QueueEvent(MediaEventType type, REFGUID extended_type, HRESULT status, const PROPVARIANT* value)
{
    DVLOG_FUNC(3);
    DCHECK(mf_media_event_queue_);
...
HRESULT MediaFoundationSourceWrapper::GetInputTrustAuthority(DWORD stream_id, REFIID riid, IUnknown** object_out)
{
    DVLOG_FUNC(1);
    ...
    RETURN_IF_FAILED(cdm_proxy_->GetInputTrustAuthority(stream_id, StreamCount(), nullptr, 0, riid, object_out));
    return S_OK;
}

HRESULT MediaFoundationSourceWrapper::GetService(REFGUID guid_service, REFIID riid, LPVOID* result)
{
    DVLOG_FUNC(3);
    DCHECK(result);
...
HRESULT MediaFoundationSourceWrapper::GetSlowestRate(MFRATE_DIRECTION direction, BOOL supports_thinning, float* rate)
{
    DVLOG_FUNC(3);
    DCHECK(rate);
...
HRESULT MediaFoundationSourceWrapper::GetFastestRate(MFRATE_DIRECTION direction, BOOL supports_thinning, float* rate)
{
    DVLOG_FUNC(3);
    DCHECK(rate);
...
HRESULT MediaFoundationSourceWrapper::IsRateSupported(BOOL supports_thinning, float new_rate, float* supported_rate)
{
    ...
    MFRATE_DIRECTION direction = (new_rate >= 0) ? MFRATE_FORWARD : MFRATE_REVERSE;
...
HRESULT MediaFoundationSourceWrapper::SetRate(BOOL supports_thinning, float rate)
{
    DVLOG_FUNC(2);
...
HRESULT MediaFoundationSourceWrapper::GetRate(BOOL* supports_thinning, float* rate)
{
    DVLOG_FUNC(2);
...
void MediaFoundationSourceWrapper::CheckForEndOfPresentation()
{
    ...
    if (presentation_ended_) {
        ...
        DLOG_IF(ERROR, FAILED(hr)) << "Failed to notify end of presentation: " << PrintHr(hr);
    }
}
...
void MediaFoundationSourceWrapper::SetCdmProxy(scoped_refptr<MediaFoundationCdmProxy> cdm_proxy)
{
    DVLOG_FUNC(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    DLOG_IF(ERROR, FAILED(hr)) << "Failed to refresh trusted input: " << PrintHr(hr);
}
...
bool MediaFoundationSourceWrapper::SetVideoStreamEnabled(bool enabled)
{
    ...
    for (auto stream : media_streams_) {
        if (stream->IsSelected() && stream->StreamType() == DemuxerStream::Type::VIDEO) {
