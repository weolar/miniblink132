...
struct PendingInputBuffer {
    PendingInputBuffer(DemuxerStream::Status status, scoped_refptr<media::DecoderBuffer> buffer);
...
class MediaFoundationStreamWrapper
    : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::RuntimeClassType::ClassicCom>, IMFMediaStream> {
public:
    MediaFoundationStreamWrapper();
    ~MediaFoundationStreamWrapper() override;

    static HRESULT Create(int stream_id, IMFMediaSource* parent_source, DemuxerStream* demuxer_stream, std::unique_ptr<MediaLog> media_log,
        scoped_refptr<base::SequencedTaskRunner> task_runner, MediaFoundationStreamWrapper** stream_out);

    HRESULT RuntimeClassInitialize(int stream_id, IMFMediaSource* parent_source, DemuxerStream* demuxer_stream, std::unique_ptr<MediaLog> media_log);
    ...
    void OnDemuxerStreamRead(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer);
    // Receive the data from MojoDemuxerStreamAdapter.
    void OnDemuxerStreamReadBuffers(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers);
    ...
    IFACEMETHODIMP GetStreamDescriptor(IMFStreamDescriptor** stream_descriptor_out) override;
...
protected:
    ...
    HRESULT ServiceSampleRequest(IUnknown* token, DecoderBuffer* buffer) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    enum class State { kInitialized, kStarted, kStopped, kPaused } state_ = State::kInitialized;
    ...
    std::queue<Microsoft::WRL::ComPtr<IUnknown>> pending_sample_request_tokens_ GUARDED_BY(lock_);
    ...
    std::queue<scoped_refptr<DecoderBuffer>> post_flush_buffers_ GUARDED_BY(lock_);
