...
public:
    ...
    // additional decoders.
    explicit DefaultDecoderFactory(std::unique_ptr<DecoderFactory> external_decoder_factory);
    ...
    void CreateAudioDecoders(
        scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog* media_log, std::vector<std::unique_ptr<AudioDecoder>>* audio_decoders) final;
...
private:
    ...
    std::unique_ptr<DecoderFactory> external_decoder_factory_ GUARDED_BY(shutdown_lock_);
