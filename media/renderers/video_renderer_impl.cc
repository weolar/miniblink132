...
{
    return base::FeatureList::IsEnabled(kLowDelayVideoRenderingOnLiveStream) && stream->liveness() == StreamLiveness::kLive;
}

} // namespace

VideoRendererImpl::VideoRendererImpl(const scoped_refptr<base::SequencedTaskRunner>& media_task_runner, VideoRendererSink* sink,
    const CreateVideoDecodersCB& create_video_decoders_cb, bool drop_frames, MediaLog* media_log, std::unique_ptr<GpuMemoryBufferVideoFramePool> gmb_pool,
...
void VideoRendererImpl::Flush(base::OnceClosure callback)
{
    ...
    if (buffering_state_ != BUFFERING_HAVE_NOTHING) {
        buffering_state_ = BUFFERING_HAVE_NOTHING;
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoRendererImpl::OnBufferingStateChange, weak_factory_.GetWeakPtr(), buffering_state_));
    }
    received_end_of_stream_ = false;
    rendered_end_of_stream_ = false;
    ...
    video_decoder_stream_->Reset(base::BindOnce(&VideoRendererImpl::OnVideoDecoderStreamResetDone, weak_factory_.GetWeakPtr()));
    ...
    if (!latency_hint_.has_value() && !low_delay_) {
        min_buffered_frames_ = max_buffered_frames_ = initial_buffering_size_.value();
    }
}
...
void VideoRendererImpl::Initialize(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client,
    const TimeSource::WallClockTimeCB& wall_clock_time_cb, PipelineStatusCallback init_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media", "VideoRendererImpl::Initialize", TRACE_ID_LOCAL(this));
    ...
    video_decoder_stream_ = std::make_unique<VideoDecoderStream>(
        std::make_unique<VideoDecoderStream::StreamTraits>(media_log_), task_runner_, create_video_decoders_cb_, media_log_);
    video_decoder_stream_->set_config_change_observer(base::BindRepeating(&VideoRendererImpl::OnConfigChange, weak_factory_.GetWeakPtr()));
    video_decoder_stream_->set_fallback_observer(base::BindRepeating(&VideoRendererImpl::OnFallback, weak_factory_.GetWeakPtr()));
    if (gpu_memory_buffer_pool_) {
        video_decoder_stream_->SetPrepareCB(base::BindRepeating(&GpuMemoryBufferVideoFramePool::MaybeCreateHardwareFrame,
    ...
    video_decoder_stream_->Initialize(demuxer_stream_, base::BindOnce(&VideoRendererImpl::OnVideoDecoderStreamInitialized, weak_factory_.GetWeakPtr()),
        cdm_context, base::BindRepeating(&VideoRendererImpl::OnStatisticsUpdate, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&VideoRendererImpl::OnWaiting, weak_factory_.GetWeakPtr()));
}

scoped_refptr<VideoFrame> VideoRendererImpl::Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, RenderingMode rendering_mode)
{
    ...
    const bool background_rendering = rendering_mode == RenderingMode::kBackground;
    ...
    if (buffering_state_ == BUFFERING_HAVE_ENOUGH && !received_end_of_stream_ && !algorithm_->effective_frames_queued() && !background_rendering
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoRendererImpl::TransitionToHaveNothing, weak_factory_.GetWeakPtr()));
    }
    ...
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoRendererImpl::AttemptReadAndCheckForMetadataChanges, weak_factory_.GetWeakPtr(), result->format(), result->natural_size()));

    TRACE_EVENT_END1("media", "VideoRendererImpl::Render", "frame", result->AsHumanReadableString());
    return result;
}
...
void VideoRendererImpl::OnVideoDecoderStreamInitialized(bool success)
{
    ...
    algorithm_ = std::make_unique<VideoRendererAlgorithm>(wall_clock_time_cb_, media_log_);
    if (!drop_frames_)
...
{
    DCHECK(init_cb_);
    TRACE_EVENT_NESTABLE_ASYNC_END1("media", "VideoRendererImpl::Initialize", TRACE_ID_LOCAL(this), "status", PipelineStatusToString(status));
...
{
    DCHECK(flush_cb_);
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "VideoRendererImpl::Flush", TRACE_ID_LOCAL(this));
...
void VideoRendererImpl::OnBufferingStateChange(BufferingState buffering_state)
{
    ...
    if (state_ == kPlaying && buffering_state == BUFFERING_HAVE_NOTHING) {
        reason = video_decoder_stream_->is_demuxer_read_pending() ? DEMUXER_UNDERFLOW : DECODER_UNDERFLOW;
    }

    media_log_->AddEvent<MediaLogEvent::kBufferingStateChanged>(SerializableBufferingState<SerializableBufferingStateType::kVideo> { buffering_state, reason });
...
void VideoRendererImpl::SetTickClockForTesting(const base::TickClock* tick_clock)
{
    ...
    if (buffering_state_ == BUFFERING_HAVE_NOTHING) {
        ...
        if (!latency_hint_.has_value() && !low_delay_) {
            ...
            if (min_buffered_frames_ < kMaxUnderflowGrowth) {
                ...
                DVLOG(2) << __func__ << " Underflow! Increased min_buffered_frames_: " << min_buffered_frames_;
            }
        }
        ...
        if (max_buffered_frames_ < kMaxUnderflowGrowth) {
            ...
            DVLOG(2) << __func__ << " Underflow! Increased max_buffered_frames_: " << max_buffered_frames_;
        }
    }
}

void VideoRendererImpl::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
    base::AutoLock auto_lock(lock_);
    ...
    if (!latency_hint_.has_value()) {
        ...
        min_buffered_frames_ = max_buffered_frames_ = initial_buffering_size_.value();
        MEDIA_LOG(DEBUG, media_log_) << "Video latency hint cleared. Default buffer size (" << min_buffered_frames_ << " frames) restored";
        ...
        min_buffered_frames_ = 1;
        max_buffered_frames_ = initial_buffering_size_.value();
        MEDIA_LOG(DEBUG, media_log_) << "Video latency hint set:" << *latency_hint << ". "
...
void VideoRendererImpl::UpdateLatencyHintBufferingCaps_Locked(base::TimeDelta average_frame_duration)
{
    ...
    int latency_hint_frames = base::ClampRound(*latency_hint_ / average_frame_duration);
    ...
    max_buffered_frames_ = std::max(min_buffered_frames_, initial_buffering_size_.value());

    if (!is_latency_hint_media_logged_) {
        is_latency_hint_media_logged_ = true;
        MEDIA_LOG(DEBUG, media_log_) << "Video latency hint set:" << *latency_hint_ << ". "
                                     << "Effective buffering latency:" << (min_buffered_frames_ * average_frame_duration) << clamp_string;
    }
}
...
void VideoRendererImpl::FrameReady(VideoDecoderStream::ReadResult result)
{
    ...
    default:
        ...
        PipelineStatus::Codes code = result.code() == DecoderStatus::Codes::kDisconnected ? PIPELINE_ERROR_DISCONNECTED : PIPELINE_ERROR_DECODE;
        PipelineStatus status = { code, std::move(result).error() };
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoRendererImpl::OnPlaybackError, weak_factory_.GetWeakPtr(), std::move(status)));
        return;
    }
    ...
    if (!sink_started_ && !painted_first_frame_ && algorithm_->frames_queued()) {
        if (received_end_of_stream_ || (algorithm_->effective_frames_queued() && has_best_first_frame)) {
            ...
            paint_first_frame_cb_.Reset(base::BindOnce(&VideoRendererImpl::PaintFirstFrame, base::Unretained(this)));
            task_runner_->PostDelayedTask(FROM_HERE, paint_first_frame_cb_.callback(), base::Milliseconds(250));
        }
    }
...
bool VideoRendererImpl::HaveEnoughData_Locked() const
{
    ...
    if (min_buffered_frames_ > 1 && video_decoder_stream_->CanReadWithoutStalling()) {
        return false;
    }
...
void VideoRendererImpl::AttemptRead_Locked()
{
    ...
    case kPlaying:
        pending_read_ = true;
        video_decoder_stream_->Read(base::BindOnce(&VideoRendererImpl::FrameReady, cancel_on_flush_weak_factory_.GetWeakPtr()));
        return;
    case kUninitialized:
    case kInitializing:
    case kFlushing:
...
void VideoRendererImpl::UpdateStats_Locked(bool force_update)
{
    ...
    if (!force_update && !stats_.video_frames_decoded && !stats_.video_frames_dropped) {
        return;
    }

    if (stats_.video_frames_dropped) {
        TRACE_EVENT_INSTANT2("media", "VideoFramesDropped", TRACE_EVENT_SCOPE_THREAD, "count", stats_.video_frames_dropped, "id", player_id_);
    }
...
void VideoRendererImpl::ReportFrameRateIfNeeded_Locked()
{
    ...
    if (last_reported_fps_ && current_fps && *last_reported_fps_ == *current_fps) {
...
bool VideoRendererImpl::HaveReachedBufferingCap(size_t buffering_cap) const
{
    ...
    return algorithm_->effective_frames_queued() >= buffering_cap || algorithm_->frames_queued() >= kAbsoluteMaxFrames;
}
...
void VideoRendererImpl::MaybeFireEndedCallback_Locked(bool time_progressing)
{
    ...
    const bool have_frames_after_start_time = algorithm_->frames_queued() > 1 && !IsBeforeStartTime(algorithm_->last_frame());
    ...
    if (!algorithm_->effective_frames_queued()) {
        ...
        && (algorithm_->average_frame_duration().is_zero() || algorithm_->render_interval().is_zero() || !time_progressing)) {
        // We'll end up here if playback never started or there was only one frame.
        should_render_end_of_stream = true;
    } else if (algorithm_->frames_queued() == 1u && algorithm_->effective_frames_queued() == 1 && time_progressing) {
        const auto end_delay = std::max(base::TimeDelta(), algorithm_->last_frame_end_time() - tick_clock_->NowTicks());
    ...
    task_runner_->PostDelayedTask(
        FROM_HERE, base::BindOnce(&VideoRendererImpl::OnPlaybackEnded, cancel_on_flush_weak_factory_.GetWeakPtr()), ended_event_delay);
}

base::TimeTicks VideoRendererImpl::ConvertMediaTimestamp(base::TimeDelta media_time)
{
...
bool VideoRendererImpl::IsBeforeStartTime(const VideoFrame& frame)
{
    ...
    return frame.timestamp() + frame.metadata().frame_duration.value_or(last_decoder_stream_avg_duration_) < start_timestamp_;
}
...
bool VideoRendererImpl::HasBestFirstFrame(const VideoFrame& frame)
{
    ...
    return frame.timestamp() >= start_timestamp_
        || frame.timestamp() + frame.metadata().frame_duration.value_or(last_decoder_stream_avg_duration_) > start_timestamp_;
}
...
void VideoRendererImpl::RemoveFramesForUnderflowOrBackgroundRendering()
{
    ...
    if (!sink_started_ && !algorithm_->effective_frames_queued()) {
        ...
        algorithm_->Reset(VideoRendererAlgorithm::ResetFlag::kPreserveNextFrameEstimates);
        painted_first_frame_ = false;
        paint_first_frame_cb_.Cancel();
    ...
    if (buffering_state_ == BUFFERING_HAVE_NOTHING) {
        stats_.video_frames_dropped += algorithm_->RemoveExpiredFrames(current_time + algorithm_->average_frame_duration());
        return;
    }
...
void VideoRendererImpl::CheckForMetadataChanges(VideoPixelFormat pixel_format, const gfx::Size& natural_size)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void VideoRendererImpl::AttemptReadAndCheckForMetadataChanges(VideoPixelFormat pixel_format, const gfx::Size& natural_size)
{
    base::AutoLock auto_lock(lock_);
...
void VideoRendererImpl::PaintFirstFrame_Locked()
{
    ...
    auto first_frame = algorithm_->Render(base::TimeTicks(), base::TimeTicks(), nullptr);
    DCHECK(first_frame);
