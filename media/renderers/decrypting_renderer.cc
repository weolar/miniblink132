...
DecryptingRenderer::DecryptingRenderer(
    std::unique_ptr<Renderer> renderer, MediaLog* media_log, const scoped_refptr<base::SequencedTaskRunner> media_task_runner)
...
void DecryptingRenderer::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
    ...
    if (cdm_context_ && cdm_context_->GetDecryptor() && cdm_context_->GetDecryptor()->CanAlwaysDecrypt()) {
...
void DecryptingRenderer::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (cdm_context_->GetDecryptor() && cdm_context_->GetDecryptor()->CanAlwaysDecrypt()) {
...
void DecryptingRenderer::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
...
void DecryptingRenderer::SetWasPlayedWithUserActivationAndHighMediaEngagement(bool was_played_with_user_activation_and_high_media_engagement)
{
    renderer_->SetWasPlayedWithUserActivationAndHighMediaEngagement(was_played_with_user_activation_and_high_media_engagement);
}
...
void DecryptingRenderer::OnSelectedVideoTracksChanged(const std::vector<DemuxerStream*>& enabled_tracks, base::OnceClosure change_completed_cb)
{
    renderer_->OnSelectedVideoTracksChanged(enabled_tracks, std::move(change_completed_cb));
}

void DecryptingRenderer::OnEnabledAudioTracksChanged(const std::vector<DemuxerStream*>& enabled_tracks, base::OnceClosure change_completed_cb)
{
    renderer_->OnEnabledAudioTracksChanged(enabled_tracks, std::move(change_completed_cb));
}
...
void DecryptingRenderer::CreateAndInitializeDecryptingMediaResource()
{
    ...
    decrypting_media_resource_ = std::make_unique<DecryptingMediaResource>(media_resource_, cdm_context_, media_log_, media_task_runner_);
    decrypting_media_resource_->Initialize(base::BindOnce(&DecryptingRenderer::InitializeRenderer, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&DecryptingRenderer::OnWaiting, weak_factory_.GetWeakPtr()));
}
...
void DecryptingRenderer::InitializeRenderer(bool success)
{
    ...
    // encrypted streams.
    MediaResource* const maybe_decrypting_media_resource = decrypting_media_resource_ ? decrypting_media_resource_.get() : media_resource_.get();
    renderer_->Initialize(maybe_decrypting_media_resource, client_, std::move(init_cb_));
}
...
bool DecryptingRenderer::HasEncryptedStream()
{
    ...
    for (media::DemuxerStream* stream : media_resource_->GetAllStreams()) {
        if ((stream->type() == DemuxerStream::AUDIO && stream->audio_decoder_config().is_encrypted())
            || (stream->type() == DemuxerStream::VIDEO && stream->video_decoder_config().is_encrypted())) {
            return true;
        }
    }
