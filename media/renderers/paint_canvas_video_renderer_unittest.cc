...
scoped_refptr<VideoFrame> CreateTestY16Frame(const gfx::Size& coded_size, const gfx::Rect& visible_rect, void* external_memory, base::TimeDelta timestamp)
{
    ...
    return media::VideoFrame::WrapExternalData(
        media::PIXEL_FORMAT_Y16, coded_size, visible_rect, visible_rect.size(), static_cast<uint8_t*>(external_memory), byte_size, timestamp);
}

// Readback the contents of a RGBA texture into an array of RGBA values.
static std::unique_ptr<uint8_t[]> ReadbackTexture(gpu::gles2::GLES2Interface* gl, GLuint texture, const gfx::Size& size)
{
    ...
    gl->ReadPixels(0, 0, size.width(), size.height(), GL_RGBA, GL_UNSIGNED_BYTE, raw_pixels);
...
public:
    ...
    PaintCanvasVideoRendererTest& operator=(const PaintCanvasVideoRendererTest&) = delete;
    ...
    void Paint(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, Color color);
    void PaintRotated(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, const gfx::RectF& dest_rect, Color color, SkBlendMode mode,
    ...
    scoped_refptr<VideoFrame> natural_frame()
    {
        return natural_frame_;
    }
    scoped_refptr<VideoFrame> larger_frame()
    {
        return larger_frame_;
    }
    scoped_refptr<VideoFrame> smaller_frame()
    {
        return smaller_frame_;
    }
    ...
    // Standard canvas.
    cc::PaintCanvas* target_canvas()
    {
        return &target_canvas_;
    }
    SkBitmap* bitmap()
    {
        return &bitmap_;
...
{
    scoped_refptr<VideoFrame> cropped_frame
        = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, gfx::Size(16, 16), gfx::Rect(6, 6, 8, 6), gfx::Size(8, 6), base::Milliseconds(4));
    ...
    EXPECT_EQ(cropped_frame->visible_rect().width() * kHeight, cropped_frame->visible_rect().height() * kWidth);
    ...
    libyuv::I420Copy(cropped_y_plane, 16, cropped_u_plane, 8, cropped_v_plane, 8, cropped_frame->writable_data(VideoFrame::Plane::kY),
        cropped_frame->stride(VideoFrame::Plane::kY), cropped_frame->writable_data(VideoFrame::Plane::kU), cropped_frame->stride(VideoFrame::Plane::kU),
        cropped_frame->writable_data(VideoFrame::Plane::kV), cropped_frame->stride(VideoFrame::Plane::kV), 16, 16);
...
PaintCanvasVideoRendererTest::PaintCanvasVideoRendererTest()
    ...
    , larger_frame_(VideoFrame::CreateBlackFrame(gfx::Size(kWidth * 2, kHeight * 2)))
    , smaller_frame_(VideoFrame::CreateBlackFrame(gfx::Size(kWidth / 2, kHeight / 2)))
...
void PaintCanvasVideoRendererTest::FillFrameWithColor(scoped_refptr<VideoFrame> video_frame, Color color)
{
    switch (color) {
    case kNone:
        break;
...
void PaintCanvasVideoRendererTest::Paint(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, Color color)
{
    PaintRotated(std::move(video_frame), canvas, kNaturalRect, color, SkBlendMode::kSrcOver, kNoTransformation);
}

void PaintCanvasVideoRendererTest::PaintRotated(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas, const gfx::RectF& dest_rect, Color color,
    SkBlendMode mode, VideoTransformation video_transformation)
{
...
void PaintCanvasVideoRendererTest::Copy(scoped_refptr<VideoFrame> video_frame, cc::PaintCanvas* canvas)
{
...
TEST_F(PaintCanvasVideoRendererTest, TransparentFrame)
{
    target_canvas()->clear(SkColors::kRed);
    PaintRotated(
        VideoFrame::CreateTransparentFrame(gfx::Size(kWidth, kHeight)).get(), target_canvas(), kNaturalRect, kNone, SkBlendMode::kSrcOver, kNoTransformation);
...
TEST_F(PaintCanvasVideoRendererTest, TransparentFrameSrcMode)
{
    ...
    PaintRotated(
        VideoFrame::CreateTransparentFrame(gfx::Size(kWidth, kHeight)).get(), target_canvas(), kNaturalRect, kNone, SkBlendMode::kSrc, kNoTransformation);
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), bitmap()->getColor(0, 0));
}
...
TEST_F(PaintCanvasVideoRendererTest, TransparentFrameSrcMode1x1)
{
    ...
    PaintRotated(frame.get(), target_canvas(), gfx::RectF(1, 1), kNone, SkBlendMode::kSrc, kNoTransformation);
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), bitmap()->getColor(0, 0));
}
...
{
    target_canvas()->clear(SkColors::kRed);
    Copy(VideoFrame::CreateTransparentFrame(gfx::Size(kWidth, kHeight)).get(), target_canvas());
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), bitmap()->getColor(0, 0));
}
...
TEST_F(PaintCanvasVideoRendererTest, CroppedFrame)
{
    ...
    EXPECT_EQ(SK_ColorBLACK, bitmap()->getColor(kWidth * 1 / 8 - 1, kHeight * 1 / 6 - 1));
    EXPECT_EQ(SK_ColorRED, bitmap()->getColor(kWidth * 3 / 8, kHeight * 1 / 6 - 1));
    EXPECT_EQ(SK_ColorGREEN, bitmap()->getColor(kWidth * 1 / 8 - 1, kHeight * 3 / 6));
...
TEST_F(PaintCanvasVideoRendererTest, CroppedFrameToRGBParallel)
{
    ...
    // size must be even.
    auto test_frame
        = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, gfx::Size(3840, 2160), gfx::Rect(1440, 810, 1920, 810), gfx::Size(1920, 810), base::TimeDelta());

    // Fill in the frame with the same data as the cropped frame.
    libyuv::I420Scale(cropped_frame()->data(0), cropped_frame()->stride(0), cropped_frame()->data(1), cropped_frame()->stride(1), cropped_frame()->data(2),
        cropped_frame()->stride(2), cropped_frame()->coded_size().width(), cropped_frame()->coded_size().height(), test_frame->writable_data(0),
        test_frame->stride(0), test_frame->writable_data(1), test_frame->stride(1), test_frame->writable_data(2), test_frame->stride(2),
        test_frame->coded_size().width(), test_frame->coded_size().height(), libyuv::kFilterNone);
    ...
    std::unique_ptr<uint8_t, base::AlignedFreeDeleter> memory(
        static_cast<uint8_t*>(base::AlignedAlloc(allocation_size, media::VideoFrame::kFrameAddressAlignment)));
    memset(memory.get(), 0, allocation_size);

    PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(test_frame.get(), memory.get(), row_bytes);
    ...
    EXPECT_EQ(MaybeConvertABGRToARGB(SK_ColorRED), rgb_pixels[visible_size.width() - 1]);
    EXPECT_EQ(SK_ColorGREEN, rgb_pixels[visible_size.width() * (visible_size.height() - 1)]);
    EXPECT_EQ(MaybeConvertABGRToARGB(SK_ColorBLUE), rgb_pixels[(visible_size.width() - 1) * visible_size.height()]);
}
...
TEST_F(PaintCanvasVideoRendererTest, CroppedFrame_NoScaling)
{
    ...
    // cause scaling.
    canvas.scale(static_cast<SkScalar>(crop_rect.width()) / kWidth, static_cast<SkScalar>(crop_rect.height()) / kHeight);

    Paint(cropped_frame(), &canvas, kNone);

    // Check the corners.
    EXPECT_EQ(SK_ColorBLACK, bitmap.getColor(offset_x, offset_y));
    EXPECT_EQ(SK_ColorRED, bitmap.getColor(offset_x + crop_rect.width() - 1, offset_y));
    EXPECT_EQ(SK_ColorGREEN, bitmap.getColor(offset_x, offset_y + crop_rect.height() - 1));
    EXPECT_EQ(SK_ColorBLUE, bitmap.getColor(offset_x + crop_rect.width() - 1, offset_y + crop_rect.height() - 1));
}
...
TEST_F(PaintCanvasVideoRendererTest, Video_Rotation_90)
{
    SkBitmap bitmap = AllocBitmap(kWidth, kHeight);
    cc::SkiaPaintCanvas canvas(bitmap);
    PaintRotated(cropped_frame(), &canvas, kNaturalRect, kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_90));
    // Check the corners.
...
TEST_F(PaintCanvasVideoRendererTest, Video_Rotation_180)
{
    SkBitmap bitmap = AllocBitmap(kWidth, kHeight);
    cc::SkiaPaintCanvas canvas(bitmap);
    PaintRotated(cropped_frame(), &canvas, kNaturalRect, kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_180));
    // Check the corners.
    EXPECT_EQ(SK_ColorBLUE, bitmap.getColor(0, 0));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Rotation_270)
{
    SkBitmap bitmap = AllocBitmap(kWidth, kHeight);
    cc::SkiaPaintCanvas canvas(bitmap);
    PaintRotated(cropped_frame(), &canvas, kNaturalRect, kNone, SkBlendMode::kSrcOver, VideoTransformation(VIDEO_ROTATION_270));
    // Check the corners.
    EXPECT_EQ(SK_ColorRED, bitmap.getColor(0, 0));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Translate)
{
    ...
    PaintRotated(cropped_frame(), &canvas, gfx::RectF(kWidth / 2, kHeight / 2, kWidth / 2, kHeight / 2), kNone, SkBlendMode::kSrcOver, kNoTransformation);
    ...
    EXPECT_EQ(SK_ColorMAGENTA, bitmap.getColor((kWidth / 2) - 1, (kHeight / 2) - 1));
...
TEST_F(PaintCanvasVideoRendererTest, Video_Translate_Rotation_90)
{
    ...
    PaintRotated(cropped_frame(), &canvas, gfx::RectF(kWidth / 2, kHeight / 2, kWidth / 2, kHeight / 2), kNone, SkBlendMode::kSrcOver,
...
TEST_F(PaintCanvasVideoRendererTest, HighBitDepth)
{
    ...
    } kBitDepthAndFormats[] = { { 9, PIXEL_FORMAT_YUV420P9 }, { 10, PIXEL_FORMAT_YUV420P10 }, { 12, PIXEL_FORMAT_YUV420P12 } };
    for (const auto param : kBitDepthAndFormats) {
        // Copy cropped_frame into a highbit frame.
        scoped_refptr<VideoFrame> frame(VideoFrame::CreateFrame(
            param.format, cropped_frame()->coded_size(), cropped_frame()->visible_rect(), cropped_frame()->natural_size(), cropped_frame()->timestamp()));
...
TEST_F(PaintCanvasVideoRendererTest, Y16)
{
    ...
    std::unique_ptr<unsigned char, base::AlignedFreeDeleter> memory(
        static_cast<unsigned char*>(base::AlignedAlloc(byte_size, media::VideoFrame::kFrameAddressAlignment)));
    const gfx::Rect rect(offset_x, offset_y, bitmap.width(), bitmap.height());
    auto video_frame = CreateTestY16Frame(gfx::Size(stride, offset_y + bitmap.height()), rect, memory.get(), cropped_frame()->timestamp());
    ...
    renderer_.Paint(std::move(video_frame), &canvas, flags, paint_params, nullptr);
...
TEST_F(PaintCanvasVideoRendererTest, Yuv420P12OddWidth)
{
    ...
    std::unique_ptr<uint16_t[]> y_plane = std::make_unique<uint16_t[]>(kImgWidth * kImgHeight);
    std::unique_ptr<uint16_t[]> u_plane = std::make_unique<uint16_t[]>(kUvWidth * kUvHeight);
    std::unique_ptr<uint16_t[]> v_plane = std::make_unique<uint16_t[]>(kUvWidth * kUvHeight);
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapExternalYuvData(
        PIXEL_FORMAT_YUV420P12, size, gfx::Rect(size), size, y_stride, uv_stride, uv_stride, y_data, u_data, v_data, base::TimeDelta());

    std::unique_ptr<uint32_t[]> rgba = std::make_unique<uint32_t[]>(kImgWidth * kImgHeight);
    PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(frame.get(), rgba.get(), frame->visible_rect().width() * 4,
...
TEST_F(PaintCanvasVideoRendererTest, I420WithFilters)
{
    ...
    constexpr int kUvWidth = (kImgWidth + 1) / 2;
    constexpr int kUvHeight = (kImgHeight + 1) / 2;
    std::unique_ptr<uint8_t[]> y_plane = std::make_unique<uint8_t[]>(kImgWidth * kImgHeight);
    std::unique_ptr<uint8_t[]> u_plane = std::make_unique<uint8_t[]>(kUvWidth * kUvHeight);
    std::unique_ptr<uint8_t[]> v_plane = std::make_unique<uint8_t[]>(kUvWidth * kUvHeight);
    ...
    auto size = gfx::Size(kImgWidth, kImgHeight);
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapExternalYuvData(
        PIXEL_FORMAT_I420, size, gfx::Rect(size), size, kImgWidth, kUvWidth, kUvWidth, y_plane.get(), u_plane.get(), v_plane.get(), base::TimeDelta());
...
public:
        ...
        GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels) override
    {
        if (!teximage2d_callback_.is_null()) {
            teximage2d_callback_.Run(target, level, internalformat, width, height, border, format, type, pixels);
        }
    }
        ...
        GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* pixels) override
    {
        if (!texsubimage2d_callback_.is_null()) {
            texsubimage2d_callback_.Run(target, level, xoffset, yoffset, width, height, format, type, pixels);
        }
    }
...
TEST_F(PaintCanvasVideoRendererTest, ContextLost)
{
    ...
    scoped_refptr<gpu::ClientSharedImage> shared_image = gpu::ClientSharedImage::CreateForTesting();
    auto video_frame = VideoFrame::WrapSharedImage(
        PIXEL_FORMAT_NV12, shared_image, gpu::SyncToken(), base::BindOnce(MailboxHoldersReleased), size, gfx::Rect(size), size, kNoTimestamp);

    cc::PaintFlags flags;
    flags.setFilterQuality(cc::PaintFlags::FilterQuality::kLow);
    PaintCanvasVideoRenderer::PaintParams params;
    params.dest_rect = kNaturalRect;
    renderer_.Paint(std::move(video_frame), &canvas, flags, params, context_provider.get());
}
#endif
...
TEST_F(PaintCanvasVideoRendererTest, CorrectFrameSizeToVisibleRect)
{
    ...
    auto video_frame = media::VideoFrame::WrapExternalData(
        media::PIXEL_FORMAT_Y16, coded_size, gfx::Rect(visible_size), visible_size, &memory[0], fWidth * fHeight * 2, base::Milliseconds(4));
...
TEST_F(PaintCanvasVideoRendererTest, TexImage2D_Y16_RGBA32F)
{
    ...
    auto video_frame = CreateTestY16Frame(gfx::Size(stride, offset_y + height), rect, memory.get(), cropped_frame()->timestamp());

    TestGLES2Interface gles2;
    // Bind the texImage2D callback to verify the uint16 to float32 conversion.
    gles2.teximage2d_callback_ = base::BindRepeating(
        [](GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void* pixels) {
    ...
    PaintCanvasVideoRenderer::TexImage2D(
        GL_TEXTURE_2D, 0, &gles2, gpu::Capabilities(), video_frame.get(), 0, GL_RGBA, GL_RGBA, GL_FLOAT, true /*flip_y*/, true);
}
...
TEST_F(PaintCanvasVideoRendererTest, TexSubImage2D_Y16_R32F)
{
    ...
    TestGLES2Interface gles2;
    // Bind the texImage2D callback to verify the uint16 to float32 conversion.
    gles2.texsubimage2d_callback_ = base::BindRepeating(
        [](GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* pixels) {
            EXPECT_EQ(static_cast<unsigned>(GL_FLOAT), type);
    ...
    PaintCanvasVideoRenderer::TexSubImage2D(
        GL_TEXTURE_2D, &gles2, video_frame.get(), 0, GL_RED, GL_FLOAT, 2 /*xoffset*/, 1 /*yoffset*/, false /*flip_y*/, true);
}
...
public:
    ...
    void SetUp() override
    {
        ...
        media_context_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kGpuRaster, /*support_locking=*/false);
        gpu::ContextResult result = media_context_->BindToCurrentSequence();
        ASSERT_EQ(result, gpu::ContextResult::kSuccess);

        raster_context_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kGpuRaster, /*support_locking=*/false);
        result = raster_context_->BindToCurrentSequence();
        ASSERT_EQ(result, gpu::ContextResult::kSuccess);

        destination_context_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kGLES2, /*support_locking=*/false);
    ...
    template <class CheckPixels> void CopyVideoFrameTexturesAndCheckPixels(scoped_refptr<VideoFrame> frame, CheckPixels check_pixels)
    {
        ...
        renderer_.CopyVideoFrameTexturesToGLTexture(media_context_.get(), destination_gl, frame, target, texture, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0,
            false /* premultiply_alpha */, false /* flip_y */);

        gfx::Size expected_size = frame->visible_rect().size();

        std::unique_ptr<uint8_t[]> pixels = ReadbackTexture(destination_gl, texture, expected_size);
    ...
    template <class CheckPixels> void PaintVideoFrameAndCheckPixels(scoped_refptr<VideoFrame> frame, CheckPixels check_pixels)
    {
        gfx::Size expected_size = frame->visible_rect().size();
        SkBitmap bitmap = AllocBitmap(expected_size.width(), expected_size.height());
        cc::SkiaPaintCanvas canvas(bitmap);
        canvas.clear(SkColors::kGray);
        renderer_.Copy(frame, &canvas, media_context_.get());

        auto get_color = base::BindRepeating([](SkBitmap* bitmap, int x, int y) { return bitmap->getColor(x, y); }, &bitmap);
        check_pixels(get_color);
    }
    ...
    {
        return CreateSharedImageRGBAFrame(raster_context_, gfx::Size(16, 8), gfx::Rect(3, 3, 12, 4), std::move(closure));
    }
    ...
    {
        return CreateSharedImageI420Frame(raster_context_, gfx::Size(16, 8), gfx::Rect(2, 2, 12, 4), std::move(closure));
    }
    // Creates a cropped I420 VideoFrame. |closure| is run once the shared images
    // backing the VideoFrame have been destroyed.
    scoped_refptr<VideoFrame> CreateTestI420FrameNotSubset(base::OnceClosure closure)
    {
        return CreateSharedImageI420Frame(raster_context_, gfx::Size(16, 8), gfx::Rect(0, 0, 16, 8), std::move(closure));
    }
    ...
    {
        return CreateSharedImageNV12Frame(raster_context_, gfx::Size(16, 8), gfx::Rect(2, 2, 12, 4), std::move(closure));
    }
...
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameYUVDataToGLTexture)
{
    ...
    renderer_.CopyVideoFrameYUVDataToGLTexture(media_context_.get(), destination_gl, cropped_frame(), target, texture, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE, 0,
        false /* premultiply_alpha */, false /* flip_y */);
...
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameYUVDataToGLTexture_FlipY)
{
    auto* destination_gl = destination_context_->ContextGL();
    DCHECK(destination_gl);
    GLenum target = GL_TEXTURE_2D;
...
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameTexturesToGLTextureRGBA)
{
...
// cropping.
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameTexturesToGLTextureRGBA_ReadLockFence)
{
    base::RunLoop run_loop;
    scoped_refptr<VideoFrame> frame = CreateTestRGBAFrame(run_loop.QuitClosure());
...
// CopyVideoFrameYUVDataToGLTexture, including correct cropping.
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameTexturesToGLTextureI420)
{
    base::RunLoop run_loop;
...
{
    base::RunLoop run_loop;
    scoped_refptr<VideoFrame> frame = CreateTestI420FrameNotSubset(run_loop.QuitClosure());
...
// CopyVideoFrameYUVDataToGLTexture, including correct cropping.
TEST_F(PaintCanvasVideoRendererWithGLTest, CopyVideoFrameTexturesToGLTextureNV12)
{
    base::RunLoop run_loop;
