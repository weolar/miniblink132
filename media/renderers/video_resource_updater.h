...
class MEDIA_EXPORT VideoResourceUpdater : public base::trace_event::MemoryDumpProvider {
public:
    ...
    VideoResourceUpdater(viz::RasterContextProvider* context_provider, viz::SharedBitmapReporter* shared_bitmap_reporter,
    ...
    // is the video.
    void AppendQuad(viz::CompositorRenderPass* render_pass, scoped_refptr<VideoFrame> frame, gfx::Transform transform, gfx::Rect quad_rect,
        gfx::Rect visible_quad_rect, const gfx::MaskFilterInfo& mask_filter_info, std::optional<gfx::Rect> clip_rect, bool context_opaque, float draw_opacity,
        int sorting_context_id);

    // TODO(kylechar): This is only public for testing, make private.
    VideoFrameExternalResource CreateExternalResourceFromVideoFrame(scoped_refptr<VideoFrame> video_frame);
...
private:
    ...
    {
        return context_provider_ == nullptr;
    }
    ...
    PlaneResource* RecycleOrAllocateResource(
        const gfx::Size& resource_size, viz::SharedImageFormat si_format, const gfx::ColorSpace& color_space, VideoFrame::ID unique_id);
    PlaneResource* AllocateResource(const gfx::Size& plane_size, viz::SharedImageFormat format, const gfx::ColorSpace& color_space);
    ...
    void CopyHardwarePlane(VideoFrame* video_frame, VideoFrameExternalResource* external_resources);
    ...
    VideoFrameExternalResource CreateForHardwarePlanes(scoped_refptr<VideoFrame> video_frame);
    ...
    void TransferRGBPixelsToPaintCanvas(scoped_refptr<VideoFrame> video_frame, PlaneResource* plane_resource);

    // Write/copy RGB pixels from video frame to hardware resource through
    // WritePixels or TexSubImage2D.
    bool WriteRGBPixelsToTexture(scoped_refptr<VideoFrame> video_frame, PlaneResource* plane_resource, viz::SharedImageFormat output_si_format);
    ...
    // format.
    bool WriteYUVPixelsForAllPlanesToTexture(scoped_refptr<VideoFrame> video_frame, HardwarePlaneResource* resource, size_t bits_per_channel);
    ...
    VideoFrameExternalResource CreateForSoftwarePlanes(scoped_refptr<VideoFrame> video_frame);

    gpu::raster::RasterInterface* RasterInterface();

    void RecycleResource(uint32_t plane_resource_id, const gpu::SyncToken& sync_token, bool lost_resource);
    void ReturnTexture(
    ...
    const raw_ptr<viz::ClientResourceProvider, DanglingUntriaged> resource_provider_;
    ...
    std::unique_ptr<uint8_t[], base::UncheckedFreeDeleter> upload_pixels_[SkYUVAInfo::kMaxPlanes] = {};
