...
public:
    ...
    RendererImplTest()
        ...
        , renderer_impl_(new RendererImpl(
              task_environment_.GetMainThreadTaskRunner(), std::unique_ptr<AudioRenderer>(audio_renderer_), std::unique_ptr<VideoRenderer>(video_renderer_)))
...
protected:
    ...
    void SetAudioRendererInitializeExpectations(PipelineStatus status)
    {
        EXPECT_CALL(*audio_renderer_, OnInitialize(audio_stream_.get(), _, _, _))
            .WillOnce(DoAll(SaveArg<2>(&audio_renderer_client_), RunOnceCallback<3>(status)));
    }
    ...
    void SetVideoRendererInitializeExpectations(PipelineStatus status)
    {
        EXPECT_CALL(*video_renderer_, OnInitialize(video_stream_.get(), _, _, _, _))
            .WillOnce(DoAll(SaveArg<2>(&video_renderer_client_), RunOnceCallback<4>(status)));
    }

    void InitializeAndExpect(PipelineStatus start_status)
    {
        EXPECT_CALL(callbacks_, OnInitialize(start_status)).WillOnce(SaveArg<0>(&initialization_status_));
        ...
        if (start_status == PIPELINE_OK && audio_stream_) {
            EXPECT_CALL(*audio_renderer_, GetTimeSource()).WillOnce(Return(&time_source_));
        } else {
            renderer_impl_->set_time_source_for_testing(&time_source_);
        }

        renderer_impl_->Initialize(demuxer_.get(), &callbacks_, base::BindOnce(&CallbackHelper::OnInitialize, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();

        if (start_status == PIPELINE_OK && audio_stream_) {
            ON_CALL(*audio_renderer_, Flush(_)).WillByDefault([this](base::OnceClosure on_done) {
                audio_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN);
                std::move(on_done).Run();
            });
            ON_CALL(*audio_renderer_, StartPlaying())
                .WillByDefault(SetBufferingState(&audio_renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        }
        if (start_status == PIPELINE_OK && video_stream_) {
            ON_CALL(*video_renderer_, Flush(_)).WillByDefault([this](base::OnceClosure on_done) {
                video_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN);
                std::move(on_done).Run();
            });
            ON_CALL(*video_renderer_, StartPlayingFrom(_))
                .WillByDefault(SetBufferingState(&video_renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        }
    }
    ...
    void InitializeWithAudio()
    {
        ...
        EXPECT_CALL(*demuxer_, GetAllStreams()).Times(3).WillRepeatedly(Return(streams_));
    ...
    void Flush(bool underflowed)
    {
        ...
        renderer_impl_->Flush(base::BindOnce(&CallbackHelper::OnFlushed, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    {
        EXPECT_CALL(callbacks_, OnCdmAttached(expected_result)).WillOnce(SaveArg<0>(&is_cdm_set_));
        renderer_impl_->SetCdm(cdm_context_.get(), base::BindOnce(&CallbackHelper::OnCdmAttached, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
...
TEST_F(RendererImplTest, ErrorDuringFlush)
{
    ...
    InSequence s;
    EXPECT_CALL(time_source_, StopTicking());
    EXPECT_CALL(*audio_renderer_, Flush(_)).WillOnce([this](base::OnceClosure on_done) {
        audio_renderer_client_->OnError(PIPELINE_ERROR_DECODE);
        std::move(on_done).Run();
    });
    EXPECT_CALL(callbacks_, OnError(HasStatusCode(PIPELINE_ERROR_DECODE)));
...
TEST_F(RendererImplTest, ErrorDuringInitialize)
{
    ...
    EXPECT_CALL(*video_renderer_, OnInitialize(video_stream_.get(), _, _, _, _))
        .WillOnce(DoAll(SetError(&audio_renderer_client_, PIPELINE_ERROR_DECODE), SaveArg<2>(&video_renderer_client_), RunOnceCallback<4>(PIPELINE_OK)));
...
TEST_F(RendererImplTest, VideoUnderflowWithAudioVideoRecovers)
{
    ...
    // Underflow should be delayed when both audio and video are present and video
    // underflows.
    EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN)).Times(0);
    video_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN);
    Mock::VerifyAndClearExpectations(&time_source_);

    // If video recovers, the underflow should never occur.
    video_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN);
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(RendererImplTest, AudioTrackDisableThenEnable)
{
    ...
    SetAudioTrackSwitchExpectations();
    renderer_impl_->OnEnabledAudioTracksChanged({ streams_[0] }, enable_wait.QuitClosure());
    enable_wait.Run();
}
...
TEST_F(RendererImplTest, VideoTrackDisableThenEnable)
{
    ...
    base::RunLoop enable_wait;
    SetVideoTrackSwitchExpectations();
    renderer_impl_->OnSelectedVideoTracksChanged({ streams_[1] }, enable_wait.QuitClosure());
    enable_wait.Run();
...
TEST_F(RendererImplTest, AudioUnderflowDuringAudioTrackChange)
{
    ...
    EXPECT_CALL(*audio_renderer_, Flush(_)).WillOnce(MoveArg(&audio_renderer_flush_cb));
...
TEST_F(RendererImplTest, VideoUnderflowDuringVideoTrackChange)
{
    ...
    base::OnceClosure video_renderer_flush_cb;
    {
        InSequence track_switch_seq;
        EXPECT_CALL(time_source_, CurrentMediaTime());
        EXPECT_CALL(*video_renderer_, Flush(_)).WillOnce(MoveArg(&video_renderer_flush_cb));
        EXPECT_CALL(*video_renderer_, StartPlayingFrom(_));
        EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    }
...
TEST_F(RendererImplTest, VideoResumedFromUnderflowDuringAudioTrackChange)
{
    ...
    EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN)).WillOnce(RunClosure(underflow_wait.QuitClosure()));
    EXPECT_CALL(time_source_, StopTicking());
    video_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN);
    underflow_wait.Run();
...
TEST_F(RendererImplTest, AudioResumedFromUnderflowDuringVideoTrackChange)
{
    ...
    EXPECT_CALL(callbacks_, OnBufferingStateChange(_, _)).Times(0);
    EXPECT_CALL(time_source_, StartTicking()).Times(0);
    audio_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN);
