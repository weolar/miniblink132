...
class MEDIA_EXPORT AudioRendererImpl : public AudioRenderer, public TimeSource, public base::PowerSuspendObserver, public AudioRendererSink::RenderCallback {
public:
    ...
    using TranscribeAudioCallback = base::RepeatingCallback<void(scoped_refptr<AudioBuffer>)>;

    using EnableSpeechRecognitionCallback = base::OnceCallback<void(TranscribeAudioCallback)>;
    ...
    AudioRendererImpl(const scoped_refptr<base::SequencedTaskRunner>& task_runner, AudioRendererSink* sink,
    ...
    bool GetWallClockTimes(const std::vector<base::TimeDelta>& media_timestamps, std::vector<base::TimeTicks>* wall_clock_times) override;
    ...
    bool was_unmuted_for_testing() const
    {
        return was_unmuted_;
    }
...
private:
    ...
    void OnDeviceInfoReceived(DemuxerStream* stream, CdmContext* cdm_context, OutputDeviceInfo output_device_info);
    ...
    int Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, const AudioGlitchInfo& glitch_info, AudioBus* dest) override;
    ...
    raw_ptr<SpeechRecognitionClient, DanglingUntriaged> speech_recognition_client_;
