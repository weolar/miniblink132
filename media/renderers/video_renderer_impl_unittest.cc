...
public:
    std::vector<std::unique_ptr<VideoDecoder>> CreateVideoDecodersForTest()
    {
        ...
        ON_CALL(*decoder_, Initialize_(_, _, _, _, _, _))
            .WillByDefault(
                DoAll(SaveArg<4>(&output_cb_), RunOnceCallback<3>(expect_init_success_ ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed)));
        // Monitor decodes from the decoder.
        ON_CALL(*decoder_, Decode_(_, _)).WillByDefault(Invoke(this, &VideoRendererImplTest::DecodeRequested));
        ON_CALL(*decoder_, Reset_(_)).WillByDefault(Invoke(this, &VideoRendererImplTest::FlushRequested));
    ...
    {
        null_video_sink_ = std::make_unique<NullVideoSink>(false, base::Seconds(1.0 / 60),
            base::BindRepeating(&MockCB::FrameReceived, base::Unretained(&mock_cb_)), base::SingleThreadTaskRunner::GetCurrentDefault());

        renderer_ = std::make_unique<VideoRendererImpl>(base::SingleThreadTaskRunner::GetCurrentDefault(), null_video_sink_.get(),
            base::BindRepeating(&VideoRendererImplTest::CreateVideoDecodersForTest, base::Unretained(this)), true, &media_log_, nullptr, 0);
        ...
        ON_CALL(demuxer_stream_, OnRead(_)).WillByDefault(Invoke(this, &VideoRendererImplTest::OnDemuxerRead));
    }
    ...
    void InitializeRenderer(MockDemuxerStream* demuxer_stream, bool low_delay, bool expect_success)
    {
        SCOPED_TRACE(base::StringPrintf("InitializeRenderer(%d)", expect_success));
        expect_init_success_ = expect_success;
        WaitableMessageLoopEvent event;
        CallInitialize(demuxer_stream, event.GetPipelineStatusCB(), low_delay, expect_success);
        event.RunAndWaitForStatus(expect_success ? PIPELINE_OK : DECODER_ERROR_NOT_SUPPORTED);
    }

    void CallInitialize(MockDemuxerStream* demuxer_stream, PipelineStatusCallback status_cb, bool low_delay, bool expect_success)
    {
        if (low_delay)
        ...
        renderer_->Initialize(demuxer_stream, nullptr, &mock_cb_, base::BindRepeating(&WallClockTimeSource::GetWallClockTimes, base::Unretained(&time_source_)),
            std::move(status_cb));
    }
    ...
    {
        for (std::string_view token : base::SplitString(str, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
            if (token == "abort") {
            ...
            auto ts_tokens = base::SplitStringPiece(token, "d", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
            if (ts_tokens.size() > 1) {
                token = ts_tokens[0];
            }
            ...
            if (base::StringToInt(token, &timestamp_in_ms)) {
                ...
                scoped_refptr<VideoFrame> frame
                    = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, natural_size, gfx::Rect(natural_size), natural_size, base::Milliseconds(timestamp_in_ms));

                int duration_in_ms = 0;
                if (ts_tokens.size() > 1 && base::StringToInt(ts_tokens[1], &duration_in_ms)) {
    ...
    void WaitForError(PipelineStatus expected)
    {
        ...
        EXPECT_CALL(mock_cb_, OnError(_)).WillOnce(Invoke([cb = &error_cb](PipelineStatus status) { std::move(*cb).Run(status); }));
    ...
    void SatisfyPendingDecode()
    {
        ...
        if (frame.get())
            task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(output_cb_, frame));
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb_), decode_results_.front().first));
    ...
    void SatisfyPendingDecodeWithEndOfStream()
    {
        ...
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb_), DecoderStatus::Codes::kOk));
    ...
    {
        return decode_results_.size() > 0;
    }

    void AdvanceWallclockTimeInMs(int time_ms)
    {
        EXPECT_TRUE(task_environment_.GetMainThreadTaskRunner()->BelongsToCurrentThread());
...
protected:
    ...
    raw_ptr<NiceMock<MockVideoDecoder>, DanglingUntriaged> decoder_; // Owned by |renderer_|.
...
private:
    void DecodeRequested(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB& decode_cb)
    {
        EXPECT_TRUE(task_environment_.GetMainThreadTaskRunner()->BelongsToCurrentThread());
        CHECK(!decode_cb_);
    ...
    void FlushRequested(base::OnceClosure& callback)
    {
        ...
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, std::move(callback));
    }
    ...
    base::circular_deque<std::pair<DecoderStatus, scoped_refptr<VideoFrame>>> decode_results_;
};
...
TEST_F(VideoRendererImplTest, InitializeAndEndOfStream)
{
    ...
    WaitForPendingDecode();
    {
        ...
        WaitableMessageLoopEvent event;
        {
            ...
            EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).WillOnce(RunOnceClosure(event.GetClosure()));
...
{
    CallInitialize(&demuxer_stream_, NewExpectedStatusCB(PIPELINE_ERROR_ABORT), false, PIPELINE_OK);
    Destroy();
}
...
static void VideoRendererImplTest_FlushDoneCB(VideoRendererImplTest* test, VideoRenderer* renderer, base::OnceClosure success_cb)
{
...
TEST_F(VideoRendererImplTest, FlushCallbackNoLock)
{
    ...
    StartPlayingFrom(0);
    WaitableMessageLoopEvent event;
    renderer_->Flush(base::BindOnce(&VideoRendererImplTest_FlushDoneCB, base::Unretained(this), base::Unretained(renderer_.get()), event.GetClosure()));
    event.RunAndWait();
    Destroy();
}
...
TEST_F(VideoRendererImplTest, DecodeError_Playing)
{
    ...
    // be called.
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _));
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).Times(testing::AtMost(1));
    EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(AnyNumber());
    EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(_)).Times(1);
    EXPECT_CALL(mock_cb_, OnVideoOpacityChange(_)).Times(1);
...
TEST_F(VideoRendererImplTest, StartPlayingFrom_LowDelay)
{
    ...
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).Times(AnyNumber());
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).Times(AnyNumber());
    EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(AnyNumber());
    EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(_)).Times(1);
    EXPECT_CALL(mock_cb_, OnVideoOpacityChange(_)).Times(1);
    StartPlayingFrom(10);
    ...
    WaitableMessageLoopEvent event;
    EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(20))).WillOnce(RunOnceClosure(event.GetClosure()));
    AdvanceTimeInMs(20);
    event.RunAndWait();
...
TEST_F(VideoRendererImplTest, RenderingStopsAfterFirstFrame)
{
...

    {
        ...
        EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(0))).WillOnce(RunOnceClosure(event.GetClosure()));
        StartPlayingFrom(0);
...
TEST_F(VideoRendererImplTest, RenderingStartedThenStopped)
{
    ...
    last_pipeline_statistics.video_frames_dropped = 1;
    {
        WaitableMessageLoopEvent event;
        EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).WillOnce(RunOnceClosure(event.GetClosure()));
        EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(5).WillRepeatedly(SaveArg<0>(&last_pipeline_statistics));
        EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(0)));
        EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(_)).Times(1);
        EXPECT_CALL(mock_cb_, OnVideoOpacityChange(_)).Times(1);
        StartPlayingFrom(0);
    ...
    EXPECT_EQ(renderer_->GetPreferredRenderInterval(), last_pipeline_statistics.video_frame_duration_average);

    // Consider the case that rendering is faster than we setup the test event.
    // In that case, when we run out of the frames, BUFFERING_HAVE_NOTHING will
    // be called. And then during SatisfyPendingDecodeWithEndOfStream,
    // BUFFER_HAVE_ENOUGH will be called again.
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).Times(testing::AtMost(1));
    EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).Times(testing::AtMost(1));
    renderer_->OnTimeProgressing();
    time_source_.StartTicking();
    ...
    EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).WillOnce(SaveArg<0>(&last_pipeline_statistics));
    SatisfyPendingDecodeWithEndOfStream();
...
TEST_F(VideoRendererImplTest, UnderflowEvictionBeforeEOS)
{
...

    {
        ...
        WaitableMessageLoopEvent event;
        EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).WillOnce(RunOnceClosure(event.GetClosure()));
        renderer_->OnTimeProgressing();
        time_source_.StartTicking();
...
TEST_F(VideoRendererImplTest, UnderflowEvictionWhileHaveEnough)
{
    ...
    // Now wait until we have no effective frames.
    {
        ...
        AdvanceTimeInMs(1000);
        renderer_->OnTimeProgressing();
        EXPECT_CALL(mock_cb_, FrameReceived(_)).WillOnce(RunOnceClosure(event.GetClosure()));
        event.RunAndWait();
...
TEST_F(VideoRendererImplTest, FramesAreNotExpiredDuringPreroll)
{
    ...
    EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(10))).WillOnce(RunOnceClosure(event.GetClosure()));
    AdvanceTimeInMs(10);
    event.RunAndWait();
...
TEST_F(VideoRendererImplTest, VideoConfigChange)
{
    ...
    EXPECT_CALL(demuxer_stream_, SupportsConfigChanges()).WillRepeatedly(Return(true));

    // Signal a config change at the next DemuxerStream::Read().
    DemuxerStream::DecoderBufferVector buffers;
    EXPECT_CALL(demuxer_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kConfigChanged, buffers));
    ...
    EXPECT_CALL(mock_cb_, OnVideoConfigChange(DecoderConfigEq(TestVideoConfig::LargeEncrypted()))).Times(1);
...
TEST_F(VideoRendererImplTest, NaturalSizeChange)
{
    ...
    QueueFrame(
        DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, initial_size, gfx::Rect(initial_size), initial_size, base::Milliseconds(0)));
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, larger_size, gfx::Rect(larger_size), larger_size, base::Milliseconds(10)));
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, larger_size, gfx::Rect(larger_size), larger_size, base::Milliseconds(20)));
    QueueFrame(
        DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(PIXEL_FORMAT_I420, initial_size, gfx::Rect(initial_size), initial_size, base::Milliseconds(30)));
    ...
    }
    {
        ...
        EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(initial_size));
        WaitableMessageLoopEvent event;
        EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(30))).WillOnce(RunOnceClosure(event.GetClosure()));
        AdvanceTimeInMs(10);
        event.RunAndWait();
    }
...
TEST_F(VideoRendererImplTest, OpacityChange)
{
    ...
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(non_opaque_format, frame_size, gfx::Rect(frame_size), frame_size, base::Milliseconds(0)));
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(non_opaque_format, frame_size, gfx::Rect(frame_size), frame_size, base::Milliseconds(10)));
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(opaque_format, frame_size, gfx::Rect(frame_size), frame_size, base::Milliseconds(20)));
    QueueFrame(DecoderStatus::Codes::kOk, VideoFrame::CreateFrame(opaque_format, frame_size, gfx::Rect(frame_size), frame_size, base::Milliseconds(30)));
...
public:
    ...
    {
        renderer_ = std::make_unique<VideoRendererImpl>(base::SingleThreadTaskRunner::GetCurrentDefault(), null_video_sink_.get(),
            base::BindRepeating(&VideoRendererImplAsyncAddFrameReadyTest::CreateVideoDecodersForTest, base::Unretained(this)), true, &media_log_,
            std::make_unique<MockGpuMemoryBufferVideoFramePool>(&frame_ready_cbs_), 0);
...
class UnderflowTest : public VideoRendererImplTest, public testing::WithParamInterface<::std::tuple<UnderflowTestType, BufferingStateChangeReason>> {
protected:
    void SetUp() override
    {
        std::tie(test_type, underflow_type) = GetParam();
    }
    ...
    void TestBufferToHaveEnoughThenUnderflow()
    {
...

        {
            WaitableMessageLoopEvent event;
            EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(0)));
            EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN)).WillOnce(RunOnceClosure(event.GetClosure()));
            EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(AnyNumber());
            EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(_)).Times(1);
            EXPECT_CALL(mock_cb_, OnVideoOpacityChange(_)).Times(1);
            StartPlayingFrom(0);
        ...
        // Frames should be dropped and we should NOT signal having nothing.
        {
            ...
            EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(60))).WillOnce(RunOnceClosure(event.GetClosure()));
            EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(AnyNumber());
            AdvanceTimeInMs(61);
        ...
        // the last frame up for display.
        {
            SCOPED_TRACE("Waiting for BUFFERING_HAVE_NOTHING");
            WaitableMessageLoopEvent event;
            EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, underflow_type)).WillOnce(RunOnceClosure(event.GetClosure()));
            AdvanceTimeInMs(18);
            event.RunAndWait();
            Mock::VerifyAndClearExpectations(&mock_cb_);
        }
        ...
        case UnderflowTestType::CANT_READ_WITHOUT_STALLING:
            EXPECT_EQ(renderer_->min_buffered_frames_for_testing(), limits::kMaxVideoFrames + 1);
            EXPECT_EQ(renderer_->max_buffered_frames_for_testing(), limits::kMaxVideoFrames + 1);
            break;
...
INSTANTIATE_TEST_SUITE_P(ChrisTest, UnderflowTest,
    Combine(Values(UnderflowTestType::NORMAL, UnderflowTestType::LOW_DELAY, UnderflowTestType::CANT_READ_WITHOUT_STALLING),
...
public:
    void VerifyDefaultRebufferingBehavior(int start_playing_from)
    {
        ...
        EXPECT_CALL(mock_cb_, FrameReceived(HasTimestampMatcher(start_playing_from)));
        EXPECT_CALL(mock_cb_, OnStatisticsUpdate(_)).Times(AnyNumber());
        EXPECT_CALL(mock_cb_, OnVideoNaturalSizeChange(_)).Times(AnyNumber());
        ...
        EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).Times(0);
...
TEST_F(VideoRendererLatencyHintTest, LatencyHintUnderflowUpdatesMaxBufferingLimit)
{
    ...
    // Advance time to trigger HAVE_NOTHING (underflow).
    {
        ...
        EXPECT_CALL(mock_cb_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, DEMUXER_UNDERFLOW)).WillOnce(RunOnceClosure(event.GetClosure()));
        renderer_->OnTimeProgressing();
        time_source_.StartTicking();
        AdvanceTimeInMs(300);
        event.RunAndWait();
...
TEST_F(VideoRendererLatencyHintTest, CantReadWithoutStallingOverridesLatencyHint)
{
    Initialize();
