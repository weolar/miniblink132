...
public:
    ...
    const uint8_t* last_upload() const
    {
        return last_upload_;
    }
...
protected:
    ...
    std::unique_ptr<VideoResourceUpdater> CreateUpdaterForHardware(bool use_stream_video_draw_quad = false)
    {
        return std::make_unique<VideoResourceUpdater>(context_provider_.get(), nullptr, resource_provider_.get(),
    ...
    std::unique_ptr<VideoResourceUpdater> CreateUpdaterForSoftware()
    {
        return std::make_unique<VideoResourceUpdater>(
            /*context_provider=*/nullptr, &shared_bitmap_reporter_, resource_provider_.get(), /*shared_image_interface=*/nullptr,
    ...
    scoped_refptr<VideoFrame> CreateTestYUVVideoFrame(const gfx::Size& size = gfx::Size(10, 10))
    {
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalYuvData(PIXEL_FORMAT_I422, // format
            size, // coded_size
    ...
    scoped_refptr<VideoFrame> CreateTestRGBVideoFrame(VideoPixelFormat format)
    {
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalData(format, // format
            kSize, // coded_size
    ...
    scoped_refptr<VideoFrame> CreateTestRGBVideoFrameWithVisibleRect(VideoPixelFormat format)
    {
    ...
    scoped_refptr<VideoFrame> CreateTestY16VideoFrameWithVisibleRect()
    {
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalData(PIXEL_FORMAT_Y16,
            kSize, // coded_size
            kVisibleRect, // visible_rect
            kVisibleRect.size(), // natural_size
    ...
    scoped_refptr<VideoFrame> CreateTestHighBitFrame()
    {
        ...
        scoped_refptr<VideoFrame> video_frame(VideoFrame::CreateFrame(PIXEL_FORMAT_YUV420P10, size, gfx::Rect(size), size, base::TimeDelta()));
        EXPECT_TRUE(video_frame);
        return video_frame;
    }
    ...
    scoped_refptr<VideoFrame> CreateNV12TestFrame()
    {
        ...
        scoped_refptr<VideoFrame> video_frame(VideoFrame::CreateFrame(PIXEL_FORMAT_NV12, size, gfx::Rect(size), size, base::TimeDelta()));
        EXPECT_TRUE(video_frame);
        return video_frame;
    }
    ...
    scoped_refptr<VideoFrame> CreateP010TestFrame()
    {
        ...
        scoped_refptr<VideoFrame> video_frame(VideoFrame::CreateFrame(PIXEL_FORMAT_P010LE, size, gfx::Rect(size), size, base::TimeDelta()));
        EXPECT_TRUE(video_frame);
        return video_frame;
    }
    ...
    scoped_refptr<VideoFrame> CreateTestHardwareVideoFrame(viz::SharedImageFormat si_format, VideoPixelFormat format, unsigned target)
    {
        const int kDimension = 10;
        gfx::Size size(kDimension, kDimension);

        scoped_refptr<gpu::ClientSharedImage> shared_image = gpu::ClientSharedImage::CreateForTesting(si_format, target);
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapSharedImage(format, shared_image, kMailboxSyncToken,
            base::BindOnce(&VideoResourceUpdaterTest::SetReleaseSyncToken, base::Unretained(this)),
            size, // coded_size
            gfx::Rect(size), // visible_rect
            size, // natural_size
            base::TimeDelta()); // timestamp
    ...
    {
        return CreateTestHardwareVideoFrame(viz::SinglePlaneFormat::kRGBA_8888, PIXEL_FORMAT_ARGB, GL_TEXTURE_2D);
    }

    scoped_refptr<VideoFrame> CreateTestStreamTextureHardwareVideoFrame(bool needs_copy)
    {
        scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(viz::SinglePlaneFormat::kRGBA_8888, PIXEL_FORMAT_ARGB, GL_TEXTURE_EXTERNAL_OES);
...
const gpu::SyncToken VideoResourceUpdaterTest::kMailboxSyncToken
    = gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 7);
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrame)
{
    ...
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrameNV12)
{
    ...
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(CreateNV12TestFrame());
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
...
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    for (const auto& fmt : { PIXEL_FORMAT_XBGR, PIXEL_FORMAT_XRGB, PIXEL_FORMAT_ABGR, PIXEL_FORMAT_ARGB }) {
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrameRGBNonOrigin)
{
    ...
    for (const auto& fmt : { PIXEL_FORMAT_XBGR, PIXEL_FORMAT_XRGB, PIXEL_FORMAT_ABGR, PIXEL_FORMAT_ARGB }) {
        ...
        const auto bytes_per_element = VideoFrame::BytesPerElement(fmt, VideoFrame::Plane::kARGB);
        auto* dest_pixels = gl_->last_upload() + rect.y() * bytes_per_row + rect.x() * bytes_per_element;
...
TEST_F(VideoResourceUpdaterTest, SoftwareFrameY16NonOrigin)
{
    ...
    const auto bytes_per_row = VideoFrame::RowBytes(VideoFrame::Plane::kARGB, kOutputFormat, video_frame->coded_size().width());
    const auto bytes_per_element = VideoFrame::BytesPerElement(kOutputFormat, VideoFrame::Plane::kARGB);
    auto* dest_pixels = gl_->last_upload() + rect.y() * bytes_per_row + rect.x() * bytes_per_element;
...
TEST_F(VideoResourceUpdaterTestWithF16, HighBitFrame)
{
    ...
    // Create the resource again, to test the path where the
    // resource are cached.
    VideoFrameExternalResource resource2 = updater->CreateExternalResourceFromVideoFrame(video_frame);
    // With multiplanar shared images, a TextureDrawQuad is created instead of
    // a YUVDrawQuad.
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
}
...
public:
    VideoResourceUpdaterTestWithR16()
        : VideoResourceUpdaterTest()
    {
...
TEST_F(VideoResourceUpdaterTest, ChangeResourceizeSoftwareCompositor)
{
    ...
    // Allocate the resource for a software video frame.
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(CreateTestYUVVideoFrame(kSize1));
    // Expect exactly one allocated shared bitmap.
    EXPECT_EQ(1u, shared_bitmap_reporter_.shared_bitmaps().size());
    auto shared_bitmaps_copy = shared_bitmap_reporter_.shared_bitmaps();
    ...
    resource = updater->CreateExternalResourceFromVideoFrame(CreateTestYUVVideoFrame(kSize2));
...
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kI420, PIXEL_FORMAT_I420, GL_TEXTURE_RECTANGLE_ARB);
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
    EXPECT_TRUE(resource.release_callback);
    EXPECT_EQ(viz::MultiPlaneFormat::kI420, resource.resource.format);
    EXPECT_EQ(resource.resource.synchronization_type, viz::TransferableResource::SynchronizationType::kSyncToken);

    video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kI420, PIXEL_FORMAT_I420, GL_TEXTURE_RECTANGLE_ARB);
    video_frame->metadata().read_lock_fences_enabled = true;

    resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    EXPECT_EQ(resource.resource.synchronization_type, viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted);
}

TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_StreamTexture_CopyToNewTexture)
{
    // Note that |use_stream_video_draw_quad| is true for this test.
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware(true);
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestStreamTextureHardwareVideoFrame(/*needs_copy=*/false);

    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::STREAM_TEXTURE, resource.type);
    EXPECT_TRUE(resource.release_callback);
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resource.resource.texture_target());
    EXPECT_EQ(0u, GetSharedImageCount());
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_DCompSurface)
{
    ...
    updater->AppendQuad(
        ...
        /*visible_quad_rect=*/gfx::Rect(video_frame->coded_size()), gfx::MaskFilterInfo(), /*clip_rect=*/std::nullopt,
        /*context_opaque=*/true, /*draw_opacity=*/1.0,
        /*sorting_context_id=*/0);

    EXPECT_EQ(1u, pass->quad_list.size());

    const viz::TextureDrawQuad* quad = pass->quad_list.ElementAt(0)->DynamicCast<viz::TextureDrawQuad>();
...
TEST_F(VideoResourceUpdaterTest, PassReleaseSyncToken)
{
    ...
    const gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 123);
...
TEST_F(VideoResourceUpdaterTest, GenerateReleaseSyncToken)
{
    ...
    const gpu::SyncToken sync_token1(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 123);

    const gpu::SyncToken sync_token2(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 234);

    {
        scoped_refptr<VideoFrame> video_frame = CreateTestRGBAHardwareVideoFrame();

        VideoFrameExternalResource resource1 = updater->CreateExternalResourceFromVideoFrame(video_frame);
...
TEST_F(VideoResourceUpdaterTest, GenerateSyncTokenOnTextureCopy)
{
    ...
    scoped_refptr<VideoFrame> video_frame = CreateTestStreamTextureHardwareVideoFrame(/*needs_copy=*/true);
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_SingleNV12)
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kNV12, PIXEL_FORMAT_NV12, GL_TEXTURE_EXTERNAL_OES);
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
    EXPECT_EQ((GLenum)GL_TEXTURE_EXTERNAL_OES, resource.resource.texture_target());
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_DualNV12_SharedImageFormat)
{
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kNV12, PIXEL_FORMAT_NV12, GL_TEXTURE_RECTANGLE_ARB);
    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    // Setting to kSharedImageFormat, resource type should bo RGB.
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
    EXPECT_EQ((GLenum)GL_TEXTURE_RECTANGLE_ARB, resource.resource.texture_target());
    EXPECT_EQ(viz::MultiPlaneFormat::kNV12, resource.resource.format);
    EXPECT_EQ(0u, GetSharedImageCount());

    video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kNV12, PIXEL_FORMAT_NV12, GL_TEXTURE_EXTERNAL_OES);
...
TEST_F(VideoResourceUpdaterTest, CreateForHardwarePlanes_SingleP010HDR)
{
    ...
    hdr_metadata.smpte_st_2086 = gfx::HdrMetadataSmpteSt2086(SkNamedPrimariesExt::kP3,
        /*luminance_max=*/1000,
        /*luminance_min=*/0);
    std::unique_ptr<VideoResourceUpdater> updater = CreateUpdaterForHardware();
    EXPECT_EQ(0u, GetSharedImageCount());
    scoped_refptr<VideoFrame> video_frame = CreateTestHardwareVideoFrame(viz::MultiPlaneFormat::kP010, PIXEL_FORMAT_P010LE, GL_TEXTURE_EXTERNAL_OES);
    video_frame->set_color_space(kHDR10ColorSpace);
    video_frame->set_hdr_metadata(hdr_metadata);

    VideoFrameExternalResource resource = updater->CreateExternalResourceFromVideoFrame(video_frame);
    EXPECT_EQ(VideoFrameResourceType::RGB, resource.type);
    EXPECT_EQ(static_cast<GLenum>(GL_TEXTURE_EXTERNAL_OES), resource.resource.texture_target());
