...
int CalculateVariance(const std::vector<int>& values)
{
    ...
    const int mean = std::accumulate(values.begin(), values.end(), 0) / values.size();
...
void AudioDelayStatsReporter::ReportDelay(base::TimeDelta capture_delay, base::TimeDelta render_delay)
{
    const base::TimeDelta total_delay = capture_delay + render_delay;

    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Audio.Processing.CaptureDelayMs", capture_delay, delay_histogram_min_, delay_histogram_max_, 50);
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Audio.Processing.RenderDelayMs", render_delay, delay_histogram_min_, delay_histogram_max_, 50);
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Audio.Processing.TotalDelayMs", total_delay, delay_histogram_min_, delay_histogram_max_, 50);
    ...
    if (capture_delays_ms_.size() == static_cast<unsigned long>(variance_window_size_)) {
        ...
        UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Audio.Processing.CaptureDelayVarianceMs", delay_variance, 1, 500, 50);
        capture_delays_ms_.clear();

        delay_variance = CalculateVariance(render_delays_ms_);
        UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Audio.Processing.RenderDelayVarianceMs", delay_variance, 1, 500, 50);
        render_delays_ms_.clear();

        delay_variance = CalculateVariance(total_delays_ms_);
        UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Audio.Processing.TotalDelayVarianceMs", delay_variance, 1, 500, 50);
