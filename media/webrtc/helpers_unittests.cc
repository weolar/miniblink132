...
webrtc::AudioProcessing::Config CreateApmGetConfig(const AudioProcessingSettings& settings)
{
    rtc::scoped_refptr<webrtc::AudioProcessing> apm = CreateWebRtcAudioProcessingModule(settings);
    DCHECK(!!apm);
...
TEST(CreateWebRtcAudioProcessingModuleTest, Agc2ConfigUnchangedIfAgcSettingsDisabled)
{
...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
TEST(CreateWebRtcAudioProcessingModuleTest, InputVolumeAdjustmentEnabledWithAgc2)
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kWebRtcAllowInputVolumeAdjustment);
    auto config = CreateApmGetConfig(
...
TEST(CreateWebRtcAudioProcessingModuleTest, CanDisableInputVolumeAdjustmentWithAgc2)
{
    ::base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(features::kWebRtcAllowInputVolumeAdjustment);
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = true });
...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
TEST(CreateWebRtcAudioProcessingModuleTest, OnlyOneInputVolumeControllerEnabledOnDesktopPlatforms)
{
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = true });
    ...
    // respectively.
    bool agc1_enabled = config.gain_controller1.enabled && config.gain_controller1.analog_gain_controller.enabled;
    bool agc2_enabled = config.gain_controller2.enabled && config.gain_controller2.input_volume_controller.enabled;
...
#else
TEST(CreateWebRtcAudioProcessingModuleTest, InputVolumeControllerDisabledOnNonDesktopPlatforms)
{
    auto config = CreateApmGetConfig(
        /*settings=*/ { .automatic_gain_control = true });
    if (!config.gain_controller1.enabled && !config.gain_controller2.enabled) {
    ...
    if (config.gain_controller1.enabled) {
        EXPECT_NE(config.gain_controller1.mode, webrtc::AudioProcessing::Config::GainController1::Mode::kAdaptiveAnalog);
        EXPECT_FALSE(config.gain_controller1.analog_gain_controller.enabled);
    }
