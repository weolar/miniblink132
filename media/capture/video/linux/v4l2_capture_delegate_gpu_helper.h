...
public:
    ...
    V4L2CaptureDelegateGpuHelper& operator=(const V4L2CaptureDelegateGpuHelper&) = delete;
...
public:
    ...
    int OnIncomingCapturedData(VideoCaptureDevice::Client* client, const uint8_t* sample, size_t sample_size, const VideoCaptureFormat& format,
        const gfx::ColorSpace& data_color_space, int rotation, base::TimeTicks reference_time, base::TimeDelta timestamp, int frame_feedback_id = 0);

private:
    int ConvertCaptureDataToNV12(const uint8_t* sample, size_t sample_size, const VideoCaptureFormat& format, const gfx::Size& dimensions,
        const gfx::ColorSpace& data_color_space, int rotation, uint8_t* dst_y, uint8_t* dst_uv, int dst_stride_y, int dst_stride_uv);

    // This method directly converts the sample data into `NV12` format when the
    // input `VideoCaptureFormat` supported.
    int FastConvertToNV12(
        const uint8_t* sample, size_t sample_size, const VideoCaptureFormat& format, uint8_t* dst_y, uint8_t* dst_uv, int dst_stride_y, int dst_stride_uv);
