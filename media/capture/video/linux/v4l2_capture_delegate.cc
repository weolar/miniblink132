...
struct {
    ...
    uint32_t class_id;
} constexpr kControls[] = { { V4L2_CID_USER_BASE, V4L2_CID_USER_CLASS }, { V4L2_CID_CAMERA_CLASS_BASE, V4L2_CID_CAMERA_CLASS } };

// Fill in |format| with the given parameters.
void FillV4L2Format(v4l2_format* format, uint32_t width, uint32_t height, uint32_t pixelformat_fourcc)
{
...
class V4L2CaptureDelegate::BufferTracker : public base::RefCounted<BufferTracker> {
public:
...
// static
VideoPixelFormat V4L2CaptureDelegate::V4l2FourCcToChromiumPixelFormat(uint32_t v4l2_fourcc)
{
    for (const auto& fourcc_and_pixel_format : kSupportedFormatsAndPlanarity) {
...
// static
std::vector<uint32_t> V4L2CaptureDelegate::GetListOfUsableFourCcs(bool prefer_mjpeg)
{
...
// static
bool V4L2CaptureDelegate::IsControllableControl(int control_id, const base::RepeatingCallback<int(int, void*)>& do_ioctl)
{
    ...
    case V4L2_CID_EXPOSURE_ABSOLUTE:
        ...
        return special_control.value == V4L2_EXPOSURE_MANUAL || special_control.value == V4L2_EXPOSURE_SHUTTER_PRIORITY;
    case V4L2_CID_IRIS_ABSOLUTE:
        DCHECK_EQ(special_control_id, V4L2_CID_EXPOSURE_AUTO);
        // For a V4L2_CID_EXPOSURE_AUTO special control, |special_control.value|
        // is an enum v4l2_exposure_auto_type.
        // Check if the iris is manual. Exposure time may be manual or automatic.
        return special_control.value == V4L2_EXPOSURE_MANUAL || special_control.value == V4L2_EXPOSURE_APERTURE_PRIORITY;
    case V4L2_CID_FOCUS_ABSOLUTE:
    case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
...
V4L2CaptureDelegate::V4L2CaptureDelegate(V4L2CaptureDevice* v4l2, const VideoCaptureDeviceDescriptor& device_descriptor,
    const scoped_refptr<base::SingleThreadTaskRunner>& v4l2_task_runner, int power_line_frequency, int rotation)
    : v4l2_(v4l2)
...
void V4L2CaptureDelegate::AllocateAndStart(int width, int height, float frame_rate, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "V4L2CaptureDelegate::AllocateAndStart");
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
    DCHECK(client);
    client_ = std::move(client);

    // Need to open camera with O_RDWR after Linux kernel 3.3.
    device_fd_.reset(HANDLE_EINTR(v4l2_->open(device_descriptor_.device_id.c_str(), O_RDWR)));
    if (!device_fd_.is_valid()) {
        SetErrorState(VideoCaptureError::kV4L2FailedToOpenV4L2DeviceDriverFile, FROM_HERE, "Failed to open V4L2 device driver file.");
        return;
    }
    ...
    if (!(DoIoctl(VIDIOC_QUERYCAP, &cap) == 0
            && (((cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) && !(cap.capabilities & V4L2_CAP_VIDEO_OUTPUT))
                || ((cap.capabilities & V4L2_CAP_DEVICE_CAPS) && (cap.device_caps & V4L2_CAP_VIDEO_CAPTURE) && !(cap.device_caps & V4L2_CAP_VIDEO_OUTPUT))))) {
        device_fd_.reset();
        SetErrorState(VideoCaptureError::kV4L2ThisIsNotAV4L2VideoCaptureDevice, FROM_HERE, "This is not a V4L2 video capture device");
        return;
    }

    // Get supported video formats in preferred order. For large resolutions,
    // favour mjpeg over raw formats.
    const std::vector<uint32_t>& desired_v4l2_formats = GetListOfUsableFourCcs(width > kMjpegWidth || height > kMjpegHeight);
    ...
    if (best == desired_v4l2_formats.end()) {
        SetErrorState(VideoCaptureError::kV4L2FailedToFindASupportedCameraFormat, FROM_HERE, "Failed to find a supported camera format.");
        return;
    }
    ...
    if (DoIoctl(VIDIOC_S_FMT, &video_fmt_) < 0) {
        SetErrorState(VideoCaptureError::kV4L2FailedToSetVideoCaptureFormat, FROM_HERE, "Failed to set video capture format");
        return;
    }
    const VideoPixelFormat pixel_format = V4l2FourCcToChromiumPixelFormat(video_fmt_.fmt.pix.pixelformat);
    if (pixel_format == PIXEL_FORMAT_UNKNOWN) {
        SetErrorState(VideoCaptureError::kV4L2UnsupportedPixelFormat, FROM_HERE, "Unsupported pixel format");
        return;
    }
    ...
    if (DoIoctl(VIDIOC_G_PARM, &streamparm) >= 0) {
        ...
        if (streamparm.parm.capture.capability & V4L2_CAP_TIMEPERFRAME) {
            ...
            streamparm.parm.capture.timeperframe.denominator = (frame_rate) ? (frame_rate * kFrameRatePrecision) : (kTypicalFramerate * kFrameRatePrecision);

            if (DoIoctl(VIDIOC_S_PARM, &streamparm) < 0) {
                SetErrorState(VideoCaptureError::kV4L2FailedToSetCameraFramerate, FROM_HERE, "Failed to set camera framerate");
                return;
            }
            DVLOG(2) << "Actual camera driverframerate: " << streamparm.parm.capture.timeperframe.denominator << "/"
    ...
    if ((power_line_frequency_ == V4L2_CID_POWER_LINE_FREQUENCY_50HZ) || (power_line_frequency_ == V4L2_CID_POWER_LINE_FREQUENCY_60HZ)
    ...
    v4l2_task_runner_->PostTask(FROM_HERE, base::BindOnce(&V4L2CaptureDelegate::DoCapture, GetWeakPtr()));
}

void V4L2CaptureDelegate::StopAndDeAllocate()
{
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "V4L2CaptureDelegate::StopAndDeAllocate");
    StopStream();
...
void V4L2CaptureDelegate::TakePhoto(VideoCaptureDevice::TakePhotoCallback callback)
{
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
    take_photo_callbacks_.push(std::move(callback));
}

void V4L2CaptureDelegate::GetPhotoState(VideoCaptureDevice::GetPhotoStateCallback callback)
{
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
    ...
    photo_capabilities->focus_distance = RetrieveUserControlRange(V4L2_CID_FOCUS_ABSOLUTE);
    ...
    if (RunIoctl(VIDIOC_QUERYCTRL, &auto_focus_ctrl) && RunIoctl(VIDIOC_G_CTRL, &current_auto_focus)) {
        photo_capabilities->current_focus_mode = current_auto_focus.value ? MeteringMode::CONTINUOUS : MeteringMode::MANUAL;
        photo_capabilities->supported_focus_modes.push_back(MeteringMode::CONTINUOUS);
    }

    // Determines the exposure time of the camera sensor. Drivers interpret values
    // as 100 µs units, same as specs.
    photo_capabilities->exposure_time = RetrieveUserControlRange(V4L2_CID_EXPOSURE_ABSOLUTE);
    if (IsNonEmptyRange(photo_capabilities->exposure_time)) {
        photo_capabilities->supported_exposure_modes.push_back(MeteringMode::MANUAL);
    }
    ...
    if (RunIoctl(VIDIOC_QUERYCTRL, &auto_exposure_ctrl) && RunIoctl(VIDIOC_G_CTRL, &current_auto_exposure)) {
        photo_capabilities->current_exposure_mode
            = (current_auto_exposure.value == V4L2_EXPOSURE_MANUAL || current_auto_exposure.value == V4L2_EXPOSURE_SHUTTER_PRIORITY) ? MeteringMode::MANUAL
                                                                                                                                     : MeteringMode::CONTINUOUS;
        photo_capabilities->supported_exposure_modes.push_back(MeteringMode::CONTINUOUS);
    }
    ...
    photo_capabilities->exposure_compensation = RetrieveUserControlRange(V4L2_CID_AUTO_EXPOSURE_BIAS);

    photo_capabilities->color_temperature = RetrieveUserControlRange(V4L2_CID_WHITE_BALANCE_TEMPERATURE);
    if (IsNonEmptyRange(photo_capabilities->color_temperature)) {
        photo_capabilities->supported_white_balance_modes.push_back(MeteringMode::MANUAL);
    }
    ...
    auto_white_balance_ctrl.id = current_auto_white_balance.id = V4L2_CID_AUTO_WHITE_BALANCE;
    if (RunIoctl(VIDIOC_QUERYCTRL, &auto_white_balance_ctrl) && RunIoctl(VIDIOC_G_CTRL, &current_auto_white_balance)) {
        photo_capabilities->current_white_balance_mode = current_auto_white_balance.value ? MeteringMode::CONTINUOUS : MeteringMode::MANUAL;
        photo_capabilities->supported_white_balance_modes.push_back(MeteringMode::CONTINUOUS);
    }
    ...
    photo_capabilities->brightness = RetrieveUserControlRange(V4L2_CID_BRIGHTNESS);
    photo_capabilities->contrast = RetrieveUserControlRange(V4L2_CID_CONTRAST);
    photo_capabilities->saturation = RetrieveUserControlRange(V4L2_CID_SATURATION);
    photo_capabilities->sharpness = RetrieveUserControlRange(V4L2_CID_SHARPNESS);

    std::move(callback).Run(std::move(photo_capabilities));
}

void V4L2CaptureDelegate::SetPhotoOptions(mojom::PhotoSettingsPtr settings, VideoCaptureDevice::SetPhotoOptionsCallback callback)
{
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
    if (!device_fd_.is_valid() || !is_capturing_)
        return;
    ...
    if (settings->has_focus_mode && (settings->focus_mode == mojom::MeteringMode::MANUAL || settings->focus_mode == mojom::MeteringMode::CONTINUOUS)) {
        v4l2_control set_auto_focus = {};
        set_auto_focus.id = V4L2_CID_FOCUS_AUTO;
        set_auto_focus.value = settings->focus_mode == mojom::MeteringMode::CONTINUOUS;
        if (DoIoctl(VIDIOC_S_CTRL, &set_auto_focus) < 0) {
            DPLOG(ERROR) << "setting focus mode to " << (settings->focus_mode == mojom::MeteringMode::CONTINUOUS ? "continuous" : "manual");
        }
    ...
    if (settings->has_white_balance_mode
        && (settings->white_balance_mode == mojom::MeteringMode::CONTINUOUS || settings->white_balance_mode == mojom::MeteringMode::MANUAL)) {
        v4l2_control set_auto_white_balance = {};
        set_auto_white_balance.id = V4L2_CID_AUTO_WHITE_BALANCE;
        set_auto_white_balance.value = settings->white_balance_mode == mojom::MeteringMode::CONTINUOUS;
        if (DoIoctl(VIDIOC_S_CTRL, &set_auto_white_balance) < 0) {
            DPLOG(ERROR) << "setting white balance mode to " << (settings->white_balance_mode == mojom::MeteringMode::CONTINUOUS ? "continuous" : "manual");
        }
        special_controls_maybe_changed = true;
    }
    ...
    if (settings->has_color_temperature) {
        ...
        if (result >= 0 && !current_auto_white_balance.value) {
            ...
            if (DoIoctl(VIDIOC_S_CTRL, &set_temperature) < 0) {
                DPLOG(ERROR) << "setting color temperature to " << settings->color_temperature;
            }
        }
    }

    if (settings->has_exposure_mode && (settings->exposure_mode == mojom::MeteringMode::CONTINUOUS || settings->exposure_mode == mojom::MeteringMode::MANUAL)) {
        ...
        set_auto_exposure.value = settings->exposure_mode == mojom::MeteringMode::CONTINUOUS ? V4L2_EXPOSURE_APERTURE_PRIORITY // Auto exposure time and manual
                                                                                             // iris.
                                                                                             : V4L2_EXPOSURE_MANUAL; // Manual exposure time and manual iris.
        if (DoIoctl(VIDIOC_S_CTRL, &set_auto_exposure) < 0) {
            DPLOG(ERROR) << "setting exposure mode to " << (settings->exposure_mode == mojom::MeteringMode::CONTINUOUS ? "continuous" : "manual");
        }
        special_controls_maybe_changed = true;
    }
    ...
    if (settings->has_exposure_compensation) {
        ...
        if (result >= 0 && current_auto_exposure.value != V4L2_EXPOSURE_MANUAL) {
            ...
            if (DoIoctl(VIDIOC_S_CTRL, &set_exposure_bias) < 0) {
                DPLOG(ERROR) << "setting exposure compensation to " << settings->exposure_compensation;
            }
        }
    }
    ...
    if (settings->has_exposure_time) {
        ...
        if (result >= 0 && (current_auto_exposure.value == V4L2_EXPOSURE_MANUAL || current_auto_exposure.value == V4L2_EXPOSURE_SHUTTER_PRIORITY)) {
...
{
    int num_retries = 0;
    for (; DoIoctl(request, argp) < 0 && num_retries < kMaxIOCtrlRetries; ++num_retries) {
        DPLOG(WARNING) << "ioctl";
    }
...
{
    return IsControllableControl(control_id, base::BindRepeating(&V4L2CaptureDelegate::DoIoctl, base::Unretained(this)));
}

void V4L2CaptureDelegate::ReplaceControlEventSubscriptions()
{
    constexpr uint32_t kControlIds[] = { V4L2_CID_AUTO_EXPOSURE_BIAS, V4L2_CID_AUTO_WHITE_BALANCE, V4L2_CID_BRIGHTNESS, V4L2_CID_CONTRAST,
        V4L2_CID_EXPOSURE_ABSOLUTE, V4L2_CID_EXPOSURE_AUTO, V4L2_CID_FOCUS_ABSOLUTE, V4L2_CID_FOCUS_AUTO, V4L2_CID_PAN_ABSOLUTE, V4L2_CID_SATURATION,
        V4L2_CID_SHARPNESS, V4L2_CID_TILT_ABSOLUTE, V4L2_CID_WHITE_BALANCE_TEMPERATURE, V4L2_CID_ZOOM_ABSOLUTE };
    for (uint32_t control_id : kControlIds) {
        int request = IsControllableControl(control_id) ? VIDIOC_SUBSCRIBE_EVENT : VIDIOC_UNSUBSCRIBE_EVENT;
        ...
        if (DoIoctl(request, &subscription) < 0) {
            DPLOG(INFO) << (request == VIDIOC_SUBSCRIBE_EVENT ? "VIDIOC_SUBSCRIBE_EVENT" : "VIDIOC_UNSUBSCRIBE_EVENT")
...
bool V4L2CaptureDelegate::StartStream()
{
    ...
    if (DoIoctl(VIDIOC_REQBUFS, &r_buffer) < 0) {
        SetErrorState(VideoCaptureError::kV4L2ErrorRequestingMmapBuffers, FROM_HERE, "Error requesting MMAP buffers from V4L2");
        return false;
    }
    for (unsigned int i = 0; i < r_buffer.count; ++i) {
        if (!MapAndQueueBuffer(i)) {
            SetErrorState(VideoCaptureError::kV4L2AllocateBufferFailed, FROM_HERE, "Allocate buffer failed");
            return false;
        }
    }
    v4l2_buf_type capture_type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    if (DoIoctl(VIDIOC_STREAMON, &capture_type) < 0) {
        SetErrorState(VideoCaptureError::kV4L2VidiocStreamonFailed, FROM_HERE, "VIDIOC_STREAMON failed");
        return false;
    }
    ReplaceControlEventSubscriptions();
    is_capturing_ = true;
...
void V4L2CaptureDelegate::DoCapture()
{
    ...
    const int result = HANDLE_EINTR(v4l2_->poll(&device_pfd, 1, kCaptureTimeoutMs));
    if (result < 0) {
    ...
    if (result == 0) {
        ...
        if (timeout_count_ == 1) {
            ...
            SetErrorState(VideoCaptureError::kV4L2MultipleContinuousTimeoutsWhileReadPolling, FROM_HERE, "Multiple continuous timeouts while read-polling.");
            timeout_count_ = 0;
            return;
        }
    } else {
    ...
    if (device_pfd.revents & POLLIN) {
        ...
        if (DoIoctl(VIDIOC_DQBUF, &buffer) < 0) {
            SetErrorState(VideoCaptureError::kV4L2FailedToDequeueCaptureBuffer, FROM_HERE, "Failed to dequeue capture buffer");
            return;
        }
        ...
        if (buf_error_flag_set) {
#ifdef V4L2_BUF_FLAG_ERROR
            LOG(ERROR) << "Dequeued v4l2 buffer contains corrupted data (" << buffer.bytesused << " bytes).";
            buffer.bytesused = 0;
            client_->OnFrameDropped(VideoCaptureFrameDropReason::kV4L2BufferErrorFlagWasSet);
#endif
        } else if (buffer.bytesused < media::VideoFrame::AllocationSize(capture_format_.pixel_format, capture_format_.frame_size)) {
            LOG(ERROR) << "Dequeued v4l2 buffer contains invalid length (" << buffer.bytesused << " bytes).";
            buffer.bytesused = 0;
            client_->OnFrameDropped(VideoCaptureFrameDropReason::kV4L2InvalidNumberOfBytesInBuffer);
        } else {
            ...
            if (use_gpu_buffer_) {
                v4l2_gpu_helper_->OnIncomingCapturedData(
                    client_.get(), buffer_tracker->start(), buffer_tracker->payload_size(), capture_format_, gfx::ColorSpace(), rotation_, now, timestamp);
            } else
#endif //  BUILDFLAG(IS_LINUX)
                client_->OnIncomingCapturedData(buffer_tracker->start(), buffer_tracker->payload_size(), capture_format_, gfx::ColorSpace(), rotation_,
                    false /* flip_y */, now, timestamp, std::nullopt);
        }
        ...
        while (!take_photo_callbacks_.empty()) {
            ...
            mojom::BlobPtr blob = RotateAndBlobify(buffer_tracker->start(), buffer.bytesused, capture_format_, rotation_);
            if (blob)
                std::move(cb).Run(std::move(blob));
        }

        if (DoIoctl(VIDIOC_QBUF, &buffer) < 0) {
            SetErrorState(VideoCaptureError::kV4L2FailedToEnqueueCaptureBuffer, FROM_HERE, "Failed to enqueue capture buffer");
            return;
        }
    }
...
bool V4L2CaptureDelegate::StopStream()
{
    ...
    if (DoIoctl(VIDIOC_STREAMOFF, &capture_type) < 0) {
        SetErrorState(VideoCaptureError::kV4L2VidiocStreamoffFailed, FROM_HERE, "VIDIOC_STREAMOFF failed");
        return false;
    }
    ...
    if (DoIoctl(VIDIOC_REQBUFS, &r_buffer) < 0) {
        SetErrorState(VideoCaptureError::kV4L2FailedToVidiocReqbufsWithCount0, FROM_HERE, "Failed to VIDIOC_REQBUFS with count = 0");
        return false;
    }
...
void V4L2CaptureDelegate::SetErrorState(VideoCaptureError error, const base::Location& from_here, const std::string& reason)
{
    DCHECK(v4l2_task_runner_->BelongsToCurrentThread());
...
gfx::ColorSpace V4L2CaptureDelegate::BuildColorSpaceFromv4l2()
{
    ...
    v4l2_quantization v4l2_range = (v4l2_quantization)video_fmt_.fmt.pix.quantization;
    v4l2_ycbcr_encoding v4l2_matrix = (v4l2_ycbcr_encoding)video_fmt_.fmt.pix.ycbcr_enc;
    v4l2_xfer_func v4l2_transfer = (v4l2_xfer_func)video_fmt_.fmt.pix.xfer_func;

    DVLOG(2) << __func__ << "v4l2_primary:" << v4l2_primary << ", v4l2_range:" << v4l2_range << ", v4l2_matrix:" << v4l2_matrix
    ...
    case V4L2_QUANTIZATION_DEFAULT:
        if (media::IsYuvPlanar(capture_format_.pixel_format) && v4l2_primary != V4L2_COLORSPACE_JPEG) {
    ...
    DVLOG(2) << __func__ << "build color space:" << gfx::ColorSpace(primary, transfer, matrix, range).ToString();
...
bool V4L2CaptureDelegate::BufferTracker::Init(int fd, const v4l2_buffer& buffer)
{
    // Some devices require mmap() to be called with both READ and WRITE.
    // See http://crbug.com/178582.
    constexpr int kFlags = PROT_READ | PROT_WRITE;
    void* const start = v4l2_->mmap(nullptr, buffer.length, kFlags, MAP_SHARED, fd, buffer.m.offset);
    if (start == MAP_FAILED) {
