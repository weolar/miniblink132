...
// devices.
class CAPTURE_EXPORT VideoCaptureDeviceFactoryV4L2 : public VideoCaptureDeviceFactory {
public:
    ...
    explicit VideoCaptureDeviceFactoryV4L2(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner);

    VideoCaptureDeviceFactoryV4L2(const VideoCaptureDeviceFactoryV4L2&) = delete;
    VideoCaptureDeviceFactoryV4L2& operator=(const VideoCaptureDeviceFactoryV4L2&) = delete;

    ~VideoCaptureDeviceFactoryV4L2() override;

    void SetV4L2EnvironmentForTesting(scoped_refptr<V4L2CaptureDevice> v4l2, std::unique_ptr<DeviceProvider> device_provider);
...
private:
    ...
    std::vector<float> GetFrameRateList(int fd, uint32_t fourcc, uint32_t width, uint32_t height);
    void GetSupportedFormatsForV4L2BufferType(int fd, VideoCaptureFormats* supported_formats);
