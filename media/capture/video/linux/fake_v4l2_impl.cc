...
size_t GetV4L2FrameSize(uint32_t pixelformat, unsigned int width, unsigned int height)
{
    auto size = VideoFrame::AllocationSize(V4L2FourccToPixelFormat(pixelformat), gfx::Size(width, width));
...
public:
    explicit OpenedDevice(const FakeV4L2DeviceConfig& config, int open_flags)
        ...
        , wait_for_outgoing_queue_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC)
        , frame_production_thread_("FakeV4L2Impl FakeProductionThread")
    {
        ...
        selected_format_.sizeimage = GetV4L2FrameSize(selected_format_.pixelformat, selected_format_.width, selected_format_.height);
    ...
    ~OpenedDevice()
    {
        DCHECK(!frame_production_thread_.IsRunning());
    }

    const std::string& device_id() const
    {
        return config_.descriptor.device_id;
    }
    ...
    {
        auto buffer_iter = base::ranges::find(device_buffers_, offset, &FakeV4L2Buffer::offset);
    ...
    bool BlockUntilOutputQueueHasBuffer(int timeout_in_milliseconds)
    {
        ...
        return wait_for_outgoing_queue_event_.TimedWait(base::Milliseconds(timeout_in_milliseconds));
    }
    ...
    int enum_fmt(v4l2_fmtdesc* fmtdesc)
    {
        ...
        fmtdesc->flags = 0u;
        strcpy(reinterpret_cast<char*>(fmtdesc->description), FourccToString(config_.v4l2_pixel_format).c_str());
    ...
    int querycap(v4l2_capability* cap)
    {
        ...
        strcpy(reinterpret_cast<char*>(cap->driver), config_.descriptor.display_name().c_str());
        cap->bus_info[0] = 0;
    ...
    int g_ctrl(v4l2_control* control)
    {
        return Error(EINVAL);
    }

    int s_ctrl(v4l2_control* control)
    {
        return kSuccessReturnValue;
    }

    int s_ext_ctrls(v4l2_ext_controls* control)
    {
        return kSuccessReturnValue;
    }
    ...
    {
        if (format->type != V4L2_BUF_TYPE_VIDEO_CAPTURE || format->fmt.pix.width > kMaxWidth || format->fmt.pix.height > kMaxHeight) {
            return Error(EINVAL);
        }
        ...
        pix_format.sizeimage = GetV4L2FrameSize(config_.v4l2_pixel_format, pix_format.width, pix_format.height);
    ...
    int reqbufs(v4l2_requestbuffers* bufs)
    {
        ...
        for (uint32_t i = 0; i < target_buffer_count; i++) {
            device_buffers_.emplace_back(i, current_offset, selected_format_.sizeimage);
    ...
    int dqbuf(v4l2_buffer* buf)
    {
        ...
        buf->bytesused = GetV4L2FrameSize(selected_format_.pixelformat, selected_format_.width, selected_format_.height);
    ...
    int streamon(const int* type)
    {
        ...
        frame_production_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&FakeV4L2Impl::OpenedDevice::RunFrameProductionLoop, base::Unretained(this)));
        return kSuccessReturnValue;
    }
    ...
    {
        if (frame_interval->index > 0 || frame_interval->width != kDefaultWidth || frame_interval->height != kDefaultWidth) {
            return kErrorReturnValue;
        }
...
FakeV4L2Impl::FakeV4L2Impl()
    : next_id_to_return_from_open_(1)
{
}

FakeV4L2Impl::~FakeV4L2Impl() = default;

void FakeV4L2Impl::AddDevice(const std::string& device_name, const FakeV4L2DeviceConfig& config)
{
...
int FakeV4L2Impl::open(const char* device_name, int flags)
{
    ...
    opened_devices_.emplace(device_id, std::make_unique<OpenedDevice>(device_configs_iter->second, flags));
    return device_id;
}
...
int FakeV4L2Impl::ioctl(int fd, int request, void* argp)
{
    ...
    case VIDIOC_S_EXT_CTRLS:
        return opened_device->s_ext_ctrls(reinterpret_cast<v4l2_ext_controls*>(argp));
    ...
    case VIDIOC_REQBUFS:
        return opened_device->reqbufs(reinterpret_cast<v4l2_requestbuffers*>(argp));
    ...
    case VIDIOC_ENUM_FRAMESIZES:
        return opened_device->enum_framesizes(reinterpret_cast<v4l2_frmsizeenum*>(argp));
    case VIDIOC_ENUM_FRAMEINTERVALS:
        return opened_device->enum_frameintervals(reinterpret_cast<v4l2_frmivalenum*>(argp));
    case VIDIOC_DQEVENT:
        return opened_device->dqevent(reinterpret_cast<v4l2_event*>(argp));
    case VIDIOC_SUBSCRIBE_EVENT:
        return opened_device->subscribe_event(reinterpret_cast<v4l2_event_subscription*>(argp));
    case VIDIOC_UNSUBSCRIBE_EVENT:
        return opened_device->unsubscribe_event(reinterpret_cast<v4l2_event_subscription*>(argp));
...
void* FakeV4L2Impl::mmap(void* /*start*/, size_t length, int prot, int flags, int fd, off_t offset)
{
    base::AutoLock lock(lock_);
    if (flags & MAP_FIXED) {
        errno = EINVAL;
