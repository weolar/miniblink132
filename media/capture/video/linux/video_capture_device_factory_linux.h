...
// devices.
class CAPTURE_EXPORT VideoCaptureDeviceFactoryLinux : public VideoCaptureDeviceFactory {
public:
    explicit VideoCaptureDeviceFactoryLinux(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner);

    VideoCaptureDeviceFactoryLinux(const VideoCaptureDeviceFactoryLinux&) = delete;
    VideoCaptureDeviceFactoryLinux& operator=(const VideoCaptureDeviceFactoryLinux&) = delete;
...
private:
#if defined(WEBRTC_USE_PIPEWIRE)
    void OnGetDevicesInfo(GetDevicesInfoCallback callback, std::vector<VideoCaptureDeviceInfo> devices_info);
