...
static struct {
    ...
    uint32_t class_id;
} const kControls[] = { { V4L2_CID_USER_BASE, V4L2_CID_USER_CLASS }, { V4L2_CID_CAMERA_CLASS_BASE, V4L2_CID_CAMERA_CLASS } };
...
static void SetControlsToMaxValues(int device_fd)
{
    ...
    if (HANDLE_EINTR(ioctl(device_fd, VIDIOC_S_EXT_CTRLS, &camera_ext_controls)) < 0) {
    ...
    for (const auto& control : kControls) {
        ...
        while (0 == HANDLE_EINTR(ioctl(device_fd, VIDIOC_QUERYCTRL, &range))) {
            ...
            EXPECT_EQ(range.maximum, readback.value) << " control " << range.name << " didn't set correctly";
        }
    }
}
...
{
    auto do_ioctl = base::BindRepeating([](int device_fd, int request, void* argp) { return HANDLE_EINTR(ioctl(device_fd, request, argp)); }, device_fd);
    ...
    for (const auto& control : kControls) {
        ...
        while (0 == HANDLE_EINTR(ioctl(device_fd, VIDIOC_QUERYCTRL, &range))) {
            ...
            DVLOG(1) << __func__ << " " << range.name << ": " << range.minimum << "-" << range.maximum << ", default: " << range.default_value;
            ...
            if (V4L2CaptureDelegate::IsBlockedControl(current.id) || !V4L2CaptureDelegate::IsControllableControl(current.id, do_ioctl)) {
            ...
            EXPECT_EQ(range.default_value, current.value) << " control " << range.name << " didn't reset correctly";
        }
    }
}
...
public:
    V4L2CaptureDelegateTest()
        ...
        , delegate_(std::make_unique<V4L2CaptureDelegate>(v4l2_.get(), device_descriptor_, base::SingleThreadTaskRunner::GetCurrentDefault(), 50, 0))
    {
    }
...
public:
    ...
    void OnIncomingCapturedGfxBuffer(gfx::GpuMemoryBuffer* buffer, const VideoCaptureFormat& frame_format, int clockwise_rotation,
        base::TimeTicks reference_time, base::TimeDelta timestamp, std::optional<base::TimeTicks> capture_begin_time, int frame_feedback_id = 0) override
    {
    }
...
class V4l2CaptureDelegateGPUMemoryBufferTest : public ::testing::TestWithParam<uint32_t> {
public:
...
public:
    void SetUp() override
    {
        device_factory_ = std::make_unique<VideoCaptureDeviceFactoryV4L2>(base::SingleThreadTaskRunner::GetCurrentDefault());
        ...
        device_factory_->SetV4L2EnvironmentForTesting(std::move(fake_v4l2), std::move(fake_device_provider));
        base::CommandLine::ForCurrentProcess()->AppendSwitch(::switches::kVideoCaptureUseGpuMemoryBuffer);
        test_sii_ = base::MakeRefCounted<gpu::TestSharedImageInterface>();
        test_sii_->UseTestGMBInSharedImageCreationWithBufferUsage();
        VideoCaptureGpuChannelHost::GetInstance().SetSharedImageInterface(test_sii_);
    }
...
#if BUILDFLAG(IS_LINUX)
#define MAYBE_CreateAndDestroyAndVerifyControls DISABLED_CreateAndDestroyAndVerifyControls
#else
#define MAYBE_CreateAndDestroyAndVerifyControls CreateAndDestroyAndVerifyControls
#endif
TEST_F(V4L2CaptureDelegateTest, MAYBE_CreateAndDestroyAndVerifyControls)
{
    ...
    {
        base::ScopedFD device_fd(HANDLE_EINTR(open(device_descriptor_.device_id.c_str(), O_RDWR)));
    ...
    // control values.
    {
        std::unique_ptr<MockVideoCaptureDeviceClient> client(new MockVideoCaptureDeviceClient());
        ...
        EXPECT_CALL(*client_ptr, OnIncomingCapturedData).Times(1).WillOnce(RunClosure(quit_closure));
        run_loop.Run();
...
TEST_P(V4l2CaptureDelegateGPUMemoryBufferTest, CameraCaptureOneCopy)
{
    ...
    VideoCaptureDeviceDescriptor descriptor(stub_display_name, stub_device_id, VideoCaptureApi::LINUX_V4L2_SINGLE_PLANE);
    fake_device_provider_->AddDevice(descriptor);
    fake_v4l2_->AddDevice(stub_device_id, FakeV4L2DeviceConfig(descriptor, fmt));
    std::unique_ptr<VideoCaptureDevice> device = device_factory_->CreateDevice(descriptor).ReleaseDevice();
    received_frame_count_ = 0;

    std::unique_ptr<MockV4l2GpuClient> client = std::make_unique<MockV4l2GpuClient>();
    MockV4l2GpuClient* client_ptr = client.get();
    EXPECT_CALL(*client_ptr, ReserveOutputBuffer)
        .WillRepeatedly(Invoke([](const gfx::Size& size, VideoPixelFormat format, int feedback_id, VideoCaptureDevice::Client::Buffer* capture_buffer,
                                   int* require_new_buffer_id, int* retire_old_buffer_id) {
            EXPECT_EQ(format, PIXEL_FORMAT_NV12);
            capture_buffer->handle_provider = std::make_unique<MockCaptureHandleProvider>(size, gfx::BufferFormat::YUV_420_BIPLANAR);
            return VideoCaptureDevice::Client::ReserveResult::kSucceeded;
        }));

    base::RunLoop wait_loop;
    EXPECT_CALL(*client_ptr, OnIncomingCapturedBufferExt).WillRepeatedly(InvokeWithoutArgs([&wait_loop, this]() {
...
INSTANTIATE_TEST_SUITE_P(
    All, V4l2CaptureDelegateGPUMemoryBufferTest, ::testing::Values((V4L2_PIX_FMT_NV12), (V4L2_PIX_FMT_YUYV), (V4L2_PIX_FMT_MJPEG), (V4L2_PIX_FMT_RGB24)));
#endif // BUILDFLAG(IS_LINUX)
} // namespace media
