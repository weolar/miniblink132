...
bool CompareCaptureDevices(const VideoCaptureDeviceInfo& a, const VideoCaptureDeviceInfo& b)
{
...
const char kInterfacePathTemplate[] = "/sys/class/video4linux/%s/device/interface";
...
class DevVideoFilePathsDeviceProvider : public VideoCaptureDeviceFactoryV4L2::DeviceProvider {
public:
    ...
    std::string GetDeviceModelId(const std::string& device_id) override
    {
        ...
        std::string usb_id;
        const std::string vid_path = base::StringPrintf(kVidPathTemplate, file_name.c_str());
        if (!ReadIdFile(vid_path, &usb_id)) {
            return usb_id;
        }

        usb_id.append(":");
        const std::string pid_path = base::StringPrintf(kPidPathTemplate, file_name.c_str());
    ...
    {
        const std::string file_name = ExtractFileNameFromDeviceId(device_id);
        const std::string interface_path = base::StringPrintf(kInterfacePathTemplate, file_name.c_str());
        std::string display_name;
        if (!base::ReadFileToStringWithMaxSize(base::FilePath(interface_path), &display_name, kMaxInterfaceNameSize)) {
            return std::string();
        }
        return display_name;
    }
...
VideoCaptureDeviceFactoryV4L2::VideoCaptureDeviceFactoryV4L2(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
...
void VideoCaptureDeviceFactoryV4L2::SetV4L2EnvironmentForTesting(
    scoped_refptr<V4L2CaptureDevice> v4l2, std::unique_ptr<VideoCaptureDeviceFactoryV4L2::DeviceProvider> device_provider)
{
    v4l2_ = std::move(v4l2);
    device_provider_ = std::move(device_provider);
}

VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryV4L2::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    auto self = std::make_unique<VideoCaptureDeviceLinux>(v4l2_.get(), device_descriptor);
    ...
    ScopedV4L2DeviceFD fd(v4l2_.get(), HANDLE_EINTR(v4l2_->open(device_descriptor.device_id.c_str(), O_RDONLY)));
    if (!fd.is_valid()) {
        DLOG(ERROR) << "Cannot open device";
        return VideoCaptureErrorOrDevice(VideoCaptureError::kV4L2FailedToOpenV4L2DeviceDriverFile);
    }

    return VideoCaptureErrorOrDevice(std::move(self));
}

void VideoCaptureDeviceFactoryV4L2::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    for (auto& unique_id : filepaths) {
        const ScopedV4L2DeviceFD fd(v4l2_.get(), HANDLE_EINTR(v4l2_->open(unique_id.c_str(), O_RDONLY)));
        if (!fd.is_valid()) {
        ...
        if ((DoIoctl(fd.get(), VIDIOC_QUERYCAP, &cap) == 0)
            ...
            const std::string model_id = device_provider_->GetDeviceModelId(unique_id);
            std::string display_name = device_provider_->GetDeviceDisplayName(unique_id);
            ...
            devices_info.emplace_back(VideoCaptureDeviceDescriptor(display_name, unique_id, model_id, VideoCaptureApi::LINUX_V4L2_SINGLE_PLANE,
                GetControlSupport(fd.get()), VideoCaptureTransportType::OTHER_TRANSPORT, facing_mode));
...
VideoCaptureControlSupport VideoCaptureDeviceFactoryV4L2::GetControlSupport(int fd)
{
...
bool VideoCaptureDeviceFactoryV4L2::GetControlSupport(int fd, int control_id)
{
    ...
    return DoIoctl(fd, VIDIOC_QUERYCTRL, &range) == 0 && range.minimum < range.maximum;
}

bool VideoCaptureDeviceFactoryV4L2::HasUsableFormats(int fd, uint32_t capabilities)
{
    if (!(capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
        return false;
    }

    const std::vector<uint32_t>& usable_fourccs = VideoCaptureDeviceLinux::GetListOfUsableFourCCs(false);
...
std::vector<float> VideoCaptureDeviceFactoryV4L2::GetFrameRateList(int fd, uint32_t fourcc, uint32_t width, uint32_t height)
{
    ...
    for (; DoIoctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frame_interval) == 0; ++frame_interval.index) {
        if (frame_interval.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
            if (frame_interval.discrete.numerator != 0) {
                frame_rates.push_back(frame_interval.discrete.denominator / static_cast<float>(frame_interval.discrete.numerator));
            }
...
void VideoCaptureDeviceFactoryV4L2::GetSupportedFormatsForV4L2BufferType(int fd, VideoCaptureFormats* supported_formats)
{
    v4l2_fmtdesc v4l2_format = {};
    v4l2_format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    for (; DoIoctl(fd, VIDIOC_ENUM_FMT, &v4l2_format) == 0; ++v4l2_format.index) {
        VideoCaptureFormat supported_format;
        supported_format.pixel_format = VideoCaptureDeviceLinux::V4l2FourCcToChromiumPixelFormat(v4l2_format.pixelformat);
        ...
        for (; DoIoctl(fd, VIDIOC_ENUM_FRAMESIZES, &frame_size) == 0; ++frame_size.index) {
            if (frame_size.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
                supported_format.frame_size.SetSize(frame_size.discrete.width, frame_size.discrete.height);
            ...
            const std::vector<float> frame_rates = GetFrameRateList(fd, v4l2_format.pixelformat, frame_size.discrete.width, frame_size.discrete.height);
