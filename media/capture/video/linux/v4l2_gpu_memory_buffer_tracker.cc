...
bool V4L2GpuMemoryBufferTracker::Init(const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides)
{
    std::optional<gfx::BufferFormat> gfx_format = ToBufferFormat(format);
    if (!gfx_format) {
        DLOG(ERROR) << "Unsupported VideoPixelFormat " << VideoPixelFormatToString(format);
        return false;
    }
    ...
    shared_image_ = sii->CreateSharedImage(
        { si_format, dimensions, gfx::ColorSpace(), gpu::SharedImageUsageSet(si_usage), "V4L2GpuMemoryBufferTracker" }, gpu::kNullSurfaceHandle, usage);
    if (!shared_image_) {
...
bool V4L2GpuMemoryBufferTracker::IsReusableForFormat(const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides)
{
    if (!is_valid_) {
        return false;
    }
    ...
    return (viz::GetSharedImageFormat(*gfx_format) == shared_image_->format() && dimensions == shared_image_->size());
}

std::unique_ptr<VideoCaptureBufferHandle> V4L2GpuMemoryBufferTracker::GetMemoryMappedAccess()
{
    NOTREACHED() << "Unsupported operation";
}

base::UnsafeSharedMemoryRegion V4L2GpuMemoryBufferTracker::DuplicateAsUnsafeRegion()
{
    NOTREACHED() << "Unsupported operation";
}

gfx::GpuMemoryBufferHandle V4L2GpuMemoryBufferTracker::GetGpuMemoryBufferHandle()
{
