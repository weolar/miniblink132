...
class CAPTURE_EXPORT V4L2CaptureDevice : public base::RefCountedThreadSafe<V4L2CaptureDevice> {
public:
    virtual int open(const char* device_name, int flags) = 0;
    virtual int close(int fd) = 0;
    virtual int ioctl(int fd, int request, void* argp) = 0;
    virtual void* mmap(void* start, size_t length, int prot, int flags, int fd, off_t offset) = 0;
