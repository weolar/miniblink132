...
VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryWebRtc::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
...
void VideoCaptureDeviceFactoryWebRtc::FinishGetDevicesInfo()
{
    ...
    if (options_) {
        ...
        if (info) {
            for (uint32_t i = 0; i < info->NumberOfDevices(); ++i) {
                ...
                if (info->GetDeviceName(i, device_name, webrtc::kVideoCaptureDeviceNameLength, unique_name, webrtc::kVideoCaptureUniqueNameLength, product_id,
                ...
                VideoCaptureDeviceInfo device_info(VideoCaptureDeviceDescriptor(device_name, unique_name, product_id,
                    VideoCaptureApi::WEBRTC_LINUX_PIPEWIRE_SINGLE_PLANE, control_support, VideoCaptureTransportType::OTHER_TRANSPORT, facing_mode));
                ...
                for (int j = 0; j < num_capabilities; ++j) {
                    ...
                    supported_format.pixel_format = WebRtcVideoTypeToChromiumPixelFormat(capability.videoType);
                    supported_format.frame_rate = capability.maxFPS;
                    supported_format.frame_size.SetSize(capability.width, capability.height);
                    device_info.supported_formats.push_back(supported_format);
                    VLOG(2) << "Found Format: " << VideoCaptureFormat::ToString(supported_format);
                }
    ...
    origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback_), devices_info));
}

void VideoCaptureDeviceFactoryWebRtc::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void VideoCaptureDeviceFactoryWebRtc::OnInitialized(webrtc::VideoCaptureOptions::Status status)
{
...
webrtc::VideoType VideoCaptureDeviceFactoryWebRtc::WebRtcVideoTypeFromChromiumPixelFormat(VideoPixelFormat pixel_format)
{
...
VideoPixelFormat VideoCaptureDeviceFactoryWebRtc::WebRtcVideoTypeToChromiumPixelFormat(webrtc::VideoType video_type)
{
    for (const auto& type_and_format : kSupportedFormats) {
