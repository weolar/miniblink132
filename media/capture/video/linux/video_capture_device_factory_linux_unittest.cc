...
class VideoCaptureDeviceFactoryLinuxTest : public ::testing::TestWithParam<VideoCaptureDeviceDescriptor> {
public:
    ...
    void SetUp() override
    {
        factory_ = std::make_unique<VideoCaptureDeviceFactoryLinux>(base::ThreadTaskRunnerHandle::Get());
        ...
        factory_->SetV4L2EnvironmentForTesting(std::move(fake_v4l2), std::move(fake_device_provider));
    }
...
TEST_P(VideoCaptureDeviceFactoryLinuxTest, EnumerateSingleFakeV4L2DeviceUsing)
{
    ...
    factory_->GetDevicesInfo(base::BindLambdaForTesting([&devices_info, &run_loop](std::vector<media::VideoCaptureDeviceInfo> result) {
    ...
    EXPECT_EQ(descriptor.display_name(), devices_info[0].descriptor.display_name());
    EXPECT_EQ(descriptor.control_support().pan, devices_info[0].descriptor.control_support().pan);
    EXPECT_EQ(descriptor.control_support().tilt, devices_info[0].descriptor.control_support().tilt);
    EXPECT_EQ(descriptor.control_support().zoom, devices_info[0].descriptor.control_support().zoom);
}

INSTANTIATE_TEST_SUITE_P(All, VideoCaptureDeviceFactoryLinuxTest,
    ::testing::Values(VideoCaptureDeviceDescriptor("Fake Device 0", "/dev/video0", VideoCaptureApi::UNKNOWN,
                          /*control_support=*/ { false, false, false }),
        VideoCaptureDeviceDescriptor("Fake Device 0", "/dev/video0", VideoCaptureApi::UNKNOWN,
            /*control_support=*/ { false, false, true }),
        VideoCaptureDeviceDescriptor("Fake Device 0", "/dev/video0", VideoCaptureApi::UNKNOWN,
            /*control_support=*/ { true, true, true })));

TEST_F(VideoCaptureDeviceFactoryLinuxTest, ReceiveFramesFromSinglePlaneFakeDevice)
{
    // Setup
    ...
    EXPECT_CALL(*client_ptr, OnIncomingCapturedData(_, _, _, _, _, _, _, _, _)).WillRepeatedly(InvokeWithoutArgs([&wait_loop]() {
