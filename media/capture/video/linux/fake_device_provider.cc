...
void FakeDeviceProvider::AddDevice(const VideoCaptureDeviceDescriptor& descriptor)
{
    descriptors_.emplace_back(descriptor);
}

void FakeDeviceProvider::GetDeviceIds(std::vector<std::string>* target_container)
{
...
{
    auto iter = base::ranges::find(descriptors_, device_id, &VideoCaptureDeviceDescriptor::device_id);
...
std::string FakeDeviceProvider::GetDeviceDisplayName(const std::string& device_id)
{
    auto iter = base::ranges::find(descriptors_, device_id, &VideoCaptureDeviceDescriptor::device_id);
    if (iter == descriptors_.end())
        CHECK(false) << "Unknown device_id " << device_id;
