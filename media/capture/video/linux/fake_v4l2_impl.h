...
struct FakeV4L2DeviceConfig {
    explicit FakeV4L2DeviceConfig(const VideoCaptureDeviceDescriptor& descriptor, uint32_t fmt = V4L2_PIX_FMT_YUV420)
        : descriptor(descriptor)
        , v4l2_pixel_format(fmt)
    {
    }
...
public:
    ...
    void AddDevice(const std::string& device_name, const FakeV4L2DeviceConfig& config);

    // Implementation of V4L2CaptureDevice interface:
    int open(const char* device_name, int flags) override;
    int close(int fd) override;
    int ioctl(int fd, int request, void* argp) override;
    void* mmap(void* start, size_t length, int prot, int flags, int fd, off_t offset) override;
...
private:
    ...
    std::map<int /*value returned by open()*/, std::unique_ptr<OpenedDevice>> opened_devices_ GUARDED_BY(lock_);
};
