...
public:
    ...
    void AddMockDevice(media::VideoCaptureDevice* device, const media::VideoCaptureDeviceDescriptor& descriptor);
    void RemoveAllDevices();

    // media::VideoCaptureDeviceFactory implementation.
    VideoCaptureErrorOrDevice CreateDevice(const media::VideoCaptureDeviceDescriptor& device_descriptor) override;
    void GetDevicesInfo(GetDevicesInfoCallback callback) override;

private:
    std::map<media::VideoCaptureDeviceDescriptor, raw_ptr<media::VideoCaptureDevice, CtnExperimental>> devices_;
};
