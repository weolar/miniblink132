...
using VideoCaptureFeedbackCB = base::RepeatingCallback<void(const VideoCaptureFeedback&)>;
...
struct CAPTURE_EXPORT VideoCaptureFeedback {
    ...
    explicit VideoCaptureFeedback(
        double resource_utilization, float max_framerate_fps = std::numeric_limits<float>::infinity(), int max_pixels = std::numeric_limits<int>::max());

    bool operator==(const VideoCaptureFeedback& other) const
    {
        return resource_utilization == other.resource_utilization && max_pixels == other.max_pixels && max_framerate_fps == other.max_framerate_fps
            && require_mapped_frame == other.require_mapped_frame && frame_id == other.frame_id;
    }
