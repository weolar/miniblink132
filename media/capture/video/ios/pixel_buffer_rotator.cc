...
PixelBufferRotator::PixelBufferRotator()
{
    if (__builtin_available(iOS 16, *)) {
        OSStatus error = VTPixelRotationSessionCreate(nil, rotation_session_.InitializeInto());
        // There is no known way to make session creation fail, so we do not deal
        // with failures gracefully.
        CHECK(error == noErr) << "Creating a   VTPixelRotationSession failed: " << error;
    }
}

bool PixelBufferRotator::Rotate(CVPixelBufferRef source, CVPixelBufferRef destination, int rotation)
{
    DCHECK(source);
    DCHECK(destination);
    ...
    if (__builtin_available(iOS 16, *)) {
        ...
        OSStatus error = VTSessionSetProperty(rotation_session_.get(), kVTPixelRotationPropertyKey_Rotation, rotation_angle);
        CHECK(error == noErr) << "Unexpected RotationPropertyKey error: " << error;
        error = VTPixelRotationSessionRotateImage(rotation_session_.get(), source, destination);
        if (error == kVTPixelRotationNotSupportedErr) {
            return false;
        }
        CHECK(error == noErr) << "Unexpected  VTPixelRotationSessionRotateImage error: " << error;
        return true;
    }
    return false;
}
