...
libyuv::RotationModeEnum TranslateIntegerRotationToLibyuvRotation(const int rotation)
{
    switch (rotation) {
    case 0:
...
mojom::BlobPtr ProduceJpegBlobFromMjpegFrame(const uint8_t* buffer, const uint32_t bytesused, const gfx::Size size, const int rotation)
{
    ...
    if (rotation != 0) {
        ...
        std::vector<uint8_t> bgra_buffer(output_width * output_height * bytes_per_pixel);
        libyuv::ConvertToARGB(buffer, static_cast<size_t>(bytesused), bgra_buffer.data(), output_width * bytes_per_pixel, /*crop_x=*/0,
            /*crop_y=*/0, size.width(), size.height(), size.width(), size.height(), TranslateIntegerRotationToLibyuvRotation(rotation), libyuv::FOURCC_MJPG);
        SkImageInfo info = SkImageInfo::Make(output_width, output_height, kBGRA_8888_SkColorType, kOpaque_SkAlphaType);
        SkPixmap src(info, &bgra_buffer[0], output_width * bytes_per_pixel);

        std::optional<std::vector<uint8_t>> rotated_jpeg_data = gfx::JPEGCodec::Encode(src, kJpegQualityDefault);
        ...
        LOG(ERROR) << "Failed to encode frame to JPEG. Using non-rotated original frame.";
...
mojom::BlobPtr RotateAndBlobify(const uint8_t* buffer, const uint32_t bytesused, const VideoCaptureFormat& capture_format, const int rotation)
{
    DCHECK(buffer);
    DCHECK(bytesused);
    ...
    if (pixel_format == VideoPixelFormat::PIXEL_FORMAT_MJPEG) {
        return ProduceJpegBlobFromMjpegFrame(buffer, bytesused, capture_format.frame_size, rotation);
    }
    ...
    auto tmp_argb = std::make_unique<uint8_t[]>(VideoFrame::AllocationSize(PIXEL_FORMAT_ARGB, frame_size));
    if (ConvertToARGB(buffer, bytesused, tmp_argb.get(), frame_size.width() * 4, 0 /* crop_x_pos */, 0 /* crop_y_pos */, frame_size.width(),
            frame_size.height(), frame_size.width(), frame_size.height(), libyuv::RotationMode::kRotate0, src_format)
        != 0) {
        return nullptr;
    }
...
#endif
    std::optional<std::vector<uint8_t>> data = gfx::PNGCodec::Encode(tmp_argb.get(), codec_color_format, frame_size, frame_size.width() * 4,
