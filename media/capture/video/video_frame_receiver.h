...
struct CAPTURE_EXPORT ReadyFrameInBuffer {
    ReadyFrameInBuffer();
    ReadyFrameInBuffer(int buffer_id, int frame_feedback_id, std::unique_ptr<VideoCaptureDevice::Client::Buffer::ScopedAccessPermission> buffer_read_permission,
    ...
    std::unique_ptr<VideoCaptureDevice::Client::Buffer::ScopedAccessPermission> buffer_read_permission;
    mojom::VideoFrameInfoPtr frame_info;
};
...
// thread.
class CAPTURE_EXPORT ScopedFrameDoneHelper final : public base::ScopedClosureRunner, public media::VideoCaptureDevice::Client::Buffer::ScopedAccessPermission {
public:
