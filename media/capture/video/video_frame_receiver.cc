...
ReadyFrameInBuffer::ReadyFrameInBuffer(int buffer_id, int frame_feedback_id,
    std::unique_ptr<VideoCaptureDevice::Client::Buffer::ScopedAccessPermission> buffer_read_permission, mojom::VideoFrameInfoPtr frame_info)
    : buffer_id(buffer_id)
...
ScopedFrameDoneHelper::ScopedFrameDoneHelper(base::OnceClosure done_callback)
    : base::ScopedClosureRunner(base::BindPostTaskToCurrentDefault(std::move(done_callback)))
{
}
