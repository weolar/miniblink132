...
class CAPTURE_EXPORT VideoCaptureDeviceFactoryAndroid : public VideoCaptureDeviceFactory {
public:
    static base::android::ScopedJavaLocalRef<jobject> createVideoCaptureAndroid(int id, jlong nativeVideoCaptureDeviceAndroid);

    VideoCaptureDeviceFactoryAndroid();

    VideoCaptureDeviceFactoryAndroid(const VideoCaptureDeviceFactoryAndroid&) = delete;
    VideoCaptureDeviceFactoryAndroid& operator=(const VideoCaptureDeviceFactoryAndroid&) = delete;
...
private:
    VideoCaptureFormats GetSupportedFormats(int device_index, const std::string& display_name);
