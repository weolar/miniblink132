...
#define MAYBE_UsingRealWebcam_AllocateBadSize DISABLED_UsingRealWebcam_AllocateBadSize
...
#define MAYBE_UsingRealWebcam_CheckPhotoCallbackRelease UsingRealWebcam_CheckPhotoCallbackRelease
...
#define MAYBE_UsingRealWebcam_GetPhotoState DISABLED_UsingRealWebcam_GetPhotoState
#define MAYBE_UsingRealWebcam_CaptureWithSize DISABLED_UsingRealWebcam_CaptureWithSize
#define MAYBE_UsingRealWebcam_CheckPhotoCallbackRelease DISABLED_UsingRealWebcam_CheckPhotoCallbackRelease
...
// fixed.
#define WRAPPED_TEST_P(test_case_name, test_name) TEST_P(test_case_name, test_name)
...
void DumpError(media::VideoCaptureError, const base::Location& location, const std::string& message)
{
...
public:
    ...
    const mojom::PhotoState* capabilities()
    {
        return state_.get();
    }
...
class VideoCaptureDeviceTest : public testing::TestWithParam<std::tuple<gfx::Size, VideoCaptureImplementationTweak>> {
public:
#if BUILDFLAG(IS_WIN)
    scoped_refptr<IMFCaptureEngineOnSampleCallback> CreateMockPhotoCallback(
        MockMFPhotoCallback* mock_photo_callback, VideoCaptureDevice::TakePhotoCallback callback, VideoCaptureFormat format)
    {
...
protected:
    ...
    {
#if BUILDFLAG(IS_CHROMEOS_ASH)
        local_gpu_memory_buffer_manager_ = std::make_unique<LocalGpuMemoryBufferManager>();
        VideoCaptureDeviceFactoryChromeOS::SetGpuBufferManager(local_gpu_memory_buffer_manager_.get());
        // TODO(b/315966244): Initialize mojo service manager when re-enabling the
        // test cases on a real device.
#endif
        video_capture_device_factory_ = CreateVideoCaptureDeviceFactory(base::SingleThreadTaskRunner::GetCurrentDefault());
    }

    void SetUp() override
    {
#if BUILDFLAG(IS_ANDROID)
        static_cast<VideoCaptureDeviceFactoryAndroid*>(video_capture_device_factory_.get())->ConfigureForTesting();
#elif BUILDFLAG(IS_WIN)
        static_cast<VideoCaptureDeviceFactoryWin*>(video_capture_device_factory_.get())->set_use_media_foundation_for_testing(UseWinMediaFoundation());
#endif
    }
    ...
    {
        return std::get<1>(GetParam()) == WIN_MEDIA_FOUNDATION && VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation();
    }
#elif BUILDFLAG(IS_CHROMEOS_ASH)
    void WaitForCameraServiceReady()
    {
        if (media::ShouldUseCrosCameraService()) {
            VideoCaptureDeviceFactoryChromeOS* vcd_factory_chromeos = static_cast<VideoCaptureDeviceFactoryChromeOS*>(video_capture_device_factory_.get());
    ...
    std::unique_ptr<MockVideoCaptureDeviceClient> CreateDeviceClient()
    {
        ...
        ON_CALL(*result, OnIncomingCapturedData)
            .WillByDefault(WithArgs<0, 1, 2>(Invoke([this](const uint8_t* data, int length, const media::VideoCaptureFormat& frame_format) {
                ASSERT_GT(length, 0);
                ASSERT_TRUE(data);
                main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoCaptureDeviceTest::OnFrameCaptured, base::Unretained(this), frame_format));
            })));
        ON_CALL(*result, OnIncomingCapturedGfxBuffer)
            .WillByDefault(WithArgs<0, 1>(Invoke([this](gfx::GpuMemoryBuffer* buffer, const media::VideoCaptureFormat& frame_format) {
                ASSERT_TRUE(buffer);
    ...
    {
        run_loop_ = std::make_unique<base::RunLoop>(base::RunLoop::Type::kNestableTasksAllowed);
        run_loop_->Run();
    }
    ...
    std::optional<VideoCaptureDeviceInfo> FindUsableDevice()
    {
        ...
        video_capture_device_factory_->GetDevicesInfo(base::BindLambdaForTesting([this, &run_loop](std::vector<VideoCaptureDeviceInfo> devices_info) {
        ...
        for (const auto& device : devices_info_) {
            ...
            if (!VideoCaptureDeviceFactoryAndroid::IsLegacyOrDeprecatedDevice(device.descriptor.device_id)) {
    ...
    const VideoCaptureFormat& last_format() const
    {
        return last_format_;
    }

    std::optional<VideoCaptureDeviceInfo> GetFirstDeviceSupportingPixelFormat(const VideoPixelFormat& pixel_format)
    {
        if (!FindUsableDevice())
            return std::nullopt;
        ...
        DVLOG_IF(1, pixel_format != PIXEL_FORMAT_MAX) << VideoPixelFormatToString(pixel_format);
        return std::nullopt;
    }

    bool IsCaptureSizeSupported(const VideoCaptureDeviceInfo& device_info, const gfx::Size& size)
    {
        auto& supported_formats = device_info.supported_formats;
        if (!base::Contains(supported_formats, size, &VideoCaptureFormat::frame_size)) {
...
#define MAYBE_UsingRealWebcam_OpenInvalidDevice DISABLED_UsingRealWebcam_OpenInvalidDevice
#else
#define MAYBE_UsingRealWebcam_OpenInvalidDevice UsingRealWebcam_OpenInvalidDevice
#endif
// Tries to allocate an invalid device and verifies it doesn't work.
WRAPPED_TEST_P(VideoCaptureDeviceTest, MAYBE_UsingRealWebcam_OpenInvalidDevice)
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunOpenInvalidDeviceTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunOpenInvalidDeviceTestCase()
{
    ...
    invalid_descriptor.capture_api
        = VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation() ? VideoCaptureApi::WIN_MEDIA_FOUNDATION : VideoCaptureApi::WIN_DIRECT_SHOW;
#elif BUILDFLAG(IS_APPLE)
    invalid_descriptor.capture_api = VideoCaptureApi::MACOSX_AVFOUNDATION;
#endif
    VideoCaptureErrorOrDevice device_status = video_capture_device_factory_->CreateDevice(invalid_descriptor);
...
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunCaptureWithSizeTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunCaptureWithSizeTestCase()
{
    ...
    VideoCaptureErrorOrDevice device_status = video_capture_device_factory_->CreateDevice(device_info->descriptor);
    ASSERT_TRUE(device_status.ok());
    std::unique_ptr<VideoCaptureDevice> device(device_status.ReleaseDevice());
...
const VideoCaptureImplementationTweak kCaptureImplementationTweaks[] = { NONE,
#if BUILDFLAG(IS_WIN)
    WIN_MEDIA_FOUNDATION
#endif
};
...
INSTANTIATE_TEST_SUITE_P(
    VideoCaptureDeviceTests, VideoCaptureDeviceTest, testing::Combine(testing::ValuesIn(kCaptureSizes), testing::ValuesIn(kCaptureImplementationTweaks)));
...
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunAllocateBadSizeTestCase, base::Unretained(this)));
}
...
WRAPPED_TEST_P(VideoCaptureDeviceTest, DISABLED_UsingRealWebcam_ReAllocateCamera)
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunReAllocateCameraTestCase, base::Unretained(this)));
}
...
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunCaptureMjpegTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunCaptureMjpegTestCase()
{
#if BUILDFLAG(IS_CHROMEOS_ASH)
    if (media::ShouldUseCrosCameraService()) {
        VLOG(1) << "Skipped on Chrome OS device where HAL v3 camera service is used";
        return;
    }
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    EXPECT_GE(static_cast<size_t>(1280 * 720), media::VideoFrame::AllocationSize(last_format().pixel_format, last_format().frame_size));
    device->StopAndDeAllocate();
#endif // BUILDFLAG(IS_WIN)
}

#define MAYBE_UsingRealWebcam_NoCameraSupportsPixelFormatMax UsingRealWebcam_NoCameraSupportsPixelFormatMax
WRAPPED_TEST_P(VideoCaptureDeviceTest, MAYBE_UsingRealWebcam_NoCameraSupportsPixelFormatMax)
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunNoCameraSupportsPixelFormatMaxTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunNoCameraSupportsPixelFormatMaxTestCase()
{
    ...
    auto device_descriptor = GetFirstDeviceSupportingPixelFormat(PIXEL_FORMAT_MAX);
...
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunTakePhotoTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunTakePhotoTestCase()
{
    ...
    if (device_info->supported_formats.empty())
        return;
    const gfx::Size frame_size = device_info->supported_formats.front().frame_size;
    ...
    VideoCaptureDevice::TakePhotoCallback scoped_callback = base::BindOnce(&MockImageCaptureClient::DoOnPhotoTaken, image_capture_client_);

    base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
    base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
    EXPECT_CALL(*image_capture_client_.get(), OnCorrectPhotoTaken()).Times(1).WillOnce(RunClosure(quit_closure));
...
{
    RunTestCase(base::BindOnce(&VideoCaptureDeviceTest::RunGetPhotoStateTestCase, base::Unretained(this)));
}
void VideoCaptureDeviceTest::RunGetPhotoStateTestCase()
{
    ...
    VideoCaptureDevice::GetPhotoStateCallback scoped_get_callback = base::BindOnce(&MockImageCaptureClient::DoOnGetPhotoState, image_capture_client_);
    ...
    base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
    base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
    EXPECT_CALL(*image_capture_client_.get(), OnCorrectGetPhotoState()).Times(1).WillOnce(RunClosure(quit_closure));
...
WRAPPED_TEST_P(VideoCaptureDeviceTest, MAYBE_UsingRealWebcam_CheckPhotoCallbackRelease)
{
    ...
    if (!static_cast<VideoCaptureDeviceMFWin*>(device.get())->get_use_photo_stream_to_take_photo_for_testing()) {
    ...
    static_cast<VideoCaptureDeviceMFWin*>(device.get())
        ->set_create_mf_photo_callback_for_testing(
            base::BindRepeating(&VideoCaptureDeviceTest::CreateMockPhotoCallback, base::Unretained(this), base::Unretained(callback)));

    VideoCaptureDevice::TakePhotoCallback scoped_callback = base::BindOnce(&MockImageCaptureClient::DoOnPhotoTaken, image_capture_client_);

    base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
    base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
    EXPECT_CALL(*image_capture_client_.get(), OnCorrectPhotoTaken()).WillOnce(RunClosure(quit_closure));
