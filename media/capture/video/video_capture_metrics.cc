...
constexpr auto kResolutions = base::MakeFixedFlatMap<gfx::Size, VideoResolutionDesignation>(
    {
...
static_assert(kResolutions.size() == static_cast<size_t>(VideoResolutionDesignation::kMaxValue), "Each resolution must have one entry in kResolutions.");
...
VideoResolutionDesignation ResolutionNameFromSize(gfx::Size frame_size)
{
    ...
    return it != kResolutions.end() ? it->second : VideoResolutionDesignation::kUnknown;
}

// These values are persisted to logs. Entries should not be renumbered and
// numeric values should never be reused.
enum class VideoEffectStatus { kUnsupported = 0, kSupported = 1, kEnabled = 2, kMaxValue = kEnabled };

} // namespace

namespace media {

void LogCaptureDeviceMetrics(base::span<const media::VideoCaptureDeviceInfo> devices_info)
{
    for (const auto& device : devices_info) {
        ...
        for (const auto& format : device.supported_formats) {
            VLOG(2) << "Device supports " << media::VideoPixelFormatToString(format.pixel_format) << " at " << format.frame_size.ToString() << " ("
            ...
            if (inserted) {
                base::UmaHistogramEnumeration("Media.VideoCapture.Device.SupportedPixelFormat", pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
            }
            if (!resolutions.contains(format.frame_size)) {
                resolutions.insert(format.frame_size);
                base::UmaHistogramEnumeration("Media.VideoCapture.Device.SupportedResolution", ResolutionNameFromSize(format.frame_size));
            }
        }
    }
    base::UmaHistogramCustomCounts("Media.VideoCapture.Device.TotalAvailable", devices_info.size(), 0, 5, 5);
}

void LogCaptureDeviceHashedModelId(const media::VideoCaptureDeviceDescriptor& descriptor)
{
    ...
    if (descriptor.model_id.length() == kModelIdStrLength) {
        ...
        const std::string pid = descriptor.model_id.substr(kColonPosIndex + 1, kModelIdStrLength);
...
VideoEffectStatus GetStatus(bool is_supported, bool is_enabled)
{
    ...
    return is_enabled ? VideoEffectStatus::kEnabled : VideoEffectStatus::kSupported;
}

void LogCaptureDeviceEffects(mojom::PhotoStatePtr photo_state)
{
    const bool has_background_blur
        = photo_state->supported_background_blur_modes && base::Contains(photo_state->supported_background_blur_modes.value(), mojom::BackgroundBlurMode::BLUR);
    const bool background_blur_enabled = photo_state->background_blur_mode != mojom::BackgroundBlurMode::OFF;
    UMA_HISTOGRAM_ENUMERATION("Media.VideoCapture.Device.Effect2.BackgroundBlur", GetStatus(has_background_blur, background_blur_enabled));

    const bool has_face_framing = photo_state->supported_face_framing_modes && photo_state->supported_face_framing_modes.value().size() > 0;
    const bool face_framing_enabled = photo_state->current_face_framing_mode != mojom::MeteringMode::NONE;
    UMA_HISTOGRAM_ENUMERATION("Media.VideoCapture.Device.Effect2.FaceFraming", GetStatus(has_face_framing, face_framing_enabled));

    const bool has_eye_gaze_correction = photo_state->supported_eye_gaze_correction_modes
        && (base::Contains(photo_state->supported_eye_gaze_correction_modes.value(), mojom::EyeGazeCorrectionMode::ON)
            || base::Contains(photo_state->supported_eye_gaze_correction_modes.value(), mojom::EyeGazeCorrectionMode::STARE));
    const bool eye_gaze_correction_enabled = photo_state->current_eye_gaze_correction_mode != mojom::EyeGazeCorrectionMode::OFF;
    UMA_HISTOGRAM_ENUMERATION("Media.VideoCapture.Device.Effect2.EyeGazeCorrection", GetStatus(has_eye_gaze_correction, eye_gaze_correction_enabled));
}
...
void LogCaptureCurrentDeviceResolution(int width, int height)
{
    ...
    if (width > kMinValue && width <= kMaxValue && height > kMinValue && height <= kMaxValue) {
    ...
    base::UmaHistogramSparse("Media.VideoCapture.Device.Opened.Resolution", result);
}

void LogCaptureCurrentDevicePixelFormat(const media::VideoPixelFormat pixel_format)
{
    base::UmaHistogramEnumeration("Media.VideoCapture.Device.Opened.PixelFormat", pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
}
