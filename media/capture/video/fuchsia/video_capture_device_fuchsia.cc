...
libyuv::FourCC GetFourccForPixelFormat(fuchsia::images2::PixelFormat src_pixel_format)
{
...
libyuv::RotationMode CameraOrientationToLibyuvRotation(fuchsia::camera3::Orientation orientation, bool* flip_y)
{
    switch (orientation) {
...
// static
VideoPixelFormat VideoCaptureDeviceFuchsia::GetConvertedPixelFormat(fuchsia::images2::PixelFormat format)
{
    switch (format) {
    case fuchsia::images2::PixelFormat::I420:
    case fuchsia::images2::PixelFormat::YV12:
    ...
    default:
        LOG(ERROR) << "Camera uses unsupported pixel format " << static_cast<int>(format);
        return PIXEL_FORMAT_UNKNOWN;
    }
}

// static
VideoPixelFormat VideoCaptureDeviceFuchsia::GetConvertedPixelFormat(fuchsia::sysmem::PixelFormatType format)
{
    ...
    // values.
    auto images2_pixel_format = static_cast<fuchsia::images2::PixelFormat>(fidl::ToUnderlying(format));
    return GetConvertedPixelFormat(images2_pixel_format);
}

bool VideoCaptureDeviceFuchsia::IsSupportedPixelFormat(fuchsia::images2::PixelFormat format)
{
    return GetConvertedPixelFormat(format) != PIXEL_FORMAT_UNKNOWN;
}

VideoCaptureDeviceFuchsia::VideoCaptureDeviceFuchsia(fidl::InterfaceHandle<fuchsia::camera3::Device> device)
    : sysmem_allocator_("CrVideoCaptureDeviceFuchsia")
{
    device_.Bind(std::move(device));
    device_.set_error_handler(fit::bind_member(this, &VideoCaptureDeviceFuchsia::OnDeviceError));
}
...
void VideoCaptureDeviceFuchsia::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<Client> client)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (!device_) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaCameraDeviceDisconnected, "fuchsia.camera3.Device disconnected");
        return;
    }
    ...
    stream_.set_error_handler(fit::bind_member(this, &VideoCaptureDeviceFuchsia::OnStreamError));
    ...
    stream_->SetBufferCollection(fuchsia::sysmem::BufferCollectionTokenHandle(sysmem_allocator_.CreateNewToken().Unbind().TakeChannel()));
...
void VideoCaptureDeviceFuchsia::OnDeviceError(zx_status_t status)
{
    OnError(FROM_HERE, VideoCaptureError::kFuchsiaCameraDeviceDisconnected,
        base::StringPrintf("fuchsia.camera3.Device disconnected: %s (%d)", zx_status_get_string(status), status));
}
...
void VideoCaptureDeviceFuchsia::OnStreamError(zx_status_t status)
{
    OnError(FROM_HERE, VideoCaptureError::kFuchsiaCameraStreamDisconnected,
        base::StringPrintf("fuchsia.camera3.Stream disconnected: %s (%d)", zx_status_get_string(status), status));
}
...
void VideoCaptureDeviceFuchsia::OnError(base::Location location, VideoCaptureError error, const std::string& reason)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VideoCaptureDeviceFuchsia::WatchResolution()
{
    ...
    stream_->WatchResolution(fit::bind_member(this, &VideoCaptureDeviceFuchsia::OnWatchResolutionResult));
}

void VideoCaptureDeviceFuchsia::OnWatchResolutionResult(fuchsia::math::Size frame_size)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VideoCaptureDeviceFuchsia::WatchOrientation()
{
    ...
    stream_->WatchOrientation(fit::bind_member(this, &VideoCaptureDeviceFuchsia::OnWatchOrientationResult));
}

void VideoCaptureDeviceFuchsia::OnWatchOrientationResult(fuchsia::camera3::Orientation orientation)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VideoCaptureDeviceFuchsia::WatchBufferCollection()
{
    ...
    stream_->WatchBufferCollection([this](fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_handle) {
        InitializeBufferCollection(fuchsia::sysmem2::BufferCollectionTokenHandle(token_handle.TakeChannel()));
        WatchBufferCollection();
    });
}

void VideoCaptureDeviceFuchsia::InitializeBufferCollection(fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_handle)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    fuchsia::sysmem2::BufferCollectionConstraints constraints = VmoBuffer::GetRecommendedConstraints(kMaxUsedOutputFrames,
        /*min_buffer_size=*/std::nullopt,
        /*writable=*/false);
    buffer_collection_ = sysmem_allocator_.BindSharedCollection(std::move(token));
    buffer_collection_->Initialize(std::move(constraints), "CrVideoCaptureDevice", kNamePriority);
    buffer_collection_->AcquireBuffers(base::BindOnce(&VideoCaptureDeviceFuchsia::OnBuffersAcquired, base::Unretained(this)));
}

void VideoCaptureDeviceFuchsia::OnBuffersAcquired(std::vector<VmoBuffer> buffers, const fuchsia::sysmem2::SingleBufferSettings& buffer_settings)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (!buffer_settings.has_image_format_constraints()) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaSysmemDidNotSetImageFormat, "Sysmem created buffer without image format constraints");
        return;
    }
    ...
    if (!IsSupportedPixelFormat(pixel_format)) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaUnsupportedPixelFormat,
            base::StringPrintf("Unsupported video frame format: %d", static_cast<int>(pixel_format)));
        return;
    }
...
void VideoCaptureDeviceFuchsia::ProcessNewFrame(fuchsia::camera3::FrameInfo frame_info)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(client_);
    ...
    if (index >= buffers_.size()) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaSysmemInvalidBufferIndex, base::StringPrintf("Received frame with invalid buffer_index=%zu", index));
        return;
    }
    ...
    size_t src_coded_width
        = RoundUp(std::max(buffers_format_.min_size().width, buffers_format_.required_max_size().width), buffers_format_.size_alignment().width);
    size_t src_coded_height
        = RoundUp(std::max(buffers_format_.min_size().height, buffers_format_.required_max_size().height), buffers_format_.size_alignment().height);
    size_t src_stride = RoundUp(std::max(static_cast<size_t>(buffers_format_.min_bytes_per_row()), src_coded_width), buffers_format_.bytes_per_row_divisor());
    gfx::Size visible_size = frame_size_.value_or(gfx::Size(src_coded_width, src_coded_height));
    gfx::Size nonrotated_output_size((visible_size.width() + 1) & ~1, (visible_size.height() + 1) & ~1);

    bool flip_y;
    libyuv::RotationMode rotation = CameraOrientationToLibyuvRotation(orientation_, &flip_y);

    gfx::Size output_size = RotateSize(nonrotated_output_size, rotation);
    visible_size = RotateSize(visible_size, rotation);

    base::TimeTicks reference_time = base::TimeTicks::FromZxTime(frame_info.timestamp);
    base::TimeDelta timestamp = std::max(reference_time - start_time_, base::TimeDelta());

    ++frames_received_;
    float frame_rate = (timestamp.is_positive()) ? static_cast<float>(frames_received_) / timestamp.InSecondsF() : 0.0;
    VideoCaptureFormat capture_format(output_size, frame_rate, PIXEL_FORMAT_I420);

    Client::Buffer buffer;
    Client::ReserveResult result = client_->ReserveOutputBuffer(capture_format.frame_size, capture_format.pixel_format,
    ...
    if (src_span.empty()) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaFailedToMapSysmemBuffer, "Failed to map buffers allocated by sysmem");
        return;
    }
    ...
    if (src_span.size() < src_buffer_size) {
        OnError(FROM_HERE, VideoCaptureError::kFuchsiaSysmemInvalidBufferSize, "Sysmem allocated buffer that's smaller than expected");
        return;
    }

    std::unique_ptr<VideoCaptureBufferHandle> output_handle = buffer.handle_provider->GetHandleForInProcessAccess();
    ...
    libyuv::ConvertToI420(src_span.data(), src_span.size(), dst_y, dst_stride_y, dst_u, dst_stride_u, dst_v, dst_stride_v,
        /*crop_x=*/0, /*crop_y=*/0, src_stride, flipped_src_height, nonrotated_output_size.width(), nonrotated_output_size.height(), rotation, four_cc);

    client_->OnIncomingCapturedBufferExt(
        std::move(buffer), capture_format, gfx::ColorSpace(), reference_time, timestamp, std::nullopt, gfx::Rect(visible_size), VideoFrameMetadata());
