...
void ValidateReceivedFrame(const ReceivedFrame& frame, gfx::Size expected_size, uint8_t salt)
{
    gfx::Size coded_size((expected_size.width() + 1) & ~1, (expected_size.height() + 1) & ~1);
...
public:
    HeapBufferHandle(size_t size, uint8_t* data)
        : size_(size)
        , data_(data)
    {
    }
...
class HeapBufferHandleProvider final : public VideoCaptureDevice::Client::Buffer::HandleProvider {
public:
    HeapBufferHandleProvider(size_t size)
        : data_(size)
    {
    }
...
private:
    ...
    ReserveResult ReserveOutputBuffer(const gfx::Size& dimensions, VideoPixelFormat format, int frame_feedback_id, Buffer* buffer, int* require_new_buffer_id,
    ...
    void OnIncomingCapturedBufferExt(Buffer buffer, const VideoCaptureFormat& format, const gfx::ColorSpace& color_space, base::TimeTicks reference_time,
        base::TimeDelta timestamp, std::optional<base::TimeTicks> capture_begin_time, gfx::Rect visible_rect,
        const VideoFrameMetadata& additional_metadata) override
    {
        EXPECT_TRUE(started_);

        received_frames_.push_back(ReceivedFrame { std::move(buffer), format, color_space, reference_time, timestamp, visible_rect });
    ...
    void OnIncomingCapturedBuffer(Buffer buffer, const VideoCaptureFormat& format, base::TimeTicks reference_time, base::TimeDelta timestamp,
        std::optional<base::TimeTicks> capture_begin_time) override
    {
        NOTREACHED();
    }
    void OnError(VideoCaptureError error, const base::Location& from_here, const std::string& reason) override
    {
        NOTREACHED();
    }
...
public:
    ...
    std::vector<VideoCaptureDeviceInfo> GetDevicesInfo()
    {
        ...
        device_factory_.GetDevicesInfo(base::BindLambdaForTesting([&devices_info, &run_loop](std::vector<VideoCaptureDeviceInfo> result) {
    ...
    void CreateDevice()
    {
        ...
        device_ = device_factory_.CreateDevice(devices_info[0].descriptor).ReleaseDevice();
    }
    ...
    void ProduceAndCaptureFrame()
    {
        ...
        ValidateReceivedFrame(client_->received_frames()[0], FakeCameraStream::kDefaultFrameSize, kFrameSalt);
    }
...
protected:
    ...
    FakeCameraDeviceWatcher fake_device_watcher_ { test_context_.additional_services() };
...
{
    GetDefaultCameraStream()->SetFirstBufferCollectionFailMode(FakeCameraStream::SysmemFailMode::kFailSync);
...
{
    GetDefaultCameraStream()->SetFirstBufferCollectionFailMode(FakeCameraStream::SysmemFailMode::kFailAllocation);
...
TEST_F(VideoCaptureDeviceFuchsiaTest, MultipleFrames)
{
    ...
    for (size_t i = 0; i < 10; ++i) {
        ...
        ValidateReceivedFrame(client_->received_frames()[i], FakeCameraStream::kDefaultFrameSize, i);
    }
}
...
TEST_F(VideoCaptureDeviceFuchsiaTest, FrameRotation)
{
    ...
    for (int i = static_cast<int>(fuchsia::camera3::Orientation::UP); i <= static_cast<int>(fuchsia::camera3::Orientation::RIGHT_FLIPPED); ++i) {
        ...
        if (orientation == fuchsia::camera3::Orientation::LEFT || orientation == fuchsia::camera3::Orientation::LEFT_FLIPPED
            || orientation == fuchsia::camera3::Orientation::RIGHT || orientation == fuchsia::camera3::Orientation::RIGHT_FLIPPED) {
...
TEST_F(VideoCaptureDeviceFuchsiaTest, MidStreamResolutionChange)
{
    ...
    ValidateReceivedFrame(client_->received_frames()[0], FakeCameraStream::kDefaultFrameSize, 1);
    ValidateReceivedFrame(client_->received_frames()[1], kUpdatedResolution, 1);
}

TEST_F(VideoCaptureDeviceFuchsiaTest, CreateDeviceAfterDeviceWatcherDisconnect)
{
    auto devices_info = GetDevicesInfo();
    ASSERT_EQ(devices_info.size(), 1U);
