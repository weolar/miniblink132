...
public:
    ...
    {
        device_.set_error_handler(fit::bind_member(this, &DeviceConfigFetcher::OnError));
    }
    ...
    void Fetch(base::OnceClosure on_fetched_callback)
    {
        ...
        device_->GetIdentifier(fit::bind_member(this, &DeviceConfigFetcher::OnDescription));
        device_->GetConfigurations(fit::bind_member(this, &DeviceConfigFetcher::OnConfigurations));
    }
    ...
    bool have_results() const
    {
        return description_ && formats_;
    }
    bool is_usable() const
    {
        return device_ || have_results();
    }
    ...
    const VideoCaptureFormats& formats() const
    {
        return formats_.value();
    }
    ...
    void OnConfigurations(std::vector<fuchsia::camera3::Configuration> configs)
    {
        ...
        for (auto& config : configs) {
            for (auto& props : config.streams) {
                ...
                format.frame_size = gfx::Size(props.image_format.display_width, props.image_format.display_height);
                format.frame_rate = static_cast<float>(props.frame_rate.numerator) / props.frame_rate.denominator;
                format.pixel_format = VideoCaptureDeviceFuchsia::GetConvertedPixelFormat(props.image_format.pixel_format.type);
...
VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryFuchsia::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    uint64_t device_id;
    bool converted = base::StringToUint64(device_descriptor.device_id, &device_id);
    ...
    return VideoCaptureErrorOrDevice(std::make_unique<VideoCaptureDeviceFuchsia>(std::move(device)));
}

void VideoCaptureDeviceFactoryFuchsia::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VideoCaptureDeviceFactoryFuchsia::Initialize()
{
    ...
    base::ComponentContextForProcess()->svc()->Connect(device_watcher_.NewRequest());

    device_watcher_.set_error_handler(fit::bind_member(this, &VideoCaptureDeviceFactoryFuchsia::OnDeviceWatcherDisconnected));

    WatchDevices();
}

void VideoCaptureDeviceFactoryFuchsia::OnDeviceWatcherDisconnected(zx_status_t status)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VideoCaptureDeviceFactoryFuchsia::WatchDevices()
{
    ...
    device_watcher_->WatchDevices(fit::bind_member(this, &VideoCaptureDeviceFactoryFuchsia::OnWatchDevicesResult));
}

void VideoCaptureDeviceFactoryFuchsia::OnWatchDevicesResult(std::vector<fuchsia::camera3::WatchDevicesEvent> events)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (auto& e : events) {
        ...
        fetcher->Fetch(base::BindOnce(&VideoCaptureDeviceFactoryFuchsia::OnDeviceInfoFetched, base::Unretained(this)));
        num_pending_device_info_requests_++;
...
std::vector<VideoCaptureDeviceInfo> VideoCaptureDeviceFactoryFuchsia::MakeDevicesInfo()
{
        ...
        for (auto& device : devices_.value()) {
            ...
            if (device.second->is_usable()) {
                devices_info.emplace_back(
                    VideoCaptureDeviceDescriptor(device.second->description(), base::NumberToString(device.first), VideoCaptureApi::FUCHSIA_CAMERA3));
...
void VideoCaptureDeviceFactoryFuchsia::MaybeResolvePendingDeviceInfoCallbacks()
{
