...
TEST_F(VideoCaptureDeviceFactoryFuchsiaTest, RemoveWhileEnumerating)
{
    ...
    fake_device_watcher_.devices().begin()->second->SetGetIdentifierHandler(
        base::BindLambdaForTesting([&get_identifier_run_loop](fuchsia::camera3::Device::GetIdentifierCallback callback) { get_identifier_run_loop.Quit(); }));

    base::RunLoop get_devices_run_loop;
    device_factory_.GetDevicesInfo(base::BindLambdaForTesting([&get_devices_run_loop, &devices_info](std::vector<VideoCaptureDeviceInfo> result) {
        devices_info = std::move(result);
    ...
    auto stream_and_device = fake_device_watcher_.RemoveDevice(fake_device_watcher_.devices().begin()->first);

    get_devices_run_loop.Run();
    EXPECT_TRUE(devices_info.empty());
}

class TestDeviceChangeObserver : public base::SystemMonitor::DevicesChangedObserver {
public:
...
TEST_F(VideoCaptureDeviceFactoryFuchsiaTest, DeviceChangeEvent)
{
    ...
    fake_device_watcher_.RemoveDevice(fake_device_watcher_.devices().begin()->first);
    base::RunLoop().RunUntilIdle();
