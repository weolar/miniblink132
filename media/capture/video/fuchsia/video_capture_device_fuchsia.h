...
class CAPTURE_EXPORT VideoCaptureDeviceFuchsia final : public VideoCaptureDevice {
public:
    ...
    // unsupported formats.
    static VideoPixelFormat GetConvertedPixelFormat(fuchsia::images2::PixelFormat format);
    static VideoPixelFormat GetConvertedPixelFormat(fuchsia::sysmem::PixelFormatType format);

    static bool IsSupportedPixelFormat(fuchsia::images2::PixelFormat format);

    explicit VideoCaptureDeviceFuchsia(fidl::InterfaceHandle<fuchsia::camera3::Device> device);
    ~VideoCaptureDeviceFuchsia() override;

    VideoCaptureDeviceFuchsia(const VideoCaptureDeviceFuchsia&) = delete;
    VideoCaptureDeviceFuchsia& operator=(const VideoCaptureDeviceFuchsia&) = delete;
...
private:
    ...
    void OnError(base::Location location, VideoCaptureError error, const std::string& reason);
    ...
    void InitializeBufferCollection(fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_handle);

    // Callback for SysmemCollectionClient::AcquireBuffers().
    void OnBuffersAcquired(std::vector<VmoBuffer> buffers, const fuchsia::sysmem2::SingleBufferSettings& buffer_settings);
