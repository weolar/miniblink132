...
public:
    ...
    static std::unique_ptr<PixelBufferPool> Create(OSType format, int width, int height, absl::optional<size_t> max_buffers);
    ~PixelBufferPool();
...
private:
    friend std::unique_ptr<PixelBufferPool> std::make_unique<PixelBufferPool>(
        base::ScopedCFTypeRef<CVPixelBufferPoolRef>&& buffer_pool, absl::optional<size_t>&& max_buffers);
