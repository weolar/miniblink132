...
public:
    ...
    // capture formats.
    static void EnumerateDevices(std::vector<VideoCaptureDeviceInfo>* devices_info);

    explicit VideoCaptureDeviceDeckLinkMac(const VideoCaptureDeviceDescriptor& descriptor);

    VideoCaptureDeviceDeckLinkMac(const VideoCaptureDeviceDeckLinkMac&) = delete;
    VideoCaptureDeviceDeckLinkMac& operator=(const VideoCaptureDeviceDeckLinkMac&) = delete;
    ...
    void OnIncomingCapturedData(const uint8_t* data, size_t length, const VideoCaptureFormat& frame_format, const gfx::ColorSpace& color_space,
        int rotation, // Clockwise.
        bool flip_y, base::TimeTicks reference_time, base::TimeDelta timestamp);

    // Forwarder to VideoCaptureDevice::Client::OnError().
    void SendErrorString(VideoCaptureError error, const base::Location& from_here, const std::string& reason);
