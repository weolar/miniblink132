...
constexpr GUID GUID_MEDIA_TYPE_INDEX = { 0x12345678, 0xaaaa, 0xbbbb, { 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7 } };
...
public:
    ...
    void OnIncomingCapturedData(const uint8_t* data, int length, const VideoCaptureFormat& frame_format, const gfx::ColorSpace& color_space,
        int clockwise_rotation, bool flip_y, base::TimeTicks reference_time, base::TimeDelta timestamp, std::optional<base::TimeTicks> capture_begin_time,
        int frame_feedback_id) override
    {
    }

    void OnIncomingCapturedGfxBuffer(gfx::GpuMemoryBuffer* buffer, const VideoCaptureFormat& frame_format, int clockwise_rotation,
        base::TimeTicks reference_time, base::TimeDelta timestamp, std::optional<base::TimeTicks> capture_begin_time, int frame_feedback_id) override
    {
    }

    void OnIncomingCapturedExternalBuffer(CapturedExternalVideoBuffer buffer, base::TimeTicks reference_time, base::TimeDelta timestamp,
        std::optional<base::TimeTicks> capture_begin_time, const gfx::Rect& visible_rect) override
    {
    }

    MOCK_METHOD6(ReserveOutputBuffer, ReserveResult(const gfx::Size&, VideoPixelFormat, int, Buffer*, int*, int*));

    void OnIncomingCapturedBuffer(Buffer buffer, const VideoCaptureFormat& format, base::TimeTicks reference_, base::TimeDelta timestamp,
        std::optional<base::TimeTicks> capture_begin_time) override
    {
    }

    MOCK_METHOD8(OnIncomingCapturedBufferExt,
        void(Buffer, const VideoCaptureFormat&, const gfx::ColorSpace&, base::TimeTicks, base::TimeDelta, std::optional<base::TimeTicks>, gfx::Rect,
            const VideoFrameMetadata&));

    MOCK_METHOD3(OnError, void(VideoCaptureError, const base::Location&, const std::string&));
...
class MockImageCaptureClient : public base::RefCountedThreadSafe<MockImageCaptureClient> {
public:
...
template <class Interface> Interface* AddReference(Interface* object)
{
    DCHECK(object);
    object->AddRef();
    return object;
...
template <class Interface> class MockInterface : public base::RefCountedThreadSafe<MockInterface<Interface>>, public Interface {
public:
    // IUnknown
...
public:
    ...
    IFACEMETHODIMP GetRange(long property, long* min, long* max, long* step, long* default_value, long* caps_flags) override
    {
        switch (property) {
        case CameraControl_Pan:
        case CameraControl_Tilt:
...
class MockMFExtendedCameraControl final : public MockInterface<IMFExtendedCameraControl> {
public:
    MockMFExtendedCameraControl(ULONG property_id)
        : property_id_(property_id)
    {
    }
    IFACEMETHODIMP CommitSettings() override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP_(ULONGLONG) GetCapabilities() override
    {
        ...
        case KSPROPERTY_CAMERACONTROL_EXTENDED_BACKGROUNDSEGMENTATION:
            return (KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_OFF | KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_BLUR);
        case KSPROPERTY_CAMERACONTROL_EXTENDED_DIGITALWINDOW:
            return (KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_AUTOFACEFRAMING | KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_MANUAL);
        case KSPROPERTY_CAMERACONTROL_EXTENDED_EYEGAZECORRECTION:
            return (KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_OFF | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_ON | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_STARE);
        default:
            return 0;
        }
    }
    ...
    IFACEMETHODIMP SetFlags(ULONGLONG flags) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP UnlockPayload() override
    {
        return E_NOTIMPL;
    }
...
class MockMFExtendedCameraController final : public MockInterface<IMFExtendedCameraController> {
public:
    IFACEMETHODIMP GetExtendedCameraControl(DWORD stream_index, ULONG property_id, IMFExtendedCameraControl** control) override
    {
...
class MockMFMediaSource final : public MockInterface<IMFMediaSourceEx>, public IMFGetService {
public:
    // IUnknown
    IFACEMETHODIMP_(ULONG) AddRef() override
    {
        return MockInterface::AddRef();
    }
    IFACEMETHODIMP QueryInterface(REFIID riid, void** object) override
    {
    ...
    IFACEMETHODIMP Stop(void) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP Pause(void) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP Shutdown(void) override
    {
        return E_NOTIMPL;
    }
    // IMFMediaSourceEx
    ...
    IFACEMETHODIMP GetStreamAttributes(DWORD stream_id, IMFAttributes** attributes)
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP SetD3DManager(IUnknown* manager)
    {
        return S_OK;
    }
    // IMFGetService
    IFACEMETHODIMP GetService(REFGUID guidService, REFIID riid, void** object) override
    {
...
public:
    IFACEMETHODIMP GetCaptureDeviceSource(MF_CAPTURE_ENGINE_DEVICE_TYPE mfCaptureEngineDeviceType, IMFMediaSource** ppMediaSource) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP GetCaptureDeviceActivate(MF_CAPTURE_ENGINE_DEVICE_TYPE mfCaptureEngineDeviceType, IMFActivate** ppActivate) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP GetService(REFIID rguidService, REFIID riid, IUnknown** ppUnknown) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP AddEffect(DWORD dwSourceStreamIndex, IUnknown* pUnknown) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP RemoveEffect(DWORD dwSourceStreamIndex, IUnknown* pUnknown) override
    {
        return E_NOTIMPL;
    }
    ...
    IFACEMETHODIMP GetAvailableDeviceMediaType(DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) override
    {
        return DoGetAvailableDeviceMediaType(stream_index, media_type_index, media_type);
    }

    MOCK_METHOD3(DoGetAvailableDeviceMediaType, HRESULT(DWORD, DWORD, IMFMediaType**));

    IFACEMETHODIMP SetCurrentDeviceMediaType(DWORD dwSourceStreamIndex, IMFMediaType* pMediaType) override
    {
    ...
    IFACEMETHODIMP GetCurrentDeviceMediaType(DWORD stream_index, IMFMediaType** media_type)
    {
    ...
    IFACEMETHODIMP GetDeviceStreamCategory(DWORD stream_index, MF_CAPTURE_ENGINE_STREAM_CATEGORY* category)
    {
        return DoGetDeviceStreamCategory(stream_index, category);
    }
    MOCK_METHOD2(DoGetDeviceStreamCategory, HRESULT(DWORD, MF_CAPTURE_ENGINE_STREAM_CATEGORY*));

    IFACEMETHODIMP GetMirrorState(DWORD dwStreamIndex, BOOL* pfMirrorState) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP SetMirrorState(DWORD dwStreamIndex, BOOL fMirrorState) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP GetStreamIndexFromFriendlyName(UINT32 uifriendlyName, DWORD* pdwActualStreamIndex) override
    {
        return E_NOTIMPL;
    }
...
public:
    IFACEMETHODIMP GetOutputMediaType(DWORD dwSinkStreamIndex, IMFMediaType** ppMediaType) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP GetService(DWORD dwSinkStreamIndex, REFGUID rguidService, REFIID riid, IUnknown** ppUnknown) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP AddStream(DWORD stream_index, IMFMediaType* media_type, IMFAttributes* attributes, DWORD* sink_stream_index) override
    {
        return DoAddStream(stream_index, media_type, attributes, sink_stream_index);
    }

    MOCK_METHOD4(DoAddStream, HRESULT(DWORD, IMFMediaType*, IMFAttributes*, DWORD*));

    IFACEMETHODIMP Prepare(void) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP RemoveAllStreams(void) override
    {
        return S_OK;
    }
    IFACEMETHODIMP SetRenderHandle(HANDLE handle) override
    {
        return E_NOTIMPL;
    }
    ...
    IFACEMETHODIMP UpdateVideo(const MFVideoNormalizedRect* pSrc, const RECT* pDst, const COLORREF* pBorderClr) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP SetSampleCallback(DWORD dwStreamSinkIndex, IMFCaptureEngineOnSampleCallback* pCallback) override
    {
    ...
    IFACEMETHODIMP GetRotation(DWORD dwStreamIndex, DWORD* pdwRotationValue) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP SetRotation(DWORD dwStreamIndex, DWORD dwRotationValue) override
    {
        return E_NOTIMPL;
    }
...
public:
    ...
    IFACEMETHODIMP AddStream(DWORD dwSourceStreamIndex, IMFMediaType* pMediaType, IMFAttributes* pAttributes, DWORD* pdwSinkStreamIndex) override
    {
        return S_OK;
    }
    IFACEMETHODIMP Prepare(void) override
    ...
    IFACEMETHODIMP SetSampleCallback(IMFCaptureEngineOnSampleCallback* pCallback) override
    {
        sample_callback = pCallback;
        return S_OK;
    }
...
public:
    IFACEMETHODIMP Initialize(
        IMFCaptureEngineOnEventCallback* pEventCallback, IMFAttributes* pAttributes, IUnknown* pAudioSource, IUnknown* pVideoSource) override
    {
        ...
        EXPECT_EQ(SUCCEEDED(pAttributes->GetUnknown(MF_CAPTURE_ENGINE_D3D_MANAGER, IID_PPV_ARGS(&device_manager))), expect_mf_dxgi_device_manager_attribute_);
        event_callback = pEventCallback;
        OnCorrectInitializeQueued();

        ON_CALL(*this, OnInitStatus).WillByDefault(Return(S_OK));
        ON_CALL(*this, OnInitEventGuid).WillByDefault(Return(MF_CAPTURE_ENGINE_INITIALIZED));
        // HW Cameras usually add about 500ms latency on init
        ON_CALL(*this, InitEventDelay).WillByDefault(Return(base::Milliseconds(500)));

        base::TimeDelta event_delay = InitEventDelay();

        base::ThreadPool::PostDelayedTask(
            FROM_HERE, base::BindOnce(&MockMFCaptureEngine::FireCaptureEvent, this, OnInitEventGuid(), OnInitStatus()), event_delay);
    ...
    IFACEMETHODIMP StartRecord(void) override
    {
        return E_NOTIMPL;
    }
    ...
    IFACEMETHODIMP GetSink(MF_CAPTURE_ENGINE_SINK_TYPE type, IMFCaptureSink** sink)
    {
        return DoGetSink(type, sink);
    }
    MOCK_METHOD2(DoGetSink, HRESULT(MF_CAPTURE_ENGINE_SINK_TYPE, IMFCaptureSink**));
...
public:
    MockDXGIDeviceManager(Microsoft::WRL::ComPtr<IMFDXGIDeviceManager> mf_dxgi_device_manager, UINT reset_tocken)
        : DXGIDeviceManager(std::move(mf_dxgi_device_manager), reset_tocken, CHROME_LUID { 0, 0 })
    {
        Microsoft::WRL::ComPtr<ID3D11Device> d3d_device;
        DXGIDeviceManager::ResetDevice(d3d_device);
        ON_CALL(*this, DoResetDevice).WillByDefault([this](Microsoft::WRL::ComPtr<ID3D11Device>* device) { return DXGIDeviceManager::ResetDevice(*device); });
        ON_CALL(*this, GetDevice).WillByDefault([this]() { return DXGIDeviceManager::GetDevice(); });
    }

    HRESULT ResetDevice(Microsoft::WRL::ComPtr<ID3D11Device>& d3d_device) override
    {
...
public:
    StubMFMediaType(GUID major_type, GUID sub_type, int media_type_index, int frame_width, int frame_height, int frame_rate)
    ...
    IFACEMETHODIMP DeleteAllItems(void) override
    {
        return E_NOTIMPL;
    }
    ...
    IFACEMETHODIMP LockStore(void) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP UnlockStore(void) override
    {
        return E_NOTIMPL;
    }
    ...
    IFACEMETHODIMP GetRepresentation(GUID guidRepresentation, LPVOID* ppvRepresentation) override
    {
        return E_NOTIMPL;
    }
    IFACEMETHODIMP FreeRepresentation(GUID guidRepresentation, LPVOID pvRepresentation) override
    {
        return E_NOTIMPL;
    }
...
public:
    ...
    IFACEMETHODIMP GetValue(PROPVARIANT* pvValue) override
    {
        return E_NOTIMPL;
    }
...
class MockCaptureHandleProvider : public VideoCaptureDevice::Client::Buffer::HandleProvider {
public:
    ...
    std::unique_ptr<VideoCaptureBufferHandle> GetHandleForInProcessAccess() override
    {
        return nullptr;
    }
    ...
    gfx::GpuMemoryBufferHandle GetGpuMemoryBufferHandle() override
    {
        ...
        // handle)
        base::win::ScopedHandle fake_dxgi_handle(CreateEvent(nullptr, FALSE, FALSE, nullptr));
...
protected:
    VideoCaptureDeviceMFWinTest()
        ...
        , device_(new VideoCaptureDeviceMFWin(descriptor_, media_source_, nullptr, engine_, task_runner_))
        , capture_source_(new MockMFCaptureSource())
        , capture_preview_sink_(new MockCapturePreviewSink())
        , media_foundation_supported_(VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation())
    {
    }
    ...
    void SetUp() override
    {
        ...
        engine_->set_expect_mf_dxgi_device_manager_attribute(dxgi_device_manager_ != nullptr);

        EXPECT_CALL(*(engine_.Get()), OnCorrectInitializeQueued());
        task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { EXPECT_TRUE(device_->Init()); }));
        task_environment_.RunUntilIdle();
        EXPECT_CALL(*(engine_.Get()), DoGetSource()).WillRepeatedly(Invoke([this]() {
    ...
    {
        EXPECT_CALL(*capture_source_, DoGetDeviceStreamCount(_)).WillRepeatedly(Invoke([](DWORD* stream_count) {
            *stream_count = 1;
            return S_OK;
        }));
        EXPECT_CALL(*capture_source_, DoGetDeviceStreamCategory(0, _))
            .WillRepeatedly(Invoke([](DWORD stream_index, MF_CAPTURE_ENGINE_STREAM_CATEGORY* category) {
                *category = MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_PREVIEW;
                return S_OK;
            }));

        EXPECT_CALL(*capture_source_, DoGetAvailableDeviceMediaType(0, _, _))
            .WillRepeatedly(Invoke([mf_video_subtype](DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) {
                if (media_type_index != 0)
                    return MF_E_NO_MORE_TYPES;

                *media_type = new StubMFMediaType(MFMediaType_Video, mf_video_subtype, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();

                return S_OK;
            }));

        EXPECT_CALL(*(engine_.Get()), DoGetSink(MF_CAPTURE_ENGINE_SINK_TYPE_PREVIEW, _))
            .WillRepeatedly(Invoke([this](MF_CAPTURE_ENGINE_SINK_TYPE sink_type, IMFCaptureSink** sink) {
        ...
        EXPECT_CALL(*capture_source_, DoGetCurrentDeviceMediaType(_, _))
            .WillRepeatedly(Invoke([mf_video_subtype](DWORD stream_index, IMFMediaType** media_type) {
                *media_type = new StubMFMediaType(MFMediaType_Video, mf_video_subtype, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
                return S_OK;
            }));
    ...
    void PrepareMFDeviceWithVideoStreams(std::vector<GUID> mf_video_subtypes)
    {
        ...
        EXPECT_CALL(*capture_source_, DoGetAvailableDeviceMediaType(0, _, _))
            .WillRepeatedly(Invoke([mf_video_subtypes](DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) {
                if (media_type_index >= mf_video_subtypes.size())
                    return MF_E_NO_MORE_TYPES;

                *media_type = new StubMFMediaType(
                    MFMediaType_Video, mf_video_subtypes[media_type_index], media_type_index, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
        ...
        EXPECT_CALL(*capture_source_, DoGetCurrentDeviceMediaType(_, _))
            .WillRepeatedly(Invoke([mf_video_subtypes](DWORD stream_index, IMFMediaType** media_type) {
                *media_type = new StubMFMediaType(MFMediaType_Video, mf_video_subtypes[0], 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
                return S_OK;
            }));
    }

    void PrepareMFDeviceWithOneVideoStreamAndOnePhotoStream(GUID mf_video_subtype)
    {
        EXPECT_CALL(*capture_source_, DoGetDeviceStreamCount(_)).WillRepeatedly(Invoke([](DWORD* stream_count) {
            *stream_count = 2;
            return S_OK;
        ...
        auto get_device_media_type = [mf_video_subtype](DWORD stream_index, IMFMediaType** media_type) {
            if (stream_index == 0) {
                *media_type = new StubMFMediaType(MFMediaType_Video, mf_video_subtype, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
                return S_OK;
            } else if (stream_index == 1) {
                *media_type = new StubMFMediaType(MFMediaType_Image, GUID_ContainerFormatJpeg, 0, kArbitraryValidPhotoWidth, kArbitraryValidPhotoHeight, 0);
                (*media_type)->AddRef();
                return S_OK;
            }
            return E_FAIL;
        ...
        EXPECT_CALL(*capture_source_, DoGetAvailableDeviceMediaType(_, _, _))
            .WillRepeatedly(Invoke([get_device_media_type](DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) {
                if (media_type_index != 0)
                    return MF_E_NO_MORE_TYPES;
                return get_device_media_type(stream_index, media_type);
            }));

        EXPECT_CALL(*(engine_.Get()), DoGetSink(_, _)).WillRepeatedly(Invoke([this](MF_CAPTURE_ENGINE_SINK_TYPE sink_type, IMFCaptureSink** sink) {
        ...
        EXPECT_CALL(*capture_source_, DoGetCurrentDeviceMediaType(_, _)).WillRepeatedly(Invoke(get_device_media_type));
    }

    void PrepareMFDepthDeviceWithCombinedFormatsAndStreams(DepthDeviceParams params)
    {
        EXPECT_CALL(*capture_source_, DoGetDeviceStreamCount(_)).WillRepeatedly(Invoke([params](DWORD* stream_count) {
        ...
        auto get_device_media_type = [params](DWORD stream_index, IMFMediaType** media_type) {
            if (stream_index == 0) {
                *media_type
                    = new StubMFMediaType(MFMediaType_Video, params.depth_video_stream_subtype, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
                return S_OK;
            } else if (stream_index == 1 && params.additional_i420_video_stream) {
                *media_type = new StubMFMediaType(MFMediaType_Video, MFVideoFormat_I420, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                (*media_type)->AddRef();
                return S_OK;
            }
            return E_FAIL;
        ...
        EXPECT_CALL(*capture_source_, DoGetAvailableDeviceMediaType(_, _, _))
            .WillRepeatedly(Invoke([params, get_device_media_type](DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) {
                if (stream_index == 0 && params.additional_i420_formats_in_depth_stream && media_type_index == 1) {
                    *media_type = new StubMFMediaType(MFMediaType_Video, MFVideoFormat_I420, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
                    (*media_type)->AddRef();
                    return S_OK;
                }
...
TEST_F(VideoCaptureDeviceMFWinTest, StartPreviewOnAllocateAndStart)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&] { device_->StopAndDeAllocate(); }));
    task_environment_.RunUntilIdle();
}
...
TEST_F(VideoCaptureDeviceMFWinTest, PopulateCameraRotationOnSample)
{
    ...
    Microsoft::WRL::ComPtr<IMFSample> test_sample = CreateEmptySampleWithBuffer(kMFSampleBufferLength, 0);
...
TEST_F(VideoCaptureDeviceMFWinTest, CallClientOnErrorDurringInit)
{
    ...
    Microsoft::WRL::ComPtr<MockMFMediaSource> media_source = new MockMFMediaSource();
    Microsoft::WRL::ComPtr<MockMFCaptureEngine> engine = new MockMFCaptureEngine();
    std::unique_ptr<VideoCaptureDeviceMFWin> device = std::make_unique<VideoCaptureDeviceMFWin>(descriptor, media_source,
        /*mf_dxgi_device_manager=*/nullptr, engine, task_runner_);

    EXPECT_CALL(*(engine.Get()), OnInitEventGuid).WillOnce([]() { return MF_CAPTURE_ENGINE_INITIALIZED; });
    // E_ACCESSDENIED is thrown if application is denied access in settings UI
    EXPECT_CALL(*(engine.Get()), OnInitStatus).WillOnce([]() { return E_ACCESSDENIED; });

    EXPECT_CALL(*(engine.Get()), OnCorrectInitializeQueued());

    task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&device] { EXPECT_FALSE(device->Init()); }));
...
TEST_F(VideoCaptureDeviceMFWinTest, CallClientOnFireCaptureEngineInitEarly)
{
    ...
    EXPECT_CALL(*(engine.Get()), OnInitEventGuid).WillOnce([]() { return MF_CAPTURE_ENGINE_INITIALIZED; });
    EXPECT_CALL(*(engine.Get()), InitEventDelay).WillOnce([]() { return base::Milliseconds(0); });

    EXPECT_CALL(*(engine.Get()), OnCorrectInitializeQueued());

    task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&device] { EXPECT_TRUE(device->Init()); }));
    task_environment_.RunUntilIdle();
}

// Send MFVideoCallback::OnEvent when VideoCaptureDeviceMFWin has been destroyed
TEST_F(VideoCaptureDeviceMFWinTest, SendMFVideoCallbackAfterVideoCaptureDeviceMFWinDestructor)
{
    if (ShouldSkipTest())
        return;
    ...
    engine->FireCaptureEvent(MF_CAPTURE_ENGINE_ERROR, MF_E_VIDEO_RECORDING_DEVICE_INVALIDATED);
    task_environment_.RunUntilIdle();
}

// Send random event before MF_CAPTURE_ENGINE_STOPPED
TEST_F(VideoCaptureDeviceMFWinTest, SendArbitraryMFVideoCallbackBeforeOnStoppedEvent)
{
    if (ShouldSkipTest())
        return;
    ...
    Microsoft::WRL::ComPtr<MockMFMediaSource> media_source = new MockMFMediaSource();
    Microsoft::WRL::ComPtr<MockMFCaptureEngine> engine = new MockMFCaptureEngine();
    auto device = std::make_unique<VideoCaptureDeviceMFWin>(descriptor, media_source,
        /*mf_dxgi_device_manager=*/nullptr, engine, task_runner_);
    ...
    EXPECT_CALL(*(engine_.Get()), OnStopPreview()).WillRepeatedly([&]() { engine->FireCaptureEvent(MF_CAPTURE_ENGINE_CAMERA_STREAM_BLOCKED, S_OK); });
...
TEST_F(VideoCaptureDeviceMFWinTest, AllocateAndStartWithFlakyInvalidRequest)
{
    ...
    EXPECT_CALL(*capture_source_, DoGetAvailableDeviceMediaType(0, _, _))
        .Times(AtLeast(2))
        .WillOnce(Return(MF_E_INVALIDREQUEST))
        .WillRepeatedly(Invoke([](DWORD stream_index, DWORD media_type_index, IMFMediaType** media_type) {
            if (media_type_index != 0)
                return MF_E_NO_MORE_TYPES;

            *media_type = new StubMFMediaType(MFMediaType_Video, MFVideoFormat_MJPG, 0, kArbitraryValidVideoWidth, kArbitraryValidVideoHeight, 30);
            (*media_type)->AddRef();
    ...
    EXPECT_CALL(*(engine_.Get()), DoGetSink(MF_CAPTURE_ENGINE_SINK_TYPE_PREVIEW, _))
        .WillRepeatedly(Invoke([&mock_sink](MF_CAPTURE_ENGINE_SINK_TYPE sink_type, IMFCaptureSink** sink) {
            *sink = mock_sink.get();
            (*sink)->AddRef();
            return S_OK;
        }));
...
TEST_F(VideoCaptureDeviceMFWinTest, AllocateAndStartWithFailingInvalidRequest)
{
    ...
    EXPECT_CALL(*capture_source_, DoGetDeviceStreamCount(_)).WillRepeatedly(Return(MF_E_INVALIDREQUEST));
...
TEST_F(VideoCaptureDeviceMFWinTest, SendsOnErrorWithoutOnStartedIfDeviceIsBusy)
{
    if (ShouldSkipTest())
        return;
    ...
    scoped_refptr<MockMFMediaEvent> media_event_preview_started = new MockMFMediaEvent();
    ON_CALL(*media_event_preview_started, DoGetStatus()).WillByDefault(Return(S_OK));
    ON_CALL(*media_event_preview_started, DoGetType()).WillByDefault(Return(MEExtendedType));
    ON_CALL(*media_event_preview_started, DoGetExtendedType()).WillByDefault(Return(MF_CAPTURE_ENGINE_PREVIEW_STARTED));
...
TEST_F(VideoCaptureDeviceMFWinTest, GetPhotoStateViaVideoStream)
{
    ...
    VideoCaptureDevice::GetPhotoStateCallback get_photo_state_callback = base::BindOnce(&MockImageCaptureClient::DoOnGetPhotoState, image_capture_client_);
...
TEST_F(VideoCaptureDeviceMFWinTest, GetPhotoStateViaPhotoStream)
{
    ...
    EXPECT_EQ(base::ranges::count(state->supported_white_balance_modes, mojom::MeteringMode::CONTINUOUS), 1);
    EXPECT_EQ(base::ranges::count(state->supported_white_balance_modes, mojom::MeteringMode::MANUAL), 1);
    EXPECT_EQ(state->current_white_balance_mode, mojom::MeteringMode::CONTINUOUS);
    EXPECT_EQ(state->supported_exposure_modes.size(), 2u);
    EXPECT_EQ(base::ranges::count(state->supported_exposure_modes, mojom::MeteringMode::CONTINUOUS), 1);
    EXPECT_EQ(base::ranges::count(state->supported_exposure_modes, mojom::MeteringMode::MANUAL), 1);
    EXPECT_EQ(state->current_exposure_mode, mojom::MeteringMode::CONTINUOUS);
    EXPECT_EQ(state->supported_focus_modes.size(), 2u);
    EXPECT_EQ(base::ranges::count(state->supported_focus_modes, mojom::MeteringMode::CONTINUOUS), 1);
    EXPECT_EQ(base::ranges::count(state->supported_focus_modes, mojom::MeteringMode::MANUAL), 1);
    EXPECT_EQ(state->current_focus_mode, mojom::MeteringMode::CONTINUOUS);
    EXPECT_EQ(state->points_of_interest.size(), 0u);

    EXPECT_EQ(state->exposure_compensation->current, kVideoProcAmpCurrentBase + VideoProcAmp_Gain);
    EXPECT_EQ(state->exposure_compensation->min, kVideoProcAmpMinBase + VideoProcAmp_Gain);
    EXPECT_EQ(state->exposure_compensation->max, kVideoProcAmpMaxBase + VideoProcAmp_Gain);
    EXPECT_EQ(state->exposure_compensation->step, kVideoProcAmpStep);
    EXPECT_DOUBLE_EQ(std::log2(state->exposure_time->current / kHundredMicrosecondsInSecond), kCameraControlCurrentBase + CameraControl_Exposure);
    EXPECT_DOUBLE_EQ(std::log2(state->exposure_time->min / kHundredMicrosecondsInSecond), kCameraControlMinBase + CameraControl_Exposure);
    EXPECT_DOUBLE_EQ(std::log2(state->exposure_time->max / kHundredMicrosecondsInSecond), kCameraControlMaxBase + CameraControl_Exposure);
    EXPECT_DOUBLE_EQ(state->exposure_time->step, std::exp2(kCameraControlStep) * state->exposure_time->min - state->exposure_time->min);
    EXPECT_EQ(state->color_temperature->current, kVideoProcAmpCurrentBase + VideoProcAmp_WhiteBalance);
    EXPECT_EQ(state->color_temperature->min, kVideoProcAmpMinBase + VideoProcAmp_WhiteBalance);
    EXPECT_EQ(state->color_temperature->max, kVideoProcAmpMaxBase + VideoProcAmp_WhiteBalance);
    EXPECT_EQ(state->color_temperature->step, kVideoProcAmpStep);
    EXPECT_EQ(state->iso->min, state->iso->max);

    EXPECT_EQ(state->brightness->current, kVideoProcAmpCurrentBase + VideoProcAmp_Brightness);
    EXPECT_EQ(state->brightness->min, kVideoProcAmpMinBase + VideoProcAmp_Brightness);
    EXPECT_EQ(state->brightness->max, kVideoProcAmpMaxBase + VideoProcAmp_Brightness);
    EXPECT_EQ(state->brightness->step, kVideoProcAmpStep);
    EXPECT_EQ(state->contrast->current, kVideoProcAmpCurrentBase + VideoProcAmp_Contrast);
    ...
    EXPECT_EQ(state->saturation->current, kVideoProcAmpCurrentBase + VideoProcAmp_Saturation);
    EXPECT_EQ(state->saturation->min, kVideoProcAmpMinBase + VideoProcAmp_Saturation);
    EXPECT_EQ(state->saturation->max, kVideoProcAmpMaxBase + VideoProcAmp_Saturation);
    EXPECT_EQ(state->saturation->step, kVideoProcAmpStep);
    EXPECT_EQ(state->sharpness->current, kVideoProcAmpCurrentBase + VideoProcAmp_Sharpness);
    EXPECT_EQ(state->sharpness->min, kVideoProcAmpMinBase + VideoProcAmp_Sharpness);
    EXPECT_EQ(state->sharpness->max, kVideoProcAmpMaxBase + VideoProcAmp_Sharpness);
    EXPECT_EQ(state->sharpness->step, kVideoProcAmpStep);

    EXPECT_EQ(state->focus_distance->current, kCameraControlCurrentBase + CameraControl_Focus);
    EXPECT_EQ(state->focus_distance->min, kCameraControlMinBase + CameraControl_Focus);
    EXPECT_EQ(state->focus_distance->max, kCameraControlMaxBase + CameraControl_Focus);
    EXPECT_EQ(state->focus_distance->step, kCameraControlStep);

    EXPECT_DOUBLE_EQ(state->pan->current / kArcSecondsInDegree, kCameraControlCurrentBase + CameraControl_Pan);
    EXPECT_DOUBLE_EQ(state->pan->min / kArcSecondsInDegree, kCameraControlMinBase + CameraControl_Pan);
    EXPECT_DOUBLE_EQ(state->pan->max / kArcSecondsInDegree, kCameraControlMaxBase + CameraControl_Pan);
    EXPECT_DOUBLE_EQ(state->pan->step / kArcSecondsInDegree, kCameraControlStep);
    EXPECT_DOUBLE_EQ(state->tilt->current / kArcSecondsInDegree, kCameraControlCurrentBase + CameraControl_Tilt);
    EXPECT_DOUBLE_EQ(state->tilt->min / kArcSecondsInDegree, kCameraControlMinBase + CameraControl_Tilt);
    EXPECT_DOUBLE_EQ(state->tilt->max / kArcSecondsInDegree, kCameraControlMaxBase + CameraControl_Tilt);
    EXPECT_DOUBLE_EQ(state->tilt->step / kArcSecondsInDegree, kCameraControlStep);
    EXPECT_EQ(state->zoom->current, kCameraControlCurrentBase + CameraControl_Zoom);
    ...
    EXPECT_EQ(base::ranges::count(*state->supported_background_blur_modes, mojom::BackgroundBlurMode::OFF), 1);
    EXPECT_EQ(base::ranges::count(*state->supported_background_blur_modes, mojom::BackgroundBlurMode::BLUR), 1);
    ...
    EXPECT_EQ(base::ranges::count(*state->supported_eye_gaze_correction_modes, mojom::EyeGazeCorrectionMode::OFF), 1);
    EXPECT_EQ(base::ranges::count(*state->supported_eye_gaze_correction_modes, mojom::EyeGazeCorrectionMode::ON), 1);
    EXPECT_EQ(base::ranges::count(*state->supported_eye_gaze_correction_modes, mojom::EyeGazeCorrectionMode::STARE), 1);
    EXPECT_EQ(state->current_eye_gaze_correction_mode, mojom::EyeGazeCorrectionMode::OFF);

    ASSERT_TRUE(state->supported_face_framing_modes);
    EXPECT_EQ(2u, state->supported_face_framing_modes->size());
    EXPECT_EQ(1, base::ranges::count(*state->supported_face_framing_modes, mojom::MeteringMode::CONTINUOUS));
    EXPECT_EQ(1, base::ranges::count(*state->supported_face_framing_modes, mojom::MeteringMode::NONE));
...
TEST_F(VideoCaptureDeviceMFWinTest, TakePhotoViaPhotoStream)
{
    ...
    capture_preview_sink_->sample_callback->OnSample(nullptr);
    task_environment_.RunUntilIdle();
    VideoCaptureDevice::TakePhotoCallback take_photo_callback = base::BindOnce(&MockImageCaptureClient::DoOnPhotoTaken, image_capture_client_);
    device_->TakePhoto(std::move(take_photo_callback));
}

class DepthCameraDeviceMFWinTest : public VideoCaptureDeviceMFWinTest, public testing::WithParamInterface<DepthDeviceParams> { };

const DepthDeviceParams kDepthCamerasParams[]
    = { { kMediaSubTypeY16, false, false }, { kMediaSubTypeZ16, false, true }, { kMediaSubTypeINVZ, true, false }, { MFVideoFormat_D16, true, true } };

INSTANTIATE_TEST_SUITE_P(DepthCameraDeviceMFWinTests, DepthCameraDeviceMFWinTest, testing::ValuesIn(kDepthCamerasParams));
...
TEST_P(DepthCameraDeviceMFWinTest, AllocateAndStartDepthCamera)
{
    ...
    if (!params.additional_i420_video_stream && !params.additional_i420_formats_in_depth_stream) {
    ...
    EXPECT_CALL(*(capture_source_.get()), DoSetCurrentDeviceMediaType(0, _)).WillOnce(Invoke([params](DWORD stream_index, IMFMediaType* media_type) {
        GUID source_video_media_subtype;
        media_type->GetGUID(MF_MT_SUBTYPE, &source_video_media_subtype);
        EXPECT_EQ(source_video_media_subtype, params.depth_video_stream_subtype);
        return S_OK;
    }));

    EXPECT_CALL(*(capture_preview_sink_.get()), DoAddStream(0, _, _, _))
        .WillOnce(Invoke([params](DWORD stream_index, IMFMediaType* media_type, IMFAttributes* attributes, DWORD* sink_stream_index) {
...
protected:
    void SetUp() override
    {
        ...
        HRESULT hr = MFCreateDXGIDeviceManager(&d3d_device_reset_token, &mf_dxgi_device_manager);
        ASSERT_EQ(hr, S_OK);
        dxgi_device_manager_ = new MockDXGIDeviceManager(std::move(mf_dxgi_device_manager), d3d_device_reset_token);
...
TEST_F(VideoCaptureDeviceMFWinTestWithDXGI, EnsureNV12SinkSubtype)
{
    ...
    EXPECT_CALL(*(capture_source_.get()), DoSetCurrentDeviceMediaType(0, _)).WillOnce(Invoke([expected_subtype](DWORD stream_index, IMFMediaType* media_type) {
        GUID source_video_media_subtype;
        media_type->GetGUID(MF_MT_SUBTYPE, &source_video_media_subtype);
    ...
    EXPECT_CALL(*(capture_preview_sink_.get()), DoAddStream(0, _, _, _))
        .WillOnce(Invoke([expected_subtype](DWORD stream_index, IMFMediaType* media_type, IMFAttributes* attributes, DWORD* sink_stream_index) {
            GUID sink_video_media_subtype;
            media_type->GetGUID(MF_MT_SUBTYPE, &sink_video_media_subtype);
...
TEST_F(VideoCaptureDeviceMFWinTestWithDXGI, DeliverGMBCaptureBuffers)
{
    ...
    EXPECT_CALL(*client_, ReserveOutputBuffer)
        .WillOnce(Invoke([expected_size](const gfx::Size& size, VideoPixelFormat format, int feedback_id, VideoCaptureDevice::Client::Buffer* capture_buffer,
            ...
            capture_buffer->handle_provider = std::make_unique<MockCaptureHandleProvider>();
            return VideoCaptureDevice::Client::ReserveResult::kSucceeded;
        }));

    Microsoft::WRL::ComPtr<MockD3D11Device> mock_device(new MockD3D11Device());

    EXPECT_CALL(*dxgi_device_manager_.get(), GetDevice).WillOnce(Return(mock_device));
    ...
    Microsoft::WRL::ComPtr<MockD3D11Texture2D> mock_source_texture(new MockD3D11Texture2D(mock_desc, mock_device.Get()));
    EXPECT_TRUE(SUCCEEDED(mock_source_texture.CopyTo(IID_PPV_ARGS(&mock_source_texture_2d))));
    ...
    Microsoft::WRL::ComPtr<MockD3D11Texture2D> mock_target_texture(new MockD3D11Texture2D(mock_desc, mock_device.Get()));
    EXPECT_TRUE(SUCCEEDED(mock_target_texture.CopyTo(IID_PPV_ARGS(&mock_target_texture_2d))));
    ...
    EXPECT_CALL(*mock_device.Get(), DoOpenSharedResource1)
        .WillOnce(Invoke([&mock_target_texture_2d](HANDLE resource, REFIID returned_interface, void** resource_out) {
    ...
    ID3D11Resource* expected_source = static_cast<ID3D11Resource*>(mock_source_texture_2d.Get());
    ID3D11Resource* expected_target = static_cast<ID3D11Resource*>(mock_target_texture_2d.Get());
    EXPECT_CALL(*mock_device->mock_immediate_context_.Get(), OnCopySubresourceRegion(expected_target, _, _, _, _, expected_source, _, _)).Times(1);
    ...
    EXPECT_CALL(*client_, OnIncomingCapturedBufferExt)
        .WillOnce(Invoke([](VideoCaptureDevice::Client::Buffer buffer, const VideoCaptureFormat&, const gfx::ColorSpace&, base::TimeTicks, base::TimeDelta,
                             std::optional<base::TimeTicks>, gfx::Rect, const VideoFrameMetadata&) {
            gfx::GpuMemoryBufferHandle gmb_handle = buffer.handle_provider->GetGpuMemoryBufferHandle();
            EXPECT_EQ(gmb_handle.type, gfx::GpuMemoryBufferType::DXGI_SHARED_HANDLE);
        }));
    ...
    EXPECT_TRUE(SUCCEEDED(MFCreateDXGISurfaceBuffer(__uuidof(ID3D11Texture2D), mock_source_texture_2d.Get(), 0, FALSE, &dxgi_buffer)));
