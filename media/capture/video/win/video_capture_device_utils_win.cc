...
bool IsPortraitDevice(DWORD display_height, DWORD display_width, DWORD display_orientation)
{
...
int GetCameraRotation(VideoFacingMode facing)
{
    ...
    if (::EnumDisplaySettings(internal_display_device.DeviceName, ENUM_CURRENT_SETTINGS, &mode)) {
        int camera_offset = 0; // Measured in degrees, clockwise.
        bool portrait_device = IsPortraitDevice(mode.dmPelsHeight, mode.dmPelsWidth, mode.dmDisplayOrientation);
        switch (mode.dmDisplayOrientation) {
        case DMDO_DEFAULT:
            if (portrait_device && facing == VideoFacingMode::MEDIA_VIDEO_FACING_ENVIRONMENT) {
...
bool IsAutoRotationEnabled()
{
    ...
    static const auto get_rotation_state = reinterpret_cast<GetAutoRotationState>(base::win::GetUser32FunctionPointer("GetAutoRotationState"));
...
{
    return facing == MEDIA_VIDEO_FACING_USER || facing == MEDIA_VIDEO_FACING_ENVIRONMENT;
}
...
bool HasActiveInternalDisplayDevice(DISPLAY_DEVICE* internal_display_device)
{
    ...
    for (int device_index = 0;; ++device_index) {
        BOOL enum_result = ::EnumDisplayDevices(NULL, device_index, &display_device, 0);
        if (!enum_result)
            break;
...
HRESULT CheckPathInfoForInternal(const PCWSTR device_name)
{
    ...
    do {
        ...
        hr = HRESULT_FROM_WIN32(::GetDisplayConfigBufferSizes(QDC_ONLY_ACTIVE_PATHS, &path_info_array_size, &mode_info_array_size));
        if (FAILED(hr)) {
            break;
        }

        path_info_array = new (std::nothrow) DISPLAYCONFIG_PATH_INFO[path_info_array_size];
        if (path_info_array == nullptr) {
            hr = E_OUTOFMEMORY;
            break;
        }

        mode_info_array = new (std::nothrow) DISPLAYCONFIG_MODE_INFO[mode_info_array_size];
        ...
        hr = HRESULT_FROM_WIN32(
            ::QueryDisplayConfig(QDC_ONLY_ACTIVE_PATHS, &path_info_array_size, path_info_array, &mode_info_array_size, mode_info_array, nullptr));
    ...
    if (SUCCEEDED(hr)) {
        ...
        // is found.
        for (UINT32 path_index = 0; path_index < path_info_array_size; ++path_index) {
            DISPLAYCONFIG_SOURCE_DEVICE_NAME source_name = {};
            source_name.header.type = DISPLAYCONFIG_DEVICE_INFO_GET_SOURCE_NAME;
            source_name.header.size = sizeof(source_name);
            source_name.header.adapterId = path_info_array[path_index].sourceInfo.adapterId;
            ...
            if (SUCCEEDED(hr)) {
                if (wcscmp(device_name, source_name.viewGdiDeviceName) == 0 && IsInternalVideoOutput(path_info_array[path_index].targetInfo.outputTechnology)) {
...
bool IsInternalVideoOutput(const DISPLAYCONFIG_VIDEO_OUTPUT_TECHNOLOGY video_output_tech_type)
{
