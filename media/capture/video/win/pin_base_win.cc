...
// Implement IEnumPins.
class TypeEnumerator final : public IEnumMediaTypes, public base::RefCountedThreadSafe<TypeEnumerator> {
public:
    ...
    IFACEMETHODIMP Next(ULONG count, AM_MEDIA_TYPE** types, ULONG* fetched) override
    {
...
// connection.
HRESULT PinBase::ReceiveConnection(IPin* connector, const AM_MEDIA_TYPE* media_type)
{
...
HRESULT PinBase::NewSegment(REFERENCE_TIME start, REFERENCE_TIME stop, double rate)
{
    NOTREACHED();
}
...
HRESULT PinBase::ReceiveMultiple(IMediaSample** samples, long sample_count, long* processed)
{
    DCHECK(samples);
