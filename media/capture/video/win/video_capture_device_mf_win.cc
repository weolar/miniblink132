...
BASE_FEATURE(kMediaFoundationVideoCaptureForwardSampleTimestamps, "MediaFoundationVideoCaptureForwardSampleTimestamps", base::FEATURE_DISABLED_BY_DEFAULT);

ULONGLONG CaptureModeToExtendedPlatformFlags(mojom::EyeGazeCorrectionMode mode)
{
    switch (mode) {
    ...
    case mojom::EyeGazeCorrectionMode::STARE:
        return KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_ON | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_STARE;
    }
    NOTREACHED();
}

mojom::EyeGazeCorrectionMode ExtendedPlatformFlagsToCaptureMode(ULONGLONG flags, mojom::EyeGazeCorrectionMode default_mode)
{
    switch (
        flags & (KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_OFF | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_ON | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_STARE)) {
    ...
    case (KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_ON | KSCAMERA_EXTENDEDPROP_EYEGAZECORRECTION_STARE):
...
std::vector<mojom::EyeGazeCorrectionMode> ExtendedPlatformFlagsToCaptureModes(ULONGLONG flags)
{
    std::vector<mojom::EyeGazeCorrectionMode> modes = { mojom::EyeGazeCorrectionMode::OFF };
...
#define DLOG_IF_FAILED_WITH_HRESULT(message, hr)                                                                                                               \
    {                                                                                                                                                          \
        DLOG_IF(ERROR, FAILED(hr)) << (message) << ": " << logging::SystemErrorCodeToString(hr);                                                               \
    }
#else
#define DLOG_IF_FAILED_WITH_HRESULT(message, hr)                                                                                                               \
    {                                                                                                                                                          \
...
std::optional<base::TimeTicks> MaybeForwardCaptureBeginTime(const base::TimeTicks capture_begin_time)
{
    if (!base::FeatureList::IsEnabled(kMediaFoundationVideoCaptureForwardSampleTimestamps)) {
        return std::nullopt;
    }
...
constexpr base::TimeDelta kMaxFeedbackPremappingEffectDuration = base::Milliseconds(500);
...
constexpr GUID DXGIHandlePrivateDataGUID = { 0x79bfe1ab, 0xce47, 0x4c3d, { 0xbd, 0xb2, 0x6, 0xe6, 0xb8, 0x86, 0x36, 0x8c } };

// How many times we try to restart D3D11 path.
constexpr int kMaxD3DRestarts = 2;

class MFPhotoCallback final : public base::RefCountedThreadSafe<MFPhotoCallback>, public IMFCaptureEngineOnSampleCallback {
public:
    MFPhotoCallback(VideoCaptureDevice::TakePhotoCallback callback, VideoCaptureFormat format)
...
public:
    ...
    {
        BOOL is_contiguous;
        return pitch_ > 0 && SUCCEEDED(buffer_2d_->IsContiguousFormat(&is_contiguous)) && is_contiguous
...
scoped_refptr<IMFCaptureEngineOnSampleCallback> CreateMFPhotoCallback(VideoCaptureDevice::TakePhotoCallback callback, VideoCaptureFormat format)
{
    return scoped_refptr<IMFCaptureEngineOnSampleCallback>(new MFPhotoCallback(std::move(callback), format));
}

void LogError(const Location& from_here, HRESULT hr)
{
    LOG(ERROR) << from_here.ToString() << " hr = " << logging::SystemErrorCodeToString(hr);
}
...
VideoPixelFormat MfSubTypeToSourcePixelFormat(const GUID& mf_source_media_subtype)
{
    static const PixelFormatMap kPixelFormatMap[] = {

        { MFVideoFormat_I420, PIXEL_FORMAT_I420 }, { MFVideoFormat_YUY2, PIXEL_FORMAT_YUY2 }, { MFVideoFormat_UYVY, PIXEL_FORMAT_UYVY },
        { MFVideoFormat_RGB24, PIXEL_FORMAT_RGB24 }, { MFVideoFormat_RGB32, PIXEL_FORMAT_XRGB }, { MFVideoFormat_ARGB32, PIXEL_FORMAT_ARGB },
        { MFVideoFormat_MJPG, PIXEL_FORMAT_MJPEG }, { MFVideoFormat_NV12, PIXEL_FORMAT_NV12 }, { MFVideoFormat_YV12, PIXEL_FORMAT_YV12 },
...
bool GetFormatFromSourceMediaType(
    IMFMediaType* source_media_type, bool photo, bool use_hardware_format, VideoCaptureFormat* format, VideoPixelFormat* source_pixel_format)
{
    GUID major_type_guid;
    if (FAILED(source_media_type->GetGUID(MF_MT_MAJOR_TYPE, &major_type_guid))
        || (major_type_guid != MFMediaType_Image && (photo || !GetFrameRateFromMediaType(source_media_type, &format->frame_rate)))) {
        return false;
    }

    GUID sub_type_guid;
    if (FAILED(source_media_type->GetGUID(MF_MT_SUBTYPE, &sub_type_guid)) || !GetFrameSizeFromMediaType(source_media_type, &format->frame_size)
        || !VideoCaptureDeviceMFWin::GetPixelFormatFromMFSourceMediaSubtype(sub_type_guid, use_hardware_format, &format->pixel_format)) {
        return false;
    }
...
bool GetMediaFormatConfigurationFromMFSourceMediaSubtype(
    const GUID& mf_source_media_subtype, bool use_hardware_format, MediaFormatConfiguration* media_format_configuration)
{
    static const MediaFormatConfiguration kMediaFormatConfigurationMap[] = { // IMFCaptureEngine inevitably performs the video frame decoding itself.
        ...
        { false, MFVideoFormat_I420, MFVideoFormat_I420, PIXEL_FORMAT_I420 }, { false, MFVideoFormat_YUY2, MFVideoFormat_I420, PIXEL_FORMAT_I420 },
        { false, MFVideoFormat_UYVY, MFVideoFormat_I420, PIXEL_FORMAT_I420 }, { false, MFVideoFormat_RGB24, MFVideoFormat_I420, PIXEL_FORMAT_I420 },
        { false, MFVideoFormat_RGB32, MFVideoFormat_I420, PIXEL_FORMAT_I420 }, { false, MFVideoFormat_ARGB32, MFVideoFormat_I420, PIXEL_FORMAT_I420 },
        { false, MFVideoFormat_MJPG, MFVideoFormat_I420, PIXEL_FORMAT_I420 }, { false, MFVideoFormat_NV12, MFVideoFormat_I420, PIXEL_FORMAT_I420 },
        ...
        { false, kMediaSubTypeY16, kMediaSubTypeY16, PIXEL_FORMAT_Y16 }, { false, kMediaSubTypeZ16, kMediaSubTypeZ16, PIXEL_FORMAT_Y16 },
        { false, kMediaSubTypeINVZ, kMediaSubTypeINVZ, PIXEL_FORMAT_Y16 }, { false, MFVideoFormat_D16, MFVideoFormat_D16, PIXEL_FORMAT_Y16 },

        // Photo type
        { false, GUID_ContainerFormatJpeg, GUID_ContainerFormatJpeg, PIXEL_FORMAT_MJPEG },

        // For hardware path we always convert to NV12, since it's the only
        // supported by GMBs format.
        { true, MFVideoFormat_I420, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 }, { true, MFVideoFormat_YUY2, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 },
        { true, MFVideoFormat_UYVY, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 }, { true, MFVideoFormat_RGB24, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 },
        { true, MFVideoFormat_RGB32, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 }, { true, MFVideoFormat_ARGB32, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 },
        { true, MFVideoFormat_MJPG, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 }, { true, MFVideoFormat_NV12, MFVideoFormat_NV12, PIXEL_FORMAT_NV12 },
        ...
        { true, kMediaSubTypeY16, kMediaSubTypeY16, PIXEL_FORMAT_Y16 }, { true, kMediaSubTypeZ16, kMediaSubTypeZ16, PIXEL_FORMAT_Y16 },
        { true, kMediaSubTypeINVZ, kMediaSubTypeINVZ, PIXEL_FORMAT_Y16 }, { true, MFVideoFormat_D16, MFVideoFormat_D16, PIXEL_FORMAT_Y16 },

        // Photo type
        { true, GUID_ContainerFormatJpeg, GUID_ContainerFormatJpeg, PIXEL_FORMAT_MJPEG }
    };
    ...
    for (const auto& kMediaFormatConfiguration : kMediaFormatConfigurationMap) {
        if (kMediaFormatConfiguration.is_hardware_format == use_hardware_format
            && kMediaFormatConfiguration.mf_source_media_subtype == mf_source_media_subtype) {
...
HRESULT GetMFSinkMediaSubtype(IMFMediaType* source_media_type, bool use_hardware_format, GUID* mf_sink_media_subtype, bool* passthrough)
{
    ...
    if (!GetMediaFormatConfigurationFromMFSourceMediaSubtype(source_subtype, use_hardware_format, &media_format_configuration))
        return E_FAIL;
    *mf_sink_media_subtype = media_format_configuration.mf_sink_media_subtype;
    *passthrough = (media_format_configuration.mf_sink_media_subtype == source_subtype);
    return S_OK;
}

HRESULT ConvertToPhotoSinkMediaType(IMFMediaType* source_media_type, IMFMediaType* destination_media_type)
{
    HRESULT hr = destination_media_type->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Image);
    if (FAILED(hr))
        return hr;

    bool passthrough = false;
    GUID mf_sink_media_subtype;
    hr = GetMFSinkMediaSubtype(source_media_type, /*use_hardware_format=*/false, &mf_sink_media_subtype, &passthrough);
    if (FAILED(hr))
        return hr;
    ...
    return CopyAttribute(source_media_type, destination_media_type, MF_MT_FRAME_SIZE);
}

HRESULT ConvertToVideoSinkMediaType(IMFMediaType* source_media_type, bool use_hardware_format, IMFMediaType* sink_media_type)
{
    ...
    bool passthrough = false;
    GUID mf_sink_media_subtype;
    hr = GetMFSinkMediaSubtype(source_media_type, use_hardware_format, &mf_sink_media_subtype, &passthrough);
    if (FAILED(hr))
        return hr;
    ...
    if (base::FeatureList::IsEnabled(media::kWebRTCColorAccuracy)) {
        hr = CopyAttribute(source_media_type, sink_media_type, MF_MT_VIDEO_NOMINAL_RANGE);
    } else {
        hr = sink_media_type->SetUINT32(MF_MT_VIDEO_NOMINAL_RANGE, MFNominalRange_16_235);
    }
    if (FAILED(hr))
        return hr;
...
const CapabilityWin& GetBestMatchedPhotoCapability(ComPtr<IMFMediaType> current_media_type, gfx::Size requested_size, const CapabilityList& capabilities)
{
    gfx::Size current_size;
    GetFrameSizeFromMediaType(current_media_type.Get(), &current_size);

    int requested_height = requested_size.height() > 0 ? requested_size.height() : current_size.height();
    int requested_width = requested_size.width() > 0 ? requested_size.width() : current_size.width();
    ...
    for (const CapabilityWin& capability : capabilities) {
        ...
        if (std::abs(height - requested_height) <= std::abs(height - best_height) && std::abs(width - requested_width) <= std::abs(width - best_width)) {
...
bool GetCameraControlSupport(ComPtr<IAMCameraControl> camera_control, CameraControlProperty control_property)
{
    long min, max, step, default_value, flags;
    HRESULT hr = camera_control->GetRange(control_property, &min, &max, &step, &default_value, &flags);
...
mojom::RangePtr RetrieveControlRangeAndCurrent(ComPtr<ControlInterface>& control_interface, ControlProperty control_property,
    std::vector<mojom::MeteringMode>* supported_modes = nullptr, mojom::MeteringMode* current_mode = nullptr,
    double (*value_converter)(long) = PlatformToCaptureValue, double (*step_converter)(long, double, double) = PlatformToCaptureStep)
{
    return media::RetrieveControlRangeAndCurrent(
        [&control_interface, control_property](auto... args) { return control_interface->GetRange(control_property, args...); },
        [&control_interface, control_property](auto... args) { return control_interface->Get(control_property, args...); }, supported_modes, current_mode,
        value_converter, step_converter);
}
...
void GetTextureInfo(ID3D11Texture2D* texture, gfx::Size& size, VideoPixelFormat& format, bool& is_cross_process_shared_texture)
{
    ...
    is_cross_process_shared_texture = (desc.MiscFlags & D3D11_RESOURCE_MISC_SHARED) && (desc.MiscFlags & D3D11_RESOURCE_MISC_SHARED_NTHANDLE)
        && !(desc.MiscFlags & D3D11_RESOURCE_MISC_RESTRICT_SHARED_RESOURCE) && !(desc.MiscFlags & D3D11_RESOURCE_MISC_RESTRICT_SHARED_RESOURCE_DRIVER);

    // Log this in an UMA histogram to determine what proportion of frames might
    // actually benefit from zero-copy.
    base::UmaHistogramBoolean("Media.VideoCapture.Win.Device.IsSharedTexture", is_cross_process_shared_texture);
}

HRESULT CopyTextureToGpuMemoryBuffer(ID3D11Texture2D* texture, HANDLE dxgi_handle)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "CopyTextureToGpuMemoryBuffer");
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to get ID3D11Device1: " << logging::SystemErrorCodeToString(hr);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to open shared camera target texture: " << logging::SystemErrorCodeToString(hr);
        return hr;
    }
    ...
    // WAIT_ABANDONED.
    if (hr != S_OK) {
        DLOG(ERROR) << "Failed to acquire the mutex:" << logging::SystemErrorCodeToString(hr);
        return E_FAIL;
    }
    device_context->CopySubresourceRegion(target_texture.Get(), 0, 0, 0, 0, texture, 0, nullptr);
...
class DXGIHandlePrivateData : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::ClassicCom>, IUnknown> {
public:
    ...
    gfx::DXGIHandleToken GetDXGIToken()
    {
        return dxgi_token_;
    }
...
class VideoCaptureDeviceMFWin::MFVideoCallback final : public base::RefCountedThreadSafe<MFVideoCallback>,
                                                       public IMFCameraControlNotify,
                                                       public IMFCaptureEngineOnSampleCallback,
                                                       public IMFCaptureEngineOnEventCallback {
public:
    MFVideoCallback(VideoCaptureDeviceMFWin* observer)
        : observer_(observer)
    {
    }
    ...
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "MFVideoCallback::OnEvent");
        base::AutoLock lock(lock_);
        if (!observer_) {
            return S_OK;
        }
    ...
    IFACEMETHODIMP OnSample(IMFSample* sample) override
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "MFVideoCallback::OnSample");
        base::AutoLock lock(lock_);
        if (!observer_) {
            return S_OK;
        }
        if (!sample) {
            observer_->OnFrameDropped(VideoCaptureFrameDropReason::kWinMediaFoundationReceivedSampleIsNull);
            return S_OK;
        }

        base::TimeTicks reference_time(base::TimeTicks::Now());
        base::TimeTicks mf_time_now = base::TimeTicks() + base::Microseconds(MFGetSystemTime() / 10);
        ...
        HRESULT hr = sample->GetUINT64(MFSampleExtension_DeviceReferenceSystemTime, &raw_capture_begin_time);
        if (FAILED(hr)) {
            hr = sample->GetUINT64(MFSampleExtension_DeviceReferenceSystemTime, &raw_capture_begin_time);
        }
        if (FAILED(hr)) {
        ...
        base::TimeTicks capture_begin_time = base::TimeTicks() + base::Microseconds(raw_capture_begin_time / 10) + mf_time_offset;

        base::UmaHistogramCustomTimes("Media.VideoCapture.Win.Device.CaptureBeginTime.MFTimeOffset", mf_time_offset + base::Milliseconds(150),
            base::Milliseconds(0), base::Milliseconds(299), 50);
        if (last_capture_begin_time_ > base::TimeTicks()) {
            base::UmaHistogramCustomTimes("Media.VideoCapture.Win.Device.CaptureBeginTime.Interval",
                capture_begin_time - last_capture_begin_time_ + base::Milliseconds(150), base::Milliseconds(0), base::Milliseconds(299), 50);
        }
        ...
        for (DWORD i = 0; i < count; ++i) {
            ...
            sample->GetBufferByIndex(i, &buffer);
            if (buffer) {
                observer_->OnIncomingCapturedData(buffer, reference_time, timestamp, capture_begin_time);
            } else {
                observer_->OnFrameDropped(VideoCaptureFrameDropReason::kWinMediaFoundationGetBufferByIndexReturnedNull);
            }
        }
        return S_OK;
    }
...
class VideoCaptureDeviceMFWin::MFActivitiesReportCallback final : public base::RefCountedThreadSafe<MFActivitiesReportCallback>,
                                                                  public IMFSensorActivitiesReportCallback {
public:
    MFActivitiesReportCallback(
        base::WeakPtr<VideoCaptureDeviceMFWin> observer, scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner, std::string device_id)
    ...
    OnActivitiesReport(IMFSensorActivitiesReport* sensorActivitiesReport) override
    {
        bool in_use = false;

        ComPtr<IMFSensorActivityReport> activity_report;
        HRESULT hr = sensorActivitiesReport->GetActivityReportByDeviceName(base::SysUTF8ToWide(device_id_).c_str(), &activity_report);
        if (FAILED(hr)) {
            return S_OK;
        }
        ...
        main_thread_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::OnCameraInUseReport, observer_, in_use, /*is_default_action=*/false));
        return S_OK;
    }
...
bool VideoCaptureDeviceMFWin::GetPixelFormatFromMFSourceMediaSubtype(
    const GUID& mf_source_media_subtype, bool use_hardware_format, VideoPixelFormat* pixel_format)
{
    MediaFormatConfiguration media_format_configuration;
    if (!GetMediaFormatConfigurationFromMFSourceMediaSubtype(mf_source_media_subtype, use_hardware_format, &media_format_configuration))
        return false;
...
// static
VideoCaptureControlSupport VideoCaptureDeviceMFWin::GetControlSupport(ComPtr<IMFMediaSource> source)
{
    ...
    if (camera_control && video_control) {
        control_support.pan = GetCameraControlSupport(camera_control, CameraControl_Pan);
        control_support.tilt = GetCameraControlSupport(camera_control, CameraControl_Tilt);
        control_support.zoom = GetCameraControlSupport(camera_control, CameraControl_Zoom);
    }
...
bool VideoCaptureDeviceMFWin::CreateMFCameraControlMonitor()
{
    ...
    using MFCreateCameraControlMonitorType = decltype(&MFCreateCameraControlMonitor);
    static const MFCreateCameraControlMonitorType create_camera_control_monitor
        = reinterpret_cast<MFCreateCameraControlMonitorType>(GetProcAddress(module, "MFCreateCameraControlMonitor"));
    ...
    HRESULT hr = create_camera_control_monitor(base::SysUTF8ToWide(device_descriptor_.device_id).c_str(), video_callback_.get(), &camera_control_monitor);
    if (!camera_control_monitor) {
        LOG(ERROR) << "Failed to create IMFCameraControlMonitor: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    hr = camera_control_monitor->AddControlSubscription(KSPROPERTYSETID_ANYCAMERACONTROL, 0);
    if (FAILED(hr)) {
        LOG(ERROR) << "Failed to add IMFCameraControlMonitor control subscription: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    hr = camera_control_monitor->Start();
    if (FAILED(hr)) {
        LOG(ERROR) << "Failed to start IMFCameraControlMonitor: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
...
HRESULT VideoCaptureDeviceMFWin::ExecuteHresultCallbackWithRetries(
    base::RepeatingCallback<HRESULT()> callback, MediaFoundationFunctionRequiringRetry which_function)
{
    ...
    LogNumberOfRetriesNeededToWorkAroundMFInvalidRequest(which_function, retry_count);

    return hr;
}

HRESULT VideoCaptureDeviceMFWin::GetDeviceStreamCount(IMFCaptureSource* source, DWORD* count)
{
    ...
    return ExecuteHresultCallbackWithRetries(
        base::BindRepeating([](IMFCaptureSource* source, DWORD* count) { return source->GetDeviceStreamCount(count); }, base::Unretained(source), count),
        MediaFoundationFunctionRequiringRetry::kGetDeviceStreamCount);
}

HRESULT VideoCaptureDeviceMFWin::GetDeviceStreamCategory(IMFCaptureSource* source, DWORD stream_index, MF_CAPTURE_ENGINE_STREAM_CATEGORY* stream_category)
{
    ...
    return ExecuteHresultCallbackWithRetries(
        base::BindRepeating([](IMFCaptureSource* source, DWORD stream_index,
                                MF_CAPTURE_ENGINE_STREAM_CATEGORY* stream_category) { return source->GetDeviceStreamCategory(stream_index, stream_category); },
            base::Unretained(source), stream_index, stream_category),
        MediaFoundationFunctionRequiringRetry::kGetDeviceStreamCategory);
}

HRESULT VideoCaptureDeviceMFWin::GetAvailableDeviceMediaType(IMFCaptureSource* source, DWORD stream_index, DWORD media_type_index, IMFMediaType** type)
{
    // Rarely, for some unknown reason, GetAvailableDeviceMediaType returns an
    // undocumented MF_E_INVALIDREQUEST. Retrying solves the issue.
    return ExecuteHresultCallbackWithRetries(
        base::BindRepeating([](IMFCaptureSource* source, DWORD stream_index, DWORD media_type_index,
                                IMFMediaType** type) { return source->GetAvailableDeviceMediaType(stream_index, media_type_index, type); },
...
HRESULT VideoCaptureDeviceMFWin::FillCapabilities(IMFCaptureSource* source, bool photo, CapabilityList* capabilities)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::FillCapabilities");
    ...
    for (DWORD stream_index = 0; stream_index < stream_count; stream_index++) {
        ...
        if ((photo && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_PHOTO_INDEPENDENT)
            || (!photo && stream_category != MF_CAPTURE_ENGINE_STREAM_CATEGORY_VIDEO_PREVIEW
        ...
        while (SUCCEEDED(hr = GetAvailableDeviceMediaType(source, stream_index, media_type_index, &type))) {
            VideoCaptureFormat format;
            VideoPixelFormat source_pixel_format;
            if (GetFormatFromSourceMediaType(type.Get(), photo,
                    /*use_hardware_format=*/!photo && static_cast<bool>(dxgi_device_manager_), &format, &source_pixel_format)) {
                uint32_t nominal_range = 0;
                auto attribute_hr = type->GetUINT32(MF_MT_VIDEO_NOMINAL_RANGE, &nominal_range);
                ...
                bool maybe_fake = SUCCEEDED(attribute_hr) && nominal_range == MFNominalRange_0_255 && source_pixel_format == media::PIXEL_FORMAT_NV12;
                capabilities->emplace_back(media_type_index, format, stream_index, source_pixel_format, maybe_fake);
            }
...
HRESULT VideoCaptureDeviceMFWin::SetAndCommitExtendedCameraControlFlags(KSPROPERTY_CAMERACONTROL_EXTENDED_PROPERTY property_id, ULONGLONG flags)
{
    DCHECK(extended_camera_controller_);
    ComPtr<IMFExtendedCameraControl> extended_camera_control;
    HRESULT hr = extended_camera_controller_->GetExtendedCameraControl(MF_CAPTURE_ENGINE_MEDIASOURCE, property_id, &extended_camera_control);
    DLOG_IF_FAILED_WITH_HRESULT("Failed to retrieve IMFExtendedCameraControl", hr);
    if (FAILED(hr)) {
        return hr;
    }
    if (extended_camera_control->GetFlags() != flags) {
        hr = extended_camera_control->SetFlags(flags);
        DLOG_IF_FAILED_WITH_HRESULT("Failed to set extended camera control flags", hr);
        if (FAILED(hr)) {
            return hr;
        }
        hr = extended_camera_control->CommitSettings();
        DLOG_IF_FAILED_WITH_HRESULT("Failed to commit extended camera control settings", hr);
        if (FAILED(hr)) {
            return hr;
        }
    }
...
HRESULT VideoCaptureDeviceMFWin::SetCameraControlProperty(CameraControlProperty property, long value, long flags)
{
...
HRESULT VideoCaptureDeviceMFWin::SetVideoControlProperty(VideoProcAmpProperty property, long value, long flags)
{
...
VideoCaptureDeviceMFWin::VideoCaptureDeviceMFWin(const VideoCaptureDeviceDescriptor& device_descriptor, ComPtr<IMFMediaSource> source,
    scoped_refptr<DXGIDeviceManager> dxgi_device_manager, scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner)
    : VideoCaptureDeviceMFWin(device_descriptor, source, std::move(dxgi_device_manager), nullptr, std::move(main_thread_task_runner))
{
}

VideoCaptureDeviceMFWin::VideoCaptureDeviceMFWin(const VideoCaptureDeviceDescriptor& device_descriptor, ComPtr<IMFMediaSource> source,
    scoped_refptr<DXGIDeviceManager> dxgi_device_manager, ComPtr<IMFCaptureEngine> engine, scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner)
    ...
    , capture_initialize_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    ,
    // We never want to reset |capture_error_|.
    capture_error_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
...
VideoCaptureDeviceMFWin::~VideoCaptureDeviceMFWin()
{
    ...
    if (engine_) {
        main_thread_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&DestroyCaptureEngine, std::move(engine_)), base::Seconds(5));
    }
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::Init");
    ...
    if (get_service) {
        hr = get_service->GetService(GUID_NULL, IID_PPV_ARGS(&extended_camera_controller_));
        DLOG_IF_FAILED_WITH_HRESULT("Failed to retrieve IMFExtendedCameraController", hr);
    }
...
void VideoCaptureDeviceMFWin::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::AllocateAndStart");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    params_ = params;
    ...
    if (!engine_) {
        OnError(VideoCaptureError::kWinMediaFoundationEngineIsNull, FROM_HERE, E_FAIL);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationEngineGetSourceFailed, FROM_HERE, hr);
        return;
    }

    hr = FillCapabilities(source.Get(), true, &photo_capabilities_);
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationFillPhotoCapabilitiesFailed, FROM_HERE, hr);
        return;
    }

    if (!photo_capabilities_.empty()) {
        selected_photo_capability_ = std::make_unique<CapabilityWin>(photo_capabilities_.front());
    }

    CapabilityList video_capabilities;
    hr = FillCapabilities(source.Get(), false, &video_capabilities);
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationFillVideoCapabilitiesFailed, FROM_HERE, hr);
        return;
    }

    if (video_capabilities.empty()) {
        OnError(VideoCaptureError::kWinMediaFoundationNoVideoCapabilityFound, FROM_HERE, "No video capability found");
        return;
    }

    const CapabilityWin best_match_video_capability = GetBestMatchedCapability(params.requested_format, video_capabilities);
    ComPtr<IMFMediaType> source_video_media_type;
    hr = GetAvailableDeviceMediaType(
        source.Get(), best_match_video_capability.stream_index, best_match_video_capability.media_type_index, &source_video_media_type);
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationGetAvailableDeviceMediaTypeFailed, FROM_HERE, hr);
        return;
    }

    hr = source->SetCurrentDeviceMediaType(best_match_video_capability.stream_index, source_video_media_type.Get());
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationSetCurrentDeviceMediaTypeFailed, FROM_HERE, hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationEngineGetSinkFailed, FROM_HERE, hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationSinkQueryCapturePreviewInterfaceFailed, FROM_HERE, hr);
        return;
    }

    hr = preview_sink->RemoveAllStreams();
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationSinkRemoveAllStreamsFailed, FROM_HERE, hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationCreateSinkVideoMediaTypeFailed, FROM_HERE, hr);
        return;
    }

    hr = ConvertToVideoSinkMediaType(source_video_media_type.Get(),
        /*use_hardware_format=*/static_cast<bool>(dxgi_device_manager_), sink_video_media_type.Get());
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationConvertToVideoSinkMediaTypeFailed, FROM_HERE, hr);
        return;
    }
    ...
    if (!base::FeatureList::IsEnabled(media::kWebRTCColorAccuracy)
        && (best_match_video_capability.source_pixel_format != best_match_video_capability.supported_format.pixel_format)) {
        source_video_media_type->SetUINT32(MF_MT_VIDEO_NOMINAL_RANGE, MFNominalRange_16_235);
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationSinkAddStreamFailed, FROM_HERE, hr);
        return;
    }

    hr = preview_sink->SetSampleCallback(dw_sink_stream_index, video_callback_.get());
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationSinkSetSampleCallbackFailed, FROM_HERE, hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        OnError(VideoCaptureError::kWinMediaFoundationEngineStartPreviewFailed, FROM_HERE, hr);
        return;
    }
    ...
    base::UmaHistogramEnumeration(
        "Media.VideoCapture.Win.Device.InternalPixelFormat", best_match_video_capability.source_pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
    base::UmaHistogramEnumeration("Media.VideoCapture.Win.Device.CapturePixelFormat", best_match_video_capability.supported_format.pixel_format,
        media::VideoPixelFormat::PIXEL_FORMAT_MAX);
    base::UmaHistogramEnumeration(
        "Media.VideoCapture.Win.Device.RequestedPixelFormat", params.requested_format.pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
}

void VideoCaptureDeviceMFWin::StopAndDeAllocate()
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::StopAndDeAllocate");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::TakePhoto");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    hr = GetAvailableDeviceMediaType(source.Get(), selected_photo_capability_->stream_index, selected_photo_capability_->media_type_index, &source_media_type);
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }

    hr = source->SetCurrentDeviceMediaType(selected_photo_capability_->stream_index, source_media_type.Get());
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }
    ...
    hr = ConvertToPhotoSinkMediaType(source_media_type.Get(), sink_media_type.Get());
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }
    ...
    hr = GetFormatFromSourceMediaType(sink_media_type.Get(), true,
             /*use_hardware_format=*/false, &format, &source_format)
        ? S_OK
        : E_FAIL;
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
    ...
    DWORD dw_sink_stream_index = 0;
    hr = photo_sink->AddStream(selected_photo_capability_->stream_index, sink_media_type.Get(), nullptr, &dw_sink_stream_index);
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }

    scoped_refptr<IMFCaptureEngineOnSampleCallback> photo_callback = create_mf_photo_callback_.Run(std::move(callback), format);
...
void VideoCaptureDeviceMFWin::GetPhotoState(GetPhotoStateCallback callback)
{
    ...
    hr = source->GetCurrentDeviceMediaType(
        selected_photo_capability_ ? selected_photo_capability_->stream_index : selected_video_capability_->stream_index, &current_media_type);
    if (FAILED(hr)) {
        LogError(FROM_HERE, hr);
        return;
    }
    ...
    photo_capabilities->height = mojom::Range::New(max_size.height(), min_size.height(), current_size.height(), 1);
    photo_capabilities->width = mojom::Range::New(max_size.width(), min_size.width(), current_size.width(), 1);

    if (camera_control_ && video_control_) {
        photo_capabilities->color_temperature = RetrieveControlRangeAndCurrent(
            video_control_, VideoProcAmp_WhiteBalance, &photo_capabilities->supported_white_balance_modes, &photo_capabilities->current_white_balance_mode);

        photo_capabilities->exposure_time
            = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Exposure, &photo_capabilities->supported_exposure_modes,
                &photo_capabilities->current_exposure_mode, PlatformExposureTimeToCaptureValue, PlatformExposureTimeToCaptureStep);

        photo_capabilities->focus_distance = RetrieveControlRangeAndCurrent(
            camera_control_, CameraControl_Focus, &photo_capabilities->supported_focus_modes, &photo_capabilities->current_focus_mode);

        photo_capabilities->brightness = RetrieveControlRangeAndCurrent(video_control_, VideoProcAmp_Brightness);
        photo_capabilities->contrast = RetrieveControlRangeAndCurrent(video_control_, VideoProcAmp_Contrast);
        photo_capabilities->exposure_compensation = RetrieveControlRangeAndCurrent(video_control_, VideoProcAmp_Gain);
        ...
        photo_capabilities->saturation = RetrieveControlRangeAndCurrent(video_control_, VideoProcAmp_Saturation);
        photo_capabilities->sharpness = RetrieveControlRangeAndCurrent(video_control_, VideoProcAmp_Sharpness);
        photo_capabilities->torch = false;
        photo_capabilities->pan
            = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Pan, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
        photo_capabilities->tilt
            = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Tilt, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
        photo_capabilities->zoom = RetrieveControlRangeAndCurrent(camera_control_, CameraControl_Zoom);
    }
    ...
    if (extended_camera_controller_) {
        ...
        hr = extended_camera_controller_->GetExtendedCameraControl(
            MF_CAPTURE_ENGINE_MEDIASOURCE, KSPROPERTY_CAMERACONTROL_EXTENDED_BACKGROUNDSEGMENTATION, &extended_camera_control);
        DLOG_IF_FAILED_WITH_HRESULT("Failed to retrieve IMFExtendedCameraControl for background "
                                    "segmentation",
            hr);
        if (SUCCEEDED(hr) && (extended_camera_control->GetCapabilities() & KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_BLUR)) {
            photo_capabilities->supported_background_blur_modes = { mojom::BackgroundBlurMode::OFF, mojom::BackgroundBlurMode::BLUR };
            photo_capabilities->background_blur_mode = (extended_camera_control->GetFlags() & KSCAMERA_EXTENDEDPROP_BACKGROUNDSEGMENTATION_BLUR)
                ? mojom::BackgroundBlurMode::BLUR
                : mojom::BackgroundBlurMode::OFF;
        }

        hr = extended_camera_controller_->GetExtendedCameraControl(
            MF_CAPTURE_ENGINE_MEDIASOURCE, KSPROPERTY_CAMERACONTROL_EXTENDED_DIGITALWINDOW, &extended_camera_control);
        DLOG_IF_FAILED_WITH_HRESULT("Failed to retrieve IMFExtendedCameraControl for digital window", hr);
        if (SUCCEEDED(hr) && (extended_camera_control->GetCapabilities() & KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_AUTOFACEFRAMING)) {
            photo_capabilities->supported_face_framing_modes = { mojom::MeteringMode::NONE, mojom::MeteringMode::CONTINUOUS };
            photo_capabilities->current_face_framing_mode = (extended_camera_control->GetFlags() & KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_AUTOFACEFRAMING)
                ? mojom::MeteringMode::CONTINUOUS
                : mojom::MeteringMode::NONE;
        }

        hr = extended_camera_controller_->GetExtendedCameraControl(
            MF_CAPTURE_ENGINE_MEDIASOURCE, KSPROPERTY_CAMERACONTROL_EXTENDED_EYEGAZECORRECTION, &extended_camera_control);
        DLOG_IF_FAILED_WITH_HRESULT("Failed to retrieve IMFExtendedCameraControl for eye gaze correction", hr);
        if (SUCCEEDED(hr)) {
            std::vector<mojom::EyeGazeCorrectionMode> capture_modes = ExtendedPlatformFlagsToCaptureModes(extended_camera_control->GetCapabilities());
            if (!capture_modes.empty()) {
                photo_capabilities->current_eye_gaze_correction_mode
                    = ExtendedPlatformFlagsToCaptureMode(extended_camera_control->GetFlags(), mojom::EyeGazeCorrectionMode::OFF);
                photo_capabilities->supported_eye_gaze_correction_modes = std::move(capture_modes);
            }
        }
    }
...
void VideoCaptureDeviceMFWin::SetPhotoOptions(mojom::PhotoSettingsPtr settings, SetPhotoOptionsCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!photo_capabilities_.empty() && (settings->has_height || settings->has_width)) {
        ComPtr<IMFMediaType> current_source_media_type;
        hr = source->GetCurrentDeviceMediaType(selected_photo_capability_->stream_index, &current_source_media_type);
        ...
        const CapabilityWin best_match = GetBestMatchedPhotoCapability(current_source_media_type, requested_size, photo_capabilities_);
    ...
    if (camera_control_ && video_control_) {
        if (settings->has_white_balance_mode) {
            if (settings->white_balance_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = SetVideoControlProperty(VideoProcAmp_WhiteBalance, 0L, VideoProcAmp_Flags_Auto);
        ...
        if (white_balance_mode_manual_ && settings->has_color_temperature) {
            hr = SetVideoControlProperty(VideoProcAmp_WhiteBalance, settings->color_temperature, VideoProcAmp_Flags_Manual);
        ...
        if (settings->has_exposure_mode) {
            if (settings->exposure_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = SetCameraControlProperty(CameraControl_Exposure, 0L, CameraControl_Flags_Auto);
        ...
        if (exposure_mode_manual_ && settings->has_exposure_time) {
            hr = SetCameraControlProperty(CameraControl_Exposure, CaptureExposureTimeToPlatformValue(settings->exposure_time), CameraControl_Flags_Manual);
        ...
        if (settings->has_focus_mode) {
            if (settings->focus_mode == mojom::MeteringMode::CONTINUOUS) {
                hr = SetCameraControlProperty(CameraControl_Focus, 0L, CameraControl_Flags_Auto);
        ...
        if (focus_mode_manual_ && settings->has_focus_distance) {
            hr = SetCameraControlProperty(CameraControl_Focus, settings->focus_distance, CameraControl_Flags_Manual);
        ...
        if (settings->has_brightness) {
            hr = SetVideoControlProperty(VideoProcAmp_Brightness, settings->brightness, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Brightness config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_contrast) {
            hr = SetVideoControlProperty(VideoProcAmp_Contrast, settings->contrast, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Contrast config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_exposure_compensation) {
            hr = SetVideoControlProperty(VideoProcAmp_Gain, settings->exposure_compensation, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Exposure Compensation config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_saturation) {
            hr = SetVideoControlProperty(VideoProcAmp_Saturation, settings->saturation, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Saturation config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_sharpness) {
            hr = SetVideoControlProperty(VideoProcAmp_Sharpness, settings->sharpness, VideoProcAmp_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Sharpness config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_pan) {
            hr = SetCameraControlProperty(CameraControl_Pan, CaptureAngleToPlatformValue(settings->pan), CameraControl_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Pan config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_tilt) {
            hr = SetCameraControlProperty(CameraControl_Tilt, CaptureAngleToPlatformValue(settings->tilt), CameraControl_Flags_Manual);
            DLOG_IF_FAILED_WITH_HRESULT("Tilt config failed", hr);
            if (FAILED(hr))
                return;
        }
        if (settings->has_zoom) {
            hr = SetCameraControlProperty(CameraControl_Zoom, settings->zoom, CameraControl_Flags_Manual);
    ...
    if (extended_camera_controller_) {
        if (settings->has_background_blur_mode) {
            ...
            hr = SetAndCommitExtendedCameraControlFlags(KSPROPERTY_CAMERACONTROL_EXTENDED_BACKGROUNDSEGMENTATION, flag);
        ...
        if (settings->eye_gaze_correction_mode.has_value()) {
            const ULONGLONG flags = CaptureModeToExtendedPlatformFlags(settings->eye_gaze_correction_mode.value());
            hr = SetAndCommitExtendedCameraControlFlags(KSPROPERTY_CAMERACONTROL_EXTENDED_EYEGAZECORRECTION, flags);
        ...
        if (settings->has_face_framing_mode) {
            const ULONGLONG flags = settings->face_framing_mode == mojom::MeteringMode::CONTINUOUS ? KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_AUTOFACEFRAMING
                                                                                                   : KSCAMERA_EXTENDEDPROP_DIGITALWINDOW_MANUAL;
            hr = SetAndCommitExtendedCameraControlFlags(KSPROPERTY_CAMERACONTROL_EXTENDED_DIGITALWINDOW, flags);
...
void VideoCaptureDeviceMFWin::OnUtilizationReport(media::VideoCaptureFeedback feedback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void VideoCaptureDeviceMFWin::OnCameraControlChange(REFGUID control_set, UINT32 id)
{
    ...
    main_thread_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::OnCameraControlChangeInternal, weak_factory_.GetWeakPtr(), control_set, id));
}

void VideoCaptureDeviceMFWin::OnCameraControlChangeInternal(REFGUID control_set, UINT32 id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (control_set == PROPSETID_VIDCAP_CAMERACONTROL) {
        ...
        if (iter != set_camera_control_properties_.end()) {
            ...
            // notified.
            long value, flags;
            if (camera_control_ && SUCCEEDED(camera_control_->Get(id, &value, &flags))
                && (flags & (CameraControl_Flags_Auto | CameraControl_Flags_Manual)) == iter->second.flags
        ...
        if (iter != set_video_control_properties_.end()) {
            ...
            // notified.
            long value, flags;
            if (video_control_ && SUCCEEDED(video_control_->Get(id, &value, &flags))
                && (flags & (VideoProcAmp_Flags_Auto | VideoProcAmp_Flags_Manual)) == iter->second.flags
        ...
        if (iter != set_extended_camera_control_flags_.end()) {
            ...
            ComPtr<IMFExtendedCameraControl> extended_camera_control;
            if (extended_camera_controller_
                && SUCCEEDED(extended_camera_controller_->GetExtendedCameraControl(MF_CAPTURE_ENGINE_MEDIASOURCE, id, &extended_camera_control))
...
void VideoCaptureDeviceMFWin::OnIncomingCapturedData(
    Microsoft::WRL::ComPtr<IMFMediaBuffer> buffer, base::TimeTicks reference_time, base::TimeDelta timestamp, base::TimeTicks capture_begin_time)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnIncomingCapturedData");
    ...
    if (need_to_post) {
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal, weak_factory_.GetWeakPtr()));
    }
}

HRESULT VideoCaptureDeviceMFWin::DeliverTextureToClient(Microsoft::WRL::ComPtr<IMFMediaBuffer> imf_buffer, ID3D11Texture2D* texture,
    base::TimeTicks reference_time, base::TimeDelta timestamp, base::TimeTicks capture_begin_time)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::DeliverTextureToClient");
    ...
    if (FAILED(hr)) {
        ...
        LOG(ERROR) << "Camera texture device lost: " << logging::SystemErrorCodeToString(hr);
        base::UmaHistogramSparse("Media.VideoCapture.Win.D3DDeviceRemovedReason", hr);
    ...
    GetTextureInfo(texture, texture_size, pixel_format, is_cross_process_shared_texture);

    if (pixel_format != PIXEL_FORMAT_NV12) {
        return MF_E_UNSUPPORTED_FORMAT;
    }

    if (base::FeatureList::IsEnabled(kMediaFoundationD3D11VideoCaptureZeroCopy) && is_cross_process_shared_texture) {
        return DeliverExternalBufferToClient(std::move(imf_buffer), texture, texture_size, pixel_format, reference_time, timestamp, capture_begin_time);
    }

    VideoCaptureDevice::Client::Buffer capture_buffer;
    constexpr int kDummyFrameFeedbackId = 0;
    auto result = client_->ReserveOutputBuffer(texture_size, pixel_format, kDummyFrameFeedbackId, &capture_buffer,
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "Failed to copy camera device texture to output texture: " << logging::SystemErrorCodeToString(hr);
        return hr;
    }

    capture_buffer.is_premapped = false;
    if (last_premapped_request_ > base::TimeTicks::Now() - kMaxFeedbackPremappingEffectDuration) {
    ...
    client_->OnIncomingCapturedBufferExt(std::move(capture_buffer),
        VideoCaptureFormat(texture_size, selected_video_capability_->supported_format.frame_rate, pixel_format), color_space_, reference_time, timestamp,
        MaybeForwardCaptureBeginTime(capture_begin_time), gfx::Rect(texture_size), frame_metadata);

    return hr;
}

HRESULT VideoCaptureDeviceMFWin::DeliverExternalBufferToClient(Microsoft::WRL::ComPtr<IMFMediaBuffer> imf_buffer, ID3D11Texture2D* texture,
    const gfx::Size& texture_size, const VideoPixelFormat& pixel_format, base::TimeTicks reference_time, base::TimeDelta timestamp,
    base::TimeTicks capture_begin_time)
{
    UINT private_data_size;
    Microsoft::WRL::ComPtr<DXGIHandlePrivateData> private_data;
    HRESULT hr = texture->GetPrivateData(DXGIHandlePrivateDataGUID, &private_data_size, static_cast<void**>(&private_data));
    ...
    if (SUCCEEDED(hr) && private_data) {
        ...
        hr = dxgi_resource->CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ | DXGI_SHARED_RESOURCE_WRITE, nullptr, &texture_handle);
        if (FAILED(hr)) {
            DLOG(ERROR) << logging::SystemErrorCodeToString(hr);
            return hr;
        }
        private_data = Microsoft::WRL::Make<DXGIHandlePrivateData>(base::win::ScopedHandle(texture_handle));

        hr = texture->SetPrivateDataInterface(DXGIHandlePrivateDataGUID, private_data.Get());
        if (FAILED(hr)) {
            DLOG(ERROR) << "failed to set private data to texture, " << logging::SystemErrorCodeToString(hr);
            return hr;
        }
    }
    ...
    CHECK(::DuplicateHandle(
        GetCurrentProcess(), private_data->GetTextureHandle(), GetCurrentProcess(), &texture_handle_duplicated, 0, FALSE, DUPLICATE_SAME_ACCESS))
    ...
    media::CapturedExternalVideoBuffer external_buffer = media::CapturedExternalVideoBuffer(std::move(imf_buffer), std::move(gmb_handle),
        VideoCaptureFormat(texture_size, selected_video_capability_->supported_format.frame_rate, pixel_format), gfx::ColorSpace());
    client_->OnIncomingCapturedExternalBuffer(
        std::move(external_buffer), reference_time, timestamp, MaybeForwardCaptureBeginTime(capture_begin_time), gfx::Rect(texture_size));
    return hr;
}

void VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal()
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnIncomingCapturedDataInternal");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (client_.get()) {
        ...
        // also NV12.
        if (dxgi_device_manager_ && selected_video_capability_->supported_format.pixel_format == PIXEL_FORMAT_NV12
            && params_.requested_format.pixel_format == PIXEL_FORMAT_NV12 && SUCCEEDED(GetTextureFromMFBuffer(buffer.Get(), &texture))) {
            HRESULT hr = DeliverTextureToClient(buffer, texture.Get(), reference_time, timestamp, capture_begin_time);
            DLOG_IF_FAILED_WITH_HRESULT("Failed to deliver D3D11 texture to client.", hr);
    ...
    if (!delivered_texture && client_.get()) {
        client_->OnIncomingCapturedData(locked_buffer.data(), locked_buffer.length(), selected_video_capability_->supported_format, color_space_,
            camera_rotation_.value(), false /* flip_y */, reference_time, timestamp, MaybeForwardCaptureBeginTime(capture_begin_time));
    }

    while (!video_stream_take_photo_callbacks_.empty()) {
        TakePhotoCallback cb = std::move(video_stream_take_photo_callbacks_.front());
        video_stream_take_photo_callbacks_.pop();

        mojom::BlobPtr blob = RotateAndBlobify(locked_buffer.data(), locked_buffer.length(), selected_video_capability_->supported_format, 0);
        if (!blob) {
            continue;
        }
...
void VideoCaptureDeviceMFWin::OnFrameDropped(VideoCaptureFrameDropReason reason)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnFrameDropped");
    // This is called on IMFCaptureEngine thread.
    // To serialize all access to this class we post to the task
    // runner which is used for Video capture service API calls
    // (E.g. DeallocateAndStop).
    main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::OnFrameDroppedInternal, weak_factory_.GetWeakPtr(), reason));
}

void VideoCaptureDeviceMFWin::OnFrameDroppedInternal(VideoCaptureFrameDropReason reason)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnFrameDroppedInternal");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnEvent");
    ...
    if (capture_event_guid == MF_CAPTURE_ENGINE_ERROR || FAILED(hr)) {
        TRACE_EVENT_INSTANT1(
            TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::OnEvent", TRACE_EVENT_SCOPE_PROCESS, "error HR", hr);
        ...
        // runner which is used for Video capture service API calls
        // (E.g. DeallocateAndStop).
        main_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoCaptureDeviceMFWin::ProcessEventError, weak_factory_.GetWeakPtr(), hr));
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::ProcessEventError");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (hr == MF_E_HW_MFT_FAILED_START_STREAMING) {
        ...
        if (!activities_report_callback_) {
            activities_report_callback_ = new MFActivitiesReportCallback(weak_factory_.GetWeakPtr(), main_thread_task_runner_, device_descriptor_.device_id);
        }
        if (!activity_monitor_) {
            bool created = CreateMFSensorActivityMonitor(activities_report_callback_.get(), &activity_monitor_);
            if (!created) {
                // Can't rely on activity monitor to check if the camera is in use.
                // Just report the error.
                RecordErrorHistogram(hr);
                OnError(VideoCaptureError::kWinMediaFoundationGetMediaEventStatusFailed, FROM_HERE, hr);
                return;
            }
        }
        // Post default action in case there will be no callback calls.
        activity_report_pending_ = true;
        main_thread_task_runner_->PostDelayedTask(FROM_HERE,
            base::BindOnce(&VideoCaptureDeviceMFWin::OnCameraInUseReport, weak_factory_.GetWeakPtr(), /*in_use=*/false,
    ...
    if (hr == DXGI_ERROR_DEVICE_REMOVED && dxgi_device_manager_ != nullptr) {
        ...
        if (try_d3d_path) {
            HRESULT removed_hr = dxgi_device_manager_->CheckDeviceRemovedAndGetDevice(&recreated_d3d_device);
            LOG(ERROR) << "OnEvent: Device was Removed. Reason: " << logging::SystemErrorCodeToString(removed_hr);
        } else {
        ...
        if ((!try_d3d_path || recreated_d3d_device) && RecreateMFSource() && Init()) {
...
void VideoCaptureDeviceMFWin::OnError(VideoCaptureError error, const Location& from_here, HRESULT hr)
{
    OnError(error, from_here, logging::SystemErrorCodeToString(hr).c_str());
}

void VideoCaptureDeviceMFWin::OnError(VideoCaptureError error, const Location& from_here, const char* message)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    client_->OnError(error, from_here, base::StringPrintf("VideoCaptureDeviceMFWin: %s", message));
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::WaitOnCaptureEvent");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    DWORD wait_result = ::WaitForMultipleObjects(std::size(events), events, FALSE, INFINITE);
    switch (wait_result) {
    case WAIT_OBJECT_0:
        break;
    case WAIT_FAILED:
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceMFWin::RecreateMFSource");
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    const bool is_sensor_api = device_descriptor_.capture_api == VideoCaptureApi::WIN_MEDIA_FOUNDATION_SENSOR;
    ComPtr<IMFAttributes> attributes;
    HRESULT hr = MFCreateAttributes(&attributes, is_sensor_api ? 3 : 2);
    if (FAILED(hr)) {
        LOG(ERROR) << "Failed to create attributes: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    attributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE, MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID);
    if (is_sensor_api) {
        attributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_CATEGORY, KSCATEGORY_SENSOR_CAMERA);
    }
    attributes->SetString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, base::SysUTF8ToWide(device_descriptor_.device_id).c_str());
    hr = MFCreateDeviceSource(attributes.Get(), &source_);
    if (FAILED(hr)) {
        LOG(ERROR) << "MFCreateDeviceSource failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
...
void VideoCaptureDeviceMFWin::OnCameraInUseReport(bool in_use, bool is_default_action)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (in_use) {
        OnError(VideoCaptureError::kWinMediaFoundationCameraBusy, FROM_HERE, "Camera is in use by another process");
    } else {
        RecordErrorHistogram(MF_E_HW_MFT_FAILED_START_STREAMING);
        OnError(VideoCaptureError::kWinMediaFoundationGetMediaEventStatusFailed, FROM_HERE, MF_E_HW_MFT_FAILED_START_STREAMING);
    }
...
bool CreateMFSensorActivityMonitor(IMFSensorActivitiesReportCallback* report_callback, IMFSensorActivityMonitor** monitor)
{
    // The MF DLLs have been loaded by VideoCaptureDeviceFactoryWin.
    // Just get a DLL module handle here, once.
    static const HMODULE module = GetModuleHandleW(L"mfsensorgroup.dll");
    ...
    using MFCreateSensorActivityMonitorType = decltype(&MFCreateSensorActivityMonitor);
    static const MFCreateSensorActivityMonitorType create_function
        = reinterpret_cast<MFCreateSensorActivityMonitorType>(GetProcAddress(module, "MFCreateSensorActivityMonitor"));
    if (!create_function) {
    ...
    if (!*monitor) {
        LOG(ERROR) << "Failed to create IMFSensorActivityMonitor: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    return true;
}
