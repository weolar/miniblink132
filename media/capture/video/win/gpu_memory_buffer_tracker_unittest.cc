...
protected:
    ...
    void SetUp() override
    {
        ...
        dxgi_device_manager_ = scoped_refptr<MockDXGIDeviceManager>(new MockDXGIDeviceManager());
    }
...
TEST_F(GpuMemoryBufferTrackerTest, TextureCreation)
{
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()),
        OnCreateTexture2D(Pointee(AllOf(Field(&D3D11_TEXTURE2D_DESC::Format, expected_buffer_format),
                              Field(&D3D11_TEXTURE2D_DESC::Width, static_cast<const unsigned int>(expected_buffer_size.width())),
                              Field(&D3D11_TEXTURE2D_DESC::Height, static_cast<const unsigned int>(expected_buffer_size.height())))),
            _, _));
    std::unique_ptr<VideoCaptureBufferTracker> tracker = std::make_unique<GpuMemoryBufferTracker>(dxgi_device_manager_);
    EXPECT_EQ(tracker->Init(expected_buffer_size, PIXEL_FORMAT_NV12, nullptr), true);
}
...
TEST_F(GpuMemoryBufferTrackerTest, InvalidateOnDeviceLoss)
{
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()),
        ...
        .Times(1);
    // Mock device loss.
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()), OnGetDeviceRemovedReason()).WillOnce(Invoke([]() { return DXGI_ERROR_DEVICE_REMOVED; }));
    // Create and init tracker (causes initial texture creation)
    std::unique_ptr<VideoCaptureBufferTracker> tracker = std::make_unique<GpuMemoryBufferTracker>(dxgi_device_manager_);
    EXPECT_EQ(tracker->Init(expected_buffer_size, PIXEL_FORMAT_NV12, nullptr), true);
    // The tracker now should be invalid.
    EXPECT_FALSE(tracker->IsReusableForFormat(expected_buffer_size, PIXEL_FORMAT_NV12, nullptr));
...
TEST_F(GpuMemoryBufferTrackerTest, GetMemorySizeInBytes)
{
    ...
    const uint32_t expectedSizeInBytes = (expected_buffer_size.width() * expected_buffer_size.height() * 3) / 2;
...
TEST_F(GpuMemoryBufferTrackerTest, DuplicateAsUnsafeRegion)
{
    ...
    Microsoft::WRL::ComPtr<MockD3D11Texture2D> dxgi_texture(new MockD3D11Texture2D(mock_desc, dxgi_device_manager_->GetMockDevice().Get()));
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()), DoOpenSharedResource1)
        .WillOnce(Invoke([&dxgi_texture](HANDLE resource, REFIID returned_interface, void** resource_out) {
    ...
    // texture.
    ID3D11Resource* expected_source = static_cast<ID3D11Resource*>(mock_dxgi_texture.Get());

    EXPECT_CALL(*dxgi_device_manager_->GetMockDevice()->mock_immediate_context_.Get(), OnCopySubresourceRegion(_, _, _, _, _, expected_source, _, _)).Times(1);

    uint8_t* tmp_mapped_resource;

    EXPECT_CALL(*dxgi_device_manager_->GetMockDevice()->mock_immediate_context_.Get(), OnMap(_, _, _, _, _))
        .WillOnce(Invoke(
            [&tmp_mapped_resource](ID3D11Resource* resource, UINT subresource, D3D11_MAP MapType, UINT MapFlags, D3D11_MAPPED_SUBRESOURCE* mapped_resource) {
    ...
    EXPECT_CALL(*dxgi_device_manager_->GetMockDevice()->mock_immediate_context_.Get(), OnUnmap(_, _))
        .WillOnce(Invoke([&tmp_mapped_resource](ID3D11Resource* resource, UINT subresource) { delete[] tmp_mapped_resource; }));
