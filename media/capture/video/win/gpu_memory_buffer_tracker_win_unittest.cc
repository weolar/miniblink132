...
protected:
    ...
    void SetUp() override
    {
        if (!VideoCaptureDeviceFactoryWin::PlatformSupportsMediaFoundation()) {
            GTEST_SKIP() << "Media foundation is not supported by the current platform.";
        }
...
TEST_F(GpuMemoryBufferTrackerWinTest, TextureCreation)
{
    ...
    EXPECT_CALL(*(dxgi_device_manager_->GetMockDevice().Get()),
        OnCreateTexture2D(Pointee(AllOf(Field(&D3D11_TEXTURE2D_DESC::Format, expected_buffer_format),
            ...
            _, _));
    std::unique_ptr<VideoCaptureBufferTracker> tracker = std::make_unique<GpuMemoryBufferTrackerWin>(dxgi_device_manager_);
