...
static mojom::RangePtr RetrieveControlRangeAndCurrent(RangeGetter range_getter, CurrentValueGetter current_value_getter,
    ...
    double (*value_converter)(long) = PlatformToCaptureValue, double (*step_converter)(long, double, double) = PlatformToCaptureStep)
{
    ...
    DLOG_IF(ERROR, FAILED(hr)) << "Control range reading failed: " << logging::SystemErrorCodeToString(hr);
    if (SUCCEEDED(hr)) {
        control_range->min = value_converter(min);
        control_range->max = value_converter(max);
        control_range->step = step_converter(step, control_range->min, control_range->max);
    ...
    DLOG_IF(ERROR, FAILED(hr)) << "Control value reading failed: " << logging::SystemErrorCodeToString(hr);
    if (SUCCEEDED(hr)) {
