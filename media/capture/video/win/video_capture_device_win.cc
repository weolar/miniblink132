...
bool PinMatchesCategory(IPin* pin, REFGUID category)
{
    ...
    if (SUCCEEDED(hr)) {
        GUID pin_category;
        DWORD return_value;
        hr = ks_property->Get(AMPROPSETID_Pin, AMPROPERTY_PIN_CATEGORY, nullptr, 0, &pin_category, sizeof(pin_category), &return_value);
...
// static
VideoCaptureControlSupport VideoCaptureDeviceWin::GetControlSupport(ComPtr<IBaseFilter> capture_filter)
{
    VideoCaptureControlSupport control_support;
    ComPtr<ICameraControl> camera_control;
    ComPtr<IVideoProcAmp> video_control;

    if (GetCameraAndVideoControls(capture_filter, &camera_control, &video_control)) {
        long min, max, step, default_value, flags;
        control_support.pan = SUCCEEDED(camera_control->getRange_Pan(&min, &max, &step, &default_value, &flags)) && min < max;
        control_support.tilt = SUCCEEDED(camera_control->getRange_Tilt(&min, &max, &step, &default_value, &flags)) && min < max;
        control_support.zoom = SUCCEEDED(camera_control->getRange_Zoom(&min, &max, &step, &default_value, &flags)) && min < max;
    }
...
// static
void VideoCaptureDeviceWin::GetDeviceCapabilityList(ComPtr<IBaseFilter> capture_filter, bool query_detailed_frame_rates, CapabilityList* out_capability_list)
{
    ComPtr<IPin> output_capture_pin(VideoCaptureDeviceWin::GetPin(capture_filter.Get(), PINDIR_OUTPUT, PIN_CATEGORY_CAPTURE, GUID_NULL));
    if (!output_capture_pin.Get()) {
        DLOG(ERROR) << "Failed to get capture output pin";
        return;
    }

    GetPinCapabilityList(capture_filter, output_capture_pin, query_detailed_frame_rates, out_capability_list);
}

// static
void VideoCaptureDeviceWin::GetPinCapabilityList(
    ComPtr<IBaseFilter> capture_filter, ComPtr<IPin> output_capture_pin, bool query_detailed_frame_rates, CapabilityList* out_capability_list)
{
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "GetNumberOfCapabilities failed: " << logging::SystemErrorCodeToString(hr);
        return;
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        hr = stream_config->GetStreamCaps(i, &media_type.Receive()->AsEphemeralRawAddr(), caps.data());
        // GetStreamCaps() may return S_FALSE, so don't use FAILED() or SUCCEED()
        // macros here since they'll trigger incorrectly.
        if (hr != S_OK || !media_type.get()) {
            DLOG(ERROR) << "GetStreamCaps failed: " << logging::SystemErrorCodeToString(hr);
            return;
        }

        if (media_type->majortype == MEDIATYPE_Video && media_type->formattype == FORMAT_VideoInfo) {
            VideoCaptureFormat format;
            format.pixel_format = VideoCaptureDeviceWin::TranslateMediaSubtypeToPixelFormat(media_type->subtype);
            ...
            if (query_detailed_frame_rates && video_control.Get()) {
                ...
                LONG list_size = 0;
                const SIZE size = { format.frame_size.width(), format.frame_size.height() };
                hr = video_control->GetFrameRateList(output_capture_pin.Get(), i, size, &list_size, &time_per_frame_list);
                ...
                if (hr == S_OK && list_size > 0 && time_per_frame_list) {
                    for (int k = 0; k < list_size; k++) {
                        ...
                        if (time_per_frame <= 0)
                            continue;
                        frame_rates.push_back(kSecondsToReferenceTime / static_cast<float>(time_per_frame));
                    }
                }
            }

            if (frame_rates.empty() && h->AvgTimePerFrame > 0) {
                frame_rates.push_back(kSecondsToReferenceTime / static_cast<float>(h->AvgTimePerFrame));
            }
...
// static
ComPtr<IPin> VideoCaptureDeviceWin::GetPin(ComPtr<IBaseFilter> capture_filter, PIN_DIRECTION pin_dir, REFGUID category, REFGUID major_type)
{
    ComPtr<IPin> pin;
...
// static
VideoPixelFormat VideoCaptureDeviceWin::TranslateMediaSubtypeToPixelFormat(const GUID& sub_type)
{
    static struct {
    ...
    DVLOG(2) << "Device (also) supports an unknown media type " << base::win::WStringFromGUID(sub_type);
...
// http://msdn.microsoft.com/en-us/library/dd375432(VS.85).aspx
void VideoCaptureDeviceWin::ScopedMediaType::DeleteMediaType(AM_MEDIA_TYPE* mt)
{
    if (mt != nullptr) {
        FreeMediaType(mt);
        CoTaskMemFree(mt);
...
VideoCaptureDeviceWin::VideoCaptureDeviceWin(const VideoCaptureDeviceDescriptor& device_descriptor, ComPtr<IBaseFilter> capture_filter)
    ...
    , enable_get_photo_state_(base::FeatureList::IsEnabled(media::kDirectShowGetPhotoState))
{
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    output_capture_pin_ = GetPin(capture_filter_.Get(), PINDIR_OUTPUT, PIN_CATEGORY_CAPTURE, GUID_NULL);
    ...
    HRESULT hr = ::CoCreateInstance(CLSID_FilterGraph, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&graph_builder_));
    DLOG_IF_FAILED_WITH_HRESULT("Failed to create capture filter", hr);
    if (FAILED(hr))
        return false;

    hr = ::CoCreateInstance(CLSID_CaptureGraphBuilder2, nullptr, CLSCTX_INPROC, IID_PPV_ARGS(&capture_graph_builder_));
    ...
    DLOG_IF_FAILED_WITH_HRESULT("Failed to give graph to capture graph builder", hr);
    if (FAILED(hr))
        return false;
    ...
    hr = capture_graph_builder_->FindInterface(
        &PIN_CATEGORY_CAPTURE, &MEDIATYPE_Interleaved, capture_filter_.Get(), IID_IAMStreamConfig, (void**)&stream_config);
    if (FAILED(hr)) {
        hr = capture_graph_builder_->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, capture_filter_.Get(), IID_IAMStreamConfig, (void**)&stream_config);
...
void VideoCaptureDeviceWin::AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<VideoCaptureDevice::Client> client)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceWin::AllocateAndStart");
    ...
    // than the capability.
    const float frame_rate = std::min(params.requested_format.frame_rate, found_capability.supported_format.frame_rate);

    ComPtr<IAMStreamConfig> stream_config;
    HRESULT hr = output_capture_pin_.As(&stream_config);
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowCantGetCaptureFormatSettings, FROM_HERE, "Can't get the Capture format settings", hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToGetNumberOfCapabilities, FROM_HERE, "Failed to GetNumberOfCapabilities", hr);
        return;
    }
    ...
    hr = stream_config->GetStreamCaps(found_capability.media_type_index, &media_type.Receive()->AsEphemeralRawAddr(), caps.data());
    if (hr != S_OK) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToGetCaptureDeviceCapabilities, FROM_HERE, "Failed to get capture device capabilities", hr);
        return;
    }
    ...
    sink_filter_->SetRequestedMediaFormat(found_capability.supported_format.pixel_format, frame_rate, found_capability.info_header);
    // Order the capture device to use this format.
    hr = stream_config->SetFormat(media_type.get());
    if (FAILED(hr)) {
        SetErrorState(
            media::VideoCaptureError::kWinDirectShowFailedToSetCaptureDeviceOutputFormat, FROM_HERE, "Failed to set capture device output format", hr);
        return;
    }
    ...
    if (media_type->subtype == kMediaSubTypeHDYC) {
        ...
        hr = graph_builder_->Connect(output_capture_pin_.Get(), input_sink_pin_.Get());
    } else {
        hr = graph_builder_->ConnectDirect(output_capture_pin_.Get(), input_sink_pin_.Get(), nullptr);
    }

    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToConnectTheCaptureGraph, FROM_HERE, "Failed to connect the Capture graph.", hr);
        return;
    }

    hr = media_control_->Pause();
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToPauseTheCaptureDevice, FROM_HERE, "Failed to pause the Capture device", hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToStartTheCaptureDevice, FROM_HERE, "Failed to start the Capture device.", hr);
        return;
    }

    base::UmaHistogramEnumeration("Media.VideoCapture.Win.Device.InternalPixelFormat", capture_format_.pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
    base::UmaHistogramEnumeration("Media.VideoCapture.Win.Device.CapturePixelFormat", capture_format_.pixel_format, media::VideoPixelFormat::PIXEL_FORMAT_MAX);
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("video_and_image_capture"), "VideoCaptureDeviceWin::StopAndDeAllocate");
    if (state_ != kCapturing)
        return;

    HRESULT hr = media_control_->Stop();
    if (FAILED(hr)) {
        SetErrorState(media::VideoCaptureError::kWinDirectShowFailedToStopTheCaptureGraph, FROM_HERE, "Failed to stop the capture graph.", hr);
        return;
    }
...
void VideoCaptureDeviceWin::GetPhotoState(GetPhotoStateCallback callback)
{
    ...
    if (!camera_control_ || !video_control_) {
        if (!GetCameraAndVideoControls(capture_filter_, &camera_control_, &video_control_)) {
            return;
        }
    }
    ...
    photo_capabilities->exposure_time = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Exposure(args...); },
        [this](auto... args) { return this->camera_control_->get_Exposure(args...); }, &photo_capabilities->supported_exposure_modes,
    ...
    photo_capabilities->color_temperature
        = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_WhiteBalance(args...); },
            [this](auto... args) { return this->video_control_->get_WhiteBalance(args...); }, &photo_capabilities->supported_white_balance_modes,
            &photo_capabilities->current_white_balance_mode);

    photo_capabilities->focus_distance = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Focus(args...); },
        [this](auto... args) { return this->camera_control_->get_Focus(args...); }, &photo_capabilities->supported_focus_modes,
        &photo_capabilities->current_focus_mode);

    photo_capabilities->iso = mojom::Range::New();

    photo_capabilities->brightness = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_Brightness(args...); },
        [this](auto... args) { return this->video_control_->get_Brightness(args...); });
    photo_capabilities->contrast = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_Contrast(args...); },
        [this](auto... args) { return this->video_control_->get_Contrast(args...); });
    photo_capabilities->saturation = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_Saturation(args...); },
        [this](auto... args) { return this->video_control_->get_Saturation(args...); });
    photo_capabilities->sharpness = RetrieveControlRangeAndCurrent([this](auto... args) { return this->video_control_->getRange_Sharpness(args...); },
        [this](auto... args) { return this->video_control_->get_Sharpness(args...); });
    photo_capabilities->pan = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Pan(args...); },
        [this](auto... args) { return this->camera_control_->get_Pan(args...); }, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
    photo_capabilities->tilt = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Tilt(args...); },
        [this](auto... args) { return this->camera_control_->get_Tilt(args...); }, nullptr, nullptr, PlatformAngleToCaptureValue, PlatformAngleToCaptureStep);
    photo_capabilities->zoom = RetrieveControlRangeAndCurrent([this](auto... args) { return this->camera_control_->getRange_Zoom(args...); },
        [this](auto... args) { return this->camera_control_->get_Zoom(args...); });

    photo_capabilities->red_eye_reduction = mojom::RedEyeReduction::NEVER;
    photo_capabilities->height
        = mojom::Range::New(capture_format_.frame_size.height(), capture_format_.frame_size.height(), capture_format_.frame_size.height(), 0 /* step */);
    photo_capabilities->width
        = mojom::Range::New(capture_format_.frame_size.width(), capture_format_.frame_size.width(), capture_format_.frame_size.width(), 0 /* step */);
    photo_capabilities->torch = false;

    std::move(callback).Run(std::move(photo_capabilities));
}

void VideoCaptureDeviceWin::SetPhotoOptions(mojom::PhotoSettingsPtr settings, VideoCaptureDevice::SetPhotoOptionsCallback callback)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    if (white_balance_mode_manual_ && settings->has_color_temperature) {
        hr = video_control_->put_WhiteBalance(settings->color_temperature, VideoProcAmp_Flags_Manual);
    ...
    if (focus_mode_manual_ && settings->has_focus_distance) {
        hr = camera_control_->put_Focus(settings->focus_distance, CameraControl_Flags_Manual);
    ...
    if (exposure_mode_manual_ && settings->has_exposure_time) {
        ...
        hr = camera_control_->put_Exposure(CaptureExposureTimeToPlatformValue(settings->exposure_time), CameraControl_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Exposure Time config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_brightness) {
        hr = video_control_->put_Brightness(settings->brightness, VideoProcAmp_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Brightness config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_contrast) {
        hr = video_control_->put_Contrast(settings->contrast, VideoProcAmp_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Contrast config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_saturation) {
        hr = video_control_->put_Saturation(settings->saturation, VideoProcAmp_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Saturation config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_sharpness) {
        hr = video_control_->put_Sharpness(settings->sharpness, VideoProcAmp_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Sharpness config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_pan) {
        hr = camera_control_->put_Pan(CaptureAngleToPlatformValue(settings->pan), CameraControl_Flags_Manual);
        DLOG_IF_FAILED_WITH_HRESULT("Pan config failed", hr);
        if (FAILED(hr))
            return;
    }
    if (settings->has_tilt) {
        hr = camera_control_->put_Tilt(CaptureAngleToPlatformValue(settings->tilt), CameraControl_Flags_Manual);
...
// static
bool VideoCaptureDeviceWin::GetCameraAndVideoControls(ComPtr<IBaseFilter> capture_filter, ICameraControl** camera_control, IVideoProcAmp** video_control)
{
...
void VideoCaptureDeviceWin::FrameReceived(const uint8_t* buffer, int length, const VideoCaptureFormat& format, base::TimeDelta timestamp, bool flip_y)
{
...

    {
        ...
        client_->OnIncomingCapturedData(
            buffer, length, format, gfx::ColorSpace(), camera_rotation_.value(), flip_y, base::TimeTicks::Now(), timestamp, std::nullopt);
    }
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    GetPinCapabilityList(capture_filter_, output_capture_pin_, true /* query_detailed_frame_rates */, &capabilities_);
    return !capabilities_.empty();
}

// Set the power line frequency removal in |capture_filter_| if available.
void VideoCaptureDeviceWin::SetAntiFlickerInCaptureFilter(const VideoCaptureParams& params)
{
    const PowerLineFrequency power_line_frequency = GetPowerLineFrequency(params);
    if (power_line_frequency != PowerLineFrequency::k50Hz && power_line_frequency != PowerLineFrequency::k60Hz) {
        return;
    }
    ...
    if (SUCCEEDED(hr = capture_filter_.As(&ks_propset))
        && SUCCEEDED(hr = ks_propset->QuerySupported(PROPSETID_VIDCAP_VIDEOPROCAMP, KSPROPERTY_VIDEOPROCAMP_POWERLINE_FREQUENCY, &type_support))
        ...
        hr = ks_propset->Set(PROPSETID_VIDCAP_VIDEOPROCAMP, KSPROPERTY_VIDEOPROCAMP_POWERLINE_FREQUENCY, &data, sizeof(data), &data, sizeof(data));
        DLOG_IF_FAILED_WITH_HRESULT("Anti-flicker setting failed", hr);
    }
}

void VideoCaptureDeviceWin::SetErrorState(media::VideoCaptureError error, const base::Location& from_here, const std::string& reason, HRESULT hr)
{
    DCHECK(thread_checker_.CalledOnValidThread());
