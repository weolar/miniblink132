...
void SinkInputPin::SetRequestedMediaFormat(VideoPixelFormat pixel_format, float frame_rate, const BITMAPINFOHEADER& info_header)
{
...
bool SinkInputPin::IsMediaTypeValid(const AM_MEDIA_TYPE* media_type)
{
    ...
    resulting_format_.frame_size.SetSize(pvi->bmiHeader.biWidth, abs(pvi->bmiHeader.biHeight));
    ...
    DVLOG(2) << __func__ << " unsupported media type: " << base::win::WStringFromGUID(sub_type);
    return false;
}
...
HRESULT SinkInputPin::Receive(IMediaSample* sample)
{
    ...
    if (length <= 0 || static_cast<size_t>(length) < media::VideoFrame::AllocationSize(resulting_format_.pixel_format, resulting_format_.frame_size)) {
        DLOG(WARNING) << "Wrong media sample length: " << length;
        observer_->FrameDropped(VideoCaptureFrameDropReason::kWinDirectShowUnexpectedSampleLength);
        return S_FALSE;
    }

    uint8_t* buffer = nullptr;
    if (FAILED(sample->GetPointer(&buffer))) {
        observer_->FrameDropped(VideoCaptureFrameDropReason::kWinDirectShowFailedToGetMemoryPointerFromMediaSample);
        return S_FALSE;
    }
    ...
    observer_->FrameReceived(buffer, length, resulting_format_, timestamp, flip_y_);
    return S_OK;
}
