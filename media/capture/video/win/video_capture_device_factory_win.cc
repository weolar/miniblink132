...
BASE_FEATURE(kMediaFoundationD3D11VideoCaptureBlocklist, "MediaFoundationD3D11VideoCaptureBlocklist", base::FEATURE_ENABLED_BY_DEFAULT);
...
const wchar_t* kVideoAndSensorCamerasAqsString = L"(System.Devices.InterfaceClassGuid:="
...
#define UWP_ENUM_ERROR_HANDLER(hr, err_log)                                                                                                                    \
    ...
    origin_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(device_info_callback), nullptr))
...
constexpr auto kModelIdsBlockedForMediaFoundation = base::MakeFixedFlatSet<std::string_view>({ // Devices using Empia 2860 or 2820 chips, see
...
constexpr auto kModelIdsBlockedForMediaFoundationD3D11VideoCapture
    = base::MakeFixedFlatSet<std::string_view>({ // D3D11 calls on textures produced by these cameras take so much time
...
constexpr auto kDisplayNamesBlockedForMediaFoundation
    = base::MakeFixedFlatSet<std::string_view>({ // VMware Virtual Webcams cause hangs when there is no physical Webcam.
        // See https://crbug.com/1044974.
        "VMware Virtual Webcam" });

const std::vector<std::pair<VideoCaptureApi, std::vector<std::pair<GUID, GUID>>>>& GetMFAttributes()
{
    if (base::FeatureList::IsEnabled(media::kIncludeIRCamerasInDeviceEnumeration)) {
        static const base::NoDestructor<std::vector<std::pair<VideoCaptureApi, std::vector<std::pair<GUID, GUID>>>>> mf_attributes(
...
bool IsDeviceBlockedForQueryingDetailedFrameRates(const std::string& display_name)
{
...
bool IsDeviceBlockedForMediaFoundationD3D11ByModelId(const std::string& model_id)
{
    return base::FeatureList::IsEnabled(kMediaFoundationD3D11VideoCaptureBlocklist) && kModelIdsBlockedForMediaFoundationD3D11VideoCapture.contains(model_id);
}

bool IsDeviceBlockedForMediaFoundationByDisplayName(const std::string& display_name)
{
...
HMODULE ExpandEnvironmentStringsAndLoadLibrary(base::wcstring_view path)
{
    ...
    return LoadLibraryExW(expanded_path->c_str(), nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
}

bool LoadMediaFoundationDlls()
{
    static constexpr base::wcstring_view kMfDLLs[] = { L"%WINDIR%\\system32\\mf.dll", L"%WINDIR%\\system32\\mfplat.dll", L"%WINDIR%\\system32\\mfreadwrite.dll",
    ...
    ExpandEnvironmentStringsAndLoadLibrary(L"%WINDIR%\\system32\\mfsensorgroup.dll");

    return true;
}

bool PrepareVideoCaptureAttributesMediaFoundation(const std::vector<std::pair<GUID, GUID>>& attributes_data, int count, IMFAttributes** attributes)
{
    DCHECK(attributes);
    DCHECK(!*attributes);
...
{
    DCHECK_EQ(BLOCKED_CAMERA_MAX + 1, static_cast<int>(std::size(kBlockedCameraNames)));
    for (size_t i = 0; i < std::size(kBlockedCameraNames); ++i) {
        if (base::StartsWith(name, kBlockedCameraNames[i], base::CompareCase::INSENSITIVE_ASCII)) {
...
bool DevicesInfoContainsDeviceId(const DevicesInfo& devices_info, const std::string& device_id)
{
    return base::Contains(devices_info, device_id, [](const VideoCaptureDeviceInfo& device_info) { return device_info.descriptor.device_id; });
}

// Returns a non DirectShow descriptor DevicesInfo with the provided name and
// model.
DevicesInfo::const_iterator FindNonDirectShowDeviceInfoByNameAndModel(const DevicesInfo& devices_info, const std::string& name_and_model)
{
    return base::ranges::find_if(devices_info, [name_and_model](const VideoCaptureDeviceInfo& device_info) {
        return device_info.descriptor.capture_api != VideoCaptureApi::WIN_DIRECT_SHOW && name_and_model == device_info.descriptor.GetNameAndModel();
    });
}

void FindAndSetDefaultVideoCamera(std::vector<VideoCaptureDeviceInfo>* devices_info)
{
    ...
    for (auto it = devices_info->begin(); it != devices_info->end(); ++it) {
        ...
        if (it->descriptor.device_id.find(kVideoCameraGuid) != std::string::npos) {
            ...
            } else if (it->descriptor.facing == VideoFacingMode::MEDIA_VIDEO_FACING_USER) {
                std::iter_swap(devices_info->begin(), it);
            }
        }
    }
...
class VideoCaptureDeviceFactoryWin::ComThreadData : public base::RefCountedThreadSafe<ComThreadData> {
public:
    ComThreadData(base::WeakPtr<VideoCaptureDeviceFactoryWin> device_factory, scoped_refptr<base::SingleThreadTaskRunner> com_thread_runner,
    ...
    void EnumerateDevicesUWP(std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback);

    void FoundAllDevicesUWP(
        std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback, IAsyncOperation<DeviceInformationCollection*>* operation);

private:
    friend class base::RefCountedThreadSafe<ComThreadData>;
    ~ComThreadData() = default;

    std::unordered_set<raw_ptr<IAsyncOperation<DeviceInformationCollection*>, CtnExperimental>> async_ops_;
...
class VideoCaptureDeviceFactoryWin::UsageReportHandler : public base::RefCountedThreadSafe<UsageReportHandler>, public IMFSensorActivitiesReportCallback {
public:
    UsageReportHandler()
        : my_pid_(base::GetCurrentProcId())
    {
    }
    ...
    OnActivitiesReport(IMFSensorActivitiesReport* report) override
    {
        ...
        for (ULONG i = 0; i < num_reports; i++) {
            ...
            ULONG num_written;
            ULONG process_count;
            bool got_activity_report = SUCCEEDED(report->GetActivityReport(i, &activity_report))
                && SUCCEEDED(activity_report->GetSymbolicLink(symbolic_name, 1000, &num_written))
            ...
            for (ULONG j = 0; j < process_count; j++) {
                ...
                ULONG pid;
                BOOL is_streaming;
                bool got_process_info = SUCCEEDED(activity_report->GetProcessActivity(j, &process_activity)) && SUCCEEDED(process_activity->GetProcessId(&pid))
                ...
                if (pid == my_pid_) {
                    ...
                    if (is_streaming) {
                        ...
                        availability = CameraAvailability::kUnavailableExclusivelyUsedByOtherApplication;
                        break;
                    }
            ...
            if ((report_availabilities.find(device_id) == report_availabilities.end())
                || (*availability == CameraAvailability::kUnavailableExclusivelyUsedByOtherApplication)) {
    ...
    void UpdateDevicesInfoAvailability(std::vector<VideoCaptureDeviceInfo>* devices_info)
    {
        ...
        std::erase_if(availability_cache_, [&device_ids](const auto& entry) { return !base::Contains(device_ids, entry.first); });
    }
...
private:
    void UpdateAvailabilityCache(const std::map<std::string, CameraAvailability>& report_availabilities)
    {
        ...
        if (should_invoke_system_monitor) {
            if (auto* system_monitor = base::SystemMonitor::Get()) {
                system_monitor->ProcessDevicesChanged(base::SystemMonitor::DEVTYPE_VIDEO_CAPTURE);
            }
        }
    }
    ...
    std::map<std::string, media::CameraAvailability> availability_cache_ GUARDED_BY(cache_lock_);
};
...
{
    static const bool has_media_foundation = InitializeMediaFoundation() && LoadMediaFoundationDlls();
    return has_media_foundation;
}

VideoCaptureDeviceFactoryWin::VideoCaptureDeviceFactoryWin()
    : use_media_foundation_(base::FeatureList::IsEnabled(media::kMediaFoundationVideoCapture))
    , use_d3d11_with_media_foundation_(media::IsMediaFoundationD3D11VideoCaptureEnabled() && switches::IsVideoCaptureUseGpuMemoryBufferEnabled())
    , com_thread_("Windows Video Capture COM Thread")
{
    ...
    if (use_media_foundation_ && switches::IsMediaFoundationCameraUsageMonitoringEnabled()) {
...
VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryWin::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    UMA_HISTOGRAM_ENUMERATION("Media.VideoCapture.Win.DeviceFactory.CaptureApi", device_descriptor.capture_api);
    ...
    case VideoCaptureApi::WIN_MEDIA_FOUNDATION_SENSOR: {
        ...
        const bool banned_for_d3d11 = IsDeviceBlockedForMediaFoundationD3D11ByModelId(GetDeviceModelId(device_descriptor.device_id));

        MFSourceOutcome outcome = CreateDeviceSourceMediaFoundation(device_descriptor.device_id, device_descriptor.capture_api, banned_for_d3d11, &source);
        switch (outcome) {
        case MFSourceOutcome::kSuccess: {
            auto device = std::make_unique<VideoCaptureDeviceMFWin>(
                device_descriptor, std::move(source), banned_for_d3d11 ? nullptr : dxgi_device_manager_, base::SingleThreadTaskRunner::GetCurrentDefault());
            ...
            return VideoCaptureErrorOrDevice(VideoCaptureError::kWinMediaFoundationDeviceInitializationFailed);
        }
        case MFSourceOutcome::kFailedSystemPermissions:
            return VideoCaptureErrorOrDevice(VideoCaptureError::kWinMediaFoundationSystemPermissionDenied);
        case MFSourceOutcome::kFailed:
            return VideoCaptureErrorOrDevice(VideoCaptureError::kWinMediaFoundationSourceCreationFailed);
        }
        NOTREACHED();
    }
    case VideoCaptureApi::WIN_DIRECT_SHOW: {
        ComPtr<IBaseFilter> capture_filter;
        if (!CreateDeviceFilterDirectShow(device_descriptor.device_id, &capture_filter)) {
            return VideoCaptureErrorOrDevice(VideoCaptureError::kWinDirectShowDeviceFilterCreationFailed);
        }
        auto device = std::make_unique<VideoCaptureDeviceWin>(device_descriptor, std::move(capture_filter));
        DVLOG(1) << " DirectShow Device: " << device_descriptor.display_name();
        if (device->Init()) {
            LogCaptureDeviceHashedModelId(device_descriptor);
            return VideoCaptureErrorOrDevice(std::move(device));
        }
        return VideoCaptureErrorOrDevice(VideoCaptureError::kWinDirectShowDeviceInitializationFailed);
    }
    default:
        NOTREACHED();
    }
...
bool VideoCaptureDeviceFactoryWin::CreateDeviceEnumMonikerDirectShow(IEnumMoniker** enum_moniker)
{
    DCHECK(enum_moniker);
    DCHECK(!*enum_moniker);
    ...
    HRESULT hr = ::CoCreateInstance(CLSID_SystemDeviceEnum, nullptr, CLSCTX_INPROC, IID_PPV_ARGS(&dev_enum));
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to create system device enumerator: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    ...
    if (hr != S_OK) {
        DLOG(ERROR) << "Failed to create video input device enum moniker: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
...
bool VideoCaptureDeviceFactoryWin::CreateDeviceFilterDirectShow(const std::string& device_id, IBaseFilter** capture_filter)
{
    ...
    HRESULT hr = S_OK;
    for (ComPtr<IMoniker> moniker; enum_moniker->Next(1, &moniker, nullptr) == S_OK; moniker.Reset()) {
        ...
        static const wchar_t* kPropertyNames[] = { L"DevicePath", L"Description", L"FriendlyName" };
    ...
    DLOG(ERROR) << "Failed to find camera filter: " << logging::SystemErrorCodeToString(hr);
    return false;
}

bool VideoCaptureDeviceFactoryWin::CreateDeviceFilterDirectShow(ComPtr<IMoniker> moniker, IBaseFilter** capture_filter)
{
    DCHECK(capture_filter);
    DCHECK(!*capture_filter);
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to bind camera filter: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    return true;
}
...
MFSourceOutcome VideoCaptureDeviceFactoryWin::CreateDeviceSourceMediaFoundation(
    const std::string& device_id, VideoCaptureApi capture_api, const bool banned_for_d3d11, IMFMediaSource** source)
{
    DCHECK(source);
    DCHECK(!*source);
    ...
    const auto& attributes_data = capture_api == VideoCaptureApi::WIN_MEDIA_FOUNDATION ? GetMFAttributes()[0].second : GetMFAttributes()[1].second;
    // We allocate attributes_data.size() + 1 (+1 is for sym_link below) elements
    // in attributes store.
    if (!PrepareVideoCaptureAttributesMediaFoundation(attributes_data, attributes_data.size() + 1, &attributes)) {
        return MFSourceOutcome::kFailed;
    }

    attributes->SetString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, base::SysUTF8ToWide(device_id).c_str());

    return CreateDeviceSourceMediaFoundation(std::move(attributes), banned_for_d3d11, source);
}

MFSourceOutcome VideoCaptureDeviceFactoryWin::CreateDeviceSourceMediaFoundation(
    ComPtr<IMFAttributes> attributes, const bool banned_for_d3d11, IMFMediaSource** source_out)
{
    ComPtr<IMFMediaSource> source;
    HRESULT hr = MFCreateDeviceSource(attributes.Get(), &source);
    DLOG_IF(ERROR, FAILED(hr)) << "MFCreateDeviceSource failed: " << logging::SystemErrorCodeToString(hr);
    if (hr == E_ACCESSDENIED)
        return MFSourceOutcome::kFailedSystemPermissions;

    if (SUCCEEDED(hr) && use_d3d11_with_media_foundation_ && dxgi_device_manager_ && !banned_for_d3d11) {
...
bool VideoCaptureDeviceFactoryWin::EnumerateDeviceSourcesMediaFoundation(
    Microsoft::WRL::ComPtr<IMFAttributes> attributes, IMFActivate*** devices, UINT32* count)
{
    HRESULT hr = MFEnumDeviceSources(attributes.Get(), devices, count);
    DLOG_IF(ERROR, FAILED(hr)) << "MFEnumDeviceSources failed: " << logging::SystemErrorCodeToString(hr);
    return SUCCEEDED(hr);
}

void VideoCaptureDeviceFactoryWin::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    com_thread_.Start();
    com_thread_data_ = base::MakeRefCounted<VideoCaptureDeviceFactoryWin::ComThreadData>(
        weak_ptr_factory_.GetWeakPtr(), com_thread_.task_runner(), base::SingleThreadTaskRunner::GetCurrentDefault());
    com_thread_.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&VideoCaptureDeviceFactoryWin::ComThreadData::EnumerateDevicesUWP, com_thread_data_, std::move(devices_info), std::move(callback)));
}

void VideoCaptureDeviceFactoryWin::ComThreadData::EnumerateDevicesUWP(std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback)
{
    ...
    auto device_info_callback = base::BindOnce(&VideoCaptureDeviceFactoryWin::ComThreadData::FoundAllDevicesUWP, scoped_refptr<ComThreadData>(this),
        std::move(devices_info), std::move(result_callback));

    ComPtr<ABI::Windows::Devices::Enumeration::IDeviceInformationStatics> dev_info_statics;
    ...
    SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY();
    HRESULT hr
        = GetActivationFactory<ABI::Windows::Devices::Enumeration::IDeviceInformationStatics, RuntimeClass_Windows_Devices_Enumeration_DeviceInformation>(
            &dev_info_statics);
    if (FAILED(hr)) {
    ...
    ScopedHString aqs_filter = ScopedHString::Create(kVideoAndSensorCamerasAqsString);
    ...
    auto callback = Microsoft::WRL::Callback<ABI::Windows::Foundation::IAsyncOperationCompletedHandler<DeviceInformationCollection*>>(
        [com_thread_runner = com_thread_runner_, device_info_callback = std::move(device_info_callback)](
            IAsyncOperation<DeviceInformationCollection*>* operation, AsyncStatus status) mutable -> HRESULT {
            com_thread_runner->PostTask(FROM_HERE, base::BindOnce(std::move(device_info_callback), base::Unretained(operation)));
            return S_OK;
        });

    hr = async_op->put_Completed(callback.Get());
    if (FAILED(hr)) {
        DLOG(WARNING) << "Register async operation callback failed: " << logging::SystemErrorCodeToString(hr);
    ...
    std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback, IAsyncOperation<DeviceInformationCollection*>* operation)
{
    if (!operation) {
        origin_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&VideoCaptureDeviceFactoryWin::DeviceInfoReady, device_factory_, std::move(devices_info), std::move(result_callback)));
        return;
    }

    ComPtr<ABI::Windows::Foundation::Collections::IVectorView<ABI::Windows::Devices::Enumeration::DeviceInformation*>> devices;
    ...
    for (unsigned int j = 0; j < count; ++j) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            ComPtr<ABI::Windows::Devices::Enumeration::IEnclosureLocation> enclosure_location;
...
void VideoCaptureDeviceFactoryWin::UpdateDevicesInfoAvailability(std::vector<VideoCaptureDeviceInfo>* devices_info)
{
    if (report_handler_) {
...
void VideoCaptureDeviceFactoryWin::DeviceInfoReady(std::vector<VideoCaptureDeviceInfo> devices_info, GetDevicesInfoCallback result_callback)
{
    if (com_thread_.IsRunning()) {
        com_thread_.Stop();
...
DevicesInfo VideoCaptureDeviceFactoryWin::GetDevicesInfoMediaFoundation()
{
    ...
    for (const auto& api_attributes : GetMFAttributes()) {
        ComPtr<IMFAttributes> attributes;
        if (!PrepareVideoCaptureAttributesMediaFoundation(api_attributes.second, api_attributes.second.size(), &attributes)) {
            return {};
        }
        ScopedCoMem<IMFActivate*> devices;
        UINT32 count;
        if (!EnumerateDeviceSourcesMediaFoundation(std::move(attributes), &devices, &count)) {
            return {};
        }
        ...
        for (UINT32 i = 0; i < count; ++i) {
            ...
            UINT32 name_size;
            HRESULT hr = devices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME, &name, &name_size);
            if (SUCCEEDED(hr)) {
                ScopedCoMem<wchar_t> id;
                UINT32 id_size;
                hr = devices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, &id, &id_size);
                if (SUCCEEDED(hr)) {
                    const std::string device_id = base::SysWideToUTF8(std::wstring(id, id_size));
                    const std::string model_id = GetDeviceModelId(device_id);
                    const std::string display_name = base::SysWideToUTF8(std::wstring(name, name_size));
                    if (IsDeviceBlockedForMediaFoundationByModelId(model_id) || IsDeviceBlockedForMediaFoundationByDisplayName(display_name)) {
                        continue;
                    }
                    if (list_was_empty || !DevicesInfoContainsDeviceId(devices_info, device_id)) {
                        ComPtr<IMFMediaSource> source;
                        VideoCaptureControlSupport control_support;
                        VideoCaptureFormats supported_formats;
                        const bool banned_for_d3d11 = IsDeviceBlockedForMediaFoundationD3D11ByModelId(model_id);
                        if (CreateDeviceSourceMediaFoundation(device_id, api_attributes.first, banned_for_d3d11, &source) == MFSourceOutcome::kSuccess) {
                            control_support = VideoCaptureDeviceMFWin::GetControlSupport(source);
                            supported_formats = GetSupportedFormatsMediaFoundation(source, banned_for_d3d11, display_name);
                        }
                        devices_info.emplace_back(VideoCaptureDeviceDescriptor(display_name, device_id, model_id, api_attributes.first, control_support));
                        devices_info.back().supported_formats = std::move(supported_formats);
                    }
                }
            }
            DLOG_IF(ERROR, FAILED(hr)) << "GetAllocatedString failed: " << logging::SystemErrorCodeToString(hr);
            devices[i]->Release();
        }
    }
...
void VideoCaptureDeviceFactoryWin::AugmentDevicesListWithDirectShowOnlyDevices(DevicesInfo* devices_info)
{
    ...
    DevicesInfo direct_show_devices_info = GetDevicesInfoDirectShow(*devices_info);
    for (const auto& direct_show_device_info : direct_show_devices_info) {
        ...
        DevicesInfo::const_iterator matching_non_direct_show_device
            = FindNonDirectShowDeviceInfoByNameAndModel(*devices_info, direct_show_device_info.descriptor.GetNameAndModel());
...
DevicesInfo VideoCaptureDeviceFactoryWin::GetDevicesInfoDirectShow(const DevicesInfo& known_devices)
{
    DVLOG(1) << __func__;
    ...
    for (ComPtr<IMoniker> moniker; enum_moniker->Next(1, &moniker, nullptr) == S_OK; moniker.Reset()) {
        ...
        auto device_descriptor = VideoCaptureDeviceDescriptor(device_name, id, model_id, VideoCaptureApi::WIN_DIRECT_SHOW, VideoCaptureControlSupport());

        DevicesInfo::const_iterator matching_non_direct_show_device
            = FindNonDirectShowDeviceInfoByNameAndModel(known_devices, device_descriptor.GetNameAndModel());
        ...
        if (matching_non_direct_show_device != known_devices.end() && matching_non_direct_show_device->supported_formats.size() > 0) {
            continue;
        }
        ...
        if (CreateDeviceFilterDirectShow(std::move(moniker), &capture_filter)) {
            control_support = VideoCaptureDeviceWin::GetControlSupport(capture_filter);
            supported_formats = GetSupportedFormatsDirectShow(capture_filter, device_name);
        }
...
VideoCaptureFormats VideoCaptureDeviceFactoryWin::GetSupportedFormatsDirectShow(ComPtr<IBaseFilter> capture_filter, const std::string& display_name)
{
    VideoCaptureFormats formats;
    bool query_detailed_frame_rates = !IsDeviceBlockedForQueryingDetailedFrameRates(display_name);
    CapabilityList capability_list;
    VideoCaptureDeviceWin::GetDeviceCapabilityList(capture_filter, query_detailed_frame_rates, &capability_list);
    for (const auto& entry : capability_list) {
        formats.emplace_back(entry.supported_format);
        DVLOG(1) << display_name << " " << VideoCaptureFormat::ToString(entry.supported_format);
    }
    return formats;
}

VideoCaptureFormats VideoCaptureDeviceFactoryWin::GetSupportedFormatsMediaFoundation(
    ComPtr<IMFMediaSource> source, const bool banned_for_d3d11, const std::string& display_name)
{
    ComPtr<IMFAttributes> source_reader_attributes;
    const bool dxgi_device_manager_available = (dxgi_device_manager_ != nullptr) && !banned_for_d3d11;
    if (dxgi_device_manager_available) {
        dxgi_device_manager_->RegisterWithMediaSource(source);

        HRESULT hr = MFCreateAttributes(&source_reader_attributes, 1);
        if (SUCCEEDED(hr)) {
            dxgi_device_manager_->RegisterInSourceReaderAttributes(source_reader_attributes.Get());
        } else {
            DLOG(ERROR) << "MFCreateAttributes failed: " << logging::SystemErrorCodeToString(hr);
        }
    }

    ComPtr<IMFSourceReader> reader;
    HRESULT hr = MFCreateSourceReaderFromMediaSource(source.Get(), source_reader_attributes.Get(), &reader);
    if (FAILED(hr)) {
        DLOG(ERROR) << "MFCreateSourceReaderFromMediaSource failed: " << logging::SystemErrorCodeToString(hr);
        return {};
    }
    ...
    while (SUCCEEDED(hr = reader->GetNativeMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM), stream_index, &type))) {
        UINT32 width, height;
        hr = MFGetAttributeSize(type.Get(), MF_MT_FRAME_SIZE, &width, &height);
        if (FAILED(hr)) {
            DLOG(ERROR) << "MFGetAttributeSize failed: " << logging::SystemErrorCodeToString(hr);
            return {};
        }
        ...
        hr = MFGetAttributeRatio(type.Get(), MF_MT_FRAME_RATE, &numerator, &denominator);
        if (FAILED(hr)) {
            DLOG(ERROR) << "MFGetAttributeSize failed: " << logging::SystemErrorCodeToString(hr);
            return {};
        }
        ...
        VideoCaptureDeviceMFWin::GetPixelFormatFromMFSourceMediaSubtype(
            type_guid, /*use_hardware_format=*/dxgi_device_manager_available, &capture_format.pixel_format);
        type.Reset();
        ++stream_index;
        ...
        DVLOG(1) << display_name << " " << VideoCaptureFormat::ToString(capture_format);
    }

    return formats;
}

scoped_refptr<DXGIDeviceManager> VideoCaptureDeviceFactoryWin::GetDxgiDeviceManager()
{
...
{
    scoped_refptr<UsageReportHandler> report_handler = base::MakeRefCounted<UsageReportHandler>();
