...
class PinBase : public IPin, public IMemInputPin, public base::RefCountedThreadSafe<PinBase> {
public:
    ...
    IFACEMETHODIMP Connect(IPin* receive_pin, const AM_MEDIA_TYPE* media_type) override;

    IFACEMETHODIMP ReceiveConnection(IPin* connector, const AM_MEDIA_TYPE* media_type) override;
    ...
    IFACEMETHODIMP NewSegment(REFERENCE_TIME start, REFERENCE_TIME stop, double dRate) override;

    // Inherited from IMemInputPin.
    IFACEMETHODIMP GetAllocator(IMemAllocator** allocator) override;

    IFACEMETHODIMP NotifyAllocator(IMemAllocator* allocator, BOOL read_only) override;

    IFACEMETHODIMP GetAllocatorRequirements(ALLOCATOR_PROPERTIES* properties) override;

    IFACEMETHODIMP ReceiveMultiple(IMediaSample** samples, long sample_count, long* processed) override;
