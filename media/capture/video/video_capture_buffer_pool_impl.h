...
class CAPTURE_EXPORT VideoCaptureBufferPoolImpl : public VideoCaptureBufferPool {
public:
    ...
    VideoCaptureBufferPoolImpl(VideoCaptureBufferType buffer_type, int count, std::unique_ptr<VideoCaptureBufferTrackerFactory> buffer_tracker_factory);

    VideoCaptureBufferPoolImpl(const VideoCaptureBufferPoolImpl&) = delete;
    VideoCaptureBufferPoolImpl& operator=(const VideoCaptureBufferPoolImpl&) = delete;

    // VideoCaptureBufferPool implementation.
    base::UnsafeSharedMemoryRegion DuplicateAsUnsafeRegion(int buffer_id) override;
    std::unique_ptr<VideoCaptureBufferHandle> GetHandleForInProcessAccess(int buffer_id) override;
    ...
    VideoCaptureDevice::Client::ReserveResult ReserveForProducer(const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides,
        int frame_feedback_id, int* buffer_id, int* buffer_id_to_drop) override;
    ...
    VideoCaptureDevice::Client::ReserveResult ReserveIdForExternalBuffer(
        CapturedExternalVideoBuffer buffer, const gfx::Size& dimensions, int* buffer_id_to_drop, int* buffer_id) override;
...
private:
    ...
    VideoCaptureDevice::Client::ReserveResult ReserveForProducerInternal(const gfx::Size& dimensions, VideoPixelFormat format,
        const mojom::PlaneStridesPtr& strides, int frame_feedback_id, int* buffer_id, int* tracker_id_to_drop);

    VideoCaptureBufferTracker* GetTracker(int buffer_id) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::map<int, std::unique_ptr<VideoCaptureBufferTracker>> trackers_ GUARDED_BY(lock_);

    const std::unique_ptr<VideoCaptureBufferTrackerFactory> buffer_tracker_factory_ GUARDED_BY(lock_);
};
