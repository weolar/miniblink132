...
// circulate.
class CAPTURE_EXPORT VideoCaptureBufferPool : public base::RefCountedThreadSafe<VideoCaptureBufferPool> {
public:
    ...
    // create regions.
    virtual base::UnsafeSharedMemoryRegion DuplicateAsUnsafeRegion(int buffer_id) = 0;

    // Try and obtain a read/write access to the buffer.
    virtual std::unique_ptr<VideoCaptureBufferHandle> GetHandleForInProcessAccess(int buffer_id) = 0;

    virtual gfx::GpuMemoryBufferHandle GetGpuMemoryBufferHandle(int buffer_id) = 0;
    ...
    virtual VideoCaptureDevice::Client::ReserveResult ReserveForProducer(const gfx::Size& dimensions, VideoPixelFormat format,
