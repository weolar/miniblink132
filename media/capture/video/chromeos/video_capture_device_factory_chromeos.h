...
class CAPTURE_EXPORT VideoCaptureDeviceFactoryChromeOS final : public VideoCaptureDeviceFactory {
public:
    explicit VideoCaptureDeviceFactoryChromeOS(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner);

    VideoCaptureDeviceFactoryChromeOS(const VideoCaptureDeviceFactoryChromeOS&) = delete;
    VideoCaptureDeviceFactoryChromeOS& operator=(const VideoCaptureDeviceFactoryChromeOS&) = delete;

    ~VideoCaptureDeviceFactoryChromeOS() override;

    // VideoCaptureDeviceFactory interface implementations.
    VideoCaptureErrorOrDevice CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor) final;
    ...
    static void SetGpuChannelHost(scoped_refptr<gpu::GpuChannelHost> gpu_channel_host);
    ...
    static void SetSharedImageInterface(scoped_refptr<gpu::SharedImageInterface> shared_image_interface);
...
private:
    ...
    base::WeakPtrFactory<VideoCaptureDeviceFactoryChromeOS> weak_ptr_factory_ { this };
};
