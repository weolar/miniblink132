...
// components.
class CAPTURE_EXPORT CameraAppDeviceBridgeImpl : public cros::mojom::CameraAppDeviceBridge {
public:
    using CameraInfoGetter = base::RepeatingCallback<cros::mojom::CameraInfoPtr(const std::string&)>;
    using VirtualDeviceController = base::RepeatingCallback<void(const std::string&, bool)>;

    CameraAppDeviceBridgeImpl();

    CameraAppDeviceBridgeImpl(const CameraAppDeviceBridgeImpl&) = delete;
    CameraAppDeviceBridgeImpl& operator=(const CameraAppDeviceBridgeImpl&) = delete;

    ~CameraAppDeviceBridgeImpl() override;

    static CameraAppDeviceBridgeImpl* GetInstance();

    void BindReceiver(mojo::PendingReceiver<cros::mojom::CameraAppDeviceBridge> receiver);

    void OnVideoCaptureDeviceCreated(const std::string& device_id, scoped_refptr<base::SingleThreadTaskRunner> vcd_task_runner);
    ...
    void InvalidateDevicePtrsOnDeviceIpcThread(const std::string& device_id, bool should_disable_new_ptrs, base::OnceClosure callback);

    void SetCameraInfoGetter(CameraInfoGetter camera_info_getter);

    void UnsetCameraInfoGetter();

    void SetVirtualDeviceController(VirtualDeviceController virtual_device_controller);

    void UnsetVirtualDeviceController();

    base::WeakPtr<CameraAppDeviceImpl> GetWeakCameraAppDevice(const std::string& device_id);

    void RemoveCameraAppDevice(const std::string& device_id);

    void RemoveVCDTaskRunner(const std::string& device_id);

    void SetUITaskRunner(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner);

    // cros::mojom::CameraAppDeviceBridge implementations.
    void GetCameraAppDevice(const std::string& device_id, GetCameraAppDeviceCallback callback) override;

    void IsSupported(IsSupportedCallback callback) override;

    void SetVirtualDeviceEnabled(const std::string& device_id, bool enabled, SetVirtualDeviceEnabledCallback callback) override;

    void SetDeviceInUse(const std::string& device_id, bool in_use);

    void IsDeviceInUse(const std::string& device_id, IsDeviceInUseCallback callback) override;
...
private:
    ...
    void BindReceiverOnIPCThread(mojo::PendingReceiver<cros::mojom::CameraAppDeviceBridge> receiver);
    ...
    VirtualDeviceController virtual_device_controller_ GUARDED_BY(virtual_device_controller_lock_);

    mojo::ReceiverSet<cros::mojom::CameraAppDeviceBridge> receivers_;

    base::Lock device_map_lock_;
    base::flat_map<std::string, std::unique_ptr<media::CameraAppDeviceImpl>> camera_app_devices_ GUARDED_BY(device_map_lock_);

    base::Lock task_runner_map_lock_;
    base::flat_map<std::string, scoped_refptr<base::SingleThreadTaskRunner>> vcd_task_runners_ GUARDED_BY(task_runner_map_lock_);
