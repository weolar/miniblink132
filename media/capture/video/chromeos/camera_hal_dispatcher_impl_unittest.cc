...
public:
    ...
    MOCK_METHOD2(GetCameraModule, void(cros::mojom::CameraClientType camera_client_type, GetCameraModuleCallback callback));

    MOCK_METHOD1(SetTracingEnabled, void(bool enabled));
    MOCK_METHOD1(SetAutoFramingState, void(cros::mojom::CameraAutoFramingState state));
    MOCK_METHOD1(GetCameraSWPrivacySwitchState, void(cros::mojom::CrosCameraService::GetCameraSWPrivacySwitchStateCallback callback));
    MOCK_METHOD1(SetCameraSWPrivacySwitchState, void(cros::mojom::CameraPrivacySwitchState state));
    MOCK_METHOD1(GetAutoFramingSupported, void(GetAutoFramingSupportedCallback callback));
    MOCK_METHOD2(SetCameraEffect, void(::cros::mojom::EffectsConfigPtr config, SetCameraEffectCallback callback));
    MOCK_METHOD1(AddCrosCameraServiceObserver, void(mojo::PendingRemote<cros::mojom::CrosCameraServiceObserver> observer));
...
private:
    ...
    void StartKioskVisionDetection(const std::string& dlc_path, mojo::PendingRemote<cros::mojom::KioskVisionObserver> observer) override
    {
    }
...
public:
    ...
    MOCK_METHOD1(DoSetUpChannel, void(mojo::PendingRemote<cros::mojom::CameraModule> camera_module));
...
public:
    void OnActiveClientChange(cros::mojom::CameraClientType type, bool is_active, const base::flat_set<std::string>& device_ids) override
    {
        DoOnActiveClientChange(type, is_active, device_ids);
    }
    MOCK_METHOD3(DoOnActiveClientChange, void(cros::mojom::CameraClientType, bool, const base::flat_set<std::string>&));
};
...
public:
    ...
    // effects.
    void OnCameraEffectChanged(const cros::mojom::EffectsConfigPtr& new_effects) override
    {
...
public:
    ...
    CameraHalDispatcherImplTest& operator=(const CameraHalDispatcherImplTest&) = delete;
    ...
    {
        delete dispatcher_.ExtractAsDangling();
    }
    ...
    static void BindCameraService(CameraHalDispatcherImpl* dispatcher, mojo::PendingRemote<cros::mojom::CrosCameraService> camera_service)
    {
        dispatcher->BindCameraServiceOnProxyThread(std::move(camera_service));
    }

    static void RegisterClientWithToken(CameraHalDispatcherImpl* dispatcher, mojo::PendingRemote<cros::mojom::CameraHalClient> client,
        cros::mojom::CameraClientType type, const base::UnguessableToken& token, cros::mojom::CameraHalDispatcher::RegisterClientWithTokenCallback callback)
    {
        dispatcher->RegisterClientWithToken(std::move(client), type, token, std::move(callback));
    }
    ...
    static void SetCameraEffectsComplete(
        CameraHalDispatcherImpl* dispatcher, cros::mojom::EffectsConfigPtr config, bool is_from_register, cros::mojom::SetEffectResult result)
    {
        dispatcher->OnSetCameraEffectsCompleteOnProxyThread(std::move(config), is_from_register, result);
    }
...
TEST_F(CameraHalDispatcherImplTest, ClientConnectionError)
{
    ...
    EXPECT_CALL(*mock_service, GetCameraModule(_, _))
        .Times(1)
        .WillOnce([this](cros::mojom::CameraClientType camera_client_type, MockCrosCameraService::GetCameraModuleCallback callback) {
            mojo::PendingReceiver<cros::mojom::CameraModule> receiver;
            std::move(callback).Run(receiver.InitWithNewPipeAndPassRemote());
            this->QuitRunLoop();
        });
    EXPECT_CALL(*mock_client, DoSetUpChannel(_)).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    EXPECT_CALL(*mock_service, SetAutoFramingState(_)).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    EXPECT_CALL(*mock_service, SetCameraEffect(_, _))
        .Times(1)
        .WillOnce([this](::cros::mojom::EffectsConfigPtr, MockCrosCameraService::SetCameraEffectCallback callback) {
            std::move(callback).Run(::cros::mojom::SetEffectResult::kOk);
            this->QuitRunLoop();
        });
    EXPECT_CALL(*mock_service, AddCrosCameraServiceObserver(_)).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));

    auto service = mock_service->GetPendingRemote();
    GetProxyTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&CameraHalDispatcherImplTest::BindCameraService, base::Unretained(dispatcher_), std::move(service)));
    auto client = mock_client->GetPendingRemote();
    auto type = cros::mojom::CameraClientType::TESTING;
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::RegisterClientWithToken, base::Unretained(dispatcher_), std::move(client), type,
            dispatcher_->GetTokenForTrustedClient(type), base::BindOnce(&CameraHalDispatcherImplTest::OnRegisteredClient, base::Unretained(this))));
...
TEST_F(CameraHalDispatcherImplTest, RegisterClientFail)
{
    ...
    auto client = mock_client->GetPendingRemote();
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::RegisterClientWithToken, base::Unretained(dispatcher_), std::move(client),
            cros::mojom::CameraClientType::TESTING, empty_token, base::BindOnce(&CameraHalDispatcherImplTest::OnRegisteredClient, base::Unretained(this))));
...
TEST_F(CameraHalDispatcherImplTest, CameraActiveClientObserverTest)
{
    ...
    CreateLoop(1);
    EXPECT_CALL(observer, DoOnActiveClientChange(cros::mojom::CameraClientType::TESTING, true, base::flat_set<std::string>({ "0" })))
        .Times(1)
        .WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    dispatcher_->CameraDeviceActivityChange(
        /*camera_id=*/0, /*opened=*/true, cros::mojom::CameraClientType::TESTING);
    RunLoop();

    CreateLoop(1);
    EXPECT_CALL(observer, DoOnActiveClientChange(cros::mojom::CameraClientType::TESTING, false, base::flat_set<std::string>()))
        .Times(1)
        .WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    dispatcher_->CameraDeviceActivityChange(
        /*camera_id=*/0, /*opened=*/false, cros::mojom::CameraClientType::TESTING);
    RunLoop();
}
...
TEST_F(CameraHalDispatcherImplTest, CameraEffectObserver)
{
    ...
    cros::mojom::EffectsConfigPtr config = GetDefaultCameraEffectsConfigForTesting();
    // Set effects for the first time.
    CreateLoop(1);
    EXPECT_CALL(observer, DoOnCameraEffectChanged()).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::SetCameraEffectsComplete, base::Unretained(dispatcher_), config.Clone(),
            /*is_from_register=*/true, cros::mojom::SetEffectResult::kOk));
    RunLoop();
    EXPECT_EQ(observer.new_effects(), config);

    cros::mojom::EffectsConfigPtr new_config = GetDefaultCameraEffectsConfigForTesting();
    ...
    EXPECT_CALL(observer, DoOnCameraEffectChanged()).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::SetCameraEffectsComplete, base::Unretained(dispatcher_), new_config.Clone(),
            /*is_from_register=*/true, cros::mojom::SetEffectResult::kError));
    RunLoop();
    ...
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::SetCameraEffectsComplete, base::Unretained(dispatcher_), new_config.Clone(),
            /*is_from_register=*/false, cros::mojom::SetEffectResult::kError));
    RunLoop();
    EXPECT_EQ(observer.new_effects(), cros::mojom::EffectsConfig::New());
    ...
    GetProxyTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CameraHalDispatcherImplTest::SetCameraEffectsComplete, base::Unretained(dispatcher_), new_config.Clone(),
            /*is_from_register=*/false, cros::mojom::SetEffectResult::kOk));
    RunLoop();
...
TEST_F(CameraHalDispatcherImplTest, SetCameraEffects)
{
    ...
    EXPECT_CALL(*mock_service, SetCameraEffect(_, _))
        .Times(1)
        .WillOnce([this](::cros::mojom::EffectsConfigPtr config, MockCrosCameraService::SetCameraEffectCallback callback) {
            std::move(callback).Run(::cros::mojom::SetEffectResult::kOk);
            this->QuitRunLoop();
        });
    EXPECT_CALL(observer, DoOnCameraEffectChanged()).Times(1).WillOnce(InvokeWithoutArgs(this, &CameraHalDispatcherImplTest::QuitRunLoop));
