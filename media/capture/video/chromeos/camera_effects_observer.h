...
// manager lives.
class CAPTURE_EXPORT CrosCameraEffectsObserver : public cros::mojom::CrosCameraServiceObserver {
public:
    using OnCameraEffectsChangedCallback = base::RepeatingCallback<void(cros::mojom::EffectsConfigPtr)>;

    // When the camera effect changes, |on_camera_effects_changed_callback| will
    // be invoked on the ui thread.
    explicit CrosCameraEffectsObserver(OnCameraEffectsChangedCallback on_camera_effects_changed_callback);

    ~CrosCameraEffectsObserver() override;

    CrosCameraEffectsObserver(const CrosCameraEffectsObserver&) = delete;
    CrosCameraEffectsObserver& operator=(const CrosCameraEffectsObserver&) = delete;
