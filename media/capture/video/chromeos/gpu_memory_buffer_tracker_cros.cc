...
GpuMemoryBufferTrackerCros::GpuMemoryBufferTrackerCros()
    : buffer_(nullptr)
{
}
...
bool GpuMemoryBufferTrackerCros::Init(const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides)
{
    ...
    const gfx::BufferUsage usage
        = *gfx_format == gfx::BufferFormat::R_8 ? gfx::BufferUsage::CAMERA_AND_CPU_READ_WRITE : gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE;

    buffer_ = buffer_factory_.CreateGpuMemoryBuffer(dimensions, *gfx_format, usage);
    if (!buffer_) {
...
bool GpuMemoryBufferTrackerCros::IsReusableForFormat(const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides)
{
    std::optional<gfx::BufferFormat> gfx_format = PixFormatVideoToGfx(format);
    if (!gfx_format) {
        return false;
    }
    return (*gfx_format == buffer_->GetFormat() && dimensions == buffer_->GetSize());
}

std::unique_ptr<VideoCaptureBufferHandle> GpuMemoryBufferTrackerCros::GetMemoryMappedAccess()
{
    NOTREACHED() << "Unsupported operation";
}

base::UnsafeSharedMemoryRegion GpuMemoryBufferTrackerCros::DuplicateAsUnsafeRegion()
{
    NOTREACHED() << "Unsupported operation";
}

gfx::GpuMemoryBufferHandle GpuMemoryBufferTrackerCros::GetGpuMemoryBufferHandle()
{
    DCHECK(buffer_);
