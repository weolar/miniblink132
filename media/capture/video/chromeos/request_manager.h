...
public:
    ...
    void OnNewBuffer(ClientType client_type, cros::mojom::CameraBufferHandlePtr buffer);
...
class CAPTURE_EXPORT RequestManager final : public cros::mojom::Camera3CallbackOps, public CaptureMetadataDispatcher {
public:
    using BlobifyCallback = base::RepeatingCallback<mojom::BlobPtr(
        const uint8_t* buffer, const uint32_t bytesused, const VideoCaptureFormat& capture_format, int screen_rotation)>;
    ...
    RequestManager(const std::string& device_id, mojo::PendingReceiver<cros::mojom::Camera3CallbackOps> callback_ops_receiver,
        ...
        std::unique_ptr<CameraBufferFactory> camera_buffer_factory, BlobifyCallback blobify_callback,
        scoped_refptr<base::SingleThreadTaskRunner> ipc_task_runner, uint32_t device_api_version, bool use_buffer_management_apis);
    ...
    void SetUpStreamsAndBuffers(base::flat_map<ClientType, VideoCaptureParams> capture_params, const cros::mojom::CameraMetadataPtr& static_metadata,
    ...
    void TakePortraitPhoto(cros::mojom::CameraMetadataPtr settings, TakePhotoCallbackMap callbacks_map);
    ...
    // capture request.
    void SetCaptureMetadata(cros::mojom::CameraMetadataTag tag, cros::mojom::EntryType type, size_t count, std::vector<uint8_t> value) override;

    void SetRepeatingCaptureMetadata(cros::mojom::CameraMetadataTag tag, cros::mojom::EntryType type, size_t count, std::vector<uint8_t> value) override;

    void UnsetRepeatingCaptureMetadata(cros::mojom::CameraMetadataTag tag) override;
...
private:
    ...
    void SetJpegOrientation(cros::mojom::CameraMetadataPtr* settings, int32_t orientation);
    ...
    void SetZeroShutterLag(cros::mojom::CameraMetadataPtr* settings, bool enabled);
    ...
    bool TryPreparePortraitModeRequest(std::set<StreamType>* stream_types, cros::mojom::CameraMetadataPtr* settings, TakePhotoCallbackMap* callbacks_map);

    bool TryPreparePreviewRequest(std::set<StreamType>* stream_types, cros::mojom::CameraMetadataPtr* settings);
    ...
    void ProcessCaptureResult(cros::mojom::Camera3CaptureResultPtr result) override;
    ...
    void HandleNotifyError(uint32_t frame_number, StreamType stream_type, cros::mojom::Camera3ErrorMsgCode error_code);

    // RequestStreamBuffers receives output buffer requests and a callback to
    // receive results.
    void RequestStreamBuffers(std::vector<cros::mojom::Camera3BufferRequestPtr> buffer_reqs, RequestStreamBuffersCallback callback) override;

    // ReturnStreamBuffers receives returned output buffers.
    void ReturnStreamBuffers(std::vector<cros::mojom::Camera3StreamBufferPtr> buffers) override;
    ...
    void SubmitCaptureResult(uint32_t frame_number, StreamType stream_type, cros::mojom::Camera3StreamBufferPtr stream_buffer);
    void SubmitCapturedPreviewRecordingBuffer(uint32_t frame_number, uint64_t buffer_ipc_id, StreamType stream_type);
    void SubmitCapturedJpegBuffer(uint32_t frame_number, uint64_t buffer_ipc_id, StreamType stream_type);
    ...
    std::unordered_set<raw_ptr<ResultMetadataObserver, CtnExperimental>> result_metadata_observers_;
    ...
    std::map<cros::mojom::CameraMetadataTag, cros::mojom::CameraMetadataEntryPtr> capture_settings_repeating_override_;
