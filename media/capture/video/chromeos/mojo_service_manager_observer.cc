...
std::unique_ptr<MojoServiceManagerObserver> MojoServiceManagerObserver::Create(
    const std::string& service_name, base::RepeatingClosure on_register_callback, base::RepeatingClosure on_unregister_callback)
{
    ...
    return base::WrapUnique<MojoServiceManagerObserver>(
        new MojoServiceManagerObserver(service_name, std::move(on_register_callback), std::move(on_unregister_callback)));
}
    ...
    , on_unregister_callback_(std::move(on_unregister_callback))
{
    ...
    proxy->Query(service_name_, base::BindOnce(&MojoServiceManagerObserver::QueryCallback, weak_ptr_factory_.GetWeakPtr()));
}

MojoServiceManagerObserver::~MojoServiceManagerObserver() = default;

void MojoServiceManagerObserver::OnServiceEvent(chromeos::mojo_service_manager::mojom::ServiceEventPtr event)
{
    ...
    case chromeos::mojo_service_manager::mojom::ServiceEvent::Type::kUnRegistered:
        CAMERA_LOG(EVENT) << service_name_ << " is unregistered.";
        on_unregister_callback_.Run();
        return;

    case chromeos::mojo_service_manager::mojom::ServiceEvent::Type::kDefaultValue:
        return;
    }
}

void MojoServiceManagerObserver::QueryCallback(chromeos::mojo_service_manager::mojom::ErrorOrServiceStatePtr result)
{
    switch (result->which()) {
    case ErrorOrServiceState::Tag::kState:
        switch (result->get_state()->which()) {
        case ServiceState::Tag::kRegisteredState:
            CAMERA_LOG(EVENT) << service_name_ << " has been registered during query.";
            on_register_callback_.Run();
            break;

        case ServiceState::Tag::kUnregisteredState:
            CAMERA_LOG(EVENT) << service_name_ << " has not been registered during query.";
            break;
    ...
    case ErrorOrServiceState::Tag::kError:
        LOG(ERROR) << "Error code: " << result->get_error()->code << ", message: " << result->get_error()->message;
        break;
