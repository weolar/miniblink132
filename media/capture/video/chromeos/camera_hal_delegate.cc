...
const std::unordered_set<int32_t> module_id_set = {
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kLifeCamHD3000_Microsoft),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC270_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kHDC615_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kHDProC920_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC930e_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC925e_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC922ProStream_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kBRIOUltraHD_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC920HDPro_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kC920PROHD_Logitech),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kCam_ARC),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kLiveStreamer313_Sunplus),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kVitadeAF_Microdia),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kCam_Sonix),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kVZR_IPEVO),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::k808Camera9_Generalplus),
    static_cast<int32_t>(CameraHalDelegate::PopularCamPeriphModuleID::kNexiGoN60FHD_2MUVC),
};
...
bool IsVividLoaded()
{
    ...
    std::vector<std::string_view> lines = base::SplitStringPieceUsingSubstr(output, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);

    return base::ranges::any_of(lines, [](const auto& line) { return base::StartsWith(line, "vivid", base::CompareCase::SENSITIVE); });
}

base::flat_set<int32_t> GetAvailableFramerates(const cros::mojom::CameraInfoPtr& camera_info)
{
    base::flat_set<int32_t> candidates;
    auto available_fps_ranges = GetMetadataEntryAsSpan<int32_t>(
        camera_info->static_camera_characteristics, cros::mojom::CameraMetadataTag::ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
...
class CameraHalDelegate::SystemEventMonitorProxy : public cros::mojom::CrosLidObserver {
public:
    explicit SystemEventMonitorProxy(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
        : ui_task_runner_(std::move(ui_task_runner))
    {
        ui_task_runner_->PostTask(FROM_HERE, base::BindOnce(&SystemEventMonitorProxy::InitOnUIThread, GetWeakPtr()));
    }
    ...
    {
        ui_task_runner_->PostTask(FROM_HERE, base::BindOnce(&SystemEventMonitorProxy::NotifyVideoCaptureDevicesChangedOnUIThread, GetWeakPtr()));
    }
...
private:
    void InitOnUIThread()
    {
        ...
        ash::mojo_service_manager::GetServiceManagerProxy()->Request(
            /*service_name=*/chromeos::mojo_services::kCrosSystemEventMonitor, std::nullopt, monitor_.BindNewPipeAndPassReceiver().PassPipe());
    ...
    base::Lock lid_lock_;
    cros::mojom::LidState lid_state_ GUARDED_BY(lid_lock_) = cros::mojom::LidState::kNotPresent;
    ...
    base::WeakPtrFactory<CameraHalDelegate::SystemEventMonitorProxy> weak_ptr_factory_ { this };
};

class CameraHalDelegate::VCDInfoMonitorImpl : public cros::mojom::VideoCaptureDeviceInfoMonitor, public chromeos::mojo_service_manager::mojom::ServiceProvider {
public:
    VCDInfoMonitorImpl()
    {
        if (!ash::mojo_service_manager::IsServiceManagerBound()) {
        ...
        vcd_info_observers_.set_disconnect_handler(base::BindRepeating(&VCDInfoMonitorImpl::RemoveObserver, weak_factory_.GetWeakPtr()));
        auto* proxy = ash::mojo_service_manager::GetServiceManagerProxy();
        proxy->Register(/*service_name=*/chromeos::mojo_services::kVideoCaptureDeviceInfoMonitor, provider_receiver_.BindNewPipeAndPassRemote());
    }
    ...
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        if (camera_id_to_device_id_.find(camera_id) == camera_id_to_device_id_.end() || camera_id_to_device_id_[camera_id] != device_id) {
    ...
    void Request(chromeos::mojo_service_manager::mojom::ProcessIdentityPtr identity, mojo::ScopedMessagePipeHandle receiver) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        receiver_set_.Add(this, mojo::PendingReceiver<cros::mojom::VideoCaptureDeviceInfoMonitor>(std::move(receiver)));
    }

    // cros::mojom::VideoCaptureDeviceInfoMonitor overrides.
    void AddVideoCaptureDeviceInfoObserver(mojo::PendingRemote<cros::mojom::VideoCaptureDeviceInfoObserver> observer) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        auto id = vcd_info_observers_.Add(std::move(observer));
        for (const auto& [camera_id, device_id] : camera_id_to_device_id_) {
            vcd_info_observers_.Get(id)->OnGetCameraIdToDeviceIdMapping(camera_id, device_id);
        }
    }
...
private:
    ...
    mojo::RemoteSet<cros::mojom::VideoCaptureDeviceInfoObserver> vcd_info_observers_;

    mojo::ReceiverSet<cros::mojom::VideoCaptureDeviceInfoMonitor> receiver_set_;

    mojo::Receiver<chromeos::mojo_service_manager::mojom::ServiceProvider> provider_receiver_ { this };
...
public:
    ...
    VideoCaptureDeviceDelegateMap& operator=(const VideoCaptureDeviceDelegateMap&) = delete;
    ~VideoCaptureDeviceDelegateMap() = default;
    ...
    void Insert(const int camera_id, std::unique_ptr<VideoCaptureDeviceChromeOSDelegate> delegate)
    {
        vcd_delegates_[camera_id] = std::move(delegate);
    }

    void Erase(const int camera_id)
    {
        vcd_delegates_.erase(camera_id);
    }
...
private:
    base::flat_map<int, std::unique_ptr<VideoCaptureDeviceChromeOSDelegate>> vcd_delegates_;
    base::WeakPtrFactory<CameraHalDelegate::VideoCaptureDeviceDelegateMap> weak_ptr_factory_ { this };
};

class CameraHalDelegate::CameraModuleConnector {
public:
    using OnGetCameraModuleCallback = base::RepeatingCallback<void(mojo::PendingRemote<cros::mojom::CameraModule> camera_module)>;

    explicit CameraModuleConnector(OnGetCameraModuleCallback on_get_camera_module_callback)
        : on_get_camera_module_callback_(on_get_camera_module_callback)
    {
        mojo_service_manager_observer_ = MojoServiceManagerObserver::Create(chromeos::mojo_services::kCrosCameraService,
            base::BindRepeating(&CameraModuleConnector::ConnectToCameraService, weak_factory_.GetWeakPtr()), base::DoNothing());
    }
...
private:
    ...
    {
        ash::mojo_service_manager::GetServiceManagerProxy()->Request(
            chromeos::mojo_services::kCrosCameraService, std::nullopt, camera_service_.BindNewPipeAndPassReceiver().PassPipe());
        camera_service_.set_disconnect_handler(base::BindOnce(&CameraModuleConnector::OnCameraServiceConnectionError, weak_factory_.GetWeakPtr()));
        camera_service_->GetCameraModule(
            cros::mojom::CameraClientType::CHROME, base::BindOnce(&CameraModuleConnector::OnGetCameraModule, base::Unretained(this)));
    }

    void OnGetCameraModule(mojo::PendingRemote<cros::mojom::CameraModule> camera_module)
    {
...
CameraHalDelegate::CameraHalDelegate(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
    : camera_module_has_been_set_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    , builtin_camera_info_updated_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    , external_camera_info_updated_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::SIGNALED)
    , has_camera_connected_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
...
bool CameraHalDelegate::Init()
{
    ...
    vendor_tag_ops_delegate_ = std::make_unique<VendorTagOpsDelegate>(ipc_task_runner_);
    return true;
}
...
{
    if (ipc_task_runner_) {
        ipc_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CameraHalDelegate::ResetMojoInterfaceOnIpcThread, base::Unretained(this)));
    }
    camera_hal_ipc_thread_.Stop();

    ui_task_runner_->DeleteSoon(FROM_HERE, std::move(system_event_monitor_proxy_));
}

void CameraHalDelegate::SetCameraModule(mojo::PendingRemote<cros::mojom::CameraModule> camera_module)
{
    ipc_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CameraHalDelegate::SetCameraModuleOnIpcThread, base::Unretained(this), std::move(camera_module)));
}

void CameraHalDelegate::SetCameraModuleOnIpcThread(mojo::PendingRemote<cros::mojom::CameraModule> camera_module)
{
    ...
    camera_module_.set_disconnect_handler(base::BindOnce(&CameraHalDelegate::ResetMojoInterfaceOnIpcThread, base::Unretained(this)));
...
std::unique_ptr<VideoCaptureDevice> CameraHalDelegate::CreateDevice(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_screen_observer, const VideoCaptureDeviceDescriptor& device_descriptor)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto* delegate = GetVCDDelegate(task_runner_for_screen_observer, device_descriptor);
    return std::make_unique<VideoCaptureDeviceChromeOSHalv3>(delegate, device_descriptor);
}

void CameraHalDelegate::GetSupportedFormats(const cros::mojom::CameraInfoPtr& camera_info, VideoCaptureFormats* supported_formats)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    base::flat_set<int32_t> candidate_fps_set = GetAvailableFramerates(camera_info);

    const cros::mojom::CameraMetadataEntryPtr* min_frame_durations
        = GetMetadataEntry(camera_info->static_camera_characteristics, cros::mojom::CameraMetadataTag::ANDROID_SCALER_AVAILABLE_MIN_FRAME_DURATIONS);
    if (!min_frame_durations) {
        LOG(ERROR) << "Failed to get available min frame durations from camera info";
        return;
    }
    ...
    int64_t* iter = reinterpret_cast<int64_t*>((*min_frame_durations)->data.data());
    for (size_t i = 0; i < (*min_frame_durations)->count; i += kStreamDurationSize) {
        auto hal_format = static_cast<cros::mojom::HalPixelFormat>(iter[kStreamFormatOffset]);
        ...
        constexpr gfx::BufferUsage kClientBufferUsage = gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE;
        const ChromiumPixelFormat cr_format = camera_buffer_factory_->ResolveStreamBufferFormat(hal_format, kClientBufferUsage);
        ...
        for (auto fps : candidate_fps_set) {
            ...
            CAMERA_LOG(DEBUG) << "Supported format: " << width << "x" << height << " fps=" << fps << " format=" << cr_format.video_format;
            supported_formats->emplace_back(gfx::Size(width, height), fps, cr_format.video_format);
        }
    }
}

void CameraHalDelegate::GetDevicesInfo(VideoCaptureDeviceFactory::GetDevicesInfoCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...

    {
        ...
        for (const auto& it : camera_info_) {
            ...
            auto get_vendor_string = [&](const std::string& key) -> const char* {
                ...
                auto val = GetMetadataEntryAsSpan<char>(camera_info->static_camera_characteristics, info->tag);
            ...
            case cros::mojom::CameraFacing::CAMERA_FACING_EXTERNAL: {
                ...
                desc.device_id = path != nullptr ? path : base::NumberToString(camera_id);
            ...
            vcd_info_monitor_impl_.AsyncCall(&VCDInfoMonitorImpl::AddCameraIdToDeviceIdMapping).WithArgs(camera_id, desc.device_id);
            devices_info.emplace_back(desc);
            GetSupportedFormats(camera_info_[camera_id], &devices_info.back().supported_formats);

            // Create a virtual device when multiple streams are enabled.
            if (enable_virtual_device_[camera_id]) {
                desc.facing = VideoFacingMode::MEDIA_VIDEO_FACING_NONE;
                desc.device_id = std::string(kVirtualPrefix) + base::NumberToString(camera_id);
    ...
    std::sort(devices_info.begin(), devices_info.end(), [&](const VideoCaptureDeviceInfo& a, const VideoCaptureDeviceInfo& b) {
        auto IsExternalCamera = [](const VideoCaptureDeviceInfo& vcd_info) { return vcd_info.descriptor.facing == VideoFacingMode::MEDIA_VIDEO_FACING_NONE; };
...
VideoCaptureControlSupport CameraHalDelegate::GetControlSupport(const cros::mojom::CameraInfoPtr& camera_info)
{
    ...
    auto is_vendor_range_valid = [&](const std::string& key) -> bool {
        ...
        if (info == nullptr)
            return false;
        auto range = GetMetadataEntryAsSpan<int32_t>(camera_info->static_camera_characteristics, info->tag);
    ...
    auto max_digital_zoom
        = GetMetadataEntryAsSpan<float>(camera_info->static_camera_characteristics, cros::mojom::CameraMetadataTag::ANDROID_SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
...
cros::mojom::CameraInfoPtr CameraHalDelegate::GetCameraInfoFromDeviceId(const std::string& device_id)
{
    ...
    if (base::StartsWith(device_id, std::string(kVirtualPrefix))) {
        ...
        case cros::mojom::CameraFacing::CAMERA_FACING_FRONT:
            ...
            break;
        case cros::mojom::CameraFacing::CAMERA_FACING_EXTERNAL:
            info->facing = cros::mojom::CameraFacing::CAMERA_FACING_VIRTUAL_EXTERNAL;
            break;
        default:
            break;
        }
...
void CameraHalDelegate::EnableVirtualDevice(const std::string& device_id, bool enable)
{
    if (base::StartsWith(device_id, std::string(kVirtualPrefix))) {
        return;
    }
...
const VendorTagInfo* CameraHalDelegate::GetVendorTagInfoByName(const std::string& full_name)
{
    return vendor_tag_ops_delegate_->GetInfoByName(full_name);
}

void CameraHalDelegate::OpenDevice(
    int32_t camera_id, const std::string& module_id, mojo::PendingReceiver<cros::mojom::Camera3DeviceOps> device_ops_receiver, OpenDeviceCallback callback)
{
    DCHECK(!ipc_task_runner_->BelongsToCurrentThread());
    ...
    ipc_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &CameraHalDelegate::OpenDeviceOnIpcThread, base::Unretained(this), camera_id, module_id, std::move(device_ops_receiver), std::move(callback)));
}
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_screen_observer, const VideoCaptureDeviceDescriptor& device_descriptor)
{
    ...
    if (!vcd_delegate_map_->HasVCDDelegate(camera_id) || vcd_delegate_map_->Get(camera_id)->HasDeviceClient() == 0) {
        ...
        // same device.
        auto cleanup_callback = base::BindOnce(&VideoCaptureDeviceDelegateMap::Erase, vcd_delegate_map_->GetWeakPtr(), camera_id);
        auto delegate = std::make_unique<VideoCaptureDeviceChromeOSDelegate>(
            std::move(task_runner_for_screen_observer), device_descriptor, this, std::move(cleanup_callback));
...
bool CameraHalDelegate::UpdateBuiltInCameraInfo()
{
    ...
    ipc_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CameraHalDelegate::UpdateBuiltInCameraInfoOnIpcThread, base::Unretained(this)));
...
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
    camera_module_->GetNumberOfCameras(base::BindOnce(&CameraHalDelegate::OnGotNumberOfCamerasOnIpcThread, base::Unretained(this)));
}
...
void CameraHalDelegate::OnGotNumberOfCamerasOnIpcThread(int32_t num_cameras)
{
    ...
    camera_module_->SetCallbacksAssociated(
        camera_module_callbacks_.BindNewEndpointAndPassRemote(), base::BindOnce(&CameraHalDelegate::OnSetCallbacksOnIpcThread, base::Unretained(this)));

    camera_module_->GetVendorTagOps(
        vendor_tag_ops_delegate_->MakeReceiver(), base::BindOnce(&CameraHalDelegate::OnGotVendorTagOpsOnIpcThread, base::Unretained(this)));
}
...
void CameraHalDelegate::OnSetCallbacksOnIpcThread(int32_t result)
{
    ...
    if (result) {
        num_builtin_cameras_ = 0;
        builtin_camera_info_updated_.Signal();
        LOG(ERROR) << "Failed to set camera module callbacks: " << base::safe_strerror(-result);
        return;
    }
    ...
    for (size_t camera_id = 0; camera_id < num_builtin_cameras_; ++camera_id) {
        GetCameraInfoOnIpcThread(camera_id, base::BindOnce(&CameraHalDelegate::OnGotCameraInfoOnIpcThread, base::Unretained(this), camera_id));
    }
}
...
void CameraHalDelegate::GetCameraInfoOnIpcThread(int32_t camera_id, GetCameraInfoCallback callback)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
    camera_module_->GetCameraInfo(camera_id, std::move(callback));
}

void CameraHalDelegate::OnGotCameraInfoOnIpcThread(int32_t camera_id, int32_t result, cros::mojom::CameraInfoPtr camera_info)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
...
void CameraHalDelegate::OpenDeviceOnIpcThread(int32_t camera_id, const std::string& module_id, /* such as abcd:1234, 8 digits hex string */
    mojo::PendingReceiver<cros::mojom::Camera3DeviceOps> device_ops_receiver, OpenDeviceCallback callback)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
    base::UmaHistogramSparse("ChromeOS.Camera.ModuleID", GetMaskedModuleID(module_id));

    camera_module_->OpenDevice(camera_id, std::move(device_ops_receiver), std::move(callback));
}

// CameraModuleCallbacks implementations.
void CameraHalDelegate::CameraDeviceStatusChange(int32_t camera_id, cros::mojom::CameraDeviceStatus new_status)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
    CAMERA_LOG(EVENT) << "camera_id = " << camera_id << ", new_status = " << new_status;
    base::AutoLock lock(camera_info_lock_);
    auto it = camera_info_.find(camera_id);
    switch (new_status) {
...
void CameraHalDelegate::TorchModeStatusChange(int32_t camera_id, cros::mojom::TorchModeStatus new_status)
{
    DCHECK(ipc_task_runner_->BelongsToCurrentThread());
...
void CameraHalDelegate::BootStrapCameraServiceConnection()
{
    camera_module_connector_ = base::SequenceBound<CameraModuleConnector>(
        ui_task_runner_, base::BindPostTask(ipc_task_runner_, base::BindRepeating(&CameraHalDelegate::SetCameraModuleOnIpcThread, base::Unretained(this))));
}
