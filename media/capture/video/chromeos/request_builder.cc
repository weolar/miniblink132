...
RequestBuilder::RequestBuilder(CameraDeviceContext* device_context, RequestBufferCallback request_buffer_callback, bool use_buffer_management_apis)
...
cros::mojom::Camera3CaptureRequestPtr RequestBuilder::BuildRequest(std::set<StreamType> stream_types, cros::mojom::CameraMetadataPtr settings)
{
    ...
    for (StreamType stream_type : stream_types) {
        ...
        if (use_buffer_management_apis_) {
            capture_request->output_buffers.push_back(CreateStreamBuffer(stream_type, std::nullopt));
            continue;
        }

        std::optional<BufferInfo> buffer_info = request_buffer_callback_.Run(stream_type);
        if (!buffer_info) {
            return capture_request;
        }
        capture_request->output_buffers.push_back(CreateStreamBuffer(stream_type, std::move(*buffer_info)));
    }
...
cros::mojom::Camera3StreamBufferPtr RequestBuilder::CreateStreamBuffer(StreamType stream_type, std::optional<BufferInfo> buffer_info)
{
    cros::mojom::Camera3StreamBufferPtr buffer = cros::mojom::Camera3StreamBuffer::New();
    ...
    if (buffer_info.has_value()) {
        ...
        buffer->buffer_handle = CreateCameraBufferHandle(stream_type, std::move(*buffer_info));
    } else {
...
cros::mojom::CameraBufferHandlePtr RequestBuilder::CreateCameraBufferHandle(StreamType stream_type, BufferInfo buffer_info)
{
    ...
    gfx::NativePixmapHandle& native_pixmap_handle = buffer_info.gpu_memory_buffer_handle.native_pixmap_handle;
