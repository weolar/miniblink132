...
constexpr std::array<ClientType, static_cast<int>(StreamType::kUnknown)> kStreamClientTypeMap = {
...
using TakePhotoCallback = base::OnceCallback<void(int32_t, media::mojom::BlobPtr)>;
...
public:
    ...
    virtual void ProcessCaptureRequest(cros::mojom::Camera3CaptureRequestPtr request, base::OnceCallback<void(int32_t)> callback) = 0;

    // Registers a new buffer in the camera HAL buffer object pool.
    virtual void OnNewBuffer(ClientType client_type, cros::mojom::CameraBufferHandlePtr buffer) = 0;
...
class CAPTURE_EXPORT CameraDeviceDelegate final : public CaptureMetadataDispatcher::ResultMetadataObserver {
public:
    CameraDeviceDelegate() = delete;

    CameraDeviceDelegate(VideoCaptureDeviceDescriptor device_descriptor, CameraHalDelegate* camera_hal_delegate,
    ...
    void AllocateAndStart(const base::flat_map<ClientType, VideoCaptureParams>& params, CameraDeviceContext* device_context);
    ...
    void ReconfigureStreams(const base::flat_map<ClientType, VideoCaptureParams>& params);
    ...
    void OnNewBufferResult(ClientType client_type, uint64_t buffer_id, int32_t ret);
...
private:
    ...
    void OnFlushed(bool require_photo, std::optional<gfx::Size> new_blob_resolution, int32_t result);
    ...
    void ConfigureStreams(bool require_photo, std::optional<gfx::Size> new_blob_resolution);
    void OnConfiguredStreams(gfx::Size blob_resolution, int32_t result, cros::mojom::Camera3StreamConfigurationPtr updated_config);
    ...
    // video capture loop.
    void OnConstructedDefaultPreviewRequestSettings(cros::mojom::CameraMetadataPtr settings);
    // OnConstructDefaultStillCaptureRequestSettings triggers
    // |request_manager_| to request a still capture.
    void OnConstructedDefaultStillCaptureRequestSettings(cros::mojom::Camera3RequestTemplate requset_template, cros::mojom::CameraMetadataPtr settings);
    // OnConstructedDefaultPortraitModeRequestSettings triggers
    // |request_manager_| to request portrait mode still captures.
    void OnConstructedDefaultPortraitModeRequestSettings(cros::mojom::CameraMetadataPtr settings);
    ...
    bool SetPointsOfInterest(const std::vector<mojom::Point2DPtr>& points_of_interest);

    // This function gets the TYPE_INT32[3] array of [max, min, step] from static
    // metadata by |range_name| and current value of |current|.
    mojom::RangePtr GetControlRangeByVendorTagName(const std::string& range_name, const std::optional<int32_t>& current);

    bool ShouldUseBlobVideoSnapshot();

    // CaptureMetadataDispatcher::ResultMetadataObserver implementation.
    void OnResultMetadataAvailable(uint32_t frame_number, const cros::mojom::CameraMetadataPtr& result_metadata) final;
    ...
    void SetDigitalZoomSessionParameters(cros::mojom::CameraMetadataPtr* session_parameters);

    // Configures the session_parameters with initial values for the keys
    // found in ANDROID_REQUEST_AVAILABLE_SESSION_KEYS.
    void ConfigureSessionParameters(cros::mojom::CameraMetadataPtr* session_parameters);
    ...
    std::optional<cros::mojom::CameraAutoFramingState> current_auto_framing_state_;
    ...
    base::SequenceBound<CrosCameraAutoFramingStateObserver> auto_framing_state_observer_;
