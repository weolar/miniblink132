...
class CAPTURE_EXPORT CameraHalDelegate final : public cros::mojom::CameraModuleCallbacks {
public:
    ...
    explicit CameraHalDelegate(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner);
    ...
    void SetCameraModule(mojo::PendingRemote<cros::mojom::CameraModule> camera_module);
    ...
    void GetDevicesInfo(VideoCaptureDeviceFactory::GetDevicesInfoCallback callback);

    // Returns camera pan, tilt, zoom capability support.
    VideoCaptureControlSupport GetControlSupport(const cros::mojom::CameraInfoPtr& camera_info);

    // Gets the camera info of |device_id|. Returns null CameraInfoPtr on error.
    cros::mojom::CameraInfoPtr GetCameraInfoFromDeviceId(const std::string& device_id);
    ...
    void OpenDevice(
        int32_t camera_id, const std::string& model_id, mojo::PendingReceiver<cros::mojom::Camera3DeviceOps> device_ops_receiver, OpenDeviceCallback callback);
...
private:
    ...
    void GetSupportedFormats(const cros::mojom::CameraInfoPtr& camera_info, VideoCaptureFormats* supported_formats);

    VideoCaptureDeviceChromeOSDelegate* GetVCDDelegate(
        scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_screen_observer, const VideoCaptureDeviceDescriptor& device_descriptor);

    void SetCameraModuleOnIpcThread(mojo::PendingRemote<cros::mojom::CameraModule> camera_module);
    ...
    using GetCameraInfoCallback = base::OnceCallback<void(int32_t, cros::mojom::CameraInfoPtr)>;
    void GetCameraInfoOnIpcThread(int32_t camera_id, GetCameraInfoCallback callback);
    void OnGotCameraInfoOnIpcThread(int32_t camera_id, int32_t result, cros::mojom::CameraInfoPtr camera_info);
    ...
    void CameraDeviceStatusChange(int32_t camera_id, cros::mojom::CameraDeviceStatus new_status) final;
    void TorchModeStatusChange(int32_t camera_id, cros::mojom::TorchModeStatus new_status) final;
    ...
    std::unordered_map<int, cros::mojom::CameraInfoPtr> camera_info_ GUARDED_BY(camera_info_lock_);
    ...
    std::map<std::string, int> device_id_to_camera_id_ GUARDED_BY(device_id_to_camera_id_lock_);
    // A virtual device is enabled/disabled for camera id.
    base::Lock enable_virtual_device_lock_;
    base::flat_map<int, bool> enable_virtual_device_ GUARDED_BY(enable_virtual_device_lock_);
    ...
    // |ipc_task_runner_|.
    mojo::AssociatedReceiver<cros::mojom::CameraModuleCallbacks> camera_module_callbacks_;
    ...
    std::unique_ptr<VideoCaptureDeviceDelegateMap> vcd_delegate_map_ GUARDED_BY_CONTEXT(sequence_checker_);
