...
public:
    ...
    StreamBufferManager(CameraDeviceContext* device_context, bool video_capture_use_gmb, std::unique_ptr<CameraBufferFactory> camera_buffer_factory,
    ...
    gfx::GpuMemoryBuffer* GetGpuMemoryBufferById(StreamType stream_type, uint64_t buffer_ipc_id);
    ...
    std::optional<Buffer> AcquireBufferForClientById(StreamType stream_type, uint64_t buffer_ipc_id, VideoCaptureFormat* format);
    ...
    std::optional<BufferInfo> RequestBufferForCaptureRequest(StreamType stream_type);

    // Releases buffer by marking it as free buffer.
    void ReleaseBufferFromCaptureResult(StreamType stream_type, uint64_t buffer_ipc_id);
...
private:
    ...
    struct BufferPair {
        BufferPair(std::unique_ptr<gfx::GpuMemoryBuffer> gmb, std::optional<Buffer> vcd_buffer);
    ...
    std::unordered_map<StreamType, std::unique_ptr<StreamContext>> stream_context_;
