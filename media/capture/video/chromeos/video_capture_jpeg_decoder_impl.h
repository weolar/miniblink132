...
class CAPTURE_EXPORT VideoCaptureJpegDecoderImpl : public VideoCaptureJpegDecoder, public chromeos_camera::MjpegDecodeAccelerator::Client {
public:
    VideoCaptureJpegDecoderImpl(MojoMjpegDecodeAcceleratorFactoryCB jpeg_decoder_factory, scoped_refptr<base::SequencedTaskRunner> decoder_task_runner,
        DecodeDoneCB decode_done_cb, base::RepeatingCallback<void(const std::string&)> send_log_message_cb);

    VideoCaptureJpegDecoderImpl(const VideoCaptureJpegDecoderImpl&) = delete;
    VideoCaptureJpegDecoderImpl& operator=(const VideoCaptureJpegDecoderImpl&) = delete;
    ...
    void DecodeCapturedData(const uint8_t* data, size_t in_buffer_size, const media::VideoCaptureFormat& frame_format, base::TimeTicks reference_time,
        base::TimeDelta timestamp, media::VideoCaptureDevice::Client::Buffer out_buffer) override;
    ...
    void NotifyError(int32_t buffer_id, chromeos_camera::MjpegDecodeAccelerator::Error error) override;
