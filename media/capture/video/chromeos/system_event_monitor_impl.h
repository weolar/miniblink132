...
// the ui thread.
class CAPTURE_EXPORT SystemEventMonitorImpl final : public cros::mojom::CrosSystemEventMonitor,
...
public:
    ...
    void AddDisplayObserver(mojo::PendingRemote<cros::mojom::CrosDisplayObserver> observer) override;

    void AddLidObserver(mojo::PendingRemote<cros::mojom::CrosLidObserver> observer) override;

    void AddPowerObserver(const std::string& info, mojo::PendingRemote<cros::mojom::CrosPowerObserver> observer) override;
...
private:
    ...
    void LidEventReceived(chromeos::PowerManagerClient::LidState state, base::TimeTicks timestamp) final;
    ...
    base::flat_map<DisplayObserver*, std::unique_ptr<DisplayObserver>> display_observers_;

    mojo::RemoteSet<cros::mojom::CrosLidObserver> lid_observers_;

    base::flat_map<PowerObserver*, std::unique_ptr<PowerObserver>> power_observers_;
...
// Constructor, destructor and all of the member functions have to be invoked on
// the ui thread.
class SystemEventMonitorImpl::DisplayObserver : public display::DisplayObserver {
public:
    DisplayObserver(mojo::PendingRemote<cros::mojom::CrosDisplayObserver> observer, base::OnceCallback<void(DisplayObserver*)> cleanup_callback);
...
private:
    ...
    void OnRemoteObserverDisconnected(base::OnceCallback<void(DisplayObserver*)> cleanup_callback);
...
// Constructor, destructor and all of the member functions have to be invoked on
// the ui thread.
class SystemEventMonitorImpl::PowerObserver : public chromeos::PowerManagerClient::Observer {
public:
    PowerObserver(const std::string& client_name, mojo::PendingRemote<cros::mojom::CrosPowerObserver> observer,
...
private:
    ...
    void OnRemoteObserverDisconnected(base::OnceCallback<void(PowerObserver*)> cleanup_callback);
