...
MockVendorTagOps::~MockVendorTagOps()
{
    mock_vendor_tag_ops_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&MockVendorTagOps::CloseBindingOnThread, base::Unretained(this)));
    mock_vendor_tag_ops_thread_.Stop();
}

void MockVendorTagOps::Bind(mojo::PendingReceiver<cros::mojom::VendorTagOps> receiver)
{
    ...
    mock_vendor_tag_ops_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&MockVendorTagOps::BindOnThread, base::Unretained(this), base::Unretained(&done), std::move(receiver)));
    done.Wait();
}
...
void MockVendorTagOps::GetSectionName(uint32_t tag, GetSectionNameCallback callback)
{
...
void MockVendorTagOps::BindOnThread(base::WaitableEvent* done, mojo::PendingReceiver<cros::mojom::VendorTagOps> receiver)
{
