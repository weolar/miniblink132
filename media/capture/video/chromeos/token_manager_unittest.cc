...
protected:
    ...
    base::flat_map<cros::mojom::CameraClientType, base::UnguessableToken> client_token_map_;
};
...
TEST_F(TokenManagerTest, AuthenticateClient)
{
    for (auto type : TokenManager::kTrustedClientTypes) {
        auto authenticated_type = manager_.AuthenticateClient(type, client_token_map_[type]);
        ...
        authenticated_type = manager_.AuthenticateClient(type, base::UnguessableToken());
...
TEST_F(TokenManagerTest, AuthenticatePluginvm)
{
    ...
    auto authenticated_type = manager_.AuthenticateClient(cros::mojom::CameraClientType::UNKNOWN, token);
    ASSERT_TRUE(authenticated_type.has_value());
    EXPECT_EQ(authenticated_type.value(), cros::mojom::CameraClientType::PLUGINVM);

    manager_.UnregisterPluginVmToken(token);
    authenticated_type = manager_.AuthenticateClient(cros::mojom::CameraClientType::UNKNOWN, token);
    EXPECT_FALSE(authenticated_type.has_value());
}
...
{
    auto authenticated_type = manager_.AuthenticateClient(cros::mojom::CameraClientType::UNKNOWN, base::UnguessableToken::Create());
    EXPECT_FALSE(authenticated_type.has_value());
}
...
{
    EXPECT_TRUE(manager_.GetTokenForTrustedClient(cros::mojom::CameraClientType::UNKNOWN).is_empty());
    EXPECT_TRUE(manager_.GetTokenForTrustedClient(cros::mojom::CameraClientType::PLUGINVM).is_empty());
}
