...
VideoCaptureJpegDecoderImpl::VideoCaptureJpegDecoderImpl(MojoMjpegDecodeAcceleratorFactoryCB jpeg_decoder_factory,
    scoped_refptr<base::SequencedTaskRunner> decoder_task_runner, DecodeDoneCB decode_done_cb,
...
void VideoCaptureJpegDecoderImpl::Initialize()
{
    ...
    decoder_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoCaptureJpegDecoderImpl::FinishInitialization, weak_ptr_factory_.GetWeakPtr()));
}
...
void VideoCaptureJpegDecoderImpl::DecodeCapturedData(const uint8_t* data, size_t in_buffer_size, const media::VideoCaptureFormat& frame_format,
    base::TimeTicks reference_time, base::TimeDelta timestamp, media::VideoCaptureDevice::Client::Buffer out_buffer)
{
    DCHECK(decoder_);

    TRACE_EVENT_BEGIN("jpeg", "VideoCaptureJpegDecoderImpl decoding", GetTraceTrack(CameraTraceEvent::kJpegDecoding, next_task_id_));
    ...
    if (!in_shared_region_.IsValid() || !in_shared_mapping_.IsValid() || in_buffer_size > in_shared_mapping_.size()) {
        ...
        if (!in_shared_region_.IsValid()) {
            base::AutoLock lock(lock_);
            decoder_status_ = FAILED;
            LOG(WARNING) << "UnsafeSharedMemoryRegion::Create failed, size=" << reserved_size;
            return;
        }
        ...
        if (!in_shared_mapping_.IsValid()) {
            base::AutoLock lock(lock_);
            decoder_status_ = FAILED;
            LOG(WARNING) << "UnsafeSharedMemoryRegion::Map failed, size=" << reserved_size;
            return;
        }
    }
    ...
    media::BitstreamBuffer in_buffer(task_id_, in_shared_region_.Duplicate(), in_buffer_size);
    // Mask against 30 bits, to avoid (undefined) wraparound on signed integer.
    next_task_id_ = (next_task_id_ + 1) & 0x3FFFFFFF;

    const gfx::Size dimensions = frame_format.frame_size;
    if (!VideoFrame::IsValidConfig(PIXEL_FORMAT_I420, VideoFrame::STORAGE_UNOWNED_MEMORY, dimensions, gfx::Rect(dimensions), dimensions)) {
        base::AutoLock lock(lock_);
        decoder_status_ = FAILED;
    ...
    base::UnsafeSharedMemoryRegion out_region = out_buffer.handle_provider->DuplicateAsUnsafeRegion();
    DCHECK(out_region.IsValid());

    media::mojom::VideoFrameInfoPtr out_frame_info = media::mojom::VideoFrameInfo::New();
    ...
    {
        base::AutoLock lock(lock_);
        decode_done_closure_ = base::BindOnce(decode_done_cb_,
            ReadyFrameInBuffer(out_buffer.id, out_buffer.frame_feedback_id, std::move(out_buffer.access_permission), std::move(out_frame_info)));
    }

    // base::Unretained is safe because |decoder_| is deleted on
    // |decoder_task_runner_|.
    decoder_task_runner_->PostTask(FROM_HERE,
        base::BindOnce([](chromeos_camera::MojoMjpegDecodeAccelerator* decoder, BitstreamBuffer in_buffer, VideoPixelFormat format, gfx::Size coded_size,
                           base::UnsafeSharedMemoryRegion out_region) { decoder->Decode(std::move(in_buffer), format, coded_size, std::move(out_region)); },
            base::Unretained(decoder_.get()), std::move(in_buffer), media::PIXEL_FORMAT_I420, dimensions, std::move(out_region)));
}
...
void VideoCaptureJpegDecoderImpl::VideoFrameReady(int32_t task_id)
{
    ...
    TRACE_EVENT_END("jpeg", GetTraceTrack(CameraTraceEvent::kJpegDecoding, task_id));
}

void VideoCaptureJpegDecoderImpl::NotifyError(int32_t task_id, chromeos_camera::MjpegDecodeAccelerator::Error error)
{
    DCHECK(decoder_task_runner_->RunsTasksInCurrentSequence());
...
void VideoCaptureJpegDecoderImpl::FinishInitialization()
{
    ...
    mojo::PendingRemote<chromeos_camera::mojom::MjpegDecodeAccelerator> remote_decoder;
    jpeg_decoder_factory_.Run(remote_decoder.InitWithNewPipeAndPassReceiver());

    base::AutoLock lock(lock_);
    decoder_ = std::make_unique<chromeos_camera::MojoMjpegDecodeAccelerator>(decoder_task_runner_, std::move(remote_decoder));

    decoder_->InitializeAsync(this, base::BindOnce(&VideoCaptureJpegDecoderImpl::OnInitializationDone, weak_ptr_factory_.GetWeakPtr()));
}
