...
public:
    ...
    void SetSharedImageInterface(scoped_refptr<gpu::SharedImageInterface> interface)
    {
        base::AutoLock lock(lock_);
...
private:
    ...
    raw_ptr<gpu::GpuMemoryBufferManager> gpu_buffer_manager_ GUARDED_BY(lock_) = nullptr;
    scoped_refptr<gpu::SharedImageInterface> shared_image_interface_ GUARDED_BY(lock_);
...
VideoCaptureDeviceFactoryChromeOS::VideoCaptureDeviceFactoryChromeOS(scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
...
VideoCaptureErrorOrDevice VideoCaptureDeviceFactoryChromeOS::CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    if (!initialized_) {
        return VideoCaptureErrorOrDevice(VideoCaptureError::kCrosHalV3DeviceDelegateFailedToInitializeCameraDevice);
    }
    auto device = camera_hal_delegate_->CreateDevice(ui_task_runner_, device_descriptor);

    if (!device) {
        return VideoCaptureErrorOrDevice(VideoCaptureError::kVideoCaptureDeviceFactoryChromeOSCreateDeviceFailed);
    }
    if (!vcd_task_runner_) {
...
void VideoCaptureDeviceFactoryChromeOS::GetDevicesInfo(GetDevicesInfoCallback callback)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    if (!initialized_) {
...
// static
gpu::GpuMemoryBufferManager* VideoCaptureDeviceFactoryChromeOS::GetBufferManager()
{
    return GetGpuResources().GetBufferManager();
}

// static
void VideoCaptureDeviceFactoryChromeOS::SetGpuBufferManager(gpu::GpuMemoryBufferManager* buffer_manager)
{
    GetGpuResources().SetBufferManager(buffer_manager);
}

// static
void VideoCaptureDeviceFactoryChromeOS::SetGpuChannelHost(scoped_refptr<gpu::GpuChannelHost> gpu_channel_host)
{
    GetGpuResources().SetGpuChannelHost(std::move(gpu_channel_host));
}

// static
scoped_refptr<gpu::GpuChannelHost> VideoCaptureDeviceFactoryChromeOS::GetGpuChannelHost()
{
    return GetGpuResources().GetGpuChannelHost();
}

// static
scoped_refptr<gpu::SharedImageInterface> VideoCaptureDeviceFactoryChromeOS::GetSharedImageInterface()
{
    return GetGpuResources().GetSharedImageInterface().get();
}

// static
void VideoCaptureDeviceFactoryChromeOS::SetSharedImageInterface(scoped_refptr<gpu::SharedImageInterface> shared_image_interface)
{
...
bool VideoCaptureDeviceFactoryChromeOS::Init()
{
    ...
    camera_app_device_bridge->SetCameraInfoGetter(
        base::BindRepeating(&CameraHalDelegate::GetCameraInfoFromDeviceId, base::Unretained(camera_hal_delegate_.get())));
    camera_app_device_bridge->SetVirtualDeviceController(
        base::BindRepeating(&CameraHalDelegate::EnableVirtualDevice, base::Unretained(camera_hal_delegate_.get())));
    return true;
}
