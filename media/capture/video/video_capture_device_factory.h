...
public:
    explicit VideoCaptureErrorOrDevice(std::unique_ptr<VideoCaptureDevice> video_device);
    ...
    bool ok() const
    {
        return error_code_ == VideoCaptureError::kNone;
    }
    VideoCaptureError error() const
    {
        return error_code_;
    }
...
public:
    ...
    VideoCaptureDeviceFactory& operator=(const VideoCaptureDeviceFactory&) = delete;
    ...
    virtual VideoCaptureErrorOrDevice CreateDevice(const VideoCaptureDeviceDescriptor& device_descriptor) = 0;
    ...
    // is destroyed.
    using GetDevicesInfoCallback = base::OnceCallback<void(std::vector<VideoCaptureDeviceInfo> devices_info)>;
