...
struct CAPTURE_EXPORT CapturedExternalVideoBuffer {
    CapturedExternalVideoBuffer(gfx::GpuMemoryBufferHandle handle, VideoCaptureFormat format, gfx::ColorSpace color_space);

#if BUILDFLAG(IS_WIN)
    CapturedExternalVideoBuffer(
        Microsoft::WRL::ComPtr<IMFMediaBuffer> imf_buffer, gfx::GpuMemoryBufferHandle handle, VideoCaptureFormat format, gfx::ColorSpace color_space);
#endif
    ...
    CapturedExternalVideoBuffer& operator=(const CapturedExternalVideoBuffer&) = delete;
...
class CAPTURE_EXPORT VideoCaptureDevice : public VideoFrameConsumerFeedbackObserver {
public:
    ...
    public:
        ...
        public:
            ...
            public:
                ...
                virtual std::unique_ptr<VideoCaptureBufferHandle> GetHandleForInProcessAccess() = 0;
            ...
            Buffer();
            Buffer(int buffer_id, int frame_feedback_id, std::unique_ptr<HandleProvider> handle_provider,
        ...
        enum class ReserveResult { kSucceeded, kMaxBufferCountExceeded, kAllocationFailed };
        ...
        // at the same time.
        virtual void OnIncomingCapturedData(const uint8_t* data, int length, const VideoCaptureFormat& frame_format, const gfx::ColorSpace& color_space,
        ...
        virtual void OnIncomingCapturedGfxBuffer(gfx::GpuMemoryBuffer* buffer, const VideoCaptureFormat& frame_format, int clockwise_rotation,
        ...
        void OnIncomingCapturedGfxBuffer(gfx::GpuMemoryBuffer* buffer, const VideoCaptureFormat& frame_format, int clockwise_rotation,
            base::TimeTicks reference_time, base::TimeDelta timestamp, std::optional<base::TimeTicks> capture_begin_timestamp);
        ...
        virtual void OnIncomingCapturedExternalBuffer(CapturedExternalVideoBuffer buffer, base::TimeTicks reference_time, base::TimeDelta timestamp,
        ...
        [[nodiscard]] virtual ReserveResult ReserveOutputBuffer(
            const gfx::Size& dimensions, VideoPixelFormat format, int frame_feedback_id, Buffer* buffer, int* require_new_buffer_id, int* retire_old_buffer_id)
            = 0;
        ...
        // |timestamp|.
        virtual void OnIncomingCapturedBuffer(Buffer buffer, const VideoCaptureFormat& format, base::TimeTicks reference_time, base::TimeDelta timestamp,
            std::optional<base::TimeTicks> capture_begin_timestamp)
            = 0;

        // Extended version of OnIncomingCapturedBuffer() allowing clients to
        // pass a custom |visible_rect| and |additional_metadata|.
        virtual void OnIncomingCapturedBufferExt(Buffer buffer, const VideoCaptureFormat& format, const gfx::ColorSpace& color_space,
        ...
        virtual void OnError(VideoCaptureError error, const base::Location& from_here, const std::string& reason) = 0;
    ...
    virtual void AllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<Client> client) = 0;
    ...
    // as needed.
    virtual void ApplySubCaptureTarget(mojom::SubCaptureTargetType type, const base::Token& target, uint32_t sub_capture_target_version,
    ...
    static PowerLineFrequency GetPowerLineFrequency(const VideoCaptureParams& params);
...
VideoCaptureFrameDropReason ConvertReservationFailureToFrameDropReason(VideoCaptureDevice::Client::ReserveResult reserve_result);
