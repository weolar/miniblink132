...
mojom::PhotoStatePtr MakePhotoState(bool blur, bool face_framing, bool eye_gaze)
{
    ...
    result->supported_eye_gaze_correction_modes = { mojom::EyeGazeCorrectionMode::ON };
    if (blur) {
...
TEST(VideoCaptureMetricsTest, TestLogCaptureDeviceMetrics)
{
    ...
    first_device.supported_formats = { // NV12 QQVGA at 30fps, 15fps
        { { 160, 120 }, 30.0, media::PIXEL_FORMAT_NV12 }, { { 160, 120 }, 15.0, media::PIXEL_FORMAT_NV12 },
        // NV12 VGA
    ...
    second_device.supported_formats = { // UYVY VGA to test that we get 2 UYVY and 2 VGA in metrics.
        { { 640, 480 }, 30.0, media::PIXEL_FORMAT_UYVY }
    };
    ...
    EXPECT_THAT(histogram_tester.GetAllSamples("Media.VideoCapture.Device.SupportedPixelFormat"),
        UnorderedElementsAre(Bucket(media::PIXEL_FORMAT_NV12, 1), Bucket(media::PIXEL_FORMAT_UYVY, 2), Bucket(media::PIXEL_FORMAT_MJPEG, 1),
            Bucket(media::PIXEL_FORMAT_UNKNOWN, 1)));

    EXPECT_THAT(histogram_tester.GetAllSamples("Media.VideoCapture.Device.SupportedResolution"),
        UnorderedElementsAre(Bucket(0 /*other*/, 1), Bucket(1 /*qqvga*/, 1), Bucket(6 /*vga*/, 2), Bucket(23 /*4k_UHD*/, 1), Bucket(18 /*hd*/, 1)));
}
...
TEST(VideoCaptureMetricsTest, TestLogCaptureDeviceEffects)
{
    ...
    EXPECT_THAT(
        histogram_tester.GetAllSamples("Media.VideoCapture.Device.Effect2.BackgroundBlur"), UnorderedElementsAre(Bucket(0, 1), Bucket(1, 3), Bucket(2, 1)));

    EXPECT_THAT(
        histogram_tester.GetAllSamples("Media.VideoCapture.Device.Effect2.EyeGazeCorrection"), UnorderedElementsAre(Bucket(0, 1), Bucket(1, 3), Bucket(2, 1)));

    EXPECT_THAT(
        histogram_tester.GetAllSamples("Media.VideoCapture.Device.Effect2.FaceFraming"), UnorderedElementsAre(Bucket(0, 1), Bucket(1, 3), Bucket(2, 1)));
}
