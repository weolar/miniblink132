...
VideoCaptureBufferPoolImpl::VideoCaptureBufferPoolImpl(VideoCaptureBufferType buffer_type)
    : VideoCaptureBufferPoolImpl(buffer_type, kVideoCaptureDefaultMaxBufferPoolSize, std::make_unique<media::VideoCaptureBufferTrackerFactoryImpl>())
{
}

VideoCaptureBufferPoolImpl::VideoCaptureBufferPoolImpl(VideoCaptureBufferType buffer_type, int count)
    : VideoCaptureBufferPoolImpl(buffer_type, count, std::make_unique<media::VideoCaptureBufferTrackerFactoryImpl>())
{
}

VideoCaptureBufferPoolImpl::VideoCaptureBufferPoolImpl(
    VideoCaptureBufferType buffer_type, int count, std::unique_ptr<VideoCaptureBufferTrackerFactory> buffer_tracker_factory)
...
base::UnsafeSharedMemoryRegion VideoCaptureBufferPoolImpl::DuplicateAsUnsafeRegion(int buffer_id)
{
...
std::unique_ptr<VideoCaptureBufferHandle> VideoCaptureBufferPoolImpl::GetHandleForInProcessAccess(int buffer_id)
{
    base::AutoLock lock(lock_);
...
gfx::GpuMemoryBufferHandle VideoCaptureBufferPoolImpl::GetGpuMemoryBufferHandle(int buffer_id)
{
    base::AutoLock lock(lock_);
    VideoCaptureBufferTracker* tracker = GetTracker(buffer_id);
    if (!tracker) {
...
VideoCaptureBufferType VideoCaptureBufferPoolImpl::GetBufferType(int buffer_id)
{
    base::AutoLock lock(lock_);
...
VideoCaptureDevice::Client::ReserveResult VideoCaptureBufferPoolImpl::ReserveForProducer(
    const gfx::Size& dimensions, VideoPixelFormat format, const mojom::PlaneStridesPtr& strides, int frame_feedback_id, int* buffer_id, int* buffer_id_to_drop)
{
    base::AutoLock lock(lock_);
    return ReserveForProducerInternal(dimensions, format, strides, frame_feedback_id, buffer_id, buffer_id_to_drop);
}
...
VideoCaptureDevice::Client::ReserveResult VideoCaptureBufferPoolImpl::ReserveIdForExternalBuffer(
    CapturedExternalVideoBuffer buffer, const gfx::Size& dimensions, int* buffer_id_to_drop, int* buffer_id)
{
    DCHECK(buffer_id);
    ...
    *buffer_id = kInvalidId;
    ...
    for (auto it = trackers_.begin(); it != trackers_.end(); ++it) {
        ...
        if (lru_tracker_it == trackers_.end() || lru_tracker_it->second->LastCustomerUseSequenceNumber() > tracker->LastCustomerUseSequenceNumber()) {
            lru_tracker_it = it;
        }
    }
    ...
    if (trackers_.size() == static_cast<size_t>(count_)) {
        if (lru_tracker_it == trackers_.end()) {
            ...
            DLOG(ERROR) << __func__ << " max buffer count exceeded count_ = " << count_;
    ...
    auto tracker = buffer_tracker_factory_->CreateTrackerForExternalGpuMemoryBuffer(std::move(buffer.handle));
#if BUILDFLAG(IS_WIN)
    // Windows needs to create buffer from external handle, but mac doesn't.
    if (!tracker || !tracker->Init(dimensions, buffer.format.pixel_format, nullptr)) {
...
void VideoCaptureBufferPoolImpl::HoldForConsumers(int buffer_id, int num_clients)
{
    base::AutoLock lock(lock_);
    VideoCaptureBufferTracker* tracker = GetTracker(buffer_id);
    if (!tracker) {
...
void VideoCaptureBufferPoolImpl::RelinquishConsumerHold(int buffer_id, int num_clients)
{
    base::AutoLock lock(lock_);
    VideoCaptureBufferTracker* tracker = GetTracker(buffer_id);
    if (!tracker) {
...
VideoCaptureDevice::Client::ReserveResult VideoCaptureBufferPoolImpl::ReserveForProducerInternal(const gfx::Size& dimensions, VideoPixelFormat pixel_format,
    const mojom::PlaneStridesPtr& strides, int frame_feedback_id, int* buffer_id, int* buffer_id_to_drop)
{
    ...
    *buffer_id = kInvalidId;
    ...
    if (buffer_type == VideoCaptureBufferType::kGpuMemoryBuffer && pixel_format != PIXEL_FORMAT_NV12
#if BUILDFLAG(IS_MAC)
...
#endif
    std::unique_ptr<VideoCaptureBufferTracker> tracker = buffer_tracker_factory_->CreateTracker(buffer_type);
...
VideoCaptureBufferTracker* VideoCaptureBufferPoolImpl::GetTracker(int buffer_id)
{
