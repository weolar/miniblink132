...
public:
    ...
    static std::unique_ptr<PixelBufferPool> Create(OSType format, int width, int height, std::optional<size_t> max_buffers);
    ~PixelBufferPool();
...
private:
    friend std::unique_ptr<PixelBufferPool> std::make_unique<PixelBufferPool>(
        base::apple::ScopedCFTypeRef<CVPixelBufferPoolRef>&& buffer_pool, std::optional<size_t>&& max_buffers);

    PixelBufferPool(base::apple::ScopedCFTypeRef<CVPixelBufferPoolRef> buffer_pool, std::optional<size_t> max_buffers);
