...
struct CAPTURE_EXPORT PostProcessDoneInfo {
    ...
    explicit PostProcessDoneInfo(VideoCaptureDevice::Client::Buffer buffer, mojom::VideoFrameInfoPtr info);
...
public:
    ...
    explicit VideoCaptureEffectsProcessor(mojo::PendingRemote<video_effects::mojom::VideoEffectsProcessor> video_effects_processor);

    VideoCaptureEffectsProcessor(const VideoCaptureEffectsProcessor& other) = delete;
    VideoCaptureEffectsProcessor& operator=(const VideoCaptureEffectsProcessor& other) = delete;

    VideoCaptureEffectsProcessor(VideoCaptureEffectsProcessor&& other) = delete;
    VideoCaptureEffectsProcessor& operator=(VideoCaptureEffectsProcessor&& other) = delete;
    ...
    using PostProcessDoneCallback = base::OnceCallback<void(base::expected<PostProcessDoneInfo, video_effects::mojom::PostProcessError>)>;

    // On-CPU variant. Prepares the shared images backed by `out_buffer` for
    // receiving the processing results. Invokes the processor.
    void PostProcessData(base::ReadOnlySharedMemoryRegion data, mojom::VideoFrameInfoPtr frame_info, VideoCaptureDevice::Client::Buffer out_buffer,
        const VideoCaptureFormat& out_buffer_format, VideoCaptureBufferType out_buffer_type,
    ...
    void PostProcessBuffer(VideoCaptureDevice::Client::Buffer in_buffer, mojom::VideoFrameInfoPtr frame_info, VideoCaptureBufferType in_buffer_type,
    ...
    void PostProcessExternalBuffer(CapturedExternalVideoBuffer in_buffer, mojom::VideoFrameInfoPtr frame_info, VideoCaptureDevice::Client::Buffer out_buffer,
        const VideoCaptureFormat& out_buffer_format, VideoCaptureBufferType out_buffer_type,
        VideoCaptureEffectsProcessor::PostProcessDoneCallback post_process_cb);
...
private:
    struct PostProcessContext {
        ...
        PostProcessContext(std::optional<VideoCaptureDevice::Client::Buffer> in_buffer, scoped_refptr<gpu::ClientSharedImage> in_shared_image,
            VideoCaptureDevice::Client::Buffer out_buffer, scoped_refptr<gpu::ClientSharedImage> out_shared_image,
            VideoCaptureEffectsProcessor::PostProcessDoneCallback post_process_cb);
        ~PostProcessContext();
    ...
    void OnPostProcess(PostProcessContext context, video_effects::mojom::PostProcessResultPtr result);

    mojo::Remote<video_effects::mojom::VideoEffectsProcessor> effects_processor_ GUARDED_BY_CONTEXT(sequence_checker_);
