...
VideoCaptureErrorOrDevice::VideoCaptureErrorOrDevice(std::unique_ptr<VideoCaptureDevice> video_device)
    : device_(std::move(video_device))
    , error_code_(VideoCaptureError::kNone)
{
}
...
VideoCaptureErrorOrDevice::VideoCaptureErrorOrDevice(VideoCaptureErrorOrDevice&& other)
...
#if BUILDFLAG(IS_WIN)
scoped_refptr<DXGIDeviceManager> VideoCaptureDeviceFactory::GetDxgiDeviceManager()
{
    return nullptr;
}
