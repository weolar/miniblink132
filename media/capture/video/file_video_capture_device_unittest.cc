...
protected:
    ...
    void SetUp() override
    {
        ...
        device_ = std::make_unique<FileVideoCaptureDevice>(GetTestDataFilePath("bear.mjpeg"), std::make_unique<FakeGpuMemoryBufferSupport>());
    ...
    std::unique_ptr<MockVideoCaptureDeviceClient> CreateClient()
    {
        return MockVideoCaptureDeviceClient::CreateMockClientWithBufferAllocator(
            base::BindPostTaskToCurrentDefault(base::BindRepeating(&FileVideoCaptureDeviceTest::OnFrameCaptured, base::Unretained(this))));
    }
    ...
    void SetPhotoOptions(double pan, double tilt, double zoom)
    {
        ...
        photo_settings->has_pan = photo_settings->has_tilt = photo_settings->has_zoom = true;
        ...
        VideoCaptureDevice::SetPhotoOptionsCallback scoped_set_callback
            = base::BindOnce(&MockImageCaptureClient::DoOnSetPhotoOptions, base::Unretained(&image_capture_client_));
        device_->SetPhotoOptions(std::move(photo_settings), std::move(scoped_set_callback));
...
TEST_F(FileVideoCaptureDeviceTest, GetPhotoState)
{
    VideoCaptureDevice::GetPhotoStateCallback scoped_get_callback
        = base::BindOnce(&MockImageCaptureClient::DoOnGetPhotoState, base::Unretained(&image_capture_client_));
...
TEST_F(FileVideoCaptureDeviceTest, TakePhoto)
{
    ...
    VideoCaptureDevice::TakePhotoCallback scoped_callback = base::BindOnce(&MockImageCaptureClient::DoOnPhotoTaken, base::Unretained(&image_capture_client_));

    base::RunLoop run_loop;
    base::RepeatingClosure quit_closure = base::BindPostTaskToCurrentDefault(run_loop.QuitClosure());
    EXPECT_CALL(image_capture_client_, OnCorrectPhotoTaken()).Times(1).WillOnce(InvokeWithoutArgs([quit_closure]() { quit_closure.Run(); }));
...
TEST_F(FileVideoCaptureDeviceTest, CaptureWithGpuMemoryBuffer)
{
    ...
    auto device = std::make_unique<FileVideoCaptureDevice>(GetTestDataFilePath("bear.mjpeg"), std::make_unique<FakeGpuMemoryBufferSupport>());
