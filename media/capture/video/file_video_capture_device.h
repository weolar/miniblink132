...
public:
    ...
    static bool GetVideoCaptureFormat(const base::FilePath& file_path, VideoCaptureFormat* video_format);
    ...
    explicit FileVideoCaptureDevice(const base::FilePath& file_path, std::unique_ptr<gpu::GpuMemoryBufferSupport> gmb_support = nullptr);
...
private:
    ...
    static std::unique_ptr<VideoFileParser> GetVideoFileParser(const base::FilePath& file_path, VideoCaptureFormat* video_format);

    // Crops frame with respect to PTZ settings.
    std::vector<uint8_t> CropPTZRegion(const uint8_t* frame, size_t frame_buffer_size, VideoPixelFormat* final_pixel_format);

    // Called on the |capture_thread_|.
    void OnAllocateAndStart(const VideoCaptureParams& params, std::unique_ptr<Client> client);
    void OnGetPhotoState(GetPhotoStateCallback callback);
    void OnSetPhotoOptions(mojom::PhotoSettingsPtr settings, SetPhotoOptionsCallback callback);
