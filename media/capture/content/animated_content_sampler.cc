...
AnimatedContentSampler::AnimatedContentSampler(base::TimeDelta min_capture_period)
...
void AnimatedContentSampler::ConsiderPresentationEvent(const gfx::Rect& damage_rect, base::TimeTicks event_time)
{
    // Analyze the current event and recent history to determine whether animating
    // content is detected.
    AddObservation(damage_rect, event_time);
    if (!AnalyzeObservations(event_time, &detected_region_, &detected_period_) || detected_period_ <= base::TimeDelta()
    ...
    sampling_period_ = ComputeSamplingPeriod(detected_period_, target_sampling_period_, min_capture_period_);
...
void AnimatedContentSampler::AddObservation(const gfx::Rect& damage_rect, base::TimeTicks event_time)
{
    ...
    while ((event_time - observations_.front().event_time) > kMaxObservationWindow)
...
gfx::Rect AnimatedContentSampler::ElectMajorityDamageRect() const
{
    ...
    int64_t votes = 0;
    for (ObservationFifo::const_iterator i = observations_.begin(); i != observations_.end(); ++i) {
...
bool AnimatedContentSampler::AnalyzeObservations(base::TimeTicks event_time, gfx::Rect* rect, base::TimeDelta* period) const
{
    ...
    for (const auto& observation : base::Reversed(observations_)) {
        ...
        if (last_event_time.is_null()) {
            ...
            const base::TimeDelta frame_duration = first_event_time - observation.event_time;
...
base::TimeTicks AnimatedContentSampler::ComputeNextFrameTimestamp(base::TimeTicks event_time) const
{
    ...
    const int64_t correct_over_num_frames = kDriftCorrection.IntDiv(sampling_period_);
...
base::TimeDelta AnimatedContentSampler::ComputeSamplingPeriod(
    base::TimeDelta animation_period, base::TimeDelta target_sampling_period, base::TimeDelta min_capture_period)
{
