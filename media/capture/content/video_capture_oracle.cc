...
constexpr auto kAnimationHaltPeriodBeforeCaptureAllowed = base::Milliseconds(250);
...
VideoCaptureOracle::VideoCaptureOracle(bool enable_auto_throttling)
    : capture_size_throttling_mode_(enable_auto_throttling ? kThrottlingEnabled : kThrottlingDisabled)
...
{
    VLOG(1) << "Capture size auto-throttling is now " << (enable_auto_throttling ? "enabled." : "disabled.");
}
...
void VideoCaptureOracle::SetCaptureSizeConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio)
{
    resolution_chooser_.SetConstraints(min_size, max_size, use_fixed_aspect_ratio);
}

void VideoCaptureOracle::SetAutoThrottlingEnabled(bool enabled)
{
    const bool was_enabled = (capture_size_throttling_mode_ != kThrottlingDisabled);
    if (was_enabled == enabled)
        return;
    capture_size_throttling_mode_ = enabled ? kThrottlingEnabled : kThrottlingDisabled;
    VLOG(1) << "Capture size auto-throttling is now " << (enabled ? "enabled." : "disabled.");
...
bool VideoCaptureOracle::ObserveEventAndDecideCapture(Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time)
{
    DCHECK_GE(event, 0);
    ...
    case kRefreshRequest:
        ...
        if (num_frames_pending_ == 0) {
            if (!content_sampler_.HasProposal() || ((event_time - last_time_animation_was_detected_) > kAnimationHaltPeriodBeforeCaptureAllowed)) {
                smoothing_sampler_.ConsiderPresentationEvent(event_time);
                should_sample = smoothing_sampler_.ShouldSample();
            }
        }
    ...
    if (duration_of_next_frame_.is_zero()) {
        ...
        duration_of_next_frame_ = std::max(std::min(duration_of_next_frame_, kUpperBoundsDurationEstimate), min_capture_period());
    }
    ...
    if (next_frame_number_ == 0) {
        ...
        if (time_since_last_change >= min_size_change_period_ || capture_size_throttling_mode_ != kThrottlingActive) {
...
{
    VLOG(1) << "Client rejects proposal to capture frame (at #" << next_frame_number_ << ").";
...
bool VideoCaptureOracle::CompleteCapture(int frame_number, bool capture_was_successful, base::TimeTicks* frame_timestamp)
{
    num_frames_pending_--;
    ...
    if (last_successfully_delivered_frame_number_ > frame_number) {
        LOG_IF(WARNING, capture_was_successful) << "Out of order frame delivery detected (have #" << frame_number << ", last was #"
                                                << last_successfully_delivered_frame_number_ << ").  Dropping frame.";
        return false;
    }
    ...
    *frame_timestamp = GetFrameTimestamp(frame_number);
    ...
    if (VLOG_IS_ON(3) && frame_number > 0) {
        ...
        if (content_sampler_.HasProposal()) {
            ...
            VLOG_STREAM(3) << base::StringPrintf("Captured #%d: delta=%" PRId64 " usec"
                                                 ", now locked into {%s}, %+0.1f%% slower than %d FPS",
                frame_number, delta.InMicroseconds(), content_sampler_.detected_region().ToString().c_str(),
                100.0 * FractionFromExpectedFrameRate(delta, rounded_frame_rate), rounded_frame_rate);
        } else {
            VLOG_STREAM(3) << base::StringPrintf("Captured #%d: delta=%" PRId64 " usec"
                                                 ", d/30fps=%+0.1f%%, d/25fps=%+0.1f%%, d/24fps=%+0.1f%%",
                frame_number, delta.InMicroseconds(), 100.0 * FractionFromExpectedFrameRate(delta, 30), 100.0 * FractionFromExpectedFrameRate(delta, 25),
...
void VideoCaptureOracle::RecordConsumerFeedback(int frame_number, const media::VideoCaptureFeedback& feedback)
{
    // Max frame-rate constraint.

    base::TimeDelta period;
    if (std::isfinite(feedback.max_framerate_fps) && feedback.max_framerate_fps > 0.0) {
        period = std::max(min_capture_period_, base::Hertz(feedback.max_framerate_fps));
    } else {
    ...
    if (!std::isfinite(feedback.resource_utilization)) {
        LOG(DFATAL) << "Non-finite utilization provided by consumer for frame #" << frame_number << ": " << feedback.resource_utilization;
        return;
    }
    ...
    if (capture_size_throttling_mode_ != kThrottlingActive) {
        VLOG(1) << "Received consumer feedback at frame #" << frame_number << "; activating capture size auto-throttling.";
    ...
    const int area_at_full_utilization = base::saturated_cast<int>(capture_size_.GetArea() / feedback.resource_utilization);
...
void VideoCaptureOracle::SetFrameTimestamp(int frame_number, base::TimeTicks timestamp)
{
    DCHECK(IsFrameInRecentHistory(frame_number));
    frame_timestamps_[frame_number % kMaxFrameTimestamps] = timestamp;
}

NOINLINE bool VideoCaptureOracle::IsFrameInRecentHistory(int frame_number) const
{
...
void VideoCaptureOracle::CommitCaptureSizeAndReset(base::TimeTicks last_frame_time)
{
...
void VideoCaptureOracle::AnalyzeAndAdjust(const base::TimeTicks analyze_time)
{
    ...
    if (decreased_area > 0) {
        ...
        if (!emit_log_message_cb_.is_null()) {
            emit_log_message_cb_.Run(base::StringPrintf("VFC: CaptureOracle - Decreasing resolution. "
                                                        "buffer_utilization_: %lf "
                                                        "estimated_cappable_area: %lf "
                                                        "capture_size: %s ",
                buffer_pool_utilization_.current(), estimated_capable_area_.current(), capture_size_.ToString().c_str()));
        }
        return;
    }
...
int VideoCaptureOracle::AnalyzeForDecreasedArea(base::TimeTicks analyze_time)
{
    ...
    if (capable_area < current_area) {
        decreased_area = std::min(capable_area, resolution_chooser_.FindSmallerFrameSize(current_area, 1).GetArea());
        VLOG_IF(2, !start_time_of_underutilization_.is_null()) << "Contiguous period of under-utilization ends: "
                                                                  "System is suddenly over-utilized.";
        start_time_of_underutilization_ = base::TimeTicks();
        VLOG(2) << "Proposing a " << (100.0 * (current_area - decreased_area) / current_area) << "% decrease in capture area.  :-(";
    }
    ...
    VLOG(decreased_area == -1 ? 3 : 2) << "Capability of pool=" << (100.0 * buffer_capable_area / current_area)
...
int VideoCaptureOracle::AnalyzeForIncreasedArea(base::TimeTicks analyze_time)
{
    ...
    if (buffer_pool_utilization_.current() > 0.0) {
        const int buffer_capable_area = base::saturated_cast<int>(current_area / buffer_pool_utilization_.current());
    ...
    if (start_time_of_underutilization_ - source_size_change_time_ <= GetExplorationPeriodAfterSourceSizeChange()) {
        VLOG(2) << "Proposing a " << (100.0 * (increased_area - current_area) / current_area) << "% increase in capture area after source size change.  :-)";
    ...
    if ((analyze_time - last_time_animation_was_detected_) < kDebouncingPeriodForAnimatedContent) {
        if ((analyze_time - start_time_of_underutilization_) < kProvingPeriodForAnimatedContent) {
            ...
            VLOG(2) << "Proposing a *cautious* " << (100.0 * (increased_area - current_area) / current_area)
    ...
    VLOG(2) << "Proposing a " << (100.0 * (increased_area - current_area) / current_area) << "% increase in capture area for non-animating content.  :-)";
    return increased_area;
}

base::TimeDelta VideoCaptureOracle::GetExplorationPeriodAfterSourceSizeChange()
{
    return 3 * min_size_change_period_;
}

bool VideoCaptureOracle::HasSufficientRecentFeedback(const FeedbackSignalAccumulator<base::TimeTicks>& accumulator, base::TimeTicks now)
{
    const base::TimeDelta amount_of_history = accumulator.update_time() - accumulator.reset_time();
    return (amount_of_history >= min_size_change_period_) && (now - accumulator.update_time() <= kMaxTimeSinceLastFeedbackUpdate);
}

void VideoCaptureOracle::SetLogCallback(base::RepeatingCallback<void(const std::string&)> emit_log_cb)
{
