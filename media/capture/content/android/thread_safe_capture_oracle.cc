...
ThreadSafeCaptureOracle::ThreadSafeCaptureOracle(std::unique_ptr<VideoCaptureDevice::Client> client, const VideoCaptureParams& params)
    : client_(std::move(client))
    , oracle_(false)
    , params_(params)
{
    DCHECK_GE(params.requested_format.frame_rate, 1e-6f);
    oracle_.SetMinCapturePeriod(base::Microseconds(static_cast<int64_t>(1000000.0 / params.requested_format.frame_rate + 0.5 /* to round to nearest int */)));
    const auto constraints = params.SuggestConstraints();
    oracle_.SetCaptureSizeConstraints(constraints.min_frame_size, constraints.max_frame_size, constraints.fixed_aspect_ratio);
}

ThreadSafeCaptureOracle::~ThreadSafeCaptureOracle() = default;

bool ThreadSafeCaptureOracle::ObserveEventAndDecideCapture(VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time,
    scoped_refptr<VideoFrame>* storage, CaptureFrameCallback* callback)
{
    ...
    base::TimeDelta estimated_frame_duration;
    {
        ...
        if (!oracle_.ObserveEventAndDecideCapture(event, damage_rect, event_time)) {
            ...
            TRACE_EVENT_INSTANT1("gpu.capture", "FpsRateLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event));
            return false;
        }

        frame_number = oracle_.next_frame_number();
        visible_size = oracle_.capture_size();
        coded_size.SetSize(base::bits::AlignUp(visible_size.width(), 16), base::bits::AlignUp(visible_size.height(), 16));

        const auto result_code = client_->ReserveOutputBuffer(coded_size, params_.requested_format.pixel_format, frame_number, &output_buffer);
        ...
        if (result_code != VideoCaptureDevice::Client::ReserveResult::kSucceeded) {
            TRACE_EVENT_INSTANT2("gpu.capture", "PipelineLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event),
                "atten_util_percent", base::saturated_cast<int>(attenuated_utilization * 100.0 + 0.5));
    ...
    if (attenuated_utilization >= 1.0) {
        TRACE_EVENT_INSTANT2("gpu.capture", "NearlyPipelineLimited", TRACE_EVENT_SCOPE_THREAD, "trigger", VideoCaptureOracle::EventAsString(event),
            "atten_util_percent", base::saturated_cast<int>(attenuated_utilization * 100.0 + 0.5));
    }

    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2("gpu.capture", "Capture", TRACE_ID_WITH_SCOPE("Capture", output_buffer.id), "frame_number", frame_number, "trigger",
        VideoCaptureOracle::EventAsString(event));

    std::unique_ptr<VideoCaptureBufferHandle> output_buffer_access = output_buffer.handle_provider->GetHandleForInProcessAccess();
    *storage = VideoFrame::WrapExternalData(params_.requested_format.pixel_format, coded_size, gfx::Rect(visible_size), visible_size,
        output_buffer_access->data(), output_buffer_access->mapped_size(), base::TimeDelta());
    ...
    std::unique_ptr<InFlightFrameCapture> capture(
        new InFlightFrameCapture { frame_number, std::move(output_buffer), std::move(output_buffer_access), capture_begin_time, estimated_frame_duration });
    ...
    *callback = base::BindOnce(&ThreadSafeCaptureOracle::DidCaptureFrame, this, std::move(capture));
...
void ThreadSafeCaptureOracle::ReportError(media::VideoCaptureError error, const base::Location& from_here, const std::string& reason)
{
    base::AutoLock guard(lock_);
    if (client_)
...
void ThreadSafeCaptureOracle::DidCaptureFrame(
    std::unique_ptr<InFlightFrameCapture> capture, scoped_refptr<VideoFrame> frame, base::TimeTicks reference_time, bool success)
{
    ...
    const bool should_deliver_frame = oracle_.CompleteCapture(capture->frame_number, success, &reference_time);

    TRACE_EVENT_NESTABLE_ASYNC_END2("gpu.capture", "Capture", TRACE_ID_WITH_SCOPE("Capture", capture->buffer.id), "success", should_deliver_frame, "timestamp",
    ...
    media::VideoCaptureFormat format(frame->coded_size(), params_.requested_format.frame_rate, frame->format());
    client_->OnIncomingCapturedBufferExt(
        std::move(capture->buffer), format, frame->ColorSpace(), reference_time, frame->timestamp(), frame->visible_rect(), frame->metadata());
}

void ThreadSafeCaptureOracle::OnConsumerReportingUtilization(int frame_number, const media::VideoCaptureFeedback& feedback)
{
    base::AutoLock guard(lock_);
