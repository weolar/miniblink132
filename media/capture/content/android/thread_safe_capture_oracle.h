...
class CAPTURE_EXPORT ThreadSafeCaptureOracle : public base::RefCountedThreadSafe<ThreadSafeCaptureOracle> {
public:
    ThreadSafeCaptureOracle(std::unique_ptr<VideoCaptureDevice::Client> client, const VideoCaptureParams& params);
    ...
    typedef base::OnceCallback<void(scoped_refptr<VideoFrame> frame, base::TimeTicks timestamp, bool success)> CaptureFrameCallback;
    ...
    // should be run.
    bool ObserveEventAndDecideCapture(VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time,
    ...
    void ReportError(media::VideoCaptureError error, const base::Location& from_here, const std::string& reason);

    // Signal device started to the client.
    void ReportStarted();

    void OnConsumerReportingUtilization(int frame_number, const media::VideoCaptureFeedback& feedback);
...
private:
    ...
    void DidCaptureFrame(std::unique_ptr<InFlightFrameCapture> capture, scoped_refptr<VideoFrame> frame, base::TimeTicks reference_time, bool success);
    ...
    void DidConsumeFrame(int frame_number, const media::VideoFrameMetadata* metadata);
