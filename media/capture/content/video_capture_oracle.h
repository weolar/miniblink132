...
public:
    ...
    // |max_size| is used.
    void SetCaptureSizeConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio);
    ...
    {
        return resolution_chooser_.source_size();
    }
    ...
    bool ObserveEventAndDecideCapture(Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time);
    ...
    // frame.
    virtual bool CompleteCapture(int frame_number, bool capture_was_successful, base::TimeTicks* frame_timestamp);
    ...
    // returned true.
    void RecordConsumerFeedback(int frame_number, const media::VideoCaptureFeedback& feedback);
    ...
    static constexpr base::TimeDelta kDefaultMinCapturePeriod = base::Milliseconds(200); // 5 FPS

    // Default minimum size change period if SetMinSizeChangePeriod is not called.
    static constexpr base::TimeDelta kDefaultMinSizeChangePeriod = base::Seconds(3);

    void SetLogCallback(base::RepeatingCallback<void(const std::string&)> emit_log_cb);
...
private:
    ...
    // to |now|.
    bool HasSufficientRecentFeedback(const FeedbackSignalAccumulator<base::TimeTicks>& accumulator, base::TimeTicks now);
    ...
    // for the first time.
    enum { kThrottlingDisabled, kThrottlingEnabled, kThrottlingActive } capture_size_throttling_mode_;
