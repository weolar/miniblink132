...
gfx::Size ComputeBoundedCaptureSize(const gfx::Size& size, const gfx::Size& min_size, const gfx::Size& max_size)
{
...
void CaptureResolutionChooser::SetConstraints(const gfx::Size& min_frame_size, const gfx::Size& max_frame_size, bool use_fixed_aspect_ratio)
{
    ...
    apply_aspect_ratio_adjustment_ = (min_frame_size != max_frame_size && use_fixed_aspect_ratio);
...
gfx::Size CaptureResolutionChooser::FindLargerFrameSize(int area, int num_steps_up) const
{
...
gfx::Size CaptureResolutionChooser::FindSmallerFrameSize(int area, int num_steps_down) const
{
...
void CaptureResolutionChooser::RecomputeCaptureSize()
{
    ...
    VLOG_IF(1, capture_size_ != old_capture_size) << "Recomputed capture size from " << old_capture_size.ToString() << " to " << capture_size_.ToString()
                                                  << " (" << (100.0 * capture_size_.height() / snapped_sizes_.back().height()) << "% of ideal size)";
}
...
void CaptureResolutionChooser::UpdateSnappedFrameSizes()
{
    ...
    gfx::Size constrained_size = source_size_.IsEmpty() ? capture_size_ : source_size_;
    constrained_size = ComputeBoundedCaptureSize(
        apply_aspect_ratio_adjustment_ ? PadToMatchAspectRatio(constrained_size, max_frame_size_) : constrained_size, min_frame_size_, max_frame_size_);
    ...
    for (int height = constrained_size.height() - kSnappedHeightStep; height >= min_frame_size_.height(); height -= kSnappedHeightStep) {
    ...
    if (VLOG_IS_ON(1)) {
        ...
        VLOG_STREAM(1) << "Recomputed snapped frame sizes: " << base::JoinString(stringified_sizes, " <--> ");
    }
}
