...
public:
    ...
    void SetUp() override
    {
        rand_seed_ = static_cast<uint32_t>((InitialTestTimeTicks() - base::TimeTicks()).InMicroseconds());
...
protected:
    ...
    {
        sampler_->observations_.push_back(AnimatedContentSampler::Observation(damage_rect, base::TimeTicks()));
    }
    ...
    static base::TimeDelta ComputeSamplingPeriod(base::TimeDelta detected_period, base::TimeDelta target_sampling_period, base::TimeDelta min_capture_period)
    {
        return AnimatedContentSampler::ComputeSamplingPeriod(detected_period, target_sampling_period, min_capture_period);
    }
    ...
    struct Helper {
        static void RunTargetSamplingPeriodTest(int target_fps)
        {
            ...
            for (int content_fps = 1; content_fps <= 60; ++content_fps) {
                ...
                const base::TimeDelta sampling_period = ComputeSamplingPeriod(content_period, target_sampling_period, min_capture_period);
                if (content_period >= target_sampling_period) {
                    ...
                    for (double divisor = 1; divisor < 4; ++divisor) {
                        SCOPED_TRACE(::testing::Message() << "target_fps=" << target_fps << ", content_fps=" << content_fps << ", divisor=" << divisor);
                        ASSERT_GE(std::abs(content_fps / divisor - target_fps), absolute_diff - fudge_for_acceptable_rounding_error);
                    }
                }
            }
        }
...
struct Scenario {
    ...
    {
        CHECK(content_period >= vsync_interval) << "Bad test params: Impossible to animate faster than the compositor.";
    }

    Scenario(int compositor_frequency, int max_frame_rate, int content_frame_rate, int target_sampling_rate)
        : vsync_interval(FpsAsPeriod(compositor_frequency))
        , min_capture_period(FpsAsPeriod(max_frame_rate))
        , content_period(FpsAsPeriod(content_frame_rate))
...
{
    return os << "{ vsync_interval=" << s.vsync_interval.InMicroseconds() << ", min_capture_period=" << s.min_capture_period.InMicroseconds()
              << ", content_period=" << s.content_period.InMicroseconds() << " }";
}

} // namespace

class AnimatedContentSamplerParameterizedTest : public AnimatedContentSamplerTest, public ::testing::WithParamInterface<Scenario> {
public:
...
protected:
    ...
    {
        return AnimatedContentSamplerTest::ComputeSamplingPeriod(GetParam().content_period, GetParam().target_sampling_period, GetParam().min_capture_period);
    }
    ...
    std::vector<Event> GenerateEventSequence(
        base::TimeTicks begin, base::TimeTicks end, bool include_content_frame_events, bool include_random_events, base::TimeTicks* next_begin_time)
    {
        ...
        for (compositor_time = begin; compositor_time < end; compositor_time += GetParam().vsync_interval) {
    ...
    void RunEventSequence(
        const std::vector<Event> events, bool was_detecting_before, bool is_detecting_after, bool simulate_pipeline_back_pressure, const char* description)
    {
        ...
        for (auto i = events.begin(); i != events.end(); ++i) {
            ...
            ASSERT_EQ(has_detection_switched ? is_detecting_after : was_detecting_before, sampler()->HasProposal());
    ...
    void ExpectFrameDropRatioIsCorrect()
    {
        ...
        const double expected_sampling_ratio = GetParam().content_period / ComputeExpectedSamplingPeriod();
        const int total_frames = count_dropped_frames_ + count_sampled_frames_;
        EXPECT_NEAR(total_frames * expected_sampling_ratio, count_sampled_frames_, 1.5);
        EXPECT_NEAR(total_frames * (1.0 - expected_sampling_ratio), count_dropped_frames_, 1.5);
    }
...
TEST_P(AnimatedContentSamplerParameterizedTest, DetectsAnimatedContent)
{
    ...
    RunEventSequence(
        GenerateEventSequence(begin, begin + base::Seconds(5), false, true, &begin), false, false, false, "Provide random events and expect no lock-in.");
    if (HasFailure())
        return;

    // Provide content frame events with some random events mixed-in, and expect
    // the sampler to lock-in.
    RunEventSequence(GenerateEventSequence(begin, begin + base::Seconds(5), true, true, &begin), false, true, false,
    ...
    RunEventSequence(GenerateEventSequence(begin, begin + base::Seconds(5), true, false, &begin), true, true, false,
    ...
    RunEventSequence(GenerateEventSequence(begin, begin + base::Seconds(20), true, false, &begin), true, true, true,
        "Continue providing just content frame events and expect the lock-in to "
        "hold.  Also simulate the capture pipeline experiencing back pressure.");
    if (HasFailure())
        return;
    ...
    // broken.
    RunEventSequence(GenerateEventSequence(begin, begin + base::Milliseconds(500), false, true, &begin), true, false, false,
    ...
    // lock-in once again.
    RunEventSequence(GenerateEventSequence(begin, begin + base::Seconds(5), true, false, &begin), false, true, false,
...
TEST_P(AnimatedContentSamplerParameterizedTest, DoesNotLockInToTwoCompetingAnimations)
{
    ...
    std::vector<Event> first_animation_events = GenerateEventSequence(begin, begin + base::Seconds(20), true, false, &begin);
    gfx::Rect second_animation_rect(gfx::Point(0, GetContentDamageRect().height()), GetContentDamageRect().size());
    std::vector<Event> both_animations_events;
    base::TimeDelta second_animation_offset = GetParam().vsync_interval;
    for (std::vector<Event>::const_iterator i = first_animation_events.begin(); i != first_animation_events.end(); ++i) {
        both_animations_events.push_back(*i);
        both_animations_events.push_back(Event(second_animation_rect, i->second + second_animation_offset));
    }
    ...
    first_animation_events = GenerateEventSequence(begin, begin + base::Seconds(20), true, false, &begin);
...
TEST_P(AnimatedContentSamplerParameterizedTest, FrameTimestampsAreSmooth)
{
    ...
    std::vector<Event> events = GenerateEventSequence(begin, begin + base::Seconds(20), true, false, nullptr);
    ...
    const int64_t max_expected_repeats_per_frame = 1 + ComputeExpectedSamplingPeriod().IntDiv(GetParam().vsync_interval);
    ...
    for (Timestamps::const_iterator i = frame_timestamps.begin() + 1; i != frame_timestamps.end(); ++i) {
        const size_t num_vsync_intervals = base::ClampFloor<size_t>((*i - last_present_time) / GetParam().vsync_interval);
    ...
    for (size_t repeats = 1; repeats < display_counts.size(); ++repeats) {
        DVLOG(1) << "display_counts[" << repeats << "] is " << display_counts[repeats];
...
TEST_P(AnimatedContentSamplerParameterizedTest, FrameTimestampsConvergeTowardsEventTimes)
{
    const int max_drift_increment_millis = 3;

    // Generate a full minute of events.
    const base::TimeTicks begin = InitialTestTimeTicks();
    std::vector<Event> events = GenerateEventSequence(begin, begin + base::Minutes(1), true, false, nullptr);
    ...
    for (size_t i = 1; i < events.size(); ++i) {
        if (i % 100 == 0) {
            accumulated_drift += base::Milliseconds(GetRandomInRange(1, max_drift_increment_millis + 1));
        }
    ...
    const base::TimeDelta max_acceptable_error = GetParam().min_capture_period + base::Milliseconds(max_drift_increment_millis);
    EXPECT_NEAR(0.0, total_error.InMicroseconds(), max_acceptable_error.InMicroseconds());
}

INSTANTIATE_TEST_SUITE_P(All, AnimatedContentSamplerParameterizedTest,
    ::testing::Values(
        // Typical frame rate content: Compositor runs at 60 Hz, capture at 30
        // Hz, and content video animates at 30, 25, or 24 Hz.
        Scenario(60, 30, 30), Scenario(60, 30, 25), Scenario(60, 30, 24),

        // High frame rate content that leverages the Compositor's
        // capabilities, but capture is still at 30 Hz.
        Scenario(60, 30, 60), Scenario(60, 30, 50), Scenario(60, 30, 48),

        // High frame rate content that leverages the Compositor's
        // capabilities, and capture is also a buttery 60 Hz.
        Scenario(60, 60, 60), Scenario(60, 60, 50), Scenario(60, 60, 48),

        // High frame rate content that leverages the Compositor's
        // capabilities, but the client has disabled HFR sampling.
        Scenario(60, 60, 60, 30), Scenario(60, 60, 50, 30), Scenario(60, 60, 48, 30),

        // On some platforms, the Compositor runs at 50 Hz.
        Scenario(50, 30, 30), Scenario(50, 30, 25), Scenario(50, 30, 24), Scenario(50, 30, 50), Scenario(50, 30, 48),

        // Stable, but non-standard content frame rates.
        Scenario(60, 30, 16), Scenario(60, 30, 20), Scenario(60, 30, 23), Scenario(60, 30, 26), Scenario(60, 30, 27), Scenario(60, 30, 28),
        Scenario(60, 30, 29), Scenario(60, 30, 31), Scenario(60, 30, 32), Scenario(60, 30, 33)));
