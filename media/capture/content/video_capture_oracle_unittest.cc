...
constexpr base::TimeTicks kInitialTestTimeTicks = base::TimeTicks() + base::Seconds(1);
...
TEST(VideoCaptureOracleTest, EnforcesEventTimeMonotonicity)
{
    ...
    for (int i = 0; i < 10; ++i) {
        t += event_increment;
        ASSERT_TRUE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, damage_rect, t));
    }

    base::TimeTicks furthest_event_time = t;
    for (int i = 0; i < 10; ++i) {
        t -= event_increment;
        ASSERT_FALSE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, damage_rect, t));
    }
...
TEST(VideoCaptureOracleTest, EnforcesFramesDeliveredInOrder)
{
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        for (int j = num_in_flight - 1; j >= 0; --j) {
            ASSERT_TRUE(oracle.CompleteCapture(last_frame_number - j, true, &ignored));
        }
    }
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        for (int j = 1; j < num_in_flight; ++j) {
            ASSERT_FALSE(oracle.CompleteCapture(last_frame_number - j, true, &ignored));
        }
    }
...
TEST(VideoCaptureOracleTest, TransitionsSmoothlyBetweenSamplers)
{
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        const bool oracle_says_sample = oracle.ObserveEventAndDecideCapture(
            VideoCaptureOracle::kCompositorUpdate, provide_animated_content_event ? animation_damage_rect : gfx::Rect(), t);
...
TEST(VideoCaptureOracleTest, SamplesAtCorrectTimesAroundRefreshRequests)
{
    ...
    for (int i = 0; i < 10; ++i) {
        t += vsync_interval;
        if (oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(), t)) {
            const int frame_number = oracle.next_frame_number();
            oracle.RecordCapture(0.0);
    ...
    for (int i = 0; i < 10; ++i) {
        t += refresh_interval;
        ASSERT_FALSE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kRefreshRequest, gfx::Rect(), t));
    }
    ...
    for (int i = 0; i < 10; ++i) {
        t += refresh_interval;
        if (oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kRefreshRequest, gfx::Rect(), t)) {
...
TEST(VideoCaptureOracleTest, DoesNotRapidlyChangeCaptureSize)
{
    ...
    for (; t < end_t; t += event_increment) {
        ASSERT_TRUE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(), t));
        ...
        // auto-throttling.
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(0.1));
    }
    ...
    for (; t < end_t; t += event_increment) {
        ...
        source_size.SetSize(((last_source_size.width() * 11 + 12345) % 1280) + 1, ((last_source_size.height() * 11 + 12345) % 720) + 1);
        ...
        oracle.RecordCapture(0.0);
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(0.0));
    }
}
...
// feedback varies.
void RunAutoThrottleTest(bool is_content_animating, bool with_consumer_feedback)
{
    SCOPED_TRACE(::testing::Message() << "RunAutoThrottleTest("
                                      << "(is_content_animating=" << is_content_animating << ", with_consumer_feedback=" << with_consumer_feedback << ")");
    ...
    for (; t < end_t; t += event_increment) {
        ASSERT_TRUE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, is_content_animating ? gfx::Rect(k720pSize) : gfx::Rect(), t));
        ASSERT_EQ(k720pSize, oracle.capture_size());
        ...
        if (with_consumer_feedback) {
            oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(utilization));
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        SCOPED_TRACE(::testing::Message() << "Stepping down from " << starting_size.ToString() << ", i=" << i);

        gfx::Size stepped_down_size;
        end_t = t + base::Seconds(10);
        for (; t < end_t; t += event_increment) {
            ASSERT_TRUE(
                oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, is_content_animating ? gfx::Rect(k720pSize) : gfx::Rect(), t));
    ...
    for (int i = 0; i < 2; ++i) {
        const gfx::Size starting_size = oracle.capture_size();
        SCOPED_TRACE(::testing::Message() << "Stepping up from " << starting_size.ToString() << ", i=" << i);
        ...
        for (; t < end_t; t += event_increment) {
            ...
            if (stepped_up_size.IsEmpty()) {
                if (oracle.capture_size() != starting_size) {
                    ...
                    ASSERT_LT(base::Seconds(is_content_animating ? 15 : 1), t - time_of_last_size_change);
                    time_of_last_size_change = t;
...
TEST(VideoCaptureOracleTest, AutoThrottlesCaptureSizeBasedOnUtilizationFeedback)
{
...
TEST(VideoCaptureOracleTest, DoesNotAutoThrottleCaptureSizeWithoutConsumerFeedback)
{
    VideoCaptureOracle oracle(true);
    oracle.SetMinCapturePeriod(k30HzPeriod);
    oracle.SetCaptureSizeConstraints(kSmallestNonEmptySize, k720pSize, false);
    ...
    base::TimeTicks end_t = t + base::Seconds(10);
    for (; t < end_t; t += event_increment) {
        ASSERT_TRUE(oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(k720pSize), t));
        ASSERT_EQ(k720pSize, oracle.capture_size());
        const int frame_number = oracle.next_frame_number();
...
TEST(VideoCaptureOracleTest, IncreasesFrequentlyOnlyAfterSourceSizeChange)
{
    ...
    base::TimeTicks end_t = t + base::Seconds(10);
    for (; t < end_t; t += event_increment) {
        if (!oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(k360pSize), t)) {
            continue;
        }
        ...
        base::TimeTicks ignored;
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(0.25));
    }
    ...
    for (; t < end_t; t += event_increment) {
        if (!oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(k720pSize), t)) {
            continue;
        }
    ...
    end_t = t + base::Seconds(10);
    for (; t < end_t; t += event_increment) {
        if (!oracle.ObserveEventAndDecideCapture(VideoCaptureOracle::kCompositorUpdate, gfx::Rect(k1080pSize), t)) {
            continue;
        }
...
TEST(VideoCaptureOracleTest, DoesNotAutoThrottleWhenResolutionIsFixed)
{
    ...
    for (; t < end_t; t += event_increment) {
        ...
        oracle.RecordCapture(0.9);
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(0.9));
    }
    ...
    for (; t < end_t; t += event_increment) {
        ...
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(2.0));
    }
}
...
TEST(VideoCaptureOracleTest, RespectsMaxPixelsFeedback)
{
    ...
    // Now run for a single frame with 360p pixel limit.
    {
        ...
        oracle.RecordCapture(0.25); // Low buffer utilization.
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(-1.0, std::numeric_limits<float>::infinity(), k360pSize.GetArea()));
        t += event_increment;
    }
...
TEST(VideoCaptureOracleTest, IgnoresMaxPixelsFeedbackIfAutoThrottlingIsOn)
{
    ...
    // Now run with a new 360p pixel limit returned in the feedback.
    {
        ...
        oracle.RecordCapture(0.25); // Low buffer utilization.
        ASSERT_TRUE(oracle.CompleteCapture(frame_number, true, &ignored));
        oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(0.25, std::numeric_limits<float>::infinity(), k360pSize.GetArea()));
        t += event_increment;
    }
...
TEST(VideoCaptureOracleTest, RespectsMaxFrameRateFeedback)
{
    ...
    oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(kNoResourceUtilization, k5Fps));
    ...
    oracle.RecordConsumerFeedback(frame_number, media::VideoCaptureFeedback(kNoResourceUtilization, kNoFpsLimit));
