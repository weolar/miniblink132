...
void ExpectIsWithinBoundsAndSameAspectRatio(const Location& location, const gfx::Size& min_size, const gfx::Size& max_size, const gfx::Size& size)
{
    ...
    EXPECT_NEAR(static_cast<double>(max_size.width()) / max_size.height(), static_cast<double>(size.width()) / size.height(), 0.01);
}
...
void TestSnappedFrameSizes(CaptureResolutionChooser* chooser, const gfx::Size& smallest_size)
{
    ...
    for (int i = 1; i < 4; ++i) {
        EXPECT_EQ(largest_size, chooser->FindLargerFrameSize(largest_size.GetArea(), i));
        EXPECT_EQ(largest_size, chooser->FindLargerFrameSize(largest_size.GetArea() * 2, +i));
    }

    // There should be no size smaller than the smallest size.
    for (int i = 1; i < 4; ++i) {
        EXPECT_EQ(smallest_size, chooser->FindSmallerFrameSize(smallest_size.GetArea(), i));
        EXPECT_EQ(smallest_size, chooser->FindSmallerFrameSize(smallest_size.GetArea() / 2, i));
    }
    ...
    for (size_t skips = 1; skips < 4; ++skips) {
        for (size_t i = skips; i < std::size(kSizes); ++i) {
            EXPECT_EQ(
                gfx::Size(kSizes[i][0], kSizes[i][1]), chooser->FindSmallerFrameSize(gfx::Size(kSizes[i - skips][0], kSizes[i - skips][1]).GetArea(), skips));
        }
    }
    ...
    for (size_t skips = 1; skips < 4; ++skips) {
        for (size_t i = skips; i < std::size(kSizes); ++i) {
            EXPECT_EQ(
                gfx::Size(kSizes[i - skips][0], kSizes[i - skips][1]), chooser->FindLargerFrameSize(gfx::Size(kSizes[i][0], kSizes[i][1]).GetArea(), skips));
        }
    }
    ...
    for (size_t i = 1; i < std::size(kSizes) - 1; ++i) {
        ...
        const int a_somewhat_smaller_area = gfx::Size((kSizes[i - 1][0] + 3 * kSizes[i][0]) / 4, (kSizes[i - 1][1] + 3 * kSizes[i][1]) / 4).GetArea();
        ...
        const int a_somewhat_larger_area = gfx::Size((kSizes[i + 1][0] + 3 * kSizes[i][0]) / 4, (kSizes[i + 1][1] + 3 * kSizes[i][1]) / 4).GetArea();
...
void TestTargetedFrameAreas(CaptureResolutionChooser* chooser, const gfx::Size& smallest_size)
{
    ...
    chooser->SetTargetFrameArea(kMaxFrameWidth * kMaxFrameHeight);
    EXPECT_EQ(gfx::Size(kMaxFrameWidth, kMaxFrameHeight), chooser->capture_size());
...
void ExpectOnlyOneSnappedFrameSize(const CaptureResolutionChooser& chooser, const gfx::Size& the_one_size)
{
    for (int i = 1; i < 4; ++i) {
        EXPECT_EQ(the_one_size, chooser.FindNearestFrameSize(the_one_size.GetArea() * i));
        EXPECT_EQ(the_one_size, chooser.FindSmallerFrameSize(the_one_size.GetArea(), i));
        EXPECT_EQ(the_one_size, chooser.FindLargerFrameSize(the_one_size.GetArea(), i));
    }
}
...
TEST(CaptureResolutionChooserTest, DefaultCaptureSizeIfNeverSetConstraints)
{
    ...
    EXPECT_EQ(CaptureResolutionChooser::kDefaultCaptureSize, chooser.capture_size());
    ...
    ExpectOnlyOneSnappedFrameSize(chooser, CaptureResolutionChooser::kDefaultCaptureSize);
}
...
TEST(CaptureResolutionChooserTest, ReasonableCaptureSizeWhenMissingSourceSize)
{
    ...
    chooser.SetConstraints(gfx::Size(kMinFrameWidth, kMinFrameHeight), gfx::Size(kMaxFrameWidth, kMaxFrameHeight), false);
    EXPECT_EQ(CaptureResolutionChooser::kDefaultCaptureSize, chooser.capture_size());
    ...
    chooser.SetConstraints(gfx::Size(kMinFrameWidth, kMinFrameHeight), gfx::Size(kMinFrameWidth, kMinFrameHeight), false);
    ...
    // same size.
    static constexpr gfx::Size middle_size(std::midpoint(kMinFrameWidth, kMaxFrameWidth), std::midpoint(kMinFrameHeight, kMaxFrameHeight));
    chooser.SetSourceSize(middle_size);
    EXPECT_EQ(middle_size, chooser.capture_size());
}

TEST(CaptureResolutionChooserTest, FixedResolutionPolicy_CaptureSizeAlwaysFixed)
{
...
TEST(CaptureResolutionChooserTest, FixedAspectRatioPolicy_CaptureSizeHasSameAspectRatio)
{
    ...
    // Starting condition.
    ExpectIsWithinBoundsAndSameAspectRatio(FROM_HERE, min_size, max_size, chooser.capture_size());
...
TEST(CaptureResolutionChooserTest, AnyWithinLimitPolicy_CaptureSizeIsAnythingWithinLimits)
{
    ...
    chooser.SetSourceSize(gfx::Size(kMaxFrameWidth * 2, kMaxFrameHeight));
    EXPECT_EQ(gfx::Size(kMaxFrameWidth, kMaxFrameHeight / 2), chooser.capture_size());

    chooser.SetSourceSize(gfx::Size(kMaxFrameWidth, kMaxFrameHeight * 2));
    EXPECT_EQ(gfx::Size(kMaxFrameWidth / 2, kMaxFrameHeight), chooser.capture_size());
