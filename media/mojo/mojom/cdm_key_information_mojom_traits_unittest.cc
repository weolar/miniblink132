...
{
    auto input = std::make_unique<CdmKeyInformation>("key_id", CdmKeyInformation::KeyStatus::USABLE, 23);
    std::vector<uint8_t> data = media::mojom::CdmKeyInformation::Serialize(&input);

    std::unique_ptr<CdmKeyInformation> output;
    EXPECT_TRUE(media::mojom::CdmKeyInformation::Deserialize(std::move(data), &output));
