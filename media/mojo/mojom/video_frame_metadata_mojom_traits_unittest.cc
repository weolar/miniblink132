...
class VideoFrameMetadataStructTraitsTest : public testing::Test, public mojom::VideoFrameMetadataTraitsTestService {
public:
    VideoFrameMetadataStructTraitsTest() = default;

    VideoFrameMetadataStructTraitsTest(const VideoFrameMetadataStructTraitsTest&) = delete;
    VideoFrameMetadataStructTraitsTest& operator=(const VideoFrameMetadataStructTraitsTest&) = delete;

protected:
    mojo::Remote<mojom::VideoFrameMetadataTraitsTestService> GetTraitsTestRemote()
    {
    ...
    {
        mojo::Remote<mojom::VideoFrameMetadataTraitsTestService> remote = GetTraitsTestRemote();
        return remote->EchoVideoFrameMetadata(in, out);
    }

private:
    void EchoVideoFrameMetadata(const VideoFrameMetadata& vfm, EchoVideoFrameMetadataCallback callback) override
    {
...
TEST_F(VideoFrameMetadataStructTraitsTest, ValidMetadata)
{
    ...
    EXPECT_EQ(metadata_in.wants_promotion_hint, metadata_out.wants_promotion_hint);
    ...
    EXPECT_EQ(metadata_in.is_webgpu_compatible, metadata_out.is_webgpu_compatible);
    EXPECT_EQ(metadata_in.power_efficient, metadata_out.power_efficient);
    EXPECT_EQ(metadata_in.read_lock_fences_enabled, metadata_out.read_lock_fences_enabled);
    ...
    EXPECT_EQ(metadata_in.root_scroll_offset_x, metadata_out.root_scroll_offset_x);
    EXPECT_EQ(metadata_in.root_scroll_offset_y, metadata_out.root_scroll_offset_y);
    EXPECT_EQ(metadata_in.top_controls_visible_height, metadata_out.top_controls_visible_height);
    ...
    EXPECT_EQ(metadata_in.wallclock_frame_duration, metadata_out.wallclock_frame_duration);
    EXPECT_EQ(metadata_in.frame_sequence, metadata_out.frame_sequence);
    EXPECT_EQ(metadata_in.background_blur->enabled, metadata_out.background_blur->enabled);
}
