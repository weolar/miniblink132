...
class VideoFrameStructTraitsTest : public testing::Test, public media::mojom::TraitsTestService {
public:
    VideoFrameStructTraitsTest() = default;

    VideoFrameStructTraitsTest(const VideoFrameStructTraitsTest&) = delete;
    VideoFrameStructTraitsTest& operator=(const VideoFrameStructTraitsTest&) = delete;
...
private:
    void EchoVideoFrame(const scoped_refptr<VideoFrame>& f, EchoVideoFrameCallback callback) override
    {
    ...
    for (auto format : formats) {
        for (auto storage_type : storage_types) {
            ...
            if (storage_type == VideoFrame::STORAGE_OWNED_MEMORY) {
                frame = media::VideoFrame::CreateFrame(format, kCodedSize, kVisibleRect, kNaturalSize, kTimestamp);
            } else {
                std::vector<int32_t> strides = VideoFrame::ComputeStrides(format, kCodedSize);
                size_t aggregate_size = 0;
                size_t sizes[3] = {};
                for (size_t i = 0; i < strides.size(); ++i) {
                    sizes[i] = media::VideoFrame::Rows(i, format, kCodedSize.height()) * strides[i];
                ...
                if (format == PIXEL_FORMAT_I420) {
                    frame = media::VideoFrame::WrapExternalYuvData(
                        format, kCodedSize, kVisibleRect, kNaturalSize, strides[0], strides[1], strides[2], data[0], data[1], data[2], kTimestamp);
                } else {
                    frame = media::VideoFrame::WrapExternalYuvData(
                        format, kCodedSize, kVisibleRect, kNaturalSize, strides[0], strides[1], data[0], data[1], kTimestamp);
                }
...
TEST_F(VideoFrameStructTraitsTest, InterleavedPlanes)
{
    ...
    frame = media::VideoFrame::WrapExternalYuvData(
        format, kCodedSize, kVisibleRect, kNaturalSize, strides[0], strides[1], strides[2], y_plane.data(), u_plane.data(), v_plane.data(), kTimestamp);
    auto ro_region = base::WritableSharedMemoryRegion::ConvertToReadOnly(std::move(region));
    ...
    for (int i = 0; i < yu_rows; ++i) {
        EXPECT_EQ(0, memcmp(frame->visible_data(1) + i * frame->stride(1), u_plane.data() + i * strides[1], normal_stride));
        EXPECT_EQ(0, memcmp(frame->visible_data(2) + i * frame->stride(2), v_plane.data() + i * strides[2], normal_stride));
    }
}
...
TEST_F(VideoFrameStructTraitsTest, InvalidOffsets)
{
    ...
    auto frame = VideoFrame::WrapExternalYuvData(
        kFormat, kSize, gfx::Rect(kSize), kSize, strides[0], strides[1], strides[2], data[0], data[1], data[2], base::Seconds(1));
    ASSERT_TRUE(frame);
    ...
    base::span<uint32_t> body(reinterpret_cast<uint32_t*>(message.mutable_payload()), message.payload_num_bytes() / sizeof(uint32_t));
    ...
    for (size_t i = 0; i + 3 < body.size(); ++i) {
        if (body[i] == offsets[0] && body[i + 1] == offsets[1] && body[i + 2] == offsets[2]) {
            body[i + 1] = 0xc01db33f;
    ...
    EXPECT_FALSE(mojom::VideoFrame::DeserializeFromMessage(std::move(message), &new_frame));
}

TEST_F(VideoFrameStructTraitsTest, HoleVideoFrame)
{
    base::UnguessableToken overlay_plane_id = base::UnguessableToken::Create();
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateVideoHoleFrame(overlay_plane_id, gfx::Size(200, 100), base::Seconds(100));
...
TEST_F(VideoFrameStructTraitsTest, TrackingTokenVideoFrame)
{
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapTrackingToken(
        PIXEL_FORMAT_ARGB, tracking_token, gfx::Size(100, 100), gfx::Rect(10, 10, 80, 80), gfx::Size(200, 100), base::Seconds(100));
...
TEST_F(VideoFrameStructTraitsTest, SharedImageVideoFrame)
{
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapSharedImage(PIXEL_FORMAT_ARGB, shared_image, gpu::SyncToken(), VideoFrame::ReleaseMailboxCB(),
        gfx::Size(100, 100), gfx::Rect(10, 10, 80, 80), gfx::Size(200, 100), base::Seconds(100));
...
TEST_F(VideoFrameStructTraitsTest, GpuMemoryBufferSharedImageVideoFrame)
{
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = std::make_unique<FakeGpuMemoryBuffer>(coded_size, gfx::BufferFormat::YUV_420_BIPLANAR);
    gfx::BufferFormat expected_gmb_format = gmb->GetFormat();
    gfx::Size expected_gmb_size = gmb->GetSize();
    scoped_refptr<gpu::ClientSharedImage> shared_image = gpu::ClientSharedImage::CreateForTesting();
    auto frame = VideoFrame::WrapExternalGpuMemoryBuffer(
        visible_rect, visible_rect.size(), std::move(gmb), shared_image, gpu::SyncToken(), base::NullCallback(), timestamp);
    ASSERT_TRUE(RoundTrip(&frame));
    ASSERT_TRUE(frame);
    ...
    EXPECT_EQ(frame->GetGpuMemoryBufferForTesting()->GetFormat(), expected_gmb_format);
    EXPECT_EQ(frame->GetGpuMemoryBufferForTesting()->GetSize(), expected_gmb_size);
}
