...
// static
bool StructTraits<media::mojom::HypothesisPartsDataView, media::HypothesisParts>::Read(media::mojom::HypothesisPartsDataView data, media::HypothesisParts* out)
{
...
bool StructTraits<media::mojom::TimingInformationDataView, media::TimingInformation>::Read(
    media::mojom::TimingInformationDataView data, media::TimingInformation* out)
{
    ...
    if (!data.ReadAudioStartTime(&audio_start_time) || !data.ReadAudioEndTime(&audio_end_time) || !data.ReadHypothesisParts(&hypothesis_parts)) {
        return false;
    }
    ...
    if (hypothesis_parts.has_value() && hypothesis_parts->size() > 0) {
        ...
        for (const auto& part : *hypothesis_parts) {
            if (part.hypothesis_part_offset < prev_offset || part.hypothesis_part_offset > max_offset) {
                return false;
            }
...
bool StructTraits<media::mojom::SpeechRecognitionResultDataView, media::SpeechRecognitionResult>::Read(
    media::mojom::SpeechRecognitionResultDataView data, media::SpeechRecognitionResult* out)
{
    std::string transcription;
    std::optional<media::TimingInformation> timing_information;

    if (!data.ReadTranscription(&transcription) || !data.ReadTimingInformation(&timing_information)) {
        return false;
    }
