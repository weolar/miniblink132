...
template <typename T> bool AreUnique(const std::vector<T>& values)
{
...
// static
bool StructTraits<media::mojom::VideoCodecInfoDataView, media::VideoCodecInfo>::Read(media::mojom::VideoCodecInfoDataView input, media::VideoCodecInfo* output)
{
    ...
    *output = media::VideoCodecInfo(std::move(supported_profiles), input.supports_clear_lead());
    return true;
}

// static
bool StructTraits<media::mojom::CdmCapabilityDataView, media::CdmCapability>::Read(media::mojom::CdmCapabilityDataView input, media::CdmCapability* output)
{
    ...
    *output = media::CdmCapability(std::move(audio_codecs), std::move(video_codecs), std::move(encryption_schemes), std::move(session_types));
    return true;
}
