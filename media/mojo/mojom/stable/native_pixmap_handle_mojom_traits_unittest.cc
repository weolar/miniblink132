...
TEST(NativePixmapHandleMojomTraitsTest, ValidNativePixmapPlane)
{
    ...
    native_pixmap_plane.fd = CreateValidLookingBufferHandle(native_pixmap_plane.offset + native_pixmap_plane.size);
    ...
    auto message = stable::mojom::NativePixmapPlane::SerializeAsMessage(&native_pixmap_plane);
    ...
    ASSERT_TRUE(stable::mojom::NativePixmapPlane::DeserializeFromMessage(std::move(received_message), &deserialized_native_pixmap_plane));

    EXPECT_EQ(native_pixmap_plane.stride, deserialized_native_pixmap_plane.stride);
    EXPECT_EQ(native_pixmap_plane.offset, deserialized_native_pixmap_plane.offset);
    ...
    EXPECT_EQ(syscall(SYS_kcmp, pid, pid, KCMP_FILE, duped_fd.get(), deserialized_native_pixmap_plane.fd.get()), 0);
}
...
TEST(NativePixmapHandleMojomTraitsTest, NativePixmapPlaneWithInvalidFd)
{
    ...
    ASSERT_CHECK_DEATH(stable::mojom::NativePixmapPlane::SerializeAsMessage(&native_pixmap_plane));
}
...
TEST(NativePixmapHandleMojomTraitsTest, ValidNativePixmapHandle)
{
    ...
    uint64_t offset = 0;
    uint64_t size = 2500;
    native_pixmap_handle.planes.emplace_back(stride, offset, size, CreateValidLookingBufferHandle(size + offset));
    stride = 25;
    offset = 2500;
    size = 625;
    native_pixmap_handle.planes.emplace_back(stride, offset, size, CreateValidLookingBufferHandle(size + offset));
    ...
    for (size_t i = 0; i < native_pixmap_handle.planes.size(); i++) {
        duped_fds[i] = base::ScopedFD(HANDLE_EINTR(dup(native_pixmap_handle.planes[i].fd.get())));
        ASSERT_TRUE(duped_fds[i].is_valid());
    }

    auto message = stable::mojom::NativePixmapHandle::SerializeAsMessage(&native_pixmap_handle);
    ASSERT_TRUE(!message.IsNull());
    ...
    ASSERT_TRUE(stable::mojom::NativePixmapHandle::DeserializeFromMessage(std::move(received_message), &deserialized_native_pixmap_handle));

    ASSERT_EQ(native_pixmap_handle.planes.size(), deserialized_native_pixmap_handle.planes.size());
    const auto pid = base::Process::Current().Pid();
    for (size_t i = 0; i < native_pixmap_handle.planes.size(); i++) {
        EXPECT_EQ(native_pixmap_handle.planes[i].stride, deserialized_native_pixmap_handle.planes[i].stride);
        EXPECT_EQ(native_pixmap_handle.planes[i].offset, deserialized_native_pixmap_handle.planes[i].offset);
        EXPECT_EQ(native_pixmap_handle.planes[i].size, deserialized_native_pixmap_handle.planes[i].size);
        ASSERT_TRUE(deserialized_native_pixmap_handle.planes[i].fd.is_valid());
        EXPECT_EQ(syscall(SYS_kcmp, pid, pid, KCMP_FILE, duped_fds[i].get(), deserialized_native_pixmap_handle.planes[i].fd.get()), 0);
    }
    EXPECT_EQ(native_pixmap_handle.modifier, deserialized_native_pixmap_handle.modifier);
    ...
    // default.
    EXPECT_FALSE(deserialized_native_pixmap_handle.supports_zero_copy_webgpu_import);
}
...
TEST(NativePixmapHandleMojomTraitsTest, NativePixmapHandleWithInvalidFd)
{
    ...
    offset = 2500;
    size = 625;
    native_pixmap_handle.planes.emplace_back(stride, offset, size, base::ScopedFD());
    native_pixmap_handle.modifier = 123u;
    native_pixmap_handle.supports_zero_copy_webgpu_import = true;

    ASSERT_CHECK_DEATH(stable::mojom::NativePixmapHandle::SerializeAsMessage(&native_pixmap_handle));
}
} // namespace media
