...
{
    auto hw_supported_svc_modes = ::media::GetSupportedScalabilityModesByHWEncoderForTesting();
    for (::media::SVCScalabilityMode input_svc_mode : hw_supported_svc_modes) {
        SVCScalabilityMode output_svc_mode;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::SVCScalabilityMode>(input_svc_mode, output_svc_mode));
...
TEST(VideoEncodeAcceleratorSupportedProfile, RoundTrip)
{
    ...
    input.rate_control_modes = VideoEncodeAccelerator::kConstantMode | VideoEncodeAccelerator::kVariableMode;
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::VideoEncodeAcceleratorSupportedProfile>(input, output));
...
TEST(VideoEncoderInfoStructTraitTest, RoundTrip)
{
    ...
    input.resolution_bitrate_limits.push_back(::media::ResolutionBitrateLimit(gfx::Size(123, 456), 123456, 123456, 789012));
    input.resolution_bitrate_limits.push_back(::media::ResolutionBitrateLimit(gfx::Size(789, 1234), 1234567, 1234567, 7890123));
    // Other bool values.
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::VideoEncoderInfo>(input, output));
    EXPECT_EQ(input, output);
}
...
TEST(VideoBitrateAllocationStructTraitTest, ConstantBitrate_RoundTrip)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::VideoBitrateAllocation>(input_allocation, output_allocation));

    EXPECT_EQ(input_allocation, output_allocation);
}

TEST(VideoBitrateAllocationStructTraitTest, VariableBitrate_PeakGreaterThanSum_RoundTrip)
{
...
TEST(VideoBitrateAllocationStructTraitTest, VariableBitrate_PeakEqualsSum_RoundTrip)
{
    ::media::VideoBitrateAllocation input_allocation(Bitrate::Mode::kVariable);
    input_allocation.SetBitrate(0, 0, 5000u);
...
TEST(VideoBitrateAllocationStructTraitTest, VariableBitrate_InvalidTooLowPeak_Fails)
{
    mojom::VideoBitrateAllocationPtr mojom_allocation = mojom::VideoBitrateAllocation::New();
    ...
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::VideoBitrateAllocation>(mojom_allocation, output));
}

TEST(VideoBitrateAllocationStructTraitTest, VariableBitrate_InvalidZeroPeak_Fails)
{
    mojom::VideoBitrateAllocationPtr mojom_allocation = mojom::VideoBitrateAllocation::New();
...
TEST(SpatialLayerStructTraitTest, RoundTrip)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::SpatialLayer>(input_spatial_layer, output_spatial_layer));
    EXPECT_EQ(input_spatial_layer, output_spatial_layer);
}

TEST(VideoEncodeAcceleratorConfigStructTraitTest, RoundTrip)
{
    std::vector<::media::VideoEncodeAccelerator::Config::SpatialLayer> input_spatial_layers(3);
    ...
    for (size_t i = 0; i < input_spatial_layers.size(); ++i) {
        input_spatial_layers[i].width = static_cast<int32_t>(kBaseSize.width() * (i + 1));
        input_spatial_layers[i].height = static_cast<int32_t>(kBaseSize.height() * (i + 1));
    ...
    constexpr ::media::Bitrate kBitrate = ::media::Bitrate::ConstantBitrate(kBaseBitrateBps);

    ::media::VideoEncodeAccelerator::Config input_config(::media::PIXEL_FORMAT_NV12, kBaseSize, ::media::VP9PROFILE_PROFILE0, kBitrate, kBaseFramerate,
        ::media::VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer, ::media::VideoEncodeAccelerator::Config::ContentType::kCamera);
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::VideoEncodeAcceleratorConfig>(input_config, output_config));
...
TEST(VideoEncodeAcceleratorConfigStructTraitTest, RoundTripVariableBitrate)
{
    ...
    const ::media::Bitrate kBitrate = ::media::Bitrate::VariableBitrate(kBaseBitrateBps, kMaximumBitrate);
    ::media::VideoEncodeAccelerator::Config input_config(::media::PIXEL_FORMAT_NV12, kBaseSize, ::media::VP9PROFILE_PROFILE0, kBitrate, 30,
        ::media::VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer, ::media::VideoEncodeAccelerator::Config::ContentType::kCamera);
...
{
    mojom::VariableBitratePtr mojom_variable_bitrate = mojom::VariableBitrate::New();
    mojom_variable_bitrate->target_bps = 0u;
    mojom_variable_bitrate->peak_bps = 0u;
    Bitrate output;

    bool result = mojo::test::SerializeAndDeserialize<mojom::VariableBitrate>(mojom_variable_bitrate, output);
    EXPECT_FALSE(result);
}
...
TEST(BitstreamBufferMetadataTraitTest, RoundTrip)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::BitstreamBufferMetadata>(input_metadata, output_metadata));
    ...
    input_metadata = BitstreamBufferMetadata::CreateForDropFrame(base::Milliseconds(123456),
