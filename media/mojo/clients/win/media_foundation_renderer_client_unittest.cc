...
public:
    ...
    void Initialize(mojo::PendingAssociatedRemote<mojom::RendererClient>, std::optional<std::vector<mojo::PendingRemote<mojom::DemuxerStream>>>,
        mojom::MediaUrlParamsPtr, InitializeCallback cb) override
    {
    ...
    MOCK_METHOD(void, SetCdm, (const std::optional<base::UnguessableToken>&, SetCdmCallback), (override));
    MOCK_METHOD(void, SetLatencyHint, (std::optional<base::TimeDelta>), (override));
};

class FakeMediaFoundationRendererExtension : public mojom::MediaFoundationRendererExtension {
public:
    ...
    MOCK_METHOD(void, NotifyFrameReleased, (const base::UnguessableToken&), (override));
    MOCK_METHOD(void, RequestNextFrame, (), (override));
    MOCK_METHOD(void, SetMediaFoundationRenderingMode, (MediaFoundationRenderingMode), (override));
};

class FakeDCOMPTextureWrapper : public DCOMPTextureWrapper {
public:
    FakeDCOMPTextureWrapper(gpu::Mailbox frame_server_mailbox, gpu::Mailbox dcomp_mailbox)
    ...
    void SetDCOMPSurfaceHandle(const base::UnguessableToken&, SetDCOMPSurfaceHandleCB cb) override
    {
        std::move(cb).Run(true);
    }
    ...
    void CreateVideoFrame(const gfx::Size& size, gfx::GpuMemoryBufferHandle, CreateDXVideoFrameCB cb) override
    {
...
class MockOverlayStateObserverSubscription : public OverlayStateObserverSubscription {
public:
...
public:
    ...
    MOCK_METHOD(void, OnStatisticsUpdate, (const PipelineStatistics&), (override));
    MOCK_METHOD(void, OnBufferingStateChange, (BufferingState, BufferingStateChangeReason), (override));
    MOCK_METHOD(void, OnWaiting, (WaitingReason), (override));
    MOCK_METHOD(void, OnAudioConfigChange, (const AudioDecoderConfig&), (override));
    MOCK_METHOD(void, OnVideoConfigChange, (const VideoDecoderConfig&), (override));
...
class MediaFoundationRendererClientTest : public ::testing::Test, public media::mojom::MediaFoundationRendererObserver {
public:
    ...
    void SetUp() override
    {
        ...
        mojo::MakeSelfOwnedReceiver(std::make_unique<FakeMojomRenderer>(), renderer_remote.InitWithNewPipeAndPassReceiver());

        mojo::PendingRemote<mojom::MediaFoundationRendererExtension> mf_renderer_extensions_remote;
        mojo::MakeSelfOwnedReceiver(std::make_unique<FakeMediaFoundationRendererExtension>(), mf_renderer_extensions_remote.InitWithNewPipeAndPassReceiver());
        ...
        auto mojo_renderer = std::make_unique<MojoRenderer>(task_environment_.GetMainThreadTaskRunner(),
        ...
        dcomp_texture_wrapper_ = std::make_unique<FakeDCOMPTextureWrapper>(frame_server_mailbox_, dcomp_mailbox_);

        ObserveOverlayStateCB observe_overlay_state_cb
            = base::BindRepeating(&MediaFoundationRendererClientTest::ObserveOverlaySupscription, base::Unretained(this));

        mojo::PendingRemote<media::mojom::MediaFoundationRendererObserver> media_foundation_renderer_observer_remote;

        media_foundation_renderer_client_ = std::make_unique<MediaFoundationRendererClient>(task_environment_.GetMainThreadTaskRunner(), media_log_.Clone(),
            std::move(mojo_renderer), std::move(mf_renderer_extensions_remote), std::move(client_extension_receiver), std::move(dcomp_texture_wrapper_),
            observe_overlay_state_cb, &mock_video_renderer_sink_, std::move(media_foundation_renderer_observer_remote));
    }

    void OnPipelineStatus(PipelineStatus status)
    {
        last_status_ = status;
    }

    std::unique_ptr<media::OverlayStateObserverSubscription> ObserveOverlaySupscription(
        const gpu::Mailbox& mailbox, OverlayStateObserverSubscription::StateChangedCB cb)
    {
    ...
    void InitializeMediaFoundationRendererClient()
    {
        ...
        media_foundation_renderer_client_->Initialize(
            fake_media_resource_.get(), renderer_client_, base::BindOnce(&MediaFoundationRendererClientTest::OnPipelineStatus, base::Unretained(this)));
        // Force a call to OnVideoNaturalSizeChange to force creation of a
        // DComp video frame.
        media_foundation_renderer_client_->OnVideoNaturalSizeChange(gfx::Size(1920, 1080));
    }
    ...
    void InitializeFramePool()
    {
        ...
        media_foundation_renderer_client_->InitializeFramePool(std::move(pool_params));
    }
...
protected:
    ...
    std::unique_ptr<MediaFoundationRendererClient> media_foundation_renderer_client_;

    gpu::Mailbox dcomp_mailbox_;
    gpu::Mailbox frame_server_mailbox_;
    OverlayStateObserverSubscription::StateChangedCB frame_server_on_state_change_cb_;
...
TEST_F(MediaFoundationRendererClientTest, InitializeFrameServer)
{
    ...
    feature_list_.InitWithFeaturesAndParameters({ { kMediaFoundationClearRendering, params }, { kMediaFoundationClearPlayback, {} } }, {});
    InitializeMediaFoundationRendererClient();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(last_status_, PIPELINE_OK);
