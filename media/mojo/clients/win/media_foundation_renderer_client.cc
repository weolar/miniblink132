...
#define REPORT_ERROR_REASON(reason) MediaFoundationRenderer::ReportErrorReason(MediaFoundationRenderer::ErrorReason::reason)

MediaFoundationRendererClient::MediaFoundationRendererClient(scoped_refptr<base::SequencedTaskRunner> media_task_runner, std::unique_ptr<MediaLog> media_log,
    std::unique_ptr<MojoRenderer> mojo_renderer, mojo::PendingRemote<RendererExtension> pending_renderer_extension,
    mojo::PendingReceiver<ClientExtension> client_extension_receiver, std::unique_ptr<DCOMPTextureWrapper> dcomp_texture_wrapper,
    ObserveOverlayStateCB observe_overlay_state_cb, VideoRendererSink* sink,
    mojo::PendingRemote<media::mojom::MediaFoundationRendererObserver> media_foundation_renderer_observer)
    ...
    , pending_media_foundation_renderer_observer_(std::move(media_foundation_renderer_observer))
{
    DVLOG_FUNC(1);
}
...
void MediaFoundationRendererClient::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
    DVLOG_FUNC(1);
    ...
    if (!dcomp_texture_wrapper_) {
        ...
        std::move(init_cb).Run({ PIPELINE_ERROR_INITIALIZATION_FAILED, "DComTextureWrapper creation failed" });
        return;
    }

    // Consume and bind the delayed PendingRemote and PendingReceiver now that
    // we are on |media_task_runner_|.
    renderer_extension_.Bind(std::move(pending_renderer_extension_), media_task_runner_);

    media_foundation_renderer_observer_.Bind(std::move(pending_media_foundation_renderer_observer_), media_task_runner_);
    client_extension_receiver_.Bind(std::move(pending_client_extension_receiver_), media_task_runner_);

    // Handle unexpected mojo pipe disconnection such as "mf_cdm" utility process
    // crashed or killed in Browser task manager.
    renderer_extension_.set_disconnect_handler(base::BindOnce(&MediaFoundationRendererClient::OnConnectionError, base::Unretained(this)));
    ...
    rendering_mode_ = rendering_strategy_ == MediaFoundationClearRenderingStrategy::kDirectComposition ? MediaFoundationRenderingMode::DirectComposition
    ...
    mojo_renderer_->Initialize(media_resource, this, base::BindOnce(&MediaFoundationRendererClient::OnRemoteRendererInitialized, weak_factory_.GetWeakPtr()));
}

void MediaFoundationRendererClient::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    ...
    mojo_renderer_->SetCdm(cdm_context_, base::BindOnce(&MediaFoundationRendererClient::OnCdmAttached, weak_factory_.GetWeakPtr()));
}

void MediaFoundationRendererClient::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
...
void MediaFoundationRendererClient::OnSelectedVideoTracksChanged(const std::vector<DemuxerStream*>& enabled_tracks, base::OnceClosure change_completed_cb)
{
...
void MediaFoundationRendererClient::OnError(PipelineStatus status)
{
    ...
    if (status == PIPELINE_ERROR_HARDWARE_CONTEXT_RESET && dcomp_video_frame_ && !IsFrameServerMode()) {
...
void MediaFoundationRendererClient::OnStatisticsUpdate(const PipelineStatistics& stats)
{
    client_->OnStatisticsUpdate(stats);
}

void MediaFoundationRendererClient::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
...
void MediaFoundationRendererClient::OnAudioConfigChange(const AudioDecoderConfig& config)
{
    client_->OnAudioConfigChange(config);
}
void MediaFoundationRendererClient::OnVideoConfigChange(const VideoDecoderConfig& config)
{
    client_->OnVideoConfigChange(config);
}

void MediaFoundationRendererClient::OnVideoNaturalSizeChange(const gfx::Size& size)
{
    ...
    natural_size_ = size;
    dcomp_texture_wrapper_->CreateVideoFrame(natural_size_, base::BindOnce(&MediaFoundationRendererClient::OnVideoFrameCreated, weak_factory_.GetWeakPtr()));
...
void MediaFoundationRendererClient::OnVideoFrameRateChange(std::optional<int> fps)
{
...
scoped_refptr<VideoFrame> MediaFoundationRendererClient::Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, RenderingMode mode)
{
    ...
    auto callback = [](base::WeakPtr<MediaFoundationRendererClient> renderer_client) {
        if (renderer_client) {
            renderer_client->renderer_extension_->RequestNextFrame();
        }
    };

    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(callback, weak_factory_.GetWeakPtr()));
...
void MediaFoundationRendererClient::InitializeFramePool(mojom::FramePoolInitializationParametersPtr pool_info)
{
    ...
    for (const auto& frame_info : pool_info->frame_textures) {
        dcomp_texture_wrapper_->CreateVideoFrame(pool_info->texture_size, std::move(frame_info->texture_handle),
            base::BindOnce(&MediaFoundationRendererClient::OnFramePoolVideoFrameCreated, weak_factory_.GetWeakPtr(), frame_info->token));
    }
}

void MediaFoundationRendererClient::OnFrameAvailable(const base::UnguessableToken& frame_token, const gfx::Size& size, base::TimeDelta timestamp)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(has_video_);
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapVideoFrame(texture_pool_video_frame, texture_pool_video_frame->format(), gfx::Rect(size), size);
    if (!frame) {
        MEDIA_LOG(WARNING, media_log_) << "OnFrameAvailable failed to wrap a VideoFrame";
        return;
    }
    frame->metadata().wants_promotion_hint = true;
    frame->metadata().allow_overlay = true;
    frame->AddDestructionObserver(
        base::BindPostTask(media_task_runner_, base::BindOnce(&MediaFoundationRendererClient::OnPaintComplete, weak_factory_.GetWeakPtr(), frame_token)));
...
void MediaFoundationRendererClient::OnRemoteRendererInitialized(PipelineStatus status)
{
    DVLOG_FUNC(1) << "status=" << status;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    bool success = dcomp_texture_wrapper_->Initialize(
        gfx::Size(1, 1), base::BindRepeating(&MediaFoundationRendererClient::OnOutputRectChange, weak_factory_.GetWeakPtr()));
    if (!success) {
        REPORT_ERROR_REASON(kFailedToInitDCompTextureWrapper);
        std::move(init_cb_).Run({ PIPELINE_ERROR_INITIALIZATION_FAILED, "DComTextureWrapper init failed" });
        return;
    }
...
void MediaFoundationRendererClient::OnOutputRectChange(gfx::Rect output_rect)
{
    ...
    renderer_extension_->SetOutputRect(
        output_rect, base::BindOnce(&MediaFoundationRendererClient::OnSetOutputRectDone, weak_factory_.GetWeakPtr(), output_rect.size()));
}

void MediaFoundationRendererClient::OnSetOutputRectDone(const gfx::Size& output_size, bool success)
{
...
void MediaFoundationRendererClient::InitializeDCOMPRenderingIfNeeded()
{
    ...
    renderer_extension_->GetDCOMPSurface(base::BindOnce(&MediaFoundationRendererClient::OnDCOMPSurfaceReceived, weak_factory_.GetWeakPtr()));
}

void MediaFoundationRendererClient::OnDCOMPSurfaceReceived(const std::optional<base::UnguessableToken>& token, const std::string& error)
{
    DVLOG_FUNC(1);
    DCHECK(has_video_);
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    dcomp_texture_wrapper_->SetDCOMPSurfaceHandle(
        token.value(), base::BindOnce(&MediaFoundationRendererClient::OnDCOMPSurfaceHandleSet, weak_factory_.GetWeakPtr()));
}
...
void MediaFoundationRendererClient::OnVideoFrameCreated(scoped_refptr<VideoFrame> video_frame, const gpu::Mailbox& mailbox)
{
    DVLOG_FUNC(1);
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(has_video_);
...
void MediaFoundationRendererClient::OnFramePoolVideoFrameCreated(
    const base::UnguessableToken& token, scoped_refptr<VideoFrame> video_frame, const gpu::Mailbox& mailbox)
{
    DVLOG_FUNC(1);
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(has_video_);
    std::unique_ptr<OverlayStateObserverSubscription> observer_subscription = ObserveMailboxForOverlayState(mailbox);
    video_frame_pool_.insert({ token, std::make_pair(std::move(video_frame), std::move(observer_subscription)) });
}
...
void MediaFoundationRendererClient::SignalMediaPlayingStateChange(bool is_playing)
{
...
std::unique_ptr<OverlayStateObserverSubscription> MediaFoundationRendererClient::ObserveMailboxForOverlayState(const gpu::Mailbox& mailbox)
{
    ...
    if (rendering_strategy_ == MediaFoundationClearRenderingStrategy::kDynamic) {
        ...
        observer_subscription = observe_overlay_state_cb_.Run(
            mailbox, base::BindRepeating(&MediaFoundationRendererClient::OnOverlayStateChanged, base::Unretained(this), mailbox));
...
void MediaFoundationRendererClient::OnOverlayStateChanged(const gpu::Mailbox& mailbox, bool promoted)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    promoted_to_overlay_signal_ = promoted;
    MEDIA_LOG(INFO, media_log_) << "Overlay state signal, promoted = " << promoted;
    UpdateRenderMode();
}
...
void MediaFoundationRendererClient::UpdateRenderMode()
{
    ...
    if (rendering_strategy_ != MediaFoundationClearRenderingStrategy::kDynamic || cdm_context_) {
        return;
    }

    // Frame Server mode is required if we are not promoted to an overlay or if
    // frame readback is required.
    bool needs_frame_server = has_frame_read_back_signal_ || !promoted_to_overlay_signal_;
...
void MediaFoundationRendererClient::OnPaintComplete(const base::UnguessableToken& token)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void MediaFoundationRendererClient::LogRenderingStrategy()
{
    ...
    MEDIA_LOG(INFO, media_log_) << "MediaFoundationClearRenderingStrategy: " << strategy;
}
