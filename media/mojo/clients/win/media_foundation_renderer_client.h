...
// ```
class MediaFoundationRendererClient : public Renderer,
...
public:
    ...
    MediaFoundationRendererClient(scoped_refptr<base::SequencedTaskRunner> media_task_runner, std::unique_ptr<MediaLog> media_log,
        ...
        media::ObserveOverlayStateCB observe_overlay_state_cb, VideoRendererSink* sink,
    ...
    MediaFoundationRendererClient& operator=(const MediaFoundationRendererClient&) = delete;
    ...
    void OnSelectedVideoTracksChanged(const std::vector<DemuxerStream*>& enabled_tracks, base::OnceClosure change_completed_cb) override;
    ...
    void OnBufferingStateChange(BufferingState state, BufferingStateChangeReason) override;
    ...
    scoped_refptr<media::VideoFrame> Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, RenderingMode rendering_mode) override;
    void OnFrameDropped() override;
    base::TimeDelta GetPreferredRenderInterval() override;

    // media::mojom::MediaFoundationRendererClientExtension
    void InitializeFramePool(mojom::FramePoolInitializationParametersPtr pool_info) override;
...
private:
    ...
    void OnDCOMPSurfaceReceived(const std::optional<base::UnguessableToken>& token, const std::string& error);
    void OnDCOMPSurfaceHandleSet(bool success);
    void OnVideoFrameCreated(scoped_refptr<VideoFrame> video_frame, const gpu::Mailbox& mailbox);
    void OnFramePoolVideoFrameCreated(const base::UnguessableToken& token, scoped_refptr<VideoFrame> video_frame, const gpu::Mailbox& mailbox);
    void OnCdmAttached(bool success);
    void SignalMediaPlayingStateChange(bool is_playing);
    std::unique_ptr<OverlayStateObserverSubscription> ObserveMailboxForOverlayState(const gpu::Mailbox& mailbox);
    ...
    std::unique_ptr<OverlayStateObserverSubscription> dcomp_frame_observer_subscription_;
    scoped_refptr<VideoFrame> next_video_frame_;

    // Rendering mode the Media Engine will use.
    MediaFoundationRenderingMode rendering_mode_ = MediaFoundationRenderingMode::DirectComposition;
    ...
    MediaFoundationClearRenderingStrategy rendering_strategy_ = MediaFoundationClearRenderingStrategy::kDirectComposition;
    ...
    // sink.
    base::flat_map<base::UnguessableToken, std::pair<scoped_refptr<VideoFrame>, std::unique_ptr<OverlayStateObserverSubscription>>> video_frame_pool_;
    ...
    mojo::PendingRemote<media::mojom::MediaFoundationRendererObserver> pending_media_foundation_renderer_observer_;
    mojo::Remote<media::mojom::MediaFoundationRendererObserver> media_foundation_renderer_observer_;
