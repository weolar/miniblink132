...
public:
    using CreateAudioDecoderCB = base::RepeatingCallback<std::unique_ptr<AudioDecoder>()>;
    ...
    std::unique_ptr<AudioDecoder> CreateAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<media::MediaLog> media_log) override
    {
...
public:
        ...
        , service_thread_("Service Thread")
    {
        ...
        service_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&MojoAudioDecoderTest::ConnectToService, base::Unretained(this), remote_audio_decoder.InitWithNewPipeAndPassReceiver()));
        mojo_audio_decoder_ = std::make_unique<MojoAudioDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_, std::move(remote_audio_decoder));
    }
    ...
    void ConnectToService(mojo::PendingReceiver<mojom::AudioDecoder> receiver)
    {
        ...
        EXPECT_CALL(*mock_audio_decoder_, Initialize_(_, _, _, _, _))
            .WillRepeatedly(DoAll(SaveArg<3>(&output_cb_), SaveArg<4>(&waiting_cb_), RunOnceCallbackRepeatedly<2>(DecoderStatus::Codes::kOk)));
        EXPECT_CALL(*mock_audio_decoder_, Decode(_, _)).WillRepeatedly([&](scoped_refptr<DecoderBuffer> buffer, AudioDecoder::DecodeCB decode_cb) {
            ReturnOutput();
            std::move(decode_cb).Run(DecoderStatus::Codes::kOk);
        });
        EXPECT_CALL(*mock_audio_decoder_, Reset_(_)).WillRepeatedly(RunOnceCallbackRepeatedly<0>());

        mojo::MakeSelfOwnedReceiver(
            std::make_unique<MojoAudioDecoderService>(&mojo_media_client_, &mojo_cdm_service_context_, service_task_runner_), std::move(receiver));
    }
    ...
    void InitializeAndExpect(DecoderStatus status)
    {
        ...
        EXPECT_CALL(*this, OnInitialized(SameStatusCode(status))).WillOnce(InvokeWithoutArgs(this, &MojoAudioDecoderTest::QuitLoop));

        AudioDecoderConfig audio_config(
            AudioCodec::kVorbis, kSampleFormat, kChannelLayout, kDefaultSampleRate, EmptyExtraData(), EncryptionScheme::kUnencrypted);

        mojo_audio_decoder_->Initialize(audio_config, nullptr, base::BindOnce(&MojoAudioDecoderTest::OnInitialized, base::Unretained(this)),
            base::BindRepeating(&MojoAudioDecoderTest::OnOutput, base::Unretained(this)),
            base::BindRepeating(&MojoAudioDecoderTest::OnWaiting, base::Unretained(this)));
    ...
    void Initialize()
    {
        InitializeAndExpect(DecoderStatus::Codes::kOk);
    }

    void Decode()
    {
        scoped_refptr<DecoderBuffer> buffer(new DecoderBuffer(100));
        mojo_audio_decoder_->Decode(buffer, base::BindRepeating(&MojoAudioDecoderTest::OnDecoded, base::Unretained(this)));
    }

    void Reset()
    {
        mojo_audio_decoder_->Reset(base::BindOnce(&MojoAudioDecoderTest::OnReset, base::Unretained(this)));
    }

    void ResetAndWaitUntilFinish()
    {
        DVLOG(1) << __func__;
        EXPECT_CALL(*this, OnReset()).WillOnce(InvokeWithoutArgs(this, &MojoAudioDecoderTest::QuitLoop));
        Reset();
        RunLoop();
    }
    ...
    void ReturnOutput()
    {
        for (int i = 0; i < kOutputPerDecode; ++i) {
            scoped_refptr<AudioBuffer> audio_buffer
                = MakeAudioBuffer<float>(kSampleFormat, kChannelLayout, kChannels, kDefaultSampleRate, 1.0, 0.0f, 100, input_timestamp_helper_.GetTimestamp());
    ...
    void KeepDecodingOrQuit()
    {
        ...
        EXPECT_CALL(*this, OnDecoded(IsOkStatus())).WillOnce(InvokeWithoutArgs(this, &MojoAudioDecoderTest::KeepDecodingOrQuit));
        Decode();
    }
    ...
    FakeMojoMediaClient mojo_media_client_ { base::BindRepeating(&MojoAudioDecoderTest::CreateAudioDecoder, base::Unretained(this)) };

    // Service side mock.
    std::unique_ptr<MockAudioDecoder> owned_mock_audio_decoder_ { std::make_unique<StrictMock<MockAudioDecoder>>() };
    raw_ptr<MockAudioDecoder, AcrossTasksDanglingUntriaged> mock_audio_decoder_ { owned_mock_audio_decoder_.get() };
...
{
    Initialize();
    EXPECT_CALL(*mock_audio_decoder_, Decode(_, _)).WillOnce([&](scoped_refptr<DecoderBuffer> buffer, AudioDecoder::DecodeCB decode_cb) {
        WaitForKey();
        std::move(decode_cb).Run(DecoderStatus::Codes::kOk);
    });
    EXPECT_CALL(*this, OnWaiting(WaitingReason::kNoDecryptionKey)).Times(1);
    EXPECT_CALL(*this, OnDecoded(IsOkStatus())).WillOnce(InvokeWithoutArgs(this, &MojoAudioDecoderTest::QuitLoop));
    Decode();
    RunLoop();
}
