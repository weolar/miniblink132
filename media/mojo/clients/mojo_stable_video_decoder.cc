...
const char kMojoStableVideoDecoderDecodeLatencyHistogram[] = "Media.MojoStableVideoDecoder.Decode";

namespace {

std::optional<viz::SharedImageFormat> GetSharedImageFormat(VideoPixelFormat format)
{
...
class MojoStableVideoDecoder::SharedImageHolder : public base::RefCountedThreadSafe<SharedImageHolder> {
public:
    static scoped_refptr<SharedImageHolder> CreateFromFrameResource(
        const scoped_refptr<FrameResource>& frame_resource, scoped_refptr<gpu::SharedImageInterface> sii)
    {
        if (!sii) {
        ...
        if (frame_resource->metadata().is_webgpu_compatible && !sii->GetCapabilities().disable_webgpu_shared_images) {
            shared_image_usage |= gpu::SHARED_IMAGE_USAGE_WEBGPU_READ;
        }

        gfx::GpuMemoryBufferHandle gmb_handle = frame_resource->CreateGpuMemoryBufferHandle();
        if (gmb_handle.type != gfx::NATIVE_PIXMAP || gmb_handle.native_pixmap_handle.planes.empty()) {
            DVLOGF(1) << "Could not obtain a GpuMemoryBufferHandle for the FrameResource";
            return nullptr;
        }

        std::optional<viz::SharedImageFormat> shared_image_format = GetSharedImageFormat(frame_resource->format());
        ...
        const gpu::SharedImageInfo shared_image_info(*shared_image_format, GetRectSizeFromOrigin(frame_resource->visible_rect()), frame_resource->ColorSpace(),
            kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, shared_image_usage,
            /*debug_label=*/"MojoStableVideoDecoder");
        scoped_refptr<gpu::ClientSharedImage> new_client_shared_image = sii->CreateSharedImage(shared_image_info, std::move(gmb_handle));
        ...
        return base::WrapRefCounted(
            new SharedImageHolder(frame_resource->GetSharedMemoryId(), std::move(new_client_shared_image), frame_resource->ColorSpace(), std::move(sii)));
    }

    SharedImageHolder(const SharedImageHolder&) = delete;
    SharedImageHolder& operator=(const SharedImageHolder&) = delete;

    gfx::GenericSharedMemoryId id() const
    {
        return id_;
    }
    ...
    bool IsCompatibleWith(const scoped_refptr<FrameResource>& frame_resource) const
    {
        return client_shared_image_->size() == GetRectSizeFromOrigin(frame_resource->visible_rect()) && color_space_ == frame_resource->ColorSpace();
    }
...
private:
    ...
    SharedImageHolder(gfx::GenericSharedMemoryId id, scoped_refptr<gpu::ClientSharedImage> client_shared_image, const gfx::ColorSpace& color_space,
    ...
    ~SharedImageHolder()
    {
        CHECK(client_shared_image_->HasOneRef());
    }
...
MojoStableVideoDecoder::MojoStableVideoDecoder(scoped_refptr<base::SequencedTaskRunner> media_task_runner, GpuVideoAcceleratorFactories* gpu_factories,
    MediaLog* media_log, mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder)
    : timestamps_(128)
...
void MojoStableVideoDecoder::Initialize(
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    OOPVideoDecoder::NotifySupportKnown(std::move(pending_remote_decoder_),
        base::BindOnce(&MojoStableVideoDecoder::InitializeOnceSupportedConfigsAreKnown, weak_this_factory_.GetWeakPtr(), config, low_delay, cdm_context,
            std::move(init_cb), output_cb, waiting_cb));
}

void MojoStableVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MojoStableVideoDecoder::InitializeOnceSupportedConfigsAreKnown(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb,
    const OutputCB& output_cb, const WaitingCB& waiting_cb, mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    std::optional<SupportedVideoDecoderConfigs> supported_configs = OOPVideoDecoder::GetSupportedConfigs();
    ...
    if (!oop_video_decoder_) {
        ...
        oop_video_decoder_ = OOPVideoDecoder::Create(std::move(pending_remote_decoder), media_log_->Clone(),
    ...
    oop_video_decoder()->Initialize(config, low_delay, cdm_context, std::move(init_cb),
        base::BindRepeating(&MojoStableVideoDecoder::OnFrameResourceDecoded, weak_this_factory_.GetWeakPtr()), waiting_cb);
}

scoped_refptr<MojoStableVideoDecoder::SharedImageHolder> MojoStableVideoDecoder::CreateOrUpdateSharedImageForFrame(
    const scoped_refptr<FrameResource>& frame_resource)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    const gfx::GenericSharedMemoryId frame_id = frame_resource->GetSharedMemoryId();
    ...
    if (shared_image_holder && shared_image_holder->IsCompatibleWith(frame_resource)) {
    ...
    auto new_shared_image = SharedImageHolder::CreateFromFrameResource(frame_resource, base::WrapRefCounted(gpu_factories_->SharedImageInterface()));
    if (!new_shared_image) {
        return nullptr;
    }
    ...
    if (shared_image_holder) {
        ...
        FrameResource* original_frame_resource = oop_video_decoder()->GetOriginalFrame(frame_id);
        ...
        original_frame_resource->AddDestructionObserver(
            base::BindPostTaskToCurrentDefault(base::BindOnce(&MojoStableVideoDecoder::UnregisterSharedImage, weak_this_factory_.GetWeakPtr(), frame_id)));
    }

    return new_shared_image;
}

void MojoStableVideoDecoder::UnregisterSharedImage(gfx::GenericSharedMemoryId frame_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(frame_id.is_valid());
...
void MojoStableVideoDecoder::OnFrameResourceDecoded(scoped_refptr<FrameResource> frame_resource)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // a SharedImageHolder.
    scoped_refptr<SharedImageHolder> shared_image = CreateOrUpdateSharedImageForFrame(frame_resource);
    if (!shared_image) {
        return;
    }
    ...
    scoped_refptr<VideoFrame> mailbox_frame = VideoFrame::WrapSharedImage(frame_resource->format(), std::move(client_shared_image), sync_token,
        /*mailbox_holders_release_cb=*/
        base::DoNothingWithBoundArgs(std::move(shared_image), frame_resource),
        /*coded_size=*/GetRectSizeFromOrigin(frame_resource->visible_rect()), frame_resource->visible_rect(), frame_resource->natural_size(),
    ...
    mailbox_frame->metadata().is_webgpu_compatible = frame_resource->metadata().is_webgpu_compatible;
    ...
    UMA_HISTOGRAM_TIMES(kMojoStableVideoDecoderDecodeLatencyHistogram, decode_end_time - decode_start_time);
