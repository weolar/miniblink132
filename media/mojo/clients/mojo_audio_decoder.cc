...
MojoAudioDecoder::MojoAudioDecoder(
    scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog* media_log, mojo::PendingRemote<mojom::AudioDecoder> remote_decoder)
    : task_runner_(task_runner)
    , pending_remote_decoder_(std::move(remote_decoder))
    , writer_capacity_(GetDefaultDecoderBufferConverterCapacity(DemuxerStream::AUDIO))
    ...
    const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    remote_decoder_->Initialize(config, cdm_id, base::BindOnce(&MojoAudioDecoder::OnInitialized, base::Unretained(this)));
}

void MojoAudioDecoder::Decode(scoped_refptr<DecoderBuffer> media_buffer, DecodeCB decode_cb)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    mojom::DecoderBufferPtr buffer = mojo_decoder_buffer_writer_->WriteDecoderBuffer(std::move(media_buffer));
    if (!buffer) {
    ...
    remote_decoder_->Decode(std::move(buffer), base::BindOnce(&MojoAudioDecoder::OnDecodeStatus, base::Unretained(this)));
}
...
void MojoAudioDecoder::Reset(base::OnceClosure closure)
{
    ...
    if (!remote_decoder_.is_connected()) {
        if (decode_cb_) {
            task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb_), DecoderStatus::Codes::kDisconnected));
        }
    ...
    remote_decoder_->Reset(base::BindOnce(&MojoAudioDecoder::OnResetDone, base::Unretained(this)));
}
...
void MojoAudioDecoder::BindRemoteDecoder()
{
    ...
    remote_decoder_.set_disconnect_handler(base::BindOnce(&MojoAudioDecoder::OnConnectionError, base::Unretained(this)));
...
void MojoAudioDecoder::OnInitialized(const DecoderStatus& status, bool needs_bitstream_conversion, AudioDecoderType decoder_type)
{
