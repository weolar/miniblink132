...
class MojoAudioDecoder final : public AudioDecoder, public mojom::AudioDecoderClient {
public:
    MojoAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog* media_log, mojo::PendingRemote<mojom::AudioDecoder> remote_decoder);
    ...
    void Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb) final;
...
private:
    ...
    void OnInitialized(const DecoderStatus& status, bool needs_bitstream_conversion, AudioDecoderType decoder_type);
