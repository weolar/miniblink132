...
public:
    ...
    MockMojoVideoEncodeAccelerator(const MockMojoVideoEncodeAccelerator&) = delete;
    MockMojoVideoEncodeAccelerator& operator=(const MockMojoVideoEncodeAccelerator&) = delete;

    // mojom::VideoEncodeAccelerator impl.
    void Initialize(const media::VideoEncodeAccelerator::Config& config, mojo::PendingAssociatedRemote<mojom::VideoEncodeAcceleratorClient> client,
        mojo::PendingRemote<mojom::MediaLog> media_log, InitializeCallback success_callback) override
    {
        if (initialization_success_) {
            ASSERT_TRUE(client);
            client_.Bind(std::move(client));
            const size_t allocation_size = VideoFrame::AllocationSize(config.input_format, config.input_visible_size);

            client_->RequireBitstreamBuffers(1, config.input_visible_size, allocation_size);

            DoInitialize(config.input_format, config.input_visible_size, config.output_profile, config.bitrate, config.content_type, &client_);
        }
        std::move(success_callback).Run(initialization_success_);
    }
    MOCK_METHOD6(DoInitialize,
        void(media::VideoPixelFormat, const gfx::Size&, media::VideoCodecProfile, media::Bitrate, media::VideoEncodeAccelerator::Config::ContentType,
            mojo::AssociatedRemote<mojom::VideoEncodeAcceleratorClient>*));

    void Encode(const scoped_refptr<VideoFrame>& frame, const VideoEncoder::EncodeOptions& options, EncodeCallback callback) override
    {
        EXPECT_NE(-1, configured_bitstream_buffer_id_);
        EXPECT_TRUE(client_);
        client_->BitstreamBufferReady(configured_bitstream_buffer_id_, BitstreamBufferMetadata(100, options.key_frame, frame->timestamp()));
    ...
    MOCK_METHOD2(DoUseOutputBitstreamBuffer, void(int32_t, base::UnsafeSharedMemoryRegion*));

    void RequestEncodingParametersChangeWithLayers(
        const media::VideoBitrateAllocation& bitrate, uint32_t framerate, const std::optional<gfx::Size>& size) override
    {
        DoRequestEncodingParametersChangeWithLayers(bitrate, framerate, size);
    }
    MOCK_METHOD3(DoRequestEncodingParametersChangeWithLayers, void(const media::VideoBitrateAllocation&, uint32_t, const std::optional<gfx::Size>&));
    ...
    MOCK_METHOD3(DoRequestEncodingParametersChangeWithBitrate, void(const media::Bitrate&, uint32_t, const std::optional<gfx::Size>&));
...
public:
    ...
    MojoVideoEncodeAcceleratorTest(const MojoVideoEncodeAcceleratorTest&) = delete;
    MojoVideoEncodeAcceleratorTest& operator=(const MojoVideoEncodeAcceleratorTest&) = delete;

    void SetUp() override
    {
        mojo::PendingRemote<mojom::VideoEncodeAccelerator> mojo_vea;
        mojo_vea_receiver_ = mojo::MakeSelfOwnedReceiver(std::make_unique<MockMojoVideoEncodeAccelerator>(), mojo_vea.InitWithNewPipeAndPassReceiver());

        mojo_vea_ = base::WrapUnique<VideoEncodeAccelerator>(new MojoVideoEncodeAccelerator(std::move(mojo_vea)));
    }
    ...
    {
        return static_cast<media::MockMojoVideoEncodeAccelerator*>(mojo_vea_receiver_->impl());
    }
    ...
    void Initialize(MockVideoEncodeAcceleratorClient* mock_vea_client)
    {
        ...
        constexpr VideoEncodeAccelerator::Config::StorageType kStorageType = VideoEncodeAccelerator::Config::StorageType::kShmem;
        constexpr VideoEncodeAccelerator::Config::ContentType kContentType = VideoEncodeAccelerator::Config::ContentType::kDisplay;

        EXPECT_CALL(*mock_mojo_vea(), DoInitialize(PIXEL_FORMAT_I420, kInputVisibleSize, kOutputProfile, kInitialBitrate, kContentType, _));
        EXPECT_CALL(*mock_vea_client, RequireBitstreamBuffers(_, kInputVisibleSize, VideoFrame::AllocationSize(PIXEL_FORMAT_I420, kInputVisibleSize)));

        VideoEncodeAccelerator::Config config(PIXEL_FORMAT_I420, kInputVisibleSize, kOutputProfile, kInitialBitrate, kFramerate, kStorageType, kContentType);
...
{
    std::unique_ptr<MockVideoEncodeAcceleratorClient> mock_vea_client = std::make_unique<MockVideoEncodeAcceleratorClient>();
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodeOneFrame)
{
    ...
    const int32_t kBitstreamBufferId = 17;
    {
        ...
        EXPECT_CALL(*mock_mojo_vea(), DoUseOutputBitstreamBuffer(kBitstreamBufferId, _));
        mojo_vea()->UseOutputBitstreamBuffer(BitstreamBuffer(kBitstreamBufferId, std::move(shmem), kShMemSize, 0 /* offset */, base::TimeDelta()));
        base::RunLoop().RunUntilIdle();
    }

    {
        ...
        EXPECT_CALL(*mock_vea_client, BitstreamBufferReady(kBitstreamBufferId, _))
            .WillOnce(Invoke([is_keyframe, &video_frame](int32_t, const BitstreamBufferMetadata& metadata) {
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersChange)
{
    ...
    EXPECT_CALL(*mock_mojo_vea(), DoRequestEncodingParametersChangeWithBitrate(bitrate, kNewFramerate, testing::Eq(std::nullopt)));
    mojo_vea()->RequestEncodingParametersChange(bitrate, kNewFramerate, std::nullopt);
    base::RunLoop().RunUntilIdle();
}

// Tests that a RequestEncodingParametersChange() works with multi-dimensional
// bitrate allocation.
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersWithBitrateAllocation)
{
    const uint32_t kNewFramerate = 321321u;
    ...
    for (size_t i = 0; i <= kMaxNumBitrates; ++i) {
        ...
        EXPECT_CALL(*mock_mojo_vea(), DoRequestEncodingParametersChangeWithLayers(bitrate_allocation, kNewFramerate, testing::Eq(std::nullopt)));
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersChangeWithFrameSize)
{
    ...
    Bitrate bitrate = Bitrate::ConstantBitrate(kNewBitrate);
    EXPECT_CALL(*mock_mojo_vea(), DoRequestEncodingParametersChangeWithBitrate(bitrate, kNewFramerate, testing::Optional(kNewSize)));
...
TEST_F(MojoVideoEncodeAcceleratorTest, EncodingParametersChangeWithBitrateAllocationAndFrameSize)
{
    const uint32_t kNewFramerate = 321321u;
    const size_t kMaxNumBitrates = VideoBitrateAllocation::kMaxSpatialLayers * VideoBitrateAllocation::kMaxTemporalLayers;
    ...
    for (size_t i = 0; i <= kMaxNumBitrates; ++i) {
        ...
        EXPECT_CALL(*mock_mojo_vea(), DoRequestEncodingParametersChangeWithLayers(bitrate_allocation, kNewFramerate, testing::Optional(frame_size)));
...
TEST_F(MojoVideoEncodeAcceleratorTest, InitializeFailure)
{
    ...
    const VideoEncodeAccelerator::Config config(
        PIXEL_FORMAT_I420, kInputVisibleSize, VIDEO_CODEC_PROFILE_UNKNOWN, kInitialBitrate, kFramerate, kStorageType, kContentType);
...
TEST_F(MojoVideoEncodeAcceleratorTest, MojoDisconnectAfterInitialize)
{
    ...
    const VideoEncodeAccelerator::Config config(
        PIXEL_FORMAT_I420, kInputVisibleSize, VIDEO_CODEC_PROFILE_UNKNOWN, kInitialBitrate, kFramerate, kStorageType, kContentType);
    EXPECT_TRUE(mojo_vea()->Initialize(config, mock_vea_client.get(), std::make_unique<media::NullMediaLog>()));
    mojo_vea_receiver_->Close();
    EXPECT_CALL(*mock_vea_client, NotifyErrorStatus(ExpectEncoderStatusCode(EncoderStatus::Codes::kEncoderMojoConnectionError)));
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(MojoVideoEncodeAcceleratorTest, IsFlushSupportedAndFlush)
{
    ...
    if (ret) {
        ...
        auto flush_callback = base::BindOnce([](bool status) { EXPECT_EQ(status, true); });
