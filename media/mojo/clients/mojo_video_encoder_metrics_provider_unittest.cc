...
class MockMojomVideoEncoderMetricsProvider : public mojom::VideoEncoderMetricsProvider {
public:
    ...
    MOCK_METHOD(void, Initialize, (uint64_t, mojom::VideoEncoderUseCase, VideoCodecProfile, const gfx::Size&, bool, SVCScalabilityMode), (override));
    MOCK_METHOD(void, SetEncodedFrameCount, (uint64_t, uint64_t), (override));
    MOCK_METHOD(void, SetError, (uint64_t, const media::EncoderStatus&), (override));
...
public:
    ...
    void SetUp() override
    {
        ...
        mojo_encoder_metrics_receiver_
            = mojo::MakeSelfOwnedReceiver(std::make_unique<MockMojomVideoEncoderMetricsProvider>(), pending_remote.InitWithNewPipeAndPassReceiver());
        mojo_encoder_metrics_provider_factory_ = base::MakeRefCounted<MojoVideoEncoderMetricsProviderFactory>(kUseCase, std::move(pending_remote));
        mojo_encoder_metrics_provider_ = mojo_encoder_metrics_provider_factory_->CreateVideoEncoderMetricsProvider();
    }
    ...
    {
        return reinterpret_cast<MockMojomVideoEncoderMetricsProvider*>(mojo_encoder_metrics_receiver_->impl());
    }

protected:
    base::test::SingleThreadTaskEnvironment task_environment_;
    // This member holds on to the mock implementation of the "service" side.
    mojo::SelfOwnedReceiverRef<mojom::VideoEncoderMetricsProvider> mojo_encoder_metrics_receiver_;

    scoped_refptr<MojoVideoEncoderMetricsProviderFactory> mojo_encoder_metrics_provider_factory_;
...
TEST_F(MojoVideoEncoderMetricsProviderTest, CreateAndBoundAndInitialize)
{
    ...
    InSequence s;
    EXPECT_CALL(*mock_mojo_receiver(), Initialize(kEncoderId, kUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode));
    mojo_encoder_metrics_provider_->Initialize(kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
    EXPECT_CALL(*mock_mojo_receiver(), Complete(kEncoderId));
    mojo_encoder_metrics_provider_.reset();
    base::RunLoop().RunUntilIdle();
}

TEST_F(MojoVideoEncoderMetricsProviderTest, CreateAndBoundAndInitializeSetEncodedFrameCount)
{
    constexpr auto kCodecProfile = media::VP9PROFILE_PROFILE0;
    constexpr gfx::Size kEncodeSize(1920, 1080);
    constexpr bool kIsHardwareEncoder = true;
...
TEST_F(MojoVideoEncoderMetricsProviderTest, CreateAndBoundAndInitializeAndSetError)
{
    constexpr auto kCodecProfile = media::VP9PROFILE_PROFILE0;
    constexpr gfx::Size kEncodeSize(1920, 1080);
    constexpr bool kIsHardwareEncoder = true;
    ...
    mojo_encoder_metrics_provider_->IncrementEncodedFrameCount();
    base::RunLoop().RunUntilIdle();
    const media::EncoderStatus kErrorStatus(media::EncoderStatus::Codes::kEncoderFailedEncode, "Encoder failed");
...
TEST_F(MojoVideoEncoderMetricsProviderTest, CreateAndIncrementEncodedFrameCount_NoCall)
{
    InSequence s;
    mojo_encoder_metrics_provider_->IncrementEncodedFrameCount();
    base::RunLoop().RunUntilIdle();
...
public:
    ...
    void SetUp() override
    {
        ...
        mojo_encoder_metrics_provider_factory_
            = base::MakeRefCounted<MojoVideoEncoderMetricsProviderFactory>(mojom::VideoEncoderUseCase::kWebRTC, std::move(pending_remote));
    }
...
TEST_F(MojoVideoEncoderMetricsProviderFactoryTest, CreateMultipleEncoders)
{
    ...
    auto metrics_provider0 = mojo_encoder_metrics_provider_factory_->CreateVideoEncoderMetricsProvider();
    auto metrics_provider1 = mojo_encoder_metrics_provider_factory_->CreateVideoEncoderMetricsProvider();

    InSequence s;
    EXPECT_CALL(*mock_mojo_receiver(), Initialize(0u, kUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode));
    EXPECT_CALL(*mock_mojo_receiver(), Initialize(1u, kUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode));
    metrics_provider0->Initialize(kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
    metrics_provider1->Initialize(kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
...
TEST_F(MojoVideoEncoderMetricsProviderFactoryTest, CanOperateAfterDestroyingOtherMetricsProvider)
{
    constexpr auto kCodecProfile = media::VP9PROFILE_PROFILE0;
    constexpr gfx::Size kEncodeSize(1920, 1080);
    constexpr bool kIsHardwareEncoder = true;
