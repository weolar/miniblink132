...
MojoRendererWrapper::MojoRendererWrapper(std::unique_ptr<MojoRenderer> mojo_renderer)
...
void MojoRendererWrapper::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
...
void MojoRendererWrapper::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    mojo_renderer_->SetCdm(cdm_context, std::move(cdm_attached_cb));
}

void MojoRendererWrapper::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
