...
MojoRenderer::MojoRenderer(const scoped_refptr<base::SequencedTaskRunner>& task_runner, std::unique_ptr<VideoOverlayFactory> video_overlay_factory,
    VideoRendererSink* video_renderer_sink, mojo::PendingRemote<mojom::Renderer> remote_renderer)
...
void MojoRenderer::Initialize(MediaResource* media_resource, media::RendererClient* client, PipelineStatusCallback init_cb)
{
    DVLOG(1) << __func__;
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    if (encountered_error_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), PIPELINE_ERROR_INITIALIZATION_FAILED));
        return;
    }
...
void MojoRenderer::InitializeRendererFromStreams(media::RendererClient* client)
{
    DVLOG(1) << __func__;
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    for (media::DemuxerStream* stream : streams) {
        ...
        auto mojo_stream = std::make_unique<MojoDemuxerStreamImpl>(stream, stream_proxy.InitWithNewPipeAndPassReceiver());
        ...
        mojo_stream->set_disconnect_handler(base::BindOnce(&MojoRenderer::OnDemuxerStreamConnectionError, base::Unretained(this), mojo_stream.get()));
    ...
    remote_renderer_->Initialize(client_receiver_.BindNewEndpointAndPassRemote(), std::move(stream_proxies), nullptr,
...
void MojoRenderer::InitializeRendererFromUrl(media::RendererClient* client)
{
    ...
    // |remote_renderer_|, and the callback won't be dispatched if
    // |remote_renderer_| is destroyed.
    mojom::MediaUrlParamsPtr media_url_params = mojom::MediaUrlParams::New(url_params.media_url, url_params.site_for_cookies, url_params.top_frame_origin,
        url_params.storage_access_api_status, url_params.allow_credentials, url_params.is_hls, url_params.headers);
    remote_renderer_->Initialize(client_receiver_.BindNewEndpointAndPassRemote(), std::nullopt, std::move(media_url_params),
        base::BindOnce(&MojoRenderer::OnInitialized, base::Unretained(this), client));
}

void MojoRenderer::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    DVLOG(1) << __func__;
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(cdm_context);
    ...
    if (encountered_error_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(cdm_attached_cb), false));
        return;
    }
    ...
    remote_renderer_->SetCdm(cdm_id, base::BindOnce(&MojoRenderer::OnCdmAttached, base::Unretained(this)));
}
...
void MojoRenderer::Flush(base::OnceClosure flush_cb)
{
    ...
    remote_renderer_->Flush(base::BindOnce(&MojoRenderer::OnFlushed, base::Unretained(this)));
}
...
void MojoRenderer::OnTimeUpdate(base::TimeDelta time, base::TimeDelta max_time, base::TimeTicks capture_time)
{
    DVLOG(4) << __func__ << "(" << time << ", " << max_time << ", " << capture_time << ")";
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void MojoRenderer::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
    DVLOG(2) << __func__;
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void MojoRenderer::OnVideoNaturalSizeChange(const gfx::Size& size)
{
    ...
    if (video_overlay_factory_) {
        video_renderer_sink_->PaintSingleFrame(video_overlay_factory_->CreateFrame(size));
    }
...
void MojoRenderer::OnDemuxerStreamConnectionError(MojoDemuxerStreamImpl* stream)
{
...
void MojoRenderer::BindRemoteRendererIfNeeded()
{
    ...
    // |remote_renderer_| is destroyed.
    remote_renderer_.set_disconnect_handler(base::BindOnce(&MojoRenderer::OnConnectionError, base::Unretained(this)));
}
...
void MojoRenderer::OnInitialized(media::RendererClient* client, bool success)
{
    ...
    std::move(init_cb_).Run(success ? PIPELINE_OK : PIPELINE_ERROR_INITIALIZATION_FAILED);
