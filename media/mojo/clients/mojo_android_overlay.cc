...
MojoAndroidOverlay::MojoAndroidOverlay(
    mojo::PendingRemote<mojom::AndroidOverlayProvider> pending_provider, AndroidOverlayConfig config, const base::UnguessableToken& routing_token)
    : config_(std::move(config))
{
    ...
    mojom::AndroidOverlayConfigPtr mojo_config = mojom::AndroidOverlayConfig::New();
    ...
    mojo::Remote<mojom::AndroidOverlayProvider> provider(std::move(pending_provider));
    provider->CreateOverlay(overlay_.BindNewPipeAndPassReceiver(), receiver_.BindNewPipeAndPassRemote(), std::move(mojo_config));
}
...
const base::android::JavaRef<jobject>& MojoAndroidOverlay::GetJavaSurface() const
{
...
void MojoAndroidOverlay::OnSurfaceReady(uint64_t surface_key)
{
    ...
    auto surface_record = gpu::GpuSurfaceLookup::GetInstance()->AcquireJavaSurface(surface_key);
    ...
    surface_ = std::move(absl::get<gl::ScopedJavaSurface>(std::move(surface_record.surface_variant)));
...
void MojoAndroidOverlay::OnSynchronouslyDestroyed(OnSynchronouslyDestroyedCallback done_cb)
{
