...
VideoDecoderConfig CreateValidSupportedVideoDecoderConfig()
{
    ...
    // VideoTransformation.
    const VideoDecoderConfig config(VideoCodec::kH264, VideoCodecProfile::H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kHasAlpha,
        ...
        /*extra_data=*/std::vector<uint8_t> { 1, 2, 3 }, EncryptionScheme::kUnencrypted);
...
VideoDecoderConfig CreateValidUnsupportedVideoDecoderConfig()
{
    // Note: the StableVideoDecoder Mojo interface doesn't support
    // VideoTransformation.
    const VideoDecoderConfig config(VideoCodec::kAV1, VideoCodecProfile::AV1PROFILE_PROFILE_MAIN, VideoDecoderConfig::AlphaMode::kHasAlpha,
        VideoColorSpace::REC709(), VideoTransformation(),
        /*coded_size=*/gfx::Size(640, 368),
        /*visible_rect=*/gfx::Rect(1, 1, 630, 360),
        /*natural_size=*/gfx::Size(1260, 720),
...
stable::mojom::VideoFramePtr CreateTestNV12DecodedFrame(
    const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    ...
    const size_t y_plane_size = y_plane_stride * base::checked_cast<size_t>(coded_size.height());
    const size_t uv_plane_stride = 2 * ((y_plane_stride + 1) / 2);
    const size_t uv_plane_size = uv_plane_stride * ((base::checked_cast<size_t>(coded_size.height()) + 1) / 2);
...
void AssertSharedImageInfoIsValidForNV12Frame(
    const gpu::SharedImageInfo& actual, const gfx::Rect& expected_visible_rect, const gfx::ColorSpace& expected_color_space, bool* result)
{
    *result = false;
    ASSERT_EQ(actual.meta.format, viz::MultiPlaneFormat::kNV12);
    ASSERT_TRUE(actual.meta.format.PrefersExternalSampler());
    ASSERT_EQ(actual.meta.size, gfx::Size(expected_visible_rect.right(), expected_visible_rect.bottom()));
    ...
    ASSERT_EQ(actual.meta.usage, gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT);
    ASSERT_EQ(actual.debug_label, "MojoStableVideoDecoder");
    *result = true;
}
MATCHER_P2(IsValidSharedImageInfoForNV12Frame, expected_visible_rect, expected_color_space, "")
{
    bool valid;
    AssertSharedImageInfoIsValidForNV12Frame(arg, expected_visible_rect, expected_color_space, &valid);
    return valid;
}
...
void AssertNV12NativeGpuMemoryBufferHandleIsValid(const gfx::GpuMemoryBufferHandle& actual, const gfx::Size& coded_size, bool* result)
{
    *result = false;
    ASSERT_EQ(actual.type, gfx::NATIVE_PIXMAP);
    ASSERT_EQ(actual.native_pixmap_handle.planes.size(), 2u);
    ASSERT_EQ(actual.native_pixmap_handle.planes[0].stride, base::checked_cast<uint32_t>(coded_size.width()));
    ASSERT_EQ(actual.native_pixmap_handle.planes[0].offset, 0u);
    ASSERT_EQ(actual.native_pixmap_handle.planes[0].size, base::checked_cast<uint64_t>(coded_size.GetArea()));
    ASSERT_TRUE(actual.native_pixmap_handle.planes[0].fd.is_valid());
    ASSERT_EQ(actual.native_pixmap_handle.planes[1].stride, base::checked_cast<uint32_t>(coded_size.width()));
    ASSERT_EQ(actual.native_pixmap_handle.planes[1].offset, base::checked_cast<uint64_t>(coded_size.GetArea()));
    ASSERT_EQ(actual.native_pixmap_handle.planes[1].size, base::checked_cast<uint64_t>(coded_size.GetArea() / 2));
...
class MockVideoFrameHandleReleaser : public stable::mojom::VideoFrameHandleReleaser {
public:
    explicit MockVideoFrameHandleReleaser(mojo::PendingReceiver<stable::mojom::VideoFrameHandleReleaser> video_frame_handle_releaser)
        : video_frame_handle_releaser_receiver_(this, std::move(video_frame_handle_releaser))
    {
    }
    MockVideoFrameHandleReleaser(const MockVideoFrameHandleReleaser&) = delete;
    MockVideoFrameHandleReleaser& operator=(const MockVideoFrameHandleReleaser&) = delete;
    ~MockVideoFrameHandleReleaser() override = default;

    // stable::mojom::VideoFrameHandleReleaser implementation.
    MOCK_METHOD1(ReleaseVideoFrame, void(const base::UnguessableToken& release_token));

private:
    mojo::Receiver<stable::mojom::VideoFrameHandleReleaser> video_frame_handle_releaser_receiver_;
};

class MockStableVideoDecoderService : public stable::mojom::StableVideoDecoder {
public:
    explicit MockStableVideoDecoderService(mojo::PendingReceiver<stable::mojom::StableVideoDecoder> pending_receiver)
        : receiver_(this, std::move(pending_receiver))
    {
    }
    MockStableVideoDecoderService(const MockStableVideoDecoderService&) = delete;
    MockStableVideoDecoderService& operator=(const MockStableVideoDecoderService&) = delete;
    ...
    void Construct(mojo::PendingAssociatedRemote<stable::mojom::VideoDecoderClient> stable_video_decoder_client_remote,
        mojo::PendingRemote<stable::mojom::MediaLog> stable_media_log_remote,
        mojo::PendingReceiver<stable::mojom::VideoFrameHandleReleaser> stable_video_frame_handle_releaser_receiver,
        mojo::ScopedDataPipeConsumerHandle decoder_buffer_pipe, const gfx::ColorSpace& target_color_space) override
    {
        video_decoder_client_remote_.Bind(std::move(stable_video_decoder_client_remote));
        media_log_remote_.Bind(std::move(stable_media_log_remote));
        mock_video_frame_handle_releaser_ = std::make_unique<StrictMock<MockVideoFrameHandleReleaser>>(std::move(stable_video_frame_handle_releaser_receiver));
    ...
    MOCK_METHOD4(Initialize,
        void(const VideoDecoderConfig& config, bool low_delay, mojo::PendingRemote<stable::mojom::StableCdmContext> cdm_context, InitializeCallback callback));
    MOCK_METHOD2(Decode, void(const scoped_refptr<DecoderBuffer>& buffer, DecodeCallback callback));
    ...
    StrictMock<MockVideoFrameHandleReleaser>* mock_video_frame_handle_releaser() const
    {
...
private:
    ...
    mojo::AssociatedRemote<stable::mojom::VideoDecoderClient> video_decoder_client_remote_;
    ...
    // client.
    std::unique_ptr<StrictMock<MockVideoFrameHandleReleaser>> mock_video_frame_handle_releaser_;
...
public:
    ...
    MOCK_METHOD2(CreateSharedImage, scoped_refptr<gpu::ClientSharedImage>(const gpu::SharedImageInfo& si_info, gpu::SurfaceHandle surface_handle));
    MOCK_METHOD2(CreateSharedImage, scoped_refptr<gpu::ClientSharedImage>(const gpu::SharedImageInfo& si_info, base::span<const uint8_t> pixel_data));
    MOCK_METHOD4(CreateSharedImage,
        scoped_refptr<gpu::ClientSharedImage>(
            const gpu::SharedImageInfo& si_info, gpu::SurfaceHandle surface_handle, gfx::BufferUsage buffer_usage, gfx::GpuMemoryBufferHandle buffer_handle));
    MOCK_METHOD2(CreateSharedImage, scoped_refptr<gpu::ClientSharedImage>(const gpu::SharedImageInfo& si_info, gfx::GpuMemoryBufferHandle buffer_handle));
    MOCK_METHOD1(CreateSharedImage, SharedImageMapping(const gpu::SharedImageInfo& si_info));
    MOCK_METHOD2(UpdateSharedImage, void(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox));
    MOCK_METHOD3(UpdateSharedImage, void(const gpu::SyncToken& sync_token, std::unique_ptr<gfx::GpuFence> acquire_fence, const gpu::Mailbox& mailbox));
    MOCK_METHOD2(DestroySharedImage, void(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox));
    MOCK_METHOD2(DestroySharedImage, void(const gpu::SyncToken& sync_token, scoped_refptr<gpu::ClientSharedImage> client_shared_image));
    MOCK_METHOD1(ImportSharedImage, scoped_refptr<gpu::ClientSharedImage>(const gpu::ExportedSharedImage& exported_shared_image));
    MOCK_METHOD6(CreateSwapChain,
        SwapChainSharedImages(viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
            SkAlphaType alpha_type, gpu::SharedImageUsageSet usage));
    MOCK_METHOD2(PresentSwapChain, void(const gpu::SyncToken& sync_token, const gpu::Mailbox& mailbox));
    ...
    MOCK_METHOD1(GetNativePixmap, scoped_refptr<gfx::NativePixmap>(const gpu::Mailbox& mailbox));
...
public:
    ...
    explicit TestEndpoints(scoped_refptr<base::SequencedTaskRunner> media_task_runner)
        : sii_(base::MakeRefCounted<StrictMock<MockSharedImageInterface>>())
        , gpu_factories_(std::make_unique<StrictMock<MockGpuVideoAcceleratorFactories>>(sii_.get()))
        , client_media_log_(std::make_unique<NullMediaLog>())
        , media_task_runner_(std::move(media_task_runner))
    {
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> mojo_stable_vd_pending_remote;
        service_ = std::make_unique<StrictMock<MockStableVideoDecoderService>>(mojo_stable_vd_pending_remote.InitWithNewPipeAndPassReceiver());
        client_ = std::make_unique<MojoStableVideoDecoder>(
            media_task_runner_, gpu_factories_.get(), client_media_log_.get(), std::move(mojo_stable_vd_pending_remote));
    }
    ...
    void DestroyClientSide()
    {
        ...
        if (sii_) {
            media_task_runner_->PostTask(FROM_HERE, base::DoNothingWithBoundArgs(std::move(sii_)));
        }
    }
    ...
    MojoStableVideoDecoder* client() const
    {
        return client_.get();
    }
    ...
    StrictMock<base::MockRepeatingCallback<void(scoped_refptr<VideoFrame>)>>* client_output_cb()
    {
    ...
    // passed to MojoStableVideoDecoder::Initialize().
    StrictMock<base::MockRepeatingCallback<void(WaitingReason)>>* client_waiting_cb()
    {
...
private:
    ...
    StrictMock<base::MockRepeatingCallback<void(scoped_refptr<VideoFrame>)>> client_output_cb_;
    StrictMock<base::MockRepeatingCallback<void(WaitingReason)>> client_waiting_cb_;
...
public:
    MojoStableVideoDecoderTest()
        : media_task_runner_(base::ThreadPool::CreateSequencedTaskRunner(/*traits=*/ {}))
    {
    }
...
protected:
    ...
    std::unique_ptr<TestEndpoints> CreateAndInitializeMojoStableVideoDecoder(const VideoDecoderConfig& config)
    {
        ...
        stable::mojom::StableVideoDecoder::GetSupportedConfigsCallback received_get_supported_configs_cb;
        EXPECT_CALL(*endpoints->service(), GetSupportedConfigs(_)).WillOnce([&](stable::mojom::StableVideoDecoder::GetSupportedConfigsCallback callback) {
        ...
        media_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&MojoStableVideoDecoder::Initialize, base::Unretained(endpoints->client()), config, kLowDelayToInitializeWith,
                /*cdm_context=*/nullptr, init_cb.Get(), endpoints->client_output_cb()->Get(), endpoints->client_waiting_cb()->Get()));
        task_environment_.RunUntilIdle();
        ...
        const std::vector<SupportedVideoDecoderConfig> supported_configs_to_reply_with = CreateListOfSupportedConfigs();
        stable::mojom::StableVideoDecoder::InitializeCallback received_initialize_cb;
        const bool config_is_supported = IsVideoDecoderConfigSupported(supported_configs_to_reply_with, config);
        if (config_is_supported) {
            InSequence sequence;
            EXPECT_CALL(*endpoints->service(), DoConstruct(_));
            EXPECT_CALL(*endpoints->service(), Initialize(_, kLowDelayToInitializeWith, _, _))
                .WillOnce(WithArgs<0, 3>([&](const VideoDecoderConfig& received_config, stable::mojom::StableVideoDecoder::InitializeCallback callback) {
                    EXPECT_TRUE(received_config.Matches(config));
                    received_initialize_cb = std::move(callback);
                }));
        } else {
            EXPECT_CALL(init_cb, Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedConfig)));
        }
        std::move(received_get_supported_configs_cb).Run(supported_configs_to_reply_with, kDecoderTypeToReplyWith);
        task_environment_.RunUntilIdle();
        if (!Mock::VerifyAndClearExpectations(endpoints->service())) {
            return nullptr;
        }
        ...
        EXPECT_CALL(init_cb, Run(kDecoderStatusToReplyWith)).WillOnce([&] {
            EXPECT_EQ(endpoints->client()->NeedsBitstreamConversion(), kNeedsBitstreamConversionToReplyWith);
            EXPECT_EQ(endpoints->client()->GetMaxDecodeRequests(), kMaxDecodeRequestsToReplyWith);
        });
        std::move(received_initialize_cb)
            .Run(kDecoderStatusToReplyWith, kNeedsBitstreamConversionToReplyWith, kMaxDecodeRequestsToReplyWith, kDecoderTypeToReplyWith,
...
TEST_F(MojoStableVideoDecoderTest, InitializeWithSupportedConfigConstructsStableVideoDecoder)
{
    const VideoDecoderConfig config = CreateValidSupportedVideoDecoderConfig();
    std::unique_ptr<TestEndpoints> endpoints = CreateAndInitializeMojoStableVideoDecoder(config);
    ASSERT_TRUE(endpoints);

    // No frames were output by the service, so the decode latency histogram
    // should be empty.
    histogram_tester_.ExpectTotalCount(kMojoStableVideoDecoderDecodeLatencyHistogram,
        /*expected_count=*/0u);
}

TEST_F(MojoStableVideoDecoderTest, InitializeWithUnsupportedConfigDoesNotConstructStableVideoDecoder)
{
...
TEST_F(MojoStableVideoDecoderTest, TwoInitializationsWithSupportedConfigsConstructStableVideoDecoderOnce)
{
    // This does the first initialization.
    const VideoDecoderConfig first_config = CreateValidSupportedVideoDecoderConfig();
    std::unique_ptr<TestEndpoints> endpoints = CreateAndInitializeMojoStableVideoDecoder(first_config);
    ASSERT_TRUE(endpoints);

    // Let's now do the second initialization.
    const VideoDecoderConfig second_config = CreateValidSupportedVideoDecoderConfig();
    ...
    StrictMock<base::MockOnceCallback<void(DecoderStatus)>> init_cb;
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&MojoStableVideoDecoder::Initialize, base::Unretained(endpoints->client()), second_config, kLowDelayToInitializeWith,
            /*cdm_context=*/nullptr, init_cb.Get(), endpoints->client_output_cb()->Get(), endpoints->client_waiting_cb()->Get()));
    task_environment_.RunUntilIdle();
...
TEST_F(MojoStableVideoDecoderTest, Decode)
{
    ...
    for (size_t i = 0; i < std::size(kRealTimestamps); i++) {
        ...
        scoped_refptr<DecoderBuffer> decoder_buffer_to_send = DecoderBuffer::CopyFrom(kEncodedData);
        ...
        decoder_buffer_to_send->set_discard_padding(std::make_pair(kFrontDiscardPadding, kBackDiscardPadding));
        ...
        EXPECT_CALL(*endpoints->service(), Decode(_, _))
            .WillOnce([&](const scoped_refptr<DecoderBuffer>& buffer, stable::mojom::StableVideoDecoder::DecodeCallback callback) {
                ASSERT_TRUE(buffer);
        ...
        media_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&MojoStableVideoDecoder::Decode, base::Unretained(endpoints->client()), decoder_buffer_to_send, decode_cb_to_send.Get()));
        task_environment_.RunUntilIdle();
        ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->service()));
        ...
        endpoints->service()->mojo_decoder_buffer_reader()->ReadDecoderBuffer(std::move(received_mojo_decoder_buffer),
            base::BindOnce([](scoped_refptr<DecoderBuffer>* dst_buffer, scoped_refptr<DecoderBuffer> buffer) { *dst_buffer = std::move(buffer); },
        ...
        received_fake_timestamps[i] = received_decoder_buffer_with_data->timestamp();
        received_decoder_buffer_with_data->set_timestamp(kTimestamp);
        EXPECT_TRUE(received_decoder_buffer_with_data->MatchesForTesting(*decoder_buffer_to_send));
        ...
        const DecoderStatus kDecoderStatusToReplyWith = DecoderStatus::Codes::kOk;
        EXPECT_CALL(decode_cb_to_send, Run(kDecoderStatusToReplyWith)).WillOnce([&]() { EXPECT_TRUE(media_task_runner_->RunsTasksInCurrentSequence()); });
        ...
        EXPECT_EQ(decoder_buffer_to_send->discard_padding().first, kFrontDiscardPadding);
        EXPECT_EQ(decoder_buffer_to_send->discard_padding().second, kBackDiscardPadding);
        EXPECT_EQ(decoder_buffer_to_send->is_key_frame(), kIsKeyFrame);
        ASSERT_EQ(decoder_buffer_to_send->size(), std::size(kEncodedData));
        EXPECT_EQ(base::make_span(decoder_buffer_to_send->data(), decoder_buffer_to_send->size()), base::make_span(kEncodedData, std::size(kEncodedData)));
    ...
    const base::UnguessableToken kDecodedFrame1ReleaseToken = base::UnguessableToken::Create();
    ...
    stable::mojom::VideoFramePtr decoded_video_frame_to_send_1
        = CreateTestNV12DecodedFrame(kDecodedFrame1CodedSize, kDecodedFrame1VisibleRect, kDecodedFrame1NaturalSize, received_fake_timestamps[0]);
    ...
    bool received_can_read_without_stalling_after_frame_1;
    {
        ...
        EXPECT_CALL(*endpoints->shared_image_interface(),
            CreateSharedImage(IsValidSharedImageInfoForNV12Frame(kDecodedFrame1VisibleRect, gfx::ColorSpace()),
                Matcher<gfx::GpuMemoryBufferHandle>(IsValidNV12NativeGpuMemoryBufferHandle(kDecodedFrame1CodedSize))))
            .WillOnce([&](const gpu::SharedImageInfo& si_info, gfx::GpuMemoryBufferHandle buffer_handle) {
                return base::MakeRefCounted<gpu::ClientSharedImage>(kDecodedFrame1Mailbox, si_info.meta, gpu::SyncToken(),
                    gpu::GpuMemoryBufferHandleInfo(std::move(buffer_handle), si_info.meta.format, kDecodedFrame1CodedSize, gfx::BufferUsage::SCANOUT_VDA_WRITE),
                    endpoints->shared_image_interface()->holder());
            });
        EXPECT_CALL(*endpoints->shared_image_interface(), GenUnverifiedSyncToken()).WillOnce(Return(kDecodedFrame1SharedImageSyncToken));
        EXPECT_CALL(*endpoints->client_output_cb(), Run(_)).WillOnce([&](scoped_refptr<VideoFrame> video_frame) {
            received_decoded_video_frame_1 = std::move(video_frame);
            received_can_read_without_stalling_after_frame_1 = endpoints->client()->CanReadWithoutStalling();
        });
    }
    endpoints->service()->video_decoder_client_remote()->OnVideoFrameDecoded(
        decoded_video_frame_to_send_1.Clone(), kCanReadWithoutStallingAfterFrame1, kDecodedFrame1ReleaseToken);
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->shared_image_interface()));
    ...
    EXPECT_EQ(received_decoded_video_frame_1->coded_size(), gfx::Size(kDecodedFrame1VisibleRect.right(), kDecodedFrame1VisibleRect.bottom()));
    EXPECT_EQ(received_decoded_video_frame_1->visible_rect(), kDecodedFrame1VisibleRect);
    EXPECT_EQ(received_decoded_video_frame_1->natural_size(), kDecodedFrame1NaturalSize);
    ...
    EXPECT_EQ(received_decoded_video_frame_1->shared_image()->mailbox(), kDecodedFrame1Mailbox);
    EXPECT_EQ(received_decoded_video_frame_1->acquire_sync_token(), kDecodedFrame1SharedImageSyncToken);
    EXPECT_TRUE(received_decoded_video_frame_1->metadata().read_lock_fences_enabled);
    EXPECT_EQ(received_can_read_without_stalling_after_frame_1, kCanReadWithoutStallingAfterFrame1);
    histogram_tester_.ExpectTotalCount(kMojoStableVideoDecoderDecodeLatencyHistogram,
        /*expected_count=*/1u);
    ...
    const base::UnguessableToken kDecodedFrame2ReleaseToken = base::UnguessableToken::Create();
    const gpu::SyncToken kDecodedFrame2SharedImageSyncToken(
        /*namespace_id=*/gpu::GPU_IO,
        /*command_buffer_id=*/gpu::CommandBufferId::FromUnsafeValue(1u),
        /*release_count=*/8u);
    stable::mojom::VideoFramePtr decoded_video_frame_to_send_2 = decoded_video_frame_to_send_1.Clone();
    ...
    {
        InSequence sequence;
        EXPECT_CALL(*endpoints->shared_image_interface(), UpdateSharedImage(gpu::SyncToken(), kDecodedFrame1Mailbox));
        EXPECT_CALL(*endpoints->shared_image_interface(), GenUnverifiedSyncToken()).WillOnce(Return(kDecodedFrame2SharedImageSyncToken));
        EXPECT_CALL(*endpoints->client_output_cb(), Run(_)).WillOnce([&](scoped_refptr<VideoFrame> video_frame) {
            received_decoded_video_frame_2 = std::move(video_frame);
            received_can_read_without_stalling_after_frame_2 = endpoints->client()->CanReadWithoutStalling();
        });
    }
    endpoints->service()->video_decoder_client_remote()->OnVideoFrameDecoded(
        decoded_video_frame_to_send_2.Clone(), kCanReadWithoutStallingAfterFrame2, kDecodedFrame2ReleaseToken);
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->shared_image_interface()));
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->client_output_cb()));
    ...
    EXPECT_EQ(received_decoded_video_frame_2->coded_size(), gfx::Size(kDecodedFrame1VisibleRect.right(), kDecodedFrame1VisibleRect.bottom()));
    EXPECT_EQ(received_decoded_video_frame_2->visible_rect(), kDecodedFrame2VisibleRect);
    EXPECT_EQ(received_decoded_video_frame_2->natural_size(), kDecodedFrame2NaturalSize);
    ...
    EXPECT_EQ(received_decoded_video_frame_2->shared_image()->mailbox(), kDecodedFrame1Mailbox);
    EXPECT_EQ(received_decoded_video_frame_2->acquire_sync_token(), kDecodedFrame2SharedImageSyncToken);
    EXPECT_TRUE(received_decoded_video_frame_2->metadata().read_lock_fences_enabled);
    EXPECT_EQ(received_can_read_without_stalling_after_frame_2, kCanReadWithoutStallingAfterFrame2);
    histogram_tester_.ExpectTotalCount(kMojoStableVideoDecoderDecodeLatencyHistogram,
        /*expected_count=*/2u);
    ...
    // to release it later.
    constexpr gfx::ColorSpace kDecodedFrame3ColorSpace = gfx::ColorSpace::CreateREC709();
    constexpr bool kCanReadWithoutStallingAfterFrame3 = true;
    const base::UnguessableToken kDecodedFrame3ReleaseToken = base::UnguessableToken::Create();
    const gpu::Mailbox kDecodedFrame3Mailbox = gpu::Mailbox::Generate();
    const gpu::SyncToken kDecodedFrame3SharedImageSyncToken(
        /*namespace_id=*/gpu::GPU_IO,
        /*command_buffer_id=*/gpu::CommandBufferId::FromUnsafeValue(1u),
        /*release_count=*/10u);
    stable::mojom::VideoFramePtr decoded_video_frame_to_send_3 = decoded_video_frame_to_send_2.Clone();
    ...
    bool received_can_read_without_stalling_after_frame_3;
    {
        InSequence sequence;
        EXPECT_CALL(*endpoints->shared_image_interface(),
            CreateSharedImage(IsValidSharedImageInfoForNV12Frame(kDecodedFrame2VisibleRect, kDecodedFrame3ColorSpace),
                Matcher<gfx::GpuMemoryBufferHandle>(IsValidNV12NativeGpuMemoryBufferHandle(kDecodedFrame1CodedSize))))
            .WillOnce([&](const gpu::SharedImageInfo& si_info, gfx::GpuMemoryBufferHandle buffer_handle) {
                return base::MakeRefCounted<gpu::ClientSharedImage>(kDecodedFrame3Mailbox, si_info.meta, gpu::SyncToken(),
                    gpu::GpuMemoryBufferHandleInfo(std::move(buffer_handle), si_info.meta.format, kDecodedFrame2CodedSize, gfx::BufferUsage::SCANOUT_VDA_WRITE),
                    endpoints->shared_image_interface()->holder());
            });
        EXPECT_CALL(*endpoints->shared_image_interface(), GenUnverifiedSyncToken()).WillOnce(Return(kDecodedFrame3SharedImageSyncToken));
        EXPECT_CALL(*endpoints->client_output_cb(), Run(_)).WillOnce([&](scoped_refptr<VideoFrame> video_frame) {
            received_decoded_video_frame_3 = std::move(video_frame);
            received_can_read_without_stalling_after_frame_3 = endpoints->client()->CanReadWithoutStalling();
        });
    }
    endpoints->service()->video_decoder_client_remote()->OnVideoFrameDecoded(
        std::move(decoded_video_frame_to_send_3), kCanReadWithoutStallingAfterFrame3, kDecodedFrame3ReleaseToken);
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->shared_image_interface()));
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->client_output_cb()));
    ...
    // Note: the output VideoFrame's coded size should match the SharedImage's
    // size, not the coded size of the frame received from the service.
    EXPECT_EQ(received_decoded_video_frame_3->coded_size(), gfx::Size(kDecodedFrame2VisibleRect.right(), kDecodedFrame2VisibleRect.bottom()));
    EXPECT_EQ(received_decoded_video_frame_3->visible_rect(), kDecodedFrame2VisibleRect);
    EXPECT_EQ(received_decoded_video_frame_3->natural_size(), kDecodedFrame2NaturalSize);
    EXPECT_EQ(received_decoded_video_frame_3->ColorSpace(), kDecodedFrame3ColorSpace);
    ASSERT_TRUE(received_decoded_video_frame_3->HasSharedImage());
    EXPECT_EQ(received_decoded_video_frame_3->shared_image()->mailbox(), kDecodedFrame3Mailbox);
    EXPECT_EQ(received_decoded_video_frame_3->acquire_sync_token(), kDecodedFrame3SharedImageSyncToken);
    EXPECT_TRUE(received_decoded_video_frame_3->metadata().read_lock_fences_enabled);
    EXPECT_EQ(received_can_read_without_stalling_after_frame_3, kCanReadWithoutStallingAfterFrame3);
    histogram_tester_.ExpectTotalCount(kMojoStableVideoDecoderDecodeLatencyHistogram,
        /*expected_count=*/3u);
    ...
    const base::UnguessableToken kDecodedFrame4ReleaseToken = base::UnguessableToken::Create();
    ...
    stable::mojom::VideoFramePtr decoded_video_frame_to_send_4
        = CreateTestNV12DecodedFrame(kDecodedFrame4CodedSize, kDecodedFrame4VisibleRect, kDecodedFrame4NaturalSize, received_fake_timestamps[3]);
    ...
    bool received_can_read_without_stalling_after_frame_4;
    {
        InSequence sequence;
        EXPECT_CALL(*endpoints->shared_image_interface(),
            CreateSharedImage(IsValidSharedImageInfoForNV12Frame(kDecodedFrame4VisibleRect, gfx::ColorSpace()),
                Matcher<gfx::GpuMemoryBufferHandle>(IsValidNV12NativeGpuMemoryBufferHandle(kDecodedFrame4CodedSize))))
            .WillOnce([&](const gpu::SharedImageInfo& si_info, gfx::GpuMemoryBufferHandle buffer_handle) {
                return base::MakeRefCounted<gpu::ClientSharedImage>(kDecodedFrame4Mailbox, si_info.meta, gpu::SyncToken(),
                    gpu::GpuMemoryBufferHandleInfo(std::move(buffer_handle), si_info.meta.format, kDecodedFrame4CodedSize, gfx::BufferUsage::SCANOUT_VDA_WRITE),
                    endpoints->shared_image_interface()->holder());
            });
        EXPECT_CALL(*endpoints->shared_image_interface(), GenUnverifiedSyncToken()).WillOnce(Return(kDecodedFrame4SharedImageSyncToken));
        EXPECT_CALL(*endpoints->client_output_cb(), Run(_)).WillOnce([&](scoped_refptr<VideoFrame> video_frame) {
            received_decoded_video_frame_4 = std::move(video_frame);
            received_can_read_without_stalling_after_frame_4 = endpoints->client()->CanReadWithoutStalling();
        });
    }
    endpoints->service()->video_decoder_client_remote()->OnVideoFrameDecoded(
        std::move(decoded_video_frame_to_send_4), kCanReadWithoutStallingAfterFrame4, kDecodedFrame4ReleaseToken);
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->shared_image_interface()));
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->client_output_cb()));
    ...
    // Note: the output VideoFrame's coded size should match the SharedImage's
    // size, not the coded size of the frame received from the service.
    EXPECT_EQ(received_decoded_video_frame_4->coded_size(), gfx::Size(kDecodedFrame4VisibleRect.right(), kDecodedFrame4VisibleRect.bottom()));
    EXPECT_EQ(received_decoded_video_frame_4->visible_rect(), kDecodedFrame4VisibleRect);
    EXPECT_EQ(received_decoded_video_frame_4->natural_size(), kDecodedFrame4NaturalSize);
    ...
    EXPECT_EQ(received_decoded_video_frame_4->shared_image()->mailbox(), kDecodedFrame4Mailbox);
    EXPECT_EQ(received_decoded_video_frame_4->acquire_sync_token(), kDecodedFrame4SharedImageSyncToken);
    EXPECT_TRUE(received_decoded_video_frame_4->metadata().read_lock_fences_enabled);
    EXPECT_EQ(received_can_read_without_stalling_after_frame_4, kCanReadWithoutStallingAfterFrame4);
    histogram_tester_.ExpectTotalCount(kMojoStableVideoDecoderDecodeLatencyHistogram,
        /*expected_count=*/4u);
    ...
    // in use.
    EXPECT_CALL(*endpoints->service()->mock_video_frame_handle_releaser(), ReleaseVideoFrame(kDecodedFrame1ReleaseToken));
    received_decoded_video_frame_1.reset();
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->service()->mock_video_frame_handle_releaser()));
    ...
    EXPECT_CALL(*endpoints->shared_image_interface(), DestroySharedImage(gpu::SyncToken(), kDecodedFrame1Mailbox));
    EXPECT_CALL(*endpoints->service()->mock_video_frame_handle_releaser(), ReleaseVideoFrame(kDecodedFrame2ReleaseToken));
    ...
    EXPECT_CALL(*endpoints->shared_image_interface(), DestroySharedImage(gpu::SyncToken(), kDecodedFrame3Mailbox));
    EXPECT_CALL(*endpoints->service()->mock_video_frame_handle_releaser(), ReleaseVideoFrame(kDecodedFrame3ReleaseToken));
    ...
    StrictMock<MockSharedImageInterface>* const sii = endpoints->shared_image_interface();
    endpoints->DestroyClientSide();
    task_environment_.RunUntilIdle();
    EXPECT_CALL(*sii, DestroySharedImage(gpu::SyncToken(), kDecodedFrame4Mailbox));
...
TEST_F(MojoStableVideoDecoderTest, Reset)
{
    ...
    EXPECT_CALL(*endpoints->service(), Reset(_)).WillOnce([&](stable::mojom::StableVideoDecoder::ResetCallback callback) {
        received_reset_cb = std::move(callback);
    });
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&MojoStableVideoDecoder::Reset, base::Unretained(endpoints->client()), reset_cb_to_send.Get()));
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(endpoints->service()));
    ...
    EXPECT_CALL(reset_cb_to_send, Run()).WillOnce([&]() { EXPECT_TRUE(media_task_runner_->RunsTasksInCurrentSequence()); });
