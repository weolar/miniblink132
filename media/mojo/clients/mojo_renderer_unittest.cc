...
void WaitFor(base::TimeDelta duration)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), duration);
    run_loop.Run();
}
} // namespace
...
public:
    MojoRendererTest()
    {
        std::unique_ptr<StrictMock<MockRenderer>> mock_renderer(new StrictMock<MockRenderer>());
        mock_renderer_ = mock_renderer.get();

        mojo::PendingRemote<mojom::Renderer> remote_renderer_remote;
        renderer_receiver_
            = MojoRendererService::Create(&mojo_cdm_service_context_, std::move(mock_renderer), remote_renderer_remote.InitWithNewPipeAndPassReceiver());

        mojo_renderer_ = std::make_unique<MojoRenderer>(
            message_loop_.task_runner(), std::unique_ptr<VideoOverlayFactory>(nullptr), nullptr, std::move(remote_renderer_remote));
        ...
        EXPECT_CALL(*mock_renderer_, GetMediaTime()).WillRepeatedly(Return(base::TimeDelta()));
    }
    ...
    {
        std::unique_ptr<StrictMock<MockDemuxerStream>> stream(new StrictMock<MockDemuxerStream>(type));
        return stream;
    }
    ...
    void InitializeAndExpect(PipelineStatus status)
    {
        ...
        mojo_renderer_->Initialize(&demuxer_, &renderer_client_, base::BindOnce(&MojoRendererTest::OnInitialized, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void Initialize()
    {
        ...
        EXPECT_CALL(*mock_renderer_, OnInitialize(_, _, _)).WillOnce(DoAll(SaveArg<1>(&remote_renderer_client_), RunOnceCallback<2>(PIPELINE_OK)));
    ...
    void Flush()
    {
        ...
        mojo_renderer_->Flush(base::BindOnce(&MojoRendererTest::OnFlushed, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void SetCdmAndExpect(bool success)
    {
        ...
        mojo_renderer_->SetCdm(&cdm_context_, base::BindOnce(&MojoRendererTest::OnCdmAttached, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void OnCdmServiceInitialized(mojom::CdmContextPtr cdm_context, CreateCdmStatus status)
    {
    ...
    void CreateCdm()
    {
        ...
        mojo_cdm_service_->Initialize(
            &cdm_factory_, { kClearKeyKeySystem, false, false, false }, base::BindOnce(&MojoRendererTest::OnCdmServiceInitialized, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void Play()
    {
        StartPlayingFrom(base::Milliseconds(kStartPlayingTimeInMs));
    }
    ...
    raw_ptr<StrictMock<MockRenderer>, AcrossTasksDanglingUntriaged> mock_renderer_;
...
TEST_F(MojoRendererTest, Initialize_Failure)
{
    ...
    EXPECT_CALL(*mock_renderer_, OnInitialize(_, _, _)).WillOnce(RunOnceCallback<2>(PIPELINE_ERROR_ABORT));
...
{
    CreateAudioStream();
    EXPECT_CALL(*mock_renderer_, OnInitialize(_, _, _)).WillOnce(InvokeWithoutArgs(this, &MojoRendererTest::ConnectionError));
    InitializeAndExpect(PIPELINE_ERROR_INITIALIZATION_FAILED);
}
...
TEST_F(MojoRendererTest, Flush_ConnectionError)
{
    ...
    EXPECT_CALL(renderer_client_, OnError(HasStatusCode(PIPELINE_ERROR_DISCONNECTED))).Times(1);
    EXPECT_CALL(*mock_renderer_, OnFlush(_)).WillOnce(InvokeWithoutArgs(this, &MojoRendererTest::ConnectionError));
    Flush();
}
...
TEST_F(MojoRendererTest, SetCdm_Success)
{
    Initialize();
    CreateCdm();
    EXPECT_CALL(*mock_renderer_, OnSetCdm(_, _)).WillOnce(RunOnceCallback<1>(true));
...
TEST_F(MojoRendererTest, SetCdm_Failure)
{
    Initialize();
    CreateCdm();
    EXPECT_CALL(*mock_renderer_, OnSetCdm(_, _)).WillOnce(RunOnceCallback<1>(false));
...
TEST_F(MojoRendererTest, SetCdm_ReleasedCdmId)
{
    ...
    Initialize();
    CreateCdm();
    EXPECT_CALL(*mock_renderer_, OnSetCdm(_, _)).WillOnce(DoAll(SaveArg<0>(&mock_renderer_cdm_context), RunOnceCallback<1>(true)));
    SetCdmAndExpect(true);
...
TEST_F(MojoRendererTest, GetMediaTime)
{
    ...
    EXPECT_CALL(*mock_renderer_, GetMediaTime()).WillRepeatedly(Return(kStartTime));
    ...
    EXPECT_CALL(*mock_renderer_, GetMediaTime()).WillRepeatedly(GetMediaTime(kStartTime, elapsed_timer.get()));
...
TEST_F(MojoRendererTest, OnBufferingStateChange)
{
    ...
    EXPECT_CALL(renderer_client_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN)).Times(1);
    remote_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN);

    EXPECT_CALL(renderer_client_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, DECODER_UNDERFLOW)).Times(1);
    remote_renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, DECODER_UNDERFLOW);
...
{
    CreateAudioStream();
    EXPECT_CALL(*mock_renderer_, OnInitialize(_, _, _)).WillRepeatedly(base::test::RunOnceCallbackRepeatedly<2>(PIPELINE_ERROR_ABORT));
    EXPECT_CALL(*this, OnInitialized(HasStatusCode(PIPELINE_ERROR_INITIALIZATION_FAILED)));
    mojo_renderer_->Initialize(&demuxer_, &renderer_client_, base::BindOnce(&MojoRendererTest::OnInitialized, base::Unretained(this)));
    Destroy();
}
...
{
    EXPECT_CALL(*mock_renderer_, OnSetCdm(_, _)).WillRepeatedly(base::test::RunOnceCallbackRepeatedly<1>(true));
...
TEST_F(MojoRendererTest, ErrorDuringPlayback)
{
    ...
    EXPECT_CALL(renderer_client_, OnError(HasStatusCode(PIPELINE_ERROR_DECODE))).Times(1);
...
TEST_F(MojoRendererTest, ErrorDuringFlush)
{
    ...
    EXPECT_CALL(*mock_renderer_, OnFlush(_))
        .WillOnce(DoAll(InvokeWithoutArgs([&]() { remote_renderer_client_->OnError(PIPELINE_ERROR_DECODE); }), RunOnceClosure<0>()));
    Flush();
}
