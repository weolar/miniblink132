...
public:
    ...
    void SetUp() override
    {
        ...
        std::unique_ptr<StrictMock<MockAudioEncoder>> mock_audio_encoder(new StrictMock<MockAudioEncoder>());
        ...
        audio_encoder_service_ = std::make_unique<MojoAudioEncoderService>(std::move(mock_audio_encoder));

        service_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&MojoAudioEncoderTest::SetupMojoService, base::Unretained(this), remote_audio_encoder.InitWithNewPipeAndPassReceiver()));
        mojo_audio_encoder_ = std::make_unique<MojoAudioEncoder>(std::move(remote_audio_encoder));
    }
    ...
    void TearDown() override
    {
        ...
        service_task_runner_->DeleteSoon(FROM_HERE, std::move(audio_encoder_service_));
        service_thread_.Stop();
    }

    void SetupMojoService(mojo::PendingReceiver<mojom::AudioEncoder> receiver)
    {
        receiver_ = std::make_unique<mojo::Receiver<mojom::AudioEncoder>>(audio_encoder_service_.get(), std::move(receiver));
    }

    std::unique_ptr<AudioBus> MakeInput(float seed, int channels, int frames)
    {
        CHECK_EQ(std::clamp(seed, -1.0f, 1.0f), seed) << "Some AudioBuffer <-> AudioBus conversions involve clipping to the "
    ...
    AudioEncoder::EncoderStatusCB ValidatingStatusCB(base::Location loc = FROM_HERE)
    {
        struct CallEnforcer {
            bool called = false;
            std::string location;
    ...
    raw_ptr<StrictMock<MockAudioEncoder>, AcrossTasksDanglingUntriaged> mock_audio_encoder_ = nullptr;
};
...
TEST_F(MojoAudioEncoderTest, Initialize_Success)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Initialize(_, _, _))
        .WillOnce(Invoke([this](const AudioEncoder::Options& options, AudioEncoder::OutputCB output_cb, AudioEncoder::EncoderStatusCB done_cb) {
            EXPECT_TRUE(service_task_runner_->RunsTasksInCurrentSequence());
            std::move(done_cb).Run(EncoderStatus::Codes::kOk);
        }));

    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, std::optional<AudioEncoder::CodecDescription>) { FAIL(); });
    ...
    mojo_audio_encoder_->Initialize(options, std::move(output_cb), std::move(done_cb));
    run_loop.Run();
}
...
TEST_F(MojoAudioEncoderTest, Initialize_Fail)
{
    ...
    AudioEncoder::Options options = MakeOptions();
    EXPECT_CALL(*mock_audio_encoder_, Initialize(_, _, _))
        .WillOnce(Invoke([](const AudioEncoder::Options& options, AudioEncoder::OutputCB output_cb, AudioEncoder::EncoderStatusCB done_cb) {
...
TEST_F(MojoAudioEncoderTest, Initialize_Twice)
{
    ...
    AudioEncoder::Options options = MakeOptions();
    EXPECT_CALL(*mock_audio_encoder_, Initialize(_, _, _))
        .WillRepeatedly(Invoke([](const AudioEncoder::Options& options, AudioEncoder::OutputCB output_cb, AudioEncoder::EncoderStatusCB done_cb) {
            std::move(done_cb).Run(EncoderStatus::Codes::kOk);
        }));
    ...
    mojo_audio_encoder_->Initialize(options, base::DoNothing(), std::move(expect_ok));
    good_init_run_loop.Run();

    mojo_audio_encoder_->Initialize(options, base::DoNothing(), std::move(expect_error));
...
TEST_F(MojoAudioEncoderTest, Encode)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&, this](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) {
            EXPECT_TRUE(service_task_runner_->RunsTasksInCurrentSequence());
            std::move(done_cb).Run(EncoderStatus::Codes::kOk);
            ...
            AudioParameters params(
                AudioParameters::AUDIO_PCM_LOW_LATENCY, { CHANNEL_LAYOUT_DISCRETE, audio_bus->channels() }, options.sample_rate, audio_bus->frames());

            const auto channel_data = base::make_span(reinterpret_cast<const uint8_t*>(audio_bus->channel(0)),
            ...
            EncodedAudioBuffer output(params, std::move(encoded_data), capture_time);

            std::optional<AudioEncoder::CodecDescription> desc;
            if (input_number > 0)
                desc.emplace(AudioEncoder::CodecDescription { static_cast<uint8_t>(input_number) });
            service_output_cb.Run(std::move(output), desc);
        }));

    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, std::optional<AudioEncoder::CodecDescription> desc) {
        ...
        auto* const encoded_data = reinterpret_cast<const float*>(output.encoded_data.data());
        for (size_t i = 0; i < kFrameCount; i++) {
            EXPECT_EQ(encoded_data[i], seed) << " output_number: " << output_number << " i: " << i;
        }
    ...
    mojo_audio_encoder_->Initialize(options, std::move(output_cb), ValidatingStatusCB());

    for (int i = 0; i < kInputCount; i++) {
        auto ts = FromMilliseconds(i);
        mojo_audio_encoder_->Encode(MakeInput(1.0 / kInputCount * i, options.channels, kFrameCount), ts, ValidatingStatusCB());
    }
...
TEST_F(MojoAudioEncoderTest, EncodeWithEmptyResult)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&, this](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) {
            ...
            AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { CHANNEL_LAYOUT_DISCRETE, 1 }, 8000, 1);

            EncodedAudioBuffer output(params, base::HeapArray<uint8_t>(), capture_time);
    ...
    mojo_audio_encoder_->Encode(MakeInput(1, options.channels, options.sample_rate), ts, ValidatingStatusCB());
...
TEST_F(MojoAudioEncoderTest, Flush)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) {
            std::move(done_cb).Run(EncoderStatus::Codes::kOk);
    ...
    EXPECT_CALL(*mock_audio_encoder_, Flush(_)).WillRepeatedly(Invoke([&, this](AudioEncoder::EncoderStatusCB done_cb) {
        EXPECT_TRUE(service_task_runner_->RunsTasksInCurrentSequence());
        std::move(done_cb).Run(EncoderStatus::Codes::kOk);
    }));

    AudioEncoder::OutputCB output_cb
        = base::BindLambdaForTesting([&](EncodedAudioBuffer output, std::optional<AudioEncoder::CodecDescription>) { output_count++; });

    mojo_audio_encoder_->Initialize(options, std::move(output_cb), ValidatingStatusCB());

    for (int i = 0; i < input_count; i++) {
        auto ts = FromMilliseconds(i);
        mojo_audio_encoder_->Encode(MakeInput(1.0 / input_count * i, options.channels, options.sample_rate), ts, ValidatingStatusCB());
    }
...
TEST_F(MojoAudioEncoderTest, MojoErrorCallsAllDoneCallbacks)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Flush(_)).WillOnce(Invoke([&](AudioEncoder::EncoderStatusCB done_cb) {
        done_callbacks.push_back(std::move(done_cb));
        service_task_runner_->DeleteSoon(FROM_HERE, std::move(receiver_));
    }));

    mojo_audio_encoder_->Initialize(options, base::DoNothing(), ValidatingStatusCB());
    ...
    for (int i = 0; i < input_count; i++) {
        ...
        mojo_audio_encoder_->Encode(MakeInput(1.0 / input_count * i, options.channels, options.sample_rate), ts, std::move(done_cb));
    }
    auto flush_cb = base::BindLambdaForTesting([&](EncoderStatus s) {
        EXPECT_EQ(s.code(), EncoderStatus::Codes::kEncoderMojoConnectionError);
        run_loop.QuitWhenIdle();
