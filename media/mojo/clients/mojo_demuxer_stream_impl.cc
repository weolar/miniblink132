...
MojoDemuxerStreamImpl::MojoDemuxerStreamImpl(media::DemuxerStream* stream, mojo::PendingReceiver<mojom::DemuxerStream> receiver)
...
void MojoDemuxerStreamImpl::Initialize(InitializeCallback callback)
{
    ...
    mojo_decoder_buffer_writer_ = MojoDecoderBufferWriter::Create(GetDefaultDecoderBufferConverterCapacity(stream_->type()), &remote_consumer_handle);

    std::move(callback).Run(stream_->type(), std::move(remote_consumer_handle), audio_config, video_config);
}

void MojoDemuxerStreamImpl::Read(uint32_t count, ReadCallback callback)
{
    DVLOG(3) << __func__ << ": count=" << count;
    stream_->Read(count, base::BindOnce(&MojoDemuxerStreamImpl::OnBufferReady, weak_factory_.GetWeakPtr(), std::move(callback)));
}
...
void MojoDemuxerStreamImpl::OnBufferReady(ReadCallback callback, Status status, media::DemuxerStream::DecoderBufferVector buffers)
{
    std::optional<AudioDecoderConfig> audio_config;
    std::optional<VideoDecoderConfig> video_config;
    DVLOG(3) << __func__ << ": status=" << ::media::DemuxerStream::GetStatusName(status) << ", buffers.size=" << buffers.size();
    ...
    if (status == Status::kConfigChanged) {
        ...
        if (stream_->type() == Type::AUDIO) {
            ...
            NOTREACHED() << "Unsupported config change encountered for type: " << stream_->type();
        }
        std::move(callback).Run(Status::kConfigChanged, {}, audio_config, video_config);
        return;
    }
    ...
    std::move(callback).Run(status, std::move(output_mojo_buffers), audio_config, video_config);
}
