...
class MojoVideoEncoderMetricsProviderFactory : public base::RefCountedThreadSafe<MojoVideoEncoderMetricsProviderFactory> {
public:
    MojoVideoEncoderMetricsProviderFactory(mojom::VideoEncoderUseCase use_case, mojo::PendingRemote<mojom::VideoEncoderMetricsProvider> pending_remote);

    // Virtual for unit tests like rtc_video_encoder_test.cc.
    virtual std::unique_ptr<VideoEncoderMetricsProvider> CreateVideoEncoderMetricsProvider();

protected:
    friend class base::RefCountedThreadSafe<MojoVideoEncoderMetricsProviderFactory>;
    virtual ~MojoVideoEncoderMetricsProviderFactory();

    // For easily mocking in unit tests.
    explicit MojoVideoEncoderMetricsProviderFactory(mojom::VideoEncoderUseCase use_case);
...
private:
    ...
    const mojom::VideoEncoderUseCase use_case_ GUARDED_BY_CONTEXT(create_provider_sequence_checker_);
    std::atomic_uint64_t encoder_id_ GUARDED_BY_CONTEXT(create_provider_sequence_checker_) { 0 };

    mojo::PendingRemote<mojom::VideoEncoderMetricsProvider> pending_remote_ GUARDED_BY_CONTEXT(remote_sequence_checker_);
    mojo::Remote<mojom::VideoEncoderMetricsProvider> remote_ GUARDED_BY_CONTEXT(remote_sequence_checker_);
