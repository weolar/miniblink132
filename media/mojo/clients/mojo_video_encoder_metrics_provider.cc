...
namespace media {
class MojoVideoEncoderMetricsProviderFactory::MojoVideoEncoderMetricsProvider : public VideoEncoderMetricsProvider {
public:
    MojoVideoEncoderMetricsProvider(scoped_refptr<MojoVideoEncoderMetricsProviderFactory> factory, mojom::VideoEncoderUseCase use_case, uint64_t encoder_id)
    ...
    void Initialize(VideoCodecProfile codec_profile, const gfx::Size& encode_size, bool is_hardware_encoder, SVCScalabilityMode svc_mode) override
    {
        ...
        (*remote_)->Initialize(encoder_id_, use_case_, codec_profile, encode_size, is_hardware_encoder, svc_mode);
    }
    ...
    void IncrementEncodedFrameCount() override
    {
        ...
        if (num_encoded_frames_ % kEncodedFrameCountBucketSize == 0 || num_encoded_frames_ == 1u) {
...
private:
    ...
    raw_ptr<mojo::Remote<mojom::VideoEncoderMetricsProvider>> remote_ GUARDED_BY_CONTEXT(sequence_checker_) { nullptr };

    const mojom::VideoEncoderUseCase use_case_ GUARDED_BY_CONTEXT(sequence_checker_);
...
MojoVideoEncoderMetricsProviderFactory::MojoVideoEncoderMetricsProviderFactory(
    mojom::VideoEncoderUseCase use_case, mojo::PendingRemote<mojom::VideoEncoderMetricsProvider> pending_remote)
    : use_case_(use_case)
...
MojoVideoEncoderMetricsProviderFactory::MojoVideoEncoderMetricsProviderFactory(mojom::VideoEncoderUseCase use_case)
    : use_case_(use_case)
{
    DETACH_FROM_SEQUENCE(remote_sequence_checker_);
    DETACH_FROM_SEQUENCE(create_provider_sequence_checker_);
...
mojo::Remote<mojom::VideoEncoderMetricsProvider>* MojoVideoEncoderMetricsProviderFactory::GetRemote()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(remote_sequence_checker_);
...
std::unique_ptr<VideoEncoderMetricsProvider> MojoVideoEncoderMetricsProviderFactory::CreateVideoEncoderMetricsProvider()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(create_provider_sequence_checker_);
    return std::make_unique<MojoVideoEncoderMetricsProvider>(this, use_case_, encoder_id_++);
}
} // namespace media
