...
class MojoAudioEncoder final : public AudioEncoder, public mojom::AudioEncoderClient {
public:
    explicit MojoAudioEncoder(mojo::PendingRemote<mojom::AudioEncoder> remote_encoder);
    ...
    void Initialize(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb) final;

    void Encode(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, EncoderStatusCB done_cb) final;

    void Flush(EncoderStatusCB done_cb) final;

    // AudioEncoderClient implementation.
    void OnEncodedBufferReady(media::EncodedAudioBuffer buffer, const CodecDescription& desc) final;
...
private:
    ...
    using WrappedEncoderStatusCB = base::OnceCallback<void(const EncoderStatus& error)>;

    void CallAndReleaseCallback(PendingCallbackHandle handle, const EncoderStatus& status);
    void CallAndReleaseAllPendingCallbacks(EncoderStatus status) VALID_CONTEXT_REQUIRED(sequence_checker_);
    WrappedEncoderStatusCB WrapCallbackAsPending(EncoderStatusCB callback) VALID_CONTEXT_REQUIRED(sequence_checker_);
    ...
    mojo::PendingRemote<mojom::AudioEncoder> pending_remote_encoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    mojo::Remote<mojom::AudioEncoder> remote_encoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    mojo::AssociatedReceiver<mojom::AudioEncoderClient> client_receiver_ { this };
    scoped_refptr<AudioBufferMemoryPool> buffer_pool_ GUARDED_BY_CONTEXT(sequence_checker_);
