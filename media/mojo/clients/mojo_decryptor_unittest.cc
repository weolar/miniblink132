...
public:
    ...
    void Initialize()
    {
        ...
        mojo_decryptor_service_ = std::make_unique<MojoDecryptorService>(decryptor_.get(), nullptr);

        receiver_ = std::make_unique<mojo::Receiver<mojom::Decryptor>>(mojo_decryptor_service_.get());
        mojo_decryptor_ = std::make_unique<MojoDecryptor>(receiver_->BindNewPipeAndPassRemote(), writer_capacity_);
        receiver_->set_disconnect_handler(base::BindOnce(&MojoDecryptorTest::OnConnectionClosed, base::Unretained(this)));
    }
    ...
    void ReturnSharedBufferVideoFrame(scoped_refptr<DecoderBuffer> encrypted, Decryptor::VideoDecodeCB video_decode_cb)
    {
        ...
        scoped_refptr<VideoFrame> frame = VideoFrame::WrapExternalYuvData(PIXEL_FORMAT_I420, gfx::Size(100, 100), gfx::Rect(100, 100), gfx::Size(100, 100), 100,
            50, 50, data, data + 100 * 100, data + (100 * 100 * 5 / 4), base::Seconds(100));
        auto read_only_mapping = region.region.Map();
        CHECK(read_only_mapping.IsValid());
        frame->BackWithOwnedSharedMemory(std::move(region.region), std::move(read_only_mapping));
        frame->AddDestructionObserver(base::BindOnce(&MojoDecryptorTest::OnFrameDestroyed, base::Unretained(this)));

        std::move(video_decode_cb).Run(Decryptor::kSuccess, std::move(frame));
    }

    void ReturnAudioFrames(scoped_refptr<DecoderBuffer> encrypted, Decryptor::AudioDecodeCB audio_decode_cb)
    {
        ...
        auto audio_buffer = MakeAudioBuffer<float>(
            kSampleFormatPlanarF32, kChannelLayout, ChannelLayoutToChannelCount(kChannelLayout), kSampleRate, 0.0f, 1.0f, kSampleRate / 10, start_time);
        Decryptor::AudioFrames audio_frames = { audio_buffer };
        std::move(audio_decode_cb).Run(Decryptor::kSuccess, audio_frames);
    }

    void ReturnEOSVideoFrame(scoped_refptr<DecoderBuffer> encrypted, Decryptor::VideoDecodeCB video_decode_cb)
    {
        // Simply create and return an End-Of-Stream VideoFrame.
        std::move(video_decode_cb).Run(Decryptor::kSuccess, VideoFrame::CreateEOSFrame());
    }

    MOCK_METHOD2(AudioDecoded, void(Decryptor::Status status, const Decryptor::AudioFrames& frames));
    MOCK_METHOD2(VideoDecoded, void(Decryptor::Status status, scoped_refptr<VideoFrame> frame));
...
TEST_F(MojoDecryptorTest, Reset_DuringDecryptAndDecode_Audio)
{
...

    {
        ...
        InSequence seq;
        EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(_, _)).WillOnce(Invoke(this, &MojoDecryptorTest::ReturnAudioFrames));
    ...
    mojo_decryptor_->DecryptAndDecodeAudio(std::move(buffer), base::BindRepeating(&MojoDecryptorTest::AudioDecoded, base::Unretained(this)));
...
TEST_F(MojoDecryptorTest, Reset_DuringDecryptAndDecode_Video)
{
...

    {
        // Make sure calls are made in order.
        InSequence seq;
        EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillOnce(Invoke(this, &MojoDecryptorTest::ReturnSharedBufferVideoFrame));
    ...
    scoped_refptr<DecoderBuffer> buffer(new DecoderBuffer(100));
    mojo_decryptor_->DecryptAndDecodeVideo(std::move(buffer), base::BindRepeating(&MojoDecryptorTest::VideoDecoded, base::Unretained(this)));
...
TEST_F(MojoDecryptorTest, Reset_DuringDecryptAndDecode_AudioAndVideo)
{
    ...
    mojo_decryptor_->DecryptAndDecodeAudio(buffer, base::BindRepeating(&MojoDecryptorTest::AudioDecoded, base::Unretained(this)));
    mojo_decryptor_->DecryptAndDecodeVideo(std::move(buffer), base::BindRepeating(&MojoDecryptorTest::VideoDecoded, base::Unretained(this)));
    mojo_decryptor_->ResetDecoder(Decryptor::kAudio);
    mojo_decryptor_->ResetDecoder(Decryptor::kVideo);
    base::RunLoop().RunUntilIdle();
...
TEST_F(MojoDecryptorTest, VideoDecodeFreesMultipleBuffers)
{
    ...
    const int TIMES = 5;
    EXPECT_CALL(*this, VideoDecoded(Decryptor::Status::kSuccess, NotNull())).Times(TIMES);
    EXPECT_CALL(*this, OnFrameDestroyed()).Times(TIMES);
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(Invoke(this, &MojoDecryptorTest::ReturnSharedBufferVideoFrame));
...
TEST_F(MojoDecryptorTest, VideoDecodeHoldThenFreeBuffers)
{
    ...
    EXPECT_CALL(*this, VideoDecoded(Decryptor::Status::kSuccess, NotNull())).WillOnce(SaveArg<1>(&saved_frame1)).WillOnce(SaveArg<1>(&saved_frame2));
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(Invoke(this, &MojoDecryptorTest::ReturnSharedBufferVideoFrame));
...
TEST_F(MojoDecryptorTest, EOSBuffer)
{
    ...
    EXPECT_CALL(*this, VideoDecoded(Decryptor::Status::kSuccess, NotNull()));
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillOnce(Invoke(this, &MojoDecryptorTest::ReturnEOSVideoFrame));
