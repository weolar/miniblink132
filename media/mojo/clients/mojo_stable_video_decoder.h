...
public:
    MojoStableVideoDecoder(scoped_refptr<base::SequencedTaskRunner> media_task_runner, GpuVideoAcceleratorFactories* gpu_factories, MediaLog* media_log,
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder);
...
private:
    ...
    void InitializeOnceSupportedConfigsAreKnown(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb,
    ...
    scoped_refptr<SharedImageHolder> CreateOrUpdateSharedImageForFrame(const scoped_refptr<FrameResource>& frame_resource);
    ...
    const raw_ptr<GpuVideoAcceleratorFactories> gpu_factories_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    mojo::PendingRemote<stable::mojom::StableVideoDecoder> pending_remote_decoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    std::unique_ptr<VideoDecoder> oop_video_decoder_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    base::WeakPtrFactory<MojoStableVideoDecoder> weak_this_factory_ GUARDED_BY_CONTEXT(sequence_checker_);
};
