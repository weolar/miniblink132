...
public:
    ...
    class MockAndroidOverlayProvider : public StrictMock<mojom::AndroidOverlayProvider> {
    public:
        // These argument types lack default constructors, so gmock can't mock them.
        void CreateOverlay(mojo::PendingReceiver<mojom::AndroidOverlay> overlay_receiver, mojo::PendingRemote<mojom::AndroidOverlayClient> client,
...
public:
    ...
    void SetUp() override
    {
        ...
        config_.ready_cb = base::BindOnce(&MockClientCallbacks::OnReady, base::Unretained(&callbacks_));
        config_.failed_cb = base::BindOnce(&MockClientCallbacks::OnFailed, base::Unretained(&callbacks_));
        config_.power_cb = base::BindRepeating(&MockClientCallbacks::OnPowerEfficient, base::Unretained(&callbacks_));
    ...
    void CreateOverlay()
    {
        ...
        overlay_client_ = std::make_unique<MojoAndroidOverlay>(provider_receiver_.BindNewPipeAndPassRemote(), std::move(config_), routing_token);
        overlay_client_->AddSurfaceDestroyedCallback(base::BindOnce(&MockClientCallbacks::OnDestroyed, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void CreateSurface()
    {
        ...
        surface_key_ = gpu::GpuSurfaceTracker::Get()->AddSurfaceForNativeWidget(
            gpu::SurfaceRecord(surface_.CopyRetainOwnership(), false /* can_be_used_with_surface_control */));
        ...
        ASSERT_TRUE(env->IsSameObject(surface_.j_surface().obj(), overlay_client_->GetJavaSurface().obj()));
    }
