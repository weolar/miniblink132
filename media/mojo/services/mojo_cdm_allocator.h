...
private:
    ...
    using AvailableRegionMap = std::multimap<size_t, std::unique_ptr<base::MappedReadOnlyRegion>>;

    // Allocates a shmem region of at least |capacity| bytes.
    std::unique_ptr<base::MappedReadOnlyRegion> AllocateNewRegion(size_t capacity);

    // Returns |region| to the map of available buffers, ready to be used the
    // next time CreateCdmBuffer() is called.
    void AddRegionToAvailableMap(std::unique_ptr<base::MappedReadOnlyRegion> region);
