...
CdmServiceBroker::CdmServiceBroker(std::unique_ptr<CdmService::Client> client, mojo::PendingReceiver<mojom::CdmServiceBroker> receiver)
...
void CdmServiceBroker::GetService(const base::FilePath& cdm_path,
#if BUILDFLAG(IS_MAC)
    ...
    mojo::PendingReceiver<mojom::CdmService> service_receiver)
{
    ...
    DCHECK(!cdm_service_);
    cdm_service_ = std::make_unique<CdmService>(std::move(client_), std::move(service_receiver));
    ...
    if (!success) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&CdmServiceBroker::TerminateService, weak_factory_.GetWeakPtr()), kServiceTerminationDelay);
    }
}
    ...
    const base::FilePath& cdm_path)
{
    ...
    if (token_provider) {
        ...
        CHECK(mojo::Remote<mojom::SeatbeltExtensionTokenProvider>(std::move(token_provider))->GetTokens(&tokens));
