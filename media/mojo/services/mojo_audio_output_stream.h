...
class MEDIA_MOJO_EXPORT MojoAudioOutputStream : public mojom::AudioOutputStream, public AudioOutputDelegate::EventHandler {
public:
    using StreamCreatedCallback = base::OnceCallback<void(mojo::PendingRemote<mojom::AudioOutputStream>, media::mojom::ReadWriteAudioDataPipePtr)>;
    using CreateDelegateCallback = base::OnceCallback<std::unique_ptr<AudioOutputDelegate>(AudioOutputDelegate::EventHandler*)>;
    ...
    MojoAudioOutputStream(CreateDelegateCallback create_delegate_callback, StreamCreatedCallback stream_created_callback, DeleterCallback deleter_callback);
...
private:
    ...
    void OnStreamCreated(
        int stream_id, base::UnsafeSharedMemoryRegion shared_memory_region, std::unique_ptr<base::CancelableSyncSocket> foreign_socket) override;
