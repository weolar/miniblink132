...
MojoDemuxerStreamAdapter::MojoDemuxerStreamAdapter(mojo::PendingRemote<mojom::DemuxerStream> demuxer_stream, base::OnceClosure stream_ready_cb)
    : demuxer_stream_(std::move(demuxer_stream))
    , stream_ready_cb_(std::move(stream_ready_cb))
{
    DVLOG(1) << __func__;
    demuxer_stream_->Initialize(base::BindOnce(&MojoDemuxerStreamAdapter::OnStreamReady, weak_factory_.GetWeakPtr()));
}
...
void MojoDemuxerStreamAdapter::Read(uint32_t count, ReadCB read_cb)
{
    ...
    demuxer_stream_->Read(count, base::BindOnce(&MojoDemuxerStreamAdapter::OnBufferReady, weak_factory_.GetWeakPtr()));
}
...
void MojoDemuxerStreamAdapter::OnStreamReady(Type type, mojo::ScopedDataPipeConsumerHandle consumer_handle,
    const std::optional<AudioDecoderConfig>& audio_config, const std::optional<VideoDecoderConfig>& video_config)
{
    DVLOG(1) << __func__;
    ...
    mojo_decoder_buffer_reader_ = std::make_unique<MojoDecoderBufferReader>(std::move(consumer_handle));
...
void MojoDemuxerStreamAdapter::OnBufferReady(Status status, std::vector<mojom::DecoderBufferPtr> batch_buffers,
    const std::optional<AudioDecoderConfig>& audio_config, const std::optional<VideoDecoderConfig>& video_config)
{
    DVLOG(3) << __func__ << ": status=" << ::media::DemuxerStream::GetStatusName(status) << ", batch_buffers.size=" << batch_buffers.size();
    DCHECK(read_cb_);
    ...
    mojo_decoder_buffer_reader_->ReadDecoderBuffer(
        std::move(batch_buffers_[0]), base::BindOnce(&MojoDemuxerStreamAdapter::OnBufferRead, weak_factory_.GetWeakPtr()));
}

void MojoDemuxerStreamAdapter::OnBufferRead(scoped_refptr<DecoderBuffer> buffer)
{
    if (!buffer) {
    ...
    if (buffer_queue_.size() < actual_read_count_) {
        ...
        mojo_decoder_buffer_reader_->ReadDecoderBuffer(std::move(batch_buffers_[next_read_index]),
            base::BindPostTaskToCurrentDefault(base::BindOnce(&MojoDemuxerStreamAdapter::OnBufferRead, weak_factory_.GetWeakPtr())));
        return;
    }
...
void MojoDemuxerStreamAdapter::UpdateConfig(const std::optional<AudioDecoderConfig>& audio_config, const std::optional<VideoDecoderConfig>& video_config)
{
    ...
    case AUDIO:
        ...
        TRACE_EVENT_INSTANT2("media", "MojoDemuxerStreamAdapter.UpdateConfig.Audio", TRACE_EVENT_SCOPE_THREAD, "CurrentConfig", old_decoder_config_str,
    ...
    case VIDEO:
        ...
        TRACE_EVENT_INSTANT2("media", "MojoDemuxerStreamAdapter.UpdateConfig.Video", TRACE_EVENT_SCOPE_THREAD, "CurrentConfig", old_decoder_config_str,
