...
class MEDIA_MOJO_EXPORT MojoAudioEncoderService final : public mojom::AudioEncoder {
public:
    explicit MojoAudioEncoderService(std::unique_ptr<media::AudioEncoder> encoder);
    ...
    void Initialize(mojo::PendingAssociatedRemote<mojom::AudioEncoderClient> client, const AudioEncoderConfig& config, InitializeCallback callback) final;
...
private:
    using MojoDoneCallback = base::OnceCallback<void(const media::EncoderStatus&)>;
    void OnDone(MojoDoneCallback callback, EncoderStatus error);
    void OnOutput(EncodedAudioBuffer output, std::optional<media::AudioEncoder::CodecDescription> desc);
