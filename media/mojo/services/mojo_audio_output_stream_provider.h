...
class MEDIA_MOJO_EXPORT MojoAudioOutputStreamProvider : public mojom::AudioOutputStreamProvider {
public:
    using CreateDelegateCallback = base::OnceCallback<std::unique_ptr<AudioOutputDelegate>(
        const AudioParameters& params, mojo::PendingRemote<mojom::AudioOutputStreamObserver>, AudioOutputDelegate::EventHandler*)>;
    ...
    MojoAudioOutputStreamProvider(mojo::PendingReceiver<mojom::AudioOutputStreamProvider> pending_receiver, CreateDelegateCallback create_delegate_callback,
        DeleterCallback deleter_callback, std::unique_ptr<mojom::AudioOutputStreamObserver> observer);

    MojoAudioOutputStreamProvider(const MojoAudioOutputStreamProvider&) = delete;
    MojoAudioOutputStreamProvider& operator=(const MojoAudioOutputStreamProvider&) = delete;

    ~MojoAudioOutputStreamProvider() override;

private:
    // mojom::AudioOutputStreamProvider implementation.
    void Acquire(const AudioParameters& params, mojo::PendingRemote<mojom::AudioOutputStreamProviderClient> provider_client) override;
