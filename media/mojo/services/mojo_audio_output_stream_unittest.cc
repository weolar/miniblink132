...
public:
    void PrepareDelegateForCreation(std::unique_ptr<AudioOutputDelegate> delegate)
    {
        ASSERT_EQ(nullptr, delegate_);
        delegate_.swap(delegate);
    }

    std::unique_ptr<AudioOutputDelegate> CreateDelegate(AudioOutputDelegate::EventHandler* handler)
    {
...
std::unique_ptr<AudioOutputDelegate> CreateNoDelegate(AudioOutputDelegate::EventHandler* event_handler)
{
    return nullptr;
}

void NotCalled(mojo::PendingRemote<mojom::AudioOutputStream>, mojom::ReadWriteAudioDataPipePtr)
{
...
public:
    ...
    mojo::Remote<mojom::AudioOutputStream> CreateAudioOutput()
    {
        ...
        impl_ = std::make_unique<MojoAudioOutputStream>(base::BindOnce(&MockDelegateFactory::CreateDelegate, base::Unretained(&mock_delegate_factory_)),
            base::BindOnce(&MojoAudioOutputStreamTest::CreatedStream, base::Unretained(this)),
            base::BindOnce(&MockDeleter::Finished, base::Unretained(&deleter_)));
        return remote;
    }

protected:
    void CreatedStream(mojo::PendingRemote<mojom::AudioOutputStream> stream, mojom::ReadWriteAudioDataPipePtr data_pipe)
    {
        EXPECT_EQ(mojo::FuseMessagePipes(pending_stream_receiver.PassPipe(), stream.PassPipe()), MOJO_RESULT_OK);
...
TEST_F(MojoAudioOutputStreamTest, NoDelegate_SignalsError)
{
    MojoAudioOutputStream stream(
        base::BindOnce(&CreateNoDelegate), base::BindOnce(&NotCalled), base::BindOnce(&MockDeleter::Finished, base::Unretained(&deleter_)));
...
{
    mojo::Remote<mojom::AudioOutputStream> audio_output_remote = CreateAudioOutput();

    EXPECT_CALL(client_, GotNotification());
    EXPECT_CALL(*delegate_, OnPlayStream());

    delegate_event_handler_->OnStreamCreated(kStreamId, std::move(mem_), std::move(foreign_socket_));
