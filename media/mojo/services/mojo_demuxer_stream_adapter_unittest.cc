...
public:
    ...
    void Initialize(DemuxerStream::Type StreamType)
    {
        ...
        mojo_client_stream_ = std::make_unique<MojoDemuxerStreamImpl>(stream_.get(), stream_remote.InitWithNewPipeAndPassReceiver());

        mojo_stream_adapter_ = std::make_unique<MojoDemuxerStreamAdapter>(std::move(stream_remote), std::move(init_done_cb));
        init_loop.Run();
    }
...
TEST_F(MojoDemuxerStreamAdapterTest, InitializeAudioStreamAndReadMultiBuffer)
{
...

    {
        ...
        EXPECT_CALL(*stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::Status::kOk, buffers));

        auto done_cb = base::BindLambdaForTesting([&](DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers) {
...

    {
        ...
        EXPECT_CALL(*stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::Status::kConfigChanged, DemuxerStream::DecoderBufferVector()));
...

    {
        ...
        EXPECT_CALL(*stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::Status::kAborted, DemuxerStream::DecoderBufferVector()));
