...
class MEDIA_MOJO_EXPORT MojoVideoEncoderMetricsProviderService : public mojom::VideoEncoderMetricsProvider {
public:
    static void Create(ukm::SourceId source_id, mojo::PendingReceiver<mojom::VideoEncoderMetricsProvider> receiver);

    ~MojoVideoEncoderMetricsProviderService() override;

    // mojom::VideoEncoderMetricsProvider implementation.
    void Initialize(uint64_t encoder_id, mojom::VideoEncoderUseCase encoder_use_case, VideoCodecProfile codec_profile, const gfx::Size& encode_size,
        bool is_hardware_encoder, SVCScalabilityMode svc_mode) override;
