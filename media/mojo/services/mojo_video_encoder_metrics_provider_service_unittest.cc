...
std::tuple<std::unique_ptr<ukm::TestAutoSetUkmRecorder>, mojo::Remote<mojom::VideoEncoderMetricsProvider>> Create(const std::string& url)
{
    ...
    MojoVideoEncoderMetricsProviderService::Create(source_id, provider.BindNewPipeAndPassReceiver());
    return { std::move(test_recorder), std::move(provider) };
}
} // namespace

class MojoVideoEncoderMetricsProviderServiceTest
    : public TestWithParam<testing::tuple<uint64_t, mojom::VideoEncoderUseCase, VideoCodecProfile, gfx::Size, bool, SVCScalabilityMode, EncoderStatus::Codes>> {
public:
...
#define EXPECT_UKM(name, value) test_recorder->ExpectEntryMetric(entry, name, value)

TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitialize_ReportUKM)
{
    auto [test_recorder, provider] = Create(kTestURL);
    ...
    provider->Initialize(kEncoderId, kEncoderUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
    provider.reset();
    base::RunLoop().RunUntilIdle();
    ...
    EXPECT_UKM(UkmEntry::kStatusName, static_cast<int64_t>(EncoderStatus::Codes::kOk));
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitializeAndComplete_ReportUKM)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitializeAndSetSmallNumberEncodedFrameCount_ReportUKMWithOneBucket)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
...
TEST_P(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitializeAndSetEncodedFrameCount_ReportUKM)
{
    auto [test_recorder, provider] = Create(kTestURL);
    ...
    provider->Initialize(encoder_id, encoder_use_case, codec_profile, encode_size, is_hardware_encoder, svc_mode);
    ...
    EXPECT_UKM(UkmEntry::kStatusName, static_cast<int64_t>(encoder_status.code()));
}

TEST_P(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitializeAndSetEncodedFrameCount_ReportUMA)
{
...
#define EXPECT_UMA(name, value)                                                                                                                                \
    do {                                                                                                                                                       \
        histogram_tester.ExpectUniqueSample(base::StrCat({ uma_prefix, name }), value, 1);                                                                     \
    } while (0)
...
INSTANTIATE_TEST_SUITE_P(All, MojoVideoEncoderMetricsProviderServiceTest,
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, InitializeWithVerfiyLargeResoloution_ReportCappedResolutionUKM)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
    ...
    constexpr auto kSVCMode = SVCScalabilityMode::kL1T3;
    constexpr uint64_t kNumEncodedFrames = 100;
    provider->Initialize(kEncoderId, kEncoderUseCase, kCodecProfile, k16kEncodeSize, kIsHardwareEncoder, kSVCMode);
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CallSetEncodedFrameCounts_ReportUKMWithTheLastEncodedFrameCount)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CreateAndInitializeAndCallSetErrors_ReportUKMWithTheFirstError)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
    ...
    constexpr auto kSVCMode = SVCScalabilityMode::kL1T3;
    provider->Initialize(kEncoderId, kEncoderUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
    provider->SetError(kEncoderId, { EncoderStatus::Codes::kEncoderMojoConnectionError, "mojo connection is disclosed" });
    provider->SetError(kEncoderId, { EncoderStatus::Codes::kEncoderFailedEncode, "Encoder failed" });
    provider.reset();
    base::RunLoop().RunUntilIdle();
    ...
    EXPECT_UKM(UkmEntry::kProfileName, kCodecProfile);
    EXPECT_UKM(UkmEntry::kStatusName, static_cast<int64_t>(EncoderStatus::Codes::kEncoderMojoConnectionError));
    EXPECT_UKM(UkmEntry::kSVCModeName, static_cast<int64_t>(kSVCMode));
    EXPECT_UKM(UkmEntry::kUseCaseName, static_cast<int64_t>(kEncoderUseCase));
    EXPECT_UKM(UkmEntry::kWidthName, kWidth);
}

TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CallErrorAndNoCallSetEncodedFramesCount_ReportUKMWithTheFirstError)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CallSetEncodedFrameCountsAndSetError_ReportUKMWithTheFirstErrorAndTheLastEncodedFrameCount)
{
    auto [test_recorder, provider] = Create(kTestURL);
    constexpr uint64_t kEncoderId = 0;
    constexpr auto kEncoderUseCase = mojom::VideoEncoderUseCase::kWebRTC;
    ...
    provider->SetEncodedFrameCount(kEncoderId, 300);
    provider->SetError(kEncoderId, { EncoderStatus::Codes::kEncoderFailedEncode, "Encoder failed" });
    provider->SetError(kEncoderId, { EncoderStatus::Codes::kEncoderIllegalState, "Encoder illegal state" });
    ...
    EXPECT_UKM(UkmEntry::kProfileName, kCodecProfile);
    EXPECT_UKM(UkmEntry::kStatusName, static_cast<int64_t>(EncoderStatus::Codes::kEncoderFailedEncode));
    EXPECT_UKM(UkmEntry::kSVCModeName, static_cast<int64_t>(kSVCMode));
    EXPECT_UKM(UkmEntry::kUseCaseName, static_cast<int64_t>(kEncoderUseCase));
    EXPECT_UKM(UkmEntry::kWidthName, kWidth);
}

TEST_F(MojoVideoEncoderMetricsProviderServiceTest, CreateAndTwoInitializeAndSetEncodedFrameCounts_ReportTwoUKMs)
{
    const struct {
    ...
    for (const auto& metrics : kMetricsCases) {
        provider->Initialize(kEncoderId, metrics.use_case, metrics.profile, metrics.size, metrics.is_hardware, metrics.svc_mode);
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, HandleTwoEncoders)
{
    ...
    auto [test_recorder, provider] = Create(kTestURL);
    for (const auto& metrics : kMetricsCases) {
        provider->Initialize(metrics.encoder_id, metrics.use_case, metrics.profile, metrics.size, metrics.is_hardware, metrics.svc_mode);
    }
    for (const auto& metrics : kMetricsCases) {
        provider->SetEncodedFrameCount(metrics.encoder_id, metrics.num_encoded_frames);
    }
    for (const auto& metrics : kMetricsCases) {
        if (metrics.status != EncoderStatus::Codes::kOk) {
...
TEST_F(MojoVideoEncoderMetricsProviderServiceTest, IgnoreUnknownEncoderIds)
{
    ...
    constexpr bool kIsHardwareEncoder = true;
    constexpr auto kSVCMode = SVCScalabilityMode::kL1T3;
    provider->Initialize(kKnownEncoderId, kEncoderUseCase, kCodecProfile, kEncodeSize, kIsHardwareEncoder, kSVCMode);
    provider->SetEncodedFrameCount(kUnknownEncoderId, 100);
    provider->SetError(kUnknownEncoderId, EncoderStatus::Codes::kEncoderFailedEncode);
    provider->Complete(kUnknownEncoderId);

    provider->Complete(kKnownEncoderId);
    // This should be ignored as Complete() is already called.
    provider->SetError(kKnownEncoderId, EncoderStatus::Codes::kEncoderFailedEncode);
