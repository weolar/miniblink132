...
public:
    MockCdmStorage(mojo::PendingReceiver<mojom::CdmStorage> receiver, MockCdmFile* cdm_file)
        : receiver_(this, std::move(receiver))
        , client_receiver_(cdm_file)
    {
    }
    ...
    void Open(const std::string& file_name, OpenCallback callback) override
    {
        ...
        std::move(callback).Run(mojom::CdmStorage::Status::kSuccess, std::move(client_remote));
...
protected:
    ...
    void SetUp() override
    {
        ...
        cdm_storage_ = std::make_unique<MockCdmStorage>(cdm_storage_remote.BindNewPipeAndPassReceiver(), &cdm_file_);

        file_io_ = std::make_unique<MojoCdmFileIO>(this, client_.get(), std::move(cdm_storage_remote));
    }
...
TEST_F(MojoCdmFileIOTest, Read)
{
    ...
    EXPECT_CALL(cdm_file_, Read(_)).WillOnce([](mojom::CdmFile::ReadCallback callback) {
...
TEST_F(MojoCdmFileIOTest, Write)
{
    ...
    EXPECT_CALL(cdm_file_, Write(_, _)).WillOnce([](Unused, mojom::CdmFile::WriteCallback callback) {
