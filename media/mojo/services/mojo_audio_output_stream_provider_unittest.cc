...
using MockDeleter = base::MockCallback<base::OnceCallback<void(mojom::AudioOutputStreamProvider*)>>;
...
public:
    explicit FakeDelegate(mojo::PendingRemote<media::mojom::AudioOutputStreamObserver> pending_observer)
...
std::unique_ptr<AudioOutputDelegate> CreateFakeDelegate(
    const AudioParameters& params, mojo::PendingRemote<media::mojom::AudioOutputStreamObserver> pending_observer, AudioOutputDelegate::EventHandler*)
{
...
TEST(MojoAudioOutputStreamProviderTest, AcquireTwice_BadMessage)
{
    ...
    mojo::SetDefaultProcessErrorHandler(base::BindRepeating([](bool* got_bad_message, const std::string& s) { *got_bad_message = true; }, &got_bad_message));
    ...
    auto* provider = new MojoAudioOutputStreamProvider(
        provider_remote.BindNewPipeAndPassReceiver(), base::BindOnce(&CreateFakeDelegate), deleter.Get(), std::make_unique<FakeObserver>());

    mojo::PendingRemote<mojom::AudioOutputStreamProviderClient> client_1;
    std::ignore = client_1.InitWithNewPipeAndPassReceiver();
    provider_remote->Acquire(media::AudioParameters::UnavailableDeviceParams(), std::move(client_1));

    mojo::PendingRemote<mojom::AudioOutputStreamProviderClient> client_2;
    std::ignore = client_2.InitWithNewPipeAndPassReceiver();
    provider_remote->Acquire(media::AudioParameters::UnavailableDeviceParams(), std::move(client_2));
...
TEST(MojoAudioOutputStreamProviderTest, Bitstream_BadMessageOnNonAndoirdPlatforms)
{
    base::test::SingleThreadTaskEnvironment task_environment;
    bool got_bad_message = false;
    mojo::SetDefaultProcessErrorHandler(base::BindRepeating([](bool* got_bad_message, const std::string& s) { *got_bad_message = true; }, &got_bad_message));
    ...
    mojo::Remote<mojom::AudioOutputStreamProvider> provider_remote;
    StrictMock<MockDeleter> deleter;
    media::AudioParameters params = media::AudioParameters::UnavailableDeviceParams();
