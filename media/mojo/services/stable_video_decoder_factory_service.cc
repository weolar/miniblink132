...
public:
    MojoMediaClientImpl(const gpu::GpuFeatureInfo& gpu_feature_info, scoped_refptr<FrameRegistry> frame_registry)
    ...
    std::vector<SupportedVideoDecoderConfig> GetSupportedVideoDecoderConfigs() final
    {
        std::optional<std::vector<SupportedVideoDecoderConfig>> configs;
        switch (GetDecoderImplementationType()) {
        case VideoDecoderType::kVaapi:
        case VideoDecoderType::kV4L2:
            configs = VideoDecoderPipeline::GetSupportedConfigs(GetDecoderImplementationType(), gpu_driver_bug_workarounds_);
            break;
        default:
            NOTREACHED();
        }
...
#error support video decode acceleration through either VA-API or V4L2.
        ...
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder) final
    {
        ...
        return VideoDecoderPipeline::Create(gpu_driver_bug_workarounds_,
            /*client_task_runner=*/std::move(task_runner), std::make_unique<PlatformVideoFramePool>(), RegisteredFrameConverter::Create(frame_registry_),
            VideoDecoderPipeline::DefaultPreferredRenderableFourccs(), std::move(log),
            /*oop_video_decoder=*/ {},
...
StableVideoDecoderFactoryService::StableVideoDecoderFactoryService(const gpu::GpuFeatureInfo& gpu_feature_info)
    : receiver_(this)
    , frame_registry_(base::MakeRefCounted<FrameRegistry>())
    , mojo_media_client_(std::make_unique<MojoMediaClientImpl>(gpu_feature_info, frame_registry_))
{
...
void StableVideoDecoderFactoryService::BindReceiver(mojo::PendingReceiver<stable::mojom::StableVideoDecoderFactory> receiver, base::OnceClosure disconnect_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void StableVideoDecoderFactoryService::CreateStableVideoDecoder(
    mojo::PendingReceiver<stable::mojom::StableVideoDecoder> receiver, mojo::PendingRemote<stable::mojom::StableVideoDecoderTracker> tracker)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    std::unique_ptr<mojom::VideoDecoder> dst_video_decoder;
    if (video_decoder_creation_cb_for_testing_) {
        dst_video_decoder = video_decoder_creation_cb_for_testing_.Run(mojo_media_client_.get(), &cdm_service_context_);
    } else {
        dst_video_decoder = std::make_unique<MojoVideoDecoderService>(
            mojo_media_client_.get(), &cdm_service_context_, mojo::PendingRemote<stable::mojom::StableVideoDecoder>());
    }
    video_decoders_.Add(std::make_unique<StableVideoDecoderService>(std::move(tracker), std::move(dst_video_decoder), &cdm_service_context_, frame_registry_),
        std::move(receiver));
}
