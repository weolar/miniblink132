...
public:
    explicit MojoMediaDrmStorage(mojo::PendingRemote<mojom::MediaDrmStorage> media_drm_storage);
    ...
    void SavePersistentSession(const std::string& session_id, const SessionData& session_data, ResultCB result_cb) override;
    void LoadPersistentSession(const std::string& session_id, LoadPersistentSessionCB load_persistent_session_cb) override;
    void RemovePersistentSession(const std::string& session_id, ResultCB result_cb) override;
    base::WeakPtr<MediaDrmStorage> AsWeakPtr() override;

private:
    void OnPersistentSessionLoaded(LoadPersistentSessionCB load_persistent_session_cb, mojom::SessionDataPtr session_data);
