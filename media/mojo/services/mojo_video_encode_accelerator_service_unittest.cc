...
std::unique_ptr<VideoEncodeAccelerator> CreateAndInitializeFakeVEA(bool will_initialization_succeed, const VideoEncodeAccelerator::Config& config,
    VideoEncodeAccelerator::Client* client, const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds,
    const gpu::GPUInfo::GPUDevice& gpu_device, std::unique_ptr<MediaLog> media_log,
    MojoVideoEncodeAcceleratorService::GetCommandBufferHelperCB get_command_buffer_helper_cb, scoped_refptr<base::SingleThreadTaskRunner> gpu_task_runner)
{
...
class MockMojoVideoEncodeAcceleratorClient : public mojom::VideoEncodeAcceleratorClient {
public:
    MockMojoVideoEncodeAcceleratorClient() = default;

    MockMojoVideoEncodeAcceleratorClient(const MockMojoVideoEncodeAcceleratorClient&) = delete;
    MockMojoVideoEncodeAcceleratorClient& operator=(const MockMojoVideoEncodeAcceleratorClient&) = delete;

    MOCK_METHOD3(RequireBitstreamBuffers, void(uint32_t, const gfx::Size&, uint32_t));
...
public:
    ...
    MojoVideoEncodeAcceleratorServiceTest(const MojoVideoEncodeAcceleratorServiceTest&) = delete;
    MojoVideoEncodeAcceleratorServiceTest& operator=(const MojoVideoEncodeAcceleratorServiceTest&) = delete;
    ...
    void CreateMojoVideoEncodeAccelerator(bool will_fake_vea_initialization_succeed = true)
    {
        mojo_vea_service_ = std::make_unique<MojoVideoEncodeAcceleratorService>(
            base::BindRepeating(&CreateAndInitializeFakeVEA, will_fake_vea_initialization_succeed), gpu::GpuPreferences(), gpu::GpuDriverBugWorkarounds(),
            gpu::GPUInfo::GPUDevice(), MojoVideoEncodeAcceleratorService::GetCommandBufferHelperCB(), base::SingleThreadTaskRunner::GetCurrentDefault());
    }

    void BindAndInitialize()
    {
        // Create an Mojo VEA Client remote and bind it to our Mock.
        mojo::PendingAssociatedRemote<mojom::VideoEncodeAcceleratorClient> pending_client_remote;
        auto pending_client_receiver = pending_client_remote.InitWithNewEndpointAndPassReceiver();
        pending_client_receiver.EnableUnassociatedUsage();

        mojo_vea_receiver_
            = mojo::MakeSelfOwnedAssociatedReceiver(std::make_unique<MockMojoVideoEncodeAcceleratorClient>(), std::move(pending_client_receiver));

        EXPECT_CALL(*mock_mojo_vea_client(), RequireBitstreamBuffers(_, kInputVisibleSize, _));

        constexpr media::Bitrate kInitialBitrate = media::Bitrate::ConstantBitrate(100000u);
        const media::VideoEncodeAccelerator::Config config(PIXEL_FORMAT_I420, kInputVisibleSize, H264PROFILE_MIN, kInitialBitrate,
            media::VideoEncodeAccelerator::kDefaultFramerate, VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer,
        ...
        mojo_vea_service()->Initialize(
            config, std::move(pending_client_remote), std::move(media_log_pending_remote), base::BindOnce([](bool success) { ASSERT_TRUE(success); }));
    ...
    {
        return static_cast<media::MockMojoVideoEncodeAcceleratorClient*>(mojo_vea_receiver_->impl());
    }

    FakeVideoEncodeAccelerator* fake_vea() const
    {
        return static_cast<FakeVideoEncodeAccelerator*>(mojo_vea_service_->encoder_.get());
    }

private:
    base::test::SingleThreadTaskEnvironment task_environment_;

    mojo::SelfOwnedAssociatedReceiverRef<mojom::VideoEncodeAcceleratorClient> mojo_vea_receiver_;
...
// isolation.
TEST_F(MojoVideoEncodeAcceleratorServiceTest, InitializeAndRequireBistreamBuffers)
{
...
TEST_F(MojoVideoEncodeAcceleratorServiceTest, EncodeOneFrame)
{
    ...
    const int32_t kBitstreamBufferId = 17;
    {
        ...
        mojo_vea_service()->UseOutputBitstreamBuffer(kBitstreamBufferId, std::move(region));
        base::RunLoop().RunUntilIdle();
    }

    {
        const auto video_frame = VideoFrame::CreateBlackFrame(kInputVisibleSize);
        EXPECT_CALL(*mock_mojo_vea_client(), BitstreamBufferReady(kBitstreamBufferId, _));
...
TEST_F(MojoVideoEncodeAcceleratorServiceTest, EncodingParametersChange)
{
    ...
    mojo_vea_service()->RequestEncodingParametersChangeWithLayers(bitrate_allocation, kNewFramerate, std::nullopt);
    base::RunLoop().RunUntilIdle();

    ASSERT_TRUE(fake_vea());
    VideoBitrateAllocation expected_allocation;
    expected_allocation.SetBitrate(0, 0, kNewBitrate);
    EXPECT_EQ(expected_allocation, fake_vea()->stored_bitrate_allocations().back());
    EXPECT_TRUE(fake_vea()->stored_frame_sizes().empty());
}

// Tests that a RequestEncodingParametersChange() ripples through correctly.
TEST_F(MojoVideoEncodeAcceleratorServiceTest, EncodingParametersWithBitrateAllocation)
{
    CreateMojoVideoEncodeAccelerator();
    BindAndInitialize();
...
// Tests that a RequestEncodingParametersChange() ripples through correctly.
TEST_F(MojoVideoEncodeAcceleratorServiceTest, EncodingParametersWithBitrateAllocationAndFrameSize)
{
    CreateMojoVideoEncodeAccelerator();
    BindAndInitialize();
    ...
    for (size_t i = 0; i <= kMaxNumBitrates; ++i) {
        ...
        EXPECT_CALL(*mock_mojo_vea_client(), RequireBitstreamBuffers(_, frame_size, _));
        mojo_vea_service()->RequestEncodingParametersChangeWithLayers(bitrate_allocation, kNewFramerate, frame_size);
        base::RunLoop().RunUntilIdle();
...
TEST_F(MojoVideoEncodeAcceleratorServiceTest, InitializeWithInvalidClientFails)
{
    CreateMojoVideoEncodeAccelerator();

    mojo::PendingAssociatedRemote<mojom::VideoEncodeAcceleratorClient> invalid_mojo_vea_client;
    ...
    mojo_vea_service()->Initialize(
        config, std::move(invalid_mojo_vea_client), std::move(media_log_pending_remote), base::BindOnce([](bool success) { ASSERT_FALSE(success); }));
    base::RunLoop().RunUntilIdle();
}
...
{
    CreateMojoVideoEncodeAccelerator(false /* will_fake_vea_initialization_succeed */);

    mojo::PendingAssociatedRemote<mojom::VideoEncodeAcceleratorClient> mojo_vea_client;
    auto mojo_vea_receiver
        = mojo::MakeSelfOwnedAssociatedReceiver(std::make_unique<MockMojoVideoEncodeAcceleratorClient>(), mojo_vea_client.InitWithNewEndpointAndPassReceiver());
    ...
    mojo_vea_service()->Initialize(
        config, std::move(mojo_vea_client), std::move(media_log_pending_remote), base::BindOnce([](bool success) { ASSERT_FALSE(success); }));
    base::RunLoop().RunUntilIdle();
...
// causes NotifyError().
TEST_F(MojoVideoEncodeAcceleratorServiceTest, UseOutputBitstreamBufferWithWrongSizeFails)
{
    CreateMojoVideoEncodeAccelerator();
    BindAndInitialize();
...
TEST_F(MojoVideoEncodeAcceleratorServiceTest, EncodeWithWrongSizeFails)
{
    ...
    const gfx::Size wrong_size(kInputVisibleSize.width() / 2, kInputVisibleSize.height() / 2);
...
TEST_F(MojoVideoEncodeAcceleratorServiceTest, IsFlushSupportedAndFlush)
{
    ...
    // Flush.
    auto flush_support = base::BindOnce([](bool status) { EXPECT_EQ(status, false); });
    mojo_vea_service()->IsFlushSupported(std::move(flush_support));
    base::RunLoop().RunUntilIdle();

    auto flush_callback = base::BindOnce([](bool status) { EXPECT_EQ(status, false); });
