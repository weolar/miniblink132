...
MojoCdmFileIO::MojoCdmFileIO(Delegate* delegate, cdm::FileIOClient* client, mojo::Remote<mojom::CdmStorage> cdm_storage)
    : delegate_(delegate)
    , client_(client)
...
void MojoCdmFileIO::Open(const char* file_name, uint32_t file_name_size)
{
    ...
    TRACE_EVENT_ASYNC_BEGIN1("media", "MojoCdmFileIO::Open", this, "file_name", file_name_);
    ...
    auto callback = mojo::WrapCallbackWithDefaultInvokeIfNotRun(
        base::BindOnce(&MojoCdmFileIO::OnFileOpened, weak_factory_.GetWeakPtr()), StorageStatus::kFailure, mojo::NullAssociatedRemote());
    cdm_storage_->Open(file_name_string, std::move(callback));
}

void MojoCdmFileIO::OnFileOpened(StorageStatus status, mojo::PendingAssociatedRemote<mojom::CdmFile> cdm_file)
{
    ...
    TRACE_EVENT_ASYNC_END1("media", "MojoCdmFileIO::Open", this, "status", ConvertStorageStatus(status));
    switch (status) {
    case StorageStatus::kSuccess:
...
void MojoCdmFileIO::Read()
{
    ...
    TRACE_EVENT_ASYNC_BEGIN1("media", "MojoCdmFileIO::Read", this, "file_name", file_name_);
    ...
    // Read() failed.
    auto callback = mojo::WrapCallbackWithDefaultInvokeIfNotRun(
        base::BindOnce(&MojoCdmFileIO::OnFileRead, weak_factory_.GetWeakPtr()), FileStatus::kFailure, std::vector<uint8_t>());
    cdm_file_->Read(std::move(callback));
}

void MojoCdmFileIO::OnFileRead(FileStatus status, const std::vector<uint8_t>& data)
{
    DVLOG(3) << __func__ << " file: " << file_name_;
    ...
    TRACE_EVENT_ASYNC_END2("media", "MojoCdmFileIO::Read", this, "bytes_read", data.size(), "status", ConvertFileStatus(status));
...
void MojoCdmFileIO::Write(const uint8_t* data, uint32_t data_size)
{
    ...
    if (data_size > kMaxFileSizeBytes) {
        DLOG(WARNING) << __func__ << " Too much data to write. #bytes = " << data_size;
        OnError(ErrorType::kWriteError);
        return;
    }

    UMA_HISTOGRAM_CUSTOM_COUNTS("Media.EME.CdmFileIO.WriteFile.DataSizeKB", data_size / 1024, kSizeKBMin, kMaxFileSizeKB, kSizeKBBuckets);

    TRACE_EVENT_ASYNC_BEGIN2("media", "MojoCdmFileIO::Write", this, "file_name", file_name_, "bytes_to_write", data_size);
    ...
    // Write() failed.
    auto callback
        = mojo::WrapCallbackWithDefaultInvokeIfNotRun(base::BindOnce(&MojoCdmFileIO::OnFileWritten, weak_factory_.GetWeakPtr()), FileStatus::kFailure);
    cdm_file_->Write(std::vector<uint8_t>(data, data + data_size), std::move(callback));
}
...
void MojoCdmFileIO::OnFileWritten(FileStatus status)
{
    ...
    TRACE_EVENT_ASYNC_END1("media", "MojoCdmFileIO::Write", this, "status", ConvertFileStatus(status));
...
void MojoCdmFileIO::OnError(ErrorType error)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&MojoCdmFileIO::NotifyClientOfError, weak_factory_.GetWeakPtr(), error));
}
