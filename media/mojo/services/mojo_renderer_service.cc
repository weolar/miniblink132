...
mojo::SelfOwnedReceiverRef<mojom::Renderer> MojoRendererService::Create(
    MojoCdmServiceContext* mojo_cdm_service_context, std::unique_ptr<media::Renderer> renderer, mojo::PendingReceiver<mojom::Renderer> receiver)
{
    MojoRendererService* service = new MojoRendererService(mojo_cdm_service_context, std::move(renderer));

    mojo::SelfOwnedReceiverRef<mojom::Renderer> self_owned_receiver
        = mojo::MakeSelfOwnedReceiver<mojom::Renderer>(base::WrapUnique(service), std::move(receiver));

    return self_owned_receiver;
}

MojoRendererService::MojoRendererService(MojoCdmServiceContext* mojo_cdm_service_context, std::unique_ptr<media::Renderer> renderer)
...
void MojoRendererService::Initialize(mojo::PendingAssociatedRemote<mojom::RendererClient> client,
    std::optional<std::vector<mojo::PendingRemote<mojom::DemuxerStream>>> streams, mojom::MediaUrlParamsPtr media_url_params, InitializeCallback callback)
{
    DVLOG(1) << __func__;
    ...
    if (!media_url_params) {
        ...
        media_resource_ = std::make_unique<MediaResourceShim>(
            std::move(*streams), base::BindOnce(&MojoRendererService::OnAllStreamsReady, weak_this_, std::move(callback)));
        return;
    }

    DCHECK(!media_url_params->media_url.is_empty());
    media_resource_ = std::make_unique<MediaUrlDemuxer>(nullptr, media_url_params->media_url, media_url_params->site_for_cookies,
        media_url_params->top_frame_origin, media_url_params->storage_access_api_status, media_url_params->allow_credentials, media_url_params->is_hls);
    media_resource_->SetHeaders(std::move(media_url_params->headers));
    renderer_->Initialize(media_resource_.get(), this, base::BindOnce(&MojoRendererService::OnRendererInitializeDone, weak_this_, std::move(callback)));
}
...
void MojoRendererService::Flush(FlushCallback callback)
{
    ...
    renderer_->Flush(base::BindOnce(&MojoRendererService::OnFlushCompleted, weak_this_, std::move(callback)));
}
...
void MojoRendererService::SetCdm(const std::optional<base::UnguessableToken>& cdm_id, SetCdmCallback callback)
{
    if (cdm_context_ref_) {
    ...
    auto cdm_context_ref = mojo_cdm_service_context_->GetCdmContextRef(cdm_id.value());
    if (!cdm_context_ref) {
    ...
    renderer_->SetCdm(cdm_context, base::BindOnce(&MojoRendererService::OnCdmAttached, weak_this_, std::move(callback)));
}

void MojoRendererService::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
...
void MojoRendererService::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
...
void MojoRendererService::OnAudioConfigChange(const AudioDecoderConfig& config)
{
    DVLOG(2) << __func__ << "(" << config.AsHumanReadableString() << ")";
    client_->OnAudioConfigChange(config);
}

void MojoRendererService::OnVideoConfigChange(const VideoDecoderConfig& config)
{
    DVLOG(2) << __func__ << "(" << config.AsHumanReadableString() << ")";
...
void MojoRendererService::OnAllStreamsReady(base::OnceCallback<void(bool)> callback)
{
...
void MojoRendererService::OnRendererInitializeDone(base::OnceCallback<void(bool)> callback, PipelineStatus status)
{
    DVLOG(1) << __func__;
    DCHECK_EQ(state_, STATE_INITIALIZING);
...
void MojoRendererService::OnCdmAttached(base::OnceCallback<void(bool)> callback, bool success)
{
