...
public:
    ...
    {
        cdm_factory_->Create(
            cdm_config, session_message_cb, session_closed_cb, session_keys_change_cb, session_expiration_update_cb, std::move(cdm_created_cb));
    }
...
public:
    ...
    std::unique_ptr<CdmFactory> CreateCdmFactory(mojom::FrameInterfaceFactory* frame_interfaces) override
    {
...
public:
    ...
    void Initialize()
    {
        EXPECT_CALL(*mock_cdm_, GetCdmContext()).WillRepeatedly(Return(&cdm_context_));

        auto mock_cdm_service_client = std::make_unique<MockCdmServiceClient>(&mock_cdm_factory_);
        mock_cdm_service_client_ = mock_cdm_service_client.get();

        service_ = std::make_unique<CdmService>(std::move(mock_cdm_service_client), cdm_service_remote_.BindNewPipeAndPassReceiver());
        cdm_service_remote_.set_idle_handler(base::TimeDelta(), base::BindRepeating(&CdmServiceTest::CdmServiceIdle, base::Unretained(this)));
        ...
        cdm_service_remote_->CreateCdmFactory(cdm_factory_remote_.BindNewPipeAndPassReceiver(), std::move(interfaces));
        cdm_service_remote_.FlushForTesting();
        ASSERT_TRUE(cdm_factory_remote_);
        cdm_factory_remote_.set_disconnect_handler(base::BindOnce(&CdmServiceTest::CdmFactoryConnectionClosed, base::Unretained(this)));
    }

    void InitializeCdm(const std::string& key_system, bool expected_result)
    {
        cdm_factory_remote_->CreateCdm(
            { key_system, false, false, false }, base::BindOnce(&CdmServiceTest::OnCdmCreated, base::Unretained(this), expected_result));
...
private:
    void OnCdmCreated(
        bool expected_result, mojo::PendingRemote<mojom::ContentDecryptionModule> remote, mojom::CdmContextPtr cdm_context, CreateCdmStatus status)
    {
        if (!expected_result) {
            EXPECT_FALSE(remote);
        ...
        cdm_remote_.set_disconnect_handler(base::BindOnce(&CdmServiceTest::CdmConnectionClosed, base::Unretained(this)));
    }
    std::unique_ptr<CdmService> service_;
    raw_ptr<MockCdmServiceClient, AcrossTasksDanglingUntriaged> mock_cdm_service_client_ = nullptr;
};
...
TEST_F(CdmServiceTest, DestroyCdmService_AfterCdmCreation)
{
    ...
    EXPECT_CALL(*this, CdmConnectionClosed()).WillOnce(Invoke(&run_loop, &base::RunLoop::Quit));
    DestroyService();
    run_loop.Run();
}
...
{
    base::RunLoop run_loop;
    EXPECT_CALL(*this, CdmFactoryConnectionClosed()).WillOnce(Invoke(&run_loop, &base::RunLoop::Quit));
    mock_cdm_factory_.SetBeforeCreationCB(base::BindRepeating(&CdmServiceTest::DestroyService, base::Unretained(this)));
    Initialize();
    InitializeCdm(kClearKeyKeySystem, true);
    run_loop.Run();
}
