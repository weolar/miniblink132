...
public:
    ...
    base::UnguessableToken RegisterRemoteCdmContext(chromeos::RemoteCdmContext* remote_context);
    ...
    std::unique_ptr<CdmContextRef> GetCdmContextRef(const base::UnguessableToken& cdm_id);
...
private:
    ...
    std::map<base::UnguessableToken, raw_ptr<MojoCdmService, CtnExperimental>> cdm_services_ GUARDED_BY(cdm_services_lock_);

#if BUILDFLAG(IS_CHROMEOS_ASH)
    // A map between CDM ID and RemoteCdmContext.
    std::map<base::UnguessableToken, raw_ptr<chromeos::RemoteCdmContext, CtnExperimental>> remote_cdm_contexts_;
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
};
