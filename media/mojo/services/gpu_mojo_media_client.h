...
using GetConfigCacheCB = base::RepeatingCallback<SupportedVideoDecoderConfigs()>;
using GetCommandBufferStubCB = base::RepeatingCallback<gpu::CommandBufferStub*()>;
...
struct VideoDecoderTraits {
    ...
    VideoDecoderTraits(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log, RequestOverlayInfoCB request_overlay_info_cb,
        const gfx::ColorSpace* target_color_space, GetConfigCacheCB get_cached_configs_cb, GetCommandBufferStubCB get_command_buffer_stub_cb,
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder);
    ~VideoDecoderTraits();
};
...
struct MEDIA_MOJO_EXPORT GpuMojoMediaClientTraits {
    ...
    GpuMojoMediaClientTraits(const gpu::GpuPreferences& gpu_preferences, const gpu::GpuDriverBugWorkarounds& gpu_workarounds,
...
public:
    ...
    static std::unique_ptr<GpuMojoMediaClient> Create(GpuMojoMediaClientTraits& traits);
    ...
    std::unique_ptr<AudioDecoder> CreateAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log) final;
    std::unique_ptr<AudioEncoder> CreateAudioEncoder(scoped_refptr<base::SequencedTaskRunner> task_runner) final;
#if BUILDFLAG(ALLOW_OOP_VIDEO_DECODER)
    void NotifyDecoderSupportKnown(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
        base::OnceCallback<void(mojo::PendingRemote<stable::mojom::StableVideoDecoder>)> cb) final;
    ...
    std::unique_ptr<VideoDecoder> CreateVideoDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog* media_log,
        ...
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder) final;
    std::unique_ptr<CdmFactory> CreateCdmFactory(mojom::FrameInterfaceFactory* interface_provider) final;
...
protected:
    ...
    virtual std::unique_ptr<VideoDecoder> CreatePlatformVideoDecoder(VideoDecoderTraits& traits) = 0;

    // Queries the platform-specific VideoDecoder implementation for its
    // supported profiles.
    virtual std::optional<SupportedVideoDecoderConfigs> GetPlatformSupportedVideoDecoderConfigs() = 0;
    ...
    virtual void NotifyPlatformDecoderSupport(mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder,
    ...
    virtual std::unique_ptr<AudioDecoder> CreatePlatformAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log);

    // Creates a platform-specific media::AudioEncoder. Most platforms don't do
    // anything here.
    virtual std::unique_ptr<AudioEncoder> CreatePlatformAudioEncoder(scoped_refptr<base::SequencedTaskRunner> task_runner);

    // Creates a CDM factory, right now only used on android and chromeos.
    virtual std::unique_ptr<CdmFactory> CreatePlatformCdmFactory(mojom::FrameInterfaceFactory* frame_interfaces);

    // Queries the platform-specific AudioDecoder implementation for its
    // supported codecs.
    virtual std::optional<SupportedAudioDecoderConfigs> GetPlatformSupportedAudioDecoderConfigs();
