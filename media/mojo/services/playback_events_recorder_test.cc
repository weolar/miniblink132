...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::MainThreadType::IO, base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
        ...
        action_callback_ = base::BindRepeating(&PlaybackEventsRecorderTest::OnAction, base::Unretained(this));
...
TEST_F(PlaybackEventsRecorderTest, Buffering)
{
    ...
    ExpectEvents({
        { time_base_, "WebEngine.Media.Playing" },
        { time_base_ + 5 * kSecond, "WebEngine.Media.PlayTimeBeforeAutoPause:2000" },
...
TEST_F(PlaybackEventsRecorderTest, BitrateAfterBuffering)
{
    ...
    ExpectEvents({
        { time_base_, "WebEngine.Media.Playing" },
        { time_base_ + 13 * kSecond, "WebEngine.Media.PlayTimeBeforeAutoPause:3000" },
