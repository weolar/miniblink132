...
template <typename Interface> class DeferredDestroy : public Interface {
public:
...
template <typename Interface> class DeferredDestroyUniqueReceiverSet {
public:
    ...
    class Deleter {
    public:
        using DeleteCallback = base::RepeatingCallback<void(std::unique_ptr<Interface>)>;
    ...
    DeferredDestroyUniqueReceiverSet(const DeferredDestroyUniqueReceiverSet&) = delete;
    DeferredDestroyUniqueReceiverSet& operator=(const DeferredDestroyUniqueReceiverSet&) = delete;

    void Add(std::unique_ptr<DeferredDestroy<Interface>> impl, mojo::PendingReceiver<Interface> receiver)
    {
        // Wrap the pointer into a unique_ptr with a deleter.
        Deleter deleter(base::BindRepeating(&DeferredDestroyUniqueReceiverSet::OnReceiverRemoved, weak_factory_.GetWeakPtr()));
        std::unique_ptr<Interface, Deleter> impl_with_deleter(impl.release(), deleter);
    ...
    size_t unbound_size() const
    {
        return unbound_impls_.size();
    }
...
private:
    void OnReceiverRemoved(std::unique_ptr<Interface> ptr)
    {
        ...
        impl_ptr->OnDestroyPending(
            base::BindPostTaskToCurrentDefault(base::BindOnce(&DeferredDestroyUniqueReceiverSet::OnDestroyable, weak_factory_.GetWeakPtr(), id_)));
    }
