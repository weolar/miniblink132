...
MediaFoundationServiceBroker::MediaFoundationServiceBroker(
    mojo::PendingReceiver<mojom::MediaFoundationServiceBroker> receiver, base::OnceClosure ensure_sandboxed_cb)
...
void MediaFoundationServiceBroker::UpdateGpuInfo(const gpu::GPUInfo& gpu_info, UpdateGpuInfoCallback callback)
{
    OnGpuInfoUpdate(gpu_info);
    std::move(callback).Run(gpu_info_observer_.BindNewPipeAndPassRemote());
}

void MediaFoundationServiceBroker::GetService(const base::FilePath& cdm_path, mojo::PendingReceiver<mojom::MediaFoundationService> service_receiver)
{
    ...
    media_foundation_service_ = std::make_unique<MediaFoundationService>(std::move(service_receiver));
    ...
    if (!success) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&MediaFoundationServiceBroker::TerminateService, weak_factory_.GetWeakPtr()), kServiceTerminationDelay);
    }
}

void MediaFoundationServiceBroker::OnGpuInfoUpdate(const gpu::GPUInfo& gpu_info)
{
