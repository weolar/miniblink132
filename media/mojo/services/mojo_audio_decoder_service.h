...
class MEDIA_MOJO_EXPORT MojoAudioDecoderService final : public mojom::AudioDecoder {
public:
    ...
    void Construct(mojo::PendingAssociatedRemote<mojom::AudioDecoderClient> client, mojo::PendingRemote<mojom::MediaLog> media_log) final;
    void Initialize(const AudioDecoderConfig& config, const std::optional<base::UnguessableToken>& cdm_id, InitializeCallback callback) final;
...
private:
    ...
    void OnReadDone(mojo::ReportBadMessageCallback bad_message_callback, DecodeCallback callback, scoped_refptr<DecoderBuffer> buffer);
