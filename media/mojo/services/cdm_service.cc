...
public:
    CdmFactoryImpl(CdmService::Client* client, mojo::PendingRemote<mojom::FrameInterfaceFactory> interfaces)
        : client_(client)
        , interfaces_(std::move(interfaces))
    {
        ...
        cdm_receivers_.set_disconnect_handler(base::BindRepeating(&CdmFactoryImpl::OnReceiverDisconnect, base::Unretained(this)));
    }
    ...
    void CreateCdm(const CdmConfig& cdm_config, CreateCdmCallback callback) final
    {
        ...
        raw_mojo_cdm_service->Initialize(cdm_factory, cdm_config,
            base::BindOnce(&CdmFactoryImpl::OnCdmServiceInitialized, weak_ptr_factory_.GetWeakPtr(), raw_mojo_cdm_service, std::move(callback)));
    }
...
CdmService::CdmService(std::unique_ptr<Client> client, mojo::PendingReceiver<mojom::CdmService> receiver)
...
void CdmService::CreateCdmFactory(mojo::PendingReceiver<mojom::CdmFactory> receiver, mojo::PendingRemote<mojom::FrameInterfaceFactory> frame_interfaces)
{
    // Ignore receiver if service has already stopped.
    if (!client_)
        return;

    cdm_factory_receivers_.Add(std::make_unique<CdmFactoryImpl>(client_.get(), std::move(frame_interfaces)), std::move(receiver));
}
