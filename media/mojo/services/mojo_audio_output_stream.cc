...
MojoAudioOutputStream::MojoAudioOutputStream(
    CreateDelegateCallback create_delegate_callback, StreamCreatedCallback stream_created_callback, DeleterCallback deleter_callback)
    ...
    , deleter_callback_(std::move(deleter_callback))
{
    ...
    if (!delegate_) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, base::BindOnce(&MojoAudioOutputStream::OnStreamError, weak_factory_.GetWeakPtr(), /* not used */ 0));
    }
}
...
void MojoAudioOutputStream::SetVolume(double volume)
{
    ...
    if (volume < 0 || volume > 1) {
        LOG(ERROR) << "MojoAudioOutputStream::SetVolume(" << volume << ") out of range.";
...
void MojoAudioOutputStream::OnStreamCreated(
    int stream_id, base::UnsafeSharedMemoryRegion shared_memory_region, std::unique_ptr<base::CancelableSyncSocket> foreign_socket)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(stream_created_callback_);
    ...
    receiver_.set_disconnect_handler(base::BindOnce(&MojoAudioOutputStream::StreamConnectionLost, base::Unretained(this)));

    std::move(stream_created_callback_).Run(std::move(pending_stream), { absl::in_place, std::move(shared_memory_region), std::move(socket_handle) });
}
