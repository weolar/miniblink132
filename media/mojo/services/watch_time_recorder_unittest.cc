...
public:
    WatchTimeRecorderTest()
        : computation_keys_({ WatchTimeKey::kAudioSrc, WatchTimeKey::kAudioMse, WatchTimeKey::kAudioEme, WatchTimeKey::kAudioVideoSrc,
            WatchTimeKey::kAudioVideoMse, WatchTimeKey::kAudioVideoEme })
        , mtbr_keys_({ kMeanTimeBetweenRebuffersAudioSrc, kMeanTimeBetweenRebuffersAudioMse, kMeanTimeBetweenRebuffersAudioEme,
              kMeanTimeBetweenRebuffersAudioVideoSrc, kMeanTimeBetweenRebuffersAudioVideoMse, kMeanTimeBetweenRebuffersAudioVideoEme })
        , smooth_keys_({ kRebuffersCountAudioSrc, kRebuffersCountAudioMse, kRebuffersCountAudioEme, kRebuffersCountAudioVideoSrc, kRebuffersCountAudioVideoMse,
              kRebuffersCountAudioVideoEme })
        , discard_keys_({ kDiscardedWatchTimeAudioSrc, kDiscardedWatchTimeAudioMse, kDiscardedWatchTimeAudioEme, kDiscardedWatchTimeAudioVideoSrc,
              kDiscardedWatchTimeAudioVideoMse, kDiscardedWatchTimeAudioVideoEme })
    {
        source_id_ = test_recorder_->GetNewSourceID();
        ResetMetricRecorders();
        MediaMetricsProvider::Create(MediaMetricsProvider::BrowsingMode::kIncognito, MediaMetricsProvider::FrameStatus::kTopFrame, GetSourceId(),
            learning::FeatureValue(0), VideoDecodePerfHistory::SaveCallback(), MediaMetricsProvider::GetLearningSessionCallback(),
            base::BindRepeating(&WatchTimeRecorderTest::IsShuttingDown, base::Unretained(this)), provider_.BindNewPipeAndPassReceiver());
    }
    ...
    ~WatchTimeRecorderTest() override
    {
    ...
    void Initialize(mojom::PlaybackPropertiesPtr properties)
    {
        provider_->Initialize(
            properties->is_mse, properties->is_mse ? mojom::MediaURLScheme::kUnknown : mojom::MediaURLScheme::kHttp, properties->media_stream_type);
        provider_->AcquireWatchTimeRecorder(std::move(properties), wtr_.BindNewPipeAndPassReceiver());
    }

    void Initialize(bool has_audio, bool has_video, bool is_mse, bool is_encrypted, mojom::MediaStreamType media_stream_type = mojom::MediaStreamType::kNone,
        RendererType renderer_type = RendererType::kRendererImpl)
    {
        Initialize(mojom::PlaybackProperties::New(has_audio, has_video, false, false, is_mse, is_encrypted, false, media_stream_type, renderer_type));
    }

    void ExpectWatchTime(const std::vector<std::string_view>& keys, base::TimeDelta value)
    {
        for (int i = 0; i <= static_cast<int>(WatchTimeKey::kWatchTimeKeyMax); ++i) {
            const std::string_view test_key = ConvertWatchTimeKeyToStringForUma(static_cast<WatchTimeKey>(i));
            if (test_key.empty())
                continue;
            if (base::Contains(keys, test_key)) {
                histogram_tester_->ExpectUniqueSample(test_key, value.InMilliseconds(), 1);
            } else {
    ...
    void ExpectHelper(const std::vector<std::string_view>& full_key_list, const std::vector<std::string_view>& keys, int64_t value)
    {
    ...
    void ExpectMtbrTime(const std::vector<std::string_view>& keys, base::TimeDelta value)
    {
    ...
    {
        constexpr char kAudioCodecProfileHistogram[] = "Media.AudioCodecProfile.AAC";
        histogram_tester_->ExpectUniqueSample(kAudioCodecProfileHistogram, static_cast<int64_t>(profile), 1);
    }
    ...
    void ExpectUkmWatchTime(const std::vector<std::string_view>& keys, base::TimeDelta value)
    {
        ...
        for (const ukm::mojom::UkmEntry* entry : entries) {
            ...
            for (auto key : keys) {
                test_recorder_->ExpectEntryMetric(entry, key.data(), value.InMilliseconds());
            }
        }
    }
    ...
    {
        return mojom::SecondaryPlaybackProperties::New(AudioCodec::kAAC, VideoCodec::kH264, AudioCodecProfile::kUnknown, H264PROFILE_MAIN,
            AudioDecoderType::kUnknown, VideoDecoderType::kUnknown, EncryptionScheme::kUnencrypted, EncryptionScheme::kUnencrypted, gfx::Size(800, 600));
    }
...
TEST_F(WatchTimeRecorderTest, TestBasicReporting)
{
    ...
    for (int i = 0; i <= static_cast<int>(WatchTimeKey::kWatchTimeKeyMax); ++i) {
        ...
        SCOPED_TRACE(key_str.empty() ? base::NumberToString(i) : std::string(key_str));
        ...
        case WatchTimeKey::kVideoDisplayFullscreen:
            ExpectUkmWatchTime({ UkmEntry::kWatchTime_DisplayFullscreenName }, kWatchTime2);
            break;
        ...
        case WatchTimeKey::kVideoDisplayInline:
            ExpectUkmWatchTime({ UkmEntry::kWatchTime_DisplayInlineName }, kWatchTime2);
            break;
        ...
        case WatchTimeKey::kVideoDisplayPictureInPicture:
            ExpectUkmWatchTime({ UkmEntry::kWatchTime_DisplayPictureInPictureName }, kWatchTime2);
            break;
        ...
        case WatchTimeKey::kVideoNativeControlsOn:
            ExpectUkmWatchTime({ UkmEntry::kWatchTime_NativeControlsOnName }, kWatchTime2);
            break;
        ...
        case WatchTimeKey::kVideoNativeControlsOff:
            ExpectUkmWatchTime({ UkmEntry::kWatchTime_NativeControlsOffName }, kWatchTime2);
            break;
        }
...
TEST_F(WatchTimeRecorderTest, TestBasicReportingMediaStream)
{
    ...
    for (int i = 0; i <= static_cast<int>(WatchTimeKey::kWatchTimeKeyMax); ++i) {
        ...
        // Values for |is_background| and |is_muted| don't matter in this test since
        // they don't prevent the muted or background keys from being recorded.
        Initialize(true, false, true, true, mojom::MediaStreamType::kLocalDeviceCapture);
        wtr_->UpdateSecondaryProperties(CreateSecondaryProperties());
...
#define EXPECT_HAS_UKM(name) EXPECT_TRUE(test_recorder_->EntryHasMetric(entry, name));

TEST_F(WatchTimeRecorderTest, TestFinalizeNoDuplication)
{
    mojom::PlaybackPropertiesPtr properties
        = mojom::PlaybackProperties::New(true, true, false, false, false, false, false, mojom::MediaStreamType::kNone, RendererType::kRendererImpl);
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties = CreateSecondaryProperties();
    ...
    const auto& empty_entries = test_recorder_->GetEntriesByName(UkmEntry::kEntryName);
    ...
    for (const ukm::mojom::UkmEntry* entry : entries) {
        ...
        EXPECT_UKM(UkmEntry::kAudioCodecName, static_cast<int>(secondary_properties->audio_codec));
        EXPECT_UKM(UkmEntry::kVideoCodecName, static_cast<int>(secondary_properties->video_codec));
        EXPECT_UKM(UkmEntry::kAudioCodecProfileName, static_cast<int64_t>(secondary_properties->audio_codec_profile));
        EXPECT_UKM(UkmEntry::kVideoCodecProfileName, secondary_properties->video_codec_profile);
        EXPECT_UKM(UkmEntry::kHasAudioName, properties->has_audio);
        EXPECT_UKM(UkmEntry::kHasVideoName, properties->has_video);
        EXPECT_UKM(UkmEntry::kAudioEncryptionSchemeName, static_cast<int64_t>(secondary_properties->audio_encryption_scheme));
        EXPECT_UKM(UkmEntry::kVideoEncryptionSchemeName, static_cast<int64_t>(secondary_properties->video_encryption_scheme));
        EXPECT_UKM(UkmEntry::kIsEMEName, properties->is_eme);
        EXPECT_UKM(UkmEntry::kIsMSEName, properties->is_mse);
        EXPECT_UKM(UkmEntry::kMediaStreamTypeName, static_cast<int64_t>(properties->media_stream_type));
        ...
        EXPECT_UKM(UkmEntry::kVideoNaturalWidthName, secondary_properties->natural_size.width());
        EXPECT_UKM(UkmEntry::kVideoNaturalHeightName, secondary_properties->natural_size.height());
...
TEST_F(WatchTimeRecorderTest, BasicUkmAudioVideo)
{
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kAAC, VideoCodec::kH264, AudioCodecProfile::kXHE_AAC, H264PROFILE_MAIN,
            AudioDecoderType::kUnknown, VideoDecoderType::kUnknown, EncryptionScheme::kCenc, EncryptionScheme::kCbcs, gfx::Size(800, 600));
    Initialize(properties.Clone());
    wtr_->UpdateSecondaryProperties(secondary_properties.Clone());
    ...
    ExpectAacAudioCodecProfileHistogram(secondary_properties->audio_codec_profile);
...
{
    mojom::PlaybackPropertiesPtr properties
        = mojom::PlaybackProperties::New(true, true, false, false, true, true, false, mojom::MediaStreamType::kNone, RendererType::kRendererImpl);
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kOpus, VideoCodec::kVP9, AudioCodecProfile::kUnknown, VP9PROFILE_PROFILE0,
            AudioDecoderType::kUnknown, VideoDecoderType::kUnknown, EncryptionScheme::kUnencrypted, EncryptionScheme::kUnencrypted, gfx::Size(800, 600));
    Initialize(properties.Clone());
    wtr_->UpdateSecondaryProperties(secondary_properties.Clone());
    ...
    wtr_->RecordWatchTime(WatchTimeKey::kAudioVideoDisplayFullscreen, kWatchTime3);
    ...
    wtr_->RecordWatchTime(WatchTimeKey::kAudioVideoDisplayPictureInPicture, kWatchTime3);
    ...
    for (const ukm::mojom::UkmEntry* entry : entries) {
        ...
        EXPECT_UKM(UkmEntry::kWatchTime_NativeControlsOnName, kWatchTime.InMilliseconds());
        EXPECT_UKM(UkmEntry::kWatchTime_NativeControlsOffName, kWatchTime.InMilliseconds());
        EXPECT_UKM(UkmEntry::kWatchTime_DisplayFullscreenName, kWatchTime3.InMilliseconds());
        EXPECT_UKM(UkmEntry::kWatchTime_DisplayInlineName, kWatchTime3.InMilliseconds());
        EXPECT_UKM(UkmEntry::kWatchTime_DisplayPictureInPictureName, kWatchTime3.InMilliseconds());
        EXPECT_UKM(UkmEntry::kMeanTimeBetweenRebuffersName, kWatchTime2.InMilliseconds() / 3);
        EXPECT_HAS_UKM(UkmEntry::kPlayerIDName);
        ...
        EXPECT_UKM(UkmEntry::kCompletedRebuffersDurationName, kUnderflowDuration.InMilliseconds());
...
{
    mojom::PlaybackPropertiesPtr properties
        = mojom::PlaybackProperties::New(true, true, true, true, false, false, false, mojom::MediaStreamType::kNone, RendererType::kRendererImpl);
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties = CreateSecondaryProperties();
    Initialize(properties.Clone());
    wtr_->UpdateSecondaryProperties(secondary_properties.Clone());
...
TEST_F(WatchTimeRecorderTest, BasicUkmMediaStreamType)
{
    ...
    for (const auto& media_stream_type : media_stream_types) {
        mojom::PlaybackPropertiesPtr properties
            = mojom::PlaybackProperties::New(true, true, false, false, false, false, false, media_stream_type, RendererType::kRendererImpl);
        Initialize(properties.Clone());
        wtr_->UpdateSecondaryProperties(CreateSecondaryProperties());
        ...
        for (const ukm::mojom::UkmEntry* entry : entries) {
            EXPECT_UKM(UkmEntry::kMediaStreamTypeName, static_cast<int64_t>(media_stream_type));
        }
        ResetMetricRecorders();
    }
}
...
TEST_F(WatchTimeRecorderTest, SingleSecondaryPropertiesUnknownToKnown)
{
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties1
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kUnknown, VideoCodec::kUnknown, AudioCodecProfile::kUnknown, VIDEO_CODEC_PROFILE_UNKNOWN,
            AudioDecoderType::kUnknown, VideoDecoderType::kUnknown, EncryptionScheme::kUnencrypted, EncryptionScheme::kUnencrypted, gfx::Size(800, 600));
    Initialize(properties.Clone());
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties2
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kAAC, VideoCodec::kH264, AudioCodecProfile::kXHE_AAC, H264PROFILE_MAIN, AudioDecoderType::kFFmpeg,
            VideoDecoderType::kFFmpeg, EncryptionScheme::kUnencrypted, EncryptionScheme::kUnencrypted, gfx::Size(800, 600));
    ...
    for (const ukm::mojom::UkmEntry* entry : entries) {
        ...
        EXPECT_UKM(UkmEntry::kCompletedRebuffersCountName, 0);
        EXPECT_UKM(UkmEntry::kCompletedRebuffersDurationName, 0);
        EXPECT_UKM(UkmEntry::kAudioCodecName, static_cast<int>(secondary_properties2->audio_codec));
        EXPECT_UKM(UkmEntry::kVideoCodecName, static_cast<int>(secondary_properties2->video_codec));
        EXPECT_UKM(UkmEntry::kAudioCodecProfileName, static_cast<int64_t>(secondary_properties2->audio_codec_profile));
        EXPECT_UKM(UkmEntry::kVideoCodecProfileName, secondary_properties2->video_codec_profile);
        EXPECT_UKM(UkmEntry::kAudioEncryptionSchemeName, static_cast<int64_t>(secondary_properties2->audio_encryption_scheme));
        EXPECT_UKM(UkmEntry::kVideoEncryptionSchemeName, static_cast<int64_t>(secondary_properties2->video_encryption_scheme));
        EXPECT_UKM(UkmEntry::kVideoNaturalWidthName, secondary_properties2->natural_size.width());
        EXPECT_UKM(UkmEntry::kVideoNaturalHeightName, secondary_properties2->natural_size.height());
        EXPECT_NO_UKM(UkmEntry::kDurationName);
    }
}
...
TEST_F(WatchTimeRecorderTest, MultipleSecondaryPropertiesNoFinalize)
{
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties1
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kOpus, VideoCodec::kVP9, AudioCodecProfile::kUnknown, VP9PROFILE_PROFILE0,
            AudioDecoderType::kMojo, VideoDecoderType::kMojo, EncryptionScheme::kUnencrypted, EncryptionScheme::kUnencrypted, gfx::Size(400, 300));
    Initialize(properties.Clone());
    wtr_->UpdateSecondaryProperties(secondary_properties1.Clone());
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties2
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kAAC, VideoCodec::kH264, AudioCodecProfile::kUnknown, H264PROFILE_MAIN, AudioDecoderType::kFFmpeg,
            VideoDecoderType::kFFmpeg, EncryptionScheme::kCenc, EncryptionScheme::kCenc, gfx::Size(800, 600));
    wtr_->UpdateSecondaryProperties(secondary_properties2.Clone());
    ...
    wtr_->RecordWatchTime(WatchTimeKey::kAudioVideoAll, kWatchTime1 + kWatchTime2);
    ...
    wtr_->UpdateVideoDecodeStats(kDecodedFrameCount1 + kDecodedFrameCount2, kDroppedFrameCount1 + kDroppedFrameCount2);
    ...
    EXPECT_UKM(UkmEntry::kMeanTimeBetweenRebuffersName, kWatchTime1.InMilliseconds() / kUnderflowCount1);
    EXPECT_UKM(UkmEntry::kAudioDecoderNameName, 2);
    EXPECT_UKM(UkmEntry::kVideoDecoderNameName, 5);
    ...
    EXPECT_UKM(UkmEntry::kAudioCodecName, static_cast<int>(secondary_properties1->audio_codec));
    EXPECT_UKM(UkmEntry::kVideoCodecName, static_cast<int>(secondary_properties1->video_codec));
    EXPECT_UKM(UkmEntry::kAudioCodecProfileName, static_cast<int64_t>(secondary_properties1->audio_codec_profile));
    EXPECT_UKM(UkmEntry::kVideoCodecProfileName, secondary_properties1->video_codec_profile);
    EXPECT_UKM(UkmEntry::kAudioEncryptionSchemeName, static_cast<int64_t>(secondary_properties1->audio_encryption_scheme));
    EXPECT_UKM(UkmEntry::kVideoEncryptionSchemeName, static_cast<int64_t>(secondary_properties1->video_encryption_scheme));
    EXPECT_UKM(UkmEntry::kVideoNaturalWidthName, secondary_properties1->natural_size.width());
    EXPECT_UKM(UkmEntry::kVideoNaturalHeightName, secondary_properties1->natural_size.height());

    // The second record should have...
    entry = entries[1];
    EXPECT_UKM(UkmEntry::kWatchTimeName, kWatchTime2.InMilliseconds());
    EXPECT_UKM(UkmEntry::kMeanTimeBetweenRebuffersName, kWatchTime2.InMilliseconds() / kUnderflowCount2);
    EXPECT_UKM(UkmEntry::kAudioDecoderNameName, 1);
    EXPECT_UKM(UkmEntry::kVideoDecoderNameName, 2);
...
TEST_F(WatchTimeRecorderTest, MultipleSecondaryPropertiesWithFinalize)
{
    ...
    mojom::SecondaryPlaybackPropertiesPtr secondary_properties1
        = mojom::SecondaryPlaybackProperties::New(AudioCodec::kOpus, VideoCodec::kVP9, AudioCodecProfile::kUnknown, VP9PROFILE_PROFILE0,
            AudioDecoderType::kMojo, VideoDecoderType::kMojo, EncryptionScheme::kCbcs, EncryptionScheme::kCbcs, gfx::Size(400, 300));
    Initialize(properties.Clone());
    wtr_->UpdateSecondaryProperties(secondary_properties1.Clone());
...
TEST_F(WatchTimeRecorderTest, MultipleSecondaryPropertiesRebufferCarryover)
{
    ...
    EXPECT_UKM(UkmEntry::kCompletedRebuffersDurationName, (kUnderflowDuration * 1.5 - kUnderflowDuration).InMilliseconds());
    EXPECT_UKM(UkmEntry::kAudioCodecName, static_cast<int>(secondary_properties2->audio_codec));
    EXPECT_UKM(UkmEntry::kVideoCodecName, static_cast<int>(secondary_properties2->video_codec));
    EXPECT_UKM(UkmEntry::kAudioCodecProfileName, static_cast<int64_t>(secondary_properties2->audio_codec_profile));
    EXPECT_UKM(UkmEntry::kVideoCodecProfileName, secondary_properties2->video_codec_profile);
