...
MediaFoundationRendererWrapper::MediaFoundationRendererWrapper(scoped_refptr<base::SequencedTaskRunner> task_runner,
    mojom::FrameInterfaceFactory* frame_interfaces, mojo::PendingRemote<mojom::MediaLog> media_log_remote,
    mojo::PendingReceiver<RendererExtension> renderer_extension_receiver, mojo::PendingRemote<ClientExtension> client_extension_remote)
    : frame_interfaces_(frame_interfaces)
    , renderer_extension_receiver_(this, std::move(renderer_extension_receiver))
    ...
    , site_mute_observer_(this)
{
    ...
    renderer_ = std::make_unique<MediaFoundationRenderer>(std::move(task_runner), std::make_unique<MojoMediaLog>(std::move(media_log_remote), task_runner),
        ChromeLuidToLuid(MediaFoundationGpuInfoMonitor::GetInstance()->gpu_luid()));

    luid_update_subscription_ = MediaFoundationGpuInfoMonitor::GetInstance()->AddLuidObserver(
        base::BindRepeating(&MediaFoundationRendererWrapper::OnGpuLuidChange, weak_factory_.GetWeakPtr()));
}
...
void MediaFoundationRendererWrapper::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
    if (HasAudio(media_resource)) {
        frame_interfaces_->RegisterMuteStateObserver(site_mute_observer_.BindNewPipeAndPassRemote());
    }

    renderer_->SetFrameReturnCallbacks(base::BindRepeating(&MediaFoundationRendererWrapper::OnFrameGeneratedByMediaFoundation, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&MediaFoundationRendererWrapper::OnFramePoolInitialized, weak_factory_.GetWeakPtr()));

    renderer_->Initialize(media_resource, client, std::move(init_cb));
}

void MediaFoundationRendererWrapper::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    renderer_->SetCdm(cdm_context, std::move(cdm_attached_cb));
}

void MediaFoundationRendererWrapper::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
...
void MediaFoundationRendererWrapper::GetDCOMPSurface(GetDCOMPSurfaceCallback callback)
{
    ...
    renderer_->GetDCompSurface(base::BindOnce(&MediaFoundationRendererWrapper::OnReceiveDCOMPSurface, weak_factory_.GetWeakPtr(), std::move(callback)));
}
...
void MediaFoundationRendererWrapper::SetOutputRect(const gfx::Rect& output_rect, SetOutputRectCallback callback)
{
...
void MediaFoundationRendererWrapper::OnGpuLuidChange(const CHROME_LUID& adapter_luid)
{
    renderer_->SetGpuProcessAdapterLuid(ChromeLuidToLuid(adapter_luid));
}

void MediaFoundationRendererWrapper::OnReceiveDCOMPSurface(GetDCOMPSurfaceCallback callback, base::win::ScopedHandle handle, const std::string& error)
{
    if (!handle.IsValid()) {
    ...
    if (!dcomp_surface_registry_) {
        frame_interfaces_->CreateDCOMPSurfaceRegistry(dcomp_surface_registry_.BindNewPipeAndPassReceiver());
    }

    auto register_cb = base::BindOnce(&MediaFoundationRendererWrapper::OnDCOMPSurfaceHandleRegistered, weak_factory_.GetWeakPtr(), std::move(callback));

    dcomp_surface_registry_->RegisterDCOMPSurfaceHandle(
        mojo::PlatformHandle(std::move(handle)), mojo::WrapCallbackWithDefaultInvokeIfNotRun(std::move(register_cb), std::nullopt));
}

void MediaFoundationRendererWrapper::OnDCOMPSurfaceHandleRegistered(GetDCOMPSurfaceCallback callback, const std::optional<base::UnguessableToken>& token)
{
    std::string error;
    if (token) {
...
void MediaFoundationRendererWrapper::OnFramePoolInitialized(std::vector<MediaFoundationFrameInfo> frame_textures, const gfx::Size& texture_size)
{
...
void MediaFoundationRendererWrapper::OnFrameGeneratedByMediaFoundation(
    const base::UnguessableToken& frame_token, const gfx::Size& frame_size, base::TimeDelta frame_timestamp)
{
    client_extension_remote_->OnFrameAvailable(frame_token, frame_size, frame_timestamp);
}

void MediaFoundationRendererWrapper::NotifyFrameReleased(const base::UnguessableToken& frame_token)
{
...
void MediaFoundationRendererWrapper::SetMediaFoundationRenderingMode(MediaFoundationRenderingMode mode)
{
