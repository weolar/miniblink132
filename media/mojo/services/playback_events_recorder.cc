...
void RecordEventWithValueAt(const char* name, int64_t value, base::TimeTicks time)
{
    base::RecordComputedActionAt(base::StrCat({ "WebEngine.Media.", name, ":", base::NumberToString(value) }), time);
}
...
void PlaybackEventsRecorder::BitrateEstimator::Update(const PipelineStatistics& stats)
{
    ...
    if (last_stats_) {
        ...
        audio_bytes_ += stats.audio_bytes_decoded - last_stats_->audio_bytes_decoded;
        video_bytes_ += stats.video_bytes_decoded - last_stats_->video_bytes_decoded;
        if (time_elapsed_ >= kBitrateReportPeriod) {
            size_t audio_bitrate_kbps = 8 * audio_bytes_ / time_elapsed_.InMilliseconds();
            RecordEventWithValueAt("AudioBitrate", audio_bitrate_kbps, now);

            size_t video_bitrate_kbps = 8 * video_bytes_ / time_elapsed_.InMilliseconds();
...
// static
void PlaybackEventsRecorder::Create(mojo::PendingReceiver<mojom::PlaybackEventsRecorder> receiver)
{
    mojo::MakeSelfOwnedReceiver(std::make_unique<PlaybackEventsRecorder>(), std::move(receiver));
}
...
void PlaybackEventsRecorder::OnBufferingComplete()
{
    ...
    if (buffering_state_ == BufferingState::kBuffering) {
        base::TimeDelta time_between_buffering = buffering_start_time_ - last_buffering_end_time_;
        RecordEventWithValueAt("PlayTimeBeforeAutoPause", time_between_buffering.InMilliseconds(), now);

        base::TimeDelta buffering_user_time = now - buffering_start_time_;
        RecordEventWithValueAt("AutoPauseTime", buffering_user_time.InMilliseconds(), now);
    }
...
void PlaybackEventsRecorder::OnNaturalSizeChanged(const gfx::Size& size)
{
    ...
    base::RecordComputedAction(base::StringPrintf("WebEngine.Media.VideoResolution:%d", encoded_video_resolution));
}

void PlaybackEventsRecorder::OnPipelineStatistics(const PipelineStatistics& stats)
{
