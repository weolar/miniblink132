...
public:
    ...
    void MakeRecorder(ukm::SourceId source_id, learning::FeatureValue origin, bool is_top_frame, uint64_t player_id)
    {
        recorder_ = std::make_unique<VideoDecodeStatsRecorder>(
            base::BindRepeating(&VideoDecodeStatsRecorderTest::PrintSavePerfRecord, base::Unretained(this)), source_id, origin, is_top_frame, player_id);
    }

    void PrintSavePerfRecord(ukm::SourceId source_id, learning::FeatureValue origin, bool is_top_frame, mojom::PredictionFeatures features,
        mojom::PredictionTargets targets, uint64_t player_id, base::OnceClosure save_done_cb)
    {
        SavePerfRecord(source_id, origin, is_top_frame, features, targets, player_id, std::move(save_done_cb));
    }

    MOCK_METHOD7(SavePerfRecord,
        void(ukm::SourceId source_id, learning::FeatureValue origin, bool is_top_frame, mojom::PredictionFeatures features, mojom::PredictionTargets targets,
            uint64_t player_id, base::OnceClosure save_done_cb));
...
TEST_F(VideoDecodeStatsRecorderTest, SaveOnStartNewRecord)
{
    ...
    EXPECT_CALL(*this,
        SavePerfRecord(kSourceIdA, kOriginA_, kIsTopFrameA, MojoEq(MakeFeatures(kProfileA, kSizeA_, kFpsA)), MojoEq(MakeTargets(3, 2, 1)), kPlayerIdA, _));
    ...
    EXPECT_CALL(*this,
        SavePerfRecord(kSourceIdA, kOriginA_, kIsTopFrameA, MojoEq(MakeFeatures(kProfileB, kSizeB_, kFpsB)), MojoEq(MakeTargets(6, 5, 4)), kPlayerIdA, _));
    recorder_->StartNewRecord(MakeFeaturesPtr(kProfileA, kSizeA_, kFpsA));
}
...
TEST_F(VideoDecodeStatsRecorderTest, SaveOnDestruction)
{
    ...
    EXPECT_CALL(*this,
        SavePerfRecord(kSourceIdB, kOriginB_, kIsTopFrameB, MojoEq(MakeFeatures(kProfileB, kSizeB_, kFpsB)), MojoEq(MakeTargets(3, 2, 1)), kPlayerIdB, _));
    recorder_.reset();
}
