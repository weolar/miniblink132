...
using RecycleRegionCB = base::OnceCallback<void(std::unique_ptr<base::MappedReadOnlyRegion>)>;
...
public:
    MojoCdmBuffer(std::unique_ptr<base::MappedReadOnlyRegion> mapped_region, RecycleRegionCB recycle_region_cb)
        ...
        , recycle_region_cb_(std::move(recycle_region_cb))
    {
        ...
        CHECK_LE(mapped_region_->region.GetSize(), std::numeric_limits<uint32_t>::max());
    }
    ...
    {
        return mapped_region_->mapping.size();
    }
...
public:
    ...
    scoped_refptr<media::VideoFrame> TransformToVideoFrame(gfx::Size natural_size) final
    {
        DCHECK(FrameBuffer());
        ...
        std::unique_ptr<base::MappedReadOnlyRegion> mapped_region = buffer->TakeRegion();
        ...
        uint8_t* data = const_cast<uint8_t*>(mapped_region->mapping.GetMemoryAs<uint8_t>());
        if (PlaneOffset(cdm::kYPlane) != 0u) {
            LOG(ERROR) << "The first buffer offset is not 0";
            return nullptr;
        }
        auto frame = media::VideoFrame::WrapExternalYuvData(ToMediaVideoFormat(Format()), frame_size, gfx::Rect(frame_size), natural_size,
            static_cast<int32_t>(Stride(cdm::kYPlane)), static_cast<int32_t>(Stride(cdm::kUPlane)), static_cast<int32_t>(Stride(cdm::kVPlane)),
            data + PlaneOffset(cdm::kYPlane), data + PlaneOffset(cdm::kUPlane), data + PlaneOffset(cdm::kVPlane), base::Microseconds(Timestamp()));
        ...
        if (frame) {
            ...
            frame->AddDestructionObserver(base::BindOnce(std::move(recycle_region_cb_), std::move(mapped_region)));
        }
        return frame;
    }
...
cdm::Buffer* MojoCdmAllocator::CreateCdmBuffer(size_t capacity)
{
    ...
    return new MojoCdmBuffer(std::move(mapped_region), base::BindOnce(&MojoCdmAllocator::AddRegionToAvailableMap, weak_ptr_factory_.GetWeakPtr()));
}
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    return std::make_unique<MojoCdmVideoFrame>(base::BindOnce(&MojoCdmAllocator::AddRegionToAvailableMap, weak_ptr_factory_.GetWeakPtr()));
}

std::unique_ptr<base::MappedReadOnlyRegion> MojoCdmAllocator::AllocateNewRegion(size_t capacity)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    return std::make_unique<base::MappedReadOnlyRegion>(base::ReadOnlySharedMemoryRegion::Create(requested_capacity.ValueOrDie()));
}

void MojoCdmAllocator::AddRegionToAvailableMap(std::unique_ptr<base::MappedReadOnlyRegion> mapped_region)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(mapped_region);
...
base::MappedReadOnlyRegion& MojoCdmAllocator::GetRegionForTesting(cdm::Buffer* buffer)
{
