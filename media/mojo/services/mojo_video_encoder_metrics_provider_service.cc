...
public:
    EncoderMetricsHandler(const ukm::SourceId source_id, mojom::VideoEncoderUseCase encoder_use_case, VideoCodecProfile codec_profile,
        const gfx::Size& encode_size, bool is_hardware_encoder, SVCScalabilityMode svc_mode)
...
private:
    ...
    void ReportUKM() const
    {
        ...
        builder.SetWidth(std::min(encode_size_.width() / 100 * 100, kMaxResolutionBucket));
        builder.SetHeight(std::min(encode_size_.height() / 100 * 100, kMaxResolutionBucket));
        ...
        if (num_encoded_frames_ == 0) {
            ...
            builder.SetNumEncodedFrames(std::max<uint64_t>(1, num_encoded_frames_ / 100 * 100));
        }
    ...
    void ReportUMA() const
    {
        ...
        if (use_case_str.empty()) {
            mojo::ReportBadMessage(base::StrCat({ "Unknown use case", base::NumberToString(static_cast<int>(encoder_use_case_)) }));
        }
        const std::string uma_prefix = base::StrCat({ "Media.VideoEncoder.", use_case_str, ".", (is_hardware_encoder_ ? "HW" : "SW"), "." });

        base::UmaHistogramEnumeration(base::StrCat({ uma_prefix, "Profile" }), codec_profile_, static_cast<VideoCodecProfile>(VIDEO_CODEC_PROFILE_MAX + 1));
        base::UmaHistogramEnumeration(base::StrCat({ uma_prefix, "SVC" }), svc_mode_);
        base::UmaHistogramCounts10000(base::StrCat({ uma_prefix, "Width" }), encode_size_.width());
        base::UmaHistogramCounts10000(base::StrCat({ uma_prefix, "Height" }), encode_size_.height());
        base::UmaHistogramCounts1M(base::StrCat({ uma_prefix, "Area" }), encode_size_.GetArea() / 100);
        base::UmaHistogramEnumeration(base::StrCat({ uma_prefix, "Status" }), encoder_status_.code());
        // One million frames is about 9.25 hours in 30 fps. That should be enough
        // thinking of the encoder is recreated often.
        base::UmaHistogramCounts1M(base::StrCat({ uma_prefix, "Frames" }), num_encoded_frames_);
    }
...
// static
void MojoVideoEncoderMetricsProviderService::Create(ukm::SourceId source_id, mojo::PendingReceiver<mojom::VideoEncoderMetricsProvider> receiver)
{
    mojo::MakeSelfOwnedReceiver(base::WrapUnique(new MojoVideoEncoderMetricsProviderService(source_id)), std::move(receiver));
}

MojoVideoEncoderMetricsProviderService::MojoVideoEncoderMetricsProviderService(ukm::SourceId source_id)
    : source_id_(source_id)
{
}

MojoVideoEncoderMetricsProviderService::~MojoVideoEncoderMetricsProviderService() = default;

void MojoVideoEncoderMetricsProviderService::Initialize(uint64_t encoder_id, mojom::VideoEncoderUseCase encoder_use_case, VideoCodecProfile codec_profile,
    const gfx::Size& encode_size, bool is_hardware_encoder, SVCScalabilityMode svc_mode)
{
    encoders_.erase(encoder_id);
    encoders_.emplace(encoder_id, EncoderMetricsHandler(source_id_, encoder_use_case, codec_profile, encode_size, is_hardware_encoder, svc_mode));
}

void MojoVideoEncoderMetricsProviderService::SetEncodedFrameCount(uint64_t encoder_id, uint64_t num_encoded_frames)
{
    auto it = encoders_.find(encoder_id);
    if (it == encoders_.end()) {
        mojo::ReportBadMessage(base::StrCat({ "Unknown encoder id: ", base::NumberToString(encoder_id) }));
        return;
    }
    it->second.SetEncodedFrameCount(num_encoded_frames);
}

void MojoVideoEncoderMetricsProviderService::SetError(uint64_t encoder_id, const EncoderStatus& status)
{
    auto it = encoders_.find(encoder_id);
    if (it == encoders_.end()) {
        mojo::ReportBadMessage(base::StrCat({ "Unknown encoder id: ", base::NumberToString(encoder_id) }));
