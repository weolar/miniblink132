...
#if BUILDFLAG(ENABLE_CAST_RECEIVER) && (BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_ANDROID))
...
MediaMetricsProvider::MediaMetricsProvider(BrowsingMode is_incognito, FrameStatus is_top_frame, ukm::SourceId source_id, learning::FeatureValue origin,
    VideoDecodePerfHistory::SaveCallback save_cb, GetLearningSessionCallback learning_session_cb, IsShuttingDownCallback is_shutting_down_cb)
...
MediaMetricsProvider::~MediaMetricsProvider()
{
    ...
    builder.SetAudioEncryptionType(static_cast<int>(uma_info_.audio_pipeline_info.encryption_type));
    builder.SetVideoEncryptionType(static_cast<int>(uma_info_.video_pipeline_info.encryption_type));
...
std::string MediaMetricsProvider::GetUMANameForAVStream(const PipelineInfo& player_info)
{
    constexpr char kPipelineUmaPrefix[] = "Media.PipelineStatus.AudioVideo.";
    std::string uma_name = kPipelineUmaPrefix + GetCodecNameForUMA(player_info.video_codec) + ".";
...
#if BUILDFLAG(IS_ANDROID)
    if (player_info.is_eme && player_info.video_pipeline_info.decoder_type == VideoDecoderType::kMediaCodec) {
        return uma_name + "MediaDrm." + (is_hardware_secure_ ? "HardwareSecure" : "SoftwareSecure");
    }
#else
    if (player_info.video_pipeline_info.decoder_type == VideoDecoderType::kDecrypting) {
        return uma_name + "DVD";
    }
#endif
...
void MediaMetricsProvider::ReportPipelineUMA()
{
    if (uma_info_.start_status_.has_value()) {
        base::UmaHistogramExactLinear("Media.PipelineStatus.Start", uma_info_.start_status_.value(), PIPELINE_STATUS_MAX + 1);
    }

    if (uma_info_.has_video && uma_info_.has_audio) {
        base::UmaHistogramExactLinear(GetUMANameForAVStream(uma_info_), uma_info_.last_pipeline_status, PIPELINE_STATUS_MAX + 1);
    } else if (uma_info_.has_audio) {
        base::UmaHistogramExactLinear("Media.PipelineStatus.AudioOnly", uma_info_.last_pipeline_status, PIPELINE_STATUS_MAX + 1);
    } else if (uma_info_.has_video) {
        base::UmaHistogramExactLinear("Media.PipelineStatus.VideoOnly", uma_info_.last_pipeline_status, PIPELINE_STATUS_MAX + 1);
    } else {
        ...
        base::UmaHistogramExactLinear("Media.PipelineStatus.Unsupported", uma_info_.last_pipeline_status, PIPELINE_STATUS_MAX + 1);
    }

    // Report whether video decoder fallback happened for each video codec, but
    // only if a video decoder was reported.
    if (uma_info_.video_pipeline_info.decoder_type != VideoDecoderType::kUnknown) {
        base::UmaHistogramBoolean("Media.VideoDecoderFallback." + GetCodecNameForUMA(uma_info_.video_codec), uma_info_.video_decoder_changed);
    }
...
// static
void MediaMetricsProvider::Create(BrowsingMode is_incognito, FrameStatus is_top_frame, ukm::SourceId source_id, learning::FeatureValue origin,
    VideoDecodePerfHistory::SaveCallback save_cb, GetLearningSessionCallback learning_session_cb, IsShuttingDownCallback is_shutting_down_cb,
    mojo::PendingReceiver<mojom::MediaMetricsProvider> receiver)
{
    mojo::MakeSelfOwnedReceiver(std::make_unique<MediaMetricsProvider>(is_incognito, is_top_frame, source_id, origin, std::move(save_cb),
...
void MediaMetricsProvider::SetVideoPipelineInfo(const VideoPipelineInfo& info)
{
    ...
    if (old_decoder != VideoDecoderType::kUnknown && old_decoder != info.decoder_type)
...
void MediaMetricsProvider::Initialize(bool is_mse, mojom::MediaURLScheme url_scheme, mojom::MediaStreamType media_stream_type)
{
    if (IsInitialized()) {
...
void MediaMetricsProvider::OnStarted(const PipelineStatus& status)
{
    ...
    if (is_shutting_down_cb_.Run()) {
        DVLOG(1) << __func__ << ": Start status " << PipelineStatusToString(status) << " ignored since it is reported during shutdown.";
        return;
    }
...
void MediaMetricsProvider::OnError(const PipelineStatus& status)
{
    DCHECK(IsInitialized());
    if (is_shutting_down_cb_.Run()) {
        DVLOG(1) << __func__ << ": Error " << PipelineStatusToString(status) << " ignored since it is reported during shutdown.";
        return;
    }
...
void MediaMetricsProvider::SetContainerName(container_names::MediaContainerName container_name)
{
    DCHECK(IsInitialized());
...
void MediaMetricsProvider::AcquireWatchTimeRecorder(mojom::PlaybackPropertiesPtr properties, mojo::PendingReceiver<mojom::WatchTimeRecorder> receiver)
{
    if (!IsInitialized()) {
        mojo::ReportBadMessage(kInvalidInitialize);
        return;
    ...
    mojo::MakeSelfOwnedReceiver(std::make_unique<WatchTimeRecorder>(std::move(properties), source_id_, is_top_frame_, player_id_), std::move(receiver));
}

void MediaMetricsProvider::AcquireVideoDecodeStatsRecorder(mojo::PendingReceiver<mojom::VideoDecodeStatsRecorder> receiver)
{
    if (!IsInitialized()) {
        mojo::ReportBadMessage(kInvalidInitialize);
        return;
    ...
    mojo::MakeSelfOwnedReceiver(std::make_unique<VideoDecodeStatsRecorder>(save_cb_, source_id_, origin_, is_top_frame_, player_id_), std::move(receiver));
}

void MediaMetricsProvider::AcquirePlaybackEventsRecorder(mojo::PendingReceiver<mojom::PlaybackEventsRecorder> receiver)
{
#if BUILDFLAG(ENABLE_CAST_RECEIVER) && (BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_ANDROID))
...
void MediaMetricsProvider::AcquireLearningTaskController(const std::string& taskName, mojo::PendingReceiver<learning::mojom::LearningTaskController> receiver)
{
    ...
    if (!controller) {
        DVLOG(3) << __func__ << " Ignoring request, no controller found for task: '" << taskName << "'.";
        return;
    }

    mojo::MakeSelfOwnedReceiver(
        std::make_unique<learning::MojoLearningTaskControllerService>(controller->GetLearningTask(), source_id_, std::move(controller)), std::move(receiver));
}
