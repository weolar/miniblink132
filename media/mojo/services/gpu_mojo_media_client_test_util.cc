...
{
#if BUILDFLAG(ENABLE_MOJO_AUDIO_DECODER) || BUILDFLAG(PLATFORM_HAS_OPTIONAL_HEVC_DECODE_SUPPORT)
    ...
    const gpu::GpuFeatureInfo gpu_feature_info = gpu::ComputeGpuFeatureInfo(gpu_info, gpu_preferences, base::CommandLine::ForCurrentProcess(),
        /*needs_more_info=*/nullptr);
    GpuMojoMediaClientTraits traits(gpu_preferences, dummy_workarounds, gpu_feature_info, gpu_info,
    ...
    for (const auto& config : client->GetSupportedVideoDecoderConfigs()) {
        for (int profile = config.profile_min; profile <= config.profile_max; ++profile) {
