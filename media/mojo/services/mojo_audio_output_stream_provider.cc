...
MojoAudioOutputStreamProvider::MojoAudioOutputStreamProvider(mojo::PendingReceiver<mojom::AudioOutputStreamProvider> pending_receiver,
    CreateDelegateCallback create_delegate_callback, DeleterCallback deleter_callback, std::unique_ptr<media::mojom::AudioOutputStreamObserver> observer)
    ...
    , observer_receiver_(observer_.get())
{
    ...
    receiver_.set_disconnect_handler(base::BindOnce(&MojoAudioOutputStreamProvider::CleanUp, base::Unretained(this), /*had_error*/ false));
...
void MojoAudioOutputStreamProvider::Acquire(const AudioParameters& params, mojo::PendingRemote<mojom::AudioOutputStreamProviderClient> provider_client)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
// support audio processing.
    ...
    if (params.IsBitstreamFormat()) {
        ...
        BadMessage("Attempted to acquire a bitstream audio stream on a platform where "
                   "it's not supported");
        return;
    }
#endif
    ...
    audio_output_.emplace(base::BindOnce(std::move(create_delegate_callback_), params, std::move(pending_observer)),
...
{
    if (had_error) {
        provider_client_.ResetWithReason(static_cast<uint32_t>(media::mojom::AudioOutputStreamObserver::DisconnectReason::kPlatformError), std::string());
    }
