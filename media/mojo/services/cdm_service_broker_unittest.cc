...
public:
    ...
    std::unique_ptr<media::CdmFactory> CreateCdmFactory(mojom::FrameInterfaceFactory* frame_interfaces) override
    {
        return nullptr;
    }
...
public:
    ...
    void Initialize()
    {
        ...
        broker_ = std::make_unique<CdmServiceBroker>(std::move(mock_cdm_service_client), remote_.BindNewPipeAndPassReceiver());
    }
...
TEST_F(CdmServiceBrokerTest, GetService)
{
    ...
    remote_->GetService(cdm_path, mojo::NullRemote(), service_remote_.BindNewPipeAndPassReceiver());
#else
