...
public:
    ...
    ~MediaMetricsProviderTest() override
    {
        base::RunLoop().RunUntilIdle();
    }

    void Initialize(bool is_mse, bool is_incognito, bool is_top_frame, const std::string& origin, mojom::MediaURLScheme scheme,
        mojom::MediaStreamType media_stream_type = mojom::MediaStreamType::kNone)
    {
        ...
        MediaMetricsProvider::Create((is_incognito ? MediaMetricsProvider::BrowsingMode::kIncognito : MediaMetricsProvider::BrowsingMode::kNormal),
            (is_top_frame ? MediaMetricsProvider::FrameStatus::kTopFrame : MediaMetricsProvider::FrameStatus::kNotTopFrame), GetSourceId(),
            learning::FeatureValue(0), VideoDecodePerfHistory::SaveCallback(), MediaMetricsProvider::GetLearningSessionCallback(),
            base::BindRepeating(&MediaMetricsProviderTest::IsShuttingDown, base::Unretained(this)), provider_.BindNewPipeAndPassReceiver());
...
TEST_F(MediaMetricsProviderTest, TestUkm)
{
    ...
    provider_->SetAudioPipelineInfo({ false, false, AudioDecoderType::kMojo, EncryptionType::kClear });
    provider_->SetVideoPipelineInfo({ false, false, VideoDecoderType::kMojo, EncryptionType::kEncrypted });
    ...
    provider_->SetContainerName(container_names::MediaContainerName::kContainerMOV);
...

    {
        ...
        for (const ukm::mojom::UkmEntry* entry : entries) {
            ...
            EXPECT_UKM(UkmEntry::kAudioEncryptionTypeName, static_cast<int64_t>(EncryptionType::kClear));
            EXPECT_UKM(UkmEntry::kVideoEncryptionTypeName, static_cast<int64_t>(EncryptionType::kEncrypted));
            EXPECT_UKM(UkmEntry::kIsMSEName, false);
            EXPECT_UKM(UkmEntry::kURLSchemeName, static_cast<int64_t>(mojom::MediaURLScheme::kHttps));
            ...
            EXPECT_UKM(UkmEntry::kTimeToFirstFrameName, kFirstFrameTime.InMilliseconds());
            EXPECT_UKM(UkmEntry::kTimeToPlayReadyName, kPlayReadyTime.InMilliseconds());
            EXPECT_UKM(UkmEntry::kContainerNameName, base::to_underlying(container_names::MediaContainerName::kContainerMOV));
        }
    }
}
...
{
    Initialize(true, false, true, kTestOrigin, mojom::MediaURLScheme::kMissing, mojom::MediaStreamType::kRemote);
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    ...
    ResetMetricRecorders();
    Initialize(false, false, false, kTestOrigin2, mojom::MediaURLScheme::kMissing, mojom::MediaStreamType::kLocalDeviceCapture);
    provider_->SetIsEME();
    provider_->SetTimeToMetadata(kMetadataTime);
    provider_->SetTimeToFirstFrame(kFirstFrameTime);
    provider_->SetTimeToPlayReady(kPlayReadyTime);
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMA)
{
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.SW", PIPELINE_OK, 1);
    histogram_tester.ExpectBucketCount("Media.VideoDecoderFallback.VP9", false, 1);
...
{
    base::HistogramTester histogram_tester;
    Initialize(false, false, false, kTestOrigin, mojom::MediaURLScheme::kHttps, mojom::MediaStreamType::kRemote);
    provider_->SetAudioPipelineInfo({ false, false, AudioDecoderType::kMojo, EncryptionType::kClear });
    provider_->SetVideoPipelineInfo({ false, false, VideoDecoderType::kMojo, EncryptionType::kEncrypted });
    provider_->SetHasAudio(AudioCodec::kVorbis);
    provider_->SetHasVideo(VideoCodec::kVP9);
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.SW", PIPELINE_OK, 0);
    histogram_tester.ExpectBucketCount("Media.VideoDecoderFallback.VP9", false, 0);
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMANoAudioWithEme)
{
    ...
    Initialize(false, false, false, kTestOrigin, mojom::MediaURLScheme::kHttps);
    provider_->SetIsEME();
    provider_->SetVideoPipelineInfo({ true, true, VideoDecoderType::kMojo, EncryptionType::kEncrypted });
    provider_->SetHasVideo(VideoCodec::kAV1);
    provider_->SetHasPlayed();
    provider_->SetHaveEnough();
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.VideoOnly", PIPELINE_OK, 1);
    histogram_tester.ExpectBucketCount("Media.VideoDecoderFallback.AV1", false, 1);
    histogram_tester.ExpectBucketCount("Media.HasEverPlayed", true, 1);
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMADecoderFallback)
{
    ...
    provider_->SetIsEME();
    provider_->SetAudioPipelineInfo({ false, false, AudioDecoderType::kMojo, EncryptionType::kClear });
    provider_->SetVideoPipelineInfo({ true, false, VideoDecoderType::kD3D11, EncryptionType::kEncrypted });
    provider_->SetHasVideo(VideoCodec::kVP9);
    provider_->SetHasAudio(AudioCodec::kVorbis);
    provider_->SetHasPlayed();
    provider_->SetHaveEnough();
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.HW", PIPELINE_OK, 1);
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMARendererType)
{
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.MediaFoundationRenderer", PIPELINE_OK, 1);
}
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMAMediaDrmSoftwareSecure)
{
    ...
    Initialize(false, false, false, kTestOrigin, mojom::MediaURLScheme::kHttps);
    provider_->SetAudioPipelineInfo({ false, false, AudioDecoderType::kMojo, EncryptionType::kClear });
    provider_->SetVideoPipelineInfo({ false, false, VideoDecoderType::kMediaCodec, EncryptionType::kEncrypted });
    provider_->SetIsEME();
    provider_->SetHasVideo(VideoCodec::kVP9);
    provider_->SetHasAudio(AudioCodec::kVorbis);
    provider_->SetHasPlayed();
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.MediaDrm.SoftwareSecure", PIPELINE_OK, 1);
}
...
TEST_F(MediaMetricsProviderTest, TestPipelineUMAMediaDrmHardwareSecure)
{
    ...
    provider_.reset();
    base::RunLoop().RunUntilIdle();
    histogram_tester.ExpectBucketCount("Media.PipelineStatus.AudioVideo.VP9.MediaDrm.HardwareSecure", PIPELINE_OK, 1);
}
