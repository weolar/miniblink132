...
public:
    ...
    void ExpectOwnershipTransfer()
    {
        expect_ownership_transfer_ = true;
    }
...
public:
    void PrepareDelegateForCreation(std::unique_ptr<AudioInputDelegate> delegate)
    {
        ASSERT_EQ(nullptr, delegate_);
        delegate_.swap(delegate);
    }

    std::unique_ptr<AudioInputDelegate> CreateDelegate(AudioInputDelegate::EventHandler* handler)
    {
...
public:
    ...
    void Initialized(mojom::ReadOnlyAudioDataPipePtr data_pipe, bool initially_muted)
    {
        ASSERT_TRUE(data_pipe->shared_memory.IsValid());
        ASSERT_TRUE(data_pipe->socket.is_valid());

        socket_ = std::make_unique<base::CancelableSyncSocket>(data_pipe->socket.TakePlatformFile());
...
std::unique_ptr<AudioInputDelegate> CreateNoDelegate(AudioInputDelegate::EventHandler* event_handler)
{
    return nullptr;
}

void NotCalled(mojom::ReadOnlyAudioDataPipePtr data_pipe, bool initially_muted)
{
...
public:
    ...
    mojo::Remote<mojom::AudioInputStream> CreateAudioInput()
    {
        ...
        impl_ = std::make_unique<MojoAudioInputStream>(stream.BindNewPipeAndPassReceiver(), client_receiver_.BindNewPipeAndPassRemote(),
            base::BindOnce(&MockDelegateFactory::CreateDelegate, base::Unretained(&mock_delegate_factory_)),
            base::BindOnce(&MockClient::Initialized, base::Unretained(&client_)), base::BindOnce(&MockDeleter::Finished, base::Unretained(&deleter_)));
...
protected:
    void ExpectDelegateCreation()
    {
        ...
        mock_delegate_factory_.PrepareDelegateForCreation(base::WrapUnique(delegate_.get()));
        EXPECT_TRUE(base::CancelableSyncSocket::CreatePair(&local_, foreign_socket_.get()));
        mem_ = base::ReadOnlySharedMemoryRegion::Create(kShmemSize).region;
        EXPECT_TRUE(mem_.IsValid());
        EXPECT_CALL(mock_delegate_factory_, MockCreateDelegate(NotNull())).WillOnce(SaveArg<0>(&delegate_event_handler_));
    }
...
TEST_F(MojoAudioInputStreamTest, NoDelegate_SignalsError)
{
    ...
    MojoAudioInputStream stream(stream_remote.BindNewPipeAndPassReceiver(), client_receiver_.BindNewPipeAndPassRemote(), base::BindOnce(&CreateNoDelegate),
        base::BindOnce(&NotCalled), base::BindOnce([](bool* p) { *p = true; }, &deleter_called));
    EXPECT_FALSE(deleter_called) << "Stream shouldn't call the deleter from its constructor.";
...
TEST_F(MojoAudioInputStreamTest, DestructWithCallPending_Safe)
{
    ...
    delegate_event_handler_->OnStreamCreated(kStreamId, std::move(mem_), std::move(foreign_socket_), kInitiallyNotMuted);
    audio_input->Record();
