...
protected:
    std::unique_ptr<VideoDecoder> CreatePlatformVideoDecoder(VideoDecoderTraits& traits) final
    {
        return std::make_unique<VideoToolboxVideoDecoder>(
            traits.task_runner, traits.media_log->Clone(), gpu_workarounds_, gpu_task_runner_, traits.get_command_buffer_stub_cb);
    }
    ...
    {
        return VideoToolboxVideoDecoder::GetSupportedVideoDecoderConfigs(gpu_workarounds_);
    }
    ...
    std::unique_ptr<AudioEncoder> CreatePlatformAudioEncoder(scoped_refptr<base::SequencedTaskRunner> task_runner) final
    {
        ...
        return std::make_unique<OffloadingAudioEncoder>(std::move(encoder), std::move(encoding_runner), std::move(task_runner));
    }
