...
public:
    GpuMojoMediaClientWin(GpuMojoMediaClientTraits& traits)
        : GpuMojoMediaClientWin(
            ...
            // GpuMojoMediaClient().
            traits.media_gpu_channel_manager ? traits.media_gpu_channel_manager->GetSharedContextState() : nullptr, traits)
    {
    }
...
protected:
    std::unique_ptr<VideoDecoder> CreatePlatformVideoDecoder(VideoDecoderTraits& traits) final
    {
        ...
        return D3D11VideoDecoder::Create(gpu_task_runner_, traits.media_log->Clone(), gpu_preferences_, gpu_workarounds_, traits.get_command_buffer_stub_cb,
            GetD3DDeviceCallback(), traits.get_cached_configs_cb.Run());
    }

    std::unique_ptr<AudioEncoder> CreatePlatformAudioEncoder(scoped_refptr<base::SequencedTaskRunner> task_runner) final
    {
        ...
        return std::make_unique<OffloadingAudioEncoder>(std::move(mf_encoder), std::move(encoding_runner), std::move(task_runner));
    }

    std::optional<SupportedAudioDecoderConfigs> GetPlatformSupportedAudioDecoderConfigs() final
    {
        ...
        if (has_legacy_dolby_ac3_eac3_mft || FindMediaFoundationPackageDecoder(AudioCodec::kEAC3)) {
        ...
        if (base::win::GetVersion() >= base::win::Version::WIN11_22H2 && InitializeMediaFoundation()) {
    ...
    std::optional<SupportedVideoDecoderConfigs> GetPlatformSupportedVideoDecoderConfigs() final
    {
        ...
        if (!gpu_workarounds_.disable_d3d11_video_decoder) {
            supported_configs = D3D11VideoDecoder::GetSupportedVideoDecoderConfigs(gpu_preferences_, gpu_workarounds_, GetD3DDeviceCallback());
        }
        return supported_configs;
    }

    std::unique_ptr<AudioDecoder> CreatePlatformAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, std::unique_ptr<MediaLog> media_log) final
    {
...
private:
    GpuMojoMediaClientWin(scoped_refptr<gpu::SharedContextState> shared_context_state, GpuMojoMediaClientTraits& traits)
    ...
    D3D11VideoDecoder::GetD3DDeviceCB GetD3DDeviceCallback()
    {
        return base::BindRepeating(
            [](Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<ID3D12Device> d3d12_device,
                D3D11VideoDecoder::D3DVersion d3d_version) -> Microsoft::WRL::ComPtr<IUnknown> {
