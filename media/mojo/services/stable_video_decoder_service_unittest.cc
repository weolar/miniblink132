...
VideoDecoderConfig CreateValidVideoDecoderConfig()
{
    const VideoDecoderConfig config(VideoCodec::kH264, VideoCodecProfile::H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kHasAlpha,
        VideoColorSpace::REC709(), VideoTransformation(VIDEO_ROTATION_90, /*mirrored=*/true),
...
scoped_refptr<VideoFrame> CreateTestNV12GpuMemoryBufferVideoFrame()
{
    ...
    auto gmb = gmb_support.CreateGpuMemoryBufferImplFromHandle(
        std::move(gmb_handle), gfx::Size(640, 368), gfx::BufferFormat::YUV_420_BIPLANAR, gfx::BufferUsage::SCANOUT_VDA_WRITE, base::DoNothing());
    if (!gmb) {
        return nullptr;
    }
...
public:
    explicit MockVideoFrameHandleReleaser(mojo::PendingReceiver<mojom::VideoFrameHandleReleaser> video_frame_handle_releaser)
    ...
    MOCK_METHOD2(ReleaseVideoFrame, void(const base::UnguessableToken& release_token, const std::optional<gpu::SyncToken>& release_sync_token));

private:
    mojo::Receiver<mojom::VideoFrameHandleReleaser> video_frame_handle_releaser_receiver_;
};
...
public:
    ...
    std::unique_ptr<StrictMock<MockVideoFrameHandleReleaser>> TakeVideoFrameHandleReleaser()
    {
    ...
    void Construct(mojo::PendingAssociatedRemote<mojom::VideoDecoderClient> client, mojo::PendingRemote<mojom::MediaLog> media_log,
        mojo::PendingReceiver<mojom::VideoFrameHandleReleaser> video_frame_handle_releaser, mojo::ScopedDataPipeConsumerHandle decoder_buffer_pipe,
        mojom::CommandBufferIdPtr command_buffer_id, const gfx::ColorSpace& target_color_space) final
    {
        client_remote_.Bind(std::move(client));
        media_log_remote_.Bind(std::move(media_log));
        video_frame_handle_releaser_ = std::make_unique<StrictMock<MockVideoFrameHandleReleaser>>(std::move(video_frame_handle_releaser));
    ...
    MOCK_METHOD2(DoConstruct, void(mojom::CommandBufferIdPtr command_buffer_id, const gfx::ColorSpace& target_color_space));
    MOCK_METHOD4(
        Initialize, void(const VideoDecoderConfig& config, bool low_delay, const std::optional<base::UnguessableToken>& cdm_id, InitializeCallback callback));
    MOCK_METHOD2(Decode, void(mojom::DecoderBufferPtr buffer, DecodeCallback callback));
...
private:
    ...
    std::unique_ptr<StrictMock<MockVideoFrameHandleReleaser>> video_frame_handle_releaser_;
    std::unique_ptr<MojoDecoderBufferReader> mojo_decoder_buffer_reader_;
};

class MockStableVideoDecoderTracker : public stable::mojom::StableVideoDecoderTracker { };

class MockStableVideoDecoderClient : public stable::mojom::VideoDecoderClient {
public:
    explicit MockStableVideoDecoderClient(mojo::PendingAssociatedReceiver<stable::mojom::VideoDecoderClient> pending_receiver)
        : receiver_(this, std::move(pending_receiver))
    {
    }
    MockStableVideoDecoderClient(const MockStableVideoDecoderClient&) = delete;
    MockStableVideoDecoderClient& operator=(const MockStableVideoDecoderClient&) = delete;
    ~MockStableVideoDecoderClient() override = default;

    // stable::mojom::VideoDecoderClient implementation.
    MOCK_METHOD3(OnVideoFrameDecoded, void(stable::mojom::VideoFramePtr frame, bool can_read_without_stalling, const base::UnguessableToken& release_token));
...
public:
    explicit MockStableMediaLog(mojo::PendingReceiver<stable::mojom::MediaLog> pending_receiver)
        : receiver_(this, std::move(pending_receiver))
    {
    }
...
struct AuxiliaryEndpoints {
    ...
    std::unique_ptr<StrictMock<MockStableVideoDecoderClient>> mock_stable_video_decoder_client;
    ...
    mojo::Remote<stable::mojom::VideoFrameHandleReleaser> stable_video_frame_handle_releaser_remote;
...
std::unique_ptr<AuxiliaryEndpoints> ConstructStableVideoDecoder(
    mojo::Remote<stable::mojom::StableVideoDecoder>& stable_video_decoder_remote, StrictMock<MockVideoDecoder>& mock_video_decoder, bool expect_construct_call)
{
    ...
    mojo::PendingAssociatedRemote<stable::mojom::VideoDecoderClient> stable_video_decoder_client_remote;
    auto mock_stable_video_decoder_client
        = std::make_unique<StrictMock<MockStableVideoDecoderClient>>(stable_video_decoder_client_remote.InitWithNewEndpointAndPassReceiver());

    mojo::PendingRemote<stable::mojom::MediaLog> stable_media_log_remote;
    auto mock_stable_media_log = std::make_unique<StrictMock<MockStableMediaLog>>(stable_media_log_remote.InitWithNewPipeAndPassReceiver());

    mojo::Remote<stable::mojom::VideoFrameHandleReleaser> video_frame_handle_releaser_remote;

    mojo::ScopedDataPipeConsumerHandle remote_consumer_handle;
    std::unique_ptr<MojoDecoderBufferWriter> mojo_decoder_buffer_writer
        = MojoDecoderBufferWriter::Create(GetDefaultDecoderBufferConverterCapacity(DemuxerStream::VIDEO), &remote_consumer_handle);

    stable_video_decoder_remote->Construct(std::move(stable_video_decoder_client_remote), std::move(stable_media_log_remote),
        video_frame_handle_releaser_remote.BindNewPipeAndPassReceiver(), std::move(remote_consumer_handle), kTargetColorSpace);
    ...
    auxiliary_endpoints->video_decoder_client_remote = mock_video_decoder.TakeClientRemote();
    auxiliary_endpoints->mock_stable_video_decoder_client = std::move(mock_stable_video_decoder_client);

    auxiliary_endpoints->media_log_remote = mock_video_decoder.TakeMediaLogRemote();
    auxiliary_endpoints->mock_stable_media_log = std::move(mock_stable_media_log);

    auxiliary_endpoints->stable_video_frame_handle_releaser_remote = std::move(video_frame_handle_releaser_remote);
    auxiliary_endpoints->mock_video_frame_handle_releaser = mock_video_decoder.TakeVideoFrameHandleReleaser();

    auxiliary_endpoints->mojo_decoder_buffer_writer = std::move(mojo_decoder_buffer_writer);
    auxiliary_endpoints->mojo_decoder_buffer_reader = mock_video_decoder.TakeMojoDecoderBufferReader();
...
public:
    ...
    {
        stable_video_decoder_factory_service_.SetVideoDecoderCreationCallbackForTesting(video_decoder_creation_cb_.Get());
    }

    StableVideoDecoderServiceTest(const StableVideoDecoderServiceTest&) = delete;
    StableVideoDecoderServiceTest& operator=(const StableVideoDecoderServiceTest&) = delete;
    ~StableVideoDecoderServiceTest() override = default;

    void SetUp() override
    {
        mojo::PendingReceiver<stable::mojom::StableVideoDecoderFactory> stable_video_decoder_factory_receiver;
        stable_video_decoder_factory_remote_
            = mojo::Remote<stable::mojom::StableVideoDecoderFactory>(stable_video_decoder_factory_receiver.InitWithNewPipeAndPassRemote());
...
protected:
    mojo::Remote<stable::mojom::StableVideoDecoder> CreateStableVideoDecoder(
        std::unique_ptr<StrictMock<MockVideoDecoder>> dst_video_decoder, mojo::PendingRemote<stable::mojom::StableVideoDecoderTracker> tracker)
    {
        ...
        EXPECT_CALL(video_decoder_creation_cb_, Run(_, _)).WillOnce(Return(ByMove(std::move(dst_video_decoder))));
        mojo::PendingReceiver<stable::mojom::StableVideoDecoder> stable_video_decoder_receiver;
        mojo::Remote<stable::mojom::StableVideoDecoder> video_decoder_remote(stable_video_decoder_receiver.InitWithNewPipeAndPassRemote());
        stable_video_decoder_factory_remote_->CreateStableVideoDecoder(std::move(stable_video_decoder_receiver), std::move(tracker));
    ...
    StrictMock<base::MockRepeatingCallback<std::unique_ptr<mojom::VideoDecoder>(MojoMediaClient*, MojoCdmServiceContext*)>> video_decoder_creation_cb_;
    StableVideoDecoderFactoryService stable_video_decoder_factory_service_;
    mojo::Remote<stable::mojom::StableVideoDecoderFactory> stable_video_decoder_factory_remote_;
...
TEST_F(StableVideoDecoderServiceTest, FactoryCanCreateStableVideoDecoders)
{
    ...
    for (size_t i = 0u; i < kNumConcurrentDecoders; i++) {
        ...
        auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
        stable_video_decoder_remotes.push_back(std::move(stable_video_decoder_remote));
    }
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanBeConstructed)
{
    ...
    ASSERT_TRUE(ConstructStableVideoDecoder(stable_video_decoder_remote, *mock_video_decoder_raw,
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCannotBeConstructedTwice)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto* mock_video_decoder_raw = mock_video_decoder.get();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
    ASSERT_TRUE(stable_video_decoder_remote.is_connected());
    EXPECT_TRUE(ConstructStableVideoDecoder(stable_video_decoder_remote, *mock_video_decoder_raw,
        /*expect_construct_call=*/true));
    EXPECT_TRUE(ConstructStableVideoDecoder(stable_video_decoder_remote, *mock_video_decoder_raw,
...
// client.
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanGetSupportedConfigs)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto* mock_video_decoder_raw = mock_video_decoder.get();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ...
    StrictMock<base::MockOnceCallback<void(const std::vector<SupportedVideoDecoderConfig>& supported_configs, VideoDecoderType decoder_type)>>
        get_supported_configs_cb_to_send;
    mojom::VideoDecoder::GetSupportedConfigsCallback received_get_supported_configs_cb;
    constexpr VideoDecoderType kDecoderTypeToReplyWith = VideoDecoderType::kVaapi;
    const std::vector<SupportedVideoDecoderConfig> supported_configs_to_reply_with({
    ...
    EXPECT_CALL(*mock_video_decoder_raw, GetSupportedConfigs(/*callback=*/_)).WillOnce([&](mojom::VideoDecoder::GetSupportedConfigsCallback callback) {
        received_get_supported_configs_cb = std::move(callback);
    });
    EXPECT_CALL(get_supported_configs_cb_to_send, Run(_, kDecoderTypeToReplyWith)).WillOnce(SaveArg<0>(&received_supported_configs));

    stable_video_decoder_remote->GetSupportedConfigs(get_supported_configs_cb_to_send.Get());
    stable_video_decoder_remote.FlushForTesting();
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanBeInitialized)
{
    ...
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
    ASSERT_TRUE(stable_video_decoder_remote.is_connected());
    auto auxiliary_endpoints = ConstructStableVideoDecoder(stable_video_decoder_remote, *mock_video_decoder_raw,
        /*expect_construct_call=*/true);
    ...
    StrictMock<base::MockOnceCallback<void(const media::DecoderStatus& status, bool needs_bitstream_conversion, int32_t max_decode_requests,
        VideoDecoderType decoder_type, bool needs_transcryption)>>
    ...
    EXPECT_CALL(*mock_video_decoder_raw,
        Initialize(/*config=*/_, kLowDelay, kCdmId,
            /*callback=*/_))
        .WillOnce([&](const VideoDecoderConfig& config, bool low_delay, const std::optional<base::UnguessableToken>& cdm_id,
                      mojom::VideoDecoder::InitializeCallback callback) {
            received_config = config;
            received_initialize_cb = std::move(callback);
        });
    EXPECT_CALL(initialize_cb_to_send, Run(kDecoderStatus, kNeedsBitstreamConversion, kMaxDecodeRequests, kDecoderType, /*needs_transcryption=*/false));
    stable_video_decoder_remote->Initialize(config_to_send, kLowDelay, mojo::PendingRemote<stable::mojom::StableCdmContext>(), initialize_cb_to_send.Get());
    stable_video_decoder_remote.FlushForTesting();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(mock_video_decoder_raw));

    std::move(received_initialize_cb).Run(kDecoderStatus, kNeedsBitstreamConversion, kMaxDecodeRequests, kDecoderType);
    task_environment_.RunUntilIdle();
}
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCannotBeInitializedBeforeConstruction)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanDecode)
{
    ...
    StrictMock<base::MockOnceCallback<void(const media::DecoderStatus& status)>> decode_cb_to_send;
    mojom::VideoDecoder::DecodeCallback received_decode_cb;
    const DecoderStatus kDecoderStatus = DecoderStatus::Codes::kAborted;

    EXPECT_CALL(*mock_video_decoder_raw, Decode(/*buffer=*/_, /*callback=*/_))
        .WillOnce([&](mojom::DecoderBufferPtr buffer, mojom::VideoDecoder::DecodeCallback callback) {
    ...
    ASSERT_TRUE(auxiliary_endpoints->mojo_decoder_buffer_writer->WriteDecoderBuffer(decoder_buffer_to_send));
    stable_video_decoder_remote->Decode(decoder_buffer_to_send, decode_cb_to_send.Get());
    stable_video_decoder_remote.FlushForTesting();
    ASSERT_TRUE(Mock::VerifyAndClearExpectations(mock_video_decoder_raw));

    ASSERT_TRUE(received_decoder_buffer_ptr);
    auxiliary_endpoints->mojo_decoder_buffer_reader->ReadDecoderBuffer(std::move(received_decoder_buffer_ptr),
        base::BindOnce(
            [](scoped_refptr<DecoderBuffer>* dst_buffer, scoped_refptr<DecoderBuffer> buffer) { *dst_buffer = std::move(buffer); }, &received_decoder_buffer));
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(received_decoder_buffer);
    EXPECT_TRUE(received_decoder_buffer->MatchesForTesting(*decoder_buffer_to_send));
...
// stable::mojom::VideoDecoder::Construct() gets called.
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCannotDecodeBeforeConstruction)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
    ...
    EXPECT_CALL(decode_cb_to_send, Run(DecoderStatus(DecoderStatus::Codes::kFailed)));
    stable_video_decoder_remote->Decode(decoder_buffer_to_send, decode_cb_to_send.Get());
    stable_video_decoder_remote.FlushForTesting();
}
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCanBeReset)
{
    ...
    EXPECT_CALL(*mock_video_decoder_raw, Reset(/*callback=*/_)).WillOnce([&](mojom::VideoDecoder::ResetCallback callback) {
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderCannotBeResetBeforeConstruction)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
...
TEST_F(StableVideoDecoderServiceTest, VideoFramesCanBeReleased)
{
    ...
    const base::UnguessableToken release_token_to_send = base::UnguessableToken::Create();
    const std::optional<gpu::SyncToken> expected_release_sync_token = std::nullopt;

    EXPECT_CALL(*auxiliary_endpoints->mock_video_frame_handle_releaser, ReleaseVideoFrame(release_token_to_send, expected_release_sync_token));
    auxiliary_endpoints->stable_video_frame_handle_releaser_remote->ReleaseVideoFrame(release_token_to_send);
    auxiliary_endpoints->stable_video_frame_handle_releaser_remote.FlushForTesting();
}

TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderClientReceivesOnVideoFrameDecodedEvent)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto* mock_video_decoder_raw = mock_video_decoder.get();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ...
    scoped_refptr<VideoFrame> video_frame_to_send = CreateTestNV12GpuMemoryBufferVideoFrame();
    ...
    EXPECT_CALL(*auxiliary_endpoints->mock_stable_video_decoder_client, OnVideoFrameDecoded(_, kCanReadWithoutStalling, token_for_release))
        .WillOnce(WithArgs<0>([&video_frame_received](stable::mojom::VideoFramePtr frame) { video_frame_received = std::move(frame); }));
    auxiliary_endpoints->video_decoder_client_remote->OnVideoFrameDecoded(video_frame_to_send, kCanReadWithoutStalling, token_for_release);
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderClientReceivesOnWaitingEvent)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto* mock_video_decoder_raw = mock_video_decoder.get();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ...
    EXPECT_CALL(*auxiliary_endpoints->mock_stable_video_decoder_client, OnWaiting(kWaitingReason));
...
// correctly.
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderClientReceivesAddLogRecordEvent)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();
    auto* mock_video_decoder_raw = mock_video_decoder.get();
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), /*tracker=*/ {});
    ...
    EXPECT_CALL(*auxiliary_endpoints->mock_stable_media_log, AddLogRecord(media_log_record_to_send));
...
TEST_F(StableVideoDecoderServiceTest, StableVideoDecoderTrackerDisconnectsWhenStableVideoDecoderDies)
{
    auto mock_video_decoder = std::make_unique<StrictMock<MockVideoDecoder>>();

    MockStableVideoDecoderTracker tracker;
    mojo::Receiver<stable::mojom::StableVideoDecoderTracker> tracker_receiver(&tracker);
    mojo::PendingRemote<stable::mojom::StableVideoDecoderTracker> tracker_remote = tracker_receiver.BindNewPipeAndPassRemote();
    ...
    auto stable_video_decoder_remote = CreateStableVideoDecoder(std::move(mock_video_decoder), std::move(tracker_remote));
    ASSERT_TRUE(stable_video_decoder_remote.is_bound());
    ASSERT_TRUE(stable_video_decoder_remote.is_connected());
