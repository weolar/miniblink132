...
gfx::GpuMemoryBufferHandle GetGpuMemoryBufferHandle(scoped_refptr<VideoFrame> media_frame, scoped_refptr<const FrameRegistry> frame_registry)
{
    ...
    case VideoFrame::STORAGE_OPAQUE: {
        ...
        auto frame_resource = frame_registry->AccessFrame(*media_frame->metadata().tracking_token);
...
stable::mojom::VideoFramePtr MediaVideoFrameToMojoVideoFrame(scoped_refptr<VideoFrame> media_frame, scoped_refptr<const FrameRegistry> frame_registry)
{
    ...
    static_assert(std::is_same<decltype(media_frame->format()), decltype(stable::mojom::VideoFrame::format)>::value,
    ...
    static_assert(std::is_same<decltype(media_frame->coded_size()),
                      std::add_lvalue_reference<std::add_const<decltype(stable::mojom::VideoFrame::coded_size)>::type>::type>::value,
        "Unexpected type for media::VideoFrame::coded_size(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->coded_size = media_frame->coded_size();

    static_assert(std::is_same<decltype(media_frame->visible_rect()),
                      std::add_lvalue_reference<std::add_const<decltype(stable::mojom::VideoFrame::visible_rect)>::type>::type>::value,
        "Unexpected type for media::VideoFrame::visible_rect(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->visible_rect = media_frame->visible_rect();

    static_assert(std::is_same<decltype(media_frame->natural_size()),
                      std::add_lvalue_reference<std::add_const<decltype(stable::mojom::VideoFrame::natural_size)>::type>::type>::value,
        "Unexpected type for media::VideoFrame::natural_size(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->natural_size = media_frame->natural_size();

    static_assert(std::is_same<decltype(media_frame->timestamp()), decltype(stable::mojom::VideoFrame::timestamp)>::value,
        "Unexpected type for media::VideoFrame::timestamp(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->timestamp = media_frame->timestamp();

    gfx::GpuMemoryBufferHandle gpu_memory_buffer_handle = GetGpuMemoryBufferHandle(media_frame, frame_registry);
    ...
    static_assert(std::is_same<decltype(media_frame->metadata()), std::add_lvalue_reference<decltype(stable::mojom::VideoFrame::metadata)>::type>::value,
        "Unexpected type for media::VideoFrame::metadata(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->metadata = media_frame->metadata();

    static_assert(std::is_same<decltype(media_frame->ColorSpace()), decltype(stable::mojom::VideoFrame::color_space)>::value,
        "Unexpected type for media::VideoFrame::ColorSpace(). If you "
        "need to change this assertion, please contact "
        "chromeos-gfx-video@google.com.");
    mojo_frame->color_space = media_frame->ColorSpace();

    static_assert(std::is_same<decltype(media_frame->hdr_metadata()),
                      std::add_lvalue_reference<std::add_const<decltype(stable::mojom::VideoFrame::hdr_metadata)>::type>::type>::value,
...
StableVideoDecoderService::StableVideoDecoderService(mojo::PendingRemote<stable::mojom::StableVideoDecoderTracker> tracker_remote,
    std::unique_ptr<mojom::VideoDecoder> dst_video_decoder, MojoCdmServiceContext* cdm_service_context, scoped_refptr<const FrameRegistry> frame_registry)
    ...
    , frame_registry_(frame_registry)
{
    ...
    dst_video_decoder_remote_.Bind(dst_video_decoder_receiver_.BindNewPipeAndPassRemote());
}
...
void StableVideoDecoderService::GetSupportedConfigs(GetSupportedConfigsCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    dst_video_decoder_remote_->GetSupportedConfigs(std::move(callback));
}

void StableVideoDecoderService::Construct(mojo::PendingAssociatedRemote<stable::mojom::VideoDecoderClient> stable_video_decoder_client_remote,
    ...
    mojo::ScopedDataPipeConsumerHandle decoder_buffer_pipe, const gfx::ColorSpace& target_color_space)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    stable_video_frame_handle_releaser_receiver_.Bind(std::move(stable_video_frame_handle_releaser_receiver));

    dst_video_decoder_remote_->Construct(video_decoder_client_receiver_.BindNewEndpointAndPassRemote(), media_log_receiver_.BindNewPipeAndPassRemote(),
        video_frame_handle_releaser_remote_.BindNewPipeAndPassReceiver(), std::move(decoder_buffer_pipe), mojom::CommandBufferIdPtr(), target_color_space);
}

void StableVideoDecoderService::Initialize(
    const VideoDecoderConfig& config, bool low_delay, mojo::PendingRemote<stable::mojom::StableCdmContext> cdm_context, InitializeCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!video_decoder_client_receiver_.is_bound()) {
    ...
    if (config.is_encrypted()) {
        ...
        if (!cdm_id_) {
            ...
            remote_cdm_context_ = base::WrapRefCounted(new chromeos::RemoteCdmContext(std::move(cdm_context)));
            cdm_id_ = cdm_service_context_->RegisterRemoteCdmContext(remote_cdm_context_.get());
        }
#if BUILDFLAG(USE_VAAPI)
        needs_transcryption = (VaapiWrapper::GetImplementationType() == VAImplementation::kMesaGallium);
#endif
#else
    ...
    dst_video_decoder_remote_->Initialize(config, low_delay, cdm_id_,
        base::BindOnce(&StableVideoDecoderService::OnInitializeDone, base::Unretained(this), std::move(callback), needs_transcryption));
}

void StableVideoDecoderService::OnInitializeDone(InitializeCallback init_cb, bool needs_transcryption, const DecoderStatus& status,
    bool needs_bitstream_conversion, int32_t max_decode_requests, VideoDecoderType decoder_type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    std::move(init_cb).Run(status, needs_bitstream_conversion, max_decode_requests, decoder_type, needs_transcryption);
}

void StableVideoDecoderService::Decode(const scoped_refptr<DecoderBuffer>& buffer, DecodeCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!video_decoder_client_receiver_.is_bound()) {
        DVLOG(2) << __func__ << " Construct() must be called first";
    ...
    CHECK(mojo_buffer);
    dst_video_decoder_remote_->Decode(std::move(mojo_buffer), std::move(callback));
}
...
void StableVideoDecoderService::ReleaseVideoFrame(const base::UnguessableToken& release_token)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(video_frame_handle_releaser_remote_.is_bound());
    ...
    video_frame_handle_releaser_remote_->ReleaseVideoFrame(release_token, /*release_sync_token=*/std::nullopt);
}
    ...
    const scoped_refptr<VideoFrame>& frame, bool can_read_without_stalling, const std::optional<base::UnguessableToken>& release_token)
{
    ...
    stable_video_decoder_client_remote_->OnVideoFrameDecoded(
        MediaVideoFrameToMojoVideoFrame(std::move(frame), frame_registry_), can_read_without_stalling, *release_token);
}
