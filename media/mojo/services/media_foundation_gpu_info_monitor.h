...
public:
    ...
    MediaFoundationGpuInfoMonitor operator=(const MediaFoundationGpuInfoMonitor&) = delete;
    ~MediaFoundationGpuInfoMonitor();

    // Updates GpuInfo in `this` and notify observers if needed.
    void UpdateGpuInfo(const gpu::GPUInfo& gpu_info);

    CHROME_LUID gpu_luid() const
    {
        return gpu_luid_;
    }
    ...
    base::CallbackListSubscription AddLuidObserver(LuidObservers::CallbackType cb);
