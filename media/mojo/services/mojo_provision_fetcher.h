...
public:
    explicit MojoProvisionFetcher(mojo::PendingRemote<mojom::ProvisionFetcher> provision_fetcher);
    ...
    void Retrieve(const GURL& default_url, const std::string& request_data, ResponseCB response_cb) final;

private:
    // Callback for mojo::Remote<mojom::ProvisionFetcher>::Retrieve().
    void OnResponse(ResponseCB response_cb, bool success, const std::string& response);
