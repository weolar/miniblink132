...
drm::ProvisioningRequest CreateProvisioningRequest(fidl::StringPtr default_provisioning_server_url, const std::string& message)
{
    drm::ProvisioningRequest request;
    request.default_provisioning_server_url = std::move(default_provisioning_server_url);
...
TEST_F(FuchsiaCdmProvisioningFetcherImplTest, Fetch)
{
    FuchsiaCdmProvisioningFetcherImpl fetcher(base::BindLambdaForTesting([]() {
        ...
        EXPECT_CALL(*mock_provision_fetcher, Retrieve(Eq(kTestDefaultUrl), Eq(kTestRequest), _))
            .WillOnce(WithArgs<2>(Invoke([](ProvisionFetcher::ResponseCB response_callback) { std::move(response_callback).Run(true, kTestResponse); })));
    ...
    fetcher.Fetch(CreateProvisioningRequest(kTestDefaultUrl, kTestRequest),
        [&](drm::ProvisioningResponse response) { response_message = base::StringFromMemBuffer(response.message); });
...
TEST_F(FuchsiaCdmProvisioningFetcherImplTest, RetrieveFails)
{
    FuchsiaCdmProvisioningFetcherImpl fetcher(base::BindLambdaForTesting([]() {
        auto mock_provision_fetcher = std::make_unique<MockProvisionFetcher>();
        EXPECT_CALL(*mock_provision_fetcher, Retrieve(_, _, _)).WillOnce(WithArgs<2>(Invoke([](ProvisionFetcher::ResponseCB response_callback) {
...
TEST_F(FuchsiaCdmProvisioningFetcherImplTest, NoDefaultProvisioningUrl)
{
    ...
    fetcher.Fetch(CreateProvisioningRequest({}, kTestRequest), [](drm::ProvisioningResponse response) { FAIL(); });
}
...
TEST_F(FuchsiaCdmProvisioningFetcherImplTest, MultipleRequestsFails)
{
    ...
    fetcher.Fetch(CreateProvisioningRequest(kTestDefaultUrl, kTestRequest), [](drm::ProvisioningResponse) { FAIL(); });
    fetcher.Fetch(CreateProvisioningRequest(kTestDefaultUrl, kTestRequest), [](drm::ProvisioningResponse) { FAIL(); });
}
