...
public:
    ...
    void AppendVideoStats(const VideoDescKey& key, const VideoStats& new_stats, AppendVideoStatsCB append_done_cb) override
    {
        ...
        if (entries_.find(key_str) == entries_.end()) {
            ...
            known_entry.insert(known_entry.begin(), VideoStats(new_stats.frames_processed, new_stats.key_frames_processed, new_stats.p99_processing_time_ms));
        }

        std::move(append_done_cb).Run(true);
    }

    void GetVideoStats(const VideoDescKey& key, GetVideoStatsCB get_stats_cb) override
    {
        if (fail_next_get_) {
    ...
    void GetVideoStatsCollection(const VideoDescKey& key, GetVideoStatsCollectionCB get_stats_cb) override
    {
        if (fail_next_get_) {
            fail_next_get_ = false;
            std::move(get_stats_cb).Run(false, std::nullopt);
...
public:
    void SetUp() override
    {
        perf_history_ = std::make_unique<WebrtcVideoPerfHistory>(std::make_unique<FakeWebrtcVideoStatsDB>());
    }
    ...
    void SavePerfRecord(mojom::WebrtcPredictionFeatures features, mojom::WebrtcVideoStats video_stats, bool expect_callback)
    {
        base::OnceClosure save_done_cb = base::BindOnce(&WebrtcVideoPerfHistoryTest::MockSaveDoneCB, base::Unretained(this));
        if (expect_callback) {
            EXPECT_CALL(*this, MockSaveDoneCB());
        }
        perf_history_->GetSaveCallback().Run(features, video_stats, std::move(save_done_cb));
    }
    void SavePerfRecord(mojom::WebrtcPredictionFeatures features, mojom::WebrtcVideoStats video_stats)
    {
...
// test start.
class WebrtcVideoPerfHistoryParamTest : public testing::WithParamInterface<WebrtcPerfHistoryTestParams>, public WebrtcVideoPerfHistoryTest { };
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfo)
{
    ...
    // Add the entries.
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt30Hz));

    // Verify perf history returns is_smooth = true for HD at 30 and 60 fps.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        /*frames_per_second=*/30, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));

    // Verify perf history returns is_smooth = true for Full HD at 30 fps and
    // is_smooth = false for Full HD at 60 fps.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
        /*frames_per_second=*/30, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    ...
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kUnknownProfile, kPixelsFullHd, kSoftware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
...
// trial parameter.
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoSmoothnessThresholdOverride)
{
    // NOTE: When the DB initialization is deferred, All EXPECT_CALLs are then
    // delayed until we db_->CompleteInitialize(). testing::InSequence enforces
    // that EXPECT_CALLs arrive in top-to-bottom order.
    ...
    field_trial_params["smoothness_threshold_decode"] = base::NumberToString(kNewSmoothnessThresholdDecode);
    field_trial_params["smoothness_threshold_encode"] = base::NumberToString(kNewSmoothnessThresholdEncode);
    scoped_feature_list.InitAndEnableFeatureWithParameters(media::kWebrtcMediaCapabilitiesParameters, field_trial_params);
    ...
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed,
            /*p99_processing_time_ms=*/1.01 * kNewSmoothnessThresholdDecode * 1000.0 / kFramesPerSecond));
    // Add an HD decode entry just below the new threshold.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed,
            /*p99_processing_time_ms=*/0.99 * kNewSmoothnessThresholdDecode * 1000.0 / kFramesPerSecond));

    // Verify that Full HD is not smooth and HD is smooth.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), kFramesPerSecond,
        base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));

    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), kFramesPerSecond,
        base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    ...
    SavePerfRecord(Features(kIsEncode, kKnownProfile, kPixelsFullHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed,
            /*p99_processing_time_ms=*/1.01 * kNewSmoothnessThresholdEncode * 1000.0 / kFramesPerSecond));
    // Add an HD encode entry just below the new threshold.
    SavePerfRecord(Features(kIsEncode, kKnownProfile, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed,
            /*p99_processing_time_ms=*/0.99 * kNewSmoothnessThresholdEncode * 1000.0 / kFramesPerSecond));

    // Verify that Full HD is not smooth and HD is smooth.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsEncode, kKnownProfile, kPixelsFullHd, kSoftware), kFramesPerSecond,
        base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));

    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsEncode, kKnownProfile, kPixelsHd, kSoftware), kFramesPerSecond,
        base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoCombinedPrediction)
{
    ...
    const int kNotSmoothEntries = kNumberOfEntries * (1 - kSmoothDecisionRatioThreshold);
    ...
    for (int i = 0; i < kSmoothEntries - 1; ++i) {
        // Add the entries.
        SavePerfRecord(
            Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    }
    // Verify perf history returns is_smooth = false at 60 fps.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoSmoothDecisionRatioThresholdOverride)
{
    // NOTE: When the DB initialization is deferred, All EXPECT_CALLs are then
    // delayed until we db_->CompleteInitialize(). testing::InSequence enforces
    // that EXPECT_CALLs arrive in top-to-bottom order.
    ...
    float previousSmoothDecisionRatioThreshold = GetSmoothDecisionRatioThreshold();
    ...
    base::FieldTrialParams field_trial_params;
    field_trial_params["smooth_decision_ratio_threshold"] = base::NumberToString(kNewSmoothDecisionRatioThreshold);
    scoped_feature_list.InitAndEnableFeatureWithParameters(media::kWebrtcMediaCapabilitiesParameters, field_trial_params);
    EXPECT_NE(kNewSmoothDecisionRatioThreshold, previousSmoothDecisionRatioThreshold);
    EXPECT_EQ(kNewSmoothDecisionRatioThreshold, GetSmoothDecisionRatioThreshold());
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoFrameRateBucketing)
{
    ...
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt20Hz));
    ...
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        /*frames_per_second=*/120, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    ...
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
        /*frames_per_second=*/20, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware),
        /*frames_per_second=*/30, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, OnlySaveValidKeysAndStats)
{
    ...
    // Unknown profile.
    SavePerfRecord(Features(kIsDecode, VideoCodecProfile::VIDEO_CODEC_PROFILE_UNKNOWN, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Out of bounds profile.
    SavePerfRecord(Features(kIsDecode, static_cast<VideoCodecProfile>(1000), kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Untracked codec profile.
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE5, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Invalid pixels.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, /*video_pixels=*/100, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    SavePerfRecord(Features(kIsDecode, kKnownProfile, /*video_pixels=*/1e9, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Too few frames processed.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        VideoStats(/*frames_processed=*/10, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Too many frames processed.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        VideoStats(/*frames_processed=*/1e6, kKeyFramesProcessed, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
    // Key frames higher than frames_processed.
    SavePerfRecord(Features(kIsDecode, kKnownProfile, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed,
            /*key_frames_processed=*/kFramesProcessed + 1, kValidP99ProcessingTimeMs),
        /*expect_callback=*/false);
...
TEST_P(WebrtcVideoPerfHistoryParamTest, SmoothIsTrueForUntrackedCodecProfiles)
{
    ...
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE5, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt60Hz),
        /*expect_callback=*/false);
    SavePerfRecord(Features(kIsDecode, DOLBYVISION_PROFILE8, kPixelsHd, kSoftware),
        VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt60Hz),
        /*expect_callback=*/false);

    // Verify perf history returns is_smooth = true anyway.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, DOLBYVISION_PROFILE5, kPixelsHd, kSoftware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsDecode, DOLBYVISION_PROFILE8, kPixelsHd, kSoftware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, AppendAndDestroyStats)
{
    ...
    // Add the entries.
    SavePerfRecord(
        Features(kIsEncode, kKnownProfile, kPixelsFullHd, kHardware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt30Hz));

    // Verify its there before we ClearHistory(). Note that perf is NOT smooth at
    // 60 fps.
    EXPECT_CALL(*this, MockGetPerfInfoCB(kIsNotSmooth));
    perf_history_->GetPerfInfo(Features::New(kIsEncode, kKnownProfile, kPixelsFullHd, kHardware),
        /*frames_per_second=*/60, base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));

    // Initiate async clearing of history.
    EXPECT_CALL(*this, MockOnClearedHistory());
    perf_history_->ClearHistory(base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockOnClearedHistory, base::Unretained(this)));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetWebrtcVideoStatsDB)
{
    ...
    EXPECT_CALL(*this, MockGetWebrtcVideoStatsDBCB(_)).WillOnce([&](const auto* db_ptr) {
    ...
    perf_history_->GetWebrtcVideoStatsDB(base::BindOnce(&WebrtcVideoPerfHistoryTest::MockGetWebrtcVideoStatsDBCB, base::Unretained(this)));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfo4KSmoothImpliesHdSmooth)
{
    ...
    constexpr float kP99ProcessingTimeMsSmoothAt60Hz = 12.0f;
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixels4K, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsSmoothAt60Hz));
    ...
    for (auto pixels : kPixels) {
        for (auto framerate : kFramerates) {
            EXPECT_CALL(*this, MockGetPerfInfoCB(kIsSmooth));
            perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, pixels, kSoftware), framerate,
                base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
        }
    }
    ...
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixelsFullHd, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt30Hz));
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoFullHdNotSmoothImplies4KNotSmooth)
{
    // NOTE: When the DB initialization is deferred, All EXPECT_CALLs are then
    // delayed until we db_->CompleteInitialize(). testing::InSequence enforces
    // that EXPECT_CALLs arrive in top-to-bottom order.
    ...
    for (auto pixels : kPixels) {
        for (auto framerate : kFramerates) {
            EXPECT_CALL(*this, MockGetPerfInfoCB(pixels < 1920 * 1080 ? kIsSmooth : kIsNotSmooth));
            perf_history_->GetPerfInfo(Features::New(kIsDecode, kKnownProfile, pixels, kSoftware), framerate,
                base::BindOnce(&WebrtcVideoPerfHistoryParamTest::MockGetPerfInfoCB, base::Unretained(this)));
        }
    }
...
TEST_P(WebrtcVideoPerfHistoryParamTest, GetPerfInfoFullHdSmoothEvenIf4KNotSmooth)
{
    // NOTE: When the DB initialization is deferred, All EXPECT_CALLs are then
    // delayed until we db_->CompleteInitialize(). testing::InSequence enforces
    // that EXPECT_CALLs arrive in top-to-bottom order.
    ...
    constexpr float kP99ProcessingTimeMsNotSmoothAt30Hz = 60.0f;
    SavePerfRecord(
        Features(kIsDecode, kKnownProfile, kPixels4K, kSoftware), VideoStats(kFramesProcessed, kKeyFramesProcessed, kP99ProcessingTimeMsNotSmoothAt30Hz));
...
INSTANTIATE_TEST_SUITE_P(VaryDBInitTiming, WebrtcVideoPerfHistoryParamTest, ::testing::ValuesIn(kWebrtcPerfHistoryTestParams));
