...
public:
    WatchTimeRecorder(mojom::PlaybackPropertiesPtr properties, ukm::SourceId source_id, bool is_top_frame, uint64_t player_id);
    ...
    void UpdateSecondaryProperties(mojom::SecondaryPlaybackPropertiesPtr secondary_properties) override;
...
private:
    ...
    struct ExtendedMetricsKeyMap {
        ...
        ExtendedMetricsKeyMap(WatchTimeKey watch_time_key, std::string_view mtbr_key, std::string_view smooth_rate_key, std::string_view discard_key);
    ...
    struct WatchTimeUkmRecord {
        explicit WatchTimeUkmRecord(mojom::SecondaryPlaybackPropertiesPtr properties);
