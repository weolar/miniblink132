...
class MEDIA_MOJO_EXPORT MojoCdmService final : public mojom::ContentDecryptionModule {
public:
    ...
    // reason.
    using InitializeCB = base::OnceCallback<void(mojom::CdmContextPtr cdm_context, CreateCdmStatus status)>;
    ...
    void Initialize(CdmFactory* cdm_factory, const CdmConfig& cdm_config, InitializeCB init_cb);

    // mojom::ContentDecryptionModule implementation.
    void SetClient(mojo::PendingAssociatedRemote<mojom::ContentDecryptionModuleClient> client) final;
    ...
    void LoadSession(CdmSessionType session_type, const std::string& session_id, LoadSessionCallback callback) final;
    ...
    base::UnguessableToken cdm_id() const
    {
        return cdm_id_.value();
    }

private:
    // Callback for CdmFactory::Create().
    void OnCdmCreated(InitializeCB callback, const scoped_refptr<::media::ContentDecryptionModule>& cdm, CreateCdmStatus status);
