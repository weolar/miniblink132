...
MojoDataPipeReader::MojoDataPipeReader(mojo::ScopedDataPipeConsumerHandle consumer_handle)
    ...
    , pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault())
{
    ...
    MojoResult result = pipe_watcher_.Watch(
        consumer_handle_.get(), MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, base::BindRepeating(&MojoDataPipeReader::TryReadData, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
        DVLOG(1) << __func__ << ": Failed to start watching the pipe. result=" << result;
...
void MojoDataPipeReader::Read(uint8_t* buffer, uint32_t num_bytes, DoneCB done_cb)
{
    DVLOG(3) << __func__;
...
void MojoDataPipeReader::TryReadData(MojoResult result)
{
    ...
    if (current_buffer_) {
        ...
        base::span<uint8_t> buffer = UNSAFE_BUFFERS(base::span(current_buffer_.get(), current_buffer_size_));
        buffer = buffer.subspan(bytes_read_);
        result = consumer_handle_->ReadData(MOJO_READ_DATA_FLAG_NONE, buffer, num_bytes);
    } else {
...
MojoDataPipeWriter::MojoDataPipeWriter(mojo::ScopedDataPipeProducerHandle producer_handle)
    ...
    , pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault())
{
    ...
    MojoResult result = pipe_watcher_.Watch(
        producer_handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, base::BindRepeating(&MojoDataPipeWriter::TryWriteData, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
        DVLOG(1) << __func__ << ": Failed to start watching the pipe. result=" << result;
...
void MojoDataPipeWriter::Write(const uint8_t* buffer, uint32_t buffer_size, DoneCB done_cb)
{
    DVLOG(3) << __func__;
    ...
    if (!producer_handle_.is_valid()) {
        DVLOG(1) << __func__ << ": Failed to write buffer because the pipe is already closed";
        std::move(done_cb).Run(false);
        return;
    }

    // TODO(lukasza): Take `span` instead of `buffer` + `buffer_size`.
    current_buffer_ = UNSAFE_TODO(base::span<const uint8_t>(buffer, size_t { buffer_size }));
    done_cb_ = std::move(done_cb);
...
void MojoDataPipeWriter::TryWriteData(MojoResult result)
{
    ...
    result = producer_handle_->WriteData(current_buffer_, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    if (IsPipeReadWriteError(result)) {
        OnPipeError(result);
    } else {
        if (result == MOJO_RESULT_OK) {
...
void MojoDataPipeWriter::OnPipeError(MojoResult result)
{
    ...
    if (!current_buffer_.empty()) {
        DVLOG(1) << __func__ << ": writing to data pipe failed. result=" << result << ", current_buffer_.size()=" << current_buffer_.size();
        current_buffer_ = base::span<const uint8_t>();
        DCHECK(done_cb_);
        std::move(done_cb_).Run(false);
    }
