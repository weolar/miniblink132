...
// static
media::mojom::DecryptConfigPtr TypeConverter<media::mojom::DecryptConfigPtr, media::DecryptConfig>::Convert(const media::DecryptConfig& input)
{
    media::mojom::DecryptConfigPtr mojo_decrypt_config(media::mojom::DecryptConfig::New());
...
// static
std::unique_ptr<media::DecryptConfig> TypeConverter<std::unique_ptr<media::DecryptConfig>, media::mojom::DecryptConfigPtr>::Convert(
    const media::mojom::DecryptConfigPtr& input)
{
    return std::make_unique<media::DecryptConfig>(input->encryption_scheme, input->key_id, input->iv, input->subsamples, input->encryption_pattern);
}

// static
media::mojom::DecoderBufferSideDataPtr TypeConverter<media::mojom::DecoderBufferSideDataPtr, media::DecoderBufferSideData>::Convert(
    const media::DecoderBufferSideData& input)
{
    media::mojom::DecoderBufferSideDataPtr mojo_side_data(media::mojom::DecoderBufferSideData::New());
    if (!input.alpha_data.empty()) {
        mojo_side_data->alpha_data.assign(input.alpha_data.begin(), input.alpha_data.end());
    }
...
// static
std::unique_ptr<media::DecoderBufferSideData> TypeConverter<std::unique_ptr<media::DecoderBufferSideData>, media::mojom::DecoderBufferSideDataPtr>::Convert(
    const media::mojom::DecoderBufferSideDataPtr& input)
{
    ...
    if (!input->alpha_data.empty()) {
        side_data->alpha_data = base::HeapArray<uint8_t>::CopiedFrom(input->alpha_data);
    }
...
// static
media::mojom::DecoderBufferPtr TypeConverter<media::mojom::DecoderBufferPtr, media::DecoderBuffer>::Convert(const media::DecoderBuffer& input)
{
    if (input.end_of_stream()) {
        auto eos = media::mojom::EosDecoderBuffer::New();
        if (input.next_config()) {
            const auto next_config = *input.next_config();
            if (const auto* ac = absl::get_if<media::AudioDecoderConfig>(&next_config)) {
                eos->next_config = media::mojom::DecoderBufferSideDataNextConfig::NewNextAudioConfig(*ac);
            } else {
                eos->next_config = media::mojom::DecoderBufferSideDataNextConfig::NewNextVideoConfig(absl::get<media::VideoDecoderConfig>(next_config));
            }
        }
    ...
    if (input.has_side_data()) {
        data_buffer->side_data = media::mojom::DecoderBufferSideData::From(*input.side_data());
    }

    if (input.decrypt_config()) {
        data_buffer->decrypt_config = media::mojom::DecryptConfig::From(*input.decrypt_config());
    }
...
// static
scoped_refptr<media::DecoderBuffer> TypeConverter<scoped_refptr<media::DecoderBuffer>, media::mojom::DecoderBufferPtr>::Convert(
    const media::mojom::DecoderBufferPtr& input)
{
    if (input->is_eos()) {
        ...
        if (eos_buffer->next_config) {
            if (eos_buffer->next_config->is_next_audio_config()) {
                return media::DecoderBuffer::CreateEOSBuffer(eos_buffer->next_config->get_next_audio_config());
            } else if (eos_buffer->next_config->is_next_video_config()) {
                return media::DecoderBuffer::CreateEOSBuffer(eos_buffer->next_config->get_next_video_config());
            }
        }
    ...
    auto buffer = base::MakeRefCounted<media::DecoderBuffer>(base::strict_cast<size_t>(mojo_buffer->data_size));

    if (mojo_buffer->side_data) {
        buffer->set_side_data(mojo_buffer->side_data.To<std::unique_ptr<media::DecoderBufferSideData>>());
    }
    ...
    if (mojo_buffer->decrypt_config) {
        buffer->set_decrypt_config(mojo_buffer->decrypt_config.To<std::unique_ptr<media::DecryptConfig>>());
    }
...
// static
media::mojom::AudioBufferPtr TypeConverter<media::mojom::AudioBufferPtr, media::AudioBuffer>::Convert(const media::AudioBuffer& input)
{
...
// static
scoped_refptr<media::AudioBuffer> TypeConverter<scoped_refptr<media::AudioBuffer>, media::mojom::AudioBufferPtr>::Convert(
    const media::mojom::AudioBufferPtr& input)
{
    if (input->end_of_stream)
        return media::AudioBuffer::CreateEOSBuffer();

    if (input->frame_count <= 0 || static_cast<size_t>(input->sample_format) > media::kSampleFormatMax
        || static_cast<size_t>(input->channel_layout) > media::CHANNEL_LAYOUT_MAX
        || ChannelLayoutToChannelCount(input->channel_layout) != input->channel_count) {
    ...
    if (IsBitstream(input->sample_format)) {
        ...
        return media::AudioBuffer::CopyBitstreamFrom(input->sample_format, input->channel_layout, input->channel_count, input->sample_rate, input->frame_count,
            &data, input->data.size(), input->timestamp);
    }
    ...
    return media::AudioBuffer::CopyFrom(
        input->sample_format, input->channel_layout, input->channel_count, input->sample_rate, input->frame_count, &channel_ptrs[0], input->timestamp);
}
