...
mojom::AudioDataS16Ptr AudioDataS16Converter::ConvertToAudioDataS16(scoped_refptr<AudioBuffer> buffer, bool is_multichannel_supported)
{
    ...
    // not supported.
    if (buffer->sample_format() == SampleFormat::kSampleFormatS16 && (buffer->channel_count() == 1 || is_multichannel_supported)) {
        ...
        signed_buffer->data.assign(audio_data, audio_data + buffer->frame_count() * buffer->channel_count());
        return signed_buffer;
    }

    CopyBufferToTempAudioBus(*buffer);
    return ConvertToAudioDataS16(*temp_audio_bus_, buffer->sample_rate(), buffer->channel_layout(), is_multichannel_supported);
}

mojom::AudioDataS16Ptr AudioDataS16Converter::ConvertToAudioDataS16(
    std::unique_ptr<AudioBus> audio_bus, int sample_rate, ChannelLayout channel_layout, bool is_multichannel_supported)
{
    DCHECK_GT(audio_bus->frames(), 0);
    DCHECK_GT(audio_bus->channels(), 0);
    return ConvertToAudioDataS16(*audio_bus, sample_rate, channel_layout, is_multichannel_supported);
}
    ...
    const AudioBus& audio_bus, int sample_rate, ChannelLayout channel_layout, bool is_multichannel_supported)
{
    ...
    if (audio_bus.channels() > 1 && !is_multichannel_supported) {
        ...
        monaural_audio_bus_->ToInterleaved<SignedInt16SampleTypeTraits>(monaural_audio_bus_->frames(), &signed_buffer->data[0]);

        return signed_buffer;
    }

    signed_buffer->data.resize(audio_bus.frames() * audio_bus.channels());
    audio_bus.ToInterleaved<SignedInt16SampleTypeTraits>(audio_bus.frames(), &signed_buffer->data[0]);

    return signed_buffer;
}

void AudioDataS16Converter::CopyBufferToTempAudioBus(const AudioBuffer& buffer)
{
    if (!temp_audio_bus_ || buffer.channel_count() != temp_audio_bus_->channels() || buffer.frame_count() != temp_audio_bus_->frames()) {
        temp_audio_bus_ = AudioBus::Create(buffer.channel_count(), buffer.frame_count());
    }

    buffer.ReadFrames(buffer.frame_count(),
        /*source_frame_offset*/ 0, /*dest_frame_offset*/ 0, temp_audio_bus_.get());
}

void AudioDataS16Converter::ResetChannelMixerIfNeeded(int frame_count, ChannelLayout channel_layout, int channel_count)
{
    ...
    if (channel_layout != channel_layout_ || channel_count != channel_count_) {
        ...
        channel_mixer_ = std::make_unique<ChannelMixer>(channel_layout, channel_count, CHANNEL_LAYOUT_MONO, 1 /*output_channels*/);
    }
}
