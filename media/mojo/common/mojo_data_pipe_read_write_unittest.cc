...
public:
    MojoDataPipeReadWrite(uint32_t data_pipe_capacity_bytes = kDefaultDataPipeCapacityBytes)
    {
        mojo::ScopedDataPipeProducerHandle producer_handle;
        mojo::ScopedDataPipeConsumerHandle consumer_handle;
        CHECK_EQ(mojo::CreateDataPipe(data_pipe_capacity_bytes, producer_handle, consumer_handle), MOJO_RESULT_OK);
    ...
    void WriteAndRead(const uint8_t* buffer, uint32_t buffer_size, bool discard_data = false)
    {
...
TEST(MojoDataPipeReadWriteTest, Normal)
{
    ...
    pipe_read_write_.WriteAndRead(reinterpret_cast<const uint8_t*>(kData.data()), kData.size());
}
...
TEST(MojoDataPipeReadWriteTest, SequentialReading)
{
    ...
    MojoDataPipeReadWrite pipe_read_write_;
    pipe_read_write_.WriteAndRead(reinterpret_cast<const uint8_t*>(kData1.data()), kData1.size());
    pipe_read_write_.WriteAndRead(reinterpret_cast<const uint8_t*>(kData2.data()), kData2.size());
}
...
TEST(MojoDataPipeReadWriteTest, DiscardDataInPipe)
{
    ...
    MojoDataPipeReadWrite pipe_read_write_(10);
    pipe_read_write_.WriteAndRead(reinterpret_cast<const uint8_t*>(kData1.data()), kData1.size(), true);
    pipe_read_write_.WriteAndRead(reinterpret_cast<const uint8_t*>(kData2.data()), kData2.size());
}
