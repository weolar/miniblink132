...
void CompareAudioBuffers(SampleFormat sample_format, const AudioBuffer& original, const AudioBuffer& result)
{
    ...
    if (IsPlanar(sample_format)) {
        for (int i = 0; i < original.channel_count(); ++i) {
            CompareBytes(original.channel_data()[i], result.channel_data()[i], bytes_per_channel);
        }
        return;
    }

    DCHECK(IsInterleaved(sample_format)) << sample_format;
    CompareBytes(original.channel_data()[0], result.channel_data()[0], bytes_per_channel * original.channel_count());
}
...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_Normal)
{
    ...
    buffer->WritableSideData().spatial_layers.assign(kSpatialLayers, kSpatialLayers + kSpatialLayersSize);
...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_EOS_Video_NextConfig)
{
    ...
    EXPECT_TRUE(absl::get<VideoDecoderConfig>(*result->next_config()).Matches(TestVideoConfig::Normal()));
}
...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_EOS_Audio_NextConfig)
{
    ...
    EXPECT_TRUE(result->end_of_stream());
    ASSERT_TRUE(result->next_config());
    EXPECT_TRUE(absl::get<AudioDecoderConfig>(*result->next_config()).Matches(TestAudioConfig::Normal()));
}
...
TEST(MediaTypeConvertersTest, ConvertDecoderBuffer_CbcsEncryptedBuffer)
{
    ...
    // Original.
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::CopyFrom(kData));
    buffer->set_decrypt_config(DecryptConfig::CreateCbcsConfig(kKeyId, kIv, subsamples, pattern));
