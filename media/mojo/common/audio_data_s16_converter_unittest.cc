...
static const int16_t kTestVectorContents[kTestVectorSize] = { INT16_MIN, 0, INT16_MAX, INT16_MIN, INT16_MAX / 2, INT16_MIN / 2, 0, INT16_MAX, 0, 0 };
static const int16_t kExpectedMixedVectorContents[kTestVectorSize / 2] = { INT16_MIN / 2, 0, 0, INT16_MAX / 2, 0 };
...
TEST_F(AudioDataS16ConverterTest, ConvertToAudioDataS16_MONO)
{
    ...
    audio_bus->FromInterleaved<SignedInt16SampleTypeTraits>(kTestVectorContents, kTestVectorSize);

    // Convert.
    mojom::AudioDataS16Ptr result = converter_->ConvertToAudioDataS16(*audio_bus, kSampleRate, CHANNEL_LAYOUT_MONO, false);
...
TEST_F(AudioDataS16ConverterTest, ConvertToAudioDataS16_STEREO)
{
    ...
    audio_bus->FromInterleaved<SignedInt16SampleTypeTraits>(kTestVectorContents, kTestVectorSize / 2);

    // Mix and convert.
    mojom::AudioDataS16Ptr result = converter_->ConvertToAudioDataS16(*audio_bus, kSampleRate, CHANNEL_LAYOUT_STEREO, false);
