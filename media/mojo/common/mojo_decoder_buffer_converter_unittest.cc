...
public:
    MojoDecoderBufferConverter(uint32_t data_pipe_capacity_bytes = kDefaultDataPipeCapacityBytes)
    {
        mojo::ScopedDataPipeProducerHandle producer_handle;
        mojo::ScopedDataPipeConsumerHandle consumer_handle;
        EXPECT_TRUE(CreateDataPipe(data_pipe_capacity_bytes, &producer_handle, &consumer_handle));
        writer = std::make_unique<MojoDecoderBufferWriter>(std::move(producer_handle));
        reader = std::make_unique<MojoDecoderBufferReader>(std::move(consumer_handle));
    }
    ...
    void ConvertAndVerify(scoped_refptr<DecoderBuffer> media_buffer)
    {
        ...
        EXPECT_CALL(mock_cb, Run(MatchesDecoderBuffer(media_buffer))).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
...
TEST(MojoDecoderBufferConverterTest, ConvertDecoderBuffer_Normal)
{
    ...
    buffer->set_discard_padding(DecoderBuffer::DiscardPadding(base::Milliseconds(5), base::Milliseconds(6)));
...
TEST(MojoDecoderBufferConverterTest, ConvertDecoderBuffer_EncryptedBuffer)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::CopyFrom(kData));
    buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(kKeyId, kIv, subsamples));
    {
        MojoDecoderBufferConverter converter;
        converter.ConvertAndVerify(buffer);
    }
    ...
    // Test 'cbcs'.
    buffer->set_decrypt_config(DecryptConfig::CreateCbcsConfig(kKeyId, kIv, subsamples, EncryptionPattern(5, 6)));
    {
        MojoDecoderBufferConverter converter;
        converter.ConvertAndVerify(buffer);
    }
...
TEST(MojoDecoderBufferConverterTest, WriterSidePipeError)
{
    ...
    base::RunLoop run_loop;
    base::MockCallback<MojoDecoderBufferReader::ReadCB> mock_cb;
    EXPECT_CALL(mock_cb, Run(testing::IsNull())).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
...
TEST(MojoDecoderBufferConverterTest, ConcurrentDecoderBuffers)
{
    ...
    EXPECT_CALL(mock_cb3, Run(MatchesDecoderBuffer(media_buffer3))).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));

    // Write all of the buffers at once.
    mojom::DecoderBufferPtr mojo_buffer1 = converter.writer->WriteDecoderBuffer(media_buffer1);
    mojom::DecoderBufferPtr mojo_buffer2 = converter.writer->WriteDecoderBuffer(media_buffer2);
    mojom::DecoderBufferPtr mojo_buffer3 = converter.writer->WriteDecoderBuffer(media_buffer3);
...
TEST(MojoDecoderBufferConverterTest, FlushAfterRead)
{
    ...
    base::MockCallback<base::OnceClosure> mock_flush_cb;
    EXPECT_CALL(mock_flush_cb, Run()).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
...
TEST(MojoDecoderBufferConverterTest, FlushBeforeRead)
{
    ...
    EXPECT_CALL(mock_flush_cb, Run());
    EXPECT_CALL(mock_read_cb, Run(MatchesDecoderBuffer(media_buffer))).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));

    // Write, Flush, then Read.
    mojom::DecoderBufferPtr mojo_buffer = converter.writer->WriteDecoderBuffer(media_buffer);
    converter.reader->Flush(mock_flush_cb.Get());
    converter.reader->ReadDecoderBuffer(std::move(mojo_buffer), mock_read_cb.Get());
    run_loop.Run();
}
...
TEST(MojoDecoderBufferConverterTest, WriterWithInvalidHandle)
{
    ...
    auto writer = std::make_unique<MojoDecoderBufferWriter>(mojo::ScopedDataPipeProducerHandle());
...
TEST(MojoDecoderBufferConverterTest, ReaderWithInvalidHandle)
{
    ...
    base::MockCallback<MojoDecoderBufferReader::ReadCB> mock_cb;
    EXPECT_CALL(mock_cb, Run(testing::IsNull())).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
    auto reader = std::make_unique<MojoDecoderBufferReader>(mojo::ScopedDataPipeConsumerHandle());
    reader->ReadDecoderBuffer(std::move(mojo_buffer), mock_cb.Get());
