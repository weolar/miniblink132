...
bool CreateDataPipe(uint32_t capacity, mojo::ScopedDataPipeProducerHandle* producer_handle, mojo::ScopedDataPipeConsumerHandle* consumer_handle)
{
    ...
    auto result = mojo::CreateDataPipe(&options, *producer_handle, *consumer_handle);
...
// static
std::unique_ptr<MojoDecoderBufferReader> MojoDecoderBufferReader::Create(uint32_t capacity, mojo::ScopedDataPipeProducerHandle* producer_handle)
{
    DVLOG(1) << __func__;
...
MojoDecoderBufferReader::MojoDecoderBufferReader(mojo::ScopedDataPipeConsumerHandle consumer_handle)
    : consumer_handle_(std::move(consumer_handle))
    , pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault())
    , armed_(false)
    , bytes_read_(0)
{
    DVLOG(1) << __func__;
    ...
    MojoResult result = pipe_watcher_.Watch(consumer_handle_.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED,
        base::BindRepeating(&MojoDecoderBufferReader::OnPipeReadable, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
        DLOG(ERROR) << __func__ << ": Failed to start watching the pipe. result=" << result;
...
void MojoDecoderBufferReader::ReadDecoderBuffer(mojom::DecoderBufferPtr mojo_buffer, ReadCB read_cb)
{
    DVLOG(3) << __func__;
    DCHECK(!flush_cb_);
    ...
    scoped_refptr<DecoderBuffer> media_buffer(mojo_buffer.To<scoped_refptr<DecoderBuffer>>());
    DCHECK(media_buffer);

    if (MediaTraceIsEnabled() && !media_buffer->end_of_stream()) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "MojoDecoderBufferReader::Read", media_buffer->timestamp().InMicroseconds());
        read_cb = base::BindOnce(
            [](ReadCB read_cb, scoped_refptr<DecoderBuffer> buffer) {
                TRACE_EVENT_NESTABLE_ASYNC_END2("media,gpu", "MojoDecoderBufferReader::Read", buffer->timestamp().InMicroseconds(), "timestamp",
                    buffer->timestamp().InMicroseconds(), "read_bytes", buffer->size());
...
void MojoDecoderBufferReader::OnPipeReadable(MojoResult result, const mojo::HandleSignalsState& state)
{
...
void MojoDecoderBufferReader::ProcessPendingReads()
{
    ...
    while (!pending_buffers_.empty()) {
        ...
        MojoResult result = consumer_handle_->ReadData(MOJO_WRITE_DATA_FLAG_NONE,
...
void MojoDecoderBufferReader::OnPipeError(MojoResult result)
{
    ...
    if (!pending_buffers_.empty()) {
        DVLOG(1) << __func__ << ": reading from data pipe failed. result=" << result << ", buffer size=" << pending_buffers_.front()->size()
...
// static
std::unique_ptr<MojoDecoderBufferWriter> MojoDecoderBufferWriter::Create(uint32_t capacity, mojo::ScopedDataPipeConsumerHandle* consumer_handle)
{
    DVLOG(1) << __func__;
    DCHECK_GT(capacity, 0u);
...
MojoDecoderBufferWriter::MojoDecoderBufferWriter(mojo::ScopedDataPipeProducerHandle producer_handle)
    ...
    , bytes_written_(0)
{
    ...
    MojoResult result = pipe_watcher_.Watch(producer_handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED,
        base::BindRepeating(&MojoDecoderBufferWriter::OnPipeWritable, base::Unretained(this)));
    if (result != MOJO_RESULT_OK) {
        DLOG(ERROR) << __func__ << ": Failed to start watching the pipe. result=" << result;
...
mojom::DecoderBufferPtr MojoDecoderBufferWriter::WriteDecoderBuffer(scoped_refptr<DecoderBuffer> media_buffer)
{
    DVLOG(3) << __func__;

    // DecoderBuffer cannot be written if the pipe is already closed.
    if (!producer_handle_.is_valid()) {
        DVLOG(1) << __func__ << ": Failed to write DecoderBuffer because the pipe is already closed";
        return nullptr;
    }
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media,gpu", "MojoDecoderBufferWriter::Write", media_buffer->timestamp().InMicroseconds());
...
void MojoDecoderBufferWriter::OnPipeWritable(MojoResult result, const mojo::HandleSignalsState& state)
{
...
void MojoDecoderBufferWriter::ProcessPendingWrites()
{
    ...
    while (!pending_buffers_.empty()) {
        ...
        MojoResult result = producer_handle_->WriteData(bytes_to_write, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
        ...
        if (actually_written_bytes == bytes_to_write.size()) {
            TRACE_EVENT_NESTABLE_ASYNC_END2("media,gpu", "MojoDecoderBufferWriter::Write", buffer->timestamp().InMicroseconds(), "timestamp",
...
void MojoDecoderBufferWriter::OnPipeError(MojoResult result)
{
    ...
    if (!pending_buffers_.empty()) {
        DVLOG(1) << __func__ << ": writing to data pipe failed. result=" << result << ", buffer size=" << pending_buffers_.front()->size()
