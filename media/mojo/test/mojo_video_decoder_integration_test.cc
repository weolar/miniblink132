...
public:
    MockVideoDecoder()
    {
        ...
        EXPECT_CALL(*this, GetDecoderType()).WillRepeatedly(Return(VideoDecoderType::kTesting));
        EXPECT_CALL(*this, NeedsBitstreamConversion()).WillRepeatedly(Return(false));
        EXPECT_CALL(*this, CanReadWithoutStalling()).WillRepeatedly(Return(true));
        EXPECT_CALL(*this, GetMaxDecodeRequests()).WillRepeatedly(Return(kMaxDecodeRequests));

        // For regular methods, only configure a default action.
        ON_CALL(*this, Decode_(_, _)).WillByDefault(Invoke(this, &MockVideoDecoder::DoDecode));
        ON_CALL(*this, Reset_(_)).WillByDefault(Invoke(this, &MockVideoDecoder::DoReset));
    }
    ...
    void DoDecode(scoped_refptr<DecoderBuffer> buffer, DecodeCB& decode_cb)
    {
        if (!buffer->end_of_stream()) {
            ...
            if (buffer->decrypt_config()) {
                ...
                scoped_refptr<VideoFrame> frame = VideoFrame::WrapSharedImage(PIXEL_FORMAT_ARGB, shared_image, gpu::SyncToken(), GetReleaseMailboxCB(),
                    config_.coded_size(), config_.visible_rect(), config_.natural_size(), buffer->timestamp());
        ...
        // |decode_cb| must not be called from the same stack.
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kOk));
    }

    void DoReset(base::OnceClosure& reset_cb)
    {
        // |reset_cb| must not be called from the same stack.
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(reset_cb));
    }
...
public:
    using CreateVideoDecoderCB = base::RepeatingCallback<std::unique_ptr<VideoDecoder>(MediaLog*)>;
    ...
    std::unique_ptr<VideoDecoder> CreateVideoDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog* media_log,
        ...
        mojo::PendingRemote<stable::mojom::StableVideoDecoder> oop_video_decoder) override
    {
...
public:
    MojoVideoDecoderIntegrationTest()
        : mojo_media_client_(base::BindRepeating(&MojoVideoDecoderIntegrationTest::CreateVideoDecoder, base::Unretained(this)))
    {
    }

    MojoVideoDecoderIntegrationTest(const MojoVideoDecoderIntegrationTest&) = delete;
    MojoVideoDecoderIntegrationTest& operator=(const MojoVideoDecoderIntegrationTest&) = delete;
...
protected:
    ...
    mojo::PendingRemote<mojom::VideoDecoder> CreateRemoteVideoDecoder()
    {
        ...
        video_decoder_receivers_.Add(std::make_unique<MojoVideoDecoderService>(
                                         &mojo_media_client_, &mojo_cdm_service_context_, mojo::PendingRemote<stable::mojom::StableVideoDecoder>()),
    ...
    void CreateClient()
    {
        ...
        // be tested.
        client_ = std::make_unique<MojoVideoDecoder>(base::SingleThreadTaskRunner::GetCurrentDefault(), nullptr, &client_media_log_, CreateRemoteVideoDecoder(),
    ...
    bool Initialize()
    {
        ...
        EXPECT_CALL(*decoder_, DoInitialize(_)).WillOnce(RunOnceCallback<0>(DecoderStatus::Codes::kOk));
        ...
        client_->Initialize(TestVideoConfig::NormalH264(), false, nullptr, init_cb.Get(), output_cb_.Get(), waiting_cb_.Get());
        RunUntilIdle();

        return result.is_ok();
    }

    DecoderStatus Decode(scoped_refptr<DecoderBuffer> buffer, VideoFrame::ReleaseMailboxCB release_cb = VideoFrame::ReleaseMailboxCB())
    {
    ...
    // lifetime.
    std::unique_ptr<MockVideoDecoder> decoder_owner_ = std::make_unique<MockVideoDecoder>();
...
{
    mojo::Remote<mojom::VideoDecoder> remote_video_decoder(CreateRemoteVideoDecoder());
    StrictMock<base::MockCallback<mojom::VideoDecoder::GetSupportedConfigsCallback>> callback;
...
TEST_F(MojoVideoDecoderIntegrationTest, InitializeFailNoDecoder)
{
    ...
    StrictMock<base::MockCallback<VideoDecoder::InitCB>> init_cb;
    EXPECT_CALL(init_cb, Run(HasStatusCode(DecoderStatus::Codes::kFailedToCreateDecoder)));
...
TEST_F(MojoVideoDecoderIntegrationTest, InitializeFailNoCdm)
{
    ...
    StrictMock<base::MockCallback<VideoDecoder::InitCB>> init_cb;
    EXPECT_CALL(init_cb, Run(HasStatusCode(DecoderStatus::Codes::kUnsupportedEncryptionMode)));

    // CdmContext* (3rd parameter) is not provided but the VideoDecoderConfig
    // specifies encrypted video, so Initialize() should fail.
    client_->Initialize(TestVideoConfig::NormalEncrypted(), false, nullptr, init_cb.Get(), output_cb_.Get(), waiting_cb_.Get());
    RunUntilIdle();
}
...
TEST_F(MojoVideoDecoderIntegrationTest, Release)
{
    ...
    gpu::SyncToken release_sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId(),
        /*release_count=*/1u);
...
TEST_F(MojoVideoDecoderIntegrationTest, CanReadWithoutStallingAfterReset)
{
    ...
    EXPECT_CALL(*decoder_, CanReadWithoutStalling()).WillRepeatedly(Return(false));
