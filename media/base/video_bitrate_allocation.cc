...
static media::Bitrate MakeReplacementBitrate(const media::Bitrate& old, uint32_t target_bps, uint32_t peak_bps)
{
    switch (old.mode()) {
...
bool VideoBitrateAllocation::SetBitrate(size_t spatial_index, size_t temporal_index, uint32_t bitrate_bps)
{
    ...
    sum_bitrate_ = MakeReplacementBitrate(sum_bitrate_, new_sum_bps, new_peak_bps);
    bitrates_[spatial_index][temporal_index] = bitrate_bps;
    return true;
}

uint32_t VideoBitrateAllocation::GetBitrateBps(size_t spatial_index, size_t temporal_index) const
{
    CHECK_LT(spatial_index, kMaxSpatialLayers);
    CHECK_LT(temporal_index, kMaxTemporalLayers);
