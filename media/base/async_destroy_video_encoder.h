...
template <typename T> class AsyncDestroyVideoEncoder final : public VideoEncoder {
public:
    explicit AsyncDestroyVideoEncoder(std::unique_ptr<T> wrapped_encoder)
        : wrapped_encoder_(std::move(wrapped_encoder))
    {
        static_assert(std::is_base_of<VideoEncoder, T>::value, "T must implement 'media::VideoEncoder'");
    ...
    {
        DCHECK(wrapped_encoder_);
        wrapped_encoder_->Initialize(profile, options, std::move(info_cb), std::move(output_cb), std::move(done_cb));
    }

    void Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& options, EncoderStatusCB done_cb) override
    {
        DCHECK(wrapped_encoder_);
    ...
    {
        DCHECK(wrapped_encoder_);
        wrapped_encoder_->ChangeOptions(options, std::move(output_cb), std::move(done_cb));
    }
