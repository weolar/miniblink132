...
public:
    ...
    using WallClockTimeCB = base::RepeatingCallback<bool(const std::vector<base::TimeDelta>&, std::vector<base::TimeTicks>*)>;
    ...
    // clock time.
    virtual bool GetWallClockTimes(const std::vector<base::TimeDelta>& media_timestamps, std::vector<base::TimeTicks>* wall_clock_times) = 0;
};
