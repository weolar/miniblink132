...
public:
    ...
    VideoThumbnailDecoderTest& operator=(const VideoThumbnailDecoderTest&) = delete;
...
protected:
    void SetUp() override
    {
        ...
        VideoDecoderConfig valid_config(VideoCodec::kVP8, VP8PROFILE_ANY, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
            gfx::Size(1, 1), gfx::Rect(1, 1), gfx::Size(1, 1), EmptyExtraData(), EncryptionScheme::kUnencrypted);

        thumbnail_decoder_ = std::make_unique<VideoThumbnailDecoder>(std::move(mock_video_decoder), valid_config, std::vector<uint8_t> { 0u });
    }
    ...
    void Start()
    {
        thumbnail_decoder_->Start(base::BindOnce(&VideoThumbnailDecoderTest::OnFrameDecoded, base::Unretained(this)));
    ...
    scoped_refptr<VideoFrame> CreateFrame()
    {
        return VideoFrame::CreateZeroInitializedFrame(
            VideoPixelFormat::PIXEL_FORMAT_I420, gfx::Size(1, 1), gfx::Rect(1, 1), gfx::Size(1, 1), base::TimeDelta());
    }
    ...
    MockVideoDecoder* mock_video_decoder()
    {
        return mock_video_decoder_;
    }
...
TEST_F(VideoThumbnailDecoderTest, Success)
{
    ...
    EXPECT_CALL(*mock_video_decoder(), Initialize_(_, _, _, _, _, _))
        .WillOnce(DoAll(RunOnceCallback<3>(DecoderStatus::Codes::kOk), RunCallback<4>(expected_frame)));
    EXPECT_CALL(*mock_video_decoder(), Decode_(_, _)).Times(2).WillRepeatedly(base::test::RunOnceCallbackRepeatedly<1>(DecoderStatus::Codes::kOk));
...
TEST_F(VideoThumbnailDecoderTest, SuccessMultipleOutputs)
{
    ...
    EXPECT_CALL(*mock_video_decoder(), Initialize_(_, _, _, _, _, _))
        .WillOnce(DoAll(testing::SaveArg<4>(&output_cb), RunOnceCallback<3>(DecoderStatus::Codes::kOk)));
...
TEST_F(VideoThumbnailDecoderTest, SuccessOutputBeforeDecodeCBCompletes)
{
    ...
    EXPECT_CALL(*mock_video_decoder(), Initialize_(_, _, _, _, _, _))
        .WillOnce(DoAll(testing::SaveArg<4>(&output_cb), RunOnceCallback<3>(DecoderStatus::Codes::kOk)));
    EXPECT_CALL(*mock_video_decoder(), Decode_(_, _)).WillOnce(DoAll([&]() { output_cb.Run(expected_frame); }, RunOnceCallback<1>(DecoderStatus::Codes::kOk)));
...
{
    auto expected_frame = CreateFrame();
    EXPECT_CALL(*mock_video_decoder(), Initialize_(_, _, _, _, _, _)).WillOnce(RunOnceCallback<3>(DecoderStatus::Codes::kFailed));
...
{
    auto expected_frame = CreateFrame();
    EXPECT_CALL(*mock_video_decoder(), Initialize_(_, _, _, _, _, _)).WillOnce(RunOnceCallback<3>(DecoderStatus::Codes::kOk));
    EXPECT_CALL(*mock_video_decoder(), Decode_(_, _)).WillOnce(RunOnceCallback<1>(DecoderStatus::Codes::kFailed));
