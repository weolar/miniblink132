...
{
    return base::StrCat({ "duration (ms): ", base::NumberToString(duration.InMilliseconds()), ", count: ", base::NumberToString(count) });
}
...
void AudioGlitchInfo::MaybeAddTraceEvent() const
{
    ...
    TRACE_EVENT_INSTANT("audio", "Glitch!", "glitch count", count, "glitch duration", duration);
}

// static
AudioGlitchInfo AudioGlitchInfo::SingleBoundedSystemGlitch(const base::TimeDelta duration, const Direction direction)
{
    CHECK(duration.is_positive());
    if (direction == Direction::kRender) {
        UMA_HISTOGRAM_LONG_TIMES("Media.Audio.Render.SystemGlitchDuration", duration);
    } else {
        UMA_HISTOGRAM_LONG_TIMES("Media.Audio.Capture.SystemGlitchDuration", duration);
    }
    return AudioGlitchInfo { .duration = std::clamp(duration, base::Seconds(0), base::Seconds(1)), .count = 1 };
}
