...
template <typename T, class SampleTraits> void RunInterleaveBench(AudioBus* bus, const std::string& trace_name, bool to_interleaved_only = false)
{
    ...
    reporter.AddResult("_to_interleaved", total_time_milliseconds / kBenchmarkIterations);
    ...
    reporter.AddResult("_from_interleaved", total_time_milliseconds / kBenchmarkIterations);
}
...
TEST(AudioBusPerfTest, Interleave)
{
    ...
    RunInterleaveBench<int16_t, SignedInt16SampleTypeTraits>(bus.get(), "int16_t");
...
TEST(AudioBusPerfTest, DISABLED_ToInterleavedFloat)
{
    ...
    RunInterleaveBench<float, Float32SampleTypeTraits>(bus.get(), "to_interleave_float", true);
    RunInterleaveBench<float, Float32SampleTypeTraitsNoClip>(bus.get(), "to_interleave_float_no_clip", true);
}

void RunCopyBench(void (AudioBus::*f)(AudioBus*) const, const std::string& trace_name)
{
    // Setup.
    std::unique_ptr<AudioBus> bus = AudioBus::Create(2, kSampleRate * 120);
    ...
    for (int i = 0; i < kBenchmarkIterations; ++i)
        (bus.get()->*f)(dest.get());
    auto total_time_milliseconds = (base::TimeTicks::Now() - start).InMillisecondsF();
