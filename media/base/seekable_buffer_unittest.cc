...
TEST_F(SeekableBufferTest, RandomReadWrite)
{
    ...
    while (read_position < kDataSize) {
        ...
        EXPECT_EQ(should_append, buffer_.forward_bytes() < kBufferSize) << "Incorrect buffer full reported";
...
TEST_F(SeekableBufferTest, ReadWriteSeek)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (int j = 0; j < kBufferSize; j += kWriteSize) {
            ...
            EXPECT_EQ(j < kBufferSize - kWriteSize, should_append) << "Incorrect buffer full reported";
...
TEST_F(SeekableBufferTest, GetCurrentChunk)
{
    ...
    scoped_refptr<DataBuffer> buffer = DataBuffer::CopyFrom(base::make_span(data_, static_cast<size_t>(kWriteSize)));
...
TEST_F(SeekableBufferTest, SeekForward)
{
    ...
    while (read_position < kDataSize) {
        for (int i = 0; i < 10 && write_position < kDataSize; ++i) {
            ...
            EXPECT_GE(write_position, read_position);
            EXPECT_EQ(write_position - read_position, buffer_.forward_bytes());
            EXPECT_EQ(should_append, buffer_.forward_bytes() < kBufferSize) << "Incorrect buffer full status reported";
        }
...
TEST_F(SeekableBufferTest, GetTime)
{
    ...
    EXPECT_EQ(kNoTimestamp.ToInternalValue(), buffer_.current_time().ToInternalValue());
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        EXPECT_EQ(tests[i].expected_time, actual) << "With test = { start:" << tests[i].first_time_useconds << ", duration:" << tests[i].duration_useconds
