...
{
    DVLOG(1) << "Callback Timeout: " << uma_name << ", called_on_destruction=" << called_on_destruction;
    base::UmaHistogramEnumeration(uma_name, called_on_destruction ? CallbackTimeoutStatus::kDestructedBeforeTimeout : CallbackTimeoutStatus::kTimeout);
}
...
public:
    RendererWrapper(
        scoped_refptr<base::SequencedTaskRunner> media_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, MediaLog* media_log);
    ...
    void Start(StartType start_type, Demuxer* demuxer, std::unique_ptr<Renderer> default_renderer, base::WeakPtr<PipelineImpl> weak_pipeline);
    void Stop();
    ...
    void SetWasPlayedWithUserActivationAndHighMediaEngagement(bool was_played_with_user_activation_and_high_media_engagement);
    ...
    void OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids, base::OnceClosure change_completed_cb);

    // |selected_track_id| is either empty, which means no video track is
    // selected, or contains the selected video track id.
    void OnSelectedVideoTrackChanged(std::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb);
...
private:
    ...
    void OnDemuxerCompletedTrackChange(DemuxerStream::Type stream_type, base::OnceClosure change_completed_cb, const std::vector<DemuxerStream*>& streams);
    ...
    void OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason) final;
    ...
    void OnCdmAttached(CdmAttachedCB cdm_attached_cb, CdmContext* cdm_context, bool success);
    ...
    void OnRendererCreated(PipelineStatusCallback done_cb, std::unique_ptr<Renderer> renderer);
...
void PipelineImpl::RendererWrapper::Start(
    StartType start_type, Demuxer* demuxer, std::unique_ptr<Renderer> default_renderer, base::WeakPtr<PipelineImpl> weak_pipeline)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kCreated || state_ == State::kStopped) << "Received start in unexpected state: " << GetStateString(state_);
    DCHECK(!demuxer_);
    ...
    error_cb_ = base::BindRepeating(&RendererWrapper::OnPipelineError, weak_factory_.GetWeakPtr());
    ...
    // Initialize demuxer.
    fns.Push(base::BindOnce(&RendererWrapper::InitializeDemuxer, weak_factory_.GetWeakPtr()));
    ...
    fns.Push(base::BindOnce(&RendererWrapper::ReportMetadata, weak_factory_.GetWeakPtr(), start_type));

    // Create renderer.
    fns.Push(base::BindOnce(&RendererWrapper::CreateRenderer, weak_factory_.GetWeakPtr()));

    // Initialize renderer.
    fns.Push(base::BindOnce(&RendererWrapper::InitializeRenderer, weak_factory_.GetWeakPtr()));

    // Run tasks.
    pending_callbacks_ = SerialRunner::Run(std::move(fns), base::BindOnce(&RendererWrapper::CompleteSeek, weak_factory_.GetWeakPtr(), base::TimeDelta()));
}
...
void PipelineImpl::RendererWrapper::Stop()
{
    ...
    if (shared_state_.statistics.video_frames_decoded > 0) {
        UMA_HISTOGRAM_COUNTS_1M("Media.DroppedFrameCount", shared_state_.statistics.video_frames_dropped);
    }
...
void PipelineImpl::RendererWrapper::Seek(base::TimeDelta time)
{
    ...
    if (state_ != State::kPlaying) {
        DCHECK(state_ == State::kStopping || state_ == State::kStopped) << "Receive seek in unexpected state: " << GetStateString(state_);
    ...
    bound_fns.Push(base::BindOnce(&Renderer::Flush, base::Unretained(shared_state_.renderer.get())));

    // Seek demuxer.
    bound_fns.Push(base::BindOnce(&Demuxer::Seek, base::Unretained(demuxer_), seek_timestamp));

    // Run tasks.
    pending_callbacks_ = SerialRunner::Run(std::move(bound_fns), base::BindOnce(&RendererWrapper::CompleteSeek, weak_factory_.GetWeakPtr(), seek_timestamp));
}
...
void PipelineImpl::RendererWrapper::Suspend()
{
    ...
    if (state_ != State::kPlaying) {
        DCHECK(state_ == State::kStopping || state_ == State::kStopped) << "Receive suspend in unexpected state: " << GetStateString(state_);
        OnPipelineError(PIPELINE_ERROR_INVALID_STATE);
        return;
    }
    ...
    pending_callbacks_ = SerialRunner::Run(std::move(fns), base::BindOnce(&RendererWrapper::CompleteSuspend, weak_factory_.GetWeakPtr()));
}

void PipelineImpl::RendererWrapper::Resume(std::unique_ptr<Renderer> default_renderer, base::TimeDelta timestamp)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    // Suppress resuming if we're not suspended.
    if (state_ != State::kSuspended) {
        DCHECK(state_ == State::kStopping || state_ == State::kStopped) << "Receive resume in unexpected state: " << GetStateString(state_);
        OnPipelineError(PIPELINE_ERROR_INVALID_STATE);
        return;
    }
    DCHECK(!pending_callbacks_.get());
    ...
    if (!default_renderer) {
        OnPipelineError({ PIPELINE_ERROR_INITIALIZATION_FAILED, "Media Renderer creation failed during resume!" });
        return;
    }
    ...
    fns.Push(base::BindOnce(&Demuxer::Seek, base::Unretained(demuxer_), start_timestamp));

    fns.Push(base::BindOnce(&RendererWrapper::CreateRenderer, weak_factory_.GetWeakPtr()));

    fns.Push(base::BindOnce(&RendererWrapper::InitializeRenderer, weak_factory_.GetWeakPtr()));

    pending_callbacks_ = SerialRunner::Run(std::move(fns), base::BindOnce(&RendererWrapper::CompleteSeek, weak_factory_.GetWeakPtr(), start_timestamp));
}
...
void PipelineImpl::RendererWrapper::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void PipelineImpl::RendererWrapper::SetWasPlayedWithUserActivationAndHighMediaEngagement(bool was_played_with_user_activation_and_high_media_engagement)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    was_played_with_user_activation_and_high_media_engagement_ = was_played_with_user_activation_and_high_media_engagement;
    if (shared_state_.renderer) {
        shared_state_.renderer->SetWasPlayedWithUserActivationAndHighMediaEngagement(was_played_with_user_activation_and_high_media_engagement_);
    }
}
...
base::TimeDelta PipelineImpl::RendererWrapper::GetMediaTime() const
{
    ...
    return shared_state_.renderer ? shared_state_.renderer->GetMediaTime() : base::TimeDelta();
}

Ranges<base::TimeDelta> PipelineImpl::RendererWrapper::GetBufferedTimeRanges() const
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void PipelineImpl::RendererWrapper::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(cdm_context);
    ...
    if (shared_state_.renderer) {
        shared_state_.renderer->SetCdm(
            cdm_context, base::BindOnce(&RendererWrapper::OnCdmAttached, weak_factory_.GetWeakPtr(), std::move(cdm_attached_cb), cdm_context));
        return;
    }
...
void PipelineImpl::RendererWrapper::CreateRendererInternal(PipelineStatusCallback done_cb)
{
    DVLOG(1) << __func__;

    DCHECK(state_ == State::kStarting || state_ == State::kResuming);
    DCHECK(cdm_context_ || !HasEncryptedStream()) << "CDM should be available now if has encrypted stream";
    ...
    if (!default_renderer_ || renderer_type) {
        ...
        // media task runner.
        auto renderer_created_cb
            = base::BindPostTaskToCurrentDefault(base::BindOnce(&RendererWrapper::OnRendererCreated, weak_factory_.GetWeakPtr(), std::move(done_cb)));
        main_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&PipelineImpl::AsyncCreateRenderer, weak_pipeline_, renderer_type, std::move(renderer_created_cb)));
        return;
    }
...
void PipelineImpl::RendererWrapper::OnBufferedTimeRangesChanged(const Ranges<base::TimeDelta>& ranges)
{
...
void PipelineImpl::RendererWrapper::SetDuration(base::TimeDelta duration)
{
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnDurationChange, weak_pipeline_, duration));
}
...
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnFallback, weak_pipeline_, std::move(fallback).AddHere()));
}
...
base::TimeDelta PipelineImpl::RendererWrapper::GetCurrentTimestamp()
{
    ...
    return state_ == State::kPlaying ? shared_state_.renderer->GetMediaTime() : demuxer_->GetStartTime();
}

void PipelineImpl::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids, base::OnceClosure change_completed_cb)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RendererWrapper::OnEnabledAudioTracksChanged, base::Unretained(renderer_wrapper_.get()), enabled_track_ids,
            base::BindPostTaskToCurrentDefault(std::move(change_completed_cb))));
}

void PipelineImpl::RendererWrapper::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids, base::OnceClosure change_completed_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    demuxer_->OnEnabledAudioTracksChanged(enabled_track_ids, GetCurrentTimestamp(),
        base::BindOnce(&RendererWrapper::OnDemuxerCompletedTrackChange, weak_factory_.GetWeakPtr(), DemuxerStream::AUDIO, std::move(change_completed_cb)));
}

void PipelineImpl::OnSelectedVideoTrackChanged(std::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RendererWrapper::OnSelectedVideoTrackChanged, base::Unretained(renderer_wrapper_.get()), selected_track_id,
            base::BindPostTaskToCurrentDefault(std::move(change_completed_cb))));
}

void PipelineImpl::RendererWrapper::OnSelectedVideoTrackChanged(std::optional<MediaTrack::Id> selected_track_id, base::OnceClosure change_completed_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    demuxer_->OnSelectedVideoTrackChanged(tracks, GetCurrentTimestamp(),
        base::BindOnce(&RendererWrapper::OnDemuxerCompletedTrackChange, weak_factory_.GetWeakPtr(), DemuxerStream::VIDEO, std::move(change_completed_cb)));
}
...
void PipelineImpl::OnExternalVideoFrameRequest()
{
    ...
    if (!external_video_frame_request_signaled_) {
        ...
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::OnExternalVideoFrameRequest, base::Unretained(renderer_wrapper_.get())));
    }
}
...
void PipelineImpl::RendererWrapper::OnDemuxerCompletedTrackChange(
    DemuxerStream::Type stream_type, base::OnceClosure change_completed_cb, const std::vector<DemuxerStream*>& streams)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    if (!shared_state_.renderer) {
    ...
    case DemuxerStream::AUDIO:
        shared_state_.renderer->OnEnabledAudioTracksChanged(streams, std::move(change_completed_cb));
        break;
    case DemuxerStream::VIDEO:
        shared_state_.renderer->OnSelectedVideoTracksChanged(streams, std::move(change_completed_cb));
        break;
...
void PipelineImpl::RendererWrapper::OnStatisticsUpdate(const PipelineStatistics& stats)
{
    DVLOG(3) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    shared_state_.statistics.video_frames_decoded_power_efficient += stats.video_frames_decoded_power_efficient;
    ...
    if (stats.audio_pipeline_info.decoder_type != AudioDecoderType::kUnknown && stats.audio_pipeline_info != shared_state_.statistics.audio_pipeline_info) {
        shared_state_.statistics.audio_pipeline_info = stats.audio_pipeline_info;
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnAudioPipelineInfoChange, weak_pipeline_, stats.audio_pipeline_info));
    }

    if (stats.video_pipeline_info.decoder_type != VideoDecoderType::kUnknown && stats.video_pipeline_info != shared_state_.statistics.video_pipeline_info) {
        shared_state_.statistics.video_pipeline_info = stats.video_pipeline_info;
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoPipelineInfoChange, weak_pipeline_, stats.video_pipeline_info));
    }

    if (stats.video_frame_duration_average != kNoTimestamp) {
        shared_state_.statistics.video_frame_duration_average = stats.video_frame_duration_average;
    }
    ...
    if (shared_state_.statistics.video_keyframe_distance_average != old_key_frame_distance_average) {
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoAverageKeyframeDistanceUpdate, weak_pipeline_));
    }
}

void PipelineImpl::RendererWrapper::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DVLOG(2) << __func__ << "(" << state << ", " << reason << ") ";

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnBufferingStateChange, weak_pipeline_, state, reason));
}
...
void PipelineImpl::RendererWrapper::OnWaiting(WaitingReason reason)
{
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnWaiting, weak_pipeline_, reason));
}

void PipelineImpl::RendererWrapper::OnVideoNaturalSizeChange(const gfx::Size& size)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoNaturalSizeChange, weak_pipeline_, size));
}
...
void PipelineImpl::RendererWrapper::OnVideoOpacityChange(bool opaque)
{
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoOpacityChange, weak_pipeline_, opaque));
}

void PipelineImpl::RendererWrapper::OnVideoFrameRateChange(std::optional<int> fps)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoFrameRateChange, weak_pipeline_, fps));
}

void PipelineImpl::RendererWrapper::OnAudioConfigChange(const AudioDecoderConfig& config)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnAudioConfigChange, weak_pipeline_, config));
}

void PipelineImpl::RendererWrapper::OnVideoConfigChange(const VideoDecoderConfig& config)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnVideoConfigChange, weak_pipeline_, config));
}
...
void PipelineImpl::RendererWrapper::OnPipelineError(PipelineStatus error)
{
    ...
    if (state_ == State::kStopping || state_ == State::kStopped || state_ == State::kSuspending || state_ == State::kSuspended) {
        return;
    }
    ...
    // pipeline.
    if (error != PIPELINE_ERROR_HARDWARE_CONTEXT_RESET && error != DEMUXER_ERROR_DETECTED_HLS) {
        status_ = error;
    }

    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnError, weak_pipeline_, error));
}

void PipelineImpl::RendererWrapper::OnCdmAttached(CdmAttachedCB cdm_attached_cb, CdmContext* cdm_context, bool success)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void PipelineImpl::RendererWrapper::CheckPlaybackEnded()
{
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnEnded, weak_pipeline_));
}
...
void PipelineImpl::RendererWrapper::SetState(State next_state)
{
    ...
    // to be called here.
    media_log_->AddEvent<MediaLogEvent::kPipelineStateChange>(std::string(GetStateString(next_state)));
}

void PipelineImpl::RendererWrapper::CompleteSeek(base::TimeDelta seek_time, PipelineStatus status)
{
    DVLOG(1) << __func__ << ": seek_time=" << seek_time << ", status=" << status;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStarting || state_ == State::kSeeking || state_ == State::kResuming);
    ...
    shared_state_.renderer->StartPlayingFrom(std::max(seek_time, demuxer_->GetStartTime()));
    {
        base::AutoLock auto_lock(shared_state_lock_);
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnSeekDone, weak_pipeline_, false));
}
...
void PipelineImpl::RendererWrapper::CompleteSuspend(PipelineStatus status)
{
    ...
    LOG_IF(WARNING, status != PIPELINE_OK) << "Encountered pipeline error while suspending: " << status;
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnSuspendDone, weak_pipeline_));
}

void PipelineImpl::RendererWrapper::InitializeDemuxer(PipelineStatusCallback done_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void PipelineImpl::RendererWrapper::CreateRenderer(PipelineStatusCallback done_cb)
{
    DVLOG(1) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStarting || state_ == State::kResuming);
...
void PipelineImpl::RendererWrapper::OnRendererCreated(PipelineStatusCallback done_cb, std::unique_ptr<Renderer> renderer)
{
    DVLOG(1) << __func__ << ": renderer=" << renderer.get();
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    if (!renderer) {
        std::move(done_cb).Run({ PIPELINE_ERROR_INITIALIZATION_FAILED, "Media Renderer creation failed!" });
        return;
    }
...
void PipelineImpl::RendererWrapper::InitializeRenderer(PipelineStatusCallback done_cb)
{
    DVLOG(1) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    shared_state_.renderer->Initialize(
        demuxer_, this, WrapCallbackWithTimeoutHandler(std::move(done_cb), /*timeout_delay=*/base::Seconds(10), base::BindOnce(&OnCallbackTimeout, uma_name)));
}
...
void PipelineImpl::RendererWrapper::ReportMetadata(StartType start_type)
{
        ...
        for (media::DemuxerStream* stream : streams) {
            if (stream->type() == DemuxerStream::VIDEO && !metadata.has_video) {
                ...
                metadata.natural_size
                    = GetRotatedVideoSize(stream->video_decoder_config().video_transformation().rotation, stream->video_decoder_config().natural_size());
    ...
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnMetadata, weak_pipeline_, metadata));
    ...
    if (start_type == StartType::kNormal || (start_type == StartType::kSuspendAfterMetadataForAudioOnly && metadata.has_video)) {
        return;
    }
    ...
    pending_callbacks_.reset();
    DestroyRenderer();
    shared_state_.suspend_timestamp = std::max(base::TimeDelta(), demuxer_->GetStartTime());
    SetState(State::kSuspended);
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&PipelineImpl::OnSeekDone, weak_pipeline_, true));
}
...
bool PipelineImpl::RendererWrapper::HasEncryptedStream()
{
    ...
    for (media::DemuxerStream* stream : streams) {
        if (stream->type() == DemuxerStream::AUDIO && stream->audio_decoder_config().is_encrypted())
            return true;
        if (stream->type() == DemuxerStream::VIDEO && stream->video_decoder_config().is_encrypted())
            return true;
    }
...
PipelineImpl::PipelineImpl(scoped_refptr<base::SequencedTaskRunner> media_task_runner, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
    CreateRendererCB create_renderer_cb, MediaLog* media_log)
    ...
    , is_suspended_(false)
{
    ...
    renderer_wrapper_ = std::make_unique<RendererWrapper>(media_task_runner_, std::move(main_task_runner), media_log_);
}
...
PipelineImpl::~PipelineImpl()
{
    ...
    DCHECK(!seek_cb_);
    DCHECK(!suspend_cb_);
    DCHECK(!weak_factory_.HasWeakPtrs()) << "Stop() should have invalidated all weak pointers";

    // RendererWrapper is deleted on the media thread.
    media_task_runner_->DeleteSoon(FROM_HERE, renderer_wrapper_.release());
}

void PipelineImpl::Start(StartType start_type, Demuxer* demuxer, Client* client, PipelineStatusCallback seek_cb)
{
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &RendererWrapper::Start, base::Unretained(renderer_wrapper_.get()), start_type, demuxer, std::move(default_renderer), weak_factory_.GetWeakPtr()));
}
...
void PipelineImpl::Stop()
{
    ...
    if (media_task_runner_->RunsTasksInCurrentSequence()) {
        ...
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::Stop, base::Unretained(renderer_wrapper_.get())));
    }
...
void PipelineImpl::Seek(base::TimeDelta time, PipelineStatusCallback seek_cb)
{
    ...
    seek_time_ = time;
    last_media_time_ = base::TimeDelta();
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::Seek, base::Unretained(renderer_wrapper_.get()), time));
}
...
void PipelineImpl::Suspend(PipelineStatusCallback suspend_cb)
{
    ...
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::Suspend, base::Unretained(renderer_wrapper_.get())));
}

void PipelineImpl::Resume(base::TimeDelta time, PipelineStatusCallback seek_cb)
{
    DVLOG(2) << __func__;
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(seek_cb);
    ...
    media_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&RendererWrapper::Resume, base::Unretained(renderer_wrapper_.get()), std::move(default_renderer), time));
}
...
void PipelineImpl::SetPlaybackRate(double playback_rate)
{
    ...
    playback_rate_ = playback_rate;
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::SetPlaybackRate, base::Unretained(renderer_wrapper_.get()), playback_rate_));
}
...
void PipelineImpl::SetVolume(float volume)
{
    ...
    volume_ = volume;
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::SetVolume, base::Unretained(renderer_wrapper_.get()), volume_));
}

void PipelineImpl::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
    DVLOG(1) << __func__ << "(" << (latency_hint ? base::NumberToString(latency_hint->InMilliseconds()) + "ms" : "null_opt") << ")";
    DCHECK(!latency_hint || (*latency_hint >= base::TimeDelta()));
    DCHECK(thread_checker_.CalledOnValidThread());

    // Not checking IsRunning() so we can set the latency hint before Start().
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::SetLatencyHint, base::Unretained(renderer_wrapper_.get()), latency_hint));
}
...
void PipelineImpl::SetPreservesPitch(bool preserves_pitch)
{
    ...
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererWrapper::SetPreservesPitch, base::Unretained(renderer_wrapper_.get()), preserves_pitch));
}

void PipelineImpl::SetWasPlayedWithUserActivationAndHighMediaEngagement(bool was_played_with_user_activation_and_high_media_engagement)
{
    DCHECK(thread_checker_.CalledOnValidThread());

    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RendererWrapper::SetWasPlayedWithUserActivationAndHighMediaEngagement, base::Unretained(renderer_wrapper_.get()),
...
base::TimeDelta PipelineImpl::GetMediaTime() const
{
    ...
    if (seek_time_ != kNoTimestamp) {
        DVLOG(3) << __func__ << ": (seeking) " << seek_time_.InMilliseconds() << " ms";
        return seek_time_;
    }
    ...
    if (media_time < last_media_time_) {
        DVLOG(2) << __func__ << ": actual=" << media_time << " clamped=" << last_media_time_;
...
void PipelineImpl::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    DVLOG(2) << __func__;
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(cdm_context);
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &RendererWrapper::SetCdm, base::Unretained(renderer_wrapper_.get()), cdm_context, base::BindPostTaskToCurrentDefault(std::move(cdm_attached_cb))));
}

void PipelineImpl::AsyncCreateRenderer(std::optional<RendererType> renderer_type, RendererCreatedCB renderer_created_cb)
{
    DVLOG(2) << __func__;
    DCHECK(thread_checker_.CalledOnValidThread());
...
void PipelineImpl::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(IsRunning());
