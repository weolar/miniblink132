...
#define TAG(a, b, c, d)                                                                                                                                        \
    ((static_cast<uint32_t>(static_cast<uint8_t>(a)) << 24) | (static_cast<uint32_t>(static_cast<uint8_t>(b)) << 16)                                           \
...
// end of the buffer.
static bool StartsWith(const uint8_t* buffer, size_t buffer_size, const char* prefix)
{
...
static bool StartsWith(const uint8_t* buffer, size_t buffer_size, const uint8_t* prefix, size_t prefix_size)
{
    return (prefix_size <= buffer_size && memcmp(buffer, prefix, prefix_size) == 0);
}
...
const int kAc3FrameSizeTable[38][3] = { { 128, 138, 192 }, { 128, 140, 192 }, { 160, 174, 240 }, { 160, 176, 240 }, { 192, 208, 288 }, { 192, 210, 288 },
    { 224, 242, 336 }, { 224, 244, 336 }, { 256, 278, 384 }, { 256, 280, 384 }, { 320, 348, 480 }, { 320, 350, 480 }, { 384, 416, 576 }, { 384, 418, 576 },
    { 448, 486, 672 }, { 448, 488, 672 }, { 512, 556, 768 }, { 512, 558, 768 }, { 640, 696, 960 }, { 640, 698, 960 }, { 768, 834, 1152 }, { 768, 836, 1152 },
    { 896, 974, 1344 }, { 896, 976, 1344 }, { 1024, 1114, 1536 }, { 1024, 1116, 1536 }, { 1280, 1392, 1920 }, { 1280, 1394, 1920 }, { 1536, 1670, 2304 },
    { 1536, 1672, 2304 }, { 1792, 1950, 2688 }, { 1792, 1952, 2688 }, { 2048, 2228, 3072 }, { 2048, 2230, 3072 }, { 2304, 2506, 3456 }, { 2304, 2508, 3456 },
...
static bool kSamplingFrequencyValid[16] = { false, true, true, true, false, false, true, true, true, false, false, true, true, true, false, false };
static bool kExtAudioIdValid[8] = { true, false, true, false, false, false, true, false };
...
// false otherwise.
static bool AdvanceToStartCode(const uint8_t* buffer, int buffer_size, int* offset, int bytes_needed, int num_bits, uint32_t start_code)
{
...
static bool CheckHls(const uint8_t* buffer, int buffer_size)
{
    ...
    if (StartsWith(buffer, buffer_size, kHlsSignature)) {
        ...
        while (offset < buffer_size) {
            if (buffer[offset] == '#') {
                if (StartsWith(buffer + offset, buffer_size - offset, kHls1) || StartsWith(buffer + offset, buffer_size - offset, kHls2)
...
static int kBitRateTableV1L1[16] = { 0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, 0 };
static int kBitRateTableV1L2[16] = { 0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 0 };
static int kBitRateTableV1L3[16] = { 0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0 };
static int kBitRateTableV2L1[16] = { 0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256, 0 };
static int kBitRateTableV2L23[16] = { 0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, 0 };

static bool ValidMpegAudioFrameHeader(const uint8_t* header, int header_size, int* framesize)
{
...
static bool CheckMp3(const uint8_t* buffer, int buffer_size)
{
    ...
    while (offset + 3 < buffer_size) {
        int framesize;
        RCHECK(ValidMpegAudioFrameHeader(buffer + offset, buffer_size - offset, &framesize));
...
static bool VerifyNumber(const uint8_t* buffer, int buffer_size, int* offset, int max_digits)
{
...
// space.
static inline bool VerifyCharacters(const uint8_t* buffer, int buffer_size, int* offset, char c1, char c2)
{
    RCHECK(*offset < buffer_size);
...
enum VC1StartCodes { VC1_FRAME_START_CODE = 0x0d, VC1_ENTRY_POINT_START_CODE = 0x0e, VC1_SEQUENCE_START_CODE = 0x0f };
...
static const uint8_t kAsfSignature[] = { 0x30, 0x26, 0xb2, 0x75, 0x8e, 0x66, 0xcf, 0x11, 0xa6, 0xd9, 0x00, 0xaa, 0x00, 0x62, 0xce, 0x6c };
static const char kAssSignature[] = "[Script Info]";
static const char kAssBomSignature[] = UTF8_BYTE_ORDER_MARK "[Script Info]";
static const uint8_t kWtvSignature[] = { 0xb7, 0xd8, 0x00, 0x20, 0x37, 0x49, 0xda, 0x11, 0xa6, 0x4e, 0x00, 0x07, 0xe9, 0x5e, 0xad, 0x8d };
...
static MediaContainerName LookupContainerByFirst4(const uint8_t* buffer, int buffer_size)
{
    ...
    case 0x3026b275:
        if (StartsWith(buffer, buffer_size, kAsfSignature, sizeof(kAsfSignature))) {
    ...
    case 0xb7d80020:
        if (StartsWith(buffer, buffer_size, kWtvSignature, sizeof(kWtvSignature))) {
