...
SilentSinkSuspender::SilentSinkSuspender(AudioRendererSink::RenderCallback* callback, base::TimeDelta silence_timeout, const AudioParameters& params,
    scoped_refptr<AudioRendererSink> sink, scoped_refptr<base::SingleThreadTaskRunner> worker)
    : callback_(callback)
    , params_(params)
    , sink_(std::move(sink))
    ...
    , sink_transition_callback_(base::BindRepeating(&SilentSinkSuspender::TransitionSinks, base::Unretained(this)))
{
...
int SilentSinkSuspender::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, const AudioGlitchInfo& glitch_info, AudioBus* dest)
{
    ...
    if (!dest) {
        ...
        base::TimeDelta elapsed_time = base::TimeTicks::Now() - fake_sink_transition_time_;
    ...
    if (!dest->AreFramesZero() || !detect_silence_) {
        ...
        if (is_using_fake_sink_) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(sink_transition_callback_.callback(), false));
            return dest->frames();
        }
        ...
        if (now - first_silence_time_ > silence_timeout_) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(sink_transition_callback_.callback(), true));
        }
    }
...
void SilentSinkSuspender::OnPaused()
{
    ...
    first_silence_time_ = base::TimeTicks();
    sink_transition_callback_.Reset(base::BindRepeating(&SilentSinkSuspender::TransitionSinks, base::Unretained(this)));
}
...
void SilentSinkSuspender::TransitionSinks(bool use_fake_sink)
{
    ...
    if (use_fake_sink) {
        ...
        fake_sink_.Start(base::BindRepeating(
            [](SilentSinkSuspender* suspender, base::TimeDelta frozen_delay, base::TimeTicks frozen_delay_timestamp, base::TimeTicks ideal_time,
                base::TimeTicks now) {
