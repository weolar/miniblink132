...
void VerifyAudioBuffer(scoped_refptr<AudioBuffer> buffer, int number_frames, AudioBus* expected_data)
{
    ...
    EXPECT_EQ(buffer->duration(), AudioTimestampHelper::FramesToTime(number_frames, kSampleRate));

    for (int ch = 0; ch < kChannels; ++ch) {
        const size_t kSpanSize = sizeof(float) * static_cast<size_t>(number_frames);
        base::span<uint8_t> input_span = base::make_span(reinterpret_cast<uint8_t*>(expected_data->channel(ch)), kSpanSize);
        base::span<uint8_t> output_span = base::make_span(buffer->channel_data()[ch], kSpanSize);
...
public:
    ...
    LimitingAudioQueueTest()
        ...
        , limiting_queue_(std::make_unique<LimitingAudioQueue>(kChannelLayout, kSampleRate, kChannels, kBufferSize))
...
TEST_F(LimitingAudioQueueTest, NoLimiting_IsPassthrough)
{
    ...
    limiting_queue_->Push(*input_bus_, kBufferSize, base::TimeDelta(), base::BindLambdaForTesting(std::move(verify_buffer)));
    limiting_queue_->Flush();
    EXPECT_TRUE(result);
}
...
TEST_F(LimitingAudioQueueTest, Clear_DropsPendingInputs)
{
    ...
    limiting_queue_->Push(
        *input_bus_, kBufferSize, base::TimeDelta(), base::BindLambdaForTesting([&](scoped_refptr<AudioBuffer>) { first_buffer_emitted = true; }));
    ...
    limiting_queue_->Push(*input_bus_, kBufferSize, base::TimeDelta(), base::BindLambdaForTesting([&](scoped_refptr<AudioBuffer> buffer) {
        const float* channel_data = reinterpret_cast<const float*>(buffer->channel_data()[0]);
...
TEST_F(LimitingAudioQueueTest, NoLimiting_PartialBuffer_IsPassthrough)
{
    ...
    limiting_queue_->Push(*input_bus_, kPartialBuffer, base::TimeDelta(), base::BindLambdaForTesting(std::move(verify_buffer)));
    limiting_queue_->Flush();
...
TEST_F(LimitingAudioQueueTest, Limiting_CompressesGain)
{
    ...
    auto verify_buffer = [&](scoped_refptr<AudioBuffer> buffer) {
        for (int ch = 0; ch < kChannels; ++ch) {
            const float* channel_data = reinterpret_cast<const float*>(buffer->channel_data()[ch]);
...
TEST_F(LimitingAudioQueueTest, MultipleBuffers)
{
    ...
    const std::vector<int> kBufferSizeSequence = { kBufferSize, kBufferSize / 2, kBufferSize / 4, kBufferSize, kBufferSize - 16 };

    int buffer_count = 0;
    base::TimeDelta current_timestamp = base::TimeDelta();

    for (int size : kBufferSizeSequence) {
        const base::TimeDelta duration = AudioTimestampHelper::FramesToTime(size, kSampleRate);
        limiting_queue_->Push(*input_bus_, size, current_timestamp,
            base::BindOnce(
                [](int expected_size, base::TimeDelta expected_duration, base::TimeDelta expected_timestamp, int* buffer_count,
