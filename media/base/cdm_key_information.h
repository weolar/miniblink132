...
struct MEDIA_EXPORT CdmKeyInformation {
    ...
    CdmKeyInformation();
    CdmKeyInformation(const std::vector<uint8_t>& key_id, KeyStatus status, uint32_t system_code);
    CdmKeyInformation(const std::string& key_id, KeyStatus status, uint32_t system_code);
    CdmKeyInformation(const uint8_t* key_id_data, size_t key_id_length, KeyStatus status, uint32_t system_code);
...
MEDIA_EXPORT std::ostream& operator<<(std::ostream& os, CdmKeyInformation::KeyStatus status);

MEDIA_EXPORT std::ostream& operator<<(std::ostream& os, const CdmKeyInformation& info);
