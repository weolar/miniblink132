...
TEST_F(UserInputMonitorTest, CreatePlatformSpecific)
{
    ...
    std::unique_ptr<UserInputMonitor> monitor
        = UserInputMonitor::Create(base::SingleThreadTaskRunner::GetCurrentDefault(), base::SingleThreadTaskRunner::GetCurrentDefault());
...
TEST_F(UserInputMonitorTest, CreatePlatformSpecificWithMapping)
{
    ...
    base::ReadOnlySharedMemoryMapping readonly_mapping = static_cast<UserInputMonitorBase*>(monitor.get())->EnableKeyPressMonitoringWithMapping().Map();
...
TEST_F(UserInputMonitorTest, ReadWriteKeyPressMonitorCount)
{
    std::unique_ptr<base::MappedReadOnlyRegion> shmem
        = std::make_unique<base::MappedReadOnlyRegion>(base::ReadOnlySharedMemoryRegion::Create(sizeof(uint32_t)));
