...
void AudioBlockFifo::Push(const void* source, int frames, int bytes_per_sample)
{
    TRACE_EVENT2("audio", "AudioBlockFifo::Push", "pushed frames", frames, "available frames", GetAvailableFrames());
    PushInternal(source, frames, bytes_per_sample);
}

void AudioBlockFifo::PushSilence(int frames)
{
    TRACE_EVENT2("audio", "AudioBlockFifo::PushSilence", "pushed frames", frames, "available frames", GetAvailableFrames());
...
const AudioBus* AudioBlockFifo::Consume()
{
    ...
    TRACE_EVENT1("audio", "AudioBlockFifo::Consume", "available frames", GetAvailableFrames());
...
void AudioBlockFifo::IncreaseCapacity(int blocks)
{
    ...
    std::rotate(audio_blocks_.begin() + read_block_, audio_blocks_.begin() + original_size, audio_blocks_.end());
...
void AudioBlockFifo::PushInternal(const void* source, int frames, int bytes_per_sample)
{
    ...
    while (frames_to_push) {
        ...
        if (source) {
            ...
            case 1:
                current_block->FromInterleavedPartial<UnsignedInt8SampleTypeTraits>(source_ptr, write_pos_, push_frames);
                break;
            case 2:
                current_block->FromInterleavedPartial<SignedInt16SampleTypeTraits>(reinterpret_cast<const int16_t*>(source_ptr), write_pos_, push_frames);
                break;
            case 4:
                current_block->FromInterleavedPartial<SignedInt32SampleTypeTraits>(reinterpret_cast<const int32_t*>(source_ptr), write_pos_, push_frames);
                break;
            default:
