...
VideoAspectRatio::VideoAspectRatio(const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
...
gfx::Size VideoAspectRatio::GetNaturalSize(const gfx::Rect& visible_rect) const
{
    ...
    w = std::round(w);
    h = std::round(h);
    if (w < 1.0 || w > std::numeric_limits<int>::max() || h < 1.0 || h > std::numeric_limits<int>::max()) {
        return visible_rect.size();
    }
