...
class MultiChannelResamplerTest : public testing::TestWithParam<int> {
public:
    ...
    MultiChannelResamplerTest& operator=(const MultiChannelResamplerTest&) = delete;
    ...
    void MultiChannelTest(int channels, int frames, double expected_max_rms_error, double expected_max_error)
    {
        InitializeAudioData(channels, frames);
        MultiChannelResampler resampler(
            channels, kScaleFactor, SincResampler::kDefaultRequestSize, base::BindRepeating(&MultiChannelResamplerTest::ProvideInput, base::Unretained(this)));
    ...
    {
        MultiChannelTest(channels, kHighLatencySize, kHighLatencyMaxRMSError, kHighLatencyMaxError);
    }

    void LowLatencyTest(int channels)
    {
        MultiChannelTest(channels, kLowLatencySize, kLowLatencyMaxRMSError, kLowLatencyMaxError);
    }
    ...
    void TestValues(double expected_max_rms_error, double expected_max_error)
    {
        ...
        double rms_error = sqrt(sum_of_squares / (frames_ * audio_bus_->channels()));
...
INSTANTIATE_TEST_SUITE_P(MultiChannelResamplerTest, MultiChannelResamplerTest, testing::Values(1, 2, 6, 8));
