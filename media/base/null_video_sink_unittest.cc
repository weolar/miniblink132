...
class NullVideoSinkTest : public testing::Test, public VideoRendererSink::RenderCallback {
public:
    NullVideoSinkTest()
    {
    ...
    std::unique_ptr<NullVideoSink> ConstructSink(bool clockless, base::TimeDelta interval)
    {
        auto new_sink = std::make_unique<NullVideoSink>(
            clockless, interval, base::BindRepeating(&NullVideoSinkTest::FrameReceived, base::Unretained(this)), task_environment_.GetMainThreadTaskRunner());
    ...
    scoped_refptr<VideoFrame> CreateFrame(base::TimeDelta timestamp)
    {
        ...
        return VideoFrame::CreateFrame(PIXEL_FORMAT_I420, natural_size, gfx::Rect(natural_size), natural_size, timestamp);
    }
    ...
    MOCK_METHOD3(Render, scoped_refptr<VideoFrame>(base::TimeTicks, base::TimeTicks, RenderingMode));
...
TEST_F(NullVideoSinkTest, BasicFunctionality)
{
...

    {
        ...
        EXPECT_CALL(*this, Render(current_time, current_interval_end, RenderingMode::kNormal)).WillOnce(Return(test_frame));
        WaitableMessageLoopEvent event;
        EXPECT_CALL(*this, FrameReceived(test_frame)).WillOnce(RunOnceClosure(event.GetClosure()));
        event.RunAndWait();
    }
    ...
    // FrameReceived().
    {
        ...
        EXPECT_CALL(*this, Render(_, _, RenderingMode::kBackground)).WillOnce(Return(test_frame)).WillOnce(Return(test_frame_2));
        EXPECT_CALL(*this, FrameReceived(test_frame)).Times(0);
        EXPECT_CALL(*this, FrameReceived(test_frame_2)).WillOnce(RunOnceClosure(event.GetClosure()));
        event.RunAndWait();
    }
...
TEST_F(NullVideoSinkTest, ClocklessFunctionality)
{
    ...
    for (int i = 0; i < kTestRuns; ++i) {
        if (i < kTestRuns - 1) {
            EXPECT_CALL(*this, Render(current_time + i * interval, current_time + (i + 1) * interval, RenderingMode::kNormal)).WillOnce(Return(test_frame));
        } else {
            EXPECT_CALL(*this, Render(current_time + i * interval, current_time + (i + 1) * interval, RenderingMode::kNormal))
                .WillOnce(DoAll(RunOnceClosure(event.GetClosure()), Return(test_frame_2)));
        }
    }
    event.RunAndWait();
