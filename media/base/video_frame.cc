...
VideoFrame::ID GetNextID()
{
    ...
    return VideoFrame::ID::FromUnsafeValue(counter.fetch_add(1u, std::memory_order_relaxed));
}
...
void ReleaseMailboxAndDropGpuMemoryBuffer(
    VideoFrame::ReleaseMailboxCB cb, const gpu::SyncToken& sync_token, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer)
{
    std::move(cb).Run(sync_token);
}

VideoFrame::ReleaseMailboxAndGpuMemoryBufferCB WrapReleaseMailboxCB(VideoFrame::ReleaseMailboxCB cb)
{
    if (cb.is_null())
...
// static
std::string VideoFrame::StorageTypeToString(const VideoFrame::StorageType storage_type)
{
    switch (storage_type) {
...
bool VideoFrame::IsStorageTypeMappable(VideoFrame::StorageType storage_type)
{
    return
        ...
        (storage_type == VideoFrame::STORAGE_UNOWNED_MEMORY || storage_type == VideoFrame::STORAGE_OWNED_MEMORY || storage_type == VideoFrame::STORAGE_SHMEM);
}
...
static bool AreValidPixelFormatsForWrap(VideoPixelFormat source_format, VideoPixelFormat target_format)
{
    return source_format == target_format || (source_format == PIXEL_FORMAT_I420A && target_format == PIXEL_FORMAT_I420)
        || (source_format == PIXEL_FORMAT_ARGB && target_format == PIXEL_FORMAT_XRGB)
        || (source_format == PIXEL_FORMAT_ABGR && target_format == PIXEL_FORMAT_XBGR);
}

// Creates VideoFrameLayout for tightly packed frame.
static std::optional<VideoFrameLayout> GetDefaultLayout(VideoPixelFormat format, const gfx::Size& coded_size)
{
    ...
    case PIXEL_FORMAT_Y16:
        planes = std::vector<ColorPlaneLayout> { ColorPlaneLayout(coded_size.width() * 2, 0, coded_size.GetArea() * 2) };
        break;
    ...
    case PIXEL_FORMAT_ABGR:
    case PIXEL_FORMAT_XBGR:
        planes = std::vector<ColorPlaneLayout> { ColorPlaneLayout(coded_size.width() * 4, 0, coded_size.GetArea() * 4) };
        break;
    ...
    default:
        DLOG(ERROR) << "Unsupported pixel format" << VideoPixelFormatToString(format);
        return std::nullopt;
    }
...
bool VideoFrame::IsValidConfig(
    VideoPixelFormat format, StorageType storage_type, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
...
scoped_refptr<VideoFrame> VideoFrame::CreateFrame(
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    return CreateFrameInternal(format, coded_size, visible_rect, natural_size, timestamp, false);
}

// static
scoped_refptr<VideoFrame> VideoFrame::CreateVideoHoleFrame(
    const base::UnguessableToken& overlay_plane_id, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    return VideoFrame::WrapTrackingToken(PIXEL_FORMAT_UNKNOWN, overlay_plane_id,
        /*coded_size=*/natural_size,
        /*visible_rect=*/gfx::Rect(natural_size), natural_size, timestamp);
}

// static
scoped_refptr<VideoFrame> VideoFrame::WrapTrackingToken(VideoPixelFormat format, const base::UnguessableToken& tracking_token, const gfx::Size& coded_size,
    const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    ...
    scoped_refptr<VideoFrame> frame = new VideoFrame(*layout, StorageType::STORAGE_OPAQUE, visible_rect, natural_size, timestamp);
...
scoped_refptr<VideoFrame> VideoFrame::CreateZeroInitializedFrame(
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    return CreateFrameInternal(format, coded_size, visible_rect, natural_size, timestamp, true);
}
...
scoped_refptr<VideoFrame> VideoFrame::CreateFrameForNativeTexturesInternal(
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    if (format != PIXEL_FORMAT_ARGB && format != PIXEL_FORMAT_XRGB && format != PIXEL_FORMAT_NV12 && format != PIXEL_FORMAT_NV16 && format != PIXEL_FORMAT_NV24
        && format != PIXEL_FORMAT_NV12A && format != PIXEL_FORMAT_I420 && format != PIXEL_FORMAT_ABGR && format != PIXEL_FORMAT_XBGR
        && format != PIXEL_FORMAT_XR30 && format != PIXEL_FORMAT_XB30 && format != PIXEL_FORMAT_P010LE && format != PIXEL_FORMAT_P210LE
        && format != PIXEL_FORMAT_P410LE && format != PIXEL_FORMAT_RGBAF16 && format != PIXEL_FORMAT_YV12 && format != PIXEL_FORMAT_BGRA) {
        DLOG(ERROR) << "Unsupported pixel format: " << VideoPixelFormatToString(format);
        return nullptr;
    }
    const StorageType storage = STORAGE_OPAQUE;
    if (!IsValidConfig(format, storage, coded_size, visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(format, storage, coded_size, visible_rect, natural_size);
        return nullptr;
    }
...
scoped_refptr<VideoFrame> VideoFrame::CreateFrameForGpuMemoryBufferOrMappableSIInternal(const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, scoped_refptr<gpu::ClientSharedImage> shared_image, const bool enable_mappable_si,
    ReleaseMailboxAndGpuMemoryBufferCB mailbox_holder_and_gmb_release_cb, base::TimeDelta timestamp)
{
    *(int*)1 = 1;
    return nullptr;
...
//         = gpu_memory_buffer ? gpu_memory_buffer->GetFormat() : viz::SharedImageFormatToBufferFormatRestrictedUtils::ToBufferFormat(shared_image->format());
//     const std::optional<VideoPixelFormat> format = GfxBufferFormatToVideoPixelFormat(buffer_format);
//     if (!format) {
//         return nullptr;
//     }
//     constexpr StorageType storage = STORAGE_GPU_MEMORY_BUFFER;
//     const gfx::Size& coded_size = gpu_memory_buffer ? gpu_memory_buffer->GetSize() : shared_image->size();
//     if (!IsValidConfig(*format, storage, coded_size, visible_rect, natural_size)) {
//         DLOG(ERROR) << __func__ << " Invalid config" << ConfigToString(*format, storage, coded_size, visible_rect, natural_size);
//         return nullptr;
//     }
...
//         planes[i].stride = gpu_memory_buffer ? gpu_memory_buffer->stride(i) : shared_image->GetStrideForVideoFrame(i);
//     }
//     uint64_t modifier = gfx::NativePixmapHandle::kNoModifier;
// #if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
//     bool is_native_buffer = gpu_memory_buffer ? (gpu_memory_buffer->GetType() != gfx::SHARED_MEMORY_BUFFER) : !shared_image->IsSharedMemoryForVideoFrame();
//     if (is_native_buffer) {
//         const auto gmb_handle = gpu_memory_buffer ? gpu_memory_buffer->CloneHandle() : shared_image->CloneGpuMemoryBufferHandle();
...
//             DLOG(ERROR) << "Invalid number of planes=" << gmb_handle.native_pixmap_handle.planes.size() << ", expected num_planes=" << num_planes;
//             return nullptr;
//         }
//         for (size_t i = 0; i < num_planes; ++i) {
...
// #endif
// 
//     const auto layout = VideoFrameLayout::CreateWithPlanes(*format, coded_size, std::move(planes), VideoFrameLayout::kBufferAddressAlignment, modifier);
//     if (!layout) {
...
//     frame->mailbox_holder_and_gmb_release_cb_ = std::move(mailbox_holder_and_gmb_release_cb);
//     frame->is_mappable_si_enabled_ = enable_mappable_si;
//     return frame;
}

// static
scoped_refptr<VideoFrame> VideoFrame::WrapSharedImage(VideoPixelFormat format, scoped_refptr<gpu::ClientSharedImage> shared_image, gpu::SyncToken sync_token,
    ReleaseMailboxCB mailbox_holder_release_cb, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    base::TimeDelta timestamp)
{
    *(int*)1 = 1;
    return nullptr;
//     scoped_refptr<VideoFrame> frame = CreateFrameForNativeTexturesInternal(format, coded_size, visible_rect, natural_size, timestamp);
//     if (!frame) {
//         return nullptr;
//     }
// 
...
//     }
//     frame->mailbox_holder_and_gmb_release_cb_ = WrapReleaseMailboxCB(std::move(mailbox_holder_release_cb));
// 
...
scoped_refptr<VideoFrame> VideoFrame::WrapMappableSharedImage(scoped_refptr<gpu::ClientSharedImage> shared_image, gpu::SyncToken sync_token,
    ReleaseMailboxAndGpuMemoryBufferCB mailbox_holder_and_gmb_release_cb, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    base::TimeDelta timestamp)
{
    *(int*)1 = 1;
    return nullptr;
//     CHECK(shared_image);
//     scoped_refptr<VideoFrame> frame = CreateFrameForGpuMemoryBufferOrMappableSIInternal(visible_rect, natural_size, /*gpu_memory_buffer=*/nullptr, shared_image,
//         /*enable_mappable_si=*/true, std::move(mailbox_holder_and_gmb_release_cb), timestamp);
//     if (!frame) {
//         return nullptr;
//     }
//     frame->acquire_sync_token_ = sync_token;
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, const uint8_t* data, size_t data_size, base::TimeDelta timestamp)
{
    auto layout = GetDefaultLayout(format, coded_size);
    if (!layout)
        return nullptr;
    return WrapExternalDataWithLayout(*layout, visible_rect, natural_size, data, data_size, timestamp);
}

// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalDataWithLayout(const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    const uint8_t* data, size_t data_size, base::TimeDelta timestamp)
{
    StorageType storage_type = STORAGE_UNOWNED_MEMORY;

    if (!IsValidConfig(layout.format(), storage_type, layout.coded_size(), visible_rect, natural_size) || !layout.FitsInContiguousBufferOfSize(data_size)) {
        DLOG(ERROR) << "Invalid config: " << ConfigToString(layout.format(), storage_type, layout.coded_size(), visible_rect, natural_size);
        return nullptr;
    }

    scoped_refptr<VideoFrame> frame = new VideoFrame(layout, storage_type, visible_rect, natural_size, timestamp);
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, int32_t y_stride, int32_t u_stride, int32_t v_stride, const uint8_t* y_data, const uint8_t* u_data, const uint8_t* v_data,
    base::TimeDelta timestamp)
{
    auto layout = VideoFrameLayout::CreateWithStrides(format, coded_size, { y_stride, u_stride, v_stride });
    if (!layout) {
        DLOG(ERROR) << "Invalid layout.";
        return nullptr;
    }
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvDataWithLayout(const VideoFrameLayout& layout, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, const uint8_t* y_data, const uint8_t* u_data, const uint8_t* v_data, base::TimeDelta timestamp)
{
    const StorageType storage = STORAGE_UNOWNED_MEMORY;
    const VideoPixelFormat format = layout.format();
    if (!IsValidConfig(format, storage, layout.coded_size(), visible_rect, natural_size)) {
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvaData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, int32_t y_stride, int32_t u_stride, int32_t v_stride, int32_t a_stride, const uint8_t* y_data, const uint8_t* u_data,
    const uint8_t* v_data, const uint8_t* a_data, base::TimeDelta timestamp)
{
    const StorageType storage = STORAGE_UNOWNED_MEMORY;
    if (!IsValidConfig(format, storage, coded_size, visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(format, storage, coded_size, visible_rect, natural_size);
    ...
    auto layout = VideoFrameLayout::CreateWithStrides(format, coded_size, { y_stride, u_stride, v_stride, a_stride });
    if (!layout) {
        DLOG(ERROR) << "Invalid layout";
        return nullptr;
    }
...
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalYuvData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, int32_t y_stride, int32_t uv_stride, const uint8_t* y_data, const uint8_t* uv_data, base::TimeDelta timestamp)
{
    const StorageType storage = STORAGE_UNOWNED_MEMORY;
    if (!IsValidConfig(format, storage, coded_size, visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(format, storage, coded_size, visible_rect, natural_size);
    ...
    auto layout = VideoFrameLayout::CreateWithStrides(format, coded_size, { y_stride, uv_stride });
    if (!layout) {
        DLOG(ERROR) << "Invalid layout";
        return nullptr;
    }
...
scoped_refptr<VideoFrame> VideoFrame::WrapExternalGpuMemoryBuffer(
    const gfx::Rect& visible_rect, const gfx::Size& natural_size, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, base::TimeDelta timestamp)
{
    return CreateFrameForGpuMemoryBufferOrMappableSIInternal(visible_rect, natural_size, std::move(gpu_memory_buffer),
        /*shared_image=*/nullptr,
        /*enable_mappable_si=*/false, base::NullCallback(), timestamp);
}

// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalGpuMemoryBuffer(const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer, scoped_refptr<gpu::ClientSharedImage> shared_image, const gpu::SyncToken& sync_token,
    ReleaseMailboxAndGpuMemoryBufferCB mailbox_holder_and_gmb_release_cb, base::TimeDelta timestamp)
{
    *(int*)1 = 1;
    return nullptr;
//     scoped_refptr<VideoFrame> frame = CreateFrameForGpuMemoryBufferOrMappableSIInternal(visible_rect, natural_size, std::move(gpu_memory_buffer),
//         /*shared_image=*/nullptr,
//         /*enable_mappable_si=*/false, std::move(mailbox_holder_and_gmb_release_cb), timestamp);
//     if (!frame) {
//         return nullptr;
//     }
// 
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
// static
scoped_refptr<VideoFrame> VideoFrame::WrapExternalDmabufs(const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    std::vector<base::ScopedFD> dmabuf_fds, base::TimeDelta timestamp)
{
    ...
    if (dmabuf_fds.empty() || dmabuf_fds.size() > NumPlanes(format)) {
        DLOG(ERROR) << __func__ << " Incorrect number of dmabuf fds provided, got: " << dmabuf_fds.size() << ", expected 1 to " << NumPlanes(format);
        return nullptr;
    }
    ...
    frame->mailbox_holder_and_gmb_release_cb_ = ReleaseMailboxAndGpuMemoryBufferCB();
...
#if BUILDFLAG(IS_APPLE)
// static
scoped_refptr<VideoFrame> VideoFrame::WrapUnacceleratedIOSurface(gfx::GpuMemoryBufferHandle handle, const gfx::Rect& visible_rect, base::TimeDelta timestamp)
{
    ...
    const gfx::Size size(IOSurfaceGetWidth(io_surface.get()), IOSurfaceGetHeight(io_surface.get()));
    std::vector<int32_t> strides;
    for (size_t i = 0; i < num_planes; ++i)
        strides.push_back(IOSurfaceGetBytesPerRowOfPlane(io_surface.get(), i));
    std::optional<VideoFrameLayout> layout = media::VideoFrameLayout::CreateWithStrides(pixel_format, size, strides);
    if (!layout) {
        DLOG(ERROR) << "Invalid layout.";
        return nullptr;
    }
    ...
    kern_return_t lock_result = IOSurfaceLock(io_surface.get(), kIOSurfaceLockReadOnly, nullptr);
    if (lock_result != kIOReturnSuccess) {
        DLOG(ERROR) << "Failed to lock IOSurface.";
        return nullptr;
    }
    auto unlock_lambda = [](base::apple::ScopedCFTypeRef<IOSurfaceRef> io_surface) { IOSurfaceUnlock(io_surface.get(), kIOSurfaceLockReadOnly, nullptr); };

    scoped_refptr<VideoFrame> frame = new VideoFrame(*layout, storage_type, visible_rect, size, timestamp);
    for (size_t i = 0; i < num_planes; ++i) {
        frame->data_[i] = reinterpret_cast<uint8_t*>(IOSurfaceGetBaseAddressOfPlane(io_surface.get(), i));
    }
    frame->AddDestructionObserver(base::BindOnce(unlock_lambda, std::move(io_surface)));
    return frame;
}

// static
scoped_refptr<VideoFrame> VideoFrame::WrapCVPixelBuffer(CVPixelBufferRef cv_pixel_buffer, base::TimeDelta timestamp)
{
    ...
    } else if (cv_format == kCVPixelFormatType_420YpCbCr8VideoRange_8A_TriPlanar) {
...
scoped_refptr<VideoFrame> VideoFrame::WrapVideoFrame(
    scoped_refptr<VideoFrame> frame, VideoPixelFormat format, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    if (!AreValidPixelFormatsForWrap(frame->format(), format)) {
        DLOG(ERROR) << __func__ << " Invalid format conversion." << VideoPixelFormatToString(frame->format()) << " to " << VideoPixelFormatToString(format);
        return nullptr;
    }

    if (!IsValidConfig(format, frame->storage_type(), frame->coded_size(), visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(format, frame->storage_type(), frame->coded_size(), visible_rect, natural_size);
        return nullptr;
    }
    ...
    if (format == frame->format()) {
        ...
        if (new_plane_count > new_planes.size()) {
            DLOG(ERROR) << " Wrapping frame has more planes than old one."
                        << " old plane count: " << new_planes.size() << " new plane count: " << new_plane_count;
            return nullptr;
        }
        new_planes.resize(new_plane_count);
        new_layout = VideoFrameLayout::CreateWithPlanes(format, frame->coded_size(), new_planes);
    }
    ...
    scoped_refptr<VideoFrame> wrapping_frame(new VideoFrame(new_layout.value(), frame->storage_type(), visible_rect, natural_size, frame->timestamp()));
...
scoped_refptr<VideoFrame> VideoFrame::CreateEOSFrame()
{
    ...
    scoped_refptr<VideoFrame> frame = new VideoFrame(*layout, STORAGE_UNKNOWN, gfx::Rect(), gfx::Size(), kNoTimestamp, FrameControlType::kEos);
    frame->metadata().end_of_stream = true;
    return frame;
}

// static
scoped_refptr<VideoFrame> VideoFrame::CreateColorFrame(const gfx::Size& size, uint8_t y, uint8_t u, uint8_t v, base::TimeDelta timestamp)
{
    scoped_refptr<VideoFrame> frame = CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, timestamp);
    if (frame)
...
// static
scoped_refptr<VideoFrame> VideoFrame::CreateTransparentFrame(const gfx::Size& size)
{
    const uint8_t kBlackY = 0x00;
    const uint8_t kBlackUV = 0x00;
    const uint8_t kTransparentA = 0x00;
    const base::TimeDelta kZero;
    scoped_refptr<VideoFrame> frame = CreateFrame(PIXEL_FORMAT_I420A, size, gfx::Rect(size), size, kZero);
    if (frame)
...
// static
size_t VideoFrame::AllocationSize(VideoPixelFormat format, const gfx::Size& coded_size)
{
...
// static
gfx::Size VideoFrame::PlaneSize(VideoPixelFormat format, size_t plane, const gfx::Size& coded_size)
{
...
// static
gfx::Size VideoFrame::PlaneSizeInSamples(VideoPixelFormat format, size_t plane, const gfx::Size& coded_size)
{
    DCHECK(IsValidPlane(format, plane));
    const gfx::Size subsample_size = SampleSize(format, plane);
    return gfx::Size(base::bits::AlignUpDeprecatedDoNotUse(coded_size.width(), subsample_size.width()) / subsample_size.width(),
        base::bits::AlignUpDeprecatedDoNotUse(coded_size.height(), subsample_size.height()) / subsample_size.height());
}

// static
int VideoFrame::PlaneHorizontalBitsPerPixel(VideoPixelFormat format, size_t plane)
{
    DCHECK(IsValidPlane(format, plane));
...
// static
std::vector<int32_t> VideoFrame::ComputeStrides(VideoPixelFormat format, const gfx::Size& coded_size)
{
    std::vector<int32_t> strides;
    ...
    if (num_planes == 1) {
        ...
        for (size_t plane = 0; plane < num_planes; ++plane) {
            strides.push_back(base::bits::AlignUp(RowBytes(plane, format, coded_size.width()), kFrameAddressAlignment));
        }
    }
    return strides;
}
...
size_t VideoFrame::Rows(size_t plane, VideoPixelFormat format, int height)
{
    ...
    return base::bits::AlignUpDeprecatedDoNotUse(height, sample_height) / sample_height;
}
...
size_t VideoFrame::Columns(size_t plane, VideoPixelFormat format, int width)
{
    ...
    return base::bits::AlignUpDeprecatedDoNotUse(width, sample_width) / sample_width;
}

// static
void VideoFrame::HashFrameForTesting(base::MD5Context* context, const VideoFrame& frame)
{
    *(int*)1 = 1;
//     DCHECK(context);
//     for (size_t plane = 0; plane < NumPlanes(frame.format()); ++plane) {
//         for (int row = 0; row < frame.rows(plane); ++row) {
//             base::MD5Update(context, base::span<const uint8_t>(frame.data(plane) + frame.stride(plane) * row, static_cast<size_t>(frame.row_bytes(plane))));
//         }
//     }
}

void VideoFrame::BackWithSharedMemory(const base::ReadOnlySharedMemoryRegion* region)
{
    DCHECK(!shm_region_);
    ...
    DCHECK(storage_type_ == STORAGE_UNOWNED_MEMORY || storage_type_ == STORAGE_SHMEM);
    DCHECK(region && region->IsValid());
    storage_type_ = STORAGE_SHMEM;
    shm_region_ = region;
}

void VideoFrame::BackWithOwnedSharedMemory(base::ReadOnlySharedMemoryRegion region, base::ReadOnlySharedMemoryMapping mapping)
{
    DCHECK(!shm_region_);
    DCHECK(!owned_shm_region_.IsValid());
...
{
    return wrapped_frame_ ? wrapped_frame_->HasSharedImage() : shared_image_ != nullptr;
}
...
{
    return wrapped_frame_ ? wrapped_frame_->GetGpuMemoryBuffer() : gpu_memory_buffer_.get();
}

std::unique_ptr<VideoFrame::ScopedMapping> VideoFrame::MapGMBOrSharedImage() const
{
    if (wrapped_frame_) {
    ...
    if (is_mappable_si_enabled_) {
        ...
        if (auto mapping = shared_image_->Map()) {
            return base::WrapUnique(new VideoFrame::ScopedMapping(nullptr, std::move(mapping)));
        }
    }
    if (gpu_memory_buffer_ && gpu_memory_buffer_->Map()) {
        return base::WrapUnique(new VideoFrame::ScopedMapping(gpu_memory_buffer_.get(), nullptr));
    }
    return nullptr;
}

void VideoFrame::MapGMBOrSharedImageAsync(base::OnceCallback<void(std::unique_ptr<VideoFrame::ScopedMapping>)> result_cb) const
{
    if (wrapped_frame_) {
    ...
    if (is_mappable_si_enabled_) {
        ...
        shared_image_->MapAsync(base::BindOnce(&VideoFrame::WrapScopedSharedImageMapping, base::Unretained(this), std::move(result_cb)));
        return;
    }
    if (gpu_memory_buffer_) {
        // `base::Unretained()` is safe because of the requirement for callers to
        // keep the VideoFrame alive until the callback executes.
        gpu_memory_buffer_->MapAsync(base::BindOnce(&VideoFrame::MakeScopedMappingForGpuMemoryBuffer, base::Unretained(this), std::move(result_cb)));
        return;
    }
...
bool VideoFrame::IsSameAllocation(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size) const
{
    ...
    return this->format() == format && this->coded_size() == new_coded_size && visible_rect_ == visible_rect && natural_size_ == natural_size;
}
...
template <typename T> T VideoFrame::GetVisibleDataInternal(T data, size_t plane) const
{
    DCHECK(IsValidPlane(format(), plane));
    DCHECK(IsMappable());
    if (!data) [[unlikely]] {
    ...
    const gfx::Point offset(base::bits::AlignDownDeprecatedDoNotUse(visible_rect_.x(), alignment.width()),
        base::bits::AlignDownDeprecatedDoNotUse(visible_rect_.y(), alignment.height()));

    const gfx::Size subsample = SampleSize(format(), plane);
    DCHECK(offset.x() % subsample.width() == 0);
    DCHECK(offset.y() % subsample.height() == 0);
    return data + stride(plane) * (offset.y() / subsample.height()) + // Row offset.
...
{
    CHECK(HasSharedImage());
    return wrapped_frame_ ? wrapped_frame_->acquire_sync_token() : acquire_sync_token_;
}
...
void VideoFrame::SetReleaseMailboxCB(ReleaseMailboxCB release_mailbox_cb)
{
    ...
    mailbox_holder_and_gmb_release_cb_ = WrapReleaseMailboxCB(std::move(release_mailbox_cb));
}

void VideoFrame::SetReleaseMailboxAndGpuMemoryBufferCB(ReleaseMailboxAndGpuMemoryBufferCB release_mailbox_cb)
{
...
{
    return wrapped_frame_ ? wrapped_frame_->HasReleaseMailboxCB() : !!mailbox_holder_and_gmb_release_cb_;
}
...
std::string VideoFrame::AsHumanReadableString() const
{
    ...
    std::ostringstream s;
    s << ConfigToString(format(), storage_type_, coded_size(), visible_rect_, natural_size_) << " timestamp:" << timestamp_.InMicroseconds();
    if (HasSharedImage()) {
        s << " shared_image: true";
    }
    return s.str();
...
VideoFrame::VideoFrame(const VideoFrameLayout& layout, StorageType storage_type, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    base::TimeDelta timestamp, FrameControlType frame_control_type)
    : layout_(layout)
...
{
    DCHECK(IsValidConfigInternal(format(), frame_control_type, coded_size(), visible_rect_, natural_size_));
    DCHECK(visible_rect_ == visible_rect) << "visible_rect " << visible_rect.ToString() << " exceeds coded_size " << coded_size().ToString();
...
VideoFrame::~VideoFrame()
{
    if (mailbox_holder_and_gmb_release_cb_) {
        ...
        std::move(mailbox_holder_and_gmb_release_cb_).Run(release_sync_token, std::move(gpu_memory_buffer_));
    }
...
// static
std::string VideoFrame::ConfigToString(
    const VideoPixelFormat format, const StorageType storage_type, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    return base::StringPrintf("format:%s storage_type:%s coded_size:%s visible_rect:%s natural_size:%s", VideoPixelFormatToString(format).c_str(),
        StorageTypeToString(storage_type).c_str(), coded_size.ToString().c_str(), visible_rect.ToString().c_str(), natural_size.ToString().c_str());
}

// static
gfx::Size VideoFrame::DetermineAlignedSize(VideoPixelFormat format, const gfx::Size& dimensions)
{
    const gfx::Size alignment = CommonAlignment(format);
    const gfx::Size adjusted = gfx::Size(base::bits::AlignUpDeprecatedDoNotUse(dimensions.width(), alignment.width()),
        base::bits::AlignUpDeprecatedDoNotUse(dimensions.height(), alignment.height()));
    DCHECK((adjusted.width() % alignment.width() == 0) && (adjusted.height() % alignment.height() == 0));
    return adjusted;
}

// static
bool VideoFrame::IsValidSize(const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
    return IsValidCodedSize(coded_size) && IsValidCodedSize(natural_size)
        && !(visible_rect.x() < 0 || visible_rect.y() < 0 || visible_rect.right() > coded_size.width() || visible_rect.bottom() > coded_size.height());
}
...
bool VideoFrame::IsValidCodedSize(const gfx::Size& size)
{
    ...
    return size_area <= limits::kMaxCanvas && size.width() <= limits::kMaxDimension && size.height() <= limits::kMaxDimension;
}

// static
bool VideoFrame::IsValidConfigInternal(
    VideoPixelFormat format, FrameControlType frame_control_type, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size)
{
...
// static
std::optional<VideoFrameLayout> VideoFrame::CreateFullySpecifiedLayoutWithStrides(VideoPixelFormat format, const gfx::Size& coded_size)
{
    const gfx::Size new_coded_size = DetermineAlignedSize(format, coded_size);
    auto layout = VideoFrameLayout::CreateWithStrides(format, new_coded_size, ComputeStrides(format, new_coded_size));
    if (!layout) {
        return {};
    }
    ...
    for (size_t plane = 0; plane < plane_sizes.size(); plane++) {
        new_planes.emplace_back(layout->planes()[plane].stride, offset, plane_sizes[plane]);
        offset += plane_sizes[plane];
    }
    return VideoFrameLayout::CreateWithPlanes(format, new_coded_size, std::move(new_planes));
}

// static
scoped_refptr<VideoFrame> VideoFrame::CreateFrameInternal(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    const gfx::Size& natural_size, base::TimeDelta timestamp, bool zero_initialize_memory)
{
    ...
    return CreateFrameWithLayout(*layout, visible_rect, natural_size, timestamp, zero_initialize_memory);
}

scoped_refptr<VideoFrame> VideoFrame::CreateFrameWithLayout(
    const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp, bool zero_initialize_memory)
{
    const StorageType storage = STORAGE_OWNED_MEMORY;
    if (!IsValidConfig(layout.format(), storage, layout.coded_size(), visible_rect, natural_size)) {
        DLOG(ERROR) << __func__ << " Invalid config." << ConfigToString(layout.format(), storage, layout.coded_size(), visible_rect, natural_size);
        return nullptr;
    }

    scoped_refptr<VideoFrame> frame(new VideoFrame(std::move(layout), storage, visible_rect, natural_size, timestamp));
...
bool VideoFrame::AllocateMemory(bool zero_initialize_memory)
{
    ...
    const size_t buffer_size = std::accumulate(plane_size.begin(), plane_size.end(), 0u);
    const size_t allocation_size = buffer_size + (layout_.buffer_addr_align() - 1);

    uint8_t* data = nullptr;
    if (zero_initialize_memory) {
        if (!base::UncheckedCalloc(1, allocation_size, reinterpret_cast<void**>(&data)) || !data) {
            return false;
        }
    } else {
        if (!base::UncheckedMalloc(allocation_size, reinterpret_cast<void**>(&data)) || !data) {
            return false;
        }
    }
...
// static
std::vector<size_t> VideoFrame::CalculatePlaneSize(const VideoFrameLayout& layout)
{
    ...
    for (size_t plane = 0; plane < num_planes; ++plane) {
        ...
        const size_t height = base::bits::AlignUp(Rows(plane, format, layout.coded_size().height()), kFrameAddressAlignment);
    ...
    if (num_planes > 1) {
        ...
        plane_size.back() += std::abs(layout.planes()[Plane::kU].stride) + kFrameSizePadding;
    }
    return plane_size;
}
...
VideoFrame::ScopedMapping::ScopedMapping(gfx::GpuMemoryBuffer* gpu_memory_buffer, std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> scoped_mapping)
...
{
    return static_cast<uint8_t*>(gpu_memory_buffer_ ? gpu_memory_buffer_->memory(plane_index) : scoped_mapping_->GetMemoryForPlane(plane_index).data());
}

size_t VideoFrame::ScopedMapping::Stride(uint32_t plane_index)
{
    return gpu_memory_buffer_ ? base::checked_cast<size_t>(gpu_memory_buffer_->stride(plane_index)) : scoped_mapping_->Stride(plane_index);
}

gfx::Size VideoFrame::ScopedMapping::Size()
{
    return gpu_memory_buffer_ ? gpu_memory_buffer_->GetSize() : scoped_mapping_->Size();
}
void VideoFrame::MakeScopedMappingForGpuMemoryBuffer(base::OnceCallback<void(std::unique_ptr<VideoFrame::ScopedMapping>)> result_cb, bool success) const
{
    std::move(result_cb).Run(success ? base::WrapUnique(new VideoFrame::ScopedMapping(gpu_memory_buffer_.get(), nullptr)) : nullptr);
}

void VideoFrame::WrapScopedSharedImageMapping(
    base::OnceCallback<void(std::unique_ptr<VideoFrame::ScopedMapping>)> result_cb, std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> mapping) const
{
    std::move(result_cb).Run(mapping ? base::WrapUnique(new VideoFrame::ScopedMapping(nullptr, std::move(mapping))) : nullptr);
}
