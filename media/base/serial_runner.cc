...
static void RunClosure(base::OnceClosure closure, PipelineStatusCallback status_cb)
{
...
static void RunBoundClosure(SerialRunner::BoundClosure bound_closure, PipelineStatusCallback status_cb)
{
    std::move(bound_closure).Run(base::BindOnce(std::move(status_cb), PIPELINE_OK));
}

// Runs |status_cb| with |last_status| on |task_runner|.
static void RunOnTaskRunner(const scoped_refptr<base::SequencedTaskRunner>& task_runner, PipelineStatusCallback status_cb, PipelineStatus last_status)
{
    // Force post to permit cancellation of a series in the scenario where all
    // bound functions run on the same thread.
    task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(status_cb), last_status));
}
...
{
    bound_fns_.push_back(base::BindOnce(&RunBoundClosure, std::move(bound_closure)));
}
    ...
    , done_cb_(std::move(done_cb))
{
    ...
    // when empty.
    if (bound_fns_.empty()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&SerialRunner::RunNextInSeries, weak_factory_.GetWeakPtr(), PIPELINE_OK));
        return;
    }
...
std::unique_ptr<SerialRunner> SerialRunner::Run(Queue&& bound_fns, PipelineStatusCallback done_cb)
{
    std::unique_ptr<SerialRunner> callback_series(new SerialRunner(std::move(bound_fns), std::move(done_cb)));
...
void SerialRunner::RunNextInSeries(PipelineStatus last_status)
{
    ...
    std::move(bound_fn).Run(base::BindOnce(&RunOnTaskRunner, task_runner_, base::BindRepeating(&SerialRunner::RunNextInSeries, weak_factory_.GetWeakPtr())));
}
