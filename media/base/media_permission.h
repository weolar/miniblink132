...
public:
    ...
    // has never been set.
    virtual void HasPermission(Type type, PermissionStatusCB permission_status_cb) = 0;

    // Requests |type| permission. This may trigger user interaction
    // (e.g. permission prompt) if the permission has never been set.
    virtual void RequestPermission(Type type, PermissionStatusCB permission_status_cb) = 0;
    ...
    virtual void IsHardwareSecureDecryptionAllowed(IsHardwareSecureDecryptionAllowedCB cb) = 0;
#endif // BUILDFLAG(IS_WIN)
};
