...
public:
    ...
    // |media::kNoTimestamp|.
    BitstreamBuffer(int32_t id, base::UnsafeSharedMemoryRegion region, size_t size, uint64_t offset = 0, base::TimeDelta presentation_timestamp = kNoTimestamp);
    ...
    void SetDecryptionSettings(const std::string& key_id, const std::string& iv, const std::vector<SubsampleEntry>& subsamples);

    // Taking the region invalidates the one in this BitstreamBuffer.
    base::UnsafeSharedMemoryRegion TakeRegion()
    {
