...
public:
    ...
    virtual AudioRendererMixer* GetMixer(const base::UnguessableToken& owner_token, const AudioParameters& input_params, AudioLatency::LatencyType latency,
        const OutputDeviceInfo& sink_info, scoped_refptr<AudioRendererSink> sink)
        = 0;
    ...
    virtual scoped_refptr<AudioRendererSink> GetSink(const base::UnguessableToken& owner_token, const std::string& device_id) = 0;
};
