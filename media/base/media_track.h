...
public:
    ...
    enum class VideoKind { kNone, kAlternative, kCaptions, kMain, kSign, kSubtitles, kCommentary };

    enum class AudioKind { kNone, kAlternative, kDescriptions, kMain, kMainDescriptions, kTranslation, kCommentary };
    ...
    static MediaTrack CreateVideoTrack(
        const std::string& id, VideoKind kind, const std::string& label, const std::string& language, bool enabled, StreamParser::TrackId stream_id = 0);

    static MediaTrack CreateAudioTrack(const std::string& id, AudioKind kind, const std::string& label, const std::string& language, bool enabled,
        StreamParser::TrackId stream_id = 0, bool exclusive = false);
...
private:
    ...
    MediaTrack(Type type, StreamParser::TrackId stream_id, const Id& track_id, const Kind& kind, const Label& label, const Language& lang, bool enabled,
        bool exclusive);
