...
public:
    explicit UselessThingToBeSerialized(const char* name)
        : name_(name)
    {
    }
    const char* name_;
...
template <> struct MediaSerializer<UselessThingToBeSerialized> {
...
struct CustomDefaultValue {
    ...
    static constexpr Codes OkEnumValue()
    {
        return Codes::kFoo;
    }
};
...
struct TraitsWithDataPacking {
    ...
    static constexpr StatusGroupType Group()
    {
        return "GroupWithDataPacking";
    }
    static void OnCreateFrom(TypedStatus<TraitsWithDataPacking>* status, const PackThis& data)
    {
...
public:
    ...
    template <typename T> NormalStatus FailWithData(const char* key, const T& t)
    {
        return NormalStatus(NormalStatus::Codes::kFoo, "Message", FROM_HERE).WithData(key, t);
    }
    ...
    static TypedStatus<MapValueCodeTraits>::Or<std::unique_ptr<int>> GetStartingValue(int key)
    {
        switch (key) {
        case 0:
    ...
    static TypedStatus<MapValueCodeTraits>::Or<int> UnwrapPtr(std::unique_ptr<int> v)
    {
        if (!v)
...
TEST_F(StatusTest, DifferentModesOfConstruction)
{
    ...
    ASSERT_EQ(unpacked->FindInt("code").value_or(0), static_cast<int>(NormalStatus::Codes::kFoo));
...
TEST_F(StatusTest, DerefOpOnOrType)
{
    ...
    NormalStatus::Or<std::unique_ptr<SimpleThing>> sor = std::make_unique<SimpleThing>();
...
TEST_F(StatusTest, SingleLayerError)
{
    ...
    ASSERT_THAT(*stack[0].GetDict().FindString("file"), HasSubstr("status_unittest.cc"));
}
...
{
    NormalStatus failed = FailWithData("example", UselessThingToBeSerialized("F"));
    base::Value actual = MediaSerializeForTesting(failed);
    const base::Value::Dict& actual_dict = actual.GetDict();
    ASSERT_EQ(actual_dict.size(), 5ul);
    ASSERT_EQ(*actual_dict.FindString("message"), "Message");
...
TEST_F(StatusTest, CausedByCanAssignCopy)
{
    ...
    base::Value::Dict* copied = copy_causal_serialized.GetDict().FindDict("cause");
...
TEST_F(StatusTest, StatusOrMoveConstructionAndAssignment)
{
    ...
    NormalStatus::Or<std::unique_ptr<int>> status_or_0(std::make_unique<int>(123));
...
{
    EXPECT_FALSE(TypedStatus<NoOkStatusTypeTraits>(NoOkStatusTypeTraits::Codes::kFoo).is_ok());

    EXPECT_FALSE(TypedStatus<ZeroValueOkTypeTraits>(ZeroValueOkTypeTraits::Codes::kFoo).is_ok());
    EXPECT_TRUE(TypedStatus<ZeroValueOkTypeTraits>(ZeroValueOkTypeTraits::Codes::kOk).is_ok());

    EXPECT_FALSE(TypedStatus<NonZeroOkTypeTraits>(NonZeroOkTypeTraits::Codes::kFoo).is_ok());
    EXPECT_TRUE(TypedStatus<NonZeroOkTypeTraits>(NonZeroOkTypeTraits::Codes::kOk).is_ok());
}

TEST_F(StatusTest, MustHaveOkOrHelperMethod)
{
    static_assert(internal::StatusTraitsHelper<CustomDefaultValue>::has_default, "WOW");
...
TEST_F(StatusTest, OkStatusInitializesToOk)
{
    ...
    // a return value.
    EXPECT_EQ(TypedStatus<ZeroValueOkTypeTraits>(OkStatus()).code(), ZeroValueOkTypeTraits::Codes::kOk);
    EXPECT_EQ(TypedStatus<NonZeroOkTypeTraits>(OkStatus()).code(), NonZeroOkTypeTraits::Codes::kOk);
}
...
{
    NormalStatus::Or<std::string> failed = FailEasily();
    NormalStatus::Or<int> failed_int = std::move(failed).MapValue([](std::string value) { return atoi(value.c_str()); });
    ASSERT_TRUE(failed_int == NormalStatus::Codes::kFoo);

    // Try it with a c++ lambda
    NormalStatus::Or<std::string> success = std::string("12345");
    NormalStatus::Or<int> success_int = std::move(success).MapValue([](std::string value) { return atoi(value.c_str()); });
    ...
    auto case_4 = GetStartingValue(4).MapValue(UnwrapPtr).MapValue(FindIntSqrt).MapValue(FindIntSqrt);
...
TEST_F(StatusTest, UKMSerializerTest)
{
    ...
    struct MockUkmBuilder {
        ...
        void SetStatus(uint64_t data)
        {
            status.packed = data;
        }
        void SetRootCause(uint64_t data)
        {
            root.packed = data;
        }
    };
    ...
    ASSERT_EQ(builder.status.bits.code, static_cast<StatusCodeType>(NormalStatus::Codes::kFoo));
    ASSERT_EQ(builder.root.bits.code, static_cast<StatusCodeType>(NormalStatus::Codes::kFoo));
    ...
    ASSERT_EQ(builder.status.bits.code, static_cast<StatusCodeType>(SerializeStatus::Codes::kFoo));
    ASSERT_EQ(builder.status.bits.extra_data, 0u);
    ...
    ASSERT_EQ(builder.root.bits.code, static_cast<StatusCodeType>(SerializeStatus::Codes::kFoo));
    ASSERT_EQ(builder.root.bits.extra_data, 0u);
    ASSERT_NE(builder.status.packed, 0u);
    ASSERT_EQ(builder.status.bits.code, static_cast<StatusCodeType>(SerializeStatus::Codes::kBar));
    ASSERT_EQ(builder.status.bits.extra_data, 0u);
