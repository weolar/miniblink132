...
constexpr base::TimeDelta kUpdateKeyPressCountIntervalMs = base::Milliseconds(5);
...
uint32_t UserInputMonitorMac::GetKeyPressCount() const
{
    ...
    return CGEventSourceCounterForEventType(kCGEventSourceStateHIDSystemState, kCGEventKeyDown);
}
...
void UserInputMonitorMac::StartKeyboardMonitoring(base::WritableSharedMemoryMapping mapping)
{
    key_press_count_mapping_ = std::make_unique<base::WritableSharedMemoryMapping>(std::move(mapping));
    key_press_count_timer_.Start(FROM_HERE, kUpdateKeyPressCountIntervalMs, this, &UserInputMonitorMac::UpdateKeyPressCountShmem);
}
...
std::unique_ptr<UserInputMonitor> UserInputMonitor::Create(
    scoped_refptr<base::SingleThreadTaskRunner> input_task_runner, scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
{
