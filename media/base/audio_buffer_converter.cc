...
static bool IsConfigChange(const AudioParameters& params, const AudioBuffer& buffer)
{
...
double AudioBufferConverter::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed, const AudioGlitchInfo& glitch_info)
{
    ...
    while (requested_frames_left > 0 && !queued_inputs_.empty()) {
        ...
        int frames_to_read = std::min(requested_frames_left, input_buffer->frame_count() - last_input_buffer_offset_);
        input_buffer->ReadFrames(frames_to_read, last_input_buffer_offset_, dest_index, audio_bus);
    ...
    if (is_flushing_ && requested_frames_left > 0) {
        audio_bus->ZeroFramesPartial(audio_bus->frames() - requested_frames_left, requested_frames_left);
    } else {
...
void AudioBufferConverter::ResetConverter(const AudioBuffer& buffer)
{
    Flush();
    audio_converter_.reset();
    input_params_.Reset(input_params_.format(), { buffer.channel_layout(), buffer.channel_count() }, buffer.sample_rate(),
        ...
        buffer.sample_rate() == output_params_.sample_rate() ? output_params_.frames_per_buffer() : SincResampler::kDefaultRequestSize);
    ...
    audio_converter_ = std::make_unique<AudioConverter>(input_params_, output_params_, true);
...
void AudioBufferConverter::ConvertIfPossible()
{
    ...
    auto output_buffer = AudioBuffer::CreateBuffer(
        kSampleFormatPlanarF32, output_params_.channel_layout(), output_params_.channels(), output_params_.sample_rate(), request_frames, pool_);
    ...
    while (frames_remaining != 0) {
        ...
        const int frames_this_iteration = std::min(static_cast<int>(SincResampler::kDefaultRequestSize), frames_remaining);
        ...
        for (int ch = 0; ch < output_buffer->channel_count(); ++ch) {
            output_bus->SetChannelData(ch, reinterpret_cast<float*>(output_buffer->channel_data()[ch]) + offset_into_buffer);
        }
