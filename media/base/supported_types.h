...
MEDIA_EXPORT void UpdateDefaultDecoderSupportedVideoProfiles(const base::flat_set<VideoCodecProfile>& profiles);
...
MEDIA_EXPORT void UpdateDefaultDecoderSupportedAudioTypes(const base::flat_set<AudioType>& types);

// This function lets the caller add additional video encoder codec profiles to
// those supported by default. Used primarily to add platform supported codec
// profiles once support is known.
MEDIA_EXPORT void UpdateDefaultEncoderSupportedVideoProfiles(const base::flat_set<VideoCodecProfile>& profiles);
