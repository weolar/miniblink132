...
// bytes.
void CreateTestY16Frame(const gfx::Size& coded_size, const gfx::Rect& visible_rect, void* memory)
{
...
void VerifyVideoFrameMetadataEquality(const media::VideoFrameMetadata& a, const media::VideoFrameMetadata& b)
{
...
void ExpectFrameColor(VideoFrame* yv12_frame, uint32_t expect_rgb_color)
{
    ...
    ASSERT_EQ(yv12_frame->stride(VideoFrame::Plane::kU), yv12_frame->stride(VideoFrame::Plane::kV));
    ASSERT_EQ(yv12_frame->coded_size().width() & (VideoFrame::kFrameSizeAlignment - 1), 0u);
    ASSERT_EQ(yv12_frame->coded_size().height() & (VideoFrame::kFrameSizeAlignment - 1), 0u);

    size_t bytes_per_row = yv12_frame->coded_size().width() * 4u;
    uint8_t* rgb_data = reinterpret_cast<uint8_t*>(
        base::AlignedAlloc(bytes_per_row * yv12_frame->coded_size().height() + VideoFrame::kFrameSizePadding, VideoFrame::kFrameAddressAlignment));

    libyuv::I420ToARGB(yv12_frame->data(VideoFrame::Plane::kY), yv12_frame->stride(VideoFrame::Plane::kY), yv12_frame->data(VideoFrame::Plane::kU),
        yv12_frame->stride(VideoFrame::Plane::kU), yv12_frame->data(VideoFrame::Plane::kV), yv12_frame->stride(VideoFrame::Plane::kV), rgb_data, bytes_per_row,
        yv12_frame->coded_size().width(), yv12_frame->coded_size().height());
...
void ExpectFrameExtents(VideoPixelFormat format, const char* expected_hash)
{
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateFrame(format, size, gfx::Rect(size), size, kTimestamp);
    ...
    for (int plane = 0; plane < planes; plane++) {
        ...
        memset(frame->writable_data(plane), kFillByte, frame->stride(plane) * frame->rows(plane));
    }
...
TEST(VideoFrame, CreateFrame)
{
    ...
    gfx::Size size(kWidth, kHeight);
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateFrame(PIXEL_FORMAT_YV12, size, gfx::Rect(size), size, kTimestamp);
    ASSERT_TRUE(frame.get());
    ...
    frame = VideoFrame::CreateFrame(PIXEL_FORMAT_ARGB, size, gfx::Rect(size), size, kTimestamp);
    EXPECT_EQ(PIXEL_FORMAT_ARGB, frame->format());
    EXPECT_GE(frame->stride(VideoFrame::Plane::kARGB), frame->coded_size().width());

    // Test double planar frame.
    frame = VideoFrame::CreateFrame(PIXEL_FORMAT_NV12, size, gfx::Rect(size), size, kTimestamp);
    ...
    frame = VideoFrame::CreateVideoHoleFrame(base::UnguessableToken::Create(), size, kTimestamp);
    ASSERT_TRUE(frame);
}
...
TEST(VideoFrame, CreateZeroInitializedFrame)
{
    ...
    // Create a YV12 Video Frame.
    gfx::Size size(kWidth, kHeight);
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateZeroInitializedFrame(PIXEL_FORMAT_YV12, size, gfx::Rect(size), size, kTimestamp);
    ASSERT_TRUE(frame.get());
...
TEST(VideoFrame, CreateBlackFrame)
{
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateBlackFrame(gfx::Size(kWidth, kHeight));
    ASSERT_TRUE(frame.get());
    EXPECT_TRUE(frame->IsMappable());
...
TEST(VideoFrame, DestructChainOfWrappedVideoFrames)
{
    ...
    frame->AddDestructionObserver(base::BindOnce(&FrameNoLongerNeededCallback, &base_frame_done_callback_was_run));
    std::array<bool, kFramesInChain> wrapped_frame_done_callback_was_run = {};
    std::vector<scoped_refptr<VideoFrame>> frames;

    for (int i = 0; i < kFramesInChain; i++) {
        frames.push_back(frame);
        frame = VideoFrame::WrapVideoFrame(frame, frame->format(), frame->visible_rect(), frame->natural_size());
        frame->AddDestructionObserver(base::BindOnce(&FrameNoLongerNeededCallback, &wrapped_frame_done_callback_was_run[i]));
    }
    frames.clear();

    EXPECT_FALSE(base_frame_done_callback_was_run);
    EXPECT_FALSE(std::accumulate(wrapped_frame_done_callback_was_run.begin(), wrapped_frame_done_callback_was_run.end(), true, std::logical_and<bool>()));

    frame.reset();
    EXPECT_TRUE(base_frame_done_callback_was_run);
    EXPECT_TRUE(std::accumulate(wrapped_frame_done_callback_was_run.begin(), wrapped_frame_done_callback_was_run.end(), true, std::logical_and<bool>()));
}
...
TEST(VideoFrame, WrapVideoFrame)
{
    ...
    bool wrapped_frame_done_callback_was_run = false;
    {
        ...
        frame = VideoFrame::WrapVideoFrame(base_frame, base_frame->format(), visible_rect, natural_size);
        base_frame->AddDestructionObserver(base::BindOnce(&FrameNoLongerNeededCallback, &base_frame_done_callback_was_run));
        ASSERT_TRUE(frame);
        EXPECT_EQ(base_frame->coded_size(), frame->coded_size());
        EXPECT_EQ(base_frame->data(VideoFrame::Plane::kY), frame->data(VideoFrame::Plane::kY));
        ...
        EXPECT_NE(base_frame->metadata().frame_duration.has_value(), frame->metadata().frame_duration.has_value());

        frame->AddDestructionObserver(base::BindOnce(&FrameNoLongerNeededCallback, &wrapped_frame_done_callback_was_run));

        visible_rect = gfx::Rect(0, 0, 1, 1);
        natural_size = visible_rect.size();
        frame2 = VideoFrame::WrapVideoFrame(frame, frame->format(), visible_rect, natural_size);
        ASSERT_TRUE(frame2);
        EXPECT_EQ(base_frame->coded_size(), frame2->coded_size());
        EXPECT_EQ(base_frame->data(VideoFrame::Plane::kY), frame2->data(VideoFrame::Plane::kY));
...

    {
        ...
        auto frame3 = VideoFrame::WrapVideoFrame(base_frame, base_frame->format(), larger_visible_rect, larger_visible_rect.size());
        ASSERT_TRUE(frame3);
        EXPECT_EQ(base_frame->coded_size(), frame3->coded_size());
        EXPECT_EQ(base_frame->data(VideoFrame::Plane::kY), frame3->data(VideoFrame::Plane::kY));
        ...
        EXPECT_FALSE(VideoFrame::WrapVideoFrame(base_frame, base_frame->format(), too_large_visible_rect, too_large_visible_rect.size()));
        // WrapVideoFrame() fails if the new visible rect is not contained by
        // |base_frame|'s coded size area.
        const gfx::Rect non_contained_visible_rect(3, 3, 2, 2);
        EXPECT_FALSE(VideoFrame::WrapVideoFrame(base_frame, base_frame->format(), non_contained_visible_rect, non_contained_visible_rect.size()));
    }
...
TEST(VideoFrame, WrapExternalData)
{
    ...
    auto frame = VideoFrame::WrapExternalData(PIXEL_FORMAT_Y16, coded_size, visible_rect, visible_rect.size(), memory, sizeof(memory), timestamp);
...
TEST(VideoFrame, WrapSharedMemory)
{
    ...
    base::MappedReadOnlyRegion mapped_region = base::ReadOnlySharedMemoryRegion::Create(kDataSize);
    ...
    auto frame = VideoFrame::WrapExternalData(
        PIXEL_FORMAT_Y16, coded_size, visible_rect, visible_rect.size(), mapped_region.mapping.GetMemoryAsSpan<uint8_t>().data(), kDataSize, timestamp);
...
TEST(VideoFrame, WrapExternalGpuMemoryBuffer)
{
...
#endif
    std::unique_ptr<gfx::GpuMemoryBuffer> gmb = std::make_unique<FakeGpuMemoryBuffer>(coded_size, gfx::BufferFormat::YUV_420_BIPLANAR, modifier);
    ...
    auto frame
        = VideoFrame::WrapExternalGpuMemoryBuffer(visible_rect, coded_size, std::move(gmb), shared_image, gpu::SyncToken(), base::DoNothing(), timestamp);
...
TEST(VideoFrame, WrapExternalDmabufs)
{
    ...
    auto timestamp = base::Milliseconds(1);
    auto layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, planes);
    ASSERT_TRUE(layout);
    std::vector<base::ScopedFD> dmabuf_fds(3u);
    auto frame = VideoFrame::WrapExternalDmabufs(*layout, visible_rect, visible_rect.size(), std::move(dmabuf_fds), timestamp);
    ...
    auto wrapped_frame = VideoFrame::WrapVideoFrame(frame, frame->format(), visible_rect, visible_rect.size());
    ...
    auto wrapped_frame2 = VideoFrame::WrapVideoFrame(wrapped_frame, frame->format(), visible_rect, visible_rect.size());
...
static void TextureCallback(gpu::SyncToken* called_sync_token, const gpu::SyncToken& release_sync_token)
{
...
{
    gpu::SyncToken called_sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1), 1);

    {
        scoped_refptr<gpu::ClientSharedImage> shared_image = gpu::ClientSharedImage::CreateForTesting();
        scoped_refptr<VideoFrame> frame = VideoFrame::WrapSharedImage(PIXEL_FORMAT_ARGB, shared_image, gpu::SyncToken(),
            base::BindOnce(&TextureCallback, &called_sync_token), gfx::Size(10, 10), // coded_size
...
TEST(VideoFrame, TexturesNoLongerNeededCallbackAfterTakingAndReleasingMailboxes)
{
    ...
    {
        scoped_refptr<VideoFrame> frame = VideoFrame::WrapSharedImage(PIXEL_FORMAT_I420, shared_image, sync_token,
            base::BindOnce(&TextureCallback, &called_sync_token), gfx::Size(10, 10), // coded_size
            gfx::Rect(10, 10), // visible_rect
            gfx::Size(10, 10), // natural_size
            base::TimeDelta()); // timestamp
        ...
        EXPECT_TRUE(frame->HasSharedImage());
        EXPECT_EQ(shared_image->mailbox().name[0], frame->shared_image()->mailbox().name[0]);
...
TEST(VideoFrame, IsValidConfig_OddCodedSize)
{
    ...
    EXPECT_TRUE(VideoFrame::IsValidConfig(PIXEL_FORMAT_I420, VideoFrame::STORAGE_OWNED_MEMORY, odd_size, gfx::Rect(odd_size), odd_size));

    // Next try a format with no sub-sampling for UV.
    EXPECT_TRUE(VideoFrame::IsValidConfig(PIXEL_FORMAT_I444, VideoFrame::STORAGE_OWNED_MEMORY, odd_size, gfx::Rect(odd_size), odd_size));
}
...
TEST(VideoFrame, CreateFrame_OddWidth)
{
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, odd_size, gfx::Rect(odd_size), odd_size, kTimestamp);
    ASSERT_TRUE(frame.get());
    ...
    frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I444, odd_size, gfx::Rect(odd_size), odd_size, kTimestamp);
    ASSERT_TRUE(frame.get());
...
TEST(VideoFrame, AllocationSize_OddSize)
{
    ...
    for (unsigned int i = 1u; i <= PIXEL_FORMAT_MAX; ++i) {
        ...
        case PIXEL_FORMAT_P410LE:
            EXPECT_EQ(90u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YUV422AP10:
            EXPECT_EQ(100u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_RGBAF16:
        case PIXEL_FORMAT_YUV444AP10:
            EXPECT_EQ(120u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YUV420AP10:
            EXPECT_EQ(84u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YUV422P9:
        case PIXEL_FORMAT_YUV422P10:
        case PIXEL_FORMAT_YUV422P12:
        case PIXEL_FORMAT_P210LE:
            EXPECT_EQ(70u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_I444:
        case PIXEL_FORMAT_NV24:
        case PIXEL_FORMAT_RGB24:
            EXPECT_EQ(45u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YUV420P9:
        case PIXEL_FORMAT_YUV420P10:
        case PIXEL_FORMAT_YUV420P12:
        case PIXEL_FORMAT_P010LE:
            EXPECT_EQ(54u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_Y16:
        case PIXEL_FORMAT_UYVY:
        case PIXEL_FORMAT_YUY2:
            EXPECT_EQ(30u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_NV16:
        case PIXEL_FORMAT_I422:
            EXPECT_EQ(35u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_YV12:
        case PIXEL_FORMAT_I420:
        case PIXEL_FORMAT_NV12:
        case PIXEL_FORMAT_NV21:
            EXPECT_EQ(27u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        ...
        case PIXEL_FORMAT_XB30:
            EXPECT_EQ(60u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_NV12A:
        case PIXEL_FORMAT_I420A:
            EXPECT_EQ(42u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
        case PIXEL_FORMAT_I422A:
            EXPECT_EQ(50u, VideoFrame::AllocationSize(format, size)) << VideoPixelFormatToString(format);
            break;
...
TEST(VideoFrame, NoFrameSizeExceedsUint32)
{
    ...
    for (unsigned int i = 1u; i <= PIXEL_FORMAT_MAX; ++i) {
        ...
        ASSERT_TRUE(VideoFrame::IsValidConfig(format, VideoFrame::STORAGE_UNOWNED_MEMORY, max_size, gfx::Rect(max_size), max_size));

        base::CheckedNumeric<uint32_t> allocation_size = VideoFrame::AllocationSize(format, max_size);
...
TEST(VideoFrame, WrapExternalDataWithInvalidLayout)
{
    ...
    uint8_t data = 0;
    auto frame = VideoFrame::WrapExternalDataWithLayout(*layout, gfx::Rect(coded_size), coded_size, &data, sizeof(data), base::TimeDelta());
    ASSERT_FALSE(frame);

    // Validate sum of planes exceeds data size.
    frame = VideoFrame::WrapExternalDataWithLayout(*layout, gfx::Rect(coded_size), coded_size, &data, sizes[0] + sizes[1], base::TimeDelta());
    ASSERT_FALSE(frame);

    // Validate offset exceeds plane size.
    planes[0].offset = 201;
    layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, planes);
    frame = VideoFrame::WrapExternalDataWithLayout(*layout, gfx::Rect(coded_size), coded_size, &data, sizes[0], base::TimeDelta());
    ASSERT_FALSE(frame);
}
