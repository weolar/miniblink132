...
public:
    ...
    base::TimeTicks ConvertMediaTime(base::TimeDelta timestamp, bool* is_time_moving)
    {
        ...
        *is_time_moving = time_source_.GetWallClockTimes(std::vector<base::TimeDelta>(1, timestamp), &wall_clock_times);
    ...
    bool IsWallClockNowForMediaTimeInSeconds(int seconds)
    {
        ...
        return tick_clock_->NowTicks() == ConvertMediaTime(base::Seconds(seconds), &is_time_moving);
    }
...
TEST_F(WallClockTimeSourceTest, SetMediaTime)
{
    ...
    EXPECT_TRUE(IsTimeStopped());
    std::vector<base::TimeTicks> wall_clock_times;
    time_source_.GetWallClockTimes(std::vector<base::TimeDelta>(), &wall_clock_times);
...
TEST_F(WallClockTimeSourceTest, ConvertsTimestampsWhenStopped)
{
    ...
    EXPECT_FALSE(is_time_moving);
    EXPECT_EQ(tick_clock_->NowTicks() + kOneSecond, ConvertMediaTime(kOneSecond, &is_time_moving));
    EXPECT_FALSE(is_time_moving);
}
...
{
    std::vector<base::TimeTicks> wall_clock_times;
    bool is_time_moving = time_source_.GetWallClockTimes(std::vector<base::TimeDelta>(), &wall_clock_times);
    EXPECT_FALSE(is_time_moving);
    EXPECT_EQ(base::TimeTicks(), wall_clock_times[0]);

    wall_clock_times.clear();
    time_source_.StartTicking();
    is_time_moving = time_source_.GetWallClockTimes(std::vector<base::TimeDelta>(), &wall_clock_times);
