...
public:
    ...
    void SetUp() final
    {
        fifo_ = std::make_unique<AudioPushFifo>(base::BindRepeating(&AudioPushFifoTest::ReceiveAndCheckNextChunk, base::Unretained(this)));
...
protected:
    ...
    void RunSimpleRechunkTest(int input_chunk_size)
    {
        ...
        EXPECT_EQ(GetExpectedOutputChunks(num_iterations * input_chunk_size), results_.size());
        ...
        const float last_value_in_last_chunk = static_cast<float>(GetExpectedOutputChunks(num_iterations * input_chunk_size) * output_chunk_size() - 1);
        ...
        for (int i = 1; i < audio_bus.frames(); ++i) {
            ...
            if (actual_value != expected_value) {
                if (actual_value == 0.0f) {
                    ...
                    ASSERT_EQ(expected_value, actual_value) << "Sample at offset " << i << " is incorrect.";
                }
            }
        }
...
TEST_P(AudioPushFifoTest, PushArbitraryNumbersOfFramesAtATime)
{
    ...
    ASSERT_FALSE(results_.empty());
    EXPECT_EQ(0.0f, results_.front().first_sample_value);
    const float last_value_in_last_chunk = static_cast<float>(GetExpectedOutputChunks(frames_pushed_so_far) * output_chunk_size() - 1);
    EXPECT_EQ(last_value_in_last_chunk, results_.back().last_sample_value);
...
INSTANTIATE_TEST_SUITE_P(All, AudioPushFifoTest,
    ::testing::Values(
