...
MEDIA_EXPORT uint32_t GetDefaultVideoEncodeBitrate(gfx::Size frame_size, uint32_t framerate);
...
public:
    ...
    using EncoderInfoCB = base::RepeatingCallback<void(const VideoEncoderInfo& encoder_info)>;

    // Callback for VideoEncoder to report an encoded video frame whenever it
    // becomes available.
    using OutputCB = base::RepeatingCallback<void(VideoEncoderOutput output, std::optional<CodecDescription>)>;
    ...
    virtual void Initialize(VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb) = 0;
    ...
    virtual void Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& options, EncoderStatusCB done_cb) = 0;
    ...
    // for it to finish.
    virtual void ChangeOptions(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb) = 0;
...
protected:
    template <typename Callback> Callback BindCallbackToCurrentLoopIfNeeded(Callback callback)
    {
        return post_callbacks_ ? base::BindPostTaskToCurrentDefault(std::move(callback)) : std::move(callback);
    }
