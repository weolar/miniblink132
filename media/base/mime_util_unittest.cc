...
// |single_value|.
static std::vector<bool> CreateTestVector(bool test_all_values, bool single_value)
{
...
static void RunCodecSupportTest(const MimeUtil::PlatformInfo& states_to_vary, const MimeUtil::PlatformInfo& test_states, TestCallback test_func)
{
#define MAKE_TEST_VECTOR(name) std::vector<bool> name##_states = CreateTestVector(states_to_vary.name, test_states.name)
...
#define RUN_TEST_VECTOR_BEGIN(name)                                                                                                                            \
    for (size_t name##_index = 0; name##_index < name##_states.size(); ++name##_index) {                                                                       \
...
#define RUN_TEST_VECTOR_END() }
    ...
    for (int codec = MimeUtil::INVALID_CODEC; codec <= MimeUtil::LAST_CODEC; ++codec) {
        SCOPED_TRACE(base::StringPrintf("has_platform_vp8_decoder=%d, "
                                        "has_platform_opus_decoder=%d, "
                                        "has_platform_vp9_decoder=%d, "
                                        "codec=%d",
            info.has_platform_vp8_decoder, info.has_platform_opus_decoder, info.has_platform_vp9_decoder, codec));
...
TEST(MimeUtilTest, SplitAndStripCodecs)
{
    const struct {
        ...
        { "\"bogus\"", 1, { "bogus" }, { "bogus" } },
        { "0", 1, { "0" }, { "0" } },
        { "avc1.42E01E, mp4a.40.2", 2, { "avc1.42E01E", "mp4a.40.2" }, { "avc1", "mp4a" } },
        { "\"mp4v.20.240, mp4a.40.2\"", 2, { "mp4v.20.240", "mp4a.40.2" }, { "mp4v", "mp4a" } },
...
TEST(MimeUtilTest, ParseVideoCodecString)
{
    ...
    {
        auto result = ParseVideoCodecString("video/webm", "vp09.02.10.10.01.06.06.06");
        ASSERT_TRUE(result);
        EXPECT_EQ(VideoCodec::kVP9, result->codec);
...
TEST(MimeUtilTest, ParseAudioCodecString)
{
    ...
    // Valid Opus string.
    EXPECT_TRUE(ParseAudioCodecString("audio/webm", "opus", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kOpus, out_codec);

    // Valid AAC string when proprietary codecs are supported.
    EXPECT_EQ(kUsePropCodecs, ParseAudioCodecString("audio/mp4", "mp4a.40.2", &out_is_ambiguous, &out_codec));
    if (kUsePropCodecs) {
        EXPECT_FALSE(out_is_ambiguous);
    ...
    EXPECT_TRUE(ParseAudioCodecString("audio/mp4", "flac", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    ...
    // MPEG4_AAC the same.
    EXPECT_EQ(kUsePropCodecs, ParseAudioCodecString("audio/mp4", "mp4a.40", &out_is_ambiguous, &out_codec));
    if (kUsePropCodecs) {
    ...
    EXPECT_TRUE(ParseAudioCodecString("audio/flac", "", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kFLAC, out_codec);

    // Valid audio codec should still be allowed with video mime type.
    EXPECT_TRUE(ParseAudioCodecString("video/webm", "opus", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kOpus, out_codec);

    // Video codec is not valid for audio API.
    EXPECT_FALSE(ParseAudioCodecString("audio/webm", "vp09.00.10.08", &out_is_ambiguous, &out_codec));

    // Made up codec is also not valid.
    EXPECT_FALSE(ParseAudioCodecString("audio/webm", "bogus", &out_is_ambiguous, &out_codec));
}
...
TEST(MimeUtilTest, ParseAudioCodecString_NoMimeType)
{
    ...
    // Valid AAC string when proprietary codecs are supported.
    EXPECT_TRUE(ParseAudioCodecString("", "mp4a.40.2", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kAAC, out_codec);
    ...
    // TODO(chcunningha): This can probably be allowed. I think we treat all
    // MPEG4_AAC the same.
    EXPECT_TRUE(ParseAudioCodecString("", "mp4a.40", &out_is_ambiguous, &out_codec));
    if (kUsePropCodecs) {
        EXPECT_TRUE(out_is_ambiguous);
        EXPECT_EQ(AudioCodec::kAAC, out_codec);
    }
    ...
    // Video codec is not valid for audio API.
    EXPECT_FALSE(ParseAudioCodecString("", "vp09.00.10.08", &out_is_ambiguous, &out_codec));

    // Made up codec is also not valid.
    EXPECT_FALSE(ParseAudioCodecString("", "bogus", &out_is_ambiguous, &out_codec));
}
...
TEST(MimeUtilTest, ParseAudioCodecString_Mp3)
{
    ...
    EXPECT_TRUE(ParseAudioCodecString("audio/mpeg", "mp3", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kMP3, out_codec);

    EXPECT_TRUE(ParseAudioCodecString("audio/mpeg", "", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kMP3, out_codec);
    ...
    EXPECT_TRUE(ParseAudioCodecString("", "mp4a.69", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kMP3, out_codec);

    EXPECT_TRUE(ParseAudioCodecString("", "mp4a.6B", &out_is_ambiguous, &out_codec));
    EXPECT_FALSE(out_is_ambiguous);
    EXPECT_EQ(AudioCodec::kMP3, out_codec);
}
...
TEST(IsCodecSupportedOnAndroidTest, EncryptedCodecBehavior)
{
    ...
    RunCodecSupportTest(states_to_vary, test_states, [](const MimeUtil::PlatformInfo& info, MimeUtil::Codec codec) {
        const bool result = MimeUtil::IsCodecSupportedOnAndroid(codec, kTestMimeType, true, VIDEO_CODEC_PROFILE_UNKNOWN, info);
        switch (codec) {
...
TEST(IsCodecSupportedOnAndroidTest, ClearCodecBehavior)
{
    ...
    RunCodecSupportTest(states_to_vary, test_states, [](const MimeUtil::PlatformInfo& info, MimeUtil::Codec codec) {
        const bool result = MimeUtil::IsCodecSupportedOnAndroid(codec, kTestMimeType, false, VIDEO_CODEC_PROFILE_UNKNOWN, info);
        switch (codec) {
        // These codecs are never supported by the Android platform.
        case MimeUtil::INVALID_CODEC:
        case MimeUtil::MPEG_H_AUDIO:
...
TEST(IsCodecSupportedOnAndroidTest, OpusOggSupport)
{
    ...
    RunCodecSupportTest(states_to_vary, test_states, [](const MimeUtil::PlatformInfo& info, MimeUtil::Codec codec) {
        EXPECT_TRUE(MimeUtil::IsCodecSupportedOnAndroid(MimeUtil::OPUS, "audio/ogg", false, VIDEO_CODEC_PROFILE_UNKNOWN, info));
    });
}
...
TEST(IsCodecSupportedOnAndroidTest, HEVCSupport)
{
    ...
    EXPECT_FALSE(MimeUtil::IsCodecSupportedOnAndroid(MimeUtil::HEVC, kTestMimeType, false, VIDEO_CODEC_PROFILE_UNKNOWN, info));

    info.has_platform_hevc_decoder = true;
    EXPECT_TRUE(MimeUtil::IsCodecSupportedOnAndroid(MimeUtil::HEVC, kTestMimeType, false, VIDEO_CODEC_PROFILE_UNKNOWN, info));
}
#endif

TEST(IsCodecSupportedOnAndroidTest, AndroidHLSAAC)
{
    const std::string hls_mime_types[] = { "application/x-mpegurl", "application/vnd.apple.mpegurl", "audio/mpegurl", "audio/x-mpegurl" };

    const std::string mpeg2_aac_codec_strings[] = { "mp4a.66", "mp4a.67", "mp4a.68" };

    const std::string mpeg4_aac_codec_strings[] = { "mp4a.40.2", "mp4a.40.02", "mp4a.40.5", "mp4a.40.05", "mp4a.40.29" };
    ...
    for (const auto& hls_mime_type : hls_mime_types) {
        ...
        for (const auto& mpeg2_aac_string : mpeg2_aac_codec_strings) {
            EXPECT_FALSE(ParseAudioCodecString(hls_mime_type, mpeg2_aac_string, &out_is_ambiguous, &out_codec));
        }

        // MPEG4_AAC is supported with HLS whenever HLS is supported.
        for (const auto& mpeg4_aac_string : mpeg4_aac_codec_strings) {
            EXPECT_EQ(kHlsSupported, ParseAudioCodecString(hls_mime_type, mpeg4_aac_string, &out_is_ambiguous, &out_codec));
        }
    }
