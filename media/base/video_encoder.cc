...
uint32_t GetDefaultVideoEncodeBitrate(gfx::Size frame_size, uint32_t framerate)
{
...
int GetNumberOfThreadsForSoftwareEncoding(gfx::Size frame_size)
{
    ...
    desired_threads = std::min(desired_threads, base::SysInfo::NumberOfProcessors());
...
std::string VideoEncoder::Options::ToString()
{
    ...
    if (keyframe_interval) {
        keys.push_back(base::StringPrintf("keyframe_interval: %d", *keyframe_interval));
    }
    keys.push_back(base::StringPrintf("latency_mode: %s", latency_mode == LatencyMode::Quality ? "quality" : "realtime"));
    if (scalability_mode) {
        keys.push_back(base::StringPrintf("scalability_mode: %s", GetScalabilityModeName(*scalability_mode)));
    }
    if (content_hint) {
        keys.push_back(base::StringPrintf("content_hint: %s", GetContentHintName(*content_hint)));
    }
    if (subsampling) {
    ...
    keys.push_back(base::StringPrintf("produce_annexb: %s", avc.produce_annexb || hevc.produce_annexb ? "true" : "false"));
