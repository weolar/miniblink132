...
AudioLimiter::PendingOutput::PendingOutput(OutputChannels channels, OutputFilledCB filled_callback)
...
AudioLimiter::AudioLimiter(int sample_rate, int channels)
    : channels_(channels)
    , attack_frames_(AudioTimestampHelper::TimeToFrames(kAttackTime, sample_rate))
    , release_frames_(AudioTimestampHelper::TimeToFrames(kReleaseTime, sample_rate))
    ,
    // Avoid dividing by 0, if `sample_rate` is 0, and CHECK below instead.
    attack_constant_(attack_frames_ ? std::pow(0.1, 1.0 / attack_frames_) : 0.0)
    , release_constant_(release_frames_ ? std::pow(0.1, 1.0 / release_frames_) : 0.0)
...
void AudioLimiter::LimitPeaks(const AudioBus& input, const OutputChannels& output, OutputFilledCB on_output_filled_cb)
{
    LimitPeaksPartial(input, input.frames(), output, std::move(on_output_filled_cb));
}

void AudioLimiter::LimitPeaksPartial(const AudioBus& input, int num_frames, const OutputChannels& output, OutputFilledCB on_output_filled_cb)
{
    CHECK(!was_flushed_);
...
void AudioLimiter::FeedInput(const AudioBus& input, int num_frames)
{
    ...
    input.ToInterleaved<Float32SampleTypeTraitsNoClip>(num_frames, interleaved_input.data());
    ...
    delayed_interleaved_input_.reserve(delayed_interleaved_input_.size() + interleaved_input.size());

    base::ranges::copy(interleaved_input, std::back_inserter(delayed_interleaved_input_));
    ...
    CHECK_LE(delayed_interleaved_input_.size(), static_cast<size_t>(channels_ * attack_frames_));
}
...
void AudioLimiter::UpdateGain(float current_maximum)
{
    ...
    if (target_gain_ < smoothed_gain_) {
        // Attack portion.
        smoothed_gain_ = attack_constant_ * (smoothed_gain_ - target_gain_) + target_gain_;

        // Don't overshoot gain reduction.
        smoothed_gain_ = std::max(smoothed_gain_, gain);
    } else {
        // Release portion.
        smoothed_gain_ = release_constant_ * (smoothed_gain_ - target_gain_) + target_gain_;
    }
...
void AudioLimiter::WriteLimitedFrameToOutput()
{
    ...
    CHECK_GT(delayed_interleaved_input_.size(), static_cast<size_t>(channels_ * attack_frames_));
    ...
    if (smoothed_gain_ < 1.0) {
        ...
        for (int ch = 0; ch < channels_; ++ch) {
            ...
            const float adjusted_sample = static_cast<float>(static_cast<double>(delayed_interleaved_input_.front()) * smoothed_gain_);
        ...
        for (int ch = 0; ch < channels_; ++ch) {
            ...
            dest.copy_from(base::byte_span_from_ref(delayed_interleaved_input_.front()));
