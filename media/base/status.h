...
namespace mojo {
template <typename T, typename U> struct StructTraits;
} // namespace mojo
...
template <typename T> struct SecondArgType { };

template <typename R, typename A1, typename A2> struct SecondArgType<R(A1, A2)> {
    using Type = A2;
};
...
struct MEDIA_EXPORT StatusData {
    ...
    StatusData(StatusGroupType group, StatusCodeType code, std::string message, UKMPackedType root_cause);
    ~StatusData();
...
#define NAME_DETECTOR(detector_name, field)                                                                                                                    \
    template <typename T> struct detector_name {                                                                                                               \
        template <typename, typename> struct field##is_enum {                                                                                                  \
            constexpr static bool value = false;                                                                                                               \
        };                                                                                                                                                     \
        template <typename V> struct field##is_enum<V, decltype(V::field)> {                                                                                   \
            constexpr static bool value = true;                                                                                                                \
        };                                                                                                                                                     \
        template <typename, typename> struct field##_is_member {                                                                                               \
            constexpr static bool value = false;                                                                                                               \
        };                                                                                                                                                     \
        template <typename V> struct field##_is_member<V, std::enable_if_t<std::is_pointer_v<decltype(&V::field)>, V>> {                                       \
            constexpr static bool value = true;                                                                                                                \
        };                                                                                                                                                     \
...
template <typename T> struct StatusTraitsHelper {
    ...
    static constexpr std::string GetMessage(std::string_view message, T::Codes code)
    {
        if (!message.empty()) {
...
MEDIA_EXPORT std::ostream& operator<<(std::ostream& stream, const OkStatusImplicitConstructionHelper&);
...
template <typename T> class MEDIA_EXPORT TypedStatus {
    static_assert(std::is_enum<typename T::Codes>::value, "TypedStatus Traits::Codes must be an enum type.");
    static_assert(std::is_same<decltype(T::Group), StatusGroupType()>::value, "TypedStatus Traits::Group() must return StatusGroupType.");
    ...
    static_assert(verify_default_okayness(), "If kOk is defined, then either no default, or default==kOk");
...
public:
    ...
    TypedStatus(const TypedStatus<T>& copy)
    {
        *this = copy;
    }
    ...
    TypedStatus(Codes code, const base::Location& location = base::Location::Current())
        : TypedStatus(code, "", location)
    {
    }

    TypedStatus(std::tuple<Codes, std::string_view> pack, const base::Location& location = base::Location::Current())
    ...
    template <typename _T = Traits, typename = std::enable_if<std::is_pointer_v<decltype(&_T::OnCreateFrom)>>>
    TypedStatus(
        Codes code, const typename internal::SecondArgType<decltype(_T::OnCreateFrom)>::Type& data, const base::Location& location = base::Location::Current())
        : TypedStatus(code, "", location)
    {
    ...
    template <typename _T = Traits, typename = std::enable_if<std::is_pointer_v<decltype(&_T::OnCreateFrom)>>>
    TypedStatus(Codes code, std::string_view message, const typename internal::SecondArgType<decltype(_T::OnCreateFrom)>::Type& data,
    ...
    TypedStatus(Codes code, TypedStatus<CausalStatusType>&& cause, const base::Location& location = base::Location::Current())
        : TypedStatus(code, "", location)
    {
    ...
    template <typename CausalStatusType>
    TypedStatus(Codes code, std::string_view message, TypedStatus<CausalStatusType>&& cause, const base::Location& location = base::Location::Current())
        : TypedStatus(code, message, location)
    {
        DCHECK(data_);
        AddCause(std::move(cause));
    ...
    TypedStatus(Codes code, std::string_view message, const base::Location& location = base::Location::Current())
    {
        ...
        data_ = std::make_unique<internal::StatusData>(
            Traits::Group(), static_cast<StatusCodeType>(code), internal::StatusTraitsHelper<Traits>::GetMessage(message, code), 0);
    ...
    TypedStatus<T>&& AddHere(const base::Location& location = base::Location::Current()) &&
    {
        DCHECK(data_);
    ...
    template <typename D> TypedStatus<T>&& WithData(const char* key, const D& value) &&
    {
        DCHECK(data_);
    ...
    template <typename D> void WithData(const char* key, const D& value) &
    {
        DCHECK(data_);
        data_->data.GetDict().Set(key, MediaSerialize(value));
    }
    ...
    template <typename AnyTraitsType> TypedStatus<T>&& AddCause(TypedStatus<AnyTraitsType>&& cause) &&
    {
        AddCause(std::move(cause));
        return std::move(*this);
    }
    ...
    // Add |cause| as the error that triggered this one.
    template <typename AnyTraitsType> void AddCause(TypedStatus<AnyTraitsType>&& cause) &
    {
    ...
    template <typename UKMBuilder> void ToUKM(UKMBuilder& builder) const
    {
    ...
    inline bool operator==(Codes code) const
    {
    ...
    template <typename OtherType> class Or {
    private:
        template <typename X> struct OrTypeUnwrapper {
            using type = Or<X>;
        };
        template <typename X> struct OrTypeUnwrapper<Or<X>> {
            using type = Or<X>;
        };
    ...
    public:
        ...
        Or(const OtherType& value)
            : value_(value)
        {
        }

        // Create an Or type explicitly from a code
        Or(typename T::Codes code, const base::Location& location = base::Location::Current())
        ...
        Or(typename T::Codes code, const First& first, const Rest&... rest, const base::Location& location = base::Location::Current())
        ...
        typename T::Codes code() const
        {
            ...
            static_assert(helper::OkEnumValue().has_value(), "Cannot call Or::code() without OkEnumValue or kOk defined");
            return error_ ? error_->code() : *helper::OkEnumValue();
        }

        template <typename FnType, typename ReturnType = decltype(std::declval<FnType>()(std::declval<OtherType>())),
        ...
        ReturnType MapValue(
            FnType&& lambda, typename ConvertTo::Codes on_error, std::string_view message = "", base::Location location = base::Location::Current()) &&
        {
            CHECK(error_ || value_);
            if (!has_value()) {
                auto error = std::move(*error_);
                error_.reset();
                return ConvertTo(on_error, message, location).AddCause(std::move(error));
            }
            CHECK(value_);
            auto value = std::move(std::get<0>(*value_));
            value_.reset();
    ...
    static Callback BindOkContinuation(Callback err, base::OnceCallback<void(Callback)> ok)
    {
        return base::BindOnce(
            [](Callback err, base::OnceCallback<void(Callback)> ok, TypedStatus<T> status) {
                if (status.is_ok()) {
...
private:
    ...
    template <typename StatusEnum, typename DataView> friend struct mojo::StructTraits;
    ...
    // Allow AddCause.
    template <typename StatusEnum> friend class TypedStatus;
...
template <typename T> inline bool operator==(typename T::Codes code, const TypedStatus<T>& status)
{
    return status == code;
}
