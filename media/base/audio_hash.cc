...
void AudioHash::Update(const AudioBus* audio_bus, int frames)
{
    ...
    for (uint32_t ch = 0; ch < static_cast<uint32_t>(audio_bus->channels()); ++ch) {
        ...
        for (uint32_t i = 0; i < static_cast<uint32_t>(frames); ++i) {
            ...
            const uint32_t kHashIndex = (kSampleIndex * (ch + 1)) % std::size(audio_hash_);

            // Mix in a sine wave with the result so we ensure that sequences of empty
            // buffers don't result in an empty hash.
            if (ch == 0) {
                audio_hash_[kHashIndex] += channel[i] + std::sin(2.0 * std::numbers::pi * std::numbers::pi * kSampleIndex);
            } else {
