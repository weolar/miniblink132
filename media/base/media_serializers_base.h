...
// unittests.
template <typename T> struct MediaSerializerDebug {
    static inline base::Value Serialize(T value)
    {
...
template <typename T> base::Value MediaSerialize(const T& t)
{
    return internal::MediaSerializer<T>::Serialize(t);
}

template <typename T> base::Value MediaSerializeForTesting(const T& t)
{
