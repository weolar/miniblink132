...
TEST(DecoderBufferTest, FromPlatformSharedMemoryRegion)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(region), 0, kDataSize));
    ASSERT_TRUE(buffer.get());
    EXPECT_EQ(buffer->size(), kDataSize);
    EXPECT_EQ(base::span(*buffer), base::span(kData));
    EXPECT_FALSE(buffer->end_of_stream());
...
TEST(DecoderBufferTest, FromPlatformSharedMemoryRegion_Unaligned)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(region), kDataOffset, kDataSize - kDataOffset));
    ASSERT_TRUE(buffer.get());
...
TEST(DecoderBufferTest, FromPlatformSharedMemoryRegion_ZeroSize)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(region), 0, 0));
...
TEST(DecoderBufferTest, FromSharedMemoryRegion)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(mapping_region.region), 0, kDataSize));
    ASSERT_TRUE(buffer.get());
    EXPECT_EQ(buffer->size(), kDataSize);
    EXPECT_EQ(base::span(*buffer), base::span(kData));
    EXPECT_FALSE(buffer->end_of_stream());
...
TEST(DecoderBufferTest, FromSharedMemoryRegion_Unaligned)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(mapping_region.region), kDataOffset, kDataSize - kDataOffset));
...
TEST(DecoderBufferTest, FromSharedMemoryRegion_ZeroSize)
{
    ...
    scoped_refptr<DecoderBuffer> buffer(DecoderBuffer::FromSharedMemoryRegion(std::move(mapping_region.region), 0, 0));
...
TEST(DecoderBufferTest, FromExternalMemory)
{
    ...
    auto external_memory = std::make_unique<ExternalMemoryAdapterForTesting>(base::make_span(kData, kDataSize));
...
TEST(DecoderBufferTest, DecryptConfig)
{
    ...
    std::unique_ptr<DecryptConfig> decrypt_config = DecryptConfig::CreateCencConfig(kKeyId, kIv, subsamples);
...
TEST(DecoderBufferTest, SideData)
{
    ...
    buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(kAlphaData);
    ...
    EXPECT_EQ(buffer->side_data()->secure_handle, cloned_side_data->secure_handle);
    EXPECT_EQ(buffer->side_data()->spatial_layers, cloned_side_data->spatial_layers);
    EXPECT_EQ(buffer->side_data()->alpha_data.as_span(), cloned_side_data->alpha_data.as_span());
