...
static void RunConvolveBenchmark(float (*convolve_fn)(const int, const float*, const float*, const float*, double), bool aligned, const std::string& trace_name)
{
    SincResampler resampler(kSampleRateRatio, SincResampler::kDefaultRequestSize, base::DoNothing());

    base::TimeTicks start = base::TimeTicks::Now();
    for (int i = 0; i < kBenchmarkIterations; ++i) {
        convolve_fn(resampler.KernelSize(), resampler.get_kernel_for_testing() + (aligned ? 0 : 1), resampler.get_kernel_for_testing(),
...
TEST(SincResamplerPerfTest, Convolve_optimized_aligned)
{
    ...
    base::CPU cpu;
    if (cpu.has_avx2()) {
        RunConvolveBenchmark(SincResampler::Convolve_AVX2, true, "optimized_aligned");
    } else if (cpu.has_sse2()) {
        RunConvolveBenchmark(SincResampler::Convolve_SSE, true, "optimized_aligned");
    }
...
TEST(SincResamplerPerfTest, Convolve_optimized_unaligned)
{
    ...
    base::CPU cpu;
    if (cpu.has_avx2()) {
        RunConvolveBenchmark(SincResampler::Convolve_AVX2, false, "optimized_unaligned");
    } else if (cpu.has_sse2()) {
        RunConvolveBenchmark(SincResampler::Convolve_SSE, false, "optimized_unaligned");
    }
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(USE_NEON)
    RunConvolveBenchmark(SincResampler::Convolve_NEON, false, "optimized_unaligned");
#endif
}
