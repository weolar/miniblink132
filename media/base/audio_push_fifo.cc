...
void AudioPushFifo::Push(const AudioBus& input_bus)
{
    ...
    do {
        ...
        const int frames_to_enqueue = std::min(static_cast<int>(input_bus.frames() - input_offset), frames_per_buffer_ - queued_frames_);
        if (frames_to_enqueue > 0) {
            DVLOG(2) << "Enqueuing " << frames_to_enqueue << " frames.";
            input_bus.CopyPartialFramesTo(input_offset, frames_to_enqueue, queued_frames_, audio_queue_.get());
...
void AudioPushFifo::Flush()
{
    ...
    audio_queue_->ZeroFramesPartial(queued_frames_, audio_queue_->frames() - queued_frames_);
