...
void FakeTextTrackStream::Read(ReadCB read_cb)
{
    ...
    if (stopping_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&FakeTextTrackStream::AbortPendingRead, base::Unretained(this)));
    }
}
...
bool FakeTextTrackStream::SupportsConfigChanges()
{
    return false;
}

void FakeTextTrackStream::SatisfyPendingRead(
    const base::TimeDelta& start, const base::TimeDelta& duration, const std::string& id, const std::string& content, const std::string& settings)
{
    DCHECK(read_cb_);
    ...
    scoped_refptr<DecoderBuffer> buffer = DecoderBuffer::CopyFrom(data_buf, data_len, sd_buf, sd_len);
