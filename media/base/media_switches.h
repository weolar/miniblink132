...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FREEBSD) || BUILDFLAG(IS_SOLARIS)
...
MEDIA_EXPORT extern const char kUnsafelyAllowProtectedMediaIdentifierForDomain[];
...
MEDIA_EXPORT BASE_DECLARE_FEATURE(kCastStreamingExponentialVideoBitrateAlgorithm);
...
MEDIA_EXPORT extern const base::FeatureParam<bool> kHardwareSecureDecryptionForceSupportClearLead;
...
MEDIA_EXPORT extern const base::FeatureParam<bool> kHardwareSecureDecryptionFallbackPerSite;
MEDIA_EXPORT extern const base::FeatureParam<int> kHardwareSecureDecryptionFallbackMinDisablingDays;
MEDIA_EXPORT extern const base::FeatureParam<int> kHardwareSecureDecryptionFallbackMaxDisablingDays;
MEDIA_EXPORT extern const base::FeatureParam<bool> kHardwareSecureDecryptionFallbackOnHardwareContextReset;
...
#if BUILDFLAG(IS_WIN)
MEDIA_EXPORT extern const base::FeatureParam<std::string> kMediaFoundationClearKeyCdmPathForTesting;
...
MEDIA_EXPORT BASE_DECLARE_FEATURE(kShareThisTabInsteadButtonGetDisplayMediaAudio);
...
MEDIA_EXPORT BASE_DECLARE_FEATURE(kAllowClearDolbyVisionInMseWhenPlatformEncryptedDvEnabled);
#endif
...
MEDIA_EXPORT std::string GetEffectiveAutoplayPolicy(const base::CommandLine& command_line);

MEDIA_EXPORT bool IsChromeWideEchoCancellationEnabled();
MEDIA_EXPORT bool IsSystemEchoCancellationEnforced();
MEDIA_EXPORT bool IsDedicatedMediaServiceThreadEnabled(gl::ANGLEImplementation impl);
