...
FakeSingleThreadTaskRunner::FakeSingleThreadTaskRunner(base::SimpleTestTickClock* clock)
    : clock_(clock)
...
bool FakeSingleThreadTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    if (fail_on_next_task_) {
        LOG(FATAL) << "Infinite task posting loop detected.  Possibly caused by " << from_here.ToString() << " posting a task with delay "
    ...
    if (!tasks_.empty()) {
        const auto after_it = tasks_.lower_bound(TaskKey(run_time + base::Microseconds(1), 0));
        if (after_it != tasks_.begin()) {
            auto it = after_it;
            --it;
            if (it->first.first == run_time) {
                tasks_.insert(after_it /* hint */, std::make_pair(TaskKey(run_time, it->first.second + 1), std::move(task)));
                return true;
            }
        }
    }
...
bool FakeSingleThreadTaskRunner::PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    NOTIMPLEMENTED();
    return false;
}
