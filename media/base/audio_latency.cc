...
bool AudioLatency::IsResamplingPassthroughSupported(Type type)
{
    ...
    return type == Type::kPlayback && base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_NOUGAT_MR1;
#else
    return false;
#endif
}
...
// static
int AudioLatency::GetHighLatencyBufferSize(int sample_rate, int preferred_buffer_size)
{
    ...
    static_assert(static_cast<int>(period / base::kAudioSchedulingPeriod) == period / base::kAudioSchedulingPeriod);
    const int high_latency_buffer_size = period.InMilliseconds() * sample_rate / base::Time::kMillisecondsPerSecond;
...
int AudioLatency::GetInteractiveBufferSize(int hardware_buffer_size)
{
    ...
    int sensible_buffer_size = std::min(LCM(hardware_buffer_size, kWebAudioRenderQuantumSize), kWebAudioRenderQuantumSize * 4);
...
int AudioLatency::GetExactBufferSize(base::TimeDelta duration, int sample_rate, int hardware_buffer_size, int min_hardware_buffer_size,
    int max_hardware_buffer_size, int max_allowed_buffer_size)
{
    ...
    DCHECK(max_hardware_buffer_size == 0 || hardware_buffer_size <= max_hardware_buffer_size);
    ...
    if (min_hardware_buffer_size && requested_buffer_size <= min_hardware_buffer_size) {
...
#else
    const int multiplier = min_hardware_buffer_size > 0 ? min_hardware_buffer_size : hardware_buffer_size;
#endif

    int buffer_size = std::ceil(requested_buffer_size / static_cast<double>(multiplier)) * multiplier;
    ...
    if (max_hardware_buffer_size && buffer_size >= max_hardware_buffer_size) {
        buffer_size = std::ceil(requested_buffer_size / static_cast<double>(max_hardware_buffer_size)) * max_hardware_buffer_size;
    }

    const int platform_max_buffer_size = (max_hardware_buffer_size && max_hardware_buffer_size <= max_allowed_buffer_size)
        ? (max_allowed_buffer_size / max_hardware_buffer_size) * max_hardware_buffer_size
