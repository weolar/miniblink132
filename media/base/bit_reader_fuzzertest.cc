...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    base::span<const uint8_t> remaining = iterator.Span<uint8_t>(iterator.total_size() - iterator.position());

    media::BitReader reader(remaining.data(), base::checked_cast<int>(remaining.size()));
