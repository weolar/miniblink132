...
void MockDemuxerStream::set_audio_decoder_config(const AudioDecoderConfig& config)
{
    DCHECK_EQ(type_, DemuxerStream::AUDIO);
    audio_decoder_config_ = config;
}

void MockDemuxerStream::set_video_decoder_config(const VideoDecoderConfig& config)
{
    DCHECK_EQ(type_, DemuxerStream::VIDEO);
...
MockVideoDecoder::MockVideoDecoder(bool is_platform_decoder, bool supports_decryption, int decoder_id)
...
MockAudioDecoder::MockAudioDecoder(bool is_platform_decoder, bool supports_decryption, int decoder_id)
    : is_platform_decoder_(is_platform_decoder)
    , supports_decryption_(supports_decryption)
    , decoder_id_(decoder_id)
{
...
MockCdmSessionPromise::MockCdmSessionPromise(bool expect_success, std::string* new_session_id)
{
    if (expect_success) {
...
MockCdmKeyStatusPromise::MockCdmKeyStatusPromise(bool expect_success, CdmKeyInformation::KeyStatus* key_status, CdmPromise::Exception* exception)
{
    if (expect_success) {
        ...
        EXPECT_CALL(*this, resolve(_)).Times(0);
        if (exception) {
            EXPECT_CALL(*this, reject(_, _, NotEmpty())).WillOnce(SaveArg<0>(exception));
        } else {
            EXPECT_CALL(*this, reject(_, _, NotEmpty()));
        }
    }
...
MockCdm::MockCdm(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
    const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb)
{
    Initialize(cdm_config, session_message_cb, session_closed_cb, session_keys_change_cb, session_expiration_update_cb);
}

MockCdm::~MockCdm() = default;

void MockCdm::Initialize(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
    const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb)
{
...
void MockCdm::CallSessionMessageCB(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message)
{
    session_message_cb_.Run(session_id, message_type, message);
}

void MockCdm::CallSessionClosedCB(const std::string& session_id, CdmSessionClosedReason reason)
{
    session_closed_cb_.Run(session_id, reason);
}

void MockCdm::CallSessionKeysChangeCB(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info)
{
    session_keys_change_cb_.Run(session_id, has_additional_usable_key, std::move(keys_info));
}

void MockCdm::CallSessionExpirationUpdateCB(const std::string& session_id, base::Time new_expiry_time)
{
...
void MockCdmFactory::Create(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
    const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb, CdmCreatedCB cdm_created_cb)
{
    ...
    if (cdm_config.key_system.empty()) {
        std::move(cdm_created_cb).Run(nullptr, CreateCdmStatus::kUnsupportedKeySystem);
        return;
    }
    ...
    mock_cdm_->Initialize(cdm_config, session_message_cb, session_closed_cb, session_keys_change_cb, session_expiration_update_cb);
    std::move(cdm_created_cb).Run(mock_cdm_, CreateCdmStatus::kSuccess);
}

void MockCdmFactory::SetBeforeCreationCB(base::RepeatingClosure before_creation_cb)
{
