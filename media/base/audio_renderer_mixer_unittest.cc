...
public:
        ...
        , half_fill_(false)
    {
        ...
        for (size_t i = 0; i < sample_rates_count; ++i)
            input_parameters_.emplace_back(
                AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::FromLayout<kChannelLayout>(), sample_rates[i], kHighLatencyBufferSize);

        // Create output parameters based on test parameters.
        output_parameters_ = AudioParameters(
            AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kChannelLayout>(), std::get<2>(GetParam()), kLowLatencyBufferSize);
        ...
        expected_callback_ = std::make_unique<FakeAudioRenderCallback>(step, output_parameters_.sample_rate());
    }
    ...
    AudioRendererMixer* GetMixer(const base::UnguessableToken& owner_token, const AudioParameters& params, AudioLatency::LatencyType latency,
        const OutputDeviceInfo& sink_info, scoped_refptr<AudioRendererSink> sink) final
    {
        return mixer_.get();
    }
    ...
    void InitializeInputs(int inputs_per_sample_rate)
    {
        ...
        for (size_t i = 0, input = 0; i < input_parameters_.size(); ++i) {
            ...
            for (int j = 0; j < inputs_per_sample_rate; ++j, ++input) {
                fake_callbacks_.push_back(std::make_unique<FakeAudioRenderCallback>(step, output_parameters_.sample_rate()));
    ...
    bool RenderAndValidateAudioData(float scale)
    {
        ...
        int frames = mixer_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus_.get());
        if (frames != audio_bus_->frames())
            return false;

        // Render expected audio data (without scaling).
        expected_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, expected_audio_bus_.get());
    ...
    {
        auto input = base::MakeRefCounted<AudioRendererMixerInput>(this, base::UnguessableToken::Create(),
            // default device ID.
...
TEST_P(AudioRendererMixerBehavioralTest, MixerPausesStream)
{
    ...
    while (!pause_event.IsSignaled()) {
        mixer_callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus_.get());
