...
TextRenderer::TextRenderer(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const AddTextTrackCB& add_text_track_cb)
...
void TextRenderer::StartPlaying()
{
    ...
    for (auto itr = text_track_state_map_.begin(); itr != text_track_state_map_.end(); ++itr) {
...
void TextRenderer::AddTextStream(DemuxerStream* text_stream, const TextTrackConfig& config)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void TextRenderer::BufferReady(DemuxerStream* stream, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> input)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    scoped_refptr<TextCue> text_cue(new TextCue(input->timestamp(), input->duration(), id, settings, text));

    CueReady(stream, text_cue);
}

void TextRenderer::CueReady(DemuxerStream* text_stream, const scoped_refptr<TextCue>& text_cue)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (state->text_ranges_.AddCue(start)) {
        ...
        state->text_track->addWebVTTCue(start, end, text_cue->id(), text_cue->text(), text_cue->settings());
    }
...
void TextRenderer::OnAddTextTrackDone(DemuxerStream* text_stream, std::unique_ptr<TextTrack> text_track)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    DCHECK_NE(state_, kUninitialized);
    DCHECK(text_stream);
    ...
    std::unique_ptr<TextTrackState> state(new TextTrackState(std::move(text_track)));
