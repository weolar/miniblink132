...
TEST(ParseVP9CodecId, NewStyleVP9CodecIDs)
{
    ...
    const base::flat_set<uint32_t> kValidVp9Levels = { 10, 11, 20, 21, 30, 31, 40, 41, 50, 51, 52, 60, 61, 62 };
    ...
    for (int eotf : { 1, 4, 6, 14, 15, 13, 16 }) {
        ...
        EXPECT_EQ(static_cast<VideoColorSpace::TransferID>(eotf), result->color_space.transfer);
    }
...
TEST(ParseAv1CodecId, VerifyOptionalMonochrome)
{
    ...
    for (int i = 0; i <= 9; ++i) {
        ...
        if (i < 2) {
            auto result = ParseAv1CodecId(codec_string);
            ASSERT_TRUE(result);
            EXPECT_EQ(result->subsampling, i == 0 ? VideoChromaSampling::k420 : VideoChromaSampling::k400);
        } else {
            EXPECT_FALSE(ParseAv1CodecId(codec_string));
        }
    }
...
TEST(ParseAv1CodecId, VerifyOptionalColorProperties)
{
    ...
    // BT2020
    {
        ...
        ASSERT_TRUE(result);
        EXPECT_EQ(VideoColorSpace::PrimaryID::BT2020, result->color_space.primaries);
    }
    // 0 is invalid.
...
TEST(ParseHEVCCodecIdTest, InvalidHEVCCodecIds)
{
    ...
    for (const auto* codec_id : { "hvc1.1.6.L93.B0", "hvc1.1.0.L93.B0", "hvc1.0.6.L93.B0" }) {
    ...
    // Spec A.3.3
    for (const auto* codec_id : { "hvc1.2.4.L93.B0", "hvc1.2.0.L93.B0", "hvc1.0.4.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // general_profile_compatibility_flag[2] should be equal to 1 as well.
    for (const auto* codec_id : { "hvc1.3.E.L93.B0", "hvc1.0.E.L93.B0", "hvc1.3.0.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec A.3.5
    for (const auto* codec_id : { "hvc1.4.10.L93.B0", "hvc1.4.0.L93.B0", "hvc1.0.10.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec A.3.6
    for (const auto* codec_id : { "hvc1.5.20.L93.B0", "hvc1.5.0.L93.B0", "hvc1.0.20.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec G.11.1.1
    for (const auto* codec_id : { "hvc1.6.40.L93.B0", "hvc1.6.0.L93.B0", "hvc1.0.40.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec H.11.1.1
    for (const auto* codec_id : { "hvc1.7.80.L93.B0", "hvc1.7.0.L93.B0", "hvc1.0.80.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec I.11.1.1
    for (const auto* codec_id : { "hvc1.8.100.L93.B0", "hvc1.8.0.L93.B0", "hvc1.0.100.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec A.3.7
    for (const auto* codec_id : { "hvc1.9.200.L93.B0", "hvc1.9.0.L93.B0", "hvc1.0.200.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec H.11.1.2
    for (const auto* codec_id : { "hvc1.10.400.L93.B0", "hvc1.10.0.L93.B0", "hvc1.0.400.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
    ...
    // Spec A.3.8
    for (const auto* codec_id : { "hvc1.11.800.L93.B0", "hvc1.11.0.L93.B0", "hvc1.0.800.L93.B0" }) {
        auto result = ParseHEVCCodecId(codec_id);
        ASSERT_TRUE(result);
