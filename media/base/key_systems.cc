...
std::string GetKeySystemNameForUMA(const std::string& key_system, std::optional<bool> use_hw_secure_codecs)
{
    ...
    if (key_system == kWidevineKeySystem) {
        ...
        if (use_hw_secure_codecs.has_value()) {
            key_system_name += ".";
            key_system_name += (use_hw_secure_codecs.value() ? kHardwareSecureForUMA : kSoftwareSecureForUMA);
        }
