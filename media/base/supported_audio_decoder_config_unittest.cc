...
{
    SupportedAudioDecoderConfig config2(supported_config_.codec, supported_config_.profile);
    EXPECT_EQ(supported_config_, config2);
}

TEST_F(SupportedAudioDecoderConfigTest, CodecMismatches)
{
    SupportedAudioDecoderConfig config2(AudioCodec::kUnknown, supported_config_.profile);
    EXPECT_NE(supported_config_, config2);
}

TEST_F(SupportedAudioDecoderConfigTest, ProfileMismatches)
{
    SupportedAudioDecoderConfig config2(supported_config_.codec, AudioCodecProfile::kUnknown);
    EXPECT_NE(supported_config_, config2);
}
