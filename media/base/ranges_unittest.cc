...
template <class T> std::ostream& operator<<(std::ostream& os, const Ranges<T>& r)
{
    os << "{ ";
...
template <class T> static void ExpectRanges(const Ranges<T>& r, std::string_view expected_string)
{
    std::stringstream ss;
    ss << r;
...
#define ASSERT_RANGES(ranges, expectation) ASSERT_NO_FATAL_FAILURE(ExpectRanges(ranges, expectation));
