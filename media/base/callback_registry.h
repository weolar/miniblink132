...
template <typename... Args> class CallbackRegistry<void(Args...)> {
public:
    ...
    [[nodiscard]] std::unique_ptr<CallbackRegistration> Register(CallbackType cb)
    {
        ...
        // callbacks.
        callbacks_[registration_id] = base::BindPostTaskToCurrentDefault(std::move(cb));
...
private:
    ...
    public:
        RegistrationImpl(CallbackRegistry<void(Args...)>* registry, uint32_t registration_id)
            : registry_(registry)
        ...
        {
            registry_->Unregister(registration_id_);
        }
