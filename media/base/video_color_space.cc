...
VideoColorSpace::VideoColorSpace(PrimaryID primaries, TransferID transfer, MatrixID matrix, gfx::ColorSpace::RangeID range)
    : primaries(primaries)
    , transfer(transfer)
    , matrix(matrix)
    , range(range)
...
VideoColorSpace::VideoColorSpace(int primaries, int transfer, int matrix, gfx::ColorSpace::RangeID range)
...
{
    return primaries == other.primaries && transfer == other.transfer && matrix == other.matrix && range == other.range;
}
...
{
    return primaries != PrimaryID::INVALID && primaries != PrimaryID::UNSPECIFIED && transfer != TransferID::INVALID && transfer != TransferID::UNSPECIFIED
        && matrix != MatrixID::INVALID && matrix != MatrixID::UNSPECIFIED && range != gfx::ColorSpace::RangeID::INVALID;
}
...
{
    return base::StringPrintf("{primary=%d, transfer=%d, matrix=%d, range=%d}", static_cast<int>(primaries), static_cast<int>(transfer),
        static_cast<int>(matrix), static_cast<int>(range));
}

VideoColorSpace VideoColorSpace::REC709()
{
    return VideoColorSpace(PrimaryID::BT709, TransferID::BT709, MatrixID::BT709, gfx::ColorSpace::RangeID::LIMITED);
}
VideoColorSpace VideoColorSpace::REC601()
{
    return VideoColorSpace(PrimaryID::SMPTE170M, TransferID::SMPTE170M, MatrixID::SMPTE170M, gfx::ColorSpace::RangeID::LIMITED);
}
VideoColorSpace VideoColorSpace::JPEG()
{
    // TODO(ccameron): Determine which primaries and transfer function were
    // intended here.
    return VideoColorSpace(PrimaryID::BT709, TransferID::IEC61966_2_1, MatrixID::SMPTE170M, gfx::ColorSpace::RangeID::FULL);
}

// static
VideoColorSpace VideoColorSpace::FromGfxColorSpace(const gfx::ColorSpace& color_space)
{
...
gfx::ColorSpace VideoColorSpace::ToGfxColorSpaceInternal(bool allow_guessing) const
{
    gfx::ColorSpace::PrimaryID primary_id = gfx::ColorSpace::PrimaryID::INVALID;
    gfx::ColorSpace::TransferID transfer_id = gfx::ColorSpace::TransferID::INVALID;
    ...
    if (!allow_guessing) {
        ...
        if (primary_id == gfx::ColorSpace::PrimaryID::INVALID || transfer_id == gfx::ColorSpace::TransferID::INVALID
            || matrix_id == gfx::ColorSpace::MatrixID::INVALID || range_id == gfx::ColorSpace::RangeID::INVALID) {
