...
int VideoDecoder::GetRecommendedThreadCount(int desired_threads)
{
    ...
    const auto threads = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kVideoThreads);
    int decode_threads;
    ...
    // each Decode() call.
    return std::clamp(desired_threads, static_cast<int>(limits::kMinVideoDecodeThreads), static_cast<int>(limits::kMaxVideoDecodeThreads));
}
