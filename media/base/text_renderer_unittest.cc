...
public:
    ...
    ~FakeTextTrack() override
    {
        std::move(destroy_cb_).Run();
    }

    MOCK_METHOD5(
        addWebVTTCue, void(base::TimeDelta start, base::TimeDelta end, const std::string& id, const std::string& content, const std::string& settings));
...
public:
    ...
    void CreateTextRenderer()
    {
        ...
        text_renderer_ = std::make_unique<TextRenderer>(
            task_environment_.GetMainThreadTaskRunner(), base::BindRepeating(&TextRendererTest::OnAddTextTrack, base::Unretained(this)));
        text_renderer_->Initialize(base::BindRepeating(&TextRendererTest::OnEnd, base::Unretained(this)));
    }
    ...
    void AddTextTrack(TextKind kind, const std::string& name, const std::string& language, bool expect_read)
    {
    ...
    void OnAddTextTrack(const TextTrackConfig& config, AddTextTrackDoneCB done_cb)
    {
        base::OnceClosure destroy_cb = base::BindOnce(&TextRendererTest::OnDestroyTextTrack, base::Unretained(this), text_tracks_.size());
    ...
    void SatisfyPendingReads(base::TimeDelta start, base::TimeDelta duration, const std::string& id, const std::string& content, const std::string& settings)
    {
        for (auto itr = text_track_streams_.begin(); itr != text_track_streams_.end(); ++itr) {
    ...
    void Pause()
    {
        text_renderer_->Pause(base::BindOnce(&TextRendererTest::OnPause, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void Flush()
    {
        ...
        text_renderer_->Flush(base::BindOnce(&TextRendererTest::OnFlush, base::Unretained(this)));
    }
