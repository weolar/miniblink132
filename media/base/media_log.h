...
// of memory on linux.
    ...
    // is correct.
    template <MediaLogProperty P, typename T> void SetProperty(const T& value)
    {
    ...
    template <MediaLogEvent E, typename... T> void AddEvent(const T&... value)
    {
    ...
    template <typename T> void NotifyError(const TypedStatus<T>& status)
    {
        DCHECK(!status.is_ok());
        std::unique_ptr<MediaLogRecord> record = CreateRecord(MediaLogRecord::Type::kMediaStatus);
...
protected:
    ...
    template <MediaLogProperty P, typename T> std::unique_ptr<MediaLogRecord> CreatePropertyRecord(const T& value)
    {
    ...
    template <MediaLogEvent E, typename... Opt> std::unique_ptr<MediaLogRecord> CreateEventRecord()
    {
        std::unique_ptr<MediaLogRecord> record(CreateRecord(MediaLogRecord::Type::kMediaEventTriggered));
        record->params.Set(MediaLog::kEventKey, MediaLogEventTypeSupport<E, Opt...>::TypeName());
        return record;
    }
...
public:
    LogHelper(MediaLogMessageLevel level, MediaLog* media_log, const char* file, int line, std::optional<logging::SystemErrorCode> code = std::nullopt);
    LogHelper(MediaLogMessageLevel level, const std::unique_ptr<MediaLog>& media_log, const char* file, int line,
...
#if DCHECK_IS_ON()
#define MEDIA_PLOG(level, code, media_log) media::LogHelper((media::MediaLogMessageLevel::k##level), (media_log), __FILE__, __LINE__, code).stream()
#define MEDIA_LOG(level, media_log) media::LogHelper((media::MediaLogMessageLevel::k##level), (media_log), __FILE__, __LINE__).stream()
#else
#define MEDIA_LOG(level, media_log) media::LogHelper((media::MediaLogMessageLevel::k##level), (media_log), nullptr, 0).stream()
#define MEDIA_PLOG(level, code, media_log) media::LogHelper((media::MediaLogMessageLevel::k##level), (media_log), nullptr, 0, code).stream()
#endif

#if BUILDFLAG(IS_APPLE)
// Prepends a description of an OSStatus to the log entry produced with
// `MEDIA_LOG`.
#define OSSTATUS_MEDIA_LOG(level, status, media_log) MEDIA_LOG(level, media_log) << logging::DescriptionFromOSStatus(status) << " (" << (status) << "): "
#endif // BUILDFLAG(IS_APPLE)
...
#define LIMITED_MEDIA_LOG(level, media_log, count, max)                                                                                                        \
    LAZY_STREAM(MEDIA_LOG(level, media_log), (count) < (max) && ((count)++ || true)) << (((count) == (max)) ? "(Log limit reached. Further similar entries "   \
                                                                                                              "may be suppressed): "                           \
                                                                                                            : "")
