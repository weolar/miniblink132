...
public:
    ...
    void Start(StartType start_type, Demuxer* demuxer, Client* client, PipelineStatusCallback seek_cb) override
    {
        OnStart(start_type, demuxer, client, seek_cb);
    }
    MOCK_METHOD4(OnStart, void(StartType, Demuxer*, Client*, PipelineStatusCallback&));
    ...
    void Suspend(PipelineStatusCallback cb) override
    {
        OnSuspend(cb);
    }
    ...
    MOCK_METHOD2(OnEnabledAudioTracksChanged, void(const std::vector<MediaTrack::Id>&, base::OnceClosure));
    MOCK_METHOD2(OnSelectedVideoTrackChanged, void(std::optional<MediaTrack::Id>, base::OnceClosure));
    ...
    MOCK_METHOD2(OnSetCdm, void(CdmContext* cdm_context, CdmAttachedCB& cdm_attached_cb));
};
...
public:
    ...
    MOCK_METHOD(void, OnInitialize, (DemuxerHost * host, PipelineStatusCallback& cb), ());
    ...
    MOCK_METHOD(std::optional<container_names::MediaContainerName>, GetContainerForMetrics, (), (const, override));
    MOCK_METHOD(void, OnEnabledAudioTracksChanged, (const std::vector<MediaTrack::Id>&, base::TimeDelta, TrackChangeCB), (override));
    MOCK_METHOD(void, OnSelectedVideoTrackChanged, (const std::vector<MediaTrack::Id>&, base::TimeDelta, TrackChangeCB), (override));
...
public:
    ...
    MockVideoDecoder(bool is_platform_decoder, bool supports_decryption, int decoder_id);
    ...
    MOCK_METHOD6(Initialize_,
        void(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB& init_cb, const OutputCB& output_cb,
    ...
    void Reset(base::OnceClosure cb) override
    {
        Reset_(cb);
    }
...
public:
    ...
    MOCK_METHOD(void, Initialize, (const Options& options, OutputCB output_cb, EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, Encode, (std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, EncoderStatusCB done_cb), (override));
...
public:
    ...
    MOCK_METHOD(
        void, Initialize, (VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, Encode, (scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb), (override));

    MOCK_METHOD(void, ChangeOptions, (const Options& options, OutputCB output_cb, EncoderStatusCB done_cb), (override));
...
public:
    ...
    explicit MockAudioDecoder(bool is_platform_decoder, bool supports_decryption, int decoder_id);
    ...
    void Initialize(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb) override
    {
        Initialize_(config, cdm_context, init_cb, output_cb, waiting_cb);
    }
    MOCK_METHOD5(
        Initialize_, void(const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB& init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb));
...
public:
    ...
    void Initialize(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, const TimeSource::WallClockTimeCB& wall_clock_time_cb,
        PipelineStatusCallback init_cb) override
    {
        OnInitialize(stream, cdm_context, client, wall_clock_time_cb, init_cb);
    }
    MOCK_METHOD5(OnInitialize,
        void(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, const TimeSource::WallClockTimeCB& wall_clock_time_cb,
    ...
    MOCK_METHOD1(SetLatencyHint, void(std::optional<base::TimeDelta> latency_hint));
};
...
public:
    ...
    void Initialize(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, PipelineStatusCallback init_cb) override
    {
        OnInitialize(stream, cdm_context, client, init_cb);
    }
    MOCK_METHOD4(OnInitialize, void(DemuxerStream* stream, CdmContext* cdm_context, RendererClient* client, PipelineStatusCallback& init_cb));
...
public:
    ...
    void Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb) override
    {
        OnInitialize(media_resource, client, init_cb);
    }
    MOCK_METHOD3(OnInitialize, void(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback& init_cb));
    MOCK_METHOD1(SetLatencyHint, void(std::optional<base::TimeDelta>));
    MOCK_METHOD1(SetPreservesPitch, void(bool));
    MOCK_METHOD1(SetWasPlayedWithUserActivationAndHighMediaEngagement, void(bool));
    ...
    MOCK_METHOD2(OnSetCdm, void(CdmContext* cdm_context, CdmAttachedCB& cdm_attached_cb));
    MOCK_METHOD2(OnSelectedVideoTrackChanged, void(std::vector<DemuxerStream*>, base::OnceClosure));
    MOCK_METHOD2(OnSelectedAudioTracksChanged, void(std::vector<DemuxerStream*>, base::OnceClosure));
...
public:
    ...
    MOCK_METHOD6(CreateRenderer,
        std::unique_ptr<Renderer>(const scoped_refptr<base::SequencedTaskRunner>&, const scoped_refptr<base::TaskRunner>&, AudioRendererSink*,
            VideoRendererSink*, RequestOverlayInfoCB, const gfx::ColorSpace&));
};
...
public:
    ...
    MOCK_METHOD2(GetWallClockTimes, bool(const std::vector<base::TimeDelta>&, std::vector<base::TimeTicks>*));
};
...
public:
    ...
    MOCK_METHOD3(OnSessionMessage, void(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message));
    MOCK_METHOD2(OnSessionClosed, void(const std::string& session_id, CdmSessionClosedReason reason));

    // Add OnSessionKeysChangeCalled() function so we can store |keys_info|.
    MOCK_METHOD2(OnSessionKeysChangeCalled, void(const std::string& session_id, bool has_additional_usable_key));
    void OnSessionKeysChange(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info)
    {
        keys_info_.swap(keys_info);
        OnSessionKeysChangeCalled(session_id, has_additional_usable_key);
    }

    MOCK_METHOD2(OnSessionExpirationUpdate, void(const std::string& session_id, base::Time new_expiry_time));

    const CdmKeysInfo& keys_info() const
    {
        return keys_info_;
    }
...
public:
    ...
    MOCK_METHOD3(Decrypt, void(StreamType stream_type, scoped_refptr<DecoderBuffer> encrypted, DecryptCB decrypt_cb));
    MOCK_METHOD1(CancelDecrypt, void(StreamType stream_type));
    MOCK_METHOD2(InitializeAudioDecoder, void(const AudioDecoderConfig& config, DecoderInitCB init_cb));
    MOCK_METHOD2(InitializeVideoDecoder, void(const VideoDecoderConfig& config, DecoderInitCB init_cb));
    MOCK_METHOD2(DecryptAndDecodeAudio, void(scoped_refptr<DecoderBuffer> encrypted, AudioDecodeCB audio_decode_cb));
    MOCK_METHOD2(DecryptAndDecodeVideo, void(scoped_refptr<DecoderBuffer> encrypted, VideoDecodeCB video_decode_cb));
...
public:
    ...
    MOCK_METHOD1(RegisterEventCB, std::unique_ptr<CallbackRegistration>(EventCB event_cb));
    MOCK_METHOD0(GetDecryptor, Decryptor*());

#if BUILDFLAG(IS_WIN)
    MOCK_METHOD0(RequiresMediaFoundationRenderer, bool());
    MOCK_METHOD0(GetMediaFoundationCdmProxy, scoped_refptr<MediaFoundationCdmProxy>());
#endif
...
public:
    ...
    MOCK_METHOD3(reject, void(CdmPromise::Exception, uint32_t, const std::string&));
};
...
public:
    ...
    // set.
    MockCdmKeyStatusPromise(bool expect_success, CdmKeyInformation::KeyStatus* key_status, CdmPromise::Exception* exception = nullptr);
...
public:
    MockCdm();
    MockCdm(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
    ...
    void Initialize(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
        const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb);

    // ContentDecryptionModule implementation.
    MOCK_METHOD2(SetServerCertificate, void(const std::vector<uint8_t>& certificate, std::unique_ptr<SimpleCdmPromise> promise));
    ...
    MOCK_METHOD3(LoadSession, void(CdmSessionType session_type, const std::string& session_id, std::unique_ptr<NewSessionCdmPromise> promise));
    MOCK_METHOD3(UpdateSession, void(const std::string& session_id, const std::vector<uint8_t>& response, std::unique_ptr<SimpleCdmPromise> promise));
    MOCK_METHOD2(CloseSession, void(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise));
    MOCK_METHOD2(RemoveSession, void(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise));

    MOCK_METHOD0(GetCdmContext, CdmContext*());

    void CallSessionMessageCB(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message);
    void CallSessionClosedCB(const std::string& session_id, CdmSessionClosedReason reason);
    void CallSessionKeysChangeCB(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info);
    void CallSessionExpirationUpdateCB(const std::string& session_id, base::Time new_expiry_time);
...
public:
    ...
    void Create(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
        const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb, CdmCreatedCB cdm_created_cb) override;
...
public:
    ...
    MOCK_METHOD7(Init,
        void(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
            NewMediaSegmentCB new_segment_cb, EndMediaSegmentCB end_of_segment_cb, MediaLog* media_log));
...
public:
    ...
    MOCK_METHOD1(GetAudioRendererAlgorithmParameters, std::optional<::media::AudioRendererAlgorithmParameters>(media::AudioParameters audio_parameters));
    MOCK_METHOD(media::ExternalMemoryAllocator*, GetMediaAllocator, (), (override));
};
...
public:
    ...
    void Initialize(VideoCodecProfile codec_profile, const gfx::Size& encode_size, bool is_hardware_encoder, SVCScalabilityMode svc_mode) override
    {
    ...
    MOCK_METHOD(void, MockInitialize, (VideoCodecProfile, (const gfx::Size&), bool, SVCScalabilityMode));
