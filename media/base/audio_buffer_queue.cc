...
int AudioBufferQueue::ReadFrames(int frames, int dest_frame_offset, AudioBus* dest)
{
    DCHECK_GE(dest->frames(), frames + dest_frame_offset);
    return InternalRead(frames, true, 0, dest_frame_offset, dest);
}

int AudioBufferQueue::PeekFrames(int frames, int source_frame_offset, int dest_frame_offset, AudioBus* dest)
{
    DCHECK_GE(dest->frames(), frames);
    return InternalRead(frames, false, source_frame_offset, dest_frame_offset, dest);
}
...
std::optional<base::TimeDelta> AudioBufferQueue::FrontTimestamp() const
{
    ...
    return buffers_.front()->timestamp() + AudioTimestampHelper::FramesToTime(front_buffer_offset_, buffers_.front()->sample_rate());
}

int AudioBufferQueue::InternalRead(int frames, bool advance_position, int source_frame_offset, int dest_frame_offset, AudioBus* dest)
{
    if (buffers_.empty())
        return 0;
    ...
    while (taken < frames) {
        ...
        if (frames_to_skip > 0) {
            ...
            // if |dest| is NULL, there's no need to copy.
            if (dest) {
                buffer->ReadFrames(copied, current_buffer_offset, dest_frame_offset + taken, dest);
            }
