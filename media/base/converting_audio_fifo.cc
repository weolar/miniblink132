...
ConvertingAudioFifo::ConvertingAudioFifo(const AudioParameters& input_params, const AudioParameters& converted_params)
    : input_params_(input_params)
    , converted_params_(converted_params)
    , converter_(std::make_unique<AudioConverter>(input_params, converted_params,
          /*disable_fifo=*/true))
{
    converter_->AddInput(this);
    converter_->PrimeWithSilence();
    min_input_frames_needed_ = converter_->GetMaxInputFramesRequested(converted_params_.frames_per_buffer());
    ...
    const int kMaxSize = std::ceil(converted_params_.sample_rate() * 0.5 / converted_params_.frames_per_buffer());
    output_pool_ = std::make_unique<AudioBusPoolImpl>(converted_params_, kPreallocated, kMaxSize);
}
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "ConvertingAudioFifo::Convert");

    DCHECK(total_frames_ >= min_input_frames_needed_ || is_flushing_)
        << "total_frames_=" << total_frames_ << ",min_input_frames_needed_=" << min_input_frames_needed_ << ",is_flushing_=" << is_flushing_;
...
double ConvertingAudioFifo::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed, const AudioGlitchInfo& glitch_info)
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "ConvertingAudioFifo::ProvideInput", "delay (frames)", frames_delayed);
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    DCHECK(is_flushing_ || total_frames_ >= frames_needed)
        << "is_flushing_=" << is_flushing_ << ",total_frames_=" << total_frames_ << ",frames_needed=" << frames_needed;
    ...
    while (frames_written < frames_needed && total_frames_) {
        ...
        int frames_to_write = std::min(frames_needed - frames_written, frames_in_front);

        front->CopyPartialFramesTo(front_frame_index_, frames_to_write, frames_written, audio_bus);
    ...
    if (frames_written != frames_needed) {
        ...
        DCHECK(!inputs_.size());
        audio_bus->ZeroFramesPartial(frames_written, frames_needed - frames_written);
    }

    return 1.0f;
}

std::unique_ptr<AudioBus> ConvertingAudioFifo::EnsureExpectedChannelCount(std::unique_ptr<AudioBus> audio_bus)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!mixer_ || mixer_input_params_.channels() != incoming_channels) {
        ...
        mixer_input_params_.Reset(input_params_.format(), ChannelLayoutConfig::Guess(incoming_channels), incoming_channels, input_params_.sample_rate());

        mixer_ = std::make_unique<ChannelMixer>(mixer_input_params_, input_params_);
    }

    auto mixed_bus = AudioBus::Create(input_params_.channels(), audio_bus->frames());
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "ConvertingAudioFifo::PopOutput", "layover_delay (ms)",
        (inputs_.size() * input_params_.GetBufferDuration() + pending_outputs_.size() * converted_params_.GetBufferDuration()).InMillisecondsF());
    CHECK(HasOutput());
