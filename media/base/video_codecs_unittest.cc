...
TEST(ParseVP9CodecId, NewStyleVP9CodecIDs)
{
    ...
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.00", &profile, &level, &color_space));
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.00.10", &profile, &level, &color_space));

    // Expect success when all required fields supplied (and valid).
    // TransferID not specified by string, should default to 709.
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.00.10.08", &profile, &level, &color_space));
    ...
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.01.10.08", &profile, &level, &color_space));
    EXPECT_EQ(VP9PROFILE_PROFILE1, profile);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.08", &profile, &level, &color_space));
    EXPECT_EQ(VP9PROFILE_PROFILE2, profile);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.03.10.08", &profile, &level, &color_space));
    EXPECT_EQ(VP9PROFILE_PROFILE3, profile);
    // Profile 4 is not a thing.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.04.10.08", &profile, &level, &color_space));

    // Verify valid levels parse correctly.
    const std::set<int> kValidVp9Levels = { 10, 11, 20, 21, 30, 31, 40, 41, 50, 51, 52, 60, 61, 62 };
    ...
    for (int i = 0; i < 99; ++i) {
        ...
        if (kValidVp9Levels.find(i) != kValidVp9Levels.end()) {
            EXPECT_TRUE(ParseNewStyleVp9CodecID(codec_string, &profile, &level, &color_space));
            EXPECT_EQ(VP9PROFILE_PROFILE0, profile);
            EXPECT_EQ(i, level);
            EXPECT_EQ(VideoColorSpace::TransferID::BT709, color_space.transfer);
            num_valid_levels++;
        } else {
            EXPECT_FALSE(ParseNewStyleVp9CodecID(codec_string, &profile, &level, &color_space));
        }
    }
    EXPECT_EQ(kValidVp9Levels.size(), num_valid_levels);

    // Verify bitdepths. Only 8, 10, 12 are valid.
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.8", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.12", &profile, &level, &color_space));
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.13", &profile, &level, &color_space));

    // Verify chroma subsampling values.
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.01", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.02", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.03", &profile, &level, &color_space));
    // Values 4 - 7 are reserved.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.10.04", &profile, &level, &color_space));

    // Verify a few color profiles.
    // BT709
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01", &profile, &level, &color_space));
    // BT2020
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.09", &profile, &level, &color_space));
    // 0 is invalid.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.00", &profile, &level, &color_space));
    // 23 - 255 are reserved.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.23", &profile, &level, &color_space));

    // Verify a few common EOTFs parse correctly.
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT709, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.04", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::GAMMA22, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.06", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::SMPTE170M, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.14", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT2020_10, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.12.00.01.15", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT2020_12, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.13", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::IEC61966_2_1, color_space.transfer);
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.16", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::SMPTEST2084, color_space.transfer);
    // Verify 0 and 3 are reserved EOTF values.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.08.00.01.00", &profile, &level, &color_space));
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.08.00.01.03", &profile, &level, &color_space));

    // Verify a few matrix coefficients.
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.00", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.01", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.10", &profile, &level, &color_space));
    // Values 12 - 255 reserved.
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.12", &profile, &level, &color_space));

    // Verify full range flag (boolean 0 or 1).
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.01.00", &profile, &level, &color_space));
    EXPECT_TRUE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.01.01", &profile, &level, &color_space));
    EXPECT_FALSE(ParseNewStyleVp9CodecID("vp09.02.10.10.00.01.01.01.02", &profile, &level, &color_space));
}
...
TEST(ParseAv1CodecId, VerifyRequiredValues)
{
    ...
    EXPECT_FALSE(ParseAv1CodecId("av01.00.04M.08", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.-0.04M.08", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.-1.04M.08", &profile, &level, &color_space));
    ...
    for (int i = 0; i < 99; ++i) {
        ...
        if (i < 32) {
            EXPECT_TRUE(ParseAv1CodecId(codec_string, &profile, &level, &color_space));
            EXPECT_EQ(AV1PROFILE_PROFILE_MAIN, profile);
            EXPECT_EQ(i, level);
            EXPECT_EQ(VideoColorSpace::TransferID::BT709, color_space.transfer);
        } else {
...
TEST(ParseAv1CodecId, VerifyOptionalMonochrome)
{
    ...
    EXPECT_FALSE(ParseAv1CodecId("av01.0.04M.08.00", &profile, &level, &color_space));
    for (int i = 0; i <= 9; ++i) {
        const std::string codec_string = base::StringPrintf("av01.0.00M.08.%d", i);
        SCOPED_TRACE(codec_string);
        EXPECT_EQ(i < 2, ParseAv1CodecId(codec_string, &profile, &level, &color_space));
    }
}
...
TEST(ParseAv1CodecId, VerifyOptionalSubsampling)
{
    ...
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.100", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.010", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.111", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.112", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.113", &profile, &level, &color_space));

    // Invalid cases.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.101", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.102", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.103", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.011", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.012", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.013", &profile, &level, &color_space));

    // The last-value may be non-zero if the first two values are non-zero.
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.110", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.100", &profile, &level, &color_space));
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.010", &profile, &level, &color_space));
    ...
    for (int i = 2; i <= 9; ++i) {
        for (int j = 2; j <= 9; ++j) {
            for (int k = 4; k <= 9; ++k) {
                const std::string codec_string = base::StringPrintf("av01.0.00M.08.0.%d%d%d", i, j, k);
                SCOPED_TRACE(codec_string);
                EXPECT_FALSE(ParseAv1CodecId(codec_string, &profile, &level, &color_space));
            }
        }
...
TEST(ParseAv1CodecId, VerifyOptionalColorProperties)
{
    ...
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.1", &profile, &level, &color_space));
    // Negative values are not allowed.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.-1", &profile, &level, &color_space));

    // BT709
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::PrimaryID::BT709, color_space.primaries);
    // BT2020
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.09", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::PrimaryID::BT2020, color_space.primaries);
    // 0 is invalid.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.00", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::PrimaryID::INVALID, color_space.primaries);
    // 23 - 255 are reserved.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.23", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::PrimaryID::INVALID, color_space.primaries);

    // Leading zeros must be provided.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.1", &profile, &level, &color_space));
    // Negative values are not allowed.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.-1", &profile, &level, &color_space));

    // Verify a few common EOTFs parse correctly.
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT709, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.04", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::GAMMA22, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.06", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::SMPTE170M, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.14", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT2020_10, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.15", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::BT2020_12, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.13", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::IEC61966_2_1, color_space.transfer);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.16", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::TransferID::SMPTEST2084, color_space.transfer);
    // Verify 0 and 3 are reserved EOTF values.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.00", &profile, &level, &color_space));
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.03", &profile, &level, &color_space));

    // Leading zeros must be provided.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.1", &profile, &level, &color_space));
    // Negative values are not allowed.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.-1", &profile, &level, &color_space));

    // Verify a few matrix coefficients.
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::MatrixID::RGB, color_space.matrix);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.01", &profile, &level, &color_space));
    EXPECT_EQ(VideoColorSpace::MatrixID::BT709, color_space.matrix);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.10", &profile, &level, &color_space));
    ...
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.12", &profile, &level, &color_space));

    // Leading zeros are not allowed.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00.00", &profile, &level, &color_space));
    // Negative values are not allowed.
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00.-1", &profile, &level, &color_space));

    // Verify full range flag (boolean 0 or 1).
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00.0", &profile, &level, &color_space));
    EXPECT_EQ(gfx::ColorSpace::RangeID::LIMITED, color_space.range);
    EXPECT_TRUE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00.1", &profile, &level, &color_space));
    EXPECT_EQ(gfx::ColorSpace::RangeID::FULL, color_space.range);
    EXPECT_FALSE(ParseAv1CodecId("av01.0.00M.10.0.000.01.01.00.2", &profile, &level, &color_space));
}
...
TEST(ParseHEVCCodecIdTest, InvalidHEVCCodecIds)
{
    ...
    EXPECT_FALSE(ParseHEVCCodecId("hvc1.1.6.L93.0..0.0.0.0.0", &profile, &level_idc));
    EXPECT_FALSE(ParseHEVCCodecId("hvc1.1.6.L93.0.0.0.0.0.0.", &profile, &level_idc));
    ...
    EXPECT_FALSE(ParseHEVCCodecId("hvc1.1.100000000.L93.B0", &profile, &level_idc));
    EXPECT_FALSE(ParseHEVCCodecId("hvc1.1.FFFFFFFFF.L93.B0", &profile, &level_idc));
    ...
    EXPECT_TRUE(ParseHEVCCodecId("hvc1.1.6.L0.0.0.0.0.0.0", &profile, &level_idc));
    EXPECT_TRUE(ParseHEVCCodecId("hvc1.1.6.L0.00.00.00.00.00.00", &profile, &level_idc));
    ...
    EXPECT_TRUE(ParseHEVCCodecId("hvc1.1.6.L0.12.34.56.78.9A.BC", &profile, &level_idc));
    EXPECT_TRUE(ParseHEVCCodecId("hvc1.1.6.L0.FF.FF.FF.FF.FF.FF", &profile, &level_idc));
    EXPECT_FALSE(ParseHEVCCodecId("hvc1.1.6.L0.FF.FF.FF.FF.FF.FF.0", &profile, &level_idc));
