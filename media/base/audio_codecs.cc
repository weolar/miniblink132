...
AudioCodec StringToAudioCodec(const std::string& codec_id)
{
    ...
    if (codec_id == "ac-4" || codec_id == "mp4a.AE" || codec_id == "mp4a.ae" || base::StartsWith(codec_id, "ac-4.", base::CompareCase::SENSITIVE)) {
    ...
    if (base::StartsWith(codec_id, "mp4a.40.", base::CompareCase::SENSITIVE))
        return AudioCodec::kAAC;
    if (codec_id == "iamf" || base::StartsWith(codec_id, "iamf.", base::CompareCase::SENSITIVE)) {
...
bool ParseDolbyAc4CodecId(const std::string& codec_id, uint8_t* bitstream_versionc, uint8_t* presentation_versionc, uint8_t* presentation_levelc)
{
    ...
    const int kMaxAc4CodecIdLength = 4 // FOURCC string "ac-4"
        + 1 // delimiting period
    ...
    if (elem[1].size() != 2 || !base::HexStringToUInt(elem[1], &bitstream_version) || bitstream_version != 0x02) {
        DVLOG(4) << __func__ << ": invalid Dolby AC-4 bitstream version: " << elem[1];
        return false;
    }
    ...
    if (elem[2].size() != 2 || !base::HexStringToUInt(elem[2], &presentation_version) || !(presentation_version == 0x01 || presentation_version == 0x02)) {
        DVLOG(4) << __func__ << ": invalid Dolby AC-4 presentation_version: " << elem[1];
        return false;
    }
    ...
    if (elem[3].size() != 2 || !base::HexStringToUInt(elem[3], &presentation_level) || presentation_level > 0x07) {
        DVLOG(4) << __func__ << ": invalid Dolby AC-4 presentation_level: " << elem[1];
        return false;
    }
...
bool ParseIamfCodecId(std::string_view codec_id, uint8_t* primary_profilec, uint8_t* additional_profilec)
{
    ...
    constexpr int kMaxIamfCodecIdLength = 4 // FOURCC string "iamf".
        + 1 // delimiting period.
    ...
    if (elem[1].size() != 3 || !base::StringToUint(elem[1], &primary_profile) || primary_profile > 0xFF) {
    ...
    if (elem[2].size() != 3 || !base::StringToUint(elem[2], &additional_profile) || additional_profile > 0xFF) {
    ...
    if (codec.size() != 4 || ((codec != "opus") && (codec != "mp4a") && (codec != "flac") && (codec != "ipcm"))) {
    ...
    if (codec == "mp4a") {
        if (elem.size() != 6) {
            DVLOG(4) << __func__ << ": incorrect mp4a codec string syntax:" << codec_id;
            return false;
        }

        // The fields following "mp4a" should be "40" and "2" to signal AAC-LC.
        uint32_t object_type_indication = 0;
        if (elem[4].size() != 2 || !base::HexStringToUInt(elem[4], &object_type_indication) || object_type_indication != 0x40) {
            DVLOG(4) << __func__ << ": invalid mp4a Object Type Indication:" << codec_id;
            return false;
        }

        uint32_t audio_object_type = 0;
        if (elem[5].size() != 1 || !base::HexStringToUInt(elem[5], &audio_object_type) || audio_object_type != 0x02) {
