...
TEST_P(FrameRateEstimatorTest, AverageConvergesThenReset)
{
    ...
    EXPECT_EQ(estimator_.GetRequiredSamplesForTesting(), estimator_.GetMinSamplesForTesting());

    // Make sure that it doesn't converge before the required sample count.
    ProvideSamples(duration(low_fps()), estimator_.GetRequiredSamplesForTesting() - 1);
    EXPECT_FALSE(estimator_.ComputeFPS());
...
TEST_P(FrameRateEstimatorTest, DurationJitterIsFine)
{
    ...
    ProvideSamples(duration(low_fps()), estimator_.GetRequiredSamplesForTesting());
...
TEST_P(FrameRateEstimatorTest, AverageDoesntSkew)
{
    ...
    EXPECT_EQ(estimator_.GetRequiredSamplesForTesting(), estimator_.GetMaxSamplesForTesting());
    ProvideSamples(duration(high_fps()), estimator_.GetRequiredSamplesForTesting() - 2);
    EXPECT_FALSE(estimator_.ComputeFPS());
    ProvideSample(duration(high_fps()));
...
INSTANTIATE_TEST_SUITE_P(All, FrameRateEstimatorTest,
    testing::Values(FpsPair(24, 30), FpsPair(24, 60), FpsPair(24, 90), FpsPair(24, 120), FpsPair(24, 240),

        FpsPair(30, 60), FpsPair(30, 90), FpsPair(30, 120), FpsPair(30, 240),

        FpsPair(60, 90), FpsPair(60, 120), FpsPair(60, 240),

        FpsPair(90, 120), FpsPair(90, 240),
