...
// static
base::TimeDelta AudioTimestampHelper::FramesToTime(int64_t frames, int samples_per_second)
{
    CHECK_GT(samples_per_second, 0);
    return base::Microseconds(frames * base::Time::kMicrosecondsPerSecond / samples_per_second);
}

// static
int64_t AudioTimestampHelper::TimeToFrames(base::TimeDelta time, int samples_per_second)
{
...
base::TimeDelta AudioTimestampHelper::GetFrameDuration(int frame_count) const
{
    ...
    if ((current_timestamp.is_min() && end_timestamp.is_min()) || (current_timestamp.is_max() && end_timestamp.is_max())) {
...
base::TimeDelta AudioTimestampHelper::ComputeTimestamp(int64_t frame_count) const
{
    CHECK_GE(frame_count, 0);
    CHECK(base_timestamp_);
