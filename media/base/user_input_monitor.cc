...
uint32_t ReadKeyPressMonitorCount(const base::ReadOnlySharedMemoryMapping& readonly_mapping)
{
    ...
    return base::subtle::NoBarrier_Load(reinterpret_cast<const base::subtle::Atomic32*>(readonly_mapping.memory()));
}

void WriteKeyPressMonitorCount(base::WritableSharedMemoryMapping& writable_mapping, uint32_t count)
{
    ...
    // No ordering constraints between Load/Store operations, a temporary
    // inconsistent value is fine.
    base::subtle::NoBarrier_Store(static_cast<base::subtle::Atomic32*>(writable_mapping.memory()), count);
}
...
base::ReadOnlySharedMemoryRegion UserInputMonitorBase::EnableKeyPressMonitoringWithMapping()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(owning_sequence_);
    if (++references_ == 1) {
        base::MappedReadOnlyRegion shmem = base::ReadOnlySharedMemoryRegion::Create(sizeof(uint32_t));
        if (!shmem.IsValid()) {
            DVLOG(2) << "Error mapping key press count shmem.";
            return base::ReadOnlySharedMemoryRegion();
        }

        key_press_count_region_ = base::ReadOnlySharedMemoryRegion(std::move(shmem.region));
