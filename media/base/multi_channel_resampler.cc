...
MultiChannelResampler::MultiChannelResampler(int channels, double io_sample_rate_ratio, size_t request_size, const ReadCB read_cb)
    ...
    , output_frames_ready_(0)
{
    ...
    for (int i = 0; i < channels; ++i) {
        resamplers_.push_back(std::make_unique<SincResampler>(
            io_sample_rate_ratio, request_size, base::BindRepeating(&MultiChannelResampler::ProvideInput, base::Unretained(this), i)));
    }
...
void MultiChannelResampler::Resample(int frames, AudioBus* audio_bus)
{
    ...
    while (output_frames_ready_ < frames) {
        ...
        for (size_t i = 0; i < resamplers_.size(); ++i) {
            ...
            // number of frames.
            resamplers_[i]->Resample(frames_this_time, audio_bus->channel(i) + output_frames_ready_);
        }
...
void MultiChannelResampler::ProvideInput(int channel, int frames, float* destination)
{
    ...
    if (channel == 0) {
        ...
        memcpy(destination, wrapped_resampler_audio_bus_->channel(channel), sizeof(*wrapped_resampler_audio_bus_->channel(channel)) * frames);
    }
}
...
int MultiChannelResampler::GetMaxInputFramesRequested(int output_frames_requested) const
{
    DCHECK(!resamplers_.empty());
