...
using WriteKeyPressCallback = base::RepeatingCallback<void(base::WritableSharedMemoryMapping& shmem, uint32_t count)>;
...
public:
    ...
    virtual void StartMonitorWithMapping(WriteKeyPressCallback callback, base::WritableSharedMemoryMapping mapping) = 0;
...
template <typename Impl> class UserInputMonitorLinuxCore final : public UserInputMonitorAdapter {
public:
    ...
    UserInputMonitorLinuxCore operator=(const UserInputMonitorLinuxCore&) = delete;
    ...
    void StartMonitorWithMapping(WriteKeyPressCallback callback, base::WritableSharedMemoryMapping mapping) override
    {
        if (!user_input_monitor_)
            return;
...
public:
    explicit UserInputMonitorLinux(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner);
...
UserInputMonitorAdapter* CreateUserInputMonitor(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner)
{
    return new UserInputMonitorLinuxCore<ui::PlatformUserInputMonitor>(ui::OzonePlatform::GetInstance()->GetPlatformUserInputMonitor(io_task_runner));
}
...
UserInputMonitorLinux::UserInputMonitorLinux(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner)
...
void UserInputMonitorLinux::StartKeyboardMonitoring()
{
    ...
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&UserInputMonitorAdapter::StartMonitor, core_->AsWeakPtr(), base::BindRepeating(&WriteKeyPressMonitorCount)));
}

void UserInputMonitorLinux::StartKeyboardMonitoring(base::WritableSharedMemoryMapping mapping)
{
    if (!core_)
        return;
    io_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &UserInputMonitorAdapter::StartMonitorWithMapping, core_->AsWeakPtr(), base::BindRepeating(&WriteKeyPressMonitorCount), std::move(mapping)));
}
...
void UserInputMonitorLinux::StopKeyboardMonitoring()
{
    if (!core_)
        return;
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&UserInputMonitorAdapter::StopMonitor, core_->AsWeakPtr()));
}
