...
MediaLog::MediaLog()
    : MediaLog(new ParentLogRecord(this))
{
}
...
{
    std::unique_ptr<MediaLogRecord> record(CreateRecord(MediaLogRecord::Type::kMessage));
...
void MediaLog::EmitConsoleErrorLog(base::Value::Dict status_dict)
{
    ...
    auto file = stack->GetList().front().GetDict().Extract(StatusConstants::kFileKey);
    DCHECK(file);
    DCHECK(file->is_string());

    auto line = stack->GetList().front().GetDict().Extract(StatusConstants::kLineKey);
    DCHECK(line);
    DCHECK(line->is_int());

    auto log_writer = logging::LogMessage(file->GetString().c_str(), line->GetInt(), logging::LOGGING_ERROR);
    if (auto message = status_dict.Extract(StatusConstants::kMsgKey); message && message->is_string()) {
...
std::unique_ptr<MediaLogRecord> MediaLog::CreateRecord(MediaLogRecord::Type type)
{
...
LogHelper::LogHelper(MediaLogMessageLevel level, MediaLog* media_log, const char* file, int line, std::optional<logging::SystemErrorCode> code)
    : file_(file)
    , line_(line)
    , level_(level)
...
LogHelper::LogHelper(
    MediaLogMessageLevel level, const std::unique_ptr<MediaLog>& media_log, const char* file, int line, std::optional<logging::SystemErrorCode> code)
...
LogHelper::~LogHelper()
{
    ...
    if (media_log_->ShouldLogToDebugConsole()) {
        ...
        case MediaLogMessageLevel::kERROR:
            ...
            if (DLOG_IS_ON(ERROR)) {
                logging::LogMessage(file_, line_, logging::LOGGING_ERROR).stream() << log;
            }
            break;
        case MediaLogMessageLevel::kWARNING:
            if (DLOG_IS_ON(WARNING) && base::FeatureList::IsEnabled(kMediaLogToConsole)) {
                logging::LogMessage(file_, line_, logging::LOGGING_WARNING).stream() << log;
            }
            break;
        case MediaLogMessageLevel::kINFO:
        case MediaLogMessageLevel::kDEBUG:
            if (DLOG_IS_ON(INFO) && base::FeatureList::IsEnabled(kMediaLogToConsole)) {
                logging::LogMessage(file_, line_, logging::LOGGING_INFO).stream() << log;
            }
            break;
        }
    }
