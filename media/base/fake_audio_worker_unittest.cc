...
public:
    FakeAudioWorkerTest()
        : params_(AudioParameters::AUDIO_FAKE, ChannelLayoutConfig::Stereo(), 44100, 128)
        , fake_worker_(task_environment_.GetMainThreadTaskRunner(), params_)
    {
        time_between_callbacks_
            = base::Microseconds(params_.frames_per_buffer() * base::Time::kMicrosecondsPerSecond / static_cast<float>(params_.sample_rate()));
    }
    ...
    void RunOnAudioThread()
    {
        ...
        fake_worker_.Start(base::BindRepeating(&FakeAudioWorkerTest::CalledByFakeWorker, base::Unretained(this)));
    }
    ...
    void TimeCallbacksOnAudioThread(size_t callbacks)
    {
        ...
        if (callbacks_.size() < callbacks) {
            TaskRunner()->PostDelayedTask(
                FROM_HERE, base::BindOnce(&FakeAudioWorkerTest::TimeCallbacksOnAudioThread, base::Unretained(this), callbacks), time_between_callbacks_ / 2);
        } else {
            EndTest();
        }
    }
...
protected:
    base::test::SingleThreadTaskEnvironment task_environment_ { base::test::SingleThreadTaskEnvironment::TimeSource::MOCK_TIME };
...
{
    base::OnceClosure run_on_audio_thread = base::BindOnce(&FakeAudioWorkerTest::RunOnAudioThread, base::Unretained(this));
    base::OnceClosure end_test = base::BindOnce(&FakeAudioWorkerTest::EndTest, base::Unretained(this));
    ...
    task_environment_.GetMainThreadTaskRunner()->PostTaskAndReply(FROM_HERE, std::move(run_on_audio_thread), std::move(end_test));
...
{
    TaskRunner()->PostTask(FROM_HERE, base::BindOnce(&FakeAudioWorkerTest::TimeCallbacksOnAudioThread, base::Unretained(this), kTestCallbacks));
    ...
    for (size_t i = 0; i < callbacks_.size() - 1; i++) {
        time_between_callbacks.push_back((callbacks_.at(i + 1) - callbacks_.at(i)).InMilliseconds());
    }

    EXPECT_THAT(time_between_callbacks, testing::Each(time_between_callbacks_.InMilliseconds()));
}
...
{
    TaskRunner()->PostTask(FROM_HERE, base::BindOnce(&FakeAudioWorkerTest::RunOnAudioThread, base::Unretained(this)));
    ...
    EXPECT_THAT(callbacks_, SizeIs(1));
    TaskRunner()->PostTask(FROM_HERE, base::BindOnce(&FakeAudioWorkerTest::StopStartOnAudioThread, base::Unretained(this)));
    ...
    TaskRunner()->PostTask(FROM_HERE, base::BindOnce(&FakeAudioWorkerTest::EndTest, base::Unretained(this)));
...
public:
        ...
        , fake_worker_(task_runner_, params_)
    {
        ...
        time_between_callbacks_
            = base::Microseconds(params_.frames_per_buffer() * base::Time::kMicrosecondsPerSecond / static_cast<float>(params_.sample_rate()));
        clock_overrides_ = std::make_unique<base::subtle::ScopedTimeClockOverrides>(nullptr, TimeTicksOverride, nullptr);
    }

    FakeAudioWorkerMockTaskTest(const FakeAudioWorkerMockTaskTest&) = delete;
    FakeAudioWorkerMockTaskTest& operator=(const FakeAudioWorkerMockTaskTest&) = delete;
    ...
    {
        {
            ...
            fake_worker_.Start(base::BindRepeating(&FakeAudioWorkerMockTaskTest::CalledByFakeWorker, base::Unretained(this)));
        }
    }
...
protected:
    scoped_refptr<base::TestMockTimeTaskRunner> task_runner_ = new base::TestMockTimeTaskRunner();
