...
FakeDemuxerStream::FakeDemuxerStream(int num_configs, int num_buffers_in_one_config, bool is_encrypted)
    : FakeDemuxerStream(num_configs, num_buffers_in_one_config, is_encrypted, gfx::Size(kDefaultStartWidth, kDefaultStartHeight),
        gfx::Vector2dF(kDefaultWidthDelta, kDefaultHeightDelta))
{
}

FakeDemuxerStream::FakeDemuxerStream(
    int num_configs, int num_buffers_in_one_config, bool is_encrypted, gfx::Size start_coded_size, gfx::Vector2dF coded_size_delta)
...
void FakeDemuxerStream::UpdateVideoDecoderConfig()
{
    ...
    video_decoder_config_.Initialize(VideoCodec::kVP8, VIDEO_CODEC_PROFILE_UNKNOWN, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(),
        kNoTransformation, next_size_, kVisibleRect, next_size_, EmptyExtraData(), is_encrypted_ ? EncryptionScheme::kCenc : EncryptionScheme::kUnencrypted);
...
void FakeDemuxerStream::DoRead(int read_count)
{
    ...
    for (int i = 0; i < read_count; ++i) {
        scoped_refptr<DecoderBuffer> buffer = CreateFakeVideoBufferForTest(video_decoder_config_, current_timestamp_, duration_);

        // TODO(xhwang): Output out-of-order buffers if needed.
        if (is_encrypted_) {
            buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(
                std::string(kKeyId, kKeyId + std::size(kKeyId)), std::string(kIv, kIv + std::size(kIv)), std::vector<SubsampleEntry>()));
        }
...
FakeMediaResource::FakeMediaResource(int num_video_configs, int num_video_buffers_in_one_config, bool is_video_encrypted)
    : fake_video_stream_(num_video_configs, num_video_buffers_in_one_config, is_video_encrypted)
{
}
