...
BitstreamBuffer::BitstreamBuffer(int32_t id, base::UnsafeSharedMemoryRegion region, size_t size, uint64_t offset, base::TimeDelta presentation_timestamp)
    : id_(id)
...
scoped_refptr<DecoderBuffer> BitstreamBuffer::ToDecoderBuffer(off_t offset, size_t size)
{
    ...
    if (total_range.ValueOrDie<size_t>() > region_.GetSize())
        return nullptr;
    scoped_refptr<DecoderBuffer> buffer = DecoderBuffer::FromSharedMemoryRegion(std::move(region_), final_offset, size);
    if (!buffer)
        return nullptr;
    buffer->set_timestamp(presentation_timestamp_);
    if (!key_id_.empty()) {
        buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(key_id_, iv_, subsamples_));
    }
    return buffer;
}

void BitstreamBuffer::SetDecryptionSettings(const std::string& key_id, const std::string& iv, const std::vector<SubsampleEntry>& subsamples)
{
    key_id_ = key_id;
    iv_ = iv;
