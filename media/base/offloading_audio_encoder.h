...
public:
    ...
    // runner.
    OffloadingAudioEncoder(std::unique_ptr<AudioEncoder> wrapped_encoder, scoped_refptr<base::SequencedTaskRunner> work_runner,
        scoped_refptr<base::SequencedTaskRunner> callback_runner);

    // Uses current task runner for callbacks and asks thread pool for a new task
    // runner to do actual encoding work.
    explicit OffloadingAudioEncoder(std::unique_ptr<AudioEncoder> wrapped_encoder);
