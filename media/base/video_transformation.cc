...
template <size_t decimal_bits> double FixedToFloatingPoint(int64_t i)
{
...
bool operator==(const struct VideoTransformation& first, const struct VideoTransformation& second)
{
    return first.rotation == second.rotation && first.mirrored == second.mirrored;
}

// static
VideoTransformation VideoTransformation::FromFFmpegDisplayMatrix(const int32_t* matrix3x3)
{
...
VideoTransformation::VideoTransformation(const int32_t matrix[4])
{
    ...
    if (abs(matrix64[0]) != abs(matrix64[3]) || abs(matrix64[1]) != abs(matrix64[2])) {
    ...
    double check_angle = base::RadToDeg(asin(FixedToFloatingPoint<16>(matrix64[1])));
...
VideoTransformation VideoTransformation::add(VideoTransformation delta) const
{
    ...
    if (mirrored) {
        ...
        return VideoTransformation(static_cast<VideoRotation>(combined_rotation), !delta.mirrored);
    }
