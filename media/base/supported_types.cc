...
template <typename T> class SupplementalProfileCache {
public:
...
SupplementalProfileCache<VideoCodecProfile>* GetSupplementalDecoderVideoProfileCache()
{
...
SupplementalProfileCache<VideoCodecProfile>* GetSupplementalEncoderVideoProfileCache()
{
    static base::NoDestructor<SupplementalProfileCache<VideoCodecProfile>> cache;
    return cache.get();
}
...
bool IsDecoderSupportedHdrMetadata(const VideoType& type)
{
    ...
    case gfx::HdrMetadataType::kSmpteSt2086:
        ...
        return type.color_space.transfer == VideoColorSpace::TransferID::SMPTEST2084;
...
bool IsDecoderHevcProfileSupported(const VideoType& type)
{
    ...
    // for HEVC support.
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kLacrosEnablePlatformHevc)) {
        return true;
    }
    ...
    return GetSupplementalDecoderVideoProfileCache()->IsProfileSupported(type.profile);
#else
    return true;
...
bool IsDecoderVp9ProfileSupported(const VideoType& type)
{
    ...
    static const bool vpx_supports_hbd = (vpx_codec_get_caps(vpx_codec_vp9_dx()) & VPX_CODEC_CAP_HIGHBITDEPTH) != 0;
    ...
    case VP9PROFILE_PROFILE2:
        return vpx_supports_hbd || MediaCodecUtil::IsVp9Profile2DecoderAvailable();
    case VP9PROFILE_PROFILE3:
        return vpx_supports_hbd || MediaCodecUtil::IsVp9Profile3DecoderAvailable();
#else
    case VP9PROFILE_PROFILE2:
    case VP9PROFILE_PROFILE3:
...
bool IsDecoderAV1Supported(const VideoType& type)
{
    ...
    return base::android::BuildInfo::GetInstance()->sdk_int() >= base::android::SDK_VERSION_Q && IsDecoderColorSpaceSupported(type.color_space);
#else
    return false;
#endif
}
...
{
#if BUILDFLAG(ENABLE_PLATFORM_HEVC) && BUILDFLAG(PLATFORM_HAS_OPTIONAL_HEVC_DECODE_SUPPORT) && BUILDFLAG(ENABLE_PLATFORM_DOLBY_VISION)
    return GetSupplementalDecoderVideoProfileCache()->IsProfileSupported(type.profile);
#else
    return false;
#endif
...
#if BUILDFLAG(ENABLE_MOJO_AUDIO_DECODER) && (BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC))
    return GetSupplementalDecoderAudioTypeCache()->IsProfileSupported(type);
#else
...
{
#if BUILDFLAG(ENABLE_PLATFORM_AC4_AUDIO) && BUILDFLAG(ENABLE_MOJO_AUDIO_DECODER) && BUILDFLAG(IS_WIN)
    return GetSupplementalDecoderAudioTypeCache()->IsProfileSupported(type);
#else
    return false;
...
bool IsEncoderH264ProfileSupported(const VideoType& type)
{
    ...
    // instead.
    return GetSupplementalEncoderVideoProfileCache()->IsProfileSupported(type.profile);
#else
    return false;
...
bool IsDefaultDecoderSupportedVideoType(const VideoType& type)
{
    ...
    case VideoCodec::kH264:
        return true;
    case VideoCodec::kVP8:
        return IsDecoderBuiltInVideoCodec(type.codec) ? true : GetSupplementalDecoderVideoProfileCache()->IsProfileSupported(type.profile);
    case VideoCodec::kAV1:
...
bool IsDecoderBuiltInVideoCodec(VideoCodec codec)
{
    ...
    if (codec == VideoCodec::kH264 && base::FeatureList::IsEnabled(kBuiltInH264Decoder)) {
        return true;
    }
...
bool MayHaveAndAllowSelectOSSoftwareEncoder(VideoCodec codec)
{
    ...
    constexpr bool kHasOSSoftwareH264Encoder = BUILDFLAG(IS_MAC) || BUILDFLAG(IS_ANDROID);
    constexpr bool kHasOSSoftwareHEVCEncoder = BUILDFLAG(IS_MAC) && BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER);
...
void UpdateDefaultDecoderSupportedVideoProfiles(const base::flat_set<media::VideoCodecProfile>& profiles)
{
    GetSupplementalDecoderVideoProfileCache()->UpdateCache(profiles);
}

void UpdateDefaultDecoderSupportedAudioTypes(const base::flat_set<AudioType>& types)
{
    GetSupplementalDecoderAudioTypeCache()->UpdateCache(types);
}

void UpdateDefaultEncoderSupportedVideoProfiles(const base::flat_set<media::VideoCodecProfile>& profiles)
{
