...
public:
    ...
    struct MEDIA_EXPORT SessionData {
        SessionData(std::vector<uint8_t> key_set_id, std::string mime_type, MediaDrmKeyType key_type);
    ...
    using InitCB = base::OnceCallback<void(bool success, const MediaDrmOriginId& origin_id)>;
    ...
    using LoadPersistentSessionCB = base::OnceCallback<void(std::unique_ptr<SessionData> session_data)>;
    ...
    // information can be cleared based on selected time range (e.g. for clearing
    // browsing data).
    virtual void SavePersistentSession(const std::string& session_id, const SessionData& session_data, ResultCB result_cb) = 0;

    // Loads the persistent session info for |session_id| from the storage.
    virtual void LoadPersistentSession(const std::string& session_id, LoadPersistentSessionCB load_persistent_session_cb) = 0;
    ...
    virtual void RemovePersistentSession(const std::string& session_id, ResultCB result_cb) = 0;
...
using CreateStorageCB = base::RepeatingCallback<std::unique_ptr<MediaDrmStorage>()>;
