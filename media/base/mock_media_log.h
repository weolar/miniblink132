...
namespace {
struct TypeAndCodec {
    TypeAndCodec(std::string t, std::string c)
        : type(t)
        , codec(c)
    {
    }
...
#define EXPECT_MEDIA_LOG_ON(log, x) EXPECT_CALL((log), DoAddLogRecordLogString((x)))
...
#define EXPECT_MEDIA_LOG_PROPERTY(property, value)                                                                                                             \
    EXPECT_CALL(media_log_,                                                                                                                                    \
        DoAddLogRecordLogString(                                                                                                                               \
            MatchesPropertyExactValue(MockMediaLog::MediaEventToLogString(*media_log_.CreatePropertyTestEvent<MediaLogProperty::property>(value)))))

#define EXPECT_MEDIA_LOG_PROPERTY_ANY_VALUE(property)                                                                                                          \
    EXPECT_CALL(media_log_, DoAddLogRecordLogString(MatchesPropertyAnyValue("{\"" + MediaLogPropertyKeyToString(MediaLogProperty::property) + "\"")))

#define EXPECT_FOUND_CODEC_NAME(stream_type, codec_name)                                                                                                       \
    EXPECT_CALL(media_log_,                                                                                                                                    \
        DoAddLogRecordLogString(TracksHasCodecName(TypeAndCodec(MediaLogPropertyKeyToString(MediaLogProperty::k##stream_type##Tracks), codec_name))))

namespace media {

MATCHER_P(TracksHasCodecName, tandc, "")
{
    return (arg.substr(0, 15) == "{\"" + tandc.type + "\"") && (arg[16] != ']') && CONTAINS_STRING(arg, tandc.codec);
}
...
public:
    ...
    template <MediaLogProperty P, typename T> std::unique_ptr<MediaLogRecord> CreatePropertyTestEvent(const T& value)
    {
