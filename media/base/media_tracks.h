...
public:
    ...
    template <typename T> using ConfigMap = std::map<StreamParser::TrackId, T>;
    ...
    MediaTrack* AddAudioTrack(const AudioDecoderConfig& config, bool enabled, StreamParser::TrackId stream_id, const MediaTrack::Kind& kind,
        const MediaTrack::Label& label, const MediaTrack::Language& language, bool exclusive = true);
    // Adds a new video track. The |bytestreamTrackId| must uniquely identify the
    // track within the bytestream.
    MediaTrack* AddVideoTrack(const VideoDecoderConfig& config, bool enabled, StreamParser::TrackId stream_id, const MediaTrack::Kind& kind,
        const MediaTrack::Label& label, const MediaTrack::Language& language);

    const MediaTracksCollection& tracks() const
    {
        return tracks_;
    }
    ...
    const AudioDecoderConfig& getAudioConfig(StreamParser::TrackId stream_id) const;
    const VideoDecoderConfig& getVideoConfig(StreamParser::TrackId stream_id) const;
