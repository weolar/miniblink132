...
VideoThumbnailDecoder::VideoThumbnailDecoder(std::unique_ptr<VideoDecoder> decoder, const VideoDecoderConfig& config, std::vector<uint8_t> encoded_data)
...
void VideoThumbnailDecoder::Start(VideoFrameCallback video_frame_callback)
{
    ...
    video_frame_callback_ = base::BindPostTaskToCurrentDefault(std::move(video_frame_callback));

    decoder_->Initialize(config_, false, nullptr, base::BindOnce(&VideoThumbnailDecoder::OnVideoDecoderInitialized, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&VideoThumbnailDecoder::OnVideoFrameDecoded, weak_factory_.GetWeakPtr()), base::DoNothing());
}
...
void VideoThumbnailDecoder::OnVideoDecoderInitialized(DecoderStatus status)
{
    ...
    encoded_data_.clear();
    decoder_->Decode(buffer, base::BindOnce(&VideoThumbnailDecoder::OnVideoBufferDecoded, weak_factory_.GetWeakPtr()));
}
...
void VideoThumbnailDecoder::OnVideoBufferDecoded(DecoderStatus status)
{
    ...
    decoder_->Decode(DecoderBuffer::CreateEOSBuffer(), base::BindOnce(&VideoThumbnailDecoder::OnEosBufferDecoded, weak_factory_.GetWeakPtr()));
}
...
void VideoThumbnailDecoder::OnVideoFrameDecoded(scoped_refptr<VideoFrame> frame)
{
