...
uint32_t CdmPromiseAdapter::SavePromise(std::unique_ptr<CdmPromise> promise, const std::string& operation)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "CdmPromise", TRACE_ID_WITH_SCOPE("CdmPromise", promise_id), "operation", operation);

    return promise_id;
}

template <typename... T> void CdmPromiseAdapter::ResolvePromise(uint32_t promise_id, const T&... result)
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END1("media", "CdmPromise", TRACE_ID_WITH_SCOPE("CdmPromise", promise_id), "status", "resolved");
...
void CdmPromiseAdapter::RejectPromise(uint32_t promise_id, CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
{
    std::unique_ptr<CdmPromise> promise = TakePromise(promise_id);
    if (!promise) {
        LOG(ERROR) << "Promise not found for " << promise_id;
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END2("media", "CdmPromise", TRACE_ID_WITH_SCOPE("CdmPromise", promise_id), "status", "rejected", "system_code", system_code);
...
void CdmPromiseAdapter::Clear(ClearReason reason)
{
    ...
    for (auto& [promise_id, promise] : promises_) {
        TRACE_EVENT_NESTABLE_ASYNC_END1("media", "CdmPromise", TRACE_ID_WITH_SCOPE("CdmPromise", promise_id), "status", "cleared");
        promise->reject(CdmPromise::Exception::INVALID_STATE_ERROR, ToSystemCode(reason), "Operation aborted.");
    }
    promises_.clear();
}

std::unique_ptr<CdmPromise> CdmPromiseAdapter::TakePromise(uint32_t promise_id)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
template MEDIA_EXPORT void CdmPromiseAdapter::ResolvePromise(uint32_t, const CdmKeyInformation::KeyStatus&);
template MEDIA_EXPORT void CdmPromiseAdapter::ResolvePromise(uint32_t, const std::string&);
