...
public:
    enum class Exception { NOT_SUPPORTED_ERROR, INVALID_STATE_ERROR, QUOTA_EXCEEDED_ERROR, TYPE_ERROR, EXCEPTION_MAX = TYPE_ERROR };

    enum ResolveParameterType { VOID_TYPE, INT_TYPE, STRING_TYPE, KEY_STATUS_TYPE };
    ...
    virtual void reject(Exception exception_code, uint32_t system_code, const std::string& error_message) = 0;
...
template <typename... T> struct CdmPromiseTraits { };

template <> struct MEDIA_EXPORT CdmPromiseTraits<> {
    static const CdmPromise::ResolveParameterType kType;
};

template <> struct MEDIA_EXPORT CdmPromiseTraits<int> {
    static const CdmPromise::ResolveParameterType kType;
};

template <> struct MEDIA_EXPORT CdmPromiseTraits<std::string> {
    static const CdmPromise::ResolveParameterType kType;
};

template <> struct MEDIA_EXPORT CdmPromiseTraits<CdmKeyInformation::KeyStatus> {
    static const CdmPromise::ResolveParameterType kType;
};
...
template <typename... T> class CdmPromiseTemplate : public CdmPromise {
public:
    CdmPromiseTemplate()
        : is_settled_(false)
    {
...
template <> MEDIA_EXPORT CdmPromise::ResolveParameterType CdmPromiseTemplate<>::GetResolveParameterType() const;

template <> MEDIA_EXPORT CdmPromise::ResolveParameterType CdmPromiseTemplate<int>::GetResolveParameterType() const;

template <> MEDIA_EXPORT CdmPromise::ResolveParameterType CdmPromiseTemplate<std::string>::GetResolveParameterType() const;

template <> MEDIA_EXPORT CdmPromise::ResolveParameterType CdmPromiseTemplate<CdmKeyInformation::KeyStatus>::GetResolveParameterType() const;

// A dummy CdmPromise that does nothing. Used for APIs requiring a CdmPromise
// while the result will be ignored.
template <typename... T> class MEDIA_EXPORT DoNothingCdmPromise : public CdmPromiseTemplate<T...> {
public:
    ...
    void reject(CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message) final
    {
        CdmPromiseTemplate<T...>::MarkPromiseSettled();
    }
};
