...
LimitingAudioQueue::LimitingAudioQueue(ChannelLayout channel_layout, int sample_rate, int channels, int max_buffer_size)
...
void LimitingAudioQueue::Push(const AudioBus& input, int num_frames, base::TimeDelta timestamp, OutputCB output_cb)
{
    ...
    // by the buffer pool.
    auto buffer = AudioBuffer::CreateBuffer(kSampleFormatPlanarF32, channel_layout_, channels_, sample_rate_, max_buffer_size_, pool_);
    ...
    limiter_->LimitPeaksPartial(input, num_frames, output_spans, base::BindOnce(std::move(output_cb), std::move(buffer)));
}
