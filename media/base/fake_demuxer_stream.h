...
public:
    ...
    FakeDemuxerStream(int num_configs, int num_buffers_in_one_config, bool is_encrypted);
    // Constructs an object that outputs |num_configs| different configs in
    // sequence with |num_frames_in_one_config| buffers for each config. The
    // output buffers are encrypted if |is_encrypted| is true.
    ...
    // properties.
    FakeDemuxerStream(int num_configs, int num_buffers_in_one_config, bool is_encrypted, gfx::Size start_coded_size, gfx::Vector2dF coded_size_delta);
    ...
    int num_buffers_returned() const
    {
        return num_buffers_returned_;
    }
...
public:
    ...
    FakeMediaResource(int num_video_configs, int num_video_buffers_in_one_config, bool is_video_encrypted);
