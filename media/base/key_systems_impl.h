...
using RegisterKeySystemsSupportCB = base::OnceCallback<std::unique_ptr<KeySystemSupportRegistration>(GetSupportedKeySystemsCB)>;
...
public:
    ...
    EmeConfig::Rule GetEncryptionSchemeConfigRule(const std::string& key_system, EncryptionScheme encryption_scheme) const override;
    ...
    void AddCodecMaskForTesting(EmeMediaType media_type, const std::string& codec, uint32_t mask);
    void AddMimeTypeCodecMaskForTesting(const std::string& mime_type, uint32_t mask);
...
private:
    ...
    bool IsValidMimeTypeCodecsCombination(const std::string& mime_type, SupportedCodecs codecs) const;

    // TODO(crbug.com/40386158): Separate container enum from codec mask value.
    // Potentially pass EmeMediaType and a container enum.
    SupportedCodecs GetCodecMaskForMimeType(const std::string& container_mime_type) const;
    ...
    EmeCodec GetEmeCodecForString(EmeMediaType media_type, const std::string& container_mime_type, const std::string& codec_string) const;
    ...
    std::unique_ptr<KeySystemSupportRegistration> key_system_support_registration_;
