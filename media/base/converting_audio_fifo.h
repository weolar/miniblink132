...
class MEDIA_EXPORT ConvertingAudioFifo final : public AudioConverter::InputCallback {
public:
    ConvertingAudioFifo(const AudioParameters& input_params, const AudioParameters& converted_params);
...
private:
    ...
    std::unique_ptr<AudioBus> EnsureExpectedChannelCount(std::unique_ptr<AudioBus> audio_bus);
    ...
    std::unique_ptr<AudioConverter> converter_ GUARDED_BY_CONTEXT(sequence_checker_);

    base::circular_deque<std::unique_ptr<AudioBus>> pending_outputs_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::unique_ptr<AudioBusPool> output_pool_ GUARDED_BY_CONTEXT(sequence_checker_);

    // All current input frames.
    base::circular_deque<std::unique_ptr<AudioBus>> inputs_ GUARDED_BY_CONTEXT(sequence_checker_);
