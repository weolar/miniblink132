...
static void GetSizes(int pos, int max_size, int in_size, int* size, int* wrap_size)
{
...
void AudioFifo::Push(const AudioBus* source, int source_size)
{
    ...
    TRACE_EVENT_BEGIN2(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Push", "this", static_cast<void*>(this), "fifo frames", frames_);
    ...
    TRACE_EVENT_END1(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Push", "frames", source_size);
}

void AudioFifo::Consume(AudioBus* destination, int start_frame, int frames_to_consume)
{
    DCHECK(destination);
    ...
    TRACE_EVENT_BEGIN2(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Consume", "this", static_cast<void*>(this), "fifo frames", frames_);

    // Figure out if wrapping is needed and if so what segment sizes we need
    // when removing audio bus content from the FIFO.
    int consume_size = 0;
    int wrap_size = 0;
    GetSizes(read_pos_, max_frames_, frames_to_consume, &consume_size, &wrap_size);
    ...
    for (int ch = 0; ch < destination->channels(); ++ch) {
        ...
        if (wrap_size > 0) {
            ...
            memcpy(&dest[consume_size + start_frame], &src[0], wrap_size * sizeof(src[0]));
        }
    }

    frames_ -= frames_to_consume;
    read_pos_ = UpdatePos(read_pos_, frames_to_consume, max_frames_);
    TRACE_EVENT_END1(TRACE_DISABLED_BY_DEFAULT("audio"), "AudioFifo::Consume", "frames", frames_to_consume);
}
