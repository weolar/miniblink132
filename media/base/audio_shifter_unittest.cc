...
public:
    AudioShifterTest()
        : shifter_(base::Milliseconds(2000), base::Milliseconds(3), base::Milliseconds(100), kSampleRate, 2)
    ...
    void SetUp() override
    {
        SetupInput(kInputPacketSize + ::testing::get<0>(GetParam()) - 1, base::Microseconds(1000 + ::testing::get<1>(GetParam()) * 5 - 5));
        SetupOutput(kOutputPacketSize, base::Microseconds(500 + ::testing::get<2>(GetParam()) * 3 - 3));
    ...
    void Run(size_t loops)
    {
        for (size_t i = 0; i < loops;) {
            ...
            if (now_ >= time_to_pull_) {
                ...
                bool silence = true;
                for (size_t j = 0; j < static_cast<size_t>(test_output_->frames()); j++) {
                    if (test_output_->channel(0)[j] != 0.0) {
                        silence = false;
                        if (test_output_->channel(0)[j] > 3000000.0) {
                            marker_outputs_.push_back(now_ + playback_latency_ + base::Seconds(j) / kSampleRate);
                        } else {
                            // We don't expect smooth output once we insert a tag,
                            // or in the very beginning.
                            if (expect_smooth_output_ && output_sample_ > 500.0) {
                                EXPECT_GT(test_output_->channel(0)[j], output_sample_ - 3) << "j = " << j;
            ...
            now_ += std::min(time_to_push_ - now_, time_to_pull_ - now_);
        }
    }
...
{
    expect_smooth_output_ = false;
    SetupInput(kInputPacketSize + ::testing::get<0>(GetParam()) * 10 - 10, base::Microseconds(1000 + ::testing::get<1>(GetParam()) * 100 - 100));
    SetupOutput(kOutputPacketSize, base::Microseconds(500 + ::testing::get<2>(GetParam()) * 50 - 50));
...
INSTANTIATE_TEST_SUITE_P(All, AudioShifterTest, ::testing::Combine(::testing::Range(0, 3), ::testing::Range(0, 3), ::testing::Range(0, 3), ::testing::Bool()));
