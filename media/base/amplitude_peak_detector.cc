    ...
    : peak_detected_cb_(std::move(peak_detected_cb))
{
    ...
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("audio.latency"), &is_tracing_enabled_);
}

AmplitudePeakDetector::~AmplitudePeakDetector() = default;

void AmplitudePeakDetector::SetIsTracingEnabledForTests(bool is_tracing_enabled)
{
    is_tracing_enabled_ = is_tracing_enabled;
}

void AmplitudePeakDetector::FindPeak(const void* data, int frames, int bytes_per_sample)
{
...
template <class T> bool IsDataLoud(const T* audio_data, int frames, const T min_loudness, const T max_loudness)
{
    int n = 0;
    do {
...
template <class T> bool LoudDetector(const void* data, int frames)
{
    const T* audio_data = reinterpret_cast<const T*>(data);

    constexpr T min_loudness = FixedSampleTypeTraits<T>::FromFloat(-kLoudnessThreshold);
    constexpr T max_loudness = FixedSampleTypeTraits<T>::FromFloat(kLoudnessThreshold);

    return IsDataLoud<T>(audio_data, frames, min_loudness, max_loudness);
}

template <> bool LoudDetector<float>(const void* data, int frames)
{
    return IsDataLoud<float>(reinterpret_cast<const float*>(data), frames, -kLoudnessThreshold, kLoudnessThreshold);
}
...
bool AmplitudePeakDetector::AreFramesLoud(const void* data, int frames, int bytes_per_sample)
{
