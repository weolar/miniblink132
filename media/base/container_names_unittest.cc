...
// failed.
#define VERIFY(buffer, name) EXPECT_EQ(name, DetermineContainer(reinterpret_cast<const uint8_t*>(buffer), sizeof(buffer)))
...
uint8_t kAsfBuffer[] = { 0x30, 0x26, 0xb2, 0x75, 0x8e, 0x66, 0xcf, 0x11, 0xa6, 0xd9, 0x00, 0xaa, 0x00, 0x62, 0xce, 0x6c };
const char kAss1Buffer[] = "[Script Info]";
const char kAss2Buffer[] = BYTE_ORDER_MARK "[Script Info]";
uint8_t kCafBuffer[] = { 'c', 'a', 'f', 'f', 0, 1, 0, 0, 'd', 'e', 's', 'c', 0, 0, 0, 0, 0, 0, 0, 32, 64, 229, 136, 128, 0, 0, 0, 0, 'a', 'a', 'c', ' ', 0, 0,
    0, 2, 0, 0, 0, 0, 0, 0, 4, 0, 0, 0, 0, 2, 0, 0, 0, 0 };
...
uint8_t kWtvBuffer[] = { 0xb7, 0xd8, 0x00, 0x20, 0x37, 0x49, 0xda, 0x11, 0xa6, 0x4e, 0x00, 0x07, 0xe9, 0x5e, 0xad, 0x8d };
uint8_t kBug263073Buffer[] = { 0x00, 0x00, 0x00, 0x18, 0x66, 0x74, 0x79, 0x70, 0x6d, 0x70, 0x34, 0x32, 0x00, 0x00, 0x00, 0x00, 0x69, 0x73, 0x6f, 0x6d, 0x6d,
    0x70, 0x34, 0x32, 0x00, 0x00, 0x00, 0x01, 0x6d, 0x64, 0x61, 0x74, 0x00, 0x00, 0x00, 0x00, 0xaa, 0x2e, 0x22, 0xcf, 0x00, 0x00, 0x00, 0x37, 0x67, 0x64, 0x00,
    0x28, 0xac, 0x2c, 0xa4, 0x01, 0xe0, 0x08, 0x9f, 0x97, 0x01, 0x52, 0x02, 0x02, 0x02, 0x80, 0x00, 0x01 };
uint8_t kBug584401Buffer[] = {  };
uint8_t kBug585243Buffer[] = { 0x1a, 0x45, 0xdf, 0xa3, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x42, 0x82, 0x42, 0x82, 0x00, 0x00 };
...
void TestFile(MediaContainerName expected, const base::FilePath& filename)
{
    ...
    // Now verify the type.
    EXPECT_EQ(expected, DetermineContainer(reinterpret_cast<const uint8_t*>(buffer), read)) << "Failure with file " << filename.value();
}
...
TEST(ContainerNamesTest, FileCheckWAV)
{
    ...
    TestFile(MediaContainerName::kContainerWAV, GetTestDataFilePath("sfx_f32le.wav"));
    TestFile(MediaContainerName::kContainerWAV, GetTestDataFilePath("sfx_s16le.wav"));
}

TEST(ContainerNamesTest, FileCheckMOV)
{
    TestFile(MediaContainerName::kContainerMOV, GetTestDataFilePath("bear_rotate_90.mp4"));
    TestFile(MediaContainerName::kContainerMOV, GetTestDataFilePath("bear-1280x720.mp4"));
    TestFile(MediaContainerName::kContainerMOV, GetTestDataFilePath("crbug657437.mp4"));
    TestFile(MediaContainerName::kContainerMOV, GetTestDataFilePath("sfx.m4a"));
}

TEST(ContainerNamesTest, FileCheckWEBM)
{
    TestFile(MediaContainerName::kContainerWEBM, GetTestDataFilePath("bear-320x240.webm"));
    TestFile(MediaContainerName::kContainerWEBM, GetTestDataFilePath("no_streams.webm"));
    TestFile(MediaContainerName::kContainerWEBM, GetTestDataFilePath("webm_ebml_element"));
}

TEST(ContainerNamesTest, FileCheckMP3)
{
    TestFile(MediaContainerName::kContainerMP3, GetTestDataFilePath("bear-audio-10s-CBR-no-TOC.mp3"));
    TestFile(MediaContainerName::kContainerMP3, GetTestDataFilePath("id3_png_test.mp3"));
    TestFile(MediaContainerName::kContainerMP3, GetTestDataFilePath("id3_test.mp3"));
    TestFile(MediaContainerName::kContainerMP3, GetTestDataFilePath("midstream_config_change.mp3"));
...
{
    TestFile(MediaContainerName::kContainerAIFF, GetTestDataFilePath("bear.aiff"));
}
...
{
    TestFile(MediaContainerName::kContainerEAC3, GetTestDataFilePath("bear.eac3"));
}

TEST(ContainerNamesTest, FileCheckFLAC)
{
    TestFile(MediaContainerName::kContainerFLAC, GetTestDataFilePath("bear.flac"));
}
...
{
    TestFile(MediaContainerName::kContainerH261, GetTestDataFilePath("bear.h261"));
}

TEST(ContainerNamesTest, FileCheckH263)
{
    TestFile(MediaContainerName::kContainerH263, GetTestDataFilePath("bear.h263"));
}

TEST(ContainerNamesTest, FileCheckMJPEG)
{
    TestFile(MediaContainerName::kContainerMJPEG, GetTestDataFilePath("bear.mjpeg"));
}

TEST(ContainerNamesTest, FileCheckMPEG2PS)
{
    TestFile(MediaContainerName::kContainerMPEG2PS, GetTestDataFilePath("bear.mpeg"));
}

TEST(ContainerNamesTest, FileCheckMPEG2TS)
{
    TestFile(MediaContainerName::kContainerMPEG2TS, GetTestDataFilePath("bear.m2ts"));
}
...
{
    TestFile(MediaContainerName::kContainerUnknown, GetTestDataFilePath("ten_byte_file"));
    TestFile(MediaContainerName::kContainerUnknown, GetTestDataFilePath("README"));
    TestFile(MediaContainerName::kContainerUnknown, GetTestDataFilePath("webm_vp8_track_entry"));
}
