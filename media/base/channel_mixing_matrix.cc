...
static void ValidateLayout(ChannelLayout layout)
{
    ...
    if (channel_count > 1) {
        ...
        DCHECK_EQ(ChannelOrder(layout, LEFT) >= 0, ChannelOrder(layout, RIGHT) >= 0);
        DCHECK_EQ(ChannelOrder(layout, SIDE_LEFT) >= 0, ChannelOrder(layout, SIDE_RIGHT) >= 0);
        DCHECK_EQ(ChannelOrder(layout, BACK_LEFT) >= 0, ChannelOrder(layout, BACK_RIGHT) >= 0);
        DCHECK_EQ(ChannelOrder(layout, LEFT_OF_CENTER) >= 0, ChannelOrder(layout, RIGHT_OF_CENTER) >= 0);
    } else {
        DCHECK_EQ(layout, CHANNEL_LAYOUT_MONO);
    }
}

ChannelMixingMatrix::ChannelMixingMatrix(ChannelLayout input_layout, int input_channels, ChannelLayout output_layout, int output_channels)
...
bool ChannelMixingMatrix::CreateTransformationMatrix(std::vector<std::vector<float>>* matrix)
{
    matrix_ = matrix;
    ...
    for (Channels ch = LEFT; ch < CHANNELS_MAX + 1; ch = static_cast<Channels>(ch + 1)) {
        ...
        const bool force_upmix_center_into_lr_channel = ch == CENTER && IsMonoInputLayout() && HasOutputChannel(LEFT);
        ...
        DCHECK_LT(static_cast<size_t>(input_ch_index), (*matrix_)[output_ch_index].size());
    ...
    if (IsUnaccounted(LEFT)) {
        ...
        float scale = IsMonoOutputLayout() && input_layout_ == CHANNEL_LAYOUT_STEREO ? 0.5 : ChannelMixer::kHalfPower;
...
{
    return input_layout_ == CHANNEL_LAYOUT_MONO || input_layout_ == CHANNEL_LAYOUT_1_1;
}

bool ChannelMixingMatrix::IsMonoOutputLayout() const
{
    return output_layout_ == CHANNEL_LAYOUT_MONO || output_layout_ == CHANNEL_LAYOUT_1_1;
}
...
void ChannelMixingMatrix::Mix(Channels input_ch, Channels output_ch, float scale)
{
    MixWithoutAccounting(input_ch, output_ch, scale);
    AccountFor(input_ch);
}

void ChannelMixingMatrix::MixWithoutAccounting(Channels input_ch, Channels output_ch, float scale)
{
