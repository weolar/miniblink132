...
bool AudioBusAreEqual(AudioBus* a, AudioBus* b)
{
    ...
    for (int ch = 0; ch < kChannels; ++ch) {
        if (base::span(a->channel(ch), static_cast<size_t>(a->frames())) != base::span(b->channel(ch), static_cast<size_t>(b->frames()))) {
            return false;
        }
    }
...
public:
    ...
    void FillWithSine(AudioBus* bus, float scale = 1.0f)
    {
        ...
        current_timestamp_ += AudioTimestampHelper::FramesToTime(kBufferSize, kSampleRate);
...
protected:
    AudioLimiter::OutputChannels AudioBusAsOutputs(AudioBus* audio_bus)
    {
        ...
        for (int ch = 0; ch < audio_bus->channels(); ++ch) {
            channels.emplace_back(reinterpret_cast<uint8_t*>(audio_bus->channel(ch)), audio_bus->frames() * sizeof(float));
        }
...
TEST_F(LimiterTest, NoLimiting_IsPassthrough)
{
    ...
    limiter_->LimitPeaks(*source_bus_, AudioBusAsOutputs(destination_bus_.get()), base::BindLambdaForTesting([&]() { callback_signaled = true; }));
...
TEST_F(LimiterTest, NoLimiting_PartialBuffer_IsPassthrough)
{
    ...
    limiter_->LimitPeaksPartial(*source_bus_, kPartialSize, AudioBusAsOutputs(dest_bus.get()), base::BindLambdaForTesting([&]() { callback_signaled = true; }));
...
{
    const int longer_frame_size = AudioTimestampHelper::TimeToFrames(base::Milliseconds(500), kSampleRate);
...
TEST_F(LimiterTest, MultipleCalls)
{
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        auto verify_outputs = base::BindOnce(
            [](AudioBusVector* inputs, AudioBusVector* outputs, int* total_output_calls, int iteration) {
                EXPECT_TRUE(AudioBusAreEqual(inputs->at(iteration).get(), outputs->at(iteration).get()));
                ++(*total_output_calls);
            },
            &inputs, &outputs, &total_output_calls, i);

        limiter_->LimitPeaks(*inputs.back(), AudioBusAsOutputs(outputs.back().get()), std::move(verify_outputs));
    }
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        auto verify_outputs = base::BindOnce(
            [](AudioBusVector* inputs, AudioBusVector* outputs, int* total_output_calls, int iteration) {
                ...
                auto resized_input = AudioBus::Create(kChannels, kPartialSize);
                inputs->at(iteration)->CopyPartialFramesTo(0, kPartialSize, 0, resized_input.get());
                EXPECT_TRUE(AudioBusAreEqual(resized_input.get(), outputs->at(iteration).get()));
                ++(*total_output_calls);
            },
            &inputs, &outputs, &total_output_calls, i);

        limiter_->LimitPeaksPartial(*inputs.back(), kPartialSize, AudioBusAsOutputs(outputs.back().get()), std::move(verify_outputs));
    }
...
{
    const int kMaxIterations = base::Milliseconds(600) / AudioTimestampHelper::FramesToTime(kBufferSize, kSampleRate);
    ...
    for (int i = 0; i < kMaxIterations; ++i) {
        ...
        // Each time an output is filled, make sure it matches the input.
        auto verify_outputs = base::BindOnce(
            [](AudioBusVector* inputs, AudioBusVector* outputs, bool* returned_to_passthrough, int iteration) {
                if (iteration == 0) {
                    EXPECT_FALSE(AudioBusAreEqual(inputs->at(iteration).get(), outputs->at(iteration).get()));
                } else {
                    bool was_passthrough = AudioBusAreEqual(inputs->at(iteration).get(), outputs->at(iteration).get());
...
TEST_F(LimiterTest, OutputsFilledSequentially)
{
    ...
    limiter_->LimitPeaks(*source_bus_, AudioBusAsOutputs(destination_bus_.get()), base::BindLambdaForTesting([&]() {
        ...
        EXPECT_EQ(kGuardSampleValue, other_destination->channel(0)[0]);
    ...
    limiter_->LimitPeaks(*source_bus_, AudioBusAsOutputs(other_destination.get()), base::DoNothing());
...
{
    const int kTinyBufferSize = AudioTimestampHelper::TimeToFrames(base::Milliseconds(1), kSampleRate);
    ...
    for (auto& input : inputs) {
        ...
        limiter_->LimitPeaks(*input, AudioBusAsOutputs(outputs.back().get()), base::BindLambdaForTesting([&]() { ++total_output_calls; }));
    }
...
{
    const std::vector<float> special_numbers = { std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::signaling_NaN(),
    ...
    for (float special_number : special_numbers) {
        ...
        limiter_->LimitPeaks(*source_bus_, AudioBusAsOutputs(destination_bus_.get()), base::DoNothing());
        limiter_->Flush();
