...
// static
MediaTrack MediaTrack::CreateVideoTrack(
    const std::string& id, VideoKind kind, const std::string& label, const std::string& language, bool enabled, StreamParser::TrackId stream_id)
{
    return MediaTrack { MediaTrack::Type::kVideo, stream_id, MediaTrack::Id { id }, VideoKindToString(kind), Label { label }, Language { language }, enabled,
        true };
}

// static
MediaTrack MediaTrack::CreateAudioTrack(
    const std::string& id, AudioKind kind, const std::string& label, const std::string& language, bool enabled, StreamParser::TrackId stream_id, bool exclusive)
{
    return MediaTrack { MediaTrack::Type::kAudio, stream_id, MediaTrack::Id { id }, AudioKindToString(kind), Label { label }, Language { language }, enabled,
        exclusive };
}

MediaTrack::MediaTrack(MediaTrack::Type type, StreamParser::TrackId stream_id, const MediaTrack::Id& track_id, const MediaTrack::Kind& kind,
    const MediaTrack::Label& label, const MediaTrack::Language& language, bool enabled, bool exclusive)
    : type_(type)
    , enabled_(enabled)
