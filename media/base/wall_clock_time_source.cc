...
bool WallClockTimeSource::GetWallClockTimes(const std::vector<base::TimeDelta>& media_timestamps, std::vector<base::TimeTicks>* wall_clock_times)
{
    base::AutoLock auto_lock(lock_);
...
base::TimeDelta WallClockTimeSource::CurrentMediaTime_Locked()
{
    ...
    return base_timestamp_ + base::Microseconds((now - reference_time_).InMicroseconds() * playback_rate_);
}
