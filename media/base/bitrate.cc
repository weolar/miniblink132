...
{
    return (this->mode_ == right.mode_) && (this->target_bps_ == right.target_bps_) && (this->peak_bps_ == right.peak_bps_);
}
...
std::string Bitrate::ToString() const
{
    ...
    case Mode::kVariable:
        return base::StringPrintf("VBR: target %d bps, peak %d bps", target_bps_, peak_bps_);
    case Mode::kExternal:
