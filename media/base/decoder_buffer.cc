...
template <class T> class ExternalSharedMemoryAdapter : public DecoderBuffer::ExternalMemory {
public:
...
DecoderBuffer::DecoderBuffer(DecoderBufferType decoder_buffer_type, std::optional<ConfigVariant> next_config)
    : is_end_of_stream_(decoder_buffer_type == DecoderBufferType::kEndOfStream)
{
    if (next_config) {
...
// static
scoped_refptr<DecoderBuffer> DecoderBuffer::CopyFrom(base::span<const uint8_t> data)
{
    return base::WrapRefCounted(new DecoderBuffer(data));
}

// static
scoped_refptr<DecoderBuffer> DecoderBuffer::FromArray(base::HeapArray<uint8_t> data)
{
    return base::WrapRefCounted(new DecoderBuffer(std::move(data)));
}

// static
scoped_refptr<DecoderBuffer> DecoderBuffer::FromSharedMemoryRegion(base::UnsafeSharedMemoryRegion region, uint64_t offset, size_t size)
{
    if (size == 0) {
        return nullptr;
    }
    ...
    return FromExternalMemory(std::make_unique<ExternalSharedMemoryAdapter<base::WritableSharedMemoryMapping>>(std::move(mapping)));
}

// static
scoped_refptr<DecoderBuffer> DecoderBuffer::FromSharedMemoryRegion(base::ReadOnlySharedMemoryRegion region, uint64_t offset, size_t size)
{
    if (size == 0) {
        return nullptr;
    }
    ...
    return FromExternalMemory(std::make_unique<ExternalSharedMemoryAdapter<base::ReadOnlySharedMemoryMapping>>(std::move(mapping)));
}

// static
scoped_refptr<DecoderBuffer> DecoderBuffer::FromExternalMemory(std::unique_ptr<ExternalMemory> external_memory)
{
...
// static
scoped_refptr<DecoderBuffer> DecoderBuffer::CreateEOSBuffer(std::optional<ConfigVariant> next_config)
{
    return base::WrapRefCounted(new DecoderBuffer(DecoderBufferType::kEndOfStream, std::move(next_config)));
}
...
void DecoderBuffer::set_side_data(std::unique_ptr<DecoderBufferSideData> side_data)
{
    DCHECK(!end_of_stream());
    side_data_ = std::move(side_data);
}

bool DecoderBuffer::MatchesMetadataForTesting(const DecoderBuffer& buffer) const
{
    ...
    if (timestamp() != buffer.timestamp() || duration() != buffer.duration() || is_key_frame() != buffer.is_key_frame()) {
        return false;
    }
    ...
    return decrypt_config() ? decrypt_config()->Matches(*buffer.decrypt_config()) : true;
}
...
std::string DecoderBuffer::AsHumanReadableString(bool verbose) const
{
    ...
    s << "{timestamp=" << timestamp_.InMicroseconds() << " duration=" << duration_.InMicroseconds() << " size=" << size() << " is_key_frame=" << is_key_frame_
      << " encrypted=" << (decrypt_config_ != nullptr);

    if (verbose) {
        s << " has_side_data=" << has_side_data();
        if (has_side_data()) {
            s << " discard_padding (us)=(" << side_data_->discard_padding.first.InMicroseconds() << ", " << side_data_->discard_padding.second.InMicroseconds()
              << ")";
        }
        if (decrypt_config_) {
...
size_t DecoderBuffer::GetMemoryUsage() const
{
    ...
    if (has_side_data()) {
        memory_usage += sizeof(decltype(side_data_->spatial_layers)::value_type) * side_data_->spatial_layers.capacity();
    ...
    if (decrypt_config_) {
        ...
        memory_usage += sizeof(SubsampleEntry) * decrypt_config_->subsamples().capacity();
    }
