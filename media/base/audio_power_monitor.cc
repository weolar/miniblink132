...
AudioPowerMonitor::AudioPowerMonitor(int sample_rate, base::TimeDelta time_constant)
    : sample_weight_(1.0f - expf(-1.0f / (sample_rate * time_constant.InSecondsF())))
{
    Reset();
}
...
void AudioPowerMonitor::Scan(const AudioBus& buffer, int num_frames)
{
    ...
    for (int i = 0; i < num_channels; ++i) {
        const std::pair<float, float> ewma_and_max = vector_math::EWMAAndMaxPower(average_power_, buffer.channel(i), num_frames, sample_weight_);
