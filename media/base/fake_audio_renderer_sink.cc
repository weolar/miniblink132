...
FakeAudioRendererSink::FakeAudioRendererSink()
    : FakeAudioRendererSink(AudioParameters(AudioParameters::AUDIO_FAKE, ChannelLayoutConfig::Stereo(), AudioParameters::kTelephoneSampleRate, 1))
{
}

FakeAudioRendererSink::FakeAudioRendererSink(const AudioParameters& hardware_params)
    : state_(kUninitialized)
    , callback_(nullptr)
    , output_device_info_(std::string(), OUTPUT_DEVICE_STATUS_OK, hardware_params)
...
void FakeAudioRendererSink::Initialize(const AudioParameters& params, RenderCallback* callback)
{
...
void FakeAudioRendererSink::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), output_device_info_));
}
...
bool FakeAudioRendererSink::Render(AudioBus* dest, base::TimeDelta delay, int* frames_written)
{
    if (state_ != kPlaying)
        return false;
...
{
    static const char* kStateNames[] = { "kUninitialized", "kInitialized", "kStarted", "kPaused", "kPlaying", "kStopped" };

    DVLOG(1) << __func__ << " : " << kStateNames[state_] << " -> " << kStateNames[new_state];
    state_ = new_state;
}
