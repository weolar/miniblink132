...
public:
    ...
    static constexpr size_t kFrameAddressAlignment = VideoFrameLayout::kBufferAddressAlignment;
    ...
    using ReleaseMailboxAndGpuMemoryBufferCB = base::OnceCallback<void(const gpu::SyncToken&, std::unique_ptr<gfx::GpuMemoryBuffer>)>;
    ...
    private:
        ...
        ScopedMapping(gfx::GpuMemoryBuffer* gpu_memory_buffer, std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> scoped_mapping);
    ...
    static bool IsValidSize(const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size);
    ...
    static std::optional<VideoFrameLayout> CreateFullySpecifiedLayoutWithStrides(VideoPixelFormat format, const gfx::Size& coded_size);
    ...
    static scoped_refptr<VideoFrame> WrapTrackingToken(VideoPixelFormat format, const base::UnguessableToken& tracking_token, const gfx::Size& coded_size,
    ...
    static scoped_refptr<VideoFrame> WrapSharedImage(VideoPixelFormat format, scoped_refptr<gpu::ClientSharedImage> shared_image, gpu::SyncToken sync_token,
    ...
    static scoped_refptr<VideoFrame> WrapMappableSharedImage(scoped_refptr<gpu::ClientSharedImage> shared_image, gpu::SyncToken sync_token,
    ...
    static scoped_refptr<VideoFrame> WrapExternalData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
        const gfx::Size& natural_size, const uint8_t* data, size_t data_size, base::TimeDelta timestamp);

    static scoped_refptr<VideoFrame> WrapExternalDataWithLayout(const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    ...
    static scoped_refptr<VideoFrame> WrapExternalYuvData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
        const gfx::Size& natural_size, int32_t y_stride, int32_t u_stride, int32_t v_stride, const uint8_t* y_data, const uint8_t* u_data,
        const uint8_t* v_data, base::TimeDelta timestamp);

    // Wraps external YUV data with VideoFrameLayout. The returned VideoFrame does
    // not own the data passed in.
    static scoped_refptr<VideoFrame> WrapExternalYuvDataWithLayout(const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
        const uint8_t* y_data, const uint8_t* u_data, const uint8_t* v_data, base::TimeDelta timestamp);

    // Wraps external YUVA data of the given parameters with a VideoFrame.
    // The returned VideoFrame does not own the data passed in.
    static scoped_refptr<VideoFrame> WrapExternalYuvaData(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    ...
    static scoped_refptr<VideoFrame> WrapExternalGpuMemoryBuffer(const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    ...
    static scoped_refptr<VideoFrame> WrapExternalDmabufs(const VideoFrameLayout& layout, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    ...
    static scoped_refptr<VideoFrame> WrapCVPixelBuffer(CVPixelBufferRef cv_pixel_buffer, base::TimeDelta timestamp);
    ...
    static scoped_refptr<VideoFrame> WrapUnacceleratedIOSurface(gfx::GpuMemoryBufferHandle handle, const gfx::Rect& visible_rect, base::TimeDelta timestamp);
#endif
    ...
    static scoped_refptr<VideoFrame> CreateColorFrame(const gfx::Size& size, uint8_t y, uint8_t u, uint8_t v, base::TimeDelta timestamp);
    ...
    static scoped_refptr<VideoFrame> CreateTransparentFrame(const gfx::Size& size);
    ...
    static size_t AllocationSize(VideoPixelFormat format, const gfx::Size& coded_size);

    // Returns |dimensions| adjusted to appropriate boundaries based on |format|.
    static gfx::Size DetermineAlignedSize(VideoPixelFormat format, const gfx::Size& dimensions);

    // Returns the plane gfx::Size (in bytes) for a plane of the given coded size
    // and format.
    static gfx::Size PlaneSize(VideoPixelFormat format, size_t plane, const gfx::Size& coded_size);

    // Returns the plane gfx::Size (in samples) for a plane of the given coded
    // size and format.
    static gfx::Size PlaneSizeInSamples(VideoPixelFormat format, size_t plane, const gfx::Size& coded_size);
    ...
    static std::vector<int32_t> ComputeStrides(VideoPixelFormat format, const gfx::Size& coded_size);
    ...
    static void HashFrameForTesting(base::MD5Context* context, const VideoFrame& frame);
    ...
    void BackWithOwnedSharedMemory(base::ReadOnlySharedMemoryRegion region, base::ReadOnlySharedMemoryMapping mapping);
    ...
    // then directly return ClientSharedImage::ScopedMapping object instead.
    void MapGMBOrSharedImageAsync(base::OnceCallback<void(std::unique_ptr<VideoFrame::ScopedMapping>)> result_cb) const;
    ...
    bool IsSameAllocation(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size) const;
    ...
    void SetReleaseMailboxAndGpuMemoryBufferCB(ReleaseMailboxAndGpuMemoryBufferCB release_mailbox_cb);
    ...
    void clear_metadata()
    {
        set_metadata(VideoFrameMetadata());
    }
...
protected:
    ...
    VideoFrame(const VideoFrameLayout& layout, StorageType storage_type, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
        base::TimeDelta timestamp, FrameControlType frame_control_type = FrameControlType::kNone);
    virtual ~VideoFrame();
...
private:
    ...
    static bool IsValidConfigInternal(VideoPixelFormat format, FrameControlType frame_control_type, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
        const gfx::Size& natural_size);

    static scoped_refptr<VideoFrame> CreateFrameInternal(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
    ...
    static scoped_refptr<VideoFrame> CreateFrameForGpuMemoryBufferOrMappableSIInternal(const gfx::Rect& visible_rect, const gfx::Size& natural_size,
    ...
    void MakeScopedMappingForGpuMemoryBuffer(base::OnceCallback<void(std::unique_ptr<VideoFrame::ScopedMapping>)> result_cb, bool success) const;
    ...
    template <typename T> T GetVisibleDataInternal(T data, size_t plane) const;
    ...
    std::vector<base::OnceClosure> done_callbacks_ GUARDED_BY(done_callbacks_lock_);
