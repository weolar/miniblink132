...
struct MEDIA_SHMEM_EXPORT alignas(kParametersAlignment) AudioInputBufferParameters {
    double volume;
...
struct MEDIA_SHMEM_EXPORT alignas(kParametersAlignment) AudioOutputBufferParameters {
...
MEDIA_SHMEM_EXPORT base::CheckedNumeric<uint32_t> ComputeAudioInputBufferSizeChecked(const AudioParameters& parameters, uint32_t audio_bus_count);

MEDIA_SHMEM_EXPORT uint32_t ComputeAudioInputBufferSize(const AudioParameters& parameters, uint32_t audio_bus_count);

MEDIA_SHMEM_EXPORT uint32_t ComputeAudioInputBufferSize(int channels, int frames, uint32_t audio_bus_count);
...
MEDIA_SHMEM_EXPORT base::CheckedNumeric<uint32_t> ComputeAudioOutputBufferSizeChecked(const AudioParameters& parameters);

MEDIA_SHMEM_EXPORT uint32_t ComputeAudioOutputBufferSize(const AudioParameters& parameters);

MEDIA_SHMEM_EXPORT uint32_t ComputeAudioOutputBufferSize(int channels, int frames);
...
public:
    ...
    template <ChannelLayout layout> static ChannelLayoutConfig FromLayout()
    {
...
template <> ChannelLayoutConfig ChannelLayoutConfig::FromLayout<CHANNEL_LAYOUT_DISCRETE>() = delete;
...
public:
    ...
    enum Format {
        ...
        AUDIO_BITSTREAM_DTS_HD_MA = 0x100, // Compressed DTS-HD Master Audio bitstream.
        AUDIO_FORMAT_LAST = AUDIO_BITSTREAM_DTS_HD_MA, // Only used for validation of format.
    };
    ...
    enum PlatformEffectsMask {
        ...
        // supported.
        CLIENT_CONTROLLED_VOICE_ISOLATION = 1 << 19, // Set when client forces to
    ...
    struct HardwareCapabilities {
        HardwareCapabilities(int min_frames_per_buffer, int max_frames_per_buffer, int default_frames_per_buffer, bool require_offload)
    ...
    AudioParameters(Format format, ChannelLayoutConfig channel_layout_config, int sample_rate, int frames_per_buffer);
    ...
    void Reset(Format format, ChannelLayoutConfig channel_layout_config, int sample_rate, int frames_per_buffer);
    ...
    int channels() const
    {
        return channel_layout_config_.channels();
    }
    ...
    void set_hardware_capabilities(const std::optional<HardwareCapabilities>& hwc)
    {
    ...
    const std::vector<Point>& mic_positions() const
    {
        return mic_positions_;
    }
    ...
    AudioLatency::Type latency_tag() const
    {
        return latency_tag_;
    }
