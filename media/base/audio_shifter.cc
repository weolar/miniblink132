...
public:
    ...
    base::TimeTicks Smooth(base::TimeTicks t, base::TimeDelta delta)
    {
        if (previous_.is_null()) {
            ...
            previous_ = (-clock_accuracy_ < diff && diff < clock_accuracy_) ? (t + diff / 1000) : t;
        }
        return previous_;
    }
    ...
    double Rate() const
    {
        return 1.0 + inaccuracy_sum_ / inaccuracy_delta_;
    }

private:
    base::TimeDelta clock_accuracy_;
    base::circular_deque<std::pair<base::TimeDelta, base::TimeDelta>> inaccuracies_;
    base::TimeDelta inaccuracy_sum_;
    base::TimeDelta inaccuracy_delta_;
    base::TimeTicks previous_;
};

AudioShifter::AudioQueueEntry::AudioQueueEntry(base::TimeTicks target_playout_time, std::unique_ptr<AudioBus> audio)
    : target_playout_time(target_playout_time)
    , audio(std::move(audio))
{
}

AudioShifter::AudioQueueEntry::AudioQueueEntry(AudioQueueEntry&& other) = default;

AudioShifter::AudioQueueEntry::~AudioQueueEntry() = default;

AudioShifter::AudioShifter(base::TimeDelta max_buffer_size, base::TimeDelta clock_accuracy, base::TimeDelta adjustment_time, int rate, int channels)
    ...
    , running_(false)
    , resampler_(channels, 1.0, 128, base::BindRepeating(&AudioShifter::ResamplerCallback, base::Unretained(this)))
{
}

AudioShifter::~AudioShifter() = default;

void AudioShifter::Push(std::unique_ptr<AudioBus> input, base::TimeTicks playout_time)
{
    TRACE_EVENT1("audio", "AudioShifter::Push", "time (ms)", (playout_time - base::TimeTicks()).InMillisecondsF());
    DCHECK_EQ(input->channels(), channels_);
    frames_pushed_for_testing_ += input->frames();
    if (!queue_.empty()) {
        playout_time = input_clock_smoother_->Smooth(playout_time, base::Seconds(queue_.back().audio->frames() / rate_));
    }
...
void AudioShifter::Pull(AudioBus* output, base::TimeTicks playout_time)
{
    TRACE_EVENT1("audio", "AudioShifter::Pull", "time (ms)", (playout_time - base::TimeTicks()).InMillisecondsF());
    ...
    playout_time = output_clock_smoother_->Smooth(playout_time, base::Seconds(previous_requested_samples_ / rate_));
    ...
    stream_time += base::Seconds((position_ - resampler_.BufferedFrames()) / rate_);

    if (!running_ && base::Seconds(output->frames() * 2 / rate_) + clock_accuracy_ > buffer_end_time - stream_time) {
        // We're not running right now, and we don't really have enough data
        // to satisfy output reliably. Wait.
        Zero(output);
        return;
    }
    if (playout_time < stream_time - base::Seconds(output->frames() / rate_ / 2) - (running_ ? clock_accuracy_ : base::TimeDelta())) {
    ...
    if (buffer_end_time < playout_time) {
        ...
        if (bias_.is_zero()) {
            bias_ = playout_time - stream_time + clock_accuracy_ + base::Seconds(output->frames() / rate_);
        }
        stream_time += bias_;
    } else {
    ...
    running_ = true;
    const double steady_ratio = output_clock_smoother_->Rate() / input_clock_smoother_->Rate();
    ...
    const base::TimeDelta adjustment_time = base::Seconds(output->frames() / rate_);
...
void AudioShifter::ResamplerCallback(int frame_delay, AudioBus* destination)
{
    ...
    while (pos < destination->frames() && !queue_.empty()) {
        size_t to_copy = std::min<size_t>(queue_.front().audio->frames() - position_, destination->frames() - pos);
        CHECK_GT(to_copy, 0UL);
        queue_.front().audio->CopyPartialFramesTo(position_, to_copy, pos, destination);
        pos += to_copy;
        position_ += to_copy;
        if (position_ >= static_cast<size_t>(queue_.front().audio->frames())) {
            end_of_last_consumed_audiobus_ = queue_.front().target_playout_time + base::Seconds(queue_.front().audio->frames() / rate_);
