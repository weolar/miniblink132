...
static scoped_refptr<AudioBuffer> MakeTestBuffer(int sample_rate, ChannelLayout channel_layout, int channel_count, int frames)
{
    return MakeAudioBuffer<uint8_t>(kSampleFormatU8, channel_layout, channel_count, sample_rate, 0, 1, frames, base::Seconds(0));
}
...
public:
    AudioBufferConverterTest()
        ...
        , output_frames_(0)
        , output_params_(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kOutChannelLayout>(), kOutSampleRate, kOutFrameSize)
    {
        audio_buffer_converter_ = std::make_unique<AudioBufferConverter>(output_params_);
    }
...
{
    AddInput(MakeTestBuffer(48000, CHANNEL_LAYOUT_STEREO, 2, SincResampler::kMinKernelSize - 1));
    ConsumeAllOutput();
}
...
TEST_F(AudioBufferConverterTest, Resample_DifferingBufferSizes)
{
    ...
    AddInput(MakeTestBuffer(input_sample_rate, kOutChannelLayout, kOutChannelCount, 100));
    AddInput(MakeTestBuffer(input_sample_rate, kOutChannelLayout, kOutChannelCount, 200));
    AddInput(MakeTestBuffer(input_sample_rate, kOutChannelLayout, kOutChannelCount, 300));
    AddInput(MakeTestBuffer(input_sample_rate, kOutChannelLayout, kOutChannelCount, 400));
    AddInput(MakeTestBuffer(input_sample_rate, kOutChannelLayout, kOutChannelCount, 500));
    ConsumeAllOutput();
}
...
{
    AddInput(MakeTestBuffer(kOutSampleRate, kOutChannelLayout, kOutChannelCount, 512));
    Reset();
...
{
    output_params_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, { CHANNEL_LAYOUT_DISCRETE, 2 }, kOutSampleRate, 512);
    audio_buffer_converter_ = std::make_unique<AudioBufferConverter>(output_params_);
    AddInput(MakeTestBuffer(kOutSampleRate, CHANNEL_LAYOUT_STEREO, 2, 512));
    ConsumeAllOutput();
}
...
{
    output_params_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::FromLayout<kOutChannelLayout>(), kOutSampleRate, 2048);
    ...
    for (int i = 0; i < kInputBuffers; ++i) {
        AddInput(MakeTestBuffer(kInputSampleRate, kOutChannelLayout, kOutChannelCount, kInputFrameSize));
    }
    ...
    ASSERT_EQ(kInputFrameSize % audio_buffer_converter_->input_buffer_size_for_testing(), 0);
