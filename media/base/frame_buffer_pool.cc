...
class FrameBufferPool::FrameBufferMemoryDumpProviderImpl : public base::trace_event::MemoryDumpProvider {
public:
    explicit FrameBufferMemoryDumpProviderImpl(scoped_refptr<FrameBufferPool> pool)
        : pool_(std::move(pool))
    {
        DCHECK(pool_);
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "FrameBufferPool", nullptr);
    }
...
uint8_t* FrameBufferPool::GetFrameBuffer(size_t min_size, void** fb_priv)
{
    ...
    if (!memory_dump_impl_) {
        memory_dump_impl_ = std::make_unique<FrameBufferMemoryDumpProviderImpl>(this);
    }

    // Check if a free frame buffer exists.
    auto it = base::ranges::find_if_not(frame_buffers_, &IsUsedLocked, &std::unique_ptr<FrameBuffer>::get);
    ...
    if (frame_buffer->data_size < min_size) {
        ...
        if (!force_allocation_error_) {
            ...
            if (zero_initialize_memory_) {
                result = base::UncheckedCalloc(1u, min_size, reinterpret_cast<void**>(&data));
            } else {
                result = base::UncheckedMalloc(min_size, reinterpret_cast<void**>(&data));
            }
...
uint8_t* FrameBufferPool::AllocateAlphaPlaneForFrameBuffer(size_t min_size, void* fb_priv)
{
    base::AutoLock lock(lock_);
    DCHECK(fb_priv);
    ...
    if (frame_buffer->alpha_data_size < min_size) {
        ...
        uint8_t* data = nullptr;
        if (force_allocation_error_ || !base::UncheckedMalloc(min_size, reinterpret_cast<void**>(&data)) || !data) {
            return nullptr;
        }
...
base::OnceClosure FrameBufferPool::CreateFrameCallback(void* fb_priv)
{
    ...
    return base::BindOnce(&FrameBufferPool::OnVideoFrameDestroyed, this, frame_buffer);
}

bool FrameBufferPool::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    base::AutoLock lock(lock_);
    ...
    base::trace_event::MemoryAllocatorDump* memory_dump
        = pmd->CreateAllocatorDump(base::StringPrintf("media/frame_buffers/memory_pool/0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this)));
    base::trace_event::MemoryAllocatorDump* used_memory_dump
        = pmd->CreateAllocatorDump(base::StringPrintf("media/frame_buffers/memory_pool/used/0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this)));

    auto* pool_name = base::trace_event::MemoryDumpManager::GetInstance()->system_allocator_pool_name();
    if (pool_name) {
    ...
    memory_dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, bytes_reserved);
    used_memory_dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, bytes_used);
...
void FrameBufferPool::Shutdown()
{
    ...
    if (memory_dump_impl_) {
        base::trace_event::MemoryDumpManager::GetInstance()->UnregisterAndDeleteDumpProviderSoon(std::move(memory_dump_impl_));
    }
...
void FrameBufferPool::EraseUnusedResourcesLocked()
{
    ...
    std::erase_if(frame_buffers_, [](const std::unique_ptr<FrameBuffer>& buf) { return !IsUsedLocked(buf.get()); });
}
...
void FrameBufferPool::OnVideoFrameDestroyed(FrameBuffer* frame_buffer)
{
    ...
    std::erase_if(frame_buffers_,
        [now](const std::unique_ptr<FrameBuffer>& buf) { return !IsUsedLocked(buf.get()) && now - buf->last_use_time > base::Seconds(kStaleFrameLimitSecs); });
}
