...
public:
    ...
    // size for `pool_`.
    LimitingAudioQueue(ChannelLayout channel_layout, int sample_rate, int channels, int max_buffer_size);
    ~LimitingAudioQueue();
    ...
    //       accept more input.
    void Push(const AudioBus& input, int num_frames, base::TimeDelta timestamp, OutputCB output_cb);
