...
public:
    ...
    {
        return helper_.GetFramesToTarget(base::Microseconds(target_in_microseconds));
    }

    void TestGetFramesToTargetRange(int frame_count, int start, int end)
    {
        for (int i = start; i <= end; ++i) {
            EXPECT_EQ(frame_count, FramesToTarget(i)) << " Failure for timestamp " << i << " us.";
        }
    }
...
{
    // Negative value.
    EXPECT_EQ(base::Seconds(-1), AudioTimestampHelper::FramesToTime(-48000, k48kHz));
    // Zero.
    EXPECT_EQ(base::Microseconds(0), AudioTimestampHelper::FramesToTime(0, k48kHz));
    // One frame.
    EXPECT_EQ(base::Microseconds(20), AudioTimestampHelper::FramesToTime(1, k48kHz));
    // Exact value with maximum precision of TimeDelta.
    EXPECT_EQ(base::Microseconds(15625), AudioTimestampHelper::FramesToTime(750, k48kHz));
    // One second.
    EXPECT_EQ(base::Seconds(1), AudioTimestampHelper::FramesToTime(48000, k48kHz));
    // Argument and return value exceeding 32 bits.
    EXPECT_EQ(base::Seconds(1000000), AudioTimestampHelper::FramesToTime(48000000000, k48kHz));
}

TEST_F(AudioTimestampHelperTest, TimeToFrames)
{
    // Negative value.
    EXPECT_EQ(-48000, AudioTimestampHelper::TimeToFrames(base::Seconds(-1), k48kHz));
    // Zero.
    EXPECT_EQ(0, AudioTimestampHelper::TimeToFrames(base::Microseconds(0), k48kHz));
    // Duration of each frame is 20.833 microseconds. The result is rounded to
    // integral.
    EXPECT_EQ(0, AudioTimestampHelper::TimeToFrames(base::Microseconds(10), k48kHz));
    EXPECT_EQ(1, AudioTimestampHelper::TimeToFrames(base::Microseconds(20), k48kHz));
    EXPECT_EQ(1, AudioTimestampHelper::TimeToFrames(base::Microseconds(21), k48kHz));
    // Exact value with maximum precision of TimeDelta.
    EXPECT_EQ(750, AudioTimestampHelper::TimeToFrames(base::Microseconds(15625), k48kHz));
    // One second.
    EXPECT_EQ(48000, AudioTimestampHelper::TimeToFrames(base::Seconds(1), k48kHz));
    // Argument and return value exceeding 32 bits.
