...
{
    for (ChannelLayout input_layout = CHANNEL_LAYOUT_MONO; input_layout <= CHANNEL_LAYOUT_MAX; input_layout = static_cast<ChannelLayout>(input_layout + 1)) {
        for (ChannelLayout output_layout = CHANNEL_LAYOUT_MONO; output_layout <= CHANNEL_LAYOUT_MAX;
            ...
            if (input_layout == CHANNEL_LAYOUT_BITSTREAM || input_layout == CHANNEL_LAYOUT_DISCRETE || input_layout == CHANNEL_LAYOUT_STEREO_AND_KEYBOARD_MIC
                || output_layout == CHANNEL_LAYOUT_BITSTREAM || output_layout == CHANNEL_LAYOUT_DISCRETE
                || output_layout == CHANNEL_LAYOUT_STEREO_AND_KEYBOARD_MIC || output_layout == CHANNEL_LAYOUT_STEREO_DOWNMIX) {
                continue;
            }

            SCOPED_TRACE(base::StringPrintf("Input Layout: %d, Output Layout: %d", input_layout, output_layout));
            ChannelMixer mixer(input_layout, ChannelLayoutToChannelCount(input_layout), output_layout, ChannelLayoutToChannelCount(output_layout));
...
struct ChannelMixerTestData {
    ChannelMixerTestData(ChannelLayout input_layout, ChannelLayout output_layout, const float* channel_values, int num_channel_values, float scale)
    ...
    ChannelMixerTestData(
        ChannelLayout input_layout, int input_channels, ChannelLayout output_layout, int output_channels, const float* channel_values, int num_channel_values)
        : input_layout(input_layout)
    ...
    {
        return base::StringPrintf("Input Layout: %d, Output Layout %d, Scale: %f", input_layout, output_layout, scale);
    }
...
TEST_P(ChannelMixerTest, Mixing)
{
    ...
    AudioParameters input_audio(AudioParameters::AUDIO_PCM_LINEAR, { input_layout, input_channels }, AudioParameters::kAudioCDSampleRate, kFrames);
    ...
    AudioParameters output_audio(AudioParameters::AUDIO_PCM_LINEAR, { output_layout, output_channels }, AudioParameters::kAudioCDSampleRate, kFrames);
    ...
    for (int ch = 0; ch < input_bus->channels(); ++ch) {
        std::fill(input_bus->channel(ch), input_bus->channel(ch) + kFrames, channel_values[ch]);
...
INSTANTIATE_TEST_SUITE_P(ChannelMixerTest, ChannelMixerTest,
    testing::Values(ChannelMixerTestData(CHANNEL_LAYOUT_STEREO, CHANNEL_LAYOUT_MONO, kStereoToMonoValues, std::size(kStereoToMonoValues), 0.5f),
        ChannelMixerTestData(CHANNEL_LAYOUT_MONO, CHANNEL_LAYOUT_STEREO, kMonoToStereoValues, std::size(kMonoToStereoValues), 1.0f),
        ChannelMixerTestData(CHANNEL_LAYOUT_5_1, CHANNEL_LAYOUT_MONO, kFiveOneToMonoValues, std::size(kFiveOneToMonoValues), ChannelMixer::kHalfPower),
        ChannelMixerTestData(CHANNEL_LAYOUT_DISCRETE, 2, CHANNEL_LAYOUT_DISCRETE, 2, kStereoToMonoValues, std::size(kStereoToMonoValues)),
        ChannelMixerTestData(CHANNEL_LAYOUT_DISCRETE, 2, CHANNEL_LAYOUT_DISCRETE, 5, kStereoToMonoValues, std::size(kStereoToMonoValues)),
        ChannelMixerTestData(CHANNEL_LAYOUT_DISCRETE, 5, CHANNEL_LAYOUT_DISCRETE, 2, kFiveDiscreteValues, std::size(kFiveDiscreteValues))));
