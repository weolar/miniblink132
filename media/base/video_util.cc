...
void WaitAndReplaceSyncTokenClient::WaitSyncToken(const gpu::SyncToken&)
{
    *(int*)1 = 1;
}
...
void FillRegionOutsideVisibleRect(uint8_t* data, size_t stride, const gfx::Size& coded_size, const gfx::Size& visible_size)
{
    ...
    if (visible_size.height() < coded_size.height()) {
        ...
        for (int i = visible_size.height(); i < coded_size.height(); ++i, dst += stride)
...
bool ReadbackTexturePlaneToMemorySyncOOP(
    const VideoFrame& src_frame, size_t src_plane, gfx::Rect& src_rect, uint8_t* dest_pixels, size_t dest_stride, gpu::raster::RasterInterface* ri)
{
    VideoPixelFormat format = ReadbackFormat(src_frame);
    if (format == PIXEL_FORMAT_UNKNOWN) {
        DLOG(ERROR) << "Readback is not possible for this frame: " << src_frame.AsHumanReadableString();
        return false;
    }

    bool has_alpha = !IsOpaque(format);
    SkColorType sk_color_type = SkColorTypeForPlane(format, src_plane);
    SkAlphaType sk_alpha_type = has_alpha ? kUnpremul_SkAlphaType : kOpaque_SkAlphaType;

    auto info = SkImageInfo::Make(src_rect.width(), src_rect.height(), sk_color_type, sk_alpha_type);
    ...
    bool result = ri->ReadbackImagePixels(mailbox, info, dest_stride, src_rect.x(), src_rect.y(), src_plane, dest_pixels);

    return result && ri->GetGraphicsResetStatusKHR() == GL_NO_ERROR && ri->GetError() == GL_NO_ERROR;
}

void LetterboxPlane(const gfx::Rect& view_area_in_bytes, uint8_t* ptr, int rows, int row_bytes, int stride, int bytes_per_element, uint8_t fill_byte)
{
    ...
    if (view_area_in_bytes.width() < row_bytes) {
        if (view_area_in_bytes.x() > 0) {
            libyuv::SetPlane(ptr, stride, view_area_in_bytes.x(), view_area_in_bytes.height(), fill_byte);
        }
        if (view_area_in_bytes.right() < row_bytes) {
            libyuv::SetPlane(ptr + view_area_in_bytes.right(), stride, row_bytes - view_area_in_bytes.right(), view_area_in_bytes.height(), fill_byte);
        }
    }
    ...
    if (view_area_in_bytes.bottom() < rows) {
        libyuv::SetPlane(ptr, stride, row_bytes, rows - view_area_in_bytes.bottom(), fill_byte);
    }
}

void LetterboxPlane(VideoFrame* frame, int plane, uint8_t* ptr, const gfx::Rect& view_area_in_pixels, uint8_t fill_byte)
{
    const int rows = frame->rows(plane);
    const int row_bytes = frame->row_bytes(plane);
    const int stride = frame->stride(plane);
    const int bytes_per_element = VideoFrame::BytesPerElement(frame->format(), plane);

    gfx::Rect view_area_in_bytes(
        view_area_in_pixels.x() * bytes_per_element, view_area_in_pixels.y(), view_area_in_pixels.width() * bytes_per_element, view_area_in_pixels.height());
    ...
    LetterboxPlane(view_area_in_bytes, ptr, rows, row_bytes, stride, bytes_per_element, fill_byte);
}
...
void LetterboxPlane(VideoFrame* frame, VideoFrame::ScopedMapping* scoped_mapping, int plane, const gfx::Rect& view_area_in_pixels, uint8_t fill_byte)
{
    uint8_t* ptr = nullptr;
    if (frame->IsMappable()) {
...
void ProcessAsyncMappingResult(scoped_refptr<VideoFrame> video_frame, base::OnceCallback<void(scoped_refptr<VideoFrame>)> result_cb,
    std::unique_ptr<VideoFrame::ScopedMapping> scoped_mapping)
{
    ...
    auto mapped_frame = VideoFrame::WrapExternalYuvDataWithLayout(video_frame->layout(), video_frame->visible_rect(), video_frame->natural_size(),
        plane_addrs[0], plane_addrs[1], plane_addrs[2], video_frame->timestamp());
    ...
    mapped_frame->AddDestructionObserver(base::BindOnce(
        [](scoped_refptr<VideoFrame> frame, std::unique_ptr<VideoFrame::ScopedMapping> scoped_mapping) {
...
{
    libyuv::I420Rect(frame->writable_data(VideoFrame::Plane::kY), frame->stride(VideoFrame::Plane::kY), frame->writable_data(VideoFrame::Plane::kU),
        frame->stride(VideoFrame::Plane::kU), frame->writable_data(VideoFrame::Plane::kV), frame->stride(VideoFrame::Plane::kV), 0, 0,
        frame->coded_size().width(), frame->coded_size().height(), y, u, v);
}
...
void FillYUVA(VideoFrame* frame, uint8_t y, uint8_t u, uint8_t v, uint8_t a)
{
    ...
    // Fill the A plane.
    libyuv::SetPlane(frame->writable_data(VideoFrame::Plane::kA), frame->stride(VideoFrame::Plane::kA), frame->row_bytes(VideoFrame::Plane::kA),
...
{
    std::unique_ptr<VideoFrame::ScopedMapping> scoped_mapping;
    if (!frame->IsMappable() && frame->storage_type() == media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
    ...
    switch (frame->format()) {
    case PIXEL_FORMAT_ARGB:
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kARGB, view_area, 0x00);
        break;
    ...
    case PIXEL_FORMAT_I420: {
        ...
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kY, view_area, 0x00);
        gfx::Rect half_view_area(view_area.x() / 2, view_area.y() / 2, view_area.width() / 2, view_area.height() / 2);
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kU, half_view_area, 0x80);
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kV, half_view_area, 0x80);
        break;
    }
    case PIXEL_FORMAT_NV12: {
        DCHECK(!(view_area.x() & 1));
        ...
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kUV, half_view_area, 0x80);
        break;
    }
    case PIXEL_FORMAT_NV12A: {
        DCHECK(!(view_area.x() & 1));
        ...
        gfx::Rect half_view_area(view_area.x() / 2, view_area.y() / 2, view_area.width() / 2, view_area.height() / 2);
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kUV, half_view_area, 0x80);
        LetterboxPlane(frame, scoped_mapping.get(), VideoFrame::Plane::kATriPlanar, view_area, 0x00);
        break;
    }
    default:
        NOTREACHED();
...
void RotatePlaneByPixels(const uint8_t* src, uint8_t* dest, int width, int height,
...
static gfx::Size ScaleSizeToTarget(const gfx::Size& size, const gfx::Size& target, bool fit_within_target)
{
    if (size.IsEmpty())
...
gfx::Rect ComputeLetterboxRegion(const gfx::Rect& bounds, const gfx::Size& content)
{
...
gfx::Rect ComputeLetterboxRegionForI420(const gfx::Rect& bounds, const gfx::Size& content)
{
...
gfx::Rect MinimallyShrinkRectForI420(const gfx::Rect& rect)
{
    ...
    DCHECK(gfx::Rect(kMinDimension, kMinDimension, limits::kMaxDimension * 2, limits::kMaxDimension * 2).Contains(rect));
    ...
    return gfx::Rect(left, top, std::max(0, right - left), std::max(0, bottom - top));
}

gfx::Size ScaleSizeToFitWithinTarget(const gfx::Size& size, const gfx::Size& target)
{
    return ScaleSizeToTarget(size, target, true);
}

gfx::Size ScaleSizeToEncompassTarget(const gfx::Size& size, const gfx::Size& target)
{
    return ScaleSizeToTarget(size, target, false);
}

gfx::Rect CropSizeForScalingToTarget(const gfx::Size& size, const gfx::Size& target, size_t alignment)
{
    ...
    crop.set_width(base::checked_cast<int>(base::bits::AlignDown(base::checked_cast<size_t>(crop.width()), alignment)));
    crop.set_height(base::checked_cast<int>(base::bits::AlignDown(base::checked_cast<size_t>(crop.height()), alignment)));
    crop.set_x(base::checked_cast<int>(base::bits::AlignDown(base::checked_cast<size_t>((size.width() - crop.width()) / 2), alignment)));
    crop.set_y(base::checked_cast<int>(base::bits::AlignDown(base::checked_cast<size_t>((size.height() - crop.height()) / 2), alignment)));
...
gfx::Size PadToMatchAspectRatio(const gfx::Size& size, const gfx::Size& target)
{
    if (target.IsEmpty())
...
scoped_refptr<VideoFrame> ConvertToMemoryMappedFrame(scoped_refptr<VideoFrame> video_frame)
{
    CHECK(video_frame);
...
void ConvertToMemoryMappedFrameAsync(scoped_refptr<VideoFrame> video_frame, base::OnceCallback<void(scoped_refptr<VideoFrame>)> result_cb)
{
    CHECK(video_frame);
    CHECK(video_frame->HasMappableGpuBuffer());

    video_frame->MapGMBOrSharedImageAsync(base::BindOnce(&ProcessAsyncMappingResult, video_frame, std::move(result_cb)));
}

scoped_refptr<VideoFrame> WrapAsI420VideoFrame(scoped_refptr<VideoFrame> frame)
{
    DCHECK_EQ(VideoFrame::STORAGE_OWNED_MEMORY, frame->storage_type());
    DCHECK_EQ(PIXEL_FORMAT_I420A, frame->format());

    scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(frame, PIXEL_FORMAT_I420, frame->visible_rect(), frame->natural_size());
...
bool I420CopyWithPadding(const VideoFrame& src_frame, VideoFrame* dst_frame)
{
    ...
    DCHECK_GE(dst_frame->coded_size().height(), src_frame.visible_rect().height());
    DCHECK(dst_frame->visible_rect().origin().IsOrigin());

    if (libyuv::I420Copy(src_frame.visible_data(VideoFrame::Plane::kY), src_frame.stride(VideoFrame::Plane::kY), src_frame.visible_data(VideoFrame::Plane::kU),
            src_frame.stride(VideoFrame::Plane::kU), src_frame.visible_data(VideoFrame::Plane::kV), src_frame.stride(VideoFrame::Plane::kV),
            dst_frame->writable_data(VideoFrame::Plane::kY), dst_frame->stride(VideoFrame::Plane::kY), dst_frame->writable_data(VideoFrame::Plane::kU),
            dst_frame->stride(VideoFrame::Plane::kU), dst_frame->writable_data(VideoFrame::Plane::kV), dst_frame->stride(VideoFrame::Plane::kV),
    ...
    FillRegionOutsideVisibleRect(
        dst_frame->writable_data(VideoFrame::Plane::kY), dst_frame->stride(VideoFrame::Plane::kY), dst_frame->coded_size(), src_frame.visible_rect().size());
    FillRegionOutsideVisibleRect(dst_frame->writable_data(VideoFrame::Plane::kU), dst_frame->stride(VideoFrame::Plane::kU),
        VideoFrame::PlaneSize(PIXEL_FORMAT_I420, VideoFrame::Plane::kU, dst_frame->coded_size()),
        VideoFrame::PlaneSize(PIXEL_FORMAT_I420, VideoFrame::Plane::kU, src_frame.visible_rect().size()));
    FillRegionOutsideVisibleRect(dst_frame->writable_data(VideoFrame::Plane::kV), dst_frame->stride(VideoFrame::Plane::kV),
        VideoFrame::PlaneSize(PIXEL_FORMAT_I420, VideoFrame::Plane::kV, dst_frame->coded_size()),
        VideoFrame::PlaneSize(PIXEL_FORMAT_I420, VideoFrame::Plane::kV, src_frame.visible_rect().size()));

    return true;
}

scoped_refptr<VideoFrame> ReadbackTextureBackedFrameToMemorySync(
    VideoFrame& txt_frame, gpu::raster::RasterInterface* ri, const gpu::Capabilities& caps, VideoFramePool* pool)
{
    DCHECK(ri);

    TRACE_EVENT1("media", "ReadbackTextureBackedFrameToMemorySync", "timestamp", txt_frame.timestamp());
    VideoPixelFormat format = ReadbackFormat(txt_frame);
    if (format == PIXEL_FORMAT_UNKNOWN) {
        DLOG(ERROR) << "Readback is not possible for this frame: " << txt_frame.AsHumanReadableString();
        return nullptr;
    }

    scoped_refptr<VideoFrame> result = pool
        ? pool->CreateFrame(format, txt_frame.coded_size(), txt_frame.visible_rect(), txt_frame.natural_size(), txt_frame.timestamp())
        : VideoFrame::CreateFrame(format, txt_frame.coded_size(), txt_frame.visible_rect(), txt_frame.natural_size(), txt_frame.timestamp());
    ...
    for (size_t plane = 0; plane < planes; plane++) {
        ...
        if (!ReadbackTexturePlaneToMemorySync(txt_frame, plane, src_rect, result->writable_data(plane), result->stride(plane), ri, caps)) {
            return nullptr;
        }
    }
    return result;
...
bool ReadbackTexturePlaneToMemorySync(VideoFrame& src_frame, size_t src_plane, gfx::Rect& src_rect, uint8_t* dest_pixels, size_t dest_stride,
    gpu::raster::RasterInterface* ri, const gpu::Capabilities& caps)
{
    DCHECK(ri);

    bool result = ReadbackTexturePlaneToMemorySyncOOP(src_frame, src_plane, src_rect, dest_pixels, dest_stride, ri);
...
// and so on.
MEDIA_EXPORT SkColorType SkColorTypeForPlane(VideoPixelFormat format, size_t plane)
{
    switch (format) {
    case PIXEL_FORMAT_I420:
    case PIXEL_FORMAT_I420A:
    ...
    case PIXEL_FORMAT_NV16:
    case PIXEL_FORMAT_NV24:
        return plane == VideoFrame::Plane::kY ? kAlpha_8_SkColorType : kR8G8_unorm_SkColorType;
    case PIXEL_FORMAT_NV12A:
        return plane == VideoFrame::Plane::kY || plane == VideoFrame::Plane::kATriPlanar ? kAlpha_8_SkColorType : kR8G8_unorm_SkColorType;
    case PIXEL_FORMAT_P010LE:
    case PIXEL_FORMAT_P210LE:
    case PIXEL_FORMAT_P410LE:
        return plane == VideoFrame::Plane::kY ? kA16_unorm_SkColorType : kR16G16_unorm_SkColorType;
    case PIXEL_FORMAT_XBGR:
    case PIXEL_FORMAT_ABGR:
...
MEDIA_EXPORT VideoPixelFormat VideoPixelFormatFromSkColorType(SkColorType sk_color_type, bool is_opaque)
{
...
scoped_refptr<VideoFrame> CreateFromSkImage(
    sk_sp<SkImage> sk_image, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp, bool force_opaque)
{
    ...
    const auto format = VideoPixelFormatFromSkColorType(sk_image->colorType(), sk_image->isOpaque() || force_opaque);
    ...
    auto layout = VideoFrameLayout::CreateWithStrides(format, coded_size, std::vector<int32_t>(1, pm.rowBytes()));
    if (!layout)
        return nullptr;

    auto frame = VideoFrame::WrapExternalDataWithLayout(*layout, visible_rect, natural_size,
        // TODO(crbug.com/40162403): We should be able to wrap readonly memory in
        // a VideoFrame instead of using writable_addr() here.
        reinterpret_cast<uint8_t*>(pm.writable_addr()), pm.computeByteSize(), timestamp);
    if (!frame)
        return nullptr;

    frame->AddDestructionObserver(base::DoNothingWithBoundArgs(std::move(sk_image)));
    return frame;
}

std::tuple<SkYUVAInfo::PlaneConfig, SkYUVAInfo::Subsampling> VideoPixelFormatToSkiaValues(VideoPixelFormat video_format)
{
...
const libyuv::YuvConstants* GetYuvContantsForColorSpace(SkYUVColorSpace cs, bool output_argb_matrix)
{
