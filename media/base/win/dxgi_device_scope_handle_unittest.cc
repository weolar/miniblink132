...
protected:
    void SetUp() override
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(MFLockDXGIDeviceManager(&device_reset_token_, &dxgi_device_man_));

        // |dxgi_device_man_| does not create the device, creates Direct3D device.
        ComPtr<ID3D11Device> d3d11_device;
        UINT creation_flags
            = (D3D11_CREATE_DEVICE_VIDEO_SUPPORT | D3D11_CREATE_DEVICE_BGRA_SUPPORT | D3D11_CREATE_DEVICE_PREVENT_INTERNAL_THREADING_OPTIMIZATIONS);
        ...
        ASSERT_HRESULT_SUCCEEDED(D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, creation_flags, feature_levels, ARRAYSIZE(feature_levels),
        ...
        ASSERT_HRESULT_SUCCEEDED(dxgi_device_man_->ResetDevice(d3d11_device.Get(), device_reset_token_));
    }
...
TEST_F(DXGIDeviceScopedHandleTest, LockDevice)
{
    ...
    }
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(device_handle_2.LockDevice(IID_PPV_ARGS(&device2)));
    }
