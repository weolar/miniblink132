...
public:
    ...
    virtual bool Initialize(const gfx::Size& output_size, OutputRectChangeCB output_rect_change_cb) = 0;
    ...
    virtual void SetDCOMPSurfaceHandle(const base::UnguessableToken& token, SetDCOMPSurfaceHandleCB set_dcomp_surface_handle_cb) = 0;

    // Creates VideoFrame which will be returned in `create_video_frame_cb`.
    using CreateVideoFrameCB = base::OnceCallback<void(scoped_refptr<VideoFrame>, const gpu::Mailbox&)>;
    virtual void CreateVideoFrame(const gfx::Size& natural_size, CreateVideoFrameCB create_video_frame_cb) = 0;

    using CreateDXVideoFrameCB = base::OnceCallback<void(scoped_refptr<VideoFrame>, const gpu::Mailbox&)>;
    virtual void CreateVideoFrame(const gfx::Size& natural_size, gfx::GpuMemoryBufferHandle dx_handle, CreateDXVideoFrameCB create_video_frame_cb) = 0;
};
