...
const VideoCodecMap& GetVideoCodecsMap()
{
    static const base::NoDestructor<VideoCodecMap> AllVideoCodecsMap(
        { { VideoCodec::kVP9, MFVideoFormat_VP90 }, { VideoCodec::kHEVC, MFVideoFormat_HEVC }, { VideoCodec::kAV1, MFVideoFormat_AV1 } });
...
const AudioCodecMap& GetAudioCodecsMap()
{
    static const base::NoDestructor<AudioCodecMap> AllAudioCodecsMap(
        { { AudioCodec::kEAC3, MFAudioFormat_Dolby_DDPlus }, { AudioCodec::kAC4, MFAudioFormat_Dolby_AC4 } });
    return *AllAudioCodecsMap;
}

template <typename TCodec, typename TType, typename TCategory, typename TFunc>
bool CanMfDecodeCodec(TCodec codec, TType mft_type, TCategory mft_category, TFunc get_codecs)
{
    ...
    uint32_t count = 0;
    if (FAILED(MFTEnumEx(mft_category, MFT_ENUM_FLAG_SYNCMFT | MFT_ENUM_FLAG_ASYNCMFT | MFT_ENUM_FLAG_HARDWARE, &input_type, /*output_type=*/nullptr,
            &imf_activates, &count))) {
        return false;
    }
...
protected:
    ...
    std::vector<base::FilePath> GetMediaFoundationPackageInstallPaths(const std::wstring_view& decoder_lib_name, MediaFoundationCodecPackage codec_package)
    {
        return MediaFoundationPackageInstallPaths(media_foundation_package_family_names_, decoder_lib_name, codec_package);
    }
...
TEST_F(MediaFoundationPackageLocatorTest, VP9)
{
    ...
    std::vector<base::FilePath> paths = GetMediaFoundationPackageInstallPaths(L"msvp9dec_store.dll", media::MediaFoundationCodecPackage::kVP9);

    if (CanMfDecodeCodec(VideoCodec::kVP9, MFMediaType_Video, MFT_CATEGORY_VIDEO_DECODER, GetVideoCodecsMap)) {
...
TEST_F(MediaFoundationPackageLocatorTest, AV1)
{
    ...
    std::vector<base::FilePath> paths = GetMediaFoundationPackageInstallPaths(L"av1decodermft_store.dll", media::MediaFoundationCodecPackage::kAV1);

    if (CanMfDecodeCodec(VideoCodec::kAV1, MFMediaType_Video, MFT_CATEGORY_VIDEO_DECODER, GetVideoCodecsMap)) {
...
TEST_F(MediaFoundationPackageLocatorTest, HEVC)
{
    ...
    std::vector<base::FilePath> paths = GetMediaFoundationPackageInstallPaths(L"hevcdecoder_store.dll", media::MediaFoundationCodecPackage::kHEVC);

    if (CanMfDecodeCodec(VideoCodec::kHEVC, MFMediaType_Video, MFT_CATEGORY_VIDEO_DECODER, GetVideoCodecsMap)) {
...
{
    AddPackageFamilyName(L"DolbyLaboratories.DolbyDigitalPlusDecoderOEM_rz1tebttyb220");
    std::vector<base::FilePath> paths = GetMediaFoundationPackageInstallPaths(L"DolbyDDPDecMft.dll", media::MediaFoundationCodecPackage::kEAC3);
    ...
    // as codec packs.
    base::FilePath dolby_dec_mft_path = base::PathService::CheckedGet(base::DIR_SYSTEM);
    ...
    bool can_decode = CanMfDecodeCodec(AudioCodec::kEAC3, MFMediaType_Audio, MFT_CATEGORY_AUDIO_DECODER, GetAudioCodecsMap);
...
TEST_F(MediaFoundationPackageLocatorTest, AC4)
{
    ...
    std::vector<base::FilePath> paths = GetMediaFoundationPackageInstallPaths(L"DolbyAc4DecMft.dll", media::MediaFoundationCodecPackage::kAC4);

    if (CanMfDecodeCodec(AudioCodec::kAC4, MFMediaType_Audio, MFT_CATEGORY_AUDIO_DECODER, GetAudioCodecsMap)) {
