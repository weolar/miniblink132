...
std::vector<base::FilePath> MediaFoundationPackageInstallPaths(
    const std::vector<std::wstring_view>& package_family_names, std::wstring_view decoder_lib_name, MediaFoundationCodecPackage codec_package)
{
    ...
    for (const auto& package_family_name : package_family_names) {
        ...
        long rc = GetPackagesByPackageFamily(package_family_name.data(), &package_count, nullptr, &buffer_size, nullptr);
        if ((rc != ERROR_SUCCESS && rc != ERROR_INSUFFICIENT_BUFFER) || package_count == 0) {
            DVLOG(2) << package_family_name << " decoder package family is not installed. rc=" << rc;
            continue;
        }
        ...
        rc = GetPackagesByPackageFamily(package_family_name.data(), &package_count, package_full_names.data(), &buffer_size, buffer.data());
        if (package_count == 0) {
        ...
        for (const wchar_t* package_full_name : package_full_names) {
            ...
            rc = GetPackagePathByFullName(package_full_name, &package_path_len, package_path);
            if (rc != ERROR_SUCCESS) {
            ...
            if (codec_package == MediaFoundationCodecPackage::kEAC3 || codec_package == MediaFoundationCodecPackage::kAC4) {
    ...
    if (codec_package == MediaFoundationCodecPackage::kAV1) {
        ...
        package_paths.insert(package_paths.end(), legacy_av1_package_paths.begin(), legacy_av1_package_paths.end());
    }
