...
static const char kMediaFoundationLoadFailedMessage[] = "Failed to start Media Foundation, accelerated media functionality "
    ...
    static const bool kDidLoadSucceed = []() {
        for (const wchar_t* mfdll : { L"mf.dll", L"mfplat.dll" }) {
            if (!::LoadLibrary(mfdll)) {
                PLOG(ERROR) << kMediaFoundationLoadFailedMessage << "Could not load " << mfdll;
                return false;
            }
        }
        ...
        if (media::LoadMediaFoundationPackageDecoder(media::AudioCodec::kEAC3)) {
            DVLOG(2) << __func__ << ": EAC3(AC3) decoder loaded from MediaFoundation codec package";
        }
#endif // BUILDFLAG(ENABLE_PLATFORM_AC3_EAC3_AUDIO)
#if BUILDFLAG(ENABLE_PLATFORM_AC4_AUDIO)
        if (media::LoadMediaFoundationPackageDecoder(media::AudioCodec::kAC4)) {
            DVLOG(2) << __func__ << ": AC4 decoder loaded from MediaFoundation codec package";
        }
...
public:
    ...
    {
        return base::Singleton<MediaFoundationCreateMutexHelper, base::StaticMemorySingletonTraits<MediaFoundationCreateMutexHelper>>::get();
    }
    ~MediaFoundationCreateMutexHelper() = default;

private:
    friend struct base::StaticMemorySingletonTraits<MediaFoundationCreateMutexHelper>;
...
public:
    static MediaFoundationSession* GetInstance()
    {
        ...
        return base::Singleton<MediaFoundationSession, base::StaticMemorySingletonTraits<MediaFoundationSession>>::get();
    }
    ...
    bool has_media_foundation() const
    {
        return has_media_foundation_;
    }
...
private:
    ...
    MediaFoundationSession()
    {
        ...
        LOG_IF(ERROR, !has_media_foundation_) << kMediaFoundationLoadFailedMessage << logging::SystemErrorCodeToString(hr);
    }
...
{
    return LoadMediaFoundationLibraries() && MediaFoundationSession::GetInstance()->has_media_foundation();
}
