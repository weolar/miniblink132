...
class MediaFoundationCdmProxy : public base::RefCountedThreadSafe<MediaFoundationCdmProxy> {
public:
    ...
    virtual HRESULT GetInputTrustAuthority(
        uint32_t stream_id, uint32_t stream_count, const uint8_t* content_init_data, uint32_t content_init_data_size, REFIID riid, IUnknown** object_out)
        = 0;
    ...
    virtual HRESULT ProcessContentEnabler(IUnknown* request, IMFAsyncResult* result) = 0;
