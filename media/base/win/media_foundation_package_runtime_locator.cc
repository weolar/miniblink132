...
constexpr wchar_t kMfPackageFamilyName_HEVC[] = L"Microsoft.HEVCVideoExtension_8wekyb3d8bbwe";
constexpr wchar_t kMfPackageFamilyName_HEVC_2[] = L"Microsoft.HEVCVideoExtensions_8wekyb3d8bbwe";
constexpr wchar_t kMfPackageFamilyName_DolbyVision[] = L"DolbyLaboratories.DolbyVisionAccess_rz1tebttyb220";
constexpr wchar_t kMfPackageFamilyName_DolbyVision_2[] = L"DolbyLaboratories.DolbyVisionHDR_rz1tebttyb220";
constexpr wchar_t kMfPackageFamilyName_AC4[] = L"DolbyLaboratories.DolbyAC4DecoderOEM_rz1tebttyb220";
constexpr wchar_t kMfPackageFamilyName_EAC3[] = L"DolbyLaboratories.DolbyDigitalPlusDecoderOEM_rz1tebttyb220";
...
constexpr MediaFoundationCodecPackage kMfCodecPackages[]
    = { MediaFoundationCodecPackage::kEAC3, MediaFoundationCodecPackage::kAC4, MediaFoundationCodecPackage::kHEVC, MediaFoundationCodecPackage::kDolbyVision };
...
std::vector<std::wstring_view> GetMfCodecPackageFamilyNames(MediaFoundationCodecPackage codec_package)
{
...
std::wstring GetMfCodecPackageLibraryName(MediaFoundationCodecPackage codec_package)
{
    switch (codec_package) {
    case MediaFoundationCodecPackage::kAC4:
...
public:
    ...
    MediaFoundationPackageRuntimeLocator(const MediaFoundationPackageRuntimeLocator&) = delete;
    MediaFoundationPackageRuntimeLocator& operator=(const MediaFoundationPackageRuntimeLocator&) = delete;
...
// instance on-demand.
MediaFoundationPackageRuntimeLocator& MediaFoundationPackageRuntimeLocator::GetInstance()
{
...
MediaFoundationPackageRuntimeLocator::MediaFoundationPackageRuntimeLocator()
{
    ...
    for (auto& codec_package : kMfCodecPackages) {
        ...
        std::vector<base::FilePath> paths = MediaFoundationPackageInstallPaths(package_names, package_library, codec_package);
        if (paths.empty()) {
            DVLOG(2) << __func__ << ": MediaFoundation codec package (" << GetMfCodecPackageName(codec_package) << ") is not installed.";
            continue;
        }
...
bool MediaFoundationPackageRuntimeLocator::FoundModule(MediaFoundationCodecPackage codec_package) const
{
    auto result = mf_dll_module_.find(codec_package);
    if (result == mf_dll_module_.cend()) {
        DVLOG(2) << __func__ << ": Can not find codec package (" << GetMfCodecPackageName(codec_package) << ")";
        return false;
    }

    auto& module_info = result->second;
    if (module_info.Paths.empty()) {
        DVLOG(2) << __func__ << ": Found codec package (" << GetMfCodecPackageName(codec_package) << "), but it's library path is empty";
        return false;
    }

    if (module_info.Loaded.has_value() && !module_info.Loaded.value()) {
        DVLOG(2) << __func__ << ": Found codec package (" << GetMfCodecPackageName(codec_package) << "), but load it's library module failed";
        return false;
    }
...
bool MediaFoundationPackageRuntimeLocator::LoadModule(MediaFoundationCodecPackage codec_package)
{
    auto result = mf_dll_module_.find(codec_package);
    if (result == mf_dll_module_.cend()) {
        DVLOG(2) << __func__ << ": Can not find codec package (" << GetMfCodecPackageName(codec_package) << ")";
    ...
    for (auto& path : module_info.Paths) {
        if (LoadNativeLibrary(path, nullptr)) {
            ...
            DVLOG(3) << __func__ << ": MediaFoundation package " << GetMfCodecPackageName(codec_package) << " loaded";
            return true;
        }
    }
...
std::optional<MediaFoundationCodecPackage> AudioCodecToMediaFoundationCodecPackage(AudioCodec codec)
{
    ...
    case AudioCodec::kAC4:
        ...
        break;
    default:
        DVLOG(3) << "Audio codec " << GetCodecName(codec) << " has no MediaFoundation package.";
        break;
    }
    return package_type;
}
...
bool LoadMediaFoundationPackageDecoder(AudioCodec codec)
{
    ...
    return codec_package.has_value() ? MediaFoundationPackageRuntimeLocator::GetInstance().LoadModule(codec_package.value()) : false;
}

bool FindMediaFoundationPackageDecoder(AudioCodec codec)
{
    auto codec_package = AudioCodecToMediaFoundationCodecPackage(codec);
    return codec_package.has_value() ? MediaFoundationPackageRuntimeLocator::GetInstance().FoundModule(codec_package.value()) : false;
}
