...
static DecoderBuffer::TimeInfo CreateTimeInfo(base::TimeDelta timestamp, base::TimeDelta duration)
{
...
{
    return MakeAudioBuffer(kSampleFormatPlanarF32, CHANNEL_LAYOUT_MONO, 1, kSampleRate, 0.0f, kDataStep, frames, kNoTimestamp);
}
...
TEST(AudioDiscardHelperTest, TimeDeltaToFrames)
{
    ...
    EXPECT_EQ(kSampleRate / 100, discard_helper.TimeDeltaToFrames(base::Milliseconds(10)));
    ...
    EXPECT_EQ(kSampleRate - 1, discard_helper.TimeDeltaToFrames(base::Microseconds(small_remainder)));
    ...
    EXPECT_EQ(kSampleRate, discard_helper.TimeDeltaToFrames(base::Microseconds(large_remainder)));
}
...
TEST(AudioDiscardHelperTest, BasicProcessBuffers)
{
    ...
    DecoderBuffer::TimeInfo time_info = CreateTimeInfo(kTimestamp, kEstimatedDuration);
...
TEST(AudioDiscardHelperTest, ProcessBuffersWithInitialDiscard)
{
    ...
    ASSERT_FLOAT_EQ(kDiscardFrames * kDataStep, ExtractDecodedData(*decoded_buffer, 0));
}
...
TEST(AudioDiscardHelperTest, InitialDiscardAndDiscardPadding)
{
    ...
    ASSERT_TRUE(discard_helper.initialized());
    EXPECT_EQ(kTimestamp, decoded_buffer->timestamp());
    EXPECT_EQ(kDuration - kDuration / 4 - kDuration / 8 - kDuration / 16, decoded_buffer->duration());
    EXPECT_EQ(kTestFrames - kTestFrames / 4 - kTestFrames / 8 - kTestFrames / 16, decoded_buffer->frame_count());
}
...
TEST(AudioDiscardHelperTest, InitialDiscardAndDiscardPaddingAndDecoderDelay)
{
    ...
    ASSERT_NEAR(kDecoderDelay * kDataStep, ExtractDecodedData(*decoded_buffer, kDecoderDelay), kDataStep / 1000);
    ASSERT_TRUE(discard_helper.ProcessBuffers(time_info, decoded_buffer.get()));
    EXPECT_EQ(kTimestamp, decoded_buffer->timestamp());
    EXPECT_EQ(kDuration / 2, decoded_buffer->duration());
    EXPECT_EQ(kTestFrames / 2, decoded_buffer->frame_count());
    ...
    const int kDiscardFrames = kTestFrames / 4;
    ASSERT_FLOAT_EQ(0.0f, ExtractDecodedData(*decoded_buffer, 0));
    ASSERT_FLOAT_EQ(kDiscardFrames * 2 * kDataStep, ExtractDecodedData(*decoded_buffer, kDecoderDelay - kDiscardFrames));
    ...
    ASSERT_FLOAT_EQ(0.0f, ExtractDecodedData(*decoded_buffer, 0));
    ASSERT_TRUE(discard_helper.ProcessBuffers(time_info, decoded_buffer.get()));
    EXPECT_EQ(kTimestamp + kDuration / 2 + 3 * kDuration / 4, decoded_buffer->timestamp());
    EXPECT_EQ(kDuration / 2, decoded_buffer->duration());
    EXPECT_EQ(kTestFrames / 2, decoded_buffer->frame_count());
    ASSERT_FLOAT_EQ(kTestFrames / 2 * kDataStep, ExtractDecodedData(*decoded_buffer, 0));
}
...
TEST(AudioDiscardHelperTest, CompleteDiscard)
{
    ...
    DecoderBuffer::TimeInfo time_info = CreateTimeInfo(kTimestamp, kDuration);
    time_info.discard_padding = std::make_pair(kInfiniteDuration, base::TimeDelta());
    scoped_refptr<AudioBuffer> decoded_buffer = CreateDecodedBuffer(kTestFrames);
...
TEST(AudioDiscardHelperTest, CompleteDiscardWithInitialDiscardDecoderDelay)
{
    ...
    ASSERT_FLOAT_EQ(kDecoderDelay * kDataStep, ExtractDecodedData(*decoded_buffer, 0));
}
