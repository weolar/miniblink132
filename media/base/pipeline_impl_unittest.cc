...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&Pipeline::Stop, base::Unretained(pipeline)));
}
...
ACTION_TEMPLATE(PostCallback, HAS_1_TEMPLATE_PARAMS(int, k), AND_1_VALUE_PARAMS(p0))
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(std::get<k>(args)), p0));
}
...
public:
        ...
        , renderer_(scoped_renderer_->AsWeakPtr())
    {
        pipeline_ = std::make_unique<PipelineImpl>(task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(),
            base::BindRepeating(&PipelineImplTest::TakeRenderer, base::Unretained(this)), &media_log_);
        ...
        EXPECT_CALL(*demuxer_, GetTimelineOffset()).WillRepeatedly(Return(base::Time()));

        EXPECT_CALL(*renderer_, GetMediaTime()).WillRepeatedly(Return(base::TimeDelta()));
    ...
    void OnDemuxerError()
    {
        demuxer_host_->OnDemuxerError(PIPELINE_ERROR_ABORT);
    }
...
protected:
    ...
    void SetDemuxerExpectations(base::TimeDelta duration)
    {
        ...
        EXPECT_CALL(*demuxer_, OnInitialize(_, _)).WillOnce(DoAll(SaveArg<0>(&demuxer_host_), SetDemuxerProperties(duration), PostCallback<1>(PIPELINE_OK)));
    ...
    std::unique_ptr<StrictMock<MockDemuxerStream>> CreateStream(DemuxerStream::Type type)
    {
    ...
    {
        EXPECT_CALL(*renderer_, OnInitialize(_, _, _)).WillOnce(DoAll(SaveArg<1>(&renderer_client_), PostCallback<2>(PIPELINE_OK)));
    }

    void StartPipeline(Pipeline::StartType start_type = Pipeline::StartType::kNormal)
    {
        EXPECT_CALL(callbacks_, OnWaiting(_)).Times(0);
        pipeline_->Start(start_type, demuxer_.get(), &callbacks_, base::BindOnce(&CallbackHelper::OnStart, base::Unretained(&callbacks_)));
    }
    ...
    void SetRendererPostStartExpectations()
    {
        ...
        EXPECT_CALL(*renderer_, SetWasPlayedWithUserActivationAndHighMediaEngagement(false));
        EXPECT_CALL(*renderer_, StartPlayingFrom(start_time_))
            .WillOnce(SetBufferingState(&renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
        EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
    }
    ...
    void StartPipelineAndExpect(PipelineStatus start_status, Pipeline::StartType start_type = Pipeline::StartType::kNormal,
    ...
    void CreateVideoStream(bool is_encrypted = false)
    {
        ...
        video_stream_->set_video_decoder_config(is_encrypted ? TestVideoConfig::NormalEncrypted() : TestVideoConfig::Normal());
    ...
    void CreateEncryptedVideoStream()
    {
        CreateVideoStream(true);
    }
    ...
    void SetCdmAndExpect(bool expected_result)
    {
        ...
        pipeline_->SetCdm(&cdm_context_, base::BindOnce(&CallbackHelper::OnCdmAttached, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void ExpectSeek(const base::TimeDelta& seek_time, bool underflowed)
    {
        ...
        EXPECT_CALL(*demuxer_, OnSeek(seek_time, _)).WillOnce(RunOnceCallback<1>(PIPELINE_OK));
    ...
    {
        pipeline_->Seek(seek_time, base::BindOnce(&CallbackHelper::OnSeek, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void DoSuspend()
    {
        pipeline_->Suspend(base::BindOnce(&CallbackHelper::OnSuspend, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
        ResetRenderer();
    }
    ...
    {
        pipeline_->Resume(seek_time, base::BindOnce(&CallbackHelper::OnResume, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
...
{
    EXPECT_CALL(*demuxer_, OnInitialize(_, _)).WillOnce(PostCallback<1>(PIPELINE_OK));
    EXPECT_CALL(*demuxer_, Stop());
...
TEST_F(PipelineImplTest, StartSuspendedAndResumeAudioOnly)
{
    ...
    StartPipelineAndExpect(PIPELINE_OK, Pipeline::StartType::kSuspendAfterMetadataForAudioOnly, PostStartStatus::kSuspended);
...
TEST_F(PipelineImplTest, StartSuspendedAndResumeAudioVideo)
{
    ...
    StartPipelineAndExpect(PIPELINE_OK, Pipeline::StartType::kSuspendAfterMetadata, PostStartStatus::kSuspended);
    ASSERT_TRUE(pipeline_->IsSuspended());
...
TEST_F(PipelineImplTest, StartSuspendedFailsOnVideoWithAudioOnlyExpectation)
{
    ...
    ExpectRendererInitialization();
    StartPipelineAndExpect(PIPELINE_OK, Pipeline::StartType::kSuspendAfterMetadataForAudioOnly, PostStartStatus::kNormal);
...
TEST_F(PipelineImplTest, DemuxerErrorDuringStop)
{
    ...
    EXPECT_CALL(*demuxer_, Stop()).WillOnce(InvokeWithoutArgs(this, &PipelineImplTest::OnDemuxerError));
    pipeline_->Stop();
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(PipelineImplTest, EncryptedStream_SetCdmAfterStart)
{
    ...
    EXPECT_CALL(callbacks_, OnWaiting(_)).WillOnce(RunClosure(run_loop.QuitClosure()));
    pipeline_->Start(Pipeline::StartType::kNormal, demuxer_.get(), &callbacks_, base::BindOnce(&CallbackHelper::OnStart, base::Unretained(&callbacks_)));
    run_loop.Run();
...
TEST_F(PipelineImplTest, SeekAfterError)
{
    ...
    pipeline_->Seek(base::Milliseconds(100), base::BindOnce(&CallbackHelper::OnSeek, base::Unretained(&callbacks_)));
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(PipelineImplTest, SuspendResume)
{
    ...
    EXPECT_EQ(stats.audio_memory_usage, pipeline_->GetStatistics().audio_memory_usage);
    EXPECT_EQ(stats.video_memory_usage, pipeline_->GetStatistics().video_memory_usage);
...
TEST_F(PipelineImplTest, SetVolumeDuringStartup)
{
    ...
    EXPECT_CALL(*renderer_, SetWasPlayedWithUserActivationAndHighMediaEngagement(false));
    EXPECT_CALL(callbacks_, OnStart(HasStatusCode(PIPELINE_OK)));
    EXPECT_CALL(callbacks_, OnMetadata(_)).WillOnce(RunOnceClosure(base::BindOnce(&PipelineImpl::SetVolume, base::Unretained(pipeline_.get()), expected)));
    ExpectRendererInitialization();
    EXPECT_CALL(*renderer_, SetPlaybackRate(0.0));
    EXPECT_CALL(*renderer_, StartPlayingFrom(start_time_))
        .WillOnce(SetBufferingState(&renderer_client_, BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN));
...
TEST_F(PipelineImplTest, Properties)
{
    ...
    StartPipelineAndExpect(PIPELINE_OK);
    EXPECT_EQ(kDuration.ToInternalValue(), pipeline_->GetMediaDuration().ToInternalValue());
    EXPECT_FALSE(pipeline_->DidLoadingProgress());
}
...
{
    EXPECT_CALL(*demuxer_, OnInitialize(_, _)).WillOnce(DoAll(SaveArg<0>(&demuxer_host_), PostCallback<1>(PIPELINE_OK)));
    EXPECT_CALL(*demuxer_, Stop());
    EXPECT_CALL(callbacks_, OnMetadata(_));
    EXPECT_CALL(callbacks_, OnStart(_));
    StartPipeline();
...
TEST_F(PipelineImplTest, DemuxerErrorDuringSeek)
{
    ...
    pipeline_->Seek(seek_time, base::BindOnce(&CallbackHelper::OnSeek, base::Unretained(&callbacks_)));
    EXPECT_CALL(callbacks_, OnSeek(HasStatusCode(PIPELINE_ERROR_READ))).WillOnce(Stop(pipeline_.get()));
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(PipelineImplTest, Underflow)
{
    ...
    // Simulate underflow.
    EXPECT_CALL(callbacks_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN));
    renderer_client_->OnBufferingStateChange(BUFFERING_HAVE_NOTHING, BUFFERING_CHANGE_REASON_UNKNOWN);
    base::RunLoop().RunUntilIdle();
...
TEST_F(PipelineImplTest, GetMediaTime)
{
    ...
    EXPECT_CALL(*renderer_, GetMediaTime()).WillRepeatedly(Return(base::Seconds(1)));
    EXPECT_EQ(kMediaTime, pipeline_->GetMediaTime());
}
...
TEST_F(PipelineImplTest, GetMediaTimeAfterSeek)
{
    ...
    ExpectSeek(kSeekTime, false);
    pipeline_->Seek(kSeekTime, base::BindOnce(&CallbackHelper::OnSeek, base::Unretained(&callbacks_)));
...
private:
    ...
    void SetInitializeExpectations(TeardownState state, StopOrError stop_or_error)
    {
        if (state == kInitDemuxer) {
            if (stop_or_error == kStop) {
                EXPECT_CALL(*demuxer_, OnInitialize(_, _)).WillOnce(DoAll(PostStop(pipeline_.get()), PostCallback<1>(PIPELINE_OK)));
                // Note: OnStart callback is not called after pipeline is stopped.
            } else {
                EXPECT_CALL(*demuxer_, OnInitialize(_, _)).WillOnce(PostCallback<1>(DEMUXER_ERROR_COULD_NOT_OPEN));
                EXPECT_CALL(callbacks_, OnStart(HasStatusCode(DEMUXER_ERROR_COULD_NOT_OPEN))).WillOnce(Stop(pipeline_.get()));
            }
        ...
        if (state == kInitRenderer) {
            if (stop_or_error == kStop) {
                EXPECT_CALL(*renderer_, OnInitialize(_, _, _)).WillOnce(DoAll(PostStop(pipeline_.get()), PostCallback<2>(PIPELINE_OK)));
                // Note: OnStart is not callback after pipeline is stopped.
            } else {
                EXPECT_CALL(*renderer_, OnInitialize(_, _, _)).WillOnce(PostCallback<2>(PIPELINE_ERROR_INITIALIZATION_FAILED));
                EXPECT_CALL(callbacks_, OnStart(HasStatusCode(PIPELINE_ERROR_INITIALIZATION_FAILED))).WillOnce(Stop(pipeline_.get()));
            }
    ...
    void DoSeek(TeardownState state, StopOrError stop_or_error)
    {
        ...
        pipeline_->Seek(base::Seconds(10), base::BindOnce(&CallbackHelper::OnSeek, base::Unretained(&callbacks_)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void SetSeekExpectations(TeardownState state, StopOrError stop_or_error)
    {
        if (state == kFlushing) {
            ...
            if (stop_or_error == kStop) {
                EXPECT_CALL(*renderer_, OnFlush(_)).WillOnce(DoAll(Stop(pipeline_.get()), RunOnceClosure<0>()));
                // Note: OnSeek callbacks are not called
                // after pipeline is stopped.
            } else {
                EXPECT_CALL(*renderer_, OnFlush(_)).WillOnce(DoAll(SetError(&renderer_client_, PIPELINE_ERROR_READ), RunOnceClosure<0>()));
                EXPECT_CALL(callbacks_, OnSeek(HasStatusCode(PIPELINE_ERROR_READ))).WillOnce(Stop(pipeline_.get()));
            }
            return;
        }
        ...
        if (state == kSeeking) {
            if (stop_or_error == kStop) {
                EXPECT_CALL(*demuxer_, OnSeek(_, _)).WillOnce(DoAll(PostStop(pipeline_.get()), RunOnceCallback<1>(PIPELINE_OK)));
    ...
    void SetSuspendExpectations(TeardownState state, StopOrError stop_or_error)
    {
        ...
        if (state == kResuming) {
            if (stop_or_error == kStop) {
                ...
                EXPECT_CALL(*demuxer_, OnSeek(_, _)).WillOnce(RunOnceCallback<1>(PIPELINE_ERROR_READ));
                EXPECT_CALL(callbacks_, OnResume(HasStatusCode(PIPELINE_ERROR_READ))).WillOnce(Stop(pipeline_.get()));
            }
    ...
    void DoStopOrError(StopOrError stop_or_error, bool expect_errors)
    {
        ...
        case kError:
            if (expect_errors) {
                EXPECT_CALL(*demuxer_, Stop());
                EXPECT_CALL(callbacks_, OnError(HasStatusCode(PIPELINE_ERROR_READ))).WillOnce(Stop(pipeline_.get()));
            }
            renderer_client_->OnError(PIPELINE_ERROR_READ);
            break;
