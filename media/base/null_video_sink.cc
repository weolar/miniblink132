...
NullVideoSink::NullVideoSink(
    bool clockless, base::TimeDelta interval, const NewFrameCB& new_frame_cb, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner)
...
void NullVideoSink::Start(RenderCallback* callback)
{
    ...
    cancelable_worker_.Reset(base::BindRepeating(&NullVideoSink::CallRender, base::Unretained(this)));
...
void NullVideoSink::CallRender()
{
    ...
    scoped_refptr<VideoFrame> new_frame = callback_->Render(current_render_time_, end_of_interval,
        background_render_ ? VideoRendererSink::RenderCallback::RenderingMode::kBackground : VideoRendererSink::RenderCallback::RenderingMode::kNormal);
    DCHECK(new_frame);
    ...
    task_runner_->PostDelayedTask(FROM_HERE, cancelable_worker_.callback(), delay);
}

void NullVideoSink::PaintSingleFrame(scoped_refptr<VideoFrame> frame, bool repaint_duplicate_frame)
{
