...
public:
    ...
    public:
        explicit FakeFactory(RendererType type)
            : type_(type)
        {
        }
        ...
        std::unique_ptr<Renderer> CreateRenderer(const scoped_refptr<base::SequencedTaskRunner>& media_task_runner,
            ...
            RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space) override
        {
            return nullptr;
        }

        RendererType factory_type()
        {
            return type_;
        }
    ...
    RendererFactorySelectorTest& operator=(const RendererFactorySelectorTest&) = delete;
    ...
    void AddConditionalFactory(RendererType type)
    {
        ...
        selector_.AddConditionalFactory(
            type, std::make_unique<FakeFactory>(type), base::BindRepeating(&RendererFactorySelectorTest::IsConditionMet, base::Unretained(this), type));
    }

    RendererType GetCurrentlySelectedRendererType()
    {
        return reinterpret_cast<FakeFactory*>(selector_.GetCurrentFactory())->factory_type();
    }
...
TEST_F(RendererFactorySelectorTest, MultipleConditionalFactories)
{
    ...
    condition_met_map_[RendererType::kFlinging] = true;
    condition_met_map_[RendererType::kCourier] = true;
    EXPECT_TRUE(GetCurrentlySelectedRendererType() == RendererType::kFlinging || GetCurrentlySelectedRendererType() == RendererType::kCourier);
}
