...
bool operator==(const AudioDecoderConfig& config, const AudioDecoderConfig& other_config)
{
    return config.Matches(other_config);
}
bool operator==(const VideoDecoderConfig& config, const VideoDecoderConfig& other_config)
{
    return config.Matches(other_config);
}
...
DecoderBufferSideData& DecoderBufferSideData::operator=(DecoderBufferSideData&&) = default;

DecoderBufferSideData::DecoderBufferSideData(DecoderBufferSideData&& other) = default;

bool DecoderBufferSideData::Matches(const DecoderBufferSideData& other) const
{
    return spatial_layers == other.spatial_layers && alpha_data.as_span() == other.alpha_data.as_span() && secure_handle == other.secure_handle
        && discard_padding == other.discard_padding && next_config == other.next_config;
}
