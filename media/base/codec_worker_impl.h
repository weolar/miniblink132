...
public:
    ...
    {
        return WorkerInterface { .init = Init, .reset = Reset, .sync = Sync, .launch = Launch, .execute = Execute, .end = End };
    }
...
private:
    ...
    static void Launch(Worker* const worker)
    {
        ChangeState(worker, StatusWork);
    }
