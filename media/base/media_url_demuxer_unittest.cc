...
public:
    ...
    void InitializeTest(const GURL& media_url, const GURL& first_party, bool allow_credentials)
    {
        demuxer_ = std::make_unique<MediaUrlDemuxer>(base::SingleThreadTaskRunner::GetCurrentDefault(), media_url, net::SiteForCookies::FromUrl(first_party),
            url::Origin::Create(first_party), net::StorageAccessApiStatus::kNone, allow_credentials, false);
    }
...
TEST_F(MediaUrlDemuxerTest, BaseCase)
{
    ...
    EXPECT_TRUE(net::SiteForCookies::FromUrl(default_first_party_url_).IsEquivalent(params.site_for_cookies));
...
TEST_F(MediaUrlDemuxerTest, AcceptsEmptyStrings)
{
    ...
    EXPECT_TRUE(net::SiteForCookies::FromUrl(GURL()).IsEquivalent(params.site_for_cookies));
...
{
    InitializeTest();
    demuxer_->Initialize(nullptr, base::BindOnce(&MediaUrlDemuxerTest::VerifyCallbackOk, base::Unretained(this)));
...
{
    InitializeTest();
    demuxer_->Seek(base::TimeDelta(), base::BindOnce(&MediaUrlDemuxerTest::VerifyCallbackOk, base::Unretained(this)));
