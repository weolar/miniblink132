    ...
    VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp)
{
    ...
    while (!frames_.empty()) {
        ...
        if (pool_frame->IsSameAllocation(format, coded_size, visible_rect, natural_size)) {
            frame = pool_frame;
    ...
    if (!frame) {
        frame = VideoFrame::CreateZeroInitializedFrame(format, coded_size, visible_rect, natural_size, timestamp);
    ...
    scoped_refptr<VideoFrame> wrapped_frame = VideoFrame::WrapVideoFrame(frame, frame->format(), frame->visible_rect(), frame->natural_size());
    wrapped_frame->AddDestructionObserver(base::BindOnce(&VideoFramePool::PoolImpl::FrameReleased, this, std::move(frame)));
