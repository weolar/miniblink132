...
template <typename Signature> class CallbackWithTimeoutHelper;
...
template <typename... Args> class CallbackWithTimeoutHelper<void(Args...)> {
public:
    ...
    explicit CallbackWithTimeoutHelper(CallbackType callback, base::TimeDelta timeout_delay, BoundArgs&&... args)
        : callback_(std::move(callback))
    {
        timeout_callback_ = base::BindOnce(&CallbackWithTimeoutHelper::RunWithReason, base::Unretained(this), std::forward<BoundArgs>(args)...);
    ...
    CallbackWithTimeoutHelper(int ignored, CallbackType callback, base::TimeDelta timeout_delay, TimeoutCallback timeout_callback)
        : callback_(std::move(callback))
    ...
    CallbackWithTimeoutHelper& operator=(const CallbackWithTimeoutHelper&) = delete;
...
private:
    ...
    void ScheduleTimeoutCallback(base::TimeDelta timeout_delay)
    {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&CallbackWithTimeoutHelper::OnTimeout, weak_factory_.GetWeakPtr()), timeout_delay);
    }
...
inline base::OnceCallback<T> WrapCallbackWithTimeoutHandler(base::OnceCallback<T> callback, base::TimeDelta timeout_delay, TimeoutCallback timeout_callback)
{
    return base::BindOnce(&internal::CallbackWithTimeoutHelper<T>::Run,
        std::make_unique<internal::CallbackWithTimeoutHelper<T>>(
            /*ignored=*/0, std::move(callback), timeout_delay, std::move(timeout_callback)));
}

template <typename T, typename... Args>
inline base::OnceCallback<T> WrapCallbackWithDefaultInvokeIfTimeout(base::OnceCallback<T> callback, base::TimeDelta timeout_delay, Args&&... args)
{
    return base::BindOnce(&internal::CallbackWithTimeoutHelper<T>::Run,
        std::make_unique<internal::CallbackWithTimeoutHelper<T>>(std::move(callback), timeout_delay, std::forward<Args>(args)...));
}
