...
ByteQueue::ByteQueue()
{
    ...
    CHECK(base::UncheckedMalloc(size_, reinterpret_cast<void**>(&new_buffer)) && new_buffer);
...
bool ByteQueue::Push(base::span<const uint8_t> data)
{
    ...
    if (size_needed > size_) {
        ...
        const size_t safe_size = (base::CheckedNumeric<size_t>(size_) + size_ / 4).ValueOrDie();
        ...
        uint8_t* new_buffer = nullptr;
        if (!base::UncheckedMalloc(new_size, reinterpret_cast<void**>(&new_buffer)) || !new_buffer) {
            return false;
        }
