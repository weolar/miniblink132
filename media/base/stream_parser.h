...
public:
    ...
    typedef base::RepeatingCallback<bool(std::unique_ptr<MediaTracks>)> NewConfigCB;
    ...
    typedef base::RepeatingCallback<void(EmeInitDataType, const std::vector<uint8_t>&)> EncryptedMediaInitDataCB;
    ...
    // and duration.
    virtual void Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
    ...
    [[nodiscard]] virtual bool AppendToParseBuffer(base::span<const uint8_t> buf) = 0;
    ...
    [[nodiscard]] virtual bool ProcessChunks(std::unique_ptr<BufferQueue> buffer_queue);
};
...
MEDIA_EXPORT bool MergeBufferQueues(const StreamParser::BufferQueueMap& buffers, StreamParser::BufferQueue* merged_buffers);
