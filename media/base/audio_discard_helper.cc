...
static void WarnOnNonMonotonicTimestamps(base::TimeDelta last_timestamp, base::TimeDelta current_timestamp)
{
...
AudioDiscardHelper::AudioDiscardHelper(int sample_rate, size_t decoder_delay, bool delayed_discard)
...
bool AudioDiscardHelper::ProcessBuffers(const DecoderBuffer::TimeInfo& time_info, AudioBuffer* decoded_buffer)
{
    ...
    if (!initialized()) {
        ...
        timestamp_helper_.SetBaseTimestamp(std::max(base::TimeDelta(), time_info.timestamp));
    }
    DCHECK(initialized());
    ...
    DecoderBuffer::DiscardPadding current_discard_padding = time_info.discard_padding;
    if (delayed_discard_) {
    ...
    if (discard_frames_ > 0) {
        ...
        DVLOG(1) << "Initial discard of " << frames_to_discard << " out of " << decoded_frames << " frames.";
    ...
    if (delayed_end_discard_ > 0) {
        ...
        DVLOG(1) << "Delayed end discard of " << delayed_end_discard_ << " out of " << decoded_frames << " frames starting at " << discard_index;

        decoded_buffer->TrimRange(discard_index, discard_index + delayed_end_discard_);
        delayed_end_discard_ = 0;
    }
    ...
    if (current_discard_padding.first.is_positive()) {
        ...
        const size_t start_frames_to_discard = current_discard_padding.first == kInfiniteDuration
            ? (decoder_delay_ > 0 ? TimeDeltaToFrames(time_info.duration) : decoded_frames)
        ...
        if (discard_start >= decoded_frames) {
            DLOG(ERROR) << "Unsupported discard padding and decoder delay mix. Due to "
        ...
        DVLOG(1) << "Front discard of " << frames_to_discard << " out of " << decoded_frames << " frames starting at " << discard_start;
    ...
    if (current_discard_padding.second.is_positive()) {
        ...
        if (end_frames_to_discard > 0) {
            DVLOG(1) << "End discard of " << end_frames_to_discard << " out of " << decoded_frames;
    ...
    DVLOG(3) << __func__ << " ts: " << timestamp_helper_.GetTimestamp() << " frames: " << decoded_buffer->frame_count();
