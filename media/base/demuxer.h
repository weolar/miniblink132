...
public:
    ...
    virtual void OnBufferedTimeRangesChanged(const Ranges<base::TimeDelta>& ranges) = 0;
...
public:
    ...
    using EncryptedMediaInitDataCB = base::RepeatingCallback<void(EmeInitDataType type, const std::vector<uint8_t>& init_data)>;
    ...
    using MediaTracksUpdatedCB = base::RepeatingCallback<void(std::unique_ptr<MediaTracks>)>;

    // Called once the demuxer has finished enabling or disabling tracks.
    using TrackChangeCB = base::OnceCallback<void(const std::vector<DemuxerStream*>&)>;
    ...
    virtual void Initialize(DemuxerHost* host, PipelineStatusCallback status_cb) = 0;
    ...
    virtual std::optional<container_names::MediaContainerName> GetContainerForMetrics() const = 0;
    ...
    virtual void OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb) = 0;

    virtual void OnSelectedVideoTrackChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb) = 0;
