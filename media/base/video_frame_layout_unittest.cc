...
std::vector<ColorPlaneLayout> CreatePlanes(const std::vector<int32_t>& strides, const std::vector<size_t>& offsets, const std::vector<size_t>& sizes)
{
...
TEST(VideoFrameLayout, CreateWithStrides)
{
    ...
    auto layout = VideoFrameLayout::CreateWithStrides(PIXEL_FORMAT_I420, coded_size, strides);
    ASSERT_TRUE(layout.has_value());
...
TEST(VideoFrameLayout, CreateWithPlanes)
{
    ...
    auto layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes));
    ASSERT_TRUE(layout.has_value());
...
TEST(VideoFrameLayout, CreateMultiPlanar)
{
    ...
    auto layout = VideoFrameLayout::CreateMultiPlanar(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes));
    ASSERT_TRUE(layout.has_value());
...
TEST(VideoFrameLayout, ToStringWithPlanes)
{
    ...
    ostream << *layout;
    const std::string kNoModifier = ModifierToHexString(gfx::NativePixmapHandle::kNoModifier);
...
TEST(VideoFrameLayout, ToStringMultiPlanar)
{
    ...
    auto layout = VideoFrameLayout::CreateMultiPlanar(PIXEL_FORMAT_NV12, coded_size, CreatePlanes(strides, offsets, sizes));
    ASSERT_TRUE(layout.has_value());
...
TEST(VideoFrameLayout, EqualOperator)
{
    ...
    auto layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes), align, modifier);
    ASSERT_TRUE(layout.has_value());

    auto same_layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes), align, modifier);
    ASSERT_TRUE(same_layout.has_value());
    EXPECT_EQ(*layout, *same_layout);

    std::vector<size_t> another_sizes = { 190, 100, 100 };
    auto different_layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, another_sizes), align, modifier);
    ASSERT_TRUE(different_layout.has_value());
    EXPECT_NE(*layout, *different_layout);

    different_layout = VideoFrameLayout::CreateMultiPlanar(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes), align, modifier);
    ASSERT_TRUE(different_layout.has_value());
    EXPECT_NE(*layout, *different_layout);

    const size_t another_align = 0x1000;
    different_layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes), another_align, modifier);
    ASSERT_TRUE(different_layout.has_value());
    EXPECT_NE(*layout, *different_layout);

    const size_t another_modifier = 2;
    different_layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, CreatePlanes(strides, offsets, sizes), align, another_modifier);
    ASSERT_TRUE(different_layout.has_value());
    EXPECT_NE(*layout, *different_layout);
}
...
TEST(VideoFrameLayout, FitsInContiguousBufferOfSize)
{
    ...
    EXPECT_TRUE(layout->FitsInContiguousBufferOfSize(sizes[0] + sizes[1] + sizes[2]));
    ...
    ASSERT_TRUE(layout.has_value());
    EXPECT_TRUE(layout->FitsInContiguousBufferOfSize(sizes[0] + sizes[1] + sizes[2] + 1));
    EXPECT_FALSE(layout->FitsInContiguousBufferOfSize(sizes[0]));

    // Validate overflow.
    planes[0].offset = std::numeric_limits<size_t>::max() / 2 + 2;
    planes[0].size = planes[1].size = planes[2].size = std::numeric_limits<size_t>::max() / 2;
    layout = VideoFrameLayout::CreateWithPlanes(PIXEL_FORMAT_I420, coded_size, planes);
    ASSERT_TRUE(layout.has_value());
    EXPECT_FALSE(layout->FitsInContiguousBufferOfSize(std::numeric_limits<size_t>::max()));
}
