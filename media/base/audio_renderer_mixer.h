...
class MEDIA_EXPORT AudioRendererMixer : public AudioRendererSink::RenderCallback {
public:
    AudioRendererMixer(const AudioParameters& output_params, scoped_refptr<AudioRendererSink> sink);
    ...
    void AddMixerInput(const AudioParameters& input_params, AudioConverter::InputCallback* input);
    void RemoveMixerInput(const AudioParameters& input_params, AudioConverter::InputCallback* input);
...
private:
    ...
    int Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, AudioBus* audio_bus) override;
    ...
    using AudioConvertersMap = base::flat_map<int, std::unique_ptr<LoopbackAudioConverter>>;
