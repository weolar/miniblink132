...
public:
    ...
    TextRenderer(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const AddTextTrackCB& add_text_track_cb);
...
private:
    ...
    void BufferReady(DemuxerStream* text_stream, DemuxerStream::Status status, scoped_refptr<DecoderBuffer> input);

    // Dispatches the decoded cue delivered on the demuxer's |text_stream|.
    void CueReady(DemuxerStream* text_stream, const scoped_refptr<TextCue>& text_cue);

    // Dispatched when the AddTextTrackCB completes, after having created
    // the TextTrack object associated with |text_stream|.
    void OnAddTextTrackDone(DemuxerStream* text_stream, std::unique_ptr<TextTrack> text_track);
    ...
    std::map<DemuxerStream*, std::unique_ptr<TextTrackState>> text_track_state_map_;
