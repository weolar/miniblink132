...
int FakeAudioRenderCallback::Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, const AudioGlitchInfo& glitch_info, AudioBus* audio_bus)
{
    cumulative_glitch_info_ += glitch_info;
    return RenderInternal(audio_bus, delay, volume_);
}

double FakeAudioRenderCallback::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed, const AudioGlitchInfo& glitch_info)
{
    cumulative_glitch_info_ += glitch_info;
...
int FakeAudioRenderCallback::RenderInternal(AudioBus* audio_bus, base::TimeDelta delay, double volume)
{
    ...
    if (needs_fade_in_) {
        ...
        const int fade_in_frames = std::min(static_cast<int>(AudioTimestampHelper::TimeToFrames(kFadeInDuration, sample_rate_)), audio_bus->frames());
    ...
    for (int i = 1; i < audio_bus->channels(); ++i) {
        memcpy(audio_bus->channel(i), audio_bus->channel(0), number_of_frames * sizeof(*audio_bus->channel(i)));
    }
