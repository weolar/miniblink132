...
protected:
    void SetUp() override
    {
        ...
        offloading_encoder_ = std::make_unique<OffloadingAudioEncoder>(std::move(mock_audio_encoder), work_runner_, callback_runner_);
        EXPECT_CALL(*mock_audio_encoder_, OnDestruct()).WillOnce(Invoke([work_runner = work_runner_]() {
...
TEST_F(OffloadingAudioEncoderTest, Initialize)
{
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer, std::optional<AudioEncoder::CodecDescription>) {
        EXPECT_TRUE(callback_runner_->RunsTasksInCurrentSequence());
        called_output = true;
    });
    AudioEncoder::EncoderStatusCB done_cb = base::BindLambdaForTesting([&](EncoderStatus s) {
        EXPECT_TRUE(callback_runner_->RunsTasksInCurrentSequence());
        called_done = true;
    });
    ...
    EXPECT_CALL(*mock_audio_encoder_, Initialize(_, _, _))
        .WillOnce(Invoke([this](const AudioEncoder::Options& options, AudioEncoder::OutputCB output_cb, AudioEncoder::EncoderStatusCB done_cb) {
            ...
            EncodedAudioBuffer buf(params, base::HeapArray<uint8_t>(), base::TimeTicks());
    ...
    offloading_encoder_->Initialize(options, std::move(output_cb), std::move(done_cb));
    RunLoop();
...
TEST_F(OffloadingAudioEncoderTest, Encode)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Encode(_, _, _))
        .WillOnce(Invoke([this](std::unique_ptr<AudioBus> audio_bus, base::TimeTicks capture_time, AudioEncoder::EncoderStatusCB done_cb) {
            EXPECT_TRUE(work_runner_->RunsTasksInCurrentSequence());
            std::move(done_cb).Run(EncoderStatus::Codes::kOk);
        }));
...
TEST_F(OffloadingAudioEncoderTest, Flush)
{
    ...
    EXPECT_CALL(*mock_audio_encoder_, Flush(_)).WillOnce(Invoke([this](AudioEncoder::EncoderStatusCB done_cb) {
        EXPECT_TRUE(work_runner_->RunsTasksInCurrentSequence());
        std::move(done_cb).Run(EncoderStatus::Codes::kOk);
    }));
