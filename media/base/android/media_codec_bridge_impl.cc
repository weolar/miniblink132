...
bool GetCodecSpecificDataForAudio(const AudioDecoderConfig& config, CodecSpecificData* output_csd0, CodecSpecificData* output_csd1,
    CodecSpecificData* output_csd2, bool* output_frame_has_adts_header)
{
    ...
    const int64_t seek_preroll_ns = config.seek_preroll().InMicroseconds() * base::Time::kNanosecondsPerMicrosecond;
    ...
    *output_frame_has_adts_header = false;
    ...
    case AudioCodec::kVorbis: {
        ...
        output_csd1->assign(extra_data + total_length, extra_data + extra_data_size);
        break;
    }
    case AudioCodec::kFLAC: {
        ...
        // STREAMINFO bytes.
        output_csd0->insert(output_csd0->end(), config.extra_data().begin(), config.extra_data().end());
        break;
    }
    case AudioCodec::kAAC: {
        ...
        *output_frame_has_adts_header = config.profile() != AudioCodecProfile::kXHE_AAC;
        break;
    }
    case AudioCodec::kOpus: {
        if (config.extra_data().empty() || codec_delay_ns < 0 || seek_preroll_ns < 0) {
        ...
        *output_csd0 = config.extra_data();
        ...
        // csd1 - Codec Delay
        const uint8_t* codec_delay_ns_ptr = reinterpret_cast<const uint8_t*>(&codec_delay_ns);
        output_csd1->assign(codec_delay_ns_ptr, codec_delay_ns_ptr + sizeof(int64_t));

        // csd2 - Seek Preroll
        const uint8_t* seek_preroll_ns_ptr = reinterpret_cast<const uint8_t*>(&seek_preroll_ns);
        output_csd2->assign(seek_preroll_ns_ptr, seek_preroll_ns_ptr + sizeof(int64_t));
        break;
    }
    default:
        if (config.extra_data().empty()) {
        ...
        LOG(ERROR) << "Unsupported audio codec encountered: " << GetCodecName(config.codec());
        return false;
    }
    return true;
}
...
std::unique_ptr<MediaCodecBridge> MediaCodecBridgeImpl::CreateAudioDecoder(
    const AudioDecoderConfig& config, const JavaRef<jobject>& media_crypto, base::RepeatingClosure on_buffers_available_cb)
{
    DVLOG(2) << __func__ << ": " << config.AsHumanReadableString() << " media_crypto:" << media_crypto.obj();

    const std::string mime = MediaCodecUtil::CodecToAndroidMimeType(config.codec(), config.target_output_sample_format());
    ...
    if (!GetCodecSpecificDataForAudio(config, &csd0, &csd1, &csd2, &output_frame_has_adts_header)) {
        return nullptr;
    }
    ...
    ScopedJavaGlobalRef<jobject> j_bridge(Java_MediaCodecBridgeBuilder_createAudioDecoder(env, j_mime, media_crypto, config.samples_per_second(), channel_count,
        j_csd0, j_csd1, j_csd2, output_frame_has_adts_header, !!on_buffers_available_cb));

    if (j_bridge.is_null()) {
        return nullptr;
    }

    return base::WrapUnique(new MediaCodecBridgeImpl(CodecType::kAny, std::nullopt, std::move(j_bridge), std::move(on_buffers_available_cb)));
}

// static
std::unique_ptr<MediaCodecBridge> MediaCodecBridgeImpl::CreateVideoDecoder(const VideoCodecConfig& config)
{
    ...
    if (config.hdr_metadata.has_value()) {
        jni_hdr_metadata = std::make_unique<JniHdrMetadata>(config.container_color_space, config.hdr_metadata.value());
    }
    ...
    ScopedJavaGlobalRef<jobject> j_bridge(Java_MediaCodecBridgeBuilder_createVideoDecoder(env, j_mime, static_cast<int>(config.codec_type), config.media_crypto,
        config.initial_expected_coded_size.width(), config.initial_expected_coded_size.height(), config.surface, j_csd0, j_csd1, j_hdr_metadata,
        /*allowAdaptivePlayback=*/true,
        /*useAsyncApi=*/!!config.on_buffers_available_cb,
        /*useBlockModel=*/config.use_block_model, j_decoder_name, config.profile));
    if (j_bridge.is_null()) {
        return nullptr;
    }

    return base::WrapUnique(new MediaCodecBridgeImpl(config.codec_type, config.codec, std::move(j_bridge), config.on_buffers_available_cb));
}

// static
std::unique_ptr<MediaCodecBridge> MediaCodecBridgeImpl::CreateVideoEncoder(
    VideoCodec codec, const gfx::Size& size, int bit_rate, int frame_rate, int i_frame_interval, int color_format)
{
    ...
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_mime = ConvertUTF8ToJavaString(env, mime);
    ScopedJavaGlobalRef<jobject> j_bridge(Java_MediaCodecBridgeBuilder_createVideoEncoder(
        env, j_mime, size.width(), size.height(), kBitrateModeCBR, bit_rate, frame_rate, i_frame_interval, color_format));

    if (j_bridge.is_null()) {
        return nullptr;
    }

    return base::WrapUnique(new MediaCodecBridgeImpl(CodecType::kAny, std::nullopt, std::move(j_bridge)));
}
...
MediaCodecBridgeImpl::MediaCodecBridgeImpl(
    CodecType codec_type, std::optional<VideoCodec> video_decoder_codec, ScopedJavaGlobalRef<jobject> j_bridge, base::RepeatingClosure on_buffers_available_cb)
    ...
    , j_bridge_(std::move(j_bridge))
{
    ...
    Java_MediaCodecBridge_setBuffersAvailableListener(AttachCurrentThread(), j_bridge_, reinterpret_cast<intptr_t>(this));
}
...
{
    JNIEnv* env = AttachCurrentThread();
    MediaCodecStatus status = static_cast<MediaCodecStatus>(Java_MediaCodecBridge_flush(env, j_bridge_));
...
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_getOutputFormat(env, j_bridge_);
    if (!result) {
        return { MediaCodecResult::Codes::kError, "Unable to get output format." };
    }

    size->SetSize(Java_MediaFormatWrapper_width(env, result), Java_MediaFormatWrapper_height(env, result));
    return OkStatus();
}

MediaCodecResult MediaCodecBridgeImpl::GetOutputSamplingRate(int* sampling_rate)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_getOutputFormat(env, j_bridge_);
    if (!result) {
...
MediaCodecResult MediaCodecBridgeImpl::GetOutputChannelCount(int* channel_count)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_getOutputFormat(env, j_bridge_);
    if (!result) {
...
MediaCodecResult MediaCodecBridgeImpl::GetOutputColorSpace(gfx::ColorSpace* color_space)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_getOutputFormat(env, j_bridge_);
    if (!result) {
    ...
    case 6: // MediaFormat.COLOR_STANDARD_BT2020
        ...
        break;
    default:
        DVLOG(3) << __func__ << ": unsupported primary in p: " << standard << " r: " << range << " t: " << transfer;
        return { MediaCodecResult::Codes::kError, "Unexpected MediaFormat.COLOR_STANDARD of " + base::NumberToString(standard) + " specified." };
    }
    ...
    case 7: // MediaFormat.COLOR_TRANSFER_HLG
        ...
        break;
    default:
        DVLOG(3) << __func__ << ": unsupported transfer in p: " << standard << " r: " << range << " t: " << transfer;
        return { MediaCodecResult::Codes::kError, "Unexpected MediaFormat.COLOR_TRANSFER of " + base::NumberToString(transfer) + " specified." };
    }
    ...
    case 2: // MediaFormat.COLOR_RANGE_LIMITED
        ...
        break;
    default:
        DVLOG(3) << __func__ << ": unsupported range in p: " << standard << " r: " << range << " t: " << transfer;
        return { MediaCodecResult::Codes::kError, "Unexpected MediaFormat.COLOR_RANGE of " + base::NumberToString(range) + " specified." };
    }
...
MediaCodecResult MediaCodecBridgeImpl::GetInputFormat(int* stride, int* slice_height, gfx::Size* encoded_size)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_getInputFormat(env, j_bridge_);
    if (!result) {
    ...
    *encoded_size = gfx::Size(Java_MediaFormatWrapper_width(env, result), Java_MediaFormatWrapper_height(env, result));
    return OkStatus();
}
MediaCodecResult MediaCodecBridgeImpl::QueueInputBuffer(int index, base::span<const uint8_t> data, base::TimeDelta presentation_time)
{
...
MediaCodecResult MediaCodecBridgeImpl::QueueFilledInputBuffer(int index, size_t data_size, base::TimeDelta presentation_time)
{
    DVLOG(3) << __func__ << " " << index << ": " << data_size;
    JNIEnv* env = AttachCurrentThread();
    auto status
        = static_cast<MediaCodecStatus>(Java_MediaCodecBridge_queueInputBuffer(env, j_bridge_, index, 0, data_size, presentation_time.InMicroseconds(), 0));
    ReportAnyErrorToUMA(status);
    return { ConvertToMediaCodecEnum(status), ApplyDescriptiveMessage(status) };
}

MediaCodecResult MediaCodecBridgeImpl::QueueInputBlock(int index, base::span<const uint8_t> data, base::TimeDelta presentation_time, bool is_eos)
{
    DVLOG(3) << __func__ << " " << index << ": " << data.size();
    CHECK_LE(data.size(), size_t { std::numeric_limits<int32_t>::max() });

    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> j_result = Java_MediaCodecBridge_obtainBlock(env, j_bridge_, data.size());
    ScopedJavaLocalRef<jobject> j_block = Java_ObtainBlockResult_block(env, j_result);
    ScopedJavaLocalRef<jobject> j_buffer = Java_ObtainBlockResult_buffer(env, j_result);
    if (j_buffer.is_null()) {
    ...
    if (!data.empty()) {
        base::android::JavaByteBufferToMutableSpan(env, j_buffer.obj()).copy_from_nonoverlapping(data);
    }

    MediaCodecStatus status = static_cast<MediaCodecStatus>(Java_MediaCodecBridge_queueInputBlock(
        env, j_bridge_, index, j_block, 0, data.size(), presentation_time.InMicroseconds(), is_eos ? kBufferFlagEndOfStream : 0));
    Java_ObtainBlockResult_recycle(env, j_result);
    ReportAnyErrorToUMA(status);
    return { ConvertToMediaCodecEnum(status), ApplyDescriptiveMessage(status) };
}

MediaCodecResult MediaCodecBridgeImpl::QueueSecureInputBuffer(int index, base::span<const uint8_t> data, const std::string& key_id, const std::string& iv,
    const std::vector<SubsampleEntry>& subsamples, EncryptionScheme encryption_scheme, std::optional<EncryptionPattern> encryption_pattern,
    base::TimeDelta presentation_time)
{
    ...
    if (subsamples.empty()) {
        ...
        for (size_t i = 0; i < subsamples.size(); ++i) {
            ...
            if (subsamples[i].cypher_bytes > static_cast<uint32_t>(std::numeric_limits<jint>::max())) {
                return { MediaCodecResult::Codes::kError, "Subsample size is too large." };
            }
    ...
    ScopedJavaLocalRef<jintArray> clear_array = base::android::ToJavaIntArray(env, native_clear_array);
    ScopedJavaLocalRef<jintArray> cypher_array = base::android::ToJavaIntArray(env, native_cypher_array);

    MediaCodecStatus status = static_cast<MediaCodecStatus>(Java_MediaCodecBridge_queueSecureInputBuffer(env, j_bridge_, index, 0, j_iv, j_key_id, clear_array,
        cypher_array, num_subsamples, static_cast<int>(encryption_scheme), static_cast<int>(encryption_pattern ? encryption_pattern->crypt_byte_block() : 0),
        static_cast<int>(encryption_pattern ? encryption_pattern->skip_byte_block() : 0), presentation_time.InMicroseconds()));
    ReportAnyErrorToUMA(status);
    return { ConvertToMediaCodecEnum(status), ApplyDescriptiveMessage(status) };
}
...
void MediaCodecBridgeImpl::QueueEOS(int input_buffer_index)
{
    ...
    JNIEnv* env = AttachCurrentThread();
    Java_MediaCodecBridge_queueInputBuffer(env, j_bridge_, input_buffer_index, 0, 0, 0, kBufferFlagEndOfStream);
}

MediaCodecResult MediaCodecBridgeImpl::DequeueInputBuffer(base::TimeDelta timeout, int* index)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_dequeueInputBuffer(env, j_bridge_, timeout.InMicroseconds());
    *index = Java_DequeueInputResult_index(env, result);
    MediaCodecStatus status = static_cast<MediaCodecStatus>(Java_DequeueInputResult_status(env, result));
...
MediaCodecResult MediaCodecBridgeImpl::DequeueOutputBuffer(
    base::TimeDelta timeout, int* index, size_t* offset, size_t* size, base::TimeDelta* presentation_time, bool* end_of_stream, bool* key_frame)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> result = Java_MediaCodecBridge_dequeueOutputBuffer(env, j_bridge_, timeout.InMicroseconds());
    ...
    *size = base::checked_cast<size_t>(Java_DequeueOutputResult_numBytes(env, result));
    if (presentation_time) {
        *presentation_time = base::Microseconds(Java_DequeueOutputResult_presentationTimeMicroseconds(env, result));
    }
    ...
    MediaCodecStatus status = static_cast<MediaCodecStatus>(Java_DequeueOutputResult_status(env, result));
    DVLOG(3) << __func__ << ": status: " << status << ", index: " << *index << ", offset: " << *offset << ", size: " << *size << ", flags: " << flags;
    ReportAnyErrorToUMA(status);
    return { ConvertToMediaCodecEnum(status), ApplyDescriptiveMessage(status) };
}
...
base::span<uint8_t> MediaCodecBridgeImpl::GetInputBuffer(int input_buffer_index)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> j_buffer(Java_MediaCodecBridge_getInputBuffer(env, j_bridge_, input_buffer_index));
    return j_buffer.is_null() ? base::span<uint8_t>() : base::android::JavaByteBufferToMutableSpan(env, j_buffer.obj());
}

MediaCodecResult MediaCodecBridgeImpl::CopyFromOutputBuffer(int index, size_t offset, base::span<uint8_t> dst)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jobject> j_buffer(Java_MediaCodecBridge_getOutputBuffer(env, j_bridge_, index));
    if (j_buffer.is_null()) {
...
void MediaCodecBridgeImpl::OnBuffersAvailable(JNIEnv* /* env */, const base::android::JavaParamRef<jobject>& /* obj */)
{
...
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jstring> j_name = Java_MediaCodecBridge_getName(env, j_bridge_);
...
bool MediaCodecBridgeImpl::FillInputBuffer(int index, base::span<const uint8_t> data)
{
    ...
    if (data.size() > dst.size()) {
        LOG(ERROR) << "Input buffer size " << data.size() << " exceeds MediaCodec input buffer capacity: " << dst.size();
        return false;
    }
...
void MediaCodecBridgeImpl::ReportAnyErrorToUMA(MediaCodecStatus status)
{
    ...
    std::string uma_name
        = "Media.MediaCodecError." + GetCodecNameForUMA(video_decoder_codec_.value()) + (IsSoftwareCodec() ? ".SoftwareSecure" : ".HardwareSecure");
    base::UmaHistogramExactLinear(uma_name, status, MediaCodecStatus::MEDIA_CODEC_MAX + 1);
}
