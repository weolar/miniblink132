...
public:
    ...
    virtual void Initialize(
        const base::RepeatingClosure& received_frame_cb, scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner, StreamTextureWrapperInitCB init_cb)
        = 0;
    ...
    // See StreamTextureWrapperImpl.
    virtual void ForwardStreamTextureForSurfaceRequest(const base::UnguessableToken& request_token) = 0;
    ...
    struct Deleter {
        inline void operator()(StreamTextureWrapper* ptr) const
        {
            ptr->Destroy();
        }
    };
...
typedef std::unique_ptr<StreamTextureWrapper, StreamTextureWrapper::Deleter> ScopedStreamTextureWrapper;
