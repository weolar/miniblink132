...
public:
    MediaServiceThrottlerTest()
    {
        ...
        test_task_runner_ = base::MakeRefCounted<FakeSingleThreadTaskRunner>(&clock_);
    ...
    MediaServiceThrottlerTest& operator=(const MediaServiceThrottlerTest&) = delete;
...
TEST_F(MediaServiceThrottlerTest, NoCrash_OverBurstThreshold_ShouldDelay)
{
    ...
    // slot.
    EXPECT_EQ(base_delay_ * (kMaxBurstClients + 1), throttler_->GetDelayForClientCreation());
}

// Makes sure that clients are scheduled |base_delay| apart.
TEST_F(MediaServiceThrottlerTest, NoCrash_OverBurstThreshold_ShouldDelayLinearly)
{
    // Schedule clients until the next one would be over burst threshold.
    SimulateClientCreations(kMaxBurstClients);
...
TEST_F(MediaServiceThrottlerTest, NoCrash_BurstThreshold_ShouldBeSlidingWindow)
{
...
// new clients.
TEST_F(MediaServiceThrottlerTest, NoCrash_OverBurstThresholdWithTimeLapse_ShouldDelay)
{
...
TEST_F(MediaServiceThrottlerTest, WithCrash_SingleCrash_DelayShouldNotIncrease)
{
    // Schedule clients until the next one would be over burst threshold.
    SimulateClientCreations(kMaxBurstClients);
...
TEST_F(MediaServiceThrottlerTest, WithCrash_ConsecutiveCrashes_DelayShouldIncreaseWithNumberOfCrashes)
{
    // Schedule clients until the next one would be over burst threshold.
    SimulateClientCreations(kMaxBurstClients);
...
TEST_F(MediaServiceThrottlerTest, CrashListener_NoRequestsWithCrashes_ShouldShutDown)
{
    // Schedule many minutes worth of clients. This is to prove that the
    // MediaServerCrashListener's clean up happens after lack of requests, as
    // opposed to lack of actually scheduled clients.
