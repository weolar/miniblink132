...
MediaPlayerBridge::MediaPlayerBridge(const GURL& url, const net::SiteForCookies& site_for_cookies, const url::Origin& top_frame_origin,
    net::StorageAccessApiStatus storage_access_api_status, const std::string& user_agent, bool hide_url_log, Client* client, bool allow_credentials,
    bool is_hls, const base::flat_map<std::string, std::string> headers)
    : prepared_(false)
    ...
    , is_hls_(is_hls)
    , watch_timer_(base::BindRepeating(&MediaPlayerBridge::OnWatchTimerTick, base::Unretained(this)),
          base::BindRepeating(&MediaPlayerBridge::GetCurrentTime, base::Unretained(this)))
    , headers_(std::move(headers))
    , client_(client)
{
    listener_ = std::make_unique<MediaPlayerListener>(base::SingleThreadTaskRunner::GetCurrentDefault(), weak_factory_.GetWeakPtr());
}
...
MediaPlayerBridge::~MediaPlayerBridge()
{
    ...
    if (has_ever_started_) {
        UMA_HISTOGRAM_ENUMERATION("Media.Android.MediaPlayerSuccess", has_error_ ? UMA_EXIT_ERROR : UMA_EXIT_SUCCESS, UMA_EXIT_STATUS_MAX + 1);
    }
}
...
void MediaPlayerBridge::Initialize()
{
    ...
    if (allow_credentials_ && !url_.SchemeIsFile()) {
        media::MediaResourceGetter* resource_getter = client_->GetMediaResourceGetter();

        pending_retrieve_cookies_ = true;
        resource_getter->GetCookies(url_, site_for_cookies_, top_frame_origin_, storage_access_api_status_,
            base::BindOnce(&MediaPlayerBridge::OnCookiesRetrieved, weak_factory_.GetWeakPtr()));
    }
}
...
void MediaPlayerBridge::CreateJavaMediaPlayerBridge()
{
    ...
    j_media_player_bridge_.Reset(Java_MediaPlayerBridge_create(env, reinterpret_cast<intptr_t>(this)));
...
void MediaPlayerBridge::SetVideoSurface(gl::ScopedJavaSurface surface)
{
    ...
    Java_MediaPlayerBridge_setSurface(env, j_media_player_bridge_, surface_.j_surface());
}
...
void MediaPlayerBridge::SetPlaybackRate(double playback_rate)
{
    ...
    Java_MediaPlayerBridge_setPlaybackRate(env, j_media_player_bridge_, playback_rate);
}
...
void MediaPlayerBridge::SetDataSource(const std::string& url)
{
    ...
    if (InterceptMediaUrl(url, &fd, &offset, &size)) {
        if (!Java_MediaPlayerBridge_setDataSourceFromFd(env, j_media_player_bridge_, fd, offset, size)) {
    ...
    if (base::StartsWith(url, data_uri_prefix, base::CompareCase::SENSITIVE)) {
        if (!Java_MediaPlayerBridge_setDataUriDataSource(env, j_media_player_bridge_, j_url_string)) {
            OnMediaError(MEDIA_ERROR_FORMAT);
        }
        return;
    }
...
void MediaPlayerBridge::SetDataSourceInternal()
{
    ...
    ScopedJavaLocalRef<jstring> j_cookies = ConvertUTF8ToJavaString(env, cookies_);
    ScopedJavaLocalRef<jstring> j_user_agent = ConvertUTF8ToJavaString(env, user_agent_);
    ScopedJavaLocalRef<jstring> j_url_string = ConvertUTF8ToJavaString(env, url_.spec());
    ...
    if (!Java_MediaPlayerBridge_setDataSource(env, j_media_player_bridge_, j_url_string, j_cookies, j_user_agent, hide_url_log_, scoped_hash_map)) {
        OnMediaError(MEDIA_ERROR_FORMAT);
        return;
    }
...
bool MediaPlayerBridge::InterceptMediaUrl(const std::string& url, int* fd, int64_t* offset, int64_t* size)
{
    ...
    *offset = kUnsetValue;
    *size = kUnsetValue;
    media::MediaUrlInterceptor* url_interceptor = client_->GetMediaUrlInterceptor();
...
void MediaPlayerBridge::OnDidSetDataUriDataSource(JNIEnv* env, const JavaParamRef<jobject>& obj, jboolean success)
{
    if (!success) {
        OnMediaError(MEDIA_ERROR_FORMAT);
        return;
...
void MediaPlayerBridge::OnCookiesRetrieved(const std::string& cookies)
{
    cookies_ = cookies;
    pending_retrieve_cookies_ = false;
    client_->GetMediaResourceGetter()->GetAuthCredentials(url_, base::BindOnce(&MediaPlayerBridge::OnAuthCredentialsRetrieved, weak_factory_.GetWeakPtr()));
...
void MediaPlayerBridge::OnAuthCredentialsRetrieved(const std::u16string& username, const std::u16string& password)
{
...
void MediaPlayerBridge::Start()
{
    ...
    if (has_ever_started_ && has_error_) {
        UMA_HISTOGRAM_ENUMERATION("Media.Android.MediaPlayerSuccess", UMA_EXIT_ERROR, UMA_EXIT_STATUS_MAX + 1);
    }
...
base::TimeDelta MediaPlayerBridge::GetCurrentTime()
{
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    return base::Milliseconds(Java_MediaPlayerBridge_getCurrentPosition(env, j_media_player_bridge_));
}
...
base::TimeDelta MediaPlayerBridge::GetDuration()
{
    ...
    return duration_ms < 0 ? media::kInfiniteDuration : base::Milliseconds(duration_ms);
}
...
ScopedJavaLocalRef<jobject> MediaPlayerBridge::GetAllowedOperations()
{
    ...
    return Java_MediaPlayerBridge_getAllowedOperations(env, j_media_player_bridge_);
}
