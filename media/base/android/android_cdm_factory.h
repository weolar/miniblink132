...
public:
    AndroidCdmFactory(CreateFetcherCB create_fetcher_cb, CreateStorageCB create_storage_cb);
...
private:
    ...
    void OnCdmCreated(uint32_t creation_id, const scoped_refptr<ContentDecryptionModule>& cdm, CreateCdmStatus status);
    ...
    using PendingCreation = std::pair<std::unique_ptr<MediaDrmBridgeFactory>, CdmCreatedCB>;
