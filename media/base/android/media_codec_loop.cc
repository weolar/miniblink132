...
MediaCodecLoop::MediaCodecLoop(int sdk_int, Client* client, std::unique_ptr<MediaCodecBridge> media_codec,
    scoped_refptr<base::SingleThreadTaskRunner> timer_task_runner, bool disable_timer)
    : state_(STATE_READY)
    , client_(client)
...
MediaCodecLoop::InputBuffer MediaCodecLoop::DequeueInputBuffer()
{
    ...
    MediaCodecResult result = media_codec_->DequeueInputBuffer(kNoWaitTimeout, &input_buf_index);
    switch (result.code()) {
    ...
    default:
        NOTREACHED() << "Unexpected DequeueInputBuffer result: " << result.message();
    }
...
void MediaCodecLoop::EnqueueInputBuffer(const InputBuffer& input_buffer)
{
    ...
    if (input_data.encryption_scheme != EncryptionScheme::kUnencrypted) {
        ...
        result = media_codec_->QueueSecureInputBuffer(input_buffer.index, input_data.memory, input_data.key_id, input_data.iv, input_data.subsamples,
            input_data.encryption_scheme, input_data.encryption_pattern, input_data.presentation_time);

    } else {
        result = media_codec_->QueueInputBuffer(input_buffer.index, input_data.memory, input_data.presentation_time);
    }

    switch (result.code()) {
    case MediaCodecResult::Codes::kError:
        DLOG(ERROR) << __func__ << ": MediaCodecResult::Codes::kError from QueueInputBuffer : " << result.message();
    ...
    default:
        NOTREACHED() << "Unknown Queue(Secure)InputBuffer status " << result.message();
    }
}
...
bool MediaCodecLoop::ProcessOneOutputBuffer()
{
    ...
    OutputBuffer out;
    MediaCodecResult result = media_codec_->DequeueOutputBuffer(kNoWaitTimeout, &out.index, &out.offset, &out.size, &out.pts, &out.is_eos, &out.is_key_frame);
    ...
    default:
        NOTREACHED() << "Unexpected DequeueOutputBuffer result: " << result.message();
    }
...
void MediaCodecLoop::ManageTimer(bool did_work)
{
    ...
    base::TimeTicks now = (test_tick_clock_ ? test_tick_clock_->NowTicks() : base::TimeTicks::Now());
    ...
    if (should_be_running && !io_timer_.IsRunning()) {
        io_timer_.Start(FROM_HERE, kDecodePollDelay, this, &MediaCodecLoop::DoPendingWork);
