...
public:
    ...
    static std::string CodecToAndroidMimeType(AudioCodec codec, SampleFormat sample_format);
    ...
    static std::optional<gfx::Size> LookupCodedSizeAlignment(std::string_view name, std::optional<int> host_sdk_int = std::nullopt);
    ...
    // WARNING: This can't be used from the renderer process since it attempts to
    // access MediaCodecList (which requires permissions).
    static void AddSupportedCodecProfileLevels(std::vector<CodecProfileLevel>* out);
    ...
    // WARNING: This can't be used from the renderer process since it attempts to
    // create a MediaCodec (which requires permissions) to get the codec name.
    static bool IsKnownUnaccelerated(VideoCodec codec, MediaCodecDirection direction);
};
