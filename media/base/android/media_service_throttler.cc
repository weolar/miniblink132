...
constexpr auto kLinearThrottlingDelay = base::Seconds(0.2) - kBaseExponentialDelay;
...
MediaServiceThrottler::MediaServiceThrottler()
    ...
    , current_crashes_(0)
    , crash_listener_task_runner_(base::SingleThreadTaskRunner::GetCurrentDefault())
{
    // base::Unretained is safe because the MediaServiceThrottler is supposed to
    // live until the process dies.
    release_crash_listener_cb_ = base::BindRepeating(&MediaServiceThrottler::ReleaseCrashListener, base::Unretained(this));
    EnsureCrashListenerStarted();
}

void MediaServiceThrottler::SetTickClockForTesting(const base::TickClock* clock)
{
    clock_ = clock;
}
...
base::TimeDelta MediaServiceThrottler::GetDelayForClientCreation()
{
    ...
    if (delay <= (kLinearThrottlingDelay + kBaseExponentialDelay) * kMaxBurstClients)
...
base::TimeDelta MediaServiceThrottler::GetThrottlingDelayFromServerCrashes()
{
    ...
    return std::min(kBaseExponentialDelay * (1 << num_crashes), kMaxExponentialDelay);
}
...
void MediaServiceThrottler::UpdateServerCrashes()
{
    ...
    if (time_since_last_crash > kTimeUntilCrashReset) {
        ...
        const double decay = (now - last_current_crash_update_time_) / base::Minutes(1);
...
void MediaServiceThrottler::EnsureCrashListenerStarted()
{
    if (!crash_listener_) {
        ...
        crash_listener_ = std::make_unique<MediaServerCrashListener>(
            base::BindRepeating(&MediaServiceThrottler::OnMediaServerCrash, base::Unretained(this)), crash_listener_task_runner_);
    } else {
    ...
    crash_listener_task_runner_->PostDelayedTask(FROM_HERE, cancelable_release_crash_listener_cb_.callback(), kReleaseInactivityDelay);
}
...
void MediaServiceThrottler::SetCrashListenerTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> crash_listener_task_runner)
{
    ...
    crash_listener_ = std::make_unique<MediaServerCrashListener>(base::NullCallback(), crash_listener_task_runner_);
}
