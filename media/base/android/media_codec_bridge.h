...
public:
    ...
    virtual MediaCodecResult GetOutputColorSpace(gfx::ColorSpace* color_space) = 0;
    ...
    virtual MediaCodecResult GetInputFormat(int* stride, int* slice_height, gfx::Size* encoded_size) = 0;

    // Submits a byte array to the given input buffer. Call this after getting an
    // available buffer from DequeueInputBuffer(). `data` will be copied into the
    // input buffer.
    virtual MediaCodecResult QueueInputBuffer(int index, base::span<const uint8_t> data, base::TimeDelta presentation_time) = 0;
    // Similar to QueueInputBuffer() but submits the input buffer referenced by
    // `index` assuming it has already been filled.
    virtual MediaCodecResult QueueFilledInputBuffer(int index, size_t data_size, base::TimeDelta presentation_time) = 0;

    // Submits a byte array to the given input buffer, using LinearBlock.
    virtual MediaCodecResult QueueInputBlock(int index, base::span<const uint8_t> data, base::TimeDelta presentation_time, bool is_eos) = 0;

    // As above but for encrypted buffers. NULL |subsamples| indicates the
    // whole buffer is encrypted.
    virtual MediaCodecResult QueueSecureInputBuffer(int index, base::span<const uint8_t> data, const std::string& key_id, const std::string& iv,
    ...
    virtual MediaCodecResult DequeueInputBuffer(base::TimeDelta timeout, int* index) = 0;
    ...
    // otherwise.
    virtual MediaCodecResult CopyFromOutputBuffer(int index, size_t offset, base::span<uint8_t> dst) = 0;
    ...
    virtual void OnBuffersAvailable(JNIEnv* env, const base::android::JavaParamRef<jobject>& obj) = 0;
