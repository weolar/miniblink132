...
{
    return MediaDrmBridge::IsKeySystemSupportedWithType(key_system, container_mime_type);
}
...
public:
    ...
    void Retrieve(const GURL& default_url, const std::string& request_data, ResponseCB response_cb) override
    {
        provision_fetcher_->Retrieve(default_url, request_data, std::move(response_cb));
    }
...
public:
    ...
    void CreateWithoutSessionSupport(const std::string& key_system, const std::string& origin_id, MediaDrmBridge::SecurityLevel security_level)
    {
        media_drm_bridge_ = MediaDrmBridge::CreateWithoutSessionSupport(
            key_system, origin_id, security_level, "Test", base::BindRepeating(&MediaDrmBridgeTest::CreateProvisionFetcher, base::Unretained(this)));
    }

    void CreateWithoutSessionSupportWithNullCallback(const std::string& key_system, const std::string& origin_id, MediaDrmBridge::SecurityLevel security_level)
    {
        media_drm_bridge_ = MediaDrmBridge::CreateWithoutSessionSupport(key_system, origin_id, security_level, "Test", base::NullCallback());
    }

    // ProvisionFetcher implementation. Done as a mock method so we can properly
    // check if |media_drm_bridge_| invokes it or not.
    MOCK_METHOD(void, Retrieve, (const GURL& default_url, const std::string& request_data, ResponseCB response_cb));

    void Provision()
    {
        media_drm_bridge_->Provision(base::BindOnce(&MediaDrmBridgeTest::ProvisioningDone, base::Unretained(this)));
    }
...
protected:
    MediaDrmBridge::CdmCreationResult media_drm_bridge_ = CreateCdmTypedStatus::Codes::kUnknownError;
...
TEST_F(MediaDrmBridgeTest, IsKeySystemSupported_Widevine)
{
    ...
    EXPECT_TRUE_IF_KEY_SYSTEM_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kVideoMp4), kWidevineKeySystem);

    EXPECT_TRUE_IF_KEY_SYSTEM_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kAudioWebM), kWidevineKeySystem);
    EXPECT_TRUE_IF_KEY_SYSTEM_AVAILABLE(IsKeySystemSupportedWithType(kWidevineKeySystem, kVideoWebM), kWidevineKeySystem);
...
TEST_F(MediaDrmBridgeTest, IsKeySystemSupported_ExternalClearKey)
{
    ...
    EXPECT_FALSE(base::FeatureList::IsEnabled(media::kExternalClearKeyForTesting));
    ...
    EXPECT_TRUE_IF_KEY_SYSTEM_AVAILABLE(IsKeySystemSupportedWithType(kExternalClearKeyKeySystem, kVideoMp4), kExternalClearKeyKeySystem);
}
...
TEST_F(MediaDrmBridgeTest, CreateWithoutSessionSupport_Widevine)
{
    ...
    EXPECT_VALUE_IF_KEY_SYSTEM_AVAILABLE_OR_ERROR(media_drm_bridge_, kWidevineKeySystem, CreateCdmTypedStatus::Codes::kUnsupportedKeySystem);
}

TEST_F(MediaDrmBridgeTest, CreateWithoutSessionSupport_ExternalClearKey)
{
    CreateWithoutSessionSupport(kExternalClearKeyKeySystem, kTestOrigin, kDefault);
    EXPECT_EQ(media_drm_bridge_.code(), CreateCdmTypedStatus::Codes::kUnsupportedKeySystem);

    scoped_feature_list_.InitWithFeatures({ media::kExternalClearKeyForTesting }, {});
    CreateWithoutSessionSupport(kExternalClearKeyKeySystem, kTestOrigin, kDefault);
    EXPECT_VALUE_IF_KEY_SYSTEM_AVAILABLE_OR_ERROR(media_drm_bridge_, kExternalClearKeyKeySystem, CreateCdmTypedStatus::Codes::kUnsupportedKeySystem);
}
...
TEST_F(MediaDrmBridgeTest, Provision_Widevine)
{
    ...
    // completes.
    EXPECT_CALL(*this, CreateProvisionFetcher()).WillOnce(Return(ByMove(std::make_unique<ProvisionFetcherWrapper>(this))));
...
TEST_F(MediaDrmBridgeTest, GetStatusForPolicy_FeatureFlagDisabled)
{
    ...
    scoped_feature_list_.InitWithFeatureState(media::kMediaDrmGetStatusForPolicy, false);
    ...
    media_drm_bridge_->GetStatusForPolicy(HdcpVersion::kHdcpVersionNone,
...
TEST_F(MediaDrmBridgeTest, GetStatusForPolicy_ExternalClearKey)
{
    ...
    media_drm_bridge_->GetStatusForPolicy(HdcpVersion::kHdcpVersion1_0,
        std::make_unique<MockCdmKeyStatusPromise>(
            /*expect_success=*/true, &key_status));
    EXPECT_EQ(CdmKeyInformation::KeyStatus::OUTPUT_RESTRICTED, key_status);

    media_drm_bridge_->GetStatusForPolicy(HdcpVersion::kHdcpVersion2_3,
        std::make_unique<MockCdmKeyStatusPromise>(
            /*expect_success=*/true, &key_status));
    EXPECT_EQ(CdmKeyInformation::KeyStatus::OUTPUT_RESTRICTED, key_status);
}
