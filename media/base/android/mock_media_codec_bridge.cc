...
{
    ON_CALL(*this, DequeueInputBuffer(_, _)).WillByDefault(Return(MediaCodecResult::Codes::kTryAgainLater));
    ON_CALL(*this, DequeueOutputBuffer(_, _, _, _, _, _, _)).WillByDefault(Return(MediaCodecResult::Codes::kTryAgainLater));
}
...
void MockMediaCodecBridge::ProduceOneOutput(IsEos eos)
{
    ...
    EXPECT_CALL(*this, DequeueOutputBuffer(_, _, _, _, _, _, _))
        .WillOnce(DoAll(SetArgPointee<5>(eos == kEos ? true : false), Return(OkStatus())))
        .WillRepeatedly(Return(MediaCodecResult::Codes::kTryAgainLater));
}
...
// static
std::unique_ptr<MediaCodecBridge> MockMediaCodecBridge::CreateVideoDecoder(const VideoCodecConfig& config)
{
    return CreateMockVideoDecoder(config);
}

std::unique_ptr<MockMediaCodecBridge> MockMediaCodecBridge::CreateMockVideoDecoder(const VideoCodecConfig& config)
{
    auto bridge = std::make_unique<MockMediaCodecBridge>();
    bridge->codec_type_ = config.codec_type;
    bridge->name_ = base::StringPrintf("c2.%s.mock.codec.%s%s", config.codec_type == CodecType::kSoftware ? "android" : "google",
        GetCodecName(config.codec).c_str(), config.codec_type == CodecType::kSecure ? ".secure" : "");
