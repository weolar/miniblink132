...
protected:
    ...
    void WaitUntilIdle(IdleExpectation idleExpectation = ShouldBeIdle)
    {
        ...
        case ShouldNotBeIdle:
            ...
            // return 'no'.
            EXPECT_CALL(*client_, IsAnyInputPending()).Times(AtLeast(1)).WillRepeatedly(Return(false));
            EXPECT_CALL(Codec(), DequeueOutputBuffer(_, _, _, _, _, _, _)).Times(AtLeast(1)).WillRepeatedly(Return(MediaCodecResult::Codes::kTryAgainLater));
            break;
        }
    ...
    void ConstructCodecLoop()
    {
        ...
        codec_loop_ = std::make_unique<MediaCodecLoop>(sdk_int, client_.get(), std::move(codec), mock_task_runner_);
    ...
    void ExpectEmptyIOLoop()
    {
        ...
        EXPECT_CALL(Codec(), DequeueOutputBuffer(_, _, _, _, _, _, _)).Times(1).WillOnce(Return(MediaCodecResult::Codes::kTryAgainLater));
    }
    ...
    void ExpectDequeueInputBuffer(int input_buffer_index, MediaCodecResult status = OkStatus())
    {
        EXPECT_CALL(Codec(), DequeueInputBuffer(_, _)).WillOnce(DoAll(SetArgPointee<1>(input_buffer_index), Return(status)));
    }
    ...
    void ExpectQueueInputBuffer(int input_buffer_index, const MediaCodecLoop::InputData& data, MediaCodecResult status = OkStatus())
    {
        EXPECT_CALL(Codec(), QueueInputBuffer(input_buffer_index, data.memory, data.presentation_time)).Times(1).WillOnce(Return(status));
    }
    ...
    {
        EXPECT_CALL(Codec(), DequeueOutputBuffer(_, _, _, _, _, _, _)).WillOnce(Return(status));
    }

    void ExpectDequeueOutputBuffer(const OutputBuffer& buffer)
    {
        EXPECT_CALL(Codec(), DequeueOutputBuffer(_, _, _, _, _, _, _))
            .WillOnce(DoAll(SetArgPointee<1>(buffer.index), SetArgPointee<2>(buffer.offset), SetArgPointee<3>(buffer.size), SetArgPointee<4>(buffer.pts),
                SetArgPointee<5>(buffer.eos), SetArgPointee<6>(buffer.key_frame), Return(OkStatus())));
    }

    void ExpectOnDecodedFrame(const OutputBuffer& buf)
    {
        EXPECT_CALL(*client_, OnDecodedFrame(Field(&MediaCodecLoop::OutputBuffer::index, Eq(buf.index)))).Times(1).WillOnce(Return(true));
    }
...
public:
    ...
    scoped_refptr<base::TestMockTimeTaskRunner> mock_task_runner_ = new base::TestMockTimeTaskRunner;
...
TEST_F(MediaCodecLoopTest, TestConstructionWithNullCodec)
{
    ...
    codec_loop_ = std::make_unique<MediaCodecLoop>(sdk_int, client_.get(), std::move(codec), scoped_refptr<base::SingleThreadTaskRunner>());
...
TEST_F(MediaCodecLoopTest, TestQueueInputDataFails)
{
    ...
    ConstructCodecLoop();
    {
        ...
        // MCL should send the buffer into MediaCodec and notify the client.
        ExpectQueueInputBuffer(input_buffer_index, data, MediaCodecResult::Codes::kError);
...
TEST_F(MediaCodecLoopTest, TestOnKeyAdded)
{
    ...
    // First provide input, but have MediaCodecBridge require a key.
    {
        ...
        ExpectQueueInputBuffer(input_buffer_index, data, MediaCodecResult::Codes::kNoKey);
