...
class FakeAudioWorker::Worker : public base::RefCountedThreadSafe<FakeAudioWorker::Worker> {
public:
    Worker(const scoped_refptr<base::SequencedTaskRunner>& worker_task_runner, const AudioParameters& params);
...
FakeAudioWorker::FakeAudioWorker(const scoped_refptr<base::SequencedTaskRunner>& worker_task_runner, const AudioParameters& params)
...
// static
base::TimeDelta FakeAudioWorker::ComputeFakeOutputDelay(const AudioParameters& params)
{
    ...
    return AudioTimestampHelper::FramesToTime(params.frames_per_buffer() * kDelayFactor, params.sample_rate());
}

FakeAudioWorker::Worker::Worker(const scoped_refptr<base::SequencedTaskRunner>& worker_task_runner, const AudioParameters& params)
...
void FakeAudioWorker::Worker::Start(FakeAudioWorker::Callback worker_cb)
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Worker::DoStart, this));
}
...
void FakeAudioWorker::Worker::Stop()
{
    ...
    worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Worker::DoCancel, this));
}
...
void FakeAudioWorker::Worker::DoRead()
{
    ...
    const base::TimeTicks read_time = first_read_time_ + AudioTimestampHelper::FramesToTime(frames_elapsed_, sample_rate_);
    frames_elapsed_ += frames_per_read_;
    base::TimeTicks next_read_time = first_read_time_ + AudioTimestampHelper::FramesToTime(frames_elapsed_, sample_rate_);
    ...
    if (next_read_time <= now) {
        frames_elapsed_ = AudioTimestampHelper::TimeToFrames(now - first_read_time_, sample_rate_);
        frames_elapsed_ = ((frames_elapsed_ / frames_per_read_) + 1) * frames_per_read_;
        next_read_time = first_read_time_ + AudioTimestampHelper::FramesToTime(frames_elapsed_, sample_rate_);
    }

    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("audio"), "Worker post", "next_read_time", (next_read_time - base::TimeTicks()).InMilliseconds());
    bool posted = worker_task_runner_->PostDelayedTaskAt(
        base::subtle::PostDelayedTaskPassKey(), FROM_HERE, worker_task_cb_.callback(), next_read_time, base::subtle::DelayPolicy::kPrecise);
    // TODO(crbug.com/366102594): Revert this once we have data on when
    // FakeAudioWorker fails.
    CHECK(posted, base::NotFatalUntil::M134);
    TRACE_EVENT_END2(TRACE_DISABLED_BY_DEFAULT("audio"), "Worker::DoRead", "read_time", (read_time - base::TimeTicks()).InMilliseconds(), "now",
