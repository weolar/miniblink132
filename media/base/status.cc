...
StatusData::StatusData(StatusGroupType group, StatusCodeType code, std::string message, UKMPackedType root_cause)
    : group(group)
    , code(code)
...
{
    auto result = std::make_unique<StatusData>(group, code, message, packed_root_cause);
...
std::ostream& operator<<(std::ostream& stream, const OkStatusImplicitConstructionHelper&)
{
    stream << "kOk";
    return stream;
}
