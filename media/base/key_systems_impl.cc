...
EmeCodec ToVideoEmeCodec(VideoCodec codec, VideoCodecProfile profile)
{
    ...
    case VideoCodec::kVP9:
        ...
        if (profile == VP9PROFILE_PROFILE0 || profile == VIDEO_CODEC_PROFILE_UNKNOWN) {
            return EME_CODEC_VP9_PROFILE0;
...
public:
    std::string GetBaseKeySystemName() const final
    {
    ...
    {
        return init_data_type == EmeInitDataType::CENC || init_data_type == EmeInitDataType::WEBM || init_data_type == EmeInitDataType::KEYIDS;
    }

    EmeConfig::Rule GetEncryptionSchemeConfigRule(EncryptionScheme encryption_scheme) const final
    {
    ...
    EmeConfig::Rule GetRobustnessConfigRule(
        const std::string& key_system, EmeMediaType media_type, const std::string& requested_robustness, const bool* /*hw_secure_requirement*/) const final
    {
    ...
    bool UseAesDecryptor() const final
    {
        return true;
    }
};
...
static bool IsPotentiallySupportedKeySystem(const std::string& key_system)
{
    ...
    return base::StartsWith(key_system, kExcludedPrefix, base::CompareCase::SENSITIVE);
}
...
static bool CanBlock(const KeySystemInfo& key_system_info)
{
    ...
    if (base::FeatureList::IsEnabled(kExternalClearKeyForTesting) && IsExternalClearKey(key_system_info.GetBaseKeySystemName())) {
        return true;
    }
...
void KeySystemsImpl::UpdateSupportedKeySystems()
{
    ...
    key_system_support_registration_
        = std::move(register_key_systems_support_cb_).Run(base::BindRepeating(&KeySystemsImpl::OnSupportedKeySystemsUpdated, weak_factory_.GetWeakPtr()));
}
...
SupportedCodecs KeySystemsImpl::GetCodecMaskForMimeType(const std::string& container_mime_type) const
{
...
EmeCodec KeySystemsImpl::GetEmeCodecForString(EmeMediaType media_type, const std::string& container_mime_type, const std::string& codec_string) const
{
    ...
    if (media_type == EmeMediaType::AUDIO) {
        ...
        ParseAudioCodecString(container_mime_type, codec_string, &is_ambiguous, &audio_codec);
    ...
    DVLOG(3) << "Video codec = " << result->codec << ", profile = " << result->profile;
...
void KeySystemsImpl::ProcessSupportedKeySystems(KeySystemInfos key_systems)
{
    ...
    for (auto& key_system : key_systems) {
        ...
        DCHECK(key_system->GetPersistentStateSupport() != EmeFeatureSupport::INVALID);
        DCHECK(key_system->GetDistinctiveIdentifierSupport() != EmeFeatureSupport::INVALID);
        ...
        if (!IsPotentiallySupportedKeySystem(key_system->GetBaseKeySystemName())) {
            ...
            DLOG(ERROR) << "Unsupported name '" << key_system->GetBaseKeySystemName() << "'. See code comments.";
            continue;
        }

        // Supporting persistent state is a prerequisite for supporting persistent
        // sessions.
        if (key_system->GetPersistentStateSupport() == EmeFeatureSupport::NOT_SUPPORTED) {
            DCHECK(!key_system->GetPersistentLicenseSessionSupport().has_value());
        }

        if (!CanBlock(*key_system)) {
            DCHECK(key_system->GetDistinctiveIdentifierSupport() == EmeFeatureSupport::ALWAYS_ENABLED);
            DCHECK(key_system->GetPersistentStateSupport() == EmeFeatureSupport::ALWAYS_ENABLED);
        }
...
const KeySystemInfo* KeySystemsImpl::GetKeySystemInfo(const std::string& key_system) const
{
    DCHECK(!is_updating_);
    for (const auto& key_system_info : key_system_info_vector_) {
        const auto& base_key_system = key_system_info->GetBaseKeySystemName();
        if ((key_system == base_key_system || IsSubKeySystemOf(key_system, base_key_system)) && key_system_info->IsSupportedKeySystem(key_system)) {
...
void KeySystemsImpl::RegisterMimeType(const std::string& mime_type, SupportedCodecs codecs)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(!mime_type_to_codecs_map_.count(mime_type));
    DCHECK(IsValidMimeTypeCodecsCombination(mime_type, codecs)) << ": mime_type = " << mime_type << ", codecs = " << codecs;
...
bool KeySystemsImpl::IsValidMimeTypeCodecsCombination(const std::string& mime_type, SupportedCodecs codecs) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
bool KeySystemsImpl::IsSupportedInitDataType(const std::string& key_system, EmeInitDataType init_data_type) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
EmeConfig::Rule KeySystemsImpl::GetEncryptionSchemeConfigRule(const std::string& key_system, EncryptionScheme encryption_scheme) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void KeySystemsImpl::AddCodecMaskForTesting(EmeMediaType media_type, const std::string& codec, uint32_t mask)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void KeySystemsImpl::AddMimeTypeCodecMaskForTesting(const std::string& mime_type, uint32_t codecs_mask)
{
...
void KeySystemsImpl::ResetForTesting()
{
    ...
    is_updating_ = false;
    DCHECK(update_callbacks_.empty()) << "Should have no update callbacks for a clean test.";
    key_system_info_vector_.clear();
...
std::string KeySystemsImpl::GetBaseKeySystemName(const std::string& key_system) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
bool KeySystemsImpl::ShouldUseBaseKeySystemName(const std::string& key_system) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
EmeConfig::Rule KeySystemsImpl::GetContentTypeConfigRule(
    const std::string& key_system, EmeMediaType media_type, const std::string& container_mime_type, const std::vector<std::string>& codecs) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    case EmeMediaType::AUDIO:
        if (!base::StartsWith(container_mime_type, "audio/", base::CompareCase::SENSITIVE)) {
            return EmeConfig::UnsupportedRule();
        }
        break;
    case EmeMediaType::VIDEO:
        if (!base::StartsWith(container_mime_type, "video/", base::CompareCase::SENSITIVE)) {
            return EmeConfig::UnsupportedRule();
        }
        break;
    }
    ...
    SupportedCodecs key_system_hw_secure_codec_mask = key_system_info->GetSupportedHwSecureCodecs();
    ...
    if ((key_system_codec_mask & mime_type_codec_mask) == 0) {
        DVLOG(2) << "Container " << container_mime_type << " not supported by " << key_system;
        return EmeConfig::UnsupportedRule();
    }
    ...
    for (auto& codec_iterator : codecs) {
        EmeCodec codec = GetEmeCodecForString(media_type, container_mime_type, codec_iterator);
        ...
        if ((codec & key_system_codec_mask & mime_type_codec_mask) != codec && (codec & key_system_hw_secure_codec_mask & mime_type_codec_mask) != codec) {
            DVLOG(2) << "Container/codec pair (" << container_mime_type << " / " << codec_iterator << ") not supported by " << key_system;
            return EmeConfig::UnsupportedRule();
        }
...
EmeConfig::Rule KeySystemsImpl::GetRobustnessConfigRule(
    const std::string& key_system, EmeMediaType media_type, const std::string& requested_robustness, const bool* hw_secure_requirement) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    return key_system_info->GetRobustnessConfigRule(key_system, media_type, requested_robustness, hw_secure_requirement);
}

EmeConfig::Rule KeySystemsImpl::GetPersistentLicenseSessionSupport(const std::string& key_system) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
EmeFeatureSupport KeySystemsImpl::GetPersistentStateSupport(const std::string& key_system) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
EmeFeatureSupport KeySystemsImpl::GetDistinctiveIdentifierSupport(const std::string& key_system) const
{
    DCHECK(thread_checker_.CalledOnValidThread());
