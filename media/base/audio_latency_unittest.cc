...
public:
    ...
    void TestExactBufferSizes()
    {
        ...
        const int platform_min_buffer_size = min_buffer_size ? min_buffer_size : hardware_buffer_size;
...
// and not the min_buffer_size.
        ...
        const int platform_max_buffer_size = (max_buffer_size && max_buffer_size < limits::kMaxWebAudioBufferSize)
            ? (limits::kMaxWebAudioBufferSize / max_buffer_size) * max_buffer_size
        ...
        EXPECT_EQ(platform_min_buffer_size,
            media::AudioLatency::GetExactBufferSize(
                base::Seconds(0.0), hardware_sample_rate, hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(platform_min_buffer_size,
            media::AudioLatency::GetExactBufferSize(base::Seconds(min_buffer_size / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
                hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(multiplier * 2,
            media::AudioLatency::GetExactBufferSize(base::Seconds((multiplier * 2) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
                hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(multiplier * 2,
            media::AudioLatency::GetExactBufferSize(base::Seconds((multiplier * 1.1) / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
                hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        EXPECT_EQ(platform_max_buffer_size,
            media::AudioLatency::GetExactBufferSize(
                base::Seconds(10.0), hardware_sample_rate, hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        if (max_buffer_size && max_buffer_size < limits::kMaxWebAudioBufferSize) {
            EXPECT_EQ(max_buffer_size,
                media::AudioLatency::GetExactBufferSize(base::Seconds(max_buffer_size / static_cast<double>(hardware_sample_rate)), hardware_sample_rate,
                    hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        }
        ...
        if (min_buffer_size && min_buffer_size < hardware_buffer_size) {
            EXPECT_EQ(hardware_buffer_size,
                media::AudioLatency::GetExactBufferSize(base::Seconds((min_buffer_size * 1.1) / static_cast<double>(hardware_sample_rate)),
                    hardware_sample_rate, hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
        }
#elif BUILDFLAG(IS_MAC)
        EXPECT_EQ(limits::kMaxWebAudioBufferSize,
            media::AudioLatency::GetExactBufferSize(base::Seconds((limits::kMaxAudioBufferSize * 1.1) / static_cast<double>(hardware_sample_rate)),
                hardware_sample_rate, hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize));
#endif
        ...
        for (int i = 0; i < 1000; i++) {
            int buffer_size = media::AudioLatency::GetExactBufferSize(
                base::Seconds(i / 1000.0), hardware_sample_rate, hardware_buffer_size, min_buffer_size, max_buffer_size, limits::kMaxWebAudioBufferSize);
            EXPECT_GE(buffer_size, previous_buffer_size);
#if BUILDFLAG(IS_WIN)
            EXPECT_TRUE(buffer_size == min_buffer_size || buffer_size % multiplier == 0 || buffer_size % max_buffer_size == 0);
#else
...
#if BUILDFLAG(IS_WIN)
        EXPECT_EQ(2 * (i / 100), AudioLatency::GetHighLatencyBufferSize(i, i / 100));
...
TEST(AudioLatency, InteractiveBufferSizes)
{
    ...
    for (auto& buffer_size_pair : buffer_size_pairs) {
        EXPECT_EQ(buffer_size_pair.second, AudioLatency::GetInteractiveBufferSize(buffer_size_pair.first));
    }
}
...
INSTANTIATE_TEST_SUITE_P(All, AudioLatencyTest,
#if BUILDFLAG(IS_WIN)
    ...
    testing::Values(std::make_tuple(44100, 440, 128, 440), std::make_tuple(44100, 440, 440, 440), std::make_tuple(44100, 440, 440, 880),
        std::make_tuple(44100, 440, 440, 4400), std::make_tuple(44100, 440, 128, 4196), std::make_tuple(44100, 440, 440, 4196),
        std::make_tuple(44100, 440, 0, 0), std::make_tuple(44100, 256, 128, 512), std::make_tuple(44100, 256, 0, 0))
#elif BUILDFLAG(IS_MAC) || BUILDFLAG(USE_CRAS)
    // These values are constant on Mac and ChromeOS, regardless of device.
    testing::Values(std::make_tuple(44100, 256, limits::kMinAudioBufferSize, limits::kMaxAudioBufferSize))
#else
    testing::Values(std::make_tuple(44100, 256, 0, 0), std::make_tuple(44100, 440, 0, 0), std::make_tuple(48000, 480, 480, 48000))
#endif
);
} // namespace media
