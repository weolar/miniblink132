...
MEDIA_EXPORT bool CopySampleBufferToAnnexBBuffer(VideoCodec codec, CMSampleBufferRef sbuf, bool keyframe, std::string* annexb_buffer);
...
struct ScopedVTCompressionSessionRefTraits {
    static VTCompressionSessionRef InvalidValue()
    {
        return nullptr;
    }
...
using ScopedVTCompressionSessionRef = base::apple::ScopedTypeRef<VTCompressionSessionRef, ScopedVTCompressionSessionRefTraits>;

// Helper class to add session properties to a VTCompressionSessionRef.
class MEDIA_EXPORT SessionPropertySetter {
public:
    SessionPropertySetter(base::apple::ScopedCFTypeRef<VTCompressionSessionRef> session);
