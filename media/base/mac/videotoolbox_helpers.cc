...
public:
    ...
    size_t GetReservedSize() const override
    {
...
public:
    ...
    size_t GetReservedSize() const override
    {
        return str_annexb_buffer_->size();
    }
...
template <typename NalSizeType>
    requires(std::is_integral_v<NalSizeType> && std::is_unsigned_v<NalSizeType> && sizeof(NalSizeType) <= 4)
void CopyNalsToAnnexB(base::span<const char> buffer, AnnexBBuffer* annexb_buffer)
{
    while (!buffer.empty()) {
        NalSizeType nal_size;
        if constexpr (sizeof(NalSizeType) == 1u) {
            nal_size = base::U8FromBigEndian(base::as_bytes(buffer).template first<1u>());
        } else if constexpr (sizeof(NalSizeType) == 2u) {
            nal_size = base::U16FromBigEndian(base::as_bytes(buffer).template first<2u>());
        } else {
            nal_size = base::U32FromBigEndian(base::as_bytes(buffer).template first<4u>());
        }

        auto [nals_buf, remain] = buffer.subspan(sizeof(NalSizeType)).split_at(nal_size);
        buffer = remain;
...
OSStatus GetParameterSetAtIndex(VideoCodec codec, CMFormatDescriptionRef videoDesc, size_t parameterSetIndex, const uint8_t** parameterSetPointerOut,
...
bool CopySampleBufferToAnnexBBuffer(VideoCodec codec, CMSampleBufferRef sbuf, AnnexBBuffer* annexb_buffer, bool keyframe)
{
    ...
    status = GetParameterSetAtIndex(codec, fdesc, 0, nullptr, nullptr, &pset_count, &nal_size_field_bytes);
    if (status == kCMFormatDescriptionBridgeError_InvalidParameter) {
        DLOG(WARNING) << " assuming " << int(codec == VideoCodec::kHEVC ? 3 : 2) << " parameter sets and 4 bytes NAL length header";
    ...
    if (keyframe) {
        ...
        for (size_t pset_i = 0; pset_i < pset_count; ++pset_i) {
            status = GetParameterSetAtIndex(codec, fdesc, pset_i, &pset, &pset_size, nullptr, nullptr);
            if (status != noErr) {
                DLOG(ERROR) << " GetParameterSetAtIndex failed: " << status;
                return false;
            }
    ...
    if (!annexb_buffer->Reserve(total_bytes)) {
        DLOG(ERROR) << "Cannot fit encode output into bitstream buffer. Requested:" << total_bytes;
        return false;
    }
    ...
    base::apple::ScopedCFTypeRef<CMBlockBufferRef> contiguous_bb(bb, base::scoped_policy::RETAIN);
    if (!CMBlockBufferIsRangeContiguous(bb, 0, 0)) {
        contiguous_bb.reset();
        status = CMBlockBufferCreateContiguous(kCFAllocatorDefault, bb, kCFAllocatorDefault, nullptr, 0, 0, 0, contiguous_bb.InitializeInto());
        if (status != noErr) {
    ...
    status = CMBlockBufferGetDataPointer(contiguous_bb.get(), 0, nullptr, nullptr, &contiguous_bb_data);
    if (status != noErr) {
...
bool CopySampleBufferToAnnexBBuffer(VideoCodec codec, CMSampleBufferRef sbuf, bool keyframe, std::string* annexb_buffer)
{
    StringAnnexBBuffer buffer(annexb_buffer);
    return CopySampleBufferToAnnexBBuffer(codec, sbuf, &buffer, keyframe);
}

bool CopySampleBufferToAnnexBBuffer(
    VideoCodec codec, CMSampleBufferRef sbuf, bool keyframe, size_t annexb_buffer_size, char* annexb_buffer, size_t* used_buffer_size)
{
    RawAnnexBBuffer buffer(annexb_buffer, annexb_buffer_size);
    const bool copy_rv = CopySampleBufferToAnnexBBuffer(codec, sbuf, &buffer, keyframe);
    *used_buffer_size = buffer.GetReservedSize();
    return copy_rv;
}

SessionPropertySetter::SessionPropertySetter(base::apple::ScopedCFTypeRef<VTCompressionSessionRef> session)
    : session_(session)
{
}
...
bool SessionPropertySetter::IsSupported(CFStringRef key)
{
    ...
    if (!supported_keys_) {
        ...
        if (VTSessionCopySupportedPropertyDictionary(session_.get(), &dict_ref) == noErr) {
...
{
    DCHECK(session_);
    base::apple::ScopedCFTypeRef<CFNumberRef> cfvalue(CFNumberCreate(nullptr, kCFNumberSInt32Type, &value));
...
{
    DCHECK(session_);
    base::apple::ScopedCFTypeRef<CFNumberRef> cfvalue(CFNumberCreate(nullptr, kCFNumberDoubleType, &value));
    return VTSessionSetProperty(session_.get(), key, cfvalue.get()) == noErr;
}
