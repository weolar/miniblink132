...
public:
    ...
    AmplitudePeakDetectorTest& operator=(const AmplitudePeakDetectorTest&) = delete;

    ~AmplitudePeakDetectorTest() override = default;

    void CreateDetector(AmplitudePeakDetector::PeakDetectedCB callback)
    {
        peak_detector_ = std::make_unique<AmplitudePeakDetector>(std::move(callback));
    ...
    {
        CreateDetector(base::BindLambdaForTesting([peak_count]() { ++(*peak_count); }));
    }
    ...
    std::unique_ptr<AudioBus> GetAudioBusWithSetValue(float value, const SampleLocation& location)
    {
    ...
    void RunSimpleDetectionTest(float value, bool expect_peak, std::string_view message)
    {
        ...
        for (const auto& location : kTestSampleLocations) {
            CreateDetector(expect_peak ? base::MakeExpectedRunAtLeastOnceClosure(FROM_HERE, message) : base::MakeExpectedNotRunClosure(FROM_HERE, message));
...
{
    peak_detector_ = std::make_unique<AmplitudePeakDetector>(base::MakeExpectedNotRunClosure(FROM_HERE, "Callback should not be run by default"));
}

TEST_F(AmplitudePeakDetectorTest, NoPeaks_Silent)
{
    CreateDetector(base::MakeExpectedNotRunClosure(FROM_HERE, "No peaks should be detected for silent data"));
...
{
    RunSimpleDetectionTest(kQuietSample, /*expect_peak=*/false, "No peaks should be detected for positive quiet data");
    RunSimpleDetectionTest(-kQuietSample, /*expect_peak=*/false, "No peaks should be detected for negative quiet data");
}

TEST_F(AmplitudePeakDetectorTest, Peaks_LoudValue)
{
    RunSimpleDetectionTest(kLoudSample, /*expect_peak=*/true, "Peaks should be detected for positive loud data");
    RunSimpleDetectionTest(-kLoudSample, /*expect_peak=*/true, "Peaks should be detected for negative loud data");
}
...
public:
    ...
    FixedSampleAmplitudePeakDetector(const FixedSampleAmplitudePeakDetector&) = delete;
    FixedSampleAmplitudePeakDetector& operator=(const FixedSampleAmplitudePeakDetector&) = delete;

    ~FixedSampleAmplitudePeakDetector() override = default;

    template <typename SampleType> void VerifyNoPeaksFoundInSilence()
    {
        std::vector<SampleType> samples(kFrames, FixedSampleTypeTraits<SampleType>::kZeroPointValue);

        VerifyFindPeaks(samples.data(), /*expect_peak=*/false, "No peak should be detected in zeroed values");
    }

    template <typename SampleType> void RunSimpleDetectionTest_Fixed(float value, bool expect_peak, std::string_view message)
    {
        std::vector<SampleType> samples(kFrames, FixedSampleTypeTraits<SampleType>::kZeroPointValue);
    ...
    void VerifyFindPeaks(const void* data, bool expect_peak, std::string_view message)
    {
        CreateDetector(expect_peak ? base::MakeExpectedRunAtLeastOnceClosure(FROM_HERE, message) : base::MakeExpectedNotRunClosure(FROM_HERE, message));
...
TEST_F(FixedSampleAmplitudePeakDetector, Sequence_SilenceAndQuiet)
{
    ...
    std::vector<uint32_t> silent_data(kFrames, SignedInt32SampleTypeTraits::kZeroPointValue);
    std::vector<uint32_t> quiet_data(kFrames, SignedInt32SampleTypeTraits::FromFloat(kQuietSample));
...
TEST_F(FixedSampleAmplitudePeakDetector, Sequence_ShortPeak)
{
    constexpr int kBytesPerSample = 4;
    std::vector<uint32_t> silent_data(kFrames, SignedInt32SampleTypeTraits::kZeroPointValue);
    std::vector<uint32_t> loud_data(kFrames, SignedInt32SampleTypeTraits::FromFloat(kLoudSample));
...
INSTANTIATE_TEST_SUITE_P(FixedSampleAmplitudePeakDetector, FixedSampleAmplitudePeakDetector, testing::Values(1, 2, 4));
