...
const AudioParameters kDefaultParams
    = AudioParameters(AudioParameters::Format::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(kDefaultChannels), kInputSampleRate, kDefaultFrames);
...
class ConvertingAudioFifoTest : public ::testing::TestWithParam<std::tuple<int, int, int>> {
public:
    ...
    {
        return AudioParameters(AudioParameters::Format::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(output_channels()), output_sample_rate(), output_frames());
    }

    void CreateFifo(const AudioParameters& output_params)
    {
        DCHECK(!fifo_);
        fifo_ = std::make_unique<ConvertingAudioFifo>(kDefaultParams, output_params);
    ...
    int current_frames_in_fifo()
    {
        return fifo()->total_frames_;
    }
...
TEST_P(ConvertingAudioFifoTest, Push_NotEnoughFrames)
{
    ...
    while (frames_to_push + total_frames_pushed < min_number_input_frames_needed()) {
...
INSTANTIATE_TEST_SUITE_P(ConvertingAudioFifoTest, ConvertingAudioFifoTest,
    testing::Combine(testing::ValuesIn(kTestChannels), testing::ValuesIn(kTestSampleRates), testing::ValuesIn(kTestOutputFrames)));
