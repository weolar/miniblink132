...
typedef std::tuple<int, int, ChannelLayoutConfig, double> AudioConverterTestData;
...
public:
        ...
        : epsilon_(std::get<3>(GetParam()))
    {
        ...
        input_parameters_ = AudioParameters(
            AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::FromLayout<kChannelLayout>(), std::get<0>(GetParam()), kHighLatencyBufferSize);
        output_parameters_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, std::get<2>(GetParam()), std::get<1>(GetParam()), kLowLatencyBufferSize);

        converter_ = std::make_unique<AudioConverter>(input_parameters_, output_parameters_, false);
    ...
    void InitializeInputs(int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            fake_callbacks_.push_back(std::make_unique<FakeAudioRenderCallback>(step, kSampleRate));
    ...
    bool ValidateAudioData(int index, int frames, float scale)
    {
        for (int i = 0; i < audio_bus_->channels(); ++i) {
            for (int j = index; j < frames; ++j) {
                ...
                if (error > epsilon_) {
                    EXPECT_NEAR(expected_audio_bus_->channel(i)[j] * scale, audio_bus_->channel(i)[j], epsilon_) << " i=" << i << ", j=" << j;
                    return false;
                }
            }
        }
    ...
    bool RenderAndValidateAudioData(float scale)
    {
        ...
        for (int i = input_parameters_.channels(); i < output_parameters_.channels(); ++i) {
            memset(expected_audio_bus_->channel(i), 0, audio_bus_->frames() * sizeof(*audio_bus_->channel(i)));
        }
...
TEST(AudioConverterTest, AudioDelayAndDiscreteChannelCount)
{
    ...
    AudioParameters input_parameters(AudioParameters::AUDIO_PCM_LINEAR, { CHANNEL_LAYOUT_DISCRETE, 10 }, kSampleRate, kLowLatencyBufferSize);
    AudioParameters output_parameters(AudioParameters::AUDIO_PCM_LINEAR, { CHANNEL_LAYOUT_DISCRETE, 5 }, kSampleRate * 2, kHighLatencyBufferSize);
...
TEST(AudioConverterTest, PropagatesGlitchInfo)
{
    // Choose input and output parameters such that the transform must make
    // multiple calls to fill the buffer.
    AudioParameters input_parameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), kSampleRate, kLowLatencyBufferSize);
    AudioParameters output_parameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), kSampleRate * 2, kHighLatencyBufferSize);
...
TEST_P(AudioConverterTest, ArbitraryOutputRequestSize)
{
    ...
    audio_bus_ = AudioBus::Create(output_parameters_.channels(), output_parameters_.frames_per_buffer() / 2);
    RunTest(1);
}
...
INSTANTIATE_TEST_SUITE_P(AudioConverterTest, AudioConverterTest,
    testing::Values(
        // No resampling. No channel mixing.
        std::make_tuple(44100, 44100, ChannelLayoutConfig::Stereo(), 0.00000048),

        // Upsampling. Channel upmixing.
        std::make_tuple(44100, 48000, ChannelLayoutConfig::FromLayout<CHANNEL_LAYOUT_QUAD>(), 0.033),
