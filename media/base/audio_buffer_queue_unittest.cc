...
static void VerifyBus(AudioBus* bus, int offset, int frames, int buffer_size, float start, float increment, ValueType type = ValueType::kNormal)
{
...
template <typename T> static scoped_refptr<AudioBuffer> MakeTestBuffer(SampleFormat format, ChannelLayout channel_layout, T start, T step, int frames)
{
    return MakeAudioBuffer<T>(format, channel_layout, ChannelLayoutToChannelCount(channel_layout), kSampleRate, start, step, frames, kNoTimestamp);
}
...
TEST(AudioBufferQueueTest, AppendAndClear)
{
    ...
    buffer.Append(MakeTestBuffer<uint8_t>(kSampleFormatU8, channel_layout, 10, 1, 8));
    EXPECT_EQ(8, buffer.frames());
    buffer.Clear();
    EXPECT_EQ(0, buffer.frames());
    buffer.Append(MakeTestBuffer<uint8_t>(kSampleFormatU8, channel_layout, 20, 1, 8));
    EXPECT_EQ(8, buffer.frames());
}
...
TEST(AudioBufferQueueTest, IteratorCheck)
{
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 10.0f, 1.0f, 8));
    EXPECT_EQ(8, buffer.frames());
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 20.0f, 1.0f, 8));
    EXPECT_EQ(12, buffer.frames());
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 30.0f, 1.0f, 8));
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 40.0f, 1.0f, 8));
    EXPECT_EQ(8, buffer.frames());
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 50.0f, 1.0f, 8));
    EXPECT_EQ(16, buffer.frames());
...
TEST(AudioBufferQueueTest, Seek)
{
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 1.0f, 1.0f, 6));
...
TEST(AudioBufferQueueTest, ReadBitstream)
{
    ...
    buffer.Append(MakeBitstreamAudioBuffer(kSampleFormatEac3, channel_layout, channels, kSampleRate, 1, 1, 4, 2, kNoTimestamp));
    buffer.Append(MakeBitstreamAudioBuffer(kSampleFormatEac3, channel_layout, channels, kSampleRate, 9, 1, 20, 10, kNoTimestamp));
    EXPECT_EQ(24, buffer.frames());
...
TEST(AudioBufferQueueTest, ReadBitstreamIECDts)
{
    ...
    buffer.Append(MakeBitstreamAudioBuffer(kSampleFormatIECDts, channel_layout, channels, kSampleRate, 1, 1, 4, 2, kNoTimestamp));
    buffer.Append(MakeBitstreamAudioBuffer(kSampleFormatIECDts, channel_layout, channels, kSampleRate, 9, 1, 20, 10, kNoTimestamp));
    EXPECT_EQ(24, buffer.frames());
...
TEST(AudioBufferQueueTest, ReadF32)
{
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 1.0f, 1.0f, 6));
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 13.0f, 1.0f, 10));
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 33.0f, 1.0f, 60));
...
TEST(AudioBufferQueueTest, ReadU8)
{
    ...
    // Add 4 frames of data.
    buffer.Append(MakeTestBuffer<uint8_t>(kSampleFormatU8, channel_layout, 128, 1, frames));
...
TEST(AudioBufferQueueTest, ReadS16)
{
    ...
    // Add 24 frames of data.
    buffer.Append(MakeTestBuffer<int16_t>(kSampleFormatS16, channel_layout, 1, 1, 4));
    buffer.Append(MakeTestBuffer<int16_t>(kSampleFormatS16, channel_layout, 9, 1, 20));
    EXPECT_EQ(24, buffer.frames());
    ...
    VerifyBus(bus.get(), 0, 4, 4, 1.0f / std::numeric_limits<int16_t>::max(), 1.0f / std::numeric_limits<int16_t>::max());
    VerifyBus(bus.get(), 4, 2, 20, 9.0f / std::numeric_limits<int16_t>::max(), 1.0f / std::numeric_limits<int16_t>::max());
}
...
TEST(AudioBufferQueueTest, ReadS32)
{
    ...
    // Add 24 frames of data.
    buffer.Append(MakeTestBuffer<int32_t>(kSampleFormatS32, channel_layout, 1, 1, 4));
    buffer.Append(MakeTestBuffer<int32_t>(kSampleFormatS32, channel_layout, 9, 1, 20));
    EXPECT_EQ(24, buffer.frames());
...
TEST(AudioBufferQueueTest, ReadF32Planar)
{
    ...
    buffer.Append(MakeTestBuffer<float>(kSampleFormatPlanarF32, channel_layout, 1.0f, 1.0f, 4));
    buffer.Append(MakeTestBuffer<float>(kSampleFormatPlanarF32, channel_layout, 50.0f, 1.0f, 10));
...
TEST(AudioBufferQueueTest, ReadS16Planar)
{
    ...
    // Add 24 frames of data.
    buffer.Append(MakeTestBuffer<int16_t>(kSampleFormatPlanarS16, channel_layout, 1, 1, 4));
    buffer.Append(MakeTestBuffer<int16_t>(kSampleFormatPlanarS16, channel_layout, 5, 1, 20));
    EXPECT_EQ(24, buffer.frames());
    ...
    EXPECT_EQ(18, buffer.frames());
    VerifyBus(bus.get(), 0, 4, 4, 1.0f / std::numeric_limits<int16_t>::max(), 1.0f / std::numeric_limits<int16_t>::max());
    VerifyBus(bus.get(), 4, 2, 20, 5.0f / std::numeric_limits<int16_t>::max(), 1.0f / std::numeric_limits<int16_t>::max());
}
...
TEST(AudioBufferQueueTest, ReadManyChannels)
{
    ...
    // Add 76 frames of data.
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 0.0f, 1.0f, 6));
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 6.0f * channels, 1.0f, 10));
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 16.0f * channels, 1.0f, 60));
    EXPECT_EQ(76, buffer.frames());
...
TEST(AudioBufferQueueTest, Peek)
{
    ...
    const int frames = 60;
    buffer.Append(MakeTestBuffer<float>(kSampleFormatF32, channel_layout, 0.0f, 1.0f, frames));
