...
public:
    ...
    void PushAndVerify(AudioBlockFifo* fifo, int frames_to_push, int channels, int block_frames, int max_frames)
    {
        for (int filled_frames = max_frames - fifo->GetUnfilledFrames(); filled_frames + frames_to_push <= max_frames;) {
            Push(fifo, frames_to_push, channels);
            filled_frames += frames_to_push;
            EXPECT_EQ(max_frames - filled_frames, fifo->GetUnfilledFrames());
            EXPECT_EQ(static_cast<int>(filled_frames / block_frames), fifo->available_blocks());
        }
    }
    ...
    void ConsumeAndVerify(AudioBlockFifo* fifo, int expected_unfilled_frames, int expected_available_blocks)
    {
...
TEST_F(AudioBlockFifoTest, PushAndConsume)
{
    ...
    EXPECT_EQ(number_of_push * new_push_frames - frames * (blocks - 1), remain_frames);
