...
public:
    SilentSinkSuspenderTest()
        : params_(AudioParameters::AUDIO_FAKE, ChannelLayoutConfig::Mono(), 44100, 128)
        ...
        suspender_(&fake_callback_, base::Seconds(-1), params_, mock_sink_, test_loop_.task_runner())
    {
    }
...
TEST_F(SilentSinkSuspenderTest, BasicPassthough)
{
    ...
    AudioGlitchInfo glitch_info { .duration = base::Milliseconds(100), .count = 123 };
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(delay, base::TimeTicks(), glitch_info, temp_bus_.get()));
...
TEST_F(SilentSinkSuspenderTest, SuspendResumeTriggered)
{
    ...
    temp_bus_->Zero();
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, temp_bus_.get()));
    EXPECT_FALSE(temp_bus_->AreFramesZero());
    ...
    EXPECT_TRUE(temp_bus_->AreFramesZero());
    {
        ...
        EXPECT_CALL(*mock_sink_, Pause()).WillOnce(RunClosure(run_loop.QuitClosure()));
        run_loop.Run();
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), {}, true_bus.get());
    EXPECT_FALSE(true_bus->AreFramesZero());
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, temp_bus_.get()));
    EXPECT_EQ(memcmp(temp_bus_->channel(0), true_bus->channel(0), temp_bus_->frames() * sizeof(float)), 0);
}
...
TEST_F(SilentSinkSuspenderTest, MultipleResume)
{
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), {}, true_bus1.get());
    std::unique_ptr<AudioBus> true_bus2 = AudioBus::Create(params_);
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks(), {}, true_bus2.get());
    EXPECT_NE(memcmp(true_bus1->channel(0), true_bus2->channel(0), true_bus1->frames() * sizeof(float)), 0);

    // Reset the fake callback data generation and force two Render() calls before
    // the sink can transition.
    fake_callback_.reset();
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, nullptr));
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, nullptr));
    ...
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, temp_bus_.get()));
    EXPECT_EQ(memcmp(temp_bus_->channel(0), true_bus1->channel(0), temp_bus_->frames() * sizeof(float)), 0);
    EXPECT_EQ(temp_bus_->frames(), suspender_.Render(base::TimeDelta(), base::TimeTicks(), {}, temp_bus_.get()));
    EXPECT_EQ(memcmp(temp_bus_->channel(0), true_bus2->channel(0), temp_bus_->frames() * sizeof(float)), 0);
}
