...
static base::debug::CrashKeyString* crash_key_string()
{
    ...
    if (!string) {
        string = base::debug::AllocateCrashKeyString("use_after_free_checker", base::debug::CrashKeySize::Size32);
    }
...
public:
    ...
    void check() const
    {
        if (state_ != State::kConstructed) {
            base::debug::ScopedCrashKeyString scoped(crash_key_string(), state_ == State::kDestructed ? "destructed" : "corrupt");
            CHECK(false);
        }
    }
