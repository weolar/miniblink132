...
template <typename... T> class CdmPromiseTemplate;
...
enum class CdmSessionType { kTemporary, kPersistentLicense, kMaxValue = kPersistentLicense };

// Type of message being sent to the application.
// Must be consistent with the values specified in the spec:
// https://w3c.github.io/encrypted-media/#idl-def-MediaKeyMessageType
enum class CdmMessageType { LICENSE_REQUEST, LICENSE_RENEWAL, LICENSE_RELEASE, INDIVIDUALIZATION_REQUEST, MESSAGE_TYPE_MAX = INDIVIDUALIZATION_REQUEST };
...
class MEDIA_EXPORT ContentDecryptionModule : public base::RefCountedThreadSafe<ContentDecryptionModule, ContentDecryptionModuleTraits> {
public:
    ...
    // license server.
    virtual void SetServerCertificate(const std::vector<uint8_t>& certificate, std::unique_ptr<SimpleCdmPromise> promise) = 0;
    ...
    virtual void GetStatusForPolicy(HdcpVersion min_hdcp_version, std::unique_ptr<KeyStatusCdmPromise> promise);
    ...
    //       called after the |promise| is resolved.
    virtual void LoadSession(CdmSessionType session_type, const std::string& session_id, std::unique_ptr<NewSessionCdmPromise> promise) = 0;

    // Updates a session specified by |session_id| with |response|.
    virtual void UpdateSession(const std::string& session_id, const std::vector<uint8_t>& response, std::unique_ptr<SimpleCdmPromise> promise) = 0;
    ...
    virtual void CloseSession(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise) = 0;

    // Removes stored session data associated with the session specified by
    // |session_id|.
    virtual void RemoveSession(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise) = 0;
...
protected:
    friend class base::RefCountedThreadSafe<ContentDecryptionModule, ContentDecryptionModuleTraits>;
...
// on failure.
MEDIA_EXPORT std::optional<media::HdcpVersion> MaybeHdcpVersionFromString(const std::string& hdcp_version_string);
...
using SessionMessageCB = base::RepeatingCallback<void(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message)>;
...
using SessionClosedCB = base::RepeatingCallback<void(const std::string& session_id, CdmSessionClosedReason reason)>;
...
using SessionKeysChangeCB = base::RepeatingCallback<void(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info)>;
...
using SessionExpirationUpdateCB = base::RepeatingCallback<void(const std::string& session_id, base::Time new_expiry_time)>;
