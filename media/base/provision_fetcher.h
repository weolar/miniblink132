...
public:
    ...
    // case of success.
    using ResponseCB = base::OnceCallback<void(bool success, const std::string& response)>;
    ...
    virtual void Retrieve(const GURL& default_url, const std::string& request_data, ResponseCB response_cb) = 0;
};

using CreateFetcherCB = base::RepeatingCallback<std::unique_ptr<ProvisionFetcher>()>;
