...
static std::optional<VideoType> ParseVp9CodecID(std::string_view mime_type_lower_case, std::string_view codec_id)
{
...
static bool IsValidH264Level(uint8_t level_idc)
{
    ...
    return ((level_idc >= 10 && level_idc <= 13) || (level_idc >= 20 && level_idc <= 22) || (level_idc >= 30 && level_idc <= 32)
        || (level_idc >= 40 && level_idc <= 42) || (level_idc >= 50 && level_idc <= 52) || (level_idc >= 60 && level_idc <= 62));
}
...
MimeUtil::MimeUtil()
{
    ...
    platform_info_.has_platform_hevc_decoder = MediaCodecUtil::IsHEVCDecoderAvailable();
#endif
    platform_info_.has_platform_opus_decoder = MediaCodecUtil::IsOpusDecoderAvailable();
...
SupportsType MimeUtil::AreSupportedCodecs(const std::vector<ParsedCodecResult>& parsed_codecs, std::string_view mime_type_lower_case, bool is_encrypted) const
{
    ...
    for (const auto& parsed_codec : parsed_codecs) {
        ...
        if (parsed_codec.is_ambiguous) {
            ...
            case MimeUtil::MPEG4_AAC:
                ...
                break;
            default:
                NOTREACHED() << "Only VP9, H264, and AAC codec strings can be ambiguous.";
            }
        }

        // Check platform support.
        SupportsType result = IsCodecSupported(mime_type_lower_case, parsed_codec.codec, video_profile, video_level, video_color_space, is_encrypted);
        if (result == SupportsType::kNotSupported) {
            DVLOG(2) << __func__ << ": Codec " << parsed_codec.codec << " not supported by platform.";
        ...
        if (result == SupportsType::kMaybeSupported ||
            ...
            (result == SupportsType::kSupported && parsed_codec.is_ambiguous && parsed_codec.codec != MimeUtil::VP9)) {
...
void MimeUtil::SplitCodecs(std::string_view codecs, std::vector<std::string>* codecs_out) const
{
...
std::optional<VideoType> MimeUtil::ParseVideoCodecString(std::string_view mime_type, std::string_view codec_id, bool allow_ambiguous_matches) const
{
    ...
    if (!ParseCodecStrings(base::ToLowerASCII(mime_type), codec_strings, &parsed_results)) {
        DVLOG(3) << __func__ << " Failed to parse mime/codec pair: " << (mime_type.empty() ? "<empty mime>" : mime_type) << "; " << codec_id;
        return std::nullopt;
    }
    ...
    if (!parsed_results[0].video) {
        DVLOG(3) << __func__ << " Codec string " << codec_id << " is not a VIDEO codec.";
        return std::nullopt;
    }
...
bool MimeUtil::ParseAudioCodecString(std::string_view mime_type, std::string_view codec_id, bool* out_is_ambiguous, AudioCodec* out_codec) const
{
    ...
    *out_codec = MimeUtilToAudioCodec(parsed_results[0].codec);
    ...
    if (*out_codec == AudioCodec::kUnknown) {
        DVLOG(3) << __func__ << " Codec string " << codec_id << " is not an AUDIO codec.";
        return false;
    }
...
SupportsType MimeUtil::IsSupportedMediaFormat(std::string_view mime_type, const std::vector<std::string>& codecs, bool is_encrypted) const
{
    ...
    if (!ParseCodecStrings(mime_type_lower_case, codecs, &parsed_results)) {
        DVLOG(3) << __func__ << " Media format unsupported; codec parsing failed " << mime_type << " " << base::JoinString(codecs, ",");
        return SupportsType::kNotSupported;
    }
    ...
    // information.
    if (codecs.empty() && parsed_results.size() == 1 && parsed_results[0].codec == INVALID_CODEC) {
...
bool MimeUtil::IsCodecSupportedOnAndroid(
    Codec codec, std::string_view mime_type_lower_case, bool is_encrypted, VideoCodecProfile video_profile, const PlatformInfo& platform_info)
{
    DVLOG(3) << __func__;
    ...
    case MPEG2_AAC:
        ...
        DCHECK(!base::EndsWith(mime_type_lower_case, "mpegurl", base::CompareCase::SENSITIVE));
        [[fallthrough]];
    case PCM:
    case MP3:
    case MPEG4_AAC:
    ...
    case VP9: {
        if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kReportVp9AsAnUnsupportedMimeType)) {
            return false;
        }
        ...
        if (!is_encrypted && (video_profile == VP9PROFILE_PROFILE0 || video_profile == VP9PROFILE_PROFILE1 || video_profile == VIDEO_CODEC_PROFILE_UNKNOWN)) {
            return true;
        }
...
bool MimeUtil::ParseCodecStrings(
    std::string_view mime_type_lower_case, const std::vector<std::string>& codecs, std::vector<ParsedCodecResult>* out_results) const
{
    DCHECK(out_results);
    ...
    if (!mime_type_lower_case.empty()) {
        ...
        if (valid_codecs_for_mime->empty()) {
            ...
            if (!codecs.empty()) {
                DVLOG(3) << __func__ << " Codecs unexpected for mime type:" << mime_type_lower_case;
                return false;
            }
        ...
        if (codecs.empty()) {
            ...
            ParsedCodecResult implied_result = MakeDefaultParsedCodecResult();
            implied_result.is_ambiguous = !GetDefaultCodec(mime_type_lower_case, &implied_result.codec);
            out_results->push_back(implied_result);
            return true;
        }
    }
    ...
    for (std::string codec_string : codecs) {
        ...
        if (!ParseCodecHelper(mime_type_lower_case, codec_string, &result)) {
            DVLOG(3) << __func__ << " Failed to parse mime/codec pair: " << (mime_type_lower_case.empty() ? "<empty mime>" : mime_type_lower_case) << "; "
                     << codec_string;
            return false;
        }
        ...
        if (!mime_type_lower_case.empty() && !valid_codecs_for_mime->contains(result.codec)) {
            DVLOG(3) << __func__ << " Incompatible mime/codec pair: " << mime_type_lower_case << "; " << codec_string;
            return false;
        }
...
bool MimeUtil::ParseCodecHelper(std::string_view mime_type_lower_case, std::string_view codec_id, ParsedCodecResult* out_result) const
{
    DCHECK_EQ(base::ToLowerASCII(mime_type_lower_case), mime_type_lower_case);
    DCHECK(out_result);
    ...
    *out_result = MakeDefaultParsedCodecResult();
    ...
    if (itr != GetStringToCodecMap().end()) {
        ...
        case Codec::VP8:
            out_result->video = { .codec = VideoCodec::kVP8, .profile = VP8PROFILE_ANY, .level = kNoVideoCodecLevel, .color_space = kDefaultColorSpace };
            break;
        case Codec::THEORA:
            out_result->video = { .codec = VideoCodec::kTheora, .profile = THEORAPROFILE_ANY, .level = kNoVideoCodecLevel, .color_space = kDefaultColorSpace };
            break;
        default:
            break;
        }
    ...
    if (codec_id == "avc1" || codec_id == "avc3") {
        ...
        out_result->video
            = { .codec = VideoCodec::kH264, .profile = VIDEO_CODEC_PROFILE_UNKNOWN, .level = kNoVideoCodecLevel, .color_space = kDefaultColorSpace };
        return true;
...
#if BUILDFLAG(ENABLE_PLATFORM_MPEG_H_AUDIO)
    if (base::StartsWith(codec_id, "mhm1.", base::CompareCase::SENSITIVE) || base::StartsWith(codec_id, "mha1.", base::CompareCase::SENSITIVE)) {
...
SupportsType MimeUtil::IsCodecSupported(std::string_view mime_type_lower_case, Codec codec, VideoCodecProfile video_profile, VideoCodecLevel video_level,
    const VideoColorSpace& color_space, bool is_encrypted) const
{
    DVLOG(3) << __func__;
    ...
    if (video_codec != VideoCodec::kUnknown) {
        if (!IsDecoderSupportedVideoType({ video_codec, video_profile, video_level, color_space })) {
            return SupportsType::kNotSupported;
        }
    }
    ...
    if (!IsCodecSupportedOnAndroid(codec, mime_type_lower_case, is_encrypted, video_profile, platform_info_)) {
        return SupportsType::kNotSupported;
    }
#endif

    return ambiguous_platform_support ? SupportsType::kMaybeSupported : SupportsType::kSupported;
}

bool MimeUtil::GetDefaultCodec(std::string_view mime_type, Codec* default_codec) const
{
    // Codecs below are unambiguously implied by the mime type string. DO NOT add
    // default codecs for ambiguous mime types.

    if (mime_type == "audio/mpeg" || mime_type == "audio/mp3" || mime_type == "audio/x-mp3") {
