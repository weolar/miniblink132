...
{
    return base::StartsWith(codec_id, "dva1.", base::CompareCase::SENSITIVE) || base::StartsWith(codec_id, "dvav.", base::CompareCase::SENSITIVE);
}

bool IsDolbyVisionHEVCCodecId(std::string_view codec_id)
{
    return base::StartsWith(codec_id, "dvh1.", base::CompareCase::SENSITIVE) || base::StartsWith(codec_id, "dvhe.", base::CompareCase::SENSITIVE);
}
...
std::optional<VideoType> ParseNewStyleVp9CodecID(std::string_view codec_id)
{
    ...
    std::vector<std::string> fields = base::SplitString(codec_id, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    case 3:
        ...
        break;
    default:
        DVLOG(3) << __func__ << " Invalid chroma subsampling (" << chroma_subsampling << ")";
        return std::nullopt;
    }

    if (result.subsampling != VideoChromaSampling::k420 && profile_idc != 1 && profile_idc != 3) {
        DVLOG(3) << __func__ << " 4:2:2 and 4:4:4 are only supported in profile 1, 3";
    ...
    if (result.color_space.matrix == VideoColorSpace::MatrixID::RGB && chroma_subsampling != 3) {
    ...
    if (video_full_range_flag > 1) {
        DVLOG(3) << __func__ << " Invalid full range flag (" << video_full_range_flag << ")";
        return std::nullopt;
    }
    result.color_space.range = video_full_range_flag == 1 ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;
...
std::optional<VideoType> ParseLegacyVp9CodecID(std::string_view codec_id)
{
    if (codec_id == "vp9" || codec_id == "vp9.0") {
        ...
        // profile.
        VideoType result { .codec = VideoCodec::kVP9, .profile = VIDEO_CODEC_PROFILE_UNKNOWN, .level = kNoVideoCodecLevel };
        return result;
    }
...
std::optional<VideoType> ParseAv1CodecId(std::string_view codec_id)
{
    ...
    if (fields[3].size() != 2 || (result.bit_depth != 8 && result.bit_depth != 10 && result.bit_depth != 12)) {
    ...
    if ((subsampling_x < '0' || subsampling_x > '1') || (subsampling_y < '0' || subsampling_y > '1')
    ...
    if (fields[6].size() != 2 || result.color_space.primaries == VideoColorSpace::PrimaryID::INVALID) {
    ...
    if (fields[7].size() != 2 || result.color_space.transfer == VideoColorSpace::TransferID::INVALID) {
    ...
    if (fields[8].size() != 2 || result.color_space.matrix == VideoColorSpace::MatrixID::INVALID) {
...
std::optional<VideoType> ParseAVCCodecId(std::string_view codec_id)
{
    ...
    uint32_t elem = 0;
    if (codec_id.size() != 11 || !base::HexStringToUInt(codec_id.substr(5), &elem)) {
...
std::optional<VideoType> ParseHEVCCodecId(std::string_view codec_id)
{
    ...
    std::vector<std::string> elem = base::SplitString(codec_id, ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    if (!base::HexStringToUInt(elem[2], &general_profile_compatibility_flags)) {
        DVLOG(4) << __func__ << ": invalid general_profile_compatibility_flags=" << elem[2];
        return std::nullopt;
    }

    VideoCodecProfile out_profile = VIDEO_CODEC_PROFILE_UNKNOWN;
    // Spec A.3.8
    if (general_profile_idc == 11 || (general_profile_compatibility_flags & 2048)) {
        out_profile = HEVCPROFILE_HIGH_THROUGHPUT_SCREEN_EXTENDED;
    }
    // Spec H.11.1.2
    if (general_profile_idc == 10 || (general_profile_compatibility_flags & 1024)) {
    ...
    if (out_profile == VIDEO_CODEC_PROFILE_UNKNOWN) {
        DVLOG(1) << "Warning: unrecognized HEVC/H.265 general_profile_idc: " << general_profile_idc
...
{
    if (!base::StartsWith(codec_id, "vvc1.", base::CompareCase::SENSITIVE) && !base::StartsWith(codec_id, "vvi1.", base::CompareCase::SENSITIVE)) {
        return std::nullopt;
    }
    ...
    for (auto& item : elem) {
        if (item.size() < 1 || ((item[0] == 'C' || item[0] == 'S' || item[0] == 'O') && item.size() < 2)) {
    ...
    unsigned general_profile_idc = 0;
    if (!base::StringToUint(elem[1], &general_profile_idc) || general_profile_idc > 0x63) {
        DVLOG(4) << __func__ << ": invalid general_profile_idc=" << elem[1];
        return std::nullopt;
    }
    ...
    if (out_profile == VIDEO_CODEC_PROFILE_UNKNOWN) {
        DVLOG(1) << "Warning: unrecognized VVC/H.266 general_profile_idc: " << general_profile_idc;
        return std::nullopt;
    }
    ...
    unsigned general_level_idc = 0;
    if (!base::StringToUint(elem[2], &general_level_idc) || general_level_idc > 0xff) {
    ...
    if (elem.size() == 4) {
        ...
        if (!((elem[3][0] == 'C' && elem[4][0] == 'S') || (elem[3][0] == 'C' && elem[4][0] == 'O') || (elem[3][0] == 'S' && elem[4][0] == 'O'))) {
            trailing_valid = false;
        }
    } else if (elem.size() == 6) {
...
std::optional<VideoType> ParseDolbyVisionCodecId(std::string_view codec_id)
{
    ...
    std::vector<std::string> elem = base::SplitString(codec_id, ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    DCHECK(elem[0] == "dvh1" || elem[0] == "dvhe" || elem[0] == "dva1" || elem[0] == "dvav");
    ...
    if (elem[1].size() != 2 || !base::StringToUint(elem[1], &profile_id) || profile_id > 9) {
    ...
    case 9:
        if (!IsDolbyVisionAVCCodecId(codec_id)) {
            DVLOG(4) << __func__ << ": codec id is mismatched with profile_id=" << profile_id;
            return std::nullopt;
        }
        if (profile_id == 0) {
    ...
    case 8:
        ...
        break;
    default:
        DVLOG(4) << __func__ << ": depecrated and not supported profile_id=" << profile_id;
        return std::nullopt;
    }

    // Level string should be two digits.
    unsigned level_id = 0;
    if (elem[2].size() != 2 || !base::StringToUint(elem[2], &level_id) || level_id > 13 || level_id < 1) {
...
std::string TranslateLegacyAvc1CodecIds(std::string_view codec_id)
{
    ...
    if (base::StartsWith(codec_id, "avc1.66.", base::CompareCase::SENSITIVE)) {
        level_start = 8;
        result = "avc1.4200";
    } else if (base::StartsWith(codec_id, "avc1.77.", base::CompareCase::SENSITIVE)) {
        level_start = 8;
        result = "avc1.4D00";
    } else if (base::StartsWith(codec_id, "avc1.100.", base::CompareCase::SENSITIVE)) {
        level_start = 9;
        result = "avc1.6400";
    }
