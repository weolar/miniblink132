...
static void GenerateBuffers(const int* decode_timestamps, StreamParserBuffer::Type type, TrackId track_id, BufferQueue* queue)
{
    ...
    for (int i = 0; decode_timestamps[i] != kEnd; ++i) {
        ...
        if (i % 3 == 0) {
            buffer = StreamParserBuffer::CopyFrom(kFakeData, sizeof(kFakeData), true, type, track_id);
        } else if (i % 3 == 1) {
            buffer = StreamParserBuffer::FromArray(base::HeapArray<uint8_t>::CopiedFrom(kFakeData), true, type, track_id);
        } else {
            buffer = StreamParserBuffer::FromExternalMemory(std::make_unique<ExternalMemoryAdapterForTesting>(kFakeData), true, type, track_id);
        }
        buffer->SetDecodeTimestamp(DecodeTimestamp::FromMicroseconds(decode_timestamps[i]));
...
protected:
    ...
    // returns true.
    size_t CountMatchingMergedBuffers(bool (*predicate)(scoped_refptr<StreamParserBuffer> buffer))
    {
        return static_cast<size_t>(base::ranges::count_if(merged_buffers_, predicate));
    }
    ...
    {
        GenerateBuffers(decode_timestamps, DemuxerStream::AUDIO, kAudioTrackId, &buffer_queue_map_[kAudioTrackId]);
    }
    ...
    {
        GenerateBuffers(decode_timestamps, DemuxerStream::VIDEO, kVideoTrackId, &buffer_queue_map_[kVideoTrackId]);
    }
    ...
    std::string MergedBufferQueueString(bool include_type)
    {
        ...
        for (BufferQueue::const_iterator itr = merged_buffers_.begin(); itr != merged_buffers_.end(); ++itr) {
    ...
    void VerifyMergeSuccess(const std::string& expected, bool verify_type_and_sequence)
    {
        ...
        EXPECT_EQ(buffer_queue_map_[kAudioTrackId].size(), audio_in_merged - original_audio_in_merged);
        if (buffer_queue_map_[kAudioTrackId].empty())
            buffer_queue_map_.erase(kAudioTrackId);
        EXPECT_EQ(buffer_queue_map_[kVideoTrackId].size(), video_in_merged - original_video_in_merged);
    ...
    void ClearBufferQueuesButKeepAnyMergedBuffers()
    {
...
TEST_F(StreamParserTest, MergeBufferQueues_ValidAppendToExistingMerge)
{
    ...
    expected = "A0:100 V1:101 V1:103 A0:105 V1:106 "
