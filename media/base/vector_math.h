...
MEDIA_SHMEM_EXPORT void FMAC(const float src[], float scale, int len, float dest[]);

// Multiply each element of |src| by |scale| and store in |dest|.  |src| and
// |dest| must be aligned by kRequiredAlignment.
MEDIA_SHMEM_EXPORT void FMUL(const float src[], float scale, int len, float dest[]);
...
MEDIA_SHMEM_EXPORT std::pair<float, float> EWMAAndMaxPower(float initial_value, const float src[], int len, float smoothing_factor);
