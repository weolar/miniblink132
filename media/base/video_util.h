...
MEDIA_EXPORT void FillYUVA(VideoFrame* frame, uint8_t y, uint8_t u, uint8_t v, uint8_t a);
...
MEDIA_EXPORT void LetterboxVideoFrame(VideoFrame* frame, const gfx::Rect& view_area);
...
MEDIA_EXPORT void RotatePlaneByPixels(const uint8_t* src, uint8_t* dest, int width, int height,
...
MEDIA_EXPORT gfx::Rect ComputeLetterboxRegion(const gfx::Rect& bounds, const gfx::Size& content);
...
MEDIA_EXPORT gfx::Rect ComputeLetterboxRegionForI420(const gfx::Rect& bounds, const gfx::Size& content);
...
// empty.
MEDIA_EXPORT gfx::Size ScaleSizeToFitWithinTarget(const gfx::Size& size, const gfx::Size& target);

// Return a scaled |size| whose area is greater than or equal to |target|, where
// one of its dimensions is equal to |target|'s.  The aspect ratio of |size| is
// preserved as closely as possible.  If |size| is empty, the result will be
// empty.
MEDIA_EXPORT gfx::Size ScaleSizeToEncompassTarget(const gfx::Size& size, const gfx::Size& target);
...
MEDIA_EXPORT gfx::Rect CropSizeForScalingToTarget(const gfx::Size& size, const gfx::Size& target, size_t alignment = 1u);
...
MEDIA_EXPORT gfx::Size PadToMatchAspectRatio(const gfx::Size& size, const gfx::Size& target);
...
// in GPU process.
MEDIA_EXPORT scoped_refptr<VideoFrame> ConvertToMemoryMappedFrame(scoped_refptr<VideoFrame> frame);
...
// sequence.
MEDIA_EXPORT void ConvertToMemoryMappedFrameAsync(scoped_refptr<VideoFrame> frame, base::OnceCallback<void(scoped_refptr<VideoFrame>)> result_cb);
...
MEDIA_EXPORT scoped_refptr<VideoFrame> ReadbackTextureBackedFrameToMemorySync(
    VideoFrame& txt_frame, gpu::raster::RasterInterface* ri, const gpu::Capabilities& caps, VideoFramePool* pool = nullptr);

// Synchronously reads a single plane. |src_rect| is relative to the plane,
// which may be smaller than |frame| due to subsampling.
MEDIA_EXPORT bool ReadbackTexturePlaneToMemorySync(VideoFrame& src_frame, size_t src_plane, gfx::Rect& src_rect, uint8_t* dest_pixels, size_t dest_stride,
...
MEDIA_EXPORT scoped_refptr<VideoFrame> WrapAsI420VideoFrame(scoped_refptr<VideoFrame> frame);
...
// identical.
[[nodiscard]] MEDIA_EXPORT bool I420CopyWithPadding(const VideoFrame& src_frame, VideoFrame* dst_frame);
...
MEDIA_EXPORT scoped_refptr<VideoFrame> CreateFromSkImage(
    sk_sp<SkImage> sk_image, const gfx::Rect& visible_rect, const gfx::Size& natural_size, base::TimeDelta timestamp, bool force_opaque = false);

// Utility to convert a media pixel format to SkYUVAInfo.
MEDIA_EXPORT std::tuple<SkYUVAInfo::PlaneConfig, SkYUVAInfo::Subsampling> VideoPixelFormatToSkiaValues(VideoPixelFormat video_format);
...
MEDIA_EXPORT const libyuv::YuvConstants* GetYuvContantsForColorSpace(SkYUVColorSpace cs, bool output_argb_matrix);
