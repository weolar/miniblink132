...
template <typename SampleType> class FloatSampleTypeTraits {
    static_assert(std::is_floating_point<SampleType>::value, "Template is only valid for float types.");
...
private:
    template <typename FloatType> static constexpr SampleType From(FloatType source_value)
    {
        ...
        return std::isnan(source_value) ? kMinValue : std::clamp(static_cast<SampleType>(source_value), kMinValue, kMaxValue);
    }

    template <typename FloatType> static constexpr FloatType To(SampleType source_value)
    {
...
template <typename SampleType> class FloatSampleTypeTraitsNoClip {
    static_assert(std::is_floating_point<SampleType>::value, "Template is only valid for float types.");
...
// See also the aliases for commonly used types at the bottom of this file.
template <typename SampleType> class FixedSampleTypeTraits {
    static_assert(std::numeric_limits<SampleType>::is_integer, "Template is only valid for integer types.");
...
private:
    ...
    template <typename FloatType> struct ScalingFactors {
    ...
    template <typename FloatType> static constexpr SampleType From(FloatType source_value)
    {
        ...
        if (source_value < 0) {
            ...
            return static_cast<SampleType>((source_value * ScalingFactors<FloatType>::kForNegativeInput) + static_cast<FloatType>(kZeroPointValue));
        } else {
            // Apply clipping (aka. clamping).
            ...
            return static_cast<SampleType>((source_value * ScalingFactors<FloatType>::kForPositiveInput) + static_cast<FloatType>(kZeroPointValue));
        }
    }
    ...
    template <typename FloatType> static constexpr FloatType To(SampleType source_value)
    {
        ...
        // on many platforms.
        return (offset_value < 0.0f) ? (offset_value * ScalingFactors<FloatType>::kInverseForNegativeInput)
