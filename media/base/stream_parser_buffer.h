...
public:
    ...
    constexpr double operator/(base::TimeDelta rhs) const
    {
        return ts_ / rhs;
    }
    ...
    static constexpr DecodeTimestamp FromPresentationTime(base::TimeDelta timestamp)
    {
    ...
    {
        return ts_.InSecondsF();
    }
    int64_t InMilliseconds() const
    {
        return ts_.InMilliseconds();
    }
    constexpr int64_t InMicroseconds() const
    {
        return ts_.InMicroseconds();
    }
    ...
    constexpr base::TimeDelta ToPresentationTime() const
    {
        return ts_;
    }
...
constexpr DecodeTimestamp kNoDecodeTimestamp = DecodeTimestamp::FromPresentationTime(kNoTimestamp);

// Similar to media::kInfiniteDuration, indicates a decode timestamp of positive
// infinity.
constexpr DecodeTimestamp kMaxDecodeTimestamp = DecodeTimestamp::FromPresentationTime(kInfiniteDuration);
...
public:
    ...
    static scoped_refptr<StreamParserBuffer> CreateEOSBuffer(std::optional<ConfigVariant> next_config = std::nullopt);

    static scoped_refptr<StreamParserBuffer> CopyFrom(const uint8_t* data, int data_size, bool is_key_frame, Type type, TrackId track_id);
    ...
    static scoped_refptr<StreamParserBuffer> FromArray(base::HeapArray<uint8_t> heap_array, bool is_key_frame, Type type, TrackId track_id);
...
private:
    StreamParserBuffer(base::HeapArray<uint8_t> heap_array, bool is_key_frame, Type type, TrackId track_id);

    StreamParserBuffer(std::unique_ptr<ExternalMemory> external_memory, bool is_key_frame, Type type, TrackId track_id);
    StreamParserBuffer(const uint8_t* data, int data_size, bool is_key_frame, Type type, TrackId track_id);
    StreamParserBuffer(DecoderBufferType decoder_buffer_type, std::optional<ConfigVariant> next_config);
