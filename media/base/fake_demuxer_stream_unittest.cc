...
static_assert(kNumBuffersToReadFirst < kNumBuffersInOneConfig, "do not read too many buffers");
static_assert(kNumConfigs > 0, "need multiple configs to trigger config change");
...
public:
    ...
    void BufferReady(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
    {
        DCHECK(read_pending_);
        DCHECK_LE(buffers.size(), 1u) << "FakeDemuxerStreamTest only reads a single-buffer.";
    ...
    {
        stream_ = std::make_unique<FakeDemuxerStream>(kNumConfigs, kNumBuffersInOneConfig, false);
    ...
    {
        stream_ = std::make_unique<FakeDemuxerStream>(1, kNumBuffersInOneConfig, false);
    ...
    void ReadAndExpect(ReadResult result)
    {
        EXPECT_FALSE(read_pending_);
        read_pending_ = true;
        stream_->Read(1, base::BindOnce(&FakeDemuxerStreamTest::BufferReady, base::Unretained(this)));
    ...
    void TestRead(int num_configs, int num_buffers_in_one_config, bool is_encrypted)
    {
        stream_ = std::make_unique<FakeDemuxerStream>(num_configs, num_buffers_in_one_config, is_encrypted);
...
{
    EXPECT_TRUE(DemuxerStream::GetTypeName(DemuxerStream::Type::AUDIO) == std::string("audio"));
    EXPECT_TRUE(DemuxerStream::GetTypeName(DemuxerStream::Type::VIDEO) == std::string("video"));
    EXPECT_TRUE(DemuxerStream::GetTypeName(DemuxerStream::Type::UNKNOWN) == std::string("unknown"));
}

TEST_F(FakeDemuxerStreamTest, DemuxerStream_GetStatusName)
{
    EXPECT_TRUE(DemuxerStream::GetStatusName(DemuxerStream::Status::kOk) == std::string("okay"));
    EXPECT_TRUE(DemuxerStream::GetStatusName(DemuxerStream::Status::kAborted) == std::string("aborted"));
    EXPECT_TRUE(DemuxerStream::GetStatusName(DemuxerStream::Status::kConfigChanged) == std::string("config_changed"));
    EXPECT_TRUE(DemuxerStream::GetStatusName(DemuxerStream::Status::kError) == std::string("error"));
}

TEST_F(FakeDemuxerStreamTest, DemuxerStream_GetLivenessName)
{
    EXPECT_TRUE(GetStreamLivenessName(StreamLiveness::kUnknown) == std::string("unknown"));
    EXPECT_TRUE(GetStreamLivenessName(StreamLiveness::kRecorded) == std::string("recorded"));
    EXPECT_TRUE(GetStreamLivenessName(StreamLiveness::kLive) == std::string("live"));
}
