...
{
    if (size > 0) {
        scoped_refptr<DataBuffer> data_buffer = DataBuffer::CopyFrom(base::make_span(data, static_cast<size_t>(size)));
...
int SeekableBuffer::InternalRead(uint8_t* data, int size, bool advance_position, int forward_offset)
{
...
void SeekableBuffer::UpdateCurrentTime(BufferQueue::iterator buffer, int offset)
{
