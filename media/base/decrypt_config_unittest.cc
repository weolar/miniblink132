...
TEST(DecryptConfigTest, CencConstruction)
{
    ...
    config = DecryptConfig::CreateCencConfig(kAlternateKeyId, kDefaultIV, { { 1, 2 } });
    ...
    config = DecryptConfig::CreateCencConfig(kDefaultKeyId, kAlternateIV, { { 1, 2 }, { 3, 4 }, { 5, 6 }, { 7, 8 } });
    EXPECT_EQ(config->key_id(), kDefaultKeyId);
...
{
    auto config = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, EncryptionPattern(1, 2));
    EXPECT_EQ(config->key_id(), kDefaultKeyId);
    EXPECT_EQ(config->iv(), kDefaultIV);
    EXPECT_EQ(config->subsamples().size(), 0u);
    ...
    // Now with multiple subsample entries.
    config = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kAlternateIV, { { 1, 2 }, { 3, 4 }, { 5, 6 }, { 7, 8 } }, EncryptionPattern(1, 0));
    EXPECT_EQ(config->key_id(), kDefaultKeyId);
    EXPECT_EQ(config->iv(), kAlternateIV);
    EXPECT_EQ(config->subsamples().size(), 4u);
    EXPECT_EQ(config->subsamples()[0].clear_bytes, 1u);
    ...
    // Now without pattern.
    config = DecryptConfig::CreateCbcsConfig(kAlternateKeyId, kDefaultIV, { { 1, 2 } }, std::nullopt);
    EXPECT_EQ(config->key_id(), kAlternateKeyId);
    EXPECT_EQ(config->iv(), kDefaultIV);
    EXPECT_EQ(config->subsamples().size(), 1u);
    EXPECT_EQ(config->subsamples()[0].clear_bytes, 1u);
...
TEST(DecryptConfigTest, Matches)
{
    ...
    auto config2 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, EncryptionPattern(1, 2));
...
TEST(DecryptConfigTest, CencMatches)
{
    ...
    // Different key_id.
    auto config2 = DecryptConfig::CreateCencConfig(kAlternateKeyId, kDefaultIV, {});
    EXPECT_FALSE(config1->Matches(*config2));
    EXPECT_FALSE(config2->Matches(*config1));

    // Different IV.
    auto config3 = DecryptConfig::CreateCencConfig(kDefaultKeyId, kAlternateIV, {});
    ...
    auto config4 = DecryptConfig::CreateCencConfig(kDefaultKeyId, kDefaultIV, { { 1, 2 }, { 3, 4 }, { 5, 6 }, { 7, 8 } });
...
{
    auto config1 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, EncryptionPattern(1, 2));
    EXPECT_TRUE(config1->Matches(*config1));

    // Different key_id.
    auto config2 = DecryptConfig::CreateCbcsConfig(kAlternateKeyId, kDefaultIV, {}, EncryptionPattern(1, 2));
    EXPECT_FALSE(config1->Matches(*config2));
    EXPECT_FALSE(config2->Matches(*config1));

    // Different IV.
    auto config3 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kAlternateIV, {}, EncryptionPattern(1, 2));
    EXPECT_FALSE(config1->Matches(*config3));
    EXPECT_FALSE(config2->Matches(*config3));
    EXPECT_FALSE(config3->Matches(*config1));
    EXPECT_FALSE(config3->Matches(*config2));
    ...
    // Different subsamples.
    auto config4 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, { { 1, 2 }, { 3, 4 }, { 5, 6 } }, EncryptionPattern(1, 2));
    EXPECT_FALSE(config1->Matches(*config4));
    EXPECT_FALSE(config2->Matches(*config4));
    EXPECT_FALSE(config3->Matches(*config4));
    EXPECT_FALSE(config4->Matches(*config1));
    ...
    // Different pattern.
    auto config5 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, EncryptionPattern(5, 6));
    ...
    // Without pattern.
    auto config6 = DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, std::nullopt);
...
TEST(DecryptConfigTest, Output)
{
    ...
    // Try with subsamples.
    stream << *DecryptConfig::CreateCencConfig(kAlternateKeyId, kAlternateIV, { { 1, 2 }, { 3, 4 }, { 5, 6 } });

    // Simple 'cbcs' config.
    stream << *DecryptConfig::CreateCbcsConfig(kDefaultKeyId, kDefaultIV, {}, std::nullopt);

    // 'cbcs' config with subsamples and pattern.
    stream << *DecryptConfig::CreateCbcsConfig(kAlternateKeyId, kAlternateIV, { { 1, 2 }, { 3, 4 }, { 5, 6 }, { 7, 8 } }, EncryptionPattern(1, 2));
}
