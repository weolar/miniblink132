...
CdmKeyInformation::CdmKeyInformation(const std::vector<uint8_t>& key_id, KeyStatus status, uint32_t system_code)
    : key_id(key_id)
    , status(status)
...
CdmKeyInformation::CdmKeyInformation(const std::string& key_id, KeyStatus status, uint32_t system_code)
    : CdmKeyInformation(reinterpret_cast<const uint8_t*>(key_id.data()), key_id.size(), status, system_code)
{
}

CdmKeyInformation::CdmKeyInformation(const uint8_t* key_id_data, size_t key_id_length, KeyStatus status, uint32_t system_code)
...
std::ostream& operator<<(std::ostream& os, CdmKeyInformation::KeyStatus status)
{
...
{
    return os << "key_id = " << base::HexEncode(info.key_id) << ", status = " << info.status << ", system_code = " << info.system_code;
}
