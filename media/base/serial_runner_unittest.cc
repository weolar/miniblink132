...
public:
    ...
    void RunSerialRunner()
    {
        task_environment_.GetMainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&SerialRunnerTest::StartRunnerInternal, base::Unretained(this), std::move(bound_fns_)));
    ...
    {
        bound_fns_.Push(base::BindOnce(&SerialRunnerTest::RunBoundFunction, base::Unretained(this), status, called_.size()));
        called_.push_back(false);
    }

    void PushBoundClosure()
    {
        bound_fns_.Push(base::BindOnce(&SerialRunnerTest::RunBoundClosure, base::Unretained(this), called_.size()));
        called_.push_back(false);
    }

    void PushClosure()
    {
        bound_fns_.Push(base::BindOnce(&SerialRunnerTest::RunClosure, base::Unretained(this), called_.size()));
        called_.push_back(false);
    }
    ...
    {
        bound_fns_.Push(base::BindOnce(&SerialRunnerTest::CancelSerialRunner, base::Unretained(this)));
    }

    // Queries final status of pushed functions and done callback. Valid only
    // after calling RunSerialRunner().
    bool called(size_t index)
    {
        return called_[index];
    }
    bool done_called()
    {
        return done_called_;
    }
    PipelineStatus done_status()
    {
        return done_status_;
    }

private:
    void RunBoundFunction(PipelineStatus status, size_t index, PipelineStatusCallback status_cb)
    {
        EXPECT_EQ(index == 0u, inside_start_) << "First bound function should run on same stack as "
    ...
    {
        inside_start_ = true;
        runner_ = SerialRunner::Run(std::move(bound_fns), base::BindOnce(&SerialRunnerTest::DoneCallback, base::Unretained(this)));
        inside_start_ = false;
    }

    void DoneCallback(PipelineStatus status)
    {
        EXPECT_FALSE(inside_start_) << "Done callback should not run on same stack as SerialRunner::Run()\n" << base::debug::StackTrace().ToString();
    ...
    void CancelSerialRunner(PipelineStatusCallback status_cb)
    {
        ...
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&SerialRunnerTest::ResetSerialRunner, base::Unretained(this)));
