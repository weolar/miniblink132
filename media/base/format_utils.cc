...
std::optional<VideoPixelFormat> GfxBufferFormatToVideoPixelFormat(gfx::BufferFormat format)
{
    switch (format) {
    ...
    default:
        DLOG(WARNING) << "Unsupported BufferFormat: " << gfx::BufferFormatToString(format);
        return std::nullopt;
    }
}

std::optional<gfx::BufferFormat> VideoPixelFormatToGfxBufferFormat(VideoPixelFormat pixel_format)
{
    switch (pixel_format) {
