...
template <class T> size_t Ranges<T>::Add(T start, T end)
{
...
template <> MEDIA_EXPORT void Ranges<base::TimeDelta>::DCheckLT(const base::TimeDelta& lhs, const base::TimeDelta& rhs) const;

template <class T> void Ranges<T>::DCheckLT(const T& lhs, const T& rhs) const
{
    DCHECK_LT(lhs, rhs);
}

template <class T> size_t Ranges<T>::size() const
{
    return ranges_.size();
}

template <class T> T Ranges<T>::start(size_t i) const
{
    return ranges_[i].first;
}

template <class T> T Ranges<T>::end(size_t i) const
{
    return ranges_[i].second;
}

template <class T> const std::pair<T, T>& Ranges<T>::back() const
{
    return ranges_[size() - 1];
}

template <class T> bool Ranges<T>::contains(size_t i, const T& entry) const
{
    return start(i) <= entry && end(i) > entry;
}

template <class T> bool Ranges<T>::empty() const
{
    return size() == 0;
}

template <class T> void Ranges<T>::clear()
{
    ranges_.clear();
}

template <class T> Ranges<T> Ranges<T>::IntersectionWith(const Ranges<T>& other) const
{
    Ranges<T> result;
