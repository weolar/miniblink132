...
public:
    ...
    void RunBenchmark(void (*fn)(const float[], float, int, float[]), bool aligned, const std::string& metric_suffix, const std::string& trace_name)
    {
        TimeTicks start = TimeTicks::Now();
        for (int i = 0; i < kBenchmarkIterations; ++i) {
            fn(input_vector_.get(), kScale, kVectorSize - (aligned ? 0 : 1), output_vector_.get());
        }
        double total_time_seconds = (TimeTicks::Now() - start).InSecondsF();
        perf_test::PerfResultReporter reporter = SetUpReporter(trace_name);
        reporter.AddResult(metric_suffix, kBenchmarkIterations / total_time_seconds);
    }

    void RunBenchmark(std::pair<float, float> (*fn)(float, const float[], int, float), int len, const std::string& metric_suffix, const std::string& trace_name)
    {
        TimeTicks start = TimeTicks::Now();
        ...
        double total_time_seconds = (TimeTicks::Now() - start).InSecondsF();
        perf_test::PerfResultReporter reporter = SetUpReporter(trace_name);
        reporter.AddResult(metric_suffix, kEWMABenchmarkIterations / total_time_seconds);
    }
...
{
    ASSERT_EQ(kVectorSize % (vector_math::kRequiredAlignment / sizeof(float)), 0U);
#if defined(ARCH_CPU_X86_FAMILY)
    base::CPU cpu;
    if (cpu.has_avx2() && cpu.has_fma3()) {
...
{
    RunBenchmark(vector_math::EWMAAndMaxPower_C, kVectorSize, "_ewma_and_max_power", "unoptimized");
}
...
TEST_F(VectorMathPerfTest, EWMAAndMaxPower_optimized_unaligned)
{
    ...
    base::CPU cpu;
    if (cpu.has_avx2() && cpu.has_fma3()) {
        RunBenchmark(vector_math::EWMAAndMaxPower_AVX2, kVectorSize - 1, "_ewma_and_max_power", "optimized_unaligned");
    } else if (cpu.has_sse2()) {
        RunBenchmark(vector_math::EWMAAndMaxPower_SSE, kVectorSize - 1, "_ewma_and_max_power", "optimized_unaligned");
    }
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(USE_NEON)
    RunBenchmark(vector_math::EWMAAndMaxPower_NEON, kVectorSize - 1, "_ewma_and_max_power", "optimized_unaligned");
#endif
}
...
TEST_F(VectorMathPerfTest, EWMAAndMaxPower_optimized_aligned)
{
    ...
    base::CPU cpu;
    if (cpu.has_avx2() && cpu.has_fma3()) {
        RunBenchmark(vector_math::EWMAAndMaxPower_AVX2, kVectorSize, "_ewma_and_max_power", "optimized_aligned");
    } else if (cpu.has_sse2()) {
        RunBenchmark(vector_math::EWMAAndMaxPower_SSE, kVectorSize, "_ewma_and_max_power", "optimized_aligned");
    }
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(USE_NEON)
    RunBenchmark(vector_math::EWMAAndMaxPower_NEON, kVectorSize, "_ewma_and_max_power", "optimized_aligned");
#endif
}
