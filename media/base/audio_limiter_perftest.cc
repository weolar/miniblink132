...
void RunConvertBenchmark(const AudioParameters& params, float amplitude, const std::string& trace_name)
{
    constexpr int kFrequency = 440;

    SineWaveAudioSource sine_source(params.channels(), kFrequency, params.sample_rate());

    auto input_bus = AudioBus::Create(params);
    auto output_bus = AudioBus::Create(params);

    sine_source.OnMoreData(base::TimeDelta(), base::TimeTicks(), {}, input_bus.get());
    ...
    for (int ch = 0; ch < output_bus->channels(); ++ch) {
        output_channels.emplace_back(reinterpret_cast<uint8_t*>(output_bus->channel(ch)), output_bus->frames() * sizeof(float));
    }
    ...
    base::TimeDelta benchmark_data_duration = kBenchmarkIterations * kBufferDuration;
...
TEST(AudioLimiterBenchmark, LimitPeaksBenchmark_NoLimiting)
{
    ...
    AudioParameters input_params(
        AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Mono(), kSampleRate, AudioTimestampHelper::TimeToFrames(kBufferDuration, kSampleRate));
