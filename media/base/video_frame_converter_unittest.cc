...
{
    return scaled ? gfx::ScaleToRoundedRect(kVisibleRect, 0.5, 0.5) : kVisibleRect;
}
...
{
    FILE* f = fopen(
        base::StringPrintf("/tmp/%s_%s_%s.bin", prefix, VideoPixelFormatToString(frame.format()).c_str(), frame.visible_rect().size().ToString().c_str())
            .c_str(),
        "wc");
    for (size_t i = 0; i < VideoFrame::NumPlanes(frame.format()); ++i) {
        auto plane_size = VideoFrame::PlaneSize(frame.format(), i, frame.visible_rect().size());
        for (int y = 0; y < plane_size.height(); ++y) {
            fwrite(frame.visible_data(i) + y * frame.stride(i), 1, plane_size.width(), f);
        }
    }
    fclose(f);
}
...
class VideoFrameConverterTest : public testing::Test, public ::testing::WithParamInterface<TestParams> {
public:
...
TEST_P(VideoFrameConverterTest, ConvertAndScale)
{
    ...
    auto src_frame = VideoFrame::CreateZeroInitializedFrame(src_format_, kCodedSize, kVisibleRect, kVisibleRect.size(), base::TimeDelta());
    auto dest_frame = VideoFrame::CreateZeroInitializedFrame(dest_format_, dest_coded_size_, dest_visible_rect_, dest_visible_rect_.size(), base::TimeDelta());
    ...
    auto expected_dest_frame
        = VideoFrame::CreateZeroInitializedFrame(dest_format_, dest_coded_size_, dest_visible_rect_, dest_visible_rect_.size(), base::TimeDelta());
    FillFourColors(*expected_dest_frame);

    auto dest_visible_size = expected_dest_frame->visible_rect().size();
    for (size_t i = 0; i < VideoFrame::NumPlanes(expected_dest_frame->format()); ++i) {
        SCOPED_TRACE(
            base::StringPrintf("%s -> %s, plane=%d, (%s -> %s)", VideoPixelFormatToString(src_format_).c_str(), VideoPixelFormatToString(dest_format_).c_str(),
                static_cast<int>(i), src_frame->visible_rect().size().ToString().c_str(), expected_dest_frame->visible_rect().size().ToString().c_str()));

        auto plane_size = VideoFrame::PlaneSize(expected_dest_frame->format(), i, dest_visible_size);
        auto ssim = libyuv::CalcFrameSsim(dest_frame->visible_data(i), dest_frame->stride(i), expected_dest_frame->visible_data(i),
            expected_dest_frame->stride(i), plane_size.width(), plane_size.height());
        auto psnr = libyuv::CalcFramePsnr(dest_frame->visible_data(i), dest_frame->stride(i), expected_dest_frame->visible_data(i),
            expected_dest_frame->stride(i), plane_size.width(), plane_size.height());
...
INSTANTIATE_TEST_SUITE_P(, VideoFrameConverterTest,
    testing::Combine(testing::Values(PIXEL_FORMAT_XBGR, PIXEL_FORMAT_XRGB, PIXEL_FORMAT_ABGR, PIXEL_FORMAT_ARGB, PIXEL_FORMAT_I420, PIXEL_FORMAT_I420A,
                         PIXEL_FORMAT_I444, PIXEL_FORMAT_I444A, PIXEL_FORMAT_NV12, PIXEL_FORMAT_NV12A),
        testing::Values(PIXEL_FORMAT_XBGR, PIXEL_FORMAT_XRGB, PIXEL_FORMAT_ABGR, PIXEL_FORMAT_ARGB, PIXEL_FORMAT_I420, PIXEL_FORMAT_I420A, PIXEL_FORMAT_I444,
