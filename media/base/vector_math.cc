...
__attribute__((target("avx2"))) void FMUL_AVX2(const float src[], float scale, int len, float dest[])
{
    const int rem = len % 8;
    const int last_index = len - rem;
    ...
    if (aligned_src) {
        if (aligned_dest) {
            ...
            for (int i = 0; i < last_index; i += 8)
                _mm256_storeu_ps(dest + i, _mm256_mul_ps(_mm256_load_ps(src + i), m_scale));
        }
    } else {
        if (aligned_dest) {
            for (int i = 0; i < last_index; i += 8)
                _mm256_store_ps(dest + i, _mm256_mul_ps(_mm256_loadu_ps(src + i), m_scale));
        } else {
            for (int i = 0; i < last_index; i += 8)
                _mm256_storeu_ps(dest + i, _mm256_mul_ps(_mm256_loadu_ps(src + i), m_scale));
        }
    }
...
__attribute__((target("avx2,fma"))) void FMAC_AVX2(const float src[], float scale, int len, float dest[])
{
    const int rem = len % 8;
    const int last_index = len - rem;
    __m256 m_scale = _mm256_set1_ps(scale);
    ...
    if (aligned_src) {
        if (aligned_dest) {
            for (int i = 0; i < last_index; i += 8)
                _mm256_store_ps(dest + i, _mm256_fmadd_ps(_mm256_load_ps(src + i), m_scale, _mm256_load_ps(dest + i)));
        } else {
            for (int i = 0; i < last_index; i += 8)
                _mm256_storeu_ps(dest + i, _mm256_fmadd_ps(_mm256_load_ps(src + i), m_scale, _mm256_loadu_ps(dest + i)));
        }
    } else {
        if (aligned_dest) {
            for (int i = 0; i < last_index; i += 8)
                _mm256_store_ps(dest + i, _mm256_fmadd_ps(_mm256_loadu_ps(src + i), m_scale, _mm256_load_ps(dest + i)));
        } else {
            for (int i = 0; i < last_index; i += 8)
                _mm256_storeu_ps(dest + i, _mm256_fmadd_ps(_mm256_loadu_ps(src + i), m_scale, _mm256_loadu_ps(dest + i)));
        }
    }
...
// operator[]().
#define EXTRACT_FLOAT(a, i) (i == 0 ? _mm_cvtss_f32(a) : _mm_cvtss_f32(_mm_shuffle_ps(a, a, i)))
...
__attribute__((target("avx2,fma"))) std::pair<float, float> EWMAAndMaxPower_AVX2(float initial_value, const float src[], int len, float smoothing_factor)
{
    const int rem = len % 8;
    const int last_index = len - rem;
    const float weight_prev = 1.0f - smoothing_factor;
    ...
    const __m256 sum_coeff = !weight_prev
        ? _mm256_set_ps(smoothing_factor, 0, 0, 0, 0, 0, 0, 0)
        : _mm256_set_ps(smoothing_factor, smoothing_factor * weight_prev, smoothing_factor * std::pow(weight_prev, 2),
            smoothing_factor * std::pow(weight_prev, 3), smoothing_factor * std::pow(weight_prev, 4), smoothing_factor * std::pow(weight_prev, 5),
            smoothing_factor * std::pow(weight_prev, 6), smoothing_factor * std::pow(weight_prev, 7));

    __m256 max = _mm256_setzero_ps();
    __m256 res = _mm256_set_ps(initial_value, 0, 0, 0, 0, 0, 0, 0);
    __m256 res_coeff = !weight_prev ? _mm256_set1_ps(0) : _mm256_set1_ps(std::pow(weight_prev, 8));
    bool aligned_src = (reinterpret_cast<uintptr_t>(src) & 0x1F) == 0;
    int i = 0;
    for (; i < last_index; i += 8) {
        __m256 sample = aligned_src ? _mm256_load_ps(src + i) : _mm256_loadu_ps(src + i);
    ...
    __m128 m128_sums = _mm_add_ps(_mm256_extractf128_ps(res, 0), _mm256_extractf128_ps(res, 1));
    m128_sums = _mm_add_ps(_mm_movehl_ps(m128_sums, m128_sums), m128_sums);
    float res_sum;
    _mm_store_ss(&res_sum, _mm_add_ss(m128_sums, _mm_shuffle_ps(m128_sums, m128_sums, 1)));
    result.first = res_sum;

    __m128 m128_max = _mm_setzero_ps();
    m128_max = _mm_max_ps(_mm256_extractf128_ps(max, 0), _mm256_extractf128_ps(max, 1));
    m128_max = _mm_max_ps(m128_max, _mm_shuffle_ps(m128_max, m128_max, _MM_SHUFFLE(3, 3, 1, 1)));
