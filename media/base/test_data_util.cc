...
const char kMp4AacAudioAvc1Video[] = "video/mp4; codecs=\"mp4a.40.2, avc1.64001E\"";
...
const char kMp4Vp9Profile2Video[] = "video/mp4; codecs=\"vp09.02.10.10.01.02.02.02.00\"";
const char kMp4Vp9Video[] = "video/mp4; codecs=\"vp09.00.10.08.01.02.02.02.00\"";
...
const char kWebMVp9Profile2Video[] = "video/webm; codecs=\"vp09.02.10.10.01.02.02.02.00\"";
...
const FileToMimeTypeMap& GetFileToMimeTypeMap()
{
    static const base::NoDestructor<FileToMimeTypeMap> kFileToMimeTypeMap({
        ...
        { "bear-320x240-v-vp9_profile2_subsample_cenc-v.mp4", kMp4Vp9Profile2Video },
        { "bear-320x240-v-vp9_profile2_subsample_cenc-v.webm", kWebMVp9Profile2Video },
        ...
        { "bear-320x240_corrupted_after_init_segment.webm", kWebMVorbisAudioVp8Video },
        { "bear-640x360-a_frag-cbcs.mp4", kMp4AacAudio },
        { "bear-640x360-a_frag-cenc.mp4", kMp4AacAudio },
        { "bear-640x360-a_frag-cenc.mp4;bear-640x360-v_frag-cenc.mp4", kMp4AacAudioAvc1Video },
        ...
        { "color_pattern_24_dvhe05_1920x1080__dvh1_st-3sec-frag-cenc.mp4", kMp4DolbyVisionProfile5 },
...
const uint8_t kSecretKey[] = { 0xeb, 0xdd, 0x62, 0xf1, 0x68, 0x14, 0xd2, 0x7b, 0x68, 0xef, 0x12, 0x2a, 0xfc, 0xe4, 0xae, 0x3c };

// The key ID for all encrypted files.
const uint8_t kKeyId[] = { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35 };
...
scoped_refptr<DecoderBuffer> ReadTestDataFile(std::string_view name)
{
    ...
    CHECK_EQ(file_size, base::ReadFile(file_path, data, file_size)) << "Failed to read '" << name << "'";

    return buffer;
}

scoped_refptr<DecoderBuffer> ReadTestDataFile(std::string_view name, base::TimeDelta pts)
{
...
bool LookupTestKeyVector(const std::vector<uint8_t>& key_id, bool allow_rotation, std::vector<uint8_t>* key)
{
    ...
    for (size_t pos = 0; pos < rotate_limit; ++pos) {
        std::rotate(starting_key_id.begin(), starting_key_id.begin() + pos, starting_key_id.end());
...
bool LookupTestKeyString(std::string_view key_id, bool allow_rotation, std::string* key)
{
    std::vector<uint8_t> key_vector;
    bool result = LookupTestKeyVector(std::vector<uint8_t>(key_id.begin(), key_id.end()), allow_rotation, &key_vector);
    if (result)
