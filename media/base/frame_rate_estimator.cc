...
{
    return static_cast<int>(((1.0 / duration.InSecondsF()) + (BucketSize / 2.0)) / BucketSize) * BucketSize;
}

} // namespace

FrameRateEstimator::FrameRateEstimator()
    : min_max_duration_(kMaxSamples)
...
void FrameRateEstimator::AddSample(base::TimeDelta frame_duration)
{
    ...
    if (min_max_duration_.Count() < static_cast<unsigned int>(required_samples_)) {
        return;
    }
