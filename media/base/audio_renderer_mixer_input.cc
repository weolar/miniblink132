...
AudioRendererMixerInput::AudioRendererMixerInput(
    AudioRendererMixerPool* mixer_pool, const base::UnguessableToken& owner_token, const std::string& device_id, AudioLatency::LatencyType latency)
...
void AudioRendererMixerInput::Initialize(const AudioParameters& params, AudioRendererSink::RenderCallback* callback)
{
    DCHECK(!started_);
    DCHECK(!mixer_);
    DCHECK(callback);
...
void AudioRendererMixerInput::Start()
{
    ...
    started_ = true;
    mixer_ = mixer_pool_->GetMixer(owner_token_, params_, latency_, *device_info_, std::move(sink_));
...
void AudioRendererMixerInput::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    ...
    if (device_info_.has_value() && (sink_ || mixer_)) {
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), *device_info_));
        return;
    }
...
void AudioRendererMixerInput::SwitchOutputDevice(const std::string& device_id, OutputDeviceStatusCB callback)
{
    ...
    if (device_id == device_id_ || (IsDefaultDevice(device_id_) && IsDefaultDevice(device_id))) {
...
double AudioRendererMixerInput::ProvideInput(AudioBus* audio_bus, uint32_t frames_delayed)
{
...
void AudioRendererMixerInput::OnDeviceInfoReceived(OutputDeviceInfoCB info_cb, OutputDeviceInfo device_info)
{
    DCHECK(godia_in_progress_);
...
void AudioRendererMixerInput::OnDeviceSwitchReady(OutputDeviceStatusCB switch_cb, scoped_refptr<AudioRendererSink> sink, OutputDeviceInfo device_info)
{
    DCHECK(switch_output_device_in_progress_);
