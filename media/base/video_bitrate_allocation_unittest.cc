...
TEST(VideoBitrateAllocationTest, Constructor_ConstantBitrate_InitializesTargetZero)
{
    VideoBitrateAllocation allocation;
...
TEST(VideoBitrateAllocationTest, Constructor_VariableBitrate_InitializesTargetZero)
{
    VideoBitrateAllocation allocation(Bitrate::Mode::kVariable);
...
TEST(VideoBitrateAllocationTest, SetAndGet)
{
    ...
    VideoBitrateAllocation allocation;
    for (size_t spatial_index = 0; spatial_index < VideoBitrateAllocation::kMaxSpatialLayers; ++spatial_index) {
        for (size_t temporal_index = 0; temporal_index < VideoBitrateAllocation::kMaxTemporalLayers; ++temporal_index) {
            sum += layer_rate;
            EXPECT_TRUE(allocation.SetBitrate(spatial_index, temporal_index, layer_rate++));
        }
    }
    ...
    for (size_t spatial_index = 0; spatial_index < VideoBitrateAllocation::kMaxSpatialLayers; ++spatial_index) {
        for (size_t temporal_index = 0; temporal_index < VideoBitrateAllocation::kMaxTemporalLayers; ++temporal_index) {
            EXPECT_EQ(allocation.GetBitrateBps(spatial_index, temporal_index), layer_rate++);
        }
    }
}
...
TEST(VideoBitrateAllocationTest, CanSetMaxValue)
{
    ...
    EXPECT_TRUE(allocation.SetBitrate(0, 0, std::numeric_limits<uint32_t>::max()));
...
TEST(VideoBitrateAllocationTest, ValidatesSumWhenOverwriting)
{
    VideoBitrateAllocation allocation;
    // Fill up to max sum.
    EXPECT_TRUE(allocation.SetBitrate(0, 0, std::numeric_limits<uint32_t>::max() - 2));
...
TEST(VideoBitrateAllocationTest, CanCopyAndCompare)
{
    ...
    EXPECT_TRUE(allocation.SetBitrate(VideoBitrateAllocation::kMaxSpatialLayers - 1, VideoBitrateAllocation::kMaxTemporalLayers - 1, 2000u));
...
TEST(VideoBitrateAllocationTest, ToString)
{
    ...
    EXPECT_EQ(allocation.ToString(), "active spatial layers: 1, {SL#0: {123, 456, 789}}, mode CBR");
    ...
    EXPECT_EQ(allocation.ToString(), "active spatial layers: 1, {SL#1: {789, 456}}, mode CBR");

    // Add one more spatial layer.
    EXPECT_TRUE(allocation.SetBitrate(2, 0, 123u));
    EXPECT_EQ(allocation.ToString(), "active spatial layers: 2, {SL#1: {789, 456}, SL#2: {123}}, mode CBR");
...
TEST(VideoBitrateAllocationTest, ToString_VariableBitrateAndSingleLayer)
{
    ...
    EXPECT_EQ(allocation.ToString(), "active spatial layers: 1, {SL#0: {1}}, mode VBR with peak bps 1");
}
