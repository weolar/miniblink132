...
template <typename T> uint32_t BuildFlags(const std::vector<T>& build_flags)
{
    uint32_t flags = 0;
...
public:
    Mp4MuxerDelegateFragment(Mp4MuxerContext& context, int video_track_id, int audio_track_id, uint32_t sequence_number);
    ~Mp4MuxerDelegateFragment() = default;

    size_t GetAudioSampleSize() const;
    size_t GetVideoSampleSize() const;

    void AddAudioData(scoped_refptr<DecoderBuffer> encoded_data, base::TimeTicks timestamp);
    void AddVideoData(scoped_refptr<DecoderBuffer> encoded_data, base::TimeTicks timestamp);
    ...
    // written to the file.
    void Finalize(base::TimeTicks start_audio_time, base::TimeTicks start_video_time);

private:
    void AddNewTrack(uint32_t track_index);
    void AddDataToRun(mp4::writable_boxes::TrackFragmentRun& trun, const DecoderBuffer& encoded_data, base::TimeTicks timestamp);
    void AddDataToMdat(std::vector<uint8_t>& track_data, const DecoderBuffer& encoded_data);
    void AddLastTimestamp(mp4::writable_boxes::TrackFragmentRun& trun, base::TimeDelta timestamp);
    void SetBaseDecodeTime(base::TimeTicks start_audio_time, base::TimeTicks start_video_time);
