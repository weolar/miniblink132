...
public:
    ...
    protected:
        ...
        base::CheckedNumeric<mkvmuxer::int64> position_ GUARDED_BY_CONTEXT(sequence_checker_) = 0;

        // Last time data was written via Write().
        base::TimeTicks last_data_output_timestamp_ GUARDED_BY_CONTEXT(sequence_checker_);
    };
    ...
    WebmMuxer(
        AudioCodec audio_codec, bool has_video_, bool has_audio_, std::unique_ptr<Delegate> delegate, std::optional<base::TimeDelta> max_data_output_interval);
    ...
    void ForceOneLibWebmErrorForTesting()
    {
        force_one_libwebm_error_ = true;
    }
...
private:
    ...
    // frame size.
    void AddVideoTrack(const gfx::Size& frame_size, double frame_rate, const std::optional<gfx::ColorSpace>& color_space);
    ...
    base::circular_deque<std::tuple<EncodedFrame, base::TimeDelta>> buffered_frames_;
