...
constexpr int32_t kDisplayIdentityMatrix[9] = { 0x00010000, 0, 0, 0, 0x00010000, 0, 0, 0, 0x40000000 };

void WriteIsoTime(BoxByteStream& writer, base::Time time)
{
    uint64_t iso_time = base::saturated_cast<uint64_t>((time - kMP4Epoch).InSecondsF());
...
Mp4FileTypeBoxWriter::Mp4FileTypeBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::FileType& box)
...
void Mp4FileTypeBoxWriter::Write(BoxByteStream& writer)
{
    ...
    CHECK(box_->compatible_brands.end() != std::find_if(box_->compatible_brands.begin(), box_->compatible_brands.end(), [](const auto type) {
        return type == mp4::FOURCC_AVC1 || type == mp4::FOURCC_AV01 || type == mp4::FOURCC_VP09
...
Mp4MovieBoxWriter::Mp4MovieBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::Movie& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (video_track) {
        ...
        if (video_is_first_track) {
            AddChildBox(std::make_unique<Mp4MovieTrackBoxWriter>(context, box_->tracks[video_track.value().index]));
        }
    }

    if (auto audio_track = context.GetAudioTrack()) {
        DCHECK_LE(audio_track.value().index, box_->tracks.size());
        AddChildBox(std::make_unique<Mp4MovieTrackBoxWriter>(context, box_->tracks[audio_track.value().index]));
    }
    ...
    AddChildBox(std::make_unique<Mp4MovieExtendsBoxWriter>(context, box_->extends));
}
...
Mp4MovieHeaderBoxWriter::Mp4MovieHeaderBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MovieHeader& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Mp4MovieHeaderBoxWriter::Write(BoxByteStream& writer)
{
    ...
    for (auto* it = std::begin(kDisplayIdentityMatrix); it != std::end(kDisplayIdentityMatrix); ++it) {
        writer.WriteU32(*it);
    }
...
Mp4MovieExtendsBoxWriter::Mp4MovieExtendsBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MovieExtends& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (auto video_track = context.GetVideoTrack()) {
        ...
        AddChildBox(std::make_unique<Mp4MovieTrackExtendsBoxWriter>(context, box_->track_extends[video_track.value().index]));
    }

    if (auto audio_track = context.GetAudioTrack()) {
        DCHECK_LE(audio_track.value().index, box_->track_extends.size());
        AddChildBox(std::make_unique<Mp4MovieTrackExtendsBoxWriter>(context, box_->track_extends[audio_track.value().index]));
    }
}
...
Mp4MovieTrackExtendsBoxWriter::Mp4MovieTrackExtendsBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::TrackExtends& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Mp4MovieTrackExtendsBoxWriter::Write(BoxByteStream& writer)
{
    ...
    writer.WriteU32(static_cast<uint32_t>(box_->default_sample_duration.InMilliseconds()));
...
Mp4MovieTrackBoxWriter::Mp4MovieTrackBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::Track& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    AddChildBox(std::make_unique<Mp4MovieTrackHeaderBoxWriter>(context, box_->header));
...
Mp4MovieTrackHeaderBoxWriter::Mp4MovieTrackHeaderBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::TrackHeader& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieMediaBoxWriter::Mp4MovieMediaBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::Media& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    AddChildBox(std::make_unique<Mp4MovieMediaHeaderBoxWriter>(context, box_->header));
    AddChildBox(std::make_unique<Mp4MovieMediaHandlerBoxWriter>(context, box_->handler));
    AddChildBox(std::make_unique<Mp4MovieMediaInformationBoxWriter>(context, box_->information));
}
...
Mp4MovieMediaHeaderBoxWriter::Mp4MovieMediaHeaderBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MediaHeader& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieMediaHandlerBoxWriter::Mp4MovieMediaHandlerBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MediaHandler& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieMediaInformationBoxWriter::Mp4MovieMediaInformationBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MediaInformation& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    AddChildBox(std::make_unique<Mp4MovieDataInformationBoxWriter>(context, box_->data_information));

    AddChildBox(std::make_unique<Mp4MovieSampleTableBoxWriter>(context, box_->sample_table));
}

Mp4MovieMediaInformationBoxWriter::~Mp4MovieMediaInformationBoxWriter() = default;
...
Mp4MovieVideoHeaderBoxWriter::Mp4MovieVideoHeaderBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
Mp4MovieSoundHeaderBoxWriter::Mp4MovieSoundHeaderBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
Mp4MovieDataInformationBoxWriter::Mp4MovieDataInformationBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::DataInformation& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    AddChildBox(std::make_unique<Mp4MovieDataReferenceBoxWriter>(context, box_->data_reference));
}
...
Mp4MovieDataReferenceBoxWriter::Mp4MovieDataReferenceBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::DataReference& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieDataUrlEntryBoxWriter::Mp4MovieDataUrlEntryBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
Mp4MovieSampleTableBoxWriter::Mp4MovieSampleTableBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::SampleTable& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    AddChildBox(std::make_unique<Mp4MovieSampleDescriptionBoxWriter>(context, box_->sample_description));
}
...
Mp4MovieSampleToChunkBoxWriter::Mp4MovieSampleToChunkBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
Mp4MovieDecodingTimeToSampleBoxWriter::Mp4MovieDecodingTimeToSampleBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

Mp4MovieDecodingTimeToSampleBoxWriter::~Mp4MovieDecodingTimeToSampleBoxWriter() = default;
...
Mp4MovieSampleSizeBoxWriter::Mp4MovieSampleSizeBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
...
Mp4MovieSampleChunkOffsetBoxWriter::Mp4MovieSampleChunkOffsetBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

Mp4MovieSampleChunkOffsetBoxWriter::~Mp4MovieSampleChunkOffsetBoxWriter() = default;
...
Mp4MovieSampleDescriptionBoxWriter::Mp4MovieSampleDescriptionBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::SampleDescription& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (box_->video_sample_entry.has_value()) {
        ...
        AddChildBox(std::make_unique<Mp4MovieVisualSampleEntryBoxWriter>(context, box_->video_sample_entry.value()));
        return;
    }

    CHECK(box_->audio_sample_entry.has_value());
    AddChildBox(std::make_unique<Mp4MovieAudioSampleEntryBoxWriter>(context, box_->audio_sample_entry.value()));
}

Mp4MovieSampleDescriptionBoxWriter::~Mp4MovieSampleDescriptionBoxWriter() = default;
...
Mp4MovieVisualSampleEntryBoxWriter::Mp4MovieVisualSampleEntryBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::VisualSampleEntry& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    AddChildBox(std::make_unique<Mp4MovieBitRateBoxWriter>(context, box_->bit_rate));

    switch (box_->codec) {
    case VideoCodec::kVP9:
        CHECK(box_->vp_decoder_configuration.has_value());
        AddChildBox(std::make_unique<Mp4MovieVPCodecConfigurationBoxWriter>(context, box_->vp_decoder_configuration.value()));
        break;
    case VideoCodec::kAV1:
        CHECK(box_->av1_decoder_configuration.has_value());
        AddChildBox(std::make_unique<Mp4MovieAV1CodecConfigurationBoxWriter>(context, box_->av1_decoder_configuration.value()));
        break;
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    case VideoCodec::kH264:
        CHECK(box_->avc_decoder_configuration.has_value());
        AddChildBox(std::make_unique<Mp4MovieAVCDecoderConfigurationBoxWriter>(context, box_->avc_decoder_configuration.value()));
        break;
#endif
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    case VideoCodec::kHEVC:
        CHECK(box_->hevc_decoder_configuration.has_value());
        AddChildBox(std::make_unique<Mp4MovieHEVCDecoderConfigurationBoxWriter>(context, box_->hevc_decoder_configuration.value()));
        break;
#endif
    default:
        NOTREACHED();
...
Mp4MovieVisualSampleEntryBoxWriter::~Mp4MovieVisualSampleEntryBoxWriter() = default;
...
void Mp4MovieVisualSampleEntryBoxWriter::Write(BoxByteStream& writer)
{
    ...
    uint8_t compressor_name_size = std::min(compressor_name.size(), kMaxCompressorNameSize);
...
Mp4MovieAVCDecoderConfigurationBoxWriter::Mp4MovieAVCDecoderConfigurationBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::AVCDecoderConfiguration& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieAVCDecoderConfigurationBoxWriter::~Mp4MovieAVCDecoderConfigurationBoxWriter() = default;
...
Mp4MovieElementaryStreamDescriptorBoxWriter::Mp4MovieElementaryStreamDescriptorBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::ElementaryStreamDescriptor& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieElementaryStreamDescriptorBoxWriter::~Mp4MovieElementaryStreamDescriptorBoxWriter() = default;
...
void Mp4MovieElementaryStreamDescriptorBoxWriter::Write(BoxByteStream& writer)
{
    ...
    std::vector<uint8_t> esds = mp4::ESDescriptor::CreateEsds(box_->aac_codec_description);
...
Mp4MovieHEVCDecoderConfigurationBoxWriter::Mp4MovieHEVCDecoderConfigurationBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::HEVCDecoderConfiguration& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieHEVCDecoderConfigurationBoxWriter::~Mp4MovieHEVCDecoderConfigurationBoxWriter() = default;
...
Mp4MovieAudioSampleEntryBoxWriter::Mp4MovieAudioSampleEntryBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::AudioSampleEntry& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    case AudioCodec::kOpus:
        AddChildBox(std::make_unique<Mp4MovieOpusSpecificBoxWriter>(context, box_->opus_specific_box.value()));
        break;
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    case AudioCodec::kAAC:
        AddChildBox(std::make_unique<Mp4MovieElementaryStreamDescriptorBoxWriter>(context, box_->elementary_stream_descriptor.value()));
        break;
#endif
    default:
        NOTREACHED();
...
Mp4MovieAudioSampleEntryBoxWriter::~Mp4MovieAudioSampleEntryBoxWriter() = default;
...
Mp4MovieOpusSpecificBoxWriter::Mp4MovieOpusSpecificBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::OpusSpecificBox& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Mp4MovieOpusSpecificBoxWriter::Write(BoxByteStream& writer)
{
    ...
    writer.WriteU16(static_cast<uint16_t>(AudioTimestampHelper::TimeToFrames(kOpusSkipDuration, box_->sample_rate))); // Preskip.
...
Mp4MovieVPCodecConfigurationBoxWriter::Mp4MovieVPCodecConfigurationBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::VPCodecConfiguration& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieVPCodecConfigurationBoxWriter::~Mp4MovieVPCodecConfigurationBoxWriter() = default;
...
void Mp4MovieVPCodecConfigurationBoxWriter::Write(BoxByteStream& writer)
{
    ...
    auto video_color_space = box_->color_space.IsValid() ? VideoColorSpace::FromGfxColorSpace(box_->color_space)
                                                         : VideoColorSpace(VideoColorSpace::PrimaryID::UNSPECIFIED, VideoColorSpace::TransferID::UNSPECIFIED,
                                                             VideoColorSpace::MatrixID::UNSPECIFIED, gfx::ColorSpace::RangeID::DERIVED);

    constexpr uint8_t bit_depth = 8u;
    constexpr uint8_t chroma_sub_sampling = 0u;
    uint8_t video_full_range_flag = video_color_space.range == gfx::ColorSpace::RangeID::FULL ? 1u : 0u;

    writer.WriteU8(bit_depth << 4 | chroma_sub_sampling << 1 | video_full_range_flag);
...
Mp4MovieAV1CodecConfigurationBoxWriter::Mp4MovieAV1CodecConfigurationBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::AV1CodecConfiguration& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4MovieAV1CodecConfigurationBoxWriter::~Mp4MovieAV1CodecConfigurationBoxWriter() = default;
...
void Mp4MovieAV1CodecConfigurationBoxWriter::Write(BoxByteStream& writer)
{
    ...
    writer.WriteBytes(box_->av1_decoder_configuration_data.data(), box_->av1_decoder_configuration_data.size());
    writer.EndBox();
}

// Mp4MoviePixelAspectRatioBoxBoxWriter (`pasp`) class.
Mp4MoviePixelAspectRatioBoxBoxWriter::Mp4MoviePixelAspectRatioBoxBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

Mp4MoviePixelAspectRatioBoxBoxWriter::~Mp4MoviePixelAspectRatioBoxBoxWriter() = default;
...
Mp4MovieBitRateBoxWriter::Mp4MovieBitRateBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::BitRate& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
