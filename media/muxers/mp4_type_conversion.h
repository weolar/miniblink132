...
inline constexpr base::Time kMP4Epoch = base::Time::FromDeltaSinceWindowsEpoch(base::Seconds(9561628800));

// "und" on ISO 639-2/T.
inline constexpr uint16_t kUndefinedLanguageCode = 0x55C4;

uint16_t MEDIA_EXPORT ConvertIso639LanguageCodeToU16(std::string_view input_language);

// Convert given default Time unit, e.g. 1000 milliseconds per 1 second, to
// a given timescale, timescale value per second.
uint64_t MEDIA_EXPORT ConvertToTimescale(base::TimeDelta time_diff, uint32_t timescale);
