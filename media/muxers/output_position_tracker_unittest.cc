...
TEST(OutputPositionTrackerTest, OutputPositionTracker)
{
    ...
    OutputPositionTracker buffer(base::BindLambdaForTesting([&written_data, &run_loop](base::span<const uint8_t> data) {
    ...
    constexpr char kExpectedResult[] = { 'a', 'b', 'c', 'a', 'b', 'c', '\0', '\0', 'a', 'b', 'c', '\0', '\0' };
    ASSERT_EQ(std::size(kExpectedResult), written_data.size());
    EXPECT_EQ(memcmp(kExpectedResult, written_data.data(), written_data.size()), 0);
}
