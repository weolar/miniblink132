...
{
    uint32_t allowed_sample_flags = static_cast<uint32_t>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagIsNonSync)
        | static_cast<uint32_t>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsYes)
        | static_cast<uint32_t>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsNo);
...
Mp4MovieFragmentBoxWriter::Mp4MovieFragmentBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MovieFragment& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    AddChildBox(std::make_unique<Mp4MovieFragmentHeaderBoxWriter>(context, box_->header));
...
Mp4MovieFragmentHeaderBoxWriter::Mp4MovieFragmentHeaderBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MovieFragmentHeader& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4TrackFragmentBoxWriter::Mp4TrackFragmentBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::TrackFragment& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    AddChildBox(std::make_unique<Mp4TrackFragmentHeaderBoxWriter>(context, box_->header));
    AddChildBox(std::make_unique<Mp4TrackFragmentDecodeTimeBoxWriter>(context, box_->decode_time));
    AddChildBox(std::make_unique<Mp4TrackFragmentRunBoxWriter>(context, box_->run));
}
...
Mp4TrackFragmentHeaderBoxWriter::Mp4TrackFragmentHeaderBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::TrackFragmentHeader& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Mp4TrackFragmentHeaderBoxWriter::Write(BoxByteStream& writer)
{
    ...
    CHECK(box_->flags & static_cast<uint32_t>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultBaseIsMoof));

    if (box_->flags & static_cast<uint32_t>(mp4::writable_boxes::TrackFragmentHeaderFlags::kSampleDescriptionIndexPresent)) {
        NOTREACHED();
    }

    if (box_->flags & static_cast<uint32_t>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultSampleDurationPresent)) {
        writer.WriteU32(box_->default_sample_duration.InMilliseconds());
    }

    if (box_->flags & static_cast<uint32_t>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultSampleSizePresent)) {
        writer.WriteU32(box_->default_sample_size);
    }

    if (box_->flags & static_cast<uint32_t>(mp4::writable_boxes::TrackFragmentHeaderFlags::kkDefaultSampleFlagsPresent)) {
...
Mp4TrackFragmentDecodeTimeBoxWriter::Mp4TrackFragmentDecodeTimeBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::TrackFragmentDecodeTime& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4TrackFragmentDecodeTimeBoxWriter::~Mp4TrackFragmentDecodeTimeBoxWriter() = default;
...
Mp4TrackFragmentRunBoxWriter::Mp4TrackFragmentRunBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::TrackFragmentRun& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Mp4TrackFragmentRunBoxWriter::Write(BoxByteStream& writer)
{
...

    {
        ...
        CHECK(box_->flags & static_cast<uint16_t>(mp4::writable_boxes::TrackFragmentRunFlags::kDataOffsetPresent));
        writer.WriteOffsetPlaceholder();
    }

    uint32_t timescale = 0;
    if (box_->flags & static_cast<uint16_t>(mp4::writable_boxes::TrackFragmentRunFlags::kFirstSampleFlagsPresent)) {
    ...
    bool duration_exists = (box_->flags & static_cast<uint16_t>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleDurationPresent));
    bool size_exists = (box_->flags & static_cast<uint16_t>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleSizePresent));
    bool flags_exists = (box_->flags & static_cast<uint16_t>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleFlagsPresent));
    ...
    for (uint32_t i = 0; i < box_->sample_count; ++i) {
        if (duration_exists) {
            base::TimeDelta time_diff = box_->sample_timestamps[i + 1] - box_->sample_timestamps[i];
            writer.WriteU32(static_cast<uint32_t>(ConvertToTimescale(time_diff, timescale)));
        }
...
Mp4MediaDataBoxWriter::Mp4MediaDataBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::MediaData& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4FragmentRandomAccessBoxWriter::Mp4FragmentRandomAccessBoxWriter(const Mp4MuxerContext& context, const mp4::writable_boxes::FragmentRandomAccess& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // audio.
    if (auto video_track = context.GetVideoTrack()) {
        AddChildBox(std::make_unique<Mp4TrackFragmentRandomAccessBoxWriter>(context, box_->tracks[video_track.value().index]));
    }

    AddChildBox(std::make_unique<Mp4FragmentRandomAccessOffsetBoxBoxWriter>(context));
}
...
Mp4TrackFragmentRandomAccessBoxWriter::Mp4TrackFragmentRandomAccessBoxWriter(
    const Mp4MuxerContext& context, const mp4::writable_boxes::TrackFragmentRandomAccess& box)
    : Mp4BoxWriter(context)
    , box_(box)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
Mp4TrackFragmentRandomAccessBoxWriter::~Mp4TrackFragmentRandomAccessBoxWriter() = default;
...
void Mp4TrackFragmentRandomAccessBoxWriter::Write(BoxByteStream& writer)
{
    ...
    for (const mp4::writable_boxes::TrackFragmentRandomAccessEntry& entry : box_->entries) {
...
Mp4FragmentRandomAccessOffsetBoxBoxWriter::Mp4FragmentRandomAccessOffsetBoxBoxWriter(const Mp4MuxerContext& context)
    : Mp4BoxWriter(context)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

Mp4FragmentRandomAccessOffsetBoxBoxWriter::~Mp4FragmentRandomAccessOffsetBoxBoxWriter() = default;
