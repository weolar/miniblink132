...
constexpr uint8_t kSPS[] = { 0x67, 0x64, 0x00, 0x0C, 0xAC, 0xD9, 0x41, 0x41, 0xFB, 0x01, 0x10, 0x00, 0x00, 0x03, 0x00, 0x10, 0x00, 0x00, 0x00, 0x03, 0x01, 0xE0,
    0xF1, 0x42, 0x99, 0x60 };
...
protected:
    ...
    void CreateContext(std::vector<uint8_t>& written_data)
    {
        auto tracker = std::make_unique<OutputPositionTracker>(base::BindRepeating(
            [&](base::OnceClosure run_loop_quit, std::vector<uint8_t>* written_data, base::span<const uint8_t> mp4_data_string) {
                // Callback is called per box output.

                base::ranges::copy(mp4_data_string, std::back_inserter(*written_data));
    ...
    void AddVideoMediaInformation(mp4::writable_boxes::MediaInformation& media_information)
    {
        AddVideoSampleTable(media_information.sample_table);
    }

    void AddAudioMediaInformation(mp4::writable_boxes::MediaInformation& media_information)
    {
    ...
    void AddVideoSampleTable(mp4::writable_boxes::SampleTable& sample_table)
    {
        ...
        mp4::writable_boxes::VisualSampleEntry visual_sample_entry(VideoCodec::kVP9);
        ...
        video_sample_description.video_sample_entry = std::move(visual_sample_entry);
    ...
    void AddAudioSampleTable(mp4::writable_boxes::SampleTable& sample_table)
    {
        ...
        mp4::writable_boxes::AudioSampleEntry audio_sample_entry(AudioCodec::kOpus, kSampleRate, 2u);
    ...
    size_t FlushWithBoxWriterAndWait(Mp4BoxWriter* box_writer, BoxByteStream& box_byte_stream)
    {
        // Flush at requested.
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieAndHeader)
{
    ...
    mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(written_data.data(), written_data.size(), nullptr, &reader);
    ...
    EXPECT_EQ(mvhd_box.modification_time, ConvertTo1904TimeInSeconds(modification_time));
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieTrackAndMediaHeader)
{
    ...
    base::Time modification_time = base::Time::FromTimeT(0x2345678);
    {
        ...
        video_track.header.flags
            = (static_cast<T>(mp4::writable_boxes::TrackHeaderFlags::kTrackEnabled) | static_cast<T>(mp4::writable_boxes::TrackHeaderFlags::kTrackInMovie));
    ...
    EXPECT_EQ(track_boxes[kVideoIndex].header.creation_time, ConvertTo1904TimeInSeconds(creation_time));
    EXPECT_EQ(track_boxes[kVideoIndex].header.modification_time, ConvertTo1904TimeInSeconds(modification_time));
    ...
    EXPECT_EQ(track_boxes[kAudioIndex].header.creation_time, ConvertTo1904TimeInSeconds(creation_time));
    EXPECT_EQ(track_boxes[kAudioIndex].header.modification_time, ConvertTo1904TimeInSeconds(modification_time));
    ...
    EXPECT_EQ(track_boxes[kAudioIndex].media.header.creation_time, ConvertTo1904TimeInSeconds(creation_time));
    EXPECT_EQ(track_boxes[kAudioIndex].media.header.modification_time, ConvertTo1904TimeInSeconds(modification_time));
    ...
    EXPECT_EQ(track_boxes[kAudioIndex].media.header.language_code, kUndefinedLanguageCode);

    EXPECT_EQ(track_boxes[kVideoIndex].media.header.creation_time, ConvertTo1904TimeInSeconds(creation_time));
    EXPECT_EQ(track_boxes[kVideoIndex].media.header.modification_time, ConvertTo1904TimeInSeconds(modification_time));
    ...
    EXPECT_EQ(track_boxes[kVideoIndex].media.header.language_code, kUndefinedLanguageCode);

    // Media Handler validation.
    EXPECT_EQ(track_boxes[kVideoIndex].media.handler.type, mp4::TrackType::kVideo);
    EXPECT_EQ(track_boxes[kVideoIndex].media.handler.name, kVideoHandlerName);

    EXPECT_EQ(track_boxes[kAudioIndex].media.handler.type, mp4::TrackType::kAudio);
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieMediaDataInformation)
{
    ...
    media_information.data_information.data_reference.entries.push_back(std::move(entry));
    ...
    // `written_data` test.
    std::unique_ptr<mp4::BoxReader> box_reader(mp4::BoxReader::ReadConcatentatedBoxes(written_data.data(), written_data.size(), nullptr));
    // `minf`.
    uint32_t fourcc;
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieVisualSampleEntry)
{
    ...
    EXPECT_EQ(VideoCodecProfile::H264PROFILE_MAIN, video_sample_entry.video_info.profile);
}
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4AacAudioSampleEntry)
{
    ...
    constexpr uint32_t kSampleRate = 48000u;
    mp4::writable_boxes::AudioSampleEntry audio_sample_entry(AudioCodec::kAAC, kSampleRate, 2u);
    ...
    bool metadata_frame;
    EXPECT_NE(
        adts_parser.ParseFrameHeader(buffer.data(), adts_header_size, &frame_size, &sample_rate, &adts_channel_layout, &sample_count, &metadata_frame, nullptr),
        -1);
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieHEVCDecoderConfigurationRecord)
{
    ...
    std::vector<uint8_t> test_data { 0x01, 0x01, 0x60, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3c, 0xf0, 0x00, 0xfc, 0xfd, 0xf8, 0xf8, 0x00,
        0x00, 0x0f, 0x03, 0x20, 0x00, 0x01, 0x00, 0x18, 0x40, 0x01, 0x0c, 0x01, 0xff, 0xff, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x03, 0x00,
        0x00, 0x03, 0x00, 0x3c, 0x95, 0xc0, 0x90, 0x21, 0x00, 0x01, 0x00, 0x27, 0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x03,
        0x00, 0x00, 0x03, 0x00, 0x3c, 0xa0, 0x0a, 0x08, 0x0b, 0x9f, 0x79, 0x65, 0x79, 0x24, 0xca, 0xe0, 0x10, 0x00, 0x00, 0x06, 0x40, 0x00, 0x00, 0xbb, 0x50,
        0x80, 0x22, 0x00, 0x01, 0x00, 0x06, 0x44, 0x01, 0xc1, 0x73, 0xd1, 0x89 };
    EXPECT_TRUE(hevc.hevc_config_record.Parse(test_data.data(), test_data.size()));
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MovieVPConfigurationRecord)
{
    ...
    mp4::writable_boxes::VPCodecConfiguration vp_config(VP9PROFILE_MIN, 0u,
        gfx::ColorSpace(
            gfx::ColorSpace::PrimaryID::BT470M, gfx::ColorSpace::TransferID::GAMMA28, gfx::ColorSpace::MatrixID::BT470BG, gfx::ColorSpace::RangeID::FULL));

    Mp4MovieVPCodecConfigurationBoxWriter box_writer(*context(), std::move(vp_config));
    FlushAndWait(&box_writer);
    ...
    EXPECT_EQ(VideoColorSpace::PrimaryID::BT470M, vp_config_record.color_space.primaries);
    EXPECT_EQ(VideoColorSpace::TransferID::GAMMA28, vp_config_record.color_space.transfer);
    EXPECT_EQ(VideoColorSpace::MatrixID::BT470BG, vp_config_record.color_space.matrix);
}
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4Fragments)
{
    ...
    constexpr uint32_t kVideoSampleDurationsAfterTimescale[] = { 28800, 28800, 28800 };
    constexpr uint32_t kAudioSampleDurationsAfterTimescale[] = { 42336, 42336, 42336 };
    ...
    using H = std::underlying_type_t<mp4::writable_boxes::TrackFragmentHeaderFlags>;
    ...
    { // `video`.
        ...
        video_fragment.header.flags = (static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultBaseIsMoof)
            | static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultSampleDurationPresent)
            | static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kkDefaultSampleFlagsPresent));

        video_fragment.header.default_sample_duration = base::Milliseconds(kDuration1);
        video_fragment.header.default_sample_flags = static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsNo);

        video_fragment.decode_time.track_id = 1;
        video_fragment.decode_time.base_media_decode_time = base::Milliseconds(kVideoBaseDecodeTime);

        { // `video, trun`
            mp4::writable_boxes::TrackFragmentRun video_trun;
            video_trun.flags = (static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kDataOffsetPresent)
                | static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kFirstSampleFlagsPresent)
                | static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleDurationPresent));

            video_trun.sample_count = kSampleCount;
            video_trun.first_sample_flags = (static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagIsNonSync)
                | static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsYes));
            ...
            for (auto* iter = std::begin(kSampleDurations); iter != std::end(kSampleDurations); ++iter) {
    ...
    { // `audio`.
        ...
        audio_fragment.header.flags = (static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultBaseIsMoof)
            | static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kDefaultSampleSizePresent)
            | static_cast<H>(mp4::writable_boxes::TrackFragmentHeaderFlags::kkDefaultSampleFlagsPresent));

        audio_fragment.header.default_sample_size = kDefaultSampleSize;
        audio_fragment.header.default_sample_flags = (static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagIsNonSync)
            | static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsYes));

        audio_fragment.decode_time.track_id = 2u;
        audio_fragment.decode_time.base_media_decode_time = base::Milliseconds(kAudioBaseDecodeTime);

        { // `audio, trun.
            mp4::writable_boxes::TrackFragmentRun audio_trun;
            audio_trun.flags = (static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kDataOffsetPresent)
                | static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleDurationPresent)
                | static_cast<R>(mp4::writable_boxes::TrackFragmentRunFlags::kSampleSizePresent));

            audio_trun.sample_count = kSampleCount;
            audio_trun.first_sample_flags = (static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagIsNonSync)
                | static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsYes));
            ...
            std::vector<uint32_t> sizes(std::begin(kSampleSizes), std::end(kSampleSizes));
    ...
    EXPECT_EQ(static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsNo), traf_boxes[0].header.default_sample_flags);

    // `tfdt` test of video.
    EXPECT_EQ(ConvertToTimescale(base::Milliseconds(kVideoBaseDecodeTime), kVideoTimescale), traf_boxes[0].decode_time.decode_time);
    ...
    EXPECT_EQ(std::vector<uint32_t>(std::begin(kVideoSampleDurationsAfterTimescale), std::end(kVideoSampleDurationsAfterTimescale)),
    ...
    EXPECT_EQ((static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagIsNonSync)
                  | static_cast<S>(mp4::writable_boxes::FragmentSampleFlags::kSampleFlagDependsYes)),
        traf_boxes[1].header.default_sample_flags);

    // `tfdt` test of audio.
    EXPECT_EQ(ConvertToTimescale(base::Milliseconds(kAudioBaseDecodeTime), kAudioTimescale), traf_boxes[1].decode_time.decode_time);
    ...
    EXPECT_EQ(std::vector<uint32_t>(std::begin(kAudioSampleDurationsAfterTimescale), std::end(kAudioSampleDurationsAfterTimescale)),
    ...
    EXPECT_EQ(std::vector<uint32_t>(std::begin(kSampleSizes), std::end(kSampleSizes)), traf_boxes[1].runs[0].sample_sizes);
    ...
    mp4::ParseResult result1 = mp4::BoxReader::ReadTopLevelBox(
        written_data.data() + mdat_video_data_offset - kBoxHeaderSize, written_data.size() - mdat_video_data_offset + kBoxHeaderSize, nullptr, &mdat_reader);

    EXPECT_EQ(result1, mp4::ParseResult::kOk);
    EXPECT_TRUE(mdat_reader);
    EXPECT_EQ(mp4::FOURCC_MDAT, mdat_reader->type());
    EXPECT_EQ(kVideoDataSize + kAudioDataSize + kBoxHeaderSize, mdat_reader->box_size());
    // Once Flush, it needs to reset the internal objects of context and buffer.
    Reset();
}
...
TEST_F(Mp4MuxerBoxWriterTest, Mp4MfraBox)
{
    ...
    std::unique_ptr<mp4::BoxReader> reader;
    mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(mfra_start_offset, mfra_box_size, nullptr, &reader);
