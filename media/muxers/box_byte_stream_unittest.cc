...
TEST(BoxByteStreamTest, Default)
{
    ...
    DataOrder test_data[]
        = { { DataType::kType_8, 0x48 }, { DataType::kPlaceHolder, /*total_size=*/44 }, { DataType::kType_16, 0x1617 }, { DataType::kType_32, 0x32333435 },
              { DataType::kType_64, 0x64646667686970 }, { DataType::kType_Bytes, 0x12345678901234 }, { DataType::kPlaceHolder, /*total_size=*/9 },
              { DataType::kType_8, 0x28 }, { DataType::kEndBox, 0x0 }, { DataType::kType_16, 0x0 }, { DataType::kType_32, 0x0 }, { DataType::kEndBox, 0x0 },
              { DataType::kType_String, reinterpret_cast<uint64_t>("") }, { DataType::kType_String, reinterpret_cast<uint64_t>("abcdabcd") } };
    ...
    for (auto& data : test_data) {
        ...
        case DataType::kPlaceHolder: {
            ...
            base::byte_span_from_ref(ret_value).first<4>().copy_from(base::byte_span_from_ref(val));
        ...
        case DataType::kType_8: {
            ...
            base::byte_span_from_ref(ret_value).first<1>().copy_from(base::byte_span_from_ref(val));
            EXPECT_EQ(data.value, ret_value);
            break;
        }
        case DataType::kType_16: {
            ...
            base::byte_span_from_ref(ret_value).first<2>().copy_from(base::byte_span_from_ref(val));
            EXPECT_EQ(data.value, ret_value);
            break;
        }
        case DataType::kType_32: {
        ...
        case DataType::kType_64:
            ...
            break;
        case DataType::kType_Bytes:
            EXPECT_TRUE(reader.ReadCopy(base::byte_span_from_ref(ret_value).first<7>()));
            EXPECT_EQ(data.value, ret_value);
            break;
        case DataType::kType_String:
            std::string expected_string = reinterpret_cast<char*>(data.value);
            if (expected_string.empty()) {
                EXPECT_TRUE(reader.ReadCopy(base::byte_span_from_ref(ret_value).first<1>()));
...
TEST(BoxByteStreamTest, GrowLimit)
{
    ...
    uint32_t expected_total_size = 8u + uint32_t { BoxByteStream::kDefaultBufferLimit } + 8u + 2u + 4u;
    uint32_t value;
