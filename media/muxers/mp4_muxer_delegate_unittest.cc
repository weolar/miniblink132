...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    void PopulateAVCDecoderConfiguration(std::vector<uint8_t>& video_codec_description)
    {
...
TEST_F(Mp4MuxerDelegateTest, AddVideoFrame)
{
    ...
    Mp4MuxerDelegate delegate(
        AudioCodec::kAAC, VideoCodec::kH264, std::nullopt, std::nullopt, base::BindLambdaForTesting([&](base::span<const uint8_t> mp4_data) {
            ...
            case 2:
                ...
                break;
            case 3:
                base::ranges::copy(mp4_data, std::back_inserter(first_moof_written_data));
                break;
            case 4:
                base::ranges::copy(mp4_data, std::back_inserter(second_moof_written_data));
                run_loop.Quit();
                break;
            }
        }));
    ...
    media::Muxer::VideoParameters params(gfx::Size(kWidth, kHeight), 30, VideoCodec::kH264, gfx::ColorSpace());
    video_stream_1->set_is_key_frame(true);
    delegate.AddVideoFrame(params, video_stream_1, video_codec_description, base_time_ticks);
    video_stream_2->set_is_key_frame(false);
    for (int i = 0; i < 3; ++i) {
        delta += base::Milliseconds(kSampleDurations[i]);
        delegate.AddVideoFrame(params, video_stream_2, std::nullopt, base_time_ticks + delta);
    }

    delta += base::Milliseconds(kSampleDurations[3]);
    video_stream_1->set_is_key_frame(true);
    delegate.AddVideoFrame(params, video_stream_1, video_codec_description, base_time_ticks + delta);
    video_stream_2->set_is_key_frame(false);
    for (int i = 0; i < 2; ++i) {
        delta += base::Milliseconds(kSampleDurations[i]);
        delegate.AddVideoFrame(params, video_stream_2, std::nullopt, base_time_ticks + delta);
...

    {
        ...
        mp4_stream_parser.Init(base::BindOnce(&Mp4MuxerDelegateTest::InitF, base::Unretained(this)),
            base::BindRepeating(&Mp4MuxerDelegateTest::NewConfigCB, base::Unretained(this)),
            base::BindRepeating(&Mp4MuxerDelegateTest::NewBuffersF, base::Unretained(this)),
            base::BindRepeating(&Mp4MuxerDelegateTest::KeyNeededF, base::Unretained(this)),
            base::BindRepeating(&Mp4MuxerDelegateTest::NewSegmentF, base::Unretained(this)),
            base::BindRepeating(&Mp4MuxerDelegateTest::EndOfSegmentF, base::Unretained(this)), &media_log_);

        bool result = mp4_stream_parser.AppendToParseBuffer(base::make_span(total_written_data));
        EXPECT_TRUE(result);

        // `MP4StreamParser::Parse` validates the MP4 format.
        StreamParser::ParseStatus parse_result = mp4_stream_parser.Parse(total_written_data.size());
    ...
    // Validates the MP4 boxes.
    {
        ...
        mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(moov_written_data.data(), moov_written_data.size(), nullptr, &reader);
        ...
        EXPECT_EQ(track_boxes[0].media.header.language_code, kUndefinedLanguageCode);
...

    {
        ...
        mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(first_moof_written_data.data(), first_moof_written_data.size(), nullptr, &moof_reader);
        ...
        EXPECT_EQ(std::vector<uint32_t>(std::begin(kSampleDurationsAfterTimescale), std::end(kSampleDurationsAfterTimescale)),
        ...
        mp4::ParseResult result1 = mp4::BoxReader::ReadTopLevelBox(first_moof_written_data.data() + mdat_video_data_offset - kBoxHeaderSize,
            first_moof_written_data.size() - mdat_video_data_offset + kBoxHeaderSize, nullptr, &mdat_reader);
...

    {
        ...
        std::unique_ptr<mp4::BoxReader> moof_reader;
        mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(second_moof_written_data.data(), second_moof_written_data.size(), nullptr, &moof_reader);
        ...
        EXPECT_EQ(kSampleDurationsAfterTimescale[0], traf_boxes[0].runs[0].sample_durations[0]);
        EXPECT_EQ(kSampleDurationsAfterTimescale[1], traf_boxes[0].runs[0].sample_durations[1]);
        ...
        mp4::ParseResult result1 = mp4::BoxReader::ReadTopLevelBox(second_moof_written_data.data() + mdat_video_data_offset - kBoxHeaderSize,
            second_moof_written_data.size() - mdat_video_data_offset + kBoxHeaderSize, nullptr, &mdat_reader);
...
TEST_F(Mp4MuxerDelegateTest, AddAudioFrame)
{
    ...
    media::AudioParameters params(media::AudioParameters::AUDIO_PCM_LOW_LATENCY, media::ChannelLayoutConfig::Stereo(), kAudioSampleRate, 1000);
    ...
    // 5 seconds.
    Mp4MuxerDelegate delegate(
        AudioCodec::kAAC, VideoCodec::kUnknown, std::nullopt, std::nullopt, base::BindLambdaForTesting([&](base::span<const uint8_t> mp4_data) {
            base::ranges::copy(mp4_data, std::back_inserter(total_written_data));
    ...
    delegate.AddAudioFrame(params, audio_stream, audio_codec_description, base_time_ticks);
    ...
    for (int i = 0; i < kAudioAdditionalSampleCount; ++i) {
        ...
        delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + delta);
        ++incremental_delta;
    }
...

    {
        ...
        std::unique_ptr<mp4::BoxReader> mdat_reader;
        // first_moof_written_data.data() is `moof` box start address.
        mp4::ParseResult result1 = mp4::BoxReader::ReadTopLevelBox(first_moof_written_data.data() + mdat_audio_data_offset - kBoxHeaderSize,
            first_moof_written_data.size() - mdat_audio_data_offset + kBoxHeaderSize, nullptr, &mdat_reader);
...
TEST_F(Mp4MuxerDelegateTest, AudioOnlyNewFragmentCreation)
{
    ...
    int callback_count = 0;
    Mp4MuxerDelegate delegate(AudioCodec::kAAC, VideoCodec::kUnknown, std::nullopt, std::nullopt,
        base::BindLambdaForTesting([&](base::span<const uint8_t> mp4_data) {
            ...
            case 4:
                ...
                break;
            case 5:
                base::ranges::copy(mp4_data, std::back_inserter(third_moof_written_data));
                run_loop.Quit();
            }
        }),
        10);
...

    {
        ...
        std::unique_ptr<mp4::BoxReader> moof_reader;
        mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(third_moof_written_data.data(), third_moof_written_data.size(), nullptr, &moof_reader);
        ...
        // `mdat` test.
        std::unique_ptr<mp4::BoxReader> mdat_reader;
        mp4::ParseResult result1 = mp4::BoxReader::ReadTopLevelBox(third_moof_written_data.data() + mdat_audio_data_offset - kBoxHeaderSize,
            third_moof_written_data.size() - mdat_audio_data_offset + kBoxHeaderSize, nullptr, &mdat_reader);
...
TEST_F(Mp4MuxerDelegateTest, AudioAndVideoAddition)
{
    ...
    int callback_count = 0;
    Mp4MuxerDelegate delegate(AudioCodec::kAAC, VideoCodec::kH264, std::nullopt, std::nullopt,
        base::BindLambdaForTesting([&](base::span<const uint8_t> mp4_data) {
            base::ranges::copy(mp4_data, std::back_inserter(total_written_data));
            ...
            case 5:
                ...
                break;
            case 6:
                base::ranges::copy(mp4_data, std::back_inserter(fourth_moof_written_data));
                // Quit.
                run_loop.Quit();
            }
        }),
    ...
    delegate.AddAudioFrame(params, audio_stream, code_description, base_time_ticks);
    // Total count is 24, which will have 3 fragments and the last fragment
    // has 4 samples.
    constexpr int kMaxAudioSampleFragment = 23;
    ...
    media::Muxer::VideoParameters video_params(gfx::Size(kWidth, kHeight), 30, VideoCodec::kH264, gfx::ColorSpace());
    video_stream->set_is_key_frame(true);
    delegate.AddVideoFrame(video_params, video_stream, video_code_description, base_time_ticks);
    for (int i = 0; i < kMaxAudioSampleFragment; ++i) {
        delta += base::Milliseconds(30);
        delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + delta);
    }
    ...
    delegate.AddVideoFrame(video_params, video_stream, video_code_description, base_time_ticks + base::Milliseconds(50));
    delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + delta + base::Milliseconds(30));
...

    {
        ...
        std::unique_ptr<mp4::BoxReader> moof_reader;
        mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(fourth_moof_written_data.data(), fourth_moof_written_data.size(), nullptr, &moof_reader);
        ...
        // video track.
        // `tfdt` test of video.
        EXPECT_EQ(ConvertToTimescale(base::Milliseconds(50), 30000), traf_boxes[1].decode_time.decode_time);
...
TEST_F(Mp4MuxerDelegateTest, MfraBoxOnAudioAndVideoAddition)
{
    ...
    media::Muxer::VideoParameters video_params(gfx::Size(kWidth, kHeight), 30, VideoCodec::kH264, gfx::ColorSpace());
    video_stream->set_is_key_frame(true);
    delegate.AddVideoFrame(video_params, video_stream, video_codec_description, base_time_ticks);
    for (int i = 0; i < kMaxAudioSampleFragment; ++i) {
        delta += base::Milliseconds(30);
        delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + delta);
    }
    video_stream->set_is_key_frame(true);
    // video stream, fourth fragment.
    delegate.AddVideoFrame(video_params, video_stream, video_codec_description, base_time_ticks + base::Milliseconds(50));
    delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + delta + base::Milliseconds(30));
    ...
    auto reader = base::SpanReader(base::span(total_written_data).subspan(track_id, 12u));
    ...
    reader = base::SpanReader(base::span(total_written_data).subspan(first_tfra_entry, kTfraEntrySize * 4u));
    uint64_t time;
    ...
    reader.ReadU32BigEndian(value);
    EXPECT_EQ(value, 1u); // sample number.
    EXPECT_EQ(base::span(total_written_data).subspan(moof_offset, third_moof_written_data.size()), third_moof_written_data);
    ...
    reader.ReadU32BigEndian(value);
    EXPECT_EQ(value, 1u); // sample number.
    EXPECT_EQ(base::span(total_written_data).subspan(fourth_moof_offset, fourth_moof_written_data.size()), fourth_moof_written_data);
}
...
TEST_F(Mp4MuxerDelegateTest, AudioVideoAndAudioVideoFragment)
{
    ...
    delegate.AddVideoFrame(video_params, video_stream, std::nullopt, base_time_ticks + kDelta);

    video_stream->set_is_key_frame(true);
    // The second fragment; video (1 sample) -> audio (2
    // samples) track.
    delegate.AddVideoFrame(video_params, video_stream, video_codec_description, base_time_ticks + kDelta * 2);
    delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + kDelta);
    delegate.AddAudioFrame(params, audio_stream, std::nullopt, base_time_ticks + kDelta * 2);
    // Write box data to the callback.
    delegate.Flush();
...
TEST_F(Mp4MuxerDelegateTest, ConvertedEncodedDataOnAvc1)
{
    ...
    Mp4MuxerDelegate delegate(AudioCodec::kAAC, VideoCodec::kH264, std::nullopt, std::nullopt,
        base::BindLambdaForTesting([&](base::span<const uint8_t> mp4_data) {
            ...
            case 3:
                ...
                base::ranges::copy(mp4_data, std::back_inserter(moof_and_mdat_written_data));
                run_loop.Quit();
                break;
            case 4:
                // MFRA data
    ...
    std::unique_ptr<mp4::BoxReader> moof_reader;
    mp4::ParseResult result = mp4::BoxReader::ReadTopLevelBox(moof_and_mdat_written_data.data(), moof_and_mdat_written_data.size(), nullptr, &moof_reader);
    EXPECT_EQ(result, mp4::ParseResult::kOk);
