...
MuxerTimestampAdapter::MuxerTimestampAdapter(std::unique_ptr<Muxer> muxer, bool has_video, bool has_audio)
...
bool MuxerTimestampAdapter::OnEncodedVideo(const Muxer::VideoParameters& params, scoped_refptr<DecoderBuffer> encoded_data,
    std::optional<media::VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    TRACE_EVENT2("media", __func__, "timestamp", timestamp - base::TimeTicks(), "is_key_frame", encoded_data->is_key_frame());
    DVLOG(2) << __func__ << " - " << encoded_data->AsHumanReadableString() << " ts " << timestamp;
    ...
    video_frames_.push_back(EncodedFrame {
        { params, std::move(codec_description), std::move(encoded_data) }, UpdateLastTimestampAndGetNext(last_video_timestamp_, timestamp_minus_paused) });
    return PartiallyFlushQueues();
}

bool MuxerTimestampAdapter::OnEncodedAudio(const AudioParameters& params, scoped_refptr<DecoderBuffer> encoded_data,
    std::optional<media::AudioEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    ...
    audio_frames_.push_back(EncodedFrame {
        { params, std::move(codec_description), std::move(encoded_data) }, UpdateLastTimestampAndGetNext(last_audio_timestamp_, timestamp_minus_paused) });
    return PartiallyFlushQueues();
}

void MuxerTimestampAdapter::SetLiveAndEnabled(bool track_live_and_enabled, bool is_video)
{
    bool& written_track_live_and_enabled = is_video ? video_track_live_and_enabled_ : audio_track_live_and_enabled_;
    if (written_track_live_and_enabled != track_live_and_enabled) {
        DVLOG(1) << __func__ << (is_video ? " video " : " audio ") << "track live-and-enabled changed to " << track_live_and_enabled;
    }
...
{
    while ((!video_frames_.empty() || !audio_frames_.empty()) && FlushNextFrame()) {
    }
}
...
bool MuxerTimestampAdapter::PartiallyFlushQueues()
{
    ...
    while ((!has_video_ || !video_frames_.empty() || !video_track_live_and_enabled_)
        && (!has_audio_ || !audio_frames_.empty() || !audio_track_live_and_enabled_) && result) {
...
bool MuxerTimestampAdapter::FlushNextFrame()
{
    ...
    bool take_video
        = !video_frames_.empty() && (audio_frames_.empty() || video_frames_.front().timestamp_minus_paused <= audio_frames_.front().timestamp_minus_paused);
    ...
    base::TimeDelta relative_timestamp = frame.timestamp_minus_paused - first_timestamp_;
    DLOG_IF(WARNING, relative_timestamp < last_timestamp_written_)
        << "Enforced a monotonically increasing timestamp. Last written " << last_timestamp_written_ << " new " << relative_timestamp;
    ...
    TRACE_EVENT2("media", __func__, "is_video", take_video, "recorded_timestamp", relative_timestamp);
    return muxer_->PutFrame(std::move(frame.frame), relative_timestamp);
}

base::TimeTicks MuxerTimestampAdapter::UpdateLastTimestampAndGetNext(std::optional<base::TimeTicks>& last_timestamp, base::TimeTicks timestamp)
{
    ...
    // MP4.
    DLOG_IF(WARNING, timestamp < last_timestamp) << "Encountered a non-monotonically increasing timestamp. Was: " << *last_timestamp
                                                 << ", timestamp: " << timestamp;
...
MuxerTimestampAdapter::EncodedFrame::EncodedFrame(Muxer::EncodedFrame frame, base::TimeTicks timestamp_minus_paused)
    : frame(std::move(frame))
    , timestamp_minus_paused(timestamp_minus_paused)
{
}
MuxerTimestampAdapter::EncodedFrame::~EncodedFrame() = default;
