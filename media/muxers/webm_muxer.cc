...
constexpr base::TimeDelta kMinimumForcedClusterDuration = base::Milliseconds(100);
...
void WriteOpusHeader(const AudioParameters& params, uint8_t* header)
{
    ...
    memcpy(header + OPUS_EXTRADATA_SAMPLE_RATE_OFFSET, &sample_rate, sizeof(uint32_t));
...
static double GetFrameRate(const Muxer::VideoParameters& params)
{
    ...
    if (frame_rate <= kZeroFrameRate || frame_rate > limits::kMaxFramesPerSecond) {
...
std::optional<mkvmuxer::Colour> ColorFromColorSpace(const gfx::ColorSpace& color)
{
...
mkvmuxer::int32 WebmMuxer::Delegate::Write(const void* buf, mkvmuxer::uint32 len)
{
...
WebmMuxer::WebmMuxer(
    AudioCodec audio_codec, bool has_video, bool has_audio, std::unique_ptr<Delegate> delegate, std::optional<base::TimeDelta> max_data_output_interval)
    : audio_codec_(audio_codec)
    , has_video_(has_video)
    , has_audio_(has_audio)
    , max_data_output_interval_(std::max(max_data_output_interval.value_or(base::TimeDelta()), kMinimumForcedClusterDuration))
    , delegate_(std::move(delegate))
{
    DCHECK(has_video_ || has_audio_);
    DCHECK(delegate_);
    DCHECK(audio_codec == AudioCodec::kOpus || audio_codec == AudioCodec::kPCM) << " Unsupported audio codec: " << GetCodecName(audio_codec);
...
void WebmMuxer::AddVideoTrack(const gfx::Size& frame_size, double frame_rate, const std::optional<gfx::ColorSpace>& color_space)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(0u, video_track_index_) << "WebmMuxer can only be initialized once.";
    ...
    mkvmuxer::VideoTrack* const video_track = reinterpret_cast<mkvmuxer::VideoTrack*>(segment_.GetTrackByNumber(video_track_index_));
    if (color_space) {
...
void WebmMuxer::AddAudioTrack(const AudioParameters& params)
{
    ...
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(0u, audio_track_index_) << "WebmMuxer audio can only be initialised once.";
    ...
    mkvmuxer::AudioTrack* const audio_track = reinterpret_cast<mkvmuxer::AudioTrack*>(segment_.GetTrackByNumber(audio_track_index_));
    DCHECK(audio_track);
    DCHECK_EQ(params.sample_rate(), audio_track->sample_rate());
    DCHECK_EQ(params.channels(), static_cast<int>(audio_track->channels()));
    DCHECK_LE(params.channels(), 2) << "Only 1 or 2 channels supported, requested " << params.channels();
...
bool WebmMuxer::PutFrame(EncodedFrame frame, base::TimeDelta relative_timestamp)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    AudioParameters* audio_params = absl::get_if<AudioParameters>(&frame.params);
    TRACE_EVENT2("media", __func__, "timestamp", relative_timestamp, "is_keyframe", frame.data->is_key_frame());
    DVLOG(2) << __func__ << " - " << (audio_params ? "A " : "V ") << frame.data->size() << "B ts " << relative_timestamp;
    if (audio_params) {
        ...
        CHECK(video_params);
        DCHECK(video_params->codec == VideoCodec::kVP8 || video_params->codec == VideoCodec::kVP9 || video_params->codec == VideoCodec::kH264
        ...
        DCHECK(video_codec_ == VideoCodec::kUnknown || video_codec_ == video_params->codec)
            << "Unsupported: codec switched, to: " << GetCodecName(video_params->codec);
        ...
        if (!video_track_index_) {
            ...
            AddVideoTrack(video_params->visible_rect_size, GetFrameRate(*video_params), video_params->color_space);

            // Add codec private for AV1.
            if (video_params->codec == VideoCodec::kAV1
                && !segment_.GetTrackByNumber(video_track_index_)->SetCodecPrivate(av1::codec_private, sizeof(av1::codec_private))) {
    ...
    if ((has_video_ && !video_track_index_) || (has_audio_ && !audio_track_index_)) {
        return true;
    }
...
bool WebmMuxer::WriteWebmFrame(EncodedFrame frame, base::TimeDelta relative_timestamp)
{
    ...
    uint8_t track_index = absl::get_if<AudioParameters>(&frame.params) ? audio_track_index_ : video_track_index_;
    return frame.data->has_side_data() && !frame.data->side_data()->alpha_data.empty()
        ? segment_.AddFrameWithAdditional(frame.data->data(), frame.data->size(), frame.data->side_data()->alpha_data.data(),
            frame.data->side_data()->alpha_data.size(), /*add_id=*/1, track_index, recorded_timestamp, frame.data->is_key_frame())
        : segment_.AddFrame(frame.data->data(), frame.data->size(), track_index, recorded_timestamp, frame.data->is_key_frame());
}
...
void WebmMuxer::MaybeForceNewCluster()
{
    ...
    if (!has_video_ || max_data_output_interval_.is_zero() || delegate_->last_data_output_timestamp().is_null()) {
        return;
    }

    // TODO(crbug.com/40876732): consider if cluster output should be based on
    // media timestamps
    if (base::TimeTicks::Now() - delegate_->last_data_output_timestamp() >= max_data_output_interval_) {
