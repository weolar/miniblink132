...
LiveWebmMuxerDelegate::LiveWebmMuxerDelegate(Muxer::WriteDataCB write_data_callback)
...
void LiveWebmMuxerDelegate::ElementStartNotify(mkvmuxer::uint64 element_id, mkvmuxer::int64 position)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    // This method gets pinged before items are sent to
    // |WebmMuxerDelegate::Write()|.
    DCHECK_GE(position, position_.ValueOrDefault(0)) << "Can't go back in a live WebM stream.";
}

mkvmuxer::int32 LiveWebmMuxerDelegate::DoWrite(const void* buf, mkvmuxer::uint32 len)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    write_data_callback_.Run(
        // SAFETY: buf is a pointer that points to exactly len length.
        UNSAFE_BUFFERS(base::span<const uint8_t>(reinterpret_cast<const uint8_t*>(buf), len)));
    return 0;
}
