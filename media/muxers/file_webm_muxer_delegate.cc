...
void FileWebmMuxerDelegate::ElementStartNotify(mkvmuxer::uint64 element_id, mkvmuxer::int64 position)
{
}

mkvmuxer::int32 FileWebmMuxerDelegate::DoWrite(const void* buf, mkvmuxer::uint32 len)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    const bool success = webm_file_.WriteAtCurrentPosAndCheck(base::as_bytes(base::make_span(static_cast<const uint8_t*>(buf), len)));
