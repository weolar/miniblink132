...
void BuildTrack(
    mp4::writable_boxes::Movie& moov, size_t track_index, bool is_audio, uint32_t timescale, const mp4::writable_boxes::SampleDescription& sample_description)
{
    mp4::writable_boxes::Track& track = moov.tracks[track_index];
    // `tkhd`.
    mp4::writable_boxes::TrackHeader track_header(track_index + 1, is_audio);
    track_header.flags = BuildFlags<TrackHeaderFlags>({ TrackHeaderFlags::kTrackEnabled, TrackHeaderFlags::kTrackInMovie });
    track.header = std::move(track_header);

    // `mdhd`
    track.media.header.timescale = timescale;
    track.media.header.language = kUndefinedLanguageName; // use 'und' as default at this time.
    ...
    // `trex`.
    mp4::writable_boxes::TrackExtends& audio_extends = moov.extends.track_extends[track_index];
    ...
    sample_table.decoding_time_to_sample = mp4::writable_boxes::DecodingTimeToSample();
...
void CopyCreationTimeAndDuration(mp4::writable_boxes::Track& track, const mp4::writable_boxes::MovieHeader& header, base::TimeDelta track_duration)
{
...
Mp4MuxerDelegate::Mp4MuxerDelegate(AudioCodec audio_codec, VideoCodec video_codec, std::optional<VideoCodecProfile> video_profile,
    std::optional<VideoCodecLevel> video_level, Muxer::WriteDataCB write_callback, size_t audio_sample_count_per_fragment)
...
void Mp4MuxerDelegate::AddVideoFrame(const Muxer::VideoParameters& params, scoped_refptr<DecoderBuffer> encoded_data,
    std::optional<VideoEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    DVLOG(1) << __func__ << ", " << params.AsHumanReadableString();

    if (!video_track_index_.has_value()) {
        CHECK(codec_description.has_value() || (params.codec != VideoCodec::kH264 && params.codec != VideoCodec::kHEVC));
...
void Mp4MuxerDelegate::BuildMovieVideoTrack(
    const Muxer::VideoParameters& params, const DecoderBuffer& encoded_data, std::optional<VideoEncoder::CodecDescription> codec_description)
{
    ...
    visual_sample_entry.pixel_aspect_ratio = mp4::writable_boxes::PixelAspectRatioBox();
    ...
    if (video_codec_ == VideoCodec::kH264) {
        ...
        bool result = avc_config_record.Parse(codec_description.value().data(), codec_description.value().size());
        DCHECK(result);
        ...
        bool result = hevc_config_record.Parse(codec_description.value().data(), codec_description.value().size());
        DCHECK(result);
        ...
        mp4::writable_boxes::VPCodecConfiguration vp_config(video_profile_.value_or(VP9PROFILE_PROFILE0), video_level_.value_or(0), color_space);
        ...
        auto codec_descriptions = libgav1::ObuParser::GetAV1CodecConfigurationBox(encoded_data.data(), encoded_data.size(), &config_size);
        CHECK(codec_descriptions);
        CHECK_GT(config_size, 0u);

        av1_config.av1_decoder_configuration_data.assign(&codec_descriptions[0], &codec_descriptions[config_size]);
    ...
    BuildTrack(*moov_, video_track_index_.value(), false, context_->GetVideoTrack().value().timescale, description);

    mp4::writable_boxes::Track& video_track = moov_->tracks[video_track_index_.value()];
...
void Mp4MuxerDelegate::AddDataToVideoFragment(scoped_refptr<DecoderBuffer> encoded_data)
{
    DCHECK(video_track_index_.has_value());
...
void Mp4MuxerDelegate::AddAudioFrame(const AudioParameters& params, scoped_refptr<DecoderBuffer> encoded_data,
    std::optional<AudioEncoder::CodecDescription> codec_description, base::TimeTicks timestamp)
{
    if (!audio_track_index_.has_value()) {
        ...
        context_->SetAudioTrack({ audio_track_index_.value(), static_cast<uint32_t>(audio_sample_rate_) });
...
void Mp4MuxerDelegate::BuildMovieAudioTrack(
    const AudioParameters& params, const DecoderBuffer& encoded_data, std::optional<AudioEncoder::CodecDescription> codec_description)
{
    ...
    mp4::writable_boxes::SampleDescription description;
    mp4::writable_boxes::AudioSampleEntry audio_sample_entry(audio_codec_, audio_sample_rate_, params.channels());

    if (audio_codec_ == AudioCodec::kAAC) {
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
        mp4::writable_boxes::ElementaryStreamDescriptor elementary_stream_descriptor;
        elementary_stream_descriptor.aac_codec_description = std::move(codec_description.value());
        audio_sample_entry.elementary_stream_descriptor = std::move(elementary_stream_descriptor);
#else
        NOTREACHED();
#endif
    } else {
    ...
    BuildTrack(*moov_, audio_track_index_.value(), true, context_->GetAudioTrack().value().timescale, description);

    mp4::writable_boxes::Track& audio_track = moov_->tracks[audio_track_index_.value()];
...
void Mp4MuxerDelegate::AddDataToAudioFragment(scoped_refptr<DecoderBuffer> encoded_data)
{
    DCHECK(audio_track_index_.has_value());
...
bool Mp4MuxerDelegate::FlushFragment()
{
    ...
    if (!Flush()) {
        DVLOG(1) << __func__ << "flush fragment failed, it could be the first video frame";
        return false;
    }
...
void Mp4MuxerDelegate::MaybeFlushMoofAndMfraBoxes(size_t written_offset)
{
    ...
    for (auto& fragment : fragments_) {
        if (video_track_index_.has_value()) {
            ...
            fragment->AddVideoLastTimestamp(base::Milliseconds(kMillisecondsTimeScale / video_frame_rate_));
        }

        if (audio_track_index_.has_value()) {
            CHECK_NE(audio_sample_rate_, 0);
            int audio_frame_rate = audio_sample_rate_ / kAudioSamplesPerFrame;
            fragment->AddAudioLastTimestamp(base::Milliseconds(kMillisecondsTimeScale / audio_frame_rate));
        }
    }
    ...
    for (auto& fragment : fragments_) {
        if (video_track_index_.has_value() && !live_mode_) {
            ...
            BuildVideoTrackFragmentRandomAccess(start_timestamp, video_track_random_access, written_offset);
        }
        ...
        Mp4MovieFragmentBoxWriter fragment_box_writer(*context_, fragment->GetMovieFragment());
    ...
    if (video_track_index_.has_value()) {
        ...
        fragment_random_access.tracks.emplace_back(std::move(video_track_random_access));
        if (video_track_index_.value() == 0) {
            std::swap(fragment_random_access.tracks[kDefaultAudioIndex], fragment_random_access.tracks[kDefaultVideoIndex]);
        }

        // Flush at requested.
        Mp4FragmentRandomAccessBoxWriter fragment_random_access_box_writer(*context_, fragment_random_access);
...
void Mp4MuxerDelegate::BuildFileTypeBox(mp4::writable_boxes::FileType& mp4_file_type_box)
{
...
void Mp4MuxerDelegate::BuildMovieBox()
{
    ...
    if (video_track_index_.has_value()) {
        CHECK_NE(video_frame_rate_, 0);
        base::TimeTicks last_video_time_include_last_sample = last_video_time_ + base::Milliseconds(kMillisecondsTimeScale / video_frame_rate_);
        video_track_duration = last_video_time_include_last_sample - start_video_time_;
        CopyCreationTimeAndDuration(moov_->tracks[video_track_index_.value()], moov_->header, video_track_duration);
    }
    ...
    if (audio_track_index_.has_value()) {
        ...
        int audio_frame_rate = audio_sample_rate_ / kAudioSamplesPerFrame;
        base::TimeTicks last_audio_time_include_last_sample = last_audio_time_ + base::Milliseconds(kMillisecondsTimeScale / audio_frame_rate);
        audio_track_duration = last_audio_time_include_last_sample - start_audio_time_;
        CopyCreationTimeAndDuration(moov_->tracks[audio_track_index_.value()], moov_->header, audio_track_duration);
    }
...
void Mp4MuxerDelegate::BuildVideoTrackFragmentRandomAccess(
    base::TimeTicks start_video_time, mp4::writable_boxes::TrackFragmentRandomAccess& fragment_random_access_box_writer, size_t written_offset)
{
...
void Mp4MuxerDelegate::CreateFragmentIfNeeded(bool audio, bool is_key_frame)
{
    ...
    if (!create_fragment && is_audio_only) {
        ...
        // limit.
        Mp4MuxerDelegateFragment* fragment = fragments_.back().get();
        create_fragment = fragment->GetAudioSampleSize() >= audio_sample_count_per_fragment_;
    }
    ...
    int video_track_id = video_track_index_.has_value() ? *video_track_index_ + 1 : 2;
    int audio_track_id = audio_track_index_.has_value() ? *audio_track_index_ + 1 : 2;
    auto new_fragment = std::make_unique<Mp4MuxerDelegateFragment>(*context_, video_track_id, audio_track_id, sequence_number_++);
...
void Mp4MuxerDelegate::EnsureInitialized()
{
    ...
    auto output_position_tracker = std::make_unique<OutputPositionTracker>(write_callback_);

    context_ = std::make_unique<Mp4MuxerContext>(std::move(output_position_tracker));
    ...
    moov_->extends.track_extends.emplace_back(mp4::writable_boxes::TrackExtends());
    moov_->extends.track_extends.emplace_back(mp4::writable_boxes::TrackExtends());
}

#if BUILDFLAG(USE_PROPRIETARY_CODECS) || BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
scoped_refptr<DecoderBuffer> Mp4MuxerDelegate::ConvertNALUData(scoped_refptr<DecoderBuffer> encoded_data)
{
    CHECK(video_codec_ == VideoCodec::kH264 || video_codec_ == VideoCodec::kHEVC);
    if (!h26x_converter_) {
        h26x_converter_ = std::make_unique<H26xAnnexBToBitstreamConverter>(video_codec_);
    }
...
void Mp4MuxerDelegate::LogBoxInfo() const
{
    ...
    s << "movie timescale:" << moov_->header.timescale << ", duration in seconds:" << moov_->header.duration.InSeconds();
    if (video_track_index_.has_value()) {
        mp4::writable_boxes::Track& track = moov_->tracks[video_track_index_.value()];
        s << ", video track index:" << video_track_index_.value() << ", video track timescale:" << context_->GetVideoTrack().value().timescale
          << ", video track duration:" << track.header.duration;
    }

    if (audio_track_index_.has_value()) {
        mp4::writable_boxes::Track& track = moov_->tracks[audio_track_index_.value()];
        s << ", audio track index:" << audio_track_index_.value() << ", audio track timescale:" << context_->GetAudioTrack().value().timescale
