...
void BoxByteStream::StartFullBox(mp4::FourCC fourcc, uint32_t flags, uint8_t version)
{
    StartBox(fourcc);
...
void BoxByteStream::EndBox()
{
    ...
    WriteSize(position_ - size_offset, base::span(buffer_).subspan(size_offset).first<4>());
}
