...
using MediaTimestamps = std::vector<std::pair</*media type*/ int, /*timestamp_ms*/ int>>;
...
public:
    ...
    MOCK_METHOD(bool, PutFrame, (EncodedFrame frame, base::TimeDelta relative_timestamp), (override));
};

class SuccessfulMuxer : public Muxer {
public:
    SuccessfulMuxer(std::unique_ptr<MockMuxer> muxer, MediaTimestamps& put_timestamps)
        : muxer_(std::move(muxer))
        , put_timestamps_(&put_timestamps)
    {
    }
    ...
    bool PutFrame(EncodedFrame frame, base::TimeDelta relative_timestamp) override
    {
        int media_type = absl::get_if<AudioParameters>(&frame.params) ? kAudio : kVideo;
        put_timestamps_->emplace_back(media_type, relative_timestamp.InMilliseconds());
...
public:
    struct Frame {
        ...
        {
            data->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(base::as_byte_span(v));
            return *this;
        }
        Frame& AsKeyframe()
        {
        ...
        scoped_refptr<media::DecoderBuffer> data = media::DecoderBuffer::CopyFrom(base::as_byte_span("data"));
    ...
    void CreateAdapter(bool has_video, bool has_audio)
    {
        ...
        adapter_ = std::make_unique<MuxerTimestampAdapter>(std::move(muxer_holder), has_video, has_audio);
    }
    ...
    void CreateAdapterWithSuccessfulPut(bool has_video, bool has_audio)
    {
        auto muxer_holder = std::make_unique<MockMuxer>();
        muxer_ = muxer_holder.get();
        auto successful_muxer = std::make_unique<SuccessfulMuxer>(std::move(muxer_holder), put_timestamps_);
        adapter_ = std::make_unique<MuxerTimestampAdapter>(std::move(successful_muxer), has_video, has_audio);
    }

    bool PutAudioFrame(const Frame& frame)
    {
        media::AudioParameters audio_params(media::AudioParameters::Format::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Mono(),
            /*sample_rate=*/48000,
            /*frames_per_buffer=*/480);
        return adapter_->OnEncodedAudio(audio_params, frame.data, std::nullopt, frame.timestamp);
    }

    bool PutVideoFrame(const Frame& frame)
    {
        auto video_params = Muxer::VideoParameters(*VideoFrame::CreateBlackFrame(gfx::Size(160, 80)));
        return adapter_->OnEncodedVideo(video_params, frame.data, std::nullopt, frame.timestamp);
    }
...
class MuxerTimestampAdapterTest : public MuxerTimestampAdapterTestBase, public ::testing::Test { };
...
MATCHER_P2(MatchBufferDataAndAlphaData, data, alpha_data, "decoderbuffer data matcher")
{
    return arg.data->AsSpan() == base::as_byte_span(data) && arg.data->WritableSideData().alpha_data == base::as_byte_span(alpha_data);
}
...
TEST_F(MuxerTimestampAdapterTest, ForwardsVideoSamples)
{
    ...
    EXPECT_CALL(*muxer_, PutFrame(MatchBufferDataAndAlphaData(str1, alpha_data1), base::Milliseconds(0)));
    EXPECT_CALL(*muxer_, PutFrame(MatchBufferDataAndAlphaData(str2, alpha_data2), base::Milliseconds(1)));
    PutVideoFrame(Frame().WithData(str1).WithAlphaData(alpha_data1).WithTimestamp(2));
    PutVideoFrame(Frame().WithData(str2).WithAlphaData(alpha_data2).WithTimestamp(3));
}
...
TEST_F(MuxerTimestampAdapterTest, ForwardsAudioVideoSamples)
{
    ...
    PutVideoFrame(Frame().WithData(str1).WithAlphaData(alpha_data1).WithTimestamp(3));
...
TEST_F(MuxerTimestampAdapterTest, VideoKeptWhileWaitingForAudio)
{
    ...
    EXPECT_CALL(*muxer_, PutFrame(Field(&Muxer::EncodedFrame::params, VariantWith<Muxer::VideoParameters>(_)), base::Milliseconds(0)));
    EXPECT_CALL(*muxer_, PutFrame(Field(&Muxer::EncodedFrame::params, VariantWith<Muxer::VideoParameters>(_)), base::Milliseconds(1)));
    EXPECT_CALL(*muxer_, PutFrame(Field(&Muxer::EncodedFrame::params, VariantWith<Muxer::VideoParameters>(_)), base::Milliseconds(2)));
    EXPECT_CALL(*muxer_, PutFrame(Field(&Muxer::EncodedFrame::params, VariantWith<AudioParameters>(_)), base::Milliseconds(3)));
...
TEST_F(MuxerTimestampAdapterTest, CompensatesForPausedTimeWithVideo)
{
    ...
    muxer_ = nullptr;
    adapter_ = nullptr;
    EXPECT_THAT(put_timestamps_, ElementsAre(Pair(kVideo, 0), Pair(kVideo, /*266 - 200=*/66)));
}
...
TEST_F(MuxerTimestampAdapterTest, CompensatesForPausedTimeWithAudio)
{
    ...
    muxer_ = nullptr;
    adapter_ = nullptr;
    EXPECT_THAT(put_timestamps_, ElementsAre(Pair(kAudio, 0), Pair(kAudio, /*686 - 666=*/20)));
}
...
TEST_F(MuxerTimestampAdapterTest, CompensatesForPausedTimeWithAudioAndVideo)
{
    ...
    muxer_ = nullptr;
    adapter_ = nullptr;
    EXPECT_THAT(put_timestamps_, ElementsAre(Pair(kAudio, 0), Pair(kVideo, 1), Pair(kVideo, /*315 - 300=*/15), Pair(kAudio, /*321 - 300=*/21)));
}
...
class MuxerTimestampAdapterParametrizedTest : public MuxerTimestampAdapterTestBase, public ::testing::TestWithParam<TestParams> {
public:
...
INSTANTIATE_TEST_SUITE_P(, MuxerTimestampAdapterParametrizedTest, ::testing::ValuesIn(kTestCases));
