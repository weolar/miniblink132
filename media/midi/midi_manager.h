...
public:
    ...
    virtual void SetInputPortState(uint32_t port_index, mojom::PortState state) = 0;
    virtual void SetOutputPortState(uint32_t port_index, mojom::PortState state) = 0;
    ...
    virtual void ReceiveMidiData(uint32_t port_index, const uint8_t* data, size_t length, base::TimeTicks timestamp) = 0;
...
public:
    ...
    // platforms.
    virtual void DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp);
...
protected:
    ...
    void ReceiveMidiData(uint32_t port_index, const uint8_t* data, size_t length, base::TimeTicks time);
...
private:
    ...
    std::set<raw_ptr<MidiManagerClient, SetExperimental>> pending_clients_ GUARDED_BY(lock_);

    // Keeps track of all clients who wish to receive MIDI data.
    std::set<raw_ptr<MidiManagerClient, SetExperimental>> clients_ GUARDED_BY(lock_);
    ...
    scoped_refptr<base::SingleThreadTaskRunner> session_thread_runner_ GUARDED_BY(lock_);
