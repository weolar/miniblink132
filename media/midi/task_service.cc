...
void TaskService::PostBoundTask(RunnerId runner_id, base::OnceClosure task)
{
    ...
    GetTaskRunner(runner_id)->PostTask(FROM_HERE, base::BindOnce(&TaskService::RunTask, base::Unretained(this), instance_id, runner_id, std::move(task)));
}

void TaskService::PostBoundDelayedTask(RunnerId runner_id, base::OnceClosure task, base::TimeDelta delay)
{
    InstanceId instance_id;
    {
        base::AutoLock lock(lock_);
    ...
    GetTaskRunner(runner_id)->PostDelayedTask(
        FROM_HERE, base::BindOnce(&TaskService::RunTask, base::Unretained(this), instance_id, runner_id, std::move(task)), delay);
}
...
scoped_refptr<base::SingleThreadTaskRunner> TaskService::GetTaskRunner(RunnerId runner_id)
{
    base::AutoLock lock(lock_);
    if (runner_id == kDefaultRunnerId)
    ...
    size_t thread = runner_id - 1;
    if (!threads_[thread]) {
        threads_[thread] = std::make_unique<base::Thread>(base::StringPrintf("MidiService_TaskService_Thread(%zu)", runner_id));
...
void TaskService::RunTask(InstanceId instance_id, RunnerId runner_id, base::OnceClosure task)
{
    {
        base::AutoLock tasks_in_flight_lock(tasks_in_flight_lock_);
        ++tasks_in_flight_;
