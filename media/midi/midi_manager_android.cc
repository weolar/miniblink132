...
bool HasSystemFeatureMidi()
{
    ...
    return Java_MidiManagerAndroid_hasSystemFeatureMidi(jni_zero::AttachCurrentThread());
}
...
{
    const BackendType type = HasSystemFeatureMidi() ? BackendType::kAndroidMidi : BackendType::kAndroidUsb;
    ...
    return new MidiManagerUsb(service, std::make_unique<UsbMidiDeviceFactoryAndroid>());
}
...
void MidiManagerAndroid::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    ...
    service()->task_service()->PostBoundDelayedTask(
        TaskService::kDefaultRunnerId, base::BindOnce(&MidiOutputPortAndroid::Send, base::Unretained(all_output_ports_[port_index]), data), delay);
    service()->task_service()->PostBoundDelayedTask(
        TaskService::kDefaultRunnerId, base::BindOnce(&MidiManagerAndroid::AccumulateMidiBytesSent, base::Unretained(this), client, data.size()), delay);
}

void MidiManagerAndroid::OnReceivedData(MidiInputPortAndroid* port, const uint8_t* data, size_t size, base::TimeTicks timestamp)
{
...
void MidiManagerAndroid::OnInitialized(JNIEnv* env, const JavaParamRef<jobjectArray>& devices)
{
    for (auto raw_device : devices.ReadElements<jobject>()) {
        AddDevice(std::make_unique<MidiDeviceAndroid>(env, raw_device, this));
    }
    service()->task_service()->PostBoundTask(
        TaskService::kDefaultRunnerId, base::BindOnce(&MidiManagerAndroid::CompleteInitialization, base::Unretained(this), Result::OK));
}

void MidiManagerAndroid::OnInitializationFailed(JNIEnv* env)
{
    service()->task_service()->PostBoundTask(
        TaskService::kDefaultRunnerId, base::BindOnce(&MidiManagerAndroid::CompleteInitialization, base::Unretained(this), Result::INITIALIZATION_ERROR));
}

void MidiManagerAndroid::OnAttached(JNIEnv* env, const JavaParamRef<jobject>& raw_device)
{
    AddDevice(std::make_unique<MidiDeviceAndroid>(env, raw_device, this));
}

void MidiManagerAndroid::OnDetached(JNIEnv* env, const JavaParamRef<jobject>& raw_device)
{
    for (auto& device : devices_) {
        if (device->HasRawDevice(env, raw_device)) {
            for (auto& port : device->input_ports()) {
                DCHECK(input_port_to_index_.end() != input_port_to_index_.find(port.get()));
            ...
            for (auto& port : device->output_ports()) {
                DCHECK(output_port_to_index_.end() != output_port_to_index_.find(port.get()));
...
void MidiManagerAndroid::AddDevice(std::unique_ptr<MidiDeviceAndroid> device)
{
    for (auto& port : device->input_ports()) {
        ...
        const std::string id(base::StringPrintf("native:port-in-%ld", static_cast<long>(index)));

        input_port_to_index_.insert(std::make_pair(port.get(), index));
        AddInputPort(mojom::PortInfo(id, device->GetManufacturer(), device->GetProductName(), device->GetDeviceVersion(), state));
    }
    for (auto& port : device->output_ports()) {
        ...
        // sufficiently unique although there is no user-friendly meaning.
        // TODO(yhirano): Use a hashed string as ID.
        const std::string id(base::StringPrintf("native:port-out-%ld", static_cast<long>(index)));

        output_port_to_index_.insert(std::make_pair(port.get(), index));
        AddOutputPort(mojom::PortInfo(id, device->GetManufacturer(), device->GetProductName(), device->GetDeviceVersion(), PortState::CONNECTED));
    }
