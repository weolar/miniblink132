...
std::unique_ptr<MidiManager> MidiService::ManagerFactory::Create(MidiService* service)
{
    return std::unique_ptr<MidiManager>(MidiManager::Create(service));
}

// static
base::TimeDelta MidiService::TimestampToTimeDeltaDelay(base::TimeTicks timestamp)
{
...
void MidiService::StartSession(MidiManagerClient* client)
{
    ...
    if (!manager_) {
        ...
        DCHECK(!manager_destructor_runner_);
        manager_destructor_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
    }
...
void MidiService::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    base::AutoLock lock(lock_);
...
scoped_refptr<base::SingleThreadTaskRunner> MidiService::GetTaskRunner(size_t runner_id)
{
    base::AutoLock lock(threads_lock_);
    if (threads_.size() <= runner_id)
        threads_.resize(runner_id + 1);
    if (!threads_[runner_id]) {
        threads_[runner_id] = std::make_unique<base::Thread>(base::StringPrintf("MidiServiceThread(%zu)", runner_id));
#if BUILDFLAG(IS_WIN)
