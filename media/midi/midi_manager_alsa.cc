...
enum { kDefaultRunnerNotUsedOnAlsa = TaskService::kDefaultRunnerId, kEventTaskRunner, kSendTaskRunner };
...
constexpr unsigned int kRequiredInputPortCaps = SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ;
constexpr unsigned int kRequiredOutputPortCaps = SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE;

constexpr unsigned int kCreateOutputPortCaps = SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_NO_EXPORT;
constexpr unsigned int kCreateInputPortCaps = SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_NO_EXPORT;
constexpr unsigned int kCreatePortType = SND_SEQ_PORT_TYPE_MIDI_GENERIC | SND_SEQ_PORT_TYPE_APPLICATION;
...
const std::string UdevDeviceGetPropertyOrSysattr(struct udev_device* udev_device, const char* property_key, const char* sysattr_key)
{
...
std::string GetVendor(udev_device* dev)
{
    ...
    std::string vendor = device::UdevDecodeString(device::UdevDeviceGetPropertyValue(dev, kUdevIdVendorEnc));
...
void SetStringIfNonEmpty(base::Value::Dict* value, const std::string& path, const std::string& in_value)
{
    if (!in_value.empty())
...
void MidiManagerAlsa::StartInitialization()
{
    ...
    // Create input port.
    in_port_id_ = snd_seq_create_simple_port(in_client_.get(), NULL, kCreateInputPortCaps, kCreatePortType);
    if (in_port_id_ < 0) {
        VLOG(1) << "snd_seq_create_simple_port fails: " << snd_strerror(in_port_id_);
        return CompleteInitialization(Result::INITIALIZATION_ERROR);
    }
    ...
    if (err != 0) {
        VLOG(1) << "snd_seq_subscribe_port on the announce port fails: " << snd_strerror(err);
        return CompleteInitialization(Result::INITIALIZATION_ERROR);
    }
    ...
    udev_monitor_ = device::ScopedUdevMonitorPtr(device::udev_monitor_new_from_netlink(udev_.get(), kUdev));
    if (!udev_monitor_.get()) {
        VLOG(1) << "udev_monitor_new_from_netlink fails";
        return CompleteInitialization(Result::INITIALIZATION_ERROR);
    }
    err = device::udev_monitor_filter_add_match_subsystem_devtype(udev_monitor_.get(), kUdevSubsystemSound, nullptr);
    if (err != 0) {
        VLOG(1) << "udev_monitor_add_match_subsystem fails: " << base::safe_strerror(-err);
        return CompleteInitialization(Result::INITIALIZATION_ERROR);
    }
    err = device::udev_monitor_enable_receiving(udev_monitor_.get());
    if (err != 0) {
        VLOG(1) << "udev_monitor_enable_receiving fails: " << base::safe_strerror(-err);
        return CompleteInitialization(Result::INITIALIZATION_ERROR);
    }
    ...
    // ALSA and udev.
    service()->task_service()->PostBoundTask(kEventTaskRunner, base::BindOnce(&MidiManagerAlsa::EventLoop, base::Unretained(this)));

    CompleteInitialization(Result::OK);
}

void MidiManagerAlsa::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    service()->task_service()->PostBoundDelayedTask(kSendTaskRunner,
        base::BindOnce(&MidiManagerAlsa::SendMidiData, base::Unretained(this), client, port_index, data), MidiService::TimestampToTimeDeltaDelay(timestamp));
}

MidiManagerAlsa::MidiPort::Id::Id() = default;

MidiManagerAlsa::MidiPort::Id::Id(
    const std::string& bus, const std::string& vendor_id, const std::string& model_id, const std::string& usb_interface_num, const std::string& serial)
    : bus_(bus)
...
{
    return (bus_ == rhs.bus_) && (vendor_id_ == rhs.vendor_id_) && (model_id_ == rhs.model_id_) && (usb_interface_num_ == rhs.usb_interface_num_)
        && (serial_ == rhs.serial_);
}
...
MidiManagerAlsa::MidiPort::MidiPort(const std::string& path, const Id& id, int client_id, int port_id, int midi_device, const std::string& client_name,
    const std::string& port_name, const std::string& manufacturer, const std::string& version, Type type)
    : id_(id)
...
bool MidiManagerAlsa::MidiPort::MatchConnected(const MidiPort& query) const
{
    ...
    // client_name
    // port_name
    return connected() && (type() == query.type()) && (path() == query.path()) && (id() == query.id()) && (client_id() == query.client_id())
        && (port_id() == query.port_id()) && (midi_device() == query.midi_device()) && (client_name() == query.client_name())
...
bool MidiManagerAlsa::MidiPort::MatchNoCardPass2(const MidiPort& query) const
{
    ...
    // port_name
    // midi_device == -1, for both this and query
    return !connected() && (type() == query.type()) && path().empty() && query.path().empty() && id().empty() && query.id().empty()
        && (port_id() == query.port_id()) && (client_name() == query.client_name()) && (port_name() == query.port_name()) && (midi_device() == -1)
        && (query.midi_device() == -1);
}

MidiManagerAlsa::MidiPortStateBase::~MidiPortStateBase() = default;

MidiManagerAlsa::MidiPortStateBase::iterator MidiManagerAlsa::MidiPortStateBase::Find(const MidiManagerAlsa::MidiPort& port)
{
...
MidiManagerAlsa::MidiPortStateBase::iterator MidiManagerAlsa::MidiPortStateBase::FindConnected(const MidiManagerAlsa::MidiPort& port)
{
    // Exact match required for connected ports.
    return base::ranges::find_if(ports_, [&port](std::unique_ptr<MidiPort>& p) { return p->MatchConnected(port); });
}

MidiManagerAlsa::MidiPortStateBase::iterator MidiManagerAlsa::MidiPortStateBase::FindDisconnected(const MidiManagerAlsa::MidiPort& port)
{
    // Always match on:
    //  type
    ...
    if (!port.path().empty()) {
        ...
        auto it = base::ranges::find_if(ports_, [&port](std::unique_ptr<MidiPort>& p) { return p->MatchCardPass1(port); });
        if (it != ports_.end())
            return it;
        ...
        if (!port.id().empty()) {
            ...
            // has a hardware id.
            it = base::ranges::find_if(ports_, [&port](std::unique_ptr<MidiPort>& p) { return p->MatchCardPass2(port); });
            if (it != ports_.end())
                return it;
        }
    } else {
        ...
        auto it = base::ranges::find_if(ports_, [&port](std::unique_ptr<MidiPort>& p) { return p->MatchNoCardPass1(port); });
        if (it != ports_.end())
            return it;
        ...
        // match.
        it = base::ranges::find_if(ports_, [&port](std::unique_ptr<MidiPort>& p) { return p->MatchNoCardPass2(port); });
        if (it != ports_.end())
            return it;
    }
...
uint32_t MidiManagerAlsa::MidiPortState::push_back(std::unique_ptr<MidiPort> port)
{
...
void MidiManagerAlsa::AlsaSeqState::ClientStart(int client_id, const std::string& client_name, snd_seq_client_type_t type)
{
    ClientExit(client_id);
    clients_.insert(std::make_pair(client_id, std::make_unique<Client>(client_name, type)));
...
void MidiManagerAlsa::AlsaSeqState::PortStart(
    int client_id, int port_id, const std::string& port_name, MidiManagerAlsa::AlsaSeqState::PortDirection direction, bool midi)
{
    auto it = clients_.find(client_id);
    if (it != clients_.end())
        it->second->AddPort(port_id, std::make_unique<Port>(port_name, direction, midi));
}
...
snd_seq_client_type_t MidiManagerAlsa::AlsaSeqState::ClientType(int client_id) const
{
    auto it = clients_.find(client_id);
    if (it == clients_.end())
...
std::unique_ptr<MidiManagerAlsa::TemporaryMidiPortState> MidiManagerAlsa::AlsaSeqState::ToMidiPortState(const AlsaCardMap& alsa_cards)
{
    std::unique_ptr<MidiManagerAlsa::TemporaryMidiPortState> midi_ports(new TemporaryMidiPortState);
    ...
    for (const auto& client_pair : clients_) {
        ...
        if (IsCardClient(client->type(), client_id)) {
            ...
            path = card->path();
            id = MidiPort::Id(card->bus(), card->vendor_id(), card->model_id(), card->usb_interface_num(), card->serial());
            card_name = card->name();
        ...
        for (const auto& port_pair : *client) {
            ...
            if (port->midi()) {
                ...
                version += base::StringPrintf("ALSA library version %d.%d.%d", SND_LIB_MAJOR, SND_LIB_MINOR, SND_LIB_SUBMINOR);
                ...
                if (direction == PortDirection::kInput || direction == PortDirection::kDuplex) {
                    midi_ports->push_back(std::make_unique<MidiPort>(
                        path, id, client_id, port_id, midi_device, client->name(), port->name(), manufacturer, version, MidiPort::Type::kInput));
                }
                if (direction == PortDirection::kOutput || direction == PortDirection::kDuplex) {
                    midi_ports->push_back(std::make_unique<MidiPort>(
                        path, id, client_id, port_id, midi_device, client->name(), port->name(), manufacturer, version, MidiPort::Type::kOutput));
                }
            }
        }
    }
...
MidiManagerAlsa::AlsaSeqState::Port::Port(const std::string& name, MidiManagerAlsa::AlsaSeqState::PortDirection direction, bool midi)
    : name_(name)
...
MidiManagerAlsa::AlsaSeqState::Client::Client(const std::string& name, snd_seq_client_type_t type)
    : name_(name)
    , type_(type)
{
}
...
void MidiManagerAlsa::AlsaSeqState::Client::AddPort(int addr, std::unique_ptr<Port> port)
{
...
MidiManagerAlsa::AlsaSeqState::Client::PortMap::const_iterator MidiManagerAlsa::AlsaSeqState::Client::begin() const
{
    return ports_.begin();
}

MidiManagerAlsa::AlsaSeqState::Client::PortMap::const_iterator MidiManagerAlsa::AlsaSeqState::Client::end() const
{
    return ports_.end();
}

MidiManagerAlsa::AlsaCard::AlsaCard(udev_device* dev, const std::string& name, const std::string& longname, const std::string& driver, int midi_device_count)
    : name_(name)
    , longname_(longname)
    , driver_(driver)
    , path_(device::UdevDeviceGetPropertyValue(dev, kUdevIdPath))
    , bus_(device::UdevDeviceGetPropertyValue(dev, kUdevIdBus))
    , vendor_id_(UdevDeviceGetPropertyOrSysattr(dev, kUdevIdVendorId, kSysattrVendor))
    , model_id_(UdevDeviceGetPropertyOrSysattr(dev, kUdevIdModelId, kSysattrModel))
    , usb_interface_num_(device::UdevDeviceGetPropertyValue(dev, kUdevIdUsbInterfaceNum))
    , serial_(UdevDeviceGetPropertyOrSysattr(dev, kUdevIdSerialShort, kSysattrGuid))
    , midi_device_count_(midi_device_count)
    , manufacturer_(ExtractManufacturerString(GetVendor(dev), vendor_id_, device::UdevDeviceGetPropertyValue(dev, kUdevIdVendorFromDatabase), name, longname))
{
}
...
// static
std::string MidiManagerAlsa::AlsaCard::ExtractManufacturerString(const std::string& udev_id_vendor, const std::string& udev_id_vendor_id,
    const std::string& udev_id_vendor_from_database, const std::string& alsa_name, const std::string& alsa_longname)
{
...
void MidiManagerAlsa::SendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data)
{
...
void MidiManagerAlsa::EventLoop()
{
    ...
    if (err < 0) {
        ...
        if (pfd[1].revents & POLLIN) {
            device::ScopedUdevDevicePtr dev(device::udev_monitor_receive_device(udev_monitor_.get()));
            if (dev.get())
...
void MidiManagerAlsa::ProcessSingleEvent(snd_seq_event_t* event, base::TimeTicks timestamp)
{
    ...
    if (source_it != source_map_.end()) {
        ...
        if (event->type == SND_SEQ_EVENT_SYSEX) {
            ...
            ReceiveMidiData(source, static_cast<const uint8_t*>(event->data.ext.ptr), event->data.ext.len, timestamp);
        } else {
...
void MidiManagerAlsa::ProcessClientStartEvent(int client_id)
{
    ...
    alsa_seq_state_.ClientStart(client_id, snd_seq_client_info_get_name(client_info), snd_seq_client_info_get_type(client_info));
...
void MidiManagerAlsa::ProcessPortStartEvent(const snd_seq_addr_t& addr)
{
    ...
    int err = snd_seq_get_any_port_info(in_client_.get(), addr.client, addr.port, port_info);
    if (err != 0)
        return;
    ...
    alsa_seq_state_.PortStart(
        addr.client, addr.port, snd_seq_port_info_get_name(port_info), direction, snd_seq_port_info_get_type(port_info) & SND_SEQ_PORT_TYPE_MIDI_GENERIC);
    // Generate Web MIDI events.
    UpdatePortStateAndGenerateEvents();
}
...
void MidiManagerAlsa::ProcessUdevEvent(udev_device* dev)
{
    ...
    // fully initialized.
    if (!device::udev_device_get_property_value(dev, kUdevPropertySoundInitialized))
        return;
...
void MidiManagerAlsa::AddCard(udev_device* dev)
{
    ...
    int midi_count = 0;
    for (int device = -1; !snd_ctl_rawmidi_next_device(handle, &device) && device >= 0;)
        ++midi_count;
    ...
    for (int device = -1; !snd_ctl_hwdep_next_device(handle, &device) && device >= 0;) {
    ...
    if (midi_count > 0) {
        auto alsa_card = std::make_unique<AlsaCard>(dev, name, longname, driver, midi_count);
...
void MidiManagerAlsa::UpdatePortStateAndGenerateEvents()
{
    ...
    for (auto& old_port : port_state_) {
        if (old_port->connected() && (new_port_state->FindConnected(*old_port) == new_port_state->end())) {
            ...
            case MidiPort::Type::kInput:
                source_map_.erase(AddrToInt(old_port->client_id(), old_port->port_id()));
    ...
    while (it != new_port_state->end()) {
        ...
        if (old_port == port_state_.end()) {
            ...
            mojom::PortInfo info(opaque_key, manufacturer, port_name, version, PortState::OPENED);
            switch (type) {
            case MidiPort::Type::kInput:
            ...
            // Reconnect.
            uint32_t web_port_index = (*old_port)->web_port_index();
            (*old_port)->Update(new_port->path(), new_port->client_id(), new_port->port_id(), new_port->client_name(), new_port->port_name(),
                new_port->manufacturer(), new_port->version());
            switch ((*old_port)->type()) {
            case MidiPort::Type::kInput:
                if (Subscribe(web_port_index, (*old_port)->client_id(), (*old_port)->port_id()))
                    SetInputPortState(web_port_index, PortState::OPENED);
                break;
            case MidiPort::Type::kOutput:
                if (CreateAlsaOutputPort(web_port_index, (*old_port)->client_id(), (*old_port)->port_id()))
...
bool MidiManagerAlsa::EnumerateUdevCards()
{
    ...
    device::ScopedUdevEnumeratePtr enumerate(device::udev_enumerate_new(udev_.get()));
    if (!enumerate.get()) {
        VLOG(1) << "udev_enumerate_new fails";
        return false;
    }

    err = device::udev_enumerate_add_match_subsystem(enumerate.get(), kUdevSubsystemSound);
    if (err) {
        VLOG(1) << "udev_enumerate_add_match_subsystem fails: " << base::safe_strerror(-err);
        return false;
    }

    err = device::udev_enumerate_scan_devices(enumerate.get());
    if (err) {
        VLOG(1) << "udev_enumerate_scan_devices fails: " << base::safe_strerror(-err);
        return false;
    }
    ...
    udev_list_entry_foreach(list_entry, devices)
    {
        ...
        device::ScopedUdevDevicePtr dev(device::udev_device_new_from_syspath(udev_.get(), path));
        if (dev.get())
            ProcessUdevEvent(dev.get());
    }
...
bool MidiManagerAlsa::CreateAlsaOutputPort(uint32_t port_index, int client_id, int port_id)
{
    // Create the port.
    int out_port;
    {
        base::AutoLock lock(out_client_lock_);
        out_port = snd_seq_create_simple_port(out_client_.get(), NULL, kCreateOutputPortCaps, kCreatePortType);
...
bool MidiManagerAlsa::Subscribe(uint32_t port_index, int client_id, int port_id)
{
    // Activate port subscription.
    snd_seq_port_subscribe_t* subs;
    snd_seq_port_subscribe_alloca(&subs);
...
MidiManagerAlsa::ScopedSndMidiEventPtr MidiManagerAlsa::CreateScopedSndMidiEventPtr(size_t size)
{
