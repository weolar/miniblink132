...
public:
    ...
    void DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp) final;

private:
    // Subclasses that access private/protected members of MidiManager.
    template <typename InterfaceType, typename RuntimeType, typename StaticsInterfaceType, wchar_t const* runtime_class_id> class MidiPortManager;
    ...
    std::unique_ptr<MidiInPortManager> port_manager_in_ GUARDED_BY(lazy_init_member_lock_);
    std::unique_ptr<MidiOutPortManager> port_manager_out_ GUARDED_BY(lazy_init_member_lock_);
