...
enum { kDefaultTaskRunner = TaskService::kDefaultRunnerId, kComTaskRunner };

template <typename T> std::string GetIdString(T* obj)
{
    HSTRING result;
...
template <typename T> std::string GetDeviceIdString(T* obj)
{
    HSTRING result;
...
{
    WRL::ComPtr<Win::Devices::Midi::IMidiSynthesizerStatics> midi_synthesizer_statics;
    HRESULT hr
        = GetActivationFactory<Win::Devices::Midi::IMidiSynthesizerStatics, RuntimeClass_Windows_Devices_Midi_MidiSynthesizer>(&midi_synthesizer_statics);
    if (FAILED(hr)) {
...
void GetDevPropString(DEVINST handle, const DEVPROPKEY* devprop_key, std::string* out)
{
    DEVPROPTYPE devprop_type;
    unsigned long buffer_size = 0;

    // Retrieve |buffer_size| and allocate buffer later for receiving data.
    CONFIGRET cr = CM_Get_DevNode_Property(handle, devprop_key, &devprop_type, nullptr, &buffer_size, 0);
    ...
    cr = CM_Get_DevNode_Property(handle, devprop_key, &devprop_type, buffer.data(), &buffer_size, 0);
    if (cr != CR_SUCCESS)
        VLOG(1) << "CM_Get_DevNode_Property failed: CONFIGRET 0x" << std::hex << cr;
    else
...
void GetDriverInfoFromDeviceId(const std::string& dev_id, std::string* out_manufacturer, std::string* out_driver_version)
{
    std::wstring dev_instance_id = base::UTF8ToWide(dev_id.substr(4, dev_id.size() - 43));
    base::ReplaceChars(dev_instance_id, L"#", L"\\", &dev_instance_id);

    DEVINST dev_instance_handle;
    CONFIGRET cr = CM_Locate_DevNode(&dev_instance_handle, &dev_instance_id[0], CM_LOCATE_DEVNODE_NORMAL);
    if (cr != CR_SUCCESS) {
    ...
    GetDevPropString(parent_handle, &DEVPKEY_Device_DriverProvider, out_manufacturer);
    GetDevPropString(parent_handle, &DEVPKEY_Device_DriverVersion, out_driver_version);
}

// Tokens with value = 0 are considered invalid (as in <wrl/event.h>).
const int64_t kInvalidTokenValue = 0;

template <typename InterfaceType> struct MidiPort {
    MidiPort() = default;
...
template <typename InterfaceType, typename RuntimeType, typename StaticsInterfaceType, wchar_t const* runtime_class_id>
...
public:
    ...
    bool StartWatcher()
    {
        ...
        HRESULT hr = GetActivationFactory<StaticsInterfaceType, runtime_class_id>(&midi_port_statics_);
        if (FAILED(hr)) {
        ...
        WRL::ComPtr<Win::Devices::Enumeration::IDeviceInformationStatics> dev_info_statics;
        hr = GetActivationFactory<Win::Devices::Enumeration::IDeviceInformationStatics, RuntimeClass_Windows_Devices_Enumeration_DeviceInformation>(
            &dev_info_statics);
        if (FAILED(hr)) {
        ...
        hr = watcher_->add_Added(
            WRL::Callback<ITypedEventHandler<DeviceWatcher*, Win::Devices::Enumeration::DeviceInformation*>>([port_manager, task_service](
                ...
                std::string dev_id = GetIdString(info), dev_name = GetNameString(info);

                task_service->PostBoundTask(kComTaskRunner, base::BindOnce(&MidiPortManager::OnAdded, base::Unretained(port_manager), dev_id, dev_name));
        ...
        hr = watcher_->add_EnumerationCompleted(
            WRL::Callback<ITypedEventHandler<DeviceWatcher*, IInspectable*>>([port_manager, task_service](IDeviceWatcher* watcher, IInspectable* insp) {
                task_service->PostBoundTask(kComTaskRunner, base::BindOnce(&MidiPortManager::OnEnumerationCompleted, base::Unretained(port_manager)));
        ...
        hr = watcher_->add_Removed(
            WRL::Callback<ITypedEventHandler<DeviceWatcher*, DeviceInformationUpdate*>>([port_manager, task_service](
                ...
                task_service->PostBoundTask(kComTaskRunner, base::BindOnce(&MidiPortManager::OnRemoved, base::Unretained(port_manager), dev_id));
        ...
        hr = watcher_->add_Stopped(WRL::Callback<ITypedEventHandler<DeviceWatcher*, IInspectable*>>([](IDeviceWatcher* watcher, IInspectable* insp) {
        ...
        hr = watcher_->add_Updated(
            WRL::Callback<ITypedEventHandler<DeviceWatcher*, DeviceInformationUpdate*>>([](IDeviceWatcher* watcher, IDeviceInformationUpdate* update) {
    ...
    void StopWatcher()
    {
        ...
        if (token_EnumerationCompleted_.value != kInvalidTokenValue) {
            ...
            VLOG_IF(1, FAILED(hr)) << "remove_EnumerationCompleted failed: " << PrintHr(hr);
...
private:
    ...
    void OnAdded(std::string dev_id, std::string dev_name)
    {
        ...
        hr = async_op->put_Completed(WRL::Callback<Win::Foundation::IAsyncOperationCompletedHandler<RuntimeType*>>(
            [port_manager, task_service](IAsyncOperation<RuntimeType*>* async_op, AsyncStatus status) {
                // A reference to |async_op| is kept in |async_ops_|, safe to pass
                // outside.
                task_service->PostBoundTask(kComTaskRunner,
                    base::BindOnce(&MidiPortManager::OnCompletedGetPortFromIdAsync, base::Unretained(port_manager), base::Unretained(async_op)));
    ...
    void OnCompletedGetPortFromIdAsync(IAsyncOperation<RuntimeType*>* async_op)
    {
        ...
        if (port == nullptr) {
            ...
            AddPort(mojom::PortInfo(dev_id, manufacturer, port_names_[dev_id], driver_version, PortState::OPENED));
    ...
    virtual bool RegisterOnMessageReceived(InterfaceType* handle, EventRegistrationToken* p_token)
    {
        return true;
    }
    ...
    EventRegistrationToken token_Added_ = { kInvalidTokenValue }, token_EnumerationCompleted_ = { kInvalidTokenValue }, token_Removed_ = { kInvalidTokenValue },
                           token_Stopped_ = { kInvalidTokenValue }, token_Updated_ = { kInvalidTokenValue };

    // All manipulations to these fields should be done on kComTaskRunner.
    std::unordered_map<std::string, std::unique_ptr<MidiPort<InterfaceType>>> ports_;
...
class MidiManagerWinrt::MidiInPortManager final : public MidiPortManager<Win::Devices::Midi::IMidiInPort, Win::Devices::Midi::MidiInPort,
                                                      Win::Devices::Midi::IMidiInPortStatics, RuntimeClass_Windows_Devices_Midi_MidiInPort> {
public:
...
private:
    ...
    bool RegisterOnMessageReceived(Win::Devices::Midi::IMidiInPort* handle, EventRegistrationToken* p_token) override
    {
        DCHECK(midi_service_->task_service()->IsOnTaskRunner(kComTaskRunner));
        ...
        HRESULT hr = handle->add_MessageReceived(
            WRL::Callback<ITypedEventHandler<Win::Devices::Midi::MidiInPort*, Win::Devices::Midi::MidiMessageReceivedEventArgs*>>(
                [port_manager, task_service](Win::Devices::Midi::IMidiInPort* handle, Win::Devices::Midi::IMidiMessageReceivedEventArgs* args) {
                    ...
                    hr = base::win::GetPointerToBufferData(buffer.Get(), &p_buffer_data, &data_length);
                    if (FAILED(hr))
                        return hr;
                    ...
                    task_service->PostBoundTask(
                        kComTaskRunner, base::BindOnce(&MidiInPortManager::OnMessageReceived, base::Unretained(port_manager), dev_id, data, now));
    ...
    void RemovePortEventHandlers(MidiPort<Win::Devices::Midi::IMidiInPort>* port) override
    {
    ...
    void OnMessageReceived(std::string dev_id, std::vector<uint8_t> data, base::TimeTicks time)
    {
        DCHECK(midi_service_->task_service()->IsOnTaskRunner(kComTaskRunner));
...
class MidiManagerWinrt::MidiOutPortManager final : public MidiPortManager<Win::Devices::Midi::IMidiOutPort, Win::Devices::Midi::IMidiOutPort,
                                                       Win::Devices::Midi::IMidiOutPortStatics, RuntimeClass_Windows_Devices_Midi_MidiOutPort> {
public:
...
void FinalizeOnComRunner(
    std::unique_ptr<MidiManagerWinrt::MidiInPortManager> port_manager_in, std::unique_ptr<MidiManagerWinrt::MidiOutPortManager> port_manager_out)
{
    if (port_manager_in)
...
MidiManagerWinrt::~MidiManagerWinrt()
{
    ...
    service()->task_service()->PostStaticTask(kComTaskRunner, base::BindOnce(&FinalizeOnComRunner, std::move(port_manager_in_), std::move(port_manager_out_)));
}
...
void MidiManagerWinrt::StartInitialization()
{
    ...
    service()->task_service()->PostBoundTask(kComTaskRunner, base::BindOnce(&MidiManagerWinrt::InitializeOnComRunner, base::Unretained(this)));
}

void MidiManagerWinrt::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    base::TimeDelta delay = MidiService::TimestampToTimeDeltaDelay(timestamp);
    service()->task_service()->PostBoundDelayedTask(
        kComTaskRunner, base::BindOnce(&MidiManagerWinrt::SendOnComRunner, base::Unretained(this), port_index, data), delay);
    service()->task_service()->PostBoundDelayedTask(
        kComTaskRunner, base::BindOnce(&MidiManagerWinrt::AccumulateMidiBytesSent, base::Unretained(this), client, data.size()), delay);
}
...
void MidiManagerWinrt::InitializeOnComRunner()
{
    ...
    if (!(port_manager_in_->StartWatcher() && port_manager_out_->StartWatcher())) {
        ...
        service()->task_service()->PostBoundTask(
            kDefaultTaskRunner, base::BindOnce(&MidiManagerWinrt::CompleteInitialization, base::Unretained(this), Result::INITIALIZATION_ERROR));
    }
}

void MidiManagerWinrt::SendOnComRunner(uint32_t port_index, const std::vector<uint8_t>& data)
{
    DCHECK(service()->task_service()->IsOnTaskRunner(kComTaskRunner));

    base::AutoLock auto_lock(lazy_init_member_lock_);
    MidiPort<Win::Devices::Midi::IMidiOutPort>* port = port_manager_out_->GetPortByIndex(port_index);
    if (!(port && port->handle)) {
    ...
    WRL::ComPtr<Win::Storage::Streams::IBuffer> buffer;
    HRESULT hr = base::win::CreateIBufferFromData(data.data(), static_cast<UINT32>(data.size()), &buffer);
    if (FAILED(hr)) {
...
void MidiManagerWinrt::OnPortManagerReady()
{
    ...
    if (++port_manager_ready_count_ == 2) {
        service()->task_service()->PostBoundTask(
            kDefaultTaskRunner, base::BindOnce(&MidiManagerWinrt::CompleteInitialization, base::Unretained(this), Result::OK));
    }
}
