...
private:
    ...
    public:
        ...
        public:
            Id();
            Id(const std::string& bus, const std::string& vendor_id, const std::string& model_id, const std::string& usb_interface_num,
            ...
            std::string usb_interface_num() const
            {
                return usb_interface_num_;
            }
        ...
        MidiPort(const std::string& path, const Id& id, int client_id, int port_id, int midi_device, const std::string& client_name,
        ...
        uint32_t web_port_index() const
        {
            return web_port_index_;
        }
        ...
        void Update(const std::string& path, int client_id, int port_id, const std::string& client_name, const std::string& port_name,
    ...
    public:
        ...
        void ClientStart(int client_id, const std::string& client_name, snd_seq_client_type_t type);
        bool ClientStarted(int client_id);
        void ClientExit(int client_id);
        void PortStart(int client_id, int port_id, const std::string& port_name, PortDirection direction, bool midi);
        void PortExit(int client_id, int port_id);
        snd_seq_client_type_t ClientType(int client_id) const;
        std::unique_ptr<TemporaryMidiPortState> ToMidiPortState(const AlsaCardMap& alsa_cards);

        int card_client_count()
        {
            return card_client_count_;
        }
    ...
    class AlsaCard {
    public:
        AlsaCard(udev_device* dev, const std::string& name, const std::string& longname, const std::string& driver, int midi_device_count);
        ...
        int midi_device_count() const
        {
            return midi_device_count_;
        }
        std::string manufacturer() const
        {
            return manufacturer_;
    ...
    private:
        ...
        static std::string ExtractManufacturerString(const std::string& udev_id_vendor, const std::string& udev_id_vendor_id,
            const std::string& udev_id_vendor_from_database, const std::string& name, const std::string& longname);
    ...
    struct SndSeqDeleter {
        void operator()(snd_seq_t* seq) const
        {
            snd_seq_close(seq);
        }
    };
    ...
    void SendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data);
