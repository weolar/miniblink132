...
MidiInputPortAndroid::MidiInputPortAndroid(JNIEnv* env, jobject raw, Delegate* delegate)
    : raw_port_(env, raw)
    , delegate_(delegate)
{
}
...
bool MidiInputPortAndroid::Open()
{
    ...
    return Java_MidiInputPortAndroid_open(env, raw_port_, reinterpret_cast<jlong>(this));
}
...
void MidiInputPortAndroid::OnData(JNIEnv* env, const JavaParamRef<jbyteArray>& data, jint offset, jint size, jlong timestamp)
{
    ...
    base::TimeTicks timestamp_to_pass = base::TimeTicks::FromInternalValue(timestamp / base::TimeTicks::kNanosecondsPerMicrosecond);
