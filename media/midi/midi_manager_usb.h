...
class USB_MIDI_EXPORT MidiManagerUsb : public MidiManager, public UsbMidiDeviceDelegate, public UsbMidiInputStream::Delegate {
public:
    MidiManagerUsb(MidiService* service, std::unique_ptr<UsbMidiDevice::Factory> device_factory);
    ...
    void ReceiveUsbMidiData(UsbMidiDevice* device, int endpoint_number, const uint8_t* data, size_t size, base::TimeTicks time) override;
    ...
    void OnReceivedData(size_t jack_index, const uint8_t* data, size_t size, base::TimeTicks time) override;

    const std::vector<std::unique_ptr<UsbMidiOutputStream>>& output_streams() const
    {
        return output_streams_;
    }
    const UsbMidiInputStream* input_stream() const
    {
...
private:
    ...
    std::unordered_map<std::pair<int, int>, size_t, base::IntPairHash<std::pair<int, int>>> input_jack_dictionary_;
};
