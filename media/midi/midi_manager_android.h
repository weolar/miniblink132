...
class MidiManagerAndroid final : public MidiManager, public MidiInputPortAndroid::Delegate {
public:
    ...
    void OnReceivedData(MidiInputPortAndroid*, const uint8_t* data, size_t size, base::TimeTicks timestamp) override;

    // Called from the Java world.
    void OnInitialized(JNIEnv* env, const base::android::JavaParamRef<jobjectArray>& devices);
    void OnInitializationFailed(JNIEnv* env);
    void OnAttached(JNIEnv* env, const base::android::JavaParamRef<jobject>& device);
    void OnDetached(JNIEnv* env, const base::android::JavaParamRef<jobject>& device);

private:
    void AddDevice(std::unique_ptr<MidiDeviceAndroid> device);
    void AddInputPortAndroid(MidiInputPortAndroid* port, MidiDeviceAndroid* device);
    void AddOutputPortAndroid(MidiOutputPortAndroid* port, MidiDeviceAndroid* device);
