...
public:
    ...
    void DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp) override
    {
    }
    ...
    {
        return GetClientCountForTesting();
    }

    size_t GetPendingClientCount()
    {
        return GetPendingClientCountForTesting();
    }
...
public:
    ...
    {
        std::unique_ptr<FakeMidiManager> manager = std::make_unique<FakeMidiManager>(service);
    ...
    base::WeakPtr<FakeMidiManager> manager()
    {
        ...
        if (!manager_ || (!manager_->GetClientCount() && !manager_->GetPendingClientCount())) {
            return nullptr;
        }
#endif
        return manager_;
...
public:
    ...
    void ReceiveMidiData(uint32_t port_index, const uint8_t* data, size_t size, base::TimeTicks timestamp) override
    {
    }
...
public:
    MidiManagerTest()
    {
        std::unique_ptr<FakeMidiManagerFactory> factory = std::make_unique<FakeMidiManagerFactory>();
...
protected:
    ...
    base::WeakPtr<FakeMidiManagerFactory> factory()
    {
        return factory_;
    }
...
TEST_F(MidiManagerTest, StartAndEndSession)
{
    ...
    std::unique_ptr<FakeMidiManagerClient> client = std::make_unique<FakeMidiManagerClient>(test_future.GetCallback());
...
TEST_F(MidiManagerTest, StartMultipleSessions)
{
    ...
    std::unique_ptr<FakeMidiManagerClient> client1 = std::make_unique<FakeMidiManagerClient>(future1.GetCallback());
    base::test::TestFuture<void> future2;
    std::unique_ptr<FakeMidiManagerClient> client2 = std::make_unique<FakeMidiManagerClient>(future2.GetCallback());
    base::test::TestFuture<void> future3;
    std::unique_ptr<FakeMidiManagerClient> client3 = std::make_unique<FakeMidiManagerClient>(future3.GetCallback());
...
TEST_F(MidiManagerTest, TooManyPendingSessions)
{
    ...
    std::unique_ptr<base::test::TestFuture<void>> test_futures[MidiManager::kMaxPendingClientCount];
    std::unique_ptr<FakeMidiManagerClient> many_existing_clients[MidiManager::kMaxPendingClientCount];
    test_futures[0] = std::make_unique<base::test::TestFuture<void>>();
    many_existing_clients[0] = std::make_unique<FakeMidiManagerClient>(test_futures[0]->GetCallback());
    ...
    for (size_t i = 1; i < MidiManager::kMaxPendingClientCount; ++i) {
        ...
        many_existing_clients[i] = std::make_unique<FakeMidiManagerClient>(test_futures[i]->GetCallback());
    ...
    std::unique_ptr<FakeMidiManagerClient> additional_client = std::make_unique<FakeMidiManagerClient>(base::DoNothing());
...
public:
    PlatformMidiManagerTest()
        : client_(std::make_unique<FakeMidiManagerClient>(test_future_.GetCallback()))
    ...
    void EndSession()
    {
        service_->EndSession(client_.get());
    }
    ...
    bool IsSupported()
    {
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_WIN) && !(defined(USE_ALSA) && defined(USE_UDEV)) && !BUILDFLAG(IS_ANDROID)
        return false;
#else
        return true;
#endif
...
TEST_F(PlatformMidiManagerTest, InstanceIdOverflow)
{
    ...
    StartSession();
    ASSERT_TRUE(future()->Wait());
    EXPECT_EQ(IsSupported() ? Result::INITIALIZATION_ERROR : Result::NOT_SUPPORTED, client()->result());
