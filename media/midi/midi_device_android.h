...
public:
    MidiDeviceAndroid(JNIEnv* env, const base::android::JavaRef<jobject>& raw_device, MidiInputPortAndroid::Delegate* delegate);
    ~MidiDeviceAndroid();
    ...
    const std::vector<std::unique_ptr<MidiInputPortAndroid>>& input_ports() const
    {
        return input_ports_;
    }
    const std::vector<std::unique_ptr<MidiOutputPortAndroid>>& output_ports() const
    {
