...
{
    UMA_HISTOGRAM_ENUMERATION("Media.Midi.Usage", usage, static_cast<Sample>(Usage::MAX) + 1);
}
...
MidiManager::~MidiManager()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Midi.SendReceiveUsage",
        data_sent_ ? (data_received_ ? SendReceiveUsage::SENT_AND_RECEIVED : SendReceiveUsage::SENT)
                   : (data_received_ ? SendReceiveUsage::RECEIVED : SendReceiveUsage::NO_USE),
...
void MidiManager::StartSession(MidiManagerClient* client)
{
...

    {
        ...
        if (initialization_state_ == InitializationState::NOT_STARTED) {
            ...
            session_thread_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
...
void MidiManager::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    NOTREACHED();
}
...
void MidiManager::ReceiveMidiData(uint32_t port_index, const uint8_t* data, size_t length, base::TimeTicks timestamp)
{
    base::AutoLock auto_lock(lock_);
    data_received_ = true;
