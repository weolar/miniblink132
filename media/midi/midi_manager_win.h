...
class MidiManagerWin final : public MidiManager, public base::SystemMonitor::DevicesChangedObserver {
public:
    class PortManager;
...
private:
    ...
    void ReceiveMidiData(uint32_t index, const std::vector<uint8_t>& data, base::TimeTicks time);
    ...
    template <typename T>
    void ReflectActiveDeviceList(MidiManagerWin* manager, std::vector<std::unique_ptr<T>>* known_ports, std::vector<std::unique_ptr<T>>* active_ports);

    // Sends MIDI data on TaskRunner.
    void SendOnTaskRunner(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data);
