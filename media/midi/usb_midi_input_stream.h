...
public:
    ...
    public:
        ...
        virtual void OnReceivedData(size_t jack_index, const uint8_t* data, size_t size, base::TimeTicks time) = 0;
    };
    ...
    void OnReceivedData(UsbMidiDevice* device, int endpoint_number, const uint8_t* data, size_t size, base::TimeTicks time);

    const std::vector<UsbMidiJack>& jacks() const
    {
        return jacks_;
    }
...
private:
    ...
    void ProcessOnePacket(UsbMidiDevice* device, int endpoint_number, const uint8_t* packet, base::TimeTicks time);
