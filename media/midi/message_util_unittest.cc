...
template <typename T, size_t N> const std::vector<T> AsVector(const T (&data)[N])
{
    std::vector<T> buffer;
...
template <typename T, size_t N> void PushToVector(const T (&data)[N], std::vector<T>* buffer)
{
    buffer->insert(buffer->end(), data, data + N);
}
...
TEST(MidiMessageUtilTest, GetMessageLength)
{
    ...
    EXPECT_EQ(std::size(kSystemCommonMessageTuneRequest), GetMessageLength(kSystemCommonMessageTuneRequest[0]));
