...
UsbMidiDeviceAndroid::UsbMidiDeviceAndroid(const base::android::JavaRef<jobject>& raw_device, UsbMidiDeviceDelegate* delegate)
    : raw_device_(raw_device)
    , delegate_(delegate)
{
    JNIEnv* env = jni_zero::AttachCurrentThread();
    Java_UsbMidiDeviceAndroid_registerSelf(env, raw_device_, reinterpret_cast<jlong>(this));
...
void UsbMidiDeviceAndroid::Send(int endpoint_number, const std::vector<uint8_t>& data)
{
    JNIEnv* env = jni_zero::AttachCurrentThread();
    ...
    Java_UsbMidiDeviceAndroid_send(env, raw_device_, endpoint_number, data_to_pass);
}

void UsbMidiDeviceAndroid::OnData(JNIEnv* env, jint endpoint_number, const JavaParamRef<jbyteArray>& data)
{
    std::vector<uint8_t> bytes;
    base::android::JavaByteArrayToByteVector(env, data, &bytes);

    const uint8_t* head = bytes.size() ? &bytes[0] : NULL;
    delegate_->ReceiveUsbMidiData(this, endpoint_number, head, bytes.size(), base::TimeTicks::Now());
}
...
void UsbMidiDeviceAndroid::InitDeviceInfo()
{
    ...
    manufacturer_ = GetString(info.manufacturer_index, base::StringPrintf("(vendor id = 0x%04x)", info.vendor_id));
    product_ = GetString(info.product_index, base::StringPrintf("(product id = 0x%04x)", info.product_id));
...
std::string UsbMidiDeviceAndroid::GetString(int index, const std::string& backup)
{
