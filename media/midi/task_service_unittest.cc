...
enum { kDefaultRunner = TaskService::kDefaultRunnerId, kFirstRunner, kSecondRunner };

base::WaitableEvent* GetEvent()
{
    static base::WaitableEvent* event = new base::WaitableEvent(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    return event;
}
...
public:
    TaskServiceClient(TaskService* task_service)
        ...
        , wait_task_event_(std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED))
        , count_(0u)
    {
        DCHECK(task_service);
    }
    ...
    bool Bind()
    {
        return task_service()->BindInstance();
    }
    ...
    {
        task_service()->PostBoundTask(runner_id, base::BindOnce(&TaskServiceClient::IncrementCount, base::Unretained(this)));
    }

    void PostBoundSignalTask(TaskService::RunnerId runner_id)
    {
        task_service()->PostBoundTask(runner_id, base::BindOnce(&TaskServiceClient::SignalEvent, base::Unretained(this)));
    }

    void PostBoundWaitTask(TaskService::RunnerId runner_id)
    {
        wait_task_event_->Reset();
        task_service()->PostBoundTask(runner_id, base::BindOnce(&TaskServiceClient::WaitEvent, base::Unretained(this)));
    }

    void PostBoundDelayedSignalTask(TaskService::RunnerId runner_id)
    {
        task_service()->PostBoundDelayedTask(runner_id, base::BindOnce(&TaskServiceClient::SignalEvent, base::Unretained(this)), base::Milliseconds(100));
    }

    void WaitTask()
    {
        wait_task_event_->Wait();
    }
...
private:
    void SetUp() override
    {
        ...
        thread_task_runner_handle_ = std::make_unique<base::SingleThreadTaskRunner::CurrentDefaultHandle>(task_runner_);
    }
    ...
    std::unique_ptr<base::SingleThreadTaskRunner::CurrentDefaultHandle> thread_task_runner_handle_;
...
{
    std::unique_ptr<TaskServiceClient> client = std::make_unique<TaskServiceClient>(task_service());
