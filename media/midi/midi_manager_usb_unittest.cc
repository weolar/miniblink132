...
template <typename T, size_t N> std::vector<T> ToVector(const T (&array)[N])
{
...
public:
    ...
    void Send(int endpoint_number, const std::vector<uint8_t>& data) override
    {
        ...
        logger_->AddLog(base::StringPrintf("endpoint = %d data =", endpoint_number));
...
public:
    explicit FakeMidiManagerClient(Logger* logger, base::OnceClosure on_session_start_cb)
    ...
    void ReceiveMidiData(uint32_t port_index, const uint8_t* data, size_t size, base::TimeTicks timestamp) override
    {
        ...
        logger_->AddLog(base::StringPrintf("usb:port_index = %d data =", port_index));
...
public:
    ...
    void EnumerateDevices(UsbMidiDeviceDelegate* device, Callback callback) override
    {
...
public:
    MidiManagerUsbForTesting(std::unique_ptr<UsbMidiDevice::Factory> device_factory, MidiService* service)
...
public:
    ...
    std::unique_ptr<midi::MidiManager> Create(midi::MidiService* service) override
    {
        std::unique_ptr<TestUsbMidiDeviceFactory> device_factory = std::make_unique<TestUsbMidiDeviceFactory>();
        device_factory_ = device_factory.get();
        std::unique_ptr<MidiManagerUsbForTesting> manager = std::make_unique<MidiManagerUsbForTesting>(std::move(device_factory), service);
...
private:
    raw_ptr<TestUsbMidiDeviceFactory, AcrossTasksDanglingUntriaged> device_factory_ = nullptr;
    raw_ptr<MidiManagerUsbForTesting, AcrossTasksDanglingUntriaged> manager_ = nullptr;
};
...
public:
    MidiManagerUsbTest()
    {
        std::unique_ptr<MidiManagerFactoryForTesting> factory = std::make_unique<MidiManagerFactoryForTesting>();
...
protected:
    void Initialize()
    {
        client_ = std::make_unique<FakeMidiManagerClient>(&logger_, test_future_.GetCallback());
    ...
    void RunCallbackUntilCallbackInvoked(bool result, UsbMidiDevice::Devices* devices)
    {
...
TEST_F(MidiManagerUsbTest, Send)
{
    ...
    manager()->DispatchSendMidiData(client_.get(), 1, ToVector(data), base::TimeTicks());
...
TEST_F(MidiManagerUsbTest, SendFromCompromizedRenderer)
{
    ...
    manager()->DispatchSendMidiData(client_.get(), 99, ToVector(data), base::TimeTicks());
    EXPECT_EQ("", logger_.TakeLog());

    // The specified port index is invalid. The manager must ignore the request.
    manager()->DispatchSendMidiData(client_.get(), 2, ToVector(data), base::TimeTicks());
    EXPECT_EQ("", logger_.TakeLog());
}
...
TEST_F(MidiManagerUsbTest, Receive)
{
    ...
    manager()->ReceiveUsbMidiData(device_raw, 2, data, std::size(data), base::TimeTicks());
    Finalize();
...
TEST_F(MidiManagerUsbTest, AttachDevice)
{
    ...
    std::unique_ptr<FakeUsbMidiDevice> new_device(new FakeUsbMidiDevice(&logger_));
