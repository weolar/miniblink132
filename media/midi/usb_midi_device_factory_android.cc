...
void UsbMidiDeviceFactoryAndroid::EnumerateDevices(UsbMidiDeviceDelegate* delegate, Callback callback)
{
    DCHECK(!delegate_);
    JNIEnv* env = jni_zero::AttachCurrentThread();
...
void UsbMidiDeviceFactoryAndroid::OnUsbMidiDeviceRequestDone(JNIEnv* env, const JavaParamRef<jobjectArray>& devices)
{
    UsbMidiDevice::Devices devices_to_pass;
    for (auto raw_device : devices.ReadElements<jobject>()) {
        devices_to_pass.push_back(std::make_unique<UsbMidiDeviceAndroid>(raw_device, delegate_));
    }
...
// Called from the Java world.
void UsbMidiDeviceFactoryAndroid::OnUsbMidiDeviceAttached(JNIEnv* env, const JavaParamRef<jobject>& device)
{
    delegate_->OnDeviceAttached(std::make_unique<UsbMidiDeviceAndroid>(device, delegate_));
}

// Called from the Java world.
void UsbMidiDeviceFactoryAndroid::OnUsbMidiDeviceDetached(JNIEnv* env, jint index)
{
