...
uint8_t UsbMidiOutputStream::Get(const std::vector<uint8_t>& data, size_t index) const
{
...
bool UsbMidiOutputStream::PushSysExMessage(const std::vector<uint8_t>& data, size_t* current, std::vector<uint8_t>* data_to_send)
{
    ...
    while (index < GetSize(data)) {
        if (message_size == kMessageSizeMax) {
            ...
            *current = index;
            ...
            data_to_send->insert(data_to_send->end(), message, message + std::size(message));
...
bool UsbMidiOutputStream::PushSysCommonMessage(const std::vector<uint8_t>& data, size_t* current, std::vector<uint8_t>* data_to_send)
{
    size_t index = *current;
...
void UsbMidiOutputStream::PushSysRTMessage(const std::vector<uint8_t>& data, size_t* current, std::vector<uint8_t>* data_to_send)
{
    size_t index = *current;
    uint8_t first_byte = Get(data, index);
...
bool UsbMidiOutputStream::PushChannelMessage(const std::vector<uint8_t>& data, size_t* current, std::vector<uint8_t>* data_to_send)
{
    size_t index = *current;
    uint8_t first_byte = Get(data, index);
