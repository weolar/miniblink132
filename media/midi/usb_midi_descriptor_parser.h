...
public:
    ...
    // never crashes.
    bool Parse(UsbMidiDevice* device, const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks);

    bool ParseDeviceInfo(const uint8_t* data, size_t size, DeviceInfo* info);

private:
    bool ParseInternal(UsbMidiDevice* device, const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks);
    bool ParseDevice(const uint8_t* data, size_t size, DeviceInfo* info);
    bool ParseInterface(const uint8_t* data, size_t size);
    bool ParseCSInterface(UsbMidiDevice* device, const uint8_t* data, size_t size);
    bool ParseEndpoint(const uint8_t* data, size_t size);
    bool ParseCSEndpoint(const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks);
    void Clear();
