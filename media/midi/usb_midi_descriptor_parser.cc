...
std::string UsbMidiDescriptorParser::DeviceInfo::BcdVersionToString(uint16_t version)
{
    const int byte_1 = version >> 8;
    const int byte_2 = version & 0xff;
    int version_major, version_minor;
    if (!DecodeBcd(byte_1, &version_major) || !DecodeBcd(byte_2, &version_minor)) {
...
bool UsbMidiDescriptorParser::Parse(UsbMidiDevice* device, const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks)
{
    jacks->clear();
...
bool UsbMidiDescriptorParser::ParseDeviceInfo(const uint8_t* data, size_t size, DeviceInfo* info)
{
    *info = DeviceInfo();
    for (const uint8_t* current = data; current < data + size; current += current[0]) {
...
bool UsbMidiDescriptorParser::ParseInternal(UsbMidiDevice* device, const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks)
{
    for (const uint8_t* current = data; current < data + size; current += current[0]) {
        uint8_t length = current[0];
        if (length < 2) {
...
bool UsbMidiDescriptorParser::ParseDevice(const uint8_t* data, size_t size, DeviceInfo* info)
{
    if (size < 0x12) {
...
bool UsbMidiDescriptorParser::ParseCSInterface(UsbMidiDevice* device, const uint8_t* data, size_t size)
{
...
bool UsbMidiDescriptorParser::ParseCSEndpoint(const uint8_t* data, size_t size, std::vector<UsbMidiJack>* jacks)
{
    ...
    for (size_t i = 0; i < num_jacks; ++i) {
        ...
        auto it = base::ranges::find(incomplete_jacks_, jack, &UsbMidiJack::jack_id);
