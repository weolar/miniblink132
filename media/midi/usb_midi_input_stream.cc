...
UsbMidiInputStream::JackUniqueKey::JackUniqueKey(UsbMidiDevice* device, int endpoint_number, int cable_number)
    : device(device)
...
bool UsbMidiInputStream::JackUniqueKey::operator==(const JackUniqueKey& that) const
{
...
{
    JackUniqueKey key(jack.device, jack.endpoint_number(), jack.cable_number);
...
void UsbMidiInputStream::OnReceivedData(UsbMidiDevice* device, int endpoint_number, const uint8_t* data, size_t size, base::TimeTicks time)
{
...
void UsbMidiInputStream::ProcessOnePacket(UsbMidiDevice* device, int endpoint_number, const uint8_t* packet, base::TimeTicks time)
{
    ...
    std::map<JackUniqueKey, size_t>::const_iterator it = jack_dictionary_.find(JackUniqueKey(device, endpoint_number, cable_number));
