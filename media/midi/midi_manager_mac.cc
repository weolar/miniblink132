...
mojom::PortInfo GetPortInfoFromEndpoint(MIDIEndpointRef endpoint)
{
    ...
    OSStatus result = MIDIObjectGetStringProperty(endpoint, kMIDIPropertyManufacturer, &manufacturer_ref);
    if (result == noErr) {
        ...
        DLOG(WARNING) << "Failed to get kMIDIPropertyManufacturer with status " << result;
    }

    std::string name;
    CFStringRef name_ref = NULL;
    result = MIDIObjectGetStringProperty(endpoint, kMIDIPropertyDisplayName, &name_ref);
    if (result == noErr) {
        name = SysCFStringRefToUTF8(name_ref);
    } else {
        DLOG(WARNING) << "Failed to get kMIDIPropertyDisplayName with status " << result;
    }

    std::string version;
    SInt32 version_number = 0;
    result = MIDIObjectGetIntegerProperty(endpoint, kMIDIPropertyDriverVersion, &version_number);
    if (result == noErr) {
        version = NumberToString(version_number);
    } else {
        // kMIDIPropertyDriverVersion is not supported in IAC driver providing
        // endpoints, and the result will be kMIDIUnknownProperty (-10835).
        DLOG(WARNING) << "Failed to get kMIDIPropertyDriverVersion with status " << result;
    }

    std::string id;
    SInt32 id_number = 0;
    result = MIDIObjectGetIntegerProperty(endpoint, kMIDIPropertyUniqueID, &id_number);
    if (result == noErr) {
        ...
        // all queries.
        DLOG(WARNING) << "Failed to get kMIDIPropertyUniqueID with status " << result;
    }
...
void MidiManagerMac::StartInitialization()
{
    ...
    service()->task_service()->PostBoundTask(kClientTaskRunner, base::BindOnce(&MidiManagerMac::InitializeCoreMIDI, base::Unretained(this)));
}

void MidiManagerMac::DispatchSendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    service()->task_service()->PostBoundTask(
        kClientTaskRunner, base::BindOnce(&MidiManagerMac::SendMidiData, base::Unretained(this), client, port_index, data, timestamp));
}
...
void MidiManagerMac::InitializeCoreMIDI()
{
    ...
    OSStatus result = MIDIClientCreate(CFSTR("Chrome"), ReceiveMidiNotifyDispatch, this, &client);
    ...
    // client disposal.
    result = MIDIInputPortCreate(client, CFSTR("MIDI Input"), ReadMidiDispatch, this, &midi_input_);
...
{
    service()->task_service()->PostBoundTask(kSessionTaskRunner, base::BindOnce(&MidiManagerMac::CompleteInitialization, base::Unretained(this), result));
}

// static
void MidiManagerMac::ReceiveMidiNotifyDispatch(const MIDINotification* message, void* refcon)
{
        ...
        if (notification->childType == kMIDIObjectType_Source) {
            ...
            if (it == sources_.end()) {
                ...
                if (!info.id.empty()) {
                    ...
                    AddInputPort(info);
                    MIDIPortConnectSource(midi_input_, endpoint, reinterpret_cast<void*>(sources_.size() - 1));
                }
            } else {
...
// static
void MidiManagerMac::ReadMidiDispatch(const MIDIPacketList* packet_list, void* read_proc_refcon, void* src_conn_refcon)
{
    ...
    for (size_t i = 0u; i < packet_list->numPackets; i++) {
        ...
        manager->ReceiveMidiData(port_index, packet->data, packet->length, timestamp);

        packet = MIDIPacketNext(packet);
    }
}

void MidiManagerMac::SendMidiData(MidiManagerClient* client, uint32_t port_index, const std::vector<uint8_t>& data, base::TimeTicks timestamp)
{
    DCHECK(service()->task_service()->IsOnTaskRunner(kClientTaskRunner));
    ...
    for (size_t sent_size = 0u; sent_size < data.size(); sent_size += send_size) {
        ...
        midi_packet = MIDIPacketListAdd(packet_list, kCoreMIDIMaxPacketListSize, midi_packet, coremidi_timestamp, send_size, &data[sent_size]);
        DCHECK(midi_packet);
