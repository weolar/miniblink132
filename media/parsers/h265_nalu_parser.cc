...
void H265NaluParser::SetEncryptedStream(const uint8_t* stream, off_t stream_size, const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK(stream);
...
bool H265NaluParser::LocateNALU(off_t* nalu_size, off_t* start_code_size)
{
    ...
    if (!H264Parser::FindStartCodeInClearRanges(stream_, bytes_left_, encrypted_ranges_, &nalu_start_off, &annexb_start_code_size)) {
    ...
    if (!H264Parser::FindStartCodeInClearRanges(nalu_data, max_nalu_data_size, encrypted_ranges_, &nalu_size_without_start_code, &next_start_code_size)) {
...
H265NaluParser::Result H265NaluParser::AdvanceToNextNALU(H265NALU* nalu)
{
    ...
    previous_nalu_range_.clear();
    previous_nalu_range_.Add(nalu->data.get(), (nalu->data + base::checked_cast<size_t>(nalu->size)).get());
    return kOk;
}
