...
void H266NaluParser::SetEncryptedStream(const uint8_t* stream, off_t stream_size, const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK(stream);
...
H266NaluParser::Result H266NaluParser::AdvanceToNextNALU(H266NALU* nalu)
{
    ...
    if (nalu->nal_unit_type >= H266NALU::kIDRWithRADL && nalu->nal_unit_type <= H266NALU::kReservedIRAP11) {
