...
public:
    ...
    Vp9CompressedHeaderParser& operator=(const Vp9CompressedHeaderParser&) = delete;
    ...
    bool ParseNoContext(const uint8_t* stream, off_t frame_size, Vp9FrameHeader* fhdr);

private:
    bool ParseInternal(const uint8_t* stream, off_t frame_size, Vp9FrameHeader* fhdr);

    void ReadTxMode(Vp9FrameHeader* fhdr);
    uint8_t DecodeTermSubexp();
    void DiffUpdateProb(Vp9Prob* prob);
    template <int N> void DiffUpdateProbArray(Vp9Prob (&prob_array)[N]);
    ...
    void ReadMvProbs(bool allow_high_precision_mv, Vp9FrameContext* frame_context);
    void UpdateMvProb(Vp9Prob* prob);
    template <int N> void UpdateMvProbArray(Vp9Prob (&prob_array)[N]);
