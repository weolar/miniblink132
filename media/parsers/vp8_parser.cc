...
bool Vp8Parser::ParseFrame(const uint8_t* ptr, size_t frame_size, Vp8FrameHeader* fhdr)
{
    stream_ = ptr;
    ...
    *fhdr = Vp8FrameHeader();
    ...
    DVLOG(4) << "Frame parsed, start: " << static_cast<const void*>(ptr) << ", size: " << frame_size << ", offsets: to first_part=" << fhdr->first_part_offset
...
bool Vp8Parser::ParseFrameTag(Vp8FrameHeader* fhdr)
{
    ...
    fhdr->frame_type = static_cast<Vp8FrameHeader::FrameType>(GetBitsAt(frame_tag, 0, 1));
...
bool Vp8Parser::ParseFrameHeader(Vp8FrameHeader* fhdr)
{
    ...
    if (!ParseIntraProbs(&fhdr->entropy_hdr, fhdr->refresh_entropy_probs, keyframe))
        return false;
...
const uint8_t kCoeffUpdateProbs[kNumBlockTypes][kNumCoeffBands][kNumPrevCoeffContexts][kNumEntropyNodes] = {
    {
        {
...
const uint8_t kDefaultCoeffProbs[kNumBlockTypes][kNumCoeffBands][kNumPrevCoeffContexts][kNumEntropyNodes] = {
    { {
        ...
        } },
    {
        ...
        { { 1, 182, 225, 249, 219, 240, 255, 224, 128, 128, 128 }, { 149, 150, 226, 252, 216, 205, 255, 171, 128, 128, 128 },
...
{
    static_assert(sizeof(curr_entropy_hdr_.coeff_probs) == sizeof(kDefaultCoeffProbs), "coeff_probs_arrays_must_be_of_correct_size");
    memcpy(curr_entropy_hdr_.coeff_probs, kDefaultCoeffProbs, sizeof(curr_entropy_hdr_.coeff_probs));

    static_assert(sizeof(curr_entropy_hdr_.mv_probs) == sizeof(kDefaultMVProbs), "mv_probs_arrays_must_be_of_correct_size");
    memcpy(curr_entropy_hdr_.mv_probs, kDefaultMVProbs, sizeof(curr_entropy_hdr_.mv_probs));

    static_assert(sizeof(curr_entropy_hdr_.y_mode_probs) == sizeof(kDefaultYModeProbs), "y_probs_arrays_must_be_of_correct_size");
    memcpy(curr_entropy_hdr_.y_mode_probs, kDefaultYModeProbs, sizeof(curr_entropy_hdr_.y_mode_probs));

    static_assert(sizeof(curr_entropy_hdr_.uv_mode_probs) == sizeof(kDefaultUVModeProbs), "uv_probs_arrays_must_be_of_correct_size");
    memcpy(curr_entropy_hdr_.uv_mode_probs, kDefaultUVModeProbs, sizeof(curr_entropy_hdr_.uv_mode_probs));
}

bool Vp8Parser::ParseTokenProbs(Vp8EntropyHeader* ehdr, bool update_curr_probs)
{
        ...
        for (size_t j = 0; j < kNumCoeffBands; ++j) {
            for (size_t k = 0; k < kNumPrevCoeffContexts; ++k) {
                for (size_t l = 0; l < kNumEntropyNodes; ++l) {
                    ...
                    BD_READ_BOOL_WITH_PROB_OR_RETURN(&coeff_prob_update_flag, kCoeffUpdateProbs[i][j][k][l]);
    ...
    if (update_curr_probs) {
        memcpy(curr_entropy_hdr_.coeff_probs, ehdr->coeff_probs, sizeof(curr_entropy_hdr_.coeff_probs));
    }

    return true;
}

bool Vp8Parser::ParseIntraProbs(Vp8EntropyHeader* ehdr, bool update_curr_probs, bool keyframe)
{
    if (keyframe) {
        static_assert(sizeof(ehdr->y_mode_probs) == sizeof(kKeyframeYModeProbs), "y_probs_arrays_must_be_of_correct_size");
        memcpy(ehdr->y_mode_probs, kKeyframeYModeProbs, sizeof(ehdr->y_mode_probs));

        static_assert(sizeof(ehdr->uv_mode_probs) == sizeof(kKeyframeUVModeProbs), "uv_probs_arrays_must_be_of_correct_size");
        memcpy(ehdr->uv_mode_probs, kKeyframeUVModeProbs, sizeof(ehdr->uv_mode_probs));
    } else {
        ...
        if (intra_16x16_prob_update_flag) {
            ...
            if (update_curr_probs) {
                memcpy(curr_entropy_hdr_.y_mode_probs, ehdr->y_mode_probs, sizeof(curr_entropy_hdr_.y_mode_probs));
            }
        }
        ...
        if (intra_chroma_prob_update_flag) {
            ...
            if (update_curr_probs) {
                memcpy(curr_entropy_hdr_.uv_mode_probs, ehdr->uv_mode_probs, sizeof(curr_entropy_hdr_.uv_mode_probs));
            }
        }
    }
...
bool Vp8Parser::ParseMVProbs(Vp8EntropyHeader* ehdr, bool update_curr_probs)
{
    for (size_t mv_ctx = 0; mv_ctx < kNumMVContexts; ++mv_ctx) {
        for (size_t p = 0; p < kNumMVProbs; ++p) {
            ...
            BD_READ_BOOL_WITH_PROB_OR_RETURN(&mv_prob_update_flag, kMVUpdateProbs[mv_ctx][p]);
    ...
    if (update_curr_probs) {
        memcpy(curr_entropy_hdr_.mv_probs, ehdr->mv_probs, sizeof(curr_entropy_hdr_.mv_probs));
    }
...
bool Vp8Parser::ParsePartitions(Vp8FrameHeader* fhdr)
{
    ...
    const uint8_t* ptr = fhdr->data + base::checked_cast<size_t>(fhdr->first_part_offset) + fhdr->first_part_size;
