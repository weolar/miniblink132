...
const JpegHuffmanTable kDefaultAcTable[kJpegMaxHuffmanTableNumBaseline] = {
    // luminance AC coefficients
    {
        ...
        { ,
    ...
    // chrominance AC coefficients
    {
        ...
        {  },
    },
};

constexpr uint8_t kZigZag8x8[64] = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35,
...
static bool ParseSOF(base::span<const uint8_t> buffer, JpegFrameHeader* frame_header)
{
    ...
    if (precision != 8) {
        DLOG(ERROR) << "Only support 8-bit precision, not " << static_cast<int>(precision) << " bit for baseline";
        return false;
    }
    if (!InRange(frame_header->num_components, 1, std::size(frame_header->components))) {
        DLOG(ERROR) << "num_components=" << static_cast<int>(frame_header->num_components) << " is not supported";
        return false;
    }
    ...
    for (size_t i = 0; i < frame_header->num_components; i++) {
        ...
        if (component.id > frame_header->num_components) {
            DLOG(ERROR) << "component id (" << static_cast<int>(component.id) << ") should be <= num_components ("
        ...
        if (!InRange(component.horizontal_sampling_factor, 1, 4)) {
            DVLOG(1) << "Invalid horizontal sampling factor " << static_cast<int>(component.horizontal_sampling_factor);
            return false;
        }
        if (!InRange(component.vertical_sampling_factor, 1, 4)) {
            DVLOG(1) << "Invalid vertical sampling factor " << static_cast<int>(component.horizontal_sampling_factor);
            return false;
        }
...
static bool ParseDQT(base::span<const uint8_t> buffer, JpegQuantizationTable* q_table)
{
    ...
    while (reader.remaining() > 0u) {
        ...
        if (table_id >= kJpegMaxQuantizationTableNum) {
            DLOG(ERROR) << "Quantization table id (" << static_cast<int>(table_id) << ") exceeded " << kJpegMaxQuantizationTableNum;
            return false;
        }
...
static bool ParseDHT(base::span<const uint8_t> buffer, JpegHuffmanTable* dc_table, JpegHuffmanTable* ac_table)
{
    ...
    while (reader.remaining() > 0u) {
        ...
        if (table_id >= 2) {
            DLOG(ERROR) << "Table id(" << table_id << ") >= 2 is invalid for baseline profile";
            return false;
        }
...
static bool ParseDRI(base::span<const uint8_t> buffer, uint16_t* restart_interval)
{
...
static bool ParseSOS(base::span<const uint8_t> buffer, const JpegFrameHeader& frame_header, JpegScanHeader* scan)
{
    ...
    if (scan->num_components != frame_header.num_components) {
        DLOG(ERROR) << "The number of scan components (" << static_cast<int>(scan->num_components) << ") mismatches the number of image components ("
                    << static_cast<int>(frame_header.num_components) << ")";
        return false;
    }
    ...
    for (int i = 0; i < scan->num_components; i++) {
        ...
        if (component->dc_selector >= kJpegMaxHuffmanTableNumBaseline) {
            DLOG(ERROR) << "DC selector (" << static_cast<int>(component->dc_selector) << ") should be 0 or 1 for baseline mode";
            return false;
        }
        if (component->ac_selector >= kJpegMaxHuffmanTableNumBaseline) {
            DLOG(ERROR) << "AC selector (" << static_cast<int>(component->ac_selector) << ") should be 0 or 1 for baseline mode";
            return false;
        }
    }
...
// otherwise.
static bool SearchEOI(base::span<const uint8_t> buffer, const char** eoi_begin_ptr, const char** eoi_end_ptr)
{
    ...
    while (reader.remaining() > 0u) {
        ...
        default:
            ...
            if (size < sizeof(size)) {
                DLOG(ERROR) << "Ill-formed JPEG. Segment size (" << size << ") is smaller than size field (" << sizeof(size) << ")";
                return false;
            }
            size -= sizeof(size);

            if (!reader.Skip(size)) {
                DLOG(ERROR) << "Ill-formed JPEG. Remaining size (" << reader.remaining() << ") is smaller than header specified (" << size << ")";
                return false;
            }
            break;
        }
...
static bool ParseSOI(base::span<const uint8_t> buffer, JpegParseResult* result)
{
    ...
    while (!has_marker_sos) {
        ...
        switch (marker2) {
        case JPEG_SOF0:
            if (!ParseSOF(reader.remaining_span().first(size), &result->frame_header)) {
        ...
        case JPEG_SOF14:
        case JPEG_SOF15:
            DLOG(ERROR) << "Only SOF0 (baseline) is supported, but got SOF" << (marker2 - JPEG_SOF0);
            return false;
        case JPEG_DQT:
            ...
            break;
        case JPEG_DHT:
            if (!ParseDHT(reader.remaining_span().first(size), result->dc_table, result->ac_table)) {
                DLOG(ERROR) << "ParseDHT failed";
                return false;
            }
            break;
        case JPEG_DRI:
            if (!ParseDRI(reader.remaining_span().first(size), &result->restart_interval)) {
                DLOG(ERROR) << "ParseDRI failed";
                return false;
            }
            break;
        case JPEG_SOS:
            if (!ParseSOS(reader.remaining_span().first(size), result->frame_header, &result->scan)) {
