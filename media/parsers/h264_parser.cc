...
// static
void H264SPS::GetLevelConfigFromProfileLevel(VideoCodecProfile profile, uint8_t level, int* level_idc, bool* constraint_set3_flag)
{
    // Spec A.3.1.
    ...
    if ((profile == H264PROFILE_BASELINE || profile == H264PROFILE_MAIN) && level == kLevelIDC1B) {
        *level_idc = 11;
...
std::optional<gfx::Size> H264SPS::GetCodedSize() const
{
    ...
    return gfx::Size(mb_unit * (pic_width_in_mbs_minus1 + 1), map_unit * (pic_height_in_map_units_minus1 + 1));
}
...
std::optional<gfx::Rect> H264SPS::GetVisibleRect() const
{
    ...
    if (coded_size->width() / crop_unit_x < frame_crop_left_offset || coded_size->width() / crop_unit_x < frame_crop_right_offset
        || coded_size->height() / crop_unit_y < frame_crop_top_offset || coded_size->height() / crop_unit_y < frame_crop_bottom_offset) {
    ...
    return gfx::Rect(crop_left, crop_top, coded_size->width() - crop_left - crop_right, coded_size->height() - crop_top - crop_bottom);
}
...
VideoColorSpace H264SPS::GetColorSpace() const
{
    if (colour_description_present_flag) {
        return VideoColorSpace(colour_primaries, transfer_characteristics, matrix_coefficients,
            video_full_range_flag ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
    } else {
...
uint8_t H264SPS::GetIndicatedLevel() const
{
    ...
    if ((profile_idc == H264SPS::kProfileIDCBaseline || profile_idc == H264SPS::kProfileIDCConstrainedBaseline || profile_idc == H264SPS::kProfileIDCMain)
...
static const int kTableSarWidth[] = { 0, 1, 12, 10, 16, 40, 24, 20, 32, 80, 18, 15, 64, 160, 4, 3, 2 };
static const int kTableSarHeight[] = { 0, 1, 11, 11, 11, 33, 11, 11, 11, 33, 11, 11, 33, 99, 3, 2, 1 };
...
void H264Parser::SetEncryptedStream(const uint8_t* stream, off_t stream_size, const std::vector<SubsampleEntry>& subsamples)
{
    DCHECK(stream);
...
// static
bool H264Parser::FindStartCode(const uint8_t* data, off_t data_size, off_t* offset, off_t* start_code_size)
{
...
bool H264Parser::LocateNALU(off_t* nalu_size, off_t* start_code_size)
{
    ...
    if (!FindStartCodeInClearRanges(stream_, bytes_left_, encrypted_ranges_, &nalu_start_off, &annexb_start_code_size)) {
    ...
    const uint8_t* nalu_data = stream_ + base::checked_cast<size_t>(annexb_start_code_size);
    ...
    if (!FindStartCodeInClearRanges(nalu_data, max_nalu_data_size, encrypted_ranges_, &nalu_size_without_start_code, &next_start_code_size)) {
...
bool H264Parser::FindStartCodeInClearRanges(
    const uint8_t* data, off_t data_size, const Ranges<const uint8_t*>& encrypted_ranges, off_t* offset, off_t* start_code_size)
{
...
// static
bool H264Parser::ParseNALUs(const uint8_t* stream, size_t stream_size, std::vector<H264NALU>* nalus)
{
    DCHECK(nalus);
    H264Parser parser;
...
H264Parser::Result H264Parser::AdvanceToNextNALU(H264NALU* nalu)
{
    ...
    if (!LocateNALU(&nalu_size_with_start_code, &start_code_size)) {
        DVLOG(4) << "Could not find next NALU, bytes left in stream: " << bytes_left_;
        stream_ = nullptr;
        bytes_left_ = 0;
        return kEOStream;
    }
    ...
    DVLOG(4) << "NALU type: " << static_cast<int>(nalu->nal_unit_type) << " at: " << reinterpret_cast<const void*>(nalu->data.get()) << " size: " << nalu->size
             << " ref: " << static_cast<int>(nalu->nal_ref_idc);

    previous_nalu_range_.clear();
    previous_nalu_range_.Add(nalu->data.get(), nalu->data + base::checked_cast<size_t>(nalu->size));
    return kOk;
}
...
static inline void DefaultScalingList4x4(int i, uint8_t scaling_list4x4[][kH264ScalingList4x4Length])
{
    DCHECK_LT(i, 6);
...
static inline void DefaultScalingList8x8(int i, uint8_t scaling_list8x8[][kH264ScalingList8x8Length])
{
    DCHECK_LT(i, 6);
...
static void FallbackScalingList4x4(
    int i, const uint8_t default_scaling_list_intra[], const uint8_t default_scaling_list_inter[], uint8_t scaling_list4x4[][kH264ScalingList4x4Length])
{
    static const int kScalingList4x4ByteSize = sizeof(scaling_list4x4[0][0]) * kH264ScalingList4x4Length;

    switch (i) {
    case 0:
        memcpy(scaling_list4x4[i], default_scaling_list_intra, kScalingList4x4ByteSize);
        break;
    ...
    case 3:
        memcpy(scaling_list4x4[i], default_scaling_list_inter, kScalingList4x4ByteSize);
        break;
...
static void FallbackScalingList8x8(
    int i, const uint8_t default_scaling_list_intra[], const uint8_t default_scaling_list_inter[], uint8_t scaling_list8x8[][kH264ScalingList8x8Length])
{
    static const int kScalingList8x8ByteSize = sizeof(scaling_list8x8[0][0]) * kH264ScalingList8x8Length;

    switch (i) {
    case 0:
        memcpy(scaling_list8x8[i], default_scaling_list_intra, kScalingList8x8ByteSize);
        break;

    case 1:
        memcpy(scaling_list8x8[i], default_scaling_list_inter, kScalingList8x8ByteSize);
        break;
...
H264Parser::Result H264Parser::ParseScalingList(int size, uint8_t* scaling_list, bool* use_default)
{
...
H264Parser::Result H264Parser::ParseSPSScalingLists(H264SPS* sps)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (seq_scaling_list_present_flag) {
            res = ParseScalingList(std::size(sps->scaling_list4x4[i]), sps->scaling_list4x4[i], &use_default);
            if (res != kOk)
                return res;
            ...
            FallbackScalingList4x4(i, kDefault4x4Intra, kDefault4x4Inter, sps->scaling_list4x4);
        }
    }
    ...
    for (int i = 0; i < ((sps->chroma_format_idc != 3) ? 2 : 6); ++i) {
        ...
        if (seq_scaling_list_present_flag) {
            res = ParseScalingList(std::size(sps->scaling_list8x8[i]), sps->scaling_list8x8[i], &use_default);
            if (res != kOk)
                return res;
            ...
            FallbackScalingList8x8(i, kDefault8x8Intra, kDefault8x8Inter, sps->scaling_list8x8);
        }
    }
...
H264Parser::Result H264Parser::ParsePPSScalingLists(const H264SPS& sps, H264PPS* pps)
{
    // See 7.4.2.2.
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (pic_scaling_list_present_flag) {
            res = ParseScalingList(std::size(pps->scaling_list4x4[i]), pps->scaling_list4x4[i], &use_default);
            if (res != kOk)
                return res;
            ...
            if (!sps.seq_scaling_matrix_present_flag) {
                ...
                FallbackScalingList4x4(i, kDefault4x4Intra, kDefault4x4Inter, pps->scaling_list4x4);
            } else {
    ...
    if (pps->transform_8x8_mode_flag) {
        for (int i = 0; i < ((sps.chroma_format_idc != 3) ? 2 : 6); ++i) {
            ...
            if (pic_scaling_list_present_flag) {
                res = ParseScalingList(std::size(pps->scaling_list8x8[i]), pps->scaling_list8x8[i], &use_default);
                if (res != kOk)
                    return res;
                ...
                if (!sps.seq_scaling_matrix_present_flag) {
                    // Table 7-2 fallback rule A in spec.
                    FallbackScalingList8x8(i, kDefault8x8Intra, kDefault8x8Inter, pps->scaling_list8x8);
                } else {
                    // Table 7-2 fallback rule B in spec.
...
H264Parser::Result H264Parser::ParseAndIgnoreHRDParameters(bool* hrd_parameters_present)
{
    int data;
...
H264Parser::Result H264Parser::ParseVUIParameters(H264SPS* sps)
{
    ...
    if (sps->bitstream_restriction_flag) {
        ...
        IN_RANGE_OR_RETURN(sps->max_num_reorder_frames, 0, sps->max_dec_frame_buffering);
    }
...
H264Parser::Result H264Parser::ParseSPS(int* sps_id)
{
    ...
    *sps_id = -1;
    ...
    if (sps->profile_idc == 100 || sps->profile_idc == 110 || sps->profile_idc == 122 || sps->profile_idc == 244 || sps->profile_idc == 44
        || sps->profile_idc == 83 || sps->profile_idc == 86 || sps->profile_idc == 118 || sps->profile_idc == 128) {
...
H264Parser::Result H264Parser::ParsePPS(int* pps_id)
{
    ...
    *pps_id = -1;
    ...
    if (encrypted_ranges_.size()) {
        ...
        const uint8_t* next_byte = previous_nalu_range_.end(0) - br_.NumBitsLeft() / 8;
        pps_range.Add(next_byte, next_byte + 1);
        pps_remainder_unencrypted = (encrypted_ranges_.IntersectionWith(pps_range).size() == 0);
    }
...
H264Parser::Result H264Parser::ParseRefPicListModification(int num_ref_idx_active_minus1, H264ModificationOfPicNum* ref_list_mods)
{
...
H264Parser::Result H264Parser::ParseRefPicListModifications(H264SliceHeader* shdr)
{
    Result res;
    ...
    if (!shdr->IsISlice() && !shdr->IsSISlice()) {
        ...
        if (shdr->ref_pic_list_modification_flag_l0) {
            res = ParseRefPicListModification(shdr->num_ref_idx_l0_active_minus1, shdr->ref_list_l0_modifications);
            if (res != kOk)
                return res;
        }
    }
    ...
    if (shdr->IsBSlice()) {
        ...
        if (shdr->ref_pic_list_modification_flag_l1) {
            res = ParseRefPicListModification(shdr->num_ref_idx_l1_active_minus1, shdr->ref_list_l1_modifications);
            if (res != kOk)
                return res;
        }
    }
...
H264Parser::Result H264Parser::ParseWeightingFactors(
    int num_ref_idx_active_minus1, int chroma_array_type, int luma_log2_weight_denom, int chroma_log2_weight_denom, H264WeightingFactors* w_facts)
{
...
H264Parser::Result H264Parser::ParsePredWeightTable(const H264SPS& sps, H264SliceHeader* shdr)
{
    ...
    Result res = ParseWeightingFactors(
        shdr->num_ref_idx_l0_active_minus1, sps.chroma_array_type, shdr->luma_log2_weight_denom, shdr->chroma_log2_weight_denom, &shdr->pred_weight_table_l0);
    if (res != kOk)
        return res;

    if (shdr->IsBSlice()) {
        res = ParseWeightingFactors(shdr->num_ref_idx_l1_active_minus1, sps.chroma_array_type, shdr->luma_log2_weight_denom, shdr->chroma_log2_weight_denom,
...
H264Parser::Result H264Parser::ParseSliceHeader(const H264NALU& nalu, H264SliceHeader* shdr)
{
    // See 7.4.3.
    const H264SPS* sps;
    const H264PPS* pps;
    ...
    *shdr = H264SliceHeader();
    ...
    if (sps->pic_order_cnt_type == 0) {
        READ_BITS_OR_RETURN(sps->log2_max_pic_order_cnt_lsb_minus4 + 4, &shdr->pic_order_cnt_lsb);
...
H264Parser::Result H264Parser::ParseSEI(H264SEI* sei)
{
    ...
    do {
        ...
        case H264SEIMessage::kSEIRecoveryPoint:
            READ_UE_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.recovery_point.recovery_frame_cnt, &num_bits_remain);
            READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.recovery_point.exact_match_flag, &num_bits_remain);
            READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.recovery_point.broken_link_flag, &num_bits_remain);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(2, &sei_msg.recovery_point.changing_slice_group_idc, &num_bits_remain);
            break;
...
{
    DCHECK_EQ(previous_nalu_range_.size(), 1u) << "This should only be called after a "
                                                  "successful call to AdvanceToNextNalu()";

    auto intersection = encrypted_ranges_.IntersectionWith(previous_nalu_range_);
    return EncryptedRangesToSubsampleEntry(previous_nalu_range_.start(0), previous_nalu_range_.end(0), intersection);
}
