...
int32_t H265POC::ComputePicOrderCnt(const H265SPS* sps, const H265PPS* pps, const H265SliceHeader& slice_hdr)
{
    ...
    if (slice_hdr.irap_pic) {
        // 8.1.3
        no_rasl_output_flag = (slice_hdr.nal_unit_type >= H265NALU::BLA_W_LP && slice_hdr.nal_unit_type <= H265NALU::IDR_N_LP) || first_picture_;
    } else {
    ...
    if (!slice_hdr.irap_pic || !no_rasl_output_flag) {
        ...
        if ((slice_hdr.slice_pic_order_cnt_lsb < prev_pic_order_cnt_lsb)
            && ((prev_pic_order_cnt_lsb - slice_hdr.slice_pic_order_cnt_lsb) >= (max_pic_order_cnt_lsb / 2))) {
            ...
            && ((slice_hdr.slice_pic_order_cnt_lsb - prev_pic_order_cnt_lsb) > (max_pic_order_cnt_lsb / 2))) {
            pic_order_cnt_msb = prev_pic_order_cnt_msb - max_pic_order_cnt_lsb;
        } else {
    ...
    if (!slice_hdr.temporal_id && (slice_hdr.nal_unit_type < H265NALU::RADL_N || slice_hdr.nal_unit_type > H265NALU::RSV_VCL_N14)) {
