...
constexpr int kTableSarWidth[] = { 0, 1, 12, 10, 16, 40, 24, 20, 32, 80, 18, 15, 64, 160, 4, 3, 2 };
constexpr int kTableSarHeight[] = { 0, 1, 11, 11, 11, 33, 11, 11, 11, 33, 11, 11, 33, 99, 3, 2, 1 };
static_assert(std::size(kTableSarWidth) == std::size(kTableSarHeight), "sar tables must have the same size");

void FillInDefaultScalingListData(H265ScalingListData* scaling_list_data, int size_id, int matrix_id)
{
    if (size_id == 0) {
        std::fill_n(
            scaling_list_data->scaling_list_4x4[matrix_id], H265ScalingListData::kScalingListSizeId0Count, H265ScalingListData::kDefaultScalingListSize0Values);
        return;
    }
    ...
    memcpy(dst, src, H265ScalingListData::kScalingListSizeId1To3Count * sizeof(*src));
...
uint8_t H265ScalingListData::GetScalingList4x4EntryInRasterOrder(size_t matrix_id, size_t raster_idx) const
{
    ...
    CHECK_LT(raster_idx, std::size(k4x4RasterScanOrderIdxToUpRightDiagScanOrderIdx));
    const size_t up_right_diag_idx = k4x4RasterScanOrderIdxToUpRightDiagScanOrderIdx[raster_idx];
    return scaling_list_4x4[matrix_id][up_right_diag_idx];
}

uint8_t H265ScalingListData::GetScalingList8x8EntryInRasterOrder(size_t matrix_id, size_t raster_idx) const
{
    ...
    CHECK_LT(raster_idx, std::size(k8x8RasterScanOrderIdxToUpRightDiagScanOrderIdx));
    const size_t up_right_diag_idx = k8x8RasterScanOrderIdxToUpRightDiagScanOrderIdx[raster_idx];
    return scaling_list_8x8[matrix_id][up_right_diag_idx];
}

uint8_t H265ScalingListData::GetScalingList16x16EntryInRasterOrder(size_t matrix_id, size_t raster_idx) const
{
...
uint8_t H265ScalingListData::GetScalingList32x32EntryInRasterOrder(size_t matrix_id, size_t raster_idx) const
{
...
size_t H265ProfileTierLevel::GetDpbMaxPicBuf() const
{
    ...
    // otherwise it is 7.
    return (general_profile_idc >= kProfileIdcMain && general_profile_idc <= kProfileIdcHighThroughput) ? 6 : 7;
}
...
gfx::Rect H265SPS::GetVisibleRect() const
{
    ...
    int left = (conf_win_left_offset + vui_parameters.def_disp_win_left_offset) * sub_width_c;
    int top = (conf_win_top_offset + vui_parameters.def_disp_win_top_offset) * sub_height_c;
    int right = (conf_win_right_offset + vui_parameters.def_disp_win_right_offset) * sub_width_c;
    int bottom = (conf_win_bottom_offset + vui_parameters.def_disp_win_bottom_offset) * sub_height_c;
    return gfx::Rect(left, top, pic_width_in_luma_samples - left - right, pic_height_in_luma_samples - top - bottom);
}
...
VideoColorSpace H265SPS::GetColorSpace() const
{
    ...
    return VideoColorSpace(vui_parameters.colour_primaries, vui_parameters.transfer_characteristics, vui_parameters.matrix_coeffs,
        vui_parameters.video_full_range_flag ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
}
...
H265Parser::Result H265Parser::ParseVPS(int* vps_id)
{
    ...
    *vps_id = -1;
    ...
    res = ParseProfileTierLevel(true, vps->vps_max_sub_layers_minus1, &vps->profile_tier_level);
    if (res != kOk) {
        return res;
    }
    ...
    for (int i = vps_sub_layer_ordering_info_present_flag ? 0 : vps->vps_max_sub_layers_minus1; i <= vps->vps_max_sub_layers_minus1; ++i) {
        ...
        IN_RANGE_OR_RETURN(vps->vps_max_num_reorder_pics[i], 0, vps->vps_max_dec_pic_buffering_minus1[i]);
        if (i > 0) {
    ...
    if (vps_timing_info_present_flag) {
        ...
        IN_RANGE_OR_RETURN(vps_num_hrd_parameters, 0, vps->vps_num_layer_sets_minus1 + 1);
        for (int i = 0; i < vps_num_hrd_parameters; ++i) {
            int hrd_layer_set_idx_i;
            READ_UE_OR_RETURN(&hrd_layer_set_idx_i);
            IN_RANGE_OR_RETURN(hrd_layer_set_idx_i, vps->vps_base_layer_internal_flag ? 0 : 1, vps->vps_num_layer_sets_minus1 + 1);
    ...
    if (vps_extension_flag) {
        ...
        if (vps->vps_max_layers_minus1 > 0 && vps->vps_base_layer_internal_flag) {
            ...
            res = ParseProfileTierLevel(false, vps->vps_max_sub_layers_minus1, &profile_tier_level);
            if (res != kOk) {
                return res;
            }
        }
        ...
        if (splitting_flag) {
            ...
            for (int j = 1; j <= num_scalability_types - 1; ++j) {
                dim_bit_offset[j] = dim_bit_offset[j - 1] + dimension_id_len_minus1[j - 1] + 1;
            }
            ...
            if (num_scalability_types) {
                dimension_id_len_minus1[num_scalability_types - 1] = 5 - dim_bit_offset[num_scalability_types - 1];
            }
        ...
        for (int i = 1; i <= std::min(62, vps->vps_max_layers_minus1); ++i) {
            ...
            if (!splitting_flag) {
                for (int j = 0; j < num_scalability_types; ++j) {
                    READ_BITS_OR_RETURN(dimension_id_len_minus1[j] + 1, &dimension_id_i[j]);
                }
            } else {
                // F.7.4.3.1.1 dimension_id
                for (int j = 0; j < num_scalability_types; ++j) {
                    dimension_id_i[j] = (layer_id_in_nuh_i & ((1 << dim_bit_offset[j + 1]) - 1)) >> dim_bit_offset[j];
                }
            }
...
H265Parser::Result H265Parser::ParseSPS(int* sps_id)
{
    ...
    *sps_id = -1;
    ...
    res = ParseProfileTierLevel(true, sps->sps_max_sub_layers_minus1, &sps->profile_tier_level);
    if (res != kOk)
        return res;
    ...
    sps->max_pic_order_cnt_lsb = std::pow(2, sps->log2_max_pic_order_cnt_lsb_minus4 + 4);
    ...
    for (int i = sps_sub_layer_ordering_info_present_flag ? 0 : sps->sps_max_sub_layers_minus1; i <= sps->sps_max_sub_layers_minus1; ++i) {
        ...
        IN_RANGE_OR_RETURN(sps->sps_max_dec_pic_buffering_minus1[i], 0, static_cast<int>(sps->max_dpb_size) - 1);
        READ_UE_OR_RETURN(&sps->sps_max_num_reorder_pics[i]);
        IN_RANGE_OR_RETURN(sps->sps_max_num_reorder_pics[i], 0, sps->sps_max_dec_pic_buffering_minus1[i]);
        if (i > 0) {
    ...
    sps->pic_width_in_ctbs_y = base::ClampCeil(static_cast<float>(sps->pic_width_in_luma_samples) / ctb_size_y);
    sps->pic_height_in_ctbs_y = base::ClampCeil(static_cast<float>(sps->pic_height_in_luma_samples) / ctb_size_y);
    ...
    IN_RANGE_OR_RETURN(sps->max_transform_hierarchy_depth_inter, 0, sps->ctb_log2_size_y - min_tb_log2_size_y);
    READ_UE_OR_RETURN(&sps->max_transform_hierarchy_depth_intra);
    IN_RANGE_OR_RETURN(sps->max_transform_hierarchy_depth_intra, 0, sps->ctb_log2_size_y - min_tb_log2_size_y);
    ...
    if (sps->sps_scaling_list_data_present_flag) {
        ...
        for (int size_id = 0; size_id < 4; ++size_id) {
            for (int matrix_id = 0; matrix_id < 6; matrix_id += (size_id == 3) ? 3 : 1) {
                FillInDefaultScalingListData(&sps->scaling_list_data, size_id, matrix_id);
            }
        }
    }
    ...
    if (sps->pcm_enabled_flag) {
        ...
        IN_RANGE_OR_RETURN(log2_min_ipcm_cb_size_y, std::min(min_cb_log2_size_y, 5), std::min(sps->ctb_log2_size_y, 5));
    ...
    IN_RANGE_OR_RETURN(sps->num_short_term_ref_pic_sets, 0, kMaxShortTermRefPicSets);
    ...
    if (sps->long_term_ref_pics_present_flag) {
        ...
        IN_RANGE_OR_RETURN(sps->num_long_term_ref_pics_sps, 0, kMaxLongTermRefPicSets);
        for (int i = 0; i < sps->num_long_term_ref_pics_sps; ++i) {
            READ_BITS_OR_RETURN(sps->log2_max_pic_order_cnt_lsb_minus4 + 4, &sps->lt_ref_pic_poc_lsb_sps[i]);
    ...
    if (vui_parameters_present_flag) {
        ...
        // ranges previously.
        base::CheckedNumeric<int> width_crop = sps->conf_win_left_offset + sps->conf_win_right_offset;
        ...
        if (!width_crop.IsValid())
            return kInvalidStream;
        TRUE_OR_RETURN(width_crop.ValueOrDefault(0) < sps->pic_width_in_luma_samples);
        base::CheckedNumeric<int> height_crop = sps->conf_win_top_offset + sps->conf_win_bottom_offset;
    ...
    sps->wp_offset_half_range_y = 1 << (sps->high_precision_offsets_enabled_flag ? sps->bit_depth_luma_minus8 + 7 : 7);
    sps->wp_offset_half_range_c = 1 << (sps->high_precision_offsets_enabled_flag ? sps->bit_depth_chroma_minus8 + 7 : 7);
...
H265Parser::Result H265Parser::ParsePPS(const H265NALU& nalu, int* pps_id)
{
    ...
    *pps_id = -1;
    ...
    IN_RANGE_OR_RETURN(pps->num_ref_idx_l0_default_active_minus1, 0, kMaxRefIdxActive - 1);
    READ_UE_OR_RETURN(&pps->num_ref_idx_l1_default_active_minus1);
    IN_RANGE_OR_RETURN(pps->num_ref_idx_l1_default_active_minus1, 0, kMaxRefIdxActive - 1);
    ...
    if (pps->cu_qp_delta_enabled_flag) {
        ...
        IN_RANGE_OR_RETURN(pps->diff_cu_qp_delta_depth, 0, sps->log2_diff_max_min_luma_coding_block_size);
    }
    ...
    if (pps->tiles_enabled_flag) {
        ...
        IN_RANGE_OR_RETURN(pps->num_tile_columns_minus1, 0, sps->pic_width_in_ctbs_y - 1);
        TRUE_OR_RETURN(pps->num_tile_columns_minus1 < H265PPS::kMaxNumTileColumnWidth);
        READ_UE_OR_RETURN(&pps->num_tile_rows_minus1);
        IN_RANGE_OR_RETURN(pps->num_tile_rows_minus1, 0, sps->pic_height_in_ctbs_y - 1);
        ...
        if (!pps->uniform_spacing_flag) {
            ...
            for (int i = 0; i < pps->num_tile_columns_minus1; ++i) {
                ...
                IN_RANGE_OR_RETURN(pps->column_width_minus1[i], 0, pps->column_width_minus1[pps->num_tile_columns_minus1] - 1);
            ...
            for (int i = 0; i < pps->num_tile_rows_minus1; ++i) {
                ...
                IN_RANGE_OR_RETURN(pps->row_height_minus1[i], 0, pps->row_height_minus1[pps->num_tile_rows_minus1] - 1);
    ...
    IN_RANGE_OR_RETURN(pps->log2_parallel_merge_level_minus2, 0, sps->ctb_log2_size_y - 2);
    ...
    if (pps->pps_range_extension_flag) {
        ...
        if (pps->chroma_qp_offset_list_enabled_flag) {
            ...
            IN_RANGE_OR_RETURN(pps->diff_cu_chroma_qp_offset_depth, 0, sps->log2_diff_max_min_luma_coding_block_size);
        ...
        IN_RANGE_OR_RETURN(pps->log2_sao_offset_scale_luma, 0, std::max(sps->bit_depth_luma_minus8 - 2, 0));
        READ_UE_OR_RETURN(&pps->log2_sao_offset_scale_chroma);
        IN_RANGE_OR_RETURN(pps->log2_sao_offset_scale_chroma, 0, std::max(sps->bit_depth_chroma_minus8 - 2, 0));
    }
...
H265Parser::Result H265Parser::ParseSliceHeaderForPictureParameterSets(const H265NALU& nalu, int* pps_id)
{
...
H265Parser::Result H265Parser::ParseSliceHeader(const H265NALU& nalu, H265SliceHeader* shdr, H265SliceHeader* prior_shdr)
{
    // 7.4.7 Slice segment header
    ...
    int clamped_temporal_id = std::min(shdr->temporal_id, sps->sps_max_sub_layers_minus1);

    if (!shdr->first_slice_segment_in_pic_flag) {
        if (pps->dependent_slice_segments_enabled_flag)
            READ_BOOL_OR_RETURN(&shdr->dependent_slice_segment_flag);
        READ_BITS_OR_RETURN(base::bits::Log2Ceiling(sps->pic_size_in_ctbs_y), &shdr->slice_segment_address);
        IN_RANGE_OR_RETURN(shdr->slice_segment_address, 0, sps->pic_size_in_ctbs_y - 1);
    }
    if (shdr->dependent_slice_segment_flag) {
        ...
        memcpy(reinterpret_cast<uint8_t*>(shdr) + skip_amount, reinterpret_cast<uint8_t*>(prior_shdr) + skip_amount, sizeof(H265SliceHeader) - skip_amount);
        ...
        if ((shdr->irap_pic || sps->sps_max_dec_pic_buffering_minus1[clamped_temporal_id] == 0) && nalu.nuh_layer_id == 0) {
        ...
        if (shdr->nal_unit_type != H265NALU::IDR_W_RADL && shdr->nal_unit_type != H265NALU::IDR_N_LP) {
            READ_BITS_OR_RETURN(sps->log2_max_pic_order_cnt_lsb_minus4 + 4, &shdr->slice_pic_order_cnt_lsb);
            IN_RANGE_OR_RETURN(shdr->slice_pic_order_cnt_lsb, 0, sps->max_pic_order_cnt_lsb - 1);
            ...
            if (!shdr->short_term_ref_pic_set_sps_flag) {
                ...
                res = ParseStRefPicSet(sps->num_short_term_ref_pic_sets, *sps, &shdr->st_ref_pic_set, true);
                if (res != kOk)
                    return res;
                shdr->st_rps_bits = (bits_left_prior - br_.NumBitsLeft()) - 8 * (br_.NumEmulationPreventionBytesRead() - num_epb_prior);
            } else if (sps->num_short_term_ref_pic_sets > 1) {
                READ_BITS_OR_RETURN(base::bits::Log2Ceiling(sps->num_short_term_ref_pic_sets), &shdr->short_term_ref_pic_set_idx);
                IN_RANGE_OR_RETURN(shdr->short_term_ref_pic_set_idx, 0, sps->num_short_term_ref_pic_sets - 1);
            }
            ...
            if (sps->long_term_ref_pics_present_flag) {
                ...
                if (sps->num_long_term_ref_pics_sps > 0) {
                    ...
                    IN_RANGE_OR_RETURN(shdr->num_long_term_sps, 0, sps->num_long_term_ref_pics_sps);
                }
                READ_UE_OR_RETURN(&shdr->num_long_term_pics);
                if (nalu.nuh_layer_id == 0) {
                    TRUE_OR_RETURN(shdr->num_long_term_pics <= (sps->sps_max_dec_pic_buffering_minus1[clamped_temporal_id]
                                       - shdr->GetStRefPicSet(sps).num_negative_pics - shdr->GetStRefPicSet(sps).num_positive_pics - shdr->num_long_term_sps));
                }
                IN_RANGE_OR_RETURN(shdr->num_long_term_pics, 0, kMaxLongTermRefPicSets - shdr->num_long_term_sps);
                for (int i = 0; i < shdr->num_long_term_sps + shdr->num_long_term_pics; ++i) {
                    if (i < shdr->num_long_term_sps) {
                        int lt_idx_sps = 0;
                        if (sps->num_long_term_ref_pics_sps > 1) {
                            READ_BITS_OR_RETURN(base::bits::Log2Ceiling(sps->num_long_term_ref_pics_sps), &lt_idx_sps);
                            IN_RANGE_OR_RETURN(lt_idx_sps, 0, sps->num_long_term_ref_pics_sps - 1);
                        }
                        ...
                        READ_BITS_OR_RETURN(sps->log2_max_pic_order_cnt_lsb_minus4 + 4, &shdr->poc_lsb_lt[i]);
                    ...
                    if (shdr->delta_poc_msb_present_flag[i]) {
                        ...
                        IN_RANGE_OR_RETURN(shdr->delta_poc_msb_cycle_lt[i], 0, std::pow(2, 32 - sps->log2_max_pic_order_cnt_lsb_minus4 - 4));
                        // Equation 7-52.
        ...
        if (shdr->IsPSlice() || shdr->IsBSlice()) {
            ...
            if (shdr->num_ref_idx_active_override_flag) {
                ...
                IN_RANGE_OR_RETURN(shdr->num_ref_idx_l0_active_minus1, 0, kMaxRefIdxActive - 1);
                if (shdr->IsBSlice()) {
                    READ_UE_OR_RETURN(&shdr->num_ref_idx_l1_active_minus1);
                    IN_RANGE_OR_RETURN(shdr->num_ref_idx_l1_active_minus1, 0, kMaxRefIdxActive - 1);
                }
            }
            ...
            if (pps->lists_modification_present_flag && shdr->num_pic_total_curr > 1) {
                res = ParseRefPicListsModifications(*shdr, &shdr->ref_pic_lists_modification);
                if (res != kOk)
                    return res;
            }
            if (shdr->IsBSlice())
            ...
            if (shdr->slice_temporal_mvp_enabled_flag) {
                ...
                if ((shdr->collocated_from_l0_flag && shdr->num_ref_idx_l0_active_minus1 > 0)
                    || (!shdr->collocated_from_l0_flag && shdr->num_ref_idx_l1_active_minus1 > 0)) {
                    READ_UE_OR_RETURN(&shdr->collocated_ref_idx);
                    if ((shdr->IsPSlice() || shdr->IsBSlice()) && shdr->collocated_from_l0_flag) {
                        IN_RANGE_OR_RETURN(shdr->collocated_ref_idx, 0, shdr->num_ref_idx_l0_active_minus1);
                    }
                    if (shdr->IsBSlice() && !shdr->collocated_from_l0_flag) {
                        IN_RANGE_OR_RETURN(shdr->collocated_ref_idx, 0, shdr->num_ref_idx_l1_active_minus1);
                    }
                }
            }
        ...
        IN_RANGE_OR_RETURN(26 + pps->init_qp_minus26 + shdr->slice_qp_delta, -pps->qp_bd_offset_y, 51);

        if (pps->pps_slice_chroma_qp_offsets_present_flag) {
            READ_SE_OR_RETURN(&shdr->slice_cb_qp_offset);
            IN_RANGE_OR_RETURN(shdr->slice_cb_qp_offset, -12, 12);
            IN_RANGE_OR_RETURN(pps->pps_cb_qp_offset + shdr->slice_cb_qp_offset, -12, 12);
            READ_SE_OR_RETURN(&shdr->slice_cr_qp_offset);
            IN_RANGE_OR_RETURN(shdr->slice_cr_qp_offset, -12, 12);
            IN_RANGE_OR_RETURN(pps->pps_cr_qp_offset + shdr->slice_cr_qp_offset, -12, 12);
        }
        ...
        if (pps->pps_loop_filter_across_slices_enabled_flag
            && (shdr->slice_sao_luma_flag || shdr->slice_sao_chroma_flag || !shdr->slice_deblocking_filter_disabled_flag)) {
    ...
    if (pps->tiles_enabled_flag || pps->entropy_coding_sync_enabled_flag) {
        ...
        if (!pps->tiles_enabled_flag) {
            IN_RANGE_OR_RETURN(num_entry_point_offsets, 0, sps->pic_height_in_ctbs_y - 1);
        } else if (!pps->entropy_coding_sync_enabled_flag) {
            IN_RANGE_OR_RETURN(num_entry_point_offsets, 0, (pps->num_tile_columns_minus1 + 1) * (pps->num_tile_rows_minus1 + 1) - 1);
        } else { // both are true
            IN_RANGE_OR_RETURN(num_entry_point_offsets, 0, (pps->num_tile_columns_minus1 + 1) * sps->pic_height_in_ctbs_y - 1);
        }
    ...
    if (prior_shdr && !shdr->first_slice_segment_in_pic_flag) {
        ...
        TRUE_OR_RETURN(!memcmp(reinterpret_cast<uint8_t*>(shdr) + block_start, reinterpret_cast<uint8_t*>(prior_shdr) + block_start, block_end - block_start));
    }
...
{
    return gfx::HdrMetadataCta861_3(max_content_light_level, max_picture_average_light_level);
}
...
gfx::HdrMetadataSmpteSt2086 H265SEIMasteringDisplayInfo::ToGfx() const
{
    ...
    return gfx::HdrMetadataSmpteSt2086(
        { display_primaries[2][0] / kChromaDenominator, display_primaries[2][1] / kChromaDenominator, display_primaries[0][0] / kChromaDenominator,
            display_primaries[0][1] / kChromaDenominator, display_primaries[1][0] / kChromaDenominator, display_primaries[1][1] / kChromaDenominator,
            white_points[0] / kChromaDenominator, white_points[1] / kChromaDenominator },
        /*luminance_max=*/max_luminance / kLumaDenoninator,
        /*luminance_min=*/min_luminance / kLumaDenoninator);
}

H265Parser::Result H265Parser::ParseProfileTierLevel(bool profile_present, int max_num_sub_layers_minus1, H265ProfileTierLevel* profile_tier_level)
{
    // 7.4.4
    ...
    if (profile_present) {
        ...
        READ_BOOL_OR_RETURN(&profile_tier_level->general_non_packed_constraint_flag);
        READ_BOOL_OR_RETURN(&profile_tier_level->general_frame_only_constraint_flag);
...
H265Parser::Result H265Parser::ParseScalingListData(H265ScalingListData* scaling_list_data)
{
    for (int size_id = 0; size_id < 4; ++size_id) {
        for (int matrix_id = 0; matrix_id < 6; matrix_id += (size_id == 3) ? 3 : 1) {
            ...
            if (!scaling_list_pred_mode_flag) {
                ...
                if (size_id <= 2) {
                    ...
                    IN_RANGE_OR_RETURN(scaling_list_pred_matrix_id_delta, 0, matrix_id / 3);
                }
...
H265Parser::Result H265Parser::ParseStRefPicSet(int st_rps_idx, const H265SPS& sps, H265StRefPicSet* st_ref_pic_set, bool is_slice_hdr)
{
    // 7.4.8
    ...
    if (inter_ref_pic_set_prediction_flag) {
        ...
        IN_RANGE_OR_RETURN(st_ref_pic_set->num_negative_pics, 0, sps.sps_max_dec_pic_buffering_minus1[sps.sps_max_sub_layers_minus1]);
        IN_RANGE_OR_RETURN(
            st_ref_pic_set->num_positive_pics, 0, sps.sps_max_dec_pic_buffering_minus1[sps.sps_max_sub_layers_minus1] - st_ref_pic_set->num_negative_pics);
    } else {
...
H265Parser::Result H265Parser::ParseVuiParameters(const H265SPS& sps, H265VUIParameters* vui)
{
    Result res = kOk;
...
H265Parser::Result H265Parser::ParseAndIgnoreHrdParameters(bool common_inf_present_flag, int max_num_sub_layers_minus1)
{
    Result res = kOk;
    int data;
    ...
    for (int i = 0; i <= max_num_sub_layers_minus1; ++i) {
        ...
        if (nal_hrd_parameters_present_flag) {
            res = ParseAndIgnoreSubLayerHrdParameters(cpb_cnt, sub_pic_hrd_params_present_flag);
            if (res != kOk)
                return res;
        }
        if (vcl_hrd_parameters_present_flag) {
...
H265Parser::Result H265Parser::ParseAndIgnoreSubLayerHrdParameters(int cpb_cnt, bool sub_pic_hrd_params_present_flag)
{
    int data;
...
H265Parser::Result H265Parser::ParseRefPicListsModifications(const H265SliceHeader& shdr, H265RefPicListsModifications* rpl_mod)
{
    ...
    if (rpl_mod->ref_pic_list_modification_flag_l0) {
        for (int i = 0; i <= shdr.num_ref_idx_l0_active_minus1; ++i) {
            READ_BITS_OR_RETURN(base::bits::Log2Ceiling(shdr.num_pic_total_curr), &rpl_mod->list_entry_l0[i]);
            IN_RANGE_OR_RETURN(rpl_mod->list_entry_l0[i], 0, shdr.num_pic_total_curr - 1);
        }
    }
    if (shdr.IsBSlice()) {
        ...
        if (rpl_mod->ref_pic_list_modification_flag_l1) {
            for (int i = 0; i <= shdr.num_ref_idx_l1_active_minus1; ++i) {
                READ_BITS_OR_RETURN(base::bits::Log2Ceiling(shdr.num_pic_total_curr), &rpl_mod->list_entry_l1[i]);
                IN_RANGE_OR_RETURN(rpl_mod->list_entry_l1[i], 0, shdr.num_pic_total_curr - 1);
            }
        }
    }
    return kOk;
...
H265Parser::Result H265Parser::ParsePredWeightTable(const H265SPS& sps, const H265SliceHeader& shdr, H265PredWeightTable* pred_weight_table)
{
    ...
    for (int i = 0; i <= shdr.num_ref_idx_l0_active_minus1; ++i) {
        if (luma_weight_flag[i]) {
            ...
            IN_RANGE_OR_RETURN(pred_weight_table->luma_offset_l0[i], -sps.wp_offset_half_range_y, sps.wp_offset_half_range_y - 1);
        }
        if (chroma_weight_flag[i]) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                IN_RANGE_OR_RETURN(pred_weight_table->delta_chroma_offset_l0[i][j], -4 * sps.wp_offset_half_range_c, 4 * sps.wp_offset_half_range_c - 1);
            }
        }
    }
    if (shdr.IsPSlice())
    ...
    if (shdr.IsBSlice()) {
        ...
        for (int i = 0; i <= shdr.num_ref_idx_l1_active_minus1; ++i) {
            if (luma_weight_flag[i]) {
                ...
                IN_RANGE_OR_RETURN(pred_weight_table->luma_offset_l1[i], -sps.wp_offset_half_range_y, sps.wp_offset_half_range_y - 1);
            }
            if (chroma_weight_flag[i]) {
                ...
                for (int j = 0; j < 2; ++j) {
                    ...
                    IN_RANGE_OR_RETURN(pred_weight_table->delta_chroma_offset_l1[i][j], -4 * sps.wp_offset_half_range_c, 4 * sps.wp_offset_half_range_c - 1);
                }
            }
        }
...
H265Parser::Result H265Parser::ParseSEI(H265SEI* sei)
{
    ...
    do {
        ...
        DVLOG(4) << "Found SEI message type: " << sei_msg.type << " payload size: " << sei_msg.payload_size;

        switch (sei_msg.type) {
        case H265SEIMessage::kSEIAlphaChannelInfo:
            READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.alpha_channel_info.alpha_channel_cancel_flag, &num_bits_remain);
            if (!sei_msg.alpha_channel_info.alpha_channel_cancel_flag) {
                READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(3, &sei_msg.alpha_channel_info.alpha_channel_use_idc, &num_bits_remain);
                READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(3, &sei_msg.alpha_channel_info.alpha_channel_bit_depth_minus8, &num_bits_remain);
                READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(
                    sei_msg.alpha_channel_info.alpha_channel_bit_depth_minus8 + 9, &sei_msg.alpha_channel_info.alpha_transparent_value, &num_bits_remain);
                READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(
                    sei_msg.alpha_channel_info.alpha_channel_bit_depth_minus8 + 9, &sei_msg.alpha_channel_info.alpha_opaque_value, &num_bits_remain);
                READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.alpha_channel_info.alpha_channel_incr_flag, &num_bits_remain);
                READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.alpha_channel_info.alpha_channel_clip_flag, &num_bits_remain);
                if (sei_msg.alpha_channel_info.alpha_channel_clip_flag) {
                    READ_BOOL_AND_MINUS_BITS_READ_OR_RETURN(&sei_msg.alpha_channel_info.alpha_channel_clip_type_flag, &num_bits_remain);
                }
            }
            break;
        case H265SEIMessage::kSEIContentLightLevelInfo:
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(16, &sei_msg.content_light_level_info.max_content_light_level, &num_bits_remain);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(16, &sei_msg.content_light_level_info.max_picture_average_light_level, &num_bits_remain);
            break;
        case H265SEIMessage::kSEIMasteringDisplayInfo:
            for (auto& primary : sei_msg.mastering_display_info.display_primaries) {
                for (auto& component : primary) {
                    READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(16, &component, &num_bits_remain);
                }
            }
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(16, &sei_msg.mastering_display_info.white_points[0], &num_bits_remain);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(16, &sei_msg.mastering_display_info.white_points[1], &num_bits_remain);
            uint32_t luminace_high_31bits, luminance_low_1bit;
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(31, &luminace_high_31bits, &num_bits_remain);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(1, &luminance_low_1bit, &num_bits_remain);
            sei_msg.mastering_display_info.max_luminance = (luminace_high_31bits << 1) + (luminance_low_1bit & 0x1);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(31, &luminace_high_31bits, &num_bits_remain);
            READ_BITS_AND_MINUS_BITS_READ_OR_RETURN(1, &luminance_low_1bit, &num_bits_remain);
            sei_msg.mastering_display_info.min_luminance = (luminace_high_31bits << 1) + (luminance_low_1bit & 0x1);
            break;
        default:
