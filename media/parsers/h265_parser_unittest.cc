...
TEST_F(H265ParserTest, SliceHeaderParsing)
{
    ...
    H265SliceHeader shdr;
    EXPECT_EQ(H265Parser::kOk, parser_.ParseSliceHeader(target_nalu, &shdr, nullptr));
...
TEST_F(H265ParserTest, SliceHeaderParsingNoValidationOnFirstSliceInFrame)
{
    ...
    while (true) {
        ...
        case H265NALU::TRAIL_R:
            [[fallthrough]];
        case H265NALU::IDR_W_RADL:
            result = parser_.ParseSliceHeader(nalu, &curr_slice_header, &last_slice_header);
...
TEST_F(H265ParserTest, HDRMetadataSEIParsing)
{
    ...
    for (auto& sei_msg : clli_sei.msgs) {
        ...
        EXPECT_EQ(sei_msg.content_light_level_info.max_picture_average_light_level, 400u);
    }
...
TEST_F(H265ParserTest, RecursiveSEIParsing)
{
    ...
    for (auto& sei_msg : clli_mdcv_sei.msgs) {
        EXPECT_TRUE(sei_msg.type == H265SEIMessage::kSEIContentLightLevelInfo || sei_msg.type == H265SEIMessage::kSEIMasteringDisplayInfo);
        switch (sei_msg.type) {
        case H265SEIMessage::kSEIContentLightLevelInfo:
            // Content light level info present.
            EXPECT_EQ(sei_msg.content_light_level_info.max_content_light_level, 1000u);
            EXPECT_EQ(sei_msg.content_light_level_info.max_picture_average_light_level, 200u);
            break;
        case H265SEIMessage::kSEIMasteringDisplayInfo:
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[0][0], 13249u);
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[0][1], 34499u);
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[1][0], 7500u);
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[1][1], 2999u);
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[2][0], 34000u);
            EXPECT_EQ(sei_msg.mastering_display_info.display_primaries[2][1], 15999u);
            EXPECT_EQ(sei_msg.mastering_display_info.white_points[0], 15635u);
