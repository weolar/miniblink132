...
struct MEDIA_EXPORT Vp9SegmentationParams {
    ...
    enum SegmentLevelFeature { SEG_LVL_ALT_Q = 0, SEG_LVL_ALT_LF = 1, SEG_LVL_REF_FRAME = 2, SEG_LVL_SKIP = 3, SEG_LVL_MAX };
...
struct MEDIA_EXPORT Vp9LoopFilterParams {
    ...
    uint8_t lvl[Vp9SegmentationParams::kNumSegments][VP9_FRAME_MAX][kNumModeDeltas];
};
...
public:
    ...
    public:
        ...
        const Vp9SegmentationParams& segmentation() const
        {
            return segmentation_;
        }

        const Vp9LoopFilterParams& loop_filter() const
        {
            return loop_filter_;
        }
    ...
    static bool IsSuperframe(const uint8_t* stream, off_t stream_size, const DecryptConfig* decrypt_config);
    ...
    static base::circular_deque<FrameInfo> ExtractFrames(const uint8_t* stream, off_t stream_size, const DecryptConfig* decrypt_config);
...
private:
    ...
    bool ParseUncompressedHeader(const FrameInfo& frame_info, Vp9FrameHeader* fhdr, Result* result, Vp9Parser::Context* context);
