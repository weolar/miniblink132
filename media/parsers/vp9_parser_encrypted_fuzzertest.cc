...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    uint8_t subsamples_count = data_provider.ConsumeIntegral<uint8_t>() % subsample_count_upper_bound;
    ...
    while (ivf_parser.ParseNextFrame(&ivf_frame_header, &ivf_payload)) {
        ...
        vp9_parser.SetStream(ivf_payload, ivf_frame_header.frame_size, media::DecryptConfig::CreateCencConfig(key_id, iv, subsamples));
        std::unique_ptr<media::DecryptConfig> null_config;
        gfx::Size allocate_size;
        while (vp9_parser.ParseNextFrame(&vp9_frame_header, &allocate_size, &null_config) == media::Vp9Parser::kOk) {
