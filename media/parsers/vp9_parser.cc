...
std::string IncrementIV(std::string_view iv, uint32_t by)
{
    ...
    counter_bytes.copy_from(base::U64ToBigEndian(base::U64FromBigEndian(counter_bytes) + by));
...
std::unique_ptr<DecryptConfig> SplitSubsamples(uint32_t frame_size, size_t* current_subsample_index, size_t* extra_clear_subsample_bytes,
    const DecryptConfig* base_decrypt_config, const std::vector<SubsampleEntry>& subsamples, std::string* iv)
{
...
Vp9Parser::FrameInfo& Vp9Parser::FrameInfo::operator=(FrameInfo&& other) = default;
...
// static
bool Vp9Parser::IsSuperframe(const uint8_t* stream, off_t stream_size, const DecryptConfig* decrypt_config)
{
    ...
    if (decrypt_config && IsByteNEncrypted(marker_offset, decrypt_config->subsamples())) {
        return false;
    }
...
// static
base::circular_deque<Vp9Parser::FrameInfo> Vp9Parser::ExtractFrames(const uint8_t* stream, off_t stream_size, const DecryptConfig* decrypt_config)
{
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        if (subsamples.size()) {
            std::unique_ptr<DecryptConfig> frame_dc = SplitSubsamples(size, &current_subsample, &extra_clear_subsample_bytes, decrypt_config, subsamples, &iv);
            if (!frame_dc) {
