...
TEST(TemporalScalabilityIdExtractorTest, H264_SvcHeaders)
{
    ...
    md.temporal_id = 99;
    EXPECT_TRUE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0x6e, 0xc0, 0x80, 0x0f, 0x20 }, 0, md));
    EXPECT_EQ(md.temporal_id, 0);

    EXPECT_TRUE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0x0e, 0x82, 0x80, 0x4f }, 0, md));
    EXPECT_EQ(md.temporal_id, 2);

    EXPECT_TRUE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0x6e, 0x81, 0x80, 0x2f, 0x20 }, 0, md));
...
TEST(TemporalScalabilityIdExtractorTest, H264_CorruptedStream)
{
    ...
    TemporalScalabilityIdExtractor::BitstreamMetadata md;
    md.temporal_id = 99;
    EXPECT_FALSE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0xff, 0xff, 0xff }, 0, md));
    EXPECT_EQ(md.temporal_id, 0);

    EXPECT_FALSE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0xff, 0xff, 0xff }, 1, md));
    EXPECT_EQ(md.temporal_id, 1);

    EXPECT_FALSE(extractor.ParseChunk(bytes { 0, 0, 0, 1, 0xff, 0xff, 0xff }, 2, md));
    EXPECT_EQ(md.temporal_id, 0);
}
...
TEST(TemporalScalabilityIdExtractorTest, AV1_TwoTemporalLayers)
{
    ...
    const std::vector<AV1TemporalScalabilityData> expected {
        { 0, 0, 0b00000000, 0b11111111 },
        { 1, 1, 0b01111111, 0b00000000 },
        { 2, 0, 0b01111111, 0b00000100 },
        { 3, 1, 0b01111111, 0b00000000 },
        { 4, 0, 0b01111111, 0b00000010 },
        { 5, 1, 0b01111111, 0b00000000 },
        { 6, 0, 0b01111111, 0b00000001 },
        { 7, 1, 0b01111111, 0b00000000 },
    };
