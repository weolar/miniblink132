...
struct MEDIA_EXPORT H264SPS {
    ...
    static void GetLevelConfigFromProfileLevel(VideoCodecProfile profile, uint8_t level, int* level_idc, bool* constraint_set3_flag);
...
struct MEDIA_EXPORT H264SliceHeader {
    ...
    raw_ptr<const uint8_t, AllowPtrArithmetic | DanglingUntriaged> nalu_data; // from NAL header
...
public:
    ...
    static bool FindStartCodeInClearRanges(const uint8_t* data, off_t data_size, const Ranges<const uint8_t*>& ranges, off_t* offset, off_t* start_code_size);
    ...
    static bool ParseNALUs(const uint8_t* stream, size_t stream_size, std::vector<H264NALU>* nalus);
...
private:
    ...
    Result ParseRefPicListModification(int num_ref_idx_active_minus1, H264ModificationOfPicNum* ref_list_mods);
