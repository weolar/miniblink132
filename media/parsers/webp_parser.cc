...
bool IsLossyWebPImage(base::span<const uint8_t> encoded_data)
{
    ...
    return !memcmp(encoded_data.data(), "RIFF", 4) && !memcmp(encoded_data.data() + kWebPFileBeginBytePosition, "WEBPVP8 ", 8);
}

std::unique_ptr<Vp8FrameHeader> ParseWebPImage(base::span<const uint8_t> encoded_data)
{
    ...
    const uint32_t file_size = *reinterpret_cast<const uint32_t*>(encoded_data.data() + kFileSizeBytePosition);
    ...
    if (base::strict_cast<size_t>(file_size) != encoded_data.size() - kWebPFileBeginBytePosition) {
        return nullptr;
    }
    ...
    const uint32_t vp8_frame_size = *reinterpret_cast<const uint32_t*>(encoded_data.data() + kVp8FrameSizePosition);

    // Check that the VP8 frame size is bounded by the WebP size.
    if (base::strict_cast<size_t>(file_size) - kWebPFileHeaderByteSize < base::strict_cast<size_t>(vp8_frame_size)) {
        return nullptr;
    }

    // Check that the size of the encoded data is consistent.
    const size_t vp8_padded_frame_size = base::bits::AlignUp(size_t { vp8_frame_size }, size_t { 2 });
    if (encoded_data.size() - kWebPFileAndVp8ChunkHeaderSizeInBytes != vp8_padded_frame_size) {
        return nullptr;
    }

    // Check that the last byte is 0 if |vp8_frame_size| is odd per WebP specs:
    // https://developers.google.com/speed/webp/docs/riff_container#riff_file_format
    if (vp8_frame_size % 2 && encoded_data.data()[encoded_data.size() - 1] != 0u) {
        return nullptr;
    }
    ...
    if (vp8_parser.ParseFrame(encoded_data.data() + kWebPFileAndVp8ChunkHeaderSizeInBytes, base::strict_cast<size_t>(vp8_frame_size), result.get())) {
        return result;
    }
    return nullptr;
}
