...
FakeVideoDecoder::FakeVideoDecoder(int decoder_id, int decoding_delay, int max_parallel_decoding_requests, const BytesDecodedCB& bytes_decoded_cb)
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    DCHECK(held_decode_callbacks_.empty()) << "No reinitialization during pending decode.";
...
void FakeVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(reset_cb_.IsNull());
    DCHECK_LE(decoded_frames_.size(), decoding_delay_ + held_decode_callbacks_.size());
    DCHECK_LT(static_cast<int>(held_decode_callbacks_.size()), max_parallel_decoding_requests_);
    DCHECK_NE(state_, STATE_END_OF_STREAM);

    int buffer_size = buffer->end_of_stream() ? 0 : buffer->size();
    DecodeCB wrapped_decode_cb
        = base::BindOnce(&FakeVideoDecoder::OnFrameDecoded, weak_factory_.GetWeakPtr(), buffer_size, base::BindPostTaskToCurrentDefault(std::move(decode_cb)));
    ...
    if (buffer->end_of_stream()) {
        if (buffer->next_config()) {
            eos_next_configs_.emplace_back(absl::get<VideoDecoderConfig>(*buffer->next_config()));

            if (enable_eliding_eos_) {
                DCHECK(held_decode_callbacks_.empty());
                current_config_ = eos_next_configs_.back();
                std::move(wrapped_decode_cb).Run(DecoderStatus::Codes::kElidedEndOfStreamForConfigChange);
                return;
            }
        }
...
scoped_refptr<VideoFrame> FakeVideoDecoder::MakeVideoFrame(const DecoderBuffer& buffer)
{
    auto frame = VideoFrame::CreateVideoHoleFrame(base::UnguessableToken(), current_config_.coded_size(), buffer.timestamp());
    DCHECK(frame);
    return frame;
}
...
void FakeVideoDecoder::SimulateError()
{
    ...
    state_ = STATE_ERROR;
    while (!held_decode_callbacks_.empty()) {
        std::move(held_decode_callbacks_.front()).Run(DecoderStatus::Codes::kFailed);
        held_decode_callbacks_.pop_front();
    }
    decoded_frames_.clear();
}
...
void FakeVideoDecoder::OnFrameDecoded(int buffer_size, DecodeCB decode_cb, DecoderStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
