...
constexpr char kInitialFetchPlaylist[] = "#EXTM3U\n"
                                         "#EXT-X-VERSION:3\n"
...
const std::string kSecondFetchLivePlaylist = "#EXTM3U\n"
                                             "#EXT-X-VERSION:3\n"
                                             "#EXT-X-TARGETDURATION:2\n"
...
const std::string kAESContent = "#EXTM3U\n"
                                "#EXT-X-VERSION:3\n"
                                "#EXT-X-TARGETDURATION:2\n"
...
const std::string kAESContentReplacement = "#EXTM3U\n"
                                           "#EXT-X-VERSION:3\n"
                                           "#EXT-X-TARGETDURATION:2\n"
                                           "#EXT-X-MEDIA-SEQUENCE:0\n"
                                           "#EXT-X-MEDIA-PLAYLIST-TYPE:VOD\n"
...
const std::string kDiscontinuous = "#EXTM3U\n"
                                   "#EXT-X-VERSION:3\n"
                                   "#EXT-X-TARGETDURATION:2\n"
                                   "#EXT-X-MEDIA-SEQUENCE:0\n"
...
protected:
    ...
    std::unique_ptr<HlsRenditionImpl> MakeVodRendition(std::string_view content)
    {
        ...
        return std::make_unique<HlsRenditionImpl>(mock_mdeh_.get(), mock_hrh_.get(), "test", std::move(playlist), duration, uri, media_log_.get());
    }

    std::unique_ptr<HlsRenditionImpl> MakeLiveRendition(GURL uri, std::string_view content)
    {
        constexpr hls::types::DecimalInteger version = 3;
        auto parsed = hls::MediaPlaylist::Parse(content, uri, version, nullptr);
        if (!parsed.has_value()) {
        ...
        media_log_ = std::make_unique<NiceMock<media::MockMediaLog>>();
        return std::make_unique<HlsRenditionImpl>(mock_mdeh_.get(), mock_hrh_.get(), "test", std::move(parsed).value(), std::nullopt, uri, media_log_.get());
    }
    ...
    ManifestDemuxer::DelayCallback BindCheckState(base::TimeDelta time)
    {
        ...
        return base::BindOnce(&HlsRenditionImplUnittest::CheckStateComplete, base::Unretained(this));
    }
    ...
    {
        EXPECT_CALL(*mock_mdeh_, AppendAndParseData(_, _, _, base::as_byte_span(data))).WillOnce(Return(return_value));
    }
    ...
    void RespondWithRange(base::TimeDelta start, base::TimeDelta end)
    {
        ...
        EXPECT_CALL(*mock_mdeh_, GetBufferedRanges("test")).WillRepeatedly(Return(ranges));
    }

    void RespondWithRangeTwice(base::TimeDelta A, base::TimeDelta B, base::TimeDelta X, base::TimeDelta Y)
    {
        ...
        EXPECT_CALL(*mock_mdeh_, GetBufferedRanges("test")).WillOnce(Return(ab)).WillOnce(Return(xy));
    }

    void SupplyAndExpectJunkData(base::TimeDelta initial_response_start, base::TimeDelta initial_response_end, base::TimeDelta fetch_expected_time)
    {
        std::string junk_content = "abcdefg, I dont like to sing rhyming songs";
        EXPECT_CALL(*mock_hrh_, ReadMediaSegment(_, _, _, _))
            .WillOnce([content = junk_content, host = mock_hrh_.get()](const hls::MediaSegment&, bool, bool, HlsDataSourceProvider::ReadCb cb) {
                auto stream = StringHlsDataSourceStreamFactory::CreateStream(content);
                std::move(cb).Run(std::move(stream));
            });
        EXPECT_CALL(*mock_mdeh_, AppendAndParseData("test", _, _, ElementsAreArray(base::as_byte_span(junk_content)))).WillOnce(Return(true));
        ...
        appended_range.Add(fetch_expected_time - base::Seconds(1), fetch_expected_time + base::Seconds(1));
        EXPECT_CALL(*mock_mdeh_, GetBufferedRanges("test")).Times(2).WillOnce(Return(initial_range)).WillOnce(Return(appended_range));
    }
    ...
    void RespondToUrl(std::string uri, std::string content)
    {
        ...
        EXPECT_CALL(*mock_hrh_, ReadMediaSegment(MediaSegmentHasUrl(uri), _, _, _))
            .WillOnce([content = std::move(content), host = mock_hrh_.get()](const hls::MediaSegment& segment, bool, bool, HlsDataSourceProvider::ReadCb cb) {
    ...
    void InterceptEncDataOnFetch(std::string uri, std::string content, base::OnceCallback<void(hls::MediaSegment::EncryptionData*)> intercept)
    {
        EXPECT_CALL(*mock_hrh_, UpdateNetworkSpeed(_));
        EXPECT_CALL(*mock_hrh_, ReadMediaSegment(MediaSegmentHasUrl(uri), _, _, _))
            .WillOnce([content = std::move(content), host = mock_hrh_.get(), intercept = std::move(intercept)](
                          const hls::MediaSegment& segment, bool, bool, HlsDataSourceProvider::ReadCb cb) mutable {
                if (auto enc_data = segment.GetEncryptionData()) {
...
TEST_F(HlsRenditionImplUnittest, TestCheckStateFromNoData)
{
    ...
    rendition->CheckState(base::Seconds(0), 1.0, BindCheckState(base::Seconds(0)));
...
TEST_F(HlsRenditionImplUnittest, TestCheckStateWithLargeBufferCached)
{
    ...
    rendition->CheckState(base::Seconds(0), 1.0, BindCheckState(base::Seconds(7)));
...
{
    auto rendition = MakeLiveRendition(GURL("http://example.com"), kInitialFetchPlaylist);
    ASSERT_NE(rendition, nullptr);
...
TEST_F(HlsRenditionImplUnittest, TestCreateRenditionPaused)
{
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(0), base::Seconds(0), base::Seconds(5));
    ...
    // Then appended.
    EXPECT_CALL(*mock_mdeh_, AppendAndParseData(_, _, _, base::as_byte_span(tscontent))).WillOnce(Return(true));
    // CheckState should in this case respond with a delay of zero seconds.
    rendition->CheckState(base::Seconds(0), 0.0, BindCheckState(base::Seconds(0)));
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsRenditionImplUnittest, TestPausedRenditionHasSomeData)
{
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(8), base::Seconds(0), base::Seconds(16));
...
TEST_F(HlsRenditionImplUnittest, TestPausedRenditionHasEnoughBufferedData)
{
    ...
    EXPECT_CALL(*mock_mdeh_, GetBufferedRanges(_)).Times(2).WillRepeatedly(Return(loaded_ranges));
    ...
    rendition->CheckState(base::Seconds(0), 0.0, BindCheckState(base::Seconds(7)));
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsRenditionImplUnittest, TestRenditionHasEnoughDataFetchNewManifest)
{
    ...
    EXPECT_CALL(*mock_hrh_, UpdateRenditionManifestUri("test", GURL("http://example.com"), _))
        .WillOnce([](std::string role, GURL uri, base::OnceCallback<void(bool)> cb) { std::move(cb).Run(true); });
...
TEST_F(HlsRenditionImplUnittest, TestRenditionHasEnoughDataDeleteOldContent)
{
    ...
    rendition->CheckState(base::Seconds(15), 0.0, BindCheckState(base::Seconds(12)));
...
TEST_F(HlsRenditionImplUnittest, TestPauseAndUnpause)
{
    ...
    ON_CALL(*mock_mdeh_, OnError(_)).WillByDefault([](PipelineStatus st) { LOG(ERROR) << MediaSerializeForTesting(st); });
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(0), base::Seconds(0), base::Seconds(2));
    rendition->CheckState(base::Seconds(0), 0.0, BindCheckState(base::Seconds(0)));
    task_environment_.RunUntilIdle();
    ...
    rendition->CheckState(base::Seconds(9), 1.0, BindCheckState(base::Seconds(18)));
    task_environment_.RunUntilIdle();
    ...
    EXPECT_CALL(*mock_hrh_, UpdateRenditionManifestUri("test", _, _)).WillOnce(base::test::RunOnceCallback<2>(true));
    task_environment_.FastForwardBy(base::Seconds(190));
    rendition->CheckState(base::Seconds(10), 1.0, BindCheckState(base::Seconds(0)));
    task_environment_.RunUntilIdle();
    ...
    auto parsed = hls::MediaPlaylist::Parse(kSecondFetchLivePlaylist, GURL("http://example.com"), 3, nullptr);
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(32), base::Seconds(0), base::Seconds(202));
    ...
    rendition->CheckState(base::Seconds(200), 1.0, BindCheckState(base::Seconds(0)));
    task_environment_.RunUntilIdle();
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(202), base::Seconds(0), base::Seconds(222));
    newcontent = "blah";
    ...
    rendition->CheckState(base::Seconds(200), 1.0, BindCheckState(base::Seconds(17)));
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsRenditionImplUnittest, TestAES128Content)
{
    ...
    InterceptEncDataOnFetch("https://example.com/media_0.ts", ciphertext,
        base::BindOnce([](crypto::SymmetricKey* key, hls::MediaSegment::EncryptionData* enc_data) { enc_data->ImportKey(key->key()); }, key.get()));
    ...
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(2), base::Seconds(0), base::Seconds(4));
    ...
    // which will trigger an attempt to fetch.
    RespondWithRangeTwice(base::Seconds(0), base::Seconds(4), base::Seconds(0), base::Seconds(6));
    ...
    auto parsed = hls::MediaPlaylist::Parse(kAESContentReplacement, GURL("https://example.com/manifest.m3u8"), 3, nullptr);
    CHECK(parsed.has_value());
    rendition->UpdatePlaylist(std::move(parsed).value(), std::nullopt);
