...
struct VideoDecoderStreamTestParams {
    VideoDecoderStreamTestParams(bool is_encrypted, bool has_decryptor, bool has_prepare, int decoding_delay, int parallel_decoding)
...
class VideoDecoderStreamTest : public testing::Test, public testing::WithParamInterface<VideoDecoderStreamTestParams> {
public:
        ...
        , has_no_key_(false)
    {
        video_decoder_stream_
            = std::make_unique<VideoDecoderStream>(std::make_unique<VideoDecoderStream::StreamTraits>(&media_log_), task_environment_.GetMainThreadTaskRunner(),
                base::BindRepeating(&VideoDecoderStreamTest::CreateVideoDecodersForTest, base::Unretained(this)), &media_log_);
        video_decoder_stream_->set_decoder_change_observer(base::BindRepeating(&VideoDecoderStreamTest::OnDecoderChanged, base::Unretained(this)));
        if (GetParam().has_prepare) {
            video_decoder_stream_->SetPrepareCB(base::BindRepeating(&VideoDecoderStreamTest::PrepareFrame, base::Unretained(this)));
        }
        ...
        if (GetParam().is_encrypted && GetParam().has_decryptor) {
            ...
            EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillRepeatedly(base::test::RunOnceCallbackRepeatedly<1>(false));
            EXPECT_CALL(*decryptor_, Decrypt(_, _, _)).WillRepeatedly(Invoke(this, &VideoDecoderStreamTest::Decrypt));
        }
        ...
        if (GetParam().is_encrypted) {
            ...
            EXPECT_CALL(*cdm_context_, GetDecryptor()).WillRepeatedly(Return(decryptor_.get()));
        }
        ...
        EXPECT_MEDIA_LOG(HasSubstr("clear to encrypted buffers")).Times(AnyNumber());
    }
    ...
    void CreateDemuxerStream(gfx::Size start_size, gfx::Vector2dF size_delta)
    {
        ...
        demuxer_stream_ = std::make_unique<FakeDemuxerStream>(kNumConfigs, kNumBuffersInOneConfig, GetParam().is_encrypted, start_size, size_delta);
    }

    void PrepareFrame(scoped_refptr<VideoFrame> frame, VideoDecoderStream::OutputReadyCB output_ready_cb)
    {
        // Simulate some delay in return of the output.
        task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(std::move(output_ready_cb), std::move(frame)));
    }

    void PrepareFrameWithDelay(scoped_refptr<VideoFrame> frame, VideoDecoderStream::OutputReadyCB output_ready_cb)
    {
    ...
    std::vector<std::unique_ptr<VideoDecoder>> CreateVideoDecodersForTest()
    {
        ...
        decoders.push_back(std::make_unique<DecryptingVideoDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_));
#endif

        for (int i = 0; i < 3; ++i) {
            auto decoder = std::make_unique<FakeVideoDecoder>(GetDecoderId(i), GetParam().decoding_delay, GetParam().parallel_decoding,
                base::BindRepeating(&VideoDecoderStreamTest::OnBytesDecoded, base::Unretained(this)));
    ...
    void OnDecoderChanged(VideoDecoder* decoder)
    {
        ...
        ASSERT_TRUE(decoder_->GetDecoderId() == GetDecoderId(0) || decoder_->GetDecoderId() == GetDecoderId(1) || decoder_->GetDecoderId() == GetDecoderId(2));
    }
    ...
    {
        if (!demuxer_stream_) {
            demuxer_stream_ = std::make_unique<FakeDemuxerStream>(kNumConfigs, kNumBuffersInOneConfig, GetParam().is_encrypted);
        }

        pending_initialize_ = true;
        video_decoder_stream_->Initialize(demuxer_stream_.get(), base::BindOnce(&VideoDecoderStreamTest::OnInitialized, base::Unretained(this)),
            cdm_context_.get(), base::BindRepeating(&VideoDecoderStreamTest::OnStatistics, base::Unretained(this)),
            base::BindRepeating(&VideoDecoderStreamTest::OnWaiting, base::Unretained(this)));
    ...
    void Decrypt(Decryptor::StreamType stream_type, scoped_refptr<DecoderBuffer> encrypted, Decryptor::DecryptCB decrypt_cb)
    {
        ...
        scoped_refptr<DecoderBuffer> decrypted = DecoderBuffer::CopyFrom(*encrypted);
    ...
    void FrameReady(VideoDecoderStream::ReadResult result)
    {
        ...
        scoped_refptr<VideoFrame> frame = last_read_status_code_ == DecoderStatus::Codes::kOk ? std::move(result).value() : nullptr;
        frame_read_ = frame;
    ...
    void ReadOneFrame()
    {
        frame_read_ = nullptr;
        pending_read_ = true;
        video_decoder_stream_->Read(base::BindOnce(&VideoDecoderStreamTest::FrameReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    enum PendingState { NOT_PENDING, DEMUXER_READ_NORMAL, DEMUXER_READ_CONFIG_CHANGE, DECRYPTOR_NO_KEY, DECODER_REINIT, DECODER_DECODE, DECODER_RESET };
    ...
    void EnterPendingState(PendingState state)
    {
        ...
        case DECODER_RESET:
            ...
            pending_reset_ = true;
            video_decoder_stream_->Reset(base::BindOnce(&VideoDecoderStreamTest::OnReset, base::Unretained(this)));
            base::RunLoop().RunUntilIdle();
            break;
    ...
    void SatisfyPendingCallback(PendingState state)
    {
        ...
        switch (state) {
        case DEMUXER_READ_CONFIG_CHANGE:
            EXPECT_MEDIA_LOG(HasSubstr("decoder config changed")).Times(testing::AtLeast(1));
            [[fallthrough]];
        case DEMUXER_READ_NORMAL:
    ...
    base::test::ScopedFeatureList enabled_features_ { kResolutionBasedDecoderPriority };
...
INSTANTIATE_TEST_SUITE_P(Clear, VideoDecoderStreamTest,
    ::testing::Values(VideoDecoderStreamTestParams(false, false, false, 0, 1), VideoDecoderStreamTestParams(false, false, false, 3, 1),
        VideoDecoderStreamTestParams(false, false, false, 7, 1), VideoDecoderStreamTestParams(false, false, true, 0, 1),
        VideoDecoderStreamTestParams(false, false, true, 3, 1)));

INSTANTIATE_TEST_SUITE_P(EncryptedWithDecryptor, VideoDecoderStreamTest,
    ::testing::Values(VideoDecoderStreamTestParams(true, true, false, 7, 1), VideoDecoderStreamTestParams(true, true, true, 7, 1)));

INSTANTIATE_TEST_SUITE_P(EncryptedWithoutDecryptor, VideoDecoderStreamTest,
    ::testing::Values(VideoDecoderStreamTestParams(true, false, false, 7, 1), VideoDecoderStreamTestParams(true, false, true, 7, 1)));

INSTANTIATE_TEST_SUITE_P(Clear_Parallel, VideoDecoderStreamTest,
    ::testing::Values(VideoDecoderStreamTestParams(false, false, false, 0, 3), VideoDecoderStreamTestParams(false, false, false, 2, 3),
        VideoDecoderStreamTestParams(false, false, true, 0, 3), VideoDecoderStreamTestParams(false, false, true, 2, 3)));
...
{
    if (base::FeatureList::IsEnabled(kVideoDecodeBatching) && GetParam().parallel_decoding != 1) {
    ...
    auto const size_delta = TestVideoConfig::LargeCodedSize() - TestVideoConfig::NormalCodedSize();
    auto const width_delta = size_delta.width() / (kNumConfigs - 1);
    auto const height_delta = size_delta.height() / (kNumConfigs - 1);
    CreateDemuxerStream(TestVideoConfig::NormalCodedSize(), gfx::Vector2dF(width_delta, height_delta));
    ...
    auto expected_size = expected_config.coded_size() + gfx::ScaleToCeiledSize(gfx::Size(width_delta, height_delta), kNumConfigs - 1);
    ...
    if (!decoder_->eos_next_configs().back().is_encrypted() && expected_config.is_encrypted()) {
...
TEST_P(VideoDecoderStreamTest, ConfigChangeHwToSw)
{
    ...
    auto const width_delta = size_delta.width() / (kNumConfigs - 1);
    auto const height_delta = size_delta.height() / (kNumConfigs - 1);
    CreateDemuxerStream(TestVideoConfig::LargeCodedSize(), gfx::Vector2dF(-width_delta, -height_delta));
    Initialize();
...
TEST_P(VideoDecoderStreamTest, Read_ProperMetadata)
{
    ...
    if (GetParam().has_prepare) {
        ...
        video_decoder_stream_->SetPrepareCB(base::BindRepeating(&VideoDecoderStreamTest::PrepareFrameWithDelay, base::Unretained(this)));
    }
    ...
    EXPECT_EQ(*metadata.decode_end_time - *metadata.decode_begin_time, kDecodeDelay);

    // Verify the processing metadata is accurate.
    const base::TimeDelta expected_processing_time = GetParam().has_prepare ? (kDecodeDelay + kPrepareDelay) : kDecodeDelay;
...
TEST_P(VideoDecoderStreamTest, Read_BlockedDemuxer)
{
    ...
    EXPECT_EQ(std::min(GetParam().decoding_delay + 1, kNumBuffersInOneConfig + 1), demuxed_buffers);
...
TEST_P(VideoDecoderStreamTest, Read_DemuxerStreamReadError)
{
    ...
    if (GetParam().is_encrypted && GetParam().has_decryptor) {
        EXPECT_MEDIA_LOG(HasSubstr("DecryptingDemuxerStream: demuxer stream read error"));
    }
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_EndOfStreamReachedBeforeFallback)
{
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_DoesReinitializeStompPendingRead)
{
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_SelectedOnMidstreamDecodeErrorAfterReinitialization)
{
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_DecodeErrorRepeated_AfterReinitialization)
{
    Initialize();
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_ConfigChangeClearsPendingBuffers)
{
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_ErrorDuringConfigChangeFlushing)
{
    // Test case is only interesting if the decoder can receive a config change
    // before returning its first frame.
    if (GetParam().decoding_delay < kNumBuffersInOneConfig)
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_PendingBuffersIsFilledAndCleared)
{
    ...
    while (demuxer_reads_satisfied < GetParam().decoding_delay - 1) {
        ...
        base::RunLoop().RunUntilIdle();
        EXPECT_TRUE(pending_read_);
        EXPECT_EQ(demuxer_reads_satisfied, video_decoder_stream_->get_pending_buffers_size_for_testing());
    ...
    EXPECT_EQ(demuxer_reads_satisfied - 1, video_decoder_stream_->get_fallback_buffers_size_for_testing());
    EXPECT_EQ(demuxer_reads_satisfied, video_decoder_stream_->get_pending_buffers_size_for_testing());
...
TEST_P(VideoDecoderStreamTest, FallbackDecoder_NotSelectedOnMidstreamDecodeError)
{
    Initialize();
    ReadOneFrame();
