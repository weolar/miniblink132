...
H26xAnnexBBitstreamBuilder::H26xAnnexBBitstreamBuilder(bool insert_emulation_prevention_bytes)
...
void H26xAnnexBBitstreamBuilder::FlushReg()
{
    ...
    if (insert_emulation_prevention_bytes_ && in_nalu_) {
        ...
        for (size_t i = 0; i < bytes_in_reg; ++i) {
            ...
            if (pos_ >= 2u && data_[pos_ - 2u] == 0 && data_[pos_ - 1u] == 0u && reg_be[i] <= kEmulationByte) {
        ...
        data_.subspan(pos_).copy_prefix_from(base::span(reg_be).first(bytes_in_reg));
...
void H26xAnnexBBitstreamBuilder::BeginNALU(H264NALU::Type nalu_type, int nal_ref_idc)
{
    DCHECK(!in_nalu_);
    DCHECK_FINISHED();
