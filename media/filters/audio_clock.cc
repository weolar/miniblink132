...
AudioClock::AudioClock(base::TimeDelta start_timestamp, int sample_rate)
    ...
    , microseconds_per_frame_(static_cast<double>(base::Time::kMicrosecondsPerSecond) / sample_rate)
...
void AudioClock::WroteAudio(int frames_written, int frames_requested, int delay_frames, double playback_rate)
{
    ...
    if (start_timestamp_.InMicrosecondsF() == front_timestamp_micros_ && buffered_.empty()) {
    ...
    front_timestamp_micros_ = std::max(front_timestamp_micros_, back_timestamp_micros_ - ComputeBufferedMediaDurationMicros());
...
void AudioClock::CompensateForSuspendedWrites(base::TimeDelta elapsed, int delay_frames)
{
    const int64_t frames_elapsed = base::ClampRound<int64_t>(elapsed.InMicrosecondsF() / microseconds_per_frame_);
...
base::TimeDelta AudioClock::TimeUntilPlayback(base::TimeDelta timestamp) const
{
    ...
    return base::Microseconds(std::round(frames_until_timestamp * microseconds_per_frame_));
}

void AudioClock::ContiguousAudioDataBufferedForTesting(base::TimeDelta* total, base::TimeDelta* same_rate_total) const
{
    ...
    *same_rate_total = base::Microseconds(scaled_frames_at_same_rate * microseconds_per_frame_);
}
