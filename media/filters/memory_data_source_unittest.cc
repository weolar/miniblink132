...
protected:
    void Initialize(size_t size)
    {
        ...
        memory_data_source_ = std::make_unique<MemoryDataSource>(data_.data(), data_.size());
    ...
    void ReadAndExpect(int64_t position, int size, int expected_read_size)
    {
        ...
        memory_data_source_->Read(position, size, data.data(), base::BindOnce(&MemoryDataSourceTest::ReadCB, base::Unretained(this)));

        if (expected_read_size != DataSource::kReadError) {
            size_t positive_expected_size = base::checked_cast<size_t>(expected_read_size);
            EXPECT_TRUE(base::ranges::equal(base::span(data_).subspan(position, positive_expected_size), base::span(data).first(positive_expected_size)));
        }
    }
