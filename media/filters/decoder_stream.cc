...
#define FUNCTION_DVLOG(level) DVLOG(level) << __func__ << "<" << GetStreamTypeString() << ">"

template <DemuxerStream::Type StreamType> static const char* GetDecodeTraceString();
template <DemuxerStream::Type StreamType> static const char* GetReadTraceString();
template <DemuxerStream::Type StreamType> static const char* GetDemuxerReadTraceString();
template <DemuxerStream::Type StreamType> static const char* GetPrepareTraceString();

template <> const char* GetDecodeTraceString<DemuxerStream::VIDEO>()
{
    return "VideoDecoderStream::Decode";
}

template <> const char* GetDecodeTraceString<DemuxerStream::AUDIO>()
{
    return "AudioDecoderStream::Decode";
}

template <> const char* GetReadTraceString<DemuxerStream::VIDEO>()
{
    return "VideoDecoderStream::Read";
}

template <> const char* GetReadTraceString<DemuxerStream::AUDIO>()
{
    return "AudioDecoderStream::Read";
}

template <> const char* GetDemuxerReadTraceString<DemuxerStream::VIDEO>()
{
    return "VideoDecoderStream::ReadFromDemuxerStream";
}

template <> const char* GetDemuxerReadTraceString<DemuxerStream::AUDIO>()
{
    return "AudioDecoderStream::ReadFromDemuxerStream";
}

template <> const char* GetPrepareTraceString<DemuxerStream::VIDEO>()
{
    return "VideoDecoderStream::PrepareOutput";
}

template <> const char* GetPrepareTraceString<DemuxerStream::AUDIO>()
{
...
DecoderStream<StreamType>::DecoderStream(std::unique_ptr<DecoderStreamTraits<StreamType>> traits, scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    , decoder_selector_(task_runner_, std::move(create_decoders_cb), media_log, base::FeatureList::IsEnabled(kResolutionBasedDecoderPriority))
    , decoding_eos_(false)
...
template <DemuxerStream::Type StreamType> DecoderStream<StreamType>::~DecoderStream()
{
    FUNCTION_DVLOG(1);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());

    if (init_cb_) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb_), false));
    }
    if (read_cb_) {
...
template <DemuxerStream::Type StreamType> std::string DecoderStream<StreamType>::GetStreamTypeString()
{
    return DecoderStreamTraits<StreamType>::ToString();
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::Initialize(DemuxerStream* stream, InitCB init_cb, CdmContext* cdm_context, StatisticsCB statistics_cb, WaitingCB waiting_cb)
{
    FUNCTION_DVLOG(1);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    decoder_selector_.Initialize(traits_.get(), stream, cdm_context, std::move(waiting_cb));

    state_ = State::kStateInitializing;
    BeginDecoderSelection();
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::Read(ReadCB read_cb)
{
    FUNCTION_DVLOG(3);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ != State::kStateUninitialized && state_ != State::kStateInitializing) << base::to_underlying(state_);
    ...
    if (state_ == State::kStateEndOfStream && ready_outputs_.empty() && unprepared_outputs_.empty()) {
        read_cb_ = base::BindPostTaskToCurrentDefault(std::move(read_cb));
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::Reset(base::OnceClosure closure)
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    if (decrypting_demuxer_stream_) {
        decrypting_demuxer_stream_->Reset(base::BindOnce(&DecoderStream<StreamType>::ResetDecoder, weak_factory_.GetWeakPtr()));
        return;
    }
...
template <DemuxerStream::Type StreamType> bool DecoderStream<StreamType>::CanReadWithoutStalling() const
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    return !ready_outputs_.empty() || !unprepared_outputs_.empty() || (decoder_ && decoder_->CanReadWithoutStalling());
}

template <> bool DecoderStream<DemuxerStream::AUDIO>::CanReadWithoutStalling() const
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    return true;
}

template <DemuxerStream::Type StreamType> int DecoderStream<StreamType>::GetMaxDecodeRequests() const
{
    // The decoder is owned by |decoder_selector_| during reinitialization, so
    // during that time we disallow decode requests.
    return state_ != State::kStateReinitializingDecoder ? decoder_->GetMaxDecodeRequests() : 0;
}
...
template <DemuxerStream::Type StreamType> bool DecoderStream<StreamType>::IsPlatformDecoder() const
{
    // The decoder is owned by |decoder_selector_| during reinitialization, so
    // during that time we return false to indicate decoder type unknown.
    return state_ != State::kStateReinitializingDecoder ? decoder_->IsPlatformDecoder() : false;
}

template <> int DecoderStream<DemuxerStream::AUDIO>::GetMaxDecodeRequests() const
{
    return 1;
}

template <DemuxerStream::Type StreamType> int DecoderStream<StreamType>::GetMaxReadyOutputs() const
{
    // The decoder is owned by |decoder_selector_| during reinitialization, so
    // during that time we assume the minimum viable number of max ready outputs.
    return state_ != State::kStateReinitializingDecoder ? decoder_->GetMaxDecodeRequests() : 1;
}

template <> int DecoderStream<DemuxerStream::AUDIO>::GetMaxReadyOutputs() const
{
    return 1;
}

template <DemuxerStream::Type StreamType> bool DecoderStream<StreamType>::CanDecodeMore() const
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    int num_decodes = ready_outputs_.size() + unprepared_outputs_.size() + pending_decode_requests_;
    return buffers_left && num_decodes < GetMaxDecodeRequests();
}

template <DemuxerStream::Type StreamType> base::TimeDelta DecoderStream<StreamType>::AverageDuration() const
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    return duration_tracker_.Mean();
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::SetPrepareCB(PrepareCB prepare_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    prepare_cb_ = std::move(prepare_cb);
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::SkipPrepareUntil(base::TimeDelta start_timestamp)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    skip_prepare_until_timestamp_ = start_timestamp;
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::BeginDecoderSelection()
{
    decoder_selector_.BeginDecoderSelection(base::BindOnce(&DecoderStream<StreamType>::OnDecoderSelected, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&DecoderStream<StreamType>::OnDecodeOutputReady, fallback_weak_factory_.GetWeakPtr()));
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ResumeDecoderSelection(DecoderStatus&& reinit_cause)
{
    decoder_selector_.ResumeDecoderSelection(base::BindOnce(&DecoderStream<StreamType>::OnDecoderSelected, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&DecoderStream<StreamType>::OnDecodeOutputReady, fallback_weak_factory_.GetWeakPtr()), std::move(reinit_cause));
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::OnDecoderSelected(
    DecoderStatus::Or<std::unique_ptr<Decoder>> decoder_or_error, std::unique_ptr<DecryptingDemuxerStream> decrypting_demuxer_stream)
{
    FUNCTION_DVLOG(1) << ": " << (decoder_or_error.has_value() ? GetDecoderName(decoder_or_error->GetDecoderType()) : "No decoder selected.");
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStateInitializing || state_ == State::kStateReinitializingDecoder) << base::to_underlying(state_);
    ...
    if (decoder_change_observer_cb_) {
        decoder_change_observer_cb_.Run(decoder_or_error.has_value() ? (*decoder_or_error).get() : nullptr);
    }
    ...
    if (!decoder_or_error.has_value()) {
        if (state_ == State::kStateInitializing) {
            state_ = State::kStateUninitialized;
            MEDIA_LOG(ERROR, media_log_) << GetStreamTypeString() << " decoder initialization failed with " << std::move(decoder_or_error).error();
    ...
    media_log_->SetProperty<StreamTraits::kIsDecryptingDemuxerStream>(!!decrypting_demuxer_stream_);
    media_log_->SetProperty<StreamTraits::kDecoderName>(decoder_->GetDecoderType());
    media_log_->SetProperty<StreamTraits::kIsPlatformDecoder>(decoder_->IsPlatformDecoder());

    if (is_decrypting_demuxer_stream_selected) {
        MEDIA_LOG(INFO, media_log_) << "Selected DecryptingDemuxerStream for " << GetStreamTypeString()
                                    << " decryption, config: " << traits_->GetDecoderConfig(original_stream).AsHumanReadableString();
    }

    MEDIA_LOG(INFO, media_log_) << "Selected " << decoder_->GetDecoderType() << " for " << GetStreamTypeString()
                                << " decoding, config: " << traits_->GetDecoderConfig(stream_).AsHumanReadableString();
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::SatisfyRead(ReadResult result)
{
    DCHECK(read_cb_);
    TRACE_EVENT_ASYNC_END1("media", GetReadTraceString<StreamType>(), this, "status", GetStatusString(result.code()));
    std::move(read_cb_).Run(std::move(result));
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::Decode(scoped_refptr<DecoderBuffer> buffer)
{
    FUNCTION_DVLOG(3);
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::DecodeInternal(scoped_refptr<DecoderBuffer> buffer)
{
    FUNCTION_DVLOG(3);
    DCHECK(state_ == State::kStateNormal || state_ == State::kStateFlushingDecoder) << base::to_underlying(state_);
    ...
    if (MediaTraceIsEnabled()) {
        ...
        trace_event = std::make_unique<ScopedDecodeTrace>(GetDecodeTraceString<StreamType>(), *buffer);
    }
    ...
    decoder_->Decode(std::move(buffer),
        base::BindOnce(&DecoderStream<StreamType>::OnDecodeDone, fallback_weak_factory_.GetWeakPtr(), buffer_size, decoding_eos_, std::move(trace_event)));
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::OnDecodeDone(int buffer_size, bool end_of_stream, std::unique_ptr<ScopedDecodeTrace> trace_event, DecoderStatus status)
{
    FUNCTION_DVLOG(status.is_ok() ? 3 : 1) << ": " << static_cast<int>(status.code());
    DCHECK(state_ == State::kStateNormal || state_ == State::kStateFlushingDecoder || state_ == State::kStateError) << base::to_underlying(state_);
    ...
    if (end_of_stream) {
        ...
        decoding_eos_ = false;
        if (status.is_ok() || status.code() == DecoderStatus::Codes::kElidedEndOfStreamForConfigChange) {
    ...
    default:
        if (!decoder_produced_a_frame_ && base::FeatureList::IsEnabled(kFallbackAfterDecodeError)) {
            MEDIA_LOG(WARNING, media_log_) << GetStreamTypeString() << " decoder fallback after initial decode error.";
            ...
            if (fallback_cb_) {
                ...
                PipelineStatus fallback_status = { PipelineStatus::Codes::PIPELINE_ERROR_DECODE, std::move(copy) };
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::OnDecodeOutputReady(scoped_refptr<Output> output)
{
    ...
    if (prepare_cb_ && output->timestamp() + AverageDuration() >= skip_prepare_until_timestamp_) {
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ReadFromDemuxerStream()
{
    FUNCTION_DVLOG(3);
    ...
    TRACE_EVENT_ASYNC_BEGIN0("media", GetDemuxerReadTraceString<StreamType>(), this);
    pending_demuxer_read_ = true;
    uint32_t buffer_read_count = 1;
    // Do not batch with software video decoder.
    if (IsPlatformDecoder() && base::FeatureList::IsEnabled(kVideoDecodeBatching)) {
        buffer_read_count = GetMaxDecodeRequests() - pending_decode_requests_;
    }
    {
        TRACE_EVENT2("media", "DecodeStreamRead", "StreamType:", GetStreamTypeString(), "buffer_read_count:", buffer_read_count);
        stream_->Read(buffer_read_count, base::BindOnce(&DecoderStream<StreamType>::OnBuffersReady, weak_factory_.GetWeakPtr()));
    }
}

template <DemuxerStream::Type StreamType>
void DecoderStream<StreamType>::OnBuffersReady(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    ...
    TRACE_EVENT_ASYNC_END1("media", GetDemuxerReadTraceString<StreamType>(), this, "status", DemuxerStream::GetStatusName(status));

    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(pending_demuxer_read_);
    if (!decoder_produced_a_frame_) {
        DCHECK(state_ == State::kStateError || state_ == State::kStateReinitializingDecoder || state_ == State::kStateNormal) << base::to_underlying(state_);
    }
    pending_demuxer_read_ = false;
    ...
    if (state_ == State::kStateReinitializingDecoder && !decoder_produced_a_frame_) {
        switch (status) {
    ...
    if (status == DemuxerStream::kError) {
        ...
        state_ = State::kStateError;
        MEDIA_LOG(ERROR, media_log_) << GetStreamTypeString() << " demuxer stream read error!";
        pending_buffers_.clear();
        ClearOutputs();
    ...
    if (status == DemuxerStream::kConfigChanged) {
        ...
        MEDIA_LOG(INFO, media_log_) << GetStreamTypeString() << " decoder config changed midstream, new config: " << config.AsHumanReadableString();
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ReinitializeDecoder()
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK_EQ(state_, State::kStateFlushingDecoder);
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::CompleteDecoderReinitialization(DecoderStatus status)
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    if (!status.is_ok() && (reset_cb_ || !read_cb_)) {
        ...
        MEDIA_LOG(ERROR, media_log_) << GetStreamTypeString() << " decoder reinitialization failed";
    }
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ResetDecoder()
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStateNormal || state_ == State::kStateFlushingDecoder || state_ == State::kStateError || state_ == State::kStateEndOfStream)
        << base::to_underlying(state_);
    DCHECK(reset_cb_);

    decoder_->Reset(base::BindOnce(&DecoderStream<StreamType>::OnDecoderReset, weak_factory_.GetWeakPtr()));
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::OnDecoderReset()
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStateNormal || state_ == State::kStateFlushingDecoder || state_ == State::kStateError || state_ == State::kStateEndOfStream)
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ClearOutputs()
{
    if (preparing_output_)
        CompletePrepare(nullptr);
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::MaybePrepareAnotherOutput()
{
    FUNCTION_DVLOG(3);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(state_ == State::kStateNormal || state_ == State::kStateFlushingDecoder || state_ == State::kStateEndOfStream
        || state_ == State::kStateReinitializingDecoder)
        << base::to_underlying(state_);
    ...
    TRACE_EVENT_ASYNC_BEGIN1("media", GetPrepareTraceString<StreamType>(), this, "timestamp_us", output->timestamp().InMicroseconds());
    preparing_output_ = true;
    prepare_cb_.Run(output, base::BindOnce(&DecoderStream<StreamType>::OnPreparedOutputReady, prepare_weak_factory_.GetWeakPtr()));
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::OnPreparedOutputReady(scoped_refptr<Output> output)
{
    FUNCTION_DVLOG(2);
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::CompletePrepare(const Output* output)
{
    DCHECK(preparing_output_);
    TRACE_EVENT_ASYNC_END1("media", GetPrepareTraceString<StreamType>(), this, "timestamp_us", (output ? output->timestamp() : kNoTimestamp).InMicroseconds());
    preparing_output_ = false;
}

template <DemuxerStream::Type StreamType> void DecoderStream<StreamType>::ReportEncryptionType(const scoped_refptr<DecoderBuffer>& buffer)
{
    auto encryption_type = EncryptionType::kClear;
    if (decrypting_demuxer_stream_) {
        encryption_type = decrypting_demuxer_stream_->HasClearLead() ? EncryptionType::kEncryptedWithClearLead : EncryptionType::kEncrypted;
    } else if (traits_->GetDecoderConfig(stream_).is_encrypted()) {
        // Treat EOS as clear buffer which should be rare.
        bool is_buffer_encrypted = !buffer->end_of_stream() && buffer->decrypt_config();
        encryption_type = !is_buffer_encrypted ? EncryptionType::kEncryptedWithClearLead : EncryptionType::kEncrypted;
    }

    if (encryption_type == EncryptionType::kEncryptedWithClearLead) {
        MEDIA_LOG(INFO, media_log_) << GetStreamTypeString() << "stream is encrypted with clear lead";
    }
