...
public:
    ...
    base::TimeTicks current() const
    {
        return base_time_ + interval(tick_count_);
    }
    base::TimeTicks step()
    {
        return step(1);
...
public:
    VideoRendererAlgorithmTest()
        ...
        , algorithm_(base::BindRepeating(&WallClockTimeSource::GetWallClockTimes, base::Unretained(&time_source_)), &media_log_)
    {
    ...
    VideoRendererAlgorithmTest& operator=(const VideoRendererAlgorithmTest&) = delete;
    ...
    scoped_refptr<VideoFrame> CreateFrame(base::TimeDelta timestamp)
    {
        ...
        return frame_pool_.CreateFrame(PIXEL_FORMAT_I420, natural_size, gfx::Rect(natural_size), natural_size, timestamp);
    }

    base::TimeDelta minimum_glitch_time() const
    {
        return base::Seconds(VideoRendererAlgorithm::kMinimumAcceptableTimeBetweenGlitchesSecs);
    }
    ...
    {
        algorithm_.cadence_estimator_.set_cadence_hysteresis_threshold_for_testing(base::TimeDelta());
    }
    ...
    {
        return algorithm_.frame_queue_.size();
    }
    ...
    base::TimeDelta CalculateAbsoluteDriftForFrame(base::TimeTicks deadline_min, int frame_index)
    {
    ...
    scoped_refptr<VideoFrame> RenderAndStep(TickGenerator* tg, size_t* frames_dropped)
    {
    ...
    void RunFramePumpTest(bool reset, TickGenerator* frame_tg, TickGenerator* display_tg, OnRenderCallback render_test_func)
    {
        SCOPED_TRACE(base::StringPrintf("Rendering %.03f fps into %0.03f", frame_tg->hertz(), display_tg->hertz()));
        ...
        while (tick_clock_->NowTicks() - start_time < minimum_glitch_time()) {
            while (EffectiveFramesQueued() < 3 || frame_tg->current() - time_source_.CurrentMediaTime() < base::TimeTicks()) {
                algorithm_.EnqueueFrame(CreateFrame(frame_tg->current() - base::TimeTicks()));
                frame_tg->step();
            }
            ...
            if (fresh_algorithm) {
                ASSERT_NEAR(frame_tg->interval(1).InMicroseconds(), algorithm_.average_frame_duration().InMicroseconds(), 1);
            ...
            if (!is_using_cadence() || !frames_queued() || GetCurrentFrameDisplayCount() < GetCurrentFrameIdealDisplayCount()) {
                ASSERT_NEAR(GetUsableFrameCount(deadline_max), EffectiveFramesQueued(), fresh_algorithm ? 0 : 1);
    ...
    int FindBestFrameByCoverage(base::TimeTicks deadline_min, base::TimeTicks deadline_max, int* second_best)
    {
        return algorithm_.FindBestFrameByCoverage(deadline_min, deadline_max, second_best);
    }

    int FindBestFrameByDrift(base::TimeTicks deadline_min, base::TimeDelta* selected_frame_drift)
    {
    ...
    int AccountForMissedIntervals(base::TimeTicks deadline_min, base::TimeTicks deadline_max)
    {
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByCadence)
{
    ...
    for (const auto& test_rate : kTestRates) {
        ...
        RunFramePumpTest(true, &frame_tg, &display_tg,
            [&current_frame, &actual_frame_pattern, desired_frame_pattern, this](scoped_refptr<VideoFrame> frame, size_t frames_dropped) {
                ASSERT_TRUE(frame);
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByCadenceOverdisplayed)
{
    ...
    ASSERT_EQ(GetCurrentFrameIdealDisplayCount() + 1, GetCurrentFrameDisplayCount());
    algorithm_.EnqueueFrame(CreateFrame(frame_tg.interval(2)));
    algorithm_.EnqueueFrame(CreateFrame(frame_tg.interval(3)));
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByCoverage)
{
    ...
    EXPECT_EQ(0, FindBestFrameByCoverage(deadline_min, deadline_max, &second_best));
    ...
    deadline_max = deadline_min + tg.interval(1);
    EXPECT_EQ(1, FindBestFrameByCoverage(deadline_min, deadline_max, &second_best));
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByDriftAndDriftCalculations)
{
    ...
    size_t frames_dropped = 0;
    scoped_refptr<VideoFrame> frame = algorithm_.Render(tg.current(), tg.current() + tg.interval(1), &frames_dropped);
    ASSERT_TRUE(frame);
    EXPECT_EQ(tg.interval(0), frame->timestamp());
    EXPECT_EQ(0u, frames_dropped);
    ...
    deadline += half_drift;
    EXPECT_EQ(tg.interval(1) + half_drift, CalculateAbsoluteDriftForFrame(deadline, 0));
    EXPECT_EQ(half_drift, CalculateAbsoluteDriftForFrame(deadline, 1));
    EXPECT_EQ(1, FindBestFrameByDrift(deadline, &detected_drift));
...
TEST_F(VideoRendererAlgorithmTest, BestFrameByFractionalCadence)
{
    ...
    for (const auto& test_rate : kTestRates) {
        ...
        scoped_refptr<VideoFrame> current_frame;
        RunFramePumpTest(true, &frame_tg, &display_tg, [&current_frame, this](scoped_refptr<VideoFrame> frame, size_t frames_dropped) {
            ASSERT_TRUE(frame);
...
TEST_F(VideoRendererAlgorithmTest, RemoveExpiredFrames)
{
    ...
    // counted as dropped.
    ASSERT_EQ(0u, algorithm_.RemoveExpiredFrames(tg.current() + tg.interval(1) * 0.9));
    EXPECT_EQ(1u, frames_queued());
    EXPECT_EQ(1u, EffectiveFramesQueued());
...
TEST_F(VideoRendererAlgorithmTest, RemoveExpiredFramesFractionalCadence)
{
    ...
    // still counted as effective.  1 undisplayed and 1 displayed frame will be
    // expired.
    ASSERT_EQ(1u, algorithm_.RemoveExpiredFrames(display_tg.current() + display_tg.interval(1) + max_acceptable_drift() * 1.25));
    EXPECT_EQ(1u, frames_queued());
    EXPECT_EQ(1u, EffectiveFramesQueued());
...
class VideoRendererAlgorithmCadenceTest : public VideoRendererAlgorithmTest, public ::testing::WithParamInterface<::testing::tuple<double, double>> { };
...
TEST_P(VideoRendererAlgorithmCadenceTest, CadenceTest)
{
    ...
    TickGenerator frame_tg(base::TimeTicks(), frame_rate);
    TickGenerator display_tg(tick_clock_->NowTicks(), display_rate);
    RunFramePumpTest(true, &frame_tg, &display_tg, [](scoped_refptr<VideoFrame> frame, size_t frames_dropped) {});
}
...
INSTANTIATE_TEST_SUITE_P(All, VideoRendererAlgorithmCadenceTest, ::testing::Combine(::testing::ValuesIn(kDisplayRates), ::testing::ValuesIn(kTestRates)));
...
TEST_F(VideoRendererAlgorithmTest, VariablePlaybackRateCadence)
{
    ...
    const bool kTestRateHasCadence[std::size(kPlaybackRates)] = { true, true, true, true, true, false };
    ...
    for (size_t i = 0; i < std::size(kPlaybackRates); ++i) {
        ...
        RunFramePumpTest(false, &frame_tg, &display_tg, [](scoped_refptr<VideoFrame> frame, size_t frames_dropped) {});
        if (HasFatalFailure())
            return;
...
TEST_F(VideoRendererAlgorithmTest, UglyTimestampsHaveCadence)
{
    ...
    const int kBadTimestampsMs[] = { 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 17, 16,
        17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 16, 17, 17, 17 };
...
TEST_F(VideoRendererAlgorithmTest, VariableFrameRateNoCadence)
{
    ...
    const int kBadTimestampsMs[] = { 200, 200, 200, 200, 200, 1000, 1000, 1000, 1000, 200, 200, 200, 200, 200, 1000, 1000, 1000, 1000 };
...
TEST_F(VideoRendererAlgorithmTest, EnqueueFrames)
{
    ...
    algorithm_.EnqueueFrame(CreateFrame(tg.interval(1) - base::Microseconds(999)));
    rendered_frame = RenderAndStep(&tg, &frames_dropped);
    EXPECT_EQ(1u, frames_queued());
...
TEST_F(VideoRendererAlgorithmTest, UsesFrameDuration)
{
    ...
    frame = RenderAndStep(&tg, &frames_dropped);
    EXPECT_EQ(tg.interval(1), algorithm_.average_frame_duration());
    EXPECT_EQ(algorithm_.last_frame_end_time(), base::TimeTicks() + kLongDuration + tg.interval(1) * 3);
}
