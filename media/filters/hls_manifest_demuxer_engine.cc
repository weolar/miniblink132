...
hls::RenditionManager::CodecSupportType GetSupportedTypes(std::string_view container, base::span<const std::string> codecs)
{
    ...
    for (const std::string& codec : codecs) {
        ...
        if (auto result = ParseCodec(codec)) {
            video_formats.push_back({ result->codec, result->profile, result->level, result->color_space, gfx::HdrMetadataType::kNone });
...
HlsDemuxerStatus::Or<RelaxedParserSupportedType> CheckMP4Bytes(const uint8_t* data, size_t size)
{
    NullMediaLog null;
...
HlsDemuxerStatus::Or<RelaxedParserSupportedType> CheckBitstreamForContainerMagic(const uint8_t* data, size_t size)
{
    CHECK_GT(size, 0lu);
...
HlsManifestDemuxerEngine::HlsManifestDemuxerEngine(base::SequenceBound<HlsDataSourceProvider> dsp, scoped_refptr<base::SequencedTaskRunner> media_task_runner,
    base::RepeatingCallback<void(const MediaTrack&)> add_track, base::RepeatingCallback<void(const MediaTrack&)> remove_track, bool was_already_tainted,
    GURL root_playlist_uri, MediaLog* media_log)
...
void HlsManifestDemuxerEngine::ProcessActionQueue()
{
    ...
    std::move(action).Run(base::BindOnce(&HlsManifestDemuxerEngine::OnActionComplete, weak_factory_.GetWeakPtr()));
}
...
HlsManifestDemuxerEngine::PlaylistParseInfo::PlaylistParseInfo(GURL uri, std::vector<std::string> codecs, std::string role, bool allow_multivariant_playlist)
    : uri(std::move(uri))
...
HlsManifestDemuxerEngine::PlaylistParseInfo::PlaylistParseInfo(const PlaylistParseInfo& copy) = default;
...
void HlsManifestDemuxerEngine::Seek(base::TimeDelta time, ManifestDemuxer::SeekCallback cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    if (!network_access_) {
    ...
    ProcessAsyncAction<ManifestDemuxer::SeekResponse>(std::move(cb), base::BindOnce(&HlsManifestDemuxerEngine::SeekAction, weak_factory_.GetWeakPtr(), time));
}

void HlsManifestDemuxerEngine::SeekAction(base::TimeDelta time, ManifestDemuxer::SeekCallback cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    network_access_->AbortPendingReads(
        base::BindPostTaskToCurrentDefault(base::BindOnce(&HlsManifestDemuxerEngine::ContinueSeekInternal, weak_factory_.GetWeakPtr(), time, std::move(cb))));
}

void HlsManifestDemuxerEngine::ContinueSeekInternal(base::TimeDelta time, ManifestDemuxer::SeekCallback cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    for (auto& [_, rendition] : renditions_) {
        ...
        buffers_needed |= (ManifestDemuxer::SeekState::kNeedsData == std::move(response).value());
    }
    std::move(cb).Run(buffers_needed ? ManifestDemuxer::SeekState::kNeedsData : ManifestDemuxer::SeekState::kIsReady);
}

void HlsManifestDemuxerEngine::Initialize(ManifestDemuxerEngineHost* host, PipelineStatusCallback status_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);

    host_ = host;
    ProcessAsyncAction<PipelineStatus>(std::move(status_cb), base::BindOnce(&HlsManifestDemuxerEngine::InitAction, weak_factory_.GetWeakPtr()));
}
...
void HlsManifestDemuxerEngine::InitAction(PipelineStatusCallback status_cb)
{
    ...
    LoadPlaylist(parse_info, base::BindOnce(&HlsManifestDemuxerEngine::FinishInitialization, weak_factory_.GetWeakPtr(), std::move(status_cb)));
}

void HlsManifestDemuxerEngine::FinishInitialization(PipelineStatusCallback cb, PipelineStatus status)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    std::move(cb).Run(std::move(status));
}

void HlsManifestDemuxerEngine::OnTimeUpdate(base::TimeDelta time, double playback_rate, ManifestDemuxer::DelayCallback cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    CHECK_LE(renditions_.size(), 3lu);
    ProcessAsyncAction<base::TimeDelta>(
        std::move(cb), base::BindOnce(&HlsManifestDemuxerEngine::OnTimeUpdateAction, weak_factory_.GetWeakPtr(), time, playback_rate));
}

void HlsManifestDemuxerEngine::OnTimeUpdateAction(base::TimeDelta time, double playback_rate, ManifestDemuxer::DelayCallback cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media", "HLS::OnTimeUpdate", this);
    cb = base::BindOnce(&HlsManifestDemuxerEngine::FinishTimeUpdate, weak_factory_.GetWeakPtr(), std::move(cb));
    for (const auto& [role, _] : renditions_) {
        cb = base::BindOnce(&HlsManifestDemuxerEngine::CheckState, weak_factory_.GetWeakPtr(), time, playback_rate, role, std::move(cb));
    }
    std::move(cb).Run(kNoTimestamp);
}

void HlsManifestDemuxerEngine::FinishTimeUpdate(ManifestDemuxer::DelayCallback cb, base::TimeDelta delay_time)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
...
void HlsManifestDemuxerEngine::CheckState(
    base::TimeDelta time, double playback_rate, std::string role, ManifestDemuxer::DelayCallback cb, base::TimeDelta delay_time)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    DCHECK(renditions_.contains(role));
    renditions_[role]->CheckState(time, playback_rate,
        base::BindOnce(&HlsManifestDemuxerEngine::OnStateChecked, weak_factory_.GetWeakPtr(), base::TimeTicks::Now(), delay_time, std::move(cb)));
}

void HlsManifestDemuxerEngine::OnStateChecked(
    base::TimeTicks start_time, base::TimeDelta prior_delay, ManifestDemuxer::DelayCallback cb, base::TimeDelta new_delay)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
...
void HlsManifestDemuxerEngine::UpdateRenditionManifestUri(std::string role, GURL uri, base::OnceCallback<void(bool)> cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "HLS::UpdateRenditionManifest", this, "uri", uri);
    GURL uri_copy = uri;
    ReadManifest(std::move(uri_copy),
        base::BindOnce(&HlsManifestDemuxerEngine::UpdateMediaPlaylistForRole, weak_factory_.GetWeakPtr(), std::move(role), std::move(uri), std::move(cb)));
}

void HlsManifestDemuxerEngine::UpdateMediaPlaylistForRole(
    std::string role, GURL uri, base::OnceCallback<void(bool)> cb, HlsDataSourceProvider::ReadResult maybe_stream)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    auto maybe_playlist = ParseMediaPlaylistFromStringSource(stream->AsString(), std::move(uri), (*maybe_info).version);
    if (!maybe_playlist.has_value()) {
        Abort(std::move(maybe_playlist).error().AddHere());
        std::move(cb).Run(false);
        return;
    }
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "HLS::UpdateRenditionManifest", this);

    renditions_[role]->UpdatePlaylist(std::move(maybe_playlist).value(), std::nullopt);
    std::move(cb).Run(true);
}

void HlsManifestDemuxerEngine::OnRenditionsReselected(
    hls::AdaptationReason reason, const hls::VariantStream* variant, const hls::AudioRendition* audio_override_rendition)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    stats_reporter_.OnAdaptation(reason);
    ProcessAsyncAction<PipelineStatus>(base::BindOnce(&HlsManifestDemuxerEngine::OnStatus, weak_factory_.GetWeakPtr()),
        base::BindOnce(&HlsManifestDemuxerEngine::AdaptationAction, weak_factory_.GetWeakPtr(), variant, audio_override_rendition));
}

void HlsManifestDemuxerEngine::AdaptationAction(
    const hls::VariantStream* variant, const hls::AudioRendition* audio_override_rendition, PipelineStatusCallback status_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "HLS::SelectRenditions", this, "reselect", true);
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    OnStatus({ PipelineStatus::Codes::DEMUXER_ERROR_COULD_NOT_PARSE, std::move(status) });
}
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    OnStatus({ PipelineStatus::Codes::DEMUXER_ERROR_COULD_NOT_OPEN, std::move(status) });
}
...
void HlsManifestDemuxerEngine::UpdateHlsDataSourceStats(
    HlsDataSourceProvider::ReadCb cb, HlsDataSourceProvider::ReadStatus::Or<std::unique_ptr<HlsDataSourceStream>> result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    if (!result.has_value()) {
...
void HlsManifestDemuxerEngine::ReadKey(const hls::MediaSegment::EncryptionData& data, HlsDataSourceProvider::ReadCb cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    network_access_->ReadKey(std::move(data), BindStatsUpdate(std::move(cb)));
}

HlsDataSourceProvider::ReadCb HlsManifestDemuxerEngine::BindStatsUpdate(HlsDataSourceProvider::ReadCb cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    return base::BindOnce(&HlsManifestDemuxerEngine::UpdateHlsDataSourceStats, weak_factory_.GetWeakPtr(), std::move(cb));
}

void HlsManifestDemuxerEngine::ReadManifest(const GURL& uri, HlsDataSourceProvider::ReadCb cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    network_access_->ReadManifest(std::move(uri), BindStatsUpdate(std::move(cb)));
}

void HlsManifestDemuxerEngine::ReadMediaSegment(const hls::MediaSegment& segment, bool read_chunked, bool include_init, HlsDataSourceProvider::ReadCb cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    network_access_->ReadMediaSegment(segment, read_chunked, include_init, BindStatsUpdate(std::move(cb)));
}

void HlsManifestDemuxerEngine::ReadStream(std::unique_ptr<HlsDataSourceStream> stream, HlsDataSourceProvider::ReadCb cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    network_access_->ReadStream(std::move(stream), BindStatsUpdate(std::move(cb)));
}
...
void HlsManifestDemuxerEngine::ParsePlaylist(PipelineStatusCallback parse_complete_cb, PlaylistParseInfo parse_info, HlsDataSourceProvider::ReadResult m_stream)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    if (stream->buffer_size() > 102400) {
        MEDIA_LOG(WARNING, media_log_) << "Large Manifest detected: " << stream->buffer_size();
    }
    ...
    case hls::Playlist::Kind::kMultivariantPlaylist: {
        ...
        auto playlist = hls::MultivariantPlaylist::Parse(stream->AsString(), parse_info.uri, (*m_info).version);
        if (!playlist.has_value()) {
            return Abort(std::move(playlist).error().AddHere());
        }
        return OnMultivariantPlaylist(std::move(parse_complete_cb), std::move(playlist).value());
    }
    case hls::Playlist::Kind::kMediaPlaylist: {
        ...
        auto playlist = ParseMediaPlaylistFromStringSource(stream->AsString(), parse_info.uri, (*m_info).version);
        if (!playlist.has_value()) {
            return Abort(std::move(playlist).error().AddHere());
        }
        return OnMediaPlaylist(std::move(parse_complete_cb), std::move(parse_info), std::move(playlist).value());
    }
    }
}

hls::ParseStatus::Or<scoped_refptr<hls::MediaPlaylist>> HlsManifestDemuxerEngine::ParseMediaPlaylistFromStringSource(
    std::string_view source, GURL uri, hls::types::DecimalInteger version)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    return hls::MediaPlaylist::Parse(source, uri, version, multivariant_root_.get());
}
...
void HlsManifestDemuxerEngine::AddRenditionForTesting(std::string role, std::unique_ptr<HlsRendition> test_rendition)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
...
void HlsManifestDemuxerEngine::OnMultivariantPlaylist(PipelineStatusCallback parse_complete_cb, scoped_refptr<hls::MultivariantPlaylist> playlist)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    rendition_manager_ = std::make_unique<hls::RenditionManager>(multivariant_root_,
        base::BindRepeating(&HlsManifestDemuxerEngine::OnRenditionsReselected, weak_factory_.GetWeakPtr()), base::BindRepeating(&GetSupportedTypes));
    ...
    rendition_manager_->Reselect(base::BindOnce(&HlsManifestDemuxerEngine::OnRenditionsSelected, weak_factory_.GetWeakPtr(), std::move(parse_complete_cb)));
}

void HlsManifestDemuxerEngine::OnRenditionsSelected(
    PipelineStatusCallback on_complete, const hls::VariantStream* variant, const hls::AudioRendition* audio_override_rendition)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    if (audio_override_rendition) {
        PlaylistParseInfo override_parse_info = { audio_override_rendition->GetUri().value(), selected_variant_codecs_, kAudioOverride };

        on_complete = PipelineStatus::BindOkContinuation(
            std::move(on_complete), base::BindOnce(&HlsManifestDemuxerEngine::LoadPlaylist, weak_factory_.GetWeakPtr(), std::move(override_parse_info)));
    }
    ...
    if (variant) {
        PlaylistParseInfo primary_parse_info = { variant->GetPrimaryRenditionUri(), selected_variant_codecs_, kPrimary };
...
void HlsManifestDemuxerEngine::LoadPlaylist(PlaylistParseInfo parse_info, PipelineStatusCallback on_complete)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    auto uri = parse_info.uri;
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "HLS::LoadPlaylist", this, "uri", uri);
    ReadManifest(
        std::move(uri), base::BindOnce(&HlsManifestDemuxerEngine::ParsePlaylist, weak_factory_.GetWeakPtr(), std::move(on_complete), std::move(parse_info)));
}

void HlsManifestDemuxerEngine::OnMediaPlaylist(
    PipelineStatusCallback parse_complete_cb, PlaylistParseInfo parse_info, scoped_refptr<hls::MediaPlaylist> playlist)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media", "HLS::DetermineStreamContainerAndCodecs", this);
    DetermineStreamContainer(playlist_ptr,
        base::BindOnce(&HlsManifestDemuxerEngine::OnStreamContainerDetermined, weak_factory_.GetWeakPtr(), std::move(parse_complete_cb), std::move(parse_info),
            std::move(playlist)));
}

void HlsManifestDemuxerEngine::OnStreamContainerDetermined(PipelineStatusCallback parse_complete_cb, PlaylistParseInfo parse_info,
    scoped_refptr<hls::MediaPlaylist> playlist, HlsDemuxerStatus::Or<RelaxedParserSupportedType> maybe_mime)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    CHECK(!renditions_.contains(parse_info.role));
    if (!maybe_mime.has_value()) {
        std::move(parse_complete_cb).Run({ DEMUXER_ERROR_COULD_NOT_OPEN, std::move(maybe_mime).error() });
        return;
    }
    ...
    auto rendition = HlsRendition::CreateRendition(host_, this, parse_info.role, std::move(playlist), parse_info.uri, media_log_.get());
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "HLS::DetermineStreamContainerAndCodecs", this);
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "HLS::LoadPlaylist", this);
    std::move(parse_complete_cb).Run(OkStatus());
}

void HlsManifestDemuxerEngine::DetermineStreamContainer(hls::MediaPlaylist* playlist, HlsDemuxerStatusCb<RelaxedParserSupportedType> container_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    if (mime.has_value()) {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "HLS::PeekSegmentChunk", this, "uri", segments[0]->GetUri());
        ...
        ReadMediaSegment(*segments[0], read_chunked, /*include_init=*/true,
            base::BindOnce(&HlsManifestDemuxerEngine::DetermineBitstreamContainer, weak_factory_.GetWeakPtr(), segments[0], std::move(container_cb)));
    }
}

void HlsManifestDemuxerEngine::DetermineBitstreamContainer(
    scoped_refptr<hls::MediaSegment> segment, HlsDemuxerStatusCb<RelaxedParserSupportedType> cb, HlsDataSourceProvider::ReadResult maybe_stream)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    ...
    if (auto enc_data = segment->GetEncryptionData()) {
        ...
        case hls::XKeyTagMethod::kAES256: {
            ...
            base::span<const uint8_t> stream_data = base::span(stream->raw_data(), stream->buffer_size());
            ...
            decryptor = nullptr;
            std::move(cb).Run(CheckBitstreamForContainerMagic(plaintext.data(), plaintext.size()));
            return;
        }
        default: {
            std::move(cb).Run(HlsDemuxerStatus::Codes::kInvalidBitstream);
    ...
    std::move(cb).Run(CheckBitstreamForContainerMagic(stream->raw_data(), stream->buffer_size()));
}

void HlsManifestDemuxerEngine::OnChunkDemuxerParseWarning(std::string role, SourceBufferParseWarning warning)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
    MEDIA_LOG(WARNING, media_log_) << "ParseWarning (" << role << "): " << static_cast<int>(warning);
}

void HlsManifestDemuxerEngine::OnChunkDemuxerTracksChanged(std::string role, std::unique_ptr<MediaTracks> tracks)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
