...
public:
    ...
    void OnMemoryPressure(base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc);
    ...
    void OnStartOfCodedFrameGroup(DecodeTimestamp start_dts, base::TimeDelta start_pts);
    ...
    bool UpdateAudioConfig(const AudioDecoderConfig& config, bool allow_codec_change, MediaLog* media_log);
    bool UpdateVideoConfig(const VideoDecoderConfig& config, bool allow_codec_change, MediaLog* media_log);
    ...
    MediaTrack::Id media_track_id() const
    {
    ...
    using GroupStartObserverCB = base::RepeatingCallback<void(DecodeTimestamp, base::TimeDelta)>;
    void set_group_start_observer_for_testing(GroupStartObserverCB group_start_observer_cb)
    {
...
private:
    ...
    std::pair<SourceBufferStreamStatus, DemuxerStream::DecoderBufferVector> GetPendingBuffers_Locked() EXCLUSIVE_LOCKS_REQUIRED(lock_);
...
public:
    ...
    ChunkDemuxer(base::OnceClosure open_cb, base::RepeatingClosure progress_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb, MediaLog* media_log);
    ...
    std::optional<container_names::MediaContainerName> GetContainerForMetrics() const override;
    ...
    // chunks for this ID.
    [[nodiscard]] Status AddId(const std::string& id, const std::string& content_type, const std::string& codecs);
    [[nodiscard]] Status AddId(const std::string& id, std::unique_ptr<AudioDecoderConfig> audio_config);
    [[nodiscard]] Status AddId(const std::string& id, std::unique_ptr<VideoDecoderConfig> video_config);
    ...
    [[nodiscard]] Status AddAutoDetectedCodecsId(const std::string& id, RelaxedParserSupportedType mime_type);
#endif

    // Notifies a caller via `tracks_updated_cb` that the set of media tracks
    // for a given `id` has changed. This callback must be set before any calls to
    // AppendToParseBuffer() for this `id`.
    void SetTracksWatcher(const std::string& id, MediaTracksUpdatedCB tracks_updated_cb);
    ...
    void SetParseWarningCallback(const std::string& id, SourceBufferParseWarningCB parse_warning_cb);
    ...
    void OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb) override;

    void OnSelectedVideoTrackChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb) override;
    ...
    // playback.
    [[nodiscard]] bool AppendToParseBuffer(const std::string& id, base::span<const uint8_t> data);
    ...
    [[nodiscard]] bool AppendChunks(const std::string& id, std::unique_ptr<StreamParser::BufferQueue> buffer_queue, base::TimeDelta append_window_start,
    ...
    void ResetParserState(const std::string& id, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset);

    // Remove buffers between |start| and |end| for the source buffer
    // associated with |id|.
    void Remove(const std::string& id, base::TimeDelta start, base::TimeDelta end);
    ...
    bool CanChangeType(const std::string& id, const std::string& content_type, const std::string& codecs);
    ...
    void ChangeType(const std::string& id, const std::string& content_type, const std::string& codecs);

    // If the buffer is full, attempts to try to free up space, as specified in
    // the "Coded Frame Eviction Algorithm" in the Media Source Extensions Spec.
    // Returns false iff buffer is still full after running eviction.
    // https://w3c.github.io/media-source/#sourcebuffer-coded-frame-eviction
    [[nodiscard]] bool EvictCodedFrames(const std::string& id, base::TimeDelta currentMediaTime, size_t newDataSize);

    void OnMemoryPressure(base::TimeDelta currentMediaTime, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc);
    ...
    void SetGroupStartTimestampIfInSequenceMode(const std::string& id, base::TimeDelta timestamp_offset);
...
private:
    ...
    void FindAndEnableProperTracks(
    ...
    void OnSourceInitDone(const std::string& source_id, const StreamParser::InitParameters& params);
    ...
    // ChunkDemuxer.
    ChunkDemuxerStream* CreateDemuxerStream(const std::string& source_id, DemuxerStream::Type type);
    ...
    std::map<MediaTrack::Id, raw_ptr<ChunkDemuxerStream, CtnExperimental>> track_id_to_demux_stream_map_;
