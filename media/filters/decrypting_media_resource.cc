...
DecryptingMediaResource::DecryptingMediaResource(
    MediaResource* media_resource, CdmContext* cdm_context, MediaLog* media_log, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void DecryptingMediaResource::Initialize(InitCB init_cb, WaitingCB waiting_cb)
{
    ...
    for (media::DemuxerStream* stream : streams) {
        auto decrypting_demuxer_stream = std::make_unique<DecryptingDemuxerStream>(task_runner_, media_log_, waiting_cb);
        ...
        decrypting_demuxer_stream->Initialize(
            stream, cdm_context_, base::BindOnce(&DecryptingMediaResource::OnDecryptingDemuxerInitialized, weak_factory_.GetWeakPtr()));
...
void DecryptingMediaResource::OnDecryptingDemuxerInitialized(PipelineStatus status)
{
