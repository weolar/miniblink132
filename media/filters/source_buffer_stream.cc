...
std::string RangeToString(const SourceBufferRange& range)
{
    ...
    std::stringstream ss;
    ss << "[" << range.GetStartTimestamp().InMicroseconds() << "us;" << range.GetEndTimestamp().InMicroseconds() << "us("
...
std::string BufferQueueBuffersToLogString(const SourceBufferStream::BufferQueue& buffers)
{
    std::stringstream result;

    result << "Buffers:\n";
    for (const auto& buf : buffers) {
        result << "\tdts=" << buf->GetDecodeTimestamp().InMicroseconds() << " " << buf->AsHumanReadableString()
...
std::string BufferQueueMetadataToLogString(const SourceBufferStream::BufferQueue& buffers)
{
    std::stringstream result;
    base::TimeDelta pts_interval_start;
    base::TimeDelta pts_interval_end;
    SourceBufferStream::GetTimestampInterval(buffers, &pts_interval_start, &pts_interval_end);

    result << "dts=[" << buffers.front()->GetDecodeTimestamp().InMicroseconds() << "us;" << buffers.back()->GetDecodeTimestamp().InMicroseconds()
           << "us(last frame dur=" << buffers.back()->duration().InMicroseconds() << "us)], pts interval=[" << pts_interval_start.InMicroseconds() << "us,"
           << pts_interval_end.InMicroseconds() << "us)";
    return result.str();
}

} // namespace

SourceBufferStream::SourceBufferStream(const AudioDecoderConfig& audio_config, MediaLog* media_log)
    : media_log_(media_log)
    , seek_buffer_timestamp_(kNoTimestamp)
    , coded_frame_group_start_pts_(kNoTimestamp)
    , range_for_next_append_(ranges_.end())
    , highest_output_buffer_timestamp_(kNoTimestamp)
    , max_interbuffer_distance_(base::Milliseconds(kMinimumInterbufferDistanceInMs))
...
SourceBufferStream::SourceBufferStream(const VideoDecoderConfig& video_config, MediaLog* media_log)
    : media_log_(media_log)
    , seek_buffer_timestamp_(kNoTimestamp)
    , coded_frame_group_start_pts_(kNoTimestamp)
    , range_for_next_append_(ranges_.end())
    , highest_output_buffer_timestamp_(kNoTimestamp)
    , max_interbuffer_distance_(base::Milliseconds(kMinimumInterbufferDistanceInMs))
    , memory_limit_(GetDemuxerStreamVideoMemoryLimit(Demuxer::DemuxerTypes::kChunkDemuxer, &video_config))
{
...
void SourceBufferStream::OnStartOfCodedFrameGroup(base::TimeDelta coded_frame_group_start_pts)
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << " (pts " << coded_frame_group_start_pts.InMicroseconds() << "us)";
    ...
    if (range_for_next_append_ == ranges_.end() || !IsNextGopAdjacentToEndOfCurrentAppendSequence(coded_frame_group_start_pts_)) {
        ResetLastAppendedState();
        DVLOG(3) << __func__ << " next appended buffers will " << (range_for_next_append_ == ranges_.end() ? "be in a new range" : "overlap an existing range");
        ...
        if (range_for_next_append_ != ranges_.end()) {
            ...
            // call.
            base::TimeDelta adjusted_start_time = (*range_for_next_append_)->FindHighestBufferedTimestampAtOrBefore(coded_frame_group_start_pts_);
            if (adjusted_start_time < coded_frame_group_start_pts_) {
                ...
                coded_frame_group_start_pts_ = adjusted_start_time + base::Microseconds(1);
            }
        }
...
{
    TRACE_EVENT2("media", "SourceBufferStream::Append", "stream type", GetStreamTypeName(), "buffers to append", buffers.size());

    DCHECK(!buffers.empty());
    DCHECK(coded_frame_group_start_pts_ != kNoTimestamp);
    DCHECK(!end_of_stream_);

    DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": buffers " << BufferQueueMetadataToLogString(buffers);
    DVLOG(4) << BufferQueueBuffersToLogString(buffers);

    DCHECK(!buffers.front()->is_key_frame() || coded_frame_group_start_pts_ <= buffers.front()->timestamp());
    DVLOG_IF(2, coded_frame_group_start_pts_ > buffers.front()->timestamp())
        << __func__ << " Suspected SAP-Type-2 occurrence: coded_frame_group_start_pts_=" << coded_frame_group_start_pts_.InMicroseconds()
        << "us, first new buffer has timestamp=" << buffers.front()->timestamp().InMicroseconds() << "us";
    ...
    if (range_for_next_append_ != ranges_.end()) {
        if (new_coded_frame_group_) {
            ...
            (*range_for_next_append_)->AppendBuffersToEnd(buffers, coded_frame_group_start_pts_);
        } else {
        ...
        last_appended_buffer_decode_timestamp_ = buffers.back()->GetDecodeTimestamp();
        highest_timestamp_in_append_sequence_ = (*range_for_next_append_)->GetEndTimestamp();
        highest_buffered_end_time_in_append_sequence_ = (*range_for_next_append_)->GetBufferedEndTimestamp();
    } else {
        base::TimeDelta new_range_start_time = std::min(coded_frame_group_start_pts_, buffers.front()->timestamp());
        ...
        if (!new_coded_frame_group_) {
            ...
            if (itr == buffers.end()) {
                ...
                DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": done. ranges_=" << RangesToString(ranges_);
                return;
        ...
        range_for_next_append_ = AddToRanges(std::make_unique<SourceBufferRange>(SourceBufferRange::NO_GAPS_ALLOWED, *buffers_for_new_range,
            new_range_start_time, base::BindRepeating(&SourceBufferStream::GetMaxInterbufferDistance, base::Unretained(this))));

        last_appended_buffer_timestamp_ = buffers_for_new_range->back()->timestamp();
        ...
        last_appended_buffer_decode_timestamp_ = buffers_for_new_range->back()->GetDecodeTimestamp();
        highest_timestamp_in_append_sequence_ = (*range_for_next_append_)->GetEndTimestamp();
        highest_buffered_end_time_in_append_sequence_ = (*range_for_next_append_)->GetBufferedEndTimestamp();
    }
    ...
    if (!deleted_buffers.empty()) {
        track_buffer_.insert(track_buffer_.end(), deleted_buffers.begin(), deleted_buffers.end());
        DVLOG(3) << __func__ << " " << GetStreamTypeName() << " Added " << deleted_buffers.size() << " buffers to track buffer. TB size is now "
                 << track_buffer_.size();
    } else {
        DVLOG(3) << __func__ << " " << GetStreamTypeName() << " No deleted buffers for track buffer";
    }

    // Prune any extra buffers in |track_buffer_| if new keyframes
    // are appended to the range covered by |track_buffer_|.
    if (!track_buffer_.empty()) {
        base::TimeDelta keyframe_timestamp = FindKeyframeAfterTimestamp(track_buffer_.front()->timestamp());
...
void SourceBufferStream::Remove(base::TimeDelta start, base::TimeDelta end, base::TimeDelta duration)
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << " (" << start.InMicroseconds() << "us, " << end.InMicroseconds() << "us, "
             << duration.InMicroseconds() << "us)";
    DCHECK(start >= base::TimeDelta()) << start.InMicroseconds() << "us";
    DCHECK(start < end) << "start " << start.InMicroseconds() << "us, end " << end.InMicroseconds() << "us";
    ...
    if (!deleted_buffers.empty()) {
        ...
        if (highest_output_buffer_timestamp_ == kNoTimestamp) {
            ...
            DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": re-seeking to " << seek_buffer_timestamp_
...
void SourceBufferStream::UpdateLastAppendStateForRemove(base::TimeDelta remove_start, base::TimeDelta remove_end, bool exclude_start)
{
    ...
    if (range_for_next_append_ != ranges_.end()) {
        if (last_appended_buffer_timestamp_ != kNoTimestamp) {
            ...
            base::TimeDelta gop_start = (*range_for_next_append_)->KeyframeBeforeTimestamp(gop_end);
            ...
            if (((include_start && remove_start == gop_end) || remove_start < gop_end) && remove_end > gop_start) {
                DVLOG(2) << __func__ << " " << GetStreamTypeName() << " Resetting next append state for remove (" << remove_start.InMicroseconds() << "us, "
                         << remove_end.InMicroseconds() << "us, " << exclude_start << ")";
                range_for_next_append_ = ranges_.end();
                ResetLastAppendedState();
            }
        } else {
            NOTREACHED() << __func__ << " " << GetStreamTypeName() << " range_for_next_append_ set, but not tracking last"
                         << " append nor new coded frame group.";
        }
    }
}

void SourceBufferStream::RemoveInternal(base::TimeDelta start, base::TimeDelta end, bool exclude_start, BufferQueue* deleted_buffers)
{
    DVLOG(2) << __func__ << " " << GetStreamTypeName() << " (" << start.InMicroseconds() << "us, " << end.InMicroseconds() << "us, " << exclude_start << ")";
    DVLOG(3) << __func__ << " " << GetStreamTypeName() << ": before remove ranges_=" << RangesToString(ranges_);
    ...
    while (itr != ranges_.end()) {
        ...
        if (new_range) {
            ...
            if (range_for_next_append_ != ranges_.end() && range_for_next_append_->get() == range) {
                base::TimeDelta potential_next_append_timestamp = PotentialNextAppendTimestamp();
                if (potential_next_append_timestamp != kNoTimestamp && (*itr)->BelongsToRange(potential_next_append_timestamp)) {
    ...
    DVLOG(3) << __func__ << " " << GetStreamTypeName() << ": after remove ranges_=" << RangesToString(ranges_);
...
bool SourceBufferStream::ShouldSeekToStartOfBuffered(base::TimeDelta seek_timestamp) const
{
    if (ranges_.empty())
        return false;
...
bool SourceBufferStream::IsDtsMonotonicallyIncreasing(const BufferQueue& buffers)
{
    DCHECK(!buffers.empty());
    DecodeTimestamp prev_dts = last_appended_buffer_decode_timestamp_;
    for (BufferQueue::const_iterator itr = buffers.begin(); itr != buffers.end(); ++itr) {
        ...
        DCHECK((*itr)->duration() >= base::TimeDelta())
            ...
            << " pts " << (*itr)->timestamp().InMicroseconds() << "us dts " << (*itr)->GetDecodeTimestamp().InMicroseconds() << "us dur "
        ...
        if (prev_dts != kNoDecodeTimestamp) {
            if (current_dts < prev_dts) {
                MEDIA_LOG(ERROR, media_log_) << "Buffers did not monotonically increase.";
                return false;
            }
        }
...
bool SourceBufferStream::UpdateMaxInterbufferDtsDistance(const BufferQueue& buffers)
{
    DCHECK(!buffers.empty());
    ...
    for (BufferQueue::const_iterator itr = buffers.begin(); itr != buffers.end(); ++itr) {
        ...
        if (prev_dts != kNoDecodeTimestamp) {
            interbuffer_distance = std::max(current_dts - prev_dts, interbuffer_distance);
        }

        DCHECK(max_interbuffer_distance_ >= base::Milliseconds(kMinimumInterbufferDistanceInMs));
    ...
    DVLOG_IF(2, changed_max) << __func__ << " " << GetStreamTypeName() << " Changed max interbuffer DTS distance from " << old_distance.InMicroseconds()
                             << "us to " << max_interbuffer_distance_.InMicroseconds() << "us";
    return changed_max;
}
...
void SourceBufferStream::OnMemoryPressure(
    base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
...
bool SourceBufferStream::GarbageCollectIfNeeded(base::TimeDelta media_time, size_t newDataSize)
{
    ...
    if ((newDataSize > memory_limit_) || (ranges_size + newDataSize < ranges_size)) {
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_garbage_collect_algorithm_logs_, kMaxGarbageCollectAlgorithmWarningLogs)
            << GetStreamTypeName() << " stream: "
            << "new append of newDataSize=" << newDataSize << " bytes exceeds memory_limit_=" << memory_limit_
    ...
    DVLOG(2) << __func__ << " " << GetStreamTypeName() << ": Before GC media_time=" << media_time.InMicroseconds() << "us ranges_=" << RangesToString(ranges_)
             << " seek_pending_=" << seek_pending_ << " ranges_size=" << ranges_size << " newDataSize=" << newDataSize << " memory_limit_=" << memory_limit_
             << " effective_memory_limit=" << effective_memory_limit << " last_appended_buffer_timestamp_=" << last_appended_buffer_timestamp_.InMicroseconds()
             << "us highest_timestamp_in_append_sequence_=" << highest_timestamp_in_append_sequence_.InMicroseconds()
             << "us highest_buffered_end_time_in_append_sequence_=" << highest_buffered_end_time_in_append_sequence_.InMicroseconds() << "us";
    ...
    if (selected_range_ && !seek_pending_ && media_time > selected_range_->GetBufferedEndTimestamp()) {
        ...
        base::TimeDelta selected_buffered_end = selected_range_->GetBufferedEndTimestamp();

        DVLOG(2) << __func__ << " media_time " << media_time.InMicroseconds() << "us is outside of selected_range_=["
                 << selected_range_->GetStartTimestamp().InMicroseconds() << "us;" << selected_buffered_end.InMicroseconds() << "us] clamping media_time to be "
    ...
    if (last_appended_buffer_timestamp_ != kNoTimestamp && last_appended_buffer_duration_ != kNoTimestamp
        && highest_buffered_end_time_in_append_sequence_ != kNoTimestamp && media_time > highest_buffered_end_time_in_append_sequence_) {
        ...
        if (range_for_next_append_ != ranges_.end()) {
            ...
            DVLOG(3) << __func__ << " media_time adjusted to " << media_time.InMicroseconds() << "us";
        }
    }
    ...
    if (bytes_freed < bytes_to_free && seek_pending_) {
        ...
        DVLOG(3) << __func__ << " Removed " << front << " bytes from the front. ranges_=" << RangesToString(ranges_);
        bytes_freed += front;
        ...
        if (bytes_freed < bytes_to_free) {
            ...
            DVLOG(3) << __func__ << " Removed " << back << " bytes from the back. ranges_=" << RangesToString(ranges_);
            bytes_freed += back;
        }
        ...
        if (bytes_freed < bytes_to_free) {
            size_t front2 = FreeBuffers(bytes_to_free - bytes_freed, ranges_.back()->GetEndTimestamp(), false);
            DVLOG(3) << __func__ << " Removed " << front2 << " bytes from the front. ranges_=" << RangesToString(ranges_);
            bytes_freed += front2;
        }
    ...
    DVLOG(2) << __func__ << " " << GetStreamTypeName() << ": After GC bytes_to_free=" << bytes_to_free << " bytes_freed=" << bytes_freed
             << " bytes_over_hard_memory_limit=" << bytes_over_hard_memory_limit << " ranges_=" << RangesToString(ranges_);

    return bytes_freed >= bytes_over_hard_memory_limit;
}

size_t SourceBufferStream::FreeBuffersAfterLastAppended(size_t total_bytes_to_free, base::TimeDelta media_time)
{
    DVLOG(4) << __func__ << " highest_buffered_end_time_in_append_sequence_=" << highest_buffered_end_time_in_append_sequence_.InMicroseconds()
             << "us media_time=" << media_time.InMicroseconds() << "us";

    base::TimeDelta remove_range_start = highest_buffered_end_time_in_append_sequence_;
    if (last_appended_buffer_is_keyframe_)
        remove_range_start += GetMaxInterbufferDistance();

    base::TimeDelta remove_range_start_keyframe = FindKeyframeAfterTimestamp(remove_range_start);
    ...
    size_t bytes_freed = GetRemovalRange(remove_range_start, media_time, total_bytes_to_free, &remove_range_end);
    if (bytes_freed > 0) {
        DVLOG(4) << __func__ << " removing [" << remove_range_start.InMicroseconds() << "us;" << remove_range_end.InMicroseconds() << "us]";
...
size_t SourceBufferStream::GetRemovalRange(
    base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t total_bytes_to_free, base::TimeDelta* removal_end_timestamp)
{
    DCHECK(start_timestamp >= base::TimeDelta()) << start_timestamp.InMicroseconds() << "us";
    DCHECK(start_timestamp < end_timestamp) << "start " << start_timestamp.InMicroseconds() << "us, end " << end_timestamp.InMicroseconds() << "us";

    size_t bytes_freed = 0;

    for (auto itr = ranges_.begin(); itr != ranges_.end() && bytes_freed < total_bytes_to_free; ++itr) {
        SourceBufferRange* range = itr->get();
        ...
        size_t bytes_removed = range->GetRemovalGOP(start_timestamp, end_timestamp, bytes_to_free, removal_end_timestamp);
        bytes_freed += bytes_removed;
    }
    return bytes_freed;
}

size_t SourceBufferStream::FreeBuffers(size_t total_bytes_to_free, base::TimeDelta media_time, bool reverse_direction)
{
    TRACE_EVENT2("media", "SourceBufferStream::FreeBuffers", "total bytes to free", total_bytes_to_free, "reverse direction", reverse_direction);
    ...
    while (!ranges_.empty() && bytes_freed < total_bytes_to_free) {
        ...
        if (reverse_direction) {
            ...
            DVLOG(4) << "Deleting GOP from front: " << RangeToString(*current_range) << ", media_time: " << media_time.InMicroseconds()
                     << ", current_range->HasNextBufferPosition(): " << current_range->HasNextBufferPosition();
        ...
        if (end_timestamp == last_appended_buffer_timestamp_) {
            ...
            new_range_for_append = std::make_unique<SourceBufferRange>(SourceBufferRange::NO_GAPS_ALLOWED, buffers, kNoTimestamp,
                base::BindRepeating(&SourceBufferStream::GetMaxInterbufferDistance, base::Unretained(this)));
        ...
        if (current_range->GetMemoryUsage() == 0) {
            ...
            DCHECK(range_for_next_append_ == ranges_.end() || range_for_next_append_->get() != current_range);
...
void SourceBufferStream::TrimSpliceOverlap(const BufferQueue& new_buffers)
{
    ...
    if (range_itr == ranges_.end()) {
        DVLOG(3) << __func__ << " No splice trimming. No range overlap at time " << splice_timestamp.InMicroseconds();
        return;
    }
    ...
    if (!(*range_itr)->GetBuffersInRange(splice_timestamp, end_pts, &overlapped_buffers)) {
        // Bail if no overlapped buffers found.
        DVLOG(3) << __func__ << " No splice trimming. No buffer overlap at time " << splice_timestamp.InMicroseconds();
        return;
    }
    ...
    if (overlapped_buffers.size() != 1U) {
        ...
        MEDIA_LOG(WARNING, media_log_) << "Media is badly muxed. Detected " << overlapped_buffers.size() << " overlapping audio buffers at time "
                                       << splice_timestamp.InMicroseconds();
        return;
    }
    ...
    if (overlapped_buffer->timestamp() == splice_timestamp) {
        ...
        DVLOG(3) << __func__ << " No splice trimming at time " << splice_timestamp.InMicroseconds() << ". Overlapped buffer will be completely removed.";
        return;
    }
    ...
    if (overlapped_buffer->is_duration_estimated()) {
        DVLOG(3) << __func__ << " Skipping audio splice trimming at PTS=" << splice_timestamp.InMicroseconds() << ". Overlapped buffer has "
                 << "estimated duration.";
        return;
    }
    ...
    if (overlap_duration < base::Milliseconds(1)) {
        ...
        log_string << "Skipping audio splice trimming at PTS=" << splice_timestamp.InMicroseconds() << "us. Found only " << overlap_duration.InMicroseconds()
                   << "us of overlap, need at least 1000us. Multiple occurrences "
                   << "may result in loss of A/V sync.";
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_splice_logs_, kMaxAudioSpliceLogs) << log_string.str();
    ...
    std::stringstream log_string;
    log_string << "Audio buffer splice at PTS=" << splice_timestamp.InMicroseconds()
               << "us. Trimmed tail of overlapped buffer (PTS=" << overlapped_buffer->timestamp().InMicroseconds() << "us) by "
               << overlap_duration.InMicroseconds() << "us.";
    LIMITED_MEDIA_LOG(DEBUG, media_log_, num_splice_logs_, kMaxAudioSpliceLogs) << log_string.str();
    DVLOG(1) << __func__ << log_string.str();
}

void SourceBufferStream::PrepareRangesForNextAppend(const BufferQueue& new_buffers, BufferQueue* deleted_buffers)
{
    DCHECK(deleted_buffers);
    ...
    GetTimestampInterval(new_buffers, &buffers_start_timestamp, &buffers_end_timestamp);
    ...
    if (highest_timestamp_in_append_sequence_ != kNoTimestamp && highest_timestamp_in_append_sequence_ < buffers_start_timestamp) {
        RemoveInternal(highest_timestamp_in_append_sequence_, buffers_start_timestamp, true, deleted_buffers);
    }
    ...
    if (highest_timestamp_in_append_sequence_ != kNoTimestamp && buffers_start_timestamp < highest_timestamp_in_append_sequence_) {
        DCHECK(highest_timestamp_in_append_sequence_ <= highest_buffered_end_time_in_append_sequence_);
    ...
    if (new_coded_frame_group_) {
        ...
        DCHECK(coded_frame_group_start_pts_ != kNoTimestamp);
        buffers_start_timestamp = std::min(coded_frame_group_start_pts_, buffers_start_timestamp);
    }
    ...
    if (buffers_start_timestamp >= buffers_end_timestamp) {
        ...
        // early.
        std::unique_ptr<SourceBufferRange> new_range = (*range_for_next_append_)->SplitRange(buffers_start_timestamp);
        if (!new_range)
            return;

        range_for_next_append_ = ranges_.insert(++range_for_next_append_, std::move(new_range));
    ...
    const bool exclude_start = highest_timestamp_in_append_sequence_ == new_buffers.front()->timestamp()
        && (GetType() == SourceBufferStreamType::kVideo || last_appended_buffer_duration_ == base::TimeDelta());

    // Finally do the deletion of overlap.
    RemoveInternal(buffers_start_timestamp, buffers_end_timestamp, exclude_start, deleted_buffers);
}

// static
void SourceBufferStream::GetTimestampInterval(const BufferQueue& buffers, base::TimeDelta* start, base::TimeDelta* end)
{
...
bool SourceBufferStream::IsNextGopAdjacentToEndOfCurrentAppendSequence(base::TimeDelta next_gop_timestamp) const
{
    base::TimeDelta upper_bound = highest_timestamp_in_append_sequence_ + ComputeFudgeRoom(GetMaxInterbufferDistance());
    DVLOG(4) << __func__ << " " << GetStreamTypeName() << " next_gop_timestamp=" << next_gop_timestamp.InMicroseconds()
             << "us, highest_timestamp_in_append_sequence_=" << highest_timestamp_in_append_sequence_.InMicroseconds()
             << "us, upper_bound=" << upper_bound.InMicroseconds() << "us";
    return highest_timestamp_in_append_sequence_ < next_gop_timestamp && next_gop_timestamp <= upper_bound;
}
...
void SourceBufferStream::PruneTrackBuffer(const base::TimeDelta timestamp)
{
    ...
    DVLOG(3) << __func__ << " " << GetStreamTypeName()
             << " Removed all buffers in track buffer sequence starting with the "
                "first at timestamp >= "
             << timestamp.InMicroseconds() << "us. New track buffer size:" << track_buffer_.size();
}

void SourceBufferStream::MergeWithNextRangeIfNecessary(const RangeList::iterator& range_with_new_buffers_itr)
{
    ...
    DVLOG(3) << __func__ << " " << GetStreamTypeName() << " merging " << RangeToString(*range_with_new_buffers) << " into " << RangeToString(**next_range_itr);
    range_with_new_buffers->AppendRangeToEnd(**next_range_itr, transfer_current_position);
...
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": Before: ranges_=" << RangesToString(ranges_);
    ...
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": After: ranges_=" << RangesToString(ranges_);
}

void SourceBufferStream::Seek(base::TimeDelta timestamp)
{
    DCHECK(timestamp >= base::TimeDelta());
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << " (" << timestamp.InMicroseconds() << "us)";
    ResetSeekState();
    ...
    if (!audio_configs_.empty()) {
        ...
        if (config.codec() == AudioCodec::kOpus && timestamp > (*itr)->GetStartTimestamp()) {
            base::TimeDelta preroll_timestamp = std::max(timestamp - config.seek_preroll(), (*itr)->GetStartTimestamp());
            if ((*itr)->CanSeekTo(preroll_timestamp) && (*itr)->SameConfigThruRange(preroll_timestamp, timestamp)) {
...
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << " (" << duration.InMicroseconds() << "us)";
    DCHECK(!end_of_stream_);
...
SourceBufferStreamStatus SourceBufferStream::GetNextBuffer(scoped_refptr<StreamParserBuffer>* out_buffer)
{
    DVLOG(2) << __func__ << " " << GetStreamTypeName();
    if (!pending_buffer_.get()) {
        const SourceBufferStreamStatus status = GetNextBufferInternal(out_buffer);
        if (status != SourceBufferStreamStatus::kSuccess || !SetPendingBuffer(out_buffer)) {
            DVLOG(2) << __func__ << " " << GetStreamTypeName() << ": no pending buffer, returning status " << StatusToString(status);
            return status;
        }
    }
    ...
    const SourceBufferStreamStatus status = HandleNextBufferWithPreroll(out_buffer);
    DVLOG(2) << __func__ << " " << GetStreamTypeName() << ": handled next buffer with preroll, returning status " << StatusToString(status);
    return status;
}

SourceBufferStreamStatus SourceBufferStream::HandleNextBufferWithPreroll(scoped_refptr<StreamParserBuffer>* out_buffer)
{
...
SourceBufferStreamStatus SourceBufferStream::GetNextBufferInternal(scoped_refptr<StreamParserBuffer>* out_buffer)
{
    ...
    if (!track_buffer_.empty()) {
        ...
        *out_buffer = std::move(track_buffer_.front());
        ...
        highest_output_buffer_timestamp_ = std::max(highest_output_buffer_timestamp_, (*out_buffer)->timestamp());
    ...
    if (!selected_range_ || !selected_range_->HasNextBuffer()) {
        ...
        DVLOG(3) << __func__ << " " << GetStreamTypeName() << ": returning kNeedBuffer "
                 << (selected_range_ ? "(selected range has no next buffer)" : "(no selected range)");
...
void SourceBufferStream::WarnIfTrackBufferExhaustionSkipsForward(scoped_refptr<StreamParserBuffer> next_buffer)
{
    ...
    base::TimeDelta delta = next_output_buffer_timestamp - highest_output_buffer_timestamp_;
    if (delta > GetMaxInterbufferDistance()) {
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_track_buffer_gap_warning_logs_, kMaxTrackBufferGapWarningLogs)
            << "Media append that overlapped current playback position may cause "
               "time gap in playing "
            << GetStreamTypeName() << " stream because the next keyframe is " << delta.InMilliseconds()
...
SourceBufferStream::RangeList::iterator SourceBufferStream::FindExistingRangeFor(base::TimeDelta start_timestamp)
{
    for (auto itr = ranges_.begin(); itr != ranges_.end(); ++itr) {
...
SourceBufferStream::RangeList::iterator SourceBufferStream::AddToRanges(std::unique_ptr<SourceBufferRange> new_range)
{
...
void SourceBufferStream::SeekAndSetSelectedRange(SourceBufferRange* range, base::TimeDelta seek_timestamp)
{
    if (range)
...
{
    DVLOG(1) << __func__ << " " << GetStreamTypeName() << ": " << selected_range_ << " " << (selected_range_ ? RangeToString(*selected_range_) : "") << " -> "
...
bool SourceBufferStream::IsEndOfStreamReached() const
{
    ...
    if (seek_pending_) {
        base::TimeDelta last_range_end_time = ranges_.back()->GetBufferedEndTimestamp();
...
const AudioDecoderConfig& SourceBufferStream::GetCurrentAudioDecoderConfig()
{
    ...
    CHECK(current_config_index_ >= 0 && static_cast<size_t>(current_config_index_) < audio_configs_.size());
...
const VideoDecoderConfig& SourceBufferStream::GetCurrentVideoDecoderConfig()
{
    if (config_change_pending_)
        CompleteConfigChange();
    // Trying to track down crash. http://crbug.com/715761
    CHECK(current_config_index_ >= 0 && static_cast<size_t>(current_config_index_) < video_configs_.size());
...
bool SourceBufferStream::UpdateAudioConfig(const AudioDecoderConfig& config, bool allow_codec_change)
{
    DCHECK(!audio_configs_.empty());
    DCHECK(video_configs_.empty());
    DVLOG(3) << "UpdateAudioConfig.";

    if (!allow_codec_change && audio_configs_[append_config_index_].codec() != config.codec()) {
    ...
    if (memory_limit_overridden_) {
        DVLOG(2) << __func__ << ": Skipping updating memory limit as memory limit was overridden.";
    } else {
        ...
        if (new_memory_limit > memory_limit_) {
            DVLOG(2) << __func__ << ": Increase memory limit from " << memory_limit_ << " to " << new_memory_limit << ".";
...
bool SourceBufferStream::UpdateVideoConfig(const VideoDecoderConfig& config, bool allow_codec_change)
{
    DCHECK(!video_configs_.empty());
    DCHECK(audio_configs_.empty());
    DVLOG(3) << "UpdateVideoConfig.";

    if (!allow_codec_change && video_configs_[append_config_index_].codec() != config.codec()) {
        // TODO(wolenetz): When we relax addSourceBuffer() and changeType() codec
        // strictness, codec changes should be allowed even without changing the
        // bytestream.
        // TODO(wolenetz): Remove "experimental" from this error message when
    ...
    if (memory_limit_overridden_) {
        ...
        size_t new_memory_limit = GetDemuxerStreamVideoMemoryLimit(Demuxer::DemuxerTypes::kChunkDemuxer, &config);
        if (new_memory_limit > memory_limit_) {
            DVLOG(2) << __func__ << ": Increase memory limit from " << memory_limit_ << " to " << new_memory_limit << ".";
            memory_limit_ = new_memory_limit;
        }
...
void SourceBufferStream::SetSelectedRangeIfNeeded(const base::TimeDelta timestamp)
{
    DVLOG(2) << __func__ << " " << GetStreamTypeName() << "(" << timestamp.InMicroseconds() << "us)";
    ...
    if (start_timestamp == kNoTimestamp) {
        if (highest_output_buffer_timestamp_ == kNoTimestamp) {
            DVLOG(2) << __func__ << " " << GetStreamTypeName() << " no previous output timestamp";
            return;
        }
    ...
    base::TimeDelta seek_timestamp = FindNewSelectedRangeSeekTimestamp(start_timestamp);

    // If we don't have buffered data to seek to, then return.
    if (seek_timestamp == kNoTimestamp) {
        DVLOG(2) << __func__ << " " << GetStreamTypeName() << " couldn't find new selected range seek timestamp";
        return;
    }

    DCHECK(track_buffer_.empty());
    SeekAndSetSelectedRange(FindExistingRangeFor(seek_timestamp)->get(), seek_timestamp);
}

base::TimeDelta SourceBufferStream::FindNewSelectedRangeSeekTimestamp(const base::TimeDelta start_timestamp)
{
    ...
    base::TimeDelta start_timestamp_plus_fudge = start_timestamp + ComputeFudgeRoom(GetMaxInterbufferDistance());
    ...
    for (; itr != ranges_.end(); ++itr) {
        ...
        base::TimeDelta keyframe_timestamp = (*itr)->NextKeyframeTimestamp(search_timestamp);
        if (keyframe_timestamp != kNoTimestamp)
            return keyframe_timestamp;
    }

    DVLOG(2) << __func__ << " " << GetStreamTypeName() << " no buffered data for pts=" << start_timestamp.InMicroseconds() << "us";
    return kNoTimestamp;
}

base::TimeDelta SourceBufferStream::FindKeyframeAfterTimestamp(const base::TimeDelta timestamp)
{
    DCHECK(timestamp != kNoTimestamp);
...
bool SourceBufferStream::SetPendingBuffer(scoped_refptr<StreamParserBuffer>* out_buffer)
{
