...
DecryptingVideoDecoder::DecryptingVideoDecoder(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log)
    ...
    const VideoDecoderConfig& config, bool /* low_delay */, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (state_ == kUninitialized) {
        ...
        event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&DecryptingVideoDecoder::OnCdmContextEvent, weak_factory_.GetWeakPtr()));
    } else {
    ...
    decryptor_->InitializeVideoDecoder(
        config_, base::BindPostTaskToCurrentDefault(base::BindOnce(&DecryptingVideoDecoder::FinishInitialization, weak_factory_.GetWeakPtr())));
}
...
void DecryptingVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOG(3) << "Decode()";
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (HasClearLead() && !switched_clear_to_encrypted_ && !buffer->end_of_stream() && buffer->is_encrypted()) {
...
void DecryptingVideoDecoder::DecodePendingBuffer()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2("media", "DecryptingVideoDecoder::DecodePendingBuffer", this, "is_encrypted", is_encrypted, "timestamp_us", timestamp_us);

    if (!DecoderBuffer::DoSubsamplesMatch(*buffer)) {
        MEDIA_LOG(ERROR, media_log_) << "DecryptingVideoDecoder: Subsamples for Buffer do not match";
        state_ = kError;
        std::move(decode_cb_).Run(DecoderStatus::Codes::kPlatformDecodeFailure);
        return;
    }
    ...
    decryptor_->DecryptAndDecodeVideo(
        buffer, base::BindPostTaskToCurrentDefault(base::BindRepeating(&DecryptingVideoDecoder::DeliverFrame, weak_factory_.GetWeakPtr())));
}
...
void DecryptingVideoDecoder::DeliverFrame(Decryptor::Status status, scoped_refptr<VideoFrame> frame)
{
    ...
    scoped_refptr<DecoderBuffer> scoped_pending_buffer_to_decode = std::move(pending_buffer_to_decode_);
    ...
    if (status == Decryptor::kNoKey) {
        ...
        std::string log_message = "no key for key ID " + base::HexEncode(key_id) + "; will resume decoding after new usable key is available";
        ...
        if (need_to_try_again_if_nokey_is_returned) {
            ...
            MEDIA_LOG(INFO, media_log_) << GetDecoderType() << ": key was added, resuming decode";
            DecodePendingBuffer();
            return;
        }

        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("media", "DecryptingVideoDecoder::WaitingForDecryptionKey", this);
    ...
    if (status == Decryptor::kNeedMoreData) {
        ...
        state_ = scoped_pending_buffer_to_decode->end_of_stream() ? kDecodeFinished : kIdle;
        std::move(decode_cb_).Run(DecoderStatus::Codes::kOk);
        return;
    }
...
void DecryptingVideoDecoder::OnCdmContextEvent(CdmContext::Event event)
{
    ...
    if (state_ == kWaitingForKey) {
        CompleteWaitingForDecryptionKey();
        MEDIA_LOG(INFO, media_log_) << GetDecoderType() << ": key added, resuming decode";
        state_ = kPendingDecode;
        DecodePendingBuffer();
    }
}
...
void DecryptingVideoDecoder::CompletePendingDecode(Decryptor::Status status)
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END1("media", "DecryptingVideoDecoder::DecodePendingBuffer", this, "status", Decryptor::GetStatusName(status));
}

void DecryptingVideoDecoder::CompleteWaitingForDecryptionKey()
{
    DCHECK_EQ(state_, kWaitingForKey);
    TRACE_EVENT_NESTABLE_ASYNC_END0("media", "DecryptingVideoDecoder::WaitingForDecryptionKey", this);
}
