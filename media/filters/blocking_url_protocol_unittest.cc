...
public:
    BlockingUrlProtocolTest()
        : url_protocol_(
            std::make_unique<BlockingUrlProtocol>(&data_source_, base::BindRepeating(&BlockingUrlProtocolTest::OnDataSourceError, base::Unretained(this))))
    {
    ...
    ~BlockingUrlProtocolTest() override
    {
        data_source_.Stop();
    }
