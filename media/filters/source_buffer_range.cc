...
SourceBufferRange::SourceBufferRange(
    GapPolicy gap_policy, const BufferQueue& new_buffers, base::TimeDelta range_start_pts, InterbufferDistanceCB interbuffer_distance_cb)
...
bool SourceBufferRange::GetNextBuffer(scoped_refptr<StreamParserBuffer>* out_buffer)
{
    if (!HasNextBuffer())
        return false;
...
void SourceBufferRange::AppendRangeToEnd(const SourceBufferRange& range, bool transfer_current_position)
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    AppendBuffersToEnd(range.buffers_, NextRangeStartTimeForAppendRangeToEnd(range));
}

bool SourceBufferRange::CanAppendRangeToEnd(const SourceBufferRange& range) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();

    return CanAppendBuffersToEnd(range.buffers_, NextRangeStartTimeForAppendRangeToEnd(range));
}

void SourceBufferRange::AppendBuffersToEnd(const BufferQueue& new_buffers, base::TimeDelta new_buffers_group_start_pts)
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();

    CHECK(buffers_.empty() || CanAppendBuffersToEnd(new_buffers, new_buffers_group_start_pts));

    DCHECK(new_buffers_group_start_pts == kNoTimestamp || new_buffers.front()->is_key_frame())
        << range_start_pts_ << ", " << new_buffers.front()->is_key_frame();

    AdjustEstimatedDurationForNewAppend(new_buffers);

    for (BufferQueue::const_iterator itr = new_buffers.begin(); itr != new_buffers.end(); ++itr) {
        ...
        if ((*itr)->is_key_frame()) {
            keyframe_map_.insert(std::make_pair((*itr)->timestamp(), buffers_.size() - 1 + keyframe_map_index_base_));
        }
    }
...
bool SourceBufferRange::AllowableAppendAfterEstimatedDuration(const BufferQueue& buffers, base::TimeDelta new_buffers_group_start_pts) const
{
    if (buffers_.empty() || !buffers_.back()->is_duration_estimated() || buffers.empty() || !buffers.front()->is_key_frame()) {
        return false;
    }
...
bool SourceBufferRange::CanAppendBuffersToEnd(const BufferQueue& buffers, base::TimeDelta new_buffers_group_start_pts) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    if (new_buffers_group_start_pts == kNoTimestamp) {
        return buffers.front()->is_key_frame()
            ? (IsNextInPresentationSequence(buffers.front()->timestamp()) || AllowableAppendAfterEstimatedDuration(buffers, new_buffers_group_start_pts))
    ...
    return IsNextInPresentationSequence(new_buffers_group_start_pts) || AllowableAppendAfterEstimatedDuration(buffers, new_buffers_group_start_pts);
}
...
void SourceBufferRange::Seek(base::TimeDelta timestamp)
{
    ...
    CHECK_LT(next_buffer_index_, static_cast<int>(buffers_.size())) << next_buffer_index_ << ", size = " << buffers_.size();
}
...
bool SourceBufferRange::CanSeekTo(base::TimeDelta timestamp) const
{
    ...
    base::TimeDelta start_timestamp = std::max(base::TimeDelta(), GetStartTimestamp() - GetFudgeRoom());
...
int SourceBufferRange::GetConfigIdAtTime(base::TimeDelta timestamp) const
{
    ...
    CHECK_LT(buffer_index, buffers_.size()) << buffer_index << ", size = " << buffers_.size();

    return buffers_[buffer_index]->GetConfigId();
}

bool SourceBufferRange::SameConfigThruRange(base::TimeDelta start, base::TimeDelta end) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    buffer_index++;
    while (buffer_index < buffers_.size() && buffers_[buffer_index]->timestamp() <= end) {
...
std::unique_ptr<SourceBufferRange> SourceBufferRange::SplitRange(base::TimeDelta timestamp)
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    base::TimeDelta new_range_start_pts = std::max(timestamp, GetStartTimestamp());
    ...
    std::unique_ptr<SourceBufferRange> split_range
        = std::make_unique<SourceBufferRange>(gap_policy_, removed_buffers, new_range_start_pts, interbuffer_distance_cb_);
    ...
    if (next_buffer_index_ >= static_cast<int>(buffers_.size())) {
        ...
        CHECK_LE(split_range_next_buffer_index, static_cast<int>(split_range->buffers_.size()));
...
bool SourceBufferRange::TruncateAt(base::TimeDelta timestamp, BufferQueue* deleted_buffers, bool is_exclusive)
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
size_t SourceBufferRange::DeleteGOPFromFront(BufferQueue* deleted_buffers)
{
    ...
    if (next_buffer_index_ > -1) {
        ...
        CHECK_GE(next_buffer_index_, 0) << next_buffer_index_ << ", deleted " << buffers_deleted;
    }
...
size_t SourceBufferRange::GetRemovalGOP(
    base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t total_bytes_to_free, base::TimeDelta* removal_end_timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    while (gop_itr != gop_end && bytes_removed < total_bytes_to_free) {
        ...
        BufferQueue::const_iterator next_gop_start = buffers_.begin() + next_gop_index;
...
bool SourceBufferRange::FirstGOPEarlierThanMediaTime(base::TimeDelta media_time) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
base::TimeDelta SourceBufferRange::GetNextTimestamp() const
{
    ...
    CHECK(HasNextBufferPosition()) << next_buffer_index_ << ", size=" << buffers_.size();
...
bool SourceBufferRange::BelongsToRange(base::TimeDelta timestamp) const
{
    ...
    return (IsNextInPresentationSequence(timestamp) || (GetStartTimestamp() <= timestamp && timestamp <= GetEndTimestamp()));
}

base::TimeDelta SourceBufferRange::FindHighestBufferedTimestampAtOrBefore(base::TimeDelta timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    CHECK(key_iter != keyframe_map_.end(), base::NotFatalUntil::M130) << "BelongsToRange() semantics failed.";
    ...
    SourceBufferRange::BufferQueue::const_iterator search_iter = buffers_.begin() + key_index;
...
base::TimeDelta SourceBufferRange::NextKeyframeTimestamp(base::TimeDelta timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    // specified timestamp.
    if (itr == keyframe_map_.begin() && timestamp > range_start_pts_ && timestamp < itr->first) {
        return timestamp;
    }
...
base::TimeDelta SourceBufferRange::KeyframeBeforeTimestamp(base::TimeDelta timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
bool SourceBufferRange::GetBuffersInRange(base::TimeDelta start, base::TimeDelta end, BufferQueue* buffers) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    for (BufferQueue::const_iterator it = GetBufferItrAt(first_timestamp, false); it != buffers_.end(); ++it) {
...
void SourceBufferRange::AdjustEstimatedDurationForNewAppend(const BufferQueue& new_buffers)
{
    ...
    if (last_appended_buffer->is_duration_estimated()) {
        ...
        if (last_appended_buffer->duration() != timestamp_delta) {
            DVLOG(1) << "Replacing estimated duration (" << last_appended_buffer->duration() << ") from previous range-end with derived duration ("
                     << timestamp_delta << ").";
...
void SourceBufferRange::FreeBufferRange(const BufferQueue::const_iterator& starting_point, const BufferQueue::const_iterator& ending_point)
{
    for (BufferQueue::const_iterator itr = starting_point; itr != ending_point; ++itr) {
...
void SourceBufferRange::UpdateEndTime(scoped_refptr<StreamParserBuffer> new_buffer)
{
    base::TimeDelta timestamp = new_buffer->timestamp();
    base::TimeDelta duration = new_buffer->duration();
    DVLOG(1) << __func__ << " timestamp=" << timestamp << ", duration=" << duration;
    DCHECK_NE(timestamp, kNoTimestamp);
    DCHECK_GE(timestamp, base::TimeDelta());
    DCHECK_GE(duration, base::TimeDelta());

    if (!highest_frame_) {
        DVLOG(1) << "Updating range end time from <empty> to " << timestamp.InMicroseconds() << "us, " << (timestamp + duration).InMicroseconds() << "us";
        highest_frame_ = std::move(new_buffer);
        return;
    }

    if (highest_frame_->timestamp() < timestamp || (highest_frame_->timestamp() == timestamp && highest_frame_->duration() <= duration)) {
        DVLOG(1) << "Updating range end time from " << highest_frame_->timestamp().InMicroseconds() << "us, "
                 << (highest_frame_->timestamp() + highest_frame_->duration()).InMicroseconds() << "us to " << timestamp.InMicroseconds() << "us, "
                 << (timestamp + duration).InMicroseconds();
        highest_frame_ = std::move(new_buffer);
    }
}

bool SourceBufferRange::IsNextInPresentationSequence(base::TimeDelta timestamp) const
{
    DCHECK_NE(timestamp, kNoTimestamp);
    CHECK(!buffers_.empty());
    ...
    return (
        highest_timestamp == timestamp || (highest_timestamp < timestamp && (gap_policy_ == ALLOW_GAPS || timestamp <= highest_timestamp + GetFudgeRoom())));
}

bool SourceBufferRange::IsNextInDecodeSequence(DecodeTimestamp decode_timestamp) const
{
    CHECK(!buffers_.empty());
    DecodeTimestamp end = buffers_.back()->GetDecodeTimestamp();
    return (end == decode_timestamp || (end < decode_timestamp && (gap_policy_ == ALLOW_GAPS || decode_timestamp <= end + GetFudgeRoom())));
}

base::TimeDelta SourceBufferRange::NextRangeStartTimeForAppendRangeToEnd(const SourceBufferRange& range) const
{
    DCHECK(!buffers_.empty());
    DCHECK(!range.buffers_.empty());

    base::TimeDelta next_range_first_buffer_time = range.buffers_.front()->timestamp();
...
size_t SourceBufferRange::GetBufferIndexAt(base::TimeDelta timestamp, bool skip_given_timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    SourceBufferRange::BufferQueue::const_iterator search_iter = buffers_.begin() + search_index;
    gop_iter++;

    SourceBufferRange::BufferQueue::const_iterator next_gop_start
        = gop_iter == keyframe_map_.end() ? buffers_.end() : buffers_.begin() + (gop_iter->second - keyframe_map_index_base_);

    while (search_iter != next_gop_start) {
        if (((*search_iter)->timestamp() > timestamp) || (!skip_given_timestamp && (*search_iter)->timestamp() == timestamp)) {
            break;
        }
        search_index++;
        search_iter++;
...
SourceBufferRange::BufferQueue::const_iterator SourceBufferRange::GetBufferItrAt(base::TimeDelta timestamp, bool skip_given_timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
SourceBufferRange::KeyframeMap::const_iterator SourceBufferRange::GetFirstKeyframeAt(base::TimeDelta timestamp, bool skip_given_timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
SourceBufferRange::KeyframeMap::const_iterator SourceBufferRange::GetFirstKeyframeAtOrBefore(base::TimeDelta timestamp) const
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
...
bool SourceBufferRange::TruncateAt(const size_t starting_point, BufferQueue* deleted_buffers)
{
    DVLOG(1) << __func__;
    DVLOG(4) << ToStringForDebugging();
    ...
    if (HasNextBufferPosition()) {
        if (static_cast<size_t>(next_buffer_index_) >= starting_point) {
            if (HasNextBuffer() && deleted_buffers) {
                BufferQueue saved(buffers_.begin() + next_buffer_index_, buffers_.end());
    ...
    const BufferQueue::const_iterator starting_point_iter = buffers_.begin() + starting_point;

    // Remove keyframes from |starting_point| onward.
    KeyframeMap::const_iterator starting_point_keyframe = keyframe_map_.lower_bound((*starting_point_iter)->timestamp());
...
void SourceBufferRange::UpdateEndTimeUsingLastGOP()
{
    ...
    for (BufferQueue::const_iterator buffer_itr = buffers_.begin() + (last_gop->second - keyframe_map_index_base_); buffer_itr != buffers_.end();
         ++buffer_itr) {
        UpdateEndTime(*buffer_itr);
    }

    DVLOG(1) << __func__ << " Updated range end time to " << highest_frame_->timestamp() << ", " << highest_frame_->timestamp() + highest_frame_->duration();
}
...
std::string SourceBufferRange::ToStringForDebugging() const
{
    ...
    result << "keyframe_map_index_base_=" << keyframe_map_index_base_ << ", buffers.size()=" << buffers_.size()
           << ", keyframe_map_.size()=" << keyframe_map_.size() << ", keyframe_map_:\n";
    for (const auto& [time_delta, idx] : keyframe_map_) {
        result << "\t pts " << time_delta.InMicroseconds() << ", unadjusted idx = " << idx << "\n";
    }
