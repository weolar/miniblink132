...
class AudioTimestampValidatorTest : public testing::Test, public ::testing::WithParamInterface<ValidatorTestParams> {
public:
...
TEST_P(AudioTimestampValidatorTest, WarnForEraticTimes)
{
    ...
    decoder_config.Initialize(
        kCodec, kSampleFormat, kChannelLayout, kSamplesPerSecond, EmptyExtraData(), EncryptionScheme::kUnencrypted, kSeekPreroll, codec_delay_);
    ...
    const base::TimeDelta kRandomOffsets[] = { base::Milliseconds(100), base::Milliseconds(350) };
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        base::TimeDelta randomOffset = kRandomOffsets[i % std::size(kRandomOffsets)];
        encoded_buffer->set_timestamp(i * kBufferDuration + randomOffset);

        if (i == 0) {
            encoded_buffer->set_discard_padding(std::make_pair(front_discard_, base::TimeDelta()));
        }
        ...
        if (i >= output_delay_) {
            ...
            scoped_refptr<AudioBuffer> decoded_buffer
                = MakeAudioBuffer<float>(kSampleFormat, kChannelLayout, kChannelCount, kSamplesPerSecond, 1.0f, 0.0f, kFramesPerBuffer, i * kBufferDuration);
...
TEST_P(AudioTimestampValidatorTest, RepeatedWarnForSlowAccumulatingDrift)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (offset > base::Milliseconds(50)) {
            EXPECT_LIMITED_MEDIA_LOG(HasSubstr("timestamp gap detected"), num_timestamp_gap_warnings, kMaxTimestampGapWarnings);
        }
...
INSTANTIATE_TEST_SUITE_P(All, AudioTimestampValidatorTest,
