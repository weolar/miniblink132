...
public:
    DecryptingMediaResourceTest()
    {
        ...
        encrypted_buffer_->set_decrypt_config(DecryptConfig::CreateCencConfig(kFakeKeyId, kFakeIv, {}));
        ...
        EXPECT_CALL(demuxer_, GetAllStreams()).WillRepeatedly(Invoke(this, &DecryptingMediaResourceTest::GetAllStreams));

        decrypting_media_resource_
            = std::make_unique<DecryptingMediaResource>(&demuxer_, &cdm_context_, &null_media_log_, task_environment_.GetMainThreadTaskRunner());
    }
    ...
    {
        for (media::DemuxerStream* stream : decrypting_media_resource_->GetAllStreams()) {
...
protected:
    ...
    base::MockCallback<DecryptingMediaResource::InitCB> decrypting_media_resource_init_cb_;
...
TEST_F(DecryptingMediaResourceTest, ClearStreams)
{
    ...
    decrypting_media_resource_->Initialize(decrypting_media_resource_init_cb_.Get(), waiting_cb_.Get());
    task_environment_.RunUntilIdle();

    EXPECT_EQ(decrypting_media_resource_->DecryptingDemuxerStreamCountForTesting(), 2);
...
TEST_F(DecryptingMediaResourceTest, OneDecryptingDemuxerStreamFailsInitialization)
{
    AddStream(DemuxerStream::AUDIO, /* encrypted = */ false);
    AddStream(DemuxerStream::VIDEO, /* encrypted = */ true);
    ...
    EXPECT_CALL(cdm_context_, GetDecryptor()).WillOnce(Return(nullptr)).WillRepeatedly(Return(&decryptor_));
...
TEST_F(DecryptingMediaResourceTest, BothDecryptingDemuxerStreamsFailInitialization)
{
    AddStream(DemuxerStream::AUDIO, /* encrypted = */ false);
    AddStream(DemuxerStream::VIDEO, /* encrypted = */ true);
...
TEST_F(DecryptingMediaResourceTest, WaitingCallback)
{
    ...
    EXPECT_CALL(*streams_.front(), OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(decryptor_, Decrypt(_, encrypted_buffer_, _))
        .WillRepeatedly(base::test::RunOnceCallbackRepeatedly<2>(Decryptor::kNoKey, scoped_refptr<DecoderBuffer>()));
    EXPECT_CALL(decrypting_media_resource_init_cb_, Run(true));
    EXPECT_CALL(waiting_cb_, Run(WaitingReason::kNoDecryptionKey));

    decrypting_media_resource_->Initialize(decrypting_media_resource_init_cb_.Get(), waiting_cb_.Get());
    decrypting_media_resource_->GetAllStreams().front()->Read(1, base::BindOnce(&DecryptingMediaResourceTest::BufferReady, base::Unretained(this)));
    task_environment_.RunUntilIdle();
}
