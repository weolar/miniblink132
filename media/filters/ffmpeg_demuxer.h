...
public:
    ...
    static std::unique_ptr<FFmpegDemuxerStream> Create(FFmpegDemuxer* demuxer, AVStream* stream, MediaLog* media_log);
    ...
    void enable_chained_ogg_fixups()
    {
        fixup_chained_ogg_ = true;
    }
    ...
    void set_start_time(base::TimeDelta time)
    {
        start_time_ = time;
    }
...
private:
    ...
    FFmpegDemuxerStream(FFmpegDemuxer* demuxer, AVStream* stream, std::unique_ptr<AudioDecoderConfig> audio_config,
    ...
    static base::TimeDelta ConvertStreamTimestamp(const AVRational& time_base, int64_t timestamp);
...
public:
    FFmpegDemuxer(const scoped_refptr<base::SequencedTaskRunner>& task_runner, DataSource* data_source,
    ...
    void OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::string& encryption_key_id);
    ...
    {
        return glue_ ? glue_->container() : container_names::MediaContainerName::kContainerUnknown;
    }
...
private:
    ...
    FFmpegDemuxerStream* GetFirstEnabledFFmpegStream(DemuxerStream::Type type) const;

    void SetLiveness(StreamLiveness liveness);

    void SeekInternal(base::TimeDelta time, base::OnceCallback<void(int)> seek_cb);
    void OnVideoSeekedForTrackChange(DemuxerStream* video_stream, base::OnceClosure seek_completed_cb, int result);
    void SeekOnVideoTrackChange(base::TimeDelta seek_to_time, TrackChangeCB seek_completed_cb, const std::vector<DemuxerStream*>& streams);
    ...
    base::flat_map<MediaTrack::Id, raw_ptr<FFmpegDemuxerStream, CtnExperimental>> track_id_to_demux_stream_map_;
