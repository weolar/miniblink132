...
DecoderStatus IsConfigSupported(DecoderCapability capability, bool is_encrypted)
{
    switch (capability) {
    case kAlwaysFail:
        return DecoderStatus::Codes::kFailed;
    case kClearOnly:
        return is_encrypted ? DecoderStatus::Codes::kUnsupportedEncryptionMode : DecoderStatus::Codes::kOk;
    case kEncryptedOnly:
        return is_encrypted ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kUnsupportedEncryptionMode;
    case kAlwaysSucceed:
...
public:
    ...
    {
        return std::make_unique<StreamTraits>(media_log, CHANNEL_LAYOUT_STEREO, kSampleFormatPlanarF32);
    }

    static void UseNormalClearDecoderConfig(StrictMock<MockDemuxerStream>& stream)
    {
        stream.set_audio_decoder_config(TestAudioConfig::Normal());
    }
    static void UseNormalEncryptedDecoderConfig(StrictMock<MockDemuxerStream>& stream)
    {
    ...
    static void ExpectInitialize(MockDecoder* decoder, DecoderCapability capability)
    {
        EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _))
            .WillRepeatedly([capability](const AudioDecoderConfig& config, CdmContext*, AudioDecoder::InitCB& init_cb, const AudioDecoder::OutputCB&,
                                const WaitingCB&) { std::move(init_cb).Run(IsConfigSupported(capability, config.is_encrypted())); });
    }
...
public:
    ...
    static void ExpectInitialize(MockDecoder* decoder, DecoderCapability capability)
    {
        EXPECT_CALL(*decoder, Initialize_(_, _, _, _, _, _))
            .WillRepeatedly(
                [capability](const VideoDecoderConfig& config, bool low_delay, CdmContext*, VideoDecoder::InitCB& init_cb, const VideoDecoder::OutputCB&,
                    const WaitingCB&) { std::move(init_cb).Run(IsConfigSupported(capability, config.is_encrypted())); });
    }
...
template <typename TypeParam> class DecoderSelectorTest : public ::testing::Test {
public:
    // Convenience aliases.
    ...
    struct MockDecoderArgs {
        static MockDecoderArgs Create(int decoder_id, DecoderCapability capability)
        {
            MockDecoderArgs result;
            result.decoder_id = decoder_id;
            result.capability = capability;
            result.supports_decryption = DecoderCapabilitySupportsDecryption(capability);
    ...
    void OnOutput(scoped_refptr<Output> output)
    {
        NOTREACHED();
    }
    ...
    MOCK_METHOD1_T(OnDemuxerStreamSelected, void(std::unique_ptr<DecryptingDemuxerStream>));

    void OnDecoderSelectedThunk(typename Selector::DecoderOrError decoder, std::unique_ptr<DecryptingDemuxerStream> decrypting_demuxer_stream)
    {
        ...
        if (decoder.has_value() && decoder->GetDecoderType() == DecoderType::kTesting) {
            OnDecoderSelected(static_cast<MockDecoder*>(std::move(decoder).value().get())->GetDecoderId());
    ...
    std::vector<std::unique_ptr<Decoder>> CreateDecoders()
    {
        ...
        if (use_decrypting_decoder_) {
            decoders.push_back(std::make_unique<typename TypeParam::DecryptingDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_));
        }
#endif // !BUILDFLAG(IS_ANDROID)

        for (const auto& args : mock_decoders_to_create_) {
            std::unique_ptr<StrictMock<MockDecoder>> decoder
                = std::make_unique<StrictMock<MockDecoder>>(args.is_platform_decoder, args.supports_decryption, args.decoder_id);
    ...
    void CreateCdmContext(DecryptorCapability capability)
    {
        ...
        case DemuxerStream::AUDIO:
            EXPECT_CALL(*decryptor_, InitializeAudioDecoder(_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<1>(capability == kDecryptAndDecode));
            break;
        case DemuxerStream::VIDEO:
            EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<1>(capability == kDecryptAndDecode));
            break;
        default:
            NOTREACHED();
        }
    ...
    {
        decoder_selector_ = std::make_unique<Selector>(task_environment_.GetMainThreadTaskRunner(),
            base::BindRepeating(&Self::CreateDecoders, base::Unretained(this)), &media_log_, /*enable_priority_based_selection=*/true);
        decoder_selector_->Initialize(traits_.get(), &demuxer_stream_, cdm_context_.get(), base::BindRepeating(&Self::OnWaiting, base::Unretained(this)));
    }
    ...
    {
        if (is_selecting_) {
            decoder_selector_->ResumeDecoderSelection(base::BindOnce(&Self::OnDecoderSelectedThunk, base::Unretained(this)),
                base::BindRepeating(&Self::OnOutput, base::Unretained(this)), DecoderStatus::Codes::kFailed);
        } else {
            decoder_selector_->BeginDecoderSelection(
                base::BindOnce(&Self::OnDecoderSelectedThunk, base::Unretained(this)), base::BindRepeating(&Self::OnOutput, base::Unretained(this)));
        }
        is_selecting_ = true;
        RunUntilIdle();
    }
...
using VideoDecoderSelectorTest = DecoderSelectorTest<VideoDecoderSelectorTestParam>;

using DecoderSelectorTestParams = ::testing::Types<AudioDecoderSelectorTestParam, VideoDecoderSelectorTestParam>;
...
TEST_F(VideoDecoderSelectorTest, ClearStream_PrioritizeSoftwareDecoders)
{
    ...
    this->demuxer_stream_.set_video_decoder_config(TestVideoConfig::Custom(gfx::Size(64, 64)));
    this->CreateDecoderSelector();
...
TEST_F(VideoDecoderSelectorTest, ClearStream_PrioritizePlatformDecoders)
{
    ...
    // on any platform.
    this->demuxer_stream_.set_video_decoder_config(TestVideoConfig::Custom(gfx::Size(4096, 4096)));
    this->CreateDecoderSelector();
...
TYPED_TEST(DecoderSelectorTest, EncryptedStream_NoDecryptor_DecodersNotInitialized)
{
    using MockDecoderArgs = typename DecoderSelectorTest<TypeParam>::MockDecoderArgs;
...
TYPED_TEST(DecoderSelectorTest, EncryptedStream_NoDecryptor_FinalizeDecoderSelection)
{
    this->AddMockDecoder(kDecoder1, kEncryptedOnly);
    this->AddMockDecoder(kDecoder2, kEncryptedOnly);
    this->CreateCdmContext(kNoDecryptor);
...
TYPED_TEST(DecoderSelectorTest, EncryptedStream_DecryptOnly_FinalizeDecoderSelection)
{
    this->AddMockDecoder(kDecoder1, kClearOnly);
    this->AddMockDecoder(kDecoder2, kClearOnly);
    this->CreateCdmContext(kDecryptOnly);
    ...
    EXPECT_CALL(*this, OnDecoderSelected(kDecoder1));
    EXPECT_CALL(*this, OnDemuxerStreamSelected(NotNull())).WillOnce([&](std::unique_ptr<DecryptingDemuxerStream> dds) { saved_dds = std::move(dds); });
...
TYPED_TEST(DecoderSelectorTest, EncryptedStream_DecryptAndDecode_ExternalFallback)
{
    this->AddDecryptingDecoder();
    this->AddMockDecoder(kDecoder1, kClearOnly);
    this->AddMockDecoder(kDecoder2, kClearOnly);
...
TEST_F(VideoDecoderSelectorTest, EncryptedStream_PrioritizeSoftwareDecoders)
{
    ...
    // prioritized on any platform.
    this->demuxer_stream_.set_video_decoder_config(TestVideoConfig::CustomEncrypted(gfx::Size(64, 64)));
    this->CreateDecoderSelector();
...
TEST_F(VideoDecoderSelectorTest, EncryptedStream_PrioritizePlatformDecoders)
{
    ...
    // prioritized on any platform.
    this->demuxer_stream_.set_video_decoder_config(TestVideoConfig::CustomEncrypted(gfx::Size(4096, 4096)));
    this->CreateDecoderSelector();
