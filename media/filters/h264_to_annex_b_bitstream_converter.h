...
public:
    ...
    H264ToAnnexBBitstreamConverter(const H264ToAnnexBBitstreamConverter&) = delete;
    H264ToAnnexBBitstreamConverter& operator=(const H264ToAnnexBBitstreamConverter&) = delete;
    ...
    bool ParseConfiguration(const uint8_t* configuration_record, int configuration_record_size, mp4::AVCDecoderConfigurationRecord* avc_config);

    // Returns the buffer size needed to store the parameter sets in |avc_config|
    // in Annex B form.
    uint32_t GetConfigSize(const mp4::AVCDecoderConfigurationRecord& avc_config) const;
    ...
    uint32_t CalculateNeededOutputBufferSize(const uint8_t* input, uint32_t input_size, const mp4::AVCDecoderConfigurationRecord* avc_config) const;
    ...
    //          of converted data)
    bool ConvertAVCDecoderConfigToByteStream(const mp4::AVCDecoderConfigurationRecord& avc_config, uint8_t* output, uint32_t* output_size);
