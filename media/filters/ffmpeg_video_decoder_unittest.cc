...
public:
        ...
        : decoder_(std::make_unique<FFmpegVideoDecoder>(&media_log_))
    {
        ...
        memset(corrupt_i_frame_buffer_->writable_data(), 0, corrupt_i_frame_buffer_->size() / 2);
    }
    ...
    void InitializeWithConfigWithResult(const VideoDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, false, nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&FFmpegVideoDecoderTest::FrameReady, base::Unretained(this)), base::NullCallback());
    ...
    void DecodeIFrameThenTestFile(const std::string& test_file_name, int expected_width, int expected_height, size_t expected_frames = 2u)
    {
        Initialize();
        ...
        EXPECT_EQ(expected_width, output_frames_[1]->visible_rect().size().width());
        EXPECT_EQ(expected_height, output_frames_[1]->visible_rect().size().height());
    }
    ...
    DecoderStatus Decode(scoped_refptr<DecoderBuffer> buffer)
    {
        ...
        decoder_->Decode(buffer, base::BindOnce(&FFmpegVideoDecoderTest::DecodeDone, base::Unretained(this)));
...
TEST_F(FFmpegVideoDecoderTest, Initialize_OpenDecoderFails)
{
    ...
    VideoDecoderConfig config(VideoCodec::kTheora, VIDEO_CODEC_PROFILE_UNKNOWN, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation,
...
TEST_F(FFmpegVideoDecoderTest, DecodeFrame_DecodeErrorAtEndOfStream)
{
    ...
    EXPECT_THAT(DecodeSingleFrame(corrupt_i_frame_buffer_), IsDecodeErrorStatus());
}
