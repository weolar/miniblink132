...
constexpr base::TimeDelta kStartingCapacityForEncrypted = base::Milliseconds(500);

AudioRendererAlgorithm::AudioRendererAlgorithm(MediaLog* media_log)
    : AudioRendererAlgorithm(media_log, { kMaxCapacity, kStartingCapacity, kStartingCapacityForEncrypted })
{
}

AudioRendererAlgorithm::AudioRendererAlgorithm(MediaLog* media_log, AudioRendererAlgorithmParameters params)
...
void AudioRendererAlgorithm::Initialize(const AudioParameters& params, bool is_encrypted)
{
    ...
    initial_capacity_ = capacity_ = playback_threshold_ = std::max(min_playback_threshold_,
        AudioTimestampHelper::TimeToFrames(
            is_encrypted ? audio_renderer_algorithm_params_.starting_capacity_for_encrypted : audio_renderer_algorithm_params_.starting_capacity,
            samples_per_second_));
    max_capacity_ = std::max(initial_capacity_, AudioTimestampHelper::TimeToFrames(audio_renderer_algorithm_params_.max_capacity, samples_per_second_));
    num_candidate_blocks_ = AudioTimestampHelper::TimeToFrames(kWsolaSearchInterval, samples_per_second_);
    ola_window_size_ = AudioTimestampHelper::TimeToFrames(kOlaWindowSize, samples_per_second_);
...
void AudioRendererAlgorithm::OnResamplerRead(int frame_delay, AudioBus* audio_bus)
{
...
int AudioRendererAlgorithm::ResampleAndFill(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate)
{
    if (!resampler_) {
        resampler_ = std::make_unique<MultiChannelResampler>(channels_, playback_rate, SincResampler::kDefaultRequestSize,
            base::BindRepeating(&AudioRendererAlgorithm::OnResamplerRead, base::Unretained(this)));
        resampler_->PrimeWithSilence();
    }

    if (reached_end_of_stream_ && resampler_only_has_silence_ && !audio_buffer_.frames()) {
    ...
    if (!reached_end_of_stream_ && audio_buffer_.frames() < resampler_->GetMaxInputFramesRequested(requested_frames)) {
    ...
    std::unique_ptr<AudioBus> resampler_output = AudioBus::Create(channels_, requested_frames);
...
int AudioRendererAlgorithm::RunWsolaAndFill(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate)
{
    ...
    if (!ola_window_) {
        ...
        internal::GetPeriodicHanningWindow(2 * ola_window_size_, transition_window_.get());
        ...
        search_block_ = AudioBus::Create(channels_, num_candidate_blocks_ + (ola_window_size_ - 1));
    ...
    do {
        rendered_frames += WriteCompletedFramesTo(requested_frames - rendered_frames, dest_offset + rendered_frames, dest);
    ...
    if (rendered_frames > 0) {
        effective_playback_rate_ = (initial_input_frames - audio_buffer_.frames()) / static_cast<double>(rendered_frames);
    } else {
...
int AudioRendererAlgorithm::FillBuffer(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate)
{
    if (playback_rate == 0) {
        return 0;
    }
    ...
    case FillBufferMode::kWSOLA:
        return RunWsolaAndFill(dest, dest_offset, requested_frames, playback_rate);
    }
}

AudioRendererAlgorithm::FillBufferMode AudioRendererAlgorithm::ChooseBufferMode(double playback_rate)
{
    ...
    const bool is_playback_rate_almost_one = ola_window_size_ <= faster_step && slower_step >= ola_window_size_;
...
void AudioRendererAlgorithm::EnqueueBuffer(scoped_refptr<AudioBuffer> buffer_in)
{
    DCHECK(!buffer_in->end_of_stream());
    audio_buffer_.Append(std::move(buffer_in));
}

void AudioRendererAlgorithm::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
    ...
    if (!latency_hint) {
        ...
        MEDIA_LOG(DEBUG, media_log_) << "Audio latency hint cleared. Default buffer size ("
                                     << AudioTimestampHelper::FramesToTime(playback_threshold_, samples_per_second_) << ") restored";
        return;
    }

    int latency_hint_frames = AudioTimestampHelper::TimeToFrames(*latency_hint_, samples_per_second_);
    ...
    MEDIA_LOG(DEBUG, media_log_) << "Audio latency hint set:" << *latency_hint << ". "
                                 << "Effective buffering latency:" << AudioTimestampHelper::FramesToTime(playback_threshold_, samples_per_second_)
                                 << clamp_string;
...
{
    return audio_buffer_.frames() + (resampler_ ? static_cast<int>(resampler_->BufferedFrames()) : 0);
}
...
bool AudioRendererAlgorithm::RunOneWsolaIteration(double playback_rate)
{
    ...
    for (int k = 0; k < channels_; ++k) {
        ...
        memcpy(&ch_output[ola_hop_size_], &ch_opt_frame[ola_hop_size_], sizeof(*ch_opt_frame) * ola_hop_size_);
    }
...
void AudioRendererAlgorithm::UpdateOutputTime(double playback_rate, double time_change)
{
    output_time_ += time_change;
    // Center of the search region, in frames.
    const int search_block_center_index = static_cast<int>(output_time_ * playback_rate + 0.5);
...
{
    const int earliest_used_index = std::min(target_block_index_, search_block_index_);
...
int AudioRendererAlgorithm::WriteCompletedFramesTo(int requested_frames, int dest_offset, AudioBus* dest)
{
...
void AudioRendererAlgorithm::GetOptimalBlock()
{
    ...
    if (TargetIsWithinSearchRegion()) {
        ...
        internal::Interval exclude_interval = std::make_pair(last_optimal - kExcludeIntervalLengthFrames / 2, last_optimal + kExcludeIntervalLengthFrames / 2);

        // |optimal_index| is in frames and it is relative to the beginning of the
        // |search_block_|.
        optimal_index = internal::OptimalIndex(search_block_wrapper_.get(), target_block_wrapper_.get(), exclude_interval);
...
void AudioRendererAlgorithm::PeekAudioWithZeroPrepend(int read_offset_frames, AudioBus* dest)
{
    ...
    if (read_offset_frames < 0) {
        int num_zero_frames_appended = std::min(-read_offset_frames, num_frames_to_read);
    ...
    audio_buffer_.PeekFrames(num_frames_to_read, read_offset_frames, write_offset, dest);
}
