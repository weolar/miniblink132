...
public:
    ...
    explicit Dav1dVideoDecoder(std::unique_ptr<MediaLog> media_log, OffloadState offload_state = OffloadState::kNormal);
...
private:
    enum class DecoderState { kUninitialized, kNormal, kFlushCodec, kDecodeFinished, kError };
...
public:
    explicit OffloadingDav1dVideoDecoder(std::unique_ptr<MediaLog> media_log)
        : OffloadingVideoDecoder(0, std::vector<VideoCodec>(1, VideoCodec::kAV1),
            std::make_unique<Dav1dVideoDecoder>(std::move(media_log), OffloadableVideoDecoder::OffloadState::kOffloaded))
    {
    }
};
