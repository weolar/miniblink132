...
static bool IsAccessUnitBoundaryNal(int nal_unit_type)
{
    ...
    // packet type.
    if (nal_unit_type == media::H265NALU::VPS_NUT || nal_unit_type == media::H265NALU::SPS_NUT || nal_unit_type == media::H265NALU::PPS_NUT
        || nal_unit_type == media::H265NALU::AUD_NUT || nal_unit_type == media::H265NALU::PREFIX_SEI_NUT
        || (nal_unit_type >= media::H265NALU::RSV_NVCL41 && nal_unit_type <= media::H265NALU::RSV_NVCL44)
        || (nal_unit_type >= media::H265NALU::UNSPEC48 && nal_unit_type <= media::H265NALU::UNSPEC55)) {
        return true;
    }
    return false;
}
...
bool H265ToAnnexBBitstreamConverter::ParseConfiguration(
    const uint8_t* configuration_record, int configuration_record_size, mp4::HEVCDecoderConfigurationRecord* hevc_config)
{
...
uint32_t H265ToAnnexBBitstreamConverter::GetConfigSize(const mp4::HEVCDecoderConfigurationRecord& hevc_config) const
{
...
uint32_t H265ToAnnexBBitstreamConverter::CalculateNeededOutputBufferSize(
    const uint8_t* input, uint32_t input_size, const mp4::HEVCDecoderConfigurationRecord* hevc_config) const
{
    ...
    while (data_left > 0) {
        ...
        for (nal_unit_length = 0, size_of_len_field = nal_unit_length_field_width_; size_of_len_field > 0; input++, size_of_len_field--, data_left--) {
            nal_unit_length <<= 8;
...
bool H265ToAnnexBBitstreamConverter::ConvertHEVCDecoderConfigToByteStream(
    const mp4::HEVCDecoderConfigurationRecord& hevc_config, uint8_t* output, uint32_t* output_size)
{
    uint8_t* out = output;
...
bool H265ToAnnexBBitstreamConverter::ConvertNalUnitStreamToByteStream(
    const uint8_t* input, uint32_t input_size, const mp4::HEVCDecoderConfigurationRecord* hevc_config, uint8_t* output, uint32_t* output_size)
{
    ...
    while (data_left > 0) {
        ...
        for (nal_unit_length = 0, i = nal_unit_length_field_width_; i > 0 && data_left > 0; inscan++, i--, data_left--) {
            nal_unit_length <<= 8;
        ...
        if (hevc_config && (nal_unit_type != H265NALU::AUD_NUT || nal_unit_count > 1)) {
            ...
            if (!ConvertHEVCDecoderConfigToByteStream(*hevc_config, outscan, &config_size)) {
...
bool H265ToAnnexBBitstreamConverter::WriteParamSet(const std::vector<uint8_t>& param_set, uint8_t** out, uint32_t* out_size) const
{
