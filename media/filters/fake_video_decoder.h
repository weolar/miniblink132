...
public:
    ...
    FakeVideoDecoder(int decoder_id, int decoding_delay, int max_parallel_decoding_requests, const BytesDecodedCB& bytes_decoded_cb);
    ...
    void enable_eliding_eos()
    {
        enable_eliding_eos_ = true;
    }
    ...
    int total_bytes_decoded() const
    {
        return total_bytes_decoded_;
    }

    int total_decoded_frames() const
    {
        return total_decoded_frames_;
    }

    auto eos_next_configs() const
    {
        return eos_next_configs_;
    }
...
protected:
    ...
    void OnFrameDecoded(int buffer_size, DecodeCB decode_cb, DecoderStatus status);
