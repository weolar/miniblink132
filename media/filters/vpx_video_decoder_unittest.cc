...
public:
    ...
    void InitializeWithConfigWithResult(const VideoDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, false, nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&VpxVideoDecoderTest::FrameReady, base::Unretained(this)), base::NullCallback());
    ...
    {
        for (auto iter = input_buffers.begin(); iter != input_buffers.end(); ++iter) {
    ...
    void DecodeIFrameThenTestFile(const std::string& test_file_name, const gfx::Size& expected_size)
    {
        Initialize();
        ...
        EXPECT_EQ(original_size.width(), output_frames_[0]->visible_rect().size().width());
        EXPECT_EQ(original_size.height(), output_frames_[0]->visible_rect().size().height());
        EXPECT_EQ(expected_size.width(), output_frames_[1]->visible_rect().size().width());
        EXPECT_EQ(expected_size.height(), output_frames_[1]->visible_rect().size().height());
    }
    ...
    DecoderStatus Decode(scoped_refptr<DecoderBuffer> buffer)
    {
        ...
        decoder_->Decode(std::move(buffer), base::BindOnce(&VpxVideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
...
{
    Initialize();
    static_cast<VpxVideoDecoder*>(decoder_.get())->force_allocation_error_for_testing();
...
{
    scoped_refptr<DecoderBuffer> large_frame = ReadTestDataFile("vp9-I-frame-1280x720");
...
TEST_F(VpxVideoDecoderTest, FrameValidAfterPoolDestruction)
{
    ...
    memset(output_frames_.front()->writable_data(VideoFrame::Plane::kY), 0xff,
        output_frames_.front()->rows(VideoFrame::Plane::kY) * output_frames_.front()->stride(VideoFrame::Plane::kY));
}
...
TEST_F(VpxVideoDecoderTest, MemoryPoolAllowsMultipleDisplay)
{
    ...
    scoped_refptr<DecoderBuffer> data = ReadTestDataFile("vp9-duplicate-frame.webm");
    ...
    while (av_read_frame(glue.format_context(), packet.get()) >= 0) {
        DecoderStatus decode_status = Decode(DecoderBuffer::CopyFrom(AVPacketData(*packet)));
    ...
    EXPECT_EQ(last_frame->data(VideoFrame::Plane::kY), dupe_frame->data(VideoFrame::Plane::kY));
    EXPECT_EQ(last_frame->data(VideoFrame::Plane::kU), dupe_frame->data(VideoFrame::Plane::kU));
    EXPECT_EQ(last_frame->data(VideoFrame::Plane::kV), dupe_frame->data(VideoFrame::Plane::kV));
    ...
    memset(last_frame->writable_data(VideoFrame::Plane::kY), 0, last_frame->row_bytes(VideoFrame::Plane::kY));
}
