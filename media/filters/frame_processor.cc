...
public:
    MseTrackBuffer(ChunkDemuxerStream* stream, MediaLog* media_log, SourceBufferParseWarningCB parse_warning_cb);
    ...
    ChunkDemuxerStream* stream() const
    {
        return stream_;
    }
    ...
    void NotifyStartOfCodedFrameGroup(DecodeTimestamp start_dts, base::TimeDelta start_pts);
...
MseTrackBuffer::MseTrackBuffer(ChunkDemuxerStream* stream, MediaLog* media_log, SourceBufferParseWarningCB parse_warning_cb)
...
void MseTrackBuffer::SetHighestPresentationTimestampIfIncreased(base::TimeDelta timestamp)
{
...
bool MseTrackBuffer::EnqueueProcessedFrame(scoped_refptr<StreamParserBuffer> frame)
{
    if (frame->is_key_frame()) {
        ...
        if (frame->timestamp() < last_keyframe_presentation_timestamp_) {
            if (!num_keyframe_time_greater_than_dependant_warnings_) {
                ...
                DCHECK(parse_warning_cb_);
                parse_warning_cb_.Run(SourceBufferParseWarning::kKeyframeTimeGreaterThanDependant);
            }

            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_keyframe_time_greater_than_dependant_warnings_, kMaxNumKeyframeTimeGreaterThanDependantWarnings)
                << "Warning: presentation time of most recently processed random "
                   "access point ("
                << last_keyframe_presentation_timestamp_ << ") is later than the presentation time of a non-keyframe (" << frame->timestamp()
    ...
    DCHECK(pending_group_start_pts_ == kNoTimestamp || pending_group_start_pts_ <= frame->timestamp());
    pending_group_start_pts_ = kNoTimestamp;
...
bool MseTrackBuffer::FlushProcessedFrames()
{
    ...
    DVLOG_IF(3, !result) << __func__ << "(): Failure appending processed frames to stream";

    return result;
}

void MseTrackBuffer::NotifyStartOfCodedFrameGroup(DecodeTimestamp start_dts, base::TimeDelta start_pts)
{
    last_keyframe_presentation_timestamp_ = kNoTimestamp;
...
FrameProcessor::FrameProcessor(UpdateDurationCB update_duration_cb, MediaLog* media_log)
...
void FrameProcessor::SetParseWarningCallback(SourceBufferParseWarningCB parse_warning_cb)
{
    DCHECK(!parse_warning_cb_);
...
bool FrameProcessor::ProcessFrames(const StreamParser::BufferQueueMap& buffer_queue_map, base::TimeDelta append_window_start, base::TimeDelta append_window_end,
    base::TimeDelta* timestamp_offset)
{
    ...
    if (sequence_mode_ && track_buffers_.size() > 1) {
        ...
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_muxed_sequence_mode_warnings_, kMaxMuxedSequenceModeWarnings)
    ...
    for (const auto& frame : frames) {
        ...
        if (!frame->size()) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_skipped_empty_frame_warnings_, kMaxSkippedEmptyFrameWarnings)
                << "Discarding empty audio or video coded frame, PTS=" << frame->timestamp().InMicroseconds()
                << "us, DTS=" << frame->GetDecodeTimestamp().InMicroseconds() << "us";
            continue;
        }

        if (!ProcessFrame(frame, append_window_start, append_window_end, timestamp_offset)) {
            FlushProcessedFrames();
            return false;
        }

        max_group_end_timestamp = std::max(group_end_timestamp_, max_group_end_timestamp);
    }
    ...
    if (max_group_end_timestamp > group_end_timestamp_) {
        ...
        // media-internals.
        DCHECK(parse_warning_cb_);
        parse_warning_cb_.Run(SourceBufferParseWarning::kGroupEndTimestampDecreaseWithinMediaSegment);
    }
...
void FrameProcessor::SetGroupStartTimestampIfInSequenceMode(base::TimeDelta timestamp_offset)
{
...
bool FrameProcessor::AddTrack(StreamParser::TrackId id, ChunkDemuxerStream* stream)
{
    ...
    if (existing_track) {
        MEDIA_LOG(ERROR, media_log_) << "Failure adding track with duplicate ID " << id;
        return false;
    }

    track_buffers_[id] = std::make_unique<MseTrackBuffer>(stream, media_log_, parse_warning_cb_);
    return true;
}
...
bool FrameProcessor::UpdateTrackIds(const TrackIdChanges& track_id_changes)
{
    ...
    for (const auto& ids : track_id_changes) {
        if (old_track_buffers.find(ids.first) == old_track_buffers.end() || new_track_buffers.find(ids.second) != new_track_buffers.end()) {
            MEDIA_LOG(ERROR, media_log_) << "Failure updating track id from " << ids.first << " to " << ids.second;
            return false;
        }
...
void FrameProcessor::OnPossibleAudioConfigUpdate(const AudioDecoderConfig& config)
{
    ...
    sample_duration_ = base::Seconds(1.0 / current_audio_config_.samples_per_second());
    has_dependent_audio_frames_ = current_audio_config_.profile() == AudioCodecProfile::kXHE_AAC || current_audio_config_.codec() == AudioCodec::kDTSXP2;
...
void FrameProcessor::NotifyStartOfCodedFrameGroup(DecodeTimestamp start_dts, base::TimeDelta start_pts)
{
    DVLOG(2) << __func__ << "(dts " << start_dts.InMicroseconds() << "us, pts " << start_pts.InMicroseconds() << "us)";
...
bool FrameProcessor::HandlePartialAppendWindowTrimming(
    base::TimeDelta append_window_start, base::TimeDelta append_window_end, scoped_refptr<StreamParserBuffer> buffer)
{
    ...
    if (audio_preroll_buffer_) {
        ...
        const int64_t delta = (audio_preroll_buffer_->timestamp() + audio_preroll_buffer_->duration() - buffer->timestamp()).InMicroseconds();
        // The only value that can't be converted with std::abs.
        if (delta == std::numeric_limits<int64_t>::min()) {
            return false;
        }
        if (std::abs(delta) < sample_duration_.InMicroseconds() && audio_preroll_buffer_->timestamp() <= buffer->timestamp()) {
            DVLOG(1) << "Attaching audio preroll buffer [" << audio_preroll_buffer_->timestamp().InMicroseconds() << "us, "
                     << (audio_preroll_buffer_->timestamp() + audio_preroll_buffer_->duration()).InMicroseconds() << "us) to "
                     << buffer->timestamp().InMicroseconds() << "us";
            buffer->SetPrerollBuffer(std::move(audio_preroll_buffer_));
            processed_buffer = true;
        } else {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_dropped_preroll_warnings_, kMaxDroppedPrerollWarnings)
                << "Partial append window trimming dropping unused audio preroll "
                   "buffer with PTS "
                << audio_preroll_buffer_->timestamp().InMicroseconds() << "us that ends too far (" << delta << "us) from next buffer with PTS "
                << buffer->timestamp().InMicroseconds() << "us";
            audio_preroll_buffer_.reset();
        }
    }
    ...
    if (buffer->timestamp() < append_window_start) {
        LIMITED_MEDIA_LOG(INFO, media_log_, num_partial_discard_warnings_, kMaxPartialDiscardWarnings)
            << "Truncating audio buffer which overlaps append window start."
            << " PTS " << buffer->timestamp().InMicroseconds() << "us frame_end_timestamp " << frame_end_timestamp.InMicroseconds() << "us append_window_start "
        ...
        buffer->set_discard_padding(std::make_pair(append_window_start - buffer->timestamp(), base::TimeDelta()));
    ...
    if (frame_end_timestamp > append_window_end) {
        LIMITED_MEDIA_LOG(INFO, media_log_, num_partial_discard_warnings_, kMaxPartialDiscardWarnings)
            ...
            << " PTS " << buffer->timestamp().InMicroseconds() << "us frame_end_timestamp " << frame_end_timestamp.InMicroseconds() << "us append_window_end "
            << append_window_end.InMicroseconds() << "us" << (buffer->is_duration_estimated() ? " (frame duration is estimated)" : "");

        // Mark the overlapping portion of the buffer for discard.
        // TODO(wolenetz): Is this correct to ignore any pre-existing discard
        // padding (e.g. WebM discard padding)? See https://crbug.com/969195.
        buffer->set_discard_padding(std::make_pair(buffer->discard_padding().first, frame_end_timestamp - append_window_end));
...
bool FrameProcessor::CheckAudioPresentationOrder(const StreamParserBuffer& frame, bool track_buffer_needs_random_access_point)
{
...
bool FrameProcessor::ProcessFrame(
    scoped_refptr<StreamParserBuffer> frame, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    ...
    while (true) {
        ...
        DVLOG(3) << __func__ << ": Processing frame Type=" << frame->type() << ", TrackID=" << frame->track_id()
                 << ", PTS=" << presentation_timestamp.InMicroseconds() << "us, DTS=" << decode_timestamp.InMicroseconds()
                 << "us, DUR=" << frame_duration.InMicroseconds() << "us, RAP=" << frame->is_key_frame();
        ...
        if (frame->type() == DemuxerStream::AUDIO && !has_dependent_audio_frames_ && !frame->is_key_frame()) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_audio_non_keyframe_warnings_, kMaxAudioNonKeyframeWarnings)
                << "Bytestream with audio frame PTS " << presentation_timestamp.InMicroseconds() << "us and DTS " << decode_timestamp.InMicroseconds()
        ...
        if (presentation_timestamp == kNoTimestamp) {
            MEDIA_LOG(ERROR, media_log_) << "Unknown PTS for " << frame->GetTypeName() << " frame";
            return false;
        }
        ...
        if (presentation_timestamp.is_inf()) {
            MEDIA_LOG(ERROR, media_log_) << "Before adjusting by timestampOffset, PTS for " << frame->GetTypeName()
                                         << " frame exceeds range allowed by implementation";
            return false;
        }

        if (decode_timestamp.is_inf()) {
            MEDIA_LOG(ERROR, media_log_) << "Before adjusting by timestampOffset, DTS for " << frame->GetTypeName()
                                         << " frame exceeds range allowed by implementation";
            return false;
        }
        ...
        DVLOG_IF(2, decode_timestamp.ToPresentationTime() > presentation_timestamp)
            << __func__ << ": WARNING: Frame DTS(" << decode_timestamp.InMicroseconds() << "us) > PTS(" << presentation_timestamp.InMicroseconds()
        ...
        if (frame_duration == kNoTimestamp) {
            MEDIA_LOG(ERROR, media_log_) << "Unknown duration for " << frame->GetTypeName() << " frame at PTS " << presentation_timestamp.InMicroseconds()
                                         << "us";
            return false;
        }
        ...
        CHECK(frame_duration >= base::TimeDelta() && frame_duration != kInfiniteDuration);
        ...
        if (sequence_mode_ && group_start_timestamp_ != kNoTimestamp) {
            ...
            if (group_start_timestamp_.is_inf()) {
                ...
                MEDIA_LOG(ERROR, media_log_) << "Sequence mode timestampOffset update prevented by a group "
            ...
            *timestamp_offset = group_start_timestamp_ - presentation_timestamp;
            if (timestamp_offset->is_inf()) {
                MEDIA_LOG(ERROR, media_log_) << "Sequence mode timestampOffset update resulted in an offset "
                                                "that exceeds range allowed by implementation";
                return false;
            }

            DVLOG(3) << __func__ << ": updated timestampOffset is now " << timestamp_offset->InMicroseconds() << "us";
        ...
        if (!timestamp_offset->is_zero()) {
            if (timestamp_offset->is_inf()) {
                ...
                // checks, above.
                MEDIA_LOG(ERROR, media_log_) << "timestampOffset exceeds range allowed by implementation";
                return false;
            }
            ...
            if (presentation_timestamp.is_inf()) {
                MEDIA_LOG(ERROR, media_log_) << "After adjusting by timestampOffset, PTS for " << frame->GetTypeName()
                                             << " frame exceeds range allowed by implementation";
                return false;
            }
            ...
            if (decode_timestamp.is_inf()) {
                MEDIA_LOG(ERROR, media_log_) << "After adjusting by timestampOffset, DTS for " << frame->GetTypeName()
                                             << " frame exceeds range allowed by implementation";
                return false;
            }
        }
        ...
        if (!track_buffer) {
            MEDIA_LOG(ERROR, media_log_) << "Unknown track with type " << frame->GetTypeName() << ", frame processor track id " << track_id
                                         << ", and parser track id " << frame->track_id();
            return false;
        }
        if (frame->type() != track_buffer->stream()->type()) {
            MEDIA_LOG(ERROR, media_log_) << "Frame type " << frame->GetTypeName() << " doesn't match track buffer type " << track_buffer->stream()->type();
            return false;
        }
        ...
        if (track_last_decode_timestamp != kNoDecodeTimestamp) {
            ...
            if (track_dts_delta.is_negative() || track_dts_delta > 2 * track_buffer->last_frame_duration()) {
                ...
                if (!sequence_mode_) {
                    ...
                    DVLOG(3) << __func__ << " : Sequence mode discontinuity, GETS: " << group_end_timestamp_.InMicroseconds() << "us";
        ...
        if (has_dependent_audio_frames_ && frame->type() == DemuxerStream::AUDIO
            && !CheckAudioPresentationOrder(*frame, track_buffer->needs_random_access_point())) {
            MEDIA_LOG(ERROR, media_log_) << "Dependent audio frame with invalid decreasing presentation "
                                            "timestamp detected.";
            return false;
        }

        // Attempt to trim audio exactly to fit the append window.
        if (frame->type() == DemuxerStream::AUDIO && (frame->is_key_frame() || !track_buffer->needs_random_access_point())
            && HandlePartialAppendWindowTrimming(append_window_start, append_window_end, frame)) {
        ...
        if (frame_end_timestamp.is_inf()) {
            MEDIA_LOG(ERROR, media_log_) << "Frame end timestamp for " << frame->GetTypeName() << " frame exceeds range allowed by implementation";
            return false;
        }
        ...
        if (presentation_timestamp < append_window_start || frame_end_timestamp > append_window_end) {
            ...
            LIMITED_MEDIA_LOG(INFO, media_log_, num_dropped_frame_warnings_, kMaxDroppedFrameWarnings)
                << "Dropping " << frame->GetTypeName() << " frame (DTS " << decode_timestamp.InMicroseconds() << "us PTS "
                << presentation_timestamp.InMicroseconds() << "us," << frame_end_timestamp.InMicroseconds() << "us) that is outside append window ["
                << append_window_start.InMicroseconds() << "us," << append_window_end.InMicroseconds() << "us).";
            return true;
        }
        ...
        if (track_buffer->needs_random_access_point()) {
            ...
            if (!frame->is_key_frame()) {
                DVLOG(3) << __func__ << ": Dropping frame that is not a random access point";
                return true;
            }
        ...
        // for this CFG.
        signal_new_cfg |= track_buffer->last_processed_decode_timestamp() > decode_timestamp
            || (track_buffer->pending_group_start_pts() != kNoTimestamp && track_buffer->pending_group_start_pts() > presentation_timestamp);
        ...
        if (frame->is_key_frame()) {
            ...
            if (track_buffer->highest_presentation_timestamp() != kNoTimestamp && track_buffer->highest_presentation_timestamp() > presentation_timestamp) {
                signal_new_cfg = true;
            ...
            signal_new_cfg |= track_buffer->highest_presentation_timestamp() != kNoTimestamp
                && track_buffer->highest_presentation_timestamp() + frame->duration() < presentation_timestamp;
        }
        ...
        if (signal_new_cfg) {
            ...
            if (pending_notify_all_group_start_) {
                ...
                DecodeTimestamp updated_dts = std::min(track_buffer->last_processed_decode_timestamp(), decode_timestamp);
                base::TimeDelta updated_pts = track_buffer->pending_group_start_pts();
                if (updated_pts == kNoTimestamp && track_buffer->highest_presentation_timestamp() != kNoTimestamp
                    && track_buffer->highest_presentation_timestamp() < presentation_timestamp) {
        ...
        DVLOG(3) << __func__ << ": Enqueueing processed frame "
                 << "PTS=" << presentation_timestamp.InMicroseconds() << "us, DTS=" << decode_timestamp.InMicroseconds() << "us";
        ...
        //     timestamp.
        track_buffer->SetHighestPresentationTimestampIfIncreased(frame_end_timestamp);
