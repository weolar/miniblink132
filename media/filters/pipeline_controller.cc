...
PipelineController::PipelineController(std::unique_ptr<Pipeline> pipeline, PipelineStatusCB started_cb, SeekedCB seeked_cb, SuspendedCB suspended_cb,
    BeforeResumeCB before_resume_cb, ResumedCB resumed_cb, PipelineStatusCB error_cb)
...
void PipelineController::Start(Pipeline::StartType start_type, Demuxer* demuxer, Pipeline::Client* client, bool is_streaming, bool is_static)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    pipeline_->Start(start_type, demuxer, client,
        base::BindOnce(&PipelineController::OnPipelineStatus, weak_factory_.GetWeakPtr(),
            start_type == Pipeline::StartType::kNormal ? State::PLAYING : State::PLAYING_OR_SUSPENDED));
}
...
void PipelineController::Resume()
{
    ...
    if (state_ == State::SUSPENDING || state_ == State::SUSPENDED || (state_ == State::SWITCHING_TRACKS && previous_track_change_state_ == State::SUSPENDED)) {
...
void PipelineController::OnPipelineStatus(State expected_state, PipelineStatus pipeline_status)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void PipelineController::Dispatch()
{
    ...
    if (pending_suspend_ && state_ == State::PLAYING) {
        ...
        pipeline_->Suspend(base::BindOnce(&PipelineController::OnPipelineStatus, weak_factory_.GetWeakPtr(), State::SUSPENDED));
        return;
    }
    ...
    if ((pending_resume_ || (pending_startup_ && pending_seek_)) && state_ == State::SUSPENDED) {
        ...
        pipeline_->Resume(seek_time_, base::BindOnce(&PipelineController::OnPipelineStatus, weak_factory_.GetWeakPtr(), State::PLAYING));
        return;
    }

    // If we have pending operations, and a seek is ongoing, abort it.
    if ((pending_seek_ || pending_suspend_ || pending_audio_track_change_ || pending_video_track_change_) && waiting_for_seek_) {
    ...
    if ((pending_audio_track_change_ || pending_video_track_change_) && (state_ == State::PLAYING || state_ == State::SUSPENDED)) {
        ...
        if (pending_audio_track_change_) {
            ...
            pipeline_->OnEnabledAudioTracksChanged(
                pending_audio_track_change_ids_, base::BindOnce(&PipelineController::OnTrackChangeComplete, weak_factory_.GetWeakPtr()));
            return;
        }
        ...
        if (pending_video_track_change_) {
            ...
            pipeline_->OnSelectedVideoTrackChanged(
                pending_video_track_change_id_, base::BindOnce(&PipelineController::OnTrackChangeComplete, weak_factory_.GetWeakPtr()));
            return;
        }
    }
    ...
    if (pending_seek_ && state_ == State::PLAYING) {
        ...
        pipeline_->Seek(seek_time_, base::BindOnce(&PipelineController::OnPipelineStatus, weak_factory_.GetWeakPtr(), State::PLAYING));
        return;
    }
    ...
    if (pending_seeked_cb_ && (state_ == State::PLAYING || (state_ == State::SUSPENDED && pending_startup_))) {
...
void PipelineController::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
...
void PipelineController::SetWasPlayedWithUserActivationAndHighMediaEngagement(bool was_played_with_user_activation_and_high_media_engagement)
{
    pipeline_->SetWasPlayedWithUserActivationAndHighMediaEngagement(was_played_with_user_activation_and_high_media_engagement);
}
...
void PipelineController::SetCdm(CdmContext* cdm_context, CdmAttachedCB cdm_attached_cb)
{
    pipeline_->SetCdm(cdm_context, std::move(cdm_attached_cb));
}

void PipelineController::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void PipelineController::OnSelectedVideoTrackChanged(std::optional<MediaTrack::Id> selected_track_id)
{
    DCHECK(thread_checker_.CalledOnValidThread());
