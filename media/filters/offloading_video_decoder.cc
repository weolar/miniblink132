...
public:
    ...
    void Decode(scoped_refptr<DecoderBuffer> buffer, VideoDecoder::DecodeCB decode_cb)
    {
    ...
    OffloadableVideoDecoder* decoder() const
    {
        return decoder_.get();
    }
...
OffloadingVideoDecoder::OffloadingVideoDecoder(
    int min_offloading_width, std::vector<VideoCodec> supported_codecs, std::unique_ptr<OffloadableVideoDecoder> decoder)
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    const bool disable_offloading
        = config.is_encrypted() || config.coded_size().width() < min_offloading_width_ || !base::Contains(supported_codecs_, config.codec());
    ...
    if (initialized_) {
        ...
        if (disable_offloading && offload_task_runner_) {
            offload_task_runner_->PostTaskAndReply(FROM_HERE, base::BindOnce(&OffloadableVideoDecoder::Detach, base::Unretained(helper_->decoder())),
                ...
                base::BindOnce(&OffloadingVideoDecoder::Initialize, weak_factory_.GetWeakPtr(), config, low_delay, cdm_context, std::move(init_cb), output_cb,
                    waiting_cb));
            return;
        }
    ...
    if (disable_offloading) {
        ...
        helper_->decoder()->Initialize(config, low_delay, cdm_context, std::move(bound_init_cb), bound_output_cb, waiting_cb);
        return;
    }

    if (!offload_task_runner_) {
        offload_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_BLOCKING });
    }

    offload_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&OffloadableVideoDecoder::Initialize, base::Unretained(helper_->decoder()), config, low_delay, cdm_context, std::move(bound_init_cb),
            bound_output_cb, waiting_cb));
}

void OffloadingVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer);
    DCHECK(decode_cb);
    ...
    offload_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&CancellationHelper::Decode, base::Unretained(helper_.get()), std::move(buffer), std::move(bound_decode_cb)));
}
...
void OffloadingVideoDecoder::Reset(base::OnceClosure reset_cb)
{
    ...
    if (!offload_task_runner_) {
        ...
        helper_->Cancel();
        offload_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CancellationHelper::Reset, base::Unretained(helper_.get()), std::move(bound_reset_cb)));
    }
}
