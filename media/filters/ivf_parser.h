...
static_assert(sizeof(IvfFileHeader) == 32, "sizeof(IvfFileHeader) must be fixed since it will be used with file IO");
...
static_assert(sizeof(IvfFrameHeader) == 12, "sizeof(IvfFrameHeader) must be fixed since it will be used with file IO");
#pragma pack(pop)
...
public:
    ...
    bool Initialize(const uint8_t* stream, size_t size, IvfFileHeader* file_header);
