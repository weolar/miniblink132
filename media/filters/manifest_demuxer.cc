...
ManifestDemuxer::ManifestDemuxerStream::ManifestDemuxerStream(DemuxerStream* stream, WrapperReadCb cb)
    : read_cb_(std::move(cb))
    , stream_(stream)
{
}

void ManifestDemuxer::ManifestDemuxerStream::Read(uint32_t count, DemuxerStream::ReadCB cb)
{
    stream_->Read(count, base::BindOnce(read_cb_, std::move(cb)));
}

AudioDecoderConfig ManifestDemuxer::ManifestDemuxerStream::audio_decoder_config()
{
    return stream_->audio_decoder_config();
}

VideoDecoderConfig ManifestDemuxer::ManifestDemuxerStream::video_decoder_config()
{
...
ManifestDemuxer::ManifestDemuxer(scoped_refptr<base::SequencedTaskRunner> media_task_runner, base::RepeatingCallback<void(base::TimeDelta)> request_seek,
    std::unique_ptr<ManifestDemuxer::Engine> impl, MediaLog* media_log)
    : request_seek_(std::move(request_seek))
    , media_log_(media_log->Clone())
    , media_task_runner_(std::move(media_task_runner))
    , impl_(std::move(impl))
{
    media_log_->AddMessage(MediaLogMessageLevel::kINFO, "Demuxing stream using ManifestDemuxer");
}
...
std::vector<DemuxerStream*> ManifestDemuxer::GetAllStreams()
{
    ...
    for (DemuxerStream* chunk_demuxer_stream : chunk_demuxer_->GetAllStreams()) {
        ...
        auto wrapper = std::make_unique<ManifestDemuxerStream>(
            chunk_demuxer_stream, base::BindRepeating(&ManifestDemuxer::OnDemuxerStreamRead, weak_factory_.GetWeakPtr()));
...
void ManifestDemuxer::Initialize(DemuxerHost* host, PipelineStatusCallback status_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    chunk_demuxer_ = std::make_unique<ChunkDemuxer>(base::BindOnce(&ManifestDemuxer::OnChunkDemuxerOpened, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&ManifestDemuxer::OnProgress, weak_factory_.GetWeakPtr()),
        base::BindRepeating(&ManifestDemuxer::OnEncryptedMediaData, weak_factory_.GetWeakPtr()), media_log_.get());

    chunk_demuxer_->Initialize(host, base::BindOnce(&ManifestDemuxer::OnChunkDemuxerInitialized, weak_factory_.GetWeakPtr()));

    impl_->Initialize(this, base::BindOnce(&ManifestDemuxer::OnEngineInitialized, weak_factory_.GetWeakPtr()));
}
...
void ManifestDemuxer::StartWaitingForSeek(base::TimeDelta seek_time)
{
    if (!media_task_runner_->RunsTasksInCurrentSequence()) {
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ManifestDemuxer::StartWaitingForSeek, weak_factory_.GetWeakPtr(), seek_time));
        return;
    }
...
{
    if (!media_task_runner_->RunsTasksInCurrentSequence()) {
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ManifestDemuxer::CancelPendingSeek, weak_factory_.GetWeakPtr(), seek_time));
        return;
    }
...
void ManifestDemuxer::Seek(base::TimeDelta time, PipelineStatusCallback status_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    CHECK(!pending_seek_);
...
void ManifestDemuxer::SeekInternal()
{
    ...
    impl_->Seek(media_time_, base::BindOnce(&ManifestDemuxer::OnEngineSeeked, weak_factory_.GetWeakPtr()));
}
...
std::optional<container_names::MediaContainerName> ManifestDemuxer::GetContainerForMetrics() const
{
...
void ManifestDemuxer::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    chunk_demuxer_->OnEnabledAudioTracksChanged(
        MapTrackIds(track_ids), curr_time, base::BindOnce(&ManifestDemuxer::MapDemuxerStreams, weak_factory_.GetWeakPtr(), std::move(change_completed_cb)));
}

void ManifestDemuxer::OnSelectedVideoTrackChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
bool ManifestDemuxer::AddRole(std::string_view role, RelaxedParserSupportedType mime)
{
    CHECK(chunk_demuxer_);
    if (ChunkDemuxer::kOk != chunk_demuxer_->AddAutoDetectedCodecsId(std::string(role), mime)) {
        return false;
    }
    chunk_demuxer_->SetParseWarningCallback(
        std::string(role), base::BindRepeating(&ManifestDemuxer::OnChunkDemuxerParseWarning, weak_factory_.GetWeakPtr(), std::string(role)));
    chunk_demuxer_->SetTracksWatcher(
        std::string(role), base::BindRepeating(&ManifestDemuxer::OnChunkDemuxerTracksChanged, weak_factory_.GetWeakPtr(), std::string(role)));
    return true;
}
...
void ManifestDemuxer::SetSequenceMode(std::string_view role, bool sequence_mode)
{
    CHECK(chunk_demuxer_);
...
Ranges<base::TimeDelta> ManifestDemuxer::GetBufferedRanges(std::string_view role)
{
    CHECK(chunk_demuxer_);
    return chunk_demuxer_->GetBufferedRanges(std::string(role));
}

void ManifestDemuxer::Remove(std::string_view role, base::TimeDelta start, base::TimeDelta end)
{
    chunk_demuxer_->Remove(std::string(role), start, end);
}

void ManifestDemuxer::RemoveAndReset(std::string_view role, base::TimeDelta start, base::TimeDelta end, base::TimeDelta* offset)
{
    CHECK(chunk_demuxer_);
...
void ManifestDemuxer::SetGroupStartIfParsingAndSequenceMode(std::string_view role, base::TimeDelta start)
{
    CHECK(chunk_demuxer_);
    if (!chunk_demuxer_->IsParsingMediaSegment(std::string(role))) {
        chunk_demuxer_->SetGroupStartTimestampIfInSequenceMode(std::string(role), start);
    }
}

void ManifestDemuxer::EvictCodedFrames(std::string_view role, base::TimeDelta time, size_t data_size)
{
    CHECK(chunk_demuxer_);
...
bool ManifestDemuxer::AppendAndParseData(std::string_view role, base::TimeDelta end, base::TimeDelta* offset, base::span<const uint8_t> data)
{
    CHECK(chunk_demuxer_);
    ...
    while (true) {
        switch (chunk_demuxer_->RunSegmentParserLoop(std::string(role), base::TimeDelta(), end, offset)) {
...
void ManifestDemuxer::ResetParserState(std::string_view role, base::TimeDelta end, base::TimeDelta* offset)
{
    CHECK(chunk_demuxer_);
    return chunk_demuxer_->ResetParserState(std::string(role), base::TimeDelta(), end, offset);
}
...
void ManifestDemuxer::SetGroupStartTimestamp(std::string_view role, base::TimeDelta time)
{
    chunk_demuxer_->SetGroupStartTimestampIfInSequenceMode(std::string(role), time);
}
...
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    auto queue_next_event = base::BindOnce(&ManifestDemuxer::OnEngineEventFinished, weak_factory_.GetWeakPtr());
    TriggerEventWithTime(std::move(queue_next_event), media_time_);
}

void ManifestDemuxer::TriggerEventWithTime(DelayCallback cb, base::TimeDelta current_time)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    has_pending_event_ = true;
    impl_->OnTimeUpdate(current_time, current_playback_rate_, base::BindPostTask(media_task_runner_, std::move(cb)));
}
...
void ManifestDemuxer::OnEngineEventFinished(base::TimeDelta delay)
{
    ...
    if (delay != kNoTimestamp) {
        ...
        cancelable_next_event_.Reset(base::BindOnce(&ManifestDemuxer::TriggerEvent, weak_factory_.GetWeakPtr()));
        media_task_runner_->PostDelayedTask(FROM_HERE, cancelable_next_event_.callback(), delay);
    }
}
...
void ManifestDemuxer::OnEngineSeeked(SeekResponse seek_status)
{
    ...
    chunk_demuxer_->Seek(media_time_, base::BindOnce(&ManifestDemuxer::OnChunkDemuxerSeeked, weak_factory_.GetWeakPtr()));
    ...
    if (std::move(seek_status).value() == SeekState::kNeedsData) {
        ...
        TriggerEventWithTime(base::BindOnce(&ManifestDemuxer::OnSeekBuffered, weak_factory_.GetWeakPtr()), media_time_);
    }
}
...
void ManifestDemuxer::OnChunkDemuxerParseWarning(std::string role, SourceBufferParseWarning warning)
{
    MEDIA_LOG(WARNING, media_log_) << "ParseWarning (" << role << "): " << static_cast<int>(warning);
}

void ManifestDemuxer::OnChunkDemuxerTracksChanged(std::string role, std::unique_ptr<MediaTracks> tracks)
{
...
void ManifestDemuxer::OnEncryptedMediaData(EmeInitDataType type, const std::vector<uint8_t>& data)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    OnError(PIPELINE_ERROR_INVALID_STATE);
}

void ManifestDemuxer::OnDemuxerStreamRead(DemuxerStream::ReadCB wrapped_read_cb, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void ManifestDemuxer::MapDemuxerStreams(TrackChangeCB cb, const std::vector<DemuxerStream*>& streams)
{
...
std::vector<MediaTrack::Id> ManifestDemuxer::MapTrackIds(const std::vector<MediaTrack::Id>& track_ids)
{
