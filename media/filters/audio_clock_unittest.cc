...
public:
    ...
    void WroteAudio(int frames_written, int frames_requested, int delay_frames, double playback_rate)
    {
        clock_->WroteAudio(frames_written, frames_requested, delay_frames, playback_rate);
    }
    ...
    int FrontTimestampInDays()
    {
        return clock_->front_timestamp().InDays();
    }
    ...
    {
        return clock_->TimeUntilPlayback(base::Milliseconds(timestamp_ms)).InMilliseconds();
    }
...
TEST_F(AudioClockTest, SupportsYearsWorthOfAudioData)
{
    ...
    const base::TimeDelta huge = base::Seconds(huge_amount_of_frames / sample_rate_);
    ...
    WroteAudio(huge_amount_of_frames, huge_amount_of_frames, huge_amount_of_frames, 1.0);
    EXPECT_EQ((huge * 3).InDays(), FrontTimestampInDays());
...
TEST_F(AudioClockTest, FramesToTimePrecision)
{
    ...
    EXPECT_EQ(std::round(frames_written * micros_per_frame), clock_->back_timestamp().InMicroseconds());
}
