...
static void GetDecoderThreadCounts(const int coded_height, int* tile_threads, int* frame_threads)
{
    ...
    if (coded_height >= 700) {
        *tile_threads = 4; // Current 720p content is encoded in 5 tiles and 1080p content with
...
static VideoPixelFormat Dav1dImgFmtToVideoPixelFormat(const Dav1dPictureParameters* pic)
{
    switch (pic->layout) {
...
Dav1dVideoDecoder::Dav1dVideoDecoder(std::unique_ptr<MediaLog> media_log, OffloadState offload_state)
...
void Dav1dVideoDecoder::Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb,
    const WaitingCB& /* waiting_cb */)
{
    ...
    if (config.codec() != VideoCodec::kAV1) {
        std::move(bound_init_cb).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedCodec).WithData("codec", config.codec()));
        return;
    }
    ...
    GetDecoderThreadCounts(config.coded_size().height(), &tile_threads, &frame_threads);
    ...
    s.n_threads = VideoDecoder::GetRecommendedThreadCount(frame_threads * (tile_threads + 1));
...

    {
        ...
        if (res < 0) {
            std::move(bound_init_cb).Run(DecoderStatus(DecoderStatus::Codes::kFailedToCreateDecoder, "dav1d_open() failed").WithData("error_code", res));
            return;
        }
...
void Dav1dVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer);
    DCHECK(decode_cb);
...
void Dav1dVideoDecoder::Reset(base::OnceClosure reset_cb)
{
    ...
    if (bind_callbacks_)
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(reset_cb));
    else
...
bool Dav1dVideoDecoder::DecodeBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    ...
    if (!buffer->end_of_stream()) {
        ...
        const int res = dav1d_data_wrap(input_buffer.get(), buffer->data(), buffer->size(), &ReleaseDecoderBuffer, buffer.get());
        if (res < 0) {
            MEDIA_LOG(ERROR, media_log_) << "dav1d_data_wrap() failed with error " << res;
            return false;
        }
    ...
    while (!input_buffer || input_buffer->sz) {
        if (input_buffer) {
            ...
            if (res < 0 && res != -EAGAIN) {
                MEDIA_LOG(ERROR, media_log_) << "dav1d_send_data() failed with error " << res << " on " << buffer->AsHumanReadableString();
                return false;
            }
        ...
        using ScopedPtrDav1dPicture = std::unique_ptr<Dav1dPicture, ScopedDav1dPictureFree>;
        ScopedPtrDav1dPicture p(new Dav1dPicture {});

        const int res = dav1d_get_picture(dav1d_decoder_.get(), p.get());
        if (res < 0) {
            if (res != -EAGAIN) {
                MEDIA_LOG(ERROR, media_log_) << "dav1d_get_picture() failed with error " << res << " on " << buffer->AsHumanReadableString();
                return false;
            }
        ...
        if (!frame) {
            MEDIA_LOG(DEBUG, media_log_) << "Failed to produce video frame from Dav1dPicture.";
            return false;
        }
        ...
        VideoColorSpace color_space(
            p->seq_hdr->pri, p->seq_hdr->trc, p->seq_hdr->mtrx, p->seq_hdr->color_range ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
...
scoped_refptr<VideoFrame> Dav1dVideoDecoder::BindImageToVideoFrame(const Dav1dPicture* pic)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (needs_fake_uv_planes) {
        ...
        uv_plane_stride = base::bits::AlignUpDeprecatedDoNotUse(pic->stride[0] / 2, ptrdiff_t { 2 });
        ...
        if (!fake_uv_data_ || fake_uv_data_->size() != size_needed) {
            if (pic->p.bpc == 8) {
                ...
                fake_uv_data_ = base::MakeRefCounted<base::RefCountedBytes>(std::vector<uint8_t>(size_needed, kBlankUV));
            } else {
                DCHECK(pic->p.bpc == 10 || pic->p.bpc == 12);
                const uint16_t kBlankUV = (1 << pic->p.bpc) / 2;
                fake_uv_data_ = base::MakeRefCounted<base::RefCountedBytes>(size_needed);

                uint16_t* data = reinterpret_cast<uint16_t*>(fake_uv_data_->as_vector().data());
                std::fill(data, data + size_needed / 2, kBlankUV);
            }
        }

        u_plane = v_plane = reinterpret_cast<uint8_t*>(fake_uv_data_->as_vector().data());
    }

    auto frame
        = VideoFrame::WrapExternalYuvData(pixel_format, visible_size, gfx::Rect(visible_size), config_.aspect_ratio().GetNaturalSize(gfx::Rect(visible_size)),
            pic->stride[0], uv_plane_stride, uv_plane_stride, static_cast<uint8_t*>(pic->data[0]), u_plane, v_plane, base::Microseconds(pic->m.timestamp));
    if (!frame)
        return nullptr;

    // Each frame needs a ref on the fake UV data to keep it alive until done.
    if (needs_fake_uv_planes) {
        frame->AddDestructionObserver(base::BindOnce([](scoped_refptr<base::RefCountedBytes>) {}, fake_uv_data_));
    }
