...
unsigned GetMSEBufferSizeLimitIfExists(std::string_view switch_string)
{
    ...
    if (command_line->HasSwitch(switch_string) && base::StringToUint(command_line->GetSwitchValueASCII(switch_string), &memory_limit)) {
...
// static
Ranges<base::TimeDelta> SourceBufferState::ComputeRangesIntersection(const RangesList& active_ranges, bool ended)
{
    ...
    for (const auto& range : active_ranges) {
        ...
        if (ended && source_ranges.size()) {
            source_ranges.Add(source_ranges.start(source_ranges.size() - 1), highest_end_time);
        }
...
SourceBufferState::SourceBufferState(std::unique_ptr<StreamParser> stream_parser, std::unique_ptr<FrameProcessor> frame_processor,
    CreateDemuxerStreamCB create_demuxer_stream_cb, MediaLog* media_log)
...
void SourceBufferState::Init(
    StreamParser::InitCB init_cb, std::optional<std::string_view> expected_codecs, const StreamParser::EncryptedMediaInitDataCB& encrypted_media_init_data_cb)
{
...
void SourceBufferState::ChangeType(std::unique_ptr<StreamParser> new_stream_parser, const std::string& new_expected_codecs)
{
...
void SourceBufferState::SetGroupStartTimestampIfInSequenceMode(base::TimeDelta timestamp_offset)
{
    DCHECK(!parsing_media_segment_);
...
void SourceBufferState::SetTracksWatcher(Demuxer::MediaTracksUpdatedCB tracks_updated_cb)
{
...
void SourceBufferState::SetParseWarningCallback(SourceBufferParseWarningCB parse_warning_cb)
{
...
StreamParser::ParseStatus SourceBufferState::RunSegmentParserLoop(
    base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    ...
    StreamParser::ParseStatus result = stream_parser_->Parse(StreamParser::kMaxPendingBytesPerParse);

    if (result == StreamParser::ParseStatus::kFailed) {
        MEDIA_LOG(ERROR, media_log_) << __func__ << ": stream parsing failed. append_window_start=" << append_window_start.InSecondsF()
...
bool SourceBufferState::AppendChunks(std::unique_ptr<StreamParser::BufferQueue> buffer_queue, base::TimeDelta append_window_start,
    base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    ...
    if (!result) {
        MEDIA_LOG(ERROR, media_log_) << __func__ << ": Processing encoded chunks for buffering failed.";
    }
...
void SourceBufferState::ResetParserState(base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    DCHECK(timestamp_offset);
    DCHECK(!timestamp_offset_during_append_);
    timestamp_offset_during_append_ = timestamp_offset;
...
void SourceBufferState::Remove(base::TimeDelta start, base::TimeDelta end, base::TimeDelta duration)
{
...
bool SourceBufferState::EvictCodedFrames(base::TimeDelta media_time, size_t newDataSize)
{
    ...
    DVLOG(3) << __func__ << " media_time=" << media_time.InSecondsF() << " newDataSize=" << newDataSize << " total_buffered_size=" << total_buffered_size;
    ...
    for (const auto& it : audio_streams_) {
        ...
        success &= it.second->EvictCodedFrames(media_time, static_cast<size_t>(estimated_new_size));
    }
    for (const auto& it : video_streams_) {
        uint64_t curr_size = it.second->GetMemoryUsage();
        if (curr_size == 0)
    ...
    base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
    ...
    for (const auto& it : video_streams_) {
        it.second->OnMemoryPressure(media_time, memory_pressure_level, force_instant_gc);
    }
    for (const auto& it : audio_streams_) {
        it.second->OnMemoryPressure(media_time, memory_pressure_level, force_instant_gc);
    }
...
Ranges<base::TimeDelta> SourceBufferState::GetBufferedRanges(base::TimeDelta duration, bool ended) const
{
...
void SourceBufferState::SetMemoryLimits(DemuxerStream::Type type, size_t memory_limit)
{
    switch (type) {
...
void SourceBufferState::InitializeParser(std::optional<std::string_view> expected_codecs)
{
    ...
    stream_parser_->Init(base::BindOnce(&SourceBufferState::OnSourceInitDone, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnNewConfigs, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnNewBuffers, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnEncryptedMediaInitData, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnNewMediaSegment, base::Unretained(this)),
        base::BindRepeating(&SourceBufferState::OnEndOfMediaSegment, base::Unretained(this)), media_log_);
}
...
bool SourceBufferState::OnNewConfigs(std::unique_ptr<MediaTracks> tracks)
{
    ...
    for (const auto& track : tracks->tracks()) {
        ...
        if (track->type() == MediaTrack::Type::kAudio) {
            ...
            DVLOG(1) << "Audio track_id=" << track_id << " config: " << audio_config.AsHumanReadableString();
            DCHECK(audio_config.IsValidConfig());

            if (strict_codec_expectations_) {
                const auto& it = base::ranges::find(expected_acodecs, audio_config.codec());
                if (it == expected_acodecs.end()) {
                    MEDIA_LOG(ERROR, media_log_) << "Audio stream codec " << GetCodecName(audio_config.codec()) << " doesn't match SourceBuffer codecs.";
                    return false;
                }
            ...
            if (!first_init_segment_received_) {
                ...
                media_log_->SetProperty<MediaLogProperty::kAudioTracks>(std::vector<AudioDecoderConfig> { audio_config });
            } else {
            ...
            success &= stream->UpdateAudioConfig(audio_config, allow_codec_changes, media_log_);
        } else if (track->type() == MediaTrack::Type::kVideo) {
            VideoDecoderConfig video_config = tracks->getVideoConfig(track_id);
            DVLOG(1) << "Video track_id=" << track_id << " config: " << video_config.AsHumanReadableString();
            ...
            if (video_config.codec() == VideoCodec::kDolbyVision) {
                ...
                if (!base::FeatureList::IsEnabled(kPlatformEncryptedDolbyVision)) {
                    MEDIA_LOG(ERROR, media_log_) << "MSE playback of DolbyVision is not supported because "
                ...
                if (!base::FeatureList::IsEnabled(kAllowClearDolbyVisionInMseWhenPlatformEncryptedDvEnabled) && !video_config.is_encrypted()) {
                    MEDIA_LOG(ERROR, media_log_) << "MSE playback of DolbyVision is only supported via platform "
            ...
            if (strict_codec_expectations_) {
                const auto& it = base::ranges::find(expected_vcodecs, video_config.codec());
                if (it == expected_vcodecs.end()) {
                    MEDIA_LOG(ERROR, media_log_) << "Video stream codec " << GetCodecName(video_config.codec()) << " doesn't match SourceBuffer codecs.";
                    return false;
                }
            ...
            if (!first_init_segment_received_) {
                ...
                media_log_->SetProperty<MediaLogProperty::kVideoTracks>(std::vector<VideoDecoderConfig> { video_config });
            } else {
            ...
            track->set_id(stream->media_track_id());
            success &= stream->UpdateVideoConfig(video_config, allow_codec_changes, media_log_);
        } else {
            MEDIA_LOG(ERROR, media_log_) << "Error: unsupported media track type " << base::to_underlying(track->type());
            return false;
        }
    }
    ...
    if (!expected_acodecs.empty() || !expected_vcodecs.empty()) {
        for (const auto& acodec : expected_acodecs) {
            MEDIA_LOG(ERROR, media_log_) << "Initialization segment misses expected " << GetCodecName(acodec) << " track.";
        }
        for (const auto& vcodec : expected_vcodecs) {
            MEDIA_LOG(ERROR, media_log_) << "Initialization segment misses expected " << GetCodecName(vcodec) << " track.";
        }
        return false;
    }
...
{
    size_t audio_buf_size_limit = GetMSEBufferSizeLimitIfExists(switches::kMSEAudioBufferSizeLimitMb);
    if (audio_buf_size_limit) {
        MEDIA_LOG(INFO, media_log_) << "Custom audio per-track SourceBuffer size limit=" << audio_buf_size_limit;
        for (const auto& it : audio_streams_)
            it.second->SetStreamMemoryLimit(audio_buf_size_limit);
    }

    size_t video_buf_size_limit = GetMSEBufferSizeLimitIfExists(switches::kMSEVideoBufferSizeLimitMb);
    if (video_buf_size_limit) {
        MEDIA_LOG(INFO, media_log_) << "Custom video per-track SourceBuffer size limit=" << video_buf_size_limit;
        for (const auto& it : video_streams_)
...
void SourceBufferState::OnEndOfMediaSegment()
{
    ...
    for (const auto& it : audio_streams_) {
        if (!media_segment_has_data_for_track_[it.first]) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_missing_track_logs_, kMaxMissingTrackInSegmentLogs)
                << "Media segment did not contain any coded frames for track " << it.first
...
bool SourceBufferState::OnNewBuffers(const StreamParser::BufferQueueMap& buffer_queue_map)
{
    ...
    const base::TimeDelta timestamp_offset_before_processing = *timestamp_offset_during_append_;
    ...
    // true.
    base::TimeDelta predicted_timestamp_offset = timestamp_offset_before_processing;
    if (generate_timestamps_flag()) {
        base::TimeDelta min_end_timestamp = kNoTimestamp;
        for (const auto& [track_id, buffer_queue] : buffer_queue_map) {
            DCHECK(!buffer_queue.empty());
            if (min_end_timestamp == kNoTimestamp || EndTimestamp(buffer_queue) < min_end_timestamp) {
    ...
    if (!frame_processor_->ProcessFrames(
            buffer_queue_map, append_window_start_during_append_, append_window_end_during_append_, timestamp_offset_during_append_)) {
        return false;
    }

    // Only update the timestamp offset if the frame processor hasn't already.
    if (generate_timestamps_flag() && timestamp_offset_before_processing == *timestamp_offset_during_append_) {
...
void SourceBufferState::OnEncryptedMediaInitData(EmeInitDataType type, const std::vector<uint8_t>& init_data)
{
    encrypted_media_init_data_reported_ = true;
    encrypted_media_init_data_cb_.Run(type, init_data);
}

void SourceBufferState::OnSourceInitDone(const StreamParser::InitParameters& params)
{
    // We've either yet-to-run |init_cb_| if pending init, or we've previously
    // run it if pending reinit.
    DCHECK((init_cb_ && state_ == PENDING_PARSER_INIT) || (!init_cb_ && state_ == PENDING_PARSER_REINIT));
