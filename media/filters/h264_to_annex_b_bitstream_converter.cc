...
bool H264ToAnnexBBitstreamConverter::ParseConfiguration(
    const uint8_t* configuration_record, int configuration_record_size, mp4::AVCDecoderConfigurationRecord* avc_config)
{
...
uint32_t H264ToAnnexBBitstreamConverter::GetConfigSize(const mp4::AVCDecoderConfigurationRecord& avc_config) const
{
...
uint32_t H264ToAnnexBBitstreamConverter::CalculateNeededOutputBufferSize(
    const uint8_t* input, uint32_t input_size, const mp4::AVCDecoderConfigurationRecord* avc_config) const
{
...
bool H264ToAnnexBBitstreamConverter::ConvertAVCDecoderConfigToByteStream(
    const mp4::AVCDecoderConfigurationRecord& avc_config, uint8_t* output, uint32_t* output_size)
{
    uint8_t* out = output;
...
bool H264ToAnnexBBitstreamConverter::ConvertNalUnitStreamToByteStream(
    const uint8_t* input, uint32_t input_size, const mp4::AVCDecoderConfigurationRecord* avc_config, uint8_t* output, uint32_t* output_size)
{
    ...
    while (data_left > 0) {
        ...
        if (avc_config && (nal_unit_type != H264NALU::kAUD || nal_unit_count > 1)) {
            ...
            if (!ConvertAVCDecoderConfigToByteStream(*avc_config, outscan, &config_size)) {
...
bool H264ToAnnexBBitstreamConverter::WriteParamSet(const std::vector<uint8_t>& param_set, uint8_t** out, uint32_t* out_size) const
{
