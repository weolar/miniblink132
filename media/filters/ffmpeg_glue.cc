...
BASE_FEATURE(kAllowOnlyAudioCodecsDuringDemuxing, "AllowOnlyAudioCodecsDuringDemuxing", base::FEATURE_ENABLED_BY_DEFAULT);
BASE_FEATURE(kForbidH264ParsingDuringDemuxing, "ForbidH264ParsingDuringDemuxing", base::FEATURE_ENABLED_BY_DEFAULT);
...
static void LogContainer(bool is_local_file, container_names::MediaContainerName container)
{
    base::UmaHistogramSparse("Media.DetectedContainer", base::to_underlying(container));
    if (is_local_file) {
        base::UmaHistogramSparse("Media.DetectedContainer.Local", base::to_underlying(container));
    }
}
...
static const char* GetAllowedDemuxers()
{
    static const base::NoDestructor<std::string> kAllowedDemuxers([]() {
        ...
        std::vector<std::string> allowed_demuxers = { "ogg", "matroska", "wav", "flac", "mp3", "mov" };
...
FFmpegGlue::FFmpegGlue(FFmpegURLProtocol* protocol)
{
    ...
    avio_context_.reset(
        avio_alloc_context(static_cast<unsigned char*>(av_malloc(kBufferSize)), kBufferSize, 0, protocol, &AVIOReadOperation, nullptr, &AVIOSeekOperation));
    ...
    if (base::FeatureList::IsEnabled(kFFmpegAllowLists)) {
        ...
        static const base::NoDestructor<std::string> kCombinedCodecList([]() {
            ...
            return base::JoinString({ GetAllowedAudioDecoders(), GetAllowedVideoDecoders() }, ",");
        }());
...
const char* FFmpegGlue::GetAllowedAudioDecoders()
{
    static const base::NoDestructor<std::string> kAllowedAudioCodecs([]() {
        // This should match the configured lists in //third_party/ffmpeg.
        std::string allowed_decoders("vorbis,libopus,flac,pcm_u8,pcm_s16le,pcm_s24le,pcm_s32le,pcm_f32le,"
...
bool FFmpegGlue::OpenContext(bool is_local_file)
{
    ...
    const int ret = avformat_open_input(&format_context_, nullptr, nullptr, &options);
    ...
    if (ret == AVERROR_INVALIDDATA) {
        ...
        const int num_read = AVIOReadOperation(avio_context_->opaque, buffer.data(), buffer.size());
        ...
        detected_hls_ = container_ == container_names::MediaContainerName::kContainerHLS;
        return false;
    } else if (ret < 0) {
        return false;
    }
