...
const std::string kInvalidMediaPlaylist = "#This Wont Parse!\n"
                                          "#EXT-X-ENDLIST\n";

const std::string kShortMediaPlaylist = "#EXTM3U\n"
...
const std::string kSimpleMediaPlaylist = "#EXTM3U\n"
                                         "#EXT-X-TARGETDURATION:10\n"
                                         "#EXT-X-VERSION:3\n"
                                         "#EXTINF:9.009,\n"
                                         "http://media.example.com/first.ts\n"
...
const std::string kSimpleLiveMediaPlaylist = "#EXTM3U\n"
                                             "#EXT-X-TARGETDURATION:10\n"
                                             "#EXT-X-VERSION:3\n"
...
const std::string kUnsupportedCodecs = "#EXTM3U\n"
...
const std::string kSimpleMultivariantPlaylist = "#EXTM3U\n"
...
const std::string kMultivariantPlaylistWithAlts = "#EXTM3U\n"
...
const std::string kLiveFullEncryptedMediaPlaylist = "#EXTM3U\n"
                                                    "#EXT-X-VERSION:4\n"
...
MATCHER_P2(
    CloseTo, Target, Radius, std::string(negation ? "isn't" : "is") + " within " + testing::PrintToString(Radius) + " of " + testing::PrintToString(Target))
{
    return (arg - Target <= Radius) || (Target - arg <= Radius);
}

MATCHER_P2(SingleSegmentQueue, urlstr, range, "Segment Queue matcher for HlsDataSourceProvider")
{
    if (arg.size() != 1) {
        return false;
    }
...
std::tuple<std::string, std::unique_ptr<crypto::SymmetricKey>> Encrypt(std::string cleartext, std::string ivstr)
{
...
public:
    FakeHlsDataSourceProvider(HlsDataSourceProvider* mock)
        : mock_(mock)
    {
    }

    void ReadFromCombinedUrlQueue(SegmentQueue segments, HlsDataSourceProvider::ReadCb callback) override
    {
        mock_->ReadFromCombinedUrlQueue(std::move(segments), std::move(callback));
    }

    void ReadFromExistingStream(std::unique_ptr<HlsDataSourceStream> stream, HlsDataSourceProvider::ReadCb cb) override
    {
...
template <typename T> class CallbackEnforcer {
public:
    explicit CallbackEnforcer(T expected, const base::Location& from = base::Location::Current())
    ...
    base::OnceCallback<void(T)> GetCallback()
    {
        return base::BindOnce(
            [](size_t line, bool* writeback, T expected, T actual) {
                *writeback = true;
                ASSERT_EQ(actual, expected) << "Callback at line:" << line << " called with wrong parameter";
            },
    ...
    {
        env.RunUntilIdle();
        ASSERT_TRUE(was_called_) << "Callback at line:" << created_.line_number() << " never called";
    }
...
protected:
    ...
    template <typename T> void BindUrlToDataSource(std::string url, std::string value, bool taint_origin = false)
    {
        EXPECT_CALL(*mock_dsp_, ReadFromCombinedUrlQueue(SingleSegmentQueue(url, std::nullopt), _))
            .Times(1)
            .WillOnce(RunOnceCallback<1>(T::CreateStream(value, taint_origin)));
    }

    template <typename T> void BindUrlAssignmentThunk(std::string url, std::string value, bool taint_origin = false)
    {
        EXPECT_CALL(*mock_dsp_, ReadFromCombinedUrlQueue(SingleSegmentQueue(std::move(url), std::nullopt), _))
            .Times(1)
            .WillOnce([this, value = std::move(value), taint_origin = taint_origin](HlsDataSourceProvider::SegmentQueue, HlsDataSourceProvider::ReadCb cb) {
                pending_url_fetch_ = base::BindOnce(std::move(cb), T::CreateStream(std::move(value), taint_origin));
            });
    }
    ...
    MockHlsRendition* SetUpInterruptTest()
    {
        ...
        EXPECT_CALL(*mock_mdeh_, AddRole("primary", RelaxedParserSupportedType::kMP2T));
        BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kSimpleMultivariantPlaylist);
        BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://example.com/hi.m3u8", kSimpleMediaPlaylist);
    ...
    base::OnceClosure StartAndCaptureSeek(MockHlsRendition* rendition_ptr)
    {
        ...
        EXPECT_CALL(*mock_dsp_, AbortPendingReads(_)).WillOnce([&continue_seek](base::OnceClosure cb) { continue_seek = std::move(cb); });
        ...
        engine_->Seek(base::Seconds(10),
            base::BindOnce(
                ...
                base::BindOnce(&HlsManifestDemuxerEngineTest::SeekFinished, base::Unretained(this))));
        task_environment_.RunUntilIdle();
        CHECK(continue_seek);
        return base::BindOnce(
            [](MockHlsRendition* rendition_ptr, base::OnceClosure cb) {
                EXPECT_CALL(*rendition_ptr, Seek(_)).WillOnce(Return(ManifestDemuxer::SeekState::kIsReady));
                std::move(cb).Run();
            },
            rendition_ptr, std::move(continue_seek));
    }

    base::OnceClosure StartAndCaptureTimeUpdate(MockHlsRendition* rendition_ptr, base::TimeDelta timestamp)
    {
        ManifestDemuxer::DelayCallback finish_time_update;
        EXPECT_CALL(*rendition_ptr, CheckState(_, _, _)).WillOnce([&finish_time_update](base::TimeDelta, double, ManifestDemuxer::DelayCallback cb) {
            finish_time_update = std::move(cb);
        });
        engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta timestamp, base::TimeDelta r) { ASSERT_EQ(r, timestamp); }, timestamp));
        task_environment_.RunUntilIdle();
        CHECK(finish_time_update);
        return base::BindOnce(std::move(finish_time_update), timestamp);
    }

    base::OnceClosure StartAndCaptureNetworkAdaptation(MockHlsRendition* rendition_ptr, std::string url, std::string value, size_t netspeed)
    {
        base::OnceClosure continue_adaptation;
        EXPECT_CALL(*mock_dsp_, ReadFromCombinedUrlQueue(SingleSegmentQueue(url, std::nullopt), _))
            .Times(1)
            .WillOnce([&continue_adaptation, value](HlsDataSourceProvider::SegmentQueue, HlsDataSourceProvider::ReadCb cb) {
                continue_adaptation = base::BindOnce(std::move(cb), StringHlsDataSourceStreamFactory::CreateStream(value));
            });
...
public:
        ...
        , mock_dsp_(std::make_unique<StrictMock<MockHlsDataSourceProvider>>())
    {
        ...
        ON_CALL(*mock_mdeh_, GetBufferedRanges(_)).WillByDefault(Return(Ranges<base::TimeDelta>()));

        EXPECT_CALL(*mock_dsp_, ReadFromExistingStream(_, _)).Times(0);

        base::SequenceBound<FakeHlsDataSourceProvider> dsp(task_environment_.GetMainThreadTaskRunner(), mock_dsp_.get());

        engine_ = std::make_unique<HlsManifestDemuxerEngine>(std::move(dsp), base::SingleThreadTaskRunner::GetCurrentDefault(),
            base::BindRepeating(&HlsManifestDemuxerEngineTest::AddMediaTrack, base::Unretained(this)),
            base::BindRepeating(&HlsManifestDemuxerEngineTest::RemoveMediaTrack, base::Unretained(this)), false, GURL("http://media.example.com/manifest.m3u8"),
            media_log_.get());
    }

    void InitializeEngine()
    {
        engine_->Initialize(mock_mdeh_.get(), base::BindOnce(&HlsManifestDemuxerEngineTest::MockInitComplete, base::Unretained(this)));
    }
...
{
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kInvalidMediaPlaylist);
    EXPECT_CALL(*mock_mdeh_, OnError(HasStatusCode(DEMUXER_ERROR_COULD_NOT_PARSE)));
...
TEST_F(HlsManifestDemuxerEngineTest, TestSimpleConfigAddsOnePrimaryRole)
{
    ...
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kSimpleMediaPlaylist);
    EXPECT_CALL(*this, MockInitComplete(HasStatusCode(PIPELINE_OK)));
    InitializeEngine();
    task_environment_.RunUntilIdle();
    ASSERT_TRUE(engine_->IsSeekable());
...
TEST_F(HlsManifestDemuxerEngineTest, TestSimpleLiveConfigAddsOnePrimaryRole)
{
    ...
    EXPECT_CALL(*mock_mdeh_, AddRole("primary", RelaxedParserSupportedType::kMP2T));
    EXPECT_CALL(*mock_mdeh_, RemoveRole("primary"));
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kSimpleLiveMediaPlaylist);
    EXPECT_CALL(*this, MockInitComplete(HasStatusCode(PIPELINE_OK)));
    InitializeEngine();
    task_environment_.RunUntilIdle();
...
TEST_F(HlsManifestDemuxerEngineTest, TestLivePlaybackManifestUpdates)
{
    EXPECT_CALL(*mock_mdeh_, SetSequenceMode("primary", true));
    EXPECT_CALL(*mock_mdeh_, AddRole("primary", RelaxedParserSupportedType::kMP2T));
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kInitialRequestLiveMediaPlaylist);
    ...
    // media for this test.
    EXPECT_CALL(*mock_mdeh_, AppendAndParseData("primary", _, _, _)).WillRepeatedly(Return(true));
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/a.ts", "Cheese in a cstring is string cheese.");
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/b.ts", "Tomatoes are a fruit. Ketchup is a jam.");
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/c.ts", "You've never been in an empty room.");
    ...
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kSecondRequestLiveMediaPlaylist);
...
TEST_F(HlsManifestDemuxerEngineTest, TestMultivariantPlaylistWithAlternates)
{
    ...
    EXPECT_CALL(*mock_mdeh_, SetSequenceMode("primary", true));
    EXPECT_CALL(*mock_mdeh_, SetDuration(21.021));
    EXPECT_CALL(*mock_mdeh_, AddRole("audio-override", RelaxedParserSupportedType::kMP2T));
    EXPECT_CALL(*mock_mdeh_, AddRole("primary", RelaxedParserSupportedType::kMP2T));
    ...
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kMultivariantPlaylistWithAlts);
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/eng-audio.m3u8", kSingleInfoMediaPlaylist);
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/hi/video-only.m3u8", kSimpleMediaPlaylist);
    EXPECT_CALL(*this, MockInitComplete(HasStatusCode(PIPELINE_OK)));
    InitializeEngine();
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsManifestDemuxerEngineTest, TestMultivariantWithNoSupportedCodecs)
{
    ...
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kUnsupportedCodecs);
    EXPECT_CALL(*mock_mdeh_, OnError(HasStatusCode(DEMUXER_ERROR_COULD_NOT_PARSE)));
    InitializeEngine();
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsManifestDemuxerEngineTest, TestAsyncSeek)
{
    ...
    engine_->Seek(base::Seconds(10), base::BindOnce([](ManifestDemuxer::SeekResponse resp) {
        ASSERT_TRUE(resp.has_value());
        ASSERT_EQ(std::move(resp).value(), ManifestDemuxer::SeekState::kIsReady);
    }));
    task_environment_.RunUntilIdle();
...
TEST_F(HlsManifestDemuxerEngineTest, TestMultiRenditionCheckState)
{
    ...
    EXPECT_CALL(*rend1, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(base::Seconds(7)));
    engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta r) { ASSERT_EQ(r, base::Seconds(7)); }));

    EXPECT_CALL(*rend1, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(kNoTimestamp));
    engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta r) { ASSERT_EQ(r, kNoTimestamp); }));
    ...
    EXPECT_CALL(*rend1, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(base::Seconds(7)));
    EXPECT_CALL(*rend2, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(base::Seconds(3)));
    engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta r) { EXPECT_THAT(r, CloseTo(base::Seconds(3), base::Milliseconds(1))); }));
    ...
    EXPECT_CALL(*rend1, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(base::Seconds(7)));
    EXPECT_CALL(*rend2, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(kNoTimestamp));
    engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta r) { EXPECT_THAT(r, CloseTo(base::Seconds(7), base::Milliseconds(1))); }));
}
...
TEST_F(HlsManifestDemuxerEngineTest, TestSeekDuringAdaptation)
{
    ...
    base::OnceClosure continue_adaptation = StartAndCaptureNetworkAdaptation(rendition_ptr, "http://example.com/low.m3u8", kSimpleMediaPlaylist, 1380000);
...
TEST_F(HlsManifestDemuxerEngineTest, TestSeekDuringTimeUpdate)
{
    ...
    base::OnceClosure continue_update = StartAndCaptureTimeUpdate(rendition_ptr, base::Seconds(10));
...
TEST_F(HlsManifestDemuxerEngineTest, TestAdaptDuringTimeUpdate)
{
    ...
    BindUrlAssignmentThunk<StringHlsDataSourceStreamFactory>("http://example.com/low.m3u8", kSimpleMediaPlaylist);
    std::move(continue_update).Run();
    task_environment_.RunUntilIdle();
}
...
TEST_F(HlsManifestDemuxerEngineTest, TestTimeUpdateDuringAdaptation)
{
    ...
    engine_->OnTimeUpdate(base::Seconds(0), 0.0, base::BindOnce([](base::TimeDelta r) { ASSERT_EQ(r, base::Seconds(10)); }));
    task_environment_.RunUntilIdle();

    // Set expectations for update finishing.
    EXPECT_CALL(*rendition_ptr, CheckState(_, _, _)).WillOnce(RunOnceCallback<2>(base::Seconds(10)));
...
TEST_F(HlsManifestDemuxerEngineTest, TestEndOfStreamAfterAllFetched)
{
    ...
    EXPECT_CALL(*mock_dsp_, ReadFromCombinedUrlQueue(SingleSegmentQueue("http://media.example.com/manifest.m3u8", std::nullopt), _))
        .WillOnce(RunOnceCallback<1>(StringHlsDataSourceStreamFactory::CreateStream(kShortMediaPlaylist)));
    EXPECT_CALL(*mock_dsp_, ReadFromCombinedUrlQueue(SingleSegmentQueue("http://media.example.com/first.ts", std::nullopt), _))
        .WillOnce(RunOnceCallback<1>(StringHlsDataSourceStreamFactory::CreateStream(bitstream)));
    ...
    EXPECT_CALL(*mock_mdeh_, AppendAndParseData("primary", _, _, base::as_byte_span(bitstream))).WillOnce(Return(true));
...
TEST_F(HlsManifestDemuxerEngineTest, TestInitialSegmentEncrypted)
{
    ...
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/manifest.m3u8", kLiveFullEncryptedMediaPlaylist);
    EXPECT_CALL(*this, MockInitComplete(HasStatusCode(PIPELINE_OK)));
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/K", key->key());
    BindUrlToDataSource<StringHlsDataSourceStreamFactory>("http://media.example.com/13979.js", ciphertext);
    InitializeEngine();
    task_environment_.RunUntilIdle();
}
