...
uint8_t* FrameBufferPool::GetFrameBuffer(size_t min_size, void** fb_priv)
{
    ...
    if (!registered_dump_provider_) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProviderWithSequencedTaskRunner(
            this, "FrameBufferPool", base::SequencedTaskRunnerHandle::Get(), MemoryDumpProvider::Options());
        registered_dump_provider_ = true;
    }

    // Check if a free frame buffer exists.
    auto it = base::ranges::find_if_not(frame_buffers_, &IsUsed, &std::unique_ptr<FrameBuffer>::get);
...
base::OnceClosure FrameBufferPool::CreateFrameCallback(void* fb_priv)
{
    ...
    return base::BindOnce(&FrameBufferPool::OnVideoFrameDestroyed, this, base::SequencedTaskRunnerHandle::Get(), frame_buffer);
}
...
bool FrameBufferPool::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    pmd->AddSuballocation(memory_dump->guid(), base::trace_event::MemoryDumpManager::GetInstance()->system_allocator_pool_name());
    size_t bytes_used = 0;
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    base::EraseIf(frame_buffers_, [](const std::unique_ptr<FrameBuffer>& buf) { return !IsUsed(buf.get()); });
}

void FrameBufferPool::OnVideoFrameDestroyed(scoped_refptr<base::SequencedTaskRunner> task_runner, FrameBuffer* frame_buffer)
{
    if (!task_runner->RunsTasksInCurrentSequence()) {
        task_runner->PostTask(FROM_HERE, base::BindOnce(&FrameBufferPool::OnVideoFrameDestroyed, this, task_runner, frame_buffer));
        return;
    }
    ...
    base::EraseIf(frame_buffers_,
        [now](const std::unique_ptr<FrameBuffer>& buf) { return !IsUsed(buf.get()) && now - buf->last_use_time > base::Seconds(kStaleFrameLimitSecs); });
}
