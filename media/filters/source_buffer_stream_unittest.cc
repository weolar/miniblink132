...
protected:
    ...
    template <typename ConfigT> void ResetStream(const ConfigT& config)
    {
    ...
    void SetAudioStream()
    {
        ...
        audio_config_.Initialize(
            AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    ...
    {
        AppendBuffers(starting_position, number_of_buffers, true, base::TimeDelta(), &kDataA, kDataSize);
    }

    void NewCodedFrameGroupAppend(int starting_position, int number_of_buffers, const uint8_t* data)
    {
        AppendBuffers(starting_position, number_of_buffers, true, base::TimeDelta(), data, kDataSize);
    }

    void NewCodedFrameGroupAppend_OffsetFirstBuffer(int starting_position, int number_of_buffers, base::TimeDelta first_buffer_offset)
    {
        AppendBuffers(starting_position, number_of_buffers, true, first_buffer_offset, &kDataA, kDataSize);
    }

    void AppendBuffers(int starting_position, int number_of_buffers)
    {
        AppendBuffers(starting_position, number_of_buffers, false, base::TimeDelta(), &kDataA, kDataSize);
    }

    void AppendBuffers(int starting_position, int number_of_buffers, const uint8_t* data)
    {
        AppendBuffers(starting_position, number_of_buffers, false, base::TimeDelta(), data, kDataSize);
    }
    ...
    void NewCodedFrameGroupAppend(base::TimeDelta start_timestamp, const std::string& buffers_to_append)
    {
    ...
    {
        return GarbageCollect(position * frame_duration_, new_data_buffers * kDataSize);
    }

    void RemoveInMs(int start, int end, int duration)
    {
        Remove(base::Milliseconds(start), base::Milliseconds(end), base::Milliseconds(duration));
    }

    void Remove(base::TimeDelta start, base::TimeDelta end, base::TimeDelta duration)
    {
    ...
    int GetRemovalRangeInMs(int start, int end, int bytes_to_free, int* removal_end)
    {
        base::TimeDelta removal_end_timestamp = base::Milliseconds(*removal_end);
        int bytes_removed = stream_->GetRemovalRange(base::Milliseconds(start), base::Milliseconds(end), bytes_to_free, &removal_end_timestamp);
    ...
    void CheckExpectedRangesByTimestamp(const std::string& expected, TimeGranularity granularity = TimeGranularity::kMillisecond)
    {
        Ranges<base::TimeDelta> r = stream_->GetBufferedTime();
        ...
        ss << "{ ";
        for (size_t i = 0; i < r.size(); ++i) {
            auto conversion = (granularity == TimeGranularity::kMillisecond) ? &base::TimeDelta::InMilliseconds : &base::TimeDelta::InMicroseconds;
    ...
    void CheckExpectedRangeEndTimes(const std::string& expected)
    {
        ...
        for (const auto& r : stream_->ranges_) {
            ...
            ss << "<" << highest_pts.InMilliseconds() << "," << end_time.InMilliseconds() << "> ";
        }
        ss << "}";
        EXPECT_EQ(expected, ss.str());
    }

    void CheckIsNextInPTSSequenceWithFirstRange(int64_t pts_in_ms, bool expectation)
    {
        ASSERT_GE(stream_->ranges_.size(), 1u);
        const auto& range_ptr = *(stream_->ranges_.begin());
        EXPECT_EQ(expectation, range_ptr->IsNextInPresentationSequence(base::Milliseconds(pts_in_ms)));
    }

    void CheckExpectedBuffers(int starting_position, int ending_position)
    {
        CheckExpectedBuffers(starting_position, ending_position, false, std::nullopt);
    }

    void CheckExpectedBuffers(int starting_position, int ending_position, bool expect_keyframe)
    {
        CheckExpectedBuffers(starting_position, ending_position, expect_keyframe, std::nullopt);
    }

    void CheckExpectedBuffers(int starting_position, int ending_position, const uint8_t* data)
    {
        CheckExpectedBuffers(starting_position, ending_position, false, UNSAFE_TODO(base::span(data, kDataSize)));
    }

    void CheckExpectedBuffers(int starting_position, int ending_position, const uint8_t* data, bool expect_keyframe)
    {
        CheckExpectedBuffers(starting_position, ending_position, expect_keyframe, UNSAFE_TODO(base::span(data, kDataSize)));
    }

    void CheckExpectedBuffers(int starting_position, int ending_position, bool expect_keyframe, std::optional<base::span<const uint8_t>> expected_data)
    {
        ...
        for (; current_position <= ending_position; current_position++) {
            ...
            EXPECT_EQ(base::ClampFloor(buffer->GetDecodeTimestamp() / frame_duration_), current_position);
        }

        EXPECT_EQ(ending_position + 1, current_position);
    }

    void CheckExpectedBuffers(const std::string& expected, TimeGranularity granularity = TimeGranularity::kMillisecond)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); i++) {
            ...
            if (base::EndsWith(timestamps[i], "P", base::CompareCase::SENSITIVE)) {
                ...
                ASSERT_EQ(buffer->GetDecodeTimestamp(), preroll_buffer->GetDecodeTimestamp());
    ...
    void CheckVideoConfig(const VideoDecoderConfig& config)
    {
        ...
        EXPECT_TRUE(actual.Matches(config)) << "Expected: " << config.AsHumanReadableString() << "\nActual: " << actual.AsHumanReadableString();
    }
...
private:
    ...
    void AppendBuffers(
        int starting_position, int number_of_buffers, bool begin_coded_frame_group, base::TimeDelta first_buffer_offset, const uint8_t* data, int size)
    {
        ...
        for (int i = 0; i < number_of_buffers; i++) {
            ...
            scoped_refptr<StreamParserBuffer> buffer = StreamParserBuffer::CopyFrom(data, size, is_keyframe, GetStreamType(), 0);
            base::TimeDelta timestamp = frame_duration_ * position;

            if (i == 0)
                timestamp += first_buffer_offset;
            buffer->SetDecodeTimestamp(DecodeTimestamp::FromPresentationTime(timestamp));
    ...
    void UpdateLastBufferDuration(DecodeTimestamp current_dts, BufferQueue* buffers)
    {
    ...
    {
        std::vector<std::string> timestamps = base::SplitString(buffers_to_append, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        ...
        for (size_t i = 0; i < timestamps.size(); i++) {
            ...
            if (duration_pos != std::string::npos) {
                ...
                CHECK(base::StringToInt(timestamps[i].substr(duration_pos + 1), &duration_in_us));
                if (!is_duration_us)
                    duration_in_us *= base::Time::kMicrosecondsPerMillisecond;
                timestamps[i] = timestamps[i].substr(0, duration_pos);
            }

            std::vector<std::string> buffer_timestamp_strings = base::SplitString(timestamps[i], "|", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
            ...
            for (size_t j = 0; j < 2; ++j) {
                ...
                if (base::EndsWith(buffer_timestamp_strings[j], "u", base::CompareCase::SENSITIVE)) {
                    is_us = true;
                    buffer_timestamp_strings[j] = buffer_timestamp_strings[j].substr(0, buffer_timestamp_strings[j].length() - 1);
                }
            ...
            scoped_refptr<StreamParserBuffer> buffer = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, GetStreamType(), 0);
            ...
            if (buffer_timestamps[1] != buffer_timestamps[0]) {
                buffer->SetDecodeTimestamp(DecodeTimestamp::FromPresentationTime(buffer_timestamps[1]));
            }
            ...
            // it as the preroll.
            if (has_preroll) {
                scoped_refptr<StreamParserBuffer> preroll_buffer = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, GetStreamType(), 0);
    ...
    void AppendBuffers(
        const std::string& buffers_to_append, bool start_new_coded_frame_group, base::TimeDelta coded_frame_group_start_timestamp, bool one_by_one)
    {
...
TEST_F(SourceBufferStreamTest, Complete_Overlap_AfterGroupTimestampAndBeforeFirstBufferTimestamp)
{
...
TEST_F(SourceBufferStreamTest, Overlap_That_Prunes_All_of_Previous_TrackBuffer)
{
...
TEST_F(SourceBufferStreamTest, Seek_StartOfGroup)
{
    ...
    EXPECT_STATUS_FOR_STREAM_OP(kSuccess, GetNextBuffer(&buffer));
    EXPECT_EQ(buffer->GetDecodeTimestamp(), DecodeTimestamp::FromPresentationTime(5 * frame_duration() + bump));
...
TEST_F(SourceBufferStreamTest, OldSeekPoint_MiddleOverlap)
{
    ...
    NewCodedFrameGroupAppend("50K 90|60 60|70 70|80 80|90 "
    ...
    CheckExpectedBuffers("50K 90|60 60|70 70|80 80|90 100K 150K 190|160 160|170 170|180 180|190");
    CheckNoNextBuffer();
}
...
TEST_F(SourceBufferStreamTest, PresentationTimestampIndependence)
{
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (buffer->is_key_frame()) {
            EXPECT_EQ(DecodeTimestamp::FromPresentationTime(buffer->timestamp()), buffer->GetDecodeTimestamp());
            last_keyframe_idx = i;
            ...
            EXPECT_LT(last_keyframe_presentation_timestamp, last_p_frame_presentation_timestamp);
        } else {
            ...
            EXPECT_LT(DecodeTimestamp::FromPresentationTime(buffer->timestamp()), buffer->GetDecodeTimestamp());
        }
    }
}
...
TEST_F(SourceBufferStreamTest, GarbageCollection_DeleteFront_PreserveSeekedGOP)
{
    ...
    EXPECT_FALSE(GarbageCollect(base::Milliseconds(95), 7 * GetMemoryUsagePerBuffer()));
...
TEST_F(SourceBufferStreamTest, GarbageCollection_MediaTimeOutsideOfStreamBufferedRange)
{
    // Set memory limit to 10 buffers.
    SetMemoryLimit(10);
...
TEST_F(SourceBufferStreamTest, GetRemovalRange_BytesToFree)
{
    ...
    bytes_removed = GetRemovalRangeInMs(300, 1080, GetMemoryUsagePerBuffer(), &remove_range_end);
    ...
    bytes_removed = GetRemovalRangeInMs(300, 1080, 3 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(390, remove_range_end);
    EXPECT_EQ(3 * GetMemoryUsagePerBuffer(), bytes_removed);

    // The same size with a range.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 6 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(480, remove_range_end);
    EXPECT_EQ(6 * GetMemoryUsagePerBuffer(), bytes_removed);

    // A frame larger than a range.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 7 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(690, remove_range_end);
    EXPECT_EQ(9 * GetMemoryUsagePerBuffer(), bytes_removed);

    // The same size with two ranges.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 12 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(780, remove_range_end);
    EXPECT_EQ(12 * GetMemoryUsagePerBuffer(), bytes_removed);

    // Larger than two ranges.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 14 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(990, remove_range_end);
    EXPECT_EQ(15 * GetMemoryUsagePerBuffer(), bytes_removed);

    // The same size with the whole ranges.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 18 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(1080, remove_range_end);
    EXPECT_EQ(18 * GetMemoryUsagePerBuffer(), bytes_removed);

    // Larger than the whole ranges.
    bytes_removed = GetRemovalRangeInMs(300, 1080, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(1080, remove_range_end);
    EXPECT_EQ(18 * GetMemoryUsagePerBuffer(), bytes_removed);
}
...
TEST_F(SourceBufferStreamTest, GetRemovalRange_Range)
{
    ...
    bytes_removed = GetRemovalRangeInMs(630, 660, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(-1, remove_range_end);
    EXPECT_EQ(0, bytes_removed);

    // Across a GOP and no keyframe.
    bytes_removed = GetRemovalRangeInMs(630, 750, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(-1, remove_range_end);
    EXPECT_EQ(0, bytes_removed);

    // The same size with a range.
    bytes_removed = GetRemovalRangeInMs(600, 780, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(780, remove_range_end);
    EXPECT_EQ(6 * GetMemoryUsagePerBuffer(), bytes_removed);

    // One frame larger than a range.
    bytes_removed = GetRemovalRangeInMs(570, 810, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(780, remove_range_end);
    EXPECT_EQ(6 * GetMemoryUsagePerBuffer(), bytes_removed);

    // Facing the other ranges.
    bytes_removed = GetRemovalRangeInMs(480, 900, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(780, remove_range_end);
    EXPECT_EQ(6 * GetMemoryUsagePerBuffer(), bytes_removed);

    // In the midle of the other ranges, but not including any GOP.
    bytes_removed = GetRemovalRangeInMs(420, 960, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(780, remove_range_end);
    EXPECT_EQ(6 * GetMemoryUsagePerBuffer(), bytes_removed);

    // In the middle of the other ranges.
    bytes_removed = GetRemovalRangeInMs(390, 990, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(990, remove_range_end);
    EXPECT_EQ(12 * GetMemoryUsagePerBuffer(), bytes_removed);

    // A frame smaller than the whole ranges.
    bytes_removed = GetRemovalRangeInMs(330, 1050, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(990, remove_range_end);
    EXPECT_EQ(12 * GetMemoryUsagePerBuffer(), bytes_removed);
    ...
    // Larger than the whole ranges.
    bytes_removed = GetRemovalRangeInMs(270, 1110, 20 * GetMemoryUsagePerBuffer(), &remove_range_end);
    EXPECT_EQ(1080, remove_range_end);
    EXPECT_EQ(18 * GetMemoryUsagePerBuffer(), bytes_removed);
}
...
TEST_F(SourceBufferStreamTest, IsNextBufferConfigChanged)
{
    ...
    // Overlap-append
    NewCodedFrameGroupAppend("21K 41 51 61 71 81 91 101 111 121 "
                             "131K 141");
...
TEST_F(SourceBufferStreamTest, SetExplicitDuration_DeletePartialRange)
{
    ...
    NewCodedFrameGroupAppend("100K 140|110 110|120 120|130 130|140 "
                             "150K 190|160 160|170 170|180 180|190");
...
TEST_F(SourceBufferStreamTest, SetExplicitDuration_AfterGroupTimestampAndBeforeFirstBufferTimestamp)
{
...
{
    AudioDecoderConfig config(AudioCodec::kMP3, kSampleFormatF32, CHANNEL_LAYOUT_STEREO, 44100, EmptyExtraData(), EncryptionScheme::kUnencrypted);
    ResetStream<>(config);
    Seek(0);
...
TEST_F(SourceBufferStreamTest, Remove_PreviousAppendDestroyedAndOverwriteExistingRange)
{
    SeekToTimestampMs(90);
...
TEST_F(SourceBufferStreamTest, OverlappingAppendRangeMembership_OneMicrosecond_Video)
{
    NewCodedFrameGroupAppend("10D20K");
    CheckExpectedRangesByTimestamp("{ [10000,30000) }", TimeGranularity::kMicrosecond);

    // Append a buffer 1 microsecond earlier, with estimated duration.
    NewCodedFrameGroupAppend("9999uD20EK");
    CheckExpectedRangesByTimestamp("{ [9999,30000) }", TimeGranularity::kMicrosecond);
...
TEST_F(SourceBufferStreamTest, OverlappingAppendRangeMembership_TwoMicroseconds_Video)
{
    NewCodedFrameGroupAppend("10D20K");
    CheckExpectedRangesByTimestamp("{ [10000,30000) }", TimeGranularity::kMicrosecond);
    ...
    CheckExpectedRangesByTimestamp("{ [9998,30000) }", TimeGranularity::kMicrosecond);
...
TEST_F(SourceBufferStreamTest, Audio_SpliceTrimming_ExistingTrimming)
{
    ...
    scoped_refptr<StreamParserBuffer> bufferA1 = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, DemuxerStream::AUDIO, 0);
    ...
    scoped_refptr<StreamParserBuffer> bufferA2 = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, DemuxerStream::AUDIO, 0);
    ...
    scoped_refptr<StreamParserBuffer> bufferB1 = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, DemuxerStream::AUDIO, 0);
    ...
    scoped_refptr<StreamParserBuffer> bufferB2 = StreamParserBuffer::CopyFrom(&kDataA, kDataSize, is_keyframe, DemuxerStream::AUDIO, 0);
    ...
    EXPECT_EQ(std::make_pair(kNoDiscard, kNoDiscard), read_buffer->discard_padding());
...
TEST_F(SourceBufferStreamTest, Audio_SpliceFrame_NoMillisecondSplices)
{
    ...
    video_config_ = TestVideoConfig::Invalid();
    audio_config_.Initialize(
        AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 4000, EmptyExtraData(), EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    ResetStream<>(audio_config_);
...
{
    AudioDecoderConfig new_config(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_MONO, 2000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
    SetAudioStream();
    Seek(0);
...
TEST_F(SourceBufferStreamTest, Audio_Opus_SeekToJustBeforeRangeStart)
{
    ...
    video_config_ = TestVideoConfig::Invalid();
    audio_config_.Initialize(
        AudioCodec::kOpus, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted, base::Milliseconds(10), 0);
    ResetStream<>(audio_config_);
...
TEST_F(SourceBufferStreamTest, ReSeekToStartSatisfiedUpToThreshold_SameTimestamps)
{
    // Append a few buffers.
    NewCodedFrameGroupAppend("999K 1010 1020D10");
    CheckExpectedRangesByTimestamp("{ [999,1030) }");
...
TEST_F(SourceBufferStreamTest, ReSeekToStartSatisfiedUpToThreshold_EarlierTimestamps)
{
    // Append a few buffers.
    NewCodedFrameGroupAppend("999K 1010 1020D10");
    CheckExpectedRangesByTimestamp("{ [999,1030) }");
...
TEST_F(SourceBufferStreamTest, ReSeekToStartSatisfiedUpToThreshold_LaterTimestamps)
{
    // Append a few buffers.
    NewCodedFrameGroupAppend("500K 510 520D10");
    CheckExpectedRangesByTimestamp("{ [500,530) }");
...
TEST_F(SourceBufferStreamTest, TrackBuffer_ExhaustionWithSkipForward)
{
    ...
    NewCodedFrameGroupAppend("31K 41 51 61 71 81 91 101 111 121 "
                             "131K 141");
    CheckExpectedRangesByTimestamp("{ [0,151) }");
...
TEST_F(SourceBufferStreamTest, TrackBuffer_ExhaustionAndImmediateNewTrackBuffer)
{
    NewCodedFrameGroupAppend("0K 10 20 30 40");
    ...
    NewCodedFrameGroupAppend("22K 32 42 52 62 72 82 92 102 112 122K 132 142 152K 162");
...
TEST_F(SourceBufferStreamTest, AdjacentCodedFrameGroupContinuation_NoGapCreatedByTinyGapInGroupContinuation)
{
...
TEST_F(SourceBufferStreamTest, AdjacentCodedFrameGroupContinuation_NoGapCreatedPrefixRemoved)
{
    NewCodedFrameGroupAppend("0K 10 20K 30 40K 50D10");
    CheckExpectedRangesByTimestamp("{ [0,60) }");
...
TEST_F(SourceBufferStreamTest, AdjacentNewCodedFrameGroupContinuation_NoGapCreatedPrefixRemoved)
{
    NewCodedFrameGroupAppend("0K 10 20K 30 40K 50D10");
    CheckExpectedRangesByTimestamp("{ [0,60) }");
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_RemoveThenAppendMoreMuchLater)
{
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_InExisting_AppendMuchLater)
{
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_InExisting_RemoveGOP_ThenAppend_1)
{
    NewCodedFrameGroupAppend("0K 10 20 30K 40 50");
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_InExisting_RemoveGOP_ThenAppend_2)
{
    NewCodedFrameGroupAppend("0K 10 20 30K 40 50");
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_InExisting_RemoveMostRecentAppend_ThenAppend_1)
{
    NewCodedFrameGroupAppend("0K 10 20 30K 40 50");
    SignalStartOfCodedFrameGroup(base::Milliseconds(45));
    RemoveInMs(50, 60, 60);
...
TEST_F(SourceBufferStreamTest, StartCodedFrameGroup_InExisting_RemoveMostRecentAppend_ThenAppend_2)
{
    NewCodedFrameGroupAppend("0K 10 20 30K 40 50");
...
TEST_F(SourceBufferStreamTest, GarbageCollectionUnderMemoryPressure)
{
    ...
    stream_->OnMemoryPressure(base::Milliseconds(0), base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_MODERATE, false);
    ...
    // playback position.
    stream_->OnMemoryPressure(base::Milliseconds(0), base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL, false);
...
TEST_F(SourceBufferStreamTest, InstantGarbageCollectionUnderMemoryPressure)
{
    ...
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(kMemoryPressureBasedSourceBufferGC);
    stream_->OnMemoryPressure(base::Milliseconds(7), base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL, true);
    CheckExpectedRangesByTimestamp("{ [6,16) }");
    stream_->OnMemoryPressure(base::Milliseconds(9), base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL, true);
    CheckExpectedRangesByTimestamp("{ [9,16) }");
}
...
TEST_F(SourceBufferStreamTest, BFrames_WithoutEditList)
{
    ...
    Seek(0);
    NewCodedFrameGroupAppend(base::Milliseconds(60), "60|0K 180|30 90|60 120|90 150|120");
...
TEST_F(SourceBufferStreamTest, VideoRangeEndTimeCases)
{
    ...
    const VideoEndTimeCase kCases[] = { { 0, 10, 10, 20 }, { 20, 1, 20, 21 }, { 15, 3, 15, 18 }, { 15, 5, 15, 20 }, { 15, 8, 15, 23 },

        // Cases where the new frame removes the previous frame:
        { 10, 3, 10, 13 }, { 10, 10, 10, 20 }, { 10, 13, 10, 23 }, { 5, 8, 5, 13 }, { 5, 15, 5, 20 }, { 5, 20, 5, 25 } };
...
TEST_F(SourceBufferStreamTest, AudioRangeEndTimeCases)
{
    ...
    const AudioEndTimeCase kCases[]
        = { { 0, 10, 10, 20, false }, { 20, 1, 20, 21, false }, { 15, 3, 15, 18, true }, { 15, 5, 15, 20, true }, { 15, 8, 15, 23, true },

              // Cases where the new frame removes the previous frame:
              { 10, 3, 10, 13, false }, { 10, 10, 10, 20, false }, { 10, 13, 10, 23, false }, { 5, 8, 5, 13, false }, { 5, 15, 5, 20, false },
              { 5, 20, 5, 25, false } };
    ...
    for (const auto& c : kCases) {
        ...
        ss << c.new_frame_pts << "D" << c.new_frame_duration << "K";
        if (c.expect_splice) {
            EXPECT_MEDIA_LOG(TrimmedSpliceOverlap(c.new_frame_pts * 1000, 10000, (20 - c.new_frame_pts) * 1000));
        }
        DVLOG(1) << "Appending " << ss.str();
        NewCodedFrameGroupAppend(ss.str());
...
TEST_F(SourceBufferStreamTest, RangeCoalescenceOnFudgeRoomIncrease_1)
{
    ...
    CheckExpectedRangesByTimestamp("{ [0,120) [500,520) [1000,1044) [2000,2045) }");
...
TEST_F(SourceBufferStreamTest, ZeroDurationBuffersThenIncreasingFudgeRoom)
{
    ...
    CheckExpectedRangesByTimestamp("{ [0,2) [5000,5001) }", TimeGranularity::kMicrosecond);
    NewCodedFrameGroupAppend("2002uD0K");
    CheckExpectedRangesByTimestamp("{ [0,2) [2002,2003) [5000,5001) }", TimeGranularity::kMicrosecond);

    // Grow the fudge room enough to coalesce the first two ranges.
    NewCodedFrameGroupAppend("8000uD1001uK");
    CheckExpectedRangesByTimestamp("{ [0,2003) [5000,5001) [8000,9001) }", TimeGranularity::kMicrosecond);
    ...
    CheckExpectedRangesByTimestamp("{ [0,9001) [100000,104000) }", TimeGranularity::kMicrosecond);
    SeekToTimestampMs(0);
    CheckExpectedBuffers("0K 1K 2002K 5000K 8000K", TimeGranularity::kMicrosecond);
    CheckNoNextBuffer();
    SeekToTimestampMs(100);
...
TEST_F(SourceBufferStreamTest, NonZeroDurationBuffersThenIncreasingFudgeRoom)
{
    ...
    CheckExpectedRangesByTimestamp("{ [0,10) [50,60) [100,110) [150,160) [500,510) }");
...
TEST_F(SourceBufferStreamTest, MergeAllowedIfRangeEndTimeWithEstimatedDurationMatchesNextRangeStart)
{
