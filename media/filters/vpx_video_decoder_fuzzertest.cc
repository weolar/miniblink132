...
void OnDecodeComplete(base::OnceClosure quit_closure, media::DecoderStatus status)
{
    std::move(quit_closure).Run();
}

void OnInitDone(base::OnceClosure quit_closure, bool* success_dest, media::DecoderStatus status)
{
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data_ptr, size_t size)
{
    ...
    auto profile = static_cast<media::VideoCodecProfile>(rng() % media::VIDEO_CODEC_PROFILE_MAX);
    auto color_space
        = media::VideoColorSpace(rng() % 256, rng() % 256, rng() % 256, (rng() & 1) ? gfx::ColorSpace::RangeID::LIMITED : gfx::ColorSpace::RangeID::FULL);
    auto rotation = static_cast<media::VideoRotation>(rng() % media::VIDEO_ROTATION_MAX);
    ...
    media::VideoDecoderConfig config(codec, profile,
        has_alpha ? media::VideoDecoderConfig::AlphaMode::kHasAlpha : media::VideoDecoderConfig::AlphaMode::kIsOpaque, color_space,
        media::VideoTransformation(rotation, reflection), coded_size, visible_rect, natural_size, media::EmptyExtraData(),
...

    {
        ...
        bool success = false;
        decoder.Initialize(config, true /* low_delay */, nullptr /* cdm_context */, base::BindOnce(&OnInitDone, run_loop.QuitClosure(), &success),
...

    {
        ...
        decoder.Decode(buffer, base::BindOnce(&OnDecodeComplete, run_loop.QuitClosure()));
        run_loop.Run();
    }
