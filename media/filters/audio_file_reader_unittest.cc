...
public:
    ...
    void ReadAndVerify(const char* expected_audio_hash, int expected_frames)
    {
        ...
        std::unique_ptr<AudioBus> decoded_audio_data = AudioBus::Create(reader_->channels(), actual_frames);
        ...
        for (size_t k = 0; k < decoded_audio_packets.size(); ++k) {
            ...
            packet->CopyPartialFramesTo(0, frame_count, dest_start_frame, decoded_audio_data.get());
    ...
    void VerifyPackets(int packet_reads)
    {
        ...
        for (int i = 0; i < kTestPasses; ++i) {
            for (int j = 0; j < packet_reads; ++j) {
                ...
                const std::string md5_hash = base::MD5String(std::string_view(reinterpret_cast<char*>(packet->data), packet->size));
                if (i == 0) {
                    packet_md5_hashes_.push_back(md5_hash);
                    if (j == 0) {
                        start_timestamp = ConvertFromTimeBase(reader_->codec_context_for_testing()->time_base, packet->pts);
                    }
                } else {
    ...
    void RunTest(
        const char* fn, const char* hash, int channels, int sample_rate, base::TimeDelta duration, int frames, int expected_frames, int packet_reads = 3)
    {
        Initialize(fn);
        ...
        if (frames >= 0) {
            EXPECT_EQ(duration.InMicroseconds(), reader_->GetDuration().InMicroseconds());
...
{
    RunTest("bear-320x240-live.webm", "-3.59,-2.06,-0.43,2.15,0.77,-0.95,", 2, 44100, base::Microseconds(-1), -1, 121024);
}

TEST_F(AudioFileReaderTest, WithVideo)
{
    RunTest("bear.ogv", "-0.73,0.92,0.48,-0.07,-0.92,-0.88,", 2, 44100, base::Microseconds(1011520), 44609, 45632);
}

TEST_F(AudioFileReaderTest, Vorbis)
{
    RunTest("sfx.ogg", "2.17,3.31,5.15,6.33,5.97,4.35,", 1, 44100, base::Microseconds(350001), 15436, 15936);
}

TEST_F(AudioFileReaderTest, WaveU8)
{
    RunTest("sfx_u8.wav", "-1.23,-1.57,-1.14,-0.91,-0.87,-0.07,", 1, 44100, base::Microseconds(288414), 12720, 12719);
}

TEST_F(AudioFileReaderTest, WaveS16LE)
{
    RunTest("sfx_s16le.wav", "3.05,2.87,3.00,3.32,3.58,4.08,", 1, 44100, base::Microseconds(288414), 12720, 12719);
}

TEST_F(AudioFileReaderTest, WaveS24LE)
{
    RunTest("sfx_s24le.wav", "3.03,2.86,2.99,3.31,3.57,4.06,", 1, 44100, base::Microseconds(288414), 12720, 12719);
}

TEST_F(AudioFileReaderTest, WaveF32LE)
{
    RunTest("sfx_f32le.wav", "3.03,2.86,2.99,3.31,3.57,4.06,", 1, 44100, base::Microseconds(288414), 12720, 12719);
}

TEST_F(AudioFileReaderTest, MP3)
{
    RunTest("sfx.mp3", "1.30,2.72,4.56,5.08,3.74,2.03,", 1, 44100, base::Microseconds(313470), 13825, 11025);
}
...
TEST_F(AudioFileReaderTest, CorruptMP3)
{
    ...
    RunTest("corrupt.mp3", "-4.95,-2.95,-0.44,1.16,0.31,-2.21,", 1, 44100, base::Microseconds(1018801), 44930, 44928);
}

#if BUILDFLAG(USE_PROPRIETARY_CODECS)
TEST_F(AudioFileReaderTest, AAC)
{
    RunTest("sfx.m4a", "0.79,2.31,4.15,4.92,4.04,1.44,", 1, 44100, base::Microseconds(347665), 15333, 12701);
}

TEST_F(AudioFileReaderTest, AAC_SinglePacket)
{
    RunTest("440hz-10ms.m4a", "3.77,4.53,4.75,3.48,3.67,3.76,", 1, 44100, base::Microseconds(69660), 3073, 441);
}

TEST_F(AudioFileReaderTest, AAC_ADTS)
{
    RunTest("sfx.adts", "1.80,1.66,2.31,3.26,4.46,3.36,", 1, 44100, base::Microseconds(384733), 16967, 13312);
}
...
{
    RunTest("4ch.wav", "131.71,38.02,130.31,44.89,135.98,42.52,", 4, 44100, base::Microseconds(100001), 4411, 4410, /*packet_reads=*/2);
}
