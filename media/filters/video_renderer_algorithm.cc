...
VideoRendererAlgorithm::ReadyFrame::ReadyFrame(scoped_refptr<VideoFrame> ready_frame)
...
bool VideoRendererAlgorithm::ReadyFrame::operator<(const ReadyFrame& other) const
{
    return frame->timestamp() < other.frame->timestamp();
}

VideoRendererAlgorithm::VideoRendererAlgorithm(const TimeSource::WallClockTimeCB& wall_clock_time_cb, MediaLog* media_log)
    : media_log_(media_log)
    , cadence_estimator_(base::Seconds(kMinimumAcceptableTimeBetweenGlitchesSecs))
...
scoped_refptr<VideoFrame> VideoRendererAlgorithm::Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, size_t* frames_dropped)
{
    ...
    if (frame_to_render >= 0) {
        cadence_frame_drift = selected_frame_drift = CalculateAbsoluteDriftForFrame(deadline_min, frame_to_render);
    }
    ...
    if (frame_to_render < 0 || selected_frame_drift > max_acceptable_drift_) {
        ...
        const int best_by_coverage = FindBestFrameByCoverage(deadline_min, deadline_max, &second_best_by_coverage);
        ...
        if (frame_to_render == best_by_coverage && second_best_by_coverage >= 0
            && CalculateAbsoluteDriftForFrame(deadline_min, second_best_by_coverage) <= selected_frame_drift) {
    ...
    if (ignored_cadence_frame) {
        DVLOG(2) << "Cadence frame " << frame_queue_[cadence_frame].frame->timestamp() << " (" << cadence_frame
                 << ") overridden by drift: " << cadence_frame_drift.InMillisecondsF() << "ms, using " << frame_queue_[frame_to_render].frame->timestamp()
                 << "(" << frame_to_render << ") instead.";
    }

    last_render_had_glitch_ = selected_frame_drift > max_acceptable_drift_;
    DVLOG_IF(2, last_render_had_glitch_) << "Frame drift is too far: " << selected_frame_drift.InMillisecondsF() << "ms";
    ...
    if (cadence_estimator_.has_cadence()) {
        ...
        if (frame_to_render && last_frame_info.render_count < last_frame_info.ideal_render_count) {
            ...
            DVLOG(2) << "Under-rendered frame " << last_frame_info.frame->timestamp() << "; only " << last_frame_info.render_count << " times instead of "
            ...
            DVLOG(2) << "Over-rendered frame " << last_frame_info.frame->timestamp() << "; rendered " << last_frame_info.render_count + 1
    ...
    if (frame_to_render > 0) {
        if (frames_dropped) {
            for (int i = 0; i < frame_to_render; ++i) {
                ...
                DVLOG(2) << "Dropping unrendered (or always dropped) frame " << frame.frame->timestamp()
                         << ", wall clock: " << frame.start_time.ToInternalValue() << " (" << frame.render_count << ", " << frame.drop_count << ")";
                ++(*frames_dropped);
                if (!cadence_estimator_.has_cadence() || frame.ideal_render_count)
                    last_render_had_glitch_ = true;
            }
        ...
        frame_queue_.erase(frame_queue_.begin(), frame_queue_.begin() + frame_to_render);
    }

    if (last_render_had_glitch_ && !first_frame_) {
        DVLOG(2) << "Deadline: [" << deadline_min.ToInternalValue() << ", " << deadline_max.ToInternalValue()
                 << "], Interval: " << render_interval_.InMicroseconds() << ", Duration: " << average_frame_duration_.InMicroseconds();
    }
...
size_t VideoRendererAlgorithm::RemoveExpiredFrames(base::TimeTicks deadline)
{
    ...
    const size_t frames_to_expire = first_good_frame < 0 ? frame_queue_.size() - 1 : first_good_frame;
    if (!frames_to_expire)
        return 0;
    ...
    for (size_t i = 0; i < frames_to_expire; ++i) {
        ...
        if (frame.render_count == frame.drop_count && (!cadence_estimator_.has_cadence() || frame.ideal_render_count)) {
            ++frames_dropped_without_rendering;
        }
    }

    cadence_frame_counter_ += frames_to_expire;
    frame_queue_.erase(frame_queue_.begin(), frame_queue_.begin() + frames_to_expire);
...
int64_t VideoRendererAlgorithm::GetMemoryUsage() const
{
    ...
    for (const auto& ready_frame : frame_queue_) {
        allocation_size += VideoFrame::AllocationSize(ready_frame.frame->format(), ready_frame.frame->coded_size());
    }
...
void VideoRendererAlgorithm::EnqueueFrame(scoped_refptr<VideoFrame> frame)
{
    ...
    base::TimeDelta metadata_frame_duration = frame->metadata().frame_duration.value_or(base::TimeDelta());
    auto timestamp = frame->timestamp();
    ReadyFrame ready_frame(std::move(frame));
    auto it = frame_queue_.empty() ? frame_queue_.end() : std::lower_bound(frame_queue_.begin(), frame_queue_.end(), ready_frame);
    ...
    if (new_frame_index <= 0 && have_rendered_frames_) {
        LIMITED_MEDIA_LOG(INFO, media_log_, out_of_order_frame_logs_, kMaxOutOfOrderFrameLogs)
            << "Dropping frame with timestamp " << timestamp << ", which is earlier than the last rendered frame (" << frame_queue_.front().frame->timestamp()
            << ").";
    ...
    const base::TimeDelta delta
        = std::min(new_frame_index < frame_queue_.size() ? frame_queue_[new_frame_index].frame->timestamp() - timestamp : base::TimeDelta::Max(),
            new_frame_index > 0 ? timestamp - frame_queue_[new_frame_index - 1].frame->timestamp() : base::TimeDelta::Max());
    if (delta < base::Milliseconds(1)) {
        DVLOG(2) << "Dropping frame too close to an already enqueued frame: " << delta.InMicroseconds() << " us";
        ++frames_dropped_during_enqueue_;
        return;
    }
    ...
    if (!frame_duration_calculator_.Count() && metadata_frame_duration.is_positive()) {
...
void VideoRendererAlgorithm::AccountForMissedIntervals(base::TimeTicks deadline_min, base::TimeTicks deadline_max)
{
    ...
    const int64_t render_cycle_count = (deadline_min - last_deadline_max_).IntDiv(render_interval_);
...
void VideoRendererAlgorithm::UpdateFrameStatistics()
{
    ...
    bool have_metadata_duration = false;
    {
        ...
        base::TimeDelta metadata_frame_duration = last_frame->metadata().frame_duration.value_or(base::TimeDelta());
        if (metadata_frame_duration.is_positive()) {
            have_metadata_duration = true;
            media_timestamps.push_back(last_frame->timestamp() + metadata_frame_duration);
        }
    }

    std::vector<base::TimeTicks> wall_clock_times;
    was_time_moving_ = wall_clock_time_cb_.Run(media_timestamps, &wall_clock_times);
    DCHECK_EQ(wall_clock_times.size(), frame_queue_.size() + (have_metadata_duration ? 1 : 0));
    ...
    // for 60fps content.
    max_acceptable_drift_ = std::max(average_frame_duration_ / 2, base::Seconds(1.0 / 60));
    ...
    const bool cadence_changed = cadence_estimator_.UpdateCadenceEstimate(render_interval_, average_frame_duration_, deviation, max_acceptable_drift_);
...
void VideoRendererAlgorithm::UpdateCadenceForFrames()
{
    for (size_t i = 0; i < frame_queue_.size(); ++i) {
        ...
        // cadence selection.
        frame_queue_[i].ideal_render_count = cadence_estimator_.has_cadence() ? cadence_estimator_.GetCadenceForFrame(cadence_frame_counter_ + i) : 0;
    }
}
...
int VideoRendererAlgorithm::FindBestFrameByCoverage(base::TimeTicks deadline_min, base::TimeTicks deadline_max, int* second_best) const
{
    DCHECK(!frame_queue_.empty());
...
int VideoRendererAlgorithm::FindBestFrameByDrift(base::TimeTicks deadline_min, base::TimeDelta* selected_frame_drift) const
{
    DCHECK(!frame_queue_.empty());
...
base::TimeDelta VideoRendererAlgorithm::CalculateAbsoluteDriftForFrame(base::TimeTicks deadline_min, int frame_index) const
{
...
void VideoRendererAlgorithm::UpdateEffectiveFramesQueued()
{
    ...
    if (frame_dropping_disabled_) {
        min_frames_queued = base::ranges::count(frame_queue_, 0, &ReadyFrame::render_count);
    }
...
{
    const auto minimum_start_time = cadence_estimator_.has_cadence() ? last_deadline_max_ - max_acceptable_drift_ : last_deadline_max_;

    size_t start_index = 0;
    for (; start_index < frame_queue_.size(); ++start_index) {
        const ReadyFrame& frame = frame_queue_[start_index];
        if ((!cadence_estimator_.has_cadence() || frame.render_count < frame.ideal_render_count)
