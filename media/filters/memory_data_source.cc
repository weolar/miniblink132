...
void MemoryDataSource::Read(int64_t position, int size, uint8_t* data, DataSource::ReadCB read_cb)
{
    DCHECK(read_cb);
    ...
    size_t clamped_size = std::min(static_cast<size_t>(size), size_ - static_cast<size_t>(position));
