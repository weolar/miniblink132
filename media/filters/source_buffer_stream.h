...
enum class SourceBufferStreamStatus { kSuccess, kNeedBuffer, kConfigChange, kEndOfStream };
...
public:
    ...
    static void GetTimestampInterval(const BufferQueue& buffers, base::TimeDelta* start, base::TimeDelta* end);

    SourceBufferStream(const AudioDecoderConfig& audio_config, MediaLog* media_log);
    SourceBufferStream(const VideoDecoderConfig& video_config, MediaLog* media_log);
    ...
    SourceBufferStreamStatus GetNextBuffer(scoped_refptr<StreamParserBuffer>* out_buffer);
    ...
    bool UpdateAudioConfig(const AudioDecoderConfig& config, bool allow_codec_change);

    // Notifies this object that the video config has changed and buffers in
    // future Append() calls should be associated with this new config.
    // If the codec is allowed to change, the caller should set
    // |allow_codec_change| to true.
    bool UpdateVideoConfig(const VideoDecoderConfig& config, bool allow_codec_change);
    ...
    {
        DVLOG(2) << __func__ << ": Override memory limit from " << memory_limit_ << " to " << memory_limit << ".";
...
private:
    ...
    size_t FreeBuffers(size_t total_bytes_to_free, base::TimeDelta media_time, bool reverse_direction);
    ...
    size_t FreeBuffersAfterLastAppended(size_t total_bytes_to_free, base::TimeDelta media_time);
    ...
    size_t GetRemovalRange(base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t total_bytes_to_free, base::TimeDelta* removal_end_timestamp);
    ...
    void PrepareRangesForNextAppend(const BufferQueue& new_buffers, BufferQueue* deleted_buffers);
    ...
    void MergeWithNextRangeIfNecessary(const RangeList::iterator& range_with_new_buffers_itr);
    ...
    bool IsNextGopAdjacentToEndOfCurrentAppendSequence(base::TimeDelta next_gop_timestamp) const;
    ...
    // |range|.
    void SeekAndSetSelectedRange(SourceBufferRange* range, base::TimeDelta seek_timestamp);
    ...
    // found.
    base::TimeDelta FindNewSelectedRangeSeekTimestamp(const base::TimeDelta start_timestamp);
    ...
    void RemoveInternal(base::TimeDelta start, base::TimeDelta end, bool exclude_start, BufferQueue* deleted_buffers);
    ...
    void UpdateLastAppendStateForRemove(base::TimeDelta remove_start, base::TimeDelta remove_end, bool exclude_start);

    SourceBufferStreamType GetType() const;

    // See GetNextBuffer() for additional details.  This method handles preroll
    // frame processing.
    SourceBufferStreamStatus HandleNextBufferWithPreroll(scoped_refptr<StreamParserBuffer>* out_buffer);
    ...
    SourceBufferStreamStatus GetNextBufferInternal(scoped_refptr<StreamParserBuffer>* out_buffer);
    ...
    void WarnIfTrackBufferExhaustionSkipsForward(scoped_refptr<StreamParserBuffer> next_buffer);
    ...
    base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level_ = base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_NONE;
