...
public:
    DecryptingVideoDecoderTest()
        : decoder_(std::make_unique<DecryptingVideoDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_))
        ...
        , decoded_video_frame_(VideoFrame::CreateBlackFrame(TestVideoConfig::NormalCodedSize()))
        , null_video_frame_(scoped_refptr<VideoFrame>())
    {
    }

    DecryptingVideoDecoderTest(const DecryptingVideoDecoderTest&) = delete;
    DecryptingVideoDecoderTest& operator=(const DecryptingVideoDecoderTest&) = delete;

    ~DecryptingVideoDecoderTest() override
    {
        Destroy();
    }
    ...
    // can succeed or fail.
    void InitializeAndExpectResult(const VideoDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, false, cdm_context_.get(),
            ...
            base::BindRepeating(&DecryptingVideoDecoderTest::FrameReady, base::Unretained(this)),
            base::BindRepeating(&DecryptingVideoDecoderTest::OnWaiting, base::Unretained(this)));
    ...
    void Initialize(const VideoDecoderConfig& config)
    {
        ...
        EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillOnce(RunOnceCallback<1>(true));
    ...
    void DecodeAndExpect(scoped_refptr<DecoderBuffer> buffer, DecoderStatus::Codes status)
    {
        EXPECT_CALL(*this, DecodeDone(HasStatusCode(status)));
        decoder_->Decode(buffer, base::BindOnce(&DecryptingVideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void DecryptAndDecodeVideo(scoped_refptr<DecoderBuffer> encrypted, Decryptor::VideoDecodeCB video_decode_cb)
    {
        ...
        if (num_decrypt_and_decode_calls_ <= kDecodingDelay || num_frames_in_decryptor_ == 0) {
            std::move(video_decode_cb).Run(Decryptor::kNeedMoreData, scoped_refptr<VideoFrame>());
            return;
        }
    ...
    void EnterPendingDecodeState()
    {
        ...
        EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(encrypted_buffer_, _)).WillOnce(MoveArg<1>(&pending_video_decode_cb_));

        decoder_->Decode(encrypted_buffer_, base::BindOnce(&DecryptingVideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    ...
    {
        EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kNoKey, null_video_frame_));
    ...
    void AbortPendingVideoDecodeCB()
    {
        if (pending_video_decode_cb_) {
            std::move(pending_video_decode_cb_).Run(Decryptor::kSuccess, scoped_refptr<VideoFrame>(nullptr));
        }
    }
    ...
    void Reset()
    {
        EXPECT_CALL(*decryptor_, ResetDecoder(Decryptor::kVideo))
            .WillRepeatedly(InvokeWithoutArgs(this, &DecryptingVideoDecoderTest::AbortPendingVideoDecodeCB));
    ...
    {
        EXPECT_CALL(*decryptor_, DeinitializeDecoder(Decryptor::kVideo))
            .WillRepeatedly(InvokeWithoutArgs(this, &DecryptingVideoDecoderTest::AbortAllPendingCBs));
...
TEST_F(DecryptingVideoDecoderTest, Initialize_Failure)
{
    ...
    EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<1>(false));
...
TEST_F(DecryptingVideoDecoderTest, ClearToEncryptedNormal)
{
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<1>(Decryptor::kSuccess, decoded_video_frame_));
...
TEST_F(DecryptingVideoDecoderTest, EncryptedBuffersNoMediaLog)
{
    ...
    EXPECT_MEDIA_LOG(HasSubstr("First switch from clear to encrypted buffers.")).Times(0);
...
TEST_F(DecryptingVideoDecoderTest, Reinitialize_Failure)
{
    ...
    EXPECT_CALL(*decryptor_, DeinitializeDecoder(Decryptor::kVideo));
    EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillOnce(RunOnceCallback<1>(false));
...
TEST_F(DecryptingVideoDecoderTest, DecryptAndDecode_SubsampleError)
{
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeVideo(_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<1>(Decryptor::kError, scoped_refptr<VideoFrame>(nullptr)));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingVideoDecoder: Subsamples for Buffer do not match"));
...
TEST_F(DecryptingVideoDecoderTest, KeyAdded_DuringWaitingForKey)
{
    ...
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingVideoDecoder: key added, resuming decode"));
...
TEST_F(DecryptingVideoDecoderTest, KeyAdded_DuringPendingDecode)
{
    ...
    EXPECT_CALL(*this, FrameReady(decoded_video_frame_));
    EXPECT_CALL(*this, DecodeDone(IsOkStatus()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingVideoDecoder: key was added, resuming decode"));
...
TEST_F(DecryptingVideoDecoderTest, Destroy_DuringPendingDecoderInit)
{
    ...
    EXPECT_CALL(*decryptor_, InitializeVideoDecoder(_, _)).WillOnce(WithArg<1>(Invoke([&](Decryptor::DecoderInitCB init_cb) {
