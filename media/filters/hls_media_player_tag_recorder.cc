...
template <typename T> void LogBitfieldHistogram(uint32_t bitfield, const char* name)
{
...
HlsMediaPlayerTagRecorder::HlsMediaPlayerTagRecorder(std::unique_ptr<HlsNetworkAccess> network_access)
...
void HlsMediaPlayerTagRecorder::OnPlaylistFetch(GURL root_playlist_uri, HlsDataSourceProvider::ReadResult result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!result.has_value()) {
        return;
    ...
    case hls::Playlist::Kind::kMultivariantPlaylist: {
        auto maybe = hls::MultivariantPlaylist::Parse(stream->AsString(), root_playlist_uri, (*m_info).version, this);
        if (!maybe.has_value()) {
    ...
    case hls::Playlist::Kind::kMediaPlaylist: {
        auto maybe = hls::MediaPlaylist::Parse(stream->AsString(), root_playlist_uri, (*m_info).version, nullptr, this);
        if (!maybe.has_value()) {
            RecordError(static_cast<uint32_t>(std::move(maybe).error().code()));
            return;
        }
...
void HlsMediaPlayerTagRecorder::SetTopLevelPlaylistType(hls::Playlist::Kind kind)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (recording_enabled_ && !root_playlist_type_.has_value()) {
        base::UmaHistogramBoolean("Media.HLS.MultivariantPlaylist", kind == hls::Playlist::Kind::kMultivariantPlaylist);
    }
    root_playlist_type_ = kind;
}

void HlsMediaPlayerTagRecorder::SetAdvancedFeaturePresent(AdvancedFeatureTagType type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    advanced_feature_bitfield_ = advanced_feature_bitfield_.value_or(0) | (1 << static_cast<uint32_t>(type));
}

void HlsMediaPlayerTagRecorder::SetSegmentTypePresent(PlaylistSegmentType type)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    playlist_segment_bitfield_ = playlist_segment_bitfield_.value_or(0) | (1 << static_cast<uint32_t>(type));
}

void HlsMediaPlayerTagRecorder::SetEncryptionModePresent(SegmentEncryptionMode mode)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (recording_enabled_ && !segment_encryption_bitfield_.has_value()) {
        base::UmaHistogramEnumeration("Media.HLS.EncryptionMode", mode);
    }
    segment_encryption_bitfield_ = segment_encryption_bitfield_.value_or(0) | (1 << static_cast<uint32_t>(mode));
}
...
void HlsMediaPlayerTagRecorder::AllowRecording()
{
    ...
    if (playlist_invalid_errorcode_.has_value()) {
        base::UmaHistogramSparse("Media.HLS.UnparsableManifest", playlist_invalid_errorcode_.value());
    }
    if (root_playlist_type_.has_value()) {
        base::UmaHistogramBoolean("Media.HLS.MultivariantPlaylist", root_playlist_type_.value() == hls::Playlist::Kind::kMultivariantPlaylist);
    }
    if (advanced_feature_bitfield_.has_value()) {
        LogBitfieldHistogram<AdvancedFeatureTagType>(advanced_feature_bitfield_.value(), "Media.HLS.AdvancedFeatureTags");
    }
    if (playlist_segment_bitfield_.has_value()) {
        LogBitfieldHistogram<PlaylistSegmentType>(playlist_segment_bitfield_.value(), "Media.HLS.PlaylistSegmentExtension");
    }
    if (segment_encryption_bitfield_.has_value()) {
        LogBitfieldHistogram<SegmentEncryptionMode>(segment_encryption_bitfield_.value(), "Media.HLS.EncryptionMode");
    }
}
...
void HlsMediaPlayerTagRecorder::Start(GURL root_playlist_uri)
{
    ...
    network_access_->ReadManifest(
        root_playlist_uri, base::BindOnce(&HlsMediaPlayerTagRecorder::OnPlaylistFetch, weak_factory_.GetWeakPtr(), root_playlist_uri));
}
