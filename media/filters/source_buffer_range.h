...
public:
    ...
    SourceBufferRange(GapPolicy gap_policy, const BufferQueue& new_buffers, base::TimeDelta range_start_pts, InterbufferDistanceCB interbuffer_distance_cb);
    ...
    void AppendRangeToEnd(const SourceBufferRange& range, bool transfer_current_position);
    ...
    // During append, |highest_frame_| is updated, if necessary.
    void AppendBuffersToEnd(const BufferQueue& buffers, base::TimeDelta new_buffers_group_start_timestamp);
    bool AllowableAppendAfterEstimatedDuration(const BufferQueue& buffers, base::TimeDelta new_buffers_group_start_pts) const;
    bool CanAppendBuffersToEnd(const BufferQueue& buffers, base::TimeDelta new_buffers_group_start_pts) const;
    ...
    // returns false.
    bool TruncateAt(base::TimeDelta timestamp, BufferQueue* deleted_buffers, bool is_exclusive);
    ...
    size_t GetRemovalGOP(base::TimeDelta start_timestamp, base::TimeDelta end_timestamp, size_t bytes_to_free, base::TimeDelta* end_removal_timestamp) const;
    ...
    base::TimeDelta FindHighestBufferedTimestampAtOrBefore(base::TimeDelta timestamp) const;
    ...
    bool GetBuffersInRange(base::TimeDelta start, base::TimeDelta end, BufferQueue* buffers) const;

    // Returns the memory usage of the buffered data in bytes.
    size_t GetMemoryUsage() const
    {
        return memory_usage_in_bytes_;
    }
...
private:
    ...
    void FreeBufferRange(const BufferQueue::const_iterator& starting_point, const BufferQueue::const_iterator& ending_point);
    ...
    // buffer's time.
    base::TimeDelta NextRangeStartTimeForAppendRangeToEnd(const SourceBufferRange& range) const;
    ...
    // |buffers_.end()|).
    size_t GetBufferIndexAt(base::TimeDelta timestamp, bool skip_given_timestamp) const;
    BufferQueue::const_iterator GetBufferItrAt(base::TimeDelta timestamp, bool skip_given_timestamp) const;
    ...
    KeyframeMap::const_iterator GetFirstKeyframeAt(base::TimeDelta timestamp, bool skip_given_timestamp) const;

    // Returns an iterator in |keyframe_map_| pointing to the first keyframe
    // before or at |timestamp|.
    KeyframeMap::const_iterator GetFirstKeyframeAtOrBefore(base::TimeDelta timestamp) const;
