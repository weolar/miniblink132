...
static inline bool IsOutOfSync(const base::TimeDelta& timestamp_1, const base::TimeDelta& timestamp_2)
{
...
DecryptingAudioDecoder::DecryptingAudioDecoder(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log)
    ...
    const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (state_ == kUninitialized) {
        ...
        event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&DecryptingAudioDecoder::OnCdmContextEvent, weak_factory_.GetWeakPtr()));
    } else {
...
void DecryptingAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOG(3) << "Decode()";
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void DecryptingAudioDecoder::InitializeDecoder()
{
    ...
    decryptor_->InitializeAudioDecoder(
        config_, base::BindPostTaskToCurrentDefault(base::BindOnce(&DecryptingAudioDecoder::FinishInitialization, weak_factory_.GetWeakPtr())));
}
...
void DecryptingAudioDecoder::FinishInitialization(bool success)
{
    ...
    // Success!
    timestamp_helper_ = std::make_unique<AudioTimestampHelper>(config_.samples_per_second());
...
void DecryptingAudioDecoder::DecodePendingBuffer()
{
    ...
    if (!DecoderBuffer::DoSubsamplesMatch(*pending_buffer_to_decode_)) {
        MEDIA_LOG(ERROR, media_log_) << "DecryptingAudioDecoder: Subsamples for Buffer do not match";
        state_ = kError;
        std::move(decode_cb_).Run(DecoderStatus::Codes::kFailed);
        return;
    }

    decryptor_->DecryptAndDecodeAudio(pending_buffer_to_decode_,
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&DecryptingAudioDecoder::DeliverFrame, weak_factory_.GetWeakPtr(), buffer_size)));
}

void DecryptingAudioDecoder::DeliverFrame(int buffer_size, Decryptor::Status status, const Decryptor::AudioFrames& frames)
{
...
void DecryptingAudioDecoder::ProcessDecodedFrames(const Decryptor::AudioFrames& frames)
{
    for (auto iter = frames.begin(); iter != frames.end(); ++iter) {
        ...
        if (IsOutOfSync(current_time, frame->timestamp())) {
            DVLOG(1) << "Timestamp returned by the decoder (" << frame->timestamp().InMilliseconds() << " ms)"
