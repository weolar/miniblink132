...
public:
    DecryptingAudioDecoderTest()
        : decoder_(std::make_unique<DecryptingAudioDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_))
    ...
    DecryptingAudioDecoderTest& operator=(const DecryptingAudioDecoderTest&) = delete;

    ~DecryptingAudioDecoderTest() override
    {
        Destroy();
    }

    void InitializeAndExpectResult(const AudioDecoderConfig& config, bool success)
    {
        ...
        if (channels < 0)
            channels = 0;
        decoded_frame_ = AudioBuffer::CreateEmptyBuffer(config.channel_layout(), channels, kSampleRate, kFakeAudioFrameSize, kNoTimestamp);
        decoded_frame_list_.push_back(decoded_frame_);

        decoder_->Initialize(config, cdm_context_.get(),
            base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&DecryptingAudioDecoderTest::FrameReady, base::Unretained(this)),
            base::BindRepeating(&DecryptingAudioDecoderTest::OnWaiting, base::Unretained(this)));
    ...
    void Initialize()
    {
        ...
        EXPECT_CALL(*decryptor_, InitializeAudioDecoder(_, _)).Times(AtMost(1)).WillOnce(RunOnceCallback<1>(true));
        ...
        config_.Initialize(
            AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, kSampleRate, EmptyExtraData(), EncryptionScheme::kCenc, base::TimeDelta(), 0);
    ...
    void ReinitializeConfigChange(const AudioDecoderConfig& new_config)
    {
        ...
        EXPECT_CALL(*decryptor_, InitializeAudioDecoder(_, _)).WillOnce(RunOnceCallback<1>(true));
        decoder_->Initialize(new_config, cdm_context_.get(), base::BindOnce([](DecoderStatus status) { EXPECT_TRUE(status.is_ok()); }),
            base::BindRepeating(&DecryptingAudioDecoderTest::FrameReady, base::Unretained(this)),
            base::BindRepeating(&DecryptingAudioDecoderTest::OnWaiting, base::Unretained(this)));
    }

    // Decode |buffer| and expect DecodeDone to get called with |status|.
    void DecodeAndExpect(scoped_refptr<DecoderBuffer> buffer, DecoderStatus status)
    {
        EXPECT_CALL(*this, DecodeDone(HasStatusCode(status)));
        decoder_->Decode(buffer, base::BindOnce(&DecryptingAudioDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void DecryptAndDecodeAudio(scoped_refptr<DecoderBuffer> encrypted, Decryptor::AudioDecodeCB audio_decode_cb)
    {
        ...
        if (num_decrypt_and_decode_calls_ <= kDecodingDelay || num_frames_in_decryptor_ == 0) {
            std::move(audio_decode_cb).Run(Decryptor::kNeedMoreData, Decryptor::AudioFrames());
            return;
        }

        num_frames_in_decryptor_--;
        std::move(audio_decode_cb).Run(Decryptor::kSuccess, Decryptor::AudioFrames(1, decoded_frame_));
    }
    ...
    void EnterEndOfStreamState()
    {
        ...
        DecodeAndExpect(DecoderBuffer::CreateEOSBuffer(), DecoderStatus::Codes::kOk);
    ...
    void EnterPendingDecodeState()
    {
        ...
        EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(encrypted_buffer_, _)).WillOnce(MoveArg<1>(&pending_audio_decode_cb_));

        decoder_->Decode(encrypted_buffer_, base::BindOnce(&DecryptingAudioDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    ...
    {
        EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(encrypted_buffer_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kNoKey, Decryptor::AudioFrames()));
    ...
    void AbortPendingAudioDecodeCB()
    {
        if (pending_audio_decode_cb_) {
            std::move(pending_audio_decode_cb_).Run(Decryptor::kSuccess, Decryptor::AudioFrames());
        }
    }
    ...
    void Reset()
    {
        EXPECT_CALL(*decryptor_, ResetDecoder(Decryptor::kAudio))
            .WillRepeatedly(InvokeWithoutArgs(this, &DecryptingAudioDecoderTest::AbortPendingAudioDecodeCB));
    ...
    {
        EXPECT_CALL(*decryptor_, DeinitializeDecoder(Decryptor::kAudio))
            .WillRepeatedly(InvokeWithoutArgs(this, &DecryptingAudioDecoderTest::AbortAllPendingCBs));
...
{
    AudioDecoderConfig config(AudioCodec::kUnknown, kUnknownSampleFormat, CHANNEL_LAYOUT_STEREO, 0, EmptyExtraData(), EncryptionScheme::kCenc);
...
TEST_F(DecryptingAudioDecoderTest, Initialize_UnsupportedAudioConfig)
{
    ...
    EXPECT_CALL(*decryptor_, InitializeAudioDecoder(_, _)).WillOnce(RunOnceCallback<1>(false));

    AudioDecoderConfig config(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, kSampleRate, EmptyExtraData(), EncryptionScheme::kCenc);
    InitializeAndExpectResult(config, false);
}
...
TEST_F(DecryptingAudioDecoderTest, DecryptAndDecode_SubsampleError)
{
    ...
    scoped_refptr<media::DecoderBuffer> mismatched_encrypted_buffer = CreateMismatchedBufferForTest();

    EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kError, Decryptor::AudioFrames()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingAudioDecoder: Subsamples for Buffer do not match"));
...
TEST_F(DecryptingAudioDecoderTest, DecryptAndDecode_MultipleFrames)
{
    ...
    scoped_refptr<AudioBuffer> frame_a = AudioBuffer::CreateEmptyBuffer(
        config_.channel_layout(), ChannelLayoutToChannelCount(config_.channel_layout()), kSampleRate, kFakeAudioFrameSize, kNoTimestamp);
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(_, _)).WillOnce(RunOnceCallback<1>(Decryptor::kSuccess, decoded_frame_list_));
...
TEST_F(DecryptingAudioDecoderTest, Reinitialize_EncryptedToEncrypted)
{
    ...
    AudioDecoderConfig new_config(AudioCodec::kVorbis, kSampleFormatPlanarS16, CHANNEL_LAYOUT_5_1, 88200, EmptyExtraData(), EncryptionScheme::kCenc);
...
TEST_F(DecryptingAudioDecoderTest, Reinitialize_EncryptedToClear)
{
    ...
    // The new config is different from the initial config in bytes-per-channel,
    // channel layout and samples_per_second.
    AudioDecoderConfig new_config(AudioCodec::kVorbis, kSampleFormatPlanarS16, CHANNEL_LAYOUT_5_1, 88200, EmptyExtraData(), EncryptionScheme::kUnencrypted);
    EXPECT_NE(new_config.bytes_per_channel(), config_.bytes_per_channel());
    EXPECT_NE(new_config.channel_layout(), config_.channel_layout());
    EXPECT_NE(new_config.samples_per_second(), config_.samples_per_second());
...
TEST_F(DecryptingAudioDecoderTest, KeyAdded_DuringWaitingForKey)
{
    ...
    EXPECT_CALL(*decryptor_, DecryptAndDecodeAudio(_, _)).WillRepeatedly(RunOnceCallback<1>(Decryptor::kSuccess, decoded_frame_list_));
    EXPECT_CALL(*this, FrameReady(decoded_frame_));
    EXPECT_CALL(*this, DecodeDone(IsOkStatus()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingAudioDecoder: key added, resuming decode"));
...
TEST_F(DecryptingAudioDecoderTest, KeyAdded_DuringPendingDecode)
{
    ...
    EXPECT_CALL(*this, FrameReady(decoded_frame_));
    EXPECT_CALL(*this, DecodeDone(IsOkStatus()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingAudioDecoder: key was added, resuming decode"));
    // The audio decode callback is returned after the correct decryption key is
    // added.
    event_cb_.Run(CdmContext::Event::kHasAdditionalUsableKey);
    std::move(pending_audio_decode_cb_).Run(Decryptor::kNoKey, Decryptor::AudioFrames());
    base::RunLoop().RunUntilIdle();
}
