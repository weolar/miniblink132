...
bool ExtractString(AVDictionaryEntry* tag, const char* expected_key, std::string* destination)
{
...
// Returns true if the |tag| matches |expected_key|.
bool ExtractInt(AVDictionaryEntry* tag, const char* expected_key, int* destination)
{
    if (!base::EqualsCaseInsensitiveASCII(std::string(tag->key), expected_key))
        return false;
...
bool AudioVideoMetadataExtractor::Extract(DataSource* source, bool extract_attached_images)
{
    DCHECK(!extracted_);

    bool read_ok = true;
    media::BlockingUrlProtocol protocol(source, base::BindRepeating(&OnError, &read_ok));
    ...
    for (unsigned int i = 0; i < format_context->nb_streams; ++i) {
        ...
        for (int j = 0; j < stream->codecpar->nb_coded_side_data; j++) {
            ...
            if (sd.type == AV_PKT_DATA_DISPLAYMATRIX) {
                ...
                rotation_ = VideoTransformation::FromFFmpegDisplayMatrix(reinterpret_cast<int32_t*>(sd.data)).rotation;
        ...
        if (stream->codecpar->width > 0 && stream->codecpar->width > width_ && stream->codecpar->height > 0 && stream->codecpar->height > height_) {
            width_ = stream->codecpar->width;
            height_ = stream->codecpar->height;
        }

        // Extract attached image if requested.
        if (extract_attached_images && stream->disposition == AV_DISPOSITION_ATTACHED_PIC && stream->attached_pic.size > 0
            && stream->attached_pic.size <= kAttachedImageSizeLimit && stream->attached_pic.data != NULL) {
            attached_images_bytes_.push_back(std::string());
            attached_images_bytes_.back().assign(reinterpret_cast<const char*>(stream->attached_pic.data), stream->attached_pic.size);
        }
    }
...
const std::vector<AudioVideoMetadataExtractor::StreamInfo>& AudioVideoMetadataExtractor::stream_infos() const
{
    DCHECK(extracted_);
    return stream_infos_;
}

const std::vector<std::string>& AudioVideoMetadataExtractor::attached_images_bytes() const
{
    DCHECK(extracted_);
    return attached_images_bytes_;
}

void AudioVideoMetadataExtractor::ExtractDictionary(AVDictionary* metadata, TagDictionary* raw_tags)
{
    if (!metadata)
        return;

    for (AVDictionaryEntry* tag = av_dict_get(metadata, "", NULL, AV_DICT_IGNORE_SUFFIX); tag; tag = av_dict_get(metadata, "", tag, AV_DICT_IGNORE_SUFFIX)) {
