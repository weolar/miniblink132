...
const int16_t kDcQLookup[][kQIndexRange] = { {
    ...
    { 4, 9, 10, 13, 15, 17, 20, 22, 25, 28, 31, 34, 37, 40, 43, 47, 50, 53, 57, 60, 64, 68, 71, 75, 78, 82, 86, 90, 93, 97, 101, 105, 109, 113, 116, 120, 124,
        128, 132, 136, 140, 143, 147, 151, 155, 159, 163, 166, 170, 174, 178, 182, 185, 189, 193, 197, 200, 204, 208, 212, 215, 219, 223, 226, 230, 233, 237,
        241, 244, 248, 251, 255, 259, 262, 266, 269, 273, 276, 280, 283, 287, 290, 293, 297, 300, 304, 307, 310, 314, 317, 321, 324, 327, 331, 334, 337, 343,
        350, 356, 362, 369, 375, 381, 387, 394, 400, 406, 412, 418, 424, 430, 436, 442, 448, 454, 460, 466, 472, 478, 484, 490, 499, 507, 516, 525, 533, 542,
        550, 559, 567, 576, 584, 592, 601, 609, 617, 625, 634, 644, 655, 666, 676, 687, 698, 708, 718, 729, 739, 749, 759, 770, 782, 795, 807, 819, 831, 844,
        856, 868, 880, 891, 906, 920, 933, 947, 961, 975, 988, 1001, 1015, 1030, 1045, 1061, 1076, 1090, 1105, 1120, 1137, 1153, 1170, 1186, 1202, 1218, 1236,
        1253, 1271, 1288, 1306, 1323, 1342, 1361, 1379, 1398, 1416, 1436, 1456, 1476, 1496, 1516, 1537, 1559, 1580, 1601, 1624, 1647, 1670, 1692, 1717, 1741,
        1766, 1791, 1817, 1844, 1871, 1900, 1929, 1958, 1990, 2021, 2054, 2088, 2123, 2159, 2197, 2236, 2276, 2319, 2363, 2410, 2458, 2508, 2561, 2616, 2675,
        2737, 2802, 2871, 2944, 3020, 3102, 3188, 3280, 3375, 3478, 3586, 3702, 3823, 3953, 4089, 4236, 4394, 4559, 4737, 4929, 5130, 5347 } };
...
const int16_t kAcQLookup[][kQIndexRange] = { {
    ...
    { 4, 9, 11, 13, 16, 18, 21, 24, 27, 30, 33, 37, 40, 44, 48, 51, 55, 59, 63, 67, 71, 75, 79, 83, 88, 92, 96, 100, 105, 109, 114, 118, 122, 127, 131, 136,
        140, 145, 149, 154, 158, 163, 168, 172, 177, 181, 186, 190, 195, 199, 204, 208, 213, 217, 222, 226, 231, 235, 240, 244, 249, 253, 258, 262, 267, 271,
        275, 280, 284, 289, 293, 297, 302, 306, 311, 315, 319, 324, 328, 332, 337, 341, 345, 349, 354, 358, 362, 367, 371, 375, 379, 384, 388, 392, 396, 401,
        409, 417, 425, 433, 441, 449, 458, 466, 474, 482, 490, 498, 506, 514, 523, 531, 539, 547, 555, 563, 571, 579, 588, 596, 604, 616, 628, 640, 652, 664,
        676, 688, 700, 713, 725, 737, 749, 761, 773, 785, 797, 809, 825, 841, 857, 873, 889, 905, 922, 938, 954, 970, 986, 1002, 1018, 1038, 1058, 1078, 1098,
        1118, 1138, 1158, 1178, 1198, 1218, 1242, 1266, 1290, 1314, 1338, 1362, 1386, 1411, 1435, 1463, 1491, 1519, 1547, 1575, 1603, 1631, 1663, 1695, 1727,
        1759, 1791, 1823, 1859, 1895, 1931, 1967, 2003, 2039, 2079, 2119, 2159, 2199, 2239, 2283, 2327, 2371, 2415, 2459, 2507, 2555, 2603, 2651, 2703, 2755,
        2807, 2859, 2915, 2971, 3027, 3083, 3143, 3203, 3263, 3327, 3391, 3455, 3523, 3591, 3659, 3731, 3803, 3876, 3952, 4028, 4104, 4184, 4264, 4348, 4432,
        4516, 4604, 4692, 4784, 4876, 4972, 5068, 5168, 5268, 5372, 5476, 5584, 5692, 5804, 5916, 6032, 6148, 6268, 6388, 6512, 6640, 6768, 6900, 7036, 7172,
        7312 } };
// clang-format on

static_assert(std::size(kDcQLookup[0]) == std::size(kAcQLookup[0]), "quantizer lookup arrays of incorrect size");

size_t ClampQ(int64_t q)
{
    return q < 0 ? 0 : base::checked_cast<size_t>(std::min(q, static_cast<int64_t>(kQIndexRange - 1)));
}
...
std::unique_ptr<DecryptConfig> SplitSubsamples(uint32_t frame_size, size_t* current_subsample_index, size_t* extra_clear_subsample_bytes,
    DecryptConfig* base_decrypt_config, const std::vector<SubsampleEntry>& subsamples, std::string* iv)
{
    ...
    do {
        ...
        uint32_t subsample_cipher = subsamples[*current_subsample_index].cypher_bytes;
        ...
        if (subsample_clear > frame_size) {
            ...
            *extra_clear_subsample_bytes = subsample_clear - frame_size;
            ...
            frame_dc_subsamples.push_back(SubsampleEntry(subsample_clear, subsample_cipher));
    ...
    return base_decrypt_config->CopyNewSubsamplesIV(frame_dc_subsamples, frame_dc_iv);
}

bool IsByteNEncrypted(off_t byte, const std::vector<SubsampleEntry>& subsamples)
{
...
{
    VideoColorSpace ret;
    ret.range = color_range ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;
    switch (color_space) {
...
Vp9Parser::FrameInfo::FrameInfo(const FrameInfo& copy_from)
    ...
    , decrypt_config(copy_from.decrypt_config ? copy_from.decrypt_config->Clone() : nullptr)
{
}

Vp9Parser::FrameInfo& Vp9Parser::FrameInfo::operator=(const FrameInfo& copy_from)
{
    this->ptr = copy_from.ptr;
    this->size = copy_from.size;
    this->allocate_size = copy_from.allocate_size;
    this->decrypt_config = copy_from.decrypt_config ? copy_from.decrypt_config->Clone() : nullptr;
    return *this;
}
...
bool Vp9FrameContext::IsValid() const
{
    ...
    static_assert(sizeof(Vp9Prob) == 1, "following checks assuming Vp9Prob is single byte");
...
const Vp9FrameContext& Vp9Parser::Context::Vp9FrameContextManager::frame_context() const
{
    DCHECK(initialized_);
...
Vp9Parser::ContextRefreshCallback Vp9Parser::Context::Vp9FrameContextManager::GetUpdateCb()
{
    if (needs_client_update_) {
        return base::BindOnce(&Vp9FrameContextManager::UpdateFromClient, weak_ptr_factory_.GetWeakPtr());
    }

    return {};
}

void Vp9Parser::Context::Vp9FrameContextManager::Update(const Vp9FrameContext& frame_context)
{
...
void Vp9Parser::Context::Vp9FrameContextManager::UpdateFromClient(const Vp9FrameContext& frame_context)
{
...
void Vp9Parser::Context::UpdateFrameContext(size_t frame_context_idx, const Vp9FrameContext& frame_context)
{
    DCHECK_LT(frame_context_idx, std::size(frame_context_managers_));
    frame_context_managers_[frame_context_idx].Update(frame_context);
}

const Vp9Parser::ReferenceSlot& Vp9Parser::Context::GetRefSlot(size_t ref_type) const
{
    DCHECK_LT(ref_type, std::size(ref_slots_));
    return ref_slots_[ref_type];
}

void Vp9Parser::Context::UpdateRefSlot(size_t ref_type, const Vp9Parser::ReferenceSlot& ref_slot)
{
    DCHECK_LT(ref_type, std::size(ref_slots_));
...
Vp9Parser::Vp9Parser(bool parsing_compressed_header, bool needs_external_context_update)
...
void Vp9Parser::SetStream(
    const uint8_t* stream, off_t stream_size, const std::vector<uint32_t>& spatial_layer_frame_size, std::unique_ptr<DecryptConfig> stream_config)
{
    DCHECK(stream);
    stream_ = stream;
...
void Vp9Parser::SetStream(const uint8_t* stream, off_t stream_size, std::unique_ptr<DecryptConfig> stream_config)
{
...
bool Vp9Parser::ParseUncompressedHeader(const FrameInfo& frame_info, Vp9FrameHeader* fhdr, Result* result, Vp9Parser::Context* context)
{
    ...
    *result = kInvalidStream;
    ...
    if (!uncompressed_parser.Parse(frame_info.ptr, frame_info.size, &curr_frame_header_)) {
        *result = kInvalidStream;
        return true;
    }
    ...
    if (curr_frame_header_.header_size_in_bytes == 0) {
        ...
        for (off_t i = curr_frame_header_.uncompressed_header_size; i < frame_info.size; i++) {
    ...
    if (curr_frame_header_.uncompressed_header_size + curr_frame_header_.header_size_in_bytes > base::checked_cast<size_t>(frame_info.size)) {
        DVLOG(1) << "header_size_in_bytes=" << curr_frame_header_.header_size_in_bytes
                 << " is larger than bytes left in buffer: " << frame_info.size - curr_frame_header_.uncompressed_header_size;
        *result = kInvalidStream;
        return true;
    }
...
bool Vp9Parser::ParseCompressedHeader(const FrameInfo& frame_info, Result* result)
{
    *result = kInvalidStream;
    ...
    if (!context_to_load.initialized()) {
        ...
        DVLOG(1) << "loading uninitialized frame context, index=" << frame_context_idx;
        *result = kInvalidStream;
        return true;
    }
    if (context_to_load.needs_client_update()) {
        DVLOG(3) << "waiting frame_context_idx=" << frame_context_idx << " to update";
    ...
    if (!needs_external_context_update_) {
        parse_success = compressed_parser.ParseNoContext(
            frame_info.ptr + curr_frame_header_.uncompressed_header_size, curr_frame_header_.header_size_in_bytes, &curr_frame_header_);
    } else {
    ...
    if (curr_frame_header_.refresh_frame_context) {
        ...
        if (curr_frame_header_.frame_parallel_decoding_mode) {
            context_.UpdateFrameContext(frame_context_idx, curr_frame_header_.frame_context);
        } else {
            if (needs_external_context_update_)
...
Vp9Parser::Result Vp9Parser::ParseNextFrame(Vp9FrameHeader* fhdr, gfx::Size* allocate_size, std::unique_ptr<DecryptConfig>* frame_decrypt_config)
{
    DCHECK(fhdr);
...
Vp9Parser::ContextRefreshCallback Vp9Parser::GetContextRefreshCb(size_t frame_context_idx)
{
...
std::string Vp9Parser::IncrementIVForTesting(const std::string& iv, uint32_t by)
{
...
base::circular_deque<Vp9Parser::FrameInfo> Vp9Parser::ParseSuperframe()
{
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        if (!base::IsValueInRangeForNumericType<off_t>(size) || static_cast<off_t>(size) > bytes_left) {
        ...
        if (subsamples.size()) {
            std::unique_ptr<DecryptConfig> frame_dc
                = SplitSubsamples(size, &current_subsample, &extra_clear_subsample_bytes, stream_decrypt_config_.get(), subsamples, &iv);
            if (!frame_dc) {
...
base::circular_deque<Vp9Parser::FrameInfo> Vp9Parser::ParseSVCFrame()
{
    ...
    memcpy(tmp_context.ref_slots_, context_.ref_slots_, sizeof(context_.ref_slots_));
    for (const auto& frame_info : frames) {
        ...
        Result result;
        if (ParseUncompressedHeader(frame_info, &dummy_fhdr, &result, &tmp_context) && result != kOk) {
            return {};
        }
        UpdateSlots(&tmp_context);
        max_frame_size.SetToMax(gfx::Size(curr_frame_header_.frame_width, curr_frame_header_.frame_height));
    }
...
int64_t Vp9Parser::GetQIndex(const Vp9QuantizationParams& quant, size_t segid) const
{
    const Vp9SegmentationParams& segmentation = context_.segmentation();

    if (segmentation.FeatureEnabled(segid, Vp9SegmentationParams::SEG_LVL_ALT_Q)) {
        int16_t feature_data = segmentation.FeatureData(segid, Vp9SegmentationParams::SEG_LVL_ALT_Q);
        int64_t q_index = segmentation.abs_or_delta_update ? feature_data : quant.base_q_idx + feature_data;
...
bool Vp9Parser::SetupSegmentationDequant()
{
    ...
    if (segmentation.enabled) {
        for (size_t i = 0; i < Vp9SegmentationParams::kNumSegments; ++i) {
            ...
            segmentation.y_dequant[i][0] = kDcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_y_dc)];
            segmentation.y_dequant[i][1] = kAcQLookup[bit_depth_index][ClampQ(q_index)];
            segmentation.uv_dequant[i][0] = kDcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_uv_dc)];
            segmentation.uv_dequant[i][1] = kAcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_uv_ac)];
        }
    } else {
        const int64_t q_index = quant.base_q_idx;
        segmentation.y_dequant[0][0] = kDcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_y_dc)];
        segmentation.y_dequant[0][1] = kAcQLookup[bit_depth_index][ClampQ(q_index)];
        segmentation.uv_dequant[0][0] = kDcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_uv_dc)];
        segmentation.uv_dequant[0][1] = kAcQLookup[bit_depth_index][ClampQ(q_index + quant.delta_q_uv_ac)];
    }
    return true;
}
...
void Vp9Parser::SetupLoopFilter()
{
    ...
    for (size_t i = 0; i < Vp9SegmentationParams::kNumSegments; ++i) {
        ...
        if (segmentation.FeatureEnabled(i, Vp9SegmentationParams::SEG_LVL_ALT_LF)) {
            ...
            level = ClampLf(segmentation.abs_or_delta_update ? feature_data : level + feature_data);
        }

        if (!loop_filter.delta_enabled) {
            memset(loop_filter.lvl[i], level, sizeof(loop_filter.lvl[i]));
        } else {
            loop_filter.lvl[i][Vp9RefType::VP9_FRAME_INTRA][0] = ClampLf(level + loop_filter.ref_deltas[Vp9RefType::VP9_FRAME_INTRA] * scale);
            loop_filter.lvl[i][Vp9RefType::VP9_FRAME_INTRA][1] = 0;

            for (size_t type = Vp9RefType::VP9_FRAME_LAST; type < Vp9RefType::VP9_FRAME_MAX; ++type) {
                for (size_t mode = 0; mode < Vp9LoopFilterParams::kNumModeDeltas; ++mode) {
