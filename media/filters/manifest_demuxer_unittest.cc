...
public:
    MOCK_METHOD(void, Initialize, (ManifestDemuxerEngineHost * demuxer, PipelineStatusCallback status_cb), (override));
    MOCK_METHOD(std::string, GetName, (), (const, override));
    MOCK_METHOD(void, OnTimeUpdate, (base::TimeDelta time, double playback_rate, ManifestDemuxer::DelayCallback cb), (override));
    MOCK_METHOD(void, Seek, (base::TimeDelta time, ManifestDemuxer::SeekCallback cb), (override));
...
public:
        ...
        , mock_host_(std::make_unique<NiceMock<MockDemuxerHost>>())
    {
        ...
        manifest_demuxer_ = std::make_unique<ManifestDemuxer>(task_environment_.GetMainThreadTaskRunner(),
            base::BindRepeating(&ManifestDemuxerTest::DemuxerRequestsSeek, base::Unretained(this)), std::move(mock_engine), media_log_.get());
    }

    ~ManifestDemuxerTest() override
    {
        manifest_demuxer_->GetChunkDemuxerForTesting()->MarkEndOfStream(PIPELINE_OK);
    ...
    void CreateIdAndAppendInitSegment(const std::string& id)
    {
        ...
        ASSERT_EQ(demuxer->AddId(id, "video/webm", "vorbis,vp8"), ChunkDemuxer::Status::kOk);

        demuxer->SetTracksWatcher(id, base::BindRepeating([](std::unique_ptr<MediaTracks>) {}));
        demuxer->SetParseWarningCallback(id, base::BindRepeating([](SourceBufferParseWarning) {}));
    ...
    void InitializeDemuxer()
    {
        ...
        EXPECT_CALL(*mock_engine_, Initialize(_, _)).WillOnce(RunOnceCallback<1>(media::PIPELINE_OK));

        manifest_demuxer_->Initialize(mock_host_.get(), base::BindOnce(&ManifestDemuxerTest::MockInitComplete, base::Unretained(this)));
...
TEST_F(ManifestDemuxerTest, InitializeStartsTimeUpdate)
{
    ...
    EXPECT_CALL(*mock_engine_, OnTimeUpdate(_, _, _)).WillOnce(RunOnceCallback<2>(kNoTimestamp));
    InitializeDemuxer();
...
TEST_F(ManifestDemuxerTest, PlaybackRateChangeUpTriggersTimeUpdate)
{
    ...
    EXPECT_CALL(*mock_engine_, OnTimeUpdate(_, _, _)).WillRepeatedly([&delay_cb](base::TimeDelta, double, ManifestDemuxer::DelayCallback cb) {
...
TEST_F(ManifestDemuxerTest, OnTimeUpdateUninterruptedBySeek)
{
    ...
    manifest_demuxer_->Seek(base::Seconds(1), base::BindOnce(&ManifestDemuxerTest::MockSeekComplete, base::Unretained(this)));
    ...
    // set again.
    EXPECT_CALL(*mock_engine_, Seek(_, _)).WillOnce(RunOnceCallback<1>(ManifestDemuxer::SeekState::kNeedsData));
...
TEST_F(ManifestDemuxerTest, SeekInterruptedByError)
{
    ...
    manifest_demuxer_->Seek(base::Seconds(100), base::BindOnce(&ManifestDemuxerTest::MockSeekComplete, base::Unretained(this)));
    task_environment_.RunUntilIdle();
...
TEST_F(ManifestDemuxerTest, CancelSeekAfterDemuxerBeforeEngine)
{
    ...
    task_environment_.RunUntilIdle();
    ASSERT_EQ(manifest_demuxer_->get_media_time_for_testing(), base::Seconds(100));
...
TEST_F(ManifestDemuxerTest, TrackChanges)
{
    ...
    manifest_demuxer_->AppendAndParseData("test", base::Seconds(10), &offset, bear->AsSpan());
    ...
    manifest_demuxer_->OnSelectedVideoTrackChanged({}, base::Seconds(0),
        base::BindOnce(
    ...
    // Enable video track:
    was_called = false;
    manifest_demuxer_->OnSelectedVideoTrackChanged({ MediaTrack::Id("video") }, base::Seconds(0),
        base::BindOnce(
            [](bool* was_called, const std::vector<DemuxerStream*>& streams) {
    ...
    // Disable audio track:
    was_called = false;
    manifest_demuxer_->OnEnabledAudioTracksChanged({}, base::Seconds(0),
        base::BindOnce(
            [](bool* was_called, const std::vector<DemuxerStream*>& streams) {
                ASSERT_TRUE(streams.empty());
                *was_called = true;
    ...
    // Enable audio track:
    was_called = false;
    manifest_demuxer_->OnEnabledAudioTracksChanged({ MediaTrack::Id("audio") }, base::Seconds(0),
        base::BindOnce(
            [](bool* was_called, const std::vector<DemuxerStream*>& streams) {
                ASSERT_EQ(streams.size(), 1u);
                *was_called = true;
