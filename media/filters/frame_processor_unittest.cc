...
public:
    ...
    FrameProcessorTestCallbackHelper(const FrameProcessorTestCallbackHelper&) = delete;
    FrameProcessorTestCallbackHelper& operator=(const FrameProcessorTestCallbackHelper&) = delete;
    ...
    MOCK_METHOD2(OnAppend, void(const DemuxerStream::Type type, const BufferQueue* buffers));
    MOCK_METHOD3(OnGroupStart, void(const DemuxerStream::Type type, DecodeTimestamp start_dts, base::TimeDelta start_pts));
};
...
protected:
        ...
        , video_id_(2)
    {
        ...
        frame_processor_ = std::make_unique<FrameProcessor>(
            base::BindRepeating(&FrameProcessorTestCallbackHelper::OnPossibleDurationIncrease, base::Unretained(&callbacks_)), &media_log_);
        frame_processor_->SetParseWarningCallback(base::BindRepeating(&FrameProcessorTestCallbackHelper::OnParseWarning, base::Unretained(&callbacks_)));
    }

    enum StreamFlags { HAS_AUDIO = 1 << 0, HAS_VIDEO = 1 << 1, OBSERVE_APPENDS_AND_GROUP_STARTS = 1 << 2, USE_AUDIO_CODEC_SUPPORTING_NONKEYFRAMES = 1 << 3 };
    ...
    void AddTestTracks(int stream_flags)
    {
        ...
        const bool setup_observers = (stream_flags & OBSERVE_APPENDS_AND_GROUP_STARTS) != 0;

        const bool support_audio_nonkeyframes = (stream_flags & USE_AUDIO_CODEC_SUPPORTING_NONKEYFRAMES) != 0;
        ASSERT_TRUE(has_audio || !support_audio_nonkeyframes);

        if (has_audio) {
            CreateAndConfigureStream(DemuxerStream::AUDIO, setup_observers, support_audio_nonkeyframes);
            ASSERT_TRUE(audio_);
    ...
    BufferQueue StringToBufferQueue(const std::string& buffers_to_append, const TrackId track_id, const DemuxerStream::Type type)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); i++) {
            ...
            std::vector<std::string> buffer_timestamps = base::SplitString(timestamps[i], "|", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
            if (buffer_timestamps.size() == 1)
                buffer_timestamps.push_back(buffer_timestamps[0]);
            CHECK_EQ(2u, buffer_timestamps.size());

            const base::TimeDelta pts = MillisecondStringToTimestamp(buffer_timestamps[0]);
            const DecodeTimestamp dts = DecodeTimestamp::FromPresentationTime(MillisecondStringToTimestamp(buffer_timestamps[1]));
            ...
            scoped_refptr<StreamParserBuffer> buffer
                = StreamParserBuffer::CopyFrom(reinterpret_cast<const uint8_t*>(pts_as_cstr), strlen(pts_as_cstr), is_keyframe, type, track_id);
    ...
    bool ProcessFrames(const std::string& audio_timestamps, const std::string& video_timestamps)
    {
        ...
        return frame_processor_->ProcessFrames(buffer_queue_map, append_window_start_, append_window_end_, &timestamp_offset_);
    }
    ...
    void CheckExpectedRangesByTimestamp(ChunkDemuxerStream* stream, const std::string& expected)
    {
    ...
    void CheckReadStalls(ChunkDemuxerStream* stream)
    {
        ...
        do {
            ...
            stream->Read(1, base::BindOnce(&FrameProcessorTest::StoreStatusAndBuffer, base::Unretained(this)));
            base::RunLoop().RunUntilIdle();
        } while (++loop_count < 2 && read_callback_called_ && last_read_status_ == DemuxerStream::kAborted);

        ASSERT_FALSE(read_callback_called_ && last_read_status_ == DemuxerStream::kAborted) << "2 kAborted reads in a row. Giving up.";
    ...
    void CheckReadsThenReadStalls(ChunkDemuxerStream* stream, const std::string& expected)
    {
    ...
    // CheckReadsAndOptionallyKeyframenessThenReadStalls().
    void CheckReadsAndKeyframenessThenReadStalls(ChunkDemuxerStream* stream, const std::string& expected)
    {
    ...
    // keyframeness.
    void CheckReadsAndOptionallyKeyframenessThenReadStalls(ChunkDemuxerStream* stream, const std::string& expected, bool check_keyframeness)
    {
        ...
        for (size_t i = 0; i < timestamps.size(); ++i) {
            ...
            ASSERT_FALSE(last_read_status_ == DemuxerStream::kAborted) << "2 kAborted reads in a row. Giving up.";
            ...
            double original_time_in_ms = DecodeTestPayload(base::as_string_view(*last_read_buffer_)).InMillisecondsF();
...
private:
    void StoreStatusAndBuffer(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
    {
        DCHECK_LE(buffers.size(), 1u) << "FrameProcessorTest only reads a single-buffer.";
        scoped_refptr<DecoderBuffer> buffer = (buffers.empty() ? nullptr : std::move(buffers[0]));

        if (status == DemuxerStream::kOk && buffer.get()) {
            DVLOG(3) << __func__ << "status: " << status << " ts: " << buffer->timestamp().InSecondsF();
        } else {
    ...
    void CreateAndConfigureStream(DemuxerStream::Type type, bool setup_observers, bool support_audio_nonkeyframes)
    {
        ...
        case DemuxerStream::AUDIO: {
            ASSERT_FALSE(audio_);
            audio_ = std::make_unique<ChunkDemuxerStream>(DemuxerStream::AUDIO, MediaTrack::Id("1"));
            AudioDecoderConfig decoder_config;
            if (support_audio_nonkeyframes) {
                decoder_config = AudioDecoderConfig(
                    AudioCodec::kAAC, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
                decoder_config.set_profile(AudioCodecProfile::kXHE_AAC);
            } else {
                decoder_config = AudioDecoderConfig(
                    AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
            }
            frame_processor_->OnPossibleAudioConfigUpdate(decoder_config);
            ASSERT_TRUE(audio_->UpdateAudioConfig(decoder_config, false, &media_log_));
        ...
        case DemuxerStream::VIDEO: {
            ...
            video_ = std::make_unique<ChunkDemuxerStream>(DemuxerStream::VIDEO, MediaTrack::Id("2"));
            ASSERT_TRUE(video_->UpdateVideoConfig(TestVideoConfig::Normal(), false, &media_log_));
            stream = video_.get();
            break;
        }
        ...
        if (setup_observers) {
            stream->set_append_observer_for_testing(base::BindRepeating(&FrameProcessorTestCallbackHelper::OnAppend, base::Unretained(&callbacks_), type));
            stream->set_group_start_observer_for_testing(
                base::BindRepeating(&FrameProcessorTestCallbackHelper::OnGroupStart, base::Unretained(&callbacks_), type));
        }
    }
};
...
TEST_P(FrameProcessorTest, WrongTypeInAppendedBuffer)
{
    ...
    ASSERT_FALSE(frame_processor_->ProcessFrames(buffer_queue_map, append_window_start_, append_window_end_, &timestamp_offset_));
    EXPECT_FALSE(in_coded_frame_group());
...
TEST_P(FrameProcessorTest, AudioVideo_SequentialProcessFrames)
{
    ...
    if (use_sequence_mode_) {
        frame_processor_->SetSequenceMode(true);
        EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kMuxedSequenceMode));
...
TEST_P(FrameProcessorTest, AudioVideo_Discontinuity_TimestampOffset)
{
    ...
    // threshold.
    CheckExpectedRangesByTimestamp(audio_.get(), "{ [55,85) [100,130) [200,230) }");
    ...
    CheckReadsThenReadStalls(video_.get(), "65:10 75:20 85:30 110:10 120:20 130:30");
...
TEST_P(FrameProcessorTest, AppendWindowFilterOfNegativeBufferTimestampsWithPrerollDiscard)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO);
    if (use_sequence_mode_)
...
TEST_P(FrameProcessorTest, PartialAppendWindowFilterNoDiscontinuity_DtsAfterPts)
{
...
TEST_P(FrameProcessorTest, PartialAppendWindowZeroDurationPreroll)
{
    ...
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(use_sequence_mode_ ? 10 : 14)));
    frame_duration_ = Milliseconds(10);
    EXPECT_TRUE(ProcessFrames("4K", ""));
    EXPECT_TRUE(in_coded_frame_group());
    ...
    ASSERT_EQ(Milliseconds(0), last_read_parser_buffer->preroll_buffer()->duration());
}

TEST_P(FrameProcessorTest, OOOKeyframePrecededByDependantNonKeyframeShouldWarn)
{
    InSequence s;
    AddTestTracks(HAS_VIDEO);
    frame_processor_->SetSequenceMode(use_sequence_mode_);
    ...
    EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kKeyframeTimeGreaterThanDependant));
...
TEST_P(FrameProcessorTest, ContinuousDts_SapType2_and_PtsJumpForward)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp(), Milliseconds(1060)));
    EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kKeyframeTimeGreaterThanDependant));
    ...
    EXPECT_TRUE(ProcessFrames("", "1060|0K 1000|10 1050|20 1010|30 1040|40 1020|50 1030|60"));
    EXPECT_EQ(Milliseconds(0), timestamp_offset_);
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(60)), Milliseconds(1070)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    ...
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(1140)));
    EXPECT_TRUE(ProcessFrames("", "1070|80 1120|90 1080|100 1110|110 1090|120 1100|130"));
    EXPECT_EQ(Milliseconds(0), timestamp_offset_);
    CheckExpectedRangesByTimestamp(video_.get(), "{ [1060,1140) }");

    // [1060,1140) should demux continuously without read stall in the middle.
    SeekStream(video_.get(), Milliseconds(1060));
    CheckReadsThenReadStalls(video_.get(), "1060 1000 1050 1010 1040 1020 1030 1130 1070 1120 1080 1110 1090 1100");
...
TEST_P(FrameProcessorTest, ContinuousDts_NewGopEndOverlapsLastGop_1)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp(), Milliseconds(100)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(140)));
    EXPECT_TRUE(ProcessFrames("", "100|0K 110|10 120|20 130|30"));
    EXPECT_EQ(Milliseconds(0), timestamp_offset_);

    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(30)), Milliseconds(125)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewGopEndOverlapsLastGop_2)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(30)), Milliseconds(115)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_1)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp(), Milliseconds(120)));
    EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kKeyframeTimeGreaterThanDependant));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_2)
{
    ...
    // flush.
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(30)), Milliseconds(140)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(180)));
    EXPECT_TRUE(ProcessFrames("", "120|0K 100|10 130|20 110|30 160|40K 140|50 170|60 150|70"));
    EXPECT_EQ(Milliseconds(0), timestamp_offset_);

    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(70)), Milliseconds(155)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_3_GopByGop)
{
    // API user might craft a continuous-in-DTS-with-previous-append GOP that has
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp(), Milliseconds(500)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_3_FrameByFrame)
{
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_4_GopByGop)
{
    // API user might craft a continuous-in-DTS-with-previous-append GOP that has
    // PTS interval overlapping the previous append, using SAP Type 2 GOPs.  Tests
    // SAP-Type 2 GOPs, where newly appended GOP overlaps enough nonkeyframes of
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(20)), Milliseconds(530)));
    EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kKeyframeTimeGreaterThanDependant));
...
TEST_P(FrameProcessorTest, ContinuousDts_NewSap2GopEndOverlapsLastGop_4_FrameByFrame)
{
    // Tests that the buffered range results match the previous GopByGop test if
    // each frame of the second GOP is explicitly appended by the app
    // one-at-a-time.
...
TEST_P(FrameProcessorTest, ContinuousDts_GopKeyframePtsOrder_2_1_3)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp(), Milliseconds(200)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(240)));
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(30)), Milliseconds(100)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(70)), Milliseconds(140)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, ContinuousPts_DiscontinuousDts_AcrossGops)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(200)), Milliseconds(200)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(240)));
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::VIDEO, DecodeTimestamp::FromPresentationTime(Milliseconds(225)), Milliseconds(240)));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::VIDEO, _));
...
TEST_P(FrameProcessorTest, OnlyKeyframes_ContinuousDts_ContinousPts_1)
{
    ...
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::AUDIO, DecodeTimestamp(), base::TimeDelta()));
...
TEST_P(FrameProcessorTest, OnlyKeyframes_ContinuousDts_DiscontinuousPtsJustBeyondFudgeRoom)
{
    ...
    // append.
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::AUDIO, DecodeTimestamp(), frame_duration_));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::AUDIO, _));

    // Frame "20|10K" following "10|5K" triggers start of new group and eventual
    // append.
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::AUDIO, DecodeTimestamp::FromPresentationTime(Milliseconds(5)), Milliseconds(10) + frame_duration_));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::AUDIO, _));

    // Frame "30|15K" following "20|10K" triggers start of new group and
    // eventual append.
    EXPECT_CALL(callbacks_, OnGroupStart(DemuxerStream::AUDIO, DecodeTimestamp::FromPresentationTime(Milliseconds(10)), Milliseconds(20) + frame_duration_));
    EXPECT_CALL(callbacks_, OnAppend(DemuxerStream::AUDIO, _));
...
TEST_P(FrameProcessorTest, GroupEndTimestampDecreaseWithinMediaSegmentShouldWarn)
{
    ...
    EXPECT_CALL(callbacks_, OnParseWarning(SourceBufferParseWarning::kGroupEndTimestampDecreaseWithinMediaSegment));
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_BasicOperation)
{
    ...
    CheckReadsAndKeyframenessThenReadStalls(audio_.get(), "0K 10N 20N 30N 40K 50N 60N 70N");
}
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_InitialNonkeyframesNotBuffered)
{
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_InvalidDecreasingNonkeyframePts)
{
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_ValidDecreasingKeyframePts)
{
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_ValidSameNonKeyframePts_1)
{
    // With the support for audio nonkeyframe buffering enabled, try to buffer a
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_ValidSameNonKeyframePts_2)
{
    // With the support for audio nonkeyframe buffering enabled, try to buffer a
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_AppendWindowFilterDroppedPrerollKeyframe)
{
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_AppendWindowFilter_TrimFront)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO | USE_AUDIO_CODEC_SUPPORTING_NONKEYFRAMES);
    if (use_sequence_mode_)
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_AppendWindowFilter_TrimEnd)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO | USE_AUDIO_CODEC_SUPPORTING_NONKEYFRAMES);
    if (use_sequence_mode_)
...
TEST_P(FrameProcessorTest, NonkeyframeAudioBuffering_TrimSpliceOverlap)
{
    ...
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(10) + frame_duration_));
    EXPECT_TRUE(ProcessFrames("10", ""));
    ...
    // muxed to avoid this.
    frame_duration_ = base::Microseconds(10250);
    EXPECT_CALL(callbacks_, PossibleDurationIncrease(Milliseconds(22) + frame_duration_));
    ...
    CheckReadsAndKeyframenessThenReadStalls(audio_.get(), "0K 10N 20N 22N 32K 42N 45K");
}
...
TEST_P(FrameProcessorTest, Pts_BeforeTimestampOffsetApplied_kNoTimestamp_Fails)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO);
    frame_processor_->SetSequenceMode(use_sequence_mode_);
...
TEST_P(FrameProcessorTest, Pts_BeforeTimestampOffsetApplied_kInfiniteDuration_Fails)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO);
    frame_processor_->SetSequenceMode(use_sequence_mode_);

    EXPECT_MEDIA_LOG(FrameTimeOutOfRange("Before adjusting by timestampOffset", "PTS", "audio"));
    EXPECT_FALSE(ProcessFrames("MaxK", ""));
}

TEST_P(FrameProcessorTest, Dts_BeforeTimestampOffsetApplied_kNoDecodeTimestamp_UsesPtsIfValid)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO);
    frame_processor_->SetSequenceMode(use_sequence_mode_);
...
TEST_P(FrameProcessorTest, Dts_BeforeTimestampOffsetApplied_kMaxDecodeTimestamp_Fails)
{
    InSequence s;
    AddTestTracks(HAS_AUDIO);
    frame_processor_->SetSequenceMode(use_sequence_mode_);

    EXPECT_MEDIA_LOG(FrameTimeOutOfRange("Before adjusting by timestampOffset", "DTS", "audio"));
...
TEST_P(FrameProcessorTest, After_Sequence_OffsetUpdate_kInfiniteDuration_Fails)
{
    if (!use_sequence_mode_) {
        DVLOG(1) << "Skipping segments mode variant; inapplicable to this case.";
        return;
...
TEST_P(FrameProcessorTest, Before_Sequence_OffsetUpdate_kInfiniteDuration_Fails)
{
    if (!use_sequence_mode_) {
        DVLOG(1) << "Skipping segments mode variant; inapplicable to this case.";
        return;
    ...
    EXPECT_MEDIA_LOG(SequenceOffsetUpdatePreventedByOutOfRangeGroupStartTimestamp());
...
TEST_P(FrameProcessorTest, Pts_AfterTimestampOffsetApplied_kNoTimestamp_Fails)
{
    ...
    if (use_sequence_mode_) {
        ...
        base::TimeDelta ts = Milliseconds(((kNoTimestamp + Milliseconds(10)) * -1).InMilliseconds());
        ...
        EXPECT_MEDIA_LOG(FrameTimeOutOfRange("After adjusting by timestampOffset", "PTS", "video"));
...
TEST_P(FrameProcessorTest, Pts_AfterTimestampOffsetApplied_kInfiniteDuration_Fails)
{
    InSequence s;
    ...
    frame_duration_ = Milliseconds(1);
    base::TimeDelta ts = Milliseconds((kInfiniteDuration - Milliseconds(2)).InMilliseconds());
...
TEST_P(FrameProcessorTest, Dts_AfterTimestampOffsetApplied_kNoDecodeTimestamp_Fails)
{
    InSequence s;
    ...
    SetTimestampOffset(kNoTimestamp + Milliseconds(5));
    EXPECT_MEDIA_LOG(FrameTimeOutOfRange("After adjusting by timestampOffset", "DTS", "audio"));
    EXPECT_FALSE(ProcessFrames("0|-10K", ""));
}

TEST_P(FrameProcessorTest, Dts_AfterTimestampOffsetApplied_kMaxDecodeTimestamp_Fails)
{
    InSequence s;
