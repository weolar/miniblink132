...
bool IsStreamEnabled(container_names::MediaContainerName container, AVStream* stream)
{
...
static base::Time ExtractTimelineOffset(container_names::MediaContainerName container, const AVFormatContext* format_context)
{
    if (container == container_names::MediaContainerName::kContainerWEBM) {
        const AVDictionaryEntry* entry = av_dict_get(format_context->metadata, "creation_time", nullptr, 0);

        base::Time timeline_offset;

        // FFmpegDemuxerTests assume base::Time::FromUTCString() is used here.
        if (entry != nullptr && entry->value != nullptr && base::Time::FromUTCString(entry->value, &timeline_offset)) {
...
{
    return base::Microseconds(frames * base::Time::kMicrosecondsPerSecond / sample_rate);
}
...
static base::TimeDelta ExtractStartTime(AVStream* stream)
{
    ...
    if (av_stream_get_first_dts(stream) != kNoFFmpegTimestamp && stream->codecpar->codec_id != AV_CODEC_ID_HEVC
        && stream->codecpar->codec_id != AV_CODEC_ID_H264 && stream->codecpar->codec_id != AV_CODEC_ID_MPEG4) {
        const base::TimeDelta first_pts = ConvertFromTimeBase(stream->time_base, av_stream_get_first_dts(stream));
...
static void RecordVideoCodecStats(
    container_names::MediaContainerName container, const VideoDecoderConfig& video_config, AVColorRange color_range, MediaLog* media_log)
{
    ...
    if (container == container_names::MediaContainerName::kContainerMOV) {
        base::UmaHistogramEnumeration("Media.SRC.VideoCodec.MP4", video_config.codec());
    } else if (container == container_names::MediaContainerName::kContainerWEBM) {
        base::UmaHistogramEnumeration("Media.SRC.VideoCodec.WebM", video_config.codec());
    }
}
...
template <> struct MediaLogPropertyTypeSupport<MediaLogProperty::kMaxDuration, base::TimeDelta> {
    static base::Value Convert(base::TimeDelta t)
    {
        return GetTimeValue(t);
    }
};

template <> struct MediaLogPropertyTypeSupport<MediaLogProperty::kStartTime, base::TimeDelta> {
    static base::Value Convert(base::TimeDelta t)
    {
        return GetTimeValue(t);
    }
...
static int ReadFrameAndDiscardEmpty(AVFormatContext* context, AVPacket* packet)
{
    ...
    do {
        ...
        if (drop_packet) {
            ...
            DLOG(WARNING) << "Dropping empty packet, size: " << packet->size << ", data: " << static_cast<void*>(packet->data);
        }
    } while (drop_packet);
...
std::unique_ptr<FFmpegDemuxerStream> FFmpegDemuxerStream::Create(FFmpegDemuxer* demuxer, AVStream* stream, MediaLog* media_log)
{
    if (!demuxer || !stream)
        return nullptr;
    ...
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (!AVStreamToAudioDecoderConfig(stream, audio_config.get()) || !audio_config->IsValidConfig()
            || !IsDecoderSupportedAudioType(AudioType::FromDecoderConfig(*audio_config))) {
        ...
        MEDIA_LOG(INFO, media_log) << "FFmpegDemuxer: created audio stream, config " << audio_config->AsHumanReadableString();
        ...
        if (!AVStreamToVideoDecoderConfig(stream, video_config.get()) || !video_config->IsValidConfig()
            || !IsDecoderSupportedVideoType(VideoType::FromDecoderConfig(*video_config))) {
            MEDIA_LOG(DEBUG, media_log) << "Warning, FFmpegDemuxer failed to create "
        ...
        MEDIA_LOG(INFO, media_log) << "FFmpegDemuxer: created video stream, config " << video_config->AsHumanReadableString();
    }

    return base::WrapUnique(new FFmpegDemuxerStream(demuxer, stream, std::move(audio_config), std::move(video_config), media_log));
}
...
// FFmpegDemuxerStream
//
FFmpegDemuxerStream::FFmpegDemuxerStream(FFmpegDemuxer* demuxer, AVStream* stream, std::unique_ptr<AudioDecoderConfig> audio_config,
    std::unique_ptr<VideoDecoderConfig> video_config, MediaLog* media_log)
    : demuxer_(demuxer)
...
void FFmpegDemuxerStream::EnqueuePacket(ScopedAVPacket packet)
{
    ...
    int64_t packet_dts = packet->dts == AV_NOPTS_VALUE ? packet->pts : packet->dts;
    ...
    if (is_audio && !fixup_chained_ogg_ && last_packet_pos_ != AV_NOPTS_VALUE) {
        ...
        if (packet->pos < last_packet_pos_) {
            DVLOG(3) << "Dropped packet with out of order position (" << packet->pos << " < " << last_packet_pos_ << ")";
            return;
        }
        if (last_packet_dts_ != AV_NOPTS_VALUE && packet->pos == last_packet_pos_ && packet_dts <= last_packet_dts_) {
            DVLOG(3) << "Dropped packet with out of order display timestamp (" << packet_dts << " < " << last_packet_dts_ << ")";
            return;
        }
    }

    if (!demuxer_ || end_of_stream_) {
        DUMP_WILL_BE_NOTREACHED() << "Attempted to enqueue packet on a stopped stream";
        return;
    }
    ...
    if (bitstream_converter_ && !bitstream_converter_->ConvertPacket(packet.get())) {
    ...
    if ((type() == DemuxerStream::AUDIO && audio_config_->is_encrypted()) || (type() == DemuxerStream::VIDEO && video_config_->is_encrypted())) {
        if (!WebMCreateDecryptConfig(packet->data, packet->size, reinterpret_cast<const uint8_t*>(encryption_key_id_.data()), encryption_key_id_.size(),
    ...
    if (is_audio && demuxer_->container() == container_names::MediaContainerName::kContainerMP3) {
        ...
        if (packet_end - header_start < MPEG1AudioStreamParser::kHeaderSize || !MPEG1AudioStreamParser::ParseHeader(nullptr, nullptr, header_start, nullptr)) {
            LIMITED_MEDIA_LOG(INFO, media_log_, num_discarded_packet_warnings_, 5)
                << "Discarding invalid MP3 packet, ts: " << ConvertStreamTimestamp(stream_->time_base, packet->pts)
                << ", duration: " << ConvertStreamTimestamp(stream_->time_base, packet->duration);
            return;
        }
    }
    ...
    if (side_data.size() > 0) {
        buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(side_data);
    }

    size_t skip_samples_size = 0;
    const uint32_t* skip_samples_ptr = reinterpret_cast<const uint32_t*>(av_packet_get_side_data(packet.get(), AV_PKT_DATA_SKIP_SAMPLES, &skip_samples_size));
    ...
    if (skip_samples_size >= kSkipSamplesValidSize) {
        ...
        const auto discard_end_samples = static_cast<int>(*(skip_samples_ptr + kSkipEndSamplesOffset));

        if (discard_front_samples || discard_end_samples) {
            DCHECK(is_audio);
            const int samples_per_second = audio_decoder_config().samples_per_second();
            buffer->set_discard_padding(
                std::make_pair(FramesToTimeDelta(discard_front_samples, samples_per_second), FramesToTimeDelta(discard_end_samples, samples_per_second)));
        }
    }
    ...
    if (packet->duration >= 0) {
        buffer->set_duration(ConvertStreamTimestamp(stream_->time_base, packet->duration));
    } else {
        // TODO(wolenetz): Remove when FFmpeg stops returning negative durations.
        // https://crbug.com/394418
        DVLOG(1) << "FFmpeg returned a buffer with a negative duration! " << packet->duration;
    ...
    if (stream_timestamp == kNoTimestamp || stream_timestamp == kInfiniteDuration) {
    ...
    if (packet->flags & AV_PKT_FLAG_DISCARD && buffer->discard_padding() == DecoderBuffer::DiscardPadding()) {
        buffer->set_discard_padding(std::make_pair(kInfiniteDuration, base::TimeDelta()));
    }
    ...
    if (is_audio) {
        ...
        if ((!fixup_chained_ogg_ || last_packet_timestamp_ == kNoTimestamp) && buffer->timestamp().is_negative()) {
            MEDIA_LOG(ERROR, media_log_) << "FFmpegDemuxer: unfixable negative timestamp.";
            demuxer_->NotifyDemuxerError(DEMUXER_ERROR_COULD_NOT_PARSE);
            return;
        }
        ...
        if (fixup_negative_timestamps_ && stream_timestamp.is_negative() && buffer->duration() != kNoTimestamp && !audio_decoder_config().codec_delay()) {
            if ((stream_timestamp + buffer->duration()).is_negative()) {
                ...
                buffer->set_discard_padding(std::make_pair(std::max(-stream_timestamp, buffer->discard_padding().first), buffer->discard_padding().second));
            }
        }
    }
    ...
    if (last_packet_timestamp_ != kNoTimestamp) {
        ...
        if (fixup_chained_ogg_ && buffer->timestamp() < last_packet_timestamp_) {
            buffer->set_timestamp(last_packet_timestamp_ + (last_packet_duration_ != kNoTimestamp ? last_packet_duration_ : base::Microseconds(1)));
        }
    ...
    if (buffer->timestamp() == kNoTimestamp || buffer->timestamp() == kInfiniteDuration) {
        MEDIA_LOG(ERROR, media_log_) << "FFmpegDemuxer: PTS is not defined";
        demuxer_->NotifyDemuxerError(DEMUXER_ERROR_COULD_NOT_PARSE);
        return;
    }
...
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(preserve_packet_position || !read_cb_) << "There should be no pending read";
...
void FFmpegDemuxerStream::Stop()
{
    ...
    end_of_stream_ = true;
    if (read_cb_) {
        std::move(read_cb_).Run(DemuxerStream::kOk, { DecoderBuffer::CreateEOSBuffer() });
    }
}
...
void FFmpegDemuxerStream::Read(uint32_t count, ReadCB read_cb)
{
    ...
    DVLOG(3) << __func__ << " requested_buffer_count_ = " << requested_buffer_count_;
...
void FFmpegDemuxerStream::InitBitstreamConverter()
{
    ...
    case AV_CODEC_ID_H264:
        ...
        bitstream_converter_ = std::make_unique<FFmpegH264ToAnnexBBitstreamConverter>(stream_->codecpar);
        break;
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    case AV_CODEC_ID_HEVC:
        bitstream_converter_ = std::make_unique<FFmpegH265ToAnnexBBitstreamConverter>(stream_->codecpar);
        break;
#endif
    case AV_CODEC_ID_AAC:
        ...
        if (audio_config_->profile() != AudioCodecProfile::kXHE_AAC) {
            bitstream_converter_ = std::make_unique<FFmpegAACBitstreamConverter>(stream_->codecpar);
        }
        break;
    default:
        break;
...
void FFmpegDemuxerStream::SetEnabled(bool enabled, base::TimeDelta timestamp)
{
    ...
    is_enabled_ = enabled;
    demuxer_->ffmpeg_task_runner()->PostTask(FROM_HERE, base::BindOnce(&SetAVStreamDiscard, av_stream(), enabled ? AVDISCARD_DEFAULT : AVDISCARD_ALL));
    if (is_enabled_) {
...
void FFmpegDemuxerStream::SatisfyPendingRead()
{
    ...
    if (read_cb_) {
        if (!buffer_queue_.IsEmpty()) {
            ...
            for (size_t i = 0; i < std::min(requested_buffer_count_, buffer_queue_.queue_size()); ++i) {
                output_buffers.emplace_back(buffer_queue_.Pop());
            }
            DVLOG(3) << __func__ << " Status:kOk, return output_buffers.size = " << output_buffers.size();
...
{
    const AVDictionaryEntry* entry = av_dict_get(stream_->metadata, key, nullptr, 0);
    return (entry == nullptr || entry->value == nullptr) ? "" : entry->value;
}

// static
base::TimeDelta FFmpegDemuxerStream::ConvertStreamTimestamp(const AVRational& time_base, int64_t timestamp)
{
...
// FFmpegDemuxer
//
FFmpegDemuxer::FFmpegDemuxer(const scoped_refptr<base::SequencedTaskRunner>& task_runner, DataSource* data_source,
    const EncryptedMediaInitDataCB& encrypted_media_init_data_cb, MediaTracksUpdatedCB media_tracks_updated_cb, MediaLog* media_log, bool is_local_file)
    ...
    // reads and seeks.
    blocking_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_BLOCKING }))
...
void FFmpegDemuxer::Initialize(DemuxerHost* host, PipelineStatusCallback init_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    host_ = host;
    ...
    url_protocol_ = std::make_unique<BlockingUrlProtocol>(
        data_source_, base::BindPostTaskToCurrentDefault(base::BindRepeating(&FFmpegDemuxer::OnDataSourceError, weak_this_)));
    ...
    blocking_task_runner_->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&FFmpegGlue::OpenContext, base::Unretained(glue_.get()), is_local_file_),
        base::BindOnce(&FFmpegDemuxer::OnOpenContextDone, weak_factory_.GetWeakPtr()));
}
...
void FFmpegDemuxer::AbortPendingReads()
{
    ...
    blocking_task_runner_->PostTask(FROM_HERE, base::BindOnce(&UnmarkEndOfStreamAndClearError, glue_->format_context()));
    pending_read_ = false;
...
void FFmpegDemuxer::CancelPendingSeek(base::TimeDelta seek_time)
{
    if (task_runner_->RunsTasksInCurrentSequence()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&FFmpegDemuxer::AbortPendingReads, weak_this_));
    }
}
...
void FFmpegDemuxer::Seek(base::TimeDelta time, PipelineStatusCallback cb)
{
    ...
    SeekInternal(time, base::BindOnce(&FFmpegDemuxer::OnSeekFrameDone, weak_factory_.GetWeakPtr()));
}
...
void FFmpegDemuxer::SeekInternal(base::TimeDelta time, base::OnceCallback<void(int)> seek_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    FFmpegDemuxerStream* audio_stream = GetFirstEnabledFFmpegStream(DemuxerStream::AUDIO);
    if (audio_stream) {
    ...
    blocking_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&AVSeekFrame, glue_->format_context(), seeking_stream->index, ConvertToTimeBase(seeking_stream->time_base, seek_time)),
        std::move(seek_cb));
}
...
FFmpegDemuxerStream* FFmpegDemuxer::GetFirstEnabledFFmpegStream(DemuxerStream::Type type) const
{
    for (const auto& stream : streams_) {
...
std::optional<container_names::MediaContainerName> FFmpegDemuxer::GetContainerForMetrics() const
{
    return container();
}

void FFmpegDemuxer::OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::string& encryption_key_id)
{
    std::vector<uint8_t> key_id_local(encryption_key_id.begin(), encryption_key_id.end());
...
static int CalculateBitrate(AVFormatContext* format_context, const base::TimeDelta& duration, int64_t filesize_in_bytes)
{
    ...
    // valid duration.
    if (duration <= base::TimeDelta() || duration == kInfiniteDuration || !filesize_in_bytes)
        return 0;
...
void FFmpegDemuxer::OnOpenContextDone(bool result)
{
    ...
    if (glue_->detected_hls()) {
        MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": detected HLS manifest";
    ...
    blocking_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&avformat_find_stream_info, glue_->format_context(), static_cast<AVDictionary**>(nullptr)),
        base::BindOnce(&FFmpegDemuxer::OnFindStreamInfoDone, weak_factory_.GetWeakPtr()));
}
...
void FFmpegDemuxer::OnFindStreamInfoDone(int result)
{
    ...
    if (result < 0) {
        MEDIA_LOG(ERROR, media_log_) << GetDisplayName() << ": find stream info failed";
    ...
    if (has_disabled_stream && !has_enabled_stream) {
        MEDIA_LOG(WARNING, media_log_) << GetDisplayName() << ": no tracks are enabled, track enabled flag will be ignored";
    }
    ...
    for (size_t i = 0; i < format_context->nb_streams; ++i) {
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            if (is_local_file_) {
                base::UmaHistogramSparse("Media.DetectedAudioCodecHash.Local", codec_hash);
            }
            ...
            if (is_local_file_) {
                base::UmaHistogramSparse("Media.DetectedVideoCodecHash.Local", codec_hash);
            }
        ...
        if (demuxer_stream.get()) {
            ...
            if (codec_type == AVMEDIA_TYPE_AUDIO) {
                MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": skipping invalid or unsupported audio track";
            } else if (codec_type == AVMEDIA_TYPE_VIDEO) {
                MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": skipping invalid or unsupported video track";
            }
        ...
        StreamParser::TrackId track_id = static_cast<StreamParser::TrackId>(media_tracks->tracks().size() + 1);
        ...
        if (glue_->container() == container_names::MediaContainerName::kContainerWEBM) {
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            if (has_enabled_stream && !IsStreamEnabled(container(), stream)) {
                MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": skipping disabled audio track";
            ...
            if (has_enabled_stream && !IsStreamEnabled(container(), stream)) {
                MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": skipping disabled video track";
                stream_enabled = false;
            } else {
        ...
        if ((codec_type == AVMEDIA_TYPE_AUDIO && media_tracks->getAudioConfig(track_id).IsValidConfig())
            || (codec_type == AVMEDIA_TYPE_VIDEO && media_tracks->getVideoConfig(track_id).IsValidConfig())) {
            MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": skipping duplicate media stream id=" << track_id;
            continue;
        }
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            media_track = media_tracks->AddAudioTrack(audio_config, stream_enabled, track_id, MediaTrack::Kind("main"), track_label, track_language);
            media_track->set_id(MediaTrack::Id(base::NumberToString(track_id)));
            DCHECK(track_id_to_demux_stream_map_.find(media_track->track_id()) == track_id_to_demux_stream_map_.end());
            track_id_to_demux_stream_map_[media_track->track_id()] = streams_[i].get();
        } else if (codec_type == AVMEDIA_TYPE_VIDEO) {
            VideoDecoderConfig video_config = streams_[i]->video_decoder_config();

            RecordVideoCodecStats(glue_->container(), video_config, stream->codecpar->color_range, media_log_);

            media_track = media_tracks->AddVideoTrack(video_config, stream_enabled, track_id, MediaTrack::Kind("main"), track_label, track_language);
            media_track->set_id(MediaTrack::Id(base::NumberToString(track_id)));
            DCHECK(track_id_to_demux_stream_map_.find(media_track->track_id()) == track_id_to_demux_stream_map_.end());
            track_id_to_demux_stream_map_[media_track->track_id()] = streams_[i].get();
        }
        ...
        if (codec_type == AVMEDIA_TYPE_AUDIO && start_time.is_negative() && is_opus_or_vorbis) {
    ...
    if (media_tracks->tracks().empty()) {
        MEDIA_LOG(ERROR, media_log_) << GetDisplayName() << ": no supported streams";
    ...
    if (format_context->duration != kNoFFmpegTimestamp) {
        ...
        max_duration = std::max(max_duration, ConvertFromTimeBase(av_time_base, format_context->duration));
    } else {
    ...
    const bool needs_chained_ogg_fixup = glue_->container() == container_names::MediaContainerName::kContainerOgg && supported_audio_track_count == 1
        && !supported_video_track_count && has_opus_or_vorbis_audio;
    ...
    if (glue_->container() == container_names::MediaContainerName::kContainerAVI) {
    ...
    if (glue_->container() == container_names::MediaContainerName::kContainerMP3) {
        start_time_ = base::TimeDelta();
    }

    // For testing purposes, don't overwrite the timeline offset if set already.
    if (timeline_offset_.is_null()) {
        timeline_offset_ = ExtractTimelineOffset(glue_->container(), format_context);
    }
...
void FFmpegDemuxer::LogMetadata(AVFormatContext* avctx, base::TimeDelta max_duration)
{
...
FFmpegDemuxerStream* FFmpegDemuxer::FindStreamWithLowestStartTimestamp(bool enabled)
{
...
FFmpegDemuxerStream* FFmpegDemuxer::FindPreferredStreamForSeeking(base::TimeDelta seek_time)
{
...
void FFmpegDemuxer::FindAndEnableProperTracks(
    const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, DemuxerStream::Type track_type, TrackChangeCB change_completed_cb)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    ...
    for (const auto& id : track_ids) {
        ...
        if (!enabled_streams.empty()) {
            MEDIA_LOG(INFO, media_log_) << "Only one enabled audio track is supported, ignoring track " << id;
            continue;
        }
    ...
    // that are enabled.
    for (const auto& stream : streams_) {
        if (stream && stream->type() == track_type && enabled_streams.find(stream.get()) == enabled_streams.end()) {
            DVLOG(1) << __func__ << ": disabling stream " << stream.get();
            stream->SetEnabled(false, curr_time);
        }
    }

    std::vector<DemuxerStream*> streams(enabled_streams.begin(), enabled_streams.end());
    std::move(change_completed_cb).Run(streams);
}

void FFmpegDemuxer::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    FindAndEnableProperTracks(track_ids, curr_time, DemuxerStream::AUDIO, std::move(change_completed_cb));
}

void FFmpegDemuxer::OnVideoSeekedForTrackChange(DemuxerStream* video_stream, base::OnceClosure seek_completed_cb, int result)
{
...
void FFmpegDemuxer::SeekOnVideoTrackChange(base::TimeDelta seek_to_time, TrackChangeCB seek_completed_cb, const std::vector<DemuxerStream*>& streams)
{
    if (streams.size() != 1u) {
    ...
    SeekInternal(seek_to_time,
        base::BindOnce(
            &FFmpegDemuxer::OnVideoSeekedForTrackChange, weak_factory_.GetWeakPtr(), streams[0], base::BindOnce(std::move(seek_completed_cb), streams)));
}

void FFmpegDemuxer::OnSelectedVideoTrackChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    ...
    FindAndEnableProperTracks(track_ids, curr_time, DemuxerStream::VIDEO,
        track_ids.empty() ? std::move(change_completed_cb)
                          : base::BindOnce(&FFmpegDemuxer::SeekOnVideoTrackChange, weak_factory_.GetWeakPtr(), curr_time, std::move(change_completed_cb)));
}
...
void FFmpegDemuxer::ReadFrameIfNeeded()
{
    ...
    if (stopped_ || !StreamsHaveAvailableCapacity() || pending_read_ || pending_seek_cb_) {
        return;
    }
    ...
    pending_read_ = true;
    blocking_task_runner_->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&ReadFrameAndDiscardEmpty, glue_->format_context(), packet_ptr),
        base::BindOnce(&FFmpegDemuxer::OnReadFrameDone, weak_factory_.GetWeakPtr(), std::move(packet)));
}
...
void FFmpegDemuxer::OnReadFrameDone(ScopedAVPacket packet, int result)
{
    ...
    if (result < 0 || IsMaxMemoryUsageReached()) {
        if (result < 0) {
            MEDIA_LOG(DEBUG, media_log_) << GetDisplayName() << ": av_read_frame(): " << AVErrorToString(result);
        } else {
            MEDIA_LOG(DEBUG, media_log_) << GetDisplayName() << ": memory limit exceeded";
        }
    ...
    if (packet->stream_index >= 0 && static_cast<size_t>(packet->stream_index) < streams_.size()) {
...
bool FFmpegDemuxer::IsMaxMemoryUsageReached() const
{
    ...
    size_t memory_left = GetDemuxerMemoryLimit(Demuxer::DemuxerTypes::kFFmpegDemuxer);
    for (const auto& stream : streams_) {
        if (!stream)
            continue;
...
{
    MEDIA_LOG(ERROR, media_log_) << GetDisplayName() << ": demuxer error: " << status;
...
void FFmpegDemuxer::RunInitCB(PipelineStatus status)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(init_cb_);
    TRACE_EVENT_ASYNC_END1("media", "FFmpegDemuxer::Initialize", this, "status", PipelineStatusToString(status));
...
void FFmpegDemuxer::RunPendingSeekCB(PipelineStatus status)
{
    DCHECK(task_runner_->RunsTasksInCurrentSequence());
    DCHECK(pending_seek_cb_);
    TRACE_EVENT_ASYNC_END1("media", "FFmpegDemuxer::Seek", this, "status", PipelineStatusToString(status));
