...
public:
    FFmpegAACBitstreamConverterTest(const FFmpegAACBitstreamConverterTest&) = delete;
    FFmpegAACBitstreamConverterTest& operator=(const FFmpegAACBitstreamConverterTest&) = delete;
...
TEST_F(FFmpegAACBitstreamConverterTest, Conversion_Success)
{
    ...
    EXPECT_EQ(static_cast<long>(test_packet->size), static_cast<long>(sizeof(dummy_packet) + FFmpegAACBitstreamConverter::kAdtsHeaderSize));
    EXPECT_EQ(memcmp(reinterpret_cast<void*>(test_packet->data + FFmpegAACBitstreamConverter::kAdtsHeaderSize), reinterpret_cast<void*>(dummy_packet),
                  sizeof(dummy_packet)),
        0);
}
...
TEST_F(FFmpegAACBitstreamConverterTest, Conversion_MultipleLength)
{
    ...
    CreatePacket(second_test_packet.get(), dummy_packet, sizeof(dummy_packet) - 1);
