...
FFmpegH264ToAnnexBBitstreamConverter::FFmpegH264ToAnnexBBitstreamConverter(AVCodecParameters* stream_codec_parameters)
...
FFmpegH264ToAnnexBBitstreamConverter::~FFmpegH264ToAnnexBBitstreamConverter() = default;
...
bool FFmpegH264ToAnnexBBitstreamConverter::ConvertPacket(AVPacket* packet)
{
    ...
    uint32_t output_packet_size = converter_.CalculateNeededOutputBufferSize(packet->data, packet->size, avc_config.get());
    ...
    if (!converter_.ConvertNalUnitStreamToByteStream(packet->data, packet->size, avc_config.get(), dest_packet.data, &io_size)) {
