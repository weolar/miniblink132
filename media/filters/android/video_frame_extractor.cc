...
void VideoFrameExtractor::Start(VideoFrameCallback video_frame_callback)
{
    ...
    protocol_ = std::make_unique<BlockingUrlProtocol>(data_source_, base::BindRepeating(&VideoFrameExtractor::OnError, weak_factory_.GetWeakPtr()));
    ...
    NotifyComplete(std::vector<uint8_t>(packet->data, packet->data + packet->size), video_config_);
}
...
void VideoFrameExtractor::ConvertPacket(AVPacket* packet)
{
    ...
    case AV_CODEC_ID_H264:
        ...
        bitstream_converter_ = std::make_unique<FFmpegH264ToAnnexBBitstreamConverter>(video_stream_->codecpar);
        break;
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    case AV_CODEC_ID_HEVC:
        bitstream_converter_ = std::make_unique<FFmpegH265ToAnnexBBitstreamConverter>(video_stream_->codecpar);
        break;
#endif
    case AV_CODEC_ID_AAC:
        bitstream_converter_ = std::make_unique<FFmpegAACBitstreamConverter>(video_stream_->codecpar);
        break;
    default:
        break;
    }
...
void VideoFrameExtractor::NotifyComplete(std::vector<uint8_t> encoded_frame, const VideoDecoderConfig& config)
{
...
{
    DCHECK(video_frame_callback_);
    std::move(video_frame_callback_).Run(false, std::vector<uint8_t>(), VideoDecoderConfig());
}
