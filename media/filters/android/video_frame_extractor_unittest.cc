...
void OnFrameExtracted(ExtractVideoFrameResult* result, base::RepeatingClosure quit_closure, bool success, std::vector<uint8_t> encoded_frame,
...
protected:
    ...
    {
        base::File file(file_path, base::File::Flags::FLAG_OPEN | base::File::Flags::FLAG_READ);
        DCHECK(file.IsValid());
        ...
        base::RunLoop loop;
        extractor_->Start(base::BindOnce(&OnFrameExtracted, &result, loop.QuitClosure()));
        loop.Run();
        return result;
    }
...
TEST_F(VideoFrameExtractorTest, ExtractInvalidVideoFile)
{
    ...
    base::FilePath file = base::CreateUniqueTempDirectoryScopedToTest().AppendASCII("test.txt");
