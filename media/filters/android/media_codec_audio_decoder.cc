...
MediaCodecAudioDecoder::MediaCodecAudioDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner)
    ...
    const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (!is_codec_supported) {
        ...
        base::BindPostTaskToCurrentDefault(std::move(init_cb)).Run(DecoderStatus::Codes::kUnsupportedCodec);
        return;
    }
...
bool MediaCodecAudioDecoder::CreateMediaCodecLoop()
{
    ...
    codec_loop_.reset();
    const base::android::JavaRef<jobject>& media_crypto = media_crypto_ ? *media_crypto_ : nullptr;
    std::unique_ptr<MediaCodecBridge> audio_codec_bridge(MediaCodecBridgeImpl::CreateAudioDecoder(config_, media_crypto,
        base::BindPostTaskToCurrentDefault(base::BindRepeating(&MediaCodecAudioDecoder::PumpMediaCodecLoop, weak_factory_.GetWeakPtr()))));
    ...
    codec_loop_ = std::make_unique<MediaCodecLoop>(
        base::android::BuildInfo::GetInstance()->sdk_int(), this, std::move(audio_codec_bridge), scoped_refptr<base::SingleThreadTaskRunner>());

    return true;
}

void MediaCodecAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    if (!buffer->end_of_stream() && buffer->timestamp() == kNoTimestamp) {
        DVLOG(2) << __func__ << " " << buffer->AsHumanReadableString() << ": no timestamp, skipping this buffer";
        std::move(bound_decode_cb).Run(DecoderStatus::Codes::kFailed);
        return;
    }
    ...
    if (state_ == STATE_ERROR) {
        ...
        DVLOG(2) << __func__ << " " << buffer->AsHumanReadableString() << ": Error state, returning decode error for all buffers";
    ...
    input_queue_.push_back(std::make_pair(std::move(buffer), std::move(bound_decode_cb)));
...
void MediaCodecAudioDecoder::SetCdm(CdmContext* cdm_context, InitCB init_cb)
{
    ...
    event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&MediaCodecAudioDecoder::OnCdmContextEvent, weak_factory_.GetWeakPtr()));

    // The callback will be posted back to this thread via
    // base::BindPostTaskToCurrentDefault.
    media_crypto_context_->SetMediaCryptoReadyCB(
        base::BindPostTaskToCurrentDefault(base::BindOnce(&MediaCodecAudioDecoder::OnMediaCryptoReady, weak_factory_.GetWeakPtr(), std::move(init_cb))));
}
...
void MediaCodecAudioDecoder::OnMediaCryptoReady(InitCB init_cb, JavaObjectPtr media_crypto, bool /*requires_secure_video_codec*/)
{
    DVLOG(1) << __func__;
...
void MediaCodecAudioDecoder::OnInputDataQueued(bool success)
{
    ...
    std::move(input_queue_.front().second).Run(success ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailed);
...
{
    DVLOG(3) << __func__ << ": " << AsString(state_) << "->" << AsString(new_state);
    state_ = new_state;
}
...
bool MediaCodecAudioDecoder::OnDecodedEos(const MediaCodecLoop::OutputBuffer& out)
{
...
bool MediaCodecAudioDecoder::OnDecodedFrame(const MediaCodecLoop::OutputBuffer& out)
{
    ...
    if (is_passthrough_) {
        audio_buffer = AudioBuffer::CreateBitstreamBuffer(sample_format_, channel_layout_, channel_count_, sample_rate_, frame_count, out.size, pool_);

        // TODO(crbug.com/373960632): Use spans from AudioBuffer directly once that
        // class is spanified.
        auto dst = UNSAFE_TODO(base::span<uint8_t>(audio_buffer->channel_data()[0], out.size));

        MediaCodecResult result = media_codec->CopyFromOutputBuffer(out.index, out.offset, dst);
        ...
        if (config_.codec() == AudioCodec::kAC3) {
            frame_count = Ac3Util::ParseTotalAc3SampleCount(audio_buffer->channel_data()[0], out.size);
        } else if (config_.codec() == AudioCodec::kEAC3) {
            frame_count = Ac3Util::ParseTotalEac3SampleCount(audio_buffer->channel_data()[0], out.size);
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
        } else if (config_.codec() == AudioCodec::kDTS) {
            frame_count = media::dts::ParseTotalSampleCount(audio_buffer->channel_data()[0], out.size, AudioCodec::kDTS);
        ...
        // Create AudioOutput buffer based on current parameters.
        audio_buffer = AudioBuffer::CreateBuffer(sample_format_, channel_layout_, channel_count_, sample_rate_, frame_count, pool_);
    }
...
bool MediaCodecAudioDecoder::OnOutputFormatChanged()
{
    ...
    MediaCodecResult result = media_codec->GetOutputSamplingRate(&new_sampling_rate);
    if (!result.is_ok()) {
        DLOG(ERROR) << "GetOutputSamplingRate failed, result: " << result.message();
        return false;
    }
    if (new_sampling_rate != sample_rate_) {
        DVLOG(1) << __func__ << ": detected sample rate change: " << sample_rate_ << " -> " << new_sampling_rate;
    ...
    if (new_channel_count != channel_count_) {
        DVLOG(1) << __func__ << ": detected channel count change: " << channel_count_ << " -> " << new_channel_count;
