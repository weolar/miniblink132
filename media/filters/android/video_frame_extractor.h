...
public:
    using VideoFrameCallback = base::OnceCallback<void(bool success, std::vector<uint8_t> data, const VideoDecoderConfig& decoder_config)>;
...
private:
    ...
    void NotifyComplete(std::vector<uint8_t> encoded_frame, const VideoDecoderConfig& config);
