...
public:
    ...
    explicit Gav1VideoDecoder(MediaLog* media_log, OffloadState offload_state = OffloadState::kNormal);
    ...
    scoped_refptr<VideoFrame> CreateVideoFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect);
...
public:
    explicit OffloadingGav1VideoDecoder(MediaLog* media_log)
        : OffloadingVideoDecoder(
            0, std::vector<VideoCodec>(1, VideoCodec::kAV1), std::make_unique<Gav1VideoDecoder>(media_log, OffloadableVideoDecoder::OffloadState::kOffloaded))
    {
    }
};
} // namespace media
