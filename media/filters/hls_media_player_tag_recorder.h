...
public:
    explicit HlsMediaPlayerTagRecorder(std::unique_ptr<HlsNetworkAccess> network_access);
...
private:
    ...
    void OnPlaylistFetch(GURL root_playlist_uri, HlsDataSourceProvider::ReadResult result);
    ...
    std::optional<uint32_t> advanced_feature_bitfield_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::optional<uint32_t> playlist_segment_bitfield_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::optional<uint32_t> segment_encryption_bitfield_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::optional<uint32_t> playlist_invalid_errorcode_ GUARDED_BY_CONTEXT(sequence_checker_);
    std::optional<hls::Playlist::Kind> root_playlist_type_ GUARDED_BY_CONTEXT(sequence_checker_);

    // Network access structure.
    std::unique_ptr<HlsNetworkAccess> network_access_ GUARDED_BY_CONTEXT(sequence_checker_);
