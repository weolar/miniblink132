...
bool IvfParser::Initialize(const uint8_t* stream, size_t size, IvfFileHeader* file_header)
{
    DCHECK(stream);
    DCHECK(file_header);
    ptr_ = stream;
    ...
    if (memcmp(file_header->signature, kIvfHeaderSignature, sizeof(file_header->signature)) != 0) {
        DLOG(ERROR) << "IVF signature mismatch";
        return false;
    }
    DLOG_IF(WARNING, file_header->version != 0) << "IVF version unknown: " << file_header->version << ", the parser may not be able to parse correctly";
...
bool IvfParser::ParseNextFrame(IvfFrameHeader* frame_header, const uint8_t** payload)
{
    DCHECK(ptr_);
    DCHECK(payload);
    CHECK_GE(end_, ptr_);
