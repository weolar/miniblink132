...
public:
    DecryptingDemuxerStream(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log, const WaitingCB& waiting_cb);
    ...
    void Initialize(DemuxerStream* stream, CdmContext* cdm_context, PipelineStatusCallback status_cb);
...
private:
    ...
    enum State { kUninitialized = 0, kIdle, kPendingDemuxerRead, kPendingDecrypt, kWaitingForKey };

    void OnBuffersReadFromDemuxerStream(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers);
    // Callback for DemuxerStream::Read().
    void OnBufferReadFromDemuxerStream(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer);

    void DecryptPendingBuffer();

    // Callback for Decryptor::Decrypt().
    void OnBufferDecrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer);
