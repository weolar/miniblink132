...
static void CheckBuffers(base::TimeDelta expected_start_time, const int expected_duration_time_ms, const DemuxerStream::DecoderBufferVector& buffers)
{
...
static void OnReadDone_Ok(base::TimeDelta expected_start_time, const int expected_duration_time_ms, const size_t expected_read_count, bool* called,
...
static void OnReadDone_AbortExpected(bool* called, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
...
static void OnReadDone_LastBufferEOSExpected(bool* called, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
...
static void StoreStatusAndBuffers(DemuxerStream::Status* status_out, DemuxerStream::DecoderBufferVector* buffers_out, DemuxerStream::Status status,
...
protected:
    ...
    {
        init_segment_received_cb_ = base::BindRepeating(&ChunkDemuxerTest::InitSegmentReceived, base::Unretained(this));
        CreateNewDemuxer();
    }
    ...
    {
        base::OnceClosure open_cb = base::BindOnce(&ChunkDemuxerTest::DemuxerOpened, base::Unretained(this));
        base::RepeatingClosure progress_cb = base::BindRepeating(&ChunkDemuxerTest::OnProgress, base::Unretained(this));
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb
            = base::BindRepeating(&ChunkDemuxerTest::OnEncryptedMediaInitData, base::Unretained(this));
        EXPECT_MEDIA_LOG(ChunkDemuxerCtor());
        demuxer_ = std::make_unique<ChunkDemuxer>(std::move(open_cb), progress_cb, encrypted_media_init_data_cb, &media_log_);
    }

    ~ChunkDemuxerTest() override
    {
        ShutdownDemuxer();
    }

    void CreateInitSegment(int stream_flags, bool is_audio_encrypted, bool is_video_encrypted, base::HeapArray<uint8_t>* buffer)
    {
        ...
        size_t size = ebml_header->size() + info->size() + kTracksHeaderSize + tracks_element_size;
        ...
        *buffer = base::HeapArray<uint8_t>::Uninit(size);
        ...
        if (has_video) {
            ...
            if (is_video_encrypted) {
                memcpy(buf + video_track_entry->size(), video_content_encodings->data(), video_content_encodings->size());
                WriteInt64(buf + kVideoTrackSizeOffset, video_track_entry->size() + video_content_encodings->size() - kVideoTrackEntryHeaderSize);
        ...
        if (has_audio) {
            ...
            if (is_audio_encrypted) {
                memcpy(buf + audio_track_entry->size(), audio_content_encodings->data(), audio_content_encodings->size());
                WriteInt64(buf + kAudioTrackSizeOffset, audio_track_entry->size() + audio_content_encodings->size() - kAudioTrackEntryHeaderSize);
    ...
    ChunkDemuxer::Status AddId(const std::string& source_id, const std::string& mime_type, const std::string& codecs)
    {
        ...
        if (status == ChunkDemuxer::kOk) {
            ...
            demuxer_->SetParseWarningCallback(source_id, base::BindRepeating(&ChunkDemuxerTest::OnParseWarningMock, base::Unretained(this)));
        }
        return status;
    }

#if BUILDFLAG(ENABLE_HLS_DEMUXER)
    void AddAutoDetectedCodecsId_Checked(const std::string& id, RelaxedParserSupportedType mime_type)
    {
        CHECK_EQ(demuxer_->AddAutoDetectedCodecsId(id, mime_type), ChunkDemuxer::kOk);
        demuxer_->SetTracksWatcher(id, init_segment_received_cb_);
        demuxer_->SetParseWarningCallback(id, base::BindRepeating(&ChunkDemuxerTest::OnParseWarningMock, base::Unretained(this)));
    }
#endif
    ...
    bool AppendCluster(const std::string& source_id, std::unique_ptr<Cluster> cluster)
    {
        return AppendData(source_id, base::make_span(cluster->data(), static_cast<size_t>(cluster->bytes_used())));
    }
    ...
    void AppendSingleStreamCluster(const std::string& source_id, int track_number, int timecode, int block_count)
    {
        ...
        ASSERT_TRUE(AppendCluster(source_id, GenerateSingleStreamCluster(timecode, end_timecode, track_number, block_duration)));
    }
    ...
    //  with duration 10ms.
    void ParseBlockDescriptions(int track_number, const std::string block_descriptions, std::vector<BlockInfo>* blocks)
    {
        std::vector<std::string> timestamps = base::SplitString(block_descriptions, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        ...
        for (size_t i = 0; i < timestamps.size(); ++i) {
            ...
            if (explicit_duration) {
                CHECK(base::StringToInt(timestamp_str.substr(duration_pos + 1), &block_info.duration));
            ...
            if (track_number == kAlternateTextTrackNum) {
                ...
                ASSERT_EQ(kWebMFlagKeyframe, block_info.flags)
                    << "Text block with timestamp " << block_info.timestamp_in_ms << " was not marked as a key frame."
    ...
    std::unique_ptr<Cluster> GenerateCluster(const std::vector<BlockInfo>& blocks, bool unknown_size)
    {
        ...
        for (size_t i = 0; i < blocks.size(); ++i) {
            ...
            if (blocks[i].duration) {
                cb.AddBlockGroup(blocks[i].track_number, blocks[i].timestamp_in_ms, blocks[i].duration, blocks[i].flags, blocks[i].flags & kWebMFlagKeyframe,
                    &data[0], block_size_);
            } else {
                cb.AddSimpleBlock(blocks[i].track_number, blocks[i].timestamp_in_ms, blocks[i].flags, &data[0], block_size_);
            }
        }
    ...
    std::unique_ptr<Cluster> GenerateCluster(std::priority_queue<BlockInfo> block_queue, bool unknown_size)
    {
    ...
    // the string format.
    void AppendSingleStreamCluster(const std::string& source_id, int track_number, const std::string& block_descriptions)
    {
    ...
    struct MuxedStreamInfo {
        ...
        MuxedStreamInfo(int track_num, const char* block_desc, int last_block_duration_estimate)
            : track_number(track_num)
            , block_descriptions(block_desc)
    ...
    void AppendMuxedCluster(const MuxedStreamInfo& msi_1, const MuxedStreamInfo& msi_2)
    {
    ...
    void AppendMuxedCluster(const MuxedStreamInfo& msi_1, const MuxedStreamInfo& msi_2, const MuxedStreamInfo& msi_3)
    {
    ...
    std::unique_ptr<Cluster> GenerateMuxedCluster(const std::vector<MuxedStreamInfo> msi)
    {
        std::priority_queue<BlockInfo> block_queue;
        for (size_t i = 0; i < msi.size(); ++i) {
            std::vector<BlockInfo> track_blocks;
            ParseBlockDescriptions(msi[i].track_number, msi[i].block_descriptions, &track_blocks);
            ...
            if (msi[i].last_blocks_estimated_duration != -1) {
                EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(msi[i].last_blocks_estimated_duration));
            }
        }
    ...
    bool AppendData(const std::string& source_id, base::span<const uint8_t> data)
    {
        ...
        // Now parse it.
        StreamParser::ParseStatus result = StreamParser::ParseStatus::kSuccessHasMoreData;
        while (result == StreamParser::ParseStatus::kSuccessHasMoreData) {
            result = demuxer_->RunSegmentParserLoop(
                source_id, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[source_id]);
        }
    ...
    bool AppendInitSegmentWithSourceId(const std::string& source_id, int stream_flags)
    {
        return AppendInitSegmentWithEncryptedInfo(source_id, stream_flags, false, false);
    }

    bool AppendInitSegmentWithEncryptedInfo(const std::string& source_id, int stream_flags, bool is_audio_encrypted, bool is_video_encrypted)
    {
        base::HeapArray<uint8_t> info_tracks;
        CreateInitSegment(stream_flags, is_audio_encrypted, is_video_encrypted, &info_tracks);
    ...
    void AppendGarbage()
    {
        ...
        auto garbage_cluster = base::HeapArray<uint8_t>::Uninit(garbage_cluster_size);
        for (int i = 0; i < garbage_cluster_size; ++i)
            garbage_cluster[i] = i;
        ASSERT_FALSE(AppendData(garbage_cluster));
    }

    PipelineStatusCallback CreateInitDoneCallback(base::TimeDelta expected_duration, PipelineStatus expected_status)
    {
    ...
    PipelineStatusCallback CreateInitDoneCallback(PipelineStatus expected_status)
    {
        EXPECT_CALL(*this, DemuxerInitialized(expected_status));
        return base::BindOnce(&ChunkDemuxerTest::DemuxerInitialized, base::Unretained(this));
    }
    ...
    bool InitDemuxerWithEncryptionInfo(int stream_flags, bool is_audio_encrypted, bool is_video_encrypted)
    {
        ...
        if (is_audio_encrypted || is_video_encrypted) {
            ...
            EXPECT_CALL(*this,
                OnEncryptedMediaInitData(
                    EmeInitDataType::WEBM, std::vector<uint8_t>(kEncryptedMediaInitData, kEncryptedMediaInitData + std::size(kEncryptedMediaInitData))))
        ...
        demuxer_->Initialize(&host_, CreateInitDoneCallback(expected_duration, expected_status));

        if (AddId(kSourceId, stream_flags) != ChunkDemuxer::kOk)
            return false;

        return AppendInitSegmentWithEncryptedInfo(kSourceId, stream_flags, is_audio_encrypted, is_video_encrypted);
    }

    bool InitDemuxerAudioAndVideoSourcesText(const std::string& audio_id, const std::string& video_id)
    {
        EXPECT_CALL(*this, DemuxerOpened());
        demuxer_->Initialize(&host_, CreateInitDoneCallback(kDefaultDuration(), PIPELINE_OK));
    ...
    bool InitDemuxerAudioAndVideoSources(const std::string& audio_id, const std::string& video_id)
    {
    ...
    bool InitDemuxerWithConfigChangeData()
    {
        ...
        EXPECT_CALL(*this, InitSegmentReceivedMock(_));
        demuxer_->Initialize(&host_, CreateInitDoneCallback(base::Milliseconds(2744), PIPELINE_OK));
    ...
    std::unique_ptr<Cluster> GenerateCluster(int first_audio_timecode, int first_video_timecode, int block_count)
    {
        return GenerateCluster(first_audio_timecode, first_video_timecode, block_count, false);
    }
    std::unique_ptr<Cluster> GenerateCluster(int first_audio_timecode, int first_video_timecode, int block_count, bool unknown_size)
    {
        ...
        if (block_count == 1) {
            block_queue.push(BlockInfo(kAudioTrackNum, first_audio_timecode, kWebMFlagKeyframe, kAudioBlockDuration));
        ...
        for (int i = 0; i < block_count - 2; i++) {
            if (audio_timecode <= video_timecode) {
                block_queue.push(BlockInfo(kAudioTrackNum, audio_timecode, kWebMFlagKeyframe, 0));
                audio_timecode += kAudioBlockDuration;
                continue;
            }

            block_queue.push(BlockInfo(kVideoTrackNum, video_timecode, video_flag, 0));
        ...
        block_queue.push(BlockInfo(kAudioTrackNum, audio_timecode, kWebMFlagKeyframe, kAudioBlockDuration));
        block_queue.push(BlockInfo(kVideoTrackNum, video_timecode, video_flag, kVideoBlockDuration));

        return GenerateCluster(block_queue, unknown_size);
    }

    std::unique_ptr<Cluster> GenerateSingleStreamCluster(int timecode, int end_timecode, int track_number, int block_duration)
    {
        ...
        while (timecode < (end_timecode - block_duration)) {
            cb.AddSimpleBlock(track_number, timecode, kWebMFlagKeyframe, &data[0], block_size_);
            timecode += block_duration;
        }

        cb.AddBlockGroup(track_number, timecode, block_duration, kWebMFlagKeyframe, static_cast<bool>(kWebMFlagKeyframe), &data[0], block_size_);
    ...
    void Read(uint32_t count, DemuxerStream::Type type, DemuxerStream::ReadCB read_cb)
    {
    ...
    void GenerateExpectedReads(int start_audio_timecode, int start_video_timecode, int block_count)
    {
        CHECK_GT(block_count, 0);
    ...
    void GenerateSingleStreamExpectedReads(int timecode, int block_count, DemuxerStream::Type type, int block_duration)
    {
        CHECK_GT(block_count, 0);
    ...
    {
        GenerateSingleStreamExpectedReads(timecode, block_count, DemuxerStream::AUDIO, kAudioBlockDuration);
    }

    void GenerateVideoStreamExpectedReads(int timecode, int block_count)
    {
        GenerateSingleStreamExpectedReads(timecode, block_count, DemuxerStream::VIDEO, kVideoBlockDuration);
    }
    ...
    void CheckExpectedRanges(DemuxerStream::Type type, const std::string& expected)
    {
        ChunkDemuxerStream* stream = static_cast<ChunkDemuxerStream*>(GetStream(type));
        CheckExpectedRanges(stream->GetBufferedRanges(kDefaultDuration()), expected);
    }

    void CheckExpectedRanges(const Ranges<base::TimeDelta>& r, const std::string& expected)
    {
        std::stringstream ss;
        ss << "{ ";
        for (size_t i = 0; i < r.size(); ++i) {
            ss << "[" << r.start(i).InMilliseconds() << "," << r.end(i).InMilliseconds() << ") ";
        }
        ss << "}";
        EXPECT_EQ(expected, ss.str());
    }

    MOCK_METHOD2(ReadDone, void(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers));

    void ReadUntilNotOkOrEndOfStream(DemuxerStream::Type type, DemuxerStream::Status* status, base::TimeDelta* last_timestamp)
    {
    ...
    void ExpectEndOfStream(DemuxerStream::Type type)
    {
        ...
        GetStream(type)->Read(1, base::BindOnce(&ChunkDemuxerTest::ReadDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }

    void ExpectRead(DemuxerStream::Type type, int64_t timestamp_in_ms)
    {
        EXPECT_CALL(*this, ReadDone(DemuxerStream::kOk, ReadOneAndHasTimestamp(timestamp_in_ms)));
        GetStream(type)->Read(1, base::BindOnce(&ChunkDemuxerTest::ReadDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void CheckExpectedBuffers(DemuxerStream* stream, const std::string& expected)
    {
        std::vector<std::string> timestamps = base::SplitString(expected, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        std::stringstream ss;
        for (size_t i = 0; i < timestamps.size(); ++i) {
    ...
    bool ParseWebMFile(const std::string& filename, const BufferTimestamps* timestamps, const base::TimeDelta& duration)
    {
        return ParseWebMFile(filename, timestamps, duration, HAS_AUDIO | HAS_VIDEO);
    }

    bool ParseWebMFile(const std::string& filename, const BufferTimestamps* timestamps, const base::TimeDelta& duration, int stream_flags)
    {
        EXPECT_CALL(*this, DemuxerOpened());
        ...
        EXPECT_TRUE(AppendDataInPieces(base::make_span(buffer->data(), buffer->size()), 512));

        // Verify that the timestamps on the first few packets match what we
        // expect.
        for (size_t i = 0; (timestamps[i].audio_time_ms != kSkip || timestamps[i].video_time_ms != kSkip); i++) {
            bool audio_read_done = false;
            bool video_read_done = false;

            if (timestamps[i].audio_time_ms != kSkip) {
                ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(timestamps[i].audio_time_ms), kAudioBlockDuration, 1, &audio_read_done));
                EXPECT_TRUE(audio_read_done);
            }

            if (timestamps[i].video_time_ms != kSkip) {
                ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(timestamps[i].video_time_ms), kVideoBlockDuration, 1, &video_read_done));
    ...
    MOCK_METHOD2(OnEncryptedMediaInitData, void(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data));
    ...
    bool SetTimestampOffset(const std::string& id, base::TimeDelta timestamp_offset)
    {
...
TEST_F(ChunkDemuxerTest, Init)
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (has_audio || has_video) {
            ASSERT_TRUE(InitDemuxerWithEncryptionInfo(stream_flags, is_audio_encrypted, is_video_encrypted));
        } else {
            ASSERT_FALSE(InitDemuxerWithEncryptionInfo(stream_flags, is_audio_encrypted, is_video_encrypted));
        }
        ...
        if (has_audio) {
            ...
            EXPECT_EQ(is_audio_encrypted, audio_stream->audio_decoder_config().is_encrypted());
        } else {
        ...
        if (has_video) {
            ...
            EXPECT_EQ(is_video_encrypted, video_stream->video_decoder_config().is_encrypted());
        } else {
...
TEST_F(ChunkDemuxerTest, AudioVideoTrackIdsChange)
{
    ...
    CreateNewDemuxer();
    ASSERT_TRUE(InitDemuxerWithEncryptionInfo(HAS_AUDIO | HAS_VIDEO, false, false));
    DemuxerStream* audio_stream = GetStream(DemuxerStream::AUDIO);
    DemuxerStream* video_stream = GetStream(DemuxerStream::VIDEO);
    ASSERT_TRUE(audio_stream);
    ASSERT_TRUE(video_stream);

    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 23K", 23), MuxedStreamInfo(kVideoTrackNum, "0K 30", 30));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAlternateAudioTrackNum, "46K 69K", 63), MuxedStreamInfo(kAlternateVideoTrackNum, "60K", 23));
...
TEST_F(ChunkDemuxerTest, InitSegmentSetsNeedRandomAccessPointFlag)
{
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "23K", WebMClusterParser::kDefaultAudioBufferDurationInMs), MuxedStreamInfo(kVideoTrackNum, "0 30K", 30));
    CheckExpectedRanges("{ [23,46) }");

    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    ASSERT_TRUE(AppendInitSegment(HAS_AUDIO | HAS_VIDEO));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "46K 69K", 23), MuxedStreamInfo(kVideoTrackNum, "60 90K", 30));
...
{
    EXPECT_CALL(*this, DemuxerOpened());
    demuxer_->Initialize(&host_, base::BindOnce(&ChunkDemuxerTest::DemuxerInitialized, base::Unretained(this)));
...
TEST_F(ChunkDemuxerTest, Shutdown_EndOfStreamWhileWaitingForData)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    audio_stream->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &audio_read_done));
    video_stream->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &video_read_done));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, ErrorWhileParsingClusterAfterInit)
{
    ...
    EXPECT_MEDIA_LOG(StreamParsingFailed());
    EXPECT_CALL(host_, OnDemuxerError(HasStatusCode(CHUNK_DEMUXER_ERROR_APPEND_FAILED)));
    AppendGarbage();
}
...
TEST_F(ChunkDemuxerTest, SeekWhileParsingCluster)
{
    ...
    ASSERT_TRUE(AppendData(base::make_span(cluster_a->data() + first_append_size, second_append_size)));
...
TEST_F(ChunkDemuxerTest, AppendToParseBufferBeforeInit)
{
    ...
    ASSERT_EQ(StreamParser::ParseStatus::kFailed,
        demuxer_->RunSegmentParserLoop(kSourceId, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]));
}
...
TEST_F(ChunkDemuxerTest, ReadMultiBuffer_kOK)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    ReadAudio(3, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kAudioBlockDuration, 2, &audio_read_done));
    ReadVideo(2, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kVideoBlockDuration, 2, &video_read_done));
...
TEST_F(ChunkDemuxerTest, ReadMultiBuffer_ActualReadCount_LE_RequestedCount)
{
    ...
    auido_stream->Read(100, base::BindOnce(StoreStatusAndBuffers, &audio_status, &audio_buffers));
    DemuxerStream* video_stream = GetStream(DemuxerStream::Type::VIDEO);
    video_stream->Read(100, base::BindOnce(StoreStatusAndBuffers, &vedio_status, &video_buffers));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, ReadMultiBuffer_LastBufferIsEOS)
{
    ...
    video_stream->Read(100, base::BindOnce(OnReadDone_LastBufferEOSExpected, &audio_read_done));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, ReadOneBuffer)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kAudioBlockDuration, 1, &audio_read_done));
    ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kVideoBlockDuration, 1, &video_read_done));
...
TEST_F(ChunkDemuxerTest, OutOfOrderClusters)
{
    ...
    ASSERT_TRUE(AppendData(base::make_span(cluster_c->data(), static_cast<size_t>(cluster_c->bytes_used()))));
...
TEST_F(ChunkDemuxerTest, NonMonotonicButAboveClusterTimecode)
{
    ...
    ASSERT_FALSE(AppendData(base::make_span(cluster_b->data(), static_cast<size_t>(cluster_b->bytes_used()))));
}
...
TEST_F(ChunkDemuxerTest, BeforeClusterTimecode)
{
    ...
    cb.AddBlockGroup(kAudioTrackNum, 92, kAudioBlockDuration, kWebMFlagKeyframe, true, data, sizeof(data));
    cb.AddBlockGroup(kVideoTrackNum, 99, kVideoBlockDuration, 0, false, data, sizeof(data));
...
{
    EXPECT_CALL(*this, DemuxerOpened());
    demuxer_->Initialize(&host_, NewExpectedStatusCB(CHUNK_DEMUXER_ERROR_APPEND_FAILED));
...
{
    EXPECT_CALL(*this, DemuxerOpened());
    demuxer_->Initialize(&host_, NewExpectedStatusCB(DEMUXER_ERROR_COULD_NOT_OPEN));
...
TEST_F(ChunkDemuxerTest, DecodeErrorEndOfStream)
{
    ...
    EXPECT_CALL(host_, OnDemuxerError(HasStatusCode(CHUNK_DEMUXER_ERROR_EOS_STATUS_DECODE_ERROR)));
...
TEST_F(ChunkDemuxerTest, NetworkErrorEndOfStream)
{
    ...
    EXPECT_CALL(host_, OnDemuxerError(HasStatusCode(CHUNK_DEMUXER_ERROR_EOS_STATUS_NETWORK_ERROR)));
...
public:
    ...
    void RequestReads()
    {
        ...
        audio_stream_->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &audio_read_done_));
        video_stream_->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &video_read_done_));
        base::RunLoop().RunUntilIdle();
    }
...
TEST_F(ChunkDemuxerTest, EndOfStreamWithPendingReads)
{
    ...
    ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kAudioBlockDuration, 1, &audio_read_done_1));
    ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(0), kVideoBlockDuration, 1, &video_read_done_1));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, EndOfStreamRangeChanges)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kVideoTrackNum, "0K 33", 33), MuxedStreamInfo(kAudioTrackNum, "0K 23K", 23));
...
TEST_F(ChunkDemuxerTest, AppendingInPieces)
{
    ...
    size_t buffer_size = info_tracks.size() + cluster_a->bytes_used() + cluster_b->bytes_used();
...
TEST_F(ChunkDemuxerTest, WebMFile_AudioAndVideo)
{
    ...
    ASSERT_TRUE(ParseWebMFile("bear-320x240.webm", buffer_timestamps, base::Milliseconds(2744)));
...
TEST_F(ChunkDemuxerTest, WebMFile_LiveAudioAndVideo)
{
    ...
    ExpectInitMediaLogs(HAS_AUDIO | HAS_VIDEO);
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(2)).Times(7);
    ASSERT_TRUE(ParseWebMFile("bear-320x240-live.webm", buffer_timestamps, kInfiniteDuration));
...
TEST_F(ChunkDemuxerTest, WebMFile_AudioOnly)
{
    ...
    ASSERT_TRUE(ParseWebMFile("bear-320x240-audio-only.webm", buffer_timestamps, base::Milliseconds(2744), HAS_AUDIO));
...
TEST_F(ChunkDemuxerTest, WebMFile_VideoOnly)
{
    ...
    ASSERT_TRUE(ParseWebMFile("bear-320x240-video-only.webm", buffer_timestamps, base::Milliseconds(2703), HAS_VIDEO));
...
TEST_F(ChunkDemuxerTest, WebMFile_AltRefFrames)
{
    ...
    ExpectInitMediaLogs(HAS_AUDIO | HAS_VIDEO);
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(2));
    ASSERT_TRUE(ParseWebMFile("bear-320x240-altref.webm", buffer_timestamps, base::Milliseconds(2767)));
}
...
TEST_F(ChunkDemuxerTest, IncrementalClusterParsing)
{
    ...
    int i = 0;
    for (; i < cluster->bytes_used() && !(audio_read_done || video_read_done); ++i) {
    ...
    ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(23), kAudioBlockDuration, 1, &audio_read_done));
    ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(33), kVideoBlockDuration, 1, &video_read_done));
    ...
    ASSERT_TRUE(AppendData(base::make_span(cluster->data() + i, static_cast<size_t>(cluster->bytes_used()) - i)));
...
{
    EXPECT_CALL(*this, DemuxerOpened());
    demuxer_->Initialize(&host_, CreateInitDoneCallback(kNoTimestamp, CHUNK_DEMUXER_ERROR_APPEND_FAILED));
...
TEST_F(ChunkDemuxerTest, AddSeparateSourcesForAudioAndVideo)
{
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(0, 92, kAudioTrackNum, kAudioBlockDuration)));
    GenerateAudioStreamExpectedReads(0, 4);
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(0, 132, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, SeekAudioAndVideoSources)
{
    ...
    audio_read_done = false;
    video_read_done = false;
    ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Seconds(3), kAudioBlockDuration, 1, &audio_read_done));
    ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Seconds(3), kVideoBlockDuration, 1, &video_read_done));
    // Read()s should not return until after data is appended at the Seek point.
    EXPECT_FALSE(audio_read_done);
    EXPECT_FALSE(video_read_done);

    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(3000, 3092, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(3000, 3132, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, EndOfStreamAfterPastEosSeek)
{
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "0K 10K 20K 30K 40K 50K 60K 70K 80K 90K 100K 110K", 10), MuxedStreamInfo(kVideoTrackNum, "0K 20K 40K 60K 80K", 20));
    ...
    demuxer_->StartWaitingForSeek(seek_time);
    demuxer_->Seek(seek_time, base::BindOnce(OnSeekDone_OKExpected, &seek_cb_was_called));
    base::RunLoop().RunUntilIdle();
...
TEST_F(ChunkDemuxerTest, EndOfStreamDuringPendingSeek)
{
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "0K 10K 20K 30K 40K 50K 60K 70K 80K 90K 100K 110K", 10), MuxedStreamInfo(kVideoTrackNum, "0K 20K 40K 60K 80K", 20));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "200K 210K 220K 230K 240K 250K 260K 270K 280K 290K", 10),
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "140K 150K 160K 170K", 10), MuxedStreamInfo(kVideoTrackNum, "140K 145K 150K 155K 160K 165K 170K 175K", 20));
...
TEST_F(ChunkDemuxerTest, GetBufferedRanges_AudioIdOnly)
{
    ...
    ASSERT_TRUE(AppendCluster(GenerateSingleStreamCluster(0, 92, kAudioTrackNum, kAudioBlockDuration)));

    CheckExpectedRanges("{ [0,92) }");

    // Append a disjoint cluster to check for two separate ranges.
    ASSERT_TRUE(AppendCluster(GenerateSingleStreamCluster(150, 219, kAudioTrackNum, kAudioBlockDuration)));
...
TEST_F(ChunkDemuxerTest, GetBufferedRanges_VideoIdOnly)
{
    ...
    // Test a simple cluster.
    ASSERT_TRUE(AppendCluster(GenerateSingleStreamCluster(0, 132, kVideoTrackNum, kVideoBlockDuration)));

    CheckExpectedRanges("{ [0,132) }");

    // Append a disjoint cluster to check for two separate ranges.
    ASSERT_TRUE(AppendCluster(GenerateSingleStreamCluster(200, 299, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, GetBufferedRanges_SeparateStreams)
{
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(0, 23, kAudioTrackNum, 23)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(0, 33, kVideoTrackNum, 33)));
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(300, 400, kAudioTrackNum, 50)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(320, 420, kVideoTrackNum, 50)));
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(620, 690, kAudioTrackNum, 70)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(600, 670, kVideoTrackNum, 70)));
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(920, 950, kAudioTrackNum, 30)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(900, 970, kVideoTrackNum, 70)));
    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [620,690) [920,950) }");
    CheckExpectedRanges(DemuxerStream::VIDEO, "{ [0,33) [320,420) [600,670) [900,970) }");
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(1200, 1270, kAudioTrackNum, 70)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(1220, 1250, kVideoTrackNum, 30)));
    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [620,690) [920,950) [1200,1270) }");
    CheckExpectedRanges(DemuxerStream::VIDEO, "{ [0,33) [320,420) [600,670) [900,970) [1220,1250) }");
    CheckExpectedRangesForMediaSource("{ [0,23) [320,400) [620,670) [920,950) [1220,1250) }");
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(1230, 1250, kAudioTrackNum, 20)));
    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [620,690) [920,950) [1200,1250) }");
    ...
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(1230, 1250, kVideoTrackNum, 20)));
    CheckExpectedRanges(DemuxerStream::VIDEO, "{ [0,33) [320,420) [600,670) [900,970) [1220,1250) }");
...
TEST_F(ChunkDemuxerTest, GetBufferedRanges_AudioVideo)
{
    ...
    // Audio block duration is smaller than video block duration,
    // so the buffered ranges should correspond to the audio blocks.
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0D23K"), MuxedStreamInfo(kVideoTrackNum, "0D33K"));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "300K 350D50K"), MuxedStreamInfo(kVideoTrackNum, "320K 370D50K"));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "620D70K"), MuxedStreamInfo(kVideoTrackNum, "600D70K"));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "920D30K"), MuxedStreamInfo(kVideoTrackNum, "900D70K"));

    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [600,690) [900,950) }");
    CheckExpectedRanges(DemuxerStream::VIDEO, "{ [0,33) [300,420) [600,670) [900,970) }");
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "1200D70K"), MuxedStreamInfo(kVideoTrackNum, "1220D30K"));

    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [600,690) [900,950) [1200,1270) }");
    CheckExpectedRanges(DemuxerStream::VIDEO, "{ [0,33) [300,420) [600,670) [900,970) [1200,1250) }");
    CheckExpectedRanges("{ [0,23) [300,400) [600,670) [900,950) [1200,1250) }");

    // Appending within existing buffered range.
    EXPECT_MEDIA_LOG(TrimmedSpliceOverlap(1230000, 1200000, 40000));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "1230D20K"), MuxedStreamInfo(kVideoTrackNum, "1230D20K"));
    ...
    // - existing frame trimmed from [1200, 1270) to [1200, 1230),
    // - newly appended audio from [1230, 1250).
    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [0,23) [300,400) [600,690) [900,950) [1200,1250) }");
...
TEST_F(ChunkDemuxerTest, GetBufferedRanges_EndOfStream)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 23K", 23), MuxedStreamInfo(kVideoTrackNum, "0K 33", 33));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "200K 223K", 23), MuxedStreamInfo(kVideoTrackNum, "200K 233 266 299 332K 365", 33));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "200K 223K", 23), MuxedStreamInfo(kVideoTrackNum, "200K 233", 33));
...
TEST_F(ChunkDemuxerTest, DifferentStreamTimecodesSeparateSources)
{
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(25, 4 * kAudioBlockDuration + 25, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(30, 4 * kVideoBlockDuration + 30, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, DifferentStreamTimecodesOutOfRange)
{
    ...
    // stream and append them.
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(0, 4 * kAudioBlockDuration + 0, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(10000, 4 * kVideoBlockDuration + 10000, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, CodecIDsThatAreNotRFC6381Compliant)
{
    ...
    const char* codec_ids[] = { // GPAC places leading zeros on the audio object type.
...
TEST_F(ChunkDemuxerTest, EndOfStreamDuringSeek)
{
    ...
    ASSERT_TRUE(AppendCluster(kDefaultSecondCluster()));
    EXPECT_CALL(host_, SetDuration(base::Milliseconds(kDefaultSecondClusterEndTimestamp)));
    MarkEndOfStream(PIPELINE_OK);
...
TEST_F(ChunkDemuxerTest, TimestampOffsetSeparateStreams)
{
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(2500, 2500 + kAudioBlockDuration * 4, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(2500, 2500 + kVideoBlockDuration * 4, kVideoTrackNum, kVideoBlockDuration)));
    GenerateAudioStreamExpectedReads(0, 4);
    GenerateVideoStreamExpectedReads(0, 4);
    ...
    ASSERT_TRUE(AppendCluster(audio_id, GenerateSingleStreamCluster(0, kAudioBlockDuration * 4, kAudioTrackNum, kAudioBlockDuration)));
    ASSERT_TRUE(AppendCluster(video_id, GenerateSingleStreamCluster(0, kVideoBlockDuration * 4, kVideoTrackNum, kVideoBlockDuration)));
...
TEST_F(ChunkDemuxerTest, IsParsingMediaSegmentMidMediaSegment)
{
    ...
    ASSERT_TRUE(AppendData(base::make_span(cluster->data(), cluster->bytes_used() - 13u)));

    // Confirm we're in the middle of parsing a media segment.
    ASSERT_TRUE(demuxer_->IsParsingMediaSegment(kSourceId));

    demuxer_->ResetParserState(kSourceId, append_window_start_for_next_append_, append_window_end_for_next_append_, &timestamp_offset_map_[kSourceId]);
...
TEST_F(ChunkDemuxerTest, EmitBuffersDuringAbort)
{
    ...
    demuxer_->Initialize(&host_, CreateInitDoneCallback(kInfiniteDuration, PIPELINE_OK));
    ...
    while (appended_bytes < buffer->size()) {
        size_t cur_chunk_size = std::min(chunk_size, buffer->size() - appended_bytes);
        ASSERT_TRUE(AppendData(kSourceId, base::make_span(buffer->data() + appended_bytes, cur_chunk_size)));
...
TEST_F(ChunkDemuxerTest, WebMIsParsingMediaSegmentDetection)
{
    ...
    static_assert(std::size(kBuffer) == std::size(kExpectedReturnValues), "test arrays out of sync");
    static_assert(std::size(kBuffer) == sizeof(kBuffer), "there should be one byte per index");
    ...
    for (size_t i = 0; i < sizeof(kBuffer); i++) {
        ...
        EXPECT_EQ(expected_return_value, demuxer_->IsParsingMediaSegment(kSourceId));
    }
}
...
TEST_F(ChunkDemuxerTest, DurationChange)
{
    ...
    ASSERT_TRUE(AppendCluster(GenerateCluster(kStreamDuration - kAudioBlockDuration, kStreamDuration - kVideoBlockDuration, 2)));
    ...
    ASSERT_TRUE(AppendCluster(GenerateCluster(kDefaultDuration().InMilliseconds(), 2)));
    ...
    ASSERT_TRUE(AppendCluster(GenerateCluster(kStreamDuration + kAudioBlockDuration, kStreamDuration + kVideoBlockDuration, 3)));
...
TEST_F(ChunkDemuxerTest, DurationChangeTimestampOffset)
{
    ...
    EXPECT_CALL(host_, SetDuration(kDefaultDuration() + base::Milliseconds(kVideoBlockDuration * 2)));
    ASSERT_TRUE(AppendCluster(GenerateCluster(0, 4)));
}
...
TEST_F(ChunkDemuxerTest, EndOfStreamTruncateDuration)
{
    ...
    EXPECT_CALL(host_, SetDuration(base::Milliseconds(kDefaultFirstClusterEndTimestamp)));
    MarkEndOfStream(PIPELINE_OK);
}
...
{
    demuxer_->Shutdown();
    demuxer_->Initialize(&host_, CreateInitDoneCallback(DEMUXER_ERROR_COULD_NOT_OPEN));
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(ChunkDemuxerTest, EndOfStreamWhileWaitingForGapToBeFilled)
{
    ...
    bool audio_read_done = false;
    bool video_read_done = false;
    ReadAudio(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(138), kAudioBlockDuration, 1, &audio_read_done));
    ReadVideo(1, base::BindOnce(&OnReadDone_Ok, base::Milliseconds(138), kVideoBlockDuration, 1, &video_read_done));
    ...
    audio_read_done = false;
    video_read_done = false;
    ReadAudio(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &audio_read_done));
    ReadVideo(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &video_read_done));
...
TEST_F(ChunkDemuxerTest, SetMemoryLimitType)
{
    ...
    demuxer_->SetMemoryLimitsForTest(DemuxerStream::AUDIO, GetExpectedMemoryUsage(10, 10 * block_size_));
    demuxer_->SetMemoryLimitsForTest(DemuxerStream::VIDEO, GetExpectedMemoryUsage(5, 5 * block_size_) + 1);

    base::TimeDelta seek_time = base::Milliseconds(1000);

    // Append data at the start that can be garbage collected:
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "0K 23K 46K 69K 92K 115K 138K 161K 184K 207K", 23), MuxedStreamInfo(kVideoTrackNum, "0K 33K 66K 99K 132K", 33));
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "1000K 1023K 1046K 1069K 1092K 1115K 1138K 1161K 1184K 1207K", 23),
...
TEST_F(ChunkDemuxerTest, GCDuringSeek_SingleRange_SeekForward)
{
    ...
    Seek(seek_time);
    EXPECT_TRUE(demuxer_->EvictCodedFrames(kSourceId, seek_time, GetExpectedMemoryUsage(5, 5 * block_size_)));
...
TEST_F(ChunkDemuxerTest, GCDuringSeek_MultipleRanges_SeekForward)
{
    ...
    Seek(seek_time);
    EXPECT_TRUE(demuxer_->EvictCodedFrames(kSourceId, seek_time, GetExpectedMemoryUsage(8, 8 * block_size_)));
...
TEST_F(ChunkDemuxerTest, GCDuringSeek)
{
    ...
    demuxer_->SetMemoryLimitsForTest(DemuxerStream::AUDIO, GetExpectedMemoryUsage(5, 5 * block_size_));
    ...
    AppendSingleStreamCluster(kSourceId, kAudioTrackNum, seek_time1.InMilliseconds(), 5);
    ...
    AppendSingleStreamCluster(kSourceId, kAudioTrackNum, seek_time2.InMilliseconds(), 5);
...
TEST_F(ChunkDemuxerTest, AppendWindow_Video)
{
    ...
    AppendSingleStreamCluster(kSourceId, kVideoTrackNum, "0K 30 60 90 120K 150 180 210 240K 270 300 330K");
    ...
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(30));
    AppendSingleStreamCluster(kSourceId, kVideoTrackNum, "360 390 420K 450 480 510 540K 570 600 630K");
...
TEST_F(ChunkDemuxerTest, AppendWindow_Audio)
{
    ...
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(30));
    AppendSingleStreamCluster(kSourceId, kAudioTrackNum, "0K 30K 60K 90K 120K 150K 180K 210K 240K 270K 300K 330K");
    ...
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(30));
    AppendSingleStreamCluster(kSourceId, kAudioTrackNum, "360K 390K 420K 450K 480K 510K 540K 570K 600K 630K");
...
TEST_F(ChunkDemuxerTest, AppendWindow_AudioOverlapStartAndEnd)
{
    ...
    EXPECT_MEDIA_LOG(TruncatedFrame(0, kAudioBlockDuration * 1000, "start", 10000));
    EXPECT_MEDIA_LOG(TruncatedFrame(10000, kAudioBlockDuration * 1000, "end", 20000));

    // Append a cluster that starts before and ends after the append window.
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(WebMClusterParser::kDefaultAudioBufferDurationInMs));
...
TEST_F(ChunkDemuxerTest, AppendWindow_WebMFile_AudioOnly)
{
    ...
    EXPECT_MEDIA_LOG(
        DroppedFrameCheckAppendWindow("audio", append_window_start_for_next_append_.InMicroseconds(), append_window_end_for_next_append_.InMicroseconds()))
        .Times(AtLeast(1));
    ...
    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(2));
    ASSERT_TRUE(AppendDataInPieces(base::make_span(buffer->data(), buffer->size()), 128));
...
TEST_F(ChunkDemuxerTest, AppendWindow_AudioConfigUpdateRemovesPreroll)
{
    ...
    EXPECT_CALL(*this, InitSegmentReceivedMock(_));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(2));
    ASSERT_TRUE(AppendDataInPieces(base::make_span(buffer->data(), buffer->size()), 512));
    CheckExpectedRanges("{ }");
    ...
    ASSERT_TRUE(AppendDataInPieces(base::make_span(buffer2->data(), buffer2->size()), 512));
...
TEST_F(ChunkDemuxerTest, Remove_AudioVideoText)
{
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "0K 20K 40K 60K 80K 100K 120K 140K", 20), MuxedStreamInfo(kVideoTrackNum, "0K 30 60 90 120K 150 180", 30));
    ...
    AppendMuxedCluster(
        MuxedStreamInfo(kAudioTrackNum, "1K 21K 41K 61K 81K 101K 121K 141K", 20), MuxedStreamInfo(kVideoTrackNum, "1K 31 61 91 121K 151 181", 30));
...
TEST_F(ChunkDemuxerTest, Remove_StartAtDuration)
{
    ...
    AppendSingleStreamCluster(kSourceId, kAudioTrackNum, "0K 20K 40K 60K 80K 100K 120K 140D20K");

    CheckExpectedRanges("{ [0,160) }");
    CheckExpectedBuffers(audio_stream, "0K 20K 40K 60K 80K 100K 120K 140K");

    demuxer_->Remove(kSourceId, base::Seconds(demuxer_->GetDuration()), kInfiniteDuration);
...
TEST_F(ChunkDemuxerTest, SeekCompletesWithoutTextCues)
{
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 20K 40K 60K 80K 100K 120K 140K 160K 180K 200K", 20),
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "220K 240K 260K 280K", 20), MuxedStreamInfo(kVideoTrackNum, "240K 270 300 330", 30));
...
TEST_F(ChunkDemuxerTest, CuesBetweenClustersWithUnknownSize)
{
    ...
    std::vector<uint8_t> data(cluster->data(), cluster->data() + cluster->bytes_used());
    data.insert(data.end(), kCuesHeader, kCuesHeader + sizeof(kCuesHeader));
    cluster = GenerateCluster(46, 66, 5, true);
    data.insert(data.end(), cluster->data(), cluster->data() + cluster->bytes_used());
...
TEST_F(ChunkDemuxerTest, EvictCodedFramesTest)
{
    ASSERT_TRUE(InitDemuxer(HAS_AUDIO | HAS_VIDEO));
    demuxer_->SetMemoryLimitsForTest(DemuxerStream::AUDIO, GetExpectedMemoryUsage(10, 10 * block_size_));
    demuxer_->SetMemoryLimitsForTest(DemuxerStream::VIDEO, GetExpectedMemoryUsage(15, 15 * block_size_));
    DemuxerStream* audio_stream = GetStream(DemuxerStream::AUDIO);
    DemuxerStream* video_stream = GetStream(DemuxerStream::VIDEO);
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, kAudioStreamInfo, 40), MuxedStreamInfo(kVideoTrackNum, kVideoStreamInfo, 10));
    ...
    ASSERT_FALSE(demuxer_->EvictCodedFrames(kSourceId, base::Milliseconds(0), GetExpectedMemoryUsage(8, 8 * block_size_)));
    ...
    ASSERT_TRUE(demuxer_->EvictCodedFrames(kSourceId, base::Milliseconds(120), GetExpectedMemoryUsage(8, 8 * block_size_)));
...
TEST_F(ChunkDemuxerTest, RelaxedKeyframe_RemoveInterruptsMuxedCodedFrameGroup_1)
{
    // Append muxed:
    //        A:[K K K]
    //        V:[K n n]
    ...
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 10K 20D10K"), MuxedStreamInfo(kVideoTrackNum, "0K 10 20", 10));
    demuxer_->Remove(kSourceId, base::TimeDelta(), base::Milliseconds(30));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "30K 40K 50D10K"), MuxedStreamInfo(kVideoTrackNum, "30 40 50", 10));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "60K 70K 80D10K"), MuxedStreamInfo(kVideoTrackNum, "60 70K 80", 10));
...
TEST_F(ChunkDemuxerTest, RelaxedKeyframe_RemoveInterruptsMuxedCodedFrameGroup_2)
{
    // Append muxed:
    //        A:[K K K]
    ...
    EXPECT_MEDIA_LOG(SegmentMissingFrames("1"));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0K 10K 20D10K"), MuxedStreamInfo(kVideoTrackNum, ""));
    demuxer_->Remove(kSourceId, base::TimeDelta(), base::Milliseconds(30));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "30K 40K 50D10K"), MuxedStreamInfo(kVideoTrackNum, "30 40 50", 10));
    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "60K 70K 80D10K"), MuxedStreamInfo(kVideoTrackNum, "60 70K 80", 10));
    CheckExpectedRanges(DemuxerStream::AUDIO, "{ [30,90) }");
...
TEST_F(ChunkDemuxerTest, RelaxedKeyframe_RemoveInterruptsMuxedCodedFrameGroup_3)
{
    // Append muxed:
    //        A:[K K K
    ...
    ASSERT_TRUE(AppendData(kSourceId, base::make_span(cluster->data(), static_cast<size_t>(video_start))));
    ...
    ASSERT_TRUE(AppendData(kSourceId, base::make_span(cluster->data() + video_start, static_cast<size_t>(cluster->bytes_used()) - video_start)));
...
namespace {
void QuitLoop(base::OnceClosure quit_closure, const std::vector<DemuxerStream*>& streams)
{
    std::move(quit_closure).Run();
}

void DisableAndEnableDemuxerTracks(ChunkDemuxer* demuxer, base::test::TaskEnvironment* task_environment)
{
    ...
    demuxer->OnSelectedVideoTrackChanged(video_tracks, base::TimeDelta(), base::BindOnce(QuitLoop, disable_video.QuitClosure()));
    disable_video.Run();

    base::RunLoop disable_audio;
    demuxer->OnEnabledAudioTracksChanged(audio_tracks, base::TimeDelta(), base::BindOnce(QuitLoop, disable_audio.QuitClosure()));
    disable_audio.Run();

    base::RunLoop enable_video;
    video_tracks.push_back(MediaTrack::Id("1"));
    demuxer->OnSelectedVideoTrackChanged(video_tracks, base::TimeDelta(), base::BindOnce(QuitLoop, enable_video.QuitClosure()));
    enable_video.Run();

    base::RunLoop enable_audio;
    audio_tracks.push_back(MediaTrack::Id("2"));
    demuxer->OnEnabledAudioTracksChanged(audio_tracks, base::TimeDelta(), base::BindOnce(QuitLoop, enable_audio.QuitClosure()));
    enable_audio.Run();
...
{
    ASSERT_TRUE(InitDemuxer(HAS_AUDIO | HAS_VIDEO));
    ChunkDemuxerStream* audio_stream = static_cast<ChunkDemuxerStream*>(GetStream(DemuxerStream::AUDIO));
    EXPECT_NE(nullptr, audio_stream);
    ChunkDemuxerStream* video_stream = static_cast<ChunkDemuxerStream*>(GetStream(DemuxerStream::VIDEO));
    ...
    audio_stream->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &read_done));
    DisableAndEnableDemuxerTracks(demuxer_.get(), &task_environment_);
    EXPECT_TRUE(read_done);
    read_done = false;
    video_stream->Read(1, base::BindOnce(&OnReadDone_LastBufferEOSExpected, &read_done));
    DisableAndEnableDemuxerTracks(demuxer_.get(), &task_environment_);
    EXPECT_TRUE(read_done);
}
...
TEST_F(ChunkDemuxerTest, MultipleIds)
{
    ...
    demuxer_->Initialize(&host_, CreateInitDoneCallback(kNoTimestamp, PIPELINE_OK));
...
TEST_F(ChunkDemuxerTest, SequenceModeMuxedAppendShouldWarn)
{
    ...
    EXPECT_CALL(*this, OnParseWarningMock(SourceBufferParseWarning::kMuxedSequenceMode));
    EXPECT_MEDIA_LOG(MuxedSequenceModeWarning());

    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "0D10K"), MuxedStreamInfo(kVideoTrackNum, "0D10K"));
}
...
TEST_F(ChunkDemuxerTest, SequenceModeSingleTrackNoWarning)
{
    ...
    EXPECT_CALL(*this, OnParseWarningMock(SourceBufferParseWarning::kMuxedSequenceMode)).Times(0);
...
TEST_F(ChunkDemuxerTest, GetLowestAndHighestPresentationTimestamps_NonMuxed)
{
    ...
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetLowestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetHighestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetLowestPresentationTimestamp(video_id));
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetHighestPresentationTimestamp(video_id));
    ...
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetLowestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::Milliseconds(20), demuxer_->GetHighestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::Milliseconds(10), demuxer_->GetLowestPresentationTimestamp(video_id));
    EXPECT_EQ(base::Milliseconds(30), demuxer_->GetHighestPresentationTimestamp(video_id));
    ...
    EXPECT_EQ(base::Milliseconds(10), demuxer_->GetLowestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::Milliseconds(10), demuxer_->GetHighestPresentationTimestamp(audio_id));
    EXPECT_EQ(base::Milliseconds(20), demuxer_->GetLowestPresentationTimestamp(video_id));
    EXPECT_EQ(base::Milliseconds(20), demuxer_->GetHighestPresentationTimestamp(video_id));
...
TEST_F(ChunkDemuxerTest, GetLowestAndHighestPresentationTimestamps_Muxed)
{
    ...
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetLowestPresentationTimestamp(kSourceId));
    EXPECT_EQ(base::TimeDelta(), demuxer_->GetHighestPresentationTimestamp(kSourceId));

    AppendMuxedCluster(MuxedStreamInfo(kAudioTrackNum, "10K 33K 56K", 23), MuxedStreamInfo(kVideoTrackNum, "20K 50K 80K", 30));
    EXPECT_EQ(base::Milliseconds(10), demuxer_->GetLowestPresentationTimestamp(kSourceId));
    EXPECT_EQ(base::Milliseconds(80), demuxer_->GetHighestPresentationTimestamp(kSourceId));
    ...
    EXPECT_EQ(base::Milliseconds(11), demuxer_->GetLowestPresentationTimestamp(kSourceId));
    EXPECT_EQ(base::Milliseconds(56), demuxer_->GetHighestPresentationTimestamp(kSourceId));
...
{
    struct ZeroLengthFrameCase cases[] = { { DemuxerStream::AUDIO, HAS_AUDIO, kAudioTrackNum }, { DemuxerStream::VIDEO, HAS_VIDEO, kVideoTrackNum } };
...
TEST_F(ChunkDemuxerTest, AddAutoDetectIDFindsCodecs)
{
    ...
    EXPECT_TRUE(AppendData(kPrimary, base::make_span(data->data(), data->size())));
