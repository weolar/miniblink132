...
std::unique_ptr<media::StreamParser> CreateParserForTypeAndCodecs(const std::string& content_type, const std::string& codecs, media::MediaLog* media_log)
{
    std::vector<std::string> parsed_codec_ids;
    media::SplitCodecs(codecs, &parsed_codec_ids);
    return media::StreamParserFactory::Create(content_type, parsed_codec_ids, media_log);
}
...
std::string ExpectedCodecs(const std::string& content_type, const std::string& codecs)
{
    if (codecs == "" && content_type == "audio/aac")
        return "aac";
    if (codecs == "" && (content_type == "audio/mpeg" || content_type == "audio/mp3"))
        return "mp3";
    return codecs;
}
...
void ChunkDemuxerStream::Seek(base::TimeDelta time)
{
    ...
    base::AutoLock auto_lock(lock_);
    DCHECK(!read_cb_);
    DCHECK(state_ == UNINITIALIZED || state_ == RETURNING_ABORT_FOR_READS) << state_;
...
void ChunkDemuxerStream::Remove(base::TimeDelta start, base::TimeDelta end, base::TimeDelta duration)
{
    base::AutoLock auto_lock(lock_);
    stream_->Remove(start, end, duration);
}

bool ChunkDemuxerStream::EvictCodedFrames(base::TimeDelta media_time, size_t newDataSize)
{
    base::AutoLock auto_lock(lock_);
    ...
    base::TimeDelta media_time, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
    ...
    base::AutoLock auto_lock(lock_);
    return stream_->OnMemoryPressure(media_time, memory_pressure_level, force_instant_gc);
}
...
Ranges<base::TimeDelta> ChunkDemuxerStream::GetBufferedRanges(base::TimeDelta duration) const
{
    base::AutoLock auto_lock(lock_);
...
void ChunkDemuxerStream::OnStartOfCodedFrameGroup(DecodeTimestamp start_dts, base::TimeDelta start_pts)
{
    DVLOG(2) << "ChunkDemuxerStream::OnStartOfCodedFrameGroup(dts " << start_dts.InSecondsF() << ", pts " << start_pts.InSecondsF() << ")";
...
bool ChunkDemuxerStream::UpdateAudioConfig(const AudioDecoderConfig& config, bool allow_codec_change, MediaLog* media_log)
{
...
bool ChunkDemuxerStream::UpdateVideoConfig(const VideoDecoderConfig& config, bool allow_codec_change, MediaLog* media_log)
{
    DCHECK(config.IsValidConfig());
...
std::pair<SourceBufferStreamStatus, DemuxerStream::DecoderBufferVector> ChunkDemuxerStream::GetPendingBuffers_Locked()
{
    lock_.AssertAcquired();
    ...
    for (uint32_t i = 0; i < requested_buffer_count_; ++i) {
        ...
        if (stream_->IsNextBufferConfigChanged() && !output_buffers.empty()) {
            DVLOG(3) << __func__ << " status=0"
                     << ", type=" << type_ << ", req_size=" << requested_buffer_count_ << ", out_size=" << output_buffers.size();
        ...
        case SourceBufferStreamStatus::kNeedBuffer:
            ...
            DVLOG(3) << __func__ << " status=" << (int)status << ", type=" << type_ << ", req_size=" << requested_buffer_count_
    ...
    DCHECK_EQ(output_buffers.size(), static_cast<size_t>(requested_buffer_count_));
...
ChunkDemuxer::ChunkDemuxer(
    base::OnceClosure open_cb, base::RepeatingClosure progress_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb, MediaLog* media_log)
...
void ChunkDemuxer::Initialize(DemuxerHost* host, PipelineStatusCallback init_cb)
{
    DVLOG(1) << "Initialize()";
...
std::optional<container_names::MediaContainerName> ChunkDemuxer::GetContainerForMetrics() const
{
    return std::nullopt;
}
...
ChunkDemuxer::Status ChunkDemuxer::AddId(const std::string& id, std::unique_ptr<AudioDecoderConfig> audio_config)
{
    DCHECK(audio_config);
    DVLOG(1) << __func__ << " id="
    ...
    if ((state_ != WAITING_FOR_INIT && state_ != INITIALIZING) || IsValidId_Locked(id)) {
    ...
    std::unique_ptr<media::StreamParser> stream_parser(media::StreamParserFactory::Create(std::move(audio_config)));
    DCHECK(stream_parser);

    return AddIdInternal(id, std::move(stream_parser), expected_codec);
}

ChunkDemuxer::Status ChunkDemuxer::AddId(const std::string& id, std::unique_ptr<VideoDecoderConfig> video_config)
{
    DCHECK(video_config);
    DVLOG(1) << __func__ << " id="
    ...
    std::unique_ptr<media::StreamParser> stream_parser(media::StreamParserFactory::Create(std::move(video_config)));
    DCHECK(stream_parser);

    return AddIdInternal(id, std::move(stream_parser), expected_codec);
}

ChunkDemuxer::Status ChunkDemuxer::AddId(const std::string& id, const std::string& content_type, const std::string& codecs)
{
    DVLOG(1) << __func__ << " id=" << id << " content_type=" << content_type << " codecs=" << codecs;
    base::AutoLock auto_lock(lock_);
    ...
    std::unique_ptr<media::StreamParser> stream_parser(CreateParserForTypeAndCodecs(content_type, codecs, media_log_));
    if (!stream_parser) {
        DVLOG(1) << __func__ << " failed: unsupported content_type=" << content_type << " codecs=" << codecs;
        return ChunkDemuxer::kNotSupported;
    }

    return AddIdInternal(id, std::move(stream_parser), ExpectedCodecs(content_type, codecs));
}

#if BUILDFLAG(ENABLE_HLS_DEMUXER)
ChunkDemuxer::Status ChunkDemuxer::AddAutoDetectedCodecsId(const std::string& id, RelaxedParserSupportedType mime_type)
{
    DVLOG(1) << __func__ << " id=" << id << " content_type=" << static_cast<int>(mime_type);
    base::AutoLock auto_lock(lock_);
    if ((state_ != WAITING_FOR_INIT && state_ != INITIALIZING) || IsValidId_Locked(id)) {
        return kReachedIdLimit;
    }
    ...
    std::unique_ptr<media::StreamParser> stream_parser = StreamParserFactory::CreateRelaxedParser(mime_type);
    if (!stream_parser) {
...
ChunkDemuxer::Status ChunkDemuxer::AddIdInternal(
    const std::string& id, std::unique_ptr<media::StreamParser> stream_parser, std::optional<std::string_view> expected_codecs)
{
    DVLOG(2) << __func__ << " id=" << id << " expected_codecs=" << expected_codecs.value_or("None");
    lock_.AssertAcquired();

    std::unique_ptr<FrameProcessor> frame_processor
        = std::make_unique<FrameProcessor>(base::BindRepeating(&ChunkDemuxer::IncreaseDurationIfNecessary, base::Unretained(this)), media_log_);

    std::unique_ptr<SourceBufferState> source_state = std::make_unique<SourceBufferState>(
        std::move(stream_parser), std::move(frame_processor), base::BindRepeating(&ChunkDemuxer::CreateDemuxerStream, base::Unretained(this), id), media_log_);
    ...
    source_state->Init(base::BindOnce(&ChunkDemuxer::OnSourceInitDone, base::Unretained(this), id), expected_codecs, encrypted_media_init_data_cb_);
...
void ChunkDemuxer::SetTracksWatcher(const std::string& id, MediaTracksUpdatedCB tracks_updated_cb)
{
    base::AutoLock auto_lock(lock_);
    CHECK(IsValidId_Locked(id));
...
void ChunkDemuxer::SetParseWarningCallback(const std::string& id, SourceBufferParseWarningCB parse_warning_cb)
{
    base::AutoLock auto_lock(lock_);
    CHECK(IsValidId_Locked(id));
...
Ranges<base::TimeDelta> ChunkDemuxer::GetBufferedRanges(const std::string& id) const
{
    base::AutoLock auto_lock(lock_);
    DCHECK(!id.empty());
...
base::TimeDelta ChunkDemuxer::GetLowestPresentationTimestamp(const std::string& id) const
{
    base::AutoLock auto_lock(lock_);
    DCHECK(!id.empty());
...
base::TimeDelta ChunkDemuxer::GetHighestPresentationTimestamp(const std::string& id) const
{
    base::AutoLock auto_lock(lock_);
    DCHECK(!id.empty());
    ...
    const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, DemuxerStream::Type track_type, TrackChangeCB change_completed_cb)
{
    ...
    for (const auto& id : track_ids) {
        ...
        if (!enabled_streams.empty()) {
            MEDIA_LOG(INFO, media_log_) << "Only one enabled track is supported, ignoring track " << id;
            continue;
        }
...
void ChunkDemuxer::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    FindAndEnableProperTracks(track_ids, curr_time, DemuxerStream::AUDIO, std::move(change_completed_cb));
}

void ChunkDemuxer::OnSelectedVideoTrackChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    FindAndEnableProperTracks(track_ids, curr_time, DemuxerStream::VIDEO, std::move(change_completed_cb));
}
...
void ChunkDemuxer::OnMemoryPressure(
    base::TimeDelta currentMediaTime, base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level, bool force_instant_gc)
{
    // TODO(sebmarchand): Check if MEMORY_PRESSURE_LEVEL_MODERATE should also be
    // ignored.
    if (memory_pressure_level == base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_NONE) {
    ...
    for (const auto& [source, state] : source_state_map_) {
        state->OnMemoryPressure(currentMediaTime, memory_pressure_level, force_instant_gc);
    }
}

bool ChunkDemuxer::EvictCodedFrames(const std::string& id, base::TimeDelta currentMediaTime, size_t newDataSize)
{
    DVLOG(1) << __func__ << "(" << id << ")"
             << " media_time=" << currentMediaTime.InSecondsF() << " newDataSize=" << newDataSize;
    base::AutoLock auto_lock(lock_);
...
bool ChunkDemuxer::AppendToParseBuffer(const std::string& id, base::span<const uint8_t> data)
{
...

    {
        ...
        case ENDED:
        case SHUTDOWN:
            DVLOG(1) << "AppendToParseBuffer(): called in unexpected state " << state_;
...
StreamParser::ParseStatus ChunkDemuxer::RunSegmentParserLoop(
    const std::string& id, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
...

    {
        ...
        case INITIALIZED:
            DCHECK(IsValidId_Locked(id));
            result = source_state_map_[id]->RunSegmentParserLoop(append_window_start, append_window_end, timestamp_offset);
        ...
        case ENDED:
        case SHUTDOWN:
            DVLOG(1) << "RunSegmentParserLoop(): called in unexpected state " << state_;
...
bool ChunkDemuxer::AppendChunks(const std::string& id, std::unique_ptr<StreamParser::BufferQueue> buffer_queue, base::TimeDelta append_window_start,
    base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    DCHECK(buffer_queue);
    DVLOG(1) << __func__ << ": " << id << ", buffer_queue size()=" << buffer_queue->size();
...

    {
        ...
        case INITIALIZED:
            DCHECK(IsValidId_Locked(id));
            if (!source_state_map_[id]->AppendChunks(std::move(buffer_queue), append_window_start, append_window_end, timestamp_offset)) {
                ReportError_Locked(CHUNK_DEMUXER_ERROR_APPEND_FAILED);
                return false;
            }
            break;
...
void ChunkDemuxer::ResetParserState(
    const std::string& id, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset)
{
    ...
    CHECK(IsValidId_Locked(id));
    bool old_waiting_for_data = IsSeekWaitingForData_Locked();
    source_state_map_[id]->ResetParserState(append_window_start, append_window_end, timestamp_offset);
...
void ChunkDemuxer::Remove(const std::string& id, base::TimeDelta start, base::TimeDelta end)
{
    DVLOG(1) << "Remove(" << id << ", " << start.InSecondsF() << ", " << end.InSecondsF() << ")";
    base::AutoLock auto_lock(lock_);

    DCHECK(!id.empty());
    CHECK(IsValidId_Locked(id));
    DCHECK(start >= base::TimeDelta()) << start.InSecondsF();
    DCHECK(start < end) << "start " << start.InSecondsF() << " end " << end.InSecondsF();
    DCHECK(duration_ != kNoTimestamp);
    DCHECK(start <= duration_) << "start " << start.InSecondsF() << " duration " << duration_.InSecondsF();
...
bool ChunkDemuxer::CanChangeType(const std::string& id, const std::string& content_type, const std::string& codecs)
{
...
void ChunkDemuxer::ChangeType(const std::string& id, const std::string& content_type, const std::string& codecs)
{
    DVLOG(1) << __func__ << " id=" << id << " content_type=" << content_type << " codecs=" << codecs;
    ...
    DCHECK(stream_parser);
    source_state_map_[id]->ChangeType(std::move(stream_parser), ExpectedCodecs(content_type, codecs));
}
...
void ChunkDemuxer::SetDuration(double duration)
{
    ...
    // delta.
    base::TimeDelta max_duration = base::TimeDelta::FromInternalValue(std::numeric_limits<int64_t>::max() - 1);
    ...
    if (duration == std::numeric_limits<double>::infinity()) {
        ...
        duration_td = base::Microseconds(duration * base::Time::kMicrosecondsPerSecond);
    }
    ...
    for (auto itr = source_state_map_.begin(); itr != source_state_map_.end(); ++itr) {
...
void ChunkDemuxer::SetGroupStartTimestampIfInSequenceMode(const std::string& id, base::TimeDelta timestamp_offset)
{
    base::AutoLock auto_lock(lock_);
    DVLOG(1) << "SetGroupStartTimestampIfInSequenceMode(" << id << ", " << timestamp_offset.InSecondsF() << ")";
    CHECK(IsValidId_Locked(id));
    DCHECK_NE(state_, ENDED);

    source_state_map_[id]->SetGroupStartTimestampIfInSequenceMode(timestamp_offset);
}
...
void ChunkDemuxer::MarkEndOfStream(PipelineStatus status)
{
    ...
    if (state_ == INITIALIZING) {
        MEDIA_LOG(ERROR, media_log_) << "MediaSource endOfStream before demuxer initialization completes "
...
void ChunkDemuxer::UnmarkEndOfStream()
{
    ...
    base::AutoLock auto_lock(lock_);
    DCHECK(state_ == ENDED || state_ == SHUTDOWN || state_ == PARSE_ERROR) << state_;
...
void ChunkDemuxer::SetMemoryLimitsForTest(DemuxerStream::Type type, size_t memory_limit)
{
    for (auto itr = source_state_map_.begin(); itr != source_state_map_.end(); ++itr) {
...
{
    lock_.AssertAcquired();
    DVLOG(1) << "ChunkDemuxer::ChangeState_Locked() : " << state_ << " -> " << new_state;
...
void ChunkDemuxer::OnSourceInitDone(const std::string& source_id, const StreamParser::InitParameters& params)
{
    DVLOG(1) << "OnSourceInitDone source_id=" << source_id << " duration=" << params.duration.InSecondsF();
    lock_.AssertAcquired();
    ...
    CHECK(pending_source_init_ids_.find(source_id) != pending_source_init_ids_.end());
    CHECK(init_cb_);
    ...
    if (!params.timeline_offset.is_null()) {
        if (!timeline_offset_.is_null() && params.timeline_offset != timeline_offset_) {
            MEDIA_LOG(ERROR, media_log_) << "Timeline offset is not the same across all SourceBuffers.";
            ReportError_Locked(DEMUXER_ERROR_COULD_NOT_OPEN);
            return;
        }
...
ChunkDemuxerStream* ChunkDemuxer::CreateDemuxerStream(const std::string& source_id, DemuxerStream::Type type)
{
    ...
    std::unique_ptr<ChunkDemuxerStream> stream = std::make_unique<ChunkDemuxerStream>(type, media_track_id);
...
void ChunkDemuxer::IncreaseDurationIfNecessary(base::TimeDelta new_duration)
{
    ...
    DVLOG(2) << __func__ << ": Increasing duration: " << duration_.InSecondsF() << " -> " << new_duration.InSecondsF();
...
void ChunkDemuxer::DecreaseDurationIfNecessary()
{
    ...
    for (auto itr = source_state_map_.begin(); itr != source_state_map_.end(); ++itr) {
        max_duration = std::max(max_duration, itr->second->GetMaxBufferedDuration());
    }
...
void ChunkDemuxer::RunInitCB_Locked(PipelineStatus status)
{
    lock_.AssertAcquired();
    DCHECK(init_cb_);
    TRACE_EVENT_ASYNC_END1("media", "ChunkDemuxer::Initialize", this, "status", PipelineStatusToString(status));
...
void ChunkDemuxer::RunSeekCB_Locked(PipelineStatus status)
{
    lock_.AssertAcquired();
    DCHECK(seek_cb_);
    TRACE_EVENT_ASYNC_END1("media", "ChunkDemuxer::Seek", this, "status", PipelineStatusToString(status));
