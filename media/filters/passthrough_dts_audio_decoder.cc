...
PassthroughDTSAudioDecoder::PassthroughDTSAudioDecoder(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log)
...
void PassthroughDTSAudioDecoder::Initialize(
    const AudioDecoderConfig& config, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& /* waiting_cb */)
{
    ...
    if (config.target_output_sample_format() != kSampleFormatDts) {
        std::move(bound_init_cb).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedConfig, "PassthroughDTSAudioDecoder does not support codec"));
        return;
    }
...
void PassthroughDTSAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(decode_cb);
...
void PassthroughDTSAudioDecoder::ProcessBuffer(const DecoderBuffer& buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void PassthroughDTSAudioDecoder::EncapsulateFrame(const DecoderBuffer& buffer)
{
    ...
    scoped_refptr<AudioBuffer> output = AudioBuffer::CopyBitstreamFrom(
        kSampleFormatIECDts, CHANNEL_LAYOUT_MONO, 1, config_.samples_per_second(), samples_per_frame, output_channels, dts_frame_size, buffer.timestamp());
