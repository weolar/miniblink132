...
static int GetAudioBufferImpl(struct AVCodecContext* s, AVFrame* frame, int flags)
{
...
FFmpegAudioDecoder::FFmpegAudioDecoder(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log)
    : task_runner_(task_runner)
    , state_(DecoderState::kUninitialized)
    , av_sample_format_(0)
    , media_log_(media_log)
    , pool_(base::MakeRefCounted<AudioBufferMemoryPool>(kFFmpegBufferAddressAlignment))
{
    ...
    const AudioDecoderConfig& config, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& /* waiting_cb */)
{
    ...
    if (config.is_encrypted()) {
        std::move(bound_init_cb).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedEncryptionMode, "FFmpegAudioDecoder does not support encrypted content"));
        return;
    }
    ...
    if (config.profile() == AudioCodecProfile::kXHE_AAC) {
        std::move(bound_init_cb)
            .Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedProfile).WithData("decoder", "FFmpegAudioDecoder").WithData("profile", config.profile()));
        return;
    }
...
void FFmpegAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(decode_cb);
...
void FFmpegAudioDecoder::DecodeBuffer(const DecoderBuffer& buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool FFmpegAudioDecoder::FFmpegDecode(const DecoderBuffer& buffer)
{
    ...
    if (buffer.end_of_stream() || buffer.size() == 0) {
        ...
        packet->pts = ConvertToTimeBase(codec_context_->time_base, buffer.timestamp());
    ...
    FFmpegDecodingLoop::DecodeStatus decode_status = decoding_loop_->DecodePacket(
        packet, base::BindRepeating(&FFmpegAudioDecoder::OnNewFrame, base::Unretained(this), std::cref(buffer), &decoded_frame_this_loop));
    av_packet_free(&packet);
    switch (decode_status) {
    case FFmpegDecodingLoop::DecodeStatus::kSendPacketFailed:
        MEDIA_LOG(ERROR, media_log_) << "Failed to send audio packet for decoding: " << buffer.AsHumanReadableString();
        return false;
    ...
    case FFmpegDecodingLoop::DecodeStatus::kDecodeFrameFailed:
        DCHECK(!buffer.end_of_stream()) << "End of stream buffer produced an error! "
                                        << "This is quite possibly a bug in the audio decoder not handling "
                                        << "end of stream AVPackets correctly.";

        MEDIA_LOG(DEBUG, media_log_) << GetDecoderType() << " failed to decode an audio buffer: " << AVErrorToString(decoding_loop_->last_averror_code())
                                     << ", at " << buffer.AsHumanReadableString();
        break;
    ...
    if (!decoded_frame_this_loop && !buffer.end_of_stream()) {
        const bool result = discard_helper_->ProcessBuffers(buffer.time_info(), nullptr);
        DCHECK(!result);
    }
...
bool FFmpegAudioDecoder::OnNewFrame(const DecoderBuffer& buffer, bool* decoded_frame_this_loop, AVFrame* frame)
{
    ...
    ChannelLayout channel_layout = ChannelLayoutToChromeChannelLayout(codec_context_->ch_layout.u.mask, codec_context_->ch_layout.nb_channels);
    if (channel_layout == CHANNEL_LAYOUT_UNSUPPORTED && config_.channel_layout() == CHANNEL_LAYOUT_DISCRETE) {
        channel_layout = CHANNEL_LAYOUT_DISCRETE;
    }

    const bool is_sample_rate_change = frame->sample_rate != config_.samples_per_second();
    const bool is_config_change = is_sample_rate_change || channels != config_.channels() || channel_layout != config_.channel_layout();
    if (is_config_change) {
        ...
        if (frame->format != av_sample_format_) {
            MEDIA_LOG(ERROR, media_log_) << "Unsupported midstream configuration change!"
                                         << " Sample Rate: " << frame->sample_rate << " vs " << config_.samples_per_second()
                                         << " ChannelLayout: " << channel_layout << " vs " << config_.channel_layout() << " << Channels: " << channels << " vs "
                                         << config_.channels() << ", Sample Format: " << frame->format << " vs " << av_sample_format_;
            // This is an unrecoverable error, so bail out.
            return false;
        }

        MEDIA_LOG(DEBUG, media_log_) << " Detected midstream configuration change"
                                     << " PTS:" << buffer.timestamp().InMicroseconds() << " Sample Rate: " << frame->sample_rate << " vs "
                                     << config_.samples_per_second() << ", ChannelLayout: " << channel_layout << " vs " << config_.channel_layout()
                                     << ", Channels: " << channels << " vs " << config_.channels();
        config_.Initialize(config_.codec(), config_.sample_format(), channel_layout, frame->sample_rate, config_.extra_data(), config_.encryption_scheme(),
    ...
    scoped_refptr<AudioBuffer> output = reinterpret_cast<AudioBuffer*>(av_buffer_get_opaque(frame->buf[0]));
    ...
    *decoded_frame_this_loop = true;
    if (discard_helper_->ProcessBuffers(buffer.time_info(), output.get())) {
        if (is_config_change && output->sample_rate() != config_.samples_per_second()) {
...
bool FFmpegAudioDecoder::ConfigureDecoder(const AudioDecoderConfig& config)
{
    ...
    if (base::FeatureList::IsEnabled(kFFmpegAllowLists)) {
        ...
        codec_context_->codec_whitelist = av_strdup(FFmpegGlue::GetAllowedAudioDecoders());
    }
    ...
    if (!codec || avcodec_open2(codec_context_.get(), codec, &codec_options) < 0) {
        DLOG(ERROR) << "Could not initialize audio decoder: " << codec_context_->codec_id;
        ReleaseFFmpegResources();
        state_ = DecoderState::kUninitialized;
        return false;
    }
    ...
    if (codec_context_->ch_layout.nb_channels != config.channels()) {
        MEDIA_LOG(ERROR, media_log_) << "Audio configuration specified " << config.channels() << " channels, but FFmpeg thinks the file contains "
    ...
    decoding_loop_ = std::make_unique<FFmpegDecodingLoop>(codec_context_.get(), true);
...
void FFmpegAudioDecoder::ResetTimestampState(const AudioDecoderConfig& config)
{
    ...
    const int codec_delay = config.codec() == AudioCodec::kOpus ? 0 : config.codec_delay();
    discard_helper_ = std::make_unique<AudioDiscardHelper>(config.samples_per_second(), codec_delay, config.codec() == AudioCodec::kVorbis);
    discard_helper_->Reset(codec_delay);
}

int FFmpegAudioDecoder::GetAudioBuffer(struct AVCodecContext* s, AVFrame* frame, int flags)
{
    ...
    if (channels <= 0 || channels >= limits::kMaxChannels) {
        DLOG(ERROR) << "Requested number of channels (" << channels << ") exceeds limit.";
        return AVERROR(EINVAL);
    }
    ...
    if (s->sample_rate != frame->sample_rate) {
        DLOG(ERROR) << "AVCodecContext and AVFrame disagree on sample rate." << s->sample_rate << " vs " << frame->sample_rate;
        return AVERROR(EINVAL);
    }
    if (s->sample_rate < limits::kMinSampleRate || s->sample_rate > limits::kMaxSampleRate) {
        DLOG(ERROR) << "Requested sample rate (" << s->sample_rate << ") is outside supported range (" << limits::kMinSampleRate << " to "
    ...
    int buffer_size_in_bytes = av_samples_get_buffer_size(&frame->linesize[0], channels, frame->nb_samples, format, 0 /* align, use ffmpeg default */);
    ...
    ChannelLayout channel_layout = config_.channel_layout() == CHANNEL_LAYOUT_DISCRETE
    ...
    scoped_refptr<AudioBuffer> buffer = AudioBuffer::CreateBuffer(sample_format, channel_layout, channels, s->sample_rate, frames_required, pool_);
    ...
    if (number_of_planes <= AV_NUM_DATA_POINTERS) {
        ...
        frame->extended_data = static_cast<uint8_t**>(av_malloc(number_of_planes * sizeof(*frame->extended_data)));
        int i = 0;
    ...
    opaque->AddRef();
    frame->buf[0] = av_buffer_create(frame->data[0], buffer_size_in_bytes, ReleaseAudioBufferImpl, opaque, 0);
    return 0;
}
