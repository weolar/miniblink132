...
void SetDiscardPadding(AVPacket* packet, DecoderBuffer* buffer, double samples_per_second)
{
    ...
    if (buffer->timestamp().is_negative()) {
        buffer->set_discard_padding(std::make_pair(-buffer->timestamp(), base::TimeDelta()));
        return;
    }

    // If the timestamp is positive, try to use FFmpeg's discard data.
    size_t skip_samples_size = 0;
    const uint32_t* skip_samples_ptr = reinterpret_cast<const uint32_t*>(av_packet_get_side_data(packet, AV_PKT_DATA_SKIP_SAMPLES, &skip_samples_size));
    if (skip_samples_size < 4)
        return;
    buffer->set_discard_padding(std::make_pair(base::Seconds(*skip_samples_ptr / samples_per_second), base::TimeDelta()));
}

} // namespace

class AudioDecoderTest : public TestWithParam<std::tuple<AudioDecoderType, TestParams>> {
public:
    AudioDecoderTest()
        ...
        , last_decode_status_(DecoderStatus::Codes::kFailed)
    {
        ...
        case AudioDecoderType::kFFmpeg:
            decoder_ = std::make_unique<FFmpegAudioDecoder>(task_environment_.GetMainThreadTaskRunner(), &media_log_);
            break;
#if BUILDFLAG(IS_ANDROID)
        case AudioDecoderType::kMediaCodec:
            decoder_ = std::make_unique<MediaCodecAudioDecoder>(task_environment_.GetMainThreadTaskRunner());
            break;
#elif BUILDFLAG(IS_MAC)
        case AudioDecoderType::kAudioToolbox:
            decoder_ = std::make_unique<AudioToolboxAudioDecoder>(media_log_.Clone());
            break;
...
protected:
    bool IsSupported() const
    {
        if (params_.profile == AudioCodecProfile::kXHE_AAC) {
            return IsDecoderSupportedAudioType({ AudioCodec::kAAC, AudioCodecProfile::kXHE_AAC, false });
        }
        return true;
    }
    ...
    void DecodeBuffer(scoped_refptr<DecoderBuffer> buffer)
    {
        ...
        decoder_->Decode(std::move(buffer), base::BindOnce(&AudioDecoderTest::DecodeFinished, base::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
        ASSERT_FALSE(pending_decode_);
    }
    ...
    void set_params(const TestParams& params)
    {
        params_ = params;
    }
    ...
    void Initialize()
    {
        ...
        protocol_ = std::make_unique<InMemoryUrlProtocol>(data_->data(), data_->size(), false);
        ...
        start_timestamp_ = ConvertFromTimeBase(reader_->GetAVStreamForTesting()->time_base, packet->pts);

        // Seek back to the beginning.
        ASSERT_TRUE(reader_->SeekForTesting(start_timestamp_));

        AudioDecoderConfig config;
        ASSERT_TRUE(AVCodecContextToAudioDecoderConfig(reader_->codec_context_for_testing(), EncryptionScheme::kUnencrypted, &config));

#if (BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)) && BUILDFLAG(USE_PROPRIETARY_CODECS)
        ...
        if ((decoder_type_ == AudioDecoderType::kMediaCodec || decoder_type_ == AudioDecoderType::kMediaFoundation) && params_.codec == AudioCodec::kAAC
            && config.aac_extra_data().empty()) {
            int sample_rate;
            ChannelLayout channel_layout;
            std::vector<uint8_t> extra_data;
            ASSERT_GT(
                ADTSStreamParser().ParseFrameHeader(packet->data, packet->size, nullptr, &sample_rate, &channel_layout, nullptr, nullptr, &extra_data), 0);
            config.Initialize(
                AudioCodec::kAAC, kSampleFormatS16, channel_layout, sample_rate, extra_data, EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    ...
    void InitializeDecoderWithResult(const AudioDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&AudioDecoderTest::OnDecoderOutput, base::Unretained(this)), base::DoNothing());
    ...
    void Decode()
    {
        ...
        scoped_refptr<DecoderBuffer> buffer = DecoderBuffer::CopyFrom(AVPacketData(*packet));
        buffer->set_timestamp(ConvertFromTimeBase(reader_->GetAVStreamForTesting()->time_base, packet->pts));
        buffer->set_duration(ConvertFromTimeBase(reader_->GetAVStreamForTesting()->time_base, packet->duration));
        ...
        if (decoder_type_ == AudioDecoderType::kFFmpeg && params_.codec != AudioCodec::kOpus) {
    ...
    void Reset()
    {
        ...
        pending_reset_ = true;
        decoder_->Reset(base::BindOnce(&AudioDecoderTest::ResetFinished, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
        ASSERT_FALSE(pending_reset_);
    }
    ...
    std::string GetDecodedAudioMD5(size_t i)
    {
        ...
        for (int ch = 0; ch < output->channels(); ++ch) {
            base::MD5Update(&context, std::string_view(reinterpret_cast<char*>(output->channel(ch)), output->frames() * sizeof(*output->channel(ch))));
        }
    ...
    void ExpectDecodedAudio(size_t i, const std::string& exact_hash)
    {
        ...
        if (sample_info.hash) {
            ...
            EXPECT_TRUE(audio_hash.IsEquivalent(sample_info.hash, 0.03))
                << "Audio hashes differ. Expected: " << sample_info.hash << " Actual: " << audio_hash.ToString();
        }
    ...
    base::TimeDelta start_timestamp() const
    {
...
constexpr TestParams kReinitializeTestParams = { AudioCodec::kOpus, "bear-opus.ogg", kBearOpusExpectations, 24, 48000, CHANNEL_LAYOUT_STEREO };

#if BUILDFLAG(IS_ANDROID)
constexpr TestParams kMediaCodecTestParams[] = {
    { AudioCodec::kOpus, "bear-opus.ogg", kBearOpusExpectations, 24, 48000, CHANNEL_LAYOUT_STEREO },
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    { AudioCodec::kAAC, "sfx.adts",
        { {
            { 0, 23219, "-1.80,-1.49,-0.23,1.11,1.54,-0.11," },
            { 23219, 23219, "-1.90,-1.53,-0.15,1.28,1.23,-0.33," },
            { 46439, 23219, "0.54,0.88,2.19,3.54,3.24,1.63," },
        } },
        0, 44100, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kAAC, "bear-audio-implicit-he-aac-v2.aac",
        ...
        0, 24000, CHANNEL_LAYOUT_MONO },
#endif // defined(USE_PROPRIETARY_CODECS)
};
#endif // BUILDFLAG(IS_ANDROID)

#if (BUILDFLAG(IS_MAC) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_WIN)) && BUILDFLAG(USE_PROPRIETARY_CODECS)
...
constexpr TestParams kXheAacTestParams[] = {
    { AudioCodec::kAAC, "noise-xhe-aac.mp4",
        { {
            { 0, 42666, nullptr },
            { 42666, 42666, nullptr },
            { 85333, 42666, nullptr },
        } },
        0, 48000, CHANNEL_LAYOUT_STEREO, AudioCodecProfile::kXHE_AAC },
// Windows doesn't support 29.4kHz
#if !BUILDFLAG(IS_WIN)
    { AudioCodec::kAAC, "noise-xhe-aac-mono.mp4",
        { {
            { 0, 34829, nullptr },
            { 34829, 34829, nullptr },
            { 69659, 34829, nullptr },
        } },
        0, 29400, CHANNEL_LAYOUT_UNSUPPORTED, AudioCodecProfile::kXHE_AAC },
#endif
    { AudioCodec::kAAC, "noise-xhe-aac-44kHz.mp4",
        { {
            { 0, 23219, nullptr },
            { 23219, 23219, nullptr },
            { 46439, 23219, nullptr },
        } },
        0, 44100, CHANNEL_LAYOUT_STEREO, AudioCodecProfile::kXHE_AAC },
};
...
constexpr TestParams kFFmpegTestParams[] = {
    ...
    { AudioCodec::kAAC, "sfx.adts",
        ...
        0, 44100, CHANNEL_LAYOUT_MONO },
#endif
    { AudioCodec::kFLAC, "sfx-flac.mp4", kSfxFlacExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kFLAC, "sfx.flac", kSfxFlacExpectations, 0, 44100, CHANNEL_LAYOUT_MONO },
    { AudioCodec::kPCM, "sfx_f32le.wav",
        { {
    ...
    { AudioCodec::kPCM, "4ch.wav",
        ...
        0, 44100, CHANNEL_LAYOUT_QUAD },
    { AudioCodec::kVorbis, "sfx.ogg",
        { {
...
void AudioDecoderTest::SetReinitializeParams()
{
    ...
    if (decoder_type_ == AudioDecoderType::kAudioToolbox || decoder_type_ == AudioDecoderType::kMediaFoundation) {
        set_params(params_.channel_layout == kXheAacTestParams[0].channel_layout ? kXheAacTestParams[1] : kXheAacTestParams[0]);
        return;
    }
#endif
...
INSTANTIATE_TEST_SUITE_P(FFmpeg, AudioDecoderTest, Combine(Values(AudioDecoderType::kFFmpeg), ValuesIn(kFFmpegTestParams)));

#if BUILDFLAG(IS_ANDROID)
std::vector<TestParams> GetAndroidParams()
{
    std::vector<TestParams> params;
    params.insert(params.end(), std::cbegin(kMediaCodecTestParams), std::cend(kMediaCodecTestParams));
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    params.insert(params.end(), std::cbegin(kXheAacTestParams), std::cend(kXheAacTestParams));
#endif
    return params;
}

INSTANTIATE_TEST_SUITE_P(MediaCodec, AudioDecoderTest, Combine(Values(AudioDecoderType::kMediaCodec), ValuesIn(GetAndroidParams())));
#endif

#if BUILDFLAG(USE_PROPRIETARY_CODECS)
#if BUILDFLAG(IS_MAC)
INSTANTIATE_TEST_SUITE_P(AudioToolbox, AudioDecoderTest, Combine(Values(AudioDecoderType::kAudioToolbox), ValuesIn(kXheAacTestParams)));
#elif BUILDFLAG(IS_WIN)
INSTANTIATE_TEST_SUITE_P(MediaFoundation, AudioDecoderTest, Combine(Values(AudioDecoderType::kMediaFoundation), ValuesIn(kXheAacTestParams)));
#endif
#endif
