...
{
    return ((stream->type() == DemuxerStream::AUDIO && stream->audio_decoder_config().IsValidConfig())
        || (stream->type() == DemuxerStream::VIDEO && stream->video_decoder_config().IsValidConfig()));
}

DecryptingDemuxerStream::DecryptingDemuxerStream(const scoped_refptr<base::SequencedTaskRunner>& task_runner, MediaLog* media_log, const WaitingCB& waiting_cb)
...
void DecryptingDemuxerStream::Initialize(DemuxerStream* stream, CdmContext* cdm_context, PipelineStatusCallback status_cb)
{
    DVLOG(2) << __func__;
    ...
    event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&DecryptingDemuxerStream::OnCdmContextEvent, weak_factory_.GetWeakPtr()));
...
void DecryptingDemuxerStream::Read(uint32_t count, ReadCB read_cb)
{
    ...
    demuxer_stream_->Read(1, base::BindOnce(&DecryptingDemuxerStream::OnBuffersReadFromDemuxerStream, weak_factory_.GetWeakPtr()));
}
...
void DecryptingDemuxerStream::OnBuffersReadFromDemuxerStream(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    DCHECK_LE(buffers.size(), 1u) << "DecryptingDemuxerStream only reads a single-buffer.";
    OnBufferReadFromDemuxerStream(status, buffers.empty() ? nullptr : std::move(buffers[0]));
}

void DecryptingDemuxerStream::OnBufferReadFromDemuxerStream(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    ...
    if (status == kAborted || status == kError) {
        if (status == kError) {
            MEDIA_LOG(ERROR, media_log_) << GetDisplayName() << ": demuxer stream read error.";
        }
        state_ = kIdle;
...
void DecryptingDemuxerStream::DecryptPendingBuffer()
{
    ...
    TRACE_EVENT_ASYNC_BEGIN2("media", "DecryptingDemuxerStream::DecryptPendingBuffer", this, "type", DemuxerStream::GetTypeName(demuxer_stream_->type()),
        "timestamp_us", pending_buffer_to_decrypt_->timestamp().InMicroseconds());

    if (!DecoderBuffer::DoSubsamplesMatch(*pending_buffer_to_decrypt_)) {
        MEDIA_LOG(ERROR, media_log_) << "DecryptingDemuxerStream: Subsamples for Buffer do not match";
        state_ = kIdle;
        std::move(read_cb_).Run(kError, {});
        return;
    }

    if (HasClearLead() && !switched_clear_to_encrypted_ && pending_buffer_to_decrypt_->is_encrypted()) {
        MEDIA_LOG(INFO, media_log_) << "First switch from clear to encrypted buffers.";
        switched_clear_to_encrypted_ = true;
    }

    decryptor_->Decrypt(GetDecryptorStreamType(), pending_buffer_to_decrypt_,
        base::BindPostTaskToCurrentDefault(base::BindOnce(&DecryptingDemuxerStream::OnBufferDecrypted, weak_factory_.GetWeakPtr())));
}

void DecryptingDemuxerStream::OnBufferDecrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer)
{
    DVLOG(3) << __func__ << " - status: " << status;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK_EQ(state_, kPendingDecrypt) << state_;
    ...
    if (status == Decryptor::kError || status == Decryptor::kNeedMoreData) {
        ...
        MEDIA_LOG(ERROR, media_log_) << GetDisplayName() << ": decrypt error " << status;
        pending_buffer_to_decrypt_ = nullptr;
        state_ = kIdle;
        std::move(read_cb_).Run(kError, {});
        return;
    ...
    if (status == Decryptor::kNoKey) {
        ...
        std::string log_message = "no key for key ID " + base::HexEncode(key_id) + "; will resume decrypting after new usable key is available";
        ...
        if (need_to_try_again_if_nokey) {
            ...
            MEDIA_LOG(INFO, media_log_) << GetDisplayName() << ": key was added, resuming decrypt";
            DecryptPendingBuffer();
            return;
        }
        ...
        TRACE_EVENT_ASYNC_BEGIN0("media", "DecryptingDemuxerStream::WaitingForDecryptionKey", this);
    ...
    decrypted_buffer->set_is_key_frame(pending_buffer_to_decrypt_->is_key_frame());
...
void DecryptingDemuxerStream::CompletePendingDecrypt(Decryptor::Status status)
{
    ...
    TRACE_EVENT_ASYNC_END1("media", "DecryptingDemuxerStream::DecryptPendingBuffer", this, "status", Decryptor::GetStatusName(status));
}

void DecryptingDemuxerStream::CompleteWaitingForDecryptionKey()
{
    DCHECK_EQ(state_, kWaitingForKey);
    TRACE_EVENT_ASYNC_END0("media", "DecryptingDemuxerStream::WaitingForDecryptionKey", this);
}
