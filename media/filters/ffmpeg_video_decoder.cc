...
struct OpaqueData {
    OpaqueData(void* fb, scoped_refptr<FrameBufferPool> pool, uint8_t* d, size_t z, VideoFrameLayout l)
        : fb_priv(fb)
...
static int GetFFmpegVideoDecoderThreadCount(const VideoDecoderConfig& config)
{
    ...
    case VideoCodec::kH264:
        ...
        desired_threads = config.coded_size().width() * config.coded_size().height() * 3 / 1920 / 1080;
    }

    return VideoDecoder::GetRecommendedThreadCount(desired_threads);
}

static int GetVideoBufferImpl(struct AVCodecContext* s, AVFrame* frame, int flags)
{
...
int FFmpegVideoDecoder::GetVideoBuffer(struct AVCodecContext* codec_context, AVFrame* frame, int flags)
{
    ...
    const auto format = AVPixelFormatToVideoPixelFormat(static_cast<AVPixelFormat>(frame->format));

    if (format == PIXEL_FORMAT_UNKNOWN)
        return AVERROR(EINVAL);
    DCHECK(format == PIXEL_FORMAT_I420 || format == PIXEL_FORMAT_I422 || format == PIXEL_FORMAT_I444 || format == PIXEL_FORMAT_YUV420P9
        || format == PIXEL_FORMAT_YUV420P10 || format == PIXEL_FORMAT_YUV422P9 || format == PIXEL_FORMAT_YUV422P10 || format == PIXEL_FORMAT_YUV444P9
        || format == PIXEL_FORMAT_YUV444P10 || format == PIXEL_FORMAT_YUV420P12 || format == PIXEL_FORMAT_YUV422P12 || format == PIXEL_FORMAT_YUV444P12);

    // Do not trust `codec_context` sizes either.  Use whatever `frame` requests.
    gfx::Size coded_size(frame->width, frame->height);
    const int ret = av_image_check_size(coded_size.width(), coded_size.height(), 0, nullptr);
    if (ret < 0)
        return ret;
    ...
    auto layout = VideoFrame::CreateFullySpecifiedLayoutWithStrides(format, coded_size);
    if (!layout) {
        return AVERROR(EINVAL);
    }
    ...
    auto* opaque = new OpaqueData(fb_priv, frame_pool_, data, allocation_size, std::move(*layout));

    frame->buf[0] = av_buffer_create(frame->data[0], VideoFrame::AllocationSize(format, coded_size), ReleaseVideoBufferImpl, opaque,
        /*flags=*/0);
    return 0;
}
...
void FFmpegVideoDecoder::Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb,
    const WaitingCB& /* waiting_cb */)
{
    ...
    if (!frame_pool_) {
        ...
        frame_pool_ = base::MakeRefCounted<FrameBufferPool>(/*clear_allocations=*/true);
    }
    ...
    if (!IsCodecSupported(config.codec()) || !ConfigureDecoder(config, low_delay)) {
...
void FFmpegVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer.get());
...
bool FFmpegVideoDecoder::FFmpegDecode(const DecoderBuffer& buffer)
{
    ...
    FFmpegDecodingLoop::DecodeStatus decode_status
        = decoding_loop_->DecodePacket(packet, base::BindRepeating(&FFmpegVideoDecoder::OnNewFrame, base::Unretained(this)));
    av_packet_free(&packet);
    switch (decode_status) {
    case FFmpegDecodingLoop::DecodeStatus::kSendPacketFailed:
        MEDIA_LOG(ERROR, media_log_) << "Failed to send video packet for decoding: " << buffer.AsHumanReadableString();
        return false;
    ...
    case FFmpegDecodingLoop::DecodeStatus::kDecodeFrameFailed:
        MEDIA_LOG(DEBUG, media_log_) << GetDecoderType() << " failed to decode a video frame: " << AVErrorToString(decoding_loop_->last_averror_code())
...
bool FFmpegVideoDecoder::OnNewFrame(AVFrame* frame)
{
    ...
    if (!frame->data[VideoFrame::Plane::kY] || !frame->data[VideoFrame::Plane::kU] || !frame->data[VideoFrame::Plane::kV]) {
    ...
    const gfx::Rect visible_rect(frame->crop_left, frame->crop_top, frame->width - frame->crop_right, frame->height - frame->crop_bottom);

    // Why do we prefer the container aspect ratio here?
    VideoAspectRatio aspect_ratio = config_.aspect_ratio();
    if (!aspect_ratio.IsValid() && frame->sample_aspect_ratio.num > 0) {
        aspect_ratio = VideoAspectRatio::PAR(frame->sample_aspect_ratio.num, frame->sample_aspect_ratio.den);
    }
    ...
    auto video_frame = VideoFrame::WrapExternalDataWithLayout(opaque->layout, visible_rect, natural_size, opaque->data, opaque->size, pts);
    if (!video_frame) {
        return false;
    }
    ...
    if (codec_context_->codec_id == AV_CODEC_ID_H264 && frame->colorspace == AVCOL_SPC_RGB
        && VideoPixelFormatToChromaSampling(video_frame->format()) != VideoChromaSampling::k444) {
    ...
    } else if (frame->color_primaries != AVCOL_PRI_UNSPECIFIED || frame->color_trc != AVCOL_TRC_UNSPECIFIED || frame->colorspace != AVCOL_SPC_UNSPECIFIED) {
        color_space = VideoColorSpace(frame->color_primaries, frame->color_trc, frame->colorspace,
            frame->color_range != AVCOL_RANGE_MPEG ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED)
                          .ToGfxColorSpace();
    ...
    video_frame->AddDestructionObserver(frame_pool_->CreateFrameCallback(opaque->fb_priv));
...
bool FFmpegVideoDecoder::ConfigureDecoder(const VideoDecoderConfig& config, bool low_delay)
{
    DCHECK(config.IsValidConfig());
    ...
    if (base::FeatureList::IsEnabled(kFFmpegAllowLists)) {
        ...
        codec_context_->codec_whitelist = av_strdup(FFmpegGlue::GetAllowedVideoDecoders());
    }
