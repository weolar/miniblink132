...
std::unique_ptr<AudioVideoMetadataExtractor> GetExtractor(
    const std::string& filename, bool extract_attached_images, bool expected_result, double expected_duration, int expected_width, int expected_height)
{
...
const std::string GetTagValue(const media::AudioVideoMetadataExtractor::TagDictionary& tags, const char* tag_name)
{
...
const std::string TagsToString(const media::AudioVideoMetadataExtractor::TagDictionary& tags)
{
    std::string result;
    for (auto& kv : tags) {
        if (!result.empty())
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("9ch.ogg", true, true, 0.1, -1, -1);
    ...
    EXPECT_EQ("Processed by SoX", GetTagValue(extractor->stream_infos()[1].tags, "Comment"));
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("sfx_u8.wav", true, true, 0.288413, -1, -1);
    ...
    EXPECT_EQ("Lavf54.37.100", GetTagValue(extractor->stream_infos()[0].tags, "encoder"));
    EXPECT_EQ("Amadeus Pro", GetTagValue(extractor->stream_infos()[0].tags, "encoded_by"));
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("sfx.flac", true, true, 0.288413, -1, -1);
    ...
    EXPECT_EQ(2u, extractor->stream_infos()[0].tags.size());
    EXPECT_EQ("Lavf55.43.100", GetTagValue(extractor->stream_infos()[0].tags, "encoder"));
    EXPECT_EQ("Amadeus Pro", GetTagValue(extractor->stream_infos()[0].tags, "encoded_by"));
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("bear-320x240-multitrack.webm", true, true, 2.744, 320, 240);
    ...
    EXPECT_EQ("Lavf53.9.0", GetTagValue(extractor->stream_infos()[0].tags, "ENCODER"));
    ...
    EXPECT_EQ("Lavc52.32.0", GetTagValue(extractor->stream_infos()[5].tags, "ENCODER"));
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("90rotation.mp4", true, true, 0.196056, 1920, 1080);
    ...
    EXPECT_EQ("2014-02-11T00:39:25.000000Z", GetTagValue(extractor->stream_infos()[0].tags, "creation_time"));
    EXPECT_EQ("isom", GetTagValue(extractor->stream_infos()[0].tags, "major_brand"));
    EXPECT_EQ("0", GetTagValue(extractor->stream_infos()[0].tags, "minor_version"));

    EXPECT_EQ("h264", extractor->stream_infos()[1].type);
    EXPECT_EQ(6u, extractor->stream_infos()[1].tags.size()) << "Tags: " << TagsToString(extractor->stream_infos()[1].tags);
    EXPECT_EQ("2014-02-11T00:39:25.000000Z", GetTagValue(extractor->stream_infos()[1].tags, "creation_time"));
    EXPECT_EQ("VideoHandle", GetTagValue(extractor->stream_infos()[1].tags, "handler_name"));
    ...
    EXPECT_EQ(4u, extractor->stream_infos()[2].tags.size()) << "Tags: " << TagsToString(extractor->stream_infos()[2].tags);
    EXPECT_EQ("2014-02-11T00:39:25.000000Z", GetTagValue(extractor->stream_infos()[2].tags, "creation_time"));
    EXPECT_EQ("SoundHandle", GetTagValue(extractor->stream_infos()[2].tags, "handler_name"));
...
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("id3_png_test.mp3", true, true, 1.018776, -1, -1);
    ...
    EXPECT_EQ("OK Computer", GetTagValue(extractor->stream_infos()[0].tags, "album"));
    EXPECT_EQ("Radiohead", GetTagValue(extractor->stream_infos()[0].tags, "artist"));
    EXPECT_EQ("1997", GetTagValue(extractor->stream_infos()[0].tags, "date"));
    EXPECT_EQ("Lavf54.4.100", GetTagValue(extractor->stream_infos()[0].tags, "encoder"));
    EXPECT_EQ("Alternative", GetTagValue(extractor->stream_infos()[0].tags, "genre"));
    ...
    EXPECT_EQ("IEND\xae\x42\x60\x82", extractor->attached_images_bytes()[0].substr(extractor->attached_images_bytes()[0].size() - 8, 8));
    EXPECT_EQ("\xF3\xED\x8F\xC7\xC7\x98\xB9V|p\xC0u!\xB5\x82\xCF\x95\xF0\xCD\xCE", base::SHA1HashString(extractor->attached_images_bytes()[0]));
}

TEST(AudioVideoMetadataExtractorTest, AudioFLACInMp4)
{
    std::unique_ptr<AudioVideoMetadataExtractor> extractor = GetExtractor("sfx-flac.mp4", true, true, 0.288413, -1, -1);
    ...
    EXPECT_EQ(4u, extractor->stream_infos()[0].tags.size());
    EXPECT_EQ("isom", GetTagValue(extractor->stream_infos()[0].tags, "major_brand"));
    EXPECT_EQ("512", GetTagValue(extractor->stream_infos()[0].tags, "minor_version"));
    EXPECT_EQ("isomiso2mp41", GetTagValue(extractor->stream_infos()[0].tags, "compatible_brands"));
    EXPECT_EQ("Lavf57.75.100", GetTagValue(extractor->stream_infos()[0].tags, "encoder"));

    EXPECT_EQ("flac", extractor->stream_infos()[1].type);
    EXPECT_EQ(3u, extractor->stream_infos()[1].tags.size()) << "Tags: " << TagsToString(extractor->stream_infos()[1].tags);
    EXPECT_EQ("SoundHandler", GetTagValue(extractor->stream_infos()[1].tags, "handler_name"));
