...
public:
    FFmpegGlueTest()
        : protocol_(std::make_unique<StrictMock<MockProtocol>>())
    {
    ...
    {
        return glue_->format_context()->pb->read_packet(protocol_.get(), data, size);
    }
...
public:
    ...
    FFmpegGlueDestructionTest& operator=(const FFmpegGlueDestructionTest&) = delete;
...
protected:
    ...
    {
        histogram_tester_.ExpectUniqueSample("Media.DetectedContainer", container, 1);
    }
...
TEST_F(FFmpegGlueTest, Read)
{
    ...
    InSequence s;
    EXPECT_CALL(*protocol_, Read(0, buffer)).WillOnce(Return(0));
    EXPECT_CALL(*protocol_, Read(kBufferSize, buffer)).WillOnce(Return(kBufferSize));
    EXPECT_CALL(*protocol_, Read(kBufferSize, buffer)).WillOnce(Return(AVERROR(EIO)));
...
TEST_F(FFmpegGlueTest, Seek)
{
    ...
    InSequence s;
    EXPECT_CALL(*protocol_, SetPosition(-16)).WillOnce(Return(false));

    EXPECT_CALL(*protocol_, SetPosition(16)).WillOnce(Return(true));
    EXPECT_CALL(*protocol_, GetPosition(_)).WillOnce(DoAll(SetArgPointee<0>(8), Return(true)));
    ...
    EXPECT_CALL(*protocol_, GetPosition(_)).WillOnce(Return(false));
    ...
    EXPECT_CALL(*protocol_, GetPosition(_)).WillOnce(DoAll(SetArgPointee<0>(8), Return(true)));
    EXPECT_CALL(*protocol_, SetPosition(16)).WillOnce(Return(true));
    EXPECT_CALL(*protocol_, GetPosition(_)).WillOnce(DoAll(SetArgPointee<0>(16), Return(true)));
    ...
    // to the result then call SetPosition()+GetPosition().
    EXPECT_CALL(*protocol_, GetSize(_)).WillOnce(Return(false));

    EXPECT_CALL(*protocol_, GetSize(_)).WillOnce(DoAll(SetArgPointee<0>(16), Return(true)));
    EXPECT_CALL(*protocol_, SetPosition(8)).WillOnce(Return(false));

    EXPECT_CALL(*protocol_, GetSize(_)).WillOnce(DoAll(SetArgPointee<0>(16), Return(true)));
    EXPECT_CALL(*protocol_, SetPosition(8)).WillOnce(Return(true));
    EXPECT_CALL(*protocol_, GetPosition(_)).WillOnce(DoAll(SetArgPointee<0>(8), Return(true)));
...
TEST_F(FFmpegGlueDestructionTest, WithOpenWithOpenStreams)
{
    ...
    // disabled.
    std::unique_ptr<AVCodecContext, ScopedPtrAVFreeContext> context(AVStreamToAVCodecContext(glue_->format_context()->streams[1]));
