...
Vp9FrameContext kVp9DefaultFrameContext = {
    ...
    { 50, 126, 123, 221, 226 },
    // y_mode_probs
    { { 65, 32, 18, 144, 162, 194, 41, 51, 98 }, { 132, 68, 18, 165, 217, 196, 45, 40, 78 }, { 173, 80, 19, 176, 240, 193, 64, 35, 46 },
        { 221, 135, 38, 194, 248, 121, 96, 85, 29 } },
    // uv_mode_probs
    { { 120, 7, 76, 176, 208, 126, 28, 54, 103 }, { 48, 12, 154, 155, 139, 90, 34, 117, 119 }, { 67, 6, 25, 204, 243, 158, 13, 21, 96 },
        { 97, 5, 44, 131, 176, 139, 48, 68, 97 }, { 83, 5, 42, 156, 111, 152, 26, 49, 152 }, { 80, 5, 58, 178, 74, 83, 33, 62, 145 },
        { 86, 5, 32, 154, 192, 168, 14, 22, 163 }, { 85, 5, 32, 156, 216, 148, 19, 29, 73 }, { 77, 7, 64, 116, 132, 122, 37, 126, 120 },
        { 101, 21, 107, 181, 192, 103, 19, 67, 125 } },
    // partition_probs
    { { 199, 122, 141 }, { 147, 63, 159 }, { 148, 133, 118 }, { 121, 104, 114 }, { 174, 73, 87 }, { 92, 41, 83 }, { 82, 99, 50 }, { 53, 39, 39 },
        { 177, 58, 59 }, { 68, 26, 63 }, { 52, 79, 25 }, { 17, 14, 12 }, { 222, 34, 30 }, { 72, 16, 44 }, { 58, 32, 12 }, { 10, 7, 6 } },
    // mv_joint_probs
    { 32, 64, 96 },
    // mv_sign_prob
    { 128, 128 },
    ...
    { 216, 208 },
    // mv_bits_prob
    { { 136, 140, 148, 160, 176, 192, 224, 234, 234, 240 }, { 136, 140, 148, 160, 176, 192, 224, 234, 234, 240 } },
    // mv_class0_fr_probs
...
Vp9UncompressedHeaderParser::Vp9UncompressedHeaderParser(Vp9Parser::Context* context)
    : context_(context)
{
}

const Vp9FrameContext& Vp9UncompressedHeaderParser::GetVp9DefaultFrameContextForTesting() const
{
...
bool Vp9UncompressedHeaderParser::ReadFrameSizeFromRefs(Vp9FrameHeader* fhdr)
{
    ...
    for (const auto& idx : fhdr->ref_frame_idx) {
        const Vp9Parser::ReferenceSlot& ref = context_->GetRefSlot(idx);
        if (2 * fhdr->frame_width >= ref.frame_width && 2 * fhdr->frame_height >= ref.frame_height && fhdr->frame_width <= 16 * ref.frame_width
...
void Vp9UncompressedHeaderParser::ReadQuantizationParams(Vp9QuantizationParams* quants)
{
...
bool Vp9UncompressedHeaderParser::Parse(const uint8_t* stream, off_t frame_size, Vp9FrameHeader* fhdr)
{
    ...
    if (fhdr->IsKeyframe()) {
        ...
        if (fhdr->intra_only) {
            ...
            static_assert(std::extent<decltype(fhdr->ref_frame_sign_bias)>() >= Vp9RefType::VP9_FRAME_LAST + kVp9NumRefsPerFrame,
            ...
            for (size_t i = 0; i < kVp9NumRefsPerFrame; i++) {
                ...
                if (!ref.initialized) {
                    DVLOG(1) << "ref_frame_idx[" << i << "]=" << static_cast<int>(fhdr->ref_frame_idx[i]) << " refers to unused frame";
                    return false;
                }
                ...
                if (i == 0) {
                    ...
                    if (fhdr->color_space != ref.color_space && fhdr->color_space != Vp9ColorSpace::UNKNOWN && ref.color_space != Vp9ColorSpace::UNKNOWN) {
    ...
    if (fhdr->IsIntra() || fhdr->error_resilient_mode) {
        ...
        if (fhdr->IsKeyframe() || fhdr->error_resilient_mode || fhdr->reset_frame_context == 3) {
            ...
            context_->UpdateFrameContext(fhdr->frame_context_idx, fhdr->frame_context);
        }
