...
public:
    ...
    void OnBufferedTimeRangesChanged(const Ranges<base::TimeDelta>& ranges) override
    {
    }
...
static void QuitLoopWithStatus(base::OnceClosure quit_cb, media::PipelineStatus status)
{
    CHECK_EQ(status, media::PIPELINE_OK);
    std::move(quit_cb).Run();
}

static void OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data)
{
...
private:
    void OnReadDone(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure quit_when_idle_closure, bool* end_of_stream,
        base::TimeDelta* timestamp, media::DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers);
...
StreamReader::StreamReader(media::Demuxer* demuxer, bool enable_bitstream_converter)
{
...
void StreamReader::Read()
{
    ...
    streams_[index]->Read(1,
        base::BindOnce(&StreamReader::OnReadDone, base::Unretained(this), base::SingleThreadTaskRunner::GetCurrentDefault(), run_loop.QuitWhenIdleClosure(),
...
void StreamReader::OnReadDone(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure quit_when_idle_closure, bool* end_of_stream,
    base::TimeDelta* timestamp, media::DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
...
static void RunDemuxerBenchmark(const std::string& filename)
{
    ...
    for (int i = 0; i < kBenchmarkIterations; ++i) {
        ...
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb = base::BindRepeating(&OnEncryptedMediaInitData);
        Demuxer::MediaTracksUpdatedCB tracks_updated_cb = base::BindRepeating(&OnMediaTracksUpdated);
        FFmpegDemuxer demuxer(
            base::SingleThreadTaskRunner::GetCurrentDefault(), &data_source, encrypted_media_init_data_cb, tracks_updated_cb, &media_log_, true);

        {
            base::RunLoop run_loop;
            demuxer.Initialize(&demuxer_host, base::BindOnce(&QuitLoopWithStatus, run_loop.QuitClosure()));
            run_loop.Run();
        }
...
INSTANTIATE_TEST_SUITE_P(All, DemuxerPerfTest, testing::ValuesIn(kDemuxerTestFiles));
