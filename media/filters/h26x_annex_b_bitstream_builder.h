...
public:
    ...
    //   an AnnexB bitstream.
    explicit H26xAnnexBBitstreamBuilder(bool insert_emulation_prevention_bytes = false);
    ~H26xAnnexBBitstreamBuilder();
    H26xAnnexBBitstreamBuilder(const H26xAnnexBBitstreamBuilder&) = delete;
    H26xAnnexBBitstreamBuilder& operator=(const H26xAnnexBBitstreamBuilder&) = delete;
...
private:
    FRIEND_TEST_ALL_PREFIXES(H26xAnnexBBitstreamBuilderAppendBitsTest, AppendAndVerifyBits);
    ...
    static_assert(kGrowBytes >= kRegByteSize, "kGrowBytes must be larger than kRegByteSize");
