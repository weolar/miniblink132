...
static scoped_refptr<DecoderBuffer> CreateFakeEncryptedStreamBuffer(bool is_clear)
{
    auto buffer = base::MakeRefCounted<DecoderBuffer>(kFakeBufferSize);
    std::string iv = is_clear ? std::string() : std::string(reinterpret_cast<const char*>(kFakeIv), std::size(kFakeIv));
    if (!is_clear) {
        buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(std::string(reinterpret_cast<const char*>(kFakeKeyId), std::size(kFakeKeyId)), iv, {}));
    }
    return buffer;
}
...
{
    std::move(arg0).Run(buffer.get() ? DemuxerStream::kOk : DemuxerStream::kAborted, { buffer });
}
...
public:
    DecryptingDemuxerStreamTest()
        : demuxer_stream_(std::make_unique<DecryptingDemuxerStream>(
            task_environment_.GetMainThreadTaskRunner(), &media_log_, base::BindRepeating(&DecryptingDemuxerStreamTest::OnWaiting, base::Unretained(this))))
        , cdm_context_(std::make_unique<StrictMock<MockCdmContext>>())
        , decryptor_(std::make_unique<StrictMock<MockDecryptor>>())
        , is_initialized_(false)
        , input_audio_stream_(std::make_unique<StrictMock<MockDemuxerStream>>(DemuxerStream::AUDIO))
        , input_video_stream_(std::make_unique<StrictMock<MockDemuxerStream>>(DemuxerStream::VIDEO))
        ...
        , decrypted_buffer_(base::MakeRefCounted<DecoderBuffer>(kFakeBufferSize))
    {
    }

    DecryptingDemuxerStreamTest(const DecryptingDemuxerStreamTest&) = delete;
    DecryptingDemuxerStreamTest& operator=(const DecryptingDemuxerStreamTest&) = delete;
    ...
    void InitializeAudioAndExpectStatus(const AudioDecoderConfig& config, PipelineStatus expected_status)
    {
        input_audio_stream_->set_audio_decoder_config(config);
        demuxer_stream_->Initialize(input_audio_stream_.get(), cdm_context_.get(),
            base::BindOnce(&DecryptingDemuxerStreamTest::OnInitialized, base::Unretained(this), expected_status));
        base::RunLoop().RunUntilIdle();
    }

    void InitializeVideoAndExpectStatus(const VideoDecoderConfig& config, PipelineStatus expected_status)
    {
        input_video_stream_->set_video_decoder_config(config);
        demuxer_stream_->Initialize(input_video_stream_.get(), cdm_context_.get(),
            base::BindOnce(&DecryptingDemuxerStreamTest::OnInitialized, base::Unretained(this), expected_status));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void SetCdmType(CdmType cdm_type)
    {
        ...
        EXPECT_CALL(*cdm_context_, GetDecryptor()).WillRepeatedly(Return(has_decryptor ? decryptor_.get() : nullptr));
    }
    ...
    void Initialize(int audio_init_times = 1, int video_init_times = 1)
    {
        ...
        AudioDecoderConfig input_config(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 44100, EmptyExtraData(), EncryptionScheme::kCenc);
        ...
        EXPECT_EQ(input_config.bytes_per_channel(), output_config.bytes_per_channel());
        ...
        EXPECT_EQ(input_config.samples_per_second(), output_config.samples_per_second());
    }

    void ReadAndExpectBufferReadyWith(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer)
    {
        ...
        demuxer_stream_->Read(1, base::BindOnce(&DecryptingDemuxerStreamTest::BufferReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void EnterClearReadingState(bool is_stream_encrypted)
    {
        ...
        EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(ReturnBuffer(is_stream_encrypted ? clear_encrypted_stream_buffer_ : clear_buffer_));

        // For clearbuffer, decryptor->Decrypt() will not be called.
        DemuxerStream::DecoderBufferVector buffers;
        EXPECT_CALL(*this, BufferReady(DemuxerStream::kOk, _)).WillOnce(SaveArg<1>(&buffers));
        demuxer_stream_->Read(1, base::BindOnce(&DecryptingDemuxerStreamTest::BufferReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    ...
    {
        EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(ReturnBuffer(encrypted_buffer_));
        EXPECT_CALL(*decryptor_, Decrypt(_, _, _)).WillOnce(RunOnceCallback<2>(Decryptor::kSuccess, decrypted_buffer_));
    ...
    void EnterPendingReadState()
    {
        ...
        EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(MoveArg<0>(&pending_demuxer_read_cb_));
        demuxer_stream_->Read(1, base::BindOnce(&DecryptingDemuxerStreamTest::BufferReady, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    ...
    void EnterPendingDecryptState()
    {
        ...
        EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
        EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillOnce(WithArg<2>(Invoke([&](Decryptor::DecryptCB callback) {
    ...
    void EnterWaitingForKeyState()
    {
        ...
        EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _))
            .WillRepeatedly(RunOnceCallbackRepeatedly<2>(Decryptor::kNoKey, scoped_refptr<DecoderBuffer>()));
    ...
    {
        scoped_refptr<DecoderBuffer> buffer = (status == DemuxerStream::kOk) ? encrypted_buffer_ : nullptr;
        std::move(pending_demuxer_read_cb_).Run(status, { buffer });
    }

    void Reset()
    {
        EXPECT_CALL(*decryptor_, CancelDecrypt(Decryptor::kAudio)).WillRepeatedly(InvokeWithoutArgs(this, &DecryptingDemuxerStreamTest::AbortPendingDecryptCB));

        demuxer_stream_->Reset(NewExpectedClosure());
        base::RunLoop().RunUntilIdle();
    }

    MOCK_METHOD2(BufferReady, void(DemuxerStream::Status, DemuxerStream::DecoderBufferVector));
...
TEST_F(DecryptingDemuxerStreamTest, Read_DecryptError)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<2>(Decryptor::kError, scoped_refptr<DecoderBuffer>()));
...
TEST_F(DecryptingDemuxerStreamTest, Read_MismatchedSubsampleError)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<2>(Decryptor::kError, scoped_refptr<DecoderBuffer>()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingDemuxerStream: Subsamples for Buffer do not match"));
    ReadAndExpectBufferReadyWith(DemuxerStream::kError, nullptr);
}
...
TEST_F(DecryptingDemuxerStreamTest, Read_DecryptNeedMoreData)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillRepeatedly(ReturnBuffer(encrypted_buffer_));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _))
        .WillRepeatedly(RunOnceCallbackRepeatedly<2>(Decryptor::kNeedMoreData, scoped_refptr<DecoderBuffer>()));
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingDemuxerStream: decrypt error"));
    ReadAndExpectBufferReadyWith(DemuxerStream::kError, nullptr);
}
...
TEST_F(DecryptingDemuxerStreamTest, Read_EndOfStream)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(ReturnBuffer(DecoderBuffer::CreateEOSBuffer()));

    ReadAndExpectBufferReadyWith(DemuxerStream::kOk, DecoderBuffer::CreateEOSBuffer());
}
...
TEST_F(DecryptingDemuxerStreamTest, KeyAdded_DuringWaitingForKey)
{
    ...
    EXPECT_MEDIA_LOG(HasSubstr("DecryptingDemuxerStream: key was added, resuming decrypt"));
    EXPECT_CALL(*decryptor_, Decrypt(_, encrypted_buffer_, _)).WillRepeatedly(RunOnceCallbackRepeatedly<2>(Decryptor::kSuccess, decrypted_buffer_));
    DemuxerStream::DecoderBufferVector buffers;
    buffers.emplace_back(decrypted_buffer_);
...
TEST_F(DecryptingDemuxerStreamTest, DemuxerRead_Aborted)
{
    ...
    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(ReturnBuffer(scoped_refptr<DecoderBuffer>()));
...
TEST_F(DecryptingDemuxerStreamTest, DemuxerRead_ConfigChanged)
{
    ...
    AudioDecoderConfig new_config(AudioCodec::kVorbis, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 88200, EmptyExtraData(), EncryptionScheme::kCenc);
    input_audio_stream_->set_audio_decoder_config(new_config);

    EXPECT_CALL(*input_audio_stream_, OnRead(_)).WillOnce(RunOnceCallback<0>(DemuxerStream::kConfigChanged, DemuxerStream::DecoderBufferVector()));
