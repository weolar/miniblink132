...
public:
    PipelineControllerTest()
        ...
        , pipeline_controller_(std::unique_ptr<Pipeline>(pipeline_), base::BindRepeating(&PipelineControllerTest::OnStarted, base::Unretained(this)),
              base::BindRepeating(&PipelineControllerTest::OnSeeked, base::Unretained(this)),
              base::BindRepeating(&PipelineControllerTest::OnSuspended, base::Unretained(this)),
              base::BindRepeating(&PipelineControllerTest::OnBeforeResume, base::Unretained(this)),
              base::BindRepeating(&PipelineControllerTest::OnResumed, base::Unretained(this)),
              base::BindRepeating(&PipelineControllerTest::OnError, base::Unretained(this)))
    {
    }
    ...
    PipelineStatusCallback StartPipeline(bool is_streaming, bool is_static)
    {
        ...
        pipeline_controller_.Start(Pipeline::StartType::kNormal, &demuxer_, this, is_streaming, is_static);
        Mock::VerifyAndClear(pipeline_);
        EXPECT_CALL(*pipeline_, IsSuspended()).Times(AnyNumber()).WillOnce(Return(false));
        EXPECT_FALSE(pipeline_controller_.IsStable());
        return start_cb;
    }

    PipelineStatusCallback StartPipeline()
    {
        return StartPipeline(false, true);
    }
    ...
    PipelineStatusCallback SuspendPipeline()
    {
        ...
        Mock::VerifyAndClear(pipeline_);
        EXPECT_CALL(*pipeline_, IsSuspended()).Times(AnyNumber()).WillOnce(Return(true));
    ...
    PipelineStatusCallback ResumePipeline()
    {
        ...
        EXPECT_CALL(*pipeline_, OnResume(_, _)).WillOnce(DoAll(SaveArg<0>(&last_resume_time_), MoveArg<1>(&resume_cb)));
        EXPECT_CALL(*pipeline_, GetMediaTime()).WillRepeatedly(Return(base::TimeDelta()));
...
protected:
    void OnStarted(PipelineStatus status)
    {
        was_started_ = true;
    }
...
TEST_F(PipelineControllerTest, StartSuspendedSeekAndResume)
{
    ...
    PipelineStatusCallback start_cb;
    EXPECT_CALL(*pipeline_, OnStart(_, _, _, _)).WillOnce(MoveArg<3>(&start_cb));
    pipeline_controller_.Start(Pipeline::StartType::kSuspendAfterMetadata, &demuxer_, this, false, true);
    Mock::VerifyAndClear(pipeline_);
    ...
    PipelineStatusCallback resume_cb;
    EXPECT_CALL(*pipeline_, OnResume(_, _)).WillOnce(DoAll(MoveArg<1>(&resume_cb)));
    EXPECT_CALL(*pipeline_, GetMediaTime()).WillRepeatedly(Return(base::TimeDelta()));
...
TEST_F(PipelineControllerTest, DecoderStateLost)
{
    ...
    EXPECT_CALL(*pipeline_, GetMediaTime()).WillRepeatedly(Return(kCurrentMediaTime));
...
TEST_F(PipelineControllerTest, AudioTrackChangeDuringVideoTrackChange)
{
    ...
    pipeline_controller_.FireOnTrackChangeCompleteForTesting(PipelineController::State::PLAYING);
...
TEST_F(PipelineControllerTest, SuspendDuringVideoTrackChange)
{
    ...
    base::RunLoop loop;
    EXPECT_CALL(*pipeline_, OnSuspend(_)).WillOnce(RunOnceClosure(loop.QuitClosure()));
...
TEST_F(PipelineControllerTest, ResumePlaybackDuringSwitchingTracksState)
{
    ...
    pipeline_controller_.OnSelectedVideoTrackChanged({});
    pipeline_controller_.Resume();
    pipeline_controller_.FireOnTrackChangeCompleteForTesting(PipelineController::State::SUSPENDED);
}
