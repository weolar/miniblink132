...
class FakeVideoDecoderTest : public testing::Test, public testing::WithParamInterface<FakeVideoDecoderTestParams> {
public:
    FakeVideoDecoderTest()
        : decoder_(std::make_unique<FakeVideoDecoder>(0xFACCE, GetParam().decoding_delay, GetParam().max_decode_requests,
            base::BindRepeating(&FakeVideoDecoderTest::OnBytesDecoded, base::Unretained(this))))
    ...
    void InitializeWithConfigAndExpectResult(const VideoDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, false, nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&FakeVideoDecoderTest::FrameReady, base::Unretained(this)), base::NullCallback());
    ...
    void Decode()
    {
        ...
        if (num_input_buffers_ < kTotalBuffers) {
            buffer = CreateFakeVideoBufferForTest(current_config_, base::Milliseconds(kDurationMs * num_input_buffers_), base::Milliseconds(kDurationMs));
        ...
        decoder_->Decode(buffer, base::BindOnce(&FakeVideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
    }
    ...
    void ResetAndExpect(CallbackResult result)
    {
        ...
        decoder_->Reset(base::BindOnce(&FakeVideoDecoderTest::OnDecoderReset, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
...
INSTANTIATE_TEST_SUITE_P(NoParallelDecode, FakeVideoDecoderTest, ::testing::Values(FakeVideoDecoderTestParams(9, 1), FakeVideoDecoderTestParams(0, 1)));
INSTANTIATE_TEST_SUITE_P(ParallelDecode, FakeVideoDecoderTest, ::testing::Values(FakeVideoDecoderTestParams(9, 3), FakeVideoDecoderTestParams(0, 3)));
...
TEST_P(FakeVideoDecoderTest, Read_DecodingDelay)
{
    ...
    while (num_input_buffers_ < kTotalBuffers) {
        ReadOneFrame();
        EXPECT_EQ(num_input_buffers_, num_decoded_frames_ + GetParam().decoding_delay);
    }
}

TEST_P(FakeVideoDecoderTest, Read_ZeroDelay)
{
    decoder_ = std::make_unique<FakeVideoDecoder>(999, 0, 1, base::BindRepeating(&FakeVideoDecoderTest::OnBytesDecoded, base::Unretained(this)));
    Initialize();
...
TEST_P(FakeVideoDecoderTest, Read_Parallel)
{
    ...
    SatisfyDecodeAndExpect(GetParam().max_decode_requests > GetParam().decoding_delay ? OK : NOT_ENOUGH_DATA);
}
