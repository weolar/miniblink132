...
const std::string kEncryptedSimpleStream = "#EXTM3U\n"
...
const std::string kEncryptedSimpleStreamBadIV = "#EXTM3U\n"
                                                "#EXT-X-TARGETDURATION:2\n"
                                                "#EXT-X-MEDIA-SEQUENCE:0\n"
                                                "#EXT-X-PLAYLIST-TYPE:VOD\n"
                                                "#EXT-X-INDEPENDENT-SEGMENTS\n"
...
public:
    HlsMediaPlayerTagRecorderTest()
    {
        ...
        recorder = std::make_unique<HlsMediaPlayerTagRecorder>(std::move(network_access));
    }

    ~HlsMediaPlayerTagRecorderTest() override
    {
        network = nullptr;
    }

    void BindManifest(std::string url, std::string value, bool taint_origin = false)
    {
        EXPECT_CALL(*network, ReadManifest(GURL(url), _)).Times(1).WillOnce(RunOnceCallback<1>(StringHlsDataSourceStreamFactory::CreateStream(value, false)));
    }
