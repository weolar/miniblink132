...
{
    return AudioDecoderConfig(codec, kSampleFormatPlanarF32, CHANNEL_LAYOUT_STEREO, 1000, EmptyExtraData(), EncryptionScheme::kUnencrypted);
}
...
VideoDecoderConfig CreateVideoConfig(VideoCodec codec, int w, int h)
{
    ...
    return VideoDecoderConfig(codec, VIDEO_CODEC_PROFILE_UNKNOWN, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation, size,
...
{
    t->AddAudioTrack(CreateAudioConfig(codec), true, id, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
}

void AddVideoTrack(std::unique_ptr<MediaTracks>& t, VideoCodec codec, int id)
{
    t->AddVideoTrack(CreateVideoConfig(codec, 16, 16), true, id, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
}
...
public:
    ...
    std::unique_ptr<SourceBufferState> CreateSourceBufferState()
    {
        std::unique_ptr<FrameProcessor> frame_processor
            = std::make_unique<FrameProcessor>(base::BindRepeating(&SourceBufferStateTest::OnUpdateDuration, base::Unretained(this)), &media_log_);
        mock_stream_parser_ = new testing::StrictMock<MockStreamParser>();
        return base::WrapUnique(new SourceBufferState(base::WrapUnique(mock_stream_parser_.get()), std::move(frame_processor),
            base::BindRepeating(&SourceBufferStateTest::CreateDemuxerStream, base::Unretained(this)), &media_log_));
    }

    std::unique_ptr<SourceBufferState> CreateAndInitSourceBufferState(const std::string& expected_codecs)
    {
        ...
        EXPECT_CALL(*mock_stream_parser_, Init(_, _, _, _, _, _, _))
            .WillOnce([&](auto init_cb, auto config_cb, auto new_buffers_cb, auto encrypted_media_init_data_cb, auto new_segment_cb, auto end_of_segment_cb,
                          auto media_log) { new_config_cb_ = config_cb; });
        sbs->Init(base::BindOnce(&SourceBufferStateTest::SourceInitDone, base::Unretained(this)), expected_codecs,
            base::BindRepeating(&SourceBufferStateTest::StreamParserEncryptedInitData, base::Unretained(this)));

        sbs->SetTracksWatcher(base::BindRepeating(&SourceBufferStateTest::OnMediaTracksUpdated, base::Unretained(this)));

        // These tests are not expected to issue any parse warnings.
        EXPECT_CALL(*this, OnParseWarningMock(_)).Times(0);

        sbs->SetParseWarningCallback(base::BindRepeating(&SourceBufferStateTest::OnParseWarningMock, base::Unretained(this)));
    ...
    bool AppendDataAndReportTracks(const std::unique_ptr<SourceBufferState>& sbs, std::unique_ptr<MediaTracks> tracks)
    {
        ...
        CHECK_GT(StreamParser::kMaxPendingBytesPerParse, static_cast<int>(stream_data.size_bytes()));

        bool new_configs_result = false;

        EXPECT_CALL(*mock_stream_parser_, AppendToParseBuffer(stream_data)).WillOnce(Return(true));
        EXPECT_CALL(*mock_stream_parser_, Parse(StreamParser::kMaxPendingBytesPerParse))
            .WillOnce(DoAll(InvokeWithoutArgs([&] { new_configs_result = new_config_cb_.Run(std::move(tracks)); }),
        ...
        EXPECT_EQ(StreamParser::ParseStatus::kSuccess, sbs->RunSegmentParserLoop(t, t, &t));
    ...
    MOCK_METHOD2(StreamParserEncryptedInitData, void(EmeInitDataType, const std::vector<uint8_t>&));
    ...
    ChunkDemuxerStream* CreateDemuxerStream(DemuxerStream::Type type)
    {
        ...
        demuxer_streams_.push_back(base::WrapUnique(new ChunkDemuxerStream(type, MediaTrack::Id(base::NumberToString(++track_id)))));
...
TEST_F(SourceBufferStateTest, InitSourceBufferWithRelaxedCodecChecks)
{
    ...
    sbs->Init(base::BindOnce(&SourceBufferStateTest::SourceInitDone, base::Unretained(this)), std::nullopt,
        base::BindRepeating(&SourceBufferStateTest::StreamParserEncryptedInitData, base::Unretained(this)));
...
TEST_F(SourceBufferStateTest, MissingExpectedVideoStream)
{
    std::unique_ptr<SourceBufferState> sbs = CreateAndInitSourceBufferState("opus,vp9");
    std::unique_ptr<MediaTracks> tracks(new MediaTracks());
    tracks->AddAudioTrack(CreateAudioConfig(AudioCodec::kOpus), true, 1, MediaTrack::Kind(), MediaTrack::Label(), MediaTrack::Language());
    EXPECT_FOUND_CODEC_NAME(Audio, "opus");
