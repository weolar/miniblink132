...
public:
    ...
    {
        return vp9_uncompressed_header_parser_.GetVp9DefaultFrameContextForTesting();
    }
...
TEST_F(Vp9UncompressedHeaderParserTest, SetupPastIndependence)
{
    ...
    static_assert(std::is_pod<Vp9FrameContext>::value, "Vp9FrameContext is not POD, rewrite the next EXPECT_TRUE");
    EXPECT_TRUE(std::memcmp(&frame_header.frame_context, &GetVp9DefaultFrameContextForTesting(), sizeof(GetVp9DefaultFrameContextForTesting())) == 0);
}
