...
template <DemuxerStream::Type StreamType> class MEDIA_EXPORT DecoderStream {
public:
    ...
    using CreateDecodersCB = base::RepeatingCallback<std::vector<std::unique_ptr<Decoder>>()>;
    ...
    DecoderStream(std::unique_ptr<DecoderStreamTraits<StreamType>> traits, scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    void Initialize(DemuxerStream* stream, InitCB init_cb, CdmContext* cdm_context, StatisticsCB statistics_cb, WaitingCB waiting_cb);
    ...
    using PrepareCB = base::RepeatingCallback<void(scoped_refptr<Output>, OutputReadyCB)>;
    ...
    using ConfigChangeObserverCB = base::RepeatingCallback<void(const DecoderConfig&)>;
    void set_config_change_observer(ConfigChangeObserverCB config_change_observer)
    {
    ...
    void set_decoder_change_observer(DecoderChangeObserverCB decoder_change_observer_cb)
    {
    ...
    bool is_demuxer_read_pending() const
    {
        return pending_demuxer_read_;
    }

    DecoderSelector<StreamType>& GetDecoderSelectorForTesting(base::PassKey<class VideoDecoderStreamTest>)
    {
...
private:
    ...
    void OnDecoderSelected(DecoderStatus::Or<std::unique_ptr<Decoder>> decoder_or_error, std::unique_ptr<DecryptingDemuxerStream> decrypting_demuxer_stream);
    ...
    void OnDecodeDone(int buffer_size, bool end_of_stream, std::unique_ptr<ScopedDecodeTrace> trace_event, DecoderStatus status);
    ...
    void OnBuffersReady(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers);
