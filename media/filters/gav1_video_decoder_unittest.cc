...
void HashFrameVisibleRectForTesting(base::MD5Context* context, const VideoFrame& frame)
{
    DCHECK(context);
    for (size_t plane = 0; plane < VideoFrame::NumPlanes(frame.format()); ++plane) {
        int rows = frame.Rows(plane, frame.format(), frame.visible_rect().height());
        for (int row = 0; row < rows; ++row) {
            int row_bytes = frame.RowBytes(plane, frame.format(), frame.visible_rect().width());
            base::MD5Update(context, base::StringPiece(reinterpret_cast<const char*>(frame.visible_data(plane) + frame.stride(plane) * row), row_bytes));
        }
    }
}
...
public:
    ...
    void InitializeWithConfigWithResult(const VideoDecoderConfig& config, bool success)
    {
        decoder_->Initialize(config, false, nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&Gav1VideoDecoderTest::FrameReady, base::Unretained(this)), base::NullCallback());
    ...
    DecoderStatus Decode(scoped_refptr<DecoderBuffer> buffer)
    {
        ...
        decoder_->Decode(std::move(buffer), base::BindOnce(&Gav1VideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
...
{
    Initialize();
    EXPECT_TRUE(DecodeSingleFrame(ReadTestDataFile("av1-monochrome-I-frame-320x240-8bpp")).is_ok());
    ASSERT_EQ(1U, output_frames_.size());
...
{
    Initialize();
    EXPECT_TRUE(DecodeSingleFrame(ReadTestDataFile("av1-monochrome-I-frame-320x240-10bpp")).is_ok());
    ASSERT_EQ(1U, output_frames_.size());
...
{
    Initialize();
    EXPECT_TRUE(DecodeSingleFrame(ReadTestDataFile("av1-monochrome-I-frame-320x240-12bpp")).is_ok());
    ASSERT_EQ(1U, output_frames_.size());
...
TEST_F(Gav1VideoDecoderTest, FrameValidAfterPoolDestruction)
{
    ...
    memset(output_frames_.front()->writable_data(VideoFrame::kYPlane), 0xff,
        output_frames_.front()->rows(VideoFrame::kYPlane) * output_frames_.front()->stride(VideoFrame::kYPlane));
}
