...
public:
    ...
    size_t cadence_size_for_testing() const
    {
        return cadence_.size();
    }
    std::string GetCadenceForTesting() const
    {
        return CadenceToString(cadence_);
    }
    ...
    static bool HasSimpleCadence(
        base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta minimum_time_until_max_drift = base::Seconds(8));
...
private:
    ...
    bool UpdateBresenhamCadenceEstimate(base::TimeDelta render_interval, base::TimeDelta frame_duration);

    void UpdateCadenceInternal(Cadence new_cadence, base::TimeDelta time_until_max_drift);
