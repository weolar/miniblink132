...
public:
    ...
    public:
        ...
        virtual void CreateDataSource(GURL uri, bool ignore_cache, DataSourceCb cb) = 0;
    };

    ~HlsDataSourceProviderImpl() override;
    explicit HlsDataSourceProviderImpl(std::unique_ptr<DataSourceFactory> factory);

    // HlsDataSourceProvider implementation
    void ReadFromCombinedUrlQueue(SegmentQueue segments, ReadCb callback) override;

    void ReadFromExistingStream(std::unique_ptr<HlsDataSourceStream> stream, ReadCb callback) override;
    void AbortPendingReads(base::OnceClosure cb) override;

private:
    void UpdateStreamMetadata(HlsDataSourceStream::StreamId, HlsDataSourceStream& stream);
    void OnDataSourceCreated(std::unique_ptr<HlsDataSourceStream> stream, ReadCb callback, std::unique_ptr<DataSource> data_source);
    void OnStreamReleased(HlsDataSourceStream::StreamId stream_id);
    void DataSourceInitialized(std::unique_ptr<HlsDataSourceStream> stream, ReadCb callback, bool success);

    std::unique_ptr<DataSourceFactory> data_source_factory_;

    HlsDataSourceStream::StreamId::Generator stream_id_generator_;

    base::flat_map<HlsDataSourceStream::StreamId, std::unique_ptr<DataSource>> data_source_map_;
