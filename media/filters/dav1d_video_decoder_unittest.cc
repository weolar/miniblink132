...
public:
    Dav1dVideoDecoderTest()
        : decoder_(std::make_unique<Dav1dVideoDecoder>(std::make_unique<NullMediaLog>()))
    ...
    ~Dav1dVideoDecoderTest() override
    {
        Destroy();
    }
    ...
    {
        decoder_->Initialize(config, true, // Use low delay so we get 1 frame out for each frame in.
            nullptr, base::BindOnce([](bool success, DecoderStatus status) { EXPECT_EQ(status.is_ok(), success); }, success),
            base::BindRepeating(&Dav1dVideoDecoderTest::FrameReady, base::Unretained(this)), base::NullCallback());
    ...
    DecoderStatus Decode(scoped_refptr<DecoderBuffer> buffer)
    {
        ...
        decoder_->Decode(std::move(buffer), base::BindOnce(&Dav1dVideoDecoderTest::DecodeDone, base::Unretained(this)));
        base::RunLoop().RunUntilIdle();
...
TEST_F(Dav1dVideoDecoderTest, DecodeFrame_8bitMono)
{
    ...
    const auto& frame = output_frames_.front();
    EXPECT_EQ(PIXEL_FORMAT_I420, frame->format());
    EXPECT_EQ(frame->data(VideoFrame::Plane::kU), frame->data(VideoFrame::Plane::kV));
...
{
    Initialize();
    EXPECT_FALSE(DecodeSingleFrame(ReadTestDataFile("vp9-I-frame-320x240")).is_ok());
}
