...
public:
    ...
    AudioRendererAlgorithm(MediaLog* media_log, AudioRendererAlgorithmParameters params);
    ...
    int FillBuffer(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate);
    ...
    int QueuePlaybackThreshold() const
    {
        return playback_threshold_;
    }
    ...
    std::vector<bool> channel_mask_for_testing()
    {
        return channel_mask_;
    }

    FillBufferMode last_mode_for_testing()
    {
        return last_mode_;
    }
    ...
    {
        return effective_playback_rate_;
    }
...
private:
    ...
    int WriteCompletedFramesTo(int requested_frames, int output_offset, AudioBus* dest);
    ...
    int ResampleAndFill(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate);

    // Uses the WSOLA algorithm to speed up or slowdown audio.
    int RunWsolaAndFill(AudioBus* dest, int dest_offset, int requested_frames, double playback_rate);
