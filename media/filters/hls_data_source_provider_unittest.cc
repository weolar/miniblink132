...
public:
    ...
    void OnReleaseInternal(HlsDataSourceStream::StreamId id)
    {
        OnRelease(id);
    }
...
protected:
    std::unique_ptr<HlsDataSourceStream> CreateStream(std::optional<hls::types::ByteRange> range)
    {
    ...
    std::unique_ptr<HlsDataSourceStream> CreateStream(HlsDataSourceProvider::SegmentQueue queue)
    {
        ...
        return std::make_unique<HlsDataSourceStream>(
            stream_id, std::move(queue), base::BindOnce(&HlsDataSourceStreamUnittest::OnReleaseInternal, base::Unretained(this), stream_id));
    }
