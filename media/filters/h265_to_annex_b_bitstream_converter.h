...
public:
    ...
    H265ToAnnexBBitstreamConverter(const H265ToAnnexBBitstreamConverter&) = delete;
    H265ToAnnexBBitstreamConverter& operator=(const H265ToAnnexBBitstreamConverter&) = delete;
    ...
    bool ParseConfiguration(const uint8_t* configuration_record, int configuration_record_size, mp4::HEVCDecoderConfigurationRecord* hevc_config);

    // Returns the buffer size needed to store the parameter sets in |hevc_config|
    // in Annex B form.
    uint32_t GetConfigSize(const mp4::HEVCDecoderConfigurationRecord& hevc_config) const;
    ...
    uint32_t CalculateNeededOutputBufferSize(const uint8_t* input, uint32_t input_size, const mp4::HEVCDecoderConfigurationRecord* hevc_config) const;
    ...
    //          of converted data)
    bool ConvertHEVCDecoderConfigToByteStream(const mp4::HEVCDecoderConfigurationRecord& hevc_config, uint8_t* output, uint32_t* output_size);
...
private:
    ...
    bool WriteParamSet(const std::vector<uint8_t>& param_set, uint8_t** out, uint32_t* out_size) const;
