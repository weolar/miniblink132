...
public:
    ...
    virtual void ReadFromCombinedUrlQueue(SegmentQueue segments, ReadCb callback) = 0;

    // Continues to read from an existing stream.
    virtual void ReadFromExistingStream(std::unique_ptr<HlsDataSourceStream> stream, ReadCb callback) = 0;
...
public:
    ...
    HlsDataSourceStream(StreamId stream_id, HlsDataSourceProvider::SegmentQueue segments, base::OnceClosure on_destructed_cb);
    ~HlsDataSourceStream();
    ...
    std::optional<size_t> max_read_position() const
    {
    ...
    // subsequent reads.
    void set_total_memory_usage(uint64_t usage)
    {
        memory_usage_ = usage;
    }
    ...
    void set_would_taint_origin()
    {
        would_taint_origin_ = true;
    }
