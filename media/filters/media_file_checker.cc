...
bool MediaFileChecker::Start(base::TimeDelta check_time)
{
    ...
    bool read_ok = true;
    media::BlockingUrlProtocol protocol(&source, base::BindRepeating(&OnMediaFileCheckerError, &read_ok));
    ...
    const base::TimeTicks deadline = base::TimeTicks::Now() + std::min(check_time, base::Seconds(kMaxCheckTimeInSeconds));
    do {
        ...
        if (decoder.loop) {
            result = decoder.loop->DecodePacket(packet.get(), do_nothing_cb) == FFmpegDecodingLoop::DecodeStatus::kOkay ? 0 : -1;
        }
