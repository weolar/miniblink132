...
bool CanUseAudioToolbox(const AudioDecoderConfig& config)
{
    ...
    if (config.codec() == AudioCodec::kEAC3 || config.codec() == AudioCodec::kAC3) {
        return true;
    }
#endif
...
OSStatus ProvideInputCallback(
    AudioConverterRef decoder, UInt32* num_packets, AudioBufferList* buffer_list, AudioStreamPacketDescription** packets, void* user_data)
{
    ...
    *num_packets = buffer_list->mNumberBuffers = 1;
    ...
    buffer_list->mBuffers[0].mData = const_cast<uint8_t*>(input_data->buffer->data());
...
// static
AudioConverterRef AudioToolboxAudioDecoder::ScopedAudioConverterRefTraits::Retain(AudioConverterRef converter)
{
    NOTREACHED() << "Only compatible with ASSUME policy";
}

// static
void AudioToolboxAudioDecoder::ScopedAudioConverterRefTraits::Release(AudioConverterRef converter)
{
    const auto result = AudioConverterDispose(converter);
    OSSTATUS_DLOG_IF(WARNING, result != noErr, result) << "AudioConverterDispose() failed";
}

AudioToolboxAudioDecoder::AudioToolboxAudioDecoder(std::unique_ptr<MediaLog> media_log)
...
void AudioToolboxAudioDecoder::Initialize(
    const AudioDecoderConfig& config, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    if (!CanUseAudioToolbox(config)) {
        DLOG(WARNING) << "Only xHE-AAC/AC3/E-AC3 decoding is supported by this decoder.";
    ...
    base::BindPostTaskToCurrentDefault(std::move(init_cb))
        .Run(CreateDecoder(config) ? DecoderStatus::Codes::kOk : DecoderStatus::Codes::kFailedToCreateDecoder);
}

void AudioToolboxAudioDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    ...
    for (int i = 0; i < output_bus_->channels(); ++i) {
        ...
        output_buffer_list_->mBuffers[i].mDataByteSize = output_bus_->frames() * sizeof(float);
    ...
    auto result = AudioConverterFillComplexBuffer(decoder_.get(), ProvideInputCallback, &input_data, &num_frames, output_buffer_list_.get(), nullptr);
    ...
    if (result != noErr && result != kNoMoreDataError) {
        OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterFillComplexBuffer() failed";
        std::move(decode_cb_bound).Run(DecoderStatus::Codes::kPlatformDecodeFailure);
        return;
    }

    limiter_queue_->Push(
        *output_bus_, num_frames, buffer->timestamp(), base::BindOnce(&AudioToolboxAudioDecoder::OnOutputReady, base::Unretained(this), buffer->time_info()));
...
void AudioToolboxAudioDecoder::Reset(base::OnceClosure reset_cb)
{
    ...
    OSSTATUS_DLOG_IF(WARNING, result != noErr, result) << "AudioConverterReset() failed";
...
bool AudioToolboxAudioDecoder::CreateDecoder(const AudioDecoderConfig& config)
{
    ...
    case AudioCodec::kAAC: {
        ...
        auto status = AudioFormatGetProperty(kAudioFormatProperty_FormatInfo, magic_cookie.size(), magic_cookie.data(), &format_size, &input_format);
        if (status != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, status, media_log_) << "AudioFormatGetProperty() failed";
            return false;
        }
        break;
    }
    ...
    case AudioCodec::kEAC3:
        // Input is AC3/E-AC3.
        input_format.mFormatID = config.codec() == AudioCodec::kAC3 ? kAudioFormatAC3 : kAudioFormatEnhancedAC3;
    ...
    output_format.mFormatFlags = kLinearPCMFormatFlagIsFloat | kLinearPCMFormatFlagIsNonInterleaved;
    ...
    channel_count_ = output_format.mChannelsPerFrame = input_format.mChannelsPerFrame;
    ...
    output_format.mBytesPerPacket = output_format.mBytesPerFrame = output_format.mBitsPerChannel / 8;

    // Create the decoder.
    auto result = AudioConverterNew(&input_format, &output_format, decoder_.InitializeInto());
    if (result != noErr) {
        OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterNew() failed";
        return false;
    }
    ...
    if (channel_count_ > kMaxConcurrentChannels) {
        ...
        UInt32 size;
        result = AudioConverterGetPropertyInfo(decoder_.get(), kAudioConverterOutputChannelLayout, &size, NULL);
        if (result != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterGetPropertyInfo() failed";
            return false;
        }

        ScopedAudioChannelLayout output_layout(size);
        result = AudioConverterGetProperty(decoder_.get(), kAudioConverterOutputChannelLayout, &size, output_layout.layout());
        if (result != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterGetProperty() failed";
            return false;
        }
        ...
        if (!AudioChannelLayoutToChannelLayout(*output_layout.layout(), &channel_layout_)) {
    ...
    if (channel_count_ != static_cast<UInt32>(config.channels()) || channel_layout_ != config.channel_layout()) {
        MEDIA_LOG(INFO, media_log_) << "Audio config updated: channels: " << channel_count_ << ", channel layout: " << ChannelLayoutToString(channel_layout_);
    }
    ...
    auto ordered_layout = ChannelLayoutToAudioChannelLayout(channel_layout_, channel_count_);
    result = AudioConverterSetProperty(decoder_.get(), kAudioConverterOutputChannelLayout, ordered_layout->layout_size(), ordered_layout->layout());
    if (result != noErr) {
        OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterSetProperty() failed";
        return false;
    }

    if (config.codec() == AudioCodec::kAAC) {
        // Instill the magic!
        result = AudioConverterSetProperty(decoder_.get(), kAudioConverterDecompressionMagicCookie, magic_cookie.size(), magic_cookie.data());
        if (result != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterSetProperty() failed";
            return false;
        }
        ...
        result = AudioConverterSetProperty(decoder_.get(), kAudioCodecPropertyProgramTargetLevel, sizeof(kDefaultLoudness), &kDefaultLoudness);
        if (result != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterSetProperty() failed to set loudness.";
            return false;
        }
        ...
        result = AudioConverterSetProperty(decoder_.get(), 0x64726370 /* "drcp" */, sizeof(kDefaultEffectType), &kDefaultEffectType);
        if (result != noErr) {
            OSSTATUS_MEDIA_LOG(ERROR, result, media_log_) << "AudioConverterSetProperty() failed to set DRC effect type.";
            return false;
        }
    }

    discard_helper_ = std::make_unique<AudioDiscardHelper>(sample_rate_, config.codec_delay(), false);
    discard_helper_->Reset(config.codec_delay());

    // Create staging structures we'll give to macOS for writing data into.
    output_bus_ = AudioBus::Create(input_format.mChannelsPerFrame, input_format.mFramesPerPacket);

    limiter_queue_
        = std::make_unique<LimitingAudioQueue>(channel_layout_, config.samples_per_second(), input_format.mChannelsPerFrame, input_format.mFramesPerPacket);
    ...
    output_buffer_list_.reset(reinterpret_cast<AudioBufferList*>(calloc(1, sizeof(AudioBufferList) + output_bus_->channels() * sizeof(AudioBuffer))));
    return true;
}

void AudioToolboxAudioDecoder::OnOutputReady(DecoderBuffer::TimeInfo time_info, scoped_refptr<AudioBuffer> output_buffer)
{
    if (discard_helper_->ProcessBuffers(time_info, output_buffer.get())) {
        base::BindPostTaskToCurrentDefault(output_cb_).Run(std::move(output_buffer));
    }
}
