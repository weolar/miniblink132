...
void GenerateOutputFormat(const AudioEncoder::Options& options, AudioStreamBasicDescription& output_format)
{
...
bool GenerateCodecDescription(AudioCodec codec, AudioConverterRef encoder, std::vector<uint8_t>& codec_desc)
{
    DCHECK(codec == AudioCodec::kAAC);

    // AAC should always have a codec description available.
    UInt32 magic_cookie_size = 0;
    auto result = AudioConverterGetPropertyInfo(encoder, kAudioConverterCompressionMagicCookie, &magic_cookie_size, nullptr);
    ...
    result = AudioConverterGetProperty(encoder, kAudioConverterCompressionMagicCookie, &magic_cookie_size, magic_cookie.data());
    if (result != noErr) {
...
std::optional<int> FindNearestSupportedBitrate(AudioConverterRef encoder, UInt32 requested_bitrate)
{
    UInt32 size;
    auto status = AudioConverterGetPropertyInfo(encoder, kAudioConverterApplicableEncodeBitRates, &size, nullptr);
    if (status != noErr || !size) {
        return std::nullopt;
    }

    auto list_storage = base::HeapArray<AudioValueRange>::Uninit(size / sizeof(AudioValueRange));
    status = AudioConverterGetProperty(encoder, kAudioConverterApplicableEncodeBitRates, &size, list_storage.data());
    if (status != noErr) {
        return std::nullopt;
    }
    ...
    for (const auto& rate : list_storage) {
        ...
        if (rate.mMinimum <= requested_bitrate && rate.mMaximum >= requested_bitrate) {
            return requested_bitrate;
        }
        if (rate.mMaximum <= requested_bitrate && rate.mMaximum > closest_match.value_or(0)) {
...
void AudioToolboxAudioEncoder::Initialize(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    if (output_cb_) {
    ...
    const AudioParameters fifo_params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(channel_count_), sample_rate_, kAacFramesPerBuffer);
...
void AudioToolboxAudioEncoder::Encode(std::unique_ptr<AudioBus> input_bus, base::TimeTicks capture_time, EncoderStatusCB done_cb)
{
    if (!encoder_) {
...
void AudioToolboxAudioEncoder::Flush(EncoderStatusCB flush_cb)
{
    ...
    if (!encoder_) {
        std::move(flush_cb).Run(EncoderStatus::Codes::kEncoderInitializeNeverCompleted);
        return;
    }
...
bool AudioToolboxAudioEncoder::CreateEncoder(const AudioStreamBasicDescription& output_format)
{
    // Input is always float planar.
    AudioStreamBasicDescription input_format = {};
    input_format.mFormatID = kAudioFormatLinearPCM;
    input_format.mFormatFlags = kLinearPCMFormatFlagIsFloat | kLinearPCMFormatFlagIsNonInterleaved;
    ...
    input_format.mBytesPerPacket = input_format.mBytesPerFrame = input_format.mBitsPerChannel / 8;
    ...
    if (options_.bitrate && options_.bitrate > 0) {
        ...
        if (options_.bitrate && options_.bitrate != rate) {
            DVLOG(1) << "Reducing bitrate from " << rate << " to nearest supported by the encoder " << *options_.bitrate;
        ...
        result = AudioConverterSetProperty(encoder_, kAudioConverterEncodeBitRate, sizeof(rate), &rate);
        if (result != noErr) {
    ...
    if (options_.bitrate_mode) {
        const bool use_vbr = options_.bitrate_mode == AudioEncoder::BitrateMode::kVariable;

        UInt32 bitrate_mode = use_vbr ? kAudioCodecBitRateControlMode_Variable : kAudioCodecBitRateControlMode_Constant;

        result = AudioConverterSetProperty(encoder_, kAudioCodecPropertyBitRateControlMode, sizeof(bitrate_mode), &bitrate_mode);
        if (result != noErr) {
    ...
    result = AudioConverterGetProperty(encoder_, kAudioConverterPropertyMaximumOutputPacketSize, &prop_size, &max_packet_size_);
    if (result != noErr) {
...
void AudioToolboxAudioEncoder::DoEncode(const AudioBus* input_bus)
{
    ...
    do {
        ...
        auto result = AudioConverterFillComplexBuffer(encoder_, ProvideInputCallback, &input_data, &num_packets, &output_buffer_list, &packet_description);
        ...
        if (result != noErr) {
            ...
            std::move(current_done_cb_).Run(EncoderStatus::Codes::kEncoderFailedEncode);
            return;
        }
        ...
        if (format == AudioEncoder::AacOutputFormat::ADTS) {
            ...
            packet_buffer = aac_config_parser_.CreateAdtsFromEsds(temp_output_buf_, &adts_header_size);
        ...
        EncodedAudioBuffer encoded_buffer(
            AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Guess(channel_count_), sample_rate_, num_frames), std::move(packet_buffer),
            base::TimeTicks() + timestamp_helper_->GetTimestamp(), timestamp_helper_->GetFrameDuration(num_frames));
