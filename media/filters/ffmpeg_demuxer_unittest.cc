...
{
    return CONTAINS_STRING(arg, "\"info\":\"FFmpegDemuxer: created " + std::string(stream_type) + " stream, config codec:");
}
...
{
    return CONTAINS_STRING(arg, "\"info\":\"FFmpegDemuxer: skipping invalid or unsupported " + std::string(stream_type) + " track");
}
...
static void EosOnReadDone(bool* got_eos_buffer, base::OnceClosure quit_closure, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    // TODO(crbug.com/40232931): add multi read unit tests in next CL.
    DCHECK_EQ(buffers.size(), 1u) << "FFmpegDemuxerTest only reads a single-buffer.";
...
protected:
    ...
    void InitializeDemuxerInternal(media::PipelineStatus expected_pipeline_status, base::Time timeline_offset)
    {
    ...
    void InitializeDemuxerAndExpectPipelineStatus(media::PipelineStatus expected_pipeline_status)
    {
        InitializeDemuxerInternal(expected_pipeline_status, base::Time());
    }

    void SeekOnVideoTrackChangePassthrough(
        base::TimeDelta time, base::OnceCallback<void(const std::vector<DemuxerStream*>&)> cb, const std::vector<DemuxerStream*>& streams)
    {
    ...
    struct ReadExpectation {
        ReadExpectation(size_t size, int64_t timestamp_us, base::TimeDelta discard_front_padding, bool is_key_frame, DemuxerStream::Status status)
            : size(size)
    ...
    void OnReadDone(const base::Location& location, const ReadExpectation& read_expectation, base::OnceClosure quit_closure, DemuxerStream::Status status,
        DemuxerStream::DecoderBufferVector buffers)
    {
        // TODO(crbug.com/40232931): add multi read unit tests in next CL.
        DCHECK_LE(buffers.size(), 1u) << "FFmpegDemuxerTest only reads a single-buffer.";
        ...
        if (status == DemuxerStream::kOk) {
            ...
            EXPECT_EQ(read_expectation.timestamp_us, buffer->timestamp().InMicroseconds());
            EXPECT_EQ(read_expectation.discard_front_padding, buffer->discard_padding().first);
    ...
    DemuxerStream::ReadCB NewReadCBWithCheckedDiscard(const base::Location& location, int size, int64_t timestamp_us, base::TimeDelta discard_front_padding,
        bool is_key_frame, DemuxerStream::Status status, base::OnceClosure quit_closure)
    {
        EXPECT_CALL(*this, OnReadDoneCalled(size, timestamp_us));

        struct ReadExpectation read_expectation(size, timestamp_us, discard_front_padding, is_key_frame, status);

        return base::BindOnce(&FFmpegDemuxerTest::OnReadDone, base::Unretained(this), location, read_expectation, std::move(quit_closure));
    }

    void Read(DemuxerStream* stream, const base::Location& location, int size, int64_t timestamp_us, bool is_key_frame,
        DemuxerStream::Status status = DemuxerStream::Status::kOk, base::TimeDelta discard_front_padding = base::TimeDelta())
    {
        base::RunLoop run_loop;
        stream->Read(1, NewReadCBWithCheckedDiscard(location, size, timestamp_us, discard_front_padding, is_key_frame, status, run_loop.QuitClosure()));
        run_loop.Run();
    ...
    {
        return !!reinterpret_cast<FFmpegDemuxerStream*>(stream)->bitstream_converter_;
    }
#endif
    ...
    void ReadUntilEndOfStream(DemuxerStream* stream)
    {
        ...
        for (int i = 0; !got_eos_buffer && i < kMaxBuffers; i++) {
            base::RunLoop loop;
            stream->Read(1, base::BindOnce(&EosOnReadDone, &got_eos_buffer, loop.QuitWhenIdleClosure()));
            loop.Run();
        }
...
private:
    void CreateDemuxerInternal(const std::string& name, MediaLog* media_log)
    {
        ...
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb
            = base::BindRepeating(&FFmpegDemuxerTest::OnEncryptedMediaInitData, base::Unretained(this));

        Demuxer::MediaTracksUpdatedCB tracks_updated_cb = base::BindRepeating(&FFmpegDemuxerTest::OnMediaTracksUpdated, base::Unretained(this));

        demuxer_ = std::make_unique<FFmpegDemuxer>(
            base::SingleThreadTaskRunner::GetCurrentDefault(), data_source_.get(), encrypted_media_init_data_cb, tracks_updated_cb, media_log, false);
    }
    ...
    void CreateDataSource(const std::string& name)
    {
        ...
        EXPECT_TRUE(base::PathService::Get(base::DIR_SRC_TEST_DATA_ROOT, &file_path));
...
TEST_F(FFmpegDemuxerTest, Initialize_Successful)
{
    ...
    EXPECT_EQ(VideoDecoderConfig::AlphaMode::kIsOpaque, video_config.alpha_mode());
...
TEST_F(FFmpegDemuxerTest, AbortPendingReads)
{
    ...
    {
        base::RunLoop run_loop;
        audio->Read(1, NewReadCBWithCheckedDiscard(FROM_HERE, 29, 0, base::TimeDelta(), true, DemuxerStream::kAborted, run_loop.QuitClosure()));
...
TEST_F(FFmpegDemuxerTest, Seeking_PreferredStreamSelection)
{
    ...
    InitializeDemuxerWithTimelineOffset(base::Time::FromMillisecondsSinceUnixEpoch(kTimelineOffsetMs));

    FFmpegDemuxerStream* video = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::VIDEO));
    FFmpegDemuxerStream* audio = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::AUDIO));
...
TEST_F(FFmpegDemuxerTest, Read_VideoPositiveStartTime)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(kTimelineOffsetMs, demuxer_->GetTimelineOffset().InMillisecondsSinceUnixEpoch());
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOggDiscard_Sync)
{
    ...
    // Run the test twice with a seek in between.
    for (int i = 0; i < 2; ++i) {
        Read(audio, FROM_HERE, 1, 0, true, DemuxerStream::Status::kOk, base::Microseconds(2902));
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOpusDiscard_Sync)
{
    ...
    static const int kTestExpectations[][2] = { { 635, 0 }, { 594, 120000 }, { 597, 240000 }, { 591, 360000 }, { 582, 480000 }, { 583, 600000 },
        { 592, 720000 }, { 567, 840000 }, { 579, 960000 }, { 572, 1080000 }, { 583, 1200000 } };

    // Run the test twice with a seek in between.
    for (int i = 0; i < 2; ++i) {
        for (size_t j = 0; j < std::size(kTestExpectations); ++j) {
            Read(audio, FROM_HERE, kTestExpectations[j][0], kTestExpectations[j][1], true);
        }
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOpusDiscardH264Mp4_Sync)
{
    ...
    // Packet size to timestamp (in microseconds) mapping for the first N packets
    // which should be fully discarded.
    static const int kTestExpectations[][2] = { { 234, 20000 }, { 228, 40000 }, { 340, 60000 } };

    // Run the test twice with a seek in between.
    for (int i = 0; i < 2; ++i) {
        Read(audio, FROM_HERE, 408, 0, true, DemuxerStream::Status::kOk, base::Microseconds(6500));
...
TEST_F(FFmpegDemuxerTest, Read_AudioVideoNegativeStartTime)
{
    ...
    Read(audio, FROM_HERE, 10, 23220, true, DemuxerStream::Status::kOk, kInfiniteDuration);

    // The rest are all similar, just verify that discard padding is correct.
    base::RunLoop run_loop;
    audio->Read(41, base::BindLambdaForTesting([&](DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers) {
        for (const auto& buffer : buffers) {
            EXPECT_EQ(buffer->discard_padding().first, kInfiniteDuration);
        }
    ...
    Read(video, FROM_HERE, 26791, -66733, true, DemuxerStream::Status::kOk, kInfiniteDuration);
    Read(video, FROM_HERE, 4233, 33367, false);
    Read(video, FROM_HERE, 3257, 0, false);
    Read(video, FROM_HERE, 2467, -33367, false, DemuxerStream::Status::kOk, kInfiniteDuration);
...
TEST_F(FFmpegDemuxerTest, Read_AudioNegativeStartTimeAndOpusSfxDiscard_Sync)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        Read(audio, FROM_HERE, 314, 0, true, DemuxerStream::Status::kOk, i == 0 ? base::Microseconds(6500) : base::TimeDelta());
...
TEST_F(FFmpegDemuxerTest, Read_DiscardDisabledVideoStream)
{
    ...
    static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::VIDEO))->SetEnabled(false, base::TimeDelta());
...
class Mp3SeekFFmpegDemuxerTest : public FFmpegDemuxerTest, public testing::WithParamInterface<const char*> { };
...
INSTANTIATE_TEST_SUITE_P(All, Mp3SeekFFmpegDemuxerTest,
    ::testing::Values("bear-audio-10s-CBR-has-TOC.mp3", "bear-audio-10s-CBR-no-TOC.mp3", "bear-audio-10s-VBR-has-TOC.mp3", "bear-audio-10s-VBR-no-TOC.mp3"));

#if BUILDFLAG(USE_PROPRIETARY_CODECS)
static void ValidateAnnexB(DemuxerStream* stream, base::OnceClosure quit_closure, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
    ...
    bool is_valid = mp4::AVC::AnalyzeAnnexB(buffer->data(), buffer->size(), subsamples).is_conformant.value_or(false);
    ...
    stream->Read(1, base::BindOnce(&ValidateAnnexB, stream, std::move(quit_closure)));
}

TEST_F(FFmpegDemuxerTest, IsValidAnnexB)
{
    const char* files[] = { "bear-1280x720-av_frag.mp4", "bear-1280x720-av_with-aud-nalus_frag.mp4" };
    ...
    for (size_t i = 0; i < std::size(files); ++i) {
        ...
        base::RunLoop loop;
        stream->Read(1, base::BindOnce(&ValidateAnnexB, stream, loop.QuitWhenIdleClosure()));
        loop.Run();
...
TEST_F(FFmpegDemuxerTest, XHE_AAC)
{
    ...
    EXPECT_EQ(audio->audio_decoder_config().profile(), AudioCodecProfile::kXHE_AAC);
...
{
    base::Time result;
    EXPECT_TRUE(base::Time::FromUTCString("2012-11-10T12:34:56.987654Z", &result));
    ...
    EXPECT_EQ("2012-11-10T12:34:56.987Z", base::TimeFormatAsIso8601(without_fractional_ms));
...
TEST_F(FFmpegDemuxerTest, UTCDateToTime_Invalid)
{
    ...
    for (size_t i = 0; i < std::size(invalid_date_strings); ++i) {
        ...
        base::Time result;
        EXPECT_FALSE(base::Time::FromUTCString(date_string, &result)) << "date_string '" << date_string << "'";
    }
}

static void VerifyFlacStream(DemuxerStream* stream, int expected_bytes_per_channel, ChannelLayout expected_channel_layout, int expected_samples_per_second,
...
TEST_F(FFmpegDemuxerTest, Read_Flac)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 4, CHANNEL_LAYOUT_MONO, 44100, kSampleFormatS32);
}
...
TEST_F(FFmpegDemuxerTest, Read_Flac_Mp4)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 4, CHANNEL_LAYOUT_STEREO, 44100, kSampleFormatS32);
}
...
TEST_F(FFmpegDemuxerTest, Read_Flac_192kHz_Mp4)
{
    ...
    VerifyFlacStream(GetStream(DemuxerStream::AUDIO), 4, CHANNEL_LAYOUT_STEREO, 192000, kSampleFormatS32);
}
...
TEST_F(FFmpegDemuxerTest, Seek_FallbackToDisabledVideoStream)
{
    ...
    FFmpegDemuxerStream* vstream = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::VIDEO));
...
TEST_F(FFmpegDemuxerTest, Seek_FallbackToDisabledAudioStream)
{
    CreateDemuxer("bear-320x240-audio-only.webm");
    InitializeDemuxer();
    FFmpegDemuxerStream* astream = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::AUDIO));
...
void DisableAndEnableDemuxerTracks(FFmpegDemuxer* demuxer, base::test::TaskEnvironment* task_environment)
{
...
void OnReadDoneExpectEos(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
{
    EXPECT_EQ(status, DemuxerStream::kOk);
    EXPECT_EQ(buffers.size(), 1u);
...
TEST_F(FFmpegDemuxerTest, StreamStatusNotifications)
{
    CreateDemuxer("bear-320x240.webm");
    InitializeDemuxer();
    FFmpegDemuxerStream* audio_stream = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::AUDIO));
    EXPECT_NE(nullptr, audio_stream);
    FFmpegDemuxerStream* video_stream = static_cast<FFmpegDemuxerStream*>(GetStream(DemuxerStream::VIDEO));
...
TEST_F(FFmpegDemuxerTest, MultitrackMemoryUsage)
{
    ...
    std::vector<DemuxerStream*> streams = demuxer_->GetAllStreams();
    for (media::DemuxerStream* stream : streams) {
        static_cast<FFmpegDemuxerStream*>(stream)->SetEnabled(true, base::TimeDelta());
    }
...
TEST_F(FFmpegDemuxerTest, SeekOnVideoTrackChangeWontSeekIfEmpty)
{
    ...
    SeekOnVideoTrackChangePassthrough(base::TimeDelta(), base::BindOnce(QuitLoop, loop.QuitClosure()), streams);
