...
const struct TestParams kTestParams[] = { { "test-25fps.vp9", 0, 8, 320, 240, true, 9, 65, 120, Vp9InterpolationFilter::EIGHTTAP, 48, 11 },
    { "test-25fps.vp9_2", 2, 10, 320, 240, true, 8, 79, 115, Vp9InterpolationFilter::SWITCHABLE, 46, 10 } };
...
protected:
    ...
    void Initialize(const std::string& filename, bool parsing_compressed_header, bool needs_external_context_update)
    {
        ...
        ASSERT_TRUE(ivf_parser_.Initialize(stream_->data(), stream_->length(), &ivf_file_header));
        ASSERT_EQ(ivf_file_header.fourcc, 0x30395056u); // VP90

        vp9_parser_ = std::make_unique<Vp9Parser>(parsing_compressed_header, needs_external_context_update);

        if (parsing_compressed_header) {
            base::FilePath context_path = GetTestDataFilePath(filename + ".context");
            context_file_.Initialize(context_path, base::File::FLAG_OPEN | base::File::FLAG_READ);
    ...
    {
        ASSERT_EQ(static_cast<int>(sizeof(*frame_context)), context_file_.ReadAtCurrentPos(reinterpret_cast<char*>(frame_context), sizeof(*frame_context)));
    }

    Vp9Parser::Result ParseNextFrame(struct Vp9FrameHeader* frame_hdr);
    void CheckSubsampleValues(
        const uint8_t* superframe, size_t framesize, std::unique_ptr<DecryptConfig> config, std::vector<std::unique_ptr<DecryptConfig>>& expected_split);
    ...
    Vp9Parser::ContextRefreshCallback GetContextRefreshCb(const Vp9FrameHeader& frame_hdr) const
    {
...
Vp9Parser::Result Vp9ParserTest::ParseNextFrame(Vp9FrameHeader* fhdr)
{
    while (true) {
        ...
        Vp9Parser::Result res = vp9_parser_->ParseNextFrame(fhdr, &allocate_size, &null_config);
    ...
    const uint8_t* superframe, size_t framesize, std::unique_ptr<DecryptConfig> config, std::vector<std::unique_ptr<DecryptConfig>>& expected_split)
{
    ...
    for (auto& expected : expected_split) {
        std::unique_ptr<DecryptConfig> actual = vp9_parser_->NextFrameDecryptContextForTesting();
...
TEST_F(Vp9ParserTest, AlignedFrameSubsampleParsing)
{
    ...
    const uint8_t kSuperframe[] = { // First frame; 32 bytes.
    ...
    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16) }, absl::nullopt));

    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kIVIncrementOne, { SubsampleEntry(16, 16) }, absl::nullopt));

    CheckSubsampleValues(
        kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(16, 16) }), expected);
}
...
TEST_F(Vp9ParserTest, UnalignedFrameSubsampleParsing)
{
    ...
    std::vector<std::unique_ptr<DecryptConfig>> expected;
    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(32, 0) }, absl::nullopt));

    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16) }, absl::nullopt));

    CheckSubsampleValues(kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(48, 16) }), expected);
}
...
TEST_F(Vp9ParserTest, ClearSectionRollsOverSubsampleParsing)
{
    ...
    const uint8_t superframe_marker_byte = make_marker_byte(true, 2);
    const uint8_t kSuperframe[] = { // First frame; 48 bytes.
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    std::vector<std::unique_ptr<DecryptConfig>> expected;
    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(16, 0) }, absl::nullopt));

    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kIVIncrementOne, { SubsampleEntry(16, 16) }, absl::nullopt));

    CheckSubsampleValues(
        kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(32, 16) }), expected);
}
...
TEST_F(Vp9ParserTest, FirstFrame2xSubsampleParsing)
{
    ...
    const uint8_t superframe_marker_byte = make_marker_byte(true, 2);
    const uint8_t kSuperframe[] = { // First frame; 64 bytes.
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    std::vector<std::unique_ptr<DecryptConfig>> expected;
    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(16, 16) }, absl::nullopt));

    expected.push_back(DecryptConfig::CreateCbcsConfig(kKeyID, kIVIncrementTwo, { SubsampleEntry(16, 16) }, absl::nullopt));

    CheckSubsampleValues(kSuperframe, sizeof(kSuperframe),
        DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(16, 16), SubsampleEntry(16, 16) }), expected);
}
...
TEST_F(Vp9ParserTest, UnalignedBigFrameSubsampleParsing)
{
    ...
    const uint8_t superframe_marker_byte = make_marker_byte(true, 2);
    const uint8_t kSuperframe[] = { // First frame; 72 bytes.
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    std::vector<std::unique_ptr<DecryptConfig>> expected;
    expected.push_back(
        DecryptConfig::CreateCbcsConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 16), SubsampleEntry(16, 16), SubsampleEntry(8, 0) }, absl::nullopt));
...
TEST_F(Vp9ParserTest, UnalignedInvalidSubsampleParsing)
{
    ...
    vp9_parser_->SetStream(kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(16, 32) }));
...
TEST_F(Vp9ParserTest, CipherBytesCoverSuperframeMarkerSubsampleParsing)
{
    ...
    const uint8_t kSuperframe[] = { // First frame; 44 bytes.
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    vp9_parser_->SetStream(kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(0, 48) }));
...
TEST_F(Vp9ParserTest, ClearBytesCoverSuperframeMarkerSubsampleParsing)
{
    ...
    vp9_parser_->SetStream(kSuperframe, sizeof(kSuperframe), DecryptConfig::CreateCencConfig(kKeyID, kInitialIV, { SubsampleEntry(48, 0) }));
...
TEST_F(Vp9ParserTest, TestIncrementIV)
{
    std::vector<std::tuple<char const*, uint32_t, char const*>> input_output
        = { { "--------aaaaaaaa", 1, "--------aaaaaaab" }, { "--------aaaaaaa\377", 1, "--------aaaaaab\0" }, { "--------aaaaaaa\377", 2, "--------aaaaaab\1" },
              { "--------\377\377\377\377\377\377\377\377", 2, "--------\0\0\0\0\0\0\0\1" } };

    for (auto& testcase : input_output) {
        EXPECT_EQ(vp9_parser_->IncrementIVForTesting(std::string(std::get<0>(testcase), 16), std::get<1>(testcase)), std::string(std::get<2>(testcase), 16));
    }
}
...
TEST_F(Vp9ParserTest, StreamFileParsingWithoutCompressedHeader)
{
    ...
    DVLOG(1) << "Number of successfully parsed frames before EOS: " << num_parsed_frames;
...
TEST_F(Vp9ParserTest, StreamFileParsingWithCompressedHeader)
{
    ...
    while (num_parsed_frames < num_expected_frames * 2) {
        ...
        EXPECT_TRUE(memcmp(&frame_context, &fhdr.initial_frame_context, sizeof(frame_context)) == 0);
        ReadContext(&frame_context);
        EXPECT_TRUE(memcmp(&frame_context, &fhdr.frame_context, sizeof(frame_context)) == 0);
...
TEST_P(Vp9ParserTest, VerifyFirstFrame)
{
    ...
    EXPECT_EQ(GetParam().frame_parallel_decoding_mode, fhdr.frame_parallel_decoding_mode);
    ...
    EXPECT_EQ(GetParam().first_frame_header_size_bytes, fhdr.header_size_in_bytes);
    ...
    EXPECT_EQ(GetParam().second_frame_header_size_bytes, fhdr.header_size_in_bytes);
    EXPECT_EQ(GetParam().second_frame_uncompressed_header_size_bytes, fhdr.uncompressed_header_size);
}
