...
public:
    VideoRendererAlgorithm(const TimeSource::WallClockTimeCB& wall_clock_time_cb, MediaLog* media_log);
    ...
    scoped_refptr<VideoFrame> Render(base::TimeTicks deadline_min, base::TimeTicks deadline_max, size_t* frames_dropped);
    ...
    size_t effective_frames_queued() const
    {
        return effective_frames_queued_;
    }
    ...
    const VideoFrame& last_frame() const
    {
        return *frame_queue_.back().frame;
    }

    // Current render interval.
    base::TimeDelta render_interval() const
    {
    ...
    void disable_frame_dropping()
    {
        frame_dropping_disabled_ = true;
    }
...
private:
    ...
    void AccountForMissedIntervals(base::TimeTicks deadline_min, base::TimeTicks deadline_max);
    ...
    int FindBestFrameByCoverage(base::TimeTicks deadline_min, base::TimeTicks deadline_max, int* second_best) const;
    ...
    int FindBestFrameByDrift(base::TimeTicks deadline_min, base::TimeDelta* selected_frame_drift) const;
    ...
    // |start_time|.
    base::TimeDelta CalculateAbsoluteDriftForFrame(base::TimeTicks deadline_min, int frame_index) const;
