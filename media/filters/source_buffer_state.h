...
public:
    ...
    using CreateDemuxerStreamCB = base::RepeatingCallback<ChunkDemuxerStream*(DemuxerStream::Type)>;

    SourceBufferState(std::unique_ptr<StreamParser> stream_parser, std::unique_ptr<FrameProcessor> frame_processor,
    ...
    void Init(StreamParser::InitCB init_cb, std::optional<std::string_view> expected_codecs,
    ...
    void ChangeType(std::unique_ptr<StreamParser> new_stream_parser, const std::string& new_expected_codecs);
    ...
    [[nodiscard]] bool AppendChunks(std::unique_ptr<StreamParser::BufferQueue> buffer_queue, base::TimeDelta append_window_start,
        base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset);

    // Aborts the current append sequence and resets the parser.
    void ResetParserState(base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::TimeDelta* timestamp_offset);
    ...
    bool parsing_media_segment() const
    {
        return parsing_media_segment_;
    }
    ...
    Ranges<base::TimeDelta> GetBufferedRanges(base::TimeDelta duration, bool ended) const;
    ...
    static Ranges<base::TimeDelta> ComputeRangesIntersection(const RangesList& active_ranges, bool ended);
...
private:
    ...
    // PARSER_INITIALIZED.)
    enum State { UNINITIALIZED = 0, PENDING_PARSER_CONFIG, PENDING_PARSER_INIT, PARSER_INITIALIZED, PENDING_PARSER_RECONFIG, PENDING_PARSER_REINIT };
    ...
    void OnEncryptedMediaInitData(EmeInitDataType type, const std::vector<uint8_t>& init_data);
