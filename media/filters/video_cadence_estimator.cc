...
static void HistogramCadenceChangeCount(int cadence_changes)
{
    ...
    UMA_HISTOGRAM_CUSTOM_COUNTS("Media.VideoRenderer.CadenceChanges", cadence_changes, 1, kCadenceChangeMax, kCadenceChangeMax);
}
...
VideoCadenceEstimator::Cadence ConstructSimpleCadence(double perfect_cadence, base::TimeDelta max_acceptable_drift, base::TimeDelta* time_until_max_drift)
{
...
VideoCadenceEstimator::VideoCadenceEstimator(base::TimeDelta minimum_time_until_max_drift)
    : cadence_hysteresis_threshold_(base::Milliseconds(kMinimumCadenceDurationMs))
...
bool VideoCadenceEstimator::UpdateCadenceEstimate(
    base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta frame_duration_deviation, base::TimeDelta max_acceptable_drift)
{
    ...
    if (last_render_interval_ > base::TimeDelta()
        && (last_render_interval_ - render_interval).magnitude() > last_render_interval_ * kRenderIntervalChangeThreshold) {
    ...
    Cadence new_cadence = CalculateCadence(render_interval, frame_duration, max_acceptable_drift, &time_until_max_drift);
    ...
    DVLOG(2) << "Hysteresis prevented cadence switch: " << CadenceToString(cadence_) << " -> " << CadenceToString(new_cadence);
...
void VideoCadenceEstimator::UpdateCadenceInternal(Cadence new_cadence, base::TimeDelta time_until_max_drift)
{
    DVLOG(1) << "Cadence switch: " << CadenceToString(cadence_) << " -> " << CadenceToString(new_cadence)
...
bool VideoCadenceEstimator::HasSimpleCadence(base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta minimum_time_until_max_drift)
{
    ...
    base::TimeDelta time_until_max_drift;
    auto cadence = ConstructSimpleCadence(perfect_cadence, render_interval, &time_until_max_drift);
    return !cadence.empty() && time_until_max_drift >= minimum_time_until_max_drift;
}

VideoCadenceEstimator::Cadence VideoCadenceEstimator::CalculateCadence(
    base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta max_acceptable_drift, base::TimeDelta* time_until_max_drift) const
{
    ...
    if (max_acceptable_drift >= minimum_time_until_max_drift_) {
        return ConstructSimpleCadence(perfect_cadence, max_acceptable_drift, time_until_max_drift);
    }
    ...
    const double drift_ratio = max_acceptable_drift / minimum_time_until_max_drift_;
    ...
    if (!best_n) {
        Cadence cadence = ConstructSimpleCadence(perfect_cadence, max_acceptable_drift, time_until_max_drift);
        if (!cadence.empty() && *time_until_max_drift >= minimum_time_until_max_drift_) {
            return cadence;
        }
        return Cadence();
    }
...
std::string VideoCadenceEstimator::CadenceToString(const Cadence& cadence) const
{
    if (cadence.empty())
        return std::string("[]");
    ...
    os << "[";
    std::copy(cadence.begin(), cadence.end() - 1, std::ostream_iterator<int>(os, ":"));
    os << cadence.back() << "]";
    return os.str();
}
