...
VideoPixelFormat Libgav1ImageFormatToVideoPixelFormat(const libgav1::ImageFormat libgav1_format, int bitdepth)
{
...
libgav1::StatusCode GetFrameBufferImpl(void* callback_private_data, int bitdepth, libgav1::ImageFormat image_format, int width, int height, int left_border,
    int right_border, int top_border, int bottom_border, int stride_alignment, Libgav1FrameBuffer* frame_buffer)
{
    ...
    CHECK_LE(static_cast<size_t>(stride_alignment), VideoFrame::kFrameAddressAlignment);

    const VideoPixelFormat format = Libgav1ImageFormatToVideoPixelFormat(image_format, bitdepth);
    ...
    gfx::Size coded_size(left_border + width + right_border, top_border + height + bottom_border);
    gfx::Rect visible_rect(left_border, top_border, width, height);

    auto* decoder = static_cast<Gav1VideoDecoder*>(callback_private_data);
    auto video_frame = decoder->CreateVideoFrame(format, coded_size, visible_rect);
    if (!video_frame)
    ...
    if (image_format == libgav1::kImageFormatMonochrome400) {
        ...
        for (int i = 1; i < 3; i++) {
            ...
            if (bitdepth == 8) {
                ...
                uint16_t* data = reinterpret_cast<uint16_t*>(video_frame->GetWritableVisibleData(i));
...
void ReleaseFrameBufferImpl(void* callback_private_data, void* buffer_private_data)
{
    DCHECK(callback_private_data);
...
void ReleaseInputBufferImpl(void* callback_private_data, void* buffer_private_data)
{
    DCHECK(callback_private_data);
    DCHECK(buffer_private_data);
...
scoped_refptr<VideoFrame> FormatVideoFrame(const libgav1::DecoderBuffer& buffer, const VideoColorSpace& container_color_space)
{
    scoped_refptr<VideoFrame> frame = static_cast<VideoFrame*>(buffer.buffer_private_data);
    ...
    media::VideoColorSpace color_space(buffer.color_primary, buffer.transfer_characteristics, buffer.matrix_coefficients,
        buffer.color_range == libgav1::kColorRangeStudio ? gfx::ColorSpace::RangeID::LIMITED : gfx::ColorSpace::RangeID::FULL);
...
Gav1VideoDecoder::Gav1VideoDecoder(MediaLog* media_log, OffloadState offload_state)
...
void Gav1VideoDecoder::Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* /* cdm_context */, InitCB init_cb, const OutputCB& output_cb,
    const WaitingCB& /* waiting_cb */)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(config.IsValidConfig());

    InitCB bound_init_cb = bind_callbacks_ ? BindToCurrentLoop(std::move(init_cb)) : std::move(init_cb);
    ...
    settings.threads = VideoDecoder::GetRecommendedThreadCount(GetDecoderThreadCounts(config.coded_size().height()));
...
void Gav1VideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(buffer);
    DCHECK(decode_cb);
    ...
    DecodeCB bound_decode_cb = bind_callbacks_ ? BindToCurrentLoop(std::move(decode_cb)) : std::move(decode_cb);
...
bool Gav1VideoDecoder::DecodeBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    ...
    while (is_end_of_stream || !enqueued) {
        ...
        if (!enqueued) {
            libgav1::StatusCode status = libgav1_decoder_->EnqueueFrame(buffer->data(), buffer->data_size(),
            ...
            if (status == kLibgav1StatusOk) {
                ...
                MEDIA_LOG(ERROR, media_log_) << "libgav1::Decoder::EnqueueFrame failed, status=" << status << " on " << buffer->AsHumanReadableString();
                return false;
            }
        }
        ...
        libgav1::StatusCode status = libgav1_decoder_->DequeueFrame(&decoded_buffer);
        if (status != kLibgav1StatusOk) {
            if (status != kLibgav1StatusTryAgain && status != kLibgav1StatusNothingToDequeue) {
                MEDIA_LOG(ERROR, media_log_) << "libgav1::Decoder::DequeueFrame failed, status=" << status;
                return false;
            }
        ...
        scoped_refptr<VideoFrame> frame = FormatVideoFrame(*decoded_buffer, color_space_);
        if (!frame) {
...
scoped_refptr<VideoFrame> Gav1VideoDecoder::CreateVideoFrame(VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect)
{
    ...
    return frame_pool_.CreateFrame(format, coded_size, visible_rect, aspect_ratio_.GetNaturalSize(visible_rect), kNoTimestamp);
}
