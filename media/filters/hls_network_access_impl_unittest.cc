...
public:
    ...
    void RecreateImpl()
    {
        ...
        base::SequenceBound<HlsDataSourceProviderImpl> dsp(task_environment_.GetMainThreadTaskRunner(), std::move(factory));
        network_access_ = std::make_unique<HlsNetworkAccessImpl>(std::move(dsp));
    }

    std::optional<hls::types::ByteRange> ByteRangeFromTuple(std::optional<std::tuple<uint64_t, uint64_t>> tuple)
    {
        if (tuple.has_value()) {
            return hls::types::ByteRange::Validate(std::get<0>(*tuple), std::get<1>(*tuple));
        }
        return std::nullopt;
    }

    scoped_refptr<hls::MediaSegment> MakeSegment(std::optional<std::tuple<uint64_t, uint64_t>> byte_range,
        std::optional<std::tuple<uint64_t, uint64_t>> init_br, InitMode init_mode = InitMode::kAbsent, KeyMode key_mode = KeyMode::kAbsent)
    {
        ...
        if (init_mode == InitMode::kPresent) {
            init = base::MakeRefCounted<hls::MediaSegment::InitializationSegment>(GURL("https://foo.com"), ByteRangeFromTuple(init_br));
        }
        if (key_mode == KeyMode::kPresent) {
            enc_data = base::MakeRefCounted<hls::MediaSegment::EncryptionData>(
                GURL("https://example.com/enc.key"), hls::XKeyTagMethod::kAES128, hls::XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 0));
        }
        return base::MakeRefCounted<hls::MediaSegment>(base::Seconds(1), 0, 0, GURL("https://example.com"), std::move(init), std::move(enc_data),
            ByteRangeFromTuple(byte_range), std::nullopt, false, false, init_mode == InitMode::kPresent, false);
    }
...
TEST_F(HlsNetworkAccessImplUnittest, TestReadSmallManifest)
{
    ...
    network_access_->ReadManifest(GURL("example.com"), base::BindOnce([](HlsDataSourceProvider::ReadResult result) {
...
TEST_F(HlsNetworkAccessImplUnittest, TestReadSimpleSegment)
{
    ...
    network_access_->ReadMediaSegment(*segment, /*read_chunked=*/false, /*include_init=*/true, base::BindOnce([&](HlsDataSourceProvider::ReadResult result) {
        ASSERT_TRUE(result.has_value());
        auto stream = std::move(result).value();
...
{
    auto segment = MakeSegment(std::nullopt, std::make_tuple(100000, 100), InitMode::kPresent);
    factory_->AddReadExpectation(100, 16384, 16384);

    network_access_->ReadMediaSegment(*segment, /*read_chunked=*/true, /*include_init=*/true, base::BindOnce([&](HlsDataSourceProvider::ReadResult result) {
        ASSERT_TRUE(result.has_value());
        auto stream = std::move(result).value();
...
{
    auto segment = MakeSegment(std::nullopt, std::nullopt, InitMode::kAbsent, KeyMode::kPresent);

    // This actually has to be 16 non-zero bytes.
    auto* ds_for_keyfetch = factory_->PregenerateNextMock();
    EXPECT_CALL(*ds_for_keyfetch, Initialize).WillOnce(base::test::RunOnceCallback<0>(true));
    EXPECT_CALL(*ds_for_keyfetch, Read(0, 16384, _, _)).WillOnce([](int64_t, int, uint8_t* data, DataSource::ReadCB cb) {
        memset(data, 'x', 16);
        std::move(cb).Run(16);
    });
    EXPECT_CALL(*ds_for_keyfetch, Read(16, 16384, _, _)).WillOnce(base::test::RunOnceCallback<3>(0));
    ...
    network_access_->ReadMediaSegment(*segment, /*read_chunked=*/false, /*include_init=*/true,
        base::BindOnce(
            [&](scoped_refptr<hls::MediaSegment> segment, HlsDataSourceProvider::ReadResult result) {
                ASSERT_TRUE(result.has_value());
                auto stream = std::move(result).value();
                ASSERT_EQ(stream->read_position(), 1000lu);
                ASSERT_EQ(stream->buffer_size(), 1000lu);
