...
template <DemuxerStream::Type StreamType> class DecoderStreamTraits { };

enum class PostDecodeAction { DELIVER, DROP };

template <> class MEDIA_EXPORT DecoderStreamTraits<DemuxerStream::AUDIO> {
public:
    ...
    static const MediaLogProperty kDecoderName = MediaLogProperty::kAudioDecoderName;
    static const MediaLogProperty kIsPlatformDecoder = MediaLogProperty::kIsPlatformAudioDecoder;
    static const MediaLogProperty kIsDecryptingDemuxerStream = MediaLogProperty::kIsAudioDecryptingDemuxerStream;

    static std::string ToString();
    static bool NeedsBitstreamConversion(DecoderType* decoder);
    static scoped_refptr<OutputType> CreateEOSOutput();

    DecoderStreamTraits(MediaLog* media_log, ChannelLayout initial_hw_layout, SampleFormat initial_hw_sample_format);
    ...
    void InitializeDecoder(DecoderType* decoder, const DecoderConfigType& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb,
        const OutputCB& output_cb, const WaitingCB& waiting_cb);
    void OnDecoderInitialized(DecoderType* decoder, InitCB cb, DecoderStatus status);
...
private:
    ...
    base::WeakPtrFactory<DecoderStreamTraits<DemuxerStream::AUDIO>> weak_factory_ { this };
};

template <> class MEDIA_EXPORT DecoderStreamTraits<DemuxerStream::VIDEO> {
public:
    ...
    static const MediaLogProperty kDecoderName = MediaLogProperty::kVideoDecoderName;
    static const MediaLogProperty kIsPlatformDecoder = MediaLogProperty::kIsPlatformVideoDecoder;
    static const MediaLogProperty kIsDecryptingDemuxerStream = MediaLogProperty::kIsVideoDecryptingDemuxerStream;
...
private:
    ...
    base::MovingAverage<base::TimeDelta, base::TimeDelta> keyframe_distance_average_;
    ...
    base::WeakPtrFactory<DecoderStreamTraits<DemuxerStream::VIDEO>> weak_factory_ { this };
};
