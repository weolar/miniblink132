...
{
    return VideoDecodeStatsDB::VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE3, gfx::Size(1024, 768), 60, "com.widevine.alpha", false);
}
...
public:
    ...
    MOCK_METHOD3(AppendDecodeStats, void(const VideoDescKey& key, const DecodeStatsEntry& entry, AppendDecodeStatsCB append_done_cb));
    MOCK_METHOD2(GetDecodeStats, void(const VideoDescKey& key, GetDecodeStatsCB get_stats_cb));
...
template <bool WithSeedDB> class InMemoryDBTestBase : public testing::Test {
public:
    InMemoryDBTestBase()
        ...
        , in_memory_db_(new InMemoryVideoDecodeStatsDBImpl(db_provider_.get()))
    {
        ...
        if (db_provider_) {
            ...
            ON_CALL(*db_provider_, GetVideoDecodeStatsDB(_)).WillByDefault([&](GetCB cb) { std::move(cb).Run(seed_db_.get()); });
        }
    ...
    void InitializeEmptyDB()
    {
        ...
        in_memory_db_->Initialize(base::BindOnce(&InMemoryDBTestBase::InitializeCB, base::Unretained(this)));
        task_environment_.RunUntilIdle();
    }

    MOCK_METHOD1(InitializeCB, void(bool success));
    MOCK_METHOD1(AppendDecodeStatsCB, void(bool success));
    MOCK_METHOD2(GetDecodeStatsCB, void(bool success, std::unique_ptr<VideoDecodeStatsDB::DecodeStatsEntry> entry));
...
TEST_F(SeedlessInMemoryDBTest, ReadExpectingEmpty)
{
    ...
    in_memory_db_->GetDecodeStats(kTestKey(), base::BindOnce(&InMemoryDBTestBase::GetDecodeStatsCB, base::Unretained(this)));
...
TEST_F(SeededInMemoryDBTest, ReadExpectingEmpty)
{
    ...
    EXPECT_CALL(*seed_db_, GetDecodeStats(Eq(kTestKey()), _)).WillOnce([](const auto& key, auto get_cb) { std::move(get_cb).Run(true, nullptr); });
...
TEST_F(SeededInMemoryDBTest, ReadExpectingSeedData)
{
    ...
    EXPECT_CALL(*seed_db_, GetDecodeStats(Eq(kTestKey()), _)).WillOnce([&](const auto& key, auto get_cb) {
        std::move(get_cb).Run(true, std::make_unique<DecodeStatsEntry>(seed_entry));
    });
...
TEST_F(SeededInMemoryDBTest, AppendReadAndClear)
{
    ...
    ON_CALL(*seed_db_, GetDecodeStats(Eq(kTestKey()), _)).WillByDefault([&](const auto& key, auto get_cb) {
        std::move(get_cb).Run(true, std::make_unique<DecodeStatsEntry>(seed_entry));
    });
    ...
    in_memory_db_->AppendDecodeStats(kTestKey(), seed_entry, base::BindOnce(&InMemoryDBTestBase::AppendDecodeStatsCB, base::Unretained(this)));
    ...
    in_memory_db_->ClearStats(base::BindOnce(&InMemoryDBTestBase::ClearStatsCB, base::Unretained(this)));
...
TEST_F(SeedlessInMemoryDBTest, AppendReadAndClear)
{
    ...
    EXPECT_CALL(*this, AppendDecodeStatsCB(true));
    in_memory_db_->AppendDecodeStats(kTestKey(), entry, base::BindOnce(&InMemoryDBTestBase::AppendDecodeStatsCB, base::Unretained(this)));
...
TEST_F(SeededInMemoryDBTest, ProvidedNullSeedDB)
{
    ...
    EXPECT_CALL(*db_provider_, GetVideoDecodeStatsDB(_)).WillOnce([](auto get_db_cb) { std::move(get_db_cb).Run(nullptr); });
...
TEST_F(SeededInMemoryDBTest, SeedReadFailureOnGettingStats)
{
    ...
    ON_CALL(*seed_db_, GetDecodeStats(_, _)).WillByDefault([](const auto& key, auto get_cb) { std::move(get_cb).Run(false, nullptr); });
