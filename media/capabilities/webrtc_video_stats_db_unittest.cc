...
const auto ParsePixelsFromKey = WebrtcVideoStatsDB::VideoDescKey::ParsePixelsFromKey;
...
TEST(WebrtcVideoStatsDBTest, OperatorEquals)
{
    ...
    EXPECT_NE(keyA,
        MakeKey(/*is_decode_stats=*/false, VP9PROFILE_PROFILE0,
            /*hardware_accelerated=*/true, 1280 * 720));
    EXPECT_NE(keyA,
        MakeKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE2,
            /*hardware_accelerated=*/true, 1280 * 720));
    EXPECT_NE(keyA,
        MakeKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE0,
...
TEST(WebrtcVideoStatsDBTest, PixelSizeBucketting)
{
    ...
    for (int pixel_size_delta = -10000; pixel_size_delta <= 10000; pixel_size_delta += 1000) {
        EXPECT_EQ(keyA,
            MakeKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE0,
                /*hardware_accelerated=*/true, 1280 * 720 + pixel_size_delta));
    }
...
TEST(WebrtcVideoStatsDBTest, CodecProfileBucketting)
{
    ...
    for (int i = H264PROFILE_MIN; i <= H264PROFILE_MAX; ++i) {
        EXPECT_EQ(keyA,
            MakeKey(/*is_decode_stats=*/true, static_cast<VideoCodecProfile>(i),
                /*hardware_accelerated=*/true, 1280 * 720));
    }
