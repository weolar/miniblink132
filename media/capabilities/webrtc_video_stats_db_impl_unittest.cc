...
public:
    ...
    WebrtcVideoStatsDBImplTest()
        : kDecodeStatsKeyVp9(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE3,
            /*hardware_accelerated=*/false, 1280 * 720))
        , kDecodeStatsKeyVp9Hw(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE3,
              /*hardware_accelerated=*/true, 1280 * 720))
        , kDecodeStatsKeyVp9FullHd(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE3,
              /*hardware_accelerated=*/false, 1920 * 1080))
        , kDecodeStatsKeyVp94K(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/true, VP9PROFILE_PROFILE3,
              /*hardware_accelerated=*/false, 3840 * 2160))
        , kDecodeStatsKeyH264(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/true, H264PROFILE_MIN,
              /*hardware_accelerated=*/false, 1920 * 1080))
        , kEncodeStatsKeyVp9(VideoDescKey::MakeBucketedKey(/*is_decode_stats=*/false, VP9PROFILE_PROFILE3,
              /*hardware_accelerated=*/false, 1280 * 720))
    {
        // Fake DB simply wraps a std::map with the LevelDB interface. We own the
        // map and will delete it in TearDown().
        fake_db_map_ = std::make_unique<FakeDB<WebrtcVideoStatsEntryProto>::EntryMap>();
        ...
        stats_db_ = base::WrapUnique(new WebrtcVideoStatsDBImpl(std::unique_ptr<FakeDB<WebrtcVideoStatsEntryProto>>(fake_db_)));
    }

    WebrtcVideoStatsDBImplTest(const WebrtcVideoStatsDBImplTest&) = delete;
    WebrtcVideoStatsDBImplTest& operator=(const WebrtcVideoStatsDBImplTest&) = delete;
    ...
    void InitializeDB()
    {
        stats_db_->Initialize(base::BindOnce(&WebrtcVideoStatsDBImplTest::OnInitialize, base::Unretained(this)));
    ...
    void AppendStats(const VideoDescKey& key, const VideoStats& entry)
    {
        ...
        stats_db_->AppendVideoStats(key, entry, base::BindOnce(&WebrtcVideoStatsDBImplTest::MockAppendVideoStatsCb, base::Unretained(this)));
    ...
    void VerifyReadStats(const VideoDescKey& key, const VideoStatsEntry& expected)
    {
        EXPECT_CALL(*this, MockGetVideoStatsCb(true, std::make_optional(expected)));
        stats_db_->GetVideoStats(key, base::BindOnce(&WebrtcVideoStatsDBImplTest::MockGetVideoStatsCb, base::Unretained(this)));
        VerifyOnePendingOp("Read");
        fake_db_->GetCallback(true);
        testing::Mock::VerifyAndClearExpectations(this);
    }

    void VerifyReadStatsCollection(const VideoDescKey& key, const WebrtcVideoStatsDB::VideoStatsCollection& expected)
    {
        EXPECT_CALL(*this, MockGetVideoStatsCollectionCb(true, std::make_optional(expected)));
        stats_db_->GetVideoStatsCollection(key, base::BindOnce(&WebrtcVideoStatsDBImplTest::MockGetVideoStatsCollectionCb, base::Unretained(this)));
        VerifyOnePendingOp("Read");
    ...
    {
        EXPECT_CALL(*this, MockGetVideoStatsCb(true, std::optional<VideoStatsEntry>()));
        stats_db_->GetVideoStats(key, base::BindOnce(&WebrtcVideoStatsDBImplTest::MockGetVideoStatsCb, base::Unretained(this)));
        VerifyOnePendingOp("Read");
        fake_db_->GetCallback(true);
        testing::Mock::VerifyAndClearExpectations(this);
    ...
    void AppendToProtoDB(const VideoDescKey& key, const WebrtcVideoStatsEntryProto* const proto)
    {
    ...
    MOCK_METHOD2(MockGetVideoStatsCb, void(bool success, std::optional<VideoStatsEntry> entry));

    MOCK_METHOD2(MockGetVideoStatsCollectionCb, void(bool success, std::optional<WebrtcVideoStatsDB::VideoStatsCollection> collection));
...
TEST_F(WebrtcVideoStatsDBImplTest, WriteReadAndClear)
{
    ...
    VideoStats stats1(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 240, 6, 7.2);
    ...
    VideoStats stats2(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 1000, 14, 6.8);
    ...
    stats_db_->ClearStats(base::BindOnce(&WebrtcVideoStatsDBImplTest::MockClearStatsCb, base::Unretained(this)));
...
TEST_F(WebrtcVideoStatsDBImplTest, ExpiredStatsAreNotReturned)
{
    ...
    clock.SetNow(base::Time::FromMillisecondsSinceUnixEpoch(stats1.timestamp) + base::Days(1) + GetMaxTimeToKeepStats());
    ...
    clock.SetNow(base::Time::FromMillisecondsSinceUnixEpoch(stats2.timestamp) + base::Days(1) + GetMaxTimeToKeepStats());
...
TEST_F(WebrtcVideoStatsDBImplTest, ConfigureExpireDays)
{
    ...
    ASSERT_LT(base::Days(kNewMaxDaysToKeepStats), previous_max_days_to_keep_stats);

    // Override field trial.
    base::FieldTrialParams params;
    params["db_days_to_keep_stats"] = base::NumberToString(kNewMaxDaysToKeepStats);
    scoped_feature_list.InitAndEnableFeatureWithParameters(media::kWebrtcMediaCapabilitiesParameters, params);
    ...
    int remaining_days_to_keep_stats = kNewMaxDaysToKeepStats - half_days_to_keep_stats;
...
TEST_F(WebrtcVideoStatsDBImplTest, NewStatsReplaceOldStats)
{
    ...
    for (int i = 0; i < kNumberOfStatsToAdd; ++i) {
        VideoStats stats(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 240 + i, 6, 7.2 + i % 3);
...
TEST_F(WebrtcVideoStatsDBImplTest, ConfigureMaxEntriesPerConfig)
{
    ...
    // Override field trial.
    base::FieldTrialParams params;
    params["db_max_entries_per_cpnfig"] = base::NumberToString(kNewMaxEntriesPerConfig);
    scoped_feature_list.InitAndEnableFeatureWithParameters(media::kWebrtcMediaCapabilitiesParameters, params);
...
TEST_F(WebrtcVideoStatsDBImplTest, OutOfOrderTimestampClearsOldStats)
{
    ...
    VideoStats stats(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 123, 5, 11.2);
    AppendStats(kDecodeStatsKeyVp9, stats);
...
TEST_F(WebrtcVideoStatsDBImplTest, FailedWrite)
{
    ...
    stats_db_->AppendVideoStats(
        kDecodeStatsKeyVp9, VideoStats(1234, 240, 6, 7.2), base::BindOnce(&WebrtcVideoStatsDBImplTest::MockAppendVideoStatsCb, base::Unretained(this)));
    fake_db_->GetCallback(true);
...
TEST_F(WebrtcVideoStatsDBImplTest, DiscardCorruptedDBData)
{
    ...
    valid_entry->set_timestamp(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull());
    ...
    VerifyReadStats(kEncodeStatsKeyVp9,
        { VideoStats {
            valid_entry->timestamp(), valid_entry->frames_processed(), valid_entry->key_frames_processed(), valid_entry->p99_processing_time_ms() } });
...
TEST_F(WebrtcVideoStatsDBImplTest, WriteAndReadCollection)
{
    ...
    VideoStats stats2(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 360, 7, 9.2);
    VideoStatsEntry entry2 { stats2 };
    AppendStats(kDecodeStatsKeyVp9FullHd, stats2);

    VideoStats stats3(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 480, 11, 13.3);
    VideoStatsEntry entry3 { stats3 };
    AppendStats(kDecodeStatsKeyVp94K, stats3);

    // Add elements that should not be returned.
    VideoStats stats4(clock.Now().InMillisecondsFSinceUnixEpochIgnoringNull(), 490, 13, 15.3);
