...
TEST_F(PendingOperationsTest, OperationTiming)
{
    ...
    histogram_.ExpectUniqueSample(kUmaPrefix + kOperation, kInitDelay.InMicroseconds(), 1);
}
...
TEST_F(PendingOperationsTest, OperationTimeout)
{
    ...
    histogram_.ExpectUniqueSample(kUmaPrefix + kOperation, kPendingOperationTimeout.InMicroseconds(), 1);
}
...
TEST_F(PendingOperationsTest, NestedOperation)
{
    ...
    for (base::TimeDelta elapsed_time; elapsed_time < base::Milliseconds(100); elapsed_time += tick_length) {
        ...
        EXPECT_EQ(pending_operations.get_pending_ops_for_test().size(), expected_pending_operations);
        task_environment_.FastForwardBy(tick_length);
    }

    for (const auto& operation : operations) {
        histogram_.ExpectUniqueSample(kUmaPrefix + operation.name, (operation.stop_time - operation.start_time).InMicroseconds(), 1);
    }
}
