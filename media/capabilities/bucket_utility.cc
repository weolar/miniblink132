...
// to playback rate.
const int kFrameRateBuckets[]
    = { 5, 10, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 120, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000, 1500 };

// A mix of width and height dimensions for common and not-so-common resolutions
// spanning 50p -> 12K.
// TODO(chcunningham): Ponder these a bit more.
const int kSizeBuckets[] = { 50, 100, 144, 240, 256, 280, 360, 426, 480, 640, 720, 854, 960, 1080, 1280, 1440, 1920, 2160, 2560, 2880, 3160, 3840, 4128, 4320,
    5120, 6144, 7360, 7680, 8000, 9000, 10000, 11000, 11520 };
...
constexpr int kWebrtcPixelsBuckets[] = { 1280 * 720, 1920 * 1080, 2560 * 1440, 3840 * 2160 };
// The boundaries between buckets are calculated as the point between the two
// buckets.
constexpr int kWebrtcPixelsBoundaries[] = { (kWebrtcPixelsBuckets[0] + kWebrtcPixelsBuckets[1]) / 2, (kWebrtcPixelsBuckets[1] + kWebrtcPixelsBuckets[2]) / 2,
...
static_assert(std::size(kWebrtcPixelsBoundaries) + 1 == std::size(kWebrtcPixelsBuckets));
...
gfx::Size GetSizeBucket(const gfx::Size& raw_size)
{
    ...
    if (raw_size.width() < kMinSizeBucketPercent * kSizeBuckets[0] || raw_size.height() < kMinSizeBucketPercent * kSizeBuckets[0]) {
        return gfx::Size();
    }

    // Round width and height to first bucket >= |raw_size| dimensions. See
    // explanation in header file.
    const int* width_bound = std::lower_bound(std::begin(kSizeBuckets), std::end(kSizeBuckets), raw_size.width());
    const int* height_bound = std::lower_bound(std::begin(kSizeBuckets), std::end(kSizeBuckets), raw_size.height());
...
int GetFpsBucket(double raw_fps)
{
    ...
    const int* upper_bound = std::upper_bound(std::begin(kFrameRateBuckets), std::end(kFrameRateBuckets), std::round(rounded_fps));
    ...
    if (std::abs(previous_bucket - rounded_fps) < std::abs(higher_bucket - rounded_fps)) {
...
{
    const int* pixels_bucket_it = std::lower_bound(std::begin(kWebrtcPixelsBoundaries), std::end(kWebrtcPixelsBoundaries), pixels);
    // The output from std::lower_bound is in the range [begin, end], hence the
    // subtraction below is well defined.
    int pixels_bucket_index = std::distance(std::begin(kWebrtcPixelsBoundaries), pixels_bucket_it);
    DCHECK_GE(pixels_bucket_index, 0);
    DCHECK_LT(pixels_bucket_index, static_cast<int>(std::size(kWebrtcPixelsBuckets)));
