...
PendingOperations::PendingOperation::PendingOperation(std::string uma_str, std::unique_ptr<base::CancelableOnceClosure> timeout_closure)
...
PendingOperations::PendingOperation::~PendingOperation()
{
    ...
    if (timeout_closure_ && !timeout_closure_->IsCancelled()) {
        ...
        DVLOG(3) << __func__ << " Completed " << uma_str_ << " (" << op_duration.InMilliseconds() << ")";
...
void PendingOperations::PendingOperation::UmaHistogramOpTime(base::TimeDelta duration)
{
    base::UmaHistogramCustomMicrosecondsTimes(uma_str_, duration, base::Milliseconds(1), kPendingOpTimeout, 50);
}

void PendingOperations::PendingOperation::OnTimeout()
{
    UmaHistogramOpTime(kPendingOpTimeout);
    LOG(WARNING) << " Timeout performing " << uma_str_ << " operation on WebrtcVideoStatsDB";
...
PendingOperations::Id PendingOperations::Start(std::string_view uma_str)
{
    ...
    auto timeout_closure = std::make_unique<base::CancelableOnceClosure>(base::BindOnce(&PendingOperations::OnTimeout, weak_ptr_factory_.GetWeakPtr(), op_id));

    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, timeout_closure->callback(), kPendingOpTimeout);

    pending_ops_.emplace(op_id, std::make_unique<PendingOperation>(uma_prefix_ + std::string(uma_str), std::move(timeout_closure)));
