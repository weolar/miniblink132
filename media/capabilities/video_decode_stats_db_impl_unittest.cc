...
public:
    ...
    VideoDecodeStatsDBImplTest()
        : kStatsKeyVp9(VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE3, gfx::Size(1024, 768), 60, "com.widevine.alpha", true))
        , kStatsKeyAvc(VideoDescKey::MakeBucketedKey(H264PROFILE_MIN, gfx::Size(1024, 768), 60, "", false))
    {
        bool parsed_time = base::Time::FromString(VideoDecodeStatsDBImpl::kDefaultWriteTime, &kDefaultWriteTime);
        DCHECK(parsed_time);
    ...
    VideoDecodeStatsDBImplTest& operator=(const VideoDecodeStatsDBImplTest&) = delete;
    ...
    {
        EXPECT_EQ(stats_db_->pending_operations_.get_pending_ops_for_test().size(), 1u);
        PendingOperations::PendingOperation* pending_op = stats_db_->pending_operations_.get_pending_ops_for_test().begin()->second.get();
        EXPECT_TRUE(pending_op->uma_str_.ends_with(op_name));
    }

    void VerifyNoPendingOps()
    {
        EXPECT_TRUE(stats_db_->pending_operations_.get_pending_ops_for_test().empty());
    }
    ...
    void InitializeDB()
    {
        stats_db_->Initialize(base::BindOnce(&VideoDecodeStatsDBImplTest::OnInitialize, base::Unretained(this)));
    ...
    void AppendStats(const VideoDescKey& key, const DecodeStatsEntry& entry)
    {
        ...
        stats_db_->AppendDecodeStats(key, entry, base::BindOnce(&VideoDecodeStatsDBImplTest::MockAppendDecodeStatsCb, base::Unretained(this)));
    ...
    void VerifyReadStats(const VideoDescKey& key, const DecodeStatsEntry& expected)
    {
        EXPECT_CALL(*this, MockGetDecodeStatsCb(true, Pointee(Eq(expected))));
        stats_db_->GetDecodeStats(key, base::BindOnce(&VideoDecodeStatsDBImplTest::GetDecodeStatsCb, base::Unretained(this)));
        VerifyOnePendingOp("Read");
        fake_db_->GetCallback(true);
        testing::Mock::VerifyAndClearExpectations(this);
    }
    ...
    void AppendToProtoDB(const VideoDescKey& key, const DecodeStatsProto* const proto)
    {
        ...
        fake_db_->UpdateEntries(std::move(entries), std::make_unique<leveldb_proto::KeyVector>(), std::move(update_done_cb));
    ...
    MOCK_METHOD2(MockGetDecodeStatsCb, void(bool success, DecodeStatsEntry* entry));
...
TEST_F(VideoDecodeStatsDBImplTest, InitializeFailed)
{
    ...
    fake_db_.ExtractAsDangling()->InitStatusCallback(leveldb_proto::Enums::InitStatus::kError);
}
...
TEST_F(VideoDecodeStatsDBImplTest, WriteReadAndClear)
{
    ...
    stats_db_->ClearStats(base::BindOnce(&VideoDecodeStatsDBImplTest::MockClearStatsCb, base::Unretained(this)));
...
TEST_F(VideoDecodeStatsDBImplTest, ConfigureMaxFramesPerBuffer)
{
    ...
    params[VideoDecodeStatsDBImpl::kMaxFramesPerBufferParamName] = base::NumberToString(new_max_frames_per_buffer);

    scoped_feature_list.InitAndEnableFeatureWithParameters(media::kMediaCapabilitiesWithParameters, params);
...
public:
    ...
    ~DBClockScope()
    {
        test_class_->SetDBClock(nullptr);
    }
...
TEST_F(VideoDecodeStatsDBImplTest, ConfigureExpireDays)
{
    ...
    // Override field trial.
    base::FieldTrialParams params;
    params[VideoDecodeStatsDBImpl::kMaxDaysToKeepStatsParamName] = base::NumberToString(new_max_days_to_keep_stats);
    ...
    int remaining_days_to_keep_stats = new_max_days_to_keep_stats - half_days_to_keep_stats;
...
TEST_F(VideoDecodeStatsDBImplTest, FailedWrite)
{
    ...
    stats_db_->AppendDecodeStats(
        kStatsKeyVp9, DecodeStatsEntry(1000, 2, 10), base::BindOnce(&VideoDecodeStatsDBImplTest::MockAppendDecodeStatsCb, base::Unretained(this)));
    fake_db_->GetCallback(true);
...
TEST_F(VideoDecodeStatsDBImplTest, FillBufferInMixedIncrements)
{
    ...
    DecodeStatsEntry entryA(kNumFramesEntryA, std::round(0.1 * kNumFramesEntryA), std::round(0.5 * kNumFramesEntryA));
    const double kDropRateA = static_cast<double>(entryA.frames_dropped) / entryA.frames_decoded;
    const double kEfficientRateA = static_cast<double>(entryA.frames_power_efficient) / entryA.frames_decoded;
    ...
    AppendStats(kStatsKeyVp9, entryA);
    VerifyReadStats(kStatsKeyVp9,
        DecodeStatsEntry(GetMaxFramesPerBuffer(), std::round(kDropRateA * GetMaxFramesPerBuffer()), std::round(kEfficientRateA * GetMaxFramesPerBuffer())));
    ...
    DecodeStatsEntry entryB(kNumFramesEntryB, std::round(0.25 * kNumFramesEntryB), std::round(1 * kNumFramesEntryB));
    const double kDropRateB = static_cast<double>(entryB.frames_dropped) / entryB.frames_decoded;
    const double kEfficientRateB = static_cast<double>(entryB.frames_power_efficient) / entryB.frames_decoded;
    ...
    VerifyReadStats(kStatsKeyVp9,
        DecodeStatsEntry(
            GetMaxFramesPerBuffer(), std::round(GetMaxFramesPerBuffer() * mixed_drop_rate), std::round(GetMaxFramesPerBuffer() * mixed_effiency_rate)));
    ...
    DecodeStatsEntry entryC(kNumFramesEntryC, std::round(0.3 * kNumFramesEntryC), std::round(0.25 * kNumFramesEntryC));
    const double kDropRateC = static_cast<double>(entryC.frames_dropped) / entryC.frames_decoded;
    const double kEfficientRateC = static_cast<double>(entryC.frames_power_efficient) / entryC.frames_decoded;
    AppendStats(kStatsKeyVp9, entryC);
    VerifyReadStats(kStatsKeyVp9,
        DecodeStatsEntry(GetMaxFramesPerBuffer(), std::round(GetMaxFramesPerBuffer() * kDropRateC), std::round(GetMaxFramesPerBuffer() * kEfficientRateC)));
}
...
TEST_F(VideoDecodeStatsDBImplTest, OverfillEmptyBuffer)
{
    ...
    DecodeStatsEntry entryA(kNumFramesOverfill, std::round(0.1 * kNumFramesOverfill), std::round(0.5 * kNumFramesOverfill));
    ...
    DecodeStatsEntry readBackEntryA(GetMaxFramesPerBuffer(), std::round(0.1 * GetMaxFramesPerBuffer()), std::round(0.5 * GetMaxFramesPerBuffer()));
    ...
    DecodeStatsEntry entryB(kNumFramesOverfill, std::round(0.2 * kNumFramesOverfill), std::round(0.6 * kNumFramesOverfill));
    AppendStats(kStatsKeyVp9, entryB);
    // Read-back stats should have same ratios, but scaled such that
    // frames_decoded = GetMaxFramesPerBuffer().
    DecodeStatsEntry readBackEntryB(GetMaxFramesPerBuffer(), std::round(0.2 * GetMaxFramesPerBuffer()), std::round(0.6 * GetMaxFramesPerBuffer()));
...
TEST_F(VideoDecodeStatsDBImplTest, EnableUnweightedEntries)
{
    ...
    VerifyReadStats(kStatsKeyVp9,
        DecodeStatsEntry(expected_denominator, unweighted_smoothness_avg * expected_denominator, unweighted_efficiency_avg * expected_denominator));

    // Append 20K frames with 5% dropped and 10% efficient.
    AppendStats(kStatsKeyVp9, DecodeStatsEntry(20000, 0.05 * 20000, 0.10 * 20000));
    num_appends++;
    ...
    AppendStats(kStatsKeyVp9, DecodeStatsEntry(1000000, 0.012345 * 1000000, 0.034567 * 1000000));
    num_appends++;
...
TEST_F(VideoDecodeStatsDBImplTest, DiscardCorruptedDBData)
{
    ...
    const auto keyA = VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE0, gfx::Size(1024, 768), 60, "", false);
    const auto keyB = VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE1, gfx::Size(1024, 768), 60, "", false);
    const auto keyC = VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE2, gfx::Size(1024, 768), 60, "", false);
    const auto keyD = VideoDescKey::MakeBucketedKey(VP9PROFILE_PROFILE3, gfx::Size(1024, 768), 60, "", false);
    const auto keyE = VideoDescKey::MakeBucketedKey(H264PROFILE_BASELINE, gfx::Size(1024, 768), 60, "", false);
    const auto keyF = VideoDescKey::MakeBucketedKey(H264PROFILE_MAIN, gfx::Size(1024, 768), 60, "", false);
    const auto keyG = VideoDescKey::MakeBucketedKey(H264PROFILE_EXTENDED, gfx::Size(1024, 768), 60, "", false);
    ...
    protoG.set_last_write_date((clock.Now() + base::Days(1)).InMillisecondsFSinceUnixEpoch());
