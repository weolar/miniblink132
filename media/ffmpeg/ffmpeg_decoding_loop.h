...
public:
    ...
    FFmpegDecodingLoop(AVCodecContext* context, bool continue_on_decoding_errors = false);
    ...
    DecodeStatus DecodePacket(const AVPacket* packet, FrameReadyCB frame_ready_cb);

    int last_averror_code() const
    {
        return last_averror_code_;
    }
