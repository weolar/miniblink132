...
{
    AVDictionaryEntry* alpha_mode = av_dict_get(stream->metadata, "alpha_mode", nullptr, 0);
    return alpha_mode && !strcmp(alpha_mode->value, "1") ? VideoDecoderConfig::AlphaMode::kHasAlpha : VideoDecoderConfig::AlphaMode::kIsOpaque;
}
...
static_assert(VideoFrame::kFrameSizePadding >= kFFmpegOutputBufferPaddingSize, "VideoFrame padding size does not fit ffmpeg requirement");

static_assert(VideoFrame::kFrameAddressAlignment >= kFFmpegBufferAddressAlignment && VideoFrame::kFrameAddressAlignment % kFFmpegBufferAddressAlignment == 0,
...
SampleFormat AVSampleFormatToSampleFormat(AVSampleFormat sample_format, AVCodecID codec_id)
{
    switch (sample_format) {
...
bool AVCodecContextToAudioDecoderConfig(const AVCodecContext* codec_context, EncryptionScheme encryption_scheme, AudioDecoderConfig* config)
{
    DCHECK_EQ(codec_context->codec_type, AVMEDIA_TYPE_AUDIO);

    AudioCodec codec = CodecIDToAudioCodec(codec_context->codec_id);

    SampleFormat sample_format = AVSampleFormatToSampleFormat(codec_context->sample_fmt, codec_context->codec_id);

    ChannelLayout channel_layout = codec_context->ch_layout.nb_channels > 8
        ? CHANNEL_LAYOUT_DISCRETE
        : ChannelLayoutToChromeChannelLayout(codec_context->ch_layout.u.mask, codec_context->ch_layout.nb_channels);
    ...
    if (codec_context->seek_preroll > 0) {
        seek_preroll = base::Microseconds(codec_context->seek_preroll * 1000000.0 / codec_context->sample_rate);
    }
    ...
    if ((codec_context->extradata_size == 0) != (codec_context->extradata == nullptr)) {
        LOG(ERROR) << __func__ << (codec_context->extradata == nullptr ? " NULL" : " Non-NULL") << " extra data cannot have size of "
    ...
    if (codec_context->extradata_size > 0) {
        extra_data.assign(codec_context->extradata, codec_context->extradata + codec_context->extradata_size);
    }

    config->Initialize(codec, sample_format, channel_layout, codec_context->sample_rate, extra_data, encryption_scheme, seek_preroll, codec_context->delay);
    ...
    if (codec == AudioCodec::kAAC) {
        ...
        if (codec_context->profile == FF_PROFILE_UNKNOWN || codec_context->profile == mp4::AAC::kXHeAAcType - 1) {
    ...
    DCHECK_EQ(av_get_bytes_per_sample(codec_context->sample_fmt), config->bytes_per_channel());
    return true;
}

std::unique_ptr<AVCodecContext, ScopedPtrAVFreeContext> AVStreamToAVCodecContext(const AVStream* stream)
{
...
void AudioDecoderConfigToAVCodecContext(const AudioDecoderConfig& config, AVCodecContext* codec_context)
{
    ...
    if (config.extra_data().empty()) {
        ...
        codec_context->extradata = reinterpret_cast<uint8_t*>(av_malloc(config.extra_data().size() + AV_INPUT_BUFFER_PADDING_SIZE));
        memcpy(codec_context->extradata, &config.extra_data()[0], config.extra_data().size());
        memset(codec_context->extradata + config.extra_data().size(), '\0', AV_INPUT_BUFFER_PADDING_SIZE);
    }
}
...
bool AVStreamToVideoDecoderConfig(const AVStream* stream, VideoDecoderConfig* config)
{
    ...
    if (stream->sample_aspect_ratio.num) {
        aspect_ratio = VideoAspectRatio::PAR(stream->sample_aspect_ratio.num, stream->sample_aspect_ratio.den);
    } else if (codec_context->sample_aspect_ratio.num) {
        aspect_ratio = VideoAspectRatio::PAR(codec_context->sample_aspect_ratio.num, codec_context->sample_aspect_ratio.den);
    }
    ...
    VideoColorSpace color_space = VideoColorSpace(codec_context->color_primaries, codec_context->color_trc, codec_context->colorspace,
        codec_context->color_range == AVCOL_RANGE_JPEG ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
    VideoPixelFormat pixel_format = AVPixelFormatToVideoPixelFormat(codec_context->pix_fmt);
    VideoDecoderConfig::AlphaMode alpha_mode = GetAlphaMode(stream);
    VideoChromaSampling chroma_sampling = VideoPixelFormatToChromaSampling(pixel_format);
    ...
    case VideoCodec::kH264: {
        ...
        if (profile == VIDEO_CODEC_PROFILE_UNKNOWN && codec_context->extradata && codec_context->extradata_size) {
            mp4::AVCDecoderConfigurationRecord avc_config;
            if (avc_config.Parse(codec_context->extradata, codec_context->extradata_size)) {
    ...
    case VideoCodec::kHEVC: {
        ...
        if (codec_context->extradata && codec_context->extradata_size) {
            ...
            if (hevc_config.Parse(codec_context->extradata, codec_context->extradata_size)) {
    ...
    case VideoCodec::kAV1:
        ...
        if (codec_context->extradata && codec_context->extradata_size) {
            ...
            if (av1_config.Parse(codec_context->extradata, codec_context->extradata_size)) {
    ...
    if (!color_space.IsSpecified()) {
        ...
        if (codec_context->codec_id != AV_CODEC_ID_VP9 && codec_context->codec_id != AV_CODEC_ID_AV1) {
            // Otherwise, assume that SD video is usually Rec.601, and HD is usually
            // Rec.709.
            color_space = (natural_size.height() < 720) ? VideoColorSpace::REC601() : VideoColorSpace::REC709();
        }
    } else if ((codec_context->codec_id == AV_CODEC_ID_HEVC || codec_context->codec_id == AV_CODEC_ID_H264) && codec_context->colorspace == AVCOL_SPC_RGB
        ...
        && (color_space.primaries == VideoColorSpace::PrimaryID::INVALID || color_space.transfer == VideoColorSpace::TransferID::INVALID
    ...
    for (int i = 0; i < stream->codecpar->nb_coded_side_data; ++i) {
        ...
        case AV_PKT_DATA_DISPLAYMATRIX: {
            ...
            video_transformation = VideoTransformation::FromFFmpegDisplayMatrix(reinterpret_cast<int32_t*>(side_data.data));
            break;
        }
        case AV_PKT_DATA_MASTERING_DISPLAY_METADATA: {
            AVMasteringDisplayMetadata* mdcv = reinterpret_cast<AVMasteringDisplayMetadata*>(side_data.data);
        ...
        case AV_PKT_DATA_CONTENT_LIGHT_LEVEL: {
            AVContentLightMetadata* clli = reinterpret_cast<AVContentLightMetadata*>(side_data.data);
            hdr_metadata.cta_861_3 = gfx::HdrMetadataCta861_3(clli->MaxCLL, clli->MaxFALL);
            break;
        }
#if BUILDFLAG(ENABLE_PLATFORM_DOLBY_VISION)
        case AV_PKT_DATA_DOVI_CONF: {
            AVDOVIDecoderConfigurationRecord* dovi = reinterpret_cast<AVDOVIDecoderConfigurationRecord*>(side_data.data);
            VideoType type;
    ...
    config->Initialize(
        codec, profile, alpha_mode, color_space, video_transformation, coded_size, visible_rect, natural_size, extra_data, GetEncryptionScheme(stream));
...
ChannelLayout ChannelLayoutToChromeChannelLayout(int64_t layout, int channels)
{
    ...
    case AV_CH_FRONT_LEFT | AV_CH_FRONT_RIGHT | AV_CH_LOW_FREQUENCY | AV_CH_BACK_CENTER:
