...
void RenderLineOfText(const std::string& line, int top, VideoFrame* frame)
{
    ...
    const size_t first_idx
        = (line_width < frame->visible_rect().width()) ? 0u : static_cast<size_t>(((line_width - frame->visible_rect().width()) / pixels_per_char) + 1);
...
scoped_refptr<VideoFrame> RenderPerformanceMetricsOverlay(base::TimeDelta target_playout_delay, bool in_low_latency_mode, int target_bitrate, int frames_ago,
    double encoder_utilization, double lossiness, scoped_refptr<VideoFrame> source)
{
    ...
    const scoped_refptr<VideoFrame> frame
        = VideoFrame::CreateFrame(source->format(), source->coded_size(), source->visible_rect(), source->natural_size(), source->timestamp());
    if (!frame)
        return source; // Allocation failure: Return source frame.
    for (size_t plane = 0, num_planes = VideoFrame::NumPlanes(source->format()); plane < num_planes; ++plane) {
        const size_t row_count = VideoFrame::Rows(plane, source->format(), source->visible_rect().height());
        const size_t bytes_per_row = VideoFrame::RowBytes(plane, source->format(), source->visible_rect().width());
        ...
        for (size_t row = 0; row < row_count; ++row, src += src_stride, dst += dst_stride) {
    ...
    frame->AddDestructionObserver(base::BindOnce(
        [](const VideoFrameMetadata& sent_frame_metadata, scoped_refptr<VideoFrame> source_frame) { source_frame->set_metadata(sent_frame_metadata); },
    ...
    if (frame->metadata().frame_duration.has_value()) {
        const int decimilliseconds = base::saturated_cast<int>(frame->metadata().frame_duration->InMicroseconds() / 100.0 + 0.5);
    ...
    RenderLineOfText(base::StringPrintf("%d.%01d %dx%d %d:%02d.%02d", frame_duration_ms, frame_duration_ms_frac, frame->visible_rect().width(),
                         frame->visible_rect().height(), minutes, seconds, hundredth_seconds),
        top, frame.get());
    ...
    if (frame->metadata().capture_begin_time && frame->metadata().capture_end_time) {
        capture_duration_ms = base::saturated_cast<int>((*frame->metadata().capture_end_time - *frame->metadata().capture_begin_time).InMillisecondsF() + 0.5);
    }
    ...
    RenderLineOfText(
        base::StringPrintf("%d %4.1d%c %4.1d", capture_duration_ms, target_playout_delay_ms, in_low_latency_mode ? '!' : '.', target_kbits), top, frame.get());
    ...
    RenderLineOfText(base::StringPrintf("%d %3.1d%% %3.1d%%", frames_ago, encoder_pct, lossy_pct), top, frame.get());
