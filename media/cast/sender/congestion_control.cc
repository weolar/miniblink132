...
public:
    AdaptiveCongestionControl(const base::TickClock* clock, int max_bitrate_configured, int min_bitrate_configured, double max_frame_rate);

    AdaptiveCongestionControl(const AdaptiveCongestionControl&) = delete;
    AdaptiveCongestionControl& operator=(const AdaptiveCongestionControl&) = delete;
    ...
    void WillSendFrameToTransport(FrameId frame_id, size_t frame_size_in_bytes, base::TimeTicks when) final;
    void AckFrame(FrameId frame_id, base::TimeTicks when) final;
    void AckLaterFrames(std::vector<FrameId> received_frames, base::TimeTicks when) final;
    int GetBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay) final;
...
private:
    ...
    // second.
    base::TimeTicks EstimatedSendingTime(FrameId frame_id, double estimated_bitrate);
...
CongestionControl* NewAdaptiveCongestionControl(const base::TickClock* clock, int max_bitrate_configured, int min_bitrate_configured, double max_frame_rate)
{
    return new AdaptiveCongestionControl(clock, max_bitrate_configured, min_bitrate_configured, max_frame_rate);
}
...
AdaptiveCongestionControl::AdaptiveCongestionControl(
    const base::TickClock* clock, int max_bitrate_configured, int min_bitrate_configured, double max_frame_rate)
    : clock_(clock)
...
void AdaptiveCongestionControl::UpdateTargetPlayoutDelay(base::TimeDelta delay)
{
    const int max_unacked_frames = std::min<int>(kMaxUnackedFrames, 1 + (delay * max_frame_rate_).InSeconds());
...
base::TimeDelta AdaptiveCongestionControl::DeadTime(const FrameStats& a, const FrameStats& b)
{
...
double AdaptiveCongestionControl::CalculateSafeBitrate()
{
    ...
    base::TimeDelta transmit_time = GetFrameStats(last_checkpoint_frame_)->ack_time - frame_stats_.front().enqueue_time - dead_time_in_history_;
...
AdaptiveCongestionControl::FrameStats* AdaptiveCongestionControl::GetFrameStats(FrameId frame_id)
{
...
void AdaptiveCongestionControl::PruneFrameStats()
{
    ...
    const size_t retention_count = std::max<size_t>(history_size_, last_frame_stats_ - last_checkpoint_frame_ + 1);

    // Sanity-check: At least one entry must be kept, but the dataset should not
    // grow indefinitely.
    DCHECK_GE(retention_count, 1u);
    constexpr size_t kMaxInFlightRangeSize = kMaxUnackedFrames + // Maximum unACKed frames.
...
void AdaptiveCongestionControl::AckFrame(FrameId frame_id, base::TimeTicks when)
{
...
void AdaptiveCongestionControl::AckLaterFrames(std::vector<FrameId> received_frames, base::TimeTicks when)
{
...
void AdaptiveCongestionControl::WillSendFrameToTransport(FrameId frame_id, size_t frame_size_in_bytes, base::TimeTicks when)
{
...
base::TimeTicks AdaptiveCongestionControl::EstimatedSendingTime(FrameId frame_id, double estimated_bitrate)
{
    ...
    for (FrameId f = last_checkpoint_frame_ + 1; f < frame_id; ++f) {
        ...
        // reach the sender.
        const base::TimeDelta frame_transmit_time = base::Seconds(stats->frame_size_in_bits / estimated_bitrate);
...
int AdaptiveCongestionControl::GetBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay)
{
    ...
    const base::TimeDelta time_to_catch_up = playout_time - EstimatedSendingTime(last_enqueued_frame_ + 1, safe_bitrate);
    ...
    const int bits_per_second = base::ClampRound(safe_bitrate * (empty_buffer_fraction / kTargetEmptyBufferFraction));
    VLOG(3) << " FBR:" << (bits_per_second / 1E6) << " EBF:" << empty_buffer_fraction << " SBR:" << (safe_bitrate / 1E6);
    TRACE_COUNTER_ID1("cast.stream", "Empty Buffer Fraction", this, empty_buffer_fraction);

    return std::clamp(bits_per_second, min_bitrate_configured_, max_bitrate_configured_);
}
