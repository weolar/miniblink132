...
protected:
        ...
        : task_runner_(new FakeSingleThreadTaskRunner(&testing_clock_))
    {
        ...
        congestion_control_.reset(NewAdaptiveCongestionControl(&testing_clock_, kMaxBitrateConfigured, kMinBitrateConfigured, kMaxFrameRate));
        const int max_unacked_frames = 10;
        const base::TimeDelta target_playout_delay = (max_unacked_frames - 1) * base::Seconds(1) / kMaxFrameRate;
    ...
    void Run(int num_frames, size_t frame_size_in_bytes, base::TimeDelta rtt, base::TimeDelta frame_delay, base::TimeDelta ack_time)
    {
        ...
        for (frame_id_ = FrameId::first(); frame_id_ < end; frame_id_++) {
            ...
            congestion_control_->WillSendFrameToTransport(frame_id_, frame_size_in_bytes, testing_clock_.NowTicks());
            task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&CongestionControlTest::AckFrame, base::Unretained(this), frame_id_), ack_time);
...
TEST_F(CongestionControlTest, SimpleRun)
{
    ...
    Run(500, frame_size_in_bytes, base::Milliseconds(10), base::Milliseconds(kFrameDelayMs), base::Milliseconds(45));
    // Empty the buffer.
    task_runner_->Sleep(base::Milliseconds(100));

    uint32_t safe_bitrate = frame_size_in_bytes * 1000 * 8 / kFrameDelayMs;
    uint32_t bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + base::Milliseconds(300), base::Milliseconds(300));
    EXPECT_NEAR(safe_bitrate / kTargetEmptyBufferFraction, bitrate, safe_bitrate * 0.05);

    bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + base::Milliseconds(200), base::Milliseconds(300));
    EXPECT_NEAR(safe_bitrate / kTargetEmptyBufferFraction * 2 / 3, bitrate, safe_bitrate * 0.05);

    bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + base::Milliseconds(100), base::Milliseconds(300));
    EXPECT_NEAR(safe_bitrate / kTargetEmptyBufferFraction * 1 / 3, bitrate, safe_bitrate * 0.05);

    // Add a large (100ms) frame.
    congestion_control_->WillSendFrameToTransport(frame_id_++, safe_bitrate * 100 / 1000 / 8, testing_clock_.NowTicks());

    // Results should show that we have ~200ms to send.
    bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + base::Milliseconds(300), base::Milliseconds(300));
    EXPECT_NEAR(safe_bitrate / kTargetEmptyBufferFraction * 2 / 3, bitrate, safe_bitrate * 0.05);
    ...
    congestion_control_->AckLaterFrames(received_frames, testing_clock_.NowTicks());
...
TEST_F(CongestionControlTest, RetainsSufficientHistory)
{
    ...
    int bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + kFakePlayoutDelay, kFakePlayoutDelay);
    ...
    for (int i = 0; i < kMaxUnackedFrames; ++i) {
        congestion_control_->WillSendFrameToTransport(frame_id, 16384 / 8, testing_clock_.NowTicks());

        bitrate = congestion_control_->GetBitrate(testing_clock_.NowTicks() + kFakePlayoutDelay, kFakePlayoutDelay);
        ASSERT_GE(bitrate, kMinBitrateConfigured);
        ASSERT_LE(bitrate, kMaxBitrateConfigured);
