...
std::unique_ptr<FrameSender> FrameSender::Create(
    scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& config, CastTransport* const transport_sender, Client& client)
{
    return std::make_unique<FrameSenderImpl>(cast_environment, config, transport_sender, client);
}

// Convenience macro used in logging statements throughout this file.
#define SENDER_SSRC (is_audio_ ? "AUDIO[" : "VIDEO[") << config_.sender_ssrc << "] "

FrameSenderImpl::Client::~Client() = default;

FrameSenderImpl::RtcpClient::RtcpClient(base::WeakPtr<FrameSenderImpl> frame_sender)
...
void FrameSenderImpl::RtcpClient::OnReceivedCastMessage(const RtcpCastMessage& cast_message)
{
    if (frame_sender_)
        frame_sender_->OnReceivedCastFeedback(cast_message);
}

void FrameSenderImpl::RtcpClient::OnReceivedRtt(base::TimeDelta round_trip_time)
{
    if (frame_sender_)
...
FrameSenderImpl::FrameSenderImpl(
    ...
    congestion_control_((!config.use_external_encoder && !is_audio_)
            ? NewAdaptiveCongestionControl(cast_environment->Clock(), config.max_bitrate, config.min_bitrate, max_frame_rate_)
            : NewFixedCongestionControl((config.min_bitrate + config.max_bitrate) / 2)
    ...
    , max_ack_delay_(config_.max_playout_delay)
{
    ...
    VLOG(1) << SENDER_SSRC << "min latency " << config_.min_playout_delay.InMilliseconds() << ", max latency " << config_.max_playout_delay.InMilliseconds();
    ...
    transport_sender_->InitializeStream(transport_config, std::make_unique<FrameSenderImpl::RtcpClient>(weak_factory_.GetWeakPtr()));
}
...
base::TimeTicks FrameSenderImpl::GetRecordedReferenceTime(FrameId frame_id) const
{
...
void FrameSenderImpl::ScheduleNextRtcpReport()
{
    ...
    cast_environment_->PostDelayedTask(
        CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameSenderImpl::SendRtcpReport, weak_factory_.GetWeakPtr(), true), kRtcpReportInterval);
}
...
void FrameSenderImpl::SendRtcpReport(bool schedule_future_reports)
{
    ...
    const RtpTimeDelta rtp_delta = ToRtpTimeDelta(time_delta, config_.rtp_timebase);
    const RtpTimeTicks now_as_rtp_timestamp = GetRecordedRtpTimestamp(last_sent_frame_id_) + rtp_delta;
    transport_sender_->SendSenderReport(config_.sender_ssrc, now, now_as_rtp_timestamp);
...
void FrameSenderImpl::OnMeasuredRoundTripTime(base::TimeDelta round_trip_time)
{
    ...
    max_ack_delay_ = 2 * std::max(current_round_trip_time_, base::TimeDelta()) + kReceiverProcessTime;
    max_ack_delay_ = std::min(max_ack_delay_, target_playout_delay_);
}

void FrameSenderImpl::SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay)
{
    if (send_target_playout_delay_ && target_playout_delay_ == new_target_playout_delay) {
        return;
    }
    new_target_playout_delay = std::max(new_target_playout_delay, config_.min_playout_delay);
    new_target_playout_delay = std::min(new_target_playout_delay, config_.max_playout_delay);
    VLOG(2) << SENDER_SSRC << "Target playout delay changing from " << target_playout_delay_.InMilliseconds() << " ms to "
...
void FrameSenderImpl::ResendCheck()
{
    ...
    if (time_since_last_send > max_ack_delay_) {
        if (latest_acked_frame_id_ == last_sent_frame_id_) {
            ...
            VLOG(1) << SENDER_SSRC << "ACK timeout; last acked frame: " << latest_acked_frame_id_;
            ResendForKickstart();
        }
    }
...
void FrameSenderImpl::ScheduleNextResendCheck()
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    DCHECK(!last_send_time_.is_null());
    base::TimeDelta time_to_next = last_send_time_ - cast_environment_->Clock()->NowTicks() + max_ack_delay_;
...
void FrameSenderImpl::ResendForKickstart()
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    DCHECK(!last_send_time_.is_null());
    VLOG(1) << SENDER_SSRC << "Resending last packet of frame " << last_sent_frame_id_ << " to kick-start.";
    last_send_time_ = cast_environment_->Clock()->NowTicks();
    transport_sender_->ResendFrameForKickstart(config_.sender_ssrc, last_sent_frame_id_);
}

void FrameSenderImpl::RecordLatestFrameTimestamps(FrameId frame_id, base::TimeTicks reference_time, RtpTimeTicks rtp_timestamp)
{
...
base::TimeDelta FrameSenderImpl::GetInFlightMediaDuration() const
{
    ...
    const RtpTimeTicks oldest_acked_timestamp = GetRecordedRtpTimestamp(latest_acked_frame_id_);
    const RtpTimeTicks newest_acked_timestamp = GetRecordedRtpTimestamp(last_sent_frame_id_);
    return ToTimeDelta(newest_acked_timestamp - oldest_acked_timestamp, config_.rtp_timebase) + encoder_duration;
}
...
int FrameSenderImpl::GetSuggestedBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay)
{
...
bool FrameSenderImpl::EnqueueFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));

    VLOG(2) << SENDER_SSRC << "About to send another frame: last_sent=" << last_sent_frame_id_ << ", latest_acked=" << latest_acked_frame_id_;

    const FrameId frame_id = encoded_frame->frame_id;
    const bool is_first_frame_to_be_sent = last_send_time_.is_null();

    if (picture_lost_at_receiver_ && (encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame)) {
        ...
        transport_sender_->CancelSendingFrames(config_.sender_ssrc, cancel_sending_frames);
    }
    ...
    VLOG_IF(1, !is_audio_ && encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame)
    ...
    encode_event->key_frame = encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame;
    ...
    RecordLatestFrameTimestamps(frame_id, encoded_frame->reference_time, encoded_frame->rtp_timestamp);

    if (!is_audio_) {
        // Used by chrome/browser/media/cast_mirroring_performance_browsertest.cc
        TRACE_EVENT_INSTANT1("cast_perf_test", "VideoFrameEncoded", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", encoded_frame->rtp_timestamp.lower_32_bits());
    }
    ...
    if (num_aggressive_rtcp_reports_sent_ < kNumAggressiveReportsSentAtStart) {
        ...
        VLOG_IF(1, is_last_aggressive_report) << SENDER_SSRC << "Sending last aggressive report.";
        SendRtcpReport(is_last_aggressive_report);
    }

    congestion_control_->WillSendFrameToTransport(frame_id, encoded_frame->data.size(), last_send_time_);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(
        "cast.stream", name, TRACE_ID_WITH_SCOPE(name, frame_id.lower_32_bits()), "rtp_timestamp", encoded_frame->rtp_timestamp.lower_32_bits());
    transport_sender_->InsertFrame(config_.sender_ssrc, *encoded_frame);
    return true;
}

void FrameSenderImpl::OnReceivedCastFeedback(const RtcpCastMessage& cast_feedback)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    if (have_valid_rtt) {
        ...
        if (num_aggressive_rtcp_reports_sent_ < kNumAggressiveReportsSentAtStart) {
            VLOG(1) << SENDER_SSRC << "No longer a need to send reports aggressively (sent " << num_aggressive_rtcp_reports_sent_ << ").";
    ...
    if (cast_feedback.missing_frames_and_packets.empty() && cast_feedback.received_later_frames.empty()) {
        if (latest_acked_frame_id_ == cast_feedback.ack_frame_id) {
            VLOG(1) << SENDER_SSRC << "Received duplicate ACK for frame " << latest_acked_frame_id_;
            TRACE_EVENT_INSTANT2("cast.stream", "Duplicate ACK", TRACE_EVENT_SCOPE_THREAD, "ack_frame_id", cast_feedback.ack_frame_id.lower_32_bits(),
    ...
    if (!cast_feedback.received_later_frames.empty()) {
        ...
        congestion_control_->AckLaterFrames(cast_feedback.received_later_frames, now);
    }
    ...
    VLOG(2) << SENDER_SSRC << "Received ACK" << (is_acked_out_of_order ? " out-of-order" : "") << " for frame " << cast_feedback.ack_frame_id;
    if (is_acked_out_of_order) {
        TRACE_EVENT_INSTANT2("cast.stream", "ACK out of order", TRACE_EVENT_SCOPE_THREAD, "ack_frame_id", cast_feedback.ack_frame_id.lower_32_bits(),
        ...
        do {
            ...
            TRACE_EVENT_NESTABLE_ASYNC_END1(
                "cast.stream", name, TRACE_ID_WITH_SCOPE(name, latest_acked_frame_id_.lower_32_bits()), "RTT_usecs", current_round_trip_time_.InMicroseconds());
        } while (latest_acked_frame_id_ < cast_feedback.ack_frame_id);
        transport_sender_->CancelSendingFrames(config_.sender_ssrc, frames_to_cancel);
    }
}
...
bool FrameSenderImpl::ShouldDropNextFrame(base::TimeDelta frame_duration) const
{
    // Check that accepting the next frame won't cause more frames to become
    // in-flight than the system's design limit.
    const int count_frames_in_flight = GetUnacknowledgedFrameCount() + client_.GetNumberOfFramesInEncoder();
    ...
    if (VLOG_IS_ON(1)) {
        const int64_t percent = allowed_in_flight.is_positive() ? base::ClampRound<int64_t>(duration_would_be_in_flight / allowed_in_flight * 100)
                                                                : std::numeric_limits<int64_t>::max();
        VLOG_IF(1, percent > 50) << SENDER_SSRC << duration_in_flight.InMicroseconds() << " usec in-flight + " << frame_duration.InMicroseconds()
