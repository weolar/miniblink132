...
CastStreamingFrameDropReason ToFrameDropReason(EnqueueFrameResult result)
{
    ...
    case EnqueueFrameResult::MAX_DURATION_IN_FLIGHT:
        return CastStreamingFrameDropReason::kInFlightDurationTooHighAfterEncoding;
    }
}

} // namespace

std::unique_ptr<FrameSender> FrameSender::Create(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& config,
    std::unique_ptr<openscreen::cast::Sender> sender, Client& client, FrameSender::GetSuggestedVideoBitrateCB get_bitrate_cb)
{
    return std::make_unique<OpenscreenFrameSender>(cast_environment, config, std::move(sender), client, std::move(get_bitrate_cb));
}

// Convenience macro used in logging statements throughout this file.
#define VLOG_WITH_SSRC(N) VLOG(N) << (is_audio_ ? "AUDIO[" : "VIDEO[") << sender_->config().sender_ssrc << "] "

OpenscreenFrameSender::OpenscreenFrameSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& config,
    std::unique_ptr<openscreen::cast::Sender> sender, Client& client, FrameSender::GetSuggestedVideoBitrateCB get_bitrate_cb)
    ...
    , max_playout_delay_(config.max_playout_delay)
{
    ...
    if (!is_audio_) {
        bitrate_suggester_ = std::make_unique<VideoBitrateSuggester>(config, std::move(get_bitrate_cb));
    }

    const std::chrono::milliseconds target_playout_delay = sender_->config().target_playout_delay;
    VLOG_WITH_SSRC(1) << "target latency " << target_playout_delay.count() << "ms";
...
void OpenscreenFrameSender::SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay)
{
    if (send_target_playout_delay_ && target_playout_delay_ == new_target_playout_delay) {
        return;
    }

    new_target_playout_delay = std::clamp(new_target_playout_delay, min_playout_delay_, max_playout_delay_);
    VLOG_WITH_SSRC(2) << "Target playout delay changing from " << target_playout_delay_.InMilliseconds() << " ms to "
...
void OpenscreenFrameSender::OnFrameCanceled(openscreen::cast::FrameId frame_id)
{
...
void OpenscreenFrameSender::RecordLatestFrameTimestamps(FrameId frame_id, base::TimeTicks reference_time, RtpTimeTicks rtp_timestamp)
{
...
base::TimeDelta OpenscreenFrameSender::GetInFlightMediaDuration() const
{
    ...
    if (!last_enqueued_frame_id_.is_null()) {
        const RtpTimeTicks newest_timestamp = GetRecordedRtpTimestamp(last_enqueued_frame_id_);
        duration += ToTimeDelta(sender_->GetInFlightMediaDuration(newest_timestamp));
    }
    return duration;
}

RtpTimeTicks OpenscreenFrameSender::GetRecordedRtpTimestamp(FrameId frame_id) const
{
    if (static_cast<size_t>(std::abs(last_enqueued_frame_id_ - frame_id)) >= std::size(frame_rtp_timestamps_)) {
        return {};
    }
...
int OpenscreenFrameSender::GetSuggestedBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay)
{
...
{
    return base::Microseconds(std::chrono::duration_cast<std::chrono::microseconds>(sender_->GetCurrentRoundTripTime()).count());
}
...
CastStreamingFrameDropReason OpenscreenFrameSender::EnqueueFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    CHECK(encoded_frame);
    VLOG_WITH_SSRC(2) << "About to send another frame (" << encoded_frame->frame_id << "). last enqueued=" << last_enqueued_frame_id_;

    CHECK_GE(encoded_frame->frame_id, last_enqueued_frame_id_) << "enqueued frames out of order.";
    last_enqueued_frame_id_ = encoded_frame->frame_id;
    last_send_time_ = cast_environment_->Clock()->NowTicks();

    if (!is_audio_ && encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) {
        VLOG_WITH_SSRC(1) << "Sending encoded key frame, id=" << encoded_frame->frame_id;
        frame_id_map_.clear();
    }

    RecordLatestFrameTimestamps(encoded_frame->frame_id, encoded_frame->reference_time, encoded_frame->rtp_timestamp);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(
        "cast.stream", name, TRACE_ID_WITH_SCOPE(name, encoded_frame->frame_id.lower_32_bits()), "rtp_timestamp", encoded_frame->rtp_timestamp.lower_32_bits());
...
CastStreamingFrameDropReason OpenscreenFrameSender::ShouldDropNextFrame(base::TimeDelta frame_duration)
{
    ...
    if (VLOG_IS_ON(1)) {
        ...
        if (percent > 50) {
            VLOG_WITH_SSRC(1) << duration_in_flight.InMicroseconds() << " usec in-flight + " << frame_duration.InMicroseconds() << " usec for next frame --> "
