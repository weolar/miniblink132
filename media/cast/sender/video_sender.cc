...
constexpr char kHistogramDroppedFrames[] = "CastStreaming.Sender.Video.PercentDroppedFrames";

// UMA histogram for recording when a frame is dropped.
constexpr char kHistogramFrameDropped[] = "CastStreaming.Sender.Video.FrameDropped";

// Extract capture begin/end timestamps from |video_frame|'s metadata and log
// it.
void LogVideoCaptureTimestamps(CastEnvironment* cast_environment, const media::VideoFrame& video_frame, RtpTimeTicks rtp_timestamp)
{
    ...
    if (video_frame.metadata().capture_begin_time.has_value() && video_frame.metadata().capture_end_time.has_value()) {
...
VideoSender::VideoSender(scoped_refptr<CastEnvironment> cast_environment, const FrameSenderConfig& video_config, StatusChangeCallback status_change_cb,
    const CreateVideoEncodeAcceleratorCallback& create_vea_cb, std::unique_ptr<openscreen::cast::Sender> sender,
    std::unique_ptr<media::VideoEncoderMetricsProvider> encoder_metrics_provider, PlayoutDelayChangeCB playout_delay_change_cb,
    media::VideoCaptureFeedbackCB feedback_cb, FrameSender::GetSuggestedVideoBitrateCB get_bitrate_cb)
    : frame_sender_(FrameSender::Create(cast_environment, video_config, std::move(sender), *this, std::move(get_bitrate_cb)))
    ...
    , feedback_cb_(feedback_cb)
{
    ...
    video_encoder_ = VideoEncoder::Create(cast_environment_, video_config, std::move(encoder_metrics_provider), status_change_cb,
        base::BindRepeating(&VideoSender::OnEncodedVideoFrame, AsWeakPtr()), create_vea_cb);
    if (!video_encoder_) {
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(std::move(status_change_cb), STATUS_UNSUPPORTED_CODEC));
    }
}
...
VideoSender::~VideoSender()
{
    ...
    base::UmaHistogramPercentage(kHistogramDroppedFrames, (number_of_frames_dropped_ * 100) / std::max(1, number_of_frames_inserted_));
}

void VideoSender::InsertRawVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    CHECK(video_encoder_);

    const RtpTimeTicks rtp_timestamp = ToRtpTimeTicks(video_frame->timestamp(), kVideoFrequency);
    LogVideoCaptureTimestamps(cast_environment_.get(), *video_frame, rtp_timestamp);

    // Used by chrome/browser/media/cast_mirroring_performance_browsertest.cc
    TRACE_EVENT_INSTANT2("cast_perf_test", "InsertRawVideoFrame", TRACE_EVENT_SCOPE_THREAD, "timestamp", (reference_time - base::TimeTicks()).InMicroseconds(),
    ...
    if (!last_enqueued_frame_reference_time_.is_null()
        && (rtp_timestamp <= last_enqueued_frame_rtp_timestamp_ || reference_time <= last_enqueued_frame_reference_time_)) {
        VLOG(1) << "Dropping video frame: RTP or reference time did not increase.";
        TRACE_EVENT_INSTANT2(
            "cast.stream", "Video Frame Drop", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits(), "reason", "time did not increase");
        return;
    }
    ...
    if (frame_sender_->NeedsKeyFrame()) {
        const base::TimeDelta min_attempt_interval
            = std::max(kMinKeyFrameRequestInterval, kMinKeyFrameRequestFrameInterval * frame_sender_->TargetPlayoutDelay());

        if (last_time_attempted_to_resolve_pli_.is_null() || ((reference_time - last_time_attempted_to_resolve_pli_) > min_attempt_interval)) {
    ...
    const base::TimeDelta duration_added_by_next_frame
        = frames_in_encoder_ > 0 ? reference_time - last_enqueued_frame_reference_time_ : base::Seconds(1.0 / frame_sender_->MaxFrameRate());

    number_of_frames_inserted_++;
    const CastStreamingFrameDropReason reason = frame_sender_->ShouldDropNextFrame(duration_added_by_next_frame);
    if (reason != CastStreamingFrameDropReason::kNotDropped) {
        base::TimeDelta new_target_delay
            = std::min(frame_sender_->CurrentRoundTripTime() * kRoundTripsNeeded + base::Milliseconds(kConstantTimeMs), max_playout_delay_);
        ...
        TRACE_EVENT_INSTANT2(
            "cast.stream", "Video Frame Drop (raw frame)", TRACE_EVENT_SCOPE_THREAD, "duration", duration_added_by_next_frame, "reason", reason);
        return;
    }
    ...
    const int bitrate = frame_sender_->GetSuggestedBitrate(reference_time + frame_sender_->TargetPlayoutDelay(), frame_sender_->TargetPlayoutDelay());
    ...
    frames_since_bitrate_reported_ = ++frames_since_bitrate_reported_ % kSampleInterval;
    ...
    if (base::FeatureList::IsEnabled(media::kCastStreamingPerformanceOverlay)) {
        video_frame = RenderPerformanceMetricsOverlay(frame_sender_->GetTargetPlayoutDelay(), low_latency_mode_, bitrate, frames_in_encoder_ + 1,
            last_reported_encoder_utilization_, last_reported_lossiness_, std::move(video_frame));
    }

    if (video_encoder_->EncodeVideoFrame(video_frame, reference_time)) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("cast.stream", "Video Encode", TRACE_ID_LOCAL((reference_time - base::TimeTicks()).InMicroseconds()), "rtp_timestamp",
        ...
        TRACE_EVENT_INSTANT1("cast.stream", "Video Encode Reject", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits());
    }
}

void VideoSender::SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay)
{
...
void VideoSender::OnEncodedVideoFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END2("cast.stream", "Video Encode", TRACE_ID_LOCAL((encoded_frame->reference_time - base::TimeTicks()).InMicroseconds()),
        "encoder_utilization", last_reported_encoder_utilization_, "lossiness", last_reported_lossiness_);
    ...
    const double attenuated_utilization = std::max(last_reported_encoder_utilization_, last_reported_lossiness_) / (kTargetUtilizationPercentage / 100.0);
    if (attenuated_utilization >= 0.0) {
        ...
        feedback.resource_utilization = encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame
    ...
    const CastStreamingFrameDropReason reason = frame_sender_->EnqueueFrame(std::move(encoded_frame));
    if (reason != CastStreamingFrameDropReason::kNotDropped) {
        ...
        base::UmaHistogramEnumeration(kHistogramFrameDropped, reason);
        TRACE_EVENT_INSTANT2(
            "cast.stream", "Video Frame Drop (already encoded)", TRACE_EVENT_SCOPE_THREAD, "rtp_timestamp", rtp_timestamp.lower_32_bits(), "reason", reason);
    }
}
