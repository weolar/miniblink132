...
VideoBitrateSuggester::VideoBitrateSuggester(const FrameSenderConfig& config, FrameSender::GetSuggestedVideoBitrateCB get_bitrate_cb)
...
int VideoBitrateSuggester::GetSuggestedBitrate()
{
    ...
    const int suggested_bitrate = std::min(get_bitrate_cb_.Run(), suggested_max_bitrate_);
...
void VideoBitrateSuggester::RecordShouldDropNextFrame(bool should_drop)
{
    ...
    if (base::FeatureList::IsEnabled(media::kCastStreamingExponentialVideoBitrateAlgorithm)) {
...
void VideoBitrateSuggester::UpdateSuggestionUsingExponentialAlgorithm()
{
    ...
    if (number_of_frames_requested_ == kWindowSize) {
        ...
        // a bad sign.
        suggested_max_bitrate_ = (number_of_frames_dropped_ > 0) ? std::max<int>(min_bitrate_, suggested_max_bitrate_ * kDecrease)
...
void VideoBitrateSuggester::UpdateSuggestionUsingLinearAlgorithm()
{
    ...
    if (number_of_frames_requested_ == kWindowSize) {
        ...
        if (number_of_frames_dropped_ > 0) {
            suggested_max_bitrate_ = std::max(min_bitrate_, suggested_max_bitrate_ - adjustment);
        } else {
            suggested_max_bitrate_ = std::min(max_bitrate_, suggested_max_bitrate_ + adjustment);
        }
