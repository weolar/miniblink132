...
static const FrameSenderConfig kAudioConfig { kFirstSsrc, kFirstSsrc + 1, base::Milliseconds(100), kDefaultTargetPlayoutDelay, RtpPayloadType::AUDIO_OPUS,
    /* use_hardware_encoder= */ false, kDefaultAudioSamplingRate,
    /* channels= */ 2, kAudioBitrate, kAudioBitrate, kAudioBitrate, kDefaultMaxFrameRate, kAesSecretKey, kAesIvMask, std::nullopt,
    AudioCodecParams { .codec = AudioCodec::kOpus } };
static const openscreen::cast::SessionConfig kOpenscreenAudioConfig = ToOpenscreenSessionConfig(kAudioConfig, /* is_pli_enabled= */ true);

static const FrameSenderConfig kVideoConfig { kFirstSsrc + 2, kFirstSsrc + 3, base::Milliseconds(100), kDefaultTargetPlayoutDelay, RtpPayloadType::VIDEO_VP8,
    /* use_hardware_encoder= */ false, kRtpTimebase,
    /* channels = */ 1, kDefaultMaxVideoBitrate, kDefaultMinVideoBitrate, std::midpoint<int>(kDefaultMinVideoBitrate, kDefaultMaxVideoBitrate),
    kDefaultMaxFrameRate, kAesSecretKey, kAesIvMask, VideoCodecParams(VideoCodec::kVP8), std::nullopt };
static const openscreen::cast::SessionConfig kOpenscreenVideoConfig = ToOpenscreenSessionConfig(kVideoConfig, /* is_pli_enabled= */ true);

} // namespace

class OpenscreenFrameSenderTest : public ::testing::Test, public FrameSender::Client {
public:
    ...
    int get_suggested_bitrate()
    {
        return suggested_bitrate_;
    }

protected:
    OpenscreenFrameSenderTest()
        : task_runner_(base::MakeRefCounted<FakeSingleThreadTaskRunner>(&testing_clock_))
        , cast_environment_(base::MakeRefCounted<CastEnvironment>(&testing_clock_, task_runner_, task_runner_, task_runner_))
        , openscreen_task_runner_(task_runner_)
        , openscreen_environment_(openscreen::Clock::now, openscreen_task_runner_, openscreen::IPEndpoint::kAnyV4())
        , openscreen_packet_router_(openscreen_environment_, 20, std::chrono::milliseconds(10))
    {
        auto openscreen_audio_sender = std::make_unique<openscreen::cast::Sender>(
            openscreen_environment_, openscreen_packet_router_, kOpenscreenAudioConfig, openscreen::cast::RtpPayloadType::kAudioOpus);
        auto openscreen_video_sender = std::make_unique<openscreen::cast::Sender>(
            openscreen_environment_, openscreen_packet_router_, kOpenscreenVideoConfig, openscreen::cast::RtpPayloadType::kVideoVp8);

        audio_sender_ = std::make_unique<OpenscreenFrameSender>(cast_environment_, kAudioConfig, std::move(openscreen_audio_sender), *this,
            base::BindRepeating(&OpenscreenFrameSenderTest::get_suggested_bitrate,
                // Safe because we destroy the audio sender before
                // destroying `this`.
                base::Unretained(this)));

        video_sender_ = std::make_unique<OpenscreenFrameSender>(cast_environment_, kVideoConfig, std::move(openscreen_video_sender), *this,
            base::BindRepeating(&OpenscreenFrameSenderTest::get_suggested_bitrate,
                // Safe because we destroy the audio sender before
                // destroying `this`.
                base::Unretained(this)));
    ...
    void set_suggested_bitrate(int bitrate)
    {
        suggested_bitrate_ = bitrate;
    }

    OpenscreenFrameSender& audio_sender()
    {
        return *audio_sender_;
    }

    OpenscreenFrameSender& video_sender()
    {
...
TEST_F(OpenscreenFrameSenderTest, CanEnqueueFirstFrame)
{
    ...
    EXPECT_EQ(CastStreamingFrameDropReason::kNotDropped, audio_sender().EnqueueFrame(std::move(audio_frame)));
    ...
    EXPECT_EQ(CastStreamingFrameDropReason::kNotDropped, video_sender().EnqueueFrame(std::move(video_frame)));
}
...
TEST_F(OpenscreenFrameSenderTest, RecordsRtpTimestamps)
{
    ...
    EXPECT_EQ(CastStreamingFrameDropReason::kNotDropped, audio_sender().EnqueueFrame(std::move(audio_frame)));
    EXPECT_EQ(kAudioRtpTimestamp, audio_sender().GetRecordedRtpTimestamp(openscreen::cast::FrameId(1)));
    ...
    video_frame->data = base::HeapArray<uint8_t>::WithSize(10);
    EXPECT_EQ(CastStreamingFrameDropReason::kNotDropped, video_sender().EnqueueFrame(std::move(video_frame)));
    EXPECT_EQ(kVideoRtpTimestamp, video_sender().GetRecordedRtpTimestamp(openscreen::cast::FrameId(1)));

    // Should return empty if the frame ID is not recorded.
    EXPECT_EQ(RtpTimeTicks {}, audio_sender().GetRecordedRtpTimestamp(openscreen::cast::FrameId(2000)));
    EXPECT_EQ(RtpTimeTicks {}, video_sender().GetRecordedRtpTimestamp(openscreen::cast::FrameId(2000)));
}
...
TEST_F(OpenscreenFrameSenderTest, HandlesReferencingUnknownFrameIds)
{
    ...
    EXPECT_EQ(CastStreamingFrameDropReason::kInvalidReferencedFrameId, audio_sender().EnqueueFrame(std::move(audio_frame_two)));
    ...
    EXPECT_EQ(CastStreamingFrameDropReason::kInvalidReferencedFrameId, video_sender().EnqueueFrame(std::move(video_frame_two)));
}
...
TEST_F(OpenscreenFrameSenderTest, HandlesSuggestedBitratesCorrectly)
{
    ...
    EXPECT_EQ(5000000, video_sender().GetSuggestedBitrate(base::TimeTicks {}, base::TimeDelta {}));

    // It should cap at the bitrate suggested by Open Screen.
    set_suggested_bitrate(4998374);
    EXPECT_EQ(4998374, video_sender().GetSuggestedBitrate(base::TimeTicks {}, base::TimeDelta {}));
}
