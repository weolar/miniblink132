...
{
    DVLOG(1) << "OperationalStatus transitioning from " << *out_status << " to " << in_status;
...
void IgnoreVideoCaptureFeedback(const media::VideoCaptureFeedback& unused_feedback)
{
}
...
protected:
        ...
        , vea_factory_(task_runner_)
    {
        ...
        mock_openscreen_environment_ = std::make_unique<MockOpenscreenEnvironment>(&FakeOpenscreenClock::now, openscreen_task_runner_);
        openscreen_packet_router_ = std::make_unique<openscreen::cast::SenderPacketRouter>(*mock_openscreen_environment_);
    ...
    void InitEncoder(bool external, bool expect_init_success)
    {
        ...
        openscreen::cast::SessionConfig openscreen_video_config = ToOpenscreenSessionConfig(video_config, /* is_pli_enabled= */ true);
        ...
        auto openscreen_video_sender = std::make_unique<openscreen::cast::Sender>(
            *mock_openscreen_environment_, *openscreen_packet_router_, openscreen_video_config, openscreen::cast::RtpPayloadType::kVideoVp8);
        openscreen_video_sender_ = openscreen_video_sender.get();

        video_sender_ = std::make_unique<VideoSender>(cast_environment_, video_config, base::BindRepeating(&SaveOperationalStatus, &operational_status_),
            base::BindRepeating(&FakeVideoEncodeAcceleratorFactory::CreateVideoEncodeAccelerator, base::Unretained(&vea_factory_)),
            std::move(openscreen_video_sender), std::make_unique<media::MockVideoEncoderMetricsProvider>(), base::BindRepeating(&IgnorePlayoutDelayChanges),
            base::BindRepeating(&IgnoreVideoCaptureFeedback), base::BindRepeating(&GetSuggestedVideoBitrate));
    ...
    scoped_refptr<media::VideoFrame> GetNewVideoFrame()
    {
        ...
        scoped_refptr<media::VideoFrame> video_frame
            = media::VideoFrame::CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, testing_clock_.NowTicks() - first_frame_timestamp_);
    ...
    std::unique_ptr<media::cast::MockOpenscreenEnvironment> mock_openscreen_environment_;
    std::unique_ptr<openscreen::cast::SenderPacketRouter> openscreen_packet_router_;
...
TEST_F(VideoSenderTest, ExternalEncoder)
{
    ...
    if (vea_factory_.vea_response_count() == 0) {
        video_sender_->InsertRawVideoFrame(GetNewVideoFrame(), testing_clock_.NowTicks());
        task_runner_->RunTasks();
    }
    ASSERT_EQ(STATUS_INITIALIZED, operational_status_);
    RunTasks(33);
