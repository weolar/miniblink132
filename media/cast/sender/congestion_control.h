...
public:
    ...
    virtual void WillSendFrameToTransport(FrameId frame_id, size_t frame_size_in_bytes, base::TimeTicks when) = 0;
    ...
    virtual void AckLaterFrames(std::vector<FrameId> received_frames, base::TimeTicks when) = 0;

    // Returns the bitrate we should use for the next frame.
    virtual int GetBitrate(base::TimeTicks playout_time, base::TimeDelta playout_delay) = 0;
};
