...
protected:
        ...
        , openscreen_task_runner_(task_runner_)
    {
        ...
        audio_config_.audio_codec_params = AudioCodecParams { .codec = AudioCodec::kOpus };
        ...
        openscreen::cast::SessionConfig openscreen_audio_config = ToOpenscreenSessionConfig(audio_config_, /* is_pli_enabled= */ true);

        auto openscreen_audio_sender = std::make_unique<openscreen::cast::Sender>(
            *mock_openscreen_environment_, *openscreen_packet_router_, openscreen_audio_config, openscreen::cast::RtpPayloadType::kAudioOpus);
        ...
        audio_sender_ = std::make_unique<AudioSender>(
            cast_environment_, audio_config_, base::BindOnce(&SaveOperationalStatus, &operational_status), std::move(openscreen_audio_sender));
...
TEST_F(AudioSenderTest, Encode20ms)
{
    ...
    std::unique_ptr<AudioBus> bus(
        TestAudioBusFactory(audio_config_.channels, audio_config_.rtp_timebase, TestAudioBusFactory::kMiddleANoteFreq, 0.5f).NextAudioBus(kDuration));
