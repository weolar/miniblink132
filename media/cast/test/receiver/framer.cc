...
Framer::Framer(
    const base::TickClock* clock, RtpPayloadFeedback* incoming_payload_feedback, uint32_t ssrc, bool decoder_faster_than_max_frame_rate, int max_unacked_frames)
...
bool Framer::InsertPacket(const uint8_t* payload_data, size_t payload_size, const RtpCastHeader& rtp_header, bool* duplicate)
{
    *duplicate = false;
    ...
    VLOG(1) << "InsertPacket frame:" << rtp_header.frame_id << " packet:" << static_cast<int>(rtp_header.packet_id)
    ...
    if (it == frames_.end()) {
        ...
        frames_.insert(std::make_pair(rtp_header.frame_id, std::unique_ptr<FrameBuffer>(buffer)));
    } else {
        buffer = it->second.get();
    }
    if (!buffer->InsertPacket(payload_data, payload_size, rtp_header)) {
        VLOG(3) << "Packet already received, ignored: frame " << rtp_header.frame_id << ", packet " << rtp_header.packet_id;
        *duplicate = true;
        return false;
    }
...
bool Framer::GetEncodedFrame(EncodedFrame* frame, bool* next_frame, bool* have_multiple_decodable_frames)
{
...
void Framer::GetMissingPackets(FrameId frame_id, bool last_frame, PacketIdSet* missing_packets) const
{
