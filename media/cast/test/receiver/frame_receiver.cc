...
FrameReceiver::FrameReceiver(
    const scoped_refptr<CastEnvironment>& cast_environment, const FrameReceiverConfig& config, EventMediaType event_media_type, CastTransport* const transport)
    : cast_environment_(cast_environment)
    , transport_(transport)
    , packet_parser_(config.sender_ssrc, config.rtp_payload_type <= RtpPayloadType::AUDIO_LAST ? 127 : 96)
    ...
    , framer_(cast_environment->Clock(), this, config.sender_ssrc, true, static_cast<int>(config.rtp_max_delay_ms * config.target_frame_rate / 1000))
    , rtcp_(cast_environment_->Clock(), config.receiver_ssrc, config.sender_ssrc)
...
bool FrameReceiver::ProcessPacket(std::unique_ptr<Packet> packet)
{
    ...
    if (IsRtcpPacket(&packet->front(), packet->size())) {
        ...
        size_t payload_size;
        if (!packet_parser_.ParsePacket(&packet->front(), packet->size(), &rtp_header, &payload_data, &payload_size)) {
            return false;
        }
...
void FrameReceiver::ProcessParsedPacket(const RtpCastHeader& rtp_header, const uint8_t* payload_data, size_t payload_size)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    if (rtp_header.packet_id == 0 || lip_sync_reference_time_.is_null()) {
        ...
        if (lip_sync_reference_time_.is_null()) {
            ...
            lip_sync_reference_time_ += ToTimeDelta(fresh_sync_rtp - lip_sync_rtp_timestamp_, rtp_timebase_);
        }
        lip_sync_rtp_timestamp_ = fresh_sync_rtp;
        lip_sync_drift_.Update(now, fresh_sync_reference - lip_sync_reference_time_);
    }
...
void FrameReceiver::CastFeedback(const RtcpCastMessage& cast_message)
{
    ...
    SendRtcpReport(rtcp_.local_ssrc(), rtcp_.remote_ssrc(), CreateRtcpTimeData(now), &cast_message, nullptr, target_playout_delay_, &rtcp_events, nullptr);
}
...
void FrameReceiver::EmitAvailableEncodedFrames()
{
    ...
    while (!frame_request_queue_.empty()) {
        ...
        if (!framer_.GetEncodedFrame(encoded_frame.get(), &is_consecutively_next_frame, &have_multiple_complete_frames)) {
            ...
            if (earliest_possible_end_time_of_missing_frame < playout_time) {
                ...
                if (!is_waiting_for_consecutive_frame_) {
                    ...
                    cast_environment_->PostDelayedTask(CastEnvironment::MAIN, FROM_HERE,
                        base::BindOnce(&FrameReceiver::EmitAvailableEncodedFramesAfterWaiting, AsWeakPtr()), playout_time - now);
                }
                return;
            }
        }
        ...
        if (decryptor_.is_activated()) {
            ...
            if (!decryptor_.Decrypt(encoded_frame->frame_id, encoded_frame->data, &decrypted_data)) {
        ...
        if (encoded_frame->new_playout_delay_ms) {
            target_playout_delay_ = base::Milliseconds(encoded_frame->new_playout_delay_ms);
        }
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE,
            base::BindOnce(&FrameReceiver::EmitOneFrame, AsWeakPtr(), std::move(*frame_request_queue_.begin()), std::move(encoded_frame)));
...
void FrameReceiver::EmitOneFrame(ReceiveEncodedFrameCallback callback, std::unique_ptr<EncodedFrame> encoded_frame) const
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    if (!callback.is_null())
...
base::TimeTicks FrameReceiver::GetPlayoutTime(const EncodedFrame& frame) const
{
    ...
    return lip_sync_reference_time_ + lip_sync_drift_.Current() + ToTimeDelta(frame.rtp_timestamp - lip_sync_rtp_timestamp_, rtp_timebase_)
...
void FrameReceiver::ScheduleNextCastMessage()
{
    ...
    time_to_send = std::max(time_to_send, base::Milliseconds(kMinSchedulingDelayMs));
    cast_environment_->PostDelayedTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameReceiver::SendNextCastMessage, AsWeakPtr()), time_to_send);
}
...
void FrameReceiver::ScheduleNextRtcpReport()
{
    ...
    cast_environment_->PostDelayedTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&FrameReceiver::SendNextRtcpReport, AsWeakPtr()), kRtcpReportInterval);
}
...
void FrameReceiver::SendNextRtcpReport()
{
    ...
    SendRtcpReport(rtcp_.local_ssrc(), rtcp_.remote_ssrc(), CreateRtcpTimeData(now), nullptr, nullptr, base::TimeDelta(), nullptr, &stats);
    ScheduleNextRtcpReport();
}

void FrameReceiver::SendRtcpReport(uint32_t rtp_receiver_ssrc, uint32_t rtp_sender_ssrc, const RtcpTimeData& time_data, const RtcpCastMessage* cast_message,
    const RtcpPliMessage* pli_message, base::TimeDelta target_delay, const ReceiverRtcpEventSubscriber::RtcpEvents* rtcp_events,
