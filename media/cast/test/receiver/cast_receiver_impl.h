...
public:
    CastReceiverImpl(scoped_refptr<CastEnvironment> cast_environment, const FrameReceiverConfig& audio_config, const FrameReceiverConfig& video_config,
    ...
    void RequestDecodedAudioFrame(const AudioFrameDecodedCallback& callback) final;
    void RequestDecodedVideoFrame(const VideoFrameDecodedCallback& callback) final;

private:
    // Feeds an EncodedFrame into |audio_decoder_|.  RequestDecodedAudioFrame()
    // uses this as a callback for RequestEncodedAudioFrame().
    void DecodeEncodedAudioFrame(const AudioFrameDecodedCallback& callback, std::unique_ptr<EncodedFrame> encoded_frame);

    // Feeds an EncodedFrame into |video_decoder_|.  RequestDecodedVideoFrame()
    // uses this as a callback for RequestEncodedVideoFrame().
    void DecodeEncodedVideoFrame(const VideoFrameDecodedCallback& callback, std::unique_ptr<EncodedFrame> encoded_frame);
    ...
    static void EmitDecodedAudioFrame(const scoped_refptr<CastEnvironment>& cast_environment, const AudioFrameDecodedCallback& callback, FrameId frame_id,
        RtpTimeTicks rtp_timestamp, base::TimeTicks playout_time, std::unique_ptr<AudioBus> audio_bus, bool is_continuous);
    ...
    static void EmitDecodedVideoFrame(const scoped_refptr<CastEnvironment>& cast_environment, const VideoFrameDecodedCallback& callback, FrameId frame_id,
        RtpTimeTicks rtp_timestamp, base::TimeTicks playout_time, scoped_refptr<VideoFrame> video_frame, bool is_continuous);
