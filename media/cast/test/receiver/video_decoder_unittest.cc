...
protected:
    void SetUp() final
    {
        video_decoder_ = std::make_unique<VideoDecoder>(cast_environment_, GetParam());
    ...
    void FeedMoreVideo(int num_dropped_frames)
    {
        ...
        const scoped_refptr<VideoFrame> video_frame
            = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, next_frame_size_, gfx::Rect(next_frame_size_), next_frame_size_, next_frame_timestamp_);
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE,
            base::BindOnce(&VideoDecoder::DecodeFrame, base::Unretained(video_decoder_.get()), std::move(encoded_frame),
                base::BindRepeating(&VideoDecoderTest::OnDecodedFrame, base::Unretained(this), video_frame, num_dropped_frames == 0)));
    }
...
private:
    ...
    void OnDecodedFrame(scoped_refptr<VideoFrame> expected_video_frame, bool should_be_continuous, scoped_refptr<VideoFrame> video_frame, bool is_continuous)
    {
        DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
        ...
        EXPECT_EQ(expected_video_frame->coded_size().width(), video_frame->coded_size().width());
        EXPECT_EQ(expected_video_frame->coded_size().height(), video_frame->coded_size().height());
...
INSTANTIATE_TEST_SUITE_P(All, VideoDecoderTest, ::testing::Values(CODEC_VIDEO_VP8));
