...
class Framer {
public:
    Framer(const base::TickClock* clock, RtpPayloadFeedback* incoming_payload_feedback, uint32_t ssrc, bool decoder_faster_than_max_frame_rate,
    ...
    bool InsertPacket(const uint8_t* payload_data, size_t payload_size, const RtpCastHeader& rtp_header, bool* duplicate);
    ...
    bool GetEncodedFrame(EncodedFrame* video_frame, bool* next_frame, bool* have_multiple_complete_frames);
    ...
    void GetMissingPackets(FrameId frame_id, bool last_frame, PacketIdSet* missing_packets) const;
