...
class VideoDecoder::ImplBase : public base::RefCountedThreadSafe<VideoDecoder::ImplBase> {
public:
    ...
    void DecodeFrame(std::unique_ptr<EncodedFrame> encoded_frame, const DecodeFrameCallback& callback)
    {
        ...
        const scoped_refptr<VideoFrame> decoded_frame = Decode(encoded_frame->mutable_bytes(), static_cast<int>(encoded_frame->data.size()));
        if (!decoded_frame) {
            VLOG(2) << "Decoding of frame " << encoded_frame->frame_id << " failed.";
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(callback, decoded_frame, false));
            return;
        }
        decoded_frame->set_timestamp(ToTimeDelta(encoded_frame->rtp_timestamp, kVideoFrequency));
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(callback, decoded_frame, is_continuous));
    }
...
public:
        ...
        : ImplBase(cast_environment, CODEC_VIDEO_VP8)
    {
        ...
        if (vpx_codec_dec_init(&context_, vpx_codec_vp8_dx(), &cfg, VPX_CODEC_USE_POSTPROC) != VPX_CODEC_OK) {
...
private:
    ...
    {
        if (len <= 0 || vpx_codec_decode(&context_, data, static_cast<unsigned int>(len), nullptr, 0) != VPX_CODEC_OK) {
            return nullptr;
        }
        ...
        DCHECK(vpx_codec_get_frame(&context_, &iter) == nullptr) << "Should have only decoded exactly one frame.";
        ...
        const scoped_refptr<VideoFrame> decoded_frame
            = video_frame_pool_.CreateFrame(PIXEL_FORMAT_I420, frame_size, gfx::Rect(frame_size), frame_size, base::TimeDelta());
        libyuv::I420Copy(image->planes[VPX_PLANE_Y], image->stride[VPX_PLANE_Y], image->planes[VPX_PLANE_U], image->stride[VPX_PLANE_U],
            image->planes[VPX_PLANE_V], image->stride[VPX_PLANE_V], decoded_frame->GetWritableVisibleData(media::VideoFrame::kYPlane),
            decoded_frame->stride(media::VideoFrame::kYPlane), decoded_frame->GetWritableVisibleData(media::VideoFrame::kUPlane),
            decoded_frame->stride(media::VideoFrame::kUPlane), decoded_frame->GetWritableVisibleData(media::VideoFrame::kVPlane),
            decoded_frame->stride(media::VideoFrame::kVPlane), frame_size.width(), frame_size.height());
...
private:
    ...
    scoped_refptr<VideoFrame> Decode(uint8_t* data, int len) final
    {
        ...
        if (!len || data[0] != '{')
            return nullptr;
        std::unique_ptr<base::Value> values(base::JSONReader::ReadDeprecated(base::StringPiece(reinterpret_cast<char*>(data), len)));
...
VideoDecoder::VideoDecoder(const scoped_refptr<CastEnvironment>& cast_environment, Codec codec)
    : cast_environment_(cast_environment)
{
    switch (codec) {
...
void VideoDecoder::DecodeFrame(std::unique_ptr<EncodedFrame> encoded_frame, const DecodeFrameCallback& callback)
{
    ...
    cast_environment_->PostTask(
        CastEnvironment::VIDEO, FROM_HERE, base::BindOnce(&VideoDecoder::ImplBase::DecodeFrame, impl_, std::move(encoded_frame), callback));
}
