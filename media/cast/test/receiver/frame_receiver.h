...
using ReceiveEncodedFrameCallback = base::OnceCallback<void(std::unique_ptr<EncodedFrame>)>;
...
public:
    FrameReceiver(const scoped_refptr<CastEnvironment>& cast_environment, const FrameReceiverConfig& config, EventMediaType event_media_type,
...
protected:
    ...
    void ProcessParsedPacket(const RtpCastHeader& rtp_header, const uint8_t* payload_data, size_t payload_size);
...
private:
    ...
    // is run.
    void EmitOneFrame(ReceiveEncodedFrameCallback callback, std::unique_ptr<EncodedFrame> encoded_frame) const;
    ...
    void SendRtcpReport(uint32_t rtp_receiver_ssrc, uint32_t rtp_sender_ssrc, const RtcpTimeData& time_data, const RtcpCastMessage* cast_message,
