...
public:
    ...
    void CastFeedback(const RtcpCastMessage& cast_feedback) final
    {
        ...
        while (frame_it != cast_feedback.missing_frames_and_packets.end()) {
            ...
            if ((frame_it->second.size() == 1) && (*frame_it->second.begin() == kRtcpCastAllPacketsLost)) {
                missing_packets_.insert(std::make_pair(frame_it->first, kRtcpCastAllPacketsLost));
            } else {
                missing_packets_.insert(std::make_pair(frame_it->first, frame_it->second.size()));
            }
            ++frame_it;
        }
        triggered_ = true;
    ...
    FrameId last_frame_acked()
    {
        return last_frame_acked_;
    }
...
protected:
    CastMessageBuilderTest()
        ...
        , cast_msg_builder_(new CastMessageBuilder(&testing_clock_, &feedback_, &framer_, kSsrc, true, 0))
    {
    ...
    {
        cast_msg_builder_ = std::make_unique<CastMessageBuilder>(&testing_clock_, &feedback_, &framer_, kSsrc, false, max_unacked_frames);
    }
...
TEST_F(CastMessageBuilderTest, CompleteFrameMissing)
{
    ...
    InsertPacket();
    EXPECT_TRUE(feedback_.triggered());
    EXPECT_EQ(kRtcpCastAllPacketsLost, feedback_.num_missing_packets(FrameId::first() + 1));
}
...
TEST_F(CastMessageBuilderTest, SlowDownAck)
{
    ...
    testing_clock_.Advance(base::Milliseconds(kShortTimeIncrementMs));
    SetKeyFrame(false);
    for (frame_id = FrameId::first() + 1; frame_id < FrameId::first() + 3; ++frame_id) {
        EXPECT_TRUE(feedback_.triggered());
