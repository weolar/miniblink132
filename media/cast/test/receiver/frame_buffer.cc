...
bool FrameBuffer::InsertPacket(const uint8_t* payload_data, size_t payload_size, const RtpCastHeader& rtp_header)
{
    ...
    std::copy(payload_data, payload_data + payload_size, retval.first->second.begin());
...
void FrameBuffer::GetMissingPackets(bool newest_frame, PacketIdSet* missing_packets) const
{
    ...
    int packet = 0;
    for (auto i = packets_.begin(); i != packets_.end() && packet <= maximum; ++i) {
