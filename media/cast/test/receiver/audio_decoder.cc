...
class AudioDecoder::ImplBase : public base::RefCountedThreadSafe<AudioDecoder::ImplBase> {
public:
    ImplBase(const scoped_refptr<CastEnvironment>& cast_environment, Codec codec, int num_channels, int sampling_rate)
    ...
    void DecodeFrame(std::unique_ptr<EncodedFrame> encoded_frame, DecodeFrameCallback callback)
    {
        ...
        std::unique_ptr<AudioBus> decoded_audio = Decode(encoded_frame->mutable_bytes(), static_cast<int>(encoded_frame->data.size()));
        if (!decoded_audio) {
            VLOG(2) << "Decoding of frame " << encoded_frame->frame_id << " failed.";
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(std::move(callback), std::move(decoded_audio), false));
            return;
        }
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(std::move(callback), std::move(decoded_audio), is_continuous));
    }
...
public:
    OpusImpl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate)
        : ImplBase(cast_environment, CODEC_AUDIO_OPUS, num_channels, sampling_rate)
...
private:
    ...
    void RecoverBecauseFramesWereDropped() final
    {
        ...
        const opus_int32 result = opus_decode_float(opus_decoder_, nullptr, 0, buffer_.get(), max_samples_per_frame_, 0);
        DCHECK_GE(result, 0);
    }
    ...
    std::unique_ptr<AudioBus> Decode(uint8_t* data, int len) final
    {
        ...
        const opus_int32 num_samples_decoded = opus_decode_float(opus_decoder_, data, len, buffer_.get(), max_samples_per_frame_, 0);
...
public:
    Pcm16Impl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate)
        : ImplBase(cast_environment, CODEC_AUDIO_PCM16, num_channels, sampling_rate)
    {
        if (ImplBase::operational_status_ != STATUS_UNINITIALIZED)
            return;
        ImplBase::operational_status_ = STATUS_INITIALIZED;
...
private:
    ...
    std::unique_ptr<AudioBus> Decode(uint8_t* data, int len) final
    {
        ...
        audio_bus->FromInterleaved<SignedInt16SampleTypeTraits>(pcm_data, num_samples);
        return audio_bus;
    }
};

AudioDecoder::AudioDecoder(const scoped_refptr<CastEnvironment>& cast_environment, int channels, int sampling_rate, Codec codec)
    : cast_environment_(cast_environment)
{
    switch (codec) {
...
void AudioDecoder::DecodeFrame(std::unique_ptr<EncodedFrame> encoded_frame, DecodeFrameCallback callback)
{
    ...
    cast_environment_->PostTask(
        CastEnvironment::AUDIO, FROM_HERE, base::BindOnce(&AudioDecoder::ImplBase::DecodeFrame, impl_, std::move(encoded_frame), std::move(callback)));
}
