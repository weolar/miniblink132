...
public:
    ...
    // in this case.
    using DecodeFrameCallback = base::OnceCallback<void(std::unique_ptr<AudioBus> audio_bus, bool is_continuous)>;

    AudioDecoder(const scoped_refptr<CastEnvironment>& cast_environment, int channels, int sampling_rate, Codec codec);
