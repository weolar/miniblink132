...
protected:
    void SetUp() final
    {
        audio_decoder_.reset(new AudioDecoder(cast_environment_, GetParam().num_channels, GetParam().sampling_rate, GetParam().codec));
        CHECK_EQ(STATUS_INITIALIZED, audio_decoder_->InitializationResult());

        audio_bus_factory_.reset(new TestAudioBusFactory(GetParam().num_channels, GetParam().sampling_rate, TestAudioBusFactory::kMiddleANoteFreq, 0.5f));
        last_frame_id_ = FrameId::first();
        decoded_frames_seen_ = 0;

        if (GetParam().codec == CODEC_AUDIO_OPUS) {
            opus_encoder_memory_.reset(new uint8_t[opus_encoder_get_size(GetParam().num_channels)]);
            ...
            CHECK_EQ(OPUS_OK, opus_encoder_ctl(opus_encoder, OPUS_SET_BITRATE(OPUS_AUTO)));
        }
    ...
    void FeedMoreAudio(base::TimeDelta duration, int num_dropped_frames)
    {
        ...
        const std::unique_ptr<AudioBus> audio_bus(audio_bus_factory_->NextAudioBus(duration));
        ...
        audio_bus->ToInterleaved<SignedInt16SampleTypeTraits>(audio_bus->frames(), &interleaved.front());
        if (GetParam().codec == CODEC_AUDIO_PCM16) {
            ...
            const int payload_size
                = opus_encode(opus_encoder, &interleaved.front(), audio_bus->frames(), encoded_frame->mutable_bytes(), encoded_frame->data.size());
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE,
            base::BindOnce(&AudioDecoder::DecodeFrame, base::Unretained(audio_decoder_.get()), std::move(encoded_frame),
                base::BindRepeating(&AudioDecoderTest::OnDecodedFrame, base::Unretained(this), num_dropped_frames == 0)));
    }
    ...
    void WaitForAllAudioToBeDecoded()
    {
        ...
        while (total_audio_decoded_ < total_audio_feed_in_)
            cond_.Wait();
        EXPECT_EQ(total_audio_feed_in_.InMicroseconds(), total_audio_decoded_.InMicroseconds());
    }

private:
    // Called by |audio_decoder_| to deliver each frame of decoded audio.
    void OnDecodedFrame(bool should_be_continuous, std::unique_ptr<AudioBus> audio_bus, bool is_continuous)
    {
        DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
        ...
        if (examine_signal) {
            for (int ch = 0; ch < audio_bus->channels(); ++ch) {
                EXPECT_NEAR(TestAudioBusFactory::kMiddleANoteFreq * 2 * audio_bus->frames() / GetParam().sampling_rate,
        ...
        base::AutoLock auto_lock(lock_);
        total_audio_decoded_ += base::Seconds(1) * audio_bus->frames() / GetParam().sampling_rate;
        cond_.Signal();
    }
...
INSTANTIATE_TEST_SUITE_P(AudioDecoderTestScenarios, AudioDecoderTest,
    ::testing::Values(TestScenario(CODEC_AUDIO_PCM16, 1, 8000), TestScenario(CODEC_AUDIO_PCM16, 2, 48000), TestScenario(CODEC_AUDIO_OPUS, 1, 8000),
