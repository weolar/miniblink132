...
void ReceiverStats::UpdateStatistics(const RtpCastHeader& header, int rtp_timebase)
{
    ...
    if (total_number_packets_ > 0) {
        ...
        const base::TimeDelta media_time_differerence = ToTimeDelta(header.rtp_timestamp - last_received_rtp_timestamp_, rtp_timebase);
