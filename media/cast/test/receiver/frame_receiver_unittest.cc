...
public:
    ...
    void AddExpectedResult(FrameId expected_frame_id, base::TimeTicks expected_playout_time)
    {
        expected_results_.push_back(std::make_pair(expected_frame_id, expected_playout_time));
    }

    void DeliverEncodedFrame(std::unique_ptr<EncodedFrame> frame)
    {
        SCOPED_TRACE(::testing::Message() << "num_called_ is " << num_called_);
        ASSERT_TRUE(frame) << "If at shutdown: There were unsatisfied requests enqueued.";
...
protected:
    ...
    void CreateFrameReceiverOfAudio()
    {
        ...
        receiver_ = std::make_unique<FrameReceiver>(cast_environment_, config_, AUDIO_EVENT, &mock_transport_);
    }
    ...
    void CreateFrameReceiverOfVideo()
    {
        ...
        receiver_ = std::make_unique<FrameReceiver>(cast_environment_, config_, VIDEO_EVENT, &mock_transport_);
    }
    ...
    void FeedLipSyncInfoIntoReceiver()
    {
        ...
        const RtpTimeTicks rtp_timestamp = ToRtpTimeTicks(now - start_time_, config_.rtp_timebase);
        CHECK_LE(RtpTimeTicks(), rtp_timestamp);
        uint32_t ntp_seconds;
        uint32_t ntp_fraction;
        ConvertTimeTicksToNtp(now, &ntp_seconds, &ntp_fraction);
        TestRtcpPacketBuilder rtcp_packet;
        rtcp_packet.AddSrWithNtp(config_.sender_ssrc, ntp_seconds, ntp_fraction, rtp_timestamp.lower_32_bits());
...
{
    EXPECT_CALL(mock_transport_, AddValidRtpReceiver(_, _)).WillRepeatedly(testing::Return());
    ...
    const bool success = receiver_->ProcessPacket(std::make_unique<Packet>(kPacketSize, 0xff));
...
TEST_F(FrameReceiverTest, ReceivesOneFrame)
{
    ...
    EXPECT_CALL(mock_transport_, InitializeRtpReceiverRtcpBuilder(_, _)).WillRepeatedly(testing::Return());
    EXPECT_CALL(mock_transport_, AddCastFeedback(_, _)).WillRepeatedly(testing::Return());
    EXPECT_CALL(mock_transport_, AddPli(_)).WillRepeatedly(testing::Return());
    EXPECT_CALL(mock_transport_, AddRtcpEvents(_)).WillRepeatedly(testing::Return());
    EXPECT_CALL(mock_transport_, SendRtcpFromRtpReceiver()).WillRepeatedly(testing::Return());

    FeedLipSyncInfoIntoReceiver();
    task_runner_->RunTasks();

    // Enqueue a request for a frame.
    receiver_->RequestEncodedFrame(base::BindOnce(&FakeFrameClient::DeliverEncodedFrame, base::Unretained(&frame_client_)));
    ...
    const base::TimeDelta target_playout_delay = base::Milliseconds(kPlayoutDelayMillis);
    frame_client_.AddExpectedResult(GetFirstTestFrameId(), testing_clock_.NowTicks() + target_playout_delay);
...
TEST_F(FrameReceiverTest, ReceivesFramesSkippingWhenAppropriate)
{
    ...
    const base::TimeDelta time_advance_per_frame = base::Seconds(1) / config_.target_frame_rate;
    const RtpTimeDelta rtp_advance_per_frame = ToRtpTimeDelta(time_advance_per_frame, config_.rtp_timebase);
    ...
    const base::TimeDelta target_playout_delay = base::Milliseconds(kPlayoutDelayMillis);
    frame_client_.AddExpectedResult(GetFirstTestFrameId(), first_frame_capture_time + target_playout_delay);
    rtp_header_.rtp_timestamp = RtpTimeTicks();
    ...
    frame_client_.AddExpectedResult(rtp_header_.frame_id, first_frame_capture_time + 2 * time_advance_per_frame + target_playout_delay);
    ...
    frame_client_.AddExpectedResult(rtp_header_.frame_id, first_frame_capture_time + 3 * time_advance_per_frame + target_playout_delay);
    ...
    for (size_t i = 0; i < frame_events.size(); ++i) {
        ...
        EXPECT_EQ(RtpTimeTicks() + (rtp_advance_per_frame * frame_offset), frame_events[i].rtp_timestamp);
    }
    cast_environment_->logger()->Unsubscribe(&event_subscriber);
}
