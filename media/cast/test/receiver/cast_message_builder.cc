...
CastMessageBuilder::CastMessageBuilder(const base::TickClock* clock, RtpPayloadFeedback* incoming_payload_feedback, const Framer* framer, uint32_t media_ssrc,
...
bool CastMessageBuilder::UpdateAckMessage(FrameId frame_id)
{
    ...
    last_update_time_ = clock_->NowTicks();
    time_last_nacked_map_.erase(time_last_nacked_map_.begin(), time_last_nacked_map_.upper_bound(last_acked_frame_id()));
    return true;
}

bool CastMessageBuilder::TimeToSendNextCastMessage(base::TimeTicks* time_to_send)
{
    ...
    for (; next_expected_frame_id <= newest_frame_id; ++next_expected_frame_id) {
        ...
        if (framer_->FrameExists(next_expected_frame_id)) {
            ...
            if (!missing.empty()) {
                ...
                cast_msg_.missing_frames_and_packets.insert(std::make_pair(next_expected_frame_id, missing));
            }
        } else {
            time_last_nacked_map_[next_expected_frame_id] = now;
