...
public:
    ...
    FakeMediaSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock, const FrameSenderConfig& audio_config,
    ...
    void Start(scoped_refptr<AudioFrameInput> audio_frame_input, scoped_refptr<VideoFrameInput> video_frame_input);

    const FrameSenderConfig& get_video_config() const
    {
...
private:
    bool is_transcoding_audio() const
    {
        return audio_stream_index_ >= 0;
    }
    bool is_transcoding_video() const
    {
    ...
    double ProvideInput(media::AudioBus* output_bus, uint32_t frames_delayed) final;
