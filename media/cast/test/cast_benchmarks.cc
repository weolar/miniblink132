...
public:
    ...
    void Init(CastTransport* transport, uint64_t* encoded_video_bytes, uint64_t* encoded_audio_bytes)
    {
    ...
    void InitializeStream(const CastTransportRtpConfig& config, std::unique_ptr<RtcpObserver> rtcp_observer) final
    {
    ...
    void SendSenderReport(uint32_t ssrc, base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp) final
    {
        transport_->SendSenderReport(ssrc, current_time, current_time_as_rtp_timestamp);
    }

    void CancelSendingFrames(uint32_t ssrc, const std::vector<FrameId>& frame_ids) final
    {
    ...
    void AddValidRtpReceiver(uint32_t rtp_sender_ssrc, uint32_t rtp_receiver_ssrc) final
    {
        return transport_->AddValidRtpReceiver(rtp_sender_ssrc, rtp_receiver_ssrc);
    }

    void InitializeRtpReceiverRtcpBuilder(uint32_t rtp_receiver_ssrc, const RtcpTimeData& time_data) final
    {
        transport_->InitializeRtpReceiverRtcpBuilder(rtp_receiver_ssrc, time_data);
    }

    void AddCastFeedback(const RtcpCastMessage& cast_message, base::TimeDelta target_delay) final
    {
        transport_->AddCastFeedback(cast_message, target_delay);
    }

    void AddRtcpEvents(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events) final
    {
...
struct MeasuringPoint {
    ...
    {
        return base::StringPrintf("%f Mbit/s %f ms %f %% ", bitrate, latency, percent_packet_drop);
    }
...
public:
    ...
    void Configure(Codec video_codec, Codec audio_codec)
    {
        ...
        audio_sender_config_.min_playout_delay = audio_sender_config_.max_playout_delay = base::Milliseconds(kTargetPlayoutDelayMs);
        ...
        video_sender_config_.min_playout_delay = video_sender_config_.max_playout_delay = base::Milliseconds(kTargetPlayoutDelayMs);
    ...
    {
        return frame_number * base::Seconds(1.0 / video_sender_config_.max_frame_rate);
    }
    ...
    void SendFakeVideoFrame()
    {
        ...
        cast_sender_->video_frame_input()->InsertRawVideoFrame(
            media::VideoFrame::CreateColorFrame(gfx::Size(2, 2), 0x00, 0x80, 0x80, VideoTimestamp(frames_sent_)), testing_clock_sender_.NowTicks());
        frames_sent_++;
    }
    ...
    void BasicPlayerGotVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks render_time, bool continuous)
    {
        video_ticks_.push_back(std::make_pair(testing_clock_receiver_.NowTicks(), render_time));
        cast_receiver_->RequestDecodedVideoFrame(base::BindRepeating(&RunOneBenchmark::BasicPlayerGotVideoFrame, base::Unretained(this)));
    }
    ...
    void BasicPlayerGotAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
    {
        ...
        cast_receiver_->RequestDecodedAudioFrame(base::BindRepeating(&RunOneBenchmark::BasicPlayerGotAudioFrame, base::Unretained(this)));
    }
    ...
    void Run(const MeasuringPoint& p)
    {
        ...
        VLOG(1) << "Video bandwidth used: " << video_bandwidth() << " mbit/s (" << (video_bandwidth() * 100 / desired_video_bitrate()) << "%)";
        VLOG(1) << "Good run: " << SimpleGood();
    }

    // Metrics
    int frames_lost() const
    {
        return frames_sent_ - video_ticks_.size();
    }
    ...
    test::MeanAndError frame_playout_buffer() const
    {
        ...
        for (size_t i = 0; i < video_ticks_.size(); i++) {
            values.push_back((video_ticks_[i].second - video_ticks_[i].first).InMillisecondsF());
        }
    ...
    {
        return std::min<double>(available_bitrate_, video_sender_config_.max_bitrate / 1000000.0);
    }

    bool SimpleGood()
    {
        return frames_lost() <= 1 && late_frames() <= 1 && video_bandwidth() > desired_video_bitrate() * 0.8
...
private:
    ...
    raw_ptr<LoopBackTransport> receiver_to_sender_; // Owned by CastTransportImpl.
    raw_ptr<LoopBackTransport> sender_to_receiver_; // Owned by CastTransportImpl.
...
void RunOneBenchmark::Create(const MeasuringPoint& p)
{
    ...
    transport_sender_.Init(new CastTransportImpl(&testing_clock_sender_, base::Seconds(1), std::make_unique<TransportClient>(nullptr),
    ...
    transport_receiver_ = std::make_unique<CastTransportImpl>(&testing_clock_receiver_, base::Seconds(1), std::make_unique<TransportClient>(this),
    ...
    cast_sender_->InitializeAudio(audio_sender_config_, base::BindOnce(&ExpectAudioSuccess));
    cast_sender_->InitializeVideo(video_sender_config_, base::BindRepeating(&ExpectVideoSuccess), base::DoNothing());

    receiver_to_sender_->Initialize(CreateSimplePipe(p), transport_sender_.PacketReceiverForTesting(), task_runner_, &testing_clock_);
    sender_to_receiver_->Initialize(CreateSimplePipe(p), transport_receiver_->PacketReceiverForTesting(), task_runner_, &testing_clock_);
...
template <class T> class BenchmarkCache {
public:
        ...
        for (size_t i = 0; i < results_.size(); i++) {
            if (results_[i].second) {
                if (x <= results_[i].first) {
                    VLOG(2) << "TRUE because: " << x.AsString() << " <= " << results_[i].first.AsString();
                    return FOUND_TRUE;
                }
            } else {
                if (x >= results_[i].first) {
                    VLOG(2) << "FALSE because: " << x.AsString() << " >= " << results_[i].first.AsString();
                    return FOUND_FALSE;
                }
            }
        }
...
struct SearchVariable {
    ...
    SearchVariable blend(const SearchVariable& other, double factor)
    {
        CHECK_GE(factor, 0);
        CHECK_LE(factor, 1.0);
        return SearchVariable(base * (1 - factor) + other.base * factor, grade * (1 - factor) + other.grade * factor);
    }
...
struct SearchVector {
    ...
    {
        return MeasuringPoint(bitrate.value(-v), latency.value(v), packet_drop.value(v));
    }
    std::string AsString(double v)
    {
        return GetMeasuringPoint(v).AsString();
    }
...
public:
    ...
    void SpanningSearch(
        int max, int x, int y, int skip, SearchVector a, SearchVector b, SearchVector c, double accuracy, std::vector<std::unique_ptr<base::Thread>>* threads)
    {
        ...
        if (skip > max) {
            ...
            (*threads)[thread_num % threads->size()]->task_runner()->PostTask(
                FROM_HERE, base::BindOnce(&CastBenchmark::BinarySearch, base::Unretained(this), v, accuracy));
        } else {
            skip *= 2;
    ...
    void Run()
    {
        ...
        for (int i = 0; i < 16; i++) {
            threads.push_back(std::make_unique<base::Thread>(base::StringPrintf("cast_bench_thread_%d", i)));
            threads[i]->Start();
        }
        ...
        if (base::CommandLine::ForCurrentProcess()->HasSwitch("single-run")) {
            ...
            threads[0]->task_runner()->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&CastBenchmark::RunOnePoint), base::Unretained(this), a, 1.0));
        } else {
            SearchVector a, b, c;
