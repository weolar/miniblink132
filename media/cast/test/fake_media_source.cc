...
{
    return static_cast<int64_t>(delta.InSecondsF() * time_base.den / time_base.num + 0.5 /* rounding */);
}
...
FakeMediaSource::FakeMediaSource(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock, const FrameSenderConfig& audio_config,
    const FrameSenderConfig& video_config, bool keep_frames)
    : task_runner_(task_runner)
    , output_audio_params_(AudioParameters::AUDIO_PCM_LINEAR, media::ChannelLayoutConfig::Guess(audio_config.channels), audio_config.rtp_timebase,
    ...
    , video_first_pts_set_(false)
{
    ...
    audio_bus_factory_ = std::make_unique<TestAudioBusFactory>(audio_config.channels, audio_config.rtp_timebase, kSoundFrequency, kSoundVolume);
}

FakeMediaSource::~FakeMediaSource() = default;

void FakeMediaSource::SetSourceFile(const base::FilePath& video_file, int final_fps)
{
    ...
    protocol_ = std::make_unique<InMemoryUrlProtocol>(file_data_.data(), file_data_.length(), false);
    ...
    for (unsigned int i = 0; i < av_format_context_->nb_streams; ++i) {
        ...
        std::unique_ptr<AVCodecContext, ScopedPtrAVFreeContext> av_codec_context(AVStreamToAVCodecContext(av_stream));
        if (!av_codec_context) {
            LOG(ERROR) << "Cannot get a codec context for the codec: " << av_stream->codecpar->codec_id;
            continue;
        }
        ...
        if (!av_codec) {
            LOG(ERROR) << "Cannot find decoder for the codec: " << av_codec_context->codec_id;
            continue;
        }
        ...
        if (avcodec_open2(av_codec_context.get(), av_codec, nullptr) < 0) {
            LOG(ERROR) << "Cannot open AVCodecContext for the codec: " << av_codec_context->codec_id;
            return;
        }
        ...
        if (av_codec->type == AVMEDIA_TYPE_AUDIO) {
            ...
            ChannelLayout layout = ChannelLayoutToChromeChannelLayout(av_codec_context->channel_layout, av_codec_context->channels);
            ...
            source_audio_params_.Reset(AudioParameters::AUDIO_PCM_LINEAR, { layout, av_audio_context_->channels }, av_audio_context_->sample_rate,
                av_audio_context_->sample_rate / kAudioPacketsPerSecond);
            CHECK(source_audio_params_.IsValid());
            LOG(INFO) << "Source file has audio.";
            audio_decoding_loop_ = std::make_unique<FFmpegDecodingLoop>(av_audio_context_.get());
            ...
            video_decoding_loop_ = std::make_unique<FFmpegDecodingLoop>(av_video_context_.get());
            if (final_fps > 0) {
...
void FakeMediaSource::Start(scoped_refptr<AudioFrameInput> audio_frame_input, scoped_refptr<VideoFrameInput> video_frame_input)
{
    ...
    LOG(INFO) << "Source Frame rate: " << video_frame_rate_numerator_ << "/" << video_frame_rate_denominator_ << " fps.";
    ...
    if (!is_transcoding_audio() && !is_transcoding_video()) {
        // Send fake patterns.
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&FakeMediaSource::SendNextFakeFrame, weak_factory_.GetWeakPtr()));
        return;
    }
    ...
    audio_fifo_input_bus_ = AudioBus::Create(source_audio_params_.channels(), source_audio_params_.frames_per_buffer());
    // Audio FIFO can carry all data fron AudioRendererAlgorithm.
    audio_fifo_ = std::make_unique<AudioFifo>(source_audio_params_.channels(), audio_algo_.QueueCapacity());
    audio_converter_ = std::make_unique<media::AudioConverter>(source_audio_params_, output_audio_params_, true);
    audio_converter_->AddInput(this);
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&FakeMediaSource::SendNextFrame, weak_factory_.GetWeakPtr()));
}
...
void FakeMediaSource::SendNextFakeFrame()
{
    ...
    while (audio_time < video_time) {
        if (is_transcoding_audio()) {
            ...
            audio_frame_input_->InsertAudio(audio_bus_factory_->NextAudioBus(base::Milliseconds(kAudioFrameMs)), start_time_ + audio_time);
        }
    ...
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&FakeMediaSource::SendNextFakeFrame, weak_factory_.GetWeakPtr()), video_time - elapsed_time);
}
...
void FakeMediaSource::UpdateNextFrameSize()
{
    if (variable_frame_size_mode_) {
        ...
        if (current_frame_size_.IsEmpty()) {
            current_frame_size_ = gfx::Size(kStartingFakeFrameWidth, kStartingFakeFrameHeight);
            update_size_change_time = true;
        } else if (clock_->NowTicks() >= next_frame_size_change_time_) {
            current_frame_size_ = gfx::Size(base::RandInt(kMinFakeFrameWidth, kMaxFakeFrameWidth), base::RandInt(kMinFakeFrameHeight, kMaxFakeFrameHeight));
            update_size_change_time = true;
        }

        if (update_size_change_time) {
            next_frame_size_change_time_ = clock_->NowTicks() + base::Milliseconds(base::RandDouble() * kMaxFrameSizeChangeMillis);
        }
    } else {
        current_frame_size_ = gfx::Size(kStartingFakeFrameWidth, kStartingFakeFrameHeight);
...
bool FakeMediaSource::SendNextTranscodedVideo(base::TimeDelta elapsed_time)
{
    ...
    if (keep_frames_)
        inserted_video_frame_queue_.push(video_frame);
    video_frame_input_->InsertRawVideoFrame(video_frame, start_time_ + video_frame->timestamp());
...
void FakeMediaSource::SendNextFrame()
{
    ...
    // Send next send.
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&FakeMediaSource::SendNextFrame, weak_factory_.GetWeakPtr()), base::Milliseconds(kAudioFrameMs));
}

base::TimeDelta FakeMediaSource::VideoFrameTime(int frame_number)
{
    return frame_number * base::Seconds(1) * video_frame_rate_denominator_ / video_frame_rate_numerator_;
}
...
{
    CHECK(av_seek_frame(av_format_context_, -1, 0, AVSEEK_FLAG_BACKWARD) >= 0) << "Failed to rewind to the beginning.";
}
...
{
    auto result = audio_decoding_loop_->DecodePacket(packet.get(), base::BindRepeating(&FakeMediaSource::OnNewAudioFrame, base::Unretained(this)));
    CHECK_EQ(result, FFmpegDecodingLoop::DecodeStatus::kOkay) << "Failed to decode audio.";
    ...
    while (frames_needed_to_scale <= audio_algo_.BufferedFrames()) {
        if (!audio_algo_.FillBuffer(audio_fifo_input_bus_.get(), 0, audio_fifo_input_bus_->frames(), playback_rate_)) {
            // Nothing can be scaled. Decode some more.
            return;
        }

        // Prevent overflow of audio data in the FIFO.
        if (audio_fifo_input_bus_->frames() + audio_fifo_->frames() <= audio_fifo_->max_frames()) {
        ...
        std::unique_ptr<media::AudioBus> resampled_bus(
            media::AudioBus::Create(output_audio_params_.channels(), output_audio_params_.sample_rate() / kAudioPacketsPerSecond));
...
bool FakeMediaSource::OnNewAudioFrame(AVFrame* frame)
{
    ...
    if (!audio_sent_ts_) {
        ...
        audio_sent_ts_ = std::make_unique<AudioTimestampHelper>(output_audio_params_.sample_rate());
        // For some files this is an invalid value.
        base::TimeDelta base_ts;
        audio_sent_ts_->SetBaseTimestamp(base_ts);
    }

    scoped_refptr<AudioBuffer> buffer = AudioBuffer::CopyFrom(AVSampleFormatToSampleFormat(av_audio_context_->sample_fmt, av_audio_context_->codec_id),
        ChannelLayoutToChromeChannelLayout(av_audio_context_->channel_layout, av_audio_context_->channels), av_audio_context_->channels,
        av_audio_context_->sample_rate, frames_read, &frame->data[0], PtsToTimeDelta(frame->pts, av_audio_stream()->time_base));
...
{
    auto result = video_decoding_loop_->DecodePacket(packet.get(), base::BindRepeating(&FakeMediaSource::OnNewVideoFrame, base::Unretained(this)));
    CHECK_EQ(result, FFmpegDecodingLoop::DecodeStatus::kOkay) << "Failed to decode video.";
}
...
bool FakeMediaSource::OnNewVideoFrame(AVFrame* frame)
{
    ...
    base::TimeDelta timestamp = PtsToTimeDelta(frame->pts - video_first_pts_, time_base);
    if (timestamp < last_video_frame_timestamp_) {
        ...
        timestamp = last_video_frame_timestamp_ + (base::Seconds(1) * frame_rate.den / frame_rate.num);
    ...
    scoped_refptr<media::VideoFrame> video_frame
        = VideoFrame::WrapExternalYuvData(media::PIXEL_FORMAT_I420, size, gfx::Rect(size), size, shallow_copy->linesize[0], shallow_copy->linesize[1],
            shallow_copy->linesize[2], shallow_copy->data[0], shallow_copy->data[1], shallow_copy->data[2], timestamp);
    if (!video_frame)
        return false;
    video_frame_queue_.push(video_frame);
    video_frame_queue_.back()->AddDestructionObserver(base::BindOnce(&AVFreeFrame, shallow_copy));
...
double FakeMediaSource::ProvideInput(media::AudioBus* output_bus, uint32_t frames_delayed)
{
...
scoped_refptr<media::VideoFrame> FakeMediaSource::PopOldestInsertedVideoFrame()
{
