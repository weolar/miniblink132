...
public:
    explicit SkewedSingleThreadTaskRunner(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner);

    SkewedSingleThreadTaskRunner(const SkewedSingleThreadTaskRunner&) = delete;
    SkewedSingleThreadTaskRunner& operator=(const SkewedSingleThreadTaskRunner&) = delete;

    // Set the delay multiplier to |skew|.
    void SetSkew(double skew);

    // base::SingleThreadTaskRunner implementation.
    bool PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) final;

    bool RunsTasksInCurrentSequence() const final;

    // This function is currently not used, and will return false.
    bool PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) final;
