...
void TestRtcpPacketBuilder::AddSr(uint32_t remote_ssrc, int number_of_report_blocks)
{
...
void TestRtcpPacketBuilder::AddSrWithNtp(uint32_t remote_ssrc, uint32_t ntp_high, uint32_t ntp_low, uint32_t rtp_timestamp)
{
    AddRtcpHeader(200, 0);
    big_endian_writer_.WriteU32(remote_ssrc);
...
void TestRtcpPacketBuilder::AddRr(uint32_t remote_ssrc, int number_of_report_blocks)
{
...
void TestRtcpPacketBuilder::AddSendReportRequest(uint32_t remote_ssrc, uint32_t local_ssrc)
{
    AddRtcpHeader(205, 5);
    big_endian_writer_.WriteU32(remote_ssrc);
    big_endian_writer_.WriteU32(local_ssrc);
...
void TestRtcpPacketBuilder::AddCast(uint32_t remote_ssrc, uint32_t local_ssrc, base::TimeDelta target_delay)
{
...
void TestRtcpPacketBuilder::AddCst2(const std::vector<FrameId>& later_received_frames)
{
    big_endian_writer_.WriteU8('C');
    big_endian_writer_.WriteU8('S');
    big_endian_writer_.WriteU8('T');
    ...
    for (size_t num_bytes_written = 6 + ack_bitmasks.size(); num_bytes_written % 4; ++num_bytes_written) {
...
void TestRtcpPacketBuilder::AddReceiverFrameLog(uint32_t rtp_timestamp, int num_events, uint32_t event_timesamp_base)
{
    big_endian_writer_.WriteU32(rtp_timestamp);
...
void TestRtcpPacketBuilder::AddReceiverEventLog(uint16_t event_data, CastLoggingEvent event, uint16_t event_timesamp_delta)
{
