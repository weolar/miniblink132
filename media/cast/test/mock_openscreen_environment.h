...
public:
    MockOpenscreenEnvironment(openscreen::ClockNowFunctionPtr now_function, openscreen::TaskRunner& task_runner);
    ~MockOpenscreenEnvironment() override;

    // Used to return fake values, to simulate a bound socket for testing.
    MOCK_METHOD(openscreen::IPEndpoint, GetBoundLocalEndpoint, (), (const, override));

    // Used for intercepting packet sends from the implementation under test.
    MOCK_METHOD(void, SendPacket, (openscreen::ByteView packet, openscreen::cast::PacketMetadata metadata), (override));
};
