...
{
    CHECK(result == media::cast::STATUS_INITIALIZED) << "Cast sender uninitialized";
...
void WriteLogsToFileAndDestroySubscribers(const scoped_refptr<media::cast::CastEnvironment>& cast_environment,
    std::unique_ptr<media::cast::EncodingEventSubscriber> video_event_subscriber, std::unique_ptr<media::cast::EncodingEventSubscriber> audio_event_subscriber,
...
void WriteStatsAndDestroySubscribers(const scoped_refptr<media::cast::CastEnvironment>& cast_environment,
    std::unique_ptr<media::cast::StatsEventSubscriber> video_stats_subscriber, std::unique_ptr<media::cast::StatsEventSubscriber> audio_stats_subscriber,
    std::unique_ptr<media::cast::ReceiverTimeOffsetEstimatorImpl> estimator)
{
    ...
    std::string json;
    base::JSONWriter::WriteWithOptions(stats, base::JSONWriter::OPTIONS_PRETTY_PRINT, &json);
...
public:
    explicit TransportClient(media::cast::LogEventDispatcher* log_event_dispatcher)
    ...
    void OnLoggingEventsReceived(
        std::unique_ptr<std::vector<media::cast::FrameEvent>> frame_events, std::unique_ptr<std::vector<media::cast::PacketEvent>> packet_events) final
    {
...
private:
    const raw_ptr<media::cast::LogEventDispatcher> log_event_dispatcher_; // Not owned by this class.
};
...
int main(int argc, char** argv)
{
    ...
    // Running transport on the main thread.
    scoped_refptr<media::cast::CastEnvironment> cast_environment(new media::cast::CastEnvironment(
        base::DefaultTickClock::GetInstance(), io_task_executor.task_runner(), audio_thread.task_runner(), video_thread.task_runner()));

    // SendProcess initialization.
    std::unique_ptr<media::cast::FakeMediaSource> fake_media_source(
        new media::cast::FakeMediaSource(test_thread.task_runner(), cast_environment->Clock(), audio_config, video_config, false));
    ...
    std::unique_ptr<media::cast::CastTransport> transport_sender
        = media::cast::CastTransport::Create(cast_environment->Clock(), base::Seconds(1), std::make_unique<TransportClient>(cast_environment->logger()),
            std::make_unique<media::cast::UdpTransportImpl>(
                io_task_executor.task_runner(), net::IPEndPoint(), remote_endpoint, base::BindRepeating(&UpdateCastTransportStatus)),
    ...
    video_event_subscriber = std::make_unique<media::cast::EncodingEventSubscriber>(media::cast::VIDEO_EVENT, 10000);
    audio_event_subscriber = std::make_unique<media::cast::EncodingEventSubscriber>(media::cast::AUDIO_EVENT, 10000);
    ...
    std::unique_ptr<media::cast::ReceiverTimeOffsetEstimatorImpl> offset_estimator(new media::cast::ReceiverTimeOffsetEstimatorImpl());
    ...
    std::unique_ptr<media::cast::StatsEventSubscriber> video_stats_subscriber(
        new media::cast::StatsEventSubscriber(media::cast::VIDEO_EVENT, cast_environment->Clock(), offset_estimator.get()));
    std::unique_ptr<media::cast::StatsEventSubscriber> audio_stats_subscriber(
        new media::cast::StatsEventSubscriber(media::cast::AUDIO_EVENT, cast_environment->Clock(), offset_estimator.get()));
    ...
    io_task_executor.task_runner()->PostDelayedTask(FROM_HERE,
        base::BindOnce(&WriteLogsToFileAndDestroySubscribers, cast_environment, std::move(video_event_subscriber), std::move(audio_event_subscriber),
            std::move(video_log_file), std::move(audio_log_file)),
        base::Seconds(logging_duration_seconds));

    io_task_executor.task_runner()->PostDelayedTask(FROM_HERE,
        base::BindOnce(&WriteStatsAndDestroySubscribers, cast_environment, std::move(video_stats_subscriber), std::move(audio_stats_subscriber),
    ...
    io_task_executor.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&media::cast::CastSender::InitializeVideo, base::Unretained(cast_sender.get()), fake_media_source->get_video_config(),
            base::BindRepeating(&QuitLoopOnInitializationResult), base::DoNothing()));
    base::RunLoop().Run(); // Wait for video initialization.
    io_task_executor.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(&media::cast::CastSender::InitializeAudio, base::Unretained(cast_sender.get()), audio_config,
            base::BindRepeating(&QuitLoopOnInitializationResult)));
    base::RunLoop().Run(); // Wait for audio initialization.

    fake_media_source->Start(cast_sender->audio_frame_input(), cast_sender->video_frame_input());
    base::RunLoop().Run();
    return 0;
}
