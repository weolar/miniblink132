...
constexpr int64_t kStartMillisecond = INT64_C(1245);
constexpr int kAudioChannels = 2;
constexpr double kSoundFrequency = 314.15926535897; // Freq of sine wave.
constexpr float kSoundVolume = 0.5f;
...
constexpr double kVideoAcceptedPSNR = 38.0;

// The tests are commonly implemented with |kFrameTimerInterval| RunTask
// function; a normal video is 30 fps hence the 33 ms between frames.
constexpr base::TimeDelta kFrameTimerInterval = base::Milliseconds(33);

// The size of audio frames.  The encoder joins/breaks all inserted audio into
// chunks of this size.
constexpr base::TimeDelta kAudioFrameDuration = base::Milliseconds(10);

// The amount of time between frame capture on the sender and playout on the
// receiver.
constexpr base::TimeDelta kTargetPlayoutDelay = base::Milliseconds(400);

// The maximum amount of deviation expected in the playout times emitted by the
// receiver.
constexpr base::TimeDelta kMaxAllowedPlayoutError = base::Milliseconds(30);
...
std::map<RtpTimeTicks, LoggingEventCounts> GetEventCountsForFrameEvents(const std::vector<FrameEvent>& frame_events)
{
    ...
    for (const FrameEvent& frame_event : frame_events) {
        ...
        if (map_it == event_counter_for_frame.end()) {
            ...
            event_counter_for_frame.insert(std::make_pair(frame_event.rtp_timestamp, new_counter));
        } else {
...
std::map<uint16_t, LoggingEventCounts> GetEventCountsForPacketEvents(const std::vector<PacketEvent>& packet_events)
{
    ...
    for (const PacketEvent& packet_event : packet_events) {
        ...
        if (map_it == event_counter_for_packet.end()) {
            ...
            event_counter_for_packet.insert(std::make_pair(packet_event.packet_id, new_counter));
        } else {
...
public:
    ...
    void SetPacketReceiver(
        const PacketReceiverCallback& packet_receiver, const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock)
    {
        std::unique_ptr<test::PacketPipe> loopback_pipe(new LoopBackPacketPipe(packet_receiver));
        if (packet_pipe_) {
...
class TestReceiverAudioCallback : public base::RefCountedThreadSafe<TestReceiverAudioCallback> {
public:
    ...
    void AddExpectedResult(const AudioBus& audio_bus, base::TimeTicks playout_time)
    {
        std::unique_ptr<ExpectedAudioFrame> expected_audio_frame(new ExpectedAudioFrame());
    ...
    void IgnoreAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
    {
        ++num_called_;
    }

    void CheckAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
    {
        ++num_called_;
        ...
        for (int ch = 0; ch < audio_bus->channels(); ++ch) {
            EXPECT_NEAR(CountZeroCrossings(expected_audio_frame->audio_bus->channel(ch), expected_audio_frame->audio_bus->frames()),
                CountZeroCrossings(audio_bus->channel(ch), audio_bus->frames()), 1);
        }

        EXPECT_NEAR((playout_time - expected_audio_frame->playout_time).InMillisecondsF(), 0.0, kMaxAllowedPlayoutError.InMilliseconds());
        VLOG_IF(1, !last_playout_time_.is_null()) << "Audio frame playout time delta (compared to last frame) is "
...
class TestReceiverVideoCallback : public base::RefCountedThreadSafe<TestReceiverVideoCallback> {
public:
    ...
    void AddExpectedResult(int frame_number, const gfx::Size& size, base::TimeTicks playout_time, bool should_be_continuous)
    {
    ...
    void CheckVideoFrame(bool examine_content, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks playout_time, bool is_continuous)
    {
        ++num_called_;
        ...
        EXPECT_EQ(expected_video_frame.size.width(), video_frame->visible_rect().width());
        EXPECT_EQ(expected_video_frame.size.height(), video_frame->visible_rect().height());
        ...
        if (examine_content && expected_video_frame.should_be_continuous) {
            scoped_refptr<media::VideoFrame> expected_I420_frame = media::VideoFrame::CreateFrame(
                PIXEL_FORMAT_I420, expected_video_frame.size, gfx::Rect(expected_video_frame.size), expected_video_frame.size, base::TimeDelta());
            PopulateVideoFrame(expected_I420_frame.get(), expected_video_frame.frame_number);
            EXPECT_LE(kVideoAcceptedPSNR, I420PSNR(*expected_I420_frame, *video_frame));
        }

        EXPECT_NEAR((playout_time - expected_video_frame.playout_time).InMillisecondsF(), 0.0, kMaxAllowedPlayoutError.InMilliseconds());
        VLOG_IF(1, !last_playout_time_.is_null()) << "Video frame playout time delta (compared to last frame) is "
                                                  << (playout_time - last_playout_time_).InMicroseconds() << " usec.";
        last_playout_time_ = playout_time;
...
protected:
    End2EndTest()
        ...
        , task_runner_sender_(new test::SkewedSingleThreadTaskRunner(task_runner_))
        , testing_clock_receiver_(&testing_clock_)
        , task_runner_receiver_(new test::SkewedSingleThreadTaskRunner(task_runner_))
        , cast_environment_sender_(new CastEnvironment(&testing_clock_sender_, task_runner_sender_, task_runner_sender_, task_runner_sender_))
        , cast_environment_receiver_(new CastEnvironment(&testing_clock_receiver_, task_runner_receiver_, task_runner_receiver_, task_runner_receiver_))
    ...
    void Configure(Codec video_codec, Codec audio_codec)
    {
        ...
        audio_receiver_config_.rtp_max_delay_ms = kTargetPlayoutDelay.InMilliseconds();
        ...
        test_receiver_audio_callback_->SetExpectedSamplingFrequency(audio_receiver_config_.rtp_timebase);
        ...
        video_receiver_config_.rtp_max_delay_ms = kTargetPlayoutDelay.InMilliseconds();
    ...
    void SetExpectedVideoPlayoutSmoothness(base::TimeDelta min_delta, base::TimeDelta max_delta, base::TimeDelta max_curvature)
    {
    ...
    void FeedAudioFrames(int count, bool will_be_checked)
    {
        for (int i = 0; i < count; ++i) {
            std::unique_ptr<AudioBus> audio_bus(audio_bus_factory_->NextAudioBus(kAudioFrameDuration));
            const base::TimeTicks reference_time = testing_clock_sender_.NowTicks() + i * kAudioFrameDuration;
            if (will_be_checked) {
                test_receiver_audio_callback_->AddExpectedResult(*audio_bus, reference_time + kTargetPlayoutDelay);
            }
    ...
    void FeedAudioFramesWithExpectedDelay(int count, base::TimeDelta delay)
    {
        for (int i = 0; i < count; ++i) {
            std::unique_ptr<AudioBus> audio_bus(audio_bus_factory_->NextAudioBus(kAudioFrameDuration));
            const base::TimeTicks reference_time = testing_clock_sender_.NowTicks() + i * kAudioFrameDuration;
            test_receiver_audio_callback_->AddExpectedResult(*audio_bus, reference_time + delay + kTargetPlayoutDelay);
            audio_frame_input_->InsertAudio(std::move(audio_bus), reference_time);
        }
    }
    ...
    void RequestAudioFrames(int count, bool with_check)
    {
        for (int i = 0; i < count; ++i) {
            cast_receiver_->RequestDecodedAudioFrame(base::BindRepeating(
                with_check ? &TestReceiverAudioCallback::CheckAudioFrame : &TestReceiverAudioCallback::IgnoreAudioFrame, test_receiver_audio_callback_));
        }
    }
    ...
    void SendVideoFrame(int frame_number, base::TimeTicks reference_time)
    {
        ...
        if (video_sender_config_.codec == CODEC_VIDEO_FAKE) {
            ...
            video_frame = media::VideoFrame::CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, time_diff);
    ...
    void RunTasks(base::TimeDelta sleep_delay)
    {
        task_runner_->Sleep(sleep_delay);
    }
    ...
    std::pair<int, int> RunAudioVideoLoop(base::TimeDelta duration)
    {
        ...
        while (testing_clock_.NowTicks() < end_time) {
            ...
            if (send_and_receive_a_video_frame) {
                ...
                next_video_frame_at += kFrameTimerInterval;
                test_receiver_video_callback_->AddExpectedResult(
                    video_frames_sent, GetTestVideoFrameSize(), testing_clock_.NowTicks() + kTargetPlayoutDelay, true);
                SendVideoFrame(video_frames_sent, video_reference_time);
                ++video_frames_sent;
            }

            RunTasks(kAudioFrameDuration);

            RequestAudioFrames(1, verify_audio_data);
            if (send_and_receive_a_video_frame) {
                cast_receiver_->RequestDecodedVideoFrame(base::BindRepeating(
                    &TestReceiverVideoCallback::CheckVideoFrame, test_receiver_video_callback_, video_sender_config_.codec != CODEC_VIDEO_FAKE));
            }
        }

        // Verify all audio and video frames were received.
        RunTasks(kFrameTimerInterval + kTargetPlayoutDelay);
        EXPECT_EQ(audio_frames_sent, test_receiver_audio_callback_->number_times_called());
        EXPECT_EQ(video_frames_sent, test_receiver_video_callback_->number_times_called());
    ...
    // correctly.
    void VerifyLogging(int num_expected_audio_frames, int num_expected_video_frames)
    {
        ...
        EXPECT_EQ(static_cast<size_t>(num_expected_audio_frames), audio_event_counts_by_frame.size());
        std::map<RtpTimeTicks, LoggingEventCounts> video_event_counts_by_frame = GetEventCountsForFrameEvents(video_frame_events);
        EXPECT_EQ(static_cast<size_t>(num_expected_video_frames), video_event_counts_by_frame.size());

        // Examine the types of each frame and packet event and verify required
        // events are present and unknown ones are not.
        VerifyLoggingEventCounts(audio_event_counts_by_frame, GetEventCountsForPacketEvents(audio_packet_events), true);
        VerifyLoggingEventCounts(video_event_counts_by_frame, GetEventCountsForPacketEvents(video_packet_events), false);
    }

    // Examines histograms of event types to verify all logging information was
    // captured correctly.
    static void VerifyLoggingEventCounts(const std::map<RtpTimeTicks, LoggingEventCounts>& event_counts_by_frame,
        const std::map<uint16_t, LoggingEventCounts>& event_counts_by_packet, bool for_audio)
    {
    ...
    void BasicPlayerGotVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks playout_time, bool continuous)
    {
        ...
        if (!last_video_playout_time_.is_null() && min_video_playout_delta_.is_positive()) {
            const base::TimeDelta delta = playout_time - last_video_playout_time_;
            VLOG(1) << "Video frame playout time delta (compared to last frame) is " << delta.InMicroseconds() << " usec.";
            EXPECT_LE(min_video_playout_delta_.InMicroseconds(), delta.InMicroseconds());
            EXPECT_GE(max_video_playout_delta_.InMicroseconds(), delta.InMicroseconds());
            if (last_video_playout_delta_.is_positive()) {
                ...
                EXPECT_GE(max_video_playout_curvature_.InMicroseconds(), abs_curvature.InMicroseconds());
            }
            last_video_playout_delta_ = delta;
        }
        last_video_playout_time_ = playout_time;

        video_ticks_.push_back(std::make_pair(testing_clock_receiver_.NowTicks(), playout_time));
        cast_receiver_->RequestDecodedVideoFrame(base::BindRepeating(&End2EndTest::BasicPlayerGotVideoFrame, base::Unretained(this)));
    }

    void BasicPlayerGotAudioFrame(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
    {
        VLOG_IF(1, !last_audio_playout_time_.is_null())
            << "Audio frame playout time delta (compared to last frame) is " << (playout_time - last_audio_playout_time_).InMicroseconds() << " usec.";
        last_audio_playout_time_ = playout_time;

        audio_ticks_.push_back(std::make_pair(testing_clock_receiver_.NowTicks(), playout_time));
        cast_receiver_->RequestDecodedAudioFrame(base::BindRepeating(&End2EndTest::BasicPlayerGotAudioFrame, base::Unretained(this)));
    }
...
public:
    TransportClient(LogEventDispatcher* log_event_dispatcher, End2EndTest* e2e_test)
    ...
    void OnLoggingEventsReceived(std::unique_ptr<std::vector<FrameEvent>> frame_events, std::unique_ptr<std::vector<PacketEvent>> packet_events) final
    {
        log_event_dispatcher_->DispatchBatchOfEvents(std::move(frame_events), std::move(packet_events));
    }
...
private:
    const raw_ptr<LogEventDispatcher> log_event_dispatcher_; // Not owned by this class.
...
{
    transport_sender_ = std::make_unique<CastTransportImpl>(&testing_clock_sender_, base::Milliseconds(1),
        std::make_unique<TransportClient>(cast_environment_sender_->logger(), nullptr), base::WrapUnique(sender_to_receiver_.get()), task_runner_sender_);

    transport_receiver_ = std::make_unique<CastTransportImpl>(&testing_clock_sender_, base::Milliseconds(1),
        std::make_unique<TransportClient>(cast_environment_receiver_->logger(), this), base::WrapUnique(receiver_to_sender_.get()), task_runner_sender_);

    cast_receiver_ = CastReceiver::Create(cast_environment_receiver_, audio_receiver_config_, video_receiver_config_, transport_receiver_.get());

    cast_sender_ = CastSender::Create(cast_environment_sender_, transport_sender_.get());

    // Initializing audio and video senders.
    cast_sender_->InitializeAudio(audio_sender_config_, base::BindOnce(&ExpectSuccessOperationalStatus));
    cast_sender_->InitializeVideo(video_sender_config_, base::BindRepeating(&ExpectSuccessOperationalStatus), base::DoNothing());
    task_runner_->RunTasks();

    receiver_to_sender_->SetPacketReceiver(transport_sender_->PacketReceiverForTesting(), task_runner_, &testing_clock_);
    sender_to_receiver_->SetPacketReceiver(transport_receiver_->PacketReceiverForTesting(), task_runner_, &testing_clock_);

    audio_frame_input_ = cast_sender_->audio_frame_input();
    video_frame_input_ = cast_sender_->video_frame_input();

    audio_bus_factory_ = std::make_unique<TestAudioBusFactory>(audio_sender_config_.channels, audio_sender_config_.rtp_timebase, kSoundFrequency, kSoundVolume);
}
...
TEST_F(End2EndTest, LoopWithLosslessEncoding)
{
    ...
    RunTasks(base::Milliseconds(750));
...
TEST_F(End2EndTest, DISABLED_StartSenderBeforeReceiver)
{
    ...
    int frame_number = 0;
    int audio_diff = kFrameTimerInterval.InMilliseconds();

    sender_to_receiver_->SetSendPackets(false);

    constexpr base::TimeDelta kTestDelay = base::Milliseconds(100);

    const int kNumVideoFramesBeforeReceiverStarted = 2;
    const base::TimeTicks initial_send_time = testing_clock_sender_.NowTicks();
    const base::TimeDelta expected_delay = kTestDelay + kFrameTimerInterval;
    for (int i = 0; i < kNumVideoFramesBeforeReceiverStarted; ++i) {
        const int num_audio_frames = audio_diff / kAudioFrameDuration.InMilliseconds();
        audio_diff -= num_audio_frames * kAudioFrameDuration.InMilliseconds();
        ...
        test_receiver_video_callback_->AddExpectedResult(frame_number, GetTestVideoFrameSize(), initial_send_time + expected_delay + kTargetPlayoutDelay, true);
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        test_receiver_video_callback_->AddExpectedResult(frame_number, GetTestVideoFrameSize(), testing_clock_sender_.NowTicks() + kTargetPlayoutDelay, true);
        SendVideoFrame(frame_number++, testing_clock_sender_.NowTicks());
        ...
        cast_receiver_->RequestDecodedVideoFrame(
            base::BindRepeating(&TestReceiverVideoCallback::CheckVideoFrame, test_receiver_video_callback_, video_sender_config_.codec != CODEC_VIDEO_FAKE));

        RunTasks(kFrameTimerInterval - kAudioFrameDuration);
        audio_diff += kFrameTimerInterval.InMilliseconds();
    }
    RunTasks(2 * kFrameTimerInterval + base::Milliseconds(1)); // Empty the receiver pipeline.
    EXPECT_EQ(num_audio_frames_requested, test_receiver_audio_callback_->number_times_called());
...
TEST_F(End2EndTest, BasicFakeSoftwareVideo)
{
    ...
    SetExpectedVideoPlayoutSmoothness(kFrameTimerInterval * 99 / 100, kFrameTimerInterval * 101 / 100, kFrameTimerInterval / 100);

    int frames_counter = 0;
    for (; frames_counter < 30; ++frames_counter) {
        SendVideoFrame(frames_counter, testing_clock_sender_.NowTicks());
        RunTasks(kFrameTimerInterval);
    }
    RunTasks(2 * kFrameTimerInterval + base::Milliseconds(1)); // Empty the pipeline.
...
TEST_F(End2EndTest, ReceiverClockFast)
{
    ...
    for (int frames_counter = 0; frames_counter < kLongTestIterations; ++frames_counter) {
        SendVideoFrame(frames_counter, testing_clock_sender_.NowTicks());
        RunTasks(kFrameTimerInterval);
    }
    RunTasks(2 * kFrameTimerInterval + base::Milliseconds(1)); // Empty the pipeline.
...
TEST_F(End2EndTest, SmoothPlayoutWithFivePercentClockRateSkew)
{
    ...
    SetExpectedVideoPlayoutSmoothness(kFrameTimerInterval * 90 / 100, kFrameTimerInterval * 110 / 100, kFrameTimerInterval / 10);
...
TEST_F(End2EndTest, EvilNetwork)
{
    ...
    RunTasks(100 * kFrameTimerInterval + base::Milliseconds(1)); // Empty the pipeline.
    EXPECT_LT(static_cast<size_t>(kLongTestIterations / 100), video_ticks_.size());
...
TEST_F(End2EndTest, ShoveHighFrameRateDownYerThroat)
{
    ...
    for (int frames_counter = 0; frames_counter < kLongTestIterations; ++frames_counter) {
        SendVideoFrame(frames_counter, testing_clock_sender_.NowTicks());
        RunTasks(base::Milliseconds(10) /* 10 ms, but 33.3 expected by system */);
    }
    base::TimeTicks test_end = testing_clock_receiver_.NowTicks();
    RunTasks(100 * kFrameTimerInterval + base::Milliseconds(1)); // Empty the pipeline.
    EXPECT_LT(static_cast<size_t>(kLongTestIterations / 100), video_ticks_.size());
...
#else
    // Otherwise ensure that at least half of the frames are dropped.
    const size_t max_iterations = kLongTestIterations / 2;
#endif
...
TEST_F(End2EndTest, OldPacketNetwork)
{
    ...
    std::unique_ptr<test::PacketPipe> echo_chamber(test::NewDuplicateAndDelay(1, 10 * kFrameTimerInterval.InMilliseconds()));
    echo_chamber->AppendToPipe(test::NewDuplicateAndDelay(1, 20 * kFrameTimerInterval.InMilliseconds()));
    echo_chamber->AppendToPipe(test::NewDuplicateAndDelay(1, 40 * kFrameTimerInterval.InMilliseconds()));
    echo_chamber->AppendToPipe(test::NewDuplicateAndDelay(1, 80 * kFrameTimerInterval.InMilliseconds()));
    echo_chamber->AppendToPipe(test::NewDuplicateAndDelay(1, 160 * kFrameTimerInterval.InMilliseconds()));
...
TEST_F(End2EndTest, TestSetPlayoutDelay)
{
    ...
    Create();
    StartBasicPlayer();
    constexpr base::TimeDelta kNewDelay = base::Milliseconds(600);
    ...
    cast_sender_->SetTargetPlayoutDelay(kNewDelay);
    ...
    for (size_t i = 1; i < video_ticks_.size(); i++) {
        const base::TimeDelta delta = video_ticks_[i].second - video_ticks_[i - 1].second;
        if (delta.InMilliseconds() > 100) {
            EXPECT_EQ(kNewDelay - kTargetPlayoutDelay + kFrameTimerInterval, delta);
            EXPECT_EQ(0u, jump);
            jump = i;
        }
    }
