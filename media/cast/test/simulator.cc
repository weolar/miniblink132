...
public:
    TransportClient(LogEventDispatcher* log_event_dispatcher, PacketProxy* packet_proxy)
...
void AppendYuvToFile(const base::FilePath& path, scoped_refptr<media::VideoFrame> frame)
{
    ...
    AppendToFile(
        path, base::make_span(frame->data(media::VideoFrame::kYPlane), frame->stride(media::VideoFrame::kYPlane) * frame->rows(media::VideoFrame::kYPlane)));
    AppendToFile(
        path, base::make_span(frame->data(media::VideoFrame::kUPlane), frame->stride(media::VideoFrame::kUPlane) * frame->rows(media::VideoFrame::kUPlane)));
    AppendToFile(
        path, base::make_span(frame->data(media::VideoFrame::kVPlane), frame->stride(media::VideoFrame::kVPlane) * frame->rows(media::VideoFrame::kVPlane)));
}
...
void GotVideoFrame(GotVideoFrameOutput* metrics_output, const base::FilePath& yuv_output, EncodedVideoFrameTracker* video_frame_tracker,
    CastReceiver* cast_receiver, scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks render_time, bool continuous)
{
    ++metrics_output->counter;
    cast_receiver->RequestDecodedVideoFrame(base::BindRepeating(&GotVideoFrame, metrics_output, yuv_output, video_frame_tracker, cast_receiver));
...
void GotAudioFrame(int* counter, CastReceiver* cast_receiver, std::unique_ptr<AudioBus> audio_bus, base::TimeTicks playout_time, bool is_continuous)
{
    ++*counter;
    cast_receiver->RequestDecodedAudioFrame(base::BindRepeating(&GotAudioFrame, counter, cast_receiver));
}
...
void RunSimulation(const base::FilePath& source_path, const base::FilePath& log_output_path, const base::FilePath& metrics_output_path,
    const base::FilePath& yuv_output_path, const std::string& extra_data, const NetworkSimulationModel& model)
{
    ...
    scoped_refptr<CastEnvironment> sender_env = new CastEnvironment(&sender_clock, task_runner, task_runner, task_runner);
    test::SkewedTickClock receiver_clock(&testing_clock);
    scoped_refptr<CastEnvironment> receiver_env = new CastEnvironment(&receiver_clock, task_runner, task_runner, task_runner);
    ...
    audio_sender_config.min_playout_delay = audio_sender_config.max_playout_delay = base::Milliseconds(GetIntegerSwitchValue(kTargetDelay, 400));
    ...
    // Cast receiver.
    std::unique_ptr<CastTransport> transport_receiver(new CastTransportImpl(&testing_clock, base::Seconds(1),
        std::make_unique<TransportClient>(receiver_env->logger(), &packet_proxy), base::WrapUnique(receiver_to_sender), task_runner));
    std::unique_ptr<CastReceiver> cast_receiver(CastReceiver::Create(receiver_env, audio_receiver_config, video_receiver_config, transport_receiver.get()));
    ...
    std::unique_ptr<CastTransport> transport_sender(new CastTransportImpl(
        &testing_clock, base::Seconds(1), std::make_unique<TransportClient>(sender_env->logger(), nullptr), base::WrapUnique(sender_to_receiver), task_runner));
    std::unique_ptr<CastSender> cast_sender(CastSender::Create(sender_env, transport_sender.get()));
    ...
    if (use_network_simulation) {
        ...
        std::copy(ipp_model.average_rate().begin(), ipp_model.average_rate().end(), average_rates.begin());
        ipp = std::make_unique<test::InterruptedPoissonProcess>(average_rates, ipp_model.coef_burstiness(), ipp_model.coef_variance(), 0);
        receiver_to_sender->Initialize(ipp->NewBuffer(128 * 1024), transport_sender->PacketReceiverForTesting(), task_runner, &testing_clock);
        sender_to_receiver->Initialize(ipp->NewBuffer(128 * 1024), transport_receiver->PacketReceiverForTesting(), task_runner, &testing_clock);
    } else {
        LOG(INFO) << "No network simulation.";
        receiver_to_sender->Initialize(std::unique_ptr<test::PacketPipe>(), transport_sender->PacketReceiverForTesting(), task_runner, &testing_clock);
        sender_to_receiver->Initialize(std::unique_ptr<test::PacketPipe>(), transport_receiver->PacketReceiverForTesting(), task_runner, &testing_clock);
    }

    // Initialize a fake media source and a tracker to encoded video frames.
    const bool quality_test = !metrics_output_path.empty();
    FakeMediaSource media_source(task_runner, &testing_clock, audio_sender_config, video_sender_config, quality_test);
    std::unique_ptr<EncodedVideoFrameTracker> video_frame_tracker;
    if (quality_test) {
        video_frame_tracker = std::make_unique<EncodedVideoFrameTracker>(&media_source);
    ...
    cast_receiver->RequestDecodedVideoFrame(
        base::BindRepeating(&GotVideoFrame, &metrics_output, yuv_output_path, video_frame_tracker.get(), cast_receiver.get()));
    cast_receiver->RequestDecodedAudioFrame(base::BindRepeating(&GotAudioFrame, &audio_frame_count, cast_receiver.get()));

    // Initializing audio and video senders.
    cast_sender->InitializeAudio(audio_sender_config, base::BindOnce(&LogAudioOperationalStatus));
    cast_sender->InitializeVideo(media_source.get_video_config(), base::BindRepeating(&LogVideoOperationalStatus), base::DoNothing());
    ...
    if (!source_path.empty()) {
        ...
        media_source.SetSourceFile(source_path, GetIntegerSwitchValue(kSourceFrameRate, 0));
    }
    media_source.Start(cast_sender->audio_frame_input(), cast_sender->video_frame_input());

    // By default runs simulation for 3 minutes or the desired duration
    // by using --run-time= flag.
    base::TimeDelta elapsed_time;
    const base::TimeDelta desired_run_time = base::Seconds(GetIntegerSwitchValue(kRunTime, 180));
    ...
    audio_event_subscriber.GetEventsAndReset(&audio_metadata, &audio_frame_events, &audio_packet_events);
    video_event_subscriber.GetEventsAndReset(&video_metadata, &video_frame_events, &video_packet_events);
    ...
    const base::TimeDelta elapsed_time_undropped
        = total_video_frames <= 0 ? base::TimeDelta() : (elapsed_time * (total_video_frames - dropped_video_frames) / total_video_frames);
    constexpr double kKilobitsPerByte = 8.0 / 1000;
    const double avg_encoded_bitrate = elapsed_time_undropped <= base::TimeDelta() ? 0 : encoded_size * kKilobitsPerByte * elapsed_time_undropped.ToHz();
    double avg_target_bitrate = encoded_video_frames ? target_bitrate / encoded_video_frames / 1000 : 0;

    LOG(INFO) << "Configured target playout delay (ms): " << video_receiver_config.rtp_max_delay_ms;
    ...
    LOG(INFO) << "Late video frames: " << late_video_frames
              << " (average lateness: " << (late_video_frames > 0 ? static_cast<double>(total_delay_of_late_frames_ms) / late_video_frames : 0) << " ms)";
    ...
    if (quality_test) {
        ...
        for (size_t i = 0; i < metrics_output.psnr.size() && i < metrics_output.ssim.size(); ++i) {
            base::StringAppendF(&line, "%f %f\n", metrics_output.psnr[i], metrics_output.ssim[i]);
        }
...
int main(int argc, char** argv)
{
    ...
    base::FilePath source_path = cmd->GetSwitchValuePath(media::cast::kSourcePath);
    base::FilePath log_output_path = cmd->GetSwitchValuePath(media::cast::kOutputPath);
    if (log_output_path.empty()) {
        base::GetTempDir(&log_output_path);
        log_output_path = log_output_path.AppendASCII("sim-events.gz");
    }
    base::FilePath metrics_output_path = cmd->GetSwitchValuePath(media::cast::kMetricsOutputPath);
    base::FilePath yuv_output_path = cmd->GetSwitchValuePath(media::cast::kYuvOutputPath);
    std::string sim_id = cmd->GetSwitchValueASCII(media::cast::kSimulationId);

    NetworkSimulationModel model = media::cast::LoadModel(cmd->GetSwitchValuePath(media::cast::kModelPath));
    ...
    // Run.
    media::cast::RunSimulation(source_path, log_output_path, metrics_output_path, yuv_output_path, extra_data, model);
    return 0;
}
