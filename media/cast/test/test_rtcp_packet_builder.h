...
public:
    ...
    void AddSrWithNtp(uint32_t remote_ssrc, uint32_t ntp_high, uint32_t ntp_low, uint32_t rtp_timestamp);
    ...
    void AddCast(uint32_t remote_ssrc, uint32_t local_ssrc, base::TimeDelta target_delay);
    ...
    void AddReceiverFrameLog(uint32_t rtp_timestamp, int num_events, uint32_t event_timesamp_base);
    void AddReceiverEventLog(uint16_t event_data, CastLoggingEvent event, uint16_t event_timesamp_delta);

    std::unique_ptr<Packet> GetPacket();
    const uint8_t* Data();
    int Length()
    {
        return kMaxIpPacketSize - big_endian_writer_.remaining();
    }
