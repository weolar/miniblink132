...
void CheckByteCounters()
{
    ...
    if ((now - counter->last_printout).InSeconds() >= 5) {
        fprintf(stderr, "Sending  : %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", counter->in_pipe_output_counter.megabits_per_second(),
            counter->in_pipe_input_counter.megabits_per_second(), counter->in_pipe_output_counter.packets_per_second(),
        ...
        fprintf(stderr, "Receiving: %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", counter->out_pipe_output_counter.megabits_per_second(),
            counter->out_pipe_input_counter.megabits_per_second(), counter->out_pipe_output_counter.packets_per_second(),
...
int main(int argc, char** argv)
{
    ...
    net::IPEndPoint remote_endpoint(remote_ip_address, static_cast<uint16_t>(remote_port));
    net::IPEndPoint local_endpoint(net::IPAddress::IPv4AllZeros(), static_cast<uint16_t>(local_port));
    std::unique_ptr<media::cast::test::PacketPipe> in_pipe, out_pipe;
    std::unique_ptr<media::cast::test::InterruptedPoissonProcess> ipp(media::cast::test::DefaultInterruptedPoissonProcess());
    ...
    SetupByteCounters(&in_pipe, &(counter->in_pipe_input_counter), &(counter->in_pipe_output_counter));
    SetupByteCounters(&out_pipe, &(counter->out_pipe_input_counter), &(counter->out_pipe_output_counter));

    printf("Press Ctrl-C when done.\n");
    std::unique_ptr<media::cast::test::UDPProxy> proxy(
        media::cast::test::UDPProxy::Create(local_endpoint, remote_endpoint, std::move(in_pipe), std::move(out_pipe), NULL));
