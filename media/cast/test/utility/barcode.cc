...
bool EncodeBarcode(const std::vector<bool>& bits, scoped_refptr<VideoFrame> output_frame)
{
    DCHECK(output_frame->format() == PIXEL_FORMAT_YV12 || output_frame->format() == PIXEL_FORMAT_I422 || output_frame->format() == PIXEL_FORMAT_I420);
    ...
    for (int row = 0; row < output_frame->rows(VideoFrame::kYPlane); row++) {
        memcpy(output_frame->writable_data(VideoFrame::kYPlane) + output_frame->stride(VideoFrame::kYPlane) * row, &bytes.front(), row_bytes);
    }
    return true;
}

namespace {
bool DecodeBarCodeRows(const VideoFrame& frame, std::vector<bool>* output, int min_row, int max_row)
{
...
{
    DCHECK(frame.format() == PIXEL_FORMAT_YV12 || frame.format() == PIXEL_FORMAT_I422 || frame.format() == PIXEL_FORMAT_I420);
    int rows = frame.rows(VideoFrame::kYPlane);
    // Middle 10 lines
    if (DecodeBarCodeRows(frame, output, std::max(0, rows / 2 - 5), std::min(rows, rows / 2 + 5))) {
        return true;
    }
