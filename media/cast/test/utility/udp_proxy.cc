...
PacketPipe::~PacketPipe() = default;
void PacketPipe::InitOnIOThread(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock)
{
...
private:
    void Schedule()
    {
        ...
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&Buffer::ProcessBuffer, weak_factory_.GetWeakPtr()), base::Microseconds(microseconds));
    }

    void ProcessBuffer()
    {
        int64_t bytes_to_send = static_cast<int64_t>((clock_->NowTicks() - last_schedule_).InSecondsF() * max_megabits_per_second_ * 1E6 / 8);
...
public:
    ...
    void Send(std::unique_ptr<Packet> packet) override
    {
        ...
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&SimpleDelayBase::SendInternal, weak_factory_.GetWeakPtr(), std::move(packet)),
...
std::unique_ptr<PacketPipe> NewDuplicateAndDelay(double delay_min, double random_delay)
{
    return std::unique_ptr<PacketPipe>(new DuplicateAndDelay(delay_min, random_delay));
}

class RandomSortedDelay : public PacketPipe {
public:
    RandomSortedDelay(double random_delay, double extra_delay, double seconds_between_extra_delay)
        : random_delay_(random_delay)
    ...
    void Send(std::unique_ptr<Packet> packet) final
    {
        buffer_.push_back(std::move(packet));
        if (buffer_.size() == 1) {
            next_send_ = std::max(clock_->NowTicks() + base::Seconds(base::RandDouble() * random_delay_), next_send_);
            ProcessBuffer();
        }
    }
    void InitOnIOThread(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock) final
    {
...
private:
    void ScheduleExtraDelay(double mult)
    {
        ...
        task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&RandomSortedDelay::CauseExtraDelay, weak_factory_.GetWeakPtr()), base::Microseconds(microseconds));
    }

    void CauseExtraDelay()
    {
        next_send_ = std::max<base::TimeTicks>(clock_->NowTicks() + base::Microseconds(static_cast<int64_t>(extra_delay_ * 1E6)), next_send_);
    ...
    void ProcessBuffer()
    {
        ...
        if (!buffer_.empty()) {
            task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&RandomSortedDelay::ProcessBuffer, weak_factory_.GetWeakPtr()), next_send_ - now);
        }
    }
...
std::unique_ptr<PacketPipe> NewRandomSortedDelay(double random_delay, double extra_delay, double seconds_between_extra_delay)
{
    return std::unique_ptr<PacketPipe>(new RandomSortedDelay(random_delay, extra_delay, seconds_between_extra_delay));
}
...
private:
    void Flip()
    {
        ...
        int64_t microseconds = static_cast<int64_t>(seconds * 1E6);
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&NetworkGlitchPipe::Flip, weak_factory_.GetWeakPtr()), base::Microseconds(microseconds));
    }
...
std::unique_ptr<PacketPipe> NewNetworkGlitchPipe(double average_work_time, double average_outage_time)
{
    return std::unique_ptr<PacketPipe>(new NetworkGlitchPipe(average_work_time, average_outage_time));
}
...
InterruptedPoissonProcess::InterruptedPoissonProcess(const std::vector<double>& average_rates, double coef_burstiness, double coef_variance, uint32_t rand_seed)
    : clock_(nullptr)
...
void InterruptedPoissonProcess::InitOnIOThread(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock)
{
...
{
    std::unique_ptr<InternalBuffer> buffer(new InternalBuffer(weak_factory_.GetWeakPtr(), size));
...
void InterruptedPoissonProcess::UpdateRates()
{
    ...
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&InterruptedPoissonProcess::UpdateRates, weak_factory_.GetWeakPtr()), base::Seconds(1));
}

void InterruptedPoissonProcess::SwitchOff()
{
    on_state_ = false;
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&InterruptedPoissonProcess::SwitchOn, weak_factory_.GetWeakPtr()), NextEvent(switch_on_rate_));
}

void InterruptedPoissonProcess::SwitchOn()
{
    on_state_ = true;
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&InterruptedPoissonProcess::SwitchOff, weak_factory_.GetWeakPtr()), NextEvent(switch_off_rate_));
}

void InterruptedPoissonProcess::SendPacket()
{
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&InterruptedPoissonProcess::SendPacket, weak_factory_.GetWeakPtr()), NextEvent(send_rate_));
...
public:
    ...
    void Send(std::unique_ptr<Packet> packet) final;
    void AppendToPipe(std::unique_ptr<PacketPipe> pipe) final
    {
        NOTREACHED();
    }
...
std::unique_ptr<InterruptedPoissonProcess> DefaultInterruptedPoissonProcess()
{
    ...
    std::unique_ptr<InterruptedPoissonProcess> ipp(new InterruptedPoissonProcess(average_rates, burstiness, variance, 0));
    return ipp;
}

class UDPProxyImpl final : public UDPProxy {
public:
    UDPProxyImpl(const net::IPEndPoint& local_port, const net::IPEndPoint& destination, std::unique_ptr<PacketPipe> to_dest_pipe,
        ...
        , blocked_(false)
    {
        ...
        proxy_thread_.StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0));
        base::WaitableEvent start_event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
        proxy_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&UDPProxyImpl::Start, base::Unretained(this), base::Unretained(&start_event), net_log));
        start_event.Wait();
    }
    ...
    {
        base::ScopedAllowBlockingForTesting allow_blocking;
        base::WaitableEvent stop_event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
        proxy_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&UDPProxyImpl::Stop, base::Unretained(this), base::Unretained(&stop_event)));
        stop_event.Wait();
        proxy_thread_.Stop();
    }

    void Send(std::unique_ptr<Packet> packet, const net::IPEndPoint& destination)
    {
        if (blocked_) {
        ...
        auto buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&packet->front()));
        ...
        if (destination.address().empty()) {
            ...
            result = socket_->SendTo(buf.get(), static_cast<int>(buf_size), destination,
                base::BindOnce(&UDPProxyImpl::AllowWrite, weak_factory_.GetWeakPtr(), buf, std::move(packet)));
        }
...
private:
    ...
    {
        socket_ = std::make_unique<net::UDPServerSocket>(net_log, net::NetLogSource());
        ...
        to_dest_pipe_->InitOnIOThread(base::ThreadTaskRunnerHandle::Get(), base::DefaultTickClock::GetInstance());
        from_dest_pipe_->InitOnIOThread(base::ThreadTaskRunnerHandle::Get(), base::DefaultTickClock::GetInstance());
    ...
    void PollRead()
    {
        while (true) {
            ...
            auto recv_buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&packet_->front()));
            int len = socket_->RecvFrom(
                recv_buf.get(), kMaxPacketSize, &recv_address_, base::BindOnce(&UDPProxyImpl::ReadCallback, base::Unretained(this), recv_buf));
    ...
    void AllowWrite(scoped_refptr<net::IOBuffer> buf, std::unique_ptr<Packet> packet, int unused_len)
    {
        DCHECK(blocked_);
        blocked_ = false;
    }
...
std::unique_ptr<UDPProxy> UDPProxy::Create(const net::IPEndPoint& local_port, const net::IPEndPoint& destination, std::unique_ptr<PacketPipe> to_dest_pipe,
    std::unique_ptr<PacketPipe> from_dest_pipe, net::NetLog* net_log)
{
    std::unique_ptr<UDPProxy> ret(new UDPProxyImpl(local_port, destination, std::move(to_dest_pipe), std::move(from_dest_pipe), net_log));
    return ret;
}
