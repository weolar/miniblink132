...
public:
    ...
    virtual void InitOnIOThread(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock);
...
public:
    InterruptedPoissonProcess(const std::vector<double>& average_rates, double coef_burstiness, double coef_variance, uint32_t rand_seed);

    InterruptedPoissonProcess(const InterruptedPoissonProcess&) = delete;
    InterruptedPoissonProcess& operator=(const InterruptedPoissonProcess&) = delete;
...
private:
    ...
    void InitOnIOThread(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, const base::TickClock* clock);
...
public:
    ...
    static std::unique_ptr<UDPProxy> Create(const net::IPEndPoint& local_port, const net::IPEndPoint& destination, std::unique_ptr<PacketPipe> to_dest_pipe,
...
std::unique_ptr<PacketPipe> NewRandomSortedDelay(double random_delay, double big_delay, double seconds_between_big_delay);
