...
public:
    ...
    InProcessReceiver(const scoped_refptr<CastEnvironment>& cast_environment, const net::IPEndPoint& local_end_point, const net::IPEndPoint& remote_end_point,
        const FrameReceiverConfig& audio_config, const FrameReceiverConfig& video_config);
    ...
    scoped_refptr<CastEnvironment> cast_env() const
    {
        return cast_environment_;
    }
    const FrameReceiverConfig& audio_config() const
    {
        return audio_config_;
    }
    const FrameReceiverConfig& video_config() const
    {
...
protected:
    ...
    virtual void OnAudioFrame(std::unique_ptr<AudioBus> audio_frame, base::TimeTicks playout_time, bool is_continuous) = 0;
    virtual void OnVideoFrame(scoped_refptr<VideoFrame> video_frame, base::TimeTicks playout_time, bool is_continuous) = 0;
...
private:
    ...
    void GotAudioFrame(std::unique_ptr<AudioBus> audio_frame, base::TimeTicks playout_time, bool is_continuous);
    void GotVideoFrame(scoped_refptr<VideoFrame> video_frame, base::TimeTicks playout_time, bool is_continuous);
