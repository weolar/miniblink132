...
public:
    ...
    {
        while (true) {
            int written = write(fd_, reinterpret_cast<char*>(&packet->front()), packet->size());
            if (written < 0) {
                if (errno == EINTR)
                    continue;
                perror("write");
...
public:
        ...
        , packet_pipe_(std::move(pipe))
    {
        read_socket_watch_controller_
            = base::FileDescriptorWatcher::WatchReadable(input_fd_, base::BindRepeating(&QueueManager::OnFileCanReadWithoutBlocking, base::Unretained(this)));
        ...
        packet_pipe_->InitOnIOThread(base::ThreadTaskRunnerHandle::Get(), base::DefaultTickClock::GetInstance());
    }
...
private:
    void OnFileCanReadWithoutBlocking()
    {
        ...
        int nread = read(input_fd_, reinterpret_cast<char*>(&packet->front()), kMaxPacketSize);
        if (nread < 0) {
            if (errno == EINTR)
                return;
            perror("read");
    ...
    std::unique_ptr<base::FileDescriptorWatcher::Controller> read_socket_watch_controller_;
};
...
void SetupByteCounters(std::unique_ptr<media::cast::test::PacketPipe>* pipe, ByteCounter* pipe_input_counter, ByteCounter* pipe_output_counter)
{
    media::cast::test::PacketPipe* new_pipe = new ByteCounterPipe(pipe_input_counter);
    new_pipe->AppendToPipe(std::move(*pipe));
    new_pipe->AppendToPipe(std::unique_ptr<media::cast::test::PacketPipe>(new ByteCounterPipe(pipe_output_counter)));
    pipe->reset(new_pipe);
}
...
void CheckByteCounters()
{
    ...
    if ((now - last_printout).InSeconds() >= 5) {
        fprintf(stderr, "Sending  : %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", in_pipe_output_counter.megabits_per_second(),
            in_pipe_input_counter.megabits_per_second(), in_pipe_output_counter.packets_per_second(), in_pipe_input_counter.packets_per_second());
        fprintf(stderr, "Receiving: %5.2f / %5.2f mbps %6.2f / %6.2f packets / s\n", out_pipe_output_counter.megabits_per_second(),
            out_pipe_input_counter.megabits_per_second(), out_pipe_output_counter.packets_per_second(), out_pipe_input_counter.packets_per_second());

        last_printout = now;
    }
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce(&CheckByteCounters), base::Milliseconds(100));
}
...
int main(int argc, char** argv)
{
    ...
    SetupByteCounters(&out_pipe, &out_pipe_input_counter, &out_pipe_output_counter);
