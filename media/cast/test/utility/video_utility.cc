...
double I420PSNR(const media::VideoFrame& frame1, const media::VideoFrame& frame2)
{
    if (frame1.visible_rect().width() != frame2.visible_rect().width() || frame1.visible_rect().height() != frame2.visible_rect().height())
        return -1;

    return libyuv::I420Psnr(frame1.visible_data(VideoFrame::Plane::kY), frame1.stride(VideoFrame::Plane::kY), frame1.visible_data(VideoFrame::Plane::kU),
        frame1.stride(VideoFrame::Plane::kU), frame1.visible_data(VideoFrame::Plane::kV), frame1.stride(VideoFrame::Plane::kV),
        frame2.visible_data(VideoFrame::Plane::kY), frame2.stride(VideoFrame::Plane::kY), frame2.visible_data(VideoFrame::Plane::kU),
        frame2.stride(VideoFrame::Plane::kU), frame2.visible_data(VideoFrame::Plane::kV), frame2.stride(VideoFrame::Plane::kV), frame1.visible_rect().width(),
        frame1.visible_rect().height());
}

double I420SSIM(const media::VideoFrame& frame1, const media::VideoFrame& frame2)
{
    if (frame1.visible_rect().width() != frame2.visible_rect().width() || frame1.visible_rect().height() != frame2.visible_rect().height())
        return -1;

    return libyuv::I420Ssim(frame1.visible_data(VideoFrame::Plane::kY), frame1.stride(VideoFrame::Plane::kY), frame1.visible_data(VideoFrame::Plane::kU),
        frame1.stride(VideoFrame::Plane::kU), frame1.visible_data(VideoFrame::Plane::kV), frame1.stride(VideoFrame::Plane::kV),
        frame2.visible_data(VideoFrame::Plane::kY), frame2.stride(VideoFrame::Plane::kY), frame2.visible_data(VideoFrame::Plane::kU),
        frame2.stride(VideoFrame::Plane::kU), frame2.visible_data(VideoFrame::Plane::kV), frame2.stride(VideoFrame::Plane::kV), frame1.visible_rect().width(),
        frame1.visible_rect().height());
...
void PopulateVideoFrameWithNoise(VideoFrame* frame)
{
    ...
    base::span<uint8_t> y_plane =
        ...
        // unbounded pointer.
        UNSAFE_TODO(base::span(frame->writable_data(VideoFrame::Plane::kY), height * base::checked_cast<size_t>(frame->stride(VideoFrame::Plane::kY))));
    base::span<uint8_t> u_plane =
        ...
        // TODO(crbug.com/338570700): Make VideoFrame return a span instead of an
        // unbounded pointer.
        UNSAFE_TODO(base::span(frame->writable_data(VideoFrame::Plane::kU), half_height * base::checked_cast<size_t>(frame->stride(VideoFrame::Plane::kU))));
    base::span<uint8_t> v_plane =
        // SAFETY: The V plane has a width specified by stride() and a height that
        // is half of coded_size(), rounding up.
        // TODO(crbug.com/338570700): Make VideoFrame return a span instead of an
        // unbounded pointer.
        UNSAFE_TODO(base::span(frame->writable_data(VideoFrame::Plane::kV), half_height * base::checked_cast<size_t>(frame->stride(VideoFrame::Plane::kV))));
...
bool PopulateVideoFrameFromFile(VideoFrame* frame, FILE* video_file)
{
    ...
    memcpy(v_plane, raw_data + width * height + half_width * half_height, half_width * half_height);
    delete[] raw_data;
    return true;
}
