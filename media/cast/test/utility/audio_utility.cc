...
TestAudioBusFactory::TestAudioBusFactory(int num_channels, int sample_rate, float sine_wave_frequency, float volume)
...
std::unique_ptr<AudioBus> TestAudioBusFactory::NextAudioBus(const base::TimeDelta& duration)
{
...
bool EncodeTimestamp(uint16_t timestamp, size_t sample_offset, size_t length, float* samples)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        for (size_t f = 0; f < frequencies.size(); f++) {
            mix_of_components += sin((i + sample_offset) * std::numbers::pi * 2.0 * frequencies[f] / kSamplingFrequency);
        }
...
// bogus data.
double DecodeOneFrequency(const float* samples, size_t length, double frequency)
{
    double sin_sum = 0.0;
    double cos_sum = 0.0;
    for (size_t i = 0; i < length; i++) {
        sin_sum += samples[i] * sin(i * std::numbers::pi * 2 * frequency / kSamplingFrequency);
        cos_sum += samples[i] * cos(i * std::numbers::pi * 2 * frequency / kSamplingFrequency);
    }
...
{
    for (size_t start = 0; start + kSamplesToAnalyze <= length; start += kSamplesToAnalyze / 4) {
        double sense = DecodeOneFrequency(&samples[start], kSamplesToAnalyze, kSenseFrequency);
        if (sense < kMinSense)
            continue;
        bool success = true;
        uint16_t gray_coded = 0;
        for (size_t bit = 0; success && bit < kNumBits; bit++) {
            double signal_strength = DecodeOneFrequency(&samples[start], kSamplesToAnalyze, kBaseFrequency * (bit + 1));
