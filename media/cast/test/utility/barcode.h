...
// from |bits|.
bool EncodeBarcode(const std::vector<bool>& bits, scoped_refptr<media::VideoFrame> output_frame);
...
// types directly.
template <class T> bool EncodeBarcode(T data, scoped_refptr<media::VideoFrame> output_frame)
{
...
template <class T> bool DecodeBarcode(const media::VideoFrame& output_frame, T* data)
{
    std::vector<bool> bits(sizeof(T) * 8);
