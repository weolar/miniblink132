...
scoped_refptr<VideoFrame> CreateFrame(const uint8_t* y_plane_data, const gfx::Size& size)
{
    scoped_refptr<VideoFrame> result = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, base::TimeDelta());
    for (int y = 0, y_end = size.height(); y < y_end; ++y) {
        memcpy(
            result->GetWritableVisibleData(VideoFrame::Plane::kY) + y * result->stride(VideoFrame::Plane::kY), y_plane_data + y * size.width(), size.width());
    }
    return result;
}
...
TEST(QuantizerEstimatorTest, EstimatesForTrivialFrames)
{
    ...
    const auto black_frame_data = std::make_unique<uint8_t[]>(frame_size.GetArea());
    ...
    const auto checkerboard_frame_data = std::make_unique<uint8_t[]>(frame_size.GetArea());
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        const auto random_frame_data = std::make_unique<uint8_t[]>(frame_size.GetArea());
