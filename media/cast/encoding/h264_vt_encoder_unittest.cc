...
void SaveOperationalStatus(OperationalStatus* out_status, OperationalStatus in_status)
{
...
public:
    ...
    void PushExpectation(
        FrameId expected_frame_id, FrameId expected_last_referenced_frame_id, RtpTimeTicks expected_rtp_timestamp, base::TimeTicks expected_reference_time)
    {
        expectations_.push(Expectation { expected_frame_id, expected_last_referenced_frame_id, expected_rtp_timestamp, expected_reference_time });
    }

    void CompareFrameWithExpected(std::unique_ptr<SenderEncodedFrame> encoded_frame)
    {
        ...
        EXPECT_EQ(e.expected_last_referenced_frame_id, encoded_frame->referenced_frame_id) << "frame id: " << e.expected_frame_id;
...
class EndToEndFrameChecker : public base::RefCountedThreadSafe<EndToEndFrameChecker> {
public:
        ...
        , count_frames_checked_(0)
    {
        ...
        decoder_.Initialize(config, false, nullptr, base::BindOnce(&SaveDecoderInitResult, &decoder_init_result),
            base::BindRepeating(&EndToEndFrameChecker::CompareFrameWithExpected, base::Unretained(this)), base::NullCallback());
    ...
    {
        auto buffer = DecoderBuffer::CopyFrom(encoded_frame->bytes(), encoded_frame->data.size());
        decoder_.Decode(buffer, base::BindOnce(&EndToEndFrameChecker::DecodeDone, base::Unretained(this)));
    }
    ...
    int count_frames_checked() const
    {
        return count_frames_checked_;
    }
...
{
    const scoped_refptr<media::VideoFrame> video_frame = video_frame_factory->MaybeCreateFrame(gfx::Size(kVideoWidth, kVideoHeight), base::TimeDelta());
...
public:
    ...
    H264VideoToolboxEncoderTest& operator=(const H264VideoToolboxEncoderTest&) = delete;
...
protected:
    ...
    void SetUp() final
    {
        ...
        base::PowerMonitor::Initialize(std::unique_ptr<TestPowerSource>(power_source_));

        cast_environment_ = new CastEnvironment(
            &clock_, task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner());
        encoder_ = std::make_unique<H264VideoToolboxEncoder>(
            cast_environment_, video_sender_config_, base::BindRepeating(&SaveOperationalStatus, &operational_status_));
        base::RunLoop().RunUntilIdle();
    ...
    static void SetUpTestCase()
    {
        ...
        frame_ = media::VideoFrame::CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, base::TimeDelta());
...
TEST_F(H264VideoToolboxEncoderTest, DISABLED_CheckFrameMetadataSequence)
{
    ...
    metadata_recorder->PushExpectation(FrameId::first(), FrameId::first(), ToRtpTimeTicks(frame_->timestamp(), kVideoFrequency), clock_.NowTicks());
    EXPECT_TRUE(encoder_->EncodeVideoFrame(frame_, clock_.NowTicks(), base::BindOnce(&MetadataRecorder::CompareFrameWithExpected, metadata_recorder.get())));
    base::RunLoop().RunUntilIdle();

    for (FrameId frame_id = FrameId::first() + 1; frame_id < FrameId::first() + 10; ++frame_id) {
        AdvanceClockAndVideoFrameTimestamp();
        metadata_recorder->PushExpectation(frame_id, frame_id - 1, ToRtpTimeTicks(frame_->timestamp(), kVideoFrequency), clock_.NowTicks());
        EXPECT_TRUE(
            encoder_->EncodeVideoFrame(frame_, clock_.NowTicks(), base::BindOnce(&MetadataRecorder::CompareFrameWithExpected, metadata_recorder.get())));
    }
...
{
    const auto alpha_mode = IsOpaque(frame_->format()) ? VideoDecoderConfig::AlphaMode::kIsOpaque : VideoDecoderConfig::AlphaMode::kHasAlpha;
    VideoDecoderConfig config(VideoCodec::kH264, H264PROFILE_MAIN, alpha_mode, VideoColorSpace(), kNoTransformation, frame_->coded_size(),
        frame_->visible_rect(), frame_->natural_size(), EmptyExtraData(), EncryptionScheme::kUnencrypted);
    scoped_refptr<EndToEndFrameChecker> checker(new EndToEndFrameChecker(config));

    for (FrameId frame_id = FrameId::first(); frame_id < FrameId::first() + 6; ++frame_id) {
        checker->PushExpectation(frame_);
        EXPECT_TRUE(encoder_->EncodeVideoFrame(frame_, clock_.NowTicks(), base::BindOnce(&EndToEndFrameChecker::EncodeDone, checker.get())));
        AdvanceClockAndVideoFrameTimestamp();
    }
...
TEST_F(H264VideoToolboxEncoderTest, CheckVideoFrameFactory)
{
    ...
    ASSERT_FALSE(video_frame_factory->MaybeCreateFrame(gfx::Size(kVideoWidth, kVideoHeight), base::TimeDelta()));
    base::RunLoop().RunUntilIdle();
...
TEST_F(H264VideoToolboxEncoderTest, CheckPowerMonitoring)
{
    ...
    EXPECT_TRUE(encoder_->EncodeVideoFrame(frame_, clock_.NowTicks(), base::DoNothing()));
    power_source_->GenerateSuspendEvent();
    EXPECT_FALSE(encoder_->EncodeVideoFrame(frame_, clock_.NowTicks(), base::DoNothing()));
...
TEST_F(H264VideoToolboxEncoderTest, CheckPowerMonitoringVideoFrameFactoryNoInitialFrame)
{
    auto video_frame_factory = encoder_->CreateVideoFrameFactory();
    ASSERT_TRUE(video_frame_factory.get());
