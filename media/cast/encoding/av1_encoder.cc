...
Av1Encoder::Av1Encoder(const FrameSenderConfig& video_config, std::unique_ptr<VideoEncoderMetricsProvider> metrics_provider)
...
void Av1Encoder::ConfigureForNewFrameSize(const gfx::Size& frame_size)
{
    ...
    CHECK_EQ(aom_codec_enc_config_default(aom_codec_av1_cx(), &config_, AOM_USAGE_REALTIME), AOM_CODEC_OK);
    ...
    if (aom_codec_err_t ret = aom_codec_enc_init(&encoder_, aom_codec_av1_cx(), &config_, flags); ret != AOM_CODEC_OK) {
        metrics_provider_->SetError(
            { media::EncoderStatus::Codes::kEncoderInitializationError, base::StrCat({ "libvpx failed to initialize: ", aom_codec_err_to_string(ret) }) });
        LOG(FATAL) << "aom_codec_enc_init() failed: " << aom_codec_err_to_string(ret);
    }

    CHECK_EQ(aom_codec_control(&encoder_, AV1E_SET_TUNE_CONTENT, AOM_CONTENT_SCREEN), AOM_CODEC_OK);
    CHECK_EQ(aom_codec_control(&encoder_, AV1E_SET_ENABLE_PALETTE, 1), AOM_CODEC_OK);
    ...
    encoding_speed_ = kHighestEncodingSpeed;
    CHECK_EQ(aom_codec_control(&encoder_, AOME_SET_CPUUSED, encoding_speed_), AOM_CODEC_OK);
}

void Av1Encoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    aom_image_t* const result = aom_img_wrap(
        &aom_image, aom_format, frame_size.width(), frame_size.height(), 1, const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kY)));
    DCHECK_EQ(result, &aom_image);

    aom_image.planes[AOM_PLANE_Y] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kY));
    aom_image.planes[AOM_PLANE_U] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kU));
    aom_image.planes[AOM_PLANE_V] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kV));
    ...
    if (aom_codec_err_t ret
        = aom_codec_encode(&encoder_, &aom_image, 0, predicted_frame_duration.InMicroseconds(), key_frame_requested_ ? AOM_EFLAG_FORCE_KF : 0);
        ret != AOM_CODEC_OK) {
        metrics_provider_->SetError({ media::EncoderStatus::Codes::kEncoderFailedEncode,
            base::StrCat({ "libaom failed to encode: ", aom_codec_err_to_string(ret), " - ", aom_codec_error_detail(&encoder_) }) });
    ...
    while ((pkt = aom_codec_get_cx_data(&encoder_, &iter)) != nullptr) {
        ...
        if (pkt->data.frame.flags & AOM_FRAME_IS_KEY) {
            ...
            encoded_frame->dependency = openscreen::cast::EncodedFrame::Dependency::kKeyFrame;
            encoded_frame->referenced_frame_id = encoded_frame->frame_id;
        } else {
            encoded_frame->dependency = openscreen::cast::EncodedFrame::Dependency::kDependent;
        ...
        encoded_frame->data
            = base::HeapArray<uint8_t>::CopiedFrom(base::span<const uint8_t>(static_cast<const uint8_t*>(pkt->data.frame.buf), pkt->data.frame.sz));
    ...
    int quantizer = -1;
    CHECK_EQ(aom_codec_control(&encoder_, AOME_GET_LAST_QUANTIZER_64, &quantizer), AOM_CODEC_OK);
    ...
    DVLOG(2) << "AV1 encoded frame_id " << encoded_frame->frame_id << ", sized: " << encoded_frame->data.size()
    ...
    if (encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) {
