...
FakeSoftwareVideoEncoder::FakeSoftwareVideoEncoder(const FrameSenderConfig& video_config)
...
void FakeSoftwareVideoEncoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    ...
    const auto values = base::Value::Dict()
                            .Set("key", encoded_frame->dependency == Dependency::kKeyFrame)
                            .Set("ref", static_cast<int>(encoded_frame->referenced_frame_id.lower_32_bits()))
    ...
    encoded_frame->data = base::HeapArray<uint8_t>::CopiedFrom(base::as_bytes(base::span(raw_data)));
