...
using VideoCodecBitset = std::bitset<static_cast<size_t>(VideoCodec::kMaxValue) + 1>;
...
bool IsHardwareEncodingEnabled(
    const std::vector<VideoEncodeAccelerator::SupportedProfile>& profiles, VideoCodecProfile min_profile, VideoCodecProfile max_profile)
{
    for (const auto& vea_profile : profiles) {
        if (vea_profile.profile >= min_profile && vea_profile.profile <= max_profile) {
            return true;
        }
    }
    return false;
...
bool IsHardwareVP8EncodingEnabled(const std::vector<VideoEncodeAccelerator::SupportedProfile>& profiles)
{
    if (!base::FeatureList::IsEnabled(kCastStreamingVp8)) {
        return false;
    }

    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kCastStreamingForceDisableHardwareVp8)) {
        return false;
    }
...
bool IsHardwareVP9EncodingEnabled(const std::vector<VideoEncodeAccelerator::SupportedProfile>& profiles)
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kCastStreamingForceDisableHardwareVp9)) {
        return false;
    }
...
bool IsHardwareH264EncodingEnabled(const std::vector<VideoEncodeAccelerator::SupportedProfile>& profiles)
{
    // Force disabling takes precedent over other flags.
    const base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
    if (command_line.HasSwitch(switches::kCastStreamingForceDisableHardwareH264)) {
        return false;
    }

#if BUILDFLAG(IS_MAC)
    if (!command_line.HasSwitch(switches::kCastStreamingForceEnableHardwareH264) && !base::FeatureList::IsEnabled(kCastStreamingMacHardwareH264)) {
        return false;
    }
#endif
    ...
    if (!command_line.HasSwitch(switches::kCastStreamingForceEnableHardwareH264) && !base::FeatureList::IsEnabled(kCastStreamingWinHardwareH264)) {
        return false;
    }
#endif
...
bool IsHardwareEnabled(VideoCodec codec, const std::vector<VideoEncodeAccelerator::SupportedProfile>& profiles)
{
