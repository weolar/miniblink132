...
bool HasSufficientFeedback(const FeedbackSignalAccumulator<base::TimeDelta>& accumulator)
{
...
VpxEncoder::VpxEncoder(const FrameSenderConfig& video_config, std::unique_ptr<VideoEncoderMetricsProvider> metrics_provider)
    : cast_config_(video_config)
    , codec_params_(cast_config_.video_codec_params.value())
    , target_encoder_utilization_(codec_params_->number_of_encode_threads > 2
              ? kHiTargetEncoderUtilization
              : (codec_params_->number_of_encode_threads > 1 ? kMidTargetEncoderUtilization : kLoTargetEncoderUtilization))
...
void VpxEncoder::ConfigureForNewFrameSize(const gfx::Size& frame_size)
{
    if (is_initialized()) {
        ...
        if (frame_size.GetArea() <= gfx::Size(config_.g_w, config_.g_h).GetArea()) {
            DVLOG(1) << "Continuing to use existing encoder at smaller frame size: " << gfx::Size(config_.g_w, config_.g_h).ToString() << " --> "
                     << frame_size.ToString();
        ...
        DVLOG(1) << "Destroying/Re-Creating encoder for larger frame size: " << gfx::Size(config_.g_w, config_.g_h).ToString() << " --> "
                 << frame_size.ToString();
        vpx_codec_destroy(&encoder_);
    } else {
        DVLOG(1) << "Creating encoder for the first frame; size: " << frame_size.ToString();
    }
    ...
    metrics_provider_->Initialize(
        codec_params_->codec == VideoCodec::kVP9 ? media::VP9PROFILE_MIN : media::VP8PROFILE_ANY, frame_size, /*is_hardware_encoder=*/false);
    if (vpx_codec_err_t ret = vpx_codec_enc_init(&encoder_, ctx, &config_, flags); ret != VPX_CODEC_OK) {
    ...
    CHECK_EQ(vpx_codec_control(&encoder_, VP8E_SET_STATIC_THRESHOLD, 100), VPX_CODEC_OK);

    if (codec_params_->codec == VideoCodec::kVP9) {
        CHECK_EQ(vpx_codec_control(&encoder_, VP9E_SET_TUNE_CONTENT, VP9E_CONTENT_SCREEN), VPX_CODEC_OK);
    } else {
        ...
        const unsigned int screen_content_mode = base::FeatureList::IsEnabled(kCastVideoEncoderFrameDrop) ? 2 : 1;
        CHECK_EQ(vpx_codec_control(&encoder_, VP8E_SET_SCREEN_CONTENT_MODE, screen_content_mode), VPX_CODEC_OK);
    }
    ...
    encoding_speed_ = kHighestEncodingSpeed;
    CHECK_EQ(vpx_codec_control(&encoder_, VP8E_SET_CPUUSED, -encoding_speed_), VPX_CODEC_OK);
}

void VpxEncoder::Encode(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, SenderEncodedFrame* encoded_frame)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    vpx_img_fmt_t vpx_format = video_frame->format() == PIXEL_FORMAT_NV12 ? VPX_IMG_FMT_NV12 : VPX_IMG_FMT_I420;
    vpx_image_t vpx_image;
    vpx_image_t* const result = vpx_img_wrap(
        &vpx_image, vpx_format, frame_size.width(), frame_size.height(), 1, const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kY)));
    DCHECK_EQ(result, &vpx_image);
    switch (vpx_format) {
    case VPX_IMG_FMT_I420:
        vpx_image.planes[VPX_PLANE_Y] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kY));
        vpx_image.planes[VPX_PLANE_U] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kU));
        vpx_image.planes[VPX_PLANE_V] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kV));
        vpx_image.stride[VPX_PLANE_Y] = video_frame->stride(VideoFrame::Plane::kY);
        vpx_image.stride[VPX_PLANE_U] = video_frame->stride(VideoFrame::Plane::kU);
        vpx_image.stride[VPX_PLANE_V] = video_frame->stride(VideoFrame::Plane::kV);
        break;
    case VPX_IMG_FMT_NV12:
        vpx_image.planes[VPX_PLANE_Y] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kY));
        // In libvpx, the UV plane of NV12 frames is represented by two planes
        // with the same stride, shifted by one byte.
        vpx_image.planes[VPX_PLANE_U] = const_cast<uint8_t*>(video_frame->visible_data(VideoFrame::Plane::kUV));
        vpx_image.planes[VPX_PLANE_V] = vpx_image.planes[VPX_PLANE_U] + 1;
        vpx_image.stride[VPX_PLANE_Y] = video_frame->stride(VideoFrame::Plane::kY);
        vpx_image.stride[VPX_PLANE_U] = video_frame->stride(VideoFrame::Plane::kUV);
        vpx_image.stride[VPX_PLANE_V] = video_frame->stride(VideoFrame::Plane::kUV);
        break;
    default:
        NOTREACHED();
    }
    ...
    const base::TimeDelta minimum_frame_duration = base::Seconds(1.0 / cast_config_.max_frame_rate);
    const base::TimeDelta maximum_frame_duration = base::Seconds(static_cast<double>(kRestartFramePeriods) / cast_config_.max_frame_rate);
    base::TimeDelta predicted_frame_duration = video_frame->metadata().frame_duration.value_or(base::TimeDelta());
    ...
    predicted_frame_duration = std::max(minimum_frame_duration, std::min(maximum_frame_duration, predicted_frame_duration));
    ...
    if (vpx_codec_err_t ret
        = vpx_codec_encode(&encoder_, &vpx_image, 0, predicted_frame_duration.InMicroseconds(), key_frame_requested_ ? VPX_EFLAG_FORCE_KF : 0, VPX_DL_REALTIME);
        ret != VPX_CODEC_OK) {
        metrics_provider_->SetError({ media::EncoderStatus::Codes::kEncoderFailedEncode,
            base::StrCat({ "libvpx failed to encode: ", vpx_codec_err_to_string(ret), " - ", vpx_codec_error_detail(&encoder_) }) });
    ...
    while ((pkt = vpx_codec_get_cx_data(&encoder_, &iter)) != NULL) {
        ...
        encoded_frame->rtp_timestamp = ToRtpTimeTicks(video_frame->timestamp(), kVideoFrequency);
        encoded_frame->reference_time = reference_time;
        encoded_frame->data = base::HeapArray<uint8_t>::CopiedFrom(base::span(static_cast<const uint8_t*>(pkt->data.frame.buf), pkt->data.frame.sz));
    ...
    encoded_frame->encoder_utilization = processing_time / predicted_frame_duration;
    ...
    int quantizer = -1;
    CHECK_EQ(vpx_codec_control(&encoder_, VP8E_GET_LAST_QUANTIZER_64, &quantizer), VPX_CODEC_OK);
    ...
    DVLOG(2) << "VPX encoded frame_id " << encoded_frame->frame_id << ", sized: " << encoded_frame->data.size()
             << ", encoder_utilization: " << encoded_frame->encoder_utilization << ", lossiness: " << encoded_frame->lossiness
    ...
    if (encoded_frame->dependency == Dependency::kKeyFrame) {
        ...
        // quantizer.
        double actual_encoding_speed = encoding_speed_ + kEquivalentEncodingSpeedStepPerQpStep * std::max(0, quantizer - codec_params_->min_qp);
        double adjusted_encoding_speed = actual_encoding_speed * encoded_frame->encoder_utilization / target_encoder_utilization_;
        encoding_speed_acc_.Update(adjusted_encoding_speed, video_frame->timestamp());
    }
    ...
    if (HasSufficientFeedback(encoding_speed_acc_)) {
        ...
        if (next_encoding_speed > kHighestEncodingSpeed) {
            ...
            next_min_qp = static_cast<int>(remainder / kEquivalentEncodingSpeedStepPerQpStep + codec_params_->min_qp + 0.5);
