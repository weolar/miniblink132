...
public:
    ...
    void EncodeOneFrame(SenderEncodedFrame* frame)
    {
        ...
        const scoped_refptr<VideoFrame> video_frame
            = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, frame_size, gfx::Rect(frame_size), frame_size, next_frame_timestamp_);
...
private:
    ...
    {
        vp8_encoder_ = std::make_unique<VpxEncoder>(video_config_, std::make_unique<media::MockVideoEncoderMetricsProvider>());
...
TEST_F(VpxQuantizerParserTest, InsufficientData)
{
    for (int i = 0; i < 3; ++i) {
        ...
        const unsigned int first_partition_size = (frame->data[0] | (frame->data[1] << 8) | (frame->data[2] << 16)) >> 5;
        if (frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) {
            ...
            EXPECT_EQ(-1, ParseVpxHeaderQuantizer(frame->data.first(10 + first_partition_size - 1)));

            // Minimum number of bytes to decode the quantizer value.
            EXPECT_EQ(kQp, ParseVpxHeaderQuantizer(frame->data.first(10 + first_partition_size)));
        } else {
            // One byte less than needed to decode the quantizer value.
            EXPECT_EQ(-1, ParseVpxHeaderQuantizer(frame->data.first(3 + first_partition_size - 1)));

            // Minimum number of bytes to decode the quantizer value.
            EXPECT_EQ(kQp, ParseVpxHeaderQuantizer(frame->data.first(3 + first_partition_size)));
        }
    }
}
