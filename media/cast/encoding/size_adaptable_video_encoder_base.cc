...
SizeAdaptableVideoEncoderBase::SizeAdaptableVideoEncoderBase(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config,
...
bool SizeAdaptableVideoEncoderBase::EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time)
{
    ...
    const bool is_frame_accepted = encoder_->EncodeVideoFrame(std::move(video_frame), reference_time);
...
StatusChangeCallback SizeAdaptableVideoEncoderBase::CreateEncoderStatusChangeCallback()
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    return base::BindRepeating(&SizeAdaptableVideoEncoderBase::OnEncoderStatusChange, weak_factory_.GetWeakPtr());
}

VideoEncoder::FrameEncodedCallback SizeAdaptableVideoEncoderBase::CreateFrameEncodedCallback()
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    return base::BindRepeating(&SizeAdaptableVideoEncoderBase::OnEncodedVideoFrame, weak_factory_.GetWeakPtr());
}

void SizeAdaptableVideoEncoderBase::OnEncoderReplaced(VideoEncoder* replacement_encoder)
{
}
...
void SizeAdaptableVideoEncoderBase::TrySpawningReplacementEncoder(const gfx::Size& size_needed)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    VLOG(1) << "Creating replacement video encoder (for frame size change from " << frame_size_.ToString() << " to " << size_needed.ToString() << ").";
    frame_size_ = size_needed;
    encoder_ = CreateEncoder();
    DCHECK(encoder_);
}

void SizeAdaptableVideoEncoderBase::OnEncoderStatusChange(OperationalStatus status)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
...
void SizeAdaptableVideoEncoderBase::OnEncodedVideoFrame(std::unique_ptr<SenderEncodedFrame> encoded_frame)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    --frames_in_encoder_;
