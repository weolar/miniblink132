...
struct InProgressExternalVideoFrameEncode {
    ...
    InProgressExternalVideoFrameEncode(scoped_refptr<VideoFrame> v_frame, base::TimeTicks r_time, int bit_rate)
...
class ExternalVideoEncoder::VEAClientImpl final : public VideoEncodeAccelerator::Client, public base::RefCountedThreadSafe<VEAClientImpl> {
public:
    using EncoderStatusChangeCallback = base::RepeatingCallback<void(media::EncoderStatus, OperationalStatus)>;
    VEAClientImpl(const scoped_refptr<CastEnvironment>& cast_environment, const scoped_refptr<base::SingleThreadTaskRunner>& encoder_task_runner,
        std::unique_ptr<media::VideoEncodeAccelerator> vea, double max_frame_rate, EncoderStatusChangeCallback status_change_cb, FrameEncodedCallback output_cb)
    ...
    void Initialize(const gfx::Size& frame_size, VideoCodecProfile codec_profile, int start_bit_rate, FrameId first_frame_id)
    {
        ...
        // uint32_t bitrates
        const media::Bitrate bitrate = media::Bitrate::ConstantBitrate(base::saturated_cast<uint32_t>(start_bit_rate));
        media::VideoEncodeAccelerator::Config config(media::PIXEL_FORMAT_I420, frame_size, codec_profile, bitrate, static_cast<uint32_t>(max_frame_rate_ + 0.5),
            media::VideoEncodeAccelerator::Config::StorageType::kShmem, media::VideoEncodeAccelerator::Config::ContentType::kDisplay);
        config.drop_frame_thresh_percentage = GetEncoderDropFrameThreshold();
        encoder_active_ = video_encode_accelerator_->Initialize(config, this, std::make_unique<media::NullMediaLog>());
        next_frame_id_ = first_frame_id;
        codec_profile_ = codec_profile;

        UMA_HISTOGRAM_BOOLEAN("Cast.Sender.VideoEncodeAcceleratorInitializeSuccess", encoder_active_);

        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE,
            base::BindOnce(status_change_cb_, encoder_active_ ? media::EncoderStatus::Codes::kOk : media::EncoderStatus::Codes::kEncoderFailedEncode,
    ...
    void SetBitRate(int bit_rate)
    {
        ...
        if (encoder_active_) {
            ...
            video_encode_accelerator_->RequestEncodingParametersChange(
                Bitrate::ConstantBitrate(base::saturated_cast<uint32_t>(bit_rate)), static_cast<uint32_t>(max_frame_rate_ + 0.5), std::nullopt);
        }
    }
    ...
    void EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, bool key_frame_requested)
    {
        TRACE_EVENT0("media", "ExternalVideoEncoder::EncodeVideoFrame");
        DCHECK(task_runner_->RunsTasksInCurrentSequence());

        in_progress_frame_encodes_.push_back(InProgressExternalVideoFrameEncode(video_frame, reference_time, requested_bit_rate_));
        ...
        if (free_input_buffer_index_.empty()) {
            if (!allocate_input_buffer_in_progress_ && input_buffers_.size() < max_allowed_input_buffers_) {
                ...
                const size_t buffer_size = media::VideoFrame::AllocationSize(media::PIXEL_FORMAT_I420, frame_coded_size_);
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&VEAClientImpl::AllocateInputBuffer, this, buffer_size));
            }
            AbortLatestEncodeAttemptDueToErrors();
            return;
        }
        ...
        if (video_frame->coded_size() != frame_coded_size_ || video_frame->storage_type() != media::VideoFrame::StorageType::STORAGE_SHMEM) {
            TRACE_EVENT1("media", "VideoFrame copy", "coded size", video_frame->coded_size().ToString());
            const int index = free_input_buffer_index_.back();
            auto& mapped_region = input_buffers_[index];
            DCHECK(mapped_region.IsValid());
            frame = VideoFrame::WrapExternalData(video_frame->format(), frame_coded_size_, video_frame->visible_rect(), video_frame->visible_rect().size(),
                static_cast<uint8_t*>(mapped_region.mapping.memory()), mapped_region.mapping.size(), video_frame->timestamp());
            ...
            frame->AddDestructionObserver(
                base::BindPostTaskToCurrentDefault(base::BindOnce(&ExternalVideoEncoder::VEAClientImpl::ReturnInputBufferToPool, this, index)));
...
protected:
    void NotifyErrorStatus(const media::EncoderStatus& status) final
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
        CHECK(!status.is_ok());
        LOG(ERROR) << "NotifyErrorStatus() is called, code=" << static_cast<int32_t>(status.code()) << ", message=" << status.message();

        encoder_active_ = false;

        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, status, STATUS_CODEC_RUNTIME_ERROR));
    ...
    void AllocateOutputBuffers(size_t size)
    {
        ...
        for (size_t i = 0; i < kOutputBufferCount; ++i) {
            ...
            output_buffers_.push_back(std::make_pair(std::move(memory), std::move(mapping)));

            video_encode_accelerator_->UseOutputBitstreamBuffer(
                media::BitstreamBuffer(static_cast<int32_t>(i), output_buffers_[i].first.Duplicate(), output_buffers_[i].first.GetSize()));
        }
    }

    // Called by the VEA to indicate its buffer requirements.
    void RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size) final
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());

        frame_coded_size_ = input_coded_size;
        max_allowed_input_buffers_ = input_count + kExtraInputBufferCount;
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&VEAClientImpl::AllocateOutputBuffers, this, output_buffer_size));
    }
    ...
    void BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata) final
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
        if (bitstream_buffer_id < 0 || bitstream_buffer_id >= static_cast<int32_t>(output_buffers_.size())) {
            NotifyErrorStatus(
                { media::EncoderStatus::Codes::kInvalidOutputBuffer, "invalid bitstream_buffer_id=" + base::NumberToString(bitstream_buffer_id) });
            return;
        }
        ...
        if (metadata.dropped_frame()) {
            ...
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(output_cb_, nullptr));
        ...
        const char* output_buffer_memory = output_buffers_[bitstream_buffer_id].second.GetMemoryAsSpan<char>(metadata.payload_size_bytes).data();
        if (metadata.payload_size_bytes > output_buffers_[bitstream_buffer_id].second.size()) {
            NotifyErrorStatus(
                { media::EncoderStatus::Codes::kInvalidOutputBuffer, "invalid payload_size=" + base::NumberToString(metadata.payload_size_bytes) });
            return;
        }
        ...
        if (!key_frame_encountered_) {
            ...
            InProgressExternalVideoFrameEncode& request = in_progress_frame_encodes_.front();

            auto encoded_frame = std::make_unique<SenderEncodedFrame>();
            encoded_frame->dependency
                = metadata.key_frame ? openscreen::cast::EncodedFrame::Dependency::kKeyFrame : openscreen::cast::EncodedFrame::Dependency::kDependent;
            ...
            encoded_frame->rtp_timestamp = ToRtpTimeTicks(request.video_frame->timestamp(), kVideoFrequency);
            encoded_frame->reference_time = request.reference_time;

            encoded_frame->capture_begin_time = request.video_frame->metadata().capture_begin_time;
            encoded_frame->capture_end_time = request.video_frame->metadata().capture_end_time;
            ...
            encoded_frame->data = base::HeapArray<uint8_t>::CopiedFrom(base::span(reinterpret_cast<const uint8_t*>(data.c_str()), data.size()));

            // If FRAME_DURATION metadata was provided in the source VideoFrame,
            // compute the utilization metrics.
            base::TimeDelta frame_duration = request.video_frame->metadata().frame_duration.value_or(base::TimeDelta());
            if (frame_duration.is_positive()) {
                ...
                const double actual_bitrate = encoded_frame->data.size() * 8.0 / frame_duration.InSecondsF();
                encoded_frame->encoder_bitrate = actual_bitrate;
                DCHECK_GT(request.target_bit_rate, 0);
                const double bitrate_utilization = actual_bitrate / request.target_bit_rate;
                ...
                if (metadata.key_frame || key_frame_quantizer_parsable_) {
                    ...
                    if (quantizer < 0) {
                        LOG(ERROR) << "Unable to parse quantizer from encoded " << (metadata.key_frame ? "key" : "delta")
                                   << " frame, id=" << encoded_frame->frame_id;
                        if (metadata.key_frame) {
                            key_frame_quantizer_parsable_ = false;
                            quantizer = quantizer_estimator_.EstimateForKeyFrame(*request.video_frame);
                        }
                    } else {
                        if (metadata.key_frame) {
                ...
                if (quantizer >= 0) {
                    const double max_quantizer
                        = IsVpxProfile(codec_profile_) ? static_cast<int>(QuantizerEstimator::MAX_VPX_QUANTIZER) : static_cast<int>(kMaxH264Quantizer);
                    encoded_frame->lossiness = bitrate_utilization * (quantizer / max_quantizer);
                }
            } else {
            ...
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(output_cb_, std::move(encoded_frame)));
...
private:
    ...
    void AbortLatestEncodeAttemptDueToErrors()
    {
        ...
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(output_cb_, std::move(no_result)));
    ...
    double GetH264FrameQuantizer(base::span<uint8_t> encoded_data)
    {
        ...
        while (true) {
            ...
            case H264NALU::kNonIDRSlice: {
                ...
                ++num_slices;
                int slice_quantizer = 26
                    + ((slice_header.IsSPSlice() || slice_header.IsSISlice()) ? pps->pic_init_qs_minus26 + slice_header.slice_qs_delta
    ...
    std::vector<std::pair<base::UnsafeSharedMemoryRegion, base::WritableSharedMemoryMapping>> output_buffers_;
...
ExternalVideoEncoder::ExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config,
    VideoEncoderMetricsProvider& metrics_provider, const gfx::Size& frame_size, FrameId first_frame_id, StatusChangeCallback status_change_cb,
    FrameEncodedCallback output_cb, const CreateVideoEncodeAcceleratorCallback& create_vea_cb)
    : cast_environment_(cast_environment)
    ...
    , bit_rate_(video_config.start_bitrate)
{
    ...
    CHECK(output_cb_);
    create_vea_cb.Run(base::BindOnce(
        &ExternalVideoEncoder::OnCreateVideoEncodeAccelerator, weak_factory_.GetWeakPtr(), video_config, first_frame_id, std::move(status_change_cb)));
}
...
void ExternalVideoEncoder::DestroyClientSoon()
{
    ...
    if (client_) {
        client_->task_runner()->PostTask(FROM_HERE, base::DoNothingWithBoundArgs(std::move(client_)));
    }
}

void ExternalVideoEncoder::SetErrorToMetricsProvider(const media::EncoderStatus& encoder_status)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    metrics_provider_->SetError(encoder_status);
}

bool ExternalVideoEncoder::EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time)
{
    DCHECK(cast_environment_->CurrentlyOn(CastEnvironment::MAIN));
    ...
    client_->task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&VEAClientImpl::EncodeVideoFrame, client_, std::move(video_frame), reference_time, key_frame_requested_));
...
void ExternalVideoEncoder::SetBitRate(int new_bit_rate)
{
    ...
    client_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&VEAClientImpl::SetBitRate, client_, bit_rate_));
}
...
void ExternalVideoEncoder::OnCreateVideoEncodeAccelerator(const FrameSenderConfig& video_config, FrameId first_frame_id,
    const StatusChangeCallback& status_change_cb, scoped_refptr<base::SingleThreadTaskRunner> encoder_task_runner,
    std::unique_ptr<media::VideoEncodeAccelerator> vea)
{
    ...
    if (!encoder_task_runner || !vea) {
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb, STATUS_CODEC_INIT_FAILED));
        return;
    }
    ...
    default:
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb, STATUS_UNSUPPORTED_CODEC));
        return;
    }
    ...
    VEAClientImpl::EncoderStatusChangeCallback wrapped_status_change_cb = base::BindRepeating(
        [](base::WeakPtr<ExternalVideoEncoder> self, const StatusChangeCallback& status_change_cb, media::EncoderStatus encoder_status,
    ...
    DCHECK(!client_);
    client_ = new VEAClientImpl(
        cast_environment_, encoder_task_runner, std::move(vea), video_config.max_frame_rate, std::move(wrapped_status_change_cb), output_cb_);
    metrics_provider_->Initialize(codec_profile, frame_size_,
        /*is_hardware_encoder=*/true);
    client_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&VEAClientImpl::Initialize, client_, frame_size_, codec_profile, bit_rate_, first_frame_id));
}

SizeAdaptableExternalVideoEncoder::SizeAdaptableExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment,
    const FrameSenderConfig& video_config, std::unique_ptr<VideoEncoderMetricsProvider> metrics_provider, StatusChangeCallback status_change_cb,
    FrameEncodedCallback output_cb, const CreateVideoEncodeAcceleratorCallback& create_vea_cb)
    : SizeAdaptableVideoEncoderBase(cast_environment, video_config, std::move(metrics_provider), std::move(status_change_cb), std::move(output_cb))
    , create_vea_cb_(create_vea_cb)
{
}

SizeAdaptableExternalVideoEncoder::~SizeAdaptableExternalVideoEncoder() = default;

std::unique_ptr<VideoEncoder> SizeAdaptableExternalVideoEncoder::CreateEncoder()
{
    return std::make_unique<ExternalVideoEncoder>(cast_environment(), video_config(), metrics_provider(), frame_size(), next_frame_id(),
        CreateEncoderStatusChangeCallback(), CreateFrameEncodedCallback(), create_vea_cb_);
}
...
double QuantizerEstimator::EstimateForKeyFrame(const VideoFrame& frame)
{
    ...
    const int rows_in_subset = std::max(1, size.height() * kFrameSamplingPercentage / 100);
    if (last_frame_size_ != size || !last_frame_pixel_buffer_) {
        last_frame_pixel_buffer_ = std::make_unique<uint8_t[]>(size.width() * rows_in_subset);
    ...
    for (int i = 0; i < rows_in_subset; ++i, y += row_skip) {
        const uint8_t* const row_begin = frame.visible_data(VideoFrame::Plane::kY) + y * frame.stride(VideoFrame::Plane::kY);
        ...
        memcpy(last_frame_pixel_buffer_.get() + i * size.width(), row_begin, size.width());
    }
    ...
    return ToQuantizerEstimate(ComputeEntropyFromHistogram(histogram.data(), histogram.size(), num_samples));
}
...
double QuantizerEstimator::EstimateForDeltaFrame(const VideoFrame& frame)
{
    ...
    for (int i = 0; i < rows_in_subset; ++i, y += row_skip) {
        ...
        for (const uint8_t *p = row_begin, *q = last_frame_row_begin; p < row_end; ++p, ++q) {
...
{
    DCHECK_EQ(8, VideoFrame::PlaneHorizontalBitsPerPixel(frame.format(), VideoFrame::Plane::kY));
    return media::IsYuvPlanar(frame.format()) && !frame.visible_rect().IsEmpty();
}

// static
double QuantizerEstimator::ComputeEntropyFromHistogram(const int* histogram, size_t histogram_size, int num_samples)
{
...
double QuantizerEstimator::ToQuantizerEstimate(double shannon_entropy)
{
    ...
    constexpr double kSlope = (MAX_VPX_QUANTIZER - MIN_VPX_QUANTIZER) / kEntropyAtMaxQuantizer;
    const double quantizer = std::min<double>(MAX_VPX_QUANTIZER, MIN_VPX_QUANTIZER + kSlope * shannon_entropy);
    return quantizer;
}
