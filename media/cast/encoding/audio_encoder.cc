...
// details.
class AudioEncoder::ImplBase : public base::RefCountedThreadSafe<AudioEncoder::ImplBase> {
public:
    ImplBase(const scoped_refptr<CastEnvironment>& cast_environment, AudioCodec codec, int num_channels, int sampling_rate, int samples_per_frame, int bitrate,
        ...
        , frame_duration_(base::Seconds(static_cast<double>(samples_per_frame_) / sampling_rate))
        , buffer_fill_end_(0)
        ...
        , samples_dropped_from_buffer_(0)
    {
        ...
        if (num_channels_ <= 0 || samples_per_frame_ <= 0 || frame_duration_.is_zero()
    ...
    void EncodeAudio(std::unique_ptr<AudioBus> audio_bus, const base::TimeTicks recorded_time)
    {
        ...
        if (!frame_capture_time_.is_null()) {
            ...
            if (num_frames_missed > kUnderrunSkipThreshold) {
                ...
                DVLOG(1) << "Skipping RTP timestamp ahead to account for " << num_frames_missed * samples_per_frame_ << " samples' worth of underrun.";
                TRACE_EVENT_INSTANT2(
                    "cast.stream", "Audio Skip", TRACE_EVENT_SCOPE_THREAD, "frames missed", num_frames_missed, "samples dropped", samples_dropped_from_buffer_);
            }
        }
        ...
        while (src_pos < audio_bus->frames()) {
            ...
            const int num_samples_to_xfer = std::min(samples_per_frame_ - buffer_fill_end_, audio_bus->frames() - src_pos);
            DCHECK_EQ(audio_bus->channels(), num_channels_);
            TransferSamplesIntoBuffer(audio_bus.get(), src_pos, buffer_fill_end_, num_samples_to_xfer);
            ...
            audio_frame->dependency = openscreen::cast::EncodedFrame::Dependency::kKeyFrame;
            ...
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN2("cast.stream", "Audio Encode", TRACE_ID_LOCAL(audio_frame.get()), "frame_id", frame_id_.lower_32_bits(),
            ...
            if (!audio_frame->data.empty()) {
                ...
                audio_frame->encoder_utilization = (base::TimeTicks::Now() - start_time) / frame_duration_;
                audio_frame->encoder_bitrate = bitrate_;
                TRACE_EVENT_NESTABLE_ASYNC_END1(
                    "cast.stream", "Audio Encode", TRACE_ID_LOCAL(audio_frame.get()), "encoder_utilization", audio_frame->encoder_utilization);

                audio_frame->encode_completion_time = cast_environment_->Clock()->NowTicks();
                cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(callback_, std::move(audio_frame), samples_dropped_from_buffer_));
                samples_dropped_from_buffer_ = 0;
            }
...
protected:
    ...
    virtual void TransferSamplesIntoBuffer(const AudioBus* audio_bus, int source_offset, int buffer_fill_offset, int num_samples) = 0;
...
public:
    OpusImpl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, FrameEncodedCallback callback)
        : ImplBase(cast_environment, AudioCodec::kOpus, num_channels, sampling_rate, sampling_rate / kDefaultFramesPerSecond, /* 10 ms frames */
            bitrate, std::move(callback))
        ...
        , buffer_(new float[num_channels * samples_per_frame_])
    {
        ...
        if (opus_encoder_init(opus_encoder_, sampling_rate, num_channels, OPUS_APPLICATION_AUDIO) != OPUS_OK) {
        ...
        CHECK_EQ(opus_encoder_ctl(opus_encoder_.get(), OPUS_SET_BITRATE(bitrate)), OPUS_OK);
    }
    ...
    {
        int bitrate = 0;
        CHECK_EQ(opus_encoder_ctl(opus_encoder_.get(), OPUS_GET_BITRATE(&bitrate)), OPUS_OK);
        return bitrate;
    }
...
private:
    ...
    void TransferSamplesIntoBuffer(const AudioBus* audio_bus, int source_offset, int buffer_fill_offset, int num_samples) final
    {
        DCHECK_EQ(audio_bus->channels(), num_channels_);
        float* dest = buffer_.get() + (buffer_fill_offset * num_channels_);
        audio_bus->ToInterleavedPartial<Float32SampleTypeTraits>(source_offset, num_samples, dest);
    }

    base::HeapArray<uint8_t> EncodeFromFilledBuffer() final
    {
        auto out = base::HeapArray<uint8_t>::Uninit(kOpusMaxPayloadSize);
        const opus_int32 result = opus_encode_float(opus_encoder_, buffer_.get(), samples_per_frame_, out.data(), kOpusMaxPayloadSize);
        if (result < 0) {
    ...
    static bool IsValidFrameDuration(base::TimeDelta duration)
    {
        ...
        return duration == base::Microseconds(2500) || duration == base::Milliseconds(5) || duration == base::Milliseconds(10)
            || duration == base::Milliseconds(20) || duration == base::Milliseconds(40) || duration == base::Milliseconds(60);
    }
...
public:
    AppleAacImpl(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, FrameEncodedCallback callback)
        : ImplBase(cast_environment, AudioCodec::kAAC, num_channels, sampling_rate, kAccessUnitSamples, bitrate, std::move(callback))
...
private:
    ...
    bool Initialize(int sampling_rate, int bitrate)
    {
        ...
        in_asbd.mFormatFlags = AudioFormatFlags { kAudioFormatFlagsNativeFloatPacked } | kAudioFormatFlagIsNonInterleaved;
        ...
        if (AudioFormatGetProperty(kAudioFormatProperty_FormatInfo, 0, nullptr, &prop_size, &out_asbd) != noErr) {
            return false;
        }
        ...
        if (AudioConverterGetProperty(converter_, kAudioConverterCurrentOutputStreamDescription, &prop_size, &out_asbd) != noErr) {
            return false;
        }
        ...
        if (bitrate > 0) {
            ...
            if (AudioConverterSetProperty(converter_, kAudioConverterEncodeBitRate, prop_size, &bitrate) != noErr) {
                return false;
            }
        }
        ...
        if (max_access_unit_size == 0) {
            ...
            if (AudioConverterGetProperty(converter_, kAudioConverterPropertyMaximumOutputPacketSize, &prop_size, &max_access_unit_size) != noErr) {
                return false;
            }
        }
        ...
        const_cast<std::unique_ptr<uint8_t[]>&>(access_unit_buffer_).reset(new uint8_t[max_access_unit_size]);
        ...
        UInt32 cookie_size;
        if (AudioConverterGetPropertyInfo(converter_, kAudioConverterCompressionMagicCookie, &cookie_size, nullptr) != noErr) {
            return false;
        }
        auto cookie_data = std::make_unique<uint8_t[]>(cookie_size);
        if (AudioConverterGetProperty(converter_, kAudioConverterCompressionMagicCookie, &cookie_size, cookie_data.get()) != noErr) {
            return false;
        }

        if (AudioFileInitializeWithCallbacks(
                this, &FileReadCallback, &FileWriteCallback, &FileGetSizeCallback, &FileSetSizeCallback, kAudioFileAAC_ADTSType, &out_asbd, 0, &file_)
            != noErr) {
            return false;
        }

        if (AudioFileSetProperty(file_, kAudioFilePropertyMagicCookieData, cookie_size, cookie_data.get()) != noErr) {
            return false;
        }
    ...
    void TransferSamplesIntoBuffer(const AudioBus* audio_bus, int source_offset, int buffer_fill_offset, int num_samples) final
    {
        ...
        audio_bus->CopyPartialFramesTo(source_offset, num_samples, buffer_fill_offset, input_buffer_.get());
    }
    ...
    base::HeapArray<uint8_t> EncodeFromFilledBuffer() final
    {
        ...
        oserr = AudioConverterFillComplexBuffer(converter_, &ConverterFillDataCallback, this, &io_num_packets, &converter_abl_, &packet_description);
        if (oserr != noErr || io_num_packets == 0) {
            return {};
        }

        // Reserve space in the output buffer to write the packet.a
        auto out = base::HeapArray<uint8_t>::Uninit(packet_description.mDataByteSize + kAdtsHeaderSize);
        ...
        oserr = AudioFileWritePackets(
            file_, false, converter_abl_.mBuffers[0].mDataByteSize, &packet_description, num_access_units_, &io_num_packets, converter_abl_.mBuffers[0].mData);
    ...
    static OSStatus ConverterFillDataCallback(
        AudioConverterRef in_converter, UInt32* io_num_packets, AudioBufferList* io_data, AudioStreamPacketDescription** out_packet_desc, void* in_encoder)
    {
        DCHECK(in_encoder);
        ...
        DCHECK_EQ(io_data->mNumberBuffers, static_cast<unsigned>(input_bus->channels()));
    ...
    static OSStatus FileReadCallback(void* in_encoder, SInt64 in_position, UInt32 in_size, void* in_buffer, UInt32* out_size)
    {
    ...
    static OSStatus FileWriteCallback(void* in_encoder, SInt64 in_position, UInt32 in_size, const void* in_buffer, UInt32* out_size)
    {
        CHECK(in_encoder);
        CHECK(in_buffer);
        auto* encoder = reinterpret_cast<AppleAacImpl*>(in_encoder);
        ...
        encoder->output_buffer_.copy_prefix_from(base::span(reinterpret_cast<const uint8_t*>(in_buffer), in_size));
...
AudioEncoder::AudioEncoder(const scoped_refptr<CastEnvironment>& cast_environment, int num_channels, int sampling_rate, int bitrate, AudioCodec codec,
    ...
    : cast_environment_(cast_environment)
{
    ...
    case AudioCodec::kOpus:
        impl_ = new OpusImpl(cast_environment, num_channels, sampling_rate, bitrate, std::move(frame_encoded_callback));
        break;
#if BUILDFLAG(IS_APPLE)
    case AudioCodec::kAAC:
        impl_ = new AppleAacImpl(cast_environment, num_channels, sampling_rate, bitrate, std::move(frame_encoded_callback));
        break;
...
void AudioEncoder::InsertAudio(std::unique_ptr<AudioBus> audio_bus, const base::TimeTicks recorded_time)
{
    DCHECK_CALLED_ON_VALID_THREAD(insert_thread_checker_);
    DCHECK(audio_bus.get());
    CHECK_EQ(InitializationResult(), STATUS_INITIALIZED);
    cast_environment_->PostTask(
        CastEnvironment::AUDIO, FROM_HERE, base::BindOnce(&AudioEncoder::ImplBase::EncodeAudio, impl_, std::move(audio_bus), recorded_time));
}
