...
public:
    ...
    ExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config,
    ...
    bool EncodeVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time) final;
...
private:
    ...
    void OnCreateVideoEncodeAccelerator(const FrameSenderConfig& video_config, FrameId first_frame_id, const StatusChangeCallback& status_change_cb,
        scoped_refptr<base::SingleThreadTaskRunner> encoder_task_runner, std::unique_ptr<media::VideoEncodeAccelerator> vea);
...
class SizeAdaptableExternalVideoEncoder final : public SizeAdaptableVideoEncoderBase {
public:
    SizeAdaptableExternalVideoEncoder(const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config,
    ...
    SizeAdaptableExternalVideoEncoder(const SizeAdaptableExternalVideoEncoder&) = delete;
    SizeAdaptableExternalVideoEncoder& operator=(const SizeAdaptableExternalVideoEncoder&) = delete;
...
private:
    ...
    // the given |histogram|.
    static double ComputeEntropyFromHistogram(const int* histogram, size_t histogram_size, int num_samples);
