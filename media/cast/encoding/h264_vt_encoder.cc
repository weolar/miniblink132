...
struct InProgressH264VTFrameEncode {
    ...
    InProgressH264VTFrameEncode(RtpTimeTicks rtp, base::TimeTicks r_time, VideoEncoder::FrameEncodedCallback callback)
        : rtp_timestamp(rtp)
...
class H264VideoToolboxEncoder::VideoFrameFactoryImpl final : public base::RefCountedThreadSafe<VideoFrameFactoryImpl>, public VideoFrameFactory {
public:
    // Type that proxies the VideoFrameFactory interface to this class.
    class Proxy;

    VideoFrameFactoryImpl(const base::WeakPtr<H264VideoToolboxEncoder>& encoder, const scoped_refptr<CastEnvironment>& cast_environment)
        : encoder_(encoder)
    ...
    scoped_refptr<VideoFrame> MaybeCreateFrame(const gfx::Size& frame_size, base::TimeDelta timestamp) override
    {
        ...
        if (frame_size != pool_frame_size_) {
            ...
            cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(&H264VideoToolboxEncoder::UpdateFrameSize, encoder_, frame_size));
        ...
        auto status = CVPixelBufferPoolCreatePixelBuffer(kCFAllocatorDefault, pool_, buffer.InitializeInto());
    ...
    void Update(const base::ScopedCFTypeRef<CVPixelBufferPoolRef>& pool, const gfx::Size& frame_size)
    {
        base::AutoLock auto_lock(lock_);
        pool_ = pool;
        pool_frame_size_ = frame_size;
...
class H264VideoToolboxEncoder::VideoFrameFactoryImpl::Proxy final : public VideoFrameFactory {
public:
    explicit Proxy(const scoped_refptr<VideoFrameFactoryImpl>& video_frame_factory)
...
// static
bool H264VideoToolboxEncoder::IsSupported(const FrameSenderConfig& video_config)
{
    return video_config.codec == CODEC_VIDEO_H264;
}

H264VideoToolboxEncoder::H264VideoToolboxEncoder(
    const scoped_refptr<CastEnvironment>& cast_environment, const FrameSenderConfig& video_config, StatusChangeCallback status_change_cb)
    : cast_environment_(cast_environment)
    , video_config_(video_config)
    , average_bitrate_((video_config_.min_bitrate + video_config_.max_bitrate) / 2)
    ...
    , weak_factory_(this)
{
    ...
    cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, operational_status));
    ...
    if (operational_status == STATUS_INITIALIZED) {
        ...
        video_frame_factory_ = scoped_refptr<VideoFrameFactoryImpl>(new VideoFrameFactoryImpl(weak_factory_.GetWeakPtr(), cast_environment_));
...
void H264VideoToolboxEncoder::ResetCompressionSession()
{
    ...
    cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, STATUS_CODEC_REINIT_PENDING));
...
#if !BUILDFLAG(IS_IOS)
    encoder_spec = video_toolbox::DictionaryWithKeyValue(kVTVideoEncoderSpecification_EnableHardwareAcceleratedVideoEncoder, kCFBooleanTrue);
#endif
    ...
    CFTypeRef attachments_keys[] = { kCVImageBufferColorPrimariesKey, kCVImageBufferTransferFunctionKey, kCVImageBufferYCbCrMatrixKey };
    ...
    CFTypeRef buffer_attributes_keys[] = { kCVPixelBufferPixelFormatTypeKey, kCVBufferPropagatedAttachmentsKey };
    CFTypeRef buffer_attributes_values[] = { video_toolbox::ArrayWithIntegers(format, std::size(format)).release(),
        video_toolbox::DictionaryWithKeysAndValues(attachments_keys, attachments_values, std::size(attachments_keys)).release() };
    const base::ScopedCFTypeRef<CFDictionaryRef> buffer_attributes
        = video_toolbox::DictionaryWithKeysAndValues(buffer_attributes_keys, buffer_attributes_values, std::size(buffer_attributes_keys));
    ...
    OSStatus status = VTCompressionSessionCreate(kCFAllocatorDefault, frame_size_.width(), frame_size_.height(), kCMVideoCodecType_H264, encoder_spec,
        buffer_attributes, nullptr /* compressedDataAllocator */, &H264VideoToolboxEncoder::CompressionCallback, reinterpret_cast<void*>(this),
        compression_session_.InitializeInto());
    if (status != noErr) {
        DLOG(ERROR) << " VTCompressionSessionCreate failed: " << status;
        // Notify that reinitialization has failed.
        cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, STATUS_CODEC_INIT_FAILED));
        return;
    }
    ...
    base::ScopedCFTypeRef<CVPixelBufferPoolRef> pool(VTCompressionSessionGetPixelBufferPool(compression_session_), base::scoped_policy::RETAIN);
    video_frame_factory_->Update(pool, frame_size_);

    // Notify that reinitialization is done.
    cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(status_change_cb_, STATUS_INITIALIZED));
}

void H264VideoToolboxEncoder::ConfigureCompressionSession()
{
    video_toolbox::SessionPropertySetter session_property_setter(compression_session_);
    session_property_setter.Set(kVTCompressionPropertyKey_ProfileLevel, kVTProfileLevel_H264_Main_AutoLevel);
    session_property_setter.Set(kVTCompressionPropertyKey_RealTime, true);
    session_property_setter.Set(kVTCompressionPropertyKey_AllowFrameReordering, false);
    session_property_setter.Set(kVTCompressionPropertyKey_MaxKeyFrameInterval, 240);
    session_property_setter.Set(kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, 240);
    session_property_setter.Set(kVTCompressionPropertyKey_AverageBitRate, average_bitrate_);
    session_property_setter.Set(kVTCompressionPropertyKey_ExpectedFrameRate, static_cast<int>(video_config_.max_frame_rate + 0.5));
    // Keep these attachment settings in-sync with those in Initialize().
    session_property_setter.Set(kVTCompressionPropertyKey_ColorPrimaries, kCVImageBufferColorPrimaries_ITU_R_709_2);
    session_property_setter.Set(kVTCompressionPropertyKey_TransferFunction, kCVImageBufferTransferFunction_ITU_R_709_2);
    session_property_setter.Set(kVTCompressionPropertyKey_YCbCrMatrix, kCVImageBufferYCbCrMatrix_ITU_R_709_2);
    if (video_config_.video_codec_params.max_number_of_video_buffers_used > 0) {
        session_property_setter.Set(kVTCompressionPropertyKey_MaxFrameDelayCount, video_config_.video_codec_params.max_number_of_video_buffers_used);
    }
}
...
void H264VideoToolboxEncoder::DestroyCompressionSession()
{
    ...
    if (compression_session_) {
        video_frame_factory_->Update(base::ScopedCFTypeRef<CVPixelBufferPoolRef>(nullptr), frame_size_);
...
bool H264VideoToolboxEncoder::EncodeVideoFrame(
    scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks reference_time, FrameEncodedCallback frame_encoded_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    std::unique_ptr<InProgressH264VTFrameEncode> request(
        new InProgressH264VTFrameEncode(ToRtpTimeTicks(video_frame->timestamp(), kVideoFrequency), reference_time, std::move(frame_encoded_callback)));
    ...
    OSStatus status = VTCompressionSessionEncodeFrame(
        compression_session_, pixel_buffer, timestamp_cm, CMTime { 0, 0, 0, 0 }, frame_props, reinterpret_cast<void*>(request.release()), nullptr);
    if (status != noErr) {
...
void H264VideoToolboxEncoder::UpdateFrameSize(const gfx::Size& size_needed)
{
    ...
    VLOG(1) << "Resetting compression session (for frame size change from " << frame_size_.ToString() << " to " << size_needed.ToString() << ").";
...
std::unique_ptr<VideoFrameFactory> H264VideoToolboxEncoder::CreateVideoFrameFactory()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return base::WrapUnique<VideoFrameFactory>(new VideoFrameFactoryImpl::Proxy(video_frame_factory_));
}
...
void H264VideoToolboxEncoder::EmitFrames()
{
    ...
    OSStatus status = VTCompressionSessionCompleteFrames(compression_session_, CMTime { 0, 0, 0, 0 });
    if (status != noErr) {
...
{
    VLOG(1) << "OnSuspend: Emitting all frames and destroying compression session.";
    EmitFrames();
    DestroyCompressionSession();
...
void H264VideoToolboxEncoder::CompressionCallback(void* encoder_opaque, void* request_opaque, OSStatus status, VTEncodeInfoFlags info, CMSampleBufferRef sbuf)
{
    auto* encoder = reinterpret_cast<H264VideoToolboxEncoder*>(encoder_opaque);
    std::unique_ptr<InProgressH264VTFrameEncode> request(reinterpret_cast<InProgressH264VTFrameEncode*>(request_opaque));
    bool is_keyframe = false;
    bool has_frame_data = false;

    if (status != noErr) {
        DLOG(ERROR) << " encoding failed: " << status;
        encoder->cast_environment_->PostTask(CastEnvironment::MAIN, FROM_HERE, base::BindOnce(encoder->status_change_cb_, STATUS_CODEC_RUNTIME_ERROR));
    } else if ((info & kVTEncodeInfo_FrameDropped)) {
        DVLOG(2) << " frame dropped";
    } else {
        auto* sample_attachments = static_cast<CFDictionaryRef>(CFArrayGetValueAtIndex(CMSampleBufferGetSampleAttachmentsArray(sbuf, true), 0));
        ...
        is_keyframe = !CFDictionaryContainsKey(sample_attachments, kCMSampleAttachmentKey_NotSync);
        has_frame_data = true;
    }
    ...
    if (has_frame_data) {
        video_toolbox::CopySampleBufferToAnnexBBuffer(sbuf, is_keyframe, &encoded_frame->data);
    }
    ...
    encoder->cast_environment_->GetTaskRunner(CastEnvironment::MAIN)
        ->PostTask(FROM_HERE, base::BindOnce(std::move(request->frame_encoded_callback), std::move(encoded_frame)));
}
