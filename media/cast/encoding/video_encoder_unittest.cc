...
class VideoEncoderTest : public ::testing::TestWithParam<std::pair<VideoCodec, bool>> {
public:
...
protected:
        ...
        , codec_params_(video_config_.video_codec_params.value())
    {
        ...
        feature_list_.InitWithFeatures(
            std::vector<base::test::FeatureRef> { kCastStreamingVp8, kCastStreamingVp9, kCastStreamingAv1 }, std::vector<base::test::FeatureRef> {});
    }
    ...
    void SetUp() final
    {
        ...
        if (is_testing_external_video_encoder()) {
            vea_factory_ = std::make_unique<FakeVideoEncodeAcceleratorFactory>(task_runner_);
        }
    }
    ...
    void CreateEncoder(VideoEncoder::FrameEncodedCallback output_cb)
    {
        ...
        video_encoder_ = VideoEncoder::Create(cast_environment_, video_config_, std::make_unique<media::MockVideoEncoderMetricsProvider>(),
            base::BindRepeating(&VideoEncoderTest::OnOperationalStatusChange, base::Unretained(this)), std::move(output_cb),
            base::BindRepeating(&FakeVideoEncodeAcceleratorFactory::CreateVideoEncodeAccelerator, base::Unretained(vea_factory_.get())));
        RunTasksAndAdvanceClock();
    ...
    {
        return codec_params_->codec == VideoCodec::kVP8 && !video_config_.use_hardware_encoder;
    }
    ...
    void RunTasksAndAdvanceClock()
    {
        ...
        const base::TimeDelta frame_duration = base::Microseconds(1000000.0 / video_config_.max_frame_rate);
    ...
    scoped_refptr<media::VideoFrame> CreateTestVideoFrame(const gfx::Size& size)
    {
        ...
        frame = media::VideoFrame::CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, timestamp);
...
private:
    ...
    {
        DVLOG(1) << "OnOperationalStatusChange: from " << operational_status_ << " to " << status;
    ...
    base::SingleThreadTaskRunner::CurrentHandleOverrideForTesting task_runner_current_handle_override_;
    ...
    OperationalStatus operational_status_ = OperationalStatus::STATUS_UNINITIALIZED;
...
TEST_P(VideoEncoderTest, EncodesVariedFrameSizes)
{
    ...
    base::WeakPtrFactory<EncodedFrames> encoded_frames_weak_factory(&encoded_frames);

    CreateEncoder(base::BindRepeating(
        [](base::WeakPtr<EncodedFrames> encoded_frames, std::unique_ptr<SenderEncodedFrame> encoded_frame) {
            if (encoded_frames) {
    ...
    for (const auto& frame_size : frame_sizes) {
        ...
        while (encoded_frames.size() <= 4
            || !(encoded_frames[encoded_frames.size() - 1] && encoded_frames[encoded_frames.size() - 2] && encoded_frames[encoded_frames.size() - 3]
                && encoded_frames[encoded_frames.size() - 4])) {
            const auto reference_time = Now();
            auto video_frame = CreateTestVideoFrame(frame_size);
            expectations.emplace(reference_time,

                ToRtpTimeTicks(video_frame->timestamp(), kVideoFrequency));

            const bool accepted_request = video_encoder()->EncodeVideoFrame(std::move(video_frame), reference_time);
    ...
    for (const std::unique_ptr<SenderEncodedFrame>& encoded_frame : encoded_frames) {
        if (!encoded_frame) {
            continue;
        }
        ...
        if (encoded_frame->dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame) {
            ...
            EXPECT_EQ(openscreen::cast::EncodedFrame::Dependency::kDependent, encoded_frame->dependency);
...
{
    CreateEncoder(base::BindRepeating([](std::unique_ptr<SenderEncodedFrame> encoded_frame) {}));

    // Send a frame to spawn creation of the ExternalVideoEncoder instance.
    video_encoder()->EncodeVideoFrame(CreateTestVideoFrame(gfx::Size(128, 72)), Now());
...
INSTANTIATE_TEST_SUITE_P(
    All, VideoEncoderTest, ::testing::ValuesIn(DetermineEncodersToTest()), [](const testing::TestParamInfo<VideoEncoderTest::ParamType>& info) {
        return base::ToUpperASCII(GetCodecName(info.param.first)) + (info.param.second ? "_Hardware" : "_Software");
    });
