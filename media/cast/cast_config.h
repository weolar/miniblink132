...
struct FrameSenderConfig {
    FrameSenderConfig();
    FrameSenderConfig(uint32_t sender_ssrc, uint32_t receiver_ssrc, base::TimeDelta min_playout_delay, base::TimeDelta max_playout_delay,
        RtpPayloadType rtp_payload_type, bool use_hardware_encoder, int rtp_timebase, int channels, int max_bitrate, int min_bitrate, int start_bitrate,
        double max_frame_rate, std::string aes_key, std::string aes_iv_mask, std::optional<VideoCodecParams> video_codec_params,
    ...
    bool is_audio() const
    {
        return rtp_payload_type >= RtpPayloadType::FIRST && rtp_payload_type <= RtpPayloadType::AUDIO_LAST;
    }
    bool is_video() const
    {
        return rtp_payload_type >= RtpPayloadType::VIDEO_VP8 && rtp_payload_type <= RtpPayloadType::LAST;
    }
    bool is_remoting() const
    {
        return rtp_payload_type == RtpPayloadType::REMOTE_AUDIO || rtp_payload_type == RtpPayloadType::REMOTE_VIDEO;
    }
    ...
    {
        return video_codec_params->codec;
    }
    ...
    {
        return audio_codec_params->codec;
    }
};

typedef base::OnceCallback<void(scoped_refptr<base::SingleThreadTaskRunner>, std::unique_ptr<media::VideoEncodeAccelerator>)>
    ReceiveVideoEncodeAcceleratorCallback;
typedef base::RepeatingCallback<void(ReceiveVideoEncodeAcceleratorCallback)> CreateVideoEncodeAcceleratorCallback;
typedef base::OnceCallback<void(base::UnsafeSharedMemoryRegion)> ReceiveVideoEncodeMemoryCallback;
