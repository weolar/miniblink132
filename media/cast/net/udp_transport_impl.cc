...
int32_t GetTransportSendBufferSize(const base::Value::Dict& options)
{
    ...
    // size.
    int32_t max_burst_size = LookupOptionWithDefault(options, kOptionPacerMaxBurstSize, media::cast::kMaxBurstSize) * media::cast::kMaxIpPacketSize;
...
UdpTransportImpl::UdpTransportImpl(const scoped_refptr<base::SingleThreadTaskRunner>& io_thread_proxy, const net::IPEndPoint& local_end_point,
    const net::IPEndPoint& remote_end_point, CastTransportStatusCallback status_callback)
    : io_thread_proxy_(io_thread_proxy)
    , local_addr_(local_end_point)
    , remote_addr_(remote_end_point)
    , udp_socket_(new net::UDPSocket(net::DatagramSocket::DEFAULT_BIND, nullptr /* net_log */, net::NetLogSource()))
    , send_pending_(false)
...
void UdpTransportImpl::StartReceiving(PacketReceiverCallbackWithStatus packet_receiver)
{
...
void UdpTransportImpl::StartReceiving(UdpTransportReceiver* receiver)
{
    ...
    StartReceiving(base::BindRepeating(&UdpTransportImpl::OnPacketReceived, base::Unretained(this)));
}
...
void UdpTransportImpl::ScheduleReceiveNextPacket()
{
    ...
    if (!packet_receiver_.is_null() && !receive_pending_) {
        ...
        io_thread_proxy_->PostTask(FROM_HERE, base::BindOnce(&UdpTransportImpl::ReceiveNextPacket, weak_factory_.GetWeakPtr(), net::ERR_IO_PENDING));
    }
}
...
void UdpTransportImpl::ReceiveNextPacket(int length_or_status)
{
    ...
    while (true) {
        if (length_or_status == net::ERR_IO_PENDING) {
            ...
            recv_buf_ = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&next_packet_->front()));
            length_or_status = udp_socket_->RecvFrom(
                recv_buf_.get(), media::cast::kMaxIpPacketSize, &recv_addr_, base::BindOnce(&UdpTransportImpl::ReceiveNextPacket, weak_factory_.GetWeakPtr()));
            if (length_or_status == net::ERR_IO_PENDING) {
                receive_pending_ = true;
                return;
            }
        ...
        if (length_or_status < 0) {
            VLOG(1) << "Failed to receive packet: Status code is " << length_or_status;
            receive_pending_ = false;
            return;
        }
        ...
        if (IsEmpty(remote_addr_)) {
            ...
            VLOG(1) << "Setting remote address from first received packet: " << remote_addr_.ToString();
            ...
            VLOG(1) << "Ignoring packet received from an unrecognized address: " << recv_addr_.ToString() << ".";
        } else {
            next_packet_->resize(length_or_status);
...
bool UdpTransportImpl::SendPacket(PacketRef packet, base::OnceClosure cb)
{
    ...
    if (next_dscp_value_ != net::DSCP_NO_CHANGE) {
        ...
        if (result != net::OK) {
            VLOG(1) << "Unable to set DSCP: " << next_dscp_value_ << " to socket; Error: " << result;
        }
    ...
    auto buf = base::MakeRefCounted<net::WrappedIOBuffer>(reinterpret_cast<char*>(&packet->data.front()));

    int result;
    net::CompletionOnceCallback callback = base::BindOnce(&UdpTransportImpl::OnSent, weak_factory_.GetWeakPtr(), buf, packet, std::move(cb));
    if (client_connected_) {
        ...
        net::NetworkTrafficAnnotationTag traffic_annotation = net::DefineNetworkTrafficAnnotation("cast_udp_transport", R"(
        semantics {
        ...
        result = udp_socket_->Write(buf.get(), static_cast<int>(packet->data.size()), std::move(callback), traffic_annotation);
    } else if (!IsEmpty(remote_addr_)) {
        result = udp_socket_->SendTo(buf.get(), static_cast<int>(packet->data.size()), remote_addr_, std::move(callback));
    } else {
...
void UdpTransportImpl::OnSent(const scoped_refptr<net::IOBuffer>& buf, PacketRef packet, base::OnceClosure cb, int result)
{
    DCHECK(io_thread_proxy_->RunsTasksInCurrentSequence());
...
void UdpTransportImpl::StartSending(mojo::ScopedDataPipeConsumerHandle packet_pipe)
{
...
{
    reader_->Read(base::BindOnce(&UdpTransportImpl::OnPacketReadFromDataPipe, base::Unretained(this)));
}

void UdpTransportImpl::OnPacketReadFromDataPipe(std::unique_ptr<Packet> packet)
{
    DVLOG(3) << __func__;
    // TODO(https://crbug.com/530834): Avoid making copy of the |packet|.
    if (!SendPacket(base::WrapRefCounted(new base::RefCountedData<Packet>(*packet)),
            base::BindRepeating(&UdpTransportImpl::ReadNextPacketToSend, base::Unretained(this)))) {
        return; // Waiting for the packet to be sent out.
    }
    // Force a post task to prevent the stack from growing too deep.
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&UdpTransportImpl::ReadNextPacketToSend, base::Unretained(this)));
}
