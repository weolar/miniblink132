...
public:
    UdpPacketPipeTest()
    {
        ...
        CHECK_EQ(mojo::CreateDataPipe(kDefaultDataPipeCapacityBytes, producer_handle, consumer_handle), MOJO_RESULT_OK);
...
TEST_F(UdpPacketPipeTest, Normal)
{
    ...
    writer_->Write(new base::RefCountedData<Packet>(packet1), done_callback.Get());
    ...
    writer_->Write(new base::RefCountedData<Packet>(packet2), done_callback2.Get());
    task_environment_.RunUntilIdle();
    ...
    reader_->Read(base::BindOnce(&UdpPacketPipeTest::OnPacketRead, base::Unretained(this)));
    task_environment_.RunUntilIdle();
    EXPECT_EQ(1u, packets_read_.size());
    EXPECT_EQ(0, std::memcmp(packet1.data(), packets_read_.front()->data(), packet1.size()));
    ...
    task_environment_.RunUntilIdle();
    EXPECT_EQ(1u, packets_read_.size());
    EXPECT_EQ(0, std::memcmp(packet2.data(), packets_read_.front()->data(), packet2.size()));
    packets_read_.pop_front();
    EXPECT_TRUE(packets_read_.empty());
}
