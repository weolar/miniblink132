...
public:
    ...
    {
        return base::BindRepeating(&MockPacketReceiver::ReceivedPacket, base::Unretained(this));
    }

    std::unique_ptr<Packet> TakePacket()
    {
...
{
    transport->SendPacket(new base::RefCountedData<Packet>(packet), base::OnceClosure());
}
...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::MainThreadType::IO)
    {
        ...
        send_transport_ = std::make_unique<UdpTransportImpl>(
            task_environment_.GetMainThreadTaskRunner(), free_local_port1, free_local_port2, base::BindRepeating(&UpdateCastTransportStatus));
        send_transport_->SetSendBufferSize(65536);

        recv_transport_ = std::make_unique<UdpTransportImpl>(
            task_environment_.GetMainThreadTaskRunner(), free_local_port2, free_local_port1, base::BindRepeating(&UpdateCastTransportStatus));
...
TEST_F(UdpTransportImplTest, PacketSenderSendAndReceive)
{
    ...
    MockPacketReceiver packet_receiver_on_receiver(base::BindRepeating(&SendPacket, recv_transport_.get(), packet));
    send_transport_->StartReceiving(packet_receiver_on_sender.packet_receiver());
    recv_transport_->StartReceiving(packet_receiver_on_receiver.packet_receiver());

    SendPacket(send_transport_.get(), packet);
    run_loop.Run();
    std::unique_ptr<Packet> received_packet = packet_receiver_on_sender.TakePacket();
    EXPECT_TRUE(received_packet);
    EXPECT_TRUE(std::equal(packet.begin(), packet.end(), received_packet->begin()));
...
TEST_F(UdpTransportImplTest, UdpTransportSendAndReceive)
{
    ...
    ASSERT_EQ(mojo::CreateDataPipe(5, producer_handle, consumer_handle), MOJO_RESULT_OK);
