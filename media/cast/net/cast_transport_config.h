...
using PacketReceiverCallback = base::RepeatingCallback<void(std::unique_ptr<Packet> packet)>;
using PacketReceiverCallbackWithStatus = base::RepeatingCallback<bool(std::unique_ptr<Packet> packet)>;
...
public:
    ...
    virtual void StartReceiving(PacketReceiverCallbackWithStatus packet_receiver) = 0;
...
{
    return lhs.ntp_seconds == rhs.ntp_seconds && lhs.ntp_fraction == rhs.ntp_fraction && lhs.rtp_timestamp == rhs.rtp_timestamp
        && lhs.send_packet_count == rhs.send_packet_count && lhs.send_octet_count == rhs.send_octet_count;
}
