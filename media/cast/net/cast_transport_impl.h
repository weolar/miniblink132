...
public:
    CastTransportImpl(const base::TickClock* clock, // Owned by the caller.
    ...
    void AddRtpReceiverReport(const RtcpReportBlock& rtp_receiver_report_block) final;
...
private:
    ...
    void ResendPackets(uint32_t ssrc, const MissingFramesAndPacketsMap& missing_packets, bool cancel_rtx_if_not_in_list, const DedupInfo& dedup_info);
    ...
    void OnReceivedLogMessage(EventMediaType media_type, const RtcpReceiverLogMessage& log);

    // Called when a RTCP Cast message is received.
    void OnReceivedCastMessage(uint32_t ssrc, const RtcpCastMessage& cast_message);
