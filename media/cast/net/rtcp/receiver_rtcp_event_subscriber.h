...
public:
    ...
    // events.
    ReceiverRtcpEventSubscriber(const size_t max_size_to_retain, EventMediaType type);

    ReceiverRtcpEventSubscriber(const ReceiverRtcpEventSubscriber&) = delete;
    ReceiverRtcpEventSubscriber& operator=(const ReceiverRtcpEventSubscriber&) = delete;
...
private:
    ...
    // should be processed.
    bool ShouldProcessEvent(CastLoggingEvent event_type, EventMediaType media_type);
