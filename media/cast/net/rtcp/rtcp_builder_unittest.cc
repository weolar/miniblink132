...
protected:
    ...
    PacketRef BuildRtcpFromReceiver(const RtcpReportBlock* report_block, const RtcpReceiverReferenceTimeReport* rrtr, const RtcpCastMessage* cast_message,
        const RtcpPliMessage* pli_message, const ReceiverRtcpEventSubscriber::RtcpEvents* rtcp_events, base::TimeDelta target_delay)
    {
...
TEST_F(RtcpBuilderTest, RtcpReceiverReport)
{
    ...
    ExpectPacketEQ(p2.GetPacket(), BuildRtcpFromReceiver(&report_block, nullptr, nullptr, nullptr, nullptr, kDefaultDelay));
}
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithRrtr)
{
    ...
    ExpectPacketEQ(p.GetPacket(), BuildRtcpFromReceiver(&report_block, &rrtr, nullptr, nullptr, nullptr, kDefaultDelay));
}
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithCast)
{
    ...
    cast_message.missing_frames_and_packets[FrameId::first() + kFrameIdWithLostPackets] = missing_packets;

    ExpectPacketEQ(p.GetPacket(), BuildRtcpFromReceiver(&report_block, nullptr, &cast_message, nullptr, nullptr, kDefaultDelay));
}
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithRrtraAndCastMessage)
{
    ...
    ExpectPacketEQ(p.GetPacket(), BuildRtcpFromReceiver(&report_block, &rrtr, &cast_message, nullptr, nullptr, kDefaultDelay));
}
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithRrtrCastMessageAndLog)
{
    ...
    ExpectPacketEQ(p.GetPacket(), BuildRtcpFromReceiver(&report_block, &rrtr, &cast_message, nullptr, &rtcp_events, kDefaultDelay));
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithOversizedFrameLog)
{
    ...
    p.AddReceiverFrameLog((test_rtp_timestamp() + RtpTimeDelta::FromTicks(2345)).lower_32_bits(), num_events, kTimeBaseMs);
    for (int i = 0; i < num_events; i++) {
        p.AddReceiverEventLog(kLostPacketId1, PACKET_RECEIVED, static_cast<uint16_t>(kTimeDelayMs * i));
    }
    ...
    ExpectPacketEQ(p.GetPacket(), BuildRtcpFromReceiver(&report_block, nullptr, nullptr, nullptr, &rtcp_events, kDefaultDelay));
}
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithTooManyLogFrames)
{
    ...
    for (int i = 0; i < num_events; i++) {
        p.AddReceiverFrameLog((test_rtp_timestamp() + RtpTimeDelta::FromTicks(i)).lower_32_bits(), 1, kTimeBaseMs + i * kTimeDelayMs);
        p.AddReceiverEventLog(0, FRAME_ACK_SENT, 0);
    }
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportWithOldLogFrames)
{
    ...
    p.AddReceiverFrameLog(test_rtp_timestamp().lower_32_bits(), 10, kTimeBaseMs + kTimeBetweenEventsMs);
    for (int i = 0; i < 10; ++i) {
...
TEST_F(RtcpBuilderTest, RtcpReceiverReportRedundancy)
{
    ...
    for (size_t i = 0; i < packet_count; i++) {
        ...
        p.AddReceiverFrameLog(test_rtp_timestamp().lower_32_bits(), num_events, time_base_ms - (num_events - 1) * kResendDelay * kTimeBetweenEventsMs);
        for (int event = 0; event < num_events; event++) {
            p.AddReceiverEventLog(0, FRAME_ACK_SENT, base::checked_cast<uint16_t>(event * kResendDelay * kTimeBetweenEventsMs));
        }
...
TEST_F(RtcpBuilderTest, RtcpSenderReport)
{
    ...
    ExpectPacketEQ(p.GetPacket(), rtcp_builder_->BuildRtcpFromSender(sender_info));
}
