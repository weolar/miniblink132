...
protected:
    ...
    {
        return parser.has_sender_report() || parser.has_last_report() || parser.has_receiver_log() || parser.has_cast_message()
    ...
    void ExpectSenderInfo(const RtcpParser& parser)
    {
        ...
        EXPECT_EQ(kRtpTimestamp, parser.sender_report().rtp_timestamp.lower_32_bits());
    ...
    void ExpectCastFeedback(const RtcpParser& parser)
    {
        ...
        EXPECT_EQ(FrameId::first() + kAckFrameId, parser.cast_message().ack_frame_id);

        auto frame_it = parser.cast_message().missing_frames_and_packets.begin();

        EXPECT_TRUE(frame_it != parser.cast_message().missing_frames_and_packets.end());
    ...
    void ExpectExtendedCastFeedback(const RtcpParser& parser, const std::vector<FrameId>& receiving_frames)
    {
        EXPECT_TRUE(parser.has_cst2_message());
        EXPECT_EQ(kFeedbackSeq, parser.cast_message().feedback_count);
        ASSERT_EQ(parser.cast_message().received_later_frames.size(), receiving_frames.size());
        for (size_t i = 0; i < receiving_frames.size(); ++i)
            EXPECT_EQ(parser.cast_message().received_later_frames[i], receiving_frames[i]);
    }

    void ExpectReceiverLog(const RtcpParser& parser, const RtcpReceiverLogMessage& expected_receiver_log)
    {
        ...
        for (; incoming_it != parser.receiver_log().end(); ++incoming_it, ++expected_it) {
            EXPECT_EQ(expected_it->rtp_timestamp_, incoming_it->rtp_timestamp_);
            EXPECT_EQ(expected_it->event_log_messages_.size(), incoming_it->event_log_messages_.size());

            auto event_incoming_it = incoming_it->event_log_messages_.begin();
            auto event_expected_it = expected_it->event_log_messages_.begin();
            for (; event_incoming_it != incoming_it->event_log_messages_.end(); ++event_incoming_it, ++event_expected_it) {
                EXPECT_EQ(event_expected_it->type, event_incoming_it->type);
                EXPECT_EQ(event_expected_it->event_timestamp, event_incoming_it->event_timestamp);
                if (event_expected_it->type == PACKET_RECEIVED) {
                    EXPECT_EQ(event_expected_it->packet_id, event_incoming_it->packet_id);
                } else {
                    EXPECT_EQ(event_expected_it->delay_delta, event_incoming_it->delay_delta);
                }
            }
        }
    }
