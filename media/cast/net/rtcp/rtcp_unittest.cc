...
public:
    explicit FakeRtcpTransport(base::SimpleTestTickClock* clock)
        : clock_(clock)
        , packet_delay_(base::Milliseconds(42))
    {
    }
    ...
    base::TimeDelta packet_delay() const
    {
        return packet_delay_;
    }
    void set_packet_delay(base::TimeDelta delay)
    {
        packet_delay_ = delay;
    }
    ...
    bool ResendPackets(const SendPacketVector& packets, const DedupInfo& dedup_info) final
    {
        return false;
    }
...
protected:
    RtcpTest()
        ...
        , rtcp_at_rtp_sender_(sender_clock_.get(), &rtp_sender_pacer_, this, kSenderSsrc, kReceiverSsrc)
        , rtcp_at_rtp_receiver_(receiver_clock_.get(), kReceiverSsrc, kSenderSsrc)
        , received_pli_(false)
    {
    ...
    void OnReceivedReceiverLog(const RtcpReceiverLogMessage& logs) override
    {
        ...
        for (const RtcpReceiverFrameLogMessage& frame_log_msg : logs) {
            last_logs_.push_back(RtcpReceiverFrameLogMessage(frame_log_msg.rtp_timestamp_));
            for (const RtcpReceiverEventLogMessage& event_log_msg : frame_log_msg.event_log_messages_) {
    ...
    PacketRef BuildRtcpPacketFromRtpReceiver(const RtcpTimeData& time_data, const RtcpCastMessage* cast_message, const RtcpPliMessage* pli_message,
        base::TimeDelta target_delay, const ReceiverRtcpEventSubscriber::RtcpEvents* rtcp_events, const RtpReceiverStatistics* rtp_receiver_statistics)
    {
        ...
        if (rtp_receiver_statistics) {
            ...
            if (!last_report_received_time.is_null()) {
                ...
                ConvertTimeToFractions(delta.InMicroseconds(), &delay_seconds, &delay_fraction);
...
TEST_F(RtcpTest, LipSyncGleanedFromSenderReport)
{
    ...
    ASSERT_FALSE(rtcp_at_rtp_receiver_.GetLatestLipSyncTimes(&rtp_timestamp, &reference_time));
    ...
    rtcp_at_rtp_sender_.SendRtcpReport(reference_time_sent, rtp_timestamp_sent, 1, 1);

    // Now the receiver should have lip-sync info.  Confirm that the lip-sync
    // reference time is the same as that sent.
    EXPECT_TRUE(rtcp_at_rtp_receiver_.GetLatestLipSyncTimes(&rtp_timestamp, &reference_time));
...
TEST_F(RtcpTest, RoundTripTimesDeterminedFromReportPingPong)
{
    ...
    for (int i = 0; i < iterations; ++i) {
        const base::TimeDelta one_way_trip_time = base::Milliseconds(static_cast<int64_t>(1) << i);
        ...
        rtcp_at_rtp_sender_.SendRtcpReport(reference_time_sent, rtp_timestamp_sent, 1, 1);
        EXPECT_EQ(expected_rtt_according_to_sender, rtcp_at_rtp_sender_.current_round_trip_time());

        // Validate last reported callback value is same as that reported by method.
        EXPECT_EQ(current_round_trip_time_, rtcp_at_rtp_sender_.current_round_trip_time());

        // Receiver --> Sender
        RtpReceiverStatistics stats;
        rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
            BuildRtcpPacketFromRtpReceiver(CreateRtcpTimeData(receiver_clock_->NowTicks()), nullptr, nullptr, base::TimeDelta(), nullptr, &stats));
...
TEST_F(RtcpTest, ReportCastFeedback)
{
    ...
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(CreateRtcpTimeData(base::TimeTicks()), &cast_message, nullptr, base::Milliseconds(kTargetDelayMs), nullptr, nullptr));
    ...
    EXPECT_EQ(last_cast_message_.missing_frames_and_packets.size(), cast_message.missing_frames_and_packets.size());
    EXPECT_TRUE(std::equal(
        cast_message.missing_frames_and_packets.begin(), cast_message.missing_frames_and_packets.end(), last_cast_message_.missing_frames_and_packets.begin()));
}
...
TEST_F(RtcpTest, ReportPli)
{
    ...
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(CreateRtcpTimeData(base::TimeTicks()), nullptr, &pli_message, base::TimeDelta(), nullptr, nullptr));
...
TEST_F(RtcpTest, DropLateRtcpPacket)
{
    ...
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(
            CreateRtcpTimeData(receiver_clock_->NowTicks()), &cast_message, nullptr, base::Milliseconds(kTargetDelayMs), nullptr, nullptr));
    ...
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(
            CreateRtcpTimeData(receiver_clock_->NowTicks() - base::Seconds(10)), &late_cast_message, nullptr, base::TimeDelta(), nullptr, nullptr));
    ...
    late_cast_message.ack_frame_id = FrameId::first() + 2;
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(CreateRtcpTimeData(receiver_clock_->NowTicks()), &late_cast_message, nullptr, base::TimeDelta(), nullptr, nullptr));
...
TEST_F(RtcpTest, ReportReceiverEvents)
{
    ...
    rtp_receiver_pacer_.SendRtcpPacket(rtcp_at_rtp_receiver_.local_ssrc(),
        BuildRtcpPacketFromRtpReceiver(CreateRtcpTimeData(receiver_clock_->NowTicks()), nullptr, nullptr, base::TimeDelta(), &rtcp_events, nullptr));
