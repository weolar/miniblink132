...
ReceiverRtcpEventSubscriber::ReceiverRtcpEventSubscriber(const size_t max_size_to_retain, EventMediaType type)
    : max_size_to_retain_(max_size_to_retain * (kResendDelay * kNumResends + 1))
    , type_(type)
    , popped_events_(0)
{
...
void ReceiverRtcpEventSubscriber::OnReceiveFrameEvent(const FrameEvent& frame_event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    if (ShouldProcessEvent(frame_event.type, frame_event.media_type)) {
        ...
        case FRAME_DECODED:
            ...
            rtcp_events_.push_back(std::make_pair(frame_event.rtp_timestamp, rtcp_event));
            break;
        default:
            break;
        }
...
void ReceiverRtcpEventSubscriber::OnReceivePacketEvent(const PacketEvent& packet_event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    if (ShouldProcessEvent(packet_event.type, packet_event.media_type)) {
        ...
        if (packet_event.type == PACKET_RECEIVED) {
            ...
            rtcp_events_.push_back(std::make_pair(packet_event.rtp_timestamp, rtcp_event));
        }
    }
...
struct CompareByFirst {
    bool operator()(const std::pair<RtpTimeTicks, RtcpEvent>& a, const std::pair<RtpTimeTicks, RtcpEvent>& b)
    {
        return a.first < b.first;
    }
};

void ReceiverRtcpEventSubscriber::GetRtcpEventsWithRedundancy(RtcpEvents* rtcp_events)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(rtcp_events);
...
bool ReceiverRtcpEventSubscriber::ShouldProcessEvent(CastLoggingEvent event_type, EventMediaType event_media_type)
{
    return type_ == event_media_type
        && (event_type == FRAME_ACK_SENT || event_type == FRAME_DECODED || event_type == FRAME_PLAYOUT || event_type == PACKET_RECEIVED);
}
