...
uint16_t MergeEventTypeAndTimestampForWireFormat(const CastLoggingEvent& event, const base::TimeDelta& time_delta)
{
...
bool EventTimestampLessThan(const RtcpReceiverEventLogMessage& lhs, const RtcpReceiverEventLogMessage& rhs)
{
...
void RtcpBuilder::PatchLengthField()
{
    if (ptr_of_length_) {
        ...
        DCHECK_EQ(0, this_packet_length % 4) << "Packets must be a multiple of 32 bits long";
...
void RtcpBuilder::Start()
{
    ...
    writer_ = base::BigEndianWriter(reinterpret_cast<char*>(&(packet_->data[0])), kMaxIpPacketSize);
}
...
void RtcpBuilder::AddCast(const RtcpCastMessage& cast, base::TimeDelta target_delay)
{
    ...
    DCHECK_LE(target_delay.InMilliseconds(), std::numeric_limits<uint16_t>::max());
    writer_.WriteU16(target_delay.InMilliseconds());

    size_t number_of_loss_fields = 0;
    size_t max_number_of_loss_fields = std::min<size_t>(kRtcpMaxCastLossFields, writer_.remaining() / 4);

    auto frame_it = cast.missing_frames_and_packets.begin();

    NackStringBuilder nack_string_builder;
    for (; frame_it != cast.missing_frames_and_packets.end() && number_of_loss_fields < max_number_of_loss_fields; ++frame_it) {
    ...
    VLOG_IF(1, !nack_string_builder.Empty()) << "SSRC: " << cast.remote_ssrc << ", ACK: " << cast.ack_frame_id << ", NACK: " << nack_string_builder.GetString();
...
void RtcpBuilder::AddReceiverLog(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events)
{
    size_t total_number_of_messages_to_send = 0;
    RtcpReceiverLogMessage receiver_log_message;

    if (!GetRtcpReceiverLogMessage(rtcp_events, &receiver_log_message, &total_number_of_messages_to_send)) {
        return;
    }
    ...
    while (!receiver_log_message.empty() && total_number_of_messages_to_send > 0) {
        RtcpReceiverFrameLogMessage& frame_log_messages(receiver_log_message.front());
        ...
        while (!frame_log_messages.event_log_messages_.empty() && messages_in_frame > 0) {
            ...
            uint16_t event_type_and_timestamp_delta
                = MergeEventTypeAndTimestampForWireFormat(event_message.type, event_message.event_timestamp - event_timestamp_base);
            switch (event_message.type) {
            case FRAME_ACK_SENT:
            case FRAME_PLAYOUT:
            case FRAME_DECODED:
                writer_.WriteU16(static_cast<uint16_t>(event_message.delay_delta.InMilliseconds()));
...
bool RtcpBuilder::GetRtcpReceiverLogMessage(
    const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events, RtcpReceiverLogMessage* receiver_log_message, size_t* total_number_of_messages_to_send)
{
    ...
    while (rit != rtcp_events.rend() && remaining_space >= kRtcpReceiverFrameLogSize + kRtcpReceiverEventLogSize) {
        ...
        std::sort(sorted_log_messages.begin(), sorted_log_messages.end(), &EventTimestampLessThan);
