...
bool RtcpParser::ParseCommonHeader(base::BigEndianReader* reader, RtcpCommonHeader* parsed_header)
{
    //  0                   1                   2                   3
...
bool RtcpParser::ParseSR(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
    ...
    if (!reader->ReadU32(&sender_report_.ntp_seconds) || !reader->ReadU32(&sender_report_.ntp_fraction) || !reader->ReadU32(&truncated_rtp_timestamp)
        || !reader->ReadU32(&sender_report_.send_packet_count) || !reader->ReadU32(&send_octet_count))
        return false;
...
bool RtcpParser::ParseRR(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
...
bool RtcpParser::ParsePli(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
    if (header.IC != 1)
        return true;
...
bool RtcpParser::ParseApplicationDefined(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
    uint32_t sender_ssrc;
    uint32_t name;
...
bool RtcpParser::ParseCastReceiverLogFrameItem(base::BigEndianReader* reader)
{
    ...
    while (reader->remaining()) {
        ...
        base::TimeTicks event_timestamp_base = base::TimeTicks() + base::Milliseconds(data & 0xffffff);
        ...
        for (size_t event = 0; event < num_events; event++) {
            ...
            event_log.type = TranslateToLogEventFromWireFormat(static_cast<uint8_t>(event_type_and_timestamp_delta >> 12));
            event_log.event_timestamp = event_timestamp_base + base::Milliseconds(event_type_and_timestamp_delta & 0xfff);
            if (event_log.type == PACKET_RECEIVED) {
                event_log.packet_id = delay_delta_or_packet_id;
            } else {
                event_log.delay_delta = base::Milliseconds(static_cast<int16_t>(delay_delta_or_packet_id));
            }
...
// RFC 4585.
bool RtcpParser::ParseFeedbackCommon(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
...
bool RtcpParser::ParseExtendedReport(base::BigEndianReader* reader, const RtcpCommonHeader& header)
{
    uint32_t remote_ssrc;
    ...
    while (reader->remaining()) {
        ...
        case 4: // RRTR. RFC3611 Section 4.4.
            if (block_length != 2)
                return false;
            if (!ParseExtendedReportReceiverReferenceTimeReport(reader, remote_ssrc))
                return false;
            break;
...
bool RtcpParser::ParseExtendedReportReceiverReferenceTimeReport(base::BigEndianReader* reader, uint32_t remote_ssrc)
{
...
void ConvertTimeToFractions(int64_t ntp_time_us, uint32_t* seconds, uint32_t* fractions)
{
    ...
    DCHECK_LT(seconds_component, INT64_C(4263431296)) << "One year left to fix the NTP year 2036 wrap-around issue!";
...
void ConvertTimeTicksToNtp(const base::TimeTicks& time, uint32_t* ntp_seconds, uint32_t* ntp_fractions)
{
...
base::TimeTicks ConvertNtpToTimeTicks(uint32_t ntp_seconds, uint32_t ntp_fractions)
{
    ...
    base::TimeDelta elapsed_since_unix_epoch = base::Microseconds(ntp_time_us - (kUnixEpochInNtpSeconds * base::Time::kMicrosecondsPerSecond));
