...
std::pair<uint64_t, uint64_t> GetReceiverEventKey(
    RtpTimeTicks frame_rtp_timestamp, const base::TimeTicks& event_timestamp, uint8_t event_type, uint16_t packet_id_or_zero)
{
    ...
    return std::make_pair(value1, static_cast<uint64_t>(event_timestamp.ToInternalValue()));
}

} // namespace

SenderRtcpSession::SenderRtcpSession(
    const base::TickClock* clock, PacedPacketSender* packet_sender, RtcpObserver* observer, uint32_t local_ssrc, uint32_t remote_ssrc)
    : clock_(clock)
    , packet_sender_(packet_sender)
    , local_ssrc_(local_ssrc)
    , remote_ssrc_(remote_ssrc)
    , rtcp_observer_(observer)
    , largest_seen_timestamp_(base::TimeTicks::FromInternalValue(std::numeric_limits<int64_t>::min()))
...
bool SenderRtcpSession::IncomingRtcpPacket(const uint8_t* data, size_t length)
{
    ...
    if (parser_.Parse(&reader)) {
        ...
        if (parser_.has_receiver_reference_time_report()) {
            base::TimeTicks t
                = ConvertNtpToTimeTicks(parser_.receiver_reference_time_report().ntp_seconds, parser_.receiver_reference_time_report().ntp_fraction);
            if (t > largest_seen_timestamp_) {
                ...
                VLOG(1) << "Rejecting RTCP packet as it is too old (" << (largest_seen_timestamp_ - t).InMilliseconds() << " ms)";
                return true;
            }
        }
        ...
        if (parser_.has_last_report()) {
            OnReceivedDelaySinceLastReport(parser_.last_report(), parser_.delay_since_last_report());
        }
...
void SenderRtcpSession::OnReceivedDelaySinceLastReport(uint32_t last_report, uint32_t delay_since_last_report)
{
    ...
    current_round_trip_time_ = std::max(current_round_trip_time_, base::Milliseconds(1));

    rtcp_observer_->OnReceivedRtt(current_round_trip_time_);
}

void SenderRtcpSession::SaveLastSentNtpTime(const base::TimeTicks& now, uint32_t last_ntp_seconds, uint32_t last_ntp_fraction)
{
    ...
    const base::TimeTicks timeout = now - base::Milliseconds(kStatsHistoryWindowMs);
...
bool SenderRtcpSession::DedupeReceiverLog(RtcpReceiverLogMessage* receiver_log)
{
    ...
    while (i != receiver_log->end()) {
        ...
        while (j != messages->end()) {
            ReceiverEventKey key = GetReceiverEventKey(i->rtp_timestamp_, j->event_timestamp, j->type, j->packet_id);
            auto tmp = j;
            ++j;
...
void SenderRtcpSession::SendRtcpReport(
    base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp, uint32_t send_packet_count, size_t send_octet_count)
{
    uint32_t current_ntp_seconds = 0;
    uint32_t current_ntp_fractions = 0;
    ConvertTimeTicksToNtp(current_time, &current_ntp_seconds, &current_ntp_fractions);
    ...
    packet_sender_->SendRtcpPacket(local_ssrc_, rtcp_builder.BuildRtcpFromSender(sender_info));
}
