...
public:
    ...
    void SendRtcpReport(base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp, uint32_t send_packet_count, size_t send_octet_count);
...
private:
    ...
    // round trip time.
    void OnReceivedDelaySinceLastReport(uint32_t last_report, uint32_t delay_since_last_report);
    ...
    void SaveLastSentNtpTime(const base::TimeTicks& now, uint32_t last_ntp_seconds, uint32_t last_ntp_fraction);
    ...
    std::unordered_set<ReceiverEventKey, base::IntPairHash<ReceiverEventKey>> receiver_event_key_set_;
