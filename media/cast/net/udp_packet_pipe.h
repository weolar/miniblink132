...
public:
    explicit UdpPacketPipeReader(mojo::ScopedDataPipeConsumerHandle consumer_handle);
...
public:
    explicit UdpPacketPipeWriter(mojo::ScopedDataPipeProducerHandle producer_handle);
...
private:
    ...
    void WritePacketPayload(PacketRef packet, base::OnceClosure done_cb, bool success);

    // Called by |data_pipe_writer_| when the writing completes.
    void OnPacketWritten(PacketRef packet, base::OnceClosure done_cb, bool success);
