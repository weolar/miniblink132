...
protected:
    ...
    void InitializeVideo()
    {
        ...
        transport_sender_->InitializeStream(rtp_config, std::make_unique<StubRtcpObserver>());
    }
...
public:
    explicit TransportClient(CastTransportImplTest* cast_transport_sender_impl_test)
...
void CastTransportImplTest::InitWithoutLogging()
{
    ...
    transport_sender_ = std::make_unique<CastTransportImpl>(
        &testing_clock_, base::TimeDelta(), std::make_unique<TransportClient>(nullptr), base::WrapUnique(transport_.get()), task_runner_);
...
void CastTransportImplTest::InitWithLogging()
{
    transport_ = new FakePacketSender();
    transport_sender_ = std::make_unique<CastTransportImpl>(
        &testing_clock_, base::Milliseconds(10), std::make_unique<TransportClient>(this), base::WrapUnique(transport_.get()), task_runner_);
    task_runner_->RunTasks();
}
...
TEST_F(CastTransportImplTest, NacksCancelRetransmits)
{
    ...
    transport_sender_->ResendPackets(kVideoSsrc, missing_packets, true, dedup_info);
