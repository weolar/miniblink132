...
using CastTransportStatusCallback = base::RepeatingCallback<void(CastTransportStatus status)>;
...
public:
    ...
    public:
        ...
        virtual void OnLoggingEventsReceived(std::unique_ptr<std::vector<FrameEvent>> frame_events, std::unique_ptr<std::vector<PacketEvent>> packet_events)
            = 0;
    ...
    static std::unique_ptr<CastTransport> Create(const base::TickClock* clock, // Owned by the caller.
    ...
    // is called.
    virtual void InitializeStream(const CastTransportRtpConfig& config, std::unique_ptr<RtcpObserver> rtcp_observer)
    {
    }
    ...
    virtual void SendSenderReport(uint32_t ssrc, base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp) = 0;
    ...
    virtual void CancelSendingFrames(uint32_t ssrc, const std::vector<FrameId>& frame_ids) = 0;
    ...
    virtual void AddValidRtpReceiver(uint32_t rtp_sender_ssrc, uint32_t rtp_receiver_ssrc) = 0;
    ...
    virtual void InitializeRtpReceiverRtcpBuilder(uint32_t rtp_receiver_ssrc, const RtcpTimeData& time_data) = 0;

    virtual void AddCastFeedback(const RtcpCastMessage& cast_message, base::TimeDelta target_delay) = 0;
    virtual void AddPli(const RtcpPliMessage& pli_message) = 0;
    virtual void AddRtcpEvents(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events) = 0;
    virtual void AddRtpReceiverReport(const RtcpReportBlock& rtp_report_block) = 0;
