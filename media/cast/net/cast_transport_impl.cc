...
std::unique_ptr<CastTransport> CastTransport::Create(const base::TickClock* clock, // Owned by the caller.
    base::TimeDelta logging_flush_interval, std::unique_ptr<Client> client, std::unique_ptr<PacketTransport> transport,
    const scoped_refptr<base::SingleThreadTaskRunner>& transport_task_runner)
{
    return std::unique_ptr<CastTransport>(
        new CastTransportImpl(clock, logging_flush_interval, std::move(client), std::move(transport), transport_task_runner.get()));
}
...
public:
    RtcpClient(std::unique_ptr<RtcpObserver> observer, uint32_t rtp_sender_ssrc, EventMediaType media_type, CastTransportImpl* cast_transport_impl)
...
CastTransportImpl::CastTransportImpl(const base::TickClock* clock, base::TimeDelta logging_flush_interval, std::unique_ptr<Client> client,
    std::unique_ptr<PacketTransport> transport, const scoped_refptr<base::SingleThreadTaskRunner>& transport_task_runner)
    : clock_(clock)
    , logging_flush_interval_(logging_flush_interval)
    , transport_client_(std::move(client))
    , transport_(std::move(transport))
    , transport_task_runner_(transport_task_runner)
    , pacer_(kTargetBurstSize, kMaxBurstSize, clock, logging_flush_interval.is_positive() ? &recent_packet_events_ : nullptr, transport_.get(),
    ...
    , last_byte_acked_for_audio_(0)
{
    ...
    if (logging_flush_interval_.is_positive()) {
        transport_task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&CastTransportImpl::SendRawEvents, weak_factory_.GetWeakPtr()), logging_flush_interval_);
    }
    transport_->StartReceiving(base::BindRepeating(&CastTransportImpl::OnReceivedPacket, base::Unretained(this)));
}
...
void CastTransportImpl::InitializeStream(const CastTransportRtpConfig& config, std::unique_ptr<RtcpObserver> rtcp_observer)
{
    if (sessions_.find(config.ssrc) != sessions_.end())
        DVLOG(1) << "Initialize an existing stream on RTP sender." << config.ssrc;

    LOG_IF(WARNING, config.aes_key.empty() || config.aes_iv_mask.empty()) << "Unsafe to send stream with encryption DISABLED.";
    ...
    session->rtp_sender = std::make_unique<RtpSender>(transport_task_runner_, &pacer_);
    ...
    session->rtcp_observer = std::make_unique<RtcpClient>(std::move(rtcp_observer), config.ssrc, is_audio ? AUDIO_EVENT : VIDEO_EVENT, this);
    session->rtcp_session = std::make_unique<SenderRtcpSession>(clock_, &pacer_, session->rtcp_observer.get(), config.ssrc, config.feedback_ssrc);
...
namespace {
void EncryptAndSendFrame(const EncodedFrame& frame, TransportEncryptionHandler* encryptor, RtpSender* sender)
{
    if (encryptor->is_activated()) {
        ...
        if (encryptor->Encrypt(frame.frame_id, frame.data, &encrypted_frame.data)) {
            ...
            LOG(ERROR) << "Encryption failed.  Not sending frame with ID " << frame.frame_id;
        }
    } else {
...
void CastTransportImpl::InsertFrame(uint32_t ssrc, const EncodedFrame& frame)
{
    ...
    EncryptAndSendFrame(frame, &it->second->encryptor, it->second->rtp_sender.get());
}

void CastTransportImpl::SendSenderReport(uint32_t ssrc, base::TimeTicks current_time, RtpTimeTicks current_time_as_rtp_timestamp)
{
    auto it = sessions_.find(ssrc);
    if (it == sessions_.end()) {
    ...
    it->second->rtcp_session->SendRtcpReport(
        current_time, current_time_as_rtp_timestamp, it->second->rtp_sender->send_packet_count(), it->second->rtp_sender->send_octet_count());
}

void CastTransportImpl::CancelSendingFrames(uint32_t ssrc, const std::vector<FrameId>& frame_ids)
{
    auto it = sessions_.find(ssrc);
    if (it == sessions_.end()) {
...
void CastTransportImpl::ResendFrameForKickstart(uint32_t ssrc, FrameId frame_id)
{
    auto it = sessions_.find(ssrc);
    if (it == sessions_.end()) {
    ...
    it->second->rtp_sender->ResendFrameForKickstart(frame_id, it->second->rtcp_session->current_round_trip_time());
}

void CastTransportImpl::ResendPackets(
    uint32_t ssrc, const MissingFramesAndPacketsMap& missing_packets, bool cancel_rtx_if_not_in_list, const DedupInfo& dedup_info)
{
    auto it = sessions_.find(ssrc);
    if (it == sessions_.end()) {
    ...
    it->second->rtp_sender->ResendPackets(missing_packets, cancel_rtx_if_not_in_list, dedup_info);
}

PacketReceiverCallback CastTransportImpl::PacketReceiverForTesting()
{
    return base::BindRepeating(base::IgnoreResult(&CastTransportImpl::OnReceivedPacket), weak_factory_.GetWeakPtr());
}
...
void CastTransportImpl::SendRawEvents()
{
    ...
    if (!recent_frame_events_.empty() || !recent_packet_events_.empty()) {
        std::unique_ptr<std::vector<FrameEvent>> frame_events(new std::vector<FrameEvent>());
        frame_events->swap(recent_frame_events_);
        std::unique_ptr<std::vector<PacketEvent>> packet_events(new std::vector<PacketEvent>());
        packet_events->swap(recent_packet_events_);
        transport_client_->OnLoggingEventsReceived(std::move(frame_events), std::move(packet_events));
    }

    transport_task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&CastTransportImpl::SendRawEvents, weak_factory_.GetWeakPtr()), logging_flush_interval_);
}
...
void CastTransportImpl::OnReceivedLogMessage(EventMediaType media_type, const RtcpReceiverLogMessage& log)
{
    ...
    for (const RtcpReceiverFrameLogMessage& frame_log_message : log) {
        for (const RtcpReceiverEventLogMessage& event_log_message : frame_log_message.event_log_messages_) {
            ...
            default:
                VLOG(2) << "Received log message via RTCP that we did not expect: " << event_log_message.type;
                break;
            }
        }
    }
...
void CastTransportImpl::OnReceivedCastMessage(uint32_t ssrc, const RtcpCastMessage& cast_message)
{
    ...
    if (it->second->is_audio) {
        const int64_t acked_bytes = it->second->rtp_sender->GetLastByteSentForFrame(cast_message.ack_frame_id);
    ...
    if (!cast_message.missing_frames_and_packets.empty()) {
        VLOG(2) << "feedback_count: " << static_cast<uint32_t>(cast_message.feedback_count);
        ...
        ResendPackets(ssrc, cast_message.missing_frames_and_packets, true, dedup_info);
    }
...
void CastTransportImpl::AddValidRtpReceiver(uint32_t rtp_sender_ssrc, uint32_t rtp_receiver_ssrc)
{
...
void CastTransportImpl::SetOptions(const base::Value::Dict& options)
{
    ...
    int burst_size = LookupOptionWithDefault(options, kOptionPacerTargetBurstSize, media::cast::kTargetBurstSize);
    if (burst_size != media::cast::kTargetBurstSize)
        pacer_.SetTargetBurstSize(burst_size);
    burst_size = LookupOptionWithDefault(options, kOptionPacerMaxBurstSize, media::cast::kMaxBurstSize);
...
void CastTransportImpl::InitializeRtpReceiverRtcpBuilder(uint32_t rtp_receiver_ssrc, const RtcpTimeData& time_data)
{
    ...
    rtcp_builder_at_rtp_receiver_ = std::make_unique<RtcpBuilder>(rtp_receiver_ssrc);
...
void CastTransportImpl::AddCastFeedback(const RtcpCastMessage& cast_message, base::TimeDelta target_delay)
{
    if (!rtcp_builder_at_rtp_receiver_) {
...
void CastTransportImpl::AddRtcpEvents(const ReceiverRtcpEventSubscriber::RtcpEvents& rtcp_events)
{
    if (!rtcp_builder_at_rtp_receiver_) {
        VLOG(1) << "rtcp_builder_at_rtp_receiver_ is not initialized before "
...
void CastTransportImpl::AddRtpReceiverReport(const RtcpReportBlock& rtp_receiver_report_block)
{
    if (!rtcp_builder_at_rtp_receiver_) {
        VLOG(1) << "rtcp_builder_at_rtp_receiver_ is not initialized before "
...
void CastTransportImpl::SendRtcpFromRtpReceiver()
{
    ...
    pacer_.SendRtcpPacket(rtcp_builder_at_rtp_receiver_->local_ssrc(), rtcp_builder_at_rtp_receiver_->Finish());
