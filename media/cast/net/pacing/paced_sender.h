...
struct PacketKey {
    ...
    PacketKey(base::TimeTicks capture_time, uint32_t ssrc, FrameId frame_id, uint16_t packet_id);
...
public:
    ...
    virtual bool ResendPackets(const SendPacketVector& packets, const DedupInfo& dedup_info) = 0;
...
public:
    ...
    // testing.
    PacedSender(size_t target_burst_size, // Should normally be kTargetBurstSize.
        size_t max_burst_size, // Should normally be kMaxBurstSize.
        const base::TickClock* clock, std::vector<PacketEvent>* recent_packet_events, PacketTransport* external_transport,
    ...
    void SetMaxBurstSize(int burst_size)
    {
        max_burst_size_ = burst_size;
    }
...
private:
    ...
    bool ShouldResend(const PacketKey& packet_key, const DedupInfo& dedup_info, const base::TimeTicks& now);

    enum PacketType { PacketType_RTCP, PacketType_Resend, PacketType_Normal };
    enum State {
    ...
    PacketRef PopNextPacket(PacketType* packet_type, PacketKey* packet_key);
