...
public:
    ...
    void AddExpectedSizesAndPacketIds(int packet_size, uint16_t first_packet_id, int sequence_length)
    {
    ...
    bool expecting_nothing_else() const
    {
        return expected_packet_sizes_.empty();
    }
...
protected:
    PacedSenderTest()
    {
        ...
        paced_sender_ = std::make_unique<PacedSender>(kTargetBurstSize, kMaxBurstSize, &testing_clock_, &packet_events_, &mock_transport_, task_runner_);
    ...
    SendPacketVector CreateSendPacketVector(size_t packet_size, int num_of_packets_in_frame, bool audio)
    {
        ...
        for (int i = 0; i < num_of_packets_in_frame; ++i) {
            PacketKey key(frame_tick, audio ? kAudioSsrc : kVideoSsrc, FrameId::first(), i);

            PacketRef packet(new base::RefCountedData<Packet>);
            packet->data.resize(packet_size, kValue);
            // Fill-in packet header fields to test the header parsing (for populating
            // the logging events).
            base::BigEndianWriter writer(reinterpret_cast<char*>(&packet->data[0]), packet_size);
            bool success = writer.Skip(4);
            success &= writer.WriteU32(audio ? kAudioFrameRtpTimestamp : kVideoFrameRtpTimestamp);
...
TEST_F(PacedSenderTest, PassThroughRtcp)
{
    ...
    EXPECT_TRUE(paced_sender_->SendRtcpPacket(1, new base::RefCountedData<Packet>(tmp)));
}

TEST_F(PacedSenderTest, BasicPace)
{
    int num_of_packets = 27;
    SendPacketVector packets = CreateSendPacketVector(kSize1, num_of_packets, false);
...
TEST_F(PacedSenderTest, PaceWithNack)
{
    ...
    EXPECT_EQ(expected_video_retransmitted_event_count, video_retransmitted_event_count);
}
...
TEST_F(PacedSenderTest, SendPriority)
{
    ...
    EXPECT_TRUE(paced_sender_->SendPackets(CreateSendPacketVector(kSize2, 20, false)));

    testing_clock_.Advance(base::Milliseconds(10));

    // Send normal audio packet. This is queued and will be sent
    // earlier than video packets.
    EXPECT_TRUE(paced_sender_->SendPackets(CreateSendPacketVector(kSize1, 1, true)));

    // Send RTCP packet. This is queued and will be sent first.
    EXPECT_TRUE(paced_sender_->SendRtcpPacket(kVideoSsrc, new base::RefCountedData<Packet>(Packet(kSize3, kValue))));
...
TEST_F(PacedSenderTest, GetLastByteSent)
{
    ...
    EXPECT_EQ(static_cast<int64_t>(kSize1), paced_sender_->GetLastByteSentForPacket(packets1[0].first));
    EXPECT_EQ(static_cast<int64_t>(kSize1), paced_sender_->GetLastByteSentForSsrc(kAudioSsrc));
    EXPECT_EQ(0, paced_sender_->GetLastByteSentForSsrc(kVideoSsrc));

    mock_transport_.AddExpectedSizesAndPacketIds(kSize1, UINT16_C(0), 1);
    EXPECT_TRUE(paced_sender_->SendPackets(packets2));
    EXPECT_EQ(static_cast<int64_t>(2 * kSize1), paced_sender_->GetLastByteSentForPacket(packets2[0].first));
    EXPECT_EQ(static_cast<int64_t>(kSize1), paced_sender_->GetLastByteSentForSsrc(kAudioSsrc));
    EXPECT_EQ(static_cast<int64_t>(2 * kSize1), paced_sender_->GetLastByteSentForSsrc(kVideoSsrc));

    mock_transport_.AddExpectedSizesAndPacketIds(kSize1, UINT16_C(0), 1);
    EXPECT_TRUE(paced_sender_->ResendPackets(packets1, DedupInfo()));
    EXPECT_EQ(static_cast<int64_t>(3 * kSize1), paced_sender_->GetLastByteSentForPacket(packets1[0].first));
    EXPECT_EQ(static_cast<int64_t>(3 * kSize1), paced_sender_->GetLastByteSentForSsrc(kAudioSsrc));
    EXPECT_EQ(static_cast<int64_t>(2 * kSize1), paced_sender_->GetLastByteSentForSsrc(kVideoSsrc));

    mock_transport_.AddExpectedSizesAndPacketIds(kSize1, UINT16_C(0), 1);
    EXPECT_TRUE(paced_sender_->ResendPackets(packets2, DedupInfo()));
    EXPECT_EQ(static_cast<int64_t>(4 * kSize1), paced_sender_->GetLastByteSentForPacket(packets2[0].first));
    EXPECT_EQ(static_cast<int64_t>(3 * kSize1), paced_sender_->GetLastByteSentForSsrc(kAudioSsrc));
    EXPECT_EQ(static_cast<int64_t>(4 * kSize1), paced_sender_->GetLastByteSentForSsrc(kVideoSsrc));
}
...
TEST_F(PacedSenderTest, AllPacketsInSameFrameAreResentFairly)
{
    ...
    mock_transport_.AddExpectedSizesAndPacketIds(kSize1, UINT16_C(0), kNumPackets);
