...
PacketKey::PacketKey(base::TimeTicks capture_time, uint32_t ssrc, FrameId frame_id, uint16_t packet_id)
...
PacedSender::PacedSender(size_t target_burst_size, size_t max_burst_size, const base::TickClock* clock, std::vector<PacketEvent>* recent_packet_events,
    PacketTransport* transport, const scoped_refptr<base::SingleThreadTaskRunner>& transport_task_runner)
    : clock_(clock)
    ...
    for (size_t i = 0; i < packets.size(); i++) {
        if (VLOG_IS_ON(2)) {
            ...
            if (history_it != send_history_.end() && history_it->second.cancel_count > 0) {
                VLOG(2) << "PacedSender::SendPackets() called for packet CANCELED " << history_it->second.cancel_count << " times: "
                        << "ssrc=" << packets[i].first.ssrc << ", frame_id=" << packets[i].first.frame_id << ", packet_id=" << packets[i].first.packet_id;
            }
        }
...
bool PacedSender::ShouldResend(const PacketKey& packet_key, const DedupInfo& dedup_info, const base::TimeTicks& now)
{
    PacketSendHistory::const_iterator it = send_history_.find(packet_key);
    ...
    if (!session_it->second.is_audio) {
        if (dedup_info.last_byte_acked_for_audio && it->second.last_byte_sent_for_audio
            && dedup_info.last_byte_acked_for_audio < it->second.last_byte_sent_for_audio) {
            return false;
        }
    }
...
bool PacedSender::ResendPackets(const SendPacketVector& packets, const DedupInfo& dedup_info)
{
    if (packets.empty()) {
        return true;
    }
    ...
    for (size_t i = 0; i < packets.size(); i++) {
        if (VLOG_IS_ON(2)) {
            PacketSendHistory::const_iterator history_it = send_history_.find(packets[i].first);
            if (history_it != send_history_.end() && history_it->second.cancel_count > 0) {
                VLOG(2) << "PacedSender::ReendPackets() called for packet CANCELED " << history_it->second.cancel_count << " times: "
                        << "ssrc=" << packets[i].first.ssrc << ", frame_id=" << packets[i].first.frame_id << ", packet_id=" << packets[i].first.packet_id;
            }
        }
...
bool PacedSender::SendRtcpPacket(uint32_t ssrc, PacketRef packet)
{
    if (state_ == State_TransportBlocked) {
        ...
        if (!transport_->SendPacket(packet, base::BindOnce(&PacedSender::SendStoredPackets, weak_factory_.GetWeakPtr()))) {
...
PacketRef PacedSender::PopNextPacket(PacketType* packet_type, PacketKey* packet_key)
{
...
void PacedSender::SendStoredPackets()
{
    ...
    if (now >= burst_end_ || previous_state == State_BurstFull) {
        ...
        size_t max_burst_size = std::min(max_burst_size_, std::max(target_burst_size_, size() / kPacingMaxBurstsPerFrame));
    ...
    base::RepeatingClosure cb = base::BindRepeating(&PacedSender::SendStoredPackets, weak_factory_.GetWeakPtr());
    while (!empty()) {
        if (current_burst_size_ >= current_max_burst_size_) {
            transport_task_runner_->PostDelayedTask(FROM_HERE, cb, burst_end_ - now);
        ...
        if (send_record->cancel_count > 0 && packet_type != PacketType_RTCP) {
            VLOG(2) << "PacedSender is sending a packet known to have been CANCELED " << send_record->cancel_count << " times: "
                    << "ssrc=" << packet_key.ssrc << ", frame_id=" << packet_key.frame_id << ", packet_id=" << packet_key.packet_id;
        }
    ...
    DCHECK_LE(send_history_buffer_.size(), max_burst_size_ * kMaxDedupeWindowMs / kPacingIntervalMs);
    state_ = State_Unblocked;
}
