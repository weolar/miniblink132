...
protected:
    ...
    void ExpectParsesPacket()
    {
        ...
        EXPECT_TRUE(rtp_parser_.ParsePacket(packet_, kPacketLength, &parsed_header, &payload, &payload_size));
        ...
        EXPECT_EQ(cast_header_.reference_frame_id, parsed_header.reference_frame_id);
    ...
    void ExpectDoesNotParsePacket()
    {
        ...
        const uint8_t* payload = NULL;
        size_t payload_size = static_cast<size_t>(-1);
        EXPECT_FALSE(rtp_parser_.ParsePacket(packet_, kPacketLength, &parsed_header, &payload, &payload_size));
    }
...
TEST_F(RtpParserTest, ParseExpandingReferenceFrameIdTo32Bits)
{
    ...
    for (int frame_id = kMaxBackReferenceOffset; frame_id <= kMaxFrameId; ++frame_id) {
