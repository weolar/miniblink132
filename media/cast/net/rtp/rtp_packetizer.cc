...
constexpr uint16_t kHeadersLength = kRtpHeaderLength + kCastHeaderLength + kPlayoutDelayHeaderLength;
...
// specifically).
constexpr uint16_t kDefaultMaxPayloadLength = kMaxIpPacketSize - kIpAndIcmpHeadersLength;
...
RtpPacketizer::RtpPacketizer(PacedSender* const transport, PacketStorage* packet_storage, RtpPacketizerConfig rtp_packetizer_config)
...
void RtpPacketizer::SendFrameAsPackets(const EncodedFrame& frame)
{
    ...
    while (remaining_size > 0) {
        ...
        BuildCommonRtpHeader(&packet->data, remaining_size == 0, frame.rtp_timestamp);

        // Build Cast header.
        DCHECK_NE(frame.dependency, openscreen::cast::EncodedFrame::Dependency::kUnknown);
        uint8_t byte0 = kCastReferenceFrameIdBitMask;
        if (frame.dependency == openscreen::cast::EncodedFrame::Dependency::kKeyFrame)
        ...
        base::BigEndianWriter big_endian_writer(reinterpret_cast<char*>(&(packet->data[start_size])), 4);
        ...
        if (packet_id == 0 && frame.new_playout_delay_ms) {
            ...
            packet->data.push_back(static_cast<uint8_t>(frame.new_playout_delay_ms >> 8));
            packet->data.push_back(static_cast<uint8_t>(frame.new_playout_delay_ms));
        }
        DCHECK_LE(packet->data.size(), kHeadersLength);

        // Copy payload data.
        packet->data.insert(packet->data.end(), data_iter, data_iter + payload_length);
        ...
        packets.push_back(make_pair(PacketKey(frame.reference_time, config_.ssrc, frame.frame_id, packet_id), packet));
...
void RtpPacketizer::BuildCommonRtpHeader(Packet* packet, bool marker_bit, RtpTimeTicks rtp_timestamp)
{
    ...
    base::BigEndianWriter big_endian_writer(reinterpret_cast<char*>(&((*packet)[start_size])), 10);
