...
RtpSender::RtpSender(const scoped_refptr<base::SingleThreadTaskRunner>& transport_task_runner, PacedSender* const transport)
...
void RtpSender::SendFrame(const EncodedFrame& frame)
{
    ...
    LOG_IF(DFATAL, storage_.GetNumberOfStoredFrames() > kMaxUnackedFrames) << "Possible bug: Frames are not being actively released from storage.";
}

void RtpSender::ResendPackets(const MissingFramesAndPacketsMap& missing_frames_and_packets, bool cancel_rtx_if_not_in_list, const DedupInfo& dedup_info)
{
    // Iterate over all frames in the list.
    for (auto it = missing_frames_and_packets.begin(); it != missing_frames_and_packets.end(); ++it) {
        ...
        for (auto packet_it = stored_packets->begin(); packet_it != stored_packets->end(); ++packet_it) {
...
void RtpSender::ResendFrameForKickstart(FrameId frame_id, base::TimeDelta dedupe_window)
{
...
void RtpSender::UpdateSequenceNumber(Packet* packet)
{
    ...
    base::BigEndianWriter big_endian_writer(reinterpret_cast<char*>((&packet->front()) + kByteOffsetToSequenceNumber), sizeof(uint16_t));
