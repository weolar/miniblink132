...
public:
    ...
    void VerifyCastRtpHeader(const RtpCastHeader& rtp_header)
    {
        ...
        if (rtp_header.packet_id != 0) {
            EXPECT_EQ(rtp_header.num_extensions, 0) << "Extensions only allowed on first packet of a frame";
        }
    }
    ...
    bool SendPacket(PacketRef packet, base::OnceClosure cb) final
    {
        ...
        size_t payload_size;
        parser.ParsePacket(&packet->data[0], packet->data.size(), &rtp_header, &payload_data, &payload_size);
    ...
    size_t number_of_packets_received() const
    {
...
protected:
        ...
        : task_runner_(new FakeSingleThreadTaskRunner(&testing_clock_))
    {
        ...
        pacer_ = std::make_unique<PacedSender>(kTargetBurstSize, kMaxBurstSize, &testing_clock_, nullptr, transport_.get(), task_runner_);
        pacer_->RegisterSsrc(config_.ssrc, false);
        rtp_packetizer_ = std::make_unique<RtpPacketizer>(pacer_.get(), &packet_storage_, config_);
        video_frame_.dependency = openscreen::cast::EncodedFrame::Dependency::kDependent;
