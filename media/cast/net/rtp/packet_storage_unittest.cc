...
static void StoreFrames(int number_of_frames, FrameId first_frame_id, PacketStorage* storage)
{
    const int kSsrc = 1;
    for (int i = 0; i < number_of_frames; ++i) {
        ...
        for (int j = 0; j < number_of_packets; ++j) {
            ...
            packets.push_back(std::make_pair(
                PacketKey(base::TimeTicks(), kSsrc, first_frame_id + i, base::checked_cast<uint16_t>(j)), new base::RefCountedData<Packet>(test_packet)));
        }
...
TEST(PacketStorageTest, NumberOfStoredFrames)
{
    ...
    EXPECT_EQ(static_cast<size_t>(kMaxUnackedFrames / 2), storage.GetNumberOfStoredFrames());
}
...
TEST(PacketStorageTest, StoreAndGetFrames)
{
    ...
    EXPECT_EQ(std::min<size_t>(kMaxUnackedFrames, kStoredFrames), storage.GetNumberOfStoredFrames());
...
TEST(PacketStorageTest, FramesReleased)
{
    ...
    EXPECT_EQ(std::min<size_t>(kMaxUnackedFrames, 5), storage.GetNumberOfStoredFrames());

    for (FrameId frame_id = first_frame_id; frame_id < first_frame_id + 5; ++frame_id) {
