...
protected:
    void ResetDataPipes()
    {
        ...
        ASSERT_EQ(mojo::CreateDataPipe(kDefaultDataPipeCapacityBytes, producer_handle, consumer_handle), MOJO_RESULT_OK);

        writer_ = std::make_unique<media::MojoDataPipeWriter>(std::move(producer_handle));
        buffer_reader_ = std::make_unique<DecoderBufferReader>(
            base::BindRepeating(&DecoderBufferReaderTest::OnBufferReady, base::Unretained(this)), std::move(consumer_handle));
    }

    void WriteBufferData()
    {
        writer_->Write(serialized_data_.data(), serialized_data_.size(), base::BindOnce(&DecoderBufferReaderTest::OnWriteDone, base::Unretained(this)));
    }
