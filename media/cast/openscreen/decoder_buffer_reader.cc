...
DecoderBufferReader::DecoderBufferReader(NewBufferCb new_buffer_cb, mojo::ScopedDataPipeConsumerHandle data_pipe)
...
DecoderBufferReader::DecoderBufferReader(DecoderBufferReader&& other, mojo::ScopedDataPipeConsumerHandle data_pipe)
    : DecoderBufferReader(std::move(other.new_buffer_cb_), std::move(data_pipe))
{
...
void DecoderBufferReader::TryGetNextBuffer()
{
    ...
    media::mojom::DecoderBufferPtr buffer = std::move(pending_buffer_metadata_.front());
    pending_buffer_metadata_.pop_front();
    mojo_buffer_reader_.ReadDecoderBuffer(std::move(buffer), base::BindOnce(&DecoderBufferReader::OnBufferReadFromDataPipe, weak_factory_.GetWeakPtr()));
}

void DecoderBufferReader::OnBufferReadFromDataPipe(scoped_refptr<media::DecoderBuffer> buffer)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
