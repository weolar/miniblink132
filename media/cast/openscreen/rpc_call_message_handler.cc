...
template <typename T> std::optional<media::AudioDecoderConfig> ExtractAudioConfig(const T& config_container)
{
...
template <typename T> std::optional<media::VideoDecoderConfig> ExtractVideoConfig(const T& config_container)
{
...
bool DispatchInitializationAcquireRendererRpcCall(openscreen::cast::RpcMessage* message, RpcInitializationCallMessageHandler* client)
{
    if (!message->has_integer_value()) {
        LOG(ERROR) << "RPC_ACQUIRE_RENDERER with no integer_value() property received";
        return false;
    }
...
bool DispatchInitializationAcquireDemuxerRpcCall(openscreen::cast::RpcMessage* message, RpcInitializationCallMessageHandler* client)
{
    ...
    client->OnRpcAcquireDemuxer(rpc.audio_demuxer_handle(), rpc.video_demuxer_handle());
    return true;
}

bool DispatchRendererFlushUntilRpcCall(openscreen::cast::RpcMessage* message, RpcRendererCallMessageHandler* client)
{
    ...
    const openscreen::cast::RendererFlushUntil flush_message = message->renderer_flushuntil_rpc();
    client->OnRpcFlush(flush_message.audio_count(), flush_message.video_count());
    return true;
}

bool DispatchRendererStartPlayingFromRpcCall(openscreen::cast::RpcMessage* message, RpcRendererCallMessageHandler* client)
{
    if (!message->has_integer64_value()) {
        LOG(ERROR) << "RPC_R_STARTPLAYINGFROM with no integer64_value() property received";
        return false;
    }
...
bool DispatchRendererSetPlaybackRateRpcCall(openscreen::cast::RpcMessage* message, RpcRendererCallMessageHandler* client)
{
    if (!message->has_double_value()) {
        LOG(ERROR) << "RPC_R_SETPLAYBACKRATE with no double_value() property received";
        return false;
    }
...
bool DispatchRendererSetVolumeRpcCall(openscreen::cast::RpcMessage* message, RpcRendererCallMessageHandler* client)
{
    if (!message->has_double_value()) {
...
bool DispatchDemuxerStreamInitializeCBRpcCall(openscreen::cast::RpcMessage* message, RpcDemuxerStreamCBMessageHandler* client)
{
    ...
    client->OnRpcInitializeCallback(message->handle(), ExtractAudioConfig(callback_message), ExtractVideoConfig(callback_message));
    return true;
}

bool DispatchDemuxerStreamReadUntilCBRpcCall(openscreen::cast::RpcMessage* message, RpcDemuxerStreamCBMessageHandler* client)
{
    ...
    if ((audio_config || video_config) && status != media::DemuxerStream::kConfigChanged) {
    ...
    client->OnRpcReadUntilCallback(message->handle(), std::move(audio_config), std::move(video_config), total_frames_received);
    return true;
}

bool DispatchDemuxerStreamEnableBitstreamConverterCBRpcCall(openscreen::cast::RpcMessage* message, RpcDemuxerStreamCBMessageHandler* client)
{
    ...
    client->OnRpcEnableBitstreamConverterCallback(message->handle(), message->boolean_value());
    return true;
}

} // namespace

RpcInitializationCallMessageHandler::~RpcInitializationCallMessageHandler() = default;
...
bool DispatchInitializationRpcCall(openscreen::cast::RpcMessage* message, RpcInitializationCallMessageHandler* client)
{
    DCHECK(message);
    DCHECK(client);
...
bool DispatchRendererRpcCall(openscreen::cast::RpcMessage* message, RpcRendererCallMessageHandler* client)
{
    DCHECK(message);
    DCHECK(client);
...
bool DispatchDemuxerStreamCBRpcCall(openscreen::cast::RpcMessage* message, RpcDemuxerStreamCBMessageHandler* client)
{
    DCHECK(message);
    DCHECK(client);
    ...
    case openscreen::cast::RpcMessage::RPC_DS_ENABLEBITSTREAMCONVERTER_CALLBACK:
        return DispatchDemuxerStreamEnableBitstreamConverterCBRpcCall(message, client);
    default:
        return false;
    }
}
