...
public:
    class MockRpcRendererCallMessageHandler : public RpcRendererCallMessageHandler {
    public:
    ...
    class MockRpcInitializationCallMessageHandler : public RpcInitializationCallMessageHandler {
    public:
    ...
    class MockRpcDemuxerStreamCBMessageHandler : public RpcDemuxerStreamCBMessageHandler {
    public:
        ~MockRpcDemuxerStreamCBMessageHandler() override = default;

        MOCK_METHOD3(OnRpcInitializeCallback, void(int, std::optional<media::AudioDecoderConfig>, std::optional<media::VideoDecoderConfig>));
        MOCK_METHOD4(OnRpcReadUntilCallback, void(int, std::optional<media::AudioDecoderConfig>, std::optional<media::VideoDecoderConfig>, uint32_t));
        MOCK_METHOD2(OnRpcEnableBitstreamConverterCallback, void(int, bool));
    };

    RpcCallMessageHandlerTest() = default;

    media::AudioDecoderConfig test_audio_config_ = media::AudioDecoderConfig(media::AudioCodec::kAAC, media::SampleFormat::kSampleFormatF32,
        media::CHANNEL_LAYOUT_MONO, 10000, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
    media::VideoDecoderConfig test_video_config_ = media::VideoDecoderConfig(media::VideoCodec::kH264, media::VideoCodecProfile::H264PROFILE_MAIN,
        media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace::JPEG(), media::VideoTransformation(), { 1920, 1080 }, { 1920, 1080 },
        { 1920, 1080 }, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
...
TEST_F(RpcCallMessageHandlerTest, OnRpcAcquireDemuxer)
{
    ...
    EXPECT_TRUE(DispatchInitializationRpcCall(rpc.get(), &initialization_client_));
}
...
TEST_F(RpcCallMessageHandlerTest, OnInvalidInitializationMessage)
{
    auto rpc = std::make_unique<openscreen::cast::RpcMessage>();
    rpc->set_proc(openscreen::cast::RpcMessage::RPC_R_SETVOLUME);
    EXPECT_FALSE(DispatchInitializationRpcCall(rpc.get(), &initialization_client_));
}
...
TEST_F(RpcCallMessageHandlerTest, OnDemuxerStreamInitializeCallbackValid)
{
    ...
    EXPECT_CALL(demuxer_stream_client_, OnRpcInitializeCallback(kHandle, _, _))
        .WillOnce([this](openscreen::cast::RpcMessenger::Handle handle, std::optional<media::AudioDecoderConfig> audio_config,
    ...
    EXPECT_TRUE(DispatchDemuxerStreamCBRpcCall(rpc.get(), &demuxer_stream_client_));
}
...
TEST_F(RpcCallMessageHandlerTest, OnDemuxerStreamInitializeCallbackNoConfig)
{
    ...
    rpc->set_proc(openscreen::cast::RpcMessage::RPC_DS_INITIALIZE_CALLBACK);
    rpc->set_handle(123);
    EXPECT_FALSE(DispatchDemuxerStreamCBRpcCall(rpc.get(), &demuxer_stream_client_));
}
...
TEST_F(RpcCallMessageHandlerTest, OnDemuxerStreamReadUntilCallbackValid)
{
    ...
    readuntil_cb->set_status(ToProtoDemuxerStreamStatus(media::DemuxerStream::kConfigChanged).value());
    EXPECT_CALL(demuxer_stream_client_, OnRpcReadUntilCallback(kHandle, _, _, kCount))
        .WillOnce([this](openscreen::cast::RpcMessenger::Handle handle, std::optional<media::AudioDecoderConfig> audio_config,
                      std::optional<media::VideoDecoderConfig> video_config, uint32_t count) {
            EXPECT_TRUE(audio_config.has_value());
            EXPECT_TRUE(test_audio_config_.Matches(audio_config.value()));
            EXPECT_TRUE(video_config.has_value());
            EXPECT_TRUE(test_video_config_.Matches(video_config.value()));
...
TEST_F(RpcCallMessageHandlerTest, OnDemuxerStreamReadUntilCallbackNonConfig)
{
    ...
    auto* readuntil_cb = rpc->mutable_demuxerstream_readuntilcb_rpc();
    readuntil_cb->set_count(kCount);
    readuntil_cb->set_status(ToProtoDemuxerStreamStatus(media::DemuxerStream::kOk).value());
    EXPECT_CALL(demuxer_stream_client_, OnRpcReadUntilCallback(kHandle, _, _, kCount))
        .WillOnce([](openscreen::cast::RpcMessenger::Handle handle, std::optional<media::AudioDecoderConfig> audio_config,
                      std::optional<media::VideoDecoderConfig> video_config, uint32_t count) {
            EXPECT_FALSE(audio_config.has_value());
...
TEST_F(RpcCallMessageHandlerTest, OnRpcEnableBitstreamConverterCallback)
{
    ...
    EXPECT_CALL(demuxer_stream_client_, OnRpcEnableBitstreamConverterCallback(kHandle, true));
    EXPECT_TRUE(DispatchDemuxerStreamCBRpcCall(rpc.get(), &demuxer_stream_client_));
}
