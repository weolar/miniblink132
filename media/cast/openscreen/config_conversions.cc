...
media::VideoCodecProfile ToVideoDecoderConfigCodecProfile(openscreen::cast::VideoCodec codec)
{
    switch (codec) {
...
media::AudioCodec ToAudioDecoderConfigCodec(openscreen::cast::AudioCodec codec)
{
    switch (codec) {
...
media::VideoCodec ToVideoDecoderConfigCodec(openscreen::cast::VideoCodec codec)
{
    switch (codec) {
    case openscreen::cast::VideoCodec::kH264:
...
openscreen::cast::AudioCaptureConfig ToAudioCaptureConfig(const media::AudioDecoderConfig& audio_config)
{
    ...
    audio_capture_config.channels = media::ChannelLayoutToChannelCount(audio_config.channel_layout());
...
openscreen::cast::VideoCaptureConfig ToVideoCaptureConfig(const media::VideoDecoderConfig& video_config)
{
    ...
    video_capture_config.resolutions.push_back({ video_config.visible_rect().width(), video_config.visible_rect().height() });
    video_capture_config.max_bit_rate = 0; // Selected by the sender.
    return video_capture_config;
}

media::AudioDecoderConfig ToAudioDecoderConfig(const openscreen::cast::AudioCaptureConfig& audio_capture_config)
{
    media::AudioCodec media_audio_codec = ToAudioDecoderConfigCodec(audio_capture_config.codec);

    return media::AudioDecoderConfig(media_audio_codec, media::SampleFormat::kSampleFormatF32, media::GuessChannelLayout(audio_capture_config.channels),
        audio_capture_config.sample_rate /* samples_per_second */, media::EmptyExtraData(), media::EncryptionScheme::kUnencrypted);
}

media::VideoDecoderConfig ToVideoDecoderConfig(const openscreen::cast::VideoCaptureConfig& video_capture_config)
{
    ...
    media::VideoCodec media_video_codec = ToVideoDecoderConfigCodec(video_capture_config.codec);
    media::VideoCodecProfile video_codec_profile = ToVideoDecoderConfigCodecProfile(video_capture_config.codec);

    return media::VideoDecoderConfig(media_video_codec, video_codec_profile, media::VideoDecoderConfig::AlphaMode::kIsOpaque, media::VideoColorSpace(),
