...
void ClockDriftSmoother::Reset(base::TimeTicks now, base::TimeDelta measured_offset)
{
    DCHECK(!now.is_null());
...
void ClockDriftSmoother::Update(base::TimeTicks now, base::TimeDelta measured_offset)
{
    DCHECK(!now.is_null());
    ...
    estimate_us_ = weight * measured_offset.InMicrosecondsF() + (1.0 - weight) * estimate_us_;
}
