...
bool TransportEncryptionHandler::Initialize(const std::string& aes_key, const std::string& aes_iv_mask)
{
    is_activated_ = false;
    if (aes_iv_mask.size() == kAesKeySize && aes_key.size() == kAesKeySize) {
        ...
        DCHECK_EQ(aes_iv_mask.size(), 0u) << "Invalid Crypto configuration: aes_iv_mask.size";
        DCHECK_EQ(aes_key.size(), 0u) << "Invalid Crypto configuration: aes_key.size";
        return false;
    }
    return true;
}

bool TransportEncryptionHandler::Encrypt(FrameId frame_id, const base::StringPiece& data, std::string* encrypted_data)
{
    if (!is_activated_)
        return false;
...
bool TransportEncryptionHandler::Decrypt(FrameId frame_id, const base::StringPiece& ciphertext, std::string* plaintext)
{
    if (!is_activated_) {
        return false;
    }
