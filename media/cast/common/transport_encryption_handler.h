...
public:
    ...
    TransportEncryptionHandler& operator=(const TransportEncryptionHandler&) = delete;

    ~TransportEncryptionHandler();

    bool Initialize(const std::string& aes_key, const std::string& aes_iv_mask);

    bool Encrypt(FrameId frame_id, const base::StringPiece& data, std::string* encrypted_data);

    bool Decrypt(FrameId frame_id, const base::StringPiece& ciphertext, std::string* plaintext);
