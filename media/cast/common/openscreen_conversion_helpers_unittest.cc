...
TEST(OpenscreenConversionHelpersTest, EncodedFrameConversions)
{
    ...
    original.encode_completion_time = base::TimeTicks() + base::Milliseconds(1337);
    original.dependency = openscreen::cast::EncodedFrame::Dependency::kIndependent;
    ...
    original.data = base::HeapArray<uint8_t>::CopiedFrom(base::byte_span_from_cstring(kData));

    const openscreen::cast::EncodedFrame converted = ToOpenscreenEncodedFrame(original);
    EXPECT_EQ(openscreen::cast::EncodedFrame::Dependency::kIndependent, converted.dependency);
    ...
    EXPECT_EQ(openscreen::Clock::time_point() + std::chrono::milliseconds(1338), converted.reference_time);
...
{
    EXPECT_EQ(openscreen::Clock::time_point() + std::chrono::milliseconds(42), ToOpenscreenTimePoint(base::TimeTicks() + base::Milliseconds(42)));

    EXPECT_EQ(base::Milliseconds(300), ToTimeDelta(std::chrono::milliseconds(300)));
}
