...
protected:
    StatsEventSubscriberTest()
        ...
        , cast_environment_(new CastEnvironment(&sender_clock_, task_runner_, task_runner_, task_runner_))
    ...
    void Init(EventMediaType event_media_type)
    {
        ...
        subscriber_ = std::make_unique<StatsEventSubscriber>(event_media_type, cast_environment_->Clock(), &fake_offset_estimator_);
...
TEST_F(StatsEventSubscriberTest, CaptureEncode)
{
    ...
    for (int i = 0; i < num_frames; i++) {
        ...
        cast_environment_->logger()->DispatchFrameEvent(std::move(capture_begin_event));
...
TEST_F(StatsEventSubscriberTest, Encode)
{
    ...
    EXPECT_DOUBLE_EQ(it->second, (first_event_time - base::TimeTicks::UnixEpoch()).InMillisecondsF());

    it = stats_map.find(StatsEventSubscriber::LAST_EVENT_TIME_MS);
    ASSERT_TRUE(it != stats_map.end());

    EXPECT_DOUBLE_EQ(it->second, (last_event_time - base::TimeTicks::UnixEpoch()).InMillisecondsF());
}
...
TEST_F(StatsEventSubscriberTest, Packets)
{
    ...
    EXPECT_DOUBLE_EQ(it->second, total_network_latency.InMillisecondsF() / num_latency_recorded_packets);

    it = stats_map.find(StatsEventSubscriber::AVG_QUEUEING_LATENCY_MS);
    ASSERT_TRUE(it != stats_map.end());

    EXPECT_DOUBLE_EQ(it->second, total_queueing_latency.InMillisecondsF() / num_packets);

    it = stats_map.find(StatsEventSubscriber::AVG_PACKET_LATENCY_MS);
    ASSERT_TRUE(it != stats_map.end());

    EXPECT_DOUBLE_EQ(it->second, total_packet_latency.InMillisecondsF() / num_latency_recorded_packets);
    ...
    EXPECT_DOUBLE_EQ(it->second, retransmit_total_size / duration.InMillisecondsF() * 8);
...
bool CheckHistogramHasValue(const base::Value::List& values, const std::string& bucket, int expected_count)
{
...
TEST_F(StatsEventSubscriberTest, Histograms)
{
    ...
    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::CAPTURE_LATENCY_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    EXPECT_TRUE(CheckHistogramHasValue(values, "10-14", 10));

    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::ENCODE_TIME_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    EXPECT_TRUE(CheckHistogramHasValue(values, "15-19", 10));

    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::QUEUEING_LATENCY_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    ...
    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::NETWORK_LATENCY_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    EXPECT_TRUE(CheckHistogramHasValue(values, "100-119", 1));
    EXPECT_TRUE(CheckHistogramHasValue(values, "200-219", 1));
    ...
    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::PACKET_LATENCY_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    EXPECT_TRUE(CheckHistogramHasValue(values, "400-419", 3));

    histogram = subscriber_->GetHistogramForTesting(StatsEventSubscriber::LATE_FRAME_MS_HISTO);
    ASSERT_TRUE(histogram);
    values = histogram->GetHistogram();
    EXPECT_TRUE(CheckHistogramHasValue(values, "100-119", 1));
}
