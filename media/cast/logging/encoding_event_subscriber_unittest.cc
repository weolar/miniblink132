...
protected:
    ...
    void Init(EventMediaType event_media_type)
    {
        ...
        event_subscriber_ = std::make_unique<EncodingEventSubscriber>(event_media_type, 10);
    ...
    {
        event_subscriber_->GetEventsAndReset(&metadata_, &frame_events_, &packet_events_);
...
TEST_F(EncodingEventSubscriberTest, FrameEvent)
{
    ...
    EXPECT_EQ((rtp_timestamp - first_rtp_timestamp_).lower_32_bits(), event->relative_rtp_timestamp());
...
TEST_F(EncodingEventSubscriberTest, MultipleFrameEvents)
{
...

    {
        ...
        EXPECT_EQ((rtp_timestamp1 - first_rtp_timestamp_).lower_32_bits(), event->relative_rtp_timestamp());
...

    {
        ...
        EXPECT_EQ((rtp_timestamp2 - first_rtp_timestamp_).lower_32_bits(), event->relative_rtp_timestamp());
...
TEST_F(EncodingEventSubscriberTest, MultiplePacketEventsForPacket)
{
    ...
    EXPECT_EQ(media::cast::proto::PACKET_SENT_TO_NETWORK, base_event.event_type(0));
...
TEST_F(EncodingEventSubscriberTest, MultiplePacketEventsForFrame)
{
    ...
    EXPECT_EQ(media::cast::proto::PACKET_RETRANSMITTED, base_event_2.event_type(0));
...
TEST_F(EncodingEventSubscriberTest, MultiplePacketEvents)
{
...

    {
        ...
        EXPECT_EQ((rtp_timestamp_1 - first_rtp_timestamp_).lower_32_bits(), event->relative_rtp_timestamp());
...

    {
        ...
        EXPECT_EQ((rtp_timestamp_2 - first_rtp_timestamp_).lower_32_bits(), event->relative_rtp_timestamp());
...
TEST_F(EncodingEventSubscriberTest, MaxEventsPerProto)
{
    ...
    {
        const AggregatedPacketEvent* packet_event = packet_it->get();
        EXPECT_EQ(kMaxEventsPerProto, packet_event->base_packet_event(0).event_type_size());
    }
