...
template <typename ProtoPtr> bool IsRtpTimestampLessThan(const ProtoPtr& lhs, const ProtoPtr& rhs)
{
    return lhs->relative_rtp_timestamp() < rhs->relative_rtp_timestamp();
}

BasePacketEvent* GetNewBasePacketEvent(AggregatedPacketEvent* event_proto, int packet_id, int size)
{
...
EncodingEventSubscriber::EncodingEventSubscriber(EventMediaType event_media_type, size_t max_frames)
...
void EncodingEventSubscriber::OnReceiveFrameEvent(const FrameEvent& frame_event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    if (it == frame_event_map_.end()) {
        ...
        frame_event_map_.insert(std::make_pair(relative_rtp_timestamp, std::move(event_proto)));
    } else {
        if (it->second->event_type_size() >= kMaxEventsPerProto) {
            DVLOG(2) << "Too many events in frame " << frame_event.rtp_timestamp << ". Using new frame event proto.";
    ...
    event_proto_ptr->add_event_timestamp_ms((frame_event.timestamp - base::TimeTicks()).InMilliseconds());
    ...
    if (frame_event.type == FRAME_CAPTURE_END) {
        ...
        if (frame_event.encoder_cpu_utilization >= 0.0) {
            event_proto_ptr->set_encoder_cpu_percent_utilized(base::saturated_cast<int32_t>(frame_event.encoder_cpu_utilization * 100.0 + 0.5));
        }
        if (frame_event.idealized_bitrate_utilization >= 0.0) {
            event_proto_ptr->set_idealized_bitrate_percent_utilized(base::saturated_cast<int32_t>(frame_event.idealized_bitrate_utilization * 100.0 + 0.5));
        }
...
void EncodingEventSubscriber::OnReceivePacketEvent(const PacketEvent& packet_event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    if (it == packet_event_map_.end()) {
        ...
        event_proto->set_relative_rtp_timestamp(lower_32_bits);
        base_packet_event_proto = GetNewBasePacketEvent(event_proto.get(), packet_event.packet_id, packet_event.size);
        packet_event_map_.insert(std::make_pair(relative_rtp_timestamp, std::move(event_proto)));
    } else {
        ...
        RepeatedPtrField<BasePacketEvent>* field = it->second->mutable_base_packet_event();
        for (RepeatedPtrField<BasePacketEvent>::pointer_iterator base_it = field->pointer_begin(); base_it != field->pointer_end(); ++base_it) {
        ...
        if (!base_packet_event_proto) {
            if (it->second->base_packet_event_size() >= kMaxPacketsPerFrame) {
                DVLOG(3) << "Too many packets in AggregatedPacketEvent " << packet_event.rtp_timestamp << ". "
            ...
            base_packet_event_proto = GetNewBasePacketEvent(it->second.get(), packet_event.packet_id, packet_event.size);
        } else if (base_packet_event_proto->event_type_size() >= kMaxEventsPerProto) {
            DVLOG(3) << "Too many events in packet " << packet_event.rtp_timestamp << ", " << packet_event.packet_id << ". Using new packet event proto.";
            AddPacketEventToStorage(std::move(it->second));
            if (!ShouldCreateNewProto(lower_32_bits)) {
                packet_event_map_.erase(it);
                return;
    ...
    base_packet_event_proto->add_event_timestamp_ms((packet_event.timestamp - base::TimeTicks()).InMilliseconds());
...
void EncodingEventSubscriber::GetEventsAndReset(LogMetadata* metadata, FrameEventList* frame_events, PacketEventList* packet_events)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    ...
    std::sort(frame_event_storage_.begin(), frame_event_storage_.end(), &IsRtpTimestampLessThan<std::unique_ptr<AggregatedFrameEvent>>);
    std::sort(packet_event_storage_.begin(), packet_event_storage_.end(), &IsRtpTimestampLessThan<std::unique_ptr<AggregatedPacketEvent>>);
    ...
    metadata->set_reference_timestamp_ms_at_unix_epoch((base::TimeTicks::UnixEpoch() - base::TimeTicks()).InMilliseconds());
...
void EncodingEventSubscriber::TransferFrameEvents(size_t max_num_entries)
{
    ...
    for (size_t i = 0; i < max_num_entries && it != frame_event_map_.end(); i++, ++it) {
        AddFrameEventToStorage(std::move(it->second));
    }
...
void EncodingEventSubscriber::TransferPacketEvents(size_t max_num_entries)
{
    ...
    for (size_t i = 0; i < max_num_entries && it != packet_event_map_.end(); i++, ++it) {
        AddPacketEventToStorage(std::move(it->second));
    }
...
void EncodingEventSubscriber::AddFrameEventToStorage(std::unique_ptr<AggregatedFrameEvent> frame_event_proto)
{
...
void EncodingEventSubscriber::AddPacketEventToStorage(std::unique_ptr<AggregatedPacketEvent> packet_event_proto)
{
...
bool EncodingEventSubscriber::ShouldCreateNewProto(uint32_t relative_rtp_timestamp_lower_32_bits) const
{
    ...
    DVLOG_IF(2, proto_count >= kMaxProtosPerFrame) << relative_rtp_timestamp_lower_32_bits << " already reached max number of protos.";
    return proto_count < kMaxProtosPerFrame;
}

void EncodingEventSubscriber::IncrementStoredProtoCount(uint32_t relative_rtp_timestamp_lower_32_bits)
{
    stored_proto_counts_[relative_rtp_timestamp_lower_32_bits]++;
    DCHECK_LE(stored_proto_counts_[relative_rtp_timestamp_lower_32_bits], kMaxProtosPerFrame)
        << relative_rtp_timestamp_lower_32_bits << " exceeded max number of event protos.";
}

void EncodingEventSubscriber::DecrementStoredProtoCount(uint32_t relative_rtp_timestamp_lower_32_bits)
{
    auto it = stored_proto_counts_.find(relative_rtp_timestamp_lower_32_bits);
    CHECK(it != stored_proto_counts_.end(), base::NotFatalUntil::M130) << "no event protos for " << relative_rtp_timestamp_lower_32_bits;
    if (it->second > 1)
        it->second--;
    else
...
RtpTimeDelta EncodingEventSubscriber::GetRelativeRtpTimestamp(RtpTimeTicks rtp_timestamp)
{
