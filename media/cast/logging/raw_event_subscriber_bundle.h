...
public:
    RawEventSubscriberBundleForStream(const scoped_refptr<CastEnvironment>& cast_environment, bool is_audio, ReceiverTimeOffsetEstimator* offset_estimator);

    RawEventSubscriberBundleForStream(const RawEventSubscriberBundleForStream&) = delete;
    RawEventSubscriberBundleForStream& operator=(const RawEventSubscriberBundleForStream&) = delete;
...
public:
    explicit RawEventSubscriberBundle(const scoped_refptr<CastEnvironment>& cast_environment);
...
private:
    ...
    std::map<bool, std::unique_ptr<RawEventSubscriberBundleForStream>> subscribers_;
