...
{
    return event == FRAME_DECODED || event == FRAME_PLAYOUT || event == FRAME_ACK_SENT || event == PACKET_RECEIVED;
}

} // namespace

StatsEventSubscriber::SimpleHistogram::SimpleHistogram(int64_t min, int64_t max, int64_t width)
    : min_(min)
    , max_(max)
    , width_(width)
...
base::Value::List StatsEventSubscriber::SimpleHistogram::GetHistogram() const
{
    ...
    for (size_t i = 1; i < buckets_.size() - 1; i++) {
        ...
        bucket.Set(base::StringPrintf("%" PRId64 "-%" PRId64, lower, upper), buckets_[i]);
        histo.Append(std::move(bucket));
    }
...
StatsEventSubscriber::StatsEventSubscriber(EventMediaType event_media_type, const base::TickClock* clock, ReceiverTimeOffsetEstimator* offset_estimator)
...
void StatsEventSubscriber::OnReceivePacketEvent(const PacketEvent& packet_event)
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
void StatsEventSubscriber::UpdateFirstLastEventTime(base::TimeTicks timestamp, bool is_receiver_event)
{
    if (is_receiver_event) {
...
base::Value::Dict StatsEventSubscriber::GetStats() const
{
    ...
    for (StatsMap::const_iterator it = stats_map.begin(); it != stats_map.end(); ++it) {
    ...
    ret.Set(event_media_type_ == AUDIO_EVENT ? StatsEventSubscriber::kAudioStatsDictKey : StatsEventSubscriber::kVideoStatsDictKey, std::move(stats));
...
{
    histograms_[E2E_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kLargeMaxLatencyBucketMs, kLargeBucketWidthMs);
    histograms_[QUEUEING_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kDefaultMaxLatencyBucketMs, kDefaultBucketWidthMs);
    histograms_[NETWORK_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kDefaultMaxLatencyBucketMs, kDefaultBucketWidthMs);
    histograms_[PACKET_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kDefaultMaxLatencyBucketMs, kDefaultBucketWidthMs);
    histograms_[FRAME_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kDefaultMaxLatencyBucketMs, kDefaultBucketWidthMs);
    histograms_[LATE_FRAME_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kDefaultMaxLatencyBucketMs, kDefaultBucketWidthMs);
    histograms_[CAPTURE_LATENCY_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kSmallMaxLatencyBucketMs, kSmallBucketWidthMs);
    histograms_[ENCODE_TIME_MS_HISTO] = std::make_unique<SimpleHistogram>(0, kSmallMaxLatencyBucketMs, kSmallBucketWidthMs);
}
...
void StatsEventSubscriber::GetStatsInternal(StatsMap* stats_map) const
{
    ...
    PopulatePacketBitrateStat(end_time, PACKET_SENT_TO_NETWORK, TRANSMISSION_KBPS, stats_map);
    PopulatePacketBitrateStat(end_time, PACKET_RETRANSMITTED, RETRANSMISSION_KBPS, stats_map);
    ...
    PopulatePacketCountStat(PACKET_RETRANSMITTED, NUM_PACKETS_RETRANSMITTED, stats_map);
    PopulatePacketCountStat(PACKET_RECEIVED, NUM_PACKETS_RECEIVED, stats_map);
    PopulatePacketCountStat(PACKET_RTX_REJECTED, NUM_PACKETS_RTX_REJECTED, stats_map);
    ...
    if (capture_latency_datapoints_ > 0) {
        ...
        stats_map->insert(std::make_pair(AVG_CAPTURE_LATENCY_MS, avg_capture_latency_ms));
    }
    ...
    if (queueing_latency_datapoints_ > 0) {
        ...
        stats_map->insert(std::make_pair(AVG_QUEUEING_LATENCY_MS, avg_queueing_latency_ms));
    }
    ...
    if (network_latency_datapoints_ > 0) {
        ...
        stats_map->insert(std::make_pair(AVG_NETWORK_LATENCY_MS, avg_network_latency_ms));
    }
    ...
    if (packet_latency_datapoints_ > 0) {
        ...
        stats_map->insert(std::make_pair(AVG_PACKET_LATENCY_MS, avg_packet_latency_ms));
    }

    if (frame_latency_datapoints_ > 0) {
        double avg_frame_latency_ms = total_frame_latency_.InMillisecondsF() / frame_latency_datapoints_;
        stats_map->insert(std::make_pair(AVG_FRAME_LATENCY_MS, avg_frame_latency_ms));
    }
    ...
    if (!last_response_received_time_.is_null()) {
        stats_map->insert(std::make_pair(MS_SINCE_LAST_RECEIVER_RESPONSE, (end_time - last_response_received_time_).InMillisecondsF()));
    }

    stats_map->insert(std::make_pair(NUM_FRAMES_DROPPED_BY_ENCODER, num_frames_dropped_by_encoder_));
    stats_map->insert(std::make_pair(NUM_FRAMES_LATE, num_frames_late_));
    if (!first_event_time_.is_null()) {
        stats_map->insert(std::make_pair(FIRST_EVENT_TIME_MS, (first_event_time_ - base::TimeTicks::UnixEpoch()).InMillisecondsF()));
    }
    if (!last_event_time_.is_null()) {
        stats_map->insert(std::make_pair(LAST_EVENT_TIME_MS, (last_event_time_ - base::TimeTicks::UnixEpoch()).InMillisecondsF()));
    }
}

StatsEventSubscriber::SimpleHistogram* StatsEventSubscriber::GetHistogramForTesting(CastStat stats) const
{
...
bool StatsEventSubscriber::GetReceiverOffset(base::TimeDelta* offset)
{
    ...
    if (!offset_estimator_->GetReceiverOffsetBounds(&receiver_offset_lower_bound, &receiver_offset_upper_bound)) {
        return false;
    }
...
void StatsEventSubscriber::MaybeInsertFrameInfo(RtpTimeTicks rtp_timestamp, const FrameInfo& frame_info)
{
...
void StatsEventSubscriber::RecordFrameCaptureTime(const FrameEvent& frame_event)
{
...
void StatsEventSubscriber::UpdateLastResponseTime(base::TimeTicks receiver_time)
{
    base::TimeDelta receiver_offset;
    if (!GetReceiverOffset(&receiver_offset))
        return;
...
void StatsEventSubscriber::ErasePacketSentTime(const PacketEvent& packet_event)
{
    std::pair<RtpTimeTicks, uint16_t> key(std::make_pair(packet_event.rtp_timestamp, packet_event.packet_id));
    packet_sent_times_.erase(key);
}

void StatsEventSubscriber::RecordPacketRelatedLatencies(const PacketEvent& packet_event)
{
    ...
    if (it == packet_sent_times_.end()) {
        ...
        if (match) {
            ...
            histograms_[NETWORK_LATENCY_MS_HISTO]->Add(latency_delta.InMillisecondsF());
...
void StatsEventSubscriber::PopulateFpsStat(base::TimeTicks end_time, CastLoggingEvent event, CastStat stat, StatsMap* stats_map) const
{
...
void StatsEventSubscriber::PopulateFrameCountStat(CastLoggingEvent event, CastStat stat, StatsMap* stats_map) const
{
    auto it = frame_stats_.find(event);
    if (it != frame_stats_.end()) {
...
void StatsEventSubscriber::PopulatePacketCountStat(CastLoggingEvent event, CastStat stat, StatsMap* stats_map) const
{
...
void StatsEventSubscriber::PopulateFrameBitrateStat(base::TimeTicks end_time, StatsMap* stats_map) const
{
...
void StatsEventSubscriber::PopulatePacketBitrateStat(base::TimeTicks end_time, CastLoggingEvent event, CastStat stat, StatsMap* stats_map) const
{
    auto it = packet_stats_.find(event);
    if (it != packet_stats_.end()) {
        double kbps = 0;
