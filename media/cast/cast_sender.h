...
public:
    ...
    virtual void InsertRawVideoFrame(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks capture_time) = 0;
    ...
    virtual scoped_refptr<VideoFrame> MaybeCreateOptimizedFrame(const gfx::Size& frame_size, base::TimeDelta timestamp) = 0;
...
public:
    ...
    // and sent to the network.
    virtual void InsertAudio(std::unique_ptr<AudioBus> audio_bus, const base::TimeTicks& recorded_time) = 0;
...
class CastSender {
public:
    static std::unique_ptr<CastSender> Create(scoped_refptr<CastEnvironment> cast_environment, CastTransport* const transport_sender);
    ...
    virtual void InitializeAudio(const FrameSenderConfig& audio_config, StatusChangeOnceCallback status_change_cb) = 0;
    ...
    virtual void SetTargetPlayoutDelay(base::TimeDelta new_target_playout_delay) = 0;
};
