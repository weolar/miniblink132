...
public:
    ...
    CastEnvironment(const base::TickClock* clock, scoped_refptr<base::SingleThreadTaskRunner> main_thread_proxy,
        scoped_refptr<base::SingleThreadTaskRunner> audio_thread_proxy, scoped_refptr<base::SingleThreadTaskRunner> video_thread_proxy);
    ...
    bool PostTask(ThreadId identifier, const base::Location& from_here, base::OnceClosure task);

    bool PostDelayedTask(ThreadId identifier, const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay);
    ...
    LogEventDispatcher* logger()
    {
        return &logger_;
    }

    scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(ThreadId identifier) const;

    bool HasAudioThread()
    {
        return audio_thread_proxy_.get() ? true : false;
    }

    bool HasVideoThread()
    {
        return video_thread_proxy_.get() ? true : false;
    }
