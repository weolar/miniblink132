...
public:
    ...
    {
        absl::optional<DataStoreId> data_store_id = GetDataStoreIdForPath(std::move(storage_path), std::move(create_fetcher_callback));
        if (!data_store_id) {
...
private:
    ...
    absl::optional<DataStoreId> GetDataStoreIdForPath(base::FilePath storage_path, CreateFetcherCB create_fetcher_callback)
    {
        ...
        auto provisioning_fetcher = std::make_unique<ProvisioningFetcherImpl>(std::move(create_fetcher_callback));
    ...
    void OnProvisioningFetcherError(ProvisioningFetcherImpl* provisioning_fetcher)
    {
    ...
    // use.
    base::flat_set<std::unique_ptr<ProvisioningFetcherImpl>, base::UniquePtrComparator> provisioning_fetchers_;
...
FuchsiaCdmManager::FuchsiaCdmManager(
    CreateKeySystemCallbackMap create_key_system_callbacks_by_name, base::FilePath cdm_data_path, absl::optional<uint64_t> cdm_data_quota_bytes)
