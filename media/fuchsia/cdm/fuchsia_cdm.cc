...
fuchsia::media::drm::LicenseServerMessage CreateLicenseServerMessage(const std::vector<uint8_t>& response)
{
    ...
    message.message
        = base::MemBufferFromString(base::StringPiece(reinterpret_cast<const char*>(response.data()), response.size()), "cr-drm-license-server-message");
    return message;
}
...
public:
    using ResultCB = base::OnceCallback<void(absl::optional<CdmPromise::Exception>)>;
...
private:
    ...
    void OnLicenseMessageGenerated(fuchsia::media::drm::LicenseMessage message)
    {
        ...
        absl::optional<std::string> session_msg = base::StringFromMemBuffer(message.message);
    ...
    {
        DCHECK(result_cb_);
        std::move(result_cb_).Run(result.is_err() ? absl::make_optional(ToCdmPromiseException(result.err())) : absl::nullopt);
    }
    ...
    , decryptor_(this)
{
    ...
    cdm_.set_error_handler([this](zx_status_t status) {
        ...
        if (ready_cb_) {
            std::move(ready_cb_).Run(false, "ContentDecryptionModule closed prior to being ready");
        }
    });
}
...
void FuchsiaCdm::OnGenerateLicenseRequestStatus(CdmSession* session, uint32_t promise_id, absl::optional<CdmPromise::Exception> exception)
{
    DCHECK(session);
...
void FuchsiaCdm::OnProcessLicenseServerMessageStatus(const std::string& session_id, uint32_t promise_id, absl::optional<CdmPromise::Exception> exception)
{
    if (exception.has_value()) {
...
void FuchsiaCdm::OnGenerateLicenseReleaseStatus(const std::string& session_id, uint32_t promise_id, absl::optional<CdmPromise::Exception> exception)
{
    if (exception.has_value()) {
