...
std::vector<fuchsia::media::SubsampleEntry> GetSubsamples(const std::vector<SubsampleEntry>& subsamples)
{
    std::vector<fuchsia::media::SubsampleEntry> fuchsia_subsamples(subsamples.size());
...
fuchsia::media::EncryptionPattern GetEncryptionPattern(EncryptionPattern pattern)
{
...
fuchsia::media::FormatDetails GetClearFormatDetails()
{
    ...
    encrypted_format.set_scheme(fuchsia::media::ENCRYPTION_SCHEME_UNENCRYPTED).set_subsamples({}).set_init_vector({});
...
fuchsia::media::FormatDetails GetEncryptedFormatDetails(const DecryptConfig* config)
{
    DCHECK(config);

    fuchsia::media::EncryptedFormat encrypted_format;
    encrypted_format.set_scheme(GetEncryptionScheme(config->encryption_scheme()))
        .set_key_id(std::vector<uint8_t>(config->key_id().begin(), config->key_id().end()))
        .set_init_vector(std::vector<uint8_t>(config->iv().begin(), config->iv().end()))
    ...
    if (config->encryption_scheme() == EncryptionScheme::kCbcs) {
        ...
        encrypted_format.set_pattern(GetEncryptionPattern(config->encryption_pattern().value()));
    }
...
FuchsiaStreamDecryptor::FuchsiaStreamDecryptor(fuchsia::media::StreamProcessorPtr processor)
...
base::RepeatingClosure FuchsiaStreamDecryptor::GetOnNewKeyClosure()
{
    ...
    return BindToCurrentLoop(base::BindRepeating(&FuchsiaStreamDecryptor::OnNewKey, weak_factory_.GetWeakPtr()));
}

void FuchsiaStreamDecryptor::Initialize(Sink* sink, size_t min_buffer_size, size_t min_buffer_count)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    input_buffer_collection_->CreateSharedToken(base::BindOnce(&StreamProcessorHelper::SetInputBufferCollectionToken, base::Unretained(&processor_)));
    auto buffer_constraints = VmoBuffer::GetRecommendedConstraints(kInputBufferCount, min_buffer_size_, /*writable=*/true);
    input_buffer_collection_->Initialize(std::move(buffer_constraints), "CrFuchsiaStreamDecryptor");
    input_buffer_collection_->AcquireBuffers(base::BindOnce(&FuchsiaStreamDecryptor::OnInputBuffersAcquired, base::Unretained(this)));
}

void FuchsiaStreamDecryptor::EnqueueBuffer(scoped_refptr<DecoderBuffer> buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FuchsiaStreamDecryptor::OnStreamProcessorAllocateOutputBuffers(const fuchsia::media::StreamBufferConstraints& stream_constraints)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    output_buffer_collection_ = allocator_.AllocateNewCollection();
    output_buffer_collection_->CreateSharedToken(base::BindOnce(&StreamProcessorHelper::CompleteOutputBuffersAllocation, base::Unretained(&processor_)));
    output_buffer_collection_->CreateSharedToken(base::BindOnce(&Sink::OnSysmemBufferStreamBufferCollectionToken, base::Unretained(sink_)));
    ...
    buffer_constraints.buffer_memory_constraints.min_size_bytes = min_buffer_size_;
    ...
    buffer_constraints.buffer_memory_constraints.inaccessible_domain_supported = true;

    output_buffer_collection_->Initialize(std::move(buffer_constraints), "CrFuchsiaStreamDecryptorOutput");
}
...
void FuchsiaStreamDecryptor::OnStreamProcessorOutputFormat(fuchsia::media::StreamOutputFormat format)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

void FuchsiaStreamDecryptor::OnStreamProcessorOutputPacket(StreamProcessorHelper::IoPacket packet)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FuchsiaStreamDecryptor::OnInputBuffersAcquired(std::vector<VmoBuffer> buffers, const fuchsia::sysmem::SingleBufferSettings&)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    input_writer_queue_.Start(std::move(buffers), base::BindRepeating(&FuchsiaStreamDecryptor::SendInputPacket, base::Unretained(this)),
        base::BindRepeating(&FuchsiaStreamDecryptor::ProcessEndOfStream, base::Unretained(this)));
}

void FuchsiaStreamDecryptor::SendInputPacket(const DecoderBuffer* buffer, StreamProcessorHelper::IoPacket packet)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    fuchsia::media::FormatDetails format = (buffer->decrypt_config()) ? GetEncryptedFormatDetails(buffer->decrypt_config()) : GetClearFormatDetails();
