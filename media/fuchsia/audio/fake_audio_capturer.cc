...
FakeAudioCapturer::FakeAudioCapturer(fidl::InterfaceRequest<fuchsia::media::AudioCapturer> request)
    : binding_(this)
{
    if (request)
...
void FakeAudioCapturer::Bind(fidl::InterfaceRequest<fuchsia::media::AudioCapturer> request)
{
...
void FakeAudioCapturer::SetPcmStreamType(fuchsia::media::AudioStreamType stream_type)
{
    EXPECT_TRUE(!stream_type_.has_value());
    EXPECT_EQ(stream_type.sample_format, fuchsia::media::AudioSampleFormat::FLOAT);
...
void FakeAudioCapturer::ProducePackets()
{
    ...
    SendData(start_timestamp_ + base::Seconds(1) * packet_index_ * frames_per_packet_ / stream_type_->frames_per_second, data.data());
    packet_index_++;
    timer_.Start(FROM_HERE, start_timestamp_ + base::Seconds(1) * packet_index_ * frames_per_packet_ / stream_type_->frames_per_second - base::TimeTicks::Now(),
        this, &FakeAudioCapturer::ProducePackets);
}

FakeAudioCapturerFactory::FakeAudioCapturerFactory(sys::OutgoingDirectory* outgoing_directory)
...
void FakeAudioCapturerFactory::CreateAudioCapturer(fidl::InterfaceRequest<fuchsia::media::AudioCapturer> request, bool loopback)
{
