...
public:
    ...
    public:
        ...
        virtual void OnSysmemBufferStreamBufferCollectionToken(fuchsia::sysmem2::BufferCollectionTokenPtr token) = 0;

        // Called when a packet has been processed. The client should drop the
        // |packet| only after it's finished using it.
        virtual void OnSysmemBufferStreamOutputPacket(StreamProcessorHelper::IoPacket packet) = 0;
    ...
    // buffer collection.
    virtual void Initialize(Sink* sink, size_t min_buffer_size, size_t min_buffer_count) = 0;
