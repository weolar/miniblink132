...
DecryptingSysmemBufferStream::DecryptingSysmemBufferStream(SysmemAllocatorClient* sysmem_allocator, CdmContext* cdm_context, Decryptor::StreamType stream_type)
    ...
    , stream_type_(stream_type)
{
    ...
    event_cb_registration_ = cdm_context->RegisterEventCB(base::BindRepeating(&DecryptingSysmemBufferStream::OnCdmContextEvent, weak_factory_.GetWeakPtr()));
}

DecryptingSysmemBufferStream::~DecryptingSysmemBufferStream() = default;

void DecryptingSysmemBufferStream::Initialize(Sink* sink, size_t min_buffer_size, size_t min_buffer_count)
{
    sink_ = sink;
    passthrough_stream_.Initialize(sink, min_buffer_size, min_buffer_count);
}

void DecryptingSysmemBufferStream::EnqueueBuffer(scoped_refptr<DecoderBuffer> buffer)
{
...
void DecryptingSysmemBufferStream::DecryptNextBuffer()
{
    ...
    decryptor_->Decrypt(stream_type_, buffer_queue_.front(), base::BindOnce(&DecryptingSysmemBufferStream::OnBufferDecrypted, weak_factory_.GetWeakPtr()));
}

void DecryptingSysmemBufferStream::OnBufferDecrypted(Decryptor::Status status, scoped_refptr<DecoderBuffer> decrypted_buffer)
{
