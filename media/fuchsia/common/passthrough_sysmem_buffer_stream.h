...
public:
    explicit PassthroughSysmemBufferStream(SysmemAllocatorClient* sysmem_allocator);
    ~PassthroughSysmemBufferStream() override;

    PassthroughSysmemBufferStream(const PassthroughSysmemBufferStream&) = delete;
    PassthroughSysmemBufferStream& operator=(const PassthroughSysmemBufferStream&) = delete;
...
private:
    ...
    void ProcessOutputPacket(const DecoderBuffer* buffer, StreamProcessorHelper::IoPacket packet);
