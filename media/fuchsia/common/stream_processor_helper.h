...
public:
    ...
    public:
        IoPacket(size_t index, size_t offset, size_t size, base::TimeDelta timestamp, bool unit_end, bool key_frame, base::OnceClosure destroy_cb);
        ...
        const fuchsia::media::FormatDetails& format() const
        {
            return format_;
        }
    ...
    public:
        ...
        // StreamProcessor.
        virtual void OnStreamProcessorAllocateInputBuffers(const fuchsia::media::StreamBufferConstraints& stream_constraints)
        {
        }
        ...
        virtual void OnStreamProcessorAllocateOutputBuffers(const fuchsia::media::StreamBufferConstraints& stream_constraints) = 0;
        ...
        virtual void OnStreamProcessorOutputFormat(fuchsia::media::StreamOutputFormat format) = 0;
    ...
    StreamProcessorHelper(fuchsia::media::StreamProcessorPtr processor, Client* client);
    ...
    void SetInputBufferCollectionToken(fuchsia::sysmem2::BufferCollectionTokenPtr token);
    ...
    void CompleteOutputBuffersAllocation(fuchsia::sysmem2::BufferCollectionTokenPtr token);
...
private:
    ...
    void OnOutputConstraints(fuchsia::media::StreamOutputConstraints output_constraints);
    void OnOutputFormat(fuchsia::media::StreamOutputFormat output_format);
    void OnOutputPacket(fuchsia::media::Packet output_packet, bool error_detected_before, bool error_detected_during);
    void OnOutputEndOfStream(uint64_t stream_lifetime_ordinal, bool error_detected_before);

    void OnError();

    void OnRecycleOutputBuffer(uint64_t buffer_lifetime_ordinal, uint32_t packet_index);
