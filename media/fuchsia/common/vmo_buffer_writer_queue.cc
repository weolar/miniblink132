...
VmoBufferWriterQueue::PendingBuffer::PendingBuffer(scoped_refptr<DecoderBuffer> buffer)
    : buffer(buffer)
{
    DCHECK(buffer);
}
...
VmoBufferWriterQueue::PendingBuffer::PendingBuffer(PendingBuffer&& other) = default;
...
void VmoBufferWriterQueue::Start(std::vector<VmoBuffer> buffers, SendPacketCB send_packet_cb, EndOfStreamCB end_of_stream_cb)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void VmoBufferWriterQueue::PumpPackets()
{
    ...
    while (!buffers_.empty() && !is_paused_ && input_queue_position_ < pending_buffers_.size()) {
        ...
        size_t bytes_filled = buffers_[buffer_index].Write(base::make_span(current_buffer->data(), current_buffer->bytes_left()));
        current_buffer->AdvanceCurrentPos(bytes_filled);

        bool buffer_end = current_buffer->bytes_left() == 0;

        StreamProcessorHelper::IoPacket packet(buffer_index, /*offset=*/0, bytes_filled, current_buffer->buffer->timestamp(), buffer_end, /*key_frame=*/false,
            base::BindOnce(&VmoBufferWriterQueue::ReleaseBuffer, weak_factory_.GetWeakPtr(), buffer_index));
