...
FuchsiaDecoderFactory::FuchsiaDecoderFactory(mojo::PendingRemote<media::mojom::FuchsiaMediaCodecProvider> resource_provider, bool allow_overlays)
...
void FuchsiaDecoderFactory::CreateVideoDecoders(scoped_refptr<base::SequencedTaskRunner> task_runner, GpuVideoAcceleratorFactories* gpu_factories,
    ...
    std::vector<std::unique_ptr<VideoDecoder>>* video_decoders)
{
    if (gpu_factories && gpu_factories->IsGpuVideoDecodeAcceleratorEnabled()) {
        ...
        if (context_provider) {
            video_decoders->push_back(std::make_unique<FuchsiaVideoDecoder>(context_provider, resource_provider_, allow_overlays_));
        } else {
