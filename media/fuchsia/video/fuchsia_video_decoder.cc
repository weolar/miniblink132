...
{
    std::string min_buffer_size_arg = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kMinVideoDecoderOutputBufferSize);
    if (min_buffer_size_arg.empty())
        return std::nullopt;
    size_t width;
    size_t height;
    if (sscanf(min_buffer_size_arg.c_str(), "%zux%zu" SCNu32, &width, &height) != 2) {
        LOG(WARNING) << "Invalid value for --" << switches::kMinVideoDecoderOutputBufferSize << ": '" << min_buffer_size_arg << "'";
        return std::nullopt;
    }
...
public:
    OutputMailbox(scoped_refptr<viz::RasterContextProvider> raster_context_provider, gfx::GpuMemoryBufferHandle gmb_handle, gfx::Size& size,
        viz::SharedImageFormat& format, gfx::ClientNativePixmapFactory* pixmap_factory, const gfx::ColorSpace& color_space)
        : raster_context_provider_(raster_context_provider)
        , size_(size)
        , weak_factory_(this)
    {
        gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_VIDEO_DECODE;

        // Note that the shared image prefers external sampler.
        format.SetPrefersExternalSampler();
        shared_image_ = raster_context_provider_->SharedImageInterface()->CreateSharedImage(
            { format, size, color_space, usage, "FuchsiaVideoDecoder" }, std::move(gmb_handle));

        create_sync_token_ = raster_context_provider_->SharedImageInterface()->GenVerifiedSyncToken();
    }
    ...
    ~OutputMailbox()
    {
        raster_context_provider_->SharedImageInterface()->DestroySharedImage(release_sync_token_, std::move(shared_image_));
    }
    ...
    scoped_refptr<VideoFrame> CreateFrame(VideoPixelFormat pixel_format, const gfx::Size& coded_size, const gfx::Rect& visible_rect,
        const gfx::Size& natural_size, base::TimeDelta timestamp, base::OnceClosure reuse_callback)
    {
        DCHECK(!is_used_);
        is_used_ = true;
        reuse_callback_ = std::move(reuse_callback);

        auto frame = VideoFrame::WrapSharedImage(pixel_format, shared_image_, create_sync_token_,
            base::BindPostTaskToCurrentDefault(base::BindOnce(&OutputMailbox::OnFrameDestroyed, base::Unretained(this))), coded_size, visible_rect,
...
private:
    void OnFrameDestroyed(const gpu::SyncToken& sync_token)
    {
        ...
        raster_context_provider_->ContextSupport()->SignalSyncToken(
            release_sync_token_, base::BindPostTaskToCurrentDefault(base::BindOnce(&OutputMailbox::OnSyncTokenSignaled, weak_factory_.GetWeakPtr())));
    }
...
FuchsiaVideoDecoder::FuchsiaVideoDecoder(scoped_refptr<viz::RasterContextProvider> raster_context_provider,
    const mojo::SharedRemote<media::mojom::FuchsiaMediaCodecProvider>& media_codec_provider, bool allow_overlays)
    : raster_context_provider_(raster_context_provider)
    ...
    const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb, const WaitingCB& waiting_cb)
{
    ...
    if (decoder_ && current_config_.codec() == config.codec() && current_config_.is_encrypted() == config.is_encrypted()) {
    ...
    DecoderStatus status = InitializeSysmemBufferStream(config.is_encrypted(), cdm_context, &secure_input);
    ...
    media::mojom::VideoDecoderSecureMemoryMode secure_mode = media::mojom::VideoDecoderSecureMemoryMode::CLEAR;
    if (secure_input) {
        if (!use_overlays_for_video_) {
            DLOG(ERROR) << "Protected content can be rendered only using overlays.";
            std::move(done_callback).Run(DecoderStatus(DecoderStatus::Codes::kUnsupportedConfig, FROM_HERE));
            return;
        }
        secure_mode = media::mojom::VideoDecoderSecureMemoryMode::SECURE;
    } else if (use_overlays_for_video_ && base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceProtectedVideoOutputBuffers)) {
        secure_mode = media::mojom::VideoDecoderSecureMemoryMode::SECURE_OUTPUT;
    }
    protected_output_ = secure_mode != media::mojom::VideoDecoderSecureMemoryMode::CLEAR;

    // Reset output buffers since we won't be able to re-use them.
    ReleaseOutputBuffers();

    fuchsia::media::StreamProcessorPtr decoder;
    media_codec_provider_->CreateVideoDecoder(config.codec(), secure_mode, decoder.NewRequest());
...
void FuchsiaVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer, DecodeCB decode_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FuchsiaVideoDecoder::Reset(base::OnceClosure closure)
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(closure));
}
...
void FuchsiaVideoDecoder::SetClientNativePixmapFactoryForTests(std::unique_ptr<gfx::ClientNativePixmapFactory> factory)
{
    client_native_pixmap_factory_ = std::move(factory);
}

DecoderStatus FuchsiaVideoDecoder::InitializeSysmemBufferStream(bool is_encrypted, CdmContext* cdm_context, bool* out_secure_mode)
{
    ...
    *out_secure_mode = false;
    ...
    if (is_encrypted) {
        ...
        if (fuchsia_cdm) {
            *out_secure_mode = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableProtectedVideoBuffers);
            sysmem_buffer_stream_ = fuchsia_cdm->CreateStreamDecryptor(*out_secure_mode);
            ...
            sysmem_buffer_stream_ = std::make_unique<DecryptingSysmemBufferStream>(&sysmem_allocator_, cdm_context, Decryptor::kVideo);
        }
    } else {
        sysmem_buffer_stream_ = std::make_unique<PassthroughSysmemBufferStream>(&sysmem_allocator_);
    }
...
void FuchsiaVideoDecoder::OnSysmemBufferStreamBufferCollectionToken(fuchsia::sysmem2::BufferCollectionTokenPtr token)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(decoder_);
...
void FuchsiaVideoDecoder::OnSysmemBufferStreamOutputPacket(StreamProcessorHelper::IoPacket packet)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    packet.AddOnDestroyClosure(base::BindOnce(&FuchsiaVideoDecoder::CallNextDecodeCallback, decode_callbacks_weak_factory_.GetWeakPtr()));
...
void FuchsiaVideoDecoder::OnStreamProcessorAllocateOutputBuffers(const fuchsia::media::StreamBufferConstraints& output_constraints)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    output_buffer_collection_->CreateSharedToken(
        base::BindOnce(&StreamProcessorHelper::CompleteOutputBuffersAllocation, base::Unretained(decoder_.get())), "codec");
    output_buffer_collection_->CreateSharedToken(base::BindOnce(&FuchsiaVideoDecoder::SetBufferCollectionTokenForGpu, base::Unretained(this)), "gpu");
    ...
    constraints.set_min_buffer_count_for_shared_slack(kMaxUsedOutputBuffers - kOutputBuffersForCamping);

    for (size_t pixel_format_index = 0; pixel_format_index < std::size(kSupportedPixelFormats); ++pixel_format_index) {
        auto& image_constraints = constraints.mutable_image_format_constraints()->emplace_back();
        image_constraints.set_pixel_format(kSupportedPixelFormats[pixel_format_index]);
    ...
    if (min_buffer_size) {
        for (size_t pixel_format_index = 0; pixel_format_index < std::size(kSupportedPixelFormats); ++pixel_format_index) {
            auto& image_constraints = constraints.mutable_image_format_constraints()->at(pixel_format_index);
            image_constraints.set_required_max_size(
                fuchsia::math::SizeU { static_cast<uint32_t>(min_buffer_size->width()), static_cast<uint32_t>(min_buffer_size->height()) });
        }
    }

    output_buffer_collection_->Initialize(std::move(constraints), "ChromiumVideoDecoderOutput");
}
...
void FuchsiaVideoDecoder::OnStreamProcessorOutputFormat(fuchsia::media::StreamOutputFormat output_format)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    auto* format = output_format.mutable_format_details();
    if (!format->has_domain() || !format->domain().is_video() || !format->domain().video().is_uncompressed()) {
...
void FuchsiaVideoDecoder::OnStreamProcessorOutputPacket(StreamProcessorHelper::IoPacket output_packet)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    // We can safely cast from fuchsia.sysmem.PixelFormatType to
    // fuchsia.images2.PixelFormat.
    auto sysmem_pixel_format = static_cast<fuchsia::images2::PixelFormat>(fidl::ToUnderlying(output_format_.image_format.pixel_format.type));
    ...
    default:
        DLOG(ERROR) << "Unsupported pixel format: " << static_cast<int>(sysmem_pixel_format);
        OnError();
        return;
    }
    ...
    auto coded_size = gfx::Size(output_format_.primary_width_pixels, output_format_.primary_height_pixels);

    if (output_mailboxes_[buffer_index] && output_mailboxes_[buffer_index]->size() != coded_size) {
    ...
    if (!output_mailboxes_[buffer_index]) {
        ...
        auto status = output_buffer_collection_handle_.duplicate(ZX_RIGHT_SAME_RIGHTS, &gmb_handle.native_pixmap_handle.buffer_collection_handle);
        ZX_DCHECK(status == ZX_OK, status);
        gmb_handle.native_pixmap_handle.buffer_index = buffer_index;

        output_mailboxes_[buffer_index] = new OutputMailbox(raster_context_provider_, std::move(gmb_handle), coded_size, si_format,
            client_native_pixmap_factory_.get(), current_config_.color_space_info().ToGfxColorSpace());
    } else {
        raster_context_provider_->SharedImageInterface()->UpdateSharedImage(gpu::SyncToken(), output_mailboxes_[buffer_index]->mailbox());
    }

    auto display_rect = gfx::Rect(output_format_.primary_display_width_pixels, output_format_.primary_display_height_pixels);

    VideoAspectRatio aspect_ratio = current_config_.aspect_ratio();
    if (!aspect_ratio.IsValid() && output_format_.has_pixel_aspect_ratio) {
        aspect_ratio = VideoAspectRatio::PAR(output_format_.pixel_aspect_ratio_width, output_format_.pixel_aspect_ratio_height);
    }
    ...
    auto frame = output_mailboxes_[buffer_index]->CreateFrame(pixel_format, coded_size, display_rect, aspect_ratio.GetNaturalSize(display_rect), timestamp,
        base::BindOnce(&FuchsiaVideoDecoder::ReleaseOutputPacket, base::Unretained(this), std::move(output_packet)));

    VkSamplerYcbcrModelConversion ycbcr_conversion = (current_config_.color_space_info().matrix == VideoColorSpace::MatrixID::BT709)
    ...
    // passed to Skia.
    frame->set_ycbcr_info(gpu::VulkanYCbCrInfo(vk_format, /*external_format=*/0, ycbcr_conversion, VK_SAMPLER_YCBCR_RANGE_ITU_NARROW,
        VK_CHROMA_LOCATION_COSITED_EVEN, VK_CHROMA_LOCATION_COSITED_EVEN, /*format_features=*/0));
...
void FuchsiaVideoDecoder::SetBufferCollectionTokenForGpu(fuchsia::sysmem2::BufferCollectionTokenPtr token)
{
    // Register the new collection with the GPU process.
    DCHECK(!output_buffer_collection_handle_);

    zx::eventpair service_handle;
    auto status = zx::eventpair::create(0, &output_buffer_collection_handle_, &service_handle);
    ZX_DCHECK(status == ZX_OK, status);
    raster_context_provider_->SharedImageInterface()->RegisterSysmemBufferCollection(std::move(service_handle), token.Unbind().TakeChannel(),
        viz::MultiPlaneFormat::kNV12, gfx::BufferUsage::GPU_READ, use_overlays_for_video_ /*register_with_image_pipe*/);
...
void FuchsiaVideoDecoder::ReleaseOutputPacket(StreamProcessorHelper::IoPacket output_packet)
{
