...
class TestRasterContextProvider : public base::RefCountedThreadSafe<TestRasterContextProvider>, public viz::RasterContextProvider {
public:
    TestRasterContextProvider()
        : shared_image_interface_(base::MakeRefCounted<gpu::TestSharedImageInterface>())
    {
    }
...
class TestFuchsiaMediaCodecProvider : public media::mojom::FuchsiaMediaCodecProvider {
public:
    // media::mojom::FuchsiaMediaCodecProvider implementation.
    void CreateVideoDecoder(media::VideoCodec codec, media::mojom::VideoDecoderSecureMemoryMode secure_mode,
        fidl::InterfaceRequest<fuchsia::media::StreamProcessor> stream_processor_request) final
    {
        EXPECT_TRUE(secure_mode == media::mojom::VideoDecoderSecureMemoryMode::CLEAR);
        ...
        default:
            ADD_FAILURE() << "CreateVideoDecoder() called with unexpected codec: " << static_cast<int>(codec);
            return;
        }
        ...
        auto decoder_factory = base::ComponentContextForProcess()->svc()->Connect<fuchsia::mediacodec::CodecFactory>();
        decoder_factory->CreateDecoder(std::move(decoder_params), std::move(stream_processor_request));
    }

    void GetSupportedVideoDecoderConfigs(GetSupportedVideoDecoderConfigsCallback callback) override
    {
        ADD_FAILURE();
    }
...
public:
    ...
    std::unique_ptr<gfx::ClientNativePixmap> ImportFromHandle(
        gfx::NativePixmapHandle handle, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage) override
    {
...
public:
    FuchsiaVideoDecoderTest()
        : raster_context_provider_(base::MakeRefCounted<TestRasterContextProvider>())
    {
        auto decoder = std::make_unique<FuchsiaVideoDecoder>(raster_context_provider_.get(),
            mojo::SharedRemote<media::mojom::FuchsiaMediaCodecProvider>(test_media_codec_provider_.receiver_.BindNewPipeAndPassRemote()),
            /*allow_overlays=*/false);
        decoder->SetClientNativePixmapFactoryForTests(std::make_unique<FakeClientNativePixmapFactory>());
    ...
    [[nodiscard]] bool InitializeDecoder(VideoDecoderConfig config)
    {
        ...
        decoder_->Initialize(config, true, /*cdm_context=*/nullptr,
            base::BindRepeating(
                [](bool* init_cb_result, base::RunLoop* run_loop, DecoderStatus status) {
                    *init_cb_result = status.is_ok();
                    run_loop->Quit();
                },
                &init_cb_result, &run_loop),
            base::BindRepeating(&FuchsiaVideoDecoderTest::OnVideoFrame, weak_factory_.GetWeakPtr()), base::DoNothing());
    ...
    {
        base::RunLoop run_loop;
        decoder_->Reset(base::BindRepeating([](base::RunLoop* run_loop) { run_loop->Quit(); }, &run_loop));
        run_loop.Run();
    }
    ...
    {
        decoder_->Decode(buffer, base::BindRepeating(&FuchsiaVideoDecoderTest::OnFrameDecoded, weak_factory_.GetWeakPtr(), num_input_buffers_));
...
{
    static scoped_refptr<DecoderBuffer> frames[] = { ReadTestDataFile("h264-320x180-frame-0"), ReadTestDataFile("h264-320x180-frame-1"),
        ReadTestDataFile("h264-320x180-frame-2"), ReadTestDataFile("h264-320x180-frame-3") };
...
TEST_F(FuchsiaVideoDecoderTest, RasterContextLifetime)
{
    ...
    raster_context_provider_->SetOnDestroyedClosure(base::BindLambdaForTesting([&context_destroyed]() { context_destroyed = true; }));
    ASSERT_TRUE(InitializeDecoder(TestVideoConfig::NormalH264()));
