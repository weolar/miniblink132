...
void FillPlane(uint8_t* data, gfx::Size size, int x_step, int y_step, fuchsia::camera3::Orientation orientation, uint8_t salt)
{
    // First flip X axis for flipped orientation.
    if (orientation == fuchsia::camera3::Orientation::UP_FLIPPED || orientation == fuchsia::camera3::Orientation::DOWN_FLIPPED
        || orientation == fuchsia::camera3::Orientation::RIGHT_FLIPPED || orientation == fuchsia::camera3::Orientation::LEFT_FLIPPED) {
...
void ValidatePlane(const uint8_t* data, gfx::Size size, size_t x_step, size_t y_step, uint8_t salt)
{
    for (int y = 0; y < size.height(); ++y) {
        for (int x = 0; x < size.width(); ++x) {
            SCOPED_TRACE(testing::Message() << "x=" << x << " y=" << y);
            EXPECT_EQ(data[x * x_step + y * y_step], GetTestFrameValue(size, x, y, salt));
        }
    }
}

fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> Sysmem2TokenFromSysmem1Token(
    fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> v1_token)
{
    return fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken>(v1_token.TakeChannel());
}

fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> Sysmem1TokenFromSysmem2Token(
    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> v2_token)
{
    return fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken>(v2_token.TakeChannel());
}
...
// static
void FakeCameraStream::ValidateFrameData(const uint8_t* data, gfx::Size size, uint8_t salt)
{
...
struct FakeCameraStream::Buffer {
    ...
    base::MessagePumpForIO::ZxHandleWatchController release_fence_watch_controller;
};
    ...
    , sysmem_allocator_(base::ComponentContextForProcess()->svc()->Connect<fuchsia::sysmem2::Allocator>())
{
    sysmem_allocator_->SetDebugClientInfo(
        std::move(fuchsia::sysmem2::AllocatorSetDebugClientInfoRequest {}.set_name("ChromiumFakeCameraStream").set_id(base::GetCurrentProcId())));
}

FakeCameraStream::~FakeCameraStream() = default;

void FakeCameraStream::Bind(fidl::InterfaceRequest<fuchsia::camera3::Stream> request)
{
    binding_.Bind(std::move(request));
}

void FakeCameraStream::SetFirstBufferCollectionFailMode(SysmemFailMode fail_mode)
{
...
void FakeCameraStream::SetFakeResolution(gfx::Size resolution)
{
    ...
    resolution_update_ = fuchsia::math::Size { resolution_.width(), resolution_.height() };
    SendResolution();
}

void FakeCameraStream::SetFakeOrientation(fuchsia::camera3::Orientation orientation)
{
...
void FakeCameraStream::ProduceFrame(base::TimeTicks timestamp, uint8_t salt)
{
    ...
    gfx::Size coded_size((resolution_.width() + 1) & ~1, (resolution_.height() + 1) & ~1);

    // Fill Y plane.
    uint8_t* y_plane = reinterpret_cast<uint8_t*>(buffer->mapping.memory());
    size_t stride = kMaxFrameSize.width();
    FillPlane(y_plane, coded_size, /*x_step=*/1, /*y_step=*/stride, orientation_, salt + kYPlaneSalt);

    // Fill UV plane.
    gfx::Size uv_size(coded_size.width() / 2, coded_size.height() / 2);
    uint8_t* uv_plane = y_plane + kMaxFrameSize.width() * kMaxFrameSize.height();
    FillPlane(uv_plane, uv_size, /*x_step=*/2, /*y_step=*/stride, orientation_, salt + kUPlaneSalt);
    FillPlane(uv_plane + 1, uv_size, /*x_step=*/2, /*y_step=*/stride, orientation_, salt + kVPlaneSalt);
    ...
    EXPECT_EQ(zx::eventpair::create(0u, &frame.release_fence, &buffer->release_fence), ZX_OK);

    // Watch release fence to get notified when the frame is released.
    base::CurrentIOThread::Get()->WatchZxHandle(
        buffer->release_fence.get(), /*persistent=*/false, ZX_EVENTPAIR_PEER_CLOSED, &buffer->release_fence_watch_controller, this);
...
void FakeCameraStream::SetBufferCollection(fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_handle_param)
{
    EXPECT_TRUE(token_handle_param);
    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_handle = Sysmem2TokenFromSysmem1Token(std::move(token_handle_param));
    ...
    new_buffer_collection_token_.set_error_handler(fit::bind_member(this, &FakeCameraStream::OnBufferCollectionError));

    // Duplicate the token to access from the stream.
    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_for_client;
    new_buffer_collection_token_->Duplicate(std::move(fuchsia::sysmem2::BufferCollectionTokenDuplicateRequest {}
    ...
    if (first_buffer_collection_fail_mode_ == SysmemFailMode::kFailSync) {
        ...
        new_buffer_collection_token_->Duplicate(
            std::move(fuchsia::sysmem2::BufferCollectionTokenDuplicateRequest {}.set_rights_attenuation_mask(0).set_token_request(failed_token.NewRequest())));
    }

    new_buffer_collection_token_->Sync(
        [this, token_for_client = std::move(token_for_client), failed_token = std::move(failed_token)](
            fuchsia::sysmem2::Node_Sync_Result sync_result) mutable { OnBufferCollectionSyncDone(std::move(token_for_client), std::move(failed_token)); });
}

void FakeCameraStream::WatchBufferCollection(WatchBufferCollectionCallback callback)
{
...
void FakeCameraStream::OnBufferCollectionSyncDone(fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_for_client,
    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> failed_token)
{
    ...
    sysmem_allocator_->BindSharedCollection(std::move(fuchsia::sysmem2::AllocatorBindSharedCollectionRequest {}
                                                          .set_token(std::move(new_buffer_collection_token_))
                                                          .set_buffer_collection_request(buffer_collection_.NewRequest())));

    buffer_collection_.set_error_handler(fit::bind_member(this, &FakeCameraStream::OnBufferCollectionError));
    ...
    image_constraints.mutable_color_spaces()->emplace_back(fuchsia::images2::ColorSpace::REC601_NTSC);
    image_constraints.set_required_max_size(
        fuchsia::math::SizeU { static_cast<uint32_t>(kMaxFrameSize.width()), static_cast<uint32_t>(kMaxFrameSize.height()) });
    ...
    if (first_buffer_collection_fail_mode_ == SysmemFailMode::kFailAllocation) {
        ...
        image_constraints.mutable_color_spaces()->at(0) = fuchsia::images2::ColorSpace::SRGB;
    }

    buffer_collection_->SetConstraints(std::move(fuchsia::sysmem2::BufferCollectionSetConstraintsRequest {}.set_constraints(std::move(constraints))));
    buffer_collection_->WaitForAllBuffersAllocated(fit::bind_member(this, &FakeCameraStream::OnBufferCollectionAllocated));
}
...
void FakeCameraStream::OnBufferCollectionError(zx_status_t status)
{
    if (first_buffer_collection_fail_mode_ != SysmemFailMode::kNone) {
        ...
        sysmem_allocator_->AllocateSharedCollection(
            std::move(fuchsia::sysmem2::AllocatorAllocateSharedCollectionRequest {}.set_token_request(token.NewRequest())));
...
void FakeCameraStream::OnBufferCollectionAllocated(fuchsia::sysmem2::BufferCollection_WaitForAllBuffersAllocated_Result wait_result)
{
    ...
    EXPECT_EQ(buffer_collection_info.settings().image_format_constraints().pixel_format(), fuchsia::images2::PixelFormat::NV12);

    size_t buffer_size = buffer_collection_info.settings().buffer_settings().size_bytes();
    for (size_t i = 0; i < buffer_collection_info.buffers().size(); ++i) {
        ...
        auto region = base::WritableSharedMemoryRegion::Deserialize(base::subtle::PlatformSharedMemoryRegion::Take(
            std::move(*buffer.mutable_vmo()), base::subtle::PlatformSharedMemoryRegion::Mode::kWritable, buffer_size, base::UnguessableToken::Create()));
...
{
    if (!watch_buffer_collection_callback_ || !new_buffer_collection_token_for_client_) {
        return;
    }
    watch_buffer_collection_callback_(Sysmem1TokenFromSysmem2Token(std::move(*new_buffer_collection_token_for_client_)));
...
void FakeCameraStream::OnZxHandleSignalled(zx_handle_t handle, zx_signals_t signals)
{
...
void FakeCameraDevice::SetGetIdentifierHandler(base::RepeatingCallback<void(GetIdentifierCallback)> get_identifier_handler)
{
...
void FakeCameraDevice::GetConfigurations(GetConfigurationsCallback callback)
{
    ...
    configurations[0].streams[0].image_format.pixel_format.type = fuchsia::sysmem::PixelFormatType::NV12;
...
void FakeCameraDevice::ConnectToStream(uint32_t index, fidl::InterfaceRequest<fuchsia::camera3::Stream> request)
{
    EXPECT_EQ(index, 0U);
...
FakeCameraDeviceWatcher::FakeCameraDeviceWatcher(sys::OutgoingDirectory* outgoing_directory)
{
    zx_status_t status
        = outgoing_directory->AddPublicService<fuchsia::camera3::DeviceWatcher>([this](fidl::InterfaceRequest<fuchsia::camera3::DeviceWatcher> request) {
    ...
    devices_.insert(std::make_pair(next_device_id_++, std::make_unique<FakeCameraDevice>()));
}
...
std::unique_ptr<FakeCameraDevice> FakeCameraDeviceWatcher::RemoveDevice(uint64_t device_id)
{
...
void FakeCameraDeviceWatcher::Client::QueueEvent(fuchsia::camera3::WatchDevicesEvent event)
{
...
void FakeCameraDeviceWatcher::Client::WatchDevices(WatchDevicesCallback callback)
{
...
void FakeCameraDeviceWatcher::Client::ConnectToDevice(uint64_t id, fidl::InterfaceRequest<fuchsia::camera3::Device> request)
{
