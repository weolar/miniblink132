...
class FakeCameraStream final : public fuchsia::camera3::testing::Stream_TestBase, public base::MessagePumpForIO::ZxHandleWatcher {
public:
    ...
    // by ProduceFrame().
    static void ValidateFrameData(const uint8_t* data, gfx::Size size, uint8_t salt);
...
private:
    ...
    void SetBufferCollection(fidl::InterfaceHandle<fuchsia::sysmem::BufferCollectionToken> token_handle) override;
    ...
    void OnBufferCollectionSyncDone(fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> token_for_client,
        fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> failed_token);

    void OnBufferCollectionError(zx_status_t status);

    void OnBufferCollectionAllocated(fuchsia::sysmem2::BufferCollection_WaitForAllBuffersAllocated_Result wait_result);
    ...
    fuchsia::camera3::Orientation orientation_ = fuchsia::camera3::Orientation::UP;

    std::optional<fuchsia::math::Size> resolution_update_ = fuchsia::math::Size { kDefaultFrameSize.width(), kDefaultFrameSize.height() };
    WatchResolutionCallback watch_resolution_callback_;

    std::optional<fuchsia::camera3::Orientation> orientation_update_ = fuchsia::camera3::Orientation::UP;
    ...
    std::optional<fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken>> new_buffer_collection_token_for_client_;
...
class FakeCameraDevice final : public fuchsia::camera3::testing::Device_TestBase {
public:
    FakeCameraDevice();
    ...
    FakeCameraStream* stream()
    {
        return &stream_;
    }

    // Sets a custom handler for GetIdentifier() messages.
    void SetGetIdentifierHandler(base::RepeatingCallback<void(GetIdentifierCallback)> get_identifier_handler);
...
private:
    ...
    void ConnectToStream(uint32_t index, fidl::InterfaceRequest<fuchsia::camera3::Stream> request) override;
...
public:
    ...
    const DevicesMap& devices() const
    {
        return devices_;
    }
...
private:
    class Client final : public fuchsia::camera3::testing::DeviceWatcher_TestBase {
    public:
        ...
        void ConnectToDevice(uint64_t id, fidl::InterfaceRequest<fuchsia::camera3::Device> request) override;
    ...
    fidl::BindingSet<fuchsia::camera3::DeviceWatcher, std::unique_ptr<Client>> bindings_;
