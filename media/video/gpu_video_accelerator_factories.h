...
public:
    ...
    virtual void GetChannelToken(gpu::mojom::GpuChannel::GetChannelTokenCallback cb) = 0;
    ...
    virtual Supported IsDecoderConfigSupported(const VideoDecoderConfig& config) = 0;
    ...
    virtual std::unique_ptr<media::VideoDecoder> CreateVideoDecoder(MediaLog* media_log, RequestOverlayInfoCB request_overlay_info_cb) = 0;
    ...
    //
    // May be called on any thread.
    virtual std::optional<SupportedVideoDecoderConfigs> GetSupportedVideoDecoderConfigs() = 0;
    ...
    virtual std::optional<VideoEncodeAccelerator::SupportedProfiles> GetVideoEncodeAcceleratorSupportedProfiles() = 0;
    ...
    virtual std::unique_ptr<VideoEncodeAccelerator> CreateVideoEncodeAccelerator() = 0;

    virtual std::unique_ptr<gfx::GpuMemoryBuffer> CreateGpuMemoryBuffer(const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage) = 0;

    // |for_media_stream| specifies webrtc use case of media streams.
    virtual bool ShouldUseGpuMemoryBuffersForVideoFrames(bool for_media_stream) const = 0;

    // Pixel format of the hardware video frames created when GpuMemoryBuffers
    // video frames are enabled.
    virtual OutputFormat VideoFrameOutputFormat(VideoPixelFormat pixel_format) = 0;
    ...
    virtual base::UnsafeSharedMemoryRegion CreateSharedMemoryRegion(size_t size) = 0;
