...
public:
    ...
    void SetUp() override
    {
        ...
        media_task_runner_handle_ = std::make_unique<base::SingleThreadTaskRunner::CurrentDefaultHandle>(media_task_runner_);
        mock_gpu_factories_ = std::make_unique<MockGpuVideoAcceleratorFactories>(sii_.get());
        gpu_memory_buffer_pool_ = std::make_unique<GpuMemoryBufferVideoFramePool>(media_task_runner_, copy_task_runner_.get(), mock_gpu_factories_.get());
    ...
    static scoped_refptr<VideoFrame> CreateTestYUVVideoFrame(int dimension, size_t bit_depth = 8, int visible_rect_crop = 0)
    {
        ...
        const VideoPixelFormat format = (bit_depth > 8) ? PIXEL_FORMAT_YUV420P10 : PIXEL_FORMAT_I420;
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalYuvData(format, // format
            size, // coded_size
            gfx::Rect(visible_rect_crop, visible_rect_crop, size.width() - visible_rect_crop,
    ...
    scoped_refptr<VideoFrame> CreateTestYUVVideoFrameWithOddSize(int dimension, size_t bit_depth = 8, int visible_rect_crop = 0)
    {
        const VideoPixelFormat format = (bit_depth > 8) ? PIXEL_FORMAT_YUV420P10 : PIXEL_FORMAT_I420;
        const int multiplier = format == PIXEL_FORMAT_YUV420P10 ? 2 : 1;
        ...
        u_data_ = new uint8_t[multiplier * dimension_aligned * dimension_aligned / 4]();
        v_data_ = new uint8_t[multiplier * dimension_aligned * dimension_aligned / 4]();
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalYuvData(format, // format
            size, // coded_size
            gfx::Rect(visible_rect_crop, visible_rect_crop, size.width() - visible_rect_crop * 2,
    ...
    static scoped_refptr<VideoFrame> CreateTestYUVAVideoFrame(int dimension)
    {
        ...
        scoped_refptr<VideoFrame> video_frame = VideoFrame::WrapExternalYuvaData(format, // format
            size, // coded_size
...
protected:
    ...
    std::unique_ptr<base::SingleThreadTaskRunner::CurrentDefaultHandle> media_task_runner_handle_;
    scoped_refptr<gpu::TestSharedImageInterface> sii_;
};

void MaybeCreateHardwareFrameCallback(scoped_refptr<VideoFrame>* video_frame_output, scoped_refptr<VideoFrame> video_frame)
{
    *video_frame_output = std::move(video_frame);
}

void MaybeCreateHardwareFrameCallbackAndTrackTime(
    scoped_refptr<VideoFrame>* video_frame_output, base::TimeTicks* output_time, scoped_refptr<VideoFrame> video_frame)
{
    *video_frame_output = std::move(video_frame);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, VideoFrameOutputFormatUnknown)
{
    ...
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::UNDEFINED);
    scoped_refptr<VideoFrame> frame;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    RunUntilIdle();
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareFrameWithOddSize)
{
    ...
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(9);
    scoped_refptr<VideoFrame> frame;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));

    RunUntilIdle();

    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        EXPECT_NE(software_frame.get(), frame.get());
        EXPECT_EQ(PIXEL_FORMAT_YV12, frame->format());
        EXPECT_TRUE(frame->HasSharedImage());
        EXPECT_EQ(1u, sii_->shared_image_count());
        ...
        const auto* y_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(0));
        const auto* u_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(1));
        const auto* v_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(2));
        ...
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kY)[80], y_memory[y_stride * 8 + 8]);
        auto u_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(1);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kU)[24], u_memory[u_stride * 4 + 4]);
        auto v_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(2);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kV)[24], v_memory[v_stride * 4 + 4]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareFrameWithOddOriginOddSize)
{
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(11, 8, 1);
    scoped_refptr<VideoFrame> frame;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOne10BppHardwareFrameWithOddSize)
{
    if (SkipTestWithMappableSI) {
        return;
    }
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(17, 10);
    scoped_refptr<VideoFrame> frame;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        const uint16_t* y_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::Plane::kY));
        const uint16_t* u_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::Plane::kU));
        const uint16_t* v_plane_data = reinterpret_cast<const uint16_t*>(software_frame->visible_data(VideoFrame::Plane::kV));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, ReuseFirstResource)
{
    ...
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame2));
    RunUntilIdle();
...
{
    scoped_refptr<VideoFrame> frame;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(CreateTestYUVVideoFrame(10), base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    RunUntilIdle();
    ...
    frame = nullptr;
    RunUntilIdle();
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(CreateTestYUVVideoFrame(4), base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    RunUntilIdle();
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareNV12Frame)
{
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareNV12FrameWithOddSize)
{
    if (SkipTestWithMappableSI) {
        return;
    }
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(5);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        const auto* y_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(0));
        const auto* uv_memory = reinterpret_cast<uint8_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(1));
        ...
        auto y_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kY)[24], y_memory[y_stride * 4 + 4]);
        auto uv_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(1);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kU)[8], uv_memory[uv_stride * 2 + 4]);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kV)[8], uv_memory[uv_stride * 2 + 5]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareFrameForNV12InputWithOddSize)
{
    if (SkipTestWithMappableSI) {
        return;
    }
    scoped_refptr<VideoFrame> software_frame = CreateTestNV12VideoFrameWithOddSize(135);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::NV12);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    ...
    if (gfx::IsOddWidthMultiPlanarBuffersAllowed() && gfx::IsOddHeightMultiPlanarBuffersAllowed()) {
        ...
        EXPECT_EQ(kYValue, software_frame->visible_data(VideoFrame::Plane::kY)[18224]);
        EXPECT_EQ(kUValue, software_frame->visible_data(VideoFrame::Plane::kUV)[9246]);
        EXPECT_EQ(kVValue, software_frame->visible_data(VideoFrame::Plane::kUV)[9247]);

        // Compare the last pixel of each plane in |software_frame| and |frame|.
        // y_memory = 135x135, uv_memory = 136x68.
        auto y_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(0);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kY)[18224], y_memory[y_stride * 134 + 134]);
        auto uv_stride = mock_gpu_factories_->created_memory_buffers()[0]->stride(1);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kUV)[9246], uv_memory[uv_stride * 67 + 134]);
        EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kUV)[9247], uv_memory[uv_stride * 67 + 135]);
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareXR30Frame)
{
    ...
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10, 10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::XR30);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareP010Frame)
{
    ...
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10, 10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::P010);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
    ...
    const uint16_t* y_memory = reinterpret_cast<uint16_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(0));
    EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kY)[0] << 6, y_memory[0]);
    const uint16_t* uv_memory = reinterpret_cast<uint16_t*>(mock_gpu_factories_->created_memory_buffers()[0]->memory(1));
    EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kU)[0] << 6, uv_memory[0]);
    EXPECT_EQ(software_frame->visible_data(VideoFrame::Plane::kV)[0] << 6, uv_memory[1]);
}

TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareP010FrameWithOddSize)
{
    if (SkipTestWithMappableSI) {
        return;
    }
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrameWithOddSize(7, 10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::P010);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateOneHardwareXB30Frame)
{
    ...
    scoped_refptr<VideoFrame> software_frame = CreateTestYUVVideoFrame(10, 10);
    scoped_refptr<VideoFrame> frame;
    mock_gpu_factories_->SetVideoFrameOutputFormat(media::GpuVideoAcceleratorFactories::OutputFormat::XB30);
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, PreservesMetadata)
{
    ...
    base::TimeTicks kTestReferenceTime = base::Milliseconds(12345) + base::TimeTicks();
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, CreateGpuMemoryBufferFailAfterShutdown)
{
    if (SkipTestWithMappableSI) {
        return;
    }
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, ShutdownReleasesUnusedResources)
{
    ...
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame_1));

    RunUntilIdle();
    EXPECT_NE(software_frame.get(), frame_1.get());

    scoped_refptr<VideoFrame> frame_2;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame_2));
    RunUntilIdle();
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, AtMostOneCopyInFlight)
{
    ...
    scoped_refptr<VideoFrame> frame_1;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame_1, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame_1));

    scoped_refptr<VideoFrame> software_frame_2 = CreateTestYUVVideoFrame(10);
    scoped_refptr<VideoFrame> frame_2;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame_2, base::BindOnce(MaybeCreateHardwareFrameCallback, &frame_2));
...
TEST_F(GpuMemoryBufferVideoFramePoolTest, PreservesOrder)
{
    ...
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame_2, base::BindOnce(MaybeCreateHardwareFrameCallbackAndTrackTime, &frame_2, &time_2));

    scoped_refptr<VideoFrame> software_frame_3 = VideoFrame::CreateEOSFrame();
    scoped_refptr<VideoFrame> frame_3;
    base::TimeTicks time_3;
    gpu_memory_buffer_pool_->MaybeCreateHardwareFrame(software_frame_3, base::BindOnce(MaybeCreateHardwareFrameCallbackAndTrackTime, &frame_3, &time_3));
