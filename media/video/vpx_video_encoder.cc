...
constexpr vpx_enc_frame_flags_t VP8_UPDATE_NOTHING = VP8_EFLAG_NO_UPD_ARF | VP8_EFLAG_NO_UPD_GF | VP8_EFLAG_NO_UPD_LAST;
...
EncoderStatus SetUpVpxConfig(const VideoEncoder::Options& opts, VideoCodecProfile profile, vpx_codec_enc_cfg_t* config)
{
    if (opts.frame_size.width() <= 0 || opts.frame_size.height() <= 0)
        return EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Negative width or height values.");

    if (!opts.frame_size.GetCheckedArea().IsValid())
        return EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Frame is too large.");
    ...
    if (profile == VP8PROFILE_ANY && opts.content_hint == VideoEncoder::ContentHint::Screen) {
        config->rc_min_quantizer = 12;
    }
    config->rc_resize_allowed = 0;
    // Only if latency_mode is real time, a frame might be dropped.
    config->rc_dropframe_thresh = opts.latency_mode == VideoEncoder::LatencyMode::Realtime ? GetDefaultVideoEncoderDropFrameThreshold() : 0;
    ...
    uint32_t default_bitrate = GetDefaultVideoEncodeBitrate(opts.frame_size, opts.framerate.value_or(30));
    ...
    case SVCScalabilityMode::kL1T2:
        ...
        DCHECK_EQ(config->ts_periodicity, sizeof(vp8_2layers_temporal_flags) / sizeof(vp8_2layers_temporal_flags[0]));
        ts_layer_id[0] = 0;
        ts_layer_id[1] = 1;
        ts_rate_decimator[0] = 2;
        ts_rate_decimator[1] = 1;
        // Bitrate allocation L0: 60% L1: 40%
        layer_target_bitrate[0] = ts_target_bitrate[0] = 60 * config->rc_target_bitrate / 100;
        layer_target_bitrate[1] = ts_target_bitrate[1] = config->rc_target_bitrate;
    ...
    case SVCScalabilityMode::kL1T3:
        ...
        DCHECK_EQ(config->ts_periodicity, sizeof(vp8_3layers_temporal_flags) / sizeof(vp8_3layers_temporal_flags[0]));
        ts_layer_id[0] = 0;
        ts_layer_id[1] = 2;
        ts_layer_id[2] = 1;
        ts_layer_id[3] = 2;
        ...
        layer_target_bitrate[0] = ts_target_bitrate[0] = 50 * config->rc_target_bitrate / 100;
        layer_target_bitrate[1] = ts_target_bitrate[1] = 70 * config->rc_target_bitrate / 100;
        layer_target_bitrate[2] = ts_target_bitrate[2] = config->rc_target_bitrate;
        config->temporal_layering_mode = VP9E_TEMPORAL_LAYERING_MODE_0212;
        config->g_error_resilient = VPX_ERROR_RESILIENT_DEFAULT;
        break;
    default: {
        return EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Unsupported number of temporal layers.");
    }
    }
...
std::string LogVpxErrorMessage(vpx_codec_ctx_t* context, const char* message, vpx_codec_err_t status)
{
    auto formatted_msg = base::StringPrintf("%s: %s (%s)", message, vpx_codec_err_to_string(status), vpx_codec_error_detail(context));
...
std::optional<VideoPixelFormat> GetConversionFormat(VideoCodecProfile profile, VideoPixelFormat format, bool needs_resize)
{
    switch (profile) {
    case VP8PROFILE_ANY:
    case VP9PROFILE_PROFILE0:
        if ((format != PIXEL_FORMAT_NV12 && format != PIXEL_FORMAT_I420) || needs_resize) {
...
void SetupStandardYuvPlanes(const VideoFrame& frame, vpx_image_t* vpx_image)
{
    ...
    planes[VPX_PLANE_Y] = const_cast<uint8_t*>(frame.visible_data(VideoFrame::Plane::kY));
    planes[VPX_PLANE_U] = const_cast<uint8_t*>(frame.visible_data(VideoFrame::Plane::kU));
    planes[VPX_PLANE_V] = const_cast<uint8_t*>(frame.visible_data(VideoFrame::Plane::kV));
...
void I444ToI410(const VideoFrame& frame, vpx_image_t* vpx_image)
{
    ...
    for (size_t i = 0; i < VideoFrame::NumPlanes(frame.format()); ++i) {
        libyuv::Convert8To16Plane(frame.visible_data(i), frame.stride(i), reinterpret_cast<uint16_t*>(planes[i]), stride[i] / 2, 1024,
            VideoFrame::Columns(i, frame.format(), frame.visible_rect().size().width()),
            VideoFrame::Rows(i, frame.format(), frame.visible_rect().size().height()));
    }
}
...
VpxVideoEncoder::VpxVideoEncoder()
    : codec_(nullptr, FreeCodecCtx)
{
}

void VpxVideoEncoder::Initialize(VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (profile == VP8PROFILE_ANY) {
        ...
        auto status = EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedProfile).WithData("profile", profile);
        std::move(done_cb).Run(status);
        return;
    }

    if (options.bitrate.has_value() && options.bitrate->mode() == Bitrate::Mode::kExternal && !is_vp9) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "VP8 doesn't support per-frame quantizer"));
        return;
    }

    auto vpx_error = vpx_codec_enc_config_default(iface, &codec_config_, 0);
    if (vpx_error != VPX_CODEC_OK) {
        auto status = EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Failed to get default VPX config.").WithData("vpx_error", vpx_error);
        std::move(done_cb).Run(status);
        return;
    }

    if (profile == VP9PROFILE_PROFILE0 || profile == VP9PROFILE_PROFILE2) {
        if (options.subsampling.value_or(VideoChromaSampling::k420) != VideoChromaSampling::k420) {
            std::move(done_cb).Run(
                EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Only 4:2:0 subsampling is supported with VP9 profiles 0 and 2."));
            return;
        }
        ...
        if (options.subsampling != VideoChromaSampling::k444) {
            std::move(done_cb).Run(
                EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Only 4:4:4 subsampling is supported with VP9 profiles 1 and 3."));
            return;
        }
    }

    if ((profile == VP9PROFILE_PROFILE0 || profile == VP9PROFILE_PROFILE1) && options.bit_depth.value_or(8) != 8) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Only 8-bit depth is supported with VP9 profiles 0 and 1."));
        return;
    }
    if ((profile == VP9PROFILE_PROFILE2 || profile == VP9PROFILE_PROFILE3) && options.bit_depth.value_or(10) != 10) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Only 10-bit depth is supported with VP9 profiles 2 and 3."));
        return;
    }
    ...
    vpx_error = vpx_codec_enc_init(codec.get(), iface, &codec_config_, codec_config_.g_bit_depth == VPX_BITS_8 ? 0 : VPX_CODEC_USE_HIGHBITDEPTH);
    if (vpx_error != VPX_CODEC_OK) {
        auto msg = LogVpxErrorMessage(codec.get(), "VPX encoder initialization error", vpx_error);
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, msg));
        return;
    }
    ...
    if (vpx_error != VPX_CODEC_OK) {
        auto msg = LogVpxErrorMessage(codec.get(), "VPX encoder VP8E_SET_CPUUSED error", vpx_error);
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, msg));
        return;
    }
    ...
    if (is_vp9) {
        ...
        int log2_tile_columns = std::min(static_cast<int>(std::log2(tile_columns)), 6);
        vpx_error = vpx_codec_control(codec.get(), VP9E_SET_TILE_COLUMNS, log2_tile_columns);
        if (vpx_error != VPX_CODEC_OK) {
            auto msg = LogVpxErrorMessage(codec.get(), "VPX encoder VP9E_SET_TILE_COLUMNS error", vpx_error);
            std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, msg));
            return;
        }
        ...
        if (codec_config_.ts_number_layers > 1) {
            ...
            if (vpx_error != VPX_CODEC_OK) {
                auto msg = LogVpxErrorMessage(codec.get(), "Can't activate SVC encoding", vpx_error);
                status = EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, msg);
                std::move(done_cb).Run(status);
                return;
            }
        }
    ...
    if (options.content_hint == ContentHint::Screen) {
        if (is_vp9) {
            ...
            // in VP9 too.
            vpx_codec_control(codec.get(), VP9E_SET_TUNE_CONTENT, VP9E_CONTENT_SCREEN);
        } else {
            static_thresh = 100;
            ...
            if (options.latency_mode == LatencyMode::Realtime && base::FeatureList::IsEnabled(kWebCodecsVideoEncoderFrameDrop)) {
                screen_content_mode = 2;
            }
            vpx_codec_control(codec.get(), VP8E_SET_SCREEN_CONTENT_MODE, screen_content_mode);
        }
    }
...
void VpxVideoEncoder::Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb)
{
    done_cb = BindCallbackToCurrentLoopIfNeeded(std::move(done_cb));
    if (!codec_) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializeNeverCompleted);
        return;
    }

    bool key_frame = encode_options.key_frame;
    if (!frame) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kInvalidInputFrame, "No frame provided for encoding."));
        return;
    }
    ...
    if (frame->format() == PIXEL_FORMAT_NV12 && frame->HasMappableGpuBuffer()) {
        ...
        if (!frame) {
            std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kSystemAPICallError, "Convert GMB frame to MemoryMappedFrame failed."));
            return;
        }
    }

    if (!frame->IsMappable()) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kInvalidInputFrame, "Frame is not mappable")
    ...
    if (auto conversion_format = GetConversionFormat(profile_, frame->format(),
            /*needs_resize=*/frame->visible_rect().size() != options_.frame_size)) {
        auto temp_frame
            = frame_pool_.CreateFrame(*conversion_format, options_.frame_size, gfx::Rect(options_.frame_size), options_.frame_size, frame->timestamp());
        if (!temp_frame) {
            std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kOutOfMemoryError, "Can't allocate a temporary frame for conversion"));
            return;
        }
    ...
    case VP8PROFILE_ANY:
    case VP9PROFILE_PROFILE0: {
        DCHECK(frame->format() == PIXEL_FORMAT_NV12 || frame->format() == PIXEL_FORMAT_I420);
        if (frame->format() == PIXEL_FORMAT_NV12) {
            RecreateVpxImageIfNeeded(VPX_IMG_FMT_NV12, /*needs_memory=*/false);
            planes[VPX_PLANE_Y] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::Plane::kY));
            planes[VPX_PLANE_U] = const_cast<uint8_t*>(frame->visible_data(VideoFrame::Plane::kUV));
    ...
    case VP9PROFILE_PROFILE2:
        DCHECK(frame->format() == PIXEL_FORMAT_YUV420P10 || frame->format() == PIXEL_FORMAT_I420);
        ...
        libyuv::I420ToI010(frame->visible_data(VideoFrame::Plane::kY), frame->stride(VideoFrame::Plane::kY), frame->visible_data(VideoFrame::Plane::kU),
            frame->stride(VideoFrame::Plane::kU), frame->visible_data(VideoFrame::Plane::kV), frame->stride(VideoFrame::Plane::kV),
            reinterpret_cast<uint16_t*>(planes[VPX_PLANE_Y]), stride[VPX_PLANE_Y] / 2, reinterpret_cast<uint16_t*>(planes[VPX_PLANE_U]),
            stride[VPX_PLANE_U] / 2, reinterpret_cast<uint16_t*>(planes[VPX_PLANE_V]), stride[VPX_PLANE_V] / 2, frame->visible_rect().width(),
    ...
    case VP9PROFILE_PROFILE3:
        DCHECK(frame->format() == PIXEL_FORMAT_YUV444P10 || frame->format() == PIXEL_FORMAT_I444);
    ...
    if (is_layer_encoding) {
        ...
        unsigned int index_in_temp_cycle = temporal_svc_frame_index_ % codec_config_.ts_periodicity;
        temporal_id = base::span(codec_config_.ts_layer_id)[index_in_temp_cycle];
        if (profile_ == VP8PROFILE_ANY) {
            auto vp8_layers_flags = codec_config_.ts_number_layers == 2 ? base::span<vpx_enc_frame_flags_t>(vp8_2layers_temporal_flags)
    ...
    if (encode_options.quantizer.has_value()) {
        ...
        qp = std::clamp(qp, static_cast<int>(codec_config_.rc_min_quantizer), static_cast<int>(codec_config_.rc_max_quantizer));
        vpx_codec_control(codec_.get(), VP9E_SET_QUANTIZER_ONE_PASS, qp);
    }

    TRACE_EVENT1("media", "vpx_codec_encode", "timestamp", frame->timestamp());
    auto vpx_error = vpx_codec_encode(codec_.get(), &vpx_image_, timestamp_us, duration_us, flags, deadline);

    if (vpx_error != VPX_CODEC_OK) {
        auto msg = LogVpxErrorMessage(codec_.get(), "VPX encoding error", vpx_error);
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedEncode, msg).WithData("vpx_error", vpx_error));
        return;
    }

    auto output = GetEncoderOutput(temporal_id, frame->timestamp(), frame->ColorSpace());
    if (is_layer_encoding) {
...
void VpxVideoEncoder::ChangeOptions(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    done_cb = BindCallbackToCurrentLoopIfNeeded(std::move(done_cb));
    if (!codec_) {
        std::move(done_cb).Run(EncoderStatus::Codes::kEncoderInitializeNeverCompleted);
    ...
    if (profile_ != VP8PROFILE_ANY) {
        ...
        if (options.frame_size.width() > originally_configured_size_.width() || options.frame_size.height() > originally_configured_size_.height()) {
            auto status = EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "libvpx/VP9 doesn't support dynamically increasing frame dimensions");
    ...
    if (error == VPX_CODEC_OK) {
        ...
        status = EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Failed to set new VPX config").WithData("vpx_error", error);
    }
...
VideoEncoderOutput VpxVideoEncoder::GetEncoderOutput(int temporal_id, base::TimeDelta timestamp, gfx::ColorSpace color_space) const
{
    ...
    while ((pkt = vpx_codec_get_cx_data(codec_.get(), &iter))) {
        if (pkt->kind == VPX_CODEC_CX_FRAME_PKT) {
            ...
            output.data = base::HeapArray<uint8_t>::CopiedFrom(UNSAFE_BUFFERS({ reinterpret_cast<uint8_t*>(pkt->data.frame.buf), pkt->data.frame.sz }));
...
void VpxVideoEncoder::RecreateVpxImageIfNeeded(vpx_img_fmt fmt, bool needs_memory)
{
    const bool has_changed = vpx_image_.fmt != fmt || vpx_image_.d_w != codec_config_.g_w || vpx_image_.d_h != codec_config_.g_h;
...
void VpxVideoEncoder::UpdateEncoderColorSpace()
{
    ...
    case gfx::ColorSpace::PrimaryID::BT709: {
        ...
        if (matrix_id == gfx::ColorSpace::MatrixID::GBR || matrix_id == gfx::ColorSpace::MatrixID::RGB) {
            vpx_cs = VPX_CS_SRGB;
        } else {
    ...
    if (vpx_cs != VPX_CS_UNKNOWN) {
        ...
        if (profile_ != VP8PROFILE_ANY) {
            auto vpx_error = vpx_codec_control(codec_.get(), VP9E_SET_COLOR_SPACE, vpx_cs);
            if (vpx_error != VPX_CODEC_OK) {
    ...
    if (last_frame_color_space_.GetRangeID() == gfx::ColorSpace::RangeID::FULL || last_frame_color_space_.GetRangeID() == gfx::ColorSpace::RangeID::LIMITED) {
        const auto vpx_range = last_frame_color_space_.GetRangeID() == gfx::ColorSpace::RangeID::FULL ? VPX_CR_FULL_RANGE : VPX_CR_STUDIO_RANGE;
        vpx_image_.range = vpx_range;
        if (profile_ != VP8PROFILE_ANY) {
            auto vpx_error = vpx_codec_control(codec_.get(), VP9E_SET_COLOR_RANGE, vpx_range);
            if (vpx_error != VPX_CODEC_OK) {
