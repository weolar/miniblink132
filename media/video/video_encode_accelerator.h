...
struct MEDIA_EXPORT BitstreamBufferMetadata final {
    ...
    BitstreamBufferMetadata(size_t payload_size_bytes, bool key_frame, base::TimeDelta timestamp);
    static BitstreamBufferMetadata CreateForDropFrame(base::TimeDelta timestamp, uint8_t spatial_idx = 0, bool end_of_picture = true);
    ~BitstreamBufferMetadata();
...
public:
    ...
    struct MEDIA_EXPORT SupportedProfile {
        SupportedProfile();
        SupportedProfile(VideoCodecProfile profile, const gfx::Size& max_resolution, uint32_t max_framerate_numerator = 0u,
            uint32_t max_framerate_denominator = 1u, SupportedRateControlMode rc_modes = kConstantMode,
            const std::vector<SVCScalabilityMode>& scalability_modes = {}, const std::vector<VideoPixelFormat>& gpu_suppoted_pixel_formats = {},
    ...
    struct MEDIA_EXPORT Config {
        ...
        Config(VideoPixelFormat input_format, const gfx::Size& input_visible_size, VideoCodecProfile output_profile, const Bitrate& bitrate, uint32_t framerate,
            StorageType storage_type, ContentType content_type);
    ...
    public:
        ...
        //  encoder in bytes.
        virtual void RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size) = 0;
        ...
        virtual void BitstreamBufferReady(int32_t bitstream_buffer_id, const BitstreamBufferMetadata& metadata) = 0;
    ...
    virtual bool Initialize(const Config& config, Client* client, std::unique_ptr<MediaLog> media_log) = 0;
    ...
    virtual void Encode(scoped_refptr<VideoFrame> frame, const VideoEncoder::EncodeOptions& options);
    ...
    // callback.
    virtual void RequestEncodingParametersChange(const Bitrate& bitrate, uint32_t framerate, const std::optional<gfx::Size>& size) = 0;
    ...
    // requirement can be removed once |RequestEncodingParametersChange| has a
    // callback.
    virtual void RequestEncodingParametersChange(const VideoBitrateAllocation& bitrate, uint32_t framerate, const std::optional<gfx::Size>& size);
    ...
    virtual void SetCommandBufferHelperCB(base::RepeatingCallback<scoped_refptr<CommandBufferHelper>()> get_command_buffer_helper_cb,
...
protected:
    ...
    static size_t EstimateBitstreamBufferSize(const Bitrate& bitrate, uint32_t framerate, const gfx::Size& coded_size);
};

MEDIA_EXPORT bool operator==(const VideoEncodeAccelerator::SupportedProfile& l, const VideoEncodeAccelerator::SupportedProfile& r);
...
MEDIA_EXPORT bool operator==(const BitstreamBufferMetadata& l, const BitstreamBufferMetadata& r);
MEDIA_EXPORT bool operator==(const VideoEncodeAccelerator::Config::SpatialLayer& l, const VideoEncodeAccelerator::Config::SpatialLayer& r);
MEDIA_EXPORT bool operator==(const VideoEncodeAccelerator::Config& l, const VideoEncodeAccelerator::Config& r);

MEDIA_EXPORT inline VideoEncodeAccelerator::SupportedRateControlMode operator|(
    VideoEncodeAccelerator::SupportedRateControlMode lhs, VideoEncodeAccelerator::SupportedRateControlMode rhs)
{
    return static_cast<VideoEncodeAccelerator::SupportedRateControlMode>(static_cast<uint8_t>(lhs) | static_cast<uint8_t>(rhs));
}
...
// use the destructor.
template <> struct MEDIA_EXPORT default_delete<media::VideoEncodeAccelerator> {
