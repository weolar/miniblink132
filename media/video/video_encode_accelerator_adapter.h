...
class MEDIA_EXPORT VideoEncodeAcceleratorAdapter : public VideoEncoder, public VideoEncodeAccelerator::Client {
public:
    VideoEncodeAcceleratorAdapter(GpuVideoAcceleratorFactories* gpu_factories, std::unique_ptr<MediaLog> media_log,
        scoped_refptr<base::SequencedTaskRunner> callback_task_runner,
        VideoEncodeAccelerator::Config::EncoderType required_encoder_type = VideoEncodeAccelerator::Config::EncoderType::kHardware);
    ...
    void Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb) override;
    ...
    void BitstreamBufferReady(int32_t buffer_id, const BitstreamBufferMetadata& metadata) override;
...
private:
    ...
    enum class State { kNotInitialized, kWaitingForFirstFrame, kInitializing, kReadyToEncode, kFlushing, kReconfiguring };
    struct PendingOp {
        PendingOp();
        ~PendingOp();
    ...
    void InitializeOnAcceleratorThread(VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb);
    void InitializeInternalOnAcceleratorThread();
    void EncodeOnAcceleratorThread(scoped_refptr<VideoFrame> frame, EncodeOptions encode_options, EncoderStatusCB done_cb);
    void FlushOnAcceleratorThread(EncoderStatusCB done_cb);
    void ChangeOptionsOnAcceleratorThread(const Options options, OutputCB output_cb, EncoderStatusCB done_cb);

    template <class T> T WrapCallback(T cb);
    EncoderStatus::Or<scoped_refptr<VideoFrame>> PrepareGpuFrame(scoped_refptr<VideoFrame> src_frame);
    EncoderStatus::Or<scoped_refptr<VideoFrame>> PrepareCpuFrame(scoped_refptr<VideoFrame> src_frame);

    scoped_refptr<ReadOnlyRegionPool> input_pool_;
    scoped_refptr<base::UnsafeSharedMemoryPool> output_pool_;
    std::vector<std::unique_ptr<base::UnsafeSharedMemoryPool::Handle>> output_buffer_handles_;
    ...
    VideoEncodeAccelerator::SupportedRateControlMode supported_rc_modes_ = VideoEncodeAccelerator::kNoMode;
    ...
    VideoEncodeAccelerator::Config::EncoderType required_encoder_type_ = VideoEncodeAccelerator::Config::EncoderType::kHardware;
