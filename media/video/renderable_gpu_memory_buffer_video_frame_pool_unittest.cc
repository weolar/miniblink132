...
public:
    ...
    {
        DoCreateSharedImage(si_format, gpu_memory_buffer->GetSize(), color_space, surface_origin, alpha_type, usage, gpu_memory_buffer->CloneHandle());
        return context_provider_->SharedImageInterface()->CreateSharedImage(
            { si_format, gpu_memory_buffer->GetSize(), color_space, surface_origin, alpha_type, usage, "RenderableGpuMemoryBufferVideoFramePoolTest" },
    ...
    scoped_refptr<gpu::ClientSharedImage> CreateSharedImage(const gfx::Size& size, gfx::BufferUsage buffer_usage, const viz::SharedImageFormat& si_format,
        ...
        gpu::SyncToken& sync_token) override
    {
        DoCreateMappableSharedImage(size, buffer_usage, si_format, color_space, surface_origin, alpha_type, usage, sync_token);
        context_provider_->SharedImageInterface()->UseTestGMBInSharedImageCreationWithBufferUsage();
        return context_provider_->SharedImageInterface()->CreateSharedImage(
            { si_format, size, color_space, surface_origin, alpha_type, usage, "RenderableGpuMemoryBufferVideoFramePoolTest" }, gpu::kNullSurfaceHandle,
            buffer_usage);
    }

    MOCK_METHOD2(DoCreateGpuMemoryBuffer, void(const gfx::Size& size, gfx::BufferFormat format));
    MOCK_METHOD7(DoCreateSharedImage,
        void(viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type,
            uint32_t usage, gfx::GpuMemoryBufferHandle buffer_handle));
    MOCK_METHOD8(DoCreateMappableSharedImage,
        void(const gfx::Size& size, gfx::BufferUsage buffer_usage, const viz::SharedImageFormat& si_format, const gfx::ColorSpace& color_space,
            GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage, gpu::SyncToken& sync_token));

    MOCK_METHOD2(DestroySharedImage, void(const gpu::SyncToken& sync_token, scoped_refptr<gpu::ClientSharedImage> shared_image));

    base::WeakPtr<FakeContext> GetWeakPtr()
    {
...
class RenderableGpuMemoryBufferVideoFramePoolTest : public testing::TestWithParam<VideoPixelFormat> {
public:
...
protected:
    void VerifySharedImageCreation(FakeContext* context)
    {
        ...
        EXPECT_CALL(*context, DoCreateMappableSharedImage(_, _, si_format, _, _, _, _, _));
    }
...
TEST_P(RenderableGpuMemoryBufferVideoFramePoolTest, SimpleLifetimes)
{
    ...
    std::unique_ptr<RenderableGpuMemoryBufferVideoFramePool> pool;
    {
        ...
        pool = RenderableGpuMemoryBufferVideoFramePool::Create(std::move(context_strong), format_);
    }
    ...
    EXPECT_CALL(*context, DoCreateMappableSharedImage(_, _, _, _, _, _, _, _)).Times(0);
...
TEST_P(RenderableGpuMemoryBufferVideoFramePoolTest, CrossThread)
{
    ...
    auto pool = RenderableGpuMemoryBufferVideoFramePool::Create(std::make_unique<FakeContext>(), format_);

    base::ThreadPool::CreateSequencedTaskRunner({})->PostTaskAndReplyWithResult(FROM_HERE,
        // Create a frame on another thread.
        base::BindLambdaForTesting([&]() { return pool->MaybeCreateVideoFrame(size0, color_space0); }),
        // Destroy the video frame on the main thread.
        base::BindLambdaForTesting([&](scoped_refptr<VideoFrame> video_frame0) {}));
    task_environment.RunUntilIdle();
...
TEST_P(RenderableGpuMemoryBufferVideoFramePoolTest, VideoFramesDestroyedConcurrently)
{
    base::test::TaskEnvironment task_environment { base::test::TaskEnvironment::TimeSource::MOCK_TIME };
    const gfx::Size size0(128, 256);
    const gfx::ColorSpace color_space0 = GetColorSpaceForPixelFormat(format_);
    ...
    for (int i = 0; i < kNumFrames; i++) {
        base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(FROM_HERE, base::DoNothingWithBoundArgs(std::move(frames[i])));
    }
...
TEST_P(RenderableGpuMemoryBufferVideoFramePoolTest, ConcurrentCreateDestroy)
{
    ...
    // problem.
    base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(FROM_HERE, base::DoNothingWithBoundArgs(std::move(video_frame0)));
...
INSTANTIATE_TEST_SUITE_P(All, RenderableGpuMemoryBufferVideoFramePoolTest,
    testing::Values(media::VideoPixelFormat::PIXEL_FORMAT_NV12, media::VideoPixelFormat::PIXEL_FORMAT_ARGB, media::VideoPixelFormat::PIXEL_FORMAT_ABGR));
