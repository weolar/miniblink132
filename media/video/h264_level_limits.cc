...
uint32_t H264ProfileLevelToMaxBR(VideoCodecProfile profile, uint8_t level)
{
    ...
    default:
        DVLOG(1) << "Failed to query MaxBR for profile: " << GetProfileName(profile);
        return 0;
    }
}
...
bool CheckH264LevelLimits(VideoCodecProfile profile, uint8_t level, uint32_t bitrate, uint32_t framerate, uint32_t framesize_in_mbs)
{
    ...
    if (bitrate > max_bitrate) {
        DVLOG(1) << "Target bitrate: " << bitrate << " exceeds Max: " << max_bitrate << " bit/s";
        return false;
    }

    if (framesize_in_mbs > H264LevelToMaxFS(level)) {
        DVLOG(1) << "Target frame size: " << framesize_in_mbs << " exceeds Max: " << H264LevelToMaxFS(level) << " Macroblocks";
        return false;
    }

    uint32_t mbps = framesize_in_mbs * framerate;
    if (mbps > H264LevelToMaxMBPS(level)) {
        DVLOG(1) << "Target macroblock processing rate: " << mbps << " exceeds Max: " << H264LevelToMaxMBPS(level) << "Macroblock/s";
        return false;
    }
...
absl::optional<uint8_t> FindValidH264Level(VideoCodecProfile profile, uint32_t bitrate, uint32_t framerate, uint32_t framesize_in_mbs)
{
    constexpr uint8_t kH264Levels[] = {
        H264SPS::kLevelIDC1p0,
        H264SPS::kLevelIDC1B,
        H264SPS::kLevelIDC1p1,
        H264SPS::kLevelIDC1p2,
        H264SPS::kLevelIDC1p3,
        H264SPS::kLevelIDC2p0,
        H264SPS::kLevelIDC2p1,
        H264SPS::kLevelIDC2p2,
        H264SPS::kLevelIDC3p0,
        H264SPS::kLevelIDC3p1,
        H264SPS::kLevelIDC3p2,
        H264SPS::kLevelIDC4p0,
        H264SPS::kLevelIDC4p1,
        H264SPS::kLevelIDC4p2,
        H264SPS::kLevelIDC5p0,
        H264SPS::kLevelIDC5p1,
        H264SPS::kLevelIDC5p2,
        H264SPS::kLevelIDC6p0,
        H264SPS::kLevelIDC6p1,
        H264SPS::kLevelIDC6p2,
    };

    for (const uint8_t level : kH264Levels) {
        if (CheckH264LevelLimits(profile, level, bitrate, framerate, framesize_in_mbs)) {
            return level;
        }
    }
    return absl::nullopt;
