...
public:
    using CreateFallbackCB = base::OnceCallback<EncoderStatus::Or<std::unique_ptr<VideoEncoder>>()>;

    VideoEncoderFallback(std::unique_ptr<VideoEncoder> main_encoder, CreateFallbackCB create_fallback_cb);
...
private:
    ...
    PendingEncode MakePendingEncode(scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb);
    void CallInfo(const VideoEncoderInfo& info);
    void CallOutput(VideoEncoderOutput output, std::optional<CodecDescription> desc);
