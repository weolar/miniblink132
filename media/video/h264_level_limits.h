...
uint32_t MEDIA_EXPORT H264ProfileLevelToMaxBR(VideoCodecProfile profile, uint8_t level);
...
bool MEDIA_EXPORT CheckH264LevelLimits(VideoCodecProfile profile, uint8_t level, uint32_t bitrate, uint32_t framerate, uint32_t framesize_in_mbs);
...
absl::optional<uint8_t> MEDIA_EXPORT FindValidH264Level(VideoCodecProfile profile, uint32_t bitrate, uint32_t framerate, uint32_t framesize_in_mbs);
} // namespace media
