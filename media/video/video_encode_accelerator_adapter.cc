...
#if BUILDFLAG(ENABLE_PLATFORM_HEVC) && BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
...
{
    base::CheckedNumeric<uint32_t> checked_bitrate_product = base::CheckMul<uint32_t>(frame_size.width(), frame_size.height(), kVEADefaultBitratePerPixel);
    // If the product has overflowed, clamp it to uint32_t max
    return checked_bitrate_product.ValueOrDefault(std::numeric_limits<uint32_t>::max());
}
...
uint32_t ComputeCheckedPeakBitrate(uint32_t target_bitrate)
{
    ...
    base::CheckedNumeric<uint32_t> checked_bitrate_product = base::CheckMul<uint32_t>(target_bitrate, 10u);
    return checked_bitrate_product.ValueOrDefault(std::numeric_limits<uint32_t>::max());
}

Bitrate CreateBitrate(
    const std::optional<Bitrate>& requested_bitrate, const gfx::Size& frame_size, VideoEncodeAccelerator::SupportedRateControlMode supported_rc_modes)
{
    ...
    if (supported_rc_modes & VideoEncodeAccelerator::kVariableMode) {
        ...
        // specified.
        return requested_bitrate.value_or(Bitrate::VariableBitrate(default_bitrate, ComputeCheckedPeakBitrate(default_bitrate)));
    }
    ...
    if (requested_bitrate && requested_bitrate->mode() == Bitrate::Mode::kConstant) {
        return *requested_bitrate;
    }

    return Bitrate::ConstantBitrate(requested_bitrate ? requested_bitrate->target_bps() : default_bitrate);
}

VideoEncodeAccelerator::Config SetUpVeaConfig(VideoCodecProfile profile, const VideoEncoder::Options& opts, VideoPixelFormat format,
    VideoFrame::StorageType storage_type, VideoEncodeAccelerator::SupportedRateControlMode supported_rc_modes,
    VideoEncodeAccelerator::Config::EncoderType required_encoder_type, bool is_gpu_supported_format)
{
    Bitrate bitrate = CreateBitrate(opts.bitrate, opts.frame_size, supported_rc_modes);
    auto config = VideoEncodeAccelerator::Config(format, opts.frame_size, profile, bitrate, opts.framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate),
        VideoEncodeAccelerator::Config::StorageType::kShmem, VideoEncodeAccelerator::Config::ContentType::kCamera);
    ...
    if (opts.latency_mode == VideoEncoder::LatencyMode::Realtime) {
        config.drop_frame_thresh_percentage = GetDefaultVideoEncoderDropFrameThreshold();
    }
    ...
    if (opts.scalability_mode) {
        ...
        case SVCScalabilityMode::kL1T3:
            ...
            break;
        default:
            NOTREACHED() << "Unsupported SVC: " << GetScalabilityModeName(opts.scalability_mode.value());
        }
    }
    ...
    config.require_low_delay = opts.latency_mode == VideoEncoder::LatencyMode::Realtime;
    config.required_encoder_type = required_encoder_type;

    const bool is_rgb = format == PIXEL_FORMAT_XBGR || format == PIXEL_FORMAT_XRGB || format == PIXEL_FORMAT_ABGR || format == PIXEL_FORMAT_ARGB;
    ...
    if (format != PIXEL_FORMAT_I420 || !VideoFrame::IsStorageTypeMappable(storage_type)) {
        ...
        config.storage_type = VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer;
    }
#endif
...
class VideoEncodeAcceleratorAdapter::GpuMemoryBufferVideoFramePool : public base::RefCountedThreadSafe<GpuMemoryBufferVideoFramePool> {
public:
    GpuMemoryBufferVideoFramePool(GpuVideoAcceleratorFactories* gpu_factories, const gfx::Size& coded_size)
        : gpu_factories_(gpu_factories)
        , coded_size_(coded_size)
    {
    }
    GpuMemoryBufferVideoFramePool(const GpuMemoryBufferVideoFramePool&) = delete;
    GpuMemoryBufferVideoFramePool& operator=(const GpuMemoryBufferVideoFramePool&) = delete;

    scoped_refptr<VideoFrame> MaybeCreateVideoFrame(const gfx::Size& visible_size)
    {
        ...
        if (available_shared_images_.empty()) {
            ...
            auto shared_image
                = sii->CreateSharedImage({ si_format, coded_size_, gfx::ColorSpace(), gpu::SharedImageUsageSet(si_usage), "VideoEncodeAcceleratorAdapter" },
        ...
        auto shared_image_release_cb = base::BindPostTaskToCurrentDefault(base::BindOnce(&GpuMemoryBufferVideoFramePool::ReuseFrame, this, shared_image));
        video_frame = media::VideoFrame::WrapMappableSharedImage(
            std::move(shared_image), sync_token, std::move(shared_image_release_cb), gfx::Rect(visible_size), visible_size, base::TimeDelta());
        return video_frame;
    }
...
private:
    ...
    // otherwise.
    void ReuseFrame(scoped_refptr<gpu::ClientSharedImage> shared_image, const gpu::SyncToken& token, std::unique_ptr<gfx::GpuMemoryBuffer> gpu_memory_buffer)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
class VideoEncodeAcceleratorAdapter::ReadOnlyRegionPool : public base::RefCountedThreadSafe<ReadOnlyRegionPool> {
public:
    struct Handle {
        using ReuseBufferCallback = base::OnceCallback<void(std::unique_ptr<base::MappedReadOnlyRegion>)>;
        Handle(std::unique_ptr<base::MappedReadOnlyRegion> mapped_region, ReuseBufferCallback reuse_buffer_cb)
    ...
    explicit ReadOnlyRegionPool(size_t buffer_size)
    ...
    std::unique_ptr<Handle> MaybeAllocateBuffer()
    {
        ...
        if (available_buffers_.empty()) {
            available_buffers_.push_back(std::make_unique<base::MappedReadOnlyRegion>(base::ReadOnlySharedMemoryRegion::Create(buffer_size_)));
        ...
        return std::make_unique<Handle>(std::move(mapped_region), base::BindPostTaskToCurrentDefault(base::BindOnce(&ReadOnlyRegionPool::ReuseBuffer, this)));
    }
...
VideoEncodeAcceleratorAdapter::VideoEncodeAcceleratorAdapter(GpuVideoAcceleratorFactories* gpu_factories, std::unique_ptr<MediaLog> media_log,
    scoped_refptr<base::SequencedTaskRunner> callback_task_runner, VideoEncodeAccelerator::Config::EncoderType required_encoder_type)
...
void VideoEncodeAcceleratorAdapter::DestroyAsync(std::unique_ptr<VideoEncodeAcceleratorAdapter> self)
{
    DCHECK(self);
...
void VideoEncodeAcceleratorAdapter::SetInputBufferPreferenceForTesting(InputBufferKind pref)
{
    input_buffer_preference_ = pref;
}

void VideoEncodeAcceleratorAdapter::Initialize(
    VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb)
{
    DCHECK(!accelerator_task_runner_->RunsTasksInCurrentSequence());
    accelerator_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAcceleratorAdapter::InitializeOnAcceleratorThread, base::Unretained(this), profile, options,
            WrapCallback(std::move(info_cb)), WrapCallback(std::move(output_cb)), WrapCallback(std::move(done_cb))));
}
    ...
    VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (state_ != State::kNotInitialized) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializeTwice, "Encoder has already been initialized."));
        return;
    }

    accelerator_ = gpu_factories_->CreateVideoEncodeAccelerator();
    if (!accelerator_) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Failed to create video encode accelerator."));
        return;
    }

    if (options.frame_size.width() <= 0 || options.frame_size.height() <= 0) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Negative width or height values."));
        return;
    }

    if (!options.frame_size.GetCheckedArea().IsValid()) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Frame is too large."));
        return;
    }

    auto supported_profiles = gpu_factories_->GetVideoEncodeAcceleratorSupportedProfiles();
    if (!supported_profiles) {
        InitCompleted(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedProfile, "No profile is supported by video encode accelerator."));
        return;
    }

    auto supported_rc_modes = VideoEncodeAccelerator::SupportedRateControlMode::kNoMode;
    ...
    for (const auto& supported_profile : *supported_profiles) {
        if (supported_profile.profile == profile) {
            ...
            gpu_supported_pixel_formats = supported_profile.gpu_supported_pixel_formats;
            break;
        }
    }

    if (supported_rc_modes == VideoEncodeAccelerator::SupportedRateControlMode::kNoMode) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedProfile, "The profile is not supported by video encode accelerator.")
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
    if (profile_ >= H264PROFILE_MIN && profile_ <= H264PROFILE_MAX && !options_.avc.produce_annexb) {
...
void VideoEncodeAcceleratorAdapter::InitializeInternalOnAcceleratorThread()
{
    ...
    const bool is_rgb = format == PIXEL_FORMAT_XBGR || format == PIXEL_FORMAT_XRGB || format == PIXEL_FORMAT_ABGR || format == PIXEL_FORMAT_ARGB;
    const bool supported_format = format == PIXEL_FORMAT_NV12 || format == PIXEL_FORMAT_I420 || is_rgb;
    const bool is_gpu_frame = first_frame->HasSharedImage() || first_frame->HasNativeGpuMemoryBuffer();
    const bool is_gpu_supported_format = is_gpu_frame && base::ranges::find(gpu_supported_pixel_formats_, format) != gpu_supported_pixel_formats_.end();
    if (!supported_format && !is_gpu_supported_format) {
        InitCompleted(
            EncoderStatus(EncoderStatus::Codes::kUnsupportedFrameFormat, "Unexpected frame format.").WithData("frame", first_frame->AsHumanReadableString()));
        return;
    }

    auto vea_config
        = SetUpVeaConfig(profile_, options_, format, first_frame->storage_type(), supported_rc_modes_, required_encoder_type_, is_gpu_supported_format);
    ...
    if (input_buffer_preference_ == InputBufferKind::Any) {
        if (vea_config.storage_type == VideoEncodeAccelerator::Config::StorageType::kGpuMemoryBuffer) {
    ...
    if (!accelerator_->Initialize(vea_config, this, media_log_->Clone())) {
        InitCompleted(EncoderStatus(EncoderStatus::Codes::kEncoderInitializationError, "Failed to initialize video encode accelerator."));
        return;
    }
...
void VideoEncodeAcceleratorAdapter::Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb)
{
    DCHECK(!accelerator_task_runner_->RunsTasksInCurrentSequence());
    accelerator_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAcceleratorAdapter::EncodeOnAcceleratorThread, base::Unretained(this), std::move(frame), encode_options,
            WrapCallback(std::move(done_cb))));
}

void VideoEncodeAcceleratorAdapter::EncodeOnAcceleratorThread(scoped_refptr<VideoFrame> frame, EncodeOptions encode_options, EncoderStatusCB done_cb)
{
    TRACE_EVENT1("media", "VideoEncodeAcceleratorAdapter::EncodeOnAcceleratorThread", "timestamp", frame->timestamp());
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);

    if (state_ == State::kWaitingForFirstFrame || state_ == State::kInitializing) {
    ...
    if (state_ != State::kReadyToEncode) {
        std::move(done_cb).Run(
            EncoderStatus(EncoderStatus::Codes::kEncoderIllegalState, "Encoder can't encode now.").WithData("state", static_cast<int32_t>(state_)));
        return;
    }

    const bool frame_needs_resizing = frame->visible_rect().size() != options_.frame_size || frame->coded_size() != input_coded_size_;
    ...
    bool use_gpu_buffer = frame->HasMappableGpuBuffer() && (!frame_needs_resizing || gpu_resize_supported_);
    ...
    if (!result.has_value()) {
        std::move(done_cb).Run(std::move(result).error().WithData("frame", frame->AsHumanReadableString()).AddHere());
        return;
    }
...
void VideoEncodeAcceleratorAdapter::ChangeOptions(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    DCHECK(!accelerator_task_runner_->RunsTasksInCurrentSequence());
    accelerator_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoEncodeAcceleratorAdapter::ChangeOptionsOnAcceleratorThread, base::Unretained(this), options, WrapCallback(std::move(output_cb)),
            WrapCallback(std::move(done_cb))));
}

void VideoEncodeAcceleratorAdapter::ChangeOptionsOnAcceleratorThread(const Options options, OutputCB output_cb, EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
    DCHECK(active_encodes_.empty());
    DCHECK(pending_encodes_.empty());
    CHECK(state_ == State::kReadyToEncode || state_ == State::kWaitingForFirstFrame);

    if (options.bitrate && options_.bitrate && options.bitrate->mode() != options_.bitrate->mode()) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Bitrate mode change is not supported.")
                                   .WithData("current bitrate", static_cast<int32_t>(options_.bitrate->mode()))
                                   .WithData("new bitrate", static_cast<int32_t>(options.bitrate->mode())));
        return;
    }
    ...
    if (options.frame_size != options_.frame_size) {
        if (supports_frame_size_change_) {
            ...
            auto status = EncoderStatus(EncoderStatus::Codes::kEncoderUnsupportedConfig, "Resolution change is not supported.");
            std::move(done_cb).Run(status);
            return;
        }
    }

    Bitrate bitrate = CreateBitrate(options.bitrate, options.frame_size, supported_rc_modes_);
    uint32_t framerate = base::ClampRound<uint32_t>(options.framerate.value_or(VideoEncodeAccelerator::kDefaultFramerate));
    ...
    accelerator_->RequestEncodingParametersChange(bitrate, framerate, new_frame_size);
...
void VideoEncodeAcceleratorAdapter::Flush(EncoderStatusCB done_cb)
{
    ...
    accelerator_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VideoEncodeAcceleratorAdapter::FlushOnAcceleratorThread, base::Unretained(this), WrapCallback(std::move(done_cb))));
}

void VideoEncodeAcceleratorAdapter::FlushOnAcceleratorThread(EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
    if (state_ == State::kWaitingForFirstFrame) {
    ...
    if (state_ != State::kReadyToEncode && state_ != State::kInitializing) {
        std::move(done_cb).Run(EncoderStatus(EncoderStatus::Codes::kEncoderFailedFlush, "Encoder can't flush now"));
        return;
    }
    ...
    if (state_ == State::kFlushing && flush_support_.value()) {
        accelerator_->Flush(base::BindOnce(&VideoEncodeAcceleratorAdapter::FlushCompleted, base::Unretained(this)));
    }
}

void VideoEncodeAcceleratorAdapter::RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
    ...
    for (int id = 0; id < kOutputBufferNumber; id++) {
        ...
        if (!handle) {
            if (state_ == State::kInitializing) {
                ...
                std::move(reconfigure_cb_).Run({ EncoderStatus::Codes::kOutOfMemoryError, "Failed to allocate buffer" });
        ...
        accelerator_->UseOutputBitstreamBuffer(BitstreamBuffer(id, region.Duplicate(), region.GetSize()));
...
void VideoEncodeAcceleratorAdapter::BitstreamBufferReady(int32_t buffer_id, const BitstreamBufferMetadata& metadata)
{
    ...
    if (buffer_id < 0 || buffer_id >= static_cast<int>(output_buffer_handles_.size())) {
        NotifyErrorStatus({ EncoderStatus::Codes::kInvalidOutputBuffer, "Buffer id is out of bounds: " + base::NumberToString(buffer_id) });
    }
    if (!output_buffer_handles_[buffer_id]) {
        NotifyErrorStatus({ EncoderStatus::Codes::kInvalidOutputBuffer, "Invalid output buffer" });
    }

    const base::WritableSharedMemoryMapping& mapping = output_buffer_handles_[buffer_id]->GetMapping();
    ...
    if (metadata.payload_size_bytes > 0) {
        ...
        if (h264_converter_) {
            do {
                status = h264_converter_->ConvertChunk(src, dst, &config_changed, &actual_output_size);
                if (status.code() == MP4Status::Codes::kBufferTooSmall) {
                    ...
                    NotifyErrorStatus(
                        EncoderStatus(EncoderStatus::Codes::kBitstreamConversionError, "Failed to convert a buffer to h264 chunk").AddCause(std::move(status)));
                    return;
                }
            } while (!status.is_ok());
            ...
            if (config_changed) {
                ...
                if (!config.Serialize(desc.value())) {
                    NotifyErrorStatus({ media::EncoderStatus::Codes::kBitstreamConversionError, "Failed to get h264 config" });
                    return;
                }
            }
        } else {
            ...
            if (h265_converter_) {
                do {
                    status = h265_converter_->ConvertChunk(src, dst, &config_changed, &actual_output_size);
                    if (status.code() == MP4Status::Codes::kBufferTooSmall) {
                        dst = base::HeapArray<uint8_t>::Uninit(actual_output_size);
                        continue;
                    } else if (!status.is_ok()) {
                        NotifyErrorStatus(EncoderStatus(EncoderStatus::Codes::kBitstreamConversionError, "Failed to convert a buffer to h265 chunk")
                ...
                if (config_changed) {
                    ...
                    desc = CodecDescription();
                    if (!config.Serialize(desc.value())) {
                        NotifyErrorStatus({ media::EncoderStatus::Codes::kBitstreamConversionError, "Failed to get h265 config" });
                        return;
                    }
                }
            }
    ...
    const base::UnsafeSharedMemoryRegion& region = output_buffer_handles_[buffer_id]->GetRegion();
    accelerator_->UseOutputBitstreamBuffer(BitstreamBuffer(buffer_id, region.Duplicate(), region.GetSize()));
    ...
    for (auto it = active_encodes_.begin(); it != active_encodes_.end(); ++it) {
        if ((*it)->timestamp == result.timestamp) {
            result.color_space = metadata.encoded_color_space.value_or((*it)->color_space);
...
void VideoEncodeAcceleratorAdapter::NotifyErrorStatus(const EncoderStatus& status)
{
    CHECK(!status.is_ok());
    MEDIA_LOG(ERROR, media_log_) << "VEA adapter error. Code: " << static_cast<int32_t>(status.code()) << ". Message: " << status.message();
    if (state_ == State::kInitializing) {
...
void VideoEncodeAcceleratorAdapter::NotifyEncoderInfoChange(const VideoEncoderInfo& info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(accelerator_sequence_checker_);
...
void VideoEncodeAcceleratorAdapter::InitCompleted(EncoderStatus status)
{
    ...
    for (auto& encode : pending_encodes_) {
        EncodeOnAcceleratorThread(std::move(encode->frame), encode->options, std::move(encode->done_callback));
    }
    pending_encodes_.clear();
...
void VideoEncodeAcceleratorAdapter::FlushCompleted(bool success)
{
    ...
    std::move(pending_flush_->done_callback).Run(success ? EncoderStatus::Codes::kOk : EncoderStatus::Codes::kEncoderFailedFlush);
    pending_flush_.reset();
    state_ = State::kReadyToEncode;
}

template <class T> T VideoEncodeAcceleratorAdapter::WrapCallback(T cb)
{
...
EncoderStatus::Or<scoped_refptr<VideoFrame>> VideoEncodeAcceleratorAdapter::PrepareCpuFrame(scoped_refptr<VideoFrame> src_frame)
{
    ...
    if (src_frame->storage_type() == VideoFrame::STORAGE_SHMEM && src_frame->format() == PIXEL_FORMAT_I420 && src_frame->visible_rect() == dest_visible_rect
    ...
    if (!input_pool_) {
        const size_t input_buffer_size = VideoFrame::AllocationSize(PIXEL_FORMAT_I420, dest_coded_size);
        input_pool_ = base::MakeRefCounted<ReadOnlyRegionPool>(input_buffer_size);
    }

    std::unique_ptr<ReadOnlyRegionPool::Handle> handle = input_pool_->MaybeAllocateBuffer();
    ...
    auto mapped_src_frame = src_frame->HasMappableGpuBuffer() ? ConvertToMemoryMappedFrame(src_frame) : src_frame;
    auto shared_frame = VideoFrame::WrapExternalData(PIXEL_FORMAT_I420, dest_coded_size, dest_visible_rect, dest_visible_rect.size(),
        static_cast<const uint8_t*>(mapping->memory()), mapping->size(), src_frame->timestamp());

    if (!shared_frame || !mapped_src_frame)
        return EncoderStatus(EncoderStatus::Codes::kSystemAPICallError);

    auto status = frame_converter_.ConvertAndScale(*mapped_src_frame, *shared_frame);
    if (!status.is_ok()) {
        return status;
    }

    shared_frame->BackWithSharedMemory(handle->region());
    shared_frame->AddDestructionObserver(base::DoNothingWithBoundArgs(std::move(handle)));
    return shared_frame;
}
...
EncoderStatus::Or<scoped_refptr<VideoFrame>> VideoEncodeAcceleratorAdapter::PrepareGpuFrame(scoped_refptr<VideoFrame> src_frame)
{
    ...
    const auto dest_coded_size = input_coded_size_;
    const auto dest_visible_rect = gfx::Rect(options_.frame_size);
    bool is_gpu_supported_format = base::ranges::find(gpu_supported_pixel_formats_, src_frame->format()) != gpu_supported_pixel_formats_.end();

    if (src_frame->HasMappableGpuBuffer() && (src_frame->format() == PIXEL_FORMAT_NV12 || is_gpu_supported_format)
    ...
    if (!gmb_frame_pool_) {
        gmb_frame_pool_ = base::MakeRefCounted<GpuMemoryBufferVideoFramePool>(gpu_factories_, dest_coded_size);
    }

    auto gpu_frame = gmb_frame_pool_->MaybeCreateVideoFrame(dest_visible_rect.size());
    if (!gpu_frame)
        return EncoderStatus(EncoderStatus::Codes::kOutOfMemoryError);
    ...
    auto status = frame_converter_.ConvertAndScale(*mapped_src_frame, *mapped_gpu_frame);
    if (!status.is_ok()) {
        return status;
    }
    ...
    // ConvertAndScale().
#if BUILDFLAG(IS_MAC)
    gpu_frame->shared_image()->SetColorSpaceOnNativeBuffer(mapped_gpu_frame->ColorSpace());
#endif
