...
AlphaVideoEncoderWrapper::AlphaVideoEncoderWrapper(std::unique_ptr<VideoEncoder> yuv_encoder, std::unique_ptr<VideoEncoder> alpha_encoder)
...
void AlphaVideoEncoderWrapper::Initialize(VideoCodecProfile profile, const Options& options, EncoderInfoCB info_cb, OutputCB output_cb, EncoderStatusCB done_cb)
{
    ...
    if (options.bitrate.has_value()) {
        ...
        case Bitrate::Mode::kConstant:
            yuv_options.bitrate = Bitrate::ConstantBitrate(bitrate.target_bps() / 4 * 3);
            alpha_options.bitrate = Bitrate::ConstantBitrate(bitrate.target_bps() / 4);
            break;
        case Bitrate::Mode::kVariable:
            yuv_options.bitrate = Bitrate::VariableBitrate(bitrate.target_bps() / 4 * 3, bitrate.peak_bps() / 4 * 3);
            alpha_options.bitrate = Bitrate::VariableBitrate(bitrate.target_bps() / 4, bitrate.peak_bps() / 4);
            break;
    ...
    auto done_callback = [](base::WeakPtr<AlphaVideoEncoderWrapper> self, EncoderStatus status) {
        if (!self) {
    ...
    yuv_encoder_->Initialize(profile, yuv_options, std::move(info_cb),
        base::BindRepeating(&AlphaVideoEncoderWrapper::YuvOutputCallback, weak_factory_.GetWeakPtr()),
        base::BindOnce(done_callback, weak_factory_.GetWeakPtr()));

    alpha_encoder_->Initialize(profile, alpha_options, EncoderInfoCB(),
        base::BindRepeating(&AlphaVideoEncoderWrapper::AlphaOutputCallback, weak_factory_.GetWeakPtr()),
        base::BindOnce(done_callback, weak_factory_.GetWeakPtr()));
...
void AlphaVideoEncoderWrapper::Encode(scoped_refptr<VideoFrame> frame, const EncodeOptions& encode_options, EncoderStatusCB done_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    done_cb = BindCallbackToCurrentLoopIfNeeded(std::move(done_cb));
    ...
    auto dummy_plane_size = VideoFrame::PlaneSize(frame->format(), VideoFrame::Plane::kV, frame_size).Area64();
    ...
    auto uv_stride = VideoFrame::RowBytes(VideoFrame::Plane::kU, frame->format(), frame_size.width());

    auto yuv_frame = WrapAsI420VideoFrame(frame);
    auto alpha_frame = VideoFrame::WrapExternalYuvData(PIXEL_FORMAT_I420, frame->visible_rect().size(), frame->visible_rect(), frame->natural_size(),
        frame->stride(VideoFrame::Plane::kA), uv_stride, uv_stride, frame->visible_data(VideoFrame::Plane::kA), dummy_uv_planes_.data(),
        dummy_uv_planes_.data(), frame->timestamp());
    ...
    yuv_encoder_->Encode(yuv_frame, encode_options, base::BindOnce(done_callback, weak_factory_.GetWeakPtr()));
    alpha_encoder_->Encode(alpha_frame, encode_options, base::BindOnce(done_callback, weak_factory_.GetWeakPtr()));
...
void AlphaVideoEncoderWrapper::ChangeOptions(const Options& options, OutputCB output_cb, EncoderStatusCB done_cb)
{
...
void AlphaVideoEncoderWrapper::YuvOutputCallback(VideoEncoderOutput output, std::optional<CodecDescription> desc)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (desc.has_value()) {
        NOTREACHED() << "AlphaVideoEncoderWrapper doesn't support codecs with extra data";
    }
    yuv_output_.emplace(std::move(output));
}
void AlphaVideoEncoderWrapper::AlphaOutputCallback(VideoEncoderOutput output, std::optional<CodecDescription> desc)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (desc.has_value()) {
        NOTREACHED() << "AlphaVideoEncoderWrapper doesn't support codecs with extra data";
