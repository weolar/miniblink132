...
#define EQ_OR_RETURN(shdr1, shdr2, field)                                                                                                                      \
    do {                                                                                                                                                       \
        if ((shdr1->field) != (shdr2->field)) {                                                                                                                \
            DVLOG(1) << "Error in stream, slice header fields must match for: " << #field;                                                                     \
            return kInvalidStream;                                                                                                                             \
        }                                                                                                                                                      \
    } while (0)
        ...
        if (shdr->nal_unit_type != H265NALU::IDR_W_RADL && shdr->nal_unit_type != H265NALU::IDR_N_LP) {
            ...
            if (sps->long_term_ref_pics_present_flag) {
                ...
                if (nalu.nuh_layer_id == 0) {
                    TRUE_OR_RETURN(shdr->num_long_term_pics <= (sps->sps_max_dec_pic_buffering_minus1[pps->temporal_id]
    ...
    for (int size_id = 0; size_id < 4; ++size_id) {
        for (int matrix_id = 0; matrix_id < 6; matrix_id += (size_id == 3) ? 3 : 1) {
            ...
            if (!scaling_list_pred_mode_flag) {
                ...
                if (size_id > 1) {
                    if (size_id == 2) {
                        READ_SE_OR_RETURN(&scaling_list_data->scaling_list_dc_coef_16x16[matrix_id]);
                        IN_RANGE_OR_RETURN(scaling_list_data->scaling_list_dc_coef_16x16[matrix_id], -7, 247);
                        ...
                        READ_SE_OR_RETURN(&scaling_list_data->scaling_list_dc_coef_32x32[matrix_id]);
                        IN_RANGE_OR_RETURN(scaling_list_data->scaling_list_dc_coef_32x32[matrix_id], -7, 247);
                        // This is parsed as minus8;
...
H265Parser::Result H265Parser::ParseSEI(H265SEIMessage* sei_msg)
{
    ...
    case H265SEIMessage::kSEIAlphaChannelInfo:
        READ_BOOL_OR_RETURN(&sei_msg->alpha_channel_info.alpha_channel_cancel_flag);
        if (!sei_msg->alpha_channel_info.alpha_channel_cancel_flag) {
            READ_BITS_OR_RETURN(3, &sei_msg->alpha_channel_info.alpha_channel_use_idc);
            READ_BITS_OR_RETURN(3, &sei_msg->alpha_channel_info.alpha_channel_bit_depth_minus8);
            READ_BITS_OR_RETURN(sei_msg->alpha_channel_info.alpha_channel_bit_depth_minus8 + 9, &sei_msg->alpha_channel_info.alpha_transparent_value);
            READ_BITS_OR_RETURN(sei_msg->alpha_channel_info.alpha_channel_bit_depth_minus8 + 9, &sei_msg->alpha_channel_info.alpha_opaque_value);
            READ_BOOL_OR_RETURN(&sei_msg->alpha_channel_info.alpha_channel_incr_flag);
            READ_BOOL_OR_RETURN(&sei_msg->alpha_channel_info.alpha_channel_clip_flag);
            if (sei_msg->alpha_channel_info.alpha_channel_clip_flag) {
                READ_BOOL_OR_RETURN(&sei_msg->alpha_channel_info.alpha_channel_clip_type_flag);
            }
        }
        break;
    case H265SEIMessage::kSEIContentLightLevelInfo:
        READ_BITS_OR_RETURN(16, &sei_msg->content_light_level_info.max_content_light_level);
        READ_BITS_OR_RETURN(16, &sei_msg->content_light_level_info.max_picture_average_light_level);
        break;
