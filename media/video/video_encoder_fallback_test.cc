...
protected:
    void SetUp() override
    {
        ...
        fallback_encoder_ = std::make_unique<VideoEncoderFallback>(
            std::move(main_video_encoder), base::BindOnce(&VideoEncoderFallbackTest::CreateSecondaryEncoder, base::Unretained(this)));
    ...
    media::EncoderStatus::Or<std::unique_ptr<media::VideoEncoder>> CreateSecondaryEncoder()
    {
        if (!secondary_video_encoder_holder_) {
            return EncoderStatus::Codes::kEncoderInitializationError;
        }
        return std::unique_ptr<VideoEncoder>(secondary_video_encoder_holder_.release());
    }

    bool FallbackHappened()
    {
        return !secondary_video_encoder_holder_;
    }

    void RunStatusCallbackAync(VideoEncoder::EncoderStatusCB callback, EncoderStatus::Codes code = EncoderStatus::Codes::kOk)
    {
        base::BindPostTask(callback_runner_, std::move(callback)).Run(code);
    }

    VideoEncoder::EncoderStatusCB ValidatingStatusCB(EncoderStatus::Codes code = EncoderStatus::Codes::kOk, base::Location loc = FROM_HERE)
    {
        struct CallEnforcer {
            bool called = false;
            std::string location;
        ...
        return base::BindPostTaskToCurrentDefault(base::BindLambdaForTesting([code, this, enforcer { std::move(enforcer) }](EncoderStatus s) {
            EXPECT_TRUE(callback_runner_->RunsTasksInCurrentSequence());
            EXPECT_EQ(s.code(), code) << " Callback created: " << enforcer->location;
    ...
    raw_ptr<MockVideoEncoder, AcrossTasksDanglingUntriaged> secondary_video_encoder_;
...
TEST_F(VideoEncoderFallbackTest, NoFallbackEncoding)
{
    ...
    VideoEncoder::EncoderInfoCB info_cb
        = base::BindPostTaskToCurrentDefault(base::BindLambdaForTesting([&](const VideoEncoderInfo& info) { info_cb_count++; }));

    int outputs = 0;
    VideoEncoder::OutputCB output_cb
        = base::BindPostTaskToCurrentDefault(base::BindLambdaForTesting([&](VideoEncoderOutput, std::optional<VideoEncoder::CodecDescription>) { outputs++; }));
    VideoEncoder::OutputCB saved_output_cb;

    EXPECT_CALL(*main_video_encoder_, Initialize(_, _, _, _, _))
        .WillOnce(Invoke([&, this](VideoCodecProfile profile, const VideoEncoder::Options& options, VideoEncoder::EncoderInfoCB info_cb,
                             VideoEncoder::OutputCB output_cb, VideoEncoder::EncoderStatusCB done_cb) {
    ...
    EXPECT_CALL(*main_video_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&, this](scoped_refptr<VideoFrame> frame, const VideoEncoder::EncodeOptions& options, VideoEncoder::EncoderStatusCB done_cb) {
    ...
    fallback_encoder_->Initialize(profile, options, std::move(info_cb), std::move(output_cb), ValidatingStatusCB());

    RunLoop();
    for (int i = 0; i < kFrameCount; i++) {
        auto frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, kFrameSize, gfx::Rect(kFrameSize), kFrameSize, base::Seconds(i));
        fallback_encoder_->Encode(frame, VideoEncoder::EncodeOptions(true), ValidatingStatusCB());
    }
    RunLoop();
...
TEST_F(VideoEncoderFallbackTest, FallbackOnInitialize)
{
    ...
    EXPECT_CALL(*main_video_encoder_, Initialize(_, _, _, _, _))
        .WillOnce(Invoke(
            [&, this](VideoCodecProfile profile, const VideoEncoder::Options& options, VideoEncoder::EncoderInfoCB info_cb, VideoEncoder::OutputCB output_cb,
...
TEST_F(VideoEncoderFallbackTest, SecondaryFailureOnInitialize)
{
    ...
    EXPECT_CALL(*main_video_encoder_, Initialize(_, _, _, _, _))
        .WillOnce(Invoke(
            [&, this](VideoCodecProfile profile, const VideoEncoder::Options& options, VideoEncoder::EncoderInfoCB info_cb, VideoEncoder::OutputCB output_cb,
                VideoEncoder::EncoderStatusCB done_cb) { RunStatusCallbackAync(std::move(done_cb), EncoderStatus::Codes::kEncoderUnsupportedProfile); }));

    // Initialize() on the second encoder should also fail
    EXPECT_CALL(*secondary_video_encoder_, Initialize(_, _, _, _, _))
        .WillOnce(Invoke(
            [&, this](VideoCodecProfile profile, const VideoEncoder::Options& options, VideoEncoder::EncoderInfoCB info_cb, VideoEncoder::OutputCB output_cb,
                VideoEncoder::EncoderStatusCB done_cb) { RunStatusCallbackAync(std::move(done_cb), EncoderStatus::Codes::kEncoderUnsupportedCodec); }));

    fallback_encoder_->Initialize(profile, options, /*info_cb=*/base::DoNothing(),
        /*output_cb=*/base::DoNothing(), ValidatingStatusCB(EncoderStatus::Codes::kEncoderUnsupportedCodec));
    RunLoop();

    EXPECT_CALL(*secondary_video_encoder_, Encode(_, _, _))
        .Times(kFrameCount)
        .WillRepeatedly(Invoke([&, this](scoped_refptr<VideoFrame> frame, const VideoEncoder::EncodeOptions& options, VideoEncoder::EncoderStatusCB done_cb) {
            RunStatusCallbackAync(std::move(done_cb), EncoderStatus::Codes::kEncoderUnsupportedCodec);
        }));
    ...
    for (int i = 0; i < kFrameCount; i++) {
        ...
        fallback_encoder_->Encode(frame, VideoEncoder::EncodeOptions(true), std::move(done_callback));
    }
...
TEST_F(VideoEncoderFallbackTest, SecondaryFailureOnEncode)
{
    ...
    fallback_encoder_->Initialize(profile, options, /*info_cb=*/base::DoNothing(), std::move(output_cb), ValidatingStatusCB());
    RunLoop();

    for (int i = 1; i <= kFrameCount; i++) {
        auto frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, kFrameSize, gfx::Rect(kFrameSize), kFrameSize, base::Seconds(i));
        auto done_cb
            = ValidatingStatusCB((frame->timestamp() <= second_encoder_fail_time) ? EncoderStatus::Codes::kOk : EncoderStatus::Codes::kEncoderFailedEncode);
        fallback_encoder_->Encode(frame, VideoEncoder::EncodeOptions(true), std::move(done_cb));
    }
    RunLoop();
    EXPECT_TRUE(FallbackHappened());
...
TEST_F(VideoEncoderFallbackTest, SecondaryFailureOnCreation)
{
    ...
    EXPECT_CALL(*main_video_encoder_, Encode(_, _, _))
        .WillRepeatedly(Invoke([&, this](scoped_refptr<VideoFrame> frame, const VideoEncoder::EncodeOptions& options, VideoEncoder::EncoderStatusCB done_cb) {
            ...
            EXPECT_TRUE(done_cb);
            if (frame->timestamp() > encoder_switch_time) {
                RunStatusCallbackAync(std::move(done_cb), EncoderStatus::Codes::kEncoderFailedEncode);
                return;
            }
    ...
    for (int i = 1; i <= kFrameCount; i++) {
        auto frame = VideoFrame::CreateFrame(PIXEL_FORMAT_I420, kFrameSize, gfx::Rect(kFrameSize), kFrameSize, base::Seconds(i));
        auto done_cb = ValidatingStatusCB((frame->timestamp() <= encoder_switch_time) ? EncoderStatus::Codes::kOk : EncoderStatus::Codes::kEncoderFailedEncode);
        fallback_encoder_->Encode(frame, VideoEncoder::EncodeOptions(true), std::move(done_cb));
