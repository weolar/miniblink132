...
std::unique_ptr<HalfFloatMaker> HalfFloatMaker::NewHalfFloatMaker(int bits_per_channel)
{
    if (bits_per_channel < 11) {
        return std::unique_ptr<HalfFloatMaker>(new HalfFloatMaker_xor(bits_per_channel));
    } else {
        return std::unique_ptr<HalfFloatMaker>(new HalfFloatMaker_libyuv(bits_per_channel));
    }
}
