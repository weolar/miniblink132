...
public:
    ...
    MockGpuVideoAcceleratorFactories(const MockGpuVideoAcceleratorFactories&) = delete;
    MockGpuVideoAcceleratorFactories& operator=(const MockGpuVideoAcceleratorFactories&) = delete;
    ...
    MOCK_METHOD1(GetChannelToken, void(gpu::mojom::GpuChannel::GetChannelTokenCallback));
    ...
    MOCK_METHOD2(CreateVideoDecoder, std::unique_ptr<media::VideoDecoder>(MediaLog*, RequestOverlayInfoCB));

    MOCK_METHOD0(GetVideoEncodeAcceleratorSupportedProfiles, std::optional<VideoEncodeAccelerator::SupportedProfiles>());
    MOCK_METHOD0(GetSupportedVideoDecoderConfigs, std::optional<media::SupportedVideoDecoderConfigs>());
    ...
    bool ShouldUseGpuMemoryBuffersForVideoFrames(bool for_media_stream) const override;
    ...
    std::unique_ptr<VideoEncodeAccelerator> CreateVideoEncodeAccelerator() override;

    const std::vector<raw_ptr<gfx::GpuMemoryBuffer, VectorExperimental>>& created_memory_buffers()
    {
...
private:
    ...
    std::vector<raw_ptr<gfx::GpuMemoryBuffer, VectorExperimental>> created_memory_buffers_;
};
