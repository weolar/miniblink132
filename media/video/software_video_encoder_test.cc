...
class SoftwareVideoEncoderTest : public ::testing::TestWithParam<SwVideoTestParams> {
public:
    ...
    void PrepareDecoder(gfx::Size size, VideoDecoder::OutputCB output_cb, std::vector<uint8_t> extra_data = std::vector<uint8_t>())
    {
        VideoDecoderConfig config(codec_, profile_, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::JPEG(), VideoTransformation(), size,
            gfx::Rect(size), size, extra_data, EncryptionScheme::kUnencrypted);
        ...
        decoder_->Initialize(config, false, nullptr, DecoderStatusCB(), std::move(output_cb), base::NullCallback());
        RunUntilIdle();
    }
    ...
    scoped_refptr<VideoFrame> CreateFrame(gfx::Size size, VideoPixelFormat format, base::TimeDelta timestamp, std::optional<uint32_t> xor_mask = std::nullopt)
    {
        if (xor_mask) {
            auto frame = frame_pool_.CreateFrame(format, size, gfx::Rect(size), size, timestamp);
            FillFourColors(*frame, xor_mask);
            return frame;
        }

        if (!four_colors_frame_ || format != four_colors_frame_->format() || size != four_colors_frame_->coded_size()) {
            four_colors_frame_.reset();
            four_colors_frame_ = frame_pool_.CreateFrame(format, size, gfx::Rect(size), size, timestamp);
            FillFourColors(*four_colors_frame_);
        }

        auto wrapped_frame = VideoFrame::WrapVideoFrame(four_colors_frame_, format, gfx::Rect(size), size);
...
#if BUILDFLAG(ENABLE_LIBVPX)
            if (profile_ == VP9PROFILE_PROFILE2 || profile_ == VP9PROFILE_PROFILE3) {
    ...
    VideoEncoder::EncoderStatusCB ValidatingStatusCB(bool quit_run_loop_on_call = false, base::Location loc = FROM_HERE)
    {
        struct CallEnforcer {
            bool called = false;
            std::string location;
        ...
        auto check_callback = base::BindLambdaForTesting([enforcer { std::move(enforcer) }](EncoderStatus s) {
            ASSERT_TRUE(s.is_ok()) << " Callback created: " << enforcer->location << " Code: " << std::hex << static_cast<StatusCodeType>(s.code())
                                   << " Error: " << s.message();
    ...
    VideoEncoder::EncoderStatusCB ValidateStatusThenQuitCB(base::Location loc = FROM_HERE)
    {
    ...
    VideoDecoder::DecodeCB DecoderStatusCB(base::Location loc = FROM_HERE)
    {
        ...
        auto enforcer = std::make_unique<CallEnforcer>();
        enforcer->location = loc.ToString();
        return base::BindLambdaForTesting([enforcer { std::move(enforcer) }](DecoderStatus s) {
            EXPECT_TRUE(s.is_ok()) << " Callback created: " << enforcer->location << " Code: " << static_cast<int>(s.code()) << " Error: " << s.message();
            enforcer->called = true;
        });
    }

    void DecodeAndWaitForStatus(scoped_refptr<DecoderBuffer> buffer, const base::Location& location = base::Location::Current())
    {
        base::RunLoop run_loop;
        decoder_->Decode(std::move(buffer), base::BindLambdaForTesting([&](DecoderStatus status) {
            EXPECT_TRUE(status.is_ok()) << " Callback created: " << location.ToString() << " Code: " << static_cast<int>(status.code())
    ...
    int CountDifferentPixels(VideoFrame& frame1, VideoFrame& frame2)
    {
        ...
        if (frame1.format() != frame2.format() || frame1.visible_rect().size() != frame2.visible_rect().size()) {
        ...
        for (size_t plane = 0; plane < num_planes; ++plane) {
            ...
            auto data1 = UNSAFE_BUFFERS(base::span(frame1.visible_data(plane), stride1 * rows));
            auto data2 = UNSAFE_BUFFERS(base::span(frame2.visible_data(plane), stride2 * rows));
    ...
    VideoEncoder::Options CreateDefaultOptions()
    {
        ...
        if (profile_ == VP9PROFILE_PROFILE1 || profile_ == VP9PROFILE_PROFILE3 || profile_ == AV1PROFILE_PROFILE_HIGH) {
...
TEST_P(SoftwareVideoEncoderTest, StopCallbackWrapping)
{
    ...
    VideoEncoder::EncoderStatusCB init_cb = base::BindLambdaForTesting([&](EncoderStatus error) { init_called = true; });

    VideoEncoder::EncoderStatusCB flush_cb = base::BindLambdaForTesting([&](EncoderStatus error) { flush_called = true; });
...
TEST_P(SoftwareVideoEncoderTest, InitializeAndFlush)
{
    ...
    VideoEncoder::OutputCB output_cb
        = base::BindLambdaForTesting([&](VideoEncoderOutput, std::optional<VideoEncoder::CodecDescription>) { output_called = true; });

    encoder_->Initialize(profile_, options, /*info_cb=*/base::DoNothing(), std::move(output_cb), ValidateStatusThenQuitCB());
    RunUntilQuit();
...
TEST_P(SoftwareVideoEncoderTest, ForceAllKeyFrames)
{
    ...
    VideoEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) {
    ...
    for (int i = 0; i < frames; i++) {
        ...
        encoder_->Encode(std::move(frame), VideoEncoder::EncodeOptions(true), ValidateStatusThenQuitCB());
        RunUntilQuit();
    }
...
TEST_P(SoftwareVideoEncoderTest, ResizeFrames)
{
    ...
    VideoEncoder::OutputCB output_cb
        = base::BindLambdaForTesting([&](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) { outputs_count++; });

    encoder_->Initialize(profile_, options, /*info_cb=*/base::DoNothing(), std::move(output_cb), ValidateStatusThenQuitCB());

    RunUntilQuit();
    auto frame1 = CreateFrame(gfx::Size(320, 200), pixel_format_, 0 * sec);
    encoder_->Encode(std::move(frame1), VideoEncoder::EncodeOptions(false), ValidatingStatusCB());
    auto frame2 = CreateFrame(gfx::Size(800, 600), pixel_format_, 1 * sec);
    encoder_->Encode(std::move(frame2), VideoEncoder::EncodeOptions(false), ValidatingStatusCB());
    auto frame3 = CreateFrame(gfx::Size(720, 1280), pixel_format_, 2 * sec);
    encoder_->Encode(std::move(frame3), VideoEncoder::EncodeOptions(false), ValidatingStatusCB());
...
TEST_P(SoftwareVideoEncoderTest, OutputCountEqualsFrameCount)
{
    ...
    int total_frames_count = options.framerate.value() * 10; // total duration 20s
    int outputs_count = 0;
    ...
    for (int frame_index = 0; frame_index < total_frames_count; frame_index++) {
        ...
        auto frame = CreateFrame(options.frame_size, pixel_format_, timestamp);
        encoder_->Encode(std::move(frame), VideoEncoder::EncodeOptions(false), ValidatingStatusCB());
    }
...
TEST_P(SoftwareVideoEncoderTest, EncodeAndDecode)
{
    ...
    int total_frames_count = options.framerate.value() * 10; // total duration 10s
    int total_decoded_frames = 0;

    auto frame_duration = base::Seconds(1.0 / options.framerate.value());

    VideoEncoder::OutputCB encoder_output_cb
        = base::BindLambdaForTesting([&, this](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) {
    ...
    VideoDecoder::OutputCB decoder_output_cb = base::BindLambdaForTesting([&](scoped_refptr<VideoFrame> decoded_frame) {
        ...
        EXPECT_EQ(decoded_frame->visible_rect().size(), original_frame->visible_rect().size());
        EXPECT_EQ(decoded_frame->format(), GetExpectedOutputPixelFormat(profile_));
        if (decoded_frame->format() == original_frame->format()) {
            EXPECT_LE(CountDifferentPixels(*decoded_frame, *original_frame), original_frame->visible_rect().width());
        }
    ...
    encoder_->Initialize(profile_, options, /*info_cb=*/base::DoNothing(), std::move(encoder_output_cb), ValidateStatusThenQuitCB());
    RunUntilQuit();

    TestRandom random_color(0x964050);
    for (int frame_index = 0; frame_index < total_frames_count; frame_index++) {
        auto timestamp = frame_index * frame_duration;
        auto frame = CreateFrame(options.frame_size, pixel_format_, timestamp, random_color.Rand() & 0x00FFFFFF);
        frames_to_encode.push_back(frame);
        encoder_->Encode(std::move(frame), VideoEncoder::EncodeOptions(false), ValidateStatusThenQuitCB());
...
TEST_P(SoftwareVideoEncoderTest, EncodeAndDecodeWithEnablingDrop)
{
    ...
    base::circular_deque<scoped_refptr<VideoFrame>> frames_to_encode;
    const int total_frames_count = options.framerate.value() * 10; // total duration 10s
    int total_decoded_frames = 0;
...
TEST_P(SVCVideoEncoderTest, EncodeClipTemporalSvc)
{
    ...
    VideoEncoder::OutputCB encoder_output_cb = base::BindLambdaForTesting(
        [&](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) { chunks.push_back(std::move(output)); });
    ...
    if (options.scalability_mode) {
        ...
        case SVCScalabilityMode::kL1T3:
            ...
            break;
        default:
            NOTREACHED() << "Unsupported SVC: " << GetScalabilityModeName(options.scalability_mode.value());
        }
    }
    ...
    for (int max_layer = 0; max_layer < num_temporal_layers; max_layer++) {
        ...
        VideoDecoder::OutputCB decoder_output_cb = base::BindLambdaForTesting([&](scoped_refptr<VideoFrame> frame) { decoded_frames.push_back(frame); });
        PrepareDecoder(options.frame_size, std::move(decoder_output_cb));
        ...
        ASSERT_EQ(decoded_frames.size(), size_t { total_frames_count / rate_decimator });
...
TEST_P(SVCVideoEncoderTest, EncodeClipTemporalSvcWithEnablingDrop)
{
    ...
    VideoEncoder::OutputCB encoder_output_cb = base::BindLambdaForTesting([&](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) {
        if (output.data.empty()) {
            dropped_frame_indices.push_back(chunks.size() + dropped_frame_indices.size());
            return;
        }
    ...
    auto last_layer = base::span(base::span(kTemporalLayerTable)[num_temporal_layers - 1]);
    ...
    for (int max_layer = 0; max_layer < num_temporal_layers; max_layer++) {
        ...
        for (size_t i = 0; i < frames_to_encode.size(); ++i) {
            ...
            const int temporal_id = base::span(kTemporalLayerTable)[num_temporal_layers - 1][encoded_frame_index % kTemporalLayerCycle];
            encoded_frame_index++;
...
TEST_P(ManualSVCVideoEncoderTest, EncodeClipTemporalSvc)
{
    ...
    VideoEncoder::EncoderInfoCB encoder_info_cb = base::BindLambdaForTesting([&](const VideoEncoderInfo& info) { encoder_info = info; });

    encoder_->Initialize(profile_, options, std::move(encoder_info_cb), std::move(encoder_output_cb), ValidateStatusThenQuitCB());
    RunUntilQuit();
    ...
    if (scalability_mode) {
        ...
        case SVCScalabilityMode::kL1T3:
            ...
            break;
        default:
            NOTREACHED() << "Unsupported SVC: " << GetScalabilityModeName(scalability_mode.value());
        }
    }
    ...
    for (auto frame_index = 0u; frame_index < total_frames_count; frame_index++) {
        ...
        encoder_->Encode(std::move(frame), encode_options, ValidateStatusThenQuitCB());
        RunUntilQuit();
    }
    ...
    for (int max_layer = 0; max_layer < num_temporal_layers; max_layer++) {
        ...
        for (size_t frame_index = 0; frame_index < chunks.size(); frame_index++) {
            ...
            int temporal_id = AssignNextTemporalId(static_cast<int>(frame_index), num_temporal_layers);
...
TEST_P(SVCVideoEncoderTest, ChangeLayers)
{
    ...
    for (auto frame_index = 0u; frame_index < total_frames_count; frame_index++) {
        ...
        if (reconfigure) {
            ...
            encoder_->ChangeOptions(options, VideoEncoder::OutputCB(), ValidateStatusThenQuitCB());
            RunUntilQuit();
        }
...
TEST_P(SoftwareVideoEncoderTest, ReconfigureWithResizingNumberOfThreads)
{
    ...
    encoder_->Initialize(profile_, options, /*info_cb=*/base::DoNothing(), std::move(output_cb), ValidatingStatusCB());

    auto frame0 = CreateFrame(options.frame_size, pixel_format_, {});
    encoder_->Encode(std::move(frame0), VideoEncoder::EncodeOptions(false), ValidatingStatusCB());

    options.frame_size = gfx::Size(1000, 608);
    encoder_->ChangeOptions(options, VideoEncoder::OutputCB(), ValidatingStatusCB());
    ...
    encoder_->Encode(std::move(frame2), VideoEncoder::EncodeOptions(false), ValidateStatusThenQuitCB());
...
TEST_P(H264VideoEncoderTest, ReconfigureWithResize)
{
    ...
    VideoDecoder::OutputCB decoder_output_cb = base::BindLambdaForTesting([&](scoped_refptr<VideoFrame> frame) {
        ASSERT_FALSE(frames_to_encode.empty());
        auto original_frame = std::move(frames_to_encode.front());
        frames_to_encode.pop_front();

        EXPECT_EQ(frame->timestamp(), original_frame->timestamp());
        EXPECT_EQ(frame->visible_rect().size(), original_frame->visible_rect().size());
        if (frame->format() != original_frame->format()) {
            ...
            auto i420_frame = frame_pool_.CreateFrame(PIXEL_FORMAT_I420, size, gfx::Rect(size), size, frame->timestamp());
            EXPECT_TRUE(frame_converter_.ConvertAndScale(*original_frame, *i420_frame).is_ok());
            original_frame = i420_frame;
        }
        EXPECT_LE(CountDifferentPixels(*frame, *original_frame), original_frame->visible_rect().width());
        ++total_decoded_frames;
    });
...
TEST_P(H264VideoEncoderTest, AvcExtraData)
{
    ...
    VideoEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](VideoEncoderOutput output, std::optional<VideoEncoder::CodecDescription> desc) {
        ...
        case 0: {
            ...
            bool parse_ok = converter.ParseConfiguration(desc->data(), desc->size(), &avc_config);
            EXPECT_TRUE(parse_ok);
            EXPECT_EQ(profile_, ProfileIDToVideoCodecProfile(avc_config.profile_indication));
    ...
    encoder_->Encode(std::move(frame1), VideoEncoder::EncodeOptions(false), ValidateStatusThenQuitCB());
    RunUntilQuit();
    ...
    encoder_->Encode(std::move(frame3), VideoEncoder::EncodeOptions(true), ValidateStatusThenQuitCB());
    RunUntilQuit();
...
TEST_P(H264VideoEncoderTest, EncodeAndDecodeWithConfig)
{
    ...
    for (auto frame_index = 0u; frame_index < total_frames_count; frame_index++) {
        ...
        auto frame = CreateFrame(options.frame_size, pixel_format_, timestamp);
        frames_to_encode.push_back(frame);
        encoder_->Encode(std::move(frame), VideoEncoder::EncodeOptions(key_frame), ValidateStatusThenQuitCB());
        RunUntilQuit();
    }
    encoder_->Flush(ValidateStatusThenQuitCB());
    RunUntilQuit();
    ...
    for (auto& chunk : chunks) {
        ...
        if (chunk.desc.has_value()) {
            if (decoder_)
                DecodeAndWaitForStatus(DecoderBuffer::CreateEOSBuffer());
            PrepareDecoder(options.frame_size, std::move(decoder_output_cb), chunk.desc.value());
        }
...
std::string PrintTestParams(const testing::TestParamInfo<SwVideoTestParams>& info)
{
    auto result = GetCodecName(info.param.codec) + "__" + GetProfileName(info.param.profile) + "__" + VideoPixelFormatToString(info.param.pixel_format) + "__"
        + (info.param.scalability_mode ? GetScalabilityModeName(info.param.scalability_mode.value()) : "");
...
INSTANTIATE_TEST_SUITE_P(H264Specific, H264VideoEncoderTest, ::testing::ValuesIn(kH264Params), PrintTestParams);

INSTANTIATE_TEST_SUITE_P(H264Generic, SoftwareVideoEncoderTest, ::testing::ValuesIn(kH264Params), PrintTestParams);

SwVideoTestParams kH264SVCParams[] = { { VideoCodec::kH264, H264PROFILE_BASELINE, PIXEL_FORMAT_I420, std::nullopt },
    { VideoCodec::kH264, H264PROFILE_BASELINE, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T1 },
    { VideoCodec::kH264, H264PROFILE_BASELINE, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T2 },
    { VideoCodec::kH264, H264PROFILE_BASELINE, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 },
    { VideoCodec::kH264, H264PROFILE_MAIN, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 },
    { VideoCodec::kH264, H264PROFILE_HIGH, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 } };

INSTANTIATE_TEST_SUITE_P(H264TemporalSvc, SVCVideoEncoderTest, ::testing::ValuesIn(kH264SVCParams), PrintTestParams);
#endif // BUILDFLAG(ENABLE_OPENH264) && BUILDFLAG(ENABLE_FFMPEG_VIDEO_DECODERS)

#if BUILDFLAG(ENABLE_LIBVPX)
SwVideoTestParams kVpxParams[] = { { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_I420 }, { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_NV12 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_XRGB }, { VideoCodec::kVP9, VP9PROFILE_PROFILE1, PIXEL_FORMAT_I444 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE1, PIXEL_FORMAT_NV12 }, { VideoCodec::kVP9, VP9PROFILE_PROFILE1, PIXEL_FORMAT_XRGB },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE2, PIXEL_FORMAT_I420 }, { VideoCodec::kVP9, VP9PROFILE_PROFILE2, PIXEL_FORMAT_NV12 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE2, PIXEL_FORMAT_XRGB }, { VideoCodec::kVP9, VP9PROFILE_PROFILE3, PIXEL_FORMAT_I444 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE3, PIXEL_FORMAT_NV12 }, { VideoCodec::kVP9, VP9PROFILE_PROFILE3, PIXEL_FORMAT_XRGB },
    { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_I420 }, { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_XRGB } };

INSTANTIATE_TEST_SUITE_P(VpxGeneric, SoftwareVideoEncoderTest, ::testing::ValuesIn(kVpxParams), PrintTestParams);

SwVideoTestParams kVpxSVCParams[] = { { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_I420, std::nullopt },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T1 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T2 },
    { VideoCodec::kVP9, VP9PROFILE_PROFILE0, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 },
    { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_I420, std::nullopt }, { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T1 },
    { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T2 },
    { VideoCodec::kVP8, VP8PROFILE_ANY, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 } };

INSTANTIATE_TEST_SUITE_P(VpxTemporalSvc, SVCVideoEncoderTest, ::testing::ValuesIn(kVpxSVCParams), PrintTestParams);
#endif // ENABLE_LIBVPX
...
SwVideoTestParams kAv1Params[]
    = { { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_I420 }, { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_NV12 },
          { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_XRGB }, { VideoCodec::kAV1, AV1PROFILE_PROFILE_HIGH, PIXEL_FORMAT_I444 },
          { VideoCodec::kAV1, AV1PROFILE_PROFILE_HIGH, PIXEL_FORMAT_NV12 }, { VideoCodec::kAV1, AV1PROFILE_PROFILE_HIGH, PIXEL_FORMAT_XRGB } };

INSTANTIATE_TEST_SUITE_P(Av1Generic, SoftwareVideoEncoderTest, ::testing::ValuesIn(kAv1Params), PrintTestParams);

SwVideoTestParams kAv1SVCParams[] = { { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_I420, std::nullopt },
    { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T1 },
    { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T2 },
    { VideoCodec::kAV1, AV1PROFILE_PROFILE_MAIN, PIXEL_FORMAT_I420, SVCScalabilityMode::kL1T3 } };

INSTANTIATE_TEST_SUITE_P(Av1TemporalSvc, SVCVideoEncoderTest, ::testing::ValuesIn(kAv1SVCParams), PrintTestParams);

INSTANTIATE_TEST_SUITE_P(Av1ManualSvc, ManualSVCVideoEncoderTest, ::testing::ValuesIn(kAv1SVCParams), PrintTestParams);
#endif // ENABLE_LIBAOM
