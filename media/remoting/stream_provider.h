...
public:
    StreamProvider(ReceiverController* receiver_controller, const scoped_refptr<base::SequencedTaskRunner>& media_task_runner);
...
private:
    ...
    // Read() is called.
    class MediaStream final : public DemuxerStream, public mojom::RemotingDataStreamReceiver {
    public:
        using UniquePtr = std::unique_ptr<MediaStream, std::function<void(MediaStream*)>>;

        // MediaStream should be created on the main thread to be able to get unique
        // handle ID from |rpc_messenger_|.
        static void CreateOnMainThread(openscreen::cast::RpcMessenger* rpc_messenger, Type type, int32_t handle,
        ...
        MediaStream(
            openscreen::cast::RpcMessenger* rpc_messenger, Type type, int32_t remote_handle, const scoped_refptr<base::SequencedTaskRunner>& media_task_runner);
        ...
        mojo::PendingRemote<mojom::RemotingDataStreamReceiver> BindNewPipeAndPassRemote()
        {
    ...
    private:
        ...
        void SendRpcMessageOnMainThread(std::unique_ptr<openscreen::cast::RpcMessage> message);
        ...
        void OnInitializeCallback(std::unique_ptr<openscreen::cast::RpcMessage> message);
        void OnReadUntilCallback(std::unique_ptr<openscreen::cast::RpcMessage> message);
        ...
        void UpdateAudioConfig(const openscreen::cast::AudioDecoderConfig& audio_message);
        void UpdateVideoConfig(const openscreen::cast::VideoDecoderConfig& video_message);
...
template <> struct default_delete<media::remoting::StreamProvider> {
    constexpr default_delete() = default;

    template <typename U, typename = typename std::enable_if<std::is_convertible<U*, media::remoting::StreamProvider*>::value>::type>
