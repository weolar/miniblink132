...
class MediaRemotingIntegrationTest : public testing::Test, public PipelineIntegrationTestBase {
public:
    MediaRemotingIntegrationTest()
    {
        SetCreateRendererCB(base::BindRepeating(&MediaRemotingIntegrationTest::CreateEnd2EndTestRenderer, base::Unretained(this)));
    }

    MediaRemotingIntegrationTest(const MediaRemotingIntegrationTest&) = delete;
    MediaRemotingIntegrationTest& operator=(const MediaRemotingIntegrationTest&) = delete;

private:
    std::unique_ptr<Renderer> CreateEnd2EndTestRenderer(std::optional<RendererType> renderer_type)
    {
        return std::make_unique<End2EndTestRenderer>(this->CreateRendererImpl(renderer_type));
    }
};
