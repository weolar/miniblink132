...
DemuxerStreamAdapter::DemuxerStreamAdapter(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
    scoped_refptr<base::SequencedTaskRunner> media_task_runner, const std::string& name, DemuxerStream* demuxer_stream,
    const openscreen::WeakPtr<RpcMessenger>& rpc_messenger, int rpc_handle, mojo::PendingRemote<mojom::RemotingDataStreamSender> stream_sender_remote,
    mojo::ScopedDataPipeProducerHandle producer_handle, ErrorCallback error_callback)
    ...
    , bytes_written_to_pipe_(0)
{
    ...
    stream_sender_.set_disconnect_handler(base::BindOnce(&DemuxerStreamAdapter::OnFatalError, weak_factory_.GetWeakPtr(), MOJO_DISCONNECTED));
}
...
void DemuxerStreamAdapter::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
...
void DemuxerStreamAdapter::Initialize(int remote_callback_handle)
{
    ...
    DEMUXER_VLOG(2) << "Received RPC_DS_INITIALIZE with remote_callback_handle=" << remote_callback_handle;

    // Checks if initialization had been called or not.
    if (remote_callback_handle_ != RpcMessenger::kInvalidHandle) {
        DEMUXER_VLOG(1) << "Duplicated initialization. Have: " << remote_callback_handle_ << ", Given: " << remote_callback_handle;
    ...
    case DemuxerStream::Type::AUDIO: {
        ...
        openscreen::cast::AudioDecoderConfig* audio_message = init_cb_message->mutable_audio_decoder_config();
        media::cast::ConvertAudioDecoderConfigToProto(audio_config_, audio_message);
        break;
    }
    case DemuxerStream::Type::VIDEO: {
        video_config_ = demuxer_stream_->video_decoder_config();
        openscreen::cast::VideoDecoderConfig* video_message = init_cb_message->mutable_video_decoder_config();
        media::cast::ConvertVideoDecoderConfigToProto(video_config_, video_message);
        break;
    }
    default:
        NOTREACHED();
    ...
    DEMUXER_VLOG(2) << "Sending RPC_DS_INITIALIZE_CALLBACK to " << rpc->handle() << " with decoder_config={"
                    << (type_ == DemuxerStream::Type::AUDIO ? audio_config_.AsHumanReadableString() : video_config_.AsHumanReadableString()) << '}';
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RpcMessenger::SendMessageToRemote, rpc_messenger_, *rpc));
}

void DemuxerStreamAdapter::ReadUntil(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    ...
    const openscreen::cast::DemuxerStreamReadUntil& rpc_message = message->demuxerstream_readuntil_rpc();
    DEMUXER_VLOG(2) << "Received RPC_DS_READUNTIL with callback_handle=" << rpc_message.callback_handle() << ", count=" << rpc_message.count();
...
void DemuxerStreamAdapter::EnableBitstreamConverter()
{
    ...
    if (remote_callback_handle_ != RpcMessenger::kInvalidHandle) {
        ...
        rpc->set_handle(remote_callback_handle_);
        rpc->set_proc(openscreen::cast::RpcMessage::RPC_DS_ENABLEBITSTREAMCONVERTER_CALLBACK);
...
void DemuxerStreamAdapter::RequestBuffer()
{
    ...
    demuxer_stream_->Read(1, base::BindOnce(&DemuxerStreamAdapter::OnNewBuffersRead, request_buffer_weak_factory_.GetWeakPtr()));
}

void DemuxerStreamAdapter::OnNewBuffersRead(DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers_queue)
{
    DCHECK_LE(buffers_queue.size(), 1u) << "DemuxerStreamAdapter only reads a single-buffer.";
    OnNewBuffer(status, buffers_queue.empty() ? nullptr : std::move(buffers_queue[0]));
}

void DemuxerStreamAdapter::OnNewBuffer(DemuxerStream::Status status, scoped_refptr<DecoderBuffer> input)
{
...
void DemuxerStreamAdapter::WriteFrame()
{
    ...
    stream_sender_->SendFrame(
        media::mojom::DecoderBuffer::From(*pending_frame_), base::BindOnce(&DemuxerStreamAdapter::OnWrittenFrameRead, base::Unretained(this)));

    if (!pending_frame_->end_of_stream()) {
        data_pipe_writer_.Write(pending_frame_->data(), pending_frame_->size(), base::BindOnce(&DemuxerStreamAdapter::OnFrameWritten, base::Unretained(this)));
    } else {
...
void DemuxerStreamAdapter::TryCompleteFrameWrite()
{
    ...
    // end of stream.
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DemuxerStreamAdapter::RequestBuffer, weak_factory_.GetWeakPtr()));
}

void DemuxerStreamAdapter::SendReadAck()
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DEMUXER_VLOG(3) << "last_count_=" << last_count_ << ", remote_read_callback_handle=" << read_until_callback_handle_ << ", media_status=" << media_status_;
    ...
    message->set_status(media::cast::ToProtoDemuxerStreamStatus(media_status_).value());
    if (media_status_ == DemuxerStream::kConfigChanged) {
        if (audio_config_.IsValidConfig()) {
            openscreen::cast::AudioDecoderConfig* audio_message = message->mutable_audio_decoder_config();
            media::cast::ConvertAudioDecoderConfigToProto(audio_config_, audio_message);
        } else if (video_config_.IsValidConfig()) {
            openscreen::cast::VideoDecoderConfig* video_message = message->mutable_video_decoder_config();
            media::cast::ConvertVideoDecoderConfigToProto(video_config_, video_message);
        } else {
            NOTREACHED();
        }
    ...
    DEMUXER_VLOG(2) << "Sending RPC_DS_READUNTIL_CALLBACK to " << rpc->handle() << " with count=" << message->count() << ", status=" << message->status()
                    << ", decoder_config={"
                    << (audio_config_.IsValidConfig()          ? audio_config_.AsHumanReadableString()
                               : video_config_.IsValidConfig() ? video_config_.AsHumanReadableString()
                                                               : "DID NOT CHANGE")
                    << '}';
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RpcMessenger::SendMessageToRemote, rpc_messenger_, *rpc));
...
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    auto receive_callback = base::BindPostTaskToCurrentDefault(base::BindRepeating(&DemuxerStreamAdapter::OnReceivedRpc, weak_factory_.GetWeakPtr()));
    main_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RegisterForRpcTask, rpc_messenger_, rpc_handle_,
            [cb = std::move(receive_callback)](std::unique_ptr<openscreen::cast::RpcMessage> message) { cb.Run(std::move(message)); }));
}
