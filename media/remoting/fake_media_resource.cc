...
FakeDemuxerStream::FakeDemuxerStream(bool is_audio)
{
    ...
    if (is_audio) {
        audio_config_.Initialize(
            AudioCodec::kAAC, kSampleFormatS16, CHANNEL_LAYOUT_STEREO, 38400, std::vector<uint8_t>(), EncryptionScheme::kUnencrypted, base::TimeDelta(), 0);
    } else {
        gfx::Size size(640, 480);
        gfx::Rect rect(0, 0, 640, 480);
        video_config_.Initialize(VideoCodec::kH264, H264PROFILE_BASELINE, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC601(),
            kNoTransformation, size, rect, size, std::vector<uint8_t>(), EncryptionScheme::kUnencrypted);
    }
    ON_CALL(*this, Read).WillByDefault(Invoke(this, &FakeDemuxerStream::FakeRead));
}
...
void FakeDemuxerStream::CreateFakeFrame(size_t size, bool key_frame, int pts_ms)
{
