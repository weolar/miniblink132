...
public:
    ...
    void SendAudioFrame(uint32_t frame_count, scoped_refptr<DecoderBuffer> buffer);
    void SendVideoFrame(uint32_t frame_count, scoped_refptr<DecoderBuffer> buffer);

    // mojom::Remotee implementation
    void OnRemotingSinkReady(mojo::PendingRemote<mojom::RemotingSink> remoting_sink) override;
    void SendMessageToSource(const std::vector<uint8_t>& message) override;
    void StartDataStreams(
        mojo::PendingRemote<mojom::RemotingDataStreamReceiver> audio_stream, mojo::PendingRemote<mojom::RemotingDataStreamReceiver> video_stream) override;
    void OnFlushUntil(uint32_t audio_count, uint32_t video_count) override;
    void OnVideoNaturalSizeChange(const gfx::Size& size) override;

    void Reset();

    gfx::Size changed_size()
    {
        return changed_size_;
    }
    uint32_t flush_audio_count()
    {
        return flush_audio_count_;
    }
    uint32_t flush_video_count()
    {
        return flush_video_count_;
    }
...
public:
    ...
    MockRemotee* mock_remotee()
    {
        return mock_remotee_.get();
    }
