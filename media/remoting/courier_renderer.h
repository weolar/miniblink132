...
public:
    ...
    // thread.
    CourierRenderer(scoped_refptr<base::SequencedTaskRunner> media_task_runner, const base::WeakPtr<RendererController>& controller,
...
private:
    ...
    // race condition.
    static void OnDataPipeCreatedOnMainThread(scoped_refptr<base::SequencedTaskRunner> media_task_runner, base::WeakPtr<CourierRenderer> self,
        openscreen::WeakPtr<openscreen::cast::RpcMessenger> rpc_messenger, mojo::PendingRemote<mojom::RemotingDataStreamSender> audio,
...
public:
    ...
    void Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb) final;
...
private:
    ...
    enum State { STATE_UNINITIALIZED, STATE_CREATE_PIPE, STATE_ACQUIRING, STATE_INITIALIZING, STATE_FLUSHING, STATE_PLAYING, STATE_ERROR };

    // Callback when attempting to establish data pipe. Runs on media thread only.
    void OnDataPipeCreated(mojo::PendingRemote<mojom::RemotingDataStreamSender> audio, mojo::PendingRemote<mojom::RemotingDataStreamSender> video,
    ...
    void AcquireRendererDone(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void InitializeCallback(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void FlushUntilCallback();
    void OnTimeUpdate(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnBufferingStateChange(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnAudioConfigChange(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnVideoConfigChange(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnVideoNaturalSizeChange(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnVideoOpacityChange(std::unique_ptr<openscreen::cast::RpcMessage> message);
    void OnStatisticsUpdate(std::unique_ptr<openscreen::cast::RpcMessage> message);

    // Called when |current_media_time_| is updated.
    void OnMediaTimeUpdated();

    // Called to update the |video_stats_queue_|.
    void UpdateVideoStatsQueue(int video_frames_decoded, int video_frames_dropped);
    ...
    const raw_ptr<VideoRendererSink> video_renderer_sink_; // Outlives this class.
    ...
    base::circular_deque<std::pair<base::TimeTicks, base::TimeDelta>> media_time_queue_;
    ...
    base::circular_deque<std::tuple<base::TimeTicks, int, int>> video_stats_queue_;
