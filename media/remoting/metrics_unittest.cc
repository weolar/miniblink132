...
{
    constexpr char kPixelRateSupportHistogramName[] = "Media.Remoting.VideoPixelRateSupport";
    ...
    EXPECT_THAT(tester.GetAllSamples(kPixelRateSupportHistogramName), ElementsAre(Bucket(static_cast<int>(PixelRateSupport::k4kNotSupported), 1)));
}
...
TEST_F(MediaRemotingMetricsTest, RecordCompatibility)
{
    ...
    // given SessionMetricsRecorder instance.
    EXPECT_THAT(tester.GetAllSamples(kCompatibilityHistogramName), ElementsAre(Bucket(static_cast<int>(RemotingCompatibility::kIncompatibleVideoCodec), 1)));
}
