...
PipelineStatistics DefaultStats()
{
    ...
    stats.audio_pipeline_info = { false, false, AudioDecoderType::kUnknown, EncryptionType::kClear };
    stats.video_pipeline_info = { false, false, VideoDecoderType::kUnknown, EncryptionType::kClear };
    return stats;
}
...
public:
    RendererClientImpl()
    {
        ON_CALL(*this, OnStatisticsUpdate(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnStatisticsUpdate));
        ON_CALL(*this, OnPipelineStatus(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnPipelineStatus));
        ON_CALL(*this, OnBufferingStateChange(_, _)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnBufferingStateChange));
        ON_CALL(*this, OnAudioConfigChange(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnAudioConfigChange));
        ON_CALL(*this, OnVideoConfigChange(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnVideoConfigChange));
        ON_CALL(*this, OnVideoNaturalSizeChange(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnVideoNaturalSizeChange));
        ON_CALL(*this, OnVideoOpacityChange(_)).WillByDefault(Invoke(this, &RendererClientImpl::DelegateOnVideoOpacityChange));
    }
    ...
    MOCK_METHOD2(OnBufferingStateChange, void(BufferingState state, BufferingStateChangeReason reason));
    ...
    void DelegateOnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
    {
        state_ = state;
    }
    ...
    void DelegateOnVideoNaturalSizeChange(const gfx::Size& size)
    {
        size_ = size;
    }
    void DelegateOnVideoOpacityChange(bool opaque)
    {
        opaque_ = opaque;
    }
    ...
    PipelineStatistics stats() const
    {
        return stats_;
    }
    BufferingState state() const
    {
        return state_;
    }
    gfx::Size size() const
...
public:
    ...
    void RpcMessageResponseBot(std::vector<uint8_t> message)
    {
        ...
        case openscreen::cast::RpcMessage::RPC_ACQUIRE_RENDERER: {
            ...
            acquire_done->set_proc(openscreen::cast::RpcMessage::RPC_ACQUIRE_RENDERER_DONE);
            acquire_done->set_integer_value(receiver_renderer_handle_);
            controller_->GetRpcMessenger()->ProcessMessageFromRemote(std::move(acquire_done));
        } break;
        case openscreen::cast::RpcMessage::RPC_ACQUIRE_DEMUXER: {
            if (!is_backward_compatible_mode_) {
                ...
                sender_audio_demuxer_handle_ = rpc.acquire_demuxer_rpc().audio_demuxer_handle();
                sender_video_demuxer_handle_ = rpc.acquire_demuxer_rpc().video_demuxer_handle();
                ...
                if (sender_audio_demuxer_handle_ != RpcMessenger::kInvalidHandle) {
                    ...
                    controller_->GetRpcMessenger()->ProcessMessageFromRemote(std::move(ds_init));
                }
        ...
        case openscreen::cast::RpcMessage::RPC_R_INITIALIZE: {
            ...
            if (is_backward_compatible_mode_) {
                ...
                init_cb->set_proc(openscreen::cast::RpcMessage::RPC_R_INITIALIZE_CALLBACK);
                init_cb->set_boolean_value(is_successfully_initialized_);
                controller_->GetRpcMessenger()->ProcessMessageFromRemote(std::move(init_cb));
            }
        } break;
        case openscreen::cast::RpcMessage::RPC_DS_INITIALIZE_CALLBACK: {
            ...
            if (received_audio_ds_init_cb_ == (sender_audio_demuxer_handle_ != RpcMessenger::kInvalidHandle)
                && received_video_ds_init_cb_ == (sender_video_demuxer_handle_ != RpcMessenger::kInvalidHandle)) {
        ...
        case openscreen::cast::RpcMessage::RPC_R_FLUSHUNTIL: {
            ...
            std::unique_ptr<openscreen::cast::RpcMessage> flush_cb(new openscreen::cast::RpcMessage());
            flush_cb->set_handle(rpc.renderer_flushuntil_rpc().callback_handle());
            flush_cb->set_proc(openscreen::cast::RpcMessage::RPC_R_FLUSHUNTIL_CALLBACK);
            controller_->GetRpcMessenger()->ProcessMessageFromRemote(std::move(flush_cb));
        } break;
    ...
    {
        controller_->GetRpcMessenger()->set_send_message_cb_for_testing([this](std::vector<uint8_t> message) { this->OnSendMessageToSink(message); });
    }
...
protected:
    void InitializeRenderer()
    {
        ...
        controller_->GetRpcMessenger()->set_send_message_cb_for_testing([this](std::vector<uint8_t> message) { this->RpcMessageResponseBot(message); });
        RunPendingTasks();
        renderer_->Initialize(
            media_resource_.get(), render_client_.get(), base::BindOnce(&RendererClientImpl::OnPipelineStatus, base::Unretained(render_client_.get())));
        RunPendingTasks();
    ...
    bool IsRendererInitialized() const
    {
        ...
        return renderer_->state_ == CourierRenderer::STATE_PLAYING && is_receiver_demuxer_initialized_;
    }
    ...
    void SetUp() override
    {
        ...
        RewireSendMessageCallbackToSink();
        renderer_ = std::make_unique<CourierRenderer>(base::SingleThreadTaskRunner::GetCurrentDefault(), controller_->GetWeakPtr(), nullptr);
    ...
    CourierRenderer::State state() const
    {
        return renderer_->state_;
    }
    ...
    int ReceivedRpcMessageCount() const
    {
        return received_rpc_.size();
    }
    void ResetReceivedRpcMessage()
    {
        received_rpc_.clear();
    }

    void ValidateCurrentTime(base::TimeDelta current, base::TimeDelta current_max) const
    {
    ...
    void IssueTimeUpdateRpc(base::TimeDelta media_time, base::TimeDelta max_media_time)
    {
    ...
    void VerifyAndReportTimeUpdates(int start_serial_number, int end_serial_number)
    {
    ...
    void IssueStatisticsUpdateRpc()
    {
        ...
        message->mutable_audio_decoder_info()->set_is_platform_decoder(stats.audio_pipeline_info.is_platform_decoder);
        message->mutable_audio_decoder_info()->set_decoder_type(static_cast<int64_t>(stats.audio_pipeline_info.decoder_type));
        message->mutable_video_decoder_info()->set_is_platform_decoder(stats.video_pipeline_info.is_platform_decoder);
        message->mutable_video_decoder_info()->set_decoder_type(static_cast<int64_t>(stats.video_pipeline_info.decoder_type));
        OnReceivedRpc(std::move(rpc));
        RunPendingTasks();
    }
    ...
    {
        std::optional<openscreen::cast::RendererClientOnBufferingStateChange::State> pb_state = media::cast::ToProtoMediaBufferingState(state);
...
TEST_F(CourierRendererTest, InitializeFailed)
{
    ...
    renderer_->Flush(base::BindOnce(&RendererClientImpl::OnFlushCallback, base::Unretained(render_client_.get())));
    RunPendingTasks();
...
TEST_F(CourierRendererTest, StartPlayingFrom)
{
    ...
    const openscreen::cast::RpcMessage* rpc = PeekRpcMessage(openscreen::cast::RpcMessage::RPC_R_STARTPLAYINGFROM);
    ASSERT_TRUE(rpc);
...
TEST_F(CourierRendererTest, SetVolume)
{
    ...
    // Checks if it sends out RPC message with correct value.
    ASSERT_EQ(1, ReceivedRpcMessageCount());
    const openscreen::cast::RpcMessage* rpc = PeekRpcMessage(openscreen::cast::RpcMessage::RPC_R_SETVOLUME);
    ASSERT_TRUE(rpc);
...
TEST_F(CourierRendererTest, SetPlaybackRate)
{
    ...
    ASSERT_EQ(1, ReceivedRpcMessageCount());
    // Checks if it sends out RPC message with correct value.
    const openscreen::cast::RpcMessage* rpc = PeekRpcMessage(openscreen::cast::RpcMessage::RPC_R_SETPLAYBACKRATE);
    ASSERT_TRUE(rpc);
...
{
    InitializeRenderer();
    EXPECT_CALL(*render_client_, OnBufferingStateChange(BUFFERING_HAVE_NOTHING, _)).Times(1);
...
TEST_F(CourierRendererTest, OnAudioConfigChange)
{
    ...
    EXPECT_CALL(*render_client_, OnAudioConfigChange(DecoderConfigEq(kNewAudioConfig))).Times(1);

    std::unique_ptr<openscreen::cast::RpcMessage> rpc(new openscreen::cast::RpcMessage());
    rpc->set_handle(5);
    rpc->set_proc(openscreen::cast::RpcMessage::RPC_RC_ONAUDIOCONFIGCHANGE);
    auto* audio_config_change_message = rpc->mutable_rendererclient_onaudioconfigchange_rpc();
    openscreen::cast::AudioDecoderConfig* proto_audio_config = audio_config_change_message->mutable_audio_decoder_config();
    media::cast::ConvertAudioDecoderConfigToProto(kNewAudioConfig, proto_audio_config);
    OnReceivedRpc(std::move(rpc));
    RunPendingTasks();
...
TEST_F(CourierRendererTest, OnVideoConfigChange)
{
    ...
    // Issues RPC_RC_ONVIDEOCONFIGCHANGE RPC message.
    EXPECT_CALL(*render_client_, OnVideoConfigChange(DecoderConfigEq(kNewVideoConfig))).Times(1);

    std::unique_ptr<openscreen::cast::RpcMessage> rpc(new openscreen::cast::RpcMessage());
    rpc->set_handle(5);
    rpc->set_proc(openscreen::cast::RpcMessage::RPC_RC_ONVIDEOCONFIGCHANGE);
    auto* video_config_change_message = rpc->mutable_rendererclient_onvideoconfigchange_rpc();
    openscreen::cast::VideoDecoderConfig* proto_video_config = video_config_change_message->mutable_video_decoder_config();
    media::cast::ConvertVideoDecoderConfigToProto(kNewVideoConfig, proto_video_config);
    OnReceivedRpc(std::move(rpc));
    RunPendingTasks();
...
TEST_F(CourierRendererTest, OnVideoNaturalSizeChange)
{
    ...
    EXPECT_CALL(*render_client_, OnVideoNaturalSizeChange(gfx::Size(100, 200))).Times(1);
    std::unique_ptr<openscreen::cast::RpcMessage> rpc(new openscreen::cast::RpcMessage());
    rpc->set_handle(5);
...
TEST_F(CourierRendererTest, OnPacingTooSlowly)
{
    ...
    RunPendingTasks();
    EXPECT_CALL(*render_client_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).Times(1);
