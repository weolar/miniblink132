...
public:
        ...
        , remote_handle_(remote_handle)
    {
        rpc_messenger_->RegisterMessageReceiverCallback(
            rpc_handle_, [this](std::unique_ptr<openscreen::cast::RpcMessage> rpc) { this->OnReceivedRpc(std::move(rpc)); });
    }
    ...
    void OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
    {
        ...
        case openscreen::cast::RpcMessage::RPC_RC_ONVIDEONATURALSIZECHANGE: {
            ...
            gfx::Size size(message->rendererclient_onvideonatualsizechange_rpc().width(), message->rendererclient_onvideonatualsizechange_rpc().height());
...
public:
    ...
    void SetUp() override
    {
        ...
        mock_controller_->Initialize(mock_controller_->mock_remotee()->BindNewPipeAndPassRemote());
        ...
        mock_sender_ = std::make_unique<StrictMock<MockSender>>(rpc_messenger_, receiver_renderer_handle_);
    ...
    {
        rpc_messenger_->UnregisterMessageReceiverCallback(RpcMessenger::kAcquireRendererHandle);
    }

    void OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
    {
        DCHECK(message);
        EXPECT_EQ(message->proc(), openscreen::cast::RpcMessage::RPC_ACQUIRE_RENDERER);
    ...
    {
        DVLOG(3) << __func__ << ": Issues RPC_ACQUIRE_RENDERER_DONE RPC message. remote_handle=" << sender_renderer_handle_
    ...
    void CreateReceiver()
    {
        ...
        receiver_ = std::make_unique<Receiver>(receiver_renderer_handle_, sender_renderer_handle_, mock_controller_,
            base::SingleThreadTaskRunner::GetCurrentDefault(), std::move(renderer),
            base::BindOnce(&ReceiverTest::OnAcquireRendererDone, weak_factory_.GetWeakPtr()));
    }
    ...
    {
        receiver_->Initialize(&mock_media_resource_, nullptr, base::BindOnce(&ReceiverTest::OnRendererInitialized, weak_factory_.GetWeakPtr()));
    }
...
TEST_F(ReceiverTest, InitializeBeforeRpcInitialize)
{
    ...
    EXPECT_CALL(*mock_renderer_, OnInitialize(&mock_media_resource_, receiver_.get(), _)).WillOnce(RunOnceCallback<2>(PIPELINE_OK));
    EXPECT_CALL(*this, OnRendererInitialized(HasStatusCode(PIPELINE_OK))).Times(1);
...
TEST_F(ReceiverTest, RendererClientInterface)
{
    ...
    EXPECT_CALL(*mock_sender_, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH)).Times(1);
    receiver_->OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, BUFFERING_CHANGE_REASON_UNKNOWN);
    task_environment_.RunUntilIdle();
    ...
    EXPECT_CALL(*mock_sender_, OnAudioConfigChange(DecoderConfigEq(kNewAudioConfig))).Times(1);
    receiver_->OnAudioConfigChange(kNewAudioConfig);
    task_environment_.RunUntilIdle();

    // OnVideoConfigChange
    const auto kNewVideoConfig = TestVideoConfig::Normal();
    EXPECT_CALL(*mock_sender_, OnVideoConfigChange(DecoderConfigEq(kNewVideoConfig))).Times(1);
