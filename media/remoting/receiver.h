...
// the remoting media.
class Receiver final : public Renderer, public RendererClient, public media::cast::RpcRendererCallMessageHandler {
public:
    Receiver(int rpc_handle, int remote_handle, ReceiverController* receiver_controller, const scoped_refptr<base::SequencedTaskRunner>& media_task_runner,
        std::unique_ptr<Renderer> renderer, base::OnceCallback<void(int)> acquire_renderer_done_cb);
    ~Receiver() override;
    ...
    base::WeakPtr<Receiver> GetWeakPtr()
    {
...
private:
    ...
    const raw_ptr<ReceiverController> receiver_controller_; // Outlives this class.
    const raw_ptr<openscreen::cast::RpcMessenger> rpc_messenger_; // Outlives this class.
