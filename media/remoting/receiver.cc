...
Receiver::Receiver(int rpc_handle, int remote_handle, ReceiverController* receiver_controller,
    const scoped_refptr<base::SequencedTaskRunner>& media_task_runner, std::unique_ptr<Renderer> renderer,
    ...
    , acquire_renderer_done_cb_(std::move(acquire_renderer_done_cb))
{
    ...
    // the media thread.
    auto receive_callback = base::BindPostTask(media_task_runner_, BindRepeating(&Receiver::OnReceivedRpc, weak_factory_.GetWeakPtr()));
...
void Receiver::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
...
void Receiver::SendRpcMessageOnMainThread(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
...
void Receiver::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
...
void Receiver::ShouldInitializeRenderer()
{
    ...
    DCHECK(renderer_);
    DCHECK(demuxer_);
    renderer_->Initialize(demuxer_, this, base::BindOnce(&Receiver::OnRendererInitialized, weak_factory_.GetWeakPtr()));
}
...
void Receiver::OnRendererInitialized(PipelineStatus status)
{
    ...
    auto rpc = media::cast::CreateMessageForInitializationComplete(status == PIPELINE_OK);
...
void Receiver::OnRpcFlush(uint32_t audio_count, uint32_t video_count)
{
    ...
    time_update_timer_.Stop();
    renderer_->Flush(base::BindOnce(&Receiver::OnFlushDone, weak_factory_.GetWeakPtr()));
}
...
void Receiver::ScheduleMediaTimeUpdates()
{
    if (time_update_timer_.IsRunning())
        return;
    SendMediaTimeUpdate();
    time_update_timer_.Start(FROM_HERE, kTimeUpdateInterval, base::BindRepeating(&Receiver::SendMediaTimeUpdate, weak_factory_.GetWeakPtr()));
}
...
void Receiver::SendMediaTimeUpdate()
{
    ...
    auto rpc = media::cast::CreateMessageForMediaTimeUpdate(renderer_->GetMediaTime());
    rpc->set_handle(remote_handle_);
    SendRpcMessageOnMainThread(std::move(rpc));
}
...
void Receiver::OnBufferingStateChange(BufferingState state, BufferingStateChangeReason reason)
{
