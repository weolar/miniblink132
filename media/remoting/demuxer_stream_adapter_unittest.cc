...
public:
    MockDemuxerStreamAdapter(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> media_task_runner,
        const std::string& name, DemuxerStream* demuxer_stream, mojo::PendingRemote<mojom::RemotingDataStreamSender> stream_sender_remote,
        mojo::ScopedDataPipeProducerHandle producer_handle)
    {
        rpc_messenger_ = std::make_unique<RpcMessenger>([cb = base::BindRepeating(&MockDemuxerStreamAdapter::OnSendMessageToSink, weak_factory_.GetWeakPtr())](
                                                            std::vector<uint8_t> message) { cb.Run(std::move(message)); });
        demuxer_stream_adapter_ = std::make_unique<DemuxerStreamAdapter>(std::move(main_task_runner), std::move(media_task_runner), name, demuxer_stream,
            rpc_messenger_->GetWeakPtr(), rpc_messenger_->GetUniqueHandle(), std::move(stream_sender_remote), std::move(producer_handle),
            base::BindOnce(&MockDemuxerStreamAdapter::OnError, weak_factory_.GetWeakPtr()));
    ...
    int rpc_handle() const
    {
        return demuxer_stream_adapter_->rpc_handle();
    }
    ...
    void DoDuplicateInitialize()
    {
        demuxer_stream_adapter_->Initialize(999);
    }
    ...
    void FakeReadUntil(int read_until_count, int callback_handle)
    {
        ...
        read_message->set_callback_handle(callback_handle); // Given an unique callback handle.
    ...
    void SignalFlush(bool flush)
    {
        demuxer_stream_adapter_->SignalFlush(flush);
    }
...
private:
    ...
    void OnError(StopTrigger stop_trigger)
    {
        errors_.push_back(stop_trigger);
    }
...
public:
    ...
    void SetUpDataPipe()
    {
        ...
        const MojoCreateDataPipeOptions data_pipe_options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, kDataPipeCapacity };
        ...
        data_stream_sender_ = std::make_unique<FakeRemotingDataStreamSender>(stream_sender.InitWithNewPipeAndPassReceiver(), std::move(consumer_end));
        demuxer_stream_adapter_ = std::make_unique<MockDemuxerStreamAdapter>(task_environment_.GetMainThreadTaskRunner(),
            task_environment_.GetMainThreadTaskRunner(), "test", demuxer_stream_.get(), std::move(stream_sender), std::move(producer_end));
...
TEST_F(DemuxerStreamAdapterTest, WriteOneFrameSmallerThanCapacity)
{
    ...
    openscreen::cast::RpcMessage* last_rpc = demuxer_stream_adapter_->last_received_rpc();
    ASSERT_TRUE(last_rpc);
    ASSERT_EQ(last_rpc->proc(), openscreen::cast::RpcMessage::RPC_DS_READUNTIL_CALLBACK);
