...
public:
    FakeRemotingDataStreamSender(mojo::PendingReceiver<mojom::RemotingDataStreamSender> receiver, mojo::ScopedDataPipeConsumerHandle consumer_handle);

    FakeRemotingDataStreamSender(const FakeRemotingDataStreamSender&) = delete;
    FakeRemotingDataStreamSender& operator=(const FakeRemotingDataStreamSender&) = delete;

    ~FakeRemotingDataStreamSender() override;

    uint32_t send_frame_count() const
    {
        return send_frame_count_;
    }
    uint32_t cancel_in_flight_count() const
    {
        return cancel_in_flight_count_;
    }
    void ResetHistory();
    bool ValidateFrameBuffer(size_t index, size_t size, bool key_frame, int pts_ms);
    void CloseDataPipe();

private:
    // mojom::RemotingDataStreamSender implementation.
    void SendFrame(media::mojom::DecoderBufferPtr buffer, SendFrameCallback callback) final;
...
public:
    ...
    FakeRemoter(mojo::PendingRemote<mojom::RemotingSource> source, bool start_will_fail);
...
public:
    ...
    void Create(mojo::PendingRemote<mojom::RemotingSource> source, mojo::PendingReceiver<mojom::Remoter> receiver) override;

    static std::unique_ptr<RendererController> CreateController(bool start_will_fail);
