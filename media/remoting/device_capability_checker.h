...
// `video_codec`.
bool IsVideoCodecCompatible(const std::string& model_name, media::VideoCodec video_codec);

// Return true if the Cast receiver with `model_name` is compatible to render
// `audio_codec`.
bool IsAudioCodecCompatible(const std::string& model_name, media::AudioCodec audio_codec);
