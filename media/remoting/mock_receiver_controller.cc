...
void MockRemotee::SendAudioFrame(uint32_t frame_count, scoped_refptr<DecoderBuffer> buffer)
{
    mojom::DecoderBufferPtr mojo_buffer = audio_buffer_writer_->WriteDecoderBuffer(std::move(buffer));
    audio_stream_->ReceiveFrame(frame_count, std::move(mojo_buffer));
}

void MockRemotee::SendVideoFrame(uint32_t frame_count, scoped_refptr<DecoderBuffer> buffer)
{
    mojom::DecoderBufferPtr mojo_buffer = video_buffer_writer_->WriteDecoderBuffer(std::move(buffer));
    video_stream_->ReceiveFrame(frame_count, std::move(mojo_buffer));
}

void MockRemotee::OnRemotingSinkReady(mojo::PendingRemote<mojom::RemotingSink> remoting_sink)
{
...
void MockRemotee::StartDataStreams(
    mojo::PendingRemote<mojom::RemotingDataStreamReceiver> audio_stream, mojo::PendingRemote<mojom::RemotingDataStreamReceiver> video_stream)
{
    if (audio_stream.is_valid()) {
        ...
        audio_buffer_writer_ = MojoDecoderBufferWriter::Create(GetDefaultDecoderBufferConverterCapacity(DemuxerStream::AUDIO), &audio_data_pipe);
    ...
    if (video_stream.is_valid()) {
        ...
        video_buffer_writer_ = MojoDecoderBufferWriter::Create(GetDefaultDecoderBufferConverterCapacity(DemuxerStream::VIDEO), &video_data_pipe);
    ...
    : mock_remotee_(new MockRemotee())
{
    ...
    rpc_messenger_.set_send_message_cb_for_testing([this](std::vector<uint8_t> message) { OnSendRpc(message); });
}
