...
public:
    ...
    void SetUp() override
    {
        ...
        stream_provider_ = std::make_unique<StreamProvider>(mock_controller_, base::SingleThreadTaskRunner::GetCurrentDefault());
        ...
        rpc_messenger_->RegisterMessageReceiverCallback(sender_audio_demuxer_stream_handle_,
            [this](std::unique_ptr<openscreen::cast::RpcMessage> message) { OnDemuxerStreamReceivedRpc(DemuxerStream::Type::AUDIO, std::move(message)); });
        rpc_messenger_->RegisterMessageReceiverCallback(sender_video_demuxer_stream_handle_,
            [this](std::unique_ptr<openscreen::cast::RpcMessage> message) { OnDemuxerStreamReceivedRpc(DemuxerStream::Type::VIDEO, std::move(message)); });
    }
    ...
    void OnDemuxerStreamReceivedRpc(DemuxerStream::Type type, std::unique_ptr<openscreen::cast::RpcMessage> message)
    {
        DCHECK(message);
        switch (message->proc()) {
    ...
    void RpcInitializeCallback(DemuxerStream::Type type)
    {
        ...
        rpc->set_handle(type == DemuxerStream::Type::AUDIO ? receiver_audio_demuxer_stream_handle_ : receiver_video_demuxer_stream_handle_);
    ...
    void OnStreamProviderInitialized(PipelineStatus status)
    {
        ...
        audio_stream_ = stream_provider_->GetFirstStream(DemuxerStream::Type::AUDIO);
        video_stream_ = stream_provider_->GetFirstStream(DemuxerStream::Type::VIDEO);
    ...
    void InitializeDemuxer()
    {
        ...
        stream_provider_->Initialize(nullptr, base::BindOnce(&StreamProviderTest::OnStreamProviderInitialized, base::Unretained(this)));
    }
    ...
    void SendRpcReadUntilCallback(DemuxerStream::Type type)
    {
        ...
        message->set_count(0);
        message->set_status(media::cast::ToProtoDemuxerStreamStatus(DemuxerStream::Status::kOk).value());
        rpc_messenger_->SendMessageToRemote(rpc);
    }
    ...
    void OnBufferReadFromDemuxerStream(DemuxerStream::Type type, DemuxerStream::Status status, DemuxerStream::DecoderBufferVector buffers)
    {
        EXPECT_EQ(status, DemuxerStream::Status::kOk);
        EXPECT_EQ(buffers.size(), 1u) << "StreamProviderTest only reads a single-buffer.";
...
TEST_F(StreamProviderTest, ReadBuffer)
{
    ...
    audio_stream_->Read(1, base::BindOnce(&StreamProviderTest::OnBufferReadFromDemuxerStream, base::Unretained(this), DemuxerStream::Type::AUDIO));
    task_environment_.RunUntilIdle();
    EXPECT_EQ(audio_buffer_->size(), received_audio_buffer_->size());
    EXPECT_EQ(audio_buffer_->end_of_stream(), received_audio_buffer_->end_of_stream());
    EXPECT_EQ(audio_buffer_->is_key_frame(), received_audio_buffer_->is_key_frame());

    video_stream_->Read(1, base::BindOnce(&StreamProviderTest::OnBufferReadFromDemuxerStream, base::Unretained(this), DemuxerStream::Type::VIDEO));
    task_environment_.RunUntilIdle();
    EXPECT_EQ(video_buffer_->end_of_stream(), received_video_buffer_->end_of_stream());
}
