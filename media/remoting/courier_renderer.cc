...
constexpr base::TimeDelta kMediaPlaybackDelayThreshold = base::Milliseconds(750);
...
void RegisterForRpcTask(
    openscreen::WeakPtr<openscreen::cast::RpcMessenger> rpc_messenger, int rpc_handle, openscreen::cast::RpcMessenger::ReceiveMessageCallback message_cb)
{
    if (rpc_messenger) {
        rpc_messenger->RegisterMessageReceiverCallback(rpc_handle, std::move(message_cb));
    }
}
void DeregisterFromRpcTask(openscreen::WeakPtr<openscreen::cast::RpcMessenger> rpc_messenger, int rpc_handle)
{
    if (rpc_messenger) {
...
CourierRenderer::CourierRenderer(
    scoped_refptr<base::SequencedTaskRunner> media_task_runner, const base::WeakPtr<RendererController>& controller, VideoRendererSink* video_renderer_sink)
    ...
    , clock_(base::DefaultTickClock::GetInstance())
{
    ...
    // the media thread.
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CourierRenderer::RegisterForRpcMessaging, weak_factory_.GetWeakPtr()));
}
...
CourierRenderer::~CourierRenderer()
{
    ...
    if (video_renderer_sink_) {
        video_renderer_sink_->PaintSingleFrame(VideoFrame::CreateBlackFrame(gfx::Size(1280, 720)));
    }
}

void CourierRenderer::Initialize(MediaResource* media_resource, RendererClient* client, PipelineStatusCallback init_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (state_ != STATE_UNINITIALIZED) {
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_cb), PIPELINE_ERROR_INVALID_STATE));
        return;
    }
    ...
    ::media::DemuxerStream* audio_demuxer_stream = media_resource_->GetFirstStream(DemuxerStream::AUDIO);
    ::media::DemuxerStream* video_demuxer_stream = media_resource_->GetFirstStream(DemuxerStream::VIDEO);

    // Establish remoting data pipe connection using main thread.
    uint32_t data_pipe_capacity = DemuxerStreamAdapter::kMojoDataPipeCapacityInBytes;
    main_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RendererController::StartDataPipe, controller_, data_pipe_capacity, audio_demuxer_stream, video_demuxer_stream,
            base::BindOnce(&CourierRenderer::OnDataPipeCreatedOnMainThread, media_task_runner_, weak_factory_.GetWeakPtr(), rpc_messenger_)));
}

void CourierRenderer::SetLatencyHint(std::optional<base::TimeDelta> latency_hint)
{
}
...
void CourierRenderer::Flush(base::OnceClosure flush_cb)
{
    ...
    if ((audio_demuxer_stream_adapter_ && !flush_audio_count.has_value()) || (video_demuxer_stream_adapter_ && !flush_video_count.has_value())
        || (audio_demuxer_stream_adapter_ && video_demuxer_stream_adapter_ && flush_audio_count.has_value() != flush_video_count.has_value())) {
        return;
    }
    ...
    openscreen::cast::RendererFlushUntil* message = rpc->mutable_renderer_flushuntil_rpc();
...
// static
void CourierRenderer::OnDataPipeCreatedOnMainThread(scoped_refptr<base::SequencedTaskRunner> media_task_runner, base::WeakPtr<CourierRenderer> self,
    openscreen::WeakPtr<RpcMessenger> rpc_messenger, mojo::PendingRemote<mojom::RemotingDataStreamSender> audio,
    mojo::PendingRemote<mojom::RemotingDataStreamSender> video, mojo::ScopedDataPipeProducerHandle audio_handle,
    mojo::ScopedDataPipeProducerHandle video_handle)
{
    media_task_runner->PostTask(FROM_HERE,
        base::BindOnce(&CourierRenderer::OnDataPipeCreated, self, std::move(audio), std::move(video), std::move(audio_handle), std::move(video_handle),
            rpc_messenger ? rpc_messenger->GetUniqueHandle() : RpcMessenger::kInvalidHandle,
            rpc_messenger ? rpc_messenger->GetUniqueHandle() : RpcMessenger::kInvalidHandle));
}

void CourierRenderer::OnDataPipeCreated(mojo::PendingRemote<mojom::RemotingDataStreamSender> audio, mojo::PendingRemote<mojom::RemotingDataStreamSender> video,
    mojo::ScopedDataPipeProducerHandle audio_handle, mojo::ScopedDataPipeProducerHandle video_handle, int audio_rpc_handle, int video_rpc_handle)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (audio_demuxer_stream && audio.is_valid() && audio_handle.is_valid() && audio_rpc_handle != RpcMessenger::kInvalidHandle) {
        audio_demuxer_stream_adapter_
            = std::make_unique<DemuxerStreamAdapter>(main_task_runner_, media_task_runner_, "audio", audio_demuxer_stream, rpc_messenger_, audio_rpc_handle,
                std::move(audio), std::move(audio_handle), base::BindOnce(&CourierRenderer::OnFatalError, base::Unretained(this)));
    }

    // Create video demuxer stream adapter if video is available.
    if (video_demuxer_stream && video.is_valid() && video_handle.is_valid() && video_rpc_handle != RpcMessenger::kInvalidHandle) {
        video_demuxer_stream_adapter_
            = std::make_unique<DemuxerStreamAdapter>(main_task_runner_, media_task_runner_, "video", video_demuxer_stream, rpc_messenger_, video_rpc_handle,
                std::move(video), std::move(video_handle), base::BindOnce(&CourierRenderer::OnFatalError, base::Unretained(this)));
    }
    ...
    openscreen::cast::AcquireDemuxer* message = rpc->mutable_acquire_demuxer_rpc();
    message->set_audio_demuxer_handle(audio_demuxer_stream_adapter_ ? audio_demuxer_stream_adapter_->rpc_handle() : RpcMessenger::kInvalidHandle);
    message->set_video_demuxer_handle(video_demuxer_stream_adapter_ ? video_demuxer_stream_adapter_->rpc_handle() : RpcMessenger::kInvalidHandle);
    SendRpcToRemote(std::move(rpc));
...
void CourierRenderer::OnMessageReceivedOnMainThread(
    scoped_refptr<base::SequencedTaskRunner> media_task_runner, base::WeakPtr<CourierRenderer> self, std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    if (media_task_runner) {
        media_task_runner->PostTask(FROM_HERE, base::BindOnce(&CourierRenderer::OnReceivedRpc, self, std::move(message)));
    } else {
        LOG(WARNING) << "No valid task runner.";
    }
}

void CourierRenderer::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    switch (message->proc()) {
...
void CourierRenderer::SendRpcToRemote(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(main_task_runner_);
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RpcMessenger::SendMessageToRemote, rpc_messenger_, *message));
}

void CourierRenderer::AcquireRendererDone(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    ...
    openscreen::cast::RendererInitialize* init = rpc->mutable_renderer_initialize_rpc();
    init->set_client_handle(rpc_handle_);
    init->set_audio_demuxer_handle(audio_demuxer_stream_adapter_ ? audio_demuxer_stream_adapter_->rpc_handle() : RpcMessenger::kInvalidHandle);
    init->set_video_demuxer_handle(video_demuxer_stream_adapter_ ? video_demuxer_stream_adapter_->rpc_handle() : RpcMessenger::kInvalidHandle);
    init->set_callback_handle(rpc_handle_);
    SendRpcToRemote(std::move(rpc));
}

void CourierRenderer::InitializeCallback(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
...
void CourierRenderer::OnTimeUpdate(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
...
void CourierRenderer::OnBufferingStateChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    ...
    std::optional<BufferingState> state = media::cast::ToMediaBufferingState(message->rendererclient_onbufferingstatechange_rpc().state());
    ...
    if (state == BufferingState::BUFFERING_HAVE_NOTHING) {
        ...
        reason = receiver_is_blocked_on_local_demuxers_ ? DEMUXER_UNDERFLOW : REMOTING_NETWORK_CONGESTION;
...
void CourierRenderer::OnAudioConfigChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    // Shutdown remoting session if receiving malformed RPC message.
    ...
    const auto* audio_config_message = message->mutable_rendererclient_onaudioconfigchange_rpc();
    const openscreen::cast::AudioDecoderConfig pb_audio_config = audio_config_message->audio_decoder_config();
    AudioDecoderConfig out_audio_config;
    media::cast::ConvertProtoToAudioDecoderConfig(pb_audio_config, &out_audio_config);
    DCHECK(out_audio_config.IsValidConfig());

    client_->OnAudioConfigChange(out_audio_config);
}

void CourierRenderer::OnVideoConfigChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    // Shutdown remoting session if receiving malformed RPC message.
    ...
    const auto* video_config_message = message->mutable_rendererclient_onvideoconfigchange_rpc();
    const openscreen::cast::VideoDecoderConfig pb_video_config = video_config_message->video_decoder_config();
    VideoDecoderConfig out_video_config;
    media::cast::ConvertProtoToVideoDecoderConfig(pb_video_config, &out_video_config);
    DCHECK(out_video_config.IsValidConfig());

    client_->OnVideoConfigChange(out_video_config);
}

void CourierRenderer::OnVideoNaturalSizeChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    // Shutdown remoting session if receiving malformed RPC message.
    ...
    if (size_change.width() <= 0 || size_change.height() <= 0)
        return;
    client_->OnVideoNaturalSizeChange(gfx::Size(size_change.width(), size_change.height()));
}

void CourierRenderer::OnVideoOpacityChange(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
...
void CourierRenderer::OnStatisticsUpdate(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message);
    // Shutdown remoting session if receiving malformed RPC message.
    ...
    media::cast::ConvertProtoToPipelineStatistics(message->rendererclient_onstatisticsupdate_rpc(), &stats);
...
void CourierRenderer::OnFatalError(StopTrigger stop_trigger)
{
    ...
    if (state_ != STATE_ERROR) {
        state_ = STATE_ERROR;
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererController::OnRendererFatalError, controller_, stop_trigger));
    }
...
void CourierRenderer::OnMediaTimeUpdated()
{
    ...
    media_time_queue_.push_back(std::make_pair(current_time, current_media_time_));
...
void CourierRenderer::UpdateVideoStatsQueue(int video_frames_decoded, int video_frames_dropped)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    if (flush_cb_)
        return; // Don't manage and check the queue when Flush() is on-going.
    ...
    video_stats_queue_.push_back(std::make_tuple(current_time, video_frames_decoded, video_frames_dropped));
...
void CourierRenderer::ResetMeasurements()
{
    ...
    if (state_ != STATE_ERROR && (audio_demuxer_stream_adapter_ || video_demuxer_stream_adapter_)) {
        data_flow_poll_timer_.Start(FROM_HERE, kDataFlowPollPeriod, this, &CourierRenderer::MeasureAndRecordDataRates);
    }
}
...
void CourierRenderer::MeasureAndRecordDataRates()
{
    ...
    if (audio_demuxer_stream_adapter_) {
        ...
        metrics_recorder_.OnAudioRateEstimate(checked_kbps.ValueOrDefault(std::numeric_limits<int>::max()));
    }
    if (video_demuxer_stream_adapter_) {
        ...
        DCHECK_GE(kilobits_per_second, 0);
        const base::CheckedNumeric<int> checked_kbps = kilobits_per_second;
        metrics_recorder_.OnVideoRateEstimate(checked_kbps.ValueOrDefault(std::numeric_limits<int>::max()));
    }
}
...
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    return ((video_demuxer_stream_adapter_ && video_demuxer_stream_adapter_->is_processing_read_request() && !video_demuxer_stream_adapter_->is_data_pending())
        || (audio_demuxer_stream_adapter_ && audio_demuxer_stream_adapter_->is_processing_read_request() && !audio_demuxer_stream_adapter_->is_data_pending()));
}

void CourierRenderer::RegisterForRpcMessaging()
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    auto receive_callback = base::BindPostTaskToCurrentDefault(base::BindRepeating(&CourierRenderer::OnReceivedRpc, weak_factory_.GetWeakPtr()));

    main_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&RegisterForRpcTask, rpc_messenger_, rpc_handle_,
            [cb = std::move(receive_callback)](

                std::unique_ptr<openscreen::cast::RpcMessage> message) { cb.Run(std::move(message)); }));
}
...
void CourierRenderer::DeregisterFromRpcMessaging()
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    if (rpc_messenger_) {
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&DeregisterFromRpcTask, rpc_messenger_, rpc_handle_));
    }
}
