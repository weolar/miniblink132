...
// static
void StreamProvider::MediaStream::CreateOnMainThread(RpcMessenger* rpc_messenger, Type type, int32_t handle,
    const scoped_refptr<base::SequencedTaskRunner>& media_task_runner, base::OnceCallback<void(MediaStream::UniquePtr)> callback)
{
    MediaStream::UniquePtr stream(new MediaStream(rpc_messenger, type, handle, media_task_runner), &DestructionHelper);
...
StreamProvider::MediaStream::MediaStream(
    RpcMessenger* rpc_messenger, Type type, int remote_handle, const scoped_refptr<base::SequencedTaskRunner>& media_task_runner)
    ...
    , rpc_handle_(rpc_messenger_->GetUniqueHandle())
{
    ...
    auto receive_callback = base::BindPostTask(media_task_runner_, BindRepeating(&MediaStream::OnReceivedRpc, media_weak_this_));
    rpc_messenger_->RegisterMessageReceiverCallback(
        rpc_handle_, [cb = std::move(receive_callback)](std::unique_ptr<openscreen::cast::RpcMessage> message) { cb.Run(std::move(message)); });
}
...
void StreamProvider::MediaStream::SendRpcMessageOnMainThread(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    // |rpc_messenger_| is owned by |receiver_controller_| which is a singleton
    // per process, so it's safe to use Unretained() here.
    main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&RpcMessenger::SendMessageToRemote, base::Unretained(rpc_messenger_), *message));
}

void StreamProvider::MediaStream::Initialize(base::OnceClosure init_done_callback)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void StreamProvider::MediaStream::InitializeDataPipe(mojo::ScopedDataPipeConsumerHandle data_pipe)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());

    decoder_buffer_reader_ = std::make_unique<MojoDecoderBufferReader>(std::move(data_pipe));
    CompleteInitialize();
}

void StreamProvider::MediaStream::ReceiveFrame(uint32_t count, mojom::DecoderBufferPtr buffer)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(decoder_buffer_reader_);

    auto callback = base::BindPostTaskToCurrentDefault(base::BindOnce(&MediaStream::AppendBuffer, media_weak_this_, count));
    decoder_buffer_reader_->ReadDecoderBuffer(std::move(buffer), std::move(callback));
}
...
void StreamProvider::MediaStream::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void StreamProvider::MediaStream::OnInitializeCallback(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    const openscreen::cast::DemuxerStreamInitializeCallback callback_message = message->demuxerstream_initializecb_rpc();
    if (callback_message.type() != type_) {
        OnError("Wrong type");
        return;
    }

    if ((type_ == DemuxerStream::AUDIO && audio_decoder_config_.IsValidConfig()) || (type_ == DemuxerStream::VIDEO && video_decoder_config_.IsValidConfig())) {
        OnError("Duplicate initialization");
        return;
    }

    if (type_ == DemuxerStream::AUDIO && callback_message.has_audio_decoder_config()) {
        const openscreen::cast::AudioDecoderConfig audio_message = callback_message.audio_decoder_config();
        media::cast::ConvertProtoToAudioDecoderConfig(audio_message, &audio_decoder_config_);
        if (!audio_decoder_config_.IsValidConfig()) {
            OnError("Invalid audio config");
            return;
        }
    } else if (type_ == DemuxerStream::VIDEO && callback_message.has_video_decoder_config()) {
        const openscreen::cast::VideoDecoderConfig video_message = callback_message.video_decoder_config();
        media::cast::ConvertProtoToVideoDecoderConfig(video_message, &video_decoder_config_);
...
void StreamProvider::MediaStream::OnReadUntilCallback(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    ...
    read_until_sent_ = false;
    const openscreen::cast::DemuxerStreamReadUntilCallback callback_message = message->demuxerstream_readuntilcb_rpc();
    total_received_frame_count_ = callback_message.count();

    if (media::cast::ToDemuxerStreamStatus(callback_message.status()) == kConfigChanged) {
...
void StreamProvider::MediaStream::UpdateAudioConfig(const openscreen::cast::AudioDecoderConfig& audio_message)
{
    DCHECK(type_ == AUDIO);
...
void StreamProvider::MediaStream::UpdateVideoConfig(const openscreen::cast::VideoDecoderConfig& video_message)
{
    DCHECK(type_ == VIDEO);
...
void StreamProvider::MediaStream::SendReadUntil()
{
    ...
    std::unique_ptr<openscreen::cast::RpcMessage> rpc(new openscreen::cast::RpcMessage());
    rpc->set_handle(remote_handle_);
...
void StreamProvider::MediaStream::Read(uint32_t /*count*/, ReadCB read_cb)
{
    ...
    if (buffers_.empty() && (next_audio_decoder_config_.IsValidConfig() || next_video_decoder_config_.IsValidConfig())) {
        CompleteRead(DemuxerStream::kConfigChanged);
        return;
    }
...
void StreamProvider::MediaStream::AppendBuffer(uint32_t count, scoped_refptr<DecoderBuffer> buffer)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
StreamProvider::StreamProvider(ReceiverController* receiver_controller, const scoped_refptr<base::SequencedTaskRunner>& media_task_runner)
    : main_task_runner_(base::SingleThreadTaskRunner::GetCurrentDefault())
    , media_task_runner_(media_task_runner)
    ...
    , rpc_messenger_(receiver_controller_->rpc_messenger())
{
    ...
    auto callback = base::BindPostTask(media_task_runner_, base::BindRepeating(&StreamProvider::OnReceivedRpc, media_weak_this_));
    rpc_messenger_->RegisterMessageReceiverCallback(
        RpcMessenger::kAcquireDemuxerHandle, [cb = std::move(callback)](std::unique_ptr<openscreen::cast::RpcMessage> message) { cb.Run(std::move(message)); });
}

StreamProvider::~StreamProvider()
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    rpc_messenger_->UnregisterMessageReceiverCallback(RpcMessenger::kAcquireDemuxerHandle);
}
...
void StreamProvider::Initialize(DemuxerHost* host, PipelineStatusCallback status_cb)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void StreamProvider::Seek(base::TimeDelta time, PipelineStatusCallback seek_cb)
{
    media_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(seek_cb), PIPELINE_OK));
}
...
std::optional<container_names::MediaContainerName> StreamProvider::GetContainerForMetrics() const
{
    return std::optional<container_names::MediaContainerName>();
}

void StreamProvider::OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
...
void StreamProvider::OnSelectedVideoTrackChanged(
    const std::vector<media::MediaTrack::Id>& track_ids, base::TimeDelta curr_time, TrackChangeCB change_completed_cb)
{
    std::vector<DemuxerStream*> streams;
    std::move(change_completed_cb).Run(streams);
    DVLOG(1) << "Track changes are not supported.";
...
void StreamProvider::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    switch (message->proc()) {
...
void StreamProvider::OnAcquireDemuxer(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    DCHECK(message->has_acquire_demuxer_rpc());

    int32_t audio_demuxer_handle = message->acquire_demuxer_rpc().audio_demuxer_handle();
    int32_t video_demuxer_handle = message->acquire_demuxer_rpc().video_demuxer_handle();
    ...
    if (has_audio_) {
        auto callback = base::BindPostTaskToCurrentDefault(base::BindOnce(&StreamProvider::OnAudioStreamCreated, media_weak_this_));
        main_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &MediaStream::CreateOnMainThread, rpc_messenger_, DemuxerStream::AUDIO, audio_demuxer_handle, media_task_runner_, std::move(callback)));
    }

    if (has_video_) {
        auto callback = base::BindPostTaskToCurrentDefault(base::BindOnce(&StreamProvider::OnVideoStreamCreated, media_weak_this_));
        main_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &MediaStream::CreateOnMainThread, rpc_messenger_, DemuxerStream::VIDEO, video_demuxer_handle, media_task_runner_, std::move(callback)));
    }
}
...
void StreamProvider::OnAudioStreamCreated(MediaStream::UniquePtr stream)
{
    ...
    audio_stream_->Initialize(base::BindOnce(&StreamProvider::OnAudioStreamInitialized, media_weak_this_));
    InitializeDataPipe();
}
...
void StreamProvider::OnVideoStreamCreated(MediaStream::UniquePtr stream)
{
    ...
    video_stream_->Initialize(base::BindOnce(&StreamProvider::OnVideoStreamInitialized, media_weak_this_));
    InitializeDataPipe();
}
...
void StreamProvider::InitializeDataPipe()
{
    ...
    receiver_controller_->StartDataStreams(has_audio_ ? audio_stream_->BindNewPipeAndPassRemote() : mojo::NullRemote(),
        has_video_ ? video_stream_->BindNewPipeAndPassRemote() : mojo::NullRemote());
}
...
void StreamProvider::CompleteInitialize()
{
    ...
    if ((has_audio_ && !audio_stream_initialized_) || (has_video_ && !video_stream_initialized_) || !init_done_callback_)
        return;
...
void default_delete<media::remoting::StreamProvider>::operator()(media::remoting::StreamProvider* ptr) const
{
    ptr->Destroy();
}
