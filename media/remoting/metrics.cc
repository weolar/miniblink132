...
{
    UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStartTrigger", *start_trigger_, START_TRIGGER_MAX + 1);
...
void SessionMetricsRecorder::StartSessionFailed(mojom::RemotingStartFailReason reason)
{
...
void SessionMetricsRecorder::WillStopSession(StopTrigger trigger)
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger", trigger, STOP_TRIGGER_MAX + 1);

    // Record the session duration.
    const base::TimeDelta session_duration = base::TimeTicks::Now() - start_time_;
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Remoting.SessionDuration", session_duration, base::Seconds(15), base::Hours(12), 50);

    if (session_duration <= base::Seconds(15)) {
        // Record the session duration in finer scale for short sessions
        UMA_HISTOGRAM_CUSTOM_TIMES("Media.Remoting.ShortSessionDuration", session_duration, base::Seconds(0.1), base::Seconds(15), 60);

        if (session_duration <= base::Seconds(0.1)) {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration0To100MilliSec", trigger, STOP_TRIGGER_MAX + 1);
        } else if (session_duration <= base::Seconds(1)) {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration100MilliSecTo1Sec", trigger, STOP_TRIGGER_MAX + 1);
        } else if (session_duration <= base::Seconds(3)) {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration1To3Sec", trigger, STOP_TRIGGER_MAX + 1);
        } else if (session_duration <= base::Seconds(5)) {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration3To5Sec", trigger, STOP_TRIGGER_MAX + 1);
        } else if (session_duration <= base::Seconds(10)) {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration5To10Sec", trigger, STOP_TRIGGER_MAX + 1);
        } else {
            UMA_HISTOGRAM_ENUMERATION("Media.Remoting.SessionStopTrigger.Duration10To15Sec", trigger, STOP_TRIGGER_MAX + 1);
        }
    }
...
void SessionMetricsRecorder::OnPipelineMetadataChanged(const PipelineMetadata& metadata)
{
    if (metadata.has_audio && metadata.audio_decoder_config.IsValidConfig()) {
        ...
        const bool need_to_record_audio_configuration = start_trigger_
            && (config.codec() != last_audio_codec_ || config.channel_layout() != last_channel_layout_ || config.samples_per_second() != last_sample_rate_);
    ...
    if (metadata.has_video && metadata.video_decoder_config.IsValidConfig()) {
        ...
        const bool need_to_record_video_configuration
            = start_trigger_ && (config.codec() != last_video_codec_ || config.profile() != last_video_profile_ || metadata.natural_size != last_natural_size_);
...
{
    if (remote_playback_is_disabled_ && disabled == remote_playback_is_disabled_.value()) {
        return; // De-dupe redundant notifications.
    }
    UMA_HISTOGRAM_BOOLEAN("Media.Remoting.RemotePlaybackEnabledByPage", !disabled);
    remote_playback_is_disabled_ = disabled;
}

void SessionMetricsRecorder::RecordVideoPixelRateSupport(PixelRateSupport support)
{
    if (did_record_pixel_rate_support_) {
        return;
    }
    did_record_pixel_rate_support_ = true;
    base::UmaHistogramEnumeration("Media.Remoting.VideoPixelRateSupport", support);
}

void SessionMetricsRecorder::RecordCompatibility(RemotingCompatibility compatibility)
{
...
void SessionMetricsRecorder::RecordAudioConfiguration()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Remoting.AudioChannelLayout", last_channel_layout_, CHANNEL_LAYOUT_MAX + 1);
    AudioSampleRate asr;
    if (ToAudioSampleRate(last_sample_rate_, &asr)) {
        UMA_HISTOGRAM_ENUMERATION("Media.Remoting.AudioSamplesPerSecond", asr, kAudioSampleRateMax + 1);
    } else {
        UMA_HISTOGRAM_COUNTS_1M("Media.Remoting.AudioSamplesPerSecondUnexpected", last_sample_rate_);
    }
}
...
void SessionMetricsRecorder::RecordVideoConfiguration()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Remoting.VideoCodecProfile", last_video_profile_, VIDEO_CODEC_PROFILE_MAX + 1);
    UMA_HISTOGRAM_CUSTOM_ENUMERATION(
        "Media.Remoting.VideoNaturalWidth", last_natural_size_.width(), base::CustomHistogram::ArrayToCustomEnumRanges(kVideoWidthBuckets));
    // Intentionally use integer division to truncate the result.
    const int aspect_ratio_100 = last_natural_size_.height() ? (last_natural_size_.width() * 100) / last_natural_size_.height() : kInfiniteRatio;
    UMA_HISTOGRAM_CUSTOM_ENUMERATION(
        "Media.Remoting.VideoAspectRatio", aspect_ratio_100, base::CustomHistogram::ArrayToCustomEnumRanges(kCommonAspectRatios100));
}
...
void SessionMetricsRecorder::RecordTrackConfiguration()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Remoting.TrackConfiguration", config, TRACK_CONFIGURATION_MAX + 1);
}
...
void RendererMetricsRecorder::OnRendererInitialized()
{
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Remoting.TimeUntilRemoteInitialized", elapsed_since_start, base::Milliseconds(10), base::Seconds(30), 50);
}
...
void RendererMetricsRecorder::OnEvidenceOfPlayoutAtReceiver()
{
    if (did_record_first_playout_)
        return;
    const base::TimeDelta elapsed_since_start = base::TimeTicks::Now() - start_time_;
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Remoting.TimeUntilFirstPlayout", elapsed_since_start, base::Milliseconds(10), base::Seconds(30), 50);
    did_record_first_playout_ = true;
}

void RendererMetricsRecorder::OnAudioRateEstimate(int kilobits_per_second)
{
    UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Remoting.AudioBitrate", kilobits_per_second, 1, 1024, 50);
}

void RendererMetricsRecorder::OnVideoRateEstimate(int kilobits_per_second)
{
    UMA_HISTOGRAM_CUSTOM_COUNTS("Media.Remoting.VideoBitrate", kilobits_per_second, 1, 16 * 1024, 50);
}
