...
RemotingRendererFactory::RemotingRendererFactory(mojo::PendingRemote<mojom::Remotee> remotee, std::unique_ptr<RendererFactory> renderer_factory,
    ...
    , media_task_runner_(media_task_runner)
{
    ...
    auto receive_callback = base::BindPostTask(media_task_runner, BindRepeating(&RemotingRendererFactory::OnAcquireRenderer, weak_factory_.GetWeakPtr()));
    rpc_messenger_->RegisterMessageReceiverCallback(RpcMessenger::kAcquireRendererHandle,
...
std::unique_ptr<Renderer> RemotingRendererFactory::CreateRenderer(const scoped_refptr<base::SequencedTaskRunner>& media_task_runner,
    ...
    RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space)
{
    ...
    auto receiver = std::make_unique<Receiver>(renderer_handle_, remote_renderer_handle_, receiver_controller_, media_task_runner,
        real_renderer_factory_->CreateRenderer(
            media_task_runner, worker_task_runner, audio_renderer_sink, video_renderer_sink, request_overlay_info_cb, target_color_space),
        base::BindOnce(&RemotingRendererFactory::OnAcquireRendererDone, base::Unretained(this)));
...
void RemotingRendererFactory::OnReceivedRpc(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    DCHECK(message);
...
void RemotingRendererFactory::OnAcquireRenderer(std::unique_ptr<openscreen::cast::RpcMessage> message)
{
    ...
    if (waiting_for_remote_handle_receiver_) {
        ...
        media_task_runner_->PostTask(FROM_HERE, base::BindOnce(&Receiver::SetRemoteHandle, waiting_for_remote_handle_receiver_, remote_renderer_handle_));
    }
}
...
void RemotingRendererFactory::OnAcquireRendererDone(int receiver_rpc_handle)
{
    ...
    DVLOG(3) << __func__ << ": Issues RPC_ACQUIRE_RENDERER_DONE RPC message. remote_handle=" << remote_renderer_handle_
             << " rpc_handle=" << receiver_rpc_handle;
    auto rpc = media::cast::CreateMessageForAcquireRendererDone(receiver_rpc_handle);
