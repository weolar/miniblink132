...
{
    static const base::NoDestructor<std::vector<DeviceCategoryToType>> device_category_to_type(
        { { KSCATEGORY_AUDIO, base::SystemMonitor::DEVTYPE_AUDIO }, { KSCATEGORY_VIDEO, base::SystemMonitor::DEVTYPE_VIDEO_CAPTURE } });
...
public:
    ...
    void Register(HWND hwnd)
    {
        ...
        for (size_t i = 0; i < GetDeviceCategoryToType().size(); ++i) {
            ...
            if (core_audio_support && KSCATEGORY_AUDIO == GetDeviceCategoryToType()[i].device_category) {
                continue;
            }

            filter.dbcc_classguid = GetDeviceCategoryToType()[i].device_category;
            DCHECK_EQ(notifications_[i], static_cast<HDEVNOTIFY>(NULL));
            notifications_[i] = RegisterDeviceNotification(hwnd, &filter, DEVICE_NOTIFY_WINDOW_HANDLE);
            DPLOG_IF(ERROR, !notifications_[i]) << "RegisterDeviceNotification failed";
        }
    }
...
SystemMessageWindowWin::SystemMessageWindowWin()
{
    ...
    singleton_hwnd_observer_ = std::make_unique<gfx::SingletonHwndObserver>(base::BindRepeating(&SystemMessageWindowWin::WndProc, base::Unretained(this)));
    device_notifications_ = std::make_unique<DeviceNotifications>(gfx::SingletonHwnd::GetInstance()->hwnd());
}
...
void SystemMessageWindowWin::WndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
