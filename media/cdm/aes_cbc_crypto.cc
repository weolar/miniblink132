...
bool AesCbcCrypto::Initialize(const crypto::SymmetricKey& key, base::span<const uint8_t> iv)
{
...
bool AesCbcCrypto::Decrypt(base::span<const uint8_t> encrypted_data, uint8_t* decrypted_data)
{
    crypto::OpenSSLErrStackTracer err_tracer(FROM_HERE);

    if (encrypted_data.size_bytes() % EVP_CIPHER_CTX_block_size(ctx_.get()) != 0) {
        DVLOG(1) << "Encrypted bytes not a multiple of block size.";
        return false;
    }

    int out_length;
    if (!EVP_DecryptUpdate(ctx_.get(), decrypted_data, &out_length, encrypted_data.data(), encrypted_data.size_bytes())) {
