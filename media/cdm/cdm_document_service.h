...
public:
    ...
    using ChallengePlatformCB = base::OnceCallback<void(
    ...
    using StorageIdCB = base::OnceCallback<void(uint32_t version, const std::vector<uint8_t>& storage_id)>;

#if BUILDFLAG(IS_WIN)
    using GetMediaFoundationCdmDataCB = base::OnceCallback<void(std::unique_ptr<MediaFoundationCdmData>)>;
#endif // BUILDFLAG(IS_WIN)
    ...
    //                               requested service.
    virtual void ChallengePlatform(const std::string& service_id, const std::string& challenge, ChallengePlatformCB callback) = 0;
    ...
    virtual void GetMediaFoundationCdmData(GetMediaFoundationCdmDataCB callback) = 0;
