...
void ExternalClearKeyTestHelper::LoadLibrary()
{
    ...
    cdm_base_path = cdm_base_path.Append(GetPlatformSpecificDirectory(kClearKeyCdmBaseDirectory));
    library_path_ = cdm_base_path.AppendASCII(base::GetLoadableModuleName(kClearKeyCdmLibraryName));
    ...
    InitializeCdmFunc initialize_cdm_func = reinterpret_cast<InitializeCdmFunc>(library_.GetFunctionPointer(MAKE_STRING(INITIALIZE_CDM_MODULE)));
...
void ExternalClearKeyTestHelper::UnloadLibrary()
{
    ...
    DeinitializeCdmFunc deinitialize_cdm_func = reinterpret_cast<DeinitializeCdmFunc>(library_.GetFunctionPointer("DeinitializeCdmModule"));
