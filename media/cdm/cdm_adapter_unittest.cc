...
const uint8_t kKeyIdAsPssh[] = {
    ...
    0xAC, 0xE3, 0x3C, 0x1E, 0x52, 0xE2, 0xFB, 0x4B, 0x00, 0x00, 0x00, 0x01, // key count
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // key
    0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x00, 0x00, 0x00, 0x00, // datasize
};
...
class CdmAdapterTestBase : public testing::Test, public testing::WithParamInterface<int> {
public:
    ...
    CdmAdapterTestBase()
    {
        base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(
            switches::kOverrideEnabledCdmInterfaceVersion, base::NumberToString(GetCdmInterfaceVersion()));
    }
...
protected:
    ...
    void InitializeWithCdmConfigAndExpect(const CdmConfig& cdm_config, ExpectedResult expected_result)
    {
        auto allocator = std::make_unique<SimpleCdmAllocator>();
        auto cdm_helper = std::make_unique<StrictMock<MockCdmAuxiliaryHelper>>(std::move(allocator));
        cdm_helper_ = cdm_helper.get();
        CdmAdapter::Create(cdm_config, GetCreateCdmFunc(), std::move(cdm_helper),
            ...
            base::BindOnce(&CdmAdapterTestBase::OnCdmCreated, base::Unretained(this), expected_result));
        RunUntilIdle();
    ...
    void OnCdmCreated(ExpectedResult expected_result, const scoped_refptr<ContentDecryptionModule>& cdm, CreateCdmStatus status)
    {
        if (cdm) {
            ASSERT_EQ(expected_result, SUCCESS) << "CDM creation succeeded unexpectedly.";
            CdmAdapter* cdm_adapter = static_cast<CdmAdapter*>(cdm.get());
            ASSERT_EQ(GetCdmInterfaceVersion(), cdm_adapter->GetInterfaceVersion());
            cdm_ = cdm;
        } else {
            ASSERT_EQ(expected_result, FAILURE) << "status = " << static_cast<int>(status);
        }
    }
...
protected:
    ...
    void CreateSessionAndExpect(EmeInitDataType data_type, const std::vector<uint8_t>& key_id, ExpectedResult expected_result)
    {
        DCHECK(!key_id.empty());
        ...
        cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, data_type, key_id, CreateSessionPromise(expected_result));
        RunUntilIdle();
    }
    ...
    void LoadSessionAndExpect(const std::string& session_id, ExpectedResult expected_result)
    {
        ...
        cdm_->LoadSession(CdmSessionType::kTemporary, session_id, CreateSessionPromise(expected_result));
        RunUntilIdle();
    }
    ...
    void UpdateSessionAndExpect(std::string session_id, const std::string& key, ExpectedResult expected_result, bool new_key_expected)
    {
        ...
        if (expected_result == SUCCESS) {
            ...
            EXPECT_CALL(cdm_client_, OnSessionExpirationUpdate(session_id, IsNullTime()));
        } else {
...
private:
    ...
    MOCK_METHOD3(OnReject, void(CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message));
    ...
    std::unique_ptr<SimpleCdmPromise> CreatePromise(ExpectedResult expected_result)
    {
        ...
        auto promise = std::make_unique<CdmCallbackPromise<>>(base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnResolve, base::Unretained(this)),
            base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnReject, base::Unretained(this)));
        return promise;
    }
    ...
    {
        if (expected_result == SUCCESS) {
            EXPECT_CALL(*this, OnResolveWithSession(_)).WillOnce(SaveArg<0>(&session_id_));
        } else {
            EXPECT_CALL(*this, OnReject(_, _, IsNotEmpty()));
        }

        auto promise
            = std::make_unique<CdmCallbackPromise<std::string>>(base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnResolveWithSession, base::Unretained(this)),
                base::BindOnce(&CdmAdapterTestWithClearKeyCdm::OnReject, base::Unretained(this)));
        return promise;
    }
...
#if BUILDFLAG(ENABLE_CDM_HOST_VERIFICATION)
    CdmModule::GetInstance()->Initialize(base::FilePath(FILE_PATH_LITERAL("no_library_here")), {});
#else
    CdmModule::GetInstance()->Initialize(base::FilePath(FILE_PATH_LITERAL("no_library_here")));
#endif // BUILDFLAG(ENABLE_CDM_HOST_VERIFICATION)
...
TEST_P(CdmAdapterTestWithClearKeyCdm, CreateKeyIdsSession)
{
    ...
    std::vector<uint8_t> key_id(kKeyIdAsJWK, kKeyIdAsJWK + std::size(kKeyIdAsJWK) - 1);
...
TEST_P(CdmAdapterTestWithClearKeyCdm, CreateCencSession)
{
    ...
    std::vector<uint8_t> key_id(kKeyIdAsPssh, kKeyIdAsPssh + std::size(kKeyIdAsPssh));
...
TEST_P(CdmAdapterTestWithMockCdm, ChallengePlatform)
{
    ...
    EXPECT_CALL(*cdm_helper_, ChallengePlatformCalled(service_id, challenge)).WillOnce(Return(true));
...
// false.
TEST_P(CdmAdapterTestWithMockCdm, ChallengePlatform_DistinctiveIdentifierNotAllowed)
{
    CdmConfig cdm_config = GetCdmConfig();
    cdm_config.allow_distinctive_identifier = false;
    InitializeWithCdmConfig(cdm_config);

    EXPECT_CALL(*mock_library_cdm_, OnPlatformChallengeResponse(IsNullPlatformChallengeResponse()));
    std::string service_id = "service_id";
    std::string challenge = "challenge";
    cdm_host_proxy_->SendPlatformChallenge(service_id.data(), service_id.size(), challenge.data(), challenge.size());
    RunUntilIdle();
