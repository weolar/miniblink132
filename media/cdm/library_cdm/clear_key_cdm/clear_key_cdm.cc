...
static scoped_refptr<media::DecoderBuffer> CopyDecoderBufferFrom(const cdm::InputBuffer_2& input_buffer)
{
    ...
    scoped_refptr<media::DecoderBuffer> output_buffer = media::DecoderBuffer::CopyFrom(
    ...
    for (uint32_t i = 0; i < input_buffer.num_subsamples; ++i) {
        subsamples.push_back(media::SubsampleEntry(input_buffer.subsamples[i].clear_bytes, input_buffer.subsamples[i].cipher_bytes));
    }

    const std::string key_id_string(reinterpret_cast<const char*>(input_buffer.key_id), input_buffer.key_id_size);
    const std::string iv_string(reinterpret_cast<const char*>(input_buffer.iv), input_buffer.iv_size);
    if (input_buffer.encryption_scheme == cdm::EncryptionScheme::kCenc) {
        output_buffer->set_decrypt_config(media::DecryptConfig::CreateCencConfig(key_id_string, iv_string, subsamples));
    } else {
        ...
        output_buffer->set_decrypt_config(media::DecryptConfig::CreateCbcsConfig(
            key_id_string, iv_string, subsamples, media::EncryptionPattern(input_buffer.pattern.crypt_byte_block, input_buffer.pattern.skip_byte_block)));
    }
...
void ConvertCdmKeysInfo(const media::CdmKeysInfo& keys_info, std::vector<cdm::KeyInformation>* keys_vector)
{
...
void* CreateCdmInstance(int cdm_interface_version, const char* key_system, uint32_t key_system_size, GetCdmHostFunc get_cdm_host_func, void* user_data)
{
    ...
    if (key_system_string != media::kExternalClearKeyKeySystem && key_system_string != media::kExternalClearKeyDecryptOnlyKeySystem
        && key_system_string != media::kExternalClearKeyMessageTypeTestKeySystem && key_system_string != media::kExternalClearKeyFileIOTestKeySystem
        && key_system_string != media::kExternalClearKeyOutputProtectionTestKeySystem
        && key_system_string != media::kExternalClearKeyPlatformVerificationTestKeySystem && key_system_string != media::kExternalClearKeyCrashKeySystem
        && key_system_string != media::kExternalClearKeyVerifyCdmHostTestKeySystem && key_system_string != media::kExternalClearKeyStorageIdTestKeySystem
        && key_system_string != media::kExternalClearKeyDifferentCdmTypeTestKeySystem) {
    ...
    if (cdm_interface_version == CDM_10::kVersion) {
        CDM_10::Host* host = static_cast<CDM_10::Host*>(get_cdm_host_func(CDM_10::Host::kVersion, user_data));
        if (!host)
            return nullptr;

        DVLOG(1) << __func__ << ": Create ClearKeyCdm with CDM_10::Host.";
        return static_cast<CDM_10*>(new media::ClearKeyCdm(host, key_system_string));
    }

    if (cdm_interface_version == CDM_11::kVersion) {
        CDM_11::Host* host = static_cast<CDM_11::Host*>(get_cdm_host_func(CDM_11::Host::kVersion, user_data));
        if (!host)
            return nullptr;

        DVLOG(1) << __func__ << ": Create ClearKeyCdm with CDM_11::Host.";
        return static_cast<CDM_11*>(new media::ClearKeyCdm(host, key_system_string));
    }
...
constexpr cdm::ColorSpace kUnspecifiedColorSpace = { 2, 2, 2, cdm::ColorRange::kInvalid };

cdm::VideoDecoderConfig_3 ToVideoDecoderConfig_3(cdm::VideoDecoderConfig_2 config)
{
    cdm::VideoDecoderConfig_3 result = { config.codec, config.profile, config.format, kUnspecifiedColorSpace, config.coded_size, config.extra_data,
...
ClearKeyCdm::ClearKeyCdm(HostInterface* host, const std::string& key_system)
    ...
    , cdm_(
          base::MakeRefCounted<ClearKeyPersistentSessionCdm>(cdm_host_proxy_.get(), base::BindRepeating(&ClearKeyCdm::OnSessionMessage, base::Unretained(this)),
              base::BindRepeating(&ClearKeyCdm::OnSessionClosed, base::Unretained(this)),
              base::BindRepeating(&ClearKeyCdm::OnSessionKeysChange, base::Unretained(this)),
              base::BindRepeating(&ClearKeyCdm::OnSessionExpirationUpdate, base::Unretained(this))))
...
void ClearKeyCdm::Initialize(bool allow_distinctive_identifier, bool allow_persistent_state, bool /* use_hw_secure_codecs */)
{
...
void ClearKeyCdm::GetStatusForPolicy(uint32_t promise_id, const cdm::Policy& policy)
{
    ...
    cdm_host_proxy_->OnResolveKeyStatusPromise(promise_id, cdm::kOutputRestricted);
}

void ClearKeyCdm::CreateSessionAndGenerateRequest(
    uint32_t promise_id, cdm::SessionType session_type, cdm::InitDataType init_data_type, const uint8_t* init_data, uint32_t init_data_size)
{
    DVLOG(1) << __func__;

    if (session_type != cdm::kTemporary && !allow_persistent_state_) {
        OnPromiseFailed(promise_id, CdmPromise::Exception::INVALID_STATE_ERROR, 0, "Persistent state not allowed.");
        return;
    }

    auto promise = std::make_unique<CdmCallbackPromise<std::string>>(base::BindOnce(&ClearKeyCdm::OnSessionCreated, base::Unretained(this), promise_id),
        base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id));
    cdm_->CreateSessionAndGenerateRequest(
        ToMediaSessionType(session_type), ToEmeInitDataType(init_data_type), std::vector<uint8_t>(init_data, init_data + init_data_size), std::move(promise));
...
void ClearKeyCdm::LoadSession(uint32_t promise_id, cdm::SessionType session_type, const char* session_id, uint32_t session_id_length)
{
    DVLOG(1) << __func__;
    ...
    auto promise = std::make_unique<CdmCallbackPromise<std::string>>(base::BindOnce(&ClearKeyCdm::OnSessionCreated, base::Unretained(this), promise_id),
        base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id));
    cdm_->LoadSession(ToMediaSessionType(session_type), std::move(web_session_str), std::move(promise));
}

void ClearKeyCdm::UpdateSession(uint32_t promise_id, const char* session_id, uint32_t session_id_length, const uint8_t* response, uint32_t response_size)
{
    DVLOG(1) << __func__;
    std::string web_session_str(session_id, session_id_length);
    std::vector<uint8_t> response_vector(response, response + response_size);

    auto promise = std::make_unique<CdmCallbackPromise<>>(base::BindOnce(&ClearKeyCdm::OnUpdateSuccess, base::Unretained(this), promise_id, web_session_str),
        base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id));

    cdm_->UpdateSession(session_id, response_vector, std::move(promise));
}

void ClearKeyCdm::OnUpdateSuccess(uint32_t promise_id, const std::string& session_id)
{
    ...
    if (key_system_ == kExternalClearKeyMessageTypeTestKeySystem) {
        ...
        if (host_interface_version_ >= cdm::Host_10::kVersion && !has_sent_individualization_request_) {
            has_sent_individualization_request_ = true;
            const std::string request = kDummyIndividualizationRequest;
            cdm_host_proxy_->OnSessionMessage(session_id.data(), session_id.length(), cdm::kIndividualizationRequest, request.data(), request.size());
        }
    }

    cdm_host_proxy_->OnExpirationChange(session_id.data(), session_id.length(), expiration);

    // Resolve the promise.
    OnPromiseResolved(promise_id);
}

void ClearKeyCdm::CloseSession(uint32_t promise_id, const char* session_id, uint32_t session_id_length)
{
    DVLOG(1) << __func__;
    std::string web_session_str(session_id, session_id_length);

    auto promise = std::make_unique<CdmCallbackPromise<>>(base::BindOnce(&ClearKeyCdm::OnPromiseResolved, base::Unretained(this), promise_id),
        base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id));
    cdm_->CloseSession(std::move(web_session_str), std::move(promise));
}

void ClearKeyCdm::RemoveSession(uint32_t promise_id, const char* session_id, uint32_t session_id_length)
{
    DVLOG(1) << __func__;
    std::string web_session_str(session_id, session_id_length);
...
void ClearKeyCdm::SetServerCertificate(uint32_t promise_id, const uint8_t* server_certificate_data, uint32_t server_certificate_data_size)
{
    DVLOG(1) << __func__;
    auto promise = std::make_unique<CdmCallbackPromise<>>(base::BindOnce(&ClearKeyCdm::OnPromiseResolved, base::Unretained(this), promise_id),
        base::BindOnce(&ClearKeyCdm::OnPromiseFailed, base::Unretained(this), promise_id));
    cdm_->SetServerCertificate(std::vector<uint8_t>(server_certificate_data, server_certificate_data + server_certificate_data_size), std::move(promise));
}
...
void ClearKeyCdm::TimerExpired(void* context)
{
    ...
    if (key_system_ == kExternalClearKeyMessageTypeTestKeySystem) {
        ...
        cdm_host_proxy_->OnSessionMessage(
            last_session_id_.data(), last_session_id_.length(), cdm::kLicenseRenewal, renewal_message.data(), renewal_message.length());
    } else if (key_system_ == kExternalClearKeyOutputProtectionTestKeySystem) {
...
static void CopyDecryptResults(
    Decryptor::Status* status_copy, scoped_refptr<DecoderBuffer>* buffer_copy, Decryptor::Status status, scoped_refptr<DecoderBuffer> buffer)
{
    *status_copy = status;
    *buffer_copy = std::move(buffer);
}

cdm::Status ClearKeyCdm::Decrypt(const cdm::InputBuffer_2& encrypted_buffer, cdm::DecryptedBlock* decrypted_block)
{
    DVLOG(1) << __func__;
    ...
    DCHECK(buffer->data());
    decrypted_block->SetDecryptedBuffer(cdm_host_proxy_->Allocate(buffer->size()));
    memcpy(reinterpret_cast<void*>(decrypted_block->DecryptedBuffer()->Data()), buffer->data(), buffer->size());
...
cdm::Status ClearKeyCdm::InitializeAudioDecoder(const cdm::AudioDecoderConfig_2& audio_decoder_config)
{
...
#if defined(CLEAR_KEY_CDM_USE_FFMPEG_DECODER)
    if (!audio_decoder_) {
        audio_decoder_ = std::make_unique<FFmpegCdmAudioDecoder>(cdm_host_proxy_.get());
    }
...
cdm::Status ClearKeyCdm::InitializeVideoDecoder(const cdm::VideoDecoderConfig_2& video_decoder_config)
{
    return InitializeVideoDecoder(ToVideoDecoderConfig_3(video_decoder_config));
}

cdm::Status ClearKeyCdm::InitializeVideoDecoder(const cdm::VideoDecoderConfig_3& video_decoder_config)
{
    if (key_system_ == kExternalClearKeyDecryptOnlyKeySystem) {
        return cdm::kInitializationError;
    }
    ...
    if (!video_decoder_) {
        video_decoder_ = CreateVideoDecoder(cdm_host_proxy_.get(), video_decoder_config);
        if (!video_decoder_)
            return cdm::kInitializationError;
    }
...
cdm::Status ClearKeyCdm::DecryptAndDecodeFrame(const cdm::InputBuffer_2& encrypted_buffer, cdm::VideoFrame* decoded_frame)
{
    CdmVideoFrameAdapter adapted_frame(decoded_frame);
    return DecryptAndDecodeFrame(encrypted_buffer, &adapted_frame);
}

cdm::Status ClearKeyCdm::DecryptAndDecodeFrame(const cdm::InputBuffer_2& encrypted_buffer, cdm::VideoFrame_2* decoded_frame)
{
    DVLOG(1) << __func__;
...
cdm::Status ClearKeyCdm::DecryptAndDecodeSamples(const cdm::InputBuffer_2& encrypted_buffer, cdm::AudioFrames* audio_frames)
{
    DVLOG(1) << __func__;
    ...
    if (!last_session_id_.empty() && key_system_ == kExternalClearKeyCrashKeySystem) {
...
void ClearKeyCdm::ScheduleNextTimer()
{
    ...
    msg_stream << "Renewal from ClearKey CDM set at time " << base::Time::FromSecondsSinceUnixEpoch(cdm_host_proxy_->GetCurrentWallTime()) << ".";
...
cdm::Status ClearKeyCdm::DecryptToMediaDecoderBuffer(const cdm::InputBuffer_2& encrypted_buffer, scoped_refptr<DecoderBuffer>* decrypted_buffer)
{
    ...
    cdm_->GetCdmContext()->GetDecryptor()->Decrypt(Decryptor::kVideo, std::move(buffer), base::BindOnce(&CopyDecryptResults, &status, decrypted_buffer));
...
void ClearKeyCdm::OnPlatformChallengeResponse(const cdm::PlatformChallengeResponse& response)
{
    DVLOG(1) << __func__;
...
void ClearKeyCdm::OnQueryOutputProtectionStatus(cdm::QueryResult result, uint32_t link_mask, uint32_t output_protection_mask)
{
    DVLOG(1) << __func__ << " result:" << result << ", link_mask:" << link_mask << ", output_protection_mask:" << output_protection_mask;
    ...
    if (result == cdm::kQuerySucceeded) {
        key_status = (link_mask & cdm::kLinkTypeNetwork) ? cdm::kOutputRestricted : cdm::kUsable;
    }
    const uint8_t kDummyKeyId[] = { 'd', 'u', 'm', 'm', 'y' };
    std::vector<cdm::KeyInformation> keys_vector = { { kDummyKeyId, std::size(kDummyKeyId), key_status, 0 } };
    cdm_host_proxy_->OnSessionKeysChange(last_session_id_.data(), last_session_id_.length(), false, keys_vector.data(), keys_vector.size());
}

void ClearKeyCdm::OnStorageId(uint32_t version, const uint8_t* storage_id, uint32_t storage_id_size)
{
    ...
    DVLOG(1) << __func__ << ": storage_id (hex encoded) = " << (storage_id_size ? base::HexEncode(storage_id, storage_id_size) : "<empty>");
...
void ClearKeyCdm::OnSessionMessage(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message)
{
    DVLOG(1) << __func__ << ": size = " << message.size();

    cdm_host_proxy_->OnSessionMessage(
        session_id.data(), session_id.length(), ToCdmMessageType(message_type), reinterpret_cast<const char*>(message.data()), message.size());
}

void ClearKeyCdm::OnSessionKeysChange(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info)
{
    ...
    cdm_host_proxy_->OnSessionKeysChange(session_id.data(), session_id.length(), has_additional_usable_key, keys_vector.data(), keys_vector.size());
}

void ClearKeyCdm::OnSessionClosed(const std::string& session_id, CdmSessionClosedReason /*reason*/)
{
    cdm_host_proxy_->OnSessionClosed(session_id.data(), session_id.length());
}

void ClearKeyCdm::OnSessionExpirationUpdate(const std::string& session_id, base::Time new_expiry_time)
{
    DVLOG(1) << __func__ << ": expiry_time = " << new_expiry_time;
    cdm_host_proxy_->OnExpirationChange(session_id.data(), session_id.length(), new_expiry_time.InSecondsFSinceUnixEpoch());
}

void ClearKeyCdm::OnSessionCreated(uint32_t promise_id, const std::string& session_id)
{
    // Save the latest session ID for renewal and file IO test messages.
    last_session_id_ = session_id;

    cdm_host_proxy_->OnResolveNewSessionPromise(promise_id, session_id.data(), session_id.length());
}
...
void ClearKeyCdm::OnPromiseFailed(uint32_t promise_id, CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
{
    DVLOG(1) << __func__ << ": error = " << error_message;
    cdm_host_proxy_->OnRejectPromise(promise_id, ToCdmException(exception_code), system_code, error_message.data(), error_message.length());
}

void ClearKeyCdm::OnUnitTestComplete(bool success)
{
    std::string message = GetUnitTestResultMessage(success);
    cdm_host_proxy_->OnSessionMessage(last_session_id_.data(), last_session_id_.length(), cdm::kLicenseRequest, message.data(), message.length());
}

void ClearKeyCdm::StartFileIOTest()
{
    file_io_test_runner_ = std::make_unique<FileIOTestRunner>(base::BindRepeating(&CdmHostProxy::CreateFileIO, base::Unretained(cdm_host_proxy_.get())));
    file_io_test_runner_->RunAllTests(base::BindOnce(&ClearKeyCdm::OnFileIOTestComplete, base::Unretained(this)));
}
...
void ClearKeyCdm::StartPlatformVerificationTest()
{
    ...
    cdm_host_proxy_->SendPlatformChallenge(service_id.data(), service_id.size(), challenge.data(), challenge.size());
}
