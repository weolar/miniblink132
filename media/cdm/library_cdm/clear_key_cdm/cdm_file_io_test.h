...
using CreateFileIOCB = base::RepeatingCallback<cdm::FileIO*(cdm::FileIOClient* client)>;
...
public:
    ...
    FileIOTest(const CreateFileIOCB& create_file_io_cb, const std::string& test_name);
    ...
    void AddTestStep(StepType type, Status status, const uint8_t* data, uint32_t data_size);
    ...
    void AddResultReadEither(Status status, const uint8_t* data, uint32_t data_size, const uint8_t* data2, uint32_t data2_size);
...
private:
    struct TestStep {
        ...
        TestStep(StepType type, Status status, const uint8_t* data = nullptr, uint32_t data_size = 0, const uint8_t* data2 = nullptr, uint32_t data2_size = 0)
            : type(type)
            , status(status)
            , data(data)
            , data_size(data_size)
    ...
    void OnReadComplete(Status status, const uint8_t* data, uint32_t data_size) override;
