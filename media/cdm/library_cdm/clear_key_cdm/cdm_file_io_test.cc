...
const uint8_t kData[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
const uint32_t kDataSize = std::size(kData);

const uint8_t kBigData[] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
...
#define START_TEST_CASE(test_name)                                                                                                                             \
    do {                                                                                                                                                       \
        auto test_case = std::make_unique<FileIOTest>(create_file_io_cb_, test_name);                                                                          \
    CREATE_FILE_IO // Create FileIO for each test case.

#define ADD_TEST_STEP(type, status, data, data_size) test_case->AddTestStep(FileIOTest::type, cdm::FileIOClient::Status::status, (data), (data_size));
...
#define CREATE_FILE_IO ADD_TEST_STEP(ACTION_CREATE, kSuccess, nullptr, 0)

#define OPEN_FILE ADD_TEST_STEP(ACTION_OPEN, kSuccess, nullptr, 0)

#define EXPECT_FILE_OPENED(status) ADD_TEST_STEP(RESULT_OPEN, status, nullptr, 0)

#define READ_FILE ADD_TEST_STEP(ACTION_READ, kSuccess, nullptr, 0)

#define EXPECT_FILE_READ(status, data, data_size) ADD_TEST_STEP(RESULT_READ, status, data, data_size)

#define EXPECT_FILE_READ_EITHER(status, data, data_size, data2, data2_size)                                                                                    \
    test_case->AddResultReadEither(cdm::FileIOClient::Status::status, (data), (data_size), (data2), (data2_size));

#define WRITE_FILE(data, data_size) ADD_TEST_STEP(ACTION_WRITE, kSuccess, data, data_size)

#define EXPECT_FILE_WRITTEN(status) ADD_TEST_STEP(RESULT_WRITE, status, nullptr, 0)

#define CLOSE_FILE ADD_TEST_STEP(ACTION_CLOSE, kSuccess, nullptr, 0)
...
FileIOTestRunner::~FileIOTestRunner()
{
    ...
    FILE_IO_DVLOG(1) << "Not Finished (probably due to timeout). " << num_passed_tests_ << " passed in " << total_num_tests_ << " tests.";
}
...
void FileIOTestRunner::AddTests()
{
    ...
    START_TEST_CASE("FileNameLongThan256Characters1234567890123456789012345678901234567890123"
    ...
    CREATE_FILE_IO // Create a second FileIO without closing the first one.
        OPEN_FILE
        EXPECT_FILE_OPENED(kInUse) END_TEST_CASE

            START_TEST_CASE("CloseAfterCreation") CLOSE_FILE END_TEST_CASE

                START_TEST_CASE("CloseDuringPendingOpen") OPEN_FILE CLOSE_FILE END_TEST_CASE

                    START_TEST_CASE("CloseDuringPendingWrite") OPEN_FILE
                    EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) CLOSE_FILE END_TEST_CASE

                        START_TEST_CASE("CloseDuringPendingOverwriteWithLargerData") OPEN_FILE
                        EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kBigData, kBigDataSize) CLOSE_FILE
                            // Write() is async, so it may or may not modify the content of the file.
                            CREATE_FILE_IO OPEN_FILE
                            EXPECT_FILE_OPENED(kSuccess) READ_FILE
                            ...
                            // is either data set.
                            EXPECT_FILE_READ_EITHER(kSuccess, kData, kDataSize, kBigData, kBigDataSize) END_TEST_CASE

                                START_TEST_CASE("CloseDuringPendingOverwriteWithSmallerData") OPEN_FILE
                                EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kBigData, kBigDataSize) EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kData, kDataSize)
                                    CLOSE_FILE
                                        // Write() is async, so it may or may not modify the content of the file.
                                        CREATE_FILE_IO OPEN_FILE
                                        EXPECT_FILE_OPENED(kSuccess) READ_FILE
                                        ...
                                        // succeeds before the file is closed. So check that the contents
                                        // is either data set.
                                        EXPECT_FILE_READ_EITHER(kSuccess, kBigData, kBigDataSize, kData, kDataSize) END_TEST_CASE

                                            START_TEST_CASE("CloseDuringPendingRead") OPEN_FILE
                                            EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize) EXPECT_FILE_WRITTEN(kSuccess) READ_FILE CLOSE_FILE
                                                // Make sure the file is not modified.
                                                CREATE_FILE_IO OPEN_FILE
                                                EXPECT_FILE_OPENED(kSuccess) READ_FILE
                                                EXPECT_FILE_READ(kSuccess, kData, kDataSize) END_TEST_CASE

                                                    START_TEST_CASE("StressTest") for (int i = 0; i < 100; ++i) {
                                                        CREATE_FILE_IO OPEN_FILE EXPECT_FILE_OPENED(kSuccess) WRITE_FILE(kData, kDataSize)
                                                            EXPECT_FILE_WRITTEN(kSuccess) WRITE_FILE(kBigData, kBigDataSize) CLOSE_FILE
                                                                // Make sure the file is not modified.
                                                                CREATE_FILE_IO OPEN_FILE EXPECT_FILE_OPENED(kSuccess) READ_FILE
                                                                    // As Write() is async, it is possible that the second write above
                                                                    // succeeds before the file is closed. So check that the contents
                                                                    // is either data set.
                                                                    EXPECT_FILE_READ_EITHER(kSuccess, kData, kDataSize, kBigData, kBigDataSize) CLOSE_FILE
                                                    } END_TEST_CASE
}
...
{
    if (remaining_tests_.empty()) {
        FILE_IO_DVLOG(1) << num_passed_tests_ << " passed and " << (total_num_tests_ - num_passed_tests_) << " failed in " << total_num_tests_ << " tests.";
        bool success = (num_passed_tests_ == total_num_tests_);
        std::move(completion_cb_).Run(success);
        return;
    }

    remaining_tests_.front()->Run(base::BindOnce(&FileIOTestRunner::OnTestComplete, base::Unretained(this)));
}
...
FileIOTest::FileIOTest(const CreateFileIOCB& create_file_io_cb, const std::string& test_name)
    : create_file_io_cb_(create_file_io_cb)
...
void FileIOTest::AddTestStep(StepType type, Status status, const uint8_t* data, uint32_t data_size)
{
    test_steps_.push_back(TestStep(type, status, data, data_size));
}

void FileIOTest::AddResultReadEither(Status status, const uint8_t* data, uint32_t data_size, const uint8_t* data2, uint32_t data2_size)
{
    DCHECK_NE(data_size, data2_size);
    test_steps_.push_back(TestStep(FileIOTest::RESULT_READ, status, data, data_size, data2, data2_size));
}
...
void FileIOTest::OnReadComplete(Status status, const uint8_t* data, uint32_t data_size)
{
...
void FileIOTest::RunNextStep()
{
    ...
    while (!test_steps_.empty()) {
        ...
        cdm::FileIO* file_io = file_io_stack_.empty() ? nullptr : file_io_stack_.top();
...
void FileIOTest::OnResult(const TestStep& result)
{
    ...
    if (!CheckResult(result)) {
        LOG(WARNING) << test_name_ << " got unexpected result. type=" << result.type << ", status=" << (uint32_t)result.status
                     << ", data_size=" << result.data_size << ", received data=" << (result.data ? base::HexEncode(result.data, result.data_size) : "<null>");
        for (const auto& step : test_steps_) {
            if (IsResult(step)) {
                LOG(WARNING) << test_name_ << " expected type=" << step.type << ", status=" << (uint32_t)step.status << ", data_size=" << step.data_size;
            }
        }
        OnTestComplete(false);
        return;
