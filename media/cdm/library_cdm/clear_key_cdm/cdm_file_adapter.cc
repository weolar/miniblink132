...
void CdmFileAdapter::OnReadComplete(cdm::FileIOClient::Status status, const uint8_t* data, uint32_t data_size)
{
    std::move(read_cb_).Run(status == FileIOClient::Status::kSuccess && data_size > 0, std::vector<uint8_t>(data, data + data_size));
}
