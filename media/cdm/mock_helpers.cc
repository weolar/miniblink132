...
MockCdmAuxiliaryHelper::MockCdmAuxiliaryHelper(std::unique_ptr<CdmAllocator> allocator)
...
void MockCdmAuxiliaryHelper::EnableProtection(uint32_t desired_protection_mask, EnableProtectionCB callback)
{
    std::move(callback).Run(EnableProtectionCalled(desired_protection_mask));
}

void MockCdmAuxiliaryHelper::ChallengePlatform(const std::string& service_id, const std::string& challenge, ChallengePlatformCB callback)
{
    std::move(callback).Run(ChallengePlatformCalled(service_id, challenge), "", "", "");
}

void MockCdmAuxiliaryHelper::GetStorageId(uint32_t version, StorageIdCB callback)
{
