...
MEDIA_EXPORT bool GetKeyIdsForCommonSystemId(const std::vector<uint8_t>& pssh_boxes, KeyIdList* key_ids);
...
// 2. Only PSSH boxes are allowed in |input|. Any other boxes in |pssh_boxes|
//    will result in false being returned.
MEDIA_EXPORT bool GetPsshData(const std::vector<uint8_t>& input, const std::vector<uint8_t>& system_id, std::vector<uint8_t>* pssh_data);
