...
base::RepeatingClosure FuchsiaStreamDecryptor::GetOnNewKeyClosure()
{
    ...
    return base::BindPostTaskToCurrentDefault(base::BindRepeating(&FuchsiaStreamDecryptor::OnNewKey, weak_factory_.GetWeakPtr()));
}
...
void FuchsiaStreamDecryptor::OnStreamProcessorAllocateOutputBuffers(const fuchsia::media::StreamBufferConstraints& stream_constraints)
{
    ...
    output_buffer_collection_->Initialize(std::move(constraints), "CrFuchsiaStreamDecryptorOutput");
}
...
void FuchsiaStreamDecryptor::OnInputBuffersAcquired(std::vector<VmoBuffer> buffers, const fuchsia::sysmem2::SingleBufferSettings&)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
