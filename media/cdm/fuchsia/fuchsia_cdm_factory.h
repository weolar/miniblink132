...
public:
    ...
    FuchsiaCdmFactory(std::unique_ptr<FuchsiaCdmProvider> provider, KeySystems* key_systems);
...
private:
    void OnCdmReady(uint32_t creation_id, CdmCreatedCB cdm_created_cb, bool success, CreateCdmStatus status);

    std::unique_ptr<FuchsiaCdmProvider> cdm_provider_;
    uint32_t creation_id_ = 0;

    // Map between creation id and pending cdms
    base::flat_map<uint32_t, scoped_refptr<ContentDecryptionModule>> pending_cdms_;
