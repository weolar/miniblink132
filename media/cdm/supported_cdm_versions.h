...
{
    return kMinSupportedCdmHostVersion <= version && version <= kMaxSupportedCdmHostVersion;
}
...
constexpr bool CheckVersions(VersionCheckFunc check_func, int min_version, int max_version)
{
...
constexpr bool CheckSupportedCdmInterfaceVersions(int min_version, int max_version)
{
    return CheckVersions(IsSupportedCdmInterfaceVersion, min_version, max_version);
}
...
template <int CdmInterfaceVersion> struct CdmInterfaceTraits { };

// TODO(xhwang): CDM_9 support has been removed; consider to use a macro to
// help define CdmInterfaceTraits specializations.
template <> struct CdmInterfaceTraits<10> {
    using CdmInterface = cdm::ContentDecryptionModule_10;
    static_assert(CdmInterface::kVersion == 10, "CDM interface version mismatch");
    static_assert(IsSupportedCdmHostVersion(CdmInterface::Host::kVersion), "Host not supported");
    static_assert(CdmInterface::kIsStable || !IsCdmInterfaceVersionEnabledByDefault(CdmInterface::kVersion),
        "Experimental CDM interface should not be enabled by default");
};

template <> struct CdmInterfaceTraits<11> {
