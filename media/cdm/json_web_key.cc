...
static base::Value::Dict CreateJSONDictionary(const uint8_t* key, int key_length, const uint8_t* key_id, int key_id_length)
{
    std::string key_string, key_id_string;
    base::Base64UrlEncode(base::span<const uint8_t>(key, static_cast<size_t>(key_length)), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_string);
    base::Base64UrlEncode(base::span<const uint8_t>(key_id, static_cast<size_t>(key_id_length)), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_id_string);
...
std::string GenerateJWKSet(const uint8_t* key, int key_length, const uint8_t* key_id, int key_id_length)
{
...
std::string GenerateJWKSet(const KeyIdAndKeyPairs& keys, CdmSessionType session_type)
{
    base::Value::List list;
    for (const auto& key_pair : keys) {
        list.Append(base::Value(CreateJSONDictionary(reinterpret_cast<const uint8_t*>(key_pair.second.data()), key_pair.second.length(),
            reinterpret_cast<const uint8_t*>(key_pair.first.data()), key_pair.first.length())));
    }
...
static bool ConvertJwkToKeyPair(const base::Value::Dict& jwk, KeyIdAndKeyPair* jwk_key)
{
    const base::Value* type = jwk.Find(kKeyTypeTag);
    if (!type || *type != kKeyTypeOct) {
        DVLOG(1) << "Missing or invalid '" << kKeyTypeTag << "': " << (type ? type->DebugString() : "");
        return false;
    }
    ...
    if (!base::Base64UrlDecode(encoded_key_id->GetString(), base::Base64UrlDecodePolicy::DISALLOW_PADDING, &raw_key_id) || raw_key_id.empty()) {
        DVLOG(1) << "Invalid '" << kKeyIdTag << "' value: " << encoded_key_id;
        return false;
    }

    std::string raw_key;
    if (!base::Base64UrlDecode(encoded_key->GetString(), base::Base64UrlDecodePolicy::DISALLOW_PADDING, &raw_key) || raw_key.empty()) {
...
bool ExtractKeysFromJWKSet(const std::string& jwk_set, KeyIdAndKeyPairs* keys, CdmSessionType* session_type)
{
    ...
    if (!list_val) {
        DVLOG(1) << "Missing '" << kKeysTag << "' parameter or not a list in JWK Set";
        return false;
    }
    ...
    for (size_t i = 0; i < list_val->size(); ++i) {
        ...
        if (!jwk.is_dict()) {
            DVLOG(1) << "Unable to access '" << kKeysTag << "'[" << i << "] in JWK Set";
            return false;
        }
...
bool ExtractKeyIdsFromKeyIdsInitData(const std::string& input, KeyIdList* key_ids, std::string* error_message)
{
    ...
    for (size_t i = 0; i < list_val->size(); ++i) {
        ...
        std::string raw_key_id;
        if (!base::Base64UrlDecode(*encoded_key_id, base::Base64UrlDecodePolicy::DISALLOW_PADDING, &raw_key_id) || raw_key_id.empty()) {
            error_message->assign("'");
            error_message->append(kKeyIdsTag);
            error_message->append("'[");
            error_message->append(base::NumberToString(i));
        ...
        local_key_ids.push_back(std::vector<uint8_t>(raw_key_id.data(), raw_key_id.data() + raw_key_id.length()));
    }
...
void CreateLicenseRequest(const KeyIdList& key_ids, CdmSessionType session_type, std::vector<uint8_t>* license)
{
    ...
    for (const auto& key_id : key_ids) {
        ...
        base::Base64UrlEncode(
            std::string_view(reinterpret_cast<const char*>(key_id.data()), key_id.size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &key_id_string);
...
std::vector<uint8_t> SerializeDictionaryToVector(const base::Value::Dict& dictionary)
{
...
void CreateKeyIdsInitData(const KeyIdList& key_ids, std::vector<uint8_t>* init_data)
{
...
bool ExtractFirstKeyIdFromLicenseRequest(const std::vector<uint8_t>& license, std::vector<uint8_t>* first_key)
{
    const std::string license_as_str(reinterpret_cast<const char*>(!license.empty() ? &license[0] : NULL), license.size());
    ...
    if (!base::Base64UrlDecode(*encoded_key, base::Base64UrlDecodePolicy::DISALLOW_PADDING, &decoded_string) || decoded_string.empty()) {
