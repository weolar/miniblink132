...
bool DecryptWithPattern(
    const crypto::SymmetricKey& key, base::span<const uint8_t> iv, const EncryptionPattern& pattern, base::span<const uint8_t> input_data, uint8_t* output_data)
{
    ...
    size_t crypt_byte_block = base::strict_cast<size_t>(pattern.crypt_byte_block());
    ...
    while (blocks_processed < total_blocks) {
        ...
        size_t blocks_to_process = std::min(is_encrypted_blocks ? crypt_byte_block : skip_byte_block, total_blocks - blocks_processed);
        ...
        if (is_encrypted_blocks) {
            if (!aes_cbc_crypto.Decrypt(base::make_span(src, bytes_to_process), dest)) {
                return false;
            }
        } else {
...
scoped_refptr<DecoderBuffer> DecryptCbcsBuffer(const DecoderBuffer& input, const crypto::SymmetricKey& key)
{
    ...
    const EncryptionPattern pattern = decrypt_config->encryption_pattern().value();
    ...
    if (subsamples.empty()) {
        ...
        return DecryptWithPattern(key, base::as_byte_span(decrypt_config->iv()), pattern, base::span(input), output_data.data()) ? buffer : nullptr;
    }
    ...
    for (const auto& subsample : subsamples) {
        ...
        if (subsample.cypher_bytes) {
            DVLOG(4) << "Processing cypher_bytes: " << subsample.cypher_bytes << ", pattern(" << pattern.crypt_byte_block() << "," << pattern.skip_byte_block()
                     << ")";
            ...
            src = src_rem;
            dest = dest_rem;
            if (!DecryptWithPattern(key, base::as_bytes(base::make_span(decrypt_config->iv())), pattern, src_cypher, dest_cypher.data())) {
                return nullptr;
            }
        }
    }
