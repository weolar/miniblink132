...
struct HostFile {
    HostFile(const FilePathCharType* file_path, PlatformFile file, PlatformFile sig_file)
        : file_path(file_path)
        , file(file)
        , sig_file(sig_file)
    {
...
CDM_API bool VerifyCdmHost_0(const cdm::HostFile* host_files, uint32_t num_files);
}
