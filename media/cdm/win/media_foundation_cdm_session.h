...
public:
    MediaFoundationCdmSession(const std::string& uma_prefix, const SessionMessageCB& session_message_cb, const SessionKeysChangeCB& session_keys_change_cb,
        const SessionExpirationUpdateCB& session_expiration_update_cb);
    MediaFoundationCdmSession(const MediaFoundationCdmSession&) = delete;
    MediaFoundationCdmSession& operator=(const MediaFoundationCdmSession&) = delete;
    ~MediaFoundationCdmSession();

    // Initializes the session. All other methods should only be called after
    // Initialize() returns S_OK.
    HRESULT Initialize(IMFContentDecryptionModule* mf_cdm, CdmSessionType session_type);
    ...
    HRESULT GenerateRequest(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, SessionIdCB session_id_cb);
...
private:
    ...
    void OnSessionMessage(CdmMessageType message_type, const std::vector<uint8_t>& message);
