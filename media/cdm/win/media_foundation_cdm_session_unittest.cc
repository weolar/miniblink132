...
public:
    MediaFoundationCdmSessionTest()
        ...
        , cdm_session_(kTestUmaPrefix, base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
    ...
    void Initialize()
    {
        ...
        ASSERT_SUCCESS(cdm_session_.Initialize(mf_cdm_.Get(), CdmSessionType::kTemporary));
    }
    ...
    void GenerateRequest()
    {
...

        {
            ...
            InSequence seq;
            COM_EXPECT_CALL(mf_cdm_session_, GenerateRequest(_, _, init_data.size())).WillOnce(WithoutArgs([&] {
                mf_cdm_session_callbacks_->KeyMessage(MF_MEDIAKEYSESSION_MESSAGETYPE_LICENSE_REQUEST, license_request.data(), license_request.size(), nullptr);
                return S_OK;
            }));
            COM_EXPECT_CALL(mf_cdm_session_, GetSessionId(_)).WillOnce(DoAll(SetArgPointee<0>(session_id), Return(S_OK)));
            EXPECT_CALL(session_id_cb, Run(_)).WillOnce(Return(true));
            EXPECT_CALL(cdm_client_, OnSessionMessage(_, CdmMessageType::LICENSE_REQUEST, license_request));
        }

        EXPECT_SUCCESS(cdm_session_.GenerateRequest(EmeInitDataType::WEBM, init_data, session_id_cb.Get()));
...
{
    COM_EXPECT_CALL(mf_cdm_, CreateSession(MF_MEDIAKEYSESSION_TYPE_TEMPORARY, _, _))
        .WillOnce(DoAll(SaveComPtr<1>(&mf_cdm_session_callbacks_), SetComPointee<2>(mf_cdm_session_.Get()), Return(E_FAIL)));
    EXPECT_FAILED(cdm_session_.Initialize(mf_cdm_.Get(), CdmSessionType::kTemporary));
}
...
TEST_F(MediaFoundationCdmSessionTest, GenerateRequest_Failure)
{
    ...
    COM_EXPECT_CALL(mf_cdm_session_, GenerateRequest(_, _, init_data.size())).WillOnce(Return(E_FAIL));
    EXPECT_FAILED(cdm_session_.GenerateRequest(EmeInitDataType::WEBM, init_data, session_id_cb.Get()));
    task_environment_.RunUntilIdle();
}
...
TEST_F(MediaFoundationCdmSessionTest, GetSessionId_Empty)
{
    ...
    COM_EXPECT_CALL(mf_cdm_session_, GetSessionId(_)).WillOnce(DoAll(SetArgPointee<0>(empty_session_id), Return(S_OK)));
    EXPECT_CALL(session_id_cb, Run(IsEmpty()));
