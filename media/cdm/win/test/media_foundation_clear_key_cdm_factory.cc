...
STDMETHODIMP_(BOOL)
MediaFoundationClearKeyCdmFactory::IsTypeSupported(_In_ LPCWSTR key_system, _In_opt_ LPCWSTR content_type)
{
    DVLOG_FUNC(1) << "key_system=" << key_system;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    return CompareStringOrdinal(kMediaFoundationClearKeyKeySystemWideString, -1, key_system, -1, FALSE) == CSTR_EQUAL;
}

STDMETHODIMP
MediaFoundationClearKeyCdmFactory::CreateContentDecryptionModuleAccess(_In_ LPCWSTR key_system, _In_count_(num_configurations) IPropertyStore** configurations,
    _In_ DWORD num_configurations, _COM_Outptr_ IMFContentDecryptionModuleAccess** cdm_access)
{
    DVLOG_FUNC(1) << "key_system=" << key_system;
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    RETURN_IF_FAILED((MakeAndInitialize<MediaFoundationClearKeyCdmAccess, IMFContentDecryptionModuleAccess>(cdm_access)));
