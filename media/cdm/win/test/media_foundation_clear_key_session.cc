...
MF_MEDIAKEYSESSION_MESSAGETYPE ToMFMessageType(media::CdmMessageType message_type)
{
    switch (message_type) {
...
public:
    ...
    MediaFoundationSimpleCdmPromise(const MediaFoundationSimpleCdmPromise&) = delete;
    MediaFoundationSimpleCdmPromise& operator=(const MediaFoundationSimpleCdmPromise&) = delete;

    ~MediaFoundationSimpleCdmPromise() override
    {
        DVLOG_FUNC(1);
    }
...
public:
    MediaFoundationCdmSessionPromise(PromiseState* promise_state, SessionIdCB session_created_cb)
    {
        DVLOG_FUNC(1);
        CHECK(promise_state);
    ...
    MediaFoundationCdmSessionPromise(const MediaFoundationCdmSessionPromise&) = delete;
    MediaFoundationCdmSessionPromise& operator=(const MediaFoundationCdmSessionPromise&) = delete;

    ~MediaFoundationCdmSessionPromise() override
    {
        DVLOG_FUNC(1);
    }
...
HRESULT MediaFoundationClearKeySession::RuntimeClassInitialize(_In_ MF_MEDIAKEYSESSION_TYPE session_type,
    _In_ IMFContentDecryptionModuleSessionCallbacks* callbacks, _In_ scoped_refptr<AesDecryptor> aes_decryptor, _In_ SessionIdCreatedCB session_id_created_cb,
...
STDMETHODIMP MediaFoundationClearKeySession::Update(_In_reads_bytes_(response_size) const BYTE* response, _In_ DWORD response_size)
{
    DVLOG_FUNC(1);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    aes_decryptor_->UpdateSession(
        session_id_, std::vector<uint8_t>(response, response + response_size), std::make_unique<MediaFoundationSimpleCdmPromise>(&promise_state));
...
STDMETHODIMP MediaFoundationClearKeySession::Close()
{
    ...
    PromiseState promise_state = PromiseState::kPending;
    aes_decryptor_->CloseSession(session_id_, std::make_unique<MediaFoundationSimpleCdmPromise>(&promise_state));
    CHECK(promise_state != PromiseState::kPending);
...
STDMETHODIMP MediaFoundationClearKeySession::GetSessionId(_COM_Outptr_ LPWSTR* id)
{
    DVLOG_FUNC(1);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    RETURN_IF_FAILED(CopyCoTaskMemWideString(base::ASCIIToWide(session_id_).c_str(), id));

    return S_OK;
}

STDMETHODIMP MediaFoundationClearKeySession::GetKeyStatuses(
    _Outptr_result_buffer_(*key_statuses_size) MFMediaKeyStatus** key_statuses, _Out_ UINT* key_statuses_count)
{
    DVLOG_FUNC(1);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    *key_statuses_count = 0;
    ...
    key_status_array = static_cast<MFMediaKeyStatus*>(CoTaskMemAlloc(key_status_count * sizeof(MFMediaKeyStatus)));
    ...
    const std::vector<uint8_t> kCrashKeyId = ByteArrayFromGUID(GetGUIDFromString("crash-crashcrash"));

    for (UINT i = 0; i < key_status_count; ++i) {
        key_status_array[i].cbKeyId = keys_info_[i]->key_id.size();
        key_status_array[i].pbKeyId = static_cast<BYTE*>(CoTaskMemAlloc(keys_info_[i]->key_id.size() * sizeof(uint8_t)));
        ...
        memcpy(key_status_array[i].pbKeyId, keys_info_[i]->key_id.data(), keys_info_[i]->key_id.size());
    }
...
STDMETHODIMP MediaFoundationClearKeySession::Load(_In_ LPCWSTR session_id, _Out_ BOOL* loaded)
{
    DVLOG_FUNC(1);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
STDMETHODIMP MediaFoundationClearKeySession::GenerateRequest(
    _In_ LPCWSTR init_data_type, _In_reads_bytes_(init_data_size) const BYTE* init_data, _In_ DWORD init_data_size)
{
    ...
    if (wcscmp(init_data_type, L"cenc") == 0) {
        ...
        DLOG(ERROR) << __func__ << ": Unsupported init_data_type=" << init_data_type;
        return MF_NOT_SUPPORTED_ERR;
    }

    PromiseState promise_state = PromiseState::kPending;
    media::CdmSessionType cdm_session_type = ToCdmSessionType(session_type_);
    aes_decryptor_->CreateSessionAndGenerateRequest(cdm_session_type, eme_init_data_type, std::vector<uint8_t>(init_data, init_data + init_data_size),
        std::make_unique<MediaFoundationCdmSessionPromise>(
            &promise_state, base::BindOnce(&MediaFoundationClearKeySession::OnSessionCreated, weak_factory_.GetWeakPtr())));

    CHECK(promise_state != PromiseState::kPending);
    return promise_state == PromiseState::kResolved ? S_OK : E_FAIL;
}

STDMETHODIMP MediaFoundationClearKeySession::GetExpiration(_Out_ double* expiration)
{
    DVLOG_FUNC(1);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
STDMETHODIMP MediaFoundationClearKeySession::Remove()
{
    ...
    PromiseState promise_state = PromiseState::kPending;
    aes_decryptor_->RemoveSession(session_id_, std::make_unique<MediaFoundationSimpleCdmPromise>(&promise_state));
    CHECK(promise_state != PromiseState::kPending);
    return promise_state == PromiseState::kResolved ? S_OK : E_FAIL;
}

void MediaFoundationClearKeySession::OnSessionCreated(const std::string& session_id)
{
...
void MediaFoundationClearKeySession::OnSessionMessage(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message)
{
    ...
    callbacks_->KeyMessage(mf_message_type, message.data(), message.size(), nullptr);
}

void MediaFoundationClearKeySession::OnSessionClosed(const std::string& session_id, CdmSessionClosedReason reason)
{
    DVLOG_FUNC(1) << "session_id=" << session_id << ", reason=" << static_cast<int>(reason);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
}

void MediaFoundationClearKeySession::OnSessionKeysChange(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info)
{
