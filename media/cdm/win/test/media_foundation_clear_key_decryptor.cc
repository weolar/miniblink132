...
HRESULT MediaFoundationClearKeyDecryptor::RuntimeClassInitialize(_In_ scoped_refptr<AesDecryptor> aes_decryptor)
{
    DVLOG_FUNC(1);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetStreamLimits(
    _Out_ DWORD* input_minimum, _Out_ DWORD* input_maximum, _Out_ DWORD* output_minimum, _Out_ DWORD* output_maximum)
{
    DVLOG_FUNC(3);
    CHECK(input_minimum);
    CHECK(input_maximum);
...
STDMETHODIMP
MediaFoundationClearKeyDecryptor::GetStreamCount(_Out_ DWORD* input_streams, _Out_ DWORD* output_streams)
{
    DVLOG_FUNC(3);
    CHECK(input_streams);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetStreamIDs(
    _In_ DWORD input_ids_size, _Out_ DWORD* input_ids, _In_ DWORD output_ids_size, _Out_ DWORD* output_ids)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::MediaFoundationClearKeyDecryptor::GetInputStreamInfo(
    _In_ DWORD input_stream_id, _Out_ MFT_INPUT_STREAM_INFO* stream_info)
{
    DVLOG_FUNC(3);
    CHECK(stream_info);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetOutputStreamInfo(_In_ DWORD output_stream_id, _Out_ MFT_OUTPUT_STREAM_INFO* stream_info)
{
    DVLOG_FUNC(3);
    CHECK(stream_info);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetAttributes(_COM_Outptr_ IMFAttributes** attributes)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetInputStreamAttributes(_In_ DWORD input_stream_id, _COM_Outptr_ IMFAttributes** attributes)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetOutputStreamAttributes(_In_ DWORD output_stream_id, _COM_Outptr_ IMFAttributes** attributes)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::DeleteInputStream(_In_ DWORD stream_id)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::AddInputStreams(_In_ DWORD streams_count, _In_count_(streams_count) DWORD* stream_ids)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetInputAvailableType(_In_ DWORD input_stream_index, _In_ DWORD type_index, _COM_Outptr_ IMFMediaType** type)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetOutputAvailableType(_In_ DWORD output_stream_index, _In_ DWORD type_index, _COM_Outptr_ IMFMediaType** type)
{
    DVLOG_FUNC(1);
    base::AutoLock lock(lock_);
    ...
    ComPtr<IMFMediaType> media_type;
    RETURN_IF_FAILED(MFCreateMediaType(&media_type));
    RETURN_IF_FAILED(available_output_media_type_->CopyAllItems(media_type.Get()));
    *type = media_type.Detach();

    return S_OK;
}

STDMETHODIMP MediaFoundationClearKeyDecryptor::SetInputType(_In_ DWORD input_stream_index, _In_ IMFMediaType* type, _In_ DWORD flags)
{
    DVLOG_FUNC(1) << "input_stream_index=" << input_stream_index << ", flags=" << flags;
    base::AutoLock lock(lock_);
    ...
    if (MFMediaType_Protected == guid_major_type) {
        ...
        RETURN_IF_FAILED(MFUnwrapMediaType(input_media_type_.Get(), &unwrapped_media_type));
    } else {
    ...
    if (guid_major_type == MFMediaType_Audio) {
        ...
        RETURN_IF_FAILED(unwrapped_media_type->GetGUID(MF_MT_SUBTYPE, &guid_sub_type));
    }
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::SetOutputType(_In_ DWORD output_stream_index, _In_ IMFMediaType* type, _In_ DWORD flags)
{
    DVLOG_FUNC(1) << "output_stream_index=" << output_stream_index << ", flags=" << flags;
    base::AutoLock lock(lock_);
    ...
    RETURN_IF_FAILED(type->IsEqual(available_output_media_type_.Get(), &is_equal_flags));
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetInputCurrentType(_In_ DWORD input_stream_index, _COM_Outptr_ IMFMediaType** type)
{
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetOutputCurrentType(_In_ DWORD output_stream_index, _COM_Outptr_ IMFMediaType** type)
{
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetInputStatus(_In_ DWORD input_stream_index, _Out_ DWORD* flags)
{
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetOutputStatus(_Out_ DWORD* flags)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::SetOutputBounds(_In_ LONGLONG lower_bound, _In_ LONGLONG upper_bound)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::ProcessEvent(_In_ DWORD input_stream_id, _In_ IMFMediaEvent* event)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::ProcessMessage(_In_ MFT_MESSAGE_TYPE message, _In_ ULONG_PTR param)
{
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::ProcessInput(_In_ DWORD input_stream_index, _In_ IMFSample* sample, _In_ DWORD flags)
{
    DVLOG_FUNC(1) << "input_stream_index=" << input_stream_index << ", flags=" << flags;
    RETURN_IF_FAILED(GetShutdownStatus());
...
STDMETHODIMP MediaFoundationClearKeyDecryptor::ProcessOutput(
    _In_ DWORD flags, _In_ DWORD output_samples_count, _Inout_count_(output_samples_count) MFT_OUTPUT_DATA_BUFFER* output_samples, _Out_ DWORD* status)
{
    DVLOG_FUNC(1) << "flags=" << flags << ", output_samples_count=" << output_samples_count;
    RETURN_IF_FAILED(GetShutdownStatus());
    ...
    *status = 0;
    ...
    DVLOG_FUNC(3) << "output_samples[0].dwStreamID=" << output_samples[0].dwStreamID << ", dwStatus=" << output_samples[0].dwStatus;
    ...
    RETURN_IF_FAILED(GenerateDecoderBufferFromSample(sample_.Detach(), &key_id_guid, &encrypted_buffer));
    DVLOG_FUNC(3) << "encrypted_buffer=" + encrypted_buffer->AsHumanReadableString(true);
    ...
    Decryptor::StreamType stream_type = stream_type_ == StreamType::kVideo ? Decryptor::kVideo : Decryptor::kAudio;
    scoped_refptr<DecoderBuffer> decrypted_buffer;
    bool is_decrypt_completed = false;
    aes_decryptor_->Decrypt(stream_type, encrypted_buffer,
        base::BindOnce(
            [](Decryptor::Status* status_copy, scoped_refptr<DecoderBuffer>* buffer_copy, bool* is_decrypt_completed, Decryptor::Status status,
    ...
    RETURN_IF_FAILED(GenerateSampleFromDecoderBuffer(decrypted_buffer.get(), &sample_decrypted, &last_key_id, base::NullCallback()));
    DVLOG_FUNC(3) << "decrypted_buffer=" + decrypted_buffer->AsHumanReadableString(true);
...
// IMFShutdown
STDMETHODIMP MediaFoundationClearKeyDecryptor::GetShutdownStatus(_Out_ MFSHUTDOWN_STATUS* status)
{
    DVLOG_FUNC(1);
...
HRESULT MediaFoundationClearKeyDecryptor::GenerateDecoderBufferFromSample(IMFSample* mf_sample, GUID* key_id, scoped_refptr<DecoderBuffer>* buffer_out)
{
    DVLOG_FUNC(3);
    CHECK(mf_sample);
    CHECK(key_id);
    ...
    RETURN_IF_FAILED(CreateDecryptConfigFromSample(mf_sample, *key_id, &decrypt_config));
