...
MediaFoundationClearKeyContentEnabler::MediaFoundationClearKeyContentEnabler() = default;
...
STDMETHODIMP MediaFoundationClearKeyContentEnabler::GetEnableData(_Outptr_result_bytebuffer_(*data_size) BYTE** data, _Out_ DWORD* data_size)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyContentEnabler::GetEnableURL(
    _Out_writes_bytes_(*url_size) LPWSTR* url, _Out_ DWORD* url_size, _Inout_ MF_URL_TRUST_STATUS* trust_status)
{
    DVLOG_FUNC(3);
...
STDMETHODIMP MediaFoundationClearKeyContentEnabler::IsAutomaticSupported(_Out_ BOOL* automatic)
{
    if (!automatic) {
        return E_INVALIDARG;
    }
...
STDMETHODIMP MediaFoundationClearKeyContentEnabler::GetEnableType(_Out_ GUID* type)
{
    DVLOG_FUNC(1);
