...
bool MediaFoundationCdmModule::Initialize(const base::FilePath& cdm_path)
{
    ...
    CHECK(!initialized_) << "MediaFoundationCdmModule can only be initialized once!";
    ...
    if (!cdm_path_.empty()) {
        ...
        if (!library_.is_valid()) {
            LOG(ERROR) << __func__ << ": Failed to load CDM at " << cdm_path_.value() << " (Error: " << library_.GetError()->ToString() << ")";
            ReportLoadResult(kUmaPrefix, base::PathExists(cdm_path) ? CdmLoadResult::kLoadFailed : CdmLoadResult::kFileMissing);
...
HRESULT MediaFoundationCdmModule::GetCdmFactory(const std::string& key_system, Microsoft::WRL::ComPtr<IMFContentDecryptionModuleFactory>& cdm_factory)
{
    if (!initialized_) {
    ...
    if (!cdm_factory_) {
        ...
        if (FAILED(hr)) {
            base::UmaHistogramSparse(std::string(kUmaPrefix) + "ActivateCdmFactoryResult", hr);
            return hr;
        }
    }
...
HRESULT MediaFoundationCdmModule::ActivateCdmFactory()
{
    ...
    if (cdm_path_.empty()) {
        ...
        RETURN_IF_FAILED(class_factory->CreateContentDecryptionModuleFactory(key_system_str.c_str(), IID_PPV_ARGS(&cdm_factory_)));
        return S_OK;
    }
    ...
    using GetActivationFactoryFunc = HRESULT(WINAPI*)(_In_ HSTRING activatible_class_id, _COM_Outptr_ IActivationFactory * *factory);
    const char kDllGetActivationFactory[] = "DllGetActivationFactory";
    auto get_activation_factory_func = reinterpret_cast<GetActivationFactoryFunc>(library_.GetFunctionPointer(kDllGetActivationFactory));
    ...
    auto class_name = base::win::ScopedHString::Create(std::string_view(key_system_ + ".ContentDecryptionModuleFactory"));
    ComPtr<IActivationFactory> activation_factory;
    RETURN_IF_FAILED(get_activation_factory_func(class_name.get(), &activation_factory));
