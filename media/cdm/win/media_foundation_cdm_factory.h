...
public:
    ...
    MediaFoundationCdmFactory& operator=(const MediaFoundationCdmFactory&) = delete;
    ...
    using CreateCdmFactoryCB = base::RepeatingCallback<HRESULT(Microsoft::WRL::ComPtr<IMFContentDecryptionModuleFactory>& factory)>;
    void SetCreateCdmFactoryCallbackForTesting(const std::string& key_system, CreateCdmFactoryCB create_cdm_factory_cb);
...
private:
    ...
    void OnCdmOriginIdObtained(const CdmConfig& cdm_config, const SessionMessageCB& session_message_cb, const SessionClosedCB& session_closed_cb,
    ...
    HRESULT GetCdmFactory(const std::string& key_system, Microsoft::WRL::ComPtr<IMFContentDecryptionModuleFactory>& cdm_factory);

    void IsTypeSupported(const std::string& key_system, const std::string& content_type, IsTypeSupportedResultCB is_type_supported_result_cb);
    ...
    void CreateMfCdm(const CdmConfig& cdm_config, const base::UnguessableToken& cdm_origin_id, const std::optional<std::vector<uint8_t>>& cdm_client_token,
        const base::FilePath& cdm_store_path_root, HRESULT& hresult, Microsoft::WRL::ComPtr<IMFContentDecryptionModule>& mf_cdm);
