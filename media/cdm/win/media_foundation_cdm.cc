...
HRESULT CreatePolicySetEvent(ComPtr<IMFMediaEvent>& policy_set_event)
{
    ...
    var_to_set->ulVal = 0;
    RETURN_IF_FAILED(MFCreateMediaEvent(MEPolicySet, GUID_NULL, S_OK, policy_set_prop.ptr(), &policy_set_event));
    return S_OK;
}
...
HRESULT RefreshDecryptor(IMFTransform* decryptor, const GUID& protection_system_id, const GUID& last_key_id)
{
    ...
    RETURN_IF_FAILED(decryptor->ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0));
    ...
    RETURN_IF_FAILED(decryptor->ProcessMessage(MFT_MESSAGE_NOTIFY_EVENT, reinterpret_cast<ULONG_PTR>(policy_set_event.Get())));

    // Prepare the MEContentProtectionMetadata event.
    ComPtr<IMFMediaEvent> key_rotation_event;
    RETURN_IF_FAILED(MFCreateMediaEvent(MEContentProtectionMetadata, GUID_NULL, S_OK, nullptr, &key_rotation_event));
    ...
    RETURN_IF_FAILED(key_rotation_event->SetBlob(MF_EVENT_STREAM_METADATA_SYSTEMID, reinterpret_cast<const uint8_t*>(&protection_system_id), sizeof(GUID)));

    std::vector<uint8_t> last_key_id_byte_array = ByteArrayFromGUID(last_key_id);
    RETURN_IF_FAILED(key_rotation_event->SetBlob(MF_EVENT_STREAM_METADATA_CONTENT_KEYIDS, last_key_id_byte_array.data(), last_key_id_byte_array.size()));
    ...
    RETURN_IF_FAILED(decryptor->ProcessEvent(/*dwInputStreamID=*/0, key_rotation_event.Get()));
    return S_OK;
}
...
public:
    CdmProxyImpl(ComPtr<IMFContentDecryptionModule> mf_cdm, base::RepeatingClosure hardware_context_reset_cb, MediaFoundationCdm::CdmEventCB cdm_event_cb)
        : mf_cdm_(mf_cdm)
    ...
    HRESULT GetPMPServer(REFIID riid, LPVOID* object_result) override
    {
        ...
        RETURN_IF_FAILED(cdm_services->GetService(MF_CONTENTDECRYPTIONMODULE_SERVICE, riid, object_result));
        return S_OK;
    }

    HRESULT GetInputTrustAuthority(uint32_t stream_id, uint32_t /*stream_count*/, const uint8_t* content_init_data, uint32_t content_init_data_size,
        REFIID riid, IUnknown** object_out) override
    {
        DVLOG_FUNC(1);
        ...
        if (!trusted_input_) {
            RETURN_IF_FAILED(mf_cdm_->CreateTrustedInput(content_init_data, content_init_data_size, &trusted_input_));
        }

        // GetInputTrustAuthority takes IUnknown* as the output. Using other COM
        // interface will have a v-table mismatch issue.
        ComPtr<IUnknown> unknown;
        RETURN_IF_FAILED(trusted_input_->GetInputTrustAuthority(stream_id, riid, &unknown));
    ...
    HRESULT RefreshTrustedInput() override
    {
        ...
        for (const auto& entry : input_trust_authorities_) {
            ...
            RETURN_IF_FAILED(input_trust_authority->GetDecrypter(IID_PPV_ARGS(&decryptor)));
            GUID protection_system_id;
            RETURN_IF_FAILED(GetProtectionSystemId(&protection_system_id));
            RETURN_IF_FAILED(RefreshDecryptor(decryptor.Get(), protection_system_id, last_key_id));
        }
...
private:
    ...
    HRESULT GetProtectionSystemId(GUID* protection_system_id)
    {
        ...
        DWORD count = 0;
        RETURN_IF_FAILED(mf_cdm_->GetProtectionSystemIds(&protection_system_ids, &count));
        if (count == 0)
            return kErrorZeroProtectionSystemId;

        *protection_system_id = *protection_system_ids;
        DVLOG(2) << __func__ << " protection_system_id=" << base::win::WStringFromGUID(*protection_system_id);
...
MediaFoundationCdm::MediaFoundationCdm(const std::string& uma_prefix, const CreateMFCdmCB& create_mf_cdm_cb, const IsTypeSupportedCB& is_type_supported_cb,
    const StoreClientTokenCB& store_client_token_cb, const CdmEventCB& cdm_event_cb, const SessionMessageCB& session_message_cb,
    const SessionClosedCB& session_closed_cb, const SessionKeysChangeCB& session_keys_change_cb, const SessionExpirationUpdateCB& session_expiration_update_cb)
...
void MediaFoundationCdm::SetServerCertificate(const std::vector<uint8_t>& certificate, std::unique_ptr<SimpleCdmPromise> promise)
{
    DVLOG_FUNC(1);
    ...
    auto hr = mf_cdm_->SetServerCertificate(certificate.data(), certificate.size());
...
void MediaFoundationCdm::GetStatusForPolicy(HdcpVersion min_hdcp_version, std::unique_ptr<KeyStatusCdmPromise> promise)
{
    if (!mf_cdm_) {
        promise->reject(Exception::INVALID_STATE_ERROR, 0, "CDM Unavailable");
        return;
    ...
    // by MFCDM.
    const std::string content_type = base::StringPrintf("video/mp4;codecs=\"avc1\";features=\"hdcp=%d\"", GetHdcpValue(min_hdcp_version));

    is_type_supported_cb_.Run(content_type, base::BindOnce(&MediaFoundationCdm::OnIsTypeSupportedResult, weak_factory_.GetWeakPtr(), std::move(promise)));
}
    ...
    CdmSessionType session_type, EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, std::unique_ptr<NewSessionCdmPromise> promise)
{
    ...
    auto session = std::make_unique<MediaFoundationCdmSession>(uma_prefix_, session_message_cb_, session_keys_change_cb_, session_expiration_update_cb_);
    ...
    if (hr == DRM_E_TEE_INVALID_HWDRM_STATE) {
        ...
        session_closed_cb_.Run(dummy_session_id, CdmSessionClosedReason::kHardwareContextReset);
        OnHardwareContextReset();
        return;
    }

    if (FAILED(hr)) {
        promise->reject(Exception::INVALID_STATE_ERROR, 0, "Failed to create session");
        return;
    }
    ...
    auto session_id_cb = base::BindOnce(&MediaFoundationCdm::OnSessionId, base::Unretained(this), session_token, std::move(promise));

    hr = session->GenerateRequest(init_data_type, init_data, std::move(session_id_cb));
    ...
    if (FAILED(hr)) {
        raw_promise->reject(Exception::INVALID_STATE_ERROR, 0, "Generate Request failed");
        return;
    }
...
void MediaFoundationCdm::LoadSession(CdmSessionType session_type, const std::string& session_id, std::unique_ptr<NewSessionCdmPromise> promise)
{
    DVLOG_FUNC(1);
...
void MediaFoundationCdm::UpdateSession(const std::string& session_id, const std::vector<uint8_t>& response, std::unique_ptr<SimpleCdmPromise> promise)
{
    DVLOG_FUNC(1);
...
void MediaFoundationCdm::CloseSession(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise)
{
    DVLOG_FUNC(1);
    ...
    CloseSessionInternal(session_id, CdmSessionClosedReason::kClose, std::move(promise));
}

void MediaFoundationCdm::RemoveSession(const std::string& session_id, std::unique_ptr<SimpleCdmPromise> promise)
{
    DVLOG_FUNC(1);
...
scoped_refptr<MediaFoundationCdmProxy> MediaFoundationCdm::GetMediaFoundationCdmProxy()
{
    DVLOG_FUNC(1);
    ...
    if (!cdm_proxy_) {
        cdm_proxy_ = base::MakeRefCounted<CdmProxyImpl>(mf_cdm_, base::BindRepeating(&MediaFoundationCdm::OnHardwareContextReset, weak_factory_.GetWeakPtr()),
            base::BindRepeating(&MediaFoundationCdm::OnCdmEvent, weak_factory_.GetWeakPtr()));
    }

    return cdm_proxy_;
}

bool MediaFoundationCdm::OnSessionId(int session_token, std::unique_ptr<NewSessionCdmPromise> promise, const std::string& session_id)
{
    DVLOG_FUNC(1) << "session_token=" << session_token << ", session_id=" << session_id;
    ...
    if (session_id.empty() || sessions_.count(session_id)) {
        promise->reject(Exception::INVALID_STATE_ERROR, 0, "Empty or duplicate session ID");
        return false;
    }
...
MediaFoundationCdmSession* MediaFoundationCdm::GetSession(const std::string& session_id)
{
...
void MediaFoundationCdm::CloseSessionInternal(const std::string& session_id, CdmSessionClosedReason reason, std::unique_ptr<SimpleCdmPromise> promise)
{
    DVLOG_FUNC(1);
...
void MediaFoundationCdm::OnHardwareContextReset()
{
    ...
    for (const auto& session_id : session_ids) {
        CloseSessionInternal(session_id, CdmSessionClosedReason::kHardwareContextReset, std::make_unique<DoNothingCdmPromise<>>());
    }
...
void MediaFoundationCdm::OnIsTypeSupportedResult(std::unique_ptr<KeyStatusCdmPromise> promise, bool is_supported)
{
    if (is_supported) {
        promise->resolve(CdmKeyInformation::KeyStatus::USABLE);
    } else {
...
void MediaFoundationCdm::StoreClientTokenIfNeeded()
{
    ...
    HRESULT hr = attributes->GetAllocatedBlob(EME_CONTENTDECRYPTIONMODULE_CLIENT_TOKEN.fmtid, &client_token, &client_token_size);
    if (FAILED(hr)) {
    ...
    client_token_vector.assign(client_token.get(), client_token.get() + client_token_size);
