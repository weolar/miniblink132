...
protected:
    void ExtractJWKKeysAndExpect(const std::string& jwk, bool expected_result, size_t expected_number_of_keys)
    {
        DCHECK(!jwk.empty());
        KeyIdAndKeyPairs keys;
        CdmSessionType session_type;
        EXPECT_EQ(expected_result, ExtractKeysFromJWKSet(jwk, &keys, &session_type));
        EXPECT_EQ(expected_number_of_keys, keys.size());
    }

    void ExtractSessionTypeAndExpect(const std::string& jwk, bool expected_result, CdmSessionType expected_type)
    {
        DCHECK(!jwk.empty());
        KeyIdAndKeyPairs keys;
        CdmSessionType session_type;
    ...
    void CreateLicenseAndExpect(const uint8_t* key_id, int key_id_length, CdmSessionType session_type, const std::string& expected_result)
    {
    ...
    void ExtractKeyFromLicenseAndExpect(const std::string& license, bool expected_result, const uint8_t* expected_key, int expected_key_length)
    {
        std::vector<uint8_t> license_vector(license.begin(), license.end());
        std::vector<uint8_t> key;
        EXPECT_EQ(expected_result, ExtractFirstKeyIdFromLicenseRequest(license_vector, &key));
        if (expected_result)
            VerifyKeyId(key, expected_key, expected_key_length);
    }

    void VerifyKeyId(std::vector<uint8_t> key, const uint8_t* expected_key, int expected_key_length)
    {
        std::vector<uint8_t> key_result(expected_key, expected_key + expected_key_length);
        EXPECT_EQ(key_result, key);
    }

    KeyIdAndKeyPair MakeKeyIdAndKeyPair(const uint8_t* key, int key_length, const uint8_t* key_id, int key_id_length)
    {
        return std::make_pair(std::string(key_id, key_id + key_id_length), std::string(key, key + key_length));
    }
};
...
TEST_F(JSONWebKeyTest, GenerateJWKSet)
{
    ...
    const uint8_t data3[] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10 };

    EXPECT_EQ("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"}]}", GenerateJWKSet(data1, std::size(data1), data1, std::size(data1)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQIDBA\",\"kid\":\"AQIDBA\",\"kty\":\"oct\"}]}", GenerateJWKSet(data2, std::size(data2), data2, std::size(data2)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQIDBA\",\"kty\":\"oct\"}]}", GenerateJWKSet(data1, std::size(data1), data2, std::size(data2)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQIDBA\",\"kid\":\"AQI\",\"kty\":\"oct\"}]}", GenerateJWKSet(data2, std::size(data2), data1, std::size(data1)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQIDBAUGBwgJCgsMDQ4PEA\",\"kid\":"
              "\"AQIDBAUGBwgJCgsMDQ4PEA\",\"kty\":\"oct\"}]}",
        GenerateJWKSet(data3, std::size(data3), data3, std::size(data3)));

    KeyIdAndKeyPairs keys;
    keys.push_back(MakeKeyIdAndKeyPair(data1, std::size(data1), data1, std::size(data1)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"}],\"type\":"
              "\"temporary\"}",
        GenerateJWKSet(keys, CdmSessionType::kTemporary));
    keys.push_back(MakeKeyIdAndKeyPair(data2, std::size(data2), data2, std::size(data2)));
    EXPECT_EQ("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"},{\"k\":"
...
TEST_F(JSONWebKeyTest, ExtractInvalidJWKKeys)
{
    ...
    ExtractJWKKeysAndExpect("This is not ASCII due to \xff\xfe\xfd in it.", false, 0);
...
{
    ExtractSessionTypeAndExpect("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"}]}", true, CdmSessionType::kTemporary);
    ExtractSessionTypeAndExpect("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"}],\"type\":"
                                "\"temporary\"}",
    ...
    ExtractSessionTypeAndExpect("{\"keys\":[{\"k\":\"AQI\",\"kid\":\"AQI\",\"kty\":\"oct\"}],\"type\":3}", false, CdmSessionType::kTemporary);
}
...
TEST_F(JSONWebKeyTest, CreateLicense)
{
    ...
    CreateLicenseAndExpect(data1, std::size(data1), CdmSessionType::kTemporary, "{\"kids\":[\"AQI\"],\"type\":\"temporary\"}");
    CreateLicenseAndExpect(data1, std::size(data1), CdmSessionType::kPersistentLicense, "{\"kids\":[\"AQI\"],\"type\":\"persistent-license\"}");
    CreateLicenseAndExpect(data2, std::size(data2), CdmSessionType::kTemporary, "{\"kids\":[\"AQIDBA\"],\"type\":\"temporary\"}");
    CreateLicenseAndExpect(data3, std::size(data3), CdmSessionType::kPersistentLicense,
...
TEST_F(JSONWebKeyTest, ExtractLicense)
{
    ...
    ExtractKeyFromLicenseAndExpect("{\"kids\":[\"AQI\"],\"type\":\"temporary\"}", true, data1, std::size(data1));
    ExtractKeyFromLicenseAndExpect("{\"kids\":[\"AQIDBA\"],\"type\":\"temporary\"}", true, data2, std::size(data2));
    ExtractKeyFromLicenseAndExpect("{\"kids\":[\"AQIDBAUGBwgJCgsMDQ4PEA\"],\"type\":\"persistent\"}", true, data3, std::size(data3));
...
TEST_F(JSONWebKeyTest, Base64UrlEncoding)
{
    ...
    std::string encoded_text = base::Base64Encode(std::string(reinterpret_cast<const char*>(&data1[0]), std::size(data1)));
    ...
    CreateLicenseAndExpect(data1, std::size(data1), CdmSessionType::kTemporary, "{\"kids\":[\"-_37_fv9-w\"],\"type\":\"temporary\"}");

    ExtractKeyFromLicenseAndExpect("{\"kids\":[\"-_37_fv9-w\"],\"type\":\"temporary\"}", true, data1, std::size(data1));
}
...
TEST_F(JSONWebKeyTest, MultipleKeys)
{
    ...
    std::string s(result.begin(), result.end());
    EXPECT_EQ("{\"kids\":[\"AQI\",\"AQIDBA\",\"AQIDBAUGBwgJCgsMDQ4PEA\"],\"type\":"
              "\"temporary\"}",
        s);
}
...
TEST_F(JSONWebKeyTest, ExtractKeyIds)
{
    ...
    EXPECT_TRUE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\":[\"AQI\"]}", &key_ids, &error_message));
    EXPECT_EQ(1u, key_ids.size());
    EXPECT_EQ(0u, error_message.length());
    VerifyKeyId(key_ids[0], data1, std::size(data1));

    EXPECT_TRUE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\":[\"AQI\",\"AQIDBA\",\"AQIDBAUGBwgJCgsMDQ4PEA\"]}", &key_ids, &error_message));
    ...
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("This is not ASCII due to \xff\xfe\x0a in it.", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "Non ASCII: This is not ASCII due to \\u00FF\\u00FE\\n in it.");

    // Expect failure when not JSON or not a dictionary.
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("This is invalid.", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    ...
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("This is a very long string that is longer than 64 characters and is "
                                                 "invalid.",
    ...
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("{\"keys\":[\"AQI\"]}", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "Missing 'kids' parameter or not a list");

    // Expect failure when invalid key_ids specified.
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\":[1]}", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "'kids'[0] is not string.");
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\": {\"id\":\"AQI\" }}", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "Missing 'kids' parameter or not a list");

    // Expect failure when non-base64 key_ids specified.
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\":[\"AQI+\"]}", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "'kids'[0] is not valid base64url encoded. Value: AQI+");
    EXPECT_FALSE(ExtractKeyIdsFromKeyIdsInitData("{\"kids\":[\"AQI\",\"AQI/\"]}", &key_ids, &error_message));
    EXPECT_EQ(3u, key_ids.size()); // |key_ids| should be unchanged.
    EXPECT_EQ(error_message, "'kids'[1] is not valid base64url encoded. Value: AQI/");
}
...
TEST_F(JSONWebKeyTest, CreateInitData)
{
    ...
    EXPECT_EQ(result3, "{\"kids\":[\"AQI\",\"AQIDBA\",\"AQIDBAUGBwgJCgsMDQ4PEA\"]}");
}
