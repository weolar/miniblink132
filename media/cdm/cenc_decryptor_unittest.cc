...
const uint8_t kKey[] = { 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13 };
...
const uint8_t kOneBlock[] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p' };
...
public:
    CencDecryptorTest()
        : key_(crypto::SymmetricKey::Import(crypto::SymmetricKey::AES, std::string(kKey, kKey + std::size(kKey))))
        , iv_(kIv, kIv + std::size(kIv))
        , one_block_(kOneBlock, kOneBlock + std::size(kOneBlock))
        , partial_block_(kPartialBlock, kPartialBlock + std::size(kPartialBlock))
    {
    }
    ...
    scoped_refptr<DecoderBuffer> CreateEncryptedBuffer(
        const std::vector<uint8_t>& data, const std::string& iv, const std::vector<SubsampleEntry>& subsample_entries)
    {
        ...
        encrypted_buffer->set_decrypt_config(DecryptConfig::CreateCencConfig("key_id", iv, subsample_entries));
    ...
    std::vector<uint8_t> DecryptWithKey(scoped_refptr<DecoderBuffer> encrypted, const crypto::SymmetricKey& key)
    {
        ...
        if (decrypted.get()) {
            ...
            decrypted_data.assign(decrypted->data(), decrypted->data() + decrypted->size());
        }
...
TEST_F(CencDecryptorTest, OneBlock)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples);
...
TEST_F(CencDecryptorTest, InvalidIv)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, invalid_iv, subsamples);
...
TEST_F(CencDecryptorTest, MultipleSubsamples)
{
    ...
    std::vector<SubsampleEntry> subsamples
        = { { 0, static_cast<uint32_t>(one_block_.size()) }, { 0, static_cast<uint32_t>(one_block_.size()) }, { 0, static_cast<uint32_t>(one_block_.size()) } };
...
TEST_F(CencDecryptorTest, MultipleSubsamplesWithClearBytes)
{
    ...
    auto encrypted_block = Encrypt(Combine({ partial_block_, one_block_ }), *key_, iv_);
    std::vector<uint8_t> encrypted_partial_block(encrypted_block.begin(), encrypted_block.begin() + partial_block_.size());
    EXPECT_EQ(encrypted_partial_block.size(), partial_block_.size());
    std::vector<uint8_t> encrypted_one_block(encrypted_block.begin() + partial_block_.size(), encrypted_block.end());
    EXPECT_EQ(encrypted_one_block.size(), one_block_.size());

    auto input_data = Combine({ one_block_, encrypted_partial_block, partial_block_, encrypted_one_block, partial_block_ });
    auto expected_result = Combine({ one_block_, partial_block_, partial_block_, one_block_, partial_block_ });
    std::vector<SubsampleEntry> subsamples = { { static_cast<uint32_t>(one_block_.size()), static_cast<uint32_t>(partial_block_.size()) },
        { static_cast<uint32_t>(partial_block_.size()), static_cast<uint32_t>(one_block_.size()) }, { static_cast<uint32_t>(partial_block_.size()), 0 } };
