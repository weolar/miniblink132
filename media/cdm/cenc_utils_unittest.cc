...
const uint8_t kKey1Data[] = { 0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x03 };
...
const uint8_t kCommonSystemSystemId[] = { 0x10, 0x77, 0xEF, 0xEC, 0xC0, 0xB2, 0x4D, 0x02, 0xAC, 0xE3, 0x3C, 0x1E, 0x52, 0xE2, 0xFB, 0x4B };
...
public:
    CencUtilsTest()
        ...
        , common_system_system_id_(kCommonSystemSystemId, kCommonSystemSystemId + std::size(kCommonSystemSystemId))
    {
    }

protected:
    // Initialize the start of the 'pssh' box (up to key_count)
    void InitializePSSHBox(std::vector<uint8_t>* box, uint8_t size, uint8_t version)
    {
        DCHECK(box->size() == 0);
        ...
        box->insert(box->end(), common_system_system_id_.begin(), common_system_system_id_.end());
    }
    ...
    std::vector<uint8_t> MakePSSHBox(uint8_t version, const std::vector<uint8_t>& key1)
    {
        DCHECK(version > 0);
        DCHECK(key1.size() == 16);
    ...
    std::vector<uint8_t> MakePSSHBox(uint8_t version, const std::vector<uint8_t>& key1, const std::vector<uint8_t>& key2)
    {
        DCHECK(version > 0);
        DCHECK(key1.size() == 16);
        DCHECK(key2.size() == 16);
    ...
    void AppendData(std::vector<uint8_t>& pssh_box, const std::vector<uint8_t>& data)
    {
...
TEST_F(CencUtilsTest, LongSize)
{
    ...
    KeyIdList key_ids;
    EXPECT_TRUE(ValidatePsshInput(std::vector<uint8_t>(data, data + std::size(data))));
    EXPECT_TRUE(GetKeyIdsForCommonSystemId(std::vector<uint8_t>(data, data + std::size(data)), &key_ids));
    EXPECT_EQ(2u, key_ids.size());
}
...
TEST_F(CencUtilsTest, HugeSize)
{
    ...
    EXPECT_FALSE(ValidatePsshInput(std::vector<uint8_t>(data, data + std::size(data))));
    EXPECT_FALSE(GetKeyIdsForCommonSystemId(std::vector<uint8_t>(data, data + std::size(data)), &key_ids));
}
...
{
    std::vector<uint8_t> unknown_system_id(kKey1Data, kKey1Data + std::size(kKey1Data));
    std::vector<uint8_t> pssh_data;
...
TEST_F(CencUtilsTest, NonPsshData)
{
    ...
    const uint8_t data[] = { 0x00, 0x00, 0x00, 0x08, // size = 8
        'p', 's', 's', 'g' };
