...
public:
    ...
    using QueryStatusCB = base::OnceCallback<void(bool success, uint32_t link_mask, uint32_t protection_mask)>;
    ...
    //   call QueryStatus().
    virtual void EnableProtection(uint32_t desired_protection_mask, EnableProtectionCB callback) = 0;
};
