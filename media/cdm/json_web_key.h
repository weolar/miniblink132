...
MEDIA_EXPORT std::string GenerateJWKSet(const uint8_t* key, int key_length, const uint8_t* key_id, int key_id_length);

// Converts a set of |key|, |key_id| pairs to a JSON Web Key Set.
MEDIA_EXPORT std::string GenerateJWKSet(const KeyIdAndKeyPairs& keys, CdmSessionType session_type);
...
MEDIA_EXPORT bool ExtractKeysFromJWKSet(const std::string& jwk_set, KeyIdAndKeyPairs* keys, CdmSessionType* session_type);
...
MEDIA_EXPORT bool ExtractKeyIdsFromKeyIdsInitData(const std::string& input, KeyIdList* key_ids, std::string* error_message);
...
MEDIA_EXPORT void CreateLicenseRequest(const KeyIdList& key_ids, CdmSessionType session_type, std::vector<uint8_t>* license);
...
MEDIA_EXPORT void CreateKeyIdsInitData(const KeyIdList& key_ids, std::vector<uint8_t>* key_ids_init_data);

MEDIA_EXPORT std::vector<uint8_t> CreateLicenseReleaseMessage(const KeyIdList& key_ids);
...
MEDIA_EXPORT bool ExtractFirstKeyIdFromLicenseRequest(const std::vector<uint8_t>& license, std::vector<uint8_t>* first_key);
