...
TEST(CdmPathsTest, Prefix)
{
    ...
    EXPECT_TRUE(base::StartsWith(path.MaybeAsASCII(), kPrefix, base::CompareCase::SENSITIVE));
}
...
TEST(CdmPathsTest, Expected)
{
    ...
    std::string expected_path = base::StringPrintf("SomeCdm/_platform_specific/%s_%s", kComponentPlatform, kComponentArch);

#if BUILDFLAG(IS_WIN)
    EXPECT_TRUE(base::ReplaceChars(expected_path, "/", "\\", &expected_path));
#endif

    EXPECT_EQ(base::FilePath::FromUTF8Unsafe(expected_path), GetPlatformSpecificDirectory("SomeCdm"));
}
