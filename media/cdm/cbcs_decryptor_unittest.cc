...
const std::array<uint8_t, 16> kKey = { 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13 };

const std::array<uint8_t, 16> kIv = { 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

const std::array<uint8_t, kBlockSize> kOneBlock = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p' };
...
public:
    CbcsDecryptorTest()
        : key_(crypto::SymmetricKey::Import(crypto::SymmetricKey::AES, std::string(std::begin(kKey), std::end(kKey))))
    ...
    std::vector<uint8_t> Encrypt(const std::vector<uint8_t>& original, const crypto::SymmetricKey& key, const std::string& iv)
    {
    ...
    scoped_refptr<DecoderBuffer> CreateEncryptedBuffer(const std::vector<uint8_t>& data, const std::string& iv,
        const std::vector<SubsampleEntry>& subsample_entries, std::optional<EncryptionPattern> encryption_pattern)
    {
        ...
        encrypted_buffer->set_decrypt_config(DecryptConfig::CreateCbcsConfig("key_id", iv, subsample_entries, encryption_pattern));
        return encrypted_buffer;
    }

    // Calls DecryptCbcsBuffer() to decrypt |encrypted| using |key|,
    // and then returns the data in the decrypted buffer.
    std::vector<uint8_t> DecryptWithKey(scoped_refptr<DecoderBuffer> encrypted, const crypto::SymmetricKey& key)
    {
...
TEST_F(CbcsDecryptorTest, OneBlock)
{
    ...
    std::vector<SubsampleEntry> subsamples = { { 0, static_cast<uint32_t>(encrypted_block.size()) } };

    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(1, 9));
...
TEST_F(CbcsDecryptorTest, AdditionalData)
{
    ...
    encrypted_buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(encrypted_block);
    ...
    EXPECT_EQ(encrypted_buffer->end_of_stream(), decrypted_buffer->end_of_stream());
    EXPECT_EQ(encrypted_buffer->is_key_frame(), decrypted_buffer->is_key_frame());
    EXPECT_TRUE(decrypted_buffer->has_side_data());
    EXPECT_TRUE(encrypted_buffer->side_data()->Matches(*decrypted_buffer->side_data()));
}
...
TEST_F(CbcsDecryptorTest, DifferentPattern)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(1, 0));
    EXPECT_EQ(one_block_, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, EmptyPattern)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(0, 0));
    EXPECT_EQ(one_block_, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, PatternTooLarge)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(100, 0));
...
TEST_F(CbcsDecryptorTest, BadSubsamples)
{
    ...
    std::vector<SubsampleEntry> subsamples = { { 0, static_cast<uint32_t>(encrypted_block.size() + 1) } };
...
TEST_F(CbcsDecryptorTest, InvalidIv)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, invalid_iv, subsamples, EncryptionPattern(1, 0));
...
TEST_F(CbcsDecryptorTest, InvalidKey)
{
    ...
    // something other than the original data.
    std::unique_ptr<crypto::SymmetricKey> bad_key = crypto::SymmetricKey::Import(crypto::SymmetricKey::AES, std::string(std::size(kKey), 'b'));
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(1, 0));
...
TEST_F(CbcsDecryptorTest, PartialBlock)
{
    ...
    std::vector<SubsampleEntry> subsamples = { { 0, static_cast<uint32_t>(partial_block_.size()) } };

    auto encrypted_buffer = CreateEncryptedBuffer(partial_block_, iv_, subsamples, EncryptionPattern(1, 0));
...
TEST_F(CbcsDecryptorTest, SingleBlockWithExtraData)
{
    ...
    auto encrypted_block = Combine({ Encrypt(one_block_, *key_, iv_), partial_block_ });
...
TEST_F(CbcsDecryptorTest, SkipBlock)
{
    ...
    std::vector<SubsampleEntry> subsamples = { { static_cast<uint32_t>(one_block_.size()), 0 } };

    auto encrypted_buffer = CreateEncryptedBuffer(one_block_, iv_, subsamples, EncryptionPattern(1, 0));
    EXPECT_EQ(one_block_, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, MultipleBlocks)
{
    ...
    auto input_data = Combine({ GetBlock(1, encrypted_block), one_block_, GetBlock(2, encrypted_block), one_block_ });
    auto expected_result = Repeat(one_block_, 4);
    std::vector<SubsampleEntry> subsamples = { { 0, 4 * kBlockSize } };

    auto encrypted_buffer = CreateEncryptedBuffer(input_data, iv_, subsamples, EncryptionPattern(1, 1));
    EXPECT_EQ(expected_result, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, PartialPattern)
{
    ...
    auto encrypted_buffer = CreateEncryptedBuffer(encrypted_block, iv_, subsamples, EncryptionPattern(8, 2));
    EXPECT_EQ(expected_result, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, SkipBlocks)
{
    ...
    auto input_data = Combine({ one_block_, GetBlock(1, encrypted_block), GetBlock(2, encrypted_block), one_block_, GetBlock(3, encrypted_block),
        GetBlock(4, encrypted_block), one_block_, GetBlock(5, encrypted_block) });
    auto expected_result = Repeat(one_block_, 8);
    std::vector<SubsampleEntry> subsamples = { { kBlockSize, 7 * kBlockSize } };

    auto encrypted_buffer = CreateEncryptedBuffer(input_data, iv_, subsamples, EncryptionPattern(2, 1));
    EXPECT_EQ(expected_result, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, MultipleSubsamples)
{
    ...
    std::vector<SubsampleEntry> subsamples = { { 0, kBlockSize }, { 0, kBlockSize }, { 0, kBlockSize } };

    auto encrypted_buffer = CreateEncryptedBuffer(input_data, iv_, subsamples, EncryptionPattern(1, 0));
    EXPECT_EQ(expected_result, DecryptWithKey(encrypted_buffer, *key_));
}
...
TEST_F(CbcsDecryptorTest, MultipleSubsamplesWithClearBytes)
{
    ...
    auto input_data = Combine({ one_block_, encrypted_block, one_block_, encrypted_block, one_block_, encrypted_block });
    auto expected_result = Repeat(one_block_, 6);
    std::vector<SubsampleEntry> subsamples = { { kBlockSize, kBlockSize + 1 }, { kBlockSize - 1, kBlockSize + 10 }, { kBlockSize - 10, kBlockSize } };
