...
const uint8_t keyids_init_data[] = "{\"kids\":[\"AQI\",\"AQIDBA\",\"AQIDBAUGBwgJCgsMDQ4PEA\"]}";
...
const media::SubsampleEntry kSubsampleEntriesNormal[] = { { 2, 7 }, { 3, 11 }, { 1, 0 } };
...
// |init_data|.
void CreateSessionDoesNotCrash(int int_init_data_type, const std::vector<uint8_t>& init_data)
{
    ...
    scoped_refptr<media::AesDecryptor> aes_decryptor = new media::AesDecryptor(base::DoNothing(), base::DoNothing(), base::DoNothing(), base::DoNothing());
    ...
    auto create_promise = std::make_unique<media::CdmCallbackPromise<std::string>>(
        base::BindLambdaForTesting([&](const std::string& session) { session_id = session; }), base::DoNothing());
    aes_decryptor->CreateSessionAndGenerateRequest(media::CdmSessionType::kTemporary, init_data_type, init_data, std::move(create_promise));

    // If a session was created, free up any session resources.
    if (!session_id.empty()) {
        auto close_promise = std::make_unique<media::CdmCallbackPromise<>>(base::DoNothing(), base::DoNothing());
...
void UpdateSessionDoesNotCrash(const std::string& response)
{
    ...
    std::string session_id;
    std::vector<uint8_t> key_id(std::begin(webm_init_data), std::end(webm_init_data));
    auto create_promise = std::make_unique<media::CdmCallbackPromise<std::string>>(
        base::BindLambdaForTesting([&](const std::string& session) { session_id = session; }), base::DoNothing());
    aes_decryptor->CreateSessionAndGenerateRequest(media::CdmSessionType::kTemporary, media::EmeInitDataType::WEBM, key_id, std::move(create_promise));
    ...
    auto update_promise = std::make_unique<media::CdmCallbackPromise<>>(base::DoNothing(), base::DoNothing());
...
void DecryptDoesNotCrash(std::size_t clear_bytes, std::size_t encrypted_bytes, const std::vector<uint8_t>& data)
{
    // Create an AesDecryptor. Ignore any messages that may be generated.
    scoped_refptr<media::AesDecryptor> aes_decryptor = new media::AesDecryptor(base::DoNothing(), base::DoNothing(), base::DoNothing(), base::DoNothing());
    ...
    // encrypted buffer.
    auto encrypted_buffer = base::MakeRefCounted<media::DecoderBuffer>(data.size());
    ...
    encrypted_buffer->set_decrypt_config(media::DecryptConfig::CreateCencConfig(key_id_string, iv_string, subsamples));
    media::Decryptor::Status result;
    aes_decryptor->Decrypt(media::Decryptor::kVideo, encrypted_buffer,
        base::BindLambdaForTesting([&](media::Decryptor::Status status, scoped_refptr<media::DecoderBuffer>) { result = status; }));
...
FUZZ_TEST(AesDecryptorFuzzTests, CreateSessionDoesNotCrash)
    .WithDomains(fuzztest::InRange<int>(1, 3), fuzztest::Arbitrary<std::vector<uint8_t>>().WithMinSize(1))
    .WithSeeds({ { 1, std::vector<uint8_t>(std::begin(webm_init_data), std::end(webm_init_data)) },
        { 2, std::vector<uint8_t>(std::begin(cenc_init_data), std::end(cenc_init_data)) },
        { 3, std::vector<uint8_t>(std::begin(keyids_init_data), std::end(keyids_init_data)) } });
...
FUZZ_TEST(AesDecryptorFuzzTests, DecryptDoesNotCrash)
    .WithDomains(fuzztest::InRange<std::size_t>(0, 100), fuzztest::InRange<std::size_t>(0, 100), fuzztest::Arbitrary<std::vector<uint8_t>>().WithMinSize(1))
    .WithSeeds({ { 0ul, sizeof(kEncryptedData), std::vector<uint8_t>(std::begin(kEncryptedData), std::end(kEncryptedData)) } });
