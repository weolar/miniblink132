...
public:
    ...
    void EnableProtection(uint32_t desired_protection_mask, EnableProtectionCB callback) override;

    MOCK_METHOD2(ChallengePlatformCalled, bool(const std::string& service_id, const std::string& challenge));
    void ChallengePlatform(const std::string& service_id, const std::string& challenge, ChallengePlatformCB callback) override;
...
#if BUILDFLAG(IS_WIN)
    MOCK_METHOD(void, GetMediaFoundationCdmData, (GetMediaFoundationCdmDataCB callback), (override));
