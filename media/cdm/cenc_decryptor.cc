...
// span-based instead.
void CopySubsamples(const std::vector<SubsampleEntry>& subsamples, const ClearBytesBufferSel sel, const uint8_t* src, uint8_t* dst)
{
...
scoped_refptr<DecoderBuffer> DecryptCencBuffer(const DecoderBuffer& input, const crypto::SymmetricKey& key)
{
    ...
    CopySubsamples(subsamples, kSrcContainsClearBytes, sample.data(), encrypted_bytes.data());
    ...
    CopySubsamples(subsamples, kDstContainsClearBytes, reinterpret_cast<const uint8_t*>(decrypted_text.data()), output->writable_data());
    CopyExtraSettings(input, output.get());
    return output;
}
