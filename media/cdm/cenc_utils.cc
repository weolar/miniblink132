...
const uint8_t kCencCommonSystemId[] = { 0x10, 0x77, 0xef, 0xec, 0xc0, 0xb2, 0x4d, 0x02, 0xac, 0xe3, 0x3c, 0x1e, 0x52, 0xe2, 0xfb, 0x4b };
...
static bool ReadAllPsshBoxes(const std::vector<uint8_t>& input, std::vector<mp4::FullProtectionSystemSpecificHeader>* pssh_boxes)
{
    DCHECK(!input.empty());
    ...
    // nothing else.
    std::unique_ptr<mp4::BoxReader> input_reader(mp4::BoxReader::ReadConcatentatedBoxes(input.data(), input.size(), &media_log));
    ...
    for (const auto& raw_pssh_box : raw_pssh_boxes) {
        std::unique_ptr<mp4::BoxReader> raw_pssh_reader(
            mp4::BoxReader::ReadConcatentatedBoxes(raw_pssh_box.raw_box.data(), raw_pssh_box.raw_box.size(), &media_log));
...
bool GetKeyIdsForCommonSystemId(const std::vector<uint8_t>& pssh_boxes, KeyIdList* key_ids)
{
    ...
    // key IDs found.
    KeyIdList result;
    std::vector<uint8_t> common_system_id(kCencCommonSystemId, kCencCommonSystemId + std::size(kCencCommonSystemId));
...
bool GetPsshData(const std::vector<uint8_t>& input, const std::vector<uint8_t>& system_id, std::vector<uint8_t>* pssh_data)
{
    if (input.empty())
        return false;
