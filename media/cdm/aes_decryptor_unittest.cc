...
const uint8_t kIv[] = { 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
const uint8_t kEncryptedData[]
    = { 0x2f, 0x03, 0x09, 0xef, 0x71, 0xaf, 0x31, 0x16, 0xfa, 0x9d, 0x18, 0x43, 0x1e, 0x96, 0x71, 0xb5, 0xbf, 0xf5, 0x30, 0x53, 0x9a, 0x20, 0xdf, 0x95 };
...
const uint8_t kSubsampleEncryptedData[]
    = { 0x4f, 0x72, 0x09, 0x16, 0x09, 0xe6, 0x79, 0xad, 0x70, 0x73, 0x75, 0x62, 0x09, 0xbb, 0x83, 0x1d, 0x4d, 0x08, 0xd7, 0x78, 0xa4, 0xa7, 0xf1, 0x2e };

const uint8_t kOriginalData2[] = "Changed Original data.";

const uint8_t kIv2[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
const uint8_t kEncryptedData2[]
    = { 0x57, 0x66, 0xf4, 0x12, 0x1a, 0xed, 0xb5, 0x79, 0x1c, 0x8e, 0x25, 0xd7, 0x17, 0xe7, 0x5e, 0x16, 0xe3, 0x40, 0x08, 0x27, 0x11, 0xe9 };
...
const SubsampleEntry kSubsampleEntriesWrongSize[] = { { 3, 6 }, // This entry doesn't match the correct entry.
    { 3, 11 }, { 1, 0 } };

const SubsampleEntry kSubsampleEntriesInvalidTotalSize[] = { { 1, 1000 }, // This entry is too large.
    { 3, 11 }, { 1, 0 } };
...
scoped_refptr<DecoderBuffer> CreateEncryptedBuffer(
    const std::vector<uint8_t>& data, const std::vector<uint8_t>& key_id, const std::vector<uint8_t>& iv, const std::vector<SubsampleEntry>& subsample_entries)
{
    DCHECK(!data.empty());
    DCHECK(!iv.empty());
    ...
    encrypted_buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(key_id_string, iv_string, subsample_entries));
    return encrypted_buffer;
}

scoped_refptr<DecoderBuffer> CreateClearBuffer(const std::vector<uint8_t>& data)
{
    DCHECK(!data.empty());
    auto encrypted_buffer = base::MakeRefCounted<DecoderBuffer>(data.size());
    memcpy(encrypted_buffer->writable_data(), data.data(), data.size());
...
public:
    AesDecryptorTest()
        ...
        , encrypted_data_(kEncryptedData, kEncryptedData + std::size(kEncryptedData))
        , subsample_encrypted_data_(kSubsampleEncryptedData, kSubsampleEncryptedData + std::size(kSubsampleEncryptedData))
        , key_id_(kKeyId, kKeyId + std::size(kKeyId))
        , iv_(kIv, kIv + std::size(kIv))
        , normal_subsample_entries_(kSubsampleEntriesNormal, kSubsampleEntriesNormal + std::size(kSubsampleEntriesNormal))
    {
    }

    MOCK_METHOD2(BufferDecrypted, void(Decryptor::Status, scoped_refptr<DecoderBuffer>));

protected:
    void SetUp() override
    {
        if (GetParam() == TestType::kAesDecryptor) {
            OnCdmCreated(new AesDecryptor(base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
                             base::BindRepeating(&MockCdmClient::OnSessionClosed, base::Unretained(&cdm_client_)),
                             base::BindRepeating(&MockCdmClient::OnSessionKeysChange, base::Unretained(&cdm_client_)),
                             base::BindRepeating(&MockCdmClient::OnSessionExpirationUpdate, base::Unretained(&cdm_client_))),
            ...
            CdmAdapter::CreateCdmFunc create_cdm_func = CdmModule::GetInstance()->GetCreateCdmFunc();

            auto allocator = std::make_unique<SimpleCdmAllocator>();
            auto cdm_helper = std::make_unique<MockCdmAuxiliaryHelper>(std::move(allocator));
            CdmAdapter::Create(helper_->CdmConfig(), create_cdm_func, std::move(cdm_helper),
                base::BindRepeating(&MockCdmClient::OnSessionMessage, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionClosed, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionKeysChange, base::Unretained(&cdm_client_)),
                base::BindRepeating(&MockCdmClient::OnSessionExpirationUpdate, base::Unretained(&cdm_client_)),
                base::BindOnce(&AesDecryptorTest::OnCdmCreated, base::Unretained(this)));

            base::RunLoop().RunUntilIdle();
#else
            NOTREACHED() << "CdmAdapter tests only supported when library CDMs are supported.";
#endif
        } else {
    ...
    void OnCdmCreated(const scoped_refptr<ContentDecryptionModule>& cdm, CreateCdmStatus status)
    {
    ...
    void OnResolveWithSession(ExpectedResult expected_result, const std::string& session_id)
    {
    ...
    void OnReject(ExpectedResult expected_result, CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
    {
        EXPECT_EQ(expected_result, REJECTED) << "Unexpectedly rejected with message: " << error_message;
    }

    std::unique_ptr<SimpleCdmPromise> CreatePromise(ExpectedResult expected_result)
    {
        auto promise = std::make_unique<CdmCallbackPromise<>>(base::BindOnce(&AesDecryptorTest::OnResolve, base::Unretained(this), expected_result),
            base::BindOnce(&AesDecryptorTest::OnReject, base::Unretained(this), expected_result));
        return promise;
    }

    std::unique_ptr<NewSessionCdmPromise> CreateSessionPromise(ExpectedResult expected_result)
    {
        auto promise = std::make_unique<CdmCallbackPromise<std::string>>(
            base::BindOnce(&AesDecryptorTest::OnResolveWithSession, base::Unretained(this), expected_result),
            base::BindOnce(&AesDecryptorTest::OnReject, base::Unretained(this), expected_result));
        return promise;
    }
    ...
    std::string CreateSession(const std::vector<uint8_t>& key_id)
    {
        ...
        cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::WEBM, key_id, CreateSessionPromise(RESOLVED));
    ...
    {
        EXPECT_CALL(cdm_client_, OnSessionClosed(session_id, CdmSessionClosedReason::kClose));
    ...
    void UpdateSessionAndExpect(std::string session_id, const std::string& key, ExpectedResult expected_result, bool new_key_expected)
    {
        DCHECK(!key.empty());

        if (expected_result == RESOLVED) {
            EXPECT_CALL(cdm_client_, OnSessionKeysChangeCalled(session_id, new_key_expected));
        } else {
        ...
        EXPECT_CALL(cdm_client_, OnSessionExpirationUpdate(session_id, IsNullTime())).Times(AtMost(1));

        cdm_->UpdateSession(session_id, std::vector<uint8_t>(key.begin(), key.end()), CreatePromise(expected_result));
    }

    bool KeysInfoContains(const std::vector<uint8_t>& expected_key_id, CdmKeyInformation::KeyStatus expected_status = CdmKeyInformation::USABLE)
    {
        for (auto& key_id : cdm_client_.keys_info()) {
            if (key_id->key_id == expected_key_id && key_id->status == expected_status) {
                return true;
            }
        }
        return false;
    ...
    enum DecryptExpectation { SUCCESS, DATA_MISMATCH, DATA_AND_SIZE_MISMATCH, DECRYPT_ERROR, NO_KEY };

    void DecryptAndExpect(scoped_refptr<DecoderBuffer> encrypted, const std::vector<uint8_t>& plain_text, DecryptExpectation result)
    {
        ...
        case DATA_AND_SIZE_MISMATCH:
            EXPECT_CALL(*this, BufferDecrypted(Decryptor::kSuccess, NotNull())).WillOnce(SaveArg<1>(&decrypted));
            break;
        case DECRYPT_ERROR:
            EXPECT_CALL(*this, BufferDecrypted(Decryptor::kError, IsNull())).WillOnce(SaveArg<1>(&decrypted));
            break;
        case NO_KEY:
            EXPECT_CALL(*this, BufferDecrypted(Decryptor::kNoKey, IsNull())).WillOnce(SaveArg<1>(&decrypted));
            break;
        }

        if (GetParam() == TestType::kCdmAdapter) {
            ANNOTATE_SCOPED_MEMORY_LEAK; // http://crbug.com/569736
            decryptor_->Decrypt(Decryptor::kVideo, encrypted, base::BindOnce(&AesDecryptorTest::BufferDecrypted, base::Unretained(this)));
        } else {
            decryptor_->Decrypt(Decryptor::kVideo, encrypted, base::BindOnce(&AesDecryptorTest::BufferDecrypted, base::Unretained(this)));
        }
...
{
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::WEBM, std::vector<uint8_t>(), CreateSessionPromise(REJECTED));
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::CENC, std::vector<uint8_t>(), CreateSessionPromise(REJECTED));
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::KEYIDS, std::vector<uint8_t>(), CreateSessionPromise(REJECTED));
}
...
TEST_P(AesDecryptorTest, CreateSessionWithVariousLengthInitData_WebM)
{
    ...
    init_data.resize(1);
    EXPECT_CALL(cdm_client_, OnSessionMessage(NotEmpty(), _, IsJSONDictionary()));
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::WEBM, std::vector<uint8_t>(init_data), CreateSessionPromise(RESOLVED));
    ...
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::WEBM, std::vector<uint8_t>(init_data), CreateSessionPromise(REJECTED));
}

TEST_P(AesDecryptorTest, MultipleCreateSession)
{
    EXPECT_CALL(cdm_client_, OnSessionMessage(NotEmpty(), _, IsJSONDictionary()));
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::WEBM, std::vector<uint8_t>(1), CreateSessionPromise(RESOLVED));
...
TEST_P(AesDecryptorTest, CreateSessionWithCencInitData)
{
    const uint8_t init_data[] = {
        ...
        0xAC, 0xE3, 0x3C, 0x1E, 0x52, 0xE2, 0xFB, 0x4B, 0x00, 0x00, 0x00, 0x02, // key count
        0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x03, // key1
        0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x03, 0x7E, 0x57, 0x1D, 0x04, 0x7E, 0x57, 0x1D, 0x04, // key2
        0x7E, 0x57, 0x1D, 0x04, 0x7E, 0x57, 0x1D, 0x04, 0x00, 0x00, 0x00, 0x00 // datasize
    };

    EXPECT_CALL(cdm_client_, OnSessionMessage(NotEmpty(), _, IsJSONDictionary()));
    cdm_->CreateSessionAndGenerateRequest(
        CdmSessionType::kTemporary, EmeInitDataType::CENC, std::vector<uint8_t>(init_data, init_data + std::size(init_data)), CreateSessionPromise(RESOLVED));
}
...
TEST_P(AesDecryptorTest, CreateSessionWithKeyIdsInitData)
{
    ...
    EXPECT_CALL(cdm_client_, OnSessionMessage(NotEmpty(), _, IsJSONDictionary()));
    cdm_->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, EmeInitDataType::KEYIDS,
        std::vector<uint8_t>(init_data, init_data + std::size(init_data) - 1), CreateSessionPromise(RESOLVED));
}
...
TEST_P(AesDecryptorTest, NormalDecryption)
{
    ...
    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(encrypted_data_, key_id_, iv_, no_subsample_entries_);
...
TEST_P(AesDecryptorTest, UnencryptedFrame)
{
    ...
    scoped_refptr<DecoderBuffer> unencrypted_buffer = CreateClearBuffer(original_data_);
...
TEST_P(AesDecryptorTest, NoKey)
{
    ...
    decryptor_->Decrypt(Decryptor::kVideo, encrypted_buffer, base::BindOnce(&AesDecryptorTest::BufferDecrypted, base::Unretained(this)));
}
...
TEST_P(AesDecryptorTest, KeyReplacement)
{
    ...
    UpdateSessionAndExpect(session_id, kWrongKeyAsJWK, RESOLVED, true);
    ASSERT_NO_FATAL_FAILURE(DecryptAndExpect(encrypted_buffer, original_data_, DATA_MISMATCH));

    UpdateSessionAndExpect(session_id, kKeyAsJWK, RESOLVED, false);
    ASSERT_NO_FATAL_FAILURE(DecryptAndExpect(encrypted_buffer, original_data_, SUCCESS));
}
...
TEST_P(AesDecryptorTest, MultipleKeysAndFrames)
{
    ...
    encrypted_buffer = CreateEncryptedBuffer(std::vector<uint8_t>(kEncryptedData2, kEncryptedData2 + std::size(kEncryptedData2)),
        std::vector<uint8_t>(kKeyId2, kKeyId2 + std::size(kKeyId2)), std::vector<uint8_t>(kIv2, kIv2 + std::size(kIv2)), no_subsample_entries_);
    ASSERT_NO_FATAL_FAILURE(DecryptAndExpect(encrypted_buffer, std::vector<uint8_t>(kOriginalData2, kOriginalData2 + std::size(kOriginalData2) - 1), SUCCESS));
}
...
TEST_P(AesDecryptorTest, CorruptedIv)
{
    ...
    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(encrypted_data_, key_id_, bad_iv, no_subsample_entries_);
...
TEST_P(AesDecryptorTest, EncryptedAsUnencryptedFailure)
{
    std::string session_id = CreateSession(key_id_);
    UpdateSessionAndExpect(session_id, kKeyAsJWK, RESOLVED, true);
    scoped_refptr<DecoderBuffer> unencrypted_buffer = CreateClearBuffer(encrypted_data_);
...
TEST_P(AesDecryptorTest, SubsampleDecryption)
{
    std::string session_id = CreateSession(key_id_);
    UpdateSessionAndExpect(session_id, kKeyAsJWK, RESOLVED, true);
    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(subsample_encrypted_data_, key_id_, iv_, normal_subsample_entries_);
    DecryptAndExpect(encrypted_buffer, original_data_, SUCCESS);
}
...
TEST_P(AesDecryptorTest, SubsampleWrongSize)
{
    ...
    std::vector<SubsampleEntry> subsample_entries_wrong_size(kSubsampleEntriesWrongSize, kSubsampleEntriesWrongSize + std::size(kSubsampleEntriesWrongSize));

    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(subsample_encrypted_data_, key_id_, iv_, subsample_entries_wrong_size);
    DecryptAndExpect(encrypted_buffer, original_data_, DATA_MISMATCH);
}
...
TEST_P(AesDecryptorTest, SubsampleInvalidTotalSize)
{
    ...
    std::vector<SubsampleEntry> subsample_entries_invalid_total_size(
        kSubsampleEntriesInvalidTotalSize, kSubsampleEntriesInvalidTotalSize + std::size(kSubsampleEntriesInvalidTotalSize));
...
TEST_P(AesDecryptorTest, SubsampleClearBytesOnly)
{
    ...
    std::vector<SubsampleEntry> clear_only_subsample_entries(kSubsampleEntriesClearOnly, kSubsampleEntriesClearOnly + std::size(kSubsampleEntriesClearOnly));

    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(original_data_, key_id_, iv_, clear_only_subsample_entries);
    DecryptAndExpect(encrypted_buffer, original_data_, SUCCESS);
}
...
TEST_P(AesDecryptorTest, SubsampleCypherBytesOnly)
{
    ...
    std::vector<SubsampleEntry> cypher_only_subsample_entries(
        kSubsampleEntriesCypherOnly, kSubsampleEntriesCypherOnly + std::size(kSubsampleEntriesCypherOnly));

    scoped_refptr<DecoderBuffer> encrypted_buffer = CreateEncryptedBuffer(encrypted_data_, key_id_, iv_, cypher_only_subsample_entries);
    DecryptAndExpect(encrypted_buffer, original_data_, SUCCESS);
}
...
TEST_P(AesDecryptorTest, RemoveSession)
{
    ...
    ASSERT_NO_FATAL_FAILURE(DecryptAndExpect(encrypted_buffer, original_data_, NO_KEY));
}
...
TEST_P(AesDecryptorTest, JWKKey)
{
    ...
    UpdateSessionAndExpect(session_id, "This is not ASCII due to \xff\xfe\xfd in it.", REJECTED, true);
    ...
    UpdateSessionAndExpect(session_id, "{ \"keys\": [ 1, 2, 3 ] }", REJECTED, true);
...
INSTANTIATE_TEST_SUITE_P(AesDecryptor, AesDecryptorTest, testing::Values(TestType::kAesDecryptor));

#if BUILDFLAG(ENABLE_LIBRARY_CDMS)
INSTANTIATE_TEST_SUITE_P(CdmAdapter, AesDecryptorTest, testing::Values(TestType::kCdmAdapter));
#endif
