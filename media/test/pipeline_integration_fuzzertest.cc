...
void OnEncryptedMediaInitData(media::PipelineIntegrationTestBase* test, media::EmeInitDataType /* type */, const std::vector<uint8_t>& /* init_data */)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&PipelineIntegrationTestBase::FailTest, base::Unretained(test), media::PIPELINE_ERROR_INITIALIZATION_FAILED));
}

void OnAudioPlayDelay(media::PipelineIntegrationTestBase* test, base::TimeDelta play_delay)
{
...
class ProgressivePipelineIntegrationFuzzerTest : public PipelineIntegrationTestBase {
public:
    ProgressivePipelineIntegrationFuzzerTest()
    {
        set_encrypted_media_init_data_cb(base::BindRepeating(&OnEncryptedMediaInitData, this));
        set_audio_play_delay_cb(base::BindPostTaskToCurrentDefault(base::BindRepeating(&OnAudioPlayDelay, this)));
    }
...
class MediaSourcePipelineIntegrationFuzzerTest : public PipelineIntegrationTestBase {
public:
    ...
    void RunTest(const uint8_t* data, size_t size, const std::string& mimetype)
    {
        ...
        auto external_memory = std::make_unique<media::ExternalMemoryAdapterForTesting>(base::make_span(data, size));
        scoped_refptr<media::DecoderBuffer> buffer = media::DecoderBuffer::FromExternalMemory(std::move(external_memory));
        ...
        source.set_encrypted_media_init_data_cb(base::BindRepeating(&OnEncryptedMediaInitData, this));

        // Allow parsing to either pass or fail without emitting a gtest failure
        // from TestMediaSource.
        source.set_expected_append_result(TestMediaSource::ExpectedAppendResult::kSuccessOrFailure);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    ::testing::internal::GetUnitTestImpl()->os_stack_trace_getter()->UponLeavingGTest();
    if (variant == SRC) {
