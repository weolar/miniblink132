...
class TestDataSourceFactory : public HlsDataSourceProviderImpl::DataSourceFactory {
public:
    ...
    void CreateDataSource(GURL uri, bool, DataSourceCb callback) override
    {
        ...
        CHECK(file_data_source->Initialize(file_path)) << "Is " << file_path.value() << " missing?";
...
static std::vector<std::unique_ptr<VideoDecoder>> CreateVideoDecodersForTest(MediaLog* media_log, CreateVideoDecodersCB prepend_video_decoders_cb)
{
...
#if BUILDFLAG(ENABLE_DAV1D_DECODER)
    video_decoders.push_back(std::make_unique<OffloadingDav1dVideoDecoder>(media_log->Clone()));
#endif
...
static std::vector<std::unique_ptr<AudioDecoder>> CreateAudioDecodersForTest(
    MediaLog* media_log, const scoped_refptr<base::SingleThreadTaskRunner>& media_task_runner, CreateAudioDecodersCB prepend_audio_decoders_cb)
{
...
#if BUILDFLAG(ENABLE_FFMPEG)
    audio_decoders.push_back(std::make_unique<FFmpegAudioDecoder>(media_task_runner, media_log));
#endif
    ...
    , current_duration_(kInfiniteDuration)
{
    ...
    pipeline_ = std::make_unique<PipelineImpl>(task_environment_.GetMainThreadTaskRunner(), task_environment_.GetMainThreadTaskRunner(),
        base::BindRepeating(&PipelineIntegrationTestBase::CreateRenderer, base::Unretained(this)), &media_log_);
...
void PipelineIntegrationTestBase::OnSeeked(base::TimeDelta seek_time, PipelineStatus status)
{
...
void PipelineIntegrationTestBase::OnStatusCallback(const base::RepeatingClosure& quit_run_loop_closure, PipelineStatus status)
{
    pipeline_status_ = status;
...
void PipelineIntegrationTestBase::DemuxerEncryptedMediaInitDataCB(EmeInitDataType type, const std::vector<uint8_t>& init_data)
{
...
void PipelineIntegrationTestBase::DemuxerMediaTracksUpdatedCB(std::unique_ptr<MediaTracks> tracks)
{
    CHECK(tracks);
...
void PipelineIntegrationTestBase::SetCreateRendererCB(CreateRendererCB create_renderer_cb)
{
    create_renderer_cb_ = std::move(create_renderer_cb);
}

#if BUILDFLAG(ENABLE_HLS_DEMUXER)
PipelineStatus PipelineIntegrationTestBase::StartPipelineWithHlsManifest(const std::string& filename)
{
    ...
    auto hls_dsp = base::SequenceBound<HlsDataSourceProviderImpl>(task_environment_.GetMainThreadTaskRunner(), std::move(multibuffer_factory));

    auto engine
        = std::make_unique<HlsManifestDemuxerEngine>(std::move(hls_dsp), task_environment_.GetMainThreadTaskRunner(), base::DoNothing(), base::DoNothing(),
            /*name=*/false, manifest_root, &media_log_);
    demuxer_ = std::make_unique<ManifestDemuxer>(task_environment_.GetMainThreadTaskRunner(), base::DoNothing(), std::move(engine), &media_log_);
    EXPECT_CALL(*this, OnMetadata(_)).Times(AtMost(1)).WillRepeatedly(SaveArg<0>(&metadata_));

    base::RunLoop run_loop;
    pipeline_->Start(Pipeline::StartType::kNormal, demuxer_.get(), this,
        base::BindOnce(&PipelineIntegrationTestBase::OnStatusCallback, base::Unretained(this), run_loop.QuitClosure()));
    RunUntilQuitOrEndedOrError(&run_loop);
    return pipeline_status_;
}
#endif // BUILDFLAG(ENABLE_HLS_DEMUXER)

PipelineStatus PipelineIntegrationTestBase::StartInternal(std::unique_ptr<DataSource> data_source, CdmContext* cdm_context, uint8_t test_type,
    CreateVideoDecodersCB prepend_video_decoders_cb, CreateAudioDecodersCB prepend_audio_decoders_cb)
{
    ...
    if (test_type & kUnreliableDuration) {
        ...
        EXPECT_CALL(*this, OnDurationChange()).Times(AtMost(2)).WillRepeatedly(Invoke(this, &PipelineIntegrationTestBase::CheckDuration));
    }
    ...
    if (cdm_context) {
        ...
        pipeline_->SetCdm(cdm_context, base::BindOnce(&PipelineIntegrationTestBase::DecryptorAttached, base::Unretained(this)));
    }
...
PipelineStatus PipelineIntegrationTestBase::StartWithFile(const std::string& filename, CdmContext* cdm_context, uint8_t test_type,
    CreateVideoDecodersCB prepend_video_decoders_cb, CreateAudioDecodersCB prepend_audio_decoders_cb)
{
    std::unique_ptr<FileDataSource> file_data_source(new FileDataSource());
    base::FilePath file_path(GetTestDataFilePath(filename));
    CHECK(file_data_source->Initialize(file_path)) << "Is " << file_path.value() << " missing?";
    return StartInternal(std::move(file_data_source), cdm_context, test_type, prepend_video_decoders_cb, prepend_audio_decoders_cb);
}
...
PipelineStatus PipelineIntegrationTestBase::Start(const std::string& filename, CdmContext* cdm_context)
{
    return StartWithFile(filename, cdm_context, kNormal);
}

PipelineStatus PipelineIntegrationTestBase::Start(
    const std::string& filename, uint8_t test_type, CreateVideoDecodersCB prepend_video_decoders_cb, CreateAudioDecodersCB prepend_audio_decoders_cb)
{
    return StartWithFile(filename, nullptr, test_type, prepend_video_decoders_cb, prepend_audio_decoders_cb);
}

PipelineStatus PipelineIntegrationTestBase::Start(const uint8_t* data, size_t size, uint8_t test_type)
{
    return StartInternal(std::make_unique<MemoryDataSource>(data, size), nullptr, test_type);
}
...
void PipelineIntegrationTestBase::OnBufferingStateChangeForSeek(BufferingState state, BufferingStateChangeReason reason)
{
...
bool PipelineIntegrationTestBase::Seek(base::TimeDelta seek_time)
{
    ...
    EXPECT_CALL(*this, OnBufferingStateChange(_, _))
        .Times(AnyNumber())
        .WillRepeatedly(Invoke(this, &PipelineIntegrationTestBase::OnBufferingStateChangeForSeek));

    bool did_call_on_seeked = false;
    pipeline_->Seek(seek_time,
        base::BindOnce(
            [](PipelineIntegrationTestBase* thiz, bool* flag, base::TimeDelta seek_time, PipelineStatus status) {
                *flag = true;
...
{
    base::RunLoop run_loop;
    pipeline_->Suspend(base::BindOnce(&PipelineIntegrationTestBase::OnStatusCallback, base::Unretained(this), run_loop.QuitClosure()));
...
bool PipelineIntegrationTestBase::Resume(base::TimeDelta seek_time)
{
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(*this, OnBufferingStateChange(BUFFERING_HAVE_ENOUGH, _)).WillOnce(InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
    pipeline_->Resume(seek_time, base::BindOnce(&PipelineIntegrationTestBase::OnSeeked, base::Unretained(this), seek_time));
    RunUntilQuitOrError(&run_loop);
    return (pipeline_status_ == PIPELINE_OK);
}
...
void PipelineIntegrationTestBase::QuitAfterCurrentTimeTask(base::TimeDelta quit_time, base::OnceClosure quit_closure)
{
    if (!pipeline_ || pipeline_->GetMediaTime() >= quit_time || pipeline_status_ != PIPELINE_OK) {
        std::move(quit_closure).Run();
        return;
    }

    task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE,
        base::BindOnce(&PipelineIntegrationTestBase::QuitAfterCurrentTimeTask, base::Unretained(this), quit_time, std::move(quit_closure)),
        base::Milliseconds(10));
}

bool PipelineIntegrationTestBase::WaitUntilCurrentTimeIsAfter(const base::TimeDelta& wait_time)
{
    DCHECK(pipeline_->IsRunning());
    ...
    base::RunLoop run_loop;
    task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE,
        base::BindOnce(&PipelineIntegrationTestBase::QuitAfterCurrentTimeTask, base::Unretained(this), wait_time, run_loop.QuitClosure()),
        base::Milliseconds(10));
...
void PipelineIntegrationTestBase::CreateDemuxer(std::unique_ptr<DataSource> data_source)
{
    data_source_ = std::move(data_source);

#if BUILDFLAG(ENABLE_FFMPEG)
    demuxer_ = std::unique_ptr<Demuxer>(new FFmpegDemuxer(task_environment_.GetMainThreadTaskRunner(), data_source_.get(),
        base::BindRepeating(&PipelineIntegrationTestBase::DemuxerEncryptedMediaInitDataCB, base::Unretained(this)),
        base::BindRepeating(&PipelineIntegrationTestBase::DemuxerMediaTracksUpdatedCB, base::Unretained(this)), &media_log_, true));
#endif
}

std::unique_ptr<Renderer> PipelineIntegrationTestBase::CreateRenderer(std::optional<RendererType> renderer_type)
{
...
std::unique_ptr<Renderer> PipelineIntegrationTestBase::CreateRendererImpl(std::optional<RendererType> renderer_type)
{
    ...
    video_sink_ = std::make_unique<NullVideoSink>(clockless_playback_, base::Seconds(1.0 / 60),
        base::BindRepeating(&PipelineIntegrationTestBase::OnVideoFramePaint, base::Unretained(this)), task_environment_.GetMainThreadTaskRunner());

    // Disable frame dropping if hashing is enabled.
    auto video_renderer = std::make_unique<VideoRendererImpl>(task_environment_.GetMainThreadTaskRunner(), video_sink_.get(),
        base::BindRepeating(&CreateVideoDecodersForTest, &media_log_, prepend_video_decoders_cb_), false, &media_log_, nullptr, 0);

    if (!clockless_playback_) {
        DCHECK(!mono_output_) << " NullAudioSink doesn't specify output parameters";

        audio_sink_ = new NullAudioSink(task_environment_.GetMainThreadTaskRunner());
    } else {
        ChannelLayoutConfig output_layout_config = mono_output_ ? ChannelLayoutConfig::Mono() : ChannelLayoutConfig::Stereo();

        clockless_audio_sink_ = new ClocklessAudioSink(
            OutputDeviceInfo("", OUTPUT_DEVICE_STATUS_OK, AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, output_layout_config, 44100, 512)));
        ...
        if (webaudio_attached_ || hashing_enabled_) {
            clockless_audio_sink_->SetIsOptimizedForHardwareParametersForTesting(false);
        }
    }

    auto audio_renderer = std::make_unique<AudioRendererImpl>(task_environment_.GetMainThreadTaskRunner(),
        (clockless_playback_) ? static_cast<AudioRendererSink*>(clockless_audio_sink_.get()) : audio_sink_.get(),
        base::BindRepeating(&CreateAudioDecodersForTest, &media_log_, task_environment_.GetMainThreadTaskRunner(), prepend_audio_decoders_cb_), &media_log_, 0,
        nullptr);
    if (hashing_enabled_) {
        if (clockless_playback_)
    ...
    static_cast<AudioRendererImpl*>(audio_renderer.get())->SetPlayDelayCBForTesting(std::move(audio_play_delay_cb_));

    std::unique_ptr<RendererImpl> renderer_impl
        = std::make_unique<RendererImpl>(task_environment_.GetMainThreadTaskRunner(), std::move(audio_renderer), std::move(video_renderer));
...
void PipelineIntegrationTestBase::OnVideoFramePaint(scoped_refptr<VideoFrame> frame)
{
...
PipelineStatus PipelineIntegrationTestBase::StartPipelineWithMediaSource(TestMediaSource* source)
{
    return StartPipelineWithMediaSource(source, kNormal, nullptr);
}

PipelineStatus PipelineIntegrationTestBase::StartPipelineWithEncryptedMedia(TestMediaSource* source, FakeEncryptedMedia* encrypted_media)
{
...
PipelineStatus PipelineIntegrationTestBase::StartPipelineWithMediaSource(
    TestMediaSource* source, uint8_t test_type, CreateAudioDecodersCB prepend_audio_decoders_cb)
{
    prepend_audio_decoders_cb_ = prepend_audio_decoders_cb;
    return StartPipelineWithMediaSource(source, test_type, nullptr);
}

PipelineStatus PipelineIntegrationTestBase::StartPipelineWithMediaSource(TestMediaSource* source, uint8_t test_type, FakeEncryptedMedia* encrypted_media)
{
    ParseTestTypeFlags(test_type);
    ...
    source->set_demuxer_failure_cb(base::BindRepeating(&PipelineIntegrationTestBase::OnStatusCallback, base::Unretained(this), run_loop.QuitClosure()));
    ...
    if (encrypted_media) {
        ...
        // never called.
        EXPECT_CALL(*this, OnWaiting(WaitingReason::kNoDecryptionKey)).Times(0);
        pipeline_->SetCdm(encrypted_media->GetCdmContext(), base::BindOnce(&PipelineIntegrationTestBase::DecryptorAttached, base::Unretained(this)));
    } else if (fuzzing_) {
        ...
        EXPECT_CALL(*this, OnWaiting(WaitingReason::kNoCdm)).Times(AnyNumber()).WillRepeatedly([this]() {
            task_environment_.GetMainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&PipelineIntegrationTestBase::FailTest, base::Unretained(this), media::PIPELINE_ERROR_INITIALIZATION_FAILED));
        });
    } else {
    ...
    if (encrypted_media) {
        source->set_encrypted_media_init_data_cb(base::BindRepeating(&FakeEncryptedMedia::OnEncryptedMediaInitData, base::Unretained(encrypted_media)));
    }
...
void PipelineIntegrationTestBase::RunUntilQuitOrEndedOrError(base::RunLoop* run_loop)
{
