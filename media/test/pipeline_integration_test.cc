...
#define EXPECT_AUDIO_HASH(expected)                                                                                                                            \
    EXPECT_TRUE(GetAudioHash().IsEquivalent(expected, 0.04)) << "Audio hashes differ. Expected: " << expected << " Actual: " << GetAudioHash().ToString()
...
constexpr char kOpusEndTrimmingHash_1[] = "-4.57,-5.67,-6.52,-6.28,-4.34,-3.58,";

// The above hash, plus an additional playthrough starting from T=1s.
constexpr char kOpusEndTrimmingHash_2[] = "-11.91,-11.10,-8.24,-7.08,-7.82,-9.99,";

// The above hash, plus an additional playthrough starting from T=6.36s.
constexpr char kOpusEndTrimmingHash_3[] = "-13.31,-14.36,-13.66,-11.65,-10.16,-10.47,";

// Hash for a full playthrough of "bear-opus.webm".
constexpr char kOpusSmallCodecDelayHash_1[] = "-0.47,-0.09,1.28,1.07,1.55,-0.22,";
...
static base::Time kLiveTimelineOffset()
{
    ...
    static constexpr base::Time::Exploded kExplodedTime
        = { .year = 2012, .month = 11, .day_of_week = 6, .day_of_month = 10, .hour = 12, .minute = 34, .second = 56, .millisecond = 789 };
...
public:
    ...
    void OnResolveWithSession(PromiseResult expected, const std::string& session_id)
    {
    ...
    void OnReject(PromiseResult expected, media::CdmPromise::Exception exception_code, uint32_t system_code, const std::string& error_message)
    {
    ...
    {
        auto promise = std::make_unique<media::CdmCallbackPromise<>>(base::BindOnce(&KeyProvidingApp::OnResolve, base::Unretained(this), expected),
            base::BindOnce(&KeyProvidingApp::OnReject, base::Unretained(this), expected));
        return promise;
    }

    std::unique_ptr<NewSessionCdmPromise> CreateSessionPromise(PromiseResult expected)
    {
        auto promise
            = std::make_unique<media::CdmCallbackPromise<std::string>>(base::BindOnce(&KeyProvidingApp::OnResolveWithSession, base::Unretained(this), expected),
                base::BindOnce(&KeyProvidingApp::OnReject, base::Unretained(this), expected));
        return promise;
    }

    void OnSessionMessage(const std::string& session_id, CdmMessageType message_type, const std::vector<uint8_t>& message, AesDecryptor* decryptor) override
    {
        ...
        EXPECT_TRUE(ExtractKeyIdsFromKeyIdsInitData(message_string, &key_ids, &error_message)) << error_message;
        ...
        std::string jwk = GenerateJWKSet(key.data(), key.size(), key_ids[0].data(), key_ids[0].size());
        decryptor->UpdateSession(session_id, std::vector<uint8_t>(jwk.begin(), jwk.end()), CreatePromise(RESOLVED));
    }

    void OnSessionClosed(const std::string& session_id, CdmSessionClosedReason /*reason*/) override
    {
        EXPECT_EQ(current_session_id_, session_id);
    }

    void OnSessionKeysChange(const std::string& session_id, bool has_additional_usable_key, CdmKeysInfo keys_info) override
    {
        EXPECT_EQ(current_session_id_, session_id);
        EXPECT_EQ(has_additional_usable_key, true);
    }

    void OnSessionExpirationUpdate(const std::string& session_id, base::Time new_expiry_time) override
    {
        EXPECT_EQ(current_session_id_, session_id);
    }

    void OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data, AesDecryptor* decryptor) override
    {
        ...
        if (current_session_id_.empty()) {
            decryptor->CreateSessionAndGenerateRequest(CdmSessionType::kTemporary, init_data_type, init_data, CreateSessionPromise(RESOLVED));
            EXPECT_FALSE(current_session_id_.empty());
        }
    }

    virtual bool LookupKey(const std::vector<uint8_t>& key_id, std::vector<uint8_t>* key)
    {
        // No key rotation.
...
public:
    ...
    bool LookupKey(const std::vector<uint8_t>& key_id, std::vector<uint8_t>* key) override
    {
        // With key rotation.
...
public:
    ...
    void Initialize(const VideoDecoderConfig& config, bool low_delay, CdmContext* cdm_context, InitCB init_cb, const OutputCB& output_cb,
    ...
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(decode_cb), DecoderStatus::Codes::kFailed));
    }
...
class PipelineIntegrationTest : public testing::Test, public PipelineIntegrationTestBase {
public:
    ...
    bool TestSeekDuringRead(const std::string& filename, int initial_append_size, base::TimeDelta start_seek_time, base::TimeDelta seek_time,
        int seek_file_position, int seek_append_size)
    {
        TestMediaSource source(filename, initial_append_size);

        if (StartPipelineWithMediaSource(&source, kNoClockless, nullptr) != PIPELINE_OK) {
            return false;
        }
    ...
    void OnEnabledAudioTracksChanged(const std::vector<MediaTrack::Id>& enabled_track_ids)
    {
        base::RunLoop run_loop;
        pipeline_->OnEnabledAudioTracksChanged(enabled_track_ids, run_loop.QuitClosure());
        run_loop.Run();
    }

    void OnSelectedVideoTrackChanged(std::optional<MediaTrack::Id> selected_track_id)
    {
        base::RunLoop run_loop;
        pipeline_->OnSelectedVideoTrackChanged(selected_track_id, run_loop.QuitClosure());
        run_loop.Run();
    }
};
...
class BasicPlaybackTest : public PipelineIntegrationTest, public testing::WithParamInterface<PlaybackTestData> { };
...
INSTANTIATE_TEST_SUITE_P(OpenCodecs, BasicPlaybackTest, testing::ValuesIn(kOpenCodecsTests));

#if BUILDFLAG(USE_PROPRIETARY_CODECS)

class BasicMSEPlaybackTest : public ::testing::WithParamInterface<MSEPlaybackTestData>, public PipelineIntegrationTest {
protected:
    void PlayToEnd()
    {
        ...
        ASSERT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source, kNormal, nullptr));
        source.EndOfStream();

        EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
        EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
        EXPECT_EQ(data.duration_ms, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
INSTANTIATE_TEST_SUITE_P(ProprietaryCodecs, BasicPlaybackTest, testing::ValuesIn(kADTSTests));
...
INSTANTIATE_TEST_SUITE_P(ProprietaryCodecs, BasicMSEPlaybackTest, testing::ValuesIn(kMediaSourceADTSTests));
...
class MSEChangeTypeTest : public ::testing::WithParamInterface<std::tuple<MSEPlaybackTestData, MSEPlaybackTestData>>, public PipelineIntegrationTest {
public:
    ...
    struct PrintToStringParamName {
        ...
        {
            std::string s = base::StrCat({ std::get<0>(info.param).filename, "_AND_", std::get<1>(info.param).filename });
            // Strip out invalid param name characters.
            std::erase_if(s, [](char c) { return !absl::ascii_isalnum(static_cast<unsigned char>(c)) && c != '_'; });
            return s;
        }
    };
...
protected:
    void PlayBackToBack()
    {
        ...
        scoped_refptr<DecoderBuffer> file_two_contents = ReadTestDataFile(file_two.filename);
        source.AppendAtTime(file_one_end_time,
            file_two.append_bytes == kAppendWholeFile ? file_two_contents->AsSpan() : file_two_contents->AsSpan().first(file_two.append_bytes));
        source.EndOfStream();
        ranges = pipeline_->GetBufferedTimeRanges();
        EXPECT_EQ(1u, ranges.size());
        EXPECT_EQ(0, ranges.start(0).InMilliseconds());

        base::TimeDelta file_two_actual_duration = ranges.end(0) - file_one_end_time;
...
#if BUILDFLAG(USE_PROPRIETARY_CODECS) && BUILDFLAG(ENABLE_FFMPEG_VIDEO_DECODERS)
    // H264 AVC3 in MP4
    { "bear-1280x720-v_frag-avc3.mp4", kAppendWholeFile, k1280IsoAVC3FileDurationMs },
#endif
};

INSTANTIATE_TEST_SUITE_P(AudioOnly, MSEChangeTypeTest, testing::Combine(testing::ValuesIn(kMediaSourceAudioFiles), testing::ValuesIn(kMediaSourceAudioFiles)),
    MSEChangeTypeTest::PrintToStringParamName());

INSTANTIATE_TEST_SUITE_P(VideoOnly, MSEChangeTypeTest, testing::Combine(testing::ValuesIn(kMediaSourceVideoFiles), testing::ValuesIn(kMediaSourceVideoFiles)),
    MSEChangeTypeTest::PrintToStringParamName());
...
{
    ASSERT_EQ(PIPELINE_OK, Start("bear-opus-11ch-channelmapping2.ogg", kWebAudio));
...
#if BUILDFLAG(IS_MAC)
#define MAYBE_TrackStatusChangesWhileSuspended DISABLED_TrackStatusChangesWhileSuspended
#else
...
TEST_F(PipelineIntegrationTest, MAYBE_TrackStatusChangesWhileSuspended)
{
    ...
    EXPECT_CALL(*this, OnVideoNaturalSizeChange(gfx::Size(320, 240))).Times(AnyNumber());
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlaybackOpusWebmTrimmingHashed)
{
    ...
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source, kHashed, nullptr));
    source.EndOfStream();
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlaybackOpusMp4TrimmingHashed)
{
    ...
    source.SetAppendWindow(base::TimeDelta(), base::TimeDelta(), base::Microseconds(12720021));
...
{
    ASSERT_EQ(PIPELINE_OK, Start("bunny-opus-intensity-stereo.webm", kHashed | kMonoOutput));
    ...
    AudioDecoderConfig config = demuxer_->GetFirstStream(DemuxerStream::AUDIO)->audio_decoder_config();
...
TEST_F(PipelineIntegrationTest, BasicPlaybackOpusPrerollExceedsCodecDelay)
{
    ...
    base::TimeDelta codec_delay = base::Seconds(static_cast<double>(config.codec_delay()) / config.samples_per_second());
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlaybackOpusMp4PrerollExceedsCodecDelay)
{
    ...
    // achieve correctness either. Delete this comment and remove the manual
    // SetAppendWindow() if/when https://crbug.com/876544 is fixed.
    source.SetAppendWindow(base::TimeDelta(), base::TimeDelta(), base::Microseconds(2740834));
...
TEST_F(PipelineIntegrationTest, BasicPlaybackEncrypted)
{
    ...
    set_encrypted_media_init_data_cb(base::BindRepeating(&FakeEncryptedMedia::OnEncryptedMediaInitData, base::Unretained(&encrypted_media)));
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(k320WebMFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_EosBeforeDemuxerOpened)
{
    ...
    EXPECT_EQ(DEMUXER_ERROR_COULD_NOT_OPEN, StartPipelineWithMediaSource(&source, kExpectDemuxerFailure, nullptr));
}
...
TEST_F(PipelineIntegrationTest, MSE_CorruptedFirstMediaSegment)
{
    ...
    TestMediaSource source("bear-320x240_corrupted_after_init_segment.webm", 4380);
    source.set_expected_append_result(TestMediaSource::ExpectedAppendResult::kFailure);
    EXPECT_EQ(CHUNK_DEMUXER_ERROR_APPEND_FAILED, StartPipelineWithMediaSource(&source));
}
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_AV1_WebM)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(kVP9WebMFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_VP8A_WebM)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(kVP8AWebMFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_ConfigChange_AV1_WebM)
{
    ...
    EXPECT_CALL(*this, OnVideoConfigChange(::testing::Property(&VideoDecoderConfig::natural_size, kNewSize))).Times(1);
    EXPECT_CALL(*this, OnVideoNaturalSizeChange(kNewSize)).Times(1);
    scoped_refptr<DecoderBuffer> second_file = ReadTestDataFile("bear-av1-640x480.webm");
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(kAppendTimeMs + kAV1640WebMFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_ConfigChange_WebM)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(kAppendTimeMs + k640WebMFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_AudioConfigChange_WebM)
{
    ...
    EXPECT_CALL(*this, OnAudioConfigChange(::testing::Property(&AudioDecoderConfig::samples_per_second, kNewSampleRate))).Times(1);
    ...
    EXPECT_MEDIA_LOG(Not(HasSubstr("Large timestamp gap detected"))).Times(AnyNumber());

    scoped_refptr<DecoderBuffer> second_file = ReadTestDataFile("bear-320x240-audio-only-48khz.webm");
    ASSERT_TRUE(source.AppendAtTime(base::Seconds(kAppendTimeSec), second_file->AsSpan()));
    source.EndOfStream();
...
TEST_F(PipelineIntegrationTest, MSE_RemoveUpdatesBufferedRanges)
{
    ...
    source.RemoveRange(base::Milliseconds(1000), base::Milliseconds(k320WebMFileDurationMs));
...
TEST_F(PipelineIntegrationTest, MSE_FillUpBuffer)
{
    ...
    do {
        ...
        buffered_ranges = pipeline_->GetBufferedTimeRanges();
    } while (buffered_ranges.size() == 1 && buffered_ranges.start(0) == base::Seconds(0));
...
{
    TestMediaSource source("bear-320x240-16x9-aspect-av_enc-av.webm", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new KeyProvidingApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_AV1_10bit_MP4)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(kAV110bitMp4FileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_fLaCInMp4_Hashed)
{
    TestMediaSource source("sfx-flac_frag.mp4", "audio/mp4; codecs=\"fLaC\"", kAppendWholeFile);
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source, kHashed, nullptr));
    source.EndOfStream();
...
class Mp3FastSeekIntegrationTest : public PipelineIntegrationTest, public testing::WithParamInterface<Mp3FastSeekParams> { };
...
INSTANTIATE_TEST_SUITE_P(
    CBRSeek_HasTOC, Mp3FastSeekIntegrationTest, ::testing::Values(Mp3FastSeekParams("bear-audio-10s-CBR-has-TOC.mp3", "-0.58,0.61,3.08,2.55,0.90,-1.20,")));

INSTANTIATE_TEST_SUITE_P(
    CBRSeeks_NoTOC, Mp3FastSeekIntegrationTest, ::testing::Values(Mp3FastSeekParams("bear-audio-10s-CBR-no-TOC.mp3", "1.16,0.68,1.25,0.60,1.66,0.93,")));

// VBR seeks can be fast *OR* accurate, but not both. We chose fast.
INSTANTIATE_TEST_SUITE_P(
    VBRSeeks_HasTOC, Mp3FastSeekIntegrationTest, ::testing::Values(Mp3FastSeekParams("bear-audio-10s-VBR-has-TOC.mp3", "-0.08,-0.53,0.75,0.89,2.44,0.73,")));

INSTANTIATE_TEST_SUITE_P(
    VBRSeeks_NoTOC, Mp3FastSeekIntegrationTest, ::testing::Values(Mp3FastSeekParams("bear-audio-10s-VBR-no-TOC.mp3", "-0.22,0.80,1.19,0.73,-0.31,-1.12,")));
...
TEST_F(PipelineIntegrationTest, MSE_MP3_TimestampOffset)
{
    ...
    source.AppendAtTimeWithWindow(append_time, append_time + mp3_preroll_duration, kInfiniteDuration, second_file->AsSpan());
    source.EndOfStream();
...
TEST_F(PipelineIntegrationTest, MSE_ADTS_TimestampOffset)
{
    ...
    const base::TimeDelta adts_preroll_duration = base::Seconds(2.5 * 1024 / 44100);
    ...
    source.AppendAtTimeWithWindow(append_time, append_time + adts_preroll_duration, kInfiniteDuration, second_file->AsSpan());
    source.EndOfStream();
...
{
    ASSERT_EQ(PIPELINE_OK, Start("440hz-10ms.m4a", kHashed | kUnreliableDuration));
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
...
std::unique_ptr<AudioDecoder> CreateXheAacDecoder(scoped_refptr<base::SequencedTaskRunner> task_runner, MediaLog& media_log)
{
#if BUILDFLAG(IS_MAC)
...
{
    if (!IsDecoderSupportedAudioType({ AudioCodec::kAAC, AudioCodecProfile::kXHE_AAC, false })) {
        GTEST_SKIP() << "Unsupported platform.";
    }

    auto prepend_audio_decoders_cb = base::BindLambdaForTesting([this]() {
        std::vector<std::unique_ptr<AudioDecoder>> audio_decoders;
        audio_decoders.push_back(CreateXheAacDecoder(task_environment_.GetMainThreadTaskRunner(), media_log_));
        return audio_decoders;
    });

    ASSERT_EQ(PIPELINE_OK, Start("noise-xhe-aac.mp4", kNormal, CreateVideoDecodersCB(), prepend_audio_decoders_cb));
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlaybackXHE_AAC)
{
    ...
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source, kNormal, prepend_audio_decoders_cb));
    source.EndOfStream();
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
    Pause();
...
{
    ASSERT_EQ(PIPELINE_OK, Start("bear.webm", kNormal, base::BindRepeating(&CreateFailingVideoDecoder)));
...
TEST_F(PipelineIntegrationTest, MSE_ConfigChange_MP4)
{
    ...
    EXPECT_EQ(kAppendTimeMs + k1280IsoFileDurationMsAV, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_ConfigChange_Encrypted_MP4_CENC_VideoOnly)
{
    ...
    EXPECT_EQ(kAppendTimeMs + k1280IsoFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());

    source.Shutdown();
    Stop();
}

TEST_F(PipelineIntegrationTest, MSE_ConfigChange_Encrypted_MP4_CENC_KeyRotation_VideoOnly)
{
    TestMediaSource source("bear-640x360-v_frag-cenc-key_rotation.mp4", kAppendWholeFile);
...
TEST_F(PipelineIntegrationTest, MSE_ConfigChange_EncryptedThenClear_MP4_CENC)
{
    ...
    EXPECT_EQ(k640IsoCencFileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
{
    TestMediaSource source("bear-320x240-av_enc-av_clear-1s.webm", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new KeyProvidingApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
{
    TestMediaSource source("bear-320x240-av_enc-av_clear-all.webm", kAppendWholeFile);
...
TEST_F(PipelineIntegrationTest, MSE_EncryptedPlayback_NoEncryptedFrames_MP4_CENC_VideoOnly)
{
    TestMediaSource source("bear-1280x720-v_frag-cenc_clear-all.mp4", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new NoResponseApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
{
    TestMediaSource source("bear-audio-mp4a.6B.ts", "video/mp2t; codecs=\"mp4a.6B\"", kAppendWholeFile);
#if BUILDFLAG(ENABLE_MSE_MPEG2TS_STREAM_PARSER)
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source));
    source.EndOfStream();
    ASSERT_EQ(PIPELINE_OK, pipeline_status_);
...
{
    TestMediaSource source("bear-audio-mp4a.69.ts", "video/mp2t; codecs=\"mp4a.69\"", kAppendWholeFile);
#if BUILDFLAG(ENABLE_MSE_MPEG2TS_STREAM_PARSER)
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithMediaSource(&source));
    source.EndOfStream();
    ASSERT_EQ(PIPELINE_OK, pipeline_status_);
...
TEST_F(PipelineIntegrationTest, MSE_EncryptedPlayback_NoEncryptedFrames_MP4_CENC_AudioOnly)
{
    TestMediaSource source("bear-1280x720-a_frag-cenc_clear-all.mp4", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new NoResponseApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
TEST_F(PipelineIntegrationTest, MSE_EncryptedPlayback_MP4_CENC_SENC_NO_SAIZ_SAIO_Video)
{
    TestMediaSource source("bear-640x360-v_frag-cenc-senc-no-saiz-saio.mp4", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new KeyProvidingApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
TEST_F(PipelineIntegrationTest, MSE_EncryptedPlayback_MP4_CENC_KeyRotation_Video)
{
    TestMediaSource source("bear-1280x720-v_frag-cenc-key_rotation.mp4", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new RotatingKeyProvidingApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_VideoOnly_MP4_AVC3)
{
    ...
    EXPECT_EQ(1u, pipeline_->GetBufferedTimeRanges().size());
    EXPECT_EQ(0, pipeline_->GetBufferedTimeRanges().start(0).InMilliseconds());
    EXPECT_EQ(k1280IsoAVC3FileDurationMs, pipeline_->GetBufferedTimeRanges().end(0).InMilliseconds());
...
TEST_F(PipelineIntegrationTest, MSE_EncryptedPlayback_MP4_CENC_KeyRotation_Audio)
{
    TestMediaSource source("bear-1280x720-a_frag-cenc-key_rotation.mp4", kAppendWholeFile);
    FakeEncryptedMedia encrypted_media(new RotatingKeyProvidingApp());
    EXPECT_EQ(PIPELINE_OK, StartPipelineWithEncryptedMedia(&source, &encrypted_media));
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_VideoOnly_MP4_HEVC)
{
    ...
    TestMediaSource source("bear-320x240-v_frag-hevc.mp4", kMp4HevcVideoOnly, kAppendWholeFile);
...
TEST_F(PipelineIntegrationTest, MSE_BasicPlayback_VideoOnly_MP4_HEV1)
{
    ...
    TestMediaSource source("bear-320x240-v_frag-hevc.mp4", kMp4Hev1VideoOnly, kAppendWholeFile);
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    PipelineStatus status = StartPipelineWithMediaSource(&source);
    EXPECT_TRUE(status == PIPELINE_OK || status == DECODER_ERROR_NOT_SUPPORTED);
#else
...
{
    ASSERT_TRUE(TestSeekDuringRead("bear-320x240-audio-only.webm", 16384, base::Milliseconds(464), base::Milliseconds(617), 0x10CA, 19730));
}

// Verify video decoder & renderer can handle aborted demuxer reads.
TEST_F(PipelineIntegrationTest, MSE_ChunkDemuxerAbortRead_VideoOnly)
{
    ASSERT_TRUE(TestSeekDuringRead("bear-320x240-video-only.webm", 32768, base::Milliseconds(167), base::Milliseconds(1668), 0x1C896, 65536));
}

TEST_F(PipelineIntegrationTest, BasicPlayback_AudioOnly_Opus_4ch_ChannelMapping2_WebM)
{
    ASSERT_EQ(PIPELINE_OK, Start("bear-opus-end-trimming-4ch-channelmapping2.webm", kWebAudio));
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
}
...
{
    ASSERT_EQ(PIPELINE_OK, Start("bear-opus-end-trimming-11ch-channelmapping2.webm", kWebAudio));
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
}
...
TEST_F(PipelineIntegrationTest, BasicPlayback_Opus441kHz)
{
    ...
    EXPECT_EQ(48000, demuxer_->GetFirstStream(DemuxerStream::AUDIO)->audio_decoder_config().samples_per_second());
}
...
{
    ASSERT_EQ(PIPELINE_OK, Start("sync2-trimmed.mp4", kHashed | kUnreliableDuration));
    Play();
    ASSERT_TRUE(WaitUntilOnEnded());
...
TEST_F(PipelineIntegrationTest, Rotated_Metadata_0)
{
    ...
    ASSERT_EQ(VIDEO_ROTATION_0, metadata_.video_decoder_config.video_transformation().rotation);
}

TEST_F(PipelineIntegrationTest, Rotated_Metadata_90)
{
    ASSERT_EQ(PIPELINE_OK, Start("bear_rotate_90.mp4"));
    ASSERT_EQ(VIDEO_ROTATION_90, metadata_.video_decoder_config.video_transformation().rotation);
}

TEST_F(PipelineIntegrationTest, Rotated_Metadata_180)
{
    ASSERT_EQ(PIPELINE_OK, Start("bear_rotate_180.mp4"));
    ASSERT_EQ(VIDEO_ROTATION_180, metadata_.video_decoder_config.video_transformation().rotation);
}

TEST_F(PipelineIntegrationTest, Rotated_Metadata_270)
{
    ASSERT_EQ(PIPELINE_OK, Start("bear_rotate_270.mp4"));
    ASSERT_EQ(VIDEO_ROTATION_270, metadata_.video_decoder_config.video_transformation().rotation);
}
