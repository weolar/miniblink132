...
void SplitMime(const std::string& mimetype, std::string* type, std::string* codecs)
{
    DCHECK(type);
    DCHECK(codecs);
    ...
    *codecs = mimetype.substr(codecs_param_start, codecs_param_end - codecs_param_start);
}
...
TestMediaSource::TestMediaSource(const std::string& filename, const std::string& mimetype, size_t initial_append_size, bool initial_sequence_mode)
    : current_position_(0)
    , initial_append_size_(initial_append_size)
    , initial_sequence_mode_(initial_sequence_mode)
    , mimetype_(mimetype)
    , chunk_demuxer_(new ChunkDemuxer(base::BindOnce(&TestMediaSource::DemuxerOpened, base::Unretained(this)), base::DoNothing(),
          base::BindRepeating(&TestMediaSource::OnEncryptedMediaInitData, base::Unretained(this)), &media_log_))
...
TestMediaSource::TestMediaSource(const std::string& filename, size_t initial_append_size, bool initial_sequence_mode)
    : TestMediaSource(filename, GetMimeTypeForFile(filename), initial_append_size, initial_sequence_mode)
{
}

TestMediaSource::TestMediaSource(scoped_refptr<DecoderBuffer> data, const std::string& mimetype, size_t initial_append_size, bool initial_sequence_mode)
    : file_data_(data)
    , current_position_(0)
    , initial_append_size_(initial_append_size)
    , initial_sequence_mode_(initial_sequence_mode)
...
void TestMediaSource::SetAppendWindow(base::TimeDelta timestamp_offset, base::TimeDelta append_window_start, base::TimeDelta append_window_end)
{
...
void TestMediaSource::Seek(base::TimeDelta seek_time, size_t new_position, size_t seek_append_size)
{
    chunk_demuxer_->StartWaitingForSeek(seek_time);

    chunk_demuxer_->ResetParserState(kSourceId, base::TimeDelta(), kInfiniteDuration, &last_timestamp_offset_);
...
void TestMediaSource::AppendData(size_t size)
{
    ...
    ASSERT_TRUE(chunk_demuxer_->AppendToParseBuffer(kSourceId, file_data_->AsSpan().subspan(current_position_, size)));
    ...
    while (success && has_more_data) {
        StreamParser::ParseStatus parse_result
            = chunk_demuxer_->RunSegmentParserLoop(kSourceId, append_window_start_, append_window_end_, &last_timestamp_offset_);
...
bool TestMediaSource::AppendAtTime(base::TimeDelta timestamp_offset, base::span<const uint8_t> data)
{
    CHECK(!chunk_demuxer_->IsParsingMediaSegment(kSourceId));
    ...
    bool has_more_data = true;
    while (success && has_more_data) {
        StreamParser::ParseStatus parse_result = chunk_demuxer_->RunSegmentParserLoop(kSourceId, append_window_start_, append_window_end_, &timestamp_offset);
        success = parse_result != StreamParser::ParseStatus::kFailed;
        has_more_data = parse_result == StreamParser::ParseStatus::kSuccessHasMoreData;
    }
...
void TestMediaSource::AppendAtTimeWithWindow(
    base::TimeDelta timestamp_offset, base::TimeDelta append_window_start, base::TimeDelta append_window_end, base::span<const uint8_t> data)
{
    CHECK(!chunk_demuxer_->IsParsingMediaSegment(kSourceId));
    ...
    bool has_more_data = true;
    while (success && has_more_data) {
        StreamParser::ParseStatus parse_result = chunk_demuxer_->RunSegmentParserLoop(kSourceId, append_window_start, append_window_end, &timestamp_offset);
        success = parse_result != StreamParser::ParseStatus::kFailed;
        has_more_data = parse_result == StreamParser::ParseStatus::kSuccessHasMoreData;
    }
...
bool TestMediaSource::EvictCodedFrames(base::TimeDelta currentMediaTime, size_t newDataSize)
{
    return chunk_demuxer_->EvictCodedFrames(kSourceId, currentMediaTime, newDataSize);
}
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&TestMediaSource::DemuxerOpenedTask, base::Unretained(this)));
}
...
void TestMediaSource::DemuxerOpenedTask()
{
    ...
    chunk_demuxer_->SetTracksWatcher(kSourceId, base::BindRepeating(&TestMediaSource::InitSegmentReceived, base::Unretained(this)));

    chunk_demuxer_->SetParseWarningCallback(kSourceId, base::BindRepeating(&TestMediaSource::OnParseWarningMock, base::Unretained(this)));
...
void TestMediaSource::OnEncryptedMediaInitData(EmeInitDataType init_data_type, const std::vector<uint8_t>& init_data)
{
...
void TestMediaSource::VerifyExpectedAppendResult(bool append_result)
{
    ...
    ASSERT_EQ(expected_append_result_ == ExpectedAppendResult::kSuccess, append_result);
}
