...
{
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalInteger(ResolvedSourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseDecimalInteger) << from.ToString();
    };

    const auto ok_test = [](std::string_view input, types::DecimalInteger expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalInteger(ResolvedSourceString::CreateForTesting(input));
...
{
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseDecimalFloatingPoint) << from.ToString();
    };

    const auto ok_test = [](std::string_view input, types::DecimalFloatingPoint expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
...
{
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseSignedDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseSignedDecimalFloatingPoint) << from.ToString();
    };

    const auto ok_test = [](std::string_view input, types::SignedDecimalFloatingPoint expected, const base::Location& from = base::Location::Current()) {
        auto result = types::ParseSignedDecimalFloatingPoint(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
        EXPECT_DOUBLE_EQ(value, expected) << from.ToString();
...
{
    using Items = std::initializer_list<std::pair<std::string_view, std::string_view>>;

    auto run_test = [](auto str, Items items, ParseStatusCode error, const auto& from) {
    ...
    auto error_test = [=](auto str, Items items, const base::Location& from = base::Location::Current()) {
        run_test(str, items, ParseStatusCode::kMalformedAttributeList, from);
    };

    // Checks for valid items, followed by EOF
    auto ok_test
        = [=](auto str, Items items, const base::Location& from = base::Location::Current()) { run_test(str, items, ParseStatusCode::kReachedEOF, from); };
    ...
    ok_test(R"(-HELLO-="",WORLD=.21-34,)", { { "-HELLO-", "\"\"" }, { "WORLD", ".21-34" } });
    ok_test(R"(1HELLO=".zxc09,1,23%${}",2WORLD=3)", { { "1HELLO", "\".zxc09,1,23%${}\"" }, { "2WORLD", "3" } });
...
{
    auto make_iter = [](auto str) { return types::AttributeListIterator(SourceString::CreateForTesting(str)); };
    ...
    // Test that the attribute map handles duplicate entries
    {
        ...
        auto result = run_fill(storage, &iter);
        EXPECT_FALSE(result.has_value());
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kAttributeListHasDuplicateNames);
    ...
    // Test that the attribute map forwards errors to the caller
    {
        ...
        auto result = run_fill(storage, &iter);
        EXPECT_FALSE(result.has_value());
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kMalformedAttributeList);
...
{
    const auto ok_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::VariableName::Parse(SourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
    ...
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::VariableName::Parse(SourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kMalformedVariableName) << from.ToString();
    };
...
{
    const auto ok_test = [](std::string_view in, bool allow_empty, std::string_view expected_out, const base::Location& from = base::Location::Current()) {
    ...
    const auto error_test = [](std::string_view in, bool allow_empty, const base::Location& from = base::Location::Current()) {
        auto in_str = SourceString::CreateForTesting(in);
        auto out = types::ParseQuotedStringWithoutSubstitution(in_str, allow_empty);
        ASSERT_FALSE(out.has_value()) << from.ToString();
        EXPECT_EQ(std::move(out).error().code(), ParseStatusCode::kFailedToParseQuotedString) << from.ToString();
    };
...
TEST(HlsTypesTest, ParseQuotedString)
{
    ...
    const auto ok_test = [&dict](std::string_view in, bool allow_empty, std::string_view expected_out, const base::Location& from = base::Location::Current()) {
        auto in_str = SourceString::CreateForTesting(in);
    ...
    const auto error_test
        = [&dict](std::string_view in, bool allow_empty, ParseStatusCode expected_error, const base::Location& from = base::Location::Current()) {
              auto in_str = SourceString::CreateForTesting(in);
              VariableDictionary::SubstitutionBuffer sub_buffer;
              auto out = types::ParseQuotedString(in_str, dict, sub_buffer, allow_empty);
              ASSERT_FALSE(out.has_value()) << from.ToString();
    ...
    ok_test("\"\"", true, "");
    error_test("\"{$EMPTY}\"", false, ParseStatusCode::kFailedToParseQuotedString);
    ok_test("\"{$EMPTY}\"", true, "");
    ...
    error_test("''", false, ParseStatusCode::kFailedToParseQuotedString);
    error_test("' '", false, ParseStatusCode::kFailedToParseQuotedString);
    error_test("'Hello, world!'", false, ParseStatusCode::kFailedToParseQuotedString);
...
{
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::DecimalResolution::Parse(ResolvedSourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseDecimalResolution) << from.ToString();
    };

    const auto ok_test = [](std::string_view input, types::DecimalResolution expected, const base::Location& from = base::Location::Current()) {
        auto result = types::DecimalResolution::Parse(ResolvedSourceString::CreateForTesting(input));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
    ...
    // Test some valid inputs
    ok_test("00000000000000000001x456", types::DecimalResolution { .width = 1, .height = 456 });
    ...
    // Test max supported value
    ok_test("18446744073709551615x18446744073709551615", types::DecimalResolution { .width = 18446744073709551615u, .height = 18446744073709551615u });
...
{
    const auto error_test = [](std::string_view input, const base::Location& from = base::Location::Current()) {
        auto result = types::parsing::ByteRangeExpression::Parse(ResolvedSourceString::CreateForTesting(input));
        ASSERT_FALSE(result.has_value());
        auto error = std::move(result).error();
        EXPECT_EQ(error.code(), ParseStatusCode::kFailedToParseByteRange) << from.ToString();
    };
    const auto ok_test = [](std::string_view input, types::parsing::ByteRangeExpression expected, const base::Location& from = base::Location::Current()) {
        auto result = types::parsing::ByteRangeExpression::Parse(ResolvedSourceString::CreateForTesting(input));
    ...
    // Test some valid inputs
    ok_test("0", types::parsing::ByteRangeExpression { .length = 0, .offset = std::nullopt });
    ok_test("12", types::parsing::ByteRangeExpression { .length = 12, .offset = std::nullopt });
    ok_test("12@0", types::parsing::ByteRangeExpression { .length = 12, .offset = 0 });
    ok_test("12@34", types::parsing::ByteRangeExpression { .length = 12, .offset = 34 });
    ok_test("0@34", types::parsing::ByteRangeExpression { .length = 0, .offset = 34 });
    ...
    ok_test("18446744073709551615@0", types::parsing::ByteRangeExpression { .length = 18446744073709551615u, .offset = 0 });
    error_test("18446744073709551616@0");
    ok_test("0@18446744073709551615", types::parsing::ByteRangeExpression { .length = 0, .offset = 18446744073709551615u });
    error_test("0@18446744073709551616");
    ok_test(
        "18446744073709551615@18446744073709551615", types::parsing::ByteRangeExpression { .length = 18446744073709551615u, .offset = 18446744073709551615u });
...
TEST(HlsTypesTest, ValidateByteRange)
{
    ...
    constexpr auto ok_test = [](types::DecimalInteger length, types::DecimalInteger offset, const base::Location& from = base::Location::Current()) {
        const auto result = types::ByteRange::Validate(length, offset);
        EXPECT_TRUE(result.has_value()) << from.ToString();
    };
    constexpr auto error_test = [](types::DecimalInteger length, types::DecimalInteger offset, const base::Location& from = base::Location::Current()) {
        const auto result = types::ByteRange::Validate(length, offset);
...
{
    constexpr auto ok_test = [](std::string_view x, const base::Location& from = base::Location::Current()) {
        auto result = types::StableId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
        EXPECT_EQ(value.Str(), x);
    };
    constexpr auto error_test = [](std::string_view x, const base::Location& from = base::Location::Current()) {
        auto result = types::StableId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kFailedToParseStableId) << from.ToString();
    };
...
{
    constexpr auto ok_test = [](std::string_view x, types::InstreamId::Type type, uint8_t number, const base::Location& from = base::Location::Current()) {
        auto result = types::InstreamId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        auto value = std::move(result).value();
    ...
    constexpr auto error_test = [](std::string_view x, const base::Location& from = base::Location::Current()) {
        auto result = types::InstreamId::Parse(ResolvedSourceString::CreateForTesting(x));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kFailedToParseInstreamId) << from.ToString();
    };
...
{
    constexpr auto ok_test = [](std::string_view str, types::DecimalInteger max_channels, const std::initializer_list<std::string>& audio_coding_identifiers,
                                 const base::Location& from = base::Location::Current()) {
        auto result = types::AudioChannels::Parse(ResolvedSourceString::CreateForTesting(str));
        ASSERT_TRUE(result.has_value()) << from.ToString();
        const auto value = std::move(result).value();
        EXPECT_EQ(value.GetMaxChannels(), max_channels) << from.ToString();
        EXPECT_TRUE(base::ranges::equal(value.GetAudioCodingIdentifiers(), audio_coding_identifiers)) << from.ToString();
    };
    constexpr auto error_test = [](std::string_view str, const base::Location& from = base::Location::Current()) {
        auto result = types::AudioChannels::Parse(ResolvedSourceString::CreateForTesting(str));
        ASSERT_FALSE(result.has_value()) << from.ToString();
        EXPECT_EQ(std::move(result).error().code(), ParseStatusCode::kFailedToParseAudioChannels) << from.ToString();
    };
...
void HexErrorTest(std::string_view str, bool extrapolate_leading_zeros = false, bool has_prefix = true, const base::Location& from = base::Location::Current())
{
    auto result = types::parsing::HexRepr<bits>::Parse(ResolvedSourceString::CreateForTesting(str), extrapolate_leading_zeros, has_prefix);
    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(std::move(result).error().code(), ParseStatusCode::kFailedToParseHexadecimalString) << from.ToString();
}

template <size_t bits>
types::parsing::HexRepr<bits>::Container HexParseOk(
    std::string_view str, bool extrapolate_leading_zeros = false, bool has_prefix = true, const base::Location& from = base::Location::Current())
{
    auto result = types::parsing::HexRepr<bits>::Parse(ResolvedSourceString::CreateForTesting(str), extrapolate_leading_zeros, has_prefix);
    EXPECT_TRUE(result.has_value()) << from.ToString();
...
TEST(HlsTypesTest, HexUnpack)
{
    ...
    std::tuple<uint8_t, uint8_t, uint8_t> expect24 = std::make_tuple(0x12, 0x34, 0x56);
    ASSERT_EQ(value24, expect24);

    std::tuple<uint16_t, uint16_t, uint16_t> value48 = HexParseOk<48>("0x123456", true);
    std::tuple<uint16_t, uint16_t, uint16_t> expect48 = std::make_tuple<uint16_t, uint16_t, uint16_t>(0x0000, 0x0012, 0x3456);
