...
public:
    ...
    const std::optional<GURL>& GetUri() const
    {
        return uri_;
    }
...
class MEDIA_EXPORT AudioRenditionGroup : public base::RefCounted<AudioRenditionGroup> {
public:
    REQUIRE_ADOPTION_FOR_REFCOUNTED_TYPE();

    explicit AudioRenditionGroup(base::PassKey<MultivariantPlaylist>, std::string id);
    ...
    // will be returned.
    ParseStatus::Or<absl::monostate> AddRendition(base::PassKey<MultivariantPlaylist>, XMediaTag tag, const GURL& playlist_uri);
    ...
    const std::list<AudioRendition>& GetRenditions() const
    {
        return renditions_;
    }
...
private:
    ...
    base::flat_map<std::string, raw_ptr<const AudioRendition, CtnExperimental>> renditions_map_;
