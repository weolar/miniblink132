...
template <typename T, typename... Args> std::optional<ParseStatus> ParseUniqueTag(TagItem tag, std::optional<T>& out, Args&&... args)
{
...
ParseStatus::Or<GURL> ParseUri(UriItem item, const GURL& playlist_uri, const CommonParserState& state, VariableDictionary::SubstitutionBuffer& buffer);
