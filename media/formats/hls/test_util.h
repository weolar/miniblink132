...
{
    return types::VariableName::Parse(SourceString::CreateForTesting(name)).value();
}

inline types::ByteRange CreateByteRange(types::DecimalInteger length, types::DecimalInteger offset)
{
...
inline testing::AssertionResult RoughlyEqual(base::TimeDelta lhs, base::TimeDelta rhs, base::TimeDelta epsilon = base::Milliseconds(1))
{
    if (lhs + epsilon >= rhs && lhs - epsilon <= rhs) {
        return testing::AssertionSuccess();
    }

    return testing::AssertionFailure() << lhs << " != " << rhs << " +- " << epsilon;
}

inline testing::AssertionResult RoughlyEqual(
    std::optional<base::TimeDelta> lhs, std::optional<base::TimeDelta> rhs, base::TimeDelta epsilon = base::Milliseconds(1))
{
