...
template <typename A, typename B> constexpr bool are_disjoint()
{
    return ToTagName(A::kMaxValue) < ToTagName(B::kMinValue) || ToTagName(B::kMaxValue) < ToTagName(A::kMinValue);
}

// Ensure all tag names are within the range [kMinValue, kMaxValue]
static_assert(ToTagName(CommonTagName::kMinValue) >= kMinTagName && ToTagName(CommonTagName::kMaxValue) <= kMaxTagName);
static_assert(ToTagName(MultivariantPlaylistTagName::kMinValue) >= kMinTagName && ToTagName(MultivariantPlaylistTagName::kMaxValue) <= kMaxTagName);
static_assert(ToTagName(MediaPlaylistTagName::kMinValue) >= kMinTagName && ToTagName(MediaPlaylistTagName::kMaxValue) <= kMaxTagName);

// Ensure tag name sets are contiguous
static_assert(ToTagName(MultivariantPlaylistTagName::kMinValue) == ToTagName(CommonTagName::kMaxValue) + 1);
static_assert(ToTagName(MediaPlaylistTagName::kMinValue) == ToTagName(MultivariantPlaylistTagName::kMaxValue) + 1);
...
static_assert(are_disjoint<MultivariantPlaylistTagName, MediaPlaylistTagName>());

template <typename T> constexpr std::pair<std::string_view, TagName> TagNameEntry(std::string_view name_str, T name)
{
...
constexpr auto kTagNames = base::MakeFixedFlatMap({
    ...
    TagNameEntry("EXT-X-CONTENT-STEERING", MultivariantPlaylistTagName::kXContentSteering),
    ...
    TagNameEntry("EXT-X-DISCONTINUITY-SEQUENCE", MediaPlaylistTagName::kXDiscontinuitySequence),
    TagNameEntry("EXT-X-ENDLIST", MediaPlaylistTagName::kXEndList),
    TagNameEntry("EXT-X-GAP", MediaPlaylistTagName::kXGap),
    TagNameEntry("EXT-X-I-FRAME-STREAM-INF", MultivariantPlaylistTagName::kXIFrameStreamInf),
    TagNameEntry("EXT-X-I-FRAMES-ONLY", MediaPlaylistTagName::kXIFramesOnly),
    TagNameEntry("EXT-X-INDEPENDENT-SEGMENTS", CommonTagName::kXIndependentSegments),
    ...
    TagNameEntry("EXT-X-PROGRAM-DATE-TIME", MediaPlaylistTagName::kXProgramDateTime),
    TagNameEntry("EXT-X-RENDITION-REPORT", MediaPlaylistTagName::kXRenditionReport),
    TagNameEntry("EXT-X-SERVER-CONTROL", MediaPlaylistTagName::kXServerControl),
    TagNameEntry("EXT-X-SESSION-DATA", MultivariantPlaylistTagName::kXSessionData),
    TagNameEntry("EXT-X-SESSION-KEY", MultivariantPlaylistTagName::kXSessionKey),
    ...
    TagNameEntry("EXT-X-TARGETDURATION", MediaPlaylistTagName::kXTargetDuration),
...
static_assert(kTagNames.size() == static_cast<size_t>(kMaxTagName - kMinTagName + 1));
