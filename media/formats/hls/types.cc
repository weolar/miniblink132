...
{
    return ParseDecimalFloatingPoint(str).MapValue([](DecimalFloatingPoint t) -> ParseStatus::Or<base::TimeDelta> {
...
// static
ParseStatus::Or<ByteRangeExpression> ByteRangeExpression::Parse(ResolvedSourceString source_str)
{
    ...
    if (!length.has_value()) {
        return ParseStatus(ParseStatusCode::kFailedToParseByteRange).AddCause(std::move(length).error());
    }
    ...
    if (at_index != std::string_view::npos) {
        ...
        if (!offset_result.has_value()) {
            return ParseStatus(ParseStatusCode::kFailedToParseByteRange).AddCause(std::move(offset_result).error());
        }

        offset = std::move(offset_result).value();
    }

    return ByteRangeExpression { .length = std::move(length).value(), .offset = offset };
}
...
std::optional<SourceString> ExtractAttributeName(SourceString* source_str)
{
    ...
    const auto is_char_valid = [](char c) -> bool { return base::IsAsciiUpper(c) || base::IsAsciiDigit(c) || c == '-'; };
...
ParseStatus::Or<DecimalInteger> ParseDecimalInteger(ResolvedSourceString source_str)
{
...
ParseStatus::Or<DecimalFloatingPoint> ParseDecimalFloatingPoint(ResolvedSourceString source_str)
{
...
ParseStatus::Or<SignedDecimalFloatingPoint> ParseSignedDecimalFloatingPoint(ResolvedSourceString source_str)
{
    // Accept no decimal point, decimal point with leading digits, trailing
    // digits, or both
    static const base::NoDestructor<re2::RE2> decimal_floating_point_regex("-?(\\d+|\\d+\\.|\\.\\d+|\\d+\\.\\d+)");
...
// static
ParseStatus::Or<DecimalResolution> DecimalResolution::Parse(ResolvedSourceString source_str)
{
    ...
    for (auto* x : { &width, &height }) {
        if (!x->has_value()) {
            return ParseStatus(ParseStatusCode::kFailedToParseDecimalResolution).AddCause(std::move(*x).error());
        }
    }

    return DecimalResolution { .width = std::move(width).value(), .height = std::move(height).value() };
}

// static
std::optional<ByteRange> ByteRange::Validate(DecimalInteger length, DecimalInteger offset)
{
    if (length == 0) {
        return std::nullopt;
    }
...
ParseStatus::Or<ResolvedSourceString> ParseQuotedString(
    SourceString source_str, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer, bool allow_empty)
{
    return ParseQuotedStringWithoutSubstitution(source_str, allow_empty)
        .MapValue([&variable_dict, &sub_buffer](auto str) { return variable_dict.Resolve(str, sub_buffer); })
        .MapValue([allow_empty](auto str) -> ParseStatus::Or<ResolvedSourceString> {
...
ParseStatus::Or<SourceString> ParseQuotedStringWithoutSubstitution(SourceString source_str, bool allow_empty)
{
    ...
    : items_(sorted_items)
{
    ...
    DCHECK(base::ranges::is_sorted(items_, std::less(), &AttributeMap::Item::first));
}

ParseStatus::Or<AttributeListIterator::Item> AttributeMap::Fill(AttributeListIterator* iter)
{
    while (true) {
        ...
        // value.
        auto entry = base::ranges::lower_bound(items_, item.name.Str(), std::less(), &AttributeMap::Item::first);
        if (entry == items_.end()) {
            return item;
        }
...
{
    static const base::NoDestructor<re2::RE2> variable_name_regex("[a-zA-Z0-9_-]+");
...
{
    const auto is_char_valid = [](char c) -> bool { return base::IsAsciiAlphaNumeric(c) || IsOneOf(c, "+/=.-_"); };
...
AudioChannels::AudioChannels(DecimalInteger max_channels, std::vector<std::string> audio_coding_identifiers)
...
ParseStatus::Or<AudioChannels> AudioChannels::Parse(ResolvedSourceString str)
{
    ...
    if (!max_channels_result.has_value()) {
        return ParseStatus(ParseStatusCode::kFailedToParseAudioChannels).AddCause(std::move(max_channels_result).error());
    }
    ...
    while (!audio_coding_identifiers_str.Empty()) {
        ...
        constexpr auto is_valid_coding_identifier_char = [](char c) -> bool { return base::IsAsciiUpper(c) || base::IsAsciiDigit(c) || c == '-'; };

        // Each string must be non-empty and consist only of the allowed characters
        if (identifier.Empty() || !base::ranges::all_of(identifier.Str(), is_valid_coding_identifier_char)) {
...
DecimalInteger DecimalResolution::Szudzik() const
{
    ...
    return (width >= height) ? (width * width + width + height) : (height * height + width);
}
