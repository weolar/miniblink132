...
template <size_t count, typename impl, typename... types> struct repeat_t {
    using type = repeat_t<count - 1, impl, impl, types...>::type;
};

template <typename impl, typename... types> struct repeat_t<0, impl, types...> {
...
template <typename Subtype, typename T, typename... ParseArgs> struct MEDIA_EXPORT SubstitutingParser {
    using ParseInto = T;

    static ParseStatus::Or<ParseInto> ParseWithSubstitution(
        SourceString str, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer, ParseArgs&&... args)
    {
        return variable_dict.Resolve(str, sub_buffer).MapValue([... args = std::forward<ParseArgs>(args)](ResolvedSourceString str) {
            return Subtype::Parse(str, std::forward<ParseArgs>(args)...);
        });
    }

    static ParseStatus::Or<ParseInto> ParseWithoutSubstitution(SourceString str, ParseArgs&&... args)
    {
        return Subtype::Parse(str.SkipVariableSubstitution(), std::forward<ParseArgs>(args)...);
    }
};
...
template <typename T> struct MEDIA_EXPORT Quoted : public SubstitutingParser<Quoted<T>, typename T::ParseInto> {
    static ParseStatus::Or<typename T::ParseInto> Parse(ResolvedSourceString str, bool allow_empty = false)
    {
        if (str.Size() < 2) {
...
struct MEDIA_EXPORT ByteRangeExpression : public SubstitutingParser<ByteRangeExpression, ByteRangeExpression> {
    static ParseStatus::Or<ByteRangeExpression> Parse(ResolvedSourceString source_str);
...
// of HexRepr::Parse.
template <int64_t bits> struct HexPackInfo {
    ...
    using Unit = std::conditional_t<(UnitSize == 64), uint64_t,
        std::conditional_t<(UnitSize == 32), uint32_t, std::conditional_t<(UnitSize == 16), uint16_t, uint8_t>>>;
...
template <int64_t bits> struct MEDIA_EXPORT HexRepr : public SubstitutingParser<HexRepr<bits>, typename HexPackInfo<bits>::Container> {
    ...
    template <size_t reps> struct ParserImpl {
        static ParseStatus::Or<typename repeat_t<reps, Unit>::type> Parse(ResolvedSourceString str, bool extrapolate_leading_zeros)
        {
            if constexpr (reps == 0) {
                if (str.Size() != 0) {
    ...
    static ParseStatus::Or<Container> Parse(ResolvedSourceString str, bool extrapolate_leading_zeros = false, bool has_prefix = true)
    {
...
MEDIA_EXPORT ParseStatus::Or<DecimalInteger> ParseDecimalInteger(ResolvedSourceString source_str);
...
MEDIA_EXPORT ParseStatus::Or<DecimalFloatingPoint> ParseDecimalFloatingPoint(ResolvedSourceString source_str);
...
MEDIA_EXPORT ParseStatus::Or<SignedDecimalFloatingPoint> ParseSignedDecimalFloatingPoint(ResolvedSourceString source_str);
...
struct MEDIA_EXPORT DecimalResolution {
    static ParseStatus::Or<DecimalResolution> Parse(ResolvedSourceString source_str);

    types::DecimalInteger width;
    types::DecimalInteger height;

    types::DecimalInteger Area() const
    {
        return width * height;
    }
...
public:
    ...
    // `DecimalInteger`.
    static std::optional<ByteRange> Validate(DecimalInteger length, DecimalInteger offset);

    DecimalInteger GetLength() const
    {
        return length_;
    }
    DecimalInteger GetOffset() const
    {
        return offset_;
    }
    DecimalInteger GetEnd() const
    {
...
MEDIA_EXPORT ParseStatus::Or<ResolvedSourceString> ParseQuotedString(
    SourceString source_str, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer, bool allow_empty = false);
...
MEDIA_EXPORT ParseStatus::Or<SourceString> ParseQuotedStringWithoutSubstitution(SourceString source_str, bool allow_empty = false);
...
struct MEDIA_EXPORT AttributeMap {
    ...
    ParseStatus::Or<AttributeListIterator::Item> Fill(AttributeListIterator* iter);
    ...
    template <typename... T> static constexpr std::array<Item, sizeof...(T)> MakeStorage(T... keys)
    {
...
private:
    explicit VariableName(std::string_view name)
        : name_(name)
    {
    }
...
private:
    explicit StableId(std::string id)
        : id_(std::move(id))
    {
    }
...
private:
    InstreamId(Type type, uint8_t number)
        : type_(type)
        , number_(number)
    {
    }
...
public:
    ...
    DecimalInteger GetMaxChannels() const
    {
...
private:
    AudioChannels(DecimalInteger max_channels, std::vector<std::string> audio_coding_identifiers);
