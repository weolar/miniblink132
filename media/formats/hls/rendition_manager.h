...
public:
    ...
    template <typename T> using SelectableOption = std::tuple<T, std::string>;
    ...
    using IsTypeSupportedCallback = base::RepeatingCallback<CodecSupportType(std::string_view, base::span<const std::string>)>;
    ...
    // selected.
    using SelectedCB = base::RepeatingCallback<void(AdaptationReason, const VariantStream*, const AudioRendition*)>;
    using SelectedCallonce = base::OnceCallback<void(const VariantStream*, const AudioRendition*)>;

    ~RenditionManager();
    RenditionManager(scoped_refptr<MultivariantPlaylist> playlist, SelectedCB on_variant_selected, IsTypeSupportedCallback is_type_supported_cb);
...
private:
    ...
    std::optional<RenditionID> SelectBestRendition(VariantID, std::optional<RenditionID>);
    ...
    std::vector<VariantStream::FormatComponent> DetermineVariantStreamFormatting() const;
    ...
    std::optional<RenditionManager::RenditionID> SelectRenditionBasedOnLanguage(
        const VariantMetadata& variant, std::optional<std::string> maybe_language, bool only_autoselect);
