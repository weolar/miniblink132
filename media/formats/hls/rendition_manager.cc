...
RenditionManager::CodecSupportType VariantTypeSupported(RenditionManager::IsTypeSupportedCallback is_type_supported_cb, const VariantStream& variant)
{
    ...
    RenditionManager::CodecSupportType mp4 = is_type_supported_cb.Run("video/mp4", *codecs);
    if (mp4 != RenditionManager::CodecSupportType::kUnsupported) {
        return mp4;
    }

    RenditionManager::CodecSupportType mp2t = is_type_supported_cb.Run("video/mp2t", *codecs);
    return mp2t;
}

std::string GetVariantDisplayString(RenditionManager::VariantID id, const std::vector<VariantStream::FormatComponent>& components, const VariantStream* variant)
{
...
RenditionManager::UpdatedSelections::UpdatedSelections() = default;
RenditionManager::UpdatedSelections::UpdatedSelections(const UpdatedSelections&) = default;

RenditionManager::VariantMetadata::~VariantMetadata() = default;
RenditionManager::VariantMetadata::VariantMetadata(const VariantMetadata&) = default;
RenditionManager::VariantMetadata::VariantMetadata(const VariantStream* stream, const AudioRenditionGroup* group)
    : track_id("")
    , stream(stream)
    , audio_rendition_group(group)
{
}

RenditionManager::RenditionMetadata::~RenditionMetadata() = default;
RenditionManager::RenditionMetadata::RenditionMetadata(const RenditionMetadata&) = default;
RenditionManager::RenditionMetadata::RenditionMetadata(const AudioRendition* rendition)
    : track_id("")
    , rendition(rendition)
{
}
...
RenditionManager::RenditionManager(scoped_refptr<MultivariantPlaylist> playlist, SelectedCB on_variant_selected, IsTypeSupportedCallback is_type_supported_cb)
...
void RenditionManager::Reselect(SelectedCallonce callback)
{
    ...
    if (!selections.variant.has_value() && !selections.audio_rendition.has_value()) {
    ...
    const VariantStream* selected_variant = selectable_variants_.at(*selected_variant_).stream;
    const AudioRendition* audio_override = nullptr;
    if (selected_audio_rendition_.has_value()) {
        audio_override = selectable_renditions_.at(selected_audio_rendition_.value()).rendition;
...
void RenditionManager::SetPreferredAudioRendition(std::optional<MediaTrack::Id> rendition_id)
{
    ...
    Reselect(base::BindOnce(on_variant_selected_, AdaptationReason::kUserSelection));
}

void RenditionManager::SetPreferredVariant(std::optional<MediaTrack::Id> variant_id)
{
...
void RenditionManager::UpdatePlayerResolution(const gfx::Size& resolution)
{
    ...
    Reselect(base::BindOnce(on_variant_selected_, AdaptationReason::kResolutionChange));
}

void RenditionManager::UpdateNetworkSpeed(uint64_t network_bps)
{
    AdaptationReason reason = network_bps_ > network_bps ? AdaptationReason::kNetworkDowngrade : AdaptationReason::kNetworkUpgrade;
...
void RenditionManager::InitializeVariantMaps(IsTypeSupportedCallback is_type_supported_cb)
{
        ...
        if (auto group = variant.GetAudioRenditionGroup()) {
            ...
            for (auto& rendition : group->GetRenditions()) {
                ...
                if (!rendition_id.has_value()) {
                    ...
                    selectable_renditions_.try_emplace(*rendition_id, RenditionMetadata(&rendition));
                }
    ...
    constexpr auto compare = [](const VariantMetadata& lhs, const VariantMetadata& rhs) {
        ...
        if (lhs.stream->GetScore().has_value() && rhs.stream->GetScore().has_value()) {
    ...
    for (auto& [variant_id, metadata] : selectable_variants_) {
        auto track = MediaTrack::CreateVideoTrack(base::StringPrintf("Video.%d", variant_id.value()), MediaTrack::VideoKind::kMain,
    ...
    for (auto& [rendition_id, metadata] : selectable_renditions_) {
        auto track = MediaTrack::CreateAudioTrack(base::StringPrintf("Audio.%d", rendition_id.value()), MediaTrack::AudioKind::kMain,
            GetAudioRenditionDisplayString(metadata.rendition), metadata.rendition->GetLanguage().value_or(""), selected_audio_rendition_ == rendition_id,
            rendition_id.value(), true);
        metadata.track_id = track.track_id();
        track_map_.try_emplace(metadata.track_id, std::move(track));
    }
}
...
RenditionManager::UpdatedSelections RenditionManager::GetUpdatedSelectionIds()
{
    ...
    selections.audio_rendition = SelectBestRendition(*selections.variant, selections.audio_rendition);

    return selections;
}

std::optional<RenditionManager::VariantID> RenditionManager::SelectBestVariant()
{
    ...
    for (auto& [variant_id, stats] : selectable_variants_) {
        ...
        if (variant_resolution.has_value() && variant_resolution->Area() > player_resolution_.Area64()) {
...
std::optional<RenditionManager::RenditionID> RenditionManager::SelectRenditionBasedOnLanguage(
    const VariantMetadata& variant, std::optional<std::string> language, bool only_autoselect)
{
    ...
    for (const auto id : variant.audio_renditions) {
        if (only_autoselect && !selectable_renditions_.at(id).rendition->MayAutoSelect()) {
            continue;
        }
        const auto& rendition_lang = selectable_renditions_.at(id).rendition->GetLanguage();
...
std::optional<RenditionManager::RenditionID> RenditionManager::SelectBestRendition(VariantID variant_id, std::optional<RenditionID> maybe_rendition)
{
    ...
    if (variant.audio_renditions.find(*maybe_rendition) != variant.audio_renditions.end()) {
    ...
    // a selection.
    return SelectRenditionBasedOnLanguage(variant, rendition_metadata.rendition->GetLanguage(), false);
}

std::optional<RenditionManager::RenditionID> RenditionManager::LookupRendition(const AudioRendition* rendition)
{
...
std::vector<VariantStream::FormatComponent> RenditionManager::DetermineVariantStreamFormatting() const
{
    ...
    if (rates.size() == selectable_variants_.size()) {
        ...
        return { VariantStream::FormatComponent::kResolution, VariantStream::FormatComponent::kFrameRate };
    }
    ...
    if (bandwidths.size() == selectable_variants_.size()) {
        if (missing_resolution || resolutions.size() == 1) {
            ...
            return { VariantStream::FormatComponent::kBandwidth, VariantStream::FormatComponent::kFrameRate };
        }
        if (missing_frame_rate || rates.size() == 1) {
            // Don't include frame rate, but resolution is ok.
            return { VariantStream::FormatComponent::kBandwidth, VariantStream::FormatComponent::kResolution };
        }
        return { VariantStream::FormatComponent::kBandwidth, VariantStream::FormatComponent::kResolution, VariantStream::FormatComponent::kFrameRate };
    }
