...
public:
    ...
    template <typename... T> void AppendLine(std::string_view part1, T&&... rem)
    {
    ...
    // `ExpectOk`.
    template <typename Fn, typename Arg> void ExpectPlaylist(Fn fn, Arg arg, base::Location location = base::Location::Current())
    {
        playlist_expectations_.push_back(
            base::BindRepeating([](Fn fn, Arg arg, const base::Location& from, const PlaylistT& playlist) { fn(arg, from, playlist); }, std::move(fn),
                std::move(arg), std::move(location)));
    }

    template <typename... Args> scoped_refptr<PlaylistT> Parse(Args&&... args, const base::Location& from = base::Location::Current())
    {
        auto result = PlaylistT::Parse(source_, uri_, version_, std::forward<Args>(args)...);

        if (!result.has_value()) {
            EXPECT_TRUE(result.has_value()) << MediaSerializeForTesting(std::move(result).error()) << "\nFrom: " << from.ToString();
            return nullptr;
        } else {
...
protected:
    ...
    // error code.
    template <typename... Args> void ExpectError(ParseStatusCode code, const base::Location& from, Args&&... args) const
    {
        auto result = PlaylistT::Parse(source_, uri_, version_, std::forward<Args>(args)...);
        ASSERT_FALSE(result.has_value()) << from.ToString();

        auto actual_code = std::move(result).error().code();
        EXPECT_EQ(actual_code, code) << "Error: " << ParseStatusCodeToString(actual_code) << "\n"
    ...
    // expectations.
    template <typename... Args> void ExpectOk(const base::Location& from, Args&&... args) const
    {
        auto result = PlaylistT::Parse(source_, uri_, version_, std::forward<Args>(args)...);
        ASSERT_TRUE(result.has_value()) << "Error: " << ParseStatusCodeToString(std::move(result).error().code()) << "\n" << from.ToString();
        auto playlist = std::move(result).value();
...
private:
    virtual void VerifyExpectations(const PlaylistT&, const base::Location& from) const = 0;

    std::vector<base::RepeatingCallback<void(const PlaylistT&)>> playlist_expectations_;
...
// value.
inline void HasIndependentSegments(bool value, const base::Location& from, const Playlist& playlist)
{
