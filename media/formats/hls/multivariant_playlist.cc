...
template <typename T>
T* GetOrCreateRenditionGroup(base::PassKey<MultivariantPlaylist> pass_key, base::flat_map<std::string_view, scoped_refptr<T>>& groups, std::string_view id)
{
    auto iter = groups.find(id);

    // If the group wasn't found, create it.
    if (iter == groups.end()) {
        auto group = base::MakeRefCounted<AudioRenditionGroup>(pass_key, std::string(id));
...
// static
ParseStatus::Or<scoped_refptr<MultivariantPlaylist>> MultivariantPlaylist::Parse(
    std::string_view source, GURL uri, types::DecimalInteger version, TagRecorder* tag_recorder)
{
    DCHECK(version != 0);
    ...
    base::flat_map<std::string_view, scoped_refptr<AudioRenditionGroup>> audio_rendition_groups;
    ...
    while (true) {
        ...
        if (auto* tag = absl::get_if<TagItem>(&item)) {
            ...
            case MultivariantPlaylistTagName::kXMedia: {
                auto result = XMediaTag::Parse(*tag, common_state.variable_dict, sub_buffer);
                if (!result.has_value()) {
                ...
                case MediaType::kAudio: {
                    auto* group = GetOrCreateRenditionGroup({}, audio_rendition_groups, media_tag.group_id.Str());
                    auto rendition_result = group->AddRendition(base::PassKey<MultivariantPlaylist>(), std::move(media_tag), uri);
            ...
            case MultivariantPlaylistTagName::kXStreamInf: {
                auto error = ParseUniqueTag(*tag, inf_tag, common_state.variable_dict, sub_buffer);
                if (error.has_value()) {
                    return std::move(error).value();
                }
                break;
        ...
        auto variant_uri_result = ParseUri(absl::get<UriItem>(std::move(item)), uri, common_state, sub_buffer);
        ...
        if (inf_tag->audio.has_value()) {
            audio_renditions = GetOrCreateRenditionGroup({}, audio_rendition_groups, inf_tag->audio->Str());
        }
        ...
        variants.emplace_back(std::move(variant_uri), inf_tag->bandwidth, inf_tag->average_bandwidth, inf_tag->score, std::move(inf_tag->codecs),
            inf_tag->resolution, inf_tag->frame_rate, std::move(audio_renditions), std::move(video_rendition_group_name));
    ...
    return base::MakeRefCounted<MultivariantPlaylist>(base::PassKey<MultivariantPlaylist>(), std::move(uri), version,
        common_state.independent_segments_tag.has_value(), std::move(variants), std::move(common_state.variable_dict));
}

MultivariantPlaylist::MultivariantPlaylist(base::PassKey<MultivariantPlaylist>, GURL uri, types::DecimalInteger version, bool independent_segments,
    std::vector<VariantStream> variants, VariableDictionary variable_dictionary)
