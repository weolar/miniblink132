...
namespace subtle {
// static
template <typename Self> Self SourceStringBase<Self>::CreateForTesting(std::string_view str)
{
    return Self(1, 1, str);
}

// static
template <typename Self> Self SourceStringBase<Self>::CreateForTesting(size_t line, size_t column, std::string_view str)
{
    return Self(line, column, str);
}

template <typename Self> Self SourceStringBase<Self>::Substr(size_t pos, size_t count) const
{
...
template <typename Self> Self SourceStringBase<Self>::Consume(size_t count)
{
...
template <typename Self> Self SourceStringBase<Self>::ConsumeDelimiter(char c)
{
...
template <typename Self> void SourceStringBase<Self>::TrimStart()
{
...
SourceStringBase<Self>::SourceStringBase(size_t line, size_t column, std::string_view str)
    : line_(line)
    , column_(column)
    , str_(str)
{
...
ResolvedSourceString::ResolvedSourceString(size_t line, size_t column, std::string_view str, SubstitutionState substitution_state)
    : SourceStringBase(line, column, str)
...
{
    return ResolvedSourceString::Create(base::PassKey<SourceString>(), Line(), Column(), Str());
}
