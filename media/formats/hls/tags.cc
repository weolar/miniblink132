...
template <typename T> ParseStatus::Or<T> ParseEmptyTag(TagItem tag)
{
...
template <typename T> ParseStatus::Or<T> ParseDecimalIntegerTag(TagItem tag, types::DecimalInteger T::*field)
{
    DCHECK(tag.GetName() == ToTagName(T::kName));
    if (!tag.GetContent().has_value()) {
        return ParseStatusCode::kMalformedTag;
    ...
    auto value = types::ParseDecimalInteger(tag.GetContent()->SkipVariableSubstitution());
    if (!value.has_value()) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(value).error());
    }
...
template <typename T> ParseStatus::Or<T> ParseISO8601DateTimeTag(TagItem tag, base::Time T::*field)
{
...
constexpr std::string_view GetAttributeName(XServerControlTagAttribute attribute)
{
    switch (attribute) {
...
template <typename T, size_t kLast> constexpr bool IsAttributeEnumSorted(std::index_sequence<kLast>)
{
    return true;
}

template <typename T, size_t kLHS, size_t kRHS, size_t... kRest> constexpr bool IsAttributeEnumSorted(std::index_sequence<kLHS, kRHS, kRest...>)
{
    const auto lhs = GetAttributeName(static_cast<T>(kLHS));
    const auto rhs = GetAttributeName(static_cast<T>(kRHS));
    return lhs < rhs && IsAttributeEnumSorted<T>(std::index_sequence<kRHS, kRest...> {});
}
...
constexpr std::array<types::AttributeMap::Item, sizeof...(Indices)> MakeTypedAttributeMapStorage(std::index_sequence<Indices...> seq)
{
    static_assert(IsAttributeEnumSorted<T>(seq), "Enum keys must be sorted alphabetically");
    return types::AttributeMap::MakeStorage(GetAttributeName(static_cast<T>(Indices))...);
}
...
template <typename T> struct TypedAttributeMap {
    static_assert(std::is_enum<T>::value, "T must be an enum");
    static_assert(std::is_same<decltype(GetAttributeName(std::declval<T>())), std::string_view>::value,
        "GetAttributeName must be overloaded for T to return a "
        "std::string_view");
    static constexpr size_t kNumKeys = static_cast<size_t>(T::kMaxValue) + 1;

    TypedAttributeMap()
        : attributes_(MakeTypedAttributeMapStorage<T>(std::make_index_sequence<kNumKeys>()))
    {
    }
    ...
    size_t Size() const
    {
        return std::count_if(attributes_.begin(), attributes_.end(), [](const types::AttributeMap::Item& item) { return item.second.has_value(); });
    }
...
template <template <typename...> typename SpecifiedContainer, typename Type> struct is_specialization_of : std::false_type { };

template <template <typename...> typename SpecifiedContainer, typename... Types>
struct is_specialization_of<SpecifiedContainer, SpecifiedContainer<Types...>> : std::true_type { };
...
template <typename Result, typename AttrEnum, typename ParseFn, typename... ParseFnArgs>
ParseStatus::Or<Result> ParseField(AttrEnum field_name, TypedAttributeMap<AttrEnum> map, ParseFn parser, ParseFnArgs&&... args)
{
    if (map.HasValue(field_name)) {
        auto maybe = parser(map.GetValue(field_name), std::forward<ParseFnArgs>(args)...);
        if (!maybe.has_value()) {
...
// static
XDefineTag XDefineTag::CreateDefinition(types::VariableName name, std::string_view value)
{
...
ParseStatus::Or<XDefineTag> XDefineTag::Parse(TagItem tag)
{
    ...
    if (result.code() != ParseStatusCode::kReachedEOF) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result));
    }

    // "NAME" and "IMPORT" are mutually exclusive
    if (map.HasValue(XDefineTagAttribute::kName) && map.HasValue(XDefineTagAttribute::kImport)) {
        return ParseStatusCode::kMalformedTag;
    }
    ...
    if (map.HasValue(XDefineTagAttribute::kName)) {
        ...
        if (!var_name.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(var_name).error());
        }
        ...
        auto value = types::ParseQuotedStringWithoutSubstitution(map.GetValue(XDefineTagAttribute::kValue), /*allow_empty*/ true);
        if (!value.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag);
        }

        return XDefineTag::CreateDefinition(std::move(var_name).value(), std::move(value).value().Str());
    }
...
// static
ParseStatus::Or<XIndependentSegmentsTag> XIndependentSegmentsTag::Parse(TagItem tag)
{
...
// static
ParseStatus::Or<XMediaTag> XMediaTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map_result.code() != ParseStatusCode::kReachedEOF) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(map_result));
    }
    ...
    if (map.HasValue(XMediaTagAttribute::kUri)) {
        ...
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kUri), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kGroupId)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kGroupId), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kLanguage)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kLanguage), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kAssocLanguage)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kAssocLanguage), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kName)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kName), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kStableRenditionId)) {
        auto result
            = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kStableRenditionId), variable_dict, sub_buffer).MapValue(types::StableId::Parse);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kInstreamId)) {
        ...
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kInstreamId), variable_dict, sub_buffer).MapValue(types::InstreamId::Parse);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kCharacteristics)) {
        auto result = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kCharacteristics), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMediaTagAttribute::kChannels)) {
        ...
        if (type == MediaType::kAudio) {
            auto result
                = types::ParseQuotedString(map.GetValue(XMediaTagAttribute::kChannels), variable_dict, sub_buffer).MapValue(types::AudioChannels::Parse);
            if (!result.has_value()) {
                return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
            }
...
// static
ParseStatus::Or<XStreamInfTag> XStreamInfTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XStreamInfTagAttribute::kBandwidth)) {
        auto bandwidth = types::ParseDecimalInteger(map.GetValue(XStreamInfTagAttribute::kBandwidth).SkipVariableSubstitution());
        if (!bandwidth.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(bandwidth).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kAverageBandwidth)) {
        auto average_bandwidth = types::ParseDecimalInteger(map.GetValue(XStreamInfTagAttribute::kAverageBandwidth).SkipVariableSubstitution());
        if (!average_bandwidth.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(average_bandwidth).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kScore)) {
        auto score = types::ParseDecimalFloatingPoint(map.GetValue(XStreamInfTagAttribute::kScore).SkipVariableSubstitution());
        if (!score.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(score).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kCodecs)) {
        auto codecs_string = types::ParseQuotedString(map.GetValue(XStreamInfTagAttribute::kCodecs), variable_dict, sub_buffer);
        if (!codecs_string.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(codecs_string).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kResolution)) {
        auto resolution = types::DecimalResolution::Parse(map.GetValue(XStreamInfTagAttribute::kResolution).SkipVariableSubstitution());
        if (!resolution.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(resolution).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kFrameRate)) {
        auto frame_rate = types::ParseDecimalFloatingPoint(map.GetValue(XStreamInfTagAttribute::kFrameRate).SkipVariableSubstitution());
        if (!frame_rate.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(frame_rate).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kAudio)) {
        auto audio = types::ParseQuotedString(map.GetValue(XStreamInfTagAttribute::kAudio), variable_dict, sub_buffer);
        if (!audio.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(audio).error());
        }
    ...
    if (map.HasValue(XStreamInfTagAttribute::kVideo)) {
        auto video = types::ParseQuotedString(map.GetValue(XStreamInfTagAttribute::kVideo), variable_dict, sub_buffer);
        if (!video.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(video).error());
        }
...
ParseStatus::Or<InfTag> InfTag::Parse(TagItem tag)
{
    ...
    // integer
    auto duration_result = types::ParseDecimalFloatingPoint(duration_str.SkipVariableSubstitution());
    if (!duration_result.has_value()) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(duration_result).error());
    }
...
ParseStatus::Or<XByteRangeTag> XByteRangeTag::Parse(TagItem tag)
{
    ...
    auto range = types::parsing::ByteRangeExpression::Parse(tag.GetContent()->SkipVariableSubstitution());
    if (!range.has_value()) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(range).error());
    }
...
// static
ParseStatus::Or<XDiscontinuitySequenceTag> XDiscontinuitySequenceTag::Parse(TagItem tag)
{
...
// static
ParseStatus::Or<XMapTag> XMapTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XMapTagAttribute::kUri)) {
        auto result = types::ParseQuotedString(map.GetValue(XMapTagAttribute::kUri), variable_dict, sub_buffer);
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(result).error());
        }
    ...
    if (map.HasValue(XMapTagAttribute::kByteRange)) {
        auto result = types::ParseQuotedString(map.GetValue(XMapTagAttribute::kByteRange), variable_dict, sub_buffer)
...
// static
ParseStatus::Or<XPartTag> XPartTag::Parse(TagItem tag, const VariableDictionary& vars, VariableDictionary::SubstitutionBuffer& subs)
{
    ...
    auto uri
        = ParseField<ResolvedSourceString>(XPartTagAttribute::kUri, map, &types::parsing::Quoted<types::parsing::RawStr>::ParseWithSubstitution, vars, subs);
    RETURN_IF_ERROR(uri);

    auto duration = ParseField<base::TimeDelta>(XPartTagAttribute::kDuration, map, &types::parsing::TimeDelta::ParseWithoutSubstitution);
    RETURN_IF_ERROR(duration);

    auto byte_range = ParseField<std::optional<types::parsing::ByteRangeExpression>>(
        XPartTagAttribute::kByteRange, map, &types::parsing::Quoted<types::parsing::ByteRangeExpression>::ParseWithSubstitution, vars, subs);
    RETURN_IF_ERROR(byte_range);

    auto independent = ParseField<std::optional<bool>>(XPartTagAttribute::kIndependent, map, &types::parsing::YesOrNo::ParseWithoutSubstitution);
    RETURN_IF_ERROR(independent);

    auto gap = ParseField<std::optional<bool>>(XPartTagAttribute::kGap, map, &types::parsing::YesOrNo::ParseWithoutSubstitution);
    RETURN_IF_ERROR(gap);
...
ParseStatus::Or<XPartInfTag> XPartInfTag::Parse(TagItem tag)
{
    ...
    if (map.HasValue(XPartInfTagAttribute::kPartTarget)) {
        auto result = types::ParseDecimalFloatingPoint(map.GetValue(XPartInfTagAttribute::kPartTarget).SkipVariableSubstitution());
...
ParseStatus::Or<XServerControlTag> XServerControlTag::Parse(TagItem tag)
{
    ...
    if (map.HasValue(XServerControlTagAttribute::kCanSkipUntil)) {
        auto result = types::ParseDecimalFloatingPoint(map.GetValue(XServerControlTagAttribute::kCanSkipUntil).SkipVariableSubstitution());
    ...
    if (map.HasValue(XServerControlTagAttribute::kCanSkipDateRanges)) {
        if (map.GetValue(XServerControlTagAttribute::kCanSkipDateRanges).Str() == "YES") {
    ...
    if (map.HasValue(XServerControlTagAttribute::kHoldBack)) {
        auto result = types::ParseDecimalFloatingPoint(map.GetValue(XServerControlTagAttribute::kHoldBack).SkipVariableSubstitution());
    ...
    if (map.HasValue(XServerControlTagAttribute::kPartHoldBack)) {
        auto result = types::ParseDecimalFloatingPoint(map.GetValue(XServerControlTagAttribute::kPartHoldBack).SkipVariableSubstitution());
    ...
    if (map.HasValue(XServerControlTagAttribute::kCanBlockReload)) {
        if (map.GetValue(XServerControlTagAttribute::kCanBlockReload).Str() == "YES") {
...
ParseStatus::Or<XTargetDurationTag> XTargetDurationTag::Parse(TagItem tag)
{
    ...
    auto duration_result = types::ParseDecimalInteger(tag.GetContent().value().SkipVariableSubstitution());
    if (!duration_result.has_value()) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(duration_result).error());
    }
...
ParseStatus::Or<XSkipTag> XSkipTag::Parse(TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    auto skip_result = types::ParseDecimalInteger(map.GetValue(XSkipTagAttribute::kSkippedSegments).SkipVariableSubstitution());

    if (!skip_result.has_value()) {
        return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(skip_result).error());
    }
    ...
    if (map.HasValue(XSkipTagAttribute::kRecentlyRemovedDateranges)) {
        ...
        auto removed_result
            = types::ParseQuotedString(map.GetValue(XSkipTagAttribute::kRecentlyRemovedDateranges), variable_dict, sub_buffer, /*allow_empty=*/true);
        if (!removed_result.has_value()) {
            return ParseStatus(ParseStatusCode::kMalformedTag).AddCause(std::move(removed_result).error());
        }
...
ParseStatus::Or<XRenditionReportTag> XRenditionReportTag::Parse(
    TagItem tag, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    if (map.HasValue(XRenditionReportTagAttribute::kUri)) {
        auto uri_result = types::ParseQuotedString(map.GetValue(XRenditionReportTagAttribute::kUri), variable_dict, sub_buffer);
    ...
    if (map.HasValue(XRenditionReportTagAttribute::kLastMSN)) {
        auto msn_result = types::ParseDecimalInteger(map.GetValue(XRenditionReportTagAttribute::kLastMSN).SkipVariableSubstitution());
    ...
    if (map.HasValue(XRenditionReportTagAttribute::kLastPart)) {
        auto part_result = types::ParseDecimalInteger(map.GetValue(XRenditionReportTagAttribute::kLastPart).SkipVariableSubstitution());
...
template <typename Attrs> ParseStatus::Or<TypedAttributeMap<Attrs>> RequireNonEmptyMap(std::optional<SourceString> content)
{
...
constexpr char const* kFmtWidevine = "urn:uuid:edef8ba9-79d6-4ace-a3c8-27dcd51d21ed";
...
ParseStatus::Or<XKeyTagKeyFormat> RecognizeFormat(std::optional<ResolvedSourceString> content)
{
    if (!content.has_value()) {
...
template <typename T>
ParseStatus::Or<T> ValidateKeyTag(XKeyTagMethod method, ResolvedSourceString uri, std::optional<XKeyTag::IVHex::Container> iv, XKeyTagKeyFormat keyformat,
    std::optional<ResolvedSourceString> keyformat_versions)
{
    ...
    return T { .method = method, .uri = std::move(uri), .iv = std::move(iv), .keyformat = keyformat, .keyformat_versions = std::move(keyformat_versions) };
}

template <typename T> ParseStatus::Or<T> ParseKeyTag(TagItem tag, const VariableDictionary& vars, VariableDictionary::SubstitutionBuffer& subs)
{
    return RequireNonEmptyMap<XKeyTagAttribute>(tag.GetContent()).MapValue([&](auto map) -> ParseStatus::Or<T> {
        auto enc_method = ParseField<XKeyTagMethod>(XKeyTagAttribute::kMethod, map, &RecognizeMethod);
        ...
        auto enc_uri
            = ParseField<ResolvedSourceString>(XKeyTagAttribute::kUri, map, &types::parsing::Quoted<types::parsing::RawStr>::ParseWithSubstitution, vars, subs);
        RETURN_IF_ERROR(enc_uri);

        auto enc_iv = ParseField<std::optional<XKeyTag::IVHex::Container>>(XKeyTagAttribute::kIv, map, &XKeyTag::IVHex::ParseWithoutSubstitution);
        RETURN_IF_ERROR(enc_iv);

        auto enc_keyformat = ParseField<std::optional<ResolvedSourceString>>(
            XKeyTagAttribute::kKeyFormat, map, &types::parsing::Quoted<types::parsing::RawStr>::ParseWithoutSubstitution)
                                 .MapValue(&RecognizeFormat);
        RETURN_IF_ERROR(enc_keyformat);

        auto enc_keyformat_versions
            = ParseField<std::optional<ResolvedSourceString>>(XKeyTagAttribute::kKeyFormatVersions, map, &types::parsing::RawStr::ParseWithoutSubstitution);
        RETURN_IF_ERROR(enc_keyformat_versions);

        return ValidateKeyTag<T>(*enc_method, *enc_uri, *enc_iv, *enc_keyformat, *enc_keyformat_versions);
    });
}

} // namespace

ParseStatus::Or<XKeyTag> XKeyTag::Parse(TagItem tag, const VariableDictionary& vars, VariableDictionary::SubstitutionBuffer& subs)
{
    DCHECK(tag.GetName() == ToTagName(XKeyTag::kName));
    return ParseKeyTag<XKeyTag>(tag, vars, subs);
}

ParseStatus::Or<XSessionKeyTag> XSessionKeyTag::Parse(TagItem tag, const VariableDictionary& vars, VariableDictionary::SubstitutionBuffer& subs)
{
