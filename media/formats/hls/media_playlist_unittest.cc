...
scoped_refptr<MultivariantPlaylist> CreateMultivariantPlaylist(std::initializer_list<std::string_view> lines,
    GURL uri = GURL("http://localhost/multi_playlist.m3u8"), types::DecimalInteger version = Playlist::kDefaultVersion)
{
    std::string source;
    for (auto line : lines) {
        source.append(line);
...
{
    constexpr types::DecimalInteger kSegmentDuration = MediaPlaylist::kMaxTargetDuration.InSeconds();
    constexpr size_t kMaxSegments = base::TimeDelta::FiniteMax().InSeconds() / kSegmentDuration;
    ...
    MediaPlaylistTestBuilder builder;
    builder.AppendLine("#EXTM3U");
    builder.AppendLine("#EXT-X-TARGETDURATION:" + base::NumberToString(kSegmentDuration));
    ...
    for (size_t i = 0; i < kMaxSegments; ++i) {
        ...
        builder.ExpectSegment(HasUri, GURL("http://localhost/segment" + base::NumberToString(i) + ".ts"));
    }

    // The segments above should not overflow the playlist duration
    builder.ExpectPlaylist(HasComputedDuration, base::Seconds(kSegmentDuration * kMaxSegments));
    builder.ExpectOk();
...
TEST(HlsMediaPlaylistTest, VariableSubstitution)
{
    ...
    builder.ExpectAdditionalSegment();
    builder.ExpectSegment(HasUri, GURL("http://video.com/some_video/low/fileSegment0.ts"));
    ...
    {
        auto fork = builder;
        fork.AppendLine(R"(#EXT-X-DEFINE:NAME="ROOT",VALUE="https://www.google.com")");
    ...
    auto parent = CreateMultivariantPlaylist(
        { "#EXTM3U", "#EXT-X-VERSION:8", R"(#EXT-X-DEFINE:NAME="IMPORTED",VALUE="HELLO")" }, GURL("http://localhost/multi_playlist.m3u8"), 8);
    {
...
TEST(HlsMediaPlaylistTest, MultivariantPlaylistTag)
{
    ...
    for (TagName name = ToTagName(MultivariantPlaylistTagName::kMinValue); name <= ToTagName(MultivariantPlaylistTagName::kMaxValue); ++name) {
...
TEST(HlsMediaPlaylistTest, XBitrateTag)
{
    ...
    builder.ExpectSegment(HasBitRate, std::numeric_limits<types::DecimalInteger>::max());
...
TEST(HlsMediaPlaylistTest, XByteRangeTag)
{
    ...
    {
        for (std::string_view x : { "", ":", ": 12@34", ":12@34 ", ":12@", ":12@{$offset}" }) {
            auto fork = builder;
    ...
    // segments.
    {
        ...
        fork = builder;
        fork.AppendLine("#EXT-X-BYTERANGE:18446744073709551615@18446744073709551615");
        fork.AppendLine("#EXTINF:9.2,\t");
        fork.AppendLine("segment1.ts");
        fork.ExpectError(ParseStatusCode::kByteRangeInvalid);
...
TEST(HlsMediaPlaylistTest, XDiscontinuitySequenceTag)
{
    ...
    // The EXT-X-DISCONTINUITY-SEQUENCE tag must appear before any media segment
    {
        ...
        fork.AppendLine("#EXT-X-DISCONTINUITY-SEQUENCE:0");
        fork.ExpectError(ParseStatusCode::kMediaSegmentBeforeDiscontinuitySequenceTag);
    }
    ...
    // EXT-X-DISCONTINUITY tag
    {
        ...
        fork.AppendLine("#EXTINF:9.8,\t");
        fork.AppendLine("segment0.ts");
        fork.ExpectError(ParseStatusCode::kDiscontinuityTagBeforeDiscontinuitySequenceTag);
    }

    const auto fill_playlist = [](auto& builder, auto first_media_sequence_number, auto first_discontinuity_sequence_number) {
        ...
        builder.ExpectSegment(HasDiscontinuitySequenceNumber, first_discontinuity_sequence_number);
        ...
        builder.ExpectAdditionalSegment();
        builder.ExpectSegment(HasDiscontinuity, true);
        builder.ExpectSegment(HasMediaSequenceNumber, first_media_sequence_number + 1);
        builder.ExpectSegment(HasDiscontinuitySequenceNumber, first_discontinuity_sequence_number + 1);

        builder.AppendLine("#EXTINF:9.8,\t");
        builder.AppendLine("segment2.ts");
        builder.ExpectAdditionalSegment();
        builder.ExpectSegment(HasDiscontinuity, false);
        builder.ExpectSegment(HasMediaSequenceNumber, first_media_sequence_number + 2);
        builder.ExpectSegment(HasDiscontinuitySequenceNumber, first_discontinuity_sequence_number + 1);
    };
...
TEST(HlsMediaPlaylistTest, XMapTag)
{
    ...
    auto init1 = base::MakeRefCounted<MediaSegment::InitializationSegment>(GURL("http://localhost/init1.ts"), std::nullopt);
    ...
    auto init3 = base::MakeRefCounted<MediaSegment::InitializationSegment>(GURL("http://localhost/init3.ts"), types::ByteRange::Validate(10, 0).value());
...
TEST(HlsMediaPlaylistTest, XPartInfTag)
{
    ...
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(0) });
    fork.ExpectOk();

    fork = builder;
    fork.AppendLine("#EXT-X-PART-INF:PART-TARGET=1");
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(1) });
    fork.ExpectOk();

    fork = builder;
    fork.AppendLine("#EXT-X-PART-INF:PART-TARGET=1.2");
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(1.2) });
    fork.ExpectOk();

    fork = builder;
    fork.AppendLine("#EXT-X-PART-INF:PART-TARGET=99.99");
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(99.99) });
    fork.ExpectOk();
    ...
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(100) });
    fork.ExpectOk();
...
TEST(HlsMediaPlaylistTest, XServerControlTag)
{
    ...
    fork = builder;
    fork.AppendLine("#EXT-X-SERVER-CONTROL:CAN-SKIP-DATERANGES=YES,CAN-SKIP-UNTIL=40");
    ...
    fork = builder;
    fork.AppendLine("#EXT-X-SERVER-CONTROL:CAN-SKIP-UNTIL=40,CAN-SKIP-DATERANGES=YES");
    fork.ExpectPlaylist(CanSkipDateRanges, true);
    fork.ExpectPlaylist(HasSkipBoundary, base::Seconds(40));
    fork.ExpectPlaylist(HasHoldBackDistance, base::Seconds(6) * 3);
    fork.ExpectPlaylist(HasPartHoldBackDistance, std::nullopt);
    ...
    fork.ExpectPlaylist(HasPartialSegmentInfo, MediaPlaylist::PartialSegmentInfo { .target_duration = base::Seconds(0.2) });
...
TEST(HlsMediaPlaylistTest, XTargetDurationTag)
{
    ...
    builder = MediaPlaylistTestBuilder();
    builder.AppendLine("#EXTM3U");
    builder.AppendLine("#EXT-X-TARGETDURATION:", base::NumberToString(MediaPlaylist::kMaxTargetDuration.InSeconds()));
    builder.ExpectPlaylist(HasTargetDuration, base::Seconds(MediaPlaylist::kMaxTargetDuration.InSeconds()));
    builder.ExpectOk();

    builder = MediaPlaylistTestBuilder();
    builder.AppendLine("#EXTM3U");
    builder.AppendLine("#EXT-X-TARGETDURATION:", base::NumberToString(MediaPlaylist::kMaxTargetDuration.InSeconds() + 1));
...
TEST(HlsMediaPlaylistTest, XKeyTagAppliesToSegments)
{
    ...
    builder.AppendLine("#EXT-X-KEY:METHOD=AES-128,URI=\"enc.key\",IV="
    ...
    builder.ExpectSegment(
        HasEncryptionData, std::make_tuple(GURL("http://localhost/enc.key"), XKeyTagMethod::kAES128, XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 0x42)));
    builder.ExpectOk();
    ...
    builder.AppendLine("#EXT-X-KEY:METHOD=AES-128,URI=\"enc.key\",KEYFORMAT=\"identity\"");
    builder.AppendLine("#EXTINF:1.60000,");
    ...
    builder.ExpectSegment(HasMediaSequenceNumber, 3);
    builder.ExpectSegment(
        HasEncryptionData, std::make_tuple(GURL("http://localhost/enc.key"), XKeyTagMethod::kAES128, XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 3)));
    builder.ExpectOk();
    ...
    builder.ExpectSegment(HasMediaSequenceNumber, 4);
    builder.ExpectSegment(
        HasEncryptionData, std::make_tuple(GURL("http://localhost/enc.key"), XKeyTagMethod::kAES128, XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 4)));
    builder.ExpectOk();

    builder.AppendLine("#EXT-X-KEY:METHOD=SAMPLE-AES,URI=\"enc.key\",KEYFORMAT=\"identity\"");
    builder.AppendLine("#EXTINF:1.60000,");
    ...
    builder.ExpectSegment(HasMediaSequenceNumber, 5);
    builder.ExpectSegment(
        HasEncryptionData, std::make_tuple(GURL("http://localhost/enc.key"), XKeyTagMethod::kSampleAES, XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 5)));
    builder.ExpectOk();

    builder.AppendLine("#EXT-X-KEY:METHOD=SAMPLE-AES-CTR,URI=\"enc.key\",KEYFORMAT="
                       "\"identity\"");
    builder.AppendLine("#EXTINF:1.60000,");
    ...
    builder.ExpectSegment(HasMediaSequenceNumber, 6);
    builder.ExpectSegment(
        HasEncryptionData, std::make_tuple(GURL("http://localhost/enc.key"), XKeyTagMethod::kSampleAESCTR, XKeyTagKeyFormat::kIdentity, std::make_tuple(0, 6)));
    builder.ExpectOk();
}
