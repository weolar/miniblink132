...
template <typename Self> class MEDIA_EXPORT SourceStringBase {
public:
    static Self CreateForTesting(std::string_view str);
    static Self CreateForTesting(size_t line, size_t column, std::string_view str);
...
class MEDIA_EXPORT SourceString final : public subtle::SourceStringBase<SourceString> {
public:
    // Only `SourceLineIterator` may create `SourceString`s.
    static SourceString Create(base::PassKey<SourceLineIterator>, size_t line, std::string_view str)
    {
...
class MEDIA_EXPORT ResolvedSourceString final : public subtle::SourceStringBase<ResolvedSourceString> {
public:
    ...
    static ResolvedSourceString Create(
        base::PassKey<VariableDictionary>, size_t line, size_t column, std::string_view str, SubstitutionState substitution_state)
    {
        return ResolvedSourceString(line, column, str, substitution_state);
    }
    static ResolvedSourceString Create(base::PassKey<SourceString>, size_t line, size_t column, std::string_view str)
    {
        return ResolvedSourceString(line, column, str, SubstitutionState::kNoSubstitutions);
    }
...
private:
    friend SourceStringBase;
    ResolvedSourceString(size_t line, size_t column, std::string_view str, SubstitutionState substitution_state = SubstitutionState::kNoSubstitutions);
...
struct MEDIA_EXPORT SourceLineIterator {
    ...
    size_t CurrentLineForTesting() const
    {
        return current_line_;
    }
    std::string_view SourceForTesting() const
    {
        return source_;
    }
