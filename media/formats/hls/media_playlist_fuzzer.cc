...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (data_provider.ConsumeBool()) {
        auto multivariant_playlist_source = data_provider.ConsumeRandomLengthString();

        // Determine playlist version (ignore type mismatch)
        const auto version = GetPlaylistVersion(multivariant_playlist_source);
        auto multivariant_playlist_result
            = media::hls::MultivariantPlaylist::Parse(multivariant_playlist_source, GURL("http://localhost/multi_playlist.m3u8"), version);
    ...
    media::hls::MediaPlaylist::Parse(media_playlist_source, GURL("http://localhost/playlist.m3u8"), version, multivariant_playlist.get());
