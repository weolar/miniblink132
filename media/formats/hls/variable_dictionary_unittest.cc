...
VariableDictionary CreateBasicDictionary(const base::Location& from = base::Location::Current())
{
    ...
    EXPECT_TRUE(dict.Insert(CreateVarName("_0THER-1dent"), "{$james}")) << from.ToString();

    return dict;
}

void OkTest(const VariableDictionary& dict, std::string_view in, std::string_view expected_out, bool substitutions_expected,
    const base::Location& from = base::Location::Current())
{
    ...
    EXPECT_EQ(result_str.ContainsSubstitutions(), substitutions_expected) << from.ToString();
}

void ErrorTest(const VariableDictionary& dict, std::string_view in, ParseStatusCode expected_error, const base::Location& from = base::Location::Current())
{
    const auto source_str = SourceString::CreateForTesting(in);
    VariableDictionary::SubstitutionBuffer buffer;
    auto result = dict.Resolve(source_str, buffer);
...
void NopTest(const VariableDictionary& dict, std::string_view in, const base::Location& from = base::Location::Current())
{
...
TEST(HlsVariableDictionaryTest, BasicSubstitution)
{
    ...
    OkTest(dict, "The NAME's {$NAME}, {$_0THER-1dent} {$NAME}. Agent {$IDENT}", "The NAME's bond, {$james} bond. Agent 007", true);
...
TEST(HlsVariableDictionaryTest, VariableUndefined)
{
    ...
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), std::make_optional<std::string_view>("FOO"));
    ...
    ErrorTest(dict, "Hello {$TEST} {$TEST1}", ParseStatusCode::kVariableUndefined);
}
...
TEST(HlsVariableDictionaryTest, RedefinitionNotAllowed)
{
    ...
    EXPECT_EQ(dict.Find(CreateVarName("TEsT")), std::make_optional<std::string_view>("BAR"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), std::make_optional<std::string_view>("FOO"));

    EXPECT_TRUE(dict.Insert(CreateVarName("TEST2"), "BAZ"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST2")), std::make_optional<std::string_view>("BAZ"));
    EXPECT_EQ(dict.Find(CreateVarName("TEsT")), std::make_optional<std::string_view>("BAR"));
    EXPECT_EQ(dict.Find(CreateVarName("TEST")), std::make_optional<std::string_view>("FOO"));
}
...
TEST(HlsVariableDictionaryTest, IgnoreInvalidRefSequence)
{
    ...
    // ignored
    OkTest(dict, "http://{$}{$ NAME}{$NAME}}{$NAME }.com", "http://{$}{$ NAME}bond}{$NAME }.com", true);
...
TEST(HlsVariableDictionaryTest, ExplosiveVariableDefs)
{
    ...
    OkTest(dict, "{$LOL3}{$LOL3}{$LOL3}", "{$LOL2}{$LOL2}{$LOL2}{$LOL2}{$LOL2}{$LOL2}{$LOL2}{$LOL2}{$LOL2}", true);
