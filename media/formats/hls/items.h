...
public:
    ...
    std::optional<TagName> GetName() const
    {
        return name_;
    }
...
private:
    TagItem(std::optional<TagName> name, std::optional<SourceString> content_or_name, size_t line_number)
        : name_(name)
...
// empty lines.
MEDIA_EXPORT ParseStatus::Or<GetNextLineItemResult> GetNextLineItem(SourceLineIterator* src);
