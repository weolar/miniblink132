...
TEST(HlsMultivariantPlaylistTest, VariableSubstitution)
{
    ...
    builder.AppendLine(R"(#EXT-X-DEFINE:NAME="HOST",VALUE="http://www.example.com")");
    builder.AppendLine(R"(#EXT-X-DEFINE:NAME="CODECS",VALUE="mp4a.40.2,avc1.4d401e")");
    ...
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://www.example.com/playlist1.m3u8"));
    builder.ExpectVariant(HasCodecs, std::vector<std::string> { "mp4a.40.2", "avc1.4d401e" });
    ...
    }
    {
        ...
        fork.AppendLine("#EXT-X-STREAM-INF:BANDWIDTH=100,AVERAGE-BANDWIDTH={$AVG-BW}");
        fork.AppendLine("playlist3.m3u8");
        fork.ExpectError(ParseStatusCode::kMalformedTag);
    }
    {
    ...
    {
        auto fork = builder;
        fork.AppendLine(R"(#EXT-X-DEFINE:NAME="HOST",VALUE="https://www.google.com")");
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://{$BAR}.com/playlist4.m3u8"));
...
TEST(HlsMultivariantPlaylistTest, MediaPlaylistTag)
{
    ...
    for (TagName name = ToTagName(MediaPlaylistTagName::kMinValue); name <= ToTagName(MediaPlaylistTagName::kMaxValue); ++name) {
...
TEST(HlsMultivariantPlaylistTest, XStreamInfTag)
{
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist1.m3u8"));
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist4.m3u8"));
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist5.m3u8"));
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist6.m3u8"));
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist7.m3u8"));
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist8.m3u8"));
    builder.ExpectVariant(HasBandwidth, 109u);
    builder.ExpectVariant(HasAverageBandwidth, std::nullopt);
    builder.ExpectVariant(HasScore, std::nullopt);
    builder.ExpectVariant(HasCodecs, std::nullopt);
    builder.ExpectVariant(HasResolution, types::DecimalResolution { .width = 1920, .height = 1080 });
    builder.ExpectVariant(HasFrameRate, std::nullopt);
    builder.ExpectOk();
    ...
    builder.ExpectAdditionalVariant();
    builder.ExpectVariant(HasPrimaryRenditionUri, GURL("http://localhost/playlist9.m3u8"));
...
TEST(HlsMultivariantPlaylistTest, XMediaTag)
{
    ...
    fork = builder;
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI=\"english."
                    "m3u8\"");
    ...
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI=\"english."
                    "m3u8\"");
    fork.ExpectAudioRendition("foo", "English", RenditionHasUri, GURL("http://localhost/english.m3u8"));
    fork.ExpectAudioRendition("foo", "English", HasLanguage, std::nullopt);
    fork.ExpectAudioRendition("foo", "English", HasAssociatedLanguage, std::nullopt);
    fork.ExpectAudioRendition("foo", "English", MayAutoSelect, false);
    fork.ExpectAudioRendition("foo", "English", HasStableRenditionId, std::nullopt);
    ...
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI=\"english."
                    "m3u8\"");
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI="
                    "\"english2.m3u8\"");
    ...
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI=\"english."
                    "m3u8\"");
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"bar\",NAME=\"English\",URI="
                    "\"english2.m3u8\"");
    fork.ExpectOk();
    ...
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"English\",URI=\"english."
                    "m3u8\",DEFAULT=YES");
    fork.AppendLine("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"foo\",NAME=\"Spanish\",URI=\"spanish."
                    "m3u8\",DEFAULT=YES");
    fork.ExpectAudioRenditionGroup("foo", HasDefaultRendition, "English");
    fork.ExpectAudioRendition("foo", "English", RenditionHasUri, GURL("http://localhost/english.m3u8"));
    fork.ExpectAudioRendition("foo", "English", MayAutoSelect, true);
    fork.ExpectAudioRendition("foo", "Spanish", RenditionHasUri, GURL("http://localhost/spanish.m3u8"));
