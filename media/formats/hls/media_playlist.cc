...
ParseStatus::Or<scoped_refptr<MediaPlaylist>> MediaPlaylist::Parse(
    std::string_view source, GURL uri, types::DecimalInteger version, const MultivariantPlaylist* parent_playlist, TagRecorder* tag_recorder)
{
    DCHECK(version != 0);
    if (version < Playlist::kMinSupportedVersion || version > Playlist::kMaxSupportedVersion) {
    ...
    if (parent_playlist) {
        common_state.parent_variable_dict = &parent_playlist->GetVariableDictionary();
    }
    ...
    while (true) {
        ...
        if (auto* tag = absl::get_if<TagItem>(&item)) {
            ...
            case MediaPlaylistTagName::kXDiscontinuitySequence: {
                if (tag_recorder) {
                    tag_recorder->SetMetric(TagRecorder::Metric::kDiscontinuitySequence);
                }
                ...
                if (discontinuity_sequence_number != 0) {
                    return ParseStatusCode::kDiscontinuityTagBeforeDiscontinuitySequenceTag;
                }
            ...
            case MediaPlaylistTagName::kXKey: {
                ...
                auto result = XKeyTag::Parse(*tag, common_state.variable_dict, sub_buffer);
                if (!result.has_value()) {
                ...
                if (value.method == XKeyTagMethod::kNone) {
                    ...
                    new_encryption_data = true;
                    encryption_data = base::MakeRefCounted<MediaSegment::EncryptionData>(std::move(resource_uri), value.method, value.keyformat, value.iv);
                }

                break;
            }
            case MediaPlaylistTagName::kXMap: {
                auto result = XMapTag::Parse(*tag, common_state.variable_dict, sub_buffer);
                if (!result.has_value()) {
                    return std::move(result).error();
                }
                auto value = std::move(result).value();
                ...
                if (value.byte_range.has_value()) {
                    ...
                    byte_range = types::ByteRange::Validate(value.byte_range->length, value.byte_range->offset.value_or(0));
                ...
                initialization_segment = base::MakeRefCounted<MediaSegment::InitializationSegment>(std::move(resource_uri), byte_range);
                break;
            }
        ...
        auto segment_uri_result = ParseUri(absl::get<UriItem>(std::move(item)), uri, common_state, sub_buffer);
        ...
        const types::DecimalInteger media_sequence_number = (media_sequence_tag ? media_sequence_tag->number : 0) + segments.size();
        ...
        segments.push_back(base::MakeRefCounted<MediaSegment>(inf_tag->duration, media_sequence_number, discontinuity_sequence_number, std::move(segment_uri),
            initialization_segment, encryption_data, byterange, bitrate, discontinuity_tag.has_value(), gap_tag.has_value(), new_init_segment,
            new_encryption_data));
    ...
    if (part_inf_tag.has_value()) {
        partial_segment_info = MediaPlaylist::PartialSegmentInfo { .target_duration = part_inf_tag->target_duration };
    ...
    if (server_control_tag.has_value()) {
        ...
        if (server_control_tag->part_hold_back.has_value()) {
            ...
            // duration.
            if (partial_segment_info.has_value() && part_hold_back_distance < partial_segment_info->target_duration * 2) {
    ...
    for (const auto& segment : segments) {
        ...
        const auto rounded_duration = std::round(segment->GetDuration().InSecondsF());
    ...
    // playlist.
    const bool independent_segments = common_state.independent_segments_tag.has_value() || (parent_playlist && parent_playlist->AreSegmentsIndependent());
    ...
    return base::MakeRefCounted<MediaPlaylist>(base::PassKey<MediaPlaylist>(),
