...
TagItem GetTagItem(SourceString tag)
{
    ...
    if (name.has_value()) {
        return content ? TagItem::Create(*name, *content) : TagItem::CreateEmpty(*name, tag.Line());
    }
...
ParseStatus::Or<GetNextLineItemResult> GetNextLineItem(SourceLineIterator* src)
{
    while (true) {
