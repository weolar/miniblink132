...
SegmentStream::SegmentIndex::SegmentIndex(const MediaSegment& segment)
    : SegmentIndex(segment.GetMediaSequenceNumber(), segment.GetDiscontinuitySequenceNumber())
{
}

SegmentStream::SegmentIndex::SegmentIndex(types::DecimalInteger discontinuity, types::DecimalInteger media)
    : media_sequence_(media)
    , discontinuity_sequence_(discontinuity)
{
}

bool SegmentStream::SegmentIndex::operator<(const SegmentStream::SegmentIndex& other) const
{
    return (discontinuity_sequence_ < other.discontinuity_sequence_)
        || (discontinuity_sequence_ == other.discontinuity_sequence_ && media_sequence_ < other.media_sequence_);
}
...
bool SegmentStream::SegmentIndex::operator==(const SegmentStream::SegmentIndex& other) const
{
    return discontinuity_sequence_ == other.discontinuity_sequence_ && media_sequence_ == other.media_sequence_;
}
...
SegmentStream::SegmentIndex SegmentStream::SegmentIndex::MaxOf(const MediaSegment& other) const
{
...
SegmentStream::SegmentStream(scoped_refptr<MediaPlaylist> playlist, bool seekable)
    : seekable_(seekable)
...
void SegmentStream::SetNewPlaylist(scoped_refptr<MediaPlaylist> playlist)
{
    ...
    if (!Exhausted()) {
        ...
        must_keep_encrypted_ = !!segments_.front()->GetEncryptionData() && !segments_.front()->HasNewEncryptionData();
    }
