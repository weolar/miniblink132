...
TEST(HlsPlaylistTest, IdentifyPlaylist)
{
    constexpr auto ok_test
        = [](types::DecimalInteger version, Playlist::Kind kind, std::string_view src, const base::Location& from = base::Location::Current()) {
    ...
    constexpr auto error_test = [](ParseStatusCode expected_error, std::string_view src, const base::Location& from = base::Location::Current()) {
        auto result = Playlist::IdentifyPlaylist(src);
        ...
        for (types::DecimalInteger version : { 0, 1, 5, 10 }) {
            ...
            for (bool common_tag : { true, false }) {
                ...
                case TagKind::kCommonTag:
                    ok_test(version ? version : Playlist::kDefaultVersion, Playlist::Kind::kMultivariantPlaylist, src);
                    break;
                case TagKind::kMultivariantPlaylistTag:
                    ok_test(version ? version : Playlist::kDefaultVersion, Playlist::Kind::kMultivariantPlaylist, src);
                    break;
                case TagKind::kMediaPlaylistTag:
                    ok_test(version ? version : Playlist::kDefaultVersion, Playlist::Kind::kMediaPlaylist, src);
                    break;
                }
            }
        }
    ...
    error_test(ParseStatusCode::kMultivariantPlaylistHasMediaPlaylistTag, "#EXT-X-STREAM-INF\n#EXTINF\n");
    ...
    // playlist kind
    ok_test(5, Playlist::Kind::kMediaPlaylist, "#EXT-X-VERSION:5\n#EXTINF\n#EXT-X-STREAM-INF\n");
