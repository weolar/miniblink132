...
template <typename T>
void ErrorTest(std::optional<std::string_view> content, ParseStatusCode expected_status, const base::Location& from = base::Location::Current())
{
    auto tag = content ? TagItem::Create(ToTagName(T::kName), SourceString::CreateForTesting(*content)) : TagItem::CreateEmpty(ToTagName(T::kName), 1);
...
template <typename T>
void ErrorTest(std::optional<std::string_view> content, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer,
    ParseStatusCode expected_status, const base::Location& from = base::Location::Current())
{
    auto tag = content ? TagItem::Create(ToTagName(T::kName), SourceString::CreateForTesting(*content)) : TagItem::CreateEmpty(ToTagName(T::kName), 1);
    auto result = T::Parse(tag, variable_dict, sub_buffer);
    ASSERT_FALSE(result.has_value()) << from.ToString();
    auto error = std::move(result).error();
    EXPECT_EQ(error.code(), expected_status) << "Actual Error: " << MediaSerializeForTesting(error) << "\n" << from.ToString();
}

template <typename T> struct OkTestResult {
    T tag;
...
template <typename T> OkTestResult<T> OkTest(std::optional<std::string> content, const base::Location& from = base::Location::Current())
{
    std::unique_ptr<std::string> source = content ? std::make_unique<std::string>(std::move(*content)) : nullptr;
    auto tag = source ? TagItem::Create(ToTagName(T::kName), SourceString::CreateForTesting(*source)) : TagItem::CreateEmpty(ToTagName(T::kName), 1);
    auto result = T::Parse(tag);
    EXPECT_TRUE(result.has_value()) << from.ToString();
    return OkTestResult<T> { .tag = std::move(result).value(), .source = std::move(source) };
}

template <typename T>
OkTestResult<T> OkTest(std::optional<std::string> content, const VariableDictionary& variable_dict, VariableDictionary::SubstitutionBuffer& sub_buffer,
    const base::Location& from = base::Location::Current())
{
    auto source = content ? std::make_unique<std::string>(std::move(*content)) : nullptr;
    auto tag = source ? TagItem::Create(ToTagName(T::kName), SourceString::CreateForTesting(*source)) : TagItem::CreateEmpty(ToTagName(T::kName), 1);
    auto result = T::Parse(tag, variable_dict, sub_buffer);
    if (!result.has_value()) {
...
void RunTagIdenficationTest(
    TagName name, std::string_view line, std::optional<std::string_view> expected_content, const base::Location& from = base::Location::Current())
{
    ...
    EXPECT_EQ(tag->GetContent().has_value(), expected_content.has_value()) << from.ToString();
...
template <typename T>
void RunTagIdenficationTest(std::string_view line, std::optional<std::string_view> expected_content, const base::Location& from = base::Location::Current())
{
...
template <typename T> void RunEmptyTagTest()
{
...
// tags.
template <typename T> void RunDecimalIntegerTagTest(types::DecimalInteger T::*field)
{
    // Content is required
...
VariableDictionary CreateBasicDictionary(const base::Location& from = base::Location::Current())
{
...
{
    RunTagIdenficationTest<XDefineTag>("#EXT-X-DEFINE:NAME=\"FOO\",VALUE=\"Bar\",\n", "NAME=\"FOO\",VALUE=\"Bar\",");
    ...
    ErrorTest<XDefineTag>(R"(NAME="Foo",IMPORT="Foo")", ParseStatusCode::kMalformedTag);

    // Name without VALUE is NOT allowed
    ErrorTest<XDefineTag>(R"(NAME="Foo",)", ParseStatusCode::kMalformedTag);

    // Empty NAME is not allowed
    ErrorTest<XDefineTag>(R"(NAME="",VALUE="Foo")", ParseStatusCode::kMalformedTag);

    // Empty IMPORT is not allowed
    ErrorTest<XDefineTag>(R"(IMPORT="")", ParseStatusCode::kMalformedTag);

    // Non-valid NAME is not allowed
    ErrorTest<XDefineTag>(R"(NAME=".FOO",VALUE="Foo")", ParseStatusCode::kMalformedTag);
    ErrorTest<XDefineTag>(R"(NAME="F++OO",VALUE="Foo")", ParseStatusCode::kMalformedTag);
    ErrorTest<XDefineTag>(R"(NAME=" FOO",VALUE="Foo")", ParseStatusCode::kMalformedTag);
    ErrorTest<XDefineTag>(R"(NAME="FOO ",VALUE="Foo")", ParseStatusCode::kMalformedTag);
}
...
{
    RunTagIdenficationTest<XIndependentSegmentsTag>("#EXT-X-INDEPENDENT-SEGMENTS\n", std::nullopt);
    RunEmptyTagTest<XIndependentSegmentsTag>();
}

TEST(HlsTagsTest, ParseXStartTag)
{
    RunTagIdenficationTest(ToTagName(CommonTagName::kXStart), "#EXT-X-START:TIME-OFFSET=30,PRECISE=YES\n", "TIME-OFFSET=30,PRECISE=YES");
...
{
    RunTagIdenficationTest(ToTagName(MultivariantPlaylistTagName::kXContentSteering),
...
{
    RunTagIdenficationTest(ToTagName(MultivariantPlaylistTagName::kXIFrameStreamInf), "#EXT-X-I-FRAME-STREAM-INF:URI=\"foo.m3u8\",BANDWIDTH=1000\n",
...
{
    RunTagIdenficationTest(ToTagName(MultivariantPlaylistTagName::kXMedia),
    ...
    ErrorTest<XMediaTag>(std::nullopt, variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("123", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("Foobar", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // TYPE attribute is required
    ErrorTest<XMediaTag>("GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=FAKE,GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE={$TYPE},GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    auto result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer);
    ...
    result = OkTest<XMediaTag>("TYPE=VIDEO,GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer);
    ...
    ErrorTest<XMediaTag>("TYPE=SUBTITLES,GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMediaTag>("TYPE=SUBTITLES,GROUP-ID=\"group\",NAME=\"name\",URI=\"foo.m3u8\"", variable_dict, sub_buffer);
    ...
    ErrorTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\",INSTREAM-ID="
                         "\"CC1\",URI=\"foo.m3u8\"",
        variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // The URI attribute must be a valid quoted-string
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",URI=foo", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",URI=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",URI=\"{$EMPTY}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",URI=\"foo.m3u8\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri->Str(), "foo.m3u8");
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=foo,NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"{$EMPTY}\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // The GROUP-ID attribute is subject to variable substitution
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"foo{$FOO}\",NAME=\"name\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=foo", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=\"{$EMPTY}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=\"en\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=\"{$FOO}\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    ASSERT_TRUE(result.tag.language.has_value());
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",ASSOC-LANGUAGE=foo", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",ASSOC-LANGUAGE=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",ASSOC-LANGUAGE=\"{$EMPTY}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",ASSOC-LANGUAGE=\"en\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",ASSOC-LANGUAGE=\"{$FOO}\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",LANGUAGE=\"foo\",ASSOC-"
                               "LANGUAGE=\"bar\"",
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,NAME=foo,GROUP-ID=\"group\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,NAME=\"\",GROUP-ID=\"group\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,NAME=\"{$EMPTY}\",GROUP-ID=\"group\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // NAME is subject to variable substitution
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"foo{$FOO}\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    // a valid StableId
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID=FOO", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID=\"{$"
                         "EMPTY}\"",
        variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID=\"*\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID="
                               "\"abcABC123+/=.-_\"",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",STABLE-RENDITION-ID=\"foo{$"
                               "FOO}\"",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    ...
    for (std::string x : { "FOO", "Y", "{$YEAH}", "NO" }) {
        result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",DEFAULT=" + x, variable_dict, sub_buffer);
        EXPECT_EQ(result.tag.type, MediaType::kAudio);
        EXPECT_EQ(result.tag.uri, std::nullopt);
        EXPECT_EQ(result.tag.group_id.Str(), "group");
        EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",DEFAULT=YES", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    // are ignored, and it is not subject to variable substitution.
    for (std::string x : { "FOO", "Y", "{$YEAH}", "NO" }) {
        result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",AUTOSELECT=" + x, variable_dict, sub_buffer);
        EXPECT_EQ(result.tag.type, MediaType::kAudio);
        EXPECT_EQ(result.tag.uri, std::nullopt);
        EXPECT_EQ(result.tag.group_id.Str(), "group");
        EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",AUTOSELECT=YES", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",DEFAULT=YES,AUTOSELECT=NO", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",DEFAULT=YES,AUTOSELECT=YES", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,URI=\"foo.m3u8\",GROUP-ID=\"group\",NAME=\"name\",FORCED=YES", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,URI=\"foo.m3u8\",GROUP-ID=\"group\",NAME=\"name\",FORCED=NO", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ...
    for (std::string x : { "FOO", "Y", "{$YEAH}", "NO" }) {
        result = OkTest<XMediaTag>("TYPE=SUBTITLES,URI=\"foo.m3u8\",GROUP-ID=\"group\",NAME=\"name\","
                                   "FORCED="
                + x,
            variable_dict, sub_buffer);
        EXPECT_EQ(result.tag.type, MediaType::kSubtitles);
    ...
    result = OkTest<XMediaTag>("TYPE=SUBTITLES,URI=\"foo.m3u8\",GROUP-ID=\"group\",NAME=\"name\",FORCED="
                               "YES",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kSubtitles);
    ASSERT_TRUE(result.tag.uri.has_value());
    ...
    ErrorTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\",INSTREAM-ID="
                               "\"CC1\"",
        variable_dict, sub_buffer);
    ...
    ErrorTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\"INSTREAM-ID=CC1", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\"INSTREAM-ID="
                         "\"FOO\"",
        variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=CLOSED-CAPTIONS,GROUP-ID=\"group\",NAME=\"name\"INSTREAM-ID="
    ...
    EXPECT_EQ(result.tag.forced, false);
    ASSERT_TRUE(result.tag.instream_id.has_value());
    EXPECT_EQ(result.tag.instream_id->GetType(), types::InstreamId::Type::kService);
    ...
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHARACTERISTICS=foo", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHARACTERISTICS=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHARACTERISTICS=\"foo,bar,"
                               "baz\"",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHARACTERISTICS=\"{$FOO},{$"
                               "BAR}\"",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    ...
    // audio.
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=foo", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"foo\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1/foo\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1/FOO,,BAR\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1/{$FOO}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1/FOO,BAR,-\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
    EXPECT_EQ(result.tag.group_id.Str(), "group");
    EXPECT_EQ(result.tag.language, std::nullopt);
    ...
    result = OkTest<XMediaTag>("TYPE=AUDIO,GROUP-ID=\"group\",NAME=\"name\",CHANNELS=\"1/"
                               "FOO,{$SRVC},-\"",
        variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.type, MediaType::kAudio);
    EXPECT_EQ(result.tag.uri, std::nullopt);
...
{
    RunTagIdenficationTest(ToTagName(MultivariantPlaylistTagName::kXSessionData), "#EXT-X-SESSION-DATA:DATA-ID=\"com.google.key\",VALUE=\"value\"\n",
...
TEST(HlsTagsTest, ParseXSessionKeyTag)
{
    ...
    // Invalid method
    ErrorTest<XSessionKeyTag>("METHOD=77", dict, subs, ParseStatusCode::kMalformedTag);
    ErrorTest<XSessionKeyTag>("METHOD=NONE", dict, subs, ParseStatusCode::kMalformedTag);

    // No IV when method is SAMPLE-AES-CTR
    ErrorTest<XSessionKeyTag>("METHOD=SAMPLE-AES-CTR,IV=0xf4d52cf0dc02329c3ad6578744590658", dict, subs, ParseStatusCode::kMalformedTag);

    // Invalid IV
    ErrorTest<XSessionKeyTag>("METHOD=AES-128,IV=0xf4d52cf0dc2329c3ad6578744590658", dict, subs, ParseStatusCode::kMalformedTag);

    {
        auto result = OkTest<XSessionKeyTag>("METHOD=AES-128,URI=\"https://example.com\","
    ...
    {
        auto result = OkTest<XSessionKeyTag>("METHOD=SAMPLE-AES,URI=\"https://example.com\","
                                             "KEYFORMAT=\"supersecretcrypto\"",
            dict, subs);
    ...
    {
        auto result = OkTest<XSessionKeyTag>("METHOD=SAMPLE-AES-CTR,URI=\"https://example.com\","
                                             "KEYFORMAT=\"supersecretcrypto\"",
            dict, subs);
...
{
    RunTagIdenficationTest<XStreamInfTag>("#EXT-X-STREAM-INF:BANDWIDTH=1010,CODECS=\"foo,bar\"\n", "BANDWIDTH=1010,CODECS=\"foo,bar\"");

    VariableDictionary variable_dict = CreateBasicDictionary();
    VariableDictionary::SubstitutionBuffer sub_buffer;

    auto result = OkTest<XStreamInfTag>(R"(BANDWIDTH=1010,AVERAGE-BANDWIDTH=1000,CODECS="foo,bar",SCORE=12.2)", variable_dict, sub_buffer);
    ...
    EXPECT_TRUE(base::ranges::equal(result.tag.codecs.value(), std::array { "foo", "bar" }));
    EXPECT_EQ(result.tag.resolution, std::nullopt);
    EXPECT_EQ(result.tag.frame_rate, std::nullopt);

    result = OkTest<XStreamInfTag>(R"(BANDWIDTH=1010,RESOLUTION=1920x1080,FRAME-RATE=29.97)", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.bandwidth, 1010u);
    ...
    result = OkTest<XStreamInfTag>(R"(BANDWIDTH=5050)", variable_dict, sub_buffer);
    ...
    ErrorTest<XStreamInfTag>(std::nullopt, variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>("", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(CODECS="foo,bar")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "BANDWIDTH" must be a valid DecimalInteger (non-negative)
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH="111")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=-1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1.5)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "AVERAGE-BANDWIDTH" must be a valid DecimalInteger (non-negative)
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AVERAGE-BANDWIDTH="111")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AVERAGE-BANDWIDTH=-1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AVERAGE-BANDWIDTH=1.5)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "SCORE" must be a valid DecimalFloatingPoint (non-negative)
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,SCORE="1")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,SCORE=-1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,SCORE=ONE)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "CODECS" must be a valid string
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,CODECS=abc,123)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,CODECS=abc)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,CODECS=123)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,CODECS="")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "CODECS" is subject to variable substitution
    result = OkTest<XStreamInfTag>(R"(BANDWIDTH=1010,CODECS="{$FOO},{$BAR}")", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.bandwidth, 1010u);
    EXPECT_EQ(result.tag.average_bandwidth, std::nullopt);
    EXPECT_EQ(result.tag.score, std::nullopt);
    ASSERT_TRUE(result.tag.codecs.has_value());
    EXPECT_TRUE(base::ranges::equal(result.tag.codecs.value(), std::array { "bar", "baz" }));
    EXPECT_EQ(result.tag.resolution, std::nullopt);

    // "RESOLUTION" must be a valid decimal-resolution
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,RESOLUTION=1920x)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,RESOLUTION=x123)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "FRAME-RATE" must be a valid decimal-floating-point (unsigned)
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,FRAME-RATE=-1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,FRAME-RATE=One)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,FRAME-RATE=30.0.0)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "AUDIO" must be a valid quoted-string
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AUDIO=1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AUDIO="")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,AUDIO=stereo)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "AUDIO" is subject to variable substitution
    result = OkTest<XStreamInfTag>(R"(BANDWIDTH=1010,AUDIO="{$FOO}{$BAR}")", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.bandwidth, 1010u);
    EXPECT_EQ(result.tag.average_bandwidth, std::nullopt);
    EXPECT_EQ(result.tag.score, std::nullopt);
    EXPECT_EQ(result.tag.codecs, std::nullopt);
    ...
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,VIDEO=1)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,VIDEO="")", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XStreamInfTag>(R"(BANDWIDTH=1010,VIDEO=stereo)", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // "VIDEO" is subject to variable substitution
    result = OkTest<XStreamInfTag>(R"(BANDWIDTH=1010,VIDEO="{$BAZ}{$FOO}")", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.bandwidth, 1010u);
    EXPECT_EQ(result.tag.average_bandwidth, std::nullopt);
    EXPECT_EQ(result.tag.score, std::nullopt);
    EXPECT_EQ(result.tag.codecs, std::nullopt);
...
TEST(HlsTagsTest, ParseInfTag)
{
    ...
    ErrorTest<InfTag>(base::NumberToString(MaxSeconds() + 1) + ",\t", ParseStatusCode::kValueOverflowsTimeDelta);
}
...
{
    RunTagIdenficationTest(ToTagName(MediaPlaylistTagName::kXDateRange), "#EXT-X-DATERANGE:ID=\"ad\",START-DATE=\"2022-07-19T01:04:57+0000\"\n",
...
{
    RunTagIdenficationTest<XDiscontinuitySequenceTag>("#EXT-X-DISCONTINUITY-SEQUENCE:3\n", "3");
...
TEST(HlsTagsTest, ParseXKeyTag)
{
    ...
    ErrorTest<XKeyTag>("METHOD=NONE,URI=\"https://example.com\"", dict, subs, ParseStatusCode::kMalformedTag);
    ErrorTest<XKeyTag>("METHOD=NONE,IV=0xf4d52cf0dc02329c3ad6578744590658", dict, subs, ParseStatusCode::kMalformedTag);
    ErrorTest<XKeyTag>("METHOD=NONE,KEYFORMAT=identity", dict, subs, ParseStatusCode::kMalformedTag);
    ErrorTest<XKeyTag>("METHOD=NONE,KEYFORMATVERSIONS=1/2/3", dict, subs, ParseStatusCode::kMalformedTag);

    // No IV when method is SAMPLE-AES-CTR
    ErrorTest<XKeyTag>("METHOD=SAMPLE-AES-CTR,IV=0xf4d52cf0dc02329c3ad6578744590658", dict, subs, ParseStatusCode::kMalformedTag);

    // Invalid IV
    ErrorTest<XKeyTag>("METHOD=AES-128,IV=0xf4d52cf0dc2329c3ad6578744590658", dict, subs, ParseStatusCode::kMalformedTag);

    // Not allowed certain methods with clearkey or widevine
    ErrorTest<XKeyTag>("METHOD=AES-128,FORMAT=\"org.w3.clearkey\",IV="
                       "0xf4d52cf0dc02329c3ad6578744590658",
        dict, subs, ParseStatusCode::kMalformedTag);
    // Not allowed certain methods with clearkey or widevine
    ErrorTest<XKeyTag>("METHOD=AES-256,FORMAT=\"org.w3.clearkey\",IV="
                       "0xf4d52cf0dc02329c3ad6578744590658",
        dict, subs, ParseStatusCode::kMalformedTag);
    ...
    {
        auto result = OkTest<XKeyTag>("METHOD=AES-128,URI=\"https://example.com\","
                                      "IV=0xf4d52cf0dc02329c3ad6578744590658",
            dict, subs);
        EXPECT_EQ(result.tag.method, XKeyTagMethod::kAES128);
    ...
    {
        auto result = OkTest<XKeyTag>("METHOD=SAMPLE-AES,URI=\"https://example.com\","
                                      "KEYFORMAT=\"supersecretcrypto\"",
            dict, subs);
        EXPECT_EQ(result.tag.method, XKeyTagMethod::kSampleAES);
        EXPECT_TRUE(result.tag.uri.has_value());
    ...
    {
        auto result = OkTest<XKeyTag>("METHOD=SAMPLE-AES-CTR,URI=\"https://example.com\","
                                      "KEYFORMAT=\"supersecretcrypto\"",
            dict, subs);
        EXPECT_EQ(result.tag.method, XKeyTagMethod::kSampleAESCTR);
        EXPECT_TRUE(result.tag.uri.has_value());
...
TEST(HlsTagsTest, ParseXMapTag)
{
    ...
    ErrorTest<XMapTag>(std::nullopt, variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMapTag>("", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMapTag>("BYTERANGE=12", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMapTag>("URI=foo.ts", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    auto result = OkTest<XMapTag>("URI=\"foo.ts\",FUTURE=PROOF", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_EQ(result.tag.byte_range, std::nullopt);

    // The URI attribute is subject to variable substitution
    ErrorTest<XMapTag>("URI=\"{$UNDEFINED}.ts\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMapTag>("URI=\"{$FOO}_{$BAR}.ts\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "bar_baz.ts");
    EXPECT_EQ(result.tag.byte_range, std::nullopt);

    // Test the BYTERANGE attribute
    ErrorTest<XMapTag>("URI=\"foo.ts\",BYTERANGE=\"{$UNDEFINED}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XMapTag>("URI=\"foo.ts\",BYTERANGE=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XMapTag>("URI=\"foo.ts\",BYTERANGE=\"10\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    ...
    result = OkTest<XMapTag>("URI=\"foo.ts\",BYTERANGE=\"{$ONE}{$TWO}@{$THREE}\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
...
TEST(HlsTagsTest, ParseXPartTag)
{
    ...
    ErrorTest<XPartTag>(std::nullopt, variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XPartTag>("", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XPartTag>("URI=\"foo.ts\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XPartTag>("DURATION=1", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XPartTag>("URI=\"\",DURATION=1", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    auto result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    ...
    // Test URI attribute
    ErrorTest<XPartTag>("URI=\"{$UNDEFINED}.ts\",DURATION=1", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XPartTag>("URI=\"{$BAR}.ts\",DURATION=1", variable_dict, sub_buffer);
    ...
    result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=" + base::NumberToString(MaxSeconds()), variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_TRUE(RoughlyEqual(result.tag.duration, base::Seconds(MaxSeconds())));
    EXPECT_EQ(result.tag.byte_range, std::nullopt);
    EXPECT_EQ(result.tag.independent, false);
    EXPECT_EQ(result.tag.gap, false);
    ErrorTest<XPartTag>("URI=\"foo.ts\",DURATION=" + base::NumberToString(MaxSeconds() + 1), variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    // Test BYTERANGE attribute
    ErrorTest<XPartTag>("URI=\"foo.ts\",DURATION=1,BYTERANGE=\"{$UNDEFINED}\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XPartTag>("URI=\"foo.ts\",DURATION=1,BYTERANGE=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,BYTERANGE=\"12\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_EQ(result.tag.duration, base::Seconds(1));
    EXPECT_EQ(result.tag.byte_range->length, 12u);
    EXPECT_EQ(result.tag.byte_range->offset, std::nullopt);
    ...
    result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,BYTERANGE=\"{$NUMBER}@3\"", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_EQ(result.tag.duration, base::Seconds(1));
    ...
    result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,INDEPENDENT=YES", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_EQ(result.tag.duration, base::Seconds(1));
    EXPECT_EQ(result.tag.byte_range, std::nullopt);
    ...
    for (std::string x : { "NO", "Y", "TRUE", "1", "yes" }) {
        result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,INDEPENDENT=" + x, variable_dict, sub_buffer);
        EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
        EXPECT_EQ(result.tag.duration, base::Seconds(1));
        EXPECT_EQ(result.tag.byte_range, std::nullopt);
        EXPECT_EQ(result.tag.independent, false);
    ...
    result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,GAP=YES", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
    EXPECT_EQ(result.tag.duration, base::Seconds(1));
    EXPECT_EQ(result.tag.byte_range, std::nullopt);
    EXPECT_EQ(result.tag.independent, false);
    ...
    for (std::string x : { "NO", "Y", "TRUE", "1", "yes" }) {
        result = OkTest<XPartTag>("URI=\"foo.ts\",DURATION=1,GAP=" + x, variable_dict, sub_buffer);
        EXPECT_EQ(result.tag.uri.Str(), "foo.ts");
        EXPECT_EQ(result.tag.duration, base::Seconds(1));
        EXPECT_EQ(result.tag.byte_range, std::nullopt);
        EXPECT_EQ(result.tag.independent, false);
...
TEST(HlsTagsTest, ParseXPartInfTag)
{
    ...
    ErrorTest<XPartInfTag>("PART-TARGET={$part-target}", ParseStatusCode::kMalformedTag);
    ErrorTest<XPartInfTag>("PART-TARGET=\"1\"", ParseStatusCode::kMalformedTag);
    ...
    ErrorTest<XPartInfTag>("PART-TARGET=10,PART-TARGET=10", ParseStatusCode::kMalformedTag);
    ...
    // Test the max value
    result = OkTest<XPartInfTag>("PART-TARGET=" + base::NumberToString(MaxSeconds()));
    EXPECT_TRUE(RoughlyEqual(result.tag.target_duration, base::Seconds(MaxSeconds())));
    ErrorTest<XPartInfTag>("PART-TARGET=" + base::NumberToString(MaxSeconds() + 1), ParseStatusCode::kValueOverflowsTimeDelta);
}
...
{
    RunTagIdenficationTest(ToTagName(MediaPlaylistTagName::kXPreloadHint), "#EXT-X-PRELOAD-HINT:TYPE=PART,URI=\"foo.ts\"\n", "TYPE=PART,URI=\"foo.ts\"");
...
TEST(HlsTagsTest, ParseXProgramDateTimeTag)
{
    RunTagIdenficationTest(
        ToTagName(MediaPlaylistTagName::kXProgramDateTime), "#EXT-X-PROGRAM-DATE-TIME:2010-02-19T14:54:23.031+08:00\n", "2010-02-19T14:54:23.031+08:00");
...
{
    RunTagIdenficationTest(
        ToTagName(MediaPlaylistTagName::kXRenditionReport), "#EXT-X-RENDITION-REPORT:URI=\"foo.m3u8\",LAST-MSN=200\n", "URI=\"foo.m3u8\",LAST-MSN=200");

    VariableDictionary dict = CreateBasicDictionary();
    VariableDictionary::SubstitutionBuffer subs;

    ErrorTest<XRenditionReportTag>(std::nullopt, dict, subs, ParseStatusCode::kMalformedTag);
    ErrorTest<XRenditionReportTag>("URI", dict, subs, ParseStatusCode::kMalformedAttributeList);
    ErrorTest<XRenditionReportTag>("URI=\"", dict, subs, ParseStatusCode::kMalformedAttributeList);
    ...
    result = OkTest<XRenditionReportTag>("URI=\"x\",LAST-PART=3,LAST-MSN=1", dict, subs);
...
{
    RunTagIdenficationTest<XServerControlTag>("#EXT-X-SERVER-CONTROL:SKIP-UNTIL=10\n", "SKIP-UNTIL=10");
    ...
    result = OkTest<XServerControlTag>("CAN-SKIP-UNTIL=50,CAN-SKIP-DATERANGES=YES,HOLD-BACK=60,PART-HOLD-BACK="
    ...
    ErrorTest<XServerControlTag>("CAN-SKIP-UNTIL=-5", ParseStatusCode::kMalformedTag);
    ErrorTest<XServerControlTag>("CAN-SKIP-UNTIL={$B}", ParseStatusCode::kMalformedTag);
    ErrorTest<XServerControlTag>("CAN-SKIP-UNTIL=\"5\"", ParseStatusCode::kMalformedTag);
    ...
    // Test max value
    result = OkTest<XServerControlTag>("CAN-SKIP-UNTIL=" + base::NumberToString(MaxSeconds()));
    EXPECT_TRUE(RoughlyEqual(result.tag.skip_boundary, base::Seconds(MaxSeconds())));
    EXPECT_EQ(result.tag.can_skip_dateranges, false);
    EXPECT_EQ(result.tag.hold_back, std::nullopt);
    EXPECT_EQ(result.tag.part_hold_back, std::nullopt);
    EXPECT_EQ(result.tag.can_block_reload, false);

    ErrorTest<XServerControlTag>("CAN-SKIP-UNTIL=" + base::NumberToString(MaxSeconds() + 1), ParseStatusCode::kValueOverflowsTimeDelta);

    // 'CAN-SKIP-DATERANGES' requires the presence of 'CAN-SKIP-UNTIL'
    ErrorTest<XServerControlTag>("CAN-SKIP-DATERANGES=YES", ParseStatusCode::kMalformedTag);
    result = OkTest<XServerControlTag>("CAN-SKIP-DATERANGES=YES,CAN-SKIP-UNTIL=50");
    EXPECT_TRUE(RoughlyEqual(result.tag.skip_boundary, base::Seconds(50)));
    EXPECT_EQ(result.tag.can_skip_dateranges, true);
    EXPECT_EQ(result.tag.hold_back, std::nullopt);
    EXPECT_EQ(result.tag.part_hold_back, std::nullopt);
    ...
    for (std::string x : { "NO", "Y", "TRUE", "1", "yes" }) {
        result = OkTest<XServerControlTag>("CAN-SKIP-DATERANGES=" + x + ",CAN-SKIP-UNTIL=50");
        EXPECT_TRUE(RoughlyEqual(result.tag.skip_boundary, base::Seconds(50)));
        EXPECT_EQ(result.tag.can_skip_dateranges, false);
        EXPECT_EQ(result.tag.hold_back, std::nullopt);
        EXPECT_EQ(result.tag.part_hold_back, std::nullopt);
    ...
    ErrorTest<XServerControlTag>("HOLD-BACK={$B}", ParseStatusCode::kMalformedTag);
    ErrorTest<XServerControlTag>("HOLD-BACK=\"5\"", ParseStatusCode::kMalformedTag);
    ...
    // Test max value
    result = OkTest<XServerControlTag>("HOLD-BACK=" + base::NumberToString(MaxSeconds()));
    EXPECT_EQ(result.tag.skip_boundary, std::nullopt);
    EXPECT_EQ(result.tag.can_skip_dateranges, false);
    EXPECT_TRUE(RoughlyEqual(result.tag.hold_back, base::Seconds(MaxSeconds())));
    EXPECT_EQ(result.tag.part_hold_back, std::nullopt);
    EXPECT_EQ(result.tag.can_block_reload, false);
    ErrorTest<XServerControlTag>("HOLD-BACK=" + base::NumberToString(MaxSeconds() + 1), ParseStatusCode::kValueOverflowsTimeDelta);

    // 'PART-HOLD-BACK' must be a valid DecimalFloatingPoint
    ErrorTest<XServerControlTag>("PART-HOLD-BACK=-5", ParseStatusCode::kMalformedTag);
    ErrorTest<XServerControlTag>("PART-HOLD-BACK={$B}", ParseStatusCode::kMalformedTag);
    ErrorTest<XServerControlTag>("PART-HOLD-BACK=\"5\"", ParseStatusCode::kMalformedTag);
    ...
    // Test max value
    result = OkTest<XServerControlTag>("PART-HOLD-BACK=" + base::NumberToString(MaxSeconds()));
    EXPECT_EQ(result.tag.skip_boundary, std::nullopt);
    EXPECT_EQ(result.tag.can_skip_dateranges, false);
    EXPECT_EQ(result.tag.hold_back, std::nullopt);
    EXPECT_TRUE(RoughlyEqual(result.tag.part_hold_back, base::Seconds(MaxSeconds())));
    EXPECT_EQ(result.tag.can_block_reload, false);
    ErrorTest<XServerControlTag>("PART-HOLD-BACK=" + base::NumberToString(MaxSeconds() + 1), ParseStatusCode::kValueOverflowsTimeDelta);
...
TEST(HlsTagsTest, ParseXSkipTag)
{
    ...
    ErrorTest<XSkipTag>(std::nullopt, variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("-1", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("UNKNOWN=10", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("SKIPPED-SEGMENTS=f", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("RECENTLY-REMOVED-DATERANGES=\"\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("SKIPPED-SEGMENTS=1,RECENTLY-REMOVED-DATERANGES=hello", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);
    ErrorTest<XSkipTag>("SKIPPED-SEGMENTS=1,RECENTLY-REMOVED-DATERANGES=\"\t\"", variable_dict, sub_buffer, ParseStatusCode::kMalformedTag);

    auto result = OkTest<XSkipTag>("SKIPPED-SEGMENTS=10", variable_dict, sub_buffer);
    EXPECT_EQ(result.tag.skipped_segments, 10u);
    EXPECT_EQ(result.tag.recently_removed_dateranges, std::nullopt);

    auto with_empty_ranges = OkTest<XSkipTag>("SKIPPED-SEGMENTS=10,RECENTLY-REMOVED-DATERANGES=\"\"", variable_dict, sub_buffer);
    ...
    auto with_some_ranges = OkTest<XSkipTag>("SKIPPED-SEGMENTS=10,RECENTLY-REMOVED-DATERANGES=\"F\tT\"", variable_dict, sub_buffer);
...
TEST(HlsTagsTest, ParseXTargetDurationTag)
{
    ...
    ErrorTest<XTargetDurationTag>(base::NumberToString(MaxSeconds() + 1), ParseStatusCode::kValueOverflowsTimeDelta);
}
