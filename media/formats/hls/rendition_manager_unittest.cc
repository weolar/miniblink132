...
std::string MakeVariantStr(uint64_t bandwidth, std::optional<std::string_view> resolution, std::optional<std::string_view> framerate)
{
...
RenditionManager::CodecSupportType GetCodecSupportType(std::string_view container, base::span<const std::string> codecs)
{
...
RenditionManager::CodecSupportType GetCodecSupportForSoftwareOnlyLinux(std::string_view container, base::span<const std::string> codecs)
{
    bool has_audio = false;
    bool has_video = false;
    for (const auto& codec : codecs) {
...
public:
    ...
    void _VariantSelected(AdaptationReason, const VariantStream* vs, const AudioRendition* ar)
    {
    ...
    {
        return base::BindRepeating(&HlsRenditionManagerTest::_VariantSelected, base::Unretained(this), AdaptationReason::kUserSelection);
    }

    decltype(auto) GetVariantWithAdaptation()
    {
        return base::BindRepeating(&HlsRenditionManagerTest::_VariantSelected, base::Unretained(this));
    }

    template <typename... Strings> RenditionManager GetRenditionManager(Strings... strings)
    {
        MultivariantPlaylistTestBuilder builder;
        builder.AppendLine("#EXTM3U");
        ([&] { builder.AppendLine(strings); }(), ...);
        return RenditionManager(builder.Parse(), base::BindRepeating(GetVariantWithAdaptation()), base::BindRepeating(&GetCodecSupportType));
    }

    template <typename... Strings>
    RenditionManager GetCustomSupportRenditionManager(
        base::RepeatingCallback<RenditionManager::CodecSupportType(std::string_view, base::span<const std::string>)> support_cb, Strings... strings)
    {
        MultivariantPlaylistTestBuilder builder;
        builder.AppendLine("#EXTM3U");
        ([&] { builder.AppendLine(strings); }(), ...);
        return RenditionManager(builder.Parse(), base::BindRepeating(GetVariantWithAdaptation()), std::move(support_cb));
    }
};

TEST_F(HlsRenditionManagerTest, MixedAVTypes)
{
    auto rm = GetRenditionManager("#EXT-X-STREAM-INF:BANDWIDTH=1280000,AVERAGE-BANDWIDTH=1000000", "http://example.com/low.m3u8",
        ...
        "#EXT-X-STREAM-INF:BANDWIDTH=65000,CODECS=\"audio.codec\"", "http://example.com/audio-only.m3u8");
...
{
    auto rm = GetRenditionManager("#EXT-X-STREAM-INF:BANDWIDTH=65000,CODECS=\"vvc1.00.00\"", "http://example.com/audio-only.m3u8",
        "#EXT-X-STREAM-INF:BANDWIDTH=65000,CODECS=\"sheet.music\"", "http://example.com/audio-only.m3u8",
        "#EXT-X-STREAM-INF:BANDWIDTH=65000,CODECS=\"av02.00.00\"", "http://example.com/audio-only.m3u8");
...
{
    auto rm = GetRenditionManager("#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=320x200", "video/cga.m3u8",
        "#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=480x320", "video/hvga.m3u8", "#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=800x480",
        "video/wvga.m3u8", "#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=1920x1080", "video/fhd.m3u8",
        "#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=2560x1440", "video/wqhd.m3u8",
        "#EXT-X-STREAM-INF:BANDWIDTH=10,CODECS=\"V\",RESOLUTION=7680x4320", "video/8kuhd.m3u8");
...
{
    auto rm = GetCustomSupportRenditionManager(base::BindRepeating(&GetCodecSupportForSoftwareOnlyLinux), "#EXT-X-INDEPENDENT-SEGMENTS",
    ...
    EXPECT_CALL(*this, VariantSelected("/v9/prog_index.m3u8", "/a1/prog_index.m3u8"));
    rm.Reselect(GetVariantCb());
}

TEST_F(HlsRenditionManagerTest, MultipleRenditionGroupsVariantsOutOfOrder)
{
    auto rm = GetRenditionManager("#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"stereo\",LANGUAGE=\"en\",NAME="
    ...
    EXPECT_CALL(*this, VariantSelected("/video/10000kbit.m3u8", "/audio/surround/en/320kbit.m3u8"));
    rm.Reselect(GetVariantCb());
    ...
    // video
    EXPECT_CALL(*this, VariantSelected("/video/6000kbit.m3u8", "/audio/surround/en/320kbit.m3u8"));
    ...
    EXPECT_CALL(*this, VariantSelected("/video/800kbit.m3u8", "/audio/stereo/en/128kbit.m3u8"));
    ...
    EXPECT_CALL(*this, VariantSelected("/video/800kbit.m3u8", "/audio/stereo/none/128kbit.m3u8"));
    ...
    EXPECT_CALL(*this, VariantSelected("/video/10000kbit.m3u8", "/audio/surround/none/320kbit.m3u8"));
    rm.UpdateNetworkSpeed(10285395);
    ...
    EXPECT_CALL(*this, VariantSelected("/video/800kbit.m3u8", "/audio/stereo/de/128kbit.m3u8"));
...
{
    {
        ...
        auto variants = GetRenditionManager(
            MakeVariantStr(1234, "1920x1080", std::nullopt), "playlist1.m3u8", MakeVariantStr(1234, "1366x768", std::nullopt), "playlist2.m3u8")
...

    {
        // No differentiation
        auto variants = GetRenditionManager(
            MakeVariantStr(1234, "1920x1080", std::nullopt), "playlist1.m3u8", MakeVariantStr(1234, "1920x1080", std::nullopt), "playlist2.m3u8")
                            .GetSelectableVariants();
...

    {
        ...
        auto variants
            = GetRenditionManager(MakeVariantStr(1234, "1920x1080", "24.00"), "playlist1.m3u8", MakeVariantStr(1234, "1920x1080", "60.00"), "playlist2.m3u8")
                  .GetSelectableVariants();
...

    {
        // No differentiation
        auto variants
            = GetRenditionManager(MakeVariantStr(1234, "1920x1080", "60.00"), "playlist1.m3u8", MakeVariantStr(1234, "1920x1080", "60.00"), "playlist2.m3u8")
                  .GetSelectableVariants();
        ASSERT_EQ(variants[0].label().value(), "Stream: 1");
        ASSERT_EQ(variants[1].label().value(), "Stream: 2");
    }

    {
        ...
        auto variants = GetRenditionManager(
            MakeVariantStr(831270, "1920x1080", "60.00"), "playlist1.m3u8", MakeVariantStr(1144430, "1920x1080", "60.00"), "playlist2.m3u8")
                            .GetSelectableVariants();
...

    {
        ...
        auto variants = GetRenditionManager(MakeVariantStr(831270, "1920x1080", "60.00"), "playlist1.m3u8", MakeVariantStr(1144430, "1920x1080", "24.00"),
            "playlist2.m3u8", MakeVariantStr(1234, "1366x768", "60.00"), "playlist3.m3u8", MakeVariantStr(67989, "1366x768", "24.00"), "playlist4.m3u8")
                            .GetSelectableVariants();
