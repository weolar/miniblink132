...
bool CommonParserState::CheckVersion(types::DecimalInteger expected_version) const
{
...
ParseStatus::Or<M3uTag> CheckM3uTag(SourceLineIterator* src_iter)
{
    ...
    if (!item_result.has_value()) {
        return ParseStatus(ParseStatusCode::kPlaylistMissingM3uTag).AddCause(std::move(item_result).error());
    }
    ...
    if (auto* tag_item = absl::get_if<TagItem>(&item)) {
        ...
        if (tag_item->GetName() != ToTagName(CommonTagName::kM3u) || tag_item->GetLineNumber() != 1) {
        ...
        if (!result.has_value()) {
            return ParseStatus(ParseStatusCode::kPlaylistMissingM3uTag).AddCause(std::move(result).error());
        }
...
std::optional<ParseStatus> ParseCommonTag(TagItem tag, CommonParserState* state)
{
    ...
    case CommonTagName::kXDefine: {
        ...
        if (!state->variable_dict.Insert(tag_value.name, std::string { *tag_value.value })) {
...
ParseStatus::Or<GURL> ParseUri(UriItem item, const GURL& playlist_uri, const CommonParserState& state, VariableDictionary::SubstitutionBuffer& sub_buffer)
{
    ...
    auto resolved_uri = playlist_uri.Resolve(std::move(uri_str_result).value().Str());
