...
Playlist::Playlist(GURL uri, types::DecimalInteger version, bool independent_segments)
    : uri_(std::move(uri))
    , version_(version)
...
// static
ParseStatus::Or<Playlist::Identification> Playlist::IdentifyPlaylist(std::string_view source)
{
    ...
    while (!playlist_kind.has_value() || !version_tag.has_value()) {
        ...
        if (auto* tag = absl::get_if<TagItem>(&item)) {
            ...
            if (tag_name == ToTagName(CommonTagName::kXVersion)) {
                ...
                if (version_tag->version < Playlist::kMinSupportedVersion || version_tag->version > Playlist::kMaxSupportedVersion) {
    ...
    return Identification {
        ...
        .kind = playlist_kind ? playlist_kind.value() : Kind::kMultivariantPlaylist,
    };
}
