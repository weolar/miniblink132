...
public:
    MultivariantPlaylist(base::PassKey<MultivariantPlaylist>, GURL uri, types::DecimalInteger version, bool independent_segments,
    ...
    const std::vector<VariantStream>& GetVariants() const
    {
        return variants_;
    }
    ...
    static ParseStatus::Or<scoped_refptr<MultivariantPlaylist>> Parse(
        std::string_view source, GURL uri, types::DecimalInteger version, TagRecorder* tag_recorder = nullptr);
