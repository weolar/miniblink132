...
AudioRendition::AudioRendition(base::PassKey<AudioRenditionGroup>, CtorArgs args)
...
AudioRenditionGroup::AudioRenditionGroup(base::PassKey<MultivariantPlaylist>, std::string id)
    : id_(std::move(id))
{
}
...
ParseStatus::Or<absl::monostate> AudioRenditionGroup::AddRendition(base::PassKey<MultivariantPlaylist>, XMediaTag tag, const GURL& playlist_uri)
{
    ...
    auto& rendition = renditions_.emplace_back(base::PassKey<AudioRenditionGroup>(),
...
const AudioRendition* AudioRenditionGroup::GetRendition(std::string_view name) const
{
