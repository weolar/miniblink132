...
GetNextVariableResult GetNextVariable(const SourceString input)
{
    ...
    for (size_t ref_start = input.Str().find("{$"); ref_start != std::string_view::npos; ref_start = input.Str().find("{$", ref_start + 2)) {
        ...
        // searching.
        auto var_name_result = types::VariableName::Parse(remaining_input.Consume(ref_end));
        ...
        return GetNextVariableResult { .head = head, .tail = std::make_pair(var_name, remaining_input) };
    }
...
VariableDictionary& VariableDictionary::operator=(VariableDictionary&&) = default;

std::optional<std::string_view> VariableDictionary::Find(types::VariableName name) const&
{
...
{
    return entries_.try_emplace(std::move(name).GetName(), std::make_unique<std::string>(std::move(value))).second;
}

ParseStatus::Or<ResolvedSourceString> VariableDictionary::Resolve(SourceString input, SubstitutionBuffer& buffer) const
{
    ...
    if (!next_var.tail) {
        return ResolvedSourceString::Create({}, input.Line(), input.Column(), input.Str(), ResolvedSourceString::SubstitutionState::kNoSubstitutions);
    }
    ...
    if (next_var.head.Empty() && next_var.tail->second.Empty()) {
        ...
        if (!value) {
            return ParseStatus(ParseStatusCode::kVariableUndefined).WithData("key", next_var.tail->first.GetName());
        }

        return ResolvedSourceString::Create({}, input.Line(), input.Column(), *value, ResolvedSourceString::SubstitutionState::kContainsSubstitutions);
    }
    ...
    return ResolvedSourceString::Create({}, input.Line(), input.Column(), string_buf, ResolvedSourceString::SubstitutionState::kContainsSubstitutions);
}
