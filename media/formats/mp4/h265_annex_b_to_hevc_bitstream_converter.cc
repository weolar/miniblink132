...
H265AnnexBToHevcBitstreamConverter::~H265AnnexBToHevcBitstreamConverter() = default;

const mp4::HEVCDecoderConfigurationRecord& H265AnnexBToHevcBitstreamConverter::GetCurrentConfig()
{
    return config_;
}

MP4Status H265AnnexBToHevcBitstreamConverter::ConvertChunk(
    const base::span<const uint8_t> input, base::span<uint8_t> output, bool* config_changed_out, size_t* size_out)
{
    ...
    while ((result = parser_.AdvanceToNextNALU(&nalu)) != H265Parser::kEOStream) {
        ...
        case H265NALU::SPS_NUT: {
            ...
            id2sps_.insert_or_assign(sps_id, blob(nalu.data.get(), (nalu.data + base::checked_cast<size_t>(nalu.size)).get()));
        ...
        case H265NALU::VPS_NUT: {
            ...
            id2vps_.insert_or_assign(vps_id, blob(nalu.data.get(), (nalu.data + base::checked_cast<size_t>(nalu.size)).get()));
        ...
        case H265NALU::PPS_NUT: {
            ...
            id2pps_.insert_or_assign(pps_id, blob(nalu.data.get(), (nalu.data + base::checked_cast<size_t>(nalu.size)).get()));
        ...
        case H265NALU::CRA_NUT: {
            ...
            if (new_active_sps_id != active_sps_id_ || new_active_vps_id != active_vps_id_) {
                if (!config_changed) {
                    DCHECK(nalu.nal_unit_type == H265NALU::IDR_W_RADL || nalu.nal_unit_type == H265NALU::IDR_N_LP)
    ...
    for (auto& unit : slice_units) {
        bool written_ok = writer.WriteU32BigEndian(unit.size)
            && writer.Write(
                ...
                // of a pointer.
                UNSAFE_TODO(base::span(unit.data.get(), base::checked_cast<size_t>(unit.size))));
        if (!written_ok) {
            return MP4Status::Codes::kBufferTooSmall;
        }
    }
    ...
    if (config_changed) {
        ...
        config_.general_profile_compatibility_flags = ptl.general_profile_compatibility_flags;
        ...
        config_.min_spatial_segmentation_idc = active_sps->vui_parameters.min_spatial_segmentation_idc;
        if (active_sps->vui_parameters.min_spatial_segmentation_idc == 0) {
            config_.parallelismType = mp4::HEVCDecoderConfigurationRecord::kMixedParallel;
        } else if (active_pps->entropy_coding_sync_enabled_flag && active_pps->tiles_enabled_flag) {
            config_.parallelismType = mp4::HEVCDecoderConfigurationRecord::kMixedParallel;
        } else if (active_pps->entropy_coding_sync_enabled_flag) {
            config_.parallelismType = mp4::HEVCDecoderConfigurationRecord::kWaveFrontParallel;
        } else if (active_pps->tiles_enabled_flag) {
            config_.parallelismType = mp4::HEVCDecoderConfigurationRecord::kTileParallel;
        } else {
            config_.parallelismType = mp4::HEVCDecoderConfigurationRecord::kSliceParallel;
        }
