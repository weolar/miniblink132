...
static const uint8_t kExpected[] = { 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x03, 0x00, 0x00, 0x00, 0x01, 0x04, 0x05, 0x06, 0x07 };

static const uint8_t kExpectedParamSets[]
    = { 0x00, 0x00, 0x00, 0x01, 0x67, 0x12, 0x00, 0x00, 0x00, 0x01, 0x67, 0x34, 0x00, 0x00, 0x00, 0x01, 0x68, 0x56, 0x78 };
...
std::ostream& operator<<(std::ostream& os, const BitstreamConverter::AnalysisResult& r)
{
    os << "{ is_conformant: " << (r.is_conformant.has_value() ? (r.is_conformant.value() ? "true" : "false") : "nullopt/unknown")
       << ", is_keyframe: " << (r.is_keyframe.has_value() ? (r.is_keyframe.value() ? "true" : "false") : "nullopt/unknown") << " }";
    return os;
}

static std::string AnnexBToString(const std::vector<uint8_t>& buffer, const std::vector<SubsampleEntry>& subsamples)
{
    std::stringstream ss;
    ...
    while (parser.AdvanceToNextNALU(&nalu) == H264Parser::kOk) {
        size_t subsample_index = AVC::FindSubsampleIndex(buffer, &subsamples, nalu.data);
        if (!first) {
...
TEST_P(AVCConversionTest, ParseCorrectly)
{
    ...
    EXPECT_PRED2(AnalysesMatch, AVC::AnalyzeAnnexB(buf.data(), buf.size(), subsamples), expected);
...
INSTANTIATE_TEST_SUITE_P(AVCConversionTestValues, AVCConversionTest, ::testing::Values(1, 2, 4));
...
{
    std::string str = "AUD SPS SPSExt SPS PPS SEI SEI Prefix I P FILL EOSeq EOStr";
    std::vector<uint8_t> buf;
    std::vector<SubsampleEntry> subsamples;
...
TEST_F(AVCConversionTest, ValidAnnexBConstructs)
{
    ...
    for (size_t i = 0; i < std::size(test_cases); ++i) {
        ...
        EXPECT_PRED2(AnalysesMatch, AVC::AnalyzeAnnexB(buf.data(), buf.size(), subsamples), expected) << "'" << test_cases[i].case_string << "' failed";
    }
}
...
TEST_F(AVCConversionTest, InvalidAnnexBConstructs)
{
    struct {
        ...
        { "P SPS P", false }, // SPS after first VCL would indicate a new access unit.
    };
...
TEST_F(AVCConversionTest, InsertParamSetsAnnexB)
{
    ...
    for (size_t i = 0; i < std::size(test_cases); ++i) {
        ...
        EXPECT_TRUE(AVC::InsertParamSetsAnnexB(avc_config, &buf, &subsamples)) << "'" << test_cases[i].input << "' insert failed.";
        EXPECT_PRED2(AnalysesMatch, AVC::AnalyzeAnnexB(buf.data(), buf.size(), subsamples), expected)
            << "'" << test_cases[i].input << "' created invalid AnnexB.";
        EXPECT_EQ(test_cases[i].expected, AnnexBToString(buf, subsamples)) << "'" << test_cases[i].input << "' generated unexpected output.";
    }
}
