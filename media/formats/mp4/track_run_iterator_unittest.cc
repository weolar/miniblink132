...
{
    int64_t max_seconds = std::numeric_limits<int64_t>::max() / base::Time::kMicrosecondsPerSecond;
...
protected:
    ...
    void SetFlagsOnSamples(const std::string& sample_info, TrackFragmentRun* trun)
    {
        ...
        std::vector<std::string> flags_data = base::SplitString(sample_info, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    void AddEncryption(Track* track)
    {
        ...
        sinf->info.track_encryption.default_kid.assign(kKeyId, kKeyId + std::size(kKeyId));
    }

    // Add SampleGroupDescription Box to track level sample table and to
    // fragment. Populate SampleToGroup Box from input array.
    void AddCencSampleGroup(Track* track, TrackFragment* frag, const SampleToGroupEntry* sample_to_group_entries, size_t num_entries)
    {
        ...
        track_cenc_group.entries[0].key_id.assign(kTrackCencSampleGroupKeyId, kTrackCencSampleGroupKeyId + std::size(kTrackCencSampleGroupKeyId));
        ...
        frag->sample_group_description.entries[1].key_id.assign(
            kFragmentCencSampleGroupKeyId, kFragmentCencSampleGroupKeyId + std::size(kFragmentCencSampleGroupKeyId));
        frag->sample_group_description.entries[2].is_encrypted = true;
        frag->sample_group_description.entries[2].iv_size = 16;
        frag->sample_group_description.entries[2].key_id.assign(kKeyId, kKeyId + std::size(kKeyId));

        frag->sample_to_group.grouping_type = FOURCC_SEIG;
        frag->sample_to_group.entries.assign(sample_to_group_entries, sample_to_group_entries + num_entries);
    }
    ...
    void AddSampleEncryption(uint8_t use_subsample_flag, TrackFragment* frag)
    {
        ...
        if (use_subsample_flag) {
            frag->sample_encryption.sample_encryption_data.assign(
                kSampleEncryptionDataWithSubsamples, kSampleEncryptionDataWithSubsamples + std::size(kSampleEncryptionDataWithSubsamples));
        } else {
            frag->sample_encryption.sample_encryption_data.assign(
                kSampleEncryptionDataWithoutSubsamples, kSampleEncryptionDataWithoutSubsamples + std::size(kSampleEncryptionDataWithoutSubsamples));
        }
    ...
    void AddSampleEncryptionCbcs(TrackFragment* frag)
    {
        ...
        frag->sample_encryption.sample_encryption_data.assign(kSampleEncryptionDataWithSubsamplesAndConstantIv,
            kSampleEncryptionDataWithSubsamplesAndConstantIv + std::size(kSampleEncryptionDataWithSubsamplesAndConstantIv));
...
TEST_F(TrackRunIteratorTest, BasicOperationTest)
{
    ...
    iter_->AdvanceSample();
    EXPECT_EQ(moof.tracks[0].runs[1].data_offset + moof.tracks[0].header.default_sample_size, iter_->sample_offset());
    iter_->AdvanceRun();
    EXPECT_FALSE(iter_->IsRunValid());
}
...
TEST_F(TrackRunIteratorTest, DecryptConfigTestWithSampleEncryptionAndNoSubsample)
{
    AddEncryption(&moov_.tracks[1]);
    iter_.reset(new TrackRunIterator(&moov_, &media_log_));

    MovieFragment moof = CreateFragment();
    AddSampleEncryption(!SampleEncryption::kUseSubsampleEncryption, &moof.tracks[1]);
    ...
    EXPECT_EQ(std::string(reinterpret_cast<const char*>(kKeyId), std::size(kKeyId)), config->key_id());
    EXPECT_EQ(std::string(reinterpret_cast<const char*>(kIv1), std::size(kIv1)), config->iv());
    EXPECT_EQ(config->subsamples().size(), 0u);
    iter_->AdvanceSample();
    config = iter_->GetDecryptConfig();
    EXPECT_EQ(std::string(reinterpret_cast<const char*>(kIv2), std::size(kIv2)), config->iv());
    EXPECT_EQ(config->subsamples().size(), 0u);
}

TEST_F(TrackRunIteratorTest, DecryptConfigTestWithSampleEncryptionAndSubsample)
{
    AddEncryption(&moov_.tracks[1]);
    iter_.reset(new TrackRunIterator(&moov_, &media_log_));
    ...
    MovieFragment moof = CreateFragment();
    AddSampleEncryption(SampleEncryption::kUseSubsampleEncryption, &moof.tracks[1]);
    const SampleToGroupEntry kSampleToGroupTable[] = { // Associated with the second entry in SampleGroupDescription Box.
    ...
    AddCencSampleGroup(&moov_.tracks[1], &moof.tracks[1], kSampleToGroupTable, std::size(kSampleToGroupTable));
    ...
    iter_->AdvanceSample();
    config = iter_->GetDecryptConfig();
    EXPECT_EQ(std::string(reinterpret_cast<const char*>(kIv3), std::size(kIv3)), config->iv());
...
TEST_F(TrackRunIteratorTest, CencSampleGroupTest)
{
    ...
    AddCencSampleGroup(&moov_.tracks[0], &moof.tracks[0], kSampleToGroupTable, std::size(kSampleToGroupTable));

    iter_.reset(new TrackRunIterator(&moov_, &media_log_));
    ASSERT_TRUE(InitMoofWithArbitraryAuxInfo(&moof));

    std::string cenc_sample_group_key_id(kFragmentCencSampleGroupKeyId, kFragmentCencSampleGroupKeyId + std::size(kFragmentCencSampleGroupKeyId));
...
TEST_F(TrackRunIteratorTest, CencSampleGroupWithTrackEncryptionBoxTest)
{
    ...
    const SampleToGroupEntry kSampleToGroupTable[] = { // Associated with the 2nd entry in fragment SampleGroupDescription Box.
    ...
    std::string track_cenc_sample_group_key_id(kTrackCencSampleGroupKeyId, kTrackCencSampleGroupKeyId + std::size(kTrackCencSampleGroupKeyId));
    std::string fragment_cenc_sample_group_key_id(kFragmentCencSampleGroupKeyId, kFragmentCencSampleGroupKeyId + std::size(kFragmentCencSampleGroupKeyId));

    for (size_t i = 0; i < kSampleToGroupTable[0].sample_count; ++i) {
        EXPECT_TRUE(iter_->is_encrypted());
        EXPECT_EQ(fragment_cenc_sample_group_key_id, iter_->GetDecryptConfig()->key_id());
        iter_->AdvanceSample();
    }
    ...
    for (size_t i = 0; i < kSampleToGroupTable[3].sample_count; ++i) {
        EXPECT_TRUE(iter_->is_encrypted());
        EXPECT_EQ(track_cenc_sample_group_key_id, iter_->GetDecryptConfig()->key_id());
        iter_->AdvanceSample();
    }
