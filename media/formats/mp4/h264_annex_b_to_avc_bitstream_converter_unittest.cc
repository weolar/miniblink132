...
std::vector<uint8_t> ReadTestFile(std::string name)
{
    ...
    path = path.Append(FILE_PATH_LITERAL("media/formats/mp4/h264_annex_b_fuzz_corpus")).AppendASCII(name);
...
{
    std::string chunks[] = { "chunk1-config-idr.bin", "chunk2-non-idr.bin", "chunk3-non-idr.bin", "chunk4-non-idr.bin", "chunk5-non-idr.bin",
        "chunk6-config-idr.bin", "chunk7-non-idr.bin", "pps_neq_sps_config_idr.bin" };
    ...
    for (std::string& name : chunks) {
        ...
        auto status = converter.ConvertChunk(input, output, &config_changed, &desired_size);
...
{
    std::vector<uint8_t> sps { 0x00, 0x00, 0x00, 0x01, 0x27, 0x42, 0x00, 0x1E, 0x89, 0x8A, 0x12, 0x05, 0x01, 0x7F, 0xCA, 0x80 };
    ...
    std::vector<uint8_t> first_frame_idr {
        0x00, 0x00, 0x00, 0x01, 0x25, 0xb4, 0x00, 0x10, 0x00, 0x24, 0xff, 0xff, 0xf8, 0x7a, 0x28, 0x00, 0x08, 0x0a, 0x7b, 0xdd
    ...
    first_chunk.insert(first_chunk.end(), first_frame_idr.begin(), first_frame_idr.end());

    std::vector<uint8_t> second_non_idr_chunk {
        0x00, 0x00, 0x00, 0x01, 0x21, 0xd8, 0x00, 0x80, 0x04, 0x95, 0x9d, 0x45, 0x70, 0xd9, 0xbe, 0x21, 0xff, 0x87, 0x20, 0x03, 0x9c, 0x66, 0x84, 0xe1
        // Encoded data omitted here, it's not important for NALU parsing
    };
    ...
    auto status = converter.ConvertChunk(first_chunk, output, &config_changed, nullptr);
    EXPECT_TRUE(status.is_ok()) << status.message();
    EXPECT_TRUE(config_changed);

    status = converter.ConvertChunk(second_non_idr_chunk, output, &config_changed, nullptr);
    EXPECT_TRUE(status.is_ok()) << status.message();
    EXPECT_FALSE(config_changed);
}
...
{
    std::vector<uint8_t> sps { 0, 0, 0, 1, 39, 100, 0, 31, 172, 86, 128, 80, 5, 186, 106, 12, 2, 12, 4 };
...
TEST(H264AnnexBToAvcBitstreamConverterTest, Failure)
{
    ...
    std::vector<uint8_t> input { 0x0, 0x0, 0x0, 0x1, 0x9, 0x10, 0x0, 0x0, 0x0, 0x1, 0x67, 0x42 };
