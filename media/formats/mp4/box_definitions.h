...
struct MEDIA_EXPORT SMPTE2086MasteringDisplayMetadataBox : MasteringDisplayColorVolume {
    bool Parse(BoxReader* reader) override;
    FourCC BoxType() const override;
};
...
struct MEDIA_EXPORT VideoSampleEntry : Box {
    ...
    static VideoColorSpace ConvertColorParameterInformationToColorSpace(const ColorParameterInformation& info);
};
...
struct MEDIA_EXPORT IamfSpecificBox : Box {
    ...
    bool ReadOBUHeader(BufferReader* reader, uint8_t* obu_type, uint32_t* obu_size);
