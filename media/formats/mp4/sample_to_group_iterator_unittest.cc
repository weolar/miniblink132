...
public:
    SampleToGroupIteratorTest()
    {
        ...
        for (size_t i = 0; i < std::size(kCompactSampleToGroupTable); ++i) {
            for (uint32_t j = 0; j < kCompactSampleToGroupTable[i].sample_count; ++j) {
                sample_to_group_table_.push_back(kCompactSampleToGroupTable[i].group_description_index);
            }
        }

        sample_to_group_.entries.assign(kCompactSampleToGroupTable, kCompactSampleToGroupTable + std::size(kCompactSampleToGroupTable));
        sample_to_group_iterator_.reset(new SampleToGroupIterator(sample_to_group_));
    }

    SampleToGroupIteratorTest(const SampleToGroupIteratorTest&) = delete;
    SampleToGroupIteratorTest& operator=(const SampleToGroupIteratorTest&) = delete;
...
{
    ASSERT_EQ(sample_to_group_table_[0], sample_to_group_iterator_->group_description_index());
    for (uint32_t sample = 1; sample < sample_to_group_table_.size(); ++sample) {
        ASSERT_TRUE(sample_to_group_iterator_->Advance());
        ASSERT_EQ(sample_to_group_table_[sample], sample_to_group_iterator_->group_description_index());
