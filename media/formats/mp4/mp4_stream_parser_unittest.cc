...
public:
        ...
        , verifying_keyframeness_sequence_(false)
    {
        ...
        parser_.reset(new MP4StreamParser(audio_object_types, false, false, false, false));
    }
...
protected:
    ...
    bool AppendAllDataThenParseInPieces(base::span<const uint8_t> data, size_t piece_size)
    {
        ...
        while (has_more_data) {
            ...
            has_more_data = parse_result == StreamParser::ParseStatus::kSuccessHasMoreData;
    ...
    void InitF(const StreamParser::InitParameters& expected_params, const StreamParser::InitParameters& params)
    {
        ...
        EXPECT_EQ(expected_params.detected_audio_track_count, params.detected_audio_track_count);
        EXPECT_EQ(expected_params.detected_video_track_count, params.detected_video_track_count);
    }
    ...
    bool NewConfigF(std::unique_ptr<MediaTracks> tracks)
    {
        ...
        for (const auto& track : tracks->tracks()) {
            ...
            if (track->type() == MediaTrack::Type::kAudio) {
                ...
                DVLOG(1) << "track_id=" << track_id
                         << " audio config=" << (audio_decoder_config_.IsValidConfig() ? audio_decoder_config_.AsHumanReadableString() : "INVALID");
                audio_config_count++;
            } else if (track->type() == MediaTrack::Type::kVideo) {
                video_track_id_ = track_id;
                video_decoder_config_ = tracks->getVideoConfig(track_id);
                DVLOG(1) << "track_id=" << track_id
                         << " video config=" << (video_decoder_config_.IsValidConfig() ? video_decoder_config_.AsHumanReadableString() : "INVALID");
                video_config_count++;
            }
        }
    ...
    bool NewBuffersF(const StreamParser::BufferQueueMap& buffer_queue_map)
    {
        ...
        for (const auto& [track_id, buffer_queue] : buffer_queue_map) {
            ...
            if (lowest_end_dts == kNoDecodeTimestamp || lowest_end_dts > buffer_queue.back()->GetDecodeTimestamp())
                lowest_end_dts = buffer_queue.back()->GetDecodeTimestamp();

            for (const auto& buf : buffer_queue) {
                DVLOG(3) << "  track_id=" << buf->track_id() << ", size=" << buf->size() << ", pts=" << buf->timestamp().InSecondsF()
                         << ", dts=" << buf->GetDecodeTimestamp().InSecondsF() << ", dur=" << buf->duration().InSecondsF();
                ...
                if (verifying_keyframeness_sequence_) {
                    keyframeness_cb_.Run(buf->is_key_frame() ? Keyframeness::kKeyframe : Keyframeness::kNonKeyframe);
                }
            }
        }
    ...
    {
        parser_->Init(base::BindOnce(&MP4StreamParserTest::InitF, base::Unretained(this), params),
            base::BindRepeating(&MP4StreamParserTest::NewConfigF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::NewBuffersF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::KeyNeededF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::NewSegmentF, base::Unretained(this)),
            base::BindRepeating(&MP4StreamParserTest::EndOfSegmentF, base::Unretained(this)), &media_log_);
    }
    ...
    {
        InitializeParserWithInitParametersExpectations(GetDefaultInitParametersExpectations());
    }
    ...
    bool ParseMP4File(const std::string& filename, int append_bytes)
    {
        ...
        do {
            size_t chunk_size = std::min(static_cast<size_t>(append_bytes), static_cast<size_t>(end - start));
            ...
            EXPECT_TRUE(AppendAllDataThenParseInPieces(buffer->AsSpan().subspan(start, chunk_size), (chunk_size > 7) ? (chunk_size - 7) : chunk_size));
            start += chunk_size;
        } while (start < end);
...
TEST_F(MP4StreamParserTest, Flush)
{
    ...
    EXPECT_TRUE(AppendAllDataThenParseInPieces(buffer->AsSpan().first(65536u), 512));
    parser_->Flush();
...
TEST_F(MP4StreamParserTest, UnknownDuration_V0_AllBitsSet)
{
    ...
    ParseMP4File("bear-1280x720-av_frag-initsegment-mvhd_version_0-mvhd_duration_bits_all_"
                 "set.mp4",
        512);
}
...
TEST_F(MP4StreamParserTest, AVC_Keyframeness_Mismatches_Container)
{
    ...
    InitializeParserWithInitParametersExpectations(params);
    verifying_keyframeness_sequence_ = true;
    EXPECT_MEDIA_LOG(DebugLog("ISO-BMFF container metadata for video frame indicates that the frame is "
                              "not a keyframe, but the video frame contents indicate the opposite."));
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kKeyframe));
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kNonKeyframe));
    ParseMP4File("bear-640x360-v-2frames-keyframe-is-non-sync-sample_frag.mp4", 512);
}
...
TEST_F(MP4StreamParserTest, AVC_NonKeyframeness_Mismatches_Container)
{
    ...
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kNonKeyframe));
    ParseMP4File("bear-640x360-v-2frames-nonkeyframe-is-sync-sample_frag.mp4", 512);
}
...
TEST_F(MP4StreamParserTest, NoMoovAfterFlush)
{
    ...
    EXPECT_TRUE(AppendAllDataThenParseInPieces(buffer->AsSpan().subspan(kFirstMoofOffset, buffer->size() - kFirstMoofOffset), 512));
}
...
TEST_F(MP4StreamParserTest, HEVC_in_MP4_container)
{
    ...
    EXPECT_EQ(expect_success, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
...
TEST_F(MP4StreamParserTest, HEVC_Keyframeness_Mismatches_Container)
{
    ...
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kKeyframe));
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kNonKeyframe));
    ParseMP4File("bear-320x240-v-2frames-keyframe-is-non-sync-sample_frag-hevc.mp4", 256);
}
...
TEST_F(MP4StreamParserTest, HEVC_NonKeyframeness_Mismatches_Container)
{
    ...
    EXPECT_MEDIA_LOG(DebugLog("ISO-BMFF container metadata for video frame indicates that the frame is "
                              "a keyframe, but the video frame contents indicate the opposite."));
    EXPECT_CALL(keyframeness_cb_, Run(Keyframeness::kNonKeyframe));
    ParseMP4File("bear-320x240-v-2frames-nonkeyframe-is-sync-sample_frag-hevc.mp4", 256);
}
#endif
...
{
    base::flat_set<int> audio_object_types;
    parser_.reset(new MP4StreamParser(audio_object_types, false, false, false, true));
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("glass-blowing2-dolby-vision-profile-5-frag.mp4");
    EXPECT_EQ(expect_success, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
...
TEST_F(MP4StreamParserTest, DemuxingDVProfile8WithDVMimeTypeSourceBuffer)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("glass-blowing2-dolby-vision-profile-8-1-frag.mp4");
    EXPECT_EQ(expect_success, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    EXPECT_EQ(VideoCodec::kDolbyVision, video_decoder_config_.codec());
...
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    bool expect_success = true;
    EXPECT_MEDIA_LOG(InfoLog("Dolby Vision video track with track_id=1 is using cross-compatible "
...
TEST_F(MP4StreamParserTest, DemuxingAc4Ims)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("ac4-only-ims-frag.mp4");
    EXPECT_EQ(kExpectSuccess, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
}
...
TEST_F(MP4StreamParserTest, DemuxingAc4AJoc)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("ac4-only-ajoc-frag.mp4");
    EXPECT_EQ(kExpectSuccess, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
}
...
TEST_F(MP4StreamParserTest, DemuxingAc4ChannelBasedCoding)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("ac4-only-channel-based-coding-frag.mp4");
    EXPECT_EQ(kExpectSuccess, AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
}
...
{
    parser_.reset(new MP4StreamParser(base::flat_set<int>(), false, true, false, false));
...
TEST_F(MP4StreamParserTest, Flac192kHz)
{
    ...
    scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile("bear-flac-192kHz_frag.mp4");
    EXPECT_TRUE(AppendAllDataThenParseInPieces(buffer->AsSpan(), 512));
}
...
TEST_F(MP4StreamParserTest, VideoColorSpaceInvalidValues)
{
    ...
    MediaSerialize(VideoSampleEntry::ConvertColorParameterInformationToColorSpace(invalid));
}
...
using MatrixRotationTestCaseParam = std::tuple<double, double, VideoTransformation>;

class MP4StreamParserRotationMatrixEvaluatorTest : public ::testing::TestWithParam<MatrixRotationTestCaseParam> {
public:
...
TEST_P(MP4StreamParserRotationMatrixEvaluatorTest, RotationCalculation)
{
    ...
    VideoTransformation actual = parser_->CalculateRotation(track_header, movie_header);
...
MatrixRotationTestCaseParam rotation_test_cases[6] = {
    ...
    { 0, -1, VideoTransformation(VIDEO_ROTATION_90) }, // cos(90) = 0, sin(90) =-1
    { -1, 0, VideoTransformation(VIDEO_ROTATION_180) }, // cos(180)=-1, sin(180)= 0
    { 0, 1, VideoTransformation(VIDEO_ROTATION_270) }, // cos(270)= 0, sin(270)= 1
    { 1, 1, VideoTransformation(VIDEO_ROTATION_0) }, // Error case
    { 5, 5, VideoTransformation(VIDEO_ROTATION_0) }, // Error case
};
INSTANTIATE_TEST_SUITE_P(CheckMath, MP4StreamParserRotationMatrixEvaluatorTest, testing::ValuesIn(rotation_test_cases));
