...
H26xAnnexBToBitstreamConverter::H26xAnnexBToBitstreamConverter(VideoCodec video_codec)
{
...
{
    if (h264_converter_) {
        return h264_converter_->ConvertChunk(input, output, config_changed_out, size_out);
    }
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    if (h265_converter_) {
        return h265_converter_->ConvertChunk(input, output, config_changed_out, size_out);
    }
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    NOTREACHED();
}

scoped_refptr<DecoderBuffer> H26xAnnexBToBitstreamConverter::Convert(const base::span<const uint8_t> input)
{
    bool config_changed = false;
    size_t desired_size = 0;

    auto status = ConvertChunk(input, base::span<uint8_t>(), &config_changed, &desired_size);
...
VideoEncoder::CodecDescription H26xAnnexBToBitstreamConverter::GetCodecDescription()
{
    if (h264_converter_) {
...
CodecProfileLevel H26xAnnexBToBitstreamConverter::GetCodecProfileLevel()
{
    if (h264_converter_) {
        const auto& config = h264_converter_->GetCurrentConfig();
        return { VideoCodec::kH264, H264Parser::ProfileIDCToVideoCodecProfile(config.profile_indication), config.avc_level };
    }
#if BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    if (h265_converter_) {
        const auto& config = h265_converter_->GetCurrentConfig();
        return { VideoCodec::kHEVC, H265Parser::ProfileIDCToVideoCodecProfile(config.general_profile_idc), config.general_level_idc };
    }
#endif // BUILDFLAG(ENABLE_HEVC_PARSER_AND_HW_DECODER)
    NOTREACHED();
}
