...
BoxReader::BoxReader(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, bool is_EOS)
...
// static
ParseResult BoxReader::ReadTopLevelBox(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, std::unique_ptr<BoxReader>* out_reader)
{
    DCHECK(out_reader);
    std::unique_ptr<BoxReader> reader(new BoxReader(buf, buf_size, media_log, false));
...
// static
ParseResult BoxReader::StartTopLevelBox(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, FourCC* out_type, size_t* out_box_size)
{
...
// static
BoxReader* BoxReader::ReadConcatentatedBoxes(const uint8_t* buf, const size_t buf_size, MediaLog* media_log)
{
...
bool BoxReader::IsValidTopLevelBox(const FourCC& type, MediaLog* media_log)
{
    ...
    default:
        ...
        MEDIA_LOG(ERROR, media_log) << "Invalid top-level ISO BMFF box type " << FourCCToString(type);
        return false;
    }
}
...
ParseResult BoxReader::ReadHeader()
{
    ...
    if (box_size == 0) {
        if (is_EOS_) {
            ...
            MEDIA_LOG(DEBUG, media_log_) << "ISO BMFF boxes that run to EOS are not supported";
            return ParseResult::kError;
        }
    } else if (box_size == 1) {
        if (!HasBytes(8))
    ...
    // removed.
    if (box_size < base::strict_cast<uint64_t>(pos_) || box_size > static_cast<uint64_t>(std::numeric_limits<int32_t>::max())) {
        return ParseResult::kError;
    }
