...
bool AAC::Parse(base::span<const uint8_t> data, MediaLog* media_log)
{
    ...
    if ((profile_ < 1 || profile_ > 4) && profile_ != kXHeAAcType) {
        MEDIA_LOG(ERROR, media_log) << "Audio codec(mp4a.40." << static_cast<int>(profile_) << ") is not supported.";
        return false;
    }
    ...
    if (frequency_ == 0) {
        if (frequency_index_ >= kADTSFrequencyTableSize) {
            MEDIA_LOG(ERROR, media_log) << "Sampling Frequency Index(0x" << std::hex << static_cast<int>(frequency_index_)
    ...
    if (extension_frequency_ == 0 && extension_frequency_index != 0xff) {
        if (extension_frequency_index >= kADTSFrequencyTableSize) {
            MEDIA_LOG(ERROR, media_log) << "Extension Sampling Frequency Index(0x" << std::hex << static_cast<int>(extension_frequency_index)
                                        << ") is not supported. Please see ISO 14496-3:2009 Table 1.18 "
                                        << "for supported Sampling Frequencies.";
            return false;
        }
    ...
    if (ps_present && channel_config_ == 1) {
        ...
        if (channel_config_ >= kADTSChannelLayoutTableSize) {
            MEDIA_LOG(ERROR, media_log) << "Channel Configuration(" << static_cast<int>(channel_config_)
...
base::HeapArray<uint8_t> AAC::CreateAdtsFromEsds(base::span<const uint8_t> buffer, int* adts_header_size) const
{
    *adts_header_size = 0;
...
void AAC::SetAdtsHeader(base::span<uint8_t> adts, size_t total_size) const
{
    ...
    adts[3] = static_cast<uint8_t>(((channel_config_ & 0x3) << 6) + (total_size >> 11));
...
{
    return profile_ == kXHeAAcType ? AudioCodecProfile::kXHE_AAC : AudioCodecProfile::kUnknown;
}
