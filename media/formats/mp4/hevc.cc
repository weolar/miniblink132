...
HEVCDecoderConfigurationRecord::HEVCDecoderConfigurationRecord(const HEVCDecoderConfigurationRecord& other) = default;
HEVCDecoderConfigurationRecord::~HEVCDecoderConfigurationRecord() = default;

FourCC HEVCDecoderConfigurationRecord::BoxType() const
{
    return FOURCC_HVCC;
}
...
bool HEVCDecoderConfigurationRecord::Serialize(std::vector<uint8_t>& output) const
{
    ...
    // profile_indication
    result &= writer.WriteU8BigEndian((general_profile_space << 6) + (general_tier_flag << 5) + general_profile_idc);
    ...
    result &= writer.WriteU16BigEndian(general_constraint_indicator_flags & 0xffff);
    // genral_level_idc
    result &= writer.WriteU8BigEndian(general_level_idc);
    // min_spatial_segmentation_idc
    result &= writer.WriteU16BigEndian(min_spatial_segmentation_idc | (0xf << 12));
    // parallelismType
    ...
    // miscs
    result &= writer.WriteU8BigEndian((constantFrameRate << 6) + (numTemporalLayers << 3) + (temporalIdNested << 2) + lengthSizeMinusOne);
    // numOfArrays
...
HEVCDecoderConfigurationRecord::HVCCNALArray::HVCCNALArray(const HVCCNALArray& other) = default;

HEVCDecoderConfigurationRecord::HVCCNALArray::~HVCCNALArray()
{
}

bool HEVCDecoderConfigurationRecord::ParseInternal(BufferReader* reader, MediaLog* media_log)
{
    ...
    uint8_t misc = 0;
    RCHECK(reader->Read1(&configurationVersion) && (configurationVersion == 0 || configurationVersion == 1) && reader->Read1(&profile_indication)
        && reader->Read4(&general_profile_compatibility_flags) && reader->Read4(&general_constraint_indicator_flags_hi)
        && reader->Read2(&general_constraint_indicator_flags_lo) && reader->Read1(&general_level_idc) && reader->Read2(&min_spatial_segmentation_idc)
        && reader->Read1(&parallelismType) && reader->Read1(&chromaFormat) && reader->Read1(&bitDepthLumaMinus8) && reader->Read1(&bitDepthChromaMinus8)
        && reader->Read2(&avgFrameRate) && reader->Read1(&misc) && reader->Read1(&numOfArrays));
    ...
    for (uint32_t j = 0; j < numOfArrays; j++) {
        ...
        for (uint32_t i = 0; i < numNalus; ++i) {
            ...
            DVLOG(4) << __func__ << " naluType=" << (int)(arrays[j].first_byte & 0x3f) << " size=" << arrays[j].units[i].size();
        }
    }
    ...
    while (true) {
        ...
        case H265NALU::VPS_NUT: {
            ...
            DCHECK(vps);
            alpha_mode = vps->aux_alpha_layer_id ? VideoDecoderConfig::AlphaMode::kHasAlpha : VideoDecoderConfig::AlphaMode::kIsOpaque;
            break;
        }
        ...
        case H265NALU::PREFIX_SEI_NUT: {
            ...
            for (auto& sei_msg : sei.msgs) {
                ...
                case H265SEIMessage::kSEIMasteringDisplayInfo:
                    hdr_metadata.smpte_st_2086 = sei_msg.mastering_display_info.ToGfx();
                    break;
                default:
                    break;
                }
...
// static
bool HEVC::InsertParamSetsAnnexB(const HEVCDecoderConfigurationRecord& hevc_config, std::vector<uint8_t>* buffer, std::vector<SubsampleEntry>* subsamples)
{
    DCHECK(HEVC::AnalyzeAnnexB(buffer->data(), buffer->size(), *subsamples).is_conformant.value_or(true));
    ...
    if (nalu.nal_unit_type == H265NALU::AUD_NUT) {
        ...
        config_insert_point += (nalu.data + base::checked_cast<size_t>(nalu.size)) - start;
    }
    ...
    DVLOG(4) << __func__ << " converted hvcC to AnnexB "
             << " size=" << param_sets.size() << " inserted at " << (int)(config_insert_point - buffer->begin());

    if (subsamples && !subsamples->empty()) {
        int subsample_index = AVC::FindSubsampleIndex(*buffer, subsamples, &(*config_insert_point));
...
// static
void HEVC::ConvertConfigToAnnexB(const HEVCDecoderConfigurationRecord& hevc_config, std::vector<uint8_t>* buffer)
{
    DCHECK(buffer->empty());
    buffer->clear();
    ...
    for (size_t j = 0; j < hevc_config.arrays.size(); j++) {
        ...
        for (size_t i = 0; i < hevc_config.arrays[j].units.size(); ++i) {
            DVLOG(3) << __func__ << " naluType=" << (int)naluType << " size=" << hevc_config.arrays[j].units[i].size();
            buffer->insert(buffer->end(), kAnnexBStartCode, kAnnexBStartCode + kAnnexBStartCodeSize);
            buffer->insert(buffer->end(), hevc_config.arrays[j].units[i].begin(), hevc_config.arrays[j].units[i].end());
        }
    }
}

// static
BitstreamConverter::AnalysisResult HEVC::AnalyzeAnnexB(const uint8_t* buffer, size_t size, const std::vector<SubsampleEntry>& subsamples)
{
    DVLOG(3) << __func__;
    DCHECK(buffer);
    ...
    while (true) {
        ...
        if (h265_result != H265NaluParser::kOk) {
            DCHECK_NE(h265_result, H265NaluParser::kUnsupportedStream) << "AdvanceToNextNALU() returned kUnsupportedStream!";
            return result;
        }
        ...
        if (order_state == kEOBitstreamAllowed && nalu.nal_unit_type != H265NALU::EOB_NUT) {
        ...
        case H265NALU::UNSPEC55:
            if (order_state > kBeforeFirstVCL) {
                DVLOG(1) << "Unexpected NALU type " << nalu.nal_unit_type << " in order_state " << order_state;
                return result;
            }
            order_state = kBeforeFirstVCL;
            break;
...
HEVCBitstreamConverter::HEVCBitstreamConverter(std::unique_ptr<HEVCDecoderConfigurationRecord> hevc_config)
...
bool HEVCBitstreamConverter::ConvertAndAnalyzeFrame(
    std::vector<uint8_t>* frame_buf, bool is_keyframe, std::vector<SubsampleEntry>* subsamples, AnalysisResult* analysis_result) const
{
    RCHECK(AVC::ConvertFrameToAnnexB(hevc_config_->lengthSizeMinusOne + 1, frame_buf, subsamples));
...
BitstreamConverter::AnalysisResult HEVCBitstreamConverter::Analyze(std::vector<uint8_t>* frame_buf, std::vector<SubsampleEntry>* subsamples) const
{
