...
bool AC4::ParseAc4DsiV1(BitReader& reader)
{
    ...
    for (int i = 0; i < n_presentations; i++) {
        ...
        if (presentation_version == 0) {
            ...
            RCHECK(ParseAc4PresentationV1Dsi(reader, pres_bytes, consumed_pres_bytes, bitstream_version, presentation_version));
        }
...
bool AC4::ParseAc4PresentationV1Dsi(BitReader& reader, int pres_bytes, int& consumed_pres_bytes, uint8_t bitstream_version, uint8_t presentation_version)
{
    ...
    if (presentation_config_v1 == 0x06) {
        ...
        if (b_presentation_channel_coded) {
            ...
            if (dsi_presentation_ch_mode == 11 || dsi_presentation_ch_mode == 12 || dsi_presentation_ch_mode == 13 || dsi_presentation_ch_mode == 14) {
        ...
        if (ac4_substream_groups) {
            for (int i = 0; i < ac4_substream_groups; i++) {
                RCHECK(ParseAc4SubstreamGroupDsi(reader, bitstream_version, presentation_version, mdcompat));
            }
        }
...
bool AC4::ParseAc4SubstreamGroupDsi(BitReader& reader, uint8_t bitstream_version, uint8_t presentation_version, uint8_t presentation_level)
{
    ...
    for (int i = 0; i < n_substreams; i++) {
        ...
        if (b_channel_coded) {
            ...
            stream_info.channels = ChannelMaskToChannelCount(dsi_substream_channel_mask);
        } else {
            int b_ajoc;
            // b_ajoc, 1 bit
...
std::vector<uint8_t> AC4::StreamInfo() const
{
    ...
    return { stream_info_internals_[0].bitstream_version, stream_info_internals_[0].presentation_version, stream_info_internals_[0].presentation_level,
        stream_info_internals_[0].is_ajoc, stream_info_internals_[0].is_ims, stream_info_internals_[0].channels };
}

void AC4::LogAC4Parameters()
{
    DVLOG(3) << "parsed AC4 sub-stream count: " << stream_info_internals_.size() << ", first sub-stream is selected: "
             << "ac4_bitstream_version " << stream_info_internals_[0].bitstream_version << ", ac4_presentation_version "
             << stream_info_internals_[0].presentation_version << ", ac4_presentation_level " << stream_info_internals_[0].presentation_level;
}
