...
base::HeapArray<uint8_t> PrepareAACBuffer(const AAC& aac_config, base::span<const uint8_t> frame_buf, std::vector<SubsampleEntry>* subsamples)
{
...
base::HeapArray<uint8_t> PrependIADescriptors(const IamfSpecificBox& iacb, base::span<const uint8_t> frame_buf, std::vector<SubsampleEntry>* subsamples)
{
...
MP4StreamParser::MP4StreamParser(std::optional<base::flat_set<int>> strict_audio_object_types, bool has_sbr, bool has_flac, bool has_iamf, bool has_dv)
...
void MP4StreamParser::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
StreamParser::ParseStatus MP4StreamParser::Parse(int max_pending_bytes_to_inspect)
{
    DCHECK_NE(state_, kWaitingForInit);
    ...
    max_parse_offset_ = std::min(queue_.tail(), max_parse_offset_ + max_pending_bytes_to_inspect);
...
void MP4StreamParser::ModulatedPeekAt(int64_t offset, const uint8_t** buf, int* size)
{
    DCHECK(buf);
    DCHECK(size);
    DCHECK_GE(max_parse_offset_, queue_.head());
...
ParseResult MP4StreamParser::ParseBox()
{
    ...
    ParseResult result = BoxReader::ReadTopLevelBox(buf, size, media_log_, &reader);
...
VideoTransformation MP4StreamParser::CalculateRotation(const TrackHeader& track, const MovieHeader& movie)
{
        ...
        for (int j = 0; j < kDisplayMatrixHeight; j++) {
            for (int e = 0; e < kDisplayMatrixHeight; e++) {
                rotation_matrix[i * kDisplayMatrixHeight + j]
                    += ((int64_t)track.display_matrix[i * kDisplayMatrixHeight + e] * movie.display_matrix[e * kDisplayMatrixHeight + j]) >> shifts[e];
            }
        }
    }

    int32_t rotation_only[4] = { rotation_matrix[0], rotation_matrix[1], rotation_matrix[3], rotation_matrix[4] };
...
bool MP4StreamParser::ParseMoov(BoxReader* reader)
{
    ...
    for (std::vector<Track>::const_iterator track = moov_->tracks.begin(); track != moov_->tracks.end(); ++track) {
        ...
        if (track->media.handler.type == kAudio) {
            ...
            FourCC audio_format = (entry.format == FOURCC_ENCA) ? entry.sinf.format.format : entry.format;
            ...
            if (audio_format != FOURCC_OPUS && audio_format != FOURCC_FLAC &&
                ...
                audio_format != FOURCC_DTSC && audio_format != FOURCC_DTSX && audio_format != FOURCC_DTSE &&
                ...
                MEDIA_LOG(ERROR, media_log_) << "Unsupported audio format 0x" << std::hex << entry.format << " in stsd box.";
                return false;
            }
    ...
    // BUILDFLAG(ENABLE_PLATFORM_IAMF_AUDIO)
            ...
            if (audio_format == FOURCC_OPUS) {
                ...
                profile = entry.iacb.profile == 0 ? AudioCodecProfile::kIAMF_SIMPLE : AudioCodecProfile::kIAMF_BASE;
                ...
                if (strict_audio_object_types_.has_value()) {
                    if (!strict_audio_object_types_->contains(audio_type)) {
                        MEDIA_LOG(ERROR, media_log_) << "audio object type 0x" << std::hex << static_cast<int>(audio_type)
                ...
                if (ESDescriptor::IsAAC(audio_type)) {
                    ...
                    MEDIA_LOG(ERROR, media_log_) << "Unsupported audio object type 0x" << std::hex << static_cast<int>(audio_type) << " in esds.";
                    return false;
                }
#endif // BUILDFLAG(USE_PROPRIETARY_CODECS)
            }
            ...
            if (audio_track_ids_.find(audio_track_id) != audio_track_ids_.end()) {
                MEDIA_LOG(ERROR, media_log_) << "Audio track with track_id=" << audio_track_id << " already present.";
                return false;
            }
            ...
            audio_config.Initialize(codec, sample_format, channel_layout, sample_per_second, extra_data, scheme, seek_preroll, codec_delay_in_frames);
            ...
            DVLOG(1) << "audio_track_id=" << audio_track_id << " config=" << audio_config.AsHumanReadableString();
            if (!audio_config.IsValidConfig()) {
                MEDIA_LOG(ERROR, media_log_) << "Invalid audio decoder config: " << audio_config.AsHumanReadableString();
                return false;
            }
            has_audio_ = true;
            audio_track_ids_.insert(audio_track_id);
            const char* track_kind = (audio_track_ids_.size() == 1 ? "main" : "");
            media_tracks->AddAudioTrack(audio_config, true, audio_track_id, MediaTrack::Kind(track_kind), MediaTrack::Label(track->media.handler.name),
        ...
        if (track->media.handler.type == kVideo) {
            ...
            if (!entry.IsFormatValid()) {
                MEDIA_LOG(ERROR, media_log_) << "Unsupported video format 0x" << std::hex << entry.format << " in stsd box.";
                return false;
            }
            ...
            if (entry.pixel_aspect.h_spacing != 1 || entry.pixel_aspect.v_spacing != 1) {
                aspect_ratio = VideoAspectRatio::PAR(entry.pixel_aspect.h_spacing, entry.pixel_aspect.v_spacing);
            } else if (track->header.width && track->header.height) {
                aspect_ratio = VideoAspectRatio::DAR(track->header.width, track->header.height);
            }
            gfx::Size natural_size = aspect_ratio.GetNaturalSize(visible_rect);

            uint32_t video_track_id = track->header.track_id;
            if (video_track_ids_.find(video_track_id) != video_track_ids_.end()) {
                MEDIA_LOG(ERROR, media_log_) << "Video track with track_id=" << video_track_id << " already present.";
                return false;
            }
            bool is_track_encrypted = entry.sinf.info.track_encryption.is_encrypted;
            EncryptionScheme scheme = EncryptionScheme::kUnencrypted;
            ...
            if (entry.dv_info.has_value()) {
                ...
                if (has_dv_) {
                    ...
                    MEDIA_LOG(INFO, media_log_) << "Dolby Vision video track with track_id=" << video_track_id
                                                << " is using cross-compatible codec: " << GetCodecName(video_codec)
                                                << ". To prevent this, where Dolby Vision is supported, use a "
                                                << "Dolby Vision codec string when constructing the "
                                                   "SourceBuffer.";
                }
            }
            video_config.Initialize(video_codec, video_codec_profile, entry.alpha_mode, VideoColorSpace::REC709(),
                CalculateRotation(track->header, moov_->header), coded_size, visible_rect, natural_size,
            ...
            DVLOG(1) << "video_track_id=" << video_track_id << " config=" << video_config.AsHumanReadableString();
            if (!video_config.IsValidConfig()) {
                MEDIA_LOG(ERROR, media_log_) << "Invalid video decoder config: " << video_config.AsHumanReadableString();
                return false;
            }
            has_video_ = true;
            video_track_ids_.insert(video_track_id);
            auto track_kind = MediaTrack::Kind(video_track_ids_.size() == 1 ? "main" : "");
            media_tracks->AddVideoTrack(video_config, true, video_track_id, track_kind, MediaTrack::Label(track->media.handler.name),
                MediaTrack::Language(track->media.header.language()));
            continue;
        }
    }
    ...
    if (moov_->extends.header.fragment_duration > 0) {
        params.duration = TimeDeltaFromRational(moov_->extends.header.fragment_duration, moov_->header.timescale);
        ...
        && ((moov_->header.version == 0 && moov_->header.duration != std::numeric_limits<uint32_t>::max())
            || (moov_->header.version == 1 && moov_->header.duration != std::numeric_limits<uint64_t>::max()))) {
...
void MP4StreamParser::OnEncryptedMediaInitData(const std::vector<ProtectionSystemSpecificHeader>& headers)
{
...
ParseResult MP4StreamParser::EnqueueSample(BufferQueueMap* buffers)
{
    ...
    if (runs_->sample_size() > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
    ...
    if (sample_size == 0) {
        ...
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_empty_samples_skipped_, kMaxEmptySampleLogs) << "Skipping 'trun' sample with size of 0.";
    ...
    if (video) {
        if (runs_->video_description().video_info.codec == VideoCodec::kH264 || runs_->video_description().video_info.codec == VideoCodec::kHEVC
            || runs_->video_description().video_info.codec == VideoCodec::kDolbyVision) {
            DCHECK(runs_->video_description().frame_bitstream_converter);
            BitstreamConverter::AnalysisResult analysis;
            frame_buf.assign(buf, buf + sample_size);
            if (!runs_->video_description().frame_bitstream_converter->ConvertAndAnalyzeFrame(&frame_buf, is_keyframe, &subsamples, &analysis)) {
                MEDIA_LOG(ERROR, media_log_) << "Failed to prepare video sample for decode";
                return ParseResult::kError;
            }
            ...
            if (!analysis.is_conformant.value_or(true)) {
                LIMITED_MEDIA_LOG(DEBUG, media_log_, num_invalid_conversions_, kMaxInvalidConversionLogs) << "Prepared video sample is not conformant";
            }
            ...
            // |frame_buf|.
            if (analysis.is_keyframe.has_value() && is_keyframe != analysis.is_keyframe.value()) {
                LIMITED_MEDIA_LOG(DEBUG, media_log_, num_video_keyframe_mismatches_, kMaxVideoKeyframeMismatchLogs)
    ...
    if (audio) {
        if (ESDescriptor::IsAAC(runs_->audio_description().esds.object_type)) {
#if BUILDFLAG(USE_PROPRIETARY_CODECS)
            heap_frame_buf = PrepareAACBuffer(runs_->audio_description().esds.aac, { buf, buf + sample_size }, &subsamples);
            ...
            if (runs_->audio_description().format == FOURCC_IAMF) {
                heap_frame_buf = PrependIADescriptors(runs_->audio_description().iacb, { buf, buf + sample_size }, &subsamples);
                if (heap_frame_buf.empty()) {
                    MEDIA_LOG(ERROR, media_log_) << "Failed to prepare IA sample for decode";
                }
            }
#endif // BUILDFLAG(ENABLE_PLATFORM_IAMF_AUDIO)
        }
    ...
    if (decrypt_config) {
        if (!subsamples.empty()) {
            ...
            decrypt_config = std::make_unique<DecryptConfig>(
                decrypt_config->encryption_scheme(), decrypt_config->key_id(), decrypt_config->iv(), subsamples, decrypt_config->encryption_pattern());
        }
    ...
    if (auto* media_client = GetMediaClient()) {
        if (auto* alloc = media_client->GetMediaAllocator()) {
            stream_buf = StreamParserBuffer::FromExternalMemory(
                alloc->CopyFrom(
                    frame_buf.empty() ? (heap_frame_buf.empty() ? base::span<const uint8_t> { buf, buf + sample_size } : heap_frame_buf) : frame_buf),
    ...
    if (!stream_buf) {
        ...
        if (frame_buf.empty() && heap_frame_buf.empty()) {
            stream_buf = StreamParserBuffer::CopyFrom(buf, sample_size, is_keyframe, buffer_type, runs_->track_id());
        } else if (frame_buf.empty()) {
            stream_buf = StreamParserBuffer::FromArray(std::move(heap_frame_buf), is_keyframe, buffer_type, runs_->track_id());

        } else {
            stream_buf = StreamParserBuffer::FromExternalMemory(
                std::make_unique<ExternalMemoryAdapter>(std::move(frame_buf)), is_keyframe, buffer_type, runs_->track_id());
        }
    }
    ...
    DVLOG(3) << "Emit " << (audio ? "audio" : "video") << " frame: "
             << " track_id=" << runs_->track_id() << ", key=" << is_keyframe << ", dur=" << runs_->duration().InMilliseconds()
             << ", dts=" << runs_->dts().InMilliseconds() << ", cts=" << runs_->cts().InMilliseconds() << ", size=" << sample_size;
...
bool MP4StreamParser::ReadAndDiscardMDATsUntil(int64_t max_clear_offset)
{
    ...
    while (mdat_tail_ < upper_bound) {
        ...
        if (type != FOURCC_MDAT) {
            MEDIA_LOG(DEBUG, media_log_) << "Unexpected box type while parsing MDATs: " << FourCCToString(type);
        }
...
bool MP4StreamParser::HaveEnoughDataToEnqueueSamples()
{
    ...
    // muxed content.
    DCHECK_LE(max_parse_offset_, queue_.tail());
    return !(has_audio_ && has_video_ && max_parse_offset_ < highest_end_offset_ + moof_head_);
}
