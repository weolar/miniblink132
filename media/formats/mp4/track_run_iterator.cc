...
struct TrackRunInfo {
    ...
    raw_ptr<const SampleGroupDescription, DanglingUntriaged> track_sample_encryption_group;
...
base::TimeDelta TimeDeltaFromRational(int64_t numer, int64_t denom)
{
    ...
    const int64_t max_seconds = std::numeric_limits<int64_t>::max() / base::Time::kMicrosecondsPerSecond;
    ...
    const int64_t result_microseconds = base::Time::kMicrosecondsPerSecond * (numer % denom) / denom;

    const int64_t total_microseconds = base::Time::kMicrosecondsPerSecond * result_seconds + result_microseconds;
    return base::Microseconds(total_microseconds);
}

DecodeTimestamp DecodeTimestampFromRational(int64_t numer, int64_t denom)
{
    return DecodeTimestamp::FromPresentationTime(TimeDeltaFromRational(numer, denom));
}
...
static std::string HexFlags(uint32_t flags)
{
    ...
    stream << std::setfill('0') << std::setw(sizeof(flags) * 2) << std::hex << flags;
    return stream.str();
}

static bool PopulateSampleInfo(const TrackExtends& trex, const TrackFragmentHeader& tfhd, const TrackFragmentRun& trun, const int64_t edit_list_offset,
    const uint32_t i, SampleInfo* sample_info, const SampleDependsOn sdtp_sample_depends_on, bool is_audio, MediaLog* media_log)
{
    ...
    DVLOG(4) << __func__ << " is_kf:" << sample_info->is_keyframe << " is_sync:" << sample_is_sync_sample << " deps:" << sample_depends_on_others
             << " audio:" << is_audio;

    return true;
}

static const CencSampleEncryptionInfoEntry* GetSampleEncryptionInfoEntry(const TrackRunInfo& run_info, uint32_t group_description_index)
{
    ...
    return (group_description_index > entries->size()) ? nullptr : &(*entries)[group_description_index - 1];
}
...
public:
    ...
    {
        int64_t a_aux = a.aux_info_total_size ? a.aux_info_start_offset : std::numeric_limits<int64_t>::max();
        int64_t b_aux = b.aux_info_total_size ? b.aux_info_start_offset : std::numeric_limits<int64_t>::max();
...
bool TrackRunIterator::Init(const MovieFragment& moof)
{
    ...
    for (size_t i = 0; i < moof.tracks.size(); i++) {
        ...
        if (!sample_encryption_data.empty()) {
            sample_encryption_reader = std::make_unique<BufferReader>(sample_encryption_data.data(), sample_encryption_data.size());
        ...
        for (size_t j = 0; j < traf.runs.size(); j++) {
            ...
            if (!sinf->HasSupportedScheme()) {
                ...
                tri.encryption_scheme = sinf->IsCbcsEncryptionScheme() ? EncryptionScheme::kCbcs : EncryptionScheme::kCenc;
                tri.encryption_pattern = EncryptionPattern(track_encryption->default_crypt_byte_block, track_encryption->default_skip_byte_block);
            }
            ...
            if (sample_encryption_entries_count == 0 && traf.auxiliary_offset.offsets.size() > j) {
                ...
                if (tri.aux_info_default_size == 0) {
                    ...
                    tri.aux_info_sizes.insert(
                        tri.aux_info_sizes.begin(), sizes.begin() + sample_count_sum, sizes.begin() + sample_count_sum + trun.sample_count);
                }
            ...
            size_t max_sample_count = GetDemuxerMemoryLimit(Demuxer::DemuxerTypes::kChunkDemuxer) / sizeof(decltype(tri.samples)::value_type);
            ...
            RCHECK_MEDIA_LOGGED(base::strict_cast<size_t>(trun.sample_count) <= max_sample_count, media_log_, "Metadata overhead exceeds storage limit.");
            tri.samples.resize(trun.sample_count);

            for (size_t k = 0; k < trun.sample_count; k++) {
                if (!PopulateSampleInfo(
                        *trex, traf.header, trun, edit_list_offset, k, &tri.samples[k], traf.sdtp.sample_depends_on(k), tri.is_audio, media_log_)) {
                    return false;
                }

                RCHECK(std::numeric_limits<int64_t>::max() - tri.samples[k].duration > run_start_dts);
            ...
            if (sample_encryption_entries_count > 0) {
                ...
                for (size_t k = 0; k < trun.sample_count; k++) {
                    ...
                    const uint8_t iv_size = index == 0 ? track_encryption->default_iv_size : info_entry->iv_size;
                    SampleEncryptionEntry& entry = tri.sample_encryption_entries[k];
                    RCHECK(entry.Parse(sample_encryption_reader.get(), iv_size, traf.sample_encryption.use_subsample_encryption));
                    // If we don't have a per-sample IV, get the constant IV.
                    bool is_encrypted = index == 0 ? track_encryption->is_encrypted : info_entry->is_encrypted;
                    ...
                    if (is_encrypted && index != 0) {
                        RCHECK_MEDIA_LOGGED(info_entry->crypt_byte_block == track_encryption->default_crypt_byte_block, media_log_,
                            "Pattern value (crypt byte block) for the "
                            "sample group does not match that in the tenc "
                            "box . This is not currently supported.");
                        RCHECK_MEDIA_LOGGED(info_entry->skip_byte_block == track_encryption->default_skip_byte_block, media_log_,
                    ...
                    if (is_encrypted && !iv_size) {
                        const uint8_t constant_iv_size = index == 0 ? track_encryption->default_constant_iv_size : info_entry->constant_iv_size;
                        RCHECK(constant_iv_size != 0);
                        const uint8_t* constant_iv = index == 0 ? track_encryption->default_constant_iv : info_entry->constant_iv;
...
bool TrackRunIterator::CacheAuxInfo(const uint8_t* buf, int buf_size)
{
    ...
    for (size_t i = 0; i < run_itr_->samples.size(); i++) {
        ...
        if (IsSampleEncrypted(i)) {
            ...
            RCHECK_MEDIA_LOGGED(entry.Parse(&reader, iv_size, has_subsamples), media_log_, "SampleEncryptionEntry parse failed when caching aux info");
            // if we don't have a per-sample IV, get the constant IV.
            if (!iv_size) {
...
std::unique_ptr<DecryptConfig> TrackRunIterator::GetDecryptConfig()
{
    ...
    if (run_itr_->sample_encryption_entries.empty()) {
        ...
        if (ApplyConstantIv(sample_idx, &sample_encryption_entry)) {
            std::string iv(
                reinterpret_cast<const char*>(sample_encryption_entry.initialization_vector), std::size(sample_encryption_entry.initialization_vector));
            switch (run_itr_->encryption_scheme) {
            case EncryptionScheme::kUnencrypted:
                return nullptr;
            case EncryptionScheme::kCenc:
                return DecryptConfig::CreateCencConfig(key_id, iv, sample_encryption_entry.subsamples);
            case EncryptionScheme::kCbcs:
                return DecryptConfig::CreateCbcsConfig(key_id, iv, sample_encryption_entry.subsamples, run_itr_->encryption_pattern);
            }
        }
    ...
    std::string iv(reinterpret_cast<const char*>(sample_encryption_entry.initialization_vector), std::size(sample_encryption_entry.initialization_vector));

    size_t total_size = 0;
    if (!sample_encryption_entry.subsamples.empty()
        && (!sample_encryption_entry.GetTotalSizeOfSubsamples(&total_size) || total_size != static_cast<size_t>(sample_size()))) {
    ...
    if (protection_scheme_info().IsCbcsEncryptionScheme()) {
        ...
        uint32_t encrypt_blocks = (index == 0) ? track_encryption().default_crypt_byte_block : GetSampleEncryptionInfoEntry(*run_itr_, index)->crypt_byte_block;
        uint32_t skip_blocks = (index == 0) ? track_encryption().default_skip_byte_block : GetSampleEncryptionInfoEntry(*run_itr_, index)->skip_byte_block;
        return DecryptConfig::CreateCbcsConfig(key_id, iv, sample_encryption_entry.subsamples, EncryptionPattern(encrypt_blocks, skip_blocks));
    }

    return DecryptConfig::CreateCencConfig(key_id, iv, sample_encryption_entry.subsamples);
}

uint32_t TrackRunIterator::GetGroupDescriptionIndex(uint32_t sample_index) const
{
    DCHECK(IsRunValid());
...
bool TrackRunIterator::IsSampleEncrypted(size_t sample_index) const
{
    ...
    return (index == 0) ? track_encryption().is_encrypted : GetSampleEncryptionInfoEntry(*run_itr_, index)->is_encrypted;
}

const std::vector<uint8_t>& TrackRunIterator::GetKeyId(size_t sample_index) const
{
    uint32_t index = GetGroupDescriptionIndex(sample_index);
    return (index == 0) ? track_encryption().default_kid : GetSampleEncryptionInfoEntry(*run_itr_, index)->key_id;
}

uint8_t TrackRunIterator::GetIvSize(size_t sample_index) const
{
    uint32_t index = GetGroupDescriptionIndex(sample_index);
    return (index == 0) ? track_encryption().default_iv_size : GetSampleEncryptionInfoEntry(*run_itr_, index)->iv_size;
}

bool TrackRunIterator::ApplyConstantIv(size_t sample_index, SampleEncryptionEntry* entry) const
{
    DCHECK(IsSampleEncrypted(sample_index));
    uint32_t index = GetGroupDescriptionIndex(sample_index);
    const uint8_t constant_iv_size
        = index == 0 ? track_encryption().default_constant_iv_size : GetSampleEncryptionInfoEntry(*run_itr_, index)->constant_iv_size;
    RCHECK(constant_iv_size != 0);
    const uint8_t* constant_iv = index == 0 ? track_encryption().default_constant_iv : GetSampleEncryptionInfoEntry(*run_itr_, index)->constant_iv;
