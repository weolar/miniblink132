...
{
    DVLOG(3) << "DecoderProfileCode " << static_cast<int>(decoder_profile_code_) << "Frame Duration " << frame_duration_ << "Max Payload " << max_payload_
             << "Num Presentations " << num_presentations_ << "Channel Mask " << channel_mask_ << "Sampling Frequency " << sampling_frequency_;
}
