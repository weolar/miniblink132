...
enum DisplayMatrixSize { kDisplayMatrixWidth = 3, kDisplayMatrixHeight = 3, kDisplayMatrixDimension = kDisplayMatrixHeight * kDisplayMatrixWidth };
...
public:
    ...
    bool HasBytes(size_t count)
    {
        ...
        constexpr size_t kImplLimit = base::checked_cast<size_t>(std::numeric_limits<int32_t>::max());
        return pos_ <= buf_.size() && count <= kImplLimit && count <= buf_.size() - pos_;
    }
    ...
    // not been appended.
    const base::span<const uint8_t> buffer() const
    {
        return buf_;
    }
...
public:
    ...
    [[nodiscard]] static ParseResult ReadTopLevelBox(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, std::unique_ptr<BoxReader>* out_reader);
    ...
    //
    // |buf| is not retained.
    [[nodiscard]] static ParseResult StartTopLevelBox(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, FourCC* out_type, size_t* out_box_size);
    ...
    //
    // |buf| is retained but not owned, and must outlive the BoxReader instance.
    static BoxReader* ReadConcatentatedBoxes(const uint8_t* buf, const size_t buf_size, MediaLog* media_log);
    ...
    template <typename T> [[nodiscard]] bool ReadChildren(std::vector<T>* children);

    // Read any number of children. False means error.
    template <typename T> [[nodiscard]] bool MaybeReadChildren(std::vector<T>* children);
    ...
    template <typename T> [[nodiscard]] bool ReadAllChildren(std::vector<T>* children);
    ...
    template <typename T> [[nodiscard]] bool ReadAllChildrenAndCheckFourCC(std::vector<T>* children);
...
private:
    ...
    BoxReader(const uint8_t* buf, const size_t buf_size, MediaLog* media_log, bool is_EOS);
    ...
    // with ScanChildren().
    template <typename T> bool ReadAllChildrenInternal(std::vector<T>* children, bool check_box_type);
...
template <typename T> bool BoxReader::MaybeReadChildren(std::vector<T>* children)
{
    DCHECK(scanned_);
    ...
    DVLOG(2) << "Found " << children->size() << " " << FourCCToString(child_type) << " boxes.";
    return true;
}

template <typename T> bool BoxReader::ReadAllChildren(std::vector<T>* children)
{
    return ReadAllChildrenInternal(children, false);
}

template <typename T> bool BoxReader::ReadAllChildrenAndCheckFourCC(std::vector<T>* children)
{
    return ReadAllChildrenInternal(children, true);
}

template <typename T> bool BoxReader::ReadAllChildrenInternal(std::vector<T>* children, bool check_box_type)
{
    DCHECK(!scanned_);
    scanned_ = true;
