...
static const uint8_t kSkipBox[] = {
    ...
    0x00, 0x00, 0x00, 0x40, 's', 'k', 'i', 'p', 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0xf9, 0x0a, 0x0b, 0x0c, 0xfd, 0x0e, 0x0f, 0x10,
    // Ordinary (8-byte header) child box
    0x00, 0x00, 0x00, 0x0c, 'p', 's', 's', 'h', 0xde, 0xad, 0xbe, 0xef,
    // Extended-size header child box
    0x00, 0x00, 0x00, 0x01, 'p', 's', 's', 'h', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x14, 0xfa, 0xce, 0xca, 0xfe,
    // Empty free box
...
protected:
    ...
    void TestTopLevelBox(const uint8_t* data, size_t data_size, uint32_t fourCC)
    {
        ...
        ParseResult result = BoxReader::ReadTopLevelBox(&buf[0], buf.size(), &media_log_, &reader);
    ...
    template <typename ChildType> void TestParsing32bitOverflow(const uint8_t* buffer, size_t size, const std::string& overflow_error)
    {
        ...
        ASSERT_TRUE(base::IsValueInRangeForNumericType<uint8_t>(buffer_wrapper.size()));
        buffer_wrapper[3] = buffer_wrapper.size();

        std::unique_ptr<BoxReader> reader;
        ParseResult result = BoxReader::ReadTopLevelBox(&buffer_wrapper[0], buffer_wrapper.size(), &media_log_, &reader);
        EXPECT_EQ(result, ParseResult::kOk);
        EXPECT_TRUE(reader);
...
TEST_F(BoxReaderTest, OuterTooShortTest)
{
    ...
    ParseResult result = BoxReader::ReadTopLevelBox(&buf[0], buf.size() - 2, &media_log_, &r);
...
TEST_F(BoxReaderTest, WrongFourCCTest)
{
    ...
    EXPECT_MEDIA_LOG(AllOf(HasSubstr("error"), HasSubstr("Invalid top-level ISO BMFF box type DALE")));
...
{
    static const uint8_t kData[] = { 0x00, 0x00, 0x00, 0x09, 'b', 'l', 'o', 'c', 0x00 };
...
TEST_F(BoxReaderTest, NestedBoxWithHugeSize)
{
    ...
    static const uint8_t kData[] = { 0x00, 0x00, 0x00, 0x24, 'e', 'm', 's', 'g', // outer box
    ...
    std::unique_ptr<BoxReader> reader;
    ParseResult result = BoxReader::ReadTopLevelBox(kData, sizeof(kData), &media_log_, &reader);
...
TEST_F(BoxReaderTest, ScanChildrenWithInvalidChild)
{
    ...
    static const uint8_t kData[] = { 0x00, 0x00, 0x00, 0x2c, 'e', 'm', 's', 'g', // outer box
...
TEST_F(BoxReaderTest, TrunSampleCount32bitOverflow)
{
    ...
    static const uint8_t kData[] = { 0x00, 0x00, 0x00, 0x18, 't', 'r', 'u', 'n', // header
        0x00, 0x00, // version = 0
    ...
    TestParsing32bitOverflow<TrackFragmentRun>(kData, sizeof(kData), "Extreme TRUN sample count exceeds implementation limit.");
}
...
TEST_F(BoxReaderTest, SaioCount32bitOverflow)
{
    ...
    // Verify we catch the overflow to avoid OOB reads/writes.
    TestParsing32bitOverflow<SampleAuxiliaryInformationOffset>(kData, sizeof(kData), "Extreme SAIO count exceeds implementation limit.");
}
...
TEST_F(BoxReaderTest, ElstCount32bitOverflow)
{
    ...
    // Verify we catch the overflow to avoid OOB reads/writes.
    TestParsing32bitOverflow<EditList>(kData, sizeof(kData), "Extreme ELST count exceeds implementation limit.");
}
...
TEST_F(BoxReaderTest, SbgpCount32bitOverflow)
{
    ...
    // Verify we catch the overflow to avoid OOB reads/writes.
    TestParsing32bitOverflow<SampleToGroup>(kData, sizeof(kData), "Extreme SBGP count exceeds implementation limit.");
}
...
TEST_F(BoxReaderTest, SgpdCount32bitOverflow)
{
    ...
    // Verify we catch the overflow to avoid OOB reads/writes.
    TestParsing32bitOverflow<SampleGroupDescription>(kData, sizeof(kData), "Extreme SGPD count exceeds implementation limit.");
}
...
{
    std::vector<uint8_t> test_data { 0x01, 0x4D, 0x00, 0x15, 0xff, 0xe1, 0x00, 0x2F, 0x67, 0x4D, 0x40, 0x15, 0x96, 0x52, 0x02, 0x83, 0xF6, 0x02, 0xA1, 0x00,
        0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x03, 0x00, 0x28, 0xE0, 0x60, 0x03, 0x0D, 0x40, 0x00, 0x49, 0x3E, 0x7F, 0x18, 0xE3, 0x03, 0x00, 0x18, 0x6A, 0x00,
        0x02, 0x49, 0xF3, 0xF8, 0xC7, 0x0E, 0xD0, 0xB1, 0x68, 0x90, 0x01, 0x00, 0x04, 0x68, 0xEB, 0x73, 0x52 };
    AVCDecoderConfigurationRecord record;
    EXPECT_TRUE(record.Parse(test_data.data(), test_data.size()));
    std::vector<uint8_t> output;
    EXPECT_TRUE(record.Serialize(output));
