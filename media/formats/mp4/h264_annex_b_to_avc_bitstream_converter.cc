...
H264AnnexBToAvcBitstreamConverter::~H264AnnexBToAvcBitstreamConverter() = default;

const mp4::AVCDecoderConfigurationRecord& H264AnnexBToAvcBitstreamConverter::GetCurrentConfig()
{
    return config_;
}
    ...
    const base::span<const uint8_t> input, base::span<uint8_t> output, bool* config_changed_out, size_t* size_out)
{
    ...
    while ((result = parser_.AdvanceToNextNALU(&nalu)) != H264Parser::kEOStream) {
        ...
        case H264NALU::kSPSExt: {
            ...
            id2sps_ext_.insert_or_assign(sps_id, blob(nalu.data.get(), (nalu.data + base::checked_cast<size_t>(nalu.size)).get()));
            config_changed = true;
            break;
        }
        ...
        case H264NALU::kIDRSlice: {
            ...
            if (new_active_sps_id != active_sps_id_) {
                if (!config_changed) {
                    DCHECK(nalu.nal_unit_type == H264NALU::kIDRSlice) << "SPS shouldn't change in non-IDR slice";
                }
                config_changed = true;
            }
        }
    ...
    if (config_changed) {
        ...
        config_.profile_compatibility = ((active_sps->constraint_set0_flag ? 1 : 0) << 7) | ((active_sps->constraint_set1_flag ? 1 : 0) << 6)
            | ((active_sps->constraint_set2_flag ? 1 : 0) << 5) | ((active_sps->constraint_set3_flag ? 1 : 0) << 4)
            | ((active_sps->constraint_set4_flag ? 1 : 0) << 3) | ((active_sps->constraint_set5_flag ? 1 : 0) << 2);
