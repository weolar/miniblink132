...
template <typename T> void WriteNALUType(std::vector<uint8_t>* buffer, const std::string& nal_unit_type);
...
template <> void WriteNALUType<H264NALU>(std::vector<uint8_t>* buffer, const std::string& nal_unit_type)
{
...
template <> void WriteNALUType<H265NALU>(std::vector<uint8_t>* buffer, const std::string& nal_unit_type)
{
    uint8_t header1 = 0;
...
template <typename T> void WriteStartCodeAndNALUType(std::vector<uint8_t>* buffer, const std::string& nal_unit_type)
{
...
template <typename T> void StringToAnnexB(const std::string& str, std::vector<uint8_t>* buffer, std::vector<SubsampleEntry>* subsamples)
{
    DCHECK(!str.empty());

    std::vector<std::string> subsample_specs = base::SplitString(str, " ", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (size_t i = 0; i < subsample_specs.size(); ++i) {
        ...
        std::vector<std::string> subsample_nalus = base::SplitString(subsample_specs[i], ",", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
void AvcStringToAnnexB(const std::string& str, std::vector<uint8_t>* buffer, std::vector<SubsampleEntry>* subsamples)
{
    StringToAnnexB<H264NALU>(str, buffer, subsamples);
}

#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
void HevcStringToAnnexB(const std::string& str, std::vector<uint8_t>* buffer, std::vector<SubsampleEntry>* subsamples)
{
    StringToAnnexB<H265NALU>(str, buffer, subsamples);
}
#endif // BUILDFLAG(ENABLE_PLATFORM_HEVC)

bool AnalysesMatch(const BitstreamConverter::AnalysisResult& r1, const BitstreamConverter::AnalysisResult& r2)
{
    return r1.is_conformant == r2.is_conformant && r1.is_keyframe == r2.is_keyframe;
}
