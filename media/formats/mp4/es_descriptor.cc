...
// static
std::vector<uint8_t> ESDescriptor::CreateEsds(const std::vector<uint8_t>& aac_extra_data)
{
#pragma pack(push, 1)
    struct Descriptor {
        uint8_t tag;
...
#pragma pack(pop)
    ...
    EncodeDescriptorSize(sizeof(EsDescriptor) - sizeof(Descriptor) + aac_extra_data.size(), esds->size);

    esds->decoder_config.tag = kDecoderConfigDescrTag;
    EncodeDescriptorSize(sizeof(DecoderConfigDescriptor) - sizeof(Descriptor) + aac_extra_data.size(), esds->decoder_config.size);
    ...
    EncodeDescriptorSize(aac_extra_data.size(), esds->decoder_config.extra_data.size);
