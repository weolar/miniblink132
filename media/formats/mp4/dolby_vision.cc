...
bool DOVIDecoderConfigurationRecord::ParseForTesting(const uint8_t* data, int data_size)
{
...
bool DOVIDecoderConfigurationRecord::Parse(BufferReader* reader, MediaLog* media_log)
{
    uint16_t profile_track_indication = 0;
    RCHECK(reader->Read1(&dv_version_major) && reader->Read1(&dv_version_minor) && reader->Read2(&profile_track_indication));
    ...
    case 9:
        ...
        break;
    default:
        DVLOG(2) << "Deprecated or invalid Dolby Vision profile:" << static_cast<int>(dv_profile);
        return false;
    }

    DVLOG(2) << "Dolby Vision profile:" << static_cast<int>(dv_profile) << " level:" << static_cast<int>(dv_level)
             << " has_bl:" << static_cast<int>(bl_present_flag) << " has_el:" << static_cast<int>(el_present_flag)
             << " has_rpu:" << static_cast<int>(rpu_present_flag) << " bl_signal_compatibility_id: " << static_cast<int>(dv_bl_signal_compatibility_id)
