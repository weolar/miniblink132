...
struct MEDIA_EXPORT VPCodecConfiguration : FullBox {
    VPCodecConfiguration(VideoCodecProfile profile, uint8_t level, const gfx::ColorSpace& color_space);
...
struct MEDIA_EXPORT AudioSampleEntry : Box {
    AudioSampleEntry(AudioCodec codec, uint32_t sample_rate, uint8_t channel_count);
    ~AudioSampleEntry();
