...
std::tuple<CodecProfileLevel, std::optional<CodecProfileLevel>> MaybeParseDOVI(BoxReader* reader, CodecProfileLevel video_info)
{
    ...
    CodecProfileLevel dv_info = { VideoCodec::kDolbyVision, dovi_config->codec_profile, dovi_config->dv_level };
    if (dovi_config->dv_bl_signal_compatibility_id == kHDR10CompatibilityId || dovi_config->dv_bl_signal_compatibility_id == kSDRCompatibilityId
...
// bound set to 50000.
bool ReadMdcvColorCoordinate(BoxReader* reader, float* normalized_value_in_float)
{
...
bool ReadFixedPoint16(float fixed_point_divisor, BoxReader* reader, float* normalized_value_in_float)
{
    uint16_t value;
...
bool ReadFixedPoint32(float fixed_point_divisor, BoxReader* reader, float* normalized_value_in_float)
{
    uint32_t value;
...
gfx::HdrMetadataSmpteSt2086 ConvertMdcvToColorVolumeMetadata(const MasteringDisplayColorVolume& mdcv)
{
...
ProtectionSystemSpecificHeader::ProtectionSystemSpecificHeader(const ProtectionSystemSpecificHeader& other) = default;
ProtectionSystemSpecificHeader::~ProtectionSystemSpecificHeader() = default;
...
FullProtectionSystemSpecificHeader::FullProtectionSystemSpecificHeader() = default;
FullProtectionSystemSpecificHeader::FullProtectionSystemSpecificHeader(const FullProtectionSystemSpecificHeader& other) = default;
FullProtectionSystemSpecificHeader::~FullProtectionSystemSpecificHeader() = default;
...
SampleAuxiliaryInformationOffset::SampleAuxiliaryInformationOffset(const SampleAuxiliaryInformationOffset& other) = default;
SampleAuxiliaryInformationOffset::~SampleAuxiliaryInformationOffset() = default;
...
SampleAuxiliaryInformationSize::SampleAuxiliaryInformationSize(const SampleAuxiliaryInformationSize& other) = default;
...
SampleEncryptionEntry::SampleEncryptionEntry(const SampleEncryptionEntry& other) = default;
SampleEncryptionEntry::~SampleEncryptionEntry() = default;

bool SampleEncryptionEntry::Parse(BufferReader* reader, uint8_t iv_size, bool has_subsamples)
{
...
bool SampleEncryption::Parse(BoxReader* reader)
{
    ...
    base::span<const uint8_t> buffer = reader->buffer().first(reader->box_size()).subspan(reader->pos());
...
bool MovieHeader::Parse(BoxReader* reader)
{
    ...
    RCHECK_MEDIA_LOGGED(timescale > 0, reader->media_log(), "Movie header's timescale must not be 0");

    RCHECK(reader->Read4s(&rate) && reader->Read2s(&volume) && reader->SkipBytes(10) && // reserved
        reader->ReadDisplayMatrix(display_matrix) && reader->SkipBytes(24) && // predefined zero
...
bool TrackHeader::Parse(BoxReader* reader)
{
    ...
    RCHECK(reader->SkipBytes(8) && // reserved
        ...
        reader->ReadDisplayMatrix(display_matrix) && reader->Read4(&width) && reader->Read4(&height));
...
AVCDecoderConfigurationRecord::AVCDecoderConfigurationRecord(const AVCDecoderConfigurationRecord& other) = default;
AVCDecoderConfigurationRecord::~AVCDecoderConfigurationRecord() = default;
FourCC AVCDecoderConfigurationRecord::BoxType() const
{
    return FOURCC_AVCC;
}
...
bool AVCDecoderConfigurationRecord::ParseInternal(BufferReader* reader, MediaLog* media_log)
{
    ...
    if (profile_indication == 100 || profile_indication == 110 || profile_indication == 122 || profile_indication == 144) {
...
bool AVCDecoderConfigurationRecord::ParseREXT(BufferReader* reader, MediaLog* media_log)
{
    ...
    for (int i = 0; i < num_sps_ext; i++) {
        ...
        RCHECK(reader->Read2(&sps_ext_length) && reader->ReadVec(&sps_ext_list[i], sps_ext_length));
    }

    return true;
}

bool AVCDecoderConfigurationRecord::Serialize(std::vector<uint8_t>& output) const
{
...
VPCodecConfigurationRecord::VPCodecConfigurationRecord(const VPCodecConfigurationRecord& other) = default;
...
bool VPCodecConfigurationRecord::Parse(BoxReader* reader)
{
    ...
    case 3:
        ...
        break;
    default:
        MEDIA_LOG(ERROR, reader->media_log()) << "Unsupported VP9 profile: 0x" << std::hex << static_cast<uint32_t>(profile_indication);
        return false;
    }
    ...
    color_space
        = VideoColorSpace(primary_id, transfer_id, matrix_id, depth_chroma_full_range & 1 ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
    return true;
}
...
AV1CodecConfigurationRecord::AV1CodecConfigurationRecord(const AV1CodecConfigurationRecord& other) = default;
...
bool AV1CodecConfigurationRecord::ParseInternal(BufferReader* reader, MediaLog* media_log)
{
    uint8_t av1c_byte = 0;
    ...
    if (av1c_version != 1) {
        MEDIA_LOG(ERROR, media_log) << "Unsupported av1C: unexpected version number: " << av1c_version;
        return false;
    }
    ...
    case 2:
        ...
        break;
    default:
        MEDIA_LOG(ERROR, media_log) << "Unsupported av1C: unknown profile 0x" << std::hex << seq_profile;
        return false;
    }
...
ColorParameterInformation::ColorParameterInformation(const ColorParameterInformation& other) = default;
ColorParameterInformation::~ColorParameterInformation() = default;
...
bool ColorParameterInformation::Parse(BoxReader* reader)
{
    ...
    RCHECK(
        reader->Read2(&colour_primaries) && reader->Read2(&transfer_characteristics) && reader->Read2(&matrix_coefficients) && reader->Read1(&full_range_byte));
...
MasteringDisplayColorVolume::MasteringDisplayColorVolume(const MasteringDisplayColorVolume& other) = default;
MasteringDisplayColorVolume::~MasteringDisplayColorVolume() = default;
...
bool MasteringDisplayColorVolume::Parse(BoxReader* reader)
{
    ...
    RCHECK(ReadMdcvColorCoordinate(reader, &display_primaries_gx) && ReadMdcvColorCoordinate(reader, &display_primaries_gy)
        && ReadMdcvColorCoordinate(reader, &display_primaries_bx) && ReadMdcvColorCoordinate(reader, &display_primaries_by)
        && ReadMdcvColorCoordinate(reader, &display_primaries_rx) && ReadMdcvColorCoordinate(reader, &display_primaries_ry)
        && ReadMdcvColorCoordinate(reader, &white_point_x) && ReadMdcvColorCoordinate(reader, &white_point_y)
        && ReadFixedPoint32(kUnitOfMasteringLuminance, reader, &max_display_mastering_luminance)
        && ReadFixedPoint32(kUnitOfMasteringLuminance, reader, &min_display_mastering_luminance));
    return true;
}
...
bool SMPTE2086MasteringDisplayMetadataBox::Parse(BoxReader* reader)
{
    ...
    RCHECK(ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_rx) && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_ry)
        && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_gx) && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_gy)
        && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_bx) && ReadFixedPoint16(kColorCoordinateUnit, reader, &display_primaries_by)
        && ReadFixedPoint16(kColorCoordinateUnit, reader, &white_point_x) && ReadFixedPoint16(kColorCoordinateUnit, reader, &white_point_y)
        && ReadFixedPoint32(kLuminanceMaxUnit, reader, &max_display_mastering_luminance)
        && ReadFixedPoint32(kLuminanceMinUnit, reader, &min_display_mastering_luminance));
    return true;
}

ContentLightLevelInformation::ContentLightLevelInformation() = default;
ContentLightLevelInformation::ContentLightLevelInformation(const ContentLightLevelInformation& other) = default;
ContentLightLevelInformation::~ContentLightLevelInformation() = default;
...
{
    return reader->Read2(&max_content_light_level) && reader->Read2(&max_pic_average_light_level);
}
...
VideoSampleEntry::VideoSampleEntry()
    ...
    , video_info({ VideoCodec::kUnknown, VIDEO_CODEC_PROFILE_UNKNOWN, kNoVideoCodecLevel })
{
}
...
// static
VideoColorSpace VideoSampleEntry::ConvertColorParameterInformationToColorSpace(const ColorParameterInformation& info)
{
    ...
    return VideoColorSpace(info.colour_primaries, info.transfer_characteristics, info.matrix_coefficients,
        info.full_range ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED);
}

bool VideoSampleEntry::Parse(BoxReader* reader)
{
    format = reader->type();
    RCHECK(reader->SkipBytes(6) && reader->Read2(&data_reference_index) && reader->SkipBytes(16) && reader->Read2(&width) && reader->Read2(&height)
        && reader->SkipBytes(50));
    ...
    case FOURCC_AVC3: {
        ...
        std::unique_ptr<AVCDecoderConfigurationRecord> avc_config(new AVCDecoderConfigurationRecord());
        RCHECK(reader->ReadChild(avc_config.get()));
        video_info.codec = VideoCodec::kH264;
        video_info.profile = H264Parser::ProfileIDCToVideoCodecProfile(avc_config->profile_indication);
        // It can be Dolby Vision stream if there is dvvC box.
        std::tie(video_info, dv_info) = MaybeParseDOVI(reader, video_info);
        frame_bitstream_converter = base::MakeRefCounted<AVCBitstreamConverter>(std::move(avc_config));
        break;
    }
#if BUILDFLAG(ENABLE_PLATFORM_HEVC)
    case FOURCC_HEV1:
    case FOURCC_HVC1: {
        ...
        std::unique_ptr<HEVCDecoderConfigurationRecord> hevc_config(new HEVCDecoderConfigurationRecord());
        ...
        std::tie(video_info, dv_info) = MaybeParseDOVI(reader, video_info);
        frame_bitstream_converter = base::MakeRefCounted<HEVCBitstreamConverter>(std::move(hevc_config));
        break;
    }
#endif // BUILDFLAG(ENABLE_PLATFORM_HEVC)
    case FOURCC_DVA1:
    ...
    case FOURCC_VP09: {
        ...
        std::unique_ptr<VPCodecConfigurationRecord> vp_config(new VPCodecConfigurationRecord());
        ...
        if (reader->HasChild(&color_volume)) {
            ...
            hdr_static_metadata.smpte_st_2086 = ConvertMdcvToColorVolumeMetadata(color_volume);
        }

        ContentLightLevel level_information;
        if (reader->HasChild(&level_information)) {
            RCHECK(reader->ReadChild(&level_information));
            hdr_static_metadata.cta_861_3 = gfx::HdrMetadataCta861_3(level_information.max_content_light_level, level_information.max_pic_average_light_level);
        }
        break;
    }
#if BUILDFLAG(ENABLE_AV1_DECODER)
    ...
    default:
        ...
        MEDIA_LOG(ERROR, reader->media_log()) << "Unsupported VisualSampleEntry type " << FourCCToString(actual_format);
        return false;
    }
    ...
    if (reader->HasChild(&color_parameter_information)) {
        ...
        if (color_parameter_information.fully_parsed) {
            video_color_space = ConvertColorParameterInformationToColorSpace(color_parameter_information);
        }
    }
...
ElementaryStreamDescriptor::ElementaryStreamDescriptor(const ElementaryStreamDescriptor& other) = default;
...
{
    RCHECK(reader->ReadFullBoxHeader());
    RCHECK_MEDIA_LOGGED(reader->version() == 0, reader->media_log(), "Only version 0 FLACSpecificBox (dfLa) is supported.");
    RCHECK_MEDIA_LOGGED(reader->flags() == 0, reader->media_log(), "Only 0 flags in FLACSpecificBox (dfLa) is supported.");
    ...
    RCHECK_MEDIA_LOGGED(reader->Read4(&metadata_framing), reader->media_log(), "Missing STREAMINFO block in FLACSpecificBox (dfLa).");
    uint8_t block_type = (metadata_framing >> 24) & 0x7f;
    RCHECK_MEDIA_LOGGED(block_type == 0, reader->media_log(), "FLACSpecificBox metadata must begin with STREAMINFO.");
    uint32_t block_length = metadata_framing & 0x00ffffff;
    RCHECK_MEDIA_LOGGED(
        block_length == kFlacMetadataBlockStreaminfoSize, reader->media_log(), "STREAMINFO block in FLACSpecificBox (dfLa) has incorrect size.");
...
OpusSpecificBox::OpusSpecificBox()
    : seek_preroll(base::Milliseconds(80))
...
bool OpusSpecificBox::Parse(BoxReader* reader)
{
    ...
    extradata.insert(extradata.end(), { 0x4f, 0x70, 0x75, 0x73, 0x48, 0x65, 0x61, 0x64 });
...
#error The code below assumes little-endianness.
    ...
    memcpy(&extradata[OPUS_EXTRADATA_SKIP_SAMPLES_OFFSET], &codec_delay_in_frames, sizeof(codec_delay_in_frames));
    memcpy(&extradata[OPUS_EXTRADATA_SAMPLE_RATE_OFFSET], &sample_rate, sizeof(sample_rate));
    ...
    for (size_t i = OPUS_EXTRADATA_CHANNEL_MAPPING_OFFSET; i < extradata.size(); ++i) {
...
bool IamfSpecificBox::Parse(BoxReader* reader)
{
    ...
    base::span<const uint8_t> buffer = reader->buffer().subspan(reader->pos(), config_obus_size);
...
bool IamfSpecificBox::ReadOBUHeader(BufferReader* reader, uint8_t* obu_type, uint32_t* obu_size)
{
...
bool IamfSpecificBox::ReadLeb128Value(BufferReader* reader, uint32_t* value) const
{
    DCHECK(reader);
    DCHECK(value);
    *value = 0;
...
{
    format = reader->type();
    RCHECK(reader->SkipBytes(6) && reader->Read2(&data_reference_index) && reader->SkipBytes(8) && reader->Read2(&channelcount) && reader->Read2(&samplesize)
        && reader->SkipBytes(4) && reader->Read4(&samplerate));
    ...
    if (format == FOURCC_OPUS || (format == FOURCC_ENCA && sinf.format.format == FOURCC_OPUS)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&dops), reader->media_log(), "Failure parsing OpusSpecificBox (dOps)");
    ...
    if (format == FOURCC_DTSC || format == FOURCC_DTSE) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&ddts), reader->media_log(), "Failure parsing DtsSpecificBox (ddts)");
    } else if (format == FOURCC_DTSX) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&udts), reader->media_log(), "Failure parsing DtsUhdSpecificBox (udts)");
...
#if BUILDFLAG(ENABLE_PLATFORM_AC3_EAC3_AUDIO)
    if (format == FOURCC_AC3 || (format == FOURCC_ENCA && sinf.format.format == FOURCC_AC3)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&ac3), reader->media_log(), "Failure parsing AC3SpecificBox (dac3)");
    }
    if (format == FOURCC_EAC3 || (format == FOURCC_ENCA && sinf.format.format == FOURCC_EAC3)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&eac3), reader->media_log(), "Failure parsing EC3SpecificBox (dec3)");
    }
#endif // BUILDFLAG(ENABLE_PLATFORM_AC3_EAC3_AUDIO)

#if BUILDFLAG(ENABLE_PLATFORM_AC4_AUDIO)
    if (format == FOURCC_AC4 || (format == FOURCC_ENCA && sinf.format.format == FOURCC_AC4)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&ac4), reader->media_log(), "Failure parsing AC4SpecificBox (dac4)");
    }
#endif // BUILDFLAG(ENABLE_PLATFORM_AC4_AUDIO)

#if BUILDFLAG(ENABLE_PLATFORM_IAMF_AUDIO)
    if (format == FOURCC_IAMF || (format == FOURCC_ENCA && sinf.format.format == FOURCC_IAMF)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&iacb), reader->media_log(), "Failure parsing IamfSpecificBox (iacb)");
    }
#endif // BUILDFLAG(ENABLE_PLATFORM_IAMF_AUDIO)

    // Read the FLACSpecificBox, even if CENC is signalled.
    if (format == FOURCC_FLAC || (format == FOURCC_ENCA && sinf.format.format == FOURCC_FLAC)) {
        RCHECK_MEDIA_LOGGED(reader->ReadChild(&dfla), reader->media_log(), "Failure parsing FLACSpecificBox (dfLa)");
        ...
        if (samplerate != dfla.sample_rate) {
            MEDIA_LOG(INFO, reader->media_log()) << "FLAC AudioSampleEntry sample rate " << samplerate << " overridden by rate " << dfla.sample_rate
...
bool MediaHeader::Parse(BoxReader* reader)
{
    ...
    RCHECK_MEDIA_LOGGED(timescale > 0, reader->media_log(), "Track media header's timescale must not be 0");
...
std::string MediaHeader::language() const
{
    ...
    if (lang_chars[0] < 'a' || lang_chars[0] > 'z' || lang_chars[1] < 'a' || lang_chars[1] > 'z' || lang_chars[2] < 'a' || lang_chars[2] > 'z') {
        ...
        DVLOG(2) << "Ignoring MDHD language_code (non ISO 639-2 compliant): " << lang_chars;
        lang_chars[0] = 'u';
        lang_chars[1] = 'n';
        lang_chars[2] = 'd';
    }
...
{
    RCHECK(reader->ReadFullBoxHeader() && reader->Read4(&track_id) && reader->Read4(&default_sample_description_index)
        && reader->Read4(&default_sample_duration) && reader->Read4(&default_sample_size) && reader->Read4(&default_sample_flags));
    return true;
}
...
TrackFragmentDecodeTime::TrackFragmentDecodeTime(const TrackFragmentDecodeTime& other) = default;
...
bool TrackFragmentRun::Parse(BoxReader* reader)
{
    ...
    if (sample_duration_present || sample_size_present || sample_flags_present || sample_composition_time_offsets_present) {
...
bool SampleToGroup::Parse(BoxReader* reader)
{
    ...
    if (grouping_type != FOURCC_SEIG) {
        DLOG(WARNING) << "SampleToGroup box with grouping_type '" << grouping_type << "' is not supported.";
        return true;
    }
...
CencSampleEncryptionInfoEntry::CencSampleEncryptionInfoEntry(const CencSampleEncryptionInfoEntry& other) = default;
...
SampleGroupDescription::SampleGroupDescription(const SampleGroupDescription& other) = default;
...
bool SampleGroupDescription::Parse(BoxReader* reader)
{
    ...
    if (grouping_type != FOURCC_SEIG) {
        DLOG(WARNING) << "SampleGroupDescription box with grouping_type '" << grouping_type << "' is not supported.";
        return true;
    }
...
bool TrackFragment::Parse(BoxReader* reader)
{
    RCHECK(reader->ScanChildren() && reader->ReadChild(&header) &&
        ...
        reader->ReadChild(&decode_time) && reader->MaybeReadChildren(&runs) && reader->MaybeReadChild(&auxiliary_offset)
        && reader->MaybeReadChild(&auxiliary_size) && reader->MaybeReadChild(&sdtp) && reader->MaybeReadChild(&sample_encryption));
...
IndependentAndDisposableSamples::IndependentAndDisposableSamples(const IndependentAndDisposableSamples& other) = default;
IndependentAndDisposableSamples::~IndependentAndDisposableSamples() = default;
...
SampleDependsOn IndependentAndDisposableSamples::sample_depends_on(size_t i) const
{
