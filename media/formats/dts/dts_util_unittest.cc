...
TEST_F(DTSUtilTest, IncompleteInputTestDTS)
{
    ...
    EXPECT_EQ(0, media::dts::ParseTotalSampleCount(data, stream.length() - 1, AudioCodec::kDTS));
}
...
TEST_F(DTSUtilTest, IncompleteInputTestDTSXP2)
{
    ...
    const uint8_t* data = stream.data();
    EXPECT_EQ(0, media::dts::ParseTotalSampleCount(data, stream.length() - 1, AudioCodec::kDTSXP2));
}
...
TEST_F(DTSUtilTest, NormalInputTestDTS)
{
    ...
    ASSERT_TRUE(stream.Initialize(file_path)) << "Couldn't open stream file: " << file_path.MaybeAsASCII();
    const uint8_t* data = stream.data();
    int total = media::dts::ParseTotalSampleCount(data, stream.length(), AudioCodec::kDTS);
    EXPECT_EQ(total, 512);
}
...
TEST_F(DTSUtilTest, WrapDTSWithIEC61937IncorrectInputTest)
{
    ...
    input_data = std::vector<uint8_t>(short_input, short_input + sizeof(short_input));
    EXPECT_EQ(0, media::dts::WrapDTSWithIEC61937(input_data, output_data, AudioCodec::kDTS));

    input_data = std::vector<uint8_t>(long_input, long_input + sizeof(long_input));
    EXPECT_EQ(0, media::dts::WrapDTSWithIEC61937(input_data, output_data, AudioCodec::kDTS));
}

TEST_F(DTSUtilTest, WrapDTSWithIEC61937NormalInputTest)
{
    constexpr uint8_t header[8] = { 0x72, 0xF8, 0x1F, 0x4E, 0x0B, 0x00, 0x00, 0x20 };
    ...
    EXPECT_EQ(2048, media::dts::WrapDTSWithIEC61937(input_data, output_data, AudioCodec::kDTS));
