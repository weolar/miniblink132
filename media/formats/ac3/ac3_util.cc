...
const int kBitrate[] = { 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 448, 512, 576, 640 };
// 16-bit words per synchronization frame, indexed by frmsizecod.
const int kSyncFrameSizeInWordsFor44kHz[] = { 69, 70, 87, 88, 104, 105, 121, 122, 139, 140, 174, 175, 208, 209, 243, 244, 278, 279, 348, 349, 417, 418, 487,
    488, 557, 558, 696, 697, 835, 836, 975, 976, 1114, 1115, 1253, 1254, 1393, 1394 };
...
public:
    ...
    uint32_t eac3_frame_size_code() const
    {
        return eac3_frame_size_code_;
    }

    uint32_t sample_rate_code() const
    {
        return sample_rate_code_;
    }
    ...
    uint32_t ac3_frame_size_code() const
    {
        return ac3_frame_size_code_;
    }
...
const uint8_t* FindNextSyncWord(const uint8_t* const begin, const uint8_t* const end)
{
    DCHECK(begin);
    DCHECK(end);
    DCHECK_LE(begin, end);
...
{
    unsigned blocks = header.sample_rate_code() == 0x03 ? 6 : kBlocksPerSyncFrame[header.eac3_number_of_audio_block_code()];
...
{
    if (header.sample_rate_code() >= std::size(kSampleRate) || header.ac3_frame_size_code() >= std::size(kSyncFrameSizeInWordsFor44kHz)) {
        DVLOG(2) << __func__ << " Invalid frame header."
                 << " fscod:" << header.sample_rate_code() << " frmsizecod:" << header.ac3_frame_size_code();
        return -1;
    }
...
int ParseTotalSampleCount(const uint8_t* data, size_t size, bool is_eac3)
{
    ...
    while (current && end - current > kHeaderSizeInByte) {
        ...
        int frame_size = is_eac3 ? ParseEac3SyncFrameSize(header) : ParseAc3SyncFrameSize(header);
        int sample_count = is_eac3 ? ParseEac3SyncFrameSampleCount(header) : GetAc3SyncFrameSampleCount();

        if (frame_size > 0 && sample_count > 0) {
            current += frame_size;
            if (current > end) {
                DVLOG(2) << __func__ << " Incomplete frame, missing " << current - end << " bytes.";
                break;
            }

            total_sample_count += sample_count;
        } else {
            DVLOG(2) << __func__ << " Invalid frame, skip 2 bytes to find next synchronization word.";
            current += 2;
        }
