...
FUZZ_TEST(Ac3UtilTest, ParseTotalAc3SampleCountNeverCrashes).WithDomains(fuzztest::NonEmpty(fuzztest::Arbitrary<std::vector<uint8_t>>()));
...
FUZZ_TEST(Ac3UtilTest, ParseTotalEac3SampleCountNeverCrashes).WithDomains(fuzztest::NonEmpty(fuzztest::Arbitrary<std::vector<uint8_t>>()));
