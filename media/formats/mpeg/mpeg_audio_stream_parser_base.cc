...
MPEGAudioStreamParserBase::MPEGAudioStreamParserBase(uint32_t start_code_mask, AudioCodec audio_codec, int codec_delay)
...
void MPEGAudioStreamParserBase::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
bool MPEGAudioStreamParserBase::AppendToParseBuffer(base::span<const uint8_t> buf)
{
...
StreamParser::ParseStatus MPEGAudioStreamParserBase::Parse(int max_pending_bytes_to_inspect)
{
    DVLOG(1) << __func__;
    ...
    int inspection_increment = std::min(max_pending_bytes_to_inspect, uninspected_pending_bytes_);
...
int MPEGAudioStreamParserBase::ParseFrame(const uint8_t* data, int size, BufferQueue* buffers)
{
    ...
    int bytes_read = ParseFrameHeader(data, size, &frame_size, &sample_rate, &channel_layout, &sample_count, &metadata_frame, &extra_data);
    ...
    DVLOG(2) << " sample_rate " << sample_rate << " channel_layout " << channel_layout << " frame_size " << frame_size << " sample_count " << sample_count;
    ...
    if (!config_.IsValidConfig()) {
        config_.Initialize(
            audio_codec_, kSampleFormatF32, channel_layout, sample_rate, extra_data, EncryptionScheme::kUnencrypted, base::TimeDelta(), codec_delay_);
        ...
        if (config_.IsValidConfig()) {
            media_tracks->AddAudioTrack(config_, true, kMpegAudioTrackId, MediaTrack::Kind("main"), MediaTrack::Label(), MediaTrack::Language());
        }
    ...
    scoped_refptr<StreamParserBuffer> buffer = StreamParserBuffer::CopyFrom(data, frame_size, true, DemuxerStream::AUDIO, kMpegAudioTrackId);
...
int MPEGAudioStreamParserBase::ParseIcecastHeader(const uint8_t* data, int size)
{
...
bool MPEGAudioStreamParserBase::ParseSyncSafeInt(BitReader* reader, int32_t* value)
{
    *value = 0;
    for (int i = 0; i < 4; ++i) {
        uint8_t tmp;
...
int MPEGAudioStreamParserBase::FindNextValidStartCode(const uint8_t* data, int size)
{
    ...
    while (start < end) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            int frame_size;
            int sync_bytes = ParseFrameHeader(sync, sync_size, &frame_size, nullptr, nullptr, nullptr, nullptr, nullptr);
...
bool MPEGAudioStreamParserBase::SendBuffers(BufferQueue* buffers, bool end_of_segment)
{
