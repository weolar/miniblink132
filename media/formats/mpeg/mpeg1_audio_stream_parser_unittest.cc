...
class MPEG1AudioStreamParserTest : public StreamParserTestBase, public testing::Test {
public:
...
TEST_F(MPEG1AudioStreamParserTest, MetadataParsing)
{
    ...
    EXPECT_EQ("", ParseData(buffer->AsSpan().subspan(offset, kXingRemainingSize)));
    ...
    EXPECT_EQ("NewSegment{ 0K }EndOfSegment", ParseData(buffer->AsSpan().subspan(offset, kFirstRealFrameSize)));
    EXPECT_TRUE(last_audio_config().IsValidConfig());
}
