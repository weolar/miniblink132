...
public:
    using NewVideoConfigCB = base::RepeatingCallback<void(const VideoDecoderConfig&)>;

    EsParserH264(NewVideoConfigCB new_video_config_cb, EmitBufferCB emit_buffer_cb);
    EsParserH264(NewVideoConfigCB new_video_config_cb, EmitBufferCB emit_buffer_cb, EncryptionScheme init_encryption_scheme,
...
private:
    ...
    bool EmitFrame(int64_t access_unit_pos, int access_unit_size, bool is_key_frame, int pps_id);
