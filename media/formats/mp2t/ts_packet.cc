    ...
    for (; k < size; k++) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (buf[idx] != kTsHeaderSyncword) {
                DVLOG(LOG_LEVEL_TS) << "ByteSync" << idx << ": " << std::hex << static_cast<int>(buf[idx]) << std::dec;
                is_header = false;
                break;
            }
        }
...
bool TsPacket::ParseAdaptationField(BitReader* bit_reader, int adaptation_field_length)
{
    ...
    for (int k = 0; k < adaptation_field_remaining_size; k++) {
        ...
        DVLOG_IF(1, stuffing_byte != 0xff) << "Stream not compliant: invalid stuffing byte " << std::hex << stuffing_byte;
    }
