...
VideoDecoderConfig CreateFakeVideoConfig()
{
    ...
    return VideoDecoderConfig(VideoCodec::kH264, H264PROFILE_MAIN, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace(), kNoTransformation, coded_size,
        visible_rect, natural_size, EmptyExtraData(), EncryptionScheme::kUnencrypted);
}

BufferQueue GenerateFakeBuffers(const int* frame_pts_ms, const bool* is_key_frame, size_t frame_count)
{
    uint8_t dummy_buffer[] = { 0, 0, 0, 0 };

    BufferQueue buffers(frame_count);
    for (size_t k = 0; k < frame_count; k++) {
        buffers[k] = StreamParserBuffer::CopyFrom(dummy_buffer, std::size(dummy_buffer), is_key_frame[k], DemuxerStream::VIDEO, 0);
        if (frame_pts_ms[k] < 0) {
...
EsAdapterVideoTest::EsAdapterVideoTest()
    : es_adapter_(base::BindRepeating(&EsAdapterVideoTest::OnNewConfig, base::Unretained(this)),
        base::BindRepeating(&EsAdapterVideoTest::OnNewBuffer, base::Unretained(this)))
{
}
...
{
    buffer_descriptors_ << "(" << buffer->duration().InMilliseconds() << "," << (buffer->is_key_frame() ? "Y" : "N") << ") ";
}

std::string EsAdapterVideoTest::RunAdapterTest(const BufferQueue& buffer_queue)
{
    buffer_descriptors_.clear();

    es_adapter_.OnConfigChanged(CreateFakeVideoConfig());
    for (BufferQueue::const_iterator it = buffer_queue.begin(); it != buffer_queue.end(); ++it) {
...
TEST_F(EsAdapterVideoTest, FrameDurationSimpleGop)
{
    ...
    BufferQueue buffer_queue = GenerateFakeBuffers(pts_ms, is_key_frame, std::size(pts_ms));

    EXPECT_EQ("(1,Y) (2,N) (3,N) (4,N) (5,N) (6,N) (7,N) (7,N)", RunAdapterTest(buffer_queue));
}
...
TEST_F(EsAdapterVideoTest, FrameDurationComplexGop)
{
    ...
    bool is_key_frame[] = { true, false, false, false, false, false, false, false, false, false };
    BufferQueue buffer_queue = GenerateFakeBuffers(pts_ms, is_key_frame, std::size(pts_ms));
...
TEST_F(EsAdapterVideoTest, LeadingNonKeyFrames)
{
    ...
    EXPECT_EQ("(30,Y) (30,Y) (30,Y) (30,Y) (30,N) (30,N)", RunAdapterTest(buffer_queue));
}
