...
static std::vector<int64_t> TruncateTimestamps(const std::vector<int64_t>& timestamps)
{
    const int nbits = 33;
...
TEST(TimestampUnrollerTest, SingleStream)
{
    ...
    std::vector<int64_t> timestamps_vector(timestamps, timestamps + std::size(timestamps));
