...
EsAdapterVideo::EsAdapterVideo(NewVideoConfigCB new_video_config_cb, EmitBufferCB emit_buffer_cb)
...
void EsAdapterVideo::OnConfigChanged(const VideoDecoderConfig& video_decoder_config)
{
    config_list_.push_back(ConfigEntry(buffer_index_ + buffer_list_.size(), video_decoder_config));
    has_valid_config_ = true;
    ProcessPendingBuffers(false);
}

bool EsAdapterVideo::OnNewBuffer(scoped_refptr<StreamParserBuffer> stream_parser_buffer)
{
    if (stream_parser_buffer->timestamp() == kNoTimestamp) {
        ...
        if (!has_valid_initial_timestamp_) {
            ...
            DVLOG(1) << "Stream not compliant: ignoring leading frame with no timestamp";
            return true;
        }
    ...
    while (!buffer_list_.empty() && (flush || buffer_list_.size() > kHistorySize)) {
        ...
        if (buffer->duration() == kNoTimestamp) {
            ...
            if (next_frame_pts == kNoTimestamp) {
                ...
                DVLOG(LOG_LEVEL_ES) << "Using last frame duration: " << last_frame_duration_.InMilliseconds();
...
base::TimeDelta EsAdapterVideo::GetNextFramePts(base::TimeDelta current_pts)
{
    ...
    for (BufferQueue::const_iterator it = buffer_list_.begin(); it != buffer_list_.end(); ++it) {
    ...
    for (std::list<base::TimeDelta>::const_iterator it = emitted_pts_.begin(); it != emitted_pts_.end(); ++it) {
        if (*it < current_pts)
            continue;
...
void EsAdapterVideo::ReplaceDiscardedFrames(const StreamParserBuffer& stream_parser_buffer)
{
    ...
    for (int i = 0; i < discarded_frame_count_; i++) {
        scoped_refptr<StreamParserBuffer> frame = StreamParserBuffer::CopyFrom(stream_parser_buffer.data(), stream_parser_buffer.size(),
            stream_parser_buffer.is_key_frame(), stream_parser_buffer.type(), stream_parser_buffer.track_id());
