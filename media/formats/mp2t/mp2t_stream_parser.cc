...
std::optional<base::flat_set<int>> MapAllowedStreamTypes(std::optional<base::span<const std::string>> allowed_codecs)
{
...
public:
    ...
    PidState(int pid, PidType pid_type, std::unique_ptr<TsSection> section_parser);
...
PidState::PidState(int pid, PidType pid_type, std::unique_ptr<TsSection> section_parser)
    : pid_(pid)
    , pid_type_(pid_type)
...
bool PidState::PushTsPacket(const TsPacket& ts_packet)
{
    ...
    bool status = section_parser_->Parse(ts_packet.payload_unit_start_indicator(), ts_packet.payload());
...
Mp2tStreamParser::BufferQueueWithConfig::BufferQueueWithConfig(bool is_cfg_sent, const AudioDecoderConfig& audio_cfg, const VideoDecoderConfig& video_cfg)
...
Mp2tStreamParser::BufferQueueWithConfig::BufferQueueWithConfig(const BufferQueueWithConfig& other) = default;

Mp2tStreamParser::BufferQueueWithConfig::~BufferQueueWithConfig()
{
}

Mp2tStreamParser::Mp2tStreamParser(std::optional<base::span<const std::string>> allowed_codecs, bool sbr_in_mimetype)
...
void Mp2tStreamParser::Init(InitCB init_cb, NewConfigCB config_cb, NewBuffersCB new_buffers_cb, EncryptedMediaInitDataCB encrypted_media_init_data_cb,
...
void Mp2tStreamParser::Flush()
{
    ...
    for (auto buffer_queue_iter = buffer_queue_chain_.begin(); buffer_queue_iter != buffer_queue_chain_.end(); ++buffer_queue_iter) {
...
bool Mp2tStreamParser::AppendToParseBuffer(base::span<const uint8_t> buf)
{
    ...
    if (!ts_byte_queue_.Push(buf)) {
        DVLOG(2) << "AppendToParseBuffer(): Failed to push buf of size " << buf.size();
        return false;
    }
...
StreamParser::ParseStatus Mp2tStreamParser::Parse(int max_pending_bytes_to_inspect)
{
    DVLOG(1) << __func__;
    ...
    int inspection_increment = std::min(base::checked_cast<size_t>(max_pending_bytes_to_inspect), uninspected_pending_bytes_);
    ...
    while (true) {
        ...
        std::unique_ptr<TsPacket> ts_packet(TsPacket::Parse(ts_buffer, ts_buffer_size));
        if (!ts_packet) {
        ...
        DVLOG(LOG_LEVEL_TS) << "Processing PID=" << ts_packet->pid() << " start_unit=" << ts_packet->payload_unit_start_indicator();

        // Parse the section.
        auto it = pids_.find(ts_packet->pid());
        if (it == pids_.end() && ts_packet->pid() == TsSection::kPidPat) {
            // Create the PAT state here if needed.
            auto pat_section_parser = std::make_unique<TsSectionPat>(base::BindRepeating(&Mp2tStreamParser::RegisterPmt, base::Unretained(this)));
            auto pat_pid_state = std::make_unique<PidState>(ts_packet->pid(), PidState::kPidPat, std::move(pat_section_parser));
            pat_pid_state->Enable();
            it = pids_.insert(std::make_pair(ts_packet->pid(), std::move(pat_pid_state))).first;
            ...
            it = pids_.insert(std::make_pair(TsSection::kPidCat, MakeCatPidState())).first;
        }
    ...
    if (!FinishInitializationIfNeeded()) {
        ...
        // method any longer.
        DLOG(WARNING) << "Failure while parsing Mpeg2TS: FinishInitializationIfNeeded()";
        return ParseStatus::kFailed;
    }
...
void Mp2tStreamParser::RegisterPmt(int program_number, int pmt_pid)
{
    ...
    for (const auto& pid_pair : pids_) {
        ...
        if (pid_state->pid_type() == PidState::kPidPmt) {
            DVLOG_IF(1, pmt_pid != pid_pair.first) << "More than one program is defined";
            return;
        }
    }
    ...
    auto pmt_section_parser = std::make_unique<TsSectionPmt>(base::BindRepeating(&Mp2tStreamParser::RegisterPes, base::Unretained(this)));
    auto pmt_pid_state = std::make_unique<PidState>(pmt_pid, PidState::kPidPmt, std::move(pmt_section_parser));
...
{
    auto on_video_config_changed = base::BindRepeating(&Mp2tStreamParser::OnVideoConfigChanged, base::Unretained(this), pes_pid);
    auto on_emit_video_buffer = base::BindRepeating(&Mp2tStreamParser::OnEmitVideoBuffer, base::Unretained(this), pes_pid);

    return std::make_unique<EsParserH264>(std::move(on_video_config_changed), std::move(on_emit_video_buffer));
}

std::unique_ptr<EsParser> Mp2tStreamParser::CreateAacParser(int pes_pid)
{
    auto on_audio_config_changed = base::BindRepeating(&Mp2tStreamParser::OnAudioConfigChanged, base::Unretained(this), pes_pid);
    auto on_emit_audio_buffer = base::BindRepeating(&Mp2tStreamParser::OnEmitAudioBuffer, base::Unretained(this), pes_pid);
    return std::make_unique<EsParserAdts>(on_audio_config_changed, std::move(on_emit_audio_buffer), sbr_in_mimetype_);
}

std::unique_ptr<EsParser> Mp2tStreamParser::CreateMpeg1AudioParser(int pes_pid)
{
    auto on_audio_config_changed = base::BindRepeating(&Mp2tStreamParser::OnAudioConfigChanged, base::Unretained(this), pes_pid);
    auto on_emit_audio_buffer = base::BindRepeating(&Mp2tStreamParser::OnEmitAudioBuffer, base::Unretained(this), pes_pid);
    return std::make_unique<EsParserMpeg1Audio>(on_audio_config_changed, std::move(on_emit_audio_buffer), media_log_);
}
...
std::unique_ptr<EsParser> Mp2tStreamParser::CreateEncryptedH264Parser(int pes_pid, bool emit_clear_buffers)
{
    auto on_video_config_changed = base::BindRepeating(&Mp2tStreamParser::OnVideoConfigChanged, base::Unretained(this), pes_pid);
    auto on_emit_video_buffer = base::BindRepeating(&Mp2tStreamParser::OnEmitVideoBuffer, base::Unretained(this), pes_pid);
    EsParserAdts::GetDecryptConfigCB get_decrypt_config;
    if (!emit_clear_buffers) {
        get_decrypt_config = base::BindRepeating(&Mp2tStreamParser::GetDecryptConfig, base::Unretained(this));
    }
    return std::make_unique<EsParserH264>(std::move(on_video_config_changed), on_emit_video_buffer, initial_encryption_scheme_, std::move(get_decrypt_config));
}

std::unique_ptr<EsParser> Mp2tStreamParser::CreateEncryptedAacParser(int pes_pid, bool emit_clear_buffers)
{
    auto on_audio_config_changed = base::BindRepeating(&Mp2tStreamParser::OnAudioConfigChanged, base::Unretained(this), pes_pid);
    auto on_emit_audio_buffer = base::BindRepeating(&Mp2tStreamParser::OnEmitAudioBuffer, base::Unretained(this), pes_pid);
    EsParserAdts::GetDecryptConfigCB get_decrypt_config;
    ...
    return std::make_unique<EsParserAdts>(
        on_audio_config_changed, std::move(on_emit_audio_buffer), std::move(get_decrypt_config), initial_encryption_scheme_, sbr_in_mimetype_);
}

void Mp2tStreamParser::RegisterPes(int pes_pid, int stream_type, const Descriptors& descriptors)
{
    // TODO(damienv): check there is no mismatch if the entry already exists.
    DVLOG(1) << "RegisterPes:"
             << " pes_pid=" << pes_pid << " stream_type=" << std::hex << stream_type << std::dec;
    ...
    if (allowed_stream_types_.has_value() && !allowed_stream_types_->contains(stream_type)) {
    ...
    case kStreamTypeAVC:
        ...
        if (ShouldForceEncryptedParser()) {
            es_parser = CreateEncryptedH264Parser(pes_pid, true /* emit_clear_buffers */);
            break;
        }
    ...
    case kStreamTypeAAC:
        if (ShouldForceEncryptedParser()) {
            es_parser = CreateEncryptedAacParser(pes_pid, true /* emit_clear_buffers */);
            break;
        }
    ...
    case kStreamTypeAVCWithSampleAES:
        if (descriptors.HasPrivateDataIndicator(kSampleAESPrivateDataIndicatorAVC)) {
            is_audio = false;
            es_parser = CreateEncryptedH264Parser(pes_pid, false /* emit_clear_buffers */);
        } else {
    ...
    case kStreamTypeAACWithSampleAES:
        if (descriptors.HasPrivateDataIndicator(kSampleAESPrivateDataIndicatorAAC)) {
            es_parser = CreateEncryptedAacParser(pes_pid, false /* emit_clear_buffers */);
        } else {
    ...
    auto pes_section_parser = std::make_unique<TsSectionPes>(std::move(es_parser), &timestamp_unroller_);
    PidState::PidType pid_type = is_audio ? PidState::kPidAudioPes : PidState::kPidVideoPes;
    auto pes_pid_state = std::make_unique<PidState>(pes_pid, pid_type, std::move(pes_section_parser));
...
void Mp2tStreamParser::UpdatePidFilter()
{
    ...
    for (auto it = pids_.begin(); it != pids_.end(); ++it) {
        ...
        if (it != lowest_audio_pid && it != lowest_video_pid
            && (pid_state->pid_type() == PidState::kPidAudioPes || pid_state->pid_type() == PidState::kPidVideoPes))
            pid_state->Disable();
    }
}

void Mp2tStreamParser::OnVideoConfigChanged(int pes_pid, const VideoDecoderConfig& video_decoder_config)
{
    ...
    if (!buffer_queue_chain_.empty() && !buffer_queue_chain_.back().video_config.IsValidConfig()) {
        ...
        BufferQueueWithConfig buffer_queue_with_config(
            false, buffer_queue_chain_.empty() ? AudioDecoderConfig() : buffer_queue_chain_.back().audio_config, video_decoder_config);
...
void Mp2tStreamParser::OnAudioConfigChanged(int pes_pid, const AudioDecoderConfig& audio_decoder_config)
{
    ...
    if (!buffer_queue_chain_.empty() && !buffer_queue_chain_.back().audio_config.IsValidConfig()) {
        ...
        // Create a new entry in |buffer_queue_chain_| with the updated configs.
        BufferQueueWithConfig buffer_queue_with_config(
            false, audio_decoder_config, buffer_queue_chain_.empty() ? VideoDecoderConfig() : buffer_queue_chain_.back().video_config);
        buffer_queue_chain_.push_back(buffer_queue_with_config);
    }
...
std::unique_ptr<MediaTracks> GenerateMediaTrackInfo(const AudioDecoderConfig& audio_config, const VideoDecoderConfig& video_config)
{
    ...
    if (audio_config.IsValidConfig()) {
        media_tracks->AddAudioTrack(audio_config, true, kMp2tAudioTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
    }
    if (video_config.IsValidConfig()) {
        media_tracks->AddVideoTrack(video_config, true, kMp2tVideoTrackId, MediaTrack::Kind("main"), MediaTrack::Label(""), MediaTrack::Language(""));
    }
    return media_tracks;
}
...
bool Mp2tStreamParser::FinishInitializationIfNeeded()
{
    ...
    std::unique_ptr<MediaTracks> media_tracks = GenerateMediaTrackInfo(queue_with_config.audio_config, queue_with_config.video_config);
...
void Mp2tStreamParser::OnEmitAudioBuffer(int pes_pid, scoped_refptr<StreamParserBuffer> stream_parser_buffer)
{
    DCHECK_EQ(pes_pid, selected_audio_pid_);

    DVLOG(LOG_LEVEL_ES) << "OnEmitAudioBuffer: "
                        << " size=" << stream_parser_buffer->size() << " dts=" << stream_parser_buffer->GetDecodeTimestamp().InMilliseconds()
                        << " pts=" << stream_parser_buffer->timestamp().InMilliseconds() << " dur=" << stream_parser_buffer->duration().InMilliseconds();
...
void Mp2tStreamParser::OnEmitVideoBuffer(int pes_pid, scoped_refptr<StreamParserBuffer> stream_parser_buffer)
{
    DCHECK_EQ(pes_pid, selected_video_pid_);

    DVLOG(LOG_LEVEL_ES) << "OnEmitVideoBuffer"
                        << " size=" << stream_parser_buffer->size() << " dts=" << stream_parser_buffer->GetDecodeTimestamp().InMilliseconds()
                        << " pts=" << stream_parser_buffer->timestamp().InMilliseconds() << " dur=" << stream_parser_buffer->duration().InMilliseconds()
...
bool Mp2tStreamParser::EmitRemainingBuffers()
{
    ...
    while (!buffer_queue_chain_.empty()) {
        ...
        if (!queue_with_config.audio_queue.empty())
            buffer_queue_map.insert(std::make_pair(kMp2tAudioTrackId, queue_with_config.audio_queue));
        if (!queue_with_config.video_queue.empty())
            buffer_queue_map.insert(std::make_pair(kMp2tVideoTrackId, queue_with_config.video_queue));
    ...
    BufferQueueWithConfig queue_with_config(true, last_audio_config, last_video_config);
...
{
    auto cat_section_parser = std::make_unique<TsSectionCat>(base::BindRepeating(&Mp2tStreamParser::RegisterCencPids, base::Unretained(this)),
        base::BindRepeating(&Mp2tStreamParser::RegisterEncryptionScheme, base::Unretained(this)));
    auto cat_pid_state = std::make_unique<PidState>(TsSection::kPidCat, PidState::kPidCat, std::move(cat_section_parser));
...
{
    auto ecm_parser = std::make_unique<TsSectionCetsEcm>(base::BindRepeating(&Mp2tStreamParser::RegisterNewKeyIdAndIv, base::Unretained(this)));
    auto ecm_pid_state = std::make_unique<PidState>(ca_pid, PidState::kPidCetsEcm, std::move(ecm_parser));
    ecm_pid_state->Enable();
    pids_.insert(std::make_pair(ca_pid, std::move(ecm_pid_state)));

    auto pssh_parser = std::make_unique<TsSectionCetsPssh>(base::BindRepeating(&Mp2tStreamParser::RegisterPsshBoxes, base::Unretained(this)));
    auto pssh_pid_state = std::make_unique<PidState>(pssh_pid, PidState::kPidCetsPssh, std::move(pssh_parser));
...
void Mp2tStreamParser::RegisterNewKeyIdAndIv(const std::string& key_id, const std::string& iv)
{
    if (!iv.empty()) {
        ...
        case EncryptionScheme::kCbcs:
            decrypt_config_ = DecryptConfig::CreateCbcsConfig(key_id, iv, {}, std::nullopt);
            break;
        }
    }
}

void Mp2tStreamParser::RegisterPsshBoxes(const std::vector<uint8_t>& init_data)
{
