...
std::string DecryptSampleAES(const std::string& key, const std::string& iv, const uint8_t* input, int input_size, bool has_pattern)
{
    DCHECK(input);
    ...
    ScopedCipherCTX ctx;
    EXPECT_EQ(EVP_CipherInit_ex(ctx.get(), cipher, NULL, reinterpret_cast<const uint8_t*>(key.data()), reinterpret_cast<const uint8_t*>(iv.data()), 0), 1);
    ...
    while (bytes_remaining) {
        ...
        EXPECT_EQ(EVP_CipherUpdate(ctx.get(), out_ptr, &unused, in_ptr, amount_to_decrypt), 1);
...
std::string DecryptBuffer(const StreamParserBuffer& buffer, EncryptionScheme scheme)
{
    ...
    // |has_pattern| = true.
    bool has_pattern = buffer.decrypt_config()->encryption_pattern() == EncryptionPattern(1, 9);

    std::string key;
    EXPECT_TRUE(LookupTestKeyString(buffer.decrypt_config()->key_id(), false, &key));
...
protected:
    ...
    bool OnNewConfig(std::unique_ptr<MediaTracks> tracks)
    {
        ...
        for (const auto& track : tracks->tracks()) {
            ...
            if (track->type() == MediaTrack::Type::kAudio) {
                ...
                LOG(ERROR) << "Unexpected track type " << base::to_underlying(track->type());
                EXPECT_TRUE(false);
            }
        }
    ...
    bool OnNewBuffers(const StreamParser::BufferQueueMap& buffer_queue_map)
    {
        ...
        const StreamParser::BufferQueue& audio_buffers = (itr_audio == buffer_queue_map.end()) ? empty_buffers : itr_audio->second;

        const auto& itr_video = buffer_queue_map.find(video_track_id_);
        const StreamParser::BufferQueue& video_buffers = (itr_video == buffer_queue_map.end()) ? empty_buffers : itr_video->second;
    ...
    {
        parser_->Init(base::BindOnce(&Mp2tStreamParserTest::OnInit, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnNewConfig, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnNewBuffers, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnKeyNeeded, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnNewSegment, base::Unretained(this)),
            base::BindRepeating(&Mp2tStreamParserTest::OnEndOfSegment, base::Unretained(this)), &media_log_);
    }
...
TEST_F(Mp2tStreamParserTest, TimestampWrapAround)
{
    ...
    EXPECT_TRUE(IsAlmostEqual(video_min_dts_, DecodeTimestamp::FromSecondsD(95443.376)));
    EXPECT_TRUE(IsAlmostEqual(video_max_dts_, DecodeTimestamp::FromSecondsD(95446.079)));
    ...
    EXPECT_TRUE(IsAlmostEqual(audio_min_dts_, DecodeTimestamp::FromSecondsD(95443.400)));
    EXPECT_TRUE(IsAlmostEqual(audio_max_dts_, DecodeTimestamp::FromSecondsD(95446.117)));
}
...
TEST_F(Mp2tStreamParserTest, HLSSampleAES)
{
    ...
    for (size_t i = 0; i + 1 < video_buffer_capture_.size(); i++) {
        ...
        std::string unencrypted_video_buffer(reinterpret_cast<const char*>(buffer->data()), buffer->size());
    ...
    for (size_t i = 0; i + 1 < audio_buffer_capture_.size(); i++) {
        ...
        std::string unencrypted_audio_buffer(reinterpret_cast<const char*>(buffer->data()), buffer->size());
