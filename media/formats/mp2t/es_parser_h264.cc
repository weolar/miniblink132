...
std::unique_ptr<uint8_t[]> AdjustAUForSampleAES(const uint8_t* au, int* au_size, const Ranges<int>& protected_blocks, std::vector<SubsampleEntry>* subsamples)
{
    DCHECK(subsamples);
    DCHECK(au_size);
    ...
    for (size_t i = 0; i < protected_blocks.size(); i++) {
        ...
        if (end_pos - start_pos > kSampleAESMaxUnprotectedNALULength)
            ...
            VLOG(1) << "Ignoring short protected block of length: " << (end_pos - start_pos);
    }
    ...
    if (adjustment) {
        ...
        au = temp;
        VLOG(2) << "Copied AU and removed emulation prevention bytes: " << adjustment;
    }
...
EsParserH264::EsParserH264(NewVideoConfigCB new_video_config_cb, EmitBufferCB emit_buffer_cb)
...
EsParserH264::EsParserH264(
    NewVideoConfigCB new_video_config_cb, EmitBufferCB emit_buffer_cb, EncryptionScheme init_encryption_scheme, const GetDecryptConfigCB& get_decrypt_config_cb)
    : es_adapter_(std::move(new_video_config_cb), std::move(emit_buffer_cb))
    , h264_parser_(new H264Parser())
    , current_access_unit_pos_(0)
    , next_access_unit_pos_(0)
...
bool EsParserH264::FindAUD(int64_t* stream_pos)
{
    while (true) {
        ...
        bool start_code_found = H264Parser::FindStartCode(es, size, &start_code_offset, &start_code_size);
...
bool EsParserH264::ParseFromEsQueue()
{
    ...
    int access_unit_size = base::checked_cast<int>(next_access_unit_pos_ - current_access_unit_pos_);
    ...
    while (true) {
        ...
        case H264NALU::kNonIDRSlice: {
            ...
            if (get_decrypt_config_cb_ && get_decrypt_config_cb_.Run() && nalu.size > kSampleAESMaxUnprotectedNALULength) {
    ...
    RCHECK(EmitFrame(current_access_unit_pos_, access_unit_size, is_key_frame, pps_id_for_access_unit));
...
bool EsParserH264::EmitFrame(int64_t access_unit_pos, int access_unit_size, bool is_key_frame, int pps_id)
{
    ...
    // Emit a frame.
    DVLOG(LOG_LEVEL_ES) << "Emit frame: stream_pos=" << current_access_unit_pos_ << " size=" << access_unit_size;
    int es_size;
    const uint8_t* es;
    ...
    if (base_decrypt_config) {
        adjusted_au = AdjustAUForSampleAES(es, &access_unit_size, protected_blocks_, &subsamples);
    ...
    scoped_refptr<StreamParserBuffer> stream_parser_buffer
        = StreamParserBuffer::CopyFrom(es, access_unit_size, is_key_frame, DemuxerStream::VIDEO, kMp2tVideoTrackId);
    ...
    if (base_decrypt_config) {
        ...
        case EncryptionScheme::kCenc:
            stream_parser_buffer->set_decrypt_config(DecryptConfig::CreateCencConfig(base_decrypt_config->key_id(), base_decrypt_config->iv(), subsamples));
            break;
        case EncryptionScheme::kCbcs:
            ...
            stream_parser_buffer->set_decrypt_config(DecryptConfig::CreateCbcsConfig(
                base_decrypt_config->key_id(), base_decrypt_config->iv(), subsamples, EncryptionPattern(kSampleAESEncryptBlocks, kSampleAESSkipBlocks)));
            break;
        }
    }
...
bool EsParserH264::UpdateVideoDecoderConfig(const H264SPS* sps, EncryptionScheme scheme)
{
    ...
    if (visible_rect->width() > std::numeric_limits<int>::max() / sar_width) {
        DVLOG(1) << "Integer overflow detected: visible_rect.width()=" << visible_rect->width() << " sar_width=" << sar_width;
        return false;
    }
    gfx::Size natural_size((visible_rect->width() * sar_width) / sar_height, visible_rect->height());
    if (natural_size.width() == 0)
        return false;

    VideoCodecProfile profile = H264Parser::ProfileIDCToVideoCodecProfile(sps->profile_idc);
    if (profile == VIDEO_CODEC_PROFILE_UNKNOWN) {
        DVLOG(1) << "Unrecognized SPS profile_idc 0x" << std::hex << sps->profile_idc;
        return false;
    }

    VideoDecoderConfig video_decoder_config(VideoCodec::kH264, profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation,
        coded_size.value(), visible_rect.value(), natural_size, EmptyExtraData(), scheme);

    if (!video_decoder_config.IsValidConfig()) {
        DVLOG(1) << "Invalid video config: " << video_decoder_config.AsHumanReadableString();
        return false;
    }
    ...
    if (!video_decoder_config.Matches(last_video_decoder_config_)) {
        ...
        DVLOG(1) << "SAR: width=" << sps->sar_width << " height=" << sps->sar_height;
