...
bool EsParserAdts::LookForAdtsFrame(AdtsFrame* adts_frame)
{
    ...
    for (int offset = 0; offset < max_offset; offset++) {
        ...
        DVLOG(LOG_LEVEL_ES) << "ADTS syncword @ pos=" << adts_frame->queue_offset << " frame_size=" << adts_frame->size;
        DVLOG(LOG_LEVEL_ES) << "ADTS header: " << base::HexEncode(adts_frame->data, kADTSHeaderMinSize);
        return true;
    }
...
EsParserAdts::EsParserAdts(NewAudioConfigCB new_audio_config_cb, EmitBufferCB emit_buffer_cb, bool sbr_in_mimetype)
...
EsParserAdts::EsParserAdts(NewAudioConfigCB new_audio_config_cb, EmitBufferCB emit_buffer_cb, GetDecryptConfigCB get_decrypt_config_cb,
    EncryptionScheme init_encryption_scheme, bool sbr_in_mimetype)
    : new_audio_config_cb_(std::move(new_audio_config_cb))
    , emit_buffer_cb_(std::move(emit_buffer_cb))
...
void EsParserAdts::CalculateSubsamplesForAdtsFrame(const AdtsFrame& adts_frame, std::vector<SubsampleEntry>* subsamples)
{
    DCHECK(subsamples);
    subsamples->clear();
...
bool EsParserAdts::ParseFromEsQueue()
{
    ...
    while (LookForAdtsFrame(&adts_frame)) {
        ...
        scoped_refptr<StreamParserBuffer> stream_parser_buffer
            = StreamParserBuffer::CopyFrom(adts_frame.data, adts_frame.size, is_key_frame, DemuxerStream::AUDIO, kMp2tAudioTrackId);
        stream_parser_buffer->set_timestamp(current_pts);
        stream_parser_buffer->SetDecodeTimestamp(DecodeTimestamp::FromPresentationTime(current_pts));
        ...
        if (get_decrypt_config_cb_) {
            ...
            if (base_decrypt_config) {
                ...
                stream_parser_buffer->set_decrypt_config(std::make_unique<DecryptConfig>(
                    base_decrypt_config->encryption_scheme(), base_decrypt_config->key_id(), base_decrypt_config->iv(), subsamples, EncryptionPattern()));
            }
        }
...
bool EsParserAdts::UpdateAudioConfiguration(const uint8_t* adts_header, int size)
{
    int orig_sample_rate;
    ChannelLayout channel_layout;
    std::vector<uint8_t> extra_data;
    if (adts_parser_.ParseFrameHeader(adts_header, size, nullptr, &orig_sample_rate, &channel_layout, nullptr, nullptr, &extra_data) <= 0) {
        return false;
    }
    ...
    const int extended_samples_per_second = sbr_in_mimetype_ ? std::min(2 * orig_sample_rate, 48000) : orig_sample_rate;
    AudioDecoderConfig audio_decoder_config(
        AudioCodec::kAAC, kSampleFormatS16, channel_layout, extended_samples_per_second, extra_data, init_encryption_scheme_);

    if (!audio_decoder_config.IsValidConfig()) {
        DVLOG(1) << "Invalid config: " << audio_decoder_config.AsHumanReadableString();
        return false;
    }

    if (!audio_decoder_config.Matches(last_audio_decoder_config_)) {
        DVLOG(1) << "Sampling frequency: " << audio_decoder_config.samples_per_second() << " SBR=" << sbr_in_mimetype_;
        DVLOG(1) << "Channel layout: " << ChannelLayoutToString(audio_decoder_config.channel_layout());
