...
bool Descriptors::HasRegistrationDescriptor(int64_t* format_identifier, std::string* additional_info) const
{
...
bool Descriptors::HasCADescriptor(int* system_id, int* pid, std::string* private_data) const
{
    DCHECK(system_id);
    DCHECK(pid);
    DCHECK(private_data);
...
bool Descriptors::HasCADescriptorCenc(int* ca_pid, int* pssh_pid, EncryptionScheme* scheme) const
{
    DCHECK(ca_pid);
    DCHECK(pssh_pid);
    int system_id;
