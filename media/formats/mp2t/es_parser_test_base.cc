...
std::vector<EsParserTestBase::Packet> EsParserTestBase::LoadPacketsFromFiles(const char* filename_template, size_t file_count)
{
    std::vector<Packet> packets;
    for (size_t i = 0; i < file_count; ++i) {
        base::FilePath file_path = GetTestDataFilePath(base::StringPrintfNonConstexpr(filename_template, i));
        base::MemoryMappedFile stream;
        EXPECT_TRUE(stream.Initialize(file_path)) << "Couldn't open stream file: " << file_path.MaybeAsASCII();
        ...
        stream_.insert(stream_.end(), stream.data(), stream.data() + stream.length());
...
bool EsParserTestBase::ProcessPesPackets(EsParser* es_parser, const std::vector<Packet>& pes_packets, bool force_timing)
{
    DCHECK(es_parser);
...
std::vector<EsParserTestBase::Packet> EsParserTestBase::GenerateFixedSizePesPacket(size_t pes_size)
{
