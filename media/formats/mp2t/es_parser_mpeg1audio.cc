...
EsParserMpeg1Audio::EsParserMpeg1Audio(const NewAudioConfigCB& new_audio_config_cb, EmitBufferCB emit_buffer_cb, MediaLog* media_log)
...
bool EsParserMpeg1Audio::ParseFromEsQueue()
{
    ...
    while (LookForMpeg1AudioFrame(&mpeg1audio_frame)) {
        ...
        base::TimeDelta frame_duration = audio_timestamp_helper_->GetFrameDuration(mpeg1audio_frame.sample_count);
        ...
        scoped_refptr<StreamParserBuffer> stream_parser_buffer
            = StreamParserBuffer::CopyFrom(mpeg1audio_frame.data, mpeg1audio_frame.size, is_key_frame, DemuxerStream::AUDIO, kMp2tAudioTrackId);
...
bool EsParserMpeg1Audio::LookForMpeg1AudioFrame(Mpeg1AudioFrame* mpeg1audio_frame)
{
    int es_size;
    const uint8_t* es;
    ...
    for (int offset = 0; offset < max_offset; offset++) {
        ...
        if (!MPEG1AudioStreamParser::ParseHeader(media_log_, &mp3_parse_error_limit_, cur_buf, &header)) {
            continue;
        }
        ...
        DVLOG(LOG_LEVEL_ES) << "MPEG1 audio syncword @ pos=" << mpeg1audio_frame->queue_offset << " frame_size=" << mpeg1audio_frame->size;
        DVLOG(LOG_LEVEL_ES) << "MPEG1 audio header: " << base::HexEncode(mpeg1audio_frame->data, MPEG1AudioStreamParser::kHeaderSize);
        return true;
    }
...
bool EsParserMpeg1Audio::UpdateAudioConfiguration(const uint8_t* mpeg1audio_header)
{
    MPEG1AudioStreamParser::Header header;
    if (!MPEG1AudioStreamParser::ParseHeader(media_log_, &mp3_parse_error_limit_, mpeg1audio_header, &header)) {
        return false;
    }
    ...
    AudioDecoderConfig audio_decoder_config(
        AudioCodec::kMP3, kSampleFormatS16, header.channel_layout, header.sample_rate, EmptyExtraData(), EncryptionScheme::kUnencrypted);
    ...
    if (!audio_decoder_config.Matches(last_audio_decoder_config_)) {
        ...
        if (audio_timestamp_helper_ && audio_timestamp_helper_->base_timestamp()) {
            ...
            audio_timestamp_helper_.reset(new AudioTimestampHelper(header.sample_rate));
...
void EsParserMpeg1Audio::SkipMpeg1AudioFrame(const Mpeg1AudioFrame& mpeg1audio_frame)
{
