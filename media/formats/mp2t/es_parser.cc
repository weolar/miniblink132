...
bool EsParser::Parse(const uint8_t* buf, int size, base::TimeDelta pts, DecodeTimestamp dts)
{
    DCHECK(buf);
    DCHECK_GE(size, 0);
    ...
    if (pts != kNoTimestamp) {
        ...
        timing_desc_list_.push_back(std::pair<int64_t, TimingDesc>(es_queue_->tail(), timing_desc));
    }

    // Add the incoming bytes to the ES queue.
    if (!es_queue_->Push(base::make_span(buf, base::checked_cast<size_t>(size)))) {
