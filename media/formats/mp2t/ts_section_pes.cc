...
TsSectionPes::TsSectionPes(std::unique_ptr<EsParser> es_parser, TimestampUnroller* timestamp_unroller)
...
bool TsSectionPes::Parse(bool payload_unit_start_indicator, base::span<const uint8_t> buf)
{
    // Ignore partial PES.
...
bool TsSectionPes::ParseInternal(const uint8_t* raw_pes, int raw_pes_size)
{
    ...
    if (!is_audio_stream_id && !is_video_stream_id && !is_private_stream_1) {
        DVLOG(LOG_LEVEL_PES) << "Dropped TsPacket for stream_id=0x" << std::hex << stream_id << std::dec;
        return true;
    }
    ...
    if (is_pts_valid) {
        int64_t pts = timestamp_unroller_->GetUnrolledTimestamp(ConvertTimestampSectionToTimestamp(pts_section));
        media_pts = base::Microseconds((1000 * pts) / 90);
    }
    if (is_dts_valid) {
        int64_t dts = timestamp_unroller_->GetUnrolledTimestamp(ConvertTimestampSectionToTimestamp(dts_section));
    ...
    DVLOG(LOG_LEVEL_PES) << "Emit a reassembled PES:"
                         << " size=" << es_size << " pts=" << media_pts.InMilliseconds() << " dts=" << media_dts.InMilliseconds()
