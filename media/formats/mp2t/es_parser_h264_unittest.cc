...
void EsParserH264Test::GetAccessUnits()
{
    ...
    while (true) {
        ...
        bool success = H264Parser::FindStartCode(&stream_[offset], stream_.size() - offset, &relative_offset, &start_code_size);
        if (!success)
            break;
...
void EsParserH264Test::InsertAUD()
{
    ...
    std::vector<EsParserTestBase::Packet> access_units_with_aud(access_units_.size());
    ...
    for (size_t k = 0; k < access_units_.size(); k++) {
        ...
        memcpy(&stream_with_aud[offset], &stream_[access_units_[k].offset], access_units_[k].size);
...
bool EsParserH264Test::Process(const std::vector<Packet>& pes_packets, bool force_timing)
{
    EsParserH264 es_parser(base::BindRepeating(&EsParserH264Test::NewVideoConfig, base::Unretained(this)),
        base::BindRepeating(&EsParserH264Test::EmitBuffer, base::Unretained(this)));
...
void EsParserH264Test::CheckAccessUnits()
{
    ...
    base::TrimWhitespaceASCII(buffer_timestamps, base::TRIM_ALL, &buffer_timestamps);
