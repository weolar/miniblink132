...
TEST(WebMCryptoHelpersTest, EmptyData)
{
    ...
    int data_offset;
    ASSERT_FALSE(WebMCreateDecryptConfig(nullptr, 0, kKeyId, sizeof(kKeyId), &decrypt_config, &data_offset));
}
...
TEST(WebMCryptoHelpersTest, ClearData)
{
    ...
    std::unique_ptr<DecryptConfig> decrypt_config;
    int data_offset;
    ASSERT_TRUE(WebMCreateDecryptConfig(kData, sizeof(kData), kKeyId, sizeof(kKeyId), &decrypt_config, &data_offset));
...
TEST(WebMCryptoHelpersTest, EncryptedButNotEnoughBytes)
{
    ...
    std::unique_ptr<DecryptConfig> decrypt_config;
    int data_offset;
    ASSERT_FALSE(WebMCreateDecryptConfig(kData, sizeof(kData), kKeyId, sizeof(kKeyId), &decrypt_config, &data_offset));
}
...
TEST(WebMCryptoHelpersTest, EncryptedNotPartitioned)
{
    ...
    EXPECT_EQ(std::string(kKeyId, kKeyId + sizeof(kKeyId)), decrypt_config->key_id());
    EXPECT_EQ(std::string(kExpectedIv, kExpectedIv + sizeof(kExpectedIv)), decrypt_config->iv());
...
TEST(WebMCryptoHelpersTest, EncryptedPartitionedEvenNumberOfPartitions)
{
    ...
    EXPECT_EQ(std::string(kKeyId, kKeyId + sizeof(kKeyId)), decrypt_config->key_id());
    EXPECT_EQ(std::string(kExpectedIv, kExpectedIv + sizeof(kExpectedIv)), decrypt_config->iv());
    EXPECT_THAT(decrypt_config->subsamples(), ElementsAre(SubsampleEntry(3, 2), SubsampleEntry(1, 0)));
