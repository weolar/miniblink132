...
const uint16_t WebMClusterParser::kOpusFrameDurationsMu[] = { 10000, 20000, 40000, 60000, 10000, 20000, 40000, 60000, 10000, 20000, 40000, 60000, 10000, 20000,
    10000, 20000, 2500, 5000, 10000, 20000, 2500, 5000, 10000, 20000, 2500, 5000, 10000, 20000, 2500, 5000, 10000, 20000 };
...
WebMClusterParser::WebMClusterParser(int64_t timecode_scale_ns, int audio_track_num, base::TimeDelta audio_default_duration, int video_track_num,
    base::TimeDelta video_default_duration, const std::set<int64_t>& ignored_tracks, const std::string& audio_encryption_key_id,
    const std::string& video_encryption_key_id, const AudioCodec audio_codec, MediaLog* media_log)
    ...
    , audio_(audio_track_num, TrackType::AUDIO, audio_default_duration, media_log)
    , video_(video_track_num, TrackType::VIDEO, video_default_duration, media_log)
...
int WebMClusterParser::Parse(const uint8_t* buf, int size)
{
    ...
    if (cluster_ended_) {
        ...
        if (cluster_start_time_ == kNoTimestamp) {
            ...
            cluster_start_time_ = base::Microseconds(cluster_timecode_ * timecode_multiplier_);
        }
...
base::TimeDelta WebMClusterParser::TryGetEncodedAudioDuration(const uint8_t* data, int size)
{
...
base::TimeDelta WebMClusterParser::ReadOpusDuration(const uint8_t* data, int size)
{
    ...
    if (size < 1) {
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs) << "Invalid zero-byte Opus packet; demuxed block duration may be "
                                                                                             "imprecise.";
        return kNoTimestamp;
    }
    ...
    case 3:
        ...
        if (size < 2) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs) << "Second byte missing from 'Code 3' Opus packet; demuxed block "
        ...
        if (frame_count == 0) {
            LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs) << "Illegal 'Code 3' Opus packet with frame count zero; demuxed "
        ...
        break;
    default:
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs) << "Unexpected Opus frame count type: " << frame_count_type << "; "
    ...
    base::TimeDelta duration = base::Microseconds(kOpusFrameDurationsMu[opusConfig] * frame_count);
    ...
    if (duration > kPacketDurationMax) {
        ...
        // things go sideways.
        LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs)
            << "Warning, demuxed Opus packet with encoded duration: " << duration.InMilliseconds() << "ms. Should be no greater than "
...
bool WebMClusterParser::OnListEnd(int id)
{
    ...
    bool result = ParseBlock(false, data.data(), data.size(), additional.data(), additional.size(), block_duration_,
...
bool WebMClusterParser::ParseBlock(bool is_simple_block, const uint8_t* buf, size_t size, const uint8_t* additional, int additional_size, int duration,
    int64_t discard_padding, bool reference_block_set)
{
    ...
    if (lacing) {
        MEDIA_LOG(ERROR, media_log_) << "Lacing " << lacing << " is not supported yet.";
        return false;
    }
    ...
    return OnBlock(is_simple_block, track_num, timecode, duration, frame_data, frame_size, additional, additional_size, discard_padding, is_keyframe);
}
...
bool WebMClusterParser::OnBinary(int id, const uint8_t* data_ptr, int size)
{
    ...
    case kWebMIdSimpleBlock:
        return ParseBlock(true, data.data(), data.size(), nullptr, 0, -1, 0, false);

    case kWebMIdBlock:
        if (block_data_) {
            MEDIA_LOG(ERROR, media_log_) << "More than 1 Block in a BlockGroup is not "
                                            "supported.";
            return false;
        }
    ...
    case kWebMIdBlockAdditional: {
        ...
        // demuxer's behavior.
        block_additional_data_ = base::HeapArray<uint8_t>::Uninit(sizeof(block_add_id) + data.size());
        auto [additional_id, additional_data] = base::span(*block_additional_data_).split_at<sizeof(block_add_id)>();
...
bool WebMClusterParser::OnBlock(bool is_simple_block, int track_num, int timecode, int block_duration, const uint8_t* data, size_t size,
    const uint8_t* additional, size_t additional_size, int64_t discard_padding, bool is_keyframe)
{
    ...
    if (last_block_timecode_.has_value() && timecode < *last_block_timecode_) {
        MEDIA_LOG(ERROR, media_log_) << "Got a block with a timecode before the previous block.";
        return false;
    }
    ...
    if (!base::CheckMul(base::CheckAdd(cluster_timecode_, timecode), timecode_multiplier_).AssignIfValid(&microseconds)) {
    ...
    if (!encryption_key_id.empty()
        && !WebMCreateDecryptConfig(
            data, size, reinterpret_cast<const uint8_t*>(encryption_key_id.data()), encryption_key_id.size(), &decrypt_config, &data_offset)) {
    ...
    auto buffer = StreamParserBuffer::CopyFrom(data + data_offset, size - data_offset, is_keyframe, buffer_type, track_num);
    if (additional_size) {
        buffer->WritableSideData().alpha_data = base::HeapArray<uint8_t>::CopiedFrom(base::span<const uint8_t>(additional, additional_size));
    }
    ...
    if (block_duration >= 0) {
        block_duration_time_delta = base::Microseconds(block_duration * timecode_multiplier_);
    }
    ...
    if (encoded_duration != kNoTimestamp) {
        ...
        if (block_duration_time_delta != kNoTimestamp) {
            ...
            const auto kWarnDurationDiff = base::Microseconds(timecode_multiplier_ * 2);
            if (duration_difference.magnitude() > kWarnDurationDiff) {
                LIMITED_MEDIA_LOG(DEBUG, media_log_, num_duration_errors_, kMaxDurationErrorLogs)
                    << "BlockDuration (" << block_duration_time_delta.InMilliseconds() << "ms) differs significantly from encoded duration ("
    ...
    if (discard_padding != 0) {
        buffer->set_discard_padding(std::make_pair(base::TimeDelta(), base::Microseconds(discard_padding / 1000)));
    }

    return track->AddBuffer(std::move(buffer));
}

WebMClusterParser::Track::Track(int track_num, TrackType track_type, base::TimeDelta default_duration, MediaLog* media_log)
...
void WebMClusterParser::Track::ExtractReadyBuffers(const DecodeTimestamp before_timestamp)
{
    DCHECK(ready_buffers_.empty());
    ...
    if (buffers_.back()->GetDecodeTimestamp() < before_timestamp) {
        ...
        DVLOG(3) << __func__ << " : " << track_num_ << " All " << ready_buffers_.size() << " are ready: before upper bound ts "
    ...
    DVLOG(3) << __func__ << " : " << track_num_ << " Only " << ready_buffers_.size() << " ready, " << buffers_.size() << " at or after upper bound ts "
             << before_timestamp.InSecondsF();
}

bool WebMClusterParser::Track::AddBuffer(scoped_refptr<StreamParserBuffer> buffer)
{
    DVLOG(2) << "AddBuffer() : " << track_num_ << " ts " << buffer->timestamp().InSecondsF() << " dur " << buffer->duration().InSecondsF() << " kf "
    ...
    if (last_added_buffer_missing_duration_) {
        ...
        DVLOG(2) << "AddBuffer() : applied derived duration to held-back buffer : "
                 << " ts " << last_added_buffer_missing_duration_->timestamp().InSecondsF() << " dur "
                 << last_added_buffer_missing_duration_->duration().InSecondsF() << " kf " << last_added_buffer_missing_duration_->is_key_frame() << " size "
                 << last_added_buffer_missing_duration_->size();
...
void WebMClusterParser::Track::ApplyDurationEstimateIfNeeded()
{
    ...
    LIMITED_MEDIA_LOG(INFO, media_log_, num_duration_estimates_, kMaxDurationEstimateLogs)
        << "Estimating WebM block duration=" << last_added_buffer_missing_duration_->duration().InMilliseconds()
        << "ms for the last (Simple)Block in the Cluster for this Track (PTS=" << last_added_buffer_missing_duration_->timestamp().InMilliseconds()
        << "ms). Use BlockGroups with BlockDurations at the end of each Cluster "
        << "to avoid estimation.";

    DVLOG(2) << __func__ << " new dur : ts " << last_added_buffer_missing_duration_->timestamp().InSecondsF() << " dur "
             << last_added_buffer_missing_duration_->duration().InSecondsF() << " kf " << last_added_buffer_missing_duration_->is_key_frame() << " size "
             << last_added_buffer_missing_duration_->size();
...
bool WebMClusterParser::Track::QueueBuffer(scoped_refptr<StreamParserBuffer> buffer)
{
    ...
    // those here.
    CHECK(buffers_.empty() || buffers_.back()->GetDecodeTimestamp() <= buffer->GetDecodeTimestamp());

    base::TimeDelta duration = buffer->duration();
    if (duration.is_negative() || duration == kNoTimestamp) {
        MEDIA_LOG(ERROR, media_log_) << "Invalid buffer duration: " << duration.InSecondsF();
        return false;
    }
    ...
    if (duration.is_positive()) {
        ...
        if (max_frame_duration_ != orig_max_duration) {
            DVLOG(3) << "Updated max duration estimate:" << orig_max_duration << " -> " << max_frame_duration_
                     << " at timestamp: " << buffer->GetDecodeTimestamp().InSecondsF();
        }
    }
...
void WebMClusterParser::UpdateReadyBuffers()
{
    ...
    if (cluster_ended_) {
        ...
        ready_buffer_upper_bound_ = std::min(audio_.GetReadyUpperBound(), video_.GetReadyUpperBound());
