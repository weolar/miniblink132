...
} kCodecTestParams[] = { { VP9PROFILE_PROFILE0, {} },
    { VP9PROFILE_PROFILE2,
...
public:
    ...
    void OnListEnd(int id)
    {
        webm_video_client_.OnListEnd(id);
    }
...
TEST_P(WebMVideoClientTest, AutodetectVp9Profile2NoDetection)
{
    ...
    parser->OnUInt(kWebMIdTransferCharacteristics, static_cast<int64_t>(VideoColorSpace::TransferID::BT709));
    OnListEnd(kWebMIdColour);

    VideoDecoderConfig config;
    EXPECT_TRUE(webm_video_client_.InitializeConfig("V_VP9", GetParam().codec_private, EncryptionScheme(), &config));
...
TEST_P(WebMVideoClientTest, AutodetectVp9Profile2HDRColorSpace)
{
    ...
    auto* parser = OnListStart(kWebMIdColour);
    parser->OnUInt(kWebMIdTransferCharacteristics, static_cast<int64_t>(VideoColorSpace::TransferID::SMPTEST2084));
    OnListEnd(kWebMIdColour);
...
TEST_P(WebMVideoClientTest, InitializeConfigVP9Profiles)
{
    ...
    VideoDecoderConfig config;
    EXPECT_TRUE(webm_video_client_.InitializeConfig(kCodecId, codec_private, EncryptionScheme(), &config));

    VideoDecoderConfig expected_config(VideoCodec::kVP9, profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(), kNoTransformation,
        kCodedSize, gfx::Rect(kCodedSize), kCodedSize, codec_private, EncryptionScheme::kUnencrypted);

    EXPECT_TRUE(config.Matches(expected_config)) << "Config (" << config.AsHumanReadableString() << ") does not match expected ("
...
TEST_F(WebMVideoClientTest, InitializeConfigAV1Profile)
{
    ...
    const std::vector<uint8_t> codec_private { 0x81, 0x20, 0x00, 0x00, 0x0a, 0x0a, 0x20, 0x00, 0x00, 0x03, 0xbf, 0x7f, 0x7b, 0xff, 0xf3, 0x04 };

    VideoDecoderConfig config;
    EXPECT_TRUE(webm_video_client_.InitializeConfig(codec_id, codec_private, EncryptionScheme(), &config));

    VideoDecoderConfig expected_config(VideoCodec::kAV1, expected_profile, VideoDecoderConfig::AlphaMode::kIsOpaque, VideoColorSpace::REC709(),
        kNoTransformation, kCodedSize, gfx::Rect(kCodedSize), kCodedSize, codec_private, EncryptionScheme::kUnencrypted);
...
INSTANTIATE_TEST_SUITE_P(All, WebMVideoClientTest, ::testing::ValuesIn(kCodecTestParams));
