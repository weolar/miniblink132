...
TEST_F(WebMTracksParserTest, AudioVideoDefaultDurationSet)
{
    ...
    EXPECT_EQ(base::Microseconds(12000), parser->GetAudioDefaultDuration(kOneMsInNs));
    ...
    EXPECT_EQ(base::Microseconds(12345), parser->GetAudioDefaultDuration(12345000));
...
TEST_F(WebMTracksParserTest, InvalidTracksCodecIdFormat)
{
    ...
    EXPECT_MEDIA_LOG(HasSubstr("Tracks CodecID element value must be an ASCII string"));
...
TEST_F(WebMTracksParserTest, InvalidTracksNameFormat)
{
    ...
    EXPECT_MEDIA_LOG(HasSubstr("Tracks Name element value must be an ASCII string"));
...
TEST_F(WebMTracksParserTest, PrecisionCapping)
{
    ...
    for (size_t i = 0; i < std::size(kCappingCases); ++i) {
        ...
        EXPECT_EQ(parser->PrecisionCappedDefaultDuration(scale_ns, duration_ns), expected_result) << i << ": " << scale_ns << ", " << duration_ns;
    }
}
