...
static void CreateClusterExpectations(int block_count, bool is_complete_cluster, MockWebMParserClient* client)
{
    ...
    for (int i = 0; i < block_count; i++) {
        EXPECT_CALL(*client, OnBinary(kWebMIdSimpleBlock, _, _)).WillOnce(Return(true));
    }
...
TEST_F(WebMParserTest, ParseListElementWithSingleCall)
{
    ...
    WebMListParser parser(kWebMIdCluster, &client_);
    EXPECT_EQ(cluster->bytes_used(), parser.Parse(cluster->data(), cluster->bytes_used()));
    EXPECT_TRUE(parser.IsParsingComplete());
}
...
TEST_F(WebMParserTest, ReservedIds)
{
    ...
    const uint8_t* kBuffers[] = { k1ByteReservedId, k2ByteReservedId, k3ByteReservedId, k4ByteReservedId };
...
TEST_F(WebMParserTest, ReservedSizes)
{
    ...
    const uint8_t k6ByteReservedSize[] = { 0xA3, 0x07, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    const uint8_t k7ByteReservedSize[] = { 0xA3, 0x03, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    const uint8_t k8ByteReservedSize[] = { 0xA3, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    const uint8_t* kBuffers[] = { k1ByteReservedSize, k2ByteReservedSize, k3ByteReservedSize, k4ByteReservedSize, k5ByteReservedSize, k6ByteReservedSize,
...
TEST_F(WebMParserTest, ZeroPaddedStrings)
{
    ...
    InSequence s;
    EXPECT_CALL(client_, OnListStart(kWebMIdEBMLHeader)).WillOnce(Return(&client_));
