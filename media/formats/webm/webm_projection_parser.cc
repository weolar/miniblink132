...
bool IsValidProjectionType(int64_t projection_type_code)
{
    ...
    return projection_type_code >= projection_type_min && projection_type_code <= projection_type_max;
}
} // namespace
...
bool WebMProjectionParser::OnUInt(int id, int64_t val)
{
    if (id != kWebMIdProjectionType) {
        MEDIA_LOG(ERROR, media_log_) << "Unexpected id in Projection: 0x" << std::hex << id;
        return false;
    }

    if (projection_type_ != INVALID_PROJECTION_TYPE) {
        MEDIA_LOG(ERROR, media_log_) << "Multiple values for id: 0x" << std::hex << id << " specified (" << projection_type_ << " and " << val << ")";
        return false;
    }

    if (!IsValidProjectionType(val)) {
        MEDIA_LOG(ERROR, media_log_) << "Unexpected value for ProjectionType: 0x" << std::hex << val;
        return false;
    }
...
bool WebMProjectionParser::OnFloat(int id, double val)
{
    ...
    if (*dst != INVALID_ANGLE) {
        MEDIA_LOG(ERROR, media_log_) << "Multiple values for id: 0x" << std::hex << id << " specified (" << *dst << " and " << val << ")";
        return false;
    }

    if (!is_valid) {
        MEDIA_LOG(ERROR, media_log_) << "Value not within valid range. id: 0x" << std::hex << id << " val:" << val;
        return false;
    }
...
{
    if (projection_type_ == INVALID_PROJECTION_TYPE) {
        MEDIA_LOG(ERROR, media_log_) << "Projection element is incomplete; ProjectionType required.";
        return false;
    }

    if (pose_yaw_ == INVALID_ANGLE) {
        MEDIA_LOG(ERROR, media_log_) << "Projection element is incomplete; ProjectionPoseYaw required.";
        return false;
    }

    if (pose_pitch_ == INVALID_ANGLE) {
        MEDIA_LOG(ERROR, media_log_) << "Projection element is incomplete; ProjectionPosePitch required.";
        return false;
    }

    if (pose_roll_ == INVALID_ANGLE) {
        MEDIA_LOG(ERROR, media_log_) << "Projection element is incomplete; ProjectionPoseRoll required.";
        return false;
    }
