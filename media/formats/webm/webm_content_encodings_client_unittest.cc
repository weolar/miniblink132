...
{
    return CONTAINS_STRING(arg, "ContentEncodingType is encryption but ContentEncryption is missing.");
}

MATCHER_P(UnexpectedContentEncAlgo, algo, "")
{
    return CONTAINS_STRING(arg, "Unexpected ContentEncAlgo " + base::NumberToString(algo) + ".");
}
...
TEST_F(WebMContentEncodingsClientTest, SingleContentEncoding)
{
    ...
    EXPECT_EQ(ContentEncoding::kScopeAllFrameContents, content_encodings[0]->scope());
    EXPECT_EQ(ContentEncoding::kTypeEncryption, content_encodings[0]->type());
    EXPECT_EQ(ContentEncoding::kEncAlgoAes, content_encodings[0]->encryption_algo());
...
TEST_F(WebMContentEncodingsClientTest, MultipleContentEncoding)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(ContentEncoding::kScopeAllFrameContents | ContentEncoding::kScopeTrackPrivateData, content_encodings[i]->scope());
        EXPECT_EQ(ContentEncoding::kTypeEncryption, content_encodings[i]->type());
        EXPECT_EQ(!i ? ContentEncoding::kEncAlgoAes : ContentEncoding::kEncAlgoDes, content_encodings[i]->encryption_algo());
...
TEST_F(WebMContentEncodingsClientTest, DefaultValues)
{
    ...
    EXPECT_EQ(ContentEncoding::kScopeAllFrameContents, content_encodings[0]->scope());
    EXPECT_EQ(ContentEncoding::kTypeEncryption, content_encodings[0]->type());
    EXPECT_EQ(ContentEncoding::kEncAlgoNotEncrypted, content_encodings[0]->encryption_algo());
