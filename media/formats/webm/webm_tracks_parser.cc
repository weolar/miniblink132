...
base::TimeDelta WebMTracksParser::PrecisionCappedDefaultDuration(const int64_t timecode_scale_in_ns, const int64_t duration_in_ns) const
{
...
base::TimeDelta WebMTracksParser::GetAudioDefaultDuration(const int64_t timecode_scale_in_ns) const
{
    return PrecisionCappedDefaultDuration(timecode_scale_in_ns, audio_default_duration_);
}

base::TimeDelta WebMTracksParser::GetVideoDefaultDuration(const int64_t timecode_scale_in_ns) const
{
    return PrecisionCappedDefaultDuration(timecode_scale_in_ns, video_default_duration_);
}
...
WebMParserClient* WebMTracksParser::OnListStart(int id)
{
    if (id == kWebMIdContentEncodings) {
        ...
        track_content_encodings_client_ = std::make_unique<WebMContentEncodingsClient>(media_log_);
...
bool WebMTracksParser::OnListEnd(int id)
{
    ...
    if (id == kWebMIdTrackEntry) {
        ...
        if (track_type_ != kWebMTrackTypeAudio && track_type_ != kWebMTrackTypeVideo && track_type_ != kWebMTrackTypeSubtitlesOrCaptions
        ...
        EncryptionScheme encryption_scheme = encryption_key_id.empty() ? EncryptionScheme::kUnencrypted : EncryptionScheme::kCenc;
        ...
        if (track_type_ == kWebMTrackTypeAudio) {
            ...
            if (audio_track_num_ == -1) {
                ...
                if (!audio_client_.InitializeConfig(codec_id_, codec_private_, seek_preroll_, codec_delay_, encryption_scheme, &audio_decoder_config_)) {
                    return false;
                }
                media_tracks_->AddAudioTrack(audio_decoder_config_, true, static_cast<StreamParser::TrackId>(track_num_), MediaTrack::Kind("main"),
                    MediaTrack::Label(track_name_), MediaTrack::Language(track_language_));
            } else {
            ...
            if (video_track_num_ == -1) {
                ...
                if (!video_client_.InitializeConfig(codec_id_, codec_private_, encryption_scheme, &video_decoder_config_)) {
                    return false;
                }
                media_tracks_->AddVideoTrack(video_decoder_config_, true, static_cast<StreamParser::TrackId>(track_num_), MediaTrack::Kind("main"),
                    MediaTrack::Label(track_name_), MediaTrack::Language(track_language_));
            } else {
...
bool WebMTracksParser::OnUInt(int id, int64_t val)
{
    ...
    if (*dst != -1) {
        MEDIA_LOG(ERROR, media_log_) << "Multiple values for id " << std::hex << id << " specified";
        return false;
    }
...
bool WebMTracksParser::OnBinary(int id, const uint8_t* data, int size)
{
    if (id == kWebMIdCodecPrivate) {
        if (!codec_private_.empty()) {
            MEDIA_LOG(ERROR, media_log_) << "Multiple CodecPrivate fields in a track.";
            return false;
        }
...
bool WebMTracksParser::OnString(int id, const std::string& str)
{
    if (id == kWebMIdCodecID) {
        ...
        if (!base::IsStringASCII(str)) {
            MEDIA_LOG(ERROR, media_log_) << "Tracks CodecID element value must be an ASCII string";
            return false;
        }
    ...
    if (id == kWebMIdName) {
        ...
        // allow also 0x01-0x1F.
        if (!base::IsStringASCII(str)) {
            MEDIA_LOG(ERROR, media_log_) << "Tracks Name element value must be an ASCII string";
            return false;
        }
        track_name_ = str;
        return true;
