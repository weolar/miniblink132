...
static const ElementIdInfo kClusterIds[] = {
    ...
    { SKIP_LIST, kWebMIdSilentTracks },
    { SKIP_UINT, kWebMIdPosition },
...
static const ElementIdInfo kBlockGroupIds[] = {
    ...
    { SKIP_UINT, kWebMIdReferencePriority },
    { BINARY, kWebMIdReferenceBlock },
    { SKIP_BINARY, kWebMIdCodecState },
    { BINARY, kWebMIdDiscardPadding },
    { SKIP_LIST, kWebMIdSlices },
};
...
static const ElementIdInfo kVideoIds[] = {
    ...
    { SKIP_BINARY, kWebMIdColorSpace },
    { SKIP_FLOAT, kWebMIdFrameRate },
    { LIST, kWebMIdColour },
...
static const ElementIdInfo kProjectionIds[] {
    ...
    { SKIP_BINARY, kWebMIdProjectionPrivate },
...
static const ElementIdInfo kContentEncodingIds[] = {
    ...
    { SKIP_LIST, kWebMIdContentCompression },
...
static const ElementIdInfo kCueTrackPositionsIds[] = {
    ...
    { SKIP_UINT, kWebMIdCueCodecState },
...
static const ElementIdInfo kAttachedFileIds[] = {
    { SKIP_STRING, kWebMIdFileDescription },
    { SKIP_STRING, kWebMIdFileName },
    { SKIP_STRING, kWebMIdFileMimeType },
    { SKIP_BINARY, kWebMIdFileData },
...
static const ElementIdInfo kTargetsIds[] = {
    ...
    { SKIP_UINT, kWebMIdTagEditionUID },
    { SKIP_UINT, kWebMIdTagChapterUID },
    { SKIP_UINT, kWebMIdTagAttachmentUID },
};
...
static int ParseWebMElementHeaderField(const uint8_t* buf, int size, int max_bytes, bool mask_first_byte, int64_t* num)
{
    DCHECK(buf);
    DCHECK(num);
...
int WebMParseElementHeader(const uint8_t* buf, int size, int* id, int64_t* element_size)
{
    DCHECK(buf);
    DCHECK_GE(size, 0);
    DCHECK(id);
    ...
    *id = static_cast<int>(tmp);
    ...
    int num_size_bytes = ParseWebMElementHeaderField(buf + num_id_bytes, size - num_id_bytes, 8, true, &tmp);
    ...
    *element_size = tmp;
    DVLOG(3) << "WebMParseElementHeader() : id " << std::hex << *id << std::dec << " size " << *element_size;
    return num_id_bytes + num_size_bytes;
}

// Finds ElementType for a specific ID.
static ElementType FindIdType(int id, const ElementIdInfo* id_info, int id_info_count)
{
...
static int ParseUInt(const uint8_t* buf, int size, int id, WebMParserClient* client)
{
    if ((size <= 0) || (size > 8)) {
        return -1;
    }
...
static int ParseFloat(const uint8_t* buf, int size, int id, WebMParserClient* client)
{
    if ((size != 4) && (size != 8)) {
        return -1;
    }
...
static int ParseBinary(const uint8_t* buf, int size, int id, WebMParserClient* client)
{
    return client->OnBinary(id, buf, size) ? size : -1;
}

static int ParseString(const uint8_t* buf, int size, int id, WebMParserClient* client)
{
...
static int ParseNonListElement(ElementType type, int id, int64_t element_size, const uint8_t* buf, int size, WebMParserClient* client)
{
...
int WebMListParser::Parse(const uint8_t* buf, int size)
{
    ...
    while (cur_size > 0 && state_ != PARSE_ERROR && state_ != DONE_PARSING_LIST) {
        ...
        int result = WebMParseElementHeader(cur, cur_size, &element_id, &element_size);
        ...
        case INSIDE_LIST: {
            ...
            result = ParseListElement(header_size, element_id, element_size, element_data, element_data_size);
...
int WebMListParser::ParseListElement(int header_size, int id, int64_t element_size, const uint8_t* data, int size)
{
    ...
    int bytes_parsed = ParseNonListElement(id_type, id, element_size, data, size, list_state.client_);
