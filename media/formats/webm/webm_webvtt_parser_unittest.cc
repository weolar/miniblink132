...
static Cue EncodeCue(const std::string& id, const std::string& settings, const std::string& content)
{
...
static void DecodeCue(const Cue& cue, std::string* id, std::string* settings, std::string* content)
{
    WebMWebVTTParser::Parse(&cue[0], static_cast<int>(cue.size()), id, settings, content);
}
...
TEST_F(WebMWebVTTParserTest, Settings)
{
    ...
    const char* const settings_str[kSettingsCount] = { "vertical:lr", "line:50%", "position:42%", "vertical:rl line:42% position:100%" };
...
TEST_F(WebMWebVTTParserTest, Content)
{
    ...
    const char* const content_str[kContentCount] = { "Subtitle", "Another Subtitle", "Yet Another Subtitle", "Another Subtitle\nSplit Across Two Lines" };
