...
media::VideoCodecProfile GetVP9CodecProfile(const std::vector<uint8_t>& data, bool is_probably_10bit)
{
    ...
    if (data.size() < 3 || data[0] != kVP9ProfileFieldId || data[1] != kVP9ProfileFieldLength || data[2] > 3) {
        return is_probably_10bit ? VP9PROFILE_PROFILE2 : VP9PROFILE_PROFILE0;
    }

    return static_cast<VideoCodecProfile>(static_cast<size_t>(VP9PROFILE_PROFILE0) + data[2]);
}
...
bool IsValidStereoMode(int64_t stereo_mode_code)
{
    ...
    return stereo_mode_code >= stereo_mode_min && stereo_mode_code <= stereo_mode_max;
}
...
bool WebMVideoClient::InitializeConfig(
    const std::string& codec_id, const std::vector<uint8_t>& codec_private, EncryptionScheme encryption_scheme, VideoDecoderConfig* config)
{
    DCHECK(config);
    ...
    if (codec_id == "V_VP8") {
        ...
        profile = GetVP9CodecProfile(codec_private, color_space.GuessGfxColorSpace().IsHDR() || config->hdr_metadata().has_value() || !is_8bit);
#if BUILDFLAG(ENABLE_AV1_DECODER)
    ...
    gfx::Rect visible_rect(crop_top_, crop_left_, pixel_width_ - (crop_left_ + crop_right_), pixel_height_ - (crop_top_ + crop_bottom_));
    ...
    if (display_unit_ == 0 || (video_codec == VideoCodec::kAV1 && display_unit_ == 4)) {
        if (display_width_ <= 0)
        ...
        MEDIA_LOG(ERROR, media_log_) << "Unsupported display unit type " << display_unit_;
        return false;
    }
    gfx::Size natural_size = gfx::Size(display_width_, display_height_);

    config->Initialize(video_codec, profile, alpha_mode_ == 1 ? VideoDecoderConfig::AlphaMode::kHasAlpha : VideoDecoderConfig::AlphaMode::kIsOpaque,
        color_space, kNoTransformation, coded_size, visible_rect, natural_size, codec_private, encryption_scheme);
...
WebMParserClient* WebMVideoClient::OnListStart(int id)
{
    ...
    if (id == kWebMIdProjection) {
        if (projection_parsed_ == true) {
            MEDIA_LOG(ERROR, media_log_) << "Unexpected multiple Projection elements.";
            return NULL;
        }
...
bool WebMVideoClient::OnUInt(int id, int64_t val)
{
    ...
    if (*dst != -1) {
        MEDIA_LOG(ERROR, media_log_) << "Multiple values for id " << std::hex << id << " specified (" << *dst << " and " << val << ")";
        return false;
    }

    if (id == kWebMIdStereoMode && !IsValidStereoMode(val)) {
        MEDIA_LOG(ERROR, media_log_) << "Unexpected value for StereoMode: 0x" << std::hex << val;
        return false;
    }
