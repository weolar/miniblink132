...
bool WebMAudioClient::InitializeConfig(const std::string& codec_id, const std::vector<uint8_t>& codec_private, int64_t seek_preroll, int64_t codec_delay,
    EncryptionScheme encryption_scheme, AudioDecoderConfig* config)
{
    DCHECK(config);
    ...
    ChannelLayout channel_layout = channels_ > 8 ? CHANNEL_LAYOUT_DISCRETE : GuessChannelLayout(channels_);
    ...
    config->Initialize(audio_codec, sample_format, channel_layout, samples_per_second, codec_private, encryption_scheme,
        base::Microseconds((seek_preroll != -1 ? seek_preroll : 0) / 1000), codec_delay_in_frames);
...
bool WebMAudioClient::OnUInt(int id, int64_t val)
{
    if (id == kWebMIdChannels) {
        if (channels_ != -1) {
            MEDIA_LOG(ERROR, media_log_) << "Multiple values for id " << std::hex << id << " specified. (" << channels_ << " and " << val << ")";
            return false;
        }
