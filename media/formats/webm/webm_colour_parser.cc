...
bool WebMColourParser::OnUInt(int id, int64_t val)
{
    ...
    DCHECK(dst);
    if (*dst != -1) {
        LOG(ERROR) << "Multiple values for id " << std::hex << id << " specified (" << *dst << " and " << val << ")";
        return false;
    }
...
WebMColorMetadata WebMColourParser::GetWebMColorMetadata() const
{
    ...
    if (range_ >= static_cast<int64_t>(mkvmuxer::Colour::kUnspecifiedCr) && range_ <= static_cast<int64_t>(mkvmuxer::Colour::kMcTcDefined)) {
        switch (range_) {
    ...
    color_metadata.color_space = VideoColorSpace(primaries_, transfer_characteristics_, matrix_coefficients_, range_id);
    ...
    if (color_volume_metadata_parsed_) {
        ...
        color_metadata.hdr_metadata->smpte_st_2086 = color_volume_metadata_parser_.GetColorVolumeMetadata();
    }
