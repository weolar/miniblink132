...
MATCHER_P(OpusPacketDurationTooHigh, actual_duration_ms, "")
{
    return CONTAINS_STRING(arg,
        "Warning, demuxed Opus packet with encoded duration: " + base::NumberToString(static_cast<int64_t>(actual_duration_ms))
            + "ms. Should be no greater than 120ms.");
}

MATCHER_P2(WebMBlockDurationMismatchesOpusDuration, block_duration_ms, opus_duration_ms, "")
{
    return CONTAINS_STRING(arg,
        "BlockDuration (" + base::NumberToString(static_cast<int64_t>(block_duration_ms)) + "ms) differs significantly from encoded duration ("
...
static_assert(static_cast<int>(kTestAudioFrameDefaultDurationInMs) != static_cast<int>(WebMClusterParser::kDefaultAudioBufferDurationInMs),
    "test default is the same as estimation fallback audio duration");
static_assert(static_cast<int>(kTestVideoFrameDefaultDurationInMs) != static_cast<int>(WebMClusterParser::kDefaultVideoBufferDurationInMs),
...
std::unique_ptr<Cluster> CreateCluster(int timecode, const BlockInfo* block_info, int block_count)
{
    ClusterBuilder cb;
    ...
    for (int i = 0; i < block_count; i++) {
        ...
        if (block_info[i].use_simple_block) {
            ...
            cb.AddSimpleBlock(block_info[i].track_num, block_info[i].timestamp, block_info[i].is_key_frame ? 0x80 : 0x00, data, data_length);
            continue;
        }

        if (block_info[i].duration < 0) {
            cb.AddBlockGroupWithoutBlockDuration(block_info[i].track_num, block_info[i].timestamp, 0, block_info[i].is_key_frame, data, data_length);
            continue;
        }

        cb.AddBlockGroup(block_info[i].track_num, block_info[i].timestamp, block_info[i].duration, 0, block_info[i].is_key_frame, data, data_length);
    }
...
bool VerifyBuffers(const StreamParser::BufferQueueMap& buffer_queue_map, const BlockInfo* block_info, int block_count)
{
    int buffer_count = 0;
    for (const auto& [track_id, buffer_queue] : buffer_queue_map)
        buffer_count += buffer_queue.size();
    if (block_count != buffer_count) {
        DVLOG(1) << __func__ << " : block_count (" << block_count << ") mismatches buffer_count (" << buffer_count << ")";
        return false;
    }
    ...
    for (int i = 0; i < block_count; i++) {
        ...
        if (*offset >= buffers->size()) {
            DVLOG(1) << __func__ << " : Too few buffers (" << buffers->size() << ") for track_num (" << block_info[i].track_num << "), expected at least "
                     << *offset + 1 << " buffers";
            return false;
        }
        ...
        EXPECT_EQ(std::abs(block_info[i].duration), buffer->duration().InMillisecondsF());
...
bool VerifyBuffers(const std::unique_ptr<WebMClusterParser>& parser, const BlockInfo* block_info, int block_count)
{
...
void VerifyEncryptedBuffer(scoped_refptr<StreamParserBuffer> buffer)
{
    ...
    EXPECT_EQ(static_cast<unsigned long>(DecryptConfig::kDecryptionKeySize), buffer->decrypt_config()->iv().length());
}

void AppendToEnd(const StreamParser::BufferQueue& src, StreamParser::BufferQueue* dest)
{
    for (StreamParser::BufferQueue::const_iterator itr = src.begin(); itr != src.end(); ++itr) {
        dest->push_back(*itr);
    }
}
...
protected:
    ...
    {
        base::TimeDelta default_audio_duration = base::Milliseconds(kTestAudioFrameDefaultDurationInMs);
        base::TimeDelta default_video_duration = base::Milliseconds(kTestVideoFrameDefaultDurationInMs);
        ...
        parser_.reset(CreateParserWithDefaultDurations(default_audio_duration, default_video_duration));
    }

    // Helper that hard-codes some non-varying constructor parameters.
    WebMClusterParser* CreateParserHelper(base::TimeDelta audio_default_duration, base::TimeDelta video_default_duration,
        const std::set<int64_t>& ignored_tracks, const std::string& audio_encryption_key_id, const std::string& video_encryption_key_id,
        const AudioCodec audio_codec)
    {
        return new WebMClusterParser(kTimecodeScale, kAudioTrackNum, audio_default_duration, kVideoTrackNum, video_default_duration, ignored_tracks,
            audio_encryption_key_id, video_encryption_key_id, audio_codec, &media_log_);
    }

    // Create a default version of the parser for test.
    WebMClusterParser* CreateDefaultParser()
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, std::set<int64_t>(), std::string(), std::string(), AudioCodec::kUnknown);
    }

    // Create a parser for test with custom audio and video default durations.
    WebMClusterParser* CreateParserWithDefaultDurations(base::TimeDelta audio_default_duration, base::TimeDelta video_default_duration)
    {
        return CreateParserHelper(audio_default_duration, video_default_duration, std::set<int64_t>(), std::string(), std::string(), AudioCodec::kUnknown);
    }

    // Create a parser for test with custom ignored tracks.
    WebMClusterParser* CreateParserWithIgnoredTracks(std::set<int64_t>& ignored_tracks)
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, ignored_tracks, std::string(), std::string(), AudioCodec::kUnknown);
    }
    ...
    WebMClusterParser* CreateParserWithKeyIdsAndAudioCodec(
        const std::string& audio_encryption_key_id, const std::string& video_encryption_key_id, const AudioCodec audio_codec)
    {
        return CreateParserHelper(kNoTimestamp, kNoTimestamp, std::set<int64_t>(), audio_encryption_key_id, video_encryption_key_id, audio_codec);
    }
...
TEST_F(WebMClusterParserTest, HeldBackBufferHoldsBackAllTracks)
{
    ...
    ASSERT_GE(default_audio_duration, base::TimeDelta());
    ASSERT_NE(kNoTimestamp, default_audio_duration);
    parser_.reset(CreateParserWithDefaultDurations(default_audio_duration, kNoTimestamp));
    ...
    const BlockInfo kBlockInfo[] = {
        ...
        { kAudioTrackNum, 23, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kVideoTrackNum, 33, 33, true, NULL, 0, false },
        { kAudioTrackNum, 36, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kVideoTrackNum, 66, kExpectedVideoEstimationInMs, true, NULL, 0, false },
        { kAudioTrackNum, 70, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kAudioTrackNum, 83, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
    };
    ...
    for (int i = 0; i < block_count; ++i) {
        ...
        std::unique_ptr<Cluster> cluster(CreateCluster(0, kBlockInfo, blocks_in_cluster));
        ...
        if (parse_full_cluster) {
            EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(kExpectedVideoEstimationInMs));
        }

        int result = parser_->Parse(cluster->data(), parse_full_cluster ? cluster->bytes_used() : cluster->bytes_used() - 1);
        if (parse_full_cluster) {
            DVLOG(1) << "Verifying parse result of full cluster of " << blocks_in_cluster << " blocks";
            EXPECT_EQ(cluster->bytes_used(), result);
        } else {
            DVLOG(1) << "Verifying parse result of cluster of " << blocks_in_cluster << " blocks with last block incomplete";
            EXPECT_GT(cluster->bytes_used(), result);
            EXPECT_LT(0, result);
        }

        EXPECT_TRUE(VerifyBuffers(parser_, kBlockInfo, kExpectedBuffersOnPartialCluster[i])) << i;
    }
}
...
TEST_F(WebMClusterParserTest, Reset)
{
    ...
    std::unique_ptr<Cluster> cluster(CreateCluster(0, kDefaultBlockInfo, block_count));
...
TEST_F(WebMClusterParserTest, IgnoredTracks)
{
    ...
    std::unique_ptr<Cluster> cluster(CreateCluster(0, kInputBlockInfo, input_block_count));
...
{
    std::unique_ptr<Cluster> cluster(CreateEncryptedCluster(sizeof(kEncryptedFrame)));

    parser_.reset(CreateParserWithKeyIdsAndAudioCodec(std::string(), "video_key_id", AudioCodec::kUnknown));

    // The encrypted cluster contains just one block, video.
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(WebMClusterParser::kDefaultVideoBufferDurationInMs));
...
{
    std::unique_ptr<Cluster> cluster(CreateEncryptedCluster(sizeof(kEncryptedFrame) - 1));
...
TEST_F(WebMClusterParserTest, ParseWithDefaultDurationsSimpleBlocks)
{
    ...
    const BlockInfo kBlockInfo[] = {
        { kAudioTrackNum, 0, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kAudioTrackNum, 23, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kVideoTrackNum, 33, kTestVideoFrameDefaultDurationInMs, true, NULL, 0, false },
        { kAudioTrackNum, 46, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kVideoTrackNum, 67, kTestVideoFrameDefaultDurationInMs, true, NULL, 0, false },
        { kAudioTrackNum, 69, kTestAudioFrameDefaultDurationInMs, true, NULL, 0, false },
        { kVideoTrackNum, 100, kTestVideoFrameDefaultDurationInMs, true, NULL, 0, false },
    };
...
TEST_F(WebMClusterParserTest, ParseWithoutAnyDurationsSimpleBlocks)
{
    ...
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(kExpectedAudioEstimationInMs));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(kExpectedVideoEstimationInMs));
    ...
    std::unique_ptr<Cluster> cluster2(CreateCluster(0, kBlockInfo2, block_count2));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(kExpectedAudioEstimationInMs));
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimated(kExpectedVideoEstimationInMs));
...
TEST_F(WebMClusterParserTest, ParseWithoutAnyDurationsBlockGroups)
{
    ...
    const BlockInfo kBlockInfo1[] = {
        ...
        { kAudioTrackNum, 68, -kExpectedAudioEstimationInMs, false, NULL, 0, false },
        { kVideoTrackNum, 100, -kExpectedVideoEstimationInMs, false, NULL, 0, false },
    };
    ...
    // each track.
    const BlockInfo kBlockInfo2[] = {
        { kAudioTrackNum, 200, -kExpectedAudioEstimationInMs, false, NULL, 0, false },
        { kVideoTrackNum, 201, -kExpectedVideoEstimationInMs, false, NULL, 0, false },
    };
...
TEST_F(WebMClusterParserTest, ParseWithDefaultDurationsBlockGroupsWithoutDurations)
{
    InSequence s;
    ResetParserToHaveDefaultDurations();
    ...
    const BlockInfo kBlockInfo[] = {
        { kAudioTrackNum, 0, -kTestAudioFrameDefaultDurationInMs, false, NULL, 0, false },
        { kAudioTrackNum, 23, -kTestAudioFrameDefaultDurationInMs, false, NULL, 0, false },
        { kVideoTrackNum, 33, -kTestVideoFrameDefaultDurationInMs, false, NULL, 0, false },
        { kAudioTrackNum, 46, -kTestAudioFrameDefaultDurationInMs, false, NULL, 0, false },
        { kVideoTrackNum, 67, -kTestVideoFrameDefaultDurationInMs, false, NULL, 0, false },
        { kAudioTrackNum, 69, -kTestAudioFrameDefaultDurationInMs, false, NULL, 0, false },
        { kVideoTrackNum, 100, -kTestVideoFrameDefaultDurationInMs, false, NULL, 0, false },
    };
...
TEST_F(WebMClusterParserTest, ParseClusterWithNegativeBlockTimestampsAndWithBlockDurations)
{
    InSequence s;
...
TEST_F(WebMClusterParserTest, ParseClusterWithNegativeBlockTimestampsAndWithoutDurations)
{
    InSequence s;
...
TEST_F(WebMClusterParserTest, ParseDegenerateClusterYieldsHardcodedEstimatedDurations)
{
    const BlockInfo kBlockInfo[] = {
        { kAudioTrackNum, 0, WebMClusterParser::kDefaultAudioBufferDurationInMs, true },
        { kVideoTrackNum, 0, WebMClusterParser::kDefaultVideoBufferDurationInMs, true },
    };
...
TEST_F(WebMClusterParserTest, ParseDegenerateClusterWithDefaultDurationsYieldsDefaultDurations)
{
    ResetParserToHaveDefaultDurations();
...
TEST_F(WebMClusterParserTest, ReadOpusDurationsSimpleBlockAtEndOfCluster)
{
    ...
    for (const auto& packet_ptr : BuildAllOpusPackets()) {
        ...
        parser_.reset(CreateParserWithKeyIdsAndAudioCodec(std::string(), std::string(), AudioCodec::kOpus));

        const BlockInfo kBlockInfo[] = { { kAudioTrackNum, 0, packet_ptr->duration_ms(),
...
TEST_F(WebMClusterParserTest, PreferOpusDurationsOverBlockDurations)
{
    ...
    for (const auto& packet_ptr : BuildAllOpusPackets()) {
        ...
        EXPECT_MEDIA_LOG(WebMBlockDurationMismatchesOpusDuration(block_duration_ms, packet_ptr->duration_ms()));
        ...
        std::unique_ptr<Cluster> cluster(CreateCluster(0, block_infos, block_count));
        int result = parser_->Parse(cluster->data(), cluster->bytes_used());
        EXPECT_EQ(cluster->bytes_used(), result);
...
TEST_F(WebMClusterParserTest, DontReadEncodedDurationWhenEncrypted)
{
    ...
    parser_.reset(CreateParserWithKeyIdsAndAudioCodec(audio_encryption_id, std::string(), AudioCodec::kOpus));

    // Single Block with BlockDuration and encrypted data.
    const BlockInfo kBlockInfo[] = { { kAudioTrackNum, 0, kTestAudioFrameDefaultDurationInMs,
        false, // Not a SimpleBlock
