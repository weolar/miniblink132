...
protected:
    void ParseWebMFile(const std::string& filename, const StreamParser::InitParameters& expected_params)
    {
        scoped_refptr<DecoderBuffer> buffer = ReadTestDataFile(filename);
        parser_ = std::make_unique<WebMStreamParser>();
        Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb
            = base::BindRepeating(&WebMStreamParserTest::OnEncryptedMediaInitData, base::Unretained(this));
        ...
        EXPECT_CALL(*this, NewBuffersCB(_)).Times(testing::AnyNumber()).WillRepeatedly(testing::Return(true));
        parser_->Init(base::BindOnce(&WebMStreamParserTest::InitF, base::Unretained(this), expected_params),
            base::BindRepeating(&WebMStreamParserTest::NewConfigCB, base::Unretained(this)),
            base::BindRepeating(&WebMStreamParserTest::NewBuffersCB, base::Unretained(this)), encrypted_media_init_data_cb,
            base::BindRepeating(&WebMStreamParserTest::NewMediaSegmentCB, base::Unretained(this)),
            base::BindRepeating(&WebMStreamParserTest::EndMediaSegmentCB, base::Unretained(this)), &media_log_);
...
{
    EXPECT_MEDIA_LOG(WebMSimpleBlockDurationEstimatedAny()).Times(testing::AnyNumber());
...
TEST_F(WebMStreamParserTest, ColourElement)
{
    ...
    const VideoDecoderConfig& video_config = media_tracks_->getVideoConfig(video_track->stream_id());

    VideoColorSpace expected_color_space(
        VideoColorSpace::PrimaryID::SMPTEST428_1, VideoColorSpace::TransferID::LOG, VideoColorSpace::MatrixID::RGB, gfx::ColorSpace::RangeID::FULL);
    ...
    const gfx::HdrMetadataSmpteSt2086& mmdata = hdr_metadata->smpte_st_2086.value();
...
TEST_F(WebMStreamParserTest, ColourElementWithUnspecifiedRange)
{
    ...
    VideoColorSpace expected_color_space(
        VideoColorSpace::PrimaryID::SMPTEST428_1, VideoColorSpace::TransferID::LOG, VideoColorSpace::MatrixID::RGB, gfx::ColorSpace::RangeID::INVALID);
    EXPECT_EQ(video_config.color_space_info(), expected_color_space);
}
