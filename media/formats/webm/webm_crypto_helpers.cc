...
bool ExtractSubsamples(base::span<const uint8_t> buf, size_t frame_data_size, size_t num_partitions, std::vector<SubsampleEntry>* subsample_entries)
{
    ...
    for (size_t i = 0; i <= num_partitions; ++i) {
        ...
        if (i < num_partitions) {
            ...
            partition_offset = base::U32FromBigEndian(buf.subspan(kWebMEncryptedFramePartitionOffsetSize * i).first<kWebMEncryptedFramePartitionOffsetSize>());
        } else {
        ...
        if (partition_offset < prev_partition_offset) {
            DVLOG(1) << "Partition should not be decreasing " << prev_partition_offset << " " << partition_offset;
            return false;
        }
...
bool WebMCreateDecryptConfig(
    const uint8_t* data_ptr, int data_size, const uint8_t* key_id_ptr, int key_id_size, std::unique_ptr<DecryptConfig>* decrypt_config, int* data_offset)
{
    // TODO(crbug.com/40284755):: The function should receive a span, not a
    // pointer/length pair.
    auto data = UNSAFE_TODO(base::span(data_ptr, base::checked_cast<size_t>(data_size)));
    // TODO(crbug.com/40284755):: The function should receive a span, not a
    // pointer/length pair.
    auto key_id = UNSAFE_TODO(base::span(key_id_ptr, base::checked_cast<size_t>(key_id_size)));
    ...
    if (signal_byte & kWebMFlagEncryptedFrame) {
        ...
        if (signal_byte & kWebMFlagEncryptedFramePartitioned) {
            ...
            static_assert(sizeof(num_partitions) == kWebMEncryptedFrameNumPartitionsSize);
            if (!reader.ReadU8BigEndian(num_partitions)) {
                DVLOG(1) << "Got a partitioned encrypted block with not enough data " << data.size();
                return false;
            }

            base::span<const uint8_t> partition_data;
            if (!reader.ReadInto(size_t { kWebMEncryptedFramePartitionOffsetSize } * num_partitions, partition_data) || reader.remaining() == 0u) {
                DVLOG(1) << "Got a partitioned encrypted block with " << num_partitions << " partitions but not enough data " << data.size();
                return false;
            }
            if (!ExtractSubsamples(partition_data, reader.remaining(), num_partitions, &subsample_entries)) {
                return false;
            }
        }
    }
    ...
    if (counter_block.empty()) {
        ...
        *decrypt_config = DecryptConfig::CreateCencConfig(std::string(key_id.begin(), key_id.end()), counter_block, subsample_entries);
    }
