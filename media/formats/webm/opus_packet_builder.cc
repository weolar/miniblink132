...
std::vector<std::unique_ptr<OpusPacket>> BuildAllOpusPackets()
{
    ...
    for (int frame_count = kMinOpusPacketFrameCount; frame_count <= kMaxOpusPacketFrameCount; frame_count++) {
        for (int opus_config_num = 0; opus_config_num < kNumPossibleOpusConfigs; opus_config_num++) {
            bool is_VBR = false;
            opus_packets.push_back(std::make_unique<OpusPacket>(opus_config_num, frame_count, is_VBR));
