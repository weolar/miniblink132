...
template <typename T> struct TestAudioManagerFactory {
    static std::unique_ptr<AudioManager> Create(AudioLogFactory* audio_log_factory)
    {
        return std::make_unique<T>(std::make_unique<TestAudioThread>(), audio_log_factory);
    }
};

#if defined(USE_PULSEAUDIO)
template <> struct TestAudioManagerFactory<AudioManagerPulse> {
    static std::unique_ptr<AudioManager> Create(AudioLogFactory* audio_log_factory)
    {
        ...
        if (!pulse::InitPulse(&pa_mainloop, &pa_context))
            return nullptr;
        return std::make_unique<AudioManagerPulse>(std::make_unique<TestAudioThread>(), audio_log_factory, pa_mainloop, pa_context);
    }
};
#endif // defined(USE_PULSEAUDIO)

template <> struct TestAudioManagerFactory<std::nullptr_t> {
    static std::unique_ptr<AudioManager> Create(AudioLogFactory* audio_log_factory)
    {
...
void CheckDescriptionLabels(const AudioDeviceDescriptions& descriptions, const std::string& real_default_id, const std::string& real_communications_id)
{
    ...
    for (const auto& description : descriptions) {
        if (AudioDeviceDescription::IsDefaultDevice(description.unique_id)) {
            EXPECT_TRUE(base::EndsWith(description.device_name, real_default_label, base::CompareCase::SENSITIVE));
        } else if (description.unique_id == AudioDeviceDescription::kCommunicationsDeviceId) {
            EXPECT_TRUE(base::EndsWith(description.device_name, real_communications_label, base::CompareCase::SENSITIVE));
...
public:
    ...
    {
        AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), 48000, 2048);

        // Create a stream with the default device id "".
        AudioOutputStream* stream = audio_manager_->MakeAudioOutputStreamProxy(params, "");
        ASSERT_TRUE(stream);
        AudioOutputDispatcher* dispatcher1 = reinterpret_cast<AudioOutputProxy*>(stream)->get_dispatcher_for_testing();

        // Closing this stream will put it up for reuse.
        stream->Close();
        stream = audio_manager_->MakeAudioOutputStreamProxy(params, AudioDeviceDescription::kDefaultDeviceId);
    ...
    void GetAssociatedOutputDeviceID(const std::string& input_device_id, std::string* output_device_id)
    {
        *output_device_id = device_info_accessor_->GetAssociatedOutputDeviceID(input_device_id);
    }
...
protected:
    ...
    static void CheckDeviceDescriptions(const AudioDeviceDescriptions& device_descriptions)
    {
    ...
    template <typename T = std::nullptr_t> void CreateAudioManagerForTesting()
    {
        ...
        device_info_accessor_ = std::make_unique<AudioDeviceInfoAccessorForTests>(audio_manager_.get());
    }
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kAlsaInputDevice, "switch-input-device");
    ...
    device_info_accessor_->GetAudioInputDeviceDescriptions(&device_descriptions);
    CheckDeviceDescriptions(device_descriptions);
    EXPECT_TRUE(base::Contains(device_descriptions, "switch-input-device", [](const auto& device_description) { return device_description.unique_id; }));
}
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kAlsaOutputDevice, "switch-output-device");
    ...
    device_info_accessor_->GetAudioOutputDeviceDescriptions(&device_descriptions);
    CheckDeviceDescriptions(device_descriptions);
    EXPECT_TRUE(base::Contains(device_descriptions, "switch-output-device", [](const auto& device_description) { return device_description.unique_id; }));
}
#endif // defined(USE_ALSA)
...
public:
    TestAudioManager(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
    ...
    std::string GetAssociatedOutputDeviceID(const std::string& input_id) override
    {
        if (input_id == "input1")
            return "output1";
        DCHECK_EQ(std::string(kRealDefaultInputDeviceID), "input2");
        if (input_id == AudioDeviceDescription::kDefaultDeviceId || input_id == kRealDefaultInputDeviceID)
            return "output2";
        return std::string();
    }
...
{
    CreateAudioManagerForTesting<TestAudioManager>();
    std::string default_input_id = device_info_accessor_->GetDefaultInputDeviceID();
    EXPECT_EQ(default_input_id, kRealDefaultInputDeviceID);
    std::string default_output_id = device_info_accessor_->GetDefaultOutputDeviceID();
    EXPECT_EQ(default_output_id, kRealDefaultOutputDeviceID);
    std::string communications_input_id = device_info_accessor_->GetCommunicationsInputDeviceID();
    EXPECT_EQ(communications_input_id, kRealCommunicationsInputDeviceID);
    std::string communications_output_id = device_info_accessor_->GetCommunicationsOutputDeviceID();
...
public:
    TestAudioSourceCallback(int expected_frames_per_buffer, base::WaitableEvent* event)
    ...
    int OnMoreData(base::TimeDelta, base::TimeTicks, const AudioGlitchInfo&, AudioBus* dest) override
    {
...
TEST_F(AudioManagerTest, CheckMinMaxAudioBufferSizeCallbacks)
{
    ...
    ASSERT_GT(default_params.frames_per_buffer(), GetMinAudioBufferSizeMacOS(media::limits::kMinAudioBufferSize, default_params.sample_rate()));
#else
    ...
    ASSERT_GE(default_params.frames_per_buffer(), media::limits::kMinAudioBufferSize);
#endif
