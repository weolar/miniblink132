...
std::unique_ptr<media::AudioManager> CreateAudioManager(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
{
    return std::make_unique<AudioManagerIOS>(std::move(audio_thread), audio_log_factory);
}

AudioManagerIOS::AudioManagerIOS(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
...
void AudioManagerIOS::GetAudioOutputDeviceNames(AudioDeviceNames* device_names)
{
    DCHECK(device_names->empty());
    AudioSessionManagerIOS::GetInstance().GetAudioDeviceInfo(false, device_names);
}

AudioParameters AudioManagerIOS::GetInputStreamParameters(const std::string& input_device_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    const int hardware_sample_rate = AudioSessionManagerIOS::GetInstance().HardwareSampleRate();
    ...
    AudioParameters params = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config, hardware_sample_rate, kDefaultInputBufferSize);
    return params;
}

std::string AudioManagerIOS::GetAssociatedOutputDeviceID(const std::string& input_device_unique_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    return std::string();
}
...
AudioOutputStream* AudioManagerIOS::MakeLinearOutputStream(const AudioParameters& params, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    return MakeLowLatencyOutputStream(params, std::string(), log_callback);
}

AudioOutputStream* AudioManagerIOS::MakeLowLatencyOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    AUHALStream* stream = new AUHALStream(this, params, kAudioObjectUnknown, log_callback);
    return stream;
}

AudioInputStream* AudioManagerIOS::MakeLinearInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
AudioInputStream* AudioManagerIOS::MakeLowLatencyInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());

    VoiceProcessingMode voice_processing_mode = VoiceProcessingMode::kDisabled;

    auto* stream = new AUAudioInputStream(this, params, kAudioObjectUnknown, log_callback, voice_processing_mode);
    return stream;
}
...
bool AudioManagerIOS::MaybeChangeBufferSize(AudioDeviceID device_id, AudioUnit audio_unit, AudioUnitElement element, size_t desired_buffer_size)
{
...
bool AudioManagerIOS::DeviceSupportsAmbientNoiseReduction(AudioDeviceID device_id)
{
    return false;
}
...
{
    return static_cast<double>(AudioSessionManagerIOS::GetInstance().GetInputGain());
}

void AudioManagerIOS::SetInputVolume(AudioDeviceID device_id, double volume)
{
    AudioSessionManagerIOS::GetInstance().SetInputGain(static_cast<float>(volume));
}
...
{
    return static_cast<int>(AudioSessionManagerIOS::GetInstance().HardwareSampleRate());
}
...
OSStatus AudioManagerIOS::GetInputDeviceStreamFormat(AudioUnit audio_unit, AudioStreamBasicDescription* input_format)
{
    // Configure audio stream format for 16-bit PCM
    const SampleFormat kSampleFormat = kSampleFormatS16;
    input_format->mSampleRate = AudioSessionManagerIOS::GetInstance().HardwareSampleRate();
    input_format->mFormatID = kAudioFormatLinearPCM;
    input_format->mFormatFlags = kLinearPCMFormatFlagIsSignedInteger | kLinearPCMFormatFlagIsPacked;
    input_format->mChannelsPerFrame = AudioSessionManagerIOS::GetInstance().GetDeviceChannels(
        /*is_input=*/true);
    ...
    input_format->mBytesPerPacket = SampleFormatToBytesPerChannel(kSampleFormat) * input_format->mChannelsPerFrame;
...
// protected
AudioParameters AudioManagerIOS::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    const bool has_valid_input_params = input_params.IsValid();
    const int hardware_sample_rate = AudioSessionManagerIOS::GetInstance().HardwareSampleRate();

    const int hardware_channels = AudioSessionManagerIOS::GetInstance().GetDeviceChannels(
        /*is_input=*/false);
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { channel_layout, output_channels }, hardware_sample_rate, kDefaultInputBufferSize);
    return params;
}
