...
TEST(SimpleSources, SineWaveAudioSource)
{
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), AudioParameters::kTelephoneSampleRate, samples);

    SineWaveAudioSource source(1, freq, params.sample_rate());
    std::unique_ptr<AudioBus> audio_bus = AudioBus::Create(params);
    source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus.get());
    ...
    EXPECT_NEAR(0, audio_bus->channel(0)[0], std::numeric_limits<float>::epsilon());
    ...
    EXPECT_NEAR(0, audio_bus->channel(0)[half_period], std::numeric_limits<float>::epsilon());
    EXPECT_FLOAT_EQ(-0.15643446f, audio_bus->channel(0)[half_period + 1]);
    EXPECT_GT(audio_bus->channel(0)[half_period + 1], audio_bus->channel(0)[half_period + 2]);
    EXPECT_GT(audio_bus->channel(0)[half_period + 2], audio_bus->channel(0)[half_period + 3]);
}
...
TEST(SimpleSources, SineWaveAudioCapped)
{
    ...
    EXPECT_EQ(source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus.get()), kSampleCap);
    EXPECT_EQ(1, source.callbacks());
    EXPECT_EQ(source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus.get()), 0);
...
void VerifyContainsTestFile(const AudioBus* audio_bus)
{
    ...
    EXPECT_FLOAT_EQ(static_cast<float>(data[0]) / ((1 << 15) - 1), audio_bus->channel(0)[0]);
    EXPECT_FLOAT_EQ(static_cast<float>(data[1]) / ((1 << 15) - 1), audio_bus->channel(1)[0]);
...
TEST(SimpleSources, FileSourceTestDataWithoutLooping)
{
    ...
    base::File temp(temp_path, base::File::FLAG_WRITE | base::File::FLAG_OPEN_ALWAYS);
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 48000, kNumFrames);
    ...
    EXPECT_EQ(kNumFrames, source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus.get()));
...
{
    AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 48000, 10);
    std::unique_ptr<AudioBus> audio_bus = AudioBus::Create(2, 10);
    audio_bus->Zero();

    // Create a FileSource that reads this file.
    base::FilePath path;
    path = path.Append(FILE_PATH_LITERAL("does")).Append(FILE_PATH_LITERAL("not")).Append(FILE_PATH_LITERAL("exist"));
    bool loop = false;
    FileSource source(params, path, loop);
    EXPECT_EQ(0, source.OnMoreData(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus.get()));
