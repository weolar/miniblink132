...
class MEDIA_EXPORT AlsaPcmInputStream : public AgcAudioStream<AudioInputStream> {
public:
    ...
    // |kAutoSelectDevice|.
    AlsaPcmInputStream(AudioManagerBase* audio_manager, const std::string& device_name, const AudioParameters& params, AlsaWrapper* wrapper);
...
private:
    ...
    raw_ptr<snd_pcm_t> device_handle_; // Handle to the ALSA PCM recording device.
    raw_ptr<snd_mixer_t> mixer_handle_; // Handle to the ALSA microphone mixer.
    raw_ptr<snd_mixer_elem_t> mixer_element_handle_; // Handle to the capture element.
