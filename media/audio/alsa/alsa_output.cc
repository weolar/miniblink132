...
std::ostream& operator<<(std::ostream& os, AlsaPcmOutputStream::InternalState state)
{
    switch (state) {
...
AlsaPcmOutputStream::AlsaPcmOutputStream(const std::string& device_name, const AudioParameters& params, AlsaWrapper* wrapper, AudioManagerBase* manager)
    ...
    , latency_(std::max(base::Microseconds(kMinLatencyMicros), AudioTimestampHelper::FramesToTime(params.frames_per_buffer() * 2, sample_rate_)))
...
bool AlsaPcmOutputStream::Open()
{
    ...
    if (requested_device_name_ == kAutoSelectDevice) {
        ...
        playback_handle_ = alsa_util::OpenPlaybackDevice(wrapper_, device_name_.c_str(), channels_, sample_rate_, pcm_format_, latency_.InMicroseconds());
    }
    ...
    bytes_per_output_frame_ = channel_mixer_ ? mixed_audio_bus_->channels() * bytes_per_sample_ : bytes_per_frame_;
    ...
    if (error < 0) {
        LOG(ERROR) << "Failed to get playback buffer size from ALSA: " << wrapper_->StrError(error);
...
void AlsaPcmOutputStream::Close()
{
    ...
    if (playback_handle_) {
        int res = alsa_util::CloseDevice(wrapper_, playback_handle_.ExtractAsDangling());
        if (res < 0) {
...
void AlsaPcmOutputStream::Start(AudioSourceCallback* callback)
{
    ...
    if (error < 0 && error != -EAGAIN) {
        LOG(ERROR) << "Failure clearing playback device (" << wrapper_->PcmName(playback_handle_) << "): " << wrapper_->StrError(error);
        stop_stream_ = true;
        return;
    }
    ...
    if (error < 0 && error != -EAGAIN) {
        LOG(ERROR) << "Failure preparing stream (" << wrapper_->PcmName(playback_handle_) << "): " << wrapper_->StrError(error);
        stop_stream_ = true;
        return;
    }
...
void AlsaPcmOutputStream::SetTickClockForTesting(const base::TickClock* tick_clock)
{
    DCHECK(tick_clock);
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT("audio", "AlsaPcmOutputStream::BufferPacket", [&](perfetto::EventContext ctx) {
    ...
    *source_exhausted = false;
    ...
    if (!buffer_->forward_bytes()) {
        ...
        output_bus->ToInterleaved<SignedInt16SampleTypeTraits>(frames_filled, reinterpret_cast<int16_t*>(packet->writable_data()));
...
void AlsaPcmOutputStream::WritePacket()
{
    ...
    if (buffer_->GetCurrentChunk(&buffer_data, &buffer_size)) {
        snd_pcm_sframes_t frames = std::min(static_cast<snd_pcm_sframes_t>(buffer_size / bytes_per_output_frame_), GetAvailableFrames());
        ...
        if (frames_written < 0) {
            ...
            frames_written = wrapper_->PcmRecover(playback_handle_, frames_written, kPcmRecoverIsSilent);
            if (frames_written < 0) {
                if (frames_written != -EAGAIN) {
                    LOG(ERROR) << "Failed to write to pcm device: " << wrapper_->StrError(frames_written);
...
void AlsaPcmOutputStream::ScheduleNextWrite(bool source_exhausted)
{
    ...
    if (buffer_->forward_bytes() && available_frames) {
        ...
        next_fill_time = AudioTimestampHelper::FramesToTime(kTargetFramesAvailable - available_frames, sample_rate_);
    ...
    task_runner_->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE,
        base::BindOnce(&AlsaPcmOutputStream::WriteTask, weak_factory_.GetWeakPtr()),
        next_fill_time.is_zero() ? base::TimeTicks() : base::TimeTicks::Now() + next_fill_time, base::subtle::DelayPolicy::kPrecise);
}
...
std::string AlsaPcmOutputStream::FindDeviceForChannels(uint32_t channels)
{
    ...
    void** hints = nullptr;
    int error = wrapper_->DeviceNameHint(kGetAllDevices, kPcmInterfaceName, &hints);
    if (error == 0) {
        ...
        for (void** hint_iter = hints; *hint_iter != nullptr; hint_iter++) {
            ...
            std::unique_ptr<char, base::FreeDeleter> name(wrapper_->DeviceNameGetHint(*hint_iter, kNameHintName));
        ...
        LOG(ERROR) << "Unable to get hints for devices: " << wrapper_->StrError(error);
    }
...
{
    TRACE_EVENT_BEGIN(TRACE_DISABLED_BY_DEFAULT("audio"), "AlsaPcmOutputStream::GetCurrentDelay");
    ...
    if (pcm_state != SND_PCM_STATE_XRUN && pcm_state != SND_PCM_STATE_PREPARED) {
        ...
        if (error < 0) {
            ...
            delay = -1;
            error = wrapper_->PcmRecover(playback_handle_, error, kPcmRecoverIsSilent);
            if (error < 0) {
...
snd_pcm_sframes_t AlsaPcmOutputStream::GetAvailableFrames()
{
    ...
    if (available_frames < 0) {
        available_frames = wrapper_->PcmRecover(playback_handle_, available_frames, kPcmRecoverIsSilent);
    }
    if (available_frames < 0) {
        LOG(ERROR) << "Failed querying available frames. Assuming 0: " << wrapper_->StrError(available_frames);
        return 0;
    }
    if (static_cast<uint32_t>(available_frames) > alsa_buffer_frames_ * 2) {
        LOG(ERROR) << "ALSA returned " << available_frames << " of " << alsa_buffer_frames_ << " frames available.";
...
snd_pcm_t* AlsaPcmOutputStream::AutoSelectDevice(unsigned int latency)
{
    ...
    if (!device_name_.empty()) {
        if ((handle = alsa_util::OpenPlaybackDevice(wrapper_, device_name_.c_str(), channels_, sample_rate_, pcm_format_, latency)) != nullptr) {
            return handle;
        }
    ...
    if (default_channels > 2) {
        default_channels = 2;
        channel_mixer_ = std::make_unique<ChannelMixer>(channel_layout_, channels_, kDefaultOutputChannelLayout, default_channels);
        mixed_audio_bus_ = AudioBus::Create(default_channels, audio_bus_->frames());
    }

    // Step 4.
    device_name_ = kDefaultDevice;
    if ((handle = alsa_util::OpenPlaybackDevice(wrapper_, device_name_.c_str(), default_channels, sample_rate_, pcm_format_, latency)) != nullptr) {
        return handle;
    }
...
AlsaPcmOutputStream::InternalState AlsaPcmOutputStream::TransitionTo(InternalState to)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
int AlsaPcmOutputStream::RunDataCallback(base::TimeDelta delay, base::TimeTicks delay_timestamp, AudioBus* audio_bus)
{
    TRACE_EVENT("audio", "AlsaPcmOutputStream::RunDataCallback", [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_linux_alsa_output();
    ...
    if (source_callback_) {
        ...
        return source_callback_->OnMoreData(BoundedDelay(delay), delay_timestamp, {}, audio_bus);
    }
