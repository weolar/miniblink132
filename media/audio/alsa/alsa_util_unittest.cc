...
TEST(AlsaUtilTest, FreeHwParams)
{
    ...
    EXPECT_CALL(mock_alsa_wrapper, PcmSetParams(_, _, _, _, _, _, _)).WillOnce(Return(-1));
    EXPECT_CALL(mock_alsa_wrapper, StrError(_)).WillOnce(Return("error"));
    EXPECT_CALL(mock_alsa_wrapper, PcmHwParamsMalloc(_)).WillOnce(Invoke([params_ptr](snd_pcm_hw_params_t** params) {
        *params = params_ptr;
        return 0;
    }));
    ...
    snd_pcm_t* handle = OpenCaptureDevice(&mock_alsa_wrapper, "default", 2, 48000, SND_PCM_FORMAT_S16, 40000, 10000);
