...
AlsaPcmInputStream::AlsaPcmInputStream(AudioManagerBase* audio_manager, const std::string& device_name, const AudioParameters& params, AlsaWrapper* wrapper)
    : audio_manager_(audio_manager)
    , device_name_(device_name)
    , params_(params)
    , bytes_per_buffer_(params.GetBytesPerBuffer(kSampleFormat))
    , wrapper_(wrapper)
    , buffer_duration_(base::Microseconds(params.frames_per_buffer() * base::Time::kMicrosecondsPerSecond / static_cast<float>(params.sample_rate())))
    , callback_(nullptr)
...
AudioInputStream::OpenOutcome AlsaPcmInputStream::Open()
{
    ...
    if (device_name_ == kAutoSelectDevice) {
        ...
        for (size_t i = 0; i < std::size(device_names); ++i) {
            device_handle_
                = alsa_util::OpenCaptureDevice(wrapper_, device_names[i], params_.channels(), params_.sample_rate(), kAlsaSampleFormat, buffer_us, packet_us);
        ...
        device_handle_
            = alsa_util::OpenCaptureDevice(wrapper_, device_name_.c_str(), params_.channels(), params_.sample_rate(), kAlsaSampleFormat, buffer_us, packet_us);
    }
    ...
    if (device_handle_) {
        ...
        if (mixer_handle_) {
            mixer_element_handle_ = alsa_util::LoadCaptureMixerElement(wrapper_, mixer_handle_);
        }
    }

    return device_handle_ != nullptr ? OpenOutcome::kSuccess : OpenOutcome::kFailed;
}
...
void AlsaPcmInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (error < 0) {
        ...
        CHECK(capture_thread_.StartWithOptions(base::Thread::Options(base::ThreadType::kRealtimeAudio)));
        ...
        running_ = true;
        capture_thread_.task_runner()->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE,
            base::BindOnce(&AlsaPcmInputStream::ReadAudio, base::Unretained(this)), next_read_time_, base::subtle::DelayPolicy::kPrecise);
    }
}
...
bool AlsaPcmInputStream::Recover(int original_error)
{
    ...
    if (error < 0) {
        ...
        // same error twice.
        LOG(WARNING) << "Unable to recover from \"" << wrapper_->StrError(original_error) << "\": " << wrapper_->StrError(error);
        return false;
    }
...
void AlsaPcmInputStream::StopRunningOnCaptureThread()
{
    ...
    if (!capture_thread_.task_runner()->BelongsToCurrentThread()) {
        capture_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&AlsaPcmInputStream::StopRunningOnCaptureThread, base::Unretained(this)));
        return;
    }
    running_ = false;
}
...
void AlsaPcmInputStream::ReadAudio()
{
    ...
    if (frames < params_.frames_per_buffer()) {
        ...
        capture_thread_.task_runner()->PostDelayedTaskAt(base::subtle::PostDelayedTaskPassKey(), FROM_HERE,
            base::BindOnce(&AlsaPcmInputStream::ReadAudio, base::Unretained(this)), next_check_time, base::subtle::DelayPolicy::kPrecise);
        return;
    }
    ...
    while (num_buffers--) {
        int frames_read = wrapper_->PcmReadi(device_handle_, audio_buffer_.get(), params_.frames_per_buffer());
        if (frames_read == params_.frames_per_buffer()) {
            audio_bus_->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<int16_t*>(audio_buffer_.get()), audio_bus_->frames());
            ...
            if (avail_frames < 0) {
                LOG(WARNING) << "PcmAvailUpdate(): " << wrapper_->StrError(avail_frames);
                avail_frames = 0; // Error getting number of avail frames, set it to 0
            }
            base::TimeDelta hardware_delay = base::Seconds(avail_frames / static_cast<double>(params_.sample_rate()));

            callback_->OnData(audio_bus_.get(), base::TimeTicks::Now() - hardware_delay, normalized_volume, {});
        } else if (frames_read < 0) {
            bool success = Recover(frames_read);
            LOG(WARNING) << "PcmReadi failed with error " << wrapper_->StrError(frames_read) << ". " << (success ? "Successfully" : "Unsuccessfully")
                         << " recovered.";
        } else {
            LOG(WARNING) << "PcmReadi returning less than expected frames: " << frames_read << " vs. " << params_.frames_per_buffer()
    ...
    if (next_read_time_ < now) {
        ...
        DVLOG(1) << "Audio read callback behind schedule by " << (buffer_duration_ + delay).InMicroseconds() << " (us).";
...
void AlsaPcmInputStream::Close()
{
    if (device_handle_) {
        Stop();
        int error = alsa_util::CloseDevice(wrapper_, device_handle_.ExtractAsDangling());
        ...
        if (mixer_handle_) {
            alsa_util::CloseMixer(wrapper_, mixer_handle_.ExtractAsDangling(), device_name_);
        }
...
double AlsaPcmInputStream::GetMaxVolume()
{
    ...
    long min = 0;
    long max = 0;
    if (wrapper_->MixerSelemGetCaptureVolumeRange(mixer_element_handle_, &min, &max)) {
...
void AlsaPcmInputStream::SetVolume(double volume)
{
    ...
    int error = wrapper_->MixerSelemSetCaptureVolumeAll(mixer_element_handle_, static_cast<long>(volume));
    if (error < 0) {
...
double AlsaPcmInputStream::GetVolume()
{
    ...
    int error = wrapper_->MixerSelemGetCaptureVolume(mixer_element_handle_, static_cast<snd_mixer_selem_channel_id_t>(0), &current_volume);
    if (error < 0) {
...
void AlsaPcmInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
