...
public:
    ...
    AlsaPcmOutputStream(const std::string& device_name, const AudioParameters& params, AlsaWrapper* wrapper, AudioManagerBase* manager);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(AlsaPcmOutputStreamTest, AutoSelectDevice_DeviceSelect);
    FRIEND_TEST_ALL_PREFIXES(AlsaPcmOutputStreamTest, AutoSelectDevice_FallbackDevices);
    ...
    FRIEND_TEST_ALL_PREFIXES(AlsaPcmOutputStreamTest, ScheduleNextWrite_StopStream);
    ...
    enum InternalState { kInError = 0, kCreated, kIsOpened, kIsPlaying, kIsStopped, kIsClosed };
    ...
    int RunDataCallback(base::TimeDelta delay, base::TimeTicks delay_timestamp, AudioBus* audio_bus);
...
MEDIA_EXPORT std::ostream& operator<<(std::ostream& os, AlsaPcmOutputStream::InternalState);
