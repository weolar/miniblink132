...
public:
    AudioLowLatencyInputOutputTest(const AudioLowLatencyInputOutputTest&) = delete;
    AudioLowLatencyInputOutputTest& operator=(const AudioLowLatencyInputOutputTest&) = delete;
...
public:
    FullDuplexAudioSinkSource(int sample_rate, int samples_per_packet, int channels)
        ...
        , previous_write_time_(base::TimeTicks::Now())
    {
        ...
        buffer_ = std::make_unique<media::SeekableBuffer>(0, samples_per_packet_ * frame_size_);
    ...
    ~FullDuplexAudioSinkSource() override
    {
        ...
        while (elements_written < std::min(input_elements_to_write_, output_elements_to_write_)) {
            ...
            fprintf(text_file, "%d %d %d %d\n", state.delta_time_ms, state.buffer_delay_ms, state.input_delay_ms, state.output_delay_ms);
            ++elements_written;
        }
    ...
    void OnData(const AudioBus* src, base::TimeTicks capture_time, double volume, const AudioGlitchInfo& glitch_info) override
    {
        ...
        if (input_elements_to_write_ < kMaxDelayMeasurements) {
            ...
            delay_states_[input_elements_to_write_].input_delay_ms = (base::TimeTicks::Now() - capture_time).InMilliseconds();
    ...
    int OnMoreData(base::TimeDelta delay, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest) override
    {
        ...
        if (buffer_->GetCurrentChunk(&source, &size) && size > 0) {
            ...
            dest->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<const int16_t*>(source), size / channels_);
...
public:
    ...
    static AudioParameters GetDefaultAudioStreamParameters(AudioManager* audio_manager)
    {
        return AudioDeviceInfoAccessorForTests(audio_manager).GetInputStreamParameters(AudioDeviceDescription::kDefaultDeviceId);
    }

    static StreamType* CreateStream(AudioManager* audio_manager, const AudioParameters& params)
    {
        return audio_manager->MakeAudioInputStream(params, AudioDeviceDescription::kDefaultDeviceId, base::BindRepeating(&OnLogMessage));
    }
};
...
public:
    ...
    static AudioParameters GetDefaultAudioStreamParameters(AudioManager* audio_manager)
    {
        std::string default_device_id = AudioDeviceInfoAccessorForTests(audio_manager).GetDefaultOutputDeviceID();
        return AudioDeviceInfoAccessorForTests(audio_manager).GetOutputStreamParameters(default_device_id);
    }

    static StreamType* CreateStream(AudioManager* audio_manager, const AudioParameters& params)
    {
        return audio_manager->MakeAudioOutputStream(params, std::string(), base::BindRepeating(&OnLogMessage));
    }
};

// Traits template holding a trait of StreamType. It encapsulates
// AudioInputStream and AudioOutputStream stream types.
template <typename StreamTraits> class StreamWrapper {
public:
...
private:
    StreamType* CreateStream()
    {
        StreamType* stream = StreamTraits::CreateStream(
            audio_manager_, AudioParameters(format_, ChannelLayoutConfig(channel_layout_, channels()), sample_rate_, samples_per_packet_));
        EXPECT_TRUE(stream);
        return stream;
    }
...
TEST_F(AudioLowLatencyInputOutputTest, DISABLED_FullDuplexDelayMeasurement)
{
    ...
    ABORT_AUDIO_TEST_IF_NOT(device_info_accessor.HasAudioInputDevices() && device_info_accessor.HasAudioOutputDevices());
    ...
    if (aisw.sample_rate() != aosw.sample_rate() || aisw.samples_per_packet() != aosw.samples_per_packet() || aisw.channels() != aosw.channels()) {
    ...
    FullDuplexAudioSinkSource full_duplex(aisw.sample_rate(), aisw.samples_per_packet(), aisw.channels());
    ...
    task_runner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), TestTimeouts::action_timeout());
    run_loop.Run();
