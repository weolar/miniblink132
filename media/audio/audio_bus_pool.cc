...
AudioBusPoolImpl::AudioBusPoolImpl(const AudioParameters& params, size_t preallocated, size_t max_capacity)
    : AudioBusPoolImpl(
        params, preallocated, max_capacity, base::BindRepeating(static_cast<std::unique_ptr<AudioBus> (*)(const AudioParameters&)>(&AudioBus::Create)))
{
}
...
AudioBusPoolImpl::AudioBusPoolImpl(const AudioParameters& params, size_t preallocated, size_t max_capacity, CreateAudioBusCallback create_audio_bus)
    : params_(params)
