...
AudioInputStreamFuchsia::AudioInputStreamFuchsia(AudioManagerFuchsia* manager, const AudioParameters& parameters, std::string device_id)
    : manager_(manager)
    , parameters_(parameters)
    , device_id_(std::move(device_id))
{
    DCHECK(device_id_.empty() || device_id_ == AudioDeviceDescription::kLoopbackInputDeviceId || device_id_ == AudioDeviceDescription::kDefaultDeviceId)
        << "AudioInput from " << device_id_ << " not implemented!";
    DCHECK(parameters_.format() == AudioParameters::AUDIO_PCM_LOW_LATENCY || parameters_.format() == AudioParameters::AUDIO_PCM_LINEAR);
}
...
AudioInputStream::OpenOutcome AudioInputStreamFuchsia::Open()
{
    ...
    auto factory = base::ComponentContextForProcess()->svc()->Connect<fuchsia::media::Audio>();
    bool is_loopback = device_id_ == AudioDeviceDescription::kLoopbackInputDeviceId;
    ...
    capturer_.events().OnPacketProduced = fit::bind_member(this, &AudioInputStreamFuchsia::OnPacketProduced);
    ...
    size_t capture_buffer_size = parameters_.GetBytesPerBuffer(kSampleFormatF32) * kBufferPacketCapacity;
    ...
    bool mapped = capture_buffer_.Initialize(std::move(buffer_vmo), /*writable=*/false,
        /*offset=*/0, /*size=*/capture_buffer_size, fuchsia::sysmem2::CoherencyDomain::CPU);

    if (!mapped)
        return OpenOutcome::kFailed;

    // Pass the buffer to the capturer.
    capturer_->AddPayloadBuffer(kBufferId, capture_buffer_.Duplicate(/*writable=*/true));
...
void AudioInputStreamFuchsia::SetOutputDeviceForAec(const std::string& output_device_id)
{
    NOTIMPLEMENTED();
}

void AudioInputStreamFuchsia::OnPacketProduced(fuchsia::media::StreamPacket packet)
{
    size_t bytes_per_frame = parameters_.GetBytesPerFrame(kSampleFormatF32);

    if (packet.payload_buffer_id != kBufferId || packet.payload_offset + packet.payload_size > capture_buffer_.size()
        || packet.payload_size % bytes_per_frame != 0 || packet.payload_size < bytes_per_frame) {
    ...
    if (callback_) {
        ...
        audio_bus_->FromInterleaved<Float32SampleTypeTraits>(
            reinterpret_cast<const float*>(capture_buffer_.GetMemory().data() + packet.payload_offset), num_frames);
