...
public:
    ...
    void InitializeCapturer(ChannelLayoutConfig channel_layout_config)
    {
        ...
        FakeAudioCapturerFactory audio_capturer_factory(test_context.additional_services());

        input_stream_ = std::make_unique<AudioInputStreamFuchsia>(
            /*manager=*/nullptr,
            AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config,
        ...
        test_capturer_->SetDataGeneration(FakeAudioCapturer::DataGeneration::MANUAL);
    ...
    void TestCapture(ChannelLayoutConfig channel_layout_config)
    {
        ...
        ASSERT_EQ(callback_.packets()[0]->frames(), static_cast<int>(kFramesPerPacket));
        for (size_t i = 0; i < samples.size(); ++i) {
            EXPECT_EQ(samples[i], callback_.packets()[0]->channel(i % num_channels)[i / num_channels]);
        }
    }
...
TEST_F(AudioInputStreamFuchsiaTest, InitializeAndStart)
{
    ...
    EXPECT_EQ(test_capturer_->GetPacketSize(), sizeof(float) * kFramesPerPacket * kNumChannels);
...
TEST_F(AudioInputStreamFuchsiaTest, InitializeStereo)
{
    ...
    EXPECT_TRUE(test_capturer_->is_active());
    EXPECT_EQ(test_capturer_->GetPacketSize(), sizeof(float) * kNumChannels * kFramesPerPacket);
}
...
TEST_F(AudioInputStreamFuchsiaTest, CaptureTwoPackets)
{
    ...
    ASSERT_EQ(callback_.packets()[0]->frames(), static_cast<int>(kFramesPerPacket));
    ASSERT_EQ(callback_.packets()[1]->frames(), static_cast<int>(kFramesPerPacket));
    for (size_t i = 0; i < kFramesPerPacket; ++i) {
