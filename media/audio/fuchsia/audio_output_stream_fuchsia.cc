...
std::optional<fuchsia::media::AudioRenderUsage> GetStreamUsage(const AudioParameters& parameters)
{
    int usage_flags = parameters.effects()
        & (AudioParameters::FUCHSIA_RENDER_USAGE_BACKGROUND | AudioParameters::FUCHSIA_RENDER_USAGE_MEDIA | AudioParameters::FUCHSIA_RENDER_USAGE_INTERRUPTION
...
AudioOutputStreamFuchsia::AudioOutputStreamFuchsia(AudioManagerFuchsia* manager, const AudioParameters& parameters)
    : manager_(manager)
...
bool AudioOutputStreamFuchsia::Open()
{
    ...
    fuchsia::media::AudioPtr audio_server = base::ComponentContextForProcess()->svc()->Connect<fuchsia::media::Audio>();
    audio_server->CreateAudioRenderer(audio_renderer_.NewRequest());
    audio_renderer_.set_error_handler(fit::bind_member(this, &AudioOutputStreamFuchsia::OnRendererError));
    ...
    audio_renderer_.events().OnMinLeadTimeChanged = fit::bind_member(this, &AudioOutputStreamFuchsia::OnMinLeadTimeChanged);
...
void AudioOutputStreamFuchsia::Stop()
{
    ...
    pause_pending_ = true;
    audio_renderer_->Pause(fit::bind_member(this, &AudioOutputStreamFuchsia::OnPauseComplete));
...
{
    DCHECK(!reference_time_.is_null());
    return reference_time_ + AudioTimestampHelper::FramesToTime(stream_position_samples_, parameters_.sample_rate());
}
...
size_t AudioOutputStreamFuchsia::GetMinBufferSize()
{
    ...
    int min_packets = (AudioTimestampHelper::TimeToFrames(min_lead_time_.value(), parameters_.sample_rate()) + parameters_.frames_per_buffer() - 1)
...
bool AudioOutputStreamFuchsia::InitializePayloadBuffer()
{
    ...
    audio_renderer_->AddPayloadBuffer(kBufferId, base::WritableSharedMemoryRegion::TakeHandleForSerialization(std::move(region)).PassPlatformHandle());
...
void AudioOutputStreamFuchsia::OnMinLeadTimeChanged(int64_t min_lead_time)
{
    ...
    if (payload_buffer_.IsValid() && GetMinBufferSize() > payload_buffer_.size()) {
        payload_buffer_ = {};
...
void AudioOutputStreamFuchsia::OnPauseComplete(int64_t reference_time, int64_t media_time)
{
...
void AudioOutputStreamFuchsia::PumpSamples()
{
    ...
    if (reference_time_.is_null()) {
        ...
        if (now + min_lead_time_.value() > stream_time) {
            stream_position_samples_ += AudioTimestampHelper::TimeToFrames(now + min_lead_time_.value() - stream_time, parameters_.sample_rate());
        }
    ...
    audio_bus_->ToInterleaved<Float32SampleTypeTraitsNoClip>(
        audio_bus_->frames(), reinterpret_cast<float*>(static_cast<uint8_t*>(payload_buffer_.memory()) + payload_buffer_pos_));
    ...
    if (reference_time_.is_null()) {
        ...
        audio_renderer_->PlayNoReply(reference_time_.ToZxTime(), stream_position_samples_);
    }

    stream_position_samples_ += frames_filled;
    payload_buffer_pos_ = (payload_buffer_pos_ + packet_size) % payload_buffer_.size();
...
{
    base::TimeTicks next_pump_time = GetCurrentStreamTime() - min_lead_time_.value() - parameters_.GetBufferDuration() / 2;

    timer_.Start(FROM_HERE, next_pump_time, base::BindOnce(&AudioOutputStreamFuchsia::PumpSamples, base::Unretained(this)));
}
