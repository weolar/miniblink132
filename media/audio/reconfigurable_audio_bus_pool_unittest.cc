...
public:
    ...
    ReconfigurableAudioBusPoolTest(const ReconfigurableAudioBusPoolTest&) = delete;
    ReconfigurableAudioBusPoolTest& operator=(const ReconfigurableAudioBusPoolTest&) = delete;

    ~ReconfigurableAudioBusPoolTest() override = default;

protected:
    std::unique_ptr<ReconfigurableAudioBusPoolImpl> reconfigurable_audio_bus_pool_;
};

TEST_F(ReconfigurableAudioBusPoolTest, Reconfigure)
{
    AudioParameters original_parameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 1000, 100);
    reconfigurable_audio_bus_pool_ = std::make_unique<ReconfigurableAudioBusPoolImpl>(kAudioBusPoolDuration);
    reconfigurable_audio_bus_pool_->Reconfigure(original_parameters);

    std::unique_ptr<AudioBus> original_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
    EXPECT_EQ(original_audio_bus->channels(), original_parameters.channels());
    EXPECT_EQ(original_audio_bus->frames(), original_parameters.frames_per_buffer());

    AudioParameters new_parameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), 2000, 200);
    reconfigurable_audio_bus_pool_->Reconfigure(new_parameters);
    std::unique_ptr<AudioBus> new_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
    ...
    std::unique_ptr<AudioBus> recycled_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
...
TEST_F(ReconfigurableAudioBusPoolTest, MaxCapacity)
{
    ...
    AudioParameters audio_parameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 1000, 250);
    reconfigurable_audio_bus_pool_ = std::make_unique<ReconfigurableAudioBusPoolImpl>(kAudioBusPoolDuration);
    reconfigurable_audio_bus_pool_->Reconfigure(audio_parameters);

    std::unique_ptr<AudioBus> first_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
    std::unique_ptr<AudioBus> second_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
    std::unique_ptr<AudioBus> third_audio_bus = reconfigurable_audio_bus_pool_->GetAudioBus();
    ...
    EXPECT_EQ(second_audio_bus_ptr, reconfigurable_audio_bus_pool_->GetAudioBus().get());
    EXPECT_EQ(first_audio_bus_ptr, reconfigurable_audio_bus_pool_->GetAudioBus().get());
}
