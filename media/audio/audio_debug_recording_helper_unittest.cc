...
const base::FilePath::CharType kFileName[] = FILE_PATH_LITERAL("debug_recording.output.1.wav");
...
public:
    explicit MockAudioDebugFileWriter(const AudioParameters& params, base::File file)
...
public:
    using ChangedWriterCallback = base::RepeatingCallback<void(MockAudioDebugFileWriter*)>;

    AudioDebugRecordingHelperUnderTest(const AudioParameters& params, base::OnceClosure on_destruction_closure, ChangedWriterCallback changed_writer_callback)
    ...
    AudioDebugRecordingHelperUnderTest(const AudioDebugRecordingHelperUnderTest&) = delete;
    AudioDebugRecordingHelperUnderTest& operator=(const AudioDebugRecordingHelperUnderTest&) = delete;

    ~AudioDebugRecordingHelperUnderTest() override = default;

private:
    // Creates the mock writer.
    AudioDebugFileWriter::Ptr CreateAudioDebugFileWriter(const AudioParameters& params, base::File file) override
    {
        MockAudioDebugFileWriter* writer = new MockAudioDebugFileWriter(params, std::move(file));
...
public:
    ...
    AudioDebugRecordingHelperTest& operator=(const AudioDebugRecordingHelperTest&) = delete;

    ~AudioDebugRecordingHelperTest() override = default;

    // Helper function that creates a recording helper.
    std::unique_ptr<AudioDebugRecordingHelper> CreateRecordingHelper(const AudioParameters& params, base::OnceClosure on_destruction_closure)
    {
        return std::make_unique<AudioDebugRecordingHelperUnderTest>(params, std::move(on_destruction_closure),
    ...
    void CreateWavFile(AudioDebugRecordingStreamType stream_type, uint32_t id, base::OnceCallback<void(base::File)> reply_callback)
    {
        ...
        EXPECT_EQ(id_, id);
        base::FilePath path(base::CreateUniqueTempDirectoryScopedToTest().Append(base::FilePath(kFileName)));
        base::File debug_file(path, base::File::FLAG_CREATE_ALWAYS | base::File::FLAG_WRITE);
...
protected:
    const AudioDebugRecordingStreamType stream_type_ = AudioDebugRecordingStreamType::kInput;
...
TEST_F(AudioDebugRecordingHelperTest, TestDestructionClosure)
{
    ...
    std::unique_ptr<AudioDebugRecordingHelper> recording_helper
        = CreateRecordingHelper(params, base::BindOnce(&AudioDebugRecordingHelperTest::OnAudioDebugRecordingHelperDestruction, base::Unretained(this)));
...
{
    const AudioParameters params;
    std::unique_ptr<AudioDebugRecordingHelper> recording_helper = CreateRecordingHelper(params, base::OnceClosure());
...
TEST_F(AudioDebugRecordingHelperTest, EnableDisable)
{
    ...
    recording_helper->EnableDebugRecording(stream_type_, id_, base::BindOnce(&AudioDebugRecordingHelperTest::CreateWavFile, base::Unretained(this)));
...
TEST_F(AudioDebugRecordingHelperTest, OnData)
{
    ...
    const AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 0, number_of_frames);
    ...
    audio_bus->FromInterleaved<Float32SampleTypeTraits>(source_data.data(), number_of_frames);
