...
class MEDIA_EXPORT AudioInputStreamDataInterceptor : public AudioInputStream, public AudioInputStream::AudioInputCallback {
public:
    using CreateDebugRecorderCB = base::RepeatingCallback<std::unique_ptr<AudioDebugRecorder>()>;
    ...
    AudioInputStreamDataInterceptor(CreateDebugRecorderCB create_debug_recorder_cb, AudioInputStream* stream);

    AudioInputStreamDataInterceptor(const AudioInputStreamDataInterceptor&) = delete;
    AudioInputStreamDataInterceptor& operator=(const AudioInputStreamDataInterceptor&) = delete;
    ...
    void OnData(const AudioBus* source, base::TimeTicks capture_time, double volume, const AudioGlitchInfo& audio_glitch_info) override;
