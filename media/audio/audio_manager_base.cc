...
// enabled.
std::unique_ptr<AudioDebugRecorder> GetNullptrAudioDebugRecorder(const AudioParameters& params)
{
    return nullptr;
}
...
{
    base::UmaHistogramEnumeration("Media.Audio.MakeAudioInputStreamStatus", result);
}

void LogMakeAudioOutputStreamResult(MakeAudioStreamResult result)
{
    base::UmaHistogramEnumeration("Media.Audio.MakeAudioOutputStreamStatus", result);
}

PRINTF_FORMAT(2, 3)
void SendLogMessage(const AudioManagerBase::LogCallback& callback, const char* format, ...)
{
    if (callback.is_null())
        return;
    va_list args;
...
struct AudioManagerBase::DispatcherParams {
    DispatcherParams(const AudioParameters& input, const AudioParameters& output, const std::string& output_device_id)
        : input_params(input)
...
AudioManagerBase::AudioManagerBase(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
...
void AudioManagerBase::GetAudioInputDeviceDescriptions(AudioDeviceDescriptions* device_descriptions)
{
    CHECK(GetTaskRunner()->BelongsToCurrentThread());
    GetAudioDeviceDescriptions(device_descriptions, &AudioManagerBase::GetAudioInputDeviceNames, &AudioManagerBase::GetDefaultInputDeviceID,
        &AudioManagerBase::GetCommunicationsInputDeviceID, &AudioManagerBase::GetGroupIDInput);
}

void AudioManagerBase::GetAudioOutputDeviceDescriptions(AudioDeviceDescriptions* device_descriptions)
{
    CHECK(GetTaskRunner()->BelongsToCurrentThread());
    GetAudioDeviceDescriptions(device_descriptions, &AudioManagerBase::GetAudioOutputDeviceNames, &AudioManagerBase::GetDefaultOutputDeviceID,
        &AudioManagerBase::GetCommunicationsOutputDeviceID, &AudioManagerBase::GetGroupIDOutput);
}

void AudioManagerBase::GetAudioDeviceDescriptions(AudioDeviceDescriptions* device_descriptions, void (AudioManagerBase::*get_device_names)(AudioDeviceNames*),
    std::string (AudioManagerBase::*get_default_device_id)(), std::string (AudioManagerBase::*get_communications_device_id)(),
    std::string (AudioManagerBase::*get_group_id)(const std::string&))
{
    ...
    std::string real_communications_device_id = (this->*get_communications_device_id)();
    ...
    for (auto& name : device_names) {
        ...
        bool is_real_communications_device = name.unique_id == real_communications_device_id;
        ...
        device_descriptions->emplace_back(std::move(name.device_name), std::move(name.unique_id), std::move(group_id),
            is_virtual_system_default || is_real_system_default, is_virtual_communications_device || is_real_communications_device);
    }
}

AudioOutputStream* AudioManagerBase::MakeAudioOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    CHECK(GetTaskRunner()->BelongsToCurrentThread());
    DCHECK(params.IsValid());

    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kFailAudioStreamCreation)) {
        LogMakeAudioOutputStreamResult(MakeAudioStreamResult::kErrorSwitchFailAudioStreamCreation);
        return nullptr;
    }

    SendLogMessage(log_callback, "%s({device_id=%s}, {params=[%s]})", __func__, device_id.c_str(), params.AsHumanReadableString().c_str());
    ...
    if (num_output_streams_ >= max_num_output_streams_) {
        LOG(ERROR) << "Number of opened output audio streams " << num_output_streams_ << " exceed the max allowed number " << max_num_output_streams_;
        LogMakeAudioOutputStreamResult(MakeAudioStreamResult::kErrorExcessiveStreams);
        return nullptr;
    }
    ...
    case AudioParameters::AUDIO_PCM_LINEAR:
        DCHECK(AudioDeviceDescription::IsDefaultDevice(device_id)) << "AUDIO_PCM_LINEAR supports only the default device.";
    ...
    if (stream) {
        ++num_output_streams_;
        SendLogMessage(log_callback, "%s => (number of streams=%d)", __func__, output_stream_count());
    }
    LogMakeAudioOutputStreamResult(stream ? MakeAudioStreamResult::kNoError : MakeAudioStreamResult::kErrorCreateStream);

    return stream;
}

AudioOutputStream* AudioManagerBase::MakeBitstreamOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    return nullptr;
}

AudioInputStream* AudioManagerBase::MakeAudioInputStream(const AudioParameters& input_params, const std::string& device_id, const LogCallback& log_callback)
{
    CHECK(GetTaskRunner()->BelongsToCurrentThread());

    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kFailAudioStreamCreation)) {
        LogMakeAudioInputStreamResult(MakeAudioStreamResult::kErrorSwitchFailAudioStreamCreation);
        return nullptr;
    }

    // If audio has been disabled force usage of a fake audio stream.
    auto params = input_params;
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableAudioInput)) {
    ...
    if (!params.IsValid() || (params.channels() > kMaxInputChannels) || device_id.empty()) {
        DLOG(ERROR) << "Audio parameters are invalid for device " << device_id << ", params: " << params.AsHumanReadableString();
        LogMakeAudioInputStreamResult(MakeAudioStreamResult::kErrorInvalidParams);
        return nullptr;
    }

    if (input_stream_count() >= kMaxInputStreams) {
        LOG(ERROR) << "Number of opened input audio streams " << input_stream_count() << " exceed the max allowed number " << kMaxInputStreams;
        LogMakeAudioInputStreamResult(MakeAudioStreamResult::kErrorExcessiveStreams);
        return nullptr;
    }

    DVLOG(2) << "Creating a new AudioInputStream with buffer size = " << params.frames_per_buffer();
    ...
    if (stream) {
        ...
        if (!log_callback.is_null()) {
            SendLogMessage(log_callback, "%s => (number of streams=%d)", __func__, input_stream_count());
        }
        ...
        if (!params.IsBitstreamFormat() && debug_recording_manager_) {
            ...
            stream = new AudioInputStreamDataInterceptor(base::BindRepeating(&AudioDebugRecordingManager::RegisterDebugRecordingSource,
                                                             base::Unretained(debug_recording_manager_.get()), AudioDebugRecordingStreamType::kInput, params),
                stream);
        }
    }

    LogMakeAudioInputStreamResult(stream ? MakeAudioStreamResult::kNoError : MakeAudioStreamResult::kErrorCreateStream);

    return stream;
}

AudioOutputStream* AudioManagerBase::MakeAudioOutputStreamProxy(const AudioParameters& params, const std::string& device_id)
{
    CHECK(GetTaskRunner()->BelongsToCurrentThread());
    DCHECK(params.IsValid());
    ...
    if (params.format() == AudioParameters::AUDIO_PCM_LOW_LATENCY && output_params.format() != AudioParameters::AUDIO_FAKE) {
        ...
        if (output_params.IsValid()) {
            if (params.effects() & AudioParameters::MULTIZONE) {
                ...
                output_params.set_effects(output_params.effects() | AudioParameters::MULTIZONE);
            }
    ...
    if (uma_stream_format) {
        UMA_HISTOGRAM_ENUMERATION("Media.AudioOutputStreamProxy.StreamFormat", *uma_stream_format, STREAM_FORMAT_MAX + 1);
    } else {
        NOTREACHED();
    }

    auto dispatcher_params = std::make_unique<DispatcherParams>(params, output_params, output_device_id);
    ...
    if (!output_params.RequireOffload()) {
        auto it = base::ranges::find_if(output_dispatchers_, [&](const std::unique_ptr<DispatcherParams>& dispatcher) {
            ...
            return params.Equals(dispatcher->input_params) && output_params.Equals(dispatcher->output_params)
    ...
    std::unique_ptr<AudioOutputDispatcher> dispatcher;
    if (output_params.format() != AudioParameters::AUDIO_FAKE && !output_params.IsBitstreamFormat()) {
        // Using unretained for |debug_recording_manager_| is safe since it
        // outlives the dispatchers (cleared in ShutdownOnAudioThread()).
        dispatcher = std::make_unique<AudioOutputResampler>(this, params, output_params, output_device_id, kCloseDelay,
            debug_recording_manager_ ? base::BindRepeating(&AudioDebugRecordingManager::RegisterDebugRecordingSource,
                base::Unretained(debug_recording_manager_.get()), AudioDebugRecordingStreamType::kOutput)
                                     : base::BindRepeating(&GetNullptrAudioDebugRecorder));
    } else {
        dispatcher = std::make_unique<AudioOutputDispatcherImpl>(this, output_params, output_device_id, kCloseDelay);
    }
...
void AudioManagerBase::GetAudioInputDeviceNames(AudioDeviceNames* device_names)
{
}

void AudioManagerBase::GetAudioOutputDeviceNames(AudioDeviceNames* device_names)
{
}
...
void AudioManagerBase::AddOutputDeviceChangeListener(AudioDeviceListener* listener)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    output_listeners_.AddObserver(listener);
}

void AudioManagerBase::RemoveOutputDeviceChangeListener(AudioDeviceListener* listener)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
AudioParameters AudioManagerBase::GetOutputStreamParameters(const std::string& device_id)
{
    return GetPreferredOutputStreamParameters(device_id, AudioParameters());
}

AudioParameters AudioManagerBase::GetInputStreamParameters(const std::string& device_id)
{
    NOTREACHED();
}

std::string AudioManagerBase::GetAssociatedOutputDeviceID(const std::string& input_device_id)
{
    return std::string();
}

std::string AudioManagerBase::GetGroupIDOutput(const std::string& output_device_id)
{
    ...
    } else if (output_device_id == AudioDeviceDescription::kCommunicationsDeviceId) {
...
std::string AudioManagerBase::GetGroupIDInput(const std::string& input_device_id)
{
    const std::string& real_input_device_id = input_device_id == AudioDeviceDescription::kDefaultDeviceId ? GetDefaultInputDeviceID()
        : input_device_id == AudioDeviceDescription::kCommunicationsDeviceId                              ? GetCommunicationsInputDeviceID()
                                                                                                          : input_device_id;
    std::string output_device_id = GetAssociatedOutputDeviceID(real_input_device_id);
...
int AudioManagerBase::GetUserBufferSize()
{
    ...
    int buffer_size = 0;
    std::string buffer_size_str(cmd_line->GetSwitchValueASCII(switches::kAudioBufferSize));
...
std::unique_ptr<AudioLog> AudioManagerBase::CreateAudioLog(AudioLogFactory::AudioComponent component, int component_id)
{
...
void AudioManagerBase::InitializeDebugRecording()
{
    if (!GetTaskRunner()->BelongsToCurrentThread()) {
        ...
        // unretained.
        GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&AudioManagerBase::InitializeDebugRecording, base::Unretained(this)));
        return;
    }
...
std::unique_ptr<AudioDebugRecordingManager> AudioManagerBase::CreateAudioDebugRecordingManager()
{
...
void AudioManagerBase::SetAecDumpRecordingManager(base::WeakPtr<AecdumpRecordingManager>)
{
