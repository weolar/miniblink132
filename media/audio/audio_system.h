...
public:
    ...
    using OnAudioParamsCallback = base::OnceCallback<void(const std::optional<AudioParameters>&)>;
    using OnDeviceIdCallback = base::OnceCallback<void(const std::optional<std::string>&)>;
    using OnInputDeviceInfoCallback = base::OnceCallback<void(const std::optional<AudioParameters>&, const std::optional<std::string>&)>;

    using OnBoolCallback = base::OnceCallback<void(bool)>;
    using OnDeviceDescriptionsCallback = base::OnceCallback<void(AudioDeviceDescriptions)>;

    virtual ~AudioSystem() = default;

    virtual void GetInputStreamParameters(const std::string& device_id, OnAudioParamsCallback on_params_cb) = 0;

    virtual void GetOutputStreamParameters(const std::string& device_id, OnAudioParamsCallback on_params_cb) = 0;
    ...
    virtual void GetDeviceDescriptions(bool for_input, OnDeviceDescriptionsCallback on_descriptions_cb) = 0;

    // Replies with an empty optional if there is no associated output device
    // found and a non-empty string otherwise.
    virtual void GetAssociatedOutputDeviceID(const std::string& input_device_id, OnDeviceIdCallback on_device_id_cb) = 0;
    ...
    virtual void GetInputDeviceInfo(const std::string& input_device_id, OnInputDeviceInfoCallback on_input_device_info_cb) = 0;
    ...
    static OnDeviceDescriptionsCallback WrapCallbackWithDeviceNameLocalization(OnDeviceDescriptionsCallback callback);
};
