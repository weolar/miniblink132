...
AudioDeviceThread::Callback::Callback(const AudioParameters& audio_parameters, uint32_t segment_length, uint32_t total_segments)
    : audio_parameters_(audio_parameters)
    , memory_length_(base::CheckMul(segment_length, total_segments).ValueOrDie())
...
void AudioDeviceThread::Callback::InitializeOnAudioThread()
{
    ...
    DCHECK(thread_checker_.CalledOnValidThread()) << "Thread checker was attached on the wrong thread";
    MapSharedMemory();
}

// AudioDeviceThread implementation

AudioDeviceThread::AudioDeviceThread(Callback* callback, base::SyncSocket::ScopedHandle socket, const char* thread_name, base::ThreadType thread_type)
    : callback_(callback)
    ...
    , socket_(std::move(socket))
{
    ...
    CHECK(base::PlatformThread::CreateWithType(kStackSize, this, &thread_handle_, thread_type));
