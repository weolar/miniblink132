...
void AudioDebugRecordingTest::CreateAudioManager()
{
    ...
    mock_audio_manager_ = std::make_unique<MockAudioManager>(std::make_unique<TestAudioThread>());
    ASSERT_NE(nullptr, AudioManager::Get());
    ASSERT_EQ(static_cast<AudioManager*>(mock_audio_manager_.get()), AudioManager::Get());
}
...
void AudioDebugRecordingTest::InitializeAudioDebugRecordingManager()
{
    ...
    mock_debug_recording_manager_ = static_cast<MockAudioDebugRecordingManager*>(mock_audio_manager_->GetAudioDebugRecordingManager());
