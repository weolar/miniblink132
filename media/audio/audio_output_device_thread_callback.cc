...
AudioOutputDeviceThreadCallback::AudioOutputDeviceThreadCallback(const media::AudioParameters& audio_parameters,
    base::UnsafeSharedMemoryRegion shared_memory_region, media::AudioRendererSink::RenderCallback* render_callback)
    : media::AudioDeviceThread::Callback(audio_parameters, ComputeAudioOutputBufferSize(audio_parameters),
        /*segment count*/ 1)
    , shared_memory_region_(std::move(shared_memory_region))
    , render_callback_(render_callback)
    , create_time_(base::TimeTicks::Now())
    , stats_reporter_(audio_parameters, AudioDeviceStatsReporter::Type::kOutput)
{
...
{
    UmaHistogramLongTimes("Media.Audio.Render.OutputStreamDuration2", base::TimeTicks::Now() - create_time_);
}
...
void AudioOutputDeviceThreadCallback::MapSharedMemory()
{
    ...
    media::AudioOutputBuffer* buffer = reinterpret_cast<media::AudioOutputBuffer*>(shared_memory_mapping_.memory());
...
void AudioOutputDeviceThreadCallback::Process(uint32_t control_signal)
{
    ...
    media::AudioOutputBuffer* buffer = reinterpret_cast<media::AudioOutputBuffer*>(shared_memory_mapping_.memory());
    media::AudioGlitchInfo glitch_info { .duration = base::Microseconds(buffer->params.glitch_duration_us), .count = buffer->params.glitch_count };
    ...
    base::TimeTicks delay_timestamp = base::TimeTicks() + base::Microseconds(buffer->params.delay_timestamp_us);

    TRACE_EVENT("audio", "AudioOutputDevice::FireRenderCallback", "callback_num", callback_num_, "delay_timestamp (ms)",
        (delay_timestamp - base::TimeTicks()).InMillisecondsF(), "playout_delay (ms)", delay.InMillisecondsF());
    ...
    if (callback_num_ == 2) {
        TRACE_EVENT_NESTABLE_ASYNC_END0("audio", "StartingPlayback", TRACE_ID_LOCAL(this));
        if (first_play_start_time_) {
            UmaHistogramTimes("Media.Audio.Render.OutputDeviceStartTime2", base::TimeTicks::Now() - *first_play_start_time_);
        }
    }
    ...
    // memory.
    render_callback_->Render(delay, delay_timestamp, glitch_info, output_bus_.get());
...
void AudioOutputDeviceThreadCallback::InitializePlayStartTime()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("audio", "StartingPlayback", TRACE_ID_LOCAL(this));
