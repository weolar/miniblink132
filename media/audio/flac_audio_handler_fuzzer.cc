...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::unique_ptr<media::FlacAudioHandler> handler = std::make_unique<media::FlacAudioHandler>(flac_data);
    ...
    std::unique_ptr<media::AudioBus> audio_bus = media::AudioBus::Create(handler->GetNumChannels(), media::AudioHandler::kDefaultFrameCount);
