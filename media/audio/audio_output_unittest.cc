...
public:
    ...
    void CreateWithDefaultParameters()
    {
        ...
        stream_params_ = audio_manager_device_info_->GetOutputStreamParameters(default_device_id);
...
TEST_P(AudioOutputTest, MAYBE_Play200HzTone)
{
    ...
    source.set_on_more_data_callback(base::BindLambdaForTesting([&source, &run_loop, samples_to_play, &got_enough_samples]() {
        if (source.pos_samples() >= samples_to_play && !got_enough_samples) {
            got_enough_samples = true;
            run_loop.Quit();
        }
    }));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), TestTimeouts::action_timeout());
