...
void WriteWavHeader(WavHeaderBuffer* buf, uint32_t channels, uint32_t sample_rate, uint64_t samples)
{
    ...
    const uint32_t bytes_in_payload = bytes_in_payload_64 > kMaxBytesInPayload ? kMaxBytesInPayload : bytes_in_payload_64;
    LOG_IF(WARNING, bytes_in_payload < bytes_in_payload_64) << "Number of samples is too large and will be clipped by Wave header,"
                                                            << " all the data above " << kMaxBytesInPayload << " bytes will appear as junk";
    ...
    base::SpanWriter writer(base::as_writable_bytes(base::span(*buf).first(kWavHeaderSize)));
...
void AudioDebugFileWriter::Write(const AudioBus& data)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioDebugFileWriter::DoWrite, weak_this_, std::move(data_copy)));
}

AudioDebugFileWriter::Ptr AudioDebugFileWriter::Create(const AudioParameters& params, base::File file)
{
    return Create(params, std::move(file), std::make_unique<AudioBusPoolImpl>(params, kPreallocatedAudioBuses, kMaxCachedAudioBuses));
}

AudioDebugFileWriter::AudioDebugFileWriter(const AudioParameters& params, base::File file, std::unique_ptr<AudioBusPool> audio_bus_pool)
    : params_(params)
    , file_(std::move(file))
...
AudioDebugFileWriter::Ptr AudioDebugFileWriter::Create(const AudioParameters& params, base::File file, std::unique_ptr<AudioBusPool> audio_bus_pool)
{
    AudioDebugFileWriter* writer = new AudioDebugFileWriter(params, std::move(file), std::move(audio_bus_pool));
    writer->task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioDebugFileWriter::WriteHeader, writer->weak_this_));
...
void AudioDebugFileWriter::DoWrite(std::unique_ptr<AudioBus> data)
{
    ...
    auto data_size = base::checked_cast<size_t>(data->frames() * data->channels());
    ...
    data->ToInterleaved<media::SignedInt16SampleTypeTraits>(data->frames(), interleaved_data_->data());
    samples_ += data_size;
