...
bool ParamsAreValid(const WavAudioParameters& params)
{
    return (
        ...
        params.num_channels != 0u && params.num_channels <= static_cast<uint16_t>(limits::kMaxChannels) &&
        // Check sample rate
        params.sample_rate != 0u
        && (
            // Check bits per second for PCM data
            (params.audio_format == WavAudioHandler::AudioFormat::kAudioFormatPCM
                && (params.bits_per_sample == 8u || params.bits_per_sample == 16u || params.bits_per_sample == 32u))
            ||
            // Check bits per second for float data
            (params.audio_format == WavAudioHandler::AudioFormat::kAudioFormatFloat && (params.bits_per_sample == 32u || params.bits_per_sample == 64u)))
        &&
        // Check extensible format bps
        (!params.is_extensible || params.valid_bits_per_sample == params.bits_per_sample));
}
...
bool ParseFmtChunk(base::span<const uint8_t> data, WavAudioParameters* params)
{
    ...
    params->audio_format
        = static_cast<WavAudioHandler::AudioFormat>(base::U16FromLittleEndian(data.subspan<kAudioFormatOffset, sizeof(WavAudioHandler::AudioFormat)>()));
    params->num_channels = base::U16FromLittleEndian(data.subspan<kChannelOffset, 2u>());
    params->sample_rate = base::U32FromLittleEndian(data.subspan<kSampleRateOffset, 4u>());
    params->bits_per_sample = base::U16FromLittleEndian(data.subspan<kBitsPerSampleOffset, 2u>());

    if (params->audio_format == WavAudioHandler::AudioFormat::kAudioFormatExtensible) {
        ...
        params->audio_format
            = static_cast<WavAudioHandler::AudioFormat>(base::U16FromLittleEndian(data.subspan<kSubFormatOffset, sizeof(WavAudioHandler::AudioFormat)>()));
        params->valid_bits_per_sample = base::U16FromLittleEndian(data.subspan<kValidBitsPerSampleOffset, 2u>());
    } else {
...
bool ParseWavData(std::string_view wav_data, std::string_view* audio_data_out, WavAudioParameters* params_out)
{
    ...
    while (buf.remaining() >= kChunkHeaderSize) {
        ...
        base::span<const uint8_t> chunk_payload = *buf.Read(std::min(size_t { chunk_length }, buf.remaining()));
        ...
        if (chunk_fmt == kFmtSubchunkId) {
            ...
            *audio_data_out = base::as_string_view(chunk_payload);
            ...
            DVLOG(1) << "Skipping unknown data chunk: " << base::as_string_view(chunk_fmt) << ".";
        }
    }
...
WavAudioHandler::WavAudioHandler(std::string_view audio_data, uint16_t num_channels, uint32_t sample_rate, uint16_t bits_per_sample, AudioFormat audio_format)
...
// static
std::unique_ptr<WavAudioHandler> WavAudioHandler::Create(std::string_view wav_data)
{
    ...
    return base::WrapUnique(new WavAudioHandler(audio_data, params.num_channels, params.sample_rate, params.bits_per_sample, params.audio_format));
}
...
bool WavAudioHandler::CopyTo(AudioBus* bus, size_t* frames_written)
{
    ...
    case AudioFormat::kAudioFormatPCM:
        ...
        case 8:
            bus->FromInterleaved<UnsignedInt8SampleTypeTraits>(reinterpret_cast<const uint8_t*>(source), frames);
            break;
        case 16:
            bus->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<const int16_t*>(source), frames);
            break;
        case 32:
            bus->FromInterleaved<SignedInt32SampleTypeTraits>(reinterpret_cast<const int32_t*>(source), frames);
            break;
        default:
            NOTREACHED() << "Unsupported bytes per sample encountered for integer PCM: " << bytes_per_frame;
        }
        break;
    case AudioFormat::kAudioFormatFloat:
        switch (bits_per_sample_) {
        case 32:
            bus->FromInterleaved<Float32SampleTypeTraitsNoClip>(reinterpret_cast<const float*>(source), frames);
            break;
        case 64:
            bus->FromInterleaved<Float64SampleTypeTraits>(reinterpret_cast<const double*>(source), frames);
            break;
        default:
            NOTREACHED() << "Unsupported bytes per sample encountered for float PCM: " << bytes_per_frame;
        }
        break;
    default:
        NOTREACHED() << "Unsupported audio format encountered: " << static_cast<uint16_t>(audio_format_);
    }
