...
bool TimesAreNear(base::TimeTicks t1, base::TimeTicks t2, base::TimeDelta error)
{
...
public:
    AudioEncodersTest()
        : audio_source_(GetParam().channels,
            /*freq=*/440, GetParam().sample_rate)
    {
        options_.codec = GetParam().codec;
        options_.sample_rate = GetParam().sample_rate;
        options_.channels = GetParam().channels;
        expected_duration_helper_ = std::make_unique<AudioTimestampHelper>(options_.sample_rate);
    ...
    void CreateEncoder()
    {
        if (options_.codec == AudioCodec::kOpus) {
            ...
            frames_per_buffer_ = AudioTimestampHelper::TimeToFrames(buffer_duration_, options_.sample_rate);
        } else if (options_.codec == AudioCodec::kAAC) {
#if BUILDFLAG(IS_WIN) && HAS_AAC_ENCODER
            if ((base::win::OSInfo::GetInstance()->version() == base::win::Version::WIN11_22H2
                    || base::win::OSInfo::GetInstance()->version() == base::win::Version::WIN11_23H2)
            ...
            encoder_ = std::make_unique<MFAudioEncoder>(base::SequencedTaskRunner::GetCurrentDefault());
            frames_per_buffer_ = kAacFramesPerBuffer;
            buffer_duration_ = AudioTimestampHelper::FramesToTime(frames_per_buffer_, options_.sample_rate);
            ...
            if (__builtin_available(android NDK_MEDIA_CODEC_MIN_API, *)) {
                encoder_ = std::make_unique<NdkAudioEncoder>(base::SequencedTaskRunner::GetCurrentDefault());
                frames_per_buffer_ = kAacFramesPerBuffer;
                buffer_duration_ = AudioTimestampHelper::FramesToTime(frames_per_buffer_, options_.sample_rate);
            } else {
    ...
    void InitializeEncoder(AudioEncoder::OutputCB output_cb = base::NullCallback())
    {
        if (!output_cb) {
            output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, MaybeDesc) { observed_output_duration_ += output.duration; });
        }

        bool called_done = false;
        AudioEncoder::EncoderStatusCB done_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
            if (!error.is_ok()) {
        ...
        if (options_.codec == AudioCodec::kOpus) {
            min_number_input_frames_needed_ = reinterpret_cast<AudioOpusEncoder*>(encoder_.get())->fifo_->min_number_input_frames_needed_for_testing();
        }
    }
    ...
    int ProduceAudioAndEncode(
        base::TimeTicks timestamp = base::TimeTicks::Now(), int num_frames = 0, AudioEncoder::EncoderStatusCB done_cb = base::NullCallback())
    {
        DCHECK(encoder_);
        if (num_frames == 0)
    ...
    void DoEncode(std::unique_ptr<AudioBus> audio_bus, base::TimeTicks timestamp, AudioEncoder::EncoderStatusCB done_cb = base::NullCallback())
    {
        if (!done_cb) {
            ...
            done_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
                ...
                pending_callback_results_[pending_callback_count_].status_code = error.code();
        ...
        expected_output_duration_ += expected_duration_helper_->GetFrameDuration(num_frames);
        expected_duration_helper_->AddFrames(num_frames);
    }

    void FlushAndVerifyStatus(EncoderStatus::Codes status_code = EncoderStatus::Codes::kOk)
    {
        ...
        auto flush_done_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
            if (error.code() != status_code) {
                FAIL() << "Expected " << EncoderStatusCodeToString(status_code) << " but got " << EncoderStatusCodeToString(error.code());
            }
            flush_done = true;
        });
        encoder()->Flush(std::move(flush_done_cb).Then(run_loop.QuitWhenIdleClosure()));
    ...
    void ValidateOutputDuration(int64_t flush_count = 1)
    {
        ...
        int64_t frame_remainder = frames_per_buffer_ - (expected_duration_helper_->frame_count() % frames_per_buffer_);
        ...
        int64_t number_of_outputs = std::ceil((expected_duration_helper_->frame_count() + amount_of_padding) / static_cast<double>(frames_per_buffer_));
        int64_t duration_of_padding_us = number_of_outputs * AudioTimestampHelper::FramesToTime(frames_per_buffer_, options_.sample_rate).InMicroseconds();
        int64_t acceptable_diff = duration_of_padding_us + 10;
        EXPECT_NEAR(expected_output_duration_.InMicroseconds(), observed_output_duration_.InMicroseconds(), acceptable_diff);
    }
...
TEST_P(AudioEncodersTest, InitializeTwice)
{
    ...
    auto done_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
        if (error.code() != EncoderStatus::Codes::kEncoderInitializeTwice)
            FAIL() << "Expected kEncoderInitializeTwice error but got " << EncoderStatusCodeToString(error.code());
        called_done = true;
    });
...
{
    bool called_done = false;
    AudioEncoder::EncoderStatusCB done_cb = base::BindLambdaForTesting([&](EncoderStatus error) { called_done = true; });
...
TEST_P(AudioEncodersTest, EncodeWithoutInitialize)
{
    ...
    auto done_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
        if (error.code() != EncoderStatus::Codes::kEncoderInitializeNeverCompleted)
            FAIL() << "Expected kEncoderInitializeNeverCompleted error but got " << EncoderStatusCodeToString(error.code());
        called_done = true;
    });

    auto audio_bus = AudioBus::Create(options_.channels, /*frames=*/1);
    encoder()->Encode(std::move(audio_bus), base::TimeTicks::Now(), std::move(done_cb));
...

        {
            ...
            auto flush_cb = base::BindLambdaForTesting([&](EncoderStatus error) {
                if (error.code() != EncoderStatus::Codes::kOk) {
                    FAIL() << "Expected kOk but got " << EncoderStatusCodeToString(error.code());
                }
                called_flush = true;
            });

            encoder()->Flush(std::move(flush_cb).Then(run_loop.QuitWhenIdleClosure()));
            run_loop.Run();
...
TEST_P(AudioEncodersTest, ProvideInputAfterDoneCb)
{
    ...
    AudioEncoder::EncoderStatusCB done_cb = base::BindLambdaForTesting(done_lambda);
    ProduceAudioAndEncode(base::TimeTicks(), frames_per_buffer_, std::move(done_cb));
    task_environment_.RunUntilIdle();
    EXPECT_TRUE(called_done);
...
TEST_P(AudioEncodersTest, Timestamps)
{
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, MaybeDesc) { timestamps.push_back(output.timestamp); });
    ...
    for (base::TimeDelta duration : { buffer_duration_ * 10, buffer_duration_, buffer_duration_ * 2 / 3 }) {
        timestamps.clear();
        int num_frames = AudioTimestampHelper::TimeToFrames(duration, options_.sample_rate);

        size_t total_frames = num_frames * kCount;

#if HAS_AAC_ENCODER
        if (options_.codec == AudioCodec::kAAC && total_frames % kAacFramesPerBuffer) {
        ...
        EXPECT_TRUE(timestamps.size() == expected_outputs || timestamps.size() == expected_outputs + 1);
        ...
        for (auto& observed_ts : timestamps) {
            base::TimeTicks expected_ts = timestamp_tracker.GetTimestamp() + base::TimeTicks();
            EXPECT_TRUE(TimesAreNear(expected_ts, observed_ts, base::Microseconds(1))) << "expected_ts: " << expected_ts << ", observed_ts: " << observed_ts;
...
INSTANTIATE_TEST_SUITE_P(Opus, AudioEncodersTest, testing::ValuesIn(kTestAudioParamsOpus));

#if HAS_AAC_ENCODER
INSTANTIATE_TEST_SUITE_P(AAC, AudioEncodersTest, testing::ValuesIn(kTestAudioParamsAAC));
#endif // HAS_AAC_ENCODER
...
TEST_P(AudioOpusEncoderTest, ExtraData)
{
    ...
    AudioEncoder::OutputCB output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, MaybeDesc desc) {
        DCHECK(desc.has_value());
        extra = desc.value();
    });

    InitializeEncoder(std::move(output_cb));
    ProduceAudioAndEncode(base::TimeTicks::Now(), min_number_input_frames_needed_);
    task_environment_.RunUntilIdle();
...
TEST_P(AudioOpusEncoderTest, FullCycleEncodeDecode)
{
    ...
    const int kOpusDecoderFramesPerBuffer = AudioTimestampHelper::TimeToFrames(kOpusBufferDuration, kOpusDecoderSampleRate);

    int error;
    OpusDecoder* opus_decoder = opus_decoder_create(kOpusDecoderSampleRate, options_.channels, &error);
    ...
    auto verify_opus_encoding = [&](EncodedAudioBuffer output, MaybeDesc) {
        ...
        EXPECT_EQ(kOpusDecoderFramesPerBuffer,
            opus_decode_float(opus_decoder, output.encoded_data.data(), output.encoded_data.size(), buffer.data(), kOpusDecoderFramesPerBuffer, 0));
    };
...
{
    constexpr AudioEncoder::BitrateMode kTestOpusBitrateMode[] = { AudioEncoder::BitrateMode::kConstant, AudioEncoder::BitrateMode::kVariable };
...
TEST_P(AudioOpusEncoderTest, FullCycleEncodeDecode_OpusOptions)
{
    ...
    constexpr media::AudioEncoder::OpusOptions kTestOpusOptions[] = {
        // Base case
        { .frame_duration = base::Milliseconds(20), .complexity = 10, .packet_loss_perc = 0, .use_in_band_fec = false, .use_dtx = false },

        // Use inband-FEC
        { .frame_duration = base::Microseconds(2500), .complexity = 0, .packet_loss_perc = 10, .use_in_band_fec = true, .use_dtx = false },

        // Use DTX
        { .frame_duration = base::Milliseconds(60), .complexity = 5, .packet_loss_perc = 0, .use_in_band_fec = false, .use_dtx = true },

        // Use inband-FEC and DTX
        { .frame_duration = base::Milliseconds(5), .complexity = 5, .packet_loss_perc = 20, .use_in_band_fec = true, .use_dtx = true },
    };
    ...
    for (const AudioEncoder::OpusOptions& opus_options : kTestOpusOptions) {
        ...
        int decoder_frames_per_buffer = AudioTimestampHelper::TimeToFrames(buffer_duration_, kOpusDecoderSampleRate);
        ...
        auto verify_opus_encoding = [&](EncodedAudioBuffer output, MaybeDesc) {
            ...
            EXPECT_EQ(decoder_frames_per_buffer,
                opus_decode_float(opus_decoder, output.encoded_data.data(), output.encoded_data.size(), buffer.data(), decoder_frames_per_buffer, 0));
        };
...
TEST_P(AudioOpusEncoderTest, VariableChannelCounts)
{
    ...
    SineWaveAudioSource sources[] = { SineWaveAudioSource(1, kTestToneFrequency, options_.sample_rate),
        SineWaveAudioSource(2, kTestToneFrequency, options_.sample_rate), SineWaveAudioSource(3, kTestToneFrequency, options_.sample_rate) };

    const int num_frames = options_.sample_rate * buffer_duration_.InSecondsF();

    auto generate_audio = [&sources, &num_frames](int channel_count, base::TimeTicks current_timestamp) {
        auto audio_bus = AudioBus::Create(channel_count, num_frames);
        sources[channel_count - 1].OnMoreData(base::TimeDelta(), current_timestamp, {}, audio_bus.get());
        return audio_bus;
    };
    ...
    auto empty_output_cb = base::BindLambdaForTesting([&](EncodedAudioBuffer output, MaybeDesc) {});
...
INSTANTIATE_TEST_SUITE_P(Opus, AudioOpusEncoderTest, testing::ValuesIn(kTestAudioParamsOpus));
...
public:
    ...
    {
        decoder_ = std::make_unique<FFmpegAudioDecoder>(base::SequencedTaskRunner::GetCurrentDefault(), &media_log);
        ...
        AudioDecoderConfig config(AudioCodec::kAAC, SampleFormat::kSampleFormatS16, channel_layout, options_.sample_rate,
            /*extra_data=*/std::vector<uint8_t>(), EncryptionScheme::kUnencrypted);
        auto init_cb = [](DecoderStatus decoder_status) { EXPECT_EQ(decoder_status, DecoderStatus::Codes::kOk); };
        ...
        decoder_->Initialize(config, /*cdm_context=*/nullptr, base::BindLambdaForTesting(init_cb), base::BindLambdaForTesting(output_cb),
...
TEST_P(AACAudioEncoderTest, FullCycleEncodeDecode)
{
    ...
    auto encode_output_cb = [&](EncodedAudioBuffer output, MaybeDesc) {
        ...
        scoped_refptr<DecoderBuffer> decoder_buffer = DecoderBuffer::FromArray(std::move(output.encoded_data));
    ...
    int expected_outputs = 3 + std::ceil(GetExpectedPadding() / static_cast<double>(frames_per_buffer_));
...
{
    constexpr AudioEncoder::BitrateMode kTestAacBitrateMode[] = { AudioEncoder::BitrateMode::kConstant, AudioEncoder::BitrateMode::kVariable };
    ...
    for (const AudioEncoder::BitrateMode& bitrate_mode : kTestAacBitrateMode) {
        ...
        auto encode_output_cb = [&](EncodedAudioBuffer output, MaybeDesc) {
            auto decode_cb = [&](DecoderStatus status) { EXPECT_EQ(status, DecoderStatus::Codes::kOk); };
            scoped_refptr<DecoderBuffer> decoder_buffer = DecoderBuffer::FromArray(std::move(output.encoded_data));
            decoder_->Decode(decoder_buffer, base::BindLambdaForTesting(decode_cb));
        };
...
{
    constexpr AudioEncoder::AacOutputFormat kTestAacOutputFormat[] = { AudioEncoder::AacOutputFormat::AAC, AudioEncoder::AacOutputFormat::ADTS };
    ...
    for (const auto& output_format : kTestAacOutputFormat) {
        ...
        const bool needs_description = output_format == AudioEncoder::AacOutputFormat::AAC;

        auto encode_output_cb = [&](EncodedAudioBuffer output, MaybeDesc codec_description) {
            if (first_output) {
                first_output = false;
...
INSTANTIATE_TEST_SUITE_P(AAC, AACAudioEncoderTest, testing::ValuesIn(kTestAudioParamsAAC));
#endif // HAS_AAC_ENCODER
