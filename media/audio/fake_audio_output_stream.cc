...
// static
AudioOutputStream* FakeAudioOutputStream::MakeFakeStream(AudioManagerBase* manager, const AudioParameters& params)
{
    return new FakeAudioOutputStream(manager, params);
}

FakeAudioOutputStream::FakeAudioOutputStream(AudioManagerBase* manager, const AudioParameters& params)
...
void FakeAudioOutputStream::Start(AudioSourceCallback* callback)
{
    DCHECK(audio_manager_->GetTaskRunner()->BelongsToCurrentThread());
    callback_ = callback;
    fake_worker_.Start(base::BindRepeating(&FakeAudioOutputStream::CallOnMoreData, base::Unretained(this)));
}
...
void FakeAudioOutputStream::CallOnMoreData(base::TimeTicks ideal_time, base::TimeTicks now)
{
    ...
    const auto delay = fixed_data_delay_ + std::max(base::TimeDelta(), ideal_time - now);
