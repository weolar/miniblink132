...
public:
    AudioBusPoolImpl(const AudioParameters& params, size_t preallocated, size_t max_capacity);
...
private:
    ...
    using CreateAudioBusCallback = base::RepeatingCallback<std::unique_ptr<AudioBus>(const AudioParameters&)>;

    AudioBusPoolImpl(const AudioParameters& params, size_t preallocated, size_t max_capacity, CreateAudioBusCallback create_audio_bus);
