...
AudioInputStreamDataInterceptor::AudioInputStreamDataInterceptor(CreateDebugRecorderCB create_debug_recorder_cb, AudioInputStream* stream)
...
void AudioInputStreamDataInterceptor::Start(AudioInputStream::AudioInputCallback* callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    callback_ = callback;
...
void AudioInputStreamDataInterceptor::SetOutputDeviceForAec(const std::string& output_device_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return stream_->SetOutputDeviceForAec(output_device_id);
}

void AudioInputStreamDataInterceptor::OnData(const AudioBus* source, base::TimeTicks capture_time, double volume, const AudioGlitchInfo& audio_glitch_info)
{
