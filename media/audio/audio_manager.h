...
public:
    ...
    static std::unique_ptr<AudioManager> Create(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory);

    // A convenience wrapper of AudioManager::Create for testing.
    static std::unique_ptr<AudioManager> CreateForTesting(std::unique_ptr<AudioThread> audio_thread);
    ...
    virtual AudioOutputStream* MakeAudioOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) = 0;
    ...
    virtual AudioOutputStream* MakeAudioOutputStreamProxy(const AudioParameters& params, const std::string& device_id) = 0;
    ...
    virtual AudioInputStream* MakeAudioInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) = 0;
    ...
    virtual void RemoveOutputDeviceChangeListener(AudioDeviceListener* listener) = 0;
    ...
    virtual std::unique_ptr<AudioLog> CreateAudioLog(AudioLogFactory::AudioComponent component, int component_id) = 0;
    ...
    // thread (GetTaskRunner()).
    virtual void SetAecDumpRecordingManager(base::WeakPtr<AecdumpRecordingManager> aecdump_recording_manager) = 0;
...
protected:
    ...
    virtual void GetAudioInputDeviceDescriptions(AudioDeviceDescriptions* device_descriptions) = 0;
    ...
    // Not threadsafe; in production this should only be called from the
    // Audio worker thread (see GetTaskRunner()).
    virtual void GetAudioOutputDeviceDescriptions(AudioDeviceDescriptions* device_descriptions) = 0;

    // Returns the output hardware audio parameters for a specific output device.
    virtual AudioParameters GetOutputStreamParameters(const std::string& device_id) = 0;
    ...
    virtual AudioParameters GetInputStreamParameters(const std::string& device_id) = 0;
    ...
    // GetTaskRunner()).
    virtual std::string GetAssociatedOutputDeviceID(const std::string& input_device_id) = 0;
