...
PCMWaveInAudioInputStream::PCMWaveInAudioInputStream(AudioManagerWin* manager, const AudioParameters& params, int num_buffers, const std::string& device_id)
    : state_(kStateEmpty)
    , manager_(manager)
    , callback_(NULL)
    ...
    , audio_bus_(media::AudioBus::Create(params))
{
    ...
    format_.wBitsPerSample = 16; // params.bits_per_sample();
    format_.cbSize = 0;
...
AudioInputStream::OpenOutcome PCMWaveInAudioInputStream::Open()
{
    ...
    result = ::waveInOpen(&wavein_, device_id, &format_, reinterpret_cast<DWORD_PTR>(WaveCallback), reinterpret_cast<DWORD_PTR>(this), CALLBACK_FUNCTION);
...
void PCMWaveInAudioInputStream::WaveCallback(HWAVEIN hwi, UINT msg, DWORD_PTR instance, DWORD_PTR param1, DWORD_PTR)
{
    ...
    if (msg == WIM_DATA) {
        ...
        if (obj->callback_) {
            ...
            //       obj->audio_bus_->FromInterleaved(reinterpret_cast<uint8_t*>(buffer->lpData),
            //                                        obj->audio_bus_->frames(),
            //                                        obj->format_.wBitsPerSample / 8);

            obj->audio_bus_->FromInterleaved<UnsignedInt8SampleTypeTraits>(reinterpret_cast<const uint8_t*>(buffer->lpData), obj->audio_bus_->frames());
...
void PCMWaveInAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
