...
bool Initialize()
{
    if (!g_set_mm_thread_priority) {
        ...
        auto path = base::win::ExpandEnvironmentVariables(L"%SystemRoot%\\system32\\avrt.dll");
        if (!path) {
            return false;
        }
        ...
        g_revert_mm_thread_characteristics = reinterpret_cast<AvRevertMmThreadCharacteristicsFn>(GetProcAddress(g_avrt, "AvRevertMmThreadCharacteristics"));
        g_set_mm_thread_characteristics = reinterpret_cast<AvSetMmThreadCharacteristicsFn>(GetProcAddress(g_avrt, "AvSetMmThreadCharacteristicsW"));
        g_set_mm_thread_priority = reinterpret_cast<AvSetMmThreadPriorityFn>(GetProcAddress(g_avrt, "AvSetMmThreadPriority"));
    }
...
HANDLE AvSetMmThreadCharacteristics(const wchar_t* task_name, DWORD* task_index)
{
