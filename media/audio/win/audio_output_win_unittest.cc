...
static int ClearData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest)
{
    dest->Zero();
    return dest->frames();
}
...
public:
        ...
        , size_(0)
    {
        ...
        if (!fmap_)
            return;
        start_ = reinterpret_cast<char*>(::MapViewOfFile(fmap_, FILE_MAP_READ, 0, 0, 0));
        if (!start_)
            return;
    ...
    const void* GetChunkAt(uint32_t offset)
    {
...
public:
    WinAudioTest()
    {
        ...
        audio_manager_device_info_ = std::make_unique<AudioDeviceInfoAccessorForTests>(audio_manager_.get());
...
TEST_F(WinAudioTest, PCMWaveStreamGetAndClose)
{
    ...
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 8000, 256), std::string(), AudioManager::LogCallback());
...
TEST_F(WinAudioTest, PCMWaveSlowSource)
{
    ...
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), 16000, 256), std::string(), AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
...
TEST_F(WinAudioTest, PCMWaveStreamPlaySlowLoop)
{
    ...
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), AudioParameters::kAudioCDSampleRate, samples_100_ms), std::string(),
...
TEST_F(WinAudioTest, PCMWaveStreamPlay200HzTone22Kss)
{
    ...
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), AudioParameters::kAudioCDSampleRate / 2, samples_100_ms), std::string(),
        AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
...
TEST_F(WinAudioTest, PushSourceFile16KHz)
{
    ...
    AudioOutputStream* oas
        = audio_manager_->MakeAudioOutputStream(AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), kSampleRate, kSamples100ms),
...
TEST_F(WinAudioTest, PCMWaveStreamPlay200HzToneLowLatency)
{
    ...
    std::string default_device_id = audio_manager_device_info_->GetDefaultOutputDeviceID();
    const AudioParameters params = audio_manager_device_info_->GetOutputStreamParameters(default_device_id);
    int sample_rate = params.sample_rate();
    uint32_t samples_10_ms = sample_rate / 100;
    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(
        AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Mono(), sample_rate, samples_10_ms), std::string(),
        AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
...
TEST_F(WinAudioTest, PCMWaveStreamPendingBytes)
{
    ...
    EXPECT_CALL(source, OnMoreData(base::TimeDelta(), _, AudioGlitchInfo(), NotNull())).WillOnce(Invoke(ClearData));

    // Note: If AudioManagerWin::NumberOfWaveOutBuffers() ever changes, or if this
    // test is run on Vista, these expectations will fail.
    EXPECT_CALL(source, OnMoreData(delay_100_ms, _, AudioGlitchInfo(), NotNull())).WillOnce(Invoke(ClearData));
    EXPECT_CALL(source, OnMoreData(delay_200_ms, _, AudioGlitchInfo(), NotNull())).WillOnce(Invoke(ClearData));
    EXPECT_CALL(source, OnMoreData(delay_200_ms, _, AudioGlitchInfo(), NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
    EXPECT_CALL(source, OnMoreData(delay_100_ms, _, AudioGlitchInfo(), NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
    EXPECT_CALL(source, OnMoreData(base::TimeDelta(), _, AudioGlitchInfo(), NotNull())).Times(AnyNumber()).WillRepeatedly(Return(0));
...
public:
    SyncSocketSource(base::SyncSocket* socket, const AudioParameters& params, int expected_packet_count)
        : socket_(socket)
        , params_(params)
        , expected_packet_count_(expected_packet_count)
    {
        ...
        data_.reset(static_cast<float*>(base::AlignedAlloc(packet_size_ + sizeof(AudioOutputBufferParameters), AudioBus::kChannelAlignment)));
    ...
    int OnMoreData(base::TimeDelta delay, base::TimeTicks delay_timestamp, const AudioGlitchInfo& glitch_info, AudioBus* dest) override
    {
        ...
        if (current_packet_count_ < expected_packet_count_) {
            ...
            output_buffer()->params.delay_timestamp_us = (delay_timestamp - base::TimeTicks()).InMicroseconds();
            const size_t span_size = static_cast<size_t>(packet_size_) / sizeof(decltype(*data_.get()));
            uint32_t size = socket_->Receive(base::as_writable_bytes(base::make_span(data_.get(), span_size)));
...
DWORD __stdcall SyncSocketThread(void* context)
{
    ...
    std::unique_ptr<float, base::AlignedFreeDeleter> data(static_cast<float*>(base::AlignedAlloc(ctx.packet_size_bytes, AudioBus::kChannelAlignment)));
    ...
    for (int ix = 0; ix < ctx.total_packets; ++ix) {
        ...
        base::TimeTicks delay_timestamp = base::TimeTicks() + base::Microseconds(ctx.buffer->params.delay_timestamp_us);
        ...
        ctx.socket->Send(UNSAFE_BUFFERS(base::make_span(reinterpret_cast<uint8_t*>(data.get()), ctx.packet_size_bytes)));
    }
...
TEST_F(WinAudioTest, SyncSocketBasic)
{
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Mono(), sample_rate, kSamples20ms);

    AudioOutputStream* oas = audio_manager_->MakeAudioOutputStream(params, std::string(), AudioManager::LogCallback());
    ASSERT_TRUE(NULL != oas);
    ...
    HANDLE thread = ::CreateThread(NULL, 0, SyncSocketThread, &thread_context, 0, NULL);
