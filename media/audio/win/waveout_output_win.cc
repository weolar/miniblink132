...
static const unsigned int kChannelsToMask[kMaxChannelsToMask + 1] = {
    ...
    // 8 = 7.1
    SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT | SPEAKER_FRONT_CENTER | SPEAKER_LOW_FREQUENCY | SPEAKER_BACK_LEFT | SPEAKER_BACK_RIGHT | SPEAKER_SIDE_LEFT
        | SPEAKER_SIDE_RIGHT
...
PCMWaveOutAudioOutputStream::PCMWaveOutAudioOutputStream(AudioManagerWin* manager, const AudioParameters& params, int num_buffers, UINT device_id)
...
bool PCMWaveOutAudioOutputStream::Open()
{
    ...
    MMRESULT result = ::waveOutOpen(
        &waveout_, device_id_, reinterpret_cast<LPCWAVEFORMATEX>(&format_), reinterpret_cast<DWORD_PTR>(buffer_event_.Get()), NULL, CALLBACK_EVENT);
...
void PCMWaveOutAudioOutputStream::Start(AudioSourceCallback* callback)
{
    ...
    if (!::RegisterWaitForSingleObject(&waiting_handle_, buffer_event_.Get(), &BufferCallback, this, INFINITE, WT_EXECUTEDEFAULT)) {
        HandleError(MMSYSERR_ERROR);
        waiting_handle_ = NULL;
        return;
    }
...
void PCMWaveOutAudioOutputStream::QueueNextPacket(WAVEHDR* buffer)
{
    ...
    const base::TimeDelta delay = base::Microseconds(pending_bytes_ * base::Time::kMicrosecondsPerSecond / format_.Format.nAvgBytesPerSec);
    int frames_filled = callback_->OnMoreData(delay, base::TimeTicks::Now(), {}, audio_bus_.get());
    ...
    if (used <= buffer_size_) {
        ...
        audio_bus_->ToInterleaved<SignedInt16SampleTypeTraits>(frames_filled, reinterpret_cast<int16_t*>(buffer->lpData));
...
void NTAPI PCMWaveOutAudioOutputStream::BufferCallback(PVOID lpParameter, BOOLEAN timer_fired)
{
    ...
    for (int ix = 0; ix != stream->num_buffers_; ++ix) {
        ...
        if (buffer->dwFlags & WHDR_DONE) {
            ...
            MMRESULT result = ::waveOutWrite(stream->waveout_, buffer, sizeof(WAVEHDR));
            if (result != MMSYSERR_NOERROR)
