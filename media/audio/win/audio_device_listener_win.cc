...
AudioDeviceListenerWin::AudioDeviceListenerWin(base::RepeatingClosure listener_cb)
    ...
    , tick_clock_(base::DefaultTickClock::GetInstance())
{
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "RegisterEndpointNotificationCallback failed: " << std::hex << hr;
        return;
    }
...
HRESULT AudioDeviceListenerWin::OnPropertyValueChanged(LPCWSTR device_id, const PROPERTYKEY key)
{
...
HRESULT AudioDeviceListenerWin::OnDeviceStateChanged(LPCWSTR device_id, DWORD new_state)
{
...
HRESULT AudioDeviceListenerWin::OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR new_default_device_id)
{
    ...
    if (flow == eRender && (now - last_device_change_time_ > kDeviceChangeLimit || new_device_id.compare(last_device_id_) != 0)) {
    ...
    DVLOG(1) << "OnDefaultDeviceChanged() "
             << "new_default_device: " << (new_default_device_id ? CoreAudioUtil::GetFriendlyName(new_device_id, flow, role) : "no device")
             << ", flow: " << FlowToString(flow) << ", role: " << RoleToString(role) << ", notified manager: " << (did_run_listener_cb ? "Yes" : "No");
