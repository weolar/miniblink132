...
{
    return media::CoreAudioUtil::IsSupported() && media::CoreAudioUtil::NumberOfActiveDevices(eRender) > 0;
}
...
public:
    AudioSessionEventListenerTest()
    {
        ...
        audio_client_ = CoreAudioUtil::CreateClient(std::string(), eRender, eConsole);
        CHECK(audio_client_);

        // AudioClient must be initialized to succeed in getting the session.
        WAVEFORMATEXTENSIBLE format;
        EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetSharedModeMixFormat(audio_client_.Get(), &format)));

        // Perform a shared-mode initialization without event-driven buffer
        // handling.
        uint32_t endpoint_buffer_size = 0;
        HRESULT hr = CoreAudioUtil::SharedModeInitialize(audio_client_.Get(), &format, nullptr, 0, &endpoint_buffer_size, nullptr);
        ...
        listener_
            = Microsoft::WRL::Make<AudioSessionEventListener>(base::BindOnce(&AudioSessionEventListenerTest::OnSessionDisconnected, base::Unretained(this)));

        if (audio_session_control_) {
            hr = audio_session_control_->RegisterAudioSessionNotification(listener_.Get());
            EXPECT_EQ(hr, S_OK);
        }
    }
    ...
    ~AudioSessionEventListenerTest() override
    {
        if (audio_session_control_ && listener_) {
            HRESULT hr = audio_session_control_->UnregisterAudioSessionNotification(listener_.Get());
            EXPECT_EQ(hr, S_OK);
        }
    }
