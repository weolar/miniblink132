...
TEST_F(CoreAudioUtilWinTest, WaveFormatWrapperExtended)
{
    ...
    EXPECT_EQ(wave_format_ex.GetExtensible()->Format.wFormatTag, WAVE_FORMAT_EXTENSIBLE);
...
TEST_F(CoreAudioUtilWinTest, CreateDeviceEnumerator)
{
    ...
    ComPtr<IMMDeviceEnumerator> enumerator = CoreAudioUtil::CreateDeviceEnumerator();
...
TEST_F(CoreAudioUtilWinTest, CreateDefaultDevice)
{
    ...
    struct {
        EDataFlow flow;
        ERole role;
    } data[] = { { eRender, eConsole }, { eRender, eCommunications }, { eRender, eMultimedia }, { eCapture, eConsole }, { eCapture, eCommunications },
        { eCapture, eMultimedia } };

    // Create default devices for all flow/role combinations above.
    ComPtr<IMMDevice> audio_device;
    for (size_t i = 0; i < std::size(data); ++i) {
        audio_device = CoreAudioUtil::CreateDevice(AudioDeviceDescription::kDefaultDeviceId, data[i].flow, data[i].role);
    ...
    audio_device = CoreAudioUtil::CreateDevice(AudioDeviceDescription::kDefaultDeviceId, eAll, eConsole);
...
TEST_F(CoreAudioUtilWinTest, CreateDevice)
{
    ...
    ComPtr<IMMDevice> default_render_device = CoreAudioUtil::CreateDevice(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    AudioDeviceName default_render_name;
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetDeviceName(default_render_device.Get(), &default_render_name)));

    // Use the unique ID as input to CreateDevice() and create a corresponding
    // IMMDevice.
    ComPtr<IMMDevice> audio_device = CoreAudioUtil::CreateDevice(default_render_name.unique_id, EDataFlow(), ERole());
    EXPECT_TRUE(audio_device.Get());

    // Verify that the two IMMDevice interfaces represents the same endpoint
    // by comparing their unique IDs.
    AudioDeviceName device_name;
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetDeviceName(audio_device.Get(), &device_name)));
...
TEST_F(CoreAudioUtilWinTest, GetDefaultDeviceName)
{
    ...
    struct {
        EDataFlow flow;
        ERole role;
    } data[] = { { eRender, eConsole }, { eRender, eCommunications }, { eCapture, eConsole }, { eCapture, eCommunications } };
...
TEST_F(CoreAudioUtilWinTest, GetAudioControllerID)
{
    ...
    for (size_t i = 0; i < std::size(flows); ++i) {
        ...
        ASSERT_TRUE(SUCCEEDED(enumerator->EnumAudioEndpoints(flows[i], DEVICE_STATE_ACTIVE, &collection)));
        UINT count = 0;
        collection->GetCount(&count);
        for (UINT j = 0; j < count; ++j) {
            ComPtr<IMMDevice> device;
            collection->Item(j, &device);
            std::string controller_id(CoreAudioUtil::GetAudioControllerID(device.Get(), enumerator.Get()));
...
TEST_F(CoreAudioUtilWinTest, GetFriendlyName)
{
    ...
    ComPtr<IMMDevice> audio_device = CoreAudioUtil::CreateDevice(AudioDeviceDescription::kDefaultDeviceId, eCapture, eConsole);
    AudioDeviceName device_name;
    ...
    std::string friendly_name = CoreAudioUtil::GetFriendlyName(device_name.unique_id, eCapture, eConsole);
    EXPECT_EQ(friendly_name, device_name.device_name);

    // Same test as above but for playback.
    audio_device = CoreAudioUtil::CreateDevice(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    hr = CoreAudioUtil::GetDeviceName(audio_device.Get(), &device_name);
    EXPECT_TRUE(SUCCEEDED(hr));
    friendly_name = CoreAudioUtil::GetFriendlyName(device_name.unique_id, eRender, eConsole);
    EXPECT_EQ(friendly_name, device_name.device_name);
}
...
TEST_F(CoreAudioUtilWinTest, CreateClient)
{
    ...
    for (size_t i = 0; i < std::size(data); ++i) {
        ComPtr<IAudioClient> client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, data[i], eConsole);
...
TEST_F(CoreAudioUtilWinTest, CreateClient3)
{
    ...
    for (size_t i = 0; i < std::size(data); ++i) {
        ComPtr<IAudioClient3> client3 = CoreAudioUtil::CreateClient3(AudioDeviceDescription::kDefaultDeviceId, data[i], eConsole);
...
TEST_F(CoreAudioUtilWinTest, GetSharedModeMixFormat)
{
    ...
    ComPtr<IAudioClient> client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    EXPECT_TRUE(client.Get());

    // Perform a simple sanity test of the acquired format structure.
    WAVEFORMATEXTENSIBLE format;
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetSharedModeMixFormat(client.Get(), &format)));
...
TEST_F(CoreAudioUtilWinTest, IsChannelLayoutSupported)
{
    ...
    HRESULT hr = CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, true, &mix_params);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_TRUE(mix_params.IsValid());
    EXPECT_TRUE(CoreAudioUtil::IsChannelLayoutSupported(std::string(), eRender, eConsole, mix_params.channel_layout()));
    ...
    if (mix_params.channel_layout() != CHANNEL_LAYOUT_STEREO) {
        ...
        // value here.
        EXPECT_FALSE(CoreAudioUtil::IsChannelLayoutSupported(std::string(), eRender, eConsole, channel_layout));
    }
}
...
TEST_F(CoreAudioUtilWinTest, GetDevicePeriod)
{
    ...
    for (size_t i = 0; i < std::size(data); ++i) {
        ...
        client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, data[i], eConsole);
        EXPECT_TRUE(client.Get());
        EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetDevicePeriod(client.Get(), AUDCLNT_SHAREMODE_SHARED, &shared_time_period)));
        EXPECT_GT(shared_time_period, 0);
        EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetDevicePeriod(client.Get(), AUDCLNT_SHAREMODE_EXCLUSIVE, &exclusive_time_period)));
...
TEST_F(CoreAudioUtilWinTest, GetPreferredAudioParameters)
{
    ...
    for (size_t i = 0; i < std::size(data); ++i) {
        ...
        EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, is_output_device, &params)));
        EXPECT_TRUE(params.IsValid());
        if (!is_output_device) {
            // Loopack devices are supported for input streams.
            EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kLoopbackInputDeviceId, is_output_device, &params)));
            EXPECT_TRUE(params.IsValid());
        }
    }
}
...
TEST_F(CoreAudioUtilWinTest, GetChannelConfig)
{
    ...
    for (auto data_flow : data_flows) {
        ChannelConfig config1 = CoreAudioUtil::GetChannelConfig(std::string(), data_flow);
        EXPECT_NE(config1, CHANNEL_LAYOUT_NONE);
        EXPECT_NE(config1, CHANNEL_LAYOUT_UNSUPPORTED);
        ChannelConfig config2 = CoreAudioUtil::GetChannelConfig(AudioDeviceDescription::kDefaultDeviceId, data_flow);
        ...
        if (data_flow == eCapture) {
            config1 = CoreAudioUtil::GetChannelConfig(AudioDeviceDescription::kLoopbackInputDeviceId, data_flow);
            config2 = CoreAudioUtil::GetChannelConfig(AudioDeviceDescription::kDefaultDeviceId, eRender);
            EXPECT_EQ(config1, config2);
        }
    }
}
...
TEST_F(CoreAudioUtilWinTest, SharedModeInitializeWithoutOffload)
{
    ...
    ComPtr<IAudioClient> client;
    client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    EXPECT_TRUE(client.Get());
    ...
    HRESULT hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_GT(endpoint_buffer_size, 0u);

    // It is only possible to create a client once.
    hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL);
    ...
    EXPECT_FALSE(CoreAudioUtil::IsFormatSupported(client.Get(), AUDCLNT_SHAREMODE_SHARED, &format));
    hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL);
    ...
    EXPECT_TRUE(client.Get());
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetSharedModeMixFormat(client.Get(), &format)));
    EXPECT_TRUE(CoreAudioUtil::IsFormatSupported(client.Get(), AUDCLNT_SHAREMODE_SHARED, &format));
    hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, event_handle.Get(), 0, &endpoint_buffer_size, NULL);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_GT(endpoint_buffer_size, 0u);
}
...
TEST_F(CoreAudioUtilWinTest, SharedModeInitializeWithOffload)
{
    ...
    uint32_t endpoint_buffer_size = 0;
    HRESULT hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL, kOffloadModeEnabled);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_GT(endpoint_buffer_size, 0u);

    // It is only possible to create a client once.
    hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL, kOffloadModeEnabled);
    EXPECT_FALSE(SUCCEEDED(hr));
    ...
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetSharedModeMixFormat(client.Get(), &format)));
    EXPECT_TRUE(CoreAudioUtil::IsFormatSupported(client.Get(), AUDCLNT_SHAREMODE_SHARED, &format));
    hr = CoreAudioUtil::SharedModeInitialize(client.Get(), &format, event_handle.Get(), 0, &endpoint_buffer_size, NULL, kOffloadModeEnabled);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_GT(endpoint_buffer_size, 0u);
}
...
TEST_F(CoreAudioUtilWinTest, CreateRenderAndCaptureClients)
{
    ...
    for (size_t i = 0; i < std::size(data); ++i) {
        ...
        if (data[i] == eRender) {
            ...
            CoreAudioUtil::SharedModeInitialize(client.Get(), &format, NULL, 0, &endpoint_buffer_size, NULL);
            render_client = CoreAudioUtil::CreateRenderClient(client.Get());
...
TEST_F(CoreAudioUtilWinTest, FillRenderEndpointBufferWithSilence)
{
    ...
    ComPtr<IAudioRenderClient> render_client(CoreAudioUtil::CreateRenderClient(client.Get()));
    EXPECT_TRUE(render_client.Get());
    ...
    // buffer.
    EXPECT_TRUE(CoreAudioUtil::FillRenderEndpointBufferWithSilence(client.Get(), render_client.Get()));
    client->GetCurrentPadding(&num_queued_frames);
...
TEST_F(CoreAudioUtilWinTest, GetMatchingOutputDeviceID)
{
    ...
    // the associated device.
    ComPtr<IMMDeviceCollection> collection;
    ASSERT_TRUE(SUCCEEDED(enumerator->EnumAudioEndpoints(eCapture, DEVICE_STATE_ACTIVE, &collection)));
    UINT count = 0;
    collection->GetCount(&count);
...
TEST_F(CoreAudioUtilWinTest, SharedModeLowerBufferSize)
{
    ...
    AudioParameters params;
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, true, &params)));
    ...
    ABORT_AUDIO_TEST_IF_NOT(hardware_capabilities.min_frames_per_buffer < params.frames_per_buffer());

    ComPtr<IAudioClient> default_client;
    default_client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    EXPECT_TRUE(default_client.Get());

    WAVEFORMATEXTENSIBLE format;
    EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetSharedModeMixFormat(default_client.Get(), &format)));

    uint32_t default_endpoint_buffer_size = 0;
    HRESULT hr = CoreAudioUtil::SharedModeInitialize(default_client.Get(), &format, NULL, 0, &default_endpoint_buffer_size, NULL);
    EXPECT_TRUE(SUCCEEDED(hr));
    EXPECT_GT(default_endpoint_buffer_size, 0u);

    ComPtr<IAudioClient> low_latency_client;
    low_latency_client = CoreAudioUtil::CreateClient(AudioDeviceDescription::kDefaultDeviceId, eRender, eConsole);
    EXPECT_TRUE(low_latency_client.Get());

    uint32_t low_latency_endpoint_buffer_size = 0;
    hr = CoreAudioUtil::SharedModeInitialize(
        low_latency_client.Get(), &format, NULL, hardware_capabilities.min_frames_per_buffer, &low_latency_endpoint_buffer_size, NULL);
    EXPECT_TRUE(SUCCEEDED(hr));
