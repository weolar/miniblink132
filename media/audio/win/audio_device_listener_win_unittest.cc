...
class AudioDeviceListenerWinTest : public testing::Test, public base::SystemMonitor::DevicesChangedObserver {
public:
    AudioDeviceListenerWinTest()
    {
        ...
        output_device_listener_
            = std::make_unique<AudioDeviceListenerWin>(base::BindRepeating(&AudioDeviceListenerWinTest::OnDeviceChange, base::Unretained(this)));
    ...
    AudioDeviceListenerWinTest& operator=(const AudioDeviceListenerWinTest&) = delete;
    ...
    {
        tick_clock_.Advance(AudioDeviceListenerWin::kDeviceChangeLimit + base::Milliseconds(1));
    }

    // Simulate a device change where no output devices are available.
    bool SimulateNullDefaultOutputDeviceChange()
    {
        auto result = output_device_listener_->OnDefaultDeviceChanged(static_cast<EDataFlow>(eConsole), static_cast<ERole>(eRender), nullptr);
    ...
    bool SimulateDefaultOutputDeviceChange(const char* new_device_id)
    {
        auto result = output_device_listener_->OnDefaultDeviceChanged(
            static_cast<EDataFlow>(eConsole), static_cast<ERole>(eRender), base::ASCIIToWide(new_device_id).c_str());
        task_environment_.RunUntilIdle();
        return result == S_OK;
    }
...
TEST_F(AudioDeviceListenerWinTest, OutputDeviceChange)
{
    ...
    EXPECT_CALL(*this, OnDevicesChanged(base::SystemMonitor::DEVTYPE_AUDIO)).Times(1);
