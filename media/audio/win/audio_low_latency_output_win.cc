...
WASAPIAudioOutputStream::WASAPIAudioOutputStream(
    AudioManagerWin* manager, const std::string& device_id, const AudioParameters& params, ERole device_role, AudioManager::LogCallback log_callback)
    ...
    , log_callback_(std::move(log_callback))
{
    ...
    SendLogMessage(
        "%s({device_id=%s}, {params=[%s]}, {role=%s})", __func__, device_id.c_str(), params.AsHumanReadableString().c_str(), RoleToString(device_role));
    ...
    AudioParameters::HardwareCapabilities hardware_capabilities = params.hardware_capabilities().value_or(AudioParameters::HardwareCapabilities());
    ...
    // IAudioClient API.
    if (params.frames_per_buffer() != hardware_capabilities.default_frames_per_buffer
        && params.frames_per_buffer() == hardware_capabilities.min_frames_per_buffer) {
        requested_iaudioclient3_buffer_size_ = hardware_capabilities.min_frames_per_buffer;
    }
...
bool WASAPIAudioOutputStream::Open()
{
    ...
    if (enable_audio_offload_ && (params_.latency_tag() != AudioLatency::Type::kPlayback || params_.IsBitstreamFormat())) {
        ...
        SendLogMessage("%s => (INFO: Not enrolling into audio offload for stream without "
                       "latency tag set to kPlayback, or the stream is in bitstream format.",
            __func__);
        return false;
    }

    const bool communications_device = device_id_.empty() ? (device_role_ == eCommunications) : false;

    Microsoft::WRL::ComPtr<IAudioClient> audio_client(CoreAudioUtil::CreateClient(device_id_, eRender, device_role_));
    if (!audio_client.Get()) {
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED && enable_audio_offload_) {
        enable_audio_offload_ = CoreAudioUtil::EnableOffloadForClient(audio_client.Get());
        if (!enable_audio_offload_) {
            SendLogMessage("%s => (INFO: Not enrolling into audio offload.", __func__);
    ...
    if (!CoreAudioUtil::IsFormatSupported(audio_client.Get(), share_mode_, &format_)) {
        RecordAudioFailure(kOpenFailureHistogram, GetLastError());
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED) {
        ...
        hr = CoreAudioUtil::SharedModeInitialize(audio_client.Get(), &format_, audio_samples_render_event_.Get(), requested_iaudioclient3_buffer_size_,
            &endpoint_buffer_size_frames_, communications_device ? &kCommunicationsSessionId : nullptr, enable_audio_offload_);
        if (FAILED(hr)) {
            RecordAudioFailure(kOpenFailureHistogram, hr);
            SendLogMessage("%s => (ERROR: IAudioClient::SharedModeInitialize=[%s])", __func__, ErrorToString(hr).c_str());
        ...
        if (FAILED(CoreAudioUtil::GetDevicePeriod(audio_client.Get(), AUDCLNT_SHAREMODE_SHARED, &device_period))) {
            RecordAudioFailure(kOpenFailureHistogram, GetLastError());
            return false;
        }
        ...
        if (enable_audio_offload_) {
            ...
            if (packet_size_frames_ != preferred_frames_per_buffer) {
                SendLogMessage("%s => (INFO: Requested buffer size in frames mismatch. "
            ...
            preferred_frames_per_buffer
                = AudioTimestampHelper::TimeToFrames(CoreAudioUtil::ReferenceTimeToTimeDelta(device_period), format_.Format.nSamplesPerSec);
        }

        SendLogMessage("%s => (preferred_frames_per_buffer=[%d audio frames])", __func__, preferred_frames_per_buffer);
        ...
        if (preferred_frames_per_buffer % packet_size_frames_) {
            SendLogMessage("%s => (WARNING: Using output audio with a non-optimal buffer size)", __func__);
        }
    } else {
        SendLogMessage("%s => (WARNING: Using exclusive mode can lead to bad performance)", __func__);
        // TODO(henrika): break out to CoreAudioUtil::ExclusiveModeInitialize()
        // when removing the enable-exclusive-audio flag.
        hr = ExclusiveModeInitialization(audio_client.Get(), audio_samples_render_event_.Get(), &endpoint_buffer_size_frames_);
        if (FAILED(hr))
            return false;
    ...
    Microsoft::WRL::ComPtr<IAudioRenderClient> audio_render_client = CoreAudioUtil::CreateRenderClient(audio_client.Get());
    ...
    if (FAILED(hr)) {
        RecordAudioFailure(kOpenFailureHistogram, hr);
        SendLogMessage("%s => (ERROR: IAudioClient::GetService(IAudioClock)=[%s])", __func__, ErrorToString(hr).c_str());
        return false;
    }
...
void WASAPIAudioOutputStream::Start(AudioSourceCallback* callback)
{
    ...
    CHECK(callback);
    CHECK(opened_);
    SendLogMessage("%s([opened=%s, started=%s])", __func__, opened_ ? "true" : "false", render_thread_ ? "true" : "false");
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED) {
        if (!CoreAudioUtil::FillRenderEndpointBufferWithSilence(audio_client_.Get(), audio_render_client_.Get())) {
            // Failed to prepare endpoint buffers with silence. Attempting recovery
            // with a new IAudioClient and IAudioRenderClient."
            SendLogMessage("%s => (WARNING: CAU::FillRenderEndpointBufferWithSilence failed)", __func__);
            opened_ = false;
            audio_client_.Reset();
            audio_render_client_.Reset();
            if (!Open() || !CoreAudioUtil::FillRenderEndpointBufferWithSilence(audio_client_.Get(), audio_render_client_.Get())) {
    ...
    peak_detector_ = std::make_unique<AmplitudePeakDetector>(base::BindRepeating(&AudioManager::TraceAmplitudePeak, base::Unretained(manager_),
        /*trace_start=*/false));

    // Create and start the thread that will drive the rendering by waiting for
    // render events.
    render_thread_ = std::make_unique<base::DelegateSimpleThread>(this, "wasapi_render_thread", base::SimpleThread::Options(base::ThreadType::kRealtimeAudio));
    render_thread_->Start();
    if (!render_thread_->HasBeenStarted()) {
        RecordAudioFailure(kStartFailureHistogram, GetLastError());
        SendLogMessage("%s => (ERROR: Failed to start \"wasapi_render_thread\")", __func__);
        StopThread();
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
        return;
    }
...
void WASAPIAudioOutputStream::Stop()
{
    ...
    DCHECK_EQ(GetCurrentThreadId(), creating_thread_id_);
    SendLogMessage("%s([started=%s])", __func__, render_thread_ ? "true" : "false");
    ...
    if (FAILED(hr)) {
        RecordAudioFailure(kStopFailureHistogram, hr);
        SendLogMessage("%s => (ERROR: IAudioClient::Reset=[%s])", __func__, ErrorToString(hr).c_str());
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
    }
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED) {
        ...
        if (num_queued_frames > 0) {
            SendLogMessage("%s => (WARNING: Buffers are not cleared correctly)", __func__);
        }
    }
}
...
void WASAPIAudioOutputStream::SendLogMessage(const char* format, ...)
{
    ...
    log_callback_.Run("WAOS::" + base::StringPrintV(format, args) + base::StringPrintf(" [this=0x%" PRIXPTR "]", reinterpret_cast<uintptr_t>(this)));
    va_end(args);
}
...
void WASAPIAudioOutputStream::Run()
{
    ...
    if (!mmcss_is_ok) {
        ...
        LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: Failed to enable MMCSS (error code=" << err << "))";
    }
    ...
    bool playing = true;
    bool error = false;
    HANDLE wait_array[] = { stop_render_event_.Get(), audio_samples_render_event_.Get() };
    ...
    if (error) {
        ...
        LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: IAudioClock::GetFrequency=[" << ErrorToString(hr).c_str() << "])";
    }
    ...
    if (playing && error) {
        ...
        LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: WASAPI rendering failed)";
    ...
    if (mm_task && !avrt::AvRevertMmThreadCharacteristics(mm_task)) {
        LOG(WARNING) << "WAOS::" << __func__ << " => (WARNING: Failed to disable MMCSS)";
    }
}

bool WASAPIAudioOutputStream::RenderAudioFromSource(UINT64 device_frequency)
{
    TRACE_EVENT("audio", "RenderAudioFromSource", [&](perfetto::EventContext ctx) {
    ...
    const base::TimeDelta buffer_duration = media::AudioTimestampHelper::FramesToTime(packet_size_frames_, format_.Format.nSamplesPerSec);
    HRESULT hr = S_FALSE;
    UINT32 num_queued_frames = 0;
    ...
    if (share_mode_ == AUDCLNT_SHAREMODE_SHARED) {
        ...
        if (FAILED(hr)) {
            ...
            LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: IAudioClient::GetCurrentPadding=[" << ErrorToString(hr).c_str() << "])";
            return false;
        }
        TRACE_COUNTER_ID1(TRACE_DISABLED_BY_DEFAULT("audio"), "IAudioClient_queued_frames", this, num_queued_frames);
    ...
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "IAudioClient frames", [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_win_render_audio_from_source();
    ...
    for (size_t n = 0; n < num_packets; ++n) {
        TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "Write packet", [&](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_win_render_audio_from_source();
        ...
        if (FAILED(hr)) {
            RecordAudioFailure(kRenderFailureHistogram, hr);
            LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: IAudioRenderClient::GetBuffer=[" << ErrorToString(hr).c_str() << "])";
            return false;
        }
        ...
        if (SUCCEEDED(hr)) {
            TRACE_EVENT_BEGIN(TRACE_DISABLED_BY_DEFAULT("audio"), "IAudioClock position", [&](perfetto::EventContext ctx) {
                auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                auto* data = event->set_win_render_audio_from_source();
            ...
            if (last_position_ != 0) {
                ...
                if (position < last_position_) {
                    ...
                    TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("audio"), "position decrease", TRACE_EVENT_SCOPE_THREAD);
                }
                ...
                // case or not.
                base::TimeDelta position_time_increase = media::AudioTimestampHelper::FramesToTime(position - last_position_, device_frequency);
                ...
                base::TimeDelta qpc_position_time_increase
                    = qpc_position < last_qpc_position_ ? base::TimeDelta() : base::Microseconds((qpc_position - last_qpc_position_) / 10);

                TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("audio"), "gap estimation", [&](perfetto::EventContext ctx) {
                    auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                    auto* data = event->set_win_render_audio_from_source();
                    data->set_iaudioclock_stream_position_increase_ms(position_time_increase.InMilliseconds());
                    data->set_iaudioclock_qpc_position_increase_ms(qpc_position_time_increase.InMilliseconds());
                });
                ...
                // glitch.
                base::TimeDelta gap_duration = qpc_position_time_increase - position_time_increase;
                ...
                glitch_reporter_.UpdateStats(is_glitch ? gap_duration : base::TimeDelta());
                if (is_glitch) {
                    TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("audio"), "glitch", TRACE_EVENT_SCOPE_THREAD);
                    glitch_info_accumulator.Add(AudioGlitchInfo::SingleBoundedSystemGlitch(gap_duration, AudioGlitchInfo::Direction::kRender));
                }
            }
            ...
            // entirely.
            const uint64_t delay_frames = num_written_frames_ > played_out_frames ? num_written_frames_ - played_out_frames : 0;

            // Convert the delay from frames to time.
            delay = media::AudioTimestampHelper::FramesToTime(delay_frames, format_.Format.nSamplesPerSec);
            ...
            RecordAudioFailure(kRenderFailureHistogram, hr);
            LOG(ERROR) << "WAOS::" << __func__ << " => (ERROR: IAudioClock::GetPosition=[" << ErrorToString(hr).c_str() << "])";
            // Use a delay of zero.
            delay_timestamp = base::TimeTicks::Now();
        }

        UMA_HISTOGRAM_COUNTS_1000("Media.Audio.Render.SystemDelay", delay.InMilliseconds());
...
#if BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
        if (params_.format() == AudioParameters::AUDIO_BITSTREAM_DTS) {
            std::unique_ptr<AudioBus> audio_bus(AudioBus::WrapMemory(params_, audio_data));
            audio_bus_->set_is_bitstream_format(true);
            int frames_filled = source_->OnMoreData(BoundedDelay(delay), delay_timestamp, glitch_info_accumulator.GetAndReset(), audio_bus.get());
...
#endif // BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
        int frames_filled = source_->OnMoreData(BoundedDelay(delay), delay_timestamp, glitch_info_accumulator.GetAndReset(), audio_bus_.get());
        uint32_t num_filled_bytes = frames_filled * format_.Format.nBlockAlign;
        DCHECK_LE(num_filled_bytes, packet_size_bytes_);
        audio_bus_->Scale(volume_);

        if (enable_audio_offload_) {
            audio_bus_->ToInterleaved<SignedInt16SampleTypeTraits>(frames_filled, reinterpret_cast<short*>(audio_data));
        } else {
            // We skip clipping since that occurs at the shared memory boundary.
            audio_bus_->ToInterleaved<Float32SampleTypeTraitsNoClip>(frames_filled, reinterpret_cast<float*>(audio_data));
        }
...
HRESULT WASAPIAudioOutputStream::ExclusiveModeInitialization(IAudioClient* client, HANDLE event_handle, uint32_t* endpoint_buffer_size)
{
    ...
    hr = client->Initialize(
        AUDCLNT_SHAREMODE_EXCLUSIVE, stream_flags, requested_buffer_duration, requested_buffer_duration, reinterpret_cast<WAVEFORMATEX*>(&format_), nullptr);
    if (FAILED(hr)) {
        if (hr == AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED) {
            ...
            // is 100 nanoseconds.
            REFERENCE_TIME aligned_buffer_duration = static_cast<REFERENCE_TIME>((10000000.0 * aligned_buffer_size / format_.Format.nSamplesPerSec) + 0.5);
            ...
            DVLOG(1) << "aligned_buffer_duration: " << static_cast<double>(aligned_buffer_duration / 10000.0) << " [ms]";
...
void WASAPIAudioOutputStream::StartAudioSessionEventListener()
{
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to get IAudioSessionControl service: " << std::hex << hr;
        return;
    }

    session_listener_ = Microsoft::WRL::Make<AudioSessionEventListener>(
        base::BindPostTaskToCurrentDefault(base::BindOnce(&WASAPIAudioOutputStream::OnDeviceChanged, weak_factory_.GetWeakPtr())));

    hr = audio_session_control_->RegisterAudioSessionNotification(session_listener_.Get());

    DLOG_IF(ERROR, FAILED(hr)) << "IAudioSessionControl::RegisterAudioSessionNotification() failed: " << std::hex << hr;
}
...
void WASAPIAudioOutputStream::StopAudioSessionEventListener()
{
    ...
    HRESULT hr = audio_session_control_->UnregisterAudioSessionNotification(session_listener_.Get());

    DLOG_IF(ERROR, FAILED(hr)) << "IAudioSessionControl::UnregisterAudioSessionNotification() failed: " << std::hex << hr;
...
#endif // BUILDFLAG(ENABLE_PLATFORM_DTS_AUDIO)
    SendLogMessage("%s => (audio engine format=[%s])", __func__, CoreAudioUtil::WaveFormatToString(&format_).c_str());

    SendLogMessage("%s => (packet size=[%zu bytes/%zu audio frames/%.3f ms])", __func__, packet_size_bytes_, packet_size_frames_,
