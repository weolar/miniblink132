...
{
    base::UmaHistogramBoolean("Media.Audio.Capture.Win.VolumeRangeAvailable", range.has_value());
    if (!range.has_value()) {
        return;
    }
    // Count the number of steps before the range is modified.
    constexpr int kMaxVolumeSteps = 2000;
    const int num_steps = std::min(CountVolumeRangeIntervals(*range), kMaxVolumeSteps);
    ...
    base::UmaHistogramExactLinear("Media.Audio.Capture.Win.VolumeRangeMin2", static_cast<int>(range->min_volume_db), kExclusiveMaxVolumeDb);
    DCHECK_GE(range->max_volume_db, 0);
    base::UmaHistogramExactLinear("Media.Audio.Capture.Win.VolumeRangeMax2", static_cast<int>(range->max_volume_db), kExclusiveMaxVolumeDb);
    ...
    base::UmaHistogramCustomCounts("Media.Audio.Capture.Win.VolumeRangeNumSteps",
        /*sample=*/num_steps,
        /*min=*/1, /*max=*/kMaxVolumeSteps, kStepsNumBuckets);
}
