...
public:
    FakeAudioInputCallback()
        ...
        , data_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , error_(false)
    {
    }
    ...
    int num_received_audio_frames() const
    {
        return num_received_audio_frames_;
    }
...
static bool HasCoreAudioAndInputDevices(AudioManager* audio_man)
{
    ...
    return CoreAudioUtil::IsSupported() && AudioDeviceInfoAccessorForTests(audio_man).HasAudioInputDevices();
}
...
public:
    ...
    {
        EXPECT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(device_id_, false, &default_params_)));
        EXPECT_EQ(format(), AudioParameters::AUDIO_PCM_LOW_LATENCY);
        frames_per_buffer_ = default_params_.frames_per_buffer();
    }

    AudioInputStreamWrapper(AudioManager* audio_manager, const AudioParameters& default_params)
        : audio_man_(audio_manager)
    ...
    AudioInputStreamWrapper(AudioManager* audio_manager, const std::string& device_id)
        : audio_man_(audio_manager)
    ...
    AudioParameters::Format format() const
    {
        return default_params_.format();
    }
    int channels() const
    {
...
private:
    AudioInputStream* CreateInputStream()
    {
        ...
        AudioInputStream* ais = audio_man_->MakeAudioInputStream(params, device_id_, base::BindRepeating(&LogCallbackDummy));
        EXPECT_TRUE(ais);
        return ais;
    }
...
static AudioInputStream* CreateDefaultAudioInputStream(AudioManager* audio_manager)
{
...
class WinAudioInputTest : public ::testing::Test, public ::testing::WithParamInterface<bool> {
public:
    WinAudioInputTest()
    {
...
TEST_F(WinAudioInputTest, WASAPIAudioInputStreamHardwareSampleRate)
{
    ...
    AudioDeviceInfoAccessorForTests(audio_manager_.get()).GetAudioInputDeviceDescriptions(&device_descriptions);
    ...
    for (const auto& device : device_descriptions) {
        ...
        ASSERT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(device.unique_id, false, &params)));
...
TEST_F(WinAudioInputTest, WASAPIAudioInputStreamEffects)
{
    ...
    ABORT_AUDIO_TEST_IF_NOT(device_info_accessor.HasAudioInputDevices() && device_info_accessor.HasAudioOutputDevices() && CoreAudioUtil::IsSupported());
    ...
    for (const auto& device : device_descriptions) {
        AudioParameters params = device_info_accessor.GetInputStreamParameters(device.unique_id);
    ...
    AudioParameters params = device_info_accessor.GetInputStreamParameters(AudioDeviceDescription::kLoopbackInputDeviceId);
    EXPECT_EQ(params.effects(), AudioParameters::NO_EFFECTS);

    params = device_info_accessor.GetInputStreamParameters(AudioDeviceDescription::kLoopbackWithMuteDeviceId);
    EXPECT_EQ(params.effects(), AudioParameters::NO_EFFECTS);
}
...
{
    ABORT_AUDIO_TEST_IF_NOT(HasCoreAudioAndInputDevices(audio_manager_.get()));
    ScopedAudioInputStream ais(CreateDefaultAudioInputStream(audio_manager_.get()));
    ais.Close();
}
...
{
    AudioDeviceInfoAccessorForTests device_info_accessor(audio_manager_.get());
    ABORT_AUDIO_TEST_IF_NOT(device_info_accessor.HasAudioInputDevices() && CoreAudioUtil::IsSupported());
...
TEST_F(WinAudioInputTest, WASAPIAudioInputStreamHistograms)
{
    ...
    ais->Stop();
    ais.Close();
    histogram_tester.ExpectTotalCount("Media.Audio.Capture.EarlyGlitchDetected", 1);
}
...
TEST_F(WinAudioInputTest, WASAPIAudioInputStreamTestPacketSizes)
{
...

    {
        ...
        EXPECT_CALL(sink, OnData(NotNull(), _, _, _))
            .Times(AtLeast(10))
            .WillRepeatedly(CheckCountAndPostQuitTask(&count, 10, task_environment_.GetMainThreadTaskRunner(), run_loop.QuitWhenIdleClosure()));
        ais->Start(&sink);
        run_loop.Run();
        ais->Stop();
    }
...
{
    AudioDeviceInfoAccessorForTests device_info_accessor(audio_manager_.get());
    ABORT_AUDIO_TEST_IF_NOT(device_info_accessor.HasAudioOutputDevices() && CoreAudioUtil::IsSupported());
    AudioParameters params = device_info_accessor.GetInputStreamParameters(AudioDeviceDescription::kLoopbackInputDeviceId);
    EXPECT_EQ(params.effects(), AudioParameters::NO_EFFECTS);

    AudioParameters output_params = device_info_accessor.GetOutputStreamParameters(std::string());
    EXPECT_EQ(params.sample_rate(), output_params.sample_rate());
    EXPECT_EQ(params.channel_layout(), output_params.channel_layout());

    ScopedAudioInputStream stream(
        audio_manager_->MakeAudioInputStream(params, AudioDeviceDescription::kLoopbackInputDeviceId, base::BindRepeating(&LogCallbackDummy)));
...
TEST_F(WinAudioInputTest, DISABLED_WASAPIAudioInputStreamResampleToFile)
{
    ...
    for (const auto& test : tests) {
        AudioParameters params;
        ASSERT_TRUE(SUCCEEDED(CoreAudioUtil::GetPreferredAudioParameters(AudioDeviceDescription::kDefaultDeviceId, false, &params)));

        VLOG(0) << ">> Hardware sample rate: " << params.sample_rate() << " [Hz]";
        VLOG(0) << ">> Hardware channel layout: " << ChannelLayoutToString(params.channel_layout());
        ...
        std::string file_name(
            base::StringPrintf("resampled_10sec_%i_to_%i_%s.pcm", hw_sample_rate, params.sample_rate(), ChannelLayoutToString(params.channel_layout())));
        ...
        VLOG(0) << ">> New layout will be: " << ChannelLayoutToString(params.channel_layout());
