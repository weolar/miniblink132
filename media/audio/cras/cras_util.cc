...
{
    return type == kInternalSpeaker || type == kHeadphone || type == kHDMI || type == kLineout || type == kMic || type == kInternalMic || type == kFrontMic
        || type == kRearMic || type == kBluetoothNBMic || type == kUSB || type == kBluetooth || type == kAlsaLoopback;
}
    ...
    : type(type)
{
    ...
    rc = libcras_node_info_get_max_supported_channels(node, &max_supported_channels);
    if (rc) {
...
CrasDevice::CrasDevice(DeviceType type, uint64_t id, uint32_t dev_idx, uint32_t max_supported_channels, bool plugged, bool active, std::string node_type,
    std::string name, std::string dev_name)
    : type(type)
    , id(id)
    , dev_idx(dev_idx)
...
void mergeDevices(CrasDevice& old_dev, CrasDevice& new_dev)
{
    if (old_dev.node_type == kLineout || new_dev.node_type == kLineout) {
        ...
        old_dev.node_type = "";
    } else if (old_dev.node_type == kInternalSpeaker || new_dev.node_type == kInternalSpeaker) {
        old_dev.name = kInternalOutputVirtualDevice;
        old_dev.node_type = "";
    } else if (IsInternalMic(old_dev.node_type) || IsInternalMic(new_dev.node_type)) {
...
bool CrasUtil::CacheEffects()
{
    ...
    if (base::FeatureList::IsEnabled(media::kCrOSSystemVoiceIsolationOption)) {
        if (libcras_client_get_voice_isolation_supported(client, &voice_isolation_supported_) < 0) {
    ...
    if (libcras_client_get_default_output_buffer_size(client, &default_output_buffer_size_) < 0) {
...
std::vector<CrasDevice> CrasUtil::CrasGetAudioDevices(DeviceType type)
{
    ...
    if (type == DeviceType::kInput) {
        rc = libcras_client_get_nodes(client, CRAS_STREAM_INPUT, &nodes, &num_nodes);
    } else {
        rc = libcras_client_get_nodes(client, CRAS_STREAM_OUTPUT, &nodes, &num_nodes);
    }
