...
const AudioDevice* GetDeviceFromId(const AudioDeviceList& devices, uint64_t id)
{
...
void ProcessVirtualDeviceName(AudioDeviceNames* device_names, const AudioDeviceList& device_list)
{
    DCHECK_EQ(2U, device_list.size());
    if (device_list[0].type == chromeos::AudioDeviceType::kLineout || device_list[1].type == chromeos::AudioDeviceType::kLineout) {
        device_names->emplace_back(kHeadphoneLineOutVirtualDevice, base::NumberToString(device_list[0].id));
    } else if (device_list[0].type == chromeos::AudioDeviceType::kInternalSpeaker || device_list[1].type == chromeos::AudioDeviceType::kInternalSpeaker) {
        device_names->emplace_back(kInternalOutputVirtualDevice, base::NumberToString(device_list[0].id));
    } else {
        DCHECK(device_list[0].IsInternalMic() || device_list[1].IsInternalMic());
        device_names->emplace_back(kInternalInputVirtualDevice, base::NumberToString(device_list[0].id));
    }
}
...
// the board.
void RetrieveSystemEffectFeatures(bool& enforce_system_aec, bool& enforce_system_ns, bool& enforce_system_agc, bool& tuned_system_aec_allowed)
{
    const bool enforce_system_aec_ns_agc_feature = base::FeatureList::IsEnabled(features::kCrOSEnforceSystemAecNsAgc);
    const bool enforce_system_aec_ns_feature = base::FeatureList::IsEnabled(features::kCrOSEnforceSystemAecNs);
    const bool enforce_system_aec_agc_feature = base::FeatureList::IsEnabled(features::kCrOSEnforceSystemAecAgc);
    const bool enforce_system_aec_feature = base::FeatureList::IsEnabled(features::kCrOSEnforceSystemAec);

    enforce_system_aec = enforce_system_aec_feature || enforce_system_aec_ns_agc_feature || enforce_system_aec_ns_feature || enforce_system_aec_agc_feature;
    enforce_system_ns = enforce_system_aec_ns_agc_feature || enforce_system_aec_ns_feature;
    enforce_system_agc = enforce_system_aec_ns_agc_feature || enforce_system_aec_agc_feature;

    tuned_system_aec_allowed = base::FeatureList::IsEnabled(features::kCrOSSystemAEC);
}
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kCrOSSystemAECDeactivatedGroups, std::to_string(aec_group_id), false);
}
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kCrOSDspBasedAecDeactivatedGroups, std::to_string(aec_group_id), false)
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kCrOSDspBasedNsDeactivatedGroups, std::to_string(aec_group_id), false)
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kCrOSDspBasedAgcDeactivatedGroups, std::to_string(aec_group_id), false)
...
void SetAllowedDspBasedEffects(int aec_group_id, AudioParameters& params)
{
    ...
    if ((effects & AudioParameters::ECHO_CANCELLER) && !IsDspBasedAecDeactivated(aec_group_id)) {
    ...
    if ((effects & AudioParameters::NOISE_SUPPRESSION) && !IsDspBasedNsDeactivated(aec_group_id)) {
    ...
    if ((effects & AudioParameters::AUTOMATIC_GAIN_CONTROL) && !IsDspBasedAgcDeactivated(aec_group_id)) {
...
AudioManagerChromeOS::AudioManagerChromeOS(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
    : AudioManagerCrasBase(std::move(audio_thread), audio_log_factory)
    , on_shutdown_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
...
void AudioManagerChromeOS::GetAudioDeviceNamesImpl(bool is_input, AudioDeviceNames* device_names)
{
    ...
    for (const auto& [dev_idx, device_list] : dev_idx_map) {
        if (1 == device_list.size()) {
            ...
            device_names->emplace_back(device.display_name, base::NumberToString(device.id));
        } else {
...
void AudioManagerChromeOS::GetAudioInputDeviceNames(AudioDeviceNames* device_names)
{
    GetAudioDeviceNamesImpl(true, device_names);
}

void AudioManagerChromeOS::GetAudioOutputDeviceNames(AudioDeviceNames* device_names)
{
    GetAudioDeviceNamesImpl(false, device_names);
}

AudioParameters AudioManagerChromeOS::GetInputStreamParameters(const std::string& device_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());

    // Retrieve buffer size.
    int user_buffer_size = GetUserBufferSize();
    user_buffer_size = user_buffer_size != 0 ? user_buffer_size : kDefaultInputBufferSize;

    // Retrieve the board support in terms of APM effects and properties.
    const SystemAudioProcessingInfo system_apm_info = GetSystemApmEffectsSupportedPerBoard();
...
std::string AudioManagerChromeOS::GetAssociatedOutputDeviceID(const std::string& input_device_id)
{
    AudioDeviceList devices;
    GetAudioDevices(&devices);
    ...
    const std::string device_name = GetHardwareDeviceFromDeviceId(devices, true, input_device_id);
    ...
    auto output_device_it
        = base::ranges::find_if(devices, [device_name](const AudioDevice& device) { return !device.is_input && device.device_name == device_name; });
    return output_device_it == devices.end() ? "" : base::NumberToString(output_device_it->id);
}
...
std::string AudioManagerChromeOS::GetGroupIDOutput(const std::string& output_device_id)
{
    AudioDeviceList devices;
    GetAudioDevices(&devices);
...
std::string AudioManagerChromeOS::GetGroupIDInput(const std::string& input_device_id)
{
    AudioDeviceList devices;
    GetAudioDevices(&devices);
...
int AudioManagerChromeOS::GetDefaultOutputBufferSizePerBoard()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &AudioManagerChromeOS::GetDefaultOutputBufferSizeOnMainThread, weak_this_, base::Unretained(&buffer_size), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
    return static_cast<int>(buffer_size);
}

AudioManagerChromeOS::SystemAudioProcessingInfo AudioManagerChromeOS::GetSystemApmEffectsSupportedPerBoard()
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    base::WaitableEvent event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &AudioManagerChromeOS::GetSystemApmEffectsSupportedOnMainThread, weak_this_, base::Unretained(&system_apm_info), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
    return system_apm_info;
}

AudioParameters AudioManagerChromeOS::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    if (input_params.IsValid()) {
        ...
        if (!buffer_size) // Not user-provided.
            buffer_size = std::min(
                static_cast<int>(limits::kMaxAudioBufferSize), std::max(static_cast<int>(limits::kMinAudioBufferSize), input_params.frames_per_buffer()));
        return AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config, sample_rate, buffer_size,
            AudioParameters::HardwareCapabilities(limits::kMinAudioBufferSize, limits::kMaxAudioBufferSize));
    }
...
bool AudioManagerChromeOS::IsDefault(const std::string& device_id, bool is_input)
{
    return AudioDeviceDescription::IsDefaultDevice(device_id);
}

std::string AudioManagerChromeOS::GetHardwareDeviceFromDeviceId(const AudioDeviceList& devices, bool is_input, const std::string& device_id)
{
    uint64_t u64_device_id = 0;
    if (AudioDeviceDescription::IsDefaultDevice(device_id)) {
        u64_device_id = is_input ? GetPrimaryActiveInputNode() : GetPrimaryActiveOutputNode();
    } else {
...
void AudioManagerChromeOS::GetAudioDevices(AudioDeviceList* devices)
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetAudioDevicesOnMainThread, weak_this_, base::Unretained(devices), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
}

void AudioManagerChromeOS::GetAudioDevicesOnMainThread(AudioDeviceList* devices, base::WaitableEvent* event)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
uint64_t AudioManagerChromeOS::GetPrimaryActiveInputNode()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioManagerChromeOS::GetPrimaryActiveInputNodeOnMainThread, weak_this_, &device_id, &event));
    }
    WaitEventOrShutdown(&event);
    return device_id;
}
...
uint64_t AudioManagerChromeOS::GetPrimaryActiveOutputNode()
{
    ...
    if (main_task_runner_->BelongsToCurrentThread()) {
        ...
        main_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&AudioManagerChromeOS::GetPrimaryActiveOutputNodeOnMainThread, weak_this_, base::Unretained(&device_id), base::Unretained(&event)));
    }
    WaitEventOrShutdown(&event);
    return device_id;
}

void AudioManagerChromeOS::GetPrimaryActiveInputNodeOnMainThread(uint64_t* active_input_node_id, base::WaitableEvent* event)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    if (CrasAudioHandler::Get()) {
        *active_input_node_id = CrasAudioHandler::Get()->GetPrimaryActiveInputNode();
    }
    event->Signal();
}

void AudioManagerChromeOS::GetPrimaryActiveOutputNodeOnMainThread(uint64_t* active_output_node_id, base::WaitableEvent* event)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    if (CrasAudioHandler::Get()) {
        *active_output_node_id = CrasAudioHandler::Get()->GetPrimaryActiveOutputNode();
    }
    event->Signal();
}

void AudioManagerChromeOS::GetDefaultOutputBufferSizeOnMainThread(int32_t* buffer_size, base::WaitableEvent* event)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    if (CrasAudioHandler::Get())
...
void AudioManagerChromeOS::GetSystemApmEffectsSupportedOnMainThread(SystemAudioProcessingInfo* system_apm_info, base::WaitableEvent* event)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    if (CrasAudioHandler::Get()) {
        system_apm_info->aec_supported = CrasAudioHandler::Get()->system_aec_supported();
        system_apm_info->aec_group_id = CrasAudioHandler::Get()->system_aec_group_id();
        system_apm_info->ns_supported = CrasAudioHandler::Get()->system_ns_supported();
        system_apm_info->agc_supported = CrasAudioHandler::Get()->system_agc_supported();
    }
    event->Signal();
}
...
AudioParameters AudioManagerChromeOS::GetStreamParametersForSystem(int user_buffer_size, const AudioManagerChromeOS::SystemAudioProcessingInfo& system_apm_info)
{
    AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kDefaultSampleRate, user_buffer_size,
        AudioParameters::HardwareCapabilities(limits::kMinAudioBufferSize, limits::kMaxAudioBufferSize));
    ...
    RetrieveSystemEffectFeatures(enforce_system_aec, enforce_system_ns, enforce_system_agc, tuned_system_aec_allowed);
    ...
    params.set_effects(params.effects() | AudioParameters::EXPERIMENTAL_ECHO_CANCELLER);
    ...
    bool use_system_aec = (tuned_system_apm_available && tuned_system_aec_allowed) || enforce_system_aec;
    ...
    if (!tuned_system_apm_available) {
        ...
        if (system_apm_info.agc_supported || enforce_system_agc) {
            params.set_effects(params.effects() | AudioParameters::AUTOMATIC_GAIN_CONTROL);
        }
    }
