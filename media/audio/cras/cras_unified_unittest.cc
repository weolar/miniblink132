...
protected:
    ...
    CrasUnifiedStream* CreateStream(ChannelLayoutConfig layout, int32_t samples_per_packet)
    {
        ...
        return new CrasUnifiedStream(params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId, AudioManager::LogCallback());
    }
...
constexpr ChannelLayout CrasUnifiedStreamTest::kTestChannelLayout = CHANNEL_LAYOUT_STEREO;
...
{
    CrasUnifiedStream* test_stream = CreateStream(ChannelLayoutConfig::FromLayout<kTestChannelLayout>());
    ...
    test_stream = CreateStream(ChannelLayoutConfig::FromLayout<CHANNEL_LAYOUT_SURROUND>());
    EXPECT_TRUE(test_stream->Open());
    test_stream->Close();

    // Bad sample rate.
    AudioParameters bad_rate_params(kTestFormat, ChannelLayoutConfig::FromLayout<kTestChannelLayout>(), 0, kTestFramesPerPacket);
    test_stream = new CrasUnifiedStream(bad_rate_params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId, AudioManager::LogCallback());
...
TEST_F(CrasUnifiedStreamTest, RenderFrames)
{
    ...
    EXPECT_CALL(mock_callback, OnMoreData(_, _, AudioGlitchInfo(), _))
        .WillRepeatedly(DoAll(InvokeWithoutArgs(&event, &base::WaitableEvent::Signal), Return(kTestFramesPerPacket)));
