...
{
    base::UmaHistogramEnumeration("Media.Audio.CrasUnifiedStreamOpenSuccess", result);
}

void ReportStreamStartResult(StreamStartResult result)
{
    base::UmaHistogramEnumeration("Media.Audio.CrasUnifiedStreamStartSuccess", result);
}

void ReportNotifyStreamErrors(int err)
{
    base::UmaHistogramSparse("Media.Audio.CrasUnifiedStreamNotifyStreamError", err);
}
...
bool CrasUnifiedStream::Open()
{
    ...
    if (!client_) {
        ...
        ReportStreamOpenResult(StreamOpenResult::kCallbackOpenCannotCreateCrasClient);
        client_ = NULL;
        return false;
    }
...
void CrasUnifiedStream::Start(AudioSourceCallback* callback)
{
    ...
    if (!stream_params) {
        ...
        ReportStreamStartResult(StreamStartResult::kCallbackStartCreatingStreamParamsFailed);
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
    }

    unsigned int frames_per_packet = params_.frames_per_buffer();

    int rc = libcras_stream_params_set(stream_params, stream_direction_, frames_per_packet * 2, frames_per_packet, CRAS_STREAM_TYPE_DEFAULT,
        manager_->GetClientType(), 0, this, CrasUnifiedStream::UnifiedCallback, CrasUnifiedStream::StreamError, params_.sample_rate(), SND_PCM_FORMAT_S16,
        params_.channels());

    if (rc) {
        LOG(WARNING) << "Error setting up stream parameters.";
        ReportStreamStartResult(StreamStartResult::kCallbackStartSettingUpStreamParamsFailed);
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
    ...
    if (rc) {
        ...
        ReportStreamStartResult(StreamStartResult::kCallbackStartSettingUpChannelLayoutFailed);
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
        libcras_stream_params_destroy(stream_params);
        return;
    }
    ...
    if (libcras_client_add_pinned_stream(client_, pin_device_, &stream_id_, stream_params)) {
        ...
        ReportStreamStartResult(StreamStartResult::kCallbackStartAddingStreamFailed);
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
        libcras_stream_params_destroy(stream_params);
        return;
    }
...
int CrasUnifiedStream::UnifiedCallback(struct libcras_stream_cb_data* data)
{
    ...
    TRACE_EVENT_BEGIN("audio", "CrasUnifiedStream::UnifiedCallback", perfetto::Flow::ProcessScoped(static_cast<uint64_t>(stream_id)));

    CrasUnifiedStream* me = static_cast<CrasUnifiedStream*>(usr_arg);
    base::TimeDelta underrun_duration = base::TimeDelta::FromTimeSpec(underrun_duration_ts);
...
int CrasUnifiedStream::StreamError(cras_client* client, cras_stream_id_t stream_id, int err, void* arg)
{
...
uint32_t CrasUnifiedStream::WriteAudio(size_t frames, uint8_t* buffer, const timespec* latency_ts)
{
    ...
    TRACE_EVENT("audio", "CrasUnifiedStream::WriteAudio", [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chromeos_cras_unified();
    ...
    int frames_filled = source_callback_->OnMoreData(BoundedDelay(delay), base::TimeTicks::Now(), glitch_info, output_bus_.get());

    peak_detector_->FindPeak(output_bus_.get());

    // Note: If this ever changes to output raw float the data must be clipped and
    // sanitized since it may come from an untrusted source such as NaCl.
    output_bus_->ToInterleaved<SignedInt16SampleTypeTraits>(frames_filled, reinterpret_cast<int16_t*>(buffer));
...
void CrasUnifiedStream::ReportAndResetStats()
{
    ...
    if (!log_callback_.is_null()) {
        std::string log_message
            = base::StringPrintf("CRAS out: (num_glitches_detected=[%d], cumulative_audio_lost=[%" PRId64 " ms],largest_glitch=[%" PRId64 " ms])",
...
void CrasUnifiedStream::CalculateAudioGlitches(base::TimeDelta underrun_duration)
{
    TRACE_EVENT("audio", "CrasUnifiedStream::CalculateAudioGlitches", [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chromeos_cras_unified();
        data->set_underrun_duration_us(underrun_duration.InMicroseconds());
        data->set_last_underrun_duration_us(last_underrun_duration_.InMicroseconds());
    });
    ...
    base::TimeDelta underrun_glitch_duration = underrun_duration - last_underrun_duration_;

    glitch_reporter_.UpdateStats(underrun_glitch_duration);

    if (underrun_glitch_duration.is_positive()) {
        glitch_info_accumulator_.Add(AudioGlitchInfo::SingleBoundedSystemGlitch(underrun_glitch_duration, AudioGlitchInfo::Direction::kRender));
        TRACE_EVENT_INSTANT("audio", "glitch", [&](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_chromeos_cras_unified();
            data->set_underrun_glitch_duration_us(underrun_glitch_duration.InMicroseconds());
        });
    }
