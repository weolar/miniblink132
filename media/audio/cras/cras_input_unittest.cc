...
public:
    MOCK_METHOD4(OnData, void(const AudioBus*, base::TimeTicks, double, const AudioGlitchInfo& glitch_info));
...
public:
    MockAudioManagerCrasInput()
        : AudioManagerCrasBase(std::make_unique<TestAudioThread>(), &fake_audio_log_factory_)
    {
    }
    ...
    AudioParameters GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
    {
        return AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 44100, 1000);
    }
    bool IsDefault(const std::string& device_id, bool is_input) override
    {
        return true;
    }
    enum CRAS_CLIENT_TYPE GetClientType()
    {
...
protected:
    ...
    {
        return CreateStream(layout, samples_per_packet, AudioDeviceDescription::kDefaultDeviceId);
    }

    CrasInputStream* CreateStream(ChannelLayoutConfig layout, int32_t samples_per_packet, const std::string& device_id)
    {
        AudioParameters params(kTestFormat, layout, kTestSampleRate, samples_per_packet);
        return new CrasInputStream(params, mock_manager_.get(), device_id, AudioManager::LogCallback());
    }

    void CaptureSomeFrames(const AudioParameters& params, unsigned int duration_ms)
    {
        CrasInputStream* test_stream = new CrasInputStream(params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId, AudioManager::LogCallback());
        ...
        EXPECT_CALL(mock_callback, OnData(_, _, _, _)).WillOnce(InvokeWithoutArgs(&event, &base::WaitableEvent::Signal));
...
constexpr ChannelLayout CrasInputStreamTest::kTestChannelLayout = CHANNEL_LAYOUT_STEREO;
...
{
    AudioParameters bad_rate_params(kTestFormat, ChannelLayoutConfig::FromLayout<kTestChannelLayout>(), 0, kTestFramesPerPacket);
    CrasInputStream* test_stream
        = new CrasInputStream(bad_rate_params, mock_manager_.get(), AudioDeviceDescription::kDefaultDeviceId, AudioManager::LogCallback());
...
TEST_F(CrasInputStreamTest, CaptureFrames)
{
    ...
    for (unsigned int i = 0; i < ARRAY_SIZE(rates); i++) {
        ...
        AudioParameters params_mono(kTestFormat, ChannelLayoutConfig::Mono(), rates[i], kTestFramesPerPacket);
        CaptureSomeFrames(params_mono, kTestCaptureDurationMs);
    }

    for (unsigned int i = 0; i < ARRAY_SIZE(rates); i++) {
        SCOPED_TRACE(testing::Message() << "Stereo " << rates[i] << "Hz");
        AudioParameters params_stereo(kTestFormat, ChannelLayoutConfig::Stereo(), rates[i], kTestFramesPerPacket);
...
{
    CrasInputStream* test_stream = CreateStream(ChannelLayoutConfig::Stereo(), kTestFramesPerPacket, AudioDeviceDescription::kLoopbackInputDeviceId);
    EXPECT_EQ(test_stream->Open(), AudioInputStream::OpenOutcome::kSuccess);
    test_stream->Close();
}
