...
{
    base::UmaHistogramEnumeration("Media.Audio.CrasInputStreamOpenSuccess", result);
}

void ReportStreamStartResult(StreamStartResult result)
{
    base::UmaHistogramEnumeration("Media.Audio.CrasInputStreamStartSuccess", result);
}
...
static constexpr char kVoiceIsolationEffectStateHistogramName[] = "Cras.StreamEffectState.VoiceIsolation";

// These values are persisted to logs. Entries should not be renumbered and
// numeric values should never be reused.
// Used to log stream effects in `CrasInputStream::Start`.
enum class StreamEffectState { kForceDisable = 0, kForceEnable = 1, kPlatformDefault = 2, kMaxValue = kPlatformDefault };
...
CrasInputStream::CrasInputStream(
    const AudioParameters& params, AudioManagerCrasBase* manager, const std::string& device_id, const AudioManager::LogCallback& log_callback)
    : audio_manager_(manager)
    , params_(params)
    , is_loopback_(AudioDeviceDescription::IsLoopbackDevice(device_id))
    , is_loopback_without_chrome_(device_id == AudioDeviceDescription::kLoopbackWithoutChromeId)
    ...
    , peak_detector_(base::BindRepeating(&AudioManager::TraceAmplitudePeak, base::Unretained(audio_manager_),
          /*trace_start=*/true))
{
...
AudioInputStream::OpenOutcome CrasInputStream::Open()
{
    ...
    if (params_.sample_rate() <= 0) {
        ...
        ReportStreamOpenResult(StreamOpenResult::kCallbackOpenUnsupportedAudioFrequency);
    ...
    if (AudioParameters::AUDIO_PCM_LINEAR != params_.format() && AudioParameters::AUDIO_PCM_LOW_LATENCY != params_.format()) {
        ...
        ReportStreamOpenResult(StreamOpenResult::kCallbackOpenUnsupportedAudioFormat);
        return OpenOutcome::kFailed;
    }
    ...
    if (!client_) {
        ...
        ReportStreamOpenResult(StreamOpenResult::kCallbackOpenCrasClientCreationFailed);
        client_ = NULL;
        return OpenOutcome::kFailed;
    }
    ...
    if (libcras_client_connect(client_)) {
        ...
        ReportStreamOpenResult(StreamOpenResult::kCallbackOpenCannotConnectToCrasClient);
    ...
    if (is_loopback_) {
        if (libcras_client_connected_wait(client_) < 0) {
            ...
            // cleanup code.
            ReportStreamOpenResult(StreamOpenResult::kCallbackOpenCannotSynchronizeData);
            libcras_client_destroy(client_);
            client_ = NULL;
            return OpenOutcome::kFailed;
        }
        ...
        if (is_loopback_without_chrome_) {
            ...
            rc = pin_device_ = libcras_client_get_floop_dev_idx_by_client_types(client_, client_types);
        } else {
            if (base::FeatureList::IsEnabled(media::kAudioFlexibleLoopbackForSystemLoopback)) {
                rc = pin_device_ = libcras_client_get_floop_dev_idx_by_client_types(client_, ~(uint32_t)0);
            } else {
                rc = libcras_client_get_loopback_dev_idx(client_, &pin_device_);
            }
        }
        if (rc < 0) {
            DLOG(WARNING) << "Couldn't find CRAS loopback device " << (is_loopback_without_chrome_ ? " for non-chrome loopback." : " for full loopback.");
            ReportStreamOpenResult(StreamOpenResult::kCallbackOpenCannotFindLoopbackDevice);
            libcras_client_destroy(client_);
            client_ = NULL;
            return OpenOutcome::kFailed;
        }
...
void CrasInputStream::Start(AudioInputCallback* callback)
{
    ...
    static const int kChannelMap[]
        = { CRAS_CH_FL, CRAS_CH_FR, CRAS_CH_FC, CRAS_CH_LFE, CRAS_CH_RL, CRAS_CH_RR, CRAS_CH_FLC, CRAS_CH_FRC, CRAS_CH_RC, CRAS_CH_SL, CRAS_CH_SR };
    static_assert(std::size(kChannelMap) == CHANNELS_MAX + 1, "kChannelMap array size should match");
    ...
    if (!stream_params) {
        ...
        ReportStreamStartResult(StreamStartResult::kCallbackStartErrorCreatingStreamParameters);
        callback_->OnError();
        callback_ = NULL;
        return;
    }

    int rc = libcras_stream_params_set(stream_params, stream_direction_, frames_per_packet, frames_per_packet, type, audio_manager_->GetClientType(), flags,
        this, CrasInputStream::SamplesReady, CrasInputStream::StreamError, params_.sample_rate(), SND_PCM_FORMAT_S16, params_.channels());

    if (rc) {
        DLOG(WARNING) << "Error setting up stream parameters.";
        ReportStreamStartResult(StreamStartResult::kCallbackStartErrorSettingUpStreamParameters);
        callback_->OnError();
        callback_ = NULL;
    ...
    for (size_t i = 0; i < std::size(kChannelMap); ++i) {
        layout[kChannelMap[i]] = ChannelOrder(params_.channel_layout(), static_cast<Channels>(i));
    }

    rc = libcras_stream_params_set_channel_layout(stream_params, CRAS_CH_MAX, layout);
    if (rc) {
        DLOG(WARNING) << "Error setting up the channel layout.";
        ReportStreamStartResult(StreamStartResult::kCallbackStartErrorSettingUpChannelLayout);
        callback_->OnError();
        callback_ = NULL;
        libcras_stream_params_destroy(stream_params);
        return;
    ...
    if (libcras_client_add_pinned_stream(client_, pin_device_, &stream_id_, stream_params)) {
        DLOG(WARNING) << "Failed to add the stream.";
        ReportStreamStartResult(StreamStartResult::kCallbackStartFailedAddingStream);
        callback_->OnError();
        callback_ = NULL;
    }
...
int CrasInputStream::SamplesReady(struct libcras_stream_cb_data* data)
{
    ...
    libcras_stream_cb_data_get_dropped_samples_duration(data, &dropped_samples_duration_ts);
    dropped_samples_duration = base::TimeDelta::FromTimeSpec(dropped_samples_duration_ts);
    me->CalculateAudioGlitches(overrun_frames, dropped_samples_duration);
    return frames;
}

// Static callback for stream errors.
int CrasInputStream::StreamError(cras_client* client, cras_stream_id_t stream_id, int err, void* arg)
{
...
void CrasInputStream::ReadAudio(size_t frames, uint8_t* buffer, const timespec* latency_ts)
{
    DCHECK(callback_);
    ...
    audio_bus_->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<int16_t*>(buffer), audio_bus_->frames());

    peak_detector_.FindPeak(audio_bus_.get());

    callback_->OnData(audio_bus_.get(), capture_time, normalized_volume, glitch_info_accumulator_.GetAndReset());
}
...
void CrasInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
    DCHECK(client_);
...
void CrasInputStream::ReportAndResetStats()
{
    ...
    std::string log_message
        = base::StringPrintf("CRAS in: (num_glitches_detected=[%d], cumulative_audio_lost=[%" PRId64 " ms],largest_glitch=[%" PRId64 " ms])",
...
void CrasInputStream::CalculateAudioGlitches(uint32_t overrun_frames, base::TimeDelta dropped_samples_duration)
{
    ...
    base::TimeDelta overrun_glitch_duration = AudioTimestampHelper::FramesToTime(overrun_frames_this_callback, params_.sample_rate());
    ...
    base::TimeDelta dropped_samples_glitch_duration = dropped_samples_duration - last_dropped_samples_duration_;

    base::TimeDelta glitch_duration = overrun_glitch_duration + dropped_samples_glitch_duration;
    glitch_reporter_.UpdateStats(glitch_duration);
    if (glitch_duration.is_positive()) {
        glitch_info_accumulator_.Add(AudioGlitchInfo::SingleBoundedSystemGlitch(glitch_duration, AudioGlitchInfo::Direction::kCapture));
    }
