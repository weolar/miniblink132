...
public:
    MOCK_METHOD(std::vector<CrasDevice>, CrasGetAudioDevices, (DeviceType type), (override));
...
public:
    AudioManagerCrasUnderTest()
        : AudioManagerCras(std::make_unique<TestAudioThread>(), &fake_audio_log_factory_)
    {
    }
...
TEST_F(AudioManagerCrasTest, HasAudioInputDevices)
{
    ...
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kInput)).WillOnce(testing::Return(devices));
...
TEST_F(AudioManagerCrasTest, CheckDefaultNoDevice)
{
    ...
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kInput)).WillOnce(testing::Return(devices));
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kOutput)).WillOnce(testing::Return(devices));
    audio_manager_->SetCrasUtil(std::move(util));
...
TEST_F(AudioManagerCrasTest, MaxChannel)
{
    ...
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kOutput)).WillRepeatedly(testing::Return(devices));
    audio_manager_->SetCrasUtil(std::move(util));
    auto params = audio_manager_->GetPreferredOutputStreamParameters("123", AudioParameters());
...
const CrasDevice kInternalSpeaker(DeviceType::kOutput, 0, 0, 2, true, false, "INTERNAL_SPEAKER", "Internal Speaker", "Fake Soundcard");

const CrasDevice kInternalMic(DeviceType::kInput, 1, 1, 2, true, false, "INTERNAL_MIC", "Internal Mic", "Fake Soundcard");

const CrasDevice kHeadphone(DeviceType::kOutput, 2, 2, 2, true, false, "HEADPHONE", "Headphone", "Fake Soundcard");

const CrasDevice kExternalMic(DeviceType::kInput, 3, 3, 2, true, false, "MIC", "Mic", "Fake Soundcard");

const CrasDevice kUSB(DeviceType::kOutput, 4, 4, 2, true, false, "USB", "USB", "Fake USB");

const CrasDevice kUSB_6CH(DeviceType::kOutput, 5, 5, 6, true, false, "USB", "USB 6ch", "Fake USB 6ch");

const CrasDevice kHDMI(DeviceType::kOutput, 6, 6, 8, true, false, "HDMI", "HDMI", "Fake HDMI");
void CheckDeviceNames(const AudioDeviceNames& device_names, const std::map<uint64_t, std::string>& expectation)
{
...
TEST_F(AudioManagerCrasTest, EnumerateInputDevices)
{
    ...
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kInput)).WillRepeatedly(testing::Return(devices));
    audio_manager_->SetCrasUtil(std::move(util));
...
AudioParameters GetPreferredOutputStreamParameters(const ChannelLayoutConfig& channel_layout_config, int32_t user_buffer_size = 0)
{
...
TEST_F(AudioManagerCrasTest, CheckOutputStreamParameters)
{
    ...
    EXPECT_CALL(*util, CrasGetAudioDevices(DeviceType::kOutput)).WillRepeatedly(testing::Return(devices));
    EXPECT_CALL(*util, CrasGetDefaultOutputBufferSize()).WillRepeatedly(testing::Return(512));
    audio_manager_->SetCrasUtil(std::move(util));
    ...
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kInternalSpeaker.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::Stereo());
    EXPECT_TRUE(params.Equals(golden_params));
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kUSB_6CH.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_5_1>());
    EXPECT_TRUE(params.Equals(golden_params));
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kHDMI.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_7_1>());
    EXPECT_TRUE(params.Equals(golden_params));
    ...
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kInternalSpeaker.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::Stereo(), 2048);
    EXPECT_TRUE(params.Equals(golden_params));
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kUSB_6CH.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_5_1>(), 2048);
    EXPECT_TRUE(params.Equals(golden_params));
    params = audio_manager_->GetPreferredOutputStreamParameters(base::NumberToString(kHDMI.id), AudioParameters());
    golden_params = GetPreferredOutputStreamParameters(ChannelLayoutConfig::FromLayout<ChannelLayout::CHANNEL_LAYOUT_7_1>(), 2048);
    EXPECT_TRUE(params.Equals(golden_params));
}
...
TEST_F(AudioManagerCrasTest, LookupDefaultInputDeviceWithProperGroupId)
{
    ...
    auto default_group_id = audio_manager_->GetGroupIDInput(audio_manager_->GetDefaultInputDeviceID());
    auto expected_group_id = audio_manager_->GetGroupIDInput(base::NumberToString(kExternalMic.id));
...
TEST_F(AudioManagerCrasTest, LookupDefaultOutputDeviceWithProperGroupId)
{
    ...
    auto default_group_id = audio_manager_->GetGroupIDOutput(audio_manager_->GetDefaultOutputDeviceID());
    auto expected_group_id = audio_manager_->GetGroupIDOutput(base::NumberToString(kHeadphone.id));
    EXPECT_EQ(default_group_id, expected_group_id);
}
...
{
    return params.effects() & AudioParameters::ALLOW_DSP_ECHO_CANCELLER && params.effects() & AudioParameters::ECHO_CANCELLER;
}

bool DspNsAllowed(const AudioParameters& params)
{
    return params.effects() & AudioParameters::ALLOW_DSP_NOISE_SUPPRESSION && params.effects() & AudioParameters::NOISE_SUPPRESSION;
}

bool DspAgcAllowed(const AudioParameters& params)
{
    return params.effects() & AudioParameters::ALLOW_DSP_AUTOMATIC_GAIN_CONTROL && params.effects() & AudioParameters::AUTOMATIC_GAIN_CONTROL;
}

class AudioManagerCrasTestAEC : public AudioManagerCrasTest, public ::testing::WithParamInterface<std::tuple<bool, bool, bool, bool>> {
protected:
    void SetUp() override
    {
        std::unique_ptr<MockCrasUtil> util = std::make_unique<MockCrasUtil>();
        ...
        EXPECT_CALL(*util, CrasGetAecSupported()).WillOnce(testing::Return(aec_supported));
        EXPECT_CALL(*util, CrasGetAecGroupId()).WillOnce(testing::Return(aec_group));
        EXPECT_CALL(*util, CrasGetNsSupported()).WillOnce(testing::Return(ns_supported));
        EXPECT_CALL(*util, CrasGetAgcSupported()).WillOnce(testing::Return(agc_supported));

        audio_manager_->SetCrasUtil(std::move(util));
    }
};

INSTANTIATE_TEST_SUITE_P(AllInputParameters, AudioManagerCrasTestAEC,
    ::testing::Combine(::testing::Values(false, true), ::testing::Values(kNoAecFlaggedGroupId, kAecTestGroupId), ::testing::Values(false, true),
...
TEST_P(AudioManagerCrasTestAEC, DefaultBehaviorSystemAecEnforcedByPolicy)
{
    ...
    base::CommandLine::ForCurrentProcess()->AppendSwitch(switches::kSystemAecEnabled);
    AudioParameters params = audio_manager_->GetInputStreamParameters("");

    EXPECT_TRUE(AecActive(params));
}

TEST_P(AudioManagerCrasTestAEC, BehaviorWithCrOSEnforceSystemAecDisabledButEnforcedByPolicy)
{
    base::test::ScopedFeatureList feature_list;
...
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeaturesAndParameters({ { media::kCrOSEnforceSystemAecNs, {} }, { media::kCrOSEnforceSystemAecAgc, {} } }, {});
    AudioParameters params = audio_manager_->GetInputStreamParameters("");
...
TEST_P(AudioManagerCrasTestAEC, BehaviorWithCrOSEnforceSystemAecNsAgcAndDisallowedSystemAec)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeaturesAndParameters({ { media::kCrOSEnforceSystemAecNsAgc, {} } }, { { media::kCrOSSystemAEC } });
    AudioParameters params = audio_manager_->GetInputStreamParameters("");
...
class AudioManagerCrasTestDSP : public AudioManagerCrasTest, public ::testing::WithParamInterface<std::tuple<bool, bool, bool>> {
protected:
    void SetUp() override
    {
        std::unique_ptr<MockCrasUtil> util = std::make_unique<MockCrasUtil>();
...
INSTANTIATE_TEST_SUITE_P(AllInputParameters, AudioManagerCrasTestDSP,
...
TEST_P(AudioManagerCrasTestDSP, BehaviorWithCrOSEnforceSystemAec)
{
    ...
    EXPECT_TRUE(DspAecAllowed(params) && aec_on_dsp_allowed_ || !DspAecAllowed(params) && !aec_on_dsp_allowed_);
    EXPECT_FALSE(DspNsAllowed(params));
    EXPECT_FALSE(DspAgcAllowed(params));
}
...
TEST_P(AudioManagerCrasTestDSP, BehaviorWithCrOSEnforceSystemAecNs)
{
    ...
    EXPECT_TRUE(DspAecAllowed(params) && aec_on_dsp_allowed_ || !DspAecAllowed(params) && !aec_on_dsp_allowed_);
    EXPECT_TRUE(DspNsAllowed(params) && ns_on_dsp_allowed_ || !DspNsAllowed(params) && !ns_on_dsp_allowed_);
    EXPECT_FALSE(DspAgcAllowed(params));
}
...
TEST_P(AudioManagerCrasTestDSP, BehaviorWithCrOSEnforceSystemAecAgc)
{
    ...
    EXPECT_TRUE(DspAecAllowed(params) && aec_on_dsp_allowed_ || !DspAecAllowed(params) && !aec_on_dsp_allowed_);
    EXPECT_FALSE(DspNsAllowed(params));
    EXPECT_TRUE(DspAgcAllowed(params) && agc_on_dsp_allowed_ || !DspAgcAllowed(params) && !agc_on_dsp_allowed_);
}
