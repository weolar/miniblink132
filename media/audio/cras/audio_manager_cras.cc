...
AudioManagerCras::AudioManagerCras(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
...
void AudioManagerCras::GetAudioInputDeviceNames(AudioDeviceNames* device_names)
{
    for (const auto& device : cras_util_->CrasGetAudioDevices(DeviceType::kInput)) {
...
void AudioManagerCras::GetAudioOutputDeviceNames(AudioDeviceNames* device_names)
{
    for (const auto& device : cras_util_->CrasGetAudioDevices(DeviceType::kOutput)) {
        device_names->emplace_back(device.name, base::NumberToString(device.id));
    }
    if (!device_names->empty()) {
        device_names->push_front(AudioDeviceName::CreateDefault());
...
{
    return base::GetFieldTrialParamByFeatureAsBool(media::kCrOSSystemAECDeactivatedGroups, base::NumberToString(aec_group_id), false);
}
...
{
    return base::GetFieldTrialParamByFeatureAsBool(media::kCrOSDspBasedAecDeactivatedGroups, base::NumberToString(aec_group_id), false)
...
{
    return base::GetFieldTrialParamByFeatureAsBool(media::kCrOSDspBasedNsDeactivatedGroups, base::NumberToString(aec_group_id), false)
...
{
    return base::GetFieldTrialParamByFeatureAsBool(media::kCrOSDspBasedAgcDeactivatedGroups, base::NumberToString(aec_group_id), false)
...
{
    const bool enforce_system_aec_ns_agc_feature = base::FeatureList::IsEnabled(media::kCrOSEnforceSystemAecNsAgc);
    const bool enforce_system_aec_ns_feature = base::FeatureList::IsEnabled(media::kCrOSEnforceSystemAecNs);
    const bool enforce_system_aec_agc_feature = base::FeatureList::IsEnabled(media::kCrOSEnforceSystemAecAgc);
    const bool enforce_system_aec_feature = base::FeatureList::IsEnabled(media::kCrOSEnforceSystemAec);
    const bool enforce_system_aec_by_policy = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kSystemAecEnabled);
    ...
    tuned_system_aec_allowed = base::FeatureList::IsEnabled(media::kCrOSSystemAEC);
}

AudioParameters AudioManagerCras::GetStreamParametersForSystem(int user_buffer_size)
{
    ...
    if (base::FeatureList::IsEnabled(media::kCrOSSystemVoiceIsolationOption)) {
        if (cras_util_->CrasGetVoiceIsolationSupported()) {
            params.set_effects(params.effects() | AudioParameters::VOICE_ISOLATION_SUPPORTED);
        }
    }
...
AudioParameters AudioManagerCras::GetInputStreamParameters(const std::string& device_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());

    int user_buffer_size = GetUserBufferSize();
    user_buffer_size = user_buffer_size ? user_buffer_size : kDefaultInputBufferSize;
...
{
    for (const auto& device : cras_util_->CrasGetAudioDevices(DeviceType::kInput)) {
        if (base::NumberToString(device.id) == device_id || (AudioDeviceDescription::IsDefaultDevice(device_id) && device.active)) {
...
std::string AudioManagerCras::GetAssociatedOutputDeviceID(const std::string& input_device_id)
{
...
AudioParameters AudioManagerCras::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
