...
class MEDIA_EXPORT CrasInputStream : public AgcAudioStream<AudioInputStream>, public AecdumpRecordingSource {
public:
    // The ctor takes all the usual parameters, plus |manager| which is the
    // audio manager who is creating this object.
    CrasInputStream(const AudioParameters& params, AudioManagerCrasBase* manager, const std::string& device_id, const AudioManager::LogCallback& log_callback);
...
private:
    ...
    static int StreamError(cras_client* client, cras_stream_id_t stream_id, int err, void* arg);
    ...
    // glitches.
    void CalculateAudioGlitches(uint32_t overrun_frames, base::TimeDelta dropped_samples_duration);
