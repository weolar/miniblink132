...
struct MEDIA_EXPORT AudioDeviceDescription {
    ...
    // be used.
    static bool UseSessionIdToSelectDevice(const base::UnguessableToken& session_id, const std::string& device_id);
    ...
    static std::string GetCommunicationsDeviceName(const std::string& real_device_name);
    ...
    static void LocalizeDeviceDescriptions(std::vector<AudioDeviceDescription>* device_descriptions);
    ...
    AudioDeviceDescription(
        std::string device_name, std::string unique_id, std::string group_id, bool is_system_default = false, bool is_communications_device = false);
