...
public:
    ...
    using RegisterDebugRecordingSourceCallback = base::RepeatingCallback<std::unique_ptr<AudioDebugRecorder>(const AudioParameters&)>;

    AudioOutputResampler(AudioManager* audio_manager, const AudioParameters& input_params, const AudioParameters& output_params,
    ...
    bool StartStream(AudioOutputStream::AudioSourceCallback* callback, AudioOutputProxy* stream_proxy) override;
...
private:
    using CallbackMap = base::flat_map<AudioOutputProxy*, std::unique_ptr<OnMoreDataConverter>>;
    ...
    std::unique_ptr<AudioOutputDispatcherImpl> MakeDispatcher(const std::string& output_device_id, const AudioParameters& params);
    ...
    RegisterDebugRecordingSourceCallback register_debug_recording_source_callback_;
