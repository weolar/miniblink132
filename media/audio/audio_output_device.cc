...
AudioOutputDevice::AudioOutputDevice(std::unique_ptr<AudioOutputIPC> ipc, const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner,
    const AudioSinkParameters& sink_params, base::TimeDelta authorization_timeout)
    ...
    , did_receive_auth_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
...
void AudioOutputDevice::Initialize(const AudioParameters& params, RenderCallback* callback)
{
...
void AudioOutputDevice::InitializeOnIOThread(const AudioParameters& params, MayBeDangling<RenderCallback> callback)
{
...
{
    {
        ...
        if (pending_device_info_cb_) {
            std::move(pending_device_info_cb_).Run(OutputDeviceInfo(OUTPUT_DEVICE_STATUS_ERROR_INTERNAL));
        }
    }
...
void AudioOutputDevice::RequestDeviceAuthorization()
{
    ...
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::RequestDeviceAuthorizationOnIOThread, this));
}

void AudioOutputDevice::Start()
{
    TRACE_EVENT0("audio", "AudioOutputDevice::Start");
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::CreateStreamOnIOThread, this));
}
...
void AudioOutputDevice::Stop()
{
    ...
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::ShutDownOnIOThread, this));
}

void AudioOutputDevice::Play()
{
    TRACE_EVENT0("audio", "AudioOutputDevice::Play");
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::PlayOnIOThread, this));
}

void AudioOutputDevice::Pause()
{
    TRACE_EVENT0("audio", "AudioOutputDevice::Pause");
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::PauseOnIOThread, this));
}

void AudioOutputDevice::Flush()
{
    TRACE_EVENT0("audio", "AudioOutputDevice::Flush");
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::FlushOnIOThread, this));
}
...
bool AudioOutputDevice::SetVolume(double volume)
{
    ...
    return io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputDevice::SetVolumeOnIOThread, this, volume));
}
...
{
    {
        ...
        if (!did_receive_auth_.IsSignaled()) {
            ...
            pending_device_info_cb_ = base::BindPostTaskToCurrentDefault(std::move(info_cb));
            return;
        }
    }
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), GetOutputDeviceInfo_Signaled()));
}
...
void AudioOutputDevice::RequestDeviceAuthorizationOnIOThread()
{
    ...
    if (auth_timeout_.is_positive()) {
        ...
        auth_timeout_action_->Start(FROM_HERE, auth_timeout_,
            base::BindOnce(&AudioOutputDevice::OnDeviceAuthorized, this, OUTPUT_DEVICE_STATUS_ERROR_TIMED_OUT, AudioParameters(), std::string()));
    }
}
...
void AudioOutputDevice::ShutDownOnIOThread()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Media.Audio.Render.StreamCallbackError2", had_error_);
    had_error_ = kNoError;
...
void AudioOutputDevice::OnDeviceAuthorized(OutputDeviceStatus device_status, const AudioParameters& output_params, const std::string& matched_device_id)
{
    DCHECK(io_task_runner_->BelongsToCurrentThread());
    ...
    UMA_HISTOGRAM_BOOLEAN("Media.Audio.Render.OutputDeviceAuthorizationTimedOut", device_status == OUTPUT_DEVICE_STATUS_ERROR_TIMED_OUT);
    LOG_IF(WARNING, device_status == OUTPUT_DEVICE_STATUS_ERROR_TIMED_OUT) << "Output device authorization timed out";
    ...
    if (!did_receive_auth_.IsSignaled()) {
        ...
        UMA_HISTOGRAM_ENUMERATION("Media.Audio.Render.OutputDeviceStatus", device_status, OUTPUT_DEVICE_STATUS_MAX + 1);
    }
    ...
    if (device_status == OUTPUT_DEVICE_STATUS_OK) {
        ...
        if (!did_receive_auth_.IsSignaled()) {
            ...
            DCHECK(AudioDeviceDescription::UseSessionIdToSelectDevice(session_id_, device_id_) || matched_device_id_.empty());
            matched_device_id_ = matched_device_id;

            DVLOG(1) << "AudioOutputDevice authorized, session_id: " << session_id_ << ", device_id: " << device_id_
                     << ", matched_device_id: " << matched_device_id_;

            OnAuthSignal();
        }
    } else {
        TRACE_EVENT1("audio", "AudioOutputDevice not authorized", "auth status", device_status_);
...
void AudioOutputDevice::OnStreamCreated(
    base::UnsafeSharedMemoryRegion shared_memory_region, base::SyncSocket::ScopedHandle socket_handle, bool playing_automatically)
{
    ...
    // details.
    {
        ...
        audio_callback_ = std::make_unique<AudioOutputDeviceThreadCallback>(audio_parameters_, std::move(shared_memory_region), callback_);
        if (playing_automatically)
            audio_callback_->InitializePlayStartTime();
        audio_thread_
            = std::make_unique<AudioDeviceThread>(audio_callback_.get(), std::move(socket_handle), "AudioOutputDevice", base::ThreadType::kRealtimeAudio);
    }
}
...
OutputDeviceInfo AudioOutputDevice::GetOutputDeviceInfo_Signaled()
{
    ...
    OutputDeviceInfo info(
        AudioDeviceDescription::UseSessionIdToSelectDevice(session_id_, device_id_) ? matched_device_id_ : device_id_, device_status_, output_params_);
    TRACE_EVENT1("audio", "AudioOutputDevice::GetOutputDeviceInfo_Signaled", "info", info.AsHumanReadableString());
    return info;
}
