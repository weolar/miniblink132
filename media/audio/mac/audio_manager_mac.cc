...
BASE_FEATURE(kMonitorOutputSampleRateChangesMac, "MonitorOutputSampleRateChangesMac", base::FEATURE_ENABLED_BY_DEFAULT);
...
static bool GetOutputDeviceChannelsAndLayout(AudioUnit audio_unit, int* channels, ChannelLayout* channel_layout);
...
static AudioObjectPropertyAddress GetAudioObjectPropertyAddress(AudioObjectPropertySelector selector, bool is_input)
{
    AudioObjectPropertyScope scope = is_input ? kAudioObjectPropertyScopeInput : kAudioObjectPropertyScopeOutput;
    AudioObjectPropertyAddress property_address = { selector, scope, kAudioObjectPropertyElementMain };
    return property_address;
}

static const AudioObjectPropertyAddress kNoiseReductionPropertyAddress = { 'nzca', kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMain };

// Get IO buffer size range from HAL given device id and scope.
static OSStatus GetIOBufferFrameSizeRange(AudioDeviceID device_id, bool is_input, UInt32* minimum, UInt32* maximum)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    AudioObjectPropertyAddress address = GetAudioObjectPropertyAddress(kAudioDevicePropertyBufferFrameSizeRange, is_input);
    AudioValueRange range = { 0, 0 };
    UInt32 data_size = sizeof(AudioValueRange);
    OSStatus result = AudioObjectGetPropertyData(device_id, &address, 0, NULL, &data_size, &range);
    if (result != noErr) {
        OSSTATUS_DLOG(WARNING, result) << "Failed to query IO buffer size range for device: " << std::hex << device_id;
    } else {
...
static bool HasAudioHardware(AudioObjectPropertySelector selector)
{
    ...
    OSStatus err = AudioObjectGetPropertyData(kAudioObjectSystemObject, &property_address,
...
static std::string GetAudioDeviceNameFromDeviceId(AudioDeviceID device_id, bool is_input)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    CFStringRef device_name = nullptr;
    UInt32 data_size = sizeof(device_name);
    AudioObjectPropertyAddress property_address = GetAudioObjectPropertyAddress(kAudioDevicePropertyDeviceNameCFString, is_input);
    OSStatus result = AudioObjectGetPropertyData(device_id, &property_address, 0, nullptr, &data_size, &device_name);
    std::string device;
    if (result == noErr) {
...
static void GetAudioDeviceInfo(bool is_input, media::AudioDeviceNames* device_names)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    std::vector<AudioObjectID> device_ids = core_audio_mac::GetAllAudioDeviceIDs();
    for (AudioObjectID device_id : device_ids) {
        const bool is_valid_for_direction = (is_input ? core_audio_mac::IsInputDevice(device_id) : core_audio_mac::IsOutputDevice(device_id));

        if (!is_valid_for_direction) {
            continue;
        }

        std::optional<std::string> unique_id = core_audio_mac::GetDeviceUniqueID(device_id);
        if (!unique_id) {
            continue;
        }

        std::optional<std::string> label = core_audio_mac::GetDeviceLabel(device_id, is_input);
        if (!label) {
            continue;
        }
...
AudioDeviceID AudioManagerMac::GetAudioDeviceIdByUId(bool is_input, const std::string& device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    AudioObjectPropertyAddress property_address = { kAudioHardwarePropertyDevices, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMain };
    ...
    if (AudioDeviceDescription::IsDefaultDevice(device_id)) {
        ...
        result = AudioObjectGetPropertyData(kAudioObjectSystemObject, &property_address, 0, 0, &device_size, &audio_device_id);
    } else {
        // Non-default device.
        base::apple::ScopedCFTypeRef<CFStringRef> uid(base::SysUTF8ToCFStringRef(device_id));
        ...
        result = AudioObjectGetPropertyData(kAudioObjectSystemObject, &property_address, 0, 0, &translation_size, &value);
    }

    if (result) {
        OSSTATUS_DLOG(WARNING, result) << "Unable to query device " << device_id << " for AudioDeviceID";
    }
...
static bool GetDefaultDevice(AudioDeviceID* device, bool input)
{
    ...
    pa.mSelector = input ? kAudioHardwarePropertyDefaultInputDevice : kAudioHardwarePropertyDefaultOutputDevice;
    pa.mScope = kAudioObjectPropertyScopeGlobal;
    pa.mElement = kAudioObjectPropertyElementMain;

    UInt32 size = sizeof(*device);
    OSStatus result = AudioObjectGetPropertyData(kAudioObjectSystemObject, &pa, 0, 0, &size, device);
...
static bool GetDeviceTotalChannelCount(AudioDeviceID device, AudioObjectPropertyScope scope, int* channels)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    CHECK(channels);
    ...
    AudioObjectPropertyAddress pa = { kAudioDevicePropertyStreamConfiguration, scope, kAudioObjectPropertyElementMain };
    ...
    *channels = 0;
    ...
    DVLOG(1) << __FUNCTION__ << (scope == kAudioDevicePropertyScopeInput ? " Input" : " Output") << " total channels: " << *channels;
    return true;
}
...
static bool GetAudioUnitStreamFormatChannelCount(AudioUnit audio_unit, AUElement element, int* channels)
{
    AudioStreamBasicDescription stream_format;
    UInt32 size = sizeof(stream_format);
    OSStatus result = AudioUnitGetProperty(audio_unit, kAudioUnitProperty_StreamFormat,
        element == AUElement::OUTPUT ? kAudioUnitScope_Output : kAudioUnitScope_Input, element, &stream_format, &size);
    if (result != noErr) {
...
static bool GetInputDeviceChannels(AudioDeviceID device, int* channels)
{
    ...
    if (!GetAudioUnitStreamFormatChannelCount(au.audio_unit(), AUElement::INPUT, channels)) {
        return false;
    }
...
static bool GetOutputDeviceChannelsAndLayout(AudioDeviceID device, int* channels, ChannelLayout* channel_layout)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    CHECK(channels);
    ...
    if (GetDeviceTotalChannelCount(device, kAudioDevicePropertyScopeOutput, &total_channel_count) && total_channel_count > kMaxConcurrentChannels) {
        ...
        *channel_layout = CHANNEL_LAYOUT_DISCRETE;
        ...
        if (!GetOutputDeviceChannelsAndLayout(au.audio_unit(), channels, channel_layout)) {
            return false;
        }
    }

    DVLOG(2) << __FUNCTION__ << " Output channels: " << *channels << ", channel layout: " << ChannelLayoutToString(*channel_layout);
    return true;
}
...
static bool GetOutputDeviceChannelsAndLayout(AudioUnit audio_unit, int* channels, ChannelLayout* channel_layout)
{
    ...
    std::unique_ptr<ScopedAudioChannelLayout> scoped_device_layout = AudioManagerApple::GetOutputDeviceChannelLayout(audio_unit);
    ...
    if (device_layout->mNumberChannelDescriptions == 1 || device_layout->mNumberChannelDescriptions == 2) {
        *channels = device_layout->mNumberChannelDescriptions;
        *channel_layout = *channels == 2 ? CHANNEL_LAYOUT_STEREO : CHANNEL_LAYOUT_MONO;
        return true;
    }
    ...
    *channel_layout = CHANNEL_LAYOUT_DISCRETE;
    ...
    for (UInt32 i = 0; i < device_layout->mNumberChannelDescriptions; i++) {
        AudioChannelLabel label = device_layout->mChannelDescriptions[i].mChannelLabel;
    ...
    if (*channels == 0 || *channels != static_cast<int>(channels_to_match.size())) {
        return true;
    }
...
public:
    ...
    size_t num_resume_notifications() const
    {
        return num_resume_notifications_;
    }
...
private:
    ...
    void OnResume() override
    {
        ...
        is_suspending_ = false;
        earliest_start_time_ = base::TimeTicks::Now() + base::Seconds(kStartDelayInSecsForPowerEvents);
    }
...
AudioManagerMac::AudioManagerMac(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
    ...
    , weak_ptr_factory_(this)
{
    ...
    // may be slow.
    GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&AudioManagerMac::InitializeOnAudioThread, weak_ptr_factory_.GetWeakPtr()));
}
...
std::vector<AudioObjectID> AudioManagerMac::GetRelatedNonBluetoothDeviceIDs(AudioObjectID device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    return core_audio_mac::GetRelatedDeviceIDs(device_id);
}

std::vector<AudioObjectID> AudioManagerMac::GetRelatedBluetoothDeviceIDs(AudioObjectID device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    ...
    std::vector<std::string> trimmed_input_vector = SplitString(input_unique_id.value(), ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const auto& id : GetAllAudioDeviceIDs()) {
        ...
        std::vector<std::string> trimmed_vector = SplitString(unique_id.value(), ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
std::vector<AudioObjectID> AudioManagerMac::GetRelatedDeviceIDs(AudioObjectID device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
...
std::optional<std::string> AudioManagerMac::GetDeviceUniqueID(AudioObjectID device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
    return core_audio_mac::GetDeviceUniqueID(device_id);
}

std::optional<uint32_t> AudioManagerMac::GetDeviceTransportType(AudioObjectID device_id)
{
    DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
...
// static
void AudioManagerMac::GetAudioInputDeviceNames(media::AudioDeviceNames* device_names)
{
    DCHECK(device_names->empty());
    GetAudioDeviceInfo(true, device_names);
}

void AudioManagerMac::GetAudioOutputDeviceNames(media::AudioDeviceNames* device_names)
{
    DCHECK(device_names->empty());
    GetAudioDeviceInfo(false, device_names);
}

AudioParameters AudioManagerMac::GetInputStreamParameters(const std::string& device_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (AudioDeviceDescription::IsLoopbackDevice(device_id)) {
        return AudioParameters(
            AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kLoopbackSampleRate, ChooseBufferSize(true, kLoopbackSampleRate));
    }
    ...
    if (device == kAudioObjectUnknown) {
        ...
        return AudioParameters(
            AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kFallbackSampleRate, ChooseBufferSize(true, kFallbackSampleRate));
    }
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config, sample_rate, buffer_size,
        AudioParameters::HardwareCapabilities(GetMinAudioBufferSizeMacOS(limits::kMinAudioBufferSize, sample_rate), limits::kMaxAudioBufferSize));
    ...
    if (media::IsSystemEchoCancellationEnforced() && (params.channel_layout() == CHANNEL_LAYOUT_MONO || params.channel_layout() == CHANNEL_LAYOUT_STEREO)
...
std::string AudioManagerMac::GetAssociatedOutputDeviceID(const std::string& input_device_unique_id)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    AudioObjectID input_device_id = GetAudioDeviceIdByUId(true, input_device_unique_id);
    if (input_device_id == kAudioObjectUnknown) {
        return std::string();
    }

    std::vector<AudioObjectID> related_device_ids = GetRelatedDeviceIDs(input_device_id);
    ...
    if (related_output_device_ids.size() == 1) {
        std::optional<std::string> related_unique_id = GetDeviceUniqueID(*related_output_device_ids.begin());
...
AudioOutputStream* AudioManagerMac::MakeLinearOutputStream(const AudioParameters& params, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    return MakeLowLatencyOutputStream(params, std::string(), log_callback);
}

AudioOutputStream* AudioManagerMac::MakeLowLatencyOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    if (!output_device_listener_) {
        ...
        output_device_listener_ = AudioDeviceListenerMac::Create(
            base::BindPostTaskToCurrentDefault(base::BindRepeating(&AudioManagerMac::HandleDeviceChanges, weak_ptr_factory_.GetWeakPtr())),
...
std::string AudioManagerMac::GetDefaultDeviceID(bool is_input)
{
    ...
    const AudioObjectPropertyAddress property_address = { kAudioDevicePropertyDeviceUID, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMain };
    CFStringRef device_uid = NULL;
    UInt32 size = sizeof(device_uid);
    OSStatus status = AudioObjectGetPropertyData(device_id, &property_address, 0, NULL, &size, &device_uid);
...
AudioInputStream* AudioManagerMac::MakeLinearInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
...
AudioInputStream* AudioManagerMac::MakeLowLatencyInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    if (AudioDeviceDescription::IsLoopbackDevice(device_id)) {
        ...
        return CreateSCKAudioInputStream(params, device_id, log_callback, base::BindRepeating(&AudioManagerBase::ReleaseInputStream, base::Unretained(this)));
    }
    ...
    VoiceProcessingMode voice_processing_mode
        = (params.effects() & AudioParameters::ECHO_CANCELLER) ? VoiceProcessingMode::kEnabled : VoiceProcessingMode::kDisabled;

    auto* stream = new AUAudioInputStream(this, params, audio_device_id, log_callback, voice_processing_mode);
    low_latency_input_streams_.insert(stream);
    return stream;
}

AudioParameters AudioManagerMac::GetPreferredOutputStreamParameters(const std::string& output_device_id, const AudioParameters& input_params)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    ...
    if (device == kAudioObjectUnknown) {
        ...
        return input_params.IsValid() ? input_params
                                      : AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), kFallbackSampleRate,
    ...
    if (has_valid_input_params) {
        ...
        // changed here.
        const int scaled_buffer_size = input_params.frames_per_buffer() * hardware_sample_rate / input_params.sample_rate();
        ...
        buffer_size = std::min(static_cast<int>(limits::kMaxAudioBufferSize), std::max(scaled_buffer_size, static_cast<int>(limits::kMinAudioBufferSize)));
    } else {
    ...
    if (!GetOutputDeviceChannelsAndLayout(device, &hardware_channels, &hardware_channel_layout)) {
        hardware_channels = 2;
    ...
    AudioParameters params(AudioParameters::AUDIO_PCM_LOW_LATENCY, { output_channel_layout, output_channels }, hardware_sample_rate, buffer_size,
        AudioParameters::HardwareCapabilities(GetMinAudioBufferSizeMacOS(limits::kMinAudioBufferSize, hardware_sample_rate), limits::kMaxAudioBufferSize));
    return params;
}
...
void AudioManagerMac::HandleDeviceChanges()
{
    ...
    DVLOG(1) << __func__ << " device changed: " << (current_sample_rate_ != new_sample_rate) << " current sample rate: " << current_sample_rate_
...
int AudioManagerMac::ChooseBufferSize(bool is_input, int sample_rate)
{
    ...
    // for details.
    int buffer_size = is_input ? limits::kMinAudioBufferSize : 2 * limits::kMinAudioBufferSize;
    const int user_buffer_size = GetUserBufferSize();
    buffer_size = user_buffer_size ? user_buffer_size : GetMinAudioBufferSizeMacOS(buffer_size, sample_rate);
    return buffer_size;
}
...
double AudioManagerMac::GetMaxInputVolume(AudioDeviceID device_id)
{
    ...
    for (int channel = 0; channel <= GetNumberOfChannelsForDevice(device_id); ++channel) {
...
bool AudioManagerMac::MaybeChangeBufferSize(AudioDeviceID device_id, AudioUnit audio_unit, AudioUnitElement element, size_t desired_buffer_size)
{
    DCHECK(GetTaskRunner()->BelongsToCurrentThread());
    if (in_shutdown_) {
    ...
    DVLOG(1) << __FUNCTION__ << " (id=0x" << std::hex << device_id << ", is_input=" << is_input << ", desired_buffer_size=" << std::dec << desired_buffer_size
             << ")";
    ...
    DVLOG(1) << __FUNCTION__ << " name: " << device_name << " (ID: 0x" << std::hex << device_id << ")";
    ...
    OSStatus result
        = AudioUnitGetProperty(audio_unit, kAudioDevicePropertyBufferFrameSize, kAudioUnitScope_Global, AUElement::OUTPUT, &buffer_size, &property_size);
    if (result != noErr) {
        OSSTATUS_DLOG(ERROR, result) << "AudioUnitGetProperty(kAudioDevicePropertyBufferFrameSize) failed.";
        return false;
    }

    DVLOG(1) << __FUNCTION__ << " current IO buffer size: " << buffer_size;
    DVLOG(1) << __FUNCTION__ << " #output streams: " << output_streams_.size();
    DVLOG(1) << __FUNCTION__ << " #input streams: " << low_latency_input_streams_.size();
    ...
    DVLOG(1) << __FUNCTION__ << " valid IO buffer size range: [" << minimum << ", " << maximum << "]";
    ...
    result = AudioUnitSetProperty(audio_unit, kAudioDevicePropertyBufferFrameSize, kAudioUnitScope_Global, 0, &buffer_size, sizeof(buffer_size));
    OSSTATUS_DLOG_IF(ERROR, result != noErr, result) << "AudioUnitSetProperty(kAudioDevicePropertyBufferFrameSize) failed.  "
                                                     << "Size:: " << buffer_size;
    DVLOG_IF(1, result == noErr) << __FUNCTION__ << " IO buffer size changed to: " << buffer_size;
    // Store the currently used (after a change) I/O buffer frame size.
    return result == noErr;
}

bool AudioManagerMac::DeviceSupportsAmbientNoiseReduction(AudioDeviceID device_id)
{
...
bool AudioManagerMac::SuppressNoiseReduction(AudioDeviceID device_id)
{
    ...
    if (state.suppression_count == 0) {
        ...
        OSStatus result = AudioObjectGetPropertyData(device_id, &kNoiseReductionPropertyAddress, 0, nullptr, &size, &initially_enabled);
        if (result != noErr) {
            return false;
        }
        ...
        if (initially_enabled) {
            ...
            result = AudioObjectSetPropertyData(device_id, &kNoiseReductionPropertyAddress, 0, nullptr, sizeof(disable), &disable);
            if (result != noErr) {
                OSSTATUS_DLOG(WARNING, result) << "Failed to disable ambient noise reduction for device: " << std::hex << device_id;
            }
...
void AudioManagerMac::UnsuppressNoiseReduction(AudioDeviceID device_id)
{
    ...
    if (state.suppression_count == 0) {
        if (state.initial_state == NoiseReductionState::ENABLED) {
            ...
            OSStatus result = AudioObjectSetPropertyData(device_id, &kNoiseReductionPropertyAddress, 0, nullptr, sizeof(enable), &enable);
            if (result != noErr) {
                OSSTATUS_DLOG(WARNING, result) << "Failed to re-enable ambient noise reduction for device: " << std::hex << device_id;
            }
        }
    }
}
...
void AudioManagerMac::ReleaseInputStream(AudioInputStream* stream)
{
    ...
    if (stream_it != basic_input_streams_.end()) {
        ...
        auto it = low_latency_input_streams_.find(static_cast<AUAudioInputStream*>(stream));
        if (it != low_latency_input_streams_.end()) {
            low_latency_input_streams_.erase(static_cast<AUAudioInputStream*>(stream));
        }
    }
...
std::unique_ptr<AudioManager> CreateAudioManager(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
{
    return std::make_unique<AudioManagerMac>(std::move(audio_thread), audio_log_factory);
}

// static
bool AudioManagerMac::IsVolumeSettableOnChannel(AudioDeviceID device_id, int channel)
{
    Boolean is_settable = false;
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyVolumeScalar, kAudioDevicePropertyScopeInput, static_cast<UInt32>(channel) };
    OSStatus result = AudioObjectIsPropertySettable(device_id, &property_address, &is_settable);
...
void AudioManagerMac::SetInputVolume(AudioDeviceID device_id, double volume)
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyVolumeScalar, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMain };

    // Try to set the volume for master volume channel.
    if (IsVolumeSettableOnChannel(device_id, kAudioObjectPropertyElementMain)) {
        OSStatus result = AudioObjectSetPropertyData(device_id, &property_address, 0, nullptr, sizeof(volume_float32), &volume_float32);
        if (result != noErr) {
    ...
    for (int channel = 1; channel <= GetNumberOfChannelsForDevice(device_id); ++channel) {
        property_address.mElement = static_cast<UInt32>(channel);
        if (IsVolumeSettableOnChannel(device_id, channel)) {
            OSStatus result = AudioObjectSetPropertyData(device_id, &property_address, 0, NULL, sizeof(volume_float32), &volume_float32);
            if (result == noErr) {
                ++successful_channels;
            }
        }
    ...
    DLOG_IF(WARNING, successful_channels == 0) << "Failed to set volume to " << volume_float32;
}
...
double AudioManagerMac::GetInputVolume(AudioDeviceID device_id)
{
    ...
    if (AudioObjectHasProperty(device_id, &property_address)) {
        ...
        OSStatus result = AudioObjectGetPropertyData(device_id, &property_address, 0, nullptr, &size, &volume_float32);
        if (result == noErr) {
        ...
        for (int i = 1; i <= GetNumberOfChannelsForDevice(device_id); ++i) {
            ...
            if (AudioObjectHasProperty(device_id, &property_address)) {
                ...
                OSStatus result = AudioObjectGetPropertyData(device_id, &property_address, 0, nullptr, &size, &channel_volume);
                if (result == noErr) {
...
bool AudioManagerMac::IsInputMuted(AudioDeviceID device_id)
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyMute, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMain };
    ...
    OSStatus result = AudioObjectGetPropertyData(device_id, &property_address, 0, nullptr, &size, &muted);
...
int AudioManagerMac::HardwareSampleRateForDevice(AudioDeviceID device_id)
{
    ...
    OSStatus result = AudioObjectGetPropertyData(device_id, &kNominalSampleRateAddress, 0, 0, &info_size, &nominal_sample_rate);
    if (result != noErr) {
        OSSTATUS_DLOG(WARNING, result) << "Could not get default sample rate for device: " << device_id << ", returning fallback sample rate "
                                       << kFallbackSampleRate;
...
// static
AudioDeviceID AudioManagerMac::FindFirstOutputSubdevice(AudioDeviceID aggregate_device_id)
{
    const AudioObjectPropertyAddress property_address
        = { kAudioAggregateDevicePropertyFullSubDeviceList, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMain };
    base::apple::ScopedCFTypeRef<CFArrayRef> subdevices;
    UInt32 size = sizeof(subdevices);
    OSStatus result = AudioObjectGetPropertyData(
        aggregate_device_id, &property_address, 0 /* inQualifierDataSize */, nullptr /* inQualifierData */, &size, subdevices.InitializeInto());

    if (result != noErr) {
        OSSTATUS_LOG(WARNING, result) << "Failed to read property " << kAudioAggregateDevicePropertyFullSubDeviceList << " for device " << aggregate_device_id;
    ...
    for (CFIndex i = 0; i < count; ++i) {
        CFStringRef value = base::apple::CFCast<CFStringRef>(CFArrayGetValueAtIndex(subdevices.get(), i));
        if (value) {
            std::string uid = base::SysCFStringRefToUTF8(value);
            output_subdevice_id = AudioManagerMac::GetAudioDeviceIdByUId(false, uid);
            if (output_subdevice_id != kAudioObjectUnknown && core_audio_mac::GetNumStreams(output_subdevice_id, false) > 0) {
...
OSStatus AudioManagerMac::GetInputDeviceStreamFormat(AudioUnit audio_unit, AudioStreamBasicDescription* input_format)
{
    DCHECK(audio_unit);
    ...
    return AudioUnitGetProperty(audio_unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, AUElement::INPUT, input_format, &property_size);
}
...
int AudioManagerMac::GetNumberOfChannelsForDevice(AudioDeviceID device_id)
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyStreamFormat, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMain };
    AudioStreamBasicDescription stream_format;
    UInt32 size = sizeof(stream_format);
    OSStatus result = AudioObjectGetPropertyData(device_id, &property_address, 0, nullptr, &size, &stream_format);
    if (result != noErr) {
