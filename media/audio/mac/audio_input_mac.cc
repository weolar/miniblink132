...
PCMQueueInAudioInputStream::PCMQueueInAudioInputStream(AudioManagerMac* manager, const AudioParameters& params)
    : manager_(manager)
    , callback_(nullptr)
    , audio_queue_(NULL)
    ...
    , audio_bus_(media::AudioBus::Create(params))
{
    ...
    format_.mBytesPerPacket = format_.mBytesPerFrame = params.GetBytesPerFrame(kSampleFormat);
    format_.mReserved = 0;
...
{
    OSStatus err = AudioQueueNewInput(&format_, &HandleInputBufferStatic, this,
    ...
    return SetupBuffers() ? AudioInputStream::OpenOutcome::kSuccess : AudioInputStream::OpenOutcome::kFailed;
}
...
void PCMQueueInAudioInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (manager_->ShouldDeferStreamStart()) {
        ...
        deferred_start_cb_.Reset(base::BindOnce(&PCMQueueInAudioInputStream::Start, base::Unretained(this), callback));
    ...
    input_callback_timer_->Start(FROM_HERE, base::Seconds(kInputCallbackStartTimeoutInSeconds), this, &PCMQueueInAudioInputStream::CheckInputStartupSuccess);
...
void PCMQueueInAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
...
bool PCMQueueInAudioInputStream::SetupBuffers()
{
    ...
    for (int i = 0; i < kNumberBuffers; ++i) {
        ...
        OSStatus err = AudioQueueAllocateBuffer(audio_queue_, buffer_size_bytes_, &buffer);
        if (err == noErr)
...
OSStatus PCMQueueInAudioInputStream::QueueNextBuffer(AudioQueueBufferRef audio_buffer)
{
...
// static
void PCMQueueInAudioInputStream::HandleInputBufferStatic(void* data, AudioQueueRef audio_queue, AudioQueueBufferRef audio_buffer,
    const AudioTimeStamp* start_time, UInt32 num_packets, const AudioStreamPacketDescription* desc)
{
    reinterpret_cast<PCMQueueInAudioInputStream*>(data)->HandleInputBuffer(audio_queue, audio_buffer, start_time, num_packets, desc);
}

void PCMQueueInAudioInputStream::HandleInputBuffer(AudioQueueRef audio_queue, AudioQueueBufferRef audio_buffer, const AudioTimeStamp* start_time,
    UInt32 num_packets, const AudioStreamPacketDescription* packet_desc)
{
    ...
    if (audio_buffer->mAudioDataByteSize) {
        ...
        if (elapsed < kMinDelay) {
            TRACE_EVENT0("audio", "PCMQueueInAudioInputStream::HandleInputBuffer sleep");
        ...
        const base::TimeTicks capture_time
            = start_time->mFlags & kAudioTimeStampHostTimeValid ? base::TimeTicks::FromMachAbsoluteTime(start_time->mHostTime) : base::TimeTicks::Now();

        uint8_t* audio_data = reinterpret_cast<uint8_t*>(audio_buffer->mAudioData);
        DCHECK_EQ(format_.mBitsPerChannel, 16u);
        audio_bus_->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<int16_t*>(audio_data), audio_bus_->frames());
...
void PCMQueueInAudioInputStream::CheckInputStartupSuccess()
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Media.Audio.InputStartupSuccessMac_HighLatency", input_callback_is_active);
    DVLOG(1) << "high_latency_input_callback_is_active: " << input_callback_is_active;
}
