...
using DispatchGetGlobalQueueFunc = dispatch_queue_t (*)(long id, unsigned long flags);
} // namespace
...
extern "C" void dyld_dynamic_interpose(const struct mach_header* mh, const struct dyld_interpose_tuple array[], size_t count) __attribute__((weak_import));
...
{
    UMA_HISTOGRAM_ENUMERATION("Media.Audio.CoreAudioDispatchOverrideInitResult", result, RESULT_MAX + 1);
}

enum CallsiteLookupEvent { LOOKUP_MISS = 0, LOOKUP_RESUMEIO_CALLSITE_FOUND = 1, LOOKUP_PAUSEIO_CALLSITE_FOUND = 2, LOOKUP_MAX = LOOKUP_PAUSEIO_CALLSITE_FOUND };

#if defined(ARCH_CPU_X86_64)
void LogCallsiteLookupEvent(CallsiteLookupEvent event)
{
    UMA_HISTOGRAM_ENUMERATION("Media.Audio.CoreAudioDispatchOverrideLookupEvent", event, LOOKUP_MAX + 1);
}
#endif

const char kCoreAudioPath[] = "/System/Library/Frameworks/CoreAudio.framework/Versions/A/CoreAudio";
...
bool AddressIsPauseOrResume(intptr_t address)
{
    ...
    intptr_t resumeio_callsite = base::subtle::NoBarrier_Load(&g_resumeio_callsite);
    ...
    if (!resumeio_callsite && info.dli_sname
        && (strcmp(info.dli_sname, "HALC_IOContext_ResumeIO") == 0 || strcmp(info.dli_sname, "_XIOContext_ResumeIO") == 0)) {
        resumeio_callsite = address;
        base::subtle::NoBarrier_CompareAndSwap(&g_resumeio_callsite, 0, resumeio_callsite);
        LogCallsiteLookupEvent(LOOKUP_RESUMEIO_CALLSITE_FOUND);
    } else if (!pauseio_callsite && info.dli_sname
        && (strcmp(info.dli_sname, "HALC_IOContext_PauseIO") == 0 || strcmp(info.dli_sname, "_XIOContext_PauseIO") == 0)) {
        pauseio_callsite = address;
        base::subtle::NoBarrier_CompareAndSwap(&g_pauseio_callsite, 0, pauseio_callsite);
...
dispatch_queue_t GetGlobalQueueOverride(long identifier, unsigned long flags)
{
    ...
    if (identifier == DISPATCH_QUEUE_PRIORITY_HIGH && AddressIsPauseOrResume(caller)) {
...
bool InitializeCoreAudioDispatchOverride()
{
    ...
    g_pause_resume_queue = dispatch_queue_create("org.chromium.CoreAudioPauseResumeQueue", nullptr);
    ...
    dyld_interpose_tuple interposition(&GetGlobalQueueOverride, reinterpret_cast<DispatchGetGlobalQueueFunc>(&dispatch_get_global_queue));
