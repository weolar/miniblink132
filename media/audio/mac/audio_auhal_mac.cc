...
static_assert(0 == LEFT && 1 == RIGHT && 2 == CENTER && 3 == LFE && 4 == BACK_LEFT && 5 == BACK_RIGHT && 6 == LEFT_OF_CENTER && 7 == RIGHT_OF_CENTER
        && 8 == BACK_CENTER && 9 == SIDE_LEFT && 10 == SIDE_RIGHT && 10 == CHANNELS_MAX,
    "Channel positions must match CoreAudio channel order.");

static void WrapBufferList(AudioBufferList* buffer_list, AudioBus* bus, int frames)
{
...
static bool SetStreamFormat(int channels, int sample_rate, AudioUnit audio_unit, AudioStreamBasicDescription* format)
{
...
static void SetAudioChannelLayout(int channels, ChannelLayout channel_layout, AudioUnit audio_unit)
{
    DCHECK(audio_unit);
    ...
    const size_t layout_size = offsetof(AudioChannelLayout, mChannelDescriptions[channels]);
    std::unique_ptr<uint8_t[]> layout_storage(new uint8_t[layout_size]);
    memset(layout_storage.get(), 0, layout_size);
    AudioChannelLayout* coreaudio_layout = reinterpret_cast<AudioChannelLayout*>(layout_storage.get());

    coreaudio_layout->mNumberChannelDescriptions = channels;
    coreaudio_layout->mChannelLayoutTag = kAudioChannelLayoutTag_UseChannelDescriptions;
    AudioChannelDescription* descriptions = coreaudio_layout->mChannelDescriptions;
    ...
    if (channel_layout == CHANNEL_LAYOUT_DISCRETE) {
        ...
        for (int ch = 0; ch < channels; ++ch) {
            descriptions[ch].mChannelLabel = ch > CHANNELS_MAX ? kAudioChannelLabel_Unknown : kCoreAudioChannelMapping[ch];
    ...
    OSStatus result
        = AudioUnitSetProperty(audio_unit, kAudioUnitProperty_AudioChannelLayout, kAudioUnitScope_Input, AUElement::OUTPUT, coreaudio_layout, layout_size);
    if (result != noErr) {
        OSSTATUS_DLOG(ERROR, result) << "Failed to set audio channel layout. Using default layout.";
    }
}

AUHALStream::AUHALStream(AudioManagerMac* manager, const AudioParameters& params, AudioDeviceID device, const AudioManager::LogCallback& log_callback)
    : manager_(manager)
    , params_(params)
...
bool AUHALStream::Open()
{
    ...
    if (configured) {
        ...
        hardware_latency_ = AudioManagerMac::GetHardwareLatency(audio_unit_->audio_unit(), device_, kAudioDevicePropertyScopeOutput, params_.sample_rate());
    }
...
{
    TRACE_EVENT2("audio", "AUHALStream::Render", "input buffer size", number_of_frames_, "output buffer size", number_of_frames);
    ...
    if (number_of_frames != number_of_frames_) {
        ...
        if (!audio_fifo_) {
            ...
            DVLOG(1) << "Audio frame size changed from " << number_of_frames_ << " to " << number_of_frames << " adding FIFO to compensate.";
            audio_fifo_ = std::make_unique<AudioPullFifo>(
                params_.channels(), number_of_frames_, base::BindRepeating(&AUHALStream::ProvideInput, base::Unretained(this)));
        }
    }
...
base::TimeTicks AUHALStream::GetPlayoutTime(const AudioTimeStamp* output_time_stamp)
{
    ...
    return std::max(base::TimeTicks::FromMachAbsoluteTime(output_time_stamp->mHostTime), base::TimeTicks::Now()) + hardware_latency_;
}
...
void AUHALStream::ReportAndResetStats()
{
    ...
    UMA_HISTOGRAM_COUNTS_1M("Media.Audio.Render.FramesRequested", number_of_frames_requested_);

    SystemGlitchReporter::Stats stats = glitch_reporter_.GetLongTermStatsAndReset();

    std::string log_message = base::StringPrintf("AU out: (num_glitches_detected=[%d], cumulative_audio_lost=[%llu ms], "
...
bool AUHALStream::ConfigureAUHAL()
{
    ...
    if (!manager_->MaybeChangeBufferSize(device_, local_audio_unit->audio_unit(), 0, number_of_frames_, &size_was_changed, &io_buffer_frame_size)) {
        return false;
    }
