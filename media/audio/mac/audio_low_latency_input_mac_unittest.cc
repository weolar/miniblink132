...
public:
    MOCK_METHOD3(OnData, void(const AudioBus* src, base::TimeTicks capture_time, double volume));
...
public:
    ...
    void OnData(const AudioBus* src, base::TimeTicks capture_time, double volume) override
    {
...
protected:
    ...
    AudioInputStream* CreateDefaultAudioInputStream()
    {
        ...
        AudioInputStream* ais = audio_manager_->MakeAudioInputStream(
            AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), fs, samples_per_packet),
            AudioDeviceDescription::kDefaultDeviceId, base::BindRepeating(&MacAudioInputTest::OnLogMessage, base::Unretained(this)));
        EXPECT_TRUE(ais);
        return ais;
    }
    ...
    AudioInputStream* CreateAudioInputStream(ChannelLayoutConfig channel_layout_config)
    {
        int fs = static_cast<int>(AUAudioInputStream::HardwareSampleRate());
        int samples_per_packet = fs / 100;
        AudioInputStream* ais
            = audio_manager_->MakeAudioInputStream(AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, channel_layout_config, fs, samples_per_packet),
                AudioDeviceDescription::kDefaultDeviceId, base::BindRepeating(&MacAudioInputTest::OnLogMessage, base::Unretained(this)));
        EXPECT_TRUE(ais);
        return ais;
    }
...
TEST_F(MacAudioInputTest, AUAudioInputStreamVerifyMonoRecording)
{
    ...
    EXPECT_CALL(sink, OnData(NotNull(), _, _))
        .Times(AtLeast(10))
        .WillRepeatedly(CheckCountAndPostQuitTask(&count, 10, task_environment_.GetMainThreadTaskRunner(), run_loop.QuitClosure()));
    ais->Start(&sink);
    run_loop.Run();
    ais->Stop();
    ais->Close();
