...
static std::ostream& operator<<(std::ostream& os, const AudioStreamBasicDescription& format)
{
    ...
    os << "sample rate       : " << format.mSampleRate << std::endl
       ...
       << "interleaved       : " << (FormatIsInterleaved(format.mFormatFlags) ? "yes" : "no");
    return os;
}

static OSStatus OnGetPlayoutData(
    void* in_ref_con, AudioUnitRenderActionFlags* flags, const AudioTimeStamp* time_stamp, UInt32 bus_number, UInt32 num_frames, AudioBufferList* io_data)
{
    *flags |= kAudioUnitRenderAction_OutputIsSilence;
    return noErr;
}

static OSStatus GetInputDeviceStreamFormat(AudioUnit audio_unit, AudioStreamBasicDescription* format)
{
    DCHECK(audio_unit);
    ...
    OSStatus result = AudioUnitGetProperty(audio_unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 1, format, &property_size);
...
static int NumberOfPhysicalProcessors()
{
    ...
    kern_return_t kr = host_info(mach_host.get(), HOST_BASIC_INFO, reinterpret_cast<host_info_t>(&hbi), &info_count);
...
static void AddSystemInfoToUMA()
{
    ...
    UMA_HISTOGRAM_COUNTS_1M("Media.Audio.LogicalProcessorsMac", base::SysInfo::NumberOfProcessors());
    // Number of physical processors/cores on the current machine.
    UMA_HISTOGRAM_COUNTS_1M("Media.Audio.PhysicalProcessorsMac", NumberOfPhysicalProcessors());
...
static AudioDeviceID FindFirstOutputSubdevice(AudioDeviceID aggregate_device_id)
{
    const AudioObjectPropertyAddress property_address
        = { kAudioAggregateDevicePropertyFullSubDeviceList, kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster };
    ...
    for (CFIndex i = 0; i != count; ++i) {
        CFStringRef value = base::mac::CFCast<CFStringRef>(CFArrayGetValueAtIndex(subdevices, i));
        if (value) {
...
AUAudioInputStream::AUAudioInputStream(AudioManagerMac* manager, const AudioParameters& input_params, AudioDeviceID audio_device_id,
    const AudioManager::LogCallback& log_callback, AudioManagerBase::VoiceProcessingMode voice_processing_mode)
    : manager_(manager)
    ...
    , fifo_(input_params.channels(), input_params.frames_per_buffer(), kNumberOfBlocksBufferInFifo)
    ...
    , use_voice_processing_(voice_processing_mode == AudioManagerBase::VoiceProcessingMode::kEnabled)
    ...
    , log_callback_(log_callback)
{
    ...
    if (use_voice_processing_) {
        ...
        const bool got_default_device = AudioManagerMac::GetDefaultOutputDevice(&output_device_id_for_aec_);
    ...
    format_.mBytesPerPacket = format_.mBytesPerFrame = input_params.GetBytesPerFrame(kSampleFormat);
    format_.mReserved = 0;
    ...
    UInt32 data_byte_size = input_params.frames_per_buffer() * format_.mBytesPerFrame;
...
AudioInputStream::OpenOutcome AUAudioInputStream::Open()
{
    ...
    const int sample_rate = AudioManagerMac::HardwareSampleRateForDevice(input_device_id_);
    DCHECK_EQ(sample_rate, format_.mSampleRate);

    log_callback_.Run(base::StrCat({ "AU in: Open using ", use_voice_processing_ ? "VPAU" : "AUHAL" }));

    const bool success = use_voice_processing_ ? OpenVoiceProcessingAU() : OpenAUHAL();

    if (!success)
        return OpenOutcome::kFailed;

    // The hardware latency is fixed and will not change during the call.
    hardware_latency_ = AudioManagerMac::GetHardwareLatency(audio_unit_, input_device_id_, kAudioDevicePropertyScopeInput, format_.mSampleRate);
...
bool AUAudioInputStream::OpenAUHAL()
{
    ...
    // Enable input on the AUHAL.
    {
        ...
        result = AudioUnitSetProperty(audio_unit_, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, AUElement::INPUT, &enableIO, sizeof(enableIO));
        if (result != noErr) {
            HandleError(result);
            return false;
        }
    ...
    // Disable output on the AUHAL.
    {
        ...
        result = AudioUnitSetProperty(audio_unit_, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, AUElement::OUTPUT, &disableIO, sizeof(disableIO));
        if (result != noErr) {
            HandleError(result);
            return false;
        }
    ...
    result = AudioUnitSetProperty(
        audio_unit_, kAudioOutputUnitProperty_CurrentDevice, kAudioUnitScope_Global, AUElement::OUTPUT, &input_device_id_, sizeof(input_device_id_));
    ...
    result
        = AudioUnitSetProperty(audio_unit_, kAudioOutputUnitProperty_SetInputCallback, kAudioUnitScope_Global, AUElement::OUTPUT, &callback, sizeof(callback));
    ...
    if (!manager_->MaybeChangeBufferSize(
            input_device_id_, audio_unit_, 1, input_params_.frames_per_buffer(), &buffer_size_was_changed_, &io_buffer_frame_size)) {
        result = kAudioUnitErr_FormatNotSupported;
        HandleError(result);
        return false;
    }
    ...
    result
        = AudioUnitGetProperty(audio_unit_, kAudioDevicePropertyBufferFrameSize, kAudioUnitScope_Global, AUElement::OUTPUT, &buffer_frame_size, &property_size);
    LOG_IF(WARNING, buffer_frame_size != static_cast<UInt32>(input_params_.frames_per_buffer()))
    ...
    DLOG_IF(WARNING, input_device_format.mChannelsPerFrame != format_.mChannelsPerFrame) << "AUHAL's audio converter must do channel conversion";
    ...
    result = AudioUnitSetProperty(audio_unit_, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &format_, sizeof(format_));
    if (result != noErr) {
        HandleError(result);
        return false;
    }
...
bool AUAudioInputStream::OpenVoiceProcessingAU()
{
    ...
    result = AudioUnitSetProperty(
        audio_unit_, kAudioOutputUnitProperty_CurrentDevice, kAudioUnitScope_Global, AUElement::INPUT, &input_device_id_, sizeof(input_device_id_));
    ...
    result = AudioUnitSetProperty(audio_unit_, kAudioOutputUnitProperty_CurrentDevice, kAudioUnitScope_Global, AUElement::OUTPUT, &output_device_id_for_aec_,
    ...
    result
        = AudioUnitSetProperty(audio_unit_, kAudioOutputUnitProperty_SetInputCallback, kAudioUnitScope_Global, AUElement::INPUT, &callback, sizeof(callback));
    if (result != noErr) {
        HandleError(result);
        return false;
    }
    ...
    callback.inputProcRefCon = this;
    result = AudioUnitSetProperty(audio_unit_, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, AUElement::OUTPUT, &callback, sizeof(callback));
    if (result != noErr) {
        HandleError(result);
        return false;
    }
    ...
    GetInputDeviceStreamFormat(audio_unit_, &input_device_format);
    if (input_device_format.mSampleRate != format_.mSampleRate) {
        LOG(ERROR) << "Input device's sample rate does not match the client's sample rate";
        result = kAudioUnitErr_FormatNotSupported;
        HandleError(result);
        return false;
    }
    ...
    // For obtaining input from a device, the device format is always expressed
    // on the output scope of the AUHAL's Element 1.
    result = AudioUnitSetProperty(audio_unit_, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, AUElement::INPUT, &mono_format, sizeof(mono_format));
    if (result != noErr) {
        HandleError(result);
        return false;
    }

    result = AudioUnitSetProperty(audio_unit_, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, AUElement::OUTPUT, &mono_format, sizeof(mono_format));
    if (result != noErr) {
        HandleError(result);
        return false;
    }
...
void AUAudioInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (manager_->ShouldDeferStreamStart()) {
        ...
        deferred_start_cb_.Reset(base::BindOnce(&AUAudioInputStream::Start, base::Unretained(this), callback));
        manager_->GetTaskRunner()->PostDelayedTask(FROM_HERE, deferred_start_cb_.callback(), base::Seconds(AudioManagerMac::kStartDelayInSecsForPowerEvents));
        return;
    }
    ...
    if (!use_voice_processing_ && !(input_params_.effects() & AudioParameters::NOISE_SUPPRESSION)
        && manager_->DeviceSupportsAmbientNoiseReduction(input_device_id_)) {
        noise_reduction_suppressed_ = manager_->SuppressNoiseReduction(input_device_id_);
    }
    StartAgc();
    OSStatus result = AudioOutputUnitStart(audio_unit_);
    OSSTATUS_DLOG_IF(ERROR, result != noErr, result) << "Failed to start acquiring data";
    if (result != noErr) {
        Stop();
        return;
    }
    ...
    input_callback_timer_->Start(FROM_HERE, base::Seconds(kInputCallbackStartTimeoutInSeconds), this, &AUAudioInputStream::CheckInputStartupSuccess);
...
void AUAudioInputStream::Stop()
{
    ...
    if (audio_unit_ != nullptr) {
        ...
        DCHECK_EQ(result, noErr);
        OSSTATUS_DLOG_IF(ERROR, result != noErr, result) << "Failed to stop acquiring data";
    }
...
void AUAudioInputStream::SetVolume(double volume)
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyVolumeScalar, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMaster };

    // Try to set the volume for master volume channel.
    if (IsVolumeSettableOnChannel(kAudioObjectPropertyElementMaster)) {
        OSStatus result = AudioObjectSetPropertyData(input_device_id_, &property_address, 0, nullptr, sizeof(volume_float32), &volume_float32);
        if (result != noErr) {
    ...
    for (int i = 1; i <= number_of_channels_in_frame_; ++i) {
        ...
        if (IsVolumeSettableOnChannel(i)) {
            OSStatus result = AudioObjectSetPropertyData(input_device_id_, &property_address, 0, NULL, sizeof(volume_float32), &volume_float32);
            if (result == noErr)
                ++successful_channels;
        }
    }
...
double AUAudioInputStream::GetVolume()
{
    ...
    if (AudioObjectHasProperty(input_device_id_, &property_address)) {
        ...
        OSStatus result = AudioObjectGetPropertyData(input_device_id_, &property_address, 0, nullptr, &size, &volume_float32);
        if (result == noErr)
        ...
        for (int i = 1; i <= number_of_channels_in_frame_; ++i) {
            ...
            if (AudioObjectHasProperty(input_device_id_, &property_address)) {
                ...
                OSStatus result = AudioObjectGetPropertyData(input_device_id_, &property_address, 0, nullptr, &size, &channel_volume);
                if (result == noErr) {
...
bool AUAudioInputStream::IsMuted()
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyMute, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMaster };
    ...
    OSStatus result = AudioObjectGetPropertyData(input_device_id_, &property_address, 0, nullptr, &size, &muted);
    DLOG_IF(WARNING, result != noErr) << "Failed to get mute state";
    return result == noErr && muted != 0;
}

void AUAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
    if (!use_voice_processing_)
        return;

    AudioDeviceID audio_device_id = AudioManagerMac::GetAudioDeviceIdByUId(false, output_device_id);
    if (audio_device_id == output_device_id_for_aec_)
        return;

    if (audio_device_id == kAudioObjectUnknown) {
        log_callback_.Run(base::StringPrintf("AU in: Unable to resolve output device id '%s'", output_device_id.c_str()));
        return;
    }

    // If the selected device is an aggregate device, try to use the first output
    // device of the aggregate device instead.
    if (core_audio_mac::GetDeviceTransportType(audio_device_id) == kAudioDeviceTransportTypeAggregate) {
        const AudioDeviceID output_subdevice_id = FindFirstOutputSubdevice(audio_device_id);

        if (output_subdevice_id == kAudioObjectUnknown) {
            log_callback_.Run(base::StringPrintf("AU in: Unable to find an output subdevice in aggregate device '%s'", output_device_id.c_str()));
            return;
        }
    ...
    if (audio_device_id != output_device_id_for_aec_) {
        ...
        log_callback_.Run(base::StringPrintf("AU in: Output device for AEC changed to '%s' (%d)", output_device_id.c_str(), output_device_id_for_aec_));
...
void AUAudioInputStream::ReinitializeVoiceProcessingAudioUnit()
{
    ...
    log_callback_.Run(base::StringPrintf("AU in: Successfully reinitialized AEC for output device id=%d.", output_device_id_for_aec_));
}

// static
OSStatus AUAudioInputStream::DataIsAvailable(
    void* context, AudioUnitRenderActionFlags* flags, const AudioTimeStamp* time_stamp, UInt32 bus_number, UInt32 number_of_frames, AudioBufferList* io_data)
{
    DCHECK(context);
...
OSStatus AUAudioInputStream::OnDataIsAvailable(AudioUnitRenderActionFlags* flags, const AudioTimeStamp* time_stamp, UInt32 bus_number, UInt32 number_of_frames)
{
    ...
    if (use_voice_processing_ && format_.mChannelsPerFrame != 1) {
        ...
        mono_buffer->mDataByteSize = audio_buffer->mDataByteSize / audio_buffer->mNumberChannels;
        mono_buffer->mData = audio_buffer->mData;

        TRACE_EVENT_BEGIN0("audio", "AudioUnitRender");
        result = AudioUnitRender(audio_unit_, flags, time_stamp, bus_number, number_of_frames, &mono_buffer_list);
        ...
        TRACE_EVENT_BEGIN0("audio", "AudioUnitRender");
        result = AudioUnitRender(audio_unit_, flags, time_stamp, bus_number, number_of_frames, &audio_buffer_list_);
        TRACE_EVENT_END0("audio", "AudioUnitRender");
    }
    ...
    if (result) {
        TRACE_EVENT_INSTANT0("audio", "AudioUnitRender error", TRACE_EVENT_SCOPE_THREAD);
        ...
        if (result == kAudioUnitErr_TooManyFramesToProcess || result == kAudioUnitErr_CannotDoInCurrentContext) {
            ...
            if ((time_since_last_success > base::Seconds(kMaxErrorTimeoutInSeconds))) {
                const char* err
                    = (result == kAudioUnitErr_TooManyFramesToProcess) ? "kAudioUnitErr_TooManyFramesToProcess" : "kAudioUnitErr_CannotDoInCurrentContext";
            ...
            if (result == kAudioUnitErr_CannotDoInCurrentContext && add_uma_histogram) {
                UMA_HISTOGRAM_BOOLEAN("Media.Audio.RenderFailsWhenBufferSizeChangesMac", new_buffer_size_detected);
                UMA_HISTOGRAM_BOOLEAN("Media.Audio.AudioUnitRenderHasWorkedMac", audio_unit_render_has_worked_);
            }
        } else {
...
OSStatus AUAudioInputStream::Provide(UInt32 number_of_frames, AudioBufferList* io_data, const AudioTimeStamp* time_stamp)
{
    TRACE_EVENT1("audio", "AUAudioInputStream::Provide", "number_of_frames", number_of_frames);
    ...
    if (number_of_frames != static_cast<UInt32>(input_params_.frames_per_buffer()) && number_of_frames_provided_ == 0)
    ...
    if (static_cast<int>(number_of_frames) > fifo_.GetUnfilledFrames()) {
        ...
        // typically one block.
        const int blocks = static_cast<int>((number_of_frames - fifo_.GetUnfilledFrames()) / input_params_.frames_per_buffer()) + 1;
        DLOG(WARNING) << "Increasing FIFO capacity by " << blocks << " blocks";
        TRACE_EVENT_INSTANT1("audio", "Increasing FIFO capacity", TRACE_EVENT_SCOPE_THREAD, "increased by", blocks);
        fifo_.IncreaseCapacity(blocks);
    }

    // Compensate the capture time for the FIFO before pushing an new frames.
    capture_time -= AudioTimestampHelper::FramesToTime(fifo_.GetAvailableFrames(), format_.mSampleRate);
    ...
    while (fifo_.available_blocks()) {
        ...
        DCHECK_EQ(audio_bus->frames(), static_cast<int>(input_params_.frames_per_buffer()));

        sink_->OnData(audio_bus, capture_time, normalized_volume);

        // Move the capture time forward for each vended block.
        capture_time += AudioTimestampHelper::FramesToTime(audio_bus->frames(), format_.mSampleRate);
    }
...
int AUAudioInputStream::HardwareSampleRate()
{
    ...
    OSStatus result = AudioObjectGetPropertyData(kAudioObjectSystemObject, &default_input_device_address, 0, 0, &info_size, &device_id);
    if (result != noErr)
        return 0.0;
    ...
    result = AudioObjectGetPropertyData(device_id, &nominal_sample_rate_address, 0, 0, &info_size, &nominal_sample_rate);
    if (result != noErr)
        return 0.0;
...
base::TimeTicks AUAudioInputStream::GetCaptureTime(const AudioTimeStamp* input_time_stamp)
{
    ...
    return (input_time_stamp->mFlags & kAudioTimeStampHostTimeValid ? base::TimeTicks::FromMachAbsoluteTime(input_time_stamp->mHostTime)
                                                                    : base::TimeTicks::Now())
        - hardware_latency_;
}
...
int AUAudioInputStream::GetNumberOfChannelsFromStream()
{
    ...
    AudioObjectPropertyAddress property_address = { kAudioDevicePropertyStreamFormat, kAudioDevicePropertyScopeInput, kAudioObjectPropertyElementMaster };
    AudioStreamBasicDescription stream_format;
    UInt32 size = sizeof(stream_format);
    OSStatus result = AudioObjectGetPropertyData(input_device_id_, &property_address, 0, nullptr, &size, &stream_format);
    if (result != noErr) {
...
bool AUAudioInputStream::IsRunning()
{
    ...
    OSStatus error = AudioUnitGetProperty(audio_unit_, kAudioOutputUnitProperty_IsRunning, kAudioUnitScope_Global, 0, &is_running, &size);
    OSSTATUS_DLOG_IF(ERROR, error != noErr, error) << "AudioUnitGetProperty(kAudioOutputUnitProperty_IsRunning) failed";
...
void AUAudioInputStream::HandleError(OSStatus err)
{
    ...
    base::UmaHistogramSparse("Media.InputErrorMac", GetInputCallbackIsActive() ? err : (err * -1));
    NOTREACHED() << "error " << logging::DescriptionFromOSStatus(err) << " (" << err << ")";
    if (sink_)
        sink_->OnError();
}
...
bool AUAudioInputStream::IsVolumeSettableOnChannel(int channel)
{
    ...
    OSStatus result = AudioObjectIsPropertySettable(input_device_id_, &property_address, &is_settable);
...
void AUAudioInputStream::CheckInputStartupSuccess()
{
    ...
    if (GetAutomaticGainControl()) {
        ...
        UMA_HISTOGRAM_BOOLEAN("Media.Audio.InputStartupSuccessMac", input_callback_is_active);
...
{
    DCHECK(thread_checker_.CalledOnValidThread());
    UMA_HISTOGRAM_BOOLEAN("Media.Audio.InputBufferSizeWasChangedMac", buffer_size_was_changed_);
    ...
    base::UmaHistogramSparse("Media.Audio.RequestedInputBufferFrameSizeMac", input_params_.frames_per_buffer());
    ...
    // buffer size.
    base::UmaHistogramSparse("Media.Audio.ActualInputBufferFrameSizeMac", io_buffer_frame_size_);
    DVLOG(1) << "io_buffer_frame_size_: " << io_buffer_frame_size_;
    // Add information about things like number of logical processors etc.
    AddSystemInfoToUMA();
}

void AUAudioInputStream::UpdateCaptureTimestamp(const AudioTimeStamp* timestamp)
{
    ...
    if (last_sample_time_) {
        ...
        UInt32 sample_time_diff = static_cast<UInt32>(timestamp->mSampleTime - last_sample_time_);
        DCHECK_GE(sample_time_diff, last_number_of_frames_);
        UInt32 lost_frames = sample_time_diff - last_number_of_frames_;
        base::TimeDelta lost_audio_duration = AudioTimestampHelper::FramesToTime(lost_frames, input_params_.sample_rate());
...
void AUAudioInputStream::ReportAndResetStats()
{
    ...
    UMA_HISTOGRAM_COUNTS_10000("Media.Audio.Capture.FramesProvided", number_of_frames_provided_);

    SystemGlitchReporter::Stats stats = glitch_reporter_.GetLongTermStatsAndReset();

    std::string log_message = base::StringPrintf("AU in: (num_glitches_detected=[%d], cumulative_audio_lost=[%llu ms], "
                                                 "largest_glitch=[%llu ms])",
        stats.glitches_detected, stats.total_glitch_duration.InMilliseconds(), stats.largest_glitch_duration.InMilliseconds());
...
void AUAudioInputStream::UpmixMonoToStereoInPlace(AudioBuffer* audio_buffer, int bytes_per_sample)
{
