...
public:
    AudioManagerMac(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory);
    ...
    AudioOutputStream* MakeLinearOutputStream(const AudioParameters& params, const LogCallback& log_callback) override;
    AudioOutputStream* MakeLowLatencyOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) override;
    AudioInputStream* MakeLinearInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) override;
    AudioInputStream* MakeLowLatencyInputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) override;
    ...
    bool MaybeChangeBufferSize(AudioDeviceID device_id, AudioUnit audio_unit, AudioUnitElement element, size_t desired_buffer_size) override;
    ...
    // will be returned.
    OSStatus GetInputDeviceStreamFormat(AudioUnit audio_unit, AudioStreamBasicDescription* input_format) override;
    ...
    static AudioDeviceID GetAudioDeviceIdByUId(bool is_input, const std::string& device_id);
    ...
    size_t output_streams() const
    {
        return output_streams_.size();
    }
    size_t low_latency_input_streams() const
    {
        return low_latency_input_streams_.size();
    }
    size_t basic_input_streams() const
    {
        return basic_input_streams_.size();
    }
...
protected:
    ...
    std::vector<AudioObjectID> GetRelatedBluetoothDeviceIDs(AudioObjectID device_id);
    ...
    virtual std::vector<AudioObjectID> GetRelatedNonBluetoothDeviceIDs(AudioObjectID device_id);
    ...
    virtual std::optional<uint32_t> GetDeviceTransportType(AudioObjectID device_id);
...
private:
    ...
    std::unique_ptr<base::apple::ScopedObjCClassSwizzler> screen_capture_kit_swizzler_;
