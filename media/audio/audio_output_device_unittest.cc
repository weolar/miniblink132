...
public:
    ...
    MOCK_METHOD4(Render, int(base::TimeDelta delay, base::TimeTicks timestamp, const AudioGlitchInfo& glitch_info, AudioBus* dest));
...
public:
    ...
    MOCK_METHOD3(RequestDeviceAuthorization, void(AudioOutputIPCDelegate* delegate, const base::UnguessableToken& session_id, const std::string& device_id));
    MOCK_METHOD2(CreateStream, void(AudioOutputIPCDelegate* delegate, const AudioParameters& params));
...
public:
    ...
    void CreateDevice(const std::string& device_id, base::TimeDelta timeout = kAuthTimeout);
...
{
    default_audio_parameters_.Reset(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 48000, 1024);
    SetDevice(kDefaultDeviceId);
}

AudioOutputDeviceTest::~AudioOutputDeviceTest() = default;

void AudioOutputDeviceTest::CreateDevice(const std::string& device_id, base::TimeDelta timeout)
{
    // Make sure the previous device is properly cleaned up.
    if (audio_device_)
        StopAudioDevice();

    audio_device_ = new AudioOutputDevice(std::make_unique<NiceMock<MockAudioOutputIPC>>(), task_env_.GetMainThreadTaskRunner(),
...
void AudioOutputDeviceTest::SetDevice(const std::string& device_id)
{
    ...
    EXPECT_CALL(*audio_output_ipc(), RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), device_id));
    audio_device_->RequestDeviceAuthorization();
    task_env_.FastForwardBy(base::TimeDelta());

    // Simulate response from browser
    OutputDeviceStatus device_status = (device_id == kUnauthorizedDeviceId) ? OUTPUT_DEVICE_STATUS_ERROR_NOT_AUTHORIZED : OUTPUT_DEVICE_STATUS_OK;
...
void AudioOutputDeviceTest::ReceiveAuthorization(OutputDeviceStatus status)
{
    ...
    audio_device_->OnDeviceAuthorized(device_status_, default_audio_parameters_, kDefaultDeviceId);
    task_env_.FastForwardBy(base::TimeDelta());
}
...
{
    const uint32_t kMemorySize = ComputeAudioOutputBufferSize(default_audio_parameters_);
    ...
    ASSERT_TRUE(CancelableSyncSocket::CreatePair(&browser_socket_, &renderer_socket_));
    ...
    // freeing.
    base::UnsafeSharedMemoryRegion duplicated_memory_region = shared_memory_region_.Duplicate();
    ASSERT_TRUE(duplicated_memory_region.IsValid());

    audio_device_->OnStreamCreated(std::move(duplicated_memory_region), renderer_socket_.Take(),
...
TEST_F(AudioOutputDeviceTest, NoErrorForNormalShutdown)
{
    ...
    EXPECT_CALL(callback_, Render(_, _, _, _)).WillOnce(DoAll(base::test::RunClosure(run_loop.QuitWhenIdleClosure()), Return(0))).WillRepeatedly(Return(0));
...
TEST_F(AudioOutputDeviceTest, ErrorFiredForSocketClose)
{
    ...
    EXPECT_CALL(callback_, OnRenderError()).WillOnce(base::test::RunClosure(run_loop.QuitWhenIdleClosure()));
...
TEST_F(AudioOutputDeviceTest, NonDefaultStartStopStartStop)
{
    ...
    EXPECT_CALL(*audio_output_ipc(), RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), _));
    StartAudioDevice();
...
TEST_F(AudioOutputDeviceTest, StartUnauthorizedDeviceAndStopBeforeErrorFires_NoError)
{
    SetDevice(kUnauthorizedDeviceId);
    audio_device_->Start();
...
TEST_F(AudioOutputDeviceTest, AuthorizationFailsBeforeInitialize_NoError)
{
    ...
    StopAudioDevice();
    audio_device_ = new AudioOutputDevice(std::make_unique<NiceMock<MockAudioOutputIPC>>(), task_env_.GetMainThreadTaskRunner(),
        AudioSinkParameters(base::UnguessableToken(), kDefaultDeviceId), kAuthTimeout);
    EXPECT_CALL(*audio_output_ipc(), RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kDefaultDeviceId));
    ...
    EXPECT_CALL(callback_, OnRenderError()).Times(0);
    audio_device_->OnDeviceAuthorized(OUTPUT_DEVICE_STATUS_ERROR_NOT_AUTHORIZED, default_audio_parameters_, kDefaultDeviceId);
    task_env_.FastForwardBy(base::TimeDelta());
}
...
TEST_F(AudioOutputDeviceTest, AuthorizationTimedOut)
{
    ...
    EXPECT_CALL(*audio_output_ipc(), RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kNonDefaultDeviceId));
    EXPECT_CALL(*audio_output_ipc(), CloseStream());
...
TEST_F(AudioOutputDeviceTest, GetOutputDeviceInfoAsync_Error)
{
    ...
    EXPECT_CALL(*audio_output_ipc(), RequestDeviceAuthorization(audio_device_.get(), base::UnguessableToken(), kUnauthorizedDeviceId));
    audio_device_->RequestDeviceAuthorization();
    audio_device_->GetOutputDeviceInfoAsync(base::BindOnce(&AudioOutputDeviceTest::OnDeviceInfoReceived, base::Unretained(this)));
    task_env_.FastForwardBy(base::TimeDelta());
    ...
    EXPECT_TRUE(AudioParameters::UnavailableDeviceParams().Equals(info.output_params()));
