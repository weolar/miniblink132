...
// in |read_length|.
std::unique_ptr<char[]> ReadWavFile(const base::FilePath& wav_filename, size_t* read_length)
{
    base::File wav_file(wav_filename, base::File::FLAG_OPEN | base::File::FLAG_READ);
    if (!wav_file.IsValid()) {
    ...
    if (wav_file_length == 0) {
        LOG(ERROR) << "Input file to fake device is empty: " << wav_filename.value();
        return nullptr;
    }
...
SineWaveAudioSource::SineWaveAudioSource(int channels, double freq, double sample_freq)
    : channels_(channels)
    , f_(freq / sample_freq)
{
}
...
int SineWaveAudioSource::OnMoreData(
    base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest)
{
    int max_frames;

    {
        ...
        max_frames = cap_ > 0 ? std::min(dest->frames(), cap_ - pos_samples_) : dest->frames();
        for (int i = 0; i < max_frames; ++i)
            dest->channel(0)[i] = sin(2.0 * std::numbers::pi * f_ * pos_samples_++);
        for (int i = 1; i < dest->channels(); ++i) {
            memcpy(dest->channel(i), dest->channel(0), max_frames * sizeof(*dest->channel(i)));
        }
    }
...
FileSource::FileSource(const AudioParameters& params, const base::FilePath& path_to_wav_file, bool loop)
    : params_(params)
...
void FileSource::LoadWavFile(const base::FilePath& path_to_wav_file)
{
    ...
    wav_audio_handler_ = WavAudioHandler::Create(std::string_view(raw_wav_data_.get(), length));
    ...
    AudioParameters file_audio_slice(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Guess(wav_audio_handler_->GetNumChannels()),
        wav_audio_handler_->GetSampleRate(), params_.frames_per_buffer());

    file_audio_converter_ = std::make_unique<AudioConverter>(file_audio_slice, params_, false);
    file_audio_converter_->AddInput(this);
}

int FileSource::OnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest)
{
...
double FileSource::ProvideInput(AudioBus* audio_bus_into_converter, uint32_t frames_delayed, const AudioGlitchInfo&)
{
...
int BeepingSource::OnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest)
{
    ...
    if (beep_context->automatic_beep()) {
        base::TimeDelta delta = interval_from_last_beep_ - base::Milliseconds(kAutomaticBeepIntervalInMs);
    ...
    last_callback_time_ = base::TimeTicks::Now();
    dest->FromInterleaved<UnsignedInt8SampleTypeTraits>(buffer_.get(), dest->frames());
    return dest->frames();
}
