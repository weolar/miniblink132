...
public:
    ...
    MOCK_METHOD4(OnData, void(const AudioBus*, base::TimeTicks, double, const AudioGlitchInfo&));
...
public:
    ...
    void ExpectRecorderCreation(std::unique_ptr<AudioDebugRecorder> recorder_ptr)
    {
        DCHECK(!prepared_recorder_);
...
void TestSetAutomaticGainControl(bool enable, bool agc_is_supported)
{
    ...
    AudioInputStream* interceptor
        = new AudioInputStreamDataInterceptor(base::BindRepeating(&MockDebugRecorderFactory::CreateDebugRecorder, base::Unretained(&factory)), &stream);

    EXPECT_CALL(stream, SetAutomaticGainControl(enable)).WillOnce(Return(agc_is_supported));
...
TEST(AudioInputStreamDataInterceptorTest, SetAutomaticGainControlTrueWhenSupported)
{
    TestSetAutomaticGainControl(true, true);
}

TEST(AudioInputStreamDataInterceptorTest, SetAutomaticGainControlFalseWhenSupported)
{
    TestSetAutomaticGainControl(false, true);
}

TEST(AudioInputStreamDataInterceptorTest, SetAutomaticGainControlTrueWhenNotSupported)
{
    TestSetAutomaticGainControl(true, false);
}

TEST(AudioInputStreamDataInterceptorTest, SetAutomaticGainControlFalseWhenNotSupported)
{
