...
void AudioDebugRecordingManager::EnableDebugRecording(CreateWavFileCallback create_file_callback)
{
    ...
    for (const auto& it : debug_recording_helpers_) {
        ...
        recording_helper->EnableDebugRecording(stream_type, id, create_file_callback_);
    }
}
...
std::unique_ptr<AudioDebugRecorder> AudioDebugRecordingManager::RegisterDebugRecordingSource(
    AudioDebugRecordingStreamType stream_type, const AudioParameters& params)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    std::unique_ptr<AudioDebugRecordingHelper> recording_helper
        = CreateAudioDebugRecordingHelper(params, base::BindOnce(&AudioDebugRecordingManager::UnregisterDebugRecordingSource, weak_factory_.GetWeakPtr(), id));

    if (IsDebugRecordingEnabled()) {
        recording_helper->EnableDebugRecording(stream_type, id, create_file_callback_);
    }

    debug_recording_helpers_[id] = std::make_pair(recording_helper.get(), stream_type);
...
std::unique_ptr<AudioDebugRecordingHelper> AudioDebugRecordingManager::CreateAudioDebugRecordingHelper(
    const AudioParameters& params, base::OnceClosure on_destruction_closure)
{
    return std::make_unique<AudioDebugRecordingHelper>(params, std::move(on_destruction_closure));
}
