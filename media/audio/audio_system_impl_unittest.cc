...
template <bool use_audio_thread> class AudioSystemImplTestBase : public testing::Test {
public:
    ...
    void SetUp() override
    {
        audio_manager_ = std::make_unique<MockAudioManager>(std::make_unique<TestAudioThread>(use_audio_thread));
...
using AudioSystemTestBaseVariations = testing::Types<AudioSystemImplTestBase<false>, AudioSystemImplTestBase<true>>;

INSTANTIATE_TYPED_TEST_SUITE_P(AudioSystemImpl, AudioSystemTestTemplate, AudioSystemTestBaseVariations);
