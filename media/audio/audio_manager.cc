...
public:
    ...
    AudioLogFactory* fake_log_factory()
    {
        return &fake_log_factory_;
    }
    ...
    {
        com_initializer_for_testing_ = std::make_unique<base::win::ScopedCOMInitializer>();
    }
#endif
...
// static
std::unique_ptr<AudioManager> AudioManager::Create(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory)
{
    std::unique_ptr<AudioManager> manager = CreateAudioManager(std::move(audio_thread), audio_log_factory);
    manager->InitializeDebugRecording();
    return manager;
}

// static
std::unique_ptr<AudioManager> AudioManager::CreateForTesting(std::unique_ptr<AudioThread> audio_thread)
{
#if BUILDFLAG(IS_WIN)
...
bool AudioManager::Shutdown()
{
    ...
    if (audio_thread_->GetTaskRunner()->BelongsToCurrentThread()) {
        ...
        audio_thread_->GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&AudioManager::ShutdownOnAudioThread, base::Unretained(this)));
    }
    audio_thread_->Stop();
    shutdown_ = true;
    return true;
...
void AudioManager::TraceAmplitudePeak(bool trace_start)
{
    ...
    if (trace_start) {
        ...
        if (is_trace_started_) {
            TRACE_EVENT_NESTABLE_ASYNC_END1(TRACE_DISABLED_BY_DEFAULT("audio.latency"), kTraceName, current_trace_id_, "aborted", true);
        }

        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0(TRACE_DISABLED_BY_DEFAULT("audio.latency"), kTraceName, ++current_trace_id_);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END1(TRACE_DISABLED_BY_DEFAULT("audio.latency"), kTraceName, current_trace_id_, "aborted", false);
