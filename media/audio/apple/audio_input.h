...
public:
    ...
    PCMQueueInAudioInputStream(AudioManagerApple* manager, const AudioParameters& params);

    PCMQueueInAudioInputStream(const PCMQueueInAudioInputStream&) = delete;
    PCMQueueInAudioInputStream& operator=(const PCMQueueInAudioInputStream&) = delete;
...
private:
    ...
    static void HandleInputBufferStatic(void* data, AudioQueueRef audio_queue, AudioQueueBufferRef audio_buffer, const AudioTimeStamp* start_time,
        UInt32 num_packets, const AudioStreamPacketDescription* desc);

    // Handles callback from OS. Will be called on OS internal thread.
    void HandleInputBuffer(AudioQueueRef audio_queue, AudioQueueBufferRef audio_buffer, const AudioTimeStamp* start_time, UInt32 num_packets,
