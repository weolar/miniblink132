...
public:
    ...
    // nullptr if failed.
    static std::unique_ptr<ScopedAudioChannelLayout> GetOutputDeviceChannelLayout(AudioUnit audio_unit);
    ...
    // will be returned.
    virtual OSStatus GetInputDeviceStreamFormat(AudioUnit audio_unit, AudioStreamBasicDescription* input_format) = 0;
    ...
    virtual bool MaybeChangeBufferSize(AudioDeviceID device_id, AudioUnit audio_unit, AudioUnitElement element, size_t desired_buffer_size) = 0;
...
protected:
    AudioManagerApple(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory);
};
