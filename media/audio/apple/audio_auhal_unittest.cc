...
public:
    AUHALStreamTest()
        ...
        , manager_(AudioManager::CreateForTesting(std::make_unique<TestAudioThread>()))
    ...
    {
        std::string default_device_id = manager_device_info_.GetDefaultOutputDeviceID();
        AudioParameters default_params = manager_device_info_.GetOutputStreamParameters(default_device_id);
        return manager_->MakeAudioOutputStream(default_params, "", base::BindRepeating(&AUHALStreamTest::OnLogMessage, base::Unretained(this)));
    }
...
TEST_F(AUHALStreamTest, HardwareSampleRate)
{
    ...
    std::string default_device_id = manager_device_info_.GetDefaultOutputDeviceID();
    const AudioParameters preferred_params = manager_device_info_.GetOutputStreamParameters(default_device_id);
...
TEST_F(AUHALStreamTest, MAYBE_CreateOpenStartStopClose)
{
    ...
    EXPECT_CALL(source_, OnMoreData(_, _, _, _))
        ...
        .WillRepeatedly(DoAll(ZeroBuffer(), MaybeSignalEvent(&callback_counter, number_of_callbacks, &event), Return(0)));
