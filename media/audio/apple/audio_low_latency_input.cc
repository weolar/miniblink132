...
AUAudioInputStream::AUAudioInputStream(AudioManagerApple* manager, const AudioParameters& input_params, AudioDeviceID audio_device_id,
    ...
    , log_callback_(log_callback)
{
    ...
    DVLOG(1) << __FUNCTION__ << " this " << this << " params " << input_params.AsHumanReadableString() << " use_voice_processing_: " << use_voice_processing_;
...
AudioInputStream::OpenOutcome AUAudioInputStream::Open()
{
    ...
    const int sample_rate = manager_->HardwareSampleRateForDevice(input_device_id_);
...
#if BUILDFLAG(IS_MAC)
    hardware_latency_
        = core_audio_mac::GetHardwareLatency(audio_unit_, input_device_id_, kAudioDevicePropertyScopeInput, format_.mSampleRate, /*is_input=*/true);
#else
...
bool AUAudioInputStream::OpenAUHAL()
{
    ...
    result = manager_->GetInputDeviceStreamFormat(audio_unit_, &input_device_format);
    if (result != noErr) {
        HandleError(result);
        return false;
    }
    ...
    if (!manager_->MaybeChangeBufferSize(input_device_id_, audio_unit_, 1, input_params_.frames_per_buffer())) {
        result = kAudioUnitErr_FormatNotSupported;
        HandleError(result);
        return false;
    }
    ...
    result = AudioUnitSetProperty(audio_unit_, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, AUElement::INPUT, &format_, sizeof(format_));
    if (result != noErr) {
        HandleError(result);
        return false;
    }
...
void AUAudioInputStream::Start(AudioInputCallback* callback)
{
    ...
    input_callback_timer_->Start(FROM_HERE, kInputCallbackStartTimeout, this, &AUAudioInputStream::CheckInputStartupSuccess);
...
void AUAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
    ...
    if (core_audio_mac::GetDeviceTransportType(audio_device_id) == kAudioDeviceTransportTypeAggregate) {
        const AudioDeviceID output_subdevice_id = AudioManagerMac::FindFirstOutputSubdevice(audio_device_id);
...
{
    TRACE_EVENT1("audio", "AUAudioInputStream::OnDataIsAvailable", "frames", number_of_frames);
    ...
    if (new_size != audio_buffer->mDataByteSize) {
        DVLOG(1) << __FUNCTION__ << " this " << this << "New size of number_of_frames detected: " << number_of_frames;
        if (new_size > audio_buffer->mDataByteSize) {
    ...
    if (result == kAudioUnitErr_TooManyFramesToProcess || result == kAudioUnitErr_CannotDoInCurrentContext) {
        ...
        const char* err = (result == kAudioUnitErr_TooManyFramesToProcess) ? "kAudioUnitErr_TooManyFramesToProcess" : "kAudioUnitErr_CannotDoInCurrentContext";
...
OSStatus AUAudioInputStream::Provide(UInt32 number_of_frames, AudioBufferList* io_data, const AudioTimeStamp* time_stamp)
{
    ...
    while (fifo_.available_blocks()) {
        ...
        sink_->OnData(audio_bus, capture_time, normalized_volume, glitch_accumulator_.GetAndReset());
...
bool AUAudioInputStream::IsRunning()
{
    ...
    OSStatus error = AudioUnitGetProperty(audio_unit_, kAudioOutputUnitProperty_IsRunning, kAudioUnitScope_Global, AUElement::OUTPUT, &is_running, &size);
...
void AUAudioInputStream::HandleError(OSStatus err, const base::Location& location)
{
    ...
    LOG(ERROR) << "Input error " << logging::DescriptionFromOSStatus(err) << " (" << err << ") at line " << location.line_number();
    if (sink_)
        sink_->OnError();
}
...
void AUAudioInputStream::CheckInputStartupSuccess()
{
    ...
    if (GetAutomaticGainControl()) {
        ...
        base::UmaHistogramBoolean("Media.Audio.InputStartupSuccessMac", input_callback_is_active);
        DVLOG(1) << __FUNCTION__ << " this " << this << "input_callback_is_active: " << input_callback_is_active;
    }
}
...
void AUAudioInputStream::UpdateCaptureTimestamp(const AudioTimeStamp* timestamp)
{
    ...
    if (last_sample_time_) {
        ...
        if (lost_audio_duration.is_positive()) {
            glitch_accumulator_.Add(AudioGlitchInfo::SingleBoundedSystemGlitch(lost_audio_duration, AudioGlitchInfo::Direction::kCapture));
        }
    }
...
void AUAudioInputStream::ReportAndResetStats()
{
    ...
    base::UmaHistogramCounts10000("Media.Audio.Capture.FramesProvided", number_of_frames_provided_);
