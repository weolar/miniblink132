...
const AudioDebugRecordingStreamType kStreamType(AudioDebugRecordingStreamType::kOutput);
...
void CreateWavFile(AudioDebugRecordingStreamType stream_type, uint32_t id, base::OnceCallback<void(base::File)>)
{
}
...
public:
    MockAudioDebugRecordingHelper(const AudioParameters& params, base::OnceClosure on_destruction_closure)
    ...
    MockAudioDebugRecordingHelper& operator=(const MockAudioDebugRecordingHelper&) = delete;
    ...
    MOCK_METHOD2(DoEnableDebugRecording, void(AudioDebugRecordingStreamType, uint32_t));
    void EnableDebugRecording(
        AudioDebugRecordingStreamType stream_type, uint32_t id, AudioDebugRecordingHelper::CreateWavFileCallback create_file_callback) override
    {
...
public:
    ...
    AudioDebugRecordingManagerUnderTest(const AudioDebugRecordingManagerUnderTest&) = delete;
    AudioDebugRecordingManagerUnderTest& operator=(const AudioDebugRecordingManagerUnderTest&) = delete;

    ~AudioDebugRecordingManagerUnderTest() override = default;

private:
    std::unique_ptr<AudioDebugRecordingHelper> CreateAudioDebugRecordingHelper(const AudioParameters& params, base::OnceClosure on_destruction_closure) override
    {
        return std::make_unique<MockAudioDebugRecordingHelper>(params, std::move(on_destruction_closure));
    }
};
...
public:
    ...
    AudioDebugRecordingManagerTest(const AudioDebugRecordingManagerTest&) = delete;
    AudioDebugRecordingManagerTest& operator=(const AudioDebugRecordingManagerTest&) = delete;

    ~AudioDebugRecordingManagerTest() override = default;

    // Registers a source and increases counter for the expected next source id.
    std::unique_ptr<AudioDebugRecorder> RegisterDebugRecordingSource(const AudioParameters& params)
    {
...
TEST_F(AudioDebugRecordingManagerTest, RegisterEnableDisable)
{
    ...
    for (const auto& recorder : recorders) {
        MockAudioDebugRecordingHelper* mock_recording_helper = static_cast<MockAudioDebugRecordingHelper*>(recorder.get());
        EXPECT_CALL(*mock_recording_helper, DoEnableDebugRecording(kStreamType, expected_id++));
