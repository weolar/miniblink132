...
OpenSLESOutputStream::OpenSLESOutputStream(AudioManagerAndroid* manager, const AudioParameters& params, SLint32 stream_type)
    ...
    , delay_calculator_(samples_per_second_)
{
    ...
    float_format_.bitsPerSample = float_format_.containerSize = SampleFormatToBitsPerChannel(sample_format_);
...
void OpenSLESOutputStream::Start(AudioSourceCallback* callback)
{
    ...
    LOG_ON_FAILURE_AND_RETURN((*simple_buffer_queue_)->Enqueue(simple_buffer_queue_, audio_data_[active_buffer_index_], buffer_size_bytes_));
    ...
    LOG_ON_FAILURE_AND_RETURN((*player_)->SetPlayState(player_, SL_PLAYSTATE_PLAYING));
...
void OpenSLESOutputStream::Stop()
{
    ...
    LOG_ON_FAILURE_AND_RETURN((*player_)->SetPlayState(player_, SL_PLAYSTATE_STOPPED));

    // Clear the buffer queue so that the old data won't be played when
    // resuming playing.
    LOG_ON_FAILURE_AND_RETURN((*simple_buffer_queue_)->Clear(simple_buffer_queue_));
...
bool OpenSLESOutputStream::CreatePlayer()
{
    ...
    LOG_ON_FAILURE_AND_RETURN(slCreateEngine(engine_object_.Receive(), 1, option, 0, nullptr, nullptr), false);
    ...
    LOG_ON_FAILURE_AND_RETURN((*engine)->CreateOutputMix(engine, output_mixer_.Receive(), 0, nullptr, nullptr), false);

    // Realizing the output mix object in synchronous mode.
    LOG_ON_FAILURE_AND_RETURN(output_mixer_->Realize(output_mixer_.Get(), SL_BOOLEAN_FALSE), false);

    // Audio source configuration.
    SLDataLocator_AndroidSimpleBufferQueue simple_buffer_queue = { SL_DATALOCATOR_ANDROIDSIMPLEBUFFERQUEUE, static_cast<SLuint32>(kMaxNumOfBuffersInQueue) };
    SLDataSource audio_source;
    audio_source = { &simple_buffer_queue, &float_format_ };

    // Audio sink configuration.
    SLDataLocator_OutputMix locator_output_mix = { SL_DATALOCATOR_OUTPUTMIX, output_mixer_.Get() };
    SLDataSink audio_sink = { &locator_output_mix, nullptr };

    // Create an audio player.
    const SLInterfaceID interface_id[] = { SL_IID_BUFFERQUEUE, SL_IID_VOLUME, SL_IID_ANDROIDCONFIGURATION };
    const SLboolean interface_required[] = { SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE };
    LOG_ON_FAILURE_AND_RETURN(
        (*engine)->CreateAudioPlayer(engine, player_object_.Receive(), &audio_source, &audio_sink, std::size(interface_id), interface_id, interface_required),
        false);

    // Create AudioPlayer and specify SL_IID_ANDROIDCONFIGURATION.
    SLAndroidConfigurationItf player_config;
    LOG_ON_FAILURE_AND_RETURN(player_object_->GetInterface(player_object_.Get(), SL_IID_ANDROIDCONFIGURATION, &player_config), false);

    // Set configuration using the stream type provided at construction.
    LOG_ON_FAILURE_AND_RETURN((*player_config)->SetConfiguration(player_config, SL_ANDROID_KEY_STREAM_TYPE, &stream_type_, sizeof(SLint32)), false);

    // Set configuration using the stream type provided at construction.
    if (performance_mode_ > SL_ANDROID_PERFORMANCE_NONE) {
        LOG_ON_FAILURE_AND_RETURN(
            (*player_config)->SetConfiguration(player_config, SL_ANDROID_KEY_PERFORMANCE_MODE, &performance_mode_, sizeof(SLuint32)), false);
    }
    ...
    LOG_ON_FAILURE_AND_RETURN(player_object_->GetInterface(player_object_.Get(), SL_IID_PLAY, &player_), false);

    // Get the simple buffer queue interface.
    LOG_ON_FAILURE_AND_RETURN(player_object_->GetInterface(player_object_.Get(), SL_IID_BUFFERQUEUE, &simple_buffer_queue_), false);

    // Register the input callback for the simple buffer queue.
    // This callback will be called when the soundcard needs data.
    LOG_ON_FAILURE_AND_RETURN((*simple_buffer_queue_)->RegisterCallback(simple_buffer_queue_, SimpleBufferQueueCallback, this), false);

    return true;
}

void OpenSLESOutputStream::SimpleBufferQueueCallback(SLAndroidSimpleBufferQueueItf buffer_queue, void* instance)
{
...
void OpenSLESOutputStream::FillBufferQueueNoLock()
{
    ...
    // |delay_calculator_|.
    const int delay_frames = err == SL_RESULT_SUCCESS ? -delay_calculator_.GetFramesToTarget(AdjustPositionForHardwareLatency(position_in_ms)) : 0;
    ...
    const int frames_filled = callback_->OnMoreData(delay, base::TimeTicks::Now(), {}, audio_bus_.get());
    if (frames_filled <= 0) {
    ...
    audio_bus_->ToInterleaved<Float32SampleTypeTraitsNoClip>(frames_filled, reinterpret_cast<float*>(audio_data_[active_buffer_index_]));
    ...
    err = (*simple_buffer_queue_)->Enqueue(simple_buffer_queue_, audio_data_[active_buffer_index_], num_filled_bytes);
    if (SL_RESULT_SUCCESS != err)
        HandleError(err);
...
base::TimeDelta OpenSLESOutputStream::AdjustPositionForHardwareLatency(uint32_t position_in_ms)
{
