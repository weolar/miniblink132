...
OpenSLESInputStream::OpenSLESInputStream(AudioManagerAndroid* audio_manager, const AudioParameters& params)
    : peak_detector_(base::BindRepeating(&AudioManager::TraceAmplitudePeak, base::Unretained(audio_manager),
        /*trace_start=*/true))
    ...
    , no_effects_(params.effects() == AudioParameters::NO_EFFECTS)
{
    ...
    hardware_delay_ = base::Seconds(params.frames_per_buffer() / static_cast<double>(params.sample_rate()));
...
void OpenSLESInputStream::Stop()
{
    ...
    LOG_ON_FAILURE_AND_RETURN((*recorder_)->SetRecordState(recorder_, SL_RECORDSTATE_STOPPED));
...
void OpenSLESInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
...
bool OpenSLESInputStream::CreateRecorder()
{
    ...
    LOG_ON_FAILURE_AND_RETURN((*engine)->CreateAudioRecorder(
                                  engine, recorder_object_.Receive(), &audio_source, &audio_sink, std::size(interface_id), interface_id, interface_required),
        false);
    ...
    SLint32 stream_type = no_effects_ ? SL_ANDROID_RECORDING_PRESET_CAMCORDER : SL_ANDROID_RECORDING_PRESET_VOICE_COMMUNICATION;
...
void OpenSLESInputStream::SimpleBufferQueueCallback(SLAndroidSimpleBufferQueueItf buffer_queue, void* instance)
{
...
void OpenSLESInputStream::ReadBufferQueue()
{
    ...
    audio_bus_->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<int16_t*>(audio_data_[active_buffer_index_]), audio_bus_->frames());

    peak_detector_.FindPeak(audio_bus_.get());

    // TODO(henrika): Investigate if it is possible to get an accurate
    // delay estimation.
    callback_->OnData(audio_bus_.get(), base::TimeTicks::Now() - hardware_delay_, 0.0, {});
