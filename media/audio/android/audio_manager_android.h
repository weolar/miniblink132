...
public:
    AudioManagerAndroid(std::unique_ptr<AudioThread> audio_thread, AudioLogFactory* audio_log_factory);
    ...
    AudioOutputStream* MakeBitstreamOutputStream(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback) override;
    ...
    void SetMute(JNIEnv* env, const base::android::JavaParamRef<jobject>& obj, jboolean muted);
...
private:
    ...
    typedef std::set<raw_ptr<MuteableAudioOutputStream, SetExperimental>> OutputStreams;
