...
AAudioInputStream::AAudioInputStream(AudioManagerAndroid* manager, const AudioParameters& params)
          ...
          /*trace_start=*/true))
{
    ...
    handle_device_change_on_main_sequence_
        = base::BindPostTaskToCurrentDefault(base::BindRepeating(&AAudioInputStream::HandleDeviceChange, weak_factory_.GetWeakPtr()));
}
...
void AAudioInputStream::CreateStreamWrapper()
{
    ...
    stream_wrapper_ = std::make_unique<AAudioStreamWrapper>(this, AAudioStreamWrapper::StreamType::kInput, params_, AAUDIO_USAGE_VOICE_COMMUNICATION);
}
...
bool AAudioInputStream::OnAudioDataRequested(void* audio_data, int32_t num_frames)
{
    ...
    audio_bus_->FromInterleaved<Float32SampleTypeTraits>(reinterpret_cast<float*>(audio_data), num_frames);
...
void AAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
