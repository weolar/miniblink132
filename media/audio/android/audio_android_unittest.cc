...
{
    return (base::Microseconds(params.frames_per_buffer() * base::Time::kMicrosecondsPerSecond / static_cast<double>(params.sample_rate()))).InMillisecondsF();
}

// Helper method which verifies that the device list starts with a valid
// default device name followed by non-default device names.
void CheckDeviceDescriptions(const AudioDeviceDescriptions& device_descriptions)
{
    ...
    EXPECT_EQ(std::string(AudioDeviceDescription::kDefaultDeviceId), it->unique_id);
    ++it;
    ...
    while (it != device_descriptions.end()) {
        ...
        DVLOG(2) << "Device ID(" << it->unique_id << "), label: " << it->device_name << " group: " << it->group_id;
        EXPECT_NE(AudioDeviceDescription::GetDefaultDeviceName(), it->device_name);
        EXPECT_NE(std::string(AudioDeviceDescription::kDefaultDeviceId), it->unique_id);
        ++it;
    }
}

// We clear the data bus to ensure that the test does not cause noise.
int RealOnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest)
{
    dest->Zero();
    return dest->frames();
}
...
std::ostream& operator<<(std::ostream& os, const AudioParameters& params)
{
    ...
    os << endl
       ...
       << "bytes per second: " << params.sample_rate() * params.GetBytesPerFrame(kSampleFormat) << endl
       << "bytes per frame: " << params.GetBytesPerFrame(kSampleFormat) << endl
       << "chunk size in ms: " << ExpectedTimeBetweenCallbacks(params) << endl
       << "echo_canceller: " << (params.effects() & AudioParameters::ECHO_CANCELLER);
    return os;
}
...
public:
    MOCK_METHOD4(OnData, void(const AudioBus* src, base::TimeTicks capture_time, double volume, const AudioGlitchInfo& glitch_info));
...
public:
    ...
    int OnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest) override
    {
    ...
    int file_size()
    {
        return base::checked_cast<int>(file_->size());
    }
...
public:
    explicit FileAudioSink(base::WaitableEvent* event, const AudioParameters& params, const std::string& file_name)
        : event_(event)
        , params_(params)
    {
        // Allocate space for ~10 seconds of data.
        const int kMaxBufferSize = 10 * params.sample_rate() * params.GetBytesPerFrame(kSampleFormat);
    ...
    void OnData(const AudioBus* src, base::TimeTicks capture_time, double volume, const AudioGlitchInfo& glitch_info) override
    {
        const int num_samples = src->frames() * src->channels();
        std::unique_ptr<int16_t> interleaved(new int16_t[num_samples]);
        src->ToInterleaved<SignedInt16SampleTypeTraits>(src->frames(), interleaved.get());
...
class FullDuplexAudioSinkSource : public AudioInputStream::AudioInputCallback, public AudioOutputStream::AudioSourceCallback {
public:
    ...
    FullDuplexAudioSinkSource& operator=(const FullDuplexAudioSinkSource&) = delete;
    ...
    int OnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const AudioGlitchInfo& /* glitch_info */, AudioBus* dest) override
    {
        const int size_in_bytes = kBytesPerSample * dest->frames() * dest->channels();
        ...
        if (fifo_->forward_bytes() < size_in_bytes) {
            ...
            dest->FromInterleaved<SignedInt16SampleTypeTraits>(reinterpret_cast<int16_t*>(buffer_.get()), dest->frames());
        }
...
private:
    ...
    double BytesToMilliseconds(int bytes) const
    {
        ...
        return (base::Microseconds(frames * base::Time::kMicrosecondsPerSecond / static_cast<double>(params_.sample_rate()))).InMillisecondsF();
    }
...
public:
    AudioAndroidOutputTest()
        ...
        , audio_manager_(AudioManager::CreateForTesting(std::make_unique<TestAudioThread>()))
...
protected:
    ...
    void RunOnAudioThread(base::OnceClosure closure)
    {
        if (!audio_manager()->GetTaskRunner()->BelongsToCurrentThread()) {
            ...
            audio_manager()->GetTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&AudioAndroidOutputTest::RunOnAudioThreadImpl, base::Unretained(this), std::move(closure), &event));
            event.Wait();
        } else {
    ...
    void RunOnAudioThreadImpl(base::OnceClosure closure, base::WaitableEvent* event)
    {
    ...
    void GetDefaultOutputStreamParametersOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(
            [](AudioAndroidOutputTest* self) {
                ...
                self->audio_output_parameters_ = self->audio_manager_device_info()->GetOutputStreamParameters(default_device_id);
    ...
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::MakeOutputStream, base::Unretained(this), params));
    }

    void OpenAndCloseAudioOutputStreamOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::OpenAndClose, base::Unretained(this)));
    }

    void OpenAndStartAudioOutputStreamOnAudioThread(AudioOutputStream::AudioSourceCallback* source)
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::OpenAndStart, base::Unretained(this), source));
    }

    void StopAndCloseAudioOutputStreamOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidOutputTest::StopAndClose, base::Unretained(this)));
    }

    double AverageTimeBetweenCallbacks(int num_callbacks) const
    {
        return ((end_time_ - start_time_) / static_cast<double>(num_callbacks - 1)).InMillisecondsF();
    }
    ...
    void StartOutputStreamCallbacks(const AudioParameters& params)
    {
        ...
        EXPECT_CALL(source, OnMoreData(_, _, AudioGlitchInfo(), NotNull()))
            ...
            .WillRepeatedly(
                DoAll(CheckCountAndPostQuitTask(&count, num_callbacks, base::SingleThreadTaskRunner::GetCurrentDefault(), run_loop.QuitWhenIdleClosure()),
        ...
        DVLOG(1) << "expected time between callbacks: " << expected_time_between_callbacks_ms << " ms";
        DVLOG(1) << "average time between callbacks: " << average_time_between_callbacks_ms << " ms";
        EXPECT_GE(average_time_between_callbacks_ms, 0.70 * expected_time_between_callbacks_ms);
        EXPECT_LT(average_time_between_callbacks_ms, 2 * expected_time_between_callbacks_ms);
    }

    void MakeOutputStream(const AudioParameters& params)
    {
        DCHECK(audio_manager()->GetTaskRunner()->BelongsToCurrentThread());
        audio_output_stream_ = audio_manager()->MakeAudioOutputStream(params, std::string(), AudioManager::LogCallback());
...
class AudioAndroidInputTest : public AudioAndroidOutputTest, public testing::WithParamInterface<bool> {
public:
...
protected:
    ...
    AudioParameters GetInputStreamParameters()
    {
        ...
        params.set_effects(GetParam() ? AudioParameters::ECHO_CANCELLER : AudioParameters::NO_EFFECTS);
        return params;
    }

    void GetDefaultInputStreamParametersOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::GetDefaultInputStreamParameters, base::Unretained(this)));
    }

    void MakeAudioInputStreamOnAudioThread(const AudioParameters& params)
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::MakeInputStream, base::Unretained(this), params));
    }

    void OpenAndCloseAudioInputStreamOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::OpenAndClose, base::Unretained(this)));
    }

    void OpenAndStartAudioInputStreamOnAudioThread(AudioInputStream::AudioInputCallback* sink)
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::OpenAndStart, base::Unretained(this), sink));
    }

    void StopAndCloseAudioInputStreamOnAudioThread()
    {
        RunOnAudioThread(base::BindOnce(&AudioAndroidInputTest::StopAndClose, base::Unretained(this)));
    }
    ...
    void StartInputStreamCallbacks(const AudioParameters& params)
    {
        ...
        EXPECT_CALL(sink, OnData(NotNull(), _, _, _))
            .Times(AtLeast(num_callbacks))
            .WillRepeatedly(
                CheckCountAndPostQuitTask(&count, num_callbacks, base::SingleThreadTaskRunner::GetCurrentDefault(), run_loop.QuitWhenIdleClosure()));
        ...
        double average_time_between_callbacks_ms = AverageTimeBetweenCallbacks(num_callbacks);
        DVLOG(0) << "expected time between callbacks: " << expected_time_between_callbacks_ms << " ms";
        DVLOG(0) << "average time between callbacks: " << average_time_between_callbacks_ms << " ms";
        EXPECT_GE(average_time_between_callbacks_ms, 0.70 * expected_time_between_callbacks_ms);
        EXPECT_LE(average_time_between_callbacks_ms, 1.30 * expected_time_between_callbacks_ms);
    }

    void GetDefaultInputStreamParameters()
    {
        DCHECK(audio_manager()->GetTaskRunner()->BelongsToCurrentThread());
        audio_input_parameters_ = audio_manager_device_info()->GetInputStreamParameters(AudioDeviceDescription::kDefaultDeviceId);
    }

    void MakeInputStream(const AudioParameters& params)
    {
        DCHECK(audio_manager()->GetTaskRunner()->BelongsToCurrentThread());
        audio_input_stream_ = audio_manager()->MakeAudioInputStream(params, AudioDeviceDescription::kDefaultDeviceId, AudioManager::LogCallback());
    ...
    {
        DCHECK(audio_manager()->GetTaskRunner()->BelongsToCurrentThread());
        EXPECT_EQ(audio_input_stream_->Open(), AudioInputStream::OpenOutcome::kSuccess);
...
TEST_F(AudioAndroidInputTest, GetAudioInputDeviceDescriptions)
{
    ...
    RunOnAudioThread(
        base::BindOnce(&AudioDeviceInfoAccessorForTests::GetAudioInputDeviceDescriptions, base::Unretained(audio_manager_device_info()), &devices));
...
TEST_F(AudioAndroidOutputTest, GetAudioOutputDeviceDescriptions)
{
    ...
    AudioDeviceDescriptions devices;
    RunOnAudioThread(
        base::BindOnce(&AudioDeviceInfoAccessorForTests::GetAudioOutputDeviceDescriptions, base::Unretained(audio_manager_device_info()), &devices));
    CheckDeviceDescriptions(devices);
}
...
TEST_P(AudioAndroidInputTest, CreateAndCloseInputStream)
{
    ...
    MakeAudioInputStreamOnAudioThread(params);
    RunOnAudioThread(base::BindOnce(&AudioInputStream::Close, base::Unretained(audio_input_stream_)));
}
...
TEST_F(AudioAndroidOutputTest, CreateAndCloseOutputStream)
{
    ...
    RunOnAudioThread(base::BindOnce(&AudioOutputStream::Close, base::Unretained(audio_output_stream_)));
}
...
{
    GetDefaultOutputStreamParametersOnAudioThread();
    AudioParameters params(audio_output_parameters().format(), ChannelLayoutConfig::Mono(), audio_output_parameters().sample_rate(),
...
TEST_P(AudioAndroidInputTest, DISABLED_RunSimplexInputStreamWithFileAsSink)
{
    ...
    std::string file_name = base::StringPrintf("out_simplex_%d_%d_%d.pcm", params.sample_rate(), params.frames_per_buffer(), params.channels());
...
TEST_P(AudioAndroidInputTest, DISABLED_RunDuplexInputStreamWithFileAsSink)
{
    ...
    std::string file_name = base::StringPrintf("out_duplex_%d_%d_%d.pcm", in_params.sample_rate(), in_params.frames_per_buffer(), in_params.channels());

    base::WaitableEvent event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
    FileAudioSink sink(&event, in_params, file_name);
    MockAudioSourceCallback source;

    EXPECT_CALL(source, OnMoreData(_, _, AudioGlitchInfo(), NotNull())).WillRepeatedly(Invoke(RealOnMoreData));
    EXPECT_CALL(source, OnError(_)).Times(0);
...
// NOTE: this test requires user interaction and is not designed to run as an
// automatized test on bots.
TEST_P(AudioAndroidInputTest, DISABLED_RunSymmetricInputAndOutputStreamsInFullDuplex)
{
...
INSTANTIATE_TEST_SUITE_P(AudioAndroidInputTest, AudioAndroidInputTest, testing::Bool());
