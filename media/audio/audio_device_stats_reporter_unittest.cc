...
const std::string kRenderGlitchCountShort = "Media.AudioOutputDevice.AudioServiceGlitchCount.Short";
const std::string kRenderGlitchCountIntervals = "Media.AudioOutputDevice.AudioServiceGlitchCount.Intervals";
const std::string kRenderGlitchDurationShort = "Media.AudioOutputDevice.AudioServiceGlitchDuration.Short";
const std::string kRenderGlitchDurationIntervals = "Media.AudioOutputDevice.AudioServiceGlitchDuration.Intervals";

const std::string kCaptureDelay = "Media.AudioInputDevice.AudioServiceDelay";
const std::string kCaptureGlitchCountShort = "Media.AudioInputDevice.AudioServiceGlitchCount.Short";
const std::string kCaptureGlitchCountIntervals = "Media.AudioInputDevice.AudioServiceGlitchCount.Intervals";
const std::string kCaptureGlitchDurationShort = "Media.AudioInputDevice.AudioServiceGlitchDuration.Short";
const std::string kCaptureGlitchDurationIntervals = "Media.AudioInputDevice.AudioServiceGlitchDuration.Intervals";

} // namespace

class AudioDeviceStatsReporterOutputTest : public ::testing::TestWithParam<std::tuple<media::AudioLatency::Type, std::string>> {
public:
    AudioDeviceStatsReporterOutputTest()
    {
        ...
        reporter_ = std::make_unique<AudioDeviceStatsReporter>(params_, AudioDeviceStatsReporter::Type::kOutput);
    }

    AudioDeviceStatsReporterOutputTest(const AudioDeviceStatsReporterOutputTest&) = delete;
    AudioDeviceStatsReporterOutputTest& operator=(const AudioDeviceStatsReporterOutputTest&) = delete;

    ~AudioDeviceStatsReporterOutputTest() override = default;

protected:
    base::HistogramTester histogram_tester_;
    AudioParameters params_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), 48000, 480);
...
TEST_P(AudioDeviceStatsReporterOutputTest, ShortStreamTest)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        bool failed = i % 2;
        media::AudioGlitchInfo glitch_info { .duration = failed ? params_.GetBufferDuration() : base::TimeDelta(), .count = static_cast<unsigned int>(failed) };
    ...
    histogram_tester_.ExpectBucketCount(kRenderGlitchCountShort + latency_tag_, 50, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationShort, 500, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationShort + latency_tag_, 500, 1);
...
TEST_P(AudioDeviceStatsReporterOutputTest, LongStreamTest)
{
    ...
    histogram_tester_.ExpectBucketCount(kRenderGlitchCountIntervals + latency_tag_, 500, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationIntervals, 500, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationIntervals + latency_tag_, 500, 1);
    ...
    histogram_tester_.ExpectBucketCount(kRenderGlitchCountIntervals + latency_tag_, 250, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationIntervals, 250, 1);
    histogram_tester_.ExpectBucketCount(kRenderGlitchDurationIntervals + latency_tag_, 250, 1);
...
INSTANTIATE_TEST_SUITE_P(All, AudioDeviceStatsReporterOutputTest,
    ::testing::Values(std::make_tuple(media::AudioLatency::Type::kExactMS, ".LatencyExactMs"),
        std::make_tuple(media::AudioLatency::Type::kInteractive, ".LatencyInteractive"), std::make_tuple(media::AudioLatency::Type::kRtc, ".LatencyRtc"),
        std::make_tuple(media::AudioLatency::Type::kPlayback, ".LatencyPlayback"), std::make_tuple(media::AudioLatency::Type::kUnknown, ".LatencyUnknown")));

class AudioDeviceStatsReporterInputTest : public ::testing::Test {
public:
    AudioDeviceStatsReporterInputTest()
    {
        reporter_ = std::make_unique<AudioDeviceStatsReporter>(params_, AudioDeviceStatsReporter::Type::kInput);
    }

    AudioDeviceStatsReporterInputTest(const AudioDeviceStatsReporterInputTest&) = delete;
    AudioDeviceStatsReporterInputTest& operator=(const AudioDeviceStatsReporterInputTest&) = delete;
