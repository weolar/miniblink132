...
class MEDIA_EXPORT AudioOutputDevice : public AudioRendererSink, public AudioOutputIPCDelegate {
public:
    // NOTE: Clients must call Initialize() before using.
    AudioOutputDevice(std::unique_ptr<AudioOutputIPC> ipc, const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner,
    ...
    void OnDeviceAuthorized(OutputDeviceStatus device_status, const AudioParameters& output_params, const std::string& matched_device_id) override;
    void OnStreamCreated(base::UnsafeSharedMemoryRegion shared_memory_region, base::SyncSocket::ScopedHandle socket_handle, bool play_automatically) override;
    void OnIPCClosed() override;

    AudioOutputIPC* GetIpcForTesting()
    {
        return ipc_.get();
    }
...
private:
    ...
    enum Error { kNoError = 0, kErrorDuringCreation = 1, kErrorDuringRendering = 2, kMaxValue = kErrorDuringRendering };
    ...
    void InitializeOnIOThread(const AudioParameters& params, MayBeDangling<RenderCallback> callback);
    ...
    void ProcessDeviceAuthorizationOnIOThread(
        OutputDeviceStatus device_status, const AudioParameters& output_params, const std::string& matched_device_id, bool timed_out);
    ...
    std::unique_ptr<AudioDeviceThread> audio_thread_ GUARDED_BY(audio_thread_lock_);
