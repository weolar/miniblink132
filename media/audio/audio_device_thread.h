...
public:
    ...
    class Callback {
    public:
        Callback(const AudioParameters& audio_parameters, uint32_t segment_length, uint32_t total_segments);
    ...
    AudioDeviceThread(Callback* callback, base::SyncSocket::ScopedHandle socket, const char* thread_name, base::ThreadType thread_type);
