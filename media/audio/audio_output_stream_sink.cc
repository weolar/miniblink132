...
void AudioOutputStreamSink::Initialize(const AudioParameters& params, RenderCallback* callback)
{
    DCHECK(callback);
    DCHECK(!started_);
    params_ = params;
...
void AudioOutputStreamSink::Start()
{
    ...
    started_ = true;
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoStart, this, params_));
}
...
void AudioOutputStreamSink::Stop()
{
    ClearCallback();
    started_ = false;
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoStop, this));
}

void AudioOutputStreamSink::Pause()
{
    ClearCallback();
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoPause, this));
}

void AudioOutputStreamSink::Flush()
{
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoFlush, this));
}
...
void AudioOutputStreamSink::Play()
{
    ...
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoPlay, this));
}

bool AudioOutputStreamSink::SetVolume(double volume)
{
    audio_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AudioOutputStreamSink::DoSetVolume, this, volume));
    return true;
}
...
void AudioOutputStreamSink::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), GetOutputDeviceInfo()));
}
...
int AudioOutputStreamSink::OnMoreData(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, AudioBus* dest)
{
    ...
    return active_render_callback_->Render(delay, delay_timestamp, prior_frames_skipped, dest);
}
...
void AudioOutputStreamSink::DoStart(const AudioParameters& params)
{
    ...
    stream_ = AudioManager::Get()->MakeAudioOutputStreamProxy(active_params_, std::string());
