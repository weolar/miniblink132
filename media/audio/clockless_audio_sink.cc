...
public:
    ClocklessAudioSinkThread(const AudioParameters& params, AudioRendererSink::RenderCallback* callback, bool hashing)
        : callback_(callback)
        , audio_bus_(AudioBus::Create(params))
        , stop_event_(new base::WaitableEvent(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED))
    {
        if (hashing)
    ...
    {
        stop_event_->Reset();
        thread_ = std::make_unique<base::DelegateSimpleThread>(this, "ClocklessAudioSink");
        thread_->Start();
    }
...
private:
    ...
    void Run() override
    {
        ...
        while (!stop_event_->IsSignaled()) {
            const int frames_received = callback_->Render(base::TimeDelta(), base::TimeTicks::Now(), {}, audio_bus_.get());
...
void ClocklessAudioSink::Initialize(const AudioParameters& params, RenderCallback* callback)
{
    DCHECK(!initialized_);
    thread_ = std::make_unique<ClocklessAudioSinkThread>(params, callback, hashing_);
    initialized_ = true;
}
...
void ClocklessAudioSink::SetIsOptimizedForHardwareParametersForTesting(bool value)
{
