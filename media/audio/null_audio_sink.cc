...
NullAudioSink::NullAudioSink(const scoped_refptr<base::SequencedTaskRunner>& task_runner)
    : initialized_(false)
    , started_(false)
    , playing_(false)
    , callback_(nullptr)
...
void NullAudioSink::Initialize(const AudioParameters& params, RenderCallback* callback)
{
    DCHECK(!started_);
...
void NullAudioSink::Play()
{
    ...
    fake_worker_->Start(base::BindRepeating(&NullAudioSink::CallRender, base::Unretained(this)));
...
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(info_cb), GetOutputDeviceInfo()));
}
...
void NullAudioSink::SwitchOutputDevice(const std::string& device_id, OutputDeviceStatusCB callback)
{
    std::move(callback).Run(OUTPUT_DEVICE_STATUS_ERROR_INTERNAL);
}

void NullAudioSink::CallRender(base::TimeTicks ideal_time, base::TimeTicks now)
{
    ...
    int frames_received = callback_->Render(fixed_data_delay_, ideal_time, {}, audio_bus_.get());
