...
const char AudioDeviceDescription::kLoopbackWithoutChromeId[] = "loopbackWithoutChrome";
...
// static
bool AudioDeviceDescription::IsCommunicationsDevice(const std::string& device_id)
{
...
{
    return device_id == kLoopbackInputDeviceId || device_id == kLoopbackWithMuteDeviceId || device_id == kLoopbackWithoutChromeId;
}

// static
bool AudioDeviceDescription::UseSessionIdToSelectDevice(const base::UnguessableToken& session_id, const std::string& device_id)
{
...
// static
std::string AudioDeviceDescription::GetDefaultDeviceName(const std::string& real_device_name)
{
...
// static
std::string AudioDeviceDescription::GetCommunicationsDeviceName(const std::string& real_device_name)
{
    if (real_device_name.empty())
...
// static
void AudioDeviceDescription::LocalizeDeviceDescriptions(AudioDeviceDescriptions* device_descriptions)
{
    for (auto& description : *device_descriptions) {
        ...
        if (media::AudioDeviceDescription::IsDefaultDevice(description.unique_id)) {
            description.device_name = media::AudioDeviceDescription::GetDefaultDeviceName(description.device_name);
        } else if (media::AudioDeviceDescription::IsCommunicationsDevice(description.unique_id)) {
            description.device_name = media::AudioDeviceDescription::GetCommunicationsDeviceName(description.device_name);
        }
    }
}
...
AudioDeviceDescription& AudioDeviceDescription::operator=(const AudioDeviceDescription& other) = default;

AudioDeviceDescription::AudioDeviceDescription(AudioDeviceDescription&& other) = default;
AudioDeviceDescription& AudioDeviceDescription::operator=(AudioDeviceDescription&& other) = default;

AudioDeviceDescription::AudioDeviceDescription(
    std::string device_name, std::string unique_id, std::string group_id, bool is_system_default, bool is_communications_device)
...
bool AudioDeviceDescription::operator==(const AudioDeviceDescription& other) const
{
    return device_name == other.device_name && unique_id == other.unique_id && group_id == other.group_id;
}
