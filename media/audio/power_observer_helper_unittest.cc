...
public:
    PowerObserverHelperTest()
        ...
        , suspend_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , resume_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    ...
    void OnSuspend()
    {
        EXPECT_TRUE(power_observer_helper_thread_.task_runner()->BelongsToCurrentThread());
...
protected:
    ~PowerObserverHelperTest() override
    {
        ...
        power_observer_helper_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&PowerObserverHelperTest::ResetPowerObserverHelperOnPowerObserverHelperThread, base::Unretained(this), &done));
        done.Wait();
    }
    ...
    void CreatePowerObserverHelper()
    {
        ...
        power_observer_helper_ = std::make_unique<PowerObserverHelper>(power_observer_helper_thread_.task_runner(),
            base::BindRepeating(&PowerObserverHelperTest::OnSuspend, base::Unretained(this)),
            base::BindRepeating(&PowerObserverHelperTest::OnResume, base::Unretained(this)));
    }
    ...
    bool IsSuspending()
    {
        ...
        base::WaitableEvent did_check(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        power_observer_helper_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&PowerObserverHelperTest::CheckIfSuspending, base::Unretained(this), &is_suspending, &did_check));
        did_check.Wait();
...
private:
    ...
    void ResetPowerObserverHelperOnPowerObserverHelperThread(base::WaitableEvent* done)
    {
        EXPECT_TRUE(power_observer_helper_thread_.task_runner()->BelongsToCurrentThread());
