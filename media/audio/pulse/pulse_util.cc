...
public:
    ...
    pa_proplist* get() const
    {
        return property_list_.get();
    }
...
void InputBusCallback(pa_context* context, const pa_source_info* info, int error, void* user_data)
{
    ...
    if (strcmp(info->name, data->name_->c_str()) == 0 && pa_proplist_contains(info->proplist, PA_PROP_DEVICE_BUS_PATH)) {
        data->bus_ = pa_proplist_gets(info->proplist, PA_PROP_DEVICE_BUS_PATH);
    }
}

void OutputBusCallback(pa_context* context, const pa_sink_info* info, int error, void* user_data)
{
    ...
    if (pa_proplist_contains(info->proplist, PA_PROP_DEVICE_BUS_PATH)
        && strcmp(pa_proplist_gets(info->proplist, PA_PROP_DEVICE_BUS_PATH), data->bus_->c_str()) == 0) {
        data->name_ = info->name;
    }
}

struct DefaultDevicesData {
    explicit DefaultDevicesData(pa_threaded_mainloop* loop)
        : loop_(loop)
    {
    }
    std::string input_;
...
void GetDefaultDeviceIdCallback(pa_context* c, const pa_server_info* info, void* userdata)
{
...
struct MonitorSourceData {
    explicit MonitorSourceData(pa_threaded_mainloop* loop)
        : loop_(loop)
    {
    }
    const raw_ptr<pa_threaded_mainloop> loop_;
...
void GetMonitorSourceNameForSinkCallback(pa_context* context, const pa_sink_info* info, int eol, void* userdata)
{
...
bool InitPulse(pa_threaded_mainloop** mainloop, pa_context** context)
{
    ...
    pa_context* pa_context = pa_context_new(pa_mainloop_api, PRODUCT_STRING " input");
    if (!pa_context) {
    ...
    pa_context_set_state_callback(pa_context, &SignalReadyOrErrorStateCallback, &data);

    if (pa_context_connect(pa_context, nullptr, PA_CONTEXT_NOAUTOSPAWN, nullptr)) {
        VLOG(1) << "Failed to connect to the context.  Error: " << pa_strerror(pa_context_errno(pa_context));
    ...
    pa_context_set_state_callback(pa_context, &pulse::ContextStateCallback, pa_mainloop);
...
pa_channel_map ChannelLayoutToPAChannelMap(ChannelLayout channel_layout)
{
    ...
    if (channel_layout == CHANNEL_LAYOUT_MONO) {
        ...
        for (Channels ch = LEFT; ch <= CHANNELS_MAX; ch = static_cast<Channels>(ch + 1)) {
...
bool WaitForOperationCompletion(pa_threaded_mainloop* mainloop, pa_operation* operation, pa_context* optional_context, pa_stream* optional_stream)
{
    if (!operation) {
    ...
    while (pa_operation_get_state(operation) == PA_OPERATION_RUNNING) {
        if (optional_context) {
            ...
            if (!PA_CONTEXT_IS_GOOD(context_state)) {
                LOG(ERROR) << "pa_context went bad while waiting: state=" << context_state << ", error=" << pa_strerror(pa_context_errno(optional_context));
...
bool CreateInputStream(pa_threaded_mainloop* mainloop, pa_context* context, raw_ptr<pa_stream>* stream, const AudioParameters& params,
    const std::string& device_id, pa_stream_notify_cb_t stream_callback, void* user_data)
{
    DCHECK(mainloop);
    DCHECK(context);
    ...
    static_assert(kInputSampleFormat == kSampleFormatS16, "Only 16-bit input supported.");
    ...
    pa_channel_map source_channel_map = ChannelLayoutToPAChannelMap(params.channel_layout());
    pa_channel_map* map = (source_channel_map.channels != 0) ? &source_channel_map : nullptr;

    // Create a new recording stream and
    // tells PulseAudio what the stream icon should be.
    ScopedPropertyList property_list;
    pa_proplist_sets(property_list.get(), PA_PROP_APPLICATION_ICON_NAME, kBrowserDisplayName);
    *stream = pa_stream_new_with_proplist(context, "RecordStream", &sample_specifications, map, property_list.get());
    ...
    RETURN_ON_FAILURE(pa_stream_connect_record(*stream, device_id == AudioDeviceDescription::kDefaultDeviceId ? nullptr : device_id.c_str(), &buffer_attributes,
    ...
    while (true) {
        ...
        RETURN_ON_FAILURE(PA_STREAM_IS_GOOD(stream_state), "Invalid PulseAudio stream state");
...
bool CreateOutputStream(raw_ptr<pa_threaded_mainloop>* mainloop, raw_ptr<pa_context>* context, raw_ptr<pa_stream>* stream, const AudioParameters& params,
    const std::string& device_id, const std::string& app_name, pa_stream_notify_cb_t stream_callback, pa_stream_request_cb_t write_callback, void* user_data)
{
    DCHECK(!*mainloop);
    DCHECK(!*context);
    ...
    *context = pa_context_new(pa_mainloop_api, app_name.empty() ? PRODUCT_STRING : app_name.c_str());
    ...
    RETURN_ON_FAILURE(pa_threaded_mainloop_start(*mainloop) == 0, "Failed to start PulseAudio main loop.");
    RETURN_ON_FAILURE(pa_context_connect(*context, nullptr, PA_CONTEXT_NOAUTOSPAWN, nullptr) == 0, "Failed to connect PulseAudio context.");
    ...
    while (true) {
        ...
        RETURN_ON_FAILURE(PA_CONTEXT_IS_GOOD(context_state), "Invalid PulseAudio context state.");
        if (context_state == PA_CONTEXT_READY)
            break;
        pa_threaded_mainloop_wait(*mainloop);
    }
    ...
    ScopedPropertyList property_list;
    pa_proplist_sets(property_list.get(), PA_PROP_APPLICATION_ICON_NAME, kBrowserDisplayName);
    *stream = pa_stream_new_with_proplist(*context, "Playback", &sample_specifications, map, property_list.get());
    ...
    // and error.
    RETURN_ON_FAILURE(
        pa_stream_connect_playback(*stream, device_id == AudioDeviceDescription::kDefaultDeviceId ? nullptr : device_id.c_str(), &pa_buffer_attributes,
...
void MuteAllSinksExcept(pa_threaded_mainloop* mainloop, pa_context* context, const std::string& exclude_sink_name)
{
    CHECK(mainloop);
    CHECK(context);
        ...
        [](pa_context* c, const pa_sink_info* i, int eol, void* userdata) {
            ...
            if (!eol) {
                ...
                if (i->name != *exclude_sink_name) {
                    pa_context_set_sink_mute_by_index(c, i->index, 1, /*callback=*/nullptr,
...
std::string GetBusOfInput(pa_threaded_mainloop* mainloop, pa_context* context, const std::string& name)
{
    DCHECK(mainloop);
    DCHECK(context);
    AutoPulseLock auto_lock(mainloop);
    InputBusData data(mainloop, name);
    pa_operation* operation = pa_context_get_source_info_list(context, InputBusCallback, &data);
    WaitForOperationCompletion(mainloop, operation, context);
    return data.bus_;
}

std::string GetOutputCorrespondingTo(pa_threaded_mainloop* mainloop, pa_context* context, const std::string& bus)
{
    DCHECK(mainloop);
    DCHECK(context);
    AutoPulseLock auto_lock(mainloop);
    OutputBusData data(mainloop, bus);
    pa_operation* operation = pa_context_get_sink_info_list(context, OutputBusCallback, &data);
    WaitForOperationCompletion(mainloop, operation, context);
    return data.name_;
}

std::string GetRealDefaultDeviceId(pa_threaded_mainloop* mainloop, pa_context* context, RequestType type)
{
    DCHECK(mainloop);
    DCHECK(context);
    AutoPulseLock auto_lock(mainloop);
    DefaultDevicesData data(mainloop);
    pa_operation* operation = pa_context_get_server_info(context, &GetDefaultDeviceIdCallback, &data);
    WaitForOperationCompletion(mainloop, operation, context);
    return (type == RequestType::INPUT) ? data.input_ : data.output_;
}

std::string GetMonitorSourceNameForSink(pa_threaded_mainloop* mainloop, pa_context* context, const std::string& sink_name)
{
    CHECK(mainloop);
    CHECK(context);
    ...
    pa_operation* operation = pa_context_get_sink_info_by_name(context, sink_name.c_str(), &GetMonitorSourceNameForSinkCallback, &data);
    WaitForOperationCompletion(mainloop, operation, context);
