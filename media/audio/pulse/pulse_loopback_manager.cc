...
std::unique_ptr<PulseLoopbackManager> PulseLoopbackManager::Create(
    ReleaseStreamCallback release_stream_callback, pa_context* context, pa_threaded_mainloop* mainloop)
{
    // We want to open the monitor device of the default sink, which is
    // specified using a special identifier.
    const std::string default_monitor_name = pulse::GetMonitorSourceNameForSink(mainloop, context, kDefaultSinkName);
    if (default_monitor_name.empty()) {
        LOG(ERROR) << "Failed to retrieve default monitor name.";
        return nullptr;
    }

    auto manager = base::WrapUnique(new PulseLoopbackManager(default_monitor_name, release_stream_callback, context, mainloop));
    ...
    pa_context_set_subscribe_callback(manager->context_, &EventCallback, manager.get());

    // TODO(crbug.com/40281249): Check if subscription was reported as successful
    // in pulse::ContextSuccessCallback.
    pa_operation* operation = pa_context_subscribe(manager->context_, PA_SUBSCRIPTION_MASK_SERVER, &pulse::ContextSuccessCallback, manager->mainloop_);
...
PulseLoopbackManager::~PulseLoopbackManager()
{
    ...
    pa_operation* operation = pa_context_subscribe(context_, PA_SUBSCRIPTION_MASK_NULL, &pulse::ContextSuccessCallback, mainloop_);
...
PulseLoopbackAudioStream* PulseLoopbackManager::MakeLoopbackStream(
    const AudioParameters& params, AudioManager::LogCallback log_callback, bool should_mute_system_audio)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    auto* stream = new PulseLoopbackAudioStream(
        release_stream_callback_, GetLoopbackSourceName(), params, mainloop_, context_, std::move(log_callback), should_mute_system_audio);
...
// static
void PulseLoopbackManager::EventCallback(pa_context* context, pa_subscription_event_type_t type, uint32_t index, void* user_data)
{
    ...
    if ((type & PA_SUBSCRIPTION_EVENT_FACILITY_MASK) == PA_SUBSCRIPTION_EVENT_SERVER
        ...
        manager->task_runner_->PostTask(FROM_HERE, base::BindOnce(&PulseLoopbackManager::OnServerChangeEvent, manager->weak_this_));
    }
...
PulseLoopbackManager::PulseLoopbackManager(
    const std::string& default_monitor_name, ReleaseStreamCallback release_stream_callback, pa_context* context, pa_threaded_mainloop* mainloop)
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    const std::string default_monitor_name = pulse::GetMonitorSourceNameForSink(mainloop_, context_, kDefaultSinkName);
    if (default_monitor_name.empty()) {
