...
PulseLoopbackAudioStream::PulseLoopbackAudioStream(ReleaseStreamCallback release_stream_callback, const std::string& source_name, const AudioParameters& params,
    pa_threaded_mainloop* mainloop, pa_context* context, AudioManager::LogCallback log_callback, bool mute_system_audio)
    ...
    , sink_(nullptr)
    , stream_(new PulseAudioInputStream(nullptr, source_name, params, mainloop, context, log_callback_))
{
    CHECK(stream_);
}
...
AudioInputStream::OpenOutcome PulseLoopbackAudioStream::Open()
{
    ...
    if (open_outcome == OpenOutcome::kSuccess) {
        ...
        if (mute_system_audio_) {
            std::string default_sink_name = media::pulse::GetRealDefaultDeviceId(mainloop_, context_, pulse::RequestType::OUTPUT);
            std::string monitor_source_name = media::pulse::GetMonitorSourceNameForSink(mainloop_, context_, default_sink_name);
...
void PulseLoopbackAudioStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    stream_->SetOutputDeviceForAec(output_device_id);
}

void PulseLoopbackAudioStream::ChangeStreamSource(const std::string& source_name)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    stream_->Stop();
    CloseWrappedStream();

    stream_ = new PulseAudioInputStream(nullptr, source_name, params_, mainloop_, context_, log_callback_);
    CHECK(stream_);
