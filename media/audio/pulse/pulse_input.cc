...
PulseAudioInputStream::PulseAudioInputStream(AudioManagerPulse* audio_manager, const std::string& source_name, const AudioParameters& params,
    pa_threaded_mainloop* mainloop, pa_context* context, AudioManager::LogCallback log_callback)
    ...
    , muted_(false)
    , fifo_(params.channels(), params.frames_per_buffer(), kNumberOfBlocksBufferInFifo)
    , pa_mainloop_(mainloop)
    , pa_context_(context)
    , log_callback_(std::move(log_callback))
    , handle_(nullptr)
    , peak_detector_(audio_manager ? base::BindRepeating(&AudioManager::TraceAmplitudePeak, base::Unretained(audio_manager_),
                         /*trace_start=*/true)
                                   : base::RepeatingClosure())
{
    ...
    SendLogMessage("%s({device_id=%s}, {params=[%s]})", __func__, source_name.c_str(), params.AsHumanReadableString().c_str());
...
AudioInputStream::OpenOutcome PulseAudioInputStream::Open()
{
    ...
    if (source_name_ == AudioDeviceDescription::kDefaultDeviceId && audio_manager_ && audio_manager_->DefaultSourceIsMonitor()) {
        SendLogMessage("%s => (ERROR: can't open monitor device)", __func__);
        return OpenOutcome::kFailed;
    }

    AutoPulseLock auto_lock(pa_mainloop_);
    if (!pulse::CreateInputStream(pa_mainloop_, pa_context_, &handle_, params_, source_name_, &StreamNotifyCallback, this)) {
...
void PulseAudioInputStream::Start(AudioInputCallback* callback)
{
    ...
    if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, handle_)) {
        callback_->OnError();
    }
}
...
void PulseAudioInputStream::Stop()
{
    ...
    pa_operation* operation = pa_stream_flush(handle_, &pulse::StreamSuccessCallback, pa_mainloop_);
    if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, handle_)) {
        callback_->OnError();
    }
    ...
    operation = pa_stream_cork(handle_, 1, &pulse::StreamSuccessCallback, pa_mainloop_);
    if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, handle_)) {
        callback_->OnError();
    }
    callback_ = nullptr;
...
void PulseAudioInputStream::SetVolume(double volume)
{
    ...
    if (!channels_) {
        ...
        operation = pa_context_get_source_info_by_index(pa_context_, index, &VolumeCallback, this);
        if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, handle_) || !channels_) {
            SendLogMessage("%s => (WARNING: failed to read number of channels)", __func__);
            return;
        }
    }
    ...
    operation = pa_context_set_source_volume_by_index(pa_context_, index, &pa_volume, nullptr, nullptr);
...
double PulseAudioInputStream::GetVolume()
{
    if (pa_threaded_mainloop_in_thread(pa_mainloop_)) {
        ...
        size_t index = pa_stream_get_device_index(handle_);
        pa_operation* operation = pa_context_get_source_info_by_index(pa_context_, index, &VolumeCallback, this);
...
void PulseAudioInputStream::SetOutputDeviceForAec(const std::string& output_device_id)
{
...
void PulseAudioInputStream::ReadCallback(pa_stream* handle, size_t length, void* user_data)
{
...
void PulseAudioInputStream::VolumeCallback(pa_context* context, const pa_source_info* info, int error, void* user_data)
{
    PulseAudioInputStream* stream = reinterpret_cast<PulseAudioInputStream*>(user_data);
...
// static, used by pa_context_get_source_info_by_index.
void PulseAudioInputStream::MuteCallback(pa_context* context, const pa_source_info* info, int error, void* user_data)
{
...
void PulseAudioInputStream::StreamNotifyCallback(pa_stream* s, void* user_data)
{
    PulseAudioInputStream* stream = reinterpret_cast<PulseAudioInputStream*>(user_data);
...
void PulseAudioInputStream::ReadData()
{
    ...
    do {
        ...
        const int number_of_frames = length / params_.GetBytesPerFrame(pulse::kInputSampleFormat);
        if (number_of_frames > fifo_.GetUnfilledFrames()) {
            // Dynamically increase capacity to the FIFO to handle larger buffer got
            // from Pulse.
            const int increase_blocks_of_buffer = static_cast<int>((number_of_frames - fifo_.GetUnfilledFrames()) / params_.frames_per_buffer()) + 1;
            fifo_.IncreaseCapacity(increase_blocks_of_buffer);
        }

        const int bytes_per_sample = SampleFormatToBytesPerChannel(pulse::kInputSampleFormat);
    ...
    const base::TimeTicks capture_time_base = base::TimeTicks::Now() - pulse::GetHardwareLatency(handle_);
    while (fifo_.available_blocks()) {
        ...
        const base::TimeTicks capture_time = capture_time_base - AudioTimestampHelper::FramesToTime(fifo_.GetAvailableFrames(), params_.sample_rate());
...
bool PulseAudioInputStream::GetSourceInformation(pa_source_info_cb_t callback)
{
    ...
    return WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, handle_);
}
