...
void PulseAudioOutputStream::StreamRequestCallback(pa_stream* s, size_t len, void* p_this)
{
...
PulseAudioOutputStream::PulseAudioOutputStream(
    const AudioParameters& params, const std::string& device_id, AudioManagerBase* manager, AudioManager::LogCallback log_callback)
    : params_(AudioParameters(params.format(), params.channel_layout_config(), params.sample_rate(), params.frames_per_buffer()))
...
bool PulseAudioOutputStream::Open()
{
    ...
    bool result = pulse::CreateOutputStream(
        &pa_mainloop_, &pa_context_, &pa_stream_, params_, device_id_, AudioManager::GetGlobalAppName(), &StreamNotifyCallback, &StreamRequestCallback, this);
    if (!result) {
...
void PulseAudioOutputStream::Reset()
{
...

    {
        ...
        if (pa_stream_) {
            ...
            pa_operation* operation = pa_stream_flush(pa_stream_, &pulse::StreamSuccessCallback, pa_mainloop_);
...
void PulseAudioOutputStream::SendLogMessage(const char* format, ...)
{
    ...
    log_callback_.Run("PAOS::" + base::StringPrintV(format, args) + base::StringPrintf(" [this=%p]", this));
    va_end(args);
}

void PulseAudioOutputStream::FulfillWriteRequest(size_t requested_bytes)
{
    TRACE_EVENT("audio", "PulseAudioOutputStream::FulfillWriteRequest", [&](perfetto::EventContext ctx) {
    ...
    while (bytes_remaining > 0) {
        ...
        if (!source_callback_) {
            ...
            pa_stream_write(pa_stream_, pa_buffer, pa_buffer_size, nullptr, 0LL, PA_SEEK_RELATIVE);
        ...
        size_t frames_filled = source_callback_->OnMoreData(BoundedDelay(delay), base::TimeTicks::Now(), {}, audio_bus_.get());

        // Zero any unfilled data so it plays back as silence.
        if (frames_filled < unwritten_frames_in_bus) {
            audio_bus_->ZeroFramesPartial(frames_filled, unwritten_frames_in_bus - frames_filled);
        }
        ...
        do {
            ...
            frames_to_copy = std::min(audio_bus_->frames() - frame_offset_in_bus, frames_to_copy);

            // We skip clipping since that occurs at the shared memory boundary.
            audio_bus_->ToInterleavedPartial<Float32SampleTypeTraitsNoClip>(frame_offset_in_bus, frames_to_copy, reinterpret_cast<float*>(pa_buffer));
            frame_offset_in_bus += frames_to_copy;
            unwritten_frames_in_bus -= frames_to_copy;

            if (pa_stream_write(pa_stream_, pa_buffer, pa_buffer_size, nullptr, 0LL, PA_SEEK_RELATIVE) < 0) {
...
void PulseAudioOutputStream::Start(AudioSourceCallback* callback)
{
    ...
    pa_operation* operation = pa_stream_cork(pa_stream_, 0, &pulse::StreamSuccessCallback, pa_mainloop_);
    if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, pa_stream_)) {
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
    }
}
...
void PulseAudioOutputStream::Stop()
{
    ...
    operation = pa_stream_cork(pa_stream_, 1, &pulse::StreamSuccessCallback, pa_mainloop_);
    if (!WaitForOperationCompletion(pa_mainloop_, operation, pa_context_, pa_stream_)) {
        callback->OnError(AudioSourceCallback::ErrorType::kUnknown);
    }
}
