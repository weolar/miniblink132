...
// Dummy function.
std::unique_ptr<media::AudioDebugRecorder> RegisterDebugRecording(const media::AudioParameters& params)
{
    return nullptr;
}
...
public:
    MockAudioOutputStream(AudioManagerBase* manager, const AudioParameters& params)
        : start_called_(false)
        , stop_called_(false)
        , params_(params)
        , fake_output_stream_(FakeAudioOutputStream::MakeFakeStream(manager, params_))
    {
    }
...
public:
    MockAudioManager()
        : AudioManagerBase(std::make_unique<TestAudioThread>(), &fake_audio_log_factory_)
    {
    }
    ...
    MOCK_METHOD3(MakeAudioOutputStream, AudioOutputStream*(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback));
    MOCK_METHOD2(MakeAudioOutputStreamProxy, AudioOutputStream*(const AudioParameters& params, const std::string& device_id));
    MOCK_METHOD3(MakeAudioInputStream, AudioInputStream*(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback));
    ...
    MOCK_METHOD3(MakeLinearInputStream, AudioInputStream*(const AudioParameters& params, const std::string& device_id, const LogCallback& log_callback));
...
public:
    int OnMoreData(base::TimeDelta /* delay */, base::TimeTicks /* delay_timestamp */, const media::AudioGlitchInfo& glitch_info, AudioBus* dest) override
    {
...
protected:
    void SetUp() override
    {
        ...
        params_ = AudioParameters(AudioParameters::AUDIO_PCM_LINEAR, ChannelLayoutConfig::Stereo(), 8000, 2048);
    ...
    {
        dispatcher_impl_ = std::make_unique<AudioOutputDispatcherImpl>(&manager(), params_, std::string(), close_delay);
    }
    ...
    void WaitForCloseTimer(MockAudioOutputStream* stream)
    {
        ...
        EXPECT_CALL(*stream, Close()).WillOnce(testing::InvokeWithoutArgs(&run_loop, &base::RunLoop::Quit));
        run_loop.Run();
    }

    void CloseAndWaitForCloseTimer(AudioOutputProxy* proxy, MockAudioOutputStream* stream)
    {
    ...
    void OpenAndClose(AudioOutputDispatcher* dispatcher)
    {
        ...
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream));
    ...
    void OpenFailed(AudioOutputDispatcher* dispatcher)
    {
        ...
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream));
        EXPECT_CALL(stream, Open()).WillOnce(Return(false));
    ...
    void OneStream_TwoPlays(AudioOutputDispatcher* dispatcher)
    {
        ...
        EXPECT_CALL(stream, Open()).WillOnce(Return(true));
        EXPECT_CALL(stream, SetVolume(_)).Times(2);
    ...
    void TwoStreams_BothPlaying(AudioOutputDispatcher* dispatcher)
    {
        ...
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream1)).WillOnce(Return(&stream2));

        EXPECT_CALL(stream1, Open()).WillOnce(Return(true));
        EXPECT_CALL(stream1, SetVolume(_)).Times(1);

        EXPECT_CALL(stream2, Open()).WillOnce(Return(true));
        EXPECT_CALL(stream2, SetVolume(_)).Times(1);
    ...
    void StartFailed(AudioOutputDispatcher* dispatcher)
    {
        ...
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).Times(2).WillRepeatedly(Return(reinterpret_cast<AudioOutputStream*>(NULL)));
    ...
    void DispatcherDestroyed_BeforeOpen(std::unique_ptr<AudioOutputDispatcher> dispatcher)
    {
    ...
    void DispatcherDestroyed_BeforeStart(std::unique_ptr<AudioOutputDispatcher> dispatcher)
    {
        MockAudioOutputStream stream(&manager_, params_);
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream));
        EXPECT_CALL(stream, Open()).WillOnce(Return(true));
    ...
    void DispatcherDestroyed_BeforeStop(std::unique_ptr<AudioOutputDispatcher> dispatcher)
    {
        MockAudioOutputStream stream(&manager_, params_);
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream));
        EXPECT_CALL(stream, Open()).WillOnce(Return(true));
    ...
    void DispatcherDestroyed_AfterStop(std::unique_ptr<AudioOutputDispatcher> dispatcher)
    {
        MockAudioOutputStream stream(&manager_, params_);
        EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).WillOnce(Return(&stream));
        EXPECT_CALL(stream, Open()).WillOnce(Return(true));
...
public:
    void InitDispatcher(base::TimeDelta close_delay) override
    {
        ...
        // FakeAudioOutputStream will keep the message loop busy indefinitely; i.e.,
        // RunUntilIdle() will never terminate.
        resampler_params_ = AudioParameters(AudioParameters::AUDIO_PCM_LOW_LATENCY, ChannelLayoutConfig::Stereo(), 16000, 1024);
        resampler_ = std::make_unique<AudioOutputResampler>(
            &manager(), params_, resampler_params_, std::string(), close_delay, base::BindRepeating(&RegisterDebugRecording));
    }
    ...
    void OnStart() override
    {
        ...
        base::RunLoop run_loop;
        task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(kStartRunTimeMs));
        run_loop.Run();
    }
...
{
    MockAudioOutputStream stream(&manager_, params_);
    EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).Times(2).WillOnce(Return(static_cast<AudioOutputStream*>(NULL))).WillRepeatedly(Return(&stream));
    EXPECT_CALL(stream, Open()).WillOnce(Return(true));
...
TEST_F(AudioOutputResamplerTest, LowLatencyOpenFailedFallback)
{
    ...
    EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).Times(2).WillOnce(Return(&failed_stream)).WillRepeatedly(Return(&okay_stream));
    EXPECT_CALL(failed_stream, Open()).WillOnce(Return(false));
    EXPECT_CALL(failed_stream, Close()).Times(1);
    EXPECT_CALL(okay_stream, Open()).WillOnce(Return(true));
...
#endif
    EXPECT_CALL(manager(), MakeAudioOutputStream(_, _, _)).Times(kFallbackCount).WillRepeatedly(Return(static_cast<AudioOutputStream*>(NULL)));

    // To prevent shared memory issues the sample rate and buffer size should
    // match the input stream parameters.
    EXPECT_CALL(manager(),
        MakeAudioOutputStream(AllOf(testing::Property(&AudioParameters::format, AudioParameters::AUDIO_FAKE),
                                  testing::Property(&AudioParameters::sample_rate, params_.sample_rate()),
                                  testing::Property(&AudioParameters::frames_per_buffer, params_.frames_per_buffer())),
            _, _))
        .Times(1)
...
TEST_F(AudioOutputResamplerTest, FallbackRecovery)
{
    ...
    base::RunLoop run_loop;
    task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(2 * kTestCloseDelayMs));
    run_loop.Run();

    // Verify a non-fake stream can be created.
    MockAudioOutputStream real_stream(&manager_, params_);
    EXPECT_CALL(manager(), MakeAudioOutputStream(testing::Property(&AudioParameters::format, testing::Ne(AudioParameters::AUDIO_FAKE)), _, _))
...
TEST_F(AudioOutputResamplerTest, PropagatesGlitchInfo)
{
    ...
    AudioOutputStream::AudioSourceCallback* inner_callback = stream.GetCallback().value();
    media::AudioGlitchInfo glitch_info { .duration = base::Seconds(0.1), .count = 123 };
    auto dest = AudioBus::Create(resampler_params_);

    inner_callback->OnMoreData(base::TimeDelta(), base::TimeTicks(), glitch_info, dest.get());
    EXPECT_EQ(callback_.cumulative_glitch_info(), glitch_info);
    inner_callback->OnMoreData(base::TimeDelta(), base::TimeTicks(), {}, dest.get());
    EXPECT_EQ(callback_.cumulative_glitch_info(), glitch_info);
