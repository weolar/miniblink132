...
bool GetNextOffset(const unsigned char** pos, const unsigned char* end, const unsigned char** offset)
{
    if (*pos == end)
        return false;
...
bool IsMatch(const unsigned char* offset, const unsigned char* end, const char* key)
{
    ASSERT(offset < end);
    return *offset == *key;
}
...
bool IsEndCharMatch(const unsigned char* offset, const unsigned char* end, const char* key)
{
    ASSERT(offset < end);
...
bool GetReturnValue(const unsigned char* offset, const unsigned char* end, int* return_value)
{
    ASSERT(offset < end);
    if ((*offset & 0xE0) == 0x80) {
...
int LookupString(const unsigned char* graph, size_t length, const char* key, size_t key_length)
{
...
std::string GetDomainAndRegistryImpl(const std::string& host, PrivateRegistryFilter private_filter)
{
    ASSERT(!host.empty());
