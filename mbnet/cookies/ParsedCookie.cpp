...
inline bool SeekTo(std::string::const_iterator* it, const std::string::const_iterator& end, const char* chars)
{
...
// Returns true if it hit the end, false otherwise.
inline bool SeekPast(std::string::const_iterator* it, const std::string::const_iterator& end, const char* chars)
{
    for (; *it != end && CharIsA(**it, chars); ++(*it)) {
    }
    return *it == end;
}
inline bool SeekBackPast(std::string::const_iterator* it, const std::string::const_iterator& end, const char* chars)
{
    for (; *it != end && CharIsA(**it, chars); --(*it)) {
    }
    return *it == end;
...
bool IsValidCookieValue(const std::string& value)
{
    ...
    if (value.size() >= 2 && *value.begin() == '"' && *(value.end() - 1) == '"')
        skip = 1;
    for (std::string::const_iterator i = value.begin() + skip; i != value.end() - skip; ++i) {
...
std::string::const_iterator ParsedCookie::FindFirstTerminator(const std::string& s)
{
...
bool ParsedCookie::ParseToken(
    std::string::const_iterator* it, const std::string::const_iterator& end, std::string::const_iterator* token_start, std::string::const_iterator* token_end)
{
...
void ParsedCookie::ParseValue(
    std::string::const_iterator* it, const std::string::const_iterator& end, std::string::const_iterator* value_start, std::string::const_iterator* value_end)
{
...
bool ParsedCookie::SetString(size_t* index, const std::string& key, const std::string& value)
{
    if (value.empty()) {
...
void ParsedCookie::ClearAttributePair(size_t index)
{
    ...
    size_t* indexes[] = { &path_index_, &domain_index_, &expires_index_, &maxage_index_, &secure_index_, &httponly_index_, &priority_index_ };
