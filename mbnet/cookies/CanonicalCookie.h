...
public:
    ...
    CanonicalCookie(const blink::KURL& url, const std::string& name, const std::string& value, const std::string& domain, const std::string& path,
        const double& creation, const double& expiration, const double& last_access, bool secure, bool httponly, CookiePriority priority);
    ...
    const double& CreationDate() const
    {
        return creation_date_;
    }
    const double& LastAccessDate() const
    {
        return last_access_date_;
    }
    bool IsPersistent() const
    {
        return expiry_date_ != 0;
    }
    const double& ExpiryDate() const
    {
        return expiry_date_;
    }
    bool IsSecure() const
...
private:
    ...
    enum CookiePrefix { COOKIE_PREFIX_NONE = 0, COOKIE_PREFIX_SECURE, COOKIE_PREFIX_HOST, COOKIE_PREFIX_LAST };
    ...
    static void RecordCookiePrefixMetrics(CookiePrefix prefix, bool is_cookie_valid);
