...
void CookieMonster::FindCookiesForKey(const std::string& key, const blink::KURL& url, const CookieOptions& options, const double& current,
    bool update_access_time, std::vector<CanonicalCookie*>* cookies)
{
...
void CookieMonster::FindCookiesForHostAndDomain(
    const blink::KURL& url, const CookieOptions& options, bool update_access_time, std::vector<CanonicalCookie*>* cookies)
{
    //lock_.AssertAcquired();
...
void CookieMonster::UpdataCookie(const blink::KURL& url, const std::string& name, const std::string& value)
{
    ...
    if (!find) {
        CanonicalCookie* cc = new CanonicalCookie(
            url, name, value, WTF::WTFStringToStdString(url.host()), "/", WTF::currentTime(), 0, 0, false, false, COOKIE_PRIORITY_DEFAULT);
