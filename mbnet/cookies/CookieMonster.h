...
public:
    ...
    void FindCookiesForHostAndDomain(const blink::KURL& url, const CookieOptions& options, bool update_access_time, std::vector<CanonicalCookie*>* cookies);

    void FindCookiesForKey(const std::string& key, const blink::KURL& url, const CookieOptions& options, const double& current, bool update_access_time,
