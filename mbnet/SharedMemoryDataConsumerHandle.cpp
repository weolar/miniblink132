...
public:
    explicit DelegateThreadSafeReceivedData(PassOwnPtr<RequestPeer::ReceivedData> data)
        : m_data(data)
        , m_taskRunner(blink::Platform::current()->currentThread())
    {
    }
...
public:
    ...
    explicit BaseAutoLock(BaseLock& lock)
        : lock_(lock)
    {
        lock_.Acquire();
    }

    BaseAutoLock(BaseLock& lock, const AlreadyAcquired&)
        : lock_(lock)
    {
...
PassOwnPtr<blink::WebDataConsumerHandle::Reader> SharedMemoryDataConsumerHandle::otainReader(Client* client)
{
    return adoptPtr(obtainReaderInternal(client));
}

SharedMemoryDataConsumerHandle::ReaderImpl* SharedMemoryDataConsumerHandle::obtainReaderInternal(Client* client)
{
