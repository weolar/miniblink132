...
class SharedMemoryReceivedDataFactory::SharedMemoryReceivedData final : public RequestPeer::ReceivedData {
public:
    SharedMemoryReceivedData(const char* payload, int length, int encoded_length, scoped_refptr<SharedMemoryReceivedDataFactory> factory,
...
SharedMemoryReceivedDataFactory::SharedMemoryReceivedDataFactory(IPC::Sender* message_sender, int request_id, linked_ptr<base::SharedMemory> memory)
    : id_(0)
    , oldest_(0)
...
scoped_ptr<RequestPeer::ReceivedData> SharedMemoryReceivedDataFactory::Create(int offset, int length, int encoded_length)
{
    const char* start = static_cast<char*>(memory_->memory());
    const char* payload = start + offset;
    TicketId id = id_++;

    return make_scoped_ptr(new SharedMemoryReceivedData(payload, length, encoded_length, this, id));
}
...
void SharedMemoryReceivedDataFactory::Reclaim(TicketId id)
{
    ...
    std::sort(released_tickets_.begin(), released_tickets_.end(), TicketComparator(oldest_));
    size_t count = 0;
    for (size_t i = 0;; ++i) {
    ...
    released_tickets_.erase(released_tickets_.begin(), released_tickets_.begin() + count);
    oldest_ += count;
    SendAck(count);
}
