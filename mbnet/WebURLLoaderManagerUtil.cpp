...
{
    DEFINE_STATIC_LOCAL(WTF::RecursiveMutex, cookieMutex, ());
    DEFINE_STATIC_LOCAL(WTF::RecursiveMutex, dnsMutex, ());
    DEFINE_STATIC_LOCAL(WTF::RecursiveMutex, shareMutex, ());
...
{
    static const char* appendableHeaders[] = { "access-control-allow-headers", "access-control-allow-methods", "access-control-allow-origin",
        "access-control-expose-headers", "allow", "cache-control", "connection", "content-encoding", "content-language", "if-match", "if-none-match",
        "keep-alive", "pragma", "proxy-authenticate", "public", "server", "set-cookie", "te", "trailer", "transfer-encoding", "upgrade", "user-agent", "vary",
        "via", "warning", "www-authenticate", 0 };

    // Custom headers start with 'X-', and need no further checking.
    if (key.StartsWith("x-", WTF::kTextCaseASCIIInsensitive))
        return true;

    for (unsigned i = 0; appendableHeaders[i]; ++i) {
        if (0 == WTF::CodeUnitCompareIgnoringASCIICase(key.Impl(), appendableHeaders[i]))
            return true;
    }
