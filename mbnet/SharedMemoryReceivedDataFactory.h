...
class CONTENT_EXPORT SharedMemoryReceivedDataFactory final : public base::RefCounted<SharedMemoryReceivedDataFactory> {
public:
    SharedMemoryReceivedDataFactory(IPC::Sender* message_sender, int request_id, linked_ptr<base::SharedMemory> memory);

    scoped_ptr<RequestPeer::ReceivedData> Create(int offset, int length, int encoded_length);
