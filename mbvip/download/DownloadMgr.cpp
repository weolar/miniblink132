...
static void WKE_CALL_TYPE onDownloadUiWillRedirectCallback(
    wkeWebView webView, void* param, wkeWebUrlRequestPtr oldRequest, wkeWebUrlRequestPtr request, wkeWebUrlResponsePtr redirectResponse)
{
}
...
unsigned DownloadMgr::seleteDialogThread(void* param)
{
    ...
    common::ThreadCall::callUiThreadAsync(MB_FROM_HERE, [self, fileResult] { self->onSeletePathFinish(fileResult); });
...
mbDownloadOpt DownloadMgr::simpleDownload(mbWebView mbWebview, const WCHAR* path, size_t expectedContentLength, const char* url, const char* mime,
    const char* disposition, mbNetJob job, mbNetJobDataBind* dataBind, mbDownloadBind* callbackBind)
{
    mb::MbWebView* webview = (mb::MbWebView*)common::LiveIdDetect::get()->getPtr(mbWebview);
    if (!webview)
    ...
    SimpleDownload* downloader = SimpleDownload::create(mbWebview, path, expectedContentLength, url, mime, disposition, job, dataBind, callbackBind);
    if (downloader)
