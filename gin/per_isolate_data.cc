...
std::shared_ptr<gin::V8ForegroundTaskRunnerBase> CreateV8ForegroundTaskRunner(
    v8::Isolate* isolate, scoped_refptr<base::SingleThreadTaskRunner> task_runner, gin::IsolateHolder::AccessMode access_mode)
{
    if (access_mode == gin::IsolateHolder::kUseLocker) {
        return std::make_shared<gin::V8ForegroundTaskRunnerWithLocker>(isolate, std::move(task_runner));
    } else {
        return std::make_shared<gin::V8ForegroundTaskRunner>(std::move(task_runner));
    }
}
} // namespace
...
PerIsolateData::PerIsolateData(Isolate* isolate, ArrayBuffer::Allocator* allocator, IsolateHolder::AccessMode access_mode,
    ...
    , allocator_(allocator)
{
    ...
    DCHECK(task_runner);
    task_runner_ = CreateV8ForegroundTaskRunner(isolate_, std::move(task_runner), access_mode);
    if (user_visible_task_runner) {
        user_visible_task_runner_ = CreateV8ForegroundTaskRunner(isolate_, std::move(user_visible_task_runner), access_mode);
    }
    if (best_effort_task_runner) {
        best_effort_task_runner_ = CreateV8ForegroundTaskRunner(isolate_, std::move(best_effort_task_runner), access_mode);
    }
}
...
void PerIsolateData::EnableIdleTasks(std::unique_ptr<V8IdleTaskRunner> idle_task_runner)
{
