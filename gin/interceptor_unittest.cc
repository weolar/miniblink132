...
class MyInterceptor : public Wrappable<MyInterceptor>, public NamedPropertyInterceptor, public IndexedPropertyInterceptor {
public:
    ...
    v8::Local<v8::Value> GetNamedProperty(v8::Isolate* isolate, const std::string& property) override
    {
        if (property == "value") {
            return ConvertToV8(isolate, value_);
        } else if (property == "func") {
            v8::Local<v8::Context> context = isolate->GetCurrentContext();
            return GetFunctionTemplate(isolate, "func")->GetFunction(context).ToLocalChecked();
        } else {
            return v8::Local<v8::Value>();
        }
    }
    bool SetNamedProperty(v8::Isolate* isolate, const std::string& property, v8::Local<v8::Value> value) override
    {
        if (property == "value") {
    ...
    std::vector<std::string> EnumerateNamedProperties(v8::Isolate* isolate) override
    {
    ...
    v8::Local<v8::Value> GetIndexedProperty(v8::Isolate* isolate, uint32_t index) override
    {
        if (index == 0)
            return ConvertToV8(isolate, value_);
        return v8::Local<v8::Value>();
    }
    bool SetIndexedProperty(v8::Isolate* isolate, uint32_t index, v8::Local<v8::Value> value) override
    {
        if (index == 0) {
            ConvertFromV8(isolate, value, &value_);
            return true;
    ...
    std::vector<uint32_t> EnumerateIndexedProperties(v8::Isolate* isolate) override
    {
...
private:
    ...
    {
        return Wrappable<MyInterceptor>::GetObjectTemplateBuilder(isolate).AddNamedPropertyInterceptor().AddIndexedPropertyInterceptor();
    }
    ...
    v8::Local<v8::FunctionTemplate> GetFunctionTemplate(v8::Isolate* isolate, const std::string& name)
    {
        v8::Local<v8::FunctionTemplate> function_template = template_cache_.Get(name);
        if (!function_template.IsEmpty())
            return function_template;
        function_template = CreateFunctionTemplate(isolate, base::BindRepeating(&MyInterceptor::Call), InvokerOptions { true, nullptr });
...
{
    RunInterceptorTest("(function (obj) {"
