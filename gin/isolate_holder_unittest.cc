...
TEST_F(UseLockerIsolateHolderTest, FunctionTemplateLifetime)
{
    ...
    object_template->Set(isolate, "someFunction", gin::CreateFunctionTemplate(isolate, base::BindRepeating([]() {})));
    context->Global()->Set(context, StringToV8(isolate, "someObjectTemplate"), object_template->NewInstance(context).ToLocalChecked()).Check();
