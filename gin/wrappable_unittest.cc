...
TEST_F(WrappableTest, WrapAndUnwrap)
{
    ...
    v8::Local<v8::Value> wrapper = ConvertToV8(isolate, obj.get()).ToLocalChecked();
...
TEST_F(WrappableTest, GetAndSetProperty)
{
    ...
    v8::Local<v8::Script> script = v8::Script::Compile(context_.Get(isolate), source).ToLocalChecked();
    v8::Local<v8::Value> val = script->Run(context_.Get(isolate)).ToLocalChecked();
    v8::Local<v8::Function> func;
    EXPECT_TRUE(ConvertFromV8(isolate, val, &func));
    v8::Local<v8::Value> argv[] = {
        ConvertToV8(isolate, obj.get()).ToLocalChecked(),
    };
    func->Call(context_.Get(isolate), v8::Undefined(isolate), 1, argv).ToLocalChecked();
...
TEST_F(WrappableTest, MethodInvocationErrorsOnUnnamedObject)
{
    ...
    v8::Local<v8::Object> v8_object = ConvertToV8(isolate, obj.get()).ToLocalChecked().As<v8::Object>();
    v8::Local<v8::Value> member_method = v8_object->Get(context, StringToV8(isolate, "memberMethod")).ToLocalChecked();
    ASSERT_TRUE(member_method->IsFunction());
    v8::Local<v8::Value> non_member_method = v8_object->Get(context, StringToV8(isolate, "nonMemberMethod")).ToLocalChecked();
    ASSERT_TRUE(non_member_method->IsFunction());

    auto get_error = [isolate, context](v8::Local<v8::Value> function_to_run, v8::Local<v8::Value> context_object) {
        constexpr char kScript[] = "(function(toRun, contextObject) { toRun.apply(contextObject, []); })";
        ...
        func->Call(context, v8::Undefined(isolate), std::size(argv), argv).FromMaybe(v8::Local<v8::Value>());
    ...
    EXPECT_EQ("Uncaught TypeError: Illegal invocation", get_error(member_method, v8::Null(isolate)));
    ...
    EXPECT_EQ("Uncaught TypeError: Illegal invocation", get_error(member_method, wrong_object));
...
TEST_F(WrappableTest, MethodInvocationErrorsOnNamedObject)
{
    ...
    EXPECT_EQ("Uncaught TypeError: Illegal invocation: Function must be called on "
...
class MyObjectWithLazyProperties : public Wrappable<MyObjectWithLazyProperties> {
public:
    MyObjectWithLazyProperties(const MyObjectWithLazyProperties&) = delete;
    MyObjectWithLazyProperties& operator=(const MyObjectWithLazyProperties&) = delete;
...
private:
    ...
    ObjectTemplateBuilder GetObjectTemplateBuilder(v8::Isolate* isolate) final
    {
        return Wrappable::GetObjectTemplateBuilder(isolate)
            ...
            .SetLazyDataProperty("self", base::BindRepeating([](gin::Arguments* arguments) {
...
TEST_F(WrappableTest, LazyPropertyGetterCanBeSetFirst)
{
    ...
    bool set_ok = false;
    ASSERT_TRUE(v8_object->Set(context, key, v8::Int32::New(isolate, 1701)).To(&set_ok));
    ASSERT_TRUE(set_ok);
    ASSERT_TRUE(v8_object->Get(context, key).ToLocal(&value));
...
TEST_F(WrappableTest, LazyPropertyGetterCanBindSpecialArguments)
{
    ...
    v8::Local<v8::Object> v8_object = handle.ToV8().As<v8::Object>();
    v8::Local<v8::Value> value;
    ASSERT_TRUE(v8_object->Get(context, StringToSymbol(isolate, "self")).ToLocal(&value));
...
TEST_F(WrappableTest, CannotConstruct)
{
    ...
    v8::Local<v8::String> source = StringToV8(isolate, "(obj => new obj.constructor())");
    v8::Local<v8::Script> script = v8::Script::Compile(context, source).ToLocalChecked();
    v8::Local<v8::Function> function = script->Run(context).ToLocalChecked().As<v8::Function>();

    {
        TryCatch try_catch(isolate);
        EXPECT_TRUE(function->Call(context, v8::Undefined(isolate), 1, (v8::Local<v8::Value>[]) { wrapper }).IsEmpty());
