...
class ConvertableToTraceFormatWrapper final : public base::trace_event::ConvertableToTraceFormat {
public:
    explicit ConvertableToTraceFormatWrapper(std::unique_ptr<v8::ConvertableToTraceFormat> inner)
        : inner_(std::move(inner))
    {
    }
    ConvertableToTraceFormatWrapper(const ConvertableToTraceFormatWrapper&) = delete;
    ConvertableToTraceFormatWrapper& operator=(const ConvertableToTraceFormatWrapper&) = delete;
...
base::LazyInstance<gin::PageAllocator>::Leaky g_page_allocator = LAZY_INSTANCE_INITIALIZER;
...
public:
    explicit JobDelegateImpl(base::JobDelegate* delegate)
        : delegate_(delegate)
    {
    }
    ...
    {
        return delegate_->GetTaskId();
    }
    bool IsJoiningThread() const override
    {
        return delegate_->IsJoiningThread();
    }
...
public:
    explicit JobHandleImpl(base::JobHandle handle)
    ...
    {
        handle_.CancelAndDetach();
    }
    bool IsActive() override
    {
        return handle_.IsActive();
    }
...
public:
    explicit ScopedBlockingCallImpl(v8::BlockingType blocking_type)
        : scoped_blocking_call_(ToBaseBlockingType(blocking_type), base::internal::UncheckedScopedBlockingCall::BlockingCallType::kRegular)
    {
    }
...
// template <> struct base::trace_event::TraceValue::Helper<std::unique_ptr<v8::ConvertableToTraceFormat>> {
//     static constexpr unsigned char kType = TRACE_VALUE_TYPE_CONVERTABLE;
//     static inline void SetValue(TraceValue* v, std::unique_ptr<v8::ConvertableToTraceFormat> value)
//     {
//         // NOTE: |as_convertable| is an owning pointer, so using new here
//         // is acceptable.
//         v->as_convertable = new gin::ConvertableToTraceFormatWrapper(std::move(value));
//     }
// };
...
int V8Platform::NumberOfWorkerThreads()
{
    ...
    const size_t num_foreground_workers
        = base::ThreadPoolInstance::Get()->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ base::TaskPriority::USER_BLOCKING });
    DCHECK_GE(
        num_foreground_workers, base::ThreadPoolInstance::Get()->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ base::TaskPriority::USER_VISIBLE }));
    return std::max(1, static_cast<int>(num_foreground_workers));
}

void V8Platform::PostTaskOnWorkerThreadImpl(v8::TaskPriority priority, std::unique_ptr<v8::Task> task, const v8::SourceLocation& location)
{
    base::ThreadPool::PostTask(V8ToBaseLocation(location), { ToBaseTaskPriority(priority) }, base::BindOnce(&v8::Task::Run, std::move(task)));
}
    ...
    v8::TaskPriority priority, std::unique_ptr<v8::Task> task, double delay_in_seconds, const v8::SourceLocation& location)
{
    base::ThreadPool::PostDelayedTask(
        V8ToBaseLocation(location), { ToBaseTaskPriority(priority) }, base::BindOnce(&v8::Task::Run, std::move(task)), base::Seconds(delay_in_seconds));
}

std::unique_ptr<v8::JobHandle> V8Platform::CreateJobImpl(v8::TaskPriority priority, std::unique_ptr<v8::JobTask> job_task, const v8::SourceLocation& location)
{
    ...
    auto handle = base::CreateJob(V8ToBaseLocation(location), { ToBaseTaskPriority(priority), base::ThreadPolicy::PREFER_BACKGROUND },
        base::BindRepeating(
            [](const std::unique_ptr<v8::JobTask>& job_task, base::JobDelegate* delegate) {
                JobDelegateImpl delegate_impl(delegate);
                job_task->Run(&delegate_impl);
            },
            std::move(job_task)),
        base::BindRepeating(
            [](v8::JobTask* job_task, size_t worker_count) { return job_task->GetMaxConcurrency(worker_count); }, base::Unretained(job_task_ptr)));
