...
public:
    ...
    TimeClamper()
        : secret_(base::RandUint64())
    {
    }
    // This constructor should only be used in tests.
    explicit TimeClamper(uint64_t secret)
        : secret_(secret)
    {
    }
    ...
    inline int64_t ClampToMillis(base::Time time) const
    {
        ...
        const int64_t now_micros = (time - base::Time::UnixEpoch()).InMicroseconds();
    ...
    {
        const int64_t clamped_time = ClampTimeResolution((now - base::Time::UnixEpoch()).InMicroseconds());
...
private:
    ...
    inline int64_t ThresholdFor(int64_t clamped_time) const
    {
        ...
        return std::min(static_cast<int64_t>(random_value / (std::numeric_limits<uint64_t>::max() / kResolutionMicros)), kResolutionMicros);
    }
