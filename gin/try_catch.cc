...
v8::Local<v8::String> GetSourceLine(v8::Isolate* isolate, v8::Local<v8::Message> message)
{
...
std::string TryCatch::GetStackTrace()
{
    ...
    ss << V8ToString(isolate_, message->Get()) << std::endl << V8ToString(isolate_, GetSourceLine(isolate_, message)) << std::endl;
    ...
    for (int i = 0; i < len; ++i) {
        ...
        ss << V8ToString(isolate_, frame->GetScriptName()) << ":" << frame->GetLineNumber() << ":" << frame->GetColumn() << ": "
