...
{
    auto maybe = object->DefineOwnProperty(isolate->GetCurrentContext(), key, value);
...
template <> struct GIN_EXPORT Converter<std::string_view> {
    // This crashes when val.size() > v8::String::kMaxLength.
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, const std::string_view& val);
...
template <> struct GIN_EXPORT Converter<base::TimeTicks> {
...
template <> struct GIN_EXPORT Converter<v8::Local<v8::Function>> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, v8::Local<v8::Function> val);
...
template <> struct GIN_EXPORT Converter<v8::Local<v8::Promise>> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, v8::Local<v8::Promise> val);
    static bool FromV8(v8::Isolate* isolate, v8::Local<v8::Value> val, v8::Local<v8::Promise>* out);
};
...
template <typename T> struct Converter<std::vector<T>> {
    static std::conditional_t<internal::ToV8ReturnsMaybe<T>, v8::MaybeLocal<v8::Value>, v8::Local<v8::Value>> ToV8(
        v8::Isolate* isolate, const std::vector<T>& val)
    {
        ...
        for (uint32_t i = 0; i < val.size(); ++i) {
            ...
            if (!result->CreateDataProperty(context, i, element).To(&property_created) || !property_created) {
...
template <typename T> struct Converter<v8::LocalVector<T>> {
    static std::conditional_t<internal::ToV8ReturnsMaybe<v8::Local<T>>, v8::MaybeLocal<v8::Value>, v8::Local<v8::Value>> ToV8(
        v8::Isolate* isolate, const v8::LocalVector<T>& val)
    {
        v8::Local<v8::Context> context = isolate->GetCurrentContext();
        v8::Local<v8::Array> result(v8::Array::New(isolate, static_cast<int>(val.size())));
        for (uint32_t i = 0; i < val.size(); ++i) {
            v8::MaybeLocal<v8::Value> maybe = Converter<v8::Local<T>>::ToV8(isolate, val[i]);
            v8::Local<v8::Value> element;
            if (!maybe.ToLocal(&element)) {
                return {};
            }
    ...
    static bool FromV8(v8::Isolate* isolate, v8::Local<v8::Value> val, v8::LocalVector<T>* out)
    {
        if (!val->IsArray()) {
            return false;
        }
...
template <typename T>
std::conditional_t<internal::ToV8ReturnsMaybe<T>, v8::MaybeLocal<v8::Value>, v8::Local<v8::Value>> ConvertToV8(v8::Isolate* isolate, const T& input)
{
...
GIN_EXPORT inline v8::Local<v8::String> StringToV8(v8::Isolate* isolate, const std::string_view& input)
{
    return ConvertToV8(isolate, input).As<v8::String>();
}

// This crashes when input.size() > v8::String::kMaxLength.
GIN_EXPORT v8::Local<v8::String> StringToSymbol(v8::Isolate* isolate, const std::string_view& val);

// This crashes when input.size() > v8::String::kMaxLength.
GIN_EXPORT v8::Local<v8::String> StringToSymbol(v8::Isolate* isolate, const std::u16string_view& val);
...
GIN_EXPORT std::string V8ToString(v8::Isolate* isolate, v8::Local<v8::Value> value);
