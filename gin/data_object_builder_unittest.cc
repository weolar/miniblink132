...
TEST_F(DataObjectBuilderTest, CreatesDataProperties)
{
    ...
    v8::Local<v8::Object> object = DataObjectBuilder(isolate).Set("key", 42).Build();
    ASSERT_TRUE(object->HasOwnProperty(context, StringToSymbol(isolate, "key")).ToChecked());

    v8::Local<v8::Value> descriptor_object;
    ASSERT_TRUE(object->GetOwnPropertyDescriptor(context, StringToSymbol(isolate, "key")).ToLocal(&descriptor_object));
...
TEST_F(DataObjectBuilderTest, DoesNotInvokeSetters)
{
    ...
    ASSERT_TRUE(context->Global()->Get(context, StringToSymbol(isolate, "Object")).ToLocal(&object_constructor));
    v8::Local<v8::Value> object_prototype;
    ASSERT_TRUE(object_constructor.As<v8::Function>()->Get(context, StringToSymbol(isolate, "prototype")).ToLocal(&object_prototype));
    ASSERT_TRUE(
        object_prototype.As<v8::Object>()
            ->SetNativeDataProperty(
                context, StringToSymbol(isolate, "key"), [](v8::Local<v8::Name>, const v8::PropertyCallbackInfo<v8::Value>&) {},
                [](v8::Local<v8::Name>, v8::Local<v8::Value>, const v8::PropertyCallbackInfo<void>&) { ADD_FAILURE() << "setter should not be invoked"; })
            .ToChecked());
...
TEST_F(DataObjectBuilderTest, UnusableAfterBuild)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(builder.Build(), "Check failed: !object_.IsEmpty\\(\\)");
}
#endif // DCHECK_IS_ON()
...
TEST_F(DataObjectBuilderTest, ReplacesExistingProperties)
{
    ...
    v8::Local<v8::Object> object = DataObjectBuilder(isolate).Set("value", 42).Set("value", 55).Build();
...
TEST_F(DataObjectBuilderTest, CreatesDataPropertiesForIndices)
{
    ...
    v8::Local<v8::Object> object = DataObjectBuilder(isolate).Set(42, std::string_view("forty-two")).Build();
    ASSERT_TRUE(object->HasOwnProperty(context, 42).ToChecked());

    v8::Local<v8::Value> descriptor_object;
    ASSERT_TRUE(object->GetOwnPropertyDescriptor(context, StringToSymbol(isolate, "42")).ToLocal(&descriptor_object));
    gin::Dictionary descriptor(isolate, descriptor_object.As<v8::Object>());
