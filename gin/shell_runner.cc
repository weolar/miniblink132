...
v8::Local<ObjectTemplate> ShellRunnerDelegate::GetGlobalTemplate(ShellRunner* runner, v8::Isolate* isolate)
{
...
void ShellRunnerDelegate::UnhandledException(ShellRunner* runner, TryCatch& try_catch)
{
...
v8::MaybeLocal<v8::Value> ShellRunner::Run(const std::string& source, const std::string& resource_name)
{
    v8::Isolate* isolate = GetContextHolder()->isolate();
    TryCatch try_catch(isolate);
    v8::ScriptOrigin origin(StringToV8(isolate, resource_name));
    auto maybe_script = Script::Compile(GetContextHolder()->context(), StringToV8(isolate, source), &origin);
