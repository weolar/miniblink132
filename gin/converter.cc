...
template <typename T, typename U> bool FromMaybe(Maybe<T> maybe, U* out)
{
    if (maybe.IsNothing())
        return false;
...
bool Converter<int32_t>::FromV8(Isolate* isolate, Local<Value> val, int32_t* out)
{
    if (!val->IsInt32())
        return false;
...
bool Converter<uint32_t>::FromV8(Isolate* isolate, Local<Value> val, uint32_t* out)
{
    if (!val->IsUint32())
        return false;
...
bool Converter<uint64_t>::FromV8(Isolate* isolate, Local<Value> val, uint64_t* out)
{
    if (!val->IsNumber())
        return false;
    return FromMaybe(val->IntegerValue(isolate->GetCurrentContext()), out);
...
Local<Value> Converter<std::string_view>::ToV8(Isolate* isolate, const std::string_view& val)
{
    return String::NewFromUtf8(isolate, val.data(), v8::NewStringType::kNormal, static_cast<uint32_t>(val.length())).ToLocalChecked();
}

Local<Value> Converter<std::string>::ToV8(Isolate* isolate, const std::string& val)
{
    return Converter<std::string_view>::ToV8(isolate, val);
}

bool Converter<std::string>::FromV8(Isolate* isolate, Local<Value> val, std::string* out)
{
    if (!val->IsString())
        return false;
...
Local<Value> Converter<std::u16string>::ToV8(Isolate* isolate, const std::u16string& val)
{
    return String::NewFromTwoByte(isolate, reinterpret_cast<const uint16_t*>(val.data()), v8::NewStringType::kNormal, val.size()).ToLocalChecked();
}

bool Converter<std::u16string>::FromV8(Isolate* isolate, Local<Value> val, std::u16string* out)
{
    if (!val->IsString())
        return false;
    Local<String> str = Local<String>::Cast(val);
    ...
    str->Write(isolate, reinterpret_cast<uint16_t*>(base::WriteInto(out, length + 1)), 0, length);
    return true;
}

v8::Local<v8::Value> Converter<base::TimeTicks>::ToV8(v8::Isolate* isolate, base::TimeTicks val)
{
    return v8::BigInt::New(isolate, val.since_origin().InMicroseconds()).As<v8::Value>();
}

Local<Value> Converter<Local<Function>>::ToV8(Isolate* isolate, Local<Function> val)
{
    return val.As<Value>();
}

bool Converter<Local<Function>>::FromV8(Isolate* isolate, Local<Value> val, Local<Function>* out)
{
    if (!val->IsFunction())
        return false;
...
bool Converter<Local<Object>>::FromV8(Isolate* isolate, Local<Value> val, Local<Object>* out)
{
    if (!val->IsObject())
        return false;
...
Local<Value> Converter<Local<Promise>>::ToV8(Isolate* isolate, Local<Promise> val)
{
    return val.As<Value>();
}

bool Converter<Local<Promise>>::FromV8(Isolate* isolate, Local<Value> val, Local<Promise>* out)
{
    if (!val->IsPromise())
        return false;
...
v8::Local<v8::String> StringToSymbol(v8::Isolate* isolate, const std::string_view& val)
{
    return String::NewFromUtf8(isolate, val.data(), v8::NewStringType::kInternalized, static_cast<uint32_t>(val.length())).ToLocalChecked();
}

v8::Local<v8::String> StringToSymbol(v8::Isolate* isolate, const std::u16string_view& val)
{
    return String::NewFromTwoByte(isolate, reinterpret_cast<const uint16_t*>(val.data()), v8::NewStringType::kInternalized, val.length()).ToLocalChecked();
}

base::Location V8ToBaseLocation(const v8::SourceLocation& location)
{
    return base::Location::Current(location.Function(), location.FileName(), location.Line());
}
