...
TEST_F(ConverterTest, Bool)
{
    ...
    struct {
        ...
        { String::NewFromUtf8(instance_->isolate(), "", v8::NewStringType::kNormal).ToLocalChecked().As<Value>(), false },
        { String::NewFromUtf8(instance_->isolate(), "foo", v8::NewStringType::kNormal).ToLocalChecked().As<Value>(), true },
    ...
    for (size_t i = 0; i < std::size(test_data); ++i) {
        bool result = false;
        EXPECT_TRUE(Converter<bool>::FromV8(instance_->isolate(), test_data[i].input, &result));
...
TEST_F(ConverterTest, String16)
{
    ...
    EXPECT_TRUE(Converter<std::u16string>::ToV8(isolate, u"")->StrictEquals(StringToV8(isolate, "")));
    EXPECT_TRUE(Converter<std::u16string>::ToV8(isolate, u"hello")->StrictEquals(StringToV8(isolate, "hello")));

    std::u16string result;

    ASSERT_FALSE(Converter<std::u16string>::FromV8(isolate, v8::False(isolate), &result));
    ASSERT_FALSE(Converter<std::u16string>::FromV8(isolate, v8::True(isolate), &result));
    ASSERT_TRUE(Converter<std::u16string>::FromV8(isolate, v8::String::Empty(isolate), &result));
    EXPECT_EQ(result, std::u16string());
    ASSERT_TRUE(Converter<std::u16string>::FromV8(isolate, StringToV8(isolate, "hello"), &result));
...
TEST_F(ConverterTest, Int32)
{
    ...
    for (size_t i = 0; i < std::size(test_data_to); ++i) {
        EXPECT_TRUE(Converter<int32_t>::ToV8(instance_->isolate(), test_data_to[i])->StrictEquals(Integer::New(instance_->isolate(), test_data_to[i])));
    }
    ...
    struct {
        ...
        { String::NewFromUtf8(instance_->isolate(), "42", v8::NewStringType::kNormal).ToLocalChecked().As<Value>(), false, 0 },
        { String::NewFromUtf8(instance_->isolate(), "foo", v8::NewStringType::kNormal).ToLocalChecked().As<Value>(), false, 0 },
    ...
    for (size_t i = 0; i < std::size(test_data_from); ++i) {
        ...
        bool success = Converter<int32_t>::FromV8(instance_->isolate(), test_data_from[i].input, &result);
...
TEST_F(ConverterTest, Vector)
{
    ...
    auto js_array = Converter<std::vector<int>>::ToV8(instance_->isolate(), expected).As<Array>();
    EXPECT_EQ(3u, js_array->Length());
    v8::Local<v8::Context> context = instance_->isolate()->GetCurrentContext();
    for (size_t i = 0; i < expected.size(); ++i) {
        EXPECT_TRUE(Integer::New(instance_->isolate(), expected[i])->StrictEquals(js_array->Get(context, static_cast<int>(i)).ToLocalChecked()));
    }
}
...
TEST_F(ConverterTest, VectorOfVectors)
{
    ...
    v8::Local<v8::Value> v8_value = ConvertToV8(instance_->isolate(), vector_of_vectors);
...
template <> struct Converter<MoveOnlyObject> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, MoveOnlyObject in)
    {
        return v8::Undefined(isolate);
    }
    static bool FromV8(v8::Isolate* isolate, v8::Local<v8::Value> val, MoveOnlyObject* out)
    {
        *out = MoveOnlyObject();
        return true;
    }
...
TEST_F(ConverterTest, MoveOnlyParameters)
{
    ...
    auto func_templ = gin::CreateFunctionTemplate(isolate, base::BindRepeating(receives_move_only_obj));
