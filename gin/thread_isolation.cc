...
WEAK_SYMBOL extern int pkey_alloc(unsigned int flags, unsigned int access_rights);
...
void PkeyDisableWriteAccess(int pkey)
{
    ...
    uint32_t disable_write = static_cast<uint32_t>(PKEY_DISABLE_WRITE) << (static_cast<uint32_t>(pkey) * kBitsPerPkey);
...
ThreadIsolationData& GetThreadIsolationData()
{
    ...
    DCHECK_EQ((reinterpret_cast<size_t>(&thread_isolation_data) % PA_THREAD_ISOLATED_ALIGN_SZ), 0llu);
