...
static const curl_OID OIDtable[] = { { "1.2.840.10040.4.1", "dsa" }, { "1.2.840.10040.4.3", "dsa-with-sha1" }, { "1.2.840.10045.2.1", "ecPublicKey" },
...
static const char* GTime2str(const char* beg, const char* end)
{
    ...
    return curl_maprintf(
        "%.4s-%.2s-%.2s %.2s:%.2s:%c%c%s%.*s%s%.*s", beg, beg + 4, beg + 6, beg + 8, beg + 10, sec1, sec2, fracl ? "." : "", fracl, fracp, sep, tzl, tzp);
}
...
static const char* UTime2str(const char* beg, const char* end)
{
    ...
    tzl = end - tzp;
    return curl_maprintf("%u%.2s-%.2s-%.2s %.2s:%.2s:%.2s %.*s", 20 - (*beg >= '5'), beg, beg + 2, beg + 4, beg + 6, beg + 8, sec, tzl, tzp);
}
...
int Curl_parseX509(curl_X509certificate* cert, const char* beg, const char* end)
{
    ...
    getASN1Element(&cert->subjectPublicKey, ccp, cert->subjectPublicKeyInfo.end);
...
static void do_pubkey_field(struct Curl_easy* data, int certnum, const char* label, curl_asn1Element* elem)
{
    const char* output;
...
static void do_pubkey(struct Curl_easy* data, int certnum, const char* algo, curl_asn1Element* param, curl_asn1Element* pubkey)
{
    curl_asn1Element elem;
    curl_asn1Element pk;
    const char* p;
...
CURLcode Curl_extract_certinfo(struct connectdata* conn, int certnum, const char* beg, const char* end)
{
    ...
    result = Curl_base64_encode(data, cert.certificate.beg, cert.certificate.end - cert.certificate.beg, &cp1, &cl1);
    if (result)
        return result;
    ...
    for (p = cert.extensions.beg; p < cert.extensions.end && matched != 1;) {
        ...
        if (ext.beg) {
            ...
            for (q = elem.beg; matched != 1 && q < elem.end;) {
                ...
                case 7: /* IP address. */
                    matched = (size_t)(name.end - q) == addrlen && !memcmp(&addr, q, addrlen);
                    break;
                }
            }
        }
