...
static bool ConnectionExists(struct Curl_easy* data, struct connectdata* needle, struct connectdata** usethis, bool* force_reuse, bool* waitpipe)
{
    ...
    *waitpipe = FALSE;
    ...
    if ((canpipe & CURLPIPE_HTTP1) && Curl_pipeline_site_blacklisted(data, needle))
        ...
        */
           ...
           is allowed to be upgraded via TLS */
                ...
                if ((strcasecompare(needle->handler->scheme, check->handler->scheme)
                        || (get_protocol_family(check->handler->protocol) == needle->handler->protocol && check->tls_upgraded))
...
#define STARTS_WITH_URL_DRIVE_PREFIX(str)                                                                                                                      \
    ((('a' <= (str)[0] && (str)[0] <= 'z') || ('A' <= (str)[0] && (str)[0] <= 'Z')) && ((str)[1] == ':' || (str)[1] == '|')                                    \
        && ((str)[2] == '/' || (str)[2] == '\\' || (str)[2] == 0))

    /* Don't mistake a drive letter for a scheme if the default protocol is file.
     curld --proto-default file c:/foo/bar.txt */
    if (STARTS_WITH_DRIVE_PREFIX(data->change.url) && data->set.str[STRING_DEFAULT_PROTOCOL]
    ...
    if ((url_has_scheme && strncasecompare(data->change.url, "file:", 5))
        || (!url_has_scheme && data->set.str[STRING_DEFAULT_PROTOCOL] && strcasecompare(data->set.str[STRING_DEFAULT_PROTOCOL], "file"))) {
        if (url_has_scheme)
     ...
     * This catches both "file:/c:" and "file:c:" */
        if (('/' == path[0] && STARTS_WITH_URL_DRIVE_PREFIX(&path[1])) || STARTS_WITH_URL_DRIVE_PREFIX(path)) {
...
static CURLcode parse_proxy(struct Curl_easy* data, struct connectdata* conn, char* proxy, curl_proxytype proxytype)
{
   ...
   */
    ...
    if (atsign) {
        CURLcode result = Curl_parse_login_details(proxyptr, atsign - proxyptr, &proxyuser, &proxypasswd, NULL);
        if (result)
            return result;
        proxyptr = atsign + 1;
    }
...
static CURLcode parse_url_login(struct Curl_easy* data, struct connectdata* conn, char** user, char** passwd, char** options)
{
   ...
   */
    ...
    result = Curl_parse_login_details(login, ptr - login - 1, &userp, &passwdp, (conn->handler->flags & PROTOPT_URLOPTIONS) ? &optionsp : NULL);
    if (result)
        goto out;
    ...
    if (userp) {
        ...
        if (data->set.disallow_username_in_url) {
            failf(data,
                "Option DISALLOW_USERNAME_IN_URL is set "
    ...
    if (data->set.use_port && data->state.allow_port) {
        ...
        if (conn->bits.httpproxy) {
            ...
            if (conn->bits.type_set)
                snprintf(type, sizeof(type), ";type=%c", data->set.prefer_ascii ? 'A' : (data->set.ftp_list_only ? 'D' : 'I'));
       ...
       */
            url = aprintf("%s://%s%s%s:%d%s%s%s", conn->given->scheme, conn->bits.ipv6_ip ? "[" : "", conn->host.name, conn->bits.ipv6_ip ? "]" : "",
                conn->remote_port, data->state.slash_removed ? "/" : "", data->state.path, type);
            if (!url)
                return CURLE_OUT_OF_MEMORY;
   ...
   *************************************************************/
    ...
    if (reuse && !force_reuse && IsPipeliningPossible(data, conn_temp)) {
        ...
        if (pipelen > 0) {
            ...
            if (Curl_conncache_bundle_size(conn_temp) < max_host_connections && Curl_conncache_size(data) < max_total_connections) {
