...
static int myssh_getsock(struct connectdata* conn, curl_socket_t* sock, int numsocks);

static int myssh_perform_getsock(const struct connectdata* conn, curl_socket_t* sock, int numsocks);
...
static int myssh_is_known(struct connectdata* conn)
{
    ...
    if (data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5]) {
        ...
        if (hlen != strlen(data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5]) || memcmp(&data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5], hash, hlen)) {
            rc = SSH_ERROR;
            goto cleanup;
        }
            ...
            if (sshc->readdir_attrs) {
             ...
             newline in this counter */
                    ...
                    if ((sshc->readdir_attrs->flags & SSH_FILEXFER_ATTR_PERMISSIONS) && ((sshc->readdir_attrs->permissions & S_IFMT) == S_IFLNK)) {
                        ...
                        snprintf(sshc->readdir_linkPath, PATH_MAX, "%s%s", protop->path, sshc->readdir_filename);
...
static int myssh_perform_getsock(const struct connectdata* conn, curl_socket_t* sock, /* points to numsocks
                                                          number of sockets */
    int numsocks)
{
...
static int myssh_getsock(struct connectdata* conn, curl_socket_t* sock, /* points to numsocks
                                                   number of sockets */
    int numsocks)
{
...
static CURLcode myssh_connect(struct connectdata* conn, bool* done)
{
    ...
    if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
        ...
        ssh_options_set(ssh->ssh_session, SSH_OPTIONS_KNOWNHOSTS, data->set.str[STRING_SSH_KNOWNHOSTS]);
    }
