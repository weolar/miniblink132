...
static CURLcode telrcv(struct connectdata*, const unsigned char* inbuf, /* Data received from socket */
...
static void printoption(struct Curl_easy* data, const char* direction, int cmd, int option)
{
    if (data->set.verbose) {
        if (cmd == CURL_IAC) {
            ...
            const char* fmt = (cmd == CURL_WILL) ? "WILL" : (cmd == CURL_WONT) ? "WONT" : (cmd == CURL_DO) ? "DO" : (cmd == CURL_DONT) ? "DONT" : 0;
            if (fmt) {
                const char* opt;
    ...
    size_t length) /* length of suboption data */
{
    if (data->set.verbose) {
        ...
        case CURL_TELOPT_NAWS:
            if (length > 4)
                infof(data, "Width: %d ; Height: %d", (pointer[1] << 8) | pointer[2], (pointer[3] << 8) | pointer[4]);
            break;
        default:
            switch (pointer[1]) {
...
static void suboption(struct connectdata* conn)
{
    ...
    case CURL_TELOPT_TTYPE:
        ...
        snprintf((char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_TTYPE, CURL_TELQUAL_IS, tn->subopt_ttype, CURL_IAC, CURL_SE);
    ...
    case CURL_TELOPT_XDISPLOC:
        ...
        snprintf((char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_XDISPLOC, CURL_TELQUAL_IS, tn->subopt_xdisploc, CURL_IAC, CURL_SE);
        bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
        if (bytes_written < 0) {
            err = SOCKERRNO;
            failf(data, "Sending data failed (%d)", err);
        ...
        break;
    case CURL_TELOPT_NEW_ENVIRON:
        snprintf((char*)temp, sizeof(temp), "%c%c%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_NEW_ENVIRON, CURL_TELQUAL_IS);
        len = 4;
        ...
        for (v = tn->telnet_vars; v; v = v->next) {
            ...
            if (len + tmplen < (int)sizeof(temp) - 6) {
                if (sscanf(v->data, "%127[^,],%127s", varname, varval)) {
                    snprintf((char*)&temp[len], sizeof(temp) - len, "%c%s%c%s", CURL_NEW_ENV_VAR, varname, CURL_NEW_ENV_VALUE, varval);
                    len += tmplen;
                }
            }
        }
        snprintf((char*)&temp[len], sizeof(temp) - len, "%c%c", CURL_IAC, CURL_SE);
        len += 2;
        bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
        if (bytes_written < 0) {
            err = SOCKERRNO;
...
static CURLcode telrcv(struct connectdata* conn, const unsigned char* inbuf, /* Data received from socket */
    ssize_t count) /* Number of bytes received */
{
    unsigned char c;
    CURLcode result;
...
#define startskipping()                                                                                                                                        \
    if (startwrite >= 0) {                                                                                                                                     \
        result = Curl_client_write(conn, CLIENTWRITE_BODY, (char*)&inbuf[startwrite], in - startwrite);                                                        \
        if (result)                                                                                                                                            \
            return result;                                                                                                                                     \
    }                                                                                                                                                          \
    startwrite = -1
...
static CURLcode send_telnet_data(struct connectdata* conn, char* buffer, ssize_t nread)
{
    ...
    while (!result && total_written < outlen) {
        ...
        default: /* write! */
            bytes_written = 0;
            result = Curl_write(conn, conn->sock[FIRSTSOCKET], outbuf + total_written, outlen - total_written, &bytes_written);
...
static CURLcode telnet_do(struct connectdata* conn, bool* done)
{
  ...
  */
    ...
    create_event_func = CURLX_FUNCTION_CAST(WSOCK2_EVENT, (GetProcAddress(wsock2, "WSACreateEvent")));
        ...
        default: /* read! */
           ...
           non-telnet servers, like POP or SMTP. */
            ...
            if (poll_cnt == 2) {
                ...
                /* read from user-supplied method */
                nread = (int)data->state.fread_func(buf, 1, data->set.buffer_size, data->state.in);
