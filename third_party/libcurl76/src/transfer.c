...
CURLcode Curl_fillreadbuffer(struct connectdata* conn, size_t bytes, size_t* nreadp)
{
    ...
    nread = data->state.fread_func(data->req.upload_fromhere, 1, buffersize, data->state.in);
     ...
     */
        ...
        hexlen = snprintf(hexbuffer, sizeof(hexbuffer), "%x%s", nread, endofline_native);
...
static void read_rewind(struct connectdata* conn, size_t thismuch)
{
...
#ifdef DEBUGBUILD
    {
        ...
        DEBUGF(infof(conn->data, "Buffer after stream rewind (read_pos = %zu): [%s]\n", conn->read_pos, buf));
    }
#endif
}
        ...
        if (k->str && !k->header && (nread > 0 || is_empty_data)) {
           ...
           write a piece of the body */
               ...
               response in "ignore more" */
                    ...
                    if (data->state.resume_from && !k->content_range && (data->set.httpreq == HTTPREQ_GET) && !k->ignorebody) {
                ...
                if (excess > 0 && !k->ignorebody) {
                    if (Curl_pipeline_wanted(conn->data->multi, CURLPIPE_HTTP1)) {
                        infof(data,
                            ...
                            " bytes on url %s (size = %" CURL_FORMAT_CURL_OFF_T ", maxdownload = %" CURL_FORMAT_CURL_OFF_T
                            ", bytecount = %" CURL_FORMAT_CURL_OFF_T ", nread = %zd)\n",
                            excess, data->state.path, k->size, k->maxdownload, k->bytecount, nread);
            ...
            if (!k->chunk && (nread || k->badheader || is_empty_data)) {
             ...
             and now we output it as body instead */
                    ...
                    if (k->maxdownload == -1 || (curl_off_t)k->hbuflen <= k->maxdownload)
                        result = Curl_client_write(conn, CLIENTWRITE_BODY, data->state.headerbuff, k->hbuflen);
                    else
                        result = Curl_client_write(conn, CLIENTWRITE_BODY, data->state.headerbuff, (size_t)k->maxdownload);
...
CURLcode Curl_readwrite(struct connectdata* conn, struct Curl_easy* data, bool* done, bool* comeback)
{
    ...
    /* We go ahead and do a read if we have a readable socket or if
     the stream was rewound (in which case we have data in a
     buffer) */
    if ((k->keepon & KEEP_RECV) && ((select_res & CURL_CSELECT_IN) || conn->bits.stream_was_rewound)) {
        ...
        if (0 > Curl_timeleft(data, &k->now, FALSE)) {
            if (k->size != -1) {
                failf(data, "Operation timed out after %ld milliseconds with %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes received",
                    Curl_timediff(k->now, data->progress.t_startsingle), k->bytecount, k->size);
            } else {
 ...
 */
int Curl_single_getsock(const struct connectdata* conn, curl_socket_t* sock, /* points to numsocks number
                                                of sockets */
    int numsocks)
{
    ...
    followtype type) /* see transfer.h */
{
   ...
   */
     ...
     */
        if ((data->set.httpreq == HTTPREQ_POST || data->set.httpreq == HTTPREQ_POST_FORM || data->set.httpreq == HTTPREQ_POST_MIME)
    ...
    case 303: /* See Other */
       ...
       asks for POST after POST */
        if (data->set.httpreq != HTTPREQ_GET && !(data->set.keep_post & CURL_REDIR_POST_303)) {
...
CURLcode Curl_retry_request(struct connectdata* conn, char** url)
{
    ...
    *url = NULL;
    ...
    if ((data->req.bytecount + data->req.headerbytecount == 0) && conn->bits.reuse && (!data->set.opt_no_body || (conn->handler->protocol & PROTO_FAMILY_HTTP))
