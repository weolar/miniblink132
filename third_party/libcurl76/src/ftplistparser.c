...
typedef union {
    ...
    enum { PL_UNIX_TIME_PREPART1 = 0, PL_UNIX_TIME_PART1, PL_UNIX_TIME_PREPART2, PL_UNIX_TIME_PART2, PL_UNIX_TIME_PREPART3, PL_UNIX_TIME_PART3 } time;

    enum { PL_UNIX_FILENAME_PRESPACE = 0, PL_UNIX_FILENAME_NAME, PL_UNIX_FILENAME_WINDOWSEOL } filename;
...
typedef enum { PL_WINNT_DATE = 0, PL_WINNT_TIME, PL_WINNT_DIRORSIZE, PL_WINNT_FILENAME } pl_winNT_mainstate;
...
typedef union {
    ...
    enum { PL_WINNT_FILENAME_PRESPACE = 0, PL_WINNT_FILENAME_CONTENT, PL_WINNT_FILENAME_WINEOL } filename;
} pl_winNT_substate;
            ...
            case PL_UNIX_SIZE:
                ...
                case PL_UNIX_SIZE_NUMBER:
                    ...
                    if (c == ' ') {
                        ...
                        curl_off_t fsize;
                        finfo->b_data[parser->item_offset + parser->item_length - 1] = 0;
                        if (!curlx_strtoofft(finfo->b_data + parser->item_offset, &p, 10, &fsize)) {
                ...
                case PL_WINNT_DIRORSIZE_CONTENT:
                    ...
                    if (c == ' ') {
                        ...
                        if (strcmp("<DIR>", finfo->b_data + parser->item_offset) == 0) {
                            ...
                            char* endptr;
                            if (curlx_strtoofft(finfo->b_data + parser->item_offset, &endptr, 10, &finfo->size)) {
                                parser->error = CURLE_FTP_BAD_FILE_LIST;
                                goto fail;
                            }
                            /* correct file type */
