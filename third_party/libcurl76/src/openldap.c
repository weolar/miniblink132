...
static const char* url_errs[] = { "success", "out of memory", "bad parameter", "unrecognized scheme", "unbalanced delimiter", "bad URL", "bad host or port",
    "bad or missing attributes", "bad or missing scope", "bad or missing filter", "bad or missing extensions" };
...
static CURLcode ldap_connect(struct connectdata* conn, bool* done)
{
    ...
    if (conn->handler->flags & PROTOPT_SSL)
        *ptr++ = 's';
    snprintf(ptr, sizeof(hosturl) - (ptr - hosturl), "://%s:%d", conn->host.name, conn->remote_port);
...
static CURLcode ldap_do(struct connectdata* conn, bool* done)
{
    ...
    rc = ldap_search_ext(li->ld, ludp->lud_dn, ludp->lud_scope, ludp->lud_filter, ludp->lud_attrs, 0, NULL, NULL, NULL, 0, &msgid);
...
static ssize_t ldap_recv(struct connectdata* conn, int sockindex, char* buf, size_t len, CURLcode* err)
{
    ...
    *err = CURLE_AGAIN;
    ...
    for (ent = ldap_first_message(li->ld, msg); ent; ent = ldap_next_message(li->ld, ent)) {
        ...
        if (msgtype == LDAP_RES_SEARCH_RESULT) {
            ...
            if (rc) {
                ...
                *err = CURLE_LDAP_SEARCH_FAILED;
                ...
                failf(data, "LDAP remote: search failed %s %s", ldap_err2string(rc), info ? info : "");
                *err = CURLE_LDAP_SEARCH_FAILED;
            } else {
                /* successful */
        ...
        for (rc = ldap_get_attribute_ber(li->ld, ent, ber, &bv, &bvals); rc == LDAP_SUCCESS; rc = ldap_get_attribute_ber(li->ld, ent, ber, &bv, &bvals)) {
            int i;
            ...
            for (i = 0; bvals[i].bv_val != NULL; i++) {
                ...
                if (!binary) {
                    ...
                    if (ISSPACE(bvals[i].bv_val[0]) || ISSPACE(bvals[i].bv_val[bvals[i].bv_len - 1]))
                        binval = 1;
                    else {
                ...
                if (binary || binval) {
                    ...
                    CURLcode error = Curl_base64_encode(data, bvals[i].bv_val, bvals[i].bv_len, &val_b64, &val_b64_sz);
                    if (error) {
                        ber_memfree(bvals);
                        ber_free(ber, 0);
                        ldap_msgfree(msg);
                    ...
                    writeerr = Curl_client_write(conn, CLIENTWRITE_BODY, bvals[i].bv_val, bvals[i].bv_len);
                    if (writeerr) {
                        *err = writeerr;
                        return -1;
                    }
