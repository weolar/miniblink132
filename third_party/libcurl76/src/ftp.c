...
static void ftp_pasv_verbose(struct connectdata* conn, Curl_addrinfo* ai, char* newhost, /* ascii version */
    int port);
#endif
...
static CURLcode InitiateTransfer(struct connectdata* conn)
{
    ...
    if (conn->proto.ftpc.state_saved == FTP_STOR) {
       ...
       size prior to the actual upload. */
        ...
        /* FTP download: */
        Curl_setup_transfer(conn, SECONDARYSOCKET, conn->proto.ftpc.retr_size_saved, FALSE, ftp->bytecountp, -1, NULL); /* no upload here */
    }
...
CURLcode Curl_GetFTPResponse(ssize_t* nreadp, /* return number of bytes read */
    struct connectdata* conn, int* ftpcode) /* return the ftp-code */
{
    /*
         ...
         to prepare the server for the upcoming PASV */
            if (!conn->proto.ftpc.file) {
                PPSENDF(&conn->proto.ftpc.pp, "PRET %s",
                    data->set.str[STRING_CUSTOMREQUEST] ? data->set.str[STRING_CUSTOMREQUEST] : (data->set.ftp_list_only ? "NLST" : "LIST"));
...
static CURLcode ftp_state_list(struct connectdata* conn)
{
  ...
  */
    ...
    cmd = aprintf("%s%s%s", data->set.str[STRING_CUSTOMREQUEST] ? data->set.str[STRING_CUSTOMREQUEST] : (data->set.ftp_list_only ? "NLST" : "LIST"),
    ...
    case 213: {
        ...
        if (data->set.opt_no_body && ftpc->file && data->set.get_filetime && (data->info.filetime >= 0)) {
            ...
            snprintf(headerbuf, sizeof(headerbuf), "Last-Modified: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n", Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6],
                tm->tm_mday, Curl_month[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min, tm->tm_sec);
    ...
    if (result || premature)
       ...
       use checking further */
        ...
        if ((-1 != data->state.infilesize) && (data->state.infilesize != *ftp->bytecountp) && !data->set.crlf && (ftp->transfer == FTPTRANSFER_BODY)) {
            failf(data, "Uploaded unaligned file size (%" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes)", *ftp->bytecountp,
        ...
        */
            ((data->req.size + data->state.crlf_conversions) != *ftp->bytecountp) &&
...
static CURLcode ftp_perform(struct connectdata* conn, bool* connected, /* connect status after PASV / PORT */
    bool* dophase_done)
{
    ...
    */
    ...
    case FTPFILE_SINGLECWD:
        ...
        if (slash_pos || !*cur_pos) {
            ...
            result = Curl_urldecode(conn->data, slash_pos ? cur_pos : "/", slash_pos ? dirlen : 1, &ftpc->dirs[0], NULL, TRUE);
            if (result) {
                freedirs(ftpc);
                return result;
            }
    ...
    if (ftpc->prevpath) {
       ...
       strings */
        size_t dlen;
        char* path;
        CURLcode result = Curl_urldecode(conn->data, data->state.path, 0, &path, &dlen, TRUE);
        if (result) {
            freedirs(ftpc);
            return result;
        }
...
static CURLcode ftp_regular_transfer(struct connectdata* conn, bool* dophase_done)
{
    ...
    result = ftp_perform(conn, &connected, /* have we connected after PASV/PORT */
