...
bool Curl_pipeline_penalized(struct Curl_easy* data, struct connectdata* conn)
{
    if (data) {
        ...
        curl_off_t penalty_size = Curl_multi_content_length_penalty_size(data->multi);
        curl_off_t chunk_penalty_size = Curl_multi_chunk_length_penalty_size(data->multi);
        ...
        infof(data, "Conn: %ld (%p) Receive pipe weight: (%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "), penalized: %s\n", conn->connection_id,
            (void*)conn, recv_size, conn->chunk.datasize, penalized ? "TRUE" : "FALSE");
        return penalized;
    }
    return FALSE;
}
...
{
    Curl_llist_insert_next(pipeline, pipeline->tail, data, &data->pipeline_queue);
    return CURLE_OK;
}
...
CURLcode Curl_add_handle_to_pipeline(struct Curl_easy* handle, struct connectdata* conn)
{
    ...
    if ((conn->bundle->multiuse == BUNDLE_PIPELINING) && (pipeline == &conn->send_pipe && sendhead != conn->send_pipe.head)) {
    ...
    if (handle->multi) {
        ...
        if (blacklist) {
            ...
            while (curr) {
                ...
                site = curr->ptr;
                if (strcasecompare(site->hostname, conn->host.name) && site->port == conn->remote_port) {
...
bool Curl_pipeline_server_blacklisted(struct Curl_easy* handle, char* server_name)
{
    if (handle->multi && server_name) {
        struct curl_llist* list = Curl_multi_pipelining_server_bl(handle->multi);

        struct curl_llist_element* e = list->head;
        while (e) {
            struct blacklist_node* bl = (struct blacklist_node*)e;
            if (strncasecompare(bl->server_name, server_name, strlen(bl->server_name))) {
