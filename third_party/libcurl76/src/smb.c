...
static void smb_format_message(struct connectdata* conn, struct smb_header* h, unsigned char cmd, size_t len)
{
    struct smb_conn* smbc = &conn->proto.smbc;
    struct smb_request* req = conn->data->req.protop;
    unsigned int pid;
...
static CURLcode smb_flush(struct connectdata* conn)
{
    ...
    result = Curl_write(conn, FIRSTSOCKET, conn->data->state.ulbuf + smbc->sent, len, &bytes_written);
    if (result)
        return result;
...
static CURLcode smb_send_message(struct connectdata* conn, unsigned char cmd, const void* msg, size_t msg_len)
{
...
static CURLcode smb_send_setup(struct connectdata* conn)
{
    ...
    return smb_send_message(conn, SMB_COM_SETUP_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_tree_connect(struct connectdata* conn)
{
    ...
    return smb_send_message(conn, SMB_COM_TREE_CONNECT_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_open(struct connectdata* conn)
{
    ...
    return smb_send_message(conn, SMB_COM_NT_CREATE_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_write(struct connectdata* conn)
{
    ...
    smb_format_message(conn, &msg->h, SMB_COM_WRITE_ANDX, sizeof(*msg) - sizeof(msg->h) + (size_t)upload_size);
...
static CURLcode smb_send_and_recv(struct connectdata* conn, void** msg)
{
    ...
    if (!smbc->send_size && smbc->upload_size) {
        size_t nread = smbc->upload_size > UPLOAD_BUFSIZE ? UPLOAD_BUFSIZE : smbc->upload_size;
    ...
    case SMB_DOWNLOAD:
        ...
        if (len > 0) {
            if (off + sizeof(unsigned int) + len > smbc->got) {
                ...
                result = Curl_client_write(conn, CLIENTWRITE_BODY, (char*)msg + off + sizeof(unsigned int), len);
            if (result) {
                req->result = result;
                next_state = SMB_CLOSE;
                break;
