...
static bool imap_endofresp(struct connectdata* conn, char* line, size_t len, int* resp)
{
    ...
    if (imap && !imap->custom && ((len == 3 && !memcmp("+", line, 1)) || (len >= 2 && !memcmp("+ ", line, 2)))) {
        switch (imapc->state) {
...
static CURLcode imap_perform_fetch(struct connectdata* conn)
{
    ...
    if (imap->partial)
        ...
        result = imap_sendf(conn, "FETCH %s BODY[%s]", imap->uid, imap->section ? imap->section : "");
...
static CURLcode imap_perform_append(struct connectdata* conn)
{
    ...
    result = imap_sendf(conn, "APPEND %s (\\Seen) {%" CURL_FORMAT_CURL_OFF_T "}", mailbox, data->state.infilesize);
    ...
    if (parsed) {
        ...
        if (pp->cache) {
         ...
         that there may even be additional "headers" after the body. */
            ...
            infof(data, "Written %zu bytes, %" CURL_FORMAT_CURL_OFF_TU " bytes are left for transfer\n", chunk, size - chunk);
...
static CURLcode imap_done(struct connectdata* conn, CURLcode status, bool premature)
{
    ...
    } else if (!data->set.connect_only && !imap->custom && (imap->uid || data->set.upload || data->set.mimepost.kind != MIMEKIND_NONE)) {
...
static CURLcode imap_perform(struct connectdata* conn, bool* connected, bool* dophase_done)
{
    ...
    *dophase_done = FALSE; /* not done yet */
    ...
    if (conn->data->set.upload || data->set.mimepost.kind != MIMEKIND_NONE)
        ...
        result = imap_perform_search(conn);
    else if (imap->mailbox && !selected && (imap->custom || imap->uid || imap->query))
        /* SELECT the mailbox */
...
static CURLcode imap_sendf(struct connectdata* conn, const char* fmt, ...)
{
    ...
    snprintf(imapc->resptag, sizeof(imapc->resptag), "%c%03d", 'A' + curlx_sltosi(conn->connection_id % 26), imapc->cmdid);
...
static CURLcode imap_parse_url_path(struct connectdata* conn)
{
    ...
    if (imap->mailbox && !imap->uid && *ptr == '?') {
        ...
        result = Curl_urldecode(data, begin, ptr - begin, &imap->query, NULL, TRUE);
        if (result)
            return result;
    }
