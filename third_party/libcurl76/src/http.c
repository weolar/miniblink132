...
CURLcode Curl_http_auth_act(struct connectdata* conn)
{
    ...
    if ((conn->bits.user_passwd || conn->oauth_bearer) && ((data->req.httpcode == 401) || (conn->bits.authneg && data->req.httpcode < 300))) {
...
static CURLcode output_auth_headers(struct connectdata* conn, struct auth* authstatus, const char* request, const char* path, bool proxy)
{
    ...
    if ((authstatus->picked == CURLAUTH_NEGOTIATE) && negdata->context && !GSS_ERROR(negdata->status)) {
        ...
        /* Basic */
        if ((proxy && conn->bits.proxy_user_passwd && !Curl_checkProxyheaders(conn, "Proxy-authorization"))
    ...
    if (authstatus->picked == CURLAUTH_BEARER) {
        /* Bearer */
        if ((!proxy && conn->oauth_bearer && !Curl_checkheaders(conn, "Authorization:"))) {
            auth = "Bearer";
    ...
    if (auth) {
        infof(data, "%s auth using %s with user '%s'\n", proxy ? "Proxy" : "Server", auth,
            proxy ? (conn->http_proxy.user ? conn->http_proxy.user : "") : (conn->user ? conn->user : ""));
 ...
 * @returns CURLcode
 */
CURLcode Curl_http_output_auth(struct connectdata* conn, const char* request, const char* path, bool proxytunnel) /* TRUE if this is the request setting
                                           up the proxy tunnel */
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    if ((conn->bits.httpproxy && conn->bits.proxy_user_passwd) || conn->bits.user_passwd || conn->oauth_bearer)
        /* continue please */;
    else {
        authhost->done = TRUE;
        authproxy->done = TRUE;
...
CURLcode Curl_http_input_auth(struct connectdata* conn, bool proxy, const char* auth) /* the first non-space */
{
    /*
...

{
    ...
    if ((conn->handler->flags & PROTOPT_SSL || conn->http_proxy.proxytype == CURLPROXY_HTTPS) && conn->httpversion != 20) {
...
static CURLcode add_haproxy_protocol_header(struct connectdata* conn)
{
    ...
    result = Curl_add_buffer_send(req_buffer, conn, &conn->data->info.request_size, 0, FIRSTSOCKET);
...
CURLcode Curl_http_done(struct connectdata* conn, CURLcode status, bool premature)
{
   ...
   * a chance to be set back to true when we output the next auth header */
    ...
    if (!premature && /* this check is pointless when DONE is called before the
        ...
        !conn->bits.retry && !data->set.connect_only && (http->readbytecount + data->req.headerbytecount - data->req.deductheadercount) <= 0) {
...
CURLcode Curl_add_timecondition(struct Curl_easy* data, Curl_send_buffer* req_buffer)
{
   ...
   */
    ...
    snprintf(datestr, sizeof(datestr), "%s: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n", condp, Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday,
...
CURLcode Curl_http(struct connectdata* conn, bool* done)
{
    ...
    *done = TRUE;
    ...
    if ((data->state.authhost.multipass || data->state.authproxy.multipass) && (httpreq != HTTPREQ_GET) && (httpreq != HTTPREQ_HEAD)) {
...
#define TE_HEADER "TE: gzip\r\n"
    ...
    if (ptr) {
        ...
        if ((conn->handler->protocol & PROTO_FAMILY_HTTP)
            && (((httpreq == HTTPREQ_POST_MIME || httpreq == HTTPREQ_POST_FORM) && http->postsize < 0) || (data->set.upload && data->state.infilesize == -1))) {
            if (conn->bits.authneg)
        ...
        if (conn->host.dispname != conn->host.name) {
           ...
           string. */
                ...
                if (newurl) {
                    ...
                    memcpy(newurl + newlen + (ptr - url), ptr + currlen, /* copy the trailing zero byte too */
    ...
    result = Curl_add_bufferf(req_buffer,
        ...
        ftp_typecode, httpstring, (conn->allocptr.host ? conn->allocptr.host : ""), conn->allocptr.proxyuserpwd ? conn->allocptr.proxyuserpwd : "",
        conn->allocptr.userpwd ? conn->allocptr.userpwd : "", (data->state.use_range && conn->allocptr.rangeline) ? conn->allocptr.rangeline : "",
        (data->set.str[STRING_USERAGENT] && *data->set.str[STRING_USERAGENT] && conn->allocptr.uagent) ? conn->allocptr.uagent : "",
        http->p_accept ? http->p_accept : "", conn->allocptr.te ? conn->allocptr.te : "",
        (data->set.str[STRING_ENCODING] && *data->set.str[STRING_ENCODING] && conn->allocptr.accept_encoding) ? conn->allocptr.accept_encoding : "",
        (data->change.referer && conn->allocptr.ref) ? conn->allocptr.ref : "" /* Referer: <data> */,
        (conn->bits.httpproxy && !conn->bits.tunnel_proxy && !Curl_checkProxyheaders(conn, "Proxy-Connection")) ? "Proxy-Connection: Keep-Alive\r\n" : "", te);
   ...
   * from re-used connections */
    ...
    if (data->cookies || addcookies) {
        ...
        if (data->cookies) {
            ...
            co = Curl_cookie_getlist(data->cookies, conn->allocptr.cookiehost ? conn->allocptr.cookiehost : host, data->state.path,
    ...
    case HTTPREQ_PUT: /* Let's PUT the data to the server! */
        ...
        if (result)
            ...
            Curl_setup_transfer(conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, postsize ? FIRSTSOCKET : -1, postsize ? &http->writebytecount : NULL);
        if (result)
            return result;
        break;
    ...
    case HTTPREQ_POST:
       ...
       here. */
        ...
        if (result)
            ...
            Curl_setup_transfer(
                conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, http->postdata ? FIRSTSOCKET : -1, http->postdata ? &http->writebytecount : NULL);
        break;
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
      ...
      */
            ...
            Curl_cookie_add(data, data->cookies, TRUE, FALSE, k->p + 11,
                ...
                conn->allocptr.cookiehost ? conn->allocptr.cookiehost : conn->host.name, data->state.path);
            Curl_share_unlock(data, CURL_LOCK_DATA_COOKIE);
        }
#endif
