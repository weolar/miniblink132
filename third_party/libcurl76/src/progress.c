...
static void time2str(char* r, curl_off_t seconds)
{
    ...
    if (h <= CURL_OFF_T_C(99)) {
        ...
        snprintf(r, 9, "%2" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T, h, m, s);
    } else {
       ...
       format to fit within the limits. */
        ...
        if (d <= CURL_OFF_T_C(999))
            snprintf(r, 9, "%3" CURL_FORMAT_CURL_OFF_T "d %02" CURL_FORMAT_CURL_OFF_T "h", d, h);
        else
...
#define ONE_PETABYTE (CURL_OFF_T_C(1024) * ONE_TERABYTE)
    ...
    if (bytes < CURL_OFF_T_C(100000))
        ...
        snprintf(max5, 6, "%2" CURL_FORMAT_CURL_OFF_T ".%0" CURL_FORMAT_CURL_OFF_T "M", bytes / ONE_MEGABYTE,
        ...
        snprintf(max5, 6, "%2" CURL_FORMAT_CURL_OFF_T ".%0" CURL_FORMAT_CURL_OFF_T "G", bytes / ONE_GIGABYTE,
...
int Curl_pgrsUpdate(struct connectdata* conn)
{
    ...
    if (data->progress.lastshow != now.tv_sec) {
       ...
       after two seconds we've filled in three entries etc. */
        countindex = ((data->progress.speeder_c >= CURR_TIME) ? CURR_TIME : data->progress.speeder_c) - 1;
        ...
        if (countindex) {
         ...
         entries, the first entry will remain the oldest. */
            checkindex = (data->progress.speeder_c >= CURR_TIME) ? data->progress.speeder_c % CURR_TIME : 0;
            ...
            {
                curl_off_t amount = data->progress.speeder[nowindex] - data->progress.speeder[checkindex];

                if (amount > CURL_OFF_T_C(4294967) /* 0xffffffff/1000 */)
                    /* the 'amount' value is bigger than would fit in 32 bits if
             multiplied with 1000, so we use the double math for this */
                    data->progress.current_speed = (curl_off_t)((double)amount / ((double)span_ms / 1000.0));
                else
    ...
    if (!(data->progress.flags & PGRS_HIDE)) {
        ...
        if (data->set.fxferinfo) {
            ...
            result = data->set.fxferinfo(
                data->set.progress_client, data->progress.size_dl, data->progress.downloaded, data->progress.size_ul, data->progress.uploaded);
        ...
        if (data->set.fprogress) {
            ...
            Curl_set_in_callback(data, true);
            result = data->set.fprogress(data->set.progress_client, (double)data->progress.size_dl, (double)data->progress.downloaded,
                (double)data->progress.size_ul, (double)data->progress.uploaded);
            Curl_set_in_callback(data, false);
            if (result)
                failf(data, "Callback aborted");
            return result;
       ...
       total estimate! */
        ...
        total_expected_transfer = (data->progress.flags & PGRS_UL_SIZE_KNOWN ? data->progress.size_ul : data->progress.uploaded)
            + (data->progress.flags & PGRS_DL_SIZE_KNOWN ? data->progress.size_dl : data->progress.downloaded);
