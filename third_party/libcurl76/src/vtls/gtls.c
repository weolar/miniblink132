...
static void showtime(struct Curl_easy* data, const char* text, time_t stamp)
{
    ...
    snprintf(str, sizeof(str), "\t %s: %s, %02d %s %4d %02d:%02d:%02d GMT", text, Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday,
   ...
   * GnuTLS version */
    ...
    if (SSL_SET_OPTION(cert)) {
        if (SSL_SET_OPTION(key_passwd)) {
            ...
            rc = gnutls_certificate_set_x509_key_file2(BACKEND->cred, SSL_SET_OPTION(cert), SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(cert),
            ...
            if (gnutls_certificate_set_x509_key_file(BACKEND->cred, SSL_SET_OPTION(cert), SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(cert),
        ...
        if (verify_status & GNUTLS_CERT_INVALID) {
            if (SSL_CONN_CONFIG(verifypeer)) {
                failf(data,
                    ...
                    "CRLfile: %s",
                    SSL_CONN_CONFIG(CAfile) ? SSL_CONN_CONFIG(CAfile) : "none", SSL_SET_OPTION(CRLfile) ? SSL_SET_OPTION(CRLfile) : "none");
