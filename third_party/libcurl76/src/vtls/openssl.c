...
static void tap_ssl_key(const SSL* ssl, ssl_tap_state_t* state)
{
   ...
   * we have a valid SSL context if we have a non-NULL session. */
    ...
    if (state->master_key_length == master_key_length && !memcmp(state->master_key, master_key, master_key_length)
...
static int select_next_proto_cb(SSL* ssl, unsigned char** out, unsigned char* outlen, const unsigned char* in, unsigned int inlen, void* arg)
{
    ...
    if (conn->data->set.httpversion >= CURL_HTTP_VERSION_2
        && !select_next_protocol(out, outlen, in, inlen, NGHTTP2_PROTO_VERSION_ID, NGHTTP2_PROTO_VERSION_ID_LEN)) {
...
static CURLcode ossl_connect_step1(struct connectdata* conn, int sockindex)
{
  ...
  */
...
#ifdef USE_NGHTTP2
        if (data->set.httpversion >= CURL_HTTP_VERSION_2 && (!SSL_IS_PROXY() || !conn->bits.tunnel_proxy)) {
    ...
    if (ssl_cert || ssl_cert_type) {
        if (!cert_stuff(conn, BACKEND->ctx, ssl_cert, ssl_cert_type, SSL_SET_OPTION(key), SSL_SET_OPTION(key_type), SSL_SET_OPTION(key_passwd))) {
...
static CURLcode ossl_connect_step2(struct connectdata* conn, int sockindex)
{
    ...
    if (1 != err) {
       ...
       */
        ...
        /* Informational message */
        infof(data, "SSL connection using %s / %s\n", get_ssl_version_txt(BACKEND->handle), SSL_get_cipher(BACKEND->handle));
...
static CURLcode ossl_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    if (SSL_SET_OPTION(primary.sessionid)) {
        ...
        if (!incache) {
            result = Curl_ssl_addsessionid(conn, our_ssl_sessionid, 0 /* unknown size */, sockindex);
            if (result) {
                Curl_ssl_sessionid_unlock(conn);
    ...
    if (rc <= 0) {
        ...
        case SSL_ERROR_SSL:
          ...
          The OpenSSL error queue contains more information on the error. */
            sslerror = ERR_get_error();
            if (ERR_GET_LIB(sslerror) == ERR_LIB_SSL && ERR_GET_REASON(sslerror) == SSL_R_BIO_NOT_SET && conn->ssl[sockindex].state == ssl_connection_complete
                && conn->proxy_ssl[sockindex].state == ssl_connection_complete) {
                char ver[120];
                Curl_ossl_version(ver, 120);
                failf(conn->data, "Error: %s does not support double SSL tunneling.", ver);
            } else
                failf(conn->data, "SSL_write() error: %s", ossl_strerror(sslerror, error_buffer, sizeof(error_buffer)));
            *curlcode = CURLE_SEND_ERROR;
            return -1;
        }
        /* a true error */
         ...
         value/errno" */
           ...
           queue */
                failf(conn->data, OSSL_PACKAGE " SSL_read: %s, errno %d",
                    (sslerror ? ossl_strerror(sslerror, error_buffer, sizeof(error_buffer)) : SSL_ERROR_to_str(err)), SOCKERRNO);
...
static CURLcode Curl_ossl_md5sum(unsigned char* tmp, /* input */
    size_t tmplen, unsigned char* md5sum /* output */, size_t unused)
{
    EVP_MD_CTX* mdctx;
    unsigned int len = 0;
    (void)unused;
