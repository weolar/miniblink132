...
static CURLcode add_certs_to_store(HCERTSTORE trust_store, const char* ca_file, struct connectdata* conn)
{
    CURLcode result;
    ...
    if (!ca_file_tstr) {
        failf(data, "schannel: invalid path name for CA file '%s': %s", ca_file, Curl_strerror(conn, GetLastError()));
   ...
   * small ( < 1 MiB ).
   */
    ca_file_handle = CreateFile(ca_file_tstr, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (ca_file_handle == INVALID_HANDLE_VALUE) {
        failf(data, "schannel: failed to open CA file '%s': %s", ca_file, Curl_strerror(conn, GetLastError()));
        result = CURLE_SSL_CACERT_BADFILE;
        goto cleanup;
    }

    if (!GetFileSizeEx(ca_file_handle, &file_size)) {
        failf(data, "schannel: failed to determine size of CA file '%s': %s", ca_file, Curl_strerror(conn, GetLastError()));
        result = CURLE_SSL_CACERT_BADFILE;
        goto cleanup;
    }
    ...
    while (total_bytes_read < ca_file_bufsize) {
        ...
        if (!ReadFile(ca_file_handle, ca_file_buffer + total_bytes_read, bytes_to_read, &bytes_read, NULL)) {
            ...
            failf(data, "schannel: failed to read from CA file '%s': %s", ca_file, Curl_strerror(conn, GetLastError()));
            result = CURLE_SSL_CACERT_BADFILE;
            goto cleanup;
        }
        if (bytes_read == 0) {
...
static CURLcode verify_host(struct Curl_easy* data, CERT_CONTEXT* pCertContextServer, const char* const conn_hostname)
{
   ...
   * any decoding */
    ...
    len = CertGetNameString(pCertContextServer, CERT_NAME_DNS_TYPE, name_flags, NULL, NULL, 0);
    if (len == 0) {
        failf(data,
   ...
   */
    ...
    actual_len = CertGetNameString(pCertContextServer, CERT_NAME_DNS_TYPE, name_flags, NULL, (LPTSTR)cert_hostname_buff, len);
...
CURLcode verify_certificate(struct connectdata* conn, int sockindex)
{
     ...
     */
        ...
        if (Curl_verify_windows_version(6, 1, PLATFORM_WINNT, VERSION_LESS_THAN)) {
            ...
            if (!trust_store) {
                failf(data, "schannel: failed to create certificate store: %s", Curl_strerror(conn, GetLastError()));
                result = CURLE_SSL_CACERT_BADFILE;
            } else {
       ...
       */
            ...
            if (!create_engine_result) {
                failf(data, "schannel: failed to create certificate chain engine: %s", Curl_strerror(conn, GetLastError()));
                result = CURLE_SSL_CACERT_BADFILE;
            }
        }
    }
    ...
    if (result == CURLE_OK) {
        ...
        if (!CertGetCertificateChain(cert_chain_engine, pCertContextServer, NULL, pCertContextServer->hCertStore, &ChainPara,
                (data->set.ssl.no_revoke ? 0 : CERT_CHAIN_REVOCATION_CHECK_CHAIN), NULL, &pChainContext)) {
