...
#if CURL_BUILD_MAC_10_8 || CURL_BUILD_IOS
static CURLcode darwinssl_version_from_curl(SSLProtocol* darwinver, long ssl_version)
{
    switch (ssl_version) {
...
static CURLcode darwinssl_connect_step1(struct connectdata* conn, int sockindex)
{
  ...
  */
   ...
   * Also: the verifyhost setting influences SNI usage */
    ...
    if (all_ciphers && allowed_ciphers && SSLGetSupportedCiphers(BACKEND->ssl_ctx, all_ciphers, &all_ciphers_count) == noErr) {
    ...
    if (err != noErr) {
        ...
        case -9841:
            if (SSL_CONN_CONFIG(CAfile) && SSL_CONN_CONFIG(verifypeer)) {
                int res = verify_cert(SSL_CONN_CONFIG(CAfile), data, BACKEND->ssl_ctx);
                if (res != CURLE_OK)
                    return res;
            }
        ...
        if (data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG]) {
            CURLcode result = pkp_pin_peer_pubkey(data, BACKEND->ssl_ctx, data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG]);
            if (result) {
...
static CURLcode Curl_darwinssl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_darwinssl_data_pending(const struct connectdata* conn, int connindex)
{
...
static CURLcode Curl_darwinssl_random(struct Curl_easy* data UNUSED_PARAM, unsigned char* entropy, size_t length)
{
...
static CURLcode Curl_darwinssl_md5sum(unsigned char* tmp, /* input */
    size_t tmplen, unsigned char* md5sum, /* output */
    size_t md5len)
{
    (void)md5len;
...
static void* Curl_darwinssl_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
