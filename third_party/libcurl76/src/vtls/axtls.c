...
static CURLcode connect_prep(struct connectdata* conn, int sockindex)
{
  ...
  */
    ...
    default:
        failf(data,
            "axTLS only supports TLS 1.0 and 1.1, "
 ...
 */
static CURLcode Curl_axtls_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    CURLcode conn_step;
    int ssl_fcn_return;
...
static CURLcode Curl_axtls_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
...
static void* Curl_axtls_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
    return BACKEND->ssl;
}
