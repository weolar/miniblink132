...
static SECStatus SelectClientCert(
    void* arg, PRFileDesc* sock, struct CERTDistNamesStr* caNames, struct CERTCertificateStr** pRetCert, struct SECKEYPrivateKeyStr** pRetKey)
{
...
static CURLcode nss_init_core(struct Curl_easy* data, const char* cert_dir)
{
    ...
    nss_context = NSS_InitContext("", "", "", "", &initparams,
        NSS_INIT_READONLY | NSS_INIT_NOCERTDB | NSS_INIT_NOMODDB | NSS_INIT_FORCEOPEN | NSS_INIT_NOROOTINIT | NSS_INIT_OPTIMIZESPACE | NSS_INIT_PK11RELOAD);
    if (nss_context != NULL)
        return CURLE_OK;
...
static CURLcode nss_setup_connect(struct connectdata* conn, int sockindex)
{
   ...
   * verify peer */
    ...
    if (data->set.ssl.falsestart) {
        ...
        if (SSL_SetCanFalseStartCallback(BACKEND->handle, CanFalseStartCallback, conn) != SECSuccess)
            goto error;
    }
#endif
    ...
    if (SSL_SetURL(BACKEND->handle, SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name) != SECSuccess)
        goto error;

    /* prevent NSS from re-using the session for a different hostname */
    if (SSL_SetSockPeerID(BACKEND->handle, SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name) != SECSuccess)
        goto error;
