...
#if (LIBCYASSL_VERSION_HEX < 0x03006006) || defined(HAVE_WOLFSSLV3_CLIENT_METHOD)
...
#if defined(HAVE_CYASSL_CTX_USESUPPORTEDCURVE) || defined(HAVE_WOLFSSL_CTX_USESUPPORTEDCURVE)
...
#if defined(HAVE_CYASSL_GET_PEER_CERTIFICATE) || defined(HAVE_WOLFSSL_GET_PEER_CERTIFICATE) || (defined(OPENSSL_EXTRA) && !defined(NO_CERTS))
...
#define use_sni(x) Curl_nop_stmt
...
#else
        infof(data,
            "CyaSSL <3.3.0 cannot be configured to use TLS 1.0-1.2, "
   ...
   * SSL_get_verify_result() below. */
...
#endif
            (CyaSSL_CTX_UseSNI(BACKEND->ctx, CYASSL_SNI_HOST_NAME, hostname, (unsigned short)hostname_len) != 1)) {
            infof(data,
    ...
    if (data->set.ssl.fsslctx) {
        ...
        failf(data,
            "SSL: Certificates couldn't be loaded because CyaSSL was built"
...
#ifdef USE_NGHTTP2
            else if (data->set.httpversion >= CURL_HTTP_VERSION_2 && protocol_len == NGHTTP2_PROTO_VERSION_ID_LEN
...
static bool Curl_cyassl_data_pending(const struct connectdata* conn, int connindex)
{
...
static CURLcode Curl_cyassl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static CURLcode Curl_cyassl_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
    RNG rng;
    (void)data;
    if (InitRng(&rng))
...
static void* Curl_cyassl_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
