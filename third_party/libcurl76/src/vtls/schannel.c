...
static CURLcode pkp_pin_peer_pubkey(struct connectdata* conn, int sockindex, const char* pinnedpubkey);
...
#endif
#if !defined(__W32API_MAJOR_VERSION) || !defined(__W32API_MINOR_VERSION) || defined(__MINGW64_VERSION_MAJOR) || (__W32API_MAJOR_VERSION > 5)                   \
...
static CURLcode schannel_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    if (Curl_verify_windows_version(5, 1, PLATFORM_WINNT, VERSION_LESS_THAN_EQUAL)) {
        ...
        infof(data,
            "schannel: WinSSL version is old and may not be able to "
    ...
    BACKEND->use_alpn = conn->bits.tls_enable_alpn && !GetProcAddress(GetModuleHandleA("ntdll"), "wine_get_version")
...
#error "compiler too old to support requisite manual cert verify for Win CE"
  ...
  */
    sspi_status = s_pSecFn->InitializeSecurityContext(&BACKEND->cred->cred_handle, NULL, host_name, BACKEND->req_flags, 0, 0,
    ...
    if (sspi_status != SEC_I_CONTINUE_NEEDED) {
        if (sspi_status == SEC_E_WRONG_PRINCIPAL)
            ...
            failf(data, "schannel: initial InitializeSecurityContext failed: %s", Curl_sspi_strerror(conn, sspi_status));
...
static CURLcode schannel_connect_step2(struct connectdata* conn, int sockindex)
{
    ...
    for (;;) {
        if (doread) {
            ...
            result = Curl_read_plain(
                conn->sock[sockindex], (char*)(BACKEND->encdata_buffer + BACKEND->encdata_offset), BACKEND->encdata_length - BACKEND->encdata_offset, &nread);
        ...
        /* https://msdn.microsoft.com/en-us/library/windows/desktop/aa375924.aspx
       */
        sspi_status = s_pSecFn->InitializeSecurityContext(&BACKEND->cred->cred_handle, &BACKEND->ctxt->ctxt_handle, host_name, BACKEND->req_flags, 0, 0,
            &inbuf_desc, 0, NULL, &outbuf_desc, &BACKEND->ret_flags, &BACKEND->ctxt->time_stamp);
       ...
       request a client certificate but do not require it. */
        ...
        if (sspi_status == SEC_I_CONTINUE_NEEDED || sspi_status == SEC_E_OK) {
            ...
            return sspi_status == SEC_E_UNTRUSTED_ROOT ? CURLE_SSL_CACERT : CURLE_SSL_CONNECT_ERROR;
        }
    ...
    pubkey_ptr = SSL_IS_PROXY() ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY] : data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG];
    if (pubkey_ptr) {
...
static CURLcode schannel_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    if (BACKEND->use_alpn) {
        ...
        if (alpn_result.ProtoNegoStatus == SecApplicationProtocolNegotiationStatus_Success) {
            ...
            if (alpn_result.ProtocolIdSize == NGHTTP2_PROTO_VERSION_ID_LEN
                && !memcmp(NGHTTP2_PROTO_VERSION_ID, alpn_result.ProtocolId, NGHTTP2_PROTO_VERSION_ID_LEN)) {
    ...
    if (SSL_SET_OPTION(primary.sessionid)) {
        ...
        if (!incache) {
            result = Curl_ssl_addsessionid(conn, (void*)BACKEND->cred, sizeof(struct curl_schannel_cred), sockindex);
            if (result) {
                Curl_ssl_sessionid_unlock(conn);
...
static ssize_t schannel_send(struct connectdata* conn, int sockindex, const void* buf, size_t len, CURLcode* err)
{
    ...
    InitSecBuffer(&outbuf[1], SECBUFFER_DATA, data + BACKEND->stream_sizes.cbHeader, curlx_uztoul(len));
    InitSecBuffer(&outbuf[2], SECBUFFER_STREAM_TRAILER, data + BACKEND->stream_sizes.cbHeader + len, BACKEND->stream_sizes.cbTrailer);
...
static CURLcode Curl_schannel_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static int Curl_schannel_shutdown(struct connectdata* conn, int sockindex)
{
   ...
   */
    ...
    if (BACKEND->cred && BACKEND->ctxt) {
        ...
        sspi_status = s_pSecFn->InitializeSecurityContext(&BACKEND->cred->cred_handle, &BACKEND->ctxt->ctxt_handle, host_name, BACKEND->req_flags, 0, 0, NULL,
            0, &BACKEND->ctxt->ctxt_handle, &outbuf_desc, &BACKEND->ret_flags, &BACKEND->ctxt->time_stamp);
...
static CURLcode Curl_schannel_random(struct Curl_easy* data UNUSED_PARAM, unsigned char* entropy, size_t length)
{
...
static CURLcode pkp_pin_peer_pubkey(struct connectdata* conn, int sockindex, const char* pinnedpubkey)
{
    ...
    do {
        status = s_pSecFn->QueryContextAttributes(&BACKEND->ctxt->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &pCertContextServer);
...
static CURLcode Curl_schannel_md5sum(unsigned char* input, size_t inputlen, unsigned char* md5sum, size_t md5len)
{
    Curl_schannel_checksum(input, inputlen, md5sum, md5len, PROV_RSA_FULL, CALG_MD5);
    return CURLE_OK;
}

static CURLcode Curl_schannel_sha256sum(const unsigned char* input, size_t inputlen, unsigned char* sha256sum, size_t sha256len)
{
    Curl_schannel_checksum(input, inputlen, sha256sum, sha256len, PROV_RSA_AES, CALG_SHA_256);
    return CURLE_OK;
}

static void* Curl_schannel_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
