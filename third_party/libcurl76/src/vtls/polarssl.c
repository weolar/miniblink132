...
static CURLcode polarssl_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    if ((ret = ctr_drbg_init(&BACKEND->ctr_drbg, entropy_func_mutex, &entropy, NULL, 0)) != 0) {
    ...
    if ((ret = ctr_drbg_init(&BACKEND->ctr_drbg, entropy_func, &BACKEND->entropy, NULL, 0)) != 0) {
        error_strerror(ret, errorbuf, sizeof(errorbuf));
        failf(data, "Failed - PolarSSL: ctr_drbg_init returned (-0x%04X) %s\n", -ret, errorbuf);
    }
#endif /* THREADING_SUPPORT */
    ...
    if (SSL_CONN_CONFIG(CAfile)) {
        ret = x509_crt_parse_file(&BACKEND->cacert, SSL_CONN_CONFIG(CAfile));
    ...
    if (SSL_SET_OPTION(cert)) {
        ret = x509_crt_parse_file(&BACKEND->clicert, SSL_SET_OPTION(cert));
    ...
    if (SSL_SET_OPTION(CRLfile)) {
        ret = x509_crl_parse_file(&BACKEND->crl, SSL_SET_OPTION(CRLfile));
    ...
    case CURL_SSLVERSION_DEFAULT:
    case CURL_SSLVERSION_TLSv1:
        ssl_set_min_version(&BACKEND->ssl, SSL_MAJOR_VERSION_3, SSL_MINOR_VERSION_1);
        break;
    case CURL_SSLVERSION_SSLv3:
        ssl_set_min_version(&BACKEND->ssl, SSL_MAJOR_VERSION_3, SSL_MINOR_VERSION_0);
        ssl_set_max_version(&BACKEND->ssl, SSL_MAJOR_VERSION_3, SSL_MINOR_VERSION_0);
    ...
    ssl_set_rng(&BACKEND->ssl, ctr_drbg_random, &BACKEND->ctr_drbg);
    ssl_set_bio(&BACKEND->ssl, net_recv, &conn->sock[sockindex], net_send, &conn->sock[sockindex]);
    ...
    ssl_set_ca_chain(&BACKEND->ssl, &BACKEND->cacert, &BACKEND->crl, hostname);

    ssl_set_own_cert_rsa(&BACKEND->ssl, &BACKEND->clicert, &BACKEND->rsa);
...
static CURLcode polarssl_connect_step2(struct connectdata* conn, int sockindex)
{
    ...
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    char buffer[1024];
    const char* const pinnedpubkey = SSL_IS_PROXY() ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY] : data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG];
    ...
    if (ssl_get_peer_cert(&(BACKEND->ssl))) {
        ...
        if (x509_crt_info(buffer, sizeof(buffer), (char*)"* ", ssl_get_peer_cert(&(BACKEND->ssl))) != -1)
...
static ssize_t polarssl_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    ...
    ret = ssl_write(&BACKEND->ssl, (unsigned char*)mem, len);
...
static CURLcode Curl_polarssl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_polarssl_data_pending(const struct connectdata* conn, int sockindex)
{
    const struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    return ssl_get_bytes_avail(&BACKEND->ssl) != 0;
}

static CURLcode Curl_polarssl_sha256sum(const unsigned char* input, size_t inputlen, unsigned char* sha256sum, size_t sha256len UNUSED_PARAM)
{
    (void)sha256len;
...
static void* Curl_polarssl_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
    return &BACKEND->ssl;
}
...
const struct Curl_ssl Curl_ssl_polarssl = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_PINNEDPUBKEY,
