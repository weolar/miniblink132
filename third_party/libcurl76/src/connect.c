...
static CURLcode singleipconnect(struct connectdata* conn, const Curl_addrinfo* ai, /* start connecting to this */
    curl_socket_t* sock);
...
void Curl_updateconninfo(struct connectdata* conn, curl_socket_t sockfd)
{
    ...
    if (!conn->bits.reuse && !conn->bits.tcp_fastopen) {
        ...
        if (!Curl_getaddressinfo((struct sockaddr*)&ssrem, conn->primary_ip, &conn->primary_port)) {
            failf(data, "ssrem inet_ntop() failed with errno %d: %s", errno, Curl_strerror(conn, errno));
            return;
        }
        memcpy(conn->ip_addr_str, conn->primary_ip, MAX_IPADR_LEN);

        if (!Curl_getaddressinfo((struct sockaddr*)&ssloc, conn->local_ip, &conn->local_port)) {
            failf(data, "ssloc inet_ntop() failed with errno %d: %s", errno, Curl_strerror(conn, errno));
            return;
        }
    }
    ...
    for (i = 0; i < 2; i++) {
       ...
       reliably. This is reported necessary on the MPE/iX operating system. */
        ...
        if (rc == 0) { /* no connection yet */
            ...
            if (i == 0 && conn->tempaddr[1] == NULL && (Curl_timediff(now, conn->connecttime) >= data->set.happy_eyeballs_timeout)) {
 ...
 * having connected.
 */
static CURLcode singleipconnect(struct connectdata* conn, const Curl_addrinfo* ai, curl_socket_t* sockp)
{
    ...
    *sockp = CURL_SOCKET_BAD;
    ...
    if (!Curl_getaddressinfo((struct sockaddr*)&addr.sa_addr, ipaddress, &port)) {
        /* malformed address or bug in inet_ntop, try next address */
        failf(data, "sa_addr inet_ntop() failed with errno %d: %s", errno, Curl_strerror(conn, errno));
...
curl_socket_t Curl_getconnectinfo(struct Curl_easy* data, struct connectdata** connp)
{
   ...
   */
    if (data->state.lastconnect && (data->multi_easy || data->multi)) {
        ...
        Curl_conncache_foreach(data, data->multi_easy ? &data->multi_easy->conn_cache : &data->multi->conn_cache, &find, conn_is_conn);
