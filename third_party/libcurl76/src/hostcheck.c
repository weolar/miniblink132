...
#if defined(USE_OPENSSL) || defined(USE_AXTLS) || defined(USE_GSKIT) || defined(USE_SCHANNEL)
...
static int hostmatch(char* hostname, char* pattern)
{
    ...
    if (pattern_label_end == NULL || strchr(pattern_label_end + 1, '.') == NULL || pattern_wildcard > pattern_label_end
    ...
    return strncasecompare(pattern, hostname, prefixlen) && strncasecompare(pattern_wildcard + 1, hostname_label_end - suffixlen, suffixlen)
        ? CURL_HOST_MATCH
        : CURL_HOST_NOMATCH;
}
