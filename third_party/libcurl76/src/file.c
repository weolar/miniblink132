...
void curl_set_file_system(void*(__cdecl* lpOpenFile)(const char* _Filename), int(__cdecl* lpCloseFile)(void* userFileHandle),
    size_t(__cdecl* lpFileSize)(void* userFileHandle), int(__cdecl* lpReadFile)(void* userFileHandle, void* buf, size_t count),
    int(__cdecl* lpSeekFile)(void* userFileHandle, size_t _Offset, int origin), bool(__cdecl* lpExistsFile)(const char* _Filename))
{
...
#ifdef _MSC_VER
WCHAR* utf8_to_wide_char(const char* inStr)
{
    int cbMultiByte = 0;
    DWORD dwMinSize = 0;
    WCHAR* outStr = 0;
    ...
    outStr = (WCHAR*)malloc((dwMinSize + 1) * 2);
...
static CURLcode file_do(struct connectdata* conn, bool* done)
{
    ...
    *done = TRUE; /* unconditionally */
    ...
    if (data->set.opt_no_body && data->set.include_header && fstated) {
        ...
        char header[80];
        snprintf(header, sizeof(header), "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", expected_size);
        ...
        snprintf(header, sizeof(header), "Last-Modified: %s, %02d %s %4d %02d:%02d:%02d GMT\r\n", Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday,
