    ...
    struct Curl_easy* data, const char* user, const char* password, const char* service, const char* host, const char* chlg64, struct negotiatedata* nego)
{
    ...
    if (GSS_ERROR(nego->status)) {
        failf(data, "InitializeSecurityContext failed: %s", Curl_sspi_strerror(data->easy_conn, nego->status));
        return CURLE_OUT_OF_MEMORY;
    }
...
CURLcode Curl_auth_create_spnego_message(struct Curl_easy* data, struct negotiatedata* nego, char** outptr, size_t* outlen)
{
    ...
    result = Curl_base64_encode(data, (const char*)nego->output_token, nego->output_token_length, outptr, outlen);
