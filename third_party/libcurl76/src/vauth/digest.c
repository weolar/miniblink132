...
static CURLcode auth_decode_digest_md5_message(
    const char* chlg64, char* nonce, size_t nlen, char* realm, size_t rlen, char* alg, size_t alen, char* qop, size_t qlen)
{
...
CURLcode Curl_auth_create_digest_md5_message(
    struct Curl_easy* data, const char* chlg64, const char* userp, const char* passwdp, const char* service, char** outptr, size_t* outlen)
{
    CURLcode result = CURLE_OK;
    size_t i;
    MD5_context* ctxt;
    ...
    result = auth_decode_digest_md5_message(chlg64, nonce, sizeof(nonce), realm, sizeof(realm), algorithm, sizeof(algorithm), qop_options, sizeof(qop_options));
    if (result)
        return result;
 ...
 * Returns CURLE_OK on success.
 */
static CURLcode _Curl_auth_create_digest_http_message(struct Curl_easy* data, const char* userp, const char* passwdp, const unsigned char* request,
    ...
    void (*hash)(unsigned char*, const unsigned char*))
{
  ...
  */
    ...
    hashthis = (unsigned char*)aprintf("%s:%s:%s", digest->userhash ? userh : userp, digest->realm, passwdp);
    if (!hashthis)
        return CURLE_OUT_OF_MEMORY;
    ...
    if (digest->algo == CURLDIGESTALGO_MD5SESS || digest->algo == CURLDIGESTALGO_SHA256SESS || digest->algo == CURLDIGESTALGO_SHA512_256SESS) {
  ...
  */
    ...
    if (digest->qop) {
        hashthis = (unsigned char*)aprintf("%s:%s:%08x:%s:%s:%s", ha1, digest->nonce, digest->nc, digest->cnonce, digest->qop, ha2);
    } else {
        hashthis = (unsigned char*)aprintf("%s:%s:%s", ha1, digest->nonce, ha2);
    }
  ...
  */
    ...
    if (digest->qop) {
        response = aprintf("username=\"%s\", "
                           ...
                           "qop=%s, "
                           "response=\"%s\"",
            userp_quoted, digest->realm, digest->nonce, uripath, digest->cnonce, digest->nc, digest->qop, request_digest);
    ...
    const unsigned char* uripath, struct digestdata* digest, char** outptr, size_t* outlen)
{
    ...
    case CURLDIGESTALGO_MD5SESS:
        return _Curl_auth_create_digest_http_message(data, userp, passwdp, request, uripath, digest, outptr, outlen, auth_digest_md5_to_ascii, Curl_md5it);
