...
bool domainIsContain(char* oldDomain, char* newDomain)
{
    ...
    if (oldDomainLength < newDomainLength && NULL != strstr(newDomain, oldDomain) || oldDomainLength >= newDomainLength && NULL != strstr(oldDomain, newDomain))
        return TRUE;
...
struct Cookie* Curl_cookie_add(struct Curl_easy* data,
    ...
    struct CookieInfo* c, bool httpheader, /* TRUE if HTTP header-style line */
