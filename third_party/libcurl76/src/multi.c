...
CURLMcode curl_multi_add_handle(struct Curl_multi* multi, struct Curl_easy* data)
{
   ...
   */
    ...
    else if (!data->dns.hostcache || (data->dns.hostcachetype == HCACHE_NONE)) {
...
static CURLcode multi_done(struct connectdata** connp, CURLcode status, /* an error if this is called
  ...
  */
    ...
    if ((data->set.reuse_forbid
#if defined(USE_NTLM)
            && !(conn->ntlm.state == NTLMSTATE_TYPE2 || conn->proxyntlm.state == NTLMSTATE_TYPE2)
#endif
                )
       ...
       if we got a new error, return that. */
        ...
        snprintf(buffer, sizeof(buffer), "Connection #%ld to host %s left intact", conn->connection_id,
...
static int multi_getsock(struct Curl_easy* data, curl_socket_t* socks, /* points to numsocks number
                                                  of sockets */
    int numsocks)
{
        ...
        if (data->easy_conn && (data->mstate >= CURLM_STATE_CONNECT) && (data->mstate < CURLM_STATE_COMPLETED)) {
         ...
         stored, but we must not check already completed handles */
            ...
            if (timeout_ms < 0) {
                /* Handle timed out */
                if (data->mstate == CURLM_STATE_WAITRESOLVE)
                    failf(data, "Resolving timed out after %ld milliseconds", Curl_timediff(now, data->progress.t_startsingle));
                else if (data->mstate == CURLM_STATE_WAITCONNECT)
                    failf(data, "Connection timed out after %ld milliseconds", Curl_timediff(now, data->progress.t_startsingle));
                else {
                    k = &data->req;
                    if (k->size != -1) {
                        failf(data,
                            "Operation timed out after %ld milliseconds with %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes received",
        ...
        case CURLM_STATE_DO_DONE:
         ...
         Having both BAD is a signal to skip immediately to DONE */
            if ((data->easy_conn->sockfd != CURL_SOCKET_BAD) || (data->easy_conn->writesockfd != CURL_SOCKET_BAD))
                ...
                if (data->state.wildcardmatch && ((data->easy_conn->handler->flags & PROTOPT_WILDCARD) == 0)) {
            ...
            if (!result) {
                ...
                if (data->set.max_send_speed > 0)
                    send_timeout_ms = Curl_pgrsLimitWaitTime(
                        data->progress.uploaded, data->progress.ul_limit_size, data->set.max_send_speed, data->progress.ul_limit_start, now);

                recv_timeout_ms = 0;
                if (data->set.max_recv_speed > 0)
                    recv_timeout_ms = Curl_pgrsLimitWaitTime(
                        data->progress.downloaded, data->progress.dl_limit_size, data->set.max_recv_speed, data->progress.dl_limit_start, now);
