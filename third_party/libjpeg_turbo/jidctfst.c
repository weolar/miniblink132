...
#if DCTSIZE != 8
Sorry,
    this code only copes with 8x8 DCTs./* deliberate syntax err */
#endif
...
#endif
#define IRIGHT_SHIFT(x, shft) ((ishift_temp = (x)) < 0 ? (ishift_temp >> (shft)) | ((~((DCTELEM)0)) << (DCTELEMBITS - (shft))) : (ishift_temp >> (shft)))
#else
#define ISHIFT_TEMPS
 ...
 */
    ...
    GLOBAL(void) jpeg_idct_ifast(j_decompress_ptr cinfo, jpeg_component_info* compptr, JCOEFPTR coef_block, JSAMPARRAY output_buf, JDIMENSION output_col)
{
