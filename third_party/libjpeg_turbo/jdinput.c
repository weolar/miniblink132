...
typedef struct {
    ...
    jpg_boolean inheaders; /* TRUE until first SOS is reached */
} my_input_controller;
   ...
   */
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        compptr->width_in_blocks
            = (JDIMENSION)jdiv_round_up((long)cinfo->image_width * (long)compptr->h_samp_factor, (long)(cinfo->max_h_samp_factor * DCTSIZE));
        compptr->height_in_blocks
            = (JDIMENSION)jdiv_round_up((long)cinfo->image_height * (long)compptr->v_samp_factor, (long)(cinfo->max_v_samp_factor * DCTSIZE));
