 ...
 */
    ...
    jpg_boolean is_os2; /* saves the OS2 format request flag */
    ...
    jpg_boolean use_inversion_array; /* TRUE = buffer the whole image, which is
...
LOCAL(void) write_colormap(j_decompress_ptr cinfo, bmp_dest_ptr dest, int map_colors, int map_entry_size);
...
/* This version is for writing 24-bit pixels */
{
   ...
   */
    ...
    if (cinfo->out_color_space == JCS_EXT_BGR) {
        ...
        jpg_boolean big_endian = is_big_endian();
...
#define PUT_4B(array, offset, value)                                                                                                                           \
    (array[offset] = (char)((value)&0xFF), array[offset + 1] = (char)(((value) >> 8) & 0xFF), array[offset + 2] = (char)(((value) >> 16) & 0xFF),              \
...
jinit_write_bmp(j_decompress_ptr cinfo, jpg_boolean is_os2, jpg_boolean use_inversion_array)
{
    bmp_dest_ptr dest;
    ...
    } else if (!cinfo->quantize_colors && (cinfo->out_color_space == JCS_RGB565 || cinfo->out_color_space == JCS_CMYK)) {
        dest->pub.put_pixel_rows = put_pixel_rows;
    } else {
    ...
    if (cinfo->out_color_space == JCS_RGB565) {
        ...
        while ((row_width & 3) != 0)
            row_width++;
    } else if (!cinfo->quantize_colors && (IsExtRGB(cinfo->out_color_space) || cinfo->out_color_space == JCS_CMYK)) {
    ...
    if (use_inversion_array) {
        ...
        dest->whole_image = (*cinfo->mem->request_virt_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, FALSE, dest->row_width, cinfo->output_height, (JDIMENSION)1);
        ...
        dest->iobuffer = (JSAMPLE*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, dest->row_width);
    }
