...
LOCAL(int)
parse_switches(j_compress_ptr cinfo, int argc, char** argv, int last_file_arg_seen, jpg_boolean for_real)
 ...
 */
{
    int argn;
    char* arg;
    jpg_boolean simple_progressive;
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        if (keymatch(arg, "arithmetic", 1)) {
            ...
            if (++argn >= argc) /* advance to next argument */
                usage();
            if (transformoption.crop /* reject multiple crop/drop/wipe requests */ || !jtransform_parse_crop_spec(&transformoption, argv[argn])) {
            ...
            if (++argn >= argc) /* advance to next argument */
                usage();
            if (transformoption.crop /* reject multiple crop/drop/wipe requests */ || !jtransform_parse_crop_spec(&transformoption, argv[argn])
                || transformoption.crop_width_set != JCROP_UNSET || transformoption.crop_height_set != JCROP_UNSET) {
...
#endif
{
   ...
   */
    ...
    if (icc_filename != NULL) {
        ...
        if (fseek(icc_file, 0, SEEK_END) < 0 || (icc_len = ftell(icc_file)) < 1 || fseek(icc_file, 0, SEEK_SET) < 0) {
...
#if TRANSFORMS_SUPPORTED
    if (dropfilename != NULL)
        return (jsrcerr.num_warnings + jdroperr.num_warnings + jdsterr.num_warnings ? EXIT_WARNING : EXIT_SUCCESS);
#endif
