   ...
   converting to extended RGB */
{
    ...
    if (maxval == MAXJSAMPLE) {
        if (aindex >= 0)
            GRAY_RGB_READ_LOOP((JSAMPLE)read_pbm_integer(cinfo, infile, maxval), ptr[aindex] = 0xFF;)
        else
...
/* This version is for reading text-format PPM files with any maxval */
{
    ...
    if (maxval == MAXJSAMPLE) {
        if (aindex >= 0)
            RGB_READ_LOOP((JSAMPLE)read_pbm_integer(cinfo, infile, maxval), ptr[aindex] = 0xFF;)
        else
...
start_input_ppm(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
    ...
    jpg_boolean need_iobuffer, use_raw_buffer, need_rescale;
    ...
    switch (c) {
    case '2': /* it's a text-format PGM file */
        if (cinfo->in_color_space == JCS_UNKNOWN || cinfo->in_color_space == JCS_RGB)
    ...
    if (need_iobuffer) {
        if (c == '6')
            ...
            source->buffer_width = (size_t)w * ((maxval <= 255) ? sizeof(U_CHAR) : (2 * sizeof(U_CHAR)));
    ...
    if (need_rescale) {
        ...
        source->rescale = (JSAMPLE*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, (size_t)(((long)MAX(maxval, 255) + 1L) * sizeof(JSAMPLE)));
        memset(source->rescale, 0, (size_t)(((long)MAX(maxval, 255) + 1L) * sizeof(JSAMPLE)));
