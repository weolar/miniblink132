...
#define GET_2B(offset) ((unsigned int)UCH(targaheader[offset]) + (((unsigned int)UCH(targaheader[offset + 1])) << 8))
    ...
    if (cmaptype > 1 || /* cmaptype must be 0 or 1 */
        source->pixel_size < 1 || source->pixel_size > 4 || (UCH(targaheader[16]) & 7) != 0 || /* bits/pixel must be multiple of 8 */
    ...
    if (is_bottom_up) {
        ...
        source->whole_image
            = (*cinfo->mem->request_virt_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, FALSE, (JDIMENSION)width * components, (JDIMENSION)height, (JDIMENSION)1);
