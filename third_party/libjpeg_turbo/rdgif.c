...
#define LM_to_uint(array, offset) ((unsigned int)UCH(array[offset]) + (((unsigned int)UCH(array[offset + 1])) << 8))
...
typedef struct {
    ...
    jpg_boolean first_time; /* flags first call to GetCode */
    jpg_boolean out_of_blocks; /* TRUE if hit terminator data block */
    ...
    jpg_boolean is_interlaced; /* TRUE if have interlaced image */
...
METHODDEF(JDIMENSION) get_pixel_rows(j_compress_ptr cinfo, cjpeg_source_ptr sinfo);
METHODDEF(JDIMENSION) load_interlaced_image(j_compress_ptr cinfo, cjpeg_source_ptr sinfo);
METHODDEF(JDIMENSION) get_interlaced_row(j_compress_ptr cinfo, cjpeg_source_ptr sinfo);
...
#define UPSCALE(x) ((x) << (BITS_IN_JSAMPLE - 8))
        ...
        if (cmap[CM_RED][i] != cmap[CM_GREEN][i] || cmap[CM_GREEN][i] != cmap[CM_BLUE][i])
            gray = 0;
    }
...
start_input_gif(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
   ...
   */
    ...
    if (sinfo->max_pixels && (unsigned long long)width * height > sinfo->max_pixels)
    ...
    source->colormap = (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, (JDIMENSION)MAXCOLORMAPSIZE, (JDIMENSION)NUMCOLORS);
   ...
   */
    ...
    source->symbol_head = (UINT16*)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, LZW_TABLE_SIZE * sizeof(UINT16));
    source->symbol_tail = (UINT8*)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, LZW_TABLE_SIZE * sizeof(UINT8));
    source->symbol_stack = (UINT8*)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, LZW_TABLE_SIZE * sizeof(UINT8));
    InitLZWCode(source);
   ...
   */
     ...
     */
        source->interlaced_image
            = (*cinfo->mem->request_virt_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, FALSE, (JDIMENSION)width, (JDIMENSION)height, (JDIMENSION)1);
    ...
    source->pub.buffer = (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, (JDIMENSION)width * cinfo->input_components, (JDIMENSION)1);
    source->pub.buffer_height = 1;

    /* Pad colormap for safety. */
    for (c = colormaplen; c < source->clear_code; c++) {
        source->colormap[CM_RED][c] = source->colormap[CM_GREEN][c] = source->colormap[CM_BLUE][c] = CENTERJSAMPLE;
    }
...
load_interlaced_image(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
    ...
    for (row = 0; row < cinfo->image_height; row++) {
        ...
        sptr = *(*cinfo->mem->access_virt_sarray)((j_common_ptr)cinfo, source->interlaced_image, row, (JDIMENSION)1, TRUE);
        for (col = cinfo->image_width; col > 0; col--) {
...
get_interlaced_row(j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
    ...
    sptr = *(*cinfo->mem->access_virt_sarray)((j_common_ptr)cinfo, source->interlaced_image, irow, (JDIMENSION)1, FALSE);
