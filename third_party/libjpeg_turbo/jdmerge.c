...
build_ycc_rgb_table(j_decompress_ptr cinfo)
{
    ...
    upsample->Cr_g_tab = (JLONG*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, (MAXJSAMPLE + 1) * sizeof(JLONG));
    upsample->Cb_g_tab = (JLONG*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, (MAXJSAMPLE + 1) * sizeof(JLONG));
...
METHODDEF(void)
merged_2v_upsample(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION* in_row_group_ctr, JDIMENSION in_row_groups_avail, JSAMPARRAY output_buf,
...
/* 2:1 vertical sampling case: may need a spare row. */
{
    ...
    if (upsample->spare_full) {
        ...
        jcopy_sample_rows(&upsample->spare_row, 0, output_buf + *out_row_ctr, 0, 1, size);
        num_rows = 1;
        upsample->spare_full = FALSE;
    } else {
...
METHODDEF(void)
merged_1v_upsample(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION* in_row_group_ctr, JDIMENSION in_row_groups_avail, JSAMPARRAY output_buf,
    JDIMENSION* out_row_ctr, JDIMENSION out_rows_avail)
...
#define PACK_SHORT_565_LE(r, g, b) ((((r) << 8) & 0xF800) | (((g) << 3) & 0x7E0) | ((b) >> 3))
#define PACK_SHORT_565_BE(r, g, b) (((r)&0xF8) | ((g) >> 5) | (((g) << 11) & 0xE000) | (((b) << 5) & 0x1F00))
...
static const JLONG dither_matrix[4] = { 0x0008020A, 0x0C040E06, 0x030B0109, 0x0F070D05 };
...
static INLINE jpg_boolean is_big_endian(void)
{
    int test_value = 1;
...
METHODDEF(void)
h2v1_merged_upsample_565(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
    if (is_big_endian())
...
METHODDEF(void)
h2v1_merged_upsample_565D(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
    if (is_big_endian())
...
METHODDEF(void)
h2v2_merged_upsample_565(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
    if (is_big_endian())
...
METHODDEF(void)
h2v2_merged_upsample_565D(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
    if (is_big_endian())
...
jinit_merged_upsampler(j_decompress_ptr cinfo)
{
    ...
    upsample = (my_merged_upsample_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, sizeof(my_merged_upsampler));
