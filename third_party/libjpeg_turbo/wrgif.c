...
typedef struct {
    ...
    jpg_boolean first_byte; /* if TRUE, waiting_code is not valid */
...
/* flush any accumulated data */
{
    if (dinfo->bytesinpkt > 0) { /* never write zero-length packet */
        ...
        if (fwrite(dinfo->packetbuf, 1, dinfo->bytesinpkt, dinfo->pub.output_file) != (size_t)dinfo->bytesinpkt)
...
jinit_write_gif(j_decompress_ptr cinfo, jpg_boolean is_lzw)
{
    gif_dest_ptr dest;
    ...
    if (is_lzw) {
        ...
        dest->hash_code = (code_int*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, HSIZE * sizeof(code_int));
        dest->hash_value = (hash_entry*)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, HSIZE * sizeof(hash_entry));
    } else {
