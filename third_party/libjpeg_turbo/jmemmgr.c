...
struct jvirt_sarray_control {
    ...
    jpg_boolean pre_zero; /* pre-zero mode requested? */
    jpg_boolean dirty; /* do current buffer contents need written? */
    jpg_boolean b_s_open; /* is backing-store data valid? */
...
print_mem_stats(j_common_ptr cinfo, int pool_id)
{
   ...
   */
    ...
    for (lhdr_ptr = mem->large_list[pool_id]; lhdr_ptr != NULL; lhdr_ptr = lhdr_ptr->next) {
        fprintf(stderr, "  Large chunk used %ld\n", (long)lhdr_ptr->bytes_used);
    }

    for (shdr_ptr = mem->small_list[pool_id]; shdr_ptr != NULL; shdr_ptr = shdr_ptr->next) {
...
request_virt_sarray(j_common_ptr cinfo, int pool_id, jpg_boolean pre_zero, JDIMENSION samplesperrow, JDIMENSION numrows, JDIMENSION maxaccess)
...
request_virt_barray(j_common_ptr cinfo, int pool_id, jpg_boolean pre_zero, JDIMENSION blocksperrow, JDIMENSION numrows, JDIMENSION maxaccess)
...
LOCAL(void)
do_sarray_io(j_common_ptr cinfo, jvirt_sarray_ptr ptr, jpg_boolean writing)
...
LOCAL(void)
do_barray_io(j_common_ptr cinfo, jvirt_barray_ptr ptr, jpg_boolean writing)
...
METHODDEF(JSAMPARRAY)
access_virt_sarray(j_common_ptr cinfo, jvirt_sarray_ptr ptr, JDIMENSION start_row, JDIMENSION num_rows, jpg_boolean writable)
...
METHODDEF(JBLOCKARRAY)
access_virt_barray(j_common_ptr cinfo, jvirt_barray_ptr ptr, JDIMENSION start_row, JDIMENSION num_rows, jpg_boolean writable)
...
free_pool(j_common_ptr cinfo, int pool_id)
{
    ...
    while (lhdr_ptr != NULL) {
        ...
        space_freed = lhdr_ptr->bytes_used + lhdr_ptr->bytes_left + sizeof(large_pool_hdr) + ALIGN_SIZE - 1;
    ...
    while (shdr_ptr != NULL) {
        ...
        space_freed = shdr_ptr->bytes_used + shdr_ptr->bytes_left + sizeof(small_pool_hdr) + ALIGN_SIZE - 1;
...
jinit_memory_mgr(j_common_ptr cinfo)
{
   ...
   */
    ...
    if ((long)test_mac != MAX_ALLOC_CHUNK || (MAX_ALLOC_CHUNK % ALIGN_SIZE) != 0)
