 ...
 */
#define INPUT_BYTE(cinfo, V, action) MAKESTMT(MAKE_BYTE_AVAIL(cinfo, action); bytes_in_buffer--; V = *next_input_byte++;)

/* As above, but read two bytes interpreted as an unsigned 16-bit integer.
 * V should be declared unsigned int or perhaps JLONG.
 */
#define INPUT_2BYTES(cinfo, V, action)                                                                                                                         \
    MAKESTMT(MAKE_BYTE_AVAIL(cinfo, action); bytes_in_buffer--; V = ((unsigned int)(*next_input_byte++)) << 8; MAKE_BYTE_AVAIL(cinfo, action);                 \
             bytes_in_buffer--; V += *next_input_byte++;)
...
LOCAL(jpg_boolean)
get_sof(j_decompress_ptr cinfo, jpg_boolean is_prog, jpg_boolean is_arith)
/* Process a SOFn marker */
{
    ...
    if (cinfo->comp_info == NULL) /* do only once, even if suspend */
        cinfo->comp_info
            = (jpeg_component_info*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, cinfo->num_components * sizeof(jpeg_component_info));
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        TRACEMS4(cinfo, 1, JTRC_SOF_COMPONENT, compptr->component_id, compptr->h_samp_factor, compptr->v_samp_factor, compptr->quant_tbl_no);
    }
...
/* Process a SOS marker */
{
    ...
    for (i = 0; i < n; i++) {
        ...
        for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components && ci < MAX_COMPS_IN_SCAN; ci++, compptr++) {
    ...
    while (length > 0) {
        ...
        if (cinfo->err->trace_level >= 2) {
            for (i = 0; i < DCTSIZE2; i += 8) {
                TRACEMS8(cinfo, 2, JTRC_QUANTVALS, quant_ptr->quantval[i], quant_ptr->quantval[i + 1], quant_ptr->quantval[i + 2], quant_ptr->quantval[i + 3],
...
LOCAL(void)
examine_app0(j_decompress_ptr cinfo, JOCTET* data, unsigned int datalen, JLONG remaining)
 ...
 */
{
     ...
     */
        ...
        TRACEMS5(cinfo, 1, JTRC_JFIF, cinfo->JFIF_major_version, cinfo->JFIF_minor_version, cinfo->X_density, cinfo->Y_density, cinfo->density_unit);
...
LOCAL(void)
examine_app14(j_decompress_ptr cinfo, JOCTET* data, unsigned int datalen, JLONG remaining)
