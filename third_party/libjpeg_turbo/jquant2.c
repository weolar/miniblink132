...
typedef struct {
    ...
    jpg_boolean needs_zeroed; /* TRUE if next pass must zero histogram */

    /* Variables for Floyd-Steinberg dithering */
    FSERRPTR fserrors; /* accumulated errors */
    jpg_boolean on_odd_row; /* flag to remember which row we are on */
...
prescan_quantize(j_decompress_ptr cinfo, JSAMPARRAY input_buf, JSAMPARRAY output_buf, int num_rows)
{
    ...
    for (row = 0; row < num_rows; row++) {
        ...
        for (col = width; col > 0; col--) {
            ...
            histp = &histogram[ptr[0] >> C0_SHIFT][ptr[1] >> C1_SHIFT][ptr[2] >> C2_SHIFT];
...
METHODDEF(void)
start_pass_2_quant(j_decompress_ptr cinfo, jpg_boolean is_pre_scan)
{
    my_cquantize_ptr cquantize = (my_cquantize_ptr)cinfo->cquantize;
    hist3d histogram = cquantize->histogram;
    int i;
    ...
    if (cquantize->needs_zeroed) {
        for (i = 0; i < HIST_C0_ELEMS; i++) {
            jzero_far((void*)histogram[i], HIST_C1_ELEMS * HIST_C2_ELEMS * sizeof(histcell));
        }
...
jinit_2pass_quantizer(j_decompress_ptr cinfo)
{
   ...
   */
    if (cinfo->dither_mode == JDITHER_FS) {
        cquantize->fserrors
            = (FSERRPTR)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, (size_t)((cinfo->output_width + 2) * (3 * sizeof(FSERROR))));
