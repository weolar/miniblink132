...
GLOBAL(void)
jpeg_add_quant_table(j_compress_ptr cinfo, int which_tbl, const unsigned int* basic_table, int scale_factor, jpg_boolean force_baseline)
...
GLOBAL(void)
jpeg_default_qtables(j_compress_ptr cinfo, jpg_boolean force_baseline)
...
GLOBAL(void)
jpeg_set_linear_quality(j_compress_ptr cinfo, int scale_factor, jpg_boolean force_baseline)
/* Set or change the 'quality' (quantization) setting, using default tables
...
GLOBAL(void)
jpeg_set_quality(j_compress_ptr cinfo, int quality, jpg_boolean force_baseline)
/* Set or change the 'quality' (quantization) setting, using default tables.
...
#define SET_COMP(index, id, hsamp, vsamp, quant, dctbl, actbl)                                                                                                 \
    (compptr = &cinfo->comp_info[index], compptr->component_id = (id), compptr->h_samp_factor = (hsamp), compptr->v_samp_factor = (vsamp),                     \
        compptr->quant_tbl_no = (quant), compptr->dc_tbl_no = (dctbl), compptr->ac_tbl_no = (actbl))
...
jpeg_simple_progression(j_compress_ptr cinfo)
{
   ...
   */
    if (cinfo->script_space == NULL || cinfo->script_space_size < nscans) {
        ...
        cinfo->script_space
            = (jpeg_scan_info*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, cinfo->script_space_size * sizeof(jpeg_scan_info));
    }
