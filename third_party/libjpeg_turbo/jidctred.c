    ...
    GLOBAL(void) jpeg_idct_4x4(j_decompress_ptr cinfo, jpeg_component_info* compptr, JCOEFPTR coef_block, JSAMPARRAY output_buf, JDIMENSION output_col)
{
    ...
    for (ctr = DCTSIZE; ctr > 0; inptr++, quantptr++, wsptr++, ctr--) {
        ...
        if (ctr == DCTSIZE - 4)
            continue;
        if (inptr[DCTSIZE * 1] == 0 && inptr[DCTSIZE * 2] == 0 && inptr[DCTSIZE * 3] == 0 && inptr[DCTSIZE * 5] == 0 && inptr[DCTSIZE * 6] == 0
            && inptr[DCTSIZE * 7] == 0) {
    ...
    for (ctr = 0; ctr < 4; ctr++) {
        ...
        tmp2 = MULTIPLY((JLONG)wsptr[2], FIX_1_847759065) + MULTIPLY((JLONG)wsptr[6], -FIX_0_765366865);
