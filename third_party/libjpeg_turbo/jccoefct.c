...
METHODDEF(jpg_boolean) compress_data(j_compress_ptr cinfo, JSAMPIMAGE input_buf);
#ifdef FULL_COEF_BUFFER_SUPPORTED
METHODDEF(jpg_boolean) compress_first_pass(j_compress_ptr cinfo, JSAMPIMAGE input_buf);
METHODDEF(jpg_boolean) compress_output(j_compress_ptr cinfo, JSAMPIMAGE input_buf);
#endif
                ...
                for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
                    if (coef->iMCU_row_num < last_iMCU_row || yoffset + yindex < compptr->last_row_height) {
                        (*cinfo->fdct->forward_DCT)(
                            cinfo, compptr, input_buf[compptr->component_index], coef->MCU_buffer[blkn], ypos, xpos, (JDIMENSION)blockcnt);
                        if (blockcnt < compptr->MCU_width) {
                            /* Create some dummy blocks at the right edge of the image. */
                            jzero_far((void*)coef->MCU_buffer[blkn + blockcnt], (compptr->MCU_width - blockcnt) * sizeof(JBLOCK));
                        ...
                        jzero_far((void*)coef->MCU_buffer[blkn], compptr->MCU_width * sizeof(JBLOCK));
...
compress_first_pass(j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        buffer = (*cinfo->mem->access_virt_barray)(
            (j_common_ptr)cinfo, coef->whole_image[ci], coef->iMCU_row_num * compptr->v_samp_factor, (JDIMENSION)compptr->v_samp_factor, TRUE);
     ...
     */
        for (block_row = 0; block_row < block_rows; block_row++) {
            ...
            (*cinfo->fdct->forward_DCT)(cinfo, compptr, input_buf[ci], thisblockrow, (JDIMENSION)(block_row * DCTSIZE), (JDIMENSION)0, blocks_across);
            if (ndummy > 0) {
...
GLOBAL(void)
jinit_c_coef_controller(j_compress_ptr cinfo, jpg_boolean need_full_buffer)
{
    my_coef_ptr coef;
