...
typedef struct {
   ...
   */
    ...
    jpg_boolean dc_needed[D_MAX_BLOCKS_IN_MCU];
    jpg_boolean ac_needed[D_MAX_BLOCKS_IN_MCU];
...
GLOBAL(void)
jpeg_make_d_derived_tbl(j_decompress_ptr cinfo, jpg_boolean isDC, int tblno, d_derived_tbl** pdtbl)
{
    JHUFF_TBL* htbl;
    d_derived_tbl* dtbl;
...
__attribute__((no_sanitize("signed-integer-overflow"), no_sanitize("unsigned-integer-overflow")))
#endif
#endif
LOCAL(jpg_boolean) decode_mcu_slow(j_decompress_ptr cinfo, JBLOCKROW* MCU_data)
{
    huff_entropy_ptr entropy = (huff_entropy_ptr)cinfo->entropy;
    BITREAD_STATE_VARS;
    int blkn;
...
#endif
#endif
LOCAL(jpg_boolean) decode_mcu_fast(j_decompress_ptr cinfo, JBLOCKROW* MCU_data)
{
    huff_entropy_ptr entropy = (huff_entropy_ptr)cinfo->entropy;
    BITREAD_STATE_VARS;
    JOCTET* buffer;
