...
static jpg_boolean memsrc; /* for -memsrc switch */
static jpg_boolean report; /* for -report switch */
jpg_boolean skip, crop;
JDIMENSION skip_start, skip_end;
JDIMENSION crop_x, crop_y, crop_width, crop_height;
static jpg_boolean strict; /* for -strict switch */
...
LOCAL(int)
parse_switches(j_decompress_ptr cinfo, int argc, char** argv, int last_file_arg_seen, jpg_boolean for_real)
 ...
 */
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        if (keymatch(arg, "bmp", 1)) {
            ...
            static jpg_boolean printed_version = FALSE;
            ...
            if (++argn >= argc) /* advance to next argument */
                usage();
            if (sscanf(argv[argn], "%u/%u", &cinfo->scale_num, &cinfo->scale_denom) != 2)
                usage();

        } else if (keymatch(arg, "skip", 2)) {
            if (++argn >= argc)
                usage();
            if (sscanf(argv[argn], "%u,%u", &skip_start, &skip_end) != 2 || skip_start > skip_end)
                usage();
            skip = TRUE;
            ...
            if (++argn >= argc)
                usage();
            if (sscanf(argv[argn], "%u%c%u+%u+%u", &crop_width, &c, &crop_height, &crop_x, &crop_y) != 5 || (c != 'X' && c != 'x') || crop_width < 1
                || crop_height < 1)
                usage();
            crop = TRUE;
...
{
    jpg_boolean traceit = (cinfo->err->trace_level >= 1);
    long length;
    unsigned int ch;
...
#endif
{
   ...
   */
     ...
     */
        ...
        if ((tmp = jpeg_skip_scanlines(&cinfo, skip_end - skip_start + 1)) != skip_end - skip_start + 1) {
     ...
     */
        ...
        if ((tmp = jpeg_skip_scanlines(&cinfo, cinfo.output_height - crop_y - crop_height)) != cinfo.output_height - crop_y - crop_height) {
