...
public:
    ...
    template <typename T> void AddField(span<char> field_name, const T& value)
    {
        cbor::EncodeString8(span<uint8_t>(reinterpret_cast<const uint8_t*>(field_name.data()), field_name.size()), bytes_);
        ProtocolTypeTraits<T>::Serialize(value, bytes_);
    }
    template <typename T> void AddField(span<char> field_name, const detail::ValueMaybe<T>& value)
    {
        if (!value.has_value()) {
            return;
        }
    ...
    template <typename T> void AddField(span<char> field_name, const detail::PtrMaybe<T>& value)
    {
        if (!value.has_value()) {
            return;
        }
...
public:
    ...
    template <typename T> void AddField(span<char> name, const T& field)
    {
...
private:
    bool DeserializeField(DeserializerState* state, span<char> name, int* seen_mandatory_fields, void* obj) const;
...
template <typename T> struct ProtocolTypeTraits<std::vector<T>> {
    ...
    static void Serialize(const std::vector<T>& value, std::vector<uint8_t>* bytes)
    {
        ContainerSerializer container_serializer(bytes, cbor::EncodeIndefiniteLengthArrayStart());
...
template <typename T> struct ProtocolTypeTraits<std::unique_ptr<std::vector<T>>> {
    static bool Deserialize(DeserializerState* state, std::unique_ptr<std::vector<T>>* value)
    {
    ...
    static void Serialize(const std::unique_ptr<std::vector<T>>& value, std::vector<uint8_t>* bytes)
    {
...
public:
    static std::unique_ptr<DeferredMessage> FromSerializable(std::unique_ptr<Serializable> serializeable);
...
template <> struct CRDTP_EXPORT ProtocolTypeTraits<std::unique_ptr<DeferredMessage>> {
    static bool Deserialize(DeserializerState* state, std::unique_ptr<DeferredMessage>* value);
    static void Serialize(const std::unique_ptr<DeferredMessage>& value, std::vector<uint8_t>* bytes);
};

template <typename T> struct ProtocolTypeTraits<detail::ValueMaybe<T>> {
    static bool Deserialize(DeserializerState* state, detail::ValueMaybe<T>* value)
    {
        T res;
    ...
    static void Serialize(const detail::ValueMaybe<T>& value, std::vector<uint8_t>* bytes)
    {
        ProtocolTypeTraits<T>::Serialize(value.value(), bytes);
    }
};

template <typename T> struct ProtocolTypeTraits<detail::PtrMaybe<T>> {
    static bool Deserialize(DeserializerState* state, detail::PtrMaybe<T>* value)
    {
    ...
    static void Serialize(const detail::PtrMaybe<T>& value, std::vector<uint8_t>* bytes)
    {
        ProtocolTypeTraits<T>::Serialize(value.value(), bytes);
    }
};

template <typename T> class DeserializableProtocolObject {
public:
    static StatusOr<std::unique_ptr<T>> ReadFrom(const DeferredMessage& deferred_message)
    {
    ...
    static std::unique_ptr<T> FromBinary(const uint8_t* bytes, size_t size)
    {
        ...
        auto deserializer = DeferredMessage::FromSpan(span<uint8_t>(bytes, size))->MakeDeserializer();
...
protected:
    ...
    template <typename U> using DeserializableBase = DeserializableProtocolObject<U>;
...
template <typename T> class ProtocolObject : public Serializable, public DeserializableProtocolObject<T> {
public:
...
template <typename T> struct ProtocolTypeTraits<T, typename std::enable_if<std::is_base_of<ProtocolObject<T>, T>::value>::type> {
...
template <typename T> struct ProtocolTypeTraits<std::unique_ptr<T>, typename std::enable_if<std::is_base_of<ProtocolObject<T>, T>::value>::type> {
...
template <typename T, typename F> bool ConvertProtocolValue(const F& from, T* to)
{
    std::vector<uint8_t> bytes;
    ProtocolTypeTraits<F>::Serialize(from, &bytes);
    auto deserializer = DeferredMessage::FromSpan(span<uint8_t>(bytes.data(), bytes.size()))->MakeDeserializer();
...
#define CRDTP_DESERIALIZE_FILED_IMPL(name, field, is_optional)                                                                                                 \
    {                                                                                                                                                          \
        MakeSpan(name), is_optional, [](DeserializerState* __state, void* __obj) -> bool {                                                                     \
            return ProtocolTypeTraits<decltype(field)>::Deserialize(__state, &static_cast<ProtocolType*>(__obj)->field);                                       \
        }                                                                                                                                                      \
    }
...
#define CRDTP_END_DESERIALIZER()                                                                                                                               \
    ...
    ;                                                                                                                                                          \
    static const DeserializerDescriptorType s_desc(fields, sizeof fields / sizeof fields[0]);                                                                  \
    return s_desc;                                                                                                                                             \
    }

#define CRDTP_DESERIALIZE_FIELD(name, field) CRDTP_DESERIALIZE_FILED_IMPL(name, field, false)
#define CRDTP_DESERIALIZE_FIELD_OPT(name, field) CRDTP_DESERIALIZE_FILED_IMPL(name, field, true)

#define CRDTP_BEGIN_SERIALIZER(type)                                                                                                                           \
    void type::AppendSerialized(std::vector<uint8_t>* bytes) const                                                                                             \
    {                                                                                                                                                          \
        using namespace crdtp;                                                                                                                                 \
        ContainerSerializer __serializer(bytes, cbor::EncodeIndefiniteLengthMapStart());

#define CRDTP_SERIALIZE_FIELD(name, field) __serializer.AddField(MakeSpan(name), field)
