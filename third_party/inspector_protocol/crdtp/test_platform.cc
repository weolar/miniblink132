...
{
    std::string out;
    bool success = base::UTF16ToUTF8(reinterpret_cast<const char16_t*>(in.data()), in.size(), &out);
    CHECK(success);
    return out;
}
...
{
    std::u16string tmp;
    bool success = base::UTF8ToUTF16(reinterpret_cast<const char*>(in.data()), in.size(), &tmp);
    CHECK(success);
