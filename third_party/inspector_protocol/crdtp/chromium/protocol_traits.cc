...
Binary::Binary()
    : bytes_(new base::RefCountedBytes)
{
}
Binary::Binary(const Binary& binary)
    : bytes_(binary.bytes_)
{
}
...
{
    return base::Base64Encode(base::span<const uint8_t>(bytes_->data(), bytes_->size()));
}
...
bool ProtocolTypeTraits<Binary>::Deserialize(DeserializerState* state, Binary* value)
{
    ...
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::BINARY) {
        crdtp::span<uint8_t> bin = tokenizer->GetBinary();
        *value = Binary::fromSpan(base::span<const uint8_t>(bin.data(), bin.size()));
        return true;
    }
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::STRING8) {
        const auto str_span = tokenizer->GetString8();
        bool success = false;
        *value = Binary::fromBase64(std::string_view(reinterpret_cast<const char*>(str_span.data()), str_span.size()), &success);
        if (!success)
...
// static
bool ProtocolTypeTraits<std::string>::Deserialize(DeserializerState* state, std::string* value)
{
    auto* tokenizer = state->tokenizer();
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::STRING8) {
        const auto str_span = tokenizer->GetString8();
        *value = std::string(reinterpret_cast<const char*>(str_span.data()), str_span.size());
        return true;
    }
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::STRING16) {
        ...
        base::UTF16ToUTF8(reinterpret_cast<const char16_t*>(str.data()), str.size() / 2, value);
        return true;
    }
...
template <typename Iterable> void SerializeDict(const Iterable& iterable, std::vector<uint8_t>* bytes)
{
    ContainerSerializer serializer(bytes, cbor::EncodeIndefiniteLengthMapStart());
    for (const auto& kv : iterable) {
        serializer.AddField(span<char>(kv.first.data(), kv.first.size()), kv.second);
    }
...
// static
void ProtocolTypeTraits<std::string>::Serialize(const std::string& str, std::vector<uint8_t>* bytes)
{
    cbor::EncodeString8(SpanFrom(str), bytes);
}

// static
bool ProtocolTypeTraits<base::Value>::Deserialize(DeserializerState* state, base::Value* value)
{
    cbor::CBORTokenizer* tokenizer = state->tokenizer();
    ...
    case cbor::CBORTokenTag::STRING8: {
        ...
        *value = base::Value(std::string_view(reinterpret_cast<const char*>(str.data()), str.size()));
        break;
    }
    case cbor::CBORTokenTag::STRING16: {
        const auto str = tokenizer->GetString16WireRep();
        // TODO(caseq): support big-endian architectures when needed.
        *value = base::Value(std::u16string_view(reinterpret_cast<const char16_t*>(str.data()), str.size() / 2));
        break;
    }

    case cbor::CBORTokenTag::ENVELOPE:
        tokenizer->EnterEnvelope();
        if (tokenizer->TokenTag() != cbor::CBORTokenTag::ARRAY_START && tokenizer->TokenTag() != cbor::CBORTokenTag::MAP_START) {
...
// static
void ProtocolTypeTraits<base::Value>::Serialize(const base::Value& value, std::vector<uint8_t>* bytes)
{
    switch (value.type()) {
    ...
    case base::Value::Type::BOOLEAN:
        bytes->push_back(value.GetBool() ? cbor::EncodeTrue() : cbor::EncodeFalse());
        return;
    ...
    case base::Value::Type::LIST: {
        ContainerSerializer serializer(bytes, cbor::EncodeIndefiniteLengthArrayStart());
...
// static
bool ProtocolTypeTraits<traits::DictionaryValue>::Deserialize(DeserializerState* state, traits::DictionaryValue* value)
{
    return DeserializeDict(state, value);
}

// static
void ProtocolTypeTraits<traits::DictionaryValue>::Serialize(const traits::DictionaryValue& value, std::vector<uint8_t>* bytes)
{
    SerializeDict(value, bytes);
}

// static
bool ProtocolTypeTraits<traits::ListValue>::Deserialize(DeserializerState* state, traits::ListValue* value)
{
    auto* tokenizer = state->tokenizer();
    if (tokenizer->TokenTag() == cbor::CBORTokenTag::ENVELOPE)
        tokenizer->EnterEnvelope();
...
void ProtocolTypeTraits<traits::ListValue>::Serialize(const traits::ListValue& value, std::vector<uint8_t>* bytes)
{
