...
template <typename T, typename F> T ConvertTo(const F& from)
{
    ...
    EXPECT_TRUE(rc) << deserializer.ErrorMessage(crdtp::MakeSpan("Error deserializing"));
    return to;
}

template <typename T> T RoundTrip(const T& from)
{
...
TEST(ProtocolTraits, BinaryBasic)
{
    ...
    constexpr uint8_t data[] = { 'H', 'e', 'l', 'l', 'o', ',', 0, 'w', 'o', 'r', 'l', 'd', '!', 0x80 };
    ...
    EXPECT_THAT(MakeVector(Binary::fromString(std::string(reinterpret_cast<const char*>(data), sizeof data))), Eq(data_vec));
    bool success = false;
    EXPECT_THAT(MakeVector(Binary::fromBase64("SGVsbG8sAHdvcmxkIYA=", &success)), Eq(data_vec));
...
TEST(ProtocolTraits, PrimitiveValueSerialization)
{
    ...
    EXPECT_THAT(ConvertTo<double>(base::Value(6.62607015e-34)), Eq(6.62607015e-34));
    EXPECT_THAT(RoundTrip(base::Value(6.62607015e-34)), IsJson(base::Value(6.62607015e-34)));
}

template <typename... Args> base::Value::List MakeList(Args&&... args)
{
    base::Value::List res;
...
{
    EXPECT_THAT(ConvertTo<std::vector<int>>(base::Value(base::Value::List())), Eq(std::vector<int>()));
    EXPECT_THAT(RoundTrip(base::Value(base::Value::List())), IsJson(base::Value(base::Value::List())));

    base::Value::List list = MakeList(2, 3, 5);
    base::Value list_value = base::Value(list.Clone());
    EXPECT_THAT(ConvertTo<std::vector<int>>(list_value), Eq(std::vector<int> { 2, 3, 5 }));
    EXPECT_THAT(ConvertTo<base::Value>(list), IsJson(list_value));
    EXPECT_THAT(RoundTrip(list_value), IsJson(list_value));

    base::Value list_of_lists_value = base::Value(MakeList(list_value.Clone(), MakeList("foo", "bar", "bazz"), MakeList(base::Value())));
...
TEST(ProtocolTraits, DictValueSerialization)
{
    ...
    EXPECT_THAT(RoundTrip(base::Value(dict.Clone())), IsJson(base::Value(base::Value::Type::DICT)));
    dict.Set("int", 42);
    ...
    EXPECT_THAT(ConvertTo<base::Value>(dict.Clone()), IsJson(base::Value(dict.Clone())));
    EXPECT_THAT(RoundTrip(base::Value(dict.Clone())), IsJson(base::Value(dict.Clone())));
}
