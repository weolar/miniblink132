...
// not be deleted.
bool GetNodeAttributeNames(xmlTextReaderPtr reader, GetAttributesQueryType query_type, std::vector<const xmlChar*>* names)
{
...
bool XmlReader::Load(const std::string& input)
{
    ...
    reader_ = xmlReaderForMemory(input.data(), static_cast<int>(input.size()), nullptr, nullptr, kParseOptions);
...
bool XmlReader::GetAllNodeAttributes(std::map<std::string, std::string>* attributes)
{
    ...
    for (const auto* name : attribute_names) {
        (*attributes)[XmlStringToStdString(name)] = XmlStringToStdStringWithDelete(xmlTextReaderGetAttribute(reader_, name));
    }
    return true;
}

bool XmlReader::GetAllDeclaredNamespaces(std::map<std::string, std::string>* namespaces)
{
    ...
    for (const auto* prefix : prefixes) {
        ...
        std::string value = XmlStringToStdStringWithDelete(xmlTextReaderLookupNamespace(reader_, prefix));
        if (value.empty() && default_namespace) {
            // Default namespace is treated as an attribute for some reason.
            value = XmlStringToStdStringWithDelete(xmlTextReaderGetAttribute(reader_, prefix));
        }
        (*namespaces)[default_namespace ? "" : XmlStringToStdString(prefix)] = value;
    }
    return true;
}
