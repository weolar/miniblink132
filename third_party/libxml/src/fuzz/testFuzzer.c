...
static int testFuzzer(initFunc init, fuzzFunc fuzz, const char* pattern)
{
    glob_t globbuf;
    int ret = -1;
    size_t i;
...
#ifdef HAVE_XML_FUZZER
static int testEntityLoader(void)
{
    static const char data[] = "doc.xml\\\n"
    ...
    doc = xmlReadMemory(docBuffer, docSize, NULL, NULL, XML_PARSE_NOENT | XML_PARSE_DTDLOAD);
