...
typedef int (*fileFunc)(const char* base, FILE* out);

typedef int (*mainFunc)(const char* arg);
...
#if defined(HAVE_SCHEMA_FUZZER) || defined(HAVE_XML_FUZZER)
/*
 ...
 */
static xmlParserInputPtr fuzzEntityRecorder(const char* URL, const char* ID, xmlParserCtxtPtr ctxt)
{
    ...
    } else if (xmlHashLookup(globalData.entities, (const xmlChar*)URL) != NULL) {
        return (in);
    }
    ...
    xmlFuzzWriteString(globalData.out, (char*)xmlBufContent(in->buf->buffer));
...
static void fuzzRecorderInit(FILE* out)
{
    globalData.out = out;
...
static void fuzzRecorderCleanup(void)
{
...
static int processXml(const char* docFile, FILE* out)
{
...
static int processHtml(const char* docFile, FILE* out)
{
...
static int processSchema(const char* docFile, FILE* out)
{
    xmlSchemaPtr schema;
...
#if defined(HAVE_HTML_FUZZER) || defined(HAVE_SCHEMA_FUZZER) || defined(HAVE_XML_FUZZER)
static int processPattern(const char* pattern)
{
    glob_t globbuf;
    int ret = 0;
    int res;
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
        ...
        size = snprintf(outPath, sizeof(outPath), "seed/%s/%s", globalData.fuzzer, base);
        if (size >= PATH_SIZE) {
...
static int processXPath(const char* testDir, const char* prefix, const char* name, const char* data, const char* subdir, int xptr)
{
    ...
    size = snprintf(pattern, sizeof(pattern), "%s/%s/%s*", testDir, subdir, prefix);
    if (size >= PATH_SIZE)
        return (-1);
    res = glob(pattern, 0, NULL, &globbuf);
    if (res == GLOB_NOMATCH)
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
        ...
        while (fgets(expr, EXPR_SIZE, in) != NULL) {
            ...
            size = snprintf(outPath, sizeof(outPath), "seed/xpath/%s-%d", name, globalData.counter);
            if (size >= PATH_SIZE) {
                ret = -1;
                continue;
            }
...
static int processXPathDir(const char* testDir)
{
    char pattern[PATH_SIZE];
    glob_t globbuf;
    size_t i, size;
