 ...
 */
void xmlFuzzErrorFunc(void* ctx ATTRIBUTE_UNUSED, const char* msg ATTRIBUTE_UNUSED, ...)
{
}
 ...
 */
xmlParserInputPtr xmlFuzzEntityLoader(const char* URL, const char* ID ATTRIBUTE_UNUSED, xmlParserCtxtPtr ctxt)
{
    ...
    input->filename = NULL;
    input->buf = xmlParserInputBufferCreateMem(entity->data, entity->size, XML_CHAR_ENCODING_NONE);
    if (input->buf == NULL) {
 ...
 */
size_t xmlFuzzExtractStrings(const char* data, size_t size, char** strings, size_t numStrings)
{
...
char* xmlSlurpFile(const char* path, size_t* sizeRet)
{
    FILE* file;
    struct stat statbuf;
    char* data;
