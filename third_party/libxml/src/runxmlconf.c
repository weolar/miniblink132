 ...
 */
static xmlParserInputPtr testExternalEntityLoader(const char* URL, const char* ID ATTRIBUTE_UNUSED, xmlParserCtxtPtr ctxt)
{
...
static void testErrorHandler(void* userData ATTRIBUTE_UNUSED, xmlErrorPtr error)
{
    int res;

    if (testErrorsSize >= 32768)
        return;
    res = snprintf(&testErrors[testErrorsSize], 32768 - testErrorsSize, "%s:%d: %s\n", (error->file ? error->file : "entity"), error->line, error->message);
...
static int xmlconfTestNotNSWF(const char* id, const char* filename, int options)
{
    ...
    if (doc == NULL) {
        ...
        if ((xmlLastError.code == XML_ERR_OK) || (xmlLastError.domain != XML_FROM_NAMESPACE)) {
...
static int xmlconfTestItem(xmlDocPtr doc, xmlNodePtr cur)
{
    ...
    if (!checkTestFile((char*)filename)) {
        test_log("test %s missing file %s \n", id, (filename ? (char*)filename : "NULL"));
        goto error;
    }
    ...
    if ((rec == NULL) || (xmlStrEqual(rec, BAD_CAST "XML1.0")) || (xmlStrEqual(rec, BAD_CAST "XML1.0-errata2e"))
        || (xmlStrEqual(rec, BAD_CAST "XML1.0-errata3e")) || (xmlStrEqual(rec, BAD_CAST "XML1.0-errata4e"))) {
        ...
        ret = 1;
    } else if ((xmlStrEqual(rec, BAD_CAST "NS1.0")) || (xmlStrEqual(rec, BAD_CAST "NS1.0-errata1e"))) {
        ret = 1;
        nstest = 1;
    } else {
