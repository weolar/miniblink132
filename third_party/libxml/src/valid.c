...
static void xmlVErrMemory(xmlValidCtxtPtr ctxt, const char* extra)
{
    ...
    if (extra)
        __xmlRaiseError(NULL, channel, data, pctxt, NULL, XML_FROM_VALID, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL, NULL, 0, 0,
            "Memory allocation failed : %s\n", extra);
    else
        __xmlRaiseError(
            NULL, channel, data, pctxt, NULL, XML_FROM_VALID, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "Memory allocation failed\n");
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlErrValid(xmlValidCtxtPtr ctxt, xmlParserErrors error, const char* msg, const char* extra)
{
    xmlGenericErrorFunc channel = NULL;
    xmlParserCtxtPtr pctxt = NULL;
    void* data = NULL;
    ...
    if (extra)
        ...
        __xmlRaiseError(NULL, channel, data, pctxt, NULL, XML_FROM_VALID, error, XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, 0, 0, "%s", msg);
}
...
static void LIBXML_ATTR_FORMAT(4, 0) xmlErrValidNode(
    xmlValidCtxtPtr ctxt, xmlNodePtr node, xmlParserErrors error, const char* msg, const xmlChar* str1, const xmlChar* str2, const xmlChar* str3)
{
    ...
    __xmlRaiseError(schannel, channel, data, pctxt, node, XML_FROM_VALID, error, XML_ERR_ERROR, NULL, 0, (const char*)str1, (const char*)str2,
    ...
    xmlErrValidNodeNr(xmlValidCtxtPtr ctxt, xmlNodePtr node, xmlParserErrors error, const char* msg, const xmlChar* str1, int int2, const xmlChar* str3)
{
    ...
    __xmlRaiseError(schannel, channel, data, pctxt, node, XML_FROM_VALID, error, XML_ERR_ERROR, NULL, 0, (const char*)str1, (const char*)str3, NULL, int2, 0,
        msg, str1, int2, str3);
}
 ...
 * Handle a validation error, provide contextual information
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlErrValidWarning(
    xmlValidCtxtPtr ctxt, xmlNodePtr node, xmlParserErrors error, const char* msg, const xmlChar* str1, const xmlChar* str2, const xmlChar* str3)
{
    xmlStructuredErrorFunc schannel = NULL;
    xmlGenericErrorFunc channel = NULL;
    ...
    __xmlRaiseError(schannel, channel, data, pctxt, node, XML_FROM_VALID, error, XML_ERR_WARNING, NULL, 0, (const char*)str1, (const char*)str2,
        (const char*)str3, 0, 0, msg, str1, str2, str3);
}
...
static int vstateVPush(xmlValidCtxtPtr ctxt, xmlElementContentPtr cont, xmlNodePtr node, unsigned char depth, long occurs, unsigned char state)
{
     ...
     */
    if ((i >= 0) && (ctxt->vstateTab[i].cont == cont) && (ctxt->vstateTab[i].node == node) && (ctxt->vstateTab[i].depth == depth)
        && (ctxt->vstateTab[i].occurs == occurs) && (ctxt->vstateTab[i].state == state))
        return (ctxt->vstateNr);
...
int xmlValidBuildContentModel(xmlValidCtxtPtr ctxt, xmlElementPtr elem)
{
    ...
    if (xmlRegexpIsDeterminist(elem->contModel) != 1) {
        ...
        xmlErrValidNode(
            ctxt, (xmlNodePtr)elem, XML_DTD_CONTENT_NOT_DETERMINIST, "Content model of %s is not deterministic: %s\n", elem->name, BAD_CAST expr, NULL);
...
void xmlSnprintfElementContent(char* buf, int size, xmlElementContentPtr content, int englob)
{
    ...
    case XML_ELEMENT_CONTENT_SEQ:
        ...
        strcat(buf, " , ");
        if (((content->c2->type == XML_ELEMENT_CONTENT_OR) || (content->c2->ocur != XML_ELEMENT_CONTENT_ONCE))
    ...
    case XML_ELEMENT_CONTENT_OR:
        ...
        strcat(buf, " | ");
        if (((content->c2->type == XML_ELEMENT_CONTENT_SEQ) || (content->c2->ocur != XML_ELEMENT_CONTENT_ONCE))
            && (content->c2->type != XML_ELEMENT_CONTENT_ELEMENT))
            xmlSnprintfElementContent(buf, size, content->c2, 1);
        else
            xmlSnprintfElementContent(buf, size, content->c2, 0);
...
static int xmlScanIDAttributeDecl(xmlValidCtxtPtr ctxt, xmlElementPtr elem, int err)
{
    ...
    while (cur != NULL) {
        if (cur->atype == XML_ATTRIBUTE_ID) {
            ...
            if ((ret > 1) && (err))
                xmlErrValidNode(
                    ctxt, (xmlNodePtr)elem, XML_DTD_MULTIPLE_ID, "Element %s has too many ID attributes defined : %s\n", elem->name, cur->name, NULL);
        }
        cur = cur->nexth;
    }
    return (ret);
 ...
 */
xmlAttributePtr xmlAddAttributeDecl(xmlValidCtxtPtr ctxt, xmlDtdPtr dtd, const xmlChar* elem, const xmlChar* name, const xmlChar* ns, xmlAttributeType type,
    xmlAttributeDefault def, const xmlChar* defaultValue, xmlEnumerationPtr tree)
{
    xmlAttributePtr ret;
...
int xmlIsID(xmlDocPtr doc, xmlNodePtr elem, xmlAttrPtr attr)
{
    ...
    if ((doc->intSubset == NULL) && (doc->extSubset == NULL) && (doc->type != XML_HTML_DOCUMENT_NODE)) {
        ...
        if ((xmlStrEqual(BAD_CAST "id", attr->name))
            || ((xmlStrEqual(BAD_CAST "name", attr->name)) && ((elem == NULL) || (xmlStrEqual(elem->name, BAD_CAST "a")))))
            return (1);
        return (0);
    } else if (elem == NULL) {
        return (0);
        ...
        fullelemname = (elem->ns != NULL && elem->ns->prefix != NULL) ? xmlBuildQName(elem->name, elem->ns->prefix, felem, 50) : (xmlChar*)elem->name;

        fullattrname = (attr->ns != NULL && attr->ns->prefix != NULL) ? xmlBuildQName(attr->name, attr->ns->prefix, fattr, 50) : (xmlChar*)attr->name;
...
static int xmlIsDocNameStartChar(xmlDocPtr doc, int c)
{
	 ...
	 * Update 5 of XML-1.0
	 */
        if (((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z')) || (c == '_') || (c == ':') || ((c >= 0xC0) && (c <= 0xD6)) || ((c >= 0xD8) && (c <= 0xF6))
        ...
        while (*cur != 0) {
            ...
            *cur = 0;
            ...
            if (ent == NULL) {
                ...
                xmlErrValidNode(
                    ctxt, (xmlNodePtr)doc, XML_DTD_ENTITY_TYPE, "ENTITIES attribute %s reference an entity \"%s\" of wrong type\n", name, nam, NULL);
                ret = 0;
            }
            if (save == 0)
                break;
...
int xmlValidateAttributeDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlAttributePtr attr)
{
    ...
    if ((attr->atype == XML_ATTRIBUTE_ID) && (attr->def != XML_ATTRIBUTE_IMPLIED) && (attr->def != XML_ATTRIBUTE_REQUIRED)) {
        xmlErrValidNode(
            ctxt, (xmlNodePtr)attr, XML_DTD_ID_FIXED, "ID attribute %s of %s is not valid must be #IMPLIED or #REQUIRED\n", attr->name, attr->elem, NULL);
        ret = 0;
    }
...
int xmlValidateElementDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlElementPtr elem)
{
            ...
            if (cur->c1->type == XML_ELEMENT_CONTENT_ELEMENT) {
                ...
                while (next != NULL) {
                    if (next->type == XML_ELEMENT_CONTENT_ELEMENT) {
                        if ((xmlStrEqual(next->name, name)) && (xmlStrEqual(next->prefix, cur->c1->prefix))) {
                            if (cur->c1->prefix == NULL) {
                                xmlErrValidNode(
                                    ctxt, (xmlNodePtr)elem, XML_DTD_CONTENT_ERROR, "Definition of %s has duplicate references of %s\n", elem->name, name, NULL);
                            } else {
                    ...
                    if ((xmlStrEqual(next->c1->name, name)) && (xmlStrEqual(next->c1->prefix, cur->c1->prefix))) {
                        if (cur->c1->prefix == NULL) {
                            xmlErrValidNode(
                                ctxt, (xmlNodePtr)elem, XML_DTD_CONTENT_ERROR, "Definition of %s has duplicate references to %s\n", elem->name, name, NULL);
                        } else {
    ...
    if ((tst != NULL) && (tst != elem) && ((tst->prefix == elem->prefix) || (xmlStrEqual(tst->prefix, elem->prefix)))
...
int xmlValidateOneAttribute(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr elem, xmlAttrPtr attr, const xmlChar* value)
{
    ...
    if (attrDecl->atype == XML_ATTRIBUTE_NOTATION) {
        ...
        if (nota == NULL) {
            xmlErrValidNode(
                ctxt, elem, XML_DTD_UNKNOWN_NOTATION, "Value \"%s\" for attribute %s of %s is not a declared Notation\n", value, attr->name, elem->name);
            ret = 0;
        }
    ...
    if (attrDecl->atype == XML_ATTRIBUTE_ENUMERATION) {
        ...
        if (tree == NULL) {
            xmlErrValidNode(
                ctxt, elem, XML_DTD_ATTRIBUTE_VALUE, "Value \"%s\" for attribute %s of %s is not among the enumerated set\n", value, attr->name, elem->name);
            ret = 0;
        }
    }
...
int xmlValidateOneNamespace(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr elem, const xmlChar* prefix, xmlNsPtr ns, const xmlChar* value)
{
    ...
    if (attrDecl->atype == XML_ATTRIBUTE_NOTATION) {
        ...
        if (nota == NULL) {
            if (ns->prefix != NULL) {
                ...
                xmlErrValidNode(
                    ctxt, elem, XML_DTD_UNKNOWN_NOTATION, "Value \"%s\" for attribute xmlns of %s is not a declared Notation\n", value, elem->name, NULL);
            }
            ret = 0;
        }
    ...
    if (attrDecl->atype == XML_ATTRIBUTE_ENUMERATION) {
        ...
        if (tree == NULL) {
            if (ns->prefix != NULL) {
                ...
                xmlErrValidNode(
                    ctxt, elem, XML_DTD_ATTRIBUTE_VALUE, "Value \"%s\" for attribute xmlns of %s is not among the enumerated set\n", value, elem->name, NULL);
            }
            ret = 0;
        }
    }
    ...
    if ((attrDecl->def == XML_ATTRIBUTE_FIXED) && (!xmlStrEqual(attrDecl->defaultValue, value))) {
        if (ns->prefix != NULL) {
            xmlErrValidNode(
                ctxt, elem, XML_DTD_ELEM_NAMESPACE, "Value for attribute xmlns:%s of %s must be \"%s\"\n", ns->prefix, elem->name, attrDecl->defaultValue);
        } else {
...
cont:
     ...
     */
    if ((CONT != NULL) && ((CONT->parent == NULL) || (CONT->parent == (xmlElementContentPtr)1) || (CONT->parent->type != XML_ELEMENT_CONTENT_OR))
...
static int xmlValidateElementContent(xmlValidCtxtPtr ctxt, xmlNodePtr child, xmlElementPtr elemDecl, int warn, xmlNodePtr parent)
{
    ...
    if ((ret == -3) && (warn)) {
        ...
        xmlErrValidNode(
            ctxt, (xmlNodePtr)elemDecl, XML_DTD_CONTENT_NOT_DETERMINIST, "Content model of %s is not deterministic: %s\n", name, BAD_CAST expr, NULL);
    } else if (ret == -2) {
        /*
    ...
    if ((warn) && ((ret != 1) && (ret != -3))) {
        if (ctxt != NULL) {
            ...
            if (name != NULL) {
                ...
                xmlErrValidNode(
                    ctxt, parent, XML_DTD_CONTENT_MODEL, "Element content does not follow the DTD, expecting %s, got %s\n", BAD_CAST expr, BAD_CAST list, NULL);
            }
        } else {
            if (name != NULL) {
        ...
        if (state->elemDecl != NULL) {
            ...
            case XML_ELEMENT_TYPE_MIXED:
                ...
                if ((elemDecl->content != NULL) && (elemDecl->content->type == XML_ELEMENT_CONTENT_PCDATA)) {
                    xmlErrValidNode(
                        ctxt, state->node, XML_DTD_NOT_PCDATA, "Element %s was declared #PCDATA but contains non text nodes\n", state->node->name, NULL, NULL);
                    ret = 0;
                } else {
        ...
        case XML_ELEMENT_TYPE_MIXED:
            ...
            while (child != NULL) {
                if (child->type == XML_ELEMENT_NODE) {
                    ...
                    if (cont == NULL) {
                        xmlErrValidNode(
                            ctxt, elem, XML_DTD_INVALID_CHILD, "Element %s is not declared in %s list of possible children\n", name, elem->name, NULL);
                        ret = 0;
                    }
                }
            child_ok:
    ...
    while (attr != NULL) {
        if (attr->def == XML_ATTRIBUTE_REQUIRED) {
            ...
            if (qualified == -1) {
                ...
                xmlErrValidWarning(
                    ctxt, elem, XML_DTD_DIFFERENT_PREFIX, "Element %s required attribute %s:%s has different prefix\n", elem->name, attr->prefix, attr->name);
            }
            ...
            if ((attr->prefix == NULL) && (xmlStrEqual(attr->name, BAD_CAST "xmlns"))) {
                ...
                while (ns != NULL) {
                    if (xmlStrEqual(attr->name, ns->prefix)) {
                        if (!xmlStrEqual(attr->defaultValue, ns->href)) {
                            xmlErrValidNode(
                                ctxt, elem, XML_DTD_ELEM_NAMESPACE, "Element %s namespace name for %s does not match the DTD\n", elem->name, ns->prefix, NULL);
                            ret = 0;
                        }
                        goto found;
                    }
