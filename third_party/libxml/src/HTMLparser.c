...
static void htmlErrMemory(xmlParserCtxtPtr ctxt, const char* extra)
{
    ...
    if (extra)
        __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL, NULL, 0, 0,
            "Memory allocation failed : %s\n", extra);
    else
        __xmlRaiseError(
            NULL, NULL, NULL, ctxt, NULL, XML_FROM_PARSER, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "Memory allocation failed\n");
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) htmlParseErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, const xmlChar* str1, const xmlChar* str2)
{
    if ((ctxt != NULL) && (ctxt->disableSAX != 0) && (ctxt->instate == XML_PARSER_EOF))
        return;
    if (ctxt != NULL)
        ctxt->errNo = error;
    __xmlRaiseError(
        NULL, NULL, NULL, ctxt, NULL, XML_FROM_HTML, error, XML_ERR_ERROR, NULL, 0, (const char*)str1, (const char*)str2, NULL, 0, 0, msg, str1, str2);
    if (ctxt != NULL)
        ctxt->wellFormed = 0;
}
 ...
 * Handle a fatal parser error, i.e. violating Well-Formedness constraints
 */
static void LIBXML_ATTR_FORMAT(3, 0) htmlParseErrInt(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, int val)
{
    if ((ctxt != NULL) && (ctxt->disableSAX != 0) && (ctxt->instate == XML_PARSER_EOF))
        return;
    if (ctxt != NULL)
        ctxt->errNo = error;
    __xmlRaiseError(NULL, NULL, NULL, ctxt, NULL, XML_FROM_HTML, error, XML_ERR_ERROR, NULL, 0, NULL, NULL, NULL, val, 0, msg, val);
    if (ctxt != NULL)
        ctxt->wellFormed = 0;
}
...
static int htmlnamePush(htmlParserCtxtPtr ctxt, const xmlChar* value)
{
    ...
    if (ctxt->nameNr >= ctxt->nameMax) {
        ctxt->nameMax *= 2;
        ctxt->nameTab = (const xmlChar**)xmlRealloc((xmlChar**)ctxt->nameTab, ctxt->nameMax * sizeof(ctxt->nameTab[0]));
...
static int htmlNodeInfoPush(htmlParserCtxtPtr ctxt, htmlParserNodeInfo* value)
{
    if (ctxt->nodeInfoNr >= ctxt->nodeInfoMax) {
        if (ctxt->nodeInfoMax == 0)
            ctxt->nodeInfoMax = 5;
        ctxt->nodeInfoMax *= 2;
        ctxt->nodeInfoTab = (htmlParserNodeInfo*)xmlRealloc((htmlParserNodeInfo*)ctxt->nodeInfoTab, ctxt->nodeInfoMax * sizeof(ctxt->nodeInfoTab[0]));
...
#define SHRINK                                                                                                                                                 \
    if ((ctxt->input->cur - ctxt->input->base > 2 * INPUT_CHUNK) && (ctxt->input->end - ctxt->input->cur < 2 * INPUT_CHUNK))                                   \
...
static xmlChar* htmlFindEncoding(xmlParserCtxtPtr ctxt)
{
    ...
    if ((ctxt == NULL) || (ctxt->input == NULL) || (ctxt->input->encoding != NULL) || (ctxt->input->buf == NULL) || (ctxt->input->buf->encoder != NULL))
        return (NULL);
    ...
    if (ctxt->charset != XML_CHAR_ENCODING_UTF8) {
        ...
        if (guess == NULL) {
            ...
            if (handler != NULL) {
                ...
                htmlParseErr(ctxt, XML_ERR_INVALID_ENCODING, "Unsupported encoding %s", guess, NULL);
            }
        }
...
static const char* const a_attrs[]
    = { ATTRS, "charset", "type", "name", "href", "hreflang", "rel", "rev", "accesskey", "shape", "coords", "tabindex", "onfocus", "onblur", NULL };
...
static const char* const form_contents[]
    = { HEADING, LIST, INLINE, "pre", "p", "div", "center", "noscript", "noframes", "blockquote", "isindex", "hr", "table", "fieldset", "address", NULL };
...
static const char* const embed_attrs[] = { COREATTRS, "align", "alt", "border", "code", "codebase", "frameborder", "height", "hidden", "hspace", "name",
    "palette", "pluginspace", "pluginurl", "src", "type", "units", "vspace", "width", NULL };
static const char* const input_attrs[] = { ATTRS, "type", "name", "value", "checked", "disabled", "readonly", "size", "maxlength", "src", "alt", "usemap",
    "ismap", "tabindex", "accesskey", "onfocus", "onblur", "onselect", "onchange", "accept", NULL };
...
static const char* const object_attrs[]
    = { ATTRS, "declare", "classid", "codebase", "data", "type", "codetype", "archive", "standby", "height", "width", "usemap", "name", "tabindex", NULL };
...
static const char* const textarea_attrs[]
    = { ATTRS, "name", "disabled", "readonly", "tabindex", "accesskey", "onfocus", "onblur", "onselect", "onchange", NULL };
...
static const htmlElemDesc html40ElementTable[] = { { "a", 0, 0, 0, 0, 0, 0, 1, "anchor ", DECL html_inline, NULL, DECL a_attrs, DECL target_attr, NULL },
    ...
    { "noframes", 0, 0, 0, 0, 0, 2, 0, "alternate content container for non frame-based rendering ", DECL noframes_content, "body", DECL html_attrs, NULL,
        NULL },
 ...
 */
static const htmlStartCloseEntry htmlStartClose[] = { { "a", "a" }, { "a", "fieldset" }, { "a", "table" }, { "a", "td" }, { "a", "th" }, { "address", "dd" },
    { "address", "dl" }, { "address", "dt" }, { "address", "form" }, { "address", "li" }, { "address", "ul" }, { "b", "center" }, { "b", "p" }, { "b", "td" },
    { "b", "th" }, { "big", "p" }, { "caption", "col" }, { "caption", "colgroup" }, { "caption", "tbody" }, { "caption", "tfoot" }, { "caption", "thead" },
    { "caption", "tr" }, { "col", "col" }, { "col", "colgroup" }, { "col", "tbody" }, { "col", "tfoot" }, { "col", "thead" }, { "col", "tr" },
    { "colgroup", "colgroup" }, { "colgroup", "tbody" }, { "colgroup", "tfoot" }, { "colgroup", "thead" }, { "colgroup", "tr" }, { "dd", "dt" },
    { "dir", "dd" }, { "dir", "dl" }, { "dir", "dt" }, { "dir", "form" }, { "dir", "ul" }, { "dl", "form" }, { "dl", "li" }, { "dt", "dd" }, { "dt", "dl" },
    { "font", "center" }, { "font", "td" }, { "font", "th" }, { "form", "form" }, { "h1", "fieldset" }, { "h1", "form" }, { "h1", "li" }, { "h1", "p" },
    { "h1", "table" }, { "h2", "fieldset" }, { "h2", "form" }, { "h2", "li" }, { "h2", "p" }, { "h2", "table" }, { "h3", "fieldset" }, { "h3", "form" },
    { "h3", "li" }, { "h3", "p" }, { "h3", "table" }, { "h4", "fieldset" }, { "h4", "form" }, { "h4", "li" }, { "h4", "p" }, { "h4", "table" },
    { "h5", "fieldset" }, { "h5", "form" }, { "h5", "li" }, { "h5", "p" }, { "h5", "table" }, { "h6", "fieldset" }, { "h6", "form" }, { "h6", "li" },
    { "h6", "p" }, { "h6", "table" }, { "head", "a" }, { "head", "abbr" }, { "head", "acronym" }, { "head", "address" }, { "head", "b" }, { "head", "bdo" },
    { "head", "big" }, { "head", "blockquote" }, { "head", "body" }, { "head", "br" }, { "head", "center" }, { "head", "cite" }, { "head", "code" },
    { "head", "dd" }, { "head", "dfn" }, { "head", "dir" }, { "head", "div" }, { "head", "dl" }, { "head", "dt" }, { "head", "em" }, { "head", "fieldset" },
    { "head", "font" }, { "head", "form" }, { "head", "frameset" }, { "head", "h1" }, { "head", "h2" }, { "head", "h3" }, { "head", "h4" }, { "head", "h5" },
    { "head", "h6" }, { "head", "hr" }, { "head", "i" }, { "head", "iframe" }, { "head", "img" }, { "head", "kbd" }, { "head", "li" }, { "head", "listing" },
    { "head", "map" }, { "head", "menu" }, { "head", "ol" }, { "head", "p" }, { "head", "pre" }, { "head", "q" }, { "head", "s" }, { "head", "samp" },
    { "head", "small" }, { "head", "span" }, { "head", "strike" }, { "head", "strong" }, { "head", "sub" }, { "head", "sup" }, { "head", "table" },
    { "head", "tt" }, { "head", "u" }, { "head", "ul" }, { "head", "var" }, { "head", "xmp" }, { "hr", "form" }, { "i", "center" }, { "i", "p" }, { "i", "td" },
    { "i", "th" }, { "legend", "fieldset" }, { "li", "li" }, { "link", "body" }, { "link", "frameset" }, { "listing", "dd" }, { "listing", "dl" },
    { "listing", "dt" }, { "listing", "fieldset" }, { "listing", "form" }, { "listing", "li" }, { "listing", "table" }, { "listing", "ul" }, { "menu", "dd" },
    { "menu", "dl" }, { "menu", "dt" }, { "menu", "form" }, { "menu", "ul" }, { "ol", "form" }, { "ol", "ul" }, { "option", "optgroup" },
    { "option", "option" }, { "p", "address" }, { "p", "blockquote" }, { "p", "body" }, { "p", "caption" }, { "p", "center" }, { "p", "col" },
    { "p", "colgroup" }, { "p", "dd" }, { "p", "dir" }, { "p", "div" }, { "p", "dl" }, { "p", "dt" }, { "p", "fieldset" }, { "p", "form" }, { "p", "frameset" },
    { "p", "h1" }, { "p", "h2" }, { "p", "h3" }, { "p", "h4" }, { "p", "h5" }, { "p", "h6" }, { "p", "head" }, { "p", "hr" }, { "p", "li" }, { "p", "listing" },
    { "p", "menu" }, { "p", "ol" }, { "p", "p" }, { "p", "pre" }, { "p", "table" }, { "p", "tbody" }, { "p", "td" }, { "p", "tfoot" }, { "p", "th" },
    { "p", "title" }, { "p", "tr" }, { "p", "ul" }, { "p", "xmp" }, { "pre", "dd" }, { "pre", "dl" }, { "pre", "dt" }, { "pre", "fieldset" }, { "pre", "form" },
    { "pre", "li" }, { "pre", "table" }, { "pre", "ul" }, { "s", "p" }, { "script", "noscript" }, { "small", "p" }, { "span", "td" }, { "span", "th" },
    { "strike", "p" }, { "style", "body" }, { "style", "frameset" }, { "tbody", "tbody" }, { "tbody", "tfoot" }, { "td", "tbody" }, { "td", "td" },
    { "td", "tfoot" }, { "td", "th" }, { "td", "tr" }, { "tfoot", "tbody" }, { "th", "tbody" }, { "th", "td" }, { "th", "tfoot" }, { "th", "th" },
    { "th", "tr" }, { "thead", "tbody" }, { "thead", "tfoot" }, { "title", "body" }, { "title", "frameset" }, { "tr", "tbody" }, { "tr", "tfoot" },
    { "tr", "tr" }, { "tt", "p" }, { "u", "p" }, { "u", "td" }, { "u", "th" }, { "ul", "address" }, { "ul", "form" }, { "ul", "menu" }, { "ul", "ol" },
    { "ul", "pre" }, { "xmp", "dd" }, { "xmp", "dl" }, { "xmp", "dt" }, { "xmp", "fieldset" }, { "xmp", "form" }, { "xmp", "li" }, { "xmp", "table" },
    { "xmp", "ul" } };
 ...
 */
static const char* const htmlScriptAttributes[] = { "onclick", "ondblclick", "onmousedown", "onmouseup", "onmouseover", "onmousemove", "onmouseout",
    "onkeypress", "onkeydown", "onkeyup", "onload", "onunload", "onfocus", "onblur", "onsubmit", "onreset", "onchange", "onselect" };
...
static const elementPriority htmlEndPriority[] = {
    { "div", 150 }, { "td", 160 }, { "th", 160 }, { "tr", 170 }, { "thead", 180 }, { "tbody", 180 }, { "tfoot", 180 }, { "table", 190 }, { "head", 200 },
...
const htmlElemDesc* htmlTagLookup(const xmlChar* tag)
{
    ...
    return ((const htmlElemDesc*)bsearch(tag, html40ElementTable, sizeof(html40ElementTable) / sizeof(htmlElemDesc), sizeof(htmlElemDesc), htmlCompareTags));
}
...
static int htmlGetEndPriority(const xmlChar* name)
{
    ...
    while ((htmlEndPriority[i].name != NULL) && (!xmlStrEqual((const xmlChar*)htmlEndPriority[i].name, name)))
        i++;
...
static int htmlCheckAutoClose(const xmlChar* newtag, const xmlChar* oldtag)
{
    ...
    res = bsearch(&key, htmlStartClose, sizeof(htmlStartClose) / sizeof(htmlStartCloseEntry), sizeof(htmlStartCloseEntry), htmlCompareStartClose);
    return (res != NULL);
}
...
{
    while ((newtag != NULL) && (ctxt->name != NULL) && (htmlCheckAutoClose(newtag, ctxt->name))) {
        if ((ctxt->sax != NULL) && (ctxt->sax->endElement != NULL))
            ctxt->sax->endElement(ctxt->userData, ctxt->name);
        htmlnamePop(ctxt);
    }
    ...
    while ((newtag == NULL) && (ctxt->name != NULL)
        && ((xmlStrEqual(ctxt->name, BAD_CAST "head")) || (xmlStrEqual(ctxt->name, BAD_CAST "body")) || (xmlStrEqual(ctxt->name, BAD_CAST "html")))) {
        if ((ctxt->sax != NULL) && (ctxt->sax->endElement != NULL))
            ctxt->sax->endElement(ctxt->userData, ctxt->name);
        htmlnamePop(ctxt);
    }
...
static void htmlCheckImplied(htmlParserCtxtPtr ctxt, const xmlChar* newtag)
{
    ...
    if ((xmlStrEqual(newtag, BAD_CAST "body")) || (xmlStrEqual(newtag, BAD_CAST "head")))
        return;
    if ((ctxt->nameNr <= 1)
        && ((xmlStrEqual(newtag, BAD_CAST "script")) || (xmlStrEqual(newtag, BAD_CAST "style")) || (xmlStrEqual(newtag, BAD_CAST "meta"))
            || (xmlStrEqual(newtag, BAD_CAST "link")) || (xmlStrEqual(newtag, BAD_CAST "title")) || (xmlStrEqual(newtag, BAD_CAST "base")))) {
        if (ctxt->html >= 3) {
    ...
    } else if ((!xmlStrEqual(newtag, BAD_CAST "noframes")) && (!xmlStrEqual(newtag, BAD_CAST "frame")) && (!xmlStrEqual(newtag, BAD_CAST "frameset"))) {
        if (ctxt->html >= 10) {
 ...
 */
static const char* allowPCData[] = { "a", "abbr", "acronym", "address", "applet", "b", "bdo", "big", "blockquote", "body", "button", "caption", "center",
    "cite", "code", "dd", "del", "dfn", "div", "dt", "em", "font", "form", "h1", "h2", "h3", "h4", "h5", "h6", "i", "iframe", "ins", "kbd", "label", "legend",
    "li", "noframes", "noscript", "object", "p", "pre", "q", "s", "samp", "small", "span", "strike", "strong", "td", "th", "tt", "u", "var" };
...
static void htmlSkipBogusComment(htmlParserCtxtPtr ctxt)
{
    int c;
...
static const xmlChar* htmlParseHTMLName_nonInvasive(htmlParserCtxtPtr ctxt)
{
    ...
    while ((i < HTML_PARSER_BUFFER_SIZE)
        && ((IS_ASCII_LETTER(NXT(1 + i))) || (IS_ASCII_DIGIT(NXT(1 + i))) || (NXT(1 + i) == ':') || (NXT(1 + i) == '-') || (NXT(1 + i) == '_'))) {
...
static const xmlChar* htmlParseNameComplex(xmlParserCtxtPtr ctxt)
{
    ...
    if (ctxt->input->cur - ctxt->input->base < len) {
        /* Sanity check */
        htmlParseErr(ctxt, XML_ERR_INTERNAL_ERROR, "unexpected change of input buffer", NULL, NULL);
        return (NULL);
    }
...
static xmlChar* htmlParseSystemLiteral(htmlParserCtxtPtr ctxt)
{
    ...
    if ((CUR != '"') && (CUR != '\'')) {
        htmlParseErr(ctxt, XML_ERR_LITERAL_NOT_STARTED, "SystemLiteral \" or ' expected\n", NULL, NULL);
        return (NULL);
    }
    quote = CUR;
    NEXT;
    ...
    while ((CUR != 0) && (CUR != quote)) {
        /* TODO: Handle UTF-8 */
        if (!IS_CHAR_CH(CUR)) {
            htmlParseErrInt(ctxt, XML_ERR_INVALID_CHAR, "Invalid char in SystemLiteral 0x%X\n", CUR);
            err = 1;
        }
        NEXT;
        len++;
...
static void htmlParseScript(htmlParserCtxtPtr ctxt)
{
    ...
    while (cur != 0) {
        if ((cur == '<') && (NXT(1) == '/')) {
             ...
             */
            if (ctxt->recovery) {
                if (xmlStrncasecmp(ctxt->name, ctxt->input->cur + 2, xmlStrlen(ctxt->name)) == 0) {
                    break; /* while */
                } else {
...
static void htmlParseCharDataInternal(htmlParserCtxtPtr ctxt, int readahead)
{
    ...
    SHRINK;
    cur = CUR_CHAR(l);
    while (((cur != '<') || (ctxt->token == '<')) && ((cur != '&') || (ctxt->token == '&')) && (cur != 0)) {
        if (!(IS_CHAR(cur))) {
            htmlParseErrInt(ctxt, XML_ERR_INVALID_CHAR, "Invalid char in CDATA 0x%X\n", cur);
        } else {
            COPY_BUF(l, buf, nbchar, cur);
...
static void htmlParsePI(htmlParserCtxtPtr ctxt)
{
	 ...
	 */
		 ...
		 */
            ...
            htmlParseErr(ctxt, XML_ERR_PI_NOT_STARTED, "PI is not started correctly", NULL, NULL);
        }
        ctxt->instate = state;
    }
}
...
static void htmlParseComment(htmlParserCtxtPtr ctxt)
{
    ...
    while ((cur != 0) && ((cur != '>') || (r != '-') || (q != '-'))) {
        ...
        if ((q == '-') && (r == '-') && (cur == '!') && (next == '>')) {
            htmlParseErr(ctxt, XML_ERR_COMMENT_NOT_FINISHED, "Comment incorrectly closed by '--!>'", NULL, NULL);
            cur = '>';
            break;
        }
        ...
        if (IS_CHAR(q)) {
            ...
            htmlParseErrInt(ctxt, XML_ERR_INVALID_CHAR, "Invalid char in comment 0x%X\n", q);
        }
...
static const xmlChar* htmlParseAttribute(htmlParserCtxtPtr ctxt, xmlChar** value)
{
    const xmlChar* name;
    xmlChar* val = NULL;
 ...
 * it and pass UTF8
 */
static void htmlCheckEncodingDirect(htmlParserCtxtPtr ctxt, const xmlChar* encoding)
{

    if ((ctxt == NULL) || (encoding == NULL) || (ctxt->options & HTML_PARSE_IGNORE_ENC))
        return;
	 ...
	 */
        ...
        if ((ctxt->input->buf != NULL) && (ctxt->input->buf->encoder != NULL) && (ctxt->input->buf->raw != NULL) && (ctxt->input->buf->buffer != NULL)) {
            int nbchars;
            int processed;
...
static void htmlCheckMeta(htmlParserCtxtPtr ctxt, const xmlChar** atts)
{
    ...
    while (att != NULL) {
        value = atts[i++];
        if ((value != NULL) && (!xmlStrcasecmp(att, BAD_CAST "http-equiv")) && (!xmlStrcasecmp(value, BAD_CAST "Content-Type")))
            http = 1;
...
static int htmlParseStartTag(htmlParserCtxtPtr ctxt)
{
    ...
    if (name == NULL) {
        ...
        while ((CUR != 0) && (CUR != '>') && (ctxt->instate != XML_PARSER_EOF))
            NEXT;
        return -1;
    }
    ...
    while ((CUR != 0) && (CUR != '>') && ((CUR != '/') || (NXT(1) != '>'))) {
	     ...
	     * the end of the tag. */
            while ((CUR != 0) && !(IS_BLANK_CH(CUR)) && (CUR != '>') && ((CUR != '/') || (NXT(1) != '>')))
                NEXT;
        }
...
static int htmlParseEndTag(htmlParserCtxtPtr ctxt)
{
     ...
     * out now.
     */
    if ((ctxt->depth > 0) && (xmlStrEqual(name, BAD_CAST "html") || xmlStrEqual(name, BAD_CAST "body") || xmlStrEqual(name, BAD_CAST "head"))) {
        ctxt->depth--;
        return (0);
    }
...
static void htmlParseContent(htmlParserCtxtPtr ctxt)
{
	 ...
	 * the next element
	 */
        if ((ctxt->nameNr > 0) && (depth >= ctxt->nameNr) && (!xmlStrEqual(currentNode, ctxt->name))) {
            if (currentNode != NULL)
                xmlFree(currentNode);
            return;
        }

        if ((CUR != 0) && ((xmlStrEqual(currentNode, BAD_CAST "script")) || (xmlStrEqual(currentNode, BAD_CAST "style")))) {
            /*
	     ...
	     */
             ...
             */
            if ((UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y') && (UPP(7) == 'P') && (UPP(8) == 'E')) {
...
int htmlParseDocument(htmlParserCtxtPtr ctxt)
{
     ...
     * (doctypedecl Misc*)?
     */
    if ((CUR == '<') && (NXT(1) == '!') && (UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y') && (UPP(7) == 'P')
        && (UPP(8) == 'E')) {
        htmlParseDocTypeDecl(ctxt);
    }
    SKIP_BLANKS;
    ...
    if ((!(ctxt->options & HTML_PARSE_NODEFDTD)) && (ctxt->myDoc != NULL)) {
        ...
        if (dtd == NULL)
            ctxt->myDoc->intSubset = xmlCreateIntSubset(
                ctxt->myDoc, BAD_CAST "html", BAD_CAST "-//W3C//DTD HTML 4.0 Transitional//EN", BAD_CAST "http://www.w3.org/TR/REC-html40/loose.dtd");
    }
    if (!ctxt->wellFormed)
        return (-1);
    return (0);
...
htmlParserCtxtPtr htmlNewSAXParserCtxt(const htmlSAXHandler* sax, void* userData)
{
 ...
 * Returns the new parser context or NULL
 */
static htmlParserCtxtPtr htmlCreateDocParserCtxt(const xmlChar* cur, const char* encoding)
{
    int len;
 ...
 */
static int htmlParseLookupSequence(htmlParserCtxtPtr ctxt, xmlChar first, xmlChar next, xmlChar third, int ignoreattrval)
{
    int base, len;
...
#ifdef DEBUG_PUSH
            if (next == 0)
                xmlGenericError(xmlGenericErrorContext, "HPP: lookup '%c' found at %d\n", first, base);
            else if (third == 0)
                xmlGenericError(xmlGenericErrorContext, "HPP: lookup '%c%c' found at %d\n", first, next, base);
            else
                xmlGenericError(xmlGenericErrorContext, "HPP: lookup '%c%c%c' found at %d\n", first, next, third, base);
#endif
...
static int htmlParseLookupCommentEnd(htmlParserCtxtPtr ctxt)
{
    ...
    while (mark >= 0) {
        ...
        if ((mark < 0) || (NXT(mark + 2) == '>') || ((NXT(mark + 2) == '!') && (NXT(mark + 3) == '>'))) {
            return mark;
        }
...
static int htmlParseTryOrFinish(htmlParserCtxtPtr ctxt, int terminate)
{
    ...
    while (1) {
        ...
        if (in->buf == NULL)
            ...
            avail = (ptrdiff_t)xmlBufUse(in->buf->buffer) - (in->cur - in->base);
		 ...
		 */
            ...
            cur = in->cur[0];
            next = in->cur[1];
            if ((cur == '<') && (next == '!') && (UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y') && (UPP(7) == 'P')
                && (UPP(8) == 'E')) {
                if ((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 1) < 0))
                    goto done;
#ifdef DEBUG_PUSH
		 ...
		 */
            ...
            if ((cur == '<') && (next == '!') && (in->cur[2] == '-') && (in->cur[3] == '-')) {
                ...
                if ((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 0) < 0))
                    goto done;
#ifdef DEBUG_PUSH
                ...
                htmlParsePI(ctxt);
                ctxt->instate = XML_PARSER_MISC;
            } else if ((cur == '<') && (next == '!') && (UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T') && (UPP(6) == 'Y')
                && (UPP(7) == 'P') && (UPP(8) == 'E')) {
                if ((!terminate) && (htmlParseLookupSequence(ctxt, '>', 0, 0, 1) < 0))
                    goto done;
#ifdef DEBUG_PUSH
		 ...
		 */
                    ...
                    if (ctxt->sax != NULL) {
                        ...
                        if (IS_BLANK_CH(cur)) {
                            if (ctxt->keepBlanks) {
                                ...
                                if (ctxt->sax->ignorableWhitespace != NULL)
                                    ctxt->sax->ignorableWhitespace(ctxt->userData, chr, 1);
                            }
                        } else {
                            htmlCheckParagraph(ctxt);
                            if (ctxt->sax->characters != NULL)
            ...
            cur = in->cur[0];
            next = in->cur[1];
            if ((xmlStrEqual(ctxt->name, BAD_CAST "script")) || (xmlStrEqual(ctxt->name, BAD_CAST "style"))) {
                /*
		     * Handle SCRIPT/STYLE separately
		     */
                if (!terminate) {
...
#endif
                while ((ctxt->instate != XML_PARSER_EOF) && (cur != '<') && (in->cur < in->end)) {
                    if (cur == '&') {
                        htmlParseReference(ctxt);
                    } else {
                        htmlParseCharData(ctxt);
    ...
    if ((!(ctxt->options & HTML_PARSE_NODEFDTD)) && (ctxt->myDoc != NULL)
        && ((terminate) || (ctxt->instate == XML_PARSER_EOF) || (ctxt->instate == XML_PARSER_EPILOG))) {
        xmlDtdPtr dtd;
        dtd = xmlGetIntSubset(ctxt->myDoc);
        if (dtd == NULL)
            ctxt->myDoc->intSubset = xmlCreateIntSubset(
 ...
 */
static htmlDocPtr htmlDoRead(htmlParserCtxtPtr ctxt, const char* URL, const char* encoding, int options, int reuse)
{
    htmlDocPtr ret;
...
htmlDocPtr htmlCtxtReadDoc(htmlParserCtxtPtr ctxt, const xmlChar* cur, const char* URL, const char* encoding, int options)
{
    if (cur == NULL)
        return (NULL);
    return (htmlCtxtReadMemory(ctxt, (const char*)cur, xmlStrlen(cur), URL, encoding, options));
}
 ...
 * Returns the resulting document tree
 */
htmlDocPtr htmlCtxtReadIO(
    htmlParserCtxtPtr ctxt, xmlInputReadCallback ioread, xmlInputCloseCallback ioclose, void* ioctx, const char* URL, const char* encoding, int options)
{
    xmlParserInputBufferPtr input;
    xmlParserInputPtr stream;
