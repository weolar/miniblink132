...
static xmlNodePtr getNext(xmlNodePtr cur, const char* xpath)
{
    xmlNodePtr ret = NULL;
...
static int xsdIncorrectTestCase(xmlNodePtr cur)
{
    xmlNodePtr test;
    xmlBufferPtr buf;
    ...
    xmlRelaxNGSetParserErrors(pctxt, testErrorHandler, testErrorHandler, pctxt);
...
static int xstcTestInstance(xmlNodePtr cur, xmlSchemaPtr schemas, const xmlChar* spath, const char* base)
{
    xmlChar* href = NULL;
    xmlChar* path = NULL;
    ...
    testErrors[0] = 0;
    mem = xmlMemUsed();
    href = getString(cur, "string(ts:instanceDocument/@xlink:href)");
...
static int xstcTestGroup(xmlNodePtr cur, const char* base)
{
    ...
    testErrors[0] = 0;
    mem = xmlMemUsed();
    href = getString(cur, "string(ts:schemaTest/ts:schemaDocument/@xlink:href)");
    if ((href == NULL) || (href[0] == 0)) {
        test_log("testGroup line %ld misses href for schemaDocument\n", xmlGetLineNo(cur));
        ret = -1;
        goto done;
    ...
    validity = getString(cur, "string(ts:schemaTest/ts:expected/@validity)");
    if (validity == NULL) {
    ...
    if (xmlStrEqual(validity, BAD_CAST "valid")) {
		...
		*/
            ...
            instance = getNext(instance, "following-sibling::ts:instanceTest[1]");
        }
    } else if (xmlStrEqual(validity, BAD_CAST "invalid")) {
        nb_schematas++;
        ctxt = xmlSchemaNewParserCtxt((const char*)path);
...
int main(int argc ATTRIBUTE_UNUSED, char** argv ATTRIBUTE_UNUSED)
{
    ...
    if ((nb_errors == old_errors) && (nb_leaks == old_leaks))
        ...
        printf("Ran %d tests (%d schemata), %d errors (%d internals), %d leaks\n", nb_tests - old_tests, nb_schematas, nb_errors - old_errors, nb_internals,
            nb_leaks - old_leaks);
    old_errors = nb_errors;
    old_tests = nb_tests;
    old_leaks = nb_leaks;
