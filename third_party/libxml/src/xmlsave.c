 ...
 */
static int xmlEscapeEntities(unsigned char* out, int* outlen, const xmlChar* in, int* inlen)
{
 ...
 */
static void xmlBufDumpNotationTable(xmlBufPtr buf, xmlNotationTablePtr table)
{
    xmlBufferPtr buffer;
 ...
 * DTD definition
 */
static void xmlBufDumpElementDecl(xmlBufPtr buf, xmlElementPtr elem)
{
    xmlBufferPtr buffer;
 ...
 * DTD definition
 */
static void xmlBufDumpAttributeDecl(xmlBufPtr buf, xmlAttributePtr attr)
{
    xmlBufferPtr buffer;
 ...
 */
static void xmlBufDumpEntityDecl(xmlBufPtr buf, xmlEntityPtr ent)
{
    xmlBufferPtr buffer;
...
static void xmlOutputBufferWriteWSNonSig(xmlSaveCtxtPtr ctxt, int extra)
{
    ...
    for (i = 0; i < (ctxt->level + extra); i += ctxt->indent_nr) {
        xmlOutputBufferWrite(
            ctxt->buf, ctxt->indent_size * ((ctxt->level + extra - i) > ctxt->indent_nr ? ctxt->indent_nr : (ctxt->level + extra - i)), ctxt->indent);
    }
}
...
static void xmlDtdDumpOutput(xmlSaveCtxtPtr ctxt, xmlDtdPtr dtd)
{
    ...
    if ((dtd->entities == NULL) && (dtd->elements == NULL) && (dtd->attributes == NULL) && (dtd->notations == NULL) && (dtd->pentities == NULL)) {
        xmlOutputBufferWrite(buf, 1, ">");
        return;
    }
    ...
    if ((dtd->notations != NULL) && ((dtd->doc == NULL) || (dtd->doc->intSubset == dtd))) {
        xmlBufDumpNotationTable(buf->buffer, (xmlNotationTablePtr)dtd->notations);
    }
    format = ctxt->format;
    level = ctxt->level;
    ctxt->format = 0;
...
static void xmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur)
{
    ...
    while (1) {
        ...
        case XML_ELEMENT_NODE:
            ...
            if (cur->children == NULL) {
                ...
                if (ctxt->format == 1) {
                    tmp = cur->children;
                    while (tmp != NULL) {
                        if ((tmp->type == XML_TEXT_NODE) || (tmp->type == XML_CDATA_SECTION_NODE) || (tmp->type == XML_ENTITY_REF_NODE)) {
                            ctxt->format = 0;
                            unformattedNode = cur;
                            break;
                        }
        ...
        while (1) {
            if (cur == root)
                return;
            if ((ctxt->format == 1) && (cur->type != XML_XINCLUDE_START) && (cur->type != XML_XINCLUDE_END))
                xmlOutputBufferWrite(buf, 1, "\n");
            if (cur->next != NULL) {
                cur = cur->next;
                break;
...
static int xmlDocContentDumpOutput(xmlSaveCtxtPtr ctxt, xmlDocPtr cur)
{
    ...
    if (((cur->type == XML_HTML_DOCUMENT_NODE) && ((ctxt->options & XML_SAVE_AS_XML) == 0) && ((ctxt->options & XML_SAVE_XHTML) == 0))
...
static void xhtmlAttrListDumpOutput(xmlSaveCtxtPtr ctxt, xmlAttrPtr cur)
{
    ...
    while (cur != NULL) {
        if ((cur->ns == NULL) && (xmlStrEqual(cur->name, BAD_CAST "id")))
            ...
            lang = cur;
        else if ((cur->ns != NULL) && (xmlStrEqual(cur->name, BAD_CAST "lang")) && (xmlStrEqual(cur->ns->prefix, BAD_CAST "xml")))
            xml_lang = cur;
        else if ((cur->ns == NULL) && ((cur->children == NULL) || (cur->children->content == NULL) || (cur->children->content[0] == 0))
    ...
    if ((name != NULL) && (id == NULL)) {
        if ((parent != NULL) && (parent->name != NULL)
            && ((xmlStrEqual(parent->name, BAD_CAST "a")) || (xmlStrEqual(parent->name, BAD_CAST "p")) || (xmlStrEqual(parent->name, BAD_CAST "div"))
                || (xmlStrEqual(parent->name, BAD_CAST "img")) || (xmlStrEqual(parent->name, BAD_CAST "map")) || (xmlStrEqual(parent->name, BAD_CAST "applet"))
                || (xmlStrEqual(parent->name, BAD_CAST "form")) || (xmlStrEqual(parent->name, BAD_CAST "frame"))
...
static void xhtmlNodeDumpOutput(xmlSaveCtxtPtr ctxt, xmlNodePtr cur)
{
    ...
    while (1) {
        ...
        case XML_ELEMENT_NODE:
            ...
            if ((cur->parent != NULL) && (cur->parent->parent == (xmlNodePtr)cur->doc) && xmlStrEqual(cur->name, BAD_CAST "head")
                    ...
                    if (addmeta == 1) {
                        ...
                        if (ctxt->format == 1) {
                            ...
                            if (xmlIndentTreeOutput)
                                xmlOutputBufferWrite(
                                    buf, ctxt->indent_size * (ctxt->level + 1 > ctxt->indent_nr ? ctxt->indent_nr : ctxt->level + 1), ctxt->indent);
                        }
...
long xmlSaveTree(xmlSaveCtxtPtr ctxt, xmlNodePtr cur)
{
    ...
    if (((cur->type != XML_NAMESPACE_DECL) && (cur->doc != NULL) && (cur->doc->type == XML_HTML_DOCUMENT_NODE) && ((ctxt->options & XML_SAVE_AS_XML) == 0))
        || (ctxt->options & XML_SAVE_AS_HTML)) {
...
void xmlBufAttrSerializeTxtContent(xmlBufPtr buf, xmlDocPtr doc, xmlAttrPtr attr, const xmlChar* string)
{
    ...
    while (*cur != 0) {
        if (*cur == '\n') {
            ...
            cur++;
            base = cur;
        } else if ((*cur >= 0x80) && (cur[1] != 0) && ((doc == NULL) || (doc->encoding == NULL))) {
            /*
...
int xmlSaveFormatFileTo(xmlOutputBufferPtr buf, xmlDocPtr cur, const char* encoding, int format)
{
    ...
    if (buf == NULL)
        return (-1);
    if ((cur == NULL) || ((cur->type != XML_DOCUMENT_NODE) && (cur->type != XML_HTML_DOCUMENT_NODE))) {
        xmlOutputBufferClose(buf);
        return (-1);
    }
    memset(&ctxt, 0, sizeof(ctxt));
