...
static int xmlXPathCmpNodesExt(xmlNodePtr node1, xmlNodePtr node2)
{
    ...
    case XML_ELEMENT_NODE:
        if (node2->type == XML_ELEMENT_NODE) {
            if ((0 > (ptrdiff_t)node1->content) && (0 > (ptrdiff_t)node2->content) && (node1->doc == node2->doc)) {
    ...
    case XML_PI_NODE: {
	    ...
	    */
        ...
        if ((node1 == NULL) || (node1->type != XML_ELEMENT_NODE) || (0 <= (ptrdiff_t)node1->content)) {
            /*
    ...
    case XML_PI_NODE: {
        ...
        if ((node2 == NULL) || (node2->type != XML_ELEMENT_NODE) || (0 <= (ptrdiff_t)node2->content)) {
            node2 = miscNode2;
            precedence2 = 0;
        } else
            misc = 1;
	...
	*/
     ...
     */
    if ((node1->type == XML_ELEMENT_NODE) && (node2->type == XML_ELEMENT_NODE) && (0 > (ptrdiff_t)node1->content) && (0 > (ptrdiff_t)node2->content)
        && (node1->doc == node2->doc)) {
...
static void xmlXPathErrMemory(xmlXPathContextPtr ctxt, const char* extra)
{
    if (ctxt != NULL) {
        ...
        if (extra)
            __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL, NULL, 0, 0,
                "Memory allocation failed : %s\n", extra);
        else
            __xmlRaiseError(
                NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "Memory allocation failed\n");
    }
}
...
void xmlXPathErr(xmlXPathParserContextPtr ctxt, int error)
{
    if ((error < 0) || (error > MAXERRNO))
        error = MAXERRNO;
    if (ctxt == NULL) {
        __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH, error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK, XML_ERR_ERROR, NULL, 0, NULL, NULL,
            NULL, 0, 0, "%s", xmlXPathErrorMessages[error]);
        return;
    }
    ctxt->error = error;
    if (ctxt->context == NULL) {
        __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH, error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK, XML_ERR_ERROR, NULL, 0,
    ...
    if (ctxt->context->error != NULL) {
        ...
        __xmlRaiseError(NULL, NULL, NULL, NULL, ctxt->context->debugNode, XML_FROM_XPATH, error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK, XML_ERR_ERROR,
            NULL, 0, (const char*)ctxt->base, NULL, NULL, ctxt->cur - ctxt->base, 0, "%s", xmlXPathErrorMessages[error]);
    }
}
 ...
 */
static int xmlXPathCheckOpLimit(xmlXPathParserContextPtr ctxt, unsigned long opCount)
{
    xmlXPathContextPtr xpctxt = ctxt->context;

    if ((opCount > xpctxt->opLimit) || (xpctxt->opCount > xpctxt->opLimit - opCount)) {
...
#define OP_LIMIT_EXCEEDED(ctxt, n) ((ctxt->context->opLimit != 0) && (xmlXPathCheckOpLimit(ctxt, n) < 0))
...
typedef enum { NODE_TEST_NONE = 0, NODE_TEST_TYPE = 1, NODE_TEST_PI = 2, NODE_TEST_ALL = 3, NODE_TEST_NS = 4, NODE_TEST_NAME = 5 } xmlXPathTestVal;

typedef enum { NODE_TYPE_NODE = 0, NODE_TYPE_COMMENT = XML_COMMENT_NODE, NODE_TYPE_TEXT = XML_TEXT_NODE, NODE_TYPE_PI = XML_PI_NODE } xmlXPathTypeVal;
...
static void xmlXPathFreeObjectEntry(void* obj, const xmlChar* name);
 ...
 */
static int xmlXPathCompExprAdd(xmlXPathParserContextPtr ctxt, int ch1, int ch2, xmlXPathOp op, int value, int value2, int value3, void* value4, void* value5)
{
...
#define PUSH_FULL_EXPR(op, op1, op2, val, val2, val3, val4, val5) xmlXPathCompExprAdd(ctxt, (op1), (op2), (op), (val), (val2), (val3), (val4), (val5))
#define PUSH_LONG_EXPR(op, val, val2, val3, val4, val5) xmlXPathCompExprAdd(ctxt, ctxt->comp->last, -1, (op), (val), (val2), (val3), (val4), (val5))

#define PUSH_LEAVE_EXPR(op, val, val2) xmlXPathCompExprAdd(ctxt, -1, -1, (op), (val), (val2), 0, NULL, NULL)

#define PUSH_UNARY_EXPR(op, ch, val, val2) xmlXPathCompExprAdd(ctxt, (ch), -1, (op), (val), (val2), 0, NULL, NULL)

#define PUSH_BINARY_EXPR(op, ch1, ch2, val, val2) xmlXPathCompExprAdd(ctxt, (ch1), (ch2), (op), (val), (val2), 0, NULL, NULL)
...
int xmlXPathNodeSetAddNs(xmlNodeSetPtr cur, xmlNodePtr node, xmlNsPtr ns)
{
    ...
    if ((cur == NULL) || (ns == NULL) || (node == NULL) || (ns->type != XML_NAMESPACE_DECL) || (node->type != XML_ELEMENT_NODE))
        return (-1);
     ...
     */
    for (i = 0; i < cur->nodeNr; i++) {
        if ((cur->nodeTab[i] != NULL) && (cur->nodeTab[i]->type == XML_NAMESPACE_DECL) && (((xmlNsPtr)cur->nodeTab[i])->next == (xmlNsPtr)node)
	...
	*/
	 ...
	 */
        ...
        for (j = 0; j < initNr; j++) {
            ...
            if (n1 == n2) {
                ...
                if ((((xmlNsPtr)n1)->next == ((xmlNsPtr)n2)->next) && (xmlStrEqual(((xmlNsPtr)n1)->prefix, ((xmlNsPtr)n2)->prefix))) {
                    skip = 1;
                    break;
                }
            }
	 ...
	 */
        if (val1->nodeMax == 0) {
            ...
            temp = (xmlNodePtr*)xmlRealloc(val1->nodeTab, val1->nodeMax * 2 * sizeof(xmlNodePtr));
            if (temp == NULL) {
                xmlXPathErrMemory(NULL, "merging nodeset\n");
                return (NULL);
            }
 ...
 */
static xmlNodeSetPtr xmlXPathNodeSetMergeAndClear(xmlNodeSetPtr set1, xmlNodeSetPtr set2)
{
    {
        int i, j, initNbSet1;
        xmlNodePtr n1, n2;
	    ...
	    */
            if (set1->nodeMax == 0) {
                ...
                temp = (xmlNodePtr*)xmlRealloc(set1->nodeTab, set1->nodeMax * 2 * sizeof(xmlNodePtr));
                if (temp == NULL) {
                    xmlXPathErrMemory(NULL, "merging nodeset\n");
                    return (NULL);
                }
 ...
 * Returns @set1 once extended or NULL in case of error.
 */
static xmlNodeSetPtr xmlXPathNodeSetMergeAndClearNoDupls(xmlNodeSetPtr set1, xmlNodeSetPtr set2)
{
    {
        int i;
        xmlNodePtr n2;
 ...
 * to 1.
 */
static void xmlXPathNodeSetKeepLast(xmlNodeSetPtr set)
{
    int i;
    xmlNodePtr node;
    ...
    if (val == NULL)
        ...
        if (ret) {
            for (i = 1; i < val->nodeNr; ++i) {
                ...
                if (xmlXPathNodeSetAddUnique(ret->nodesetval, val->nodeTab[i]) < 0)
                    break;
            }
        }
    }
...
int xmlXPathRegisterVariableNS(xmlXPathContextPtr ctxt, const xmlChar* name, const xmlChar* ns_uri, xmlXPathObjectPtr value)
{
    ...
    if (ctxt->varHash == NULL)
        return (-1);
    if (value == NULL)
        return (xmlHashRemoveEntry2(ctxt->varHash, name, ns_uri, xmlXPathFreeObjectEntry));
    return (xmlHashUpdateEntry2(ctxt->varHash, name, ns_uri, (void*)value, xmlXPathFreeObjectEntry));
}
...
int xmlXPathRegisterNs(xmlXPathContextPtr ctxt, const xmlChar* prefix, const xmlChar* ns_uri)
{
    ...
    if (ctxt->nsHash == NULL)
        return (-1);
    if (ns_uri == NULL)
        return (xmlHashRemoveEntry(ctxt->nsHash, prefix, xmlHashDefaultDeallocator));
    return (xmlHashUpdateEntry(ctxt->nsHash, prefix, (void*)xmlStrdup(ns_uri), xmlHashDefaultDeallocator));
}
...
static void xmlXPathFreeObjectEntry(void* obj, const xmlChar* name ATTRIBUTE_UNUSED)
{
...
#define CHECK_CTXT(ctxt)                                                                                                                                       \
    if (ctxt == NULL) {                                                                                                                                        \
        __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_XPATH, XML_ERR_INTERNAL_ERROR, XML_ERR_FATAL, __FILE__, __LINE__, NULL, NULL, NULL, 0, 0,       \
 ...
 */
static int xmlXPathCompareNodeSetFloat(xmlXPathParserContextPtr ctxt, int inf, int strict, xmlXPathObjectPtr arg, xmlXPathObjectPtr f)
{
    int i, ret = 0;
    xmlNodeSetPtr ns;
    xmlChar* str2;
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathCompareNodeSetString(xmlXPathParserContextPtr ctxt, int inf, int strict, xmlXPathObjectPtr arg, xmlXPathObjectPtr s)
{
    int i, ret = 0;
    xmlNodeSetPtr ns;
    xmlChar* str2;
 ...
 * Returns 0 or 1 depending on the results of the test.
 */
static int xmlXPathCompareNodeSetValue(xmlXPathParserContextPtr ctxt, int inf, int strict, xmlXPathObjectPtr arg, xmlXPathObjectPtr val)
{
    ...
    if (cur == NULL) {
        ...
        case XML_ENTITY_DECL:
            ...
            if ((ctxt->context->node->parent->type == XML_ELEMENT_NODE)
                && ((ctxt->context->node->parent->name[0] == ' ') || (xmlStrEqual(ctxt->context->node->parent->name, BAD_CAST "fake node libxslt"))))
                return (NULL);
...
xmlNodePtr xmlXPathNextAncestor(xmlXPathParserContextPtr ctxt, xmlNodePtr cur)
{
    ...
    case XML_XINCLUDE_END:
        if (cur->parent == NULL)
            return (NULL);
        if ((cur->parent->type == XML_ELEMENT_NODE) && ((cur->parent->name[0] == ' ') || (xmlStrEqual(cur->parent->name, BAD_CAST "fake node libxslt"))))
            return (NULL);
        return (cur->parent);
    case XML_ATTRIBUTE_NODE: {
...
xmlNodePtr xmlXPathNextFollowing(xmlXPathParserContextPtr ctxt, xmlNodePtr cur)
{
    if ((ctxt == NULL) || (ctxt->context == NULL))
        return (NULL);
    if ((cur != NULL) && (cur->type != XML_ATTRIBUTE_NODE) && (cur->type != XML_NAMESPACE_DECL) && (cur->children != NULL))
        return (cur->children);
...
void xmlXPathRoot(xmlXPathParserContextPtr ctxt)
{
    if ((ctxt == NULL) || (ctxt->context == NULL))
        return;
    valuePush(ctxt, xmlXPathCacheNewNodeSet(ctxt->context, (xmlNodePtr)ctxt->context->doc));
}
...
void xmlXPathSubstringAfterFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (target) {
        ...
        if (point) {
            ...
            xmlBufAdd(target, &str->stringval[offset], xmlStrlen(str->stringval) - offset);
        }
        valuePush(ctxt, xmlXPathCacheNewString(ctxt->context, xmlBufContent(target)));
        xmlBufFree(target);
    }
    ...
    while ((c != ' ') && (c != '>') && (c != '/') && /* test bigname.xml */
	     ...
	     */
            ...
            while ((IS_LETTER(c)) || (IS_DIGIT(c)) || /* test bigname.xml */
                (c == '.') || (c == '-') || (c == '_') || ((qualified) && (c == ':')) || (IS_COMBINING(c)) || (IS_EXTENDER(c))) {
                if (len + 10 > max) {
                    xmlChar* tmp;
...
static void xmlXPathCompNumber(xmlXPathParserContextPtr ctxt)
{
    ...
    if (num == NULL) {
        ctxt->error = XPATH_MEMORY_ERROR;
    } else if (PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_NUMBER, 0, 0, num, NULL) == -1) {
...
static void xmlXPathCompLiteral(xmlXPathParserContextPtr ctxt)
{
    ...
    if (lit == NULL) {
        ctxt->error = XPATH_MEMORY_ERROR;
    } else if (PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_STRING, 0, 0, lit, NULL) == -1) {
            ...
            while (NXT(len) != 0) {
                if (NXT(len) == '/') {
                    ...
                    if (xmlXPathIsNodeType(name)) {
                        ...
                        lc = 1;
#ifdef LIBXML_XPTR_LOCS_ENABLED
                    } else if (ctxt->xptr && xmlStrEqual(name, BAD_CAST "range-to")) {
                        lc = 1;
#endif
                    } else {
#ifdef DEBUG_STEP
 ...
 */
static xmlChar* xmlXPathCompNodeTest(xmlXPathParserContextPtr ctxt, xmlXPathTestVal* test, xmlXPathTypeVal* type, xmlChar** prefix, xmlChar* name)
{
    int blanks;
...
#endif
            if (PUSH_FULL_EXPR(XPATH_OP_COLLECT, op1, ctxt->comp->last, axis, test, type, (void*)prefix, (void*)name) == -1) {
            xmlFree(prefix);
            xmlFree(name);
        }
    }
 ...
 * filtered result.
 */
static void xmlXPathNodeSetFilter(xmlXPathParserContextPtr ctxt, xmlNodeSetPtr set, int filterOpIndex, int minPos, int maxPos, int hasNsNodes)
{
    ...
    */
    ...
    for (i = 0, j = 0, pos = 1; i < set->nodeNr; i++) {
        ...
        */
        if ((node->type != XML_NAMESPACE_DECL) && (node->doc != NULL))
            xpctxt->doc = node->doc;
    ...
    if ((set->nodeMax > XML_NODESET_DEFAULT) && (set->nodeNr < set->nodeMax / 2)) {
        xmlNodePtr* tmp;
        int nodeMax = set->nodeNr;

        if (nodeMax < XML_NODESET_DEFAULT)
            nodeMax = XML_NODESET_DEFAULT;
        tmp = (xmlNodePtr*)xmlRealloc(set->nodeTab, nodeMax * sizeof(xmlNodePtr));
        if (tmp == NULL) {
 ...
 */
static void xmlXPathLocationSetFilter(xmlXPathParserContextPtr ctxt, xmlLocationSetPtr locset, int filterOpIndex, int minPos, int maxPos)
{
    xmlXPathContextPtr xpctxt;
    xmlNodePtr oldnode;
    xmlDocPtr olddoc;
    ...
    /* If too many elements were removed, shrink table to preserve memory. */
    if ((locset->locMax > XML_NODESET_DEFAULT) && (locset->locNr < locset->locMax / 2)) {
        xmlXPathObjectPtr* tmp;
        int locMax = locset->locNr;

        if (locMax < XML_NODESET_DEFAULT)
            locMax = XML_NODESET_DEFAULT;
        tmp = (xmlXPathObjectPtr*)xmlRealloc(locset->locTab, locMax * sizeof(xmlXPathObjectPtr));
        if (tmp == NULL) {
 ...
 * in the filtered result.
 */
static void xmlXPathCompOpEvalPredicate(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNodeSetPtr set, int minPos, int maxPos, int hasNsNodes)
{
    if (op->ch1 != -1) {
        xmlXPathCompExprPtr comp = ctxt->comp;
        /*
	...
	*/
        if (comp->steps[op->ch1].op != XPATH_OP_PREDICATE) {
            xmlGenericError(xmlGenericErrorContext, "xmlXPathCompOpEvalPredicate: Expected a predicate\n");
            XP_ERROR(XPATH_INVALID_OPERAND);
        }
        if (ctxt->context->depth >= XPATH_MAX_RECURSION_DEPTH)
            XP_ERROR(XPATH_RECURSION_LIMIT_EXCEEDED);
        ctxt->context->depth += 1;
        xmlXPathCompOpEvalPredicate(ctxt, &comp->steps[op->ch1], set, 1, set->nodeNr, hasNsNodes);
        ctxt->context->depth -= 1;
        CHECK_ERROR;
    }
...
static int xmlXPathIsPositionalPredicate(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, int* maxPos)
{
    ...
    */
    ...
    if ((exprOp != NULL) && (exprOp->op == XPATH_OP_VALUE) && (exprOp->value4 != NULL) && (((xmlXPathObjectPtr)exprOp->value4)->type == XPATH_NUMBER)) {
...
static int xmlXPathNodeCollectAndTest(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNodePtr* first, xmlNodePtr* last, int toBool)
{
...
#define XP_TEST_HIT_NS                                                                                                                                         \
    ...
    */
    ...
    while (((contextIdx < contextSeq->nodeNr) || (contextNode != NULL)) && (ctxt->error == XPATH_EXPRESSION_OK)) {
	...
	*/
	    ...
	    */
            if (hasPredicateRange != 0)
                ...
                xmlXPathCompOpEvalPredicate(ctxt, predOp, seq, 1, seq->nodeNr, hasNsNodes);
...
static int xmlXPathCompOpEvalFirst(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op, xmlNodePtr* first)
{
    ...
    case XPATH_OP_UNION:
        ...
        CHECK_ERROR0;
        if ((ctxt->value != NULL) && (ctxt->value->type == XPATH_NODESET) && (ctxt->value->nodesetval != NULL) && (ctxt->value->nodesetval->nodeNr >= 1)) {
            /*
		...
		*/
        ...
        arg2 = valuePop(ctxt);
        arg1 = valuePop(ctxt);
        if ((arg1 == NULL) || (arg1->type != XPATH_NODESET) || (arg2 == NULL) || (arg2->type != XPATH_NODESET)) {
            xmlXPathReleaseObject(ctxt->context, arg1);
            xmlXPathReleaseObject(ctxt->context, arg2);
            XP_ERROR0(XPATH_INVALID_TYPE);
        }
        if ((ctxt->context->opLimit != 0)
            && (((arg1->nodesetval != NULL) && (xmlXPathCheckOpLimit(ctxt, arg1->nodesetval->nodeNr) < 0))
                || ((arg2->nodesetval != NULL) && (xmlXPathCheckOpLimit(ctxt, arg2->nodesetval->nodeNr) < 0)))) {
            xmlXPathReleaseObject(ctxt->context, arg1);
            xmlXPathReleaseObject(ctxt->context, arg2);
            break;
        }
    ...
    */
    if ((op->ch1 != -1) && (op->ch2 != -1) && (comp->steps[op->ch1].op == XPATH_OP_SORT) && (comp->steps[op->ch2].op == XPATH_OP_SORT)) {
        ...
        if ((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) && (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0)
            && (comp->steps[f].value4 != NULL) && (xmlStrEqual(comp->steps[f].value4, BAD_CAST "last"))) {
	    ...
	    */
            if ((ctxt->value != NULL) && (ctxt->value->type == XPATH_NODESET) && (ctxt->value->nodesetval != NULL) && (ctxt->value->nodesetval->nodeTab != NULL)
...
static int xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op)
{
    ...
    case XPATH_OP_FUNCTION: {
        ...
        if (op->cache != NULL)
            ...
            if (op->value5 == NULL)
                ...
                if (URI == NULL) {
                    xmlGenericError(
                        xmlGenericErrorContext, "xmlXPathCompOpEval: function %s bound to undefined prefix %s\n", (char*)op->value4, (char*)op->value5);
                    xmlXPathPopFrame(ctxt, frame);
                    ctxt->error = XPATH_UNDEF_PREFIX_ERROR;
                    break;
                }
        ...
        if ((ctxt->error == XPATH_EXPRESSION_OK) && (ctxt->valueNr != ctxt->valueFrame + 1))
    ...
    case XPATH_OP_FILTER: {
		    ...
		    */
            ...
            if ((val != NULL) && (val->type == XPATH_NUMBER) && (val->floatval == 1.0)) {
                ...
                /*
                         ...
                         */
                if ((ctxt->value != NULL) && (ctxt->value->type == XPATH_NODESET) && (ctxt->value->nodesetval != NULL) && (ctxt->value->nodesetval->nodeNr > 1))
                    xmlXPathNodeSetClearFromPos(ctxt->value->nodesetval, 1, 1);
                break;
            }
        }
        /*
        ...
        if ((op->ch1 != -1) && (op->ch2 != -1) && (comp->steps[op->ch1].op == XPATH_OP_SORT) && (comp->steps[op->ch2].op == XPATH_OP_SORT)) {
            ...
            if ((f != -1) && (comp->steps[f].op == XPATH_OP_FUNCTION) && (comp->steps[f].value5 == NULL) && (comp->steps[f].value == 0)
                ...
                /*
                         ...
                         */
                if ((ctxt->value != NULL) && (ctxt->value->type == XPATH_NODESET) && (ctxt->value->nodesetval != NULL)
                    && (ctxt->value->nodesetval->nodeTab != NULL) && (ctxt->value->nodesetval->nodeNr > 1))
                    xmlXPathNodeSetKeepLast(ctxt->value->nodesetval);
                break;
            }
        }
		...
		*/
        ...
        if (ctxt->value->type == XPATH_LOCATIONSET) {
            xmlLocationSetPtr locset = ctxt->value->user;
            xmlXPathLocationSetFilter(ctxt, locset, op->ch2, 1, locset->locNr);
            break;
        }
#endif /* LIBXML_XPTR_LOCS_ENABLED */
    ...
    case XPATH_OP_RANGETO: {
        ...
        if (ctxt->value->type == XPATH_LOCATIONSET) {
            ...
            if ((ctxt->value->user == NULL) || (((xmlLocationSetPtr)ctxt->value->user)->locNr == 0))
                break;
            ...
            for (i = 0; i < oldlocset->locNr; i++) {
                ...
                if (res->type == XPATH_LOCATIONSET) {
                    ...
                    for (j = 0; j < rloc->locNr; j++) {
                        range = xmlXPtrNewRange(oldlocset->locTab[i]->user, oldlocset->locTab[i]->index, rloc->locTab[j]->user2, rloc->locTab[j]->index2);
                        if (range != NULL) {
...
next_node:
    do {
        if (ctxt->opLimit != 0) {
            if (ctxt->opCount >= ctxt->opLimit) {
                xmlGenericError(xmlGenericErrorContext, "XPath operation limit exceeded\n");
    ...
    scan_children:
        ...
        do {
            cur = cur->parent;
            depth--;
            if ((cur == NULL) || (cur == limit) || (cur->type == XML_DOCUMENT_NODE))
                goto done;
            if (cur->type == XML_ELEMENT_NODE) {
                ret = xmlStreamPop(patstream);
            } else if ((eval_all_nodes)
                && ((cur->type == XML_TEXT_NODE) || (cur->type == XML_CDATA_SECTION_NODE) || (cur->type == XML_COMMENT_NODE) || (cur->type == XML_PI_NODE))) {
                ret = xmlStreamPop(patstream);
            }
            if (cur->next != NULL) {
                cur = cur->next;
...
int xmlXPathEvaluatePredicateResult(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr res)
{
    ...
    case XPATH_NUMBER:
        ...
        return ((res->floatval == ctxt->context->proximityPosition) && (!xmlXPathIsNaN(res->floatval))); /* MSC pbm Mark Vakoc !*/
#else
...
static void xmlXPathOptimizeExpression(xmlXPathParserContextPtr pctxt, xmlXPathStepOpPtr op)
{
    ...
    */
    ...
    if ((op->op == XPATH_OP_COLLECT /* 11 */) && (op->ch1 != -1) && (op->ch2 == -1 /* no predicate */)) {
        ...
        if ((prevop->op == XPATH_OP_COLLECT /* 11 */) && ((xmlXPathAxisVal)prevop->value == AXIS_DESCENDANT_OR_SELF) && (prevop->ch2 == -1)
            && ((xmlXPathTestVal)prevop->value2 == NODE_TEST_TYPE) && ((xmlXPathTypeVal)prevop->value3 == NODE_TYPE_NODE)) {
            /*
 ...
 *         the caller has to free the object.
 */
static int xmlXPathCompiledEvalInternal(xmlXPathCompExprPtr comp, xmlXPathContextPtr ctxt, xmlXPathObjectPtr* resObjPtr, int toBool)
{
    xmlXPathParserContextPtr pctxt;
    ...
    if (pctxt->error != XPATH_EXPRESSION_OK) {
        ...
        if (resObj == NULL) {
            if (!toBool)
                xmlGenericError(xmlGenericErrorContext, "xmlXPathCompiledEval: No result on the stack.\n");
        } else if (pctxt->valueNr > 0) {
            xmlGenericError(xmlGenericErrorContext, "xmlXPathCompiledEval: %d object(s) left on the stack.\n", pctxt->valueNr);
        }
    }
...
static void xmlXPathEscapeUriFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (target) {
        for (cptr = str->stringval; *cptr; cptr++) {
            if ((*cptr >= 'A' && *cptr <= 'Z') || (*cptr >= 'a' && *cptr <= 'z') || (*cptr >= '0' && *cptr <= '9') || *cptr == '-' || *cptr == '_'
                || *cptr == '.' || *cptr == '!' || *cptr == '~' || *cptr == '*' || *cptr == '\'' || *cptr == '(' || *cptr == ')'
                || (*cptr == '%' && ((cptr[1] >= 'A' && cptr[1] <= 'F') || (cptr[1] >= 'a' && cptr[1] <= 'f') || (cptr[1] >= '0' && cptr[1] <= '9'))
                    && ((cptr[2] >= 'A' && cptr[2] <= 'F') || (cptr[2] >= 'a' && cptr[2] <= 'f') || (cptr[2] >= '0' && cptr[2] <= '9')))
                || (!escape_reserved
