...
#define UPDATE_COMPAT(buf)                                                                                                                                     \
    if (buf->size < INT_MAX)                                                                                                                                   \
        ...
        buf->compat_size = INT_MAX;                                                                                                                            \
    if (buf->use < INT_MAX)                                                                                                                                    \
        buf->compat_use = buf->use;                                                                                                                            \
    else                                                                                                                                                       \
        buf->compat_use = INT_MAX;
...
int xmlBufGetAllocationScheme(xmlBufPtr buf)
{
    if (buf == NULL) {
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufGetAllocationScheme: buf == NULL\n");
#endif
        return (-1);
    }
    return (buf->alloc);
...
int xmlBufSetAllocationScheme(xmlBufPtr buf, xmlBufferAllocationScheme scheme)
{
...
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufSetAllocationScheme: buf == NULL or in error\n");
#endif
        return (-1);
    }
    ...
    if ((scheme == XML_BUFFER_ALLOC_DOUBLEIT) || (scheme == XML_BUFFER_ALLOC_EXACT) || (scheme == XML_BUFFER_ALLOC_HYBRID)
        || (scheme == XML_BUFFER_ALLOC_IMMUTABLE) || (scheme == XML_BUFFER_ALLOC_BOUNDED)) {
        buf->alloc = scheme;
        if (buf->buffer)
...
void xmlBufFree(xmlBufPtr buf)
{
    if (buf == NULL) {
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufFree: buf == NULL\n");
#endif
        return;
    }
...
static size_t xmlBufGrowInternal(xmlBufPtr buf, size_t len)
{
	 ...
	 */
        if ((buf->use + len + 1 >= XML_MAX_TEXT_LENGTH) || (buf->size >= XML_MAX_TEXT_LENGTH)) {
...
size_t xmlBufDump(FILE* file, xmlBufPtr buf)
{
    ...
    if ((buf == NULL) || (buf->error != 0)) {
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufDump: buf == NULL or in error\n");
#endif
        return (0);
    }
    if (buf->content == NULL) {
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufDump: buf->content == NULL\n");
#endif
        return (0);
    }
    CHECK_COMPAT(buf)
...
int xmlBufAdd(xmlBufPtr buf, const xmlChar* str, int len)
{
    ...
    if (len < -1) {
#ifdef DEBUG_BUFFER
        xmlGenericError(xmlGenericErrorContext, "xmlBufAdd: len < 0\n");
#endif
        return -1;
    }
    if (len == 0)
...
int xmlBufWriteQuotedString(xmlBufPtr buf, const xmlChar* string)
{
    ...
    if (xmlStrchr(string, '\"')) {
        if (xmlStrchr(string, '\'')) {
#ifdef DEBUG_BUFFER
            xmlGenericError(xmlGenericErrorContext, "xmlBufWriteQuotedString: string contains quote and double-quotes !\n");
#endif
            xmlBufCCat(buf, "\"");
            base = cur = string;
            while (*cur != 0) {
