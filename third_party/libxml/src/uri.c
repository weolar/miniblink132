...
{
    if (extra)
        __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_URI, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, extra, NULL, NULL, 0, 0,
            "Memory allocation failed : %s\n", extra);
    else
        __xmlRaiseError(
            NULL, NULL, NULL, NULL, NULL, XML_FROM_URI, XML_ERR_NO_MEMORY, XML_ERR_FATAL, NULL, 0, NULL, NULL, NULL, 0, 0, "Memory allocation failed\n");
}
...
static int xmlParse3986Fragment(xmlURIPtr uri, const char** str)
{
    ...
    while ((ISA_PCHAR(cur)) || (*cur == '/') || (*cur == '?') || (*cur == '[') || (*cur == ']') || ((uri != NULL) && (uri->cleanup & 1) && (IS_UNWISE(cur))))
        NEXT(cur);
    if (uri != NULL) {
...
static int xmlParse3986Query(xmlURIPtr uri, const char** str)
{
    ...
    while ((ISA_PCHAR(cur)) || (*cur == '/') || (*cur == '?') || ((uri != NULL) && (uri->cleanup & 1) && (IS_UNWISE(cur))))
        NEXT(cur);
    if (uri != NULL) {
        if (uri->query != NULL)
            xmlFree(uri->query);
...
xmlChar* xmlSaveUri(xmlURIPtr uri)
{
    ...
    if (uri->opaque != NULL) {
        ...
        if ((uri->server != NULL) || (uri->port == -1)) {
            ...
            if (uri->user != NULL) {
                ...
                while (*p != 0) {
                    ...
                    if ((IS_UNRESERVED(*(p))) || ((*(p) == ';')) || ((*(p) == ':')) || ((*(p) == '&')) || ((*(p) == '=')) || ((*(p) == '+')) || ((*(p) == '$'))
                        || ((*(p) == ',')))
                        ret[len++] = *p++;
                    else {
                        int val = *(unsigned char*)p++;
	     ...
	     */
            if ((uri->scheme != NULL) && (p[0] == '/') && (((p[1] >= 'a') && (p[1] <= 'z')) || ((p[1] >= 'A') && (p[1] <= 'Z'))) && (p[2] == ':')
...
done_cd:
    ...
    while (1) {
         ...
         */
        ++segp;
        if (((cur[0] == '.') && (cur[1] == '.') && (segp == cur + 3)) || ((segp[0] != '.') || (segp[1] != '.') || ((segp[2] != '/') && (segp[2] != '\0')))) {
            cur = segp;
            continue;
        }
    ...
    if (path[0] == '/') {
        cur = path;
        while ((cur[0] == '/') && (cur[1] == '.') && (cur[2] == '.') && ((cur[3] == '/') || (cur[3] == '\0')))
            cur += 3;
...
xmlChar* xmlBuildURI(const xmlChar* URI, const xmlChar* base)
{
    ...
    if (res == NULL)
        goto done;
    if ((ref->scheme == NULL) && (ref->path == NULL) && ((ref->authority == NULL) && (ref->server == NULL))) {
        if (bas->scheme != NULL)
...
xmlChar* xmlBuildRelativeURI(const xmlChar* URI, const xmlChar* base)
{
     ...
     */
    if ((ref->scheme != NULL)
        && ((bas->scheme == NULL) || (xmlStrcmp((xmlChar*)bas->scheme, (xmlChar*)ref->scheme)) || (xmlStrcmp((xmlChar*)bas->server, (xmlChar*)ref->server)))) {
        val = xmlStrdup(URI);
        goto done;
    }
...
#define IS_WINDOWS_PATH(p)                                                                                                                                     \
    ((p != NULL) && (((p[0] >= 'a') && (p[0] <= 'z')) || ((p[0] >= 'A') && (p[0] <= 'Z'))) && (p[1] == ':') && ((p[2] == '/') || (p[2] == '\\')))
