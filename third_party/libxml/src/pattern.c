...
typedef enum { XML_OP_END = 0, XML_OP_ROOT, XML_OP_ELEM, XML_OP_CHILD, XML_OP_ATTR, XML_OP_PARENT, XML_OP_ANCESTOR, XML_OP_NS, XML_OP_ALL } xmlPatOp;
...
static int xmlPatternAdd(xmlPatParserContextPtr ctxt ATTRIBUTE_UNUSED, xmlPatternPtr comp, xmlPatOp op, xmlChar* value, xmlChar* value2)
{
    if (comp->nbStep >= comp->maxStep) {
        xmlStepOpPtr temp;
        temp = (xmlStepOpPtr)xmlRealloc(comp->steps, comp->maxStep * 2 * sizeof(xmlStepOp));
        if (temp == NULL) {
...
static int xmlPatPushState(xmlStepStates* states, int step, xmlNodePtr node)
{
    if ((states->states == NULL) || (states->maxstates <= 0)) {
        ...
        tmp = (xmlStepStatePtr)xmlRealloc(states->states, 2 * states->maxstates * sizeof(xmlStepState));
        if (tmp == NULL)
            return (-1);
        states->states = tmp;
        states->maxstates *= 2;
...
restart:
    for (; i < comp->nbStep; i++) {
        ...
        case XML_OP_CHILD: {
            ...
            if ((node->type != XML_ELEMENT_NODE) && (node->type != XML_DOCUMENT_NODE) && (node->type != XML_HTML_DOCUMENT_NODE))
                goto rollback;
        ...
        case XML_OP_ATTR:
            ...
            continue;
        case XML_OP_PARENT:
            if ((node->type == XML_DOCUMENT_NODE) || (node->type == XML_HTML_DOCUMENT_NODE) || (node->type == XML_NAMESPACE_DECL))
                goto rollback;
            node = node->parent;
            if (node == NULL)
                goto rollback;
	 ...
	 */
	    ...
	    */
		    ...
		    */
            ...
            } else if (((step.ns != NULL) == (ns != NULL)) && (name != NULL) && (step.name[0] == name[0]) && xmlStrEqual(step.name, name)
