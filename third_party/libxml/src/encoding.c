 ...
 * n encoding error
 */
static void LIBXML_ATTR_FORMAT(2, 0) xmlEncodingErr(xmlParserErrors error, const char* msg, const char* val)
{
    __xmlRaiseError(NULL, NULL, NULL, NULL, NULL, XML_FROM_I18N, error, XML_ERR_FATAL, NULL, 0, val, NULL, NULL, 0, 0, msg, val);
}
 ...
 * The value of @outlen after return is the number of octets produced.
 */
static int UTF8Toascii(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    const unsigned char* processed = in;
 ...
 */
static int UTF8ToUTF16LE(unsigned char* outb, int* outlen, const unsigned char* in, int* inlen)
{
 ...
 *     if the transcoding failed.
 */
static int UTF8ToUTF16(unsigned char* outb, int* outlen, const unsigned char* in, int* inlen)
{
    if (in == NULL) {
        /*
 ...
 *     if the transcoding failed.
 */
static int UTF8ToUTF16BE(unsigned char* outb, int* outlen, const unsigned char* in, int* inlen)
{
    unsigned short* out = (unsigned short*)outb;
    const unsigned char* processed = in;
    const unsigned char* const instart = in;
...
xmlCharEncoding xmlDetectCharEncoding(const unsigned char* in, int len)
{
    ...
    if (len >= 4) {
        ...
        if ((in[0] == 0x00) && (in[1] == 0x00) && (in[2] == 0x3C) && (in[3] == 0x00))
            return (XML_CHAR_ENCODING_UCS4_2143);
        if ((in[0] == 0x00) && (in[1] == 0x3C) && (in[2] == 0x00) && (in[3] == 0x00))
...
int xmlAddEncodingAlias(const char* name, const char* alias)
{
    ...
    if (xmlCharEncodingAliases == NULL) {
        ...
        xmlCharEncodingAliases = (xmlCharEncodingAliasPtr)xmlRealloc(xmlCharEncodingAliases, xmlCharEncodingAliasesMax * sizeof(xmlCharEncodingAlias));
    }
    /*
     * Walk down the list looking for a definition of the alias
     */
...
int xmlDelEncodingAlias(const char* alias)
{
    ...
    for (i = 0; i < xmlCharEncodingAliasesNb; i++) {
        if (!strcmp(xmlCharEncodingAliases[i].alias, alias)) {
            ...
            xmlFree((char*)xmlCharEncodingAliases[i].alias);
            xmlCharEncodingAliasesNb--;
            memmove(&xmlCharEncodingAliases[i], &xmlCharEncodingAliases[i + 1], sizeof(xmlCharEncodingAlias) * (xmlCharEncodingAliasesNb - i));
            return (0);
        }
    }
    return (-1);
...
void xmlInitCharEncodingHandlers(void)
{
    ...
    handlers = (xmlCharEncodingHandlerPtr*)xmlMalloc(MAX_ENCODING_HANDLERS * sizeof(xmlCharEncodingHandlerPtr));
 ...
 * The value of @outlen after return is the number of octets produced.
 */
static int xmlIconvWrapper(iconv_t cd, unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
 ...
 * The value of @outlen after return is the number of octets produced.
 */
static int xmlUconvWrapper(uconv_t* cd, int toUnicode, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush)
{
    ...
    if (toUnicode) {
        ...
        ucnv_convertEx(cd->utf8, cd->uconv, &ucv_out, ucv_out + *outlen, &ucv_in, ucv_in + *inlen, cd->pivot_buf, &cd->pivot_source, &cd->pivot_target,
            cd->pivot_buf + ICU_PIVOT_BUF_SIZE, 0, flush, &err);
    } else {
        /* UTF-8 => UTF-16 => encoding */
        ucnv_convertEx(cd->uconv, cd->utf8, &ucv_out, ucv_out + *outlen, &ucv_in, ucv_in + *inlen, cd->pivot_buf, &cd->pivot_source, &cd->pivot_target,
            cd->pivot_buf + ICU_PIVOT_BUF_SIZE, 0, flush, &err);
    }
...
static int xmlEncInputChunk(xmlCharEncodingHandler* handler, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush)
{
    ...
    if (handler->input != NULL) {
        ...
        ret = xmlUconvWrapper(handler->uconv_in, 1, out, outlen, in, inlen, flush);
    }
#endif /* LIBXML_ICU_ENABLED */
    else {
        *outlen = 0;
...
static int xmlEncOutputChunk(xmlCharEncodingHandler* handler, unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    ...
    if (handler->output != NULL) {
        ...
        ret = xmlUconvWrapper(handler->uconv_out, 0, out, outlen, in, inlen, 1);
    }
#endif /* LIBXML_ICU_ENABLED */
    else {
        *outlen = 0;
...
int xmlCharEncFirstLine(xmlCharEncodingHandler* handler, xmlBufferPtr out, xmlBufferPtr in)
{
    ...
    ret = xmlEncInputChunk(handler, &out->content[out->use], &written, in->content, &toconv, 0);
...
int xmlCharEncFirstLineInput(xmlParserInputBufferPtr input, int len)
{
    ...
    c_in = toconv;
    c_out = written;
    ret = xmlEncInputChunk(input->encoder, xmlBufEnd(out), &c_out, xmlBufContent(in), &c_in, 0);
...
int xmlCharEncInput(xmlParserInputBufferPtr input, int flush)
{
    ...
    c_in = toconv;
    c_out = written;
    ret = xmlEncInputChunk(input->encoder, xmlBufEnd(out), &c_out, xmlBufContent(in), &c_in, flush);
    xmlBufShrink(in, c_in);
    xmlBufAddLen(out, c_out);
    if (ret == -1)
        ret = -3;
...
int xmlCharEncInFunc(xmlCharEncodingHandler* handler, xmlBufferPtr out, xmlBufferPtr in)
{
    ...
    ret = xmlEncInputChunk(handler, &out->content[out->use], &written, in->content, &toconv, 1);
    xmlBufferShrink(in, toconv);
    out->use += written;
    out->content[out->use] = 0;
    if (ret == -1)
...
retry:
    ...
    case -2: {
        ...
        /*
             ...
             */
        charrefLen = snprintf((char*)&charref[0], sizeof(charref), "&#%d;", cur);
        xmlBufShrink(in, len);
...
int xmlCharEncCloseFunc(xmlCharEncodingHandler* handler)
{
     ...
     */
    if ((handler_in_list == 0) && ((handler->iconv_out != NULL) || (handler->iconv_in != NULL))) {
        tofree = 1;
...
#endif /* LIBXML_ICONV_ENABLED */
#ifdef LIBXML_ICU_ENABLED
    if ((handler_in_list == 0) && ((handler->uconv_out != NULL) || (handler->uconv_in != NULL))) {
        tofree = 1;
	 ...
	 */
        if (in->end - in->cur > 0) {
            ...
            do {
                toconv = in->end - cur;
                written = 32000;
                ret = xmlEncOutputChunk(handler, &convbuf[0], &written, cur, &toconv);
                if (ret < 0) {
                    if (written > 0)
                        ret = -2;
                    else
 ...
 */
static int UTF8ToISO8859x(unsigned char* out, int* outlen, const unsigned char* in, int* inlen, const unsigned char* const xlattable)
{
    const unsigned char* outstart = out;
    const unsigned char* inend;
    const unsigned char* instart = in;
    ...
    if ((out == NULL) || (outlen == NULL) || (inlen == NULL) || (xlattable == NULL))
        return (-1);
    if (in == NULL) {
        /*
        * initialization nothing to do
 ...
 * The value of @outlen after return is the number of octets produced.
 */
static int ISO8859xToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen, unsigned short const* unicodetable)
{
    unsigned char* outstart = out;
    unsigned char* outend;
    const unsigned char* instart = in;
...
static const unsigned char xmltranscodetable_ISO8859_2[48 + 6 * 64] = { "\x00\x00\x01\x05\x02\x04\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_3[48 + 7 * 64] = { "\x04\x00\x01\x06\x02\x05\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_4[48 + 6 * 64] = { "\x00\x00\x01\x05\x02\x03\x00\x00\x00\x00\x00\x04\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_5[48 + 6 * 64] = { "\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_6[48 + 5 * 64] = { "\x02\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_7[48 + 7 * 64] = { "\x04\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x05\x06"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_8[48 + 7 * 64] = { "\x02\x00\x01\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_9[48 + 5 * 64] = { "\x00\x00\x01\x02\x03\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_10[48 + 7 * 64] = { "\x00\x00\x01\x06\x02\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_11[48 + 6 * 64] = { "\x04\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_13[48 + 7 * 64] = { "\x00\x00\x01\x04\x06\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_14[48 + 10 * 64] = { "\x00\x00\x01\x09\x04\x07\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                          "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_15[48 + 6 * 64] = { "\x00\x00\x01\x05\x00\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static const unsigned char xmltranscodetable_ISO8859_16[48 + 9 * 64] = { "\x00\x00\x01\x08\x02\x03\x00\x00\x07\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                                                                         "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
...
static int ISO8859_2ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_2);
}
static int UTF8ToISO8859_2(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_2);
}

static int ISO8859_3ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_3);
}
static int UTF8ToISO8859_3(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_3);
}

static int ISO8859_4ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_4);
}
static int UTF8ToISO8859_4(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_4);
}

static int ISO8859_5ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_5);
}
static int UTF8ToISO8859_5(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_5);
}

static int ISO8859_6ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_6);
}
static int UTF8ToISO8859_6(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_6);
}

static int ISO8859_7ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_7);
}
static int UTF8ToISO8859_7(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_7);
}

static int ISO8859_8ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_8);
}
static int UTF8ToISO8859_8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_8);
}

static int ISO8859_9ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_9);
}
static int UTF8ToISO8859_9(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_9);
}

static int ISO8859_10ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_10);
}
static int UTF8ToISO8859_10(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_10);
}

static int ISO8859_11ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_11);
}
static int UTF8ToISO8859_11(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_11);
}

static int ISO8859_13ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_13);
}
static int UTF8ToISO8859_13(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_13);
}

static int ISO8859_14ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_14);
}
static int UTF8ToISO8859_14(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_14);
}

static int ISO8859_15ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_15);
}
static int UTF8ToISO8859_15(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return UTF8ToISO8859x(out, outlen, in, inlen, xmltranscodetable_ISO8859_15);
}

static int ISO8859_16ToUTF8(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
    return ISO8859xToUTF8(out, outlen, in, inlen, xmlunicodetable_ISO8859_16);
}
static int UTF8ToISO8859_16(unsigned char* out, int* outlen, const unsigned char* in, int* inlen)
{
