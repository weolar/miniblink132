...
void xmlMallocBreakpoint(void)
{
    xmlGenericError(xmlGenericErrorContext, "xmlMallocBreakpoint reached on block %d\n", xmlMemStopAtBlock);
}
...
void* xmlReallocLoc(void* ptr, size_t size, const char* file, int line)
{
    ...
    if (xmlMemTraceBlockAt == ptr) {
        xmlGenericError(xmlGenericErrorContext, "%p : Realloced(%lu -> %lu) Ok\n", xmlMemTraceBlockAt, (long unsigned)p->mh_size, (long unsigned)size);
        xmlMallocBreakpoint();
    }
    p->mh_tag = MEMTAG;
    p->mh_number = number;
...
error:
    xmlGenericError(xmlGenericErrorContext, "xmlMemFree(%p) error\n", ptr);
    xmlMallocBreakpoint();
    return;
}
...
void xmlMemDisplayLast(FILE* fp, long nbBytes)
{
    ...
    p = memlist;
    while ((p) && (nbBytes > 0)) {
        fprintf(fp, "%-5u  %6lu %6lu ", idx++, p->mh_number, (unsigned long)p->mh_size);
        switch (p->mh_type) {
        case STRDUP_TYPE:
