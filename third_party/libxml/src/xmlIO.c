...
static const char* const IOerr[] = {
    ...
    "flush error", "write error", "no input", "buffer full", "loading error", "not a socket", /* ENOTSOCK */
 ...
 *
 */
static int xmlWrapStatUtf8(const char* path, struct _stat* info)
{
    int retval = -1;
    wchar_t* wPath;
 ...
 * Returns zero
 */
int xmlInputReadCallbackNop(void* context ATTRIBUTE_UNUSED, char* buffer ATTRIBUTE_UNUSED, int len ATTRIBUTE_UNUSED)
{
    return (0);
}
 ...
 * Returns an I/O context or NULL in case of error
 */
static void* xmlXzfileOpen_real(const char* filename)
{
    const char* path = NULL;
    xzFile fd;
 ...
 * Returns the number of bytes written
 */
static int xmlXzfileRead(void* context, char* buffer, int len)
{
    int ret;
 ...
 * Close a compressed I/O channel
 */
static int xmlXzfileClose(void* context)
{
    int ret;
...
static void xmlFreeZMemBuff(xmlZMemBuffPtr buff)
{
    ...
    if (z_err != Z_OK)
        xmlGenericError(xmlGenericErrorContext, "xmlFreeZMemBuff:  Error releasing zlib context:  %d\n", z_err);
#else
...
static void* xmlCreateZMemBuff(int compression)
{
    ...
    z_err = deflateInit2(&buff->zctrl, compression, Z_DEFLATED, DFLT_WBITS, DFLT_MEM_LVL, Z_DEFAULT_STRATEGY);
    if (z_err != Z_OK) {
        xmlChar msg[500];
        xmlFreeZMemBuff(buff);
        buff = NULL;
        xmlStrPrintf(msg, 500, "xmlCreateZMemBuff:  %s %d\n", "Error initializing compression context.  ZLIB error:", z_err);
    ...
    hdr_lgth = snprintf((char*)buff->zbuff, buff->size, "%c%c%c%c%c%c%c%c%c%c", GZ_MAGIC1, GZ_MAGIC2, Z_DEFLATED, 0, 0, 0, 0, 0, 0, LXML_ZLIB_OS_CODE);
...
static int xmlZMemBuffExtend(xmlZMemBuffPtr buff, size_t ext_amt)
{
    ...
    if (cur_used > new_size)
        xmlGenericError(xmlGenericErrorContext, "xmlZMemBuffExtend:  %s\n%s %d bytes.\n", "Buffer overwrite detected during compressed memory",
    ...
    if (tmp_ptr != NULL) {
        ...
        xmlChar msg[500];
        xmlStrPrintf(msg, 500, "xmlZMemBuffExtend:  %s %lu bytes.\n", "Allocation failure extending output buffer to", (unsigned long)new_size);
        xmlIOErr(XML_IO_WRITE, (const char*)msg);
    }
...
static int xmlZMemBuffAppend(xmlZMemBuffPtr buff, const char* src, int len)
{
	...
	*/
        ...
        if (z_err != Z_OK) {
            xmlChar msg[500];
            xmlStrPrintf(msg, 500, "xmlZMemBuffAppend:  %s %d %s - %d", "Compression error while appending", len, "bytes to buffer.  ZLIB error", z_err);
            xmlIOErr(XML_IO_WRITE, (const char*)msg);
            return (-1);
        }
    }
...
void* xmlIOHTTPOpenW(const char* post_uri, int compression ATTRIBUTE_UNUSED)
{
...
static int xmlIOHTTPWrite(void* context, const char* buffer, int len)
{
    ...
    if (len > 0) {
        ...
        if (len < 0) {
            xmlChar msg[500];
            xmlStrPrintf(msg, 500, "xmlIOHTTPWrite:  %s\n%s '%s'.\n", "Error appending to internal buffer.", "Error sending document to URI", ctxt->uri);
            xmlIOErr(XML_IO_WRITE, (const char*)msg);
        }
    }
...
static int xmlIOHTTPCloseWrite(void* context, const char* http_mthd)
{
    ...
    if (http_content == NULL) {
        ...
        if (http_ctxt != NULL) {
            ...
            xmlGenericError(
                xmlGenericErrorContext, "xmlNanoHTTPCloseWrite:  HTTP %s to\n%s returned %d.\n", http_mthd, ctxt->uri, xmlNanoHTTPReturnCode(http_ctxt));
	    ...
	    */
            ...
            if (dump_name != NULL) {
                ...
                if (tst_file != NULL) {
                    xmlGenericError(xmlGenericErrorContext, "Transmitted content saved in file:  %s\n", buffer);
                ...
                tst_file = fopen(buffer, "wb");
                if (tst_file != NULL) {
                    xmlGenericError(xmlGenericErrorContext, "Reply content saved in file:  %s\n", buffer);

                    while ((avail = xmlNanoHTTPRead(http_ctxt, buffer, sizeof(buffer))) > 0) {
...
int xmlRegisterOutputCallbacks(
    xmlOutputMatchCallback matchFunc, xmlOutputOpenCallback openFunc, xmlOutputWriteCallback writeFunc, xmlOutputCloseCallback closeFunc)
{
...
#ifdef LIBXML_FTP_ENABLED
    xmlRegisterInputCallbacks(xmlIOFTPMatch, xmlIOFTPOpen, xmlIOFTPRead, xmlIOFTPClose);
#endif /* LIBXML_FTP_ENABLED */
    xmlInputCallbackInitialized = 1;
}
...
#ifdef LIBXML_HTTP_ENABLED
    xmlRegisterOutputCallbacks(xmlIOHTTPMatch, xmlIOHTTPDfltOpenW, xmlIOHTTPWrite, xmlIOHTTPClosePut);
#endif
...
xmlOutputBufferPtr __xmlOutputBufferCreateFilename(const char* URI, xmlCharEncodingHandlerPtr encoder, int compression ATTRIBUTE_UNUSED)
{
    ...
    if (puri != NULL) {
#ifdef LIBXML_ZLIB_ENABLED
        if ((puri->scheme != NULL) && (!xmlStrEqual(BAD_CAST puri->scheme, BAD_CAST "file")))
            is_file_uri = 0;
#endif
        /*
...
#endif
        for (i = xmlOutputCallbackNr - 1; i >= 0; i--) {
            if ((xmlOutputCallbackTable[i].matchcallback != NULL) && (xmlOutputCallbackTable[i].matchcallback(unescaped) != 0)) {
...
#endif
        for (i = xmlOutputCallbackNr - 1; i >= 0; i--) {
            if ((xmlOutputCallbackTable[i].matchcallback != NULL) && (xmlOutputCallbackTable[i].matchcallback(URI) != 0)) {
#if defined(LIBXML_HTTP_ENABLED) && defined(LIBXML_ZLIB_ENABLED)
                /*  Need to pass compression parameter into HTTP open calls  */
                if (xmlOutputCallbackTable[i].matchcallback == xmlIOHTTPMatch)
...
#ifdef DEBUG_INPUT
    xmlGenericError(xmlGenericErrorContext, "I/O: pushed %d chars, buffer %d/%d\n", nbchars, xmlBufUse(in->buffer), xmlBufLength(in->buffer));
#endif
    return (nbchars);
}
 ...
 */
static int endOfInput(void* context ATTRIBUTE_UNUSED, char* buffer ATTRIBUTE_UNUSED, int len ATTRIBUTE_UNUSED)
{
    return (0);
}
...
#ifdef DEBUG_INPUT
    xmlGenericError(xmlGenericErrorContext, "I/O: read %d chars, buffer %d\n", nbchars, xmlBufUse(in->buffer));
#endif
    return (nbchars);
}
 ...
 */
static int xmlEscapeContent(unsigned char* out, int* outlen, const xmlChar* in, int* inlen)
{
...
int xmlOutputBufferWriteEscape(xmlOutputBufferPtr out, const xmlChar* str, xmlCharEncodingOutputFunc escaping)
{
    ...
    if ((out == NULL) || (out->error) || (str == NULL) || (out->buffer == NULL) || (xmlBufGetAllocationScheme(out->buffer) == XML_BUFFER_ALLOC_IMMUTABLE))
        return (-1);
...
static xmlChar* xmlResolveResourceFromCatalog(const char* URL, const char* ID, xmlParserCtxtPtr ctxt)
{
	 ...
	 * Do a local lookup
	 */
        if ((ctxt != NULL) && (ctxt->catalogs != NULL) && ((pref == XML_CATA_ALLOW_ALL) || (pref == XML_CATA_ALLOW_DOCUMENT))) {
            resource = xmlCatalogLocalResolve(ctxt->catalogs, (const xmlChar*)ID, (const xmlChar*)URL);
        }
        /*
	 * Try a global lookup
	 */
        if ((resource == NULL) && ((pref == XML_CATA_ALLOW_ALL) || (pref == XML_CATA_ALLOW_GLOBAL))) {
...
xmlParserInputPtr xmlNoNetExternalEntityLoader(const char* URL, const char* ID, xmlParserCtxtPtr ctxt)
{
    ...
    if (resource != NULL) {
        if ((!xmlStrncasecmp(BAD_CAST resource, BAD_CAST "ftp://", 6)) || (!xmlStrncasecmp(BAD_CAST resource, BAD_CAST "http://", 7))) {
