 ...
 */
static int ares_inet_net_pton_ipv4(const char* src, unsigned char* dst, size_t size)
{
...
static int ares_inet_pton6(const char* src, unsigned char* dst)
{
    ...
    while ((ch = *src++) != '\0') {
        ...
        if (ch == '.' && ((tp + NS_INADDRSZ) <= endp) && ares_inet_net_pton_ipv4(curtok, tp, NS_INADDRSZ) > 0) {
            tp += NS_INADDRSZ;
            saw_xdigit = 0;
...
static int ares_inet_net_pton_ipv6(const char* src, unsigned char* dst, size_t size)
{
