...
#if !defined(_MSC_VER)
#if defined(HAVE_CORETEXT)
int OpenAndLoadChars(const char* file_name, uint8_t* trans_font, size_t trans_len)
{
    CFDataRef data = CFDataCreate(0, trans_font, trans_len);
    if (!data) {
        std::fprintf(stderr, "OK: font renderer couldn't open the transcoded font: %s\n", file_name);
        return 0;
    }
...
int OpenAndLoadChars(const char* file_name, uint8_t* trans_font, size_t trans_len)
{
    ...
    if (error) {
        std::fprintf(stderr, "OK: FreeType2 couldn't open the transcoded font: %s\n", file_name);
        return 0;
    }
