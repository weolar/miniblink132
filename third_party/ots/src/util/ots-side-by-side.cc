...
int CompareBitmaps(const FT_Bitmap* orig, const FT_Bitmap* trans)
{
    ...
    if (orig->width == trans->width && orig->rows == trans->rows) {
...
int LoadCharThenCompare(FT_Library library, FT_Face orig_face, FT_Face trans_face, int pt, FT_ULong c)
{
...
int SideBySide(FT_Library library, const char* file_name, uint8_t* orig_font, size_t orig_len, uint8_t* trans_font, size_t trans_len)
{
    FT_Face orig_face;
    FT_Error error = FT_New_Memory_Face(library, orig_font, orig_len, 0, &orig_face);
    if (error) {
    ...
    static const size_t kUnicodeRangesLen = sizeof(kUnicodeRanges) / sizeof(kUnicodeRanges[0]);

    for (size_t i = 0; i < kPtsLen; ++i) {
        for (size_t j = 0; j < kUnicodeRangesLen; j += 2) {
            for (int k = 0; k <= kUnicodeRanges[j + 1] - kUnicodeRanges[j]; ++k) {
                int ret = LoadCharThenCompare(library, orig_face, trans_face, kPts[i], kUnicodeRanges[j] + k);
                if (ret > 0) {
...
int main(int argc, char** argv)
{
    ...
    return SideBySide(library, argv[1], orig_font, orig_len, trans_font, trans_len);
}
