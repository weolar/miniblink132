...
public:
    ...
    bool Write(const void* data, size_t length)
    {
        ...
        while (length >= 4) {
            uint32_t tmp;
            std::memcpy(&tmp, reinterpret_cast<const uint8_t*>(data) + offset, sizeof(uint32_t));
            chksum_ += ots_ntohl(tmp);
            length -= 4;
            offset += 4;
        }
        ...
        if (length) {
            ...
            uint32_t tmp = 0;
            std::memcpy(&tmp, reinterpret_cast<const uint8_t*>(data) + offset, length);
            chksum_ += ots_ntohl(tmp);
        }
...
public:
    ...
    virtual TableAction GetTableAction(uint32_t tag OTS_UNUSED)
    {
