...
// Lookup type parsers.
bool ParseSingleSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseMutipleSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseAlternateSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseLigatureSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseContextSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseChainingContextSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseExtensionSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseReverseChainingContextSingleSubstitution(const ots::Font* font, const uint8_t* data, const size_t length);

const ots::LookupSubtableParser::TypeParser kGsubTypeParsers[]
    = { { GSUB_TYPE_SINGLE, ParseSingleSubstitution }, { GSUB_TYPE_MULTIPLE, ParseMutipleSubstitution }, { GSUB_TYPE_ALTERNATE, ParseAlternateSubstitution },
          { GSUB_TYPE_LIGATURE, ParseLigatureSubstitution }, { GSUB_TYPE_CONTEXT, ParseContextSubstitution },
          { GSUB_TYPE_CHANGING_CONTEXT, ParseChainingContextSubstitution }, { GSUB_TYPE_EXTENSION_SUBSTITUTION, ParseExtensionSubstitution },
          { GSUB_TYPE_REVERSE_CHAINING_CONTEXT_SINGLE, ParseReverseChainingContextSingleSubstitution } };

const ots::LookupSubtableParser kGsubLookupSubtableParser = { arraysize(kGsubTypeParsers), GSUB_TYPE_EXTENSION_SUBSTITUTION, kGsubTypeParsers };
...
bool ParseSequenceTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
...
bool ParseMutipleSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    for (unsigned i = 0; i < sequence_count; ++i) {
        ...
        if (!ParseSequenceTable(font, data + offset_sequence, length - offset_sequence, num_glyphs)) {
...
bool ParseAlternateSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
...
bool ParseAlternateSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    for (unsigned i = 0; i < alternate_set_count; ++i) {
        ...
        if (!ParseAlternateSetTable(font, data + offset_alternate_set, length - offset_alternate_set, num_glyphs)) {
...
bool ParseLigatureTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
...
bool ParseLigatureSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < ligature_count; ++i) {
        ...
        if (!ParseLigatureTable(font, data + offset_ligature, length - offset_ligature, num_glyphs)) {
...
bool ParseLigatureSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    for (unsigned i = 0; i < lig_set_count; ++i) {
        ...
        if (!ParseLigatureSetTable(font, data + offset_ligature_set, length - offset_ligature_set, num_glyphs)) {
...
bool ParseContextSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    ots::OpenTypeGSUB* gsub = static_cast<ots::OpenTypeGSUB*>(font->GetTypedTable(OTS_TAG_GSUB));
    if (!gsub) {
        return OTS_FAILURE_MSG("Internal error!");
    }
    return ots::ParseContextSubtable(font, data, length, maxp->num_glyphs, gsub->num_lookups);
}
...
bool ParseChainingContextSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    return ots::ParseChainingContextSubtable(font, data, length, maxp->num_glyphs, gsub->num_lookups);
}
...
bool ParseExtensionSubstitution(const ots::Font* font, const uint8_t* data, const size_t length)
{
    return ots::ParseExtensionSubtable(font, data, length, &kGsubLookupSubtableParser);
}
...
bool OpenTypeGSUB::Parse(const uint8_t* data, size_t length)
{
    ...
    const size_t header_size = (version_minor == 0) ? kGsubHeaderSize_1_0 : kGsubHeaderSize_1_1;
    ...
    if (offset_lookup_list) {
        ...
        if (!ParseLookupListTable(font, data + offset_lookup_list, length - offset_lookup_list, &kGsubLookupSubtableParser, &this->num_lookups)) {
