...
bool ExecuteCharString(ots::OpenTypeCFF& cff, size_t call_depth, const ots::CFFIndex& global_subrs_index, const ots::CFFIndex& local_subrs_index,
    ots::Buffer* cff_table, ots::Buffer* char_string, std::stack<int32_t>* argument_stack, ots::CharStringContext& cs_ctx);

bool ArgumentStackOverflows(std::stack<int32_t>* argument_stack, bool cff2)
{
    if ((cff2 && argument_stack->size() > ots::kMaxCFF2ArgumentStack) || (!cff2 && argument_stack->size() > ots::kMaxCFF1ArgumentStack)) {
        return true;
    }
    return false;
}
...
bool ReadNextNumberFromCharString(ots::Buffer* char_string, int32_t* out_number, bool* out_is_operator)
{
    uint8_t v = 0;
...
bool ExecuteCharStringOperator(ots::OpenTypeCFF& cff, int32_t op, size_t call_depth, const ots::CFFIndex& global_subrs_index,
    const ots::CFFIndex& local_subrs_index, ots::Buffer* cff_table, ots::Buffer* char_string, std::stack<int32_t>* argument_stack,
    ots::CharStringContext& cs_ctx)
{
    ...
    case ots::kCallGSubr: {
        ...
        return ExecuteCharString(cff, call_depth + 1, global_subrs_index, local_subrs_index, cff_table, &char_string_to_jump, argument_stack, cs_ctx);
    }
    ...
    case ots::kVSIndex: {
        ...
        if (argument_stack->top() < 0 || argument_stack->top() >= (int32_t)cff.region_index_count.size()) {
            return OTS_FAILURE();
        }
    ...
    ots::Buffer* cff_table, ots::Buffer* char_string, std::stack<int32_t>* argument_stack, ots::CharStringContext& cs_ctx)
{
    ...
    while (char_string->offset() < length) {
        ...
        if (!ReadNextNumberFromCharString(char_string, &operator_or_operand, &is_operator)) {
            return OTS_FAILURE();
        }
        ...
        */
        ...
        if (!is_operator) {
            ...
            std::fprintf(stderr, "%s\n", CharStringOperatorToString(ots::CharStringOperator(operator_or_operand)));
        }
#endif
        ...
        if (!ExecuteCharStringOperator(
                cff, operator_or_operand, call_depth, global_subrs_index, local_subrs_index, cff_table, char_string, argument_stack, cs_ctx)) {
            return OTS_FAILURE();
        }
    ...
    const ots::CFFIndex** out_local_subrs_to_use)
{
    ...
    *out_local_subrs_to_use = NULL;
    ...
    if ((cff.fd_select.size() > 0) && (!cff.local_subrs_per_font.empty())) {
...
bool ValidateCFFCharStrings(ots::OpenTypeCFF& cff, const CFFIndex& global_subrs_index, Buffer* cff_table)
{
    ...
    for (unsigned i = 1; i < char_strings_index.offsets.size(); ++i) {
        ...
        if (!SelectLocalSubr(cff, glyph_index, &local_subrs_to_use)) {
            return OTS_FAILURE();
        }
        ...
        if (!ExecuteCharString(
                cff, 0 /* initial call_depth is zero */, global_subrs_index, *local_subrs_to_use, cff_table, &char_string, &argument_stack, cs_ctx)) {
            return OTS_FAILURE();
        }
