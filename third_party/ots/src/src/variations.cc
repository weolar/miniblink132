...
bool ParseVariationRegionList(const ots::Font* font, const uint8_t* data, const size_t length, uint16_t* regionCount)
{
    ots::Buffer subtable(data, length);

    uint16_t axisCount;

    if (!subtable.ReadU16(&axisCount) || !subtable.ReadU16(regionCount)) {
    ...
    const ots::OpenTypeFVAR* fvar = static_cast<ots::OpenTypeFVAR*>(font->GetTypedTable(OTS_TAG_FVAR));
    if (!fvar) {
    ...
    for (unsigned i = 0; i < *regionCount; i++) {
        for (unsigned j = 0; j < axisCount; j++) {
            ...
            if (!subtable.ReadS16(&startCoord) || !subtable.ReadS16(&peakCoord) || !subtable.ReadS16(&endCoord)) {
...
bool ParseVariationDataSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t regionCount, uint16_t* regionIndexCount)
{
    ots::Buffer subtable(data, length);
    ...
    if (!subtable.ReadU16(&itemCount) || !subtable.ReadU16(&shortDeltaCount) || !subtable.ReadU16(regionIndexCount)) {
...
bool ParseItemVariationStore(const Font* font, const uint8_t* data, const size_t length, std::vector<uint16_t>* regionIndexCounts)
{
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU32(&variationRegionListOffset) || !subtable.ReadU16(&itemVariationDataCount)) {
    ...
    if (variationRegionListOffset < subtable.offset() + 4 * itemVariationDataCount || variationRegionListOffset > length) {
        return OTS_FAILURE_MSG("Invalid variation region list offset");
    }

    uint16_t regionCount;
    if (!ParseVariationRegionList(font, data + variationRegionListOffset, length - variationRegionListOffset, &regionCount)) {
    ...
    for (unsigned i = 0; i < itemVariationDataCount; i++) {
        ...
        if (!ParseVariationDataSubtable(font, data + offset, length - offset, regionCount, &regionIndexCount)) {
...
bool ParseDeltaSetIndexMap(const Font* font, const uint8_t* data, const size_t length)
{
    ...
    if (!subtable.ReadU16(&entryFormat) || !subtable.ReadU16(&mapCount)) {
...
bool ParseVariationData(const Font* font, const uint8_t* data, size_t length, size_t axisCount, size_t sharedTupleCount)
{
    Buffer subtable(data, length);

    uint16_t tupleVariationCount;
    uint16_t dataOffset;
    if (!subtable.ReadU16(&tupleVariationCount) || !subtable.ReadU16(&dataOffset)) {
    ...
    for (unsigned i = 0; i < tupleVariationCount; i++) {
        ...
        if (!subtable.ReadU16(&variationDataSize) || !subtable.ReadU16(&tupleIndex)) {
