...
bool OpenTypeSILL::Parse(const uint8_t* data, size_t length)
{
    ...
    if (this->numLangs == 0) {
        ...
        if (this->searchRange != (unsigned)std::pow(2, floorLog2) || this->entrySelector != floorLog2
    ...
    for (unsigned long i = 0; i <= this->numLangs; ++i) {
        ...
        for (unsigned j = 0; j < entry.numSettings; ++j) {
            ...
            if (offset < entry.offset || offset > length) {
                return DropGraphite("Invalid LangFeatureSetting offset %zu/%zu", offset, length);
            }
...
{
    if (!out->WriteU32(this->version) || !out->WriteU16(this->numLangs) || !out->WriteU16(this->searchRange) || !out->WriteU16(this->entrySelector)
        || !out->WriteU16(this->rangeShift) || !SerializeParts(this->entries, out) || !SerializeParts(this->settings, out)) {
...
{
    if (!table.ReadU8(&this->langcode[0]) || !table.ReadU8(&this->langcode[1]) || !table.ReadU8(&this->langcode[2]) || !table.ReadU8(&this->langcode[3])) {
...
{
    if (!out->WriteU8(this->langcode[0]) || !out->WriteU8(this->langcode[1]) || !out->WriteU8(this->langcode[2]) || !out->WriteU8(this->langcode[3])
        || !out->WriteU16(this->numSettings) || !out->WriteU16(this->offset)) {
...
{
    OpenTypeFEAT* feat = static_cast<OpenTypeFEAT*>(parent->GetFont()->GetTypedTable(OTS_TAG_FEAT));
    if (!feat) {
        return parent->Error("FeatureDefn: Required Feat table is missing");
    }

    if (!table.ReadU32(&this->featureId) || !feat->IsValidFeatureId(this->featureId)) {
...
{
    if (!out->WriteU32(this->featureId) || !out->WriteS16(this->value) || !out->WriteU16(this->reserved)) {
        return parent->Error("LangFeatureSetting: Failed to read reserved");
    }
    return true;
}
