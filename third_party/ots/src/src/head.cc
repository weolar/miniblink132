...
bool OpenTypeHEAD::Parse(const uint8_t* data, size_t length)
{
    ...
    uint32_t version = 0;
    if (!table.ReadU32(&version) || !table.ReadU32(&this->revision)) {
    ...
    if (this->upem < 16 || this->upem > 16384) {
        return Error("unitsPerEm on in the range [16, 16384]: %d", this->upem);
    }

    if (!table.ReadR64(&this->created) || !table.ReadR64(&this->modified)) {
        return Error("Can't read font dates");
    }

    if (!table.ReadS16(&this->xmin) || !table.ReadS16(&this->ymin) || !table.ReadS16(&this->xmax) || !table.ReadS16(&this->ymax)) {
    ...
    if (this->index_to_loc_format < 0 || this->index_to_loc_format > 1) {
...
{
    if (!out->WriteU32(0x00010000) || !out->WriteU32(this->revision) || !out->WriteU32(0) || // check sum not filled in yet
        !out->WriteU32(0x5F0F3CF5) || !out->WriteU16(this->flags) || !out->WriteU16(this->upem) || !out->WriteR64(this->created)
        || !out->WriteR64(this->modified) || !out->WriteS16(this->xmin) || !out->WriteS16(this->ymin) || !out->WriteS16(this->xmax)
        || !out->WriteS16(this->ymax) || !out->WriteU16(this->mac_style) || !out->WriteU16(this->min_ppem) || !out->WriteS16(2)
        || !out->WriteS16(this->index_to_loc_format) || !out->WriteS16(0)) {
