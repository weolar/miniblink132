...
bool ParseIndex(ots::Buffer& table, ots::CFFIndex& index, bool cff2 = false)
{
    ...
    for (unsigned i = 0; i <= index.count; ++i) { // '<=' is not a typo.
        ...
        index.offsets.push_back(object_data_offset + (rel_offset - 1)); // less than length(), 1GB.
    }
...
bool ParseNameData(ots::Buffer* table, const ots::CFFIndex& index, std::string* out_name)
{
    uint8_t name[256] = { 0 };
    ...
    while (true) {
        ...
        if ((nibble & 0xf0) == 0xf0) {
            if ((nibble & 0xf) == 0xf) {
                ...
                operands.push_back(std::make_pair(static_cast<uint32_t>(0), DICT_OPERAND_REAL));
                return true;
            }
            return OTS_FAILURE();
        }
...
bool ParseDictDataEscapedOperator(ots::Buffer& table, std::vector<Operand>& operands)
{
    uint8_t op = 0;
    if (!table.ReadU8(&op)) {
        return OTS_FAILURE();
...
bool ParseDictDataNumber(ots::Buffer& table, uint8_t b0, std::vector<Operand>& operands)
{
    uint8_t b1 = 0;
    uint8_t b2 = 0;
    uint8_t b3 = 0;
    ...
    case 28: // shortint
        ...
        operands.push_back(std::make_pair(static_cast<uint32_t>((b1 << 8) + b2), DICT_OPERAND_INTEGER));
        return true;
    ...
    case 29: // longint
        ...
        operands.push_back(std::make_pair(static_cast<uint32_t>((b1 << 24) + (b2 << 16) + (b3 << 8) + b4), DICT_OPERAND_INTEGER));
        return true;
...
bool ParseDictDataReadNext(ots::Buffer& table, std::vector<Operand>& operands)
{
    uint8_t op = 0;
    if (!table.ReadU8(&op)) {
        return OTS_FAILURE();
    ...
    if (op <= 24) {
        ...
        operands.push_back(std::make_pair(static_cast<uint32_t>(op), DICT_OPERATOR));
        return true;
...
bool OperandsOverflow(std::vector<Operand>& operands, bool cff2)
{
    ...
    if ((cff2 && operands.size() > ots::kMaxCFF2ArgumentStack) || (!cff2 && operands.size() > ots::kMaxCFF1ArgumentStack)) {
        return true;
    }
    return false;
}

bool ParseDictDataReadOperands(ots::Buffer& dict, std::vector<Operand>& operands, bool cff2)
{
...
bool ParsePrivateDictData(ots::Buffer& table, size_t offset, size_t dict_length, DICT_DATA_TYPE type, ots::OpenTypeCFF* out_cff)
{
    ...
    while (dict.offset() < dict.length()) {
        ...
        case 22: { // vsindex
            ...
            if (vsindex < 0 || vsindex >= (int32_t)out_cff->region_index_count.size()) {
                return OTS_FAILURE();
            }
...
bool ParseVariationStore(ots::OpenTypeCFF& out_cff, ots::Buffer& table)
{
    ...
    if (!ParseItemVariationStore(out_cff.GetFont(), table.buffer() + table.offset(), length, &(out_cff.region_index_count))) {
        return OTS_FAILURE();
    }
...
bool ParseDictData(ots::Buffer& table, ots::Buffer& dict, uint16_t glyphs, size_t sid_max, DICT_DATA_TYPE type, ots::OpenTypeCFF* out_cff);

bool ParseDictData(ots::Buffer& table, const ots::CFFIndex& index, uint16_t glyphs, size_t sid_max, DICT_DATA_TYPE type, ots::OpenTypeCFF* out_cff)
{
    for (unsigned i = 1; i < index.offsets.size(); ++i) {
...
bool ParseDictData(ots::Buffer& table, ots::Buffer& dict, uint16_t glyphs, size_t sid_max, DICT_DATA_TYPE type, ots::OpenTypeCFF* out_cff)
{
    ...
    while (dict.offset() < dict.length()) {
        ...
        case (12U << 8) + 36: { // FDArray
            ...
            if (!ParseDictData(table, sub_dict_index, glyphs, sid_max, DICT_DATA_FDARRAY, out_cff)) {
                return OTS_FAILURE();
            }
                ...
                for (unsigned j = 0; j < n_ranges; ++j) {
                    ...
                    if (j != 0) {
                        for (auto k = last_gid; k < first; ++k) {
                            if (!out_cff->fd_select.insert(std::make_pair(k, fd_index)).second) {
                                return OTS_FAILURE();
                            }
                        }
                    }
        ...
        case 18: {
            ...
            if (!ParsePrivateDictData(table, private_offset, private_length, type, out_cff)) {
                return OTS_FAILURE();
            }
            break;
        }
...
bool OpenTypeCFF::Parse(const uint8_t* data, size_t length)
{
    ...
    uint8_t hdr_size = 0;
    uint8_t off_size = 0;
    if (!table.ReadU8(&major) || !table.ReadU8(&minor) || !table.ReadU8(&hdr_size) || !table.ReadU8(&off_size)) {
    ...
    if (name_index.count != 1 || name_index.offsets.size() != 2) {
        return Error("Name INDEX must contain only one entry, not %d", name_index.count);
    }
    ...
    if (top_dict_index.count != 1) {
        return Error("Top DICT INDEX must contain only one entry, not %d", top_dict_index.count);
    }
    ...
    if (!ParseDictData(table, top_dict_index, num_glyphs, sid_max, DICT_DATA_TOPLEVEL, this)) {
...
bool OpenTypeCFF2::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU8(&major) || !table.ReadU8(&minor) || !table.ReadU8(&hdr_size) || !table.ReadU16(&top_dict_size)) {
        return Error("Failed to read table header");
    }
    ...
    table.set_offset(hdr_size);
    this->charstrings_index = new ots::CFFIndex;
    if (!ParseDictData(table, top_dict, num_glyphs, sid_max, DICT_DATA_TOPLEVEL, this)) {
        return Error("Failed to parse Top DICT Data");
    }
