...
bool OpenTypeGDEF::ParseAttachListTable(const uint8_t* data, size_t length)
{
    ...
    if (!ots::ParseCoverageTable(GetFont(), data + offset_coverage, length - offset_coverage, this->m_num_glyphs)) {
    ...
    for (unsigned i = 0; i < attach_points.size(); ++i) {
        ...
        for (unsigned j = 0; j < point_count; ++j) {
            ...
            if (last_point_index != 0 && last_point_index >= point_index) {
                return Error("bad contour indices: %u >= %u", last_point_index, point_index);
            }
    ...
    for (unsigned i = 0; i < lig_glyphs.size(); ++i) {
        ...
        for (unsigned j = 0; j < caret_count; ++j) {
            ...
            if (caret_format == 3) {
                ...
                uint16_t absolute_offset = lig_glyphs[i] + caret_value_offsets[j] + offset_device;
                if (offset_device == 0 || absolute_offset >= length) {
                    return Error("Bad device offset for caret value %d in glyph %d: %d", j, i, offset_device);
                }
                if (!ots::ParseDeviceTable(GetFont(), data + absolute_offset, length - absolute_offset)) {
                    return Error("Bad device table for caret value %d in glyph %d", j, i, offset_device);
                }
            }
        }
    }
...
bool OpenTypeGDEF::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&version_major) || !table.ReadU16(&version_minor)) {
    ...
    if (!table.ReadU16(&offset_glyph_class_def) || !table.ReadU16(&offset_attach_list) || !table.ReadU16(&offset_lig_caret_list)
    ...
    if (offset_glyph_class_def) {
        ...
        if (!ots::ParseClassDefTable(GetFont(), data + offset_glyph_class_def, length - offset_glyph_class_def, this->m_num_glyphs, kMaxGlyphClassDefValue)) {
    ...
    if (offset_attach_list) {
        ...
        if (!ParseAttachListTable(data + offset_attach_list, length - offset_attach_list)) {
    ...
    if (offset_lig_caret_list) {
        ...
        if (!ParseLigCaretListTable(data + offset_lig_caret_list, length - offset_lig_caret_list)) {
    ...
    if (offset_mark_attach_class_def) {
        ...
        if (!ots::ParseClassDefTable(
                GetFont(), data + offset_mark_attach_class_def, length - offset_mark_attach_class_def, this->m_num_glyphs, kMaxClassDefValue)) {
    ...
    if (offset_mark_glyph_sets_def) {
        ...
        if (!ParseMarkGlyphSetsDefTable(data + offset_mark_glyph_sets_def, length - offset_mark_glyph_sets_def)) {
            return Error("Invalid mark glyph sets");
        }
    }

    if (item_var_store_offset) {
        if (item_var_store_offset >= length || item_var_store_offset < gdef_header_end) {
            return Error("invalid offset to item variation store");
        }
        if (!ParseItemVariationStore(GetFont(), data + item_var_store_offset, length - item_var_store_offset)) {
