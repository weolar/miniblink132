...
bool OpenTypeGLOC::Parse(const uint8_t* data, size_t length)
{
    ...
    if (this->flags & ATTRIB_IDS && this->numAttribs * sizeof(uint16_t) > table.remaining()) {
        return DropGraphite("Failed to calculate length of locations");
    }
    size_t locations_len = (table.remaining() - (this->flags & ATTRIB_IDS ? this->numAttribs * sizeof(uint16_t) : 0))
    ...
    if (this->flags & ATTRIB_IDS) { // attribIds array present
        ...
        for (unsigned i = 0; i < this->numAttribs; ++i) {
            ...
            if (!table.ReadU16(&this->attribIds[i]) || !name->IsValidNameId(this->attribIds[i])) {
