...
{
    OpenTypeNAME* name = static_cast<OpenTypeNAME*>(GetFont()->GetTypedTable(OTS_TAG_NAME));
...
bool OpenTypeSTAT::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&this->majorVersion) || !table.ReadU16(&this->minorVersion) || !table.ReadU16(&this->designAxisSize)
        || !table.ReadU16(&this->designAxisCount) || !table.ReadU32(&this->designAxesOffset) || !table.ReadU16(&this->axisValueCount)
        || !table.ReadU32(&this->offsetToAxisValueOffsets) || !(this->minorVersion < 1 || table.ReadU16(&this->elidedFallbackNameID))) {
    ...
    if (this->designAxisCount == 0) {
        ...
        if (this->designAxesOffset < headerEnd || size_t(this->designAxesOffset) + size_t(this->designAxisCount) * size_t(this->designAxisSize) > length) {
    ...
    for (size_t i = 0; i < this->designAxisCount; i++) {
        ...
        if (!table.ReadU32(&axis.axisTag) || !table.ReadU16(&axis.axisNameID) || !table.ReadU16(&axis.axisOrdering)) {
    ...
    if (this->axisValueCount == 0) {
        ...
        if (this->offsetToAxisValueOffsets < headerEnd || size_t(this->offsetToAxisValueOffsets) + size_t(this->axisValueCount) * sizeof(uint16_t) > length) {
    ...
    for (size_t i = 0; i < this->axisValueCount; i++) {
        ...
        switch (format) {
        case 1:
            if (!table.ReadU16(&axisValue.format1.axisIndex) || !table.ReadU16(&axisValue.format1.flags) || !table.ReadU16(&axisValue.format1.valueNameID)
            ...
            break;
        case 2:
            if (!table.ReadU16(&axisValue.format2.axisIndex) || !table.ReadU16(&axisValue.format2.flags) || !table.ReadU16(&axisValue.format2.valueNameID)
                || !table.ReadS32(&axisValue.format2.nominalValue) || !table.ReadS32(&axisValue.format2.rangeMinValue)
            ...
            if (!(axisValue.format2.rangeMinValue <= axisValue.format2.nominalValue && axisValue.format2.nominalValue <= axisValue.format2.rangeMaxValue)) {
                Warning("Bad axis value range or nominal value");
            }
            break;
        case 3:
            if (!table.ReadU16(&axisValue.format3.axisIndex) || !table.ReadU16(&axisValue.format3.flags) || !table.ReadU16(&axisValue.format3.valueNameID)
                || !table.ReadS32(&axisValue.format3.value) || !table.ReadS32(&axisValue.format3.linkedValue)) {
        ...
        case 4:
            ...
            if (!table.ReadU16(&axisValue.format4.axisCount) || !table.ReadU16(&axisValue.format4.flags) || !table.ReadU16(&axisValue.format4.valueNameID)) {
            ...
            for (unsigned j = 0; j < axisValue.format4.axisCount; j++) {
                ...
                if (!table.ReadU16(&v.axisIndex) || !table.ReadS32(&v.value)) {
...
bool OpenTypeSTAT::Serialize(OTSStream* out)
{
    ...
    if (!out->WriteU16(this->majorVersion) || !out->WriteU16(this->minorVersion) || !out->WriteU16(this->designAxisSize)
        || !out->WriteU16(this->designAxisCount) || !out->WriteU32(this->designAxesOffset) || !out->WriteU16(this->axisValueCount)
        || !out->WriteU32(this->offsetToAxisValueOffsets) || !(this->minorVersion < 1 || out->WriteU16(this->elidedFallbackNameID))) {
    ...
    for (unsigned i = 0; i < this->designAxisCount; i++) {
        ...
        if (!out->WriteU32(axis.axisTag) || !out->WriteU16(axis.axisNameID) || !out->WriteU16(axis.axisOrdering)) {
    ...
    for (unsigned i = 0; i < this->axisValueCount; i++) {
        ...
        switch (value.format) {
        case 1:
            if (!out->WriteU16(value.format1.axisIndex) || !out->WriteU16(value.format1.flags) || !out->WriteU16(value.format1.valueNameID)
                || !out->WriteS32(value.format1.value)) {
                return Error("Failed to write axis value");
            }
            break;
        case 2:
            if (!out->WriteU16(value.format2.axisIndex) || !out->WriteU16(value.format2.flags) || !out->WriteU16(value.format2.valueNameID)
                || !out->WriteS32(value.format2.nominalValue) || !out->WriteS32(value.format2.rangeMinValue) || !out->WriteS32(value.format2.rangeMaxValue)) {
                return Error("Failed to write axis value");
            }
            break;
        case 3:
            if (!out->WriteU16(value.format3.axisIndex) || !out->WriteU16(value.format3.flags) || !out->WriteU16(value.format3.valueNameID)
                || !out->WriteS32(value.format3.value) || !out->WriteS32(value.format3.linkedValue)) {
                return Error("Failed to write axis value");
            }
            break;
        case 4:
            if (!out->WriteU16(value.format4.axisCount) || !out->WriteU16(value.format4.flags) || !out->WriteU16(value.format4.valueNameID)) {
                return Error("Failed to write axis value");
            }
            for (unsigned j = 0; j < value.format4.axisValues.size(); j++) {
                if (!out->WriteU16(value.format4.axisValues[j].axisIndex) || !out->WriteS32(value.format4.axisValues[j].value)) {
                    return Error("Failed to write axis value");
                }
            }
            break;
