    ...
    for (unsigned i = 0; i < this->numFeat; ++i) {
        ...
        for (unsigned j = 0; j < feature.numSettings; ++j) {
            ...
            if (offset < feature.offset || offset > length) {
                return DropGraphite("Invalid FeatSettingDefn offset %zu/%zu", offset, length);
            }
...
{
    if (!out->WriteU32(this->version) || !out->WriteU16(this->numFeat) || !out->WriteU16(this->reserved) || !out->WriteU32(this->reserved2)
        || !SerializeParts(this->features, out) || !SerializeParts(this->featSettings, out)) {
...
bool OpenTypeFEAT::FeatureDefn::ParsePart(Buffer& table)
{
    ...
    if (this->flags & HAS_DEFAULT_SETTING && (this->flags & DEFAULT_SETTING) >= this->numSettings) {
        return parent->Error("FeatureDefn: (flags & 0x%x) is set but (flags & 0x%x "
                             "is not a valid setting index",
            HAS_DEFAULT_SETTING, DEFAULT_SETTING);
    }
...
{
    if ((parent->version >> 16 >= 2 && !out->WriteU32(this->id)) || (parent->version >> 16 == 1 && !out->WriteU16(static_cast<uint16_t>(this->id)))
        || !out->WriteU16(this->numSettings) || (parent->version >> 16 >= 2 && !out->WriteU16(this->reserved)) || !out->WriteU32(this->offset)
        || !out->WriteU16(this->flags) || !out->WriteU16(this->label)) {
...
bool OpenTypeFEAT::FeatureSettingDefn::ParsePart(Buffer& table, bool used)
{
    ...
    if (!table.ReadU16(&this->label) || (used && !name->IsValidNameId(this->label))) {
...
{
    if (!out->WriteS16(this->value) || !out->WriteU16(this->label)) {
