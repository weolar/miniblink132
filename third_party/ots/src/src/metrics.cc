...
bool OpenTypeMetricsHeader::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadS16(&this->ascent) || !table.ReadS16(&this->descent) || !table.ReadS16(&this->linegap) || !table.ReadU16(&this->adv_width_max)
        || !table.ReadS16(&this->min_sb1) || !table.ReadS16(&this->min_sb2) || !table.ReadS16(&this->max_extent) || !table.ReadS16(&this->caret_slope_rise)
        || !table.ReadS16(&this->caret_slope_run) || !table.ReadS16(&this->caret_offset)) {
    ...
    if (!(head->mac_style & 2) && (this->caret_offset != 0)) {
...
{
    if (!out->WriteU32(this->version) || !out->WriteS16(this->ascent) || !out->WriteS16(this->descent) || !out->WriteS16(this->linegap)
        || !out->WriteU16(this->adv_width_max) || !out->WriteS16(this->min_sb1) || !out->WriteS16(this->min_sb2) || !out->WriteS16(this->max_extent)
        || !out->WriteS16(this->caret_slope_rise) || !out->WriteS16(this->caret_slope_run) || !out->WriteS16(this->caret_offset) || !out->WriteR64(0)
        || // reserved
...
bool OpenTypeMetricsTable::Parse(const uint8_t* data, size_t length)
{
    ...
    OpenTypeMetricsHeader* header = static_cast<OpenTypeMetricsHeader*>(GetFont()->GetTypedTable(m_header_tag));
    if (!header) {
...
bool OpenTypeMetricsTable::Serialize(OTSStream* out)
{
    for (unsigned i = 0; i < this->entries.size(); ++i) {
        if (!out->WriteU16(this->entries[i].first) || !out->WriteS16(this->entries[i].second)) {
