...
#else
#define OTS_FAILURE() (std::fprintf(stderr, "ERROR at %s:%d (%s)\n", __FILE__, __LINE__, __FUNCTION__) && false)
#endif
...
#if !defined(OTS_DEBUG)
#define OTS_MESSAGE_(level, otf_, ...) (otf_)->context->Message(level, __VA_ARGS__)
#else
#define OTS_MESSAGE_(level, otf_, ...) OTS_FAILURE(), (otf_)->context->Message(level, __VA_ARGS__)
#endif

// Generate a simple message
#define OTS_FAILURE_MSG_(otf_, ...) (OTS_MESSAGE_(0, otf_, __VA_ARGS__), false)

#define OTS_WARNING_MSG_(otf_, ...) OTS_MESSAGE_(1, otf_, __VA_ARGS__)
...
struct Font {
    ...
    bool ParseTable(const TableEntry& tableinfo, const uint8_t* data, Arena& arena);
