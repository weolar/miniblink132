...
bool OpenTypeKERN::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&this->version) || !table.ReadU16(&num_tables)) {
    ...
    for (unsigned i = 0; i < num_tables; ++i) {
        ...
        if (subtable.version > 0) {
            Warning("Ignoring subtable %d with unsupported version: %d", i, subtable.version);
            continue;
        }
        ...
        if (!(subtable.coverage & 0x1)) {
            Warning("We don't support vertical data as the renderer doesn't support it.");
            continue;
        }
        ...
        if (!table.ReadU16(&num_pairs) || !table.ReadU16(&subtable.search_range) || !table.ReadU16(&subtable.entry_selector)
        ...
        for (unsigned j = 0; j < num_pairs; ++j) {
            ...
            const uint32_t current_pair = (kerning_pair.left << 16) + kerning_pair.right;
...
bool OpenTypeKERN::Serialize(OTSStream* out)
{
    ...
    if (num_subtables != this->subtables.size() || !out->WriteU16(this->version) || !out->WriteU16(num_subtables)) {
        return Error("Failed to write kern table header");
    }

    for (uint16_t i = 0; i < num_subtables; ++i) {
        const size_t length = 14 + (6 * this->subtables[i].pairs.size());
        if (length > std::numeric_limits<uint16_t>::max() || !out->WriteU16(this->subtables[i].version) || !out->WriteU16(static_cast<uint16_t>(length))
            || !out->WriteU16(this->subtables[i].coverage) || !out->WriteU16(static_cast<uint16_t>(this->subtables[i].pairs.size()))
            || !out->WriteU16(this->subtables[i].search_range) || !out->WriteU16(this->subtables[i].entry_selector)
        ...
        for (unsigned j = 0; j < this->subtables[i].pairs.size(); ++j) {
            if (!out->WriteU16(this->subtables[i].pairs[j].left) || !out->WriteU16(this->subtables[i].pairs[j].right)
