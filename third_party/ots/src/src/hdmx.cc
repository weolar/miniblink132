...
bool OpenTypeHDMX::Parse(const uint8_t* data, size_t length)
{
    ...
    int16_t num_recs;
    if (!table.ReadU16(&this->version) || !table.ReadS16(&num_recs) || !table.ReadS32(&this->size_device_record)) {
    ...
    for (int i = 0; i < num_recs; ++i) {
        ...
        if ((this->pad_len > 0) && !table.Skip(this->pad_len)) {
...
bool OpenTypeHDMX::Serialize(OTSStream* out)
{
    ...
    if (this->records.size() > static_cast<size_t>(std::numeric_limits<int16_t>::max()) || !out->WriteU16(this->version) || !out->WriteS16(num_recs)
    ...
    for (int16_t i = 0; i < num_recs; ++i) {
        ...
        if ((this->pad_len > 0) && !out->Write((const uint8_t*)"\x00\x00\x00", this->pad_len)) {
