...
bool OpenTypeMVAR::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&majorVersion) || !table.ReadU16(&minorVersion) || !table.ReadU16(&reserved) || !table.ReadU16(&valueRecordSize)
        || !table.ReadU16(&valueRecordCount) || !table.ReadU16(&itemVariationStoreOffset)) {
    ...
    for (unsigned i = 0; i < valueRecordCount; i++) {
        ...
        if (!table.ReadU32(&tag) || !table.ReadU16(&deltaSetOuterIndex) || !table.ReadU16(&deltaSetInnerIndex)) {
            return DropVariations("Failed to read value record");
        }
        if (tag <= prevTag) {
            return DropVariations("Out-of-order value tag: '%c%c%c%c', previous tag: '%c%c%c%c'", OTS_UNTAG(tag), OTS_UNTAG(prevTag));
        }
        prevTag = tag;
