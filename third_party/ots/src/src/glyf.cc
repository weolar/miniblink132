...
bool OpenTypeGLYF::ParseFlagsForSimpleGlyph(Buffer& glyph, uint32_t num_flags, uint32_t* flag_index, uint32_t* coordinates_length)
{
    uint8_t flag = 0;
    ...
    if (flag & (1u << 6) && *flag_index != 0) {
        return Error("Bad glyph flag (%d), "
                     "bit 6 must be set to zero for flag %d",
            flag, *flag_index);
    }
    ...
    *coordinates_length += delta;
    if (glyph.length() < *coordinates_length) {
        return Error("Glyph coordinates length bigger than glyph length (%d > %d)", *coordinates_length, glyph.length());
    }

    return true;
}

bool OpenTypeGLYF::ParseSimpleGlyph(Buffer& glyph, int16_t num_contours)
{
    ...
    if (this->maxp->version_1 && num_flags > this->maxp->max_points) {
    ...
    if (this->maxp->version_1 && this->maxp->max_size_glyf_instructions < bytecode_length) {
        this->maxp->max_size_glyf_instructions = bytecode_length;
        Warning("Bytecode length is bigger than maxp.maxSizeOfInstructions %d: %d", this->maxp->max_size_glyf_instructions, bytecode_length);
    }
...
bool OpenTypeGLYF::ParseCompositeGlyph(Buffer& glyph, ComponentPointCount* component_point_count)
{
    uint16_t flags = 0;
    uint16_t gid = 0;
    do {
        ...
        if (flags & WE_HAVE_A_SCALE) {
            ...
            int16_t scale10;
            int16_t yscale;
            if (!glyph.ReadS16(&xscale) || !glyph.ReadS16(&scale01) || !glyph.ReadS16(&scale10) || !glyph.ReadS16(&yscale)) {
...
bool OpenTypeGLYF::Parse(const uint8_t* data, size_t length)
{
    ...
    OpenTypeLOCA* loca = static_cast<OpenTypeLOCA*>(GetFont()->GetTypedTable(OTS_TAG_LOCA));
    ...
    for (unsigned i = 0; i < num_glyphs; ++i) {
        ...
        if (!glyph.ReadS16(&num_contours) || !glyph.ReadS16(&xmin) || !glyph.ReadS16(&ymin) || !glyph.ReadS16(&xmax) || !glyph.ReadS16(&ymax)) {
        ...
        if (num_contours == 0) {
            ...
            while (component_point_count.gid_stack.size()) {
                ...
                Buffer points_count_glyph(GetGlyphBufferSection(data, length, offsets, stack_top_gid.gid));
                ...
                if (!TraverseComponentsCountingPoints(points_count_glyph, i, stack_top_gid.level, &component_point_count)) {
                    return Error("Error validating component points and depth.");
                }

                if (component_point_count.accumulated_component_points > std::numeric_limits<uint16_t>::max()) {
                    return Error("Illegal composite points value "
                                 "exceeding 0xFFFF for base glyph %d.",
                        i);
                } else if (this->maxp->version_1 && component_point_count.accumulated_component_points > this->maxp->max_c_points) {
                    Warning("Number of composite points in glyph %d exceeds "
                            "maxp maxCompositePoints: %d vs %d, adjusting limit.",
                        i, component_point_count.accumulated_component_points, this->maxp->max_c_points);
                    this->maxp->max_c_points = component_point_count.accumulated_component_points;
                }
            }
        }
        ...
        if (padding) {
            this->iov.push_back(std::make_pair(reinterpret_cast<const uint8_t*>("\x00\x00\x00\x00"), static_cast<size_t>(padding)));
            new_size += padding;
        }
    ...
    if ((*std::max_element(resulting_offsets.begin(), resulting_offsets.end()) >= (max16 * 2u)) && (head->index_to_loc_format != 1)) {
...
bool OpenTypeGLYF::TraverseComponentsCountingPoints(Buffer& glyph, uint16_t base_glyph_id, uint32_t level, ComponentPointCount* component_point_count)
{

    int16_t num_contours;
    if (!glyph.ReadS16(&num_contours) || !glyph.Skip(8)) {
    ...
    if (level > std::numeric_limits<uint16_t>::max()) {
        return Error("Illegal component depth exceeding 0xFFFF in base glyph id %d.", base_glyph_id);
    } else if (this->maxp->version_1 && level > this->maxp->max_c_depth) {
...
Buffer OpenTypeGLYF::GetGlyphBufferSection(const uint8_t* data, size_t length, const std::vector<uint32_t>& loca_offsets, unsigned glyph_id)
{
