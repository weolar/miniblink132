...
bool ParseLangSysTable(const ots::Font* font, ots::Buffer* subtable, const uint32_t tag, const uint16_t num_features)
{
...
bool ParseScriptTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint32_t tag, const uint16_t num_features)
{
    ...
    for (unsigned i = 0; i < lang_sys_count; ++i) {
        ...
        if (lang_sys_records[i].offset < lang_sys_record_end || lang_sys_records[i].offset >= length) {
            return OTS_FAILURE_MSG("bad offset to lang sys table: %x", lang_sys_records[i].offset);
        }
    ...
    for (unsigned i = 0; i < lang_sys_count; ++i) {
        ...
        if (!ParseLangSysTable(font, &subtable, lang_sys_records[i].tag, num_features)) {
            return OTS_FAILURE_MSG(
                "Failed to parse langsys table %d (%c%c%c%c) for script tag %c%c%c%c", i, OTS_UNTAG(lang_sys_records[i].tag), OTS_UNTAG(tag));
        }
    }
...
bool ParseFeatureTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseLookupTable(ots::Font* font, const uint8_t* data, const size_t length, const ots::LookupSubtableParser* parser)
{
    ots::Buffer subtable(data, length);
    ...
    if (use_mark_filtering_set) {
        ...
        ots::OpenTypeGDEF* gdef = static_cast<ots::OpenTypeGDEF*>(font->GetTypedTable(OTS_TAG_GDEF));

        if (gdef && (gdef->num_mark_glyph_sets == 0 || mark_filtering_set >= gdef->num_mark_glyph_sets)) {
    ...
    for (unsigned i = 0; i < subtable_count; ++i) {
        if (!parser->Parse(font, data + subtables[i], length - subtables[i], lookup_type)) {
...
bool ParseClassDefFormat1(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t num_classes)
{
    ots::Buffer subtable(data, length);
...
bool ParseClassDefFormat2(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t num_classes)
{
    ots::Buffer subtable(data, length);
...
bool ParseCoverageFormat1(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t expected_num_glyphs)
{
    ots::Buffer subtable(data, length);
...
bool ParseCoverageFormat2(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t expected_num_glyphs)
{
    ots::Buffer subtable(data, length);
...
bool ParseLookupRecord(const ots::Font* font, ots::Buffer* subtable, const uint16_t num_glyphs, const uint16_t num_lookups)
{
...
bool ParseRuleSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseRuleSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < rule_count; ++i) {
        ...
        if (!ParseRuleSubtable(font, data + offset_rule, length - offset_rule, num_glyphs, num_lookups)) {
...
bool ParseContextFormat1(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < rule_set_count; ++i) {
        ...
        if (!ParseRuleSetTable(font, data + offset_rule, length - offset_rule, num_glyphs, num_lookups)) {
...
bool ParseClassRuleTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseClassSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < class_rule_count; ++i) {
        ...
        if (!ParseClassRuleTable(font, data + offset_class_rule, length - offset_class_rule, num_glyphs, num_lookups)) {
...
bool ParseContextFormat2(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    if (!ots::ParseClassDefTable(font, data + offset_class_def, length - offset_class_def, num_glyphs, ots::kMaxClassDefValue)) {
    ...
    for (unsigned i = 0; i < class_set_cnt; ++i) {
        ...
        if (offset_class_rule) {
            ...
            if (!ParseClassSetTable(font, data + offset_class_rule, length - offset_class_rule, num_glyphs, num_lookups)) {
...
bool ParseContextFormat3(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseChainRuleSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseChainRuleSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < chain_rule_count; ++i) {
        ...
        if (!ParseChainRuleSubtable(font, data + offset_chain_rule, length - offset_chain_rule, num_glyphs, num_lookups)) {
...
bool ParseChainContextFormat1(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < chain_rule_set_count; ++i) {
        ...
        if (!ParseChainRuleSetTable(font, data + offset_chain_rule_set, length - offset_chain_rule_set, num_glyphs, num_lookups)) {
...
bool ParseChainClassRuleSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
...
bool ParseChainClassSetTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < chain_class_rule_count; ++i) {
        ...
        if (!ParseChainClassRuleSubtable(font, data + offset_chain_class_rule, length - offset_chain_class_rule, num_glyphs, num_lookups)) {
...
bool ParseChainContextFormat2(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    // Skip format field.
    if (!subtable.Skip(2) || !subtable.ReadU16(&offset_coverage) || !subtable.ReadU16(&offset_backtrack_class_def) || !subtable.ReadU16(&offset_input_class_def)
        || !subtable.ReadU16(&offset_lookahead_class_def) || !subtable.ReadU16(&chain_class_set_count)) {
    ...
    if (offset_backtrack_class_def) {
        ...
        if (!ots::ParseClassDefTable(font, data + offset_backtrack_class_def, length - offset_backtrack_class_def, num_glyphs, ots::kMaxClassDefValue)) {
    ...
    if (!ots::ParseClassDefTable(font, data + offset_input_class_def, length - offset_input_class_def, num_glyphs, ots::kMaxClassDefValue)) {
    ...
    if (offset_lookahead_class_def) {
        ...
        if (!ots::ParseClassDefTable(font, data + offset_lookahead_class_def, length - offset_lookahead_class_def, num_glyphs, ots::kMaxClassDefValue)) {
    ...
    for (unsigned i = 0; i < chain_class_set_count; ++i) {
        ...
        if (offset_chain_class_set) {
            ...
            if (!ParseChainClassSetTable(font, data + offset_chain_class_set, length - offset_chain_class_set, num_glyphs, num_lookups)) {
...
bool ParseChainContextFormat3(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    ots::Buffer subtable(data, length);
    ...
    const unsigned lookup_record_end
        = 2 * (static_cast<unsigned>(backtrack_count) + static_cast<unsigned>(input_count) + static_cast<unsigned>(lookahead_count))
    ...
    for (unsigned i = 0; i < backtrack_count; ++i) {
        ...
        if (!ots::ParseCoverageTable(font, data + offsets_backtrack[i], length - offsets_backtrack[i], num_glyphs)) {
    ...
    for (unsigned i = 0; i < input_count; ++i) {
        ...
        if (!ots::ParseCoverageTable(font, data + offsets_input[i], length - offsets_input[i], num_glyphs)) {
    ...
    for (unsigned i = 0; i < lookahead_count; ++i) {
        ...
        if (!ots::ParseCoverageTable(font, data + offsets_lookahead[i], length - offsets_lookahead[i], num_glyphs)) {
...
bool LookupSubtableParser::Parse(const Font* font, const uint8_t* data, const size_t length, const uint16_t lookup_type) const
{
...
bool ParseScriptListTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_features)
{
    ...
    for (unsigned i = 0; i < script_count; ++i) {
        if (!ParseScriptTable(font, data + script_list[i].offset, length - script_list[i].offset, script_list[i].tag, num_features)) {
...
bool ParseFeatureListTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_lookups, uint16_t* num_features)
{
    Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < feature_count; ++i) {
        if (!ParseFeatureTable(font, data + feature_records[i].offset, length - feature_records[i].offset, num_lookups)) {
...
// the number.
bool ParseLookupListTable(Font* font, const uint8_t* data, const size_t length, const LookupSubtableParser* parser, uint16_t* num_lookups)
{
    Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < *num_lookups; ++i) {
        if (!ParseLookupTable(font, data + lookups[i], length - lookups[i], parser)) {
...
bool ParseClassDefTable(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t num_classes)
{
    Buffer subtable(data, length);
...
bool ParseCoverageTable(const ots::Font* font, const uint8_t* data, size_t length, const uint16_t num_glyphs, const uint16_t expected_num_glyphs)
{
    Buffer subtable(data, length);
...
bool ParseDeviceTable(const ots::Font* font, const uint8_t* data, size_t length)
{
    Buffer subtable(data, length);
...
bool ParseContextSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    Buffer subtable(data, length);
...
bool ParseChainingContextSubtable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t num_glyphs, const uint16_t num_lookups)
{
    Buffer subtable(data, length);
...
bool ParseExtensionSubtable(const Font* font, const uint8_t* data, const size_t length, const LookupSubtableParser* parser)
{
    Buffer subtable(data, length);
    ...
    if (!parser->Parse(font, data + offset_extension, length - offset_extension, lookup_type)) {
...
bool ParseConditionTable(const Font* font, const uint8_t* data, const size_t length, const uint16_t axis_count)
{
    Buffer subtable(data, length);
    ...
    if (!subtable.ReadU16(&axis_index) || !subtable.ReadS16(&filter_range_min_value) || !subtable.ReadS16(&filter_range_max_value)) {
...
bool ParseConditionSetTable(const Font* font, const uint8_t* data, const size_t length, const uint16_t axis_count)
{
    Buffer subtable(data, length);
    ...
    for (uint16_t i = 0; i < condition_count; i++) {
        ...
        if (!ParseConditionTable(font, data + condition_offset, length - condition_offset, axis_count)) {
...
bool ParseFeatureTableSubstitutionTable(const Font* font, const uint8_t* data, const size_t length, const uint16_t num_lookups)
{
    Buffer subtable(data, length);
    ...
    if (!subtable.ReadU16(&version_major) || !subtable.ReadU16(&version_minor) || !subtable.ReadU16(&substitution_count)) {
    ...
    for (uint16_t i = 0; i < substitution_count; i++) {
        ...
        if (!subtable.ReadU16(&feature_index) || !subtable.ReadU32(&alternate_feature_table_offset)) {
            return OTS_FAILURE_MSG("Failed to read feature table substitution record");
        }

        if (alternate_feature_table_offset < kFeatureTableSubstitutionHeaderSize + kFeatureTableSubstitutionRecordSize * substitution_count
            || alternate_feature_table_offset >= length) {
            return OTS_FAILURE_MSG("Invalid alternate feature table offset");
        }

        if (!ParseFeatureTable(font, data + alternate_feature_table_offset, length - alternate_feature_table_offset, num_lookups)) {
...
bool ParseFeatureVariationsTable(const Font* font, const uint8_t* data, const size_t length, const uint16_t num_lookups)
{
    Buffer subtable(data, length);
    ...
    if (!subtable.ReadU16(&version_major) || !subtable.ReadU16(&version_minor) || !subtable.ReadU32(&feature_variation_record_count)) {
    ...
    const size_t kEndOfFeatureVariationRecords = 2 * sizeof(uint16_t) + sizeof(uint32_t) + feature_variation_record_count * 2 * sizeof(uint32_t);
    ...
    for (uint32_t i = 0; i < feature_variation_record_count; i++) {
        ...
        if (!subtable.ReadU32(&condition_set_offset) || !subtable.ReadU32(&feature_table_substitution_offset)) {
            return OTS_FAILURE_MSG("Failed to read feature variation record");
        }

        if (condition_set_offset) {
            if (condition_set_offset < kEndOfFeatureVariationRecords || condition_set_offset >= length) {
                return OTS_FAILURE_MSG("Condition set offset out of range");
            }
            if (!ParseConditionSetTable(font, data + condition_set_offset, length - condition_set_offset, axis_count)) {
                return OTS_FAILURE_MSG("Failed to parse condition set table");
            }
        }

        if (feature_table_substitution_offset) {
            if (feature_table_substitution_offset < kEndOfFeatureVariationRecords || feature_table_substitution_offset >= length) {
                return OTS_FAILURE_MSG("Feature table substitution offset out of range");
            }
            if (!ParseFeatureTableSubstitutionTable(font, data + feature_table_substitution_offset, length - feature_table_substitution_offset, num_lookups)) {
