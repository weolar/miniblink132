...
bool OpenTypeVDMX::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&this->version) || !table.ReadU16(&this->num_recs) || !table.ReadU16(&this->num_ratios)) {
    ...
    for (unsigned i = 0; i < this->num_ratios; ++i) {
        ...
        if ((i < this->num_ratios - 1u) && (rec.x_ratio == 0) && (rec.y_start_ratio == 0) && (rec.y_end_ratio == 0)) {
...
{
    if (!out->WriteU16(this->version) || !out->WriteU16(this->num_recs) || !out->WriteU16(this->num_ratios)) {
