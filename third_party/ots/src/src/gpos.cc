...
// Lookup type parsers.
bool ParseSingleAdjustment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParsePairAdjustment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseCursiveAttachment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseMarkToBaseAttachment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseMarkToLigatureAttachment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseMarkToMarkAttachment(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseContextPositioning(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseChainedContextPositioning(const ots::Font* font, const uint8_t* data, const size_t length);
bool ParseExtensionPositioning(const ots::Font* font, const uint8_t* data, const size_t length);

const ots::LookupSubtableParser::TypeParser kGposTypeParsers[] = { { GPOS_TYPE_SINGLE_ADJUSTMENT, ParseSingleAdjustment },
    { GPOS_TYPE_PAIR_ADJUSTMENT, ParsePairAdjustment }, { GPOS_TYPE_CURSIVE_ATTACHMENT, ParseCursiveAttachment },
    ...
    { GPOS_TYPE_MARK_TO_MARK_ATTACHMENT, ParseMarkToMarkAttachment }, { GPOS_TYPE_CONTEXT_POSITIONING, ParseContextPositioning },
    { GPOS_TYPE_CHAINED_CONTEXT_POSITIONING, ParseChainedContextPositioning }, { GPOS_TYPE_EXTENSION_POSITIONING, ParseExtensionPositioning } };

const ots::LookupSubtableParser kGposLookupSubtableParser = { arraysize(kGposTypeParsers), GPOS_TYPE_EXTENSION_POSITIONING, kGposTypeParsers };
...
bool ParseValueRecord(const ots::Font* font, ots::Buffer* subtable, const uint16_t value_format)
{
...
bool ParseAnchorTable(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    if (format == 2) {
        ...
        if (offset_x_device) {
            ...
            if (!ots::ParseDeviceTable(font, data + offset_x_device, length - offset_x_device)) {
        ...
        if (offset_y_device) {
            ...
            if (!ots::ParseDeviceTable(font, data + offset_y_device, length - offset_y_device)) {
                return OTS_FAILURE_MSG("Failed to parse device table in anchor table");
            }
        }
    }
...
bool ParseMarkArrayTable(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < mark_count; ++i) {
        ...
        if (!ParseAnchorTable(font, data + offset_mark_anchor, length - offset_mark_anchor)) {
...
bool ParseSingleAdjustment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    ots::OpenTypeMAXP* maxp = static_cast<ots::OpenTypeMAXP*>(font->GetTypedTable(OTS_TAG_MAXP));
    if (!maxp) {
    ...
    if (!ots::ParseCoverageTable(font, data + offset_coverage, length - offset_coverage, maxp->num_glyphs)) {
...
bool ParsePairSetTable(
    const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t value_format1, const uint16_t value_format2, const uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
    ...
    const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t value_format1, const uint16_t value_format2, const uint16_t num_glyphs)
{
    ...
    for (unsigned i = 0; i < pair_set_count; ++i) {
        ...
        if (!ParsePairSetTable(font, data + pair_set_offset, length - pair_set_offset, value_format1, value_format2, num_glyphs)) {
    ...
    const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t value_format1, const uint16_t value_format2, const uint16_t num_glyphs)
{
    ...
    if (!subtable.ReadU16(&offset_class_def1) || !subtable.ReadU16(&offset_class_def2) || !subtable.ReadU16(&class1_count)
    ...
    size_t value_records_size = size_t(class1_count) * size_t(class2_count) * (value_record1_size + value_record2_size);

    // Check the validity of class definition offsets.
    if (offset_class_def1 < subtable.offset() + value_records_size || offset_class_def2 < subtable.offset() + value_records_size || offset_class_def1 >= length
        || offset_class_def2 >= length) {
    ...
    if (value_record1_size || value_record2_size) {
        for (unsigned i = 0; i < class1_count; ++i) {
            ...
            for (unsigned j = 0; j < class2_count; ++j) {
                if (value_format1 && value_record2_size && !ParseValueRecord(font, &subtable, value_format1)) {
                    return OTS_FAILURE_MSG("Failed to parse value record 1 %d and %d", j, i);
                }
                if (value_format2 && value_record2_size && !ParseValueRecord(font, &subtable, value_format2)) {
    ...
    if (!ots::ParseClassDefTable(font, data + offset_class_def1, length - offset_class_def1, num_glyphs, ots::kMaxClassDefValue)) {
        return OTS_FAILURE_MSG("Failed to parse class definition table 1");
    }
    if (!ots::ParseClassDefTable(font, data + offset_class_def2, length - offset_class_def2, num_glyphs, ots::kMaxClassDefValue)) {
...
bool ParsePairAdjustment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    if (format == 1) {
        if (!ParsePairPosFormat1(font, data, length, value_format1, value_format2, maxp->num_glyphs)) {
            return OTS_FAILURE_MSG("Failed to parse pair pos format 1");
        }
    } else if (format == 2) {
        if (!ParsePairPosFormat2(font, data, length, value_format1, value_format2, maxp->num_glyphs)) {
...
bool ParseCursiveAttachment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    for (unsigned i = 0; i < entry_exit_count; ++i) {
        ...
        if (offset_entry_anchor) {
            ...
            if (!ParseAnchorTable(font, data + offset_entry_anchor, length - offset_entry_anchor)) {
        ...
        if (offset_exit_anchor) {
            ...
            if (!ParseAnchorTable(font, data + offset_exit_anchor, length - offset_exit_anchor)) {
...
bool ParseAnchorArrayTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t class_count)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < record_count; ++i) {
        for (unsigned j = 0; j < class_count; ++j) {
            ...
            if (offset_record) {
                ...
                if (!ParseAnchorTable(font, data + offset_record, length - offset_record)) {
...
bool ParseLigatureArrayTable(const ots::Font* font, const uint8_t* data, const size_t length, const uint16_t class_count)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < ligature_count; ++i) {
        ...
        if (!ParseAnchorArrayTable(font, data + offset_ligature_attach, length - offset_ligature_attach, class_count)) {
...
bool ParseMarkToAttachmentSubtables(const ots::Font* font, const uint8_t* data, const size_t length, const GPOS_TYPE type)
{
    ots::Buffer subtable(data, length);
    ...
    if (!subtable.ReadU16(&format) || !subtable.ReadU16(&offset_coverage1) || !subtable.ReadU16(&offset_coverage2) || !subtable.ReadU16(&class_count)
        || !subtable.ReadU16(&offset_mark_array) || !subtable.ReadU16(&offset_type_specific_array)) {
    ...
    if (!ots::ParseCoverageTable(font, data + offset_coverage1, length - offset_coverage1, maxp->num_glyphs)) {
    ...
    if (!ots::ParseCoverageTable(font, data + offset_coverage2, length - offset_coverage2, maxp->num_glyphs)) {
    ...
    if (!ParseMarkArrayTable(font, data + offset_mark_array, length - offset_mark_array)) {
    ...
    if (type == GPOS_TYPE_MARK_TO_BASE_ATTACHMENT || type == GPOS_TYPE_MARK_TO_MARK_ATTACHMENT) {
        if (!ParseAnchorArrayTable(font, data + offset_type_specific_array, length - offset_type_specific_array, class_count)) {
            return OTS_FAILURE_MSG("Failed to parse anchor array");
        }
    } else if (type == GPOS_TYPE_MARK_TO_LIGATURE_ATTACHMENT) {
        if (!ParseLigatureArrayTable(font, data + offset_type_specific_array, length - offset_type_specific_array, class_count)) {
...
bool ParseMarkToBaseAttachment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    return ParseMarkToAttachmentSubtables(font, data, length, GPOS_TYPE_MARK_TO_BASE_ATTACHMENT);
}
...
bool ParseMarkToLigatureAttachment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    return ParseMarkToAttachmentSubtables(font, data, length, GPOS_TYPE_MARK_TO_LIGATURE_ATTACHMENT);
}
...
bool ParseMarkToMarkAttachment(const ots::Font* font, const uint8_t* data, const size_t length)
{
    return ParseMarkToAttachmentSubtables(font, data, length, GPOS_TYPE_MARK_TO_MARK_ATTACHMENT);
}
...
bool ParseContextPositioning(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    ots::OpenTypeGPOS* gpos = static_cast<ots::OpenTypeGPOS*>(font->GetTypedTable(OTS_TAG_GPOS));
    if (!gpos) {
        return OTS_FAILURE_MSG("Internal error!");
    }
    return ots::ParseContextSubtable(font, data, length, maxp->num_glyphs, gpos->num_lookups);
}
...
bool ParseChainedContextPositioning(const ots::Font* font, const uint8_t* data, const size_t length)
{
    ...
    return ots::ParseChainingContextSubtable(font, data, length, maxp->num_glyphs, gpos->num_lookups);
}
...
bool ParseExtensionPositioning(const ots::Font* font, const uint8_t* data, const size_t length)
{
    return ots::ParseExtensionSubtable(font, data, length, &kGposLookupSubtableParser);
}
...
bool OpenTypeGPOS::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&version_major) || !table.ReadU16(&version_minor) || !table.ReadU16(&offset_script_list) || !table.ReadU16(&offset_feature_list)
    ...
    const size_t header_size = (version_minor == 0) ? kGposHeaderSize_1_0 : kGposHeaderSize_1_1;
    ...
    if (offset_lookup_list) {
        ...
        if (!ParseLookupListTable(font, data + offset_lookup_list, length - offset_lookup_list, &kGposLookupSubtableParser, &this->num_lookups)) {
    ...
    if (offset_feature_list) {
        ...
        if (!ParseFeatureListTable(font, data + offset_feature_list, length - offset_feature_list, this->num_lookups, &num_features)) {
    ...
    if (offset_script_list) {
        ...
        if (!ParseScriptListTable(font, data + offset_script_list, length - offset_script_list, num_features)) {
    ...
    if (offset_feature_variations) {
        ...
        if (!ParseFeatureVariationsTable(font, data + offset_feature_variations, length - offset_feature_variations, this->num_lookups)) {
