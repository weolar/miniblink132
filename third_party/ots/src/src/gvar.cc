...
static bool ParseSharedTuples(const Font* font, const uint8_t* data, size_t length, size_t sharedTupleCount, size_t axisCount)
{
...
static bool ParseGlyphVariationDataArray(const Font* font, const uint8_t* data, size_t length, uint16_t flags, size_t glyphCount, size_t axisCount,
    size_t sharedTupleCount, const uint8_t* glyphVariationData, size_t glyphVariationDataLength)
{
    Buffer subtable(data, length);
    ...
    for (size_t i = 0; i < glyphCount + 1; i++) {
        ...
        if (i > 0 && offset > prevOffset) {
            ...
            if (!ParseVariationData(font, glyphVariationData + prevOffset, glyphVariationDataLength - prevOffset, axisCount, sharedTupleCount)) {
...
bool OpenTypeGVAR::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&majorVersion) || !table.ReadU16(&minorVersion) || !table.ReadU16(&axisCount) || !table.ReadU16(&sharedTupleCount)
        || !table.ReadU32(&sharedTuplesOffset) || !table.ReadU16(&glyphCount) || !table.ReadU16(&flags) || !table.ReadU32(&glyphVariationDataArrayOffset)) {
    ...
    OpenTypeFVAR* fvar = static_cast<OpenTypeFVAR*>(GetFont()->GetTypedTable(OTS_TAG_FVAR));
    if (!fvar) {
    ...
    if (sharedTupleCount > 0) {
        ...
        if (!ParseSharedTuples(GetFont(), data + sharedTuplesOffset, length - sharedTuplesOffset, sharedTupleCount, axisCount)) {
    ...
    if (glyphVariationDataArrayOffset) {
        ...
        if (!ParseGlyphVariationDataArray(GetFont(), data + table.offset(), length - table.offset(), flags, glyphCount, axisCount, sharedTupleCount,
                data + glyphVariationDataArrayOffset, length - glyphVariationDataArrayOffset)) {
