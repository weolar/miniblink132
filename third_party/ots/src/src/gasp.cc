...
bool OpenTypeGASP::Parse(const uint8_t* data, size_t length)
{
    ...
    if (!table.ReadU16(&this->version) || !table.ReadU16(&num_ranges)) {
...
bool OpenTypeGASP::Serialize(OTSStream* out)
{
    ...
    if (num_ranges != this->gasp_ranges.size() || !out->WriteU16(this->version) || !out->WriteU16(num_ranges)) {
        return Error("Failed to write table header");
    }

    for (uint16_t i = 0; i < num_ranges; ++i) {
        if (!out->WriteU16(this->gasp_ranges[i].first) || !out->WriteU16(this->gasp_ranges[i].second)) {
