...
void AssignToUtf16BeFromAscii(std::string* target, const std::string& source)
{
...
bool OpenTypeNAME::Parse(const uint8_t* data, size_t length)
{
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (!table.ReadU16(&rec.platform_id) || !table.ReadU16(&rec.encoding_id) || !table.ReadU16(&rec.language_id) || !table.ReadU16(&rec.name_id)
            || !table.ReadU16(&name_length) || !table.ReadU16(&name_offset)) {
    ...
    static const char* kStdNames[kStdNameCount] = { NULL, "OTS derived font", "Unspecified", NULL, "OTS derived font", "1.000", "OTS-derived-font" };
    ...
    for (uint16_t i = 0; i < kStdNameCount; ++i) {
        ...
        if (!mac_name[i] && !win_name[i]) {
            NameRecord mac_rec(1 /* platform_id */, 0 /* encoding_id */, 0 /* language_id */, i /* name_id */);
            mac_rec.text.assign(kStdNames[i]);

            NameRecord win_rec(3 /* platform_id */, 1 /* encoding_id */, 1033 /* language_id */, i /* name_id */);
...
bool OpenTypeNAME::Serialize(OTSStream* out)
{
    ...
    for (const auto& rec : this->names) {
        if (string_data.size() + rec.text.size() > std::numeric_limits<uint16_t>::max() || !out->WriteU16(rec.platform_id) || !out->WriteU16(rec.encoding_id)
            || !out->WriteU16(rec.language_id) || !out->WriteU16(rec.name_id) || !out->WriteU16(static_cast<uint16_t>(rec.text.size()))
    ...
    if (format == 1) {
        ...
        for (const auto& tag : this->lang_tags) {
            if (string_data.size() + tag.size() > std::numeric_limits<uint16_t>::max() || !out->WriteU16(static_cast<uint16_t>(tag.size()))
                || !out->WriteU16(static_cast<uint16_t>(string_data.size()))) {
