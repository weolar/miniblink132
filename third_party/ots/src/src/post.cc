...
bool OpenTypePOST::Parse(const uint8_t* data, size_t length)
{
    ...
    if (this->version != 0x00010000 && this->version != 0x00020000 && this->version != 0x00030000) {
        // 0x00025000 is deprecated. We don't accept it.
        return Error("Unsupported table version 0x%x", this->version);
    }

    if (!table.ReadU32(&this->italic_angle) || !table.ReadS16(&this->underline) || !table.ReadS16(&this->underline_thickness)
    ...
    OpenTypeMAXP* maxp = static_cast<OpenTypeMAXP*>(GetFont()->GetTable(OTS_TAG_MAXP));
    if (!maxp) {
    ...
    for (;;) {
        ...
        this->names.push_back(std::string(reinterpret_cast<const char*>(strings + 1), string_length));
...
bool OpenTypePOST::Serialize(OTSStream* out)
{
    ...
    if (!out->WriteU32(this->version) || !out->WriteU32(this->italic_angle) || !out->WriteS16(this->underline) || !out->WriteS16(this->underline_thickness)
        || !out->WriteU32(this->is_fixed_pitch) || !out->WriteU32(0) || !out->WriteU32(0) || !out->WriteU32(0) || !out->WriteU32(0)) {
    ...
    const uint16_t num_indexes = static_cast<uint16_t>(this->glyph_name_index.size());
    if (num_indexes != this->glyph_name_index.size() || !out->WriteU16(num_indexes)) {
