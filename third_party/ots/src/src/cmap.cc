...
bool OpenTypeCMAP::ParseFormat4(int platform, int encoding, const uint8_t* data, size_t length, uint16_t num_glyphs)
{
    ...
    OpenTypeOS2* os2 = static_cast<OpenTypeOS2*>(GetFont()->GetTypedTable(OTS_TAG_OS2));
    if (!os2) {
    ...
    for (unsigned i = 1; i < segcount; ++i) {
        if ((i == segcount - 1u) && (ranges[i - 1].start_range == 0xffff) && (ranges[i - 1].end_range == 0xffff) && (ranges[i].start_range == 0xffff)
        ...
        // Fix them.
        if (os2->table.first_char_index != 0xFFFF && ranges[i].start_range != 0xFFFF && os2->table.first_char_index > ranges[i].start_range) {
            os2->table.first_char_index = ranges[i].start_range;
        }
        if (os2->table.last_char_index != 0xFFFF && ranges[i].end_range != 0xFFFF && os2->table.last_char_index < ranges[i].end_range) {
    ...
    if (ranges[segcount - 1].start_range != 0xffff || ranges[segcount - 1].end_range != 0xffff) {
        return Error("Final segment start and end must be 0xFFFF (0x%04X-0x%04X)", ranges[segcount - 1].start_range, ranges[segcount - 1].end_range);
    }
...
bool OpenTypeCMAP::Parse31012(const uint8_t* data, size_t length, uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
    ...
    std::vector<ots::OpenTypeCMAPSubtableRange>& groups = this->subtable_3_10_12;
    ...
    for (unsigned i = 0; i < num_groups; ++i) {
        ...
        if (groups[i].start_range > kUnicodeUpperLimit || groups[i].end_range > kUnicodeUpperLimit || groups[i].start_glyph_id > 0xFFFF) {
            return Error("bad format 12 subtable group (startCharCode=0x%4X, endCharCode=0x%4X, startGlyphID=%d)", groups[i].start_range, groups[i].end_range,
                groups[i].start_glyph_id);
        }
        ...
        if (groups[i].end_range < groups[i].start_range) {
            return Error("format 12 subtable group endCharCode before startCharCode (0x%4X < 0x%4X)", groups[i].end_range, groups[i].start_range);
        }
    ...
    for (unsigned i = 1; i < num_groups; ++i) {
        if (groups[i].start_range <= groups[i - 1].start_range) {
            return Error("out of order format 12 subtable group (startCharCode=0x%4X <= startCharCode=0x%4X of previous group)", groups[i].start_range,
                groups[i - 1].start_range);
        }
        if (groups[i].start_range <= groups[i - 1].end_range) {
            return Error("overlapping format 12 subtable groups (startCharCode=0x%4X <= endCharCode=0x%4X of previous group)", groups[i].start_range,
                groups[i - 1].end_range);
        }
    }
...
bool OpenTypeCMAP::Parse31013(const uint8_t* data, size_t length, uint16_t num_glyphs)
{
    ots::Buffer subtable(data, length);
    ...
    for (unsigned i = 0; i < num_groups; ++i) {
        ...
        if (groups[i].start_range > kUnicodeUpperLimit || groups[i].end_range > kUnicodeUpperLimit || groups[i].start_glyph_id > 0xFFFF) {
            return Error(
                "Bad subrange with start_range=%d, end_range=%d, start_glyph_id=%d", groups[i].start_range, groups[i].end_range, groups[i].start_glyph_id);
        }
...
bool OpenTypeCMAP::Parse0514(const uint8_t* data, size_t length)
{
    ...
    std::vector<ots::OpenTypeCMAPSubtableVSRecord>& records = this->subtable_0_5_14;
    ...
    for (unsigned i = 0; i < num_records; ++i) {
        ...
        if (!((records[i].var_selector >= kMongolianVSStart && records[i].var_selector <= kMongolianVSEnd)
                || (records[i].var_selector >= kVSStart && records[i].var_selector <= kVSEnd)
                || (records[i].var_selector >= kIVSStart && records[i].var_selector <= kIVSEnd))) {
    ...
    for (unsigned i = 0; i < num_records; ++i) {
        ...
        if (records[i].default_offset) {
            ...
            for (unsigned j = 0; j < num_ranges; ++j) {
                ...
                if (ranges[j].unicode_value == 0 || ranges[j].unicode_value > kUnicodeUpperLimit || check_value > kUVSUpperLimit
                    || (last_unicode_value && ranges[j].unicode_value <= last_unicode_value)) {
        ...
        if (records[i].non_default_offset) {
            ...
            uint32_t last_unicode_value = 0;
            std::vector<ots::OpenTypeCMAPSubtableVSMapping>& mappings = records[i].mappings;
            ...
            for (unsigned j = 0; j < num_mappings; ++j) {
                ...
                if (mappings[j].unicode_value > kUnicodeUpperLimit) {
                    return Error(
                        "Invalid Unicode value (%04X > %04X) in mapping %d of variation selector %d", mappings[j].unicode_value, kUnicodeUpperLimit, j, i);
                }
                if (last_unicode_value && mappings[j].unicode_value <= last_unicode_value) {
                    return Error("Out of order Unicode value (%04X <= %04X) in mapping %d of variation selector %d", mappings[j].unicode_value,
                        last_unicode_value, j, i);
                }
...
bool OpenTypeCMAP::Parse(const uint8_t* data, size_t length)
{
        ...
        if (subtable_headers[i - 1].platform > subtable_headers[i].platform
            || (subtable_headers[i - 1].platform == subtable_headers[i].platform
                && (subtable_headers[i - 1].encoding > subtable_headers[i].encoding
                    || (subtable_headers[i - 1].encoding == subtable_headers[i].encoding && subtable_headers[i - 1].language > subtable_headers[i].language))))
            Warning("subtable %d with platform ID %d, encoding ID %d, language ID %d "
                ...
                i, subtable_headers[i].platform, subtable_headers[i].encoding, subtable_headers[i].language, subtable_headers[i - 1].platform,
                subtable_headers[i - 1].encoding, subtable_headers[i - 1].language);
    }
    ...
    for (unsigned i = 0; i < num_tables; ++i) {
        ...
        if (!uniq_checker.insert(std::make_pair(subtable_headers[i].offset, end_byte)).second) {
            // Sometimes Unicode table and MS table share exactly the same data.
            // We'll allow this.
            continue;
        }
        overlap_checker.push_back(std::make_pair(subtable_headers[i].offset, static_cast<uint8_t>(1) /* start */));
        overlap_checker.push_back(std::make_pair(end_byte, static_cast<uint8_t>(0) /* end */));
    }
    ...
    OpenTypeMAXP* maxp = static_cast<OpenTypeMAXP*>(GetFont()->GetTypedTable(OTS_TAG_MAXP));
    if (!maxp) {
    ...
    for (unsigned i = 0; i < num_tables; ++i) {
        if (subtable_headers[i].platform == 0) {
            ...
            if ((subtable_headers[i].encoding == 0 || subtable_headers[i].encoding == 1) && (subtable_headers[i].format == 4)) {
                ...
                if (!ParseFormat4(3, 1, data + subtable_headers[i].offset, subtable_headers[i].length, num_glyphs)) {
                ...
                if (!ParseFormat4(0, 3, data + subtable_headers[i].offset, subtable_headers[i].length, num_glyphs)) {
                    return Error("Failed to parse format 4 cmap subtable %d", i);
                }
            } else if ((subtable_headers[i].encoding == 3 || subtable_headers[i].encoding == 4) && (subtable_headers[i].format == 12)) {
                // parse and output the 0-3-12 or 0-4-12 tables as 3-10-12 table.
                if (!Parse31012(data + subtable_headers[i].offset, subtable_headers[i].length, num_glyphs)) {
                ...
                if (!Parse0514(data + subtable_headers[i].offset, subtable_headers[i].length)) {
            ...
            if ((subtable_headers[i].encoding == 0) && (subtable_headers[i].format == 0)) {
                ...
                if (!Parse100(data + subtable_headers[i].offset, subtable_headers[i].length)) {
                    return OTS_FAILURE();
                }
            }
            ...
            case 1:
                if (subtable_headers[i].format == 4) {
                    ...
                    if (!ParseFormat4(subtable_headers[i].platform, subtable_headers[i].encoding, data + subtable_headers[i].offset, subtable_headers[i].length,
                            num_glyphs)) {
                        return OTS_FAILURE();
                    }
                }
            ...
            case 10:
                if (subtable_headers[i].format == 12) {
                    ...
                    if (!Parse31013(data + subtable_headers[i].offset, subtable_headers[i].length, num_glyphs)) {
                        return OTS_FAILURE();
                    }
                }
                break;
...
bool OpenTypeCMAP::Serialize(OTSStream* out)
{
    ...
    const uint16_t num_subtables = static_cast<uint16_t>(have_034) + static_cast<uint16_t>(have_0514) + static_cast<uint16_t>(have_100)
        + static_cast<uint16_t>(have_304) + static_cast<uint16_t>(have_314) + static_cast<uint16_t>(have_31012) + static_cast<uint16_t>(have_31013);
    ...
    if (have_034) {
        if (!out->Write(this->subtable_0_3_4_data, this->subtable_0_3_4_length)) {
            return OTS_FAILURE();
        }
    }
    ...
    if (have_0514) {
        const std::vector<ots::OpenTypeCMAPSubtableVSRecord>& records = this->subtable_0_5_14;
        const unsigned num_records = records.size();
        if (!out->WriteU16(14) || !out->WriteU32(this->subtable_0_5_14_length) || !out->WriteU32(num_records)) {
            return OTS_FAILURE();
        }
        for (unsigned i = 0; i < num_records; ++i) {
        ...
        for (unsigned i = 0; i < num_records; ++i) {
            if (records[i].default_offset) {
                const std::vector<ots::OpenTypeCMAPSubtableVSRange>& ranges = records[i].ranges;
            ...
            if (records[i].non_default_offset) {
                const std::vector<ots::OpenTypeCMAPSubtableVSMapping>& mappings = records[i].mappings;
    ...
    if (have_304) {
        if (!out->Write(this->subtable_3_0_4_data, this->subtable_3_0_4_length)) {
            return OTS_FAILURE();
        }
    }
    ...
    if (have_31012) {
        std::vector<OpenTypeCMAPSubtableRange>& groups = this->subtable_3_10_12;
    ...
    if (have_31013) {
        std::vector<OpenTypeCMAPSubtableRange>& groups = this->subtable_3_10_13;
        const unsigned num_groups = groups.size();
