...
template <typename ParentType> class TablePart {
public:
    TablePart(ParentType* parent)
        : parent(parent)
    {
    }
    virtual ~TablePart()
...
template <typename T> bool SerializeParts(const std::vector<T>& vec, OTSStream* out)
{
...
template <typename T> bool SerializeParts(const std::vector<std::vector<T>>& vec, OTSStream* out)
{
...
template <typename T> size_t datasize(std::vector<T> vec)
{
