...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    while (provider.remaining_bytes() > 0) {
        ...
        case AppendableApi::GetAppendBuffer: {
            ...
            char16_t* out_buffer = strAppendable.getAppendBuffer(min_capacity,
                min_capacity + provider.ConsumeIntegralInRange<int32_t>(0, kMaxAdditionalDesiredSize), scratch_buf, kScratchBufSize, &out_capacity);
