...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    const char* converter_name = ucnv_getAvailableName(rng() % ucnv_countAvailable());

    deleted_unique_ptr<UConverter> converter(ucnv_open(converter_name, &status), &ucnv_close);
