...
UTF16CollationIterator::UTF16CollationIterator(const UTF16CollationIterator& other, const UChar* newText)
...
UBool UTF16CollationIterator::foundNULTerminator()
{
    if (limit == NULL) {
        limit = --pos;
        return TRUE;
...
FCDUTF16CollationIterator::FCDUTF16CollationIterator(const FCDUTF16CollationIterator& other, const UChar* newText)
...
bool FCDUTF16CollationIterator::operator==(const CollationIterator& other) const
{
    ...
    if (checkDir != 0 || start == segmentStart) {
        ...
        return (segmentStart - rawStart) == (o.segmentStart - o.rawStart) && (pos - start) == (o.pos - o.start);
    }
}
    ...
    for (;;) {
        if (checkDir > 0) {
            ...
            if (CollationFCD::hasTccc(c)) {
                if (CollationFCD::maybeTibetanCompositeVowel(c) || (pos != limit && CollationFCD::hasLccc(*pos))) {
                    --pos;
...
UBool FCDUTF16CollationIterator::foundNULTerminator()
{
    if (limit == NULL) {
        limit = rawLimit = --pos;
        return TRUE;
    ...
    for (;;) {
        if (checkDir < 0) {
            ...
            if (CollationFCD::hasLccc(c)) {
                if (CollationFCD::maybeTibetanCompositeVowel(c) || (pos != start && CollationFCD::hasTccc(*(pos - 1)))) {
                    ++pos;
...
UBool FCDUTF16CollationIterator::nextSegment(UErrorCode& errorCode)
{
...
UBool FCDUTF16CollationIterator::previousSegment(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
    ...
    for (;;) {
        ...
        if (trailCC != 0 && ((nextCC != 0 && trailCC > nextCC) || CollationFCD::isFCD16OfTibetanCompositeVowel(fcd16))) {
            ...
            do {
                q = p;
            } while (fcd16 > 0xff && p != rawStart && (fcd16 = nfcImpl.previousFCD16(rawStart, p)) != 0);
...
UBool FCDUTF16CollationIterator::normalize(const UChar* from, const UChar* to, UErrorCode& errorCode)
{
