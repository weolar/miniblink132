...
public:
    ...
    void disableFastLatin()
    {
        fastLatinEnabled = false;
    }

    CollationTailoring* parseAndBuild(const UnicodeString& ruleString, const UVersionInfo rulesVersion, CollationRuleParser::Importer* importer,
...
private:
    ...
    virtual void addReset(int32_t strength, const UnicodeString& str, const char*& errorReason, UErrorCode& errorCode) override;
    /**
    ...
    int64_t getSpecialResetPosition(const UnicodeString& str, const char*& parserErrorReason, UErrorCode& errorCode);

    /** Implements CollationRuleParser::Sink. */
    virtual void addRelation(int32_t strength, const UnicodeString& prefix, const UnicodeString& str, const UnicodeString& extension, const char*& errorReason,
     ...
     * for the CE + strength.
     */
    int32_t findOrInsertNodeForCEs(int32_t strength, const char*& parserErrorReason, UErrorCode& errorCode);
    ...
    int32_t findOrInsertWeakNode(int32_t index, uint32_t weight16, int32_t level, UErrorCode& errorCode);
     ...
     * @return the new node's index
     */
    int32_t insertNodeBetween(int32_t index, int32_t nextIndex, int64_t node, UErrorCode& errorCode);
    ...
    void setCaseBits(const UnicodeString& nfdString, const char*& parserErrorReason, UErrorCode& errorCode);

    /** Implements CollationRuleParser::Sink. */
    virtual void suppressContractions(const UnicodeSet& set, const char*& parserErrorReason, UErrorCode& errorCode) override;

    /** Implements CollationRuleParser::Sink. */
    virtual void optimize(const UnicodeSet& set, const char*& parserErrorReason, UErrorCode& errorCode) override;
    ...
    uint32_t addOnlyClosure(
        const UnicodeString& nfdPrefix, const UnicodeString& nfdString, const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32, UErrorCode& errorCode);
    void addTailComposites(const UnicodeString& nfdPrefix, const UnicodeString& nfdString, UErrorCode& errorCode);
    UBool mergeCompositeIntoString(const UnicodeString& nfdString, int32_t indexAfterLastStarter, UChar32 composite, const UnicodeString& decomp,
    ...
    static UBool sameCEs(const int64_t ces1[], int32_t ces1Length, const int64_t ces2[], int32_t ces2Length);
    ...
    static inline int32_t indexFromTempCE(int64_t tempCE)
    {
        ...
        return ((int32_t)(tempCE >> 43) & 0xfe000) | ((int32_t)(tempCE >> 42) & 0x1fc0) | ((int32_t)(tempCE >> 24) & 0x3f);
    }
    ...
    {
        tempCE32 -= 0x40400620;
        return ((int32_t)(tempCE32 >> 11) & 0xfe000) | ((int32_t)(tempCE32 >> 10) & 0x1fc0) | ((int32_t)(tempCE32 >> 8) & 0x3f);
    }
    static inline UBool isTempCE32(uint32_t ce32)
    {
        return (ce32 & 0xff) >= 2 && // not a long-primary/long-secondary CE32
