...
static inline uint32_t getWeightTrail(uint32_t weight, int32_t length)
{
    return (uint32_t)(weight >> (8 * (4 - length))) & 0xff;
}

static inline uint32_t setWeightTrail(uint32_t weight, int32_t length, uint32_t trail)
{
    length = 8 * (4 - length);
    return (uint32_t)((weight & (0xffffff00 << length)) | (trail << length));
}

static inline uint32_t getWeightByte(uint32_t weight, int32_t idx)
{
    return getWeightTrail(weight, idx); /* same calculation */
}

static inline uint32_t setWeightByte(uint32_t weight, int32_t idx, uint32_t byte)
{
...
static inline uint32_t truncateWeight(uint32_t weight, int32_t length)
{
    return (uint32_t)(weight & (0xffffffff << (8 * (4 - length))));
}

static inline uint32_t incWeightTrail(uint32_t weight, int32_t length)
{
    return (uint32_t)(weight + (1UL << (8 * (4 - length))));
}

static inline uint32_t decWeightTrail(uint32_t weight, int32_t length)
{
    return (uint32_t)(weight - (1UL << (8 * (4 - length))));
}
...
void CollationWeights::initForPrimary(UBool compressible)
{
    middleLength = 1;
...
void CollationWeights::initForSecondary()
{
...
void CollationWeights::initForTertiary()
{
...
uint32_t CollationWeights::incWeight(uint32_t weight, int32_t length) const
{
    for (;;) {
...
uint32_t CollationWeights::incWeightByOffset(uint32_t weight, int32_t length, int32_t offset) const
{
    for (;;) {
...
void CollationWeights::lengthenRange(WeightRange& range) const
{
...
static int32_t U_CALLCONV compareRanges(const void* /*context*/, const void* left, const void* right)
{
    uint32_t l, r;
...
UBool CollationWeights::getWeightRanges(uint32_t lowerLimit, uint32_t upperLimit)
{
        ...
        for (int32_t length = 4; length > middleLength; --length) {
            if (lower[length].count > 0 && upper[length].count > 0) {
                ...
                if (lowerEnd > upperStart) {
                    ...
                    U_ASSERT(truncateWeight(lowerEnd, length - 1) == truncateWeight(upperStart, length - 1));
...
UBool CollationWeights::allocWeightsInShortRanges(int32_t n, int32_t minLength)
{
...
UBool CollationWeights::allocWeightsInMinLengthRanges(int32_t n, int32_t minLength)
{
    ...
    for (minLengthRangeCount = 0; minLengthRangeCount < rangeCount && ranges[minLengthRangeCount].length == minLength; ++minLengthRangeCount) {
 ...
 * two limits
 */
UBool CollationWeights::allocWeights(uint32_t lowerLimit, uint32_t upperLimit, int32_t n)
{
#ifdef UCOL_DEBUG
    puts("");
#endif
...
uint32_t CollationWeights::nextWeight()
{
