...
TimeUnitFormat::TimeUnitFormat(const TimeUnitFormat& other)
    ...
    , fStyle(other.fStyle)
{
    for (TimeUnit::UTimeUnitFields i = TimeUnit::UTIMEUNIT_YEAR; i < TimeUnit::UTIMEUNIT_FIELD_COUNT; i = (TimeUnit::UTimeUnitFields)(i + 1)) {
...
TimeUnitFormat* TimeUnitFormat::clone() const
{
...
void TimeUnitFormat::parseObject(const UnicodeString& source, Formattable& result, ParsePosition& pos) const
{
    ...
    for (TimeUnit::UTimeUnitFields i = TimeUnit::UTIMEUNIT_YEAR; i < TimeUnit::UTIMEUNIT_FIELD_COUNT; i = (TimeUnit::UTimeUnitFields)(i + 1)) {
        ...
        while ((elem = countToPatterns->nextElement(elemPos)) != NULL) {
            ...
            for (UTimeUnitFormatStyle style = UTMUTFMT_FULL_STYLE; style < UTMUTFMT_FORMAT_STYLE_COUNT; style = (UTimeUnitFormatStyle)(style + 1)) {
...
void TimeUnitFormat::create(UTimeUnitFormatStyle style, UErrorCode& status)
{
...
struct TimeUnitFormatReadSink : public ResourceSink {
    ...
    TimeUnitFormatReadSink(TimeUnitFormat* timeUnitFormatObj, const UVector& pluralCounts, UTimeUnitFormatStyle style)
        : timeUnitFormatObj(timeUnitFormatObj)
    ...
    virtual void put(const char* key, ResourceValue& value, UBool, UErrorCode& errorCode) override
    {
        ...
        for (int32_t i = 0; units.getKeyAndValue(i, key, value); ++i) {
            ...
            for (int32_t j = 0; countsToPatternTable.getKeyAndValue(j, key, value); ++j) {
                ...
                LocalPointer<MessageFormat> messageFormat(new MessageFormat(pattern, timeUnitFormatObj->getLocale(errorCode), errorCode), errorCode);
                if (U_FAILURE(errorCode)) {
                    return;
                }
...
void TimeUnitFormat::readFromCurrentLocale(UTimeUnitFormatStyle style, const char* key, const UVector& pluralCounts, UErrorCode& err)
{
    if (U_FAILURE(err)) {
        return;
    }
...
void TimeUnitFormat::checkConsistency(UTimeUnitFormatStyle style, const char* key, UErrorCode& err)
{
    if (U_FAILURE(err)) {
        return;
    }
    ...
    while (U_SUCCESS(err) && (pluralCount = keywords->snext(err)) != NULL) {
        for (int32_t i = 0; i < TimeUnit::UTIMEUNIT_FIELD_COUNT; ++i) {
            ...
            if (formatters == NULL || formatters[style] == NULL) {
                ...
                searchInLocaleChain(style, key, localeName, (TimeUnit::UTimeUnitFields)i, *pluralCount, pluralCountChars.data(), countToPatterns, err);
            }
...
void TimeUnitFormat::searchInLocaleChain(UTimeUnitFormatStyle style, const char* key, const char* localeName, TimeUnit::UTimeUnitFields srcTimeUnitField,
    const UnicodeString& srcPluralCount, const char* searchPluralCount, Hashtable* countToPatterns, UErrorCode& err)
{
    if (U_FAILURE(err)) {
        return;
    }
    ...
    while ((locNameLen = uloc_getParent(parentLocale, parentLocale, ULOC_FULLNAME_CAPACITY, &status)) >= 0) {
        ...
        if (U_SUCCESS(status)) {
            // found
            LocalPointer<MessageFormat> messageFormat(new MessageFormat(UnicodeString(TRUE, pattern, ptLength), getLocale(err), err), err);
            if (U_FAILURE(err)) {
                return;
            }
    ...
    if (locNameLen == 0 && uprv_strcmp(key, gShortUnitsTag) == 0) {
        ...
        searchInLocaleChain(style, gUnitsTag, pLocale.data(), srcTimeUnitField, srcPluralCount, searchPluralCount, countToPatterns, err);
        if (U_FAILURE(err)) {
            return;
        }
        MessageFormat** formatters = (MessageFormat**)countToPatterns->get(srcPluralCount);
