...
UBool CollationIterator::CEBuffer::ensureAppendCapacity(int32_t appCap, UErrorCode& errorCode)
{
...
bool CollationIterator::operator==(const CollationIterator& other) const
{
    ...
    if (!(typeid(*this) == typeid(other) && ceBuffer.length == other.ceBuffer.length && cesIndex == other.cesIndex && numCpFwd == other.numCpFwd
            && isNumeric == other.isNumeric)) {
        return false;
    }
    for (int32_t i = 0; i < ceBuffer.length; ++i) {
        if (ceBuffer.get(i) != other.ceBuffer.get(i)) {
            return false;
        }
    }
    return true;
...
uint32_t CollationIterator::handleNextCE32(UChar32& c, UErrorCode& errorCode)
{
...
UBool CollationIterator::foundNULTerminator()
{
    return FALSE;
}

UBool CollationIterator::forbidSurrogateCodePoints() const
{
    return FALSE;
}

uint32_t CollationIterator::getDataCE32(UChar32 c) const
{
    return data->getCE32(c);
}

uint32_t CollationIterator::getCE32FromBuilderData(uint32_t /*ce32*/, UErrorCode& errorCode)
{
...
int64_t CollationIterator::nextCEFromCE32(const CollationData* d, UChar32 c, uint32_t ce32, UErrorCode& errorCode)
{
...
void CollationIterator::appendCEsFromCE32(const CollationData* d, UChar32 c, uint32_t ce32, UBool forward, UErrorCode& errorCode)
{
    while (Collation::isSpecialCE32(ce32)) {
        ...
        case Collation::CONTRACTION_TAG: {
            ...
            if (skipped == NULL && numCpFwd < 0) {
                ...
                if (nextCp < 0) {
                    ...
                    ce32 = defaultCE32;
                    break;
                } else if ((ce32 & Collation::CONTRACT_NEXT_CCC) != 0 && !CollationFCD::mayHaveLccc(nextCp)) {
        ...
        case Collation::LEAD_SURROGATE_TAG: {
            ...
            if (U16_IS_TRAIL(trail = handleGetTrailSurrogate())) {
                ...
                } else if (ce32 == Collation::LEAD_ALL_FALLBACK || (ce32 = d->getCE32FromSupplementary(c)) == Collation::FALLBACK_CE32) {
...
uint32_t CollationIterator::getCE32FromPrefix(const CollationData* d, uint32_t ce32, UErrorCode& errorCode)
{
    const UChar* p = d->contexts + Collation::indexFromCE32(ce32);
...
uint32_t CollationIterator::nextCE32FromContraction(
    const CollationData* d, uint32_t contractionCE32, const UChar* p, uint32_t ce32, UChar32 c, UErrorCode& errorCode)
{
    ...
    for (;;) {
        ...
        if (USTRINGTRIE_HAS_VALUE(match)) {
            ...
            if ((contractionCE32 & Collation::CONTRACT_TRAILING_CCC) != 0 &&
                ...
                ((contractionCE32 & Collation::CONTRACT_SINGLE_CP_NO_MATCH) == 0 || sinceMatch < lookAhead)) {
                ...
                if (d->getFCD16(c) > 0xff) {
                    return nextCE32FromDiscontiguousContraction(d, suffixes, ce32, lookAhead, c, errorCode);
                }
            }
            break;
        } else {
...
uint32_t CollationIterator::nextCE32FromDiscontiguousContraction(
    const CollationData* d, UCharsTrie& suffixes, uint32_t ce32, int32_t lookAhead, UChar32 c, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
...
void CollationIterator::appendNumericCEs(uint32_t ce32, UBool forward, UErrorCode& errorCode)
{
    // Collect digits.
    CharString digits;
    if (forward) {
    ...
    do {
        // Skip leading zeros.
        while (pos < (digits.length() - 1) && digits[pos] == 0) {
            ++pos;
        }
...
void CollationIterator::appendNumericSegmentCEs(const char* digits, int32_t length, UErrorCode& errorCode)
{
    ...
    if (length <= 7) {
        ...
        if (value < numBytes * 254) {
            ...
            uint32_t primary = numericPrimary | ((firstByte + value / 254) << 16) | ((2 + value % 254) << 8);
            ceBuffer.append(Collation::makeCE(primary), errorCode);
            return;
        }
        value -= numBytes * 254;
...
int64_t CollationIterator::previousCE(UVector32& offsets, UErrorCode& errorCode)
{
...
int64_t CollationIterator::previousCEUnsafe(UChar32 c, UVector32& offsets, UErrorCode& errorCode)
{
