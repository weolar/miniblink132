...
ScriptRunIterator::ScriptRunIterator(const Replaceable& theText, int32_t myStart, int32_t myLimit)
    : text(theText)
{
    textStart = myStart;
    textLimit = myLimit;
...
AnyTransliterator::AnyTransliterator(
    const UnicodeString& id, const UnicodeString& theTarget, const UnicodeString& theVariant, UScriptCode theTargetScript, UErrorCode& ec)
 ...
 */
void AnyTransliterator::handleTransliterate(Replaceable& text, UTransPosition& pos, UBool isIncremental) const
{
...
void AnyTransliterator::registerIDs()
{
    ...
    for (int32_t s = 0; s < sourceCount; ++s) {
        ...
        if (source.caseCompare(ANY, 3, 0 /*U_FOLD_CASE_DEFAULT*/) == 0)
            continue;
        ...
        for (int32_t t = 0; t < targetCount; ++t) {
            ...
            for (int32_t v = 0; v < variantCount; ++v) {
                ...
                ec = U_ZERO_ERROR;
                AnyTransliterator* tl = new AnyTransliterator(id, target, variant, targetScript, ec);
                if (U_FAILURE(ec)) {
                    delete tl;
                } else {
