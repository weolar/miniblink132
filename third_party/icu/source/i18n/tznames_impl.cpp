...
void CharacterNode::addValue(void* value, UObjectDeleter* valueDeleter, UErrorCode& status)
{
    ...
    if (fValues == NULL) {
        ...
        if (!fHasValuesVector) {
            ...
            LocalPointer<UVector> values(new UVector(valueDeleter, NULL, DEFAULT_CHARACTERNODE_CAPACITY, status), status);
            if (U_FAILURE(status)) {
                if (valueDeleter) {
                    valueDeleter(value);
                }
...
void TextTrieMap::search(const UnicodeString& text, int32_t start, TextTrieMapSearchResultHandler* handler, UErrorCode& status) const
{
    {
...
void TextTrieMap::search(
    CharacterNode* node, const UnicodeString& text, int32_t start, int32_t index, TextTrieMapSearchResultHandler* handler, UErrorCode& status) const
{
    if (U_FAILURE(status)) {
        return;
    }
...
ZNStringPool::ZNStringPool(UErrorCode& status)
{
    ...
    fHash = uhash_open(uhash_hashUChars /* keyHash */, uhash_compareUChars /* keyComp */, uhash_compareUChars /* valueComp */, &status);
    if (U_FAILURE(status)) {
        return;
    }
}
...
private:
    ...
    static void* createTimeZoneAndPutInCache(UHashtable* cache, const UChar* names[], const UnicodeString& tzID, UErrorCode& status)
    {
        if (U_FAILURE(status)) {
            return NULL;
        }
    ...
    void addNamesIntoTrie(const UChar* mzID, const UChar* tzID, TextTrieMap& trie, UErrorCode& status)
    {
        if (U_FAILURE(status)) {
            return;
        }
    ...
    static UTimeZoneNameTypeIndex nameTypeFromKey(const char* key)
    {
        ...
        if (c0 == 'l') {
            ...
            return c1 == 'g' ? UTZNM_INDEX_SHORT_GENERIC
                ...
                : c1 == 'd'  ? UTZNM_INDEX_SHORT_DAYLIGHT
                             : UTZNM_INDEX_UNKNOWN;
        } else if (c0 == 'e' && c1 == 'c') {
            return UTZNM_INDEX_EXEMPLAR_LOCATION;
        }
...
MetaZoneIDsEnumeration::MetaZoneIDsEnumeration(LocalPointer<UVector> mzIDs)
    : fLen(0)
    , fPos(0)
    , fMetaZoneIDs(nullptr)
    , fLocalVector(std::move(mzIDs))
{
...
UBool ZNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode* node, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
...
UnicodeString& TimeZoneNamesImpl::getMetaZoneDisplayName(const UnicodeString& mzID, UTimeZoneNameType type, UnicodeString& name) const
{
...
TimeZoneNames::MatchInfoCollection* TimeZoneNamesImpl::find(const UnicodeString& text, int32_t start, uint32_t types, UErrorCode& status) const
{
...
struct TimeZoneNamesImpl::ZoneStringsLoader : public ResourceSink {
    ...
    void consumeNamesTable(const char* key, ResourceValue& value, UBool noFallback, UErrorCode& status)
    {
        if (U_FAILURE(status)) {
            return;
        }
    ...
    virtual void put(const char* key, ResourceValue& value, UBool noFallback, UErrorCode& status) override
    {
...
void TimeZoneNamesImpl::getDisplayNames(
...
{
    if (tzID.isEmpty() || tzID.startsWith(gEtcPrefix, gEtcPrefixLen) || tzID.startsWith(gSystemVPrefix, gSystemVPrefixLen)
        || tzID.indexOf(gRiyadh8, gRiyadh8Len, 0) > 0) {
        name.setToBogus();
        return name;
    }
    ...
    if (sep > 0 && sep + 1 < tzID.length()) {
        ...
        name.findAndReplace(UnicodeString((UChar)0x5f /* _ */), UnicodeString((UChar)0x20 /* space */));
    } else {
        name.setToBogus();
    }
    return name;
...
UBool TZDBNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode* node, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
    ...
    if (node->hasValues()) {
        ...
        if (match != NULL) {
            ...
            if (match->ambiguousType && (ntype == UTZNM_SHORT_STANDARD || ntype == UTZNM_SHORT_DAYLIGHT) && (fTypes & UTZNM_SHORT_STANDARD) != 0
...
UnicodeString& TZDBTimeZoneNames::getTimeZoneDisplayName(const UnicodeString& /* tzID */, UTimeZoneNameType /* type */, UnicodeString& name) const
{
