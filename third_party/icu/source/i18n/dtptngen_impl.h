...
typedef enum dtStrEnum { DT_BASESKELETON, DT_SKELETON, DT_PATTERN } dtStrEnum;
...
{
    return (uprv_memcmp(chars, other.chars, sizeof(chars)) == 0 && uprv_memcmp(lengths, other.lengths, sizeof(lengths)) == 0);
}
...
public:
    ...
    static int32_t getCanonicalIndex(const UnicodeString& s)
    {
        return getCanonicalIndex(s, true);
    }
    static int32_t getCanonicalIndex(const UnicodeString& s, UBool strict);

private:
    typedef enum TokenStatus { START, ADD_TOKEN, SYNTAX_ERROR, DONE } TokenStatus;
...
public:
    ...
    void addMissing(int32_t field)
    {
        missingFieldMask |= (1 << field);
    }
    void addExtra(int32_t field)
    {
