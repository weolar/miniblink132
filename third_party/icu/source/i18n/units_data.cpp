...
public:
     ...
     */
    explicit ConversionRateDataSink(MaybeStackVector<ConversionRateInfo>* out)
        : outVector(out)
    {
    }
    ...
    void put(const char* source, ResourceValue& value, UBool /*noFallback*/, UErrorCode& status) override
    {
        ...
        for (int32_t unit = 0; conversionRateTable.getKeyAndValue(unit, srcUnit, value); unit++) {
            ...
            if (!cr) {
                ...
                if (!offset.isBogus())
                    cr->offset.appendInvariantChars(offset, status);
            }
        }
        return;
    }
...
public:
     ...
     */
    explicit UnitPreferencesSink(MaybeStackVector<UnitPreference>* outPrefs, MaybeStackVector<UnitPreferenceMetadata>* outMetadata)
        ...
        for (int32_t i = 0; unitPreferenceDataTable.getKeyAndValue(i, category, value); i++) {
            ...
            for (int32_t j = 0; categoryTable.getKeyAndValue(j, usage, value); j++) {
                ...
                for (int32_t k = 0; regionTable.getKeyAndValue(k, region, value); k++) {
                    ...
                    UnitPreferenceMetadata* meta = metadata->emplaceBack(category, usage, region, preferences->length(), prefLen, status);
                    if (!meta) {
                        status = U_MEMORY_ALLOCATION_ERROR;
                        return;
                    }
                    ...
                    if (metadata->length() > 1) {
                        ...
                        // without duplicates.
                        if (!(*(*metadata)[metadata->length() - 2] < *(*metadata)[metadata->length() - 1])) {
...
int32_t binarySearch(const MaybeStackVector<UnitPreferenceMetadata>* metadata, const UnitPreferenceMetadata& desired, bool* foundCategory, bool* foundUsage,
 ...
 */
int32_t getPreferenceMetadataIndex(
    const MaybeStackVector<UnitPreferenceMetadata>* metadata, StringPiece category, StringPiece usage, StringPiece region, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return -1;
    }
...
UnitPreferenceMetadata::UnitPreferenceMetadata(
    StringPiece category, StringPiece usage, StringPiece region, int32_t prefsOffset, int32_t prefsCount, UErrorCode& status)
{
...
// of some kind.
void U_I18N_API UnitPreferences::getPreferencesFor(StringPiece category, StringPiece usage, StringPiece region, const UnitPreference* const*& outPreferences,
