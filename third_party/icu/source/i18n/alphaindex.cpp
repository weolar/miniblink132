...
UBool isOneLabelBetterThanOther(const Normalizer2& nfkdNormalizer, const UnicodeString& one, const UnicodeString& other);
...
public:
    ...
    int32_t getBucketIndex(const UnicodeString& name, const Collator& collatorPrimaryOnly, UErrorCode& errorCode)
    {
        ...
        while ((start + 1) < limit) {
            ...
            UCollationResult nameVsBucket = collatorPrimaryOnly.compare(name, bucket->lowerBoundary_, errorCode);
            if (nameVsBucket < 0) {
                limit = i;
            } else {
                start = i;
...
void AlphabeticIndex::initLabels(UVector& indexCharacters, UErrorCode& errorCode) const
{
    ...
    while (U_SUCCESS(errorCode) && iter.next()) {
        ...
        if (collatorPrimaryOnly_->compare(*item, firstScriptBoundary, errorCode) < 0) {
        ...
        } else if (checkDistinct && collatorPrimaryOnly_->compare(*item, separated(*item), errorCode) == 0) {
            ...
            if (insertionPoint < 0) {
                indexCharacters.insertElementAt(ownedString(*item, ownedItem, errorCode), ~insertionPoint, errorCode);
            } else {
                ...
                if (isOneLabelBetterThanOther(*nfkdNormalizer, *item, itemAlreadyIn)) {
                    indexCharacters.setElementAt(ownedString(*item, ownedItem, errorCode), insertionPoint);
                }
            }
        }
    }
...
UBool hasMultiplePrimaryWeights(const RuleBasedCollator& coll, uint32_t variableTop, const UnicodeString& s, UVector64& ces, UErrorCode& errorCode)
{
...
BucketList* AlphabeticIndex::createBucketList(UErrorCode& errorCode) const
{
    ...
    Bucket* asciiBuckets[26] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
        NULL, NULL, NULL, NULL, NULL };
    Bucket* pinyinBuckets[26] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
        NULL, NULL, NULL, NULL, NULL };
    ...
    for (int32_t i = 0; i < indexCharacters.size(); ++i) {
        ...
        } else if (current.length() == BASE_LENGTH + 1 && current.startsWith(BASE, BASE_LENGTH) && 0x41 <= (c = current.charAt(BASE_LENGTH)) && c <= 0x5A) {
        ...
        if (!current.startsWith(BASE, BASE_LENGTH) && hasMultiplePrimaryWeights(*collatorPrimaryOnly_, variableTop, current, ces, errorCode)
            ...
            for (int32_t j = bucketList->size() - 2;; --j) {
                ...
                if (singleBucket->displayBucket_ == NULL
                    && !hasMultiplePrimaryWeights(*collatorPrimaryOnly_, variableTop, singleBucket->lowerBoundary_, ces, errorCode)) {
                    ...
                    bucket.adoptInsteadAndCheckErrorCode(
                        new Bucket(emptyString_, UnicodeString(current).append((UChar)0xFFFF), U_ALPHAINDEX_NORMAL), errorCode);
                    if (U_FAILURE(errorCode)) {
                        return NULL;
                    }
...
void AlphabeticIndex::initBuckets(UErrorCode& errorCode)
{
    ...
    for (int32_t i = 0; i < inputList_->size(); ++i) {
        ...
        while (upperBoundary != NULL && collatorPrimaryOnly_->compare(r->name_, *upperBoundary, errorCode) >= 0) {
...
void AlphabeticIndex::init(const Locale* locale, UErrorCode& status)
{
    ...
    for (;;) {
        ...
        if (collatorPrimaryOnly_->compare(*static_cast<UnicodeString*>(firstCharsInScripts_->elementAt(0)), emptyString_, status) == UCOL_EQUAL) {
