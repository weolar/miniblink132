...
void number::impl::parseIncrementOption(const StringSegment& segment, Precision& outPrecision, UErrorCode& status)
{
...
{
    if (minFractionPlaces >= 0 && maxFractionPlaces <= kMaxIntFracSig && minFractionPlaces <= maxFractionPlaces) {
...
{
    if (minSignificantDigits >= 1 && maxSignificantDigits <= kMaxIntFracSig && minSignificantDigits <= maxSignificantDigits) {
...
Precision FractionPrecision::withSignificantDigits(int32_t minSignificantDigits, int32_t maxSignificantDigits, UNumberRoundingPriority priority) const
{
    if (fType == RND_ERROR) {
        return *this;
    } // no-op in error state
    if (minSignificantDigits >= 1 && maxSignificantDigits >= minSignificantDigits && maxSignificantDigits <= kMaxIntFracSig) {
...
Precision FractionPrecision::withMinDigits(int32_t minSignificantDigits) const
{
    ...
    if (minSignificantDigits >= 1 && minSignificantDigits <= kMaxIntFracSig) {
        return constructFractionSignificant(*this, 1, minSignificantDigits, UNUM_ROUNDING_PRIORITY_RELAXED, true);
    } else {
        return { U_NUMBER_ARG_OUTOFBOUNDS_ERROR };
    }
}
...
Precision FractionPrecision::withMaxDigits(int32_t maxSignificantDigits) const
{
    ...
    if (maxSignificantDigits >= 1 && maxSignificantDigits <= kMaxIntFracSig) {
        return constructFractionSignificant(*this, 1, maxSignificantDigits, UNUM_ROUNDING_PRIORITY_STRICT, true);
    } else {
        return { U_NUMBER_ARG_OUTOFBOUNDS_ERROR };
    }
}
...
Precision Precision::withCurrency(const CurrencyUnit& currency, UErrorCode& status) const
{
    ...
    int32_t minMaxFrac = ucurr_getDefaultFractionDigitsForUsage(isoCode, fUnion.currencyUsage, &status);
    Precision retval = (increment != 0.0) ? Precision::increment(increment) : static_cast<Precision>(Precision::fixedFraction(minMaxFrac));
...
Precision Precision::constructFractionSignificant(const FractionPrecision& base, int32_t minSig, int32_t maxSig, UNumberRoundingPriority priority, bool retain)
{
...
RoundingImpl::RoundingImpl(const Precision& precision, UNumberFormatRoundingMode roundingMode, const CurrencyUnit& currency, UErrorCode& status)
...
int32_t RoundingImpl::chooseMultiplierAndApply(impl::DecimalQuantity& input, const impl::MultiplierProducer& producer, UErrorCode& status)
{
...
void RoundingImpl::apply(impl::DecimalQuantity& value, UErrorCode& status) const
{
    ...
    case Precision::RND_FRACTION:
        value.roundToMagnitude(getRoundingMagnitudeFraction(fPrecision.fUnion.fracSig.fMaxFrac), fRoundingMode, status);
        resolvedMinFraction = uprv_max(0, -getDisplayMagnitudeFraction(fPrecision.fUnion.fracSig.fMinFrac));
        break;

    case Precision::RND_SIGNIFICANT:
        value.roundToMagnitude(getRoundingMagnitudeSignificant(value, fPrecision.fUnion.fracSig.fMaxSig), fRoundingMode, status);
        resolvedMinFraction = uprv_max(0, -getDisplayMagnitudeSignificant(value, fPrecision.fUnion.fracSig.fMinSig));
    ...
    case Precision::RND_INCREMENT:
        value.roundToIncrement(fPrecision.fUnion.increment.fIncrement, fPrecision.fUnion.increment.fIncrementMagnitude, fRoundingMode, status);
        resolvedMinFraction = fPrecision.fUnion.increment.fMinFrac;
        break;

    case Precision::RND_INCREMENT_ONE:
        value.roundToMagnitude(fPrecision.fUnion.increment.fIncrementMagnitude, fRoundingMode, status);
        resolvedMinFraction = fPrecision.fUnion.increment.fMinFrac;
        break;

    case Precision::RND_INCREMENT_FIVE:
        value.roundToNickel(fPrecision.fUnion.increment.fIncrementMagnitude, fRoundingMode, status);
        resolvedMinFraction = fPrecision.fUnion.increment.fMinFrac;
        break;
