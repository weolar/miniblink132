...
U_CAPI void U_CALLCONV uprv_deleteConditionalCE32(void* obj)
{
    ...
    , pos(0)
{
    ...
    for (int32_t j = 0; j < CollationData::JAMO_CE32S_LENGTH; ++j) { // Count across Jamo types.
        ...
        jamoCE32s[j] = Collation::makeCE32FromTagAndIndex(Collation::BUILDER_DATA_TAG, jamo) | CollationDataBuilder::IS_BUILDER_JAMO_CE32;
    }
...
UBool CollationDataBuilder::maybeSetPrimaryRange(UChar32 start, UChar32 end, uint32_t primary, int32_t step, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
uint32_t CollationDataBuilder::setPrimaryRangeAndReturnNext(UChar32 start, UChar32 end, uint32_t primary, int32_t step, UErrorCode& errorCode)
{
    ...
    if (maybeSetPrimaryRange(start, end, primary, step, errorCode)) {
        return Collation::incThreeBytePrimaryByOffset(primary, isCompressible, (end - start + 1) * step);
    } else {
...
uint32_t CollationDataBuilder::getCE32FromOffsetCE32(UBool fromBase, UChar32 c, uint32_t ce32) const
{
...
UBool CollationDataBuilder::isCompressibleLeadByte(uint32_t b) const
{
    return base->isCompressibleLeadByte(b);
}

UBool CollationDataBuilder::isAssigned(UChar32 c) const
{
...
uint32_t CollationDataBuilder::encodeCEs(const int64_t ces[], int32_t cesLength, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
    ...
    if (cesLength == 0) {
        ...
        if ((ce0 & INT64_C(0xffffffffff00ff)) == Collation::COMMON_SECONDARY_CE && (ce1 & INT64_C(0xffffffff00ffffff)) == Collation::COMMON_TERTIARY_CE
            && p0 != 0) {
            // Latin mini expansion
            return p0 | (((uint32_t)ce0 & 0xff00u) << 8) | (uint32_t)(ce1 >> 16) | Collation::SPECIAL_CE32_LOW_BYTE | Collation::LATIN_EXPANSION_TAG;
        }
    }
...
uint32_t CollationDataBuilder::copyFromBaseCE32(UChar32 c, uint32_t ce32, UBool withContext, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
    ...
    case Collation::EXPANSION32_TAG: {
        ...
        ce32 = encodeExpansion32(reinterpret_cast<const int32_t*>(baseCE32s), length, errorCode);
        break;
    }
    case Collation::EXPANSION_TAG: {
        ...
        if (!Collation::isSpecialCE32(ce32)) {
            ...
            if (tag == Collation::EXPANSION32_TAG) {
                ...
                for (int32_t i = 0; i < length; ++i) {
                    ce32 = srcCE32s[i];
                    int64_t ce;
                    if (Collation::isSpecialCE32(ce32) || (ce = modifier.modifyCE32(ce32)) == Collation::NO_CE) {
                        if (isModified) {
                ...
                if (isModified) {
                    ...
                    ce32 = dest.encodeExpansion32(reinterpret_cast<const int32_t*>(srcCE32s), length, errorCode);
                }
                ...
                int32_t destIndex = dest.addConditionalCE32(cond->context, copyCE32(cond->ce32), errorCode);
                ...
                U_ASSERT(tag == Collation::LONG_PRIMARY_TAG || tag == Collation::LONG_SECONDARY_TAG || tag == Collation::LATIN_EXPANSION_TAG
...
static UBool U_CALLCONV enumRangeForCopy(const void* context, UChar32 start, UChar32 end, uint32_t value)
{
...
UBool CollationDataBuilder::getJamoCE32s(uint32_t jamoCE32s[], UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
static UBool U_CALLCONV enumRangeLeadValue(const void* context, UChar32 /*start*/, UChar32 /*end*/, uint32_t value)
{
...
void CollationDataBuilder::setLeadSurrogates(UErrorCode& errorCode)
{
    for (UChar lead = 0xd800; lead < 0xdc00; ++lead) {
        ...
        utrie2_set32ForLeadSurrogateCodeUnit(trie, lead, Collation::makeCE32FromTagAndIndex(Collation::LEAD_SURROGATE_TAG, 0) | (uint32_t)value, &errorCode);
    }
}
...
uint32_t CollationDataBuilder::buildContext(ConditionalCE32* head, UErrorCode& errorCode)
{
    ...
    for (ConditionalCE32* cond = head;; cond = getConditionalCE32(cond->next)) {
        if (U_FAILURE(errorCode)) {
            return 0;
        } // early out for memory allocation errors
        ...
        } while (cond->next >= 0 && (cond = getConditionalCE32(cond->next))->context.startsWith(prefix));
        uint32_t ce32;
        ...
        if (lastCond->context.length() == suffixStart) {
            ...
            if (firstCond->context.length() == suffixStart) {
                ...
                for (cond = head;; cond = getConditionalCE32(cond->next)) {
                    ...
                    if (cond->defaultCE32 != Collation::NO_CE32 && (length == 0 || prefix.endsWith(cond->context, 1, length))) {
...
void CollationDataBuilder::buildFastLatinTable(CollationData& data, UErrorCode& errorCode)
{
    ...
    if (fastLatinBuilder->forData(data, errorCode)) {
        ...
        if (base != NULL && length == base->fastLatinTableLength && uprv_memcmp(table, base->fastLatinTable, length * 2) == 0) {
