...
void IntegerWidth::apply(impl::DecimalQuantity& quantity, UErrorCode& status) const
{
    ...
    if (fHasError) {
        ...
        if (fUnion.minMaxInt.fFormatFailIfMoreThanMaxDigits && fUnion.minMaxInt.fMaxInt < quantity.getMagnitude()) {
            status = U_ILLEGAL_ARGUMENT_ERROR;
        }
        quantity.setMinInteger(fUnion.minMaxInt.fMinInt);
...
bool IntegerWidth::operator==(const IntegerWidth& other) const
{
    ...
    return fUnion.minMaxInt.fMinInt == other.fUnion.minMaxInt.fMinInt && fUnion.minMaxInt.fMaxInt == other.fUnion.minMaxInt.fMaxInt;
}
