...
U_NAMESPACE_BEGIN namespace number
{
    ...
    public:
        ...
        void populate(const Locale& locale, const char* nsName, CompactStyle compactStyle, CompactType compactType, UErrorCode& status);

        int32_t getMultiplier(int32_t magnitude) const U_OVERRIDE;

        const UChar* getPattern(int32_t magnitude, const PluralRules* rules, const DecimalQuantity& dq) const;
    ...
    private:
        ...
        public:
            explicit CompactDataSink(CompactData& data)
                : data(data)
            {
            }
    ...
    public:
        CompactHandler(CompactStyle compactStyle, const Locale& locale, const char* nsName, CompactType compactType, const PluralRules* rules,
