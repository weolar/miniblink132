 ...
 */
static void U_CALLCONV deleteOlsonToMetaMappingEntry(void* obj)
{
...
static const UChar gDefaultFrom[]
    = { 0x31, 0x39, 0x37, 0x30, 0x2D, 0x30, 0x31, 0x2D, 0x30, 0x31, 0x20, 0x30, 0x30, 0x3A, 0x30, 0x30, 0x00 }; // "1970-01-01 00:00"
static const UChar gDefaultTo[]
    = { 0x39, 0x39, 0x39, 0x39, 0x2D, 0x31, 0x32, 0x2D, 0x33, 0x31, 0x20, 0x32, 0x33, 0x3A, 0x35, 0x39, 0x00 }; // "9999-12-31 23:59"
...
static UDate parseDate(const UChar* text, UErrorCode& status)
{
    ...
    if (U_SUCCESS(status)) {
        UDate date = Grego::fieldsToDay(year, month - 1, day) * U_MILLIS_PER_DAY + hour * U_MILLIS_PER_HOUR + min * U_MILLIS_PER_MINUTE;
        return date;
    }
    return 0;
}
...
UnicodeString& U_EXPORT2 ZoneMeta::getCanonicalCountry(const UnicodeString& tzid, UnicodeString& country, UBool* isPrimary /* = NULL */)
{
    if (isPrimary != NULL) {
        *isPrimary = FALSE;
    }
...
UnicodeString& U_EXPORT2 ZoneMeta::getMetazoneID(const UnicodeString& tzid, UDate date, UnicodeString& result)
{
    UBool isSet = FALSE;
        ...
        if (U_SUCCESS(status)) {
            ...
            while (ures_hasNext(rb)) {
                ...
                if (mzMappings.isNull()) {
                    mzMappings.adoptInsteadAndCheckErrorCode(new UVector(deleteOlsonToMetaMappingEntry, nullptr, status), status);
                    if (U_FAILURE(status)) {
                        break;
                    }
                }
...
UnicodeString& ZoneMeta::formatCustomID(uint8_t hour, uint8_t min, uint8_t sec, UBool negative, UnicodeString& id)
{
