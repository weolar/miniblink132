...
public:
    ...
    virtual void getRules(const char* localeID, const char* collationType, UnicodeString& rules, const char*& errorReason, UErrorCode& errorCode) override;
};
...
void BundleImporter::getRules(const char* localeID, const char* collationType, UnicodeString& rules, const char*& /*errorReason*/, UErrorCode& errorCode)
{
...
RuleBasedCollator::RuleBasedCollator(const UnicodeString& rules, ECollationStrength strength, UErrorCode& errorCode)
    : data(NULL)
    , settings(NULL)
    , tailoring(NULL)
    , cacheEntry(NULL)
...
RuleBasedCollator::RuleBasedCollator(const UnicodeString& rules, UColAttributeValue decompositionMode, UErrorCode& errorCode)
    : data(NULL)
    , settings(NULL)
    , tailoring(NULL)
    , cacheEntry(NULL)
...
RuleBasedCollator::RuleBasedCollator(const UnicodeString& rules, ECollationStrength strength, UColAttributeValue decompositionMode, UErrorCode& errorCode)
    : data(NULL)
    , settings(NULL)
    , tailoring(NULL)
    , cacheEntry(NULL)
...
RuleBasedCollator::RuleBasedCollator(const UnicodeString& rules, UParseError& parseError, UnicodeString& reason, UErrorCode& errorCode)
    : data(NULL)
    , settings(NULL)
    , tailoring(NULL)
    , cacheEntry(NULL)
...
void RuleBasedCollator::internalBuildTailoring(const UnicodeString& rules, int32_t strength, UColAttributeValue decompositionMode, UParseError* outParseError,
    UnicodeString* outReason, UErrorCode& errorCode)
{
    ...
    LocalPointer<CollationTailoring> t(builder.parseAndBuild(rules, noVersion, &importer, outParseError, errorCode));
    if (U_FAILURE(errorCode)) {
...
CollationTailoring* CollationBuilder::parseAndBuild(const UnicodeString& ruleString, const UVersionInfo rulesVersion, CollationRuleParser::Importer* importer,
    UParseError* outParseError, UErrorCode& errorCode)
{
    ...
    ownedSettings.fastLatinOptions
        = CollationFastLatin::getOptions(tailoring->data, ownedSettings, ownedSettings.fastLatinPrimaries, UPRV_LENGTHOF(ownedSettings.fastLatinPrimaries));
...
int64_t CollationBuilder::getSpecialResetPosition(const UnicodeString& str, const char*& parserErrorReason, UErrorCode& errorCode)
{
    ...
    case CollationRuleParser::LAST_REGULAR:
        ...
        ce = rootElements.firstCEWithPrimaryAtLeast(baseData->getFirstPrimaryForGroup(USCRIPT_HAN));
        break;
...
void CollationBuilder::addRelation(int32_t strength, const UnicodeString& prefix, const UnicodeString& str, const UnicodeString& extension,
    const char*& parserErrorReason, UErrorCode& errorCode)
{
    ...
    if (!extension.isEmpty()) {
        ...
        if (cesLength > Collation::MAX_EXPANSION_LENGTH) {
            errorCode = U_ILLEGAL_ARGUMENT_ERROR;
            parserErrorReason = "extension string adds too many collation elements (more than 31 total)";
            return;
        }
    }
    uint32_t ce32 = Collation::UNASSIGNED_CE32;
    if ((prefix != nfdPrefix || str != nfdString) && !ignorePrefix(prefix, errorCode) && !ignoreString(str, errorCode)) {
...
int32_t CollationBuilder::findOrInsertNodeForCEs(int32_t strength, const char*& parserErrorReason, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
...
int32_t CollationBuilder::findOrInsertNodeForRootCE(int64_t ce, int32_t strength, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
    ...
    if (strength >= UCOL_SECONDARY) {
        ...
        if (strength >= UCOL_TERTIARY) {
            index = findOrInsertWeakNode(index, lower32 & Collation::ONLY_TERTIARY_MASK, UCOL_TERTIARY, errorCode);
        }
    }
    return index;
}
 ...
 */
int32_t binarySearchForRootPrimaryNode(const int32_t* rootPrimaryIndexes, int32_t length, const int64_t* nodes, uint32_t p)
{
    if (length == 0) {
        return ~0;
    }
...
int32_t CollationBuilder::findOrInsertNodeForPrimary(uint32_t p, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }

    int32_t rootIndex = binarySearchForRootPrimaryNode(rootPrimaryIndexes.getBuffer(), rootPrimaryIndexes.size(), nodes.getBuffer(), p);
    if (rootIndex >= 0) {
...
int32_t CollationBuilder::insertNodeBetween(int32_t index, int32_t nextIndex, int64_t node, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return 0;
    }
...
int32_t CollationBuilder::findCommonNode(int32_t index, int32_t strength) const
{
    ...
    node = nodes.elementAti(index);
    U_ASSERT(!isTailoredNode(node) && strengthFromNode(node) == strength && weight16FromNode(node) < Collation::COMMON_WEIGHT16);
    // Skip to the explicit common node.
    do {
        index = nextIndexFromNode(node);
        node = nodes.elementAti(index);
        U_ASSERT(strengthFromNode(node) >= strength);
    } while (isTailoredNode(node) || strengthFromNode(node) > strength || weight16FromNode(node) < Collation::COMMON_WEIGHT16);
    U_ASSERT(weight16FromNode(node) == Collation::COMMON_WEIGHT16);
    return index;
}

void CollationBuilder::setCaseBits(const UnicodeString& nfdString, const char*& parserErrorReason, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
    ...
    for (int32_t i = 0; i < cesLength; ++i) {
        if (ceStrength(ces[i]) == UCOL_PRIMARY) {
...
void CollationBuilder::suppressContractions(const UnicodeSet& set, const char*& parserErrorReason, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
...
void CollationBuilder::optimize(const UnicodeSet& set, const char*& /* parserErrorReason */, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
...
uint32_t CollationBuilder::addWithClosure(
    const UnicodeString& nfdPrefix, const UnicodeString& nfdString, const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32, UErrorCode& errorCode)
{
...
uint32_t CollationBuilder::addOnlyClosure(
    const UnicodeString& nfdPrefix, const UnicodeString& nfdString, const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return ce32;
    ...
    if (nfdPrefix.isEmpty()) {
        ...
        for (;;) {
            ...
            if (ignoreString(str, errorCode) || str == nfdString) {
                continue;
            }
        ...
        for (;;) {
            ...
            if (ignorePrefix(prefix, errorCode)) {
                continue;
            }
            ...
            for (;;) {
                ...
                if (ignoreString(str, errorCode) || (samePrefix && str == nfdString)) {
                    continue;
                }
                ce32 = addIfDifferent(prefix, str, newCEs, newCEsLength, ce32, errorCode);
                if (U_FAILURE(errorCode)) {
...
void CollationBuilder::addTailComposites(const UnicodeString& nfdPrefix, const UnicodeString& nfdString, UErrorCode& errorCode)
{
    ...
    while (iter.next()) {
        ...
        if (!mergeCompositeIntoString(nfdString, indexAfterLastStarter, composite, decomp, newNFDString, newString, errorCode)) {
            continue;
        }
        ...
        uint32_t ce32 = addIfDifferent(nfdPrefix, newString, newCEs, newCEsLength, Collation::UNASSIGNED_CE32, errorCode);
...
UBool CollationBuilder::mergeCompositeIntoString(const UnicodeString& nfdString, int32_t indexAfterLastStarter, UChar32 composite, const UnicodeString& decomp,
    UnicodeString& newNFDString, UnicodeString& newString, UErrorCode& errorCode) const
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
    ...
    if (nfdString.compare(indexAfterLastStarter, 0x7fffffff, decomp, lastStarterLength, 0x7fffffff) == 0) {
...
UBool CollationBuilder::ignorePrefix(const UnicodeString& s, UErrorCode& errorCode) const
{
    // Do not map non-FCD prefixes.
    return !isFCD(s, errorCode);
}

UBool CollationBuilder::ignoreString(const UnicodeString& s, UErrorCode& errorCode) const
{
...
UBool CollationBuilder::isFCD(const UnicodeString& s, UErrorCode& errorCode) const
{
    return U_SUCCESS(errorCode) && fcd.isNormalized(s, errorCode);
}

void CollationBuilder::closeOverComposites(UErrorCode& errorCode)
{
...
uint32_t CollationBuilder::addIfDifferent(
    const UnicodeString& prefix, const UnicodeString& str, const int64_t newCEs[], int32_t newCEsLength, uint32_t ce32, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return ce32;
    }
...
UBool CollationBuilder::sameCEs(const int64_t ces1[], int32_t ces1Length, const int64_t ces2[], int32_t ces2Length)
{
...
uint32_t alignWeightRight(uint32_t w)
{
    if (w != 0) {
        while ((w & 0xff) == 0) {
            w >>= 8;
...
void CollationBuilder::makeTailoredCEs(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
                ...
                if (strength == UCOL_TERTIARY) {
                    if (isTailoredNode(node)) {
                        ...
                        if (!tIsTailored) {
                            ...
                            int32_t tCount = countTailoredNodes(nodesArray, nextIndex, UCOL_TERTIARY) + 1;
                            uint32_t tLimit;
                            if (t == 0) {
                    ...
                    if (strength == UCOL_SECONDARY) {
                        if (isTailoredNode(node)) {
                            ...
                            if (!sIsTailored) {
                                ...
                                int32_t sCount = countTailoredNodes(nodesArray, nextIndex, UCOL_SECONDARY) + 1;
                                uint32_t sLimit;
                                if (s == 0) {
                        ...
                        if (!pIsTailored) {
                            ...
                            int32_t pCount = countTailoredNodes(nodesArray, nextIndex, UCOL_PRIMARY) + 1;
                            UBool isCompressible = baseData->isCompressiblePrimary(p);
                            uint32_t pLimit = rootElements.getPrimaryAfter(p, pIndex, isCompressible);
...
void CollationBuilder::finalizeCEs(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
...
int32_t CollationBuilder::ceStrength(int64_t ce)
{
    return isTempCE(ce)                           ? strengthFromTempCE(ce)
...
U_CAPI UCollator* U_EXPORT2 ucol_openRules(
    const UChar* rules, int32_t rulesLength, UColAttributeValue normalizationMode, UCollationStrength strength, UParseError* parseError, UErrorCode* pErrorCode)
{
...
U_CAPI int32_t U_EXPORT2 ucol_getUnsafeSet(const UCollator* coll, USet* unsafe, UErrorCode* status)
{
    ...
    static const UChar cccpattern[25] = { 0x5b, 0x5b, 0x3a, 0x5e, 0x74, 0x63, 0x63, 0x63, 0x3d, 0x30, 0x3a, 0x5d, 0x5b, 0x3a, 0x5e, 0x6c, 0x63, 0x63, 0x63,
        0x3d, 0x30, 0x3a, 0x5d, 0x5d, 0x00 };
