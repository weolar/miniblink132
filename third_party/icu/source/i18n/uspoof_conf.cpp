...
SPUStringPool::SPUStringPool(UErrorCode& status)
    : fVec(nullptr)
    , fHash(nullptr)
{
    ...
    vec->setDeleter([](void* obj) { delete (SPUString*)obj; });
    fVec = vec.orphan();
...
void ConfusabledataBuilder::buildConfusableData(
    SpoofImpl* spImpl, const char* confusables, int32_t confusablesLen, int32_t* errorType, UParseError* pe, UErrorCode& status)
{
...
void ConfusabledataBuilder::build(const char* confusables, int32_t confusablesLen, UErrorCode& status)
{
    ...
    UnicodeString pattern("(?m)^[ \\t]*([0-9A-Fa-f]+)[ \\t]+;" // Match the source char
    ...
    while (uregex_findNext(fParseLine, &status)) {
        ...
        UChar32 keyChar = SpoofImpl::ScanHex(fInput, uregex_start(fParseLine, 1, &status), uregex_end(fParseLine, 1, &status), status);
        ...
        while (uregex_findNext(fParseHexNum, &status)) {
            UChar32 c = SpoofImpl::ScanHex(&fInput[mapStringStart], uregex_start(fParseHexNum, 1, &status), uregex_end(fParseHexNum, 1, &status), status);
            mapString->append(c);
        }
    ...
    for (int32_t range = 0; range < fKeySet->getRangeCount(); range++) {
        ...
        for (UChar32 keyChar = fKeySet->getRangeStart(range); keyChar <= fKeySet->getRangeEnd(range); keyChar++) {
            ...
            int32_t key = ConfusableDataUtils::codePointAndLengthToKey(keyChar, targetMapping->fStr->length());
...
void ConfusabledataBuilder::outputData(UErrorCode& status)
{
    ...
    int32_t* keys = static_cast<int32_t*>(fSpoofImpl->fSpoofData->reserveSpace(numKeys * sizeof(int32_t), status));
    if (U_FAILURE(status)) {
        return;
    }
    int i;
    ...
    uint16_t* values = static_cast<uint16_t*>(fSpoofImpl->fSpoofData->reserveSpace(numKeys * sizeof(uint16_t), status));
    if (U_FAILURE(status)) {
        return;
    }
    ...
    UChar* strings = static_cast<UChar*>(fSpoofImpl->fSpoofData->reserveSpace(stringsLength * sizeof(UChar) + 2, status));
    if (U_FAILURE(status)) {
        return;
    }
