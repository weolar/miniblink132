...
static UBool U_CALLCONV enumTailoredRange(const void* context, UChar32 start, UChar32 end, uint32_t ce32)
{
...
UBool TailoredSet::handleCE32(UChar32 start, UChar32 end, uint32_t ce32)
{
...
void TailoredSet::compare(UChar32 c, uint32_t ce32, uint32_t baseCE32)
{
    ...
    if (tag == Collation::EXPANSION32_TAG) {
        ...
        if (tailored->contains(jamos[0]) || tailored->contains(jamos[1]) || (length == 3 && tailored->contains(jamos[2]))) {
            add(c);
        }
...
void TailoredSet::addPrefix(const CollationData* d, const UnicodeString& pfx, UChar32 c, uint32_t ce32)
{
    setPrefix(pfx);
...
void TailoredSet::addSuffix(UChar32 c, const UnicodeString& sfx)
{
    tailored->add(UnicodeString(unreversedPrefix).append(c).append(sfx));
}

void TailoredSet::add(UChar32 c)
{
...
static UBool U_CALLCONV enumCnERange(const void* context, UChar32 start, UChar32 end, uint32_t ce32)
{
...
void ContractionsAndExpansions::forData(const CollationData* d, UErrorCode& ec)
{
    if (U_FAILURE(ec)) {
        return;
    }
...
void ContractionsAndExpansions::forCodePoint(const CollationData* d, UChar32 c, UErrorCode& ec)
{
    if (U_FAILURE(ec)) {
        return;
    }
...
void ContractionsAndExpansions::handleCE32(UChar32 start, UChar32 end, uint32_t ce32)
{
    for (;;) {
...
void ContractionsAndExpansions::addStrings(UChar32 start, UChar32 end, UnicodeSet* set)
{
    if (set == NULL) {
        return;
    }
