...
int32_t CollationFastLatin::getOptions(const CollationData* data, const CollationSettings& settings, uint16_t* primaries, int32_t capacity)
{
    ...
    if (settings.hasReordering()) {
        ...
        for (int32_t group = UCOL_REORDER_CODE_FIRST; group < UCOL_REORDER_CODE_FIRST + CollationData::MAX_NUM_SPECIAL_REORDER_CODES; ++group) {
...
int32_t CollationFastLatin::compareUTF16(
...
int32_t CollationFastLatin::compareUTF8(
    ...
    for (;;) {
        ...
        while (leftPair == 0) {
            ...
            if (c <= 0x7f) {
                ...
                leftPair = table[c];
            } else if (c <= LATIN_MAX_UTF8_LEAD && 0xc2 <= c && leftIndex != leftLength && 0x80 <= (t = left[leftIndex]) && t <= 0xbf) {
                ++leftIndex;
                c = ((c - 0xc2) << 6) + t;
                leftPair = primaries[c];
                if (leftPair != 0) {
        ...
        while (rightPair == 0) {
            ...
            if (c <= 0x7f) {
                ...
                rightPair = table[c];
            } else if (c <= LATIN_MAX_UTF8_LEAD && 0xc2 <= c && rightIndex != rightLength && 0x80 <= (t = right[rightIndex]) && t <= 0xbf) {
                ++rightIndex;
                c = ((c - 0xc2) << 6) + t;
                rightPair = primaries[c];
                if (rightPair != 0) {
...
uint32_t CollationFastLatin::lookupUTF8(const uint16_t* table, UChar32 c, const uint8_t* s8, int32_t& sIndex, int32_t sLength)
{
...
uint32_t CollationFastLatin::lookupUTF8Unsafe(const uint16_t* table, UChar32 c, const uint8_t* s8, int32_t& sIndex)
{
...
uint32_t CollationFastLatin::nextPair(const uint16_t* table, UChar32 c, uint32_t ce, const UChar* s16, const uint8_t* s8, int32_t& sIndex, int32_t& sLength)
{
    if (ce >= MIN_LONG || ce < CONTRACTION) {
        ...
        if (sIndex != sLength) {
            ...
            if (s16 != NULL) {
                ...
                if (c2 > 0x7f) {
                    uint8_t t;
                    if (c2 <= 0xc5 && 0xc2 <= c2 && nextIndex != sLength && 0x80 <= (t = s8[nextIndex]) && t <= 0xbf) {
...
uint32_t CollationFastLatin::getCases(uint32_t variableTop, UBool strengthIsPrimary, uint32_t pair)
{
...
uint32_t CollationFastLatin::getTertiaries(uint32_t variableTop, UBool withCaseBits, uint32_t pair)
{
    if (pair <= 0xffff) {
        // one mini CE
        if (pair >= MIN_SHORT) {
