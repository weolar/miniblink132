...
void CollationRuleParser::parse(const UnicodeString& ruleString, UErrorCode& errorCode)
{
    ...
    while (ruleIndex < rules->length()) {
        ...
        case 0x40: // '@' is equivalent to [backwards 2]
            settings->setFlag(CollationSettings::BACKWARD_SECONDARY, UCOL_ON, 0, errorCode);
            ++ruleIndex;
            break;
...
void CollationRuleParser::parseRuleChain(UErrorCode& errorCode)
{
...
int32_t CollationRuleParser::parseResetAndPosition(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return UCOL_DEFAULT;
    }
    ...
    if (rules->compare(i, BEFORE_LENGTH, BEFORE, 0, BEFORE_LENGTH) == 0 && (j = i + BEFORE_LENGTH) < rules->length()
        && PatternProps::isWhiteSpace(rules->charAt(j)) && ((j = skipWhiteSpace(j + 1)) + 1) < rules->length() && 0x31 <= (c = rules->charAt(j)) && c <= 0x33
...
int32_t CollationRuleParser::parseRelationOperator(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return UCOL_DEFAULT;
    }
...
void CollationRuleParser::parseRelationStrings(int32_t strength, int32_t i, UErrorCode& errorCode)
{
    // Parse
    ...
    if (!prefix.isEmpty()) {
        ...
        if (!nfc.hasBoundaryBefore(prefix0) || !nfc.hasBoundaryBefore(c)) {
            setParseError("in 'prefix|str', prefix and str must each start with an NFC boundary", errorCode);
            return;
        }
    }
...
void CollationRuleParser::parseStarredCharacters(int32_t strength, int32_t i, UErrorCode& errorCode)
{
...
int32_t CollationRuleParser::parseTailoringString(int32_t i, UnicodeString& raw, UErrorCode& errorCode)
{
    i = parseString(skipWhiteSpace(i), raw, errorCode);
...
int32_t CollationRuleParser::parseString(int32_t i, UnicodeString& raw, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return i;
    }
...
static const char* const positions[] = { "first tertiary ignorable", "last tertiary ignorable", "first secondary ignorable", "last secondary ignorable",
    "first primary ignorable", "last primary ignorable", "first variable", "last variable", "first regular", "last regular", "first implicit", "last implicit",
...
void CollationRuleParser::parseSetting(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
    ...
    if (rules->charAt(j) == 0x5d) { // words end with ]
        ++j;
        if (raw.startsWith(UNICODE_STRING_SIMPLE("reorder")) && (raw.length() == 7 || raw.charAt(7) == 0x20)) {
        ...
        if (raw == UNICODE_STRING_SIMPLE("strength") && v.length() == 1) {
            ...
            if (value != UCOL_DEFAULT) {
                ...
                settings->variableTop = baseData->getLastPrimaryForGroup(UCOL_REORDER_CODE_FIRST + value);
            ...
            int32_t length = uloc_forLanguageTag(lang.data(), localeID, ULOC_FULLNAME_CAPACITY, &parsedLength, &errorCode);
            if (U_FAILURE(errorCode) || parsedLength != lang.length() || length >= ULOC_FULLNAME_CAPACITY) {
            ...
            length = uloc_getKeywordValue(localeID, "collation", collationType, ULOC_KEYWORDS_CAPACITY, &errorCode);
            if (U_FAILURE(errorCode) || length >= ULOC_KEYWORDS_CAPACITY) {
                errorCode = U_ZERO_ERROR;
                setParseError("expected language tag in [import langTag]", errorCode);
                return;
...
static const char* const gSpecialReorderCodes[] = { "space", "punct", "symbol", "currency", "digit" };
...
UColAttributeValue CollationRuleParser::getOnOffValue(const UnicodeString& s)
{
...
int32_t CollationRuleParser::parseUnicodeSet(int32_t i, UnicodeSet& set, UErrorCode& errorCode)
{
...
UBool CollationRuleParser::isSyntaxChar(UChar32 c)
{
