...
char* SortKeyByteSink::GetAppendBuffer(int32_t min_capacity, int32_t desired_capacity_hint, char* scratch, int32_t scratch_capacity, int32_t* result_capacity)
{
...
UBool CollationKeys::LevelCallback::needToWrite(Collation::Level /*level*/)
{
    return TRUE;
}
...
void CollationKeys::writeSortKeyUpToQuaternary(CollationIterator& iter, const UBool* compressibleBytes, const CollationSettings& settings,
    SortKeyByteSink& sink, Collation::Level minLevel, LevelCallback& callback, UBool preflight, UErrorCode& errorCode)
{
    ...
    for (;;) {
        ...
        if ((levels & Collation::SECONDARY_LEVEL_FLAG) != 0) {
            ...
            if (s == 0) {
                // secondary ignorable
            } else if (s == Collation::COMMON_WEIGHT16 && ((options & CollationSettings::BACKWARD_SECONDARY) == 0 || p != Collation::MERGE_SEPARATOR_PRIMARY)) {
                ...
                if (0 < p && p <= Collation::MERGE_SEPARATOR_PRIMARY) {
                    ...
                    secondaries.appendByte(p == Collation::NO_CE_PRIMARY ? Collation::LEVEL_SEPARATOR_BYTE : Collation::MERGE_SEPARATOR_BYTE);
                    prevSecondary = 0;
        ...
        if ((levels & Collation::CASE_LEVEL_FLAG) != 0) {
            if ((CollationSettings::getStrength(options) == UCOL_PRIMARY) ? p == 0 : lower32 <= 0xffff) {
                ...
                if ((c & 0xc0) == 0 && c > Collation::LEVEL_SEPARATOR_BYTE) {
                    ...
                    if ((options & CollationSettings::UPPER_FIRST) == 0) {
                        ...
                        if (commonCases != 0 && (c > Collation::LEVEL_SEPARATOR_BYTE || !cases.isEmpty())) {
                            --commonCases;
        ...
        if ((levels & Collation::QUATERNARY_LEVEL_FLAG) != 0) {
            ...
            if ((q & 0xc0) == 0 && q > Collation::NO_CE_WEIGHT16) {
                ++commonQuaternaries;
            } else if (q == Collation::NO_CE_WEIGHT16 && (options & CollationSettings::ALTERNATE_MASK) == 0 && quaternaries.isEmpty()) {
