...
void UIterCollationIterator::resetToOffset(int32_t newOffset)
{
    reset();
    iter.move(&iter, newOffset, UITER_START);
}

int32_t UIterCollationIterator::getOffset() const
{
    return iter.getIndex(&iter, UITER_CURRENT);
}

uint32_t UIterCollationIterator::handleNextCE32(UChar32& c, UErrorCode& /*errorCode*/)
{
    c = iter.next(&iter);
    if (c < 0) {
...
UChar32 UIterCollationIterator::previousCodePoint(UErrorCode& /*errorCode*/)
{
    return uiter_previous32(&iter);
}

void UIterCollationIterator::forwardNumCodePoints(int32_t num, UErrorCode& /*errorCode*/)
{
...
void UIterCollationIterator::backwardNumCodePoints(int32_t num, UErrorCode& /*errorCode*/)
{
...
void FCDUIterCollationIterator::resetToOffset(int32_t newOffset)
{
...
int32_t FCDUIterCollationIterator::getOffset() const
{
...
uint32_t FCDUIterCollationIterator::handleNextCE32(UChar32& c, UErrorCode& errorCode)
{
    for (;;) {
        if (state == ITER_CHECK_FWD) {
            ...
            if (CollationFCD::hasTccc(c)) {
                if (CollationFCD::maybeTibetanCompositeVowel(c) || CollationFCD::hasLccc(iter.current(&iter))) {
                    iter.previous(&iter);
...
UChar32 FCDUIterCollationIterator::nextCodePoint(UErrorCode& errorCode)
{
    UChar32 c;
    for (;;) {
        if (state == ITER_CHECK_FWD) {
...
UChar32 FCDUIterCollationIterator::previousCodePoint(UErrorCode& errorCode)
{
    UChar32 c;
    for (;;) {
        if (state == ITER_CHECK_BWD) {
            ...
            if (CollationFCD::hasLccc(c)) {
                ...
                if (CollationFCD::maybeTibetanCompositeVowel(c) || CollationFCD::hasTccc(prev = iter.previous(&iter))) {
                    iter.next(&iter);
                    if (prev >= 0) {
                        iter.next(&iter);
                    }
...
void FCDUIterCollationIterator::forwardNumCodePoints(int32_t num, UErrorCode& errorCode)
{
...
void FCDUIterCollationIterator::backwardNumCodePoints(int32_t num, UErrorCode& errorCode)
{
    // Specify the class to avoid a virtual-function indirection.
    // In Java, we would declare this class final.
...
{
    U_ASSERT(state == ITER_CHECK_BWD || (state == ITER_IN_FCD_SEGMENT && pos == limit) || (state >= IN_NORM_ITER_AT_LIMIT && pos == normalized.length()));
    if (state == ITER_CHECK_BWD) {
...
UBool FCDUIterCollationIterator::nextSegment(UErrorCode& errorCode)
{
...
{
    U_ASSERT(state == ITER_CHECK_FWD || (state == ITER_IN_FCD_SEGMENT && pos == start) || (state >= IN_NORM_ITER_AT_LIMIT && pos == 0));
    if (state == ITER_CHECK_FWD) {
...
UBool FCDUIterCollationIterator::previousSegment(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UBool FCDUIterCollationIterator::normalize(const UnicodeString& s, UErrorCode& errorCode)
{
