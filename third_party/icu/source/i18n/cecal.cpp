...
CECalendar& CECalendar::operator=(const CECalendar& right)
{
...
int32_t CECalendar::handleComputeMonthStart(int32_t eyear, int32_t emonth, UBool /*useMonth*/) const
{
    return ceToJD(eyear, emonth, 0, getJDEpochOffset());
}

int32_t CECalendar::handleGetLimit(UCalendarDateFields field, ELimitType limitType) const
{
    return LIMITS[field][limitType];
}

UBool CECalendar::inDaylightTime(UErrorCode& status) const
{
...
UBool CECalendar::haveDefaultCentury() const
{
    return TRUE;
}
...
int32_t CECalendar::ceToJD(int32_t year, int32_t month, int32_t date, int32_t jdEpochOffset)
{
    ...
    return (int32_t)(jdEpochOffset // difference from Julian epoch to 1,1,1
...
void CECalendar::jdToCE(int32_t julianDay, int32_t jdEpochOffset, int32_t& year, int32_t& month, int32_t& day)
{
