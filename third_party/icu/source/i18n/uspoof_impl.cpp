...
URestrictionLevel SpoofImpl::getRestrictionLevel(const UnicodeString& input, UErrorCode& status) const
{
    ...
    // Section 5.2 step 6:
    if (resolvedNoLatn.test(USCRIPT_HAN_WITH_BOPOMOFO, status) || resolvedNoLatn.test(USCRIPT_JAPANESE, status)
        || resolvedNoLatn.test(USCRIPT_KOREAN, status)) {
        return USPOOF_HIGHLY_RESTRICTIVE;
    }

    // Section 5.2 step 7:
    if (!resolvedNoLatn.isEmpty() && !resolvedNoLatn.test(USCRIPT_CYRILLIC, status) && !resolvedNoLatn.test(USCRIPT_GREEK, status)
...
{
    return cp == u'i' || cp == u'j' || cp == u'ı' || cp == u'ȷ' || cp == u'l' || u_hasBinaryProperty(cp, UCHAR_SOFT_DOTTED);
}
...
{
    if (U_FAILURE(status) || fRawData == NULL || fRawData->fMagic != USPOOF_MAGIC || fRawData->fFormatVersion[0] != USPOOF_CONFUSABLE_DATA_FORMAT_VERSION
...
static UBool U_CALLCONV spoofDataIsAcceptable(void* context, const char* /* type */, const char* /*name*/, const UDataInfo* pInfo)
{
    if (pInfo->size >= 20 && pInfo->isBigEndian == U_IS_BIG_ENDIAN && pInfo->charsetFamily == U_CHARSET_FAMILY && pInfo->dataFormat[0] == 0x43
        && // dataFormat="Cfu "
        pInfo->dataFormat[1] == 0x66 && pInfo->dataFormat[2] == 0x75 && pInfo->dataFormat[3] == 0x20
...
static UBool U_CALLCONV uspoof_cleanupDefaultData(void)
{
...
{
    UDataMemory* udm = udata_openChoice(nullptr, "cfu", "confusables", spoofDataIsAcceptable,
...
//
//-----------------------------------------------------------------------------
U_CAPI int32_t U_EXPORT2 uspoof_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* status)
{
    ...
    if (!(pInfo->dataFormat[0] == 0x43 && /* dataFormat="Cfu " */
            pInfo->dataFormat[1] == 0x66 && pInfo->dataFormat[2] == 0x75 && pInfo->dataFormat[3] == 0x20
            && pInfo->formatVersion[0] == USPOOF_CONFUSABLE_DATA_FORMAT_VERSION && pInfo->formatVersion[1] == 0 && pInfo->formatVersion[2] == 0
    ...
    if (ds->readUInt32(spoofDH->fMagic) != USPOOF_MAGIC || ds->readUInt32(spoofDH->fLength) < sizeof(SpoofDataHeader)) {
