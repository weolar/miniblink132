...
UBool CollationFastLatinBuilder::forData(const CollationData& data, UErrorCode& errorCode)
{
    ...
    UBool ok = !shortPrimaryOverflow && encodeCharCEs(errorCode) && encodeContractions(errorCode);
    contractionCEs.removeAllElements(); // might reduce heap memory usage
    uniqueCEs.removeAllElements();
    return ok;
}

UBool CollationFastLatinBuilder::loadGroups(const CollationData& data, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UBool CollationFastLatinBuilder::inSameGroup(uint32_t p, uint32_t q) const
{
...
UBool CollationFastLatinBuilder::getCEsFromCE32(const CollationData& data, UChar32 c, uint32_t ce32, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UBool CollationFastLatinBuilder::getCEsFromContractionCE32(const CollationData& data, uint32_t ce32, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
void CollationFastLatinBuilder::addContractionEntry(int32_t x, int64_t cce0, int64_t cce1, UErrorCode& errorCode)
{
...
UBool CollationFastLatinBuilder::encodeUniqueCEs(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UBool CollationFastLatinBuilder::encodeCharCEs(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UBool CollationFastLatinBuilder::encodeContractions(UErrorCode& errorCode)
{
    ...
    for (int32_t i = 0; i < CollationFastLatin::NUM_FAST_CHARS; ++i) {
        ...
        // However, that seems unlikely.
        result.setCharAt(headerLength + i, (UChar)(CollationFastLatin::CONTRACTION | contractionIndex));
    }
...
uint32_t CollationFastLatinBuilder::encodeTwoCEs(int64_t first, int64_t second) const
{
    ...
    if (miniCE >= CollationFastLatin::MIN_SHORT && (miniCE & CollationFastLatin::SECONDARY_MASK) == CollationFastLatin::COMMON_SEC) {
        ...
        if (sec1 >= CollationFastLatin::MIN_SEC_HIGH && case1 == 0 && ter1 == CollationFastLatin::COMMON_TER) {
