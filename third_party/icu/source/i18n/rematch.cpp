...
RegexMatcher::RegexMatcher(const UnicodeString& regexp, const UnicodeString& input, uint32_t flags, UErrorCode& status)
{
    init(status);
    if (U_FAILURE(status)) {
        return;
...
RegexMatcher::RegexMatcher(UText* regexp, UText* input, uint32_t flags, UErrorCode& status)
{
    init(status);
    if (U_FAILURE(status)) {
        return;
...
RegexMatcher::RegexMatcher(const UnicodeString& regexp, uint32_t flags, UErrorCode& status)
{
    init(status);
    if (U_FAILURE(status)) {
        return;
...
UBool RegexMatcher::find(UErrorCode& status)
{
    ...
    case START_SET: {
        ...
        for (;;) {
            ...
            if (c >= 0 && ((c < 256 && fPattern->fInitialChars8->contains(c)) || (c >= 256 && fPattern->fInitialChars->contains(c)))) {
...
UBool RegexMatcher::findUsingChunk(UErrorCode& status)
{
    ...
    case START_SET: {
        ...
        for (;;) {
            ...
            if ((c < 256 && fPattern->fInitialChars8->contains(c)) || (c >= 256 && fPattern->fInitialChars->contains(c))) {
...
//   split, UText mode
//
int32_t RegexMatcher::split(UText* input, UText* dest[], int32_t destCapacity, UErrorCode& status)
{
    //
    ...
    for (i = 0;; i++) {
        if (i >= destCapacity - 1) {
            ...
            if (fActiveLimit > nextOutputStringStart) {
                if (UTEXT_FULL_TEXT_IN_CHUNK(input, fInputLength)) {
                    if (dest[i]) {
                        utext_replace(dest[i], 0, utext_nativeLength(dest[i]), input->chunkContents + nextOutputStringStart,
        ...
        if (find()) {
            ...
            for (groupNum = 1; groupNum <= numCaptureGroups; groupNum++) {
                ...
                i++;
                dest[i] = utext_extract_replace(fInputText, dest[i], start64(groupNum, status), end64(groupNum, status), &status);
            }
...
UBool RegexMatcher::isWordBoundary(int64_t pos)
{
    ...
    for (;;) {
        ...
        if (!(u_hasBinaryProperty(prevChar, UCHAR_GRAPHEME_EXTEND) || u_charType(prevChar) == U_FORMAT_CHAR)) {
...
void RegexMatcher::MatchAt(int64_t startIdx, UBool toEnd, UErrorCode& status)
{
    ...
    for (;;) {
        ...
        if (fTraceDebug) {
            ...
            printf("inputIdx=%ld   inputChar=%x   sp=%3ld   activeLimit=%ld  ", fp->fInputIdx, UTEXT_CURRENT32(fInputText), (int64_t*)fp - fStack->getBuffer(),
                fActiveLimit);
        ...
        case URX_LOOP_C: {
            ...
            if (prevC == 0x0a && fp->fInputIdx > backSearchIndex && twoPrevC == 0x0d) {
...
void RegexMatcher::MatchChunkAt(int32_t startIdx, UBool toEnd, UErrorCode& status)
{
    ...
    for (;;) {
        ...
        case URX_DOLLAR: //  $, test for End of line
            ...
            } else if (fp->fInputIdx == fAnchorLimit - 2 && inputBuf[fp->fInputIdx] == 0x0d && inputBuf[fp->fInputIdx + 1] == 0x0a) {
                fHitEnd = TRUE;
                fRequireEnd = TRUE;
                break; // At CR/LF at end of input.  Success
            }
        ...
        case URX_BACKREF: {
            ...
            if (success && groupStartIdx < groupEndIdx && U16_IS_LEAD(inputBuf[groupEndIdx - 1]) && inputIndex < fActiveLimit
                && U16_IS_TRAIL(inputBuf[inputIndex])) {
        ...
        case URX_LOOP_C: {
            ...
            if (prevC == 0x0a && fp->fInputIdx > backSearchIndex && inputBuf[fp->fInputIdx - 1] == 0x0d) {
                int32_t prevOp = (int32_t)pat[fp->fPatIdx - 2];
                if (URX_TYPE(prevOp) == URX_LOOP_DOT_I) {
                    // .*, stepping back over CRLF pair.
