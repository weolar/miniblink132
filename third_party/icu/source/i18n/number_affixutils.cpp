...
int32_t AffixUtils::unescape(
    const UnicodeString& affixPattern, FormattedStringBuilder& output, int32_t position, const SymbolProvider& provider, Field field, UErrorCode& status)
{
    int32_t length = 0;
    AffixTag tag;
    while (hasNext(tag, affixPattern)) {
        ...
        if (tag.type == TYPE_CURRENCY_OVERFLOW) {
            ...
            length += output.insertCodePoint(position + length, 0xFFFD, { UFIELD_CATEGORY_NUMBER, UNUM_CURRENCY_FIELD }, status);
        } else if (tag.type < 0) {
            length += output.insert(position + length, provider.getSymbol(tag.type), getFieldForType(tag.type), status);
        } else {
...
int32_t AffixUtils::unescapedCodePointCount(const UnicodeString& affixPattern, const SymbolProvider& provider, UErrorCode& status)
{
    int32_t length = 0;
    AffixTag tag;
    while (hasNext(tag, affixPattern)) {
...
bool AffixUtils::containsType(const UnicodeString& affixPattern, AffixPatternType type, UErrorCode& status)
{
...
UnicodeString AffixUtils::replaceType(const UnicodeString& affixPattern, AffixPatternType type, char16_t replacementChar, UErrorCode& status)
{
...
bool AffixUtils::containsOnlySymbolsAndIgnorables(const UnicodeString& affixPattern, const UnicodeSet& ignorables, UErrorCode& status)
{
    if (affixPattern.length() == 0) {
        return true;
    }
...
void AffixUtils::iterateWithConsumer(const UnicodeString& affixPattern, TokenConsumer& consumer, UErrorCode& status)
{
    if (affixPattern.length() == 0) {
        return;
    }
...
bool AffixUtils::hasNext(const AffixTag& tag, const UnicodeString& string)
{
    ...
    if (tag.state == STATE_INSIDE_QUOTE && tag.offset == string.length() - 1 && string.charAt(tag.offset) == u'\'') {
        return false;
