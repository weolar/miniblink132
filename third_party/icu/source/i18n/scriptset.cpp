...
void ScriptSet::setScriptExtensions(UChar32 codePoint, UErrorCode& status)
{
    ...
    while (TRUE) {
        script_count = uscript_getScriptExtensions(codePoint, scripts.getAlias(), scripts.getCapacity(), &internalStatus);
...
U_CAPI UBool U_EXPORT2 uhash_equalsScriptSet(const UElement key1, const UElement key2)
{
