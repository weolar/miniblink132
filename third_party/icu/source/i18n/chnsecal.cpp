...
#define U_DEBUG_CHNSECAL_MSG(x)                                                                                                                                \
    {                                                                                                                                                          \
        debug_chnsecal_loc(__FILE__, __LINE__);                                                                                                                \
        debug_chnsecal_msg x;                                                                                                                                  \
    }
#else
#define U_DEBUG_CHNSECAL_MSG(x)
#endif
...
ChineseCalendar::ChineseCalendar(const Locale& aLocale, int32_t epochYear, const TimeZone* zoneAstroCalc, UErrorCode& success)
    : Calendar(TimeZone::forLocaleOrDefault(aLocale), aLocale, success)
    , isLeapYear(FALSE)
...
ChineseCalendar::ChineseCalendar(const ChineseCalendar& other)
    : Calendar(other)
{
...
{
    int32_t thisStart = handleComputeMonthStart(extendedYear, month, TRUE) - kEpochStartAsJulianDay + 1; // Julian day -> local days
...
const UFieldResolutionTable ChineseCalendar::CHINESE_DATE_PRECEDENCE[]
    = { { { UCAL_DAY_OF_MONTH, kResolveSTOP }, { UCAL_WEEK_OF_YEAR, UCAL_DAY_OF_WEEK, kResolveSTOP }, { UCAL_WEEK_OF_MONTH, UCAL_DAY_OF_WEEK, kResolveSTOP },
            ...
            { UCAL_DAY_OF_YEAR, kResolveSTOP }, { kResolveRemap | UCAL_DAY_OF_MONTH, UCAL_IS_LEAP_MONTH, kResolveSTOP }, { kResolveSTOP } },
...
int32_t ChineseCalendar::handleComputeMonthStart(int32_t eyear, int32_t month, UBool useMonth) const
{
    ...
    nonConstThis->computeChineseFields(newMoon, getGregorianYear(), getGregorianMonth(), FALSE);

    if (month != internalGet(UCAL_MONTH) || isLeapMonth != internalGet(UCAL_IS_LEAP_MONTH)) {
...
{
    return majorSolarTerm(newMoon) == majorSolarTerm(newMoonNear(newMoon + SYNODIC_GAP, TRUE));
}
 ...
 */
void ChineseCalendar::computeChineseFields(int32_t days, int32_t gyear, int32_t gmonth, UBool setAllFields)
{
    ...
    UBool isLeapMonth = isLeapYear && hasNoMajorSolarTerm(thisMoon) && !isLeapMonthBetween(firstMoon, newMoonNear(thisMoon - SYNODIC_GAP, FALSE));
...
UBool ChineseCalendar::inDaylightTime(UErrorCode& status) const
{
...
UDate ChineseCalendar::internalGetDefaultCenturyStart() const
{
...
int32_t ChineseCalendar::internalGetDefaultCenturyStartYear() const
{
