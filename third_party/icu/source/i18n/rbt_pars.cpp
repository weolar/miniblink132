...
static const UChar DOT_SET[] = { // "[^[:Zp:][:Zl:]\r\n$]";
    91, 94, 91, 58, 90, 112, 58, 93, 91, 58, 90, 108, 58, 93, 92, 114, 92, 110, 36, 93, 0
};
...
static const UChar gOPERATORS[] = { // "=><"
    VARIABLE_DEF_OP, FORWARD_RULE_OP, REVERSE_RULE_OP, ALT_FORWARD_RULE_OP, ALT_REVERSE_RULE_OP, ALT_FWDREV_RULE_OP, 0
};

static const UChar HALF_ENDERS[] = { // "=><;"
    VARIABLE_DEF_OP, FORWARD_RULE_OP, REVERSE_RULE_OP, ALT_FORWARD_RULE_OP, ALT_REVERSE_RULE_OP, ALT_FWDREV_RULE_OP, END_OF_RULE, 0
};
...
ParseData::ParseData(const TransliterationRuleData* d, const UVector* sets, const Hashtable* vNames)
    : data(d)
...
const UnicodeFunctor* ParseData::lookupMatcher(UChar32 ch) const
{
    ...
    if (i >= 0 && i < variablesVector->size()) {
        ...
        set = (j < variablesVector->size()) ? (UnicodeFunctor*)variablesVector->elementAt(j) : 0;
    }
    return set;
}
 ...
 * name.
 */
UnicodeString ParseData::parseReference(const UnicodeString& text, ParsePosition& pos, int32_t limit) const
{
...
public:
    ...
    int32_t parseSection(
        const UnicodeString& rule, int32_t pos, int32_t limit, UnicodeString& buf, const UnicodeString& illegal, UBool isSegment, UErrorCode& status);
    ...
    int syntaxError(UErrorCode code, const UnicodeString& rule, int32_t start, UErrorCode& status)
    {
    ...
    while (pos < limit && !done) {
        ...
        case ANCHOR_START:
            if (buf.length() == 0 && !anchorStart) {
                ...
                return syntaxError(U_MISPLACED_ANCHOR_START, rule, start, status);
            }
            break;
        case SEGMENT_OPEN: {
            ...
            StringMatcher* m = new StringMatcher(buf, bufSegStart, buf.length(), segmentNumber, *parser.curData);
            if (m == NULL) {
        ...
        case ALT_FUNCTION: {
            ...
            if (single == NULL || !ICU_Utility::parseChar(rule, iref, SEGMENT_OPEN)) {
            ...
            FunctionReplacer* r = new FunctionReplacer(t, new StringReplacer(output, parser.curData));
            if (r == NULL) {
                return syntaxError(U_MEMORY_ALLOCATION_ERROR, rule, start, status);
            }
        ...
        case SymbolTable::SYMBOL_REF:
            // Handle variable references and segment references "$1" .. "$9"
            {
                ...
                if (r >= 1 && r <= 9) {
                    ...
                    if (r < 0) {
                        return syntaxError(U_UNDEFINED_SEGMENT_REFERENCE, rule, start, status);
                    }
                    buf.append(parser.getSegmentStandin(r, status));
                } else {
                    pp.setIndex(pos);
                    UnicodeString name = parser.parseData->parseReference(rule, pp, limit);
                    if (name.length() == 0) {
        ...
        case ZERO_OR_ONE:
            ...
            //  (seg)+  matches  segsegseg
            {
                ...
                UnicodeFunctor* m = new StringMatcher(buf, qstart, qlimit, 0, *parser.curData);
                if (m == NULL) {
                    return syntaxError(U_MEMORY_ALLOCATION_ERROR, rule, start, status);
                }
                int32_t min = 0;
        ...
        default:
            ...
            if (c >= 0x0021 && c <= 0x007E
                && !((c >= 0x0030 /*'0'*/ && c <= 0x0039 /*'9'*/) || (c >= 0x0041 /*'A'*/ && c <= 0x005A /*'Z'*/)
...
void TransliteratorParser::parse(const UnicodeString& rules, UTransDirection transDirection, UParseError& pe, UErrorCode& ec)
{
    if (U_SUCCESS(ec)) {
...
void TransliteratorParser::parseRules(const UnicodeString& rule, UTransDirection theDirection, UErrorCode& status)
{
    ...
    while (pos < limit && U_SUCCESS(status)) {
        ...
        // chars left.
        if ((pos + ID_TOKEN_LEN + 1) <= limit && rule.compare(pos, ID_TOKEN_LEN, ID_TOKEN) == 0) {
            pos += ID_TOKEN_LEN;
            c = rule.charAt(pos);
            ...
            TransliteratorIDParser::SingleID* id = TransliteratorIDParser::parseSingleID(rule, p, direction, status);
    ...
    if (U_SUCCESS(status)) {
        ...
        for (i = 0; i < dataVectorSize; i++) {
            ...
            for (int32_t j = 0; j < data->variablesLength; j++) {
                data->variables[j] = static_cast<UnicodeFunctor*>(variablesVector.elementAt(j));
            }
            ...
            while (he != NULL) {
                ...
                data->variableNames.put(*((UnicodeString*)(he->key.pointer)), tempus, status);
...
static const UChar PRAGMA_VARIABLE_RANGE[] = { 0x7E, 0x76, 0x61, 0x72, 0x69, 0x61, 0x62, 0x6C, 0x65, 0x20, 0x72, 0x61, 0x6E, 0x67, 0x65, 0x20, 0x23, 0x20, 0x23,
    0x7E, 0x3B, 0 }; // "~variable range # #~;"
...
int32_t TransliteratorParser::parseRule(const UnicodeString& rule, int32_t pos, int32_t limit, UErrorCode& status)
{
    ...
    if (op == REVERSE_RULE_OP && (pos < limit && rule.charAt(pos) == FORWARD_RULE_OP)) {
        ++pos;
        op = FWDREV_RULE_OP;
    }
    ...
    if (undefinedVariableName.length() != 0) {
        return syntaxError( // "Undefined variable $" + undefinedVariableName,
            U_UNDEFINED_VARIABLE, rule, start, status);
    }
    ...
    if (op != FWDREV_RULE_OP && ((direction == UTRANS_FORWARD) != (op == FORWARD_RULE_OP))) {
        return pos;
    }
    ...
    TransliterationRule* temptr = new TransliterationRule(left->text, left->ante, left->post, right->text, right->cursor, right->cursorOffset, segmentsArray,
        segmentObjects.size(), left->anchorStart, left->anchorEnd, curData, status);
    // Null pointer check
    if (temptr == NULL) {
 ...
 */
int32_t TransliteratorParser::syntaxError(UErrorCode parseErrorCode, const UnicodeString& rule, int32_t pos, UErrorCode& status)
{
 ...
 */
UChar TransliteratorParser::parseSet(const UnicodeString& rule, ParsePosition& pos, UErrorCode& status)
{
 ...
 * UnicodeString.
 */
void TransliteratorParser::appendVariableDef(const UnicodeString& name, UnicodeString& buf, UErrorCode& status)
{
