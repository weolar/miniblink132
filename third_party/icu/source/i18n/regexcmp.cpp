...
RegexCompile::RegexCompile(RegexPattern* rxp, UErrorCode& status)
    : fParenStack(status)
    , fSetStack(uprv_deleteUObject, nullptr, status)
    , fSetOpStack(status)
{
...
//
//------------------------------------------------------------------------------
void RegexCompile::compile(const UnicodeString& pat, // Source pat to be compiled.
...
//
void RegexCompile::compile(UText* pat, // Source pat to be compiled.
    UParseError& pp, // Error position info
    UErrorCode& e) // Error Code
{
    fStatus = &e;
    ...
    for (;;) {
        ...
        REGEX_SCAN_DEBUG_PRINTF(("char, line, col = (\'%c\', %d, %d)    state=%s ", fC.fChar, fLineNum, fCharNum, RegexStateNames[state]));
...
UBool RegexCompile::doParseActions(int32_t action)
{
    ...
    case doPlus:
        ...
        //     3.   LOOP_C       stack location
        {
            ...
            if (topLoc == fRXPat->fCompiledPat->size() - 1) {
                ...
                if (URX_TYPE(repeatedOp) == URX_DOTANY || URX_TYPE(repeatedOp) == URX_DOTANY_ALL || URX_TYPE(repeatedOp) == URX_DOTANY_UNIX) {
    ...
    case doCompleteNamedBackRef: {
        int32_t groupNumber = fRXPat->fNamedCaptureMap ? uhash_geti(fRXPat->fNamedCaptureMap, fCaptureName) : 0;
        if (groupNumber == 0) {
...
void RegexCompile::fixLiterals(UBool split)
{
    ...
    if (indexOfLastCodePoint == 0) {
        ...
        if ((fModeFlags & UREGEX_CASE_INSENSITIVE) && u_hasBinaryProperty(lastCodePoint, UCHAR_CASE_SENSITIVE)) {
...
void RegexCompile::insertOp(int32_t where)
{
    ...
    for (loc = 0; loc < code->size(); loc++) {
        ...
        if ((opType == URX_JMP || opType == URX_JMPX || opType == URX_STATE_SAVE || opType == URX_CTR_LOOP || opType == URX_CTR_LOOP_NG || opType == URX_JMP_SAV
                || opType == URX_JMP_SAV_X || opType == URX_RELOC_OPRND)
            && opValue > where) {
...
void RegexCompile::compileInterval(int32_t InitOp, int32_t LoopOp)
{
    ...
    if ((fIntervalLow & 0xff000000) != 0 || (fIntervalUpper > 0 && (fIntervalUpper & 0xff000000) != 0)) {
        error(U_REGEX_NUMBER_TOO_BIG);
    }
...
UBool RegexCompile::compileInlineInterval()
{
    ...
    //
    int32_t endOfSequenceLoc = fRXPat->fCompiledPat->size() - 1 + fIntervalUpper + (fIntervalUpper - fIntervalLow);
...
void RegexCompile::findCaseInsensitiveStarters(UChar32 c, UnicodeSet* starterChars)
{
    ...
    static const UChar32 RECaseFixCodePoints[] = { 0x61, 0x66, 0x68, 0x69, 0x6a, 0x73, 0x74, 0x77, 0x79, 0x2bc, 0x3ac, 0x3ae, 0x3b1, 0x3b7, 0x3b9, 0x3c1, 0x3c5,
        0x3c9, 0x3ce, 0x565, 0x574, 0x57e, 0x1f00, 0x1f01, 0x1f02, 0x1f03, 0x1f04, 0x1f05, 0x1f06, 0x1f07, 0x1f20, 0x1f21, 0x1f22, 0x1f23, 0x1f24, 0x1f25,
        0x1f26, 0x1f27, 0x1f60, 0x1f61, 0x1f62, 0x1f63, 0x1f64, 0x1f65, 0x1f66, 0x1f67, 0x1f70, 0x1f74, 0x1f7c, 0x110000 };

    static const int16_t RECaseFixStringOffsets[]
        = { 0x0, 0x1, 0x6, 0x7, 0x8, 0x9, 0xd, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x17, 0x1b, 0x20, 0x21, 0x2a, 0x2e, 0x2f, 0x30, 0x34, 0x35, 0x37, 0x39, 0x3b,
              0x3d, 0x3f, 0x41, 0x43, 0x45, 0x47, 0x49, 0x4b, 0x4d, 0x4f, 0x51, 0x53, 0x55, 0x57, 0x59, 0x5b, 0x5d, 0x5f, 0x61, 0x63, 0x65, 0x66, 0x67, 0 };

    static const int16_t RECaseFixCounts[] = { 0x1, 0x5, 0x1, 0x1, 0x1, 0x4, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x4, 0x4, 0x5, 0x1, 0x9, 0x4, 0x1, 0x1, 0x4, 0x1,
        0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x1, 0x1, 0x1, 0 };

    static const UChar RECaseFixData[] = { 0x1e9a, 0xfb00, 0xfb01, 0xfb02, 0xfb03, 0xfb04, 0x1e96, 0x130, 0x1f0, 0xdf, 0x1e9e, 0xfb05, 0xfb06, 0x1e97, 0x1e98,
        0x1e99, 0x149, 0x1fb4, 0x1fc4, 0x1fb3, 0x1fb6, 0x1fb7, 0x1fbc, 0x1fc3, 0x1fc6, 0x1fc7, 0x1fcc, 0x390, 0x1fd2, 0x1fd3, 0x1fd6, 0x1fd7, 0x1fe4, 0x3b0,
        0x1f50, 0x1f52, 0x1f54, 0x1f56, 0x1fe2, 0x1fe3, 0x1fe6, 0x1fe7, 0x1ff3, 0x1ff6, 0x1ff7, 0x1ffc, 0x1ff4, 0x587, 0xfb13, 0xfb14, 0xfb15, 0xfb17, 0xfb16,
        0x1f80, 0x1f88, 0x1f81, 0x1f89, 0x1f82, 0x1f8a, 0x1f83, 0x1f8b, 0x1f84, 0x1f8c, 0x1f85, 0x1f8d, 0x1f86, 0x1f8e, 0x1f87, 0x1f8f, 0x1f90, 0x1f98, 0x1f91,
        0x1f99, 0x1f92, 0x1f9a, 0x1f93, 0x1f9b, 0x1f94, 0x1f9c, 0x1f95, 0x1f9d, 0x1f96, 0x1f9e, 0x1f97, 0x1f9f, 0x1fa0, 0x1fa8, 0x1fa1, 0x1fa9, 0x1fa2, 0x1faa,
        0x1fa3, 0x1fab, 0x1fa4, 0x1fac, 0x1fa5, 0x1fad, 0x1fa6, 0x1fae, 0x1fa7, 0x1faf, 0x1fb2, 0x1fc2, 0x1ff2, 0 };
...
void RegexCompile::matchStartType()
{
    ...
    } else if (fRXPat->fInitialChars->contains((UChar32)0, (UChar32)0x10ffff) == FALSE && fRXPat->fMinMatchLen > 0) {
...
tailRecursion:
    ...
    if (fQuoteMode) {
        c.fQuoted = TRUE;
        if ((c.fChar == chBackSlash && peekCharLL() == chE && ((fModeFlags & UREGEX_LITERAL) == 0)) || c.fChar == (UChar32)-1) {
                ...
                if (c.fChar == chPound && fEOLComments == TRUE) {
                    ...
                    for (;;) {
                        ...
                        if (c.fChar == (UChar32)-1 || // EOF
                            c.fChar == chCR || c.fChar == chLF || c.fChar == chNEL || c.fChar == chLS) {
                            break;
                        }
                    }
                }
...
UChar32 RegexCompile::scanNamedChar()
{
    ...
    char name[100];
    if (!uprv_isInvariantUString(charName.getBuffer(), charName.length()) || (uint32_t)charName.length() >= sizeof(name)) {
...
UnicodeSet* RegexCompile::createSetForProperty(const UnicodeString& propName, UBool negated)
{
    ...
    do { // non-loop, exists to allow breaks from the block.
        ...
        if (propName.caseCompare(u"word", -1, 0) == 0) {
            set.adoptInsteadAndCheckErrorCode(RegexStaticSets::gStaticSets->fPropSets[URX_ISWORD_SET].cloneAsThawed(), status);
            break;
        }
