...
UPRV_FORMATTED_VALUE_CAPI_AUTO_IMPL(
    FormattedDateInterval, UFormattedDateInterval, UFormattedDateIntervalImpl, UFormattedDateIntervalApiHelper, udtitvfmt, 0x46444956)
    ...
    const UDateIntervalFormat* formatter, UDate fromDate, UDate toDate, UFormattedDateInterval* result, UErrorCode* status)
{
    ...
    if (resultImpl != nullptr) {
        resultImpl->fImpl = reinterpret_cast<const DateIntervalFormat*>(formatter)->formatToValue(interval, *status);
    }
}
    ...
    const UDateIntervalFormat* formatter, UCalendar* fromCalendar, UCalendar* toCalendar, UFormattedDateInterval* result, UErrorCode* status)
{
    ...
    auto* resultImpl = UFormattedDateIntervalApiHelper::validate(result, *status);
    if (resultImpl != nullptr) {
        resultImpl->fImpl = reinterpret_cast<const DateIntervalFormat*>(formatter)->formatToValue(*(Calendar*)fromCalendar, *(Calendar*)toCalendar, *status);
    }
}
