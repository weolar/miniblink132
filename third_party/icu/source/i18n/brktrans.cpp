 ...
 */
void BreakTransliterator::handleTransliterate(Replaceable& text, UTransPosition& offsets, UBool isIncremental) const
{
    ...
    for (boundary = bi->next(); boundary != UBRK_DONE && boundary < offsets.limit; boundary = bi->next()) {
        ...
        if ((U_MASK(type) & (U_GC_L_MASK | U_GC_M_MASK)) == 0)
            continue;
