    ...
    while (writtenElementCount < elementCount) {
        ...
        while ((element = uhash_nextElement(hashTable, &pos)) != NULL) {
            ...
            if (oldMappingLength != mappingLength) {
                ...
                if (oldMappingLength <= _SPREP_MAX_INDEX_TOP_LENGTH && mappingLength == _SPREP_MAX_INDEX_TOP_LENGTH + 1) {
...
static uint32_t U_CALLCONV getFoldedValue(UNewTrie* trie, UChar32 start, int32_t offset)
{
    uint32_t value;
    UChar32 limit = 0;
    UBool inBlockZero;
...
extern void generateData(const char* dataDir, const char* bundleName)
{
    ...
    /* write the data */
    pData = udata_create(dataDir, DATA_TYPE, fileName, &dataInfo, haveCopyright ? U_COPYRIGHT_STRING : NULL, &errorCode);
    if (U_FAILURE(errorCode)) {
