...
enum { HELP, HELP_QUESTION_MARK, VERBOSE, COPYRIGHT, DESTDIR, SOURCEDIR, ICUDATADIR, BUNDLE_NAME, NORMALIZE, NORM_CORRECTION_DIR, CHECK_BIDI, UNICODE_VERSION };
...
extern int main(int argc, char* argv[])
{
    ...
    fprintf(stderr,
        "gensprep writes dummy " U_ICUDATA_NAME "_" DATA_NAME "." DATA_TYPE " because UCONFIG_NO_IDNA is set, \n"
    ...
    filename = (char*)uprv_malloc(
        uprv_strlen(srcDir) + uprv_strlen(inputFileName) + (icuUniDataDir == NULL ? 0 : uprv_strlen(icuUniDataDir)) + 40); /* hopefully this should be enough */
