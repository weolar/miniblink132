...
ExtraData::ExtraData(Norms& n, UBool fast)
    ...
    , // 0=inert, 1=Jamo L, 2=start of compositions
    yesNoMappingsAndCompositions(1000, (UChar32)0, 1)
    , // 0=Hangul LV, 1=start of normal data
    yesNoMappingsOnly(1000, (UChar32)0, 1)
    , // 0=Hangul LVT, 1=start of normal data
    optimizeFast(fast)
{
...
int32_t ExtraData::writeMapping(UChar32 c, const Norm& norm, UnicodeString& dataString)
{
    ...
    if (norm.rawMapping != NULL) {
        ...
        if (rmLength == length - 1 &&
            ...
            0 == rm.compare(1, 99, m, 2, 99) && rm0 > Normalizer2Impl::MAPPING_LENGTH_MASK) {
            // Compression:
...
int32_t ExtraData::writeNoNoMapping(UChar32 c, const Norm& norm, UnicodeString& dataString, Hashtable& previousMappings)
{
...
UBool ExtraData::setNoNoDelta(UChar32 c, Norm& norm) const
{
    ...
    if (norm.mappingCP >= 0 && !(c <= 0x7f && norm.mappingCP > 0x7f) && norms.getNormRef(norm.mappingCP).type < Norm::NO_NO_COMP_YES) {
...
void ExtraData::writeCompositions(UChar32 c, const Norm& norm, UnicodeString& dataString)
{
    ...
    for (int32_t i = 0; i < length; ++i) {
        ...
        if (pair.trail < Normalizer2Impl::COMP_1_TRAIL_LIMIT) {
            ...
            firstUnit = (Normalizer2Impl::COMP_1_TRAIL_LIMIT + (pair.trail >> Normalizer2Impl::COMP_1_TRAIL_SHIFT)) | Normalizer2Impl::COMP_1_TRIPLE;
            secondUnit = (pair.trail << Normalizer2Impl::COMP_2_TRAIL_SHIFT) | (compositeAndFwd >> 16);
            thirdUnit = compositeAndFwd;
        }
...
void ExtraData::writeExtraData(UChar32 c, Norm& norm)
{
    ...
    case Norm::YES_NO_COMBINES_FWD:
        norm.offset = yesNoMappingsAndCompositions.length() + writeMapping(c, norm, yesNoMappingsAndCompositions);
        writeCompositions(c, norm, yesNoMappingsAndCompositions);
        break;
    case Norm::YES_NO_MAPPING_ONLY:
        norm.offset = yesNoMappingsOnly.length() + writeMapping(c, norm, yesNoMappingsOnly);
        break;
    ...
    case Norm::NO_NO_COMP_BOUNDARY_BEFORE:
        ...
        norm.offset = writeNoNoMapping(c, norm, noNoMappingsCompBoundaryBefore, previousNoNoMappingsCompBoundaryBefore);
        break;
