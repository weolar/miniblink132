...
Normalizer2DataBuilder::Normalizer2DataBuilder(UErrorCode& errorCode)
    : norms(errorCode)
    , phase(0)
    , overrideHandling(OVERRIDE_PREVIOUS)
    , optimization(OPTIMIZE_NORMAL)
    , norm16TrieBytes(nullptr)
    , norm16TrieLength(0)
{
...
void Normalizer2DataBuilder::setUnicodeVersion(const char* v)
{
    UVersionInfo nullVersion = { 0, 0, 0, 0 };
    UVersionInfo version;
    u_versionFromString(version, v);
    if (0 != memcmp(version, unicodeVersion, U_MAX_VERSION_LENGTH) && 0 != memcmp(nullVersion, unicodeVersion, U_MAX_VERSION_LENGTH)) {
...
UBool Normalizer2DataBuilder::mappingHasCompBoundaryAfter(const BuilderReorderingBuffer& buffer, Norm::MappingType mappingType) const
{
    if (buffer.isEmpty()) {
    ...
    if (Hangul::isJamoL(starter) || (Hangul::isJamoV(starter) && 0 < lastStarterIndex && Hangul::isJamoL(buffer.charAt(lastStarterIndex - 1)))) {
    ...
    if (i == lastStarterIndex && (starterNorm == nullptr || starterNorm->compositions == nullptr)) {
    ...
    while (++i < buffer.length()) {
        ...
        if (starterNorm != nullptr && (prevCC < cc || prevCC == 0) && norms.getNormRef(c).combinesBack && (starter = starterNorm->combine(c)) >= 0) {
...
UBool Normalizer2DataBuilder::mappingRecomposes(const BuilderReorderingBuffer& buffer) const
{
    ...
    for (int32_t i = 0; i < buffer.length(); ++i) {
        ...
        if (starterNorm != nullptr && (prevCC < cc || prevCC == 0) && norms.getNormRef(c).combinesBack && starterNorm->combine(c) >= 0) {
...
void Normalizer2DataBuilder::postProcess(Norm& norm)
{
    ...
    if (norm.hasMapping()) {
        ...
        norm.hasCompBoundaryBefore = !buffer.isEmpty() && norm.leadCC == 0 && !norms.combinesBack(buffer.charAt(0));
        norm.hasCompBoundaryAfter = norm.compositions == nullptr && mappingHasCompBoundaryAfter(buffer, norm.mappingType);
        ...
        norm.hasCompBoundaryAfter = norm.cc == 0 && !norm.combinesBack && norm.compositions == nullptr;
...
void Normalizer2DataBuilder::writeNorm16(UMutableCPTrie* norm16Trie, UChar32 start, UChar32 end, Norm& norm)
{
    ...
    UBool isDecompNo = (Norm::YES_NO_COMBINES_FWD <= norm.type && norm.type <= Norm::NO_NO_DELTA) || norm.cc != 0;
...
LocalUCPTriePointer Normalizer2DataBuilder::processData()
{
    ...
    indexes[Normalizer2Impl::IX_MIN_MAYBE_YES] = Normalizer2Impl::MIN_NORMAL_MAYBE_YES - extra.maybeYesCompositions.length() * 2;
    ...
    uint32_t value;
    UChar32 end = umutablecptrie_getRange(norm16Trie, 0xd800, UCPMAP_RANGE_NORMAL, 0, nullptr, nullptr, &value);
    ...
    for (UChar32 start = 0x10000;;) {
        if (start > end) {
            end = umutablecptrie_getRange(norm16Trie, start, UCPMAP_RANGE_NORMAL, 0, nullptr, nullptr, &value);
            if (end < 0) {
                break;
            }
        }
        ...
        if (leadEnd <= end) {
            ...
            maxNorm16 = (maxNorm16 & ~Normalizer2Impl::HAS_COMP_BOUNDARY_AFTER) | (andedNorm16 & Normalizer2Impl::HAS_COMP_BOUNDARY_AFTER);
    ...
    LocalUCPTriePointer builtTrie(umutablecptrie_buildImmutable(norm16Trie, UCPTRIE_TYPE_FAST, UCPTRIE_VALUE_BITS_16, errorCode));
...
void Normalizer2DataBuilder::writeBinaryFile(const char* filename)
{
    ...
    UNewDataMemory* pData = udata_create(NULL, NULL, filename, &dataInfo, haveCopyright ? U_COPYRIGHT_STRING : NULL, errorCode);
...
void Normalizer2DataBuilder::writeCSourceFile(const char* filename)
{
...
void Normalizer2DataBuilder::writeDataFile(const char* filename, bool writeRemoved) const
{
    ...
    if (unicodeVersion[0] != 0 || unicodeVersion[1] != 0 || unicodeVersion[2] != 0 || unicodeVersion[3] != 0) {
...
void Normalizer2DataBuilder::computeDiff(const Normalizer2DataBuilder& b1, const Normalizer2DataBuilder& b2, Normalizer2DataBuilder& diff)
{
    // Compute diff = b1 - b2
