...
U_CAPI UBool U_EXPORT2 ucbuf_autodetect_fs(FileStream* in, const char** cp, UConverter** conv, int32_t* signatureLength, UErrorCode* error)
{
    char start[8];
    int32_t numRead;
...
U_CAPI FileStream* U_EXPORT2 ucbuf_autodetect(const char* fileName, const char** cp, UConverter** conv, int32_t* signatureLength, UErrorCode* error)
{
    FileStream* in = NULL;
...
static UCHARBUF* ucbuf_fillucbuf(UCHARBUF* buf, UErrorCode* error)
{
    ...
    if (buf->conv) {
        ...
        ucnv_setToUCallBack(buf->conv, UCNV_TO_U_CALLBACK_STOP, toUNewContext, &toUOldAction, (const void**)&toUOldContext, error);
        /* since state is saved in the converter we add offset to source*/
        target = pTarget + offset;
        source = cbuf;
        sourceLimit = source + inputRead;
        ucnv_toUnicode(buf->conv, &target, target + (buf->bufCapacity - offset), &source, sourceLimit, NULL, (UBool)(buf->remaining == 0), error);
        ...
        if (U_FAILURE(*error)) {
             ...
             */
            ucnv_setToUCallBack(buf->conv, UCNV_TO_U_CALLBACK_SUBSTITUTE, toUNewContext, &toUOldAction, (const void**)&toUOldContext, &error1);
            ...
            /* re convert */
            ucnv_toUnicode(buf->conv, &target, target + (buf->bufCapacity - offset), &source, sourceLimit, NULL, (UBool)(buf->remaining == 0), &error1);
        }
...
U_CAPI UCHARBUF* U_EXPORT2 ucbuf_open(const char* fileName, const char** cp, UBool showWarning, UBool buffered, UErrorCode* error)
{
