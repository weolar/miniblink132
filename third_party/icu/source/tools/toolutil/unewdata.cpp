...
U_CAPI UNewDataMemory* U_EXPORT2 udata_create(
    const char* dir, const char* type, const char* name, const UDataInfo* pInfo, const char* comment, UErrorCode* pErrorCode)
{
...
{
    static const uint8_t padding[16] = { 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa };
    if (pData != NULL && pData->file != NULL) {
        while (length >= 16) {
