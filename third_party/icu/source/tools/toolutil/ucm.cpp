...
static int32_t U_CALLCONV compareMappingsBytesFirst(const void* context, const void* left, const void* right)
{
...
U_CAPI void U_EXPORT2 ucm_sortTable(UCMTable* t)
{
    ...
    uprv_sortArray(t->mappings, t->mappingsLength, sizeof(UCMapping), compareMappingsUnicodeFirst, t, FALSE, &errorCode);
    ...
    uprv_sortArray(t->reverseMap, t->mappingsLength, sizeof(int32_t), compareMappingsBytesFirst, t, FALSE, &errorCode);
...
enum { NEEDS_MOVE = 1, HAS_ERRORS = 2 };
...
U_CAPI UBool U_EXPORT2 ucm_checkValidity(UCMTable* table, UCMStates* baseStates)
{
    UCMapping *m, *mLimit;
    int32_t count;
    UBool isOK;
...
U_CAPI UBool U_EXPORT2 ucm_checkBaseExt(UCMStates* baseStates, UCMTable* base, UCMTable* ext, UCMTable* moveTarget, UBool intersectBase)
{
    ...
    /* check */
    result = checkBaseExtUnicode(baseStates, base, ext, (UBool)(moveTarget != NULL), intersectBase)
...
U_CAPI void U_EXPORT2 ucm_mergeTables(UCMTable* fromUTable, UCMTable* toUTable, const uint8_t* subchar, int32_t subcharLength, uint8_t subchar1)
{
    ...
    while (fromUIndex < fromUTop && toUIndex < toUTop) {
        ...
        if (cmp == 0) {
             ...
             */
            if ((fromUMapping->bLen == subcharLength && 0 == uprv_memcmp(UCM_GET_BYTES(fromUTable, fromUMapping), subchar, subcharLength))
...
U_CAPI UBool U_EXPORT2 ucm_separateMappings(UCMFile* ucm, UBool isSISO)
{
    ...
    for (; m < mLimit; ++m) {
        ...
        type = ucm_mappingType(&ucm->states, m, UCM_GET_CODE_POINTS(table, m), UCM_GET_BYTES(table, m));
        if (type < 0) {
...
U_CAPI int8_t U_EXPORT2 ucm_parseBytes(uint8_t bytes[UCNV_EXT_MAX_BYTES], const char* line, const char** ps)
{
    ...
    for (;;) {
        ...
        if (s[1] != 'x' || (byte = (uint8_t)uprv_strtoul(s + 2, &end, 16), end) != s + 4) {
...
U_CAPI UBool U_EXPORT2 ucm_parseMappingLine(UCMapping* m, UChar32 codePoints[UCNV_EXT_MAX_UCHARS], uint8_t bytes[UCNV_EXT_MAX_BYTES], const char* line)
{
    ...
    for (;;) {
        ...
        if (s[1] != 'U' || (cp = (UChar32)uprv_strtoul(s + 2, &end, 16), end) == s + 2 || *end != '>') {
    ...
    if (uLen == 0) {
        ...
        if ((U_FAILURE(errorCode) && errorCode != U_BUFFER_OVERFLOW_ERROR) || u16Length > UCNV_EXT_MAX_UCHARS) {
...
U_CAPI UCMTable* U_EXPORT2 ucm_openTable()
{
...
U_CAPI void U_EXPORT2 ucm_addMapping(UCMTable* table, UCMapping* m, UChar32 codePoints[UCNV_EXT_MAX_UCHARS], uint8_t bytes[UCNV_EXT_MAX_BYTES])
{
    ...
    if (table->mappingsLength >= table->mappingsCapacity) {
        ...
        table->mappings = (UCMapping*)uprv_realloc(table->mappings, table->mappingsCapacity * sizeof(UCMapping));
...
U_CAPI UCMFile* U_EXPORT2 ucm_open()
{
...
U_CAPI int32_t U_EXPORT2 ucm_mappingType(UCMStates* baseStates, UCMapping* m, UChar32 codePoints[UCNV_EXT_MAX_UCHARS], uint8_t bytes[UCNV_EXT_MAX_BYTES])
{
    ...
    if (m->uLen == 1 && count == 1 && m->f <= 3
        && (baseStates->maxCharLength == 1 || !((m->f == 2 && m->bLen == 1) || (m->f == 1 && bytes[0] == 0) || (m->f <= 1 && m->bLen > 1 && bytes[0] == 0)))) {
...
U_CAPI UBool U_EXPORT2 ucm_addMappingFromLine(UCMFile* ucm, const char* line, UBool forBase, UCMStates* baseStates)
{
    ...
    return ucm_parseMappingLine(&m, codePoints, bytes, line) && ucm_addMappingAuto(ucm, forBase, baseStates, &m, codePoints, bytes);
}
