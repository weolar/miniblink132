...
static const Field names_UCalendarMonths[]
    = { FIELD_NAME_STR(LEN_UCAL, UCAL_JANUARY), FIELD_NAME_STR(LEN_UCAL, UCAL_FEBRUARY), FIELD_NAME_STR(LEN_UCAL, UCAL_MARCH),
          FIELD_NAME_STR(LEN_UCAL, UCAL_APRIL), FIELD_NAME_STR(LEN_UCAL, UCAL_MAY), FIELD_NAME_STR(LEN_UCAL, UCAL_JUNE), FIELD_NAME_STR(LEN_UCAL, UCAL_JULY),
          FIELD_NAME_STR(LEN_UCAL, UCAL_AUGUST), FIELD_NAME_STR(LEN_UCAL, UCAL_SEPTEMBER), FIELD_NAME_STR(LEN_UCAL, UCAL_OCTOBER),
          FIELD_NAME_STR(LEN_UCAL, UCAL_NOVEMBER), FIELD_NAME_STR(LEN_UCAL, UCAL_DECEMBER), FIELD_NAME_STR(LEN_UCAL, UCAL_UNDECIMBER) };
...
static const Field names_UDateFormatStyle[] = {
    FIELD_NAME_STR(LEN_UDAT, UDAT_FULL), FIELD_NAME_STR(LEN_UDAT, UDAT_LONG), FIELD_NAME_STR(LEN_UDAT, UDAT_MEDIUM), FIELD_NAME_STR(LEN_UDAT, UDAT_SHORT),
    /* end regular */
    /*
...
#define COUNT_CASE(x)                                                                                                                                          \
    case UDBG_##x:                                                                                                                                             \
        return (actual ? count_##x : UPRV_LENGTHOF(names_##x));
...
{
    if (field < 0 || field >= _udbg_enumCount(type, FALSE)) { // also will catch unsupported items
        return NULL;
    } else {
...
{
    if (param->paramStr == NULL)
        return paramEmpty(param, target, targetCapacity, status);
    if (U_FAILURE(*status))
        return 0;
...
#define STRING_PARAM(func, str)                                                                                                                                \
    U_CAPI int32_t func(const USystemParams*, char* target, int32_t targetCapacity, UErrorCode* status)                                                        \
    {                                                                                                                                                          \
        return stringToStringBuffer(target, targetCapacity, (str), status);                                                                                    \
    }
...
private:
    std::map<std::string, std::map<std::string, std::set<std::string>>> fTable;
};
...
UBool KnownIssues::print()
{
    ...
    for (std::map<std::string, std::map<std::string, std::set<std::string>>>::iterator i = fTable.begin(); i != fTable.end(); i++) {
        ...
        for (std::map<std::string, std::set<std::string>>::iterator ii = (*i).second.begin(); ii != (*i).second.end(); ii++) {
            std::cout << "  " << (*ii).first << std::endl;
            for (std::set<std::string>::iterator iii = (*ii).second.begin(); iii != (*ii).second.end(); iii++) {
