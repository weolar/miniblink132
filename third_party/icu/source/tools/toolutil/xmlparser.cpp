...
// character constants
enum { x_QUOT = 0x22, x_AMP = 0x26, x_APOS = 0x27, x_LT = 0x3c, x_GT = 0x3e, x_l = 0x6c };

#define XML_SPACES "[ \\u0009\\u000d\\u000a]"

// XML #4
#define XML_NAMESTARTCHAR                                                                                                                                      \
    "[[A-Z]:_[a-z][\\u00c0-\\u00d6][\\u00d8-\\u00f6]"                                                                                                          \
    "[\\u00f8-\\u02ff][\\u0370-\\u037d][\\u037F-\\u1FFF][\\u200C-\\u200D]"                                                                                     \
    "[\\u2070-\\u218F][\\u2C00-\\u2FEF][\\u3001-\\uD7FF][\\uF900-\\uFDCF]"                                                                                     \
...
UXMLParser::UXMLParser(UErrorCode& status)
    ...
    //             1     2     3      4      5           6                    7          8
    mAmps(UnicodeString("&(?:(amp;)|(lt;)|(gt;)|(apos;)|(quot;)|#x([0-9A-Fa-f]{1,8});|#([0-9]{1,8});|(.))"), 0, status)
    ,
...
UXMLElement* UXMLParser::parseFile(const char* filename, UErrorCode& errorCode)
{
    ...
    if (U_SUCCESS(errorCode) && charset != NULL) {
        ...
        pb = bytes;
        pu = buffer;
        ucnv_toUnicode(cnv, &pu, buffer + src.getCapacity(), &pb, bytes + bytesLength, NULL, TRUE, &errorCode);
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            pu = buffer + length;
            ucnv_toUnicode(cnv, &pu, buffer + src.getCapacity(), &pb, bytes + bytesLength, NULL, FALSE, &errorCode);
            src.releaseBuffer(U_SUCCESS(errorCode) ? (int32_t)(pu - buffer) : 0);
...
UnicodeString UXMLElement::getText(UBool recurse) const
{
    UnicodeString text;
...
void UXMLElement::appendText(UnicodeString& text, UBool recurse) const
{
    const UObject* node;
