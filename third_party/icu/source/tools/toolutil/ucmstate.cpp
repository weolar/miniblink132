...
U_CAPI void U_EXPORT2 ucm_addState(UCMStates* states, const char* s)
{
    ...
    error = parseState(s, states->stateTable[states->countStates], &states->stateFlags[states->countStates]);
    if (error != NULL) {
...
U_CAPI void U_EXPORT2 ucm_processStates(UCMStates* states, UBool ignoreSISOCheck)
{
    ...
    if (states->minCharLength == 1) {
        ...
        for (cell = 0; cell < 256; ++cell) {
            ...
            if (MBCS_ENTRY_IS_FINAL(entry) && ((action = MBCS_ENTRY_FINAL_ACTION(entry)) == MBCS_STATE_VALID_DIRECT_16 || action == MBCS_STATE_UNASSIGNED)) {
                break;
            }
        }
...
static void compactToUnicode2(UCMStates* states, uint16_t** pUnicodeCodeUnits, _MBCSToUFallback* toUFallbacks, int32_t countToUFallbacks, UBool verbose)
{
    ...
    for (i = 0; i < 256; ++i) {
        entry = states->stateTable[leadState][i];
        if (MBCS_ENTRY_IS_TRANSITION(entry) && (MBCS_ENTRY_TRANSITION_STATE(entry)) == static_cast<uint32_t>(trailState)) {
 ...
 */
static int32_t findUnassigned(
    UCMStates* states, uint16_t* unicodeCodeUnits, _MBCSToUFallback* toUFallbacks, int32_t countToUFallbacks, int32_t state, int32_t offset, uint32_t b)
{
    ...
    UCMStates* states, uint16_t** pUnicodeCodeUnits, _MBCSToUFallback* toUFallbacks, int32_t countToUFallbacks, UBool verbose)
{
    ...
    if (countToUFallbacks > 0) {
        ...
        uprv_sortArray(toUFallbacks, countToUFallbacks, sizeof(_MBCSToUFallback), compareFallbacks, NULL, FALSE, &errorCode);
    }
}
