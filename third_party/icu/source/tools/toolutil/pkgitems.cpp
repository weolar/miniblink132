...
static void U_CALLCONV printError(void* context, const char* fmt, va_list args)
{
...
public:
    ...
    NativeItem(const Item* item, UDataSwapFn* swap)
        : swapped(NULL)
    {
        setItem(item, swap);
    }
 ...
 * and a suffix
 */
static void makeTargetName(const char* itemName, const char* id, int32_t idLength, const char* suffix, char* target, int32_t capacity, UErrorCode* pErrorCode)
{
    const char* itemID;
...
static void checkIDSuffix(
    const char* itemName, const char* id, int32_t idLength, const char* suffix, CheckDependency check, void* context, UErrorCode* pErrorCode)
{
    char target[200];
...
static void checkParent(const char* itemName, CheckDependency check, void* context, UErrorCode* pErrorCode)
{
...
static void checkAlias(
    const char* itemName, Resource res, const UChar* alias, int32_t length, UBool useResSuffix, CheckDependency check, void* context, UErrorCode* pErrorCode)
{
    int32_t i;
 ...
 * aliases.
 */
static void ures_enumDependencies(const char* itemName, const ResourceData* pResData, Resource res, const char* inKey, const char* parentKey, int32_t depth,
    CheckDependency check, void* context, Package* pkg, UErrorCode* pErrorCode)
{
    ...
    case URES_TABLE: {
        ...
        for (int32_t i = 0; i < count; ++i) {
            ...
            ures_enumDependencies(itemName, pResData, item, itemKey, inKey, depth + 1, check, context, pkg, pErrorCode);
...
static void ures_enumDependencies(const char* itemName, const UDataInfo* pInfo, const uint8_t* inBytes, int32_t length, CheckDependency check, void* context,
    Package* pkg, UErrorCode* pErrorCode)
{
    ...
    if (resData.usesPoolBundle) {
        ...
        if (!(poolIndexLength > URES_INDEX_POOL_CHECKSUM && (poolIndexes[URES_INDEX_ATTRIBUTES] & URES_ATT_IS_POOL_BUNDLE))) {
            fprintf(stderr, "icupkg: %s is not a pool bundle\n", poolName);
            return;
        }
    ...
    ures_enumDependencies(itemName, &resData, resData.rootRes, NULL, NULL, 0, check, context, pkg, pErrorCode);
}
...
static void ucnv_enumDependencies(const UDataSwapper* ds, const char* itemName, const UDataInfo* pInfo, const uint8_t* inBytes, int32_t length,
    CheckDependency check, void* context, UErrorCode* pErrorCode)
{
    ...
    if (!(pInfo->formatVersion[0] == 6 && pInfo->formatVersion[1] >= 2)) {
    ...
    if (inStaticData->conversionType == UCNV_MBCS) {
        ...
        if (inMBCSHeader->version[0] == 4 && inMBCSHeader->version[1] >= 1) {
            ...
            && ((mbcsHeaderOptions = ds->readUInt32(inMBCSHeader->options)) & MBCS_OPT_UNKNOWN_INCOMPATIBLE_MASK) == 0) {
        ...
        if (outputType == MBCS_OUTPUT_EXT_ONLY) {
            ...
            if (length < (extOffset + UCNV_EXT_INDEXES_MIN_LENGTH * 4)) {
                udata_printError(
                    ds, "icupkg/ucnv_enumDependencies(): too few bytes (%d after headers) for an ICU MBCS .cnv conversion table with extension data\n", length);
                *pErrorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                return;
            }
...
enum { FMT_RES, FMT_CNV, FMT_ALIAS, FMT_COUNT };

static int32_t getDataFormat(const uint8_t dataFormat[4])
{
    int32_t i;
...
void Package::enumDependencies(Item* pItem, void* context, CheckDependency check)
{
    int32_t infoLength, itemHeaderLength;
    UErrorCode errorCode = U_ZERO_ERROR;
