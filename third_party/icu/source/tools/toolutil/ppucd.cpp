...
static const char* lineTypeStrings[] = { NULL, NULL, "ucd", "property", "binary", "value", "defaults", "block", "cp", "unassigned", "algnamesrange" };

PreparsedUCD::LineType PreparsedUCD::readLine(UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return NO_LINE;
    }
    ...
    *limit = 0;
    ...
    for (type = EMPTY_LINE + 1;; ++type) {
        if (type == LINE_TYPE_COUNT) {
            fprintf(stderr, "error in preparsed UCD: unknown line type (first field) '%s' on line %ld\n", line, (long)lineNumber);
...
const char* PreparsedUCD::firstField()
{
...
const char* PreparsedUCD::nextField()
{
    if (fieldLimit == lineLimit) {
        return NULL;
    }
...
const UniProps* PreparsedUCD::getProps(UnicodeSet& newValues, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return NULL;
    }
    ...
    UChar32 start, end;
    if (!parseCodePointRange(field, start, end, errorCode)) {
        return NULL;
    }
    UniProps* props;
    ...
    case DEFAULTS_LINE:
        ...
        if (blockLineIndex >= 0) {
            fprintf(stderr, "error in preparsed UCD: default line %ld after one or more block lines\n", (long)lineNumber);
            errorCode = U_PARSE_ERROR;
            return NULL;
        }
        if (defaultLineIndex >= 0) {
            fprintf(stderr, "error in preparsed UCD: second line with default properties on line %ld\n", (long)lineNumber);
            errorCode = U_PARSE_ERROR;
            return NULL;
        }
        if (start != 0 || end != 0x10ffff) {
            fprintf(stderr, "error in preparsed UCD: default range must be 0..10FFFF, not '%s' on line %ld\n", field, (long)lineNumber);
            errorCode = U_PARSE_ERROR;
            return NULL;
        }
        props = &defaultProps;
    ...
    while ((field = nextField()) != NULL) {
        if (!parseProperty(*props, field, newValues, errorCode)) {
            return NULL;
        }
    }
...
static const struct {
    const char* name;
    int32_t prop;
} ppucdProperties[] = { { "Name_Alias", PPUCD_NAME_ALIAS }, { "Conditional_Case_Mappings", PPUCD_CONDITIONAL_CASE_MAPPINGS },
    { "Turkic_Case_Folding", PPUCD_TURKIC_CASE_FOLDING } };

// Returns TRUE for "ok to continue parsing fields".
UBool PreparsedUCD::parseProperty(UniProps& props, const char* field, UnicodeSet& newValues, UErrorCode& errorCode)
{
    CharString pBuffer;
    const char* p = field;
    ...
    if (prop < UCHAR_BINARY_LIMIT) {
        ...
        fprintf(stderr, "error in preparsed UCD: prop value is invalid: '%d' for line %ld\n", prop, (long)lineNumber);
        errorCode = U_PARSE_ERROR;
        ...
        if (value == UCHAR_INVALID_CODE) {
            fprintf(stderr, "error in preparsed UCD: '%s' is not a valid value on line %ld\n", field, (long)lineNumber);
            errorCode = U_PARSE_ERROR;
        } else {
        ...
        case UCHAR_SCRIPT_EXTENSIONS:
            ...
            break;
        default:
            fprintf(stderr, "error in preparsed UCD: '%s' is not a valid default value on line %ld\n", field, (long)lineNumber);
            errorCode = U_PARSE_ERROR;
        }
    } else {
        char c;
...
UBool PreparsedUCD::getRangeForAlgNames(UChar32& start, UChar32& end, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
...
UChar32 PreparsedUCD::parseCodePoint(const char* s, UErrorCode& errorCode)
{
    char* end;
    uint32_t value = (uint32_t)uprv_strtoul(s, &end, 16);
    if (end <= s || *end != 0 || value >= 0x110000) {
        fprintf(stderr, "error in preparsed UCD: '%s' is not a valid code point on line %ld\n", s, (long)lineNumber);
        errorCode = U_PARSE_ERROR;
        return U_SENTINEL;
    }
...
UBool PreparsedUCD::parseCodePointRange(const char* s, UChar32& start, UChar32& end, UErrorCode& errorCode)
{
    uint32_t st, e;
    u_parseCodePointRange(s, &st, &e, &errorCode);
    if (U_FAILURE(errorCode)) {
        fprintf(stderr, "error in preparsed UCD: '%s' is not a valid code point range on line %ld\n", s, (long)lineNumber);
        return FALSE;
    }
    start = (UChar32)st;
    end = (UChar32)e;
...
void PreparsedUCD::parseString(const char* s, UnicodeString& uni, UErrorCode& errorCode)
{
    ...
    if (U_FAILURE(errorCode)) {
        fprintf(stderr, "error in preparsed UCD: '%s' is not a valid Unicode string on line %ld\n", s, (long)lineNumber);
    }
}

void PreparsedUCD::parseScriptExtensions(const char* s, UnicodeSet& scx, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
    ...
    for (;;) {
        ...
        if (script == UCHAR_INVALID_CODE) {
            fprintf(stderr, "error in preparsed UCD: '%s' is not a valid script code on line %ld\n", scs, (long)lineNumber);
            errorCode = U_PARSE_ERROR;
            return;
        } else if (scx.contains(script)) {
            fprintf(stderr, "error in preparsed UCD: scx has duplicate '%s' codes on line %ld\n", scs, (long)lineNumber);
            errorCode = U_PARSE_ERROR;
            return;
        } else {
            scx.add(script);
