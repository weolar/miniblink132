...
struct U_TOOLUTIL_API UniProps {
    ...
    int32_t getIntProp(int32_t prop) const
    {
        return intProps[prop - UCHAR_INT_START];
    }
...
public:
    ...
    void setPropertyNames(const PropertyNames* pn)
    {
        pnames = pn;
    }
    ...
    const UVersionInfo& getUnicodeVersion() const
    {
        return ucdVersion;
    }
...
private:
    ...
    UBool parseProperty(UniProps& props, const char* field, UnicodeSet& newValues, UErrorCode& errorCode);
