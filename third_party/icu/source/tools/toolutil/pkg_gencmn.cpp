...
U_CAPI void U_EXPORT2 createCommonDataFile(const char* destDir, const char* name, const char* entrypointName, const char* type, const char* source,
    const char* copyRight, const char* dataFile, uint32_t max_size, UBool sourceTOC, UBool verbose, char* gencmnFileName)
{
    ...
    while ((s != NULL && *s != 0) || (s = T_FileStream_readLine(in, (line = linePtr), LINE_BUFFER_SIZE)) != NULL) {
    ...
    if (!sourceTOC) {
        ...
        sprintf(buffer,
            ...
            "    },\n"
            "    \"\", %lu, 0, {\n",
            static_cast<unsigned long>(32 - 4 - sizeof(UDataInfo)), static_cast<unsigned long>(fileCount), entrypointName,
            static_cast<unsigned long>(sizeof(UDataInfo)), U_IS_BIG_ENDIAN, U_CHARSET_FAMILY, U_SIZEOF_UCHAR, static_cast<unsigned long>(fileCount));
        T_FileStream_writeLine(out, buffer);
...
static void addFile(const char* filename, const char* name, const char* source, UBool sourceTOC, UBool verbose)
{
    ...
    if (!sourceTOC) {
        ...
        if (uprv_pathIsAbsolute(filename)) {
            fprintf(stderr,
                "gencmn: Error: absolute path encountered. Old style paths are not supported. Use relative paths such as 'fur.res' or "
                "'translit%cfur.res'.\n\tBad path: '%s'\n",
