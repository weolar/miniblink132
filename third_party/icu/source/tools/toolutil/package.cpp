...
static const UDataInfo dataInfo = {
    (uint16_t)sizeof(UDataInfo), 0,

    U_IS_BIG_ENDIAN, U_CHARSET_FAMILY, (uint8_t)sizeof(UChar), 0,
...
U_CDECL_BEGIN
static void U_CALLCONV printPackageError(void* context, const char* fmt, va_list args)
{
    vfprintf((FILE*)context, fmt, args);
}
U_CDECL_END

static uint16_t readSwapUInt16(uint16_t x)
{
...
static inline int32_t makeTypeEnum(uint8_t charset, UBool isBigEndian)
{
    return 2 * (int32_t)charset + isBigEndian;
}

static inline int32_t makeTypeEnum(char type)
{
    return type == 'l' ? TYPE_L : type == 'b' ? TYPE_B : type == 'e' ? TYPE_E : -1;
}

static inline char makeTypeLetter(uint8_t charset, UBool isBigEndian)
{
    return types[makeTypeEnum(charset, isBigEndian)];
}

static inline char makeTypeLetter(int32_t typeEnum)
{
    return types[typeEnum];
}

static void makeTypeProps(char type, uint8_t& charset, UBool& isBigEndian)
{
...
U_CFUNC const UDataInfo* getDataInfo(const uint8_t* data, int32_t length, int32_t& infoLength, int32_t& headerLength, UErrorCode* pErrorCode)
{
    ...
    if (data == NULL || (length >= 0 && length < (int32_t)sizeof(DataHeader))) {
    ...
    pInfo = &pHeader->info;
    if ((length >= 0 && length < (int32_t)sizeof(DataHeader)) || pHeader->dataHeader.magic1 != 0xda || pHeader->dataHeader.magic2 != 0x27
        || pInfo->sizeofUChar != 2) {
    ...
    if (headerLength < (int32_t)sizeof(DataHeader) || infoLength < (int32_t)sizeof(UDataInfo)
        || headerLength < (int32_t)(sizeof(pHeader->dataHeader) + infoLength) || (length >= 0 && length < headerLength)) {
        *pErrorCode = U_UNSUPPORTED_ERROR;
        return NULL;
    }
...
static int32_t getTypeEnumForInputData(const uint8_t* data, int32_t length, UErrorCode* pErrorCode)
{
    const UDataInfo* pInfo;
...
static void extractPackageName(const char* filename, char pkg[], int32_t capacity)
{
    const char* basename;
    int32_t len;
...
static int32_t getFileLength(FILE* f)
{
    int32_t length;
 ...
 */
static void makeFullFilename(const char* path, const char* name, char* filename, int32_t capacity)
{
    char* s;
...
static void makeFullFilenameAndDirs(const char* path, const char* name, char* filename, int32_t capacity)
{
    char* sep;
...
static uint8_t* readFile(const char* path, const char* name, int32_t& length, char& type)
{
    char filename[1024];
    FILE* file;
    UErrorCode errorCode;
...
static int32_t U_CALLCONV compareItems(const void* /*context*/, const void* left, const void* right)
{
    U_NAMESPACE_USE
...
void Package::readPackage(const char* filename)
{
    UDataSwapper* ds;
    const UDataInfo* pInfo;
    UErrorCode errorCode;
    ...
    if (!(pInfo->dataFormat[0] == 0x43 && /* dataFormat="CmnD" */
            pInfo->dataFormat[1] == 0x6d && pInfo->dataFormat[2] == 0x6e && pInfo->dataFormat[3] == 0x44 && pInfo->formatVersion[0] == 1)) {
...
char Package::getInType()
{
    return makeTypeLetter(inCharset, inIsBigEndian);
}

void Package::writePackage(const char* filename, char outType, const char* comment)
{
    char prefix[MAX_PKG_NAME_LENGTH + 4];
    ...
    for (pItem = items, i = 0; i < itemCount; ++pItem, ++i) {
        ...
        if (ds[type] != NULL) {
            ...
            udata_swap(ds[type], pItem->data, pItem->length, pItem->data, &errorCode);
            if (U_FAILURE(errorCode)) {
...
int32_t Package::findItem(const char* name, int32_t length) const
{
...
void Package::findItems(const char* pattern)
{
    const char* wild;
...
void Package::setMatchMode(uint32_t mode)
{
    matchMode = mode;
}

void Package::addItem(const char* name)
{
    addItem(name, NULL, 0, FALSE, U_ICUDATA_TYPE_LETTER[0]);
}

void Package::addItem(const char* name, uint8_t* data, int32_t length, UBool isDataOwned, char type)
{
    int32_t idx;
...
void Package::addFile(const char* filesPath, const char* name)
{
    uint8_t* data;
    int32_t length;
    char type;
...
void Package::addItems(const Package& listPkg)
{
    const Item* pItem;
    int32_t i;
...
void Package::removeItem(int32_t idx)
{
    if (idx >= 0) {
        // remove the item
        if (items[idx].isDataOwned) {
...
void Package::removeItems(const char* pattern)
{
    int32_t idx;
...
void Package::removeItems(const Package& listPkg)
{
    const Item* pItem;
    int32_t i;
...
void Package::extractItem(const char* filesPath, const char* outName, int32_t idx, char outType)
{
    char filename[1024];
    UDataSwapper* ds;
    FILE* file;
...
void Package::extractItem(const char* filesPath, int32_t idx, char outType)
{
    extractItem(filesPath, items[idx].name, idx, outType);
}

void Package::extractItems(const char* filesPath, const char* pattern, char outType)
{
    int32_t idx;
...
void Package::extractItems(const char* filesPath, const Package& listPkg, char outType)
{
    const Item* pItem;
    int32_t i;
...
int32_t Package::getItemCount() const
{
    return itemCount;
}

const Item* Package::getItem(int32_t idx) const
{
...
void Package::checkDependency(void* context, const char* itemName, const char* targetName)
{
...
UBool Package::checkDependencies()
{
...
void Package::enumDependencies(void* context, CheckDependency check)
{
    int32_t i;
...
char* Package::allocString(UBool in, int32_t length)
{
    char* p;
    int32_t top;
