...
/* prototypes */
U_CDECL_BEGIN
static void MBCSClose(NewConverter* cnvData);

static UBool MBCSStartMappings(MBCSData* mbcsData);

static UBool MBCSAddToUnicode(MBCSData* mbcsData, const uint8_t* bytes, int32_t length, UChar32 c, int8_t flag);

static UBool MBCSIsValid(NewConverter* cnvData, const uint8_t* bytes, int32_t length);

static UBool MBCSSingleAddFromUnicode(MBCSData* mbcsData, const uint8_t* bytes, int32_t length, UChar32 c, int8_t flag);

static UBool MBCSAddFromUnicode(MBCSData* mbcsData, const uint8_t* bytes, int32_t length, UChar32 c, int8_t flag);

static void MBCSPostprocess(MBCSData* mbcsData, const UConverterStaticData* staticData);

static UBool MBCSAddTable(NewConverter* cnvData, UCMTable* table, UConverterStaticData* staticData);
...
static inline char hexDigit(uint8_t digit)
{
    return digit <= 9 ? (char)('0' + digit) : (char)('a' - 10 + digit);
}

static inline char* printBytes(char* buffer, const uint8_t* bytes, int32_t length)
{
    char* s = buffer;
    while (length > 0) {
...
static void MBCSDestruct(MBCSData* mbcsData)
{
...
static UBool MBCSStartMappings(MBCSData* mbcsData)
{
    int32_t i, sum, maxCharLength, stage2NullLength, stage2AllocLength, stage3NullLength, stage3AllocLength;
...
static UBool setFallback(MBCSData* mbcsData, uint32_t offset, UChar32 c)
{
...
static int32_t removeFallback(MBCSData* mbcsData, uint32_t offset)
{
    int32_t i = ucm_findFallback(mbcsData->toUFallbacks, mbcsData->countToUFallbacks, offset);
    if (i >= 0) {
        ...
        if (MBCS_ENTRY_IS_TRANSITION(entry)) {
            ...
            if (i < length) {
                fprintf(stderr, "error: byte sequence too long by %d bytes, final state %u: 0x%s (U+%x)\n", (int)(length - i), state,
                    printBytes(buffer, bytes, length), (int)c);
                return FALSE;
            }
...
static UBool MBCSAddTable(NewConverter* cnvData, UCMTable* table, UConverterStaticData* staticData)
{
    ...
    if (utf8Friendly) {
        ...
        mbcsData->utf8Max = 0;
        if (SMALL && maxCharLength > 1) {
            fprintf(stderr, "makeconv warning: --small not available for .ucm files without |0 etc.\n");
        }
    }
    ...
    for (i = 0; i < table->mappingsLength; ++m, ++i) {
         ...
         */
        if (mbcsData->omitFromU && f <= 1 && mbcsData->utf8Max < c && c <= 0xffff && mbcsData->utf8Max < 0xfeff) {
...
U_CDECL_END
static UBool transformEUC(MBCSData* mbcsData)
{
    uint8_t* p8;
 ...
 */
static void singleCompactStage2(MBCSData* mbcsData)
{
...
static void singleCompactStage3(MBCSData* mbcsData)
{
 ...
 * This function is very similar to genprops/store.c/compactStage().
 */
static void compactStage2(MBCSData* mbcsData)
{
    /* this array maps the ordinal number of a stage 2 block to its new stage 1 index */
    uint16_t map[MBCS_STAGE_2_MAX_BLOCKS];
    uint16_t i, start, prevEnd, newStart;
...
static void MBCSPostprocess(MBCSData* mbcsData, const UConverterStaticData* /*staticData*/)
{
    ...
    ucm_optimizeStates(states, &mbcsData->unicodeCodeUnits, mbcsData->toUFallbacks, mbcsData->countToUFallbacks, VERBOSE);
...
static uint32_t MBCSWrite(NewConverter* cnvData, const UConverterStaticData* staticData, UNewDataMemory* pData, int32_t tableType)
{
    ...
    header.offsetToUCodeUnits = headerLength * 4 + mbcsData->ucm->states.countStates * 1024 + mbcsData->countToUFallbacks * sizeof(_MBCSToUFallback);
    header.offsetFromUTable = header.offsetToUCodeUnits + mbcsData->ucm->states.countToUCodeUnits * 2;
    header.offsetFromUBytes = header.offsetFromUTable + stage1Top * 2 + stage2Length;
