...
static struct lookup const line_codes[] = { { "Rule", LC_RULE }, { "Zone", LC_ZONE }, { "Link", LC_LINK }, { "Leap", LC_LEAP }, { NULL, 0 } };

static struct lookup const mon_names[] = { { "January", TM_JANUARY }, { "February", TM_FEBRUARY }, { "March", TM_MARCH }, { "April", TM_APRIL },
    { "May", TM_MAY }, { "June", TM_JUNE }, { "July", TM_JULY }, { "August", TM_AUGUST }, { "September", TM_SEPTEMBER }, { "October", TM_OCTOBER },
    { "November", TM_NOVEMBER }, { "December", TM_DECEMBER }, { NULL, 0 } };

static struct lookup const wday_names[] = { { "Sunday", TM_SUNDAY }, { "Monday", TM_MONDAY }, { "Tuesday", TM_TUESDAY }, { "Wednesday", TM_WEDNESDAY },
    { "Thursday", TM_THURSDAY }, { "Friday", TM_FRIDAY }, { "Saturday", TM_SATURDAY }, { NULL, 0 } };

static struct lookup const lasts[] = { { "last-Sunday", TM_SUNDAY }, { "last-Monday", TM_MONDAY }, { "last-Tuesday", TM_TUESDAY },
    { "last-Wednesday", TM_WEDNESDAY }, { "last-Thursday", TM_THURSDAY }, { "last-Friday", TM_FRIDAY }, { "last-Saturday", TM_SATURDAY }, { NULL, 0 } };

static struct lookup const begin_years[] = { { "minimum", YR_MINIMUM }, { "maximum", YR_MAXIMUM }, { NULL, 0 } };

static struct lookup const end_years[] = { { "minimum", YR_MINIMUM }, { "maximum", YR_MAXIMUM }, { "only", YR_ONLY }, { NULL, 0 } };

static struct lookup const leap_types[] = { { "Rolling", TRUE }, { "Stationary", FALSE }, { NULL, 0 } };

static const int len_months[2][MONSPERYEAR] = { { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }, { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 } };
...
static ATTRIBUTE_PURE void* memcheck(void* const ptr)
{
    if (ptr == NULL)
...
static void* growalloc(void* ptr, size_t itemsize, int nitems, int* nitems_alloc)
{
...
static void emit_icu_zone(FILE* f, const char* zoneName, int zoneOffset, const struct rule* rule, int ruleIndex, int startYear)
{
    ...
    fprintf(f, " # zone %s, offset %d, year >= %d, rule %s (%d)\n", zoneName, zoneOffset, startYear, rule->r_name, ruleIndex);
}
...
static void emit_icu_rule(FILE* f, const struct rule* r, int ruleIndex)
{
    ...
    /* machine-readable section */
    fprintf(f, "rule %s %s %d %d %d %lld %d %d %lld", r->r_name, DYCODE[r->r_dycode], r->r_month, r->r_dayofmonth, (r->r_dycode == DC_DOM ? -1 : r->r_wday),
        r->r_tod, r->r_todisstd, r->r_todisgmt, r->r_stdoff);

    /* human-readable section */
    fprintf(f, " # %d: %s, file %s, line %d", ruleIndex, r->r_name, r->r_filename, r->r_linenum);
...
static int add_icu_final_rules(const struct rule* r1, const struct rule* r2)
{
    ...
    for (i = 0; i < finalRulesCount; ++i) { /* i+=2 should work too */
        if (r1 == finalRules[i])
            return i; /* [sic] pointer comparison */
    }

    finalRules = (const struct rule**)(void*)erealloc((char*)finalRules, (finalRulesCount + 2) * sizeof(*finalRules));
...
static void dolink(const char* const fromfield, const char* const tofield)
{
	...
	*/
    if (!itsdir(toname))
        (void)remove(toname);
    if (link(fromname, toname) != 0 && access(fromname, F_OK) == 0 && !itsdir(fromname)) {
        int result;
        ...
        if (result != 0) {
            ...
            do
                t = s;
            while ((s = strchr(s, '/')) && !strncmp(fromfield, tofield, ++s - fromfield));

            for (s = tofield + (t - fromfield); (s = strchr(s, '/')); s++)
        ...
        if (result != 0) {
            ...
            if (!fp) {
                const char* e = strerror(errno);
                (void)fprintf(stderr, _("%s: Can't read %s: %s\n"), progname, fromname, e);
                exit(EXIT_FAILURE);
            }
            tp = fopen(toname, "wb");
            if (!tp) {
...
static int itsdir(const char* const name)
{
...
static zic_t gethms(const char* string, const char* const errstring, const int signable)
{
    zic_t hh;
    int mm, ss, sign;
    ...
    if (sscanf(string, scheck(string, "%" SCNdZIC), &hh) == 1)
        ...
        ss = 0;
    else if (sscanf(string, scheck(string, "%" SCNdZIC ":%d:%d"), &hh, &mm, &ss) != 3) {
        error("%s", errstring);
        return 0;
    }
...
static void inrule(register char** const fields, const int nfields)
{
    ...
    rulesub(&r, fields[RF_LOYEAR], fields[RF_HIYEAR], fields[RF_COMMAND], fields[RF_MONTH], fields[RF_DAY], fields[RF_TOD]);
...
static int inzone(register char** const fields, const int nfields)
{
    ...
    for (i = 0; i < nzones; ++i)
        if (zones[i].z_name != NULL && strcmp(zones[i].z_name, fields[ZF_NAME]) == 0) {
            error(_("duplicate zone name %s (file \"%s\", line %d)"), fields[ZF_NAME], zones[i].z_filename, zones[i].z_linenum);
            return FALSE;
        }
...
static int inzsub(register char** const fields, const int nfields, const int iscont)
{
    ...
    if (hasuntil) {
        ...
        rulesub(&z.z_untilrule, fields[i_untilyear], "only", "", (nfields > i_untilmonth) ? fields[i_untilmonth] : "Jan",
            (nfields > i_untilday) ? fields[i_untilday] : "1", (nfields > i_untiltime) ? fields[i_untiltime] : "0");
        z.z_untiltime = rpytime(&z.z_untilrule, z.z_untilrule.r_loyear);
        if (iscont && nzones > 0 && z.z_untiltime > min_time && z.z_untiltime < max_time && zones[nzones - 1].z_untiltime > min_time
            && zones[nzones - 1].z_untiltime < max_time && zones[nzones - 1].z_untiltime >= z.z_untiltime) {
...
static void inleap(register char** const fields, const int nfields)
{
    ...
    cp = fields[LP_DAY];
    if (sscanf(cp, scheck(cp, "%d"), &day) != 1 || day <= 0 || day > len_months[isleap(year)][month]) {
...
static void rulesub(register struct rule* const rp, const char* const loyearp, const char* const hiyearp, const char* const typep, const char* const monthp,
    const char* const dayp, const char* const timep)
{
    register const struct lookup* lp;
    register const char* cp;
    register char* dp;
	...
	*/
    ...
    if ((lp = byword(dp, lasts)) != NULL) {
        ...
        if (sscanf(ep, scheck(ep, "%d"), &rp->r_dayofmonth) != 1 || rp->r_dayofmonth <= 0 || (rp->r_dayofmonth > len_months[1][rp->r_month])) {
            error(_("invalid day of month"));
            free(dp);
            return;
        }
...
static void writezone(const char* const name, const char* const string, char version)
{
	...
	*/
    {
		...
		*/
        ...
        for (; fromi < timecnt; ++fromi) {
            if (toi != 0 && ((attypes[fromi].at + gmtoffs[attypes[toi - 1].type]) <= (attypes[toi - 1].at + gmtoffs[toi == 1 ? 0 : attypes[toi - 2].type]))) {
	...
	*/
    ...
    fullname = erealloc(fullname, strlen(directory) + 1 + strlen(name) + 1);
		...
		*/
        {
            ...
            if (hidst >= 0 && mrudst >= 0 && hidst != mrudst && gmtoffs[hidst] != gmtoffs[mrudst]) {
                isdsts[mrudst] = -1;
                type = addtype(gmtoffs[mrudst],
#ifdef ICU
                    rawoffs[mrudst], dstoffs[mrudst],
#endif
                    &chars[abbrinds[mrudst]], TRUE, ttisstds[mrudst], ttisgmts[mrudst]);
                isdsts[mrudst] = TRUE;
                writetype[type] = TRUE;
            }
            if (histd >= 0 && mrustd >= 0 && histd != mrustd && gmtoffs[histd] != gmtoffs[mrustd]) {
                isdsts[mrustd] = -1;
                type = addtype(gmtoffs[mrustd],
#ifdef ICU
                    rawoffs[mrudst], dstoffs[mrudst],
#endif
                    &chars[abbrinds[mrustd]], FALSE, ttisstds[mrustd], ttisgmts[mrustd]);
                isdsts[mrustd] = FALSE;
                writetype[type] = TRUE;
            }
        }
        ...
        for (i = 0; i < typecnt; ++i) {
            ...
            for (j = 0; j < thischarcnt; ++j)
                if (strcmp(&thischars[j], thisabbr) == 0)
                    break;
            if (j == thischarcnt) {
                (void)strcpy(&thischars[(int)thischarcnt], thisabbr);
...
#undef DO
        ...
        if (thischarcnt != 0)
            (void)fwrite(thischars, sizeof thischars[0], thischarcnt, fp);
...
static int stringrule(char* result, const struct rule* const rp, const zic_t dstoff, const zic_t gmtoff)
{
    ...
    if (rp->r_dycode == DC_DOM) {
        ...
        if (wday < 0)
            wday += DAYSPERWEEK;
        (void)sprintf(result, "M%d.%d.%d", rp->r_month + 1, week, wday);
    }
    if (rp->r_todisgmt)
        tod += gmtoff;
...
static void outzone(const struct zone* const zpfirst, const int zonecount)
{
    register const struct zone* zp;
    register struct rule* rp;
    register int i, j;
	...
	*/
    ...
    if (noise) {
        if (!*envvar)
            warning("%s %s", _("no POSIX environment variable for zone"), zpfirst->z_name);
	...
	*/
    ...
    for (i = 0; i < zonecount; ++i) {
        ...
        *startbuf = '\0';
        ...
        if (zp->z_nrules == 0) {
            stdoff = zp->z_stdoff;
            doabbr(startbuf, zp->z_format, NULL, stdoff != 0, FALSE);
            type = addtype(oadd(zp->z_gmtoff, stdoff),
#ifdef ICU
                zp->z_gmtoff, stdoff,
#endif
                startbuf, stdoff != 0, startttisstd, startttisgmt);
            if (usestart) {
				...
				*/
                    ...
                    if (usestart) {
                        if (ktime < starttime) {
                            ...
                            doabbr(startbuf, zp->z_format, rp->r_abbrvar, rp->r_stdoff != 0, FALSE);
                            continue;
                        }
					 ...
					 * dropped from zoneinfo.txt. */
                        emit_icu_zone(icuFile, zpfirst->z_name, zp->z_gmtoff, rp, finalRuleIndex, year + 1);
                        /* only emit this for the first year */
                        finalRule1 = NULL;
                    }
#endif
                    eats(zp->z_filename, zp->z_linenum, rp->r_filename, rp->r_linenum);
                    doabbr(ab, zp->z_format, rp->r_abbrvar, rp->r_stdoff != 0, FALSE);
                    offset = oadd(zp->z_gmtoff, rp->r_stdoff);
#ifdef ICU
                    type = addtype(offset, zp->z_gmtoff, rp->r_stdoff, ab, rp->r_stdoff != 0, rp->r_todisstd, rp->r_todisgmt);
#else
        ...
        if (usestart) {
            if (*startbuf == '\0' && zp->z_format != NULL && strchr(zp->z_format, '%') == NULL && strchr(zp->z_format, '/') == NULL)
            ...
            if (*startbuf == '\0')
                ...
                addtt(starttime,
#ifdef ICU
                    addtype(startoff, zp->z_gmtoff, startoff - zp->z_gmtoff, startbuf, startoff != zp->z_gmtoff, startttisstd, startttisgmt));
#else
                    addtype(startoff, startbuf, startoff != zp->z_gmtoff, startttisstd, startttisgmt));
#endif
        }
        /*
...
static int
#ifdef ICU
addtype(const zic_t gmtoff, const zic_t rawoff, const zic_t dstoff, char* const abbr, const int isdst, const int ttisstd, const int ttisgmt)
#else
addtype(const zic_t gmtoff, const char* const abbr, const int isdst, const int ttisstd, const int ttisgmt)
#endif
{
    register int i, j;
...
#endif
            strcmp(abbr, &chars[abbrinds[i]]) == 0 && ttisstd == ttisstds[i] && ttisgmt == ttisgmts[i])
            return i;
    }
    /*
...
static void leapadd(const zic_t t, const int positive, const int rolling, int count)
{
    register int i, j;
...
static int yearistype(const int year, const char* const type)
{
    static char* buf;
    int result;
...
static ATTRIBUTE_PURE int ciequal(register const char* ap, register const char* bp)
{
...
static ATTRIBUTE_PURE int itsabbr(register const char* abbr, register const char* word)
{
...
static ATTRIBUTE_PURE const struct lookup* byword(register const char* const word, register const struct lookup* const table)
{
...
static ATTRIBUTE_PURE zic_t oadd(const zic_t t1, const zic_t t2)
{
...
static ATTRIBUTE_PURE zic_t tadd(const zic_t t1, const zic_t t2)
{
    if (t1 == max_time && t2 > 0)
        return max_time;
    if (t1 == min_time && t2 < 0)
...
static void newabbr(const char* const string)
{
    register int i;
			...
			*/
            if (mkdir(name, MKDIR_UMASK) != 0) {
                ...
                if (errno != EEXIST || !itsdir(name)) {
                    (void)fprintf(stderr, _("%s: Can't create directory %s: %s\n"), progname, name, e);
                    free(name);
                    return -1;
                }
            }
