...
#define HAVE_STDINT_H (199901 <= __STDC_VERSION__ || 2 < (__GLIBC__ + (0 < __GLIBC_MINOR__)))
#endif
#if HAVE_STDINT_H
#include "stdint.h"
#endif
...
static time_t const absolute_min_time = ((time_t)-1 < 0 ? (time_t)-1 << (CHAR_BIT * sizeof(time_t) - 1) : 0);
static time_t const absolute_max_time = ((time_t)-1 < 0 ? -(~0 < 0) - ((time_t)-1 << (CHAR_BIT * sizeof(time_t) - 1)) : -1);
...
static void abbrok(const char* const abbrp, const char* const zone)
{
    ...
    if (wp == NULL)
        return;
    (void)fflush(stdout);
    (void)fprintf(stderr, _("%s: warning: zone \"%s\" abbreviation \"%s\" %s\n"), progname, zone, abbrp, wp);
    warned = TRUE;
}
...
arg_processing_done:;
    ...
    if (vflag | Vflag) {
        ...
        if (cutarg != NULL) {
            ...
            if (cutarg != loend && !*loend) {
                hi = lo;
                cuthiyear = hi;
            } else if (cutarg != loend && *loend == ',' && (hi = strtoimax(loend + 1, &hiend, 10), loend + 1 != hiend && !*hiend)) {
                cutloyear = lo;
                cuthiyear = hi;
            } else {
        ...
        if (cuttimes != NULL) {
            ...
            if (cuttimes != loend && !*loend) {
            ...
            } else if (cuttimes != loend && *loend == ',' && (hi = strtoimax(loend + 1, &hiend, 10), loend + 1 != hiend && !*hiend)) {
                if (cutlotime < lo) {
                ...
                (void)fprintf(stderr, _("%s: wild -t argument %s\n"), progname, cuttimes);
                exit(EXIT_FAILURE);
            }
        }
    }
    ...
    for (i = optind; i < argc; ++i) {
        ...
        for (;;) {
            newt = (t < absolute_max_time - SECSPERDAY / 2 ? t + SECSPERDAY / 2 : absolute_max_time);
            if (cuthitime <= newt)
                break;
            ...
            if (iflag) {
                ...
                if ((tmp == NULL || newtmp == NULL) ? (tmp != newtmp) : (delta(&newtm, &tm) != (newt - t) || newtm.tm_isdst != tm.tm_isdst)) {
                ...
                if ((tmp == NULL || newtmp == NULL) ? (tmp != newtmp)
                                                    : (delta(&newtm, &tm) != (newt - t) || newtm.tm_isdst != tm.tm_isdst || strcmp(abbr(&newtm), buf) != 0)) {
...
static time_t yeartot(const intmax_t y)
{
...
static time_t hunt(char* name, time_t lot, time_t hit)
{
    ...
    for (;;) {
        ...
        if ((lotmp == NULL || tmp == NULL) ? (lotmp == tmp)
                                           : (delta(&tm, &lotm) == (t - lot) && tm.tm_isdst == lotm.tm_isdst && strcmp(abbr(&tm), loab) == 0)) {
            lot = t;
            lotm = tm;
            lotmp = tmp;
        } else
...
static void dumptime(register const struct tm* timeptr)
{
    ...
    static const char mon_name[][3] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
	...
	*/
    ...
    (void)printf("%.3s %.3s%3d %.2d:%.2d:%.2d ", wn, mn, timeptr->tm_mday, timeptr->tm_hour, timeptr->tm_min, timeptr->tm_sec);
#define DIVISOR 10
    trail = timeptr->tm_year % DIVISOR + TM_YEAR_BASE % DIVISOR;
    lead = timeptr->tm_year / DIVISOR + TM_YEAR_BASE / DIVISOR + trail / DIVISOR;
    trail %= DIVISOR;
    if (trail < 0 && lead > 0) {
        trail += DIVISOR;
        --lead;
...
static time_t huntICU(char* name, time_t lot, time_t hit, FILE* fp)
{
    ...
    for (;;) {
        ...
        /* We  do not want to capture transitions just for
		 * abbreviated zone name changes */
        if ((lotmp == NULL || tmp == NULL) ? (lotmp == tmp) : (delta(&tm, &lotm) == (t - lot) && tm.tm_isdst == lotm.tm_isdst)) {
            lot = t;
            lotm = tm;
            lotmp = tmp;
        } else
