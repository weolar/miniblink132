...
//
DataHeader dh = { { sizeof(DataHeader), // Struct MappedData
                      0xda, 0x27 },

    {
        ...
        0, //     reserved
        U_IS_BIG_ENDIAN, U_CHARSET_FAMILY, U_SIZEOF_UCHAR,
        0, //     reserved
...
int main(int argc, char** argv)
{
    ...
    const char* encoding = ucnv_detectUnicodeSignature(ruleSourceC, ruleFileSize, &signatureLength, &status);
    if (U_FAILURE(status)) {
        exit(status);
    }
    if (encoding != NULL) {
    ...
    uint32_t destCap = ucnv_toUChars(conv,
        NULL, //  dest,
        0, //  destCapacity,
        ruleSourceC, ruleFileSize, &status);
    ...
    ucnv_toUChars(conv,
        ruleSourceU, //  dest,
        destCap + 1, ruleSourceC, ruleFileSize, &status);
    if (U_FAILURE(status)) {
        fprintf(stderr, "ucnv_toUChars: ICU Error \"%s\"\n", u_errorName(status));
        exit(status);
    }
