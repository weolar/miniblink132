...
enum arguments { ARG_HELP = 0, ARG_QMARK, ARG_VERBOSE, ARG_ICUDATADIR, ARG_COPYRIGHT, ARG_UCHARS, ARG_BYTES, ARG_TRANSFORM, ARG_QUIET };
...
public:
    ...
    DataDict(UBool isBytesTrie, UErrorCode& status)
        : bt(NULL)
        , ut(NULL)
        , transformConstant(0)
        , transformType(DictionaryData::TRANSFORM_NONE)
    {
        if (isBytesTrie) {
...
int main(int argc, char** argv)
{
    ...
    while (readLine(f.getAlias(), fileLine, status)) {
        ...
        int32_t valueStart;
        for (valueStart = keyLen; valueStart < fileLine.length() && u_isspace(fileLine[valueStart]); ++valueStart) {
        }
    ...
    int32_t indexes[DictionaryData::IX_COUNT] = { DictionaryData::IX_COUNT * sizeof(int32_t), 0, 0, 0, 0, 0, 0, 0 };
