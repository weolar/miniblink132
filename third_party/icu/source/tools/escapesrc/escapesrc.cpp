...
static const char kSPACE = 0x20, kTAB = 0x09, kLF = 0x0A, kCR = 0x0D;
 ...
 */
bool appendUtf8(std::string& outstr, const std::string& linestr, size_t& pos, size_t chars)
{
    char tmp[9];
...
bool fixAt(std::string& linestr, size_t pos)
{
    ...
    for (; pos < linestr.size(); pos++) {
        ...
        if (linestr[pos] == '\\') {
            ...
            if (linestr[pos] == 'u')
                continue; // for now ... unicode escape
            if (linestr[pos] == '\\')
                continue;
