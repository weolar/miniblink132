...
/* Protos */
void processFile(const char* filename, const char* cp, const char* inputDir, const char* outputDir, const char* filterDir, const char* packageName,
...
UOption options[] = {
    UOPTION_HELP_H, UOPTION_HELP_QUESTION_MARK, UOPTION_VERBOSE, UOPTION_QUIET, UOPTION_VERSION, UOPTION_SOURCEDIR, UOPTION_DESTDIR, UOPTION_ENCODING,
    UOPTION_ICUDATADIR, UOPTION_WRITE_JAVA, UOPTION_COPYRIGHT, UOPTION_DEF("java-package", '\x01', UOPT_REQUIRES_ARG), UOPTION_BUNDLE_NAME,
    UOPTION_DEF("write-xliff", 'x', UOPT_OPTIONAL_ARG), UOPTION_DEF("strict", 'k', UOPT_NO_ARG), /* 14 */
...
int main(int argc, char* argv[])
{
    ...
    if (options[USE_POOL_BUNDLE].doesOccur) {
         ...
         */
        ds = udata_openSwapperForInputData(poolBundle.fBytes, bytesRead, U_IS_BIG_ENDIAN, U_CHARSET_FAMILY, &status);
        if (U_FAILURE(status)) {
        ...
        const int32_t* pRoot = (const int32_t*)((const char*)header + header->dataHeader.headerSize);
        ...
        if (stringUnitsLength >= 2 && getFormatVersion() >= 3) {
            ...
            do {
                ...
                if (!U16_IS_TRAIL(first)) {
                    // NUL-terminated
                    numCharsForLength = 0;
                    for (length = 0; length < remaining && p[length] != 0; ++length) {
                    }
                ...
                if ((numCharsForLength + length) >= remaining || p[numCharsForLength + length] != 0) {
                ...
                if (length != 0) {
                    StringResource* sr = new StringResource(poolStringIndex, numCharsForLength, p, length, status);
                    if (sr == NULL) {
    ...
    if (!options[FORMAT_VERSION].doesOccur && getFormatVersion() == 3 && poolBundle.fStrings == NULL && !options[WRITE_POOL_BUNDLE].doesOccur) {
    ...
    for (i = 1; i < argc; ++i) {
        ...
        processFile(arg, encoding, inputDir, outputDir, filterDir, NULL, newPoolBundle.getAlias(), options[NO_BINARY_COLLATION].doesOccur, status);
    }
    ...
    SRBRoot* newPoolBundle, UBool omitBinaryCollation, UErrorCode& status)
{
    ...
    data.adoptInstead(parse(ucbuf.getAlias(), inputDir, outputDir, filename, !omitBinaryCollation, options[NO_COLLATION_RULES].doesOccur, &status));
    ...
    if (write_java == TRUE) {
        bundle_write_java(
            data.getAlias(), outputDir, outputEnc, outputFileName, sizeof(outputFileName), options[JAVA_PACKAGE].value, options[BUNDLE_NAME].value, &status);
    } else if (write_xliff == TRUE) {
        bundle_write_xml(data.getAlias(), outputDir, outputEnc, filename, outputFileName, sizeof(outputFileName), language, xliffOutputFileName, &status);
    } else {
...
static char* make_res_filename(const char* filename, const char* outputDir, const char* packageName, UErrorCode& status)
{
    ...
    if (outputDir == NULL) {
        ...
        resName = (char*)uprv_malloc(sizeof(char) * (uprv_strlen(dirname) + pkgLen + uprv_strlen(basename) + uprv_strlen(RES_SUFFIX) + 8));
        if (resName == 0) {
            status = U_MEMORY_ALLOCATION_ERROR;
            goto finish;
        }
