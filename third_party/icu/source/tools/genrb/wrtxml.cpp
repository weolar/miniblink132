...
static int32_t write_utf8_file(FileStream* fileStream, UnicodeString outString)
{
    ...
    u_strToUTF8(NULL, 0, &len, toUCharPtr(outString.getBuffer()), outString.length(), &status);
    ...
    // convert the data
    u_strToUTF8(dest, len, &len, toUCharPtr(outString.getBuffer()), outString.length(), &status);
...
static char* convertAndEscape(char** pDest, int32_t destCap, int32_t* destLength, const UChar* src, int32_t srcLen, UErrorCode* status)
{
    int32_t srcIndex = 0;
    char* dest = NULL;
    char* temp = NULL;
...
static void printComments(struct UString* src, const char* resName, UBool printTranslate, UErrorCode* status)
{
...
static void bin_write_xml(BinaryResource* res, const char* id, const char* /*language*/, UErrorCode* status)
{
    ...
    char* fn = (char*)uprv_malloc(sizeof(char) * (tLen + 1024 + (res->fFileName != NULL ? uprv_strlen(res->fFileName) : 0)));
    const char* ext = NULL;
...
static void table_write_xml(TableResource* res, const char* id, const char* language, UBool isTopLevel, UErrorCode* status)
{
...
void bundle_write_xml(struct SRBRoot* bundle, const char* outputDir, const char* outputEnc, const char* filename, char* writtenFilename, int writtenFilenameLen,
