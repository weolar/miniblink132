...
static void intvector_write_java(const IntVectorResource* res, UErrorCode* /*status*/)
{
    uint32_t i = 0;
...
static void int_write_java(const IntResource* res, UErrorCode* /*status*/)
{
    const char* intC = "new Integer(";
    char buf[100];
    int len = 0;
...
static void bytes_write_java(const BinaryResource* res, UErrorCode* /*status*/)
{
    ...
    if (res->fCount > 0) {
        ...
        while (current != NULL) {
            ...
            if (currentKeyString != NULL) {
                T_FileStream_write(out, "\"", 1);
                T_FileStream_write(out, currentKeyString, (int32_t)uprv_strlen(currentKeyString));
                T_FileStream_write(out, "\",\n", 2);
