...
static const UVersionInfo gFormatVersions[4] = { /* indexed by a major-formatVersion integer */
    { 0, 0, 0, 0 }, { 1, 3, 0, 0 }, { 2, 0, 0, 0 }, { 3, 0, 0, 0 }
};
...
SResource::SResource(SRBRoot* bundle, const char* tag, int8_t type, const UString* comment, UErrorCode& errorCode)
    : fType(type)
    , fWritten(FALSE)
    , fRes(RES_BOGUS)
    , fRes16(-1)
...
StringBaseResource::StringBaseResource(
    SRBRoot* bundle, const char* tag, int8_t type, const UChar* value, int32_t len, const UString* comment, UErrorCode& errorCode)
...
StringBaseResource::StringBaseResource(SRBRoot* bundle, int8_t type, const icu::UnicodeString& value, UErrorCode& errorCode)
    : SResource(bundle, NULL, type, NULL, errorCode)
    , fString(value)
{
...
StringBaseResource::StringBaseResource(int8_t type, const UChar* value, int32_t len, UErrorCode& errorCode)
    : SResource(NULL, NULL, type, NULL, errorCode)
    , fString(TRUE, value, len)
{
    assert(len > 0);
...
static int32_t U_CALLCONV string_hash(const UElement key)
{
    const StringResource* res = static_cast<const StringResource*>(key.pointer);
    return res->fString.hashCode();
}

static UBool U_CALLCONV string_comp(const UElement key1, const UElement key2)
{
...
IntResource::IntResource(SRBRoot* bundle, const char* tag, int32_t value, const UString* comment, UErrorCode& errorCode)
...
IntVectorResource::IntVectorResource(SRBRoot* bundle, const char* tag, const UString* comment, UErrorCode& errorCode)
    : SResource(bundle, tag, URES_INT_VECTOR, comment, errorCode)
    , fCount(0)
    , fSize(RESLIST_INT_VECTOR_INIT_SIZE)
...
BinaryResource::BinaryResource(
    SRBRoot* bundle, const char* tag, uint32_t length, uint8_t* data, const char* fileName, const UString* comment, UErrorCode& errorCode)
...
void StringResource::handlePreflightStrings(SRBRoot* bundle, UHashtable* stringSet, UErrorCode& errorCode)
{
    assert(fSame == NULL);
    ...
    if (bundle->fStringsForm != STRINGS_UTF16_V1) {
        int32_t len = length();
        if (len <= MAX_IMPLICIT_STRING_LENGTH && !U16_IS_TRAIL(fString[0]) && fString.indexOf((UChar)0) < 0) {
            /*
...
void ContainerResource::handlePreflightStrings(SRBRoot* bundle, UHashtable* stringSet, UErrorCode& errorCode)
{
...
void SResource::preflightStrings(SRBRoot* bundle, UHashtable* stringSet, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return;
    }
...
void SResource::handlePreflightStrings(SRBRoot* /*bundle*/, UHashtable* /*stringSet*/, UErrorCode& /*errorCode*/)
 ...
 * and exits early.
 */
void StringResource::handleWrite16(SRBRoot* /*bundle*/)
{
    SResource* same;
    if ((same = fSame) != NULL) {
        /* This is a duplicate. */
...
void ArrayResource::handleWrite16(SRBRoot* bundle)
{
...
void TableResource::handleWrite16(SRBRoot* bundle)
{
    if (fCount == 0 && gFormatVersion > 1) {
...
void PseudoListResource::handleWrite16(SRBRoot* /*bundle*/)
{
    fRes = URES_MAKE_EMPTY_RESOURCE(URES_TABLE);
    fWritten = TRUE;
}

void SResource::write16(SRBRoot* bundle)
{
    if (fKey >= 0) {
...
void SResource::handleWrite16(SRBRoot* /*bundle*/)
 ...
 * and exits early.
 */
void StringBaseResource::handlePreWrite(uint32_t* byteOffset)
{
...
void IntVectorResource::handlePreWrite(uint32_t* byteOffset)
{
    if (fCount == 0 && gFormatVersion > 1) {
...
void ArrayResource::handlePreWrite(uint32_t* byteOffset)
{
...
void TableResource::handlePreWrite(uint32_t* byteOffset)
{
    preWriteAllRes(byteOffset);
...
void SResource::preWrite(uint32_t* byteOffset)
{
    if (fRes != RES_BOGUS) {
        /*
         * The resource item word was already precomputed, which means
...
void SResource::handlePreWrite(uint32_t* /*byteOffset*/)
{
    assert(FALSE);
}
 ...
 */
void StringBaseResource::handleWrite(UNewDataMemory* mem, uint32_t* byteOffset)
{
    /* Write the UTF-16 v1 string. */
    int32_t len = length();
...
void ArrayResource::handleWrite(UNewDataMemory* mem, uint32_t* byteOffset)
{
...
void IntVectorResource::handleWrite(UNewDataMemory* mem, uint32_t* byteOffset)
{
    udata_write32(mem, fCount);
...
void BinaryResource::handleWrite(UNewDataMemory* mem, uint32_t* byteOffset)
{
    uint32_t pad = 0;
    uint32_t dataStart = *byteOffset + sizeof(fLength);
...
void TableResource::handleWrite(UNewDataMemory* mem, uint32_t* byteOffset)
{
    writeAllRes(mem, byteOffset);
    if (fTableType == URES_TABLE) {
...
void SResource::write(UNewDataMemory* mem, uint32_t* byteOffset)
{
    if (fWritten) {
...
void SResource::handleWrite(UNewDataMemory* /*mem*/, uint32_t* /*byteOffset*/)
{
    assert(FALSE);
}

void SRBRoot::write(const char* outputDir, const char* outputPkg, char* writtenFilename, int writtenFilenameLen, UErrorCode& errorCode)
{
    ...
    if (gFormatVersion > 1) {
        ...
        if (U_SUCCESS(errorCode) && fUsePoolBundle != NULL && fUsePoolBundle->fStrings != NULL) {
            for (SResource* current = fUsePoolBundle->fStrings->fFirst; current != NULL; current = current->fNext) {
    ...
    if (fPoolStringIndexLimit != 0) {
        ...
        if (fPoolStringIndexLimit < 0x10000 && sum <= 0x10000) {
            ...
            fPoolStringIndex16Limit = (int32_t)(((int64_t)fPoolStringIndexLimit * 0xffff) / sum);
        }
    ...
    mem = udata_create(outputDir, "res", dataName, &dataInfo, (gIncludeCopyright == TRUE) ? U_COPYRIGHT_STRING : NULL, &errorCode);
    if (U_FAILURE(errorCode)) {
        return;
    }
    ...
    if (URES_INDEX_POOL_CHECKSUM < fIndexLength) {
        if (fIsPoolBundle) {
            ...
            uint32_t checksum = computeCRC((const char*)(fKeys + fKeysBottom), (uint32_t)(fKeysTop - fKeysBottom), 0);
            if (f16BitUnits.length() <= 1) {
                // no pool strings to checksum
            } else if (U_IS_BIG_ENDIAN) {
                checksum = computeCRC(reinterpret_cast<const char*>(f16BitUnits.getBuffer()), (uint32_t)f16BitUnits.length() * 2, checksum);
            } else {
                ...
                checksum = computeCRC((const char*)s.getBuffer(), (uint32_t)f16BitUnits.length() * 2, checksum);
            }
    ...
    udata_writeBlock(mem, fKeys + fKeysBottom, fKeysTop - fKeysBottom);
...
{
    LocalPointer<SResource> res(new StringResource(bundle, tag, value, len, comment, *status), *status);
    return U_SUCCESS(*status) ? res.orphan() : NULL;
}
...
{
    LocalPointer<SResource> res(new AliasResource(bundle, tag, value, len, comment, *status), *status);
    return U_SUCCESS(*status) ? res.orphan() : NULL;
}

IntVectorResource* intvector_open(struct SRBRoot* bundle, const char* tag, const struct UString* comment, UErrorCode* status)
{
    LocalPointer<IntVectorResource> res(new IntVectorResource(bundle, tag, comment, *status), *status);
    return U_SUCCESS(*status) ? res.orphan() : NULL;
}
...
SRBRoot::SRBRoot(const UString* comment, UBool isPoolBundle, UErrorCode& errorCode)
    ...
    , fLocalKeyLimit(0)
    , f16BitUnits()
    , f16BitStringsLength(0)
    , fUsePoolBundle(&kNoPoolBundle)
    , fPoolStringIndexLimit(0)
    , fPoolStringIndex16Limit(0)
    , fLocalStringIndexLimit(0)
...
const char* SRBRoot::getKeyString(int32_t key) const
{
    if (key < 0) {
...
const char* SResource::getKeyString(const SRBRoot* bundle) const
{
    if (fKey == -1) {
        return NULL;
    }
...
const char* SRBRoot::getKeyBytes(int32_t* pLength) const
{
    *pLength = fKeysTop - fKeysBottom;
    return fKeys + fKeysBottom;
}

int32_t SRBRoot::addKeyBytes(const char* keyBytes, int32_t length, UErrorCode& errorCode)
{
    int32_t keypos;
...
int32_t SRBRoot::addTag(const char* tag, UErrorCode& errorCode)
{
    int32_t keypos;
...
static int32_t U_CALLCONV compareKeySuffixes(const void* context, const void* l, const void* r)
{
...
static int32_t U_CALLCONV compareKeyNewpos(const void* /*context*/, const void* l, const void* r)
{
    return compareInt32(((const KeyMapEntry*)l)->newpos, ((const KeyMapEntry*)r)->newpos);
}

static int32_t U_CALLCONV compareKeyOldpos(const void* /*context*/, const void* l, const void* r)
{
...
void SRBRoot::compactKeys(UErrorCode& errorCode)
{
    KeyMapEntry* map;
    char* keys;
    int32_t i;
    ...
    for (i = 0; i < fUsePoolBundle->fKeysCount; ++i) {
        map[i].oldpos = (int32_t)(keys - fUsePoolBundle->fKeys) | 0x80000000; /* negative oldpos */
        map[i].newpos = 0;
        while (*keys != 0) {
            ++keys;
        } /* skip the key */
    ...
    uprv_sortArray(map, keysCount, (int32_t)sizeof(KeyMapEntry), compareKeySuffixes, this, FALSE, &errorCode);
    /*
    ...
    if (U_SUCCESS(errorCode)) {
         ...
         */
        uprv_sortArray(map, keysCount, (int32_t)sizeof(KeyMapEntry), compareKeyNewpos, NULL, FALSE, &errorCode);
        if (U_SUCCESS(errorCode)) {
            ...
            uprv_sortArray(map, keysCount, (int32_t)sizeof(KeyMapEntry), compareKeyOldpos, NULL, FALSE, &errorCode);
            if (U_SUCCESS(errorCode)) {
...
static int32_t U_CALLCONV compareStringLengths(const void* /*context*/, const void* l, const void* r)
{
    const StringResource* left = *((const StringResource**)l);
    const StringResource* right = *((const StringResource**)r);
    int32_t diff;
...
void StringResource::writeUTF16v2(int32_t base, UnicodeString& dest)
{
    int32_t len = length();
...
void SRBRoot::compactStringsV2(UHashtable* stringSet, UErrorCode& errorCode)
{
    ...
    uprv_sortArray(array.getAlias(), count, (int32_t)sizeof(struct SResource**), compareStringSuffixes, NULL, FALSE, &errorCode);
    if (U_FAILURE(errorCode)) {
        return;
    }
    /*
            ...
            if (res->fString.endsWith(suffixRes->fString)) {
                ...
                if (suffixRes->fWritten) {
                    ...
                    if (res->fWritten) {
                        ...
                        suffixRes->fRes = res->fRes + res->fNumCharsForLength + suffixRes->fSuffixOffset;
     ...
     */
    uprv_sortArray(array.getAlias(), count, (int32_t)sizeof(struct SResource**), compareStringLengths, NULL, FALSE, &errorCode);
    if (U_FAILURE(errorCode)) {
        return;
    }
    if (fIsPoolBundle) {
        ...
        for (int32_t i = 0; i < count; ++i) {
            ...
            int32_t maxStringIndex = f16BitUnits.length() + res->fNumCharsForLength + res->length() - 1;
            if (res->fNumUnitsSaved >= GENRB_MIN_16BIT_UNITS_SAVED_FOR_POOL_STRING && maxStringIndex < RES_MAX_OFFSET) {
        ...
        if (fWritePoolBundle != NULL && gFormatVersion >= 3) {
            PseudoListResource* poolStrings = static_cast<PseudoListResource*>(fWritePoolBundle->fRoot);
            for (i = 0; i < count && array[i]->fSame == NULL; ++i) {
                assert(!array[i]->fString.isEmpty());
                StringResource* poolString = new StringResource(fWritePoolBundle, array[i]->fString, errorCode);
                if (poolString == NULL) {
                    errorCode = U_MEMORY_ALLOCATION_ERROR;
                    break;
                }
...
void SResource::applyFilter(const PathFilter& /*filter*/, ResKeyPath& /*path*/, const SRBRoot* /*bundle*/)
{
    // Only a few resource types (tables) are capable of being filtered.
}

void TableResource::applyFilter(const PathFilter& filter, ResKeyPath& path, const SRBRoot* bundle)
{
