...
U_CFUNC void ustr_initChars(struct UString* s, const char* source, int32_t length, UErrorCode* status)
{
    int i = 0;
    if (U_FAILURE(*status))
        return;
...
U_CFUNC void ustr_cpy(struct UString* dst, const struct UString* src, UErrorCode* status)
{
...
U_CFUNC void ustr_cat(struct UString* dst, const struct UString* src, UErrorCode* status)
{
    ustr_ncat(dst, src, src->fLength, status);
}

U_CFUNC void ustr_ncat(struct UString* dst, const struct UString* src, int32_t n, UErrorCode* status)
{
    if (U_FAILURE(*status) || dst == src)
        return;
    ...
    uprv_memcpy(dst->fChars + dst->fLength, src->fChars, sizeof(UChar) * n);
    dst->fLength += src->fLength;
    dst->fChars[dst->fLength] = 0x0000;
}
...
U_CFUNC void ustr_ucat(struct UString* dst, UChar c, UErrorCode* status)
{
    ...
    uprv_memcpy(dst->fChars + dst->fLength, &c, sizeof(UChar) * 1);
    dst->fLength += 1;
    dst->fChars[dst->fLength] = 0x0000;
}
...
U_CFUNC void ustr_uscat(struct UString* dst, const UChar* src, int len, UErrorCode* status)
{
    ...
    uprv_memcpy(dst->fChars + dst->fLength, src, sizeof(UChar) * len);
    dst->fLength += len;
    dst->fChars[dst->fLength] = 0x0000;
}
