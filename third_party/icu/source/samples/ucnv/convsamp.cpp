...
void printUChars(const char* name = "?", const UChar* uch = kNone, int32_t len = -1)
{
    int32_t i;
...
void printBytes(const char* name = "?", const char* uch = "", int32_t len = -1)
{
    int32_t i;
...
UErrorCode convsample_05()
{
    ...
    while ((!feof(f)) && ((count = static_cast<int32_t>(fread(inBuf, 1, BUFFERSIZE, f))) > 0)) {
        ...
        do {
            ...
            ucnv_toUnicode(conv, &target, targetLimit, &source, sourceLimit, NULL, feof(f) ? true : false, /* pass 'flush' when eof */
...
UBool convsample_20_didSubstitute(const char* source)
{
    ...
    ucnv_setFromUCallBack(conv, flagCB_fromU, context, &(context->subCallback), &(context->subContext), &status);
...
UBool convsample_21_didSubstitute(const char* source)
{
    ...
    FromUFLAGContext *flagCtx = NULL, *cloneFlagCtx = NULL;

    debugCBContext *debugCtx1 = NULL, *debugCtx2 = NULL, *cloneDebugCtx = NULL;
    ...
    ucnv_setFromUCallBack(conv, debugCB_fromU, debugCtx1, &(debugCtx2->subCallback), &(debugCtx2->subContext), &status);
    ...
    printf("Cloned converter chain: %p -> %p[debug1] -> %p[flag] -> %p[debug2] -> substitute\n", cloneCnv, cloneDebugCtx, cloneFlagCtx,
        cloneFlagCtx ? cloneFlagCtx->subContext : NULL);
...
UErrorCode convsample_46()
{
    ...
    // grab another buffer's worth
    while ((!feof(f)) && ((count = static_cast<int32_t>(fread(inBuf, sizeof(UChar), BUFFERSIZE, f))) > 0)) {
        inchars += count;
        ...
        do {
            ...
            ucnv_fromUnicode(conv, &target, targetLimit, &source, sourceLimit, NULL, feof(f) ? true : false, /* pass 'flush' when eof */
                /* is true (when no more data will come) */
                &status);
...
void convsample_50()
{
    ...
    if (encoding != NULL && U_SUCCESS(err)) {
        ...
        // do the conversion
        ucnv_toUnicode(conv, &target, output + UPRV_LENGTHOF(output), &source, input + sizeof(input), NULL, true, &err);
        out = output;
        if (discardSignature) {
