...
U_CAPI UChar* U_EXPORT2 u_strFindFirst(const UChar* s, int32_t length, const UChar* sub, int32_t subLength)
{
...
U_CAPI UChar* U_EXPORT2 u_strFindLast(const UChar* s, int32_t length, const UChar* sub, int32_t subLength)
{
 ...
 */
static int32_t _matchFromSet(const UChar* string, const UChar* matchSet, UBool polarity)
{
...
U_CFUNC int32_t U_EXPORT2 uprv_strCompare(const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, UBool strncmpStyle, UBool codePointOrder)
{
    ...
    if (c1 >= 0xd800 && c2 >= 0xd800 && codePointOrder) {
        ...
        if ((c1 <= 0xdbff && (s1 + 1) != limit1 && U16_IS_TRAIL(*(s1 + 1))) || (U16_IS_TRAIL(c1) && start1 != s1 && U16_IS_LEAD(*(s1 - 1)))) {
        ...
        if ((c2 <= 0xdbff && (s2 + 1) != limit2 && U16_IS_TRAIL(*(s2 + 1))) || (U16_IS_TRAIL(c2) && start2 != s2 && U16_IS_LEAD(*(s2 - 1)))) {
            /* part of a surrogate pair, leave >=d800 */
        } else {
            /* BMP code point - may be surrogate code point - make <d800 */
            c2 -= 0x2800;
 ...
 * have stopped.
 */
U_CAPI int32_t U_EXPORT2 u_strCompareIter(UCharIterator* iter1, UCharIterator* iter2, UBool codePointOrder)
{
    UChar32 c1, c2;
    ...
    if (c1 >= 0xd800 && c2 >= 0xd800 && codePointOrder) {
        /* subtract 0x2800 from BMP code points to make them smaller than supplementary ones */
        if ((c1 <= 0xdbff && U16_IS_TRAIL(iter1->current(iter1))) || (U16_IS_TRAIL(c1) && (iter1->previous(iter1), U16_IS_LEAD(iter1->previous(iter1))))) {
            /* part of a surrogate pair, leave >=d800 */
        } else {
            /* BMP code point - may be surrogate code point - make <d800 */
            c1 -= 0x2800;
        ...
        if ((c2 <= 0xdbff && U16_IS_TRAIL(iter2->current(iter2))) || (U16_IS_TRAIL(c2) && (iter2->previous(iter2), U16_IS_LEAD(iter2->previous(iter2))))) {
            /* part of a surrogate pair, leave >=d800 */
        } else {
            /* BMP code point - may be surrogate code point - make <d800 */
            c2 -= 0x2800;
...
U_CAPI int32_t U_EXPORT2 u_strncmp(const UChar* s1, const UChar* s2, int32_t n)
{
    if (n > 0) {
        int32_t rc;
        for (;;) {
...
U_CAPI UBool U_EXPORT2 u_strHasMoreChar32Than(const UChar* s, int32_t length, int32_t number)
{
...
static UChar U_CALLCONV _charPtr_charAt(int32_t offset, void* context)
{
    UChar c16;
...
static void _appendUChars(UChar* dest, int32_t destCapacity, const char* src, int32_t srcLen)
{
    if (destCapacity < 0) {
        destCapacity = 0;
    }
...
U_CAPI int32_t U_EXPORT2 u_unescape(const char* src, UChar* dest, int32_t destCapacity)
{
    ...
    while ((c = *src) != 0) {
        ...
        if (c == '\\') {
            ...
            if (src != segment) {
                if (dest != NULL) {
                    _appendUChars(dest + i, destCapacity - i, segment, (int32_t)(src - segment));
                }
