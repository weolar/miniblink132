...
static int32_t U_CALLCONV noopGetIndex(UCharIterator* /*iter*/, UCharIteratorOrigin /*origin*/)
{
    return 0;
}

static int32_t U_CALLCONV noopMove(UCharIterator* /*iter*/, int32_t /*delta*/, UCharIteratorOrigin /*origin*/)
{
    return 0;
}

static UBool U_CALLCONV noopHasNext(UCharIterator* /*iter*/)
{
    return FALSE;
}

static UChar32 U_CALLCONV noopCurrent(UCharIterator* /*iter*/)
{
    return U_SENTINEL;
}

static uint32_t U_CALLCONV noopGetState(const UCharIterator* /*iter*/)
{
    return UITER_NO_STATE;
}

static void U_CALLCONV noopSetState(UCharIterator* /*iter*/, uint32_t /*state*/, UErrorCode* pErrorCode)
{
    *pErrorCode = U_UNSUPPORTED_ERROR;
}

static const UCharIterator noopIterator
    = { 0, 0, 0, 0, 0, 0, noopGetIndex, noopMove, noopHasNext, noopHasNext, noopCurrent, noopCurrent, noopCurrent, NULL, noopGetState, noopSetState };
...
static int32_t U_CALLCONV stringIteratorGetIndex(UCharIterator* iter, UCharIteratorOrigin origin)
{
    switch (origin) {
    case UITER_ZERO:
        return 0;
...
static int32_t U_CALLCONV stringIteratorMove(UCharIterator* iter, int32_t delta, UCharIteratorOrigin origin)
{
    int32_t pos;
...
static UBool U_CALLCONV stringIteratorHasNext(UCharIterator* iter)
{
    return iter->index < iter->limit;
}

static UBool U_CALLCONV stringIteratorHasPrevious(UCharIterator* iter)
{
    return iter->index > iter->start;
}

static UChar32 U_CALLCONV stringIteratorCurrent(UCharIterator* iter)
{
...
static UChar32 U_CALLCONV stringIteratorNext(UCharIterator* iter)
{
    if (iter->index < iter->limit) {
        return ((const UChar*)(iter->context))[iter->index++];
    } else {
...
static UChar32 U_CALLCONV stringIteratorPrevious(UCharIterator* iter)
{
...
static uint32_t U_CALLCONV stringIteratorGetState(const UCharIterator* iter)
{
    return (uint32_t)iter->index;
}

static void U_CALLCONV stringIteratorSetState(UCharIterator* iter, uint32_t state, UErrorCode* pErrorCode)
{
...
static const UCharIterator stringIterator = { 0, 0, 0, 0, 0, 0, stringIteratorGetIndex, stringIteratorMove, stringIteratorHasNext, stringIteratorHasPrevious,
    stringIteratorCurrent, stringIteratorNext, stringIteratorPrevious, NULL, stringIteratorGetState, stringIteratorSetState };
...
static UChar32 U_CALLCONV utf16BEIteratorCurrent(UCharIterator* iter)
{
    int32_t index;
...
static UChar32 U_CALLCONV utf16BEIteratorNext(UCharIterator* iter)
{
    int32_t index;
...
static UChar32 U_CALLCONV utf16BEIteratorPrevious(UCharIterator* iter)
{
    int32_t index;
...
static const UCharIterator utf16BEIterator = { 0, 0, 0, 0, 0, 0, stringIteratorGetIndex, stringIteratorMove, stringIteratorHasNext, stringIteratorHasPrevious,
    utf16BEIteratorCurrent, utf16BEIteratorNext, utf16BEIteratorPrevious, NULL, stringIteratorGetState, stringIteratorSetState };
...
static UBool U_CALLCONV characterIteratorHasNext(UCharIterator* iter)
{
    return ((CharacterIterator*)(iter->context))->hasNext();
}

static UBool U_CALLCONV characterIteratorHasPrevious(UCharIterator* iter)
{
...
static const UCharIterator characterIteratorWrapper
    = { 0, 0, 0, 0, 0, 0, characterIteratorGetIndex, characterIteratorMove, characterIteratorHasNext, characterIteratorHasPrevious, characterIteratorCurrent,
          characterIteratorNext, characterIteratorPrevious, NULL, characterIteratorGetState, characterIteratorSetState };
...
static UChar32 U_CALLCONV replaceableIteratorCurrent(UCharIterator* iter)
{
    if (iter->index < iter->limit) {
...
static UChar32 U_CALLCONV replaceableIteratorNext(UCharIterator* iter)
{
    if (iter->index < iter->limit) {
        return ((Replaceable*)(iter->context))->charAt(iter->index++);
    } else {
...
static UChar32 U_CALLCONV replaceableIteratorPrevious(UCharIterator* iter)
{
    if (iter->index > iter->start) {
        return ((Replaceable*)(iter->context))->charAt(--iter->index);
    } else {
...
static const UCharIterator replaceableIterator
    = { 0, 0, 0, 0, 0, 0, stringIteratorGetIndex, stringIteratorMove, stringIteratorHasNext, stringIteratorHasPrevious, replaceableIteratorCurrent,
          replaceableIteratorNext, replaceableIteratorPrevious, NULL, stringIteratorGetState, stringIteratorSetState };
...
static int32_t U_CALLCONV utf8IteratorGetIndex(UCharIterator* iter, UCharIteratorOrigin origin)
{
    switch (origin) {
    case UITER_ZERO:
    case UITER_START:
...
static UBool U_CALLCONV utf8IteratorHasNext(UCharIterator* iter)
{
    return iter->start < iter->limit || iter->reservedField != 0;
}

static UBool U_CALLCONV utf8IteratorHasPrevious(UCharIterator* iter)
{
    return iter->start > 0;
}

static UChar32 U_CALLCONV utf8IteratorCurrent(UCharIterator* iter)
{
    if (iter->reservedField != 0) {
...
static UChar32 U_CALLCONV utf8IteratorNext(UCharIterator* iter)
{
    int32_t index;
...
static UChar32 U_CALLCONV utf8IteratorPrevious(UCharIterator* iter)
{
    int32_t index;
...
static uint32_t U_CALLCONV utf8IteratorGetState(const UCharIterator* iter)
{
...
static void U_CALLCONV utf8IteratorSetState(UCharIterator* iter, uint32_t state, UErrorCode* pErrorCode)
{
    if (pErrorCode == NULL || U_FAILURE(*pErrorCode)) {
        /* do nothing */
    } else if (iter == NULL) {
...
static const UCharIterator utf8Iterator = { 0, 0, 0, 0, 0, 0, utf8IteratorGetIndex, utf8IteratorMove, utf8IteratorHasNext, utf8IteratorHasPrevious,
    utf8IteratorCurrent, utf8IteratorNext, utf8IteratorPrevious, NULL, utf8IteratorGetState, utf8IteratorSetState };

U_CAPI void U_EXPORT2 uiter_setUTF8(UCharIterator* iter, const char* s, int32_t length)
{
    if (iter != 0) {
        if (s != 0 && length >= -1) {
            *iter = utf8Iterator;
...
U_CAPI UChar32 U_EXPORT2 uiter_next32(UCharIterator* iter)
{
    UChar32 c, c2;
...
U_CAPI UChar32 U_EXPORT2 uiter_previous32(UCharIterator* iter)
{
    UChar32 c, c2;
...
U_CAPI uint32_t U_EXPORT2 uiter_getState(const UCharIterator* iter)
{
...
U_CAPI void U_EXPORT2 uiter_setState(UCharIterator* iter, uint32_t state, UErrorCode* pErrorCode)
{
    if (pErrorCode == NULL || U_FAILURE(*pErrorCode)) {
        /* do nothing */
    } else if (iter == NULL) {
