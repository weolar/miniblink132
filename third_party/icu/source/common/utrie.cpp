...
U_CAPI UNewTrie* U_EXPORT2 utrie_clone(UNewTrie* fillIn, const UNewTrie* other, uint32_t* aliasData, int32_t aliasDataCapacity)
{
    ...
    trie = utrie_open(fillIn, aliasData, aliasDataCapacity, other->data[0], other->leadUnitValue, other->isLatin1Linear);
    if (trie == NULL) {
        uprv_free(aliasData);
    } else {
 ...
 * @internal
 */
static void utrie_fillBlock(uint32_t* block, UChar32 start, UChar32 limit, uint32_t value, uint32_t initialValue, UBool overwrite)
{
    uint32_t* pLimit;
 ...
 */
static void utrie_compact(UNewTrie* trie, UBool overlap, UErrorCode* pErrorCode)
{
    ...
    for (start = newStart; start < trie->dataLength;) {
        ...
        if (start >= overlapStart && (i = _findSameDataBlock(trie->data, newStart, start, overlap ? UTRIE_DATA_GRANULARITY : UTRIE_DATA_BLOCK_LENGTH)) >= 0) {
        ...
        if (overlap && start >= overlapStart) {
            ...
            for (i = UTRIE_DATA_BLOCK_LENGTH - UTRIE_DATA_GRANULARITY; i > 0 && !equal_uint32(trie->data + (newStart - i), trie->data + start, i);
...
U_CAPI int32_t U_EXPORT2 utrie_serialize(
    UNewTrie* trie, void* dt, int32_t capacity, UNewTrieGetFoldedValue* getFoldedValue, UBool reduceTo16Bits, UErrorCode* pErrorCode)
{
    UTrieHeader* header;
    uint32_t* p;
    uint16_t* dest16;
