...
public:
    ...
    ResourceArray(const uint16_t* i16, const uint32_t* i32, int32_t len, const ResourceTracer& traceInfo)
        : items16(i16)
        , items32(i32)
        , length(len)
...
public:
    ...
    /** Only for implementation use. @internal */
    ResourceTable(const uint16_t* k16, const int32_t* k32, const uint16_t* i16, const uint32_t* i32, int32_t len, const ResourceTracer& traceInfo)
        : keys16(k16)
        , keys32(k32)
        , items16(i16)
        , items32(i32)
...
public:
     ...
     */
    virtual int32_t getStringArray(UnicodeString* dest, int32_t capacity, UErrorCode& errorCode) const = 0;
     ...
     * @see getStringArray()
     */
    virtual int32_t getStringArrayOrStringAsArray(UnicodeString* dest, int32_t capacity, UErrorCode& errorCode) const = 0;
...
public:
     ...
     */
    virtual void put(const char* key, ResourceValue& value, UBool noFallback, UErrorCode& errorCode) = 0;
