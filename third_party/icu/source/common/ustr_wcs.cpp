...
static inline UBool u_growAnyBufferFromStatic(void* context, void** pBuffer, int32_t* pCapacity, int32_t reqCapacity, int32_t length, int32_t size)
{
...
/* helper function */
static wchar_t* _strToWCS(wchar_t* dest, int32_t destCapacity, int32_t* pDestLength, const UChar* src, int32_t srcLength, UErrorCode* pErrorCode)
{
    ...
    for (;;) {
        ...
        *pErrorCode = U_ZERO_ERROR;
        ...
        if (*pErrorCode == U_BUFFER_OVERFLOW_ERROR) {
            ...
            if (!u_growAnyBufferFromStatic(stackBuffer, (void**)&tempBuf, &tempBufCapacity, newCapacity, count, 1)) {
                goto cleanup;
            }
    ...
    if (count >= tempBufCapacity) {
        tempBuf = saveBuf;
        /* we don't have enough room on the stack grow the buffer */
        if (!u_growAnyBufferFromStatic(stackBuffer, (void**)&tempBuf, &tempBufCapacity, count + 1, count, 1)) {
            goto cleanup;
        }
        saveBuf = tempBuf;
    }
            ...
            if (retVal == -1) {
                *pErrorCode = U_INVALID_CHAR_FOUND;
                ...
                u_growAnyBufferFromStatic(
                    NULL, (void**)&intTarget, &intTargetCapacity, intTargetCapacity * _BUFFER_CAPACITY_MULTIPLIER, numWritten, sizeof(wchar_t));
...
#if !defined(U_WCHAR_IS_UTF16) && !defined(U_WCHAR_IS_UTF32)
/* helper function */
static UChar* _strFromWCS(UChar* dest, int32_t destCapacity, int32_t* pDestLength, const wchar_t* src, int32_t srcLength, UErrorCode* pErrorCode)
{
        ...
        for (;;) {
            ...
            if (retVal == -1) {
                *pErrorCode = U_ILLEGAL_CHAR_FOUND;
                ...
                /* Should rarely occur */
                u_growAnyBufferFromStatic(cStack, (void**)&pCSrc, &cStackCap, cStackCap * _BUFFER_CAPACITY_MULTIPLIER, 0, sizeof(char));
                pCSave = pCSrc;
            } else {
        ...
        for (;;) {
            ...
            if ((pSrc + nulLen) < pSrcLimit) {
                ...
                if (remaining < (nulLen * MB_CUR_MAX)) {
                    ...
                    u_growAnyBufferFromStatic(
                        cStack, (void**)&pCSrc, &cStackCap, _BUFFER_CAPACITY_MULTIPLIER * cStackCap + (nulLen * MB_CUR_MAX), len, sizeof(char));
                ...
                if (remaining < (nulLen * MB_CUR_MAX)) {
                    ...
                    pCSrc = pCSave;
                    /* we do not have enough room so grow the buffer*/
                    u_growAnyBufferFromStatic(cStack, (void**)&pCSrc, &cStackCap, cStackCap + (nulLen * MB_CUR_MAX), len, sizeof(char));
