...
typedef enum { SPECIALTYPE_NONE = 0, SPECIALTYPE_CODEPOINTS = 1, SPECIALTYPE_REORDER_CODE = 2, SPECIALTYPE_RG_KEY_VALUE = 4 } SpecialType;
...
static UBool U_CALLCONV uloc_key_type_cleanup(void)
{
            ...
            while (ures_hasNext(typeMapResByKey.getAlias())) {
                ...
                if (isTZ) {
                    ...
                    if (uprv_strchr(legacyTypeId, ':') != NULL) {
                        ...
                        std::replace(legacyTypeIdBuf->data(), legacyTypeIdBuf->data() + legacyTypeIdBuf->length(), ':', '/');
...
static UBool isSpecialTypeCodepoints(const char* val)
{
    int32_t subtagLen = 0;
    const char* p = val;
    while (*p) {
        if (*p == '-') {
            ...
            subtagLen = 0;
        } else if ((*p >= '0' && *p <= '9') || (*p >= 'A' && *p <= 'F') || // A-F/a-f are contiguous
...
static UBool isSpecialTypeReorderCode(const char* val)
{
    int32_t subtagLen = 0;
    const char* p = val;
    while (*p) {
...
static UBool isSpecialTypeRgKeyValue(const char* val)
{
    int32_t subtagLen = 0;
    const char* p = val;
    while (*p) {
