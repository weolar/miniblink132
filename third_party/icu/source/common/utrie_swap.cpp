...
U_CAPI int32_t U_EXPORT2 utrie_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* pErrorCode)
{
    ...
    if (trie.signature != 0x54726965 || (trie.options & UTRIE_OPTIONS_SHIFT_MASK) != UTRIE_SHIFT
        || ((trie.options >> UTRIE_OPTIONS_INDEX_SHIFT) & UTRIE_OPTIONS_SHIFT_MASK) != UTRIE_INDEX_SHIFT || trie.indexLength < UTRIE_BMP_INDEX_LENGTH
        || (trie.indexLength & (UTRIE_SURROGATE_BLOCK_COUNT - 1)) != 0 || trie.dataLength < UTRIE_DATA_BLOCK_LENGTH
    ...
    if (length >= 0) {
        ...
        if (dataIs32) {
            ...
            ds->swapArray32(ds, (const uint16_t*)(inTrie + 1) + trie.indexLength, trie.dataLength * 4, (uint16_t*)(outTrie + 1) + trie.indexLength, pErrorCode);
        } else {
...
U_CAPI int32_t U_EXPORT2 utrie2_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* pErrorCode)
{
    ...
    if (trie.signature != UTRIE2_SIG || valueBits < 0 || UTRIE2_COUNT_VALUE_BITS <= valueBits || trie.indexLength < UTRIE2_INDEX_1_OFFSET
    ...
    if (length >= 0) {
        ...
        case UTRIE2_32_VALUE_BITS:
            ds->swapArray16(ds, inTrie + 1, trie.indexLength * 2, outTrie + 1, pErrorCode);
            ds->swapArray32(ds, (const uint16_t*)(inTrie + 1) + trie.indexLength, dataLength * 4, (uint16_t*)(outTrie + 1) + trie.indexLength, pErrorCode);
            break;
        default:
            *pErrorCode = U_INVALID_FORMAT_ERROR;
            return 0;
...
U_CAPI int32_t U_EXPORT2 ucptrie_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* pErrorCode)
{
    ...
    int32_t minIndexLength = type == UCPTRIE_TYPE_FAST ? UCPTRIE_BMP_INDEX_LENGTH : UCPTRIE_SMALL_INDEX_LENGTH;
    if (trie.signature != UCPTRIE_SIG || type > UCPTRIE_TYPE_SMALL || (trie.options & UCPTRIE_OPTIONS_RESERVED_MASK) != 0 || valueWidth > UCPTRIE_VALUE_BITS_8
        || trie.indexLength < minIndexLength || dataLength < ASCII_LIMIT) {
 ...
 */
int32_t getVersion(const void* data, int32_t length, UBool anyEndianOk)
{
    uint32_t signature;
