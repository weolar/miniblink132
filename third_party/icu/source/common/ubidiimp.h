...
#define MASK_LTR                                                                                                                                               \
    (DIRPROP_FLAG(L) | DIRPROP_FLAG(EN) | DIRPROP_FLAG(ENL) | DIRPROP_FLAG(ENR) | DIRPROP_FLAG(AN) | DIRPROP_FLAG(LRE) | DIRPROP_FLAG(LRO) | DIRPROP_FLAG(LRI))
...
U_CFUNC UBiDiLevel ubidi_getParaLevelAtIndex(const UBiDi* pBiDi, int32_t index);

#define GET_PARALEVEL(ubidi, index)                                                                                                                            \
    ((UBiDiLevel)(!(ubidi)->defaultParaLevel || (index) < (ubidi)->paras[0].limit ? (ubidi)->paraLevel : ubidi_getParaLevelAtIndex((ubidi), (index))))
...
U_CFUNC UBool ubidi_getRuns(UBiDi* pBiDi, UErrorCode* pErrorCode);
...
U_CFUNC UBool ubidi_getMemory(BidiMemoryForAllocation* pMemory, int32_t* pSize, UBool mayAllocate, int32_t sizeNeeded);

/* helper macros for each allocated array in UBiDi */
#define getDirPropsMemory(pBiDi, length)                                                                                                                       \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->dirPropsMemory, &(pBiDi)->dirPropsSize, (pBiDi)->mayAllocateText, (length))

#define getLevelsMemory(pBiDi, length)                                                                                                                         \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->levelsMemory, &(pBiDi)->levelsSize, (pBiDi)->mayAllocateText, (length))

#define getRunsMemory(pBiDi, length)                                                                                                                           \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->runsMemory, &(pBiDi)->runsSize, (pBiDi)->mayAllocateRuns, (length) * sizeof(Run))

/* additional macros used by ubidi_open() - always allow allocation */
#define getInitialDirPropsMemory(pBiDi, length) ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->dirPropsMemory, &(pBiDi)->dirPropsSize, true, (length))

#define getInitialLevelsMemory(pBiDi, length) ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->levelsMemory, &(pBiDi)->levelsSize, true, (length))

#define getInitialOpeningsMemory(pBiDi, length)                                                                                                                \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->openingsMemory, &(pBiDi)->openingsSize, true, (length) * sizeof(Opening))

#define getInitialParasMemory(pBiDi, length)                                                                                                                   \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->parasMemory, &(pBiDi)->parasSize, true, (length) * sizeof(Para))

#define getInitialRunsMemory(pBiDi, length) ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->runsMemory, &(pBiDi)->runsSize, true, (length) * sizeof(Run))

#define getInitialIsolatesMemory(pBiDi, length)                                                                                                                \
    ubidi_getMemory((BidiMemoryForAllocation*)&(pBiDi)->isolatesMemory, &(pBiDi)->isolatesSize, true, (length) * sizeof(Isolate))
