...
RBBIDataWrapper::RBBIDataWrapper(UDataMemory* udm, UErrorCode& status)
{
    ...
    if (!(headerSize >= 20 && dh->info.isBigEndian == U_IS_BIG_ENDIAN && dh->info.charsetFamily == U_CHARSET_FAMILY && dh->info.dataFormat[0] == 0x42
            && // dataFormat="Brk "
            dh->info.dataFormat[1] == 0x72 && dh->info.dataFormat[2] == 0x6b && dh->info.dataFormat[3] == 0x20
...
void RBBIDataWrapper::init(const RBBIDataHeader* data, UErrorCode& status)
{
    ...
    fTrie = ucptrie_openFromBinary(UCPTRIE_TYPE_FAST, UCPTRIE_VALUE_BITS_ANY, (uint8_t*)data + fHeader->fTrie, fHeader->fTrieLen,
    ...
    if (debugEnv && uprv_strstr(debugEnv, "data")) {
        this->printData();
    }
#endif
}
...
void RBBIDataWrapper::printTable(const char* heading, const RBBIStateTable* table)
{
    ...
    RBBIDebugPrintf("   Flags: %4x RBBI_LOOKAHEAD_HARD_BREAK=%s RBBI_BOF_REQUIRED=%s  RBBI_8BITS_ROWS=%s\n", table->fFlags,
        table->fFlags & RBBI_LOOKAHEAD_HARD_BREAK ? "T" : "F", table->fFlags & RBBI_BOF_REQUIRED ? "T" : "F", table->fFlags & RBBI_8BITS_ROWS ? "T" : "F");
    RBBIDebugPrintf("\nState |  Acc  LA TagIx");
    ...
    for (s = 0; s < table->fNumStates; s++) {
        RBBIStateTableRow* row = (RBBIStateTableRow*)(table->fTableData + (table->fRowLen * s));
        if (use8Bits) {
...
void RBBIDataWrapper::printData()
{
    ...
    RBBIDebugPrintf(
        "   Version = {%d %d %d %d}\n", fHeader->fFormatVersion[0], fHeader->fFormatVersion[1], fHeader->fFormatVersion[2], fHeader->fFormatVersion[3]);
...
U_CAPI int32_t U_EXPORT2 ubrk_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* status)
{
    ...
    if (!(pInfo->dataFormat[0] == 0x42 && /* dataFormat="Brk " */
            pInfo->dataFormat[1] == 0x72 && pInfo->dataFormat[2] == 0x6b && pInfo->dataFormat[3] == 0x20
    ...
    if (tableLength > 0) {
        ...
        ds->swapArray32(ds, inBytes + tableStartOffset, topSize, outBytes + tableStartOffset, status);
        ...
        if (use8Bits) {
            ...
            ds->swapArray16(ds, inBytes + tableStartOffset + topSize, tableLength - topSize, outBytes + tableStartOffset + topSize, status);
        }
    }
    ...
    ds->swapArray32(
        ds, inBytes + ds->readUInt32(rbbiDH->fStatusTable), ds->readUInt32(rbbiDH->fStatusTableLen), outBytes + ds->readUInt32(rbbiDH->fStatusTable), status);
