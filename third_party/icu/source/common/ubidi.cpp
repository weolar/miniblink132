...
U_CAPI UBiDi* U_EXPORT2 ubidi_openSized(int32_t maxLength, int32_t maxRunCount, UErrorCode* pErrorCode)
{
    ...
    if (maxLength > 0) {
        if (!getInitialDirPropsMemory(pBiDi, maxLength) || !getInitialLevelsMemory(pBiDi, maxLength)) {
            *pErrorCode = U_MEMORY_ALLOCATION_ERROR;
        }
    } else {
 ...
 */
U_CFUNC UBool ubidi_getMemory(BidiMemoryForAllocation* bidiMem, int32_t* pSize, UBool mayAllocate, int32_t sizeNeeded)
{
 ...
 */
static UBool checkParaCount(UBiDi* pBiDi)
{
 ...
 * replaced by an RLI.
 */
static UBool getDirProps(UBiDi* pBiDi)
{
    ...
    UBool isDefaultLevelInverse = isDefaultLevel
        && (UBool)(pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_LIKE_DIRECT || pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_FOR_NUMBERS_SPECIAL);
...
U_CFUNC UBiDiLevel ubidi_getParaLevelAtIndex(const UBiDi* pBiDi, int32_t pindex)
{
    int32_t i;
    for (i = 0; i < pBiDi->paraCount; i++)
...
static void bracketInit(UBiDi* pBiDi, BracketData* bd)
{
    bd->pBiDi = pBiDi;
    bd->isoRunLast = 0;
    ...
    bd->isNumbersSpecial = bd->pBiDi->reorderingMode == UBIDI_REORDER_NUMBERS_SPECIAL || bd->pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_FOR_NUMBERS_SPECIAL;
}
...
/* LRE, LRO, RLE, RLO, PDF */
static void bracketProcessBoundary(BracketData* bd, int32_t lastCcPos, UBiDiLevel contextLevel, UBiDiLevel embeddingLevel)
{
...
bracketAddOpening(BracketData* bd, UChar match, int32_t position)
{
    ...
    if (pLastIsoRun->limit >= bd->openingsCount) { /* no available new entry */
        ...
        if (bd->openings == bd->simpleOpenings)
            uprv_memcpy(pBiDi->openingsMemory, bd->simpleOpenings, SIMPLE_OPENINGS_COUNT * sizeof(Opening));
...
bracketProcessClosing(BracketData* bd, int32_t openIdx, int32_t position)
{
    ...
    if ((direction == 0 && pOpening->flags & FOUND_L) || (direction == 1 && pOpening->flags & FOUND_R)) { /* N0b */
...
static UBiDiDirection directionFromFlags(UBiDi* pBiDi)
{
    ...
    }
    {
        ...
        for (i = 0; i < length; ++i) {
            ...
            case RLO:
                ...
                if (newLevel <= UBIDI_MAX_EXPLICIT_LEVEL && overflowIsolateCount == 0 && overflowEmbeddingCount == 0) {
                    lastCcPos = i;
 ...
 * after taking the explicit embeddings into account.
 */
static UBiDiDirection checkExplicitLevels(UBiDi* pBiDi, UErrorCode* pErrorCode)
{
    DirProp* dirProps = pBiDi->dirProps;
    UBiDiLevel* levels = pBiDi->levels;
    ...
    for (int32_t i = 0; i < length; ++i) {
        ...
        if (pBiDi->defaultParaLevel != 0 && i == currentParaLimit && (currentParaIndex + 1) < pBiDi->paraCount) {
...
static const ImpTabPair impTab_DEFAULT = { { &impTabL_DEFAULT, &impTabR_DEFAULT }, { &impAct0, &impAct0 } };
...
static const ImpTabPair impTab_NUMBERS_SPECIAL = { { &impTabL_NUMBERS_SPECIAL, &impTabR_DEFAULT }, { &impAct0, &impAct0 } };
...
static const ImpTabPair impTab_GROUP_NUMBERS_WITH_R = { { &impTabL_GROUP_NUMBERS_WITH_R, &impTabR_GROUP_NUMBERS_WITH_R }, { &impAct0, &impAct0 } };
...
static const ImpTabPair impTab_INVERSE_NUMBERS_AS_L = { { &impTabL_INVERSE_NUMBERS_AS_L, &impTabR_INVERSE_NUMBERS_AS_L }, { &impAct0, &impAct0 } };
 ...
 */
static const ImpTabPair impTab_INVERSE_LIKE_DIRECT = { { &impTabL_DEFAULT, &impTabR_INVERSE_LIKE_DIRECT }, { &impAct0, &impAct1 } };
...
static const ImpTabPair impTab_INVERSE_FOR_NUMBERS_SPECIAL = { { &impTabL_NUMBERS_SPECIAL, &impTabR_INVERSE_LIKE_DIRECT }, { &impAct0, &impAct1 } };
...
#define FIRSTALLOC 10
    ...
    if (pInsertPoints->size >= pInsertPoints->capacity) /* no room for new point */
    {
        ...
        pInsertPoints->points = static_cast<Point*>(uprv_realloc(pInsertPoints->points, pInsertPoints->capacity * 2 * sizeof(Point)));
        if (pInsertPoints->points == NULL) {
...
static void setLevelsOutsideIsolates(UBiDi* pBiDi, int32_t start, int32_t limit, UBiDiLevel level)
{
...
static void processPropertySeq(UBiDi* pBiDi, LevState* pLevState, uint8_t _prop, int32_t start, int32_t limit)
{
    ...
    if (actionSeq) {
        ...
        case 5: /* L or S after possible relevant EN/AN */
            ...
            if ((pInsertPoints->capacity == 0) || (pInsertPoints->size <= pInsertPoints->confirmed)) {
        ...
        case 7: /* EN/AN after R/AL + possible cont */
            /* check for real AN */
            if ((_prop == DirProp_AN) && (pBiDi->dirProps[start0] == AN) && (pBiDi->reorderingMode != UBIDI_REORDER_INVERSE_FOR_NUMBERS_SPECIAL)) {
                /* real AN */
...
static void resolveImplicitLevels(UBiDi* pBiDi, int32_t start, int32_t limit, DirProp sor, DirProp eor)
{
     ...
     */
    inverseRTL = (UBool)((start < pBiDi->lastArabicPos) && (GET_PARALEVEL(pBiDi, start) & 1)
        && (pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_LIKE_DIRECT || pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_FOR_NUMBERS_SPECIAL));
 ...
 */
static void adjustWSLevels(UBiDi* pBiDi)
{
    const DirProp* dirProps = pBiDi->dirProps;
    UBiDiLevel* levels = pBiDi->levels;
    int32_t i;
...
static void setParaRunsOnly(UBiDi* pBiDi, const UChar* text, int32_t length, UBiDiLevel paraLevel, UErrorCode* pErrorCode)
{
    ...
    Run* runs;
    int32_t visualLength, i, j, visualStart, logicalStart, runCount, runLength, addedRuns, insertRemove, start, limit, step, indexOddBit, logicalPos, index0,
        index1;
     ...
     */
    visualLength = ubidi_writeReordered(pBiDi, visualText, length, UBIDI_DO_MIRRORING, pErrorCode);
    ...
    for (i = runCount - 1; i >= 0; i--) {
        runLength = i == 0 ? runs[0].visualLimit : runs[i].visualLimit - runs[i - 1].visualLimit;
        ...
        if (runLength < 2) {
            ...
            runs[i + addedRuns].logicalStart = MAKE_INDEX_ODD_PAIR(logicalPos, saveLevels[logicalPos] ^ indexOddBit);
            continue;
        }
        if (indexOddBit) {
            start = logicalStart;
...
U_CAPI void U_EXPORT2 ubidi_setPara(UBiDi* pBiDi, const UChar* text, int32_t length, UBiDiLevel paraLevel, UBiDiLevel* embeddingLevels, UErrorCode* pErrorCode)
{
    ...
    /* check the argument values */
    RETURN_VOID_IF_NULL_OR_FAILING_ERRCODE(pErrorCode);
    if (pBiDi == NULL || text == NULL || length < -1 || (paraLevel > UBIDI_MAX_EXPLICIT_LEVEL && paraLevel < UBIDI_DEFAULT_LTR)) {
        *pErrorCode = U_ILLEGAL_ARGUMENT_ERROR;
        return;
    }
    ...
    else if ((int32_t)(pBiDi->isolateCount * sizeof(Isolate)) <= pBiDi->isolatesSize)
    ...
    default:
         ...
         */
        if (embeddingLevels == NULL && pBiDi->paraCount <= 1 && !(pBiDi->flags & DIRPROP_FLAG_MULTI_RUNS)) {
     ...
     */
    if ((pBiDi->defaultParaLevel > 0) && (pBiDi->reorderingOptions & UBIDI_OPTION_INSERT_MARKS)
        && ((pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_LIKE_DIRECT) || (pBiDi->reorderingMode == UBIDI_REORDER_INVERSE_FOR_NUMBERS_SPECIAL))) {
...
U_CAPI void U_EXPORT2 ubidi_getParagraphByIndex(
    const UBiDi* pBiDi, int32_t paraIndex, int32_t* pParaStart, int32_t* pParaLimit, UBiDiLevel* pParaLevel, UErrorCode* pErrorCode)
{
    int32_t paraStart;
...
U_CAPI int32_t U_EXPORT2 ubidi_getParagraph(
    const UBiDi* pBiDi, int32_t charIndex, int32_t* pParaStart, int32_t* pParaLimit, UBiDiLevel* pParaLevel, UErrorCode* pErrorCode)
{
    int32_t paraIndex;
...
U_CAPI void U_EXPORT2 ubidi_setClassCallback(
    UBiDi* pBiDi, UBiDiClassCallback* newFn, const void* newContext, UBiDiClassCallback** oldFn, const void** oldContext, UErrorCode* pErrorCode)
{
    RETURN_VOID_IF_NULL_OR_FAILING_ERRCODE(pErrorCode);
    if (pBiDi == NULL) {
        *pErrorCode = U_ILLEGAL_ARGUMENT_ERROR;
