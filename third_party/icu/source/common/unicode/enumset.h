...
public:
    ...
    inline void add(T toAdd)
    {
        set(toAdd, 1);
    }
    inline void remove(T toRemove)
    {
        set(toRemove, 0);
    }
    inline int32_t contains(T toCheck) const
    {
        return get(toCheck);
    }
    inline void set(T toSet, int32_t v)
    {
        fBools = (fBools & (~flag(toSet))) | (v ? (flag(toSet)) : 0);
    }
    ...
    {
        return ((uint32_t)toCheck >= minValue && (uint32_t)toCheck < limitValue);
    }
