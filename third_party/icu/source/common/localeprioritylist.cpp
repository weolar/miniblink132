...
int32_t U_CALLCONV compareLocaleAndWeight(const void* /*context*/, const void* left, const void* right)
{
    return static_cast<const LocaleAndWeight*>(left)->compare(*static_cast<const LocaleAndWeight*>(right));
}
...
LocalePriorityList::LocalePriorityList(StringPiece s, UErrorCode& errorCode)
{
    ...
    while ((p = skipSpaces(p, limit)) != limit) {
        ...
        if ((p = skipSpaces(p + tagLength, limit)) != limit && *p == ';') {
            if ((p = skipSpaces(p + 1, limit)) == limit || *p != 'q' || (p = skipSpaces(p + 1, limit)) == limit || *p != '='
...
void LocalePriorityList::sort(UErrorCode& errorCode)
{
    ...
    if (U_FAILURE(errorCode) || getLength() <= 1 || !hasWeights) {
        return;
    }
    uprv_sortArray(list->array.getAlias(), listLength, sizeof(LocaleAndWeight), compareLocaleAndWeight, nullptr, FALSE, &errorCode);
}
