...
U_CAPI void U_EXPORT2 upvec_compact(UPropsVectors* pv, UPVecCompactHandler* handler, void* context, UErrorCode* pErrorCode)
{
    ...
    uprv_sortArray(pv->v, rows, columns * 4, upvec_compareRows, pv, FALSE, pErrorCode);
    if (U_FAILURE(*pErrorCode)) {
        return;
    }
    ...
    handler(context, UPVEC_START_REAL_VALUES_CP, UPVEC_START_REAL_VALUES_CP, count, row - valueColumns, valueColumns, pErrorCode);
    if (U_FAILURE(*pErrorCode)) {
        return;
    }
        ...
        case UPVEC_START_REAL_VALUES_CP:
            ...
            if (rowIndex > 0xffff) {
                ...
                *pErrorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                ...
                toUTrie2->trie = utrie2_open(toUTrie2->initialValue, toUTrie2->errorValue, pErrorCode);
            }
            break;
        default:
            break;
