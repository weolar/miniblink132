...
U_CAPI void U_EXPORT2 ubidi_setLine(const UBiDi* pParaBiDi, int32_t start, int32_t limit, UBiDi* pLineBiDi, UErrorCode* pErrorCode)
{
    ...
    if (ubidi_getParagraph(pParaBiDi, start, NULL, NULL, NULL, pErrorCode) != ubidi_getParagraph(pParaBiDi, limit - 1, NULL, NULL, NULL, pErrorCode)) {
 ...
 */
U_CFUNC UBool ubidi_getRuns(UBiDi* pBiDi, UErrorCode*)
{
    /*
    ...
    if (pBiDi->insertPoints.size > 0) {
        Point *point, *start = pBiDi->insertPoints.points, *limit = start + pBiDi->insertPoints.size;
        int32_t runIndex;
