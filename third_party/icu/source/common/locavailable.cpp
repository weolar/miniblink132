...
const Locale* U_EXPORT2 Locale::getAvailableLocales(int32_t& count)
{
...
public:
    void put(const char* key, ResourceValue& value, UBool /*noFallback*/, UErrorCode& status) U_OVERRIDE
    {
        ...
        for (int32_t i = 0; resIndexTable.getKeyAndValue(i, key, value); ++i) {
            ...
            gAvailableLocaleNames[type] = static_cast<const char**>(uprv_malloc(gAvailableLocaleCounts[type] * sizeof(const char*)));
...
public:
    AvailableLocalesStringEnumeration(ULocAvailableType type)
        : fType(type)
    {
    }
...
U_CAPI const char* U_EXPORT2 uloc_getAvailable(int32_t offset)
{
...
U_CAPI UEnumeration* U_EXPORT2 uloc_openAvailableByType(ULocAvailableType type, UErrorCode* status)
{
    ...
    LocalPointer<AvailableLocalesStringEnumeration> result(new AvailableLocalesStringEnumeration(type), *status);
    if (U_FAILURE(*status)) {
        return nullptr;
    }
