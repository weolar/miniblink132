...
public:
    ...
    CacheKeyBase(const CacheKeyBase& other)
        : UObject(other)
        , fCreationStatus(other.fCreationStatus)
        , fIsPrimary(false)
    {
    }
    ...
    friend inline bool operator==(const CacheKeyBase& lhs, const CacheKeyBase& rhs)
    {
 ...
 */
template <typename T> class CacheKey : public CacheKeyBase {
public:
    virtual ~CacheKey()
    {
    }
 ...
 * A key of type LocaleCacheKey<T> maps to a value of type T.
 */
template <typename T> class LocaleCacheKey : public CacheKey<T> {
protected:
    Locale fLoc;
    virtual bool equals(const CacheKeyBase& other) const override
    {
...
public:
    LocaleCacheKey(const Locale& loc)
        : fLoc(loc)
    {
    }
...
public:
     ...
     */
    template <typename T> void get(const CacheKey<T>& key, const void* creationContext, const T*& ptr, UErrorCode& status) const
    {
        if (U_FAILURE(status)) {
            return;
        }
     ...
     *               warning value even if ptr is set.
     */
    template <typename T> static void getByLocale(const Locale& loc, const T*& ptr, UErrorCode& status)
    {
...
private:
     ...
     */
    UBool _poll(const CacheKeyBase& key, const SharedObject*& value, UErrorCode& status) const;
