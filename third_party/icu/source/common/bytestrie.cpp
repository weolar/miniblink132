...
UStringTrieResult BytesTrie::next(const char* s, int32_t sLength)
{
...
const uint8_t* BytesTrie::findUniqueValueFromBranch(const uint8_t* pos, int32_t length, UBool haveUniqueValue, int32_t& uniqueValue)
{
    while (length > kMaxBranchLinearSubNodeLength) {
...
UBool BytesTrie::findUniqueValue(const uint8_t* pos, UBool haveUniqueValue, int32_t& uniqueValue)
{
    for (;;) {
        int32_t node = *pos++;
        if (node < kMinLinearMatch) {
...
void BytesTrie::append(ByteSink& out, int c)
{
    char ch = (char)c;
    out.Append(&ch, 1);
}
