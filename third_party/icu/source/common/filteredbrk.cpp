...
public:
    UStringSet(UErrorCode& status)
        : UVector(uprv_deleteUObject, uhash_compareUnicodeString, 1, status)
    {
    }
    virtual ~UStringSet();
    /**
...
public:
    ...
    bool hasForwardsPartialTrie() const
    {
        return fForwardsPartialTrie.isValid();
    }
    bool hasBackwardsTrie() const
    {
        return fBackwardsTrie.isValid();
    }

    const UCharsTrie& getForwardsPartialTrie() const
    {
        return *fForwardsPartialTrie;
    }
    const UCharsTrie& getBackwardsTrie() const
    {
        return *fBackwardsTrie;
    }
...
public:
    ...
    virtual BreakIterator* createBufferClone(void* /*stackBuffer*/, int32_t& /*BufferSize*/, UErrorCode& status) override
    {
        // for now - always deep clone
        status = U_SAFECLONE_ALLOCATED_WARNING;
        return clone();
    }
    virtual SimpleFilteredSentenceBreakIterator* clone() const override
    {
        return new SimpleFilteredSentenceBreakIterator(*this);
    }
    ...
    {
        fDelegate->setText(text, status);
    }
    virtual BreakIterator& refreshInputText(UText* input, UErrorCode& status) override
    {
        fDelegate->refreshInputText(input, status);
        return *this;
    }
    virtual void adoptText(CharacterIterator* it) override
    {
        fDelegate->adoptText(it);
    }
    virtual void setText(const UnicodeString& text) override
    {
        fDelegate->setText(text);
    }
    ...
    {
        return fDelegate->current();
    } // we keep the delegate current, so this should be correct.
...
SimpleFilteredSentenceBreakIterator::SimpleFilteredSentenceBreakIterator(const SimpleFilteredSentenceBreakIterator& other)
    : BreakIterator(other)
    , fData(other.fData->incr())
    , fDelegate(other.fDelegate->clone())
{
}
...
SimpleFilteredSentenceBreakIterator::EFBMatchResult SimpleFilteredSentenceBreakIterator::breakExceptionAt(int32_t n)
{
    int64_t bestPosn = -1;
    int32_t bestValue = -1;
    ...
    if (bestPosn >= 0) {
        ...
        if (bestValue == kMATCH) { // exact match!
        ...
        } else if (bestValue == kPARTIAL && fData->hasForwardsPartialTrie()) { // make sure there's a forward trie
            ...
            UChar32 uch;
            while ((uch = utext_next32(fText.getAlias())) != U_SENTINEL && USTRINGTRIE_HAS_NEXT(rfwd = iter.nextForCodePoint(uch))) {
...
int32_t SimpleFilteredSentenceBreakIterator::internalNext(int32_t n)
{
...
int32_t SimpleFilteredSentenceBreakIterator::internalPrev(int32_t n)
{
...
{
    if (!fDelegate->isBoundary(offset))
        return false; // no break to suppress

    if (!fData->hasBackwardsTrie())
        return true; // no data = no suppressions
...
int32_t SimpleFilteredSentenceBreakIterator::next(int32_t offset)
{
...
UBool SimpleFilteredBreakIteratorBuilder::suppressBreakAfter(const UnicodeString& exception, UErrorCode& status)
{
...
UBool SimpleFilteredBreakIteratorBuilder::unsuppressBreakAfter(const UnicodeString& exception, UErrorCode& status)
{
...
BreakIterator* SimpleFilteredBreakIteratorBuilder::build(BreakIterator* adoptBreakIterator, UErrorCode& status)
{
...
FilteredBreakIteratorBuilder* FilteredBreakIteratorBuilder::createInstance(const Locale& where, UErrorCode& status)
{
    if (U_FAILURE(status))
        return NULL;
...
FilteredBreakIteratorBuilder* FilteredBreakIteratorBuilder::createInstance(UErrorCode& status)
{
    return createEmptyInstance(status);
}

FilteredBreakIteratorBuilder* FilteredBreakIteratorBuilder::createEmptyInstance(UErrorCode& status)
{
    if (U_FAILURE(status))
        return NULL;
