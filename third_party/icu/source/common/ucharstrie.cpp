...
UStringTrieResult UCharsTrie::current() const
{
    ...
    if (pos == NULL) {
        ...
        int32_t node;
        return (remainingMatchLength_ < 0 && (node = *pos) >= kMinValueLead) ? valueResult(node) : USTRINGTRIE_NO_VALUE;
    }
}

UStringTrieResult UCharsTrie::firstForCodePoint(UChar32 cp)
{
    return cp <= 0xffff ? first(cp) : (USTRINGTRIE_HAS_NEXT(first(U16_LEAD(cp))) ? next(U16_TRAIL(cp)) : USTRINGTRIE_NO_MATCH);
}

UStringTrieResult UCharsTrie::nextForCodePoint(UChar32 cp)
{
    return cp <= 0xffff ? next(cp) : (USTRINGTRIE_HAS_NEXT(next(U16_LEAD(cp))) ? next(U16_TRAIL(cp)) : USTRINGTRIE_NO_MATCH);
}
...
UStringTrieResult UCharsTrie::nextImpl(const UChar* pos, int32_t uchar)
{
    int32_t node = *pos++;
    for (;;) {
...
UStringTrieResult UCharsTrie::next(ConstChar16Ptr ptr, int32_t sLength)
{
    const UChar* s = ptr;
...
const UChar* UCharsTrie::findUniqueValueFromBranch(const UChar* pos, int32_t length, UBool haveUniqueValue, int32_t& uniqueValue)
{
    while (length > kMaxBranchLinearSubNodeLength) {
...
UBool UCharsTrie::findUniqueValue(const UChar* pos, UBool haveUniqueValue, int32_t& uniqueValue)
{
    int32_t node = *pos++;
    for (;;) {
        if (node < kMinLinearMatch) {
...
void UCharsTrie::getNextBranchUChars(const UChar* pos, int32_t length, Appendable& out)
{
    while (length > kMaxBranchLinearSubNodeLength) {
        ++pos; // ignore the comparison unit
