...
UnicodeString& FilteredNormalizer2::normalize(const UnicodeString& src, UnicodeString& dest, UErrorCode& errorCode) const
{
...
// an in-filter prefix.
UnicodeString& FilteredNormalizer2::normalize(const UnicodeString& src, UnicodeString& dest, USetSpanCondition spanCondition, UErrorCode& errorCode) const
{
    ...
    for (int32_t prevSpanLimit = 0; prevSpanLimit < src.length();) {
        ...
        if (spanCondition == USET_SPAN_NOT_CONTAINED) {
            ...
            if (spanLength != 0) {
                ...
                dest.append(norm2.normalize(src.tempSubStringBetween(prevSpanLimit, spanLimit), tempDest, errorCode));
                if (U_FAILURE(errorCode)) {
                    break;
                }
            }
...
void FilteredNormalizer2::normalizeUTF8(uint32_t options, StringPiece src, ByteSink& sink, Edits* edits, UErrorCode& errorCode) const
{
    if (U_FAILURE(errorCode)) {
        return;
    }
...
void FilteredNormalizer2::normalizeUTF8(
    uint32_t options, const char* src, int32_t length, ByteSink& sink, Edits* edits, USetSpanCondition spanCondition, UErrorCode& errorCode) const
{
    while (length > 0) {
...
UnicodeString& FilteredNormalizer2::normalizeSecondAndAppend(UnicodeString& first, const UnicodeString& second, UErrorCode& errorCode) const
{
    return normalizeSecondAndAppend(first, second, TRUE, errorCode);
}

UnicodeString& FilteredNormalizer2::append(UnicodeString& first, const UnicodeString& second, UErrorCode& errorCode) const
{
    return normalizeSecondAndAppend(first, second, FALSE, errorCode);
}

UnicodeString& FilteredNormalizer2::normalizeSecondAndAppend(UnicodeString& first, const UnicodeString& second, UBool doNormalize, UErrorCode& errorCode) const
{
...
UBool FilteredNormalizer2::getDecomposition(UChar32 c, UnicodeString& decomposition) const
{
    return set.contains(c) && norm2.getDecomposition(c, decomposition);
}

UBool FilteredNormalizer2::getRawDecomposition(UChar32 c, UnicodeString& decomposition) const
{
...
UBool FilteredNormalizer2::isNormalized(const UnicodeString& s, UErrorCode& errorCode) const
{
    ...
    for (int32_t prevSpanLimit = 0; prevSpanLimit < s.length();) {
        ...
        if (spanCondition == USET_SPAN_NOT_CONTAINED) {
            ...
            if (!norm2.isNormalized(s.tempSubStringBetween(prevSpanLimit, spanLimit), errorCode) || U_FAILURE(errorCode)) {
                return FALSE;
            }
            spanCondition = USET_SPAN_NOT_CONTAINED;
        }
...
UBool FilteredNormalizer2::isNormalizedUTF8(StringPiece sp, UErrorCode& errorCode) const
{
    if (U_FAILURE(errorCode)) {
        return FALSE;
    }
    ...
    while (length > 0) {
        ...
        if (spanCondition == USET_SPAN_NOT_CONTAINED) {
            ...
            if (!norm2.isNormalizedUTF8(StringPiece(s, spanLength), errorCode) || U_FAILURE(errorCode)) {
                return FALSE;
            }
            spanCondition = USET_SPAN_NOT_CONTAINED;
        }
...
UNormalizationCheckResult FilteredNormalizer2::quickCheck(const UnicodeString& s, UErrorCode& errorCode) const
{
    ...
    for (int32_t prevSpanLimit = 0; prevSpanLimit < s.length();) {
        ...
        if (spanCondition == USET_SPAN_NOT_CONTAINED) {
            ...
            UNormalizationCheckResult qcResult = norm2.quickCheck(s.tempSubStringBetween(prevSpanLimit, spanLimit), errorCode);
...
int32_t FilteredNormalizer2::spanQuickCheckYes(const UnicodeString& s, UErrorCode& errorCode) const
{
    ...
    for (int32_t prevSpanLimit = 0; prevSpanLimit < s.length();) {
        ...
        if (spanCondition == USET_SPAN_NOT_CONTAINED) {
            ...
            int32_t yesLimit = prevSpanLimit + norm2.spanQuickCheckYes(s.tempSubStringBetween(prevSpanLimit, spanLimit), errorCode);
...
UBool FilteredNormalizer2::hasBoundaryBefore(UChar32 c) const
{
    return !set.contains(c) || norm2.hasBoundaryBefore(c);
}

UBool FilteredNormalizer2::hasBoundaryAfter(UChar32 c) const
{
    return !set.contains(c) || norm2.hasBoundaryAfter(c);
}

UBool FilteredNormalizer2::isInert(UChar32 c) const
{
...
U_CAPI UNormalizer2* U_EXPORT2 unorm2_openFiltered(const UNormalizer2* norm2, const USet* filterSet, UErrorCode* pErrorCode)
{
    ...
    Normalizer2* fn2 = new FilteredNormalizer2(*(Normalizer2*)norm2, *UnicodeSet::fromUSet(filterSet));
    if (fn2 == NULL) {
