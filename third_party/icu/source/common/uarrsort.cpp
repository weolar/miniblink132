...
U_CAPI int32_t U_EXPORT2 uprv_stableBinarySearch(char* array, int32_t limit, void* item, int32_t itemSize, UComparator* cmp, const void* context)
{
    int32_t start = 0;
    UBool found = FALSE;
...
static void insertionSort(char* array, int32_t length, int32_t itemSize, UComparator* cmp, const void* context, UErrorCode* pErrorCode)
{
    ...
    if (sizeInMaxAlignTs(itemSize) > v.getCapacity() && v.resize(sizeInMaxAlignTs(itemSize)) == nullptr) {
...
static void quickSort(char* array, int32_t length, int32_t itemSize, UComparator* cmp, const void* context, UErrorCode* pErrorCode)
{
    ...
    if (sizeInMaxAlignTs(itemSize) * 2 > xw.getCapacity() && xw.resize(sizeInMaxAlignTs(itemSize) * 2) == nullptr) {
        *pErrorCode = U_MEMORY_ALLOCATION_ERROR;
        return;
    }

    subQuickSort(array, 0, length, itemSize, cmp, context, xw.getAlias(), xw.getAlias() + sizeInMaxAlignTs(itemSize));
}
