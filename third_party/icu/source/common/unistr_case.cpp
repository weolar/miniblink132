...
int8_t UnicodeString::doCaseCompare(int32_t start, int32_t length, const UChar* srcChars, int32_t srcStart, int32_t srcLength, uint32_t options) const
{
...
UnicodeString& UnicodeString::caseMap(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_PARAM UStringCaseMapper* stringCaseMapper)
{
    ...
    if (writable ? oldLength <= UPRV_LENGTHOF(oldBuffer) : oldLength < US_STACKBUF_SIZE) {
...
#endif
        newLength = stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR buffer, capacity, oldArray, oldLength, NULL, errorCode);
...
#endif
        stringCaseMapper(caseLocale, options | U_OMIT_UNCHANGED_TEXT, UCASEMAP_BREAK_ITERATOR replacementChars, UPRV_LENGTHOF(replacementChars), oldArray,
        ...
        if (U_SUCCESS(errorCode)) {
            ...
            for (Edits::Iterator ei = edits.getCoarseChangesIterator(); ei.next(errorCode);) {
                doReplace(ei.destinationIndex(), ei.oldLength(), replacementChars, ei.replacementIndex(), ei.newLength());
            }
    ...
    newLength = stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR getArrayStart(), getCapacity(), oldArray, oldLength, NULL, errorCode);
    if (bufferToDelete) {
