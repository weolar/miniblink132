...
UBool Normalizer2::getRawDecomposition(UChar32, UnicodeString&) const
{
    return FALSE;
}
...
UBool Normalizer2::isNormalizedUTF8(StringPiece s, UErrorCode& errorCode) const
{
...
class NoopNormalizer2 : public Normalizer2 {
    ...
    virtual UBool getDecomposition(UChar32, UnicodeString&) const U_OVERRIDE
    {
        return FALSE;
    }
    // No need to U_OVERRIDE the default getRawDecomposition().
    virtual UBool isNormalized(const UnicodeString&, UErrorCode& errorCode) const U_OVERRIDE
    {
        return U_SUCCESS(errorCode);
    }
    virtual UBool isNormalizedUTF8(StringPiece, UErrorCode& errorCode) const U_OVERRIDE
    {
        return U_SUCCESS(errorCode);
    }
    ...
    virtual UBool hasBoundaryBefore(UChar32) const U_OVERRIDE
    {
        return TRUE;
    }
    virtual UBool hasBoundaryAfter(UChar32) const U_OVERRIDE
    {
        return TRUE;
    }
    virtual UBool isInert(UChar32) const U_OVERRIDE
    {
        return TRUE;
    }
};
...
Norm2AllModes* Norm2AllModes::createNFCInstance(UErrorCode& errorCode)
{
    ...
    impl->init(norm2_nfc_data_indexes, &norm2_nfc_data_trie, norm2_nfc_data_extraData, norm2_nfc_data_smallFCD);
...
static int32_t normalizeSecondAndAppend(const UNormalizer2* norm2, UChar* first, int32_t firstLength, int32_t firstCapacity, const UChar* second,
    int32_t secondLength, UBool doNormalize, UErrorCode* pErrorCode)
{
    if (U_FAILURE(*pErrorCode)) {
        return 0;
    }
    if ((second == NULL ? secondLength != 0 : secondLength < -1)
        || (first == NULL ? (firstCapacity != 0 || firstLength != 0) : (firstCapacity < 0 || firstLength < -1)) || (first == second && first != NULL)) {
        *pErrorCode = U_ILLEGAL_ARGUMENT_ERROR;
        return 0;
    }
        ...
        if (n2wi != NULL) {
            ...
            UnicodeString safeMiddle;
            {
                ...
                if (buffer.init(firstLength + secondLength + 1, *pErrorCode)) { // destCapacity>=-1
                    n2wi->normalizeAndAppend(second, secondLength >= 0 ? second + secondLength : NULL, doNormalize, safeMiddle, buffer, *pErrorCode);
                }
