...
inline int32_t appendResult(
    UChar* dest, int32_t destIndex, int32_t destCapacity, int32_t result, const UChar* s, int32_t cpLength, uint32_t options, icu::Edits* edits)
{
    UChar32 c;
    int32_t length;
...
UChar32 U_CALLCONV utf16_caseContextIterator(void* context, int8_t dir)
{
 ...
 */
int32_t toLower(int32_t caseLocale, uint32_t options, UChar* dest, int32_t destCapacity, const UChar* src, UCaseContext* csc, int32_t srcStart,
    int32_t srcLimit, icu::Edits* edits, UErrorCode& errorCode)
{
    const int8_t* latinToLower;
    if (caseLocale == UCASE_LOC_ROOT
        || (caseLocale >= 0 ? !(caseLocale == UCASE_LOC_TURKISH || caseLocale == UCASE_LOC_LITHUANIAN)
    ...
    for (;;) {
        ...
        while (srcIndex < srcLimit) {
            ...
            destIndex = appendUnchanged(dest, destIndex, destCapacity, src + prev, srcIndex - 1 - prev, options, edits);
            if (destIndex >= 0) {
        ...
        if (c >= 0) {
            destIndex = appendUnchanged(dest, destIndex, destCapacity, src + prev, cpStart - prev, options, edits);
            if (destIndex >= 0) {
                destIndex = appendResult(dest, destIndex, destCapacity, c, s, srcIndex - cpStart, options, edits);
            }
            if (destIndex < 0) {
                errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                return 0;
    ...
    destIndex = appendUnchanged(dest, destIndex, destCapacity, src + prev, srcIndex - prev, options, edits);
    if (destIndex < 0) {
        errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
        return 0;
    }
...
int32_t toUpper(int32_t caseLocale, uint32_t options, UChar* dest, int32_t destCapacity, const UChar* src, UCaseContext* csc, int32_t srcLength,
 ...
 */
int32_t maybeTitleDutchIJ(const UChar* src, UChar32 c, int32_t start, int32_t segmentLimit, UChar* dest, int32_t& destIndex, int32_t destCapacity,
...
U_CFUNC int32_t U_CALLCONV ustrcase_internalToTitle(int32_t caseLocale, uint32_t options, BreakIterator* iter, UChar* dest, int32_t destCapacity,
    const UChar* src, int32_t srcLength, icu::Edits* edits, UErrorCode& errorCode)
{
    ...
    while (prev < srcLength) {
        ...
        if (prev < index) {
            ...
            if ((options & U_TITLECASE_NO_BREAK_ADJUSTMENT) == 0) {
                ...
                if (prev < titleStart) {
                    destIndex = appendUnchanged(dest, destIndex, destCapacity, src + prev, titleStart - prev, options, edits);
                    if (destIndex < 0) {
                        errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                        return 0;
                    }
            ...
            if (titleStart < titleLimit) {
                ...
                destIndex = appendResult(dest, destIndex, destCapacity, c, s, titleLimit - titleStart, options, edits);
                if (destIndex < 0) {
                    errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                    return 0;
                }
                ...
                if (titleStart + 1 < index && caseLocale == UCASE_LOC_DUTCH) {
                    if (c < 0) {
                        c = ~c;
                    }

                    if (c == u'I' || c == u'Í') {
                        titleLimit = maybeTitleDutchIJ(src, c, titleStart + 1, index, dest, destIndex, destCapacity, options, edits);
                    }
                }
                ...
                if (titleLimit < index) {
                    if ((options & U_TITLECASE_NO_LOWERCASE) == 0) {
                        ...
                        destIndex += toLower(caseLocale, options, dest + destIndex, destCapacity - destIndex, src, &csc, titleLimit, index, edits, errorCode);
                        ...
                        destIndex = appendUnchanged(dest, destIndex, destCapacity, src + titleLimit, index - titleLimit, options, edits);
                        if (destIndex < 0) {
                            errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                            return 0;
                        }
 ...
 */
int32_t toUpper(uint32_t options, UChar* dest, int32_t destCapacity, const UChar* src, int32_t srcLength, Edits* edits, UErrorCode& errorCode)
{
    int32_t destIndex = 0;
    uint32_t state = 0;
    for (int32_t i = 0; i < srcLength;) {
        ...
        if (data > 0) {
            ...
            if ((data & HAS_VOWEL) != 0 && (state & AFTER_VOWEL_WITH_ACCENT) != 0 && (upper == 0x399 || upper == 0x3A5)) {
                data |= HAS_DIALYTIKA;
            }
            ...
            if (upper == 0x397 && (data & HAS_ACCENT) != 0 && numYpogegrammeni == 0 && (state & AFTER_CASED) == 0
            ...
            destIndex = appendResult(dest, destIndex, destCapacity, c, s, nextIndex - i, options, edits);
            if (destIndex < 0) {
                errorCode = U_INDEX_OUTOFBOUNDS_ERROR;
                return 0;
            }
...
U_CFUNC int32_t U_CALLCONV ustrcase_internalToLower(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED UChar* dest, int32_t destCapacity,
    const UChar* src, int32_t srcLength, icu::Edits* edits, UErrorCode& errorCode)
{
    UCaseContext csc = UCASECONTEXT_INITIALIZER;
    csc.p = (void*)src;
    csc.limit = srcLength;
    int32_t destIndex = toLower(caseLocale, options, dest, destCapacity, src, &csc, 0, srcLength, edits, errorCode);
    return checkOverflowAndEditsError(destIndex, destCapacity, edits, errorCode);
}

U_CFUNC int32_t U_CALLCONV ustrcase_internalToUpper(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED UChar* dest, int32_t destCapacity,
    const UChar* src, int32_t srcLength, icu::Edits* edits, UErrorCode& errorCode)
{
    int32_t destIndex;
    if (caseLocale == UCASE_LOC_GREEK) {
        destIndex = GreekUpper::toUpper(options, dest, destCapacity, src, srcLength, edits, errorCode);
    } else {
        UCaseContext csc = UCASECONTEXT_INITIALIZER;
        csc.p = (void*)src;
        csc.limit = srcLength;
        destIndex = toUpper(caseLocale, options, dest, destCapacity, src, &csc, srcLength, edits, errorCode);
    }
    return checkOverflowAndEditsError(destIndex, destCapacity, edits, errorCode);
}

U_CFUNC int32_t U_CALLCONV ustrcase_internalFold(int32_t /* caseLocale */, uint32_t options, UCASEMAP_BREAK_ITERATOR_UNUSED UChar* dest, int32_t destCapacity,
    const UChar* src, int32_t srcLength, icu::Edits* edits, UErrorCode& errorCode)
{
    int32_t destIndex = toLower(-1, options, dest, destCapacity, src, nullptr, 0, srcLength, edits, errorCode);
    return checkOverflowAndEditsError(destIndex, destCapacity, edits, errorCode);
}

U_CFUNC int32_t ustrcase_map(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_PARAM UChar* dest, int32_t destCapacity, const UChar* src,
    int32_t srcLength, UStringCaseMapper* stringCaseMapper, icu::Edits* edits, UErrorCode& errorCode)
{
    int32_t destLength;
    ...
    if (dest != NULL && ((src >= dest && src < (dest + destCapacity)) || (dest >= src && dest < (src + srcLength)))) {
        errorCode = U_ILLEGAL_ARGUMENT_ERROR;
        return 0;
    }
    ...
    destLength = stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR dest, destCapacity, src, srcLength, edits, errorCode);
    return u_terminateUChars(dest, destCapacity, destLength, &errorCode);
}

U_CFUNC int32_t ustrcase_mapWithOverlap(int32_t caseLocale, uint32_t options, UCASEMAP_BREAK_ITERATOR_PARAM UChar* dest, int32_t destCapacity, const UChar* src,
    int32_t srcLength, UStringCaseMapper* stringCaseMapper, UErrorCode& errorCode)
{
    UChar buffer[300];
    UChar* temp;
    ...
    destLength = stringCaseMapper(caseLocale, options, UCASEMAP_BREAK_ITERATOR temp, destCapacity, src, srcLength, NULL, errorCode);
    if (temp != dest) {
...
U_CAPI int32_t U_EXPORT2 u_strFoldCase(UChar* dest, int32_t destCapacity, const UChar* src, int32_t srcLength, uint32_t options, UErrorCode* pErrorCode)
{
    return ustrcase_mapWithOverlap(
        UCASE_LOC_ROOT, options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity, src, srcLength, ustrcase_internalFold, *pErrorCode);
}

U_NAMESPACE_BEGIN

int32_t CaseMap::fold(uint32_t options, const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, Edits* edits, UErrorCode& errorCode)
{
    return ustrcase_map(UCASE_LOC_ROOT, options, UCASEMAP_BREAK_ITERATOR_NULL dest, destCapacity, src, srcLength, ustrcase_internalFold, edits, errorCode);
}
...
static int32_t _cmpFold(
    const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, uint32_t options, int32_t* matchLen1, int32_t* matchLen2, UErrorCode* pErrorCode)
{
    int32_t cmpRes = 0;
    ...
    for (;;) {
        ...
        if (level1 == 0 && (length = ucase_toFullFolding((UChar32)cp1, &p, options)) >= 0) {
        ...
        if (level2 == 0 && (length = ucase_toFullFolding((UChar32)cp2, &p, options)) >= 0) {
...
/* internal function */
U_CFUNC int32_t u_strcmpFold(const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, uint32_t options, UErrorCode* pErrorCode)
{
...
U_CAPI int32_t U_EXPORT2 u_strCaseCompare(const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, uint32_t options, UErrorCode* pErrorCode)
{
    ...
    return u_strcmpFold(s1, length1, s2, length2, options | U_COMPARE_IGNORE_CASE, pErrorCode);
}

U_CAPI int32_t U_EXPORT2 u_strcasecmp(const UChar* s1, const UChar* s2, uint32_t options)
{
    UErrorCode errorCode = U_ZERO_ERROR;
    return u_strcmpFold(s1, -1, s2, -1, options | U_COMPARE_IGNORE_CASE, &errorCode);
}
...
{
    UErrorCode errorCode = U_ZERO_ERROR;
    return u_strcmpFold(s1, n, s2, n, options | (U_COMPARE_IGNORE_CASE | _STRNCMP_STYLE), &errorCode);
}
...
U_CAPI void u_caseInsensitivePrefixMatch(
    const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, uint32_t options, int32_t* matchLen1, int32_t* matchLen2, UErrorCode* pErrorCode)
{
    _cmpFold(s1, length1, s2, length2, options, matchLen1, matchLen2, pErrorCode);
}
