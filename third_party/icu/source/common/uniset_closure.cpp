...
static bool scfString(const UnicodeString& s, UnicodeString& scf) 
{
    ...
    if (attribute & (USET_CASE_INSENSITIVE | USET_ADD_CASE_MAPPINGS)) {
        {
            ...
            UnicodeString str;
            USetAdder sa = {
                foldSet.toUSet(), _set_add, _set_addRange, _set_addString,
            ...
            for (int32_t i = 0; i < n; ++i) {
                ...
                } else if (attribute & USET_SIMPLE_CASE_INSENSITIVE) {
                    for (UChar32 cp = start; cp <= end; ++cp) {
