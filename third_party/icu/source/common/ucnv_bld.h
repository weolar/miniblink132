...
struct UConverterSharedData {
    ...
    UBool
        sharedDataCached; /* true:  shared data is in cache, don't destroy on ucnv_close() if 0 ref.  false: shared data isn't in the cache, do attempt to clean it up if the ref
                               is 0 */
...
struct UConverter {
     ...
     */
    void(U_EXPORT2* fromUCharErrorBehaviour)(const void* context, UConverterFromUnicodeArgs* args, const UChar* codeUnits, int32_t length, UChar32 codePoint,
        UConverterCallbackReason reason, UErrorCode*);
    /*
     * Error function pointer called when conversion issues
     * occur during a ucnv_toUnicode call
     */
    void(U_EXPORT2* fromCharErrorBehaviour)(
        const void* context, UConverterToUnicodeArgs* args, const char* codeUnits, int32_t length, UConverterCallbackReason reason, UErrorCode*);
    ...
    UBool
        sharedDataIsCached; /* true:  shared data is in cache, don't destroy on ucnv_close() if 0 ref.  false: shared data isn't in the cache, do attempt to clean it up if the
                                 ref is 0 */
