...
static inline uint8_t makeSpanLengthByte(int32_t spanLength)
{
    ...
    , all((UBool)(which == ALL))
{
    ...
    for (i = 0; i < stringsLength; ++i) {
        ...
        if (spanLength < length16 && length16 > 0) { // Relevant string.
            ...
            if (all) {
                spanLengths[i] = spanBackLengths[i] = spanUTF8Lengths[i] = spanBackUTF8Lengths[i] = (uint8_t)ALL_CP_CONTAINED;
            } else {
...
UnicodeSetStringSpan::UnicodeSetStringSpan(const UnicodeSetStringSpan& otherStringSpan, const UVector& newParentSetStrings)
...
static inline UBool matches16(const UChar* s, const UChar* t, int32_t length)
{
    do {
        if (*s++ != *t++) {
            return FALSE;
...
static inline UBool matches8(const uint8_t* s, const uint8_t* t, int32_t length)
{
    do {
        if (*s++ != *t++) {
            return FALSE;
...
static inline UBool matches16CPB(const UChar* s, int32_t start, int32_t limit, const UChar* t, int32_t length)
{
    s += start;
    limit -= start;
    return matches16(s, t, length) && !(0 < start && U16_IS_LEAD(s[-1]) && U16_IS_TRAIL(s[0]))
        && !(length < limit && U16_IS_LEAD(s[length - 1]) && U16_IS_TRAIL(s[length]));
}

// Does the set contain the next code point?
// If so, return its length; otherwise return its negative length.
static inline int32_t spanOne(const UnicodeSet& set, const UChar* s, int32_t length)
{
    UChar c = *s, c2;
...
static inline int32_t spanOneBack(const UnicodeSet& set, const UChar* s, int32_t length)
{
...
static inline int32_t spanOneUTF8(const UnicodeSet& set, const uint8_t* s, int32_t length)
{
    UChar32 c = *s;
    if (U8_IS_SINGLE(c)) {
        return set.contains(c) ? 1 : -1;
...
static inline int32_t spanOneBackUTF8(const UnicodeSet& set, const uint8_t* s, int32_t length)
{
        ...
        if (spanCondition == USET_SPAN_CONTAINED) {
            ...
            for (i = 0; i < stringsLength; ++i) {
                ...
                for (;;) {
                    ...
                    if ((overlap > maxOverlap || /* redundant overlap==maxOverlap && */ dec > maxDec) && matches16CPB(s, pos - dec, length, s16, length16)) {
        ...
        if (spanCondition == USET_SPAN_CONTAINED) {
            for (i = 0; i < stringsLength; ++i) {
                ...
                for (;;) {
                    ...
                    if (!U8_IS_TRAIL(s[pos - overlap]) && !offsets.containsOffset(inc) && matches8(s + pos - overlap, s8, length8)) {
                        if (inc == rest) {
                            return length; // Reached the end of the string.
                        }
                        offsets.addOffset(inc);
        ...
        if (spanCondition == USET_SPAN_CONTAINED) {
            for (i = 0; i < stringsLength; ++i) {
                ...
                for (;;) {
                    ...
                    // Match at code point boundaries. (The UTF-8 strings were converted
                    // from UTF-16 and are guaranteed to be well-formed.)
                    if (!U8_IS_TRAIL(s[pos - dec]) && !offsets.containsOffset(dec) && matches8(s + pos - dec, s8, length8)) {
                        if (dec == pos) {
                            return 0; // Reached the start of the string.
                        }
                        offsets.addOffset(dec);
            ...
            for (i = 0; i < stringsLength; ++i) {
                ...
                for (;;) {
                    ...
                    // Match at code point boundaries. (The UTF-8 strings were converted
                    // from UTF-16 and are guaranteed to be well-formed.)
                    if (!U8_IS_TRAIL(s[pos - dec]) && (overlap > maxOverlap || /* redundant overlap==maxOverlap && */ dec > maxDec)
