    ...
    UCPTrieType type, UCPTrieValueWidth valueWidth, const void* data, int32_t length, int32_t* pActualLength, UErrorCode* pErrorCode)
{
    ...
    if (length <= 0 || (U_POINTER_MASK_LSB(data, 3) != 0) || type < UCPTRIE_TYPE_ANY || UCPTRIE_TYPE_SMALL < type || valueWidth < UCPTRIE_VALUE_BITS_ANY
...
inline uint32_t maybeFilterValue(uint32_t value, uint32_t trieNullValue, uint32_t nullValue, UCPMapValueFilter* filter, const void* context)
{
...
UChar32 getRange(const void* t, UChar32 start, UCPMapValueFilter* filter, const void* context, uint32_t* pValue)
{
    ...
    do {
        ...
        if (c <= 0xffff && (trie->type == UCPTRIE_TYPE_FAST || c <= UCPTRIE_SMALL_MAX)) {
            ...
            i3BlockLength = trie->type == UCPTRIE_TYPE_FAST ? UCPTRIE_BMP_INDEX_LENGTH : UCPTRIE_SMALL_INDEX_LENGTH;
            ...
            i3Block = trie->index[(int32_t)trie->index[i1] + ((c >> UCPTRIE_SHIFT_2) & UCPTRIE_INDEX_2_MASK)];
                ...
                if (block == trie->dataNullOffset) {
                    ...
                    if (haveValue) {
                        if (trieValue2 != trieValue) {
                            if (filter == nullptr || maybeFilterValue(trieValue2, trie->nullValue, nullValue, filter, context) != value) {
                                return c - 1;
                            }
                        ...
                        value = maybeFilterValue(trieValue2, trie->nullValue, nullValue, filter, context);
                        if (pValue != nullptr) {
                            *pValue = value;
                        }
                        haveValue = true;
    ...
    if (maybeFilterValue(highValue, trie->nullValue, nullValue, filter, context) != value) {
        return c - 1;
    } else {
        return MAX_UNICODE;
    }
...
U_CAPI int32_t U_EXPORT2 ucptrie_toBinary(const UCPTrie* trie, void* data, int32_t capacity, UErrorCode* pErrorCode)
{
    ...
    UCPTrieValueWidth valueWidth = (UCPTrieValueWidth)trie->valueWidth;
    if (type < UCPTRIE_TYPE_FAST || UCPTRIE_TYPE_SMALL < type || valueWidth < UCPTRIE_VALUE_BITS_16 || UCPTRIE_VALUE_BITS_8 < valueWidth || capacity < 0
    ...
    header->options = (uint16_t)(((trie->dataLength & 0xf0000) >> 4) | ((trie->dataNullOffset & 0xf0000) >> 8) | (trie->type << 6) | valueWidth);
...
U_CFUNC void ucptrie_printLengths(const UCPTrie* trie, const char* which)
{
    ...
    long totalLength = (long)sizeof(UCPTrieHeader) + indexLength * 2
        + dataLength
            * (trie->valueWidth == UCPTRIE_VALUE_BITS_16        ? 2
                    : trie->valueWidth == UCPTRIE_VALUE_BITS_32 ? 4
                                                                : 1);
    printf("**UCPTrieLengths(%s %s)** index:%6ld  data:%6ld  countNull:%6ld  serialized:%6ld\n", which, trie->name, indexLength, dataLength, countNull(trie),
        totalLength);
}
#endif
...
{
    return ucptrie_getRange(reinterpret_cast<const UCPTrie*>(map), start, option, surrogateValue, filter, context, pValue);
}
