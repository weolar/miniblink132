...
static int32_t allocIndex2Block(UNewTrie2* trie);
...
U_CAPI UTrie2* U_EXPORT2 utrie2_open(uint32_t initialValue, uint32_t errorValue, UErrorCode* pErrorCode)
{
    ...
    newTrie->map[i++] = (0x110000 >> UTRIE2_SHIFT_2) - (0x80 >> UTRIE2_SHIFT_2) + 1 + UTRIE2_LSCP_INDEX_2_LENGTH;
...
static UNewTrie2* cloneBuilder(const UNewTrie2* other)
{
    UNewTrie2* trie;
...
static UBool U_CALLCONV copyEnumRange(const void* context, UChar32 start, UChar32 end, uint32_t value)
{
...
static void utrie_printLengths(const UTrie* trie)
{
...
U_CAPI UTrie2* U_EXPORT2 utrie2_fromUTrie(const UTrie* trie1, uint32_t errorValue, UErrorCode* pErrorCode)
{
    ...
    *pErrorCode = context.errorCode;
    ...
    if (U_SUCCESS(*pErrorCode)) {
        utrie2_freeze(context.trie, trie1->data32 != NULL ? UTRIE2_32_VALUE_BITS : UTRIE2_16_VALUE_BITS, pErrorCode);
    }
#ifdef UTRIE2_DEBUG
    if (U_SUCCESS(*pErrorCode)) {
...
static inline UBool isInNullBlock(UNewTrie2* trie, UChar32 c, UBool forLSCP)
{
    int32_t i2, block;
    ...
    if (U_IS_LEAD(c) && forLSCP) {
        ...
        i2 = trie->index1[c >> UTRIE2_SHIFT_1] + ((c >> UTRIE2_SHIFT_2) & UTRIE2_INDEX_2_MASK);
    }
...
static int32_t getIndex2Block(UNewTrie2* trie, UChar32 c, UBool forLSCP)
{
    int32_t i1, i2;
...
static int32_t allocDataBlock(UNewTrie2* trie, int32_t copyBlock)
{
    int32_t newBlock, newTop;
...
static void releaseDataBlock(UNewTrie2* trie, int32_t block)
{
...
static inline UBool isWritableBlock(UNewTrie2* trie, int32_t block)
{
    return (UBool)(block != trie->dataNullOffset && 1 == trie->map[block >> UTRIE2_SHIFT_2]);
}

static inline void setIndex2Entry(UNewTrie2* trie, int32_t i2, int32_t block)
{
    int32_t oldBlock;
 ...
 * @internal
 */
static int32_t getDataBlock(UNewTrie2* trie, UChar32 c, UBool forLSCP)
{
 ...
 */
static void set32(UNewTrie2* trie, UChar32 c, UBool forLSCP, uint32_t value, UErrorCode* pErrorCode)
{
    int32_t block;
...
U_CAPI void U_EXPORT2 utrie2_set32ForLeadSurrogateCodeUnit(UTrie2* trie, UChar32 c, uint32_t value, UErrorCode* pErrorCode)
{
    if (U_FAILURE(*pErrorCode)) {
        return;
    }
...
static void writeBlock(uint32_t* block, uint32_t value)
{
 ...
 * @internal
 */
static void fillBlock(uint32_t* block, UChar32 start, UChar32 limit, uint32_t value, uint32_t initialValue, UBool overwrite)
{
    uint32_t* pLimit;
...
U_CAPI void U_EXPORT2 utrie2_setRange32(UTrie2* trie, UChar32 start, UChar32 end, uint32_t value, UBool overwrite, UErrorCode* pErrorCode)
{
    ...
    if (start & UTRIE2_DATA_MASK) {
        ...
        if (nextStart <= limit) {
            fillBlock(newTrie->data + block, start & UTRIE2_DATA_MASK, UTRIE2_DATA_BLOCK_LENGTH, value, newTrie->initialValue, overwrite);
            start = nextStart;
        } else {
            fillBlock(newTrie->data + block, start & UTRIE2_DATA_MASK, limit & UTRIE2_DATA_MASK, value, newTrie->initialValue, overwrite);
            return;
        }
    }
    ...
    while (start < limit) {
        ...
        if (isWritableBlock(newTrie, block)) {
            ...
            if (overwrite && block >= UNEWTRIE2_DATA_0800_OFFSET) {
                ...
                fillBlock(newTrie->data + block, 0, UTRIE2_DATA_BLOCK_LENGTH, value, newTrie->initialValue, overwrite);
            }
...
static inline UBool equal_int32(const int32_t* s, const int32_t* t, int32_t length)
{
    while (length > 0 && *s == *t) {
        ++s;
        ++t;
...
static inline UBool equal_uint32(const uint32_t* s, const uint32_t* t, int32_t length)
{
    while (length > 0 && *s == *t) {
        ++s;
        ++t;
...
static int32_t findSameIndex2Block(const int32_t* idx, int32_t index2Length, int32_t otherBlock)
{
    int32_t block;
...
static int32_t findSameDataBlock(const uint32_t* data, int32_t dataLength, int32_t otherBlock, int32_t blockLength)
{
    int32_t block;
 ...
 */
static UChar32 findHighStart(UNewTrie2* trie, uint32_t highValue)
{
 ...
 */
static void compactData(UNewTrie2* trie)
{
#ifdef UTRIE2_DEBUG
    ...
    for (start = newStart; start < trie->dataLength;) {
        ...
        if ((movedStart = findSameDataBlock(trie->data, newStart, start, blockLength)) >= 0) {
#ifdef UTRIE2_DEBUG
            ++countSame;
#endif
        ...
        for (overlap = blockLength - UTRIE2_DATA_GRANULARITY; overlap > 0 && !equal_uint32(trie->data + (newStart - overlap), trie->data + start, overlap);
...
static void compactIndex2(UNewTrie2* trie)
{
    ...
    for (start = UNEWTRIE2_INDEX_2_NULL_OFFSET; start < trie->index2Length;) {
        ...
        /* search for an identical block */
        if ((movedStart = findSameIndex2Block(trie->index2, newStart, start)) >= 0) {
            /* found an identical block, set the other block's index value for the current block */
        ...
        for (overlap = UTRIE2_INDEX_2_BLOCK_LENGTH - 1; overlap > 0 && !equal_int32(trie->index2 + (newStart - overlap), trie->index2 + start, overlap);
             --overlap) {
        }
...
static void compactTrie(UTrie2* trie, UErrorCode* pErrorCode)
{
    UNewTrie2* newTrie;
...
U_CAPI void U_EXPORT2 utrie2_freeze(UTrie2* trie, UTrie2ValueBits valueBits, UErrorCode* pErrorCode)
{
    ...
    if (trie == NULL || valueBits < 0 || UTRIE2_COUNT_VALUE_BITS <= valueBits) {
        *pErrorCode = U_ILLEGAL_ARGUMENT_ERROR;
        return;
    }
    newTrie = trie->newTrie;
    if (newTrie == NULL) {
        /* already frozen */
        UTrie2ValueBits frozenValueBits = trie->data16 != NULL ? UTRIE2_16_VALUE_BITS : UTRIE2_32_VALUE_BITS;
    ...
    ucptrie_close(umutablecptrie_buildImmutable(newTrie->t3, UCPTRIE_TYPE_FAST, (UCPTrieValueWidth)valueBits, pErrorCode));
#endif
