...
LocaleDistanceData::LocaleDistanceData(LocaleDistanceData&& data)
    ...
    , paradigmsLength(data.paradigmsLength)
...
struct XLikelySubtagsData {
    ...
    XLikelySubtagsData(UErrorCode& errorCode)
        : strings(errorCode)
    {
    }
    ...
    void load(UErrorCode& errorCode)
    {
        ...
        ures_getValueWithFallback(langInfoBundle, "likely", stackTempBundle.getAlias(), value, errorCode);
        ...
        if (!readStrings(likelyTable, "languageAliases", value, languageIndexes, languagesLength, errorCode)
            || !readStrings(likelyTable, "regionAliases", value, regionIndexes, regionsLength, errorCode)
            || !readStrings(likelyTable, "lsrs", value, lsrSubtagIndexes, lsrSubtagsLength, errorCode)) {
            return;
        }
        if ((languagesLength & 1) != 0 || (regionsLength & 1) != 0 || (lsrSubtagsLength % 3) != 0) {
        ...
        ures_getValueWithFallback(langInfoBundle, "match", stackTempBundle.getAlias(), value, matchErrorCode);
        ...
        if (U_SUCCESS(matchErrorCode)) {
            ...
            if (!readStrings(matchTable, "partitions", value, partitionIndexes, partitionsLength, errorCode)
                || !readStrings(matchTable, "paradigms", value, paradigmSubtagIndexes, paradigmSubtagsLength, errorCode)) {
                return;
            }
        ...
        for (int32_t i = 0; i < languagesLength; i += 2) {
            languageAliases.put(strings.get(languageIndexes[i]), strings.get(languageIndexes[i + 1]), errorCode);
        }

        regionAliases = CharStringMap(regionsLength / 2, errorCode);
        for (int32_t i = 0; i < regionsLength; i += 2) {
            regionAliases.put(strings.get(regionIndexes[i]), strings.get(regionIndexes[i + 1]), errorCode);
        }
        if (U_FAILURE(errorCode)) {
            return;
        }
        ...
        for (int32_t i = 0, j = 0; i < lsrSubtagsLength; i += 3, ++j) {
            lsrs[j] = LSR(strings.get(lsrSubtagIndexes[i]), strings.get(lsrSubtagIndexes[i + 1]), strings.get(lsrSubtagIndexes[i + 2]), LSR::IMPLICIT_LSR);
        }

        if (partitionsLength > 0) {
            distanceData.partitions = static_cast<const char**>(uprv_malloc(partitionsLength * sizeof(const char*)));
        ...
        if (paradigmSubtagsLength > 0) {
            ...
            for (int32_t i = 0, j = 0; i < paradigmSubtagsLength; i += 3, ++j) {
                paradigms[j] = LSR(strings.get(paradigmSubtagIndexes[i]), strings.get(paradigmSubtagIndexes[i + 1]), strings.get(paradigmSubtagIndexes[i + 2]),
                    LSR::DONT_CARE_FLAGS);
            }
...
private:
    bool readStrings(const ResourceTable& table, const char* key, ResourceValue& value, LocalMemory<int32_t>& indexes, int32_t& length, UErrorCode& errorCode)
    {
...
LSR XLikelySubtags::makeMaximizedLsrFrom(const Locale& locale, UErrorCode& errorCode) const
{
    ...
    return makeMaximizedLsr(locale.getLanguage(), locale.getScript(), locale.getCountry(), locale.getVariant(), errorCode);
}
...
LSR XLikelySubtags::makeMaximizedLsr(const char* language, const char* script, const char* region, const char* variant, UErrorCode& errorCode) const
{
    ...
    if (region[0] == 'X' && (c1 = region[1]) != 0 && region[2] == 0) {
        switch (c1) {
        case 'A':
            return LSR(PSEUDO_ACCENTS_PREFIX, language, script, region, LSR::EXPLICIT_LSR, errorCode);
        case 'B':
            return LSR(PSEUDO_BIDI_PREFIX, language, script, region, LSR::EXPLICIT_LSR, errorCode);
        case 'C':
            return LSR(PSEUDO_CRACKED_PREFIX, language, script, region, LSR::EXPLICIT_LSR, errorCode);
    ...
    if (variant[0] == 'P' && variant[1] == 'S') {
        int32_t lsrFlags = *region == 0 ? LSR::EXPLICIT_LANGUAGE | LSR::EXPLICIT_SCRIPT : LSR::EXPLICIT_LSR;
...
LSR XLikelySubtags::maximize(const char* language, const char* script, const char* region) const
{
    ...
    int32_t c0;
    if (0 <= (c0 = uprv_lowerOrdinal(language[0])) && c0 <= 25 && language[1] != 0 && // language.length() >= 2
