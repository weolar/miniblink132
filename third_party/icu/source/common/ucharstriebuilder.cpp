...
UCharsTrieBuilder& UCharsTrieBuilder::add(const UnicodeString& s, int32_t value, UErrorCode& errorCode)
{
    if (U_FAILURE(errorCode)) {
        return *this;
    }
...
UCharsTrie* UCharsTrieBuilder::build(UStringTrieBuildOption buildOption, UErrorCode& errorCode)
{
...
void UCharsTrieBuilder::buildUChars(UStringTrieBuildOption buildOption, UErrorCode& errorCode)
{
    ...
    if (ucharsLength == 0) {
        ...
        uprv_sortArray(elements, elementsLength, (int32_t)sizeof(UCharsTrieElement), compareElementStrings, &strings,
...
int32_t UCharsTrieBuilder::getElementStringLength(int32_t i) const
{
    return elements[i].getStringLength(strings);
}

UChar UCharsTrieBuilder::getElementUnit(int32_t i, int32_t unitIndex) const
{
    return elements[i].charAt(unitIndex, strings);
}

int32_t UCharsTrieBuilder::getElementValue(int32_t i) const
{
    return elements[i].getValue();
}

int32_t UCharsTrieBuilder::getLimitOfLinearMatch(int32_t first, int32_t last, int32_t unitIndex) const
{
    const UCharsTrieElement& firstElement = elements[first];
    const UCharsTrieElement& lastElement = elements[last];
    int32_t minStringLength = firstElement.getStringLength(strings);
    while (++unitIndex < minStringLength && firstElement.charAt(unitIndex, strings) == lastElement.charAt(unitIndex, strings)) {
    }
    return unitIndex;
}

int32_t UCharsTrieBuilder::countElementUnits(int32_t start, int32_t limit, int32_t unitIndex) const
{
...
int32_t UCharsTrieBuilder::skipElementsBySomeUnits(int32_t i, int32_t unitIndex, int32_t count) const
{
    do {
        UChar unit = elements[i++].charAt(unitIndex, strings);
...
int32_t UCharsTrieBuilder::indexOfElementWithNextUnit(int32_t i, int32_t unitIndex, UChar unit) const
{
    while (unit == elements[i].charAt(unitIndex, strings)) {
        ++i;
    }
...
void UCharsTrieBuilder::UCTLinearMatchNode::write(StringTrieBuilder& builder)
{
...
StringTrieBuilder::Node* UCharsTrieBuilder::createLinearMatchNode(int32_t i, int32_t unitIndex, int32_t length, Node* nextNode) const
{
    return new UCTLinearMatchNode(elements[i].getString(strings).getBuffer() + unitIndex, length, nextNode);
}

UBool UCharsTrieBuilder::ensureCapacity(int32_t length)
{
    if (uchars == NULL) {
    ...
    if (length > ucharsCapacity) {
        ...
        u_memcpy(newUChars + (newCapacity - ucharsLength), uchars + (ucharsCapacity - ucharsLength), ucharsLength);
        uprv_free(uchars);
        uchars = newUChars;
...
int32_t UCharsTrieBuilder::write(int32_t unit)
{
...
int32_t UCharsTrieBuilder::write(const UChar* s, int32_t length)
{
...
int32_t UCharsTrieBuilder::writeElementUnits(int32_t i, int32_t unitIndex, int32_t length)
{
    return write(elements[i].getString(strings).getBuffer() + unitIndex, length);
}

int32_t UCharsTrieBuilder::writeValueAndFinal(int32_t i, UBool isFinal)
{
...
int32_t UCharsTrieBuilder::writeValueAndType(UBool hasValue, int32_t value, int32_t node)
{
    if (!hasValue) {
        return write(node);
    }
