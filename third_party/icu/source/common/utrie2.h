...
U_CAPI void U_EXPORT2 utrie2_enumForLeadSurrogate(
    const UTrie2* trie, UChar32 lead, UTrie2EnumValue* enumValue, UTrie2EnumRange* enumRange, const void* context);
 ...
 * @internal
 */
U_CAPI int32_t U_EXPORT2 utrie2_internalU8NextIndex(const UTrie2* trie, UChar32 c, const uint8_t* src, const uint8_t* limit);
 ...
 * @internal
 */
U_CAPI int32_t U_EXPORT2 utrie2_internalU8PrevIndex(const UTrie2* trie, UChar32 c, const uint8_t* start, const uint8_t* src);

/** Internal low-level trie getter. Returns a data index. */
#define _UTRIE2_INDEX_RAW(offset, trieIndex, c) (((int32_t)((trieIndex)[(offset) + ((c) >> UTRIE2_SHIFT_2)]) << UTRIE2_INDEX_SHIFT) + ((c)&UTRIE2_DATA_MASK))
...
#define _UTRIE2_INDEX_FROM_BMP(trieIndex, c) _UTRIE2_INDEX_RAW(U_IS_LEAD(c) ? UTRIE2_LSCP_INDEX_2_OFFSET - (0xd800 >> UTRIE2_SHIFT_2) : 0, trieIndex, c)
...
#define _UTRIE2_INDEX_FROM_SUPP(trieIndex, c)                                                                                                                  \
    (((int32_t)((trieIndex)[(trieIndex)[(UTRIE2_INDEX_1_OFFSET - UTRIE2_OMITTED_BMP_INDEX_1_LENGTH) + ((c) >> UTRIE2_SHIFT_1)]                                 \
    ...
    {                                                                                                                                                          \
        ...
        if (U8_IS_SINGLE(__lead)) {                                                                                                                            \
            ...
            if (/* handle U+0800..U+FFFF inline */                                                                                                             \
                0xe0 <= __lead && __lead < 0xf0 && ((src) + 1) < (limit) && U8_IS_VALID_LEAD3_AND_T1(__lead, __t1 = (uint8_t) * (src))                         \
                && (__t2 = (uint8_t)(*((src) + 1) - 0x80)) <= 0x3f) {                                                                                          \
                (src) += 2;                                                                                                                                    \
                (result) = (trie)->data[((int32_t)((trie)->index[((__lead - 0xe0) << (12 - UTRIE2_SHIFT_2)) + ((__t1 & 0x3f) << (6 - UTRIE2_SHIFT_2))          \
                                             + (__t2 >> UTRIE2_SHIFT_2)])                                                                                      \
                                            << UTRIE2_INDEX_SHIFT)                                                                                             \
                    + (__t2 & UTRIE2_DATA_MASK)];                                                                                                              \
            } else if (/* handle U+0080..U+07FF inline */                                                                                                      \
                __lead < 0xe0 && __lead >= 0xc2 && (src) < (limit) && (__t1 = (uint8_t)(*(src)-0x80)) <= 0x3f) {                                               \
                ++(src);                                                                                                                                       \
                (result) = (trie)->data[(trie)->index[(UTRIE2_UTF8_2B_INDEX_2_OFFSET - 0xc0) + __lead] + __t1];                                                \
            } else {                                                                                                                                           \
                int32_t __index = utrie2_internalU8NextIndex((trie), __lead, (const uint8_t*)(src), (const uint8_t*)(limit));                                  \
                (src) += __index & 7;                                                                                                                          \
...
#define _UTRIE2_U8_PREV(trie, ascii, data, start, src, result)                                                                                                 \
    ...
    {                                                                                                                                                          \
        ...
        if (U8_IS_SINGLE(__b)) {                                                                                                                               \
            ...
            int32_t __index = utrie2_internalU8PrevIndex((trie), __b, (const uint8_t*)(start), (const uint8_t*)(src));                                         \
            (src) -= __index & 7;                                                                                                                              \
            (result) = (trie)->data[__index >> 3];                                                                                                             \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
