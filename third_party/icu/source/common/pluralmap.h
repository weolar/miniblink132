...
public:
     ...
     */
    enum Category { NONE = -1, OTHER, ZERO, ONE, TWO, FEW, MANY, CATEGORY_COUNT };
 ...
 */
template <typename T> class PluralMap : public PluralMapBase {
public:
    /**
     ...
     */
    PluralMap(const T& otherVariant)
        : fOtherVariant(otherVariant)
    {
        initializeNew();
    }

    PluralMap(const PluralMap<T>& other)
        : fOtherVariant(other.fOtherVariant)
    {
        fVariants[0] = &fOtherVariant;
        for (int32_t i = 1; i < UPRV_LENGTHOF(fVariants); ++i) {
            fVariants[i] = other.fVariants[i] ? new T(*other.fVariants[i]) : NULL;
        }
    }
     ...
     */
    T* getMutable(Category category, UErrorCode& status)
    {
     ...
     */
    T* getMutable(const char* category, UErrorCode& status)
    {
     ...
     */
    T* getMutableWithDefault(Category category, const T& defaultValue, UErrorCode& status)
    {
     ...
     */
    UBool equals(const PluralMap<T>& rhs, UBool (*eqFunc)(const T&, const T&)) const
    {
        for (int32_t i = 0; i < UPRV_LENGTHOF(fVariants); ++i) {
...
private:
    ...
    T* getMutable(Category category, const T* defaultValue, UErrorCode& status)
    {
        if (U_FAILURE(status)) {
            return NULL;
        }
        ...
        if (fVariants[index] == NULL) {
            fVariants[index] = defaultValue == NULL ? new T() : new T(*defaultValue);
        }
        if (!fVariants[index]) {
