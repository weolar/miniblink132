 ...
 */
#define UCNV_EXT_TO_U_VERIFY_SISO_MATCH(sisoState, match) ((sisoState) < 0 || ((sisoState) == 0) == (match == 1))
 ...
 */
static int32_t ucnv_extMatchToU(const int32_t* cx, int8_t sisoState, const char* pre, int32_t preLength, const char* src, int32_t srcLength,
    uint32_t* pMatchValue, UBool /*useFallback*/, UBool flush)
{
    ...
    for (;;) {
        ...
        if (value != 0 && (UCNV_EXT_TO_U_IS_ROUNDTRIP(value) || TO_U_USE_FALLBACK(useFallback)) && UCNV_EXT_TO_U_VERIFY_SISO_MATCH(sisoState, i + j)) {
        ...
        if (value == 0) {
            ...
            if (UCNV_EXT_TO_U_IS_PARTIAL(value)) {
                ...
                if ((UCNV_EXT_TO_U_IS_ROUNDTRIP(value) || TO_U_USE_FALLBACK(useFallback)) && UCNV_EXT_TO_U_VERIFY_SISO_MATCH(sisoState, i + j)) {
...
static inline void ucnv_extWriteToU(
    UConverter* cnv, const int32_t* cx, uint32_t value, UChar** target, const UChar* targetLimit, int32_t** offsets, int32_t srcIndex, UErrorCode* pErrorCode)
{
    /* output the result */
    if (UCNV_EXT_TO_U_IS_CODE_POINT(value)) {
        /* output a single code point */
        ucnv_toUWriteCodePoint(cnv, UCNV_EXT_TO_U_GET_CODE_POINT(value), target, targetLimit, offsets, srcIndex, pErrorCode);
    } else {
        /* output a string - with correct data we have resultLength>0 */
        ucnv_toUWriteUChars(cnv, UCNV_EXT_ARRAY(cx, UCNV_EXT_TO_U_UCHARS_INDEX, UChar) + UCNV_EXT_TO_U_GET_INDEX(value), UCNV_EXT_TO_U_GET_LENGTH(value),
            target, targetLimit, offsets, srcIndex, pErrorCode);
    }
}
...
#define UCNV_SISO_STATE(cnv)                                                                                                                                   \
    ((cnv)->sharedData->mbcs.outputType == MBCS_OUTPUT_2_SISO ? (int8_t)(cnv)->mode : (cnv)->sharedData->mbcs.outputType == MBCS_OUTPUT_DBCS_ONLY ? 1 : -1)

/*
 * target<targetLimit; set error code for overflow
 */
U_CFUNC UBool ucnv_extInitialMatchToU(UConverter* cnv, const int32_t* cx, int32_t firstLength, const char** src, const char* srcLimit, UChar** target,
    const UChar* targetLimit, int32_t** offsets, int32_t srcIndex, UBool flush, UErrorCode* pErrorCode)
{
    uint32_t value = 0; /* initialize output-only param to 0 to silence gcc */
    int32_t match;

    /* try to match */
    match = ucnv_extMatchToU(
        cx, (int8_t)UCNV_SISO_STATE(cnv), (const char*)cnv->toUBytes, firstLength, *src, (int32_t)(srcLimit - *src), &value, cnv->useFallback, flush);
    if (match > 0) {
        /* advance src pointer for the consumed input */
        *src += match - firstLength;

        /* write result to target */
        ucnv_extWriteToU(cnv, cx, value, target, targetLimit, offsets, srcIndex, pErrorCode);
        return TRUE;
    } else if (match < 0) {
...
U_CFUNC UChar32 ucnv_extSimpleMatchToU(const int32_t* cx, const char* source, int32_t length, UBool useFallback)
{
    uint32_t value = 0; /* initialize output-only param to 0 to silence gcc */
    int32_t match;
    ...
    /* try to match */
    match = ucnv_extMatchToU(cx, -1, source, length, NULL, 0, &value, useFallback, TRUE);
    if (match == length) {
...
U_CFUNC void ucnv_extContinueMatchToU(UConverter* cnv, UConverterToUnicodeArgs* pArgs, int32_t srcIndex, UErrorCode* pErrorCode)
{
    ...
    match = ucnv_extMatchToU(cnv->sharedData->mbcs.extIndexes, (int8_t)UCNV_SISO_STATE(cnv), cnv->preToU, cnv->preToULength, pArgs->source,
        (int32_t)(pArgs->sourceLimit - pArgs->source), &value, cnv->useFallback, pArgs->flush);
    if (match > 0) {
        ...
        /* write result */
        ucnv_extWriteToU(cnv, cnv->sharedData->mbcs.extIndexes, value, &pArgs->target, pArgs->targetLimit, &pArgs->offsets, srcIndex, pErrorCode);
    } else if (match < 0) {
        /* save state for partial match */
        const char* s;
        int32_t j;
...
static inline UBool extFromUUseMapping(UBool useFallback, uint32_t value, UChar32 firstCP)
{
    return ((value & UCNV_EXT_FROM_U_STATUS_MASK) != 0 || FROM_U_USE_FALLBACK(useFallback, firstCP)) && (value & UCNV_EXT_FROM_U_RESERVED_MASK) == 0;
}
 ...
 */
static int32_t ucnv_extMatchFromU(const int32_t* cx, UChar32 firstCP, const UChar* pre, int32_t preLength, const UChar* src, int32_t srcLength,
    uint32_t* pMatchValue, UBool useFallback, UBool flush)
{
 ...
 */
static inline void ucnv_extWriteFromU(
    UConverter* cnv, const int32_t* cx, uint32_t value, char** target, const char* targetLimit, int32_t** offsets, int32_t srcIndex, UErrorCode* pErrorCode)
{
    ...
    ucnv_fromUWriteBytes(cnv, (const char*)result, length, target, targetLimit, offsets, srcIndex, pErrorCode);
}

/*
 * target<targetLimit; set error code for overflow
 */
U_CFUNC UBool ucnv_extInitialMatchFromU(UConverter* cnv, const int32_t* cx, UChar32 cp, const UChar** src, const UChar* srcLimit, char** target,
    const char* targetLimit, int32_t** offsets, int32_t srcIndex, UBool flush, UErrorCode* pErrorCode)
{
    uint32_t value = 0; /* initialize output-only param to 0 to silence gcc */
    int32_t match;

    /* try to match */
    match = ucnv_extMatchFromU(cx, cp, NULL, 0, *src, (int32_t)(srcLimit - *src), &value, cnv->useFallback, flush);

    /* reject a match if the result is a single byte for DBCS-only */
    if (match >= 2 && !(UCNV_EXT_FROM_U_GET_LENGTH(value) == 1 && cnv->sharedData->mbcs.outputType == MBCS_OUTPUT_DBCS_ONLY)) {
        /* advance src pointer for the consumed input */
...
U_CFUNC int32_t ucnv_extSimpleMatchFromU(const int32_t* cx, UChar32 cp, uint32_t* pValue, UBool useFallback)
{
    ...
    /* try to match */
    match = ucnv_extMatchFromU(cx, cp, NULL, 0, NULL, 0, &value, useFallback, TRUE);
    if (match >= 2) {
        /* write result for simple, single-character conversion */
        int32_t length;
        int isRoundtrip;
...
U_CFUNC void ucnv_extContinueMatchFromU(UConverter* cnv, UConverterFromUnicodeArgs* pArgs, int32_t srcIndex, UErrorCode* pErrorCode)
{
    ...
    match = ucnv_extMatchFromU(cnv->sharedData->mbcs.extIndexes, cnv->preFromUFirstCP, cnv->preFromU, cnv->preFromULength, pArgs->source,
        (int32_t)(pArgs->sourceLimit - pArgs->source), &value, cnv->useFallback, pArgs->flush);
    if (match >= 2) {
        ...
        /* write result */
        ucnv_extWriteFromU(cnv, cnv->sharedData->mbcs.extIndexes, value, &pArgs->target, pArgs->targetLimit, &pArgs->offsets, srcIndex, pErrorCode);
    } else if (match < 0) {
        /* save state for partial match */
        const UChar* s;
        int32_t j;
...
static UBool extSetUseMapping(UConverterUnicodeSet which, int32_t minLength, uint32_t value)
{
    if (which == UCNV_ROUNDTRIP_SET) {
        ...
        if (((value & (UCNV_EXT_FROM_U_ROUNDTRIP_FLAG | UCNV_EXT_FROM_U_RESERVED_MASK)) != UCNV_EXT_FROM_U_ROUNDTRIP_FLAG)) {
            return FALSE;
        }
...
static void ucnv_extGetUnicodeSetString(const UConverterSharedData* sharedData, const int32_t* cx, const USetAdder* sa, UConverterUnicodeSet which,
    int32_t minLength, UChar32 firstCP, UChar s[UCNV_EXT_MAX_UCHARS], int32_t length, int32_t sectionIndex, UErrorCode* pErrorCode)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (value == 0) {
            ...
            ucnv_extGetUnicodeSetString(
                sharedData, cx, sa, which, minLength, firstCP, s, length + 1, (int32_t)UCNV_EXT_FROM_U_GET_PARTIAL_INDEX(value), pErrorCode);
...
U_CFUNC void ucnv_extGetUnicodeSet(
    const UConverterSharedData* sharedData, const USetAdder* sa, UConverterUnicodeSet which, UConverterSetFilter filter, UErrorCode* pErrorCode)
{
    ...
    if (filter == UCNV_SET_FILTER_2022_CN) {
        minLength = 3;
    } else if (sharedData->mbcs.outputType == MBCS_OUTPUT_DBCS_ONLY || filter != UCNV_SET_FILTER_NONE) {
                ...
                if ((st3 = (int32_t)ps2[st2] << UCNV_EXT_STAGE_2_LEFT_SHIFT) != 0) {
                    ...
                    do {
                        ...
                        if (value == 0) {
                            ...
                            ucnv_extGetUnicodeSetString(
                                sharedData, cx, sa, which, minLength, c, s, length, (int32_t)UCNV_EXT_FROM_U_GET_PARTIAL_INDEX(value), pErrorCode);
                        } else if (extSetUseMapping(which, minLength, value)) {
                            switch (filter) {
