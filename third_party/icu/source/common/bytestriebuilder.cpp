...
void BytesTrieElement::setTo(StringPiece s, int32_t val, CharString& strings, UErrorCode& errorCode)
{
...
int32_t BytesTrieElement::compareStringTo(const BytesTrieElement& other, const CharString& strings) const
{
...
void BytesTrieBuilder::buildBytes(UStringTrieBuildOption buildOption, UErrorCode& errorCode)
{
    ...
    if (bytesLength == 0) {
        ...
        uprv_sortArray(elements, elementsLength, (int32_t)sizeof(BytesTrieElement), compareElementStrings, strings,
...
int32_t BytesTrieBuilder::getLimitOfLinearMatch(int32_t first, int32_t last, int32_t byteIndex) const
{
    ...
    while (++byteIndex < minStringLength && firstElement.charAt(byteIndex, *strings) == lastElement.charAt(byteIndex, *strings)) {
    }
    return byteIndex;
}
...
UBool BytesTrieBuilder::ensureCapacity(int32_t length)
{
    if (bytes == NULL) {
...
int32_t BytesTrieBuilder::write(const char* b, int32_t length)
{
...
int32_t BytesTrieBuilder::writeElementUnits(int32_t i, int32_t byteIndex, int32_t length)
{
    return write(elements[i].getString(*strings).data() + byteIndex, length);
}

int32_t BytesTrieBuilder::writeValueAndFinal(int32_t i, UBool isFinal)
{
...
int32_t BytesTrieBuilder::writeValueAndType(UBool hasValue, int32_t value, int32_t node)
{
