...
UCharsTrie::Iterator::Iterator(ConstChar16Ptr trieUChars, int32_t maxStringLength, UErrorCode& errorCode)
    : uchars_(trieUChars)
    , pos_(uchars_)
    , initialPos_(uchars_)
    , remainingMatchLength_(-1)
    , initialRemainingMatchLength_(-1)
    , skipValue_(FALSE)
...
UCharsTrie::Iterator::Iterator(const UCharsTrie& trie, int32_t maxStringLength, UErrorCode& errorCode)
    : uchars_(trie.uchars_)
    , pos_(trie.pos_)
    , initialPos_(trie.pos_)
...
UCharsTrie::Iterator& UCharsTrie::Iterator::reset()
{
    pos_ = initialPos_;
...
{
    return pos_ != NULL || !stack_->isEmpty();
}
...
const UChar* UCharsTrie::Iterator::branchNext(const UChar* pos, int32_t length, UErrorCode& errorCode)
{
