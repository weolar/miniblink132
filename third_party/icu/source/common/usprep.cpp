...
static UBool U_CALLCONV isSPrepAcceptable(void* /* context */, const char* /* type */, const char* /* name */, const UDataInfo* pInfo)
{
    if (pInfo->size >= 20 && pInfo->isBigEndian == U_IS_BIG_ENDIAN && pInfo->charsetFamily == U_CHARSET_FAMILY && pInfo->dataFormat[0] == 0x53
        && /* dataFormat="SPRP" */
        pInfo->dataFormat[1] == 0x50 && pInfo->dataFormat[2] == 0x52 && pInfo->dataFormat[3] == 0x50 && pInfo->formatVersion[0] == 3
        && pInfo->formatVersion[2] == UTRIE_SHIFT && pInfo->formatVersion[3] == UTRIE_INDEX_SHIFT) {
...
static int32_t U_CALLCONV getSPrepFoldingOffset(uint32_t data)
{
    return (int32_t)data;
}
...
static int32_t U_CALLCONV hashEntry(const UHashTok parm)
{
    ...
    uint32_t unsignedHash = static_cast<uint32_t>(uhash_hashChars(namekey)) + 37u * static_cast<uint32_t>(uhash_hashChars(pathkey));
...
static int32_t usprep_internal_flushCache(UBool noRefCount)
{
    ...
    while ((e = uhash_nextElement(SHARED_DATA_HASHTABLE, &pos)) != NULL) {
        ...
        if ((noRefCount == FALSE && profile->refCount == 0) || noRefCount == TRUE) {
            deletedNum++;
...
static UBool U_CALLCONV loadData(UStringPrepProfile* profile, const char* path, const char* name, const char* type, UErrorCode* errorCode)
{
    ...
    normUniVer = (normUnicodeVersion[0] << 24) + (normUnicodeVersion[1] << 16) + (normUnicodeVersion[2] << 8) + (normUnicodeVersion[3]);
    sprepUniVer = (dataVersion[0] << 24) + (dataVersion[1] << 16) + (dataVersion[2] << 8) + (dataVersion[3]);
    ...
    if (normUniVer < sprepUniVer && /* the Unicode version of SPREP file must be less than the Unicode Version of the normalization data */
        ...
        && /* the Unicode version of the NormalizationCorrections.txt file should be less than the Unicode Version of the normalization data */
...
U_CAPI UStringPrepProfile* U_EXPORT2 usprep_open(const char* path, const char* name, UErrorCode* status)
{
...
U_CAPI UStringPrepProfile* U_EXPORT2 usprep_openByType(UStringPrepProfileType type, UErrorCode* status)
{
    if (status == NULL || U_FAILURE(*status)) {
        return NULL;
    }
...
U_CFUNC void uprv_syntaxError(const UChar* rules, int32_t pos, int32_t rulesLen, UParseError* parseError)
{
    if (parseError == NULL) {
        return;
    }
...
static inline UStringPrepType getValues(uint16_t trieWord, int16_t& value, UBool& isIndex)
{
...
static int32_t usprep_map(const UStringPrepProfile* profile, const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, int32_t options,
    ...
    for (srcIndex = 0; srcIndex < srcLength;) {
        ...
        if (type == USPREP_UNASSIGNED && allowUnassigned == FALSE) {
            ...
            *status = U_STRINGPREP_UNASSIGNED_ERROR;
            ...
            if (isIndex) {
                index = value;
                if (index >= indexes[_SPREP_ONE_UCHAR_MAPPING_INDEX_START] && index < indexes[_SPREP_TWO_UCHARS_MAPPING_INDEX_START]) {
                    length = 1;
                } else if (index >= indexes[_SPREP_TWO_UCHARS_MAPPING_INDEX_START] && index < indexes[_SPREP_THREE_UCHARS_MAPPING_INDEX_START]) {
                    length = 2;
                } else if (index >= indexes[_SPREP_THREE_UCHARS_MAPPING_INDEX_START] && index < indexes[_SPREP_FOUR_UCHARS_MAPPING_INDEX_START]) {
                    length = 3;
                } else {
...
*/
U_CAPI int32_t U_EXPORT2 usprep_prepare(const UStringPrepProfile* profile, const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity,
    int32_t options, UParseError* parseError, UErrorCode* status)
{
    ...
    if (*status == U_BUFFER_OVERFLOW_ERROR) {
        ...
        *status = U_ZERO_ERROR; // reset error
        b1Len = usprep_map(profile, src, srcLength, b1, s1.getCapacity(), options, parseError, status);
        s1.releaseBuffer(U_SUCCESS(*status) ? b1Len : 0);
    }
    if (U_FAILURE(*status)) {
        return 0;
    ...
    for (int32_t b2Index = 0; b2Index < b2Len;) {
        ...
        if (type == USPREP_PROHIBITED || ((result < _SPREP_TYPE_THRESHOLD) && (result & 0x01) /* first bit says it the code point is prohibited*/)) {
...
U_CAPI int32_t U_EXPORT2 usprep_swap(const UDataSwapper* ds, const void* inData, int32_t length, void* outData, UErrorCode* pErrorCode)
{
    ...
    if (!(pInfo->dataFormat[0] == 0x53 && /* dataFormat="SPRP" */
            pInfo->dataFormat[1] == 0x50 && pInfo->dataFormat[2] == 0x52 && pInfo->dataFormat[3] == 0x50 && pInfo->formatVersion[0] == 3)) {
    ...
    size = 16 * 4 + /* size of indexes[] */
        indexes[_SPREP_INDEX_TRIE_SIZE] + indexes[_SPREP_INDEX_MAPPING_DATA_SIZE];
