...
protected:
    ...
    Offset16To<Device> xDeviceTable; /* Offset to Device table for X
                                         * coordinate-- from beginning of
                                         * Anchor table (may be NULL) */
    Offset16To<Device> yDeviceTable; /* Offset to Device table for Y
                                         * coordinate-- from beginning of
                                         * Anchor table (may be NULL) */
public:
    ...
    bool subset(hb_subset_context_t* c) const
    {
        ...
        if (unlikely(!c->serializer->embed(format)))
            return_trace(false);
        if (unlikely(!c->serializer->embed(xCoordinate)))
            return_trace(false);
        if (unlikely(!c->serializer->embed(yCoordinate)))
            return_trace(false);
        ...
        if (c->plan->layout_variation_idx_delta_map->has(x_varidx)) {
            ...
            if (delta != 0) {
                if (!c->serializer->check_assign(out->xCoordinate, xCoordinate + delta, HB_SERIALIZE_ERROR_INT_OVERFLOW))
                    return_trace(false);
            }
        }
        ...
        if (c->plan->layout_variation_idx_delta_map->has(y_varidx)) {
            ...
            if (delta != 0) {
                if (!c->serializer->check_assign(out->yCoordinate, yCoordinate + delta, HB_SERIALIZE_ERROR_INT_OVERFLOW))
                    return_trace(false);
            }
        }
        ...
        if (!c->serializer->embed(xDeviceTable))
            return_trace(false);
        if (!c->serializer->embed(yDeviceTable))
            return_trace(false);
