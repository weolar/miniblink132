...
struct glyf {
    ...
    template <typename Iterator> bool serialize(hb_serialize_context_t* c, Iterator it, bool use_short_loca, const hb_subset_plan_t* plan)
    {
        TRACE_SERIALIZE(this);
        unsigned init_len = c->length();
        for (const auto& _ : it)
            _.serialize(c, use_short_loca, plan);
    ...
    bool subset(hb_subset_context_t* c) const
    {
        ...
        if (unlikely(!c->serializer->check_success(glyf_prime)))
            return_trace(false);
        ...
        auto padded_offsets = +hb_iter(glyphs) | hb_map(&glyf_impl::SubsetGlyph::padded_size);
        ...
        if (!use_short_loca) {
            padded_offsets = +hb_iter(glyphs) | hb_map(&glyf_impl::SubsetGlyph::length);
        }
        ...
        if (unlikely(c->serializer->in_error()))
            return_trace(false);
        return_trace(c->serializer->check_success(glyf_impl::_add_loca_and_head(c->plan, padded_offsets, use_short_loca)));
    }

    void _populate_subset_glyphs(const hb_subset_plan_t* plan, hb_vector_t<glyf_impl::SubsetGlyph>* glyphs /* OUT */) const;

    void _compile_subset_glyphs_with_deltas(const hb_subset_plan_t* plan, hb_vector_t<glyf_impl::SubsetGlyph>* glyphs /* OUT */) const;
...
protected:
    template <typename T> bool get_points(hb_font_t* font, hb_codepoint_t gid, T consumer) const
    {
        if (gid >= num_glyphs)
            return false;
    ...
    struct points_aggregator_t {
        ...
        void points_end()
        {
            bounds.get_extents(font, extents, scaled);
        }
...
public:
    unsigned get_advance_with_var_unscaled(hb_font_t* font, hb_codepoint_t gid, bool is_vertical) const
    {
        ...
        float result = is_vertical ? phantoms[glyf_impl::PHANTOM_TOP].y - phantoms[glyf_impl::PHANTOM_BOTTOM].y
    ...
    bool get_leading_bearing_with_var_unscaled(hb_font_t* font, hb_codepoint_t gid, bool is_vertical, int* lsb) const
    {
        ...
        *lsb = is_vertical ? roundf(phantoms[glyf_impl::PHANTOM_TOP].y) - extents.y_bearing : roundf(phantoms[glyf_impl::PHANTOM_LEFT].x);
        return true;
    }
#endif
...
public:
    ...
    const glyf_impl::Glyph glyph_for_gid(hb_codepoint_t gid, bool needs_padding_removal = false) const
    {
        if (unlikely(gid >= num_glyphs))
        ...
        glyf_impl::Glyph glyph(hb_bytes_t((const char*)this->glyf_table + start_offset, end_offset - start_offset), gid);
...
inline void glyf::_populate_subset_glyphs(const hb_subset_plan_t* plan, hb_vector_t<glyf_impl::SubsetGlyph>* glyphs /* OUT */) const
{
    OT::glyf_accelerator_t glyf(plan->source);

    +hb_range(plan->num_output_glyphs()) | hb_map([&](hb_codepoint_t new_gid) {
...
inline void glyf::_compile_subset_glyphs_with_deltas(const hb_subset_plan_t* plan, hb_vector_t<glyf_impl::SubsetGlyph>* glyphs /* OUT */) const
{
    OT::glyf_accelerator_t glyf(plan->source);
