...
struct path_builder_t {
    ...
    void consume_point(const contour_point_t& point)
    {
        ...
        if (!first_oncurve) {
            ...
            if (last_offcurve) {
                if (is_on_curve) {
                    draw_session->quadratic_to(last_offcurve.x, last_offcurve.y, p.x, p.y);
                    last_offcurve = optional_point_t();
                } else {
                    optional_point_t mid = last_offcurve.lerp(p, .5f);
                    draw_session->quadratic_to(last_offcurve.x, last_offcurve.y, mid.x, mid.y);
                    last_offcurve = p;
                }
            } else {
                if (is_on_curve)
        ...
        if (point.is_end_point) {
            ...
            if (first_offcurve && first_oncurve)
                draw_session->quadratic_to(first_offcurve.x, first_offcurve.y, first_oncurve.x, first_oncurve.y);
            else if (last_offcurve && first_oncurve)
                draw_session->quadratic_to(last_offcurve.x, last_offcurve.y, first_oncurve.x, first_oncurve.y);
