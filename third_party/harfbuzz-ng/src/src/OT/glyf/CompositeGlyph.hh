...
public:
    unsigned int get_size() const
    {
        ...
        if (flags & GID_IS_24BIT)
            size += HBGlyphID24::static_size - HBGlyphID16::static_size;
#endif
    ...
    unsigned compile_with_deltas(const contour_point_t& p_delta, char* out) const
    {
        ...
        if (flags & ARG_1_AND_2_ARE_WORDS) {
            ...
            if (new_x <= 127 && new_x >= -128 && new_y <= 127 && new_y >= -128) {
                memcpy(out, this, len);
...
struct composite_iter_t : hb_iter_with_fallback_t<composite_iter_t, const CompositeGlyphRecord&> {
    ...
    composite_iter_t __end__() const
    {
...
struct CompositeGlyph {
   ...
   * If removing hints it falls out of that. */
    ...
    void set_overlaps_flag()
    {
        CompositeGlyphRecord& glyph_chain = const_cast<CompositeGlyphRecord&>(StructAfter<CompositeGlyphRecord, GlyphHeader>(header));
        if (!bytes.check_range(&glyph_chain, CompositeGlyphRecord::min_size))
            return;
        glyph_chain.set_overlaps_flag();
    }

    bool compile_bytes_with_deltas(const hb_bytes_t& source_bytes, const contour_point_vector_t& deltas, hb_bytes_t& dest_bytes /* OUT */)
    {
        if (source_bytes.length <= GlyphHeader::static_size || header.numberOfContours != -1) {
