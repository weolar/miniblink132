...
struct SimpleGlyph {
    ...
    unsigned int instructions_length() const
    {
        ...
        if (unlikely(instruction_length_offset + 2 > bytes.length))
            return 0;

        const HBUINT16& instructionLength = StructAtOffset<HBUINT16>(&bytes, instruction_length_offset);
        /* Out of bounds of the current glyph */
        if (unlikely(length(instructionLength) > bytes.length))
            return 0;
    ...
    const hb_bytes_t trim_padding() const
    {
        ...
        if (unlikely(coords_with_flags != num_coordinates))
            return hb_bytes_t();
    ...
    {
        if (unlikely(!header.numberOfContours))
            return;

        unsigned flags_offset = length(instructions_length());
        if (unlikely(flags_offset + 1 > bytes.length))
            return;
    ...
    static bool read_flags(const HBUINT8*& p /* IN/OUT */, contour_point_vector_t& points_ /* IN/OUT */, const HBUINT8* end)
    {
    ...
    static bool read_points(const HBUINT8*& p /* IN/OUT */, contour_point_vector_t& points_ /* IN/OUT */, const HBUINT8* end, float contour_point_t::*m,
        const simple_glyph_flag_t short_flag, const simple_glyph_flag_t same_flag)
    {
        int v = 0;
    ...
    bool get_contour_points(contour_point_vector_t& points_ /* OUT */, bool phantom_only = false) const
    {
        ...
        if (unlikely((const char*)p < bytes.arrayZ))
            return false; /* Unlikely overflow */
        const HBUINT8* end = (const HBUINT8*)(bytes.arrayZ + bytes.length);
        if (unlikely(p >= end))
            return false;

        /* Read x & y coordinates */
        return read_flags(p, points_, end) && read_points(p, points_, end, &contour_point_t::x, FLAG_X_SHORT, FLAG_X_SAME)
            && read_points(p, points_, end, &contour_point_t::y, FLAG_Y_SHORT, FLAG_Y_SAME);
    }

    static void encode_coord(
        int value, uint8_t& flag, const simple_glyph_flag_t short_flag, const simple_glyph_flag_t same_flag, hb_vector_t<uint8_t>& coords /* OUT */)
    {
        if (value == 0) {
            flag |= same_flag;
    ...
    static void encode_flag(uint8_t& flag, uint8_t& repeat, uint8_t& lastflag, hb_vector_t<uint8_t>& flags /* OUT */)
    {
    ...
    bool compile_bytes_with_deltas(const contour_point_vector_t& all_points, bool no_hinting, hb_bytes_t& dest_bytes /* OUT */)
    {
        ...
        for (unsigned i = 0; i < num_points; i++) {
            ...
            if (i == 0)
                lastflag = flag + 1; //make lastflag != flag for the first point
