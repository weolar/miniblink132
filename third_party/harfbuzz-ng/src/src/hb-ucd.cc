...
static hb_bool_t hb_ucd_compose(hb_unicode_funcs_t* ufuncs HB_UNUSED, hb_codepoint_t a, hb_codepoint_t b, hb_codepoint_t* ab, void* user_data HB_UNUSED)
{
    ...
    if ((a & 0xFFFFF800u) == 0x0000u && (b & 0xFFFFFF80) == 0x0300u) {
        ...
        const uint32_t* v = hb_bsearch(k, _hb_ucd_dm2_u32_map, ARRAY_LENGTH(_hb_ucd_dm2_u32_map), sizeof(*_hb_ucd_dm2_u32_map), _cmp_pair_11_7_14);
        if (likely(!v))
            return false;
        u = HB_CODEPOINT_DECODE3_11_7_14_3(*v);
    } else {
        uint64_t k = HB_CODEPOINT_ENCODE3(a, b, 0);
        const uint64_t* v = hb_bsearch(k, _hb_ucd_dm2_u64_map, ARRAY_LENGTH(_hb_ucd_dm2_u64_map), sizeof(*_hb_ucd_dm2_u64_map), _cmp_pair);
        if (likely(!v))
            return false;
...
static hb_bool_t hb_ucd_decompose(hb_unicode_funcs_t* ufuncs HB_UNUSED, hb_codepoint_t ab, hb_codepoint_t* a, hb_codepoint_t* b, void* user_data HB_UNUSED)
{
...
static inline void free_static_ucd_funcs()
{
    static_ucd_funcs.free_instance();
}

hb_unicode_funcs_t* hb_ucd_get_unicode_funcs()
{
#ifdef HB_NO_UCD
