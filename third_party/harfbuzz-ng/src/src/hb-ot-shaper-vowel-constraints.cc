...
void _hb_preprocess_text_vowel_constraints(const hb_ot_shape_plan_t* plan HB_UNUSED, hb_buffer_t* buffer, hb_font_t* font HB_UNUSED)
{
    ...
    case HB_SCRIPT_DEVANAGARI:
        for (buffer->idx = 0; buffer->idx + 1 < count && buffer->successful;) {
            ...
            case 0x090Fu:
                ...
                break;
            case 0x0930u:
                if (0x094Du == buffer->cur(1).codepoint && buffer->idx + 2 < count && 0x0907u == buffer->cur(2).codepoint) {
                    (void)buffer->next_glyph();
                    matched = true;
                }
                break;
    ...
    case HB_SCRIPT_TAMIL:
        for (buffer->idx = 0; buffer->idx + 1 < count && buffer->successful;) {
            bool matched = false;
            if (0x0B85u == buffer->cur().codepoint && 0x0BC2u == buffer->cur(1).codepoint) {
                matched = true;
            }
            (void)buffer->next_glyph();
            if (matched)
