...
BENCHMARK(BM_MapInsert)->Range(1 << 4, 1 << 20);
...
static void BM_MapLookup(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        benchmark::DoNotOptimize(hb_map_get(original, needle++));
    }

    hb_map_destroy(original);
}
BENCHMARK(BM_MapLookup)->Range(1 << 4, 1 << 20); // Map size
