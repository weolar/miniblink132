...
void TrySubset(hb_face_t* face, const hb_codepoint_t text[], const int text_length, const uint8_t flags)
{
    bool drop_layout = flags & (1 << 1);
    unsigned input_set_flags = 0 //
    ...
    if (!drop_layout) {
        hb_set_del(hb_subset_input_set(input.get(), HB_SUBSET_SETS_DROP_TABLE_TAG), HB_TAG('G', 'S', 'U', 'B'));
        hb_set_del(hb_subset_input_set(input.get(), HB_SUBSET_SETS_DROP_TABLE_TAG), HB_TAG('G', 'P', 'O', 'S'));
        hb_set_del(hb_subset_input_set(input.get(), HB_SUBSET_SETS_DROP_TABLE_TAG), HB_TAG('G', 'D', 'E', 'F'));
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    hb::unique_ptr<hb_blob_t> blob(hb_blob_create(data_ptr, size, HB_MEMORY_MODE_READONLY, nullptr, nullptr));
    ...
    const hb_codepoint_t text[] = { 'A', 'B', 'C', 'D', 'E', 'X', 'Y', 'Z', '1', '2', '3', '@', '_', '%', '&', ')', '*', '$', '!' };

    TrySubset(face.get(), text, std::size(text), subset_flags);

    hb_codepoint_t text_from_data[16];
    if (size > sizeof(text_from_data) + 1) {
        memcpy(text_from_data, data + size - sizeof(text_from_data), sizeof(text_from_data));
