...
static bool ExtractResolutionFromFilename(const char* name, int* width_ptr, int* height_ptr)
{
...
static int TileARGBScale(const uint8_t* src_argb, int src_stride_argb, int src_width, int src_height, uint8_t* dst_argb, int dst_stride_argb,
    int destination_width, int destination_height, libyuv::FilterMode filtering)
{
    for (int y = 0; y < destination_height; y += kTileY) {
        for (int x = 0; x < destination_width; x += kTileX) {
            ...
            int r = libyuv::ARGBScaleClip(src_argb, src_stride_argb, src_width, src_height, dst_argb, dst_stride_argb, destination_width, destination_height, x,
...
int main(int argc, const char* argv[])
{
    ...
    if (org_is_yuv) {
        ...
        const int uv_size = ((Abs(image_width) + 1) / 2) * ((Abs(image_height) + 1) / 2);
...
#if defined(_MSC_VER)
    _fseeki64(file_org, static_cast<__int64>(num_skip_org) * static_cast<__int64>(org_size), SEEK_SET);
#else
    ...
    if (verbose) {
        printf("Size: %dx%d to %dx%d\n", image_width, image_height, dst_width, dst_height);
    }
    ...
    for (number_of_frames = 0;; ++number_of_frames) {
        ...
        size_t bytes_org = fread(ch_org, sizeof(uint8_t), static_cast<size_t>(org_size), file_org);
        ...
        for (int cur_rec = 0; cur_rec < num_rec; ++cur_rec) {
            ...
            if (org_is_yuv) {
                ...
                I420Scale(ch_org, src_width, ch_org + src_width * src_height, half_src_width,
                    ch_org + src_width * src_height + half_src_width * half_src_height, half_src_width, image_width, image_height, ch_rec, dst_width,
                    ch_rec + dst_width * dst_height, half_dst_width, ch_rec + dst_width * dst_height + half_dst_width * half_dst_height, half_dst_width,
                    dst_width, dst_height, static_cast<libyuv::FilterMode>(filter));
            } else {
                TileARGBScale(ch_org, Abs(image_width) * 4, image_width, image_height, ch_dst, dst_width * 4, dst_width, dst_height,
                    static_cast<libyuv::FilterMode>(filter));
            }
            bool rec_is_yuv = strstr(argv[fileindex_rec + cur_rec], "_P420.") != NULL;
            bool rec_is_argb = strstr(argv[fileindex_rec + cur_rec], "_ARGB.") != NULL;
            ...
            if (!org_is_yuv && rec_is_yuv) {
                ...
                libyuv::ARGBToI420(ch_dst, dst_width * 4, ch_rec, dst_width, ch_rec + dst_width * dst_height, half_width,
                    ch_rec + dst_width * dst_height + half_width * half_height, half_width, dst_width, dst_height);
            }

            // Output YUV or ARGB frame.
            if (rec_is_yuv) {
                size_t bytes_rec = fwrite(ch_rec, sizeof(uint8_t), static_cast<size_t>(total_size), file_rec[cur_rec]);
                if (bytes_rec < static_cast<size_t>(total_size)) {
                    break;
                }
            } else {
                size_t bytes_rec = fwrite(ch_dst, sizeof(uint8_t), static_cast<size_t>(dst_size), file_rec[cur_rec]);
