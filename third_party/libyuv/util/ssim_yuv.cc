...
#if !defined(LIBYUV_DISABLE_X86) && !defined(__SSE2__) && (defined(_M_X64) || (defined(_M_IX86_FP) && (_M_IX86_FP >= 2)))
#define __SSE2__
#endif
...
#define MAKE_WEIGHT(L)                                                                                                                                         \
    {                                                                                                                                                          \
        {                                                                                                                                                      \
            {                                                                                                                                                  \
                PWEIGHT(L, 0)                                                                                                                                  \
                , PWEIGHT(L, 1), PWEIGHT(L, 2), PWEIGHT(L, 3), PWEIGHT(L, 4), PWEIGHT(L, 5), PWEIGHT(L, 6), 0                                                  \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    }
...
} W0 = MAKE_WEIGHT(0), W1 = MAKE_WEIGHT(1), W2 = MAKE_WEIGHT(2), W3 = MAKE_WEIGHT(3);
...
static double FinalizeSSIM(double iw, double xm, double ym, double xxm, double xym, double yym)
{
...
double GetSSIM(const uint8_t* org, const uint8_t* rec, int xo, int yo, int W, int H, int stride)
{
...
#define LOAD_LINE_PAIR(L, WEIGHT)                                                                                                                              \
    do {                                                                                                                                                       \
        const __m128i v0 = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(org + (L)*stride));                                                                \
        const __m128i v1 = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(rec + (L)*stride));                                                                \
...
double CalcSSIM(const uint8_t* org, const uint8_t* rec, const int image_width, const int image_height)
{
    double SSIM = 0.;
    ...
    for (int j = KERNEL_Y; j < image_height - KERNEL_Y; ++j) {
        ...
        if (start_x < image_width) {
            ...
            for (int k = 0; k < KERNEL_SIZE; ++k) {
                const int offset = (j - KERNEL + k) * stride + image_width - kScratchWidth;
            ...
            for (int k = 0; k <= KERNEL_X + 1; ++k) {
                SSIM += GetSSIMFullKernel(scratch_org, scratch_rec, KERNEL + k, KERNEL, kScratchStride, kiW[k]);
            }
        }
    }
