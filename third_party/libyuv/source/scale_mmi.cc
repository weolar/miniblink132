...
void ScaleRowDown2_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst), [width] "r"(dst_width), [shift] "f"(shift)
                     : "memory");
}

void ScaleRowDown2Linear_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "daddi      %[width],         %[width],         -0x08         \n\t"
                     "bnez       %[width],         1b                              \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest0] "=&f"(dest0), [dest1] "=&f"(dest1), [dest] "=&f"(dest)
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst), [mask] "f"(mask), [shift] "f"(shift), [width] "r"(dst_width)
                     : "memory");
}

void ScaleRowDown2Box_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    ...
    __asm__ volatile(
        ...
        "daddi      %[width],        %[width],         -0x08          \n\t"
        "bnez       %[width],        1b                               \n\t"
        : [s0] "=&f"(s0), [s1] "=&f"(s1), [t0] "=&f"(t0), [t1] "=&f"(t1), [dest0] "=&f"(dest0), [dest1] "=&f"(dest1), [dest] "=&f"(dest)
        : [s] "r"(s), [t] "r"(t), [dst_ptr] "r"(dst), [width] "r"(dst_width), [shift0] "f"(shift0), [shift1] "f"(shift1), [ph] "f"(ph), [mask] "f"(mask)
        : "memory");
}

void ScaleARGBRowDown2_MMI(const uint8_t* src_argb, ptrdiff_t src_stride, uint8_t* dst_argb, int dst_width)
{
    (void)src_stride;
...
void ScaleARGBRowDown2Linear_MMI(const uint8_t* src_argb, ptrdiff_t src_stride, uint8_t* dst_argb, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "daddi      %[width],        %[width],         -0x02          \n\t"
                     "bnez       %[width],        1b                               \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest_hi] "=&f"(dest_hi), [dest_lo] "=&f"(dest_lo), [dest] "=&f"(dest)
                     : [src_ptr] "r"(src_argb), [dst_ptr] "r"(dst_argb), [width] "r"(dst_width)
                     : "memory");
}

void ScaleARGBRowDown2Box_MMI(const uint8_t* src_argb, ptrdiff_t src_stride, uint8_t* dst_argb, int dst_width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "daddi      %[width],        %[width],         -0x02          \n\t"
                     "bnez       %[width],        1b                               \n\t"
                     : [s0] "=&f"(s0), [t0] "=&f"(t0), [dest_hi] "=&f"(dest_hi), [dest_lo] "=&f"(dest_lo), [s_hi] "=&f"(s_hi), [s_lo] "=&f"(s_lo),
                     [t_hi] "=&f"(t_hi), [t_lo] "=&f"(t_lo), [dest] "=&f"(dest)
                     : [s] "r"(s), [t] "r"(t), [dst_ptr] "r"(dst_argb), [width] "r"(dst_width), [mask] "f"(mask), [ph] "f"(ph), [shfit] "f"(shfit)
                     : "memory");
}

void ScaleRowDown2_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    (void)src_stride;
...
void ScaleRowDown2Linear_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    (void)src_stride;
...
void ScaleRowDown2Box_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    ...
    __asm__ volatile(
        ...
        "daddi      %[width],         %[width],         -0x04         \n\t"
        "bnez       %[width],         1b                              \n\t"
        : [s0] "=&f"(s0), [s1] "=&f"(s1), [t0] "=&f"(t0), [t1] "=&f"(t1), [s_hi] "=&f"(s_hi), [s_lo] "=&f"(s_lo), [t_hi] "=&f"(t_hi), [t_lo] "=&f"(t_lo),
        [dest0] "=&f"(dest0), [dest1] "=&f"(dest1), [dest] "=&f"(dest)
        : [s] "r"(s), [t] "r"(t), [dst_ptr] "r"(dst), [width] "r"(dst_width), [shift0] "f"(shift0), [shift1] "f"(shift1), [ph] "f"(ph), [mask] "f"(mask)
        : "memory");
}

void ScaleRowDown4_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "bnez       %[width],        1b                               \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest_hi] "=&f"(dest_hi), [dest_lo] "=&f"(dest_lo), [dest] "=&f"(dest)
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst), [width] "r"(dst_width), [shift] "f"(shift), [mask] "f"(mask)
                     : "memory");
}

void ScaleRowDown4_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "bnez       %[width],        1b                               \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest_hi] "=&f"(dest_hi), [dest_lo] "=&f"(dest_lo), [dest] "=&f"(dest)
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst), [width] "r"(dst_width), [mask] "f"(mask)
                     : "memory");
}
...
void ScaleRowDown4Box_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     DO_SCALEROWDOWN4BOX_LOOP(% [dest0]) DO_SCALEROWDOWN4BOX_LOOP(% [dest1]) DO_SCALEROWDOWN4BOX_LOOP(% [dest2])
                     ...
                     [dest1] "=&f"(dest1), [dest2] "=&f"(dest2), [dest3] "=&f"(dest3), [src] "=&f"(src), [dest] "=&f"(dest)
                     : [src0_ptr] "r"(src0_ptr), [src1_ptr] "r"(src1_ptr), [src2_ptr] "r"(src2_ptr), [src3_ptr] "r"(src3_ptr), [dst_ptr] "r"(dst),
                     [width] "r"(dst_width), [shift] "f"(shift), [mask0] "f"(mask0), [ph] "f"(ph), [mask1] "f"(mask1)
                     : "memory");
}
...
void ScaleRowDown4Box_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    ...
    __asm__ volatile("1:                                                        \n\t"
                     ...
                     DO_SCALEROWDOWN4BOX_16_LOOP(% [dest0]) DO_SCALEROWDOWN4BOX_16_LOOP(% [dest1]) DO_SCALEROWDOWN4BOX_16_LOOP(% [dest2])
                         DO_SCALEROWDOWN4BOX_16_LOOP(% [dest3]) "punpcklwd  %[dest_lo],      %[dest0],          %[dest1]   \n\t"
...
void ScaleColsUp2_MMI(uint8_t* dst_ptr, const uint8_t* src_ptr, int dst_width, int x, int dx)
{
    uint64_t src, dest;
...
void ScaleColsUp2_16_MMI(uint16_t* dst_ptr, const uint16_t* src_ptr, int dst_width, int x, int dx)
{
    uint64_t src, dest;
...
void ScaleAddRow_MMI(const uint8_t* src_ptr, uint16_t* dst_ptr, int src_width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "daddi      %[width],        %[width],         -0x08          \n\t"
                     "bnez       %[width],        1b                               \n\t"
                     : [dest0] "=&f"(dest0), [dest1] "=&f"(dest1), [src_hi] "=&f"(src_hi), [src_lo] "=&f"(src_lo), [src] "=&f"(src)
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst_ptr), [width] "r"(src_width), [mask] "f"(mask)
                     : "memory");
}

void ScaleAddRow_16_MMI(const uint16_t* src_ptr, uint32_t* dst_ptr, int src_width)
{
    uint64_t src, src_hi, src_lo, dest0, dest1;
    const uint64_t mask = 0x0ULL;
...
void ScaleARGBRowDownEven_MMI(const uint8_t* src_argb, ptrdiff_t src_stride, int src_stepx, uint8_t* dst_argb, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "bnez       %[width],         1b                              \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest] "=&f"(dest)
                     : [src_ptr] "r"(src_argb), [dst_ptr] "r"(dst_argb), [src_stepx_4] "r"(src_stepx << 2), [width] "r"(dst_width)
                     : "memory");
}

void ScaleARGBRowDownEvenBox_MMI(const uint8_t* src_argb, ptrdiff_t src_stride, int src_stepx, uint8_t* dst_argb, int dst_width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "bnez       %[width],        1b                               \n\t"
                     : [src_hi] "=&f"(src_hi), [src_lo] "=&f"(src_lo), [dest_hi] "=&f"(dest_hi), [dest_lo] "=&f"(dest_lo), [dest0] "=&f"(dest0),
                     [dest1] "=&f"(dest1), [src0] "=&f"(src0), [src1] "=&f"(src1), [dest] "=&f"(dest)
                     : [src0_ptr] "r"(src0_ptr), [src1_ptr] "r"(src1_ptr), [dst_ptr] "r"(dst_argb), [width] "r"(dst_width), [src_stepx_4] "r"(src_stepx << 2),
                     [shift] "f"(shift), [mask] "f"(mask), [ph] "f"(ph)
                     : "memory");
}

// Scales a single row of pixels using point sampling.
void ScaleARGBCols_MMI(uint8_t* dst_argb, const uint8_t* src_argb, int dst_width, int x, int dx)
{
    const uint32_t* src = (const uint32_t*)(src_argb);
    uint32_t* dst = (uint32_t*)(dst_argb);
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     : [src_ptr] "r"(src), [dst_ptr] "r"(dst), [width] "r"(dst_width), [dx] "r"(dx), [x] "r"(x), [shift0] "r"(shift0), [shift1] "r"(shift1)
                     : "memory");
}

// Scales a single row of pixels up by 2x using point sampling.
void ScaleARGBColsUp2_MMI(uint8_t* dst_argb, const uint8_t* src_argb, int dst_width, int x, int dx)
{
...
int FixedDiv1_MIPS(int num, int div)
{
    ...
    asm("dsll    %[num],     %[num],     %[shift]    \n\t"
        ...
        "mflo    %[quo]                              \t\n"
        : [quo] "+&r"(quotient)
        : [num] "r"(num), [div] "r"(div), [val1] "r"(val1), [val11] "r"(val11), [shift] "r"(shift));
...
void ScaleRowUp2_16_MMI(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     "daddi      %[width],         %[width],        -0x08          \n\t"
                     "bnez       %[width],         1b                              \n\t"
                     : [src0] "=&f"(src0), [src1] "=&f"(src1), [dest04] "=&f"(dest04), [dest15] "=&f"(dest15), [dest26] "=&f"(dest26), [dest37] "=&f"(dest37),
                     [tmp0] "=&f"(tmp0), [tmp1] "=&f"(tmp1), [tmp2] "=&f"(tmp2), [tmp3] "=&f"(tmp3), [dest] "=&f"(dest)
                     : [src1_ptr] "r"(src_ptr), [src2_ptr] "r"(src2_ptr), [dst_ptr] "r"(dst), [width] "r"(dst_width), [mask0] "f"(mask0), [mask1] "f"(mask1),
                     [mask2] "f"(mask2), [mask3] "f"(mask3), [shift] "f"(shift), [ph] "f"(ph)
                     : "memory");
}

void ScaleRowDown34_MMI(const uint8_t* src_ptr, ptrdiff_t src_stride, uint8_t* dst, int dst_width)
{
    (void)src_stride;
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     : [src0] "=&f"(src[0]), [src1] "=&f"(src[1]), [tmp0] "=&f"(tmp[0]), [tmp1] "=&f"(tmp[1])
                     : [src_ptr] "r"(src_ptr), [dst_ptr] "r"(dst), [lmov] "f"(0xc), [rmov] "f"(0x18), [mask1] "f"(0xffff0000ffff), [rmov8] "f"(0x8),
                     [zero] "f"(0x0), [mask2] "f"(0xff000000), [width] "r"(dst_width), [lmov1] "f"(0x10)
                     : "memory");
}
// clang-format on
