...
#define ANY41(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, BPP, MASK)                                                                                                 \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* u_buf, const uint8_t* v_buf, const uint8_t* a_buf, uint8_t* dst_ptr, int width)                          \
    {                                                                                                                                                          \
        ...
        memcpy(dst_ptr + (n >> DUVSHIFT) * BPP, temp + 256, SS(r, DUVSHIFT) * BPP);                                                                            \
    }
...
#define ANY41C(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, BPP, MASK)                                                                                                \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* u_buf, const uint8_t* v_buf, const uint8_t* a_buf, uint8_t* dst_ptr,                                     \
    ...
    {                                                                                                                                                          \
        ...
        ANY_SIMD(temp, temp + 64, temp + 128, temp + 192, temp + 256, yuvconstants, MASK + 1);                                                                 \
        memcpy(dst_ptr + (n >> DUVSHIFT) * BPP, temp + 256, SS(r, DUVSHIFT) * BPP);                                                                            \
    }
...
#define ANY41CT(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, T, SBPP, BPP, MASK)                                                                                      \
    void NAMEANY(const T* y_buf, const T* u_buf, const T* v_buf, const T* a_buf, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)         \
    {                                                                                                                                                          \
        ...
        ANY_SIMD(temp, temp + 16, temp + 32, temp + 48, out, yuvconstants, MASK + 1);                                                                          \
...
#define ANY41PT(NAMEANY, ANY_SIMD, STYPE, SBPP, DTYPE, BPP, MASK)                                                                                              \
    void NAMEANY(const STYPE* r_buf, const STYPE* g_buf, const STYPE* b_buf, const STYPE* a_buf, DTYPE* dst_ptr, int depth, int width)                         \
    {                                                                                                                                                          \
...
#define ANY31(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, BPP, MASK)                                                                                                 \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* u_buf, const uint8_t* v_buf, uint8_t* dst_ptr, int width)                                                \
    {                                                                                                                                                          \
...
#define ANY31C(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, BPP, MASK)                                                                                                \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* u_buf, const uint8_t* v_buf, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)       \
    {                                                                                                                                                          \
        ...
        ANY_SIMD(temp, temp + 128, temp + 256, temp + 384, yuvconstants, MASK + 1);                                                                            \
        memcpy(dst_ptr + (n >> DUVSHIFT) * BPP, temp + 384, SS(r, DUVSHIFT) * BPP);                                                                            \
    }
...
#define ANY31CT(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, T, SBPP, BPP, MASK)                                                                                      \
    void NAMEANY(const T* y_buf, const T* u_buf, const T* v_buf, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)                         \
    {                                                                                                                                                          \
...
#define ANY31PT(NAMEANY, ANY_SIMD, STYPE, SBPP, DTYPE, BPP, MASK)                                                                                              \
    void NAMEANY(const STYPE* r_buf, const STYPE* g_buf, const STYPE* b_buf, DTYPE* dst_ptr, int depth, int width)                                             \
    {                                                                                                                                                          \
...
#define ANY21(NAMEANY, ANY_SIMD, UVSHIFT, SBPP, SBPP2, BPP, MASK)                                                                                              \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* uv_buf, uint8_t* dst_ptr, int width)                                                                     \
    {                                                                                                                                                          \
        ...
        memcpy(temp + 128, uv_buf + (n >> UVSHIFT) * SBPP2, SS(r, UVSHIFT) * SBPP2);                                                                           \
...
#define ANY21S(NAMEANY, ANY_SIMD, SBPP, BPP, MASK)                                                                                                             \
    void NAMEANY(const uint8_t* src_yuy2, int stride_yuy2, uint8_t* dst_uv, int width)                                                                         \
    {                                                                                                                                                          \
...
#define ANY21C(NAMEANY, ANY_SIMD, UVSHIFT, SBPP, SBPP2, BPP, MASK)                                                                                             \
    void NAMEANY(const uint8_t* y_buf, const uint8_t* uv_buf, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)                            \
    {                                                                                                                                                          \
        SIMD_ALIGNED(uint8_t temp[128 * 3]);                                                                                                                   \
        memset(temp, 0, 128 * 2); /* for msan */                                                                                                               \
        int r = width & MASK;                                                                                                                                  \
...
#define ANY21CT(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, T, SBPP, BPP, MASK)                                                                                      \
    void NAMEANY(const T* y_buf, const T* uv_buf, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)                                        \
    {                                                                                                                                                          \
        SIMD_ALIGNED(T temp[16 * 3]);                                                                                                                          \
        SIMD_ALIGNED(uint8_t out[64]);                                                                                                                         \
        memset(temp, 0, 16 * 3 * SBPP); /* for YUY2 and msan */                                                                                                \
...
#define ANY21PT(NAMEANY, ANY_SIMD, T, BPP, MASK)                                                                                                               \
    void NAMEANY(const T* src_u, const T* src_v, T* dst_uv, int depth, int width)                                                                              \
    {                                                                                                                                                          \
        SIMD_ALIGNED(T temp[16 * 4]);                                                                                                                          \
...
ANY11P(I400ToARGBRow_Any_SSE2, I400ToARGBRow_SSE2, const struct YuvConstants*, 1, 4, 7)
#endif
#if defined(HAS_I400TOARGBROW_AVX2)
ANY11P(I400ToARGBRow_Any_AVX2, I400ToARGBRow_AVX2, const struct YuvConstants*, 1, 4, 15)
#endif
#if defined(HAS_I400TOARGBROW_NEON)
ANY11P(I400ToARGBRow_Any_NEON, I400ToARGBRow_NEON, const struct YuvConstants*, 1, 4, 7)
#endif
#if defined(HAS_I400TOARGBROW_MSA)
ANY11P(I400ToARGBRow_Any_MSA, I400ToARGBRow_MSA, const struct YuvConstants*, 1, 4, 15)
#endif
#if defined(HAS_I400TOARGBROW_LSX)
ANY11P(I400ToARGBRow_Any_LSX, I400ToARGBRow_LSX, const struct YuvConstants*, 1, 4, 15)
#endif

#if defined(HAS_ARGBTORGB565DITHERROW_SSE2)
ANY11P(ARGBToRGB565DitherRow_Any_SSE2, ARGBToRGB565DitherRow_SSE2, const uint32_t, 4, 2, 3)
#endif
#if defined(HAS_ARGBTORGB565DITHERROW_AVX2)
ANY11P(ARGBToRGB565DitherRow_Any_AVX2, ARGBToRGB565DitherRow_AVX2, const uint32_t, 4, 2, 7)
#endif
#if defined(HAS_ARGBTORGB565DITHERROW_NEON)
ANY11P(ARGBToRGB565DitherRow_Any_NEON, ARGBToRGB565DitherRow_NEON, const uint32_t, 4, 2, 7)
#endif
#if defined(HAS_ARGBTORGB565DITHERROW_MSA)
ANY11P(ARGBToRGB565DitherRow_Any_MSA, ARGBToRGB565DitherRow_MSA, const uint32_t, 4, 2, 7)
#endif
#if defined(HAS_ARGBTORGB565DITHERROW_LASX)
ANY11P(ARGBToRGB565DitherRow_Any_LASX, ARGBToRGB565DitherRow_LASX, const uint32_t, 4, 2, 15)
#endif
...
ANY11C(Convert16To8Row_Any_SSSE3, Convert16To8Row_SSSE3, 2, 1, uint16_t, uint8_t, 15)
#endif
#ifdef HAS_CONVERT16TO8ROW_AVX2
ANY11C(Convert16To8Row_Any_AVX2, Convert16To8Row_AVX2, 2, 1, uint16_t, uint8_t, 31)
#endif
#ifdef HAS_CONVERT16TO8ROW_NEON
ANY11C(Convert16To8Row_Any_NEON, Convert16To8Row_NEON, 2, 1, uint16_t, uint8_t, 15)
#endif
...
ANY11C(MultiplyRow_16_Any_AVX2, MultiplyRow_16_AVX2, 2, 2, uint16_t, uint16_t, 31)
#endif
#ifdef HAS_MULTIPLYROW_16_NEON
ANY11C(MultiplyRow_16_Any_NEON, MultiplyRow_16_NEON, 2, 2, uint16_t, uint16_t, 15)
#endif
...
ANY11P16(HalfFloat1Row_Any_F16C, HalfFloat1Row_F16C, uint16_t, uint16_t, 2, 2, 15)
#endif
#ifdef HAS_HALFFLOATROW_NEON
ANY11P16(HalfFloatRow_Any_NEON, HalfFloatRow_NEON, uint16_t, uint16_t, 2, 2, 7)
ANY11P16(HalfFloat1Row_Any_NEON, HalfFloat1Row_NEON, uint16_t, uint16_t, 2, 2, 7)
#endif
...
#define ANY11C(NAMEANY, ANY_SIMD, UVSHIFT, SBPP, BPP, MASK)                                                                                                    \
    void NAMEANY(const uint8_t* src_ptr, uint8_t* dst_ptr, const struct YuvConstants* yuvconstants, int width)                                                 \
    {                                                                                                                                                          \
        SIMD_ALIGNED(uint8_t temp[128 * 2]);                                                                                                                   \
        memset(temp, 0, 128); /* for YUY2 and msan */                                                                                                          \
        int r = width & MASK;                                                                                                                                  \
...
#define ANY11I(NAMEANY, ANY_SIMD, TD, TS, SBPP, BPP, MASK)                                                                                                     \
    void NAMEANY(TD* dst_ptr, const TS* src_ptr, ptrdiff_t src_stride, int width, int source_y_fraction)                                                       \
    {                                                                                                                                                          \
        ...
        if (source_y_fraction) {                                                                                                                               \
            memcpy(temps + 64, src_ptr + src_stride + n * SBPP, r * SBPP * sizeof(TS));                                                                        \
        }                                                                                                                                                      \
...
ANY11I(InterpolateRow_16_Any_NEON, InterpolateRow_16_NEON, uint16_t, uint16_t, 1, 1, 7)
#endif
#undef ANY11I

// Any 1 to 1 interpolate with scale param
#define ANY11IS(NAMEANY, ANY_SIMD, TD, TS, SBPP, BPP, MASK)                                                                                                    \
    void NAMEANY(TD* dst_ptr, const TS* src_ptr, ptrdiff_t src_stride, int scale, int width, int source_y_fraction)                                            \
    {                                                                                                                                                          \
        SIMD_ALIGNED(TS temps[64 * 2]);                                                                                                                        \
        SIMD_ALIGNED(TD tempd[64]);                                                                                                                            \
        memset(temps, 0, sizeof(temps)); /* for msan */                                                                                                        \
...
ANY11IS(InterpolateRow_16To8_Any_NEON, InterpolateRow_16To8_NEON, uint8_t, uint16_t, 1, 1, 7)
#endif
#ifdef HAS_INTERPOLATEROW_16TO8_AVX2
ANY11IS(InterpolateRow_16To8_Any_AVX2, InterpolateRow_16To8_AVX2, uint8_t, uint16_t, 1, 1, 31)
#endif
...
#define ANY13(NAMEANY, ANY_SIMD, BPP, MASK)                                                                                                                    \
    void NAMEANY(const uint8_t* src_ptr, uint8_t* dst_r, uint8_t* dst_g, uint8_t* dst_b, int width)                                                            \
    {                                                                                                                                                          \
...
#define ANY14(NAMEANY, ANY_SIMD, BPP, MASK)                                                                                                                    \
    void NAMEANY(const uint8_t* src_ptr, uint8_t* dst_r, uint8_t* dst_g, uint8_t* dst_b, uint8_t* dst_a, int width)                                            \
    {                                                                                                                                                          \
        ...
        memcpy(temp, src_ptr + n * BPP, r * BPP);                                                                                                              \
        ANY_SIMD(temp, temp + 16 * 4, temp + 16 * 5, temp + 16 * 6, temp + 16 * 7, MASK + 1);                                                                  \
...
#define ANY12S(NAMEANY, ANY_SIMD, UVSHIFT, BPP, MASK)                                                                                                          \
    void NAMEANY(const uint8_t* src_ptr, int src_stride, uint8_t* dst_u, uint8_t* dst_v, int width)                                                            \
    {                                                                                                                                                          \
        SIMD_ALIGNED(uint8_t temp[128 * 4]);                                                                                                                   \
        memset(temp, 0, 128 * 2); /* for msan */                                                                                                               \
        int r = width & MASK;                                                                                                                                  \
        ...
        memcpy(temp, src_ptr + (n >> UVSHIFT) * BPP, SS(r, UVSHIFT) * BPP);                                                                                    \
        memcpy(temp + 128, src_ptr + src_stride + (n >> UVSHIFT) * BPP, SS(r, UVSHIFT) * BPP);                                                                 \
        if ((width & 1) && UVSHIFT == 0) { /* repeat last pixel for subsample */                                                                               \
            memcpy(temp + SS(r, UVSHIFT) * BPP, temp + SS(r, UVSHIFT) * BPP - BPP, BPP);                                                                       \
            memcpy(temp + 128 + SS(r, UVSHIFT) * BPP, temp + 128 + SS(r, UVSHIFT) * BPP - BPP, BPP);                                                           \
        }                                                                                                                                                      \
...
#define ANY11S(NAMEANY, ANY_SIMD, UVSHIFT, BPP, MASK)                                                                                                          \
    void NAMEANY(const uint8_t* src_ptr, int src_stride, uint8_t* dst_vu, int width)                                                                           \
    {                                                                                                                                                          \
        SIMD_ALIGNED(uint8_t temp[128 * 3]);                                                                                                                   \
        memset(temp, 0, 128 * 2); /* for msan */                                                                                                               \
        int r = width & MASK;                                                                                                                                  \
...
#define ANYDETILE(NAMEANY, ANY_SIMD, MASK)                                                                                                                     \
    void NAMEANY(const uint8_t* src, ptrdiff_t src_tile_stride, uint8_t* dst, int width)                                                                       \
    {                                                                                                                                                          \
...
#define ANYDETILESPLITUV(NAMEANY, ANY_SIMD, MASK)                                                                                                              \
    void NAMEANY(const uint8_t* src_uv, ptrdiff_t src_tile_stride, uint8_t* dst_u, uint8_t* dst_v, int width)                                                  \
    {                                                                                                                                                          \
        SIMD_ALIGNED(uint8_t temp[16 * 2]);                                                                                                                    \
...
#define ANYDETILEMERGE(NAMEANY, ANY_SIMD, MASK)                                                                                                                \
    void NAMEANY(const uint8_t* src_y, ptrdiff_t src_y_tile_stride, const uint8_t* src_uv, ptrdiff_t src_uv_tile_stride, uint8_t* dst_yuy2, int width)         \
    {                                                                                                                                                          \
        ...
        int n = width & ~MASK;                                                                                                                                 \
        if (n > 0) {                                                                                                                                           \
            ANY_SIMD(src_y, src_y_tile_stride, src_uv, src_uv_tile_stride, dst_yuy2, n);                                                                       \
        }                                                                                                                                                      \
        memcpy(temp, src_y + (n / 16) * src_y_tile_stride, r);                                                                                                 \
        memcpy(temp + 16, src_uv + (n / 16) * src_uv_tile_stride, r);                                                                                          \
        ANY_SIMD(temp, src_y_tile_stride, temp + 16, src_uv_tile_stride, temp + 32, r);                                                                        \
