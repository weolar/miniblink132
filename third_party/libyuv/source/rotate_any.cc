...
#define TANY(NAMEANY, TPOS_SIMD, MASK)                                                                                                                         \
    void NAMEANY(const uint8_t* src, int src_stride, uint8_t* dst, int dst_stride, int width)                                                                  \
    {                                                                                                                                                          \
        int r = width & MASK;                                                                                                                                  \
        int n = width - r;                                                                                                                                     \
        if (n > 0) {                                                                                                                                           \
...
#define TUVANY(NAMEANY, TPOS_SIMD, MASK)                                                                                                                       \
    void NAMEANY(const uint8_t* src, int src_stride, uint8_t* dst_a, int dst_stride_a, uint8_t* dst_b, int dst_stride_b, int width)                            \
    {                                                                                                                                                          \
        int r = width & MASK;                                                                                                                                  \
        int n = width - r;                                                                                                                                     \
        if (n > 0) {                                                                                                                                           \
        ...
        TransposeUVWx8_C(src + n * 2, src_stride, dst_a + n * dst_stride_a, dst_stride_a, dst_b + n * dst_stride_b, dst_stride_b, r);                          \
    }
