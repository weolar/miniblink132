...
void TransposeWx8_MMI(const uint8_t* src, int src_stride, uint8_t* dst, int dst_stride, int width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     : [tmp0] "=&f"(tmp0), [tmp1] "=&f"(tmp1), [tmp2] "=&f"(tmp2), [tmp3] "=&f"(tmp3), [tmp4] "=&f"(tmp4), [tmp5] "=&f"(tmp5),
                     [tmp6] "=&f"(tmp6), [tmp7] "=&f"(tmp7), [tmp8] "=&f"(tmp8), [tmp9] "=&f"(tmp9), [tmp10] "=&f"(tmp10), [tmp11] "=&f"(tmp11),
                     [tmp12] "=&f"(tmp12), [tmp13] "=&f"(tmp13), [dst] "+&r"(dst), [src_tmp] "+&r"(src_tmp)
                     : [src] "r"(src), [width] "r"(width), [src_stride] "r"(src_stride), [dst_stride] "r"(dst_stride)
                     : "memory");
}
...
void TransposeUVWx8_MMI(const uint8_t* src, int src_stride, uint8_t* dst_a, int dst_stride_a, uint8_t* dst_b, int dst_stride_b, int width)
{
    ...
    __asm__ volatile("1:                                                           \n\t"
                     ...
                     : [tmp0] "=&f"(tmp0), [tmp1] "=&f"(tmp1), [tmp2] "=&f"(tmp2), [tmp3] "=&f"(tmp3), [tmp4] "=&f"(tmp4), [tmp5] "=&f"(tmp5),
                     [tmp6] "=&f"(tmp6), [tmp7] "=&f"(tmp7), [tmp8] "=&f"(tmp8), [tmp9] "=&f"(tmp9), [tmp10] "=&f"(tmp10), [tmp11] "=&f"(tmp11),
                     [tmp12] "=&f"(tmp12), [tmp13] "=&f"(tmp13), [dst_a] "+&r"(dst_a), [dst_b] "+&r"(dst_b), [src_tmp] "+&r"(src_tmp)
                     : [src] "r"(src), [width] "r"(width), [dst_stride_a] "r"(dst_stride_a), [dst_stride_b] "r"(dst_stride_b), [src_stride] "r"(src_stride)
                     : "memory");
}
