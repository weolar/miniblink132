    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    int64_t src_argb_plane_size = (Abs(src_width) + b * 2) * (Abs(src_height) + b * 2) * 4LL;
    ...
    ARGBScale(src_argb + (src_stride_argb * b) + b * 4, src_stride_argb, src_width, src_height, dst_argb_c + (dst_stride_argb * b) + b * 4, dst_stride_argb,
        dst_width, dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    ARGBScale(src_argb + (src_stride_argb * b) + b * 4, src_stride_argb, src_width, src_height, dst_argb_opt + (dst_stride_argb * b) + b * 4, dst_stride_argb,
        dst_width, dst_height, f);
    ...
    for (i = 0; i < benchmark_iterations; ++i) {
        ARGBScale(src_argb + (src_stride_argb * b) + b * 4, src_stride_argb, src_width, src_height, dst_argb_opt + (dst_stride_argb * b) + b * 4,
    ...
    for (i = b; i < (dst_height + b); ++i) {
        for (j = b * 4; j < (dst_width + b) * 4; ++j) {
            int abs_diff = Abs(dst_argb_c[(i * dst_stride_argb) + j] - dst_argb_opt[(i * dst_stride_argb) + j]);
...
static int TileARGBScale(const uint8_t* src_argb, int src_stride_argb, int src_width, int src_height, uint8_t* dst_argb, int dst_stride_argb, int dst_width,
    int dst_height, FilterMode filtering)
{
    for (int y = 0; y < dst_height; y += kTileY) {
        for (int x = 0; x < dst_width; x += kTileX) {
            ...
            int r = ARGBScaleClip(
                src_argb, src_stride_argb, src_width, src_height, dst_argb, dst_stride_argb, dst_width, dst_height, x, y, clip_width, clip_height, filtering);
            if (r) {
                return r;
            }
        }
...
static int ARGBClipTestFilter(int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations)
{
    if (!SizeValid(src_width, src_height, dst_width, dst_height)) {
        return 0;
    }
    ...
    const int b = 128;
    int64_t src_argb_plane_size = (Abs(src_width) + b * 2) * (Abs(src_height) + b * 2) * 4;
    int src_stride_argb = (b * 2 + Abs(src_width)) * 4;
    ...
    double opt_time = get_time();
    for (i = 0; i < benchmark_iterations; ++i) {
        TileARGBScale(src_argb + (src_stride_argb * b) + b * 4, src_stride_argb, src_width, src_height, dst_argb_opt + (dst_stride_argb * b) + b * 4,
            dst_stride_argb, dst_width, dst_height, f);
    }
    opt_time = (get_time() - opt_time) / benchmark_iterations;
...
#define TEST_FACTOR1(DISABLED_, name, filter, nom, denom, max_diff)                                                                                            \
    ...
    {                                                                                                                                                          \
        int diff = ARGBTestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                       \
    ...
    {                                                                                                                                                          \
        int diff = ARGBClipTestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                   \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_);                                                                        \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }
...
#else
#define TEST_FACTOR(name, nom, denom) TEST_FACTOR1(DISABLED_, name, Bilinear, nom, denom, 3)
#endif
#endif
...
#define TEST_SCALETO1(DISABLED_, name, width, height, filter, max_diff)                                                                                        \
    ...
    {                                                                                                                                                          \
        int diff = ARGBClipTestFilter(benchmark_width_, benchmark_height_, width, height, kFilter##filter, benchmark_iterations_);                             \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##name##ClipFrom##width##x##height##_##filter)                                                                            \
    {                                                                                                                                                          \
        int diff = ARGBClipTestFilter(width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_);                   \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }
...
#else
#define TEST_SCALETO(name, width, height) TEST_SCALETO1(DISABLED_, name, width, height, Bilinear, 3)
#endif
#endif
...
#define TEST_SCALESWAPXY1(name, filter, max_diff)                                                                                                              \
    ...
    {                                                                                                                                                          \
        int diff = ARGBTestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,            \
...
LIBYUV_API
int YUVToARGBScaleReference2(const uint8_t* src_y, int src_stride_y, const uint8_t* src_u, int src_stride_u, const uint8_t* src_v, int src_stride_v,
...
// Test scaling with C vs Opt and return maximum pixel difference. 0 = exact.
static int YUVToARGBTestFilter(int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations)
{
    int64_t src_y_plane_size = Abs(src_width) * Abs(src_height);
    int64_t src_uv_plane_size = ((Abs(src_width) + 1) / 2) * ((Abs(src_height) + 1) / 2);
    ...
    YUVToARGBScaleReference2(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, libyuv::FOURCC_I420, src_width, src_height, dst_argb_c,
        dst_stride_argb, libyuv::FOURCC_I420, dst_width, dst_height, 0, 0, dst_width, dst_height, f);

    for (int i = 0; i < benchmark_iterations; ++i) {
        YUVToARGBScaleClip(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, libyuv::FOURCC_I420, src_width, src_height, dst_argb_opt,
            dst_stride_argb, libyuv::FOURCC_I420, dst_width, dst_height, 0, 0, dst_width, dst_height, f);
    }
    int max_diff = 0;
    for (int i = 0; i < dst_height; ++i) {
        for (int j = 0; j < dst_width * 4; ++j) {
            int abs_diff = Abs(dst_argb_c[(i * dst_stride_argb) + j] - dst_argb_opt[(i * dst_stride_argb) + j]);
            if (abs_diff > max_diff) {
                printf("error %d at %d,%d c %d opt %d", abs_diff, j, i, dst_argb_c[(i * dst_stride_argb) + j], dst_argb_opt[(i * dst_stride_argb) + j]);
...
TEST_F(LibYUVScaleTest, YUVToRGBScaleUp)
{
    int diff = YUVToARGBTestFilter(
        benchmark_width_, benchmark_height_, benchmark_width_ * 3 / 2, benchmark_height_ * 3 / 2, libyuv::kFilterBilinear, benchmark_iterations_);
    EXPECT_LE(diff, 10);
}
...
{
    int diff = YUVToARGBTestFilter(
        benchmark_width_ * 3 / 2, benchmark_height_ * 3 / 2, benchmark_width_, benchmark_height_, libyuv::kFilterBilinear, benchmark_iterations_);
    EXPECT_LE(diff, 10);
}
...
TEST_F(LibYUVScaleTest, ARGBTest3x)
{
    ...
    for (int i = 0; i < iterations160; ++i) {
        ARGBScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }
    ...
    ARGBScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterNone);
...
TEST_F(LibYUVScaleTest, ARGBTest4x)
{
    ...
    int iterations160 = (benchmark_width_ * benchmark_height_ + (160 - 1)) / 160 * benchmark_iterations_;
    for (int i = 0; i < iterations160; ++i) {
        ARGBScale(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }
    ...
    ARGBScale(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterNone);
