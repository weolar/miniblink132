...
static int I420TestFilter(
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    if (!dst_y_c || !dst_u_c || !dst_v_c || !dst_y_opt || !dst_u_opt || !dst_v_opt) {
        printf("Skipped.  Alloc failed " FILELINESTR(__FILE__, __LINE__) "\n");
        return 0;
    }
    ...
    I420Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_c, dst_stride_y, dst_u_c, dst_stride_uv, dst_v_c,
    ...
    for (i = 0; i < benchmark_iterations; ++i) {
        I420Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_opt, dst_stride_y, dst_u_opt, dst_stride_uv,
            dst_v_opt, dst_stride_uv, dst_width, dst_height, f);
    }
    ...
    for (i = 0; i < (dst_height); ++i) {
        for (j = 0; j < (dst_width); ++j) {
            int abs_diff = Abs(dst_y_c[(i * dst_stride_y) + j] - dst_y_opt[(i * dst_stride_y) + j]);
    ...
    for (i = 0; i < (dst_height_uv); ++i) {
        for (j = 0; j < (dst_width_uv); ++j) {
            int abs_diff = Abs(dst_u_c[(i * dst_stride_uv) + j] - dst_u_opt[(i * dst_stride_uv) + j]);
            if (abs_diff > max_diff) {
                max_diff = abs_diff;
            }
            abs_diff = Abs(dst_v_c[(i * dst_stride_uv) + j] - dst_v_opt[(i * dst_stride_uv) + j]);
            if (abs_diff > max_diff) {
                max_diff = abs_diff;
            }
        }
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(disable_cpu_flags); // Disable all CPU optimization.
    I420Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_8, dst_stride_y, dst_u_8, dst_stride_uv, dst_v_8,
        dst_stride_uv, dst_width, dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    for (i = 0; i < benchmark_iterations; ++i) {
        I420Scale_12(p_src_y_12, src_stride_y, p_src_u_12, src_stride_uv, p_src_v_12, src_stride_uv, src_width, src_height, p_dst_y_12, dst_stride_y,
            p_dst_u_12, dst_stride_uv, p_dst_v_12, dst_stride_uv, dst_width, dst_height, f);
    }
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    for (i = 0; i < benchmark_iterations; ++i) {
        I420Scale_16(p_src_y_16, src_stride_y, p_src_u_16, src_stride_uv, p_src_v_16, src_stride_uv, src_width, src_height, p_dst_y_16, dst_stride_y,
            p_dst_u_16, dst_stride_uv, p_dst_v_16, dst_stride_uv, dst_width, dst_height, f);
    }
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(disable_cpu_flags); // Disable all CPU optimization.
    double c_time = get_time();
    I444Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_c, dst_stride_y, dst_u_c, dst_stride_uv, dst_v_c,
        dst_stride_uv, dst_width, dst_height, f);
    c_time = (get_time() - c_time);
    ...
    double opt_time = get_time();
    for (i = 0; i < benchmark_iterations; ++i) {
        I444Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_opt, dst_stride_y, dst_u_opt, dst_stride_uv,
            dst_v_opt, dst_stride_uv, dst_width, dst_height, f);
    }
    opt_time = (get_time() - opt_time) / benchmark_iterations;
    // Report performance of C vs OPT.
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(disable_cpu_flags); // Disable all CPU optimization.
    I444Scale(src_y, src_stride_y, src_u, src_stride_uv, src_v, src_stride_uv, src_width, src_height, dst_y_8, dst_stride_y, dst_u_8, dst_stride_uv, dst_v_8,
        dst_stride_uv, dst_width, dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    for (i = 0; i < benchmark_iterations; ++i) {
        I444Scale_12(p_src_y_12, src_stride_y, p_src_u_12, src_stride_uv, p_src_v_12, src_stride_uv, src_width, src_height, p_dst_y_12, dst_stride_y,
            p_dst_u_12, dst_stride_uv, p_dst_v_12, dst_stride_uv, dst_width, dst_height, f);
    }
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    for (i = 0; i < benchmark_iterations; ++i) {
        I444Scale_16(p_src_y_16, src_stride_y, p_src_u_16, src_stride_uv, p_src_v_16, src_stride_uv, src_width, src_height, p_dst_y_16, dst_stride_y,
            p_dst_u_16, dst_stride_uv, p_dst_v_16, dst_stride_uv, dst_width, dst_height, f);
    }
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(disable_cpu_flags); // Disable all CPU optimization.
    double c_time = get_time();
    NV12Scale(src_y, src_stride_y, src_uv, src_stride_uv, src_width, src_height, dst_y_c, dst_stride_y, dst_uv_c, dst_stride_uv, dst_width, dst_height, f);
    c_time = (get_time() - c_time);
    ...
    for (i = 0; i < benchmark_iterations; ++i) {
        NV12Scale(
            src_y, src_stride_y, src_uv, src_stride_uv, src_width, src_height, dst_y_opt, dst_stride_y, dst_uv_opt, dst_stride_uv, dst_width, dst_height, f);
    }
    opt_time = (get_time() - opt_time) / benchmark_iterations;
    // Report performance of C vs OPT.
    printf("filter %d - %8d us C - %8d us OPT\n", f, static_cast<int>(c_time * 1e6), static_cast<int>(opt_time * 1e6));
    ...
    for (i = 0; i < (dst_height_uv); ++i) {
        for (j = 0; j < (dst_width_uv * 2); ++j) {
            int abs_diff = Abs(dst_uv_c[(i * dst_stride_uv) + j] - dst_uv_opt[(i * dst_stride_uv) + j]);
            if (abs_diff > max_diff) {
                max_diff = abs_diff;
            }
        }
...
#define TEST_FACTOR1(DISABLED_, name, filter, nom, denom, max_diff)                                                                                            \
    ...
    {                                                                                                                                                          \
        int diff = I420TestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                       \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, I444ScaleDownBy##name##_##filter)                                                                                                  \
    {                                                                                                                                                          \
        int diff = I444TestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                       \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I420ScaleDownBy##name##_##filter##_12)                                                                                  \
    {                                                                                                                                                          \
        int diff = I420TestFilter_12(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                    \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444ScaleDownBy##name##_##filter##_12)                                                                                  \
    {                                                                                                                                                          \
        int diff = I444TestFilter_12(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                    \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, NV12ScaleDownBy##name##_##filter)                                                                                                  \
    {                                                                                                                                                          \
        int diff = NV12TestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                       \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }
...
#define TEST_SCALETO1(DISABLED_, name, width, height, filter, max_diff)                                                                                        \
    ...
    {                                                                                                                                                          \
        int diff = I420TestFilter(                                                                                                                             \
            benchmark_width_, benchmark_height_, width, height, kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);              \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    ...
    TEST_F(LibYUVScaleTest, DISABLED_##I420##name##To##width##x##height##_##filter##_12)                                                                       \
    {                                                                                                                                                          \
        int diff = I420TestFilter_12(                                                                                                                          \
            benchmark_width_, benchmark_height_, width, height, kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);              \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444##name##To##width##x##height##_##filter##_12)                                                                       \
    {                                                                                                                                                          \
    ...
    {                                                                                                                                                          \
        int diff = I420TestFilter(                                                                                                                             \
            width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);    \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    ...
    TEST_F(LibYUVScaleTest, DISABLED_##I420##name##From##width##x##height##_##filter##_12)                                                                     \
    {                                                                                                                                                          \
        int diff = I420TestFilter_12(                                                                                                                          \
            width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);    \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444##name##From##width##x##height##_##filter##_12)                                                                     \
    {                                                                                                                                                          \
        int diff = I444TestFilter_12(                                                                                                                          \
            width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);    \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I420##name##From##width##x##height##_##filter##_16)                                                                     \
    {                                                                                                                                                          \
        int diff = I420TestFilter_16(                                                                                                                          \
            width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);    \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444##name##From##width##x##height##_##filter##_16)                                                                     \
    {                                                                                                                                                          \
        int diff = I444TestFilter_16(                                                                                                                          \
            width, height, Abs(benchmark_width_), Abs(benchmark_height_), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);    \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
...
#define TEST_SCALESWAPXY1(DISABLED_, name, filter, max_diff)                                                                                                   \
    ...
    {                                                                                                                                                          \
        int diff = I420TestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,            \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, I444##name##SwapXY_##filter)                                                                                                       \
    {                                                                                                                                                          \
        int diff = I444TestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,            \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I420##name##SwapXY_##filter##_12)                                                                                       \
    {                                                                                                                                                          \
        int diff = I420TestFilter_12(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,         \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444##name##SwapXY_##filter##_12)                                                                                       \
    {                                                                                                                                                          \
        int diff = I444TestFilter_12(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,         \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I420##name##SwapXY_##filter##_16)                                                                                       \
    {                                                                                                                                                          \
        int diff = I420TestFilter_16(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,         \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, DISABLED_##I444##name##SwapXY_##filter##_16)                                                                                       \
    {                                                                                                                                                          \
        int diff = I444TestFilter_16(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,         \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }                                                                                                                                                          \
    TEST_F(LibYUVScaleTest, NV12##name##SwapXY_##filter)                                                                                                       \
    {                                                                                                                                                          \
        int diff = NV12TestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,            \
            disable_cpu_flags_, benchmark_cpu_info_);                                                                                                          \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }
...
extern "C" void ScaleRowUp2_16_NEON(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width);
extern "C" void ScaleRowUp2_16_C(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width);
...
extern "C" void ScaleRowDown2Box_16_NEON(const uint16_t* src_ptr, ptrdiff_t src_stride, uint16_t* dst, int dst_width);
    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    MaskCpuFlags(disable_cpu_flags); // Disable all CPU optimization.
    ScalePlane(src_y, src_stride_y, src_width, src_height, dst_y_8, dst_stride_y, dst_width, dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.

    for (i = 0; i < benchmark_iterations; ++i) {
        ScalePlane_16(p_src_y_16, src_stride_y, src_width, src_height, p_dst_y_16, dst_stride_y, dst_width, dst_height, f);
    }
...
#define TEST_FACTOR1(name, filter, nom, denom, max_diff)                                                                                                       \
    ...
    {                                                                                                                                                          \
        int diff = TestPlaneFilter_16(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                   \
            DX(benchmark_height_, nom, denom), kFilter##filter, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_);                               \
        EXPECT_LE(diff, max_diff);                                                                                                                             \
    }
...
TEST_F(LibYUVScaleTest, PlaneTest3x)
{
    ...
    int iterations160 = (benchmark_width_ * benchmark_height_ + (160 - 1)) / 160 * benchmark_iterations_;
    for (int i = 0; i < iterations160; ++i) {
        ScalePlane(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(225, dest_pixels[0]);

    ScalePlane(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterNone);
...
TEST_F(LibYUVScaleTest, PlaneTest4x)
{
    ...
    int iterations160 = (benchmark_width_ * benchmark_height_ + (160 - 1)) / 160 * benchmark_iterations_;
    for (int i = 0; i < iterations160; ++i) {
        ScalePlane(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(66, dest_pixels[0]);

    ScalePlane(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterNone);
...
TEST_F(LibYUVScaleTest, PlaneTestRotate_None)
{
    ...
    ScalePlane(
        orig_pixels, benchmark_width_, benchmark_width_, benchmark_height_, dest_c_pixels, benchmark_height_, benchmark_height_, benchmark_width_, kFilterNone);
    MaskCpuFlags(benchmark_cpu_info_); // Enable all CPU optimization.

    for (int i = 0; i < benchmark_iterations_; ++i) {
        ScalePlane(orig_pixels, benchmark_width_, benchmark_width_, benchmark_height_, dest_opt_pixels, benchmark_height_, benchmark_height_, benchmark_width_,
            kFilterNone);
    }
...
TEST_F(LibYUVScaleTest, PlaneTestRotate_Bilinear)
{
    ...
    MaskCpuFlags(disable_cpu_flags_); // Disable all CPU optimization.
    ScalePlane(orig_pixels, benchmark_width_, benchmark_width_, benchmark_height_, dest_c_pixels, benchmark_height_, benchmark_height_, benchmark_width_,
        kFilterBilinear);
    MaskCpuFlags(benchmark_cpu_info_); // Enable all CPU optimization.
...
TEST_F(LibYUVScaleTest, PlaneTestRotate_Box)
{
    ...
    MaskCpuFlags(disable_cpu_flags_); // Disable all CPU optimization.
    ScalePlane(
        orig_pixels, benchmark_width_, benchmark_width_, benchmark_height_, dest_c_pixels, benchmark_height_, benchmark_height_, benchmark_width_, kFilterBox);
    MaskCpuFlags(benchmark_cpu_info_); // Enable all CPU optimization.
...
TEST_F(LibYUVScaleTest, PlaneTest1_Box)
{
    ...
    libyuv::ScalePlane(orig_pixels + 1, /* src_stride= */ 1, /* src_width= */ 1,
        ...
        /* dst_width= */ 1, /* dst_height= */ 2, libyuv::kFilterBox);
...
TEST_F(LibYUVScaleTest, PlaneTest1_16_Box)
{
    ...
    libyuv::ScalePlane_16(orig_pixels + 1, /* src_stride= */ 1, /* src_width= */ 1,
        /* src_height= */ 1, dst_pixels, /* dst_stride= */ 1,
