...
static int TestAttenuateI(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    ARGBAttenuate(src_argb + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBAttenuate(src_argb + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestAttenuateI(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
...
{
    int max_diff = TestAttenuateI(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_LE(max_diff, EXPECTED_ATTENUATE_DIFF);
}

TEST_F(LibYUVPlanarTest, ARGBAttenuate_Invert)
{
    int max_diff = TestAttenuateI(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_LE(max_diff, EXPECTED_ATTENUATE_DIFF);
}
...
static int TestUnattenuateI(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    ARGBAttenuate(src_argb + off, kStride, src_argb + off, kStride, width, height);
    memset(dst_argb_c, 0, kStride * height);
    memset(dst_argb_opt, 0, kStride * height);

    MaskCpuFlags(disable_cpu_flags);
    ARGBUnattenuate(src_argb + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBUnattenuate(src_argb + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestUnattenuateI(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_LE(max_diff, EXPECTED_ATTENUATE_DIFF);
}

TEST_F(LibYUVPlanarTest, ARGBUnattenuate_Unaligned)
{
    int max_diff = TestUnattenuateI(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_LE(max_diff, EXPECTED_ATTENUATE_DIFF);
}

TEST_F(LibYUVPlanarTest, ARGBUnattenuate_Invert)
{
    int max_diff = TestUnattenuateI(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_LE(max_diff, EXPECTED_ATTENUATE_DIFF);
}
...
TEST_F(LibYUVPlanarTest, TestARGBComputeCumulativeSum)
{
    ...
    ARGBComputeCumulativeSum(&orig_pixels[0][0][0], 16 * 4, &added_pixels[0][0][0], 16 * 4, 16, 16);
...
TEST_F(LibYUVPlanarTest, TestARGBColorMatrix)
{
    ...
    SIMD_ALIGNED(static const int8_t kRGBToSepia[]) = {
        17 / 2, 68 / 2, 35 / 2, 0, 22 / 2, 88 / 2, 45 / 2, 0, 24 / 2, 98 / 2, 50 / 2, 0, 0, 0, 0, 64, // Copy alpha.
    };
    memset(orig_pixels, 0, sizeof(orig_pixels));
    ...
    // Do 16 to test asm version.
    ARGBColorMatrix(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, &kRGBToSepia[0], 16, 1);
    ...
    ARGBColorMatrix(&orig_pixels[0][0], 0, &dst_pixels_c[0][0], 0, &kRGBToSepia[0], 1280, 1);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        ARGBColorMatrix(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, &kRGBToSepia[0], 1280, 1);
    }
...
TEST_F(LibYUVPlanarTest, TestARGBQuantize)
{
    ...
    ARGBQuantize(&orig_pixels[0][0], 0, (65536 + (8 / 2)) / 8, 8, 8 / 2, 0, 0, 1280, 1);
...
TEST_F(LibYUVPlanarTest, ARGBMirror_Opt)
{
    ...
    align_buffer_page_end(dst_pixels_opt, benchmark_width_ * benchmark_height_ * 4);
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBMirror(src_pixels, benchmark_width_ * 4, dst_pixels_c, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        ARGBMirror(src_pixels, benchmark_width_ * 4, dst_pixels_opt, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MirrorPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    MirrorPlane(src_pixels, benchmark_width_, dst_pixels_c, benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        MirrorPlane(src_pixels, benchmark_width_, dst_pixels_opt, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MirrorUVPlane_Opt)
{
    ...
    align_buffer_page_end(dst_pixels_opt, benchmark_width_ * benchmark_height_ * 2);
    ...
    MaskCpuFlags(disable_cpu_flags_);
    MirrorUVPlane(src_pixels, benchmark_width_ * 2, dst_pixels_c, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        MirrorUVPlane(src_pixels, benchmark_width_ * 2, dst_pixels_opt, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, TestShade)
{
    ...
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        ARGBShade(&orig_pixels[0][0], 0, &shade_pixels[0][0], 0, 1280, 1, 0x80808080);
    }
}
...
TEST_F(LibYUVPlanarTest, TestARGBInterpolate)
{
    ...
    ARGBInterpolate(&orig_pixels_0[0][0], 0, &orig_pixels_1[0][0], 0, &interpolate_pixels[0][0], 0, 4, 1, 128);
    ...
    ARGBInterpolate(&orig_pixels_0[0][0], 0, &orig_pixels_1[0][0], 0, &interpolate_pixels[0][0], 0, 4, 1, 0);
    ...
    ARGBInterpolate(&orig_pixels_0[0][0], 0, &orig_pixels_1[0][0], 0, &interpolate_pixels[0][0], 0, 4, 1, 192);
    ...
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        ARGBInterpolate(&orig_pixels_0[0][0], 0, &orig_pixels_1[0][0], 0, &interpolate_pixels[0][0], 0, 1280, 1, 128);
    }
}
...
TEST_F(LibYUVPlanarTest, TestInterpolatePlane)
{
    ...
    InterpolatePlane(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 128);
    ...
    InterpolatePlane(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 0);
    ...
    InterpolatePlane(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 192);
    ...
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        InterpolatePlane(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 1280, 1, 123);
    }
}
...
TEST_F(LibYUVPlanarTest, TestInterpolatePlane_16)
{
    ...
    InterpolatePlane_16(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 128);
    EXPECT_EQ(8u, interpolate_pixels[0]);
    EXPECT_EQ(16u, interpolate_pixels[1]);
    EXPECT_EQ(32u, interpolate_pixels[2]);
    EXPECT_EQ(64u, interpolate_pixels[3]);
    ...
    InterpolatePlane_16(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 0);
    EXPECT_EQ(16u, interpolate_pixels[0]);
    EXPECT_EQ(32u, interpolate_pixels[1]);
    EXPECT_EQ(64u, interpolate_pixels[2]);
    EXPECT_EQ(128u, interpolate_pixels[3]);

    InterpolatePlane_16(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 16, 1, 192);
    ...
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        InterpolatePlane_16(&orig_pixels_0[0], 0, &orig_pixels_1[0], 0, &interpolate_pixels[0], 0, 1280, 1, 123);
    }
}
...
#define TESTTERP(FMT_A, BPP_A, STRIDE_A, FMT_B, BPP_B, STRIDE_B, W1280, TERP, N, NEG, OFF)                                                                     \
    ...
    {                                                                                                                                                          \
        ...
        MaskCpuFlags(disable_cpu_flags_);                                                                                                                      \
        ARGBInterpolate(src_argb_a + OFF, kStrideA, src_argb_b + OFF, kStrideA, dst_argb_c, kStrideB, kWidth, NEG kHeight, TERP);                              \
        MaskCpuFlags(benchmark_cpu_info_);                                                                                                                     \
        for (int i = 0; i < benchmark_iterations_; ++i) {                                                                                                      \
            ARGBInterpolate(src_argb_a + OFF, kStrideA, src_argb_b + OFF, kStrideA, dst_argb_opt, kStrideB, kWidth, NEG kHeight, TERP);                        \
        }                                                                                                                                                      \
...
static int TestBlend(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off, int attenuate)
{
    if (width < 1) {
        width = 1;
    }
    ...
    if (attenuate) {
        ARGBAttenuate(src_argb_a + off, kStride, src_argb_a + off, kStride, width, height);
    }
    memset(dst_argb_c, 255, kStride * height);
    memset(dst_argb_opt, 255, kStride * height);

    MaskCpuFlags(disable_cpu_flags);
    ARGBBlend(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBBlend(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestBlend(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBBlend_Unaligned)
{
    int max_diff = TestBlend(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBBlend_Invert)
{
    int max_diff = TestBlend(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBBlend_Unattenuated)
{
    int max_diff = TestBlend(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, 0);
    EXPECT_LE(max_diff, 1);
}
...
static void TestBlendPlane(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    BlendPlane(src_argb_a + off, width, src_argb_b + off, width, src_argb_alpha + off, width, dst_argb_opt + off, width, width, 1);
    for (int i = 0; i < width; ++i) {
    ...
    MaskCpuFlags(disable_cpu_flags);
    BlendPlane(src_argb_a + off, width, src_argb_b + off, width, src_argb_alpha + off, width, dst_argb_c + off, width, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        BlendPlane(src_argb_a + off, width, src_argb_b + off, width, src_argb_alpha + off, width, dst_argb_opt + off, width, width, invert * height);
    }
    for (int i = 0; i < kStride * height; ++i) {
...
{
    TestBlendPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
}
TEST_F(LibYUVPlanarTest, BlendPlane_Unaligned)
{
    TestBlendPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
}
TEST_F(LibYUVPlanarTest, BlendPlane_Any)
{
    TestBlendPlane(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
}
...
static void TestI420Blend(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    ...
    MaskCpuFlags(disable_cpu_flags);
    I420Blend(src_y0 + off, width, src_u0 + off, kStrideUV, src_v0 + off, kStrideUV, src_y1 + off, width, src_u1 + off, kStrideUV, src_v1 + off, kStrideUV,
        src_a + off, width, dst_y_c + off, width, dst_u_c + off, kStrideUV, dst_v_c + off, kStrideUV, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        I420Blend(src_y0 + off, width, src_u0 + off, kStrideUV, src_v0 + off, kStrideUV, src_y1 + off, width, src_u1 + off, kStrideUV, src_v1 + off, kStrideUV,
            src_a + off, width, dst_y_opt + off, width, dst_u_opt + off, kStrideUV, dst_v_opt + off, kStrideUV, width, invert * height);
    }
...
{
    TestI420Blend(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
}
TEST_F(LibYUVPlanarTest, I420Blend_Unaligned)
{
    TestI420Blend(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
}

// TODO(fbarchard): DISABLED because _Any uses C.  Avoid C and re-enable.
TEST_F(LibYUVPlanarTest, DISABLED_I420Blend_Any)
{
    TestI420Blend(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
}
...
TEST_F(LibYUVPlanarTest, TestAffine)
{
    ...
    ARGBAffineRow_C(&orig_pixels_0[0][0], 0, &interpolate_pixels_C[0][0], uv_step, 1280);
    ...
    ARGBAffineRow_SSE2(&orig_pixels_0[0][0], 0, &interpolate_pixels_Opt[0][0], uv_step, 1280);
...
TEST_F(LibYUVPlanarTest, TestDetilePlane)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    for (j = 0; j < benchmark_iterations_; j++) {
        DetilePlane(orig_y, orig_width, dst_c, benchmark_width_, benchmark_width_, benchmark_height_, 16);
    }

    // Enable optimizations.
    MaskCpuFlags(benchmark_cpu_info_);
    for (j = 0; j < benchmark_iterations_; j++) {
        DetilePlane(orig_y, orig_width, dst_opt, benchmark_width_, benchmark_width_, benchmark_height_, 16);
    }
...
TEST_F(LibYUVPlanarTest, TestDetileSplitUVPlane_Correctness)
{
    ...
    DetileSplitUVPlane(
        orig_uv, orig_width, dst_u_opt, (benchmark_width_ + 1) / 2, dst_v_opt, (benchmark_width_ + 1) / 2, benchmark_width_, benchmark_height_, 16);

    // Benchmark 2 step conversion for comparison.
    for (j = 0; j < benchmark_iterations_; j++) {
        DetilePlane(orig_uv, orig_width, detiled_uv, benchmark_width_, benchmark_width_, benchmark_height_, 16);
        SplitUVPlane(detiled_uv, orig_width, dst_u_two_stage, (benchmark_width_ + 1) / 2, dst_v_two_stage, (benchmark_width_ + 1) / 2,
            (benchmark_width_ + 1) / 2, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, TestDetileSplitUVPlane_Benchmark)
{
    ...
    DetileSplitUVPlane(orig_uv, orig_width, dst_u_c, (benchmark_width_ + 1) / 2, dst_v_c, (benchmark_width_ + 1) / 2, benchmark_width_, benchmark_height_, 16);
...
static int TestMultiply(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBMultiply(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBMultiply(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestMultiply(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBMultiply_Unaligned)
{
    int max_diff = TestMultiply(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBMultiply_Invert)
{
    int max_diff = TestMultiply(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_LE(max_diff, 1);
}
...
static int TestAdd(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBAdd(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBAdd(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestAdd(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBAdd_Unaligned)
{
    int max_diff = TestAdd(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBAdd_Invert)
{
    int max_diff = TestAdd(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_LE(max_diff, 1);
}
...
static int TestSubtract(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBSubtract(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBSubtract(src_argb_a + off, kStride, src_argb_b + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestSubtract(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBSubtract_Unaligned)
{
    int max_diff = TestSubtract(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, ARGBSubtract_Invert)
{
    int max_diff = TestSubtract(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_LE(max_diff, 1);
}
...
static int TestSobel(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBSobel(src_argb_a + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBSobel(src_argb_a + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestSobel(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobel_Unaligned)
{
    int max_diff = TestSobel(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobel_Invert)
{
    int max_diff = TestSobel(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_EQ(0, max_diff);
}
...
static int TestSobelToPlane(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBSobelToPlane(src_argb_a + off, kSrcStride, dst_argb_c, kDstStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBSobelToPlane(src_argb_a + off, kSrcStride, dst_argb_opt, kDstStride, width, invert * height);
    }
    int max_diff = 0;
...
{
    int max_diff = TestSobelToPlane(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobelToPlane_Unaligned)
{
    int max_diff = TestSobelToPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobelToPlane_Invert)
{
    int max_diff = TestSobelToPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_EQ(0, max_diff);
}
...
static int TestSobelXY(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBSobelXY(src_argb_a + off, kStride, dst_argb_c, kStride, width, invert * height);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBSobelXY(src_argb_a + off, kStride, dst_argb_opt, kStride, width, invert * height);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestSobelXY(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobelXY_Unaligned)
{
    int max_diff = TestSobelXY(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBSobelXY_Invert)
{
    int max_diff = TestSobelXY(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0);
    EXPECT_EQ(0, max_diff);
}
...
static int TestBlur(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off, int radius)
{
    if (width < 1) {
        width = 1;
    }
    ...
    MaskCpuFlags(disable_cpu_flags);
    ARGBBlur(src_argb_a + off, kStride, dst_argb_c, kStride, reinterpret_cast<int32_t*>(dst_cumsum), width * 4, width, invert * height, radius);
    MaskCpuFlags(benchmark_cpu_info);
    for (int i = 0; i < benchmark_iterations; ++i) {
        ARGBBlur(src_argb_a + off, kStride, dst_argb_opt, kStride, reinterpret_cast<int32_t*>(dst_cumsum), width * 4, width, invert * height, radius);
    }
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i]) - static_cast<int>(dst_argb_opt[i]));
...
{
    int max_diff = TestBlur(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, kBlurSize);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, DISABLED_ARM(ARGBBlur_Unaligned))
{
    int max_diff = TestBlur(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1, kBlurSize);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, DISABLED_ARM(ARGBBlur_Invert))
{
    int max_diff = TestBlur(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0, kBlurSize);
    EXPECT_LE(max_diff, 1);
}
...
{
    int max_diff = TestBlur(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, kBlurSmallSize);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, DISABLED_ARM(ARGBBlurSmall_Unaligned))
{
    int max_diff = TestBlur(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1, kBlurSmallSize);
    EXPECT_LE(max_diff, 1);
}

TEST_F(LibYUVPlanarTest, DISABLED_ARM(ARGBBlurSmall_Invert))
{
    int max_diff = TestBlur(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0, kBlurSmallSize);
    EXPECT_LE(max_diff, 1);
}
...
TEST_F(LibYUVPlanarTest, DISABLED_ARM(TestARGBPolynomial))
{
    ...
    // Do 16 to test asm version.
    ARGBPolynomial(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, &kWarmifyPolynomial[0], 16, 1);
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBPolynomial(&orig_pixels[0][0], 0, &dst_pixels_c[0][0], 0, &kWarmifyPolynomial[0], 1280, 1);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        ARGBPolynomial(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, &kWarmifyPolynomial[0], 1280, 1);
    }
...
int TestHalfFloatPlane(
    int benchmark_width, int benchmark_height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, float scale, int mask)
{
    int i, j;
    ...
    for (j = 0; j < benchmark_iterations; j++) {
        HalfFloatPlane(reinterpret_cast<uint16_t*>(orig_y), benchmark_width * 2, reinterpret_cast<uint16_t*>(dst_c), benchmark_width * 2, scale,
    ...
    MaskCpuFlags(benchmark_cpu_info);
    for (j = 0; j < benchmark_iterations; j++) {
        HalfFloatPlane(reinterpret_cast<uint16_t*>(orig_y), benchmark_width * 2, reinterpret_cast<uint16_t*>(dst_opt), benchmark_width * 2, scale,
            benchmark_width, benchmark_height);
    }

    int max_diff = 0;
    for (i = 0; i < y_plane_size / 2; ++i) {
        int abs_diff = abs(static_cast<int>(reinterpret_cast<uint16_t*>(dst_c)[i]) - static_cast<int>(reinterpret_cast<uint16_t*>(dst_opt)[i]));
        if (abs_diff > max_diff) {
            max_diff = abs_diff;
        }
    }
...
#endif
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 65536.0f, 65535);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_16bit_One)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f, 65535);
    EXPECT_LE(diff, 1);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_16bit_Opt)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 4096.0f, 65535);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_10bit_Opt)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 1024.0f, 1023);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_9bit_Opt)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 512.0f, 511);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_Opt)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 4096.0f, 4095);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_Offby1)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f / 4095.0f, 4095);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_One)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f, 2047);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestHalfFloatPlane_12bit_One)
{
    int diff = TestHalfFloatPlane(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f, 4095);
    EXPECT_LE(diff, 1);
}

float TestByteToFloat(int benchmark_width, int benchmark_height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, float scale)
{
    int i, j;
...
{
    float diff = TestByteToFloat(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, 1.0f);
    EXPECT_EQ(0.f, diff);
}
...
TEST_F(LibYUVPlanarTest, TestARGBLumaColorTable)
{
    ...
    orig_pixels[3][3] = 224u;
    // Do 16 to test asm version.
    ARGBLumaColorTable(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, &lumacolortable[0], 16, 1);
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBLumaColorTable(&orig_pixels[0][0], 0, &dst_pixels_c[0][0], 0, lumacolortable, 1280, 1);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
        ARGBLumaColorTable(&orig_pixels[0][0], 0, &dst_pixels_opt[0][0], 0, lumacolortable, 1280, 1);
    }
    for (int i = 0; i < 1280; ++i) {
        EXPECT_EQ(dst_pixels_c[i][0], dst_pixels_opt[i][0]);
        EXPECT_EQ(dst_pixels_c[i][1], dst_pixels_opt[i][1]);
...
TEST_F(LibYUVPlanarTest, TestARGBCopyAlpha)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBCopyAlpha(orig_pixels, benchmark_width_ * 4, dst_pixels_c, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        ARGBCopyAlpha(orig_pixels, benchmark_width_ * 4, dst_pixels_opt, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, TestARGBExtractAlpha)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBExtractAlpha(src_pixels, benchmark_width_ * 4, dst_pixels_c, benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        ARGBExtractAlpha(src_pixels, benchmark_width_ * 4, dst_pixels_opt, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, TestARGBCopyYToAlpha)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    ARGBCopyYToAlpha(orig_pixels, benchmark_width_, dst_pixels_c, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        ARGBCopyYToAlpha(orig_pixels, benchmark_width_, dst_pixels_opt, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    }
...
static int TestARGBRect(int width, int height, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info, int invert, int off, int bpp)
{
    if (width < 1) {
        width = 1;
    }
    ...
    int max_diff = 0;
    for (int i = 0; i < kStride * height; ++i) {
        int abs_diff = abs(static_cast<int>(dst_argb_c[i + off]) - static_cast<int>(dst_argb_opt[i + off]));
        if (abs_diff > max_diff) {
            max_diff = abs_diff;
        }
    }
...
{
    int max_diff = TestARGBRect(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, 4);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBRect_Unaligned)
{
    int max_diff = TestARGBRect(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1, 4);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, ARGBRect_Invert)
{
    int max_diff = TestARGBRect(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0, 4);
    EXPECT_EQ(0, max_diff);
}
...
{
    int max_diff = TestARGBRect(benchmark_width_ + 1, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 0, 1);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, SetPlane_Unaligned)
{
    int max_diff = TestARGBRect(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, +1, 1, 1);
    EXPECT_EQ(0, max_diff);
}

TEST_F(LibYUVPlanarTest, SetPlane_Invert)
{
    int max_diff = TestARGBRect(benchmark_width_, benchmark_height_, benchmark_iterations_, disable_cpu_flags_, benchmark_cpu_info_, -1, 0, 1);
    EXPECT_EQ(0, max_diff);
}
...
TEST_F(LibYUVPlanarTest, MergeUVPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    MergeUVPlane(src_pixels_u, benchmark_width_, src_pixels_v, benchmark_width_, dst_pixels_c, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        MergeUVPlane(src_pixels_u, benchmark_width_, src_pixels_v, benchmark_width_, dst_pixels_opt, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MergeUVPlane_16_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    MergeUVPlane_16((const uint16_t*)src_pixels_u, benchmark_width_, (const uint16_t*)src_pixels_v, benchmark_width_, (uint16_t*)dst_pixels_c,
        benchmark_width_ * 2, benchmark_width_, benchmark_height_, 12);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        MergeUVPlane_16((const uint16_t*)src_pixels_u, benchmark_width_, (const uint16_t*)src_pixels_v, benchmark_width_, (uint16_t*)dst_pixels_opt,
            benchmark_width_ * 2, benchmark_width_, benchmark_height_, 12);
    }
...
TEST_F(LibYUVPlanarTest, SplitUVPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SplitUVPlane(src_pixels, benchmark_width_ * 2, dst_pixels_u_c, benchmark_width_, dst_pixels_v_c, benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        SplitUVPlane(
            src_pixels, benchmark_width_ * 2, dst_pixels_u_opt, benchmark_width_, dst_pixels_v_opt, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, SplitUVPlane_16_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SplitUVPlane_16((const uint16_t*)src_pixels, benchmark_width_ * 2, (uint16_t*)dst_pixels_u_c, benchmark_width_, (uint16_t*)dst_pixels_v_c, benchmark_width_,
        benchmark_width_, benchmark_height_, 10);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        SplitUVPlane_16((const uint16_t*)src_pixels, benchmark_width_ * 2, (uint16_t*)dst_pixels_u_opt, benchmark_width_, (uint16_t*)dst_pixels_v_opt,
            benchmark_width_, benchmark_width_, benchmark_height_, 10);
    }
...
TEST_F(LibYUVPlanarTest, SwapUVPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SwapUVPlane(src_pixels, benchmark_width_ * 2, dst_pixels_c, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        SwapUVPlane(src_pixels, benchmark_width_ * 2, dst_pixels_opt, benchmark_width_ * 2, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MergeRGBPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SplitRGBPlane(src_pixels, benchmark_width_ * 3, tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_,
        benchmark_width_, benchmark_height_);
    MergeRGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, dst_pixels_c, benchmark_width_ * 3,
        benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        MergeRGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, dst_pixels_opt, benchmark_width_ * 3,
            benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MergeARGBPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SplitARGBPlane(src_pixels, benchmark_width_ * 4, tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_,
        tmp_pixels_a, benchmark_width_, benchmark_width_, benchmark_height_);
    MergeARGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, tmp_pixels_a, benchmark_width_, dst_pixels_c,
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        MergeARGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, tmp_pixels_a, benchmark_width_,
            dst_pixels_opt, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, MergeXRGBPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    SplitARGBPlane(src_pixels, benchmark_width_ * 4, tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, NULL, 0,
        benchmark_width_, benchmark_height_);
    MergeARGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, NULL, 0, dst_pixels_c, benchmark_width_ * 4,
        benchmark_width_, benchmark_height_);
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        MergeARGBPlane(tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, NULL, 0, dst_pixels_opt,
            benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, SplitXRGBPlane_Opt)
{
    ...
    MaskCpuFlags(benchmark_cpu_info_);
    for (int i = 0; i < benchmark_iterations_; ++i) {
        SplitARGBPlane(src_pixels, benchmark_width_ * 4, tmp_pixels_r, benchmark_width_, tmp_pixels_g, benchmark_width_, tmp_pixels_b, benchmark_width_, NULL,
            0, benchmark_width_, benchmark_height_);
    }
...
#define TESTQPLANARTOPI(FUNC, STYPE, DTYPE, DEPTH, W1280, N, NEG, OFF)                                                                                         \
    ...
    {                                                                                                                                                          \
        ...
        MaskCpuFlags(disable_cpu_flags_);                                                                                                                      \
        FUNC##Plane(src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, src_pixels_a, kWidth, dst_pixels_c, kWidth * 4, kWidth,                  \
            NEG benchmark_height_, DEPTH);                                                                                                                     \
        MaskCpuFlags(benchmark_cpu_info_);                                                                                                                     \
        for (int i = 0; i < benchmark_iterations_; ++i) {                                                                                                      \
            FUNC##Plane(src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, src_pixels_a, kWidth, dst_pixels_opt, kWidth * 4, kWidth,            \
                NEG benchmark_height_, DEPTH);                                                                                                                 \
        }                                                                                                                                                      \
        for (int i = 0; i < kPixels * 4; ++i) {                                                                                                                \
            EXPECT_EQ(dst_pixels_c[i], dst_pixels_opt[i]);                                                                                                     \
...
#define TESTQPLANAROTOPI(FUNC, STYPE, DTYPE, DEPTH, W1280, N, NEG, OFF)                                                                                        \
    ...
    {                                                                                                                                                          \
        ...
        MaskCpuFlags(disable_cpu_flags_);                                                                                                                      \
        FUNC##Plane(                                                                                                                                           \
            src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, NULL, 0, dst_pixels_c, kWidth * 4, kWidth, NEG benchmark_height_, DEPTH);        \
        MaskCpuFlags(benchmark_cpu_info_);                                                                                                                     \
        for (int i = 0; i < benchmark_iterations_; ++i) {                                                                                                      \
            FUNC##Plane(                                                                                                                                       \
                src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, NULL, 0, dst_pixels_opt, kWidth * 4, kWidth, NEG benchmark_height_, DEPTH);  \
        }                                                                                                                                                      \
        for (int i = 0; i < kPixels * 4; ++i) {                                                                                                                \
            EXPECT_EQ(dst_pixels_c[i], dst_pixels_opt[i]);                                                                                                     \
        }                                                                                                                                                      \
...
#define TESTQPLANARTOP(FUNC, STYPE, DTYPE, DEPTH)                                                                                                              \
    ...
    TESTQPLANARTOPI(FUNC, STYPE, DTYPE, DEPTH, benchmark_width_, _Unaligned, +, 2)                                                                             \
    ...
    TESTQPLANAROTOPI(FUNC, STYPE, DTYPE, DEPTH, benchmark_width_ + 1, _Any, +, 0)                                                                              \
    TESTQPLANAROTOPI(FUNC, STYPE, DTYPE, DEPTH, benchmark_width_, _Unaligned, +, 2)                                                                            \
...
#define TESTTPLANARTOPI(FUNC, STYPE, DTYPE, DEPTH, W1280, N, NEG, OFF)                                                                                         \
    ...
    {                                                                                                                                                          \
        ...
        MaskCpuFlags(disable_cpu_flags_);                                                                                                                      \
        FUNC##Plane(src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, dst_pixels_c, kWidth * 4, kWidth, NEG benchmark_height_, DEPTH);         \
        MaskCpuFlags(benchmark_cpu_info_);                                                                                                                     \
        for (int i = 0; i < benchmark_iterations_; ++i) {                                                                                                      \
            FUNC##Plane(src_pixels_r, kWidth, src_pixels_g, kWidth, src_pixels_b, kWidth, dst_pixels_opt, kWidth * 4, kWidth, NEG benchmark_height_, DEPTH);   \
        }                                                                                                                                                      \
        for (int i = 0; i < kPixels * 4; ++i) {                                                                                                                \
            EXPECT_EQ(dst_pixels_c[i], dst_pixels_opt[i]);                                                                                                     \
        }                                                                                                                                                      \
...
#define TESTTPLANARTOP(FUNC, STYPE, DTYPE, DEPTH)                                                                                                              \
    ...
    TESTTPLANARTOPI(FUNC, STYPE, DTYPE, DEPTH, benchmark_width_, _Unaligned, +, 2)                                                                             \
...
TEST_F(LibYUVPlanarTest, MergeUVRow_16_Opt)
{
    ...
    MergeUVRow_16_C(reinterpret_cast<const uint16_t*>(src_pixels_u), reinterpret_cast<const uint16_t*>(src_pixels_v),
        reinterpret_cast<uint16_t*>(dst_pixels_uv_c), 16, kPixels);

    int has_avx2 = TestCpuFlag(kCpuHasAVX2);
    for (int i = 0; i < benchmark_iterations_; ++i) {
        if (has_avx2) {
            MergeUVRow_16_AVX2(reinterpret_cast<const uint16_t*>(src_pixels_u), reinterpret_cast<const uint16_t*>(src_pixels_v),
                reinterpret_cast<uint16_t*>(dst_pixels_uv_opt), 16, kPixels);
        } else {
            MergeUVRow_16_C(reinterpret_cast<const uint16_t*>(src_pixels_u), reinterpret_cast<const uint16_t*>(src_pixels_v),
                reinterpret_cast<uint16_t*>(dst_pixels_uv_opt), 16, kPixels);
        }
...
TEST_F(LibYUVPlanarTest, MultiplyRow_16_Opt)
{
    ...
    MultiplyRow_16_C(reinterpret_cast<const uint16_t*>(src_pixels_y), reinterpret_cast<uint16_t*>(dst_pixels_y_c), 64, kPixels);

    int has_avx2 = TestCpuFlag(kCpuHasAVX2);
    for (int i = 0; i < benchmark_iterations_; ++i) {
        if (has_avx2) {
            MultiplyRow_16_AVX2(reinterpret_cast<const uint16_t*>(src_pixels_y), reinterpret_cast<uint16_t*>(dst_pixels_y_opt), 64, kPixels);
        } else {
            MultiplyRow_16_C(reinterpret_cast<const uint16_t*>(src_pixels_y), reinterpret_cast<uint16_t*>(dst_pixels_y_opt), 64, kPixels);
        }
    }
...
TEST_F(LibYUVPlanarTest, Convert16To8Plane)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    Convert16To8Plane(
        reinterpret_cast<const uint16_t*>(src_pixels_y), benchmark_width_, dst_pixels_y_c, benchmark_width_, 16384, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        Convert16To8Plane(
            reinterpret_cast<const uint16_t*>(src_pixels_y), benchmark_width_, dst_pixels_y_opt, benchmark_width_, 16384, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, YUY2ToY)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    YUY2ToY(src_pixels_y, benchmark_width_ * 2, dst_pixels_y_c, benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        YUY2ToY(src_pixels_y, benchmark_width_ * 2, dst_pixels_y_opt, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, UYVYToY)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    UYVYToY(src_pixels_y, benchmark_width_ * 2, dst_pixels_y_c, benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        UYVYToY(src_pixels_y, benchmark_width_ * 2, dst_pixels_y_opt, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, Convert16To8Row_Opt)
{
    ...
    Convert16To8Row_C(reinterpret_cast<const uint16_t*>(src_pixels_y), dst_pixels_y_c, 16384, kPixels);

    int has_avx2 = TestCpuFlag(kCpuHasAVX2);
    int has_ssse3 = TestCpuFlag(kCpuHasSSSE3);
    for (int i = 0; i < benchmark_iterations_; ++i) {
        if (has_avx2) {
            Convert16To8Row_AVX2(reinterpret_cast<const uint16_t*>(src_pixels_y), dst_pixels_y_opt, 16384, kPixels);
        } else if (has_ssse3) {
            Convert16To8Row_SSSE3(reinterpret_cast<const uint16_t*>(src_pixels_y), dst_pixels_y_opt, 16384, kPixels);
        } else {
            Convert16To8Row_C(reinterpret_cast<const uint16_t*>(src_pixels_y), dst_pixels_y_opt, 16384, kPixels);
        }
    }
...
TEST_F(LibYUVPlanarTest, Convert8To16Plane)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    Convert8To16Plane(src_pixels_y, benchmark_width_, reinterpret_cast<uint16_t*>(dst_pixels_y_c), benchmark_width_, 1024, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        Convert8To16Plane(
            src_pixels_y, benchmark_width_, reinterpret_cast<uint16_t*>(dst_pixels_y_opt), benchmark_width_, 1024, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, Convert8To16Row_Opt)
{
    ...
    Convert8To16Row_C(src_pixels_y, reinterpret_cast<uint16_t*>(dst_pixels_y_c), 1024, kPixels);

    int has_avx2 = TestCpuFlag(kCpuHasAVX2);
    int has_sse2 = TestCpuFlag(kCpuHasSSE2);
    for (int i = 0; i < benchmark_iterations_; ++i) {
        if (has_avx2) {
            Convert8To16Row_AVX2(src_pixels_y, reinterpret_cast<uint16_t*>(dst_pixels_y_opt), 1024, kPixels);
        } else if (has_sse2) {
            Convert8To16Row_SSE2(src_pixels_y, reinterpret_cast<uint16_t*>(dst_pixels_y_opt), 1024, kPixels);
        } else {
            Convert8To16Row_C(src_pixels_y, reinterpret_cast<uint16_t*>(dst_pixels_y_opt), 1024, kPixels);
        }
    }
...
float TestScaleMaxSamples(int benchmark_width, int benchmark_height, int benchmark_iterations, float scale, bool opt)
{
    int i, j;
    ...
    max_c = ScaleMaxSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_c), scale, kPixels);

    for (j = 0; j < benchmark_iterations; j++) {
        if (opt) {
#ifdef HAS_SCALESUMSAMPLES_NEON
            max_opt = ScaleMaxSamples_NEON(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#else
            max_opt = ScaleMaxSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#endif
        } else {
            max_opt = ScaleMaxSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
        }
    ...
    for (i = 0; i < kPixels; ++i) {
        float abs_diff = FAbs((reinterpret_cast<float*>(dst_c)[i]) - (reinterpret_cast<float*>(dst_opt)[i]));
        if (abs_diff > max_diff) {
            max_diff = abs_diff;
        }
    }
...
{
    float diff = TestScaleMaxSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, false);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestScaleMaxSamples_Opt)
{
    float diff = TestScaleMaxSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, true);
    EXPECT_EQ(0, diff);
}

float TestScaleSumSamples(int benchmark_width, int benchmark_height, int benchmark_iterations, float scale, bool opt)
{
    int i, j;
    ...
    sum_c = ScaleSumSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_c), scale, kPixels);

    for (j = 0; j < benchmark_iterations; j++) {
        if (opt) {
#ifdef HAS_SCALESUMSAMPLES_NEON
            sum_opt = ScaleSumSamples_NEON(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#else
            sum_opt = ScaleSumSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#endif
        } else {
            sum_opt = ScaleSumSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
        }
...
{
    float diff = TestScaleSumSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, false);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestScaleSumSamples_Opt)
{
    float diff = TestScaleSumSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, true);
    EXPECT_EQ(0, diff);
}

float TestScaleSamples(int benchmark_width, int benchmark_height, int benchmark_iterations, float scale, bool opt)
{
    int i, j;
    // NEON does multiple of 8, so round count up
    const int kPixels = (benchmark_width * benchmark_height + 7) & ~7;
    ...
    ScaleSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_c), scale, kPixels);

    for (j = 0; j < benchmark_iterations; j++) {
        if (opt) {
#ifdef HAS_SCALESUMSAMPLES_NEON
            ScaleSamples_NEON(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#else
            ScaleSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
#endif
        } else {
            ScaleSamples_C(reinterpret_cast<float*>(orig_y), reinterpret_cast<float*>(dst_opt), scale, kPixels);
        }
...
{
    float diff = TestScaleSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, false);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestScaleSamples_Opt)
{
    float diff = TestScaleSamples(benchmark_width_, benchmark_height_, benchmark_iterations_, 1.2f, true);
    EXPECT_EQ(0, diff);
}

float TestCopySamples(int benchmark_width, int benchmark_height, int benchmark_iterations, bool opt)
{
    int i, j;
    ...
    memcpy(reinterpret_cast<void*>(dst_c), reinterpret_cast<void*>(orig_y), kPixels * 4);
...
{
    float diff = TestCopySamples(benchmark_width_, benchmark_height_, benchmark_iterations_, false);
    EXPECT_EQ(0, diff);
}

TEST_F(LibYUVPlanarTest, TestCopySamples_Opt)
{
    float diff = TestCopySamples(benchmark_width_, benchmark_height_, benchmark_iterations_, true);
    EXPECT_EQ(0, diff);
}
...
TEST_F(LibYUVPlanarTest, TestGaussRow_Opt)
{
    ...
    EXPECT_EQ(dst_pixels_c[0], static_cast<uint16_t>(0 * 1 + 1 * 4 + 2 * 6 + 3 * 4 + 4 * 1));
    EXPECT_EQ(dst_pixels_c[639], static_cast<uint16_t>(10256));
}

extern "C" void GaussCol_NEON(
    const uint16_t* src0, const uint16_t* src1, const uint16_t* src2, const uint16_t* src3, const uint16_t* src4, uint32_t* dst, int width);

extern "C" void GaussCol_C(
    const uint16_t* src0, const uint16_t* src1, const uint16_t* src2, const uint16_t* src3, const uint16_t* src4, uint32_t* dst, int width);
...
TEST_F(LibYUVPlanarTest, TestGaussCol_Opt)
{
    ...
    GaussCol_C(&orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_c[0], 1280);
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
#if !defined(LIBYUV_DISABLE_NEON) && (defined(__aarch64__) || defined(__ARM_NEON__) || defined(LIBYUV_NEON))
        int has_neon = TestCpuFlag(kCpuHasNEON);
        if (has_neon) {
            GaussCol_NEON(
                &orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_opt[0], 1280);
        } else {
            GaussCol_C(&orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_opt[0], 1280);
        }
#else
        GaussCol_C(&orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_opt[0], 1280);
#endif
...
TEST_F(LibYUVPlanarTest, TestGaussCol_F32_Opt)
{
    ...
    GaussCol_F32_C(&orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_c[0], 1280);
    for (int i = 0; i < benchmark_pixels_div1280_; ++i) {
#if !defined(LIBYUV_DISABLE_NEON) && defined(__aarch64__)
        int has_neon = TestCpuFlag(kCpuHasNEON);
        if (has_neon) {
...
#else
        GaussCol_F32_C(&orig_pixels[0], &orig_pixels[1280], &orig_pixels[1280 * 2], &orig_pixels[1280 * 3], &orig_pixels[1280 * 4], &dst_pixels_opt[0], 1280);
#endif
    }
...
{
    const int kPixels = benchmark_width_ * benchmark_height_;
    void (*SwapUVRow)(const uint8_t* src_uv, uint8_t* dst_vu, int width) = SwapUVRow_C;
...
TEST_F(LibYUVPlanarTest, TestGaussPlane_F32)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    GaussPlane_F32((const float*)(orig_pixels), benchmark_width_, (float*)(dst_pixels_c), benchmark_width_, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        GaussPlane_F32((const float*)(orig_pixels), benchmark_width_, (float*)(dst_pixels_opt), benchmark_width_, benchmark_width_, benchmark_height_);
    }
    for (int i = 0; i < benchmark_width_ * benchmark_height_; ++i) {
        EXPECT_NEAR(((float*)(dst_pixels_c))[i], ((float*)(dst_pixels_opt))[i], 1.f) << i;
    }
...
TEST_F(LibYUVPlanarTest, HalfMergeUVPlane_Opt)
{
    ...
    MaskCpuFlags(disable_cpu_flags_);
    HalfMergeUVPlane(src_pixels_u, benchmark_width_, src_pixels_v, benchmark_width_, dst_pixels_uv_c, dst_width * 2, benchmark_width_, benchmark_height_);
    MaskCpuFlags(benchmark_cpu_info_);

    for (int i = 0; i < benchmark_iterations_; ++i) {
        HalfMergeUVPlane(src_pixels_u, benchmark_width_, src_pixels_v, benchmark_width_, dst_pixels_uv_opt, dst_width * 2, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, NV12Copy)
{
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        NV12Copy(src_y, benchmark_width_, src_uv, halfwidth * 2, dst_y, benchmark_width_, dst_uv, halfwidth * 2, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVPlanarTest, NV21Copy)
{
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        NV21Copy(src_y, benchmark_width_, src_vu, halfwidth * 2, dst_y, benchmark_width_, dst_vu, halfwidth * 2, benchmark_width_, benchmark_height_);
    }
