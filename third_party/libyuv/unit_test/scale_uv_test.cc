    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    int64_t src_uv_plane_size = (Abs(src_width) + b * 2) * (Abs(src_height) + b * 2) * 2LL;
    ...
    UVScale(src_uv + (src_stride_uv * b) + b * 2, src_stride_uv, src_width, src_height, dst_uv_c + (dst_stride_uv * b) + b * 2, dst_stride_uv, dst_width,
        dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    UVScale(src_uv + (src_stride_uv * b) + b * 2, src_stride_uv, src_width, src_height, dst_uv_opt + (dst_stride_uv * b) + b * 2, dst_stride_uv, dst_width,
        dst_height, f);
...
#define TEST_FACTOR1(name, filter, nom, denom, max_diff)                                                                                                       \
    ...
    {                                                                                                                                                          \
        int diff = UVTestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                         \
...
#define TEST_SCALESWAPXY1(name, filter, max_diff)                                                                                                              \
    ...
    {                                                                                                                                                          \
        int diff = UVTestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,              \
...
TEST_F(LibYUVScaleTest, UVTest3x)
{
    ...
    for (int i = 0; i < iterations160; ++i) {
        UVScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(225, dest_pixels[0]);
    EXPECT_EQ(255 - 225, dest_pixels[1]);

    UVScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterNone);
...
TEST_F(LibYUVScaleTest, UVTest4x)
{
    ...
    int iterations160 = (benchmark_width_ * benchmark_height_ + (160 - 1)) / 160 * benchmark_iterations_;
    for (int i = 0; i < iterations160; ++i) {
        UVScale(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(66, dest_pixels[0]);
    EXPECT_EQ(190, dest_pixels[1]);

    UVScale(orig_pixels, kSrcStride, 64, 4, dest_pixels, kDstStride, 16, 1, kFilterNone);
