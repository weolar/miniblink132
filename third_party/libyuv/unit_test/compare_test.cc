...
TEST_F(LibYUVCompareTest, BenchmarkARGBDetect_Opt)
{
    ...
    src_a[0] = 0;
    fourcc = ARGBDetect(src_a, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
...
TEST_F(LibYUVCompareTest, BenchmarkARGBDetect_Unaligned)
{
    ...
    src_a[0 + 1] = 0;
    fourcc = ARGBDetect(src_a + 1, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
    EXPECT_EQ(static_cast<uint32_t>(libyuv::FOURCC_BGRA), fourcc);
    src_a[0 + 1] = 255;
    src_a[3 + 1] = 0;
    fourcc = ARGBDetect(src_a + 1, benchmark_width_ * 4, benchmark_width_, benchmark_height_);
...
TEST_F(LibYUVCompareTest, TestHammingDistance_Opt)
{
    ...
    if (kMaxWidth <= kMaxOptCount) {
        ...
        if (kMaxWidth * 8ULL != static_cast<uint64_t>(h1)) {
            printf("warning - HammingDistance_Opt %u does not match %llu "
                   "but length of %u is longer than guaranteed.\n",
                h1, kMaxWidth * 8ULL, kMaxWidth);
        } else {
            printf("warning - HammingDistance_Opt %u matches but length of %u "
...
TEST_F(LibYUVCompareTest, TestHammingDistance)
{
    ...
    uint64_t h1 = 0;
    for (int i = 0; i < benchmark_iterations_; ++i) {
        h1 = ComputeHammingDistance(src_a, src_b, benchmark_width_ * benchmark_height_);
    }
...
TEST_F(LibYUVCompareTest, BenchmarkPsnr_Opt)
{
    ...
    for (int i = 0; i < benchmark_iterations_; ++i) {
        CalcFramePsnr(src_a, benchmark_width_, src_b, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVCompareTest, BenchmarkPsnr_Unaligned)
{
    ...
    double opt_time = get_time();
    for (int i = 0; i < benchmark_iterations_; ++i) {
        CalcFramePsnr(src_a + 1, benchmark_width_, src_b, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVCompareTest, Psnr)
{
    ...
    double err;
    err = CalcFramePsnr(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);
    ...
    c_err = CalcFramePsnr(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);

    MaskCpuFlags(benchmark_cpu_info_);

    opt_err = CalcFramePsnr(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);
...
TEST_F(LibYUVCompareTest, DISABLED_BenchmarkSsim_Opt)
{
    ...
    double opt_time = get_time();
    for (int i = 0; i < benchmark_iterations_; ++i) {
        CalcFrameSsim(src_a, benchmark_width_, src_b, benchmark_width_, benchmark_width_, benchmark_height_);
    }
...
TEST_F(LibYUVCompareTest, Ssim)
{
    ...
    double err;
    err = CalcFrameSsim(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);
    ...
    c_err = CalcFrameSsim(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);

    MaskCpuFlags(benchmark_cpu_info_);

    opt_err = CalcFrameSsim(src_a + kSrcStride * b + b, kSrcStride, src_b + kSrcStride * b + b, kSrcStride, kSrcWidth, kSrcHeight);
