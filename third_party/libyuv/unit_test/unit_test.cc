...
ABSL_FLAG(int32_t, libyuv_flags, 0, "cpu flags for reference code. 1 = C, -1 = SIMD");
ABSL_FLAG(int32_t, libyuv_cpu_info, 0, "cpu flags for benchmark code. 1 = C, -1 = SIMD");
#else
    ...
    , benchmark_cpu_info_(-1)
{
    ...
    benchmark_pixels_div1280_ = static_cast<int>(
        (static_cast<double>(Abs(benchmark_width_)) * static_cast<double>(Abs(benchmark_height_)) * static_cast<double>(benchmark_iterations_) + 1279.0)
        / 1280.0);
}
