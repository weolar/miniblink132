...
static inline bool SizeValid(int src_width, int src_height, int dst_width, int dst_height)
{
    if (src_width > kMaxWidth || src_height > kMaxHeight || dst_width > kMaxWidth || dst_height > kMaxHeight) {
...
#define align_buffer_page_end(var, size)                                                                                                                       \
    uint8_t* var##_mem = reinterpret_cast<uint8_t*>(malloc(((size) + 4095 + 63) & ~4095));                                                                     \
    uint8_t* var = reinterpret_cast<uint8_t*>((intptr_t)(var##_mem + (((size) + 4095 + 63) & ~4095) - (size)) & ~63)
