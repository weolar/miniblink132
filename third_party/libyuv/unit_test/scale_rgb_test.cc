    ...
    int src_width, int src_height, int dst_width, int dst_height, FilterMode f, int benchmark_iterations, int disable_cpu_flags, int benchmark_cpu_info)
{
    ...
    int64_t src_rgb_plane_size = (Abs(src_width) + b * 3) * (Abs(src_height) + b * 3) * 3LL;
    ...
    RGBScale(src_rgb + (src_stride_rgb * b) + b * 3, src_stride_rgb, src_width, src_height, dst_rgb_c + (dst_stride_rgb * b) + b * 3, dst_stride_rgb, dst_width,
        dst_height, f);
    MaskCpuFlags(benchmark_cpu_info); // Enable all CPU optimization.
    RGBScale(src_rgb + (src_stride_rgb * b) + b * 3, src_stride_rgb, src_width, src_height, dst_rgb_opt + (dst_stride_rgb * b) + b * 3, dst_stride_rgb,
    ...
    for (i = b; i < (dst_height + b); ++i) {
        for (j = b * 3; j < (dst_width + b) * 3; ++j) {
            int abs_diff = Abs(dst_rgb_c[(i * dst_stride_rgb) + j] - dst_rgb_opt[(i * dst_stride_rgb) + j]);
...
#define TEST_FACTOR1(name, filter, nom, denom, max_diff)                                                                                                       \
    ...
    {                                                                                                                                                          \
        int diff = RGBTestFilter(SX(benchmark_width_, nom, denom), SX(benchmark_height_, nom, denom), DX(benchmark_width_, nom, denom),                        \
...
#define TEST_FACTOR(name, nom, denom) TEST_FACTOR1(name, Bilinear, nom, denom, 3)
#endif
...
#else
#define TEST_SCALETO(name, width, height) TEST_SCALETO1(name, width, height, Bilinear, 3)
#endif
...
#define TEST_SCALESWAPXY1(name, filter, max_diff)                                                                                                              \
    ...
    {                                                                                                                                                          \
        int diff = RGBTestFilter(benchmark_width_, benchmark_height_, benchmark_height_, benchmark_width_, kFilter##filter, benchmark_iterations_,             \
...
TEST_F(LibYUVScaleTest, RGBTest3x)
{
    ...
    for (int i = 0; i < iterations160; ++i) {
        RGBScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(225, dest_pixels[0]);
    EXPECT_EQ(255 - 225, dest_pixels[1]);

    RGBScale(orig_pixels, kSrcStride, 480, 3, dest_pixels, kDstStride, 160, 1, kFilterNone);
...
TEST_F(LibYUVScaleTest, RGBTest4x)
{
    ...
    int iterations160 = (benchmark_width_ * benchmark_height_ + (160 - 1)) / 160 * benchmark_iterations_;
    for (int i = 0; i < iterations160; ++i) {
        RGBScale(orig_pixels, kSrcStride, 640, 4, dest_pixels, kDstStride, 160, 1, kFilterBilinear);
    }

    EXPECT_EQ(66, dest_pixels[0]);
    EXPECT_EQ(190, dest_pixels[1]);

    RGBScale(orig_pixels, kSrcStride, 64, 4, dest_pixels, kDstStride, 16, 1, kFilterNone);
