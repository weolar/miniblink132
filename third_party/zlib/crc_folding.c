...
local void partial_fold(
    deflate_state* const s, const size_t len, __m128i* xmm_crc0, __m128i* xmm_crc1, __m128i* xmm_crc2, __m128i* xmm_crc3, __m128i* xmm_crc_part)
{
...
ZLIB_INTERNAL void crc_fold_copy(deflate_state* const s, unsigned char* dst, const unsigned char* src, long len)
{
    ...
    if (algn_diff) {
        ...
        partial_fold(s, algn_diff, &xmm_crc0, &xmm_crc1, &xmm_crc2, &xmm_crc3, &xmm_crc_part);
    }
...
#else
{
    ...
    _mm_storeu_si128((__m128i*)dst, xmm_crc_part);
    partial_fold(s, len, &xmm_crc0, &xmm_crc1, &xmm_crc2, &xmm_crc3, &xmm_crc_part);
done:
    CRC_SAVE(s)
}
...
local const unsigned zalign(16) crc_mask[4] = { 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000, 0x00000000 };

local const unsigned zalign(16) crc_mask2[4] = { 0x00000000, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF };
