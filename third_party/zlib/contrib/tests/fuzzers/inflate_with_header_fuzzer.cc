...
static void chunked_inflate(gz_header* header, uint8_t* data, size_t size, size_t in_chunk_size, size_t out_chunk_size)
{
    z_stream stream;
    stream.next_in = data;
    stream.avail_in = 0;
    ...
    while (true) {
        ...
        stream.avail_in = std::min(in_chunk_size, size - static_cast<size_t>(stream.total_in));
        stream.next_out = out_buffer.get();
        stream.avail_out = out_chunk_size;

        if (inflate(&stream, stream.avail_in == 0 ? Z_SYNC_FLUSH : Z_NO_FLUSH) != Z_OK) {
            break;
        }
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    chunked_inflate(&header, remaining_data.data(), remaining_data.size(), in_chunk_size, out_chunk_size);
