...
struct Data {
    Data(size_t s)
    {
        data.reset(new (std::nothrow) char[size = s]);
    }
...
void zlib_compress(const zlib_wrapper type, const char* input, const size_t input_size, std::string* output, bool resize_output = false)
{
    if (resize_output)
    ...
    int result = deflateInit2(&stream, zlib_compression_level, Z_DEFLATED, zlib_stream_wrapper_type(type), MAX_MEM_LEVEL, zlib_strategy);
    if (result != Z_OK)
...
void zlib_uncompress(const zlib_wrapper type, const std::string& input, const size_t output_size, std::string* output)
{
    z_stream stream;
    memset(&stream, 0, sizeof(stream));
...
{
    printf("%s %d %s%s\n", zlib_wrapper_name(type), zlib_compression_level, zlib_level_strategy_name(zlib_compression_level), file.name.c_str());
...
void zlib_file(const char* name, zlib_wrapper type, int width, int check)
{
    ...
    printf("%s: [b %dM] bytes %*d -> %*u %4.2f%%", zlib_wrapper_name(type), block_size / (1 << 20), width, length, width, unsigned(output_length),
        output_length * 100.0 / length);
