...
unsigned copy;
{
    ...
    if (state->window == Z_NULL) {
        ...
        state->window = (unsigned char FAR*)ZALLOC(strm, wsize + CHUNKCOPY_CHUNK_SIZE, sizeof(unsigned char));
        if (state->window == Z_NULL)
            return 1;
        ...
        case EXTRA:
            if (state->flags & 0x0400) {
                ...
                if (copy > have)
                    copy = have;
                if (copy) {
                    if (state->head != Z_NULL && state->head->extra != Z_NULL && (len = state->head->extra_len - state->length) < state->head->extra_max) {
