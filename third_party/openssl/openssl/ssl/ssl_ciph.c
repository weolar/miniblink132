...
/* *INDENT-OFF* */
static const ssl_cipher_table ssl_cipher_table_kx[] = { { SSL_kRSA, NID_kx_rsa }, { SSL_kECDHE, NID_kx_ecdhe }, { SSL_kDHE, NID_kx_dhe },
    { SSL_kECDHEPSK, NID_kx_ecdhe_psk }, { SSL_kDHEPSK, NID_kx_dhe_psk }, { SSL_kRSAPSK, NID_kx_rsa_psk }, { SSL_kPSK, NID_kx_psk }, { SSL_kSRP, NID_kx_srp },
    { SSL_kGOST, NID_kx_gost }, { SSL_kGOST18, NID_kx_gost18 }, { SSL_kANY, NID_kx_any } };

static const ssl_cipher_table ssl_cipher_table_auth[]
    = { { SSL_aRSA, NID_auth_rsa }, { SSL_aECDSA, NID_auth_ecdsa }, { SSL_aPSK, NID_auth_psk }, { SSL_aDSS, NID_auth_dss }, { SSL_aGOST01, NID_auth_gost01 },
          { SSL_aGOST12, NID_auth_gost12 }, { SSL_aSRP, NID_auth_srp }, { SSL_aNULL, NID_auth_null }, { SSL_aANY, NID_auth_any } };
/* *INDENT-ON* */
...
static const SSL_CIPHER cipher_aliases[] = {
    ...
    { 0, SSL_TXT_AES128, NULL, 0, 0, 0, SSL_AES128 | SSL_AES128GCM | SSL_AES128CCM | SSL_AES128CCM8 },
    { 0, SSL_TXT_AES256, NULL, 0, 0, 0, SSL_AES256 | SSL_AES256GCM | SSL_AES256CCM | SSL_AES256CCM8 },
    { 0, SSL_TXT_AES, NULL, 0, 0, 0, SSL_AES },
    { 0, SSL_TXT_AES_GCM, NULL, 0, 0, 0, SSL_AES128GCM | SSL_AES256GCM },
    { 0, SSL_TXT_AES_CCM, NULL, 0, 0, 0, SSL_AES128CCM | SSL_AES256CCM | SSL_AES128CCM8 | SSL_AES256CCM8 },
...
int ssl_load_ciphers(SSL_CTX* ctx)
{
    ...
    for (i = 0, t = ssl_cipher_table_cipher; i < SSL_ENC_NUM_IDX; i++, t++) {
        if (t->nid != NID_undef) {
            const EVP_CIPHER* cipher = ssl_evp_cipher_fetch(ctx->libctx, t->nid, ctx->propq);
    ...
    for (i = 0, t = ssl_cipher_table_mac; i < SSL_MD_NUM_IDX; i++, t++) {
        const EVP_MD* md = ssl_evp_md_fetch(ctx->libctx, t->nid, ctx->propq);
     ...
     */
    memcpy(ctx->ssl_mac_pkey_id, default_mac_pkey_id, sizeof(ctx->ssl_mac_pkey_id));
        ...
        *enc = NULL;
        ...
        if (i == SSL_ENC_NULL_IDX) {
            ...
            *enc = EVP_CIPHER_fetch(ctx->libctx, "NULL", ctx->propq);
            ...
            if (cipher == NULL || !ssl_evp_cipher_up_ref(cipher))
                return 0;
...
int ssl_cipher_get_evp(
    SSL_CTX* ctx, const SSL_SESSION* s, const EVP_CIPHER** enc, const EVP_MD** md, int* mac_pkey_type, size_t* mac_secret_size, SSL_COMP** comp, int use_etm)
{
    int i;
    const SSL_CIPHER* c;
    ...
    if ((*enc != NULL) && (*md != NULL || (EVP_CIPHER_get_flags(*enc) & EVP_CIPH_FLAG_AEAD_CIPHER)) && (!mac_pkey_type || *mac_pkey_type != NID_undef)) {
        const EVP_CIPHER* evp = NULL;

        if (use_etm || s->ssl_version >> 8 != TLS1_VERSION_MAJOR || s->ssl_version < TLS1_VERSION)
            return 1;

        if (c->algorithm_enc == SSL_RC4 && c->algorithm_mac == SSL_MD5)
            evp = ssl_evp_cipher_fetch(ctx->libctx, NID_rc4_hmac_md5, ctx->propq);
        else if (c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA1)
            evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_128_cbc_hmac_sha1, ctx->propq);
        else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA1)
            evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_256_cbc_hmac_sha1, ctx->propq);
        else if (c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA256)
            evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_128_cbc_hmac_sha256, ctx->propq);
        else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA256)
            evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_256_cbc_hmac_sha256, ctx->propq);
...
static void ssl_cipher_collect_ciphers(const SSL_METHOD* ssl_method, int num_of_ciphers, uint32_t disabled_mkey, uint32_t disabled_auth, uint32_t disabled_enc,
    uint32_t disabled_mac, CIPHER_ORDER* co_list, CIPHER_ORDER** head_p, CIPHER_ORDER** tail_p)
{
    int i, co_list_num;
    const SSL_CIPHER* c;
    ...
    for (i = 0; i < num_of_ciphers; i++) {
        ...
        if (c == NULL || !c->valid)
            continue;
        if ((c->algorithm_mkey & disabled_mkey) || (c->algorithm_auth & disabled_auth) || (c->algorithm_enc & disabled_enc)
...
static void ssl_cipher_collect_aliases(const SSL_CIPHER** ca_list, int num_of_group_aliases, uint32_t disabled_mkey, uint32_t disabled_auth,
...
static void ssl_cipher_apply_rule(uint32_t cipher_id, uint32_t alg_mkey, uint32_t alg_auth, uint32_t alg_enc, uint32_t alg_mac, int min_tls,
    uint32_t algo_strength, int rule, int32_t strength_bits, CIPHER_ORDER** head_p, CIPHER_ORDER** tail_p)
{
    ...
    {
        BIO_printf(trc_out, "Applying rule %d with %08x/%08x/%08x/%08x/%08x %08x (%d)\n", rule, alg_mkey, alg_auth, alg_enc, alg_mac, min_tls, algo_strength,
            strength_bits);
    }
...
static int ssl_cipher_process_rulestr(const char* rule_str, CIPHER_ORDER** head_p, CIPHER_ORDER** tail_p, const SSL_CIPHER** ca_list, CERT* c)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            cipher_id = 0;
            while (ca_list[j]) {
                if (strncmp(buf, ca_list[j]->name, buflen) == 0 && (ca_list[j]->name[buflen] == '\0')) {
                    found = 1;
                    break;
                } else
                    j++;
...
static __owur int set_ciphersuites(STACK_OF(SSL_CIPHER) * *currciphers, const char* str)
{
    ...
    if (*str != '\0' && (CONF_parse_list(str, ':', 1, ciphersuite_cb, newciphers) <= 0 || sk_SSL_CIPHER_num(newciphers) == 0)) {
...
static int update_cipher_list(
    SSL_CTX* ctx, STACK_OF(SSL_CIPHER) * *cipher_list, STACK_OF(SSL_CIPHER) * *cipher_list_by_id, STACK_OF(SSL_CIPHER) * tls13_ciphersuites)
{
    int i;
    ...
    for (i = sk_SSL_CIPHER_num(tls13_ciphersuites) - 1; i >= 0; i--) {
        ...
        if ((sslc->algorithm_enc & ctx->disabled_enc_mask) == 0
            && (ssl_cipher_table_mac[sslc->algorithm2 & SSL_HANDSHAKE_MAC_MASK].mask & ctx->disabled_mac_mask) == 0) {
...
int SSL_CTX_set_ciphersuites(SSL_CTX* ctx, const char* str)
{
    ...
    if (ret && ctx->cipher_list != NULL)
        return update_cipher_list(ctx, &ctx->cipher_list, &ctx->cipher_list_by_id, ctx->tls13_ciphersuites);
...
int SSL_set_ciphersuites(SSL* s, const char* str)
{
    ...
    if (ret && s->cipher_list != NULL)
        return update_cipher_list(s->ctx, &s->cipher_list, &s->cipher_list_by_id, s->tls13_ciphersuites);

    return ret;
}

STACK_OF(SSL_CIPHER)
*ssl_create_cipher_list(SSL_CTX* ctx, STACK_OF(SSL_CIPHER) * tls13_ciphersuites, STACK_OF(SSL_CIPHER) * *cipher_list, STACK_OF(SSL_CIPHER) * *cipher_list_by_id,
    const char* rule_str, CERT* c)
{
    ...
    if (strncmp(rule_str, "DEFAULT", 7) == 0) {
        ok = ssl_cipher_process_rulestr(OSSL_default_cipher_list(), &head, &tail, ca_list, c);
        rule_p += 7;
        if (*rule_p == ':')
            rule_p++;
    }
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(tls13_ciphersuites); i++) {
        ...
        if ((sslc->algorithm_enc & disabled_enc) != 0 || (ssl_cipher_table_mac[sslc->algorithm2 & SSL_HANDSHAKE_MAC_MASK].mask & ctx->disabled_mac_mask) != 0) {
...
const SSL_CIPHER* ssl_get_cipher_by_char(SSL* ssl, const unsigned char* ptr, int all)
{
...
int ssl_cipher_get_overhead(const SSL_CIPHER* c, size_t* mac_overhead, size_t* int_overhead, size_t* blocksize, size_t* ext_overhead)
{
