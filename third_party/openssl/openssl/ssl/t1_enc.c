...
static int tls1_PRF(SSL* s, const void* seed1, size_t seed1_len, const void* seed2, size_t seed2_len, const void* seed3, size_t seed3_len, const void* seed4,
    size_t seed4_len, const void* seed5, size_t seed5_len, const unsigned char* sec, size_t slen, unsigned char* out, size_t olen, int fatal)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SECRET, (unsigned char*)sec, (size_t)slen);
...
static int tls1_generate_key_block(SSL* s, unsigned char* km, size_t num)
{
    ...
    ret = tls1_PRF(s, TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE, s->s3.server_random, SSL3_RANDOM_SIZE, s->s3.client_random, SSL3_RANDOM_SIZE,
        NULL, 0, NULL, 0, s->session->master_key, s->session->master_key_length, km, num, 1);
...
int tls_provider_set_tls_params(SSL* s, EVP_CIPHER_CTX* ctx, const EVP_CIPHER* ciph, const EVP_MD* md)
{
    ...
    *pprm++ = OSSL_PARAM_construct_int(OSSL_CIPHER_PARAM_TLS_VERSION, &s->version);
    *pprm++ = OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_TLS_MAC_SIZE, &macsize);
...
int tls1_change_cipher_state(SSL* s, int which)
{
    ...
    if (which & SSL3_CC_READ) {
        ...
        if (comp != NULL) {
            ...
            if (s->expand == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_COMPRESSION_LIBRARY_ERROR);
                goto err;
            }
        }
#endif
    ...
    if (!(EVP_CIPHER_get_flags(c) & EVP_CIPH_FLAG_AEAD_CIPHER)) {
        if (mac_type == EVP_PKEY_HMAC) {
            mac_key = EVP_PKEY_new_raw_private_key_ex(s->ctx->libctx, "HMAC", s->ctx->propq, mac_secret, *mac_secret_size);
        } else {
            /*
             ...
             * a MAC key.
             */
            mac_key = EVP_PKEY_new_mac_key(mac_type, NULL, mac_secret, (int)*mac_secret_size);
        }
        if (mac_key == NULL || EVP_DigestSignInit_ex(mac_ctx, NULL, EVP_MD_get0_name(m), s->ctx->libctx, s->ctx->propq, mac_key, NULL) <= 0) {
    ...
    if (EVP_CIPHER_get_mode(c) == EVP_CIPH_GCM_MODE) {
        if (!EVP_CipherInit_ex(dd, c, NULL, key, NULL, (which & SSL3_CC_WRITE)) || EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_GCM_SET_IV_FIXED, (int)k, iv) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
    } else if (EVP_CIPHER_get_mode(c) == EVP_CIPH_CCM_MODE) {
        ...
        if (!EVP_CipherInit_ex(dd, c, NULL, NULL, NULL, (which & SSL3_CC_WRITE)) || (EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_AEAD_SET_IVLEN, 12, NULL) <= 0)
            || (EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_AEAD_SET_TAG, taglen, NULL) <= 0) || (EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_CCM_SET_IV_FIXED, (int)k, iv) <= 0)
    ...
    if ((EVP_CIPHER_get_flags(c) & EVP_CIPH_FLAG_AEAD_CIPHER) && *mac_secret_size
        && EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_AEAD_SET_MAC_KEY, (int)*mac_secret_size, mac_secret) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
    }
     ...
     */
    if (EVP_CIPHER_get0_provider(EVP_CIPHER_CTX_get0_cipher(dd)) != NULL && !tls_provider_set_tls_params(s, dd, c, m)) {
    ...
    if (!ktls_configure_crypto(s, c, dd, rl_sequence, &crypto_info, &rec_seq, iv, key, ms, *mac_secret_size))
        goto skip_ktls;
...
int tls1_setup_key_block(SSL* s)
{
    ...
    if (!ssl_cipher_get_evp(s->ctx, s->session, &c, &hash, &mac_type, &mac_secret_size, &comp, s->ext.use_etm)) {
    ...
    num = mac_secret_size + EVP_CIPHER_get_key_length(c) + tls_iv_length_within_key_block(c);
    num *= 2;
    ...
    OSSL_TRACE_BEGIN(TLS)
    {
        ...
        BIO_dump_indent(trc_out, s->session->master_key, s->session->master_key_length, 4);
    }
    OSSL_TRACE_END(TLS);
...
size_t tls1_final_finish_mac(SSL* s, const char* str, size_t slen, unsigned char* out)
{
    size_t hashlen;
    ...
    if (!tls1_PRF(s, str, slen, hash, hashlen, NULL, 0, NULL, 0, NULL, 0, s->session->master_key, s->session->master_key_length, out, finished_size, 1)) {
        /* SSLfatal() already called */
        return 0;
    }
...
int tls1_generate_master_secret(SSL* s, unsigned char* out, unsigned char* p, size_t len, size_t* secret_size)
{
    if (s->session->flags & SSL_SESS_FLAG_EXTMS) {
        ...
        OSSL_TRACE_END(TLS);
        if (!tls1_PRF(s, TLS_MD_EXTENDED_MASTER_SECRET_CONST, TLS_MD_EXTENDED_MASTER_SECRET_CONST_SIZE, hash, hashlen, NULL, 0, NULL, 0, NULL, 0, p, len, out,
        ...
        if (!tls1_PRF(s, TLS_MD_MASTER_SECRET_CONST, TLS_MD_MASTER_SECRET_CONST_SIZE, s->s3.client_random, SSL3_RANDOM_SIZE, NULL, 0, s->s3.server_random,
                SSL3_RANDOM_SIZE, NULL, 0, p, len, out, SSL3_MASTER_SECRET_SIZE, 1)) {
            /* SSLfatal() already called */
            return 0;
        }
    }
