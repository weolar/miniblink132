 ...
 * processed yet
 */
#define dtls1_get_unprocessed_record(s) dtls1_retrieve_buffered_record((s), &((s)->rlayer.d->unprocessed_rcds))
 ...
 */
int dtls1_read_bytes(SSL* s, int type, int* recvd_type, unsigned char* buf, size_t len, int peek, size_t* readbytes)
{
    int i, j, iret;
    size_t n;
    SSL3_RECORD* rr;
...
start:
    ...
    if ((SSL3_RECORD_get_length(rr) == 0) || (s->rlayer.rstate == SSL_ST_READ_BODY)) {
    ...
    if (type == SSL3_RECORD_get_type(rr) || (SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC && type == SSL3_RT_HANDSHAKE && recvd_type != NULL)) {
         ...
         */
        if (BIO_dgram_is_sctp(SSL_get_rbio(s)) && s->d1->shutdown_received && BIO_dgram_sctp_msg_waiting(SSL_get_rbio(s)) <= 0) {
    ...
    if ((SSL3_RECORD_get_type(rr) == SSL3_RT_HANDSHAKE) && !ossl_statem_get_in_handshake(s)) {
         ...
         */
        if (SSL3_RECORD_get_epoch(rr) != s->rlayer.d->r_epoch || SSL3_RECORD_get_length(rr) < DTLS1_HM_HEADER_LENGTH) {
            SSL3_RECORD_set_length(rr, 0);
            SSL3_RECORD_set_read(rr);
            goto start;
        }
 ...
 */
int dtls1_write_bytes(SSL* s, int type, const void* buf, size_t len, size_t* written)
{
    int i;
...
int do_dtls1_write(SSL* s, int type, const unsigned char* buf, size_t len, int create_empty_fragment, size_t* written)
{
    ...
    *(p++) = type & 0xff;
    ...
    if (!SSL_WRITE_ETM(s) && mac_size != 0) {
        if (!s->method->ssl3_enc->mac(s, &wr, &(p[SSL3_RECORD_get_length(&wr) + eivlen]), 1)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return -1;
        }
    ...
    if (SSL_WRITE_ETM(s) && mac_size != 0) {
        if (!s->method->ssl3_enc->mac(s, &wr, &(p[SSL3_RECORD_get_length(&wr)]), 1)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return -1;
        }
        SSL3_RECORD_add_length(&wr, mac_size);
...
DTLS1_BITMAP* dtls1_get_bitmap(SSL* s, SSL3_RECORD* rr, unsigned int* is_next_epoch)
{
    ...
    *is_next_epoch = 0;
     ...
     */
    else if (rr->epoch == (unsigned long)(s->rlayer.d->r_epoch + 1) && s->rlayer.d->unprocessed_rcds.epoch != s->rlayer.d->r_epoch) {
        *is_next_epoch = 1;
