...
int ssl3_cbc_digest_record(const EVP_MD* md, unsigned char* md_out, size_t* md_out_size, const unsigned char* header, const unsigned char* data,
 ...
 */
#define u32toLE(n, p)                                                                                                                                          \
    (*((p)++) = (unsigned char)(n), *((p)++) = (unsigned char)(n >> 8), *((p)++) = (unsigned char)(n >> 16), *((p)++) = (unsigned char)(n >> 24))
    ...
    size_t data_size, size_t data_plus_mac_plus_padding_size, const unsigned char* mac_secret, size_t mac_secret_length, char is_sslv3)
{
    ...
    size_t sslv3_pad_length = 40, header_length, variance_blocks, len, max_mac_bytes, num_blocks, num_starting_blocks, k, mac_end_offset, c, index_a, index_b;
    ...
    if (!ossl_assert(md_length_size <= MAX_HASH_BIT_COUNT_BYTES) || !ossl_assert(md_block_size <= MAX_HASH_BLOCK_SIZE)
    ...
    if (is_sslv3) {
        ...
        if (EVP_DigestUpdate(md_ctx, mac_secret, mac_secret_length) <= 0 || EVP_DigestUpdate(md_ctx, hmac_pad, sslv3_pad_length) <= 0
