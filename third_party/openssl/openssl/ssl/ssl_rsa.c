...
static int ssl_set_pkey(CERT* c, EVP_PKEY* pkey)
{
    ...
    if (c->pkeys[i].x509 != NULL && !X509_check_private_key(c->pkeys[i].x509, pkey))
        return 0;
...
int SSL_use_PrivateKey_file(SSL* ssl, const char* file, int type)
{
    ...
    if (type == SSL_FILETYPE_PEM) {
        j = ERR_R_PEM_LIB;
        pkey = PEM_read_bio_PrivateKey_ex(in, NULL, ssl->default_passwd_callback, ssl->default_passwd_callback_userdata, ssl->ctx->libctx, ssl->ctx->propq);
    } else if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
        pkey = d2i_PrivateKey_ex_bio(in, NULL, ssl->ctx->libctx, ssl->ctx->propq);
    } else {
        ERR_raise(ERR_LIB_SSL, SSL_R_BAD_SSL_FILETYPE);
        goto end;
    }
...
int SSL_use_PrivateKey_ASN1(int type, SSL* ssl, const unsigned char* d, long len)
{
    ...
    p = d;
    if ((pkey = d2i_PrivateKey_ex(type, NULL, &p, (long)len, ssl->ctx->libctx, ssl->ctx->propq)) == NULL) {
        ERR_raise(ERR_LIB_SSL, ERR_R_ASN1_LIB);
        return 0;
    }
...
int SSL_CTX_use_certificate_file(SSL_CTX* ctx, const char* file, int type)
{
    ...
    if (type == SSL_FILETYPE_ASN1) {
        ...
        j = ERR_R_PEM_LIB;
        cert = PEM_read_bio_X509(in, &x, ctx->default_passwd_callback, ctx->default_passwd_callback_userdata);
    }
    if (cert == NULL) {
        ERR_raise(ERR_LIB_SSL, j);
        goto end;
...
int SSL_CTX_use_PrivateKey_file(SSL_CTX* ctx, const char* file, int type)
{
    ...
    if (type == SSL_FILETYPE_PEM) {
        j = ERR_R_PEM_LIB;
        pkey = PEM_read_bio_PrivateKey_ex(in, NULL, ctx->default_passwd_callback, ctx->default_passwd_callback_userdata, ctx->libctx, ctx->propq);
    } else if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
...
int SSL_CTX_use_PrivateKey_ASN1(int type, SSL_CTX* ctx, const unsigned char* d, long len)
{
    ...
    p = d;
    if ((pkey = d2i_PrivateKey_ex(type, NULL, &p, (long)len, ctx->libctx, ctx->propq)) == NULL) {
        ERR_raise(ERR_LIB_SSL, ERR_R_ASN1_LIB);
        return 0;
    }
...
static int use_certificate_chain_file(SSL_CTX* ctx, SSL* ssl, const char* file)
{
    ...
    if (PEM_read_bio_X509_AUX(in, &x, passwd_callback, passwd_callback_userdata) == NULL) {
    ...
    if (ret) {
        ...
        while (1) {
            ...
            if (PEM_read_bio_X509(in, &ca, passwd_callback, passwd_callback_userdata) != NULL) {
                if (ctx)
...
static int serverinfo_find_extension(
    const unsigned char* serverinfo, size_t serverinfo_length, unsigned int extension_type, const unsigned char** extension_data, size_t* extension_length)
{
    PACKET pkt, data;
    ...
    *extension_length = 0;
    ...
    for (;;) {
        ...
        if (!PACKET_get_net_4(&pkt, &context) || !PACKET_get_net_2(&pkt, &type) || !PACKET_get_length_prefixed_2(&pkt, &data))
            return -1;
...
static int serverinfoex_srv_parse_cb(
    SSL* s, unsigned int ext_type, unsigned int context, const unsigned char* in, size_t inlen, X509* x, size_t chainidx, int* al, void* arg)
{
...
static int serverinfo_srv_parse_cb(SSL* s, unsigned int ext_type, const unsigned char* in, size_t inlen, int* al, void* arg)
{
    return serverinfoex_srv_parse_cb(s, ext_type, 0, in, inlen, NULL, 0, al, arg);
}

static int serverinfoex_srv_add_cb(
    SSL* s, unsigned int ext_type, unsigned int context, const unsigned char** out, size_t* outlen, X509* x, size_t chainidx, int* al, void* arg)
{
...
static int serverinfo_srv_add_cb(SSL* s, unsigned int ext_type, const unsigned char** out, size_t* outlen, int* al, void* arg)
{
    return serverinfoex_srv_add_cb(s, ext_type, 0, out, outlen, NULL, 0, al, arg);
}
 ...
 */
static int serverinfo_process_buffer(unsigned int version, const unsigned char* serverinfo, size_t serverinfo_length, SSL_CTX* ctx)
{
    PACKET pkt;
    ...
    while (PACKET_remaining(&pkt)) {
        ...
        if ((version == SSL_SERVERINFOV2 && !PACKET_get_net_4(&pkt, &context)) || !PACKET_get_net_2(&pkt, &ext_type)
...
static void extension_append(unsigned int version, const unsigned char* extension, const size_t extension_length, unsigned char* serverinfo)
{
...
int SSL_CTX_use_serverinfo_ex(SSL_CTX* ctx, unsigned int version, const unsigned char* serverinfo, size_t serverinfo_length)
{
    ...
    if (version == SSL_SERVERINFOV1) {
         ...
         */
        const size_t sinfo_length = extension_append_length(SSL_SERVERINFOV1, serverinfo_length);
        unsigned char* sinfo;
        int ret;
        ...
        ret = SSL_CTX_use_serverinfo_ex(ctx, SSL_SERVERINFOV2, sinfo, sinfo_length);

        OPENSSL_free(sinfo);
        return ret;
    }
    if (!serverinfo_process_buffer(version, serverinfo, serverinfo_length, NULL)) {
    ...
    new_serverinfo = OPENSSL_realloc(ctx->cert->key->serverinfo, serverinfo_length);
...
{
    return SSL_CTX_use_serverinfo_ex(ctx, SSL_SERVERINFOV1, serverinfo, serverinfo_length);
}
...
int SSL_CTX_use_serverinfo_file(SSL_CTX* ctx, const char* file)
{
    ...
    for (num_extensions = 0;; num_extensions++) {
        ...
        if (version == SSL_SERVERINFOV1) {
            ...
            if (extension_length < 8 || (extension[6] << 8) + extension[7] != extension_length - 8) {
                ERR_raise(ERR_LIB_SSL, SSL_R_BAD_DATA);
                goto end;
            }
        }
        ...
        serverinfo = tmp;
        extension_append(version, extension, extension_length, serverinfo + serverinfo_length);
