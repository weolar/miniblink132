...
static int ssl_undefined_function_1(SSL* ssl, SSL3_RECORD* r, size_t s, int t, SSL_MAC_BUF* mac, size_t macsize)
{
    return ssl_undefined_function(ssl);
}

static int ssl_undefined_function_2(SSL* ssl, SSL3_RECORD* r, unsigned char* s, int t)
{
    return ssl_undefined_function(ssl);
}

static int ssl_undefined_function_3(SSL* ssl, unsigned char* r, unsigned char* s, size_t t, size_t* u)
{
    return ssl_undefined_function(ssl);
}
...
static size_t ssl_undefined_function_5(SSL* ssl, const char* r, size_t s, unsigned char* t)
{
    return ssl_undefined_function(ssl);
}
...
static int ssl_undefined_function_7(SSL* ssl, unsigned char* r, size_t s, const char* t, size_t u, const unsigned char* v, size_t w, int x)
{
    return ssl_undefined_function(ssl);
}
...
static int ssl_dane_dup(SSL* to, SSL* from)
{
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (SSL_dane_tlsa_add(to, t->usage, t->selector, t->mtype, t->data, t->dlen) <= 0)
            return 0;
    }
    return 1;
}

static int dane_mtype_set(struct dane_ctx_st* dctx, const EVP_MD* md, uint8_t mtype, uint8_t ord)
{
    int i;
...
static int dane_tlsa_add(SSL_DANE* dane, uint8_t usage, uint8_t selector, uint8_t mtype, const unsigned char* data, size_t dlen)
{
    danetls_record* t;
...
static int ssl_check_allowed_versions(int min_version, int max_version)
{
    ...
    if ((minisdtls && !maxisdtls && max_version != 0) || (maxisdtls && !minisdtls && min_version != 0)) {
    ...
    if (minisdtls || maxisdtls) {
        ...
        if (0
#ifdef OPENSSL_NO_DTLS1
            || (DTLS_VERSION_GE(min_version, DTLS1_VERSION) && DTLS_VERSION_GE(DTLS1_VERSION, max_version))
#endif
#ifdef OPENSSL_NO_DTLS1_2
            || (DTLS_VERSION_GE(min_version, DTLS1_2_VERSION) && DTLS_VERSION_GE(DTLS1_2_VERSION, max_version))
#endif
        )
            return 0;
    } else {
...
int SSL_CTX_set_ssl_version(SSL_CTX* ctx, const SSL_METHOD* meth)
{
    ...
    sk = ssl_create_cipher_list(ctx, ctx->tls13_ciphersuites, &(ctx->cipher_list), &(ctx->cipher_list_by_id), OSSL_default_cipher_list(), ctx->cert);
...
SSL* SSL_new(SSL_CTX* ctx)
{
    ...
    if (ctx->ext.ecpointformats) {
        s->ext.ecpointformats = OPENSSL_memdup(ctx->ext.ecpointformats, ctx->ext.ecpointformats_len);
    ...
    if (ctx->ext.supportedgroups) {
        s->ext.supportedgroups = OPENSSL_memdup(ctx->ext.supportedgroups, ctx->ext.supportedgroups_len * sizeof(*ctx->ext.supportedgroups));
    ...
    if (!SSL_set_ct_validation_callback(s, ctx->ct_validation_callback, ctx->ct_validation_callback_arg))
        goto err;
#endif
...
int SSL_get0_dane_tlsa(SSL* s, uint8_t* usage, uint8_t* selector, uint8_t* mtype, const unsigned char** data, size_t* dlen)
{
    SSL_DANE* dane = &s->dane;
...
int SSL_set_rfd(SSL* s, int fd)
{
    ...
    if (wbio == NULL || BIO_method_type(wbio) != BIO_TYPE_SOCKET || ((int)BIO_get_fd(wbio, NULL) != fd)) {
        BIO* bio = BIO_new(BIO_s_socket());
...
int SSL_get_changed_async_fds(SSL* s, OSSL_ASYNC_FD* addfd, size_t* numaddfds, OSSL_ASYNC_FD* delfd, size_t* numdelfds)
{
    ...
    if (ctx == NULL)
        return 0;
    return ASYNC_WAIT_CTX_get_changed_fds(ctx, addfd, numaddfds, delfd, numdelfds);
}
...
static int ssl_start_async_job(SSL* s, struct ssl_async_args* args, int (*func)(void*))
{
    int ret;
    if (s->waitctx == NULL) {
        ...
        if (s->waitctx == NULL)
            return -1;
        if (s->async_cb != NULL && !ASYNC_WAIT_CTX_set_callback(s->waitctx, ssl_async_wait_ctx_cb, s))
            return -1;
    }

    s->rwstate = SSL_NOTHING;
    switch (ASYNC_start_job(&s->job, s->waitctx, &ret, func, args, sizeof(struct ssl_async_args))) {
    case ASYNC_ERR:
        s->rwstate = SSL_NOTHING;
...
int ssl_read_internal(SSL* s, void* buf, size_t num, size_t* readbytes)
{
    ...
    if (s->early_data_state == SSL_EARLY_DATA_CONNECT_RETRY || s->early_data_state == SSL_EARLY_DATA_ACCEPT_RETRY) {
        ERR_raise(ERR_LIB_SSL, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED);
        return 0;
    }
    /*
    ...
    case SSL_EARLY_DATA_READ_RETRY:
        if (s->ext.early_data == SSL_EARLY_DATA_ACCEPTED) {
            ...
            if (ret > 0 || (ret <= 0 && s->early_data_state != SSL_EARLY_DATA_FINISHED_READING)) {
                ...
                return ret > 0 ? SSL_READ_EARLY_DATA_SUCCESS : SSL_READ_EARLY_DATA_ERROR;
            }
        } else {
...
ossl_ssize_t SSL_sendfile(SSL* s, int fd, off_t offset, size_t size, int flags)
{
    ...
    ERR_raise_data(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR, "can't call ktls_sendfile(), ktls disabled");
    return -1;
#else
...
int SSL_write_early_data(SSL* s, const void* buf, size_t num, size_t* written)
{
    ...
    switch (s->early_data_state) {
    case SSL_EARLY_DATA_NONE:
        if (s->server || !SSL_in_before(s) || ((s->session == NULL || s->session->ext.max_early_data == 0) && (s->psk_use_session_cb == NULL))) {
            ERR_raise(ERR_LIB_SSL, ERR_R_SHOULD_NOT_HAVE_BEEN_CALLED);
            return 0;
        }
        /* fall through */
...
int SSL_new_session_ticket(SSL* s)
{
    ...
    if ((SSL_in_init(s) && s->ext.extra_tickets_expected == 0) || SSL_IS_FIRST_HANDSHAKE(s) || !s->server || !SSL_IS_TLS13(s))
        return 0;
...
long SSL_ctrl(SSL* s, int cmd, long larg, void* parg)
{
    ...
    case SSL_CTRL_SET_MIN_PROTO_VERSION:
        return ssl_check_allowed_versions(larg, s->max_proto_version) && ssl_set_version_bound(s->ctx->method->version, (int)larg, &s->min_proto_version);
...
long SSL_CTX_ctrl(SSL_CTX* ctx, int cmd, long larg, void* parg)
{
    ...
    case SSL_CTRL_CLEAR_CERT_FLAGS:
        return (ctx->cert->cert_flags &= ~larg);
    case SSL_CTRL_SET_MIN_PROTO_VERSION:
        return ssl_check_allowed_versions(larg, ctx->max_proto_version) && ssl_set_version_bound(ctx->method->version, (int)larg, &ctx->min_proto_version);
    case SSL_CTRL_GET_MIN_PROTO_VERSION:
        return ctx->min_proto_version;
    case SSL_CTRL_SET_MAX_PROTO_VERSION:
        return ssl_check_allowed_versions(ctx->min_proto_version, larg) && ssl_set_version_bound(ctx->method->version, (int)larg, &ctx->max_proto_version);
    case SSL_CTRL_GET_MAX_PROTO_VERSION:
...
int SSL_CTX_set_cipher_list(SSL_CTX* ctx, const char* str)
{
    ...
    sk = ssl_create_cipher_list(ctx, ctx->tls13_ciphersuites, &ctx->cipher_list, &ctx->cipher_list_by_id, str, ctx->cert);
    /*
...
int SSL_set_cipher_list(SSL* s, const char* str)
{
    ...
    sk = ssl_create_cipher_list(s->ctx, s->tls13_ciphersuites, &s->cipher_list, &s->cipher_list_by_id, str, s->cert);
...
char* SSL_get_shared_ciphers(const SSL* s, char* buf, int size)
{
    ...
    if (!s->server || s->peer_ciphers == NULL || size < 2)
        return NULL;
...
const char* SSL_get_servername(const SSL* s, const int type)
{
    ...
    if (server) {
         ...
         */
        if (SSL_in_before(s)) {
            if (s->ext.hostname == NULL && s->session != NULL && s->session->ssl_version != TLS1_3_VERSION)
                return s->session->ext.hostname;
        } else {
...
int SSL_select_next_proto(
    unsigned char** out, unsigned char* outlen, const unsigned char* server, unsigned int server_len, const unsigned char* client, unsigned int client_len)
{
    unsigned int i, j;
 ...
 * ServerHello.
 */
void SSL_CTX_set_npn_advertised_cb(SSL_CTX* ctx, SSL_CTX_npn_advertised_cb_func cb, void* arg)
{
 ...
 */
void SSL_CTX_set_npn_select_cb(SSL_CTX* ctx, SSL_CTX_npn_select_cb_func cb, void* arg)
{
...
{
    if (s->session == NULL || (s->version < TLS1_VERSION && s->version != DTLS1_BAD_VER))
        return -1;

    return s->method->ssl3_enc->export_keying_material(s, out, olen, label, llen, context, contextlen, use_context);
}
...
int SSL_export_keying_material_early(SSL* s, unsigned char* out, size_t olen, const char* label, size_t llen, const unsigned char* context, size_t contextlen)
{
    ...
    return tls13_export_keying_material_early(s, out, olen, label, llen, context, contextlen);
}
...
static unsigned long ssl_session_hash(const SSL_SESSION* a)
{
    ...
    l = (unsigned long)((unsigned long)session_id[0]) | ((unsigned long)session_id[1] << 8L) | ((unsigned long)session_id[2] << 16L)
...
SSL_CTX* SSL_CTX_new_ex(OSSL_LIB_CTX* libctx, const char* propq, const SSL_METHOD* meth)
{
    ...
    if (!ssl_create_cipher_list(ret, ret->tls13_ciphersuites, &ret->cipher_list, &ret->cipher_list_by_id, OSSL_default_cipher_list(), ret->cert)
    ...
    if ((RAND_bytes_ex(libctx, ret->ext.tick_key_name, sizeof(ret->ext.tick_key_name), 0) <= 0)
        || (RAND_priv_bytes_ex(libctx, ret->ext.secure->tick_hmac_key, sizeof(ret->ext.secure->tick_hmac_key), 0) <= 0)
        || (RAND_priv_bytes_ex(libctx, ret->ext.secure->tick_aes_key, sizeof(ret->ext.secure->tick_aes_key), 0) <= 0))
        ret->options |= SSL_OP_NO_TICKET;

    if (RAND_priv_bytes_ex(libctx, ret->ext.cookie_hmac_key, sizeof(ret->ext.cookie_hmac_key), 0) <= 0)
        goto err;
...
void ssl_set_masks(SSL* s)
{
    ...
    if (rsa_enc || rsa_sign
        || (ssl_has_cert(s, SSL_PKEY_RSA_PSS_SIGN) && pvalid[SSL_PKEY_RSA_PSS_SIGN] & CERT_PKEY_EXPLICIT_SIGN && TLS1_get_version(s) == TLS1_2_VERSION))
        mask_a |= SSL_aRSA;
    ...
    if (!(mask_a & SSL_aECDSA) && ssl_has_cert(s, SSL_PKEY_ED25519) && pvalid[SSL_PKEY_ED25519] & CERT_PKEY_EXPLICIT_SIGN
        && TLS1_get_version(s) == TLS1_2_VERSION)
        mask_a |= SSL_aECDSA;

    /* Allow Ed448 for TLS 1.2 if peer supports it */
    if (!(mask_a & SSL_aECDSA) && ssl_has_cert(s, SSL_PKEY_ED448) && pvalid[SSL_PKEY_ED448] & CERT_PKEY_EXPLICIT_SIGN && TLS1_get_version(s) == TLS1_2_VERSION)
        mask_a |= SSL_aECDSA;
...
void ssl_update_cache(SSL* s, int mode)
{
     ...
     */
    if (s->server && s->session->sid_ctx_length == 0 && (s->verify_mode & SSL_VERIFY_PEER) != 0)
        return;
    ...
    if ((i & mode) != 0 && (!s->hit || SSL_IS_TLS13(s))) {
        ...
        if ((i & SSL_SESS_CACHE_NO_INTERNAL_STORE) == 0
            && (!SSL_IS_TLS13(s) || !s->server || (s->max_early_data > 0 && (s->options & SSL_OP_NO_ANTI_REPLAY) == 0)
                || s->session_ctx->remove_session_cb != NULL || (s->options & SSL_OP_NO_TICKET) != 0))
...
SSL* SSL_dup(SSL* s)
{
    ...
    if (s->session != NULL) {
        ...
        if (!SSL_set_session_id_context(ret, s->sid_ctx, (int)s->sid_ctx_length))
            goto err;
    }
...
{
    return X509_STORE_set_default_paths_ex(ctx->cert_store, ctx->libctx, ctx->propq);
}
...
int SSL_CTX_set_default_verify_file(SSL_CTX* ctx)
{
    ...
    X509_LOOKUP_load_file_ex(lookup, NULL, X509_FILETYPE_DEFAULT, ctx->libctx, ctx->propq);
...
{
    return X509_STORE_load_file_ex(ctx->cert_store, CAfile, ctx->libctx, ctx->propq);
}
...
{
    return X509_STORE_load_store_ex(ctx->cert_store, CAstore, ctx->libctx, ctx->propq);
}
...
{
    SSL_CTX_callback_ctrl(ctx, SSL_CTRL_SET_NOT_RESUMABLE_SESS_CB, (void (*)(void))cb);
}

void SSL_set_not_resumable_session_callback(SSL* ssl, int (*cb)(SSL* ssl, int is_forward_secure))
{
    SSL_callback_ctrl(ssl, SSL_CTRL_SET_NOT_RESUMABLE_SESS_CB, (void (*)(void))cb);
}
...
int ssl_handshake_hash(SSL* s, unsigned char* out, size_t outlen, size_t* hashlen)
{
    ...
    if (!EVP_MD_CTX_copy_ex(ctx, hdgst) || EVP_DigestFinal_ex(ctx, out, NULL) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
    }
...
static int ct_permissive(const CT_POLICY_EVAL_CTX* ctx, const STACK_OF(SCT) * scts, void* unused_arg)
{
    return 1;
}

static int ct_strict(const CT_POLICY_EVAL_CTX* ctx, const STACK_OF(SCT) * scts, void* unused_arg)
{
...
int ssl_validate_ct(SSL* s)
{
    ...
    CT_POLICY_EVAL_CTX_set_time(ctx, (uint64_t)SSL_SESSION_get_time(SSL_get0_session(s)) * 1000);
...
int ssl_log_rsa_client_key_exchange(
...
int ssl_cache_cipherlist(SSL* s, PACKET* cipher_suites, int sslv2format)
{
    ...
    if (sslv2format) {
        ...
        for (s->s3.tmp.ciphers_rawlen = 0; PACKET_remaining(&sslv2ciphers) > 0; raw += TLS_CIPHER_LEN) {
            if (!PACKET_get_1(&sslv2ciphers, &leadbyte) || (leadbyte == 0 && !PACKET_copy_bytes(&sslv2ciphers, raw, TLS_CIPHER_LEN))
...
int bytes_to_cipher_list(SSL* s, PACKET* cipher_suites, STACK_OF(SSL_CIPHER) * *skp, STACK_OF(SSL_CIPHER) * *scsvs_out, int sslv2format, int fatal)
{
    const SSL_CIPHER* c;
...
{
    if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey), 0, dhpkey)) {
...
{
    if (!ssl_ctx_security(ctx, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey), 0, dhpkey)) {
        ERR_raise(ERR_LIB_SSL, SSL_R_DH_KEY_TOO_SMALL);
        return 0;
    }
