...
static int ssl_undefined_function_1(SSL* ssl, unsigned char* r, size_t s, const char* t, size_t u, const unsigned char* v, size_t w, int x)
{
    (void)r;
    (void)s;
    (void)t;
...
const SSL3_ENC_METHOD SSLv3_enc_data = { ssl3_enc, n_ssl3_mac, ssl3_setup_key_block, ssl3_generate_master_secret, ssl3_change_cipher_state,
    ssl3_final_finish_mac, SSL3_MD_CLIENT_FINISHED_CONST, 4, SSL3_MD_SERVER_FINISHED_CONST, 4, ssl3_alert_code, ssl_undefined_function_1, 0,
...
long ssl3_ctrl(SSL* s, int cmd, long larg, void* parg)
{
    ...
    case SSL_CTRL_SET_TMP_ECDH: {
        ...
        return ssl_set_tmp_ecdh_groups(&s->ext.supportedgroups, &s->ext.supportedgroups_len, parg);
    }
    ...
    case SSL_CTRL_SET_GROUPS:
        return tls1_set_groups(&s->ext.supportedgroups, &s->ext.supportedgroups_len, parg, larg);

    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(s->ctx, &s->ext.supportedgroups, &s->ext.supportedgroups_len, parg);
...
long ssl3_callback_ctrl(SSL* s, int cmd, void (*fp)(void))
{
    ...
    case SSL_CTRL_SET_TLSEXT_DEBUG_CB:
        s->ext.debug_cb = (void (*)(SSL*, int, int, const unsigned char*, int, void*))fp;
        ret = 1;
        break;
...
long ssl3_ctx_ctrl(SSL_CTX* ctx, int cmd, long larg, void* parg)
{
    ...
    case SSL_CTRL_SET_TMP_ECDH: {
        ...
        return ssl_set_tmp_ecdh_groups(&ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, parg);
    }
#endif /* !OPENSSL_NO_DEPRECATED_3_0 */
    ...
    case SSL_CTRL_GET_TLSEXT_TICKET_KEYS: {
        ...
        if (cmd == SSL_CTRL_SET_TLSEXT_TICKET_KEYS) {
            ...
            memcpy(keys + sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key), ctx->ext.secure->tick_aes_key,
                sizeof(ctx->ext.secure->tick_aes_key));
        }
        return 1;
    }
    ...
    case SSL_CTRL_SET_GROUPS:
        return tls1_set_groups(&ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, parg, larg);

    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(ctx, &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len, parg);
...
long ssl3_ctx_callback_ctrl(SSL_CTX* ctx, int cmd, void (*fp)(void))
{
    ...
    case SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB:
        ctx->ext.ticket_key_cb = (int (*)(SSL*, unsigned char*, unsigned char*, EVP_CIPHER_CTX*, HMAC_CTX*, int))fp;
        break;
#endif
...
const SSL_CIPHER* ssl3_choose_cipher(SSL* s, STACK_OF(SSL_CIPHER) * clnt, STACK_OF(SSL_CIPHER) * srvr)
{
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
        ...
        if (!SSL_IS_TLS13(s)) {
            ...
            OSSL_TRACE7(TLS_CIPHER, "%d:[%08lX:%08lX:%08lX:%08lX]%p:%s\n", ok, alg_k, alg_a, mask_k, mask_a, (void*)c, c->name);
 ...
 */
int ssl_fill_hello_random(SSL* s, int server, unsigned char* result, size_t len, DOWNGRADE dgrd)
{
    int send_time = 0, ret;
...
int ssl_generate_master_secret(SSL* s, unsigned char* pms, size_t pmslen, int free_pms)
{
...
EVP_PKEY* ssl_generate_pkey_group(SSL* s, uint16_t id)
{
    ...
    pctx = EVP_PKEY_CTX_new_from_name(s->ctx->libctx, ginf->algorithm, s->ctx->propq);
...
int ssl_gensecret(SSL* s, unsigned char* pms, size_t pmslen)
{
    ...
    if (SSL_IS_TLS13(s)) {
         ...
         */
        if (!s->hit)
            rv = tls13_generate_secret(s, ssl_handshake_md(s), NULL, NULL, 0, (unsigned char*)&s->early_secret);
        else
            rv = 1;
...
int ssl_decapsulate(SSL* s, EVP_PKEY* privkey, const unsigned char* ct, size_t ctlen, int gensecret)
{
    int rv = 0;
    unsigned char* pms = NULL;
    size_t pmslen = 0;
...
int ssl_encapsulate(SSL* s, EVP_PKEY* pubkey, unsigned char** ctp, size_t* ctlenp, int gensecret)
{
    int rv = 0;
