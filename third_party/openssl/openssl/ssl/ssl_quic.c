...
{
    if (ssl->ext.peer_quic_transport_params_len != 0 && ssl->ext.peer_quic_transport_params_draft_len != 0)
        return -1;
...
size_t SSL_quic_max_handshake_flight_len(const SSL* ssl, OSSL_ENCRYPTION_LEVEL level)
{
    ...
    case ssl_encryption_handshake:
        if (ssl->server) {
             ...
             * client certificates.
             */
            if ((ssl->verify_mode & SSL_VERIFY_PEER) && ssl->max_cert_list > DEFAULT_FLIGHT_LIMIT)
...
int SSL_provide_quic_data(SSL* ssl, OSSL_ENCRYPTION_LEVEL level, const uint8_t* data, size_t len)
{
    ...
    if (level < ssl->quic_read_level || (ssl->quic_input_data_tail != NULL && level < ssl->quic_input_data_tail->level)
    ...
    if (ssl->quic_buf->length != ssl->quic_next_record_start && level != ssl->quic_latest_level_received) {
        ERR_raise(ERR_LIB_SSL, SSL_R_WRONG_ENCRYPTION_LEVEL_RECEIVED);
        return 0;
    }
    ...
    while (ssl->quic_buf->length > ssl->quic_next_record_start + SSL3_HM_HEADER_LENGTH) {
        QUIC_DATA* qd;
        const uint8_t* p;

        /* TLS Handshake message header has 1-byte type and 3-byte length */
        p = (const uint8_t*)ssl->quic_buf->data + ssl->quic_next_record_start + 1;
        n2l3(p, l);
...
int quic_set_encryption_secrets(SSL* ssl, OSSL_ENCRYPTION_LEVEL level)
{
    ...
    if (level == ssl_encryption_early_data) {
        ...
        if (ssl->early_data_state == SSL_EARLY_DATA_CONNECTING && ssl->max_early_data > 0 && ssl->session->ext.max_early_data == 0) {
            if (!ossl_assert(ssl->psksession != NULL && ssl->max_early_data == ssl->psksession->ext.max_early_data)) {
    ...
    if (ssl->server) {
        if (!ssl->quic_method->set_encryption_secrets(ssl, level, c2s_secret, s2c_secret, len)) {
            SSLfatal(ssl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
    } else {
        if (!ssl->quic_method->set_encryption_secrets(ssl, level, s2c_secret, c2s_secret, len)) {
            SSLfatal(ssl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
    }
...
void SSL_set_quic_early_data_enabled(SSL* ssl, int enabled)
{
    ...
    if ((ssl->session == NULL || ssl->session->ext.max_early_data == 0) && ssl->psk_use_session_cb == NULL)
        return;
