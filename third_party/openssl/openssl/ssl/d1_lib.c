...
const SSL3_ENC_METHOD DTLSv1_enc_data
    = { tls1_enc, tls1_mac, tls1_setup_key_block, tls1_generate_master_secret, tls1_change_cipher_state, tls1_final_finish_mac, TLS_MD_CLIENT_FINISH_CONST,
          TLS_MD_CLIENT_FINISH_CONST_SIZE, TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE, tls1_alert_code, tls1_export_keying_material,
          SSL_ENC_FLAG_DTLS | SSL_ENC_FLAG_EXPLICIT_IV, dtls1_set_handshake_header, dtls1_close_construct_packet, dtls1_handshake_write };

const SSL3_ENC_METHOD DTLSv1_2_enc_data
    = { tls1_enc, tls1_mac, tls1_setup_key_block, tls1_generate_master_secret, tls1_change_cipher_state, tls1_final_finish_mac, TLS_MD_CLIENT_FINISH_CONST,
          TLS_MD_CLIENT_FINISH_CONST_SIZE, TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE, tls1_alert_code, tls1_export_keying_material,
          SSL_ENC_FLAG_DTLS | SSL_ENC_FLAG_EXPLICIT_IV | SSL_ENC_FLAG_SIGALGS | SSL_ENC_FLAG_SHA256_PRF | SSL_ENC_FLAG_TLS1_2_CIPHERS,
          dtls1_set_handshake_header, dtls1_close_construct_packet, dtls1_handshake_write };
...
void dtls1_clear_sent_buffer(SSL* s)
{
    ...
    while ((item = pqueue_pop(s->d1->sent_messages)) != NULL) {
        ...
        if (frag->msg_header.is_ccs) {
             ...
             */
            if (s->enc_write_ctx != frag->msg_header.saved_retransmit_state.enc_write_ctx)
                EVP_CIPHER_CTX_free(frag->msg_header.saved_retransmit_state.enc_write_ctx);

            if (s->write_hash != frag->msg_header.saved_retransmit_state.write_hash)
...
int dtls1_check_timeout_num(SSL* s)
{
    ...
    if (s->d1->timeout_num_alerts > 2 && !(SSL_get_options(s) & SSL_OP_NO_QUERY_MTU)) {
...
int DTLSv1_listen(SSL* s, BIO_ADDR* client)
{
    ...
    do {
        ...
        if (s->msg_callback)
            s->msg_callback(0, 0, SSL3_RT_HEADER, buf, DTLS1_RT_HEADER_LENGTH, s, s->msg_callback_arg);
        ...
        if (!PACKET_forward(&pkt, 1)
            ...
            || !PACKET_copy_bytes(&pkt, seq, SEQ_NUM_SIZE) || !PACKET_get_length_prefixed_2(&pkt, &msgpkt)) {
            ERR_raise(ERR_LIB_SSL, SSL_R_LENGTH_MISMATCH);
            goto end;
        }
        ...
        if (!PACKET_get_1(&msgpkt, &msgtype) || !PACKET_get_net_3_len(&msgpkt, &msglen) || !PACKET_get_net_2(&msgpkt, &msgseq)
            || !PACKET_get_net_3_len(&msgpkt, &fragoff) || !PACKET_get_net_3_len(&msgpkt, &fraglen) || !PACKET_get_sub_packet(&msgpkt, &msgpayload, fraglen)
        ...
        if (s->msg_callback)
            s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, data, fraglen + DTLS1_HM_HEADER_LENGTH, s, s->msg_callback_arg);
        ...
        if (!PACKET_forward(&msgpayload, SSL3_RANDOM_SIZE) || !PACKET_get_length_prefixed_1(&msgpayload, &session)
        ...
        if (PACKET_remaining(&cookiepkt) == 0) {
            ...
            if (s->ctx->app_verify_cookie_cb(s, PACKET_data(&cookiepkt), (unsigned int)PACKET_remaining(&cookiepkt)) == 0) {
                /*
        ...
        if (next == LISTEN_SEND_VERIFY_REQUEST) {
             ...
             */
            version = (s->method->version == DTLS_ANY_VERSION) ? DTLS1_VERSION : s->version;

            /* Construct the record and message headers */
            if (!WPACKET_init_static_len(&wpkt, wbuf, ssl_get_max_send_fragment(s) + DTLS1_RT_HEADER_LENGTH, 0)
                ...
                || !WPACKET_close(&wpkt)
                /* Close record body */
                || !WPACKET_close(&wpkt) || !WPACKET_get_total_written(&wpkt, &wreclen) || !WPACKET_finish(&wpkt)) {
                ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
             ...
             */
            memcpy(&wbuf[DTLS1_RT_HEADER_LENGTH + 1], &wbuf[DTLS1_RT_HEADER_LENGTH + DTLS1_HM_HEADER_LENGTH - 3], 3);
    ...
    if (s->d1->mtu < dtls1_min_mtu(s)) {
        if (!(SSL_get_options(s) & SSL_OP_NO_QUERY_MTU)) {
            ...
            if (s->d1->mtu < dtls1_min_mtu(s)) {
                ...
                BIO_ctrl(SSL_get_wbio(s), BIO_CTRL_DGRAM_SET_MTU, (long)s->d1->mtu, NULL);
            }
        } else
            return 0;
    }
...
size_t DTLS_get_data_mtu(const SSL* s)
{
    ...
    if (!ssl_cipher_get_overhead(ciph, &mac_overhead, &int_overhead, &blocksize, &ext_overhead))
        return 0;
