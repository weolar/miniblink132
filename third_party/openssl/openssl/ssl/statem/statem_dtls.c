...
static void dtls1_set_message_header_int(SSL* s, unsigned char mt, size_t len, unsigned short seq_num, size_t frag_off, size_t frag_len);
...
int dtls1_do_write(SSL* s, int type)
{
    ...
    if (s->write_hash) {
        if (s->enc_write_ctx && (EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(s->enc_write_ctx)) & EVP_CIPH_FLAG_AEAD_CIPHER) != 0)
            mac_size = 0;
        else
    ...
    while (s->init_num > 0) {
        ...
        ret = dtls1_write_bytes(s, type, &s->init_buf->data[s->init_off], len, &written);
        if (ret <= 0) {
            /*
...
again:
    if (!dtls_get_reassembled_message(s, &errtype, &tmplen)) {
        if (errtype == DTLS1_HM_BAD_FRAGMENT || errtype == DTLS1_HM_FRAGMENT_RETRY) {
...
int dtls_get_message_body(SSL* s, size_t* len)
{
    ...
    if (s->msg_callback)
        s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, s->init_buf->data, s->init_num + DTLS1_HM_HEADER_LENGTH, s, s->msg_callback_arg);
...
static int dtls1_preprocess_fragment(SSL* s, struct hm_header_st* msg_hdr)
{
    ...
    /* sanity checking */
    if ((frag_off + frag_len) > msg_len || msg_len > dtls1_max_handshake_message_len(s)) {
...
static int dtls1_retrieve_buffered_fragment(SSL* s, size_t* len)
{
    ...
    do {
        ...
        if (frag->msg_header.seq < s->d1->handshake_read_seq) {
            ...
            if (!s->server || frag->msg_header.seq != 0 || s->d1->handshake_read_seq != 1 || s->statem.hand_state != DTLS_ST_SW_HELLO_VERIFY_REQUEST) {
                /*
...
static int dtls1_reassemble_fragment(SSL* s, const struct hm_header_st* msg_hdr)
{
    ...
    if (frag->reassembly == NULL) {
        ...
        while (frag_len) {
            i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, NULL, devnull, frag_len > sizeof(devnull) ? sizeof(devnull) : frag_len, 0, &readbytes);
            if (i <= 0)
                goto err;
            frag_len -= readbytes;
        }
    ...
    i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, NULL, frag->fragment + msg_hdr->frag_off, frag_len, 0, &readbytes);
...
static int dtls1_process_out_of_seq_message(SSL* s, const struct hm_header_st* msg_hdr)
{
    int i = -1;
    hm_fragment* frag = NULL;
    pitem* item = NULL;
    ...
    if (msg_hdr->seq <= s->d1->handshake_read_seq || msg_hdr->seq > s->d1->handshake_read_seq + 10 || item != NULL
        ...
        if (frag_len) {
             ...
             * read the body of the fragment (header has already been read
             */
            i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, NULL, frag->fragment, frag_len, 0, &readbytes);
            if (i <= 0 || readbytes != frag_len)
                i = -1;
            if (i <= 0)
                goto err;
...
redo:
    ...
    i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, &recvd_type, wire, DTLS1_HM_HEADER_LENGTH, 0, &readbytes);
    ...
    if (msg_hdr.seq != s->d1->handshake_read_seq) {
        if (!s->server || msg_hdr.seq != 0 || s->d1->handshake_read_seq != 1 || wire[0] != SSL3_MT_CLIENT_HELLO
    ...
    if (!s->server && s->d1->r_msg_hdr.frag_off == 0 && s->statem.hand_state != TLS_ST_OK && wire[0] == SSL3_MT_HELLO_REQUEST) {
        /*
    ...
    if (frag_len > 0) {
        ...
        i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, NULL, &p[frag_off], frag_len, 0, &readbytes);
...
int dtls1_buffer_message(SSL* s, int is_ccs)
{
    ...
    if (is_ccs) {
        ...
        if (!ossl_assert(s->d1->w_msg_hdr.msg_len + ((s->version == DTLS1_BAD_VER) ? 3 : DTLS1_CCS_HEADER_LENGTH) == (unsigned int)s->init_num)) {
            dtls1_hm_fragment_free(frag);
            return 0;
        }
    } else {
...
int dtls1_set_handshake_header(SSL* s, WPACKET* pkt, int htype)
{
    ...
    if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) {
         ...
         * filled in later
         */
        if (!WPACKET_allocate_bytes(pkt, DTLS1_HM_HEADER_LENGTH, &header) || !WPACKET_start_sub_packet(pkt))
            return 0;
    }
