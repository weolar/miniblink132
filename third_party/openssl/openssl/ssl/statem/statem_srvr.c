...
static int ossl_statem_server13_read_transition(SSL* s, int mt)
{
    ...
    case TLS_ST_EARLY_DATA:
        if (s->hello_retry_request == SSL_HRR_PENDING) {
            ...
            break;
        } else if (s->ext.early_data == SSL_EARLY_DATA_ACCEPTED && !SSL_IS_QUIC(s)) {
    ...
    case TLS_ST_OK:
        ...
        if (mt == SSL3_MT_CERTIFICATE && s->post_handshake_auth == SSL_PHA_REQUESTED) {
            st->hand_state = TLS_ST_SR_CERT;
            return 1;
        }
            ...
            if (s->s3.tmp.cert_request) {
                if (s->version == SSL3_VERSION) {
                    if ((s->verify_mode & SSL_VERIFY_PEER) && (s->verify_mode & SSL_VERIFY_FAIL_IF_NO_PEER_CERT)) {
                         ...
                         * cert.
                         */
                        SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_PEER_DID_NOT_RETURN_A_CERTIFICATE);
                        return 0;
                    }
...
int send_certificate_request(SSL* s)
{
    if (
        ...
        /*
            ...
            */
        && (!SSL_IS_TLS13(s) || !(s->verify_mode & SSL_VERIFY_POST_HANDSHAKE) || s->post_handshake_auth == SSL_PHA_REQUEST_PENDING)
        /*
...
static WRITE_TRAN ossl_statem_server13_write_transition(SSL* s)
{
    ...
    case TLS_ST_SW_SRVR_HELLO:
        if ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0 && s->hello_retry_request != SSL_HRR_COMPLETE)
...
WORK_STATE ossl_statem_server_post_work(SSL* s, WORK_STATE wst)
{
    ...
    case TLS_ST_SW_SRVR_HELLO:
        if (SSL_IS_TLS13(s) && s->hello_retry_request == SSL_HRR_PENDING) {
            if ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) == 0 && statem_flush(s) != 1)
                return WORK_MORE_A;
            break;
        }
#ifndef OPENSSL_NO_SCTP
        if (SSL_IS_DTLS(s) && s->hit) {
            ...
            if (SSL_export_keying_material(s, sctpauthkey, sizeof(sctpauthkey), labelbuffer, labellen, NULL, 0, 0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return WORK_ERROR;
            }
...
#endif
        if (!SSL_IS_TLS13(s) || ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0 && s->hello_retry_request != SSL_HRR_COMPLETE))
            break;
        /* Fall through */
    ...
    case TLS_ST_SW_CHANGE:
        ...
        if (SSL_IS_TLS13(s)) {
            ...
            if (s->ext.early_data != SSL_EARLY_DATA_ACCEPTED
                && !s->method->ssl3_enc->change_cipher_state(s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_SERVER_READ)) {
                /* SSLfatal() already called */
                return WORK_ERROR;
            }
            /*
    ...
    case TLS_ST_SW_FINISHED:
        ...
        if (SSL_IS_TLS13(s)) {
            ...
            size_t dummy;
            if (!s->method->ssl3_enc->generate_master_secret(s, s->master_secret, s->handshake_secret, 0, &dummy)
                || !s->method->ssl3_enc->change_cipher_state(s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_SERVER_WRITE))
                /* SSLfatal() already called */
                return WORK_ERROR;
            ...
            if (SSL_IS_QUIC(s) && s->ext.early_data == SSL_EARLY_DATA_ACCEPTED) {
                ...
                if (!s->method->ssl3_enc->change_cipher_state(s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_SERVER_READ))
                    /* SSLfatal() already called */
                    return WORK_ERROR;
            }
#endif
    ...
    case TLS_ST_SW_SESSION_TICKET:
        ...
        if (SSL_IS_TLS13(s) && statem_flush(s) != 1) {
            if (SSL_get_error(s, 0) == SSL_ERROR_SYSCALL && conn_is_closed()) {
                /*
    ...
    if ((s->s3.tmp.new_cipher->algorithm_mkey & SSL_kSRP) && (s->srp_ctx.TLS_ext_srp_username_callback != NULL)) {
        if (s->srp_ctx.login == NULL) {
            ...
            if (ret == SSL3_AL_FATAL) {
                SSLfatal(s, al, al == SSL_AD_UNKNOWN_PSK_IDENTITY ? SSL_R_PSK_IDENTITY_NOT_FOUND : SSL_R_CLIENTHELLO_TLSEXT);
                return -1;
            }
        }
    }
...
static void ssl_check_for_safari(SSL* s, const CLIENTHELLO_MSG* hello)
{
    ...
    if (!PACKET_forward(&tmppkt, 2) || !PACKET_get_net_2(&tmppkt, &type) || !PACKET_get_length_prefixed_2(&tmppkt, &sni)) {
        return;
    }
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL* s, PACKET* pkt)
{
    ...
    if (clienthello->isv2) {
        ...
        if (!SSL_IS_FIRST_HANDSHAKE(s) || s->hello_retry_request != SSL_HRR_NONE) {
    ...
    if (clienthello->isv2) {
        ...
        if (!PACKET_get_net_2(pkt, &ciphersuite_len) || !PACKET_get_net_2(pkt, &session_id_len) || !PACKET_get_net_2(pkt, &challenge_len)) {
        ...
        if (!PACKET_get_sub_packet(pkt, &clienthello->ciphersuites, ciphersuite_len) || !PACKET_copy_bytes(pkt, clienthello->session_id, session_id_len)
        ...
        /* Regular ClientHello. */
        if (!PACKET_copy_bytes(pkt, clienthello->random, SSL3_RANDOM_SIZE) || !PACKET_get_length_prefixed_1(pkt, &session_id)
            || !PACKET_copy_all(&session_id, clienthello->session_id, SSL_MAX_SSL_SESSION_ID_LENGTH, &clienthello->session_id_len)) {
        ...
        if (SSL_IS_DTLS(s)) {
            ...
            if (!PACKET_copy_all(&cookie, clienthello->dtls_cookie, DTLS1_COOKIE_LENGTH, &clienthello->dtls_cookie_len)) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                goto err;
            }
            /*
        ...
        if (PACKET_remaining(pkt) == 0) {
            ...
            if (!PACKET_get_length_prefixed_2(pkt, &clienthello->extensions) || PACKET_remaining(pkt) != 0) {
                SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
                goto err;
            }
        }
    ...
    if (!PACKET_copy_all(&compression, clienthello->compressions, MAX_COMPRESSIONS_SIZE, &clienthello->compressions_len)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
    }
    ...
    if (!tls_collect_extensions(s, &extensions, SSL_EXT_CLIENT_HELLO, &clienthello->pre_proc_exts, &clienthello->pre_proc_exts_len, 1)) {
...
static int tls_early_post_process_client_hello(SSL* s)
{
    ...
    if (clienthello->isv2) {
        if (clienthello->legacy_version == SSL2_VERSION || (clienthello->legacy_version & 0xff00) != (SSL3_VERSION_MAJOR << 8)) {
            /*
    ...
    if (!ssl_cache_cipherlist(s, &clienthello->ciphersuites, clienthello->isv2)
        || !bytes_to_cipher_list(s, &clienthello->ciphersuites, &ciphers, &scsvs, clienthello->isv2, 1)) {
        /* SSLfatal() already called */
        goto err;
    }
        ...
        for (i = 0; i < sk_SSL_CIPHER_num(scsvs); i++) {
            ...
            if (SSL_CIPHER_get_id(c) == SSL3_CK_SCSV) {
                if (s->renegotiate) {
                    ...
                    SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_SCSV_RECEIVED_WHEN_RENEGOTIATING);
                    goto err;
                }
                 ...
                 */
                SSLfatal(s, SSL_AD_INAPPROPRIATE_FALLBACK, SSL_R_INAPPROPRIATE_FALLBACK);
                goto err;
            }
        }
    }
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (s->hello_retry_request == SSL_HRR_PENDING && (s->s3.tmp.new_cipher == NULL || s->s3.tmp.new_cipher->id != cipher->id)) {
            /*
    ...
    /* TLS extensions */
    if (!tls_parse_all_extensions(s, SSL_EXT_CLIENT_HELLO, clienthello->pre_proc_exts, NULL, 0, 1)) {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (!s->hit && s->version >= TLS1_VERSION && !SSL_IS_TLS13(s) && !SSL_IS_DTLS(s) && s->ext.session_secret_cb) {
        ...
        if (s->ext.session_secret_cb(s, s->session->master_key, &master_key_length, ciphers, &pref_cipher, s->ext.session_secret_cb_arg)
            && master_key_length > 0) {
            ...
            if (pref_cipher == NULL)
                pref_cipher = ssl3_choose_cipher(s, s->peer_ciphers, SSL_get_ciphers(s));
            if (pref_cipher == NULL) {
                SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_NO_SHARED_CIPHER);
                goto err;
            }
    ...
    /* This only happens if we have a cache hit */
        ...
        if (s->s3.tmp.new_compression == NULL) {
            SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_INVALID_COMPRESSION_ALGORITHM);
            goto err;
        }
...
int tls_handle_alpn(SSL* s)
{
    ...
    if (s->ctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
        int r = s->ctx->ext.alpn_select_cb(
            s, &selected, &selected_len, s->s3.alpn_proposed, (unsigned int)s->s3.alpn_proposed_len, s->ctx->ext.alpn_select_cb_arg);
        ...
        if (r == SSL_TLSEXT_ERR_OK) {
            ...
            if (s->session->ext.alpn_selected == NULL || selected_len != s->session->ext.alpn_selected_len
                ...
                if (!s->hit) {
                    ...
                    s->session->ext.alpn_selected = OPENSSL_memdup(selected, selected_len);
        ...
        if (!s->hit || SSL_IS_TLS13(s)) {
            ...
            if (!s->hit) {
                ...
                if (s->not_resumable_session_cb != NULL)
                    s->session->not_resumable = s->not_resumable_session_cb(s, ((s->s3.tmp.new_cipher->algorithm_mkey & (SSL_kDHE | SSL_kECDHE)) != 0));
...
int tls_construct_server_hello(SSL* s, WPACKET* pkt)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, version)
        /*
                ...
                */
        || !WPACKET_memcpy(pkt, s->hello_retry_request == SSL_HRR_PENDING ? hrrrandom : s->s3.server_random, SSL3_RANDOM_SIZE)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    if (!WPACKET_sub_memcpy_u8(pkt, session_id, sl) || !s->method->put_cipher_by_char(s->s3.tmp.new_cipher, pkt, &len) || !WPACKET_put_bytes_u8(pkt, compm)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }

    if (!tls_construct_extensions(s, pkt,
            s->hello_retry_request == SSL_HRR_PENDING ? SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST
                                                      : (SSL_IS_TLS13(s) ? SSL_EXT_TLS1_3_SERVER_HELLO : SSL_EXT_TLS1_2_SERVER_HELLO),
            NULL, 0)) {
        /* SSLfatal() already called */
        return 0;
    }
    ...
    } else if (!(s->verify_mode & SSL_VERIFY_PEER) && !ssl3_digest_cached_records(s, 0)) {
        /* SSLfatal() already called */;
        return 0;
    }
...
int tls_construct_server_key_exchange(SSL* s, WPACKET* pkt)
{
    ...
    if (type & (SSL_kPSK | SSL_kRSAPSK)) {
        ...
        if (type & (SSL_kDHE | SSL_kDHEPSK)) {
            ...
            if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(pkdhp), 0, pkdhp)) {
            ...
            freer = 1;
            if (!EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_P, &r[0]) || !EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_G, &r[1])
                || !EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_PUB_KEY, &r[2])) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                goto err;
            }
            ...
            if (curve_id == 0) {
                SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_UNSUPPORTED_ELLIPTIC_CURVE);
                goto err;
            }
            ...
            encodedlen = EVP_PKEY_get1_encoded_public_key(s->s3.tmp.pkey, &encodedPoint);
            if (encodedlen == 0) {
    ...
    if (((s->s3.tmp.new_cipher->algorithm_auth & (SSL_aNULL | SSL_aSRP)) != 0) || ((s->s3.tmp.new_cipher->algorithm_mkey & SSL_PSK)) != 0) {
        lu = NULL;
    } else if (lu == NULL) {
...
#ifndef OPENSSL_NO_PSK
    if (type & SSL_PSK) {
        size_t len = (s->cert->psk_identity_hint == NULL) ? 0 : strlen(s->cert->psk_identity_hint);

        /*
         * It should not happen that len > PSK_MAX_IDENTITY_LEN - we already
         * checked this when we set the identity hint - but just in case
         */
        if (len > PSK_MAX_IDENTITY_LEN || !WPACKET_sub_memcpy_u16(pkt, s->cert->psk_identity_hint, len)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
    }
    ...
    for (i = 0; i < 4 && r[i] != NULL; i++) {
        ...
        if (!WPACKET_allocate_bytes(pkt, BN_num_bytes(r[i]), &binval) || !WPACKET_close(pkt)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
    ...
    if (type & (SSL_kECDHE | SSL_kECDHEPSK)) {
         ...
         * point itself
         */
        if (!WPACKET_put_bytes_u8(pkt, NAMED_CURVE_TYPE) || !WPACKET_put_bytes_u8(pkt, 0) || !WPACKET_put_bytes_u8(pkt, curve_id)
    ...
    if (lu != NULL) {
        ...
        if (EVP_DigestSignInit_ex(md_ctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md), s->ctx->libctx, s->ctx->propq, pkey, NULL) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
        ...
        if (EVP_DigestSign(md_ctx, NULL, &siglen, tbs, tbslen) <= 0 || !WPACKET_sub_reserve_bytes_u16(pkt, siglen, &sigbytes1)
            || EVP_DigestSign(md_ctx, sigbytes1, &siglen, tbs, tbslen) <= 0 || !WPACKET_sub_allocate_bytes_u16(pkt, siglen, &sigbytes2)
            || sigbytes1 != sigbytes2) {
            OPENSSL_free(tbs);
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
...
int tls_construct_certificate_request(SSL* s, WPACKET* pkt)
{
    if (SSL_IS_TLS13(s)) {
        ...
        if (s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
            ...
            if (RAND_bytes_ex(s->ctx->libctx, s->pha_context, s->pha_context_len, 0) <= 0 || !WPACKET_sub_memcpy_u8(pkt, s->pha_context, s->pha_context_len)) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
        ...
        if (!tls_construct_extensions(s, pkt, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, NULL, 0)) {
            /* SSLfatal() already called */
            return 0;
        }
        goto done;
    ...
    if (!WPACKET_start_sub_packet_u8(pkt) || !ssl3_get_req_cert_type(s, pkt) || !WPACKET_close(pkt)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    if (SSL_USE_SIGALGS(s)) {
        ...
        if (!WPACKET_start_sub_packet_u16(pkt) || !WPACKET_set_flags(pkt, WPACKET_FLAGS_NON_ZERO_LENGTH) || !tls12_copy_sigalgs(s, pkt, psigs, nl)
            || !WPACKET_close(pkt)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
...
static int tls_process_cke_rsa(SSL* s, PACKET* pkt)
{
    ...
    *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION, (unsigned int*)&s->client_version);
    if ((s->options & SSL_OP_TLS_ROLLBACK_BUG) != 0)
        *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION, (unsigned int*)&s->version);
    *p++ = OSSL_PARAM_construct_end();

    if (!EVP_PKEY_CTX_set_params(ctx, params)
        || EVP_PKEY_decrypt(ctx, rsa_decrypt, &outlen, PACKET_data(&enc_premaster), PACKET_remaining(&enc_premaster)) <= 0) {
        SSLfatal(s, SSL_AD_DECRYPT_ERROR, SSL_R_DECRYPTION_FAILED);
        goto err;
    }
    ...
    if (!ssl_generate_master_secret(s, rsa_decrypt, SSL_MAX_MASTER_KEY_LENGTH, 0)) {
        /* SSLfatal() already called */
        goto err;
    }
...
static int tls_process_cke_gost(SSL* s, PACKET* pkt)
{
    ...
    if (pKX == NULL || pKX->kxBlob == NULL || ASN1_TYPE_get(pKX->kxBlob) != V_ASN1_SEQUENCE) {
...
static int tls_process_cke_gost18(SSL* s, PACKET* pkt)
{
    ...
    /* Get our certificate private key */
    pk = s->cert->pkeys[SSL_PKEY_GOST12_512].privatekey != NULL ? s->cert->pkeys[SSL_PKEY_GOST12_512].privatekey
    ...
    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_DECRYPT, EVP_PKEY_CTRL_SET_IV, 32, rnd_dgst) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }

    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_DECRYPT, EVP_PKEY_CTRL_CIPHER, cipher_nid, NULL) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }
...
MSG_PROCESS_RETURN tls_process_client_certificate(SSL* s, PACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s)
        && (!PACKET_get_length_prefixed_1(pkt, &context) || (s->pha_context == NULL && PACKET_remaining(&context) != 0)
    ...
    if (sk_X509_num(sk) <= 0) {
        ...
        if (s->version == SSL3_VERSION) {
            SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_NO_CERTIFICATES_RETURNED);
            goto err;
        }
        /* Fail for TLS only if we required a certificate */
        else if ((s->verify_mode & SSL_VERIFY_PEER) && (s->verify_mode & SSL_VERIFY_FAIL_IF_NO_PEER_CERT)) {
            SSLfatal(s, SSL_AD_CERTIFICATE_REQUIRED, SSL_R_PEER_DID_NOT_RETURN_A_CERTIFICATE);
            goto err;
        }
    ...
    if (SSL_IS_TLS13(s)) {
        if (!ssl_handshake_hash(s, s->cert_verify_hash, sizeof(s->cert_verify_hash), &s->cert_verify_hash_len)) {
            /* SSLfatal() already called */
            goto err;
        }
...
static int create_ticket_prequel(SSL* s, WPACKET* pkt, uint32_t age_add, unsigned char* tick_nonce)
{
...
#define ONE_WEEK_SEC (7 * 24 * 60 * 60)
    ...
    if (SSL_IS_TLS13(s)) {
        if (!WPACKET_put_bytes_u32(pkt, age_add) || !WPACKET_sub_memcpy_u8(pkt, tick_nonce, TICKET_NONCE_SIZE)) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
    }
 ...
 * -1 on fatal error
 */
static int construct_stateless_ticket(SSL* s, WPACKET* pkt, uint32_t age_add, unsigned char* tick_nonce)
{
...
#endif
    {
        ...
        if (tctx->ext.ticket_key_evp_cb != NULL)
            ret = tctx->ext.ticket_key_evp_cb(s, key_name, iv, ctx, ssl_hmac_get0_EVP_MAC_CTX(hctx), 1);
#ifndef OPENSSL_NO_DEPRECATED_3_0
        else if (tctx->ext.ticket_key_cb != NULL)
            /* if 0 is returned, write an empty ticket */
            ret = tctx->ext.ticket_key_cb(s, key_name, iv, ctx, ssl_hmac_get0_HMAC_CTX(hctx), 1);
#endif
        ...
        if (ret == 0) {
            ...
            if (!WPACKET_put_bytes_u32(pkt, 0) || !WPACKET_put_bytes_u16(pkt, 0)) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                goto err;
            }
            OPENSSL_free(senc);
    ...
    if (!WPACKET_get_total_written(pkt, &macoffset)
        ...
        || !EVP_EncryptUpdate(ctx, encdata1, &len, senc, slen) || !WPACKET_allocate_bytes(pkt, len, &encdata2) || encdata1 != encdata2
        || !EVP_EncryptFinal(ctx, encdata1 + len, &lenfinal) || !WPACKET_allocate_bytes(pkt, lenfinal, &encdata2) || encdata1 + len != encdata2
        || len + lenfinal > slen + EVP_MAX_BLOCK_LENGTH || !WPACKET_get_total_written(pkt, &macendoffset)
        || !ssl_hmac_update(hctx, (unsigned char*)s->init_buf->data + macoffset, macendoffset - macoffset)
        || !WPACKET_reserve_bytes(pkt, EVP_MAX_MD_SIZE, &macdata1) || !ssl_hmac_final(hctx, macdata1, &hlen, EVP_MAX_MD_SIZE) || hlen > EVP_MAX_MD_SIZE
...
static int construct_stateful_ticket(SSL* s, WPACKET* pkt, uint32_t age_add, unsigned char* tick_nonce)
{
    if (!create_ticket_prequel(s, pkt, age_add, tick_nonce)) {
        /* SSLfatal() already called */
        return 0;
    ...
    if (!WPACKET_memcpy(pkt, s->session->session_id, s->session->session_id_length) || !WPACKET_close(pkt)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
...
int tls_construct_new_session_ticket(SSL* s, WPACKET* pkt)
{
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (RAND_bytes_ex(s->ctx->libctx, age_add_u.age_add_c, sizeof(age_add_u), 0) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
        ...
        if (!tls13_hkdf_expand(
                s, md, s->resumption_master_secret, nonce_label, sizeof(nonce_label) - 1, tick_nonce, TICKET_NONCE_SIZE, s->session->master_key, hashlen, 1)) {
            /* SSLfatal() already called */
            goto err;
        }
     ...
     */
    if (SSL_IS_TLS13(s) && ((s->options & SSL_OP_NO_TICKET) != 0 || (s->max_early_data > 0 && (s->options & SSL_OP_NO_ANTI_REPLAY) == 0))) {
        ...
        tmpret = construct_stateless_ticket(s, pkt, age_add_u.age_add, tick_nonce);
        if (tmpret != 1) {
            if (tmpret == 0) {
...
{
    if (!WPACKET_put_bytes_u8(pkt, s->ext.status_type) || !WPACKET_sub_memcpy_u24(pkt, s->ext.ocsp.resp, s->ext.ocsp.resp_len)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
...
static int tls_construct_encrypted_extensions(SSL* s, WPACKET* pkt)
{
    if (!tls_construct_extensions(s, pkt, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, NULL, 0)) {
        /* SSLfatal() already called */
        return 0;
    }
...
MSG_PROCESS_RETURN tls_process_end_of_early_data(SSL* s, PACKET* pkt)
{
    ...
    if (s->early_data_state != SSL_EARLY_DATA_READING && s->early_data_state != SSL_EARLY_DATA_READ_RETRY) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return MSG_PROCESS_ERROR;
    }
