...
#define check_fatal(s)                                                                                                                                         \
    do {                                                                                                                                                       \
        if (!ossl_assert((s)->statem.in_init && (s)->statem.state == MSG_FLOW_ERROR))                                                                          \
...
int ossl_statem_skip_early_data(SSL* s)
{
    ...
    if (!s->server || s->statem.hand_state != TLS_ST_EARLY_DATA || s->hello_retry_request == SSL_HRR_COMPLETE)
        return 0;
...
static SUB_STATE_RETURN read_state_machine(SSL* s)
{
    ...
    while (1) {
        ...
        case READ_STATE_HEADER:
            ...
            if (!SSL_IS_DTLS(s) && !SSL_IS_QUIC(s) && s->s3.tmp.message_size > 0 && !grow_init_buf(s, s->s3.tmp.message_size + SSL3_HM_HEADER_LENGTH)) {
...
static SUB_STATE_RETURN write_state_machine(SSL* s)
{
    ...
    int (*get_construct_message_f)(SSL* s, WPACKET* pkt, int (**confunc)(SSL* s, WPACKET* pkt), int* mt);
    void (*cb)(const SSL* ssl, int type, int val) = NULL;
