...
#define c2l(c, l)                                                                                                                                              \
    (l = ((unsigned long)(*((c)++))), l |= (((unsigned long)(*((c)++))) << 8), l |= (((unsigned long)(*((c)++))) << 16),                                       \
...
#define l2c(l, c)                                                                                                                                              \
    (*((c)++) = (unsigned char)(((l)) & 0xff), *((c)++) = (unsigned char)(((l) >> 8) & 0xff), *((c)++) = (unsigned char)(((l) >> 16) & 0xff),                  \
        *((c)++) = (unsigned char)(((l) >> 24) & 0xff))

#define n2l(c, l)                                                                                                                                              \
    (l = ((unsigned long)(*((c)++))) << 24, l |= ((unsigned long)(*((c)++))) << 16, l |= ((unsigned long)(*((c)++))) << 8, l |= ((unsigned long)(*((c)++))))

#define n2l8(c, l)                                                                                                                                             \
    (l = ((uint64_t)(*((c)++))) << 56, l |= ((uint64_t)(*((c)++))) << 48, l |= ((uint64_t)(*((c)++))) << 40, l |= ((uint64_t)(*((c)++))) << 32,                \
        l |= ((uint64_t)(*((c)++))) << 24, l |= ((uint64_t)(*((c)++))) << 16, l |= ((uint64_t)(*((c)++))) << 8, l |= ((uint64_t)(*((c)++))))

#define l2n(l, c)                                                                                                                                              \
    (*((c)++) = (unsigned char)(((l) >> 24) & 0xff), *((c)++) = (unsigned char)(((l) >> 16) & 0xff), *((c)++) = (unsigned char)(((l) >> 8) & 0xff),            \
        *((c)++) = (unsigned char)(((l)) & 0xff))

#define l2n6(l, c)                                                                                                                                             \
    (*((c)++) = (unsigned char)(((l) >> 40) & 0xff), *((c)++) = (unsigned char)(((l) >> 32) & 0xff), *((c)++) = (unsigned char)(((l) >> 24) & 0xff),           \
        *((c)++) = (unsigned char)(((l) >> 16) & 0xff), *((c)++) = (unsigned char)(((l) >> 8) & 0xff), *((c)++) = (unsigned char)(((l)) & 0xff))

#define l2n8(l, c)                                                                                                                                             \
    (*((c)++) = (unsigned char)(((l) >> 56) & 0xff), *((c)++) = (unsigned char)(((l) >> 48) & 0xff), *((c)++) = (unsigned char)(((l) >> 40) & 0xff),           \
        *((c)++) = (unsigned char)(((l) >> 32) & 0xff), *((c)++) = (unsigned char)(((l) >> 24) & 0xff), *((c)++) = (unsigned char)(((l) >> 16) & 0xff),        \
...
#define l2n3(l, c)                                                                                                                                             \
    (((c)[0] = (unsigned char)(((l) >> 16) & 0xff), (c)[1] = (unsigned char)(((l) >> 8) & 0xff), (c)[2] = (unsigned char)(((l)) & 0xff)), (c) += 3)
...
#define SSL_IS_TLS13(s) (!SSL_IS_DTLS(s) && (s)->method->version >= TLS1_3_VERSION && (s)->method->version != TLS_ANY_VERSION)

#define SSL_TREAT_AS_TLS13(s)                                                                                                                                  \
    (SSL_IS_TLS13(s) || (s)->early_data_state == SSL_EARLY_DATA_CONNECTING || (s)->early_data_state == SSL_EARLY_DATA_CONNECT_RETRY                            \
        || (s)->early_data_state == SSL_EARLY_DATA_WRITING || (s)->early_data_state == SSL_EARLY_DATA_WRITE_RETRY                                              \
        || (s)->hello_retry_request == SSL_HRR_PENDING)

#define SSL_IS_FIRST_HANDSHAKE(S) ((s)->s3.tmp.finish_md_len == 0 || (s)->s3.tmp.peer_finish_md_len == 0)

/* See if we need explicit IV */
#define SSL_USE_EXPLICIT_IV(s) (s->method->ssl3_enc->enc_flags & SSL_ENC_FLAG_EXPLICIT_IV)
/*
 * See if we use signature algorithms extension and signature algorithm
 * before signatures.
 */
#define SSL_USE_SIGALGS(s) (s->method->ssl3_enc->enc_flags & SSL_ENC_FLAG_SIGALGS)
/*
 * Allow TLS 1.2 ciphersuites: applies to DTLS 1.2 as well as TLS 1.2: may
 * apply to others in future.
 */
#define SSL_USE_TLS1_2_CIPHERS(s) (s->method->ssl3_enc->enc_flags & SSL_ENC_FLAG_TLS1_2_CIPHERS)
/*
...
#define IS_MAX_FRAGMENT_LENGTH_EXT_VALID(value) (((value) >= TLSEXT_max_fragment_length_512) && ((value) <= TLSEXT_max_fragment_length_4096))
#define USE_MAX_FRAGMENT_LENGTH_EXT(session) IS_MAX_FRAGMENT_LENGTH_EXT_VALID(session->ext.max_fragment_len_mode)
#define GET_MAX_FRAGMENT_LENGTH(session) (512U << (session->ext.max_fragment_len_mode - 1))
...
struct ssl_ctx_st {
    ...
    struct {
        ...
        int (*ticket_key_cb)(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, HMAC_CTX* hctx, int enc);
#endif
        int (*ticket_key_evp_cb)(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, EVP_MAC_CTX* hctx, int enc);
...
struct ssl_st {
    ...
    enum { SSL_HRR_NONE = 0, SSL_HRR_PENDING, SSL_HRR_COMPLETE } hello_retry_request;
    ...
    struct {
        ...
        void (*debug_cb)(SSL* s, int client_server, int type, const unsigned char* data, int len, void* arg);
        void* debug_arg;
        char* hostname;
        /* certificate status request info */
...
#define SSL_CERT_FLAGS_CHECK_TLS_STRICT (SSL_CERT_FLAG_SUITEB_128_LOS | SSL_CERT_FLAG_TLS_STRICT)

typedef enum { ENDPOINT_CLIENT = 0, ENDPOINT_SERVER, ENDPOINT_BOTH } ENDPOINT;
...
#define ssl_set_handshake_header(s, pkt, htype) s->method->ssl3_enc->set_handshake_header((s), (pkt), (htype))
#define ssl_close_construct_packet(s, pkt, htype) s->method->ssl3_enc->close_construct_packet((s), (pkt), (htype))
...
typedef enum downgrade_en { DOWNGRADE_NONE, DOWNGRADE_TO_1_2, DOWNGRADE_TO_1_1 } DOWNGRADE;
...
static ossl_inline int ssl_has_cert(const SSL* s, int idx)
{
    if (idx < 0 || idx >= SSL_PKEY_NUM)
        return 0;
    return s->cert->pkeys[idx].x509 != NULL && s->cert->pkeys[idx].privatekey != NULL;
}

static ossl_inline void tls1_get_peer_groups(SSL* s, const uint16_t** pgroups, size_t* pgroupslen)
{
...
__owur SSL_SESSION* lookup_sess_in_cache(SSL* s, const unsigned char* sess_id, size_t sess_id_len);
...
__owur STACK_OF(SSL_CIPHER)
    * ssl_create_cipher_list(SSL_CTX* ctx, STACK_OF(SSL_CIPHER) * tls13_ciphersuites, STACK_OF(SSL_CIPHER) * *cipher_list,
...
__owur int bytes_to_cipher_list(SSL* s, PACKET* cipher_suites, STACK_OF(SSL_CIPHER) * *skp, STACK_OF(SSL_CIPHER) * *scsvs, int sslv2format, int fatal);
void ssl_update_cache(SSL* s, int mode);
__owur int ssl_cipher_get_evp_cipher(SSL_CTX* ctx, const SSL_CIPHER* sslc, const EVP_CIPHER** enc);
__owur int ssl_cipher_get_evp(
    SSL_CTX* ctxc, const SSL_SESSION* s, const EVP_CIPHER** enc, const EVP_MD** md, int* mac_pkey_type, size_t* mac_secret_size, SSL_COMP** comp, int use_etm);
__owur int ssl_cipher_get_overhead(const SSL_CIPHER* c, size_t* mac_overhead, size_t* int_overhead, size_t* blocksize, size_t* ext_overhead);
__owur int ssl_cert_is_disabled(SSL_CTX* ctx, size_t idx);
__owur const SSL_CIPHER* ssl_get_cipher_by_char(SSL* ssl, const unsigned char* ptr, int all);
...
__owur const SSL_CERT_LOOKUP* ssl_cert_lookup_by_pkey(const EVP_PKEY* pk, size_t* pidx);
__owur const SSL_CERT_LOOKUP* ssl_cert_lookup_by_idx(size_t idx);

int ssl_undefined_function(SSL* s);
__owur int ssl_undefined_void_function(void);
__owur int ssl_undefined_const_function(const SSL* s);
__owur int ssl_get_server_cert_serverinfo(SSL* s, const unsigned char** serverinfo, size_t* serverinfo_length);
...
__owur int ssl_fill_hello_random(SSL* s, int server, unsigned char* field, size_t len, DOWNGRADE dgrd);
__owur int ssl_generate_master_secret(SSL* s, unsigned char* pms, size_t pmslen, int free_pms);
__owur EVP_PKEY* ssl_generate_pkey(SSL* s, EVP_PKEY* pm);
__owur int ssl_gensecret(SSL* s, unsigned char* pms, size_t pmslen);
__owur int ssl_derive(SSL* s, EVP_PKEY* privkey, EVP_PKEY* pubkey, int genmaster);
__owur int ssl_decapsulate(SSL* s, EVP_PKEY* privkey, const unsigned char* ct, size_t ctlen, int gensecret);
__owur int ssl_encapsulate(SSL* s, EVP_PKEY* pubkey, unsigned char** ctp, size_t* ctlenp, int gensecret);
__owur EVP_PKEY* ssl_dh_to_pkey(DH* dh);
__owur int ssl_set_tmp_ecdh_groups(uint16_t** pext, size_t* pextlen, void* key);
...
__owur int ssl3_put_cipher_by_char(const SSL_CIPHER* c, WPACKET* pkt, size_t* len);
...
__owur int ssl3_generate_master_secret(SSL* s, unsigned char* out, unsigned char* p, size_t len, size_t* secret_size);
...
__owur int dtls_raw_hello_verify_request(WPACKET* pkt, unsigned char* cookie, size_t cookie_len);
...
__owur size_t tls1_final_finish_mac(SSL* s, const char* str, size_t slen, unsigned char* p);
__owur int tls1_generate_master_secret(SSL* s, unsigned char* out, unsigned char* p, size_t len, size_t* secret_size);
__owur int tls13_setup_key_block(SSL* s);
__owur size_t tls13_final_finish_mac(SSL* s, const char* str, size_t slen, unsigned char* p);
__owur int tls13_change_cipher_state(SSL* s, int which);
__owur int tls13_update_key(SSL* s, int send);
__owur int tls13_hkdf_expand(SSL* s, const EVP_MD* md, const unsigned char* secret, const unsigned char* label, size_t labellen, const unsigned char* data,
    size_t datalen, unsigned char* out, size_t outlen, int fatal);
__owur int tls13_derive_key(SSL* s, const EVP_MD* md, const unsigned char* secret, unsigned char* key, size_t keylen);
__owur int tls13_derive_iv(SSL* s, const EVP_MD* md, const unsigned char* secret, unsigned char* iv, size_t ivlen);
__owur int tls13_derive_finishedkey(SSL* s, const EVP_MD* md, const unsigned char* secret, unsigned char* fin, size_t finlen);
int tls13_generate_secret(
    SSL* s, const EVP_MD* md, const unsigned char* prevsecret, const unsigned char* insecret, size_t insecretlen, unsigned char* outsecret);
__owur int tls13_generate_handshake_secret(SSL* s, const unsigned char* insecret, size_t insecretlen);
__owur int tls13_generate_master_secret(SSL* s, unsigned char* out, unsigned char* prev, size_t prevlen, size_t* secret_size);
__owur int tls1_export_keying_material(
    SSL* s, unsigned char* out, size_t olen, const char* label, size_t llen, const unsigned char* p, size_t plen, int use_context);
__owur int tls13_export_keying_material(
    SSL* s, unsigned char* out, size_t olen, const char* label, size_t llen, const unsigned char* context, size_t contextlen, int use_context);
__owur int tls13_export_keying_material_early(
    SSL* s, unsigned char* out, size_t olen, const char* label, size_t llen, const unsigned char* context, size_t contextlen);
...
__owur SSL_TICKET_STATUS tls_decrypt_ticket(
    SSL* s, const unsigned char* etick, size_t eticklen, const unsigned char* sess_id, size_t sesslen, SSL_SESSION** psess);
 ...
 * |encrypted_premaster|.
 */
__owur int ssl_log_rsa_client_key_exchange(
    SSL* ssl, const uint8_t* encrypted_premaster, size_t encrypted_premaster_len, const uint8_t* premaster, size_t premaster_len);
...
/* ktls.c */
int ktls_check_supported_cipher(const SSL* s, const EVP_CIPHER* c, const EVP_CIPHER_CTX* dd);
int ktls_configure_crypto(const SSL* s, const EVP_CIPHER* c, EVP_CIPHER_CTX* dd, void* rl_sequence, ktls_crypto_info_t* crypto_info, unsigned char** rec_seq,
    unsigned char* iv, unsigned char* key, unsigned char* mac_key, size_t mac_secret_size);
#endif

/* s3_cbc.c */
__owur char ssl3_cbc_record_digest_supported(const EVP_MD_CTX* ctx);
__owur int ssl3_cbc_digest_record(const EVP_MD* md, unsigned char* md_out, size_t* md_out_size, const unsigned char* header, const unsigned char* data,
    size_t data_size, size_t data_plus_mac_plus_padding_size, const unsigned char* mac_secret, size_t mac_secret_length, char is_sslv3);
