...
static char* file_get_pass(const UI_METHOD* ui_method, char* pass, size_t maxsize, const char* desc, const char* info, void* data)
{
    UI* ui = UI_new();
    char* prompt = NULL;
    ...
    if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
        ATTICerr(0, ERR_R_MALLOC_FAILURE);
        pass = NULL;
    } else if (UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, pass, 0, maxsize - 1) <= 0) {
...
static int file_fill_pem_pass_data(struct pem_pass_data* pass_data, const char* desc, const char* info, const UI_METHOD* ui_method, void* ui_data)
{
    if (pass_data == NULL)
        return 0;
...
static int file_get_pem_pass(char* buf, int num, int w, void* data)
{
    ...
    char* pass = file_get_pass(pass_data->ui_method, buf, num, pass_data->prompt_desc, pass_data->prompt_info, pass_data->data);
...
static OSSL_STORE_INFO* new_EMBEDDED(const char* new_pem_name, BUF_MEM* embedded)
{
    OSSL_STORE_INFO* info = NULL;
    struct embedded_st* data = NULL;

    if ((data = OPENSSL_zalloc(sizeof(*data))) == NULL || (info = OSSL_STORE_INFO_new(STORE_INFO_EMBEDDED, data)) == NULL) {
        ATTICerr(0, ERR_R_MALLOC_FAILURE);
        OPENSSL_free(data);
        return NULL;
    }
 ...
 *    a OSSL_STORE_INFO
 */
typedef OSSL_STORE_INFO* (*file_try_decode_fn)(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** handler_ctx,
    int* matchcount, const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq);
/*
 ...
 */
static OSSL_STORE_INFO* try_decode_PKCS12(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx, int* matchcount,
    const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (ctx == NULL) {
        ...
        if ((p12 = d2i_PKCS12(NULL, &blob, len)) != NULL) {
            ...
            *matchcount = 1;
            ...
            if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, "", 0) || PKCS12_verify_mac(p12, NULL, 0)) {
                pass = "";
            } else {
                if ((pass = file_get_pass(ui_method, tpass, PEM_BUFSIZE, "PKCS12 import", uri, ui_data)) == NULL) {
                ...
                if ((ctx = sk_OSSL_STORE_INFO_new_null()) != NULL) {
                    if (pkey != NULL) {
                        if ((osi_pkey = OSSL_STORE_INFO_new_PKEY(pkey)) != NULL
                            ...
                            && (pkey = NULL) == NULL && sk_OSSL_STORE_INFO_push(ctx, osi_pkey) != 0)
                            osi_pkey = NULL;
                        else
                            ok = 0;
                    }
                    if (ok && cert != NULL) {
                        if ((osi_cert = OSSL_STORE_INFO_new_CERT(cert)) != NULL
                            ...
                            && (cert = NULL) == NULL && sk_OSSL_STORE_INFO_push(ctx, osi_cert) != 0)
                            osi_cert = NULL;
                        else
                            ok = 0;
                    }
                    while (ok && sk_X509_num(chain) > 0) {
                        ...
                        if ((osi_ca = OSSL_STORE_INFO_new_CERT(ca)) != NULL && sk_X509_shift(chain) != NULL && sk_OSSL_STORE_INFO_push(ctx, osi_ca) != 0)
                            osi_ca = NULL;
                        else
                            ok = 0;
                    }
...
static FILE_HANDLER PKCS12_handler = {
    "PKCS12", try_decode_PKCS12, eof_PKCS12, destroy_ctx_PKCS12, 1 /* repeatable */
};
 ...
 */
static OSSL_STORE_INFO* try_decode_PKCS8Encrypted(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx,
    int* matchcount, const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    X509_SIG* p8 = NULL;
    ...
    *matchcount = 1;
    ...
    if ((pass = file_get_pass(ui_method, kbuf, PEM_BUFSIZE, "PKCS8 decrypt pass phrase", uri, ui_data)) == NULL) {
...
static FILE_HANDLER PKCS8Encrypted_handler = { "PKCS8Encrypted", try_decode_PKCS8Encrypted };
 ...
 * the PEM name).
 */
static OSSL_STORE_INFO* try_decode_PrivateKey(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx, int* matchcount,
    const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    OSSL_STORE_INFO* store_info = NULL;
    EVP_PKEY* pkey = NULL;
    ...
    if (pem_name != NULL) {
        if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
            ...
            *matchcount = 1;
            ...
            if ((slen = check_suffix(pem_name, "PRIVATE KEY")) > 0 && (ameth = EVP_PKEY_asn1_find_str(NULL, pem_name, slen)) != NULL
                && EVP_PKEY_asn1_get0_info(&pkey_id, NULL, NULL, NULL, NULL, ameth)) {
                *matchcount = 1;
                pkey = d2i_PrivateKey_ex(pkey_id, NULL, &blob, len, libctx, propq);
            }
        }
    } else {
        int i;
        ...
        while (curengine != NULL) {
            ...
            if (asn1meths != NULL) {
                ...
                for (i = 0; i < nids_n; i++) {
                    ...
                    if (!asn1meths(curengine, &ameth2, NULL, nids[i]) || !EVP_PKEY_asn1_get0_info(&pkey_id, NULL, &pkey_flags, NULL, NULL, ameth2)
                        || (pkey_flags & ASN1_PKEY_ALIAS) != 0)
                        continue;

                    ERR_set_mark(); /* prevent flooding error queue */
                    tmp_pkey = d2i_PrivateKey_ex(pkey_id, NULL, &tmp_blob, len, libctx, propq);
                    if (tmp_pkey != NULL) {
                        if (pkey != NULL)
        ...
        for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
            ...
            if (!EVP_PKEY_asn1_get0_info(&pkey_id, NULL, &pkey_flags, NULL, NULL, ameth) || (pkey_flags & ASN1_PKEY_ALIAS) != 0)
                continue;
 ...
 */
static OSSL_STORE_INFO* try_decode_PUBKEY(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx, int* matchcount,
    const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    OSSL_STORE_INFO* store_info = NULL;
    EVP_PKEY* pkey = NULL;
 ...
 */
static OSSL_STORE_INFO* try_decode_params(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx, int* matchcount,
    const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    OSSL_STORE_INFO* store_info = NULL;
    EVP_PKEY* pkey = NULL;
    ...
    if (pem_name != NULL) {
        ...
        if ((slen = check_suffix(pem_name, "PARAMETERS")) > 0 && (ameth = EVP_PKEY_asn1_find_str(NULL, pem_name, slen)) != NULL
            && EVP_PKEY_asn1_get0_info(&pkey_id, NULL, NULL, NULL, NULL, ameth)) {
            *matchcount = 1;
 ...
 */
static OSSL_STORE_INFO* try_decode_X509Certificate(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx,
    int* matchcount, const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    OSSL_STORE_INFO* store_info = NULL;
    X509* cert = NULL;
    ...
    if ((d2i_X509_AUX(&cert, &blob, len)) != NULL || (ignore_trusted && (d2i_X509(&cert, &blob, len)) != NULL)) {
        *matchcount = 1;
...
static FILE_HANDLER X509Certificate_handler = { "X509Certificate", try_decode_X509Certificate };

/*
 * X.509 CRL decoder.
 */
static OSSL_STORE_INFO* try_decode_X509CRL(const char* pem_name, const char* pem_header, const unsigned char* blob, size_t len, void** pctx, int* matchcount,
    const UI_METHOD* ui_method, void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    OSSL_STORE_INFO* store_info = NULL;
    X509_CRL* crl = NULL;
...
struct ossl_store_loader_ctx_st {
    ...
    enum { is_raw = 0, is_pem, is_dir } type;
    int errcnt;
...
static OSSL_STORE_LOADER_CTX* file_open_ex(
    const OSSL_STORE_LOADER* loader, const char* uri, OSSL_LIB_CTX* libctx, const char* propq, const UI_METHOD* ui_method, void* ui_data)
{
    ...
    for (i = 0, path = NULL; path == NULL && i < path_data_n; i++) {
        ...
        if (stat(path_data[i].path, &st) < 0) {
            ERR_raise_data(ERR_LIB_SYS, errno, "calling stat(%s)", path_data[i].path);
        } else {
    ...
    } else if ((ctx->_.file.file = BIO_new_file(path, "rb")) == NULL || !file_find_type(ctx)) {
...
static OSSL_STORE_LOADER_CTX* file_attach(
    const OSSL_STORE_LOADER* loader, BIO* bp, OSSL_LIB_CTX* libctx, const char* propq, const UI_METHOD* ui_method, void* ui_data)
{
    OSSL_STORE_LOADER_CTX* ctx = NULL;

    if ((ctx = OPENSSL_zalloc(sizeof(*ctx))) == NULL || (propq != NULL && (ctx->propq = OPENSSL_strdup(propq)) == NULL)) {
        ATTICerr(0, ERR_R_MALLOC_FAILURE);
        OSSL_STORE_LOADER_CTX_free(ctx);
        return NULL;
    }
...
static int file_find(OSSL_STORE_LOADER_CTX* ctx, const OSSL_STORE_SEARCH* search)
{
    /*
    ...
    if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
        ...
        hash = X509_NAME_hash_ex(OSSL_STORE_SEARCH_get0_name(search), NULL, NULL, NULL);
...
static OSSL_STORE_INFO* file_load_try_decode(OSSL_STORE_LOADER_CTX* ctx, const char* pem_name, const char* pem_header, unsigned char* data, size_t len,
    const UI_METHOD* ui_method, void* ui_data, int* matchcount)
{
...
again : {
    ...
    const FILE_HANDLER** matching_handlers = OPENSSL_zalloc(sizeof(*matching_handlers) * OSSL_NELEM(file_handlers));
    ...
    *matchcount = 0;
    for (i = 0; i < OSSL_NELEM(file_handlers); i++) {
        ...
        ERR_set_mark();
        tmp_result
            = handler->try_decode(pem_name, pem_header, data, len, &tmp_handler_ctx, &try_matchcount, ui_method, ui_data, ctx->uri, ctx->libctx, ctx->propq);
        /* avoid flooding error queue with low-level ASN.1 parse errors */
        err = ERR_peek_last_error();
        if (ERR_GET_LIB(err) == ERR_LIB_ASN1 && ERR_GET_REASON(err) == ERR_R_NESTED_ASN1_ERROR)
            ERR_pop_to_mark();
        else
            ERR_clear_last_mark();
    ...
    if (result != NULL && *matchcount == 1 && matching_handlers[0]->repeatable) {
...
err:
    ...
    if (result != NULL && (t = OSSL_STORE_INFO_get_type(result)) == STORE_INFO_EMBEDDED) {
...
static OSSL_STORE_INFO* file_load_try_repeat(OSSL_STORE_LOADER_CTX* ctx, const UI_METHOD* ui_method, void* ui_data)
{
    OSSL_STORE_INFO* result = NULL;
    int try_matchcount = 0;
    ...
    if (ctx->_.file.last_handler != NULL) {
        result = ctx->_.file.last_handler->try_decode(
            NULL, NULL, NULL, 0, &ctx->_.file.last_handler_ctx, &try_matchcount, ui_method, ui_data, ctx->uri, ctx->libctx, ctx->propq);
...
static int file_read_pem(
    BIO* bp, char** pem_name, char** pem_header, unsigned char** data, long* len, const UI_METHOD* ui_method, void* ui_data, const char* uri, int secure)
{
    int i = secure ? PEM_read_bio_ex(bp, pem_name, pem_header, data, len, PEM_FLAG_SECURE | PEM_FLAG_EAY_COMPATIBLE)
    ...
    if (strlen(*pem_header) > 10) {
        ...
        if (!PEM_get_EVP_CIPHER_INFO(*pem_header, &cipher) || !file_fill_pem_pass_data(&pass_data, "PEM pass phrase", uri, ui_method, ui_data)
...
static OSSL_STORE_INFO* file_try_read_msblob(BIO* bp, int* matchcount)
{
...

    {
        ...
        if (BIO_buffer_peek(bp, peekbuf, sizeof(peekbuf)) <= 0)
            return 0;
        if (ossl_do_blob_header(&p, sizeof(peekbuf), &magic, &bitlen, &isdss, &ispub) <= 0)
            return 0;
    }
    ...
    {
        EVP_PKEY* tmp = ispub ? b2i_PublicKey_bio(bp) : b2i_PrivateKey_bio(bp);

        if (tmp == NULL || (result = OSSL_STORE_INFO_new_PKEY(tmp)) == NULL) {
            EVP_PKEY_free(tmp);
            return 0;
        }
    }
...
static OSSL_STORE_INFO* file_try_read_PVK(BIO* bp, const UI_METHOD* ui_method, void* ui_data, const char* uri, int* matchcount)
{
    OSSL_STORE_INFO* result = NULL;
...

    {
        ...
        if (!file_fill_pem_pass_data(&pass_data, "PVK pass phrase", uri, ui_method, ui_data) || (tmp = b2i_PVK_bio(bp, file_get_pem_pass, &pass_data)) == NULL
...
static int file_name_to_uri(OSSL_STORE_LOADER_CTX* ctx, const char* name, char** data)
{
    ...
    assert(data != NULL);
    {
        ...
        long calculated_length = strlen(ctx->uri) + strlen(pathsep) + strlen(name) + 1 /* \0 */;
...
static int file_name_check(OSSL_STORE_LOADER_CTX* ctx, const char* name)
{
    ...
    if (ctx->expected_type != 0 && ctx->expected_type != OSSL_STORE_INFO_CERT && ctx->expected_type != OSSL_STORE_INFO_CRL)
        return 0;

    /*
     * First, check the basename
     */
    if (OPENSSL_strncasecmp(name, ctx->_.dir.search_name, len) != 0 || name[len] != '.')
        return 0;
    p = &name[len + 1];
...
static OSSL_STORE_INFO* file_load(OSSL_STORE_LOADER_CTX* ctx, const UI_METHOD* ui_method, void* ui_data)
{
    OSSL_STORE_INFO* result = NULL;
    ...
    again:
        ...
        do {
            ...
            matchcount = -1;
            if (ctx->type == is_pem) {
                if (!file_read_pem(ctx->_.file.file, &pem_name, &pem_header, &data, &len, ui_method, ui_data, ctx->uri, (ctx->flags & FILE_FLAG_SECMEM) != 0)) {
                    ctx->errcnt++;
                    goto endloop;
                }
            } else {
                if ((result = file_try_read_msblob(ctx->_.file.file, &matchcount)) != NULL
                    || (result = file_try_read_PVK(ctx->_.file.file, ui_method, ui_data, ctx->uri, &matchcount)) != NULL)
                    goto endloop;
            ...
            result = file_load_try_decode(ctx, pem_name, pem_header, data, len, ui_method, ui_data, &matchcount);
...
static int file_eof(OSSL_STORE_LOADER_CTX* ctx)
{
    ...
    if (ctx->_.file.last_handler != NULL && !ctx->_.file.last_handler->eof(ctx->_.file.last_handler_ctx))
        return 0;
...
static int bind_loader_attic(ENGINE* e)
{
    ...
    if (/* Create the OSSL_STORE_LOADER */
        (loader_attic = OSSL_STORE_LOADER_new(e, "file")) == NULL || !OSSL_STORE_LOADER_set_open_ex(loader_attic, file_open_ex)
        || !OSSL_STORE_LOADER_set_open(loader_attic, file_open) || !OSSL_STORE_LOADER_set_attach(loader_attic, file_attach)
        || !OSSL_STORE_LOADER_set_ctrl(loader_attic, file_ctrl) || !OSSL_STORE_LOADER_set_expect(loader_attic, file_expect)
        || !OSSL_STORE_LOADER_set_find(loader_attic, file_find) || !OSSL_STORE_LOADER_set_load(loader_attic, file_load)
        || !OSSL_STORE_LOADER_set_eof(loader_attic, file_eof) || !OSSL_STORE_LOADER_set_error(loader_attic, file_error)
        || !OSSL_STORE_LOADER_set_close(loader_attic, file_close)
        /* Init the engine itself */
        || !ENGINE_set_id(e, loader_attic_id) || !ENGINE_set_name(e, loader_attic_name) || !ENGINE_set_destroy_function(e, loader_attic_destroy)
