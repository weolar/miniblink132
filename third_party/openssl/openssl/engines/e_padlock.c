...
static int padlock_bind_helper(ENGINE* e)
{
    ...
    BIO_snprintf(padlock_name, sizeof(padlock_name), "VIA PadLock (%s, %s)", padlock_use_rng ? "RNG" : "no-RNG", padlock_use_ace ? "ACE" : "no-ACE");

    /* Register everything or return with an error */
    if (!ENGINE_set_id(e, padlock_id) || !ENGINE_set_name(e, padlock_name) || !ENGINE_set_init_function(e, padlock_init)
        || (padlock_use_ace && !ENGINE_set_ciphers(e, padlock_ciphers)) || (padlock_use_rng && !ENGINE_set_RAND(e, &padlock_rand))) {
        return 0;
    }
...
static const int padlock_cipher_nids[] = { NID_aes_128_ecb, NID_aes_128_cbc, NID_aes_128_cfb, NID_aes_128_ofb, NID_aes_128_ctr,

    NID_aes_192_ecb, NID_aes_192_cbc, NID_aes_192_cfb, NID_aes_192_ofb, NID_aes_192_ctr,

    NID_aes_256_ecb, NID_aes_256_cbc, NID_aes_256_cfb, NID_aes_256_ofb, NID_aes_256_ctr };
...
#define ALIGNED_CIPHER_DATA(ctx) ((struct padlock_cipher_data*)NEAREST_ALIGNED(EVP_CIPHER_CTX_get_cipher_data(ctx)))
...
static void padlock_ctr32_encrypt_glue(const unsigned char* in, unsigned char* out, size_t blocks, struct padlock_cipher_data* ctx, const unsigned char* ivec)
{
...
static int padlock_ctr_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out_arg, const unsigned char* in_arg, size_t nbytes)
{
    ...
    CRYPTO_ctr128_encrypt_ctr32(
        in_arg, out_arg, nbytes, cdata, EVP_CIPHER_CTX_iv_noconst(ctx), EVP_CIPHER_CTX_buf_noconst(ctx), &num, (ctr128_f)padlock_ctr32_encrypt_glue);
...
#define DECLARE_AES_EVP(ksize, lmode, umode)                                                                                                                   \
    ...
    {                                                                                                                                                          \
        if (_hidden_aes_##ksize##_##lmode == NULL                                                                                                              \
            && ((_hidden_aes_##ksize##_##lmode = EVP_CIPHER_meth_new(NID_aes_##ksize##_##lmode, EVP_CIPHER_block_size_##umode, AES_KEY_SIZE_##ksize)) == NULL  \
                || !EVP_CIPHER_meth_set_iv_length(_hidden_aes_##ksize##_##lmode, AES_BLOCK_SIZE)                                                               \
                || !EVP_CIPHER_meth_set_flags(_hidden_aes_##ksize##_##lmode, 0 | EVP_CIPH_##umode##_MODE)                                                      \
                || !EVP_CIPHER_meth_set_init(_hidden_aes_##ksize##_##lmode, padlock_aes_init_key)                                                              \
                || !EVP_CIPHER_meth_set_do_cipher(_hidden_aes_##ksize##_##lmode, padlock_##lmode##_cipher)                                                     \
                || !EVP_CIPHER_meth_set_impl_ctx_size(_hidden_aes_##ksize##_##lmode, sizeof(struct padlock_cipher_data) + 16)                                  \
                || !EVP_CIPHER_meth_set_set_asn1_params(_hidden_aes_##ksize##_##lmode, EVP_CIPHER_set_asn1_iv)                                                 \
