...
static struct file_ctx_st* new_file_ctx(int type, const char* uri, void* provctx)
{
    ...
    if ((ctx = OPENSSL_zalloc(sizeof(*ctx))) != NULL && (uri == NULL || (ctx->uri = OPENSSL_strdup(uri)) != NULL)) {
        ctx->type = type;
        ctx->provctx = provctx;
        return ctx;
    }
...
static void* file_open(void* provctx, const char* uri)
{
    ...
    for (i = 0, path = NULL; path == NULL && i < path_data_n; i++) {
        ...
        if (path_data[i].check_absolute && path_data[i].path[0] != '/') {
            ERR_clear_last_mark();
            ERR_raise_data(ERR_LIB_PROV, PROV_R_PATH_MUST_BE_ABSOLUTE, "Given path=%s", path_data[i].path);
            return NULL;
        }
...
static const OSSL_PARAM* file_settable_ctx_params(void* provctx)
{
    static const OSSL_PARAM known_settable_ctx_params[]
        = { OSSL_PARAM_utf8_string(OSSL_STORE_PARAM_PROPERTIES, NULL, 0), OSSL_PARAM_int(OSSL_STORE_PARAM_EXPECT, NULL),
              OSSL_PARAM_octet_string(OSSL_STORE_PARAM_SUBJECT, NULL, 0), OSSL_PARAM_utf8_string(OSSL_STORE_PARAM_INPUT_TYPE, NULL, 0), OSSL_PARAM_END };
...
static int file_set_ctx_params(void* loaderctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (ctx->type != IS_DIR) {
            ERR_raise(ERR_LIB_PROV, PROV_R_SEARCH_ONLY_SUPPORTED_FOR_DIRECTORIES);
            return 0;
        }

        if (!OSSL_PARAM_get_octet_string_ptr(p, (const void**)&der, &der_len) || (x509_name = d2i_X509_NAME(NULL, &der, der_len)) == NULL)
            return 0;
        hash = X509_NAME_hash_ex(x509_name, ossl_prov_ctx_get0_libctx(ctx->provctx), NULL, &ok);
...
static int file_load_construct(OSSL_DECODER_INSTANCE* decoder_inst, const OSSL_PARAM* params, void* construct_data)
{
...
static int file_setup_decoders(struct file_ctx_st* ctx)
{
    ...
    if (ctx->_.file.decoderctx == NULL) {
        ...
        if (!OSSL_DECODER_CTX_set_input_type(ctx->_.file.decoderctx, ctx->_.file.input_type)) {
        ...
        case OSSL_STORE_INFO_CERT:
            if (!OSSL_DECODER_CTX_set_input_structure(ctx->_.file.decoderctx, "Certificate")) {
                ERR_raise(ERR_LIB_PROV, ERR_R_OSSL_DECODER_LIB);
                goto err;
            }
            break;
        case OSSL_STORE_INFO_CRL:
            if (!OSSL_DECODER_CTX_set_input_structure(ctx->_.file.decoderctx, "CertificateList")) {
                ERR_raise(ERR_LIB_PROV, ERR_R_OSSL_DECODER_LIB);
                goto err;
            }
            break;
         ...
         */
        if (!OSSL_DECODER_CTX_set_construct(ctx->_.file.decoderctx, file_load_construct)
            || !OSSL_DECODER_CTX_set_cleanup(ctx->_.file.decoderctx, file_load_cleanup)) {
            ERR_raise(ERR_LIB_PROV, ERR_R_OSSL_DECODER_LIB);
            goto err;
        }
    }
...
static int file_load_file(struct file_ctx_st* ctx, OSSL_CALLBACK* object_cb, void* object_cbarg, OSSL_PASSPHRASE_CALLBACK* pw_cb, void* pw_cbarg)
{
    ...
    if (BIO_eof(ctx->_.file.file) && ((err = ERR_peek_last_error()) != 0) && ERR_GET_LIB(err) == ERR_LIB_OSSL_DECODER
...
static int file_load_dir_entry(struct file_ctx_st* ctx, OSSL_CALLBACK* object_cb, void* object_cbarg, OSSL_PASSPHRASE_CALLBACK* pw_cb, void* pw_cbarg)
{
    ...
    OSSL_PARAM object[]
        = { OSSL_PARAM_int(OSSL_OBJECT_PARAM_TYPE, (int*)&object_type), OSSL_PARAM_utf8_string(OSSL_OBJECT_PARAM_DATA, NULL, 0), OSSL_PARAM_END };
    char* newname = NULL;
    int ok;
    ...
    do {
        ...
        if (ctx->_.dir.last_entry[0] != '.' && file_name_check(ctx, ctx->_.dir.last_entry)) {
...
const OSSL_DISPATCH ossl_file_store_functions[] = {
    ...
    { OSSL_FUNC_STORE_SETTABLE_CTX_PARAMS, (void (*)(void))file_settable_ctx_params },
