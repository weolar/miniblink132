...
typedef int(PROV_CIPHER_HW_FN)(PROV_CIPHER_CTX* dat, unsigned char* out, const unsigned char* in, size_t len);
...
void ossl_cipher_generic_initkey(
    void* vctx, size_t kbits, size_t blkbits, size_t ivbits, unsigned int mode, uint64_t flags, const PROV_CIPHER_HW* hw, void* provctx);

#define IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                          \
    const OSSL_DISPATCH ossl_##alg##kbits##lcmode##_functions[]                                                                                                \
        = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))alg##_##kbits##_##lcmode##_newctx }, { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))alg##_freectx },       \
              { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))alg##_dupctx }, { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))ossl_cipher_generic_einit },         \
              ...
              { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))alg##_##kbits##_##lcmode##_get_params },                                                          \
              ...
              { OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))ossl_cipher_generic_settable_ctx_params }, { 0, NULL } };

#define IMPLEMENT_var_keylen_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                       \
    const OSSL_DISPATCH ossl_##alg##kbits##lcmode##_functions[]                                                                                                \
        = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))alg##_##kbits##_##lcmode##_newctx }, { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))alg##_freectx },       \
              { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))alg##_dupctx }, { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))ossl_cipher_generic_einit },         \
              { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))ossl_cipher_generic_dinit },                                                                    \
              ...
              { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))alg##_##kbits##_##lcmode##_get_params },                                                          \
              { OSSL_FUNC_CIPHER_GET_CTX_PARAMS, (void (*)(void))ossl_cipher_generic_get_ctx_params },                                                         \
              { OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))ossl_cipher_var_keylen_set_ctx_params },                                                      \
              { OSSL_FUNC_CIPHER_GETTABLE_PARAMS, (void (*)(void))ossl_cipher_generic_gettable_params },                                                       \
              { OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS, (void (*)(void))ossl_cipher_generic_gettable_ctx_params },                                               \
              { OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))ossl_cipher_var_keylen_settable_ctx_params }, { 0, NULL } };

#define IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                         \
    ...
    {                                                                                                                                                          \
        PROV_##UCALG##_CTX* ctx = ossl_prov_is_running() ? OPENSSL_zalloc(sizeof(*ctx)) : NULL;                                                                \
        if (ctx != NULL) {                                                                                                                                     \
            ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags, ossl_prov_cipher_hw_##alg##_##lcmode(kbits), provctx);   \
        }                                                                                                                                                      \
        return ctx;                                                                                                                                            \
    }

#define IMPLEMENT_generic_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                               \
    IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                             \
        IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)

#define IMPLEMENT_var_keylen_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                            \
    IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                             \
        IMPLEMENT_var_keylen_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)
...
#define IMPLEMENT_CIPHER_HW_OFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)                                                                                   \
    static int cipher_hw_##NAME##_##MODE##_cipher(PROV_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)                               \
    {                                                                                                                                                          \
        int num = ctx->num;                                                                                                                                    \
...
#define IMPLEMENT_CIPHER_HW_CFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)                                                                                   \
    ...
    {                                                                                                                                                          \
        ...
        while (len > 0 && len >= chunk) {                                                                                                                      \
            FUNC_PREFIX##_encrypt(in, out, (long)chunk, key, ctx->iv, &num, ctx->enc);                                                                         \
            len -= chunk;                                                                                                                                      \
            in += chunk;                                                                                                                                       \
            out += chunk;                                                                                                                                      \
            if (len < chunk)                                                                                                                                   \
...
#define CIPHER_DEFAULT_GETTABLE_CTX_PARAMS_START(name)                                                                                                         \
    static const OSSL_PARAM name##_known_gettable_ctx_params[] = { OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL),                                          \
        OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_IVLEN, NULL), OSSL_PARAM_uint(OSSL_CIPHER_PARAM_PADDING, NULL), OSSL_PARAM_uint(OSSL_CIPHER_PARAM_NUM, NULL),      \
        OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_IV, NULL, 0), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_UPDATED_IV, NULL, 0),

#define CIPHER_DEFAULT_GETTABLE_CTX_PARAMS_END(name)                                                                                                           \
    OSSL_PARAM_END                                                                                                                                             \
    }                                                                                                                                                          \
    ;                                                                                                                                                          \
    const OSSL_PARAM* name##_gettable_ctx_params(ossl_unused void* cctx, ossl_unused void* provctx)                                                            \
    {                                                                                                                                                          \
...
#define CIPHER_DEFAULT_SETTABLE_CTX_PARAMS_START(name)                                                                                                         \
    static const OSSL_PARAM name##_known_settable_ctx_params[]                                                                                                 \
        = { OSSL_PARAM_uint(OSSL_CIPHER_PARAM_PADDING, NULL), OSSL_PARAM_uint(OSSL_CIPHER_PARAM_NUM, NULL),
#define CIPHER_DEFAULT_SETTABLE_CTX_PARAMS_END(name)                                                                                                           \
    OSSL_PARAM_END                                                                                                                                             \
    }                                                                                                                                                          \
    ;                                                                                                                                                          \
    const OSSL_PARAM* name##_settable_ctx_params(ossl_unused void* cctx, ossl_unused void* provctx)                                                            \
    {                                                                                                                                                          \
...
size_t ossl_cipher_fillblock(unsigned char* buf, size_t* buflen, size_t blocksize, const unsigned char** in, size_t* inlen);
int ossl_cipher_trailingdata(unsigned char* buf, size_t* buflen, size_t blocksize, const unsigned char** in, size_t* inlen);
