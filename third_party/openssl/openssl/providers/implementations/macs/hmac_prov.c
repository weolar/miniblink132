...
int ssl3_cbc_digest_record(const EVP_MD* md, unsigned char* md_out, size_t* md_out_size, const unsigned char header[13], const unsigned char* data,
...
static void* hmac_new(void* provctx)
{
    ...
    if ((macctx = OPENSSL_zalloc(sizeof(*macctx))) == NULL || (macctx->ctx = HMAC_CTX_new()) == NULL) {
        OPENSSL_free(macctx);
        return NULL;
    }
...
static void* hmac_dup(void* vsrc)
{
    ...
    *dst = *src;
    ...
    if (!HMAC_CTX_copy(dst->ctx, src->ctx) || !ossl_prov_digest_copy(&dst->digest, &src->digest)) {
        hmac_free(dst);
        return NULL;
    }
    if (src->key != NULL) {
...
static int hmac_setkey(struct hmac_data_st* macctx, const unsigned char* key, size_t keylen)
{
    const EVP_MD* digest;
    ...
    if (key != NULL || (macctx->tls_data_size == 0 && digest != NULL))
        return HMAC_Init_ex(macctx->ctx, key, keylen, digest, ossl_prov_digest_engine(&macctx->digest));
    return 1;
}

static int hmac_init(void* vmacctx, const unsigned char* key, size_t keylen, const OSSL_PARAM params[])
{
    struct hmac_data_st* macctx = vmacctx;
...
static int hmac_update(void* vmacctx, const unsigned char* data, size_t datalen)
{
    ...
    if (macctx->tls_data_size > 0) {
        ...
        return ssl3_cbc_digest_record(ossl_prov_digest_md(&macctx->digest), macctx->tls_mac_out, &macctx->tls_mac_out_size, macctx->tls_header, data, datalen,
            macctx->tls_data_size, macctx->key, macctx->keylen, 0);
    }
...
static const OSSL_PARAM* hmac_gettable_ctx_params(ossl_unused void* ctx, ossl_unused void* provctx)
{
...
static int hmac_get_ctx_params(void* vmacctx, OSSL_PARAM params[])
{
    ...
    if ((p = OSSL_PARAM_locate(params, OSSL_MAC_PARAM_SIZE)) != NULL && !OSSL_PARAM_set_size_t(p, hmac_size(macctx)))
        return 0;

    if ((p = OSSL_PARAM_locate(params, OSSL_MAC_PARAM_BLOCK_SIZE)) != NULL && !OSSL_PARAM_set_int(p, hmac_block_size(macctx)))
        return 0;
...
static const OSSL_PARAM known_settable_ctx_params[]
    = { OSSL_PARAM_utf8_string(OSSL_MAC_PARAM_DIGEST, NULL, 0), OSSL_PARAM_utf8_string(OSSL_MAC_PARAM_PROPERTIES, NULL, 0),
          OSSL_PARAM_octet_string(OSSL_MAC_PARAM_KEY, NULL, 0), OSSL_PARAM_int(OSSL_MAC_PARAM_DIGEST_NOINIT, NULL),
          OSSL_PARAM_int(OSSL_MAC_PARAM_DIGEST_ONESHOT, NULL), OSSL_PARAM_size_t(OSSL_MAC_PARAM_TLS_DATA_SIZE, NULL), OSSL_PARAM_END };
static const OSSL_PARAM* hmac_settable_ctx_params(ossl_unused void* ctx, ossl_unused void* provctx)
{
    return known_settable_ctx_params;
}

static int set_flag(const OSSL_PARAM params[], const char* key, int mask, int* flags)
{
...
static int hmac_set_ctx_params(void* vmacctx, const OSSL_PARAM params[])
{
    ...
    if (!set_flag(params, OSSL_MAC_PARAM_DIGEST_NOINIT, EVP_MD_CTX_FLAG_NO_INIT, &flags))
        return 0;
    if (!set_flag(params, OSSL_MAC_PARAM_DIGEST_ONESHOT, EVP_MD_CTX_FLAG_ONESHOT, &flags))
        return 0;
    if (flags)
...
const OSSL_DISPATCH ossl_hmac_functions[] = { { OSSL_FUNC_MAC_NEWCTX, (void (*)(void))hmac_new }, { OSSL_FUNC_MAC_DUPCTX, (void (*)(void))hmac_dup },
    { OSSL_FUNC_MAC_FREECTX, (void (*)(void))hmac_free }, { OSSL_FUNC_MAC_INIT, (void (*)(void))hmac_init },
    { OSSL_FUNC_MAC_UPDATE, (void (*)(void))hmac_update }, { OSSL_FUNC_MAC_FINAL, (void (*)(void))hmac_final },
    { OSSL_FUNC_MAC_GETTABLE_CTX_PARAMS, (void (*)(void))hmac_gettable_ctx_params }, { OSSL_FUNC_MAC_GET_CTX_PARAMS, (void (*)(void))hmac_get_ctx_params },
    { OSSL_FUNC_MAC_SETTABLE_CTX_PARAMS, (void (*)(void))hmac_settable_ctx_params }, { OSSL_FUNC_MAC_SET_CTX_PARAMS, (void (*)(void))hmac_set_ctx_params },
    { 0, NULL } };
