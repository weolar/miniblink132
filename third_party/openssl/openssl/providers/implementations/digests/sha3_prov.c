...
#define S390_SHA3 1
#define S390_SHA3_CAPABLE(name) ((OPENSSL_s390xcap_P.kimd[0] & S390X_CAPBIT(S390X_##name)) && (OPENSSL_s390xcap_P.klmd[0] & S390X_CAPBIT(S390X_##name)))
...
static int keccak_final(void* vctx, unsigned char* out, size_t* outl, size_t outsz)
{
    int ret = 1;
    KECCAK1600_CTX* ctx = vctx;
...
static PROV_SHA3_METHOD sha3_generic_md = { generic_sha3_absorb, generic_sha3_final };
...
static PROV_SHA3_METHOD sha3_s390x_md = { s390x_sha3_absorb, s390x_sha3_final };

static PROV_SHA3_METHOD shake_s390x_md = { s390x_sha3_absorb, s390x_shake_final };
...
#define SHA3_newctx(typ, uname, name, bitlen, pad)                                                                                                             \
    ...
    {                                                                                                                                                          \
        KECCAK1600_CTX* ctx = ossl_prov_is_running() ? OPENSSL_zalloc(sizeof(*ctx)) : NULL;                                                                    \
                                                                                                                                                               \
        if (ctx == NULL)                                                                                                                                       \
            return NULL;                                                                                                                                       \
...
#define PROV_FUNC_SHA3_DIGEST_COMMON(name, bitlen, blksize, dgstsize, flags)                                                                                   \
    ...
    const OSSL_DISPATCH ossl_##name##_functions[]                                                                                                              \
        = { { OSSL_FUNC_DIGEST_NEWCTX, (void (*)(void))name##_newctx }, { OSSL_FUNC_DIGEST_UPDATE, (void (*)(void))keccak_update },                            \
              { OSSL_FUNC_DIGEST_FINAL, (void (*)(void))keccak_final }, { OSSL_FUNC_DIGEST_FREECTX, (void (*)(void))keccak_freectx },                          \
              { OSSL_FUNC_DIGEST_DUPCTX, (void (*)(void))keccak_dupctx }, PROV_DISPATCH_FUNC_DIGEST_GET_PARAMS(name)

#define PROV_FUNC_SHA3_DIGEST(name, bitlen, blksize, dgstsize, flags)                                                                                          \
    PROV_FUNC_SHA3_DIGEST_COMMON(name, bitlen, blksize, dgstsize, flags), { OSSL_FUNC_DIGEST_INIT, (void (*)(void))keccak_init },                              \
        PROV_DISPATCH_FUNC_DIGEST_CONSTRUCT_END

#define PROV_FUNC_SHAKE_DIGEST(name, bitlen, blksize, dgstsize, flags)                                                                                         \
    PROV_FUNC_SHA3_DIGEST_COMMON(name, bitlen, blksize, dgstsize, flags), { OSSL_FUNC_DIGEST_INIT, (void (*)(void))keccak_init_params },                       \
...
static const OSSL_PARAM known_shake_settable_ctx_params[] = { { OSSL_DIGEST_PARAM_XOFLEN, OSSL_PARAM_UNSIGNED_INTEGER, NULL, 0, 0 }, OSSL_PARAM_END };
...
#define IMPLEMENT_SHA3_functions(bitlen)                                                                                                                       \
    SHA3_newctx(sha3, SHA3_##bitlen, sha3_##bitlen, bitlen, '\x06')                                                                                            \
        PROV_FUNC_SHA3_DIGEST(sha3_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen), SHA3_MDSIZE(bitlen), SHA3_FLAGS)

#define IMPLEMENT_SHAKE_functions(bitlen)                                                                                                                      \
    SHA3_newctx(shake, SHAKE_##bitlen, shake_##bitlen, bitlen, '\x1f')                                                                                         \
        PROV_FUNC_SHAKE_DIGEST(shake_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen), SHA3_MDSIZE(bitlen), SHAKE_FLAGS)
#define IMPLEMENT_KMAC_functions(bitlen)                                                                                                                       \
    KMAC_newctx(keccak_kmac_##bitlen, bitlen, '\x04')                                                                                                          \
        PROV_FUNC_SHAKE_DIGEST(keccak_kmac_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen), KMAC_MDSIZE(bitlen), KMAC_FLAGS)
