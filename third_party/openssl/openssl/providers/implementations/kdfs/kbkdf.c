...
typedef enum { COUNTER = 0, FEEDBACK } kbkdf_mode;
...
static int derive(EVP_MAC_CTX* ctx_init, kbkdf_mode mode, unsigned char* iv, size_t iv_len, unsigned char* label, size_t label_len, unsigned char* context,
    size_t context_len, unsigned char* k_i, size_t h, uint32_t l, int has_separator, unsigned char* ko, size_t ko_len)
{
    int ret = 0;
    ...
    for (counter = 1; written < ko_len; counter++) {
        ...
        if (!EVP_MAC_update(ctx, (unsigned char*)&i, 4) || !EVP_MAC_update(ctx, label, label_len) || (has_separator && !EVP_MAC_update(ctx, &zero, 1))
            || !EVP_MAC_update(ctx, context, context_len) || (has_l && !EVP_MAC_update(ctx, (unsigned char*)&l, 4)) || !EVP_MAC_final(ctx, k_i, NULL, h))
            goto done;
...
static int kbkdf_derive(void* vctx, unsigned char* key, size_t keylen, const OSSL_PARAM params[])
{
    KBKDF* ctx = (KBKDF*)vctx;
    int ret = 0;
    ...
    ret = derive(
        ctx->ctx_init, ctx->mode, ctx->iv, ctx->iv_len, ctx->label, ctx->label_len, ctx->context, ctx->context_len, k_i, h, l, ctx->use_separator, key, keylen);
done:
    if (ret != 1)
...
static int kbkdf_set_buffer(unsigned char** out, size_t* out_len, const OSSL_PARAM* p)
{
...
static int kbkdf_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
    ...
    if (!ossl_prov_macctx_load_from_params(&ctx->ctx_init, params, NULL, NULL, NULL, libctx))
        return 0;
    else if (ctx->ctx_init != NULL && !EVP_MAC_is_a(EVP_MAC_CTX_get0_mac(ctx->ctx_init), OSSL_MAC_NAME_HMAC)
        && !EVP_MAC_is_a(EVP_MAC_CTX_get0_mac(ctx->ctx_init), OSSL_MAC_NAME_CMAC)) {
    ...
    if (ctx->ctx_init != NULL && ctx->ki_len != 0 && !EVP_MAC_init(ctx->ctx_init, ctx->ki, ctx->ki_len, NULL))
        return 0;
    return 1;
}

static const OSSL_PARAM* kbkdf_settable_ctx_params(ossl_unused void* ctx, ossl_unused void* provctx)
{
...
static const OSSL_PARAM* kbkdf_gettable_ctx_params(ossl_unused void* ctx, ossl_unused void* provctx)
{
...
const OSSL_DISPATCH ossl_kdf_kbkdf_functions[] = {
    ...
    { OSSL_FUNC_KDF_SETTABLE_CTX_PARAMS, (void (*)(void))kbkdf_settable_ctx_params },
    { OSSL_FUNC_KDF_SET_CTX_PARAMS, (void (*)(void))kbkdf_set_ctx_params },
    { OSSL_FUNC_KDF_GETTABLE_CTX_PARAMS, (void (*)(void))kbkdf_gettable_ctx_params },
