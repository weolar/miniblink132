...
static int print_labeled_bignum(BIO* out, const char* label, const BIGNUM* bn)
{
    ...
    while (*p != '\0') {
        ...
        if (BIO_printf(out, "%s%c%c", use_sep ? ":" : "", tolower(p[0]), tolower(p[1])) <= 0)
            goto err;
        ++bytes;
        p += 2;
        use_sep = 1;
...
static int print_labeled_buf(BIO* out, const char* label, const unsigned char* buf, size_t buflen)
{
    size_t i;
...
static int dh_to_text(BIO* out, const void* key, int selection)
{
    ...
    if (priv_key != NULL && !print_labeled_bignum(out, "private-key:", priv_key))
        return 0;
    if (pub_key != NULL && !print_labeled_bignum(out, "public-key:", pub_key))
        return 0;
    if (params != NULL && !ffc_params_to_text(out, params))
        return 0;
...
static int dsa_to_text(BIO* out, const void* key, int selection)
{
    ...
    if (BIO_printf(out, "%s: (%d bit)\n", type_label, BN_num_bits(p)) <= 0)
        return 0;
    if (priv_key != NULL && !print_labeled_bignum(out, "priv:", priv_key))
        return 0;
    if (pub_key != NULL && !print_labeled_bignum(out, "pub: ", pub_key))
        return 0;
    if (params != NULL && !ffc_params_to_text(out, params))
        return 0;
...
static int ec_param_explicit_curve_to_text(BIO* out, const EC_GROUP* group, BN_CTX* ctx)
{
    ...
    return print_labeled_bignum(out, plabel, p) && print_labeled_bignum(out, "A:   ", a) && print_labeled_bignum(out, "B:   ", b);
}
...
static int ec_param_explicit_to_text(BIO* out, const EC_GROUP* group, OSSL_LIB_CTX* libctx)
{
    ...
    if (BIO_printf(out, "Field Type: %s\n", OBJ_nid2sn(tmp_nid)) <= 0 || !ec_param_explicit_curve_to_text(out, group, ctx)
        || !ec_param_explicit_gen_to_text(out, group, ctx) || !print_labeled_bignum(out, "Order: ", order)
        || (cofactor != NULL && !print_labeled_bignum(out, "Cofactor: ", cofactor)) || (seed != NULL && !print_labeled_buf(out, "Seed:", seed, seed_len)))
        goto err;
    ret = 1;
err:
    BN_CTX_end(ctx);
...
static int ec_to_text(BIO* out, const void* key, int selection)
{
    ...
    if (BIO_printf(out, "%s: (%d bit)\n", type_label, EC_GROUP_order_bits(group)) <= 0)
        goto err;
    if (priv != NULL && !print_labeled_buf(out, "priv:", priv, priv_len))
        goto err;
    if (pub != NULL && !print_labeled_buf(out, "pub:", pub, pub_len))
        goto err;
    if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0)
...
static int rsa_to_text(BIO* out, const void* key, int selection)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
        ...
        if (!print_labeled_bignum(out, "privateExponent:", rsa_d))
            goto err;
        if (!print_labeled_bignum(out, "prime1:", sk_BIGNUM_const_value(factors, 0)))
            goto err;
        if (!print_labeled_bignum(out, "prime2:", sk_BIGNUM_const_value(factors, 1)))
            goto err;
        if (!print_labeled_bignum(out, "exponent1:", sk_BIGNUM_const_value(exps, 0)))
            goto err;
        if (!print_labeled_bignum(out, "exponent2:", sk_BIGNUM_const_value(exps, 1)))
            goto err;
        if (!print_labeled_bignum(out, "coefficient:", sk_BIGNUM_const_value(coeffs, 0)))
            goto err;
        for (i = 2; i < sk_BIGNUM_const_num(factors); i++) {
            if (BIO_printf(out, "prime%d:", i + 1) <= 0)
                goto err;
            if (!print_labeled_bignum(out, NULL, sk_BIGNUM_const_value(factors, i)))
                goto err;
            if (BIO_printf(out, "exponent%d:", i + 1) <= 0)
                goto err;
            if (!print_labeled_bignum(out, NULL, sk_BIGNUM_const_value(exps, i)))
                goto err;
            if (BIO_printf(out, "coefficient%d:", i + 1) <= 0)
                goto err;
            if (!print_labeled_bignum(out, NULL, sk_BIGNUM_const_value(coeffs, i - 1)))
                goto err;
        }
    }
        ...
        case RSA_FLAG_TYPE_RSASSAPSS:
            if (ossl_rsa_pss_params_30_is_unrestricted(pss_params)) {
                ...
                if (BIO_printf(out, "PSS parameter restrictions:\n") <= 0)
                    goto err;
                if (BIO_printf(out, "  Hash Algorithm: %s%s\n", ossl_rsa_oaeppss_nid2name(hashalg_nid), (hashalg_nid == NID_sha1 ? " (default)" : "")) <= 0)
                    goto err;
                if (BIO_printf(out, "  Mask Algorithm: %s with %s%s\n", ossl_rsa_mgf_nid2name(maskgenalg_nid), ossl_rsa_oaeppss_nid2name(maskgenhashalg_nid),
...
static int key2text_encode(void* vctx, const void* key, int selection, OSSL_CORE_BIO* cout, int (*key2text)(BIO* out, const void* key, int selection),
...
#define MAKE_TEXT_ENCODER(impl, type)                                                                                                                          \
    ...
    static OSSL_FUNC_encoder_encode_fn impl##2text_encode;                                                                                                     \
                                                                                                                                                               \
    static void* impl##2text_import_object(void* ctx, int selection, const OSSL_PARAM params[])                                                                \
    {                                                                                                                                                          \
    ...
    static int impl##2text_encode(                                                                                                                             \
        void* vctx, OSSL_CORE_BIO* cout, const void* key, const OSSL_PARAM key_abstract[], int selection, OSSL_PASSPHRASE_CALLBACK* cb, void* cbarg)           \
    {                                                                                                                                                          \
        ...
        return key2text_encode(vctx, key, selection, cout, type##_to_text, cb, cbarg);                                                                         \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##impl##_to_text_encoder_functions[] = { { OSSL_FUNC_ENCODER_NEWCTX, (void (*)(void))key2text_newctx },                           \
        { OSSL_FUNC_ENCODER_FREECTX, (void (*)(void))key2text_freectx }, { OSSL_FUNC_ENCODER_IMPORT_OBJECT, (void (*)(void))impl##2text_import_object },       \
        { OSSL_FUNC_ENCODER_FREE_OBJECT, (void (*)(void))impl##2text_free_object }, { OSSL_FUNC_ENCODER_ENCODE, (void (*)(void))impl##2text_encode },          \
        { 0, NULL } }
