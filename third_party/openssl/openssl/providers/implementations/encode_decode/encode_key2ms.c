...
static int key2msblob_encode(
    void* vctx, const void* key, int selection, OSSL_CORE_BIO* cout, evp_pkey_set1_fn* set1_key, OSSL_PASSPHRASE_CALLBACK* pw_cb, void* pw_cbarg)
{
    struct key2ms_ctx_st* ctx = vctx;
    int ispub = -1;
    EVP_PKEY* pkey = NULL;
...
#define pvk_set_params                                                                                                                                         \
    { OSSL_FUNC_ENCODER_SETTABLE_CTX_PARAMS, (void (*)(void))key2pvk_settable_ctx_params },                                                                    \
        { OSSL_FUNC_ENCODER_SET_CTX_PARAMS, (void (*)(void))key2pvk_set_ctx_params },

#define MAKE_MS_ENCODER(impl, output, type)                                                                                                                    \
    static OSSL_FUNC_encoder_import_object_fn impl##2##output##_import_object;                                                                                 \
    ...
    static OSSL_FUNC_encoder_encode_fn impl##2##output##_encode;                                                                                               \
                                                                                                                                                               \
    static void* impl##2##output##_import_object(void* ctx, int selection, const OSSL_PARAM params[])                                                          \
    {                                                                                                                                                          \
    ...
    static int impl##2##output##_encode(                                                                                                                       \
    ...
    {                                                                                                                                                          \
        ...
        return key2##output##_encode(vctx, key, selection, cout, type##_set1, cb, cbarg);                                                                      \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##impl##_to_##output##_encoder_functions[]                                                                                        \
        = { { OSSL_FUNC_ENCODER_NEWCTX, (void (*)(void))key2ms_newctx }, { OSSL_FUNC_ENCODER_FREECTX, (void (*)(void))key2ms_freectx },                        \
              output##_set_params { OSSL_FUNC_ENCODER_DOES_SELECTION, (void (*)(void))key2ms_does_selection },                                                 \
              { OSSL_FUNC_ENCODER_IMPORT_OBJECT, (void (*)(void))impl##2##output##_import_object },                                                            \
              { OSSL_FUNC_ENCODER_FREE_OBJECT, (void (*)(void))impl##2##output##_free_object },                                                                \
              { OSSL_FUNC_ENCODER_ENCODE, (void (*)(void))impl##2##output##_encode }, { 0, NULL } }
