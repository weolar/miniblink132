    ...
    void* vctx, OSSL_CORE_BIO* cin, int selection, OSSL_CALLBACK* data_cb, void* data_cbarg, OSSL_PASSPHRASE_CALLBACK* pw_cb, void* pw_cbarg)
{
    ...
    if ((selection == 0 || (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) && !ispub && ctx->desc->read_private_key != NULL) {
    ...
    if (key == NULL && (selection == 0 || (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) && ispub && ctx->desc->read_public_key != NULL) {
        p = buf;
...
static int msblob2key_export_object(void* vctx, const void* reference, size_t reference_sz, OSSL_CALLBACK* export_cb, void* export_cbarg)
{
    struct msblob2key_ctx_st* ctx = vctx;
...
#define IMPLEMENT_MSBLOB(KEYTYPE, keytype)                                                                                                                     \
    static const struct keytype_desc_st mstype##2##keytype##_desc = { EVP_PKEY_##KEYTYPE, #KEYTYPE, ossl_##keytype##_keymgmt_functions,                        \
        keytype##_decode_private_key, keytype##_decode_public_key, keytype##_adjust, keytype##_free };                                                         \
    ...
    const OSSL_DISPATCH ossl_msblob_to_##keytype##_decoder_functions[] = { { OSSL_FUNC_DECODER_NEWCTX, (void (*)(void))msblob2##keytype##_newctx },            \
        { OSSL_FUNC_DECODER_FREECTX, (void (*)(void))msblob2key_freectx }, { OSSL_FUNC_DECODER_DOES_SELECTION, (void (*)(void))msblob2key_does_selection },    \
        { OSSL_FUNC_DECODER_DECODE, (void (*)(void))msblob2key_decode }, { OSSL_FUNC_DECODER_EXPORT_OBJECT, (void (*)(void))msblob2key_export_object },        \
        { 0, NULL } }
