...
typedef int key_to_paramstring_fn(const void* key, int nid, int save, void** str, int* strtype);
typedef int key_to_der_fn(
    BIO* out, const void* key, int key_nid, const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx);
...
static PKCS8_PRIV_KEY_INFO* key_to_p8info(const void* key, int key_nid, void* params, int params_type, i2d_of_void* k2d)
{
    ...
    if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL || (derlen = k2d(key, &der)) <= 0
        || !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, params_type, params, der, derlen)) {
...
static X509_SIG* p8info_to_encp8(PKCS8_PRIV_KEY_INFO* p8info, struct key2any_ctx_st* ctx)
{
    X509_SIG* p8 = NULL;
    ...
    if (!ossl_pw_get_passphrase(kstr, sizeof(kstr), &klen, NULL, 1, &ctx->pwdata)) {
...
static X509_SIG* key_to_encp8(const void* key, int key_nid, void* params, int params_type, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
...
static X509_PUBKEY* key_to_pubkey(const void* key, int key_nid, void* params, int params_type, i2d_of_void k2d)
{
    /* der, derlen store the key DER output and its length */
    unsigned char* der = NULL;
    int derlen;
...
static int key_to_epki_der_priv_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
...
static int key_to_epki_pem_priv_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
...
static int key_to_pki_der_priv_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
...
static int key_to_pki_pem_priv_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
...
static int key_to_spki_der_pub_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
...
static int key_to_spki_pem_pub_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    int ret = 0;
    void* str = NULL;
 ...
 */
static int key_to_type_specific_der_bio(
    BIO* out, const void* key, int key_nid, ossl_unused const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    unsigned char* der = NULL;
    int derlen;
...
static int key_to_type_specific_pem_bio_cb(BIO* out, const void* key, int key_nid, const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d,
    struct key2any_ctx_st* ctx, pem_password_cb* cb, void* cbarg)
{
    return PEM_ASN1_write_bio(k2d, pemname, out, key, ctx->cipher, NULL, 0, cb, cbarg) > 0;
}

static int key_to_type_specific_pem_priv_bio(
    BIO* out, const void* key, int key_nid, const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d, ctx, ossl_pw_pem_password, &ctx->pwdata);
}

static int key_to_type_specific_pem_pub_bio(
    BIO* out, const void* key, int key_nid, const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d, ctx, NULL, NULL);
}

#ifndef OPENSSL_NO_KEYPARAMS
static int key_to_type_specific_pem_param_bio(
    BIO* out, const void* key, int key_nid, const char* pemname, key_to_paramstring_fn* p2s, i2d_of_void* k2d, struct key2any_ctx_st* ctx)
{
    return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d, ctx, NULL, NULL);
}
...
static int prepare_dsa_params(const void* dsa, int nid, int save, void** pstr, int* pstrtype)
{
 ...
 */
static int prepare_ec_params(const void* eckey, int nid, int save, void** pstr, int* pstrtype)
{
    int curve_nid;
    ...
    if (curve_nid != NID_undef && (EC_GROUP_get_asn1_flag(group) & OPENSSL_EC_NAMED_CURVE)) {
...
static int prepare_rsa_params(const void* rsa, int nid, int save, void** pstr, int* pstrtype)
{
            ...
            *pstrtype = V_ASN1_UNDEF;
            ...
            for (i = 0; i < 2; i++) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if ((str = OPENSSL_malloc(str_sz)) == NULL || !WPACKET_init_der(&pkt, str, str_sz)) {
                        WPACKET_cleanup(&pkt);
                        goto err;
                    }
                    break;
                }
                if (!ossl_DER_w_RSASSA_PSS_params(&pkt, -1, pss) || !WPACKET_finish(&pkt) || !WPACKET_get_total_written(&pkt, &str_sz)) {
                    WPACKET_cleanup(&pkt);
                    goto err;
                }
                WPACKET_cleanup(&pkt);
...
static int key2any_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
    ...
    if (cipherp != NULL) {
        ...
        if (ciphername != NULL && ((ctx->cipher = EVP_CIPHER_fetch(libctx, ciphername, props)) == NULL))
            return 0;
    }
...
static int key2any_check_selection(int selection, int selection_mask)
{
     ...
     */
    int checks[] = { OSSL_KEYMGMT_SELECT_PRIVATE_KEY, OSSL_KEYMGMT_SELECT_PUBLIC_KEY, OSSL_KEYMGMT_SELECT_ALL_PARAMETERS };
    size_t i;
...
static int key2any_encode(struct key2any_ctx_st* ctx, OSSL_CORE_BIO* cout, const void* key, int type, const char* pemname, check_key_type_fn* checker,
    key_to_der_fn* writer, OSSL_PASSPHRASE_CALLBACK* pwcb, void* pwcbarg, key_to_paramstring_fn* key2paramstring, i2d_of_void* key2der)
{
    int ret = 0;

    if (key == NULL) {
        ERR_raise(ERR_LIB_PROV, ERR_R_PASSED_NULL_PARAMETER);
    } else if (writer != NULL && (checker == NULL || checker(key, type))) {
        BIO* out = ossl_bio_new_from_core_bio(ctx->provctx, cout);

        if (out != NULL && (pwcb == NULL || ossl_pw_set_ossl_passphrase_cb(&ctx->pwdata, pwcb, pwcbarg)))
...
#define DO_PRIVATE_KEY(impl, type, kind, output)                                                                                                               \
    if ((selection & DO_PRIVATE_KEY_selection_mask) != 0)                                                                                                      \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PRIVATE KEY", type##_check_key_type, key_to_##kind##_##output##_priv_bio, cb, \
            cbarg, prepare_##type##_params, type##_##kind##_priv_to_der);
...
#define DO_PUBLIC_KEY(impl, type, kind, output)                                                                                                                \
    if ((selection & DO_PUBLIC_KEY_selection_mask) != 0)                                                                                                       \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PUBLIC KEY", type##_check_key_type, key_to_##kind##_##output##_pub_bio, cb,   \
            cbarg, prepare_##type##_params, type##_##kind##_pub_to_der);
...
#define DO_PARAMETERS(impl, type, kind, output)                                                                                                                \
    if ((selection & DO_PARAMETERS_selection_mask) != 0)                                                                                                       \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PARAMETERS", type##_check_key_type, key_to_##kind##_##output##_param_bio,     \
            NULL, NULL, NULL, type##_##kind##_params_to_der);
...
#define DO_PrivateKeyInfo(impl, type, output) DO_PRIVATE_KEY(impl, type, pki, output)

#define DO_EncryptedPrivateKeyInfo_selection_mask DO_PRIVATE_KEY_selection_mask
#define DO_EncryptedPrivateKeyInfo(impl, type, output) DO_PRIVATE_KEY(impl, type, epki, output)

/* SubjectPublicKeyInfo is a structure for public keys only */
#define DO_SubjectPublicKeyInfo_selection_mask DO_PUBLIC_KEY_selection_mask
#define DO_SubjectPublicKeyInfo(impl, type, output) DO_PUBLIC_KEY(impl, type, spki, output)
...
#define DO_type_specific_params(impl, type, output) DO_PARAMETERS(impl, type, type_specific, output)
#define DO_type_specific_keypair_selection_mask (DO_PRIVATE_KEY_selection_mask | DO_PUBLIC_KEY_selection_mask)
...
#define DO_type_specific_selection_mask (DO_type_specific_keypair_selection_mask | DO_type_specific_params_selection_mask)
#define DO_type_specific(impl, type, output) DO_type_specific_keypair(impl, type, output) DO_type_specific_params(impl, type, output)
#define DO_type_specific_no_pub_selection_mask (DO_PRIVATE_KEY_selection_mask | DO_PARAMETERS_selection_mask)
...
#define MAKE_ENCODER(impl, type, evp_type, kind, output)                                                                                                       \
    static OSSL_FUNC_encoder_import_object_fn impl##_to_##kind##_##output##_import_object;                                                                     \
    static OSSL_FUNC_encoder_free_object_fn impl##_to_##kind##_##output##_free_object;                                                                         \
    static OSSL_FUNC_encoder_encode_fn impl##_to_##kind##_##output##_encode;                                                                                   \
                                                                                                                                                               \
    static void* impl##_to_##kind##_##output##_import_object(void* vctx, int selection, const OSSL_PARAM params[])                                             \
    {                                                                                                                                                          \
        struct key2any_ctx_st* ctx = vctx;                                                                                                                     \
                                                                                                                                                               \
        return ossl_prov_import_key(ossl_##impl##_keymgmt_functions, ctx->provctx, selection, params);                                                         \
    }                                                                                                                                                          \
    static void impl##_to_##kind##_##output##_free_object(void* key)                                                                                           \
    {                                                                                                                                                          \
        ossl_prov_free_key(ossl_##impl##_keymgmt_functions, key);                                                                                              \
    }                                                                                                                                                          \
    static int impl##_to_##kind##_##output##_does_selection(void* ctx, int selection)                                                                          \
    {                                                                                                                                                          \
        return key2any_check_selection(selection, DO_##kind##_selection_mask);                                                                                 \
    }                                                                                                                                                          \
    static int impl##_to_##kind##_##output##_encode(                                                                                                           \
        void* ctx, OSSL_CORE_BIO* cout, const void* key, const OSSL_PARAM key_abstract[], int selection, OSSL_PASSPHRASE_CALLBACK* cb, void* cbarg)            \
    {                                                                                                                                                          \
    ...
    const OSSL_DISPATCH ossl_##impl##_to_##kind##_##output##_encoder_functions[]                                                                               \
        = { { OSSL_FUNC_ENCODER_NEWCTX, (void (*)(void))key2any_newctx }, { OSSL_FUNC_ENCODER_FREECTX, (void (*)(void))key2any_freectx },                      \
              { OSSL_FUNC_ENCODER_SETTABLE_CTX_PARAMS, (void (*)(void))key2any_settable_ctx_params },                                                          \
              { OSSL_FUNC_ENCODER_SET_CTX_PARAMS, (void (*)(void))key2any_set_ctx_params },                                                                    \
              { OSSL_FUNC_ENCODER_DOES_SELECTION, (void (*)(void))impl##_to_##kind##_##output##_does_selection },                                              \
              { OSSL_FUNC_ENCODER_IMPORT_OBJECT, (void (*)(void))impl##_to_##kind##_##output##_import_object },                                                \
              { OSSL_FUNC_ENCODER_FREE_OBJECT, (void (*)(void))impl##_to_##kind##_##output##_free_object },                                                    \
              { OSSL_FUNC_ENCODER_ENCODE, (void (*)(void))impl##_to_##kind##_##output##_encode }, { 0, NULL } }
