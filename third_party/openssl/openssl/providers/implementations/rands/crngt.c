...
static int crngt_get_entropy(PROV_CTX* provctx, const EVP_MD* digest, unsigned char* buf, unsigned char* md, unsigned int* md_size)
{
    int r;
    size_t n;
    unsigned char* p;
...
size_t ossl_crngt_get_entropy(PROV_DRBG* drbg, unsigned char** pout, int entropy, size_t min_len, size_t max_len, int prediction_resistance)
{
    ...
    CRNG_TEST_GLOBAL* crngt_glob = ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_RAND_CRNGT_INDEX, &rand_crng_ossl_ctx_method);
    ...
    if (stcb != NULL) {
        ...
        if (st == NULL)
            goto err;
        OSSL_SELF_TEST_onbegin(st, OSSL_SELF_TEST_TYPE_CRNG, OSSL_SELF_TEST_DESC_RNG);
    }
...
void ossl_crngt_cleanup_entropy(ossl_unused PROV_DRBG* drbg, unsigned char* out, size_t outlen)
{
