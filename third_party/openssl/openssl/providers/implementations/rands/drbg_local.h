...
#define DRBG_DEFAULT_PERS_STRING                                                                                                                               \
    { 0x4f, 0x70, 0x65, 0x6e, 0x53, 0x53, 0x4c, 0x20, 0x4e, 0x49, 0x53, 0x54, 0x20, 0x53, 0x50, 0x20, 0x38, 0x30, 0x30, 0x2d, 0x39, 0x30, 0x41, 0x20, 0x44,    \
        0x52, 0x42, 0x47, 0x00 };
#else
...
struct prov_drbg_st {
    ...
    int (*reseed)(PROV_DRBG* drbg, const unsigned char* ent, size_t ent_len, const unsigned char* adin, size_t adin_len);
...
PROV_DRBG* ossl_rand_drbg_new(void* provctx, void* parent, const OSSL_DISPATCH* parent_dispatch, int (*dnew)(PROV_DRBG* ctx),
    int (*instantiate)(PROV_DRBG* drbg, const unsigned char* entropy, size_t entropylen, const unsigned char* nonce, size_t noncelen, const unsigned char* pers,
        size_t perslen),
...
#define OSSL_PARAM_DRBG_SETTABLE_CTX_COMMON                                                                                                                    \
    OSSL_PARAM_uint(OSSL_DRBG_PARAM_RESEED_REQUESTS, NULL), OSSL_PARAM_uint64(OSSL_DRBG_PARAM_RESEED_TIME_INTERVAL, NULL)

#define OSSL_PARAM_DRBG_GETTABLE_CTX_COMMON                                                                                                                    \
    OSSL_PARAM_int(OSSL_RAND_PARAM_STATE, NULL), OSSL_PARAM_uint(OSSL_RAND_PARAM_STRENGTH, NULL), OSSL_PARAM_size_t(OSSL_RAND_PARAM_MAX_REQUEST, NULL),        \
        OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MIN_ENTROPYLEN, NULL), OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MAX_ENTROPYLEN, NULL),                                      \
        OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MIN_NONCELEN, NULL), OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MAX_NONCELEN, NULL),                                          \
        OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MAX_PERSLEN, NULL), OSSL_PARAM_size_t(OSSL_DRBG_PARAM_MAX_ADINLEN, NULL),                                            \
        OSSL_PARAM_uint(OSSL_DRBG_PARAM_RESEED_COUNTER, NULL), OSSL_PARAM_time_t(OSSL_DRBG_PARAM_RESEED_TIME, NULL),                                           \
        OSSL_PARAM_uint(OSSL_DRBG_PARAM_RESEED_REQUESTS, NULL), OSSL_PARAM_uint64(OSSL_DRBG_PARAM_RESEED_TIME_INTERVAL, NULL)
...
void ossl_crngt_cleanup_entropy(PROV_DRBG* drbg, unsigned char* out, size_t outlen);
