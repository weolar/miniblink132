...
static int ossl_drbg_lock_parent(PROV_DRBG* drbg)
{
    ...
    if (parent != NULL && drbg->parent_lock != NULL && !drbg->parent_lock(parent)) {
 ...
 */
size_t ossl_drbg_get_seed(
    void* vdrbg, unsigned char** pout, int entropy, size_t min_len, size_t max_len, int prediction_resistance, const unsigned char* adin, size_t adin_len)
{
     ...
     */
    if (!ossl_prov_drbg_generate(drbg, buffer, bytes_needed, drbg->strength, prediction_resistance, (unsigned char*)&drbg, sizeof(drbg))) {
...
static size_t get_entropy(PROV_DRBG* drbg, unsigned char** pout, int entropy, size_t min_len, size_t max_len, int prediction_resistance)
{
    size_t bytes;
    unsigned int p_str;
    ...
    if (drbg->parent == NULL)
#ifdef FIPS_MODULE
        return ossl_crngt_get_entropy(drbg, pout, entropy, min_len, max_len, prediction_resistance);
#else
     ...
     *       intentional and correct here.
     */
    bytes = drbg->parent_get_seed(drbg->parent, pout, drbg->strength, min_len, max_len, prediction_resistance, (unsigned char*)&drbg, sizeof(drbg));
    ossl_drbg_unlock_parent(drbg);
    return bytes;
}
...
static size_t prov_drbg_get_nonce(PROV_DRBG* drbg, unsigned char** pout, size_t min_len, size_t max_len)
{
    size_t ret = 0, n;
    unsigned char* buf = NULL;
    OSSL_LIB_CTX* libctx = ossl_prov_ctx_get0_libctx(drbg->provctx);
    PROV_DRBG_NONCE_GLOBAL* dngbl = ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_DRBG_NONCE_INDEX, &drbg_nonce_ossl_ctx_method);
    struct {
        void* drbg;
        int count;
    } data;
    ...
    if (drbg->parent != NULL && drbg->parent_nonce != NULL) {
        n = drbg->parent_nonce(drbg->parent, NULL, 0, drbg->min_noncelen, drbg->max_noncelen);
        if (n > 0 && (buf = OPENSSL_malloc(n)) != NULL) {
            ret = drbg->parent_nonce(drbg->parent, buf, 0, drbg->min_noncelen, drbg->max_noncelen);
            if (ret == n) {
                *pout = buf;
                return ret;
            }
 ...
 */
int ossl_prov_drbg_instantiate(PROV_DRBG* drbg, unsigned int strength, int prediction_resistance, const unsigned char* pers, size_t perslen)
{
    ...
    if (drbg->min_noncelen > 0) {
        if (drbg->parent_nonce != NULL) {
            noncelen = drbg->parent_nonce(drbg->parent, NULL, drbg->strength, drbg->min_noncelen, drbg->max_noncelen);
            if (noncelen == 0) {
            ...
            noncelen = prov_drbg_get_nonce(drbg, &nonce, drbg->min_noncelen, drbg->max_noncelen);
    ...
    entropylen = get_entropy(drbg, &entropy, min_entropy, min_entropylen, max_entropylen, prediction_resistance);
    if (entropylen < min_entropylen || entropylen > max_entropylen) {
        ERR_raise(ERR_LIB_PROV, PROV_R_ERROR_RETRIEVING_ENTROPY);
        goto end;
    }

    if (!drbg->instantiate(drbg, entropy, entropylen, nonce, noncelen, pers, perslen)) {
 ...
 * Returns 1 on success, 0 on failure.
 */
int ossl_prov_drbg_reseed(PROV_DRBG* drbg, int prediction_resistance, const unsigned char* ent, size_t ent_len, const unsigned char* adin, size_t adinlen)
{
    ...
    entropylen = get_entropy(drbg, &entropy, drbg->strength, drbg->min_entropylen, drbg->max_entropylen, prediction_resistance);
 ...
 *
 */
int ossl_prov_drbg_generate(
    PROV_DRBG* drbg, unsigned char* out, size_t outlen, unsigned int strength, int prediction_resistance, const unsigned char* adin, size_t adinlen)
{
    int fork_id;
    ...
    if (drbg->parent != NULL && get_parent_reseed_count(drbg) != drbg->parent_reseed_counter)
        reseed_required = 1;

    if (reseed_required || prediction_resistance) {
        if (!ossl_prov_drbg_reseed(drbg, prediction_resistance, NULL, 0, adin, adinlen)) {
...
PROV_DRBG* ossl_rand_drbg_new(void* provctx, void* parent, const OSSL_DISPATCH* p_dispatch, int (*dnew)(PROV_DRBG* ctx),
    int (*instantiate)(PROV_DRBG* drbg, const unsigned char* entropy, size_t entropylen, const unsigned char* nonce, size_t noncelen, const unsigned char* pers,
        size_t perslen),
    int (*uninstantiate)(PROV_DRBG* ctx), int (*reseed)(PROV_DRBG* drbg, const unsigned char* ent, size_t ent_len, const unsigned char* adin, size_t adin_len),
