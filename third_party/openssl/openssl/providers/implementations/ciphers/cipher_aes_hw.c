...
static int cipher_hw_aes_initkey(PROV_CIPHER_CTX* dat, const unsigned char* key, size_t keylen)
{
    ...
    if ((dat->mode == EVP_CIPH_ECB_MODE || dat->mode == EVP_CIPH_CBC_MODE) && !dat->enc) {
#ifdef HWAES_CAPABLE
        if (HWAES_CAPABLE) {
            ...
            dat->stream.cbc = (dat->mode == EVP_CIPH_CBC_MODE) ? (cbc128_f)vpaes_cbc_encrypt : NULL;
        } else
#endif
        {
            ret = AES_set_decrypt_key(key, keylen * 8, ks);
            dat->block = (block128_f)AES_decrypt;
            dat->stream.cbc = (dat->mode == EVP_CIPH_CBC_MODE) ? (cbc128_f)AES_cbc_encrypt : NULL;
        }
    } else
#ifdef HWAES_CAPABLE
        if (HWAES_CAPABLE) {
...
#define PROV_CIPHER_HW_aes_mode(mode)                                                                                                                          \
    static const PROV_CIPHER_HW aes_##mode = { cipher_hw_aes_initkey, ossl_cipher_hw_generic_##mode, cipher_hw_aes_copyctx };                                  \
    PROV_CIPHER_HW_declare(mode) const PROV_CIPHER_HW* ossl_prov_cipher_hw_aes_##mode(size_t keybits)                                                          \
    {                                                                                                                                                          \
...
PROV_CIPHER_HW_aes_mode(cbc) PROV_CIPHER_HW_aes_mode(ecb) PROV_CIPHER_HW_aes_mode(ofb128) PROV_CIPHER_HW_aes_mode(cfb128) PROV_CIPHER_HW_aes_mode(cfb1)
    PROV_CIPHER_HW_aes_mode(cfb8) PROV_CIPHER_HW_aes_mode(ctr)
