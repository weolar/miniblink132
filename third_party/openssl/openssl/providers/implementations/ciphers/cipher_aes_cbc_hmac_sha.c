...
#define IMPLEMENT_CIPHER(nm, sub, kbits, blkbits, ivbits, flags) const OSSL_DISPATCH ossl_##nm##kbits##sub##_functions[] = { { 0, NULL } };
#else
...
static int aes_einit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int aes_dinit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static const OSSL_PARAM cipher_aes_known_settable_ctx_params[]
    = { OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_AEAD_MAC_KEY, NULL, 0), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_AEAD_TLS1_AAD, NULL, 0),
#if !defined(OPENSSL_NO_MULTIBLOCK)
          OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_SEND_FRAGMENT, NULL), OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD, NULL),
          OSSL_PARAM_uint(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_INTERLEAVE, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC, NULL, 0),
...
const OSSL_PARAM* aes_settable_ctx_params(ossl_unused void* cctx, ossl_unused void* provctx)
{
...
static int aes_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
...
#if !defined(OPENSSL_NO_MULTIBLOCK)
    p = OSSL_PARAM_locate_const(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_SEND_FRAGMENT);
    if (p != NULL && !OSSL_PARAM_get_size_t(p, &ctx->multiblock_max_send_fragment)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GET_PARAMETER);
        return 0;
    }
    /*
    ...
    if (p != NULL) {
        ...
        if (p->data_type != OSSL_PARAM_OCTET_STRING || p1 == NULL || !OSSL_PARAM_get_uint(p1, &mb_param.interleave)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GET_PARAMETER);
            return 0;
        }
        mb_param.inp = p->data;
    ...
    if (p != NULL) {
        const OSSL_PARAM* p1 = OSSL_PARAM_locate_const(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_INTERLEAVE);
        const OSSL_PARAM* pin = OSSL_PARAM_locate_const(params, OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_IN);

        if (p->data_type != OSSL_PARAM_OCTET_STRING || pin == NULL || pin->data_type != OSSL_PARAM_OCTET_STRING || p1 == NULL
...
static int aes_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_string(p, ctx->base.oiv, ctx->base.ivlen) && !OSSL_PARAM_set_octet_ptr(p, &ctx->base.oiv, ctx->base.ivlen)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
        return 0;
    }
    p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_UPDATED_IV);
    if (p != NULL && !OSSL_PARAM_set_octet_string(p, ctx->base.iv, ctx->base.ivlen) && !OSSL_PARAM_set_octet_ptr(p, &ctx->base.iv, ctx->base.ivlen)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
        return 0;
    }
    return 1;
...
#if !defined(OPENSSL_NO_MULTIBLOCK)
    OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_BUFSIZE, NULL), OSSL_PARAM_uint(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_INTERLEAVE, NULL),
    OSSL_PARAM_uint(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD_PACKLEN, NULL), OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_LEN, NULL),
#endif /* !defined(OPENSSL_NO_MULTIBLOCK) */
    OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_AEAD_TLS1_AAD_PAD, NULL), OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL),
    OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_IVLEN, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_IV, NULL, 0),
    OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_UPDATED_IV, NULL, 0), OSSL_PARAM_END
};
const OSSL_PARAM* aes_gettable_ctx_params(ossl_unused void* cctx, ossl_unused void* provctx)
{
    return cipher_aes_known_gettable_ctx_params;
}

static void base_init(
    void* provctx, PROV_AES_HMAC_SHA_CTX* ctx, const PROV_CIPHER_HW_AES_HMAC_SHA* meths, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ossl_cipher_generic_initkey(&ctx->base, kbits, blkbits, ivbits, EVP_CIPH_CBC_MODE, flags, &meths->base, provctx);
    ctx->hw = (PROV_CIPHER_HW_AES_HMAC_SHA*)ctx->base.hw;
}

static void* aes_cbc_hmac_sha1_newctx(void* provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ...
    if (ctx != NULL)
        base_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha1(), kbits, blkbits, ivbits, flags);
    return ctx;
}
...
static void* aes_cbc_hmac_sha256_newctx(void* provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ...
    ctx = OPENSSL_zalloc(sizeof(*ctx));
    if (ctx != NULL)
        base_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha256(), kbits, blkbits, ivbits, flags);
    return ctx;
}
...
#define IMPLEMENT_CIPHER(nm, sub, kbits, blkbits, ivbits, flags)                                                                                               \
    ...
    {                                                                                                                                                          \
        return ossl_cipher_generic_get_params(params, EVP_CIPH_CBC_MODE, flags, kbits, blkbits, ivbits);                                                       \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##nm##kbits##sub##_functions[] = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))nm##_##kbits##_##sub##_newctx },                    \
        { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))nm##_##sub##_freectx }, { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))nm##_##sub##_dupctx },                  \
        { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))nm##_einit }, { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))nm##_dinit },                          \
        { OSSL_FUNC_CIPHER_UPDATE, (void (*)(void))nm##_update }, { OSSL_FUNC_CIPHER_FINAL, (void (*)(void))nm##_final },                                      \
        { OSSL_FUNC_CIPHER_CIPHER, (void (*)(void))nm##_cipher }, { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))nm##_##kbits##_##sub##_get_params },          \
        { OSSL_FUNC_CIPHER_GETTABLE_PARAMS, (void (*)(void))nm##_gettable_params }, { OSSL_FUNC_CIPHER_GET_CTX_PARAMS, (void (*)(void))nm##_get_ctx_params },  \
        { OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS, (void (*)(void))nm##_gettable_ctx_params },                                                                    \
        { OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))nm##_set_ctx_params },                                                                              \
        { OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))nm##_settable_ctx_params }, { 0, NULL } };
