...
static int cipher_hw_sm4_initkey(PROV_CIPHER_CTX* ctx, const unsigned char* key, size_t keylen)
{
...
#define PROV_CIPHER_HW_sm4_mode(mode)                                                                                                                          \
    static const PROV_CIPHER_HW sm4_##mode = { cipher_hw_sm4_initkey, ossl_cipher_hw_chunked_##mode, cipher_hw_sm4_copyctx };                                  \
    const PROV_CIPHER_HW* ossl_prov_cipher_hw_sm4_##mode(size_t keybits)                                                                                       \
    {                                                                                                                                                          \
        return &sm4_##mode;                                                                                                                                    \
    }

PROV_CIPHER_HW_sm4_mode(cbc) PROV_CIPHER_HW_sm4_mode(ecb) PROV_CIPHER_HW_sm4_mode(ofb128) PROV_CIPHER_HW_sm4_mode(cfb128) PROV_CIPHER_HW_sm4_mode(ctr)
