...
typedef size_t (*aeswrap_fn)(void* key, const unsigned char* iv, unsigned char* out, const unsigned char* in, size_t inlen, block128_f block);
...
static int aes_wrap_init(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[], int enc)
{
...
static int aes_wrap_einit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
    return aes_wrap_init(ctx, key, keylen, iv, ivlen, params, 1);
}

static int aes_wrap_dinit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int aes_wrap_cipher(void* vctx, unsigned char* out, size_t* outl, size_t outsize, const unsigned char* in, size_t inl)
{
...
#define IMPLEMENT_cipher(mode, fname, UCMODE, flags, kbits, blkbits, ivbits)                                                                                   \
    ...
    {                                                                                                                                                          \
        return aes_##mode##_newctx(kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags);                                                                   \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##aes##kbits##fname##_functions[] = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))aes_##kbits##fname##_newctx },                   \
        { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))aes_##mode##_einit }, { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))aes_##mode##_dinit },          \
        { OSSL_FUNC_CIPHER_UPDATE, (void (*)(void))aes_##mode##_cipher }, { OSSL_FUNC_CIPHER_FINAL, (void (*)(void))aes_##mode##_final },                      \
        { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))aes_##mode##_freectx }, { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))aes_##mode##_dupctx },                  \
        { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))aes_##kbits##_##fname##_get_params },                                                                   \
        ...
        { OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))aes_wrap_set_ctx_params },                                                                          \
