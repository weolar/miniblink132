...
static int cipher_hw_rc4_hmac_md5_initkey(PROV_CIPHER_CTX* bctx, const uint8_t* key, size_t keylen)
{
...
static int cipher_hw_rc4_hmac_md5_cipher(PROV_CIPHER_CTX* bctx, unsigned char* out, const unsigned char* in, size_t len)
{
    PROV_RC4_HMAC_MD5_CTX* ctx = (PROV_RC4_HMAC_MD5_CTX*)bctx;
    ...
    if (ctx->base.enc) {
        ...
        if (len > rc4_off && (blocks = (len - rc4_off) / MD5_CBLOCK) && (OPENSSL_ia32cap_P[0] & (1 << 20)) == 0) {
            ...
            rc4_md5_enc(ks, in + rc4_off, out + rc4_off, &ctx->md, out + md5_off, blocks);
            blocks *= MD5_CBLOCK;
            rc4_off += blocks;
            md5_off += blocks;
...
static int cipher_hw_rc4_hmac_md5_tls_init(PROV_CIPHER_CTX* bctx, unsigned char* aad, size_t aad_len)
{
    PROV_RC4_HMAC_MD5_CTX* ctx = (PROV_RC4_HMAC_MD5_CTX*)bctx;
    unsigned int len;
...
static void cipher_hw_rc4_hmac_md5_init_mackey(PROV_CIPHER_CTX* bctx, const unsigned char* key, size_t len)
{
    PROV_RC4_HMAC_MD5_CTX* ctx = (PROV_RC4_HMAC_MD5_CTX*)bctx;
    unsigned int i;
...
static const PROV_CIPHER_HW_RC4_HMAC_MD5 rc4_hmac_md5_hw
    = { { cipher_hw_rc4_hmac_md5_initkey, cipher_hw_rc4_hmac_md5_cipher }, cipher_hw_rc4_hmac_md5_tls_init, cipher_hw_rc4_hmac_md5_init_mackey };
