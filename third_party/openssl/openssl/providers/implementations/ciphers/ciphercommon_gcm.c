 ...
 */
static int gcm_init(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[], int enc)
{
...
int ossl_gcm_einit(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
    return gcm_init(vctx, key, keylen, iv, ivlen, params, 1);
}

int ossl_gcm_dinit(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
int ossl_gcm_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (!OSSL_PARAM_set_octet_string(p, ctx->iv, ctx->ivlen) && !OSSL_PARAM_set_octet_ptr(p, &ctx->iv, ctx->ivlen)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
            return 0;
        }
    }
    ...
    if (p != NULL) {
        if (p->data == NULL || p->data_type != OSSL_PARAM_OCTET_STRING || !getivgen(ctx, p->data, p->data_size))
            return 0;
    }
    return 1;
}
...
int ossl_gcm_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        if (p->data == NULL || p->data_type != OSSL_PARAM_OCTET_STRING || !setivinv(ctx, p->data, p->data_size))
            return 0;
    }
...
int ossl_gcm_stream_update(void* vctx, unsigned char* out, size_t* outl, size_t outsize, const unsigned char* in, size_t inl)
{
    PROV_GCM_CTX* ctx = (PROV_GCM_CTX*)vctx;
...
int ossl_gcm_stream_final(void* vctx, unsigned char* out, size_t* outl, size_t outsize)
{
    PROV_GCM_CTX* ctx = (PROV_GCM_CTX*)vctx;
    int i;
...
int ossl_gcm_cipher(void* vctx, unsigned char* out, size_t* outl, size_t outsize, const unsigned char* in, size_t inl)
{
    PROV_GCM_CTX* ctx = (PROV_GCM_CTX*)vctx;
...
static int gcm_tls_cipher(PROV_GCM_CTX* ctx, unsigned char* out, size_t* padlen, const unsigned char* in, size_t len)
{
    ...
    if (!ctx->hw->oneshot(ctx, ctx->buf, ctx->tls_aad_len, in, len, out, tag, EVP_GCM_TLS_TAG_LEN)) {
        if (!ctx->enc)
