...
static int chacha_poly1305_tls_iv_set_fixed(PROV_CIPHER_CTX* bctx, unsigned char* fixed, size_t flen)
{
    PROV_CHACHA20_POLY1305_CTX* ctx = (PROV_CHACHA20_POLY1305_CTX*)bctx;
...
static int chacha20_poly1305_initkey(PROV_CIPHER_CTX* bctx, const unsigned char* key, size_t keylen)
{
    PROV_CHACHA20_POLY1305_CTX* ctx = (PROV_CHACHA20_POLY1305_CTX*)bctx;
...
static int chacha20_poly1305_initiv(PROV_CIPHER_CTX* bctx)
{
    ...
    if (bctx->enc)
        ret = ossl_chacha20_einit(&ctx->chacha, NULL, 0, tempiv, sizeof(tempiv), NULL);
    else
        ret = ossl_chacha20_dinit(&ctx->chacha, NULL, 0, tempiv, sizeof(tempiv), NULL);
...
static int chacha20_poly1305_tls_cipher(PROV_CIPHER_CTX* bctx, unsigned char* out, size_t* out_padlen, const unsigned char* in, size_t len)
{
    PROV_CHACHA20_POLY1305_CTX* ctx = (PROV_CHACHA20_POLY1305_CTX*)bctx;
    ...
    if (plen <= CHACHA_BLK_SIZE) {
        ...
        ctx->chacha.counter[0] = 0;
        ChaCha20_ctr32(buf, zero, (buf_len = 2 * CHACHA_BLK_SIZE), ctx->chacha.key.d, ctx->chacha.counter);
        Poly1305_Init(poly, buf);
        ctx->chacha.partial_len = 0;
        memcpy(tohash, ctx->tls_aad, POLY1305_BLOCK_SIZE);
        tohash_len = POLY1305_BLOCK_SIZE;
        ...
        ctx->chacha.counter[0] = 0;
        ChaCha20_ctr32(buf, zero, (buf_len = CHACHA_BLK_SIZE), ctx->chacha.key.d, ctx->chacha.counter);
        Poly1305_Init(poly, buf);
    ...
    if (bctx->enc) {
        ...
        if (CRYPTO_memcmp(tohash, in, POLY1305_BLOCK_SIZE)) {
            if (len > POLY1305_BLOCK_SIZE)
                memset(out - (len - POLY1305_BLOCK_SIZE), 0, len - POLY1305_BLOCK_SIZE);
            return 0;
        }
        /* Strip the tag */
...
static int chacha20_poly1305_aead_cipher(PROV_CIPHER_CTX* bctx, unsigned char* out, size_t* outl, const unsigned char* in, size_t inl)
{
    PROV_CHACHA20_POLY1305_CTX* ctx = (PROV_CHACHA20_POLY1305_CTX*)bctx;
    POLY1305* poly = &ctx->poly1305;
    ...
    if (!ctx->mac_inited) {
        ...
        ctx->chacha.counter[0] = 0;
        ChaCha20_ctr32(ctx->chacha.buf, zero, CHACHA_BLK_SIZE, ctx->chacha.key.d, ctx->chacha.counter);
    ...
    if (in == NULL || inl != plen) {
        ...
        if (IS_LITTLE_ENDIAN) {
            Poly1305_Update(poly, (unsigned char*)&ctx->len, POLY1305_BLOCK_SIZE);
        } else {
...
static const PROV_CIPHER_HW_CHACHA20_POLY1305 chacha20poly1305_hw = { { chacha20_poly1305_initkey, NULL }, chacha20_poly1305_aead_cipher,
