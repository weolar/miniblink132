...
static int rc4_einit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int rc4_dinit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
#define IMPLEMENT_cipher(alg, UCALG, flags, kbits, blkbits, ivbits, typ)                                                                                       \
    ...
    {                                                                                                                                                          \
        return ossl_cipher_generic_get_params(params, 0, flags, kbits, blkbits, ivbits);                                                                       \
    }                                                                                                                                                          \
    ...
    {                                                                                                                                                          \
        ...
        if (ctx != NULL) {                                                                                                                                     \
            ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, 0, flags, ossl_prov_cipher_hw_##alg(kbits), NULL);                                        \
        }                                                                                                                                                      \
        return ctx;                                                                                                                                            \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##alg##kbits##_functions[] = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))alg##_##kbits##_newctx },                               \
        { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))alg##_freectx }, { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))alg##_dupctx },                                \
        { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))rc4_einit }, { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))rc4_dinit },                            \
        ...
        { OSSL_FUNC_CIPHER_CIPHER, (void (*)(void))ossl_cipher_generic_cipher }, { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))alg##_##kbits##_get_params },  \
