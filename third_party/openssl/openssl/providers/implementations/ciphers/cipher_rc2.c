...
static int rc2_einit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int rc2_dinit(void* ctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
...
static int rc2_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (!ASN1_TYPE_set_int_octetstring(type, num, ctx->base.iv, ctx->base.ivlen)) {
            ASN1_TYPE_free(type);
            ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
            return 0;
        }
...
static int rc2_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (p->data_type != OSSL_PARAM_OCTET_STRING || ctx->base.ivlen > sizeof(iv) || (type = d2i_ASN1_TYPE(NULL, &d, p->data_size)) == NULL
            || ((size_t)ASN1_TYPE_get_int_octetstring(type, &num, iv, ctx->base.ivlen) != ctx->base.ivlen)
            || !ossl_cipher_generic_initiv(&ctx->base, iv, ctx->base.ivlen) || (ctx->key_bits = rc2_magic_to_keybits(num)) == 0) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
            ret = 0;
        }
        ASN1_TYPE_free(type);
...
OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_RC2_KEYBITS, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_ALGORITHM_ID_PARAMS, NULL, 0),
    CIPHER_DEFAULT_GETTABLE_CTX_PARAMS_END(rc2)

        CIPHER_DEFAULT_SETTABLE_CTX_PARAMS_START(rc2) OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL),
    OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_RC2_KEYBITS, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_ALGORITHM_ID_PARAMS, NULL, 0),
    CIPHER_DEFAULT_SETTABLE_CTX_PARAMS_END(rc2)

#define IMPLEMENT_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)                                                                       \
    ...
    {                                                                                                                                                          \
        ...
        if (ctx != NULL) {                                                                                                                                     \
            ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags, ossl_prov_cipher_hw_##alg##_##lcmode(kbits), NULL);      \
            ctx->key_bits = kbits;                                                                                                                             \
        }                                                                                                                                                      \
        return ctx;                                                                                                                                            \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##alg##kbits##lcmode##_functions[]                                                                                                \
        = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))alg##_##kbits##_##lcmode##_newctx }, { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))alg##_freectx },       \
              { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))alg##_dupctx }, { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))rc2_einit },                         \
              { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))rc2_dinit }, { OSSL_FUNC_CIPHER_UPDATE, (void (*)(void))ossl_cipher_generic_##typ##_update },   \
              ...
              { OSSL_FUNC_CIPHER_GET_CTX_PARAMS, (void (*)(void))rc2_get_ctx_params },                                                                         \
              { OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS, (void (*)(void))rc2_gettable_ctx_params },                                                               \
              { OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))rc2_set_ctx_params },                                                                         \
              { OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))rc2_settable_ctx_params }, { 0, NULL } };
