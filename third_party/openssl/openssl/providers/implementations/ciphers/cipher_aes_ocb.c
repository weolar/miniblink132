 ...
 */
static ossl_inline int aes_generic_ocb_setiv(PROV_AES_OCB_CTX* ctx, const unsigned char* iv, size_t ivlen, size_t taglen)
{
    return (CRYPTO_ocb128_setiv(&ctx->ocb, iv, ivlen, taglen) == 1);
}

static ossl_inline int aes_generic_ocb_setaad(PROV_AES_OCB_CTX* ctx, const unsigned char* aad, size_t alen)
{
    return CRYPTO_ocb128_aad(&ctx->ocb, aad, alen) == 1;
}

static ossl_inline int aes_generic_ocb_gettag(PROV_AES_OCB_CTX* ctx, unsigned char* tag, size_t tlen)
{
...
static ossl_inline int aes_generic_ocb_cipher(PROV_AES_OCB_CTX* ctx, const unsigned char* in, unsigned char* out, size_t len)
{
    if (ctx->base.enc) {
...
static ossl_inline int aes_generic_ocb_copy_ctx(PROV_AES_OCB_CTX* dst, PROV_AES_OCB_CTX* src)
{
    return CRYPTO_ocb128_copy_ctx(&dst->ocb, &src->ocb, &dst->ksenc.ks, &dst->ksdec.ks);
}

/*-
 * Provider dispatch functions
 */
static int aes_ocb_init(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[], int enc)
{
...
static int aes_ocb_einit(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
    return aes_ocb_init(vctx, key, keylen, iv, ivlen, params, 1);
}

static int aes_ocb_dinit(void* vctx, const unsigned char* key, size_t keylen, const unsigned char* iv, size_t ivlen, const OSSL_PARAM params[])
{
 ...
 */
static int aes_ocb_block_update_internal(PROV_AES_OCB_CTX* ctx, unsigned char* buf, size_t* bufsz, unsigned char* out, size_t* outl, size_t outsize,
    const unsigned char* in, size_t inl, OSSL_ocb_cipher_fn ciph)
{
    size_t nextblocks;
    size_t outlint = 0;
    ...
    if (inl != 0 && !ossl_cipher_trailingdata(buf, bufsz, AES_BLOCK_SIZE, &in, &inl)) {
...
static int cipher_updateaad(PROV_AES_OCB_CTX* ctx, const unsigned char* in, unsigned char* out, size_t len)
{
...
static int update_iv(PROV_AES_OCB_CTX* ctx)
{
    ...
    if (ctx->iv_state == IV_STATE_BUFFERED) {
        if (!aes_generic_ocb_setiv(ctx, ctx->base.iv, ctx->base.ivlen, ctx->taglen))
            return 0;
...
static int aes_ocb_block_update(void* vctx, unsigned char* out, size_t* outl, size_t outsize, const unsigned char* in, size_t inl)
{
    PROV_AES_OCB_CTX* ctx = (PROV_AES_OCB_CTX*)vctx;
    unsigned char* buf;
    size_t* buflen;
    ...
    return aes_ocb_block_update_internal(ctx, buf, buflen, out, outl, outsize, in, inl, fn);
}

static int aes_ocb_block_final(void* vctx, unsigned char* out, size_t* outl, size_t outsize)
{
    PROV_AES_OCB_CTX* ctx = (PROV_AES_OCB_CTX*)vctx;
...
static void* aes_ocb_newctx(void* provctx, size_t kbits, size_t blkbits, size_t ivbits, unsigned int mode, uint64_t flags)
{
    ...
    if (ctx != NULL) {
        ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, mode, flags, ossl_prov_cipher_hw_aes_ocb(kbits), NULL);
...
static int aes_ocb_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (!OSSL_PARAM_set_octet_string(p, ctx->base.oiv, ctx->base.ivlen) && !OSSL_PARAM_set_octet_ptr(p, &ctx->base.oiv, ctx->base.ivlen)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
            return 0;
        }
    }
    ...
    if (p != NULL) {
        ...
        if (!OSSL_PARAM_set_octet_string(p, ctx->base.iv, ctx->base.ivlen) && !OSSL_PARAM_set_octet_ptr(p, &ctx->base.iv, ctx->base.ivlen)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
            return 0;
        }
    }
...
static const OSSL_PARAM cipher_ocb_known_gettable_ctx_params[]
    = { OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL), OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_IVLEN, NULL),
          OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_AEAD_TAGLEN, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_IV, NULL, 0),
          OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_UPDATED_IV, NULL, 0), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, NULL, 0), OSSL_PARAM_END };
...
static const OSSL_PARAM cipher_ocb_known_settable_ctx_params[] = { OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL),
    OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_AEAD_IVLEN, NULL), OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, NULL, 0), OSSL_PARAM_END };
...
#define IMPLEMENT_cipher(mode, UCMODE, flags, kbits, blkbits, ivbits)                                                                                          \
    ...
    {                                                                                                                                                          \
        return ossl_cipher_generic_get_params(params, EVP_CIPH_##UCMODE##_MODE, flags, kbits, blkbits, ivbits);                                                \
    }                                                                                                                                                          \
    static OSSL_FUNC_cipher_newctx_fn aes_##kbits##_##mode##_newctx;                                                                                           \
    static void* aes_##kbits##_##mode##_newctx(void* provctx)                                                                                                  \
    {                                                                                                                                                          \
        return aes_##mode##_newctx(provctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags);                                                          \
    }                                                                                                                                                          \
    const OSSL_DISPATCH ossl_##aes##kbits##mode##_functions[] = { { OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))aes_##kbits##_##mode##_newctx },                  \
        { OSSL_FUNC_CIPHER_ENCRYPT_INIT, (void (*)(void))aes_##mode##_einit }, { OSSL_FUNC_CIPHER_DECRYPT_INIT, (void (*)(void))aes_##mode##_dinit },          \
        { OSSL_FUNC_CIPHER_UPDATE, (void (*)(void))aes_##mode##_block_update }, { OSSL_FUNC_CIPHER_FINAL, (void (*)(void))aes_##mode##_block_final },          \
        { OSSL_FUNC_CIPHER_CIPHER, (void (*)(void))aes_ocb_cipher }, { OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))aes_##mode##_freectx },                       \
        { OSSL_FUNC_CIPHER_DUPCTX, (void (*)(void))aes_##mode##_dupctx }, { OSSL_FUNC_CIPHER_GET_PARAMS, (void (*)(void))aes_##kbits##_##mode##_get_params },  \
        { OSSL_FUNC_CIPHER_GET_CTX_PARAMS, (void (*)(void))aes_##mode##_get_ctx_params },                                                                      \
        { OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))aes_##mode##_set_ctx_params },                                                                      \
        { OSSL_FUNC_CIPHER_GETTABLE_PARAMS, (void (*)(void))ossl_cipher_generic_gettable_params },                                                             \
        { OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS, (void (*)(void))cipher_ocb_gettable_ctx_params },                                                              \
        { OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))cipher_ocb_settable_ctx_params }, { 0, NULL } }
