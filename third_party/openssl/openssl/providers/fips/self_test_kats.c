...
static int self_test_digest(const ST_KAT_DIGEST* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    ...
    if (ctx == NULL || md == NULL || !EVP_DigestInit_ex(ctx, md, NULL) || !EVP_DigestUpdate(ctx, t->pt, t->pt_len) || !EVP_DigestFinal(ctx, out, &out_len))
        goto err;
...
static int cipher_init(EVP_CIPHER_CTX* ctx, const EVP_CIPHER* cipher, const ST_KAT_CIPHER* t, int enc)
{
    ...
    return EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc) && (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, t->iv_len, NULL) > 0)
        && (in_tag == NULL || EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, t->tag_len, in_tag) > 0) && EVP_CipherInit_ex(ctx, NULL, NULL, t->key, t->iv, enc)
        && EVP_CIPHER_CTX_set_padding(ctx, pad) && EVP_CipherUpdate(ctx, NULL, &tmp, t->aad, t->aad_len);
}
...
static int self_test_cipher(const ST_KAT_CIPHER* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    ...
    if ((t->mode & CIPHER_MODE_ENCRYPT) != 0) {
        if (!cipher_init(ctx, cipher, t, encrypt) || !EVP_CipherUpdate(ctx, ct_buf, &len, t->base.pt, t->base.pt_len)
        ...
        if (t->tag != NULL) {
            ...
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, t->tag_len, tag) <= 0 || memcmp(tag, t->tag, t->tag_len) != 0)
                goto err;
        }
    }
    ...
    if ((t->mode & CIPHER_MODE_DECRYPT) != 0) {
        if (!(cipher_init(ctx, cipher, t, !encrypt) && EVP_CipherUpdate(ctx, pt_buf, &len, t->base.expected, t->base.expected_len)
...
static int add_params(OSSL_PARAM_BLD* bld, const ST_KAT_PARAM* params, BN_CTX* ctx)
{
    ...
    for (p = params; p->data != NULL; ++p) {
        ...
        case OSSL_PARAM_UNSIGNED_INTEGER: {
            ...
            if (bn == NULL || (BN_bin2bn(p->data, p->data_len, bn) == NULL) || !OSSL_PARAM_BLD_push_BN(bld, p->name, bn))
                goto err;
            break;
        }
...
static int self_test_drbg(const ST_KAT_DRBG* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    ...
    drbg_params[0] = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY, (void*)t->entropyin, t->entropyinlen);
    ...
    drbg_params[0] = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY, (void*)t->entropyinpr1, t->entropyinpr1len);
    if (!EVP_RAND_CTX_set_params(test, drbg_params))
        goto err;

    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength, prediction_resistance, t->entropyaddin1, t->entropyaddin1len))
        goto err;

    drbg_params[0] = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY, (void*)t->entropyinpr2, t->entropyinpr2len);
    if (!EVP_RAND_CTX_set_params(test, drbg_params))
        goto err;
     ...
     */
    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength, prediction_resistance, t->entropyaddin2, t->entropyaddin2len))
        goto err;
...
static int self_test_ka(const ST_KAT_KAS* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    ...
    if (!add_params(bld, t->key_group, bnctx) || !add_params(bld, t->key_host_data, bnctx))
        goto err;
    params = OSSL_PARAM_BLD_to_param(bld);

    if (!add_params(bld, t->key_group, bnctx) || !add_params(bld, t->key_peer_data, bnctx))
        goto err;
    ...
    if (EVP_PKEY_derive_init(dctx) <= 0 || EVP_PKEY_derive_set_peer(dctx, peerkey) <= 0 || EVP_PKEY_derive(dctx, secret, &secret_len) <= 0)
        goto err;
...
static int self_test_sign(const ST_KAT_SIGN* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    int ret = 0;
    ...
    static const unsigned char dgst[] = { 0x7f, 0x83, 0xb1, 0x65, 0x7f, 0xf1, 0xfc, 0x53, 0xb9, 0x2d, 0xc1, 0x81, 0x48, 0xa1, 0xd6, 0x5d, 0xfc, 0x2d, 0x4b,
        0x1f, 0xa3, 0xd6, 0x77, 0x28, 0x4a, 0xdd, 0xd2, 0x00, 0x12, 0x6d, 0x90, 0x69 };
    ...
    if (kctx == NULL || params == NULL)
        goto err;
    if (EVP_PKEY_fromdata_init(kctx) <= 0 || EVP_PKEY_fromdata(kctx, &pkey, EVP_PKEY_KEYPAIR, params) <= 0)
        goto err;
    ...
    if (!OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_SIGNATURE_PARAM_DIGEST, t->mdalgorithm, strlen(t->mdalgorithm) + 1))
        goto err;
    params_sig = OSSL_PARAM_BLD_to_param(bld);
    if (EVP_PKEY_CTX_set_params(sctx, params_sig) <= 0)
        goto err;

    if (EVP_PKEY_sign(sctx, sig, &siglen, dgst, sizeof(dgst)) <= 0 || EVP_PKEY_verify_init(sctx) <= 0 || EVP_PKEY_CTX_set_params(sctx, params_sig) <= 0)
        goto err;
     ...
     */
    if (t->sig_expected != NULL && (siglen != t->sig_expected_len || memcmp(sig, t->sig_expected, t->sig_expected_len) != 0))
        goto err;
 ...
 * and decrypt..
 */
static int self_test_asym_cipher(const ST_KAT_ASYM_CIPHER* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
    int ret = 0;
    ...
    if (keybld == NULL || !add_params(keybld, t->key, bnctx))
        goto err;
    ...
    if (keyctx == NULL || keyparams == NULL)
        goto err;
    if (EVP_PKEY_fromdata_init(keyctx) <= 0 || EVP_PKEY_fromdata(keyctx, &key, EVP_PKEY_KEYPAIR, keyparams) <= 0)
        goto err;

    /* Create a EVP_PKEY_CTX to use for the encrypt or decrypt operation */
    encctx = EVP_PKEY_CTX_new_from_pkey(libctx, key, NULL);
    if (encctx == NULL || (t->encrypt && EVP_PKEY_encrypt_init(encctx) <= 0) || (!t->encrypt && EVP_PKEY_decrypt_init(encctx) <= 0))
        goto err;
    ...
    if (t->encrypt) {
        if (EVP_PKEY_encrypt(encctx, out, &outlen, t->in, t->in_len) <= 0)
            goto err;
    } else {
        if (EVP_PKEY_decrypt(encctx, out, &outlen, t->in, t->in_len) <= 0)
            goto err;
    }
    /* Check the KAT */
    OSSL_SELF_TEST_oncorrupt_byte(st, out);
    if (outlen != t->expected_len || memcmp(out, t->expected, t->expected_len) != 0)
        goto err;
