...
static int store_loader_cmp(const OSSL_STORE_LOADER* a, const OSSL_STORE_LOADER* b)
{
...
int ossl_store_register_loader_int(OSSL_STORE_LOADER* loader)
{
    ...
    if (loader->open == NULL || loader->load == NULL || loader->eof == NULL || loader->error == NULL || loader->closefn == NULL) {
    ...
    if (ossl_store_register_init() && (lh_OSSL_STORE_LOADER_insert(loader_register, loader) != NULL || lh_OSSL_STORE_LOADER_error(loader_register) == 0))
        ok = 1;
...
const OSSL_STORE_LOADER* ossl_store_get0_loader_int(const char* scheme)
{
    ...
    else if ((loader = lh_OSSL_STORE_LOADER_retrieve(loader_register, &template)) == NULL)
        ERR_raise_data(ERR_LIB_OSSL_STORE, OSSL_STORE_R_UNREGISTERED_SCHEME, "scheme=%s", scheme);
...
OSSL_STORE_LOADER* ossl_store_unregister_loader_int(const char* scheme)
{
    ...
    if (!ossl_store_register_init())
        ERR_raise(ERR_LIB_OSSL_STORE, ERR_R_INTERNAL_ERROR);
    else if ((loader = lh_OSSL_STORE_LOADER_delete(loader_register, &template)) == NULL)
        ERR_raise_data(ERR_LIB_OSSL_STORE, OSSL_STORE_R_UNREGISTERED_SCHEME, "scheme=%s", scheme);
