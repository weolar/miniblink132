...
static int try_key(struct extracted_param_data_st*, OSSL_STORE_INFO**, OSSL_STORE_CTX*, const OSSL_PROVIDER*, OSSL_LIB_CTX*, const char*);
static int try_cert(struct extracted_param_data_st*, OSSL_STORE_INFO**, OSSL_LIB_CTX*, const char*);
static int try_crl(struct extracted_param_data_st*, OSSL_STORE_INFO**, OSSL_LIB_CTX*, const char*);
static int try_pkcs12(struct extracted_param_data_st*, OSSL_STORE_INFO**, OSSL_STORE_CTX*, OSSL_LIB_CTX*, const char*);
...
int ossl_store_handle_load_result(const OSSL_PARAM params[], void* arg)
{
    ...
    if (p != NULL && !OSSL_PARAM_get_octet_string_ptr(p, &helper_data.octet_data, &helper_data.octet_data_size)
...
static int try_name(struct extracted_param_data_st* data, OSSL_STORE_INFO** v)
{
    if (data->object_type == OSSL_OBJECT_NAME) {
        ...
        if (data->utf8_data == NULL)
            return 0;
        if ((newname = OPENSSL_strdup(data->utf8_data)) == NULL || (data->desc != NULL && (newdesc = OPENSSL_strdup(data->desc)) == NULL)
...
static EVP_PKEY* try_key_ref(struct extracted_param_data_st* data, OSSL_STORE_CTX* ctx, const OSSL_PROVIDER* provider, OSSL_LIB_CTX* libctx, const char* propq)
{
    EVP_PKEY* pk = NULL;
    ...
    while (keymgmt != NULL && keydata == NULL && try_fallback-- > 0) {
        ...
        if (EVP_KEYMGMT_get0_provider(keymgmt) == provider) {
            ...
            if (export_object != NULL) {
                 ...
                 */
                (void)export_object(ctx->loader_ctx, data->ref, data->ref_size, &evp_keymgmt_util_try_import, &import_data);
            }
        ...
        if (keydata == NULL && try_fallback > 0) {
            ...
            keymgmt = evp_keymgmt_fetch_from_prov((OSSL_PROVIDER*)provider, data->data_type, propq);
            if (keymgmt != NULL) {
                ERR_pop_to_mark();
                ERR_set_mark();
            }
...
static EVP_PKEY* try_key_value(
    struct extracted_param_data_st* data, OSSL_STORE_CTX* ctx, OSSL_PASSPHRASE_CALLBACK* cb, void* cbarg, OSSL_LIB_CTX* libctx, const char* propq)
{
    EVP_PKEY* pk = NULL;
    ...
    case OSSL_STORE_INFO_PUBKEY:
        selection = OSSL_KEYMGMT_SELECT_PUBLIC_KEY | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS;
        break;
    ...
    decoderctx = OSSL_DECODER_CTX_new_for_pkey(&pk, NULL, data->data_structure, data->data_type, selection, libctx, propq);
...
static EVP_PKEY* try_key_value_legacy(struct extracted_param_data_st* data, store_info_new_fn** store_info_new, OSSL_STORE_CTX* ctx,
...
static int try_key(
    struct extracted_param_data_st* data, OSSL_STORE_INFO** v, OSSL_STORE_CTX* ctx, const OSSL_PROVIDER* provider, OSSL_LIB_CTX* libctx, const char* propq)
{
...
static int try_cert(struct extracted_param_data_st* data, OSSL_STORE_INFO** v, OSSL_LIB_CTX* libctx, const char* propq)
{
    if (data->object_type == OSSL_OBJECT_UNKNOWN || data->object_type == OSSL_OBJECT_CERT) {
        ...
        if (data->data_type != NULL && (OPENSSL_strcasecmp(data->data_type, PEM_STRING_X509_TRUSTED) == 0))
            ignore_trusted = 0;

        if (d2i_X509_AUX(&cert, (const unsigned char**)&data->octet_data, data->octet_data_size) == NULL
            && (!ignore_trusted || d2i_X509(&cert, (const unsigned char**)&data->octet_data, data->octet_data_size) == NULL)) {
            X509_free(cert);
            cert = NULL;
        }
...
static int try_crl(struct extracted_param_data_st* data, OSSL_STORE_INFO** v, OSSL_LIB_CTX* libctx, const char* propq)
{
    if (data->object_type == OSSL_OBJECT_UNKNOWN || data->object_type == OSSL_OBJECT_CRL) {
        ...
        crl = d2i_X509_CRL(NULL, (const unsigned char**)&data->octet_data, data->octet_data_size);
...
static int try_pkcs12(struct extracted_param_data_st* data, OSSL_STORE_INFO** v, OSSL_STORE_CTX* ctx, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (data->object_type == OSSL_OBJECT_UNKNOWN) {
        ...
        p12 = d2i_PKCS12(NULL, (const unsigned char**)&data->octet_data, data->octet_data_size);
        ...
        if (p12 != NULL) {
            ...
            if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, NULL, 0)) {
                ...
                OSSL_PARAM pw_params[] = { OSSL_PARAM_utf8_string(OSSL_PASSPHRASE_PARAM_INFO, prompt_info, sizeof(prompt_info) - 1), OSSL_PARAM_END };
                ...
                if ((infos = sk_OSSL_STORE_INFO_new_null()) != NULL) {
                    if (pkey != NULL) {
                        if ((osi_pkey = OSSL_STORE_INFO_new_PKEY(pkey)) != NULL
                            ...
                            && (pkey = NULL) == NULL && sk_OSSL_STORE_INFO_push(infos, osi_pkey) != 0)
                            osi_pkey = NULL;
                        else
                            ok = 0;
                    }
