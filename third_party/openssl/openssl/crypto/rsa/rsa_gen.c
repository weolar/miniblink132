...
BIGNUM* GetRSA_N(RSA* rsa)
{
    return rsa->n;
}

BIGNUM* GetRSA_E(RSA* rsa)
{
    return rsa->e;
}

static int rsa_keygen_pairwise_test(RSA* rsa, OSSL_CALLBACK* cb, void* cbarg);
static int rsa_keygen(OSSL_LIB_CTX* libctx, RSA* rsa, int bits, int primes, BIGNUM* e_value, BN_GENCB* cb, int pairwise_test);
...
static int rsa_keygen_pairwise_test(RSA* rsa, OSSL_CALLBACK* cb, void* cbarg)
{
    ...
    if (st == NULL)
        goto err;
    OSSL_SELF_TEST_onbegin(st, OSSL_SELF_TEST_TYPE_PCT, OSSL_SELF_TEST_DESC_PCT_RSA_PKCS1);
    ...
    if (ciphertxt_len <= 0)
        goto err;
    if (ciphertxt_len == plaintxt_len && memcmp(ciphertxt, plaintxt, plaintxt_len) == 0)
        goto err;
