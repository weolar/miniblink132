...
static int pkey_rsa_sign(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen, const unsigned char* tbs, size_t tbslen)
{
    ...
    if (rctx->md) {
        ...
        if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
            ...
            unsigned int sltmp;
            ret = RSA_sign(EVP_MD_get_type(rctx->md), tbs, tbslen, sig, &sltmp, rsa);
            if (ret <= 0)
                return ret;
            ret = sltmp;
...
static int pkey_rsa_verifyrecover(EVP_PKEY_CTX* ctx, unsigned char* rout, size_t* routlen, const unsigned char* sig, size_t siglen)
{
    int ret;
    RSA_PKEY_CTX* rctx = ctx->data;
    /*
    ...
    if (rctx->md) {
        if (rctx->pad_mode == RSA_X931_PADDING) {
            if (!setup_tbuf(rctx, ctx))
                return -1;
            ret = RSA_public_decrypt(siglen, sig, rctx->tbuf, rsa, RSA_X931_PADDING);
            if (ret < 1)
                return 0;
            ret--;
            ...
            size_t sltmp;
            ret = ossl_rsa_verify(EVP_MD_get_type(rctx->md), NULL, 0, rout, &sltmp, sig, siglen, rsa);
            if (ret <= 0)
                return 0;
            ret = sltmp;
        } else {
...
static int pkey_rsa_verify(EVP_PKEY_CTX* ctx, const unsigned char* sig, size_t siglen, const unsigned char* tbs, size_t tbslen)
{
    ...
    if (rctx->md) {
        if (rctx->pad_mode == RSA_PKCS1_PADDING)
            return RSA_verify(EVP_MD_get_type(rctx->md), tbs, tbslen, sig, siglen, rsa);
        if (tbslen != (size_t)EVP_MD_get_size(rctx->md)) {
            ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_DIGEST_LENGTH);
            return -1;
        }
...
static int pkey_rsa_encrypt(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen, const unsigned char* in, size_t inlen)
{
    int ret;
    RSA_PKEY_CTX* rctx = ctx->data;
    /*
...
static int pkey_rsa_decrypt(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen, const unsigned char* in, size_t inlen)
{
    int ret;
    RSA_PKEY_CTX* rctx = ctx->data;
    /*
    ...
    if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        if (ret <= 0)
            return ret;
        ret = RSA_padding_check_PKCS1_OAEP_mgf1(out, ret, rctx->tbuf, ret, ret, rctx->oaep_label, rctx->oaep_labellen, rctx->md, rctx->mgf1md);
    } else {
        ...
        if (type == EVP_PKEY_CTRL_GET_RSA_PSS_SALTLEN) {
            *(int*)p2 = rctx->saltlen;
            ...
            if (rsa_pss_restricted(rctx)) {
                if (p1 == RSA_PSS_SALTLEN_AUTO && ctx->operation == EVP_PKEY_OP_VERIFY) {
                    ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_PSS_SALTLEN);
                    return -2;
                }
                if ((p1 == RSA_PSS_SALTLEN_DIGEST && rctx->min_saltlen > EVP_MD_get_size(rctx->md)) || (p1 >= 0 && p1 < rctx->min_saltlen)) {
...
static int rsa_set_pss_param(RSA* rsa, EVP_PKEY_CTX* ctx)
{
    ...
    if (rctx->md == NULL && rctx->mgf1md == NULL && rctx->saltlen == -2)
        return 1;
    rsa->pss = ossl_rsa_pss_params_create(rctx->md, rctx->mgf1md, rctx->saltlen == -2 ? 0 : rctx->saltlen);
    if (rsa->pss == NULL)
        return 0;
    return 1;
}
...
static int pkey_rsa_keygen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
    ...
    ret = RSA_generate_multi_prime_key(rsa, rctx->nbits, rctx->primes, rctx->pub_exp, pcb);
    BN_GENCB_free(pcb);
...
static const EVP_PKEY_METHOD rsa_pkey_meth = { EVP_PKEY_RSA, EVP_PKEY_FLAG_AUTOARGLEN, pkey_rsa_init, pkey_rsa_copy, pkey_rsa_cleanup,
    ...
    pkey_rsa_ctrl, pkey_rsa_ctrl_str };
...
static const EVP_PKEY_METHOD rsa_pss_pkey_meth = { EVP_PKEY_RSA_PSS, EVP_PKEY_FLAG_AUTOARGLEN, pkey_rsa_init, pkey_rsa_copy, pkey_rsa_cleanup,

    0, 0,

    0, pkey_rsa_keygen,

    pkey_pss_init, pkey_rsa_sign,

    pkey_pss_init, pkey_rsa_verify,
