...
int RSA_pkey_ctx_ctrl(EVP_PKEY_CTX* ctx, int optype, int cmd, int p1, void* p2)
{
    ...
    if (ctx != NULL && ctx->pmeth != NULL && ctx->pmeth->pkey_id != EVP_PKEY_RSA && ctx->pmeth->pkey_id != EVP_PKEY_RSA_PSS)
        return -1;
...
int ossl_rsa_set0_all_params(RSA* r, const STACK_OF(BIGNUM) * primes, const STACK_OF(BIGNUM) * exps, const STACK_OF(BIGNUM) * coeffs)
{
#ifndef FIPS_MODULE
...
int ossl_rsa_get0_all_params(RSA* r, STACK_OF(BIGNUM_const) * primes, STACK_OF(BIGNUM_const) * exps, STACK_OF(BIGNUM_const) * coeffs)
{
#ifndef FIPS_MODULE
    RSA_PRIME_INFO* pinfo;
    int i, pnum;
...
static int int_set_rsa_md_name(EVP_PKEY_CTX* ctx,
    ...
    const char* mdkey, const char* mdname, const char* propkey, const char* mdprops)
{
...
static int int_get_rsa_md_name(EVP_PKEY_CTX* ctx,
    ...
    const char* mdkey, char* mdname, size_t mdnamesize)
{
...
{
    return int_set_rsa_md_name(ctx, EVP_PKEY_RSA_PSS, EVP_PKEY_OP_KEYGEN, OSSL_PKEY_PARAM_RSA_DIGEST, mdname, OSSL_PKEY_PARAM_RSA_DIGEST_PROPS, mdprops);
}
...
{
    return int_set_rsa_md_name(
        ctx, EVP_PKEY_RSA, EVP_PKEY_OP_TYPE_CRYPT, OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, mdname, OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST_PROPS, mdprops);
}

int EVP_PKEY_CTX_get_rsa_oaep_md_name(EVP_PKEY_CTX* ctx, char* name, size_t namesize)
{
    return int_get_rsa_md_name(ctx, EVP_PKEY_RSA, EVP_PKEY_OP_TYPE_CRYPT, OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST, name, namesize);
}
...
{
    return RSA_pkey_ctx_ctrl(ctx, EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT, EVP_PKEY_CTRL_RSA_MGF1_MD, 0, (void*)(md));
}

int EVP_PKEY_CTX_set_rsa_mgf1_md_name(EVP_PKEY_CTX* ctx, const char* mdname, const char* mdprops)
{
    return int_set_rsa_md_name(
        ctx, -1, EVP_PKEY_OP_TYPE_CRYPT | EVP_PKEY_OP_TYPE_SIG, OSSL_PKEY_PARAM_MGF1_DIGEST, mdname, OSSL_PKEY_PARAM_MGF1_PROPERTIES, mdprops);
}

int EVP_PKEY_CTX_get_rsa_mgf1_md_name(EVP_PKEY_CTX* ctx, char* name, size_t namesize)
{
    return int_get_rsa_md_name(ctx, -1, EVP_PKEY_OP_TYPE_CRYPT | EVP_PKEY_OP_TYPE_SIG, OSSL_PKEY_PARAM_MGF1_DIGEST, name, namesize);
}
...
{
    return int_set_rsa_md_name(ctx, EVP_PKEY_RSA_PSS, EVP_PKEY_OP_KEYGEN, OSSL_PKEY_PARAM_MGF1_DIGEST, mdname, NULL, NULL);
}
...
{
    return RSA_pkey_ctx_ctrl(ctx, EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT, EVP_PKEY_CTRL_GET_RSA_MGF1_MD, 0, (void*)(md));
}
