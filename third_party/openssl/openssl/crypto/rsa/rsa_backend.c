...
static int collect_numbers(STACK_OF(BIGNUM) * numbers, const OSSL_PARAM params[], const char* names[])
{
...
int ossl_rsa_fromdata(RSA* rsa, const OSSL_PARAM params[], int include_private)
{
    ...
    if ((param_n != NULL && !OSSL_PARAM_get_BN(param_n, &n)) || (param_e != NULL && !OSSL_PARAM_get_BN(param_e, &e))
    ...
    if (is_private) {
        if (!collect_numbers(factors = sk_BIGNUM_new_null(), params, ossl_rsa_mp_factor_names)
            || !collect_numbers(exps = sk_BIGNUM_new_null(), params, ossl_rsa_mp_exp_names)
            || !collect_numbers(coeffs = sk_BIGNUM_new_null(), params, ossl_rsa_mp_coeff_names))
            goto err;

        /* It's ok if this private key just has n, e and d */
        if (sk_BIGNUM_num(factors) != 0 && !ossl_rsa_set0_all_params(rsa, factors, exps, coeffs))
            goto err;
    }
...
int ossl_rsa_todata(RSA* rsa, OSSL_PARAM_BLD* bld, OSSL_PARAM params[], int include_private)
{
    ...
    if (!ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_N, rsa_n) || !ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_E, rsa_e))
        goto err;

    /* Check private key data integrity */
    if (include_private && rsa_d != NULL) {

        if (!ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_D, rsa_d)
            || !ossl_param_build_set_multi_key_bn(bld, params, ossl_rsa_mp_factor_names, factors)
            || !ossl_param_build_set_multi_key_bn(bld, params, ossl_rsa_mp_exp_names, exps)
            || !ossl_param_build_set_multi_key_bn(bld, params, ossl_rsa_mp_coeff_names, coeffs))
            goto err;
    }
...
int ossl_rsa_pss_params_30_todata(const RSA_PSS_PARAMS_30* pss, OSSL_PARAM_BLD* bld, OSSL_PARAM params[])
{
    if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
        ...
        const char* mdname = (hashalg_nid == default_hashalg_nid ? NULL : ossl_rsa_oaeppss_nid2name(hashalg_nid));
        const char* mgfname = (maskgenalg_nid == default_maskgenalg_nid ? NULL : ossl_rsa_oaeppss_nid2name(maskgenalg_nid));
        const char* mgf1mdname = (maskgenhashalg_nid == default_maskgenhashalg_nid ? NULL : ossl_rsa_oaeppss_nid2name(maskgenhashalg_nid));
         ...
         */
        if ((mdname != NULL && !ossl_param_build_set_utf8_string(bld, params, key_md, mdname))
            || (mgfname != NULL && !ossl_param_build_set_utf8_string(bld, params, key_mgf, mgfname))
            || (mgf1mdname != NULL && !ossl_param_build_set_utf8_string(bld, params, key_mgf1_md, mgf1mdname))
...
int ossl_rsa_pss_params_30_fromdata(RSA_PSS_PARAMS_30* pss_params, int* defaults_set, const OSSL_PARAM params[], OSSL_LIB_CTX* libctx)
{
     ...
     */
    if (!*defaults_set && (param_md != NULL || param_mgf != NULL || param_mgf1md != NULL || param_saltlen != NULL)) {
    ...
    if (param_mgf != NULL) {
        ...
        if (OPENSSL_strcasecmp(param_mgf->data, ossl_rsa_mgf_nid2name(default_maskgenalg_nid)) != 0)
            return 0;
    }
    ...
    if (param_md != NULL) {
        ...
        if ((md = EVP_MD_fetch(libctx, mdname, propq)) == NULL || !ossl_rsa_pss_params_30_set_hashalg(pss_params, ossl_rsa_oaeppss_md2nid(md)))
            goto err;
    }
    ...
    if (param_mgf1md != NULL) {
        ...
        if ((mgf1md = EVP_MD_fetch(libctx, mgf1mdname, propq)) == NULL
            || !ossl_rsa_pss_params_30_set_maskgenhashalg(pss_params, ossl_rsa_oaeppss_md2nid(mgf1md)))
            goto err;
    }

    if (param_saltlen != NULL) {
        if (!OSSL_PARAM_get_int(param_saltlen, &saltlen) || !ossl_rsa_pss_params_30_set_saltlen(pss_params, saltlen))
            goto err;
    }
...
RSA* ossl_rsa_dup(const RSA* rsa, int selection)
{
...
#ifndef FIPS_MODULE
    /* multiprime */
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 && (pnum = sk_RSA_PRIME_INFO_num(rsa->prime_infos)) > 0) {
    ...
    if (rsa->pss != NULL) {
        ...
        if (rsa->pss->maskGenAlgorithm != NULL && dupkey->pss->maskGenAlgorithm == NULL) {
            dupkey->pss->maskHash = ossl_x509_algor_mgf1_decode(rsa->pss->maskGenAlgorithm);
            if (dupkey->pss->maskHash == NULL)
                goto err;
        }
    }
    if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_RSA, &dupkey->ex_data, &rsa->ex_data))
        goto err;
#endif
...
static int ossl_rsa_sync_to_pss_params_30(RSA* rsa)
{
    ...
    if (rsa != NULL && (legacy_pss = RSA_get0_pss_params(rsa)) != NULL && (pss = ossl_rsa_get0_pss_params_30(rsa)) != NULL) {
         ...
         */
        if (!ossl_rsa_pss_get_param_unverified(legacy_pss, &md, &mgf1md, &saltlen, &trailerField))
            return 0;
        ...
        if (!ossl_rsa_pss_params_30_set_defaults(&pss_params) || !ossl_rsa_pss_params_30_set_hashalg(&pss_params, md_nid)
            ...
            || !ossl_rsa_pss_params_30_set_trailerfield(&pss_params, trailerField))
            return 0;
        *pss = pss_params;
    }
    return 1;
...
int ossl_rsa_param_decode(RSA* rsa, const X509_ALGOR* alg)
{
    ...
    if ((pss = ossl_rsa_pss_decode(alg)) == NULL || !ossl_rsa_set0_pss_params(rsa, pss)) {
        RSA_PSS_PARAMS_free(pss);
        return 0;
    }
...
RSA* ossl_rsa_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf, OSSL_LIB_CTX* libctx, const char* propq)
{
