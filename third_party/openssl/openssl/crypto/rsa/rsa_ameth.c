...
static int rsa_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey)
{
    ...
    if (X509_PUBKEY_set0_param(pk, OBJ_nid2obj(pkey->ameth->pkey_id), strtype, str, penc, penclen))
        return 1;
...
static int rsa_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b)
{
     ...
     * cards.
     */
    if (((RSA_flags(a->pkey.rsa) & RSA_METHOD_FLAG_NO_CHECK)) || (RSA_flags(b->pkey.rsa) & RSA_METHOD_FLAG_NO_CHECK)) {
        return 1;
    }
...
static int rsa_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
    ...
    if (!PKCS8_pkey_set0(p8, OBJ_nid2obj(pkey->ameth->pkey_id), 0, strtype, str, rk, rklen)) {
        ERR_raise(ERR_LIB_RSA, ERR_R_MALLOC_FAILURE);
        ASN1_STRING_free(str);
...
static int rsa_pkey_ctrl(EVP_PKEY* pkey, int op, long arg1, void* arg2)
{
    ...
    case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
        if (pkey->pkey.rsa->pss != NULL) {
            if (!ossl_rsa_pss_get_param(pkey->pkey.rsa->pss, &md, &mgf1md, &min_saltlen)) {
...
RSA_PSS_PARAMS* ossl_rsa_pss_params_create(const EVP_MD* sigmd, const EVP_MD* mgf1md, int saltlen)
{
...
int ossl_rsa_pss_to_ctx(EVP_MD_CTX* ctx, EVP_PKEY_CTX* pkctx, const X509_ALGOR* sigalg, EVP_PKEY* pkey)
{
    int rv = -1;
    int saltlen;
...
static int rsa_pss_verify_param(const EVP_MD** pmd, const EVP_MD** pmgf1md, int* psaltlen, int* ptrailerField)
{
...
int ossl_rsa_pss_get_param(const RSA_PSS_PARAMS* pss, const EVP_MD** pmd, const EVP_MD** pmgf1md, int* psaltlen)
{
    /*
    ...
    return ossl_rsa_pss_get_param_unverified(pss, pmd, pmgf1md, psaltlen, &trailerField) && rsa_pss_verify_param(pmd, pmgf1md, psaltlen, &trailerField);
}
...
static int rsa_item_verify(EVP_MD_CTX* ctx, const ASN1_ITEM* it, const void* asn, const X509_ALGOR* sigalg, const ASN1_BIT_STRING* sig, EVP_PKEY* pkey)
{
    /* Sanity check: make sure it is PSS */
    if (OBJ_obj2nid(sigalg->algorithm) != EVP_PKEY_RSA_PSS) {
        ERR_raise(ERR_LIB_RSA, RSA_R_UNSUPPORTED_SIGNATURE_TYPE);
...
static int rsa_item_sign(EVP_MD_CTX* ctx, const ASN1_ITEM* it, const void* asn, X509_ALGOR* alg1, X509_ALGOR* alg2, ASN1_BIT_STRING* sig)
{
    int pad_mode;
    ...
    if (pad_mode == RSA_PKCS1_PSS_PADDING) {
        ...
        if (evp_pkey_ctx_is_legacy(pkctx)) {
            ...
            if (alg2 != NULL) {
                ...
                if (!X509_ALGOR_set0(alg2, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE, os2)) {
                    ASN1_STRING_free(os1);
                    ASN1_STRING_free(os2);
                    return 0;
                }
            }
            if (!X509_ALGOR_set0(alg1, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE, os1)) {
                ASN1_STRING_free(os1);
                return 0;
            }
            return 3;
...
static int rsa_sig_info_set(X509_SIG_INFO* siginf, const X509_ALGOR* sigalg, const ASN1_STRING* sig)
{
     ...
     */
    if ((mdnid == NID_sha256 || mdnid == NID_sha384 || mdnid == NID_sha512) && mdnid == EVP_MD_get_type(mgf1md) && saltlen == EVP_MD_get_size(md))
 ...
 */
static int rsa_int_export_to(
    const EVP_PKEY* from, int rsa_type, void* to_keydata, OSSL_FUNC_keymgmt_import_fn* importer, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (rsa->pss != NULL) {
        ...
        if (!ossl_rsa_pss_get_param_unverified(rsa->pss, &md, &mgf1md, &saltlen, &trailerfield))
            goto err;
        md_nid = EVP_MD_get_type(md);
        mgf1md_nid = EVP_MD_get_type(mgf1md);
        if (!ossl_rsa_pss_params_30_set_defaults(&pss_params) || !ossl_rsa_pss_params_30_set_hashalg(&pss_params, md_nid)
            || !ossl_rsa_pss_params_30_set_maskgenhashalg(&pss_params, mgf1md_nid) || !ossl_rsa_pss_params_30_set_saltlen(&pss_params, saltlen)
...
static int rsa_int_import_from(const OSSL_PARAM params[], void* vpctx, int rsa_type)
{
    ...
    if (!ossl_rsa_pss_params_30_fromdata(&rsa_pss_params, &pss_defaults_set, params, pctx->libctx))
        goto err;
    ...
    case RSA_FLAG_TYPE_RSASSAPSS:
        ...
        if (!ossl_rsa_pss_params_30_is_unrestricted(&rsa_pss_params)) {
            ...
            if ((rsa->pss = ossl_rsa_pss_params_create(md, mgf1md, saltlen)) == NULL)
                goto err;
        }
        break;
    default:
...
static int rsa_pkey_export_to(const EVP_PKEY* from, void* to_keydata, OSSL_FUNC_keymgmt_import_fn* importer, OSSL_LIB_CTX* libctx, const char* propq)
{
    return rsa_int_export_to(from, RSA_FLAG_TYPE_RSA, to_keydata, importer, libctx, propq);
}

static int rsa_pss_pkey_export_to(const EVP_PKEY* from, void* to_keydata, OSSL_FUNC_keymgmt_import_fn* importer, OSSL_LIB_CTX* libctx, const char* propq)
{
    return rsa_int_export_to(from, RSA_FLAG_TYPE_RSASSAPSS, to_keydata, importer, libctx, propq);
}
...
const EVP_PKEY_ASN1_METHOD ossl_rsa_asn1_meths[2] = { { EVP_PKEY_RSA, EVP_PKEY_RSA, ASN1_PKEY_SIGPARAM_NULL,

                                                          "RSA", "OpenSSL RSA method",

                                                          rsa_pub_decode, rsa_pub_encode, rsa_pub_cmp, rsa_pub_print,

                                                          rsa_priv_decode, rsa_priv_encode, rsa_priv_print,

                                                          int_rsa_size, rsa_bits, rsa_security_bits,

                                                          0, 0, 0, 0, 0, 0,

                                                          rsa_sig_print, int_rsa_free, rsa_pkey_ctrl, old_rsa_priv_decode, old_rsa_priv_encode, rsa_item_verify,
                                                          rsa_item_sign, rsa_sig_info_set, rsa_pkey_check,

                                                          0, 0, 0, 0, 0, 0,

                                                          rsa_pkey_dirty_cnt, rsa_pkey_export_to, rsa_pkey_import_from, rsa_pkey_copy },

    { EVP_PKEY_RSA2, EVP_PKEY_RSA, ASN1_PKEY_ALIAS } };

const EVP_PKEY_ASN1_METHOD ossl_rsa_pss_asn1_meth = { EVP_PKEY_RSA_PSS, EVP_PKEY_RSA_PSS, ASN1_PKEY_SIGPARAM_NULL,

    "RSA-PSS", "OpenSSL RSA-PSS method",
    ...
    rsa_sig_print, int_rsa_free, rsa_pkey_ctrl, 0, 0, rsa_item_verify, rsa_item_sign, rsa_sig_info_set, rsa_pkey_check,

    0, 0, 0, 0, 0, 0,

    rsa_pkey_dirty_cnt, rsa_pss_pkey_export_to, rsa_pss_pkey_import_from, rsa_pkey_copy };
