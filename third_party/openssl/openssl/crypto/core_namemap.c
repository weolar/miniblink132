 ...
 */
int ossl_namemap_doall_names(const OSSL_NAMEMAP* namemap, int number, void (*fn)(const char* name, void* data), void* data)
{
...
static int namemap_name2num_n(const OSSL_NAMEMAP* namemap, const char* name, size_t name_len)
{
...
static int namemap_add_name_n(OSSL_NAMEMAP* namemap, int number, const char* name, size_t name_len)
{
    ...
    if ((namenum = OPENSSL_zalloc(sizeof(*namenum))) == NULL || (namenum->name = OPENSSL_strndup(name, name_len)) == NULL)
        goto err;
...
int ossl_namemap_add_name_n(OSSL_NAMEMAP* namemap, int number, const char* name, size_t name_len)
{
    int tmp_number;
...
int ossl_namemap_add_names(OSSL_NAMEMAP* namemap, int number, const char* names, const char separator)
{
    ...
    for (p = names; *p != '\0'; p = (q == NULL ? p + l : q + 1)) {
        ...
        if (number == 0) {
            ...
            ERR_raise_data(
                ERR_LIB_CRYPTO, CRYPTO_R_CONFLICTING_NAMES, "\"%.*s\" has an existing different identity %d (from \"%s\")", l, p, this_number, names);
            goto err;
        }
    }
    ...
    for (p = names; *p != '\0'; p = (q == NULL ? p + l : q + 1)) {
        ...
        if (number == 0) {
            ...
            ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "Got number %d when expecting %d", this_number, number);
            goto err;
        }
    }
...
OSSL_NAMEMAP* ossl_namemap_stored(OSSL_LIB_CTX* libctx)
{
    ...
    int nms;
#endif
    OSSL_NAMEMAP* namemap = ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_NAMEMAP_INDEX, &stored_namemap_method);
    ...
    if (nms == 1) {
        ...
        OBJ_NAME_do_all(OBJ_NAME_TYPE_CIPHER_METH, get_legacy_cipher_names, namemap);
        OBJ_NAME_do_all(OBJ_NAME_TYPE_MD_METH, get_legacy_md_names, namemap);
...
OSSL_NAMEMAP* ossl_namemap_new(void)
{
    ...
    if ((namemap = OPENSSL_zalloc(sizeof(*namemap))) != NULL && (namemap->lock = CRYPTO_THREAD_lock_new()) != NULL
