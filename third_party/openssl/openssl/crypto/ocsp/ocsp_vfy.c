...
static int ocsp_verify_signer(X509* signer, int response, X509_STORE* st, unsigned long flags, STACK_OF(X509) * untrusted, STACK_OF(X509) * *chain)
{
    ...
    if (ret <= 0) {
        ...
        ERR_raise_data(ERR_LIB_OCSP, OCSP_R_CERTIFICATE_VERIFY_ERROR, "Verify error: %s", X509_verify_cert_error_string(err));
        goto end;
    }
    if (chain != NULL)
...
int OCSP_request_verify(OCSP_REQUEST* req, STACK_OF(X509) * certs, X509_STORE* store, unsigned long flags)
{
    ...
    if ((flags & OCSP_NOVERIFY) != 0)
        return 1;
    return ocsp_verify_signer(signer, 0, store, flags, (flags & OCSP_NOCHAIN) != 0 ? NULL : req->optionalSignature->certs, NULL) > 0;
