...
static EVP_PKEY* pem_read_bio_key_decoder(BIO* bp, EVP_PKEY** x, pem_password_cb* cb, void* u, OSSL_LIB_CTX* libctx, const char* propq, int selection)
{
    EVP_PKEY* pkey = NULL;
    ...
    dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "PEM", NULL, NULL, selection, libctx, propq);
...
static EVP_PKEY* pem_read_bio_key_legacy(BIO* bp, EVP_PKEY** x, pem_password_cb* cb, void* u, OSSL_LIB_CTX* libctx, const char* propq, int selection)
{
    char* nm = NULL;
    ...
    } else if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == 0 && (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
        /* Trying legacy PUBKEY decoding only if we do not want private key. */
        ret = ossl_d2i_PUBKEY_legacy(x, &p, len);
    } else if ((selection & EVP_PKEY_KEYPAIR) == 0 && (slen = ossl_pem_check_suffix(nm, "PARAMETERS")) > 0) {
...
static EVP_PKEY* pem_read_bio_key(BIO* bp, EVP_PKEY** x, pem_password_cb* cb, void* u, OSSL_LIB_CTX* libctx, const char* propq, int selection)
{
    EVP_PKEY* ret = NULL;
    BIO* new_bio = NULL;
    int pos;
    ...
    if (!ossl_pw_set_pem_password_cb(&pwdata, cb, u) || !ossl_pw_enable_passphrase_caching(&pwdata))
        goto err;

    ERR_set_mark();
    ret = pem_read_bio_key_decoder(bp, x, ossl_pw_pem_password, &pwdata, libctx, propq, selection);
    if (ret == NULL && (BIO_seek(bp, pos) < 0 || (ret = pem_read_bio_key_legacy(bp, x, ossl_pw_pem_password, &pwdata, libctx, propq, selection)) == NULL))
        ERR_clear_last_mark();
    else
        ERR_pop_to_mark();
...
int PEM_write_bio_PrivateKey_traditional(BIO* bp, const EVP_PKEY* x, const EVP_CIPHER* enc, const unsigned char* kstr, int klen, pem_password_cb* cb, void* u)
{
    ...
    if (evp_pkey_is_assigned(x) && evp_pkey_is_provided(x) && evp_pkey_copy_downgraded(&copy, x))
        x = copy;
    ...
    ret = PEM_ASN1_write_bio((i2d_of_void*)i2d_PrivateKey, pem_str, bp, x, enc, kstr, klen, cb, u);
...
EVP_PKEY* PEM_read_bio_Parameters_ex(BIO* bp, EVP_PKEY** x, OSSL_LIB_CTX* libctx, const char* propq)
{
     ...
     */
    return pem_read_bio_key(bp, x, no_password_cb, NULL, libctx, propq, EVP_PKEY_KEY_PARAMETERS);
}
...
legacy:
    ...
    return PEM_ASN1_write_bio((i2d_of_void*)x->ameth->param_encode, pem_str, out, x, NULL, NULL, 0, 0, NULL);
}
...
PEM_write_cb_ex_fnsig(PrivateKey, EVP_PKEY, FILE, write)
{
    ...
    ret = PEM_write_bio_PrivateKey_ex(b, x, enc, kstr, klen, cb, u, libctx, propq);
    BIO_free(b);
    return ret;
}
