 ...
 *      header data.
 */
int ossl_do_blob_header(const unsigned char** in, unsigned int length, unsigned int* pmagic, unsigned int* pbitlen, int* pisdss, int* pispub)
{
    const unsigned char* p = *in;
...
static int do_i2b(unsigned char** out, const EVP_PKEY* pk, int ispub)
{
    ...
    outlen = 16 + ossl_blob_length(bitlen, keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0, ispub);
    if (out == NULL)
        goto end;
    if (*out)
        p = *out;
...
static int check_bitlen_rsa(const RSA* rsa, int ispub, unsigned int* pmagic)
{
    ...
    if (ispub) {
        ...
        *pmagic = MS_RSA2MAGIC;
        ...
        if ((BN_num_bytes(iqmp) > hnbyte) || (BN_num_bytes(p) > hnbyte) || (BN_num_bytes(q) > hnbyte) || (BN_num_bytes(dmp1) > hnbyte)
...
int ossl_do_PVK_header(const unsigned char** in, unsigned int length, int skip_magic, unsigned int* psaltlen, unsigned int* pkeylen)
{
    const unsigned char* p = *in;
...
static int derive_pvk_key(
    unsigned char* key, const unsigned char* salt, unsigned int saltlen, const unsigned char* pass, int passlen, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (mctx == NULL || !EVP_DigestInit_ex(mctx, sha1, NULL) || !EVP_DigestUpdate(mctx, salt, saltlen) || !EVP_DigestUpdate(mctx, pass, passlen)
...
static void* do_PVK_body_key(const unsigned char** in, unsigned int saltlen, unsigned int keylen, pem_password_cb* cb, void* u, int* isdss, int* ispub,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (saltlen) {
        ...
        if (!derive_pvk_key(keybuf, p, saltlen, (unsigned char*)psbuf, inlen, libctx, propq))
            goto err;
        p += saltlen;
...
static void* do_PVK_key_bio(BIO* in, pem_password_cb* cb, void* u, int* isdss, int* ispub, OSSL_LIB_CTX* libctx, const char* propq)
{
...
static int i2b_PVK(unsigned char** out, const EVP_PKEY* pk, int enclevel, pem_password_cb* cb, void* u, OSSL_LIB_CTX* libctx, const char* propq)
{
    int ret = -1;
    int outlen = 24, pklen;
    ...
    if (enclevel != 0) {
        ...
        if (!derive_pvk_key(keybuf, salt, PVK_SALTLEN, (unsigned char*)psbuf, inlen, libctx, propq))
            goto error;
        if ((rc4 = EVP_CIPHER_fetch(libctx, "RC4", propq)) == NULL)
            goto error;
        if (enclevel == 1)
