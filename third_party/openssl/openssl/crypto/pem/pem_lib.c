...
static int pem_bytes_read_bio_flags(unsigned char** pdata, long* plen, char** pnm, const char* name, BIO* bp, pem_password_cb* cb, void* u, unsigned int flags)
{
...
{
    return pem_bytes_read_bio_flags(pdata, plen, pnm, name, bp, cb, u, PEM_FLAG_EAY_COMPATIBLE);
}

int PEM_bytes_read_bio_secmem(unsigned char** pdata, long* plen, char** pnm, const char* name, BIO* bp, pem_password_cb* cb, void* u)
{
    return pem_bytes_read_bio_flags(pdata, plen, pnm, name, bp, cb, u, PEM_FLAG_SECURE | PEM_FLAG_EAY_COMPATIBLE);
}

#ifndef OPENSSL_NO_STDIO
int PEM_ASN1_write(
    i2d_of_void* i2d, const char* name, FILE* fp, const void* x, const EVP_CIPHER* enc, const unsigned char* kstr, int klen, pem_password_cb* callback, void* u)
{
    BIO* b;
    int ret;
...
int PEM_ASN1_write_bio(
    i2d_of_void* i2d, const char* name, BIO* bp, const void* x, const EVP_CIPHER* enc, const unsigned char* kstr, int klen, pem_password_cb* callback, void* u)
{
    ...
    if (enc != NULL) {
        ...
        ret = 1;
        if ((ctx = EVP_CIPHER_CTX_new()) == NULL || !EVP_EncryptInit_ex(ctx, enc, NULL, key, iv) || !EVP_EncryptUpdate(ctx, data, &j, data, i)
...
int PEM_do_header(EVP_CIPHER_INFO* cipher, unsigned char* data, long* plen, pem_password_cb* callback, void* u)
{
    ...
    if (!EVP_BytesToKey(cipher->cipher, EVP_md5(), &(cipher->iv[0]), (unsigned char*)buf, keylen, 1, key, NULL))
        return 0;
...
static int get_name(BIO* bp, char** name, unsigned int flags)
{
    ...
    } while (strncmp(linebuf, beginstr, BEGINLEN) != 0 || len < TAILLEN || strncmp(linebuf + len - TAILLEN, tailstr, TAILLEN) != 0);
...
enum header_status { MAYBE_HEADER, IN_HEADER, POST_HEADER };
