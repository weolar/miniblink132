...
static int t_fromb64(unsigned char* a, size_t alen, const char* src)
{
    ...
    if (padsize != 0 && EVP_DecodeUpdate(ctx, a, &outl, pad, padsize) < 0) {
        outl = -1;
        goto err;
    }
...
static int t_tob64(char* dst, const unsigned char* src, int size)
{
     ...
     */
    leadz = 3 - (size % 3);
    if (leadz != 3 && !EVP_EncodeUpdate(ctx, (unsigned char*)dst, &outl, pad, leadz)) {
        EVP_ENCODE_CTX_free(ctx);
        return 0;
    }

    if (!EVP_EncodeUpdate(ctx, (unsigned char*)dst + outl, &outl2, src, size)) {
        EVP_ENCODE_CTX_free(ctx);
        return 0;
    }
    outl += outl2;
...
SRP_user_pwd* SRP_VBASE_get1_by_user(SRP_VBASE* vb, char* username)
{
    ...
    if (md == NULL)
        goto err;
    ctxt = EVP_MD_CTX_new();
    if (ctxt == NULL || !EVP_DigestInit_ex(ctxt, md, NULL) || !EVP_DigestUpdate(ctxt, vb->seed_key, strlen(vb->seed_key))
        || !EVP_DigestUpdate(ctxt, username, strlen(username)) || !EVP_DigestFinal_ex(ctxt, digs, NULL))
        goto err;
    EVP_MD_CTX_free(ctxt);
    ctxt = NULL;
    EVP_MD_free(md);
    md = NULL;
    if (SRP_user_pwd_set0_sv(user, BN_bin2bn(digs, SHA_DIGEST_LENGTH, NULL), BN_bin2bn(digv, SHA_DIGEST_LENGTH, NULL)))
        return user;
...
char* SRP_create_verifier_ex(
    const char* user, const char* pass, char** salt, char** verifier, const char* N, const char* g, OSSL_LIB_CTX* libctx, const char* propq)
{
    int len;
    ...
    if (!SRP_create_verifier_BN_ex(user, pass, &s, &v, N_bn, g_bn, libctx, propq))
        goto err;
 ...
 * BIGNUMS.
 */
int SRP_create_verifier_BN_ex(
    const char* user, const char* pass, BIGNUM** salt, BIGNUM** verifier, const BIGNUM* N, const BIGNUM* g, OSSL_LIB_CTX* libctx, const char* propq)
{
    int result = 0;
    BIGNUM* x = NULL;
...
{
    return SRP_create_verifier_BN_ex(user, pass, salt, verifier, N, g, NULL, NULL);
}
#endif
