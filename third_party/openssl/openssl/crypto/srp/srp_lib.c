...
static BIGNUM* srp_Calc_xy(const BIGNUM* x, const BIGNUM* y, const BIGNUM* N, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if ((tmp = OPENSSL_malloc(numN * 2)) == NULL)
        goto err;
    if (BN_bn2binpad(x, tmp, numN) < 0 || BN_bn2binpad(y, tmp + numN, numN) < 0 || !EVP_Digest(tmp, numN * 2, digest, NULL, sha1, NULL))
        goto err;
...
static BIGNUM* srp_Calc_k(const BIGNUM* N, const BIGNUM* g, OSSL_LIB_CTX* libctx, const char* propq)
{
...
BIGNUM* SRP_Calc_B_ex(const BIGNUM* b, const BIGNUM* N, const BIGNUM* g, const BIGNUM* v, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (!BN_mod_exp(gb, g, b, N, bn_ctx) || (k = srp_Calc_k(N, g, libctx, propq)) == NULL || !BN_mod_mul(kv, v, k, N, bn_ctx)
...
BIGNUM* SRP_Calc_x_ex(const BIGNUM* s, const char* user, const char* pass, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (!EVP_DigestInit_ex(ctxt, sha1, NULL) || !EVP_DigestUpdate(ctxt, user, strlen(user)) || !EVP_DigestUpdate(ctxt, ":", 1)
        || !EVP_DigestUpdate(ctxt, pass, strlen(pass)) || !EVP_DigestFinal_ex(ctxt, dig, NULL) || !EVP_DigestInit_ex(ctxt, sha1, NULL))
        goto err;
    if (BN_bn2bin(s, cs) < 0)
        goto err;
...
BIGNUM* SRP_Calc_client_key_ex(
    const BIGNUM* N, const BIGNUM* B, const BIGNUM* g, const BIGNUM* x, const BIGNUM* a, const BIGNUM* u, OSSL_LIB_CTX* libctx, const char* propq)
{
    BIGNUM *tmp = NULL, *tmp2 = NULL, *tmp3 = NULL, *k = NULL, *K = NULL;
    BIGNUM* xtmp = NULL;
    BN_CTX* bn_ctx;

    if (u == NULL || B == NULL || N == NULL || g == NULL || x == NULL || a == NULL || (bn_ctx = BN_CTX_new_ex(libctx)) == NULL)
        return NULL;
