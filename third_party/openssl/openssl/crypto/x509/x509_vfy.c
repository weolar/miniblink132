...
#define CRL_SCORE_VALID /* If this score or above CRL is probably valid */ (CRL_SCORE_NOCRITICAL | CRL_SCORE_TIME | CRL_SCORE_SCOPE)
...
static int check_auth_level(X509_STORE_CTX* ctx)
{
    ...
    for (i = 0; i < num; ++i) {
         ...
         */
        CB_FAIL_IF(i < num - 1 && !check_sig_level(ctx, cert), ctx, cert, i, X509_V_ERR_CA_MD_TOO_WEAK);
    }
    return 1;
}
...
static int verify_chain(X509_STORE_CTX* ctx)
{
    ...
    if ((ok = build_chain(ctx)) <= 0 || (ok = check_extensions(ctx)) <= 0 || (ok = check_auth_level(ctx)) <= 0 || (ok = check_id(ctx)) <= 0
        || (ok = X509_get_pubkey_parameters(NULL, ctx->chain) ? 1 : -1) <= 0 || (ok = ctx->check_revocation(ctx)) <= 0)
        return ok;
...
int X509_verify_cert(X509_STORE_CTX* ctx)
{
    ...
    CB_FAIL_IF(!check_key_level(ctx, ctx->cert), ctx, ctx->cert, 0, X509_V_ERR_EE_KEY_TOO_SMALL);
...
static X509* find_issuer(X509_STORE_CTX* ctx, STACK_OF(X509) * sk, X509* x)
{
    ...
    for (i = 0; i < sk_X509_num(sk); i++) {
        ...
        if (ctx->check_issued(ctx, x, issuer) && (((x->ex_flags & EXFLAG_SI) != 0 && sk_X509_num(ctx->chain) == 1) || !sk_X509_contains(ctx->chain, issuer))) {
 ...
 */
static int check_purpose(X509_STORE_CTX* ctx, X509* x, int purpose, int depth, int must_be_ca)
{
...
static int check_extensions(X509_STORE_CTX* ctx)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        CB_FAIL_IF(
            (ctx->param->flags & X509_V_FLAG_IGNORE_CRITICAL) == 0 && (x->ex_flags & EXFLAG_CRITICAL) != 0, ctx, x, i, X509_V_ERR_UNHANDLED_CRITICAL_EXTENSION);
        CB_FAIL_IF(!allow_proxy_certs && (x->ex_flags & EXFLAG_PROXY) != 0, ctx, x, i, X509_V_ERR_PROXY_CERTIFICATES_NOT_ALLOWED);
        ret = X509_check_ca(x);
        switch (must_be_ca) {
        case -1:
        ...
        default:
            ...
            CB_FAIL_IF(ret == 0 || ((i + 1 < num || (ctx->param->flags & X509_V_FLAG_X509_STRICT) != 0) && ret != 1), ctx, x, i, X509_V_ERR_INVALID_CA);
            break;
        }
        if (num > 1) {
        ...
        if ((ctx->param->flags & X509_V_FLAG_X509_STRICT) != 0 && num > 1) { /*
            ...
            if (x->ex_pathlen != -1) {
                CB_FAIL_IF((x->ex_flags & EXFLAG_CA) == 0, ctx, x, i, X509_V_ERR_PATHLEN_INVALID_FOR_NON_CA);
                CB_FAIL_IF((x->ex_kusage & KU_KEY_CERT_SIGN) == 0, ctx, x, i, X509_V_ERR_PATHLEN_WITHOUT_KU_KEY_CERT_SIGN);
            }
            CB_FAIL_IF((x->ex_flags & EXFLAG_CA) != 0 && (x->ex_flags & EXFLAG_BCONS) != 0 && (x->ex_flags & EXFLAG_BCONS_CRITICAL) == 0, ctx, x, i,
                X509_V_ERR_CA_BCONS_NOT_CRITICAL);
            /* Check Key Usage according to RFC 5280 section 4.2.1.3 */
            if ((x->ex_flags & EXFLAG_CA) != 0) {
                CB_FAIL_IF((x->ex_flags & EXFLAG_KUSAGE) == 0, ctx, x, i, X509_V_ERR_CA_CERT_MISSING_KEY_USAGE);
            } else {
                CB_FAIL_IF((x->ex_kusage & KU_KEY_CERT_SIGN) != 0, ctx, x, i, X509_V_ERR_KU_KEY_CERT_SIGN_INVALID_FOR_NON_CA);
            }
            /* Check issuer is non-empty acc. to RFC 5280 section 4.1.2.4 */
            CB_FAIL_IF(X509_NAME_entry_count(X509_get_issuer_name(x)) == 0, ctx, x, i, X509_V_ERR_ISSUER_NAME_EMPTY);
            /* Check subject is non-empty acc. to RFC 5280 section 4.1.2.6 */
            CB_FAIL_IF(((x->ex_flags & EXFLAG_CA) != 0 || (x->ex_kusage & KU_CRL_SIGN) != 0 || x->altname == NULL)
                    && X509_NAME_entry_count(X509_get_subject_name(x)) == 0,
                ctx, x, i, X509_V_ERR_SUBJECT_NAME_EMPTY);
            CB_FAIL_IF(X509_NAME_entry_count(X509_get_subject_name(x)) == 0 && x->altname != NULL && (x->ex_flags & EXFLAG_SAN_CRITICAL) == 0, ctx, x, i,
                X509_V_ERR_EMPTY_SUBJECT_SAN_NOT_CRITICAL);
            /* Check SAN is non-empty according to RFC 5280 section 4.2.1.6 */
            CB_FAIL_IF(x->altname != NULL && sk_GENERAL_NAME_num(x->altname) <= 0, ctx, x, i, X509_V_ERR_EMPTY_SUBJECT_ALT_NAME);
            /* Check sig alg consistency acc. to RFC 5280 section 4.1.1.2 */
            CB_FAIL_IF(X509_ALGOR_cmp(&x->sig_alg, &x->cert_info.signature) != 0, ctx, x, i, X509_V_ERR_SIGNATURE_ALGORITHM_INCONSISTENCY);
            CB_FAIL_IF(x->akid != NULL && (x->ex_flags & EXFLAG_AKID_CRITICAL) != 0, ctx, x, i, X509_V_ERR_AUTHORITY_KEY_IDENTIFIER_CRITICAL);
            CB_FAIL_IF(x->skid != NULL && (x->ex_flags & EXFLAG_SKID_CRITICAL) != 0, ctx, x, i, X509_V_ERR_SUBJECT_KEY_IDENTIFIER_CRITICAL);
            if (X509_get_version(x) >= X509_VERSION_3) {
                ...
                CB_FAIL_IF((x->ex_flags & EXFLAG_CA) != 0 && x->skid == NULL, ctx, x, i, X509_V_ERR_MISSING_SUBJECT_KEY_IDENTIFIER);
            } else {
                CB_FAIL_IF(sk_X509_EXTENSION_num(X509_get0_extensions(x)) > 0, ctx, x, i, X509_V_ERR_EXTENSIONS_REQUIRE_VERSION_3);
            }
        }
        ...
        if (purpose > 0 && !check_purpose(ctx, x, purpose, i, must_be_ca))
            return 0;
        /* Check path length */
        CB_FAIL_IF(i > 1 && x->ex_pathlen != -1 && plen > x->ex_pathlen + proxy_path_length, ctx, x, i, X509_V_ERR_PATH_LENGTH_EXCEEDED);
        ...
        if (x->ex_flags & EXFLAG_PROXY) {
             ...
             */
            if (x->ex_pcpathlen != -1) {
                CB_FAIL_IF(proxy_path_length > x->ex_pcpathlen, ctx, x, i, X509_V_ERR_PROXY_PATH_LENGTH_EXCEEDED);
...
static int check_name_constraints(X509_STORE_CTX* ctx)
{
    ...
    for (i = sk_X509_num(ctx->chain) - 1; i >= 0; i--) {
        ...
        if ((x->ex_flags & EXFLAG_PROXY) != 0) {
             ...
             * multi-valued RDN
             */
            if (X509_NAME_ENTRY_set(X509_NAME_get_entry(tmpsubject, last_loc)) == X509_NAME_ENTRY_set(X509_NAME_get_entry(tmpsubject, last_loc - 1))) {
                err = X509_V_ERR_PROXY_SUBJECT_NAME_VIOLATION;
                goto proxy_name_done;
            }
        ...
        for (j = sk_X509_num(ctx->chain) - 1; j > i; j--) {
            ...
            if (nc) {
                ...
                if (rv == X509_V_OK && i == 0 && (ctx->param->hostflags & X509_CHECK_FLAG_NEVER_CHECK_SUBJECT) == 0
                    && ((ctx->param->hostflags & X509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT) != 0 || (ret = has_san_id(x, GEN_DNS)) == 0))
...
rejected:
    return verify_cb_cert(ctx, x, i, X509_V_ERR_CERT_REJECTED) == 0 ? X509_TRUST_REJECTED : X509_TRUST_UNTRUSTED;
...
static int get_crl_sk(X509_STORE_CTX* ctx, X509_CRL** pcrl, X509_CRL** pdcrl, X509** pissuer, int* pscore, unsigned int* preasons, STACK_OF(X509_CRL) * crls)
{
    ...
    for (i = 0; i < sk_X509_CRL_num(crls); i++) {
        ...
        if (crl_score == best_score && best_crl != NULL) {
            ...
            if (ASN1_TIME_diff(&day, &sec, X509_CRL_get0_lastUpdate(best_crl), X509_CRL_get0_lastUpdate(crl)) == 0)
                continue;
            /*
...
static int crl_crldp_check(X509* x, X509_CRL* crl, int crl_score, unsigned int* preasons)
{
    ...
    *preasons = crl->idp_reasons;
    ...
    return (crl->idp == NULL || crl->idp->distpoint == NULL) && (crl_score & CRL_SCORE_ISSUER_NAME) != 0;
}
...
static int check_crl(X509_STORE_CTX* ctx, X509_CRL* crl)
{
    ...
    if (crl->base_crl_number == NULL) {
        ...
        if ((ctx->current_crl_score & CRL_SCORE_SAME_PATH) == 0 && check_crl_path(ctx, ctx->current_issuer) <= 0
...
static int cert_crl(X509_STORE_CTX* ctx, X509_CRL* crl, X509* x)
{
     ...
     */
    if ((ctx->param->flags & X509_V_FLAG_IGNORE_CRITICAL) == 0 && (crl->flags & EXFLAG_CRITICAL) != 0
    ...
    if (ret == X509_PCY_TREE_INVALID) {
        ...
        for (i = 0; i < sk_X509_num(ctx->chain); i++) {
            ...
            if ((x->ex_flags & EXFLAG_INVALID_POLICY) != 0)
                cbcalled = 1;
            CB_FAIL_IF((x->ex_flags & EXFLAG_INVALID_POLICY) != 0, ctx, x, i, X509_V_ERR_INVALID_POLICY_EXTENSION);
        }
        if (!cbcalled) {
...
static int internal_verify(X509_STORE_CTX* ctx)
{
    ...
    while (n >= 0) {
         ...
         */
        if (xi != NULL && (xs != xi || ((ctx->param->flags & X509_V_FLAG_CHECK_SS_SIGNATURE) != 0 && (xi->ex_flags & EXFLAG_SS) != 0))) {
            EVP_PKEY* pkey;
            /*
             ...
             */
            int ret = xs == xi && (xi->ex_flags & EXFLAG_CA) == 0 ? X509_V_OK : ossl_x509_signing_allowed(xi, xs);
            ...
            if ((pkey = X509_get0_pubkey(xi)) == NULL) {
                ...
                CB_FAIL_IF(X509_verify(xs, pkey) <= 0, ctx, xs, n, X509_V_ERR_CERT_SIGNATURE_FAILURE);
            }
        }
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
     ...
     */
    if (DANETLS_ENABLED(dane) && dane->certs != NULL && !X509_add_certs(sk_untrusted, dane->certs, X509_ADD_FLAG_DEFAULT))
        goto memerr;
    ...
    default:
        ...
        CB_FAIL_IF(DANETLS_ENABLED(dane) && (!DANETLS_HAS_PKIX(dane) || dane->pdpth >= 0), ctx, NULL, num - 1, X509_V_ERR_DANE_NO_MATCH);
        if (X509_self_signed(sk_X509_value(ctx->chain, num - 1), 0) > 0)
            return verify_cb_cert(ctx, NULL, num - 1, num == 1 ? X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT : X509_V_ERR_SELF_SIGNED_CERT_IN_CHAIN);
        return verify_cb_cert(
            ctx, NULL, num - 1, ctx->num_untrusted < num ? X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT : X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY);
    }
