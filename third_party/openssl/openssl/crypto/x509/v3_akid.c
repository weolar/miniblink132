...
const X509V3_EXT_METHOD ossl_v3_akey_id = { NID_authority_key_identifier, X509V3_EXT_MULTILINE, ASN1_ITEM_ref(AUTHORITY_KEYID), 0, 0, 0, 0, 0, 0,
    (X509V3_EXT_I2V)i2v_AUTHORITY_KEYID, (X509V3_EXT_V2I)v2i_AUTHORITY_KEYID, 0, 0, NULL };
...
static STACK_OF(CONF_VALUE) * i2v_AUTHORITY_KEYID(X509V3_EXT_METHOD* method, AUTHORITY_KEYID* akeyid, STACK_OF(CONF_VALUE) * extlist)
{
    ...
    if (akeyid->keyid) {
        ...
        if (!X509V3_add_value((akeyid->issuer || akeyid->serial) ? "keyid" : NULL, tmp, &extlist)) {
            OPENSSL_free(tmp);
...
static AUTHORITY_KEYID* v2i_AUTHORITY_KEYID(X509V3_EXT_METHOD* method, X509V3_CTX* ctx, STACK_OF(CONF_VALUE) * values)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (strcmp(cnf->name, "keyid") == 0) {
            ...
            ERR_raise_data(ERR_LIB_X509V3, X509V3_R_UNKNOWN_OPTION, "name=%s", cnf->name);
            goto err;
        }
    }
