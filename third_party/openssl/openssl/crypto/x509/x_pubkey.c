...
ASN1_SEQUENCE(X509_PUBKEY_INTERNAL) = { ASN1_SIMPLE(X509_PUBKEY, algor, X509_ALGOR),
    ASN1_SIMPLE(X509_PUBKEY, public_key, ASN1_BIT_STRING) } static_ASN1_SEQUENCE_END_name(X509_PUBKEY, X509_PUBKEY_INTERNAL)
...
static int x509_pubkey_ex_populate(ASN1_VALUE** pval, const ASN1_ITEM* it)
{
    ...
    return (pubkey->algor != NULL || (pubkey->algor = X509_ALGOR_new()) != NULL)
        && (pubkey->public_key != NULL || (pubkey->public_key = ASN1_BIT_STRING_new()) != NULL);
}
...
static int x509_pubkey_ex_new_ex(ASN1_VALUE** pval, const ASN1_ITEM* it, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if ((ret = OPENSSL_zalloc(sizeof(*ret))) == NULL || !x509_pubkey_ex_populate((ASN1_VALUE**)&ret, NULL) || !x509_pubkey_set0_libctx(ret, libctx, propq)) {
...
static int x509_pubkey_ex_d2i_ex(ASN1_VALUE** pval, const unsigned char** in, long len, const ASN1_ITEM* it, int tag, int aclass, char opt, ASN1_TLC* ctx,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (ret <= 0 && !pubkey->flag_force_legacy) {
        ...
        */
        ...
        if ((dctx = OSSL_DECODER_CTX_new_for_pkey(&pubkey->pkey, "DER", "SubjectPublicKeyInfo", txtoidname, EVP_PKEY_PUBLIC_KEY, pubkey->libctx, pubkey->propq))
            != NULL)
            /*
...
static int x509_pubkey_ex_i2d(const ASN1_VALUE** pval, unsigned char** out, const ASN1_ITEM* it, int tag, int aclass)
{
...
X509_PUBKEY* X509_PUBKEY_dup(const X509_PUBKEY* a)
{
    ...
    if (pubkey == NULL || !x509_pubkey_set0_libctx(pubkey, a->libctx, a->propq) || (pubkey->algor = X509_ALGOR_dup(a->algor)) == NULL
        || (pubkey->public_key = ASN1_BIT_STRING_new()) == NULL || !ASN1_BIT_STRING_set(pubkey->public_key, a->public_key->data, a->public_key->length)) {
 ...
 */
static EVP_PKEY* d2i_PUBKEY_int(EVP_PKEY** a, const unsigned char** pp, long length, OSSL_LIB_CTX* libctx, const char* propq, unsigned int force_legacy,
