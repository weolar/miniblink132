...
static int cache_objects(X509_LOOKUP* lctx, const char* uri, const OSSL_STORE_SEARCH* criterion, int depth, OSSL_LIB_CTX* libctx, const char* propq)
{
    int ok = 0;
    OSSL_STORE_CTX* ctx = NULL;
    X509_STORE* xstore = X509_LOOKUP_get_store(lctx);

    if ((ctx = OSSL_STORE_open_ex(uri, libctx, propq, NULL, NULL, NULL, NULL, NULL)) == NULL)
        return 0;
    ...
    for (;;) {
        ...
        if (infotype == OSSL_STORE_INFO_NAME) {
             ...
             */
            if (depth > 0)
                ok = cache_objects(lctx, OSSL_STORE_INFO_get0_NAME(info), criterion, depth - 1, libctx, propq);
        } else {
            /*
...
static int by_store_ctrl_ex(X509_LOOKUP* ctx, int cmd, const char* argp, long argl, char** retp, OSSL_LIB_CTX* libctx, const char* propq)
{
    switch (cmd) {
...
static int by_store(X509_LOOKUP* ctx, X509_LOOKUP_TYPE type, const OSSL_STORE_SEARCH* criterion, X509_OBJECT* ret, OSSL_LIB_CTX* libctx, const char* propq)
{
    STACK_OF(OPENSSL_STRING)* uris = X509_LOOKUP_get_method_data(ctx);
    int i;
    int ok = 0;
...
static int by_store_subject_ex(X509_LOOKUP* ctx, X509_LOOKUP_TYPE type, const X509_NAME* name, X509_OBJECT* ret, OSSL_LIB_CTX* libctx, const char* propq)
{
...
static int by_store_subject(X509_LOOKUP* ctx, X509_LOOKUP_TYPE type, const X509_NAME* name, X509_OBJECT* ret)
{
...
static X509_LOOKUP_METHOD x509_store_lookup = { "Load certs from STORE URIs", NULL, /* new_item */
    by_store_free, /* free */
    NULL, /* init */
    NULL, /* shutdown */
    by_store_ctrl, /* ctrl */
    ...
    NULL, /* get_by_alias */
    by_store_subject_ex, by_store_ctrl_ex };
