...
{
    if (ctx->skip || ctx->method == NULL || (ctx->method->get_by_subject == NULL && ctx->method->get_by_subject_ex == NULL))
        return 0;
...
int X509_STORE_CTX_get_by_subject(const X509_STORE_CTX* vs, X509_LOOKUP_TYPE type, const X509_NAME* name, X509_OBJECT* ret)
{
    ...
    if (tmp == NULL || type == X509_LU_CRL) {
        for (i = 0; i < sk_X509_LOOKUP_num(store->get_cert_methods); i++) {
            ...
            j = X509_LOOKUP_by_subject_ex(lu, type, name, &stmp, vs->libctx, vs->propq);
            if (j) {
                tmp = &stmp;
                break;
            }
...
static int x509_object_idx_cnt(STACK_OF(X509_OBJECT) * h, X509_LOOKUP_TYPE type, const X509_NAME* name, int* pnmatch)
{
    X509_OBJECT stmp;
    X509 x509_s;
    X509_CRL crl_s;
...
STACK_OF(X509_CRL) * X509_STORE_CTX_get1_crls(const X509_STORE_CTX* ctx, const X509_NAME* nm)
{
    int i, idx, cnt;
    ...
    if (sk == NULL || xobj == NULL || store == NULL || !X509_STORE_CTX_get_by_subject(ctx, X509_LU_CRL, nm, xobj)) {
        X509_OBJECT_free(xobj);
        sk_X509_CRL_free(sk);
        return NULL;
    }
    ...
    if (idx != -1) { /* should be true as we've had at least one match */
        ...
        for (i = idx; i < idx + nmatch; i++) {
            ...
            if (ctx->check_issued(ctx, x, pobj->data.x509)) {
                 ...
                 */
                if (*issuer == NULL || ASN1_TIME_compare(X509_get0_notAfter(pobj->data.x509), X509_get0_notAfter(*issuer)) > 0)
                    *issuer = pobj->data.x509;
            }
        }
    }
