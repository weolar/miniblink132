...
ASN1_SEQUENCE(ASRange) = { ASN1_SIMPLE(ASRange, min, ASN1_INTEGER), ASN1_SIMPLE(ASRange, max, ASN1_INTEGER) } ASN1_SEQUENCE_END(ASRange)

    ASN1_CHOICE(ASIdOrRange)
    = { ASN1_SIMPLE(ASIdOrRange, u.id, ASN1_INTEGER), ASN1_SIMPLE(ASIdOrRange, u.range, ASRange) } ASN1_CHOICE_END(ASIdOrRange)

        ASN1_CHOICE(ASIdentifierChoice)
    = { ASN1_SIMPLE(ASIdentifierChoice, u.inherit, ASN1_NULL), ASN1_SEQUENCE_OF(ASIdentifierChoice, u.asIdsOrRanges, ASIdOrRange) } ASN1_CHOICE_END(
        ASIdentifierChoice)

        ASN1_SEQUENCE(ASIdentifiers)
    = { ASN1_EXP_OPT(ASIdentifiers, asnum, ASIdentifierChoice, 0), ASN1_EXP_OPT(ASIdentifiers, rdi, ASIdentifierChoice, 1) } ASN1_SEQUENCE_END(ASIdentifiers)

        IMPLEMENT_ASN1_FUNCTIONS(ASRange) IMPLEMENT_ASN1_FUNCTIONS(ASIdOrRange) IMPLEMENT_ASN1_FUNCTIONS(ASIdentifierChoice)
...
static int ASIdOrRange_cmp(const ASIdOrRange* const* a_, const ASIdOrRange* const* b_)
{
    ...
    assert((a->type == ASIdOrRange_id && a->u.id != NULL)
        || (a->type == ASIdOrRange_range && a->u.range != NULL && a->u.range->min != NULL && a->u.range->max != NULL));

    assert((b->type == ASIdOrRange_id && b->u.id != NULL)
        || (b->type == ASIdOrRange_range && b->u.range != NULL && b->u.range->min != NULL && b->u.range->max != NULL));
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice* choice)
{
    ...
    for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
        ...
        if (!extract_min_max(a, &a_min, &a_max) || !extract_min_max(b, &b_min, &b_max))
            goto done;
    ...
    i = sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1;
    {
        ...
        if (a != NULL && a->type == ASIdOrRange_range) {
            if (!extract_min_max(a, &a_min, &a_max) || ASN1_INTEGER_cmp(a_min, a_max) > 0)
                goto done;
        }
    }
 ...
 */
static void* v2i_ASIdentifiers(const struct v3_ext_method* method, struct v3_ext_ctx* ctx, STACK_OF(CONF_VALUE) * values)
{
...
{
    return (asid != NULL
        && ((asid->asnum != NULL && asid->asnum->type == ASIdentifierChoice_inherit) || (asid->rdi != NULL && asid->rdi->type == ASIdentifierChoice_inherit)));
}
    ...
    for (c = 0; c < sk_ASIdOrRange_num(child); c++) {
        ...
        for (;; p++) {
            if (p >= sk_ASIdOrRange_num(parent))
                return 0;
            if (!extract_min_max(sk_ASIdOrRange_value(parent, p), &p_min, &p_max))
                return 0;
...
int X509v3_asid_subset(ASIdentifiers* a, ASIdentifiers* b)
{
    ...
    subset = a->asnum == NULL || (b->asnum != NULL && asid_contains(b->asnum->u.asIdsOrRanges, a->asnum->u.asIdsOrRanges));
    if (!subset)
        return 0;

    return a->rdi == NULL || (b->rdi != NULL && asid_contains(b->rdi->u.asIdsOrRanges, a->rdi->u.asIdsOrRanges));
}
