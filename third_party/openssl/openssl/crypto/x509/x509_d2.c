...
int X509_STORE_load_file_ex(X509_STORE* ctx, const char* file, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (file == NULL || (lookup = X509_STORE_add_lookup(ctx, X509_LOOKUP_file())) == NULL
        || X509_LOOKUP_load_file_ex(lookup, file, X509_FILETYPE_PEM, libctx, propq) <= 0)
        return 0;
...
int X509_STORE_load_path(X509_STORE* ctx, const char* path)
{
    ...
    if (path == NULL || (lookup = X509_STORE_add_lookup(ctx, X509_LOOKUP_hash_dir())) == NULL || X509_LOOKUP_add_dir(lookup, path, X509_FILETYPE_PEM) <= 0)
        return 0;
...
int X509_STORE_load_store_ex(X509_STORE* ctx, const char* uri, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (uri == NULL || (lookup = X509_STORE_add_lookup(ctx, X509_LOOKUP_store())) == NULL || X509_LOOKUP_add_store_ex(lookup, uri, libctx, propq) == 0)
        return 0;
...
int X509_STORE_load_locations_ex(X509_STORE* ctx, const char* file, const char* path, OSSL_LIB_CTX* libctx, const char* propq)
{
