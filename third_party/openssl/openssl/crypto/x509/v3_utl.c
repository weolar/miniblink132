...
{
    return x509v3_add_len_value(name, value, value != NULL ? strlen((const char*)value) : 0, extlist);
}

int X509V3_add_value_uchar(const char* name, const unsigned char* value, STACK_OF(CONF_VALUE) * *extlist)
{
    return x509v3_add_len_value(name, (const char*)value, value != NULL ? strlen((const char*)value) : 0, extlist);
}

int x509v3_add_len_value_uchar(const char* name, const unsigned char* value, size_t vallen, STACK_OF(CONF_VALUE) * *extlist)
{
...
char* i2s_ASN1_ENUMERATED(X509V3_EXT_METHOD* method, const ASN1_ENUMERATED* a)
{
    ...
    if (!a)
        return NULL;
    if ((bntmp = ASN1_ENUMERATED_to_BN(a, NULL)) == NULL || (strtmp = bignum_to_string(bntmp)) == NULL)
        ERR_raise(ERR_LIB_X509V3, ERR_R_MALLOC_FAILURE);
    BN_free(bntmp);
    return strtmp;
}
...
char* i2s_ASN1_INTEGER(X509V3_EXT_METHOD* method, const ASN1_INTEGER* a)
{
    ...
    if (!a)
        return NULL;
    if ((bntmp = ASN1_INTEGER_to_BN(a, NULL)) == NULL || (strtmp = bignum_to_string(bntmp)) == NULL)
        ERR_raise(ERR_LIB_X509V3, ERR_R_MALLOC_FAILURE);
    BN_free(bntmp);
    return strtmp;
}
...
int X509V3_get_value_bool(const CONF_VALUE* value, int* asn1_bool)
{
    ...
    if ((btmp = value->value) == NULL)
        goto err;
    if (strcmp(btmp, "TRUE") == 0 || strcmp(btmp, "true") == 0 || strcmp(btmp, "Y") == 0 || strcmp(btmp, "y") == 0 || strcmp(btmp, "YES") == 0
        || strcmp(btmp, "yes") == 0) {
        *asn1_bool = 0xff;
        return 1;
    }
    if (strcmp(btmp, "FALSE") == 0 || strcmp(btmp, "false") == 0 || strcmp(btmp, "N") == 0 || strcmp(btmp, "n") == 0 || strcmp(btmp, "NO") == 0
        || strcmp(btmp, "no") == 0) {
        *asn1_bool = 0;
        return 1;
    }
...
static STACK_OF(OPENSSL_STRING) * get_email(const X509_NAME* name, GENERAL_NAMES* gens)
{
    ...
    while ((i = X509_NAME_get_index_by_NID(name, NID_pkcs9_emailAddress, i)) >= 0) {
 ...
 */
static int wildcard_match(const unsigned char* prefix, size_t prefix_len, const unsigned char* suffix, size_t suffix_len, const unsigned char* subject,
...
static const unsigned char* valid_star(const unsigned char* p, size_t len, unsigned int flags)
{
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (p[i] == '*') {
            ...
            if ((state & LABEL_START) != 0 && len - i >= 4 && OPENSSL_strncasecmp((char*)&p[i], "xn--", 4) == 0)
                state |= LABEL_IDNA;
            ...
            if ((gen->type == GEN_OTHERNAME) && (check_type == GEN_EMAIL)) {
                if (OBJ_obj2nid(gen->d.otherName->type_id) == NID_id_on_SmtpUTF8Mailbox) {
                    ...
                    if (gen->d.otherName->value->type == V_ASN1_UTF8STRING) {
                        ...
                        if ((rv = do_check_string(cstr, 0, equal, flags, chk, chklen, peername)) != 0)
                            break;
                    }
                } else
                    continue;
...
int X509V3_NAME_from_section(X509_NAME* nm, STACK_OF(CONF_VALUE) * dn_sk, unsigned long chtype)
{
    ...
    for (i = 0; i < sk_CONF_VALUE_num(dn_sk); i++) {
        ...
        if (!X509_NAME_add_entry_by_txt(nm, type, chtype, (unsigned char*)v->value, -1, -1, mval))
            return 0;
    }
    return 1;
}
