...
static DSO_METHOD dso_meth_vms = {
    "OpenSSL 'VMS' shared library method", vms_load, NULL, /* unload */
    vms_bind_func, NULL, /* ctrl */
    vms_name_converter, vms_merger, NULL, /* init */
    NULL, /* finish */
    NULL, /* pathbyaddr */
    NULL /* globallookup */
};
    ...
    if (!$VMS_STATUS_SUCCESS(status)) {
        ...
        *sym = NULL;
        ...
        if (!$VMS_STATUS_SUCCESS(status))
            ...
            if (ptr->imagename_dsc.dsc$w_length)
                ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE, "Symbol %s in %s (%s): %s", symname, ptr->filename, ptr->imagename, errstring);
            else
                ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE, "Symbol %s in %s: %s", symname, ptr->filename, errstring);
        }
        return;
    }
    return;
...
#define FILESPEC2 ((char*)filespec2)
    ...
    if (!$VMS_STATUS_SUCCESS(status)) {
        ...
        if (!$VMS_STATUS_SUCCESS(status))
            ...
            ERR_raise_data(ERR_LIB_DSO, DSO_R_FAILURE, "filespec \"%s\", default \"%s\": %s", filespec1, filespec2, errstring);
        }
        return NULL;
    }
