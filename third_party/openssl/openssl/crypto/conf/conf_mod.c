...
int CONF_modules_load(const CONF* cnf, const char* appname, unsigned long flags)
{
    ...
    if (conf_diagnostics(cnf))
        flags &= ~(CONF_MFLAGS_IGNORE_ERRORS | CONF_MFLAGS_IGNORE_RETURN_CODES | CONF_MFLAGS_SILENT | CONF_MFLAGS_IGNORE_MISSING_FILE);
...
static int module_run(const CONF* cnf, const char* name, const char* value, unsigned long flags)
{
    ...
    if (ret <= 0) {
        if (!(flags & CONF_MFLAGS_SILENT))
            ERR_raise_data(ERR_LIB_CONF, CONF_R_MODULE_INITIALIZATION_ERROR, "module=%s, value=%s retcode=%-8d", name, value, ret);
    }
...
static int conf_modules_finish_int(void)
{
    ...
    if (module_list_lock == NULL || !CRYPTO_THREAD_write_lock(module_list_lock))
        return 0;
