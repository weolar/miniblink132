...
int ossl_param_build_set_utf8_string(OSSL_PARAM_BLD* bld, OSSL_PARAM* p, const char* key, const char* buf)
{
    if (bld != NULL)
...
int ossl_param_build_set_octet_string(OSSL_PARAM_BLD* bld, OSSL_PARAM* p, const char* key, const unsigned char* data, size_t data_len)
{
    if (bld != NULL)
...
int ossl_param_build_set_bn_pad(OSSL_PARAM_BLD* bld, OSSL_PARAM* p, const char* key, const BIGNUM* bn, size_t sz)
{
    if (bld != NULL)
...
int ossl_param_build_set_multi_key_bn(OSSL_PARAM_BLD* bld, OSSL_PARAM* params, const char* names[], STACK_OF(BIGNUM_const) * stk)
{
