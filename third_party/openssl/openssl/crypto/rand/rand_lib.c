...
{
    return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_DRBG_INDEX, &rand_drbg_ossl_ctx_method);
}
...
EVP_RAND_CTX* RAND_get0_public(OSSL_LIB_CTX* ctx)
{
    ...
    if (rand == NULL) {
         ...
         * used this thread.
         */
        if (CRYPTO_THREAD_get_local(&dgbl->private) == NULL && !ossl_init_thread_start(NULL, ctx, rand_delete_thread_state))
            return NULL;
...
EVP_RAND_CTX* RAND_get0_private(OSSL_LIB_CTX* ctx)
{
    ...
    if (rand == NULL) {
         ...
         * used this thread.
         */
        if (CRYPTO_THREAD_get_local(&dgbl->public) == NULL && !ossl_init_thread_start(NULL, ctx, rand_delete_thread_state))
            return NULL;
        rand = rand_new_drbg(ctx, primary, SECONDARY_RESEED_INTERVAL, SECONDARY_RESEED_TIME_INTERVAL);
...
int RAND_set_DRBG_type(OSSL_LIB_CTX* ctx, const char* drbg, const char* propq, const char* cipher, const char* digest)
{
    ...
    return random_set_string(&dgbl->rng_name, drbg) && random_set_string(&dgbl->rng_propq, propq) && random_set_string(&dgbl->rng_cipher, cipher)
