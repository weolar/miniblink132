...
int OSSL_HTTP_REQ_CTX_set_expected(OSSL_HTTP_REQ_CTX* rctx, const char* content_type, int asn1, int timeout, int keep_alive)
{
    ...
    if (content_type != NULL && (rctx->expected_ct = OPENSSL_strdup(content_type)) == NULL)
        return 0;
...
static int set1_content(OSSL_HTTP_REQ_CTX* rctx, const char* content_type, BIO* req)
{
    ...
    if (rctx->keep_alive != 0 && !OSSL_HTTP_REQ_CTX_add1_header(rctx, "Connection", "keep-alive"))
        return 0;
...
static int add1_headers(OSSL_HTTP_REQ_CTX* rctx, const STACK_OF(CONF_VALUE) * headers, const char* host)
{
    int i;
...
static OSSL_HTTP_REQ_CTX* http_req_ctx_new(int free_wbio, BIO* wbio, BIO* rbio, OSSL_HTTP_bio_cb_t bio_update_fn, void* arg, int use_ssl, const char* proxy,
    const char* server, const char* port, int buf_size, int overall_timeout)
{
    ...
    if (proxy != NULL && (rctx->proxy = OPENSSL_strdup(proxy)) == NULL)
        goto err;
    if (server != NULL && (rctx->server = OPENSSL_strdup(server)) == NULL)
        goto err;
    if (port != NULL && (rctx->port = OPENSSL_strdup(port)) == NULL)
        goto err;
    ...
    *reason++ = '\0';
    ...
    default:
        ...
        if (*reason == '\0')
            ...
            ERR_raise_data(ERR_LIB_HTTP, err, "code=%s, reason=%s", code, reason);
        return retcode;
    }
...
static int check_set_resp_len(OSSL_HTTP_REQ_CTX* rctx, size_t len)
{
    if (rctx->max_resp_len != 0 && len > rctx->max_resp_len) {
        ERR_raise_data(ERR_LIB_HTTP, HTTP_R_MAX_RESP_LEN_EXCEEDED, "length=%zu, max=%zu", len, rctx->max_resp_len);
        return 0;
    }
    if (rctx->resp_len != 0 && rctx->resp_len != len) {
        ERR_raise_data(ERR_LIB_HTTP, HTTP_R_INCONSISTENT_CONTENT_LENGTH, "ASN.1 length=%zu, Content-Length=%zu", len, rctx->resp_len);
        return 0;
    }
    rctx->resp_len = len;
    return 1;
...
next_io:
    ...
    next_line:
        ...
        if (value != NULL && line_end != NULL) {
            if (rctx->state == OHS_REDIRECT && OPENSSL_strcasecmp(key, "Location") == 0) {
                rctx->redirection_url = value;
                return 0;
            }
            if (rctx->state == OHS_HEADERS && rctx->expected_ct != NULL && OPENSSL_strcasecmp(key, "Content-Type") == 0) {
            ...
            if (OPENSSL_strcasecmp(key, "Connection") == 0) {
                ...
                if (line_end == value || *line_end != '\0') {
                    ERR_raise_data(ERR_LIB_HTTP, HTTP_R_ERROR_PARSING_CONTENT_LENGTH, "input=%s", value);
                    return 0;
                }
        ...
        if (rctx->expected_ct != NULL && !found_expected_ct) {
            ERR_raise_data(ERR_LIB_HTTP, HTTP_R_MISSING_CONTENT_TYPE, "expected=%s", rctx->expected_ct);
            return 0;
        }
...
static BIO* http_new_bio(const char* server /* optionally includes ":port" */, const char* server_port /* explicit server port */, int use_ssl,
    const char* proxy /* optionally includes ":port" */, const char* proxy_port /* explicit proxy port */)
{
...
OSSL_HTTP_REQ_CTX* OSSL_HTTP_open(const char* server, const char* port, const char* proxy, const char* no_proxy, int use_ssl, BIO* bio, BIO* rbio,
    OSSL_HTTP_bio_cb_t bio_update_fn, void* arg, int buf_size, int overall_timeout)
{
    ...
    if (bio != NULL) {
        ...
        if (proxy != NULL
            && !OSSL_HTTP_parse_url(proxy, NULL /* use_ssl */, NULL /* user */, &proxy_host, &proxy_port, NULL /* num */, NULL /* path */, NULL, NULL))
            return NULL;
    ...
    rctx = http_req_ctx_new(bio == NULL, cbio, rbio != NULL ? rbio : cbio, bio_update_fn, arg, use_ssl, proxy, server, port, buf_size, overall_timeout);
...
int OSSL_HTTP_set1_request(OSSL_HTTP_REQ_CTX* rctx, const char* path, const STACK_OF(CONF_VALUE) * headers, const char* content_type, BIO* req,
    const char* expected_content_type, int expect_asn1, size_t max_resp_len, int timeout, int keep_alive)
{
    int use_http_proxy;
    ...
    return OSSL_HTTP_REQ_CTX_set_request_line(rctx, req != NULL, use_http_proxy ? rctx->server : NULL, rctx->port, path)
        && add1_headers(rctx, headers, rctx->server) && OSSL_HTTP_REQ_CTX_set_expected(rctx, expected_content_type, expect_asn1, timeout, keep_alive)
...
BIO* OSSL_HTTP_exchange(OSSL_HTTP_REQ_CTX* rctx, char** redirection_url)
{
    ...
    if (resp == NULL) {
        if (rctx->redirection_url != NULL) {
            ...
            if (lib == ERR_LIB_SSL || lib == ERR_LIB_HTTP || (lib == ERR_LIB_BIO && reason == BIO_R_CONNECT_TIMEOUT)
                ...
                if (rctx->server != NULL) {
                    BIO_snprintf(buf, sizeof(buf), "server=http%s://%s%s%s", rctx->use_ssl ? "s" : "", rctx->server, rctx->port != NULL ? ":" : "",
...
BIO* OSSL_HTTP_get(const char* url, const char* proxy, const char* no_proxy, BIO* bio, BIO* rbio, OSSL_HTTP_bio_cb_t bio_update_fn, void* arg, int buf_size,
    const STACK_OF(CONF_VALUE) * headers, const char* expected_ct, int expect_asn1, size_t max_resp_len, int timeout)
{
    ...
    for (;;) {
        if (!OSSL_HTTP_parse_url(current_url, &use_ssl, NULL /* user */, &host, &port, NULL /* port_num */, &path, NULL, NULL))
            break;

        rctx = OSSL_HTTP_open(host, port, proxy, no_proxy, use_ssl, bio, rbio, bio_update_fn, arg, buf_size, timeout);
    new_rpath:
        if (rctx != NULL) {
            if (!OSSL_HTTP_set1_request(rctx, path, headers, NULL /* content_type */, NULL /* req */, expected_ct, expect_asn1, max_resp_len,
                    -1 /* use same max time (timeout) */, 0 /* no keep_alive */)) {
                OSSL_HTTP_REQ_CTX_free(rctx);
                rctx = NULL;
            } else {
        ...
        if (resp == NULL && redirection_url != NULL) {
            if (redirection_ok(++n_redirs, current_url, redirection_url) && may_still_retry(max_time, &timeout)) {
                (void)BIO_reset(bio);
...
BIO* OSSL_HTTP_transfer(OSSL_HTTP_REQ_CTX** prctx, const char* server, const char* port, const char* path, int use_ssl, const char* proxy, const char* no_proxy,
    BIO* bio, BIO* rbio, OSSL_HTTP_bio_cb_t bio_update_fn, void* arg, int buf_size, const STACK_OF(CONF_VALUE) * headers, const char* content_type, BIO* req,
    const char* expected_ct, int expect_asn1, size_t max_resp_len, int timeout, int keep_alive)
{
    OSSL_HTTP_REQ_CTX* rctx = prctx == NULL ? NULL : *prctx;
    BIO* resp = NULL;

    if (rctx == NULL) {
        rctx = OSSL_HTTP_open(server, port, proxy, no_proxy, use_ssl, bio, rbio, bio_update_fn, arg, buf_size, timeout);
        timeout = -1; /* Already set during opening the connection */
    }
    if (rctx != NULL) {
        if (OSSL_HTTP_set1_request(rctx, path, headers, content_type, req, expected_ct, expect_asn1, max_resp_len, timeout, keep_alive))
 ...
 */
int OSSL_HTTP_proxy_connect(
    BIO* bio, const char* server, const char* port, const char* proxyuser, const char* proxypass, int timeout, BIO* bio_err, const char* prog)
{
#undef BUF_SIZE
#define BUF_SIZE (8 * 1024)
    ...
    if (bio == NULL || server == NULL || (bio_err != NULL && prog == NULL)) {
        ERR_raise(ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER);
        goto end;
    }
    if (port == NULL || *port == '\0')
    ...
    if (proxyuser != NULL) {
        ...
        if (proxyauth == NULL)
            goto end;
        if (BIO_snprintf(proxyauth, len + 1, "%s:%s", proxyuser, proxypass != NULL ? proxypass : "") != (int)len)
            goto proxy_end;
    ...
    for (;;) {
        ...
        if (rv <= 0) {
            BIO_printf(bio_err, "%s: HTTP CONNECT %s\n", prog, rv == 0 ? "timed out" : "failed waiting for data");
            goto end;
        }
        ...
        if (!HAS_PREFIX(mbufp, " 2")) {
            ...
            while (read_len > 0 && ossl_isspace(mbuf[read_len - 1]))
                read_len--;
            mbuf[read_len] = '\0';
            ERR_raise_data(ERR_LIB_HTTP, HTTP_R_CONNECT_FAILURE, "reason=%s", mbufp);
