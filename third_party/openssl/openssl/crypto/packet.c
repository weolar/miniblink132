...
{
    if (!WPACKET_start_sub_packet_len__(pkt, lenbytes) || !WPACKET_allocate_bytes(pkt, len, allocbytes) || !WPACKET_close(pkt))
        return 0;

    return 1;
}

#define GETBUF(p) (((p)->staticbuf != NULL) ? (p)->staticbuf : ((p)->buf != NULL ? (unsigned char*)(p)->buf->data : NULL))
...
static int wpacket_intern_close(WPACKET* pkt, WPACKET_SUB* sub, int doclose)
{
    ...
    if (sub->lenbytes > 0) {
        ...
        if (buf != NULL && !put_value(&buf[sub->packet_len], packlen, sub->lenbytes))
            return 0;
    } else if (pkt->endfirst && sub->parent != NULL && (packlen != 0 || (sub->flags & WPACKET_FLAGS_ABANDON_ON_ZERO_LENGTH) == 0)) {
...
int WPACKET_put_bytes__(WPACKET* pkt, uint64_t val, size_t size)
{
    ...
    /* Internal API, so should not fail */
    if (!ossl_assert(size <= sizeof(uint64_t)) || !WPACKET_allocate_bytes(pkt, size, &data) || !put_value(data, val, size))
        return 0;
