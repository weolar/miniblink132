...
static int ossl_child_provider_init(const OSSL_CORE_HANDLE* handle, const OSSL_DISPATCH* in, const OSSL_DISPATCH** out, void** provctx)
{
    ...
    gbl = ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
    if (gbl == NULL)
        return 0;
...
static int provider_create_child_cb(const OSSL_CORE_HANDLE* prov, void* cbdata)
{
    ...
    if ((cprov = ossl_provider_find(ctx, provname, 1)) != NULL) {
         ...
         * init children
         */
        if ((cprov = ossl_provider_new(ctx, provname, ossl_child_provider_init, 1)) == NULL)
            goto err;
        ...
        if (!ossl_provider_set_child(cprov, prov) || !ossl_provider_add_to_store(cprov, NULL, 0)) {
...
int ossl_provider_init_as_child(OSSL_LIB_CTX* ctx, const OSSL_CORE_HANDLE* handle, const OSSL_DISPATCH* in)
{
    ...
    if (gbl->c_get_libctx == NULL || gbl->c_provider_register_child_cb == NULL || gbl->c_prov_name == NULL || gbl->c_prov_get0_provider_ctx == NULL
        || gbl->c_prov_get0_dispatch == NULL || gbl->c_prov_up_ref == NULL || gbl->c_prov_free == NULL)
        return 0;
...
{
    struct child_prov_globals* gbl = ossl_lib_ctx_get_data(ctx, OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
    if (gbl == NULL)
        return;
...
int ossl_provider_up_ref_parent(OSSL_PROVIDER* prov, int activate)
{
    ...
    gbl = ossl_lib_ctx_get_data(ossl_provider_libctx(prov), OSSL_LIB_CTX_CHILD_PROVIDER_INDEX, &child_prov_ossl_ctx_method);
    if (gbl == NULL)
        return 0;
