...
static OSSL_DECODER* ossl_decoder_new(void)
{
    ...
    if ((decoder = OPENSSL_zalloc(sizeof(*decoder))) == NULL || (decoder->base.lock = CRYPTO_THREAD_lock_new()) == NULL) {
...
{
    return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_DECODER_STORE_INDEX, &decoder_store_method);
}
...
static int put_decoder_in_store(void* store, void* method, const OSSL_PROVIDER* prov, const char* names, const char* propdef, void* data)
{
    ...
    return ossl_method_store_add(store, prov, id, propdef, method, (int (*)(void*))OSSL_DECODER_up_ref, (void (*)(void*))OSSL_DECODER_free);
}
...
void* ossl_decoder_from_algorithm(int id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov)
{
    ...
    if ((decoder->base.parsed_propdef = ossl_parse_property(libctx, algodef->property_definition)) == NULL) {
        OSSL_DECODER_free(decoder);
        return NULL;
    }
     ...
     */
    if (!((decoder->newctx == NULL && decoder->freectx == NULL) || (decoder->newctx != NULL && decoder->freectx != NULL)) || decoder->decode == NULL) {
        OSSL_DECODER_free(decoder);
...
static OSSL_DECODER* inner_ossl_decoder_fetch(struct decoder_data_st* methdata, int id, const char* name, const char* properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = { get_tmp_decoder_store, reserve_decoder_store, unreserve_decoder_store, get_decoder_from_store,
            put_decoder_in_store, construct_decoder, destruct_decoder };
        ...
        if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_DECODER, &prov, 0 /* !force_cache */, &mcm, methdata)) != NULL) {
            /*
    ...
    if ((id != 0 || name != NULL) && method == NULL) {
        ...
        ERR_raise_data(ERR_LIB_OSSL_DECODER, code, "%s, Name (%s : %d), Properties (%s)", ossl_lib_ctx_get_descriptor(methdata->libctx),
...
int OSSL_DECODER_names_do_all(const OSSL_DECODER* decoder, void (*fn)(const char* name, void* data), void* data)
{
    if (decoder == NULL)
        return 0;
...
void OSSL_DECODER_CTX_free(OSSL_DECODER_CTX* ctx)
{
    if (ctx != NULL) {
        ...
        sk_OSSL_DECODER_INSTANCE_pop_free(ctx->decoder_insts, ossl_decoder_instance_free);
