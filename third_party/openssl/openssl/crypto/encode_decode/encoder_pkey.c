...
int OSSL_ENCODER_CTX_set_cipher(OSSL_ENCODER_CTX* ctx, const char* cipher_name, const char* propquery)
{
    ...
    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_ENCODER_PARAM_CIPHER, (void*)cipher_name, 0);
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_ENCODER_PARAM_PROPERTIES, (void*)propquery, 0);
...
static void collect_encoder(OSSL_ENCODER* encoder, void* arg)
{
    ...
    for (i = 0; i < end_i; i++) {
        ...
        if (!OSSL_ENCODER_is_a(encoder, name) || (encoder->does_selection != NULL && !encoder->does_selection(provctx, data->ctx->selection))
            || (data->keymgmt_prov != prov && encoder->import_object == NULL))
            continue;
...
static int ossl_encoder_ctx_setup_for_pkey(OSSL_ENCODER_CTX* ctx, const EVP_PKEY* pkey, int selection, const char* propquery)
{
    ...
    if (data != NULL && OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0) {
        if (!OSSL_ENCODER_CTX_set_construct(ctx, encoder_construct_pkey) || !OSSL_ENCODER_CTX_set_construct_data(ctx, data)
...
OSSL_ENCODER_CTX* OSSL_ENCODER_CTX_new_for_pkey(const EVP_PKEY* pkey, int selection, const char* output_type, const char* output_struct, const char* propquery)
{
    ...
    if (OSSL_ENCODER_CTX_set_output_type(ctx, output_type) && (output_struct == NULL || OSSL_ENCODER_CTX_set_output_structure(ctx, output_struct))
        && OSSL_ENCODER_CTX_set_selection(ctx, selection) && ossl_encoder_ctx_setup_for_pkey(ctx, pkey, selection, propquery)
