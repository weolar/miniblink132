...
int OSSL_ENCODER_to_data(OSSL_ENCODER_CTX* ctx, unsigned char** pdata, size_t* pdata_len)
{
    ...
    if (out != NULL && OSSL_ENCODER_to_bio(ctx, out) && BIO_get_mem_ptr(out, &buf) > 0) {
...
static OSSL_ENCODER_INSTANCE* ossl_encoder_instance_new(OSSL_ENCODER* encoder, void* encoderctx)
{
    ...
    if (props == NULL) {
        ERR_raise_data(
            ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION, "there are no property definitions with encoder %s", OSSL_ENCODER_get0_name(encoder));
        goto err;
    }
    ...
    if (encoder_inst->output_type == NULL) {
        ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION,
            ...
            OSSL_ENCODER_get0_name(encoder), OSSL_ENCODER_get0_properties(encoder));
        goto err;
    }
...
static int ossl_encoder_ctx_add_encoder_inst(OSSL_ENCODER_CTX* ctx, OSSL_ENCODER_INSTANCE* ei)
{
    int ok;

    if (ctx->encoder_insts == NULL && (ctx->encoder_insts = sk_OSSL_ENCODER_INSTANCE_new_null()) == NULL) {
        ERR_raise(ERR_LIB_OSSL_ENCODER, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    ...
    if (ok) {
        OSSL_TRACE_BEGIN(ENCODER)
        {
            BIO_printf(trc_out,
                ...
                "    %s with %s\n",
                (void*)ctx, (void*)ei, (void*)ei->encoder, OSSL_ENCODER_get0_name(ei->encoder), OSSL_ENCODER_get0_properties(ei->encoder));
        }
...
int OSSL_ENCODER_CTX_add_encoder(OSSL_ENCODER_CTX* ctx, OSSL_ENCODER* encoder)
{
    ...
    if ((encoderctx = encoder->newctx(provctx)) == NULL || (encoder_inst = ossl_encoder_instance_new(encoder, encoderctx)) == NULL)
        goto err;
...
static int encoder_process(struct encoder_process_data_st* data)
{
    ...
    for (i = data->current_encoder_inst_index; i-- > 0;) {
        ...
        OSSL_TRACE_BEGIN(ENCODER)
        {
            BIO_printf(trc_out, "[%d] (ctx %p) Considering encoder instance %p (encoder %p)\n", data->level, (void*)data->ctx, (void*)current_encoder_inst,
         ...
         */
        if (top) {
            if (data->ctx->output_type != NULL && OPENSSL_strcasecmp(current_output_type, data->ctx->output_type) != 0) {
                OSSL_TRACE_BEGIN(ENCODER)
                {
            ...
            if (!OSSL_ENCODER_is_a(next_encoder, current_output_type)) {
                OSSL_TRACE_BEGIN(ENCODER)
                {
                    BIO_printf(trc_out, "[%d]    Skipping because current encoder output type (%s) != name of encoder %p\n", data->level, current_output_type,
                        (void*)next_encoder);
                }
                OSSL_TRACE_END(ENCODER);
                continue;
         ...
         * the current encoder.
         */
        if (data->ctx->output_structure != NULL && current_output_structure != NULL) {
            if (OPENSSL_strcasecmp(data->ctx->output_structure, current_output_structure) != 0) {
                OSSL_TRACE_BEGIN(ENCODER)
                {
                    BIO_printf(trc_out, "[%d]    Skipping because current encoder output structure (%s) != ctx output structure (%s)\n", data->level,
                        current_output_structure, data->ctx->output_structure);
                }
                OSSL_TRACE_END(ENCODER);
                continue;
            }
        ...
        OSSL_TRACE_BEGIN(ENCODER)
        {
            BIO_printf(trc_out, "[%d]    Skipping because recusion level %d failed\n", data->level, new_data.level);
        }
        OSSL_TRACE_END(ENCODER);
    }
    ...
    if (i < 0) {
        ...
        OSSL_TRACE_BEGIN(ENCODER)
        {
            BIO_printf(trc_out, "[%d] (ctx %p) No suitable encoder found\n", data->level, (void*)data->ctx);
        }
        OSSL_TRACE_END(ENCODER);
    } else {
        /* Preparations */
        ...
        case -1:
            ...
            original_data = data->ctx->construct(current_encoder_inst, data->ctx->construct_data);
        ...
        case 1:
...

            {
                ...
                *abstract_p++ = OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_TYPE, (char*)data->data_type, 0);
                if (prev_output_structure != NULL)
                    *abstract_p++ = OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_STRUCTURE, (char*)prev_output_structure, 0);
                *abstract_p++ = OSSL_PARAM_construct_octet_string(OSSL_OBJECT_PARAM_DATA, data->running_output, data->running_output_length);
        ...
        if (ok) {
            ...
            if (ok) {
                ok = current_encoder->encode(
                    current_encoder_ctx, cbio, original_data, current_abstract, data->ctx->selection, ossl_pw_passphrase_callback_enc, &data->ctx->pwdata);
                OSSL_TRACE_BEGIN(ENCODER)
                {
                    BIO_printf(trc_out, "[%d] (ctx %p) Running encoder instance %p => %d\n", data->level, (void*)data->ctx, (void*)current_encoder_inst, ok);
                }
                OSSL_TRACE_END(ENCODER);
            }
