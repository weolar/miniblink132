...
int OSSL_DECODER_from_bio(OSSL_DECODER_CTX* ctx, BIO* in)
{
    ...
    if (!data.flag_construct_called) {
        const char* spaces = ctx->start_input_type != NULL && ctx->input_structure != NULL ? " " : "";
        const char* input_type_label = ctx->start_input_type != NULL ? "Input type: " : "";
        const char* input_structure_label = ctx->input_structure != NULL ? "Input structure: " : "";
        const char* comma = ctx->start_input_type != NULL && ctx->input_structure != NULL ? ", " : "";
        const char* input_type = ctx->start_input_type != NULL ? ctx->start_input_type : "";
        const char* input_structure = ctx->input_structure != NULL ? ctx->input_structure : "";

        if (ERR_peek_last_error() == lasterr || ERR_peek_error() == 0)
            /* Prevent spurious decoding error but add at least something */
            ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_UNSUPPORTED, "No supported data to decode. %s%s%s%s%s%s", spaces, input_type_label, input_type, comma,
...
OSSL_DECODER_INSTANCE* ossl_decoder_instance_new(OSSL_DECODER* decoder, void* decoderctx)
{
    ...
    if (props == NULL) {
        ERR_raise_data(
            ERR_LIB_OSSL_DECODER, ERR_R_INVALID_PROPERTY_DEFINITION, "there are no property definitions with decoder %s", OSSL_DECODER_get0_name(decoder));
        goto err;
    }
...
int ossl_decoder_ctx_add_decoder_inst(OSSL_DECODER_CTX* ctx, OSSL_DECODER_INSTANCE* di)
{
    ...
    if (ctx->decoder_insts == NULL && (ctx->decoder_insts = sk_OSSL_DECODER_INSTANCE_new_null()) == NULL) {
        ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    ...
    if (ok) {
        OSSL_TRACE_BEGIN(DECODER)
        {
            BIO_printf(trc_out,
                ...
                "    %s with %s\n",
                (void*)ctx, (void*)di, (void*)di->decoder, OSSL_DECODER_get0_name(di->decoder), OSSL_DECODER_get0_properties(di->decoder));
        }
...
int OSSL_DECODER_CTX_add_decoder(OSSL_DECODER_CTX* ctx, OSSL_DECODER* decoder)
{
    ...
    if ((decoderctx = decoder->newctx(provctx)) == NULL || (decoder_inst = ossl_decoder_instance_new(decoder, decoderctx)) == NULL)
        goto err;
...
static void collect_all_decoders(OSSL_DECODER* decoder, void* arg)
{
    ...
    if (OSSL_DECODER_up_ref(decoder) && !sk_OSSL_DECODER_push(skdecoders, decoder))
    ...
    if (OSSL_DECODER_is_a(decoder, data->output_type)) {
        ...
        for (j = data->w_prev_start; j < data->w_new_end; j++) {
            ...
            if (decoder->base.algodef == check_inst->decoder->base.algodef) {
                ...
                OSSL_TRACE_BEGIN(DECODER)
                {
                    BIO_printf(trc_out, "    REJECTED: already exists in the chain\n");
                }
                OSSL_TRACE_END(DECODER);
                return;
            }
        ...
        case IS_SAME:
            ...
            if (!OSSL_DECODER_is_a(decoder, OSSL_DECODER_INSTANCE_get_input_type(di))) {
                ossl_decoder_instance_free(di);
                OSSL_TRACE_BEGIN(DECODER)
                {
                    BIO_printf(trc_out, "    REJECTED: input type doesn't match output type\n");
                }
                OSSL_TRACE_END(DECODER);
                return;
            }
        ...
        case IS_DIFFERENT:
            ...
            if (OSSL_DECODER_is_a(decoder, OSSL_DECODER_INSTANCE_get_input_type(di))) {
                ...
                OSSL_TRACE_BEGIN(DECODER)
                {
                    BIO_printf(trc_out, "    REJECTED: input type matches output type\n");
                }
                OSSL_TRACE_END(DECODER);
                return;
            }
...
int OSSL_DECODER_CTX_add_extra(OSSL_DECODER_CTX* ctx, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    do {
         ...
         */
        for (data.type_check = IS_SAME; data.type_check <= IS_DIFFERENT; data.type_check++) {
...
int OSSL_DECODER_export(OSSL_DECODER_INSTANCE* decoder_inst, void* reference, size_t reference_sz, OSSL_CALLBACK* export_cb, void* export_cbarg)
{
    ...
    if (!(ossl_assert(decoder_inst != NULL) && ossl_assert(reference != NULL) && ossl_assert(export_cb != NULL) && ossl_assert(export_cbarg != NULL))) {
        ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
    }
    ...
    return decoder->export_object(decoderctx, reference, reference_sz, export_cb, export_cbarg);
}
...
#define LEVEL_STR ">>>>>>>>>>>>>>>>"
#define LEVEL (new_data.recursion < sizeof(LEVEL_STR) ? &LEVEL_STR[sizeof(LEVEL_STR) - new_data.recursion - 1] : LEVEL_STR "...")
    ...
    if (params == NULL) {
        ...
        decoder_inst = sk_OSSL_DECODER_INSTANCE_value(ctx->decoder_insts, data->current_decoder_inst_index);
        decoder = OSSL_DECODER_INSTANCE_get_decoder(decoder_inst);
        ...
        if (ctx->construct != NULL) {
            ...
            OSSL_TRACE_BEGIN(DECODER)
            {
                BIO_printf(trc_out, "(ctx %p) %s Running constructor => %d\n", (void*)new_data.ctx, LEVEL, rv);
            }
            OSSL_TRACE_END(DECODER);
        ...
        if (data_type != NULL && data_structure != NULL && OPENSSL_strcasecmp(data_structure, "type-specific") == 0)
            data_structure = NULL;
        ...
        OSSL_TRACE_BEGIN(DECODER)
        {
            BIO_printf(trc_out,
                ...
                (void*)new_data.ctx, LEVEL, (void*)decoder, data_type, trace_data_structure, (trace_data_structure == data_structure ? "" : " (dropped)"));
        }
        OSSL_TRACE_END(DECODER);
    }
    ...
    for (i = data->current_decoder_inst_index; i-- > 0;) {
        ...
        const char* new_input_structure = OSSL_DECODER_INSTANCE_get_input_structure(new_decoder_inst, &n_i_s_was_set);
         ...
         */
        if (decoder == NULL && ctx->start_input_type != NULL && OPENSSL_strcasecmp(ctx->start_input_type, new_input_type) != 0) {
            OSSL_TRACE_BEGIN(DECODER)
            {
                BIO_printf(trc_out, "(ctx %p) %s [%u] the start input type '%s' doesn't match the input type of the considered decoder, skipping...\n",
                    (void*)new_data.ctx, LEVEL, (unsigned int)i, ctx->start_input_type);
            }
            OSSL_TRACE_END(DECODER);
            continue;
        }
        ...
        if (decoder != NULL && !OSSL_DECODER_is_a(decoder, new_input_type)) {
            OSSL_TRACE_BEGIN(DECODER)
            {
                BIO_printf(trc_out, "(ctx %p) %s [%u] the input type doesn't match the name of the previous decoder (%p), skipping...\n", (void*)new_data.ctx,
                    LEVEL, (unsigned int)i, (void*)decoder);
            }
            OSSL_TRACE_END(DECODER);
            continue;
        }
        ...
        if (data_type != NULL && !OSSL_DECODER_is_a(new_decoder, data_type)) {
            OSSL_TRACE_BEGIN(DECODER)
            {
                BIO_printf(trc_out, "(ctx %p) %s [%u] the previous decoder's data type doesn't match the name of the considered decoder, skipping...\n",
         ...
         */
        if (data_structure != NULL && (new_input_structure == NULL || OPENSSL_strcasecmp(data_structure, new_input_structure) != 0)) {
            OSSL_TRACE_BEGIN(DECODER)
            {
                BIO_printf(trc_out,
         ...
         */
        if (!data->flag_input_structure_checked && ctx->input_structure != NULL && new_input_structure != NULL) {
        ...
        ok = new_decoder->decode(
            new_decoderctx, cbio, new_data.ctx->selection, decoder_process, &new_data, ossl_pw_passphrase_callback_dec, &new_data.ctx->pwdata);
        ...
        OSSL_TRACE_BEGIN(DECODER)
        {
            BIO_printf(trc_out,
                ...
                (void*)new_data.ctx, LEVEL, (unsigned int)i, (void*)new_decoder_inst, ok, new_data.flag_next_level_called ? "yes" : "no",
