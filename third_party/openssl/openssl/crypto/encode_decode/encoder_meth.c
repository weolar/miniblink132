...
static OSSL_ENCODER* ossl_encoder_new(void)
{
    ...
    if ((encoder = OPENSSL_zalloc(sizeof(*encoder))) == NULL || (encoder->base.lock = CRYPTO_THREAD_lock_new()) == NULL) {
...
{
    return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_ENCODER_STORE_INDEX, &encoder_store_method);
}
...
static int put_encoder_in_store(void* store, void* method, const OSSL_PROVIDER* prov, const char* names, const char* propdef, void* data)
{
    struct encoder_data_st* methdata = data;
    ...
    return ossl_method_store_add(store, prov, id, propdef, method, (int (*)(void*))OSSL_ENCODER_up_ref, (void (*)(void*))OSSL_ENCODER_free);
}

/* Create and populate a encoder method */
static void* encoder_from_algorithm(int id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov)
{
    OSSL_ENCODER* encoder = NULL;
    ...
    if ((encoder->base.parsed_propdef = ossl_parse_property(libctx, algodef->property_definition)) == NULL) {
        OSSL_ENCODER_free(encoder);
        return NULL;
    }
     ...
     */
    if (!((encoder->newctx == NULL && encoder->freectx == NULL) || (encoder->newctx != NULL && encoder->freectx != NULL)
            || (encoder->import_object != NULL && encoder->free_object != NULL) || (encoder->import_object == NULL && encoder->free_object == NULL))
        || encoder->encode == NULL) {
        OSSL_ENCODER_free(encoder);
 ...
 */
static void* construct_encoder(const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov, void* data)
{
    /*
...
static OSSL_ENCODER* inner_ossl_encoder_fetch(struct encoder_data_st* methdata, int id, const char* name, const char* properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = { get_tmp_encoder_store, reserve_encoder_store, unreserve_encoder_store, get_encoder_from_store,
            put_encoder_in_store, construct_encoder, destruct_encoder };
        ...
        if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_ENCODER, &prov, 0 /* !force_cache */, &mcm, methdata)) != NULL) {
            /*
            ...
            if (id == 0)
                id = ossl_namemap_name2num(namemap, name);
            ossl_method_store_cache_set(store, prov, id, propq, method, up_ref_encoder, free_encoder);
        }
    ...
    if ((id != 0 || name != NULL) && method == NULL) {
        ...
        ERR_raise_data(ERR_LIB_OSSL_ENCODER, code, "%s, Name (%s : %d), Properties (%s)", ossl_lib_ctx_get_descriptor(methdata->libctx),
            name == NULL ? "<null>" : name, id, properties == NULL ? "<null>" : properties);
    }
...
OSSL_ENCODER* ossl_encoder_fetch_by_number(OSSL_LIB_CTX* libctx, int id, const char* properties)
{
    struct encoder_data_st methdata;
    void* method;
...
void OSSL_ENCODER_do_all_provided(OSSL_LIB_CTX* libctx, void (*user_fn)(OSSL_ENCODER* encoder, void* arg), void* user_arg)
{
    struct encoder_data_st methdata;
...
{
    if (ctx != NULL) {
        sk_OSSL_ENCODER_INSTANCE_pop_free(ctx->encoder_insts, ossl_encoder_instance_free);
