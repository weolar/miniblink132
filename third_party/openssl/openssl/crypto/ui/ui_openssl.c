...
static int read_string_inner(UI* ui, UI_STRING* uis, int echo, int strip_nl)
{
    ...
    if (is_a_tty) {
        ...
        if (GetEnvironmentVariableW(L"OPENSSL_WIN32_UTF8", NULL, 0) != 0) {
            ...
            if (ReadConsoleW(GetStdHandle(STD_INPUT_HANDLE), wresult, maxsize, &numread, NULL)) {
...
#endif
            if (ReadConsoleA(GetStdHandle(STD_INPUT_HANDLE), result, maxsize, &numread, NULL)) {
...
#define DEV_TTY "/dev/tty"
    ...
    if (status != SS$_NORMAL) {
        ERR_raise_data(ERR_LIB_UI, UI_R_SYSASSIGN_ERROR, "status=%%X%08X", status);
        return 0;
    }
...
static int noecho_console(UI* ui)
{
    ...
    if (is_a_tty) {
        ...
        if ((status != SS$_NORMAL) || (iosb.iosb$w_value != SS$_NORMAL)) {
            ERR_raise_data(ERR_LIB_UI, UI_R_SYSQIOW_ERROR, "status=%%X%08X, iosb.iosb$w_value=%%X%08X", status, iosb.iosb$w_value);
            return 0;
        }
    }
#endif
...
static int close_console(UI* ui)
{
    ...
    if (status != SS$_NORMAL) {
        ERR_raise_data(ERR_LIB_UI, UI_R_SYSDASSGN_ERROR, "status=%%X%08X", status);
        ret = 0;
    }
#endif
...
static UI_METHOD ui_openssl = { "OpenSSL default user interface", open_console, write_string, NULL, /* No flusher is needed for command lines */
    read_string, close_console, NULL };
