...
int ossl_ffc_params_fromdata(FFC_PARAMS* ffc, const OSSL_PARAM params[])
{
    ...
    if (prm != NULL) {
        ...
        if (prm->data_type != OSSL_PARAM_UTF8_STRING || prm->data == NULL || (group = ossl_ffc_name_to_dh_named_group(prm->data)) == NULL
    ...
    if ((param_p != NULL && !OSSL_PARAM_get_BN(param_p, &p)) || (param_q != NULL && !OSSL_PARAM_get_BN(param_q, &q))
