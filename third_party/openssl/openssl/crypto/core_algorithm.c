 ...
 */
static int algorithm_do_map(OSSL_PROVIDER* provider, const OSSL_ALGORITHM* map, int cur_operation, int no_store, void* cbdata)
{
    ...
    if (data->pre == NULL) {
        ...
        ret = 1;
    } else if (!data->pre(provider, cur_operation, no_store, data->data, &ret)) {
        /* Error, bail out! */
        ret = -1;
        goto end;
    }
    ...
    if (data->post == NULL) {
        ...
        ret = 1;
    } else if (!data->post(provider, cur_operation, no_store, data->data, &ret)) {
        /* Error, bail out! */
        ret = -1;
    }
...
void ossl_algorithm_do_all(OSSL_LIB_CTX* libctx, int operation_id, OSSL_PROVIDER* provider,
    int (*pre)(OSSL_PROVIDER*, int operation_id, int no_store, void* data, int* result), int (*reserve_store)(int no_store, void* data),
    void (*fn)(OSSL_PROVIDER* provider, const OSSL_ALGORITHM* algo, int no_store, void* data), int (*unreserve_store)(void* data),
    int (*post)(OSSL_PROVIDER*, int operation_id, int no_store, void* data, int* result), void* data)
{
    ...
    if (provider == NULL) {
         ...
         */
        if (!ossl_assert(ossl_lib_ctx_get_concrete(libctx) == ossl_lib_ctx_get_concrete(libctx2)))
            return;
