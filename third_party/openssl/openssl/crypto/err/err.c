...
typedef enum ERR_GET_ACTION_e { EV_POP, EV_PEEK, EV_PEEK_LAST } ERR_GET_ACTION;

static unsigned long get_error_values(ERR_GET_ACTION g, const char** file, int* line, const char** func, const char** data, int* flags);
...
ERR_STATE* ossl_err_get_state_int(void)
{
    ...
    if (state == NULL) {
        ...
        if (!ossl_init_thread_start(NULL, NULL, err_delete_thread_state) || !CRYPTO_THREAD_set_local(&err_thread_local, state)) {
            ERR_STATE_free(state);
            CRYPTO_THREAD_set_local(&err_thread_local, NULL);
            return NULL;
        }
...
static int err_set_error_data_int(char* data, size_t size, int flags, int deallocate)
{
    ERR_STATE* es;
...
void err_clear_last_constant_time(int clear)
{
     ...
     */
    clear = constant_time_select_int(constant_time_eq_int(clear, 0), 0, ERR_FLAG_CLEAR);
