...
static EVP_PKEY* d2i_PrivateKey_decoder(int keytype, EVP_PKEY** a, const unsigned char** pp, long length, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    for (i = 0; i < (int)OSSL_NELEM(input_structures); ++i) {
        ...
        if (a != NULL && (bak_a = *a) != NULL)
            ppkey = a;
        dctx = OSSL_DECODER_CTX_new_for_pkey(ppkey, "DER", input_structures[i], key_name, EVP_PKEY_KEYPAIR, libctx, propq);
        if (a != NULL)
            *a = bak_a;
        if (dctx == NULL)
            continue;
        ...
        if (ret) {
            if (*ppkey != NULL && evp_keymgmt_util_has(*ppkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY)) {
                if (a != NULL)
                    *a = *ppkey;
                return *ppkey;
            }
...
EVP_PKEY* ossl_d2i_PrivateKey_legacy(int keytype, EVP_PKEY** a, const unsigned char** pp, long length, OSSL_LIB_CTX* libctx, const char* propq)
{
    EVP_PKEY* ret;
    const unsigned char* p = *pp;
...
EVP_PKEY* d2i_PrivateKey_ex(int keytype, EVP_PKEY** a, const unsigned char** pp, long length, OSSL_LIB_CTX* libctx, const char* propq)
{
    EVP_PKEY* ret;
...
static EVP_PKEY* d2i_AutoPrivateKey_legacy(EVP_PKEY** a, const unsigned char** pp, long length, OSSL_LIB_CTX* libctx, const char* propq)
{
