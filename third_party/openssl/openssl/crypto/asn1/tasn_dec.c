...
static int asn1_check_tlen(
    long* olen, int* otag, unsigned char* oclass, char* inf, char* cst, const unsigned char** in, long len, int exptag, int expclass, char opt, ASN1_TLC* ctx);
...
static int asn1_template_noexp_d2i(
    ASN1_VALUE** val, const unsigned char** in, long len, const ASN1_TEMPLATE* tt, char opt, ASN1_TLC* ctx, int depth, OSSL_LIB_CTX* libctx, const char* propq);
...
static int asn1_item_ex_d2i_intern(ASN1_VALUE** pval, const unsigned char** in, long len, const ASN1_ITEM* it, int tag, int aclass, char opt, ASN1_TLC* ctx,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    int rv;
...
ASN1_VALUE* ASN1_item_d2i_ex(ASN1_VALUE** pval, const unsigned char** in, long len, const ASN1_ITEM* it, OSSL_LIB_CTX* libctx, const char* propq)
{
    ASN1_TLC c;
    ...
    int depth, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    case ASN1_ITYPE_PRIMITIVE:
        if (it->templates) {
             ...
             */
            if ((tag != -1) || opt) {
                ERR_raise(ERR_LIB_ASN1, ASN1_R_ILLEGAL_OPTIONS_ON_ITEM_TEMPLATE);
                goto err;
            }
            return asn1_template_ex_d2i(pval, in, len, it->templates, opt, ctx, depth, libctx, propq);
        }
...
static int asn1_template_ex_d2i(ASN1_VALUE** val, const unsigned char** in, long inlen, const ASN1_TEMPLATE* tt, char opt, ASN1_TLC* ctx, int depth,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    int flags, aclass;
    int ret;
    ...
    if (flags & ASN1_TFLG_EXPTAG) {
        ...
        ret = asn1_template_noexp_d2i(val, &p, len, tt, 0, ctx, depth, libctx, propq);
        if (!ret) {
            ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
            return 0;
        }
    ...
    ASN1_VALUE** val, const unsigned char** in, long len, const ASN1_TEMPLATE* tt, char opt, ASN1_TLC* ctx, int depth, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (flags & ASN1_TFLG_SK_MASK) {
        ...
        while (len > 0) {
            ...
            skfield = NULL;
            if (asn1_item_embed_d2i(&skfield, &p, len, ASN1_ITEM_ptr(tt->item), -1, 0, 0, ctx, depth, libctx, propq) <= 0) {
                ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
        ...
        /* IMPLICIT tagging */
        ret = asn1_item_embed_d2i(val, &p, len, ASN1_ITEM_ptr(tt->item), tt->tag, aclass, opt, ctx, depth, libctx, propq);
        if (!ret) {
            ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
            goto err;
        } else if (ret == -1)
        ...
        /* Nothing special */
        ret = asn1_item_embed_d2i(val, &p, len, ASN1_ITEM_ptr(tt->item), -1, 0, opt, ctx, depth, libctx, propq);
        if (!ret) {
            ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
            goto err;
        } else if (ret == -1)
