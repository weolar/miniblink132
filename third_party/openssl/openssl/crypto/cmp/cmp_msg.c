...
OSSL_CMP_MSG* OSSL_CMP_MSG_new(OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    msg = (OSSL_CMP_MSG*)ASN1_item_new_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx, propq);
...
static int add_extensions(STACK_OF(X509_EXTENSION) * *target, const STACK_OF(X509_EXTENSION) * exts)
{
    int i;
...
OSSL_CMP_MSG* ossl_cmp_msg_create(OSSL_CMP_CTX* ctx, int bodytype)
{
    ...
    if ((msg = OSSL_CMP_MSG_new(ctx->libctx, ctx->propq)) == NULL)
        return NULL;
    if (!ossl_cmp_hdr_init(ctx, msg->header) || !ossl_cmp_msg_set_bodytype(msg, bodytype))
        goto err;
    if (ctx->geninfo_ITAVs != NULL && !ossl_cmp_hdr_generalInfo_push1_items(msg->header, ctx->geninfo_ITAVs))
        goto err;
...
#define HAS_SAN(ctx) (sk_GENERAL_NAME_num((ctx)->subjectAltNames) > 0 || OSSL_CMP_CTX_reqExtensions_have_SAN(ctx) == 1)

static const X509_NAME* determine_subj(OSSL_CMP_CTX* ctx, int for_KUR, const X509_NAME* ref_subj)
{
...
OSSL_CRMF_MSG* OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX* ctx, int for_KUR, int rid)
{
    ...
    const X509_NAME* issuer = ctx->issuer != NULL || refcert == NULL ? (IS_NULL_DN(ctx->issuer) ? NULL : ctx->issuer) : X509_get_issuer_name(refcert);
    ...
    if (!OSSL_CRMF_MSG_set_certReqId(crm, rid)
        /*
             ...
             */
        || !OSSL_CRMF_CERTTEMPLATE_fill(OSSL_CRMF_MSG_get0_tmpl(crm), rkey, subject, issuer, NULL /* serial */))
        goto err;
    if (ctx->days != 0) {
        ...
        if (notBefore == NULL || notAfter == NULL || !OSSL_CRMF_MSG_set0_validity(crm, notBefore, notAfter)) {
    ...
    /* extensions */
    if (ctx->p10CSR != NULL && (exts = X509_REQ_get_extensions(ctx->p10CSR)) == NULL)
        goto err;
    if (!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) && refcert != NULL
        && (default_sans = X509V3_get_d2i(X509_get0_extensions(refcert), NID_subject_alt_name, NULL, NULL)) != NULL
    ...
    if (ctx->reqExtensions != NULL /* augment/override existing ones */
        ...
        goto err;
    if (sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 && !add1_extension(&exts, NID_subject_alt_name, crit, ctx->subjectAltNames))
        goto err;
    if (ctx->policies != NULL && !add1_extension(&exts, NID_certificate_policies, ctx->setPoliciesCritical, ctx->policies))
        goto err;
    ...
    if (for_KUR && refcert != NULL) {
        OSSL_CRMF_CERTID* cid = OSSL_CRMF_CERTID_gen(X509_get_issuer_name(refcert), X509_get0_serialNumber(refcert));
        int ret;
...
OSSL_CMP_MSG* ossl_cmp_certreq_new(OSSL_CMP_CTX* ctx, int type, const OSSL_CRMF_MSG* crm)
{
    ...
    if (type != OSSL_CMP_PKIBODY_IR && type != OSSL_CMP_PKIBODY_CR && type != OSSL_CMP_PKIBODY_KUR && type != OSSL_CMP_PKIBODY_P10CR) {
    ...
    if (type != OSSL_CMP_PKIBODY_P10CR) {
        ...
        if (crm == NULL) {
            local_crm = OSSL_CMP_CTX_setup_CRM(ctx, type == OSSL_CMP_PKIBODY_KUR, OSSL_CMP_CERTREQID);
            if (local_crm == NULL || !OSSL_CRMF_MSG_create_popo(ctx->popoMethod, local_crm, privkey, ctx->digest, ctx->libctx, ctx->propq))
                goto err;
        } else {
...
OSSL_CMP_MSG* ossl_cmp_certrep_new(OSSL_CMP_CTX* ctx, int bodytype, int certReqId, const OSSL_CMP_PKISI* si, X509* cert, const X509* encryption_recip,
    STACK_OF(X509) * chain, STACK_OF(X509) * caPubs, int unprotectedErrors)
{
    OSSL_CMP_MSG* msg = NULL;
    ...
    if ((resp->status = OSSL_CMP_PKISI_dup(si)) == NULL || !ASN1_INTEGER_set(resp->certReqId, certReqId))
        goto err;

    status = ossl_cmp_pkisi_get_status(resp->status);
    if (status != OSSL_CMP_PKISTATUS_rejection && status != OSSL_CMP_PKISTATUS_waiting && cert != NULL) {
    ...
    if (bodytype == OSSL_CMP_PKIBODY_IP && caPubs != NULL && (repMsg->caPubs = X509_chain_up_ref(caPubs)) == NULL)
        goto err;
    if (sk_X509_num(chain) > 0 && !ossl_x509_add_certs_new(&msg->extraCerts, chain, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
        goto err;

    if (!unprotectedErrors || ossl_cmp_pkisi_get_status(si) != OSSL_CMP_PKISTATUS_rejection)
        if (!ossl_cmp_msg_protect(ctx, msg))
            goto err;
...
OSSL_CMP_MSG* ossl_cmp_rr_new(OSSL_CMP_CTX* ctx)
{
    ...
    ret = ctx->oldCert != NULL
        ? OSSL_CRMF_CERTTEMPLATE_fill(rd->certDetails, NULL /* pubkey would be redundant */, NULL /* subject would be redundant */,
            X509_get_issuer_name(ctx->oldCert), X509_get0_serialNumber(ctx->oldCert))
    ...
    if (ctx->revocationReason != CRL_REASON_NONE && !add_crl_reason_extension(&rd->crlEntryDetails, ctx->revocationReason))
        goto err;
...
OSSL_CMP_MSG* ossl_cmp_rp_new(OSSL_CMP_CTX* ctx, const OSSL_CMP_PKISI* si, const OSSL_CRMF_CERTID* cid, int unprotectedErrors)
{
...
int ossl_cmp_msg_gen_push1_ITAVs(OSSL_CMP_MSG* msg, const STACK_OF(OSSL_CMP_ITAV) * itavs)
{
    ...
    for (i = 0; i < sk_OSSL_CMP_ITAV_num(itavs); i++) {
        ...
        if (itav == NULL || !ossl_cmp_msg_gen_push0_ITAV(msg, itav)) {
 ...
 */
static OSSL_CMP_MSG* gen_new(OSSL_CMP_CTX* ctx, const STACK_OF(OSSL_CMP_ITAV) * itavs, int body_type, int err_code)
{
    OSSL_CMP_MSG* msg = NULL;
...
{
    return gen_new(ctx, ctx->genm_ITAVs, OSSL_CMP_PKIBODY_GENM, CMP_R_ERROR_CREATING_GENM);
}

OSSL_CMP_MSG* ossl_cmp_genp_new(OSSL_CMP_CTX* ctx, const STACK_OF(OSSL_CMP_ITAV) * itavs)
{
    return gen_new(ctx, itavs, OSSL_CMP_PKIBODY_GENP, CMP_R_ERROR_CREATING_GENP);
}

OSSL_CMP_MSG* ossl_cmp_error_new(OSSL_CMP_CTX* ctx, const OSSL_CMP_PKISI* si, int64_t errorCode, const char* details, int unprotected)
{
    OSSL_CMP_MSG* msg = NULL;
    ...
    if (!ASN1_INTEGER_set_int64(msg->body->value.error->errorCode, errorCode))
        goto err;
    if (errorCode > 0 && (uint64_t)errorCode < ((uint64_t)ERR_SYSTEM_FLAG << 1)) {
    ...
    if (lib != NULL || reason != NULL || details != NULL) {
        ...
        if (lib != NULL && *lib != '\0' && !ossl_cmp_sk_ASN1_UTF8STRING_push_str(ft, lib, -1))
            goto err;
        if (reason != NULL && *reason != '\0' && !ossl_cmp_sk_ASN1_UTF8STRING_push_str(ft, reason, -1))
            goto err;
        if (details != NULL && !ossl_cmp_sk_ASN1_UTF8STRING_push_str(ft, details, -1))
            goto err;
    }
 ...
 */
int ossl_cmp_certstatus_set0_certHash(OSSL_CMP_CERTSTATUS* certStatus, ASN1_OCTET_STRING* hash)
{
...
OSSL_CMP_MSG* ossl_cmp_certConf_new(OSSL_CMP_CTX* ctx, int certReqId, int fail_info, const char* text)
{
    OSSL_CMP_MSG* msg = NULL;
    ...
    if (!ossl_assert(ctx != NULL && ctx->newCert != NULL && (certReqId == OSSL_CMP_CERTREQID || certReqId == OSSL_CMP_CERTREQID_NONE)))
        return NULL;
     ...
     * the CA/RA.
     */
    sinfo = fail_info != 0 ? OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_rejection, fail_info, text)
...
OSSL_CMP_MSG* ossl_cmp_pollReq_new(OSSL_CMP_CTX* ctx, int crid)
{
    ...
    if ((preq = OSSL_CMP_POLLREQ_new()) == NULL || !ASN1_INTEGER_set(preq->certReqId, crid) || !sk_OSSL_CMP_POLLREQ_push(msg->body->value.pollReq, preq))
        goto err;
 ...
 */
OSSL_CMP_POLLREP* ossl_cmp_pollrepcontent_get0_pollrep(const OSSL_CMP_POLLREPCONTENT* prc, int rid)
{
    ...
    ERR_raise_data(ERR_LIB_CMP, CMP_R_CERTRESPONSE_NOT_FOUND, "expected certReqId = %d", rid);
    return NULL;
}
 ...
 */
OSSL_CMP_CERTRESPONSE* ossl_cmp_certrepmessage_get0_certresponse(const OSSL_CMP_CERTREPMESSAGE* crm, int rid)
{
...
int OSSL_CMP_MSG_update_recipNonce(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
    ...
    if (ctx->recipNonce == NULL) /* nothing to do for 1st msg in transaction */
        return 1;
    if (!ossl_cmp_asn1_octet_string_set1(&msg->header->recipNonce, ctx->recipNonce))
        return 0;
    return msg->header->protectionAlg == NULL || ossl_cmp_msg_protect(ctx, msg);
}
...
OSSL_CMP_MSG* OSSL_CMP_MSG_read(const char* file, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if ((bio = BIO_new_file(file, "rb")) == NULL || d2i_OSSL_CMP_MSG_bio(bio, &msg) == NULL) {
        OSSL_CMP_MSG_free(msg);
        msg = NULL;
    }
    BIO_free(bio);
...
OSSL_CMP_MSG* d2i_OSSL_CMP_MSG_bio(BIO* bio, OSSL_CMP_MSG** msg)
{
    ...
    return ASN1_item_d2i_bio_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), bio, msg, libctx, propq);
}
