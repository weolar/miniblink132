...
static int verify_signature(const OSSL_CMP_CTX* cmp_ctx, const OSSL_CMP_MSG* msg, X509* cert)
{
    ...
    if (!cmp_ctx->ignore_keyusage && (X509_get_key_usage(cert) & X509v3_KU_DIGITAL_SIGNATURE) == 0) {
    ...
    if (ASN1_item_verify_ex(
            ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), msg->header->protectionAlg, msg->protection, &prot_part, NULL, pubkey, cmp_ctx->libctx, cmp_ctx->propq)
        > 0) {
        res = 1;
        goto end;
    }
...
static int verify_PBMAC(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg)
{
    ...
    valid = msg->protection != NULL && msg->protection->length >= 0 && msg->protection->type == protection->type
        && msg->protection->length == protection->length && CRYPTO_memcmp(msg->protection->data, protection->data, protection->length) == 0;
...
int OSSL_CMP_validate_cert_path(const OSSL_CMP_CTX* ctx, X509_STORE* trusted_store, X509* cert)
{
    ...
    if ((csc = X509_STORE_CTX_new_ex(ctx->libctx, ctx->propq)) == NULL || !X509_STORE_CTX_init(csc, trusted_store, cert, ctx->untrusted))
        goto err;
...
static int check_name(
    const OSSL_CMP_CTX* ctx, int log_success, const char* actual_desc, const X509_NAME* actual_name, const char* expect_desc, const X509_NAME* expect_name)
{
    char* str;
    ...
    const STACK_OF(X509) * already_checked2, const OSSL_CMP_MSG* msg)
{
    ...
    if (already_checked(cert, already_checked1) || already_checked(cert, already_checked2)) {
        ossl_cmp_info(ctx, " cert has already been checked");
        return 0;
    }

    time_cmp = X509_cmp_timeframe(vpm, X509_get0_notBefore(cert), X509_get0_notAfter(cert));
    if (time_cmp != 0) {
 ...
 */
static int check_msg_with_certs(OSSL_CMP_CTX* ctx, const STACK_OF(X509) * certs, const char* desc, const STACK_OF(X509) * already_checked1,
    const STACK_OF(X509) * already_checked2, const OSSL_CMP_MSG* msg, int mode_3gpp)
{
    ...
    for (i = 0; i < sk_X509_num(certs); i++) { /* certs may be NULL */
        ...
        if (!ossl_assert(cert != NULL))
            return 0;
        if (!cert_acceptable(ctx, "cert from", desc, cert, already_checked1, already_checked2, msg))
            continue;
        n_acceptable_certs++;
        if (mode_3gpp ? check_cert_path_3gpp(ctx, msg, cert) : check_cert_path(ctx, ctx->trusted, cert)) {
...
static int check_msg_all_certs(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg, int mode_3gpp)
{
    ...
    if (ctx->trusted == NULL) {
        ...
        ret = check_msg_with_certs(
            ctx, trusted, mode_3gpp ? "self-issued extraCerts" : "certs in trusted store", msg->extraCerts, ctx->untrusted, msg, mode_3gpp);
...
int OSSL_CMP_validate_msg(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg)
{
    ...
    if (ctx == NULL || msg == NULL || msg->header == NULL || msg->body == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
        return 0;
    }
    ...
    default:
        ...
        if (scrt == NULL) {
            ...
            if (verify_signature(ctx, msg, scrt)) {
                ossl_cmp_debug(ctx, "sucessfully validated signature-based CMP message protection");
...
int ossl_cmp_msg_check_update(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg, ossl_cmp_allow_unprotected_cb_t cb, int cb_arg)
{
    ...
    if (!check_name(ctx, 0, "sender DN field", hdr->sender->d.directoryName, "expected sender", expected_sender))
        return 0;
    /* Note: if recipient was NULL-DN it could be learned here if needed */

    if (sk_X509_num(msg->extraCerts) > 10)
        ossl_cmp_warn(ctx, "received CMP message contains more than 10 extraCerts");
    /*
    ...
    if (hdr->protectionAlg != NULL) {
        ...
        if (!OSSL_CMP_validate_msg(ctx, msg) && (cb == NULL || (*cb)(ctx, msg, 1, cb_arg) <= 0)) {
...
int ossl_cmp_verify_popo(const OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg, int acceptRAVerified)
{
    ...
    case OSSL_CMP_PKIBODY_P10CR: {
        ...
        if (X509_REQ_verify_ex(req, X509_REQ_get0_pubkey(req), ctx->libctx, ctx->propq) <= 0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    ...
    case OSSL_CMP_PKIBODY_KUR:
        if (!OSSL_CRMF_MSGS_verify_popo(msg->body->value.ir, OSSL_CMP_CERTREQID, acceptRAVerified, ctx->libctx, ctx->propq)) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
            return 0;
#endif
        }
