 ...
 */
static int unprotected_exception(const OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* rep, int invalid_protection, int expected_type /* ignored here */)
{
...
{
    int begin_transaction = expected_type != OSSL_CMP_PKIBODY_POLLREP && expected_type != OSSL_CMP_PKIBODY_PKICONF;
    ...
    *rep = (*transfer_cb)(ctx, req);
    ...
    if (*rep == NULL) {
        ERR_raise_data(ERR_LIB_CMP, ctx->total_timeout != 0 && time(NULL) >= ctx->end_time ? CMP_R_TOTAL_TIMEOUT : CMP_R_TRANSFER_ERROR,
            "request sent: %s, expected response: %s", req_type_str, expected_type_str);
        return 0;
    }
    ...
    if (bt != OSSL_CMP_PKIBODY_POLLREP && bt != OSSL_CMP_PKIBODY_PKICONF && !ossl_cmp_ctx_set1_extraCertsIn(ctx, (*rep)->extraCerts))
        return 0;
    ...
    if (bt != OSSL_CMP_PKIBODY_ERROR) {
        ...
        if (emc->errorCode != NULL && BIO_snprintf(buf, sizeof(buf), "; errorCode: %08lX", ASN1_INTEGER_get(emc->errorCode)) > 0)
            ERR_add_error_data(1, buf);
...
int ossl_cmp_exchange_error(OSSL_CMP_CTX* ctx, int status, int fail_info, const char* txt, int errorCode, const char* details)
{
...
static X509* get1_cert_status(OSSL_CMP_CTX* ctx, int bodytype, OSSL_CMP_CERTRESPONSE* crep)
{
    ...
    case OSSL_CMP_PKISTATUS_waiting:
        ossl_cmp_err(ctx, "received \"waiting\" status for cert when actually aiming to extract cert");
    ...
    case OSSL_CMP_PKISTATUS_revocationWarning:
        ossl_cmp_warn(ctx, "received \"revocationWarning\" - a revocation of the cert is imminent");
        break;
    case OSSL_CMP_PKISTATUS_revocationNotification:
        ossl_cmp_warn(ctx, "received \"revocationNotification\" - a revocation of the cert has occurred");
        break;
    case OSSL_CMP_PKISTATUS_keyUpdateWarning:
        ...
        break;
    default:
        ossl_cmp_log1(ERROR, ctx, "received unsupported PKIStatus %d for certificate", ctx->status);
...
int OSSL_CMP_certConf_cb(OSSL_CMP_CTX* ctx, X509* cert, int fail_info, const char** text)
{
    ...
    if (out_trusted == NULL) {
        ...
        chain = X509_build_chain(cert, ctx->untrusted, out_trusted, 0, ctx->libctx, ctx->propq);
    } else {
        ...
        X509_VERIFY_PARAM_clear_flags(
            X509_STORE_CTX_get0_param(csc), ~(X509_V_FLAG_USE_CHECK_TIME | X509_V_FLAG_NO_CHECK_TIME | X509_V_FLAG_PARTIAL_CHAIN | X509_V_FLAG_POLICY_CHECK));
        if (X509_verify_cert(csc) <= 0)
            goto err;

        if (!ossl_x509_add_certs_new(&chain, X509_STORE_CTX_get0_chain(csc), X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP | X509_ADD_FLAG_NO_SS)) {
            sk_X509_free(chain);
            chain = NULL;
        }
    err:
    ...
    if (out_trusted != NULL) {
        if (chain == NULL) {
            ...
            ossl_cmp_debug(ctx, "success validating newly enrolled cert");
        }
    } else if (chain == NULL) {
        ...
        ossl_cmp_debug(ctx, "success building approximate chain for newly enrolled cert");
    }
 ...
 */
static int cert_response(OSSL_CMP_CTX* ctx, int sleep, int rid, OSSL_CMP_MSG** resp, int* checkAfter, int req_type, int expected_type)
{
...
retry:
     ...
     */
    if (crepmsg->caPubs != NULL && !ossl_cmp_ctx_set1_caPubs(ctx, crepmsg->caPubs))
        return 0;
...
static int initial_certreq(OSSL_CMP_CTX* ctx, int req_type, const OSSL_CRMF_MSG* crm, OSSL_CMP_MSG** p_rep, int rep_type)
{
    OSSL_CMP_MSG* req;
    int res;
...
int OSSL_CMP_try_certreq(OSSL_CMP_CTX* ctx, int req_type, const OSSL_CRMF_MSG* crm, int* checkAfter)
{
    ...
    if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
        ...
        if (req_type < 0)
            return ossl_cmp_exchange_error(ctx, OSSL_CMP_PKISTATUS_rejection, 0, "polling aborted", 0 /* errorCode */, "by application");
        res = poll_for_response(ctx, 0 /* no sleep */, rid, &rep, checkAfter);
        if (res <= 0) /* waiting or error */
            return res;
    }
    res = cert_response(ctx, 0 /* no sleep */, rid, &rep, checkAfter, req_type, rep_type);
...
X509* OSSL_CMP_exec_certreq(OSSL_CMP_CTX* ctx, int req_type, const OSSL_CRMF_MSG* crm)
{
    ...
    if (cert_response(ctx, 1 /* sleep */, rid, &rep, NULL, req_type, rep_type) <= 0)
        goto err;
...
int OSSL_CMP_exec_RR_ses(OSSL_CMP_CTX* ctx)
{
    ...
    case OSSL_CMP_PKISTATUS_revocationNotification:
        ...
        ossl_cmp_warn(ctx, "revocation accepted (PKIStatus=revocationNotification)");
        ret = 1;
        break;
    case OSSL_CMP_PKISTATUS_waiting:
    case OSSL_CMP_PKISTATUS_keyUpdateWarning:
    ...
    if (sk_OSSL_CRMF_CERTID_num(rrep->revCerts) >= 1) {
        ...
        if (ASN1_INTEGER_cmp(serial, OSSL_CRMF_CERTID_get0_serialNumber(cid)) != 0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
