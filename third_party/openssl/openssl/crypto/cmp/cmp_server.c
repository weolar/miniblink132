...
int OSSL_CMP_SRV_CTX_init(OSSL_CMP_SRV_CTX* srv_ctx, void* custom_ctx, OSSL_CMP_SRV_cert_request_cb_t process_cert_request, OSSL_CMP_SRV_rr_cb_t process_rr,
    OSSL_CMP_SRV_genm_cb_t process_genm, OSSL_CMP_SRV_error_cb_t process_error, OSSL_CMP_SRV_certConf_cb_t process_certConf,
    OSSL_CMP_SRV_pollReq_cb_t process_pollReq)
{
    if (srv_ctx == NULL) {
 ...
 * incoming message
 */
static OSSL_CMP_MSG* process_genm(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
...
static OSSL_CMP_MSG* process_error(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
    ...
    srv_ctx->process_error(srv_ctx, req, errorContent->pKIStatusInfo, errorContent->errorCode, errorContent->errorDetails);
...
static OSSL_CMP_MSG* process_certConf(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
    OSSL_CMP_CTX* ctx;
    ...
    if (OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_IMPLICIT_CONFIRM) == 1 || ctx->status != OSSL_CMP_PKISTATUS_trans) {
...
static OSSL_CMP_MSG* process_pollReq(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
    ...
    if (!srv_ctx->process_pollReq(srv_ctx, req, certReqId, &certReq, &check_after))
        return NULL;
 ...
 */
static int unprotected_exception(const OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* req, int invalid_protection, int accept_unprotected_requests)
{
    if (!ossl_assert(ctx != NULL && req != NULL))
        return -1;

    if (accept_unprotected_requests) {
        ossl_cmp_log1(WARN, ctx, "ignoring %s protection of request message", invalid_protection ? "invalid" : "missing");
        return 1;
    }
    if (OSSL_CMP_MSG_get_bodytype(req) == OSSL_CMP_PKIBODY_ERROR && OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_UNPROTECTED_ERRORS) == 1) {
...
OSSL_CMP_MSG* OSSL_CMP_SRV_process_request(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
    ...
    if (srv_ctx == NULL || srv_ctx->ctx == NULL || req == NULL || req->body == NULL || (hdr = OSSL_CMP_MSG_get0_header(req)) == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
        return 0;
    }
    ctx = srv_ctx->ctx;
    ...
    case OSSL_CMP_PKIBODY_ERROR:
        if (ctx->transactionID != NULL) {
            ...
            tid = OPENSSL_buf2hexstr(ctx->transactionID->data, ctx->transactionID->length);
            if (tid != NULL)
                ossl_cmp_log1(WARN, ctx, "Assuming that last transaction with ID=%s got aborted", tid);
            OPENSSL_free(tid);
        }
        /* start of a new transaction, reset transactionID and senderNonce */
        if (!OSSL_CMP_CTX_set1_transactionID(ctx, NULL) || !OSSL_CMP_CTX_set1_senderNonce(ctx, NULL))
            goto err;
        break;
    default:
    ...
    req_verified = ossl_cmp_msg_check_update(ctx, req, unprotected_exception, srv_ctx->acceptUnprotected);
    if (ctx->secretValue != NULL && ctx->pkey != NULL && ossl_cmp_hdr_get_protection_nid(hdr) != NID_id_PasswordBasedMAC)
...
err:
    if (rsp == NULL) {
        ...
        if ((si = OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_rejection, fail_info, reason)) != NULL) {
