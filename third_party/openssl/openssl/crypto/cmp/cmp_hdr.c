...
int ossl_cmp_hdr_init(OSSL_CMP_CTX* ctx, OSSL_CMP_PKIHEADER* hdr)
{
     ...
     */
    sender = ctx->cert != NULL ? X509_get_subject_name(ctx->cert) : ctx->oldCert != NULL ? X509_get_subject_name(ctx->oldCert) : ctx->subjectName;
    ...
    if (ctx->recipNonce != NULL && !ossl_cmp_asn1_octet_string_set1(&hdr->recipNonce, ctx->recipNonce))
        return 0;
