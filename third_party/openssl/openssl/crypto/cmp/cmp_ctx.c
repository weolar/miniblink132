...
int OSSL_CMP_CTX_set1_untrusted(OSSL_CMP_CTX* ctx, STACK_OF(X509) * certs)
{
    ...
    if (!ossl_x509_add_certs_new(&untrusted, certs, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
        goto err;
...
#define OSSL_CMP_ITAVs_free(itavs) sk_OSSL_CMP_ITAV_pop_free(itavs, OSSL_CMP_ITAV_free);
#define X509_EXTENSIONS_free(exts) sk_X509_EXTENSION_pop_free(exts, X509_EXTENSION_free)
#define OSSL_CMP_PKIFREETEXT_free(text) sk_ASN1_UTF8STRING_pop_free(text, ASN1_UTF8STRING_free)
...
int OSSL_CMP_CTX_reinit(OSSL_CMP_CTX* ctx)
{
    ...
    return ossl_cmp_ctx_set0_statusString(ctx, NULL) && ossl_cmp_ctx_set0_newCert(ctx, NULL) && ossl_cmp_ctx_set1_newChain(ctx, NULL)
        && ossl_cmp_ctx_set1_caPubs(ctx, NULL) && ossl_cmp_ctx_set1_extraCertsIn(ctx, NULL) && ossl_cmp_ctx_set0_validatedSrvCert(ctx, NULL)
        && OSSL_CMP_CTX_set1_transactionID(ctx, NULL) && OSSL_CMP_CTX_set1_senderNonce(ctx, NULL) && ossl_cmp_ctx_set1_recipNonce(ctx, NULL);
}
...
static size_t ossl_cmp_log_trace_cb(const char* buf, size_t cnt, int category, int cmd, void* vdata)
{
    ...
    if (!ctx->log_cb(func != NULL ? func : "(no func)", file != NULL ? file : "(no file)", line, level, msg))
        cnt = 0;
    ...
    OSSL_CMP_severity level, const OSSL_CMP_CTX* ctx, const char* func, const char* file, int line, const char* level_str, const char* format, ...)
{
    ...
    if (OSSL_TRACE_ENABLED(CMP)) {
        ...
        {
            int printed = BIO_snprintf(hugebuf, sizeof(hugebuf), "%s:%s:%d:" OSSL_CMP_LOG_PREFIX "%s: ", func, file, line, level_str);
            if (printed > 0 && (size_t)printed < sizeof(hugebuf)) {
                if (BIO_vsnprintf(hugebuf + printed, sizeof(hugebuf) - printed, format, args) > 0)
...
int OSSL_CMP_CTX_set_log_cb(OSSL_CMP_CTX* ctx, OSSL_CMP_log_cb_t cb)
{
    ...
    if (!OSSL_trace_set_callback(OSSL_TRACE_CATEGORY_CMP, ossl_cmp_log_trace_cb, ctx))
        return 0;
#endif
    ...
    int OSSL_CMP_CTX_set0_reqExtensions(OSSL_CMP_CTX* ctx, X509_EXTENSIONS* exts)
{
    ...
    if (sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 && exts != NULL && X509v3_get_ext_by_NID(exts, NID_subject_alt_name, -1) >= 0) {
...
int OSSL_CMP_CTX_reqExtensions_have_SAN(OSSL_CMP_CTX* ctx)
{
    ...
    return ctx->reqExtensions != NULL && X509v3_get_ext_by_NID(ctx->reqExtensions, NID_subject_alt_name, -1) >= 0;
}
...
int OSSL_CMP_CTX_push1_subjectAltName(OSSL_CMP_CTX* ctx, const GENERAL_NAME* name)
{
    ...
    if (ctx->subjectAltNames == NULL && (ctx->subjectAltNames = sk_GENERAL_NAME_new_null()) == NULL)
        return 0;
    ...
    int OSSL_CMP_CTX_build_cert_chain(OSSL_CMP_CTX* ctx, X509_STORE* own_trusted, STACK_OF(X509) * candidates)
{
    ...
    if (!ossl_x509_add_certs_new(&ctx->untrusted, candidates, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
        return 0;

    ossl_cmp_debug(ctx, "trying to build chain for own CMP signer cert");
    chain = X509_build_chain(ctx->cert, ctx->untrusted, own_trusted, 0, ctx->libctx, ctx->propq);
    if (chain == NULL) {
