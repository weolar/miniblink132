 ...
 */
static char* snprint_PKIStatusInfo_parts(int status, int fail_info, const OSSL_CMP_PKIFREETEXT* status_strings, char* buf, size_t bufsize)
{
    int failure;
...
#define ADVANCE_BUFFER                                                                                                                                         \
        ...
        for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX; failure++) {
            if ((fail_info & (1 << failure)) != 0) {
                ...
                if (failure_string != NULL) {
                    printed_chars = BIO_snprintf(write_ptr, bufsize, "%s%s", failinfo_found ? ", " : "", failure_string);
                    ADVANCE_BUFFER;
                    failinfo_found = 1;
                }
            }
    ...
    if (!failinfo_found && status != OSSL_CMP_PKISTATUS_accepted && status != OSSL_CMP_PKISTATUS_grantedWithMods) {
    ...
    if (n_status_strings > 0) {
        printed_chars = BIO_snprintf(write_ptr, bufsize, "; StatusString%s: ", n_status_strings > 1 ? "s" : "");
        ADVANCE_BUFFER;
        for (i = 0; i < n_status_strings; i++) {
            text = sk_ASN1_UTF8STRING_value(status_strings, i);
            printed_chars
                = BIO_snprintf(write_ptr, bufsize, "\"%.*s\"%s", ASN1_STRING_length(text), ASN1_STRING_get0_data(text), i < n_status_strings - 1 ? ", " : "");
            ADVANCE_BUFFER;
        }
    }
...
char* OSSL_CMP_snprint_PKIStatusInfo(const OSSL_CMP_PKISI* statusInfo, char* buf, size_t bufsize)
{
    ...
    return snprint_PKIStatusInfo_parts(ASN1_INTEGER_get(statusInfo->status), failure_info, statusInfo->statusString, buf, bufsize);
}
...
char* OSSL_CMP_CTX_snprint_PKIStatus(const OSSL_CMP_CTX* ctx, char* buf, size_t bufsize)
{
    ...
    return snprint_PKIStatusInfo_parts(OSSL_CMP_CTX_get_status(ctx), OSSL_CMP_CTX_get_failInfoCode(ctx), OSSL_CMP_CTX_get0_statusString(ctx), buf, bufsize);
}
...
OSSL_CMP_PKISI* OSSL_CMP_STATUSINFO_new(int status, int fail_info, const char* text)
{
    ...
    if (text != NULL) {
        if ((utf8_text = ASN1_UTF8STRING_new()) == NULL || !ASN1_STRING_set(utf8_text, text, -1))
            goto err;
    ...
    for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX; failure++) {
        if ((fail_info & (1 << failure)) != 0) {
            if (si->failInfo == NULL && (si->failInfo = ASN1_BIT_STRING_new()) == NULL)
                goto err;
