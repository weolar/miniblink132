...
ASN1_BIT_STRING* ossl_cmp_calc_protection(const OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg)
{
    ...
    if (OBJ_obj2nid(algorOID) == NID_id_PasswordBasedMAC) {
        ...
        if (!OSSL_CRMF_pbm_new(
                ctx->libctx, ctx->propq, pbm, prot_part_der, prot_part_der_len, ctx->secretValue->data, ctx->secretValue->length, &protection, &sig_len))
            goto end;
        ...
        if (!OBJ_find_sigid_algs(OBJ_obj2nid(algorOID), &md_nid, NULL) || (md = EVP_get_digestbynid(md_nid)) == NULL) {
            ERR_raise(ERR_LIB_CMP, CMP_R_UNKNOWN_ALGORITHM_ID);
            return NULL;
        }
        ...
        if ((prot = ASN1_BIT_STRING_new()) == NULL)
            return NULL;
        if (ASN1_item_sign_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), NULL, NULL, prot, &prot_part, NULL, ctx->pkey, md, ctx->libctx, ctx->propq))
            return prot;
        ASN1_BIT_STRING_free(prot);
        return NULL;
    }
...
int ossl_cmp_msg_add_extraCerts(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
    ...
    if (!ossl_x509_add_certs_new(&msg->extraCerts, ctx->extraCertsOut, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
        return 0;
...
static int set_pbmac_algor(const OSSL_CMP_CTX* ctx, X509_ALGOR** alg)
{
    ...
    pbm = OSSL_CRMF_pbmp_new(ctx->libctx, ctx->pbm_slen, EVP_MD_get_type(ctx->pbm_owf), ctx->pbm_itercnt, ctx->pbm_mac);
    ...
    X509_ALGOR_set0(*alg, OBJ_nid2obj(NID_id_PasswordBasedMAC), V_ASN1_SEQUENCE, pbm_str);
    OSSL_CRMF_PBMPARAMETER_free(pbm);
    return 1;
...
static int set_sig_algor(const OSSL_CMP_CTX* ctx, X509_ALGOR** alg)
{
    ...
    if (!OBJ_find_sigid_by_algs(&nid, EVP_MD_get_type(ctx->digest), EVP_PKEY_get_id(ctx->pkey))) {
...
static int set_senderKID(const OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg, const ASN1_OCTET_STRING* id)
{
    if (id == NULL)
...
int ossl_cmp_msg_protect(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
    ...
    if (!ctx->unprotectedSend && ((msg->protection = ossl_cmp_calc_protection(ctx, msg)) == NULL))
        goto err;
