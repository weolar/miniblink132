...
int ossl_pw_set_passphrase(struct ossl_passphrase_data_st* data, const unsigned char* passphrase, size_t passphrase_len)
{
    ...
    data->_.expl_passphrase.passphrase_copy = passphrase_len != 0 ? OPENSSL_memdup(passphrase, passphrase_len) : OPENSSL_malloc(1);
...
static int do_ui_passphrase(char* pass, size_t pass_size, size_t* pass_len, const char* prompt_info, int verify, const UI_METHOD* ui_method, void* ui_data)
{
    ...
    prompt_idx = UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, ipass, 0, pass_size) - 1;
    if (prompt_idx < 0) {
    ...
    if (verify) {
        ...
        verify_idx = UI_add_verify_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, vpass, 0, pass_size, ipass) - 1;
        if (verify_idx < 0) {
            ERR_raise(ERR_LIB_CRYPTO, ERR_R_UI_LIB);
            goto end;
        }
...
int ossl_pw_get_passphrase(char* pass, size_t pass_size, size_t* pass_len, const OSSL_PARAM params[], int verify, struct ossl_passphrase_data_st* data)
{
    ...
    if (ui_method == NULL) {
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT, "No password method specified");
        return 0;
    }
...
do_cache:
    if (ret && data->flag_cache_passphrase) {
        if (data->cached_passphrase == NULL || *pass_len > data->cached_passphrase_len) {
            void* new_cache = OPENSSL_clear_realloc(data->cached_passphrase, data->cached_passphrase_len, *pass_len + 1);
...
static int ossl_pw_get_password(char* buf, int size, int rwflag, void* userdata, const char* info)
{
    size_t password_len = 0;
    OSSL_PARAM params[] = { OSSL_PARAM_utf8_string(OSSL_PASSPHRASE_PARAM_INFO, NULL, 0), OSSL_PARAM_END };
