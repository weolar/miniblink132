...
int ossl_ecx_public_from_private(ECX_KEY* key)
{
    ...
    case ECX_KEY_TYPE_ED25519:
        if (!ossl_ed25519_public_from_private(key->libctx, key->pubkey, key->privkey, key->propq)) {
    ...
    case ECX_KEY_TYPE_ED448:
        if (!ossl_ed448_public_from_private(key->libctx, key->pubkey, key->privkey, key->propq)) {
            ERR_raise(ERR_LIB_EC, EC_R_FAILED_MAKING_PUBLIC_KEY);
            return 0;
        }
        break;
...
int ossl_ecx_key_fromdata(ECX_KEY* ecx, const OSSL_PARAM params[], int include_private)
{
    ...
    if (param_priv_key != NULL) {
        if (!OSSL_PARAM_get_octet_string(param_priv_key, (void**)&ecx->privkey, ecx->keylen, &privkeylen))
            return 0;
    ...
    pubkey = ecx->pubkey;
    if (param_pub_key != NULL && !OSSL_PARAM_get_octet_string(param_pub_key, (void**)&pubkey, sizeof(ecx->pubkey), &pubkeylen))
        return 0;
...
#ifndef FIPS_MODULE
ECX_KEY* ossl_ecx_key_op(const X509_ALGOR* palg, const unsigned char* p, int plen, int id, ecx_key_op_t op, OSSL_LIB_CTX* libctx, const char* propq)
{
    ECX_KEY* key = NULL;
...
ECX_KEY* ossl_ecx_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf, OSSL_LIB_CTX* libctx, const char* propq)
{
     ...
     * on its own.
     */
    ecx = ossl_ecx_key_op(palg, p, plen, EVP_PKEY_NONE, KEY_OP_PRIVATE, libctx, propq);
