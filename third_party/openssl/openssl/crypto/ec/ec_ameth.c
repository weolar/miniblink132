...
static int eckey_pub_decode(EVP_PKEY* pkey, const X509_PUBKEY* pubkey)
{
    ...
    if (!ossl_x509_PUBKEY_get0_libctx(&libctx, &propq, pubkey) || !X509_PUBKEY_get0_param(NULL, &p, &pklen, &palg, pubkey))
        return 0;
...
static int eckey_priv_decode_ex(EVP_PKEY* pkey, const PKCS8_PRIV_KEY_INFO* p8, OSSL_LIB_CTX* libctx, const char* propq)
{
    int ret = 0;
...
static int ec_pkey_export_to(const EVP_PKEY* from, void* to_keydata, OSSL_FUNC_keymgmt_import_fn* importer, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (pub_point != NULL) {
        ...
        if ((pub_key_buflen = EC_POINT_point2buf(ecg, pub_point, format, &pub_key_buf, bnctx)) == 0
            || !OSSL_PARAM_BLD_push_octet_string(tmpl, OSSL_PKEY_PARAM_PUB_KEY, pub_key_buf, pub_key_buflen))
            goto err;
...
static int ec_pkey_import_from(const OSSL_PARAM params[], void* vpctx)
{
    ...
    if (!ossl_ec_group_fromdata(ec, params) || !ossl_ec_key_otherparams_fromdata(ec, params) || !ossl_ec_key_fromdata(ec, params, 1)
        || !EVP_PKEY_assign_EC_KEY(pkey, ec)) {
        EC_KEY_free(ec);
        return 0;
    }
...
const EVP_PKEY_ASN1_METHOD ossl_eckey_asn1_meth = { EVP_PKEY_EC, EVP_PKEY_EC, 0, "EC", "OpenSSL EC algorithm",

    eckey_pub_decode, eckey_pub_encode, eckey_pub_cmp, eckey_pub_print,

    NULL, eckey_priv_encode, eckey_priv_print,

    int_ec_size, ec_bits, ec_security_bits,

    eckey_param_decode, eckey_param_encode, ec_missing_parameters, ec_copy_parameters, ec_cmp_parameters, eckey_param_print, 0,

    int_ec_free, ec_pkey_ctrl, old_ec_priv_decode, old_ec_priv_encode,

    0, 0, 0,

    ec_pkey_check, ec_pkey_public_check, ec_pkey_param_check,
