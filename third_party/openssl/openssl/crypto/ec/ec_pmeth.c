...
static int pkey_ec_verify(EVP_PKEY_CTX* ctx, const unsigned char* sig, size_t siglen, const unsigned char* tbs, size_t tbslen)
{
    int ret, type;
    EC_PKEY_CTX* dctx = ctx->data;
    /*
...
static int pkey_ec_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
    ...
    eckey = dctx->co_key ? dctx->co_key : (EC_KEY*)EVP_PKEY_get0_EC_KEY(ctx->pkey);
...
static int pkey_ec_kdf_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
    EC_PKEY_CTX* dctx = ctx->data;
    ...
    if (!pkey_ec_derive(ctx, ktmp, &ktmplen))
        goto err;
    /* Do KDF stuff */
    if (!ossl_ecdh_kdf_X9_63(key, *keylen, ktmp, ktmplen, dctx->kdf_ukm, dctx->kdf_ukmlen, dctx->kdf_md, ctx->libctx, ctx->propquery))
        goto err;
    rv = 1;
...
static int pkey_ec_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2)
{
    ...
    case EVP_PKEY_CTRL_MD:
        if (EVP_MD_get_type((const EVP_MD*)p2) != NID_sha1 && EVP_MD_get_type((const EVP_MD*)p2) != NID_ecdsa_with_SHA1
            ...
            && EVP_MD_get_type((const EVP_MD*)p2) != NID_sha3_224 && EVP_MD_get_type((const EVP_MD*)p2) != NID_sha3_256
...
static int pkey_ec_paramgen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
    ...
    if (ec == NULL)
        return 0;
    if (!(ret = EC_KEY_set_group(ec, dctx->gen_group)) || !ossl_assert(ret = EVP_PKEY_assign_EC_KEY(pkey, ec)))
        EC_KEY_free(ec);
    return ret;
}
...
static const EVP_PKEY_METHOD ec_pkey_meth = { EVP_PKEY_EC, 0, pkey_ec_init, pkey_ec_copy, pkey_ec_cleanup,
