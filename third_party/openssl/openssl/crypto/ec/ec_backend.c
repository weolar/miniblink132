...
static int ec_group_explicit_todata(const EC_GROUP* group, OSSL_PARAM_BLD* tmpl, OSSL_PARAM params[], BN_CTX* bnctx, unsigned char** genbuf)
{
    int ret = 0, fid;
    ...
    if (tmpl != NULL || param_p != NULL || param_a != NULL || param_b != NULL) {
        ...
        if (!ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_P, p) || !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_A, a)
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (!ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_ORDER, order)) {
            ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
            goto err;
        }
    }
    ...
    if (tmpl != NULL || param != NULL) {
        if (!ossl_param_build_set_utf8_string(tmpl, params, OSSL_PKEY_PARAM_EC_FIELD_TYPE, field_type)) {
            ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
            goto err;
        }
    }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (!ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_EC_GENERATOR, *genbuf, genbuf_len)) {
            ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
            goto err;
        }
    }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (cofactor != NULL && !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_COFACTOR, cofactor)) {
            ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
            goto err;
        }
    }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (seed != NULL && seed_len > 0 && !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_EC_SEED, seed, seed_len)) {
            ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
            goto err;
        }
    }
...
int ossl_ec_group_todata(
    const EC_GROUP* group, OSSL_PARAM_BLD* tmpl, OSSL_PARAM params[], OSSL_LIB_CTX* libctx, const char* propq, BN_CTX* bnctx, unsigned char** genbuf)
{
    ...
    if (!ossl_param_build_set_int(tmpl, params, OSSL_PKEY_PARAM_EC_DECODED_FROM_EXPLICIT_PARAMS, group->decoded_from_explicit_params))
        return 0;
...
int ossl_ec_key_fromdata(EC_KEY* ec, const OSSL_PARAM params[], int include_private)
{
    ...
    if (param_pub_key != NULL)
        if (!OSSL_PARAM_get_octet_string(param_pub_key, (void**)&pub_key, 0, &pub_key_len) || (pub_point = EC_POINT_new(ecg)) == NULL
...
EC_KEY* ossl_ec_key_dup(const EC_KEY* src, int selection)
{
    ...
    /* copy the parameters */
    if (src->group != NULL && (selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
    ...
    /*  copy the public key */
    if (src->pub_key != NULL && (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
        if (ret->group == NULL)
