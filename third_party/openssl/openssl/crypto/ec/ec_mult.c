...
int ossl_ec_scalar_mul_ladder(const EC_GROUP* group, EC_POINT* r, const BIGNUM* scalar, const EC_POINT* point, BN_CTX* ctx)
{
    ...
    if (((p = EC_POINT_new(group)) == NULL) || ((s = EC_POINT_new(group)) == NULL)) {
        ERR_raise(ERR_LIB_EC, ERR_R_MALLOC_FAILURE);
        goto err;
    }
 ...
 */
#define EC_window_bits_for_scalar_size(b) ((size_t)((b) >= 2000 ? 6 : (b) >= 800 ? 5 : (b) >= 300 ? 4 : (b) >= 70 ? 3 : (b) >= 20 ? 2 : 1))
 ...
 */
int ossl_ec_wNAF_mul(const EC_GROUP* group, EC_POINT* r, const BIGNUM* scalar, size_t num, const EC_POINT* points[], const BIGNUM* scalars[], BN_CTX* ctx)
{
    ...
    if (group->meth->points_make_affine == NULL || !group->meth->points_make_affine(group, num_val, val, ctx))
        goto err;
...
int ossl_ec_wNAF_precompute_mult(EC_GROUP* group, BN_CTX* ctx)
{
    ...
    if (group->meth->points_make_affine == NULL || !group->meth->points_make_affine(group, num, points, ctx))
        goto err;
