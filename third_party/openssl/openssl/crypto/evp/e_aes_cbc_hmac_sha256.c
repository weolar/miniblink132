...
static int aesni_cbc_hmac_sha256_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* inkey, const unsigned char* iv, int enc)
{
    ...
    if (enc)
        ret = aesni_set_encrypt_key(inkey, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &key->ks);
    else
        ret = aesni_set_decrypt_key(inkey, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &key->ks);
...
static int aesni_cbc_hmac_sha256_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
        ...
        if (((OPENSSL_ia32cap_P[2] & (1 << 29)) || /* SHAEXT? */
            ...
            (void)aesni_cbc_sha256_enc(in, out, blocks, &key->ks, ctx->iv, &key->md, in + iv + sha_off);
        ...
        if (plen != len) { /* "TLS" mode of operation */
            ...
            aesni_cbc_encrypt(out + aes_off, out + aes_off, len - aes_off, &key->ks, ctx->iv, 1);
        } else {
            aesni_cbc_encrypt(in + aes_off, out + aes_off, len - aes_off, &key->ks, ctx->iv, 1);
        }
    } else {
        union {
        ...
        if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
...
#if 1 /* see original reference version in #else */
            {
                ...
                for (res = 0, i = 0, j = 0; j < maxpad + SHA256_DIGEST_LENGTH; j++) {
                    c = p[j];
...
static int aesni_cbc_hmac_sha256_ctrl(EVP_CIPHER_CTX* ctx, int type, int arg, void* ptr)
{
    ...
    case EVP_CTRL_AEAD_TLS1_AAD: {
        ...
        if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
            ...
            return (int)(((len + SHA256_DIGEST_LENGTH + AES_BLOCK_SIZE) & -AES_BLOCK_SIZE) - len);
        } else {
...
static EVP_CIPHER aesni_128_cbc_hmac_sha256_cipher = {
    ...
    NID_undef,
#endif
    AES_BLOCK_SIZE, 16, AES_BLOCK_SIZE, EVP_CIPH_CBC_MODE | EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_FLAG_AEAD_CIPHER | EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK,
    EVP_ORIG_GLOBAL, aesni_cbc_hmac_sha256_init_key, aesni_cbc_hmac_sha256_cipher, NULL, sizeof(EVP_AES_HMAC_SHA256),
    EVP_CIPH_FLAG_DEFAULT_ASN1 ? NULL : EVP_CIPHER_set_asn1_iv, EVP_CIPH_FLAG_DEFAULT_ASN1 ? NULL : EVP_CIPHER_get_asn1_iv, aesni_cbc_hmac_sha256_ctrl, NULL
};
...
static EVP_CIPHER aesni_256_cbc_hmac_sha256_cipher = {
    ...
    NID_undef,
#endif
    AES_BLOCK_SIZE, 32, AES_BLOCK_SIZE, EVP_CIPH_CBC_MODE | EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_FLAG_AEAD_CIPHER | EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK,
    EVP_ORIG_GLOBAL, aesni_cbc_hmac_sha256_init_key, aesni_cbc_hmac_sha256_cipher, NULL, sizeof(EVP_AES_HMAC_SHA256),
    EVP_CIPH_FLAG_DEFAULT_ASN1 ? NULL : EVP_CIPHER_set_asn1_iv, EVP_CIPH_FLAG_DEFAULT_ASN1 ? NULL : EVP_CIPHER_get_asn1_iv, aesni_cbc_hmac_sha256_ctrl, NULL
};
