...
static void* evp_mac_new(void)
{
    ...
    if ((mac = OPENSSL_zalloc(sizeof(*mac))) == NULL || (mac->lock = CRYPTO_THREAD_lock_new()) == NULL) {
        evp_mac_free(mac);
        return NULL;
    }
...
static void* evp_mac_from_algorithm(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov)
{
...
{
    return evp_generic_fetch(libctx, OSSL_OP_MAC, algorithm, properties, evp_mac_from_algorithm, evp_mac_up_ref, evp_mac_free);
}
...
{
    evp_generic_do_all(libctx, OSSL_OP_MAC, (void (*)(void*, void*))fn, arg, evp_mac_from_algorithm, evp_mac_up_ref, evp_mac_free);
}
