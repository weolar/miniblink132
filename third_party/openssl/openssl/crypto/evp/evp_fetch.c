...
{
    return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_EVP_METHOD_STORE_INDEX, &evp_method_store_method);
}
...
{
    if (!ossl_assert(name_id > 0 && name_id <= METHOD_ID_NAME_MAX) || !ossl_assert(operation_id > 0 && operation_id <= METHOD_ID_OPERATION_MAX))
        return 0;
    return (((name_id << METHOD_ID_NAME_OFFSET) & METHOD_ID_NAME_MASK) | (operation_id & METHOD_ID_OPERATION_MASK));
}
...
static void* get_evp_method_from_store(void* store, const OSSL_PROVIDER** prov, void* data)
{
    ...
    if (name_id == 0 || (meth_id = evp_method_id(name_id, methdata->operation_id)) == 0)
        return NULL;

    if (store == NULL && (store = get_evp_method_store(methdata->libctx)) == NULL)
        return NULL;

    if (!ossl_method_store_fetch(store, meth_id, methdata->propquery, prov, &method))
        return NULL;
    return method;
}

static int put_evp_method_in_store(void* store, void* method, const OSSL_PROVIDER* prov, const char* names, const char* propdef, void* data)
{
    struct evp_method_data_st* methdata = data;
    ...
    if ((namemap = ossl_namemap_stored(methdata->libctx)) == NULL || (name_id = ossl_namemap_name2num_n(namemap, names, l)) == 0
    ...
    return ossl_method_store_add(store, prov, meth_id, propdef, method, methdata->refcnt_up_method, methdata->destruct_method);
}
 ...
 */
static void* construct_evp_method(const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov, void* data)
{
    /*
...
static void* inner_evp_generic_fetch(struct evp_method_data_st* methdata, OSSL_PROVIDER* prov, int operation_id, int name_id, const char* name,
    const char* properties, void* (*new_method)(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov), int (*up_ref_method)(void*),
    void (*free_method)(void*))
{
    ...
    if (meth_id == 0 || !ossl_method_store_cache_get(store, prov, meth_id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = { get_tmp_evp_method_store, reserve_evp_method_store, unreserve_evp_method_store, get_evp_method_from_store,
            put_evp_method_in_store, construct_evp_method, destruct_evp_method };
        ...
        if ((method = ossl_method_construct(methdata->libctx, operation_id, &prov, 0 /* !force_cache */, &mcm, methdata)) != NULL) {
            /*
            ...
            if (name_id == 0)
                name_id = ossl_namemap_name2num(namemap, name);
            if (name_id == 0) {
                ERR_raise_data(ERR_LIB_EVP, ERR_R_FETCH_FAILED, "Algorithm %s cannot be found", name);
                free_method(method);
                method = NULL;
            } else {
                meth_id = evp_method_id(name_id, operation_id);
                if (meth_id != 0)
                    ossl_method_store_cache_set(store, prov, meth_id, propq, method, up_ref_method, free_method);
            }
        }
    ...
    if ((name_id != 0 || name != NULL) && method == NULL) {
        ...
        ERR_raise_data(ERR_LIB_EVP, code, "%s, Algorithm (%s : %d), Properties (%s)", ossl_lib_ctx_get_descriptor(methdata->libctx),
            name == NULL ? "<null>" : name, name_id, properties == NULL ? "<null>" : properties);
    }

    return method;
}

void* evp_generic_fetch(OSSL_LIB_CTX* libctx, int operation_id, const char* name, const char* properties,
    void* (*new_method)(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov), int (*up_ref_method)(void*), void (*free_method)(void*))
{
    ...
    method = inner_evp_generic_fetch(&methdata, NULL, operation_id, 0, name, properties, new_method, up_ref_method, free_method);
 ...
 * method.
 */
void* evp_generic_fetch_by_number(OSSL_LIB_CTX* libctx, int operation_id, int name_id, const char* properties,
    void* (*new_method)(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov), int (*up_ref_method)(void*), void (*free_method)(void*))
{
    struct evp_method_data_st methdata;
    void* method;
    ...
    methdata.libctx = libctx;
    methdata.tmp_store = NULL;
    method = inner_evp_generic_fetch(&methdata, NULL, operation_id, name_id, NULL, properties, new_method, up_ref_method, free_method);
    dealloc_tmp_evp_method_store(methdata.tmp_store);
    return method;
}
    ...
    void* (*new_method)(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov), int (*up_ref_method)(void*), void (*free_method)(void*))
{
    ...
    methdata.tmp_store = NULL;
    method = inner_evp_generic_fetch(&methdata, prov, operation_id, 0, name, properties, new_method, up_ref_method, free_method);
    dealloc_tmp_evp_method_store(methdata.tmp_store);
    return method;
}
...
static int evp_set_parsed_default_properties(OSSL_LIB_CTX* libctx, OSSL_PROPERTY_LIST* def_prop, int loadconfig, int mirrored)
{
    OSSL_METHOD_STORE* store = get_evp_method_store(libctx);
...
int evp_set_default_properties_int(OSSL_LIB_CTX* libctx, const char* propq, int loadconfig, int mirrored)
{
...
static int evp_default_properties_merge(OSSL_LIB_CTX* libctx, const char* propq, int loadconfig)
{
    OSSL_PROPERTY_LIST** plp = ossl_ctx_global_properties(libctx, loadconfig);
...
void evp_generic_do_all(OSSL_LIB_CTX* libctx, int operation_id, void (*user_fn)(void* method, void* arg), void* user_arg,
    void* (*new_method)(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov), int (*up_ref_method)(void*), void (*free_method)(void*))
{
    struct evp_method_data_st methdata;
    ...
    methdata.libctx = libctx;
    methdata.tmp_store = NULL;
    (void)inner_evp_generic_fetch(&methdata, NULL, operation_id, 0, NULL, NULL, new_method, up_ref_method, free_method);
    ...
    ossl_method_store_do_all(get_evp_method_store(libctx), &filter_on_operation_id, &data);
    dealloc_tmp_evp_method_store(methdata.tmp_store);
}
