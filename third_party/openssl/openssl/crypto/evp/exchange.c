    ...
    for (; fns->function_id != 0; fns++) {
        ...
        case OSSL_FUNC_KEYEXCH_GETTABLE_CTX_PARAMS:
            if (exchange->gettable_ctx_params != NULL)
                break;
            exchange->gettable_ctx_params = OSSL_FUNC_keyexch_gettable_ctx_params(fns);
            gparamfncnt++;
            break;
        ...
        case OSSL_FUNC_KEYEXCH_SETTABLE_CTX_PARAMS:
            if (exchange->settable_ctx_params != NULL)
                break;
            exchange->settable_ctx_params = OSSL_FUNC_keyexch_settable_ctx_params(fns);
            sparamfncnt++;
            break;
        }
    }
    if (fncnt != 4 || (gparamfncnt != 0 && gparamfncnt != 2) || (sparamfncnt != 0 && sparamfncnt != 2)) {
        /*
...
EVP_KEYEXCH* EVP_KEYEXCH_fetch(OSSL_LIB_CTX* ctx, const char* algorithm, const char* properties)
{
    return evp_generic_fetch(
        ctx, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm, (int (*)(void*))EVP_KEYEXCH_up_ref, (void (*)(void*))EVP_KEYEXCH_free);
}
...
EVP_KEYEXCH* evp_keyexch_fetch_from_prov(OSSL_PROVIDER* prov, const char* algorithm, const char* properties)
{
    return evp_generic_fetch_from_prov(
        prov, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm, (int (*)(void*))EVP_KEYEXCH_up_ref, (void (*)(void*))EVP_KEYEXCH_free);
}
...
int EVP_PKEY_derive_init_ex(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
    ...
    supported_exch = evp_keymgmt_util_query_operation_name(ctx->keymgmt, OSSL_OP_KEYEXCH);
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        ...
        case 2:
            ...
            exchange = evp_keyexch_fetch_from_prov((OSSL_PROVIDER*)tmp_prov, supported_exch, ctx->propquery);
            if (exchange == NULL)
                goto legacy;
            break;
        }
...
int EVP_PKEY_derive_set_peer_ex(EVP_PKEY_CTX* ctx, EVP_PKEY* peer, int validate_peer)
{
    ...
    tmp_keymgmt_tofree = tmp_keymgmt
        = evp_keymgmt_fetch_from_prov((OSSL_PROVIDER*)EVP_KEYEXCH_get0_provider(ctx->op.kex.exchange), EVP_KEYMGMT_get0_name(ctx->keymgmt), ctx->propquery);
    if (tmp_keymgmt != NULL)
        provkey = evp_pkey_export_to_provider(peer, ctx->libctx, &tmp_keymgmt, ctx->propquery);
    EVP_KEYMGMT_free(tmp_keymgmt_tofree);
...
legacy:
    ...
    return ret;
#else
    if (ctx->pmeth == NULL || !(ctx->pmeth->derive != NULL || ctx->pmeth->encrypt != NULL || ctx->pmeth->decrypt != NULL) || ctx->pmeth->ctrl == NULL) {
        ERR_raise(ERR_LIB_EVP, EVP_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE);
        return -2;
    }
...
int EVP_PKEY_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* pkeylen)
{
    ...
    ret = ctx->op.kex.exchange->derive(ctx->op.kex.algctx, key, pkeylen, key != NULL ? *pkeylen : 0);
...
{
    return keyexch != NULL && evp_is_a(keyexch->prov, keyexch->name_id, NULL, name);
}

void EVP_KEYEXCH_do_all_provided(OSSL_LIB_CTX* libctx, void (*fn)(EVP_KEYEXCH* keyexch, void* arg), void* arg)
{
    evp_generic_do_all(libctx, OSSL_OP_KEYEXCH, (void (*)(void*, void*))fn, arg, evp_keyexch_from_algorithm, (int (*)(void*))EVP_KEYEXCH_up_ref,
