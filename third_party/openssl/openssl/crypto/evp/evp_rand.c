...
static void* evp_rand_new(void)
{
    ...
    if (rand == NULL || (rand->refcnt_lock = CRYPTO_THREAD_lock_new()) == NULL) {
        OPENSSL_free(rand);
        return NULL;
    }
    rand->refcnt = 1;
...
static void* evp_rand_from_algorithm(int name_id, const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov)
{
     ...
     * locking functions.
     */
    if (fnrandcnt != 3 || fnctxcnt != 3 || (fnenablelockcnt != 0 && fnenablelockcnt != 1) || (fnlockcnt != 0 && fnlockcnt != 2)
#ifdef FIPS_MODULE
        || fnzeroizecnt != 1
#endif
    ) {
...
{
    return evp_generic_fetch(libctx, OSSL_OP_RAND, algorithm, properties, evp_rand_from_algorithm, evp_rand_up_ref, evp_rand_free);
}
...
EVP_RAND_CTX* EVP_RAND_CTX_new(EVP_RAND* rand, EVP_RAND_CTX* parent)
{
    ...
    if ((ctx->algctx = rand->newctx(ossl_provider_ctx(rand->prov), parent_ctx, parent_dispatch)) == NULL || !EVP_RAND_up_ref(rand)) {
        ERR_raise(ERR_LIB_EVP, ERR_R_MALLOC_FAILURE);
...
{
    evp_generic_do_all(libctx, OSSL_OP_RAND, (void (*)(void*, void*))fn, arg, evp_rand_from_algorithm, evp_rand_up_ref, evp_rand_free);
}
...
static int evp_rand_generate_locked(
...
static int evp_rand_reseed_locked(
    EVP_RAND_CTX* ctx, int prediction_resistance, const unsigned char* ent, size_t ent_len, const unsigned char* addin, size_t addin_len)
{
