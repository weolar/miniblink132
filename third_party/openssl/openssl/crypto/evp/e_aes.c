...
{
    aesni_cbc_encrypt(in, out, len, &EVP_C_DATA(EVP_AES_KEY, ctx)->ks.ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
...
static int aesni_ecb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    aesni_ecb_encrypt(in, out, len, &EVP_C_DATA(EVP_AES_KEY, ctx)->ks.ks, EVP_CIPHER_CTX_is_encrypting(ctx));
...
static int aesni_gcm_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    ...
    if (!iv && !key)
        return 1;
    if (key) {
        aesni_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &gctx->ks.ks);
...
static int aesni_ocb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    ...
    if (key) {
        do {
            ...
            if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)aesni_encrypt, (block128_f)aesni_decrypt,
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, flags)                                                                          \
    static const EVP_CIPHER aesni_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE,                \
        EVP_ORIG_GLOBAL, aesni_init_key, aesni_##mode##_cipher, NULL, sizeof(EVP_AES_KEY), NULL, NULL, NULL, NULL };                                           \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, \
        aes_init_key, aes_##mode##_cipher, NULL, sizeof(EVP_AES_KEY), NULL, NULL, NULL, NULL };                                                                \
...
#define BLOCK_CIPHER_custom(nid, keylen, blocksize, ivlen, mode, MODE, flags)                                                                                  \
    static const EVP_CIPHER aesni_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                    \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aesni_##mode##_init_key, aesni_##mode##_cipher, aes_##mode##_cleanup, sizeof(EVP_AES_##MODE##_CTX),   \
        NULL, NULL, aes_##mode##_ctrl, NULL };                                                                                                                 \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                      \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aes_##mode##_init_key, aes_##mode##_cipher, aes_##mode##_cleanup, sizeof(EVP_AES_##MODE##_CTX), NULL, \
        NULL, aes_##mode##_ctrl, NULL };                                                                                                                       \
    const EVP_CIPHER* EVP_aes_##keylen##_##mode(void)                                                                                                          \
    {                                                                                                                                                          \
        return AESNI_CAPABLE ? &aesni_##keylen##_##mode : &aes_##keylen##_##mode;                                                                              \
    }
...
static int aes_t4_ocb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    ...
    if (key) {
        do {
             ...
             * decrypt for an encryption operation.
             */
            aes_t4_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksenc.ks);
            aes_t4_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksdec.ks);
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, flags)                                                                          \
    static const EVP_CIPHER aes_t4_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE,               \
        EVP_ORIG_GLOBAL, aes_t4_init_key, aes_t4_##mode##_cipher, NULL, sizeof(EVP_AES_KEY), NULL, NULL, NULL, NULL };                                         \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, \
        aes_init_key, aes_##mode##_cipher, NULL, sizeof(EVP_AES_KEY), NULL, NULL, NULL, NULL };                                                                \
    const EVP_CIPHER* EVP_aes_##keylen##_##mode(void)                                                                                                          \
    {                                                                                                                                                          \
...
#define BLOCK_CIPHER_custom(nid, keylen, blocksize, ivlen, mode, MODE, flags)                                                                                  \
    static const EVP_CIPHER aes_t4_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                   \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aes_t4_##mode##_init_key, aes_t4_##mode##_cipher, aes_##mode##_cleanup, sizeof(EVP_AES_##MODE##_CTX), \
        NULL, NULL, aes_##mode##_ctrl, NULL };                                                                                                                 \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                      \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aes_##mode##_init_key, aes_##mode##_cipher, aes_##mode##_cleanup, sizeof(EVP_AES_##MODE##_CTX), NULL, \
...
static int s390x_aes_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc);
...
static int s390x_aes_cbc_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len);

static int s390x_aes_ecb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
...
static int s390x_aes_ecb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_ECB_CTX* cctx = EVP_C_DATA(S390X_AES_ECB_CTX, ctx);
...
static int s390x_aes_ofb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* ivec, int enc)
{
...
static int s390x_aes_ofb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_OFB_CTX* cctx = EVP_C_DATA(S390X_AES_OFB_CTX, ctx);
    const int ivlen = EVP_CIPHER_CTX_get_iv_length(ctx);
    ...
    if (rem) {
        s390x_km(cctx->kmo.param.cv, 16, cctx->kmo.param.cv, cctx->fc, cctx->kmo.param.k);
...
static int s390x_aes_cfb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* ivec, int enc)
{
...
static int s390x_aes_cfb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_CFB_CTX* cctx = EVP_C_DATA(S390X_AES_CFB_CTX, ctx);
    const int keylen = EVP_CIPHER_CTX_get_key_length(ctx);
    ...
    if (rem) {
        s390x_km(cctx->kmf.param.cv, 16, cctx->kmf.param.cv, S390X_AES_FC(keylen), cctx->kmf.param.k);
...
static int s390x_aes_cfb8_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* ivec, int enc)
{
    S390X_AES_CFB_CTX* cctx = EVP_C_DATA(S390X_AES_CFB_CTX, ctx);
    const unsigned char* iv = ctx->oiv;
    const int keylen = EVP_CIPHER_CTX_get_key_length(ctx);
...
static int s390x_aes_cfb8_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_CFB_CTX* cctx = EVP_C_DATA(S390X_AES_CFB_CTX, ctx);
    const int ivlen = EVP_CIPHER_CTX_get_iv_length(ctx);
    unsigned char* iv = EVP_CIPHER_CTX_iv_noconst(ctx);
...
static int s390x_aes_cfb1_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len);
...
static int s390x_aes_ctr_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len);
 ...
 * big-endian.
 */
static int s390x_aes_gcm_aad(S390X_AES_GCM_CTX* ctx, const unsigned char* aad, size_t len)
{
 ...
 */
static int s390x_aes_gcm(S390X_AES_GCM_CTX* ctx, const unsigned char* in, unsigned char* out, size_t len)
{
    ...
    if (n) {
        ...
        if (!n) {
            s390x_kma(ctx->ares, ctx->areslen, ctx->mres, 16, buf.b, ctx->fc | S390X_KMA_LAAD, &ctx->kma.param);
            ctx->fc |= S390X_KMA_HS;
            ctx->areslen = 0;
    ...
    len &= ~(size_t)0xf;
    if (len) {
        s390x_kma(ctx->ares, ctx->areslen, in, len, out, ctx->fc | S390X_KMA_LAAD, &ctx->kma.param);
        in += len;
        out += len;
        ctx->fc |= S390X_KMA_HS;
        ctx->areslen = 0;
...
static void s390x_aes_gcm_setiv(S390X_AES_GCM_CTX* ctx, const unsigned char* iv)
{
    ...
    if (ctx->ivlen == 12) {
        ...
        s390x_kma(ctx->iv, S390X_gcm_ivpadlen(ctx->ivlen), NULL, 0, NULL, ctx->fc, &ctx->kma.param);
        ctx->fc |= S390X_KMA_HS;
 ...
 */
static int s390x_aes_gcm_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
 ...
 */
static int s390x_aes_gcm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_GCM_CTX* gctx = EVP_C_DATA(S390X_AES_GCM_CTX, ctx);
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, enc ? EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV, EVP_GCM_TLS_EXPLICIT_IV_LEN, out) <= 0)
        goto err;
    ...
    s390x_kma(buf, gctx->tls_aad_len, in, len, out, gctx->fc | S390X_KMA_LAAD | S390X_KMA_LPC, &gctx->kma.param);
 ...
 */
static int s390x_aes_gcm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_GCM_CTX* gctx = EVP_C_DATA(S390X_AES_GCM_CTX, ctx);
    ...
    if (in != NULL) {
        ...
        s390x_kma(gctx->ares, gctx->areslen, gctx->mres, gctx->mreslen, tmp, gctx->fc | S390X_KMA_LAAD | S390X_KMA_LPC, &gctx->kma.param);
...
static int s390x_aes_xts_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc);
#define s390x_aes_xts_cipher aes_xts_cipher
static int s390x_aes_xts_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len);
 ...
 */
static inline void s390x_aes_ccm_setiv(S390X_AES_CCM_CTX* ctx, const unsigned char* nonce, size_t mlen)
{
 ...
 */
static void s390x_aes_ccm_aad(S390X_AES_CCM_CTX* ctx, const unsigned char* aad, size_t alen)
{
    unsigned char* ptr;
    int i, rem;
    ...
    s390x_kmac(ctx->aes.ccm.nonce.b, 32, ctx->aes.ccm.fc, &ctx->aes.ccm.kmac_param);
    ...
    if (rem) {
        ...
        s390x_km(ctx->aes.ccm.kmac_param.icv.b, 16, ctx->aes.ccm.kmac_param.icv.b, ctx->aes.ccm.fc, ctx->aes.ccm.kmac_param.k);
        ctx->aes.ccm.blocks++;
    }
}
 ...
 * success.
 */
static int s390x_aes_ccm(S390X_AES_CCM_CTX* ctx, const unsigned char* in, unsigned char* out, size_t len, int enc)
{
    size_t n, rem;
    ...
    if (!(flags & S390X_CCM_AAD_FLAG)) {
        s390x_km(ctx->aes.ccm.nonce.b, 16, ctx->aes.ccm.kmac_param.icv.b, ctx->aes.ccm.fc, ctx->aes.ccm.kmac_param.k);
        ctx->aes.ccm.blocks++;
    }
    l = flags & 0x7;
    ...
    if (enc) {
        ...
        CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k, ctx->aes.ccm.nonce.b, ctx->aes.ccm.buf.b, &num, (ctr128_f)AES_ctr32_encrypt);
    } else {
        /* decrypt-then-mac */
        CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k, ctx->aes.ccm.nonce.b, ctx->aes.ccm.buf.b, &num, (ctr128_f)AES_ctr32_encrypt);
    ...
    s390x_km(ctx->aes.ccm.nonce.b, 16, ctx->aes.ccm.buf.b, ctx->aes.ccm.fc, ctx->aes.ccm.kmac_param.k);
 ...
 */
static int s390x_aes_ccm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    if (out != in || len < (EVP_CCM_TLS_EXPLICIT_IV_LEN + (size_t)cctx->aes.ccm.m))
        return -1;
    ...
    if (enc) {
        ...
        if (!s390x_aes_ccm(cctx, in, out, len, enc)) {
            if (!CRYPTO_memcmp(cctx->aes.ccm.kmac_param.icv.b, in + len, cctx->aes.ccm.m))
                return len;
        }
 ...
 */
static int s390x_aes_ccm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    S390X_AES_CCM_CTX* cctx = EVP_C_DATA(S390X_AES_CCM_CTX, ctx);
    const int enc = EVP_CIPHER_CTX_is_encrypting(ctx);
    int rv;
    ...
    if (enc) {
        ...
        if (!s390x_aes_ccm(cctx, in, out, len, enc)) {
            buf = EVP_CIPHER_CTX_buf_noconst(ctx);
            if (!CRYPTO_memcmp(cctx->aes.ccm.kmac_param.icv.b, buf, cctx->aes.ccm.m))
                rv = len;
        }
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, flags)                                                                          \
    static const EVP_CIPHER s390x_aes_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE,            \
        EVP_ORIG_GLOBAL, s390x_aes_##mode##_init_key, s390x_aes_##mode##_cipher, NULL, sizeof(S390X_AES_##MODE##_CTX), NULL, NULL, NULL, NULL };               \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, \
        aes_init_key, aes_##mode##_cipher, NULL, sizeof(EVP_AES_KEY), NULL, NULL, NULL, NULL };                                                                \
    const EVP_CIPHER* EVP_aes_##keylen##_##mode(void)                                                                                                          \
    {                                                                                                                                                          \
...
#define BLOCK_CIPHER_custom(nid, keylen, blocksize, ivlen, mode, MODE, flags)                                                                                  \
    static const EVP_CIPHER s390x_aes_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, s390x_aes_##mode##_init_key, s390x_aes_##mode##_cipher, s390x_aes_##mode##_cleanup,                   \
        sizeof(S390X_AES_##MODE##_CTX), NULL, NULL, s390x_aes_##mode##_ctrl, NULL };                                                                           \
    static const EVP_CIPHER aes_##keylen##_##mode = { nid##_##keylen##_##mode, blocksize,                                                                      \
        (EVP_CIPH_##MODE##_MODE == EVP_CIPH_XTS_MODE || EVP_CIPH_##MODE##_MODE == EVP_CIPH_SIV_MODE ? 2 : 1) * keylen / 8, ivlen,                              \
        flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aes_##mode##_init_key, aes_##mode##_cipher, aes_##mode##_cleanup, sizeof(EVP_AES_##MODE##_CTX), NULL, \
        NULL, aes_##mode##_ctrl, NULL };                                                                                                                       \
        ...
        BLOCK_CIPHER_generic(nid, keylen, 16, 0, ecb, ecb, ECB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)                                                            \
            BLOCK_CIPHER_generic(nid, keylen, 1, 16, ofb128, ofb, OFB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)                                                     \
                BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb128, cfb, CFB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)                                                 \
                    BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb1, cfb1, CFB, flags) BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb8, cfb8, CFB, flags)          \
...
#ifdef HWAES_CAPABLE
        if (HWAES_CAPABLE) {
            ret = HWAES_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
            ...
            ret = AES_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
...
#ifdef VPAES_CAPABLE
            if (VPAES_CAPABLE) {
            ret = vpaes_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
...
#ifdef HWAES_CAPABLE
        if (HWAES_CAPABLE) {
        ret = HWAES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks.ks);
...
static int aes_cfb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    int num = EVP_CIPHER_CTX_get_num(ctx);
    CRYPTO_cfb128_encrypt(in, out, len, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
    return 1;
}
...
static int aes_cfb8_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    int num = EVP_CIPHER_CTX_get_num(ctx);
    CRYPTO_cfb128_8_encrypt(in, out, len, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
    return 1;
}
...
static int aes_cfb1_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_test_flags(ctx, EVP_CIPH_FLAG_LENGTH_BITS)) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, len, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
        EVP_CIPHER_CTX_set_num(ctx, num);
        return 1;
    }
    ...
    while (len >= MAXBITCHUNK) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, MAXBITCHUNK * 8, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
        EVP_CIPHER_CTX_set_num(ctx, num);
        len -= MAXBITCHUNK;
        out += MAXBITCHUNK;
        in += MAXBITCHUNK;
    ...
    if (len) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, len * 8, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
        EVP_CIPHER_CTX_set_num(ctx, num);
    }
...
static int aes_ctr_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    if (dat->stream.ctr)
        CRYPTO_ctr128_encrypt_ctr32(in, out, len, &dat->ks, ctx->iv, EVP_CIPHER_CTX_buf_noconst(ctx), &num, dat->stream.ctr);
    else
        CRYPTO_ctr128_encrypt(in, out, len, &dat->ks, ctx->iv, EVP_CIPHER_CTX_buf_noconst(ctx), &num, dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
    return 1;
}
...
static int aes_gcm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
     ...
     * buffer.
     */
    if (EVP_CIPHER_CTX_ctrl(ctx, ctx->encrypt ? EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV, EVP_GCM_TLS_EXPLICIT_IV_LEN, out) <= 0)
        goto err;
    /* Use saved AAD */
...
static int aes_xts_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    if (xctx->xts.key1 == NULL || xctx->xts.key2 == NULL || out == NULL || in == NULL || len < AES_BLOCK_SIZE)
        return 0;
...
BLOCK_CIPHER_custom(NID_aes, 128, 1, 16, xts, XTS, XTS_FLAGS) BLOCK_CIPHER_custom(NID_aes, 256, 1, 16, xts, XTS, XTS_FLAGS)
...
static int aes_wrap_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    ...
    if (key != NULL) {
        if (EVP_CIPHER_CTX_is_encrypting(ctx))
            AES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &wctx->ks.ks);
        else
            AES_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &wctx->ks.ks);
        if (iv == NULL)
            wctx->iv = NULL;
    }
    if (iv != NULL) {
...
static const EVP_CIPHER aes_128_wrap
    = { NID_id_aes128_wrap, 8, 16, 8, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL, sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
static const EVP_CIPHER aes_192_wrap
    = { NID_id_aes192_wrap, 8, 24, 8, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL, sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
static const EVP_CIPHER aes_256_wrap
    = { NID_id_aes256_wrap, 8, 32, 8, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL, sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
static const EVP_CIPHER aes_128_wrap_pad = { NID_id_aes128_wrap_pad, 8, 16, 4, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
static const EVP_CIPHER aes_192_wrap_pad = { NID_id_aes192_wrap_pad, 8, 24, 4, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
static const EVP_CIPHER aes_256_wrap_pad = { NID_id_aes256_wrap_pad, 8, 32, 4, WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL };
...
#ifdef HWAES_CAPABLE
            if (HWAES_CAPABLE) {
                HWAES_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksenc.ks);
                HWAES_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &octx->ksdec.ks);
                if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)HWAES_encrypt, (block128_f)HWAES_decrypt,
...
BLOCK_CIPHER_custom(NID_aes, 128, 16, 12, ocb, OCB, EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS) BLOCK_CIPHER_custom(NID_aes, 192, 16, 12, ocb, OCB,
    EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS) BLOCK_CIPHER_custom(NID_aes, 256, 16, 12, ocb, OCB, EVP_CIPH_FLAG_AEAD_CIPHER | CUSTOM_FLAGS)
#endif /* OPENSSL_NO_OCB */
