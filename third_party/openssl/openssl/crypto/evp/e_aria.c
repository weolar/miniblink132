...
static int aria_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    ...
    if (enc || (mode != EVP_CIPH_ECB_MODE && mode != EVP_CIPH_CBC_MODE))
        ret = ossl_aria_set_encrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, EVP_CIPHER_CTX_get_cipher_data(ctx));
    else
        ret = ossl_aria_set_decrypt_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, EVP_CIPHER_CTX_get_cipher_data(ctx));
    if (ret < 0) {
...
static void aria_cbc_encrypt(const unsigned char* in, unsigned char* out, size_t len, const ARIA_KEY* key, unsigned char* ivec, const int enc)
{
...
static void aria_cfb128_encrypt(const unsigned char* in, unsigned char* out, size_t length, const ARIA_KEY* key, unsigned char* ivec, int* num, const int enc)
{

    CRYPTO_cfb128_encrypt(in, out, length, key, ivec, num, enc, (block128_f)ossl_aria_encrypt);
}

static void aria_cfb1_encrypt(const unsigned char* in, unsigned char* out, size_t length, const ARIA_KEY* key, unsigned char* ivec, int* num, const int enc)
{
    CRYPTO_cfb128_1_encrypt(in, out, length, key, ivec, num, enc, (block128_f)ossl_aria_encrypt);
}

static void aria_cfb8_encrypt(const unsigned char* in, unsigned char* out, size_t length, const ARIA_KEY* key, unsigned char* ivec, int* num, const int enc)
{
    CRYPTO_cfb128_8_encrypt(in, out, length, key, ivec, num, enc, (block128_f)ossl_aria_encrypt);
}

static void aria_ecb_encrypt(const unsigned char* in, unsigned char* out, const ARIA_KEY* key, const int enc)
{
    ossl_aria_encrypt(in, out, key);
}

static void aria_ofb128_encrypt(const unsigned char* in, unsigned char* out, size_t length, const ARIA_KEY* key, unsigned char* ivec, int* num)
{
    CRYPTO_ofb128_encrypt(in, out, length, key, ivec, num, (block128_f)ossl_aria_encrypt);
}

IMPLEMENT_BLOCK_CIPHER(
    aria_128, ks, aria, EVP_ARIA_KEY, NID_aria_128, 16, 16, 16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, NULL)
IMPLEMENT_BLOCK_CIPHER(
    aria_192, ks, aria, EVP_ARIA_KEY, NID_aria_192, 16, 24, 16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, NULL)
IMPLEMENT_BLOCK_CIPHER(
    aria_256, ks, aria, EVP_ARIA_KEY, NID_aria_256, 16, 32, 16, 128, 0, aria_init_key, NULL, EVP_CIPHER_set_asn1_iv, EVP_CIPHER_get_asn1_iv, NULL)

#define IMPLEMENT_ARIA_CFBR(ksize, cbits) IMPLEMENT_CFBR(aria, aria, EVP_ARIA_KEY, ks, ksize, cbits, 16, 0)
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, flags)                                                                          \
    static const EVP_CIPHER aria_##keylen##_##mode = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, flags | EVP_CIPH_##MODE##_MODE,                 \
        EVP_ORIG_GLOBAL, aria_init_key, aria_##mode##_cipher, NULL, sizeof(EVP_ARIA_KEY), NULL, NULL, NULL, NULL };                                            \
    const EVP_CIPHER* EVP_aria_##keylen##_##mode(void)                                                                                                         \
    {                                                                                                                                                          \
        return &aria_##keylen##_##mode;                                                                                                                        \
    }

static int aria_ctr_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    ...
    CRYPTO_ctr128_encrypt(in, out, len, &dat->ks, ctx->iv, EVP_CIPHER_CTX_buf_noconst(ctx), &num, (block128_f)ossl_aria_encrypt);
    EVP_CIPHER_CTX_set_num(ctx, num);
    return 1;
}

BLOCK_CIPHER_generic(NID_aria, 128, 1, 16, ctr, ctr, CTR, 0) BLOCK_CIPHER_generic(NID_aria, 192, 1, 16, ctr, ctr, CTR, 0)
...
static int aria_gcm_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    int ret;
...
static int aria_gcm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    EVP_ARIA_GCM_CTX* gctx = EVP_C_DATA(EVP_ARIA_GCM_CTX, ctx);
    int rv = -1;
     ...
     * buffer.
     */
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CIPHER_CTX_is_encrypting(ctx) ? EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV, EVP_GCM_TLS_EXPLICIT_IV_LEN, out) <= 0)
        goto err;
    /* Use saved AAD */
...
static int aria_gcm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    EVP_ARIA_GCM_CTX* gctx = EVP_C_DATA(EVP_ARIA_GCM_CTX, ctx);
...
static int aria_ccm_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key, const unsigned char* iv, int enc)
{
    int ret;
...
static int aria_ccm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    EVP_ARIA_CCM_CTX* cctx = EVP_C_DATA(EVP_ARIA_CCM_CTX, ctx);
...
static int aria_ccm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out, const unsigned char* in, size_t len)
{
    EVP_ARIA_CCM_CTX* cctx = EVP_C_DATA(EVP_ARIA_CCM_CTX, ctx);
    CCM128_CONTEXT* ccm = &cctx->ccm;
...
#define BLOCK_CIPHER_aead(nid, keylen, blocksize, ivlen, nmode, mode, MODE, flags)                                                                             \
    static const EVP_CIPHER aria_##keylen##_##mode                                                                                                             \
        = { nid##_##keylen##_##nmode, blocksize, keylen / 8, ivlen, ARIA_AUTH_FLAGS | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, aria_##mode##_init_key,         \
              aria_##mode##_cipher, aria_##mode##_cleanup, sizeof(EVP_ARIA_##MODE##_CTX), NULL, NULL, aria_##mode##_ctrl, NULL };                              \
    const EVP_CIPHER* EVP_aria_##keylen##_##mode(void)                                                                                                         \
    {                                                                                                                                                          \
        return (EVP_CIPHER*)&aria_##keylen##_##mode;                                                                                                           \
    }

BLOCK_CIPHER_aead(NID_aria, 128, 1, 12, gcm, gcm, GCM, 0) BLOCK_CIPHER_aead(NID_aria, 192, 1, 12, gcm, gcm, GCM, 0)
    BLOCK_CIPHER_aead(NID_aria, 256, 1, 12, gcm, gcm, GCM, 0)

        BLOCK_CIPHER_aead(NID_aria, 128, 1, 12, ccm, ccm, CCM, 0) BLOCK_CIPHER_aead(NID_aria, 192, 1, 12, ccm, ccm, CCM, 0)
