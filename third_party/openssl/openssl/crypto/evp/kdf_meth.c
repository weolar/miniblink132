...
static void* evp_kdf_new(void)
{
    ...
    if ((kdf = OPENSSL_zalloc(sizeof(*kdf))) == NULL || (kdf->lock = CRYPTO_THREAD_lock_new()) == NULL) {
        OPENSSL_free(kdf);
        return NULL;
    }
    kdf->refcnt = 1;
...
{
    return evp_generic_fetch(libctx, OSSL_OP_KDF, algorithm, properties, evp_kdf_from_algorithm, evp_kdf_up_ref, evp_kdf_free);
}
