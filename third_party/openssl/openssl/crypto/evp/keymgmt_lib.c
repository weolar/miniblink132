...
EVP_PKEY* evp_keymgmt_util_make_pkey(EVP_KEYMGMT* keymgmt, void* keydata)
{
    ...
    if (keymgmt == NULL || keydata == NULL || (pkey = EVP_PKEY_new()) == NULL || !evp_keymgmt_util_assign_pkey(pkey, keymgmt, keydata)) {
        EVP_PKEY_free(pkey);
        return NULL;
    }
    return pkey;
...
void* evp_keymgmt_util_gen(EVP_PKEY* target, EVP_KEYMGMT* keymgmt, void* genctx, OSSL_CALLBACK* cb, void* cbarg)
{
    ...
    if ((keydata = evp_keymgmt_gen(keymgmt, genctx, cb, cbarg)) == NULL || !evp_keymgmt_util_assign_pkey(target, keymgmt, keydata)) {
        evp_keymgmt_freedata(keymgmt, keydata);
        keydata = NULL;
    }
