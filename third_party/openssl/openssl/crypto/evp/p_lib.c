...
int EVP_PKEY_copy_parameters(EVP_PKEY* to, const EVP_PKEY* from)
{
    ...
    if (to->keymgmt != NULL && to->keydata == NULL) {
        ...
        void* from_keydata = evp_pkey_export_to_provider((EVP_PKEY*)from, NULL, &to_keymgmt, NULL);
        ...
        if (from_keydata == NULL)
            ...
            ok = (to->keydata = evp_keymgmt_dup(to->keymgmt, from_keydata, SELECT_PARAMETERS)) != NULL;
        goto end;
    }
...
int EVP_PKEY_eq(const EVP_PKEY* a, const EVP_PKEY* b)
{
    ...
    if (a->keymgmt != NULL || b->keymgmt != NULL) {
        ...
        if (evp_keymgmt_util_has((EVP_PKEY*)a, OSSL_KEYMGMT_SELECT_PUBLIC_KEY) && evp_keymgmt_util_has((EVP_PKEY*)b, OSSL_KEYMGMT_SELECT_PUBLIC_KEY))
...
static EVP_PKEY* new_raw_key_int(
    OSSL_LIB_CTX* libctx, const char* strtype, const char* propq, int nidtype, ENGINE* e, const unsigned char* key, size_t len, int key_is_priv)
{
    EVP_PKEY* pkey = NULL;
    ...
    if (e == NULL && ameth == NULL) {
         ...
         * a provider.
         */
        ctx = EVP_PKEY_CTX_new_from_name(libctx, strtype != NULL ? strtype : OBJ_nid2sn(nidtype), propq);
        if (ctx == NULL)
            goto err;
...
{
    return new_raw_key_int(libctx, keytype, propq, EVP_PKEY_NONE, NULL, priv, len, 1);
}
...
{
    return new_raw_key_int(libctx, keytype, propq, EVP_PKEY_NONE, NULL, pub, len, 0);
}
...
int EVP_PKEY_get_raw_private_key(const EVP_PKEY* pkey, unsigned char* priv, size_t* len)
{
    if (pkey->keymgmt != NULL) {
        ...
        return evp_keymgmt_util_export(pkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY, get_raw_key_details, &raw_key);
    }
...
int EVP_PKEY_get_raw_public_key(const EVP_PKEY* pkey, unsigned char* pub, size_t* len)
{
    if (pkey->keymgmt != NULL) {
        ...
        return evp_keymgmt_util_export(pkey, OSSL_KEYMGMT_SELECT_PUBLIC_KEY, get_raw_key_details, &raw_key);
    }
...
static EVP_PKEY* new_cmac_key_int(
    const unsigned char* priv, size_t len, const char* cipher_name, const EVP_CIPHER* cipher, OSSL_LIB_CTX* libctx, const char* propq, ENGINE* e)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PRIV_KEY, (void*)priv, len);
    *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_CIPHER, (char*)cipher_name, 0);
    if (propq != NULL)
        *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_PROPERTIES, (char*)propq, 0);
#ifndef OPENSSL_NO_ENGINE
    if (engine_id != NULL)
        *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_ENGINE, (char*)engine_id, 0);
#endif
...
static void detect_foreign_key(EVP_PKEY* pkey)
{
    ...
    case EVP_PKEY_RSA_PSS:
        pkey->foreign = pkey->pkey.rsa != NULL && ossl_rsa_is_foreign(pkey->pkey.rsa);
        break;
#ifndef OPENSSL_NO_EC
    case EVP_PKEY_SM2:
        break;
    case EVP_PKEY_EC:
        pkey->foreign = pkey->pkey.ec != NULL && ossl_ec_key_is_foreign(pkey->pkey.ec);
        break;
#endif
#ifndef OPENSSL_NO_DSA
    case EVP_PKEY_DSA:
        pkey->foreign = pkey->pkey.dsa != NULL && ossl_dsa_is_foreign(pkey->pkey.dsa);
        break;
#endif
...
int EVP_PKEY_can_sign(const EVP_PKEY* pkey)
{
    if (pkey->keymgmt == NULL) {
        ...
        const char* supported_sig
            = pkey->keymgmt->query_operation_name != NULL ? pkey->keymgmt->query_operation_name(OSSL_OP_SIGNATURE) : EVP_KEYMGMT_get0_name(pkey->keymgmt);
...
static int print_pkey(const EVP_PKEY* pkey, BIO* out, int indent, int selection /* For provided encoding */, const char* propquery /* For provided encoding */,
    int (*legacy_print)(BIO* out, const EVP_PKEY* pkey, int indent, ASN1_PCTX* pctx), ASN1_PCTX* legacy_pctx /* For legacy print */)
{
    int pop_f_prefix;
    long saved_indent;
...
{
    return print_pkey(pkey, out, indent, EVP_PKEY_PUBLIC_KEY, NULL, (pkey->ameth != NULL ? pkey->ameth->pub_print : NULL), pctx);
}

int EVP_PKEY_print_private(BIO* out, const EVP_PKEY* pkey, int indent, ASN1_PCTX* pctx)
{
    return print_pkey(pkey, out, indent, EVP_PKEY_PRIVATE_KEY, NULL, (pkey->ameth != NULL ? pkey->ameth->priv_print : NULL), pctx);
}

int EVP_PKEY_print_params(BIO* out, const EVP_PKEY* pkey, int indent, ASN1_PCTX* pctx)
{
    return print_pkey(pkey, out, indent, EVP_PKEY_KEY_PARAMETERS, NULL, (pkey->ameth != NULL ? pkey->ameth->param_print : NULL), pctx);
}
...
{
    if (pkey->ameth == NULL)
        return evp_keymgmt_util_get_deflt_digest_name(pkey->keymgmt, pkey->keydata, mdname, mdname_sz);
...
int EVP_PKEY_get_group_name(const EVP_PKEY* pkey, char* gname, size_t gname_sz, size_t* gname_len)
{
    return EVP_PKEY_get_utf8_string_param(pkey, OSSL_PKEY_PARAM_GROUP_NAME, gname, gname_sz, gname_len);
}
...
int EVP_PKEY_set1_encoded_public_key(EVP_PKEY* pkey, const unsigned char* pub, size_t publen)
{
    if (pkey == NULL)
        return 0;
    if (evp_pkey_is_provided(pkey))
        return EVP_PKEY_set_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, (unsigned char*)pub, publen);

    if (publen > INT_MAX)
        return 0;
    /* Historically this function was EVP_PKEY_set1_tls_encodedpoint */
    if (evp_pkey_asn1_ctrl(pkey, ASN1_PKEY_CTRL_SET1_TLS_ENCPT, publen, (void*)pub) <= 0)
        return 0;
    return 1;
}
...
size_t EVP_PKEY_get1_encoded_public_key(EVP_PKEY* pkey, unsigned char** ppub)
{
    ...
    if (evp_pkey_is_provided(pkey)) {
         ...
         * to allocate.
         */
        EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, NULL, 0, &return_size);
        ...
        *ppub = NULL;
        ...
        if (!EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, buf, return_size, NULL)) {
            OPENSSL_free(buf);
            return 0;
        }
        *ppub = buf;
...
static int pkey_set_type(EVP_PKEY* pkey, ENGINE* e, int type, const char* str, int len, EVP_KEYMGMT* keymgmt)
{
     ...
     * It is forbidden
     */
    if (!ossl_assert(type == EVP_PKEY_NONE || keymgmt == NULL) || !ossl_assert(e == NULL || keymgmt == NULL)) {
    ...
    if (pkey != NULL) {
         ...
         */
        if (pkey->type != EVP_PKEY_NONE && type == pkey->save_type && pkey->ameth != NULL)
            return 1;
#ifndef OPENSSL_NO_ENGINE
...
#endif
    return pkey_set_type(pkey, NULL, EVP_PKEY_NONE, EVP_PKEY_TYPE_STR, EVP_PKEY_TYPE_STRLEN, keymgmt);
...
EVP_PKEY* EVP_PKEY_dup(EVP_PKEY* pkey)
{
    ...
    if (evp_pkey_is_provided(pkey)) {
        if (!evp_keymgmt_util_copy(dup_pk, pkey, OSSL_KEYMGMT_SELECT_ALL))
            goto err;
        goto done;
    }
    ...
    /* copy auxiliary data */
    if (!CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_EVP_PKEY, &dup_pk->ex_data, &pkey->ex_data))
        goto err;
...
void* evp_pkey_export_to_provider(EVP_PKEY* pk, OSSL_LIB_CTX* libctx, EVP_KEYMGMT** keymgmt, const char* propquery)
{
    ...
    if (pk->pkey.ptr != NULL) {
        ...
        if (!pk->ameth->export_to(pk, keydata, tmp_keymgmt->import, libctx, propquery)) {
        ...
        if (!CRYPTO_THREAD_write_lock(pk->lock))
            goto end;
        if (pk->ameth->dirty_cnt(pk) != pk->dirty_cnt_copy && !evp_keymgmt_util_clear_operation_cache(pk, 0)) {
            CRYPTO_THREAD_unlock(pk->lock);
            evp_keymgmt_freedata(tmp_keymgmt, keydata);
            keydata = NULL;
        ...
        if (!evp_keymgmt_util_cache_keydata(pk, tmp_keymgmt, keydata, selection)) {
            CRYPTO_THREAD_unlock(pk->lock);
            evp_keymgmt_freedata(tmp_keymgmt, keydata);
            keydata = NULL;
            goto end;
...
int EVP_PKEY_set_octet_string_param(EVP_PKEY* pkey, const char* key_name, const unsigned char* buf, size_t bsize)
{
    ...
    params[0] = OSSL_PARAM_construct_octet_string(key_name, (unsigned char*)buf, bsize);
    params[1] = OSSL_PARAM_construct_end();
    return EVP_PKEY_set_params(pkey, params);
}
...
{
    return (pkey != NULL && evp_pkey_is_provided(pkey)) ? EVP_KEYMGMT_gettable_params(pkey->keymgmt) : NULL;
}
...
int EVP_PKEY_get_ec_point_conv_form(const EVP_PKEY* pkey)
{
    ...
    if (!EVP_PKEY_get_utf8_string_param(pkey, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, name, sizeof(name), &name_len))
        return 0;
...
int EVP_PKEY_get_field_type(const EVP_PKEY* pkey)
{
    ...
    if (!EVP_PKEY_get_utf8_string_param(pkey, OSSL_PKEY_PARAM_EC_FIELD_TYPE, fstr, sizeof(fstr), &fstrlen))
        return 0;
