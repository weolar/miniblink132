    ...
    while (outl > 0) {
        ...
        if (ctx->start && (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL)) {
            ...
            for (j = 0; j < i; j++) {
                ...
                k = EVP_DecodeUpdate(ctx->base64, (unsigned char*)ctx->buf, &num, p, q - p);
        ...
        if (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) {
            ...
            i = EVP_DecodeUpdate(ctx->base64, (unsigned char*)ctx->buf, &ctx->buf_len, (unsigned char*)ctx->tmp, i);
            ctx->tmp_len = 0;
        }
        /*
...
static int b64_write(BIO* b, const char* in, int inl)
{
    ...
    while (inl > 0) {
        ...
        if (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) {
            ...
            if (!EVP_EncodeUpdate(ctx->base64, (unsigned char*)ctx->buf, &ctx->buf_len, (unsigned char*)in, n))
                return ((ret == 0) ? -1 : ret);
            OPENSSL_assert(ctx->buf_len <= (int)sizeof(ctx->buf));
            OPENSSL_assert(ctx->buf_len >= ctx->buf_off);
            ret += n;
...
static long b64_ctrl(BIO* b, int cmd, long num, void* ptr)
{
    ...
    again:
        ...
        if (BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) {
            ...
            ctx->buf_off = 0;
            EVP_EncodeFinal(ctx->base64, (unsigned char*)ctx->buf, &(ctx->buf_len));
            /* push out the bytes */
            goto again;
        }
