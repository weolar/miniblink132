    ...
    unsigned char* out, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (unsigned char*)salt, saltlen);
...
{
    return ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen, iter, digest, keylen, out, NULL, NULL);
}
...
int PKCS5_PBKDF2_HMAC_SHA1(const char* pass, int passlen, const unsigned char* salt, int saltlen, int iter, int keylen, unsigned char* out)
{
    ...
    if ((digest = EVP_MD_fetch(NULL, SN_sha1, NULL)) != NULL)
        r = ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen, iter, digest, keylen, out, NULL, NULL);
    EVP_MD_free(digest);
    return r;
}
...
int PKCS5_v2_PBE_keyivgen_ex(EVP_CIPHER_CTX* ctx, const char* pass, int passlen, ASN1_TYPE* param, const EVP_CIPHER* c, const EVP_MD* md, int en_de,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (!EVP_PBE_find_ex(EVP_PBE_TYPE_KDF, OBJ_obj2nid(pbe2->keyfunc->algorithm), NULL, NULL, NULL, &kdf)) {
...
int PKCS5_v2_PBKDF2_keyivgen_ex(EVP_CIPHER_CTX* ctx, const char* pass, int passlen, ASN1_TYPE* param, const EVP_CIPHER* c, const EVP_MD* md, int en_de,
    OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if (!ossl_pkcs5_pbkdf2_hmac_ex(pass, passlen, salt, saltlen, iter, prfmd, keylen, key, libctx, propq))
        goto err;
...
{
    return PKCS5_v2_PBKDF2_keyivgen_ex(ctx, pass, passlen, param, c, md, en_de, NULL, NULL);
}
