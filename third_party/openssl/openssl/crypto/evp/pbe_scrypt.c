    ...
    unsigned char* key, size_t keylen, OSSL_LIB_CTX* ctx, const char* propq)
{
    ...
    *z++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (unsigned char*)pass, passlen);
    *z++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (unsigned char*)salt, saltlen);
...
{
    return EVP_PBE_scrypt_ex(pass, passlen, salt, saltlen, N, r, p, maxmem, key, keylen, NULL, NULL);
}
