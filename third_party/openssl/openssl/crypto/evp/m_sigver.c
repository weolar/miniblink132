...
static int do_sigver_init(EVP_MD_CTX* ctx, EVP_PKEY_CTX** pctx, const EVP_MD* type, const char* mdname, OSSL_LIB_CTX* libctx, const char* props, ENGINE* e,
    EVP_PKEY* pkey, int ver, const OSSL_PARAM params[])
{
    ...
    if (reinit
        && (pkey != NULL || locpctx->operation != (ver ? EVP_PKEY_OP_VERIFYCTX : EVP_PKEY_OP_SIGNCTX) || (signature = locpctx->op.sig.signature) == NULL
...
reinitialize:
    ...
    if (type != NULL) {
        ...
        if (mdname == NULL && !reinit) {
            if (evp_keymgmt_util_get_deflt_digest_name(tmp_keymgmt, provkey, locmdname, sizeof(locmdname)) > 0) {
...
int EVP_DigestSignUpdate(EVP_MD_CTX* ctx, const void* data, size_t dsize)
{
    ...
    return pctx->op.sig.signature->digest_sign_update(pctx->op.sig.algctx, data, dsize);
...
int EVP_DigestVerifyUpdate(EVP_MD_CTX* ctx, const void* data, size_t dsize)
{
    ...
    return pctx->op.sig.signature->digest_verify_update(pctx->op.sig.algctx, data, dsize);
...
int EVP_DigestVerifyFinal(EVP_MD_CTX* ctx, const unsigned char* sig, size_t siglen)
{
    ...
    if ((ctx->flags & EVP_MD_CTX_FLAG_FINALISE) != 0)
        return pctx->op.sig.signature->digest_verify_final(pctx->op.sig.algctx, sig, siglen);
    dctx = EVP_PKEY_CTX_dup(pctx);
    if (dctx == NULL)
        return 0;

    r = dctx->op.sig.signature->digest_verify_final(dctx->op.sig.algctx, sig, siglen);
    EVP_PKEY_CTX_free(dctx);
    return r;
