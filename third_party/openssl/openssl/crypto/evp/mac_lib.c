...
EVP_MAC_CTX* EVP_MAC_CTX_new(EVP_MAC* mac)
{
    ...
    if (ctx == NULL || (ctx->algctx = mac->newctx(ossl_provider_ctx(mac->prov))) == NULL || !EVP_MAC_up_ref(mac)) {
...
static int evp_mac_final(EVP_MAC_CTX* ctx, int xof, unsigned char* out, size_t* outl, size_t outsize)
{
    size_t l;
    int res;
    OSSL_PARAM params[2];
    ...
    size_t keylen, const unsigned char* data, size_t datalen, unsigned char* out, size_t outsize, size_t* outlen)
{
    ...
    if (key == NULL && keylen == 0)
        key = data;
    if ((ctx = EVP_MAC_CTX_new(mac)) != NULL && EVP_MAC_CTX_set_params(ctx, subalg_param) && EVP_MAC_CTX_set_params(ctx, params)
        && EVP_MAC_init(ctx, key, keylen, params) && EVP_MAC_update(ctx, data, datalen) && EVP_MAC_final(ctx, out, &len, outsize)) {
        if (out == NULL) {
