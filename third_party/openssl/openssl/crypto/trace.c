 ...
 * callback function.
 */
static int trace_write(BIO* b, const char* buf, size_t num, size_t* written);
...
static const BIO_METHOD trace_method = {
    BIO_TYPE_SOURCE_SINK, "trace", trace_write, NULL, /* old write */
    NULL, /* read_ex */
    NULL, /* read */
    trace_puts, NULL, /* gets */
    trace_ctrl, /* ctrl */
    NULL, /* create */
    trace_free, /* free */
    NULL, /* callback_ctrl */
...
static int trace_write(BIO* channel, const char* buf, size_t num, size_t* written)
{
    struct trace_data_st* ctx = BIO_get_data(channel);
    size_t cnt = ctx->callback(buf, num, ctx->category, OSSL_TRACE_CTRL_WRITE, ctx->data);
...
#define TRACE_CATEGORY_(name)                                                                                                                                  \
    {                                                                                                                                                          \
        #name, OSSL_TRACE_CATEGORY_##name                                                                                                                      \
    }

static const struct trace_category_st trace_categories[OSSL_TRACE_CATEGORY_NUM] = { TRACE_CATEGORY_(ALL), TRACE_CATEGORY_(TRACE), TRACE_CATEGORY_(INIT),
    TRACE_CATEGORY_(TLS), TRACE_CATEGORY_(TLS_CIPHER), TRACE_CATEGORY_(CONF), TRACE_CATEGORY_(ENGINE_TABLE), TRACE_CATEGORY_(ENGINE_REF_COUNT),
    TRACE_CATEGORY_(PKCS5V2), TRACE_CATEGORY_(PKCS12_KEYGEN), TRACE_CATEGORY_(PKCS12_DECRYPT), TRACE_CATEGORY_(X509V3_POLICY), TRACE_CATEGORY_(BN_CTX),
    TRACE_CATEGORY_(CMP), TRACE_CATEGORY_(STORE), TRACE_CATEGORY_(DECODER), TRACE_CATEGORY_(ENCODER), TRACE_CATEGORY_(REF_COUNT) };
...
const char* OSSL_trace_get_category_name(int num)
{
     ...
     */
    if (!ossl_assert(trace_categories[num].name != NULL) || !ossl_assert(trace_categories[num].num == num))
        return NULL;
...
static int set_trace_data(int category, int type, BIO** channel, const char** prefix, const char** suffix, int (*attach_cb)(int, int, const void*),
...
void ossl_trace_cleanup(void)
{
    ...
    for (category = 0; category < OSSL_TRACE_CATEGORY_NUM; category++) {
        ...
        if (category == OSSL_TRACE_CATEGORY_TRACE)
            continue;
        set_trace_data(category, 0, &channel, &prefix, &suffix, trace_attach_cb, trace_detach_cb);
    }
    set_trace_data(OSSL_TRACE_CATEGORY_TRACE, 0, &channel, &prefix, &suffix, trace_attach_cb, trace_detach_cb);
...
int OSSL_trace_set_channel(int category, BIO* channel)
{
    ...
    if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
        return set_trace_data(category, SIMPLE_CHANNEL, &channel, NULL, NULL, trace_attach_cb, trace_detach_cb);
#endif
    return 0;
}
...
int OSSL_trace_set_callback(int category, OSSL_trace_cb callback, void* data)
{
    ...
    if (callback != NULL) {
        if ((channel = BIO_new(&trace_method)) == NULL || (trace_data = OPENSSL_zalloc(sizeof(struct trace_data_st))) == NULL)
            goto err;
    ...
    if (!set_trace_data(category, CALLBACK_CHANNEL, &channel, NULL, NULL, trace_attach_w_callback_cb, trace_detach_cb))
        goto err;
...
#ifndef OPENSSL_NO_TRACE
    if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
        return set_trace_data(category, 0, NULL, &prefix, NULL, trace_attach_cb, trace_detach_cb);
#endif
    return 0;
}
...
#ifndef OPENSSL_NO_TRACE
    if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
        return set_trace_data(category, 0, NULL, NULL, &suffix, trace_attach_cb, trace_detach_cb);
#endif
    return 0;
}
...
BIO* OSSL_trace_begin(int category)
{
    ...
    if (channel != NULL) {
        ...
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, OSSL_TRACE_CTRL_BEGIN, prefix == NULL ? 0 : strlen(prefix), prefix);
            break;
        }
    }
#endif
...
void OSSL_trace_end(int category, BIO* channel)
{
    ...
    if (channel != NULL && ossl_assert(channel == current_channel)) {
        ...
        case SIMPLE_CHANNEL:
            ...
            break;
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, OSSL_TRACE_CTRL_END, suffix == NULL ? 0 : strlen(suffix), suffix);
            break;
        }
