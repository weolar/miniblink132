...
static int dh_cms_set_peerkey(EVP_PKEY_CTX* pctx, X509_ALGOR* alg, ASN1_BIT_STRING* pubkey)
{
    ...
    pkpeer = EVP_PKEY_new();
    if (pkpeer == NULL || !EVP_PKEY_copy_parameters(pkpeer, pk) || !EVP_PKEY_set1_encoded_public_key(pkpeer, buf, plen))
        goto err;
...
static int dh_cms_set_shared_info(EVP_PKEY_CTX* pctx, CMS_RecipientInfo* ri)
{
    ...
    if (EVP_PKEY_CTX_set_dh_kdf_type(pctx, EVP_PKEY_DH_KDF_X9_42) <= 0 || EVP_PKEY_CTX_set_dh_kdf_md(pctx, EVP_sha1()) <= 0)
        goto err;
