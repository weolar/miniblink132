...
CMS_RecipientInfo* CMS_add0_recipient_password(
    CMS_ContentInfo* cms, int iter, int wrap_nid, int pbe_nid, unsigned char* pass, ossl_ssize_t passlen, const EVP_CIPHER* kekciph)
{
...
static int kek_unwrap_key(unsigned char* out, size_t* outlen, const unsigned char* in, size_t inlen, EVP_CIPHER_CTX* ctx)
{
    ...
    if (!EVP_DecryptUpdate(ctx, tmp + inlen - 2 * blocklen, &outl, in + inlen - 2 * blocklen, blocklen * 2)
        /*
...
static int kek_wrap_key(unsigned char* out, size_t* outlen, const unsigned char* in, size_t inlen, EVP_CIPHER_CTX* ctx, const CMS_CTX* cms_ctx)
{
    size_t blocklen = EVP_CIPHER_CTX_get_block_size(ctx);
    size_t olen;
    int dummy;
    ...
    if (out) {
        ...
        if (olen > inlen + 4 && RAND_bytes_ex(ossl_cms_ctx_get0_libctx(cms_ctx), out + 4 + inlen, olen - 4 - inlen, 0) <= 0)
            return 0;
        /* Encrypt twice */
        if (!EVP_EncryptUpdate(ctx, out, &dummy, out, olen) || !EVP_EncryptUpdate(ctx, out, &dummy, out, olen))
            return 0;
    }
...
int ossl_cms_RecipientInfo_pwri_crypt(const CMS_ContentInfo* cms, CMS_RecipientInfo* ri, int en_de)
{
    ...
    kekalg = ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(X509_ALGOR), algtmp->parameter);
