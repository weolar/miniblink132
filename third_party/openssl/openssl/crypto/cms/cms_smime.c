...
CMS_ContentInfo* CMS_EncryptedData_encrypt_ex(
    BIO* in, const EVP_CIPHER* cipher, const unsigned char* key, size_t keylen, unsigned int flags, OSSL_LIB_CTX* libctx, const char* propq)
{
    CMS_ContentInfo* cms;
...
static int cms_signerinfo_verify_cert(
    CMS_SignerInfo* si, X509_STORE* store, STACK_OF(X509) * certs, STACK_OF(X509_CRL) * crls, STACK_OF(X509) * *chain, const CMS_CTX* cms_ctx)
{
    X509_STORE_CTX* ctx;
    X509* signer;
    int i, j, r = 0;
    ...
    if (i <= 0) {
        ...
        ERR_raise_data(ERR_LIB_CMS, CMS_R_CERTIFICATE_VERIFY_ERROR, "Verify error: %s", X509_verify_cert_error_string(j));
        goto err;
    }
    r = 1;
...
CMS_ContentInfo* CMS_sign_receipt(CMS_SignerInfo* si, X509* signcert, EVP_PKEY* pkey, STACK_OF(X509) * certs, unsigned int flags)
{
    ...
    cms = CMS_sign_ex(NULL, NULL, certs, NULL, flags, ossl_cms_ctx_get0_libctx(ctx), ossl_cms_ctx_get0_propq(ctx));
    if (cms == NULL)
        goto err;
...
CMS_ContentInfo* CMS_encrypt_ex(STACK_OF(X509) * certs, BIO* data, const EVP_CIPHER* cipher, unsigned int flags, OSSL_LIB_CTX* libctx, const char* propq)
{
    CMS_ContentInfo* cms;
    int i;
    X509* recip;

    cms = (EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER) ? CMS_AuthEnvelopedData_create_ex(cipher, libctx, propq)
