...
static void cms_sd_set_version(CMS_SignedData* sd)
{
    ...
    if ((OBJ_obj2nid(sd->encapContentInfo->eContentType) != NID_pkcs7_data) && (sd->version < 3))
        sd->version = 3;
...
static int cms_set_si_contentType_attr(CMS_ContentInfo* cms, CMS_SignerInfo* si)
{
    ...
    return CMS_signed_add1_attr_by_NID(si, NID_pkcs9_contentType, V_ASN1_OBJECT, ctype, -1) > 0;
}
...
static int cms_copy_messageDigest(CMS_ContentInfo* cms, CMS_SignerInfo* si)
{
    ...
    for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
        ...
        if (OBJ_cmp(si->digestAlgorithm->algorithm, sitmp->digestAlgorithm->algorithm))
            continue;
        messageDigest = CMS_signed_get0_data_by_OBJ(sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
        if (!messageDigest) {
...
static int ossl_cms_add1_signing_cert(CMS_SignerInfo* si, const ESS_SIGNING_CERT* sc)
{
    ...
    OPENSSL_free(pp);
    ret = CMS_signed_add1_attr_by_NID(si, NID_id_smime_aa_signingCertificate, V_ASN1_SEQUENCE, seq, -1);
    ASN1_STRING_free(seq);
    return ret;
}

/* Add SigningCertificateV2 signed attribute to the signer info. */
static int ossl_cms_add1_signing_cert_v2(CMS_SignerInfo* si, const ESS_SIGNING_CERT_V2* sc)
{
    ASN1_STRING* seq = NULL;
    unsigned char *p, *pp = NULL;
    ...
    OPENSSL_free(pp);
    ret = CMS_signed_add1_attr_by_NID(si, NID_id_smime_aa_signingCertificateV2, V_ASN1_SEQUENCE, seq, -1);
    ASN1_STRING_free(seq);
    return ret;
}
...
CMS_SignerInfo* CMS_add1_signer(CMS_ContentInfo* cms, X509* signer, EVP_PKEY* pk, const EVP_MD* md, unsigned int flags)
{
    ...
    if (md == NULL) {
        ...
        if (EVP_PKEY_get_default_digest_nid(pk, &def_nid) <= 0) {
            ERR_raise_data(ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST, "pkey nid=%d", EVP_PKEY_get_id(pk));
            goto err;
        }
        md = EVP_get_digestbynid(def_nid);
        if (md == NULL) {
            ERR_raise_data(ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST, "default md nid=%d", def_nid);
            goto err;
        }
    }
    ...
    if (!(flags & CMS_KEY_PARAM) && !cms_sd_asn1_ctrl(si, 0)) {
        ERR_raise_data(ERR_LIB_CMS, CMS_R_UNSUPPORTED_SIGNATURE_ALGORITHM, "pkey nid=%d", EVP_PKEY_get_id(pk));
        goto err;
    }
    if (!(flags & CMS_NOATTR)) {
        /*
        ...
        if (flags & CMS_CADES) {
            ...
            if (md == NULL || EVP_MD_is_a(md, SN_sha1)) {
                if ((sc = OSSL_ESS_signing_cert_new_init(signer, NULL, 1)) == NULL)
                    goto err;
                add_sc = ossl_cms_add1_signing_cert(si, sc);
                ESS_SIGNING_CERT_free(sc);
            } else {
                if ((sc2 = OSSL_ESS_signing_cert_v2_new_init(md, signer, NULL, 1)) == NULL)
                    goto err;
    ...
    if (flags & CMS_KEY_PARAM) {
        if (flags & CMS_NOATTR) {
            si->pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey, ossl_cms_ctx_get0_propq(ctx));
            if (si->pctx == NULL)
                goto err;
            ...
            if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
                goto err;
        } else if (EVP_DigestSignInit_ex(si->mctx, &si->pctx, EVP_MD_get0_name(md), ossl_cms_ctx_get0_libctx(ctx), ossl_cms_ctx_get0_propq(ctx), pk, NULL)
            <= 0) {
            goto err;
        }
    }
...
STACK_OF(X509) * CMS_get0_signers(CMS_ContentInfo* cms)
{
    ...
    for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
        si = sk_CMS_SignerInfo_value(sinfos, i);
        if (si->signer != NULL) {
            if (!ossl_x509_add_cert_new(&signers, si->signer, X509_ADD_FLAG_DEFAULT)) {
...
static int cms_SignerInfo_content_sign(CMS_ContentInfo* cms, CMS_SignerInfo* si, BIO* chain)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        ...
        if (!EVP_DigestFinal_ex(mctx, md, &mdlen))
            goto err;
        if (!CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, md, mdlen))
            goto err;
        ...
        if (!EVP_SignFinal_ex(mctx, sig, &siglen, si->pkey, ossl_cms_ctx_get0_libctx(ctx), ossl_cms_ctx_get0_propq(ctx))) {
...
int CMS_SignerInfo_sign(CMS_SignerInfo* si)
{
    ...
    if (OBJ_obj2txt(md_name, sizeof(md_name), si->digestAlgorithm->algorithm, 0) <= 0)
        return 0;
    ...
    if (si->pctx)
        ...
        if (EVP_DigestSignInit_ex(mctx, &pctx, md_name, ossl_cms_ctx_get0_libctx(ctx), ossl_cms_ctx_get0_propq(ctx), si->pkey, NULL) <= 0)
            goto err;
        si->pctx = pctx;
    }
...
int CMS_SignerInfo_verify(CMS_SignerInfo* si)
{
    ...
    mctx = si->mctx;
    if (EVP_DigestVerifyInit_ex(mctx, &si->pctx, EVP_MD_get0_name(md), libctx, propq, si->pkey, NULL) <= 0)
        goto err;
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo* si, BIO* chain)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        os = CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
        if (os == NULL) {
            ERR_raise(ERR_LIB_CMS, CMS_R_ERROR_READING_MESSAGEDIGEST_ATTRIBUTE);
            goto err;
        }
    ...
    if (os != NULL) {
        ...
        pkctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey, ossl_cms_ctx_get0_propq(ctx));
        if (pkctx == NULL)
            goto err;
...
int CMS_add_simple_smimecap(STACK_OF(X509_ALGOR) * *algs, int algnid, int keysize)
{
    ...
    if (!X509_ALGOR_set0(alg, OBJ_nid2obj(algnid), key ? V_ASN1_INTEGER : V_ASN1_UNDEF, key))
        goto err;
    key = NULL;
    if (*algs == NULL)
...
{
    if (!cms_add_cipher_smcap(smcap, NID_aes_256_cbc, -1) || !cms_add_digest_smcap(smcap, NID_id_GostR3411_2012_256, -1)
        || !cms_add_digest_smcap(smcap, NID_id_GostR3411_2012_512, -1) || !cms_add_digest_smcap(smcap, NID_id_GostR3411_94, -1)
        || !cms_add_cipher_smcap(smcap, NID_id_Gost28147_89, -1) || !cms_add_cipher_smcap(smcap, NID_aes_192_cbc, -1)
        || !cms_add_cipher_smcap(smcap, NID_aes_128_cbc, -1) || !cms_add_cipher_smcap(smcap, NID_des_ede3_cbc, -1)
        || !cms_add_cipher_smcap(smcap, NID_rc2_cbc, 128) || !cms_add_cipher_smcap(smcap, NID_rc2_cbc, 64) || !cms_add_cipher_smcap(smcap, NID_des_cbc, -1)
