...
static const struct {
    ...
    { NID_pkcs9_signingTime, CMS_ATTR_F_SIGNED | CMS_ATTR_F_ONLY_ONE | CMS_ATTR_F_ONE_ATTR_VALUE }, { NID_pkcs9_countersignature, CMS_ATTR_F_UNSIGNED },
    /* ESS */
...
{
    if (ossl_x509at_add1_attr_by_txt(&si->signedAttrs, attrname, type, bytes, len))
        return 1;
    return 0;
}
...
{
    if (ossl_x509at_add1_attr_by_txt(&si->unsignedAttrs, attrname, type, bytes, len))
        return 1;
    return 0;
}
 ...
 */
static X509_ATTRIBUTE* cms_attrib_get(int nid, const STACK_OF(X509_ATTRIBUTE) * attrs, int* lastpos)
{
    X509_ATTRIBUTE* at;
    int loc;
...
int ossl_cms_si_check_attributes(const CMS_SignerInfo* si)
{
    ...
    for (i = 0; i < (int)OSSL_NELEM(cms_attribute_properties); ++i) {
        ...
        if (!cms_check_attribute(nid, flags, CMS_ATTR_F_SIGNED, si->signedAttrs, have_signed_attrs)
            || !cms_check_attribute(nid, flags, CMS_ATTR_F_UNSIGNED, si->unsignedAttrs, have_unsigned_attrs)) {
