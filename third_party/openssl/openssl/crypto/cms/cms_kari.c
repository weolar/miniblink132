...
int CMS_RecipientInfo_kari_get0_orig_id(
    CMS_RecipientInfo* ri, X509_ALGOR** pubalg, ASN1_BIT_STRING** pubkey, ASN1_OCTET_STRING** keyid, X509_NAME** issuer, ASN1_INTEGER** sno)
{
...
int CMS_RecipientEncryptedKey_get0_id(
    CMS_RecipientEncryptedKey* rek, ASN1_OCTET_STRING** keyid, ASN1_GENERALIZEDTIME** tm, CMS_OtherKeyAttribute** other, X509_NAME** issuer, ASN1_INTEGER** sno)
{
...
int CMS_RecipientEncryptedKey_cert_cmp(CMS_RecipientEncryptedKey* rek, X509* cert)
{
    ...
    if (rid->type == CMS_REK_ISSUER_SERIAL)
        ...
        return ossl_cms_keyid_cert_cmp(rid->d.rKeyId->subjectKeyIdentifier, cert);
    else
        return -1;
}
...
int CMS_RecipientInfo_kari_set0_pkey_and_peer(CMS_RecipientInfo* ri, EVP_PKEY* pk, X509* peer)
{
    ...
    pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(kari->cms_ctx), pk, ossl_cms_ctx_get0_propq(kari->cms_ctx));
...
static int cms_kek_cipher(unsigned char** pout, size_t* poutlen, const unsigned char* in, size_t inlen, CMS_KeyAgreeRecipientInfo* kari, int enc)
{
    /* Key encryption key */
...
static int cms_kari_set_originator_private_key(CMS_KeyAgreeRecipientInfo* kari, EVP_PKEY* originatorPrivKey)
{
    ...
    pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), originatorPrivKey, ossl_cms_ctx_get0_propq(ctx));
    if (pctx == NULL)
        goto err;
    if (EVP_PKEY_derive_init(pctx) <= 0)
        goto err;
...
int ossl_cms_RecipientInfo_kari_init(
    CMS_RecipientInfo* ri, X509* recip, EVP_PKEY* recipPubKey, X509* originator, EVP_PKEY* originatorPrivKey, unsigned int flags, const CMS_CTX* ctx)
{
...
enc:
    fetched_kekcipher = EVP_CIPHER_fetch(ossl_cms_ctx_get0_libctx(cms_ctx), kekcipher_name, ossl_cms_ctx_get0_propq(cms_ctx));
