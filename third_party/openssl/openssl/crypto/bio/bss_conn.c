    ...
    for (;;) {
        ...
        case BIO_CONN_S_BEFORE:
            if (c->param_hostname == NULL && c->param_service == NULL) {
                ERR_raise_data(ERR_LIB_BIO, BIO_R_NO_HOSTNAME_OR_SERVICE_SPECIFIED, "hostname=%s service=%s", c->param_hostname, c->param_service);
                goto exit_loop;
            }
        ...
        case BIO_CONN_S_GET_ADDR: {
            ...
            if (BIO_lookup(c->param_hostname, c->param_service, BIO_LOOKUP_CLIENT, family, SOCK_STREAM, &c->addr_first) == 0)
                goto exit_loop;
        }
        ...
        case BIO_CONN_S_CREATE_SOCKET:
            ret = BIO_socket(BIO_ADDRINFO_family(c->addr_iter), BIO_ADDRINFO_socktype(c->addr_iter), BIO_ADDRINFO_protocol(c->addr_iter), 0);
            if (ret == (int)INVALID_SOCKET) {
                ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling socket(%s, %s)", c->param_hostname, c->param_service);
        ...
        case BIO_CONN_S_CONNECT:
            ...
            ERR_set_mark();
            ret = BIO_connect(b->num, BIO_ADDRINFO_address(c->addr_iter), BIO_SOCK_KEEPALIVE | c->connect_mode);
            b->retry_reason = 0;
            if (ret == 0) {
                if (BIO_sock_should_retry(ret)) {
                    ...
                    ERR_clear_last_mark();
                    ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling connect(%s, %s)", c->param_hostname, c->param_service);
        ...
        case BIO_CONN_S_BLOCKED_CONNECT:
            ...
            if (i != 0) {
                ...
                ERR_raise_data(ERR_LIB_SYS, i, "calling connect(%s, %s)", c->param_hostname, c->param_service);
    ...
    case BIO_C_SET_CONNECT:
        if (ptr != NULL) {
            ...
            if (num == 0) { /* BIO_set_conn_hostname */
                ...
                ret = BIO_parse_hostserv(ptr, &data->param_hostname, &data->param_service, BIO_PARSE_PRIO_HOST);
