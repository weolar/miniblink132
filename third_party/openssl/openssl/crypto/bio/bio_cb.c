...
long BIO_debug_callback_ex(BIO* bio, int cmd, const char* argp, size_t len, int argi, long argl, int ret, size_t* processed)
{
    BIO* b;
    char buf[256];
    char* p;
...
long BIO_debug_callback(BIO* bio, int cmd, const char* argp, int argi, long argl, long ret)
{
    ...
    BIO_debug_callback_ex(bio, cmd, argp, (size_t)argi, argi, argl, ret > 0 ? 1 : (int)ret, &processed);
    return ret;
}
#endif
