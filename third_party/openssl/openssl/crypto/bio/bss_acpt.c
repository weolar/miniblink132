...
static const BIO_METHOD methods_acceptp = {
    BIO_TYPE_ACCEPT, "socket accept", bwrite_conv, acpt_write, bread_conv, acpt_read, acpt_puts, NULL, /* connect_gets,         */
    ...
    for (;;) {
        ...
        case ACPT_S_BEFORE:
            if (c->param_addr == NULL && c->param_serv == NULL) {
                ERR_raise_data(ERR_LIB_BIO, BIO_R_NO_ACCEPT_ADDR_OR_SERVICE_SPECIFIED, "hostname=%s, service=%s", c->param_addr, c->param_serv);
                goto exit_loop;
            }
        ...
        case ACPT_S_GET_ADDR: {
            ...
            if (BIO_lookup(c->param_addr, c->param_serv, BIO_LOOKUP_SERVER, family, SOCK_STREAM, &c->addr_first) == 0)
                goto exit_loop;
        }
        ...
        case ACPT_S_CREATE_SOCKET:
            ERR_set_mark();
            s = BIO_socket(BIO_ADDRINFO_family(c->addr_iter), BIO_ADDRINFO_socktype(c->addr_iter), BIO_ADDRINFO_protocol(c->addr_iter), 0);
        ...
        case ACPT_S_LISTEN: {
            if (!BIO_listen(c->accept_sock, BIO_ADDRINFO_address(c->addr_iter), c->bind_mode)) {
...

            {
                ...
                if (!BIO_sock_info(c->accept_sock, BIO_SOCK_INFO_ADDRESS, &info)) {
                    BIO_closesocket(c->accept_sock);
                    goto exit_loop;
                }
            }
        ...
        case ACPT_S_ACCEPT:
            ...
            s = BIO_accept_ex(c->accept_sock, &c->cache_peer_addr, c->accepted_mode);
    ...
    case BIO_C_SET_ACCEPT:
        if (ptr != NULL) {
            if (num == 0) {
                ...
                ret = BIO_parse_hostserv(ptr, &data->param_addr, &data->param_serv, BIO_PARSE_PRIO_SERV);
