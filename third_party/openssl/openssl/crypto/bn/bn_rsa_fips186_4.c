...
static const BN_ULONG inv_sqrt_2_val[]
    = { BN_DEF(0x83339916UL, 0xED17AC85UL), BN_DEF(0x893BA84CUL, 0x1D6F60BAUL), BN_DEF(0x754ABE9FUL, 0x597D89B3UL), BN_DEF(0xF9DE6484UL, 0xB504F333UL) };

const BIGNUM ossl_bn_inv_sqrt_2 = { (BN_ULONG*)inv_sqrt_2_val, OSSL_NELEM(inv_sqrt_2_val), OSSL_NELEM(inv_sqrt_2_val), 0, BN_FLG_STATIC_DATA };
 ...
 */
static int bn_rsa_fips186_4_find_aux_prob_prime(const BIGNUM* Xp1, BIGNUM* p1, BN_CTX* ctx, BN_GENCB* cb)
{
    int ret = 0;
    int i = 0;
    int tmp = 0;
 ...
 * Returns: 1 on success otherwise it returns 0.
 */
int ossl_bn_rsa_fips186_4_gen_prob_primes(BIGNUM* p, BIGNUM* Xpout, BIGNUM* p1, BIGNUM* p2, const BIGNUM* Xp, const BIGNUM* Xp1, const BIGNUM* Xp2, int nlen,
    const BIGNUM* e, BN_CTX* ctx, BN_GENCB* cb)
{
    ...
    if (Xp1 == NULL) {
        ...
        if (!BN_priv_rand_ex(Xp1i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0, ctx))
            goto err;
    }
    /* (Steps 4.1/5.1): Randomly generate Xp2 if it is not passed in */
    if (Xp2 == NULL) {
        /* Set the top and bottom bits to make it odd and the correct size */
        if (!BN_priv_rand_ex(Xp2i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0, ctx))
            goto err;
    }

    /* (Steps 4.2/5.2) - find first auxiliary probable primes */
    if (!bn_rsa_fips186_4_find_aux_prob_prime(Xp1i, p1i, ctx, cb) || !bn_rsa_fips186_4_find_aux_prob_prime(Xp2i, p2i, ctx, cb))
        goto err;
    ...
    if (!ossl_bn_rsa_fips186_4_derive_prime(p, Xpout, Xp, p1i, p2i, nlen, e, ctx, cb))
        goto err;
    ret = 1;
err:
 ...
 */
int ossl_bn_rsa_fips186_4_derive_prime(
    BIGNUM* Y, BIGNUM* X, const BIGNUM* Xin, const BIGNUM* r1, const BIGNUM* r2, int nlen, const BIGNUM* e, BN_CTX* ctx, BN_GENCB* cb)
{
    int ret = 0;
    int i, imax;
    int bits = nlen >> 1;
    ...
    if (Xin == NULL) {
        if (bits < BN_num_bits(&ossl_bn_inv_sqrt_2))
            goto err;
        if (!BN_lshift(base, &ossl_bn_inv_sqrt_2, bits - BN_num_bits(&ossl_bn_inv_sqrt_2)) || !BN_lshift(range, BN_value_one(), bits)
    ...
    if (!(BN_lshift1(r1x2, r1)
            ...
            && BN_mod_inverse(R, r2, r1x2, ctx) && BN_mul(R, R, r2, ctx) /* R = (r2^-1 mod 2r1) * r2 */
            && BN_mod_inverse(tmp, r1x2, r2, ctx) && BN_mul(tmp, tmp, r1x2, ctx) /* tmp = (2r1^-1 mod r2)*2r1 */
            && BN_sub(R, R, tmp)
            /* Calculate 2r1r2 */
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (BN_copy(y1, Y) == NULL || !BN_sub_word(y1, 1) || !BN_gcd(tmp, y1, e, ctx))
                goto err;
            if (BN_is_one(tmp)) {
