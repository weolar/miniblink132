...
static int bnrand(BNRAND_FLAG flag, BIGNUM* rnd, int bits, int top, int bottom, unsigned int strength, BN_CTX* ctx)
{
    ...
    b = flag == NORMAL ? RAND_bytes_ex(libctx, buf, bytes, strength) : RAND_priv_bytes_ex(libctx, buf, bytes, strength);
    if (b <= 0)
        goto err;
...
static int bnrand_range(BNRAND_FLAG flag, BIGNUM* r, const BIGNUM* range, unsigned int strength, BN_CTX* ctx)
{
    int n;
    int count = 100;
    ...
    if (n == 1)
         ...
         */
        do {
            if (!bnrand(flag, r, n + 1, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, strength, ctx))
                return 0;
        ...
        do {
            ...
            if (!bnrand(flag, r, n, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, 0, ctx))
                return 0;
...
int BN_generate_dsa_nonce(BIGNUM* out, const BIGNUM* range, const BIGNUM* priv, const unsigned char* message, size_t message_len, BN_CTX* ctx)
{
    ...
    for (done = 0; done < num_k_bytes;) {
        ...
        if (!EVP_DigestInit_ex(mdctx, md, NULL) || !EVP_DigestUpdate(mdctx, &done, sizeof(done))
            || !EVP_DigestUpdate(mdctx, private_bytes, sizeof(private_bytes)) || !EVP_DigestUpdate(mdctx, message, message_len)
            || !EVP_DigestUpdate(mdctx, random_bytes, sizeof(random_bytes)) || !EVP_DigestFinal_ex(mdctx, digest, NULL))
            goto err;
