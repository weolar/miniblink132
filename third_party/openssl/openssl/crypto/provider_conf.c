 ...
 * < 0 for fatal errors
 */
static int provider_conf_params_internal(
    OSSL_PROVIDER* prov, OSSL_PROVIDER_INFO* provinfo, const char* name, const char* value, const CONF* cnf, STACK_OF(OPENSSL_CSTRING) * visited)
{
    ...
    if (sect != NULL) {
        ...
        for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
            ...
            rc = provider_conf_params_internal(prov, provinfo, buffer, sectconf->value, cnf, visited);
            if (rc < 0) {
                sk_OPENSSL_CSTRING_pop(visited);
                return rc;
            }
...
static int provider_conf_load(OSSL_LIB_CTX* libctx, const char* name, const char* value, const CONF* cnf)
{
    ...
    if (activate) {
        PROVIDER_CONF_GLOBAL* pcgbl = ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_PROVIDER_CONF_INDEX, &provider_conf_ossl_ctx_method);
            ...
            */
            ...
            if (ok > 0) {
                if (!ossl_provider_activate(prov, 1, 0)) {
                    ...
                    ok = 0;
                } else if (actual != prov && !ossl_provider_activate(actual, 1, 0)) {
...
static int provider_conf_init(CONF_IMODULE* md, const CONF* cnf)
{
    ...
    for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
        ...
        if (!provider_conf_load(NCONF_get0_libctx((CONF*)cnf), cval->name, cval->value, cnf))
            return 0;
    }
