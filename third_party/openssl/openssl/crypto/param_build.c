...
static OSSL_PARAM_BLD_DEF* param_push(OSSL_PARAM_BLD* bld, const char* key, int size, size_t alloc, int type, int secure)
{
...
static int param_push_num(OSSL_PARAM_BLD* bld, const char* key, void* num, size_t size, int type)
{
...
static OSSL_PARAM* param_bld_convert(OSSL_PARAM_BLD* bld, OSSL_PARAM* param, OSSL_PARAM_ALIGNED_BLOCK* blk, OSSL_PARAM_ALIGNED_BLOCK* secure)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        } else if (pd->type == OSSL_PARAM_OCTET_PTR || pd->type == OSSL_PARAM_UTF8_PTR) {
            /* PTR */
            *(const void**)p = pd->string;
        } else if (pd->type == OSSL_PARAM_OCTET_STRING || pd->type == OSSL_PARAM_UTF8_STRING) {
            if (pd->string != NULL)
