...
int ossl_dh_params_fromdata(DH* dh, const OSSL_PARAM params[])
{
    ...
    if (param_priv_len != NULL && (!OSSL_PARAM_get_long(param_priv_len, &priv_len) || !DH_set_length(dh, priv_len)))
        return 0;
...
int ossl_dh_key_fromdata(DH* dh, const OSSL_PARAM params[], int include_private)
{
    ...
    if (include_private && param_priv_key != NULL && !OSSL_PARAM_get_BN(param_priv_key, &priv_key))
        goto err;
...
int ossl_dh_params_todata(DH* dh, OSSL_PARAM_BLD* bld, OSSL_PARAM params[])
{
    ...
    if (!ossl_ffc_params_todata(ossl_dh_get0_params(dh), bld, params))
        return 0;
    if (l > 0 && !ossl_param_build_set_long(bld, params, OSSL_PKEY_PARAM_DH_PRIV_LEN, l))
        return 0;
    return 1;
}
...
int ossl_dh_key_todata(DH* dh, OSSL_PARAM_BLD* bld, OSSL_PARAM params[], int include_private)
{
    ...
    if (priv != NULL && include_private && !ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_PRIV_KEY, priv))
        return 0;
...
DH* ossl_dh_dup(const DH* dh, int selection)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0 && !ossl_ffc_params_copy(&dupkey->params, &dh->params))
        goto err;

    dupkey->flags = dh->flags;

    if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0
        && ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) == 0 || !dh_bn_dup_check(&dupkey->pub_key, dh->pub_key)))
        goto err;

    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0
        && ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) == 0 || !dh_bn_dup_check(&dupkey->priv_key, dh->priv_key)))
        goto err;
...
#ifndef FIPS_MODULE
DH* ossl_dh_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf, OSSL_LIB_CTX* libctx, const char* propq)
{
    ...
    if ((privkey_bn = BN_secure_new()) == NULL || !ASN1_INTEGER_to_BN(privkey, privkey_bn)) {
