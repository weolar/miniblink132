...
static DH_METHOD dh_ossl = { "OpenSSL DH Method", generate_key, ossl_dh_compute_key, dh_bn_mod_exp, dh_init, dh_finish, DH_FLAG_FIPS_METHOD, NULL, NULL };
    ...
    if (generate_new_key) {
        ...
        if (DH_get_nid(dh) != NID_undef) {
            ...
            if (dh->params.q == NULL) {
                ...
                if (dh->length != 0 && dh->length >= BN_num_bits(dh->params.p))
                    goto err;
