...
{
    if (X509at_add1_attr_by_NID(&bag->attrib, NID_localKeyID, V_ASN1_OCTET_STRING, name, namelen) != NULL)
        return 1;
    else
        return 0;
}
...
int PKCS8_add_keyusage(PKCS8_PRIV_KEY_INFO* p8, int usage)
{
    ...
    return PKCS8_pkey_add1_attr_by_NID(p8, NID_key_usage, V_ASN1_BIT_STRING, &us_val, 1);
}
...
{
    if (X509at_add1_attr_by_NID(&bag->attrib, NID_friendlyName, MBSTRING_ASC, (unsigned char*)name, namelen) != NULL)
        return 1;
    else
        return 0;
}
...
{
    if (X509at_add1_attr_by_NID(&bag->attrib, NID_friendlyName, MBSTRING_UTF8, (unsigned char*)name, namelen) != NULL)
        return 1;
    else
        return 0;
}
...
{
    if (X509at_add1_attr_by_NID(&bag->attrib, NID_friendlyName, MBSTRING_BMP, name, namelen) != NULL)
        return 1;
    else
        return 0;
}
...
{
    if (X509at_add1_attr_by_NID(&bag->attrib, NID_ms_csp_name, MBSTRING_ASC, (unsigned char*)name, namelen) != NULL)
        return 1;
    else
        return 0;
}
...
char* PKCS12_get_friendlyname(PKCS12_SAFEBAG* bag)
{
    ...
    if (atype->type != V_ASN1_BMPSTRING)
        return NULL;
    return OPENSSL_uni2utf8(atype->value.bmpstring->data, atype->value.bmpstring->length);
}
