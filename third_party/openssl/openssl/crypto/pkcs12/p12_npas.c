...
static int newpass_p12(PKCS12* p12, const char* oldpass, const char* newpass)
{
    ...
    for (i = 0; i < sk_PKCS7_num(asafes); i++) {
        ...
        if (bagnid == NID_pkcs7_data) {
            ...
            if (p7->d.encrypted == NULL || !alg_get(p7->d.encrypted->enc_data->algorithm, &pbe_nid, &pbe_iter, &pbe_saltlen))
                goto err;
        } else {
            continue;
        }
        ...
        if (bagnid == NID_pkcs7_data)
            ...
            p7new = PKCS12_pack_p7encdata(pbe_nid, newpass, -1, NULL, pbe_saltlen, pbe_iter, bags);
...
static int newpass_bag(PKCS12_SAFEBAG* bag, const char* oldpass, const char* newpass)
{
    ...
    p8new = PKCS8_encrypt(p8_nid, NULL, newpass, -1, NULL, p8_saltlen, p8_iter, p8);
    PKCS8_PRIV_KEY_INFO_free(p8);
    if (p8new == NULL)
        return 0;
