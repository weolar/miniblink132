...
PKCS7* PKCS12_pack_p7encdata_ex(int pbe_nid, const char* pass, int passlen, unsigned char* salt, int saltlen, int iter, STACK_OF(PKCS12_SAFEBAG) * bags,
    OSSL_LIB_CTX* ctx, const char* propq)
{
    ...
    if (!(p7->d.encrypted->enc_data->enc_data = PKCS12_item_i2d_encrypt_ex(pbe, ASN1_ITEM_rptr(PKCS12_SAFEBAGS), pass, passlen, bags, 1, ctx, propq))) {
...
{
    return PKCS12_pack_p7encdata_ex(pbe_nid, pass, passlen, salt, saltlen, iter, bags, NULL, NULL);
}
...
STACK_OF(PKCS12_SAFEBAG) * PKCS12_unpack_p7encdata(PKCS7* p7, const char* pass, int passlen)
{
    ...
    return PKCS12_item_decrypt_d2i_ex(p7->d.encrypted->enc_data->algorithm, ASN1_ITEM_rptr(PKCS12_SAFEBAGS), pass, passlen, p7->d.encrypted->enc_data->enc_data,
        1, p7->ctx.libctx, p7->ctx.propq);
}
...
STACK_OF(PKCS7) * PKCS12_unpack_authsafes(const PKCS12* p12)
{
    ...
    p7s = ASN1_item_unpack(p12->authsafes->d.data, ASN1_ITEM_rptr(PKCS12_AUTHSAFES));
    if (p7s != NULL) {
