...
static OSSL_CMP_PKISI* process_cert_request(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* cert_req, ossl_unused int certReqId, const OSSL_CRMF_MSG* crm,
    const X509_REQ* p10cr, X509** certOut, STACK_OF(X509) * *chainOut, STACK_OF(X509) * *caPubs)
{
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(cert_req)) {
    ...
    *caPubs = NULL;
    ...
    if (OSSL_CMP_MSG_get_bodytype(cert_req) == OSSL_CMP_KUR && crm != NULL && ctx->certOut != NULL) {
        ...
        if (issuer != NULL && X509_NAME_cmp(issuer, OSSL_CRMF_CERTID_get0_issuer(cid)) != 0) {
            ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_CERTID);
            return NULL;
        }
        if (serial != NULL && ASN1_INTEGER_cmp(serial, OSSL_CRMF_CERTID_get0_serialNumber(cid)) != 0) {
            ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_CERTID);
            return NULL;
        }
    }
...
static OSSL_CMP_PKISI* process_rr(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* rr, const X509_NAME* issuer, const ASN1_INTEGER* serial)
{
    ...
    if (ctx->certOut == NULL || ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(rr)) {
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return NULL;
    }
    ...
    if (X509_NAME_cmp(issuer, X509_get_issuer_name(ctx->certOut)) != 0 || ASN1_INTEGER_cmp(serial, X509_get0_serialNumber(ctx->certOut)) != 0) {
...
static int process_genm(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* genm, const STACK_OF(OSSL_CMP_ITAV) * in, STACK_OF(OSSL_CMP_ITAV) * *out)
{
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(genm) || sk_OSSL_CMP_ITAV_num(in) > 1) {
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return 0;
    }
    ...
    if (sk_ASN1_UTF8STRING_num(errorDetails) <= 0) {
        ...
        for (i = 0; i < sk_ASN1_UTF8STRING_num(errorDetails); i++) {
            if (i > 0)
                BIO_printf(bio_err, ", ");
            BIO_printf(bio_err, "\"");
            ASN1_STRING_print(bio_err, sk_ASN1_UTF8STRING_value(errorDetails, i));
            BIO_printf(bio_err, "\"");
        }
        BIO_printf(bio_err, "\n");
    }
...
static int process_certConf(
    OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* certConf, ossl_unused int certReqId, const ASN1_OCTET_STRING* certHash, const OSSL_CMP_PKISI* si)
{
    mock_srv_ctx* ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(certConf) || ctx->certOut == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return 0;
    }
...
static int process_pollReq(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* pollReq, ossl_unused int certReqId, OSSL_CMP_MSG** certReq, int64_t* check_after)
{
    mock_srv_ctx* ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(pollReq)) {
        *certReq = NULL;
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return 0;
    }
