...
char* opt_init(int ac, char** av, const OPTIONS* o)
{
    ...
    for (; o->name != NULL && o->name != OPT_PARAM_STR; ++o) {
        ...
        if (o->name == OPT_HELP_STR || o->name == OPT_MORE_STR || o->name == OPT_SECTION_STR)
            continue;
#ifndef NDEBUG
        i = o->valtype;
        ...
        for (next = o + 1; next->name; ++next) {
             ...
             */
            duplicated = next->retval != OPT_DUP && strcmp(o->name, next->name) == 0;
            if (duplicated) {
...
static OPT_PAIR formats[] = { { "PEM/DER", OPT_FMT_PEMDER }, { "pkcs12", OPT_FMT_PKCS12 }, { "smime", OPT_FMT_SMIME }, { "engine", OPT_FMT_ENGINE },
    { "msblob", OPT_FMT_MSBLOB }, { "nss", OPT_FMT_NSS }, { "text", OPT_FMT_TEXT }, { "http", OPT_FMT_HTTP }, { "pvk", OPT_FMT_PVK }, { NULL } };
        ...
        if (s[1] == '\0' || strcmp(s, "PEM") == 0 || strcmp(s, "pem") == 0) {
            ...
            *result = FORMAT_PEM;
            ...
            if ((flags & OPT_FMT_PVK) == 0)
                return opt_format_error(s, flags);
            *result = FORMAT_PVK;
        } else if (strcmp(s, "P12") == 0 || strcmp(s, "p12") == 0 || strcmp(s, "PKCS12") == 0 || strcmp(s, "pkcs12") == 0) {
            if ((flags & OPT_FMT_PKCS12) == 0)
                return opt_format_error(s, flags);
            *result = FORMAT_PKCS12;
        } else {
...
int opt_cipher_silent(const char* name, EVP_CIPHER** cipherp)
{
    ...
    ERR_set_mark();
    if ((c = EVP_CIPHER_fetch(app_get0_libctx(), name, app_get0_propq())) != NULL
        || (opt_legacy_okay() && (c = (EVP_CIPHER*)EVP_get_cipherbyname(name)) != NULL)) {
        ERR_pop_to_mark();
        if (cipherp != NULL) {
...
int opt_md_silent(const char* name, EVP_MD** mdp)
{
    ...
    ERR_set_mark();
    if ((md = EVP_MD_fetch(app_get0_libctx(), name, app_get0_propq())) != NULL || (opt_legacy_okay() && (md = (EVP_MD*)EVP_get_digestbyname(name)) != NULL)) {
        ERR_pop_to_mark();
        if (mdp != NULL) {
            EVP_MD_free(*mdp);
            *mdp = md;
...
static void opt_number_error(const char* v)
{
    ...
    struct strstr_pair_st {
        char* prefix;
        char* name;
    } b[] = { { "0x", "a hexadecimal" }, { "0X", "a hexadecimal" }, { "0", "an octal" } };
...
int opt_long(const char* value, long* result)
{
    ...
    if (*endp || endp == value || ((l == LONG_MAX || l == LONG_MIN) && errno == ERANGE) || (l == 0 && errno != 0)) {
...
int opt_intmax(const char* value, ossl_intmax_t* result)
{
    ...
    if (*endp || endp == value || ((m == INTMAX_MAX || m == INTMAX_MIN) && errno == ERANGE) || (m == 0 && errno != 0)) {
        opt_number_error(value);
        errno = oerrno;
        return 0;
    }
    /* Ensure that the value in |m| is never too big for |*result| */
    if (sizeof(m) > sizeof(*result) && (m < OSSL_INTMAX_MIN || m > OSSL_INTMAX_MAX)) {
        opt_number_error(value);
        return 0;
    }
...
int opt_uintmax(const char* value, ossl_uintmax_t* result)
{
    ...
    if (*endp || endp == value || (m == UINTMAX_MAX && errno == ERANGE) || (m == 0 && errno != 0)) {
        opt_number_error(value);
        errno = oerrno;
        return 0;
    }
...
int opt_ulong(const char* value, unsigned long* result)
{
    ...
    if (*endptr || endptr == value || ((l == ULONG_MAX) && errno == ERANGE) || (l == 0 && errno != 0)) {
        opt_number_error(value);
        errno = oerrno;
        return 0;
    }
...
int opt_next(void)
{
    ...
    for (o = opts; o->name; ++o) {
        ...
        if (!(strcmp(p, "h") == 0 && strcmp(o->name, "help") == 0) && strcmp(p, o->name) != 0)
            continue;
        ...
        case 'F':
        case 'f':
            if (opt_format(arg, o->valtype == 'c' ? OPT_FMT_PDS : o->valtype == 'E' ? OPT_FMT_PDE : o->valtype == 'F' ? OPT_FMT_PEMDER : OPT_FMT_ANY, &ival))
                break;
