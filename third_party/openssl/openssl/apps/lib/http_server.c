    ...
    while (termsig == 0) {
        ...
        while (termsig == 0 && procs >= multi) {
            if ((fpid = waitpid(-1, &status, 0)) > 0) {
                ...
                if (status != 0) {
                    if (WIFEXITED(status))
                        syslog(LOG_WARNING, "child process: %ld, exit status: %d", (long)fpid, WEXITSTATUS(status));
                    else if (WIFSIGNALED(status))
                        syslog(LOG_WARNING, "child process: %ld, term signal %d%s", (long)fpid, WTERMSIG(status),
#ifdef WCOREDUMP
...
BIO* http_server_init_bio(const char* prog, const char* port)
{
    ...
    if (acbio == NULL || BIO_set_bind_mode(acbio, BIO_BIND_REUSEADDR) < 0 || BIO_set_accept_port(acbio, port) < 0) {
...
int http_server_get_asn1_req(const ASN1_ITEM* it, ASN1_VALUE** preq, char** ppath, BIO** pcbio, BIO* acbio, int* found_keep_alive, const char* prog,
    const char* port, int accept_get, int timeout)
{
    ...
    *preq = NULL;
    ...
    if (cbio == NULL) {
        log_message(prog, LOG_DEBUG, "Awaiting new connection on port %s...", port);
        if (BIO_do_accept(acbio) <= 0)
    ...
    if ((end = strchr(reqbuf, '\r')) != NULL || (end = strchr(reqbuf, '\n')) != NULL)
        *end = '\0';
    log_message(prog, LOG_INFO, "Received request, 1st line: %s", reqbuf);

    meth = reqbuf;
    url = meth + 3;
    if ((accept_get && strncmp(meth, "GET ", 4) == 0) || (url++, strncmp(meth, "POST ", 5) == 0)) {
        ...
        *(url++) = '\0';
        while (*url == ' ')
            url++;
        if (*url != '/') {
            log_message(prog, LOG_WARNING, "Invalid %s -- URL does not begin with '/': %s", meth, url);
            (void)http_server_send_status(cbio, 400, "Bad Request");
            goto out;
        }
        url++;
        ...
        if (strncmp(end, http_version_str, http_version_str_len) != 0) {
            log_message(prog, LOG_WARNING, "Invalid %s -- bad HTTP/version string: %s", meth, end + 1);
            (void)http_server_send_status(cbio, 400, "Bad Request");
            goto out;
        }
        *end = '\0';
        ...
        len = urldecode(url);
        if (len < 0) {
            log_message(prog, LOG_WARNING, "Invalid %s request -- bad URL encoding: %s", meth, url);
            (void)http_server_send_status(cbio, 400, "Bad Request");
            goto out;
        }
        if (strlen(meth) == 3) { /* GET */
            if ((getbio = BIO_new_mem_buf(url, len)) == NULL || (b64 = BIO_new(BIO_f_base64())) == NULL) {
                log_message(prog, LOG_ERR, "Could not allocate base64 bio with size = %d", len);
                goto fatal;
            }
        ...
        log_message(prog, LOG_WARNING, "HTTP request does not begin with %sPOST: %s", accept_get ? "GET or " : "", reqbuf);
        (void)http_server_send_status(cbio, 400, "Bad Request");
        goto out;
    }
    ...
    *end = '\0';
    ...
    for (;;) {
        ...
        value = strchr(key, ':');
        if (value == NULL) {
            log_message(prog, LOG_WARNING, "Error parsing HTTP header: missing ':'");
            (void)http_server_send_status(cbio, 400, "Bad Request");
            goto out;
        }
        *(value++) = '\0';
        ...
        if (line_end == NULL) {
            ...
            if (line_end == NULL) {
                log_message(prog, LOG_WARNING, "Error parsing HTTP header: missing end of line");
                (void)http_server_send_status(cbio, 400, "Bad Request");
                goto out;
            }
        }
    ...
    if (req == NULL) {
        ...
        log_message(prog, LOG_ERR, "Out of memory allocating %zu bytes", strlen(url) + 1);
...
int http_server_send_asn1_resp(BIO* cbio, int keep_alive, const char* content_type, const ASN1_ITEM* it, const ASN1_VALUE* resp)
{
