...
int TerminalSocket(int FunctionCode, int* ReturnSocket)
{
    ...
    */
    ...
    case TERM_SOCK_CREATE:
        ...
        */
        status = sys$qio(EFN$C_ENF, TerminalDeviceChan, IO$_READVBLK, &TerminalDeviceIosb, TerminalDeviceAst, 0, TerminalDeviceBuff,
            sizeof(TerminalDeviceBuff) - 2, 0, 0, 0, 0);
        if (!(status & 1)) {
...
/*                                                                            */
/*----------------------------------------------------------------------------*/
static int CreateSocketPair(int SocketFamily, int SocketType, int SocketProtocol, int* SocketPair)
{
    struct dsc$descriptor AscTimeDesc = { 0, DSC$K_DTYPE_T, DSC$K_CLASS_S, NULL };
    static const char* LocalHostAddr = { "127.0.0.1" };
    unsigned short TcpAcceptChan = 0, TcpDeviceChan = 0;
    ...
    */
    status = sys$qio(EFN$C_ENF, TcpAcceptChan, IO$_ACCESS | IO$M_ACCEPT, &iosb, 0, 0, 0, 0, 0, &TcpDeviceChan, 0, 0);
    if (!(status & 1)) {
    ...
    */
    status = sys$setimr(EFN$C_ENF, BinTimeBuff, SocketPairTimeoutAst, &sptb, 0);
    if (!(status & 1)) {
...
static int TerminalDeviceAst(int astparm)
{
    ...
    */
    status = sys$qio(
        EFN$C_ENF, TerminalDeviceChan, IO$_READVBLK, &TerminalDeviceIosb, TerminalDeviceAst, 0, TerminalDeviceBuff, sizeof(TerminalDeviceBuff) - 1, 0, 0, 0, 0);
...
static void LogMessage(char* msg, ...)
{
    ...
    */
    sprintf(MsgBuff, "%02d-%s-%04d %02d:%02d:%02d [%08X] %s\n", LocTime->tm_mday, Month[LocTime->tm_mon], (LocTime->tm_year + 1900), LocTime->tm_hour,
        LocTime->tm_min, LocTime->tm_sec, pid, msg);
