...
int init_client(int* sock, const char* host, const char* port, const char* bindhost, const char* bindport, int family, int type, int protocol)
{
    ...
    ret = BIO_lookup_ex(host, port, BIO_LOOKUP_CLIENT, family, type, protocol, &res);
    if (ret == 0) {
    ...
    if (bindhost != NULL || bindport != NULL) {
        ret = BIO_lookup_ex(bindhost, bindport, BIO_LOOKUP_CLIENT, family, type, protocol, &bindaddr);
        if (ret == 0) {
            ERR_print_errors(bio_err);
            goto out;
        }
    ...
    for (ai = res; ai != NULL; ai = BIO_ADDRINFO_next(ai)) {
         ...
         * asked for. */
        OPENSSL_assert((family == AF_UNSPEC || family == BIO_ADDRINFO_family(ai)) && (type == 0 || type == BIO_ADDRINFO_socktype(ai))
        ...
        *sock = BIO_socket(BIO_ADDRINFO_family(ai), BIO_ADDRINFO_socktype(ai), BIO_ADDRINFO_protocol(ai), 0);
        ...
        if (!BIO_connect(*sock, BIO_ADDRINFO_address(ai), BIO_ADDRINFO_protocol(ai) == IPPROTO_TCP ? BIO_SOCK_NODELAY : 0)) {
            BIO_closesocket(*sock);
            *sock = INVALID_SOCKET;
            continue;
        }
            ...
            BIO_printf(bio_err, "Can't bind %saddress for %s%s%s\n",
                ...
                BIO_ADDRINFO_family(res) == AF_INET6 ? "IPv6 " :
                    ...
                    : BIO_ADDRINFO_family(res) == AF_UNIX ? "unix "
                                                          : "",
                bindhost != NULL ? bindhost : "", bindport != NULL ? ":" : "", bindport != NULL ? bindport : "");
            ERR_clear_error();
            ret = 0;
        }
        ERR_print_errors(bio_err);
...
int report_server_accept(BIO* out, int asock, int with_address, int with_pid)
{
    ...
    if (with_address) {
        ...
        if ((info.addr = BIO_ADDR_new()) != NULL && BIO_sock_info(asock, BIO_SOCK_INFO_ADDRESS, &info)
            && (hostname = BIO_ADDR_hostname_string(info.addr, 1)) != NULL && (service = BIO_ADDR_service_string(info.addr, 1)) != NULL) {
            success = BIO_printf(out, strchr(hostname, ':') == NULL ? /* IPv4 */ " %s:%s" : /* IPv6 */ " [%s]:%s", hostname, service) > 0;
        } else {
 ...
 */
int do_server(int* accept_sock, const char* host, const char* port, int family, int type, int protocol, do_server_cb cb, unsigned char* context, int naccept,
    BIO* bio_s_out)
{
    int asock = 0;
    int sock;
    ...
    if (!BIO_lookup_ex(host, port, BIO_LOOKUP_SERVER, family, type, protocol, &res)) {
        ERR_print_errors(bio_err);
        return 0;
    }
    ...
    OPENSSL_assert((family == AF_UNSPEC || family == BIO_ADDRINFO_family(res)) && (type == 0 || type == BIO_ADDRINFO_socktype(res))
    ...
    if (next != NULL && BIO_ADDRINFO_socktype(next) == sock_type && BIO_ADDRINFO_protocol(next) == sock_protocol) {
    ...
    if (asock == INVALID_SOCKET || !BIO_listen(asock, sock_address, sock_options)) {
        BIO_ADDRINFO_free(res);
        ERR_print_errors(bio_err);
    ...
    for (;;) {
        ...
        if (type == SOCK_STREAM) {
            ...
            } while (select(sock + 1, &readfds, NULL, NULL, &timeout) > 0 && readsocket(sock, sink, sizeof(sink)) > 0);
