...
static int load_key_certs_crls_suppress(const char* uri, int format, int maybe_stdin, const char* pass, const char* desc, EVP_PKEY** ppkey, EVP_PKEY** ppubkey,
    EVP_PKEY** pparams, X509** pcert, STACK_OF(X509) * *pcerts, X509_CRL** pcrl, STACK_OF(X509_CRL) * *pcrls, int suppress_decode_errors);
...
X509* load_cert_pass(const char* uri, int format, int maybe_stdin, const char* pass, const char* desc)
{
    ...
    if (IS_HTTPS(uri))
        ...
        (void)load_key_certs_crls(uri, format, maybe_stdin, pass, desc, NULL, NULL, NULL, &cert, NULL, NULL, NULL);
    if (cert == NULL) {
...
X509_CRL* load_crl(const char* uri, int format, int maybe_stdin, const char* desc)
{
    X509_CRL* crl = NULL;
    ...
    if (IS_HTTPS(uri))
        ...
        (void)load_key_certs_crls(uri, format, maybe_stdin, NULL, desc, NULL, NULL, NULL, NULL, NULL, &crl, NULL);
    if (crl == NULL) {
        BIO_printf(bio_err, "Unable to load %s\n", desc);
        ERR_print_errors(bio_err);
    }
...
EVP_PKEY* load_key(const char* uri, int format, int may_stdin, const char* pass, ENGINE* e, const char* desc)
{
    EVP_PKEY* pkey = NULL;
    ...
    EVP_PKEY** pparams, X509** pcert, STACK_OF(X509) * *pcerts, X509_CRL** pcrl, STACK_OF(X509_CRL) * *pcrls, int suppress_decode_errors)
{
    ...
    int ncerts = 0;
    int ncrls = 0;
    const char* failed = ppkey != NULL ? "key"
        : ppubkey != NULL              ? "public key"
        : pparams != NULL              ? "params"
        : pcert != NULL                ? "cert"
        : pcrl != NULL                 ? "CRL"
        : pcerts != NULL               ? "certs"
    ...
    while (cnt_expectations > 0 && !OSSL_STORE_eof(ctx)) {
        ...
        case OSSL_STORE_INFO_CERT:
            if (pcert != NULL && *pcert == NULL) {
                ...
                ok = X509_add_cert(*pcerts, OSSL_STORE_INFO_get1_CERT(info), X509_ADD_FLAG_DEFAULT);
            ncerts += ok;
            break;
    ...
    if (failed == NULL) {
        ...
        if ((ppkey != NULL && *ppkey == NULL) || (ppubkey != NULL && *ppubkey == NULL)) {
            failed = "key";
...
int load_key_certs_crls(const char* uri, int format, int maybe_stdin, const char* pass, const char* desc, EVP_PKEY** ppkey, EVP_PKEY** ppubkey,
    EVP_PKEY** pparams, X509** pcert, STACK_OF(X509) * *pcerts, X509_CRL** pcrl, STACK_OF(X509_CRL) * *pcrls)
{
    return load_key_certs_crls_suppress(uri, format, maybe_stdin, pass, desc, ppkey, ppubkey, pparams, pcert, pcerts, pcrl, pcrls, 0);
}
...
{
    static const NAME_EX_TBL cert_tbl[] = { { "compatible", X509_FLAG_COMPAT, 0xffffffffl }, { "ca_default", X509_FLAG_CA, 0xffffffffl },
        { "no_header", X509_FLAG_NO_HEADER, 0 }, { "no_version", X509_FLAG_NO_VERSION, 0 }, { "no_serial", X509_FLAG_NO_SERIAL, 0 },
        { "no_signame", X509_FLAG_NO_SIGNAME, 0 }, { "no_validity", X509_FLAG_NO_VALIDITY, 0 }, { "no_subject", X509_FLAG_NO_SUBJECT, 0 },
        { "no_issuer", X509_FLAG_NO_ISSUER, 0 }, { "no_pubkey", X509_FLAG_NO_PUBKEY, 0 }, { "no_extensions", X509_FLAG_NO_EXTENSIONS, 0 },
        { "no_sigdump", X509_FLAG_NO_SIGDUMP, 0 }, { "no_aux", X509_FLAG_NO_AUX, 0 }, { "no_attributes", X509_FLAG_NO_ATTRIBUTES, 0 },
        { "ext_default", X509V3_EXT_DEFAULT, X509V3_EXT_UNKNOWN_MASK }, { "ext_error", X509V3_EXT_ERROR_UNKNOWN, X509V3_EXT_UNKNOWN_MASK },
        { "ext_parse", X509V3_EXT_PARSE_UNKNOWN, X509V3_EXT_UNKNOWN_MASK }, { "ext_dump", X509V3_EXT_DUMP_UNKNOWN, X509V3_EXT_UNKNOWN_MASK }, { NULL, 0, 0 } };
    return set_multi_opts(flags, arg, cert_tbl);
}

int set_name_ex(unsigned long* flags, const char* arg)
{
    static const NAME_EX_TBL ex_tbl[] = { { "esc_2253", ASN1_STRFLGS_ESC_2253, 0 }, { "esc_2254", ASN1_STRFLGS_ESC_2254, 0 },
        { "esc_ctrl", ASN1_STRFLGS_ESC_CTRL, 0 }, { "esc_msb", ASN1_STRFLGS_ESC_MSB, 0 }, { "use_quote", ASN1_STRFLGS_ESC_QUOTE, 0 },
        { "utf8", ASN1_STRFLGS_UTF8_CONVERT, 0 }, { "ignore_type", ASN1_STRFLGS_IGNORE_TYPE, 0 }, { "show_type", ASN1_STRFLGS_SHOW_TYPE, 0 },
        { "dump_all", ASN1_STRFLGS_DUMP_ALL, 0 }, { "dump_nostr", ASN1_STRFLGS_DUMP_UNKNOWN, 0 }, { "dump_der", ASN1_STRFLGS_DUMP_DER, 0 },
        { "compat", XN_FLAG_COMPAT, 0xffffffffL }, { "sep_comma_plus", XN_FLAG_SEP_COMMA_PLUS, XN_FLAG_SEP_MASK },
        { "sep_comma_plus_space", XN_FLAG_SEP_CPLUS_SPC, XN_FLAG_SEP_MASK }, { "sep_semi_plus_space", XN_FLAG_SEP_SPLUS_SPC, XN_FLAG_SEP_MASK },
        { "sep_multiline", XN_FLAG_SEP_MULTILINE, XN_FLAG_SEP_MASK }, { "dn_rev", XN_FLAG_DN_REV, 0 }, { "nofname", XN_FLAG_FN_NONE, XN_FLAG_FN_MASK },
        { "sname", XN_FLAG_FN_SN, XN_FLAG_FN_MASK }, { "lname", XN_FLAG_FN_LN, XN_FLAG_FN_MASK }, { "align", XN_FLAG_FN_ALIGN, 0 },
        { "oid", XN_FLAG_FN_OID, XN_FLAG_FN_MASK }, { "space_eq", XN_FLAG_SPC_EQ, 0 }, { "dump_unknown", XN_FLAG_DUMP_UNKNOWN_FIELDS, 0 },
        { "RFC2253", XN_FLAG_RFC2253, 0xffffffffL }, { "oneline", XN_FLAG_ONELINE, 0xffffffffL }, { "multiline", XN_FLAG_MULTILINE, 0xffffffffL },
        { "ca_default", XN_FLAG_MULTILINE, 0xffffffffL }, { NULL, 0, 0 } };
...
X509_STORE* setup_verify(const char* CAfile, int noCAfile, const char* CApath, int noCApath, const char* CAstore, int noCAstore)
{
    ...
    if (CAfile != NULL || !noCAfile) {
        ...
        if (lookup == NULL)
            goto end;
        if (CAfile != NULL) {
            if (X509_LOOKUP_load_file_ex(lookup, CAfile, X509_FILETYPE_PEM, libctx, propq) <= 0) {
...
static IMPLEMENT_LHASH_HASH_FN(index_serial, OPENSSL_CSTRING) static IMPLEMENT_LHASH_COMP_FN(index_serial, OPENSSL_CSTRING) static IMPLEMENT_LHASH_HASH_FN(
    index_name, OPENSSL_CSTRING) static IMPLEMENT_LHASH_COMP_FN(index_name, OPENSSL_CSTRING)
#undef BSIZE
#define BSIZE 256
    BIGNUM* load_serial(const char* serialfile, int* exists, int create, ASN1_INTEGER** retai)
{
    BIO* in = NULL;
    BIGNUM* ret = NULL;
    char buf[1024];
...
CA_DB* load_index(const char* dbfile, DB_ATTR* db_attr)
{
    ...
    if (fstat(fileno(dbfp), &dbst) == -1) {
        ERR_raise_data(ERR_LIB_SYS, errno, "calling fstat(%s)", dbfile);
        goto err;
    }
#endif
...
{
    if (!TXT_DB_create_index(db->db, DB_serial, NULL, LHASH_HASH_FN(index_serial), LHASH_COMP_FN(index_serial))) {
...
X509_NAME* parse_name(const char* cp, int chtype, int canmulti, const char* desc)
{
    ...
    while (*cp != '\0') {
        ...
        *bp++ = '\0';
        ...
        if (nid == NID_undef) {
            BIO_printf(bio_err, "%s warning: Skipping unknown %s name attribute \"%s\"\n", opt_getprog(), desc, typestr);
            if (ismulti)
                BIO_printf(bio_err, "%s hint: a '+' in a value string needs be escaped using '\\' else a new member of a multi-valued RDN is expected\n",
                    opt_getprog());
            continue;
        }
        if (*valstr == '\0') {
            BIO_printf(bio_err, "%s warning: No value provided for %s name attribute \"%s\", skipped\n", opt_getprog(), desc, typestr);
            continue;
        }
...
int do_X509_REQ_sign(X509_REQ* x, EVP_PKEY* pkey, const char* md, STACK_OF(OPENSSL_STRING) * sigopts)
{
    int rv = 0;
    EVP_MD_CTX* mctx = EVP_MD_CTX_new();
...
/* Sign the CRL info */
int do_X509_CRL_sign(X509_CRL* x, EVP_PKEY* pkey, const char* md, STACK_OF(OPENSSL_STRING) * sigopts)
{
    int rv = 0;
    EVP_MD_CTX* mctx = EVP_MD_CTX_new();
...
BIO* app_http_tls_cb(BIO* bio, void* arg, int connect, int detail)
{
    ...
    if (connect) {
        ...
        if ((info->use_proxy
                && !OSSL_HTTP_proxy_connect(bio, info->server, info->port, NULL, NULL, /* no proxy credentials */
...
ASN1_VALUE* app_http_get_asn1(const char* url, const char* proxy, const char* no_proxy, SSL_CTX* ssl_ctx, const STACK_OF(CONF_VALUE) * headers, long timeout,
    const char* expected_content_type, const ASN1_ITEM* it)
{
    ...
    if (!use_ssl && ssl_ctx != NULL) {
        ERR_raise_data(ERR_LIB_HTTP, ERR_R_PASSED_INVALID_ARGUMENT, "SSL_CTX given but use_ssl == 0");
        goto end;
    }
    ...
    mem = OSSL_HTTP_get(url, proxy, no_proxy, NULL /* bio */, NULL /* rbio */, app_http_tls_cb, &info, 0 /* buf_size */, headers, expected_content_type,
        1 /* expect_asn1 */, OSSL_HTTP_DEFAULT_MAX_RESP_LEN, timeout);
...
ASN1_VALUE* app_http_post_asn1(const char* host, const char* port, const char* path, const char* proxy, const char* no_proxy, SSL_CTX* ssl_ctx,
    const STACK_OF(CONF_VALUE) * headers, const char* content_type, ASN1_VALUE* req, const ASN1_ITEM* req_it, const char* expected_content_type, long timeout,
    const ASN1_ITEM* rsp_it)
{
    ...
    info.timeout = timeout;
    info.ssl_ctx = ssl_ctx;
    rsp = OSSL_HTTP_transfer(NULL, host, port, path, use_ssl, proxy, no_proxy, NULL /* bio */, NULL /* rbio */, app_http_tls_cb, &info, 0 /* buf_size */,
        headers, content_type, req_mem, expected_content_type, 1 /* expect_asn1 */, OSSL_HTTP_DEFAULT_MAX_RESP_LEN, timeout, 0 /* keep_alive */);
    BIO_free(req_mem);
...
double app_tminterval(int stop, int usertime)
{
    ...
    if (stop == TM_START)
        ...
        ret = ((now.tv_sec + now.tv_nsec * 1e-9) - (tmstart.tv_sec + tmstart.tv_nsec * 1e-9));
#else
    now = tickGet();
    if (stop == TM_START)
        tmstart = now;
...
int set_crl_nextupdate(X509_CRL* crl, const char* nextupdate, long days, long hours, long secs)
{
    int ret = 0;
    ASN1_TIME* tm = ASN1_TIME_new();
