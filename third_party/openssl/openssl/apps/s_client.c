...
static int psk_use_session_cb(SSL* s, const EVP_MD* md, const unsigned char** id, size_t* idlen, SSL_SESSION** sess)
{
    ...
    if (psksess != NULL) {
        ...
        if (usesess == NULL || !SSL_SESSION_set1_master_key(usesess, key, key_len) || !SSL_SESSION_set_cipher(usesess, cipher)
...
static int serverinfo_cli_parse_cb(SSL* s, unsigned int ext_type, const unsigned char* in, size_t inlen, int* al, void* arg)
{
    char pem_name[100];
...
static int tlsa_import_rr(SSL* con, const char* rrdata)
{
    ...
    static struct tlsa_field tlsa_fields[]
        = { { &usage, "usage", checked_uint8 }, { &selector, "selector", checked_uint8 }, { &mtype, "mtype", checked_uint8 }, { &data, "data", hexdecode },
              {
                  NULL,
              } };
    struct tlsa_field* f;
...
const OPTIONS s_client_options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] [host:port]\n" },

    OPT_SECTION("General"), { "help", OPT_HELP, '-', "Display this summary" },
...
#ifndef OPENSSL_NO_CT
    { "ct", OPT_CT, '-', "Request and parse SCTs (also enables OCSP stapling)" }, { "noct", OPT_NOCT, '-', "Do not request or parse SCTs (default)" },
    { "ctlogfile", OPT_CTLOG_FILE, '<', "CT log list CONF file" },
#endif

    OPT_SECTION("Network"), { "host", OPT_HOST, 's', "Use -connect instead" }, { "port", OPT_PORT, 'p', "Use -connect instead" },
    { "connect", OPT_CONNECT, 's', "TCP/IP where to connect; default: " PORT ")" }, { "bind", OPT_BIND, 's', "bind local address for connection" },
    { "proxy", OPT_PROXY, 's', "Connect to via specified proxy to the real server" }, { "proxy_user", OPT_PROXY_USER, 's', "UserID for proxy authentication" },
...
#endif
    { "maxfraglen", OPT_MAXFRAGLEN, 'p', "Enable Maximum Fragment Length Negotiation (len values: 512, 1024, 2048 and 4096)" },
    ...
    { "read_buf", OPT_READ_BUF, 'p', "Default read buffer size to be used for connections" },
    { "fallback_scsv", OPT_FALLBACKSCSV, '-', "Send the fallback SCSV" },

    OPT_SECTION("Identity"), { "cert", OPT_CERT, '<', "Client certificate file to use" },
    { "certform", OPT_CERTFORM, 'F', "Client certificate file format (PEM/DER/P12); has no effect" },
    { "cert_chain", OPT_CERT_CHAIN, '<', "Client certificate chain file (in PEM format)" },
    { "build_chain", OPT_BUILD_CHAIN, '-', "Build client certificate chain" }, { "key", OPT_KEY, 's', "Private key file to use; default: -cert file" },
    { "keyform", OPT_KEYFORM, 'E', "Key format (ENGINE, other values ignored)" }, { "pass", OPT_PASS, 's', "Private key and cert file pass phrase source" },
    { "verify", OPT_VERIFY, 'p', "Turn on peer certificate verification" }, { "nameopt", OPT_NAMEOPT, 's', "Certificate subject/issuer name printing options" },
    { "CApath", OPT_CAPATH, '/', "PEM format directory of CA's" }, { "CAfile", OPT_CAFILE, '<', "PEM format file of CA's" },
    { "CAstore", OPT_CASTORE, ':', "URI to store of CA's" }, { "no-CAfile", OPT_NOCAFILE, '-', "Do not load the default certificates file" },
    ...
    { "requestCAfile", OPT_REQCAFILE, '<', "PEM format file of CA names to send to the server" },
    { "dane_tlsa_domain", OPT_DANE_TLSA_DOMAIN, 's', "DANE TLSA base domain" },
    { "dane_tlsa_rrdata", OPT_DANE_TLSA_RRDATA, 's', "DANE TLSA rrdata presentation form" },
    { "dane_ee_no_namechecks", OPT_DANE_EE_NO_NAME, '-', "Disable name checks when matching DANE-EE(3) TLSA records" },
    { "psk_identity", OPT_PSK_IDENTITY, 's', "PSK identity" }, { "psk", OPT_PSK, 's', "PSK in hex (without 0x)" },
    { "psk_session", OPT_PSK_SESS, '<', "File to read PSK SSL session from" },
    { "name", OPT_PROTOHOST, 's', "Hostname to use for \"-starttls lmtp\", \"-starttls smtp\" or \"-starttls xmpp[-server]\"" },

    OPT_SECTION("Session"), { "reconnect", OPT_RECONNECT, '-', "Drop and re-make the connection with the same Session-ID" },
    { "sess_out", OPT_SESS_OUT, '>', "File to write SSL session to" }, { "sess_in", OPT_SESS_IN, '<', "File to read SSL session from" },

    OPT_SECTION("Input/Output"), { "crlf", OPT_CRLF, '-', "Convert LF from terminal into CRLF" }, { "quiet", OPT_QUIET, '-', "No s_client output" },
    { "ign_eof", OPT_IGN_EOF, '-', "Ignore input eof (default when -quiet)" }, { "no_ign_eof", OPT_NO_IGN_EOF, '-', "Don't ignore input eof" },
    { "starttls", OPT_STARTTLS, 's', "Use the appropriate STARTTLS command before starting TLS" },
    { "xmpphost", OPT_XMPPHOST, 's', "Alias of -name option for \"-starttls xmpp[-server]\"" },
    { "brief", OPT_BRIEF, '-', "Restrict output to brief summary of connection parameters" },
    { "prexit", OPT_PREXIT, '-', "Print session information when the program exits" },

    OPT_SECTION("Debug"), { "showcerts", OPT_SHOWCERTS, '-', "Show all certificates sent by the server" }, { "debug", OPT_DEBUG, '-', "Extra output" },
    { "msg", OPT_MSG, '-', "Show protocol messages" }, { "msgfile", OPT_MSGFILE, '>', "File to send output of -msg or -trace, instead of stdout" },
    { "nbio_test", OPT_NBIO_TEST, '-', "More ssl protocol testing" }, { "state", OPT_STATE, '-', "Print the ssl states" },
    ...
    { "security_debug", OPT_SECURITY_DEBUG, '-', "Enable security debug messages" },
    { "security_debug_verbose", OPT_SECURITY_DEBUG_VERBOSE, '-', "Output more security debug output" },
...
#endif
    { "keylogfile", OPT_KEYLOG_FILE, '>', "Write TLS secrets to file" }, { "nocommands", OPT_NOCMDS, '-', "Do not use interactive command letters" },
    { "servername", OPT_SERVERNAME, 's', "Set TLS extension servername (SNI) in ClientHello (default)" },
    { "noservername", OPT_NOSERVERNAME, '-', "Do not send the server name (SNI) extension in the ClientHello" },
    ...
    { "async", OPT_ASYNC, '-', "Support asynchronous operation" }, { "nbio", OPT_NBIO, '-', "Use non-blocking IO" },
    ...
    { "dtls", OPT_DTLS, '-', "Use any version of DTLS" }, { "timeout", OPT_TIMEOUT, '-', "Enable send/receive timeout on DTLS connections" },
...
#endif
    { "early_data", OPT_EARLY_DATA, '<', "File to send as early data" }, { "enable_pha", OPT_ENABLE_PHA, '-', "Enable post-handshake-authentication" },
#ifndef OPENSSL_NO_SRTP
    { "use_srtp", OPT_USE_SRTP, 's', "Offer SRTP key management with a colon-separated profile list" },
#endif
#ifndef OPENSSL_NO_SRP
    { "srpuser", OPT_SRPUSER, 's', "(deprecated) SRP authentication for 'user'" }, { "srppass", OPT_SRPPASS, 's', "(deprecated) Password for 'user'" },
    { "srp_lateuser", OPT_SRP_LATEUSER, '-', "(deprecated) SRP username into second ClientHello message" },
    { "srp_moregroups", OPT_SRP_MOREGROUPS, '-', "(deprecated) Tolerate other than the known g N values." },
    { "srp_strength", OPT_SRP_STRENGTH, 'p', "(deprecated) Minimal length in bits for N" },
#endif

    OPT_R_OPTIONS, OPT_S_OPTIONS, OPT_V_OPTIONS, { "CRL", OPT_CRL, '<', "CRL file to use" },
    { "crl_download", OPT_CRL_DOWNLOAD, '-', "Download CRL from distribution points" }, { "CRLform", OPT_CRLFORM, 'F', "CRL format (PEM or DER); default PEM" },
    ...
    { "chainCAstore", OPT_CHAINCASTORE, ':', "CA store URI for certificate chain" },
    ...
    { "verifyCAstore", OPT_VERIFYCASTORE, ':', "CA store URI for certificate verification" }, OPT_X_OPTIONS, OPT_PROV_OPTIONS,

    OPT_PARAMETERS(), { "host:port", 0, 0, "Where to connect; same as -connect option" }, { NULL } };
...
static const OPT_PAIR services[] = { { "smtp", PROTO_SMTP }, { "pop3", PROTO_POP3 }, { "imap", PROTO_IMAP }, { "ftp", PROTO_FTP }, { "xmpp", PROTO_XMPP },
    { "xmpp-server", PROTO_XMPP_SERVER }, { "telnet", PROTO_TELNET }, { "irc", PROTO_IRC }, { "mysql", PROTO_MYSQL }, { "postgres", PROTO_POSTGRES },
    { "lmtp", PROTO_LMTP }, { "nntp", PROTO_NNTP }, { "sieve", PROTO_SIEVE }, { "ldap", PROTO_LDAP }, { NULL, 0 } };
...
/* Known false-positive of MemorySanitizer. */
    ...
    if (argc == 1) {
        ...
        if (connectstr != NULL) {
            BIO_printf(bio_err, "%s: cannot provide both -connect option and target parameter\n", prog);
            goto opthelp;
        }
        connect_type = use_inet;
    ...
    if (bindstr != NULL) {
        int res;
        res = BIO_parse_hostserv(bindstr, &bindhost, &bindport, BIO_PARSE_PRIO_HOST);
        if (!res) {
    ...
    if (maxfraglen > 0 && !SSL_CTX_set_tlsext_max_fragment_length(ctx, maxfraglen)) {
        BIO_printf(bio_err,
...
#ifndef OPENSSL_NO_SRP
    if (srp_arg.srplogin != NULL && !set_up_srp_arg(ctx, &srp_arg, srp_lateuser, c_msg, c_debug))
        goto end;
#endif
...
re_start:
    if (init_client(&sock, host, port, bindhost, bindport, socket_family, socket_type, protocol) == 0) {
    ...
    if (proxystr != NULL) {
        ...
        if (!OSSL_HTTP_proxy_connect(sbio, thost, tport, proxyuser, proxypass, 0 /* no timeout */, bio_err, prog))
            goto shut;
    }
    ...
    case PROTO_XMPP_SERVER: {
        ...
        BIO_printf(sbio,
            ...
            starttls_proto == PROTO_XMPP ? "client" : "server", protohost ? protohost : host);
    ...
    case PROTO_TELNET: {
        ...
        static const unsigned char tls_follows[] = { /* IAC  SB   START_TLS FOLLOWS IAC  SE */
    ...
    case PROTO_IRC: {
        ...
        do {
            ...
            /*
                 ...
                 */
            if (!BIO_get_buffer_num_lines(fbio) && !BIO_pending(fbio) && !BIO_pending(sbio) && select(width, (void*)&readfds, NULL, NULL, &timeout) < 1) {
    ...
    case PROTO_MYSQL: {
        /* SSL request packet */
        static const unsigned char ssl_req[] = { /* payload_length,   sequence_id */
            0x20, 0x00, 0x00, 0x01,
            /* payload */
        ...
        if (bytes < 0) {
            ...
            BIO_printf(bio_err, "Only MySQL protocol version 10 is supported.\n");
            goto shut;
        }
        ...
        if (pos + 15 > bytes) {
            BIO_printf(bio_err, "MySQL server handshake packet is broken.\n");
            goto shut;
        }
    ...
    case PROTO_POSTGRES: {
        static const unsigned char ssl_request[] = { /* Length        SSLRequest */
            0, 0, 0, 8, 4, 210, 22, 47
        };
        int bytes;
    ...
    case PROTO_LDAP: {
        ...
        /* Send SSLRequest packet */
        BIO_write(sbio, atyp->value.sequence->data, atyp->value.sequence->length);
        (void)BIO_flush(sbio);
        ASN1_TYPE_free(atyp);
    ...
    if (early_data_file != NULL
        && ((SSL_get0_session(con) != NULL && SSL_SESSION_get_max_early_data(SSL_get0_session(con)) > 0)
    ...
    for (;;) {
        ...
        if (!SSL_is_init_finished(con) && SSL_total_renegotiations(con) == 0 && SSL_get_key_update_type(con) == SSL_KEY_UPDATE_NONE) {
            in_init = 1;
            tty_on = 0;
        } else {
            tty_on = 1;
        ...
        if (!ssl_pending) {
            ...
            if (!write_tty) {
                if (read_tty) {
                    tv.tv_sec = 1;
                    tv.tv_usec = 0;
                    i = select(width, (void*)&readfds, (void*)&writefds, NULL, &tv);
...
/* OPENSSL_SYS_MSDOS includes OPENSSL_SYS_WINDOWS */
...
#endif
        {
            ...
            if ((!c_ign_eof) && (cbuf[0] == 'R' && cmdletters)) {
                ...
                SSL_renegotiate(con);
                cbuf_len = 0;
            } else if (!c_ign_eof && (cbuf[0] == 'K' || cbuf[0] == 'k') && cmdletters) {
                BIO_printf(bio_err, "KEYUPDATE\n");
                SSL_key_update(con, cbuf[0] == 'K' ? SSL_KEY_UPDATE_REQUESTED : SSL_KEY_UPDATE_NOT_REQUESTED);
                cbuf_len = 0;
            } else {
                cbuf_len = i;
                cbuf_off = 0;
...
shut:
    ...
    } while (select(sock + 1, &readfds, NULL, NULL, &timeout) > 0 && BIO_read(sbio, sbuf, BUFSIZZ) > 0);
...
static void print_stuff(BIO* bio, SSL* s, int full)
{
    ...
    if (full) {
        ...
        if (sk != NULL) {
            ...
            for (i = 0; i < sk_X509_num(sk); i++) {
                ...
                if (public_key != NULL) {
                    BIO_printf(bio, "   a:PKEY: %s, %d (bit); sigalg: %s\n", OBJ_nid2sn(EVP_PKEY_get_base_id(public_key)), EVP_PKEY_get_bits(public_key),
    ...
    if (SSL_get_session(s) != NULL && keymatexportlabel != NULL) {
        ...
        if (SSL_export_keying_material(s, exportedkeymat, keymatexportlen, keymatexportlabel, strlen(keymatexportlabel), NULL, 0, 0) <= 0) {
