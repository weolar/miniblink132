 ...
 */
static int add_entry(enum Type type, unsigned int hash, const char* filename, const unsigned char* digest, int need_symlink, unsigned short old_id)
{
...
static int do_file(const char* filename, const char* fullpath, enum Hash h)
{
    ...
    if (name != NULL) {
        if (h == HASH_NEW || h == HASH_BOTH) {
            int ok;
            unsigned long hash_value = X509_NAME_hash_ex(name, app_get0_libctx(), app_get0_propq(), &ok);
        ...
        for (bp = hash_table[i]; bp; bp = nextbp) {
            ...
            for (ep = bp->first_entry; ep; ep = nextep) {
                ...
                if (ep->old_id < bp->num_needed) {
                    ...
                    BIO_snprintf(buf, buflen, "%08x.%s%d", bp->hash, suffixes[bp->type], ep->old_id);
                    if (verbose)
                    ...
                    BIO_snprintf(buf, buflen, "%s%s%08x.%s%d", dirname, pathsep, bp->hash, suffixes[bp->type], nextid);
                    if (verbose)
                    ...
                    if (symlink(ep->filename, buf) < 0) {
                        BIO_printf(bio_err, "%s: Can't symlink %s, %s\n", opt_getprog(), ep->filename, strerror(errno));
                        errs++;
                    }
                    bit_set(idmask, nextid);
                } else if (remove_links) {
                    /* Link to be deleted */
                    BIO_snprintf(buf, buflen, "%s%s%08x.%s%d", dirname, pathsep, bp->hash, suffixes[bp->type], ep->old_id);
                    if (verbose)
...
typedef enum OPTION_choice { OPT_COMMON, OPT_COMPAT, OPT_OLD, OPT_N, OPT_VERBOSE, OPT_PROV_ENUM } OPTION_CHOICE;

const OPTIONS rehash_options[] = { { OPT_HELP_STR, 1, '-', "Usage: %s [options] [directory...]\n" },

    OPT_SECTION("General"), { "help", OPT_HELP, '-', "Display this summary" }, { "h", OPT_HELP, '-', "Display this summary" },
    { "compat", OPT_COMPAT, '-', "Create both new- and old-style hash links" }, { "old", OPT_OLD, '-', "Use old-style hash to generate links" },
    { "n", OPT_N, '-', "Do not remove existing links" },

    OPT_SECTION("Output"), { "v", OPT_VERBOSE, '-', "Verbose output" },

    OPT_PROV_OPTIONS,

    OPT_PARAMETERS(), { "directory", 0, 0, "One or more directories to process (optional)" }, { NULL } };
