...
#define BLOCK_CIPHER_func_ecb(cname, cprefix, kstruct, ksched)                                                                                                 \
    ...
    {                                                                                                                                                          \
        BLOCK_CIPHER_ecb_loop() cprefix##_ecb_encrypt(in + i, out + i, &EVP_C_DATA(kstruct, ctx)->ksched, EVP_CIPHER_CTX_is_encrypting(ctx));                  \
        return 1;                                                                                                                                              \
    }
    ...
    {                                                                                                                                                          \
        ...
        while (inl && inl >= chunk) {                                                                                                                          \
            ...
            cprefix##_cfb##cbits##_encrypt(in, out, (long)((cbits == 1) && !EVP_CIPHER_CTX_test_flags(ctx, EVP_CIPH_FLAG_LENGTH_BITS) ? chunk * 8 : chunk),    \
                &EVP_C_DATA(kstruct, ctx)->ksched, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx));                                                          \
            EVP_CIPHER_CTX_set_num(ctx, num);                                                                                                                  \
            inl -= chunk;                                                                                                                                      \
            in += chunk;                                                                                                                                       \
            out += chunk;                                                                                                                                      \
...
#define BLOCK_CIPHER_all_funcs(cname, cprefix, cbits, kstruct, ksched)                                                                                         \
    BLOCK_CIPHER_func_cbc(cname, cprefix, kstruct, ksched) BLOCK_CIPHER_func_cfb(cname, cprefix, cbits, kstruct, ksched)                                       \
        BLOCK_CIPHER_func_ecb(cname, cprefix, kstruct, ksched) BLOCK_CIPHER_func_ofb(cname, cprefix, cbits, kstruct, ksched)
...
#define BLOCK_CIPHER_def1(cname, nmode, mode, MODE, kstruct, nid, block_size, key_len, iv_len, flags, init_key, cleanup, set_asn1, get_asn1, ctrl)             \
    static const EVP_CIPHER cname##_##mode = { nid##_##nmode, block_size, key_len, iv_len, flags | EVP_CIPH_##MODE##_MODE, EVP_ORIG_GLOBAL, init_key,          \
        cname##_##mode##_cipher, cleanup, sizeof(kstruct), set_asn1, get_asn1, ctrl, NULL };                                                                   \
...
#define IMPLEMENT_CFBR(cipher, cprefix, kstruct, ksched, keysize, cbits, iv_len, fl)                                                                           \
    BLOCK_CIPHER_func_cfb(cipher##_##keysize, cprefix, cbits, kstruct, ksched) BLOCK_CIPHER_def_cfb(cipher##_##keysize, kstruct, NID_##cipher##_##keysize,     \
        keysize / 8, iv_len, cbits, (fl) | EVP_CIPH_FLAG_DEFAULT_ASN1, cipher##_init_key, NULL, NULL, NULL, NULL)
...
#define EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)                                                                                                                      \
    ((ctx)->operation == EVP_PKEY_OP_SIGN || (ctx)->operation == EVP_PKEY_OP_SIGNCTX || (ctx)->operation == EVP_PKEY_OP_VERIFY                                 \
        || (ctx)->operation == EVP_PKEY_OP_VERIFYCTX || (ctx)->operation == EVP_PKEY_OP_VERIFYRECOVER)

#define EVP_PKEY_CTX_IS_DERIVE_OP(ctx) ((ctx)->operation == EVP_PKEY_OP_DERIVE)

#define EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) ((ctx)->operation == EVP_PKEY_OP_ENCRYPT || (ctx)->operation == EVP_PKEY_OP_DECRYPT)

#define EVP_PKEY_CTX_IS_GEN_OP(ctx) ((ctx)->operation == EVP_PKEY_OP_PARAMGEN || (ctx)->operation == EVP_PKEY_OP_KEYGEN)
...
EVP_PKEY* evp_pkcs82pkey_legacy(const PKCS8_PRIV_KEY_INFO* p8inf, OSSL_LIB_CTX* libctx, const char* propq);
#endif
...
int ossl_pkcs5_pbkdf2_hmac_ex(const char* pass, int passlen, const unsigned char* salt, int saltlen, int iter, const EVP_MD* digest, int keylen,
    unsigned char* out, OSSL_LIB_CTX* libctx, const char* propq);
...
int evp_pkey_ctx_ctrl_to_param(EVP_PKEY_CTX* ctx, int keytype, int optype, int cmd, int p1, void* p2);
