...
static WEBP_INLINE uint32_t Extract(uint32_t max_value, const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    uint32_t v = 0;
    uint32_t range = 1;
...
static WEBP_INLINE void ExtractAndDisableOptimizations(VP8CPUInfo default_VP8GetCPUInfo, const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    GetCPUInfo = default_VP8GetCPUInfo;
    const VP8CPUInfo kVP8CPUInfos[5] = { GetCPUInfoOnlyC, GetCPUInfoForceSlowSSSE3, GetCPUInfoNoSSE41, GetCPUInfoNoAVX, GetCPUInfo };
...
static WEBP_INLINE int ExtractWebPConfig(WebPConfig* const config, const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    ...
    config->image_hint = (WebPImageHint)Extract(WEBP_HINT_LAST - 1, data, size, bit_pos);
...
static WEBP_INLINE int ExtractSourcePicture(WebPPicture* const pic, const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    if (pic == NULL)
        return 0;
...
static WEBP_INLINE int Max(int a, int b)
{
    return ((a < b) ? b : a);
}

static WEBP_INLINE int ExtractAndCropOrScale(WebPPicture* const pic, const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    if (pic == NULL)
        return 0;
    ...
    if (alter_input) {
        if (crop_or_scale) {
            ...
            return WebPPictureCrop(pic, cropped_left, cropped_top, cropped_width, cropped_height);
        } else {
