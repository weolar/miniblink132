...
int AddFrame(WebPAnimEncoder** const enc, const WebPAnimEncoderOptions& anim_config, int* const width, int* const height, int timestamp_ms,
    const uint8_t data[], size_t size, uint32_t* const bit_pos)
{
    ...
    if (pic.width * pic.height > 32 * 32) {
        ...
        config.alpha_quality = (config.alpha_quality > 99) ? 99 : config.alpha_quality;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* const data, size_t size)
{
    ...
    anim_config.kmin = min_kmin + Extract((uint32_t)(max_kmin - min_kmin), data, size, &bit_pos);
    ...
    for (int i = 0; i < nb_frames; ++i) {
        if (!AddFrame(&enc, anim_config, &width, &height, timestamp_ms, data, size, &bit_pos)) {
            return 0;
        }

        timestamp_ms += (1 << (2 + Extract(15, data, size, &bit_pos))) + Extract(1, data, size, &bit_pos); // [1..131073], arbitrary
    }
