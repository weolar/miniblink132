...
static HRESULT WriteUsingWIC(
    const char* out_file_name, int use_stdout, REFGUID container_guid, uint8_t* rgb, int stride, uint32_t width, uint32_t height, int has_alpha)
{
    HRESULT hr = S_OK;
    ...
    if (out_file_name == NULL || rgb == NULL)
        return E_INVALIDARG;

    IFS(CoInitialize(NULL));
    IFS(CoCreateInstance(MAKE_REFGUID(CLSID_WICImagingFactory), NULL, CLSCTX_INPROC_SERVER, MAKE_REFGUID(IID_IWICImagingFactory), (LPVOID*)&factory));
    ...
    if (SUCCEEDED(hr) && use_stdout) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (update_mode)
                SetConsoleMode(std_output, 0);
            if (!WriteFile(std_output, image_mem, (DWORD)GlobalSize(image), &bytes_written, NULL) || bytes_written != GlobalSize(image)) {
                hr = E_FAIL;
            }
            if (update_mode)
                SetConsoleMode(std_output, mode);
            GlobalUnlock(image);
        }
    }
...
int WebPWritePNG(const char* out_file_name, int use_stdout, const WebPDecBuffer* const buffer)
{
    ...
    return SUCCEEDED(WriteUsingWIC(out_file_name, use_stdout, MAKE_REFGUID(GUID_ContainerFormatPng), rgb, stride, width, height, has_alpha));
}
...
int WebPWritePNG(FILE* out_file, const WebPDecBuffer* const buffer)
{
    ...
    png_init_io(png, out_file);
    {
        ...
        png_set_IHDR(png, info, width, height, 8, has_alpha ? PNG_COLOR_TYPE_RGBA : PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT,
...
static int WritePPMPAM(FILE* fout, const WebPDecBuffer* const buffer, int alpha)
{
    if (fout == NULL || buffer == NULL) {
        return 0;
    } else {
...
{
    const int has_alpha = WebPIsAlphaMode(buffer->colorspace);
    const int header_size = BMP_HEADER_SIZE + (has_alpha ? BMP_HEADER_ALPHA_EXTRA_SIZE : 0);
    const uint32_t width = buffer->width;
    const uint32_t height = buffer->height;
    const uint8_t* rgba = buffer->u.RGBA.rgba;
    const int stride = buffer->u.RGBA.stride;
...
int WebPWriteTIFF(FILE* fout, const WebPDecBuffer* const buffer)
{
    ...
    const uint8_t assoc_alpha = WebPIsPremultipliedMode(buffer->colorspace) ? 1 : 2;
    ...
    uint8_t tiff_header[TIFF_HEADER_SIZE] = {
        ...
        0x52, 0x01, 3, 0, 1, 0, 0, 0, assoc_alpha, 0, 0, 0, // 178: ExtraSamples: rgbA/RGBA
    ...
    if (!has_alpha)
        PutLE32(tiff_header + 178, 0); // IFD terminator
...
int WebPSaveImage(const WebPDecBuffer* const buffer, WebPOutputFileFormat format, const char* const out_file_name)
{
    FILE* fout = NULL;
    int needs_open_file = 1;
    const int use_stdout = (out_file_name != NULL) && !WSTRCMP(out_file_name, "-");
    int ok = 1;
    ...
    if (needs_open_file) {
        fout = use_stdout ? ImgIoUtilSetBinaryMode(stdout) : WFOPEN(out_file_name, "wb");
        if (fout == NULL) {
    ...
    if (format == PNG || format == RGBA || format == BGRA || format == ARGB || format == rgbA || format == bgrA || format == Argb) {
#ifdef HAVE_WINCODEC_H
