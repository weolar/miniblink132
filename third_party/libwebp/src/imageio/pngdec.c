        ...
        for (i = 0; i < num; ++i, ++text) {
            ...
            for (j = 0; kPNGMetadataMap[j].name != NULL; ++j) {
                if (!strcmp(text->key, kPNGMetadataMap[j].name)) {
                    MetadataPayload* const payload = (MetadataPayload*)((uint8_t*)metadata + kPNGMetadataMap[j].storage_offset);
...
int ReadPNG(const uint8_t* const data, size_t data_size, struct WebPPicture* const pic, int keep_alpha, struct Metadata* const metadata)
{
...
#if LOCAL_PNG_PREREQ(1, 5) || (LOCAL_PNG_PREREQ(1, 4) && PNG_LIBPNG_VER_RELEASE >= 1)
