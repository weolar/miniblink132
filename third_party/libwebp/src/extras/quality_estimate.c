...
static WEBP_INLINE uint32_t GetBit(const uint8_t* const data, size_t nb, uint64_t max_size, uint64_t* const bit_pos)
{
    uint32_t val = 0;
...
int VP8EstimateQuality(const uint8_t* const data, size_t size)
{
    ...
    if (features.format == 2)
        return 101; // lossless
    if (features.format == 0 || features.has_animation)
        return -1; // mixed
    ...
    if (GET_BIT(1)) { // use_segment_
        ...
        if (GET_BIT(1)) { // update data
            ...
            for (s = 0; s < 4; ++s)
                CONDITIONAL_SKIP(7); //  filter strength
        }
        if (update_map) {
            for (s = 0; s < 3; ++s)
                CONDITIONAL_SKIP(8);
