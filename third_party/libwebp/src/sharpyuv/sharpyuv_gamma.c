...
void SharpYuvInitGammaTables(void)
{
    ...
    if (!kGammaTablesSOk) {
        ...
        // Precompute gamma to linear table.
        {
            ...
            kGammaToLinearTabS[GAMMA_TO_LINEAR_TAB_SIZE + 1] = kGammaToLinearTabS[GAMMA_TO_LINEAR_TAB_SIZE];
        }
        // Precompute linear to gamma table.
        {
            ...
            // to prevent small rounding errors to cause read-overflow:
            kLinearToGammaTabS[LINEAR_TO_GAMMA_TAB_SIZE + 1] = kLinearToGammaTabS[LINEAR_TO_GAMMA_TAB_SIZE];
        }
        kGammaTablesSOk = 1;
    }
}
...
static WEBP_INLINE uint32_t FixedPointInterpolation(int v, uint32_t* tab, int tab_pos_shift_right, int tab_value_shift)
{
    ...
    const int half = (tab_pos_shift_right > 0) ? 1 << (tab_pos_shift_right - 1) : 0;
...
{
    return FixedPointInterpolation(value, kLinearToGammaTabS, (GAMMA_TO_LINEAR_BITS - LINEAR_TO_GAMMA_TAB_BITS), bit_depth - GAMMA_TO_LINEAR_BITS);
}
