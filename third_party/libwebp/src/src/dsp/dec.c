...
{ // vertical
    const uint8_t* top = dst - BPS;
    const uint8_t vals[4] = { AVG3(top[-1], top[0], top[1]), AVG3(top[0], top[1], top[2]), AVG3(top[1], top[2], top[3]), AVG3(top[2], top[3], top[4]) };
    int i;
    for (i = 0; i < 4; ++i) {
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static WEBP_INLINE int NeedsFilter2_C(const uint8_t* p, int step, int t, int it)
{
    ...
    if ((4 * VP8kabs0[p0 - q0] + VP8kabs0[p1 - q1]) > t)
        return 0;
    return VP8kabs0[p3 - p2] <= it && VP8kabs0[p2 - p1] <= it && VP8kabs0[p1 - p0] <= it && VP8kabs0[q3 - q2] <= it && VP8kabs0[q2 - q1] <= it
        && VP8kabs0[q1 - q0] <= it;
}
#endif // !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static WEBP_INLINE void FilterLoop26_C(uint8_t* p, int hstride, int vstride, int size, int thresh, int ithresh, int hev_thresh)
{
    const int thresh2 = 2 * thresh + 1;
    while (size-- > 0) {
...
static WEBP_INLINE void FilterLoop24_C(uint8_t* p, int hstride, int vstride, int size, int thresh, int ithresh, int hev_thresh)
{
    const int thresh2 = 2 * thresh + 1;
    while (size-- > 0) {
        if (NeedsFilter2_C(p, hstride, thresh2, ithresh)) {
...
static void VFilter8_C(uint8_t* u, uint8_t* v, int stride, int thresh, int ithresh, int hev_thresh)
{
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static void HFilter8_C(uint8_t* u, uint8_t* v, int stride, int thresh, int ithresh, int hev_thresh)
{
...
#if !WEBP_NEON_OMIT_C_CODE
static void VFilter8i_C(uint8_t* u, uint8_t* v, int stride, int thresh, int ithresh, int hev_thresh)
{
...
#if !WEBP_NEON_OMIT_C_CODE || WEBP_NEON_WORK_AROUND_GCC
static void HFilter8i_C(uint8_t* u, uint8_t* v, int stride, int thresh, int ithresh, int hev_thresh)
{
...
static void DitherCombine8x8_C(const uint8_t* dither, uint8_t* dst, int dst_stride)
{
    int i, j;
    for (j = 0; j < 8; ++j) {
        for (i = 0; i < 8; ++i) {
            const int delta0 = dither[i] - VP8_DITHER_AMP_CENTER;
            const int delta1 = (delta0 + VP8_DITHER_DESCALE_ROUNDER) >> VP8_DITHER_DESCALE;
...
void (*VP8DitherCombine8x8)(const uint8_t* dither, uint8_t* dst, int dst_stride);
