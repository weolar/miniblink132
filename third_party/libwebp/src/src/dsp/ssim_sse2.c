...
// Helper function
static WEBP_INLINE void SubtractAndSquare_SSE2(const __m128i a, const __m128i b, __m128i* const sum)
{
    // take abs(a-b) in 8b
...
static uint32_t AccumulateSSE_SSE2(const uint8_t* src1, const uint8_t* src2, int len)
{
    int i = 0;
    uint32_t sse2 = 0;
    if (len >= 16) {
...
static double SSIMGet_SSE2(const uint8_t* src1, int stride1, const uint8_t* src2, int stride2)
{
    VP8DistoStats stats;
    const __m128i zero = _mm_setzero_si128();
