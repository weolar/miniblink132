...
static WEBP_INLINE void PredictLine_MIPSdspR2(const uint8_t* src, uint8_t* dst, int length)
{
...
#define DO_PREDICT_LINE_VERTICAL(SRC, PRED, DST, LENGTH, INVERSE)                                                                                              \
    do {                                                                                                                                                       \
        ...
        __asm__ volatile(".set      push                                   \n\t"                                                                               \
                         ...
                         ".set      pop                                    \n\t"                                                                               \
                         : [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3), [temp4] "=&r"(temp4), [temp5] "=&r"(temp5), \
                         [temp6] "=&r"(temp6), [temp7] "=&r"(temp7), [pred] "+&r"(ppred), [dst] "+&r"(pdst), [src] "+&r"(psrc)                                 \
                         : [length] "r"(ilength)                                                                                                               \
                         : "memory");                                                                                                                          \
    } while (0)
...
static WEBP_INLINE void DoHorizontalFilter_MIPSdspR2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
    const uint8_t* preds;
...
static void HorizontalFilter_MIPSdspR2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
    DoHorizontalFilter_MIPSdspR2(data, width, height, stride, 0, height, filtered_data);
}
...
static WEBP_INLINE void DoVerticalFilter_MIPSdspR2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
    const uint8_t* preds;
    const size_t start_offset = row * stride;
    const int last_row = row + num_rows;
...
static void VerticalFilter_MIPSdspR2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
    DoVerticalFilter_MIPSdspR2(data, width, height, stride, 0, height, filtered_data);
}
...
{
    int temp0;
    __asm__ volatile("addu             %[temp0],   %[a],       %[b]        \n\t"
                     "subu             %[temp0],   %[temp0],   %[c]        \n\t"
...
#define FILTER_LINE_BY_LINE(PREDS, OPERATION)                                                                                                                  \
    do {                                                                                                                                                       \
        while (row < last_row) {                                                                                                                               \
            ...
            PREDICT_LINE_ONE_PASS(in, PREDS - stride, out);                                                                                                    \
            for (w = 1; w < width; ++w) {                                                                                                                      \
                const int pred = GradientPredictor_MIPSdspR2(PREDS[w - 1], PREDS[w - stride], PREDS[w - stride - 1]);                                          \
...
static void DoGradientFilter_MIPSdspR2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
    const uint8_t* preds;
    const size_t start_offset = row * stride;
    const int last_row = row + num_rows;
...
static void GradientFilter_MIPSdspR2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
    DoGradientFilter_MIPSdspR2(data, width, height, stride, 0, height, filtered_data);
}

//------------------------------------------------------------------------------

static void HorizontalUnfilter_MIPSdspR2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    out[0] = in[0] + (prev == NULL ? 0 : prev[0]);
    DO_PREDICT_LINE(in + 1, out + 1, width - 1, 1);
}

static void VerticalUnfilter_MIPSdspR2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    if (prev == NULL) {
...
static void GradientUnfilter_MIPSdspR2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    if (prev == NULL) {
        HorizontalUnfilter_MIPSdspR2(NULL, in, out, width);
    } else {
