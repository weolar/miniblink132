...
#define MULT_FIX(A, B) vreinterpretq_u32_s32(vqrdmulhq_s32(vreinterpretq_s32_u32((A)), (B)))
#define MULT_FIX_FLOOR(A, B) vreinterpretq_u32_s32(vqdmulhq_s32(vreinterpretq_s32_u32((A)), (B)))
#else
#error "MULT_FIX/WEBP_RESCALER_RFIX need some more work"
#endif

static uint32x4_t Interpolate_NEON(const rescaler_t* const frow, const rescaler_t* const irow, uint32_t A, uint32_t B)
{
    LOAD_32x4(frow, A0);
    LOAD_32x4(irow, B0);
...
static void RescalerExportRowExpand_NEON(WebPRescaler* const wrk)
{
    ...
    if (wrk->y_accum == 0) {
        ...
        for (x_out = 0; x_out < max_span; x_out += 8) {
            const uint32x4_t C0 = Interpolate_NEON(frow + x_out + 0, irow + x_out + 0, A, B);
            const uint32x4_t C1 = Interpolate_NEON(frow + x_out + 4, irow + x_out + 4, A, B);
