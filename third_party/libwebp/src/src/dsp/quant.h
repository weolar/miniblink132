...
static uint32x2_t horizontal_add_uint32x4(const uint32x4_t a)
{
    ...
    return vadd_u32(vreinterpret_u32_u64(vget_low_u64(b)), vreinterpret_u32_u64(vget_high_u64(b)));
}

static WEBP_INLINE int IsFlat(const int16_t* levels, int num_blocks, int thresh)
{
...
static WEBP_INLINE int IsFlatSource16(const uint8_t* src)
{
    ...
    int i;
    for (i = 0; i < 16; ++i) {
        if (memcmp(src + 0, &v, 4) || memcmp(src + 4, &v, 4) || memcmp(src + 8, &v, 4) || memcmp(src + 12, &v, 4)) {
            return 0;
        }
        src += BPS;
    }
