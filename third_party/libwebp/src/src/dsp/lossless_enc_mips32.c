...
//   }
//   return (float)cost;
static float ExtraCostCombined_MIPS32(const uint32_t* const X, const uint32_t* const Y, int length)
{
    ...
    __asm__ volatile("mult   $zero,    $zero                  \n\t"
                     ...
                     "mfhi   %[temp0]                         \n\t"
                     "mflo   %[temp1]                         \n\t"
                     : [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3), [i] "=&r"(i), [pX] "+r"(pX), [pY] "+r"(pY)
                     : [LoopEnd] "r"(LoopEnd)
                     : "memory", "hi", "lo");
...
#define HUFFMAN_COST_PASS                                                                                                                                      \
    __asm__ volatile("sll   %[temp1],  %[temp0],    3           \n\t"                                                                                          \
...
static void GetEntropyUnrefined_MIPS32(const uint32_t X[], int length, VP8LBitEntropy* const bit_entropy, VP8LStreaks* const stats)
{
    int i;
    int i_prev = 0;
    uint32_t x_prev = X[0];
...
static void GetCombinedEntropyUnrefined_MIPS32(const uint32_t X[], const uint32_t Y[], int length, VP8LBitEntropy* const entropy, VP8LStreaks* const stats)
{
    int i = 1;
    int i_prev = 0;
...
static void AddVector_MIPS32(const uint32_t* pa, const uint32_t* pb, uint32_t* pout, int size)
{
