...
#define MULTHI_16(in0, in1, in2, in3, cnst, out0, out1)                                                                                                        \
    do {                                                                                                                                                       \
        ...
        MUL4(in0, const0, in1, const0, in2, const0, in3, const0, temp0, temp1, temp2, temp3);                                                                  \
...
#define STORE16_3(a0, a1, a2, dst)                                                                                                                             \
    do {                                                                                                                                                       \
        const v16u8 mask0 = { 0, 1, 16, 2, 3, 17, 4, 5, 18, 6, 7, 19, 8, 9, 20, 10 };                                                                          \
        const v16u8 mask1 = { 0, 21, 1, 2, 22, 3, 4, 23, 5, 6, 24, 7, 8, 25, 9, 10 };                                                                          \
        const v16u8 mask2 = { 26, 0, 1, 27, 2, 3, 28, 4, 5, 29, 6, 7, 30, 8, 9, 31 };                                                                          \
...
#define STORE8_3(a0, a1, a2, dst)                                                                                                                              \
    do {                                                                                                                                                       \
        int64_t out_m;                                                                                                                                         \
        const v16u8 mask0 = { 0, 1, 16, 2, 3, 17, 4, 5, 18, 6, 7, 19, 8, 9, 20, 10 };                                                                          \
        const v16u8 mask1 = { 11, 21, 12, 13, 22, 14, 15, 23, 255, 255, 255, 255, 255, 255, 255, 255 };                                                        \
...
#if !defined(WEBP_REDUCE_CSP)
static void YuvToRgbLine(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
    v16u8 R, G, B;
    while (length >= 16) {
...
static void YuvToBgrLine(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
    v16u8 R, G, B;
    while (length >= 16) {
        CALC_RGB16(y, u, v, R, G, B);
...
static void YuvToRgbaLine(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
    v16u8 R, G, B;
...
static void YuvToBgraLine(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
    v16u8 R, G, B;
    const v16u8 A = (v16u8)__msa_ldi_b(ALPHAVAL);
    while (length >= 16) {
...
#if !defined(WEBP_REDUCE_CSP)
static void YuvToArgbLine(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
    v16u8 R, G, B;
    const v16u8 A = (v16u8)__msa_ldi_b(ALPHAVAL);
    while (length >= 16) {
...
static void YuvToRgba4444Line(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
...
static void YuvToRgb565Line(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int length)
{
...
#define UPSAMPLE_FUNC(FUNC_NAME, FUNC, XSTEP)                                                                                                                  \
    static void FUNC_NAME(const uint8_t* top_y, const uint8_t* bot_y, const uint8_t* top_u, const uint8_t* top_v, const uint8_t* cur_u, const uint8_t* cur_v,  \
    ...
    {                                                                                                                                                          \
        ...
        if (!(len & 1)) {                                                                                                                                      \
            ...
            FUNC(top_y[len - 1], tmp0 & 0xff, (tmp0 >> 16), top_dst + (len - 1) * XSTEP);                                                                      \
            if (bot_y != NULL) {                                                                                                                               \
                const uint32_t tmp1 = (3 * c0 + t0 + 0x00020002u) >> 2;                                                                                        \
                FUNC(bot_y[len - 1], tmp1 & 0xff, (tmp1 >> 16), bot_dst + (len - 1) * XSTEP);                                                                  \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    }
