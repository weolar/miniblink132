...
static void SubtractGreenFromBlueAndRed_MIPSdspR2(uint32_t* argb_data, int num_pixels)
{
    ...
    __asm__ volatile(".set       push                                          \n\t"
                     ...
                     "2:                                                         \n\t"
                     ".set       pop                                           \n\t"
                     : [argb_data] "+&r"(argb_data), [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3),
...
static void TransformColor_MIPSdspR2(const VP8LMultipliers* const m, uint32_t* data, int num_pixels)
{
    ...
    __asm__ volatile(".set            push                                    \n\t"
                     ...
                     [new_red1] "=&r"(new_red1), [new_red] "=&r"(new_red), [argb] "=&r"(argb), [argb1] "=&r"(argb1), [data] "+&r"(data)
    ...
    const uint32_t* argb, int stride, int tile_width, int tile_height, int green_to_blue, int red_to_blue, int histo[])
{
    ...
    while (tile_height-- > 0) {
        ...
        for (x = 0; x < (tile_width >> 1); ++x) {
            ...
            __asm__ volatile("lw           %[temp0],  0(%[p_argb])             \n\t"
                             "lw           %[temp1],  4(%[p_argb])             \n\t"
                             ...
                             "subu.qb      %[temp2],  %[temp2],  %[temp6]      \n\t"
                             : [p_argb] "+&r"(p_argb), [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3),
...
static void CollectColorRedTransforms_MIPSdspR2(const uint32_t* argb, int stride, int tile_width, int tile_height, int green_to_red, int histo[])
{
