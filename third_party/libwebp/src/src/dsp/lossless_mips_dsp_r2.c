...
#define MAP_COLOR_FUNCS(FUNC_NAME, TYPE, GET_INDEX, GET_VALUE)                                                                                                 \
    static void FUNC_NAME(const TYPE* src, const uint32_t* const color_map, TYPE* dst, int y_start, int y_end, int width)                                      \
    {                                                                                                                                                          \
        int y;                                                                                                                                                 \
        for (y = y_start; y < y_end; ++y) {                                                                                                                    \
            ...
            for (x = 0; x < (width >> 2); ++x) {                                                                                                               \
                ...
                __asm__ volatile(".ifc        " #TYPE ",  uint8_t                  \n\t"                                                                       \
                                 "lbu       %[tmp1],  0(%[src])                  \n\t"                                                                         \
                                 "lbu       %[tmp2],  1(%[src])                  \n\t"                                                                         \
                                 "lbu       %[tmp3],  2(%[src])                  \n\t"                                                                         \
                                 "lbu       %[tmp4],  3(%[src])                  \n\t"                                                                         \
                                 ...
                                 "addiu     %[dst],   %[dst],      16            \n\t"                                                                         \
                                 ".endif                                           \n\t"                                                                       \
                                 : [tmp1] "=&r"(tmp1), [tmp2] "=&r"(tmp2), [tmp3] "=&r"(tmp3), [tmp4] "=&r"(tmp4), [src] "+&r"(src), [dst] "+r"(dst)           \
...
static WEBP_INLINE uint32_t ClampedAddSubtractFull(uint32_t c0, uint32_t c1, uint32_t c2)
{
    ...
    __asm__ volatile("preceu.ph.qbr   %[temp1],   %[c0]                 \n\t"
...
{
    int temp0, temp1, temp2, temp3, temp4, temp5;
    __asm__ volatile("adduh.qb         %[temp5],   %[c0],      %[c1]       \n\t"
...
static uint32_t Predictor5_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return Average3(*left, top[0], top[1]);
}

static uint32_t Predictor6_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return Average2(*left, top[-1]);
}

static uint32_t Predictor7_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return Average2(*left, top[0]);
}

static uint32_t Predictor8_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    (void)left;
    return Average2(top[-1], top[0]);
}

static uint32_t Predictor9_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    (void)left;
    return Average2(top[0], top[1]);
}

static uint32_t Predictor10_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return Average4(*left, top[-1], top[0], top[1]);
}

static uint32_t Predictor11_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return Select(top[0], *left, top[-1]);
}

static uint32_t Predictor12_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
    return ClampedAddSubtractFull(*left, top[0], top[-1]);
}

static uint32_t Predictor13_MIPSdspR2(const uint32_t* const left, const uint32_t* const top)
{
...
// 'subtract green').
static void AddGreenToBlueAndRed_MIPSdspR2(const uint32_t* src, int num_pixels, uint32_t* dst)
{
    ...
    __asm__ volatile(".set       push                                          \n\t"
                     ...
                     "2:                                                         \n\t"
                     ".set       pop                                           \n\t"
                     : [dst] "+&r"(dst), [src] "+&r"(src), [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3),
                     [temp4] "=&r"(temp4), [temp5] "=&r"(temp5), [temp6] "=&r"(temp6), [temp7] "=&r"(temp7)
                     : [p_loop1_end] "r"(p_loop1_end), [p_loop2_end] "r"(p_loop2_end)
                     : "memory");
}

static void TransformColorInverse_MIPSdspR2(const VP8LMultipliers* const m, const uint32_t* src, int num_pixels, uint32_t* dst)
{
    int temp0, temp1, temp2, temp3, temp4, temp5;
    ...
    __asm__ volatile(".set            push                                    \n\t"
                     ...
                     ".set            pop                                     \n\t"
                     : [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3), [temp4] "=&r"(temp4), [temp5] "=&r"(temp5),
                     [new_red] "=&r"(new_red), [argb] "=&r"(argb), [argb1] "=&r"(argb1), [dst] "+&r"(dst), [src] "+&r"(src)
...
static void ConvertBGRAToRGB_MIPSdspR2(const uint32_t* src, int num_pixels, uint8_t* dst)
{
...
static void ConvertBGRAToRGBA_MIPSdspR2(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    int temp0, temp1, temp2, temp3;
    const uint32_t* const p_loop1_end = src + (num_pixels & ~3);
    const uint32_t* const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToRGBA4444_MIPSdspR2(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    int temp0, temp1, temp2, temp3, temp4, temp5;
    const uint32_t* const p_loop1_end = src + (num_pixels & ~3);
    const uint32_t* const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToRGB565_MIPSdspR2(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    int temp0, temp1, temp2, temp3, temp4, temp5;
    const uint32_t* const p_loop1_end = src + (num_pixels & ~3);
    const uint32_t* const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToBGR_MIPSdspR2(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    int temp0, temp1, temp2, temp3;
    const uint32_t* const p_loop1_end = src + (num_pixels & ~3);
    const uint32_t* const p_loop2_end = src + num_pixels;
