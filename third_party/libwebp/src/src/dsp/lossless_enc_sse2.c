...
#define MK_CST_16(HI, LO) _mm_set1_epi32((int)(((uint32_t)(HI) << 16) | ((LO)&0xffff)))

static void TransformColor_SSE2(const VP8LMultipliers* const m, uint32_t* argb_data, int num_pixels)
{
    const __m128i mults_rb = MK_CST_16(CST_5b(m->green_to_red_), CST_5b(m->green_to_blue_));
...
//------------------------------------------------------------------------------
#define SPAN 8
static void CollectColorBlueTransforms_SSE2(const uint32_t* argb, int stride, int tile_width, int tile_height, int green_to_blue, int red_to_blue, int histo[])
{
...
static void CollectColorRedTransforms_SSE2(const uint32_t* argb, int stride, int tile_width, int tile_height, int green_to_red, int histo[])
{
...
static void AddVector_SSE2(const uint32_t* a, const uint32_t* b, uint32_t* out, int size)
{
    int i;
...
static int VectorMismatch_SSE2(const uint32_t* const array1, const uint32_t* const array2, int length)
{
    int match_len;
    ...
    if (length >= 12) {
        ...
        do {
            ...
            const __m128i B0 = _mm_loadu_si128((const __m128i*)&array1[match_len + 4]);
            const __m128i B1 = _mm_loadu_si128((const __m128i*)&array2[match_len + 4]);
        ...
        if (length >= 4
            && _mm_movemask_epi8(_mm_cmpeq_epi32(_mm_loadu_si128((const __m128i*)&array1[0]), _mm_loadu_si128((const __m128i*)&array2[0]))) == 0xffff) {
            match_len = 4;
            if (length >= 8
                && _mm_movemask_epi8(_mm_cmpeq_epi32(_mm_loadu_si128((const __m128i*)&array1[4]), _mm_loadu_si128((const __m128i*)&array2[4]))) == 0xffff) {
                match_len = 8;
            }
        }
    }
...
static void BundleColorMap_SSE2(const uint8_t* const row, int width, int xbits, uint32_t* dst)
{
    int x;
    assert(xbits >= 0);
    assert(xbits <= 3);
...
static WEBP_INLINE void Average2_m128i(const __m128i* const a0, const __m128i* const a1, __m128i* const avg)
{
    // (a + b) >> 1 = ((a + b + 1) >> 1) - ((a ^ b) & 1)
...
static void PredictorSub0_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
...
#define GENERATE_PREDICTOR_1(X, IN)                                                                                                                            \
    static void PredictorSub##X##_SSE2(const uint32_t* const in, const uint32_t* const upper, int num_pixels, uint32_t* const out)                             \
    {                                                                                                                                                          \
        int i;                                                                                                                                                 \
        for (i = 0; i + 4 <= num_pixels; i += 4) {                                                                                                             \
            const __m128i src = _mm_loadu_si128((const __m128i*)&in[i]);                                                                                       \
        ...
        if (i != num_pixels) {                                                                                                                                 \
            VP8LPredictorsSub_C[(X)](in + i, WEBP_OFFSET_PTR(upper, i), num_pixels - i, out + i);                                                              \
        }                                                                                                                                                      \
    }
...
static void PredictorSub5_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
#define GENERATE_PREDICTOR_2(X, A, B)                                                                                                                          \
    static void PredictorSub##X##_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)                                               \
    {                                                                                                                                                          \
        int i;                                                                                                                                                 \
        for (i = 0; i + 4 <= num_pixels; i += 4) {                                                                                                             \
...
static void PredictorSub10_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    for (i = 0; i + 4 <= num_pixels; i += 4) {
        const __m128i L = _mm_loadu_si128((const __m128i*)&in[i - 1]);
...
// Predictor11: select.
static void GetSumAbsDiff32_SSE2(const __m128i* const A, const __m128i* const B, __m128i* const out)
{
...
static void PredictorSub11_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    for (i = 0; i + 4 <= num_pixels; i += 4) {
        const __m128i L = _mm_loadu_si128((const __m128i*)&in[i - 1]);
...
static void PredictorSub12_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    const __m128i zero = _mm_setzero_si128();
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
static void PredictorSub13_SSE2(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    const __m128i zero = _mm_setzero_si128();
