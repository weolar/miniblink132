...
static void PredictLineTop_SSE2(const uint8_t* src, const uint8_t* pred, uint8_t* dst, int length)
{
    int i;
...
static WEBP_INLINE void DoHorizontalFilter_SSE2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
...
static WEBP_INLINE void DoVerticalFilter_SSE2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
    const size_t start_offset = row * stride;
    const int last_row = row + num_rows;
    SANITY_CHECK(in, out);
...
static void GradientPredictDirect_SSE2(const uint8_t* const row, const uint8_t* const top, uint8_t* const out, int length)
{
...
static WEBP_INLINE void DoGradientFilter_SSE2(const uint8_t* in, int width, int height, int stride, int row, int num_rows, uint8_t* out)
{
    const size_t start_offset = row * stride;
    const int last_row = row + num_rows;
    SANITY_CHECK(in, out);
...
static void HorizontalFilter_SSE2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
    DoHorizontalFilter_SSE2(data, width, height, stride, 0, height, filtered_data);
}

static void VerticalFilter_SSE2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
    DoVerticalFilter_SSE2(data, width, height, stride, 0, height, filtered_data);
}

static void GradientFilter_SSE2(const uint8_t* data, int width, int height, int stride, uint8_t* filtered_data)
{
...
static void HorizontalUnfilter_SSE2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    int i;
    __m128i last;
...
static void VerticalUnfilter_SSE2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    if (prev == NULL) {
...
static void GradientPredictInverse_SSE2(const uint8_t* const in, const uint8_t* const top, uint8_t* const row, int length)
{
    if (length > 0) {
        int i;
        const int max_pos = length & ~7;
...
static void GradientUnfilter_SSE2(const uint8_t* prev, const uint8_t* in, uint8_t* out, int width)
{
    if (prev == NULL) {
        HorizontalUnfilter_SSE2(NULL, in, out, width);
    } else {
