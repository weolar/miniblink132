...
static void ImportRowShrink_MIPS32(WebPRescaler* const wrk, const uint8_t* src)
{
    ...
    for (channel = 0; channel < x_stride; ++channel) {
        ...
        __asm__ volatile(
            ...
            "bgtz   %[loop_c],  1b                        \n\t"
            : [accum] "=&r"(accum), [src1] "+r"(src1), [temp3] "=&r"(temp3), [sum] "=&r"(sum), [base] "=&r"(base), [frac] "=&r"(frac), [frow] "+r"(frow),
            [accum1] "=&r"(accum1), [temp2] "=&r"(temp2), [temp1] "=&r"(temp1)
...
static void ImportRowExpand_MIPS32(WebPRescaler* const wrk, const uint8_t* src)
{
    const int x_stride = wrk->num_channels;
    const int x_out_max = wrk->dst_width * wrk->num_channels;
    const int x_add = wrk->x_add;
    ...
    for (channel = 0; channel < x_stride; ++channel) {
        ...
        __asm__ volatile("addiu  %[temp3],   %[src_width], -1            \n\t"
                         ...
                         "b      1b                                      \n\t"
                         "2:                                               \n\t"
                         : [src1] "+r"(src1), [accum] "=&r"(accum), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3), [temp4] "=&r"(temp4),
                         [x_out] "+r"(x_out), [frac] "=&r"(frac), [frow] "+r"(frow)
                         : [x_stride] "r"(x_stride), [x_add] "r"(x_add), [x_sub] "r"(x_sub), [x_stride1] "r"(x_stride1), [src_width] "r"(src_width),
...
static void ExportRowExpand_MIPS32(WebPRescaler* const wrk)
{
    ...
    assert(wrk->y_sub != 0);
    if (wrk->y_accum == 0) {
        __asm__ volatile("li       %[temp3],    0x10000                    \n\t"
                         "li       %[temp4],    0x8000                     \n\t"
                         ...
                         : [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp3] "=&r"(temp3), [temp4] "=&r"(temp4), [temp5] "=&r"(temp5), [frow] "+r"(frow),
