...
static void ITransform_MSA(const uint8_t* ref, const int16_t* in, uint8_t* dst, int do_two)
{
...
static void FTransform_MSA(const uint8_t* src, const uint8_t* ref, int16_t* out)
{
...
static int Disto4x4_MSA(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
...
static int Disto16x16_MSA(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
    int D = 0;
    int x, y;
...
static void CollectHistogram_MSA(const uint8_t* ref, const uint8_t* pred, int start_block, int end_block, VP8Histogram* const histo)
{
    int j;
...
static WEBP_INLINE void VE4(uint8_t* dst, const uint8_t* top)
{ // vertical
    const v16u8 A1 = { 0 };
...
static WEBP_INLINE void HE4(uint8_t* dst, const uint8_t* top)
{ // horizontal
    const int X = top[-1];
    const int I = top[-2];
    const int J = top[-3];
...
static WEBP_INLINE void HorizontalPred16x16(uint8_t* dst, const uint8_t* left)
{
    if (left != NULL) {
        int j;
        for (j = 0; j < 16; j += 4) {
...
static WEBP_INLINE void TrueMotion16x16(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    if (left != NULL) {
        if (top != NULL) {
            int j;
...
static WEBP_INLINE void DCMode16x16(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    int DC;
    v16u8 out;
...
static void Intra16Preds_MSA(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
...
static WEBP_INLINE void TrueMotion8x8(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    if (left != NULL) {
        if (top != NULL) {
            int j;
...
static WEBP_INLINE void DCMode8x8(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    uint64_t out;
    v16u8 src = { 0 };
    if (top != NULL && left != NULL) {
...
static void IntraChromaPreds_MSA(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    // U block
...
static int QuantizeBlock_MSA(int16_t in[16], int16_t out[16], const VP8Matrix* const mtx)
{
    int sum;
...
static int Quantize2Blocks_MSA(int16_t in[32], int16_t out[32], const VP8Matrix* const mtx)
{
    int nz;
