...
static int x86CPUInfo(CPUFeature feature)
{
    ...
    if (max_cpuid_value < 1) {
        ...
        is_intel = (cpu_info[1] == VENDOR_ID_INTEL_EBX && cpu_info[2] == VENDOR_ID_INTEL_ECX && cpu_info[3] == VENDOR_ID_INTEL_EDX); // genuine Intel?
    }
...
static int AndroidCPUInfo(CPUFeature feature)
{
    ...
    if (feature == kNEON) {
        return cpu_family == ANDROID_CPU_FAMILY_ARM && (cpu_features & ANDROID_CPU_ARM_FEATURE_NEON) != 0;
    }
    return 0;
}
...
#elif defined(WEBP_USE_MIPS32) || defined(WEBP_USE_MIPS_DSP_R2) || defined(WEBP_USE_MSA)
