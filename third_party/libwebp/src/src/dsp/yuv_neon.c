...
static uint8x8_t ConvertRGBToY_NEON(const uint8x8_t R, const uint8x8_t G, const uint8x8_t B)
{
    ...
    const uint16x8_t Y1 = vcombine_u16(vrshrn_n_u32(tmp2_lo, 16), vrshrn_n_u32(tmp2_hi, 16));
...
#define MULTIPLY_16b(C0, C1, C2, CST, DST_s16)                                                                                                                 \
    do {                                                                                                                                                       \
        ...
        const int16x8_t tmp3 = vcombine_s16(vshrn_n_s32(tmp2_lo, 16), vshrn_n_s32(tmp2_hi, 16));                                                               \
...
static void ConvertRGBA32ToUV_NEON(const uint16_t* rgb, uint8_t* u, uint8_t* v, int width)
{
    int i;
...
static void ConvertARGBToUV_NEON(const uint32_t* argb, uint8_t* u, uint8_t* v, int src_width, int do_store)
{
    int i;
