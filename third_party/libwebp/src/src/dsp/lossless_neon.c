...
// gcc-4.8.x at least.
static void ConvertBGRAToRGBA_NEON(const uint32_t* src, int num_pixels, uint8_t* dst)
{
...
static void ConvertBGRAToBGR_NEON(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    const uint32_t* const end = src + (num_pixels & ~15);
    for (; src < end; src += 16) {
...
static void ConvertBGRAToRGB_NEON(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    const uint32_t* const end = src + (num_pixels & ~15);
    for (; src < end; src += 16) {
        const uint8x16x4_t pixel = vld4q_u8((uint8_t*)src);
...
static const uint8_t kBGRShuffle[3][8] = { { 0, 1, 2, 4, 5, 6, 8, 9 }, { 10, 12, 13, 14, 16, 17, 18, 20 }, { 21, 22, 24, 25, 26, 28, 29, 30 } };
...
static void ConvertBGRAToBGR_NEON(const uint32_t* src, int num_pixels, uint8_t* dst)
{
    ...
    for (; src < end; src += 8) {
        uint8x8x4_t pixels;
        INIT_VECTOR4(pixels, vld1_u8((const uint8_t*)(src + 0)), vld1_u8((const uint8_t*)(src + 2)), vld1_u8((const uint8_t*)(src + 4)),
...
static const uint8_t kRGBShuffle[3][8] = { { 2, 1, 0, 6, 5, 4, 10, 9 }, { 8, 14, 13, 12, 18, 17, 16, 22 }, { 21, 20, 26, 25, 24, 30, 29, 28 } };
...
static WEBP_INLINE uint32_t ClampedAddSubtractHalf_NEON(uint32_t c0, uint32_t c1, uint32_t c2)
{
...
static WEBP_INLINE uint32_t Average3_NEON(uint32_t a0, uint32_t a1, uint32_t a2)
{
...
static uint32_t Predictor5_NEON(const uint32_t* const left, const uint32_t* const top)
{
    return Average3_NEON(*left, top[0], top[1]);
}
static uint32_t Predictor6_NEON(const uint32_t* const left, const uint32_t* const top)
{
    return Average2_NEON(*left, top[-1]);
}
static uint32_t Predictor7_NEON(const uint32_t* const left, const uint32_t* const top)
{
    return Average2_NEON(*left, top[0]);
}
static uint32_t Predictor13_NEON(const uint32_t* const left, const uint32_t* const top)
{
...
static void PredictorAdd0_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
...
// Predictor1: left.
static void PredictorAdd1_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
...
#define GENERATE_PREDICTOR_1(X, IN)                                                                                                                            \
    static void PredictorAdd##X##_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)                                               \
    {                                                                                                                                                          \
        int i;                                                                                                                                                 \
        for (i = 0; i + 4 <= num_pixels; i += 4) {                                                                                                             \
            const uint8x16_t src = LOADQ_U32P_AS_U8(&in[i]);                                                                                                   \
...
static void PredictorAdd5_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
...
static void PredictorAdd6_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    uint8x16_t L = LOADQ_U32_AS_U8(out[-1]);
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
static void PredictorAdd7_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    uint8x16_t L = LOADQ_U32_AS_U8(out[-1]);
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
static void PredictorAdd10_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    uint8x16_t L = LOADQ_U32_AS_U8(out[-1]);
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
static void PredictorAdd11_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    uint8x16_t L = LOADQ_U32_AS_U8(out[-1]);
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
#define DO_PRED12(DIFF, LANE)                                                                                                                                  \
    do {                                                                                                                                                       \
        const uint8x8_t pred = vqmovun_s16(vaddq_s16(vreinterpretq_s16_u16(L), (DIFF)));                                                                       \
        const uint8x8_t res = vadd_u8(pred, (LANE <= 1) ? vget_low_u8(src) : vget_high_u8(src));                                                               \
...
static void PredictorAdd12_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    ...
    for (i = 0; i + 4 <= num_pixels; i += 4) {
        ...
        const uint8x16_t TL = LOADQ_U32P_AS_U8(&upper[i - 1]);
        const uint8x16_t T = LOADQ_U32P_AS_U8(&upper[i]);
        const int16x8_t diff_lo = vreinterpretq_s16_u16(vsubl_u8(vget_low_u8(T), vget_low_u8(TL)));
        const int16x8_t diff_hi = vreinterpretq_s16_u16(vsubl_u8(vget_high_u8(T), vget_high_u8(TL)));
...
#define DO_PRED13(LANE, LOW_OR_HI)                                                                                                                             \
    do {                                                                                                                                                       \
        ...
        const int8x8_t diff_avg = vreinterpret_s8_u8(LOW_OR_HI(vhsubq_u8(avg, TL_1)));                                                                         \
        /* Compute the sum with avg and saturate. */                                                                                                           \
...
static void PredictorAdd13_NEON(const uint32_t* in, const uint32_t* upper, int num_pixels, uint32_t* out)
{
    int i;
    uint8x16_t L = LOADQ_U32_AS_U8(out[-1]);
    for (i = 0; i + 4 <= num_pixels; i += 4) {
...
static WEBP_INLINE uint8x16_t DoGreenShuffle_NEON(const uint8x16_t argb, const uint8x16_t shuffle)
{
...
static WEBP_INLINE uint8x16_t DoGreenShuffle_NEON(const uint8x16_t argb, const uint8x8_t shuffle)
{
    return vcombine_u8(vtbl1_u8(vget_low_u8(argb), shuffle), vtbl1_u8(vget_high_u8(argb), shuffle));
}
#endif // USE_VTBLQ

static void AddGreenToBlueAndRed_NEON(const uint32_t* src, int num_pixels, uint32_t* dst)
{
...
static void TransformColorInverse_NEON(const VP8LMultipliers* const m, const uint32_t* const src, int num_pixels, uint32_t* dst)
{
// sign-extended multiplying constants, pre-shifted by 6.
#define CST(X) (((int16_t)(m->X << 8)) >> 6)
    const int16_t rb[8] = { CST(green_to_blue_), CST(green_to_red_), CST(green_to_blue_), CST(green_to_red_), CST(green_to_blue_), CST(green_to_red_),
