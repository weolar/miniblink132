...
static void ITransform_SSE2(const uint8_t* ref, const int16_t* in, uint8_t* dst, int do_two)
{
...
static void FTransformPass1_SSE2(const __m128i* const in01, const __m128i* const in23, __m128i* const out01, __m128i* const out32)
{
...
static void FTransformPass2_SSE2(const __m128i* const v01, const __m128i* const v32, int16_t* out)
{
    const __m128i zero = _mm_setzero_si128();
...
static void FTransform_SSE2(const uint8_t* src, const uint8_t* ref, int16_t* out)
{
    const __m128i zero = _mm_setzero_si128();
    // Load src.
...
static void FTransform2_SSE2(const uint8_t* src, const uint8_t* ref, int16_t* out)
{
    const __m128i zero = _mm_setzero_si128();
...
static void CollectHistogram_SSE2(const uint8_t* ref, const uint8_t* pred, int start_block, int end_block, VP8Histogram* const histo)
{
    const __m128i zero = _mm_setzero_si128();
...
static WEBP_INLINE void VerticalPred_SSE2(uint8_t* dst, const uint8_t* top, int size)
{
    if (top != NULL) {
        if (size == 8) {
            VE8uv_SSE2(dst, top);
...
static WEBP_INLINE void HorizontalPred_SSE2(uint8_t* dst, const uint8_t* left, int size)
{
    if (left != NULL) {
        if (size == 8) {
            HE8uv_SSE2(dst, left);
...
static WEBP_INLINE void TM_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top, int size)
{
    const __m128i zero = _mm_setzero_si128();
    int y;
    if (size == 8) {
...
static WEBP_INLINE void TrueMotion_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top, int size)
{
    if (left != NULL) {
        if (top != NULL) {
...
static WEBP_INLINE void DC8uv_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    const __m128i top_values = _mm_loadl_epi64((const __m128i*)top);
...
static WEBP_INLINE void DC8uvMode_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    if (top != NULL) {
...
static WEBP_INLINE void DC16_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
...
static WEBP_INLINE void DC16Mode_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    if (top != NULL) {
        if (left != NULL) { // top and left present
...
static WEBP_INLINE void VE4_SSE2(uint8_t* dst, const uint8_t* top)
{ // vertical
...
static WEBP_INLINE void HE4_SSE2(uint8_t* dst, const uint8_t* top)
{ // horizontal
    const int X = top[-1];
    const int I = top[-2];
    const int J = top[-3];
...
static WEBP_INLINE void LD4_SSE2(uint8_t* dst, const uint8_t* top)
{ // Down-Left
    const __m128i one = _mm_set1_epi8(1);
...
static WEBP_INLINE void VR4_SSE2(uint8_t* dst, const uint8_t* top)
{ // Vertical-Right
    const __m128i one = _mm_set1_epi8(1);
    const int I = top[-2];
    const int J = top[-3];
...
static WEBP_INLINE void VL4_SSE2(uint8_t* dst, const uint8_t* top)
{ // Vertical-Left
    const __m128i one = _mm_set1_epi8(1);
    const __m128i ABCDEFGH = _mm_loadl_epi64((const __m128i*)top);
...
static WEBP_INLINE void RD4_SSE2(uint8_t* dst, const uint8_t* top)
{ // Down-right
    const __m128i one = _mm_set1_epi8(1);
...
static void IntraChromaPreds_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    // U block
...
static void Intra16Preds_SSE2(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
...
static WEBP_INLINE void SubtractAndAccumulate_SSE2(const __m128i a, const __m128i b, __m128i* const sum)
{
    // take abs(a-b) in 8b
...
static WEBP_INLINE int SSE_16xN_SSE2(const uint8_t* a, const uint8_t* b, int num_pairs)
{
...
static int TTransform_SSE2(const uint8_t* inA, const uint8_t* inB, const uint16_t* const w)
{
    int32_t sum[4];
...
static int Disto4x4_SSE2(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
    const int diff_sum = TTransform_SSE2(a, b, w);
    return abs(diff_sum) >> 5;
}

static int Disto16x16_SSE2(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
    int D = 0;
    int x, y;
...
static WEBP_INLINE int DoQuantizeBlock_SSE2(int16_t in[16], int16_t out[16], const uint16_t* const sharpen, const VP8Matrix* const mtx)
{
