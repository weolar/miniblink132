...
static int DispatchAlpha_MIPSdspR2(const uint8_t* alpha, int alpha_stride, int width, int height, uint8_t* dst, int dst_stride)
{
    ...
    for (j = 0; j < height; ++j) {
        ...
        for (i = 0; i < (width >> 2); ++i) {
            ...
            __asm__ volatile("ulw    %[temp0],      0(%[palpha])                \n\t"
                             ...
                             "sb     %[temp2],      -8(%[pdst])                 \n\t"
                             "sb     %[temp3],      -4(%[pdst])                 \n\t"
                             : [temp0] "=&r"(temp0), [temp1] "=&r"(temp1), [temp2] "=&r"(temp2), [temp3] "=&r"(temp3), [palpha] "+r"(palpha), [pdst] "+r"(pdst),
        ...
        for (i = 0; i < (width & 3); ++i) {
            __asm__ volatile("lbu    %[temp0],      0(%[palpha])                \n\t"
    ...
    __asm__ volatile("ext    %[temp0],      %[alpha_mask], 0, 16            \n\t"
    ...
    for (x = 0; x < width; ++x) {
        ...
        if (argb < 0xff000000u) { // alpha < 255
            if (argb <= 0x00ffffffu) { // alpha == 0
                ...
                __asm__ volatile("srl          %[alpha],   %[argb],       24                \n\t"
                                 ...
                                 : [inverse] "r"(inverse), [c_00ffffff] "r"(c_00ffffff), [c_8000000] "r"(c_8000000), [c_8000080] "r"(c_8000080),
...
static void PackARGB_MIPSdspR2(const uint8_t* a, const uint8_t* r, const uint8_t* g, const uint8_t* b, int len, uint32_t* out)
{
    int temp0, temp1, temp2, temp3, offset;
    const int rest = len & 1;
    const uint32_t* const loop_end = out + len - rest;
    const int step = 4;
    __asm__ volatile("xor          %[offset],   %[offset], %[offset]    \n\t"
                     "beq          %[loop_end], %[out],    0f           \n\t"
                     "2:                                                  \n\t"
                     "lbux         %[temp0],    %[offset](%[a])         \n\t"
                     "lbux         %[temp1],    %[offset](%[r])         \n\t"
...
static void PackRGB_MIPSdspR2(const uint8_t* r, const uint8_t* g, const uint8_t* b, int len, int step, uint32_t* out)
{
