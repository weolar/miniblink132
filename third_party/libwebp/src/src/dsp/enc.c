...
const int VP8DspScan[16 + 4 + 4] = {
    // Luma
    0 + 0 * BPS, 4 + 0 * BPS, 8 + 0 * BPS, 12 + 0 * BPS, 0 + 4 * BPS, 4 + 4 * BPS, 8 + 4 * BPS, 12 + 4 * BPS, 0 + 8 * BPS, 4 + 8 * BPS, 8 + 8 * BPS,
    12 + 8 * BPS, 0 + 12 * BPS, 4 + 12 * BPS, 8 + 12 * BPS, 12 + 12 * BPS,
...
#if !WEBP_NEON_OMIT_C_CODE
static void CollectHistogram_C(const uint8_t* ref, const uint8_t* pred, int start_block, int end_block, VP8Histogram* const histo)
{
    int j;
...
static void ITransform_C(const uint8_t* ref, const int16_t* in, uint8_t* dst, int do_two)
{
...
static void FTransform2_C(const uint8_t* src, const uint8_t* ref, int16_t* out)
{
...
static void IntraChromaPreds_C(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
    // U block
...
static void Intra16Preds_C(uint8_t* dst, const uint8_t* left, const uint8_t* top)
{
...
static int Disto4x4_C(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
...
static int Disto16x16_C(const uint8_t* const a, const uint8_t* const b, const uint16_t* const w)
{
    int D = 0;
    int x, y;
...
// Simple quantization
static int QuantizeBlock_C(int16_t in[16], int16_t out[16], const VP8Matrix* const mtx)
{
    int last = -1;
    int n;
    for (n = 0; n < 16; ++n) {
...
static int Quantize2Blocks_C(int16_t in[32], int16_t out[32], const VP8Matrix* const mtx)
{
    int nz;
