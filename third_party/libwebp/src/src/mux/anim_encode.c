...
WebPAnimEncoder* WebPAnimEncoderNewInternal(int width, int height, const WebPAnimEncoderOptions* enc_options, int abi_version)
{
    ...
    *(int*)&enc->canvas_height_ = height;
    ...
    // Canvas buffers.
    if (!WebPPictureInit(&enc->curr_canvas_copy_) || !WebPPictureInit(&enc->prev_canvas_) || !WebPPictureInit(&enc->prev_canvas_disposed_)) {
        goto Err;
    }
...
typedef int (*ComparePixelsFunc)(const uint32_t*, int, const uint32_t*, int, int, int);
...
static WEBP_INLINE int ComparePixelsLossless(const uint32_t* src, int src_step, const uint32_t* dst, int dst_step, int length, int max_allowed_diff)
{
...
// 'max_allowed_diff'.
static WEBP_INLINE int PixelsAreSimilar(uint32_t src, uint32_t dst, int max_allowed_diff)
{
    ...
    return (src_a == dst_a) && (abs(src_r - dst_r) * dst_a <= (max_allowed_diff * 255)) && (abs(src_g - dst_g) * dst_a <= (max_allowed_diff * 255))
...
static WEBP_INLINE int ComparePixelsLossy(const uint32_t* src, int src_step, const uint32_t* dst, int dst_step, int length, int max_allowed_diff)
{
    assert(length > 0);
    while (length-- > 0) {
...
static void MinimizeChangeRectangle(const WebPPicture* const src, const WebPPicture* const dst, FrameRectangle* const rect, int is_lossless, float quality)
{
    int i, j;
    const ComparePixelsFunc compare_pixels = is_lossless ? ComparePixelsLossless : ComparePixelsLossy;
    ...
    for (i = rect->x_offset_; i < rect->x_offset_ + rect->width_; ++i) {
        ...
        if (compare_pixels(src_argb, src->argb_stride, dst_argb, dst->argb_stride, rect->height_, max_allowed_diff)) {
    ...
    for (j = rect->y_offset_; j < rect->y_offset_ + rect->height_; ++j) {
        ...
        if (compare_pixels(src_argb, 1, dst_argb, 1, rect->width_, max_allowed_diff)) {
...
static int SubFrameParamsInit(SubFrameParams* const params, int should_try, int empty_rect_allowed)
{
    ...
    if (!WebPPictureInit(&params->sub_frame_ll_) || !WebPPictureInit(&params->sub_frame_lossy_)) {
        return 0;
    }
    return 1;
}
...
static int GetSubRect(const WebPPicture* const prev_canvas, const WebPPicture* const curr_canvas, int is_key_frame, int is_first_frame, int empty_rect_allowed,
    int is_lossless, float quality, FrameRectangle* const rect, WebPPicture* const sub_frame)
{
...
static int GetSubRects(const WebPPicture* const prev_canvas, const WebPPicture* const curr_canvas, int is_key_frame, int is_first_frame, float quality,
    SubFrameParams* const params)
{
    ...
    if (!GetSubRect(
            prev_canvas, curr_canvas, is_key_frame, is_first_frame, params->empty_rect_allowed_, 1, quality, &params->rect_ll_, &params->sub_frame_ll_)) {
        return 0;
    }
    // Lossy frame rectangle.
    params->rect_lossy_ = params->rect_ll_; // seed with lossless rect.
    return GetSubRect(
        prev_canvas, curr_canvas, is_key_frame, is_first_frame, params->empty_rect_allowed_, 0, quality, &params->rect_lossy_, &params->sub_frame_lossy_);
}
...
int WebPAnimEncoderRefineRect(const WebPPicture* const prev_canvas, const WebPPicture* const curr_canvas, int is_lossless, float quality, int* const x_offset,
    int* const y_offset, int* const width, int* const height)
{
    ...
    if (prev_canvas == NULL || curr_canvas == NULL || prev_canvas->width != curr_canvas->width || prev_canvas->height != curr_canvas->height
...
static int IsLossyBlendingPossible(const WebPPicture* const src, const WebPPicture* const dst, const FrameRectangle* const rect, float quality)
{
    ...
    for (j = rect->y_offset_; j < rect->y_offset_ + rect->height_; ++j) {
        for (i = rect->x_offset_; i < rect->x_offset_ + rect->width_; ++i) {
            ...
            const uint32_t dst_pixel = dst->argb[j * dst->argb_stride + i];
            const uint32_t dst_alpha = dst_pixel >> 24;
            if (dst_alpha != 0xff && !PixelsAreSimilar(src_pixel, dst_pixel, max_allowed_diff_lossy)) {
                // In this case, if we use blending, we can't attain the desired
                // 'dst_pixel' value for this pixel. So, blending is not possible.
                return 0;
            }
...
static int IncreaseTransparency(const WebPPicture* const src, const FrameRectangle* const rect, WebPPicture* const dst)
{
    int i, j;
    int modified = 0;
...
// Returns true if at least one pixel gets modified.
static int FlattenSimilarBlocks(const WebPPicture* const src, const FrameRectangle* const rect, WebPPicture* const dst, float quality)
{
    const int max_allowed_diff_lossy = QualityToMaxDiff(quality);
    int i, j;
    int modified = 0;
        ...
        for (i = x_start; i < x_end; i += block_size) {
            ...
            for (y = 0; y < block_size; ++y) {
                for (x = 0; x < block_size; ++x) {
                    ...
                    if (alpha == 0xff && PixelsAreSimilar(src_pixel, pdst[x + y * dst->argb_stride], max_allowed_diff_lossy)) {
                        ++cnt;
            ...
            if (cnt == block_size * block_size) {
                const uint32_t color = (0x00 << 24) | ((avg_r / cnt) << 16) | ((avg_g / cnt) << 8) | ((avg_b / cnt) << 0);
                for (y = 0; y < block_size; ++y) {
                    for (x = 0; x < block_size; ++x) {
...
// 'params'.
static WebPEncodingError GenerateCandidates(WebPAnimEncoder* const enc, Candidate candidates[CANDIDATE_COUNT], WebPMuxAnimDispose dispose_method,
    int is_lossless, int is_key_frame, SubFrameParams* const params, const WebPConfig* const config_ll, const WebPConfig* const config_lossy)
{
    WebPEncodingError error_code = VP8_ENC_OK;
...
static void SetPreviousDisposeMethod(WebPAnimEncoder* const enc, WebPMuxAnimDispose dispose_method)
{
    ...
    if (enc->prev_candidate_undecided_) {
        ...
        WebPMuxFrameInfo* const prev_info = prev_enc_frame->is_key_frame_ ? &prev_enc_frame->key_frame_ : &prev_enc_frame->sub_frame_;
...
static int IncreasePreviousDuration(WebPAnimEncoder* const enc, int duration)
{
    ...
    assert(enc->count_ >= 1);
    assert(!prev_enc_frame->is_key_frame_ || prev_enc_frame->sub_frame_.duration == prev_enc_frame->key_frame_.duration);
    ...
    if (new_duration >= MAX_DURATION) { // Special case.
        ...
        const uint8_t lossless_1x1_bytes[] = { 0x52, 0x49, 0x46, 0x46, 0x14, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42, 0x50, 0x56, 0x50, 0x38, 0x4c, 0x08, 0x00, 0x00,
            0x00, 0x2f, 0x00, 0x00, 0x00, 0x10, 0x88, 0x88, 0x08 };
        const WebPData lossless_1x1 = { lossless_1x1_bytes, sizeof(lossless_1x1_bytes) };
        const uint8_t lossy_1x1_bytes[] = { 0x52, 0x49, 0x46, 0x46, 0x40, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42, 0x50, 0x56, 0x50, 0x38, 0x58, 0x0a, 0x00, 0x00,
            0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x41, 0x4c, 0x50, 0x48, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x56, 0x50, 0x38,
            0x20, 0x18, 0x00, 0x00, 0x00, 0x30, 0x01, 0x00, 0x9d, 0x01, 0x2a, 0x01, 0x00, 0x01, 0x00, 0x02, 0x00, 0x34, 0x25, 0xa4, 0x00, 0x03, 0x70, 0x00,
...
static void PickBestCandidate(WebPAnimEncoder* const enc, Candidate* const candidates, int is_key_frame, EncodedFrame* const encoded_frame)
{
    int i;
    int best_idx = -1;
    size_t best_size = ~0;
                ...
                *dst = candidates[i].info_;
                ...
                if (!is_key_frame) {
                    ...
                    const WebPMuxAnimDispose prev_dispose_method
                        = (best_idx == LL_DISP_NONE || best_idx == LOSSY_DISP_NONE) ? WEBP_MUX_DISPOSE_NONE : WEBP_MUX_DISPOSE_BACKGROUND;
...
static WebPEncodingError SetFrame(
    WebPAnimEncoder* const enc, const WebPConfig* const config, int is_key_frame, EncodedFrame* const encoded_frame, int* const frame_skipped)
{
    int i;
    WebPEncodingError error_code = VP8_ENC_OK;
    ...
    *frame_skipped = 0;
    ...
    if (!SubFrameParamsInit(&dispose_none_params, 1, empty_rect_allowed_none) || !SubFrameParamsInit(&dispose_bg_params, 0, empty_rect_allowed_bg)) {
    ...
    if (!GetSubRects(prev_canvas, curr_canvas, is_key_frame, is_first_frame, config_lossy.quality, &dispose_none_params)) {
        error_code = VP8_ENC_ERROR_INVALID_CONFIGURATION;
        goto Err;
    }

    if ((consider_lossless && IsEmptyRect(&dispose_none_params.rect_ll_)) || (consider_lossy && IsEmptyRect(&dispose_none_params.rect_lossy_))) {
    ...
    if (dispose_bg_possible) {
        ...
        if (!GetSubRects(prev_canvas_disposed, curr_canvas, is_key_frame, is_first_frame, config_lossy.quality, &dispose_bg_params)) {
            error_code = VP8_ENC_ERROR_INVALID_CONFIGURATION;
            goto Err;
        }
        ...
        } else if ((is_lossless && RectArea(&dispose_bg_params.rect_ll_) < RectArea(&dispose_none_params.rect_ll_))
            || (!is_lossless && RectArea(&dispose_bg_params.rect_lossy_) < RectArea(&dispose_none_params.rect_lossy_))) {
    ...
    if (dispose_none_params.should_try_) {
        error_code = GenerateCandidates(enc, candidates, WEBP_MUX_DISPOSE_NONE, is_lossless, is_key_frame, &dispose_none_params, &config_ll, &config_lossy);
        if (error_code != VP8_ENC_OK)
            goto Err;
    }
    ...
    if (dispose_bg_params.should_try_) {
        ...
        error_code = GenerateCandidates(enc, candidates, WEBP_MUX_DISPOSE_BACKGROUND, is_lossless, is_key_frame, &dispose_bg_params, &config_ll, &config_lossy);
        if (error_code != VP8_ENC_OK)
            goto Err;
    }
...
static int CacheFrame(WebPAnimEncoder* const enc, const WebPConfig* const config)
{
    ...
    if (enc->is_first_frame_) { // Add this as a key-frame.
        ...
        if (enc->count_since_key_frame_ <= enc->options_.kmin) {
            ...
            if (!enc->prev_candidate_undecided_) {
                enc->prev_rect_ = encoded_frame->is_key_frame_ ? prev_rect_key : prev_rect_sub;
            }
        }
    }
...
static int DecodeFrameOntoCanvas(const WebPMuxFrameInfo* const frame, WebPPicture* const canvas)
{
    ...
    if (!WebPPictureView(canvas, frame->x_offset, frame->y_offset, config.input.width, config.input.height, &sub_image)) {
        return 0;
    }
...
int WebPAnimEncoderAssemble(WebPAnimEncoder* enc, WebPData* webp_data)
{
    ...
    if (!enc->got_null_frame_ && enc->in_frame_count_ > 1 && enc->count_ > 0) {
        ...
        const double delta_time = (uint32_t)enc->prev_timestamp_ - enc->first_timestamp_;
