...
int WebPValidatePicture(const WebPPicture* const picture)
{
    ...
    if (picture->width <= 0 || picture->width / 4 > INT_MAX / 4 || picture->height <= 0 || picture->height / 4 > INT_MAX / 4) {
        return WebPEncodingSetError(picture, VP8_ENC_ERROR_BAD_DIMENSION);
    }
...
int WebPPictureAllocARGB(WebPPicture* const picture)
{
    ...
    if (!WebPValidatePicture(picture))
        return 0;
...
static size_t Encode(const uint8_t* rgba, int width, int height, int stride, Importer import, float quality_factor, int lossless, uint8_t** output)
{
    WebPPicture pic;
    WebPConfig config;
