...
typedef double (*AccumulateFunc)(const uint8_t* src, int src_stride, const uint8_t* ref, int ref_stride, int w, int h);
...
static double AccumulateLSIM(const uint8_t* src, int src_stride, const uint8_t* ref, int ref_stride, int w, int h)
{
    int x, y;
    double total_sse = 0.;
    for (y = 0; y < h; ++y) {
...
static double AccumulateSSE(const uint8_t* src, int src_stride, const uint8_t* ref, int ref_stride, int w, int h)
{
    int y;
    double total_sse = 0.;
    for (y = 0; y < h; ++y) {
...
static double AccumulateSSIM(const uint8_t* src, int src_stride, const uint8_t* ref, int ref_stride, int w, int h)
{
...
{
    return (v > 0. && size > 0.) ? -4.3429448 * log(v / (size * 255 * 255.)) : kMinDistortion_dB;
}
...
int WebPPlaneDistortion(const uint8_t* src, size_t src_stride, const uint8_t* ref, size_t ref_stride, int width, int height, size_t x_step, int type,
    float* distortion, float* result)
{
    uint8_t* allocated = NULL;
    const AccumulateFunc metric = (type == 0) ? AccumulateSSE : (type == 1) ? AccumulateSSIM : AccumulateLSIM;
    if (src == NULL || ref == NULL || src_stride < x_step * width || ref_stride < x_step * width || result == NULL || distortion == NULL) {
        return 0;
    }
    ...
    if (x_step != 1) { // extract a packed plane if needed
        ...
        uint8_t* tmp1;
        uint8_t* tmp2;
        allocated = (uint8_t*)WebPSafeMalloc(2ULL * width * height, sizeof(*allocated));
        if (allocated == NULL)
            return 0;
        tmp1 = allocated;
...
int WebPPictureDistortion(const WebPPicture* src, const WebPPicture* ref, int type, float results[5])
{
    ...
    for (c = 0; c < 4; ++c) {
        ...
        if (!WebPPlaneDistortion(
                (const uint8_t*)p0.argb + offset, stride0, (const uint8_t*)p1.argb + offset, stride1, w, h, 4, type, &distortion, results + c)) {
            goto Error;
        }
    ...
    results[4] = (type == 1) ? (float)GetLogSSIM(total_distortion, total_size) : (float)GetPSNR(total_distortion, total_size);
    ok = 1;
