...
static int AdjustAndCheckRectangle(const WebPPicture* const pic, int* const left, int* const top, int width, int height)
{
...
int WebPPictureCopy(const WebPPicture* src, WebPPicture* dst)
{
    ...
    if (!src->use_argb) {
        ...
        WebPCopyPlane((const uint8_t*)src->argb, 4 * src->argb_stride, (uint8_t*)dst->argb, 4 * dst->argb_stride, 4 * dst->width, dst->height);
    }
    return 1;
}
#endif // !defined(WEBP_REDUCE_SIZE)
...
static int RescalePlane(const uint8_t* src, int src_width, int src_height, int src_stride, uint8_t* dst, int dst_width, int dst_height, int dst_stride,
    rescaler_t* const work, int num_channels)
{
...
int WebPPictureRescale(WebPPicture* picture, int width, int height)
{
    ...
    if (!picture->use_argb) {
        ...
        if (!RescalePlane(picture->y, prev_width, prev_height, picture->y_stride, tmp.y, width, height, tmp.y_stride, work, 1)
            ...
            || !RescalePlane(
                picture->v, HALVE(prev_width), HALVE(prev_height), picture->uv_stride, tmp.v, HALVE(width), HALVE(height), tmp.uv_stride, work, 1)) {
            return 0;
        }
        ...
        if (!RescalePlane((const uint8_t*)picture->argb, prev_width, prev_height, picture->argb_stride * 4, (uint8_t*)tmp.argb, width, height,
