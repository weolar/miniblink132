...
static int FinalizeSkipProba(VP8Encoder* const enc)
{
    ...
    if (proba->use_skip_proba_) {
        size += nb_events * VP8BitCost(1, proba->skip_proba_) + (nb_mbs - nb_events) * VP8BitCost(0, proba->skip_proba_);
...
static void SetSegmentProbas(VP8Encoder* const enc)
{
    ...
    if (enc->segment_hdr_.num_segments_ > 1) {
        ...
        if (!enc->segment_hdr_.update_map_)
            ResetSegments(enc);
        enc->segment_hdr_.size_ = p[0] * (VP8BitCost(0, probas[0]) + VP8BitCost(0, probas[1])) + p[1] * (VP8BitCost(0, probas[0]) + VP8BitCost(1, probas[1]))
            + p[2] * (VP8BitCost(1, probas[0]) + VP8BitCost(0, probas[2])) + p[3] * (VP8BitCost(1, probas[0]) + VP8BitCost(1, probas[2]));
    } else {
...
static int RecordTokens(VP8EncIterator* const it, const VP8ModeScore* const rd, VP8TBuffer* const tokens)
{
    ...
    if (it->mb_->type_ == 1) { // i16x16
        ...
        VP8InitResidual(0, 1, enc, &res);
        VP8SetResidualCoeffs(rd->y_dc_levels, &res);
        it->top_nz_[8] = it->left_nz_[8] = VP8RecordCoeffTokens(ctx, &res, tokens);
        VP8InitResidual(1, 0, enc, &res);
    } else {
        VP8InitResidual(0, 3, enc, &res);
    }
    ...
    for (y = 0; y < 4; ++y) {
        for (x = 0; x < 4; ++x) {
            const int ctx = it->top_nz_[x] + it->left_nz_[y];
            VP8SetResidualCoeffs(rd->y_ac_levels[x + y * 4], &res);
            it->top_nz_[x] = it->left_nz_[y] = VP8RecordCoeffTokens(ctx, &res, tokens);
        }
    }
    ...
    for (ch = 0; ch <= 2; ch += 2) {
        for (y = 0; y < 2; ++y) {
            for (x = 0; x < 2; ++x) {
                const int ctx = it->top_nz_[4 + ch + x] + it->left_nz_[4 + ch + y];
                VP8SetResidualCoeffs(rd->uv_levels[ch * 2 + x + y * 2], &res);
                it->top_nz_[4 + ch + x] = it->left_nz_[4 + ch + y] = VP8RecordCoeffTokens(ctx, &res, tokens);
            }
        }
    }
    VP8IteratorBytesToNz(it);
...
static void StoreSideInfo(const VP8EncIterator* const it)
{
    ...
    if (pic->extra_info != NULL) {
        if (it->x_ == 0 && it->y_ == 0) { // only do it once, at start
            memset(pic->extra_info, 0, enc->mb_w_ * enc->mb_h_ * sizeof(*pic->extra_info));
        }
    }
}
...
static uint64_t OneStatPass(VP8Encoder* const enc, VP8RDLevel rd_opt, int nb_mbs, int percent_delta, PassStats* const s)
{
    VP8EncIterator it;
    uint64_t size = 0;
    uint64_t size_p0 = 0;
...
static int StatLoop(VP8Encoder* const enc)
{
    ...
    while (num_pass_left-- > 0) {
        const int is_last_pass = (fabs(stats.dq) <= DQ_LIMIT) || (num_pass_left == 0) || (enc->max_i4_header_bits_ == 0);
...
int VP8EncTokenLoop(VP8Encoder* const enc)
{
    ...
    ok = ok && WebPReportProgress(enc->pic_, enc->percent_ + remaining_progress, &enc->percent_);
    return PostLoopFinalize(&it, ok);
}
