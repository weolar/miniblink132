...
static int RemapMemBuffer(MemBuffer* const mem, const uint8_t* data, size_t size)
{
...
static int InitMemBuffer(MemBuffer* const mem, const uint8_t* data, size_t size)
{
...
static void SetFrameInfo(size_t start_offset, size_t size, int frame_num, int complete, const WebPBitstreamFeatures* const features, Frame* const frame)
{
...
static ParseStatus StoreFrame(int frame_num, uint32_t min_size, MemBuffer* const mem, Frame* const frame)
{
    int alpha_chunks = 0;
    int image_chunks = 0;
    int done = (MemDataSize(mem) < CHUNK_HEADER_SIZE || MemDataSize(mem) < min_size);
    ...
    do {
        ...
        payload_available = (payload_size_padded > MemDataSize(mem)) ? MemDataSize(mem) : payload_size_padded;
        ...
        case MKFOURCC('V', 'P', '8', ' '):
            if (image_chunks == 0) {
                ...
                const VP8StatusCode vp8_status = WebPGetFeatures(mem->buf_ + chunk_start_offset, chunk_size, &features);
                if (status == PARSE_NEED_MORE_DATA && vp8_status == VP8_STATUS_NOT_ENOUGH_DATA) {
                    return PARSE_NEED_MORE_DATA;
...
static ParseStatus NewFrame(const MemBuffer* const mem, uint32_t min_size, uint32_t actual_size, Frame** frame)
{
...
static ParseStatus ParseAnimationFrame(WebPDemuxer* const dmux, uint32_t frame_chunk_size)
{
    ...
    bits = ReadByte(mem);
    frame->dispose_method_ = (bits & 1) ? WEBP_MUX_DISPOSE_BACKGROUND : WEBP_MUX_DISPOSE_NONE;
...
// Returns true on success, false otherwise.
static int StoreChunk(WebPDemuxer* const dmux, size_t start_offset, uint32_t size)
{
...
static int CheckFrameBounds(const Frame* const frame, int exact, int canvas_width, int canvas_height)
{
    if (exact) {
    ...
    while (f != NULL) {
        ...
        for (; f != NULL && f->frame_num_ == cur_frame_set; f = f->next_) {
            ...
            if (f->complete_) {
                ...
                // Ensure alpha precedes image bitstream.
                if (alpha->size_ > 0 && image->size_ > 0 && alpha->offset_ > image->offset_) {
                    return 0;
                }
            ...
            if (f->width_ > 0 && f->height_ > 0 && !CheckFrameBounds(f, !is_animation, dmux->canvas_width_, dmux->canvas_height_)) {
                return 0;
            }
        }
    }
...
static ParseStatus CreateRawImageDemuxer(MemBuffer* const mem, WebPDemuxer** demuxer)
{
    WebPBitstreamFeatures features;
    const VP8StatusCode status = WebPGetFeatures(mem->buf_, mem->buf_size_, &features);
    *demuxer = NULL;
    if (status != VP8_STATUS_OK) {
        return (status == VP8_STATUS_NOT_ENOUGH_DATA) ? PARSE_NEED_MORE_DATA : PARSE_ERROR;
    }

    {
        ...
        if (dmux == NULL || frame == NULL)
            goto Error;
        InitDemux(dmux, mem);
        SetFrameInfo(0, mem->buf_size_, 1 /*frame_num*/, 1 /*complete*/, &features, frame);
        if (!AddFrame(dmux, frame))
            goto Error;
...
WebPDemuxer* WebPDemuxInternal(const WebPData* data, int allow_partial, WebPDemuxState* state, int version)
{
    ...
    if (status != PARSE_OK) {
        ...
        if (state != NULL) {
            *state = (status == PARSE_NEED_MORE_DATA) ? WEBP_DEMUX_PARSING_HEADER : WEBP_DEMUX_PARSE_ERROR;
        }
        return NULL;
    }
...
static const uint8_t* GetFramePayload(const uint8_t* const mem_buf, const Frame* const frame, size_t* const data_size)
{
    *data_size = 0;
    if (frame != NULL) {
        ...
        *data_size = image->size_;
        ...
        // intervening chunks.
        if (alpha->size_ > 0) {
            const size_t inter_size = (image->offset_ > 0) ? image->offset_ - (alpha->offset_ + alpha->size_) : 0;
...
static int SynthesizeFrame(const WebPDemuxer* const dmux, const Frame* const frame, WebPIterator* const iter)
{
...
static const Chunk* GetChunk(const WebPDemuxer* const dmux, const char fourcc[4], int chunk_num)
{
    const uint8_t* const mem_buf = dmux->mem_.buf_;
    const Chunk* c;
    int count = 0;
...
static int SetChunk(const char fourcc[4], int chunk_num, WebPChunkIterator* const iter)
{
    const WebPDemuxer* const dmux = (WebPDemuxer*)iter->private_;
    int count;
...
{
    if (iter != NULL) {
        const char* const fourcc = (const char*)iter->chunk.bytes - CHUNK_HEADER_SIZE;
