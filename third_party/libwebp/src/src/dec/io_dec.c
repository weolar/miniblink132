...
static int InitYUVRescaler(const VP8Io* const io, WebPDecParams* const p)
{
    ...
    scalers = (WebPRescaler*)WEBP_ALIGN((const uint8_t*)work + total_size - rescaler_size);
...
static int EmitRescaledRGB(const VP8Io* const io, WebPDecParams* const p)
{
    ...
    int num_lines_out = 0;
    while (j < mb_h) {
        const int y_lines_in = WebPRescalerImport(p->scaler_y, mb_h - j, io->y + (size_t)j * io->y_stride, io->y_stride);
        j += y_lines_in;
        if (WebPRescaleNeededLines(p->scaler_u, uv_mb_h - uv_j)) {
            const int u_lines_in = WebPRescalerImport(p->scaler_u, uv_mb_h - uv_j, io->u + (size_t)uv_j * io->uv_stride, io->uv_stride);
            const int v_lines_in = WebPRescalerImport(p->scaler_v, uv_mb_h - uv_j, io->v + (size_t)uv_j * io->uv_stride, io->uv_stride);
...
static int CustomSetup(VP8Io* io)
{
    ...
    if (io->use_scaling) {
        ...
        if (is_alpha) { // need transparency output
            p->emit_alpha = (colorspace == MODE_RGBA_4444 || colorspace == MODE_rgbA_4444) ? EmitAlphaRGBA4444 : is_rgb ? EmitAlphaRGB : EmitAlphaYUV;
            if (is_rgb) {
                WebPInitAlphaProcessing();
            }
        }
