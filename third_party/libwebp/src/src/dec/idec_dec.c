            ...
            if (NeedCompressedAlpha(idec)) {
                ...
                if (alph_dec != NULL && alph_dec->vp8l_dec_ != NULL) {
                    if (alph_dec->method_ == ALPHA_LOSSLESS_COMPRESSION) {
                        ...
                        VP8LBitReaderSetBuffer(&alph_vp8l_dec->br_, dec->alpha_data_ + ALPHA_HEADER_LEN, dec->alpha_data_size_ - ALPHA_HEADER_LEN);
...
static int AppendToMemBuffer(WebPIDecoder* const idec, const uint8_t* const data, size_t data_size)
{
    ...
    const uint8_t* const old_start = (mem->buf_ == NULL) ? NULL : mem->buf_ + mem->start_;
...
static int RemapMemBuffer(WebPIDecoder* const idec, const uint8_t* const data, size_t data_size)
{
    ...
    const uint8_t* const old_start = (old_buf == NULL) ? NULL : old_buf + mem->start_;
...
static VP8StatusCode DecodeRemaining(WebPIDecoder* const idec)
{
    ...
    for (; dec->mb_y_ < dec->mb_h_; ++dec->mb_y_) {
        ...
        for (; dec->mb_x_ < dec->mb_w_; ++dec->mb_x_) {
            VP8BitReader* const token_br = &dec->parts_[dec->mb_y_ & dec->num_parts_minus_one_];
            MBContext context;
            SaveContext(dec, token_br, &context);
            if (!VP8DecodeMB(dec, token_br)) {
                // We shouldn't fail when MAX_MB data was available
                if (dec->num_parts_minus_one_ == 0 && MemDataSize(&idec->mem_) > MAX_MB_SIZE) {
                    return IDecError(idec, VP8_STATUS_BITSTREAM_ERROR);
                }
...
WebPIDecoder* WebPINewYUVA(uint8_t* luma, size_t luma_size, int luma_stride, uint8_t* u, size_t u_size, int u_stride, uint8_t* v, size_t v_size, int v_stride,
