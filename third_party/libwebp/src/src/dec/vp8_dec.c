    ...
    if (hdr->use_segment_) {
        ...
        if (VP8Get(br, "global-header")) { // update data
            ...
            for (s = 0; s < NUM_MB_SEGMENTS; ++s) {
                hdr->quantizer_[s] = VP8Get(br, "global-header") ? VP8GetSignedValue(br, 7, "global-header") : 0;
            }
            for (s = 0; s < NUM_MB_SEGMENTS; ++s) {
                hdr->filter_strength_[s] = VP8Get(br, "global-header") ? VP8GetSignedValue(br, 6, "global-header") : 0;
            }
        }
        if (hdr->update_map_) {
            int s;
            for (s = 0; s < MB_FEATURE_TREE_PROBS; ++s) {
                proba->segments_[s] = VP8Get(br, "global-header") ? VP8GetValue(br, 8, "global-header") : 255u;
            }
        }
    } else {
        hdr->update_map_ = 0;
...
static int GetCoeffsFast(VP8BitReader* const br, const VP8BandProbas* const prob[], int ctx, const quant_t dq, int n, int16_t* out)
{
...
static int GetCoeffsAlt(VP8BitReader* const br, const VP8BandProbas* const prob[], int ctx, const quant_t dq, int n, int16_t* out)
{
    const uint8_t* p = prob[n]->probas_[ctx];
    for (; n < 16; ++n) {
