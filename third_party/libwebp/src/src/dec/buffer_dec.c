...
static const uint8_t kModeBpp[MODE_LAST] = { 3, 4, 3, 4, 4, 2, 2, 4, 4, 4, 2, // pre-multiplied modes
    1, 1 };
...
static VP8StatusCode CheckDecBuffer(const WebPDecBuffer* const buffer)
{
    ...
    if (!IsValidColorspace(mode)) {
        ...
        const uint64_t size = MIN_BUFFER_SIZE(width * kModeBpp[mode], height, stride);
        ok &= (size <= buf->size);
...
{
    assert(output != NULL);
    return (output->is_external_memory >= 2) && WebPIsPremultipliedMode(output->colorspace) && (features != NULL && features->has_alpha);
}
