...
static int FramesAreEqual(const uint8_t* const rgba1, const uint8_t* const rgba2, int width, int height)
{
...
static WEBP_INLINE int PixelsAreSimilar(uint32_t src, uint32_t dst, int max_allowed_diff)
{
    ...
    return (abs(src_r * src_a - dst_r * dst_a) <= (max_allowed_diff * 255)) && (abs(src_g * src_a - dst_g * dst_a) <= (max_allowed_diff * 255))
        && (abs(src_b * src_a - dst_b * dst_a) <= (max_allowed_diff * 255)) && (abs(src_a - dst_a) <= max_allowed_diff);
}

static int FramesAreSimilar(const uint8_t* const rgba1, const uint8_t* const rgba2, int width, int height, int max_allowed_diff)
{
    int i, j;
...
static void MinimizeAnimationFrames(AnimatedImage* const img, int max_diff)
{
    ...
    for (i = 1; i < img->num_frames; ++i) {
        ...
        // skip merging.
        if (AdditionWillOverflow(frame1->duration, frame2->duration))
            continue;
        if (max_diff > 0) {
            should_merge_frames = FramesAreSimilar(rgba1, rgba2, img->canvas_width, img->canvas_height, max_diff);
        } else {
            should_merge_frames = FramesAreEqual(rgba1, rgba2, img->canvas_width, img->canvas_height);
        }
...
static int CompareAnimatedImagePair(const AnimatedImage* const img1, const AnimatedImage* const img2, int premultiply, double min_psnr)
{
    int ok = 1;
    ...
    if (!ok)
        return 0; // These are fatal failures, can't proceed.
    ...
    if (is_multi_frame_image) { // Checks relevant for multi-frame images only.
        ...
        if ((img1->format == ANIM_GIF && img1->loop_count == 65536 && img2->format == ANIM_WEBP && img2->loop_count == 65535)
            || (img1->format == ANIM_WEBP && img1->loop_count == 65535 && img2->format == ANIM_GIF && img2->loop_count == 65536)) {
            max_loop_count_workaround = 1;
        }
        ok = (max_loop_count_workaround || CompareValues(img1->loop_count, img2->loop_count, "Loop count mismatch")) && ok;
        ok = CompareBackgroundColor(img1->bgcolor, img2->bgcolor, premultiply) && ok;
    }
...
int main(int argc, const char* argv[])
{
    ...
    for (c = 1; c < argc; ++c) {
        ...
        if (!strcmp(argv[c], "-dump_frames")) {
            ...
            printf("WebP Decoder version: %d.%d.%d\nWebP Demux version: %d.%d.%d\n", (dec_version >> 16) & 0xff, (dec_version >> 8) & 0xff,
                (dec_version >> 0) & 0xff, (demux_version >> 16) & 0xff, (demux_version >> 8) & 0xff, (demux_version >> 0) & 0xff);
            FREE_WARGV_AND_RETURN(0);
        } else {
            if (!got_input1) {
    ...
    if (!CompareAnimatedImagePair(&images[0], &images[1], premultiply, min_psnr)) {
        WFPRINTF(stderr, "\nFiles %s and %s differ.\n", (const W_CHAR*)files[0], (const W_CHAR*)files[1]);
        return_code = -3;
    } else {
