...
static int AllocateFrames(AnimatedImage* const image, uint32_t num_frames)
{
    ...
    const uint64_t rgba_size = (uint64_t)image->canvas_width * kNumChannels * image->canvas_height;
    ...
    if (!CheckSizeForOverflow(total_size) || !CheckSizeForOverflow(total_frame_size)) {
        return 0;
    }
...
static void CopyFrameRectangle(const uint8_t* src, uint8_t* dst, int stride, int x_offset, int y_offset, int width, int height)
{
    int j;
...
static int DumpFrame(const char filename[], const char dump_folder[], uint32_t frame_num, const uint8_t rgba[], int canvas_width, int canvas_height)
{
    int ok = 0;
    size_t max_len;
    int y;
    ...
    if (dump_folder == NULL)
        dump_folder = (const char*)TO_W_CHAR(".");

    base_name = WSTRRCHR(filename, '/');
    base_name = (base_name == NULL) ? (const W_CHAR*)filename : base_name + 1;
    max_len = WSTRLEN(dump_folder) + 1 + WSTRLEN(base_name) + strlen("_frame_") + strlen(".pam") + 8;
    file_name = (W_CHAR*)WebPMalloc(max_len * sizeof(*file_name));
    if (file_name == NULL)
        goto End;

    if (WSNPRINTF(file_name, max_len, "%s/%s_frame_%d.pam", (const W_CHAR*)dump_folder, base_name, frame_num) < 0) {
...
static int ReadAnimatedWebP(const char filename[], const WebPData* const webp_data, AnimatedImage* const image, int dump_frames, const char dump_folder[])
{
    int ok = 0;
    int dump_ok = 1;
    ...
    while (WebPAnimDecoderHasMoreFrames(dec)) {
        ...
        if (dump_frames && dump_ok) {
            dump_ok = DumpFrame(filename, dump_folder, frame_index, curr_rgba, image->canvas_width, image->canvas_height);
...
static int DGifExtensionToGCB(const size_t GifExtensionLength, const GifByteType* GifExtension, GraphicsControlBlock* gcb)
{
...
static int IsKeyFrameGIF(const GifImageDesc* prev_desc, int prev_dispose, const DecodedFrame* const prev_frame, int canvas_width, int canvas_height)
{
    if (prev_frame == NULL)
        return 1;
...
static uint32_t GetBackgroundColorGIF(GifFileType* gif)
{
    ...
    if (transparent_index != NO_TRANSPARENT_COLOR && gif->SBackGroundColor == transparent_index) {
        ...
        return (0xffu << 24) | (color.Red << 16) | (color.Green << 8) | (color.Blue << 0);
    }
}
...
static uint32_t GetLoopCountGIF(const GifFileType* const gif)
{
    ...
    for (i = 0; i < gif->ImageCount; ++i) {
        ...
        for (j = 0; (j + 1) < image->ExtensionBlockCount; ++j) {
            ...
            const int signature_is_ok = (eb1->Function == APPLICATION_EXT_FUNC_CODE) && (eb1->ByteCount == 11)
                && (!memcmp(signature, "NETSCAPE2.0", 11) || !memcmp(signature, "ANIMEXTS1.0", 11));
            if (signature_is_ok && eb2->Function == CONTINUE_EXT_FUNC_CODE && eb2->ByteCount >= 3 && eb2->Bytes[0] == 1) {
                const uint32_t extra_loop = ((uint32_t)(eb2->Bytes[2]) << 8) + ((uint32_t)(eb2->Bytes[1]) << 0);
...
{
    return target->Left <= covered->Left && covered->Left + covered->Width <= target->Left + target->Width && target->Top <= covered->Top
        && covered->Top + covered->Height <= target->Top + target->Height;
}

static void RemapPixelsGIF(const uint8_t* const src, const ColorMapObject* const cmap, int transparent_color, int len, uint8_t* dst)
{
    int i;
    for (i = 0; i < len; ++i) {
...
static int ReadFrameGIF(const SavedImage* const gif_image, const ColorMapObject* cmap, int transparent_color, int out_stride, uint8_t* const dst)
{
...
static int ReadAnimatedGIF(const char filename[], AnimatedImage* const image, int dump_frames, const char dump_folder[])
{
    ...
    for (i = 0; i < frame_count; ++i) {
        ...
        if (curr_frame->duration <= 10)
            curr_frame->duration = 100;
        ...
        if (i == 0) { // Initialize as transparent.
            ...
            curr_frame->is_key_frame = IsKeyFrameGIF(prev_desc, prev_gcb.DisposalMode, prev_frame, canvas_width, canvas_height);
            ...
            if (curr_frame->is_key_frame) { // Initialize as transparent.
                ...
                prev_frame_disposed = (prev_gcb.DisposalMode == DISPOSE_BACKGROUND || prev_gcb.DisposalMode == DISPOSE_PREVIOUS);
                curr_frame_opaque = (curr_gcb.TransparentColor == NO_TRANSPARENT_COLOR);
                prev_frame_completely_covered = curr_frame_opaque && CoversFrameGIF(&curr_gif_image->ImageDesc, prev_desc);

                if (prev_frame_disposed && !prev_frame_completely_covered) {
                    switch (prev_gcb.DisposalMode) {
                    case DISPOSE_BACKGROUND: {
                        ZeroFillFrameRect(curr_rgba, canvas_width_in_bytes, prev_desc->Left, prev_desc->Top, prev_desc->Width, prev_desc->Height);
                        break;
                    }
                    case DISPOSE_PREVIOUS: {
                        ...
                        while (src_frame_num >= 0) {
                            ...
                            if (src_frame_gcb.DisposalMode != DISPOSE_PREVIOUS)
                                break;
                            --src_frame_num;
                        }
                        if (src_frame_num >= 0) {
                            ...
                            CopyFrameRectangle(
                                src_frame_rgba, curr_rgba, canvas_width_in_bytes, prev_desc->Left, prev_desc->Top, prev_desc->Width, prev_desc->Height);
                        } else {
        ...
        if (!ReadFrameGIF(curr_gif_image, gif->SColorMap, curr_gcb.TransparentColor, canvas_width_in_bytes, curr_rgba)) {
            DGifCloseFile(gif, NULL);
            return 0;
        }
...
int ReadAnimatedImage(const char filename[], AnimatedImage* const image, int dump_frames, const char dump_folder[])
{
    ...
    if (IsWebP(&webp_data)) {
        ...
        WFPRINTF(stderr, "Unknown file type: %s. Supported file types are WebP and GIF\n", (const W_CHAR*)filename);
        ok = 0;
    }
    if (!ok)
...
void GetDiffAndPSNR(const uint8_t rgba1[], const uint8_t rgba2[], uint32_t width, uint32_t height, int premultiply, int* const max_diff, double* const psnr)
{
