...
int main(int argc, const char* argv[])
{
    ...
    if (!WebPConfigInit(&config) || !WebPAnimEncoderOptionsInit(&enc_options) || !WebPPictureInit(&frame) || !WebPPictureInit(&curr_canvas)
    ...
    for (c = 1; c < argc; ++c) {
        ...
        if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
            ...
            printf("WebP Encoder version: %d.%d.%d\nWebP Mux version: %d.%d.%d\n", (enc_version >> 16) & 0xff, (enc_version >> 8) & 0xff, enc_version & 0xff,
                (mux_version >> 16) & 0xff, (mux_version >> 8) & 0xff, mux_version & 0xff);
            FREE_WARGV_AND_RETURN(0);
