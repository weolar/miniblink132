...
static const char* const kLosslessTransforms[4] = { "Predictor", "Cross Color", "Subtract Green", "Color Indexing" };

static const char* const kAlphaFilterMethods[4] = { "None", "Horizontal", "Vertical", "Gradient" };
...
static int GetBits(const uint8_t* const data, size_t data_size, size_t nb, int* val, uint64_t* const bit_pos)
{
    *val = 0;
    while (nb-- > 0) {
...
static int GetSignedBits(const uint8_t* const data, size_t data_size, size_t nb, int* val, uint64_t* const bit_pos)
{
    int sign;
    if (!GetBits(data, data_size, nb, val, bit_pos))
        return 0;
    if (!GetBits(data, data_size, 1, &sign, bit_pos))
        return 0;
    if (sign)
        *val = -(*val);
    return 1;
...
static WebPInfoStatus ParseLossySegmentHeader(const WebPInfo* const webp_info, const uint8_t* const data, size_t data_size, uint64_t* const bit_pos)
{
    int use_segment;
    ...
    if (use_segment) {
        ...
        if (update_data) {
            ...
            for (i = 0; i < 4; ++i) {
                ...
                GET_BITS(bit, 1);
                if (bit)
                    GET_SIGNED_BITS(quantizer[i], 7);
            }
            for (i = 0; i < 4; ++i) {
                int bit;
                GET_BITS(bit, 1);
                if (bit)
                    GET_SIGNED_BITS(filter_strength[i], 6);
            }
            printf("  Quantizer:        %d %d %d %d\n", quantizer[0], quantizer[1], quantizer[2], quantizer[3]);
            printf("  Filter strength:  %d %d %d %d\n", filter_strength[0], filter_strength[1], filter_strength[2], filter_strength[3]);
        }
        if (update_map) {
            int i;
...
static WebPInfoStatus ParseLossyFilterHeader(const WebPInfo* const webp_info, const uint8_t* const data, size_t data_size, uint64_t* const bit_pos)
{
...
static WebPInfoStatus ParseLossyHeader(const ChunkData* const chunk_data, const WebPInfo* const webp_info)
{
    ...
    printf("  Key frame:        %s\n"
           ...
           "  Display:          %s\n"
           "  Part. 0 length:   %d\n",
        key_frame ? "Yes" : "No", profile, display ? "Yes" : "No", partition0_length);
    if (key_frame) {
        ...
        printf("  Width:            %d\n"
               ...
               "  Height:           %d\n"
               "  Y scale:          %d\n",
            ((data[4] << 8) | data[3]) & 0x3fff, data[4] >> 6, ((data[6] << 8) | data[5]) & 0x3fff, data[6] >> 6);
        data += 7;
        data_size -= 7;
    } else {
    ...
    { // Partition number and size.
        ...
        for (i = 1; i < num_parts; ++i) {
            const size_t psize = part_size[0] | (part_size[1] << 8) | (part_size[2] << 16);
    ...
    // Quantizer.
    {
        ...
        GET_BITS(bit, 1);
        if (bit)
            GET_SIGNED_BITS(dq_y1_dc, 4);
        GET_BITS(bit, 1);
        if (bit)
            GET_SIGNED_BITS(dq_y2_dc, 4);
        GET_BITS(bit, 1);
        if (bit)
            GET_SIGNED_BITS(dq_y2_ac, 4);
        GET_BITS(bit, 1);
        if (bit)
            GET_SIGNED_BITS(dq_uv_dc, 4);
        GET_BITS(bit, 1);
        if (bit)
            GET_SIGNED_BITS(dq_uv_ac, 4);
        printf("  Base Q:           %d\n", base_q);
        printf("  DQ Y1 DC:         %d\n", dq_y1_dc);
        printf("  DQ Y2 DC:         %d\n", dq_y2_dc);
        printf("  DQ Y2 AC:         %d\n", dq_y2_ac);
...
static int LLGetBits(const uint8_t* const data, size_t data_size, size_t nb, int* val, uint64_t* const bit_pos)
{
    uint32_t i = 0;
    *val = 0;
    while (i < nb) {
...
static WebPInfoStatus ParseLosslessTransform(WebPInfo* const webp_info, const uint8_t* const data, size_t data_size, uint64_t* const bit_pos)
{
...
static WebPInfoStatus ParseLosslessHeader(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    const uint8_t* data = chunk_data->payload_;
    size_t data_size = chunk_data->size_ - CHUNK_HEADER_SIZE;
    uint64_t bit_position = 0;
...
static WebPInfoStatus ParseAlphaHeader(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    const uint8_t* data = chunk_data->payload_;
    size_t data_size = chunk_data->size_ - CHUNK_HEADER_SIZE;
    ...
    printf("  Parsing ALPH chunk...\n");
    {
        ...
        if (compression_method == ALPHA_LOSSLESS_COMPRESSION) {
            uint64_t bit_pos = 0;
            WebPInfoStatus status = ParseLosslessTransform(webp_info, data, data_size, &bit_pos);
            if (status != WEBP_INFO_OK)
                return status;
        }
    }
...
static WebPInfoStatus ParseRIFFHeader(WebPInfo* const webp_info, MemBuffer* const mem)
{
    ...
    if (memcmp(GetBuffer(mem), "RIFF", CHUNK_SIZE_BYTES) || memcmp(GetBuffer(mem) + CHUNK_HEADER_SIZE, "WEBP", CHUNK_SIZE_BYTES)) {
...
{
    const uint8_t* data = chunk_data->payload_;
    if (webp_info->chunk_counts_[CHUNK_VP8] || webp_info->chunk_counts_[CHUNK_VP8L] || webp_info->chunk_counts_[CHUNK_VP8X]) {
    ...
    if (!webp_info->quiet_) {
        printf("  ICCP: %d\n  Alpha: %d\n  EXIF: %d\n  XMP: %d\n  Animation: %d\n", (webp_info->feature_flags_ & ICCP_FLAG) != 0,
            (webp_info->feature_flags_ & ALPHA_FLAG) != 0, (webp_info->feature_flags_ & EXIF_FLAG) != 0, (webp_info->feature_flags_ & XMP_FLAG) != 0,
    ...
    if ((uint64_t)webp_info->canvas_width_ * webp_info->canvas_height_ > MAX_IMAGE_AREA) {
...
static WebPInfoStatus ProcessANIMChunk(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    ...
    if (!webp_info->quiet_) {
        printf("  Background color:(ARGB) %02x %02x %02x %02x\n", (webp_info->bgcolor_ >> 24) & 0xff, (webp_info->bgcolor_ >> 16) & 0xff,
            (webp_info->bgcolor_ >> 8) & 0xff, webp_info->bgcolor_ & 0xff);
...
static WebPInfoStatus ProcessANMFChunk(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    ...
    if ((uint64_t)offset_x + width > (uint64_t)webp_info->canvas_width_ || (uint64_t)offset_y + height > (uint64_t)webp_info->canvas_height_) {
...
static WebPInfoStatus ProcessImageChunk(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    ...
    if (!webp_info->quiet_) {
        ...
        printf("  Width: %d\n  Height: %d\n  Alpha: %d\n  Animation: %d\n"
               "  Format: %s (%d)\n",
            features.width, features.height, features.has_alpha, features.has_animation, kFormats[features.format], features.format);
    }
    if (webp_info->is_processing_anim_frame_) {
        ...
        if (webp_info->chunk_counts_[CHUNK_VP8X]) {
            if (webp_info->canvas_width_ != features.width || webp_info->canvas_height_ != features.height) {
            ...
            if (webp_info->canvas_width_ < 1 || webp_info->canvas_height_ < 1 || webp_info->canvas_width_ > MAX_CANVAS_SIZE
                || webp_info->canvas_height_ > MAX_CANVAS_SIZE || (uint64_t)webp_info->canvas_width_ * webp_info->canvas_height_ > MAX_IMAGE_AREA) {
    ...
    if (webp_info->parse_bitstream_) {
        ...
        const WebPInfoStatus status = is_lossy ? ParseLossyHeader(chunk_data, webp_info) : ParseLosslessHeader(chunk_data, webp_info);
        if (status != WEBP_INFO_OK)
            return status;
    }
    return WEBP_INFO_OK;
...
static WebPInfoStatus ProcessICCPChunk(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    ...
    if (webp_info->chunk_counts_[CHUNK_VP8] || webp_info->chunk_counts_[CHUNK_VP8L] || webp_info->chunk_counts_[CHUNK_ANIM]) {
...
static WebPInfoStatus ProcessChunk(const ChunkData* const chunk_data, WebPInfo* const webp_info)
{
    ...
    if (chunk_data->id_ == CHUNK_UNKNOWN) {
        ...
        snprintf(error_message, 50, "Unknown chunk at offset %6d, length %6d", (int)chunk_data->offset_, (int)chunk_data->size_);
        ...
        if (!webp_info->quiet_) {
            ...
            fourcc = (fourcc >> 24) | ((fourcc >> 8) & 0xff00) | ((fourcc << 8) & 0xff0000) | (fourcc << 24);
#endif
...
static void ShowSummary(const WebPInfo* const webp_info)
{
    ...
    for (i = 0; i < CHUNK_TYPES; ++i) {
        ...
        if (i == CHUNK_ANMF) {
            printf("%4d %4d %4d  ", webp_info->anmf_subchunk_counts_[0], webp_info->anmf_subchunk_counts_[1], webp_info->anmf_subchunk_counts_[2]);
        }
    }
    printf("\n");
}
...
int main(int argc, const char* argv[])
{
    ...
    for (c = 1; c < argc; ++c) {
        if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help") || !strcmp(argv[c], "-H") || !strcmp(argv[c], "-longhelp")) {
            Help();
            FREE_WARGV_AND_RETURN(WEBP_INFO_OK);
