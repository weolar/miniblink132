...
void GIFGetBackgroundColor(const ColorMapObject* const color_map, int bgcolor_index, int transparent_index, uint32_t* const bgcolor)
{
    if (transparent_index != GIF_INDEX_INVALID && bgcolor_index == transparent_index) {
        ...
        *bgcolor = (0xffu << 24) | (color.Red << 16) | (color.Green << 8) | (color.Blue << 0);
    }
}
...
static int Remap(const GifFileType* const gif, const uint8_t* const src, int len, int transparent_index, uint32_t* dst)
{
    int i;
...
int GIFReadFrame(GifFileType* const gif, int transparent_index, GIFFrameRect* const gif_rect, WebPPicture* const picture)
{
    ...
    uint32_t* dst = NULL;
    uint8_t* tmp = NULL;
    const GIFFrameRect rect = { image_desc->Left, image_desc->Top, image_desc->Width, image_desc->Height };
    ...
    if (image_desc->Interlace) { // Interlaced image.
        ...
        for (pass = 0; pass < 4; ++pass) {
            ...
            for (; y < rect.height; y += interlace_jumps[pass], row += jump) {
                if (DGifGetLine(gif, tmp, rect.width) == GIF_ERROR)
                    goto End;
                if (!Remap(gif, tmp, rect.width, transparent_index, row))
                    goto End;
            }
        }
        ...
        for (y = 0; y < rect.height; ++y, ptr += sub_image.argb_stride) {
            if (DGifGetLine(gif, tmp, rect.width) == GIF_ERROR)
                goto End;
            if (!Remap(gif, tmp, rect.width, transparent_index, ptr))
                goto End;
        }
    }
    ok = 1;
...
End:
    if (!ok)
        picture->error_code = sub_image.error_code;
...
void GIFDisposeFrame(GIFDisposeMethod dispose, const GIFFrameRect* const rect, const WebPPicture* const prev_canvas, WebPPicture* const curr_canvas)
{
    ...
    if (dispose == GIF_DISPOSE_BACKGROUND) {
        ...
        WebPCopyPlane((uint8_t*)src, (int)(4 * src_stride), (uint8_t*)dst, (int)(4 * dst_stride), 4 * rect->width, rect->height);
    }
}
...
void GIFGetBackgroundColor(const struct ColorMapObject* const color_map, int bgcolor_index, int transparent_index, uint32_t* const bgcolor)
{
    (void)color_map;
    (void)bgcolor_index;
...
int GIFReadGraphicsExtension(const GifByteType* const data, int* const duration, GIFDisposeMethod* const dispose, int* const transparent_index)
{
    (void)data;
    (void)duration;
    (void)dispose;
...
int GIFReadFrame(struct GifFileType* const gif, int transparent_index, GIFFrameRect* const gif_rect, struct WebPPicture* const picture)
{
    (void)gif;
    (void)transparent_index;
    (void)gif_rect;
...
void GIFDisposeFrame(
    GIFDisposeMethod dispose, const GIFFrameRect* const rect, const struct WebPPicture* const prev_canvas, struct WebPPicture* const curr_canvas)
{
    (void)dispose;
    (void)rect;
    (void)prev_canvas;
