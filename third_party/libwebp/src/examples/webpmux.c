...
typedef enum { NIL_ACTION = 0, ACTION_GET, ACTION_SET, ACTION_STRIP, ACTION_INFO, ACTION_HELP, ACTION_DURATION } ActionType;
...
static const char* const kFourccList[LAST_FEATURE] = { NULL, "EXIF", "XMP ", "ICCP", "ANMF" };

static const char* const kDescriptions[LAST_FEATURE] = { NULL, "EXIF metadata", "XMP metadata", "ICC profile", "Animation frame" };
...
static int CountOccurrences(const CommandLineArguments* const args, const char* const arg)
{
    int i;
...
static const char* const kErrorMessages[-WEBP_MUX_NOT_ENOUGH_DATA + 1]
    = { "WEBP_MUX_NOT_FOUND", "WEBP_MUX_INVALID_ARGUMENT", "WEBP_MUX_BAD_DATA", "WEBP_MUX_MEMORY_ERROR", "WEBP_MUX_NOT_ENOUGH_DATA" };
        ...
        if (nFrames > 0) {
            ...
            for (i = 1; i <= nFrames; i++) {
                ...
                if (err == WEBP_MUX_OK) {
                    ...
                    const VP8StatusCode status = WebPGetFeatures(frame.bitstream.bytes, frame.bitstream.size, &features);
                    assert(status == VP8_STATUS_OK); // Checked by WebPMuxCreate().
                    (void)status;
                    printf("%3d: %5d %5d %5s %8d %8d ", i, features.width, features.height, features.has_alpha ? "yes" : "no", frame.x_offset, frame.y_offset);
                    {
                        const char* const dispose = (frame.dispose_method == WEBP_MUX_DISPOSE_NONE) ? "none" : "background";
                        const char* const blend = (frame.blend_method == WEBP_MUX_BLEND) ? "yes" : "no";
                        printf("%8d %10s %5s ", frame.duration, dispose, blend);
                    }
                    printf("%10d %11s\n", (int)frame.bitstream.size, (features.format == 1) ? "lossy" : (features.format == 2) ? "lossless" : "undefined");
                }
...
static int CreateMux(const char* const filename, WebPMux** mux)
{
    WebPData bitstream;
    assert(mux != NULL);
    if (!ExUtilReadFileToWebPData(filename, &bitstream))
        return 0;
...
{
    int ok = 0;
    FILE* fout = WSTRCMP(filename, "-") ? WFOPEN(filename, "wb") : ImgIoUtilSetBinaryMode(stdout);
    if (fout == NULL) {
...
static WebPMux* DuplicateMuxHeader(const WebPMux* const mux)
{
    ...
    for (i = 1; i <= 3; ++i) {
        ...
        if (err == WEBP_MUX_OK && metadata.size > 0) {
            ...
            if (err != WEBP_MUX_OK) {
                ERROR_GOTO1("Error transferring metadata in DuplicateMuxHeader().", End);
            }
        }
    }
...
static int ParseFrameArgs(const char* args, WebPMuxFrameInfo* const info)
{
    ...
    const int num_args
        = sscanf(args, "+%d+%d+%d+%d%c%c+%d", &info->duration, &info->x_offset, &info->y_offset, &dispose_method, &plus_minus, &blend_method, &unused);
    switch (num_args) {
    case 1:
...
{
    uint32_t a, r, g, b;
    if (sscanf(args, "%u,%u,%u,%u", &a, &r, &g, &b) != 4)
        return 0;
    if (a >= 256 || r >= 256 || g >= 256 || b >= 256)
        return 0;
...
static int ValidateCommandLine(const CommandLineArguments* const cmd_args, int* num_feature_args)
{
    int num_frame_args;
    int num_loop_args;
...
static int ParseCommandLine(Config* config, const W_CHAR** const unicode_argv)
{
    ...
    const char* const* wargv = (unicode_argv != NULL) ? (const char**)(unicode_argv + 1) : argv;
        ...
        if (argv[i][0] == '-') { // One of the action types or output.
            ...
            if (!strcmp(argv[i], "icc") || !strcmp(argv[i], "exif") || !strcmp(argv[i], "xmp")) {
                if (FEATURETYPE_IS_NIL) {
                    config->type_ = (!strcmp(argv[i], "icc")) ? FEATURE_ICCP : (!strcmp(argv[i], "exif")) ? FEATURE_EXIF : FEATURE_XMP;
                } else {
                    ERROR_GOTO1("ERROR: Multiple features specified.\n", ErrParse);
                }
                ...
                ++feature_arg_index;
                i += 2;
            } else if (!strcmp(argv[i], "loop") && (config->action_type_ == ACTION_SET)) {
                CHECK_NUM_ARGS_AT_LEAST(2, ErrParse);
                config->type_ = FEATURE_LOOP;
                arg->params_ = argv[i + 1];
                ++feature_arg_index;
                i += 2;
            } else if (!strcmp(argv[i], "bgcolor") && (config->action_type_ == ACTION_SET)) {
                CHECK_NUM_ARGS_AT_LEAST(2, ErrParse);
...
static int InitializeConfig(int argc, const char* argv[], Config* const config, const W_CHAR** const unicode_argv)
{
...
static int Process(const Config* config)
{
    ...
    case ACTION_SET: {
        ...
        case FEATURE_ANMF: {
            ...
            for (i = 0; i < config->arg_count_; ++i) {
                ...
                case SUBTYPE_LOOP: {
                    int parse_error = 0;
                    const int loop_count = ExUtilGetInt(config->args_[i].params_, 10, &parse_error);
                ...
                case SUBTYPE_ANMF: {
                    ...
                    ok = ExUtilReadFileToWebPData(config->args_[i].filename_, &frame.bitstream);
                    if (!ok)
                        goto Err2;
        ...
        case FEATURE_LOOP: {
            WebPMuxAnimParams params = { 0xFFFFFFFF, 0 };
            int parse_error = 0;
            const int loop_count = ExUtilGetInt(config->args_[0].params_, 10, &parse_error);
    ...
    case ACTION_DURATION: {
        ...
        if (num_frames == 0) {
            ...
            for (i = 0; i < config->arg_count_; ++i) {
                ...
                const int nb_args = ExUtilGetInts(config->args_[i].params_, 10, 3, args);
                ok = (nb_args >= 1);
                if (!ok)
                    goto Err3;
                duration = args[0];
    ...
    case ACTION_STRIP: {
        ...
        if (!ok)
            goto Err2;
        if (config->type_ == FEATURE_ICCP || config->type_ == FEATURE_EXIF || config->type_ == FEATURE_XMP) {
