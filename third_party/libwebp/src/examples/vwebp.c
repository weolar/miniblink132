...
static void HandleDisplay(void)
{
    ...
    if (pic == NULL)
        return;
    glPushMatrix();
    glPixelZoom((GLfloat)(+1. / kParams.canvas_width * kParams.viewport_width), (GLfloat)(-1. / kParams.canvas_height * kParams.viewport_height));
...
int main(int argc, char* argv[])
{
    ...
    for (c = 1; c < argc; ++c) {
        ...
        if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
            ...
            printf("WebP Decoder version: %d.%d.%d\nWebP Demux version: %d.%d.%d\n", (dec_version >> 16) & 0xff, (dec_version >> 8) & 0xff, dec_version & 0xff,
                (dmux_version >> 16) & 0xff, (dmux_version >> 8) & 0xff, dmux_version & 0xff);
            FREE_WARGV_AND_RETURN(0);
