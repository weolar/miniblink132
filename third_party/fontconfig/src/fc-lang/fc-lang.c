...
FcPrivate void FcCacheObjectReference(void* object FC_UNUSED)
{
}

FcPrivate void FcCacheObjectDereference(void* object FC_UNUSED)
{
}

FcPrivate FcChar8* FcLangNormalize(const FcChar8* lang FC_UNUSED)
{
    return NULL;
}
...
static FcCharSet* scan(FILE* f, char* file, FcCharSetFreezer* freezer)
{
    ...
    while ((line = get_line(f, buf, &lineno))) {
        if (!strncmp(line, "include", 7)) {
            ...
            if (!included_file)
                fatal(file, lineno, "invalid syntax, expected: include filename");
...
int main(int argc FC_UNUSED, char** argv)
{
    ...
    printf("static const struct {\n"
           ...
           "} fcLangData = {\n",
        nsets, tl, tn, tn, nsets < 256 ? "8 " : "16", nsets, nsets < 256 ? "8 " : "16", nsets);
    ...
    for (i = 0; sets[i]; i++) {
        ...
        printf("    { \"%s\", "
            ...
            langs[i], sets[j]->num, i, off[j], i, off[j], i);
    }
    printf("},\n");
    ...
    printf("\n");
    for (setRangeChar = 'a'; setRangeChar <= 'z'; setRangeChar++) {
        printf("    { %d, %d }, /* %c */\n", setRangeStart[setRangeChar - 'a'], setRangeEnd[setRangeChar - 'a'], setRangeChar);
    }
    printf("};\n\n");
