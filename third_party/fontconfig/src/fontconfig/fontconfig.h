...
typedef enum _FcResult { FcResultMatch, FcResultNoMatch, FcResultTypeMismatch, FcResultNoId, FcResultOutOfMemory } FcResult;
...
typedef enum _FcMatchKind { FcMatchPattern, FcMatchFont, FcMatchScan, FcMatchKindEnd, FcMatchKindBegin = FcMatchPattern } FcMatchKind;

typedef enum _FcLangResult { FcLangEqual = 0, FcLangDifferentCountry = 1, FcLangDifferentTerritory = 1, FcLangDifferentLang = 2 } FcLangResult;

typedef enum _FcSetName { FcSetSystem = 0, FcSetApplication = 1 } FcSetName;
...
FcPublic void FcCacheCreateTagFile(FcConfig* config);

FcPublic FcBool FcDirCacheCreateUUID(FcChar8* dir, FcBool force, FcConfig* config);
...
FcPublic FcChar8* FcConfigGetFilename(FcConfig* config, const FcChar8* url);
...
FcPublic FcStrList* FcConfigGetCacheDirs(FcConfig* config);
...
FcPublic void FcConfigFileInfoIterInit(FcConfig* config, FcConfigFileInfoIter* iter);

FcPublic FcBool FcConfigFileInfoIterNext(FcConfig* config, FcConfigFileInfoIter* iter);

FcPublic FcBool FcConfigFileInfoIterGet(FcConfig* config, FcConfigFileInfoIter* iter, FcChar8** name, FcChar8** description, FcBool* enabled);
...
FcPublic FcChar32 FcCharSetFirstPage(const FcCharSet* a, FcChar32 map[FC_CHARSET_MAP_SIZE], FcChar32* next);

FcPublic FcChar32 FcCharSetNextPage(const FcCharSet* a, FcChar32 map[FC_CHARSET_MAP_SIZE], FcChar32* next);
...
FcPublic FcCache* FcDirCacheLoadFile(const FcChar8* cache_file, struct stat* file_stat);

FcPublic void FcDirCacheUnload(FcCache* cache);

/* fcfreetype.c */
FcPublic FcPattern* FcFreeTypeQuery(const FcChar8* file, unsigned int id, FcBlanks* blanks, int* count);

FcPublic unsigned int FcFreeTypeQueryAll(const FcChar8* file, unsigned int id, FcBlanks* blanks, int* count, FcFontSet* set);
...
FcPublic FcResult FcPatternGetWithBinding(const FcPattern* p, const char* object, int id, FcValue* v, FcValueBinding* b);
...
FcPublic FcBool FcPatternAddRange(FcPattern* p, const char* object, const FcRange* r);
...
FcPublic FcResult FcPatternGetRange(const FcPattern* p, const char* object, int id, FcRange** r);
...
FcPublic FcBool FcRangeGetDouble(const FcRange* range, double* begin, double* end);
...
FcPublic FcBool FcPatternIterEqual(const FcPattern* p1, FcPatternIter* i1, const FcPattern* p2, FcPatternIter* i2);

FcPublic FcBool FcPatternFindIter(const FcPattern* pat, FcPatternIter* iter, const char* object);

FcPublic FcBool FcPatternIterIsValid(const FcPattern* pat, FcPatternIter* iter);

FcPublic const char* FcPatternIterGetObject(const FcPattern* pat, FcPatternIter* iter);

FcPublic int FcPatternIterValueCount(const FcPattern* pat, FcPatternIter* iter);

FcPublic FcResult FcPatternIterGetValue(const FcPattern* pat, FcPatternIter* iter, int id, FcValue* v, FcValueBinding* b);
...
FcPublic FcBool FcUtf8Len(const FcChar8* string, int len, int* nchar, int* wchar);

#define FC_UTF8_MAX_LEN 6

FcPublic int FcUcs4ToUtf8(FcChar32 ucs4, FcChar8 dest[FC_UTF8_MAX_LEN]);

FcPublic int FcUtf16ToUcs4(const FcChar8* src_orig, FcEndian endian, FcChar32* dst, int len); /* in bytes */

FcPublic FcBool FcUtf16Len(const FcChar8* string, FcEndian endian, int len, /* in bytes */
    int* nchar, int* wchar);

FcPublic FcChar8* FcStrBuildFilename(const FcChar8* path, ...);
...
FcPublic FcBool FcConfigParseAndLoadFromMemory(FcConfig* config, const FcChar8* buffer, FcBool complain);
