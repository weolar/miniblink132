...
static FcChar8* FcDirCacheBasenameMD5(FcConfig* config, const FcChar8* dir, FcChar8 cache_base[CACHEBASE_LEN])
{
    ...
    if (FcDebug() & FC_DBG_CACHE) {
        printf("cache: %s (dir: %s%s%s%s%s%s)\n", cache_base, orig_dir ? orig_dir : dir, mapped_dir ? " (mapped to " : "", mapped_dir ? (char*)mapped_dir : "",
            mapped_dir ? ")" : "", salt ? ", salt: " : "", salt ? (char*)salt : "");
    }
...
#ifndef _WIN32
static FcChar8* FcDirCacheBasenameUUID(FcConfig* config, const FcChar8* dir, FcChar8 cache_base[CACHEBASE_LEN])
{
...
static FcBool FcDirCacheProcess(FcConfig* config, const FcChar8* dir,
    FcBool (*callback)(FcConfig* config, int fd, struct stat* fd_stat, struct stat* dir_stat, struct timeval* cache_mtime, void* closure), void* closure,
    FcChar8** cache_file_ret)
{
    int fd = -1;
    FcChar8 cache_base[CACHEBASE_LEN];
    FcStrList* list;
...
static FcCache* FcCacheFindByStat(struct stat* cache_stat)
{
    ...
    for (s = fcCacheChains[0]; s; s = s->next[0])
        if (s->cache_dev == cache_stat->st_dev && s->cache_ino == cache_stat->st_ino && s->cache_mtime == cache_stat->st_mtime) {
#ifdef HAVE_STRUCT_STAT_ST_MTIM
...
static FcBool FcCacheOffsetsValid(FcCache* cache)
{
    ...
    if (cache->dir < 0 || cache->dir > cache->size - sizeof(intptr_t) || memchr(base + cache->dir, '\0', cache->size - cache->dir) == NULL)
        return FcFalse;

    if (cache->dirs < 0 || cache->dirs >= cache->size || cache->dirs_count < 0 || cache->dirs_count > (cache->size - cache->dirs) / sizeof(intptr_t))
        return FcFalse;
    ...
    if (fs) {
        ...
        for (i = 0; i < fs->nfont; i++) {
            ...
            if ((char*)font < base || (char*)font > end - sizeof(FcFontSet) || font->elts_offset < 0 || font->elts_offset > end - (char*)font
                || font->num > (end - (char*)font - font->elts_offset) / sizeof(FcPatternElt) || !FcRefIsConst(&font->ref))
                return FcFalse;
            ...
            for (j = 0; j < font->num; j++) {
                ...
                for (l = FcPatternEltValues(&e[j]); l; l = FcValueListNext(l)) {
                    if ((char*)l < last_offset || (char*)l > end - sizeof(*l) || (l->next != NULL && !FcIsEncodedOffset(l->next)))
                        return FcFalse;
...
static FcCache* FcDirCacheMapFd(FcConfig* config, int fd, struct stat* fd_stat, struct stat* dir_stat)
{
    ...
    if (fd_stat->st_size > INTPTR_MAX || fd_stat->st_size < (int)sizeof(FcCache))
        return NULL;
    ...
    if (cache->magic != FC_CACHE_MAGIC_MMAP || cache->version < FC_CACHE_VERSION_NUMBER || cache->size != (intptr_t)fd_stat->st_size
        || !FcCacheOffsetsValid(cache) || !FcCacheTimeValid(config, cache, dir_stat) || !FcCacheInsert(cache, fd_stat)) {
        if (allocated)
            free(cache);
        else {
#if defined(HAVE_MMAP) || defined(__CYGWIN__)
...
FcCache* FcDirCacheLoad(const FcChar8* dir, FcConfig* config, FcChar8** cache_file)
{
    ...
    if (!config)
        return NULL;
    if (!FcDirCacheProcess(config, dir, FcDirCacheMapHelper, &cache, cache_file))
        cache = NULL;
...
static int FcDirChecksum(struct stat* statb)
{
    ...
    if (source_date_epoch) {
        ...
        if (endptr == source_date_epoch)
            ...
            fprintf(stderr, "Fontconfig: SOURCE_DATE_EPOCH: strtoull: %s: %llu\n", strerror(errno), epoch);
        else if (*endptr != '\0')
            fprintf(stderr, "Fontconfig: SOURCE_DATE_EPOCH has trailing garbage\n");
        else if (epoch > ULONG_MAX)
            fprintf(stderr, "Fontconfig: SOURCE_DATE_EPOCH must be <= %lu but saw: %llu\n", ULONG_MAX, epoch);
        else if (epoch < ret)
 ...
 */
static FcBool FcDirCacheValidateHelper(
    FcConfig* config, int fd, struct stat* fd_stat, struct stat* dir_stat, struct timeval* latest_cache_mtime, void* closure FC_UNUSED)
{
    FcBool ret = FcTrue;
    FcCache c;
...
FcBool FcDirCacheClean(const FcChar8* cache_dir, FcBool verbose)
{
    ...
    while ((ent = readdir(d))) {
        ...
        if (strlen(ent->d_name) != 32 + strlen("-" FC_ARCHITECTURE FC_CACHE_SUFFIX) || strcmp(ent->d_name + 32, "-" FC_ARCHITECTURE FC_CACHE_SUFFIX))
            continue;
...
void byteReverse(unsigned char* buf, unsigned longs)
{
    FcChar32 t;
    do {
        t = (FcChar32)((unsigned)buf[3] << 8 | buf[2]) << 16 | ((unsigned)buf[1] << 8 | buf[0]);
        *(FcChar32*)buf = t;
        buf += 4;
    } while (--longs);
}
...
FcBool FcDirCacheCreateTagFile(const FcChar8* cache_dir)
{
    ...
    FILE* fp;
    FcAtomic* atomic;
    static const FcChar8 cache_tag_contents[] = "Signature: 8a477f597d28d172789f06886806bc55\n"
