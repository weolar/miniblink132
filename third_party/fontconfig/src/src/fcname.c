...
FcBool FcObjectValidType(FcObject object, FcType type)
{
    ...
    if (t) {
        ...
        case FcTypeLangSet:
            ...
            break;
        case FcTypeRange:
            if (type == FcTypeRange || type == FcTypeDouble || type == FcTypeInteger)
                return FcTrue;
            break;
        default:
            if (type == t->type)
...
FcBool FcNameConstantWithObjectCheck(const FcChar8* string, const char* object, int* result)
{
    const FcConstant* c;
...
static FcValue FcNameConvert(FcType type, const char* object, FcChar8* string)
{
    FcValue v;
    FcMatrix m;
    double b, e;
    ...
    case FcTypeRange:
        if (sscanf((char*)string, "[%lg %lg]", &b, &e) != 2) {
            ...
            if (sc && ec && sscanf((char*)string, "[%s %[^]]]", sc, ec) == 2) {
                if (FcNameConstantWithObjectCheck((const FcChar8*)sc, object, &si) && FcNameConstantWithObjectCheck((const FcChar8*)ec, object, &ei))
...
static FcBool FcNameUnparseString(FcStrBuf* buf, const FcChar8* string, const FcChar8* escape)
{
    FcChar8 c;
    while ((c = *string++)) {
...
FcBool FcNameUnparseValue(FcStrBuf* buf, FcValue* v0, FcChar8* escape)
{
    ...
    case FcTypeBool:
        return FcNameUnparseString(buf, v.u.b == FcTrue ? (FcChar8*)"True" : v.u.b == FcFalse ? (FcChar8*)"False" : (FcChar8*)"DontCare", 0);
    case FcTypeMatrix:
        sprintf((char*)temp, "%g %g %g %g", v.u.m->xx, v.u.m->xy, v.u.m->yx, v.u.m->yy);
        return FcNameUnparseString(buf, temp, 0);
    case FcTypeCharSet:
