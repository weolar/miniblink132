...
static FcBool FcFormatContextInit(FcFormatContext* c, const FcChar8* format, FcChar8* scratch, int scratch_len)
{
...
static FcBool expect_char(FcFormatContext* c, FcChar8 term)
{
    ...
    if (!res) {
        if (c->format == c->format_orig + c->format_len)
            ...
            message("expected '%c' at %d", term, c->format - c->format_orig + 1);
    }
    return res;
}
...
static FcBool read_chars(FcFormatContext* c, FcChar8 term)
{
    ...
    *p = '\0';
    ...
    if (p == c->word) {
        message("expected character data at %d", c->format - c->format_orig + 1);
        return FcFalse;
    }
...
static FcBool FcPatternFormatToBuf(FcPattern* pat, const FcChar8* format, FcStrBuf* buf);

static FcBool interpret_builtin(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcChar8* new_str;
    FcBool ret;
...
#define BUILTIN(name, format) else if (0 == strcmp((const char*)c->word, name)) ret = FcPatternFormatToBuf(pat, (const FcChar8*)format, buf)
...
static FcBool interpret_expr(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf, FcChar8 term);

static FcBool interpret_subexpr(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    return expect_char(c, '{') && interpret_expr(c, pat, buf, '}') && expect_char(c, '}');
}

static FcBool maybe_interpret_subexpr(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
...
static FcBool interpret_filter_in(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcObjectSet* os;
    FcPattern* subpat;
...
static FcBool interpret_filter_out(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcPattern* subpat;
...
static FcBool interpret_cond(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    ...
    do {
        ...
        pass = pass && (negate ^ (FcResultMatch == FcPatternGet(pat, (const char*)c->word, 0, &v)));
    } while (consume_char(c, ','));
...
static FcBool interpret_enumerate(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcObjectSet* os;
    FcPattern* subpat;
    ...
    if (os->nobject == 1) {
        FcLangSet* langset;
        if (FcResultMatch == FcPatternGetLangSet(pat, os->objects[0], 0, &langset)) {
            FcStrSet* ss;
            if (!(ss = FcLangSetGetLangs(langset)) || !(lang_strs = FcStrListCreate(ss)))
                goto bail0;
        }
    }
...
static FcBool interpret_simple(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcPatternIter iter;
    ...
    if (consume_char(c, '[')) {
        ...
        if (idx < 0) {
            message("expected non-negative number at %d", c->format - 1 - c->format_orig + 1);
            return FcFalse;
        }
        if (!expect_char(c, ']'))
            return FcFalse;
...
static FcBool cescape(FcFormatContext* c FC_UNUSED, const FcChar8* str, FcStrBuf* buf)
{
    /* XXX escape \n etc? */
...
static FcBool shescape(FcFormatContext* c FC_UNUSED, const FcChar8* str, FcStrBuf* buf)
{
    FcStrBufChar(buf, '\'');
    while (*str) {
        if (*str == '\'')
...
static FcBool xmlescape(FcFormatContext* c FC_UNUSED, const FcChar8* str, FcStrBuf* buf)
{
    /* XXX escape \n etc? */
...
static FcBool delete_chars(FcFormatContext* c, const FcChar8* str, FcStrBuf* buf)
{
    /* XXX not UTF-8 aware */
...
static FcBool escape_chars(FcFormatContext* c, const FcChar8* str, FcStrBuf* buf)
{
    /* XXX not UTF-8 aware */
...
static FcBool translate_chars(FcFormatContext* c, const FcChar8* str, FcStrBuf* buf)
{
    char *from, *to, repeat;
    int from_len, to_len;
...
#define CONVERTER(name, func) else if (0 == strcmp((const char*)c->word, name)) ret = func(c, str, &new_buf)
...
static FcBool maybe_interpret_converts(FcFormatContext* c, FcStrBuf* buf, int start)
{
    while (*c->format == '|')
...
static FcBool interpret_percent(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    int width, start;
    FcBool ret;
    ...
    return ret && maybe_interpret_converts(c, buf, start) && align_to_width(buf, start, width) && expect_char(c, '}');
}
