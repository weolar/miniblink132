...
static FcChar8* FcStrMakeTriple(const FcChar8* s1, const FcChar8* s2, const FcChar8* s3)
{
...
#define FcCaseFoldUpperCount(cf) ((cf)->method == FC_CASE_FOLD_FULL ? 1 : (cf)->count)
...
int FcStrCmpIgnoreCaseAndDelims(const FcChar8* s1, const FcChar8* s2, const FcChar8* delims)
{
    FcCaseWalker w1, w2;
    FcChar8 c1, c2;
...
const FcChar8* FcStrContainsIgnoreBlanksAndCase(const FcChar8* s1, const FcChar8* s2)
{
    while (*s1) {
...
const FcChar8* FcStrContainsWord(const FcChar8* s1, const FcChar8* s2)
{
    ...
    while (s1len >= s2len) {
        if (wordStart && FcStrIsAtIgnoreCase(s1, s2) && (s1len == s2len || FcCharIsPunct(s1[s2len]))) {
            return s1;
        }
        wordStart = FcFalse;
        if (FcCharIsPunct(*s1))
...
int FcStrMatchIgnoreCaseAndDelims(const FcChar8* s1, const FcChar8* s2, const FcChar8* delims)
{
    FcCaseWalker w1, w2;
    FcChar8 c1, c2;
...
#define GetUtf16(src, endian) ((FcChar16)((src)[endian == FcEndianBig ? 0 : 1] << 8) | (FcChar16)((src)[endian == FcEndianBig ? 1 : 0]))
...
FcBool FcUtf16Len(const FcChar8* string, FcEndian endian, int len, /* in bytes */
    int* nchar, int* wchar)
{
    int n;
    int clen;
...
static int fc_strcmp_r(const FcChar8* s1, const FcChar8* s2, const FcChar8** ret)
{
    FcChar8 c1, c2;
...
FcBool FcStrSetMemberAB(FcStrSet* set, const FcChar8* a, FcChar8* b, FcChar8** ret)
{
    int i;
...
FcBool FcStrSetAddTriple(FcStrSet* set, const FcChar8* a, const FcChar8* b, const FcChar8* c)
{
...
const FcChar8* FcStrTripleSecond(FcChar8* str)
{
...
FcBool FcStrSetAddFilenamePairWithSalt(FcStrSet* set, const FcChar8* a, const FcChar8* b, const FcChar8* salt)
{
    FcChar8* new_a = NULL;
    FcChar8* new_b = NULL;
    FcChar8* rs = NULL;
...
FcBool FcStrSetDel(FcStrSet* set, const FcChar8* s)
{
    ...
    for (i = 0; i < set->num; i++)
	     ...
	     * NULL
	     */
            memmove(&set->strs[i], &set->strs[i + 1], (set->num - i) * sizeof(FcChar8*));
            set->num--;
            return FcTrue;
        }
    return FcFalse;
