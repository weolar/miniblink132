...
FcBool FcObjectSetAdd(FcObjectSet* os, const char* object)
{
    ...
    if (os->nobject == os->sobject) {
        s = os->sobject + 4;
        if (os->objects)
            objects = (const char**)realloc((void*)os->objects, s * sizeof(const char*));
        else
    ...
    if (c < 0)
        mid++;
    memmove(os->objects + mid + 1, os->objects + mid, (os->nobject - mid) * sizeof(const char*));
    ...
    FcValueListPtr fntOrig) /* font */
{
    ...
    for (pat = patOrig; pat != NULL; pat = FcValueListNext(pat)) {
	     ...
	     */
            if (FcConfigCompareValue(&fnt->value, FC_OP(FcOpListing, FcOpFlagIgnoreBlanks), &pat->value))
                break;
        }
        if (fnt == NULL)
            return FcFalse;
...
static FcBool FcListValueListEqual(FcValueListPtr v1orig, FcValueListPtr v2orig)
{
    ...
    for (v1 = v1orig; v1 != NULL; v1 = FcValueListNext(v1)) {
        for (v2 = v2orig; v2 != NULL; v2 = FcValueListNext(v2))
            if (FcValueEqual(FcValueCanonicalize(&(v1)->value), FcValueCanonicalize(&(v2)->value)))
                break;
        if (v2 == NULL)
            return FcFalse;
    }
...
static FcBool FcListPatternEqual(FcPattern* p1, FcPattern* p2, FcObjectSet* os)
{
    ...
    for (i = 0; i < os->nobject; i++) {
        ...
        if (!e1 || !e2)
            return FcFalse;
        if (!FcListValueListEqual(FcPatternEltValues(e1), FcPatternEltValues(e2)))
            return FcFalse;
    }
    return FcTrue;
}
...
{
    int xx = (int)(m->xx * 100), xy = (int)(m->xy * 100), yx = (int)(m->yx * 100), yy = (int)(m->yy * 100);
...
static int FcGetDefaultObjectLangIndex(FcPattern* font, FcObject object, const FcChar8* lang)
{
...
static FcBool FcListAppend(FcListHashTable* table, FcPattern* font, FcObjectSet* os, const FcChar8* lang)
{
    int o;
    FcPatternElt* e;
    FcValueListPtr v;
    ...
    for (prev = &table->buckets[hash % FC_LIST_HASH_SIZE]; (bucket = *prev); prev = &(bucket->next)) {
        if (bucket->hash == hash && FcListPatternEqual(bucket->pattern, font, os))
            return FcTrue;
    }
    ...
    for (o = 0; o < os->nobject; o++) {
        ...
        if (e) {
            for (v = FcPatternEltValues(e), idx = 0; v; v = FcValueListNext(v), ++idx) {
                if (!FcPatternAdd(bucket->pattern, os->objects[o], FcValueCanonicalize(&v->value), defidx != idx))
                    goto bail2;
            }
        }
    }
