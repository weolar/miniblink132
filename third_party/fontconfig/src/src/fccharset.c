...
static FcBool FcCharSetPutLeaf(FcCharSet* fcs, FcChar32 ucs4, FcCharLeaf* leaf, int pos)
{
    ...
    memmove(numbers + pos + 1, numbers + pos, (fcs->num - pos) * sizeof(*numbers));
...
FcBool FcCharSetEqual(const FcCharSet* a, const FcCharSet* b)
{
    ...
    if (!a || !b)
        return FcFalse;
    for (FcCharSetIterStart(a, &ai), FcCharSetIterStart(b, &bi); ai.leaf && bi.leaf; FcCharSetIterNext(a, &ai), FcCharSetIterNext(b, &bi)) {
        if (ai.ucs4 != bi.ucs4)
            return FcFalse;
        for (i = 0; i < 256 / 32; i++)
...
static FcCharSet* FcCharSetOperate(
    const FcCharSet* a, const FcCharSet* b, FcBool (*overlap)(FcCharLeaf* result, const FcCharLeaf* al, const FcCharLeaf* bl), FcBool aonly, FcBool bonly)
{
    FcCharSet* fcs;
    FcCharSetIter ai, bi;
...
static FcBool FcCharSetIntersectLeaf(FcCharLeaf* result, const FcCharLeaf* al, const FcCharLeaf* bl)
{
    int i;
...
static FcBool FcCharSetUnionLeaf(FcCharLeaf* result, const FcCharLeaf* al, const FcCharLeaf* bl)
{
    int i;
...
static FcBool FcCharSetSubtractLeaf(FcCharLeaf* result, const FcCharLeaf* al, const FcCharLeaf* bl)
{
    int i;
    FcBool nonempty = FcFalse;
...
static FcBool FcNameParseRange(FcChar8** string, FcChar32* pfirst, FcChar32* plast)
{
    char* s = (char*)*string;
    char* t;
    long first, last;
...
static FcCharLeaf* FcCharSetFreezeLeaf(FcCharSetFreezer* freezer, FcCharLeaf* leaf)
{
...
static FcBool FcCharSetFreezeOrig(FcCharSetFreezer* freezer, const FcCharSet* orig, const FcCharSet* frozen)
{
...
static FcCharSet* FcCharSetFreezeBase(FcCharSetFreezer* freezer, FcCharSet* fcs)
{
    ...
    for (ent = *bucket; ent; ent = ent->next) {
        if (ent->hash == hash && ent->set.num == fcs->num && !memcmp(FcCharSetNumbers(&ent->set), FcCharSetNumbers(fcs), fcs->num * sizeof(FcChar16))) {
            FcBool ok = FcTrue;
            int i;
    ...
    size = (sizeof(FcCharSetEnt) + fcs->num * sizeof(FcCharLeaf*) + fcs->num * sizeof(FcChar16));
    ent = malloc(size);
    if (!ent)
        return 0;
    ...
    if (fcs->num) {
        ...
        ent->set.numbers_offset = (ent->set.leaves_offset + fcs->num * sizeof(intptr_t));

        ent_leaves = FcCharSetLeaves(&ent->set);
        for (i = 0; i < fcs->num; i++)
            ent_leaves[i] = FcPtrToOffset(ent_leaves, FcCharSetLeaf(fcs, i));
        memcpy(FcCharSetNumbers(&ent->set), FcCharSetNumbers(fcs), fcs->num * sizeof(FcChar16));
    } else {
...
static const FcCharSet* FcCharSetFindFrozen(FcCharSetFreezer* freezer, const FcCharSet* orig)
{
    FcCharSetOrigEnt** bucket = &freezer->orig_hash_table[((uintptr_t)orig) % FC_CHAR_SET_HASH_SIZE];
    FcCharSetOrigEnt* ent;
...
void FcCharSetFreezerDestroy(FcCharSetFreezer* freezer)
{
    ...
    if (FcDebug() & FC_DBG_CACHE) {
        printf("\ncharsets %d -> %d leaves %d -> %d\n", freezer->charsets_seen, freezer->charsets_allocated, freezer->leaves_seen, freezer->leaves_allocated);
    }
...
FcBool FcCharSetSerializeAlloc(FcSerialize* serialize, const FcCharSet* cs)
{
    ...
    if (!FcSerializeAlloc(serialize, numbers, cs->num * sizeof(FcChar16)))
        return FcFalse;
    for (i = 0; i < cs->num; i++)
        if (!FcSerializeAlloc(serialize, FcCharSetLeaf(cs, i), sizeof(FcCharLeaf)))
            return FcFalse;
    return FcTrue;
}
...
FcCharSet* FcCharSetSerialize(FcSerialize* serialize, const FcCharSet* cs)
{
    ...
    if (cs->num) {
        ...
        cs_serialized->leaves_offset = FcPtrToOffset(cs_serialized, leaves_serialized);
        ...
        cs_serialized->numbers_offset = FcPtrToOffset(cs_serialized, numbers_serialized);
        ...
        for (i = 0; i < cs->num; i++) {
            ...
            *leaf_serialized = *leaf;
            leaves_serialized[i] = FcPtrToOffset(leaves_serialized, leaf_serialized);
