...
typedef enum _FcConfigSeverity { FcSevereInfo, FcSevereWarning, FcSevereError } FcConfigSeverity;

static void FcConfigMessage(FcConfigParse* parse, FcConfigSeverity severe, const char* fmt, ...)
{
    ...
    if (parse) {
        if (parse->name)
            fprintf(stderr, "Fontconfig %s: \"%s\", line %d: ", s, parse->name, (int)XML_GetCurrentLineNumber(parse->parser));
        else
            fprintf(stderr, "Fontconfig %s: line %d: ", s, (int)XML_GetCurrentLineNumber(parse->parser));
...
static void FcTypecheckValue(FcConfigParse* parse, FcType value, FcType type)
{
    ...
    if (type == FcTypeInteger)
        type = FcTypeDouble;
    if (value != type) {
        if ((value == FcTypeLangSet && type == FcTypeString) || (value == FcTypeString && type == FcTypeLangSet)
...
static void FcTypecheckExpr(FcConfigParse* parse, FcExpr* expr, FcType type)
{
    ...
    case FcOpConst:
        ...
        if (c) {
            ...
            FcConfigMessage(parse, FcSevereWarning, "invalid constant used : %s", expr->u.constant);
        break;
    case FcOpQuest:
...
static FcTest* FcTestCreate(FcConfigParse* parse, FcMatchKind kind, FcQual qual, const FcChar8* field, unsigned int compare, FcExpr* expr)
{
...
static FcEdit* FcEditCreate(FcConfigParse* parse, FcObject object, FcOp op, FcExpr* expr, FcValueBinding binding)
{
...
static FcBool FcVStackPushString(FcConfigParse* parse, FcVStackTag tag, FcChar8* string)
{
...
static FcBool FcVStackPushExpr(FcConfigParse* parse, FcVStackTag tag, FcExpr* expr)
{
    FcVStack* vstack = FcVStackCreateAndPush(parse);
    if (!vstack)
        return FcFalse;
...
static FcChar8** FcConfigSaveAttr(const XML_Char** attr, FcChar8** buf, int size_bytes)
{
    int slen;
    int i;
    FcChar8** new;
...
static FcBool FcPStackPush(FcConfigParse* parse, FcElement element, const XML_Char** attr)
{
    FcPStack* new;
...
static const FcChar8* FcConfigGetAttribute(FcConfigParse* parse, const char* attr)
{
    FcChar8** attrs;
    if (!parse->pstack)
        return 0;
...
static FcChar8* _get_real_path_from_prefix(FcConfigParse* parse, const FcChar8* path, const FcChar8* prefix)
{
#ifdef _WIN32
    ...
    if (prefix) {
        ...
        } else if (FcStrCmp(prefix, (const FcChar8*)"default") == 0 || FcStrCmp(prefix, (const FcChar8*)"cwd") == 0) {
            /* Nothing to do */
    ...
    /* For Win32, check this later for dealing with special cases */
        ...
        if (!FcStrIsAbsoluteFilename(path) && path[0] != '~')
            FcConfigMessage(parse, FcSevereWarning, "Use of ambiguous path in <%s> element. please add prefix=\"cwd\" if current behavior is desired.",
                FcElementReverseMap(parse->pstack->element));
    }
#else
...
static double FcStrtod(char* s, char** end)
{
    ...
    if ((dot = strchr(s, 0x2e)) && (decimal_point[0] != 0x2e || decimal_point[1] != 0)) {
        char buf[128];
        int slen = strlen(s);
...
static FcBool FcConfigLexBool(FcConfigParse* parse, const FcChar8* bool_)
{
    ...
    if (!FcNameBool(bool_, &result))
        FcConfigMessage(parse, FcSevereWarning, "\"%s\" is not known boolean", bool_);
    return result;
}
...
static FcBool FcConfigLexBinding(FcConfigParse* parse, const FcChar8* binding_string, FcValueBinding* binding_ret)
{
...
static void FcParseAlias(FcConfigParse* parse)
{
    ...
    if (!prefer && !accept && !def) {
        ...
        FcTest* t = FcTestCreate(parse, FcMatchPattern, FcQualAny, (FcChar8*)FC_FAMILY, FC_OP(FcOpEqual, FcOpFlagIgnoreBlanks), family);
        if (rule) {
    ...
    if (prefer) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpPrepend, prefer, binding);
        if (!edit)
            FcExprDestroy(prefer);
        else {
    ...
    if (accept) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppend, accept, binding);
        if (!edit)
            FcExprDestroy(accept);
        else {
            r->next = FcRuleCreate(FcRuleEdit, edit);
    ...
    if (def) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppendLast, def, binding);
        if (!edit)
            FcExprDestroy(def);
        else {
            r->next = FcRuleCreate(FcRuleEdit, edit);
...
static void FcParseRemapDir(FcConfigParse* parse)
{
...
static void FcParseResetDirs(FcConfigParse* parse)
{
    if (!parse->scanOnly) {
...
static void FcParseInclude(FcConfigParse* parse)
{
    ...
    if (!_FcConfigParse(parse->config, s, !ignore_missing, !parse->scanOnly))
        ...
        if (deprecated == FcTrue && filename != NULL && userdir != NULL && !FcFileIsLink(filename)) {
            if (FcFileIsDir(filename)) {
                ...
                FcStrFree(parent);
                if (FcFileIsDir(userdir) || rename((const char*)filename, (const char*)userdir) != 0
                ...
                if (!FcFileIsDir(parent))
                    FcMakeDirectory(parent);
                FcStrFree(parent);
                if (FcFileIsFile(userconf) || rename((const char*)filename, (const char*)userconf) != 0
...
static const FcOpMap fcCompareOps[] = { { "eq", FcOpEqual }, { "not_eq", FcOpNotEqual }, { "less", FcOpLess }, { "less_eq", FcOpLessEqual },
    { "more", FcOpMore }, { "more_eq", FcOpMoreEqual }, { "contains", FcOpContains }, { "not_contains", FcOpNotContains } };
...
static void FcParseTest(FcConfigParse* parse)
{
    ...
    if (iblanks_string) {
        ...
        if (!FcNameBool(iblanks_string, &f)) {
            FcConfigMessage(parse, FcSevereWarning, "invalid test ignore-blanks \"%s\"", iblanks_string);
        }
        if (f)
...
static void FcParseEdit(FcConfigParse* parse)
{
    ...
    edit = FcEditCreate(parse, FcObjectFromName((char*)name), mode, expr, binding);
    if (!edit) {
        FcConfigMessage(parse, FcSevereError, "out of memory");
        FcExprDestroy(expr);
        return;
    ...
    while ((vstack = FcVStackPeek(parse))) {
        ...
        case FcVStackEdit:
            if (kind == FcMatchScan && vstack->u.edit->object > FC_MAX_BASE_OBJECT) {
                FcConfigMessage(parse, FcSevereError, "<match target=\"scan\"> cannot edit user-defined object \"%s\"", FcObjectName(vstack->u.edit->object));
                if (rule)
                    FcRuleDestroy(rule);
                return;
            }
...
static void FcParseAcceptRejectFont(FcConfigParse* parse, FcElement element)
{
    ...
    while ((vstack = FcVStackPeek(parse))) {
        switch ((int)vstack->tag) {
        case FcVStackGlob:
            if (!parse->scanOnly && !FcConfigGlobAdd(parse->config, vstack->u.string, element == FcElementAcceptfont)) {
                FcConfigMessage(parse, FcSevereError, "out of memory");
            } else {
            ...
            break;
        case FcVStackPattern:
            if (!parse->scanOnly && !FcConfigPatternsAdd(parse->config, vstack->u.pattern, element == FcElementAcceptfont)) {
                FcConfigMessage(parse, FcSevereError, "out of memory");
            } else {
...
static FcValue FcPopValue(FcConfigParse* parse)
{
    ...
    case FcVStackRange:
        ...
        break;
    default:
        FcConfigMessage(parse, FcSevereWarning, "unknown pattern element %d", vstack->tag);
        break;
    }
    FcVStackPopAndDestroy(parse);
...
static void FcStartDoctypeDecl(
    void* userData, const XML_Char* doctypeName, const XML_Char* sysid FC_UNUSED, const XML_Char* pubid FC_UNUSED, int has_internal_subset FC_UNUSED)
{
    FcConfigParse* parse = userData;
...
static void FcInternalSubsetDecl(void* userData, const XML_Char* doctypeName, const XML_Char* sysid, const XML_Char* pubid)
{
    FcStartDoctypeDecl(userData, doctypeName, sysid, pubid, 1);
}

static void FcExternalSubsetDecl(void* userData, const XML_Char* doctypeName, const XML_Char* sysid, const XML_Char* pubid)
{
...
static FcBool FcConfigParseAndLoadDir(FcConfig* config, const FcChar8* name, const FcChar8* dir, FcBool complain, FcBool load)
{
    ...
    if (!d) {
        if (complain)
            FcConfigMessage(0, FcSevereError, "Cannot open config dir \"%s\"", name);
        ret = FcFalse;
        goto bail0;
    }
    /* freed below */
...
#define TAIL_LEN 5
	 ...
	 */
        ...
        if ('0' <= e->d_name[0] && e->d_name[0] <= '9' && d_len > TAIL_LEN && strcmp(e->d_name + d_len - TAIL_LEN, TAIL) == 0) {
    ...
    if (ret) {
        int i;
        qsort(files->strs, files->num, sizeof(FcChar8*), (int (*)(const void*, const void*))FcSortCmpStr);
...
static FcBool FcConfigParseAndLoadFromMemoryInternal(FcConfig* config, const FcChar8* filename, const FcChar8* buffer, FcBool complain, FcBool load)
{
...
#endif
        {
            FcConfigMessage(&parse, FcSevereError, "%s", XML_ErrorString(XML_GetErrorCode(p)));
            goto bail3;
        }
#ifndef ENABLE_LIBXML2
    } while (buflen != 0);
...
FcBool FcConfigParseAndLoad(FcConfig* config, const FcChar8* name, FcBool complain)
{
