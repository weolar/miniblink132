	 ...
	 */
        ...
        if (FcDebug() & FC_DBG_SCANV) {
            if (missing && missing < 10) {
                FcCharSet* missed = FcCharSetSubtract(&fcLangCharSets[i].charset, charset);
                FcChar32 ucs4;
                ...
                printf("{");
                for (ucs4 = FcCharSetFirstPage(missed, map, &next); ucs4 != FC_CHARSET_DONE; ucs4 = FcCharSetNextPage(missed, map, &next)) {
                    int i, j;
...
FcChar8* FcLangNormalize(const FcChar8* lang)
{
    ...
    if (FcStrCmpIgnoreCase(lang, (const FcChar8*)"C") == 0 || FcStrCmpIgnoreCase(lang, (const FcChar8*)"C.UTF-8") == 0
        || FcStrCmpIgnoreCase(lang, (const FcChar8*)"C.utf8") == 0 || FcStrCmpIgnoreCase(lang, (const FcChar8*)"POSIX") == 0) {
    ...
    if (territory && (tlen < 2 || tlen > 3) && !(territory[0] == 'z' && tlen < 5)) {
...
FcLangResult FcLangCompare(const FcChar8* s1, const FcChar8* s2)
{
    ...
    is_und = FcToLower(s1[0]) == 'u' && FcToLower(s1[1]) == 'n' && FcToLower(s1[2]) == 'd' && FcLangEnd(s1[3]);
...
static int FcLangSetIndex(const FcChar8* lang)
{
    ...
    while (low <= high) {
	     ...
	     * finding the first char (probably already true because of the hash table) */
            ...
            if (cmp == 0 && (fcLangCharSets[mid].lang[2] != '\0' || lang[2] != '\0')) {
...
static FcBool FcLangSetContainsLang(const FcLangSet* ls, const FcChar8* lang)
{
    ...
    for (i = id - 1; i >= 0; i--) {
        if (FcLangCompare(fcLangCharSets[i].lang, lang) == FcLangDifferentLang)
            break;
        if (FcLangSetBitGet(ls, i) && FcLangContains(fcLangCharSets[i].lang, lang))
            return FcTrue;
    }
    for (i = id; i < NUM_LANG_CHAR_SET; i++) {
        if (FcLangCompare(fcLangCharSets[i].lang, lang) == FcLangDifferentLang)
...
FcBool FcLangSetContains(const FcLangSet* lsa, const FcLangSet* lsb)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (missing) {
            for (j = 0; j < 32; j++)
                if (missing & (1U << j)) {
                    if (!FcLangSetContainsLang(lsa, fcLangCharSets[fcLangCharSetIndicesInv[i * 32 + j]].lang)) {
                        if (FcDebug() & FC_DBG_MATCHV)
