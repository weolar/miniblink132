...
FcHashTable* FcHashTableCreate(FcHashFunc hash_func, FcCompareFunc compare_func, FcCopyFunc key_copy_func, FcCopyFunc value_copy_func,
...
static FcBool FcHashTableAddInternal(FcHashTable* table, void* key, void* value, FcBool replace)
{
...
retry:
    for (prev = &table->buckets[hash % FC_HASH_SIZE]; (b = fc_atomic_ptr_get(prev)); prev = &(b->next)) {
...
retry:
    for (prev = &table->buckets[hash % FC_HASH_SIZE]; (bucket = fc_atomic_ptr_get(prev)); prev = &(bucket->next)) {
        if (!table->compare_func(bucket->key, key)) {
