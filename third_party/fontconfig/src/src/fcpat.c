...
static FcChar32 FcValueHash(const FcValue* v)
{
    ...
    case FcTypeMatrix:
        return (FcDoubleHash(v->u.m->xx) ^ FcDoubleHash(v->u.m->xy) ^ FcDoubleHash(v->u.m->yx) ^ FcDoubleHash(v->u.m->yy));
    case FcTypeCharSet:
        return (FcChar32)FcValueCharSet(v)->num;
    case FcTypeFTFace:
        return FcStringHash((const FcChar8*)((FT_Face)v->u.f)->family_name) ^ FcStringHash((const FcChar8*)((FT_Face)v->u.f)->style_name);
    case FcTypeLangSet:
...
FcPattern* FcPatternCacheRewriteFile(const FcPattern* p, FcCache* cache, const FcChar8* relocated_font_file)
{
     ...
     * cache */
    data = FcCacheAllocate(cache, sizeof(FcPattern) + p->num * sizeof(FcPatternElt) + sizeof(FcValueList) + new_path_len + 1);
...
FcPatternElt* FcPatternObjectInsertElt(FcPattern* p, FcObject object)
{
    ...
    if (i < 0) {
        ...
        e = FcPatternElts(p);
        /* move elts up */
        memmove(e + i + 1, e + i, sizeof(FcPatternElt) * (FcPatternObjectCount(p) - i));
...
FcChar32 FcPatternHash(const FcPattern* p)
{
    ...
    for (i = 0; i < FcPatternObjectCount(p); i++) {
        h = (((h << 1) | (h >> 31)) ^ pe[i].object ^ FcValueListHash(FcPatternEltValues(&pe[i])));
    }
    return h;
}
...
FcBool FcPatternObjectListAdd(FcPattern* p, FcObject object, FcValueListPtr list, FcBool append)
{
    ...
    for (l = list; l != NULL; l = FcValueListNext(l)) {
        if (!FcObjectValidType(object, l->value.type)) {
            fprintf(stderr, "Fontconfig warning: FcPattern object %s does not accept value", FcObjectName(object));
...
{
    return FcPatternObjectAddWithBinding(p, object, value, FcValueBindingStrong, append);
}

FcBool FcPatternAdd(FcPattern* p, const char* object, FcValue value, FcBool append)
{
    return FcPatternObjectAddWithBinding(p, FcObjectFromName(object), value, FcValueBindingStrong, append);
}

FcBool FcPatternAddWeak(FcPattern* p, const char* object, FcValue value, FcBool append)
{
    return FcPatternObjectAddWithBinding(p, FcObjectFromName(object), value, FcValueBindingWeak, append);
}
...
FcBool FcPatternObjectDel(FcPattern* p, FcObject object)
{
    ...
    memmove(e, e + 1, (FcPatternElts(p) + FcPatternObjectCount(p) - (e + 1)) * sizeof(FcPatternElt));
    p->num--;
...
FcPattern* FcPatternDuplicate(const FcPattern* orig)
{
    ...
    do {
        for (l = FcPatternIterGetValues(orig, &iter); l; l = FcValueListNext(l)) {
            if (!FcPatternObjectAddWithBinding(new, FcPatternIterGetObjectId(orig, &iter), FcValueCanonicalize(&l->value), l->binding, FcTrue))
                goto bail1;
        }
...
FcBool FcPatternAppend(FcPattern* p, FcPattern* s)
{
    ...
    do {
        for (v = FcPatternIterGetValues(s, &iter); v; v = FcValueListNext(v)) {
            if (!FcPatternObjectAddWithBinding(p, FcPatternIterGetObjectId(s, &iter), FcValueCanonicalize(&v->value), v->binding, FcTrue))
                return FcFalse;
        }
    ...
    for (i = 0; i < os->nobject; i++) {
        ...
        if (e) {
            for (v = FcPatternEltValues(e); v; v = FcValueListNext(v)) {
                if (!FcPatternObjectAddWithBinding(ret, e->object, FcValueCanonicalize(&v->value), v->binding, FcTrue))
                    goto bail0;
            }
        }
    }
...
FcBool FcPatternIterEqual(const FcPattern* p1, FcPatternIter* i1, const FcPattern* p2, FcPatternIter* i2)
{
    ...
    return FcValueListEqual(FcPatternIterGetValues(p1, i1), FcPatternIterGetValues(p2, i2));
}
...
FcPattern* FcPatternSerialize(FcSerialize* serialize, const FcPattern* pat)
{
    ...
    *pat_serialized = *pat;
    ...
    pat_serialized->elts_offset = FcPtrToOffset(pat_serialized, elts_serialized);
    ...
    for (i = 0; i < FcPatternObjectCount(pat); i++) {
        ...
        elts_serialized[i].values = FcPtrToEncodedOffset(&elts_serialized[i], values_serialized, FcValueList);
    }
...
FcValueList* FcValueListSerialize(FcSerialize* serialize, const FcValueList* vl)
{
    ...
    while (vl) {
        ...
        if (prev_serialized)
            prev_serialized->next = FcPtrToEncodedOffset(prev_serialized, vl_serialized, FcValueList);
        else
        ...
        case FcTypeString:
            ...
            if (!s_serialized)
                return NULL;
            vl_serialized->value.u.s = FcPtrToEncodedOffset(&vl_serialized->value, s_serialized, FcChar8);
            break;
        case FcTypeBool:
        ...
        case FcTypeCharSet:
            ...
            if (!c_serialized)
                return NULL;
            vl_serialized->value.u.c = FcPtrToEncodedOffset(&vl_serialized->value, c_serialized, FcCharSet);
            break;
        case FcTypeFTFace:
            /* can't happen */
            break;
        case FcTypeLangSet:
            ...
            if (!l_serialized)
                return NULL;
            vl_serialized->value.u.l = FcPtrToEncodedOffset(&vl_serialized->value, l_serialized, FcLangSet);
            break;
        case FcTypeRange:
            r_serialized = FcRangeSerialize(serialize, vl->value.u.r);
            if (!r_serialized)
                return NULL;
            vl_serialized->value.u.r = FcPtrToEncodedOffset(&vl_serialized->value, r_serialized, FcRange);
            break;
        default:
            break;
        }
