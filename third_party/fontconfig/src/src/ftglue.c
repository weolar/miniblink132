...
static FT_Pointer ftglue_qalloc(FT_Memory memory, FT_ULong size, FT_Error* perror)
{
    FT_Error error = 0;
...
ftglue_face_goto_table(FT_Face face, FT_ULong the_tag, FT_Stream stream)
{
    ...
    LOG(("ftglue_face_goto_table( %p, %c%c%c%c, %p )\n", face, (int)((the_tag >> 24) & 0xFF), (int)((the_tag >> 16) & 0xFF), (int)((the_tag >> 8) & 0xFF),
        (int)(the_tag & 0xFF), stream));
