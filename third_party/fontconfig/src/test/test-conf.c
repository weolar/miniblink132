...
static FcPattern* build_pattern(json_object* obj)
{
...
static FcFontSet* build_fs(json_object* obj)
{
...
static FcBool build_fonts(FcConfig* config, json_object* root)
{
    json_object* fonts;
    FcFontSet* fs;

    if (!json_object_object_get_ex(root, "fonts", &fonts) || json_object_get_type(fonts) != json_type_array) {
...
static FcBool run_test(FcConfig* config, json_object* root)
{
    json_object* tests;
    FcFontSet* fs;
    int i, n, fail = 0;

    if (!json_object_object_get_ex(root, "tests", &tests) || json_object_get_type(tests) != json_type_array) {
...
static FcBool run_scenario(FcConfig* config, char* file)
{
    FcBool ret = FcTrue;
...
static FcBool load_config(FcConfig* config, char* file)
{
    FILE* fp;
    long len;
    char* buf = NULL;
