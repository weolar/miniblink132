...
static const CaseFoldClassMap caseFoldClassMap[]
    = { { "C", CaseFoldCommon }, { "F", CaseFoldFull }, { "S", CaseFoldSimple }, { "T", CaseFoldTurkic }, { 0, 0 } };
...
static FcCaseFold* addFold(void)
{
    if (folds)
...
static int ucs4_to_utf8(FcChar32 ucs4, FcChar8 dest[FC_UTF8_MAX_LEN])
{
    int bits;
    FcChar8* d = dest;
...
static int utf8_size(FcChar32 ucs4)
{
...
static void addChar(FcChar32 c)
{
    FcChar8 utf8[FC_UTF8_MAX_LEN];
    int len;
    int i;
...
static int foldExtends(FcCaseFold* fold, CaseFoldRaw* raw)
{
    switch (fold->method) {
...
static const char* case_fold_method_name(FcChar16 method)
{
    switch (method) {
    case FC_CASE_FOLD_RANGE:
...
static void dump(void)
{
    ...
    for (i = 0; i < nfolds; i++) {
        printf("    { 0x%08x, %-22s 0x%04x, %6d },\n", folds[i].upper, case_fold_method_name(folds[i].method), folds[i].count, folds[i].offset);
    }
    printf("};\n\n");
...
static int parseRaw(char* line, CaseFoldRaw* raw)
{
    char *tok, *end;
    int i;
...
static int caseFoldReadRaw(FILE* in, CaseFoldRaw* raw)
{
    char line[MAX_LINE];
