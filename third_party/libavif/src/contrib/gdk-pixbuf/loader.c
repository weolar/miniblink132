...
static gboolean avif_context_try_load(struct avif_context* context, GError** error)
{
    ...
    if (ret != AVIF_RESULT_OK) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, "Couldn't decode image: %s", avifResultToString(ret));
        return FALSE;
    }
    ...
    if (decoder->imageCount > 1) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, "Image sequences not yet implemented");
        return FALSE;
    }
    ...
    if (ret == AVIF_RESULT_NO_IMAGES_REMAINING) {
        ...
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, "Failed to decode all frames: %s", avifResultToString(ret));
        return FALSE;
    }
    ...
    if (image->alphaPlane) {
        ...
        output = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, width, height);
    } else {
    ...
    if (output == NULL) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY, "Insufficient memory to open AVIF file");
        return FALSE;
    }
    ...
    if (ret != AVIF_RESULT_OK) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, "Failed to convert YUV to RGB: %s", avifResultToString(ret));
        return FALSE;
    }
    ...
    if (width == 0 || height == 0) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, "Transformed AVIF has zero width or height");
        return FALSE;
    }
...
static gpointer begin_load(
    GdkPixbufModuleSizeFunc size_func, GdkPixbufModulePreparedFunc prepared_func, GdkPixbufModuleUpdatedFunc updated_func, gpointer user_data, GError** error)
{
...
static gboolean avif_image_saver(FILE* f, GdkPixbuf* pixbuf, gchar** keys, gchar** values, GError** error)
{
            ...
            if (strcmp(*kiter, "quality") == 0) {
                ...
                if (endptr == *viter) {
                    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                        "AVIF quality must be a value between 0 and 100; value \"%s\" could not be parsed.", *viter);
                ...
                if (quality < 0 || quality > 100) {
                    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                        "AVIF quality must be a value between 0 and 100; value \"%ld\" is not allowed.", quality);
    ...
    if (gdk_pixbuf_get_bits_per_sample(pixbuf) != 8) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, "Sorry, only 8bit images are supported by this AVIF saver");
        return FALSE;
    }
    ...
    if (width == 0 || height == 0) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, "Empty image, nothing to save");
        return FALSE;
    }
    ...
    if (save_alpha) {
        if (gdk_pixbuf_get_n_channels(pixbuf) != 4) {
            g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, "Unsupported number of channels");
            return FALSE;
        }
    } else {
    ...
    if (res != AVIF_RESULT_OK) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, "Problem in RGB->YUV conversion: %s", avifResultToString(res));
    ...
    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, "AVIF encoder problem: %s", avifResultToString(res));
    return FALSE;
}
...
G_MODULE_EXPORT void fill_info(GdkPixbufFormat* info)
{
    static GdkPixbufModulePattern signature[] = { { "    ftypavif", "zzz         ", 100 }, /* file begins with 'ftypavif' at offset 4 */
        { NULL, NULL, 0 } };
    static gchar* mime_types[] = { "image/avif", NULL };
    static gchar* extensions[] = { "avif", NULL };
