...
#define FUNC(RETURN_TYPE, NAME, ...)                                                                                                                           \
    extern "C" {                                                                                                                                               \
    JNIEXPORT RETURN_TYPE Java_org_aomedia_avif_android_AvifDecoder_##NAME(JNIEnv* env, jobject /*thiz*/, ##__VA_ARGS__);                                      \
    }                                                                                                                                                          \
    JNIEXPORT RETURN_TYPE Java_org_aomedia_avif_android_AvifDecoder_##NAME(JNIEnv* env, jobject /*thiz*/, ##__VA_ARGS__)
...
bool CreateDecoderAndParse(AvifDecoderWrapper* const decoder, const uint8_t* const buffer, int length, int threads)
{
...
jint JNI_OnLoad(JavaVM* vm, void* /*reserved*/)
{
    ...
    const jclass info_class = env->FindClass("org/aomedia/avif/android/AvifDecoder$Info");
...
FUNC(jboolean, decode, jobject encoded, int length, jobject bitmap)
{
    const uint8_t* const buffer = static_cast<const uint8_t*>(env->GetDirectBufferAddress(encoded));
    AvifDecoderWrapper decoder;
    if (!CreateDecoderAndParse(&decoder, buffer, length, android_getCpuCount())) {
        return false;
    }
    ...
    if (bitmap_info.width < decoder.decoder->image->width || bitmap_info.height < decoder.decoder->image->height) {
        LOGE("Bitmap is not large enough to fit the image. Bitmap %dx%d Image "
             "%dx%d.",
            bitmap_info.width, bitmap_info.height, decoder.decoder->image->width, decoder.decoder->image->height);
        return false;
    }
    // Ensure that the bitmap format is RGBA_8888, RGB_565 or RGBA_F16.
    if (bitmap_info.format != ANDROID_BITMAP_FORMAT_RGBA_8888 && bitmap_info.format != ANDROID_BITMAP_FORMAT_RGB_565
