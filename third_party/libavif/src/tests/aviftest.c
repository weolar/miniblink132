    ...
    for (; filename != NULL; filename = nextFilename(ioDir, "avif", &nfd)) {
        ...
        for (int pass = 0; pass < 4; ++pass) {
            ...
            for (io->availableBytes = 0; io->availableBytes <= io->io.sizeHint; ++io->availableBytes) {
                ...
                printf("File: [%s @ %zu / %" PRIu64 " bytes, %s, %s] parse returned: %s\n", filename, io->availableBytes, io->io.sizeHint,
                    io->io.persistent ? "Persistent" : "NonPersistent", decoder->ignoreExif ? "IgnoreMetadata" : "Metadata", avifResultToString(parseResult));
                break;
            }
                ...
                for (; io->availableBytes <= io->io.sizeHint; ++io->availableBytes) {
                    ...
                    if (extentResult != AVIF_RESULT_OK) {
                        ...
                        printf("File: [%s @ %zu / %" PRIu64 " bytes, %s, %s] maxExtent returned: %s\n", filename, io->availableBytes, io->io.sizeHint,
                            io->io.persistent ? "Persistent" : "NonPersistent", decoder->ignoreExif ? "IgnoreMetadata" : "Metadata",
                        ...
                        printf("File: [%s @ %zu / %" PRIu64 " bytes, %s, %s] nextImage [MaxExtent off %" PRIu64 ", size %zu] returned: %s\n", filename,
                            io->availableBytes, io->io.sizeHint, io->io.persistent ? "Persistent" : "NonPersistent",
                            decoder->ignoreExif ? "IgnoreMetadata" : "Metadata", extent.offset, extent.size, avifResultToString(nextImageResult));
                    }
                    break;
                }
            }
