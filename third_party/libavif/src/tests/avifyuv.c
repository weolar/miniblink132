                    ...
                    for (int rangeIndex = 0; rangeIndex < 2; ++rangeIndex) {
                        ...
                        if (range == AVIF_RANGE_LIMITED && cicp->mc == AVIF_MATRIX_COEFFICIENTS_YCGCO) {
                            printf(" * RGB depth: %d, YUV depth: %d, colorPrimaries: %d, transferCharas: %d, matrixCoeffs: %d, range: Limited\n"
                                ...
                                rgbDepth, yuvDepth, cicp->cp, cicp->tc, cicp->mc);
                            continue;
                        }
                        ...
                        for (int r = 0; r < dim; ++r) {
                            if (verbose) {
                                printf("[%4d/%4d] RGB depth: %d, YUV depth: %d, colorPrimaries: %d, transferCharas: %d, matrixCoeffs: %d, range: %s\r", r + 1,
                                    dim, rgbDepth, yuvDepth, cicp->cp, cicp->tc, cicp->mc, range == AVIF_RANGE_FULL ? "Full" : "Limited");
                            }
        ...
        for (int yuvDepthIndex = 0; yuvDepthIndex < yuvDepthsCount; ++yuvDepthIndex) {
            ...
            for (int depthIndex = 0; depthIndex < 4; ++depthIndex) {
                ...
                for (int rangeIndex = 0; rangeIndex < 2; ++rangeIndex) {
                    ...
                    const avifRGBFormat rgbFormats[6]
                                ...
                                if (yuvDepth > 8) {
                                    ...
                                    if (showAllResults || !matches) {
                                        printf("yuvDepth:%2d rgbFormat:%s rgbDepth:%2d yuvRange:%7s (%d,%d) [%7s] (%d, %d, %d, %d) -> (%d, %d, %d, %d)\n",
                                            yuvDepth, rgbFormatToString(rgbFormat), rgbDepth, (yuvRange == AVIF_RANGE_LIMITED) ? "Limited" : "Full", i, j,
                                            matches ? "Match" : "NoMatch", srcPixel[0], srcPixel[1], srcPixel[2], srcPixel[3], dstPixel[0], dstPixel[1],
            ...
            for (uint32_t a = 0; a < size; ++a) {
                ...
                for (uint32_t r = 0; r <= a; ++r) {
                    if (rgbDepth == 8) {
                        ...
                        if (drift >= MAX_DRIFT) {
                            printf("ERROR: Premultiply round-trip difference greater than or equal to MAX_DRIFT(%d): RGB depth: %d, src: %d, dst: %d, alpha: "
                                   "%d.\n",
                                MAX_DRIFT, rgbDepth, pixel[0], r, a);
                            return 1;
                        }
                        if (maxDrift < drift) {
                            maxDrift = drift;
