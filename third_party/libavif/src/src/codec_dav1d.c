...
static avifBool dav1dCodecGetNextImage(
    struct avifCodec* codec, struct avifDecoder* decoder, const avifDecodeSample* sample, avifBool alpha, avifBool* isLimitedRangeAlpha, avifImage* image)
{
...
#if 1 // DAV1D_API_VERSION_MAJOR >= 6
    ...
    if (isColor) {
        ...
        if (image->width && image->height) {
            if ((image->width != (uint32_t)dav1dImage->p.w) || (image->height != (uint32_t)dav1dImage->p.h) || (image->depth != (uint32_t)dav1dImage->p.bpc)
