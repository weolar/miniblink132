...
typedef struct avifDecoderItem {
    ...
    avifBool
        hasUnsupportedEssentialProperty; // If true, this item cites a property flagged as 'essential' that libavif doesn't support (yet). Ignore the item, if so.
    avifBool ipmaSeen; // if true, this item already received a property association
    avifBool
        progressive; // if true, this item has progressive layers (a1lx), but does not select a specific layer (the layer_id value in lsel is set to 0xFFFF)
} avifDecoderItem;
...
static avifBool avifCodecDecodeInputFillFromSampleTable(
    avifCodecDecodeInput* decodeInput, avifSampleTable* sampleTable, const uint32_t imageCountLimit, const uint64_t sizeHint, avifDiagnostics* diag)
{
    if (imageCountLimit) {
    ...
    for (uint32_t chunkIndex = 0; chunkIndex < sampleTable->chunks.count; ++chunkIndex) {
        ...
        for (uint32_t sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex) {
            ...
            if (sampleSize > UINT64_MAX - sampleOffset) {
                avifDiagnosticsPrintf(diag, "Sample table contains an offset/size pair which overflows: [%" PRIu64 " / %u]", sampleOffset, sampleSize);
                return AVIF_FALSE;
            }
...
static avifBool avifCodecDecodeInputFillFromDecoderItem(avifCodecDecodeInput* decodeInput, avifDecoderItem* item, avifBool allowProgressive,
    const uint32_t imageCountLimit, const uint64_t sizeHint, avifDiagnostics* diag)
{
        ...
        if (layerCount > 0) {
            ...
            if (lselProp->u.lsel.layerID >= layerCount) {
                avifDiagnosticsPrintf(
                    diag, "lsel property requests layer index [%u] which isn't present in a1lx property ([%u] layers)", lselProp->u.lsel.layerID, layerCount);
                return AVIF_FALSE;
            }
...
static avifResult avifDecoderItemValidateAV1(const avifDecoderItem* item, avifDiagnostics* diag, const avifStrictFlags strictFlags)
{
    ...
    if (!pixiProp && (strictFlags & AVIF_STRICT_PIXI_REQUIRED)) {
        ...
        avifDiagnosticsPrintf(diag, "[Strict] Item ID %u of type '%.4s' is missing mandatory pixi property", item->id, (const char*)item->type);
        return AVIF_RESULT_BMFF_PARSE_FAILED;
    }
    ...
    if (pixiProp) {
        ...
        for (uint8_t i = 0; i < pixiProp->u.pixi.planeCount; ++i) {
            if (pixiProp->u.pixi.planeDepths[i] != av1CDepth) {
                ...
                avifDiagnosticsPrintf(diag, "Item ID %u depth specified by pixi property [%u] does not match av1C property depth [%u]", item->id,
                    pixiProp->u.pixi.planeDepths[i], av1CDepth);
                return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
        }
    }
    ...
    if (strictFlags & AVIF_STRICT_CLAP_VALID) {
        ...
        if (clapProp) {
            ...
            if (!ispeProp) {
                avifDiagnosticsPrintf(diag, "[Strict] Item ID %u is missing an ispe property, so its clap property cannot be validated", item->id);
                return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
...
static avifResult avifDecoderItemRead(avifDecoderItem* item, avifIO* io, avifROData* outData, size_t offset, size_t partialByteCount, avifDiagnostics* diag)
{
    ...
    for (uint32_t extentIter = 0; extentIter < item->extents.count; ++extentIter) {
        ...
        if (idatBuffer) {
            ...
            if (bytesToRead != offsetBuffer.size) {
                avifDiagnosticsPrintf(diag, "Item ID %u tried to read %zu bytes, but only received %zu bytes", item->id, bytesToRead, offsetBuffer.size);
                return AVIF_RESULT_TRUNCATED_DATA;
            }
        }
...
static avifBool avifDecoderGenerateImageGridTiles(avifDecoder* decoder, avifImageGrid* grid, avifDecoderItem* gridItem, avifBool alpha)
{
    ...
    if (tilesRequested != tilesAvailable) {
        avifDiagnosticsPrintf(&decoder->diag, "Grid image of dimensions %ux%u requires %u tiles, and only %u were found", grid->columns, grid->rows,
            tilesRequested, tilesAvailable);
        return AVIF_FALSE;
    }
    ...
    for (uint32_t i = 0; i < gridItem->meta->items.count; ++i) {
        ...
        if (item->dimgForID == gridItem->id) {
            ...
            if (!avifCodecDecodeInputFillFromDecoderItem(
                    tile->input, item, decoder->allowProgressive, decoder->imageCountLimit, decoder->io->sizeHint, &decoder->diag)) {
                return AVIF_FALSE;
            }
...
static avifBool avifDecoderDataFillImageGrid(avifDecoderData* data, avifImageGrid* grid, avifImage* dstImage, unsigned int firstTileIndex,
    unsigned int oldDecodedTileCount, unsigned int decodedTileCount, avifBool alpha)
{
    ...
    if (((firstTile->image->width * grid->columns) < grid->outputWidth) || ((firstTile->image->height * grid->rows) < grid->outputHeight)) {
        avifDiagnosticsPrintf(data->diag, "Grid image tiles do not completely cover the image (HEIF (ISO/IEC 23008-12:2017), Section 6.6.2.3.1)");
        return AVIF_FALSE;
    }
    ...
    if (((firstTile->image->width * (grid->columns - 1)) >= grid->outputWidth) || ((firstTile->image->height * (grid->rows - 1)) >= grid->outputHeight)) {
        avifDiagnosticsPrintf(data->diag,
            "Grid image tiles in the rightmost column and bottommost row do not overlap the reconstructed image grid canvas. See MIAF (ISO/IEC 23000-22:2019), "
            "Section 7.3.11.4.2, Figure 2");
        return AVIF_FALSE;
    }
    ...
    if (!avifAreGridDimensionsValid(
            firstTile->image->yuvFormat, grid->outputWidth, grid->outputHeight, firstTile->image->width, firstTile->image->height, data->diag)) {
        return AVIF_FALSE;
    }

    // Lazily populate dstImage with the new frame's properties. If we're decoding alpha,
    // these values must already match.
    if ((dstImage->width != grid->outputWidth) || (dstImage->height != grid->outputHeight) || (dstImage->depth != firstTile->image->depth)
        || (!alpha && (dstImage->yuvFormat != firstTile->image->yuvFormat))) {
        if (alpha) {
        ...
        for (; colIndex < grid->columns; ++colIndex, ++tileIndex) {
            ...
            if (alpha) {
                ...
                for (unsigned int j = 0; j < heightToCopy; ++j) {
                    ...
                    uint8_t* dst = &dstImage->yuvPlanes[AVIF_CHAN_Y][(yaColOffset * pixelBytes) + ((yaRowOffset + j) * dstImage->yuvRowBytes[AVIF_CHAN_Y])];
                    memcpy(dst, src, yaRowBytes);
                }
                ...
                for (unsigned int j = 0; j < heightToCopy; ++j) {
                    ...
                    uint8_t* dstU = &dstImage->yuvPlanes[AVIF_CHAN_U][(uvColOffset * pixelBytes) + ((uvRowOffset + j) * dstImage->yuvRowBytes[AVIF_CHAN_U])];
                    memcpy(dstU, srcU, uvRowBytes);

                    uint8_t* srcV = &tile->image->yuvPlanes[AVIF_CHAN_V][j * tile->image->yuvRowBytes[AVIF_CHAN_V]];
                    uint8_t* dstV = &dstImage->yuvPlanes[AVIF_CHAN_V][(uvColOffset * pixelBytes) + ((uvRowOffset + j) * dstImage->yuvRowBytes[AVIF_CHAN_V])];
...
static avifResult avifDecoderFindMetadata(avifDecoder* decoder, avifMeta* meta, avifImage* image, uint32_t colorId)
{
    ...
    for (uint32_t itemIndex = 0; itemIndex < meta->items.count; ++itemIndex) {
        ...
        } else if (!decoder->ignoreXMP && !memcmp(item->type, "mime", 4) && !memcmp(item->contentType.contentType, xmpContentType, xmpContentTypeSize)) {
    ...
    for (uint32_t i = 0; i < itemCount; ++i) {
        ...
        for (int extentIter = 0; extentIter < extentCount; ++extentIter) {
            ...
            if (extentOffset > UINT64_MAX - baseOffset) {
                avifDiagnosticsPrintf(
                    diag, "Item ID [%u] contains an extent offset which overflows: [base: %" PRIu64 " offset:%" PRIu64 "]", itemID, baseOffset, extentOffset);
                return AVIF_FALSE;
            }
            ...
            if (extent->size > SIZE_MAX - item->size) {
                avifDiagnosticsPrintf(
                    diag, "Item ID [%u] contains an extent length which overflows the item size: [%zu, %zu]", itemID, extent->size, item->size);
                return AVIF_FALSE;
            }
...
static avifBool avifParseImageGridBox(
    avifImageGrid* grid, const uint8_t* raw, size_t rawLen, uint32_t imageSizeLimit, uint32_t imageDimensionLimit, avifDiagnostics* diag)
{
...
static avifBool avifParseItemPropertyContainerBox(avifPropertyArray* properties, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, avifDiagnostics* diag)
{
    ...
    for (uint32_t entryIndex = 0; entryIndex < entryCount; ++entryIndex) {
        ...
        for (uint8_t associationIndex = 0; associationIndex < associationCount; ++associationIndex) {
            ...
            if (propertyIndex >= meta->properties.count) {
                avifDiagnosticsPrintf(diag, "Box[ipma] for item ID [%u] contains an illegal property index [%u] (out of [%u] properties)", itemID,
                    propertyIndex, meta->properties.count);
                return AVIF_FALSE;
            }
            ...
            static const char* supportedTypes[] = { "ispe", "auxC", "colr", "av1C", "pasp", "clap", "irot", "imir", "pixi", "a1op", "lsel", "a1lx" };
                ...
                if (essential) {
                    ...
                    for (size_t i = 0; i < nonessentialTypesCount; ++i) {
                        if (!memcmp(srcProp->type, nonessentialTypes[i], 4)) {
                            avifDiagnosticsPrintf(
                                diag, "Item ID [%u] has a %s property association which must not be marked essential, but is", itemID, nonessentialTypes[i]);
                            return AVIF_FALSE;
                        }
                    }
                } else {
                    ...
                    for (size_t i = 0; i < essentialTypesCount; ++i) {
                        if (!memcmp(srcProp->type, essentialTypes[i], 4)) {
                            avifDiagnosticsPrintf(
                                diag, "Item ID [%u] has a %s property association which must be marked essential, but is not", itemID, essentialTypes[i]);
                            return AVIF_FALSE;
                        }
                    }
                }
...
static avifBool avifParseItemPropertiesBox(avifMeta* meta, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, avifDiagnostics* diag)
{
    ...
    AVIF_CHECK(avifParseItemPropertyContainerBox(&meta->properties, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), ipcoHeader.size, diag));
...
static avifBool avifParseTrackHeaderBox(
    avifTrack* track, const uint8_t* raw, size_t rawLen, uint32_t imageSizeLimit, uint32_t imageDimensionLimit, avifDiagnostics* diag)
{
...
static avifBool avifParseSampleDescriptionBox(avifSampleTable* sampleTable, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, avifDiagnostics* diag)
{
    ...
    for (uint32_t i = 0; i < entryCount; ++i) {
        ...
        if (!memcmp(description->format, "av01", 4) && (remainingBytes > VISUALSAMPLEENTRY_SIZE)) {
            AVIF_CHECK(avifParseItemPropertyContainerBox(&description->properties, rawOffset + avifROStreamOffset(&s) + VISUALSAMPLEENTRY_SIZE,
                avifROStreamCurrent(&s) + VISUALSAMPLEENTRY_SIZE, remainingBytes - VISUALSAMPLEENTRY_SIZE, diag));
        }
...
static avifBool avifParseSampleTableBox(avifTrack* track, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, avifDiagnostics* diag)
{
    ...
    while (avifROStreamHasBytesLeft(&s, 1)) {
        ...
        if (!memcmp(header.type, "stco", 4)) {
            ...
            AVIF_CHECK(avifParseSampleDescriptionBox(track->sampleTable, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, diag));
        }
...
static avifBool avifParseTrackBox(
    avifDecoderData* data, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, uint32_t imageSizeLimit, uint32_t imageDimensionLimit)
{
...
static avifBool avifParseMovieBox(
    avifDecoderData* data, uint64_t rawOffset, const uint8_t* raw, size_t rawLen, uint32_t imageSizeLimit, uint32_t imageDimensionLimit)
{
    ...
    while (avifROStreamHasBytesLeft(&s, 1)) {
        ...
        if (!memcmp(header.type, "trak", 4)) {
            AVIF_CHECK(avifParseTrackBox(data, rawOffset + avifROStreamOffset(&s), avifROStreamCurrent(&s), header.size, imageSizeLimit, imageDimensionLimit));
        }
...
static avifResult avifParse(avifDecoder* decoder)
{
    ...
    for (;;) {
        ...
        if (!memcmp(header.type, "ftyp", 4)) {
            ...
            AVIF_CHECKERR(avifParseMetaBox(data->meta, boxOffset, boxContents.data, boxContents.size, data->diag), AVIF_RESULT_BMFF_PARSE_FAILED);
            metaSeen = AVIF_TRUE;
        ...
        if (ispeProp) {
            ...
            if (auxCProp && isAlphaURN(auxCProp->u.auxC.auxType)) {
                if (decoder->strictFlags & AVIF_STRICT_ALPHA_ISPE_REQUIRED) {
                    avifDiagnosticsPrintf(data->diag, "[Strict] Alpha auxiliary image item ID [%u] is missing a mandatory ispe property", item->id);
                    return AVIF_RESULT_BMFF_PARSE_FAILED;
                }
            } else {
...
avifResult avifDecoderReset(avifDecoder* decoder)
{
    ...
    if (data->source == AVIF_DECODER_SOURCE_TRACKS) {
        ...
        if (!avifCodecDecodeInputFillFromSampleTable(colorTile->input, colorTrack->sampleTable, decoder->imageCountLimit, decoder->io->sizeHint, data->diag)) {
            return AVIF_RESULT_BMFF_PARSE_FAILED;
        }
        ...
        if (alphaTrack) {
            ...
            if (!avifCodecDecodeInputFillFromSampleTable(
                    alphaTile->input, alphaTrack->sampleTable, decoder->imageCountLimit, decoder->io->sizeHint, data->diag)) {
                return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
        ...
        for (uint32_t itemIndex = 0; itemIndex < data->meta->items.count; ++itemIndex) {
            ...
            if (isGrid) {
                ...
                if (!avifParseImageGridBox(&data->colorGrid, readData.data, readData.size, decoder->imageSizeLimit, decoder->imageDimensionLimit, data->diag)) {
        ...
        for (uint32_t itemIndex = 0; itemIndex < data->meta->items.count; ++itemIndex) {
            ...
            if (auxCProp && isAlphaURN(auxCProp->u.auxC.auxType) && (item->auxForID == colorItem->id)) {
                if (isGrid) {
                    ...
                    if (!avifParseImageGridBox(
                            &data->alphaGrid, readData.data, readData.size, decoder->imageSizeLimit, decoder->imageDimensionLimit, data->diag)) {
                        return AVIF_RESULT_INVALID_IMAGE_GRID;
                    }
                }
        ...
        if ((data->colorGrid.rows > 0) && (data->colorGrid.columns > 0)) {
            ...
            avifTile* colorTile = avifDecoderDataCreateTile(data, colorItem->width, colorItem->height, avifDecoderItemOperatingPoint(colorItem));
            if (!colorTile) {
                return AVIF_RESULT_OUT_OF_MEMORY;
            }
            if (!avifCodecDecodeInputFillFromDecoderItem(
                    colorTile->input, colorItem, decoder->allowProgressive, decoder->imageCountLimit, decoder->io->sizeHint, &decoder->diag)) {
                return AVIF_RESULT_BMFF_PARSE_FAILED;
            }
            data->colorTileCount = 1;
        ...
        if (alphaItem) {
            ...
            if ((data->alphaGrid.rows > 0) && (data->alphaGrid.columns > 0)) {
                ...
                avifTile* alphaTile = avifDecoderDataCreateTile(data, alphaItem->width, alphaItem->height, avifDecoderItemOperatingPoint(alphaItem));
                if (!alphaTile) {
                    return AVIF_RESULT_OUT_OF_MEMORY;
                }
                if (!avifCodecDecodeInputFillFromDecoderItem(
                        alphaTile->input, alphaItem, decoder->allowProgressive, decoder->imageCountLimit, decoder->io->sizeHint, &decoder->diag)) {
                    return AVIF_RESULT_BMFF_PARSE_FAILED;
                }
                alphaTile->input->alpha = AVIF_TRUE;
                data->alphaTileCount = 1;
...
static avifResult avifDecoderPrepareTiles(
    avifDecoder* decoder, uint32_t nextImageIndex, unsigned int firstTileIndex, unsigned int tileCount, unsigned int decodedTileCount)
{
...
static avifResult avifDecoderDecodeTiles(
    avifDecoder* decoder, uint32_t nextImageIndex, unsigned int firstTileIndex, unsigned int tileCount, unsigned int* decodedTileCount)
{
    ...
    for (unsigned int tileIndex = oldDecodedTileCount; tileIndex < tileCount; ++tileIndex) {
        ...
        if ((tile->width != tile->image->width) || (tile->height != tile->image->height)) {
            if (!avifImageScale(tile->image, tile->width, tile->height, decoder->imageSizeLimit, decoder->imageDimensionLimit, &decoder->diag)) {
...
avifResult avifDecoderNextImage(avifDecoder* decoder)
{
    ...
    if ((decoder->data->decodedColorTileCount == decoder->data->colorTileCount) && (decoder->data->decodedAlphaTileCount == decoder->data->alphaTileCount)) {
    ...
    if (decoder->data->decodedColorTileCount > oldDecodedColorTileCount) {
        ...
        if ((decoder->data->colorGrid.rows > 0) && (decoder->data->colorGrid.columns > 0)) {
            ...
            if (!avifDecoderDataFillImageGrid(decoder->data, &decoder->data->colorGrid, decoder->image, firstColorTileIndex, oldDecodedColorTileCount,
                    decoder->data->decodedColorTileCount, AVIF_FALSE)) {
                return AVIF_RESULT_INVALID_IMAGE_GRID;
            }
        } else {
            ...
            if ((decoder->image->width != srcColor->width) || (decoder->image->height != srcColor->height) || (decoder->image->depth != srcColor->depth)) {
    ...
    if (decoder->data->decodedAlphaTileCount > oldDecodedAlphaTileCount) {
        ...
        if ((decoder->data->alphaGrid.rows > 0) && (decoder->data->alphaGrid.columns > 0)) {
            ...
            if (!avifDecoderDataFillImageGrid(decoder->data, &decoder->data->alphaGrid, decoder->image, firstAlphaTileIndex, oldDecodedAlphaTileCount,
                    decoder->data->decodedAlphaTileCount, AVIF_TRUE)) {
                return AVIF_RESULT_INVALID_IMAGE_GRID;
            }
        } else {
            // Normal (most common) non-grid path. Just steal the planes from the only "tile".
            ...
            if ((decoder->image->width != srcAlpha->width) || (decoder->image->height != srcAlpha->height) || (decoder->image->depth != srcAlpha->depth)) {
...
static uint32_t avifGetDecodedRowCount(
    const avifDecoder* decoder, const avifImageGrid* grid, unsigned int firstTileIndex, unsigned int tileCount, unsigned int decodedTileCount)
{
...
{
    const uint32_t colorRowCount = avifGetDecodedRowCount(decoder, &decoder->data->colorGrid,
        /*firstTileIndex=*/0, decoder->data->colorTileCount, decoder->data->decodedColorTileCount);
    const uint32_t alphaRowCount = avifGetDecodedRowCount(decoder, &decoder->data->alphaGrid,
        /*firstTileIndex=*/decoder->data->colorTileCount, decoder->data->alphaTileCount, decoder->data->decodedAlphaTileCount);
