...
static uint8_t avifItemPropertyDedupFinish(avifItemPropertyDedup* dedup, avifRWStream* outputStream)
{
    ...
    for (size_t i = 0; i < dedup->properties.count; ++i) {
        ...
        if ((property->size == newPropertySize) && !memcmp(&outputStream->raw->data[property->offset], dedup->buffer.data, newPropertySize)) {
...
static avifBool avifEncoderDetectChanges(const avifEncoder* encoder, avifEncoderChanges* encoderChanges)
{
    ...
    *encoderChanges = 0;
    ...
    if ((lastEncoder->codecChoice != encoder->codecChoice) || (lastEncoder->maxThreads != encoder->maxThreads) || (lastEncoder->speed != encoder->speed)
        || (lastEncoder->keyframeInterval != encoder->keyframeInterval) || (lastEncoder->timescale != encoder->timescale)) {
        return AVIF_FALSE;
    }
...
static void avifEncoderWriteColorProperties(avifRWStream* outputStream, const avifImage* imageMetadata, struct ipmaArray* ipma, avifItemPropertyDedup* dedup)
{
...
static void avifCopyAndPadPlane(uint8_t* dstPlane, uint32_t dstRowBytes, uint32_t dstWidth, uint32_t dstHeight, const uint8_t* srcPlane, uint32_t srcRowBytes,
    uint32_t srcWidth, uint32_t srcHeight, uint32_t sampleByteCount)
{
...
static avifImage* avifImageCopyAndPad(const avifImage* srcImage, uint32_t dstWidth, uint32_t dstHeight)
{
    ...
    if (srcImage->yuvPlanes[AVIF_CHAN_Y]) {
        ...
        for (int yuvPlane = 0; yuvPlane < 3; ++yuvPlane) {
            ...
            avifCopyAndPadPlane(dstImage->yuvPlanes[yuvPlane], dstImage->yuvRowBytes[yuvPlane], (yuvPlane == AVIF_CHAN_Y) ? dstImage->width : dstUvWidth,
                (yuvPlane == AVIF_CHAN_Y) ? dstImage->height : dstUvHeight, srcImage->yuvPlanes[yuvPlane], srcImage->yuvRowBytes[yuvPlane],
                (yuvPlane == AVIF_CHAN_Y) ? srcImage->width : srcUvWidth, (yuvPlane == AVIF_CHAN_Y) ? srcImage->height : srcUvHeight,
    ...
    if (srcImage->alphaPlane) {
        ...
        avifCopyAndPadPlane(dstImage->alphaPlane, dstImage->alphaRowBytes, dstImage->width, dstImage->height, srcImage->alphaPlane, srcImage->alphaRowBytes,
            srcImage->width, srcImage->height, avifImageUsesU16(srcImage) ? 2 : 1);
    }
    return dstImage;
}

static avifResult avifEncoderAddImageInternal(avifEncoder* encoder, uint32_t gridCols, uint32_t gridRows, const avifImage* const* cellImages,
    uint64_t durationInTimescales, avifAddImageFlags addImageFlags)
{
    // -----------------------------------------------------------------------
    ...
    if ((cellCount > 1) && !avifAreGridDimensionsValid(firstCell->yuvFormat, gridWidth, gridHeight, tileWidth, tileHeight, &encoder->diag)) {
        return AVIF_RESULT_INVALID_IMAGE_GRID;
    }
    ...
    for (uint32_t cellIndex = 0; cellIndex < cellCount; ++cellIndex) {
        ...
        if ((cellImage->depth != firstCell->depth) || (cellImage->yuvFormat != firstCell->yuvFormat) || (cellImage->yuvRange != firstCell->yuvRange)
            || (cellImage->colorPrimaries != firstCell->colorPrimaries) || (cellImage->transferCharacteristics != firstCell->transferCharacteristics)
    ...
    if (encoder->data->items.count == 0) {
        ...
        if ((imageMetadata->depth != firstCell->depth) || (imageMetadata->yuvFormat != firstCell->yuvFormat) || (imageMetadata->yuvRange != firstCell->yuvRange)
            || (imageMetadata->colorPrimaries != firstCell->colorPrimaries) || (imageMetadata->transferCharacteristics != firstCell->transferCharacteristics)
            || (imageMetadata->matrixCoefficients != firstCell->matrixCoefficients) || (imageMetadata->alphaPremultiplied != firstCell->alphaPremultiplied)
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
        ...
        if (item->codec) {
            ...
            avifResult encodeResult = item->codec->encodeImage(item->codec, encoder, cellImage, item->alpha, encoder->data->tileRowsLog2,
                encoder->data->tileColsLog2, encoderChanges, addImageFlags, item->encodeOutput);
            if (paddedCellImage) {
...
avifResult avifEncoderAddImageGrid(
    avifEncoder* encoder, uint32_t gridCols, uint32_t gridRows, const avifImage* const* cellImages, avifAddImageFlags addImageFlags)
{
    avifDiagnosticsClearError(&encoder->diag);
    ...
    return avifEncoderAddImageInternal(
        encoder, gridCols, gridRows, cellImages, 1, addImageFlags | AVIF_ADD_IMAGE_FLAG_SINGLE); // image grids cannot be image sequences
}
...
avifResult avifEncoderFinish(avifEncoder* encoder, avifRWData* output)
{
    ...
    for (uint32_t itemPasses = 0; itemPasses < 3; ++itemPasses) {
        ...
        for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
            avifEncoderItem* item = &encoder->data->items.item[itemIndex];
            const avifBool isGrid
                = (item->gridCols > 0); // Grids store their payload in metadataPayload, so use this to distinguish grid payloads from XMP/Exif
