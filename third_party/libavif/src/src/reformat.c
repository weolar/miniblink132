...
static avifBool avifPrepareReformatState(const avifImage* image, const avifRGBImage* rgb, avifReformatState* state)
{
    ...
    if (image->yuvFormat <= AVIF_PIXEL_FORMAT_NONE || image->yuvFormat >= AVIF_PIXEL_FORMAT_COUNT || rgb->format < AVIF_RGB_FORMAT_RGB
        || rgb->format >= AVIF_RGB_FORMAT_COUNT) {
        return AVIF_FALSE;
    }
    ...
    if ((image->matrixCoefficients == 3 /* CICP reserved */)
        ...
        || (image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_BT2020_CL) || (image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_SMPTE2085)
                ...
                for (int bJ = 0; bJ < blockH; ++bJ) {
                    for (int bI = 0; bI < blockW; ++bI) {
                        ...
                        if (state.rgbChannelBytes > 1) {
                            rgbPixel[0]
                                = *((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesR + (i * state.rgbPixelBytes) + (j * rgb->rowBytes)])) / rgbMaxChannelF;
                            rgbPixel[1]
                                = *((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesG + (i * state.rgbPixelBytes) + (j * rgb->rowBytes)])) / rgbMaxChannelF;
                            rgbPixel[2]
                                = *((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesB + (i * state.rgbPixelBytes) + (j * rgb->rowBytes)])) / rgbMaxChannelF;
                        } else {
                        ...
                        if (alphaMode != AVIF_ALPHA_MULTIPLY_MODE_NO_OP) {
                            float a;
                            if (state.rgbChannelBytes > 1) {
                                a = *((uint16_t*)(&rgb->pixels[state.rgbOffsetBytesA + (i * state.rgbPixelBytes) + (j * rgb->rowBytes)])) / rgbMaxChannelF;
                            } else {
                        ...
                        if (state.yuvChannelBytes > 1) {
                            ...
                            *pY = (uint16_t)avifReformatStateYToUNorm(&state, yuvBlock[bI][bJ].y);
                            ...
                            yuvPlanes[AVIF_CHAN_Y][i + (j * yuvRowBytes[AVIF_CHAN_Y])] = (uint8_t)avifReformatStateYToUNorm(&state, yuvBlock[bI][bJ].y);
                            if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) {
                                // YUV444, full chroma
                                yuvPlanes[AVIF_CHAN_U][i + (j * yuvRowBytes[AVIF_CHAN_U])] = (uint8_t)avifReformatStateUVToUNorm(&state, yuvBlock[bI][bJ].u);
                                yuvPlanes[AVIF_CHAN_V][i + (j * yuvRowBytes[AVIF_CHAN_V])] = (uint8_t)avifReformatStateUVToUNorm(&state, yuvBlock[bI][bJ].v);
                            }
                        }
                    }
                }
        ...
        for (uint32_t i = 0; i < image->width; ++i) {
            ...
            if (hasColor) {
                if (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) {
                    ...
                    if ((rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_FASTEST) || (rgb->chromaUpsampling == AVIF_CHROMA_UPSAMPLING_NEAREST)) {
            ...
            if (rgb->depth == 8) {
                avifStoreRGB8Pixel(rgb->format, (uint8_t)(0.5f + (Rc * rgbMaxChannelF)), (uint8_t)(0.5f + (Gc * rgbMaxChannelF)),
                    (uint8_t)(0.5f + (Bc * rgbMaxChannelF)), ptrR, ptrG, ptrB);
            } else {
...
avifResult avifImageYUVToRGB(const avifImage* image, avifRGBImage* rgb)
{
    ...
    if (!convertedWithLibYUV) {
        ...
        const avifBool hasColor = (image->yuvRowBytes[AVIF_CHAN_U] && image->yuvRowBytes[AVIF_CHAN_V] && (image->yuvFormat != AVIF_PIXEL_FORMAT_YUV400));
        ...
        if ((!hasColor || (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444)
            ...
            if (state.mode == AVIF_REFORMAT_MODE_IDENTITY) {
                if ((image->depth == 8) && (rgb->depth == 8) && (image->yuvFormat == AVIF_PIXEL_FORMAT_YUV444) && (image->yuvRange == AVIF_RANGE_FULL)) {
