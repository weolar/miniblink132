...
static avifResult rav1eCodecEncodeImage(avifCodec* codec, avifEncoder* encoder, const avifImage* image, avifBool alpha, int tileRowsLog2, int tileColsLog2,
    avifEncoderChanges encoderChanges, uint32_t addImageFlags, avifCodecEncodeOutput* output)
{
    ...
    if (!codec->internal->rav1eContext) {
        ...
        if (rav1e_config_set_pixel_format(
                rav1eConfig, (uint8_t)image->depth, codec->internal->chromaSampling, (RaChromaSamplePosition)image->yuvChromaSamplePosition, rav1eRange)
            < 0) {
            goto cleanup;
        }
        ...
        rav1e_config_set_color_description(rav1eConfig, (RaMatrixCoefficients)image->matrixCoefficients, (RaColorPrimaries)image->colorPrimaries,
