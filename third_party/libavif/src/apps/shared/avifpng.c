    ...
    for (size_t i = 1; i < profileLength; ++i) { // i starts at 1 because the first '\n' was already checked above.
        ...
        if (profile[i] == '\n') {
            if (!lengthStart) {
                ...
                if ((expectedLength <= 0) || (expectedLength == LONG_MAX) || ((unsigned long)expectedLength > (hexPayloadMaxLength / 2))) {
...
avifBool avifPNGRead(const char* inputFilename, avifImage* avif, avifPixelFormat requestedFormat, uint32_t requestedDepth,
    avifChromaDownsampling chromaDownsampling, avifBool ignoreICC, avifBool ignoreExif, avifBool ignoreXMP, uint32_t* outPNGDepth)
{
...
avifBool avifPNGWrite(const char* outputFilename, const avifImage* avif, uint32_t requestedDepth, avifChromaUpsampling chromaUpsampling, int compressionLevel)
{
    ...
    volatile avifBool monochrome8bit = (avif->yuvFormat == AVIF_PIXEL_FORMAT_YUV400) && !avif->alphaPlane && (avif->depth == 8) && (rgbDepth == 8);
