...
static void syntax(void)
{
    ...
    printf("    --stdin                           : Read y4m frames from stdin instead of files; no input filenames allowed, must set before offering output "
           "filename\n");
    ...
    printf("    -g,--grid MxN                     : Encode a single-image grid AVIF with M cols & N rows. Either supply MxN identical W/H/D images, or a "
           "single\n");
    printf("                                        image that can be evenly split into the MxN grid and follow AVIF grid image restrictions. The grid will "
           "adopt\n");
    ...
    printf("    -a,--advanced KEY[=VALUE]         : Pass an advanced, codec-specific key/value string pair directly to the codec. avifenc will warn on any not "
           "used by the codec.\n");
    printf("    --duration D                      : Set all following frame durations (in timescales) to D; default 1. Can be set multiple times (before "
           "supplying each filename)\n");
    printf("    --timescale,--fps V               : Set the timescale to V. If all frames are 1 timescale in length, this is equivalent to frames per second "
           "(Default: 30)\n");
    printf("                                        If neither duration nor timescale are set, avifenc will attempt to use the framerate stored in a y4m "
           "header, if present.\n");
    ...
    if (avifCodecName(AVIF_CODEC_CHOICE_AOM, 0)) {
        ...
        printf("    sharpness=S                       : Bias towards block sharpness in rate-distortion optimization of transform coefficients (0-7, default: "
               "0)\n");
...
static avifBool convertCropToClap(uint32_t srcW, uint32_t srcH, avifPixelFormat yuvFormat, uint32_t clapValues[8])
{
    ...
    if (!convertResult) {
        fprintf(stderr, "ERROR: Impossible crop rect: imageSize:[%ux%u], pixelFormat:%s, cropRect:[%u,%u, %ux%u] - %s\n", srcW, srcH,
            avifPixelFormatToString(yuvFormat), cropRect.x, cropRect.y, cropRect.width, cropRect.height, diag.error);
...
static avifAppFileFormat avifInputReadImage(avifInput* input, avifBool ignoreICC, avifBool ignoreExif, avifBool ignoreXMP, avifImage* image, uint32_t* outDepth,
    avifAppSourceTiming* sourceTiming, avifChromaDownsampling chromaDownsampling)
{
    if (sourceTiming) {
    ...
    const avifAppFileFormat nextInputFormat = avifReadImage(input->files[input->fileIndex].filename, input->requestedFormat, input->requestedDepth,
        chromaDownsampling, ignoreICC, ignoreExif, ignoreXMP, image, outDepth, sourceTiming, &input->frameIter);
...
static avifBool avifImageSplitGrid(const avifImage* gridSplitImage, uint32_t gridCols, uint32_t gridRows, avifImage** gridCells)
{
    ...
    for (uint32_t gridY = 0; gridY < gridRows; ++gridY) {
        for (uint32_t gridX = 0; gridX < gridCols; ++gridX) {
            ...
            cellImage->yuvPlanes[AVIF_CHAN_Y] = &gridSplitImage->yuvPlanes[AVIF_CHAN_Y][(gridX * bytesPerRowY) + (gridY * cellHeight) * srcRowBytesY];
            ...
            if (gridSplitImage->yuvFormat != AVIF_PIXEL_FORMAT_YUV400) {
                ...
                cellImage->yuvPlanes[AVIF_CHAN_U] = &gridSplitImage->yuvPlanes[AVIF_CHAN_U][(gridX * bytesPerRowUV) + (gridY * uvHeight) * srcRowBytesU];
                cellImage->yuvRowBytes[AVIF_CHAN_U] = srcRowBytesU;

                const uint32_t srcRowBytesV = gridSplitImage->yuvRowBytes[AVIF_CHAN_V];
                cellImage->yuvPlanes[AVIF_CHAN_V] = &gridSplitImage->yuvPlanes[AVIF_CHAN_V][(gridX * bytesPerRowUV) + (gridY * uvHeight) * srcRowBytesV];
...
int main(int argc, char* argv[])
{
    ...
    avifAppFileFormat inputFormat = avifInputReadImage(&input, ignoreICC, ignoreExif, ignoreXMP, image, &sourceDepth, &firstSourceTiming, chromaDownsampling);
    ...
    if ((outputTiming.duration == 0) && (outputTiming.timescale == 0) && (firstSourceTiming.duration > 0) && (firstSourceTiming.timescale > 0)) {
    ...
    if (clapCount == 8) {
        ...
        if (!avifCropRectConvertCleanApertureBox(&cropRect, &image->clap, image->width, image->height, image->yuvFormat, &diag)) {
            fprintf(stderr, "ERROR: Invalid clap: width:[%d / %d], height:[%d / %d], horizOff:[%d / %d], vertOff:[%d / %d] - %s\n", (int32_t)image->clap.widthN,
                (int32_t)image->clap.widthD, (int32_t)image->clap.heightN, (int32_t)image->clap.heightD, (int32_t)image->clap.horizOffN,
                (int32_t)image->clap.horizOffD, (int32_t)image->clap.vertOffN, (int32_t)image->clap.vertOffD, diag.error);
            returnCode = 1;
            goto cleanup;
        }
    }
    ...
    if (lossless) {
        ...
        if (!losslessColorQP) {
            fprintf(stderr, "WARNING: [--lossless] Color quantizer range (--min, --max) not set to %d. Color output might not be lossless.\n",
                AVIF_QUANTIZER_LOSSLESS);
            lossless = AVIF_FALSE;
        }
    ...
    if (gridDimsCount > 0) {
        ...
        while ((nextFile = avifInputGetNextFile(&input)) != NULL) {
            ...
            if (gridCellIndex >= gridCellCount) {
                ...
                fprintf(stderr, "WARNING: [--grid] More than %u images were supplied for this %ux%u grid. The rest will be ignored.\n", gridCellCount,
            ...
            avifAppFileFormat nextInputFormat = avifInputReadImage(&input, ignoreICC, ignoreExif, ignoreXMP, cellImage, NULL, NULL, chromaDownsampling);
            if (nextInputFormat == AVIF_APP_FILE_FORMAT_UNKNOWN) {
                returnCode = 1;
                goto cleanup;
            }
            ...
            if ((image->width != cellImage->width) || (image->height != cellImage->height)) {
                fprintf(stderr, "ERROR: Image grid dimensions mismatch, [%ux%u] vs [%ux%u]: %s\n", image->width, image->height, cellImage->width,
                    cellImage->height, nextFile->filename);
                returnCode = 1;
                goto cleanup;
            }
            ...
            if (image->yuvRange != cellImage->yuvRange) {
                fprintf(stderr, "ERROR: Image grid range mismatch, [%s] vs [%s]: %s\n", (image->yuvRange == AVIF_RANGE_FULL) ? "Full" : "Limited",
                    (nextImage->yuvRange == AVIF_RANGE_FULL) ? "Full" : "Limited", nextFile->filename);
                returnCode = 1;
                goto cleanup;
            }
        }
    ...
    printf("Encoding with AV1 codec '%s' speed [%d], color QP [%d (%s) <-> %d (%s)], alpha QP [%d (%s) <-> %d (%s)], %s, %d worker thread(s), please wait...\n",
        avifCodecName(codecChoice, AVIF_CODEC_FLAG_CAN_ENCODE), speed, minQuantizer, quantizerString(minQuantizer), maxQuantizer, quantizerString(maxQuantizer),
        minQuantizerAlpha, quantizerString(minQuantizerAlpha), maxQuantizerAlpha, quantizerString(maxQuantizerAlpha),
        autoTiling ? "automatic tiling" : manualTilingStr, jobs);
    ...
    if (gridDimsCount > 0) {
        avifResult addImageResult = avifEncoderAddImageGrid(encoder, gridDims[0], gridDims[1], (const avifImage* const*)gridCells, AVIF_ADD_IMAGE_FLAG_SINGLE);
        ...
        if (input.useStdin || (input.filesCount > 1)) {
            printf(" * Encoding frame 1 [%" PRIu64 "/%" PRIu64 " ts]: %s\n", firstDurationInTimescales, outputTiming.timescale, firstFile->filename);
        }
        ...
        while ((nextFile = avifInputGetNextFile(&input)) != NULL) {
            ...
            printf(" * Encoding frame %d [%" PRIu64 "/%" PRIu64 " ts]: %s\n", nextImageIndex + 1, nextDurationInTimescales, outputTiming.timescale,
                nextFile->filename);
            ...
            avifAppFileFormat nextInputFormat = avifInputReadImage(&input, ignoreICC, ignoreExif, ignoreXMP, nextImage, NULL, NULL, chromaDownsampling);
            if (nextInputFormat == AVIF_APP_FILE_FORMAT_UNKNOWN) {
                returnCode = 1;
                goto cleanup;
            }
            ...
            if ((image->width != nextImage->width) || (image->height != nextImage->height)) {
                fprintf(stderr, "ERROR: Image sequence dimensions mismatch, [%ux%u] vs [%ux%u]: %s\n", image->width, image->height, nextImage->width,
                    nextImage->height, nextFile->filename);
                returnCode = 1;
                goto cleanup;
            }
            ...
            if ((image->colorPrimaries != nextImage->colorPrimaries) || (image->transferCharacteristics != nextImage->transferCharacteristics)
                || (image->matrixCoefficients != nextImage->matrixCoefficients)) {
                fprintf(stderr, "ERROR: Image sequence CICP mismatch, [%u/%u/%u] vs [%u/%u/%u]: %s\n", image->colorPrimaries, image->matrixCoefficients,
                    image->transferCharacteristics, nextImage->colorPrimaries, nextImage->transferCharacteristics, nextImage->matrixCoefficients,
                    nextFile->filename);
                returnCode = 1;
                goto cleanup;
            }
            if (image->yuvRange != nextImage->yuvRange) {
                fprintf(stderr, "ERROR: Image sequence range mismatch, [%s] vs [%s]: %s\n", (image->yuvRange == AVIF_RANGE_FULL) ? "Full" : "Limited",
                    (nextImage->yuvRange == AVIF_RANGE_FULL) ? "Full" : "Limited", nextFile->filename);
                returnCode = 1;
                goto cleanup;
            }
