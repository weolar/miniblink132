...
bool ReadGlyph(const uint8_t* data, size_t len, Glyph* glyph)
{
    ...
    if (!buffer.ReadS16(&glyph->x_min) || !buffer.ReadS16(&glyph->y_min) || !buffer.ReadS16(&glyph->x_max) || !buffer.ReadS16(&glyph->y_max)) {
        return FONT_COMPRESSION_FAILURE();
    }
...
bool StoreEndPtsOfContours(const Glyph& glyph, size_t* offset, uint8_t* dst)
{
    ...
    for (const auto& contour : glyph.contours) {
        ...
        if (contour.size() > std::numeric_limits<uint16_t>::max() || end_point > std::numeric_limits<uint16_t>::max()) {
            return FONT_COMPRESSION_FAILURE();
        }
...
bool StorePoints(const Glyph& glyph, size_t* offset, uint8_t* dst, size_t dst_size)
{
    int previous_flag = -1;
    int repeat_count = 0;
    int last_x = 0;
...
bool StoreGlyph(const Glyph& glyph, uint8_t* dst, size_t* dst_size)
{
    ...
    if (glyph.composite_data_size > 0) {
        // Composite glyph.
        if (*dst_size < ((10ULL + glyph.composite_data_size) + ((glyph.have_instructions ? 2ULL : 0) + glyph.instructions_size))) {
            return FONT_COMPRESSION_FAILURE();
        }
        ...
        if (*dst_size < ((12ULL + 2 * glyph.contours.size()) + glyph.instructions_size)) {
            return FONT_COMPRESSION_FAILURE();
        }
