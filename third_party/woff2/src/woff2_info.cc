...
int main(int argc, char** argv)
{
    ...
    woff2::Buffer file(reinterpret_cast<const uint8_t*>(input.data()), input.size());
    ...
    if (!file.ReadU32(&signature))
        return 1;
    if (!file.ReadU32(&flavor))
        return 1;
    if (!file.ReadU32(&length))
        return 1;
    if (!file.ReadU16(&num_tables))
        return 1;
    if (!file.ReadU16(&reserved))
        return 1;
    if (!file.ReadU32(&totalSfntSize))
        return 1;
    if (!file.ReadU32(&totalCompressedSize))
        return 1;
    if (!file.ReadU16(&major))
        return 1;
    if (!file.ReadU16(&minor))
        return 1;
    if (!file.ReadU32(&metaOffset))
        return 1;
    if (!file.ReadU32(&metaLength))
        return 1;
    if (!file.ReadU32(&metaOrigLength))
        return 1;
    if (!file.ReadU32(&privOffset))
        return 1;
    if (!file.ReadU32(&privLength))
        return 1;
    ...
    for (auto i = 0; i < num_tables; i++) {
        ...
        if (!ReadBase128(&file, &origLength))
            return 1;

        printf("%5d %6zu  0x%02x %s %10d", i, offset, flags, PrintTag(tag).c_str(), origLength);

        uint8_t xform_version = (flags >> 6) & 0x3;
        if (tag == woff2::kGlyfTableTag || tag == woff2::kLocaTableTag) {
            if (xform_version == 0) {
                if (!ReadBase128(&file, &transformLength))
                    return 1;
    ...
    if (flavor == woff2::kTtcFontFlavor) {
        ...
        for (auto i = 0; i < numFonts; i++) {
            ...
            for (auto j = 0; j < numTables; j++) {
                uint32_t table_idx;
                if (!woff2::Read255UShort(&file, &table_idx))
                    return 1;
                if (table_idx >= table_tags.size())
                    return 1;
                printf("  %d %s (idx %d)\n", j, PrintTag(table_tags[table_idx]).c_str(), table_idx);
            }
        }
    }
