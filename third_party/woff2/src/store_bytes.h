...
inline size_t Store16(uint8_t* dst, size_t offset, int x)
{
    ...
    *reinterpret_cast<uint16_t*>(dst + offset) = ((x & 0xFF) << 8) | ((x & 0xFF00) >> 8);
...
{
#if defined(WOFF_LITTLE_ENDIAN)
    *reinterpret_cast<uint16_t*>(dst + *offset) = ((val & 0xFF) << 8) | ((val & 0xFF00) >> 8);
    *offset += 2;
#elif defined(WOFF_BIG_ENDIAN)
...
inline void StoreBytes(const uint8_t* data, size_t len, size_t* offset, uint8_t* dst)
{
