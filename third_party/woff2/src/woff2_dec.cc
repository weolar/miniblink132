...
{
    if (PREDICT_FALSE(((a > 0) && (b > std::numeric_limits<int>::max() - a)) || ((a < 0) && (b < std::numeric_limits<int>::min() - a)))) {
        return false;
    }
    *result = a + b;
    return true;
...
bool TripletDecode(const uint8_t* flags_in, const uint8_t* in, size_t in_size, unsigned int n_points, Point* result, size_t* in_bytes_consumed)
{
    int x = 0;
    int y = 0;
    ...
    for (unsigned int i = 0; i < n_points; ++i) {
        ...
        if (flag < 10) {
            ...
            dy = WithSign(flag >> 1, 1 + (((b0 % 12) >> 2) << 8) + in[triplet_index + 1]);
        } else if (flag < 124) {
            ...
            dy = WithSign(flag >> 1, (in[triplet_index + 2] << 8) + in[triplet_index + 3]);
        }
...
bool StorePoints(unsigned int n_points, const Point* points, unsigned int n_contours, unsigned int instruction_length, bool has_overlap_bit, uint8_t* dst,
    size_t dst_size, size_t* glyph_size)
{
    ...
    if (PREDICT_FALSE(xy_bytes < x_bytes || flag_offset + xy_bytes < flag_offset || flag_offset + xy_bytes > dst_size)) {
        return FONT_COMPRESSION_FAILURE();
    }
...
bool SizeOfComposite(Buffer composite_stream, size_t* size, bool* have_instructions)
{
...
bool StoreLoca(const std::vector<uint32_t>& loca_values, int index_format, uint32_t* checksum, WOFF2Out* out)
{
...
bool ReconstructGlyf(
    const uint8_t* data, Table* glyf_table, uint32_t* glyf_checksum, Table* loca_table, uint32_t* loca_checksum, WOFF2FontInfo* info, WOFF2Out* out)
{
    ...
    if (PREDICT_FALSE(!file.ReadU16(&info->num_glyphs) || !file.ReadU16(&info->index_format))) {
        return FONT_COMPRESSION_FAILURE();
    }

    // https://dev.w3.org/webfonts/WOFF2/spec/#conform-mustRejectLoca
    // dst_length here is origLength in the spec
    uint32_t expected_loca_dst_length = (info->index_format ? 4 : 2) * (static_cast<uint32_t>(info->num_glyphs) + 1);
    ...
    if (has_overlap_bitmap) {
        ...
        if (PREDICT_FALSE(overlap_bitmap_length > glyf_table->transform_length - offset)) {
            return FONT_COMPRESSION_FAILURE();
        }
    }
    ...
    for (unsigned int i = 0; i < info->num_glyphs; ++i) {
        ...
        if (n_contours == 0xffff) {
            ...
            if (PREDICT_FALSE(!SizeOfComposite(composite_stream, &composite_size, &have_instructions))) {
                return FONT_COMPRESSION_FAILURE();
            }
            ...
            if (PREDICT_FALSE(!composite_stream.Read(glyph_buf.get() + glyph_size, composite_size))) {
                return FONT_COMPRESSION_FAILURE();
            }
            glyph_size += composite_size;
            if (have_instructions) {
                glyph_size = Store16(glyph_buf.get(), glyph_size, instruction_size);
                if (PREDICT_FALSE(!instruction_stream.Read(glyph_buf.get() + glyph_size, instruction_size))) {
                    return FONT_COMPRESSION_FAILURE();
                }
            ...
            if (PREDICT_FALSE(flag_size > flag_stream.length() - flag_stream.offset())) {
                return FONT_COMPRESSION_FAILURE();
            }
            ...
            if (PREDICT_FALSE(!TripletDecode(flags_buf, triplet_buf, triplet_size, total_n_points, points.get(), &triplet_bytes_consumed))) {
                return FONT_COMPRESSION_FAILURE();
            }
            ...
            size_t size_needed = 12 + 2 * n_contours + 5 * total_n_points + instruction_size;
            if (PREDICT_FALSE(glyph_buf_size < size_needed)) {
                glyph_buf.reset(new uint8_t[size_needed]);
                glyph_buf_size = size_needed;
            }
            ...
            bool has_overlap_bit = has_overlap_bitmap && overlap_bitmap[i >> 3] & (0x80 >> (i & 7));

            if (PREDICT_FALSE(
                    !StorePoints(total_n_points, points.get(), n_contours, instruction_size, has_overlap_bit, glyph_buf.get(), glyph_buf_size, &glyph_size))) {
                return FONT_COMPRESSION_FAILURE();
            }
        } else {
    ...
    if (PREDICT_FALSE(!StoreLoca(loca_values, info->index_format, loca_checksum, out))) {
        return FONT_COMPRESSION_FAILURE();
    }
...
bool ReadNumHMetrics(const uint8_t* data, size_t data_size, uint16_t* num_hmetrics)
{
...
bool ReconstructTransformedHmtx(const uint8_t* transformed_buf, size_t transformed_size, uint16_t num_glyphs, uint16_t num_hmetrics,
    const std::vector<int16_t>& x_mins, uint32_t* checksum, WOFF2Out* out)
{
...
bool Woff2Uncompress(uint8_t* dst_buf, size_t dst_size, const uint8_t* src_buf, size_t src_size)
{
    size_t uncompressed_size = dst_size;
    BrotliDecoderResult result = BrotliDecoderDecompress(src_size, src_buf, &uncompressed_size, dst_buf);
    if (PREDICT_FALSE(result != BROTLI_DECODER_RESULT_SUCCESS || uncompressed_size != dst_size)) {
        return FONT_COMPRESSION_FAILURE();
    }
    return true;
}

bool ReadTableDirectory(Buffer* file, std::vector<Table>* tables, size_t num_tables)
{
...
size_t StoreOffsetTable(uint8_t* result, size_t offset, uint32_t flavor, uint16_t num_tables)
{
...
{
    uint64_t offset = kSfntHeaderSize + kSfntEntrySize * static_cast<uint64_t>(hdr.num_tables);
    if (hdr.header_version) {
        offset = CollectionHeaderSize(hdr.header_version, hdr.ttc_fonts.size()) + kSfntHeaderSize * hdr.ttc_fonts.size();
...
bool ReconstructFont(
    uint8_t* transformed_buf, const uint32_t transformed_buf_size, RebuildMetadata* metadata, WOFF2Header* hdr, size_t font_index, WOFF2Out* out)
{
    ...
    if (PREDICT_FALSE(static_cast<bool>(glyf_table) != static_cast<bool>(loca_table))) {
#ifdef FONT_COMPRESSION_BIN
    ...
    if (glyf_table != NULL) {
        if (PREDICT_FALSE((glyf_table->flags & kWoff2FlagsTransform) != (loca_table->flags & kWoff2FlagsTransform))) {
#ifdef FONT_COMPRESSION_BIN
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        ...
        bool reused = metadata->checksums.find(checksum_key) != metadata->checksums.end();
        ...
        if (PREDICT_FALSE(static_cast<uint64_t>(table.src_offset) + table.src_length > transformed_buf_size)) {
            return FONT_COMPRESSION_FAILURE();
        }

        if (table.tag == kHheaTableTag) {
            if (!ReadNumHMetrics(transformed_buf + table.src_offset, table.src_length, &info->num_hmetrics)) {
                return FONT_COMPRESSION_FAILURE();
            }
        }
        ...
        if (!reused) {
            if ((table.flags & kWoff2FlagsTransform) != kWoff2FlagsTransform) {
                ...
                checksum = ComputeULongSum(transformed_buf + table.src_offset, table.src_length);
                if (PREDICT_FALSE(!out->Write(transformed_buf + table.src_offset, table.src_length))) {
                    return FONT_COMPRESSION_FAILURE();
                }
            } else {
                if (table.tag == kGlyfTableTag) {
                    ...
                    if (PREDICT_FALSE(!ReconstructGlyf(transformed_buf + table.src_offset, &table, &checksum, loca_table, &loca_checksum, info, out))) {
                        return FONT_COMPRESSION_FAILURE();
                    }
                    ...
                    if (PREDICT_FALSE(!ReconstructTransformedHmtx(
                            transformed_buf + table.src_offset, table.src_length, info->num_glyphs, info->num_hmetrics, info->x_mins, &checksum, out))) {
                        return FONT_COMPRESSION_FAILURE();
                    }
                } else {
        ...
        if (PREDICT_FALSE(!out->Write(table_entry, info->table_entry_by_tag[table.tag] + 4, 12))) {
            return FONT_COMPRESSION_FAILURE();
        }
        ...
        if (PREDICT_FALSE(static_cast<uint64_t>(table.dst_offset + table.dst_length) > out->Size())) {
            return FONT_COMPRESSION_FAILURE();
        }
    ...
    if (head_table) {
        ...
        if (PREDICT_FALSE(!out->Write(checksum_adjustment, head_table->dst_offset + 8, 4))) {
            return FONT_COMPRESSION_FAILURE();
        }
    }
...
bool ReadWOFF2Header(const uint8_t* data, size_t length, WOFF2Header* hdr)
{
    ...
    uint32_t signature;
    if (PREDICT_FALSE(!file.ReadU32(&signature) || signature != kWoff2Signature || !file.ReadU32(&hdr->flavor))) {
        return FONT_COMPRESSION_FAILURE();
    }
    ...
    if (PREDICT_FALSE(!file.ReadU32(&reported_length) || length != reported_length)) {
        return FONT_COMPRESSION_FAILURE();
    }
    ...
    if (PREDICT_FALSE(!file.ReadU32(&meta_offset) || !file.ReadU32(&meta_length) || !file.ReadU32(&meta_length_orig))) {
        return FONT_COMPRESSION_FAILURE();
    }
    if (meta_offset) {
        if (PREDICT_FALSE(meta_offset >= length || length - meta_offset < meta_length)) {
            return FONT_COMPRESSION_FAILURE();
        }
    }
    uint32_t priv_offset;
    uint32_t priv_length;
    if (PREDICT_FALSE(!file.ReadU32(&priv_offset) || !file.ReadU32(&priv_length))) {
        return FONT_COMPRESSION_FAILURE();
    }
    if (priv_offset) {
        if (PREDICT_FALSE(priv_offset >= length || length - priv_offset < priv_length)) {
            return FONT_COMPRESSION_FAILURE();
        }
    }
    hdr->tables.resize(hdr->num_tables);
    if (PREDICT_FALSE(!ReadTableDirectory(&file, &hdr->tables, hdr->num_tables))) {
        return FONT_COMPRESSION_FAILURE();
    }
    ...
    if (hdr->flavor == kTtcFontFlavor) {
        ...
        for (uint32_t i = 0; i < num_fonts; i++) {
            ...
            for (uint32_t j = 0; j < num_tables; j++) {
                ...
                if (PREDICT_FALSE(!Read255UShort(&file, &table_idx)) || table_idx >= hdr->tables.size()) {
                    return FONT_COMPRESSION_FAILURE();
                }
    ...
    if (PREDICT_FALSE(hdr->compressed_offset > std::numeric_limits<uint32_t>::max())) {
        return FONT_COMPRESSION_FAILURE();
    }
...
#ifdef FONT_COMPRESSION_BIN
        fprintf(stderr,
            "offset fail; src_offset %" PRIu64 " length %lu "
            "dst_offset %" PRIu64 "\n",
...
bool WriteHeaders(const uint8_t* data, size_t length, RebuildMetadata* metadata, WOFF2Header* hdr, WOFF2Out* out)
{
    ...
    if (hdr->header_version) {
        ...
        for (size_t i = 0; i < hdr->ttc_fonts.size(); i++) {
            ...
            offset = StoreOffsetTable(result, offset, ttc_font.flavor, ttc_font.table_indices.size());
...
bool ConvertWOFF2ToTTF(const uint8_t* data, size_t length, WOFF2Out* out)
{
    ...
    if (PREDICT_FALSE(!Woff2Uncompress(&uncompressed_buf[0], hdr.uncompressed_size, src_buf, hdr.compressed_length))) {
        return FONT_COMPRESSION_FAILURE();
    }

    for (size_t i = 0; i < metadata.font_infos.size(); i++) {
        if (PREDICT_FALSE(!ReconstructFont(&uncompressed_buf[0], hdr.uncompressed_size, &metadata, &hdr, i, out))) {
            return FONT_COMPRESSION_FAILURE();
        }
    }
