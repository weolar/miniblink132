...
std::vector<uint32_t> Font::OutputOrderedTags() const
{
    ...
    auto glyf_loc = std::find(output_order.begin(), output_order.end(), kGlyfTableTag);
    auto loca_loc = std::find(output_order.begin(), output_order.end(), kLocaTableTag);
    if (glyf_loc != output_order.end() && loca_loc != output_order.end()) {
        output_order.erase(loca_loc);
        output_order.insert(std::find(output_order.begin(), output_order.end(), kGlyfTableTag) + 1, kLocaTableTag);
    }
...
bool ReadTrueTypeFont(Buffer* file, const uint8_t* data, size_t len, Font* font)
{
    ...
    if (!file->ReadU16(&font->num_tables) || !file->Skip(6)) {
    ...
    for (uint16_t i = 0; i < font->num_tables; ++i) {
        ...
        table.flag_byte = 0;
        table.reuse_of = NULL;
        if (!file->ReadU32(&table.tag) || !file->ReadU32(&table.checksum) || !file->ReadU32(&table.offset) || !file->ReadU32(&table.length)) {
            return FONT_COMPRESSION_FAILURE();
        }
...
bool ReadTrueTypeCollection(Buffer* file, const uint8_t* data, size_t len, FontCollection* font_collection)
{
    ...
    if (!file->ReadU32(&font_collection->header_version) || !file->ReadU32(&num_fonts)) {
        return FONT_COMPRESSION_FAILURE();
    }
...
bool WriteTable(const Font::Table& table, size_t* offset, uint8_t* dst, size_t dst_size)
{
    ...
    if (!table.IsReused()) {
        if (table.offset + table.length < table.offset || dst_size < table.offset + table.length) {
            return FONT_COMPRESSION_FAILURE();
        }
        memcpy(dst + table.offset, table.data, table.length);
        size_t padding_size = (4 - (table.length & 3)) & 3;
        if (table.offset + table.length + padding_size < padding_size || dst_size < table.offset + table.length + padding_size) {
            return FONT_COMPRESSION_FAILURE();
        }
...
bool GetGlyphData(const Font& font, int glyph_index, const uint8_t** glyph_data, size_t* glyph_size)
{
    ...
    if (head_table == NULL || loca_table == NULL || glyf_table == NULL || head_table->length < 52) {
        return FONT_COMPRESSION_FAILURE();
    }
    ...
    if (index_fmt == 0) {
        ...
        if (!loca_buf.Skip(2 * glyph_index) || !loca_buf.ReadU16(&offset1) || !loca_buf.ReadU16(&offset2) || offset2 < offset1
        ...
        *glyph_size = 2 * (offset2 - offset1);
        ...
        if (!loca_buf.Skip(4 * glyph_index) || !loca_buf.ReadU32(&offset1) || !loca_buf.ReadU32(&offset2) || offset2 < offset1
...
{
    std::map<uint32_t, Font::Table>::iterator it = font->tables.find(kDsigTableTag);
