...
bool Compress(const uint8_t* data, const size_t len, uint8_t* result, uint32_t* result_len, BrotliEncoderMode mode, int quality)
{
    size_t compressed_len = *result_len;
    if (BrotliEncoderCompress(quality, BROTLI_DEFAULT_WINDOW, mode, len, data, &compressed_len, result) == 0) {
        return false;
    }
...
bool Woff2Compress(const uint8_t* data, const size_t len, uint8_t* result, uint32_t* result_len, int quality)
{
    return Compress(data, len, result, result_len, BROTLI_MODE_FONT, quality);
}

bool TextCompress(const uint8_t* data, const size_t len, uint8_t* result, uint32_t* result_len, int quality)
{
    return Compress(data, len, result, result_len, BROTLI_MODE_TEXT, quality);
}
...
size_t ComputeWoff2Length(const FontCollection& font_collection, const std::vector<Table>& tables,
    std::map<std::pair<uint32_t, uint32_t>, uint16_t> index_by_tag_offset, size_t compressed_data_length, size_t extended_metadata_length)
{
...
size_t ComputeUncompressedLength(const Font& font)
{
    ...
    for (const auto& entry : font.tables) {
        const Font::Table& table = entry.second;
        if (table.tag & 0x80808080)
            continue; // xform tables don't stay
        if (table.IsReused())
            continue; // don't have to pay twice
...
size_t ComputeUncompressedLength(const FontCollection& font_collection)
{
    ...
    size_t size = CollectionHeaderSize(font_collection.header_version, font_collection.fonts.size());
    for (const auto& font : font_collection.fonts) {
...
bool ConvertTTFToWOFF2(const uint8_t* data, size_t length, uint8_t* result, size_t* result_length, const WOFF2Params& params)
{
    ...
    for (const auto& font : font_collection.fonts) {
        for (const auto tag : font.OutputOrderedTags()) {
            ...
            if (original.IsReused())
                continue;
            if (tag & 0x80808080)
                continue;
            const Font::Table* table_to_store = font.FindTable(tag ^ 0x80808080);
            if (table_to_store == NULL)
                table_to_store = &original;

            StoreBytes(table_to_store->data, table_to_store->length, &transform_offset, &transform_buf[0]);
        }
    }

    // Compress all transformed data in one stream.
    if (!Woff2Compress(transform_buf.data(), total_transform_length, &compression_buf[0], &total_compressed_length, params.brotli_quality)) {
#ifdef FONT_COMPRESSION_BIN
...
#ifdef FONT_COMPRESSION_BIN
    fprintf(stderr, "Compressed %zu to %u.\n", total_transform_length, total_compressed_length);
#endif

    // Compress the extended metadata
    // TODO(user): how does this apply to collections
    uint32_t compressed_metadata_buf_length = CompressedBufferSize(params.extended_metadata.length());
    std::vector<uint8_t> compressed_metadata_buf(compressed_metadata_buf_length);

    if (params.extended_metadata.length() > 0) {
        if (!TextCompress((const uint8_t*)params.extended_metadata.data(), params.extended_metadata.length(), compressed_metadata_buf.data(),
                &compressed_metadata_buf_length, params.brotli_quality)) {
#ifdef FONT_COMPRESSION_BIN
    ...
    for (const auto& font : font_collection.fonts) {
        ...
        for (const auto tag : font.OutputOrderedTags()) {
            ...
            const Font::Table* transformed_table = font.FindTable(src_table.tag ^ 0x80808080);
    ...
    size_t woff2_length = ComputeWoff2Length(font_collection, tables, index_by_tag_offset, total_compressed_length, compressed_metadata_buf_length);
    ...
    *result_length = woff2_length;
    ...
    if (compressed_metadata_buf_length > 0) {
        StoreU32(woff2_length - compressed_metadata_buf_length, &offset, result); // metaOffset
        StoreU32(compressed_metadata_buf_length, &offset, result); // metaLength
        StoreU32(params.extended_metadata.length(), &offset, result); // metaOrigLength
    } else {
    ...
    if (font_collection.flavor == kTtcFontFlavor) {
        ...
        for (const Font& font : font_collection.fonts) {
            ...
            for (const auto& entry : font.tables) {
                const Font::Table& table = entry.second;
                if (table.tag & 0x80808080)
                    continue; // don't write xform tables
                num_tables++;
            }
            ...
            for (const auto& entry : font.tables) {
                ...
                uint32_t table_offset = table.IsReused() ? table.reuse_of->offset : table.offset;
                uint32_t table_length = table.IsReused() ? table.reuse_of->length : table.length;
    ...
    StoreBytes(compressed_metadata_buf.data(), compressed_metadata_buf_length, &offset, result);
