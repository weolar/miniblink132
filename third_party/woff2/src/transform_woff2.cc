...
public:
    ...
    void GetTransformedGlyfBytes(std::vector<uint8_t>* result)
    {
        ...
        WriteUShort(result, overlap_bitmap_.empty() ? 0x00 : FLAG_OVERLAP_SIMPLE_BITMAP); // Flags
...
private:
    void WriteInstructions(const Glyph& glyph)
    {
        ...
        WriteBytes(&instruction_stream_, glyph.instructions_data, glyph.instructions_size);
    }
    ...
    void WriteCompositeGlyph(int glyph_id, const Glyph& glyph)
    {
        ...
        WriteBbox(glyph_id, glyph);
        WriteBytes(&composite_stream_, glyph.composite_data, glyph.composite_data_size);
    ...
    void WriteTriplet(bool on_curve, int x, int y)
    {
        ...
        if (x == 0 && abs_y < 1280) {
            flag_byte_stream_.push_back(on_curve_bit + ((abs_y & 0xf00) >> 7) + y_sign_bit);
            glyph_stream_.push_back(abs_y & 0xff);
        } else if (y == 0 && abs_x < 1280) {
            flag_byte_stream_.push_back(on_curve_bit + 10 + ((abs_x & 0xf00) >> 7) + x_sign_bit);
            glyph_stream_.push_back(abs_x & 0xff);
        } else if (abs_x < 65 && abs_y < 65) {
            flag_byte_stream_.push_back(on_curve_bit + 20 + ((abs_x - 1) & 0x30) + (((abs_y - 1) & 0x30) >> 2) + xy_sign_bits);
            glyph_stream_.push_back((((abs_x - 1) & 0xf) << 4) | ((abs_y - 1) & 0xf));
        } else if (abs_x < 769 && abs_y < 769) {
            flag_byte_stream_.push_back(on_curve_bit + 84 + 12 * (((abs_x - 1) & 0x300) >> 8) + (((abs_y - 1) & 0x300) >> 6) + xy_sign_bits);
...
bool TransformGlyfAndLocaTables(Font* font)
{
    ...
    for (int i = 0; i < num_glyphs; ++i) {
        ...
        size_t glyph_size;
        if (!GetGlyphData(*font, i, &glyph_data, &glyph_size) || (glyph_size > 0 && !ReadGlyph(glyph_data, glyph_size, &glyph))) {
            return FONT_COMPRESSION_FAILURE();
        }
