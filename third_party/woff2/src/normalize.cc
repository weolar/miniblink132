...
bool WriteNormalizedLoca(int index_fmt, int num_glyphs, Font* font)
{
    ...
    for (int i = 0; i < num_glyphs; ++i) {
        ...
        if (glyf_dst_size > std::numeric_limits<uint32_t>::max() || glyf_offset + static_cast<uint32_t>(glyf_dst_size) < glyf_offset
...
{
    return (
        MakeEditableBuffer(font, kHeadTableTag) && RemoveDigitalSignature(font) && MarkTransformed(font) && NormalizeGlyphs(font) && NormalizeOffsets(font));
}
...
bool NormalizeFontCollection(FontCollection* font_collection)
{
    ...
    uint32_t offset = CollectionHeaderSize(font_collection->header_version, font_collection->fonts.size());
