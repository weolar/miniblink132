...
struct PERFETTO_EXPORT_COMPONENT Category {
    ...
    template <typename... Args> constexpr Category SetTags(Args&&... args) const
    {
        return Category(name, description, { std::forward<Args>(args)... }, name_sizes_);
    }
    ...
    template <typename T> void ForEachGroupMember(T callback) const
    {
...
private:
    ...
    constexpr Category(const char* name_, const char* description_, Tags tags_, NameSizes name_sizes)
        : name(name_)
    ...
    static constexpr NameSizes ComputeNameSizes(const char* s)
    {
        ...
        return NameSizes { { static_cast<uint8_t>(GetNthNameSize(0, s, s)), static_cast<uint8_t>(GetNthNameSize(1, s, s)),
            static_cast<uint8_t>(GetNthNameSize(2, s, s)), static_cast<uint8_t>(GetNthNameSize(3, s, s)) } };
    }

    static constexpr ptrdiff_t GetNthNameSize(int n, const char* start, const char* end, int counter = 0)
    {
        return (!*end || *end == ',') ? ((!*end || counter == n) ? (counter == n ? end - start : 0) : GetNthNameSize(n, end + 1, end + 1, counter + 1))
    ...
    static constexpr const char* CheckIsValidCategoryGroup(const char* n)
    {
        ...
        return !GetNthNameSize(1, n, n) || GetNthNameSize(kMaxGroupSize, n, n) ? nullptr : n;
    }
...
public:
    explicit DynamicCategory(const std::string& name_)
        : name(name_)
    {
    }
    explicit DynamicCategory(const char* name_)
        : name(name_)
    {
    }
    DynamicCategory()
...
{
    return !*str ? !*prefix : !*prefix ? true : *str != *prefix ? false : StringMatchesPrefix(str + 1, prefix + 1);
}
...
template <typename... Args> constexpr bool IsStringInPrefixList(const char* str, const char* prefix, Args... args)
{
    return StringMatchesPrefix(str, prefix) || IsStringInPrefixList(str, std::forward<Args>(args)...);
}
...
public:
    constexpr TrackEventCategoryRegistry(size_t category_count, const Category* categories, std::atomic<uint8_t>* state_storage)
        : categories_(categories)
        , category_count_(category_count)
        , state_storage_(state_storage)
    {
        static_assert(sizeof(state_storage[0].load()) * 8 >= kMaxDataSourceInstances,
    ...
    // instance.
    void EnableCategoryForInstance(size_t category_index, uint32_t instance_index) const;
    void DisableCategoryForInstance(size_t category_index, uint32_t instance_index) const;

    constexpr std::atomic<uint8_t>* GetCategoryState(size_t category_index) const
    {
    ...
    {
        return (index == category_count_) ? true : IsValidCategoryName(categories_[index].name) ? ValidateCategories(index + 1) : false;
    }

private:
    // TODO(skyostil): Make the compile-time routines nicer with C++14.
    constexpr size_t FindImpl(const char* name, bool is_dynamic, size_t index = 0) const
    {
        return is_dynamic                             ? kDynamicCategoryIndex
            : (index == category_count_)              ? kInvalidCategoryIndex
            : StringEq(categories_[index].name, name) ? index
    ...
    static constexpr size_t CheckIsValidCategoryIndex(size_t index)
    {
        ...
        // clang-format on
        return index != kInvalidCategoryIndex ? index : /* Invalid category -- add it to PERFETTO_DEFINE_CATEGORIES(). */ [] {
    ...
    {
        return (!name || *name == '\"' || *name == '*' || *name == ' ') ? false : *name ? IsValidCategoryName(name + 1) : true;
    }
