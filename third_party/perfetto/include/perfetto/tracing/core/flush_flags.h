...
public:
    ...
    explicit FlushFlags(uint64_t flags = 0)
        : flags_(flags)
    {
    }
    FlushFlags(Initiator i, Reason r, CloneTarget c = CloneTarget::kUnknown)
        : flags_((static_cast<uint64_t>(i) << kInitiatorShift) | (static_cast<uint64_t>(r) << kReasonShift) | (static_cast<uint64_t>(c) << kCloneTargetShift))
    {
    }

    bool operator==(const FlushFlags& o) const
    {
    ...
    Initiator initiator() const
    {
        ...
        static_assert(uint64_t(Initiator::kMax) - 1 <= (kInitiatorMask >> kInitiatorShift), "enum out of range");
        const uint64_t value = (flags_ & kInitiatorMask) >> kInitiatorShift;
        return value < uint64_t(Initiator::kMax) ? Initiator(value) : Initiator::kUnknown;
    }

    Reason reason() const
    {
        static_assert(uint64_t(Reason::kMax) - 1 <= (kReasonMask >> kReasonShift), "enum out of range");
    ...
    {
        static_assert(uint64_t(CloneTarget::kMax) - 1 <= (kCloneTargetMask >> kCloneTargetShift), "enum out of range");
        const uint64_t value = (flags_ & kCloneTargetMask) >> kCloneTargetShift;
        return value < uint64_t(CloneTarget::kMax) ? CloneTarget(value) : CloneTarget::kUnknown;
    }
