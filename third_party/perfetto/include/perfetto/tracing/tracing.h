...
public:
    ...
    static inline void Initialize(const TracingInitArgs& args) PERFETTO_ALWAYS_INLINE
    {
        ...
        if (args.backends & kInProcessBackend) {
            args_copy.in_process_backend_factory_ = &internal::InProcessTracingBackend::GetInstance;
        }
        if (args.backends & kSystemBackend) {
            args_copy.system_producer_backend_factory_ = &internal::SystemProducerTracingBackend::GetInstance;
            if (args.enable_system_consumer) {
                args_copy.system_consumer_backend_factory_ = &internal::SystemConsumerTracingBackend::GetInstance;
            }
        }
    ...
    static PERFETTO_ALWAYS_INLINE inline std::unique_ptr<TracingSession> NewTrace(BackendType backend = kUnspecifiedBackend);
    ...
    static std::unique_ptr<StartupTracingSession> SetupStartupTracing(const TraceConfig& config, SetupStartupTracingOpts);
    ...
    static std::unique_ptr<StartupTracingSession> SetupStartupTracingBlocking(const TraceConfig& config, SetupStartupTracingOpts);
    ...
    static void ActivateTriggers(const std::vector<std::string>& triggers, uint32_t ttl_ms);

private:
    static void InitializeInternal(const TracingInitArgs&);
    static std::unique_ptr<TracingSession> NewTraceInternal(BackendType, TracingConsumerBackend* (*system_backend_factory)());
...
public:
    ...
    using QueryServiceStateCallback = std::function<void(QueryServiceStateCallbackArgs)>;
...
PERFETTO_ALWAYS_INLINE inline std::unique_ptr<TracingSession> Tracing::NewTrace(BackendType backend)
{
    ...
    if (backend & kSystemBackend) {
        system_backend_factory = &internal::SystemConsumerTracingBackend::GetInstance;
    }
#endif
