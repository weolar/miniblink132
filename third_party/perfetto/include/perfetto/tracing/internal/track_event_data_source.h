...
template <typename T> struct TraceTimestampTraits;

// A pass-through implementation for raw uint64_t nanosecond timestamps.
template <> struct TraceTimestampTraits<uint64_t> {
    static inline TraceTimestamp ConvertTimestampToTraceTimeNs(const uint64_t& timestamp)
    {
...
template <> struct TraceTimestampTraits<TraceTimestamp> {
    static inline TraceTimestamp ConvertTimestampToTraceTimeNs(const TraceTimestamp& timestamp)
    {
        return timestamp;
    }
};
...
template <typename T> static constexpr bool IsValidTrack()
{
    return std::is_convertible<T, Track>::value;
}

// Checks if |T| is a valid non-counter track.
template <typename T> static constexpr bool IsValidNormalTrack()
{
    return std::is_convertible<T, Track>::value && !std::is_convertible<T, CounterTrack>::value;
}
...
template <typename T,
    ...
    = decltype(::perfetto::TraceTimestampTraits<typename base::remove_cvref_t<T>>::ConvertTimestampToTraceTimeNs(std::declval<T>())),
    typename NotTrackCheck = typename std::enable_if<!IsValidNormalTrack<T>()>::type,
    typename NotLambdaCheck = typename std::enable_if<!IsValidTraceLambda<T>()>::type>
...
template <typename T, typename = void> struct IsValidEventNameType : std::is_same<perfetto::DynamicString, typename std::decay<T>::type> { };

template <typename T> struct IsValidEventNameType<T, void_t<decltype(GetStaticString(std::declval<T>()))>> : std::true_type { };

template <typename T> inline void ValidateEventNameType()
{
    static_assert(IsValidEventNameType<T>::value,
...
inline bool UnorderedEqual(std::vector<std::string> vec1, std::vector<std::string> vec2)
{
...
inline ::perfetto::DynamicString DecayEventNameType(::perfetto::DynamicString name)
{
    return name;
}

inline ::perfetto::StaticString DecayEventNameType(::perfetto::StaticString name)
{
    return name;
}
...
template <typename CategoryType> struct CategoryTraits {
    static constexpr bool kIsDynamic = true;
    static constexpr const Category* GetStaticCategory(const TrackEventCategoryRegistry*, const CategoryType&)
    {
        return nullptr;
    }
...
template <> struct CategoryTraits<size_t> {
    static constexpr bool kIsDynamic = false;
    static const Category* GetStaticCategory(const TrackEventCategoryRegistry* registry, size_t category_index)
    {
...
struct TrackEventDataSourceTraits : public perfetto::DefaultDataSourceTraits {
    ...
    static DataSourceThreadLocalState* GetDataSourceTLS(DataSourceStaticState*, TracingTLS* root_tls)
    {
...
// category namespace.
template <typename DerivedDataSource, const TrackEventCategoryRegistry* Registry>
class TrackEventDataSource : public DataSource<DerivedDataSource, TrackEventDataSourceTraits> {
...
public:
    ...
    void OnStop(const DataSourceBase::StopArgs& args) override
    {
        ...
        inner_stop_args.async_stop_closure = [internal_instance_index, outer_stop_closure] {
    ...
    void WillClearIncrementalState(const DataSourceBase::ClearIncrementalStateArgs& args) override
    {
    ...
    bool CanAdoptStartupSession(const DataSourceConfig& startup_config, const DataSourceConfig& service_config) override
    {
        if (startup_config.track_event_config_raw().empty() || service_config.track_event_config_raw().empty()) {
            return false;
        }
        ...
        if (!UnorderedEqual(startup_te_cfg.enabled_categories(), service_te_cfg.enabled_categories())) {
            return false;
        }
        if (!UnorderedEqual(startup_te_cfg.disabled_categories(), service_te_cfg.disabled_categories())) {
            return false;
        }
        if (!UnorderedEqual(startup_te_cfg.enabled_tags(), service_te_cfg.enabled_tags())) {
            return false;
        }
        if (!UnorderedEqual(startup_te_cfg.disabled_tags(), service_te_cfg.disabled_tags())) {
            return false;
        }
        if (startup_te_cfg.filter_debug_annotations() != service_te_cfg.filter_debug_annotations()) {
            return false;
        }
        if (startup_te_cfg.filter_dynamic_event_names() != service_te_cfg.filter_dynamic_event_names()) {
            return false;
        }
    ...
    {
        return Registry->GetCategoryState(category_index)->load(std::memory_order_relaxed);
    }

    // Determine if tracing for the given dynamic category is enabled.
    static bool IsDynamicCategoryEnabled(const DynamicCategory& dynamic_category)
    {
        bool enabled = false;
        Base::Trace([&](typename Base::TraceContext ctx) { enabled = enabled || IsDynamicCategoryEnabled(&ctx, dynamic_category); });
        return enabled;
    }
    ...
    // function.
    template <typename Callback> static void CallIfCategoryEnabled(size_t category_index, Callback callback) PERFETTO_ALWAYS_INLINE
    {
        Base::template CallIfEnabled<CategoryTracePointTraits>([&callback](uint32_t instances) { callback(instances); }, { category_index });
    }
    ...
    static void TraceForCategory(uint32_t instances, const CategoryType& category, const EventNameType& name, perfetto::protos::pbzero::TrackEvent::Type type,
        Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        TraceForCategoryBody(instances, DecayStrType(category), DecayStrType(name), type, DecayArgType(args)...);
    }

#if PERFETTO_ENABLE_LEGACY_TRACE_EVENTS
    template <typename TrackType, typename CategoryType, typename EventNameType, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type>
    static void TraceForCategoryLegacy(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        TraceForCategoryLegacyBody(instances, DecayStrType(category), DecayStrType(event_name), type, track, phase, flags, DecayArgType(args)...);
    }

    template <typename TrackType, typename CategoryType, typename EventNameType, typename TimestampType = uint64_t, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type>
    static void TraceForCategoryLegacy(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, TimestampType&& timestamp,
        Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        TraceForCategoryLegacyBody(instances, DecayStrType(category), DecayStrType(event_name), type, track, phase, flags, timestamp, DecayArgType(args)...);
    }

    template <typename TrackType, typename CategoryType, typename EventNameType, typename ThreadIdType, typename LegacyIdType, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type>
    static void TraceForCategoryLegacyWithId(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, ThreadIdType thread_id, LegacyIdType legacy_id,
        Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        TraceForCategoryLegacyWithIdBody(
            instances, DecayStrType(category), DecayStrType(event_name), type, track, phase, flags, thread_id, legacy_id, DecayArgType(args)...);
    }

    template <typename TrackType, typename CategoryType, typename EventNameType, typename ThreadIdType, typename LegacyIdType,
        typename TimestampType = uint64_t, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type>
    static void TraceForCategoryLegacyWithId(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, ThreadIdType thread_id, LegacyIdType legacy_id,
        TimestampType&& timestamp, Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        TraceForCategoryLegacyWithIdBody(
            instances, DecayStrType(category), DecayStrType(event_name), type, track, phase, flags, thread_id, legacy_id, timestamp, DecayArgType(args)...);
    }
#endif
    ...
    static bool Register()
    {
        ...
        return TrackEventInternal::Initialize(*Registry, [](const DataSourceDescriptor& dsd) { return Base::Register(dsd); });
    }

    // Record metadata about different types of timeline tracks. See Track.
    static void SetTrackDescriptor(const Track& track, const protos::gen::TrackDescriptor& desc)
    {
        PERFETTO_DCHECK(track.uuid == desc.uuid());
        TrackRegistry::Get()->UpdateTrack(track, desc.SerializeAsString());
        Base::Trace([&](typename Base::TraceContext ctx) {
            TrackEventInternal::WriteTrackDescriptor(
                track, ctx.tls_inst_->trace_writer.get(), ctx.GetIncrementalState(), *ctx.GetCustomTlsState(), TrackEventInternal::GetTraceTime());
        });
    }
    ...
    static uint64_t GetTraceTimeNs()
    {
    ...
    const protos::gen::TrackEventConfig& GetConfig() const
    {
        return config_;
    }
...
private:
    ...
    template <typename T> static T&& DecayStrType(T&& t)
    {
        return std::forward<T>(t);
    }

    static const char* DecayStrType(const char* t)
    {
        return t;
    }
    ...
    template <typename T> static T&& DecayArgType(T&& t)
    {
        return std::forward<T>(t);
    }

    static const char* DecayArgType(const char* s)
    {
        return s;
    }
    static uint64_t DecayArgType(uint64_t u)
    {
        return u;
    }
    static uint32_t DecayArgType(uint32_t u)
    {
        return u;
    }
    static uint16_t DecayArgType(uint16_t u)
    {
        return u;
    }
    static uint8_t DecayArgType(uint8_t u)
    {
        return u;
    }
    static int64_t DecayArgType(int64_t i)
    {
        return i;
    }
    static int32_t DecayArgType(int32_t i)
    {
        return i;
    }
    static int16_t DecayArgType(int16_t i)
    {
        return i;
    }
    static int8_t DecayArgType(int8_t i)
    {
        return i;
    }
    static bool DecayArgType(bool b)
    {
        return b;
    }
    static float DecayArgType(float f)
    {
        return f;
    }
    static double DecayArgType(double f)
    {
        return f;
    }
    ...
    template <typename CategoryType, typename EventNameType, typename... Arguments>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImplNoTimestamp(instances, category, event_name, type, TrackEventInternal::kDefaultTrack, std::forward<Arguments>(args)...);
    }
    ...
    template <typename TrackType, typename CategoryType, typename EventNameType, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImplNoTimestamp(instances, category, event_name, type, std::forward<TrackType>(track), std::forward<Arguments>(args)...);
    }

    // Trace point which takes a timestamp, but not track.
    template <typename CategoryType, typename EventNameType, typename TimestampType = uint64_t, typename... Arguments,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TimestampType&& timestamp, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImpl(
            instances, category, event_name, type, TrackEventInternal::kDefaultTrack, std::forward<TimestampType>(timestamp), std::forward<Arguments>(args)...);
    }
    ...
    template <typename TrackType, typename CategoryType, typename EventNameType, typename TimestampType = uint64_t, typename... Arguments,
        ...
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, TimestampType&& timestamp, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImpl(
            instances, category, event_name, type, std::forward<TrackType>(track), std::forward<TimestampType>(timestamp), std::forward<Arguments>(args)...);
    }

    // Trace point with with a counter sample.
    template <typename CategoryType, typename EventNameType, typename ValueType>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType&, perfetto::protos::pbzero::TrackEvent::Type type,
        CounterTrack track, ValueType value) PERFETTO_ALWAYS_INLINE
    {
        PERFETTO_DCHECK(type == perfetto::protos::pbzero::TrackEvent::TYPE_COUNTER);
        TraceForCategory(instances, category, /*name=*/nullptr, type, track, TrackEventInternal::GetTraceTime(), value);
    }

    // Trace point with with a timestamp and a counter sample.
    template <typename CategoryType, typename EventNameType, typename TimestampType = uint64_t,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type, typename ValueType>
    static void TraceForCategoryBody(uint32_t instances, const CategoryType& category, const EventNameType&, perfetto::protos::pbzero::TrackEvent::Type type,
        CounterTrack track, TimestampType timestamp, ValueType value) PERFETTO_NO_INLINE
    {
        PERFETTO_DCHECK(type == perfetto::protos::pbzero::TrackEvent::TYPE_COUNTER);
        TraceForCategoryImpl(instances, category, /*name=*/nullptr, type, track, timestamp, [&](EventContext event_ctx) {
            if (std::is_integral<ValueType>::value) {
                int64_t value_int64 = static_cast<int64_t>(value);
                if (track.is_incremental()) {
                    TrackEventIncrementalState* incr_state = event_ctx.GetIncrementalState();
                    PERFETTO_DCHECK(incr_state != nullptr);
                    auto prv_value = incr_state->last_counter_value_per_track[track.uuid];
                ...
                event_ctx.event()->set_double_counter_value(static_cast<double>(value));
            }
        });
    }
...
// folded by the linker.
    ...
    template <typename TrackType, typename CategoryType, typename EventNameType, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type>
    static void TraceForCategoryLegacyBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImplNoTimestamp(instances, category, event_name, type, track, [&](perfetto::EventContext ctx) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {
            using ::perfetto::internal::TrackEventLegacy;
            TrackEventLegacy::WriteLegacyEvent(std::move(ctx), phase, flags, args...);
        });
    }
    ...
    {
        TraceForCategoryImpl(instances, category, event_name, type, track, timestamp, [&](perfetto::EventContext ctx) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {
            using ::perfetto::internal::TrackEventLegacy;
            TrackEventLegacy::WriteLegacyEvent(std::move(ctx), phase, flags, args...);
        });
    }
    ...
    template <typename TrackType, typename CategoryType, typename EventNameType, typename ThreadIdType, typename LegacyIdType, typename... Arguments,
        typename TrackTypeCheck = typename std::enable_if<std::is_convertible<TrackType, Track>::value>::type>
    static void TraceForCategoryLegacyWithIdBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, ThreadIdType thread_id, LegacyIdType legacy_id,
        Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImplNoTimestamp(instances, category, event_name, type, track, [&](perfetto::EventContext ctx) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {
            ...
            TrackEventLegacy::WriteLegacyEventWithIdAndTid(std::move(ctx), phase, flags, trace_id, thread_id, args...);
        });
    }
    ...
    static void TraceForCategoryLegacyWithIdBody(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, TrackType&& track, char phase, uint32_t flags, ThreadIdType thread_id, LegacyIdType legacy_id,
        TimestampType&& timestamp, Arguments&&... args) PERFETTO_NO_INLINE
    {
        TraceForCategoryImpl(instances, category, event_name, type, track, timestamp, [&](perfetto::EventContext ctx) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {
            using ::perfetto::internal::TrackEventLegacy;
            ::perfetto::internal::LegacyTraceId trace_id { legacy_id };
    ...
    struct CategoryTracePointTraits {
        ...
        static constexpr std::atomic<uint8_t>* GetActiveInstances(TracePointData data)
        {
            return Registry->GetCategoryState(data.category_index);
        }
    };

    template <typename CategoryType, typename EventNameType, typename TrackType = Track,
        typename TrackTypeCheck = typename std::enable_if<IsValidTrack<TrackType>()>::type>
    static perfetto::EventContext WriteTrackEventImpl(typename Base::TraceContext& ctx, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, const TrackType& track, const TraceTimestamp& trace_timestamp) PERFETTO_ALWAYS_INLINE
    {
        using CatTraits = CategoryTraits<CategoryType>;
        const Category* static_category = CatTraits::GetStaticCategory(Registry, category);
        ...
        TrackEventInternal::ResetIncrementalStateIfRequired(trace_writer, incr_state, tls_state, trace_timestamp);

        // Write the track descriptor before any event on the track.
        if (track) {
            TrackEventInternal::WriteTrackDescriptorIfNeeded(track, trace_writer, incr_state, tls_state, trace_timestamp);
        }

        // Write the event itself.
        bool on_current_thread_track = (&track == &TrackEventInternal::kDefaultTrack);
        auto event_ctx = TrackEventInternal::WriteEvent(trace_writer, incr_state, tls_state, static_category, type, trace_timestamp, on_current_thread_track);
        ...
        if (CatTraits::kIsDynamic && type != protos::pbzero::TrackEvent::TYPE_SLICE_END && type != protos::pbzero::TrackEvent::TYPE_COUNTER) {
            DynamicCategory dynamic_category = CatTraits::GetDynamicCategory(category);
    ...
    template <typename CategoryType, typename EventNameType, typename TrackType = Track, typename TimestampType = uint64_t,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type,
        typename TrackTypeCheck = typename std::enable_if<IsValidTrack<TrackType>()>::type>
    static perfetto::EventContext WriteTrackEvent(typename Base::TraceContext& ctx, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, const TrackType& track, const TimestampType& timestamp) PERFETTO_NO_INLINE
    {
        TraceTimestamp trace_timestamp = ::perfetto::TraceTimestampTraits<TimestampType>::ConvertTimestampToTraceTimeNs(timestamp);
        return WriteTrackEventImpl(ctx, category, event_name, type, track, trace_timestamp);
    }

    template <typename CategoryType, typename EventNameType, typename TrackType = Track,
        typename TrackTypeCheck = typename std::enable_if<IsValidTrack<TrackType>()>::type>
    static perfetto::EventContext WriteTrackEvent(typename Base::TraceContext& ctx, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, const TrackType& track) PERFETTO_NO_INLINE
    {
    ...
    template <typename CategoryType, typename EventNameType, typename TrackType = Track, typename TimestampType = uint64_t,
        typename TimestampTypeCheck = typename std::enable_if<IsValidTimestamp<TimestampType>()>::type,
        typename TrackTypeCheck = typename std::enable_if<IsValidTrack<TrackType>()>::type, typename... Arguments>
    static void TraceForCategoryImpl(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, const TrackType& track, const TimestampType& timestamp, Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        using CatTraits = CategoryTraits<CategoryType>;
        TraceWithInstances(instances, category, [&](typename Base::TraceContext ctx) {
            // If this category is dynamic, first check whether it's enabled.
            if (CatTraits::kIsDynamic && !IsDynamicCategoryEnabled(&ctx, CatTraits::GetDynamicCategory(category))) {
                return;
            }

            auto event_ctx = WriteTrackEvent(ctx, category, event_name, type, track, timestamp);
            WriteTrackEventArgs(std::move(event_ctx), std::forward<Arguments>(args)...);
        });
    }

    template <typename CategoryType, typename EventNameType, typename TrackType = Track,
        typename TrackTypeCheck = typename std::enable_if<IsValidTrack<TrackType>()>::type, typename... Arguments>
    static void TraceForCategoryImplNoTimestamp(uint32_t instances, const CategoryType& category, const EventNameType& event_name,
        perfetto::protos::pbzero::TrackEvent::Type type, const TrackType& track, Arguments&&... args) PERFETTO_ALWAYS_INLINE
    {
        using CatTraits = CategoryTraits<CategoryType>;
        TraceWithInstances(instances, category, [&](typename Base::TraceContext ctx) {
            // If this category is dynamic, first check whether it's enabled.
            ...
            auto event_ctx = WriteTrackEvent(ctx, category, event_name, type, track);
            WriteTrackEventArgs(std::move(event_ctx), std::forward<Arguments>(args)...);
        });
    }

    template <typename CategoryType, typename Lambda>
    static void TraceWithInstances(uint32_t instances, const CategoryType& category, Lambda lambda) PERFETTO_ALWAYS_INLINE
    {
        using CatTraits = CategoryTraits<CategoryType>;
        if (CatTraits::kIsDynamic) {
            Base::TraceWithInstances(instances, std::move(lambda));
        } else {
            Base::template TraceWithInstances<CategoryTracePointTraits>(instances, std::move(lambda), { CatTraits::GetStaticIndex(category) });
        }
    }
    ...
    static bool IsDynamicCategoryEnabled(typename Base::TraceContext* ctx, const DynamicCategory& dynamic_category)
    {
        ...
        if (it == incr_state->dynamic_categories.end()) {
            ...
            bool enabled = TrackEventInternal::IsCategoryEnabled(*Registry, ds->config_, category);
