...
#define PERFETTO_INTERNAL_DECLARE_CATEGORIES(attrs, ...)                                                                                                       \
    ...
    attrs extern const ::perfetto::internal::TrackEventCategoryRegistry kCategoryRegistry;                                                                     \
...
#define PERFETTO_INTERNAL_CATEGORY_STORAGE(attrs)                                                                                                              \
    ...
    attrs const ::perfetto::internal::TrackEventCategoryRegistry kCategoryRegistry(kCategoryCount, &kCategories[0], &g_category_state_storage[0]);             \
    } // namespace internal
...
#define PERFETTO_INTERNAL_DECLARE_TRACK_EVENT_DATA_SOURCE(attrs)                                                                                               \
    struct attrs TrackEvent : public ::perfetto::internal::TrackEventDataSource<TrackEvent, &internal::kCategoryRegistry> {                                    \
        virtual ~TrackEvent();                                                                                                                                 \
    }

#define PERFETTO_INTERNAL_DEFINE_TRACK_EVENT_DATA_SOURCE() TrackEvent::~TrackEvent() = default;
...
#define PERFETTO_GET_CATEGORY_INDEX(category)                                                                                                                  \
    PERFETTO_TRACK_EVENT_NAMESPACE::internal::kConstExprCategoryRegistry.Find(category, ::PERFETTO_TRACK_EVENT_NAMESPACE::internal::IsDynamicCategory(category))
...
#define PERFETTO_INTERNAL_TRACK_EVENT_WITH_METHOD(method, category, name, ...)                                                                                 \
    do {                                                                                                                                                       \
        ...
        /* GCC 7 bug */                                                                                                                                        \
        constexpr auto PERFETTO_UID(kCatIndex_ADD_TO_PERFETTO_DEFINE_CATEGORIES_IF_FAILS_) = PERFETTO_GET_CATEGORY_INDEX(category);                            \
        if (::PERFETTO_TRACK_EVENT_NAMESPACE::internal::IsDynamicCategory(category)) {                                                                         \
            tns::TrackEvent::CallIfEnabled([&](uint32_t instances) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {                                                        \
                tns::TrackEvent::method(instances, category, ::perfetto::internal::DecayEventNameType(name), ##__VA_ARGS__);                                   \
            });                                                                                                                                                \
        } else {                                                                                                                                               \
            tns::TrackEvent::CallIfCategoryEnabled(                                                                                                            \
                PERFETTO_UID(kCatIndex_ADD_TO_PERFETTO_DEFINE_CATEGORIES_IF_FAILS_), [&](uint32_t instances) PERFETTO_NO_THREAD_SAFETY_ANALYSIS {              \
                    tns::TrackEvent::method(instances, PERFETTO_UID(kCatIndex_ADD_TO_PERFETTO_DEFINE_CATEGORIES_IF_FAILS_),                                    \
                        ::perfetto::internal::DecayEventNameType(name), ##__VA_ARGS__);                                                                        \
                });                                                                                                                                            \
        }                                                                                                                                                      \
    } while (false)
    ...
    struct PERFETTO_UID(ScopedEvent) {                                                                                                                         \
        struct EventFinalizer {                                                                                                                                \
            ...
            inline EventFinalizer& operator=(const EventFinalizer&) = delete;                                                                                  \
                                                                                                                                                               \
            EventFinalizer(EventFinalizer&&) = PERFETTO_INTERNAL_EVENT_FINALIZER_KEYWORD;                                                                      \
...
// id.
#define PERFETTO_INTERNAL_SCOPED_LEGACY_TRACK_EVENT_WITH_ID(category, name, track, flags, thread_id, id, ...)                                                  \
    PERFETTO_INTERNAL_SCOPED_EVENT_FINALIZER(category)                                                                                                         \
    PERFETTO_UID(scoped_event)                                                                                                                                 \
    {                                                                                                                                                          \
        [&]() {                                                                                                                                                \
            PERFETTO_INTERNAL_TRACK_EVENT_WITH_METHOD(TraceForCategoryLegacyWithId, category, name, ::perfetto::protos::pbzero::TrackEvent::TYPE_SLICE_BEGIN,  \
                track, 'B', flags, thread_id, id, ##__VA_ARGS__);                                                                                              \
            return 0;                                                                                                                                          \
        }()                                                                                                                                                    \
    }
#endif // PERFETTO_ENABLE_LEGACY_TRACE_EVENTS

#if PERFETTO_BUILDFLAG(PERFETTO_COMPILER_GCC) || PERFETTO_BUILDFLAG(PERFETTO_COMPILER_MSVC)
...
#define PERFETTO_INTERNAL_CATEGORY_ENABLED(category)                                                                                                           \
    (::PERFETTO_TRACK_EVENT_NAMESPACE::internal::IsDynamicCategory(category)                                                                                   \
            ? PERFETTO_TRACK_EVENT_NAMESPACE::TrackEvent::IsDynamicCategoryEnabled(::perfetto::DynamicCategory(category))                                      \
            : PERFETTO_TRACK_EVENT_NAMESPACE::TrackEvent::IsCategoryEnabled(PERFETTO_GET_CATEGORY_INDEX(category)))
...
#define PERFETTO_INTERNAL_CATEGORY_ENABLED(category)                                                                                                           \
    [&]() -> bool {                                                                                                                                            \
        ...
        constexpr auto PERFETTO_UID(index) = PERFETTO_GET_CATEGORY_INDEX(category);                                                                            \
        constexpr auto PERFETTO_UID(dynamic) = IsDynamicCategory(category);                                                                                    \
        return PERFETTO_UID(dynamic) ? TrackEvent::IsDynamicCategoryEnabled(::perfetto::DynamicCategory(category))                                             \
...
#define PERFETTO_INTERNAL_ADD_EMPTY_EVENT()                                                                                                                    \
    do {                                                                                                                                                       \
        PERFETTO_TRACK_EVENT_NAMESPACE::TrackEvent::Trace([](PERFETTO_TRACK_EVENT_NAMESPACE::TrackEvent::TraceContext ctx) { ctx.AddEmptyTracePacket(); });    \
    } while (false)
