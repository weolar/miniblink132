...
{
    object->Set(realm->context(), FIXED_ONE_BYTE_STRING(realm->isolate(), "urlComponents"), url_components_buffer_.GetJSArray()).Check();
...
std::string EncodePathChars(std::string_view input_str, OS operating_system)
{
    ...
    for (size_t i : input_str) {
        if (i > LARGEST_ASCII_CHAR_CODE_TO_ENCODE)
            [[unlikely]]
            {
                encoded.push_back(i);
                continue;
...
void BindingData::PathToFileURL(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto out = ada::parse<ada::url_aggregator>(EncodePathChars(input_str, os), nullptr);
...
bool BindingData::FastCanParse(Local<Value> receiver, const FastOneByteString& input)
{
    TRACK_V8_FAST_API_CALL("url.canParse");
    return ada::can_parse(std::string_view(input.data, input.length));
}

bool BindingData::FastCanParseWithBase(Local<Value> receiver, const FastOneByteString& input, const FastOneByteString& base)
{
...
CFunction BindingData::fast_can_parse_methods_[] = { CFunction::Make(FastCanParse), CFunction::Make(FastCanParseWithBase) };
...
void BindingData::UpdateComponents(const ada::url_components& components, const ada::scheme::type type)
{
    ...
    static_assert(kURLComponentsLength == 9, "kURLComponentsLength should be up-to-date");
}
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    SetFastMethodNoSideEffect(isolate, target, "canParse", CanParse, { fast_can_parse_methods_, 2 });
}
...
void ThrowInvalidURL(node::Environment* env, std::string_view input, std::optional<std::string> base)
{
    ...
    Local<Value> tmp;
    if (!ToV8Value(env->context(), input, env->isolate()).ToLocal(&tmp) || err_object->Set(env->context(), env->input_string(), tmp).IsNothing())
        [[unlikely]]
        {
    ...
    if (base.has_value()) {
        if (!ToV8Value(env->context(), base.value(), env->isolate()).ToLocal(&tmp) || err_object->Set(env->context(), env->base_string(), tmp).IsNothing())
            [[unlikely]]
            {
                return;
            }
...
std::optional<std::string> FileURLToPath(Environment* env, const ada::url_aggregator& file_url)
{
    ...
    for (size_t i = 0; i < pathname_size; i++) {
        ...
        THROW_ERR_INVALID_FILE_URL_PATH(env->isolate(), "File URL path must not include encoded \\ or / characters");
        return std::nullopt;
    }

    std::string_view hostname = file_url.get_hostname();
    std::string decoded_pathname = ada::unicode::percent_decode(std::string_view(pathname_escaped_slash), first_percent);
    ...
    if (letter < 'a' || letter > 'z' || sep != ':') {
        THROW_ERR_INVALID_FILE_URL_PATH(env->isolate(), "File URL path must be absolute");
        return std::nullopt;
    }
    ...
    if (hostname.size() > 0) {
        THROW_ERR_INVALID_FILE_URL_HOST(env->isolate(), "File URL host must be \"localhost\" or empty on ", std::string(per_process::metadata.platform));
        return std::nullopt;
    }
    ...
    for (size_t i = 0; (i + 2) < pathname.size(); i++) {
        ...
        if (pathname[i + 1] == '2' && (pathname[i + 2] | 0x20) == 102) {
            THROW_ERR_INVALID_FILE_URL_PATH(env->isolate(), "File URL path must not include encoded / characters");
            return std::nullopt;
        }
    }
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(url, node::url::BindingData::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(url, node::url::BindingData::CreatePerIsolateProperties)
NODE_BINDING_EXTERNAL_REFERENCE(url, node::url::BindingData::RegisterExternalReferences)
