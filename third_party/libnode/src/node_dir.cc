...
#define GET_TRACE_ENABLED (*TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACING_CATEGORY_NODE2(fs_dir, sync)) != 0)
#define FS_DIR_SYNC_TRACE_BEGIN(syscall, ...)                                                                                                                  \
    if (GET_TRACE_ENABLED)                                                                                                                                     \
        TRACE_EVENT_BEGIN(TRACING_CATEGORY_NODE2(fs_dir, sync), TRACE_NAME(syscall), ##__VA_ARGS__);
#define FS_DIR_SYNC_TRACE_END(syscall, ...)                                                                                                                    \
    if (GET_TRACE_ENABLED)                                                                                                                                     \
        TRACE_EVENT_END(TRACING_CATEGORY_NODE2(fs_dir, sync), TRACE_NAME(syscall), ##__VA_ARGS__);

#define FS_DIR_ASYNC_TRACE_BEGIN0(fs_type, id) TRACE_EVENT_NESTABLE_ASYNC_BEGIN0(TRACING_CATEGORY_NODE2(fs_dir, async), get_dir_func_name_by_type(fs_type), id);
#define FS_DIR_ASYNC_TRACE_END0(fs_type, id) TRACE_EVENT_NESTABLE_ASYNC_END0(TRACING_CATEGORY_NODE2(fs_dir, async), get_dir_func_name_by_type(fs_type), id);

#define FS_DIR_ASYNC_TRACE_BEGIN1(fs_type, id, name, value)                                                                                                    \
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(TRACING_CATEGORY_NODE2(fs_dir, async), get_dir_func_name_by_type(fs_type), id, name, value);

#define FS_DIR_ASYNC_TRACE_END1(fs_type, id, name, value)                                                                                                      \
    TRACE_EVENT_NESTABLE_ASYNC_END1(TRACING_CATEGORY_NODE2(fs_dir, async), get_dir_func_name_by_type(fs_type), id, name, value);
...
{
    Local<Object> obj;
    if (!env->dir_instance_template()->NewInstance(env->context()).ToLocal(&obj)) {
        return nullptr;
    }
...
inline void DirHandle::GCClose()
{
    ...
    env()->SetImmediate([](Environment* env) { ProcessEmitWarning(env, "Closing directory handle on garbage collection"); }, CallbackFlags::kUnrefed);
}
...
void AfterClose(uv_fs_t* req)
{
    ...
    FS_DIR_ASYNC_TRACE_END1(req->fs_type, req_wrap, "result", static_cast<int>(req->result))
    if (after.Proceed())
...
void DirHandle::Close(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsUndefined()) { // close(req)
        ...
        AsyncCall(env, req_wrap_async, args, "closedir", UTF8, AfterClose, uv_fs_closedir, dir->dir());
    } else { // close()
...
static MaybeLocal<Array> DirentListToArray(Environment* env, uv_dirent_t* ents, int num, enum encoding encoding, Local<Value>* err_out)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (!StringBytes::Encode(env->isolate(), ents[i].name, namelen, encoding, &error).ToLocal(&filename)) {
            *err_out = error;
...
static void AfterDirRead(uv_fs_t* req)
{
    ...
    if (!DirentListToArray(env, dir->dirents, static_cast<int>(req->result), req_wrap->encoding(), &error).ToLocal(&js_array)) {
...
void DirHandle::Read(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[2]->IsUndefined()) { // dir.read(encoding, bufferSize, req)
        ...
        AsyncCall(env, req_wrap_async, args, "readdir", encoding, AfterDirRead, uv_fs_readdir, dir->dir());
    } else { // dir.read(encoding, bufferSize)
        FSReqWrapSync req_wrap_sync("readdir");
        FS_DIR_SYNC_TRACE_BEGIN(readdir);
        int err = SyncCallAndThrowOnError(env, &req_wrap_sync, uv_fs_readdir, dir->dir());
        ...
        Local<Value> error;
        Local<Array> js_array;
        if (!DirentListToArray(env, dir->dir()->dirents, static_cast<int>(req_wrap_sync.req.result), encoding, &error).ToLocal(&js_array)) {
...
static void OpenDir(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[2]->IsUndefined()) { // openDir(path, encoding, req)
        ...
        FS_DIR_ASYNC_TRACE_BEGIN1(UV_FS_OPENDIR, req_wrap_async, "path", TRACE_STR_COPY(*path))
        AsyncCall(env, req_wrap_async, args, "opendir", encoding, AfterOpenDir, uv_fs_opendir, *path);
        ...
        int result = SyncCallAndThrowOnError(env, &req_wrap_sync, uv_fs_opendir, *path);
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(fs_dir, node::fs_dir::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(fs_dir, node::fs_dir::CreatePerIsolateProperties)
NODE_BINDING_EXTERNAL_REFERENCE(fs_dir, node::fs_dir::RegisterExternalReferences)
