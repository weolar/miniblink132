...
#define V(modname) void _register_isolate_##modname(node::IsolateData* isolate_data, v8::Local<v8::ObjectTemplate> target);
...
struct dl_wrap {
    ...
    struct hash {
        ...
        {
            return std::hash<uint64_t>()(wrap->st_dev) ^ std::hash<uint64_t>()(wrap->st_ino);
        }
    };
...
using InitializerCallback = void (*)(Local<Object> exports, Local<Value> module, Local<Context> context);
...
inline node_api_addon_get_api_version_func GetNapiAddonGetApiVersionCallback(DLib* dlib)
{
    return reinterpret_cast<node_api_addon_get_api_version_func>(dlib->GetSymbolAddress(STRINGIFY(NODE_API_MODULE_GET_API_VERSION)));
}
...
void DLOpen(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (env->no_native_addons()) {
        return THROW_ERR_DLOPEN_DISABLED(env, "Cannot load native addon because loading addons is disabled.");
    }
    ...
    if (!args[0]->ToObject(context).ToLocal(&module) || !module->Get(context, env->exports_string()).ToLocal(&exports_v)
    ...
    env->TryLoadAddon(*filename, flags, [&](DLib* dlib) {
        ...
        if (mp != nullptr) {
            ...
            if (auto callback = GetInitializerCallback(dlib)) {
                ...
                napi_module_register_by_symbol(exports, module, context, napi_callback, module_api_version);
                return true;
            } else {
                mp = dlib->GetSavedModuleFromGlobalHandleMap();
                if (mp == nullptr || mp->nm_context_register_func == nullptr) {
                    dlib->Close();
                    THROW_ERR_DLOPEN_FAILED(env, "Module did not self-register: '%s'.", *filename);
                    return false;
                }
            }
        }
        ...
        if ((mp->nm_version != -1) && (mp->nm_version != NODE_MODULE_VERSION)) {
            ...
            THROW_ERR_DLOPEN_FAILED(env,
                ...
                "or `npm install`).",
                *filename, actual_nm_version, NODE_MODULE_VERSION);
            return false;
        }
...
static Local<Object> GetInternalBindingExportObject(IsolateData* isolate_data, const char* mod_name, Local<Context> context)
{
...
static Local<Object> InitInternalBinding(Realm* realm, node_module* mod)
{
    ...
    Local<Object> exports = GetInternalBindingExportObject(realm->isolate_data(), mod->nm_modname, context);
...
void GetLinkedBinding(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (mod == nullptr) {
        return THROW_ERR_INVALID_MODULE(env, "No such binding was linked: %s", *module_name_v);
    }
...
NODE_BINDING_EXTERNAL_REFERENCE(binding, node::binding::RegisterExternalReferences)
