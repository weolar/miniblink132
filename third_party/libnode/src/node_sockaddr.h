...
public:
    ...
    static bool ToSockAddr(int32_t family, const char* host, uint32_t port, sockaddr_storage* addr);

    // Returns true if converting {family, host, port} to *addr succeeded.
    static bool New(int32_t family, const char* host, uint32_t port, SocketAddress* addr);
    ...
    inline v8::MaybeLocal<v8::Object> ToJS(Environment* env, v8::Local<v8::Object> obj = v8::Local<v8::Object>()) const;
    ...
    template <typename T> using Map = std::unordered_map<SocketAddress, T, Hash>;
...
public:
    ...
    static BaseObjectPtr<SocketAddressBase> Create(Environment* env, std::shared_ptr<SocketAddress> address);
    ...
    SocketAddressBase(Environment* env, v8::Local<v8::Object> wrap, std::shared_ptr<SocketAddress> address);
...
template <typename T> class SocketAddressLRU : public MemoryRetainer {
public:
...
public:
    explicit SocketAddressBlockList(std::shared_ptr<SocketAddressBlockList> parent = {});
    ...
    void AddSocketAddressRange(const std::shared_ptr<SocketAddress>& start, const std::shared_ptr<SocketAddress>& end);

    void AddSocketAddressMask(const std::shared_ptr<SocketAddress>& address, int prefix);
    ...
    struct SocketAddressRangeRule final : Rule {
        ...
        SocketAddressRangeRule(const std::shared_ptr<SocketAddress>& start, const std::shared_ptr<SocketAddress>& end);
    ...
    struct SocketAddressMaskRule final : Rule {
        ...
        SocketAddressMaskRule(const std::shared_ptr<SocketAddress>& address, int prefix);
...
public:
    ...
    static BaseObjectPtr<SocketAddressBlockListWrap> New(Environment* env, std::shared_ptr<SocketAddressBlockList> blocklist);
    ...
    SocketAddressBlockListWrap(
        Environment* env, v8::Local<v8::Object> wrap, std::shared_ptr<SocketAddressBlockList> blocklist = std::make_shared<SocketAddressBlockList>());
    ...
    public:
        ...
        inline explicit TransferData(std::shared_ptr<SocketAddressBlockList> blocklist)
