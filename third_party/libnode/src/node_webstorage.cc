...
#define THROW_SQLITE_ERROR(env, r) THROW_ERR_INVALID_STATE((env), sqlite3_errstr((r)))
...
static void ThrowQuotaExceededException(Local<Context> context)
{
    ...
    auto err_message = FIXED_ONE_BYTE_STRING(isolate, "Setting the value exceeded the quota");
    Local<Object> per_context_bindings;
    Local<Value> domexception_ctor_val;
    if (!GetPerContextExports(context).ToLocal(&per_context_bindings)
        || !per_context_bindings->Get(context, dom_exception_str).ToLocal(&domexception_ctor_val)) {
        return;
    }
    ...
    if (!domexception_ctor->NewInstance(context, arraysize(argv), argv).ToLocal(&exception)) {
        return;
    }
...
Storage::Storage(Environment* env, Local<Object> object, std::string_view location)
...
Maybe<void> Storage::Open()
{
    ...
    static constexpr std::string_view get_schema_version_sql = "SELECT schema_version FROM nodejs_webstorage_state";
    static constexpr std::string_view init_sql_v0 = "PRAGMA encoding = 'UTF-16le';"
    ...
    r = sqlite3_prepare_v2(db, get_schema_version_sql.data(), get_schema_version_sql.size(), &s, 0);
    r = sqlite3_exec(db, init_sql_v0.data(), 0, 0, nullptr);
    CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, Nothing<void>());
    auto stmt = stmt_unique_ptr(s);
    CHECK_ERROR_OR_THROW(env(), sqlite3_step(stmt.get()), SQLITE_ROW, Nothing<void>());
    ...
    if (schema_version > kCurrentSchemaVersion) {
        THROW_ERR_INVALID_STATE(env(), "localStorage was created with a newer version of Node.js");
        return Nothing<void>();
    }

    if (schema_version < kCurrentSchemaVersion) {
        // Run any migrations and update the schema version.
        std::string set_user_version_sql = "UPDATE nodejs_webstorage_state SET schema_version = " + std::to_string(kCurrentSchemaVersion) + ";";
...
Maybe<void> Storage::Clear()
{
    ...
    sqlite3_stmt* s = nullptr;
    CHECK_ERROR_OR_THROW(env(), sqlite3_prepare_v2(db_.get(), sql.data(), sql.size(), &s, 0), SQLITE_OK, Nothing<void>());
    auto stmt = stmt_unique_ptr(s);
    CHECK_ERROR_OR_THROW(env(), sqlite3_step(stmt.get()), SQLITE_DONE, Nothing<void>());
    return JustVoid();
}
...
MaybeLocal<Array> Storage::Enumerate()
{
    ...
    while ((r = sqlite3_step(stmt.get())) == SQLITE_ROW) {
        ...
        if (!String::NewFromTwoByte(env()->isolate(), reinterpret_cast<const uint16_t*>(sqlite3_column_blob(stmt.get(), 0)), NewStringType::kNormal, size)
                 .ToLocal(&value)) {
            return Local<Array>();
        }
...
MaybeLocal<Value> Storage::Length()
{
    ...
    static constexpr std::string_view sql = "SELECT count(*) FROM nodejs_webstorage";
    sqlite3_stmt* s = nullptr;
    int r = sqlite3_prepare_v2(db_.get(), sql.data(), sql.size(), &s, 0);
    CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, Local<Value>());
    auto stmt = stmt_unique_ptr(s);
    CHECK_ERROR_OR_THROW(env(), sqlite3_step(stmt.get()), SQLITE_ROW, Local<Value>());
    CHECK(sqlite3_column_type(stmt.get(), 0) == SQLITE_INTEGER);
...
MaybeLocal<Value> Storage::Load(Local<Name> key)
{
    ...
    static constexpr std::string_view sql = "SELECT value FROM nodejs_webstorage WHERE key = ? LIMIT 1";
    sqlite3_stmt* s = nullptr;
    int r = sqlite3_prepare_v2(db_.get(), sql.data(), sql.size(), &s, 0);
    CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, Local<Value>());
    auto stmt = stmt_unique_ptr(s);
    ...
    if (r == SQLITE_ROW) {
        CHECK(sqlite3_column_type(stmt.get(), 0) == SQLITE_BLOB);
        auto size = sqlite3_column_bytes(stmt.get(), 0) / sizeof(uint16_t);
        return String::NewFromTwoByte(env()->isolate(), reinterpret_cast<const uint16_t*>(sqlite3_column_blob(stmt.get(), 0)), NewStringType::kNormal, size)
            .As<Value>();
...
MaybeLocal<Value> Storage::LoadKey(const int index)
{
    ...
    static constexpr std::string_view sql = "SELECT key FROM nodejs_webstorage LIMIT 1 OFFSET ?";
    sqlite3_stmt* s = nullptr;
    int r = sqlite3_prepare_v2(db_.get(), sql.data(), sql.size(), &s, 0);
    CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, Local<Value>());
    auto stmt = stmt_unique_ptr(s);
...
Maybe<void> Storage::Remove(Local<Name> key)
{
    ...
    static constexpr std::string_view sql = "DELETE FROM nodejs_webstorage WHERE key = ?";
    sqlite3_stmt* s = nullptr;
    int r = sqlite3_prepare_v2(db_.get(), sql.data(), sql.size(), &s, 0);
    CHECK_ERROR_OR_THROW(env(), r, SQLITE_OK, Nothing<void>());
    auto stmt = stmt_unique_ptr(s);
...
Maybe<void> Storage::Store(Local<Name> key, Local<Value> value)
{
    ...
    static constexpr std::string_view sql = "INSERT INTO nodejs_webstorage (key, value) VALUES (?, ?)"
...
static void GetItem(const FunctionCallbackInfo<Value>& info)
{
    ...
    if (info.Length() < 1) {
        return THROW_ERR_MISSING_ARGS(env, "Failed to execute 'getItem' on 'Storage': 1 argument required");
    }
...
static void Key(const FunctionCallbackInfo<Value>& info)
{
    ...
    if (info.Length() < 1) {
        return THROW_ERR_MISSING_ARGS(env, "Failed to execute 'key' on 'Storage': 1 argument required");
    }
...
static void RemoveItem(const FunctionCallbackInfo<Value>& info)
{
    ...
    if (info.Length() < 1) {
        return THROW_ERR_MISSING_ARGS(env, "Failed to execute 'removeItem' on 'Storage': 1 argument required");
    }
...
static void SetItem(const FunctionCallbackInfo<Value>& info)
{
    ...
    if (info.Length() < 2) {
        return THROW_ERR_MISSING_ARGS(env, "Failed to execute 'setItem' on 'Storage': 2 arguments required");
    }
...
template <typename T> static bool ShouldIntercept(Local<Name> property, const PropertyCallbackInfo<T>& info)
{
    Environment* env = Environment::GetCurrent(info);
...
static Intercepted StorageGetter(Local<Name> property, const PropertyCallbackInfo<Value>& info)
{
...
static Intercepted StorageSetter(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
    Storage* storage;
    ASSIGN_OR_RETURN_UNWRAP(&storage, info.This(), Intercepted::kNo);
...
static Intercepted StorageQuery(Local<Name> property, const PropertyCallbackInfo<Integer>& info)
{
    if (!ShouldIntercept(property, info)) {
        return Intercepted::kNo;
    }
...
static Intercepted StorageDeleter(Local<Name> property, const PropertyCallbackInfo<Boolean>& info)
{
    Storage* storage;
    ASSIGN_OR_RETURN_UNWRAP(&storage, info.This(), Intercepted::kNo);
...
static Intercepted StorageDefiner(Local<Name> property, const PropertyDescriptor& desc, const PropertyCallbackInfo<void>& info)
{
    Storage* storage;
    ASSIGN_OR_RETURN_UNWRAP(&storage, info.This(), Intercepted::kNo);
...
static Intercepted IndexedGetter(uint32_t index, const PropertyCallbackInfo<Value>& info)
{
    Environment* env = Environment::GetCurrent(info);
    Local<Name> name;
...
static Intercepted IndexedSetter(uint32_t index, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
    Environment* env = Environment::GetCurrent(info);
    Local<Name> name;
    if (!Uint32ToName(env->context(), index).ToLocal(&name)) {
...
static Intercepted IndexedQuery(uint32_t index, const PropertyCallbackInfo<Integer>& info)
{
    Environment* env = Environment::GetCurrent(info);
    Local<Name> name;
    if (!Uint32ToName(env->context(), index).ToLocal(&name)) {
...
static Intercepted IndexedDeleter(uint32_t index, const PropertyCallbackInfo<Boolean>& info)
{
    Environment* env = Environment::GetCurrent(info);
    Local<Name> name;
    if (!Uint32ToName(env->context(), index).ToLocal(&name)) {
...
static Intercepted IndexedDefiner(uint32_t index, const PropertyDescriptor& desc, const PropertyCallbackInfo<void>& info)
{
    Environment* env = Environment::GetCurrent(info);
    Local<Name> name;
    if (!Uint32ToName(env->context(), index).ToLocal(&name)) {
...
static void Initialize(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    ...
    inst_tmpl->SetHandler(NamedPropertyHandlerConfiguration(StorageGetter, StorageSetter, StorageQuery, StorageDeleter, StorageEnumerator, StorageDefiner,
        nullptr, Local<Value>(), PropertyHandlerFlags::kHasNoSideEffect));
    inst_tmpl->SetHandler(IndexedPropertyHandlerConfiguration(
        IndexedGetter, IndexedSetter, IndexedQuery, IndexedDeleter, nullptr, IndexedDefiner, nullptr, Local<Value>(), PropertyHandlerFlags::kHasNoSideEffect));

    Local<FunctionTemplate> length_getter = FunctionTemplate::New(isolate, StorageLengthGetter);
    ctor_tmpl->PrototypeTemplate()->SetAccessorProperty(FIXED_ONE_BYTE_STRING(isolate, "length"), length_getter, Local<FunctionTemplate>(), DontDelete);
    ...
    target->DefineOwnProperty(context, FIXED_ONE_BYTE_STRING(isolate, "kConstructorKey"), symbol, PropertyAttribute::ReadOnly).Check();
}
