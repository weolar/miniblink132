...
static void GetOwnNonIndexProperties(const FunctionCallbackInfo<Value>& args)
{
    ...
    PropertyFilter filter = static_cast<PropertyFilter>(args[1].As<Uint32>()->Value());

    if (!object->GetPropertyNames(context, KeyCollectionMode::kOwnOnly, filter, IndexFilter::kSkipIndices).ToLocal(&properties)) {
        return;
    }
...
static void GetCallerLocation(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> ret[] = { Integer::New(isolate, frame->GetLineNumber()), Integer::New(isolate, frame->GetColumn()), file };
...
static void PreviewEntries(const FunctionCallbackInfo<Value>& args)
{
    ...
    return args.GetReturnValue().Set(Array::New(env->isolate(), ret, arraysize(ret)));
}
...
static void GuessHandleType(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    int fd;
    if (!args[0]->Int32Value(env->context()).To(&fd))
        return;
    CHECK_GE(fd, 0);
...
static void GetCallSites(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (int i = 1; i < frame_count; ++i) {
        ...
        Local<Object> obj = Object::New(isolate, v8::Null(isolate), names, values, arraysize(names));

        callsite_objects.push_back(obj);
    }

    Local<Array> callsites = Array::New(isolate, callsite_objects.data(), callsite_objects.size());
...
static void DefineLazyPropertiesGetter(Local<v8::Name> name, const v8::PropertyCallbackInfo<Value>& info)
{
    ...
    if (!realm->builtin_module_require()->Call(context, Null(isolate), 1, &arg).ToLocal(&require_result)) {
...
static void DefineLazyProperties(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        CHECK(key->IsString());
        if (target->SetLazyDataProperty(context, key.As<String>(), DefineLazyPropertiesGetter, id, attribute).IsNothing()) {
            // V8 will have scheduled an error to be thrown.
            return;
        };
    }
...
#define V(PropertyName, _) tmpl->Set(FIXED_ONE_BYTE_STRING(env->isolate(), #PropertyName), env->PropertyName());

        PER_ISOLATE_PRIVATE_SYMBOL_PROPERTIES(V)
#undef V

        target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "privateSymbols"), tmpl->NewInstance(context).ToLocalChecked()).Check();
    }

    {
        Local<Object> constants = Object::New(isolate);
#define V(name) constants->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, Promise::PromiseState::name)).Check();
...
#define V(name) constants->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, Environment::ExitInfoField::name)).Check();
...
#define V(name) constants->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, PropertyFilter::name)).Check();
...
#define V(name)                                                                                                                                                \
    constants->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, static_cast<int32_t>(BaseObject::TransferMode::name))).Check();
...
#undef V
    ...
    SetMethodNoSideEffect(context, target, "getPromiseDetails", GetPromiseDetails);
    SetMethodNoSideEffect(context, target, "getProxyDetails", GetProxyDetails);
    SetMethodNoSideEffect(context, target, "getCallerLocation", GetCallerLocation);
    SetMethodNoSideEffect(context, target, "previewEntries", PreviewEntries);
    SetMethodNoSideEffect(context, target, "getOwnNonIndexProperties", GetOwnNonIndexProperties);
    SetMethodNoSideEffect(context, target, "getConstructorName", GetConstructorName);
    ...
    SetMethod(context, target, "arrayBufferViewHasBuffer", ArrayBufferViewHasBuffer);

    Local<String> should_abort_on_uncaught_toggle = FIXED_ONE_BYTE_STRING(env->isolate(), "shouldAbortOnUncaughtToggle");
    CHECK(target->Set(context, should_abort_on_uncaught_toggle, env->should_abort_on_uncaught_toggle().GetJSArray()).FromJust());

    SetFastMethodNoSideEffect(context, target, "guessHandleType", GuessHandleType, &fast_guess_handle_type_);
}
