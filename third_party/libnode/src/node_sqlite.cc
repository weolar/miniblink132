...
#define SQLITE_VALUE_TO_JS(from, isolate, use_big_int_args, result, ...)                                                                                       \
    do {                                                                                                                                                       \
        ...
        case SQLITE_TEXT: {                                                                                                                                    \
            const char* v = reinterpret_cast<const char*>(sqlite3_##from##_text(__VA_ARGS__));                                                                 \
        ...
        case SQLITE_BLOB: {                                                                                                                                    \
            ...
            auto data = reinterpret_cast<const uint8_t*>(sqlite3_##from##_blob(__VA_ARGS__));                                                                  \
...
inline MaybeLocal<Object> CreateSQLiteError(Isolate* isolate, const char* message)
{
    Local<String> js_msg;
    Local<Object> e;
    Environment* env = Environment::GetCurrent(isolate);
    if (!String::NewFromUtf8(isolate, message).ToLocal(&js_msg) || !Exception::Error(js_msg)->ToObject(isolate->GetCurrentContext()).ToLocal(&e)
        || e->Set(isolate->GetCurrentContext(), env->code_string(), env->err_sqlite_error_string()).IsNothing()) {
...
inline MaybeLocal<Object> CreateSQLiteError(Isolate* isolate, int errcode)
{
    ...
    Local<Object> e;
    Environment* env = Environment::GetCurrent(isolate);
    if (!String::NewFromUtf8(isolate, errstr).ToLocal(&js_errmsg) || !CreateSQLiteError(isolate, errstr).ToLocal(&e)
        || e->Set(env->context(), env->errcode_string(), Integer::New(isolate, errcode)).IsNothing()
...
inline MaybeLocal<Object> CreateSQLiteError(Isolate* isolate, sqlite3* db)
{
    ...
    Local<Object> e;
    Environment* env = Environment::GetCurrent(isolate);
    if (!String::NewFromUtf8(isolate, errstr).ToLocal(&js_errmsg) || !CreateSQLiteError(isolate, errmsg).ToLocal(&e)
        || e->Set(isolate->GetCurrentContext(), env->errcode_string(), Integer::New(isolate, errcode)).IsNothing()
        || e->Set(isolate->GetCurrentContext(), env->errstr_string(), js_errmsg).IsNothing()) {
        return MaybeLocal<Object>();
    }
    return e;
}
...
inline void THROW_ERR_SQLITE_ERROR(Isolate* isolate, int errcode)
{
    ...
    if (CreateSQLiteError(isolate, errstr).ToLocal(&error)
        && error->Set(isolate->GetCurrentContext(), env->errcode_string(), Integer::New(isolate, errcode)).IsJust()) {
        isolate->ThrowException(error);
    }
}

inline MaybeLocal<Value> NullableSQLiteStringToValue(Isolate* isolate, const char* str)
{
    if (str == nullptr) {
        return Null(isolate);
    }

    return String::NewFromUtf8(isolate, str, NewStringType::kInternalized).As<Value>();
}

class BackupJob : public ThreadPoolWork {
public:
    explicit BackupJob(Environment* env, DatabaseSync* source, Local<Promise::Resolver> resolver, std::string source_db, std::string destination_name,
        std::string dest_db, int pages, Local<Function> progressFunc)
    ...
    void ScheduleBackup()
    {
        ...
        backup_status_ = sqlite3_open_v2(destination_name_.c_str(), &dest_, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_URI, nullptr);
        ...
        backup_ = sqlite3_backup_init(dest_, dest_db_.c_str(), source_->Connection(), source_db_.c_str());
        if (backup_ == nullptr) {
            HandleBackupError(resolver);
            return;
        }
    ...
    void AfterThreadPoolWork(int status) override
    {
        ...
        if (remaining_pages != 0) {
            ...
            if (!fn.IsEmpty()) {
                ...
                if (progress_info->Set(env()->context(), env()->total_pages_string(), Integer::New(env()->isolate(), total_pages)).IsNothing()
                    || progress_info->Set(env()->context(), env()->remaining_pages_string(), Integer::New(env()->isolate(), remaining_pages)).IsNothing()) {
                    return;
                }

                Local<Value> argv[] = { progress_info };
                TryCatch try_catch(env()->isolate());
                fn->Call(env()->context(), Null(env()->isolate()), 1, argv).FromMaybe(Local<Value>());
        ...
        Finalize();
        resolver->Resolve(env()->context(), Integer::New(env()->isolate(), total_pages)).ToChecked();
    }
...
UserDefinedFunction::UserDefinedFunction(Environment* env, Local<Function> fn, DatabaseSync* db, bool use_bigint_args)
    : env_(env)
    , fn_(env->isolate(), fn)
    , db_(db)
...
void UserDefinedFunction::xFunc(sqlite3_context* ctx, int argc, sqlite3_value** argv)
{
    UserDefinedFunction* self = static_cast<UserDefinedFunction*>(sqlite3_user_data(ctx));
    ...
    MaybeLocal<Value> retval = fn->Call(env->context(), recv, argc, js_argv.data());
    Local<Value> result;
...
DatabaseSync::DatabaseSync(Environment* env, Local<Object> object, DatabaseOpenConfiguration&& open_config, bool open, bool allow_load_extension)
...
void DatabaseSync::MemoryInfo(MemoryTracker* tracker) const
{
    ...
    tracker->TrackFieldWithSize("open_config", sizeof(open_config_), "DatabaseOpenConfiguration");
}
...
bool DatabaseSync::Open()
{
    ...
    int flags = open_config_.get_read_only() ? SQLITE_OPEN_READONLY : SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE;
    int r = sqlite3_open_v2(open_config_.location().c_str(), &connection_, flags | default_flags, nullptr);
    CHECK_ERROR_OR_THROW(env()->isolate(), this, r, SQLITE_OK, false);

    r = sqlite3_db_config(connection_, SQLITE_DBCONFIG_DQS_DML, static_cast<int>(open_config_.get_enable_dqs()), nullptr);
    CHECK_ERROR_OR_THROW(env()->isolate(), this, r, SQLITE_OK, false);
    r = sqlite3_db_config(connection_, SQLITE_DBCONFIG_DQS_DDL, static_cast<int>(open_config_.get_enable_dqs()), nullptr);
    CHECK_ERROR_OR_THROW(env()->isolate(), this, r, SQLITE_OK, false);

    int foreign_keys_enabled;
    r = sqlite3_db_config(connection_, SQLITE_DBCONFIG_ENABLE_FKEY, static_cast<int>(open_config_.get_enable_foreign_keys()), &foreign_keys_enabled);
    CHECK_ERROR_OR_THROW(env()->isolate(), this, r, SQLITE_OK, false);
    ...
    if (allow_load_extension_) {
        ...
        const int load_extension_ret = sqlite3_db_config(connection_, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION, 1, nullptr);
        CHECK_ERROR_OR_THROW(env()->isolate(), this, load_extension_ret, SQLITE_OK, false);
    }
...
std::optional<std::string> ValidateDatabasePath(Environment* env, Local<Value> path, const std::string& field_name)
{
    constexpr auto has_null_bytes = [](std::string_view str) { return str.find('\0') != std::string_view::npos; };
    if (path->IsString()) {
        ...
        auto data = static_cast<const uint8_t*>(buffer->Buffer()->Data()) + byteOffset;
        ...
        Local<Value> href;
        if (url->Get(env->context(), env->href_string()).ToLocal(&href) && href->IsString()) {
...
void DatabaseSync::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    std::optional<std::string> location = ValidateDatabasePath(env, args[0], "path");
    ...
    if (args.Length() > 1) {
        if (!args[1]->IsObject()) {
            THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options\" argument must be an object.");
            return;
        }
        ...
        if (!open_v->IsUndefined()) {
            if (!open_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.open\" argument must be a boolean.");
                return;
            }
            open = open_v.As<Boolean>()->Value();
        }

        Local<String> read_only_string = FIXED_ONE_BYTE_STRING(env->isolate(), "readOnly");
        ...
        if (!read_only_v->IsUndefined()) {
            if (!read_only_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.readOnly\" argument must be a boolean.");
                return;
            }
            open_config.set_read_only(read_only_v.As<Boolean>()->Value());
        }

        Local<String> enable_foreign_keys_string = FIXED_ONE_BYTE_STRING(env->isolate(), "enableForeignKeyConstraints");
        Local<Value> enable_foreign_keys_v;
        if (!options->Get(env->context(), enable_foreign_keys_string).ToLocal(&enable_foreign_keys_v)) {
            return;
        }
        if (!enable_foreign_keys_v->IsUndefined()) {
            ...
            open_config.set_enable_foreign_keys(enable_foreign_keys_v.As<Boolean>()->Value());
        }

        Local<String> enable_dqs_string = FIXED_ONE_BYTE_STRING(env->isolate(), "enableDoubleQuotedStringLiterals");
        ...
        Local<String> allow_extension_string = FIXED_ONE_BYTE_STRING(env->isolate(), "allowExtension");
        Local<Value> allow_extension_v;
        if (!options->Get(env->context(), allow_extension_string).ToLocal(&allow_extension_v)) {
            return;
        }

        if (!allow_extension_v->IsUndefined()) {
            if (!allow_extension_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.allowExtension\" argument must be a boolean.");
                return;
            }
    ...
    new DatabaseSync(env, args.This(), std::move(open_config), open, allow_load_extension);
}
...
void DatabaseSync::Prepare(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsString()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"sql\" argument must be a string.");
        return;
    }
    ...
    CHECK_ERROR_OR_THROW(env->isolate(), db, r, SQLITE_OK, void());
    BaseObjectPtr<StatementSync> stmt = StatementSync::Create(env, BaseObjectPtr<DatabaseSync>(db), s);
...
void DatabaseSync::CustomFunction(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsString()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"name\" argument must be a string.");
        return;
    }
    ...
    if (fn_index > 1) {
        ...
        if (!options->Get(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "useBigIntArguments")).ToLocal(&use_bigint_args_v)) {
            return;
        }

        if (!use_bigint_args_v->IsUndefined()) {
            if (!use_bigint_args_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.useBigIntArguments\" argument must be a boolean.");
                return;
            }
        ...
        if (!options->Get(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "varargs")).ToLocal(&varargs_v)) {
            return;
        }

        if (!varargs_v->IsUndefined()) {
            if (!varargs_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.varargs\" argument must be a boolean.");
                return;
            }
        ...
        if (!options->Get(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "deterministic")).ToLocal(&deterministic_v)) {
            return;
        }

        if (!deterministic_v->IsUndefined()) {
            if (!deterministic_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.deterministic\" argument must be a boolean.");
                return;
            }
        ...
        if (!options->Get(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "directOnly")).ToLocal(&direct_only_v)) {
            return;
        }

        if (!direct_only_v->IsUndefined()) {
            if (!direct_only_v->IsBoolean()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.directOnly\" argument must be a boolean.");
                return;
            }
    ...
    if (!args[fn_index]->IsFunction()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"function\" argument must be a function.");
        return;
    }
    ...
    if (varargs) {
        ...
        Local<Value> js_len;
        if (!fn->Get(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "length")).ToLocal(&js_len)) {
            return;
        }
        argc = js_len.As<Int32>()->Value();
    }

    UserDefinedFunction* user_data = new UserDefinedFunction(env, fn, db, use_bigint_args);
    ...
    int r = sqlite3_create_function_v2(
        db->connection_, *name, argc, text_rep, user_data, UserDefinedFunction::xFunc, nullptr, nullptr, UserDefinedFunction::xDestroy);
    CHECK_ERROR_OR_THROW(env->isolate(), db, r, SQLITE_OK, void());
}
...
void DatabaseSync::CreateSession(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() > 0) {
        ...
        if (hasIt) {
            ...
            if (table_value->IsString()) {
                ...
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.table\" argument must be a string.");
                return;
            }
        }
        ...
        if (hasIt) {
            ...
            if (db_value->IsString()) {
                ...
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.db\" argument must be a string.");
                return;
            }
        }
    }
    ...
    BaseObjectPtr<Session> session = Session::Create(env, BaseObjectWeakPtr<DatabaseSync>(db), pSession);
...
void Backup(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() < 1 || !args[0]->IsObject()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"sourceDb\" argument must be an object.");
        return;
    }
    ...
    THROW_AND_RETURN_ON_BAD_STATE(env, !db->IsOpen(), "database is not open");
    std::optional<std::string> dest_path = ValidateDatabasePath(env, args[1], "path");
    ...
    if (args.Length() > 2) {
        ...
        if (!rate_v->IsUndefined()) {
            if (!rate_v->IsInt32()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.rate\" argument must be an integer.");
                return;
            }
        ...
        if (!options->Get(env->context(), env->source_string()).ToLocal(&source_v)) {
            return;
        }

        if (!source_v->IsUndefined()) {
            if (!source_v->IsString()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.source\" argument must be a string.");
                return;
            }
        ...
        if (!options->Get(env->context(), env->target_string()).ToLocal(&target_v)) {
            return;
        }

        if (!target_v->IsUndefined()) {
            if (!target_v->IsString()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.target\" argument must be a string.");
                return;
            }
        ...
        if (!options->Get(env->context(), env->progress_string()).ToLocal(&progress_v)) {
            return;
        }

        if (!progress_v->IsUndefined()) {
            if (!progress_v->IsFunction()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.progress\" argument must be a function.");
                return;
            }
    ...
    BackupJob* job = new BackupJob(env, db, resolver, std::move(source_db), dest_path.value(), std::move(dest_db), rate, progressFunc);
    db->AddBackup(job);
    job->ScheduleBackup();
}
...
void DatabaseSync::ApplyChangeset(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsUint8Array()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"changeset\" argument must be a Uint8Array.");
        return;
    }
    ...
    if (args.Length() > 1 && !args[1]->IsUndefined()) {
        ...
        if (!options->Get(env->context(), env->onconflict_string()).ToLocal(&conflictValue)) {
            // An error will have been scheduled.
            return;
        }
        ...
        if (!conflictValue->IsUndefined()) {
            if (!conflictValue->IsFunction()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.onConflict\" argument must be a function.");
                return;
            }
            ...
            conflictCallback = [env, conflictFunc](int conflictType) -> int {
                ...
                TryCatch try_catch(env->isolate());
                Local<Value> result = conflictFunc->Call(env->context(), Null(env->isolate()), 1, argv).FromMaybe(Local<Value>());
                if (try_catch.HasCaught()) {
                    try_catch.ReThrow();
                    return SQLITE_CHANGESET_ABORT;
                }
        ...
        bool hasIt;
        if (!options->HasOwnProperty(env->context(), env->filter_string()).To(&hasIt)) {
            return;
        }
        if (hasIt) {
            Local<Value> filterValue;
            if (!options->Get(env->context(), env->filter_string()).ToLocal(&filterValue)) {
                // An error will have been scheduled.
                return;
            }

            if (!filterValue->IsFunction()) {
                THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"options.filter\" argument must be a function.");
                return;
            }
            ...
            filterCallback = [env, filterFunc](std::string item) -> bool {
                ...
                Local<Value> argv[] = { String::NewFromUtf8(env->isolate(), item.c_str(), NewStringType::kNormal).ToLocalChecked() };
                Local<Value> result = filterFunc->Call(env->context(), Null(env->isolate()), 1, argv).ToLocalChecked();
    ...
    int r = sqlite3changeset_apply(db->connection_, buf.length(), const_cast<void*>(static_cast<const void*>(buf.data())), xFilter, xConflict, nullptr);
    if (r == SQLITE_OK) {
...
void DatabaseSync::EnableLoadExtension(const FunctionCallbackInfo<Value>& args)
{
    DatabaseSync* db;
    ASSIGN_OR_RETURN_UNWRAP(&db, args.This());
    Environment* env = Environment::GetCurrent(args);
    if (!args[0]->IsBoolean()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"allow\" argument must be a boolean.");
        return;
    }
    ...
    if (db->allow_load_extension_ == false && enable == true) {
        THROW_ERR_INVALID_STATE(isolate,
            "Cannot enable extension loading because it was disabled at database "
            "creation.");
        return;
    }
    db->enable_load_extension_ = enable;
    const int load_extension_ret = sqlite3_db_config(db->connection_, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION, enable, nullptr);
...
void DatabaseSync::LoadExtension(const FunctionCallbackInfo<Value>& args)
{
    ...
    ASSIGN_OR_RETURN_UNWRAP(&db, args.This());
    Environment* env = Environment::GetCurrent(args);
    THROW_AND_RETURN_ON_BAD_STATE(env, db->connection_ == nullptr, "database is not open");
    THROW_AND_RETURN_ON_BAD_STATE(env, !db->allow_load_extension_, "extension loading is not allowed");
    THROW_AND_RETURN_ON_BAD_STATE(env, !db->enable_load_extension_, "extension loading is not allowed");

    if (!args[0]->IsString()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"path\" argument must be a string.");
        return;
    }
    ...
    char* errmsg = nullptr;
    const int r = sqlite3_load_extension(db->connection_, *path, *entryPoint, &errmsg);
    if (r != SQLITE_OK) {
        isolate->ThrowException(ERR_LOAD_SQLITE_EXTENSION(isolate, errmsg));
    }
}

StatementSync::StatementSync(Environment* env, Local<Object> object, BaseObjectPtr<DatabaseSync> db, sqlite3_stmt* stmt)
    : BaseObject(env, object)
    , db_(std::move(db))
{
    MakeWeak();
                ...
                if (insertion.second == false) {
                    ...
                    if (full_name != existing_full_name) {
                        THROW_ERR_INVALID_STATE(env(),
                            ...
                            bare_name, existing_full_name, full_name);
                        return false;
                    }
                }
            }
            ...
            if (r == 0) {
                if (allow_bare_named_params_) {
                    ...
                    if (lookup != bare_named_params_->end()) {
                        r = sqlite3_bind_parameter_index(statement_, lookup->second.c_str());
                    }
                }
                ...
                if (r == 0) {
                    if (allow_unknown_named_params_) {
                        ...
                        THROW_ERR_INVALID_STATE(env(), "Unknown named parameter '%s'", *utf8_key);
                        return false;
                    }
                }
            }
...
bool StatementSync::BindValue(const Local<Value>& value, const int index)
{
    ...
    if (value->IsNumber()) {
        ...
        THROW_ERR_INVALID_ARG_TYPE(env()->isolate(), "Provided value cannot be bound to SQLite parameter %d.", index);
        return false;
    }
...
MaybeLocal<Value> StatementSync::ColumnToValue(const int column)
{
    Isolate* isolate = env()->isolate();
    MaybeLocal<Value> js_val = MaybeLocal<Value>();
    SQLITE_VALUE_TO_JS(column, isolate, use_big_ints_, js_val, statement_, column);
    return js_val;
}
...
void StatementSync::All(const FunctionCallbackInfo<Value>& args)
{
    ...
    Environment* env = Environment::GetCurrent(args);
    THROW_AND_RETURN_ON_BAD_STATE(env, stmt->IsFinalized(), "statement has been finalized");
    Isolate* isolate = env->isolate();
    ...
    while ((r = sqlite3_step(stmt->statement_)) == SQLITE_ROW) {
        if (row_keys.size() == 0) {
            ...
            for (int i = 0; i < num_cols; ++i) {
                Local<Name> key;
                if (!stmt->ColumnNameToName(i).ToLocal(&key))
                    return;
        ...
        for (int i = 0; i < num_cols; ++i) {
            Local<Value> val;
            if (!stmt->ColumnToValue(i).ToLocal(&val))
                return;
...
void StatementSync::Iterate(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!js_iterator.As<Object>()->Get(context, env->prototype_string()).ToLocal(&js_iterator_prototype)) {
        return;
    }

    BaseObjectPtr<StatementSyncIterator> iter = StatementSyncIterator::Create(env, BaseObjectPtr<StatementSync>(stmt));
    if (iter->object()->GetPrototype().As<Object>()->SetPrototype(context, js_iterator_prototype).IsNothing()) {
        return;
    }
...
void StatementSync::Run(const FunctionCallbackInfo<Value>& args)
{
    ...
    sqlite3_int64 last_insert_rowid = sqlite3_last_insert_rowid(stmt->db_->Connection());
    ...
    if (result->Set(env->context(), env->last_insert_rowid_string(), last_insert_rowid_val).IsNothing()
        || result->Set(env->context(), env->changes_string(), changes_val).IsNothing()) {
        return;
    }
...
void StatementSync::Columns(const FunctionCallbackInfo<Value>& args)
{
    ...
    LocalVector<Name> col_keys(isolate, { env->column_string(), env->database_string(), env->name_string(), env->table_string(), env->type_string() });
    Local<Value> value;
    ...
    for (int i = 0; i < num_cols; ++i) {
        ...
        if (!NullableSQLiteStringToValue(isolate, sqlite3_column_origin_name(stmt->statement_, i)).ToLocal(&value)) {
            return;
        }
        col_values.emplace_back(value);

        if (!NullableSQLiteStringToValue(isolate, sqlite3_column_database_name(stmt->statement_, i)).ToLocal(&value)) {
            return;
        }
        col_values.emplace_back(value);
        ...
        if (!NullableSQLiteStringToValue(isolate, sqlite3_column_table_name(stmt->statement_, i)).ToLocal(&value)) {
            return;
        }
        col_values.emplace_back(value);

        if (!NullableSQLiteStringToValue(isolate, sqlite3_column_decltype(stmt->statement_, i)).ToLocal(&value)) {
            return;
        }
        col_values.emplace_back(value);

        Local<Object> column = Object::New(isolate, Null(isolate), col_keys.data(), col_values.data(), col_keys.size());
...
void StatementSync::SourceSQLGetter(const FunctionCallbackInfo<Value>& args)
{
    ...
    THROW_AND_RETURN_ON_BAD_STATE(env, stmt->IsFinalized(), "statement has been finalized");
    Local<String> sql;
    if (!String::NewFromUtf8(env->isolate(), sqlite3_sql(stmt->statement_)).ToLocal(&sql)) {
        return;
    }
...
void StatementSync::ExpandedSQLGetter(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (expanded == nullptr) {
        return THROW_ERR_SQLITE_ERROR(env->isolate(), "Expanded SQL text would exceed configured limits");
    }
...
void StatementSync::SetAllowBareNamedParameters(const FunctionCallbackInfo<Value>& args)
{
    StatementSync* stmt;
    ASSIGN_OR_RETURN_UNWRAP(&stmt, args.This());
    Environment* env = Environment::GetCurrent(args);
    ...
    if (!args[0]->IsBoolean()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"allowBareNamedParameters\" argument must be a boolean.");
        return;
    }
...
void StatementSync::SetAllowUnknownNamedParameters(const FunctionCallbackInfo<Value>& args)
{
    StatementSync* stmt;
    ASSIGN_OR_RETURN_UNWRAP(&stmt, args.This());
    Environment* env = Environment::GetCurrent(args);
    ...
    if (!args[0]->IsBoolean()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"enabled\" argument must be a boolean.");
        return;
    }
...
void StatementSync::SetReadBigInts(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsBoolean()) {
        THROW_ERR_INVALID_ARG_TYPE(env->isolate(), "The \"readBigInts\" argument must be a boolean.");
        return;
    }
...
static inline void SetSideEffectFreeGetter(Isolate* isolate, Local<FunctionTemplate> class_template, Local<String> name, FunctionCallback fn)
{
    Local<FunctionTemplate> getter = FunctionTemplate::New(isolate, fn, Local<Value>(), v8::Signature::New(isolate, class_template),
        /* length */ 0, ConstructorBehavior::kThrow, SideEffectType::kHasNoSideEffect);
    class_template->InstanceTemplate()->SetAccessorProperty(name, getter, Local<FunctionTemplate>(), DontDelete);
}

Local<FunctionTemplate> StatementSync::GetConstructorTemplate(Environment* env)
{
    ...
    if (tmpl.IsEmpty()) {
        ...
        tmpl->InstanceTemplate()->SetInternalFieldCount(StatementSync::kInternalFieldCount);
        ...
        SetProtoMethodNoSideEffect(isolate, tmpl, "columns", StatementSync::Columns);
        SetSideEffectFreeGetter(isolate, tmpl, FIXED_ONE_BYTE_STRING(isolate, "sourceSQL"), StatementSync::SourceSQLGetter);
        SetSideEffectFreeGetter(isolate, tmpl, FIXED_ONE_BYTE_STRING(isolate, "expandedSQL"), StatementSync::ExpandedSQLGetter);
        SetProtoMethod(isolate, tmpl, "setAllowBareNamedParameters", StatementSync::SetAllowBareNamedParameters);
        SetProtoMethod(isolate, tmpl, "setAllowUnknownNamedParameters", StatementSync::SetAllowUnknownNamedParameters);
        SetProtoMethod(isolate, tmpl, "setReadBigInts", StatementSync::SetReadBigInts);
        env->set_sqlite_statement_sync_constructor_template(tmpl);
    }
    return tmpl;
}

BaseObjectPtr<StatementSync> StatementSync::Create(Environment* env, BaseObjectPtr<DatabaseSync> db, sqlite3_stmt* stmt)
{
    Local<Object> obj;
    if (!GetConstructorTemplate(env)->InstanceTemplate()->NewInstance(env->context()).ToLocal(&obj)) {
...
StatementSyncIterator::StatementSyncIterator(Environment* env, Local<Object> object, BaseObjectPtr<StatementSync> stmt)
    : BaseObject(env, object)
    , stmt_(std::move(stmt))
{
    MakeWeak();
...
Local<FunctionTemplate> StatementSyncIterator::GetConstructorTemplate(Environment* env)
{
    Local<FunctionTemplate> tmpl = env->sqlite_statement_sync_iterator_constructor_template();
    if (tmpl.IsEmpty()) {
        Isolate* isolate = env->isolate();
        tmpl = NewFunctionTemplate(isolate, IllegalConstructor);
...
BaseObjectPtr<StatementSyncIterator> StatementSyncIterator::Create(Environment* env, BaseObjectPtr<StatementSync> stmt)
{
    Local<Object> obj;
    if (!GetConstructorTemplate(env)->InstanceTemplate()->NewInstance(env->context()).ToLocal(&obj)) {
...
void StatementSyncIterator::Next(const FunctionCallbackInfo<Value>& args)
{
    ...
    Environment* env = Environment::GetCurrent(args);
    THROW_AND_RETURN_ON_BAD_STATE(env, iter->stmt_->IsFinalized(), "statement has been finalized");
    Isolate* isolate = env->isolate();
    LocalVector<Name> keys(isolate, { env->done_string(), env->value_string() });

    if (iter->done_) {
        LocalVector<Value> values(isolate, { Boolean::New(isolate, true), Null(isolate) });
        Local<Object> result = Object::New(isolate, Null(isolate), keys.data(), values.data(), keys.size());
        args.GetReturnValue().Set(result);
        return;
    }
    ...
    if (r != SQLITE_ROW) {
        CHECK_ERROR_OR_THROW(env->isolate(), iter->stmt_->db_.get(), r, SQLITE_DONE, void());
    ...
    for (int i = 0; i < num_cols; ++i) {
        Local<Name> key;
        if (!iter->stmt_->ColumnNameToName(i).ToLocal(&key))
            return;
        Local<Value> val;
        if (!iter->stmt_->ColumnToValue(i).ToLocal(&val))
            return;
        row_keys.emplace_back(key);
        row_values.emplace_back(val);
    }
...
Session::Session(Environment* env, Local<Object> object, BaseObjectWeakPtr<DatabaseSync> database, sqlite3_session* session)
    : BaseObject(env, object)
    , session_(session)
...
BaseObjectPtr<Session> Session::Create(Environment* env, BaseObjectWeakPtr<DatabaseSync> database, sqlite3_session* session)
{
    Local<Object> obj;
    if (!GetConstructorTemplate(env)->InstanceTemplate()->NewInstance(env->context()).ToLocal(&obj)) {
...
Local<FunctionTemplate> Session::GetConstructorTemplate(Environment* env)
{
    ...
    if (tmpl.IsEmpty()) {
        ...
        tmpl->InstanceTemplate()->SetInternalFieldCount(Session::kInternalFieldCount);
        SetProtoMethod(isolate, tmpl, "changeset", Session::Changeset<sqlite3session_changeset>);
        SetProtoMethod(isolate, tmpl, "patchset", Session::Changeset<sqlite3session_patchset>);
...
template <Sqlite3ChangesetGenFunc sqliteChangesetFunc> void Session::Changeset(const FunctionCallbackInfo<Value>& args)
{
    Session* session;
    ASSIGN_OR_RETURN_UNWRAP(&session, args.This());
    Environment* env = Environment::GetCurrent(args);
    THROW_AND_RETURN_ON_BAD_STATE(env, !session->database_->IsOpen(), "database is not open");
    ...
    CHECK_ERROR_OR_THROW(env->isolate(), session->database_.get(), r, SQLITE_OK, void());
...
void Session::Delete()
{
    if (!database_ || !database_->connection_ || session_ == nullptr)
        return;
...
static void Initialize(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    ...
    Isolate* isolate = env->isolate();
    Local<FunctionTemplate> db_tmpl = NewFunctionTemplate(isolate, DatabaseSync::New);
    db_tmpl->InstanceTemplate()->SetInternalFieldCount(DatabaseSync::kInternalFieldCount);
    ...
    SetProtoMethod(isolate, db_tmpl, "enableLoadExtension", DatabaseSync::EnableLoadExtension);
    SetProtoMethod(isolate, db_tmpl, "loadExtension", DatabaseSync::LoadExtension);
    SetSideEffectFreeGetter(isolate, db_tmpl, FIXED_ONE_BYTE_STRING(isolate, "isOpen"), DatabaseSync::IsOpenGetter);
    SetConstructorFunction(context, target, "DatabaseSync", db_tmpl);
    SetConstructorFunction(context, target, "StatementSync", StatementSync::GetConstructorTemplate(env));
