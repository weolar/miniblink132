...
// support.
#define SET_OBJECT_ID(TypeId) static constexpr EmbedderObjectType type_int = EmbedderObjectType::k_##TypeId;

// Binding data should call this so that they can be looked up from the binding
// data store.
#define SET_BINDING_ID(TypeId)                                                                                                                                 \
    static constexpr BindingDataType binding_type_int = BindingDataType::k_##TypeId;                                                                           \
    SET_OBJECT_ID(TypeId)                                                                                                                                      \
    static_assert(static_cast<uint8_t>(type_int) == static_cast<uint8_t>(binding_type_int));
