...
void VerifySpkac(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!input.CheckSizeInt32())
        [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "spkac is too large");
...
void ExportChallenge(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto cert = ByteSource::Allocated(ncrypto::ExportChallenge(input.data(), input.size()));
    if (!cert)
        return args.GetReturnValue().SetEmptyString();

    Local<Value> outString = Encode(env->isolate(), cert.data<char>(), cert.size(), BUFFER);
...
void Initialize(Environment* env, Local<Object> target)
{
    ...
    SetMethodNoSideEffect(context, target, "certExportPublicKey", ExportPublicKey);
    SetMethodNoSideEffect(context, target, "certExportChallenge", ExportChallenge);
}
