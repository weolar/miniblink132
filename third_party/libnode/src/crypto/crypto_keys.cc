...
Maybe<EVPKeyPointer::AsymmetricKeyEncodingConfig> GetKeyFormatAndTypeFromJs(
    const FunctionCallbackInfo<Value>& args, unsigned int* offset, KeyEncodingContext context)
{
    ...
    if (args[*offset]->IsUndefined()) {
        ...
        config.format = static_cast<EVPKeyPointer::PKFormatType>(args[*offset].As<Int32>()->Value());

        if (args[*offset + 1]->IsInt32()) {
            config.type = static_cast<EVPKeyPointer::PKEncodingType>(args[*offset + 1].As<Int32>()->Value());
        } else {
            CHECK((context == kKeyContextInput && config.format == EVPKeyPointer::PKFormatType::PEM)
                || (context == kKeyContextGenerate && config.format == EVPKeyPointer::PKFormatType::JWK));
...
MaybeLocal<Value> ToV8Value(Environment* env, const BIOPointer& bio, const EVPKeyPointer::AsymmetricKeyEncodingConfig& config)
{
    if (!bio)
        return {};
    BUF_MEM* bptr = bio;
    if (config.format == EVPKeyPointer::PKFormatType::PEM) {
        ...
        return String::NewFromUtf8(env->isolate(), bptr->data, NewStringType::kNormal, bptr->length).FromMaybe(Local<Value>());
    }
...
MaybeLocal<Value> WritePrivateKey(Environment* env, const EVPKeyPointer& pkey, const EVPKeyPointer::PrivateKeyEncodingConfig& config)
{
    if (!pkey)
        return {};
    auto res = pkey.writePrivateKey(config);
    if (res)
        return ToV8Value(env, std::move(res.value), config);

    ThrowCryptoError(env, res.openssl_error.value_or(0), "Failed to encode private key");
    return MaybeLocal<Value>();
}

MaybeLocal<Value> WritePublicKey(Environment* env, const EVPKeyPointer& pkey, const EVPKeyPointer::PublicKeyEncodingConfig& config)
{
    if (!pkey)
        return {};
    auto res = pkey.writePublicKey(config);
    if (res)
        return ToV8Value(env, res.value, config);

    ThrowCryptoError(env, res.openssl_error.value_or(0), "Failed to encode public key");
    return MaybeLocal<Value>();
}

bool ExportJWKSecretKey(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    CHECK_EQ(key.GetKeyType(), kKeyTypeSecret);

    Local<Value> error;
    Local<Value> raw;
    if (!StringBytes::Encode(env->isolate(), key.GetSymmetricKey(), key.GetSymmetricKeySize(), BASE64URL, &error).ToLocal(&raw)) {
        DCHECK(!error.IsEmpty());
        env->isolate()->ThrowException(error);
        return false;
    }

    return target->Set(env->context(), env->jwk_kty_string(), env->jwk_oct_string()).IsJust() && target->Set(env->context(), env->jwk_k_string(), raw).IsJust();
}

KeyObjectData ImportJWKSecretKey(Environment* env, Local<Object> jwk)
{
    Local<Value> key;
    if (!jwk->Get(env->context(), env->jwk_k_string()).ToLocal(&key) || !key->IsString()) {
        THROW_ERR_CRYPTO_INVALID_JWK(env, "Invalid JWK secret key format");
        return {};
    }

    static_assert(String::kMaxLength <= INT_MAX);
    return KeyObjectData::CreateSecret(ByteSource::FromEncodedString(env, key.As<String>()));
}

bool ExportJWKAsymmetricKey(Environment* env, const KeyObjectData& key, Local<Object> target, bool handleRsaPss)
{
...
KeyObjectData ImportJWKAsymmetricKey(Environment* env, Local<Object> jwk, std::string_view kty, const FunctionCallbackInfo<Value>& args, unsigned int offset)
{
    if (kty == "RSA") {
    ...
    THROW_ERR_CRYPTO_INVALID_JWK(env, "%s is not a supported JWK key type", kty.data());
    return {};
}

bool GetSecretKeyDetail(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    // For the secret key detail, all we care about is the length,
    // converted to bits.
    return target->Set(env->context(), env->length_string(), Number::New(env->isolate(), static_cast<double>(key.GetSymmetricKeySize() * CHAR_BIT))).IsJust();
}

bool GetAsymmetricKeyDetail(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    if (!key) {
    ...
    case EVP_PKEY_DSA:
        return GetDsaKeyDetail(env, key, target);
    case EVP_PKEY_EC:
        return GetEcKeyDetail(env, key, target);
    case EVP_PKEY_DH:
        return GetDhKeyDetail(env, key, target);
    }
    THROW_ERR_CRYPTO_INVALID_KEYTYPE(env);
    return false;
}

KeyObjectData TryParsePrivateKey(Environment* env, const EVPKeyPointer::PrivateKeyEncodingConfig& config, const ncrypto::Buffer<const unsigned char>& buffer)
{
    auto res = EVPKeyPointer::TryParsePrivateKey(config, buffer);
    if (res) {
        return KeyObjectData::CreateAsymmetric(KeyType::kKeyTypePrivate, std::move(res.value));
    }
    ...
    if (res.error.value() == EVPKeyPointer::PKParseError::NEED_PASSPHRASE) {
        ...
        ThrowCryptoError(env, res.openssl_error.value_or(0), "Failed to read private key");
    }
    return {};
}

bool ExportJWKInner(Environment* env, const KeyObjectData& key, Local<Value> result, bool handleRsaPss)
{
    return key.GetKeyType() == kKeyTypeSecret ? ExportJWKSecretKey(env, key, result.As<Object>())
                                              : ExportJWKAsymmetricKey(env, key, result.As<Object>(), handleRsaPss);
}
...
bool KeyObjectData::ToEncodedPublicKey(Environment* env, const EVPKeyPointer::PublicKeyEncodingConfig& config, Local<Value>* out)
{
    CHECK(key_type_ != KeyType::kKeyTypeSecret);
    if (config.output_key_object) {
        // Note that this has the downside of containing sensitive data of the
        // private key.
        return KeyObjectHandle::Create(env, addRefWithType(KeyType::kKeyTypePublic)).ToLocal(out);
    } else if (config.format == EVPKeyPointer::PKFormatType::JWK) {
        *out = Object::New(env->isolate());
        return ExportJWKInner(env, addRefWithType(KeyType::kKeyTypePublic), *out, false);
    }

    return WritePublicKey(env, GetAsymmetricKey(), config).ToLocal(out);
}

bool KeyObjectData::ToEncodedPrivateKey(Environment* env, const EVPKeyPointer::PrivateKeyEncodingConfig& config, Local<Value>* out)
{
    CHECK(key_type_ != KeyType::kKeyTypeSecret);
    if (config.output_key_object) {
        return KeyObjectHandle::Create(env, addRefWithType(KeyType::kKeyTypePrivate)).ToLocal(out);
    } else if (config.format == EVPKeyPointer::PKFormatType::JWK) {
        *out = Object::New(env->isolate());
        return ExportJWKInner(env, addRefWithType(KeyType::kKeyTypePrivate), *out, false);
    }

    return WritePrivateKey(env, GetAsymmetricKey(), config).ToLocal(out);
}

Maybe<EVPKeyPointer::PrivateKeyEncodingConfig> KeyObjectData::GetPrivateKeyEncodingFromJs(
    const FunctionCallbackInfo<Value>& args, unsigned int* offset, KeyEncodingContext context)
{
...
Maybe<EVPKeyPointer::PublicKeyEncodingConfig> KeyObjectData::GetPublicKeyEncodingFromJs(
    const FunctionCallbackInfo<Value>& args, unsigned int* offset, KeyEncodingContext context)
{
    return GetKeyFormatAndTypeFromJs(args, offset, context);
}

KeyObjectData KeyObjectData::GetPrivateKeyFromJs(const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, bool allow_key_object)
{
    if (args[*offset]->IsString() || IsAnyBufferSource(args[*offset])) {
        ...
        EVPKeyPointer::PrivateKeyEncodingConfig config;
        if (!GetPrivateKeyEncodingFromJs(args, offset, kKeyContextInput).To(&config)) {
            return {};
        }

        return TryParsePrivateKey(env, config,
...
KeyObjectData KeyObjectData::GetPublicOrPrivateKeyFromJs(const FunctionCallbackInfo<Value>& args, unsigned int* offset)
{
    if (IsAnyBufferSource(args[*offset])) {
        Environment* env = Environment::GetCurrent(args);
        ...
        EVPKeyPointer::PrivateKeyEncodingConfig config;
        if (!KeyObjectData::GetPrivateKeyEncodingFromJs(args, offset, kKeyContextInput).To(&config)) {
            return {};
        }
        ...
        if (config.format == EVPKeyPointer::PKFormatType::PEM) {
            ...
            ThrowCryptoError(env, res.openssl_error.value_or(0), "Failed to read asymmetric key");
            return {};
        }

        // For DER, the type determines how to parse it. SPKI, PKCS#8 and SEC1 are
        // easy, but PKCS#1 can be a public key or a private key.
        static const auto is_public = [](const auto& config, const auto& buffer) -> bool {
            switch (config.type) {
    ...
    KeyObjectHandle* key = BaseObject::Unwrap<KeyObjectHandle>(args[*offset].As<Object>());
    CHECK_NOT_NULL(key);
...
KeyObjectData KeyObjectData::GetParsedKey(KeyType type, Environment* env, EVPKeyPointer&& pkey, ParseKeyResult ret, const char* default_msg)
{
...
void KeyObjectData::MemoryInfo(MemoryTracker* tracker) const
{
    ...
    case kKeyTypeSecret: {
        if (data_->symmetric_key) {
            tracker->TrackFieldWithSize("symmetric_key", data_->symmetric_key.size());
        }
        break;
    }
    ...
    case kKeyTypePublic: {
        if (data_->asymmetric_key) {
            tracker->TrackFieldWithSize("key", kSizeOf_EVP_PKEY + data_->asymmetric_key.rawPublicKeySize() + data_->asymmetric_key.rawPrivateKeySize());
        }
        break;
    }
    default:
...
KeyObjectData KeyObjectData::CreateAsymmetric(KeyType key_type, EVPKeyPointer&& pkey)
{
    CHECK(pkey);
...
Local<Function> KeyObjectHandle::Initialize(Environment* env)
{
    ...
    if (templ.IsEmpty()) {
        ...
        templ->InstanceTemplate()->SetInternalFieldCount(KeyObjectHandle::kInternalFieldCount);

        SetProtoMethod(isolate, templ, "init", Init);
        SetProtoMethodNoSideEffect(isolate, templ, "getSymmetricKeySize", GetSymmetricKeySize);
        SetProtoMethodNoSideEffect(isolate, templ, "getAsymmetricKeyType", GetAsymmetricKeyType);
        SetProtoMethodNoSideEffect(isolate, templ, "checkEcKeyData", CheckEcKeyData);
...
void KeyObjectHandle::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
...
MaybeLocal<Object> KeyObjectHandle::Create(Environment* env, const KeyObjectData& data)
{
    Local<Object> obj;
...
KeyObjectHandle::KeyObjectHandle(Environment* env, Local<Object> wrap)
    : BaseObject(env, wrap)
{
    MakeWeak();
}
...
void KeyObjectHandle::InitJWK(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> kty;
    if (!input->Get(env->context(), env->jwk_kty_string()).ToLocal(&kty) || !kty->IsString()) {
...
void KeyObjectHandle::InitEDRaw(const FunctionCallbackInfo<Value>& args)
{
    ...
    typedef EVPKeyPointer (*new_key_fn)(int, const ncrypto::Buffer<const unsigned char>&);
    new_key_fn fn = type == kKeyTypePrivate ? EVPKeyPointer::NewRawPrivate : EVPKeyPointer::NewRawPublic;
...
void KeyObjectHandle::Equals(const FunctionCallbackInfo<Value>& args)
{
    ...
    case kKeyTypeSecret: {
        ...
        if (size == key2.GetSymmetricKeySize()) {
            ret = CRYPTO_memcmp(key.GetSymmetricKey(), key2.GetSymmetricKey(), size) == 0;
        } else {
            ret = false;
        }
        break;
...
void KeyObjectHandle::GetKeyDetail(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (data.GetKeyType() == kKeyTypeSecret) {
        if (GetSecretKeyDetail(env, data, args[0].As<Object>()))
            [[likely]]
            {
    ...
    if (GetAsymmetricKeyDetail(env, data, args[0].As<Object>()))
        [[likely]]
        {
            args.GetReturnValue().Set(args[0]);
        }
...
void KeyObjectHandle::GetAsymmetricKeyType(const FunctionCallbackInfo<Value>& args)
{
    KeyObjectHandle* key;
    ASSIGN_OR_RETURN_UNWRAP(&key, args.This());
...
bool KeyObjectHandle::CheckEcKeyData() const
{
    ...
    return data_.GetKeyType() == kKeyTypePrivate ? ctx.privateCheck() : ctx.publicCheck();
}
...
void KeyObjectHandle::GetSymmetricKeySize(const FunctionCallbackInfo<Value>& args)
{
    KeyObjectHandle* key;
    ASSIGN_OR_RETURN_UNWRAP(&key, args.This());
    args.GetReturnValue().Set(static_cast<uint32_t>(key->Data().GetSymmetricKeySize()));
}
...
void KeyObjectHandle::Export(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (type == kKeyTypeSecret) {
        if (key->ExportSecretKey().ToLocal(&result))
            [[likely]]
            {
    ...
    if (type == kKeyTypePublic) {
        ...
        if (!KeyObjectData::GetPublicKeyEncodingFromJs(args, &offset, kKeyContextExport).To(&config)) {
            return;
        }
        CHECK_EQ(offset, static_cast<unsigned int>(args.Length()));
        if (key->ExportPublicKey(config).ToLocal(&result))
            [[likely]]
            {
                args.GetReturnValue().Set(result);
            }
    ...
    EVPKeyPointer::PrivateKeyEncodingConfig config;
    if (!KeyObjectData::GetPrivateKeyEncodingFromJs(args, &offset, kKeyContextExport).To(&config)) {
        return;
    }
    CHECK_EQ(offset, static_cast<unsigned int>(args.Length()));
    if (key->ExportPrivateKey(config).ToLocal(&result))
        [[likely]]
        {
            args.GetReturnValue().Set(result);
        }
...
{
    return Buffer::Copy(env(), data_.GetSymmetricKey(), data_.GetSymmetricKeySize()).FromMaybe(Local<Value>());
}

MaybeLocal<Value> KeyObjectHandle::ExportPublicKey(const EVPKeyPointer::PublicKeyEncodingConfig& config) const
{
    return WritePublicKey(env(), data_.GetAsymmetricKey(), config);
}

MaybeLocal<Value> KeyObjectHandle::ExportPrivateKey(const EVPKeyPointer::PrivateKeyEncodingConfig& config) const
{
    return WritePrivateKey(env(), data_.GetAsymmetricKey(), config);
}

void KeyObjectHandle::ExportJWK(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    KeyObjectHandle* key;
    ASSIGN_OR_RETURN_UNWRAP(&key, args.This());
...
{
    SetMethod(env->context(), target, "createNativeKeyObjectClass", NativeKeyObject::CreateNativeKeyObjectClass);
}

void NativeKeyObject::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
...
void NativeKeyObject::CreateNativeKeyObjectClass(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    Isolate* isolate = env->isolate();
    ...
    Local<FunctionTemplate> t = NewFunctionTemplate(isolate, NativeKeyObject::New);
    t->InstanceTemplate()->SetInternalFieldCount(KeyObjectHandle::kInternalFieldCount);
    ...
    Local<Value> ret_v;
    if (!callback.As<Function>()->Call(env->context(), recv, 1, &ctor).ToLocal(&ret_v)) {
        return;
    }
    Local<Array> ret = ret_v.As<Array>();
    if (!ret->Get(env->context(), 1).ToLocal(&ctor))
        return;
    env->set_crypto_key_object_secret_constructor(ctor.As<Function>());
    if (!ret->Get(env->context(), 2).ToLocal(&ctor))
        return;
    env->set_crypto_key_object_public_constructor(ctor.As<Function>());
    if (!ret->Get(env->context(), 3).ToLocal(&ctor))
        return;
...
BaseObjectPtr<BaseObject> NativeKeyObject::KeyObjectTransferData::Deserialize(
    Environment* env, Local<Context> context, std::unique_ptr<worker::TransferData> self)
{
    ...
    Local<Value> arg = FIXED_ONE_BYTE_STRING(env->isolate(), "internal/crypto/keys");
    if (env->builtin_module_require()->Call(context, Null(env->isolate()), 1, &arg).IsEmpty()) {
        return {};
    }
...
std::unique_ptr<worker::TransferData> NativeKeyObject::CloneForMessaging() const
{
    return std::make_unique<KeyObjectTransferData>(handle_data_);
}

WebCryptoKeyExportStatus PKEY_SPKI_Export(const KeyObjectData& key_data, ByteSource* out)
{
...
WebCryptoKeyExportStatus PKEY_PKCS8_Export(const KeyObjectData& key_data, ByteSource* out)
{
...
{
    target->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "KeyObjectHandle"), KeyObjectHandle::Initialize(env)).Check();

    constexpr int kKeyEncodingPKCS1 = static_cast<int>(EVPKeyPointer::PKEncodingType::PKCS1);
    constexpr int kKeyEncodingPKCS8 = static_cast<int>(EVPKeyPointer::PKEncodingType::PKCS8);
    constexpr int kKeyEncodingSPKI = static_cast<int>(EVPKeyPointer::PKEncodingType::SPKI);
    constexpr int kKeyEncodingSEC1 = static_cast<int>(EVPKeyPointer::PKEncodingType::SEC1);
    constexpr int kKeyFormatDER = static_cast<int>(EVPKeyPointer::PKFormatType::DER);
    constexpr int kKeyFormatPEM = static_cast<int>(EVPKeyPointer::PKFormatType::PEM);
    constexpr int kKeyFormatJWK = static_cast<int>(EVPKeyPointer::PKFormatType::JWK);
