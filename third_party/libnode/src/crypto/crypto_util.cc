...
bool ProcessFipsOptions()
{
    ...
    if (per_process::cli_options->enable_fips_crypto || per_process::cli_options->force_fips_crypto) {
#if OPENSSL_VERSION_MAJOR >= 3
...
void InitCryptoOnce()
{
    ...
    if (per_process::cli_options->secure_heap != 0) {
        switch (DataPointer::TryInitSecureHeap(per_process::cli_options->secure_heap, per_process::cli_options->secure_heap_min)) {
...
MaybeLocal<Value> cryptoErrorListToException(Environment* env, const CryptoErrorList& errors)
{
    ...
    if (!String::NewFromUtf8(env->isolate(), last.data(), NewStringType::kNormal, last.size()).ToLocal(&message)) {
        return {};
    }
    ...
    if (errors.size() > 1) {
        ...
        Local<v8::Array> stackArray = v8::Array::New(env->isolate(), stack.data(), stack.size());

        if (exception_obj->Set(env->context(), env->openssl_error_stack(), stackArray).IsNothing()) {
            return {};
        }
    }
    return exception;
...
MaybeLocal<Value> CryptoErrorStore::ToException(Environment* env, Local<String> exception_string) const
{
    if (exception_string.IsEmpty()) {
        ...
        if (!ToV8Value(env->context(), last_error_string).ToLocal(&exception_string)) {
    ...
    if (!Empty()) {
        ...
        Local<Value> stack;
        if (!ToV8Value(env->context(), errors_).ToLocal(&stack) || exception->Set(env->context(), env->openssl_error_stack(), stack).IsNothing()) {
            return MaybeLocal<Value>();
        }
    }
...
std::unique_ptr<BackingStore> ByteSource::ReleaseToBackingStore()
{
    ...
    std::unique_ptr<BackingStore> ptr = ArrayBuffer::NewBackingStore(
        allocated_data_, size(), [](void* data, size_t length, void* deleter_data) { OPENSSL_clear_free(deleter_data, length); }, allocated_data_);
    CHECK(ptr);
...
ByteSource ByteSource::FromEncodedString(Environment* env, Local<String> key, enum encoding enc)
{
    size_t length = 0;
    ByteSource out;
    ...
    if (StringBytes::Size(env->isolate(), key, enc).To(&length) && length > 0) {
        ...
        size_t actual = StringBytes::Write(env->isolate(), static_cast<char*>(buf.get()), length, key, enc);
...
ByteSource ByteSource::FromStringOrBuffer(Environment* env, Local<Value> value)
{
    return IsAnyBufferSource(value) ? FromBuffer(value) : FromString(env, value.As<String>());
}

ByteSource ByteSource::FromString(Environment* env, Local<String> str, bool ntc)
{
    CHECK(str->IsString());
    ...
    if (!ntc)
        opts |= String::NO_NULL_TERMINATION;
    str->WriteUtf8(env->isolate(), static_cast<char*>(out.get()), alloc_size, nullptr, opts);
    return ByteSource::Allocated(out.release());
}
...
ByteSource ByteSource::FromSecretKeyBytes(Environment* env, Local<Value> value)
{
    // A key can be passed as a string, buffer or KeyObject with type 'secret'.
    // If it is a string, we need to convert it to a buffer. We are not doing that
    // in JS to avoid creating an unprotected copy on the heap.
    return value->IsString() || IsAnyBufferSource(value) ? ByteSource::FromStringOrBuffer(env, value) : ByteSource::FromSymmetricKeyObjectHandle(value);
}

ByteSource ByteSource::NullTerminatedCopy(Environment* env, Local<Value> value)
{
    return Buffer::HasInstance(value) ? FromBuffer(value, true) : FromString(env, value.As<String>(), true);
}

ByteSource ByteSource::FromSymmetricKeyObjectHandle(Local<Value> handle)
{
    CHECK(handle->IsObject());
    KeyObjectHandle* key = BaseObject::Unwrap<KeyObjectHandle>(handle.As<Object>());
    CHECK_NOT_NULL(key);
    return Foreign(key->Data().GetSymmetricKey(), key->Data().GetSymmetricKeySize());
}
...
namespace error {
Maybe<void> Decorate(Environment* env, Local<Object> obj, unsigned long err)
{ // NOLINT(runtime/int)
    if (err == 0)
        return JustVoid(); // No decoration necessary.
    ...
    if (ls != nullptr) {
        if (obj->Set(context, env->library_string(), OneByteString(isolate, ls)).IsNothing()) {
            return Nothing<void>();
        }
    }
    if (fs != nullptr) {
        if (obj->Set(context, env->function_string(), OneByteString(isolate, fs)).IsNothing()) {
            return Nothing<void>();
        }
    }
    if (rs != nullptr) {
        if (obj->Set(context, env->reason_string(), OneByteString(isolate, rs)).IsNothing()) {
            return Nothing<void>();
        }
...
#undef OSSL_ERROR_CODES_MAP
        ...
        if (obj->Set(env->isolate()->GetCurrentContext(), env->code_string(), OneByteString(env->isolate(), code)).IsNothing())
            return Nothing<void>();
    }
    ...
    const char* message)
{
    ...
    errors.Capture();
    if (!errors.ToException(env, exception_string).ToLocal(&exception) || !exception->ToObject(env->context()).ToLocal(&obj)
...
void SetEngine(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(EnginePointer::getEngineByName(engine_id.ToStringView()).setAsDefault(flags));
}
#endif // !OPENSSL_NO_ENGINE
...
MaybeLocal<Value> EncodeBignum(Environment* env, const BIGNUM* bn, int size)
{
    ...
    Local<Value> ret;
    Local<Value> error;
    if (!StringBytes::Encode(env->isolate(), reinterpret_cast<const char*>(buf.get()), buf.size(), BASE64URL, &error).ToLocal(&ret)) {
        CHECK(!error.IsEmpty());
...
Maybe<void> SetEncodedValue(Environment* env, Local<Object> target, Local<String> name, const BIGNUM* bn, int size)
{
    Local<Value> value;
    CHECK_NOT_NULL(bn);
    if (size == 0)
        size = BignumPointer::GetByteCount(bn);
    if (!EncodeBignum(env, bn, size).ToLocal(&value)) {
        return Nothing<void>();
    }
    return target->Set(env->context(), name, value).IsJust() ? JustVoid() : Nothing<void>();
}
...
{
    Environment* env = Environment::GetCurrent(args);
    args.GetReturnValue().Set(BigInt::New(env->isolate(), DataPointer::GetSecureHeapUsed()));
}
} // namespace
...
void Initialize(Environment* env, Local<Object> target)
{
    ...
    SetMethodNoSideEffect(context, target, "getOpenSSLSecLevelCrypto", GetOpenSSLSecLevelCrypto);
}
