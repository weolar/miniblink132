...
void GetCipherInfo(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (mode_label.length()
        && info->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "mode"), OneByteString(env->isolate(), mode_label.data(), mode_label.length()))
               .IsNothing()) {
        return;
    }

    if (info->Set(env->context(), env->name_string(), OneByteString(env->isolate(), name.data(), name.length())).IsNothing()) {
        return;
    }

    if (info->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "nid"), Int32::New(env->isolate(), cipher.getNid())).IsNothing()) {
        return;
    }

    // Stream ciphers do not have a meaningful block size
    if (!cipher.isStreamMode()
        && info->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "blockSize"), Int32::New(env->isolate(), block_length)).IsNothing()) {
        return;
    }

    // Ciphers that do not use an IV shouldn't report a length
    if (iv_length != 0 && info->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "ivLength"), Int32::New(env->isolate(), iv_length)).IsNothing()) {
        return;
    }

    if (info->Set(env->context(), FIXED_ONE_BYTE_STRING(env->isolate(), "keyLength"), Int32::New(env->isolate(), key_length)).IsNothing()) {
        return;
    }
...
void CipherBase::GetSSLCiphers(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!ctx) {
        return ThrowCryptoError(env, clear_error_on_return.peekError(), "SSL_CTX_new");
    }
    ...
    ssl.getCiphers([&](const std::string_view name) { arr.push_back(OneByteString(env->isolate(), name.data(), name.length())); });
...
void CipherBase::GetCiphers(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!errored) {
        args.GetReturnValue().Set(Array::New(env->isolate(), ciphers.data(), ciphers.size()));
    }
}
...
void CipherBase::Initialize(Environment* env, Local<Object> target)
{
    ...
    SetMethod(context, target, "publicEncrypt", PublicKeyCipher::Cipher<PublicKeyCipher::kPublic, ncrypto::Cipher::encrypt>);
    SetMethod(context, target, "privateDecrypt", PublicKeyCipher::Cipher<PublicKeyCipher::kPrivate, ncrypto::Cipher::decrypt>);
    SetMethod(context, target, "privateEncrypt", PublicKeyCipher::Cipher<PublicKeyCipher::kPrivate, ncrypto::Cipher::sign>);
    SetMethod(context, target, "publicDecrypt", PublicKeyCipher::Cipher<PublicKeyCipher::kPublic, ncrypto::Cipher::recover>);
...
void CipherBase::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
    ...
    registry->Register(PublicKeyCipher::Cipher<PublicKeyCipher::kPublic, ncrypto::Cipher::encrypt>);
    registry->Register(PublicKeyCipher::Cipher<PublicKeyCipher::kPrivate, ncrypto::Cipher::decrypt>);
    registry->Register(PublicKeyCipher::Cipher<PublicKeyCipher::kPrivate, ncrypto::Cipher::sign>);
    registry->Register(PublicKeyCipher::Cipher<PublicKeyCipher::kPublic, ncrypto::Cipher::recover>);
...
void CipherBase::CommonInit(std::string_view cipher_type, const ncrypto::Cipher& cipher, const unsigned char* key, int key_len, const unsigned char* iv,
    int iv_len, unsigned int auth_tag_len)
{
    ...
    if (!ctx_.init(cipher, encrypt)) {
        return ThrowCryptoError(env(), mark_pop_error_on_return.peekError(), "Failed to initialize cipher");
    }
...
void CipherBase::Init(std::string_view cipher_type, const ArrayBufferOrViewContents<unsigned char>& key_buf, unsigned int auth_tag_len)
{
    ...
    if (kind_ == kCipher && (cipher.isCtrMode() || cipher.isGcmMode() || cipher.isCcmMode())) {
...
void CipherBase::InitIv(
    std::string_view cipher_type, const ByteSource& key_buf, const ArrayBufferOrViewContents<unsigned char>& iv_buf, unsigned int auth_tag_len)
{
    HandleScope scope(env()->isolate());
    MarkPopErrorOnReturn mark_pop_error_on_return;
    ...
    if (!cipher.isSupportedAuthenticatedMode() && has_iv && static_cast<int>(iv_buf.size()) != expected_iv_len) {
        return THROW_ERR_CRYPTO_INVALID_IV(env());
    }
    ...
    CommonInit(cipher_type, cipher, key_buf.data<unsigned char>(), key_buf.size(), iv_buf.data(), iv_buf.size(), auth_tag_len);
}
...
void CipherBase::InitIv(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (key_buf.size() > INT_MAX)
        [[unlikely]]
        {
            return THROW_ERR_OUT_OF_RANGE(env, "key is too big");
        }

    ArrayBufferOrViewContents<unsigned char> iv_buf(!args[2]->IsNull() ? args[2] : Local<Value>());

    if (!iv_buf.CheckSizeInt32())
        [[unlikely]]
        {
...
bool CipherBase::InitAuthenticated(std::string_view cipher_type, int iv_len, unsigned int auth_tag_len)
{
    ...
    if (ctx_.isGcmMode()) {
        if (auth_tag_len != kNoAuthTagLength) {
            if (!Cipher::IsValidGCMTagLength(auth_tag_len)) {
                THROW_ERR_CRYPTO_INVALID_AUTH_TAG(env(), "Invalid authentication tag length: %u", auth_tag_len);
                return false;
            }
        ...
        if (auth_tag_len == kNoAuthTagLength) {
            ...
            if (ctx_.isChaCha20Poly1305()) {
                ...
                THROW_ERR_CRYPTO_INVALID_AUTH_TAG(env(), "authTagLength required for %s", cipher_type);
                return false;
            }
        }
        ...
        if (ctx_.isCcmMode() && kind_ == kDecipher && ncrypto::isFipsEnabled()) {
            THROW_ERR_CRYPTO_UNSUPPORTED_OPERATION(env(), "CCM encryption not supported in FIPS mode");
            return false;
        }
...
void CipherBase::GetAuthTag(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (cipher->ctx_ || cipher->kind_ != kCipher || cipher->auth_tag_len_ == kNoAuthTagLength) {
        return;
    }

    Local<Value> ret;
    if (Buffer::Copy(env, cipher->auth_tag_, cipher->auth_tag_len_).ToLocal(&ret)) {
...
void CipherBase::SetAuthTag(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!cipher->ctx_ || !cipher->IsAuthenticatedMode() || cipher->kind_ != kDecipher || cipher->auth_tag_state_ != kAuthTagUnknown) {
        return args.GetReturnValue().Set(false);
    }

    ArrayBufferOrViewContents<char> auth_tag(args[0]);
    if (!auth_tag.CheckSizeInt32())
        [[unlikely]]
        {
    ...
    if (cipher->ctx_.isGcmMode()) {
        ...
        is_valid = (cipher->auth_tag_len_ == kNoAuthTagLength || cipher->auth_tag_len_ == tag_len) && Cipher::IsValidGCMTagLength(tag_len);
    } else {
    ...
    if (!is_valid) {
        return THROW_ERR_CRYPTO_INVALID_AUTH_TAG(env, "Invalid authentication tag length: %u", tag_len);
    }

    if (cipher->ctx_.isGcmMode() && cipher->auth_tag_len_ == kNoAuthTagLength && tag_len != 16 && env->EmitProcessEnvWarning()) {
        if (ProcessEmitDeprecationWarning(env,
...
bool CipherBase::SetAAD(const ArrayBufferOrViewContents<unsigned char>& data, int plaintext_len)
{
    ...
    if (ctx_.isCcmMode()) {
        if (plaintext_len < 0) {
            THROW_ERR_MISSING_ARGS(env(), "options.plaintextLength required for CCM mode with AAD");
            return false;
        }
...
CipherBase::UpdateResult CipherBase::Update(const char* data, size_t len, std::unique_ptr<BackingStore>* out)
{
    if (!ctx_ || len > INT_MAX)
        return kErrorState;
    MarkPopErrorOnReturn mark_pop_error_on_return;
    ...
    if (kind_ == kCipher && ctx_.isWrapMode() && !ctx_.update(buffer, nullptr, &buf_len)) {
        return kErrorState;
    }

    *out = ArrayBuffer::NewBackingStore(env()->isolate(), buf_len, BackingStoreInitializationMode::kUninitialized);
    ...
    bool r = ctx_.update(buffer, static_cast<unsigned char*>((*out)->Data()), &buf_len);
    ...
    if (buf_len == 0) {
        ...
        *out = ArrayBuffer::NewBackingStore(env()->isolate(), buf_len);
        memcpy(static_cast<char*>((*out)->Data()), static_cast<char*>(old_out->Data()), buf_len);
    }
...
{
    Decode<CipherBase>(args, [](CipherBase* cipher, const FunctionCallbackInfo<Value>& args, const char* data, size_t size) {
        MarkPopErrorOnReturn mark_pop_error_on_return;
        ...
        if (r != kSuccess) {
            if (r == kErrorState) {
                ThrowCryptoError(env, mark_pop_error_on_return.peekError(), "Trying to add data in unsupported state");
            }
            return;
        }

        auto ab = ArrayBuffer::New(env->isolate(), std::move(out));
        args.GetReturnValue().Set(Buffer::New(env, ab, 0, ab->ByteLength()).FromMaybe(Local<Value>()));
    });
}
...
bool CipherBase::Final(std::unique_ptr<BackingStore>* out)
{
    ...
    *out = ArrayBuffer::NewBackingStore(env()->isolate(), static_cast<size_t>(ctx_.getBlockSize()), BackingStoreInitializationMode::kUninitialized);

    if (kind_ == kDecipher && Cipher::FromCtx(ctx_).isSupportedAuthenticatedMode()) {
    ...
    if (kind_ == kDecipher && ctx_.isChaCha20Poly1305() && auth_tag_state_ != kAuthTagPassedToOpenSSL) {
        return false;
    }
#endif
    ...
    if (kind_ == kDecipher && ctx_.isCcmMode()) {
        ...
        *out = ArrayBuffer::NewBackingStore(env()->isolate(), 0);
        ...
        ok = ctx_.update({}, static_cast<unsigned char*>((*out)->Data()), &out_len, true);
        ...
        if (out_len == 0) {
            ...
            *out = ArrayBuffer::NewBackingStore(env()->isolate(), out_len);
            memcpy(static_cast<char*>((*out)->Data()), static_cast<char*>(old_out->Data()), out_len);
        }
        ...
        if (ok && kind_ == kCipher && IsAuthenticatedMode()) {
            ...
            ok = ctx_.getAeadTag(auth_tag_len_, reinterpret_cast<unsigned char*>(auth_tag_));
        }
    }
...
void CipherBase::Final(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!r) {
        const char* msg = is_auth_mode ? "Unsupported state or unable to authenticate data" : "Unsupported state";
...
bool PublicKeyCipher::Cipher(Environment* env, const EVPKeyPointer& pkey, int padding, const Digest& digest,
    const ArrayBufferOrViewContents<unsigned char>& oaep_label, const ArrayBufferOrViewContents<unsigned char>& data, std::unique_ptr<BackingStore>* out)
{
    ...
    if (buf.size() == 0) {
        ...
        *out = ArrayBuffer::NewBackingStore(env->isolate(), buf.size());
        memcpy(static_cast<char*>((*out)->Data()), static_cast<char*>(buf.get()), buf.size());
    }

    return true;
}

template <PublicKeyCipher::Operation operation, PublicKeyCipher::Cipher_t cipher> void PublicKeyCipher::Cipher(const FunctionCallbackInfo<Value>& args)
{
    MarkPopErrorOnReturn mark_pop_error_on_return;
    Environment* env = Environment::GetCurrent(args);
    ...
    if (cipher == ncrypto::Cipher::decrypt && operation == PublicKeyCipher::kPrivate && padding == RSA_PKCS1_PADDING) {
        ...
        if (!ctx.setRsaImplicitRejection())
            [[unlikely]]
            {
                return THROW_ERR_INVALID_ARG_VALUE(env, "RSA_PKCS1_PADDING is no longer supported for private decryption");
            }
    }
    ...
    ArrayBufferOrViewContents<unsigned char> oaep_label(!args[offset + 3]->IsUndefined() ? args[offset + 3] : Local<Value>());
    if (!oaep_label.CheckSizeInt32())
        [[unlikely]]
        {
