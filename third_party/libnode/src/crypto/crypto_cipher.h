...
protected:
    ...
    bool SetAAD(const ArrayBufferOrViewContents<unsigned char>& data, int plaintext_len);
...
enum class WebCryptoCipherStatus { OK, INVALID_KEY_TYPE, FAILED };
...
// use CipherJob.
template <typename CipherTraits> class CipherJob final : public CryptoJob<CipherTraits> {
public:
    ...
    static void New(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (CipherTraits::AdditionalConfig(mode, args, 4, cipher_mode, &params).IsNothing()) {
        ...
        new CipherJob<CipherTraits>(env, args.This(), mode, key, cipher_mode, data, std::move(params));
    }
    ...
    WebCryptoCipherMode cipher_mode() const
    {
        return cipher_mode_;
    }
    ...
    {
        const WebCryptoCipherStatus status = CipherTraits::DoCipher(AsyncWrap::env(), key(), cipher_mode_, *CryptoJob<CipherTraits>::params(), in_, &out_);
