...
MaybeLocal<Value> HKDFTraits::EncodeOutput(Environment* env, const HKDFConfig& params, ByteSource* out)
{
    return out->ToArrayBuffer(env);
}

Maybe<void> HKDFTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, HKDFConfig* params)
{
    ...
    params->info = mode == kCryptoJobAsync ? info.ToCopy() : info.ToByteSource();
...
bool HKDFTraits::DeriveBits(Environment* env, const HKDFConfig& params, ByteSource* out)
{
