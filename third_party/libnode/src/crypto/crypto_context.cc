...
namespace {
int SSL_CTX_use_certificate_chain(SSL_CTX* ctx, X509Pointer&& x, STACK_OF(X509) * extra_certs, X509Pointer* cert, X509Pointer* issuer_)
{
    CHECK(!*issuer_);
    CHECK(!*cert);
    X509* issuer = nullptr;
...
int SSL_CTX_use_certificate_chain(SSL_CTX* ctx, BIOPointer&& in, X509Pointer* cert, X509Pointer* issuer)
{
    // Just to ensure that `ERR_peek_last_error` below will return only errors
    // that we are interested in
    ERR_clear_error();

    X509Pointer x(PEM_read_bio_X509_AUX(in.get(), nullptr, NoPasswordCallback, nullptr));
    ...
    while (X509Pointer extra { PEM_read_bio_X509(in.get(), nullptr, NoPasswordCallback, nullptr) }) {
    ...
    return SSL_CTX_use_certificate_chain(ctx, std::move(x), extra_certs.get(), cert, issuer);
}

static unsigned long LoadCertsFromFile( // NOLINT(runtime/int)
    std::vector<X509*>* certs, const char* file)
{
    ...
    while (X509* x509 = PEM_read_bio_X509(bio.get(), nullptr, NoPasswordCallback, nullptr)) {
...
// details.
#ifdef __APPLE__
TrustStatus IsTrustDictionaryTrustedForPolicy(CFDictionaryRef trust_dict, bool is_self_issued)
{
    ...
    if (CFDictionaryContainsKey(trust_dict, kSecTrustSettingsPolicy)) {
        SecPolicyRef policy_ref = reinterpret_cast<SecPolicyRef>(const_cast<void*>(CFDictionaryGetValue(trust_dict, kSecTrustSettingsPolicy)));
        ...
        CFStringRef policy_oid = reinterpret_cast<CFStringRef>(const_cast<void*>(CFDictionaryGetValue(policy_dict, kSecPolicyOid)));
    ...
    if (CFDictionaryContainsKey(trust_dict, kSecTrustSettingsResult)) {
        CFNumberRef trust_settings_result_ref = reinterpret_cast<CFNumberRef>(const_cast<void*>(CFDictionaryGetValue(trust_dict, kSecTrustSettingsResult)));

        if (!trust_settings_result_ref || !CFNumberGetValue(trust_settings_result_ref, kCFNumberIntType, &trust_settings_result)) {
            return TrustStatus::UNSPECIFIED;
        }
        ...
        if (is_self_issued) {
            return trust_settings_result == kSecTrustSettingsResultTrustRoot || trust_settings_result == kSecTrustSettingsResultTrustAsRoot
                ? TrustStatus::TRUSTED
                : TrustStatus::UNSPECIFIED;
        }

        // kSecTrustSettingsResultTrustAsRoot can only be applied to non-root certs.
        return (trust_settings_result == kSecTrustSettingsResultTrustAsRoot) ? TrustStatus::TRUSTED : TrustStatus::UNSPECIFIED;
    }

    return TrustStatus::UNSPECIFIED;
}

TrustStatus IsTrustSettingsTrustedForPolicy(CFArrayRef trust_settings, bool is_self_issued)
{
    ...
    for (CFIndex i = 0; i < CFArrayGetCount(trust_settings); ++i) {
        CFDictionaryRef trust_dict = reinterpret_cast<CFDictionaryRef>(const_cast<void*>(CFArrayGetValueAtIndex(trust_settings, i)));

        TrustStatus trust = IsTrustDictionaryTrustedForPolicy(trust_dict, is_self_issued);
...
bool IsCertificateTrustValid(SecCertificateRef ref)
{
    ...
    OSStatus ortn = SecTrustCreateWithCertificates(subj_certs, policy, &sec_trust);
    bool result = false;
    if (ortn) {
        /* should never happen */
    } else {
...
bool IsCertificateTrustedForPolicy(X509* cert, SecCertificateRef ref)
{
    ...
    for (const auto& trust_domain : { kSecTrustSettingsDomainUser, kSecTrustSettingsDomainAdmin }) {
        ...
        if (err != errSecSuccess && err != errSecItemNotFound) {
            fprintf(stderr, "ERROR: failed to copy trust settings of system certificate%d\n", err);
            continue;
        }

        if (err == errSecSuccess && trust_settings != nullptr) {
            TrustStatus result = IsTrustSettingsTrustedForPolicy(trust_settings, is_self_issued);
...
void ReadMacOSKeychainCertificates(std::vector<X509*>* system_root_certificates_X509)
{
    CFTypeRef search_keys[] = { kSecClass, kSecMatchLimit, kSecReturnRef };
    CFTypeRef search_values[] = { kSecClassCertificate, kSecMatchLimitAll, kCFBooleanTrue };
    CFDictionaryRef search
        = CFDictionaryCreate(kCFAllocatorDefault, search_keys, search_values, 3, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);

    CFArrayRef curr_anchors = nullptr;
    OSStatus ortn = SecItemCopyMatching(search, reinterpret_cast<CFTypeRef*>(&curr_anchors));
    CFRelease(search);
    ...
    for (int i = 0; i < count; ++i) {
        SecCertificateRef cert_ref = reinterpret_cast<SecCertificateRef>(const_cast<void*>(CFArrayGetValueAtIndex(curr_anchors, i)));
        ...
        X509* cert = d2i_X509(nullptr, &data_buffer_pointer, CFDataGetLength(der_data));
        CFRelease(der_data);
...
void GatherCertsForLocation(std::vector<X509*>* vector, DWORD location, LPCWSTR store_name)
{
    ...
    HCERTSTORE opened_store(CertOpenStore(CERT_STORE_PROV_SYSTEM, 0,
    ...
    auto cleanup = OnScopeLeave([opened_store]() { CHECK_EQ(CertCloseStore(opened_store, 0), TRUE); });

    PCCERT_CONTEXT cert_from_store = nullptr;
    while ((cert_from_store = CertEnumCertificatesInStore(opened_store, cert_from_store)) != nullptr) {
        if (!IsCertTrustedForServerAuth(cert_from_store)) {
            continue;
        }
        const unsigned char* cert_data = reinterpret_cast<const unsigned char*>(cert_from_store->pbCertEncoded);
...
void ReadWindowsCertificates(std::vector<X509*>* system_root_certificates_X509)
{
    ...
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE, L"ROOT");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_GROUP_POLICY, L"ROOT");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE, L"ROOT");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_CURRENT_USER, L"ROOT");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_CURRENT_USER_GROUP_POLICY, L"ROOT");

    // Grab the intermediate certs
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE, L"CA");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_GROUP_POLICY, L"CA");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE, L"CA");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_CURRENT_USER, L"CA");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_CURRENT_USER_GROUP_POLICY, L"CA");
    ...
    // "current user" store.
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE, L"TrustedPeople");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_GROUP_POLICY, L"TrustedPeople");
    GatherCertsForLocation(system_root_certificates_X509, CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE, L"TrustedPeople");
}
#endif

static void LoadCertsFromDir(std::vector<X509*>* certs, std::string_view cert_dir)
{
    uv_fs_t dir_req;
    auto cleanup = OnScopeLeave([&dir_req]() { uv_fs_req_cleanup(&dir_req); });
    int err = uv_fs_scandir(nullptr, &dir_req, cert_dir.data(), 0, nullptr);
    if (err < 0) {
        fprintf(stderr, "Cannot open directory %s to load OpenSSL certificates.\n", cert_dir.data());
        return;
    }
    ...
    for (;;) {
        ...
        if (r < 0) {
            ...
            fprintf(stderr, "Cannot scan directory %s to load OpenSSL certificates.\n", cert_dir.data());
            return;
        }
        ...
        if (stats_r == 0 && (static_cast<uv_stat_t*>(stats_req.ptr)->st_mode & S_IFREG)) {
...
static std::vector<X509*> InitializeBundledRootCertificates()
{
    ...
    for (size_t i = 0; i < bundled_root_cert_count; i++) {
        X509* x509 = PEM_read_bio_X509(NodeBIO::NewFixed(root_certs[i], strlen(root_certs[i])).get(),
...
static std::vector<X509*>& GetBundledRootCertificates()
{
    ...
    static std::vector<X509*> bundled_root_certs = InitializeBundledRootCertificates();
...
{
    // Use function-local static to guarantee thread safety.
    static std::vector<X509*> system_store_certs = InitializeSystemStoreCertificates();
...
void GetBundledRootCertificates(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (size_t i = 0; i < arraysize(root_certs); i++) {
        if (!String::NewFromOneByte(env->isolate(), reinterpret_cast<const uint8_t*>(root_certs[i])).ToLocal(&result[i])) {
            return;
        }
    }

    args.GetReturnValue().Set(Array::New(env->isolate(), result, arraysize(root_certs)));
}

MaybeLocal<Array> X509sToArrayOfStrings(Environment* env, const std::vector<X509*>& certs)
{
    ...
    for (size_t i = 0; i < certs.size(); ++i) {
        ...
        if (!String::NewFromOneByte(env->isolate(), reinterpret_cast<uint8_t*>(pem_data), v8::NewStringType::kNormal, pem_size).ToLocal(&result[i])) {
            return MaybeLocal<Array>();
        }
    }
...
void GetSystemCACertificates(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    Local<Array> results;
    if (X509sToArrayOfStrings(env, GetSystemStoreCACertificates()).ToLocal(&results)) {
...
Local<FunctionTemplate> SecureContext::GetConstructorTemplate(Environment* env)
{
    Local<FunctionTemplate> tmpl = env->secure_context_constructor_template();
    if (tmpl.IsEmpty()) {
        Isolate* isolate = env->isolate();
        tmpl = NewFunctionTemplate(isolate, New);
        tmpl->InstanceTemplate()->SetInternalFieldCount(SecureContext::kInternalFieldCount);
        ...
        SetProtoMethod(isolate, tmpl, "enableTicketKeyCallback", EnableTicketKeyCallback);
        ...
        SetProtoMethodNoSideEffect(isolate, tmpl, "getIssuer", GetCertificate<false>);
...
#define SET_INTEGER_CONSTANTS(name, value) tmpl->Set(FIXED_ONE_BYTE_STRING(isolate, name), Integer::NewFromUnsigned(isolate, value));
...
#undef SET_INTEGER_CONSTANTS
        ...
        Local<FunctionTemplate> ctx_getter_templ = FunctionTemplate::New(isolate, CtxGetter, Local<Value>(), Signature::New(isolate, tmpl));

        tmpl->PrototypeTemplate()->SetAccessorProperty(
            FIXED_ONE_BYTE_STRING(isolate, "_external"), ctx_getter_templ, Local<FunctionTemplate>(), static_cast<PropertyAttribute>(ReadOnly | DontDelete));
...
void SecureContext::Initialize(Environment* env, Local<Object> target)
{
    ...
    SetConstructorFunction(context, target, "SecureContext", GetConstructorTemplate(env), SetConstructorFunctionFlag::NONE);

    SetMethodNoSideEffect(context, target, "getBundledRootCertificates", GetBundledRootCertificates);
    SetMethodNoSideEffect(context, target, "getSystemCACertificates", GetSystemCACertificates);
    SetMethodNoSideEffect(context, target, "getExtraCACertificates", GetExtraCACertificates);
}

void SecureContext::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
...
void SecureContext::Init(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[0]->IsString()) {
        ...
        if (sslmethod == "SSLv2_method" || sslmethod == "SSLv2_server_method" || sslmethod == "SSLv2_client_method") {
            THROW_ERR_TLS_INVALID_PROTOCOL_METHOD(env, "SSLv2 methods disabled");
            return;
        } else if (sslmethod == "SSLv3_method" || sslmethod == "SSLv3_server_method" || sslmethod == "SSLv3_client_method") {
            ...
            THROW_ERR_TLS_INVALID_PROTOCOL_METHOD(env, "Unknown method: %s", *sslmethod);
            return;
        }
    }
    ...
    SSL_CTX_set_session_cache_mode(sc->ctx_.get(), SSL_SESS_CACHE_CLIENT | SSL_SESS_CACHE_SERVER | SSL_SESS_CACHE_NO_INTERNAL | SSL_SESS_CACHE_NO_AUTO_CLEAR);
    ...
    if (!ncrypto::CSPRNG(sc->ticket_key_name_, sizeof(sc->ticket_key_name_)) || !ncrypto::CSPRNG(sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_))
        || !ncrypto::CSPRNG(sc->ticket_key_aes_, sizeof(sc->ticket_key_aes_))) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Error generating ticket keys");
    }
...
void SecureContext::SetEngineKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (env->permission()->enabled())
        [[unlikely]]
        {
            return THROW_ERR_CRYPTO_CUSTOM_ENGINE_NOT_SUPPORTED(env,
    ...
    auto engine = EnginePointer::getEngineByName(engine_id.ToStringView(), &errors);
    if (!engine) {
        Local<Value> exception;
        if (errors.empty()) {
            errors.add(getNodeCryptoErrorString(NodeCryptoError::ENGINE_NOT_FOUND, *engine_id));
        }
    ...
    if (!engine.init(true /* finish on exit*/)) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failure to initialize engine");
    }
...
Maybe<void> SecureContext::AddCert(Environment* env, BIOPointer&& bio)
{
    ...
    if (SSL_CTX_use_certificate_chain(ctx_.get(), std::move(bio), &cert_, &issuer_) == 0) {
...
{
    if (own_cert_store_cache_ != nullptr)
        return own_cert_store_cache_;
...
void SecureContext::SetCACert(const BIOPointer& bio)
{
    ...
    if (!bio)
        return;
    while (X509Pointer x509 = X509Pointer(PEM_read_bio_X509_AUX(bio.get(), nullptr, NoPasswordCallback, nullptr))) {
        CHECK_EQ(1, X509_STORE_add_cert(GetCertStoreOwnedByThisSecureContext(), x509));
...
Maybe<void> SecureContext::SetCRL(Environment* env, const BIOPointer& bio)
{
    ...
    DeleteFnPtr<X509_CRL, X509_CRL_free> crl(PEM_read_bio_X509_CRL(bio.get(), nullptr, NoPasswordCallback, nullptr));
    ...
    CHECK_EQ(1, X509_STORE_set_flags(cert_store, X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL));
    return JustVoid();
}
...
void SecureContext::SetDHParam(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (size < 1024) {
        return THROW_ERR_INVALID_ARG_VALUE(env, "DH parameter is less than 1024 bits");
    } else if (size < 2048) {
        args.GetReturnValue().Set(FIXED_ONE_BYTE_STRING(env->isolate(), "DH parameter is less than 2048 bits"));
    }

    if (!SSL_CTX_set_tmp_dh(sc->ctx_.get(), dh.get())) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Error setting temp DH parameter");
    }
}
...
void SecureContext::SetSessionIdContext(const FunctionCallbackInfo<Value>& args)
{
    SecureContext* sc;
    ASSIGN_OR_RETURN_UNWRAP(&sc, args.This());
    Environment* env = sc->env();
    ...
    if (!bio) {
        message = FIXED_ONE_BYTE_STRING(env->isolate(), "SSL_CTX_set_session_id_context error");
    } else {
...
void SecureContext::LoadPKCS12(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!in) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Unable to load PFX certificate");
    }
    ...
    if (!PKCS12_parse(p12.get(), pass.data(), &pkey_ptr, &cert_ptr, &extra_certs_ptr)) {
        goto done;
    }
    ...
    if (!pkey) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Unable to load private key from PFX data");
    }

    if (!cert) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Unable to load certificate from PFX data");
    }

    if (!SSL_CTX_use_certificate_chain(sc->ctx_.get(), std::move(cert), extra_certs.get(), &sc->cert_, &sc->issuer_)) {
        goto done;
    }
    ...
    if (!ret) {
        ...
        if (ERR_GET_REASON(err) == ERR_R_UNSUPPORTED) {
            ...
            return THROW_ERR_CRYPTO_UNSUPPORTED_OPERATION(env, "Unsupported PKCS12 PFX data");
        }
#endif
...
void SecureContext::EnableTicketKeyCallback(const FunctionCallbackInfo<Value>& args)
{
    SecureContext* wrap;
    ASSIGN_OR_RETURN_UNWRAP(&wrap, args.This());
...
int SecureContext::TicketKeyCallback(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, HMAC_CTX* hctx, int enc)
{
    static const int kTicketPartSize = 16;

    SecureContext* sc = static_cast<SecureContext*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)));
    ...
    if (!Buffer::Copy(env, reinterpret_cast<char*>(name), kTicketPartSize).ToLocal(&argv[0])
        || !Buffer::Copy(env, reinterpret_cast<char*>(iv), kTicketPartSize).ToLocal(&argv[1])) {
        return -1;
    }
    ...
    Local<Value> ret;
    if (!node::MakeCallback(env->isolate(), sc->object(), env->ticketkeycallback_string(), arraysize(argv), argv, { 0, 0 }).ToLocal(&ret) || !ret->IsArray()) {
        return -1;
    }
    Local<Array> arr = ret.As<Array>();

    Local<Value> val;
    if (!arr->Get(env->context(), kTicketKeyReturnIndex).ToLocal(&val) || !val->IsInt32()) {
        return -1;
    }
    ...
    if (!arr->Get(env->context(), kTicketKeyHMACIndex).ToLocal(&hmac) || !arr->Get(env->context(), kTicketKeyAESIndex).ToLocal(&aes)
    ...
    if (enc) {
        ...
        Local<Value> iv_val;
        if (!arr->Get(env->context(), kTicketKeyNameIndex).ToLocal(&name_val) || !arr->Get(env->context(), kTicketKeyIVIndex).ToLocal(&iv_val)
            || Buffer::Length(name_val) != kTicketPartSize || Buffer::Length(iv_val) != kTicketPartSize) {
            return -1;
        }
    ...
    HMAC_Init_ex(hctx, hmac_buf.data(), hmac_buf.length(), Digest::SHA256, nullptr);
...
int SecureContext::TicketCompatibilityCallback(SSL* ssl, unsigned char* name, unsigned char* iv, EVP_CIPHER_CTX* ectx, HMAC_CTX* hctx, int enc)
{
    SecureContext* sc = static_cast<SecureContext*>(SSL_CTX_get_app_data(SSL_get_SSL_CTX(ssl)));

    if (enc) {
        memcpy(name, sc->ticket_key_name_, sizeof(sc->ticket_key_name_));
        if (!ncrypto::CSPRNG(iv, 16) || EVP_EncryptInit_ex(ectx, Cipher::AES_128_CBC, nullptr, sc->ticket_key_aes_, iv) <= 0
            || HMAC_Init_ex(hctx, sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_), Digest::SHA256, nullptr) <= 0) {
            return -1;
        }
        return 1;
    }
    ...
    if (EVP_DecryptInit_ex(ectx, Cipher::AES_128_CBC, nullptr, sc->ticket_key_aes_, iv) <= 0
        || HMAC_Init_ex(hctx, sc->ticket_key_hmac_, sizeof(sc->ticket_key_hmac_), Digest::SHA256, nullptr) <= 0) {
        return -1;
    }
    return 1;
...
template <bool primary> void SecureContext::GetCertificate(const FunctionCallbackInfo<Value>& args)
{
    SecureContext* wrap;
    ASSIGN_OR_RETURN_UNWRAP(&wrap, args.This());
