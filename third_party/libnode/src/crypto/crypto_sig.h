...
public:
    enum class Error { Ok, UnknownDigest, Init, NotInitialised, Update, PrivateKey, PublicKey, MalformedSignature };
...
public:
    ...
    struct SignResult {
        ...
        inline explicit SignResult(Error err, std::unique_ptr<v8::BackingStore>&& sig = nullptr)
            : error(err)
    ...
    SignResult SignFinal(const ncrypto::EVPKeyPointer& pkey, int padding, std::optional<int> saltlen, DSASigEnc dsa_sig_enc);
...
public:
    ...
    Error VerifyFinal(const ncrypto::EVPKeyPointer& key, const ByteSource& sig, int padding, std::optional<int> saltlen, bool* verify_result);
...
struct SignConfiguration final : public MemoryRetainer {
    ...
    enum Flags { kHasNone = 0, kHasSaltLength = 1, kHasPadding = 2 };
...
struct SignTraits final {
    ...
    static v8::Maybe<void> AdditionalConfig(
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int offset, SignConfiguration* params);

    static bool DeriveBits(Environment* env, const SignConfiguration& params, ByteSource* out);

    static v8::MaybeLocal<v8::Value> EncodeOutput(Environment* env, const SignConfiguration& params, ByteSource* out);
};
