...
// string input.
template <typename T>
void Decode(const v8::FunctionCallbackInfo<v8::Value>& args, void (*callback)(T*, const v8::FunctionCallbackInfo<v8::Value>&, const char*, size_t))
{
    T* ctx;
...
template <typename... Args> std::string getNodeCryptoErrorString(const NodeCryptoError error, Args&&... args)
{
...
public:
    ...
    template <typename... Args> void Insert(const NodeCryptoError error, Args&&... args);

    v8::MaybeLocal<v8::Value> ToException(Environment* env, v8::Local<v8::String> exception_string = v8::Local<v8::String>()) const;
...
template <typename... Args> void CryptoErrorStore::Insert(const NodeCryptoError error, Args&&... args)
{
    const char* error_string = nullptr;
    switch (error) {
#define V(CODE, DESCRIPTION)                                                                                                                                   \
...
#undef V
    ...
    errors_.emplace_back(SPrintF(error_string, std::forward<Args>(args)...));
}

v8::MaybeLocal<v8::Value> cryptoErrorListToException(Environment* env, const ncrypto::CryptoErrorList& errors);

template <typename T> T* MallocOpenSSL(size_t count)
{
...
public:
    ...
    template <typename T = void> inline const T* data() const
    {
        return reinterpret_cast<const T*>(data_);
    }

    template <typename T = void> operator ncrypto::Buffer<const T>() const
    {
    ...
    template <typename T> static ByteSource Allocated(const ncrypto::Buffer<T>& buffer)
    {
    ...
    static ByteSource FromEncodedString(Environment* env, v8::Local<v8::String> value, enum encoding enc = BASE64);

    static ByteSource FromStringOrBuffer(Environment* env, v8::Local<v8::Value> value);

    static ByteSource FromString(Environment* env, v8::Local<v8::String> str, bool ntc = false);
    ...
    static ByteSource NullTerminatedCopy(Environment* env, v8::Local<v8::Value> value);

    static ByteSource FromSymmetricKeyObjectHandle(v8::Local<v8::Value> handle);

    static ByteSource FromSecretKeyBytes(Environment* env, v8::Local<v8::Value> value);
...
enum CryptoJobMode { kCryptoJobAsync, kCryptoJobSync };

CryptoJobMode GetCryptoJobMode(v8::Local<v8::Value> args);

template <typename CryptoJobTraits> class CryptoJob : public AsyncWrap, public ThreadPoolWork {
public:
    using AdditionalParams = typename CryptoJobTraits::AdditionalParameters;

    explicit CryptoJob(Environment* env, v8::Local<v8::Object> object, AsyncWrap::ProviderType type, CryptoJobMode mode, AdditionalParams&& params)
    ...
    virtual v8::Maybe<void> ToResult(v8::Local<v8::Value>* err, v8::Local<v8::Value>* result) = 0;

    CryptoJobMode mode() const
    {
        return mode_;
    }
    ...
    static void Run(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (job->ToResult(&ret[0], &ret[1]).IsJust()) {
            CHECK(!ret[0].IsEmpty());
            CHECK(!ret[1].IsEmpty());
            args.GetReturnValue().Set(v8::Array::New(env->isolate(), ret, arraysize(ret)));
        }
    }

    static void Initialize(v8::FunctionCallback new_fn, Environment* env, v8::Local<v8::Object> target)
    {
    ...
    static void RegisterExternalReferences(v8::FunctionCallback new_fn, ExternalReferenceRegistry* registry)
    {
...
template <typename DeriveBitsTraits> class DeriveBitsJob final : public CryptoJob<DeriveBitsTraits> {
public:
    ...
    static void New(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        AdditionalParams params;
        if (DeriveBitsTraits::AdditionalConfig(mode, args, 1, &params).IsNothing()) {
    ...
    DeriveBitsJob(Environment* env, v8::Local<v8::Object> object, CryptoJobMode mode, AdditionalParams&& params)
        : CryptoJob<DeriveBitsTraits>(env, object, DeriveBitsTraits::Provider, mode, std::move(params))
    {
    }

    void DoThreadPoolWork() override
    {
        if (!DeriveBitsTraits::DeriveBits(AsyncWrap::env(), *CryptoJob<DeriveBitsTraits>::params(), &out_)) {
    ...
    v8::Maybe<void> ToResult(v8::Local<v8::Value>* err, v8::Local<v8::Value>* result) override
    {
        ...
        if (success_) {
            ...
            *err = v8::Undefined(env->isolate());
            if (!DeriveBitsTraits::EncodeOutput(env, *CryptoJob<DeriveBitsTraits>::params(), &out_).ToLocal(result)) {
...
template <typename T> class ArrayBufferOrViewContents final {
public:
    ...
    template <typename M> void CopyTo(M* dest, size_t len) const
    {
...
v8::MaybeLocal<v8::Value> EncodeBignum(Environment* env, const BIGNUM* bn, int size);

v8::Maybe<void> SetEncodedValue(Environment* env, v8::Local<v8::Object> target, v8::Local<v8::String> name, const BIGNUM* bn, int size = 0);
