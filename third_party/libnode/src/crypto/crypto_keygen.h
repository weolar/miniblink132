...
enum class KeyGenJobStatus { OK, FAILED };
...
// mechanisms.
template <typename KeyGenTraits> class KeyGenJob final : public CryptoJob<KeyGenTraits> {
public:
    ...
    static void New(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (KeyGenTraits::AdditionalConfig(mode, args, &offset, &params).IsNothing()) {
    ...
    KeyGenJob(Environment* env, v8::Local<v8::Object> object, CryptoJobMode mode, AdditionalParams&& params)
        : CryptoJob<KeyGenTraits>(env, object, KeyGenTraits::Provider, mode, std::move(params))
    {
    }
...
template <typename KeyPairAlgorithmTraits> struct KeyPairGenTraits final {
    using AdditionalParameters = typename KeyPairAlgorithmTraits::AdditionalParameters;

    static const AsyncWrap::ProviderType Provider = AsyncWrap::PROVIDER_KEYPAIRGENREQUEST;
    static constexpr const char* JobName = KeyPairAlgorithmTraits::JobName;

    static v8::Maybe<void> AdditionalConfig(
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, AdditionalParameters* params)
    {
        ...
        if (KeyPairAlgorithmTraits::AdditionalConfig(mode, args, offset, params).IsNothing()
            || !KeyObjectData::GetPublicKeyEncodingFromJs(args, offset, kKeyContextGenerate).To(&params->public_key_encoding)
            || !KeyObjectData::GetPrivateKeyEncodingFromJs(args, offset, kKeyContextGenerate).To(&params->private_key_encoding)) {
            return v8::Nothing<void>();
        }
    ...
    static KeyGenJobStatus DoKeyGen(Environment* env, AdditionalParameters* params)
    {
        ...
        auto data = KeyObjectData::CreateAsymmetric(KeyType::kKeyTypePrivate, ncrypto::EVPKeyPointer(pkey));
        if (!data)
            [[unlikely]] return KeyGenJobStatus::FAILED;
        params->key = std::move(data);
        return KeyGenJobStatus::OK;
    }

    static v8::MaybeLocal<v8::Value> EncodeKey(Environment* env, AdditionalParameters* params)
    {
        v8::Local<v8::Value> keys[2];
        if (!params->key.ToEncodedPublicKey(env, params->public_key_encoding, &keys[0])
            || !params->key.ToEncodedPrivateKey(env, params->private_key_encoding, &keys[1])) {
            return {};
        }
...
struct SecretKeyGenTraits final {
    ...
    static const AsyncWrap::ProviderType Provider = AsyncWrap::PROVIDER_KEYGENREQUEST;
    static constexpr const char* JobName = "SecretKeyGenJob";

    static v8::Maybe<void> AdditionalConfig(
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, SecretKeyGenConfig* params);

    static KeyGenJobStatus DoKeyGen(Environment* env, SecretKeyGenConfig* params);

    static v8::MaybeLocal<v8::Value> EncodeKey(Environment* env, SecretKeyGenConfig* params);
};

template <typename AlgorithmParams> struct KeyPairGenConfig final : public MemoryRetainer {
    ...
    explicit KeyPairGenConfig(KeyPairGenConfig&& other) noexcept
        ...
        , private_key_encoding(std::forward<ncrypto::EVPKeyPointer::PrivateKeyEncodingConfig>(other.private_key_encoding))
    ...
    void MemoryInfo(MemoryTracker* tracker) const override
    {
        ...
        if (private_key_encoding.passphrase.has_value()) {
            ...
            tracker->TrackFieldWithSize("private_key_encoding.passphrase", passphrase.size());
        }
...
struct NidKeyPairGenTraits final {
    ...
    static v8::Maybe<void> AdditionalConfig(
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, NidKeyPairGenConfig* params);
};
