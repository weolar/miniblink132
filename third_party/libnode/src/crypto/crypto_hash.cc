...
void SaveSupportedHashAlgorithmsAndCacheMD(const EVP_MD* md, const char* from, const char* to, void* arg)
{
    if (!from)
        return;
...
#else
void SaveSupportedHashAlgorithms(const EVP_MD* md, const char* from, const char* to, void* arg)
{
    if (!from)
        return;
    Environment* env = static_cast<Environment*>(arg);
...
void Hash::GetCachedAliases(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Object> result = Object::New(isolate, prototype, names.data(), values.data(), size);
    args.GetReturnValue().Set(result);
}

const EVP_MD* GetDigestImplementation(Environment* env, Local<Value> algorithm, Local<Value> cache_id_val, Local<Value> algorithm_cache)
{
    ...
    if (result.cache_id != -1) {
        ...
        if (algorithm_cache.As<Object>()->Set(isolate->GetCurrentContext(), algorithm, v8::Int32::New(isolate, result.cache_id)).IsNothing()) {
            return nullptr;
        }
    }
...
void Hash::OneShotDigest(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (md == nullptr)
        [[unlikely]]
        {
            ...
            std::string message = "Digest method " + method.ToString() + " is not supported";
    ...
    Local<Value> error;
    MaybeLocal<Value> rc = StringBytes::Encode(env->isolate(), static_cast<const char*>(output.get()), output.size(), output_enc, &error);
    if (rc.IsEmpty())
        [[unlikely]]
        {
            CHECK(!error.IsEmpty());
...
void Hash::Initialize(Environment* env, Local<Object> target)
{
    ...
    SetMethodNoSideEffect(context, target, "internalVerifyIntegrity", InternalVerifyIntegrity);
}
...
{
    Decode<Hash>(args, [](Hash* hash, const FunctionCallbackInfo<Value>& args, const char* data, size_t size) {
        Environment* env = Environment::GetCurrent(args);
        if (size > INT_MAX)
...
void Hash::HashDigest(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> error;
    MaybeLocal<Value> rc = StringBytes::Encode(env->isolate(), hash->digest_.data<char>(), len, encoding, &error);
    if (rc.IsEmpty())
        [[unlikely]]
        {
            CHECK(!error.IsEmpty());
...
MaybeLocal<Value> HashTraits::EncodeOutput(Environment* env, const HashConfig& params, ByteSource* out)
{
    return out->ToArrayBuffer(env);
}

Maybe<void> HashTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, HashConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
    ...
    params->in = mode == kCryptoJobAsync ? data.ToCopy() : data.ToByteSource();

    unsigned int expected = EVP_MD_size(params->digest);
    params->length = expected;
    if (args[offset + 2]->IsUint32())
        [[unlikely]]
        {
            // length is expressed in terms of bits
            params->length = static_cast<uint32_t>(args[offset + 2].As<Uint32>()->Value()) / CHAR_BIT;
...
bool HashTraits::DeriveBits(Environment* env, const HashConfig& params, ByteSource* out)
{
...
void InternalVerifyIntegrity(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (md_type == nullptr || EVP_Digest(content.data(), content.size(), digest, &digest_size, md_type, nullptr) != 1)
        [[unlikely]]
        {
            return ThrowCryptoError(env, ERR_get_error(), "Digest method not supported");
        }

    if (digest_size != expected.size() || CRYPTO_memcmp(digest, expected.data(), digest_size) != 0) {
        Local<Value> error;
        MaybeLocal<Value> rc = StringBytes::Encode(env->isolate(), reinterpret_cast<const char*>(digest), digest_size, BASE64, &error);
        if (rc.IsEmpty())
            [[unlikely]]
            {
                CHECK(!error.IsEmpty());
