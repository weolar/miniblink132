...
WebCryptoCipherStatus AES_Cipher(
    Environment* env, const KeyObjectData& key_data, WebCryptoCipherMode cipher_mode, const AESCipherConfig& params, const ByteSource& in, ByteSource* out)
{
    ...
    if (!ctx.setKeyLength(key_data.GetSymmetricKeySize())
        || !ctx.init(Cipher(), encrypt, reinterpret_cast<const unsigned char*>(key_data.GetSymmetricKey()), params.iv.data<unsigned char>())) {
        return WebCryptoCipherStatus::FAILED;
    }
    ...
    if (params.cipher.isGcmMode() && params.additional_data.size() && !ctx.update(buffer, nullptr, &out_len)) {
        return WebCryptoCipherStatus::FAILED;
    }
...
BignumPointer GetCounter(const AESCipherConfig& params)
{
    ...
    unsigned int byte_length = CeilDiv(params.length, static_cast<size_t>(CHAR_BIT));

    std::vector<unsigned char> counter(data + params.iv.size() - byte_length, data + params.iv.size());
    counter[0] &= ~(0xFF << remainder);

    return BignumPointer(counter.data(), counter.size());
}

std::vector<unsigned char> BlockWithZeroedCounter(const AESCipherConfig& params)
{
...
WebCryptoCipherStatus AES_CTR_Cipher2(const KeyObjectData& key_data, WebCryptoCipherMode cipher_mode, const AESCipherConfig& params, const ByteSource& in,
    unsigned const char* counter, unsigned char* out)
{
    auto ctx = CipherCtxPointer::New();
    if (!ctx) {
        return WebCryptoCipherStatus::FAILED;
    ...
    if (!ctx.init(params.cipher, cipher_mode == kWebCryptoCipherEncrypt, reinterpret_cast<const unsigned char*>(key_data.GetSymmetricKey()), counter)) {
        // Cipher init failed
        return WebCryptoCipherStatus::FAILED;
    }
    ...
    if (!ctx.update(buffer, out, &out_len) || !ctx.update({}, out + out_len, &final_len, true)) {
        return WebCryptoCipherStatus::FAILED;
    }

    return static_cast<unsigned>(out_len + final_len) != in.size() ? WebCryptoCipherStatus::FAILED : WebCryptoCipherStatus::OK;
}
    ...
    Environment* env, const KeyObjectData& key_data, WebCryptoCipherMode cipher_mode, const AESCipherConfig& params, const ByteSource& in, ByteSource* out)
{
    ...
    auto remaining_until_reset = BignumPointer::NewSub(num_counters, current_counter);
    ...
    if (remaining_until_reset >= num_output) {
        auto status = AES_CTR_Cipher2(key_data, cipher_mode, params, in, params.iv.data<unsigned char>(), static_cast<unsigned char*>(buf.get()));
    ...
    auto status = AES_CTR_Cipher2(key_data, cipher_mode, params, ByteSource::Foreign(in.data<char>(), input_size_part1), params.iv.data<unsigned char>(),
    ...
    status = AES_CTR_Cipher2(
        key_data, cipher_mode, params, ByteSource::Foreign(in.data<char>() + input_size_part1, in.size() - input_size_part1), new_counter_block.data(), ptr);
...
bool ValidateIV(Environment* env, CryptoJobMode mode, Local<Value> value, AESCipherConfig* params)
{
    ArrayBufferOrViewContents<char> iv(value);
    if (!iv.CheckSizeInt32())
        [[unlikely]]
        {
            THROW_ERR_OUT_OF_RANGE(env, "iv is too big");
            return false;
        }
    params->iv = (mode == kCryptoJobAsync) ? iv.ToCopy() : iv.ToByteSource();
    return true;
}

bool ValidateCounter(Environment* env, Local<Value> value, AESCipherConfig* params)
{
    CHECK(value->IsUint32()); // Length
    params->length = value.As<Uint32>()->Value();
    if (params->iv.size() != 16 || params->length == 0 || params->length > 128) {
...
bool ValidateAuthTag(Environment* env, CryptoJobMode mode, WebCryptoCipherMode cipher_mode, Local<Value> value, AESCipherConfig* params)
{
    switch (cipher_mode) {
    case kWebCryptoCipherDecrypt: {
        ...
        if (!tag_contents.CheckSizeInt32())
            [[unlikely]]
            {
                THROW_ERR_OUT_OF_RANGE(env, "tagLength is too big");
                return false;
            }
        params->tag = mode == kCryptoJobAsync ? tag_contents.ToCopy() : tag_contents.ToByteSource();
        break;
    }
    case kWebCryptoCipherEncrypt: {
        if (!value->IsUint32()) {
...
bool ValidateAdditionalData(Environment* env, CryptoJobMode mode, Local<Value> value, AESCipherConfig* params)
{
    // Additional Data
    if (IsAnyBufferSource(value)) {
        ArrayBufferOrViewContents<char> additional(value);
        if (!additional.CheckSizeInt32())
            [[unlikely]]
            {
                THROW_ERR_OUT_OF_RANGE(env, "additionalData is too big");
                return false;
            }
        params->additional_data = mode == kCryptoJobAsync ? additional.ToCopy() : additional.ToByteSource();
    }
    return true;
}
...
Maybe<void> AESCipherTraits::AdditionalConfig(
    CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, WebCryptoCipherMode cipher_mode, AESCipherConfig* params)
{
    ...
    params->variant = static_cast<AESKeyVariant>(args[offset].As<Uint32>()->Value());
...
#undef V
    ...
    if (!params->cipher.isWrapMode()) {
        ...
        if (params->cipher.isCtrMode()) {
            ...
            if (!ValidateAuthTag(env, mode, cipher_mode, args[offset + 2], params) || !ValidateAdditionalData(env, mode, args[offset + 3], params)) {
                return Nothing<void>();
            }
        }
    } else {
...
#define V(name, _, __)                                                                                                                                         \
    constexpr static auto kKeyVariantAES_##name = static_cast<int>(AESKeyVariant::name);                                                                       \
