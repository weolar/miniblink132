...
MaybeLocal<Value> ScryptTraits::EncodeOutput(Environment* env, const ScryptConfig& params, ByteSource* out)
{
    return out->ToArrayBuffer(env);
}

Maybe<void> ScryptTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, ScryptConfig* params)
{
    ...
    params->pass = mode == kCryptoJobAsync ? pass.ToCopy() : pass.ToByteSource();

    params->salt = mode == kCryptoJobAsync ? salt.ToCopy() : salt.ToByteSource();
    ...
    if (!ncrypto::checkScryptParams(params->N, params->r, params->p, params->maxmem)) {
        ...
        if (err != 0) {
            ...
            THROW_ERR_CRYPTO_INVALID_SCRYPT_PARAMS(env, "Invalid scrypt params: %s", buf);
        } else {
...
bool ScryptTraits::DeriveBits(Environment* env, const ScryptConfig& params, ByteSource* out)
{
    ...
    auto dp = ncrypto::scrypt(
        ...
        params.N, params.r, params.p, params.maxmem, params.length);
