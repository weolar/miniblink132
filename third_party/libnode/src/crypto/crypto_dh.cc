...
void New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[0]->IsInt32()) {
        ...
        if (!args[1]->IsInt32()) {
            return THROW_ERR_INVALID_ARG_TYPE(env, "Second argument must be an int32");
        }
    ...
    if (!arg0.CheckSizeInt32())
        [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "prime is too big");
    ...
    if (args[1]->IsInt32()) {
        ...
        if (!arg1.CheckSizeInt32())
            [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "generator is too big");
...
void GetPublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!dp) {
        return THROW_ERR_CRYPTO_INVALID_STATE(env, "No public key - did you forget to generate one?");
    }
    Local<Value> buffer;
    if (DataPointerToBuffer(env, std::move(dp)).ToLocal(&buffer)) {
        args.GetReturnValue().Set(buffer);
...
void GetPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!dp) {
        return THROW_ERR_CRYPTO_INVALID_STATE(env, "No private key - did you forget to generate one?");
    }
    Local<Value> buffer;
    if (DataPointerToBuffer(env, std::move(dp)).ToLocal(&buffer)) {
        args.GetReturnValue().Set(buffer);
...
void ComputeSecret(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!key_buf.CheckSizeInt32())
        [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "secret is too big");
    ...
    case DHPointer::CheckPublicKeyResult::CHECK_FAILED:
        return THROW_ERR_CRYPTO_INVALID_KEYTYPE(env, "Unspecified validation error");
...
void SetPublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!buf.CheckSizeInt32())
        [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "buf is too big");
...
void Check(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (result == DHPointer::CheckResult::CHECK_FAILED) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Checking DH parameters failed");
    }
...
//   * Cipher
//   * Passphrase
Maybe<void> DhKeyGenTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int* offset, DhKeyPairGenConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
...
EVPKeyCtxPointer DhKeyGenTraits::Setup(DhKeyPairGenConfig* params)
{
    ...
    if (BignumPointer* prime_fixed_value = std::get_if<BignumPointer>(&params->params.prime)) {
        ...
        if (!param_ctx.initForParamgen() || !param_ctx.setDhParameters(*prime_size, params->params.generator)) {
            return {};
        }
...
Maybe<void> DHKeyExportTraits::AdditionalConfig(const FunctionCallbackInfo<Value>& args, unsigned int offset, DHKeyExportConfig* params)
{
    return JustVoid();
}

WebCryptoKeyExportStatus DHKeyExportTraits::DoExport(const KeyObjectData& key_data, WebCryptoKeyFormat format, const DHKeyExportConfig& params, ByteSource* out)
{
...
Maybe<void> DHBitsTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, DHBitsConfig* params)
{
...
MaybeLocal<Value> DHBitsTraits::EncodeOutput(Environment* env, const DHBitsConfig& params, ByteSource* out)
{
    return out->ToArrayBuffer(env);
}

bool DHBitsTraits::DeriveBits(Environment* env, const DHBitsConfig& params, ByteSource* out)
{
    auto dp = DHPointer::stateless(params.private_key.GetAsymmetricKey(), params.public_key.GetAsymmetricKey());
    if (!dp) {
        bool can_throw = per_process::v8_initialized && Isolate::TryGetCurrent() != nullptr;
        if (can_throw) {
            unsigned long err = ERR_get_error(); // NOLINT(runtime/int)
            if (err)
                ThrowCryptoError(env, err, "diffieHellman failed");
        }
        return false;
    }
...
bool GetDhKeyDetail(Environment* env, const KeyObjectData& key, Local<Object> target)
{
...
void DiffieHellman::Initialize(Environment* env, Local<Object> target)
{
    ...
    auto make = [&](Local<String> name, FunctionCallback callback) {
        ...
        const PropertyAttribute attributes = static_cast<PropertyAttribute>(ReadOnly | DontDelete);

        t->InstanceTemplate()->SetInternalFieldCount(DiffieHellman::kInternalFieldCount);
        ...
        Local<FunctionTemplate> verify_error_getter_templ = FunctionTemplate::New(isolate, Check, Local<Value>(), Signature::New(env->isolate(), t),
            /* length */ 0, ConstructorBehavior::kThrow, SideEffectType::kHasNoSideEffect);

        t->InstanceTemplate()->SetAccessorProperty(env->verify_error_string(), verify_error_getter_templ, Local<FunctionTemplate>(), attributes);

        SetConstructorFunction(context, target, name, t);
    };

    make(FIXED_ONE_BYTE_STRING(env->isolate(), "DiffieHellman"), New);
    make(FIXED_ONE_BYTE_STRING(env->isolate(), "DiffieHellmanGroup"), DiffieHellmanGroup);
...
void DiffieHellman::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
