...
ManagedX509::ManagedX509(X509Pointer&& cert)
    : cert_(std::move(cert))
{
}
...
namespace {
MaybeLocal<Value> GetFingerprintDigest(Environment* env, const Digest& method, const X509View& cert)
{
...
template <const ncrypto::Digest& algo> void Fingerprint(const FunctionCallbackInfo<Value>& args)
{
...
{
    return String::NewFromUtf8(env->isolate(), val.data(), NewStringType::kNormal, val.size());
}
...
MaybeLocal<Value> ToV8Value(Local<Context> context, BIOPointer&& bio)
{
    ...
    BUF_MEM* mem = bio;
    Local<Value> ret;
    if (!String::NewFromUtf8(context->GetIsolate(), mem->data, NewStringType::kNormal, mem->length).ToLocal(&ret))
        return {};
    return ret;
}
...
MaybeLocal<Value> ToBuffer(Environment* env, BIOPointer* bio)
{
    ...
    auto backing = ArrayBuffer::NewBackingStore(
        mem->data, mem->length, [](void*, size_t, void* data) { BIOPointer free_me(static_cast<BIO*>(data)); }, bio->release());
    auto ab = ArrayBuffer::New(env->isolate(), std::move(backing));
    Local<Value> ret;
    if (!Buffer::New(env, ab, 0, ab->ByteLength()).ToLocal(&ret))
        return {};
    return ret;
}
...
MaybeLocal<Value> GetDer(Environment* env, const X509View& view)
{
    ...
    auto bio = view.toDER();
    if (!bio)
        [[unlikely]] return Undefined(env->isolate());
...
MaybeLocal<Value> GetSubjectAltNameString(Environment* env, const X509View& view)
{
    Local<Value> ret;
...
MaybeLocal<Value> GetSerialNumber(Environment* env, const X509View& view)
{
    if (auto serial = view.getSerialNumber()) {
        return OneByteString(env->isolate(), static_cast<unsigned char*>(serial.get()));
    }
    return Undefined(env->isolate());
}
...
MaybeLocal<Value> GetKeyUsage(Environment* env, const X509View& cert)
{
    ...
    bool res = cert.enumUsages([&](std::string_view view) { vec.push_back(OneByteString(env->isolate(), view)); });
    if (!res)
        return Undefined(env->isolate());
...
void PublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto key_data = KeyObjectData::CreateAsymmetric(kKeyTypePublic, std::move(result.value));
...
void CheckPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(cert->view().checkPrivateKey(key->Data().GetAsymmetricKey()));
}
...
void CheckPublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(cert->view().checkPublicKey(key->Data().GetAsymmetricKey()));
}
...
void CheckHost(const FunctionCallbackInfo<Value>& args)
{
    ...
    case X509View::CheckMatch::MATCH: { // Match!
        ...
        if (peername) {
            ret = OneByteString(env->isolate(), static_cast<const char*>(peername.get()), peername.size());
        }
...
void GetIssuerCert(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (issuer)
        args.GetReturnValue().Set(issuer->object());
}
...
template <typename T> bool Set(Environment* env, Local<Object> target, Local<Value> name, MaybeLocal<T> maybe_value)
{
    Local<Value> value;
...
template <typename T> bool Set(Environment* env, Local<Object> target, uint32_t index, MaybeLocal<T> maybe_value)
{
    Local<Value> value;
    if (!maybe_value.ToLocal(&value))
        [[unlikely]] return false;
...
static MaybeLocal<Value> GetX509NameObject(Environment* env, const X509Name& name)
{
    if (!name)
        return {};
    ...
    Local<Object> result = Object::New(env->isolate(), Null(env->isolate()), nullptr, nullptr, 0);
    if (result.IsEmpty())
        return {};

    for (auto i : name) {
        if (!ToV8Value(env, i.first).ToLocal(&v8_name) || !ToV8Value(env, i.second).ToLocal(&v8_value)) {
            return {};
        }
...
MaybeLocal<Object> GetPubKey(Environment* env, const ncrypto::Rsa& rsa)
{
    ...
    auto bs = ArrayBuffer::NewBackingStore(env->isolate(), size, BackingStoreInitializationMode::kUninitialized);
...
MaybeLocal<Value> GetECPubKey(Environment* env, const EC_GROUP* group, OSSL3_CONST EC_KEY* ec)
{
    const auto pubkey = ECKeyPointer::GetPublicKey(ec);
    if (pubkey == nullptr)
        [[unlikely]] return Undefined(env->isolate());

    return ECPointToBuffer(env, group, pubkey, EC_KEY_get_conv_form(ec)).FromMaybe(Local<Object>());
}
...
template <const char* (*nid2string)(int nid)> MaybeLocal<Value> GetCurveName(Environment* env, const int nid)
{
    std::string_view name = nid2string(nid);
    return name.size() ? MaybeLocal<Value>(OneByteString(env->isolate(), name)) : MaybeLocal<Value>(Undefined(env->isolate()));
}
...
MaybeLocal<Object> X509ToObject(Environment* env, const X509View& cert)
{
    ...
    if (!Set<Value>(env, info, env->subject_string(), GetX509NameObject(env, cert.getSubjectName()))
        || !Set<Value>(env, info, env->issuer_string(), GetX509NameObject(env, cert.getIssuerName()))
        || !Set<Value>(env, info, env->subjectaltname_string(), GetSubjectAltNameString(env, cert))
        || !Set<Value>(env, info, env->infoaccess_string(), GetInfoAccessString(env, cert))
        || !Set<Boolean>(env, info, env->ca_string(), Boolean::New(env->isolate(), cert.isCA())))
        [[unlikely]]
        {
            return {};
        }
    ...
    if (!cert.ifRsa([&](const ncrypto::Rsa& rsa) {
            ...
            if (!Set<Value>(env, info, env->modulus_string(), GetModulusString(env, pub_key.n))
                || !Set<Value>(env, info, env->bits_string(), Integer::New(env->isolate(), BignumPointer::GetBitCount(pub_key.n)))
                || !Set<Value>(env, info, env->exponent_string(), GetExponentString(env, pub_key.e))
    ...
    if (!cert.ifEc([&](const ncrypto::Ec& ec) {
            ...
            if (!Set<Value>(env, info, env->bits_string(), GetECGroupBits(env, group))
                || !Set<Value>(env, info, env->pubkey_string(), GetECPubKey(env, group, ec)))
                [[unlikely]]
                {
                    return false;
                }
            ...
            if (nid != 0)
                [[likely]]
                {
                    ...
                    if (!Set<Value>(env, info, env->asn1curve_string(), GetCurveName<OBJ_nid2sn>(env, nid))
    ...
    if (!Set<Value>(env, info, env->valid_from_string(), GetValidFrom(env, cert)) || !Set<Value>(env, info, env->valid_to_string(), GetValidTo(env, cert))
        || !Set<Value>(env, info, env->fingerprint_string(), GetFingerprintDigest(env, Digest::SHA1, cert))
        || !Set<Value>(env, info, env->fingerprint256_string(), GetFingerprintDigest(env, Digest::SHA256, cert))
        || !Set<Value>(env, info, env->fingerprint512_string(), GetFingerprintDigest(env, Digest::SHA512, cert))
        || !Set<Value>(env, info, env->ext_key_usage_string(), GetKeyUsage(env, cert))
        || !Set<Value>(env, info, env->serial_number_string(), GetSerialNumber(env, cert)) || !Set<Value>(env, info, env->raw_string(), GetDer(env, cert)))
        [[unlikely]]
        {
            return {};
        }
...
Local<FunctionTemplate> X509Certificate::GetConstructorTemplate(Environment* env)
{
    ...
    if (tmpl.IsEmpty()) {
        ...
        SetProtoMethodNoSideEffect(isolate, tmpl, "fingerprint", Fingerprint<Digest::SHA1>);
        SetProtoMethodNoSideEffect(isolate, tmpl, "fingerprint256", Fingerprint<Digest::SHA256>);
        SetProtoMethodNoSideEffect(isolate, tmpl, "fingerprint512", Fingerprint<Digest::SHA512>);
        ...
        SetProtoMethodNoSideEffect(isolate, tmpl, "checkPrivateKey", CheckPrivateKey);
...
MaybeLocal<Object> X509Certificate::New(Environment* env, X509Pointer cert, STACK_OF(X509) * issuer_chain)
{
    std::shared_ptr<ManagedX509> mcert(new ManagedX509(std::move(cert)));
    return New(env, std::move(mcert), issuer_chain);
}

MaybeLocal<Object> X509Certificate::New(Environment* env, std::shared_ptr<ManagedX509> cert, STACK_OF(X509) * issuer_chain)
{
    EscapableHandleScope scope(env->isolate());
    Local<Function> ctor;
...
MaybeLocal<Object> X509Certificate::GetCert(Environment* env, const SSLPointer& ssl)
{
...
MaybeLocal<Object> X509Certificate::GetPeerCert(Environment* env, const SSLPointer& ssl, GetPeerCertificateFlag flag)
{
    ClearErrorOnReturn clear_error_on_return;
    MaybeLocal<Object> maybe_cert;

    X509Pointer cert;
    if ((flag & GetPeerCertificateFlag::SERVER) == GetPeerCertificateFlag::SERVER) {
    ...
    return sk_X509_num(ssl_certs) ? New(env, std::move(cert), ssl_certs) : New(env, std::move(cert));
}
...
v8::MaybeLocal<v8::Value> X509Certificate::toObject(Environment* env, const X509View& cert)
{
    if (!cert)
        [[unlikely]] return {};
...
X509Certificate::X509Certificate(Environment* env, Local<Object> object, std::shared_ptr<ManagedX509> cert, STACK_OF(X509) * issuer_chain)
    ...
    , cert_(std::move(cert))
{
    ...
    if (issuer_chain != nullptr && sk_X509_num(issuer_chain)) {
        ...
        Local<Object> obj = sk_X509_num(issuer_chain) ? X509Certificate::New(env, std::move(cert), issuer_chain).ToLocalChecked()
                                                      : X509Certificate::New(env, std::move(cert)).ToLocalChecked();
...
BaseObjectPtr<BaseObject> X509Certificate::X509CertificateTransferData::Deserialize(
    Environment* env, Local<Context> context, std::unique_ptr<worker::TransferData> self)
{
    ...
    return BaseObjectPtr<BaseObject>(Unwrap<X509Certificate>(handle.As<Object>()));
}
...
std::unique_ptr<worker::TransferData> X509Certificate::CloneForMessaging() const
{
...
void X509Certificate::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
