...
MaybeLocal<Value> RandomBytesTraits::EncodeOutput(Environment* env, const RandomBytesConfig& params, ByteSource* unused)
{
    return Undefined(env->isolate());
}

Maybe<void> RandomBytesTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, RandomBytesConfig* params)
{
...
bool RandomBytesTraits::DeriveBits(Environment* env, const RandomBytesConfig& params, ByteSource* unused)
{
...
MaybeLocal<Value> RandomPrimeTraits::EncodeOutput(Environment* env, const RandomPrimeConfig& params, ByteSource* unused)
{
    size_t size = params.prime.byteLength();
    auto store = ArrayBuffer::NewBackingStore(env->isolate(), size);
    CHECK_EQ(size, BignumPointer::EncodePaddedInto(params.prime.get(), reinterpret_cast<unsigned char*>(store->Data()), size));
    return ArrayBuffer::New(env->isolate(), std::move(store));
}

Maybe<void> RandomPrimeTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, RandomPrimeConfig* params)
{
...
bool RandomPrimeTraits::DeriveBits(Environment* env, const RandomPrimeConfig& params, ByteSource* unused)
{
...
Maybe<void> CheckPrimeTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, CheckPrimeConfig* params)
{
    ...
    if (!params->candidate) {
        ThrowCryptoError(Environment::GetCurrent(args), ERR_get_error(), "BignumPointer");
        return Nothing<void>();
    }
...
bool CheckPrimeTraits::DeriveBits(Environment* env, const CheckPrimeConfig& params, ByteSource* out)
{
...
MaybeLocal<Value> CheckPrimeTraits::EncodeOutput(Environment* env, const CheckPrimeConfig& params, ByteSource* out)
{
