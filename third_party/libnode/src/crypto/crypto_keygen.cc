...
Maybe<void> NidKeyPairGenTraits::AdditionalConfig(
    CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int* offset, NidKeyPairGenConfig* params)
{
...
Maybe<void> SecretKeyGenTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int* offset, SecretKeyGenConfig* params)
{
...
KeyGenJobStatus SecretKeyGenTraits::DoKeyGen(Environment* env, SecretKeyGenConfig* params)
{
    auto bytes = DataPointer::Alloc(params->length);
    if (!ncrypto::CSPRNG(static_cast<unsigned char*>(bytes.get()), params->length)) {
...
MaybeLocal<Value> SecretKeyGenTraits::EncodeKey(Environment* env, SecretKeyGenConfig* params)
{
