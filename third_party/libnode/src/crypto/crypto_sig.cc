...
bool ApplyRSAOptions(const EVPKeyPointer& pkey, EVP_PKEY_CTX* pkctx, int padding, std::optional<int> salt_len)
{
...
std::unique_ptr<BackingStore> Node_SignFinal(Environment* env, EVPMDCtxPointer&& mdctx, const EVPKeyPointer& pkey, int padding, std::optional<int> pss_salt_len)
{
    ...
    if (pkctx.initForSign() > 0 && ApplyRSAOptions(pkey, pkctx.get(), padding, pss_salt_len) && pkctx.setSignatureMd(mdctx) && pkctx.signInto(data, &sig_buf))
        [[likely]]
        {
            ...
            if (sig_buf.len < sig->ByteLength()) {
                ...
                if (sig_buf.len > 0)
                    [[likely]]
                    {
                        memcpy(static_cast<char*>(new_sig->Data()), static_cast<char*>(sig->Data()), sig_buf.len);
                    }
...
std::unique_ptr<BackingStore> ConvertSignatureToP1363(Environment* env, const EVPKeyPointer& pkey, std::unique_ptr<BackingStore>&& signature)
{
    uint32_t n = pkey.getBytesOfRS().value_or(kNoDsaSignature);
    if (n == kNoDsaSignature)
        return std::move(signature);

    auto buf = ArrayBuffer::NewBackingStore(env->isolate(), 2 * n, BackingStoreInitializationMode::kUninitialized);
    ...
    if (!ncrypto::extractP1363(sig_buffer, static_cast<unsigned char*>(buf->Data()), n)) {
        return std::move(signature);
    }
...
// Returns the maximum size of each of the integers (r, s) of the DSA signature.
ByteSource ConvertSignatureToP1363(Environment* env, const EVPKeyPointer& pkey, const ByteSource& signature)
{
    ...
    if (!ncrypto::extractP1363(signature, out, n))
        [[unlikely]]
        {
            return {};
        }
...
void CheckThrow(Environment* env, SignBase::Error error)
{
    ...
    case SignBase::Error::PublicKey: {
        ...
        switch (error) {
        case SignBase::Error::Init:
            return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "EVP_SignInit_ex failed");
        case SignBase::Error::Update:
            return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "EVP_SignUpdate failed");
        case SignBase::Error::PrivateKey:
            return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "PEM_read_bio_PrivateKey failed");
        case SignBase::Error::PublicKey:
            return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "PEM_read_bio_PUBKEY failed");
        default:
            ABORT();
        }
    }
...
SignBase::Error SignBase::Init(std::string_view digest)
{
    ...
    if (!md)
        [[unlikely]] return Error::UnknownDigest;
...
{
    if (mdctx_ == nullptr)
        [[unlikely]] return Error::NotInitialised;
...
Sign::Sign(Environment* env, Local<Object> wrap)
    : SignBase(env, wrap)
{
}
...
void Sign::Initialize(Environment* env, Local<Object> target)
{
    ...
    constexpr int kSignJobModeSign = static_cast<int>(SignConfiguration::Mode::Sign);
    constexpr int kSignJobModeVerify = static_cast<int>(SignConfiguration::Mode::Verify);
...
{
    Decode<Sign>(args, [](Sign* sign, const FunctionCallbackInfo<Value>& args, const char* data, size_t size) {
        Environment* env = Environment::GetCurrent(args);
        if (size > INT_MAX)
            [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "data is too long");
        Error err = sign->Update(data, size);
        crypto::CheckThrow(sign->env(), err);
    });
}

Sign::SignResult Sign::SignFinal(const EVPKeyPointer& pkey, int padding, std::optional<int> salt_len, DSASigEnc dsa_sig_enc)
{
    if (!mdctx_)
        [[unlikely]]
        {
    ...
    auto buffer = Node_SignFinal(env(), std::move(mdctx), pkey, padding, salt_len);
...
{
    Decode<Verify>(args, [](Verify* verify, const FunctionCallbackInfo<Value>& args, const char* data, size_t size) {
        Environment* env = Environment::GetCurrent(args);
        if (size > INT_MAX)
            [[unlikely]]
            {
...
SignBase::Error Verify::VerifyFinal(const EVPKeyPointer& pkey, const ByteSource& sig, int padding, std::optional<int> saltlen, bool* verify_result)
{
    if (!mdctx_)
        [[unlikely]] return Error::NotInitialised;
    ...
    *verify_result = false;
    ...
    auto data = mdctx.digestFinal(mdctx.getExpectedSize());
    if (!data)
        [[unlikely]] return Error::PublicKey;
    ...
    if (pkctx)
        [[likely]]
        {
            ...
            if (init_ret == -2)
                [[unlikely]] return Error::PublicKey;
            if (init_ret > 0 && ApplyRSAOptions(pkey, pkctx.get(), padding, saltlen) && pkctx.setSignatureMd(mdctx)) {
...
void Verify::VerifyFinal(const FunctionCallbackInfo<Value>& args)
{
    ...
    bool verify_result;
    Error err = verify->VerifyFinal(key, signature, padding, salt_len, &verify_result);
    if (err != Error::Ok)
        [[unlikely]] return crypto::CheckThrow(env, err);
...
SignConfiguration& SignConfiguration::operator=(SignConfiguration&& other) noexcept
{
    if (&other == this)
        return *this;
...
Maybe<void> SignTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, SignConfiguration* params)
{
    ClearErrorOnReturn clear_error_on_return;
    Environment* env = Environment::GetCurrent(args);
    ...
    params->mode = static_cast<SignConfiguration::Mode>(args[offset].As<Uint32>()->Value());

    unsigned int keyParamOffset = offset + 1;
    if (params->mode == SignConfiguration::Mode::Verify) {
        auto data = KeyObjectData::GetPublicOrPrivateKeyFromJs(args, &keyParamOffset);
        if (!data)
            return Nothing<void>();
    ...
    if (args[offset + 7]->IsInt32()) { // Salt length
        ...
        params->salt_length = GetSaltLenFromJS(args[offset + 7]).value_or(params->salt_length);
    }
    if (args[offset + 8]->IsUint32()) { // Padding
        params->flags |= SignConfiguration::kHasPadding;
        params->padding = GetPaddingFromJS(params->key.GetAsymmetricKey(), args[offset + 8]);
    }
    ...
    if (params->mode == SignConfiguration::Mode::Verify) {
        ...
        if (UseP1363Encoding(akey, params->dsa_encoding)) {
            ...
            params->signature = mode == kCryptoJobAsync ? signature.ToCopy() : signature.ToByteSource();
        }
    }
...
bool SignTraits::DeriveBits(Environment* env, const SignConfiguration& params, ByteSource* out)
{
    ClearErrorOnReturn clear_error_on_return;
    ...
    int padding = params.flags & SignConfiguration::kHasPadding ? params.padding : key.getDefaultSignPadding();

    std::optional<int> salt_length = params.flags & SignConfiguration::kHasSaltLength ? std::optional<int>(params.salt_length) : std::nullopt;
...
MaybeLocal<Value> SignTraits::EncodeOutput(Environment* env, const SignConfiguration& params, ByteSource* out)
{
    switch (params.mode) {
    case SignConfiguration::Mode::Sign:
