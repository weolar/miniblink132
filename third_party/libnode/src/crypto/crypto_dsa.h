...
struct DsaKeyGenTraits final {
    ...
    static v8::Maybe<void> AdditionalConfig(
        CryptoJobMode mode, const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int* offset, DsaKeyPairGenConfig* params);
};
...
struct DSAKeyExportTraits final {
    ...
    static v8::Maybe<void> AdditionalConfig(const v8::FunctionCallbackInfo<v8::Value>& args, unsigned int offset, DSAKeyExportConfig* config);

    static WebCryptoKeyExportStatus DoExport(const KeyObjectData& key_data, WebCryptoKeyFormat format, const DSAKeyExportConfig& params, ByteSource* out);
};

using DSAKeyExportJob = KeyExportJob<DSAKeyExportTraits>;

bool GetDsaKeyDetail(Environment* env, const KeyObjectData& key, v8::Local<v8::Object> target);
