...
void Hmac::HmacInit(const char* hash_type, const char* key, int key_len)
{
    ...
    if (!md)
        [[unlikely]]
        {
            return THROW_ERR_CRYPTO_INVALID_DIGEST(env(), "Invalid digest: %s", hash_type);
        }
    if (key_len == 0) {
        key = "";
    }
...
{
    Decode<Hmac>(args, [](Hmac* hmac, const FunctionCallbackInfo<Value>& args, const char* data, size_t size) {
        Environment* env = Environment::GetCurrent(args);
        if (size > INT_MAX)
...
void Hmac::HmacDigest(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> error;
    MaybeLocal<Value> rc = StringBytes::Encode(env->isolate(), reinterpret_cast<const char*>(md_value), buf.len, encoding, &error);
    if (rc.IsEmpty())
        [[unlikely]]
        {
            CHECK(!error.IsEmpty());
...
Maybe<void> HmacTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, HmacConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
...
bool HmacTraits::DeriveBits(Environment* env, const HmacConfig& params, ByteSource* out)
{
    ...
    if (!ctx.init(key_buf, params.digest))
        [[unlikely]]
        {
            return false;
        }
...
MaybeLocal<Value> HmacTraits::EncodeOutput(Environment* env, const HmacConfig& params, ByteSource* out)
{
    switch (params.mode) {
    ...
    case SignConfiguration::Mode::Verify:
        return Boolean::New(
            env->isolate(), out->size() > 0 && out->size() == params.signature.size() && memcmp(out->data(), params.signature.data(), out->size()) == 0);
    }
    UNREACHABLE();
}
