...
void ECDH::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!key)
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to create key using named curve");
...
ECPointPointer ECDH::BufferToPoint(Environment* env, const EC_GROUP* group, Local<Value> buf)
{
    ArrayBufferOrViewContents<unsigned char> input(buf);
    if (!input.CheckSizeInt32())
        [[unlikely]]
        {
    ...
    if (!pub) {
        THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to allocate EC_POINT for a public key");
        return pub;
    }
...
void ECDH::ComputeSecret(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!pub) {
        args.GetReturnValue().Set(FIXED_ONE_BYTE_STRING(env->isolate(), "ERR_CRYPTO_ECDH_INVALID_PUBLIC_KEY"));
        return;
    }

    int field_size = EC_GROUP_get_degree(ecdh->group_);
    size_t out_len = (field_size + 7) / 8;
    auto bs = ArrayBuffer::NewBackingStore(env->isolate(), out_len, BackingStoreInitializationMode::kUninitialized);

    if (!ECDH_compute_key(bs->Data(), bs->ByteLength(), pub, ecdh->key_.get(), nullptr))
...
void ECDH::GetPublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (pub == nullptr)
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to get ECDH public key");
...
void ECDH::GetPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (b == nullptr)
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to get ECDH private key");

    auto bs = ArrayBuffer::NewBackingStore(env->isolate(), BignumPointer::GetByteCount(b), BackingStoreInitializationMode::kUninitialized);

    CHECK_EQ(bs->ByteLength(), BignumPointer::EncodePaddedInto(b, static_cast<unsigned char*>(bs->Data()), bs->ByteLength()));
...
void ECDH::SetPrivateKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!priv_buffer.CheckSizeInt32())
        [[unlikely]] return THROW_ERR_OUT_OF_RANGE(env, "key is too big");

    BignumPointer priv(priv_buffer.data(), priv_buffer.size());
    if (!priv) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to convert Buffer to BN");
    }

    if (!ecdh->IsKeyValidForCurve(priv)) {
        return THROW_ERR_CRYPTO_INVALID_KEYTYPE(env, "Private key is not valid for specified curve.");
    }
    ...
    if (!result) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to convert BN to a private key");
    }
    ...
    if (!pub.mul(ecdh->group_, priv_key)) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to generate ECDH public key");
    }

    if (!new_key.setPublicKey(pub)) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to set generated public key");
    }
...
void ECDH::SetPublicKey(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto pub = ECDH::BufferToPoint(env, ecdh->group_, args[0]);
    if (!pub) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to convert Buffer to EC_POINT");
    }

    if (!ecdh->key_.setPublicKey(pub)) {
        return THROW_ERR_CRYPTO_OPERATION_FAILED(env, "Failed to set EC_POINT as the public key");
    }
}
...
bool ECDH::IsKeyValidForCurve(const BignumPointer& private_key)
{
    ...
    CHECK(order);
    return EC_GROUP_get_order(group_, order.get(), nullptr) && private_key < order;
}
...
MaybeLocal<Value> ECDHBitsTraits::EncodeOutput(Environment* env, const ECDHBitsConfig& params, ByteSource* out)
{
    return out->ToArrayBuffer(env);
}

Maybe<void> ECDHBitsTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int offset, ECDHBitsConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
    ...
    if (private_key->Data().GetKeyType() != kKeyTypePrivate || public_key->Data().GetKeyType() != kKeyTypePublic) {
...
bool ECDHBitsTraits::DeriveBits(Environment* env, const ECDHBitsConfig& params, ByteSource* out)
{
    size_t len = 0;
    ...
    default: {
        ...
        if (ECDH_compute_key(static_cast<char*>(buf.get()), len, pub, private_key, nullptr) <= 0) {
            return false;
        }
...
EVPKeyCtxPointer EcKeyGenTraits::Setup(EcKeyPairGenConfig* params)
{
    ...
    default: {
        ...
        if (!param_ctx.initForParamgen() || !param_ctx.setEcParameters(params->params.curve_nid, params->params.param_encoding)) {
            return {};
        }
...
//   8. Cipher
//   9. Passphrase
Maybe<void> EcKeyGenTraits::AdditionalConfig(CryptoJobMode mode, const FunctionCallbackInfo<Value>& args, unsigned int* offset, EcKeyPairGenConfig* params)
{
    Environment* env = Environment::GetCurrent(args);
    ...
    if (params->params.param_encoding != OPENSSL_EC_NAMED_CURVE && params->params.param_encoding != OPENSSL_EC_EXPLICIT_CURVE) {
...
namespace {
WebCryptoKeyExportStatus EC_Raw_Export(const KeyObjectData& key_data, const ECKeyExportConfig& params, ByteSource* out)
{
    ...
    if (ec_key == nullptr) {
        ...
        size_t check_len = EC_POINT_point2oct(group, point, form, static_cast<unsigned char*>(data.get()), len, nullptr);
        if (check_len == 0)
            return WebCryptoKeyExportStatus::FAILED;
...
Maybe<void> ECKeyExportTraits::AdditionalConfig(const FunctionCallbackInfo<Value>& args, unsigned int offset, ECKeyExportConfig* params)
{
    return JustVoid();
}

WebCryptoKeyExportStatus ECKeyExportTraits::DoExport(const KeyObjectData& key_data, WebCryptoKeyFormat format, const ECKeyExportConfig& params, ByteSource* out)
{
    ...
    case kWebCryptoKeyFormatSPKI: {
        ...
        if (m_pkey.id() != EVP_PKEY_EC) {
            ...
            const size_t need = EC_POINT_point2oct(group, point, form, nullptr, 0, nullptr);
            if (need == 0)
                return WebCryptoKeyExportStatus::FAILED;
            auto data = DataPointer::Alloc(need);
            const size_t have = EC_POINT_point2oct(group, point, form, static_cast<unsigned char*>(data.get()), need, nullptr);
            if (have == 0)
                return WebCryptoKeyExportStatus::FAILED;
...
bool ExportJWKEcKey(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    ...
    if (!EC_POINT_get_affine_coordinates(group, pub, x.get(), y.get(), nullptr)) {
        ThrowCryptoError(env, ERR_get_error(), "Failed to get elliptic-curve point coordinates");
        return false;
    }

    if (target->Set(env->context(), env->jwk_kty_string(), env->jwk_ec_string()).IsNothing()) {
        return false;
    }

    if (SetEncodedValue(env, target, env->jwk_x_string(), x.get(), degree_bytes).IsNothing()
        || SetEncodedValue(env, target, env->jwk_y_string(), y.get(), degree_bytes).IsNothing()) {
        return false;
    }
    ...
    case NID_secp521r1:
        ...
        break;
    default: {
        THROW_ERR_CRYPTO_JWK_UNSUPPORTED_CURVE(env, "Unsupported JWK EC curve: %s.", OBJ_nid2sn(nid));
        return false;
    }
    }
    if (target->Set(env->context(), env->jwk_crv_string(), crv_name).IsNothing()) {
        return false;
    }

    if (key.GetKeyType() == kKeyTypePrivate) {
        auto pvt = ECKeyPointer::GetPrivateKey(ec);
        return SetEncodedValue(env, target, env->jwk_d_string(), pvt, degree_bytes).IsJust();
    }

    return true;
}

bool ExportJWKEdKey(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    Mutex::ScopedLock lock(key.mutex());
    ...
    static constexpr auto trySetKey = [](Environment* env, DataPointer data, Local<Object> target, Local<String> key) {
        Local<Value> encoded;
        Local<Value> error;
        if (!data)
            return false;
        const ncrypto::Buffer<const char> out = data;
        if (!StringBytes::Encode(env->isolate(), out.data, out.len, BASE64URL, &error).ToLocal(&encoded)
    ...
    return !(target->Set(env->context(), env->jwk_crv_string(), OneByteString(env->isolate(), curve)).IsNothing()
        || (key.GetKeyType() == kKeyTypePrivate && !trySetKey(env, pkey.rawPrivateKey(), target, env->jwk_d_string()))
        || !trySetKey(env, pkey.rawPublicKey(), target, env->jwk_x_string())
        || target->Set(env->context(), env->jwk_kty_string(), env->jwk_okp_string()).IsNothing());
}

KeyObjectData ImportJWKEcKey(Environment* env, Local<Object> jwk, const FunctionCallbackInfo<Value>& args, unsigned int offset)
{
    CHECK(args[offset]->IsString()); // curve name
    ...
    if (!jwk->Get(env->context(), env->jwk_x_string()).ToLocal(&x_value) || !jwk->Get(env->context(), env->jwk_y_string()).ToLocal(&y_value)
        || !jwk->Get(env->context(), env->jwk_d_string()).ToLocal(&d_value)) {
        return {};
    }

    if (!x_value->IsString() || !y_value->IsString() || (!d_value->IsUndefined() && !d_value->IsString())) {
...
bool GetEcKeyDetail(Environment* env, const KeyObjectData& key, Local<Object> target)
{
    Mutex::ScopedLock lock(key.mutex());
    const auto& m_pkey = key.GetAsymmetricKey();
    CHECK_EQ(m_pkey.id(), EVP_PKEY_EC);
    ...
    return target->Set(env->context(), env->named_curve_string(), OneByteString(env->isolate(), OBJ_nid2sn(nid))).IsJust();
}
