...
struct LocalConnection {
    static std::unique_ptr<InspectorSession> Connect(Agent* inspector, std::unique_ptr<InspectorSessionDelegate> delegate)
    {
...
template <typename ConnectionType> class JSBindingsConnection : public BaseObject {
public:
    class JSBindingsSessionDelegate : public InspectorSessionDelegate {
    public:
        JSBindingsSessionDelegate(Environment* env, JSBindingsConnection* connection)
            : env_(env)
        ...
        void SendMessageToFrontend(const v8_inspector::StringView& message) override
        {
            ...
            if (!String::NewFromTwoByte(isolate, message.characters16(), NewStringType::kNormal, message.length()).ToLocal(&argument))
                return;
        ...
        , callback_(env->isolate(), callback)
    {
        ...
        session_ = ConnectionType::Connect(inspector, std::make_unique<JSBindingsSessionDelegate>(env, this));
    }

    void OnMessage(Local<Value> value)
    {
        auto result = callback_.Get(env()->isolate())->Call(env()->context(), object(), 1, &value);
        (void)result;
    }
    ...
    static void Bind(Environment* env, Local<Object> target)
    {
        ...
        Local<FunctionTemplate> tmpl = NewFunctionTemplate(isolate, JSBindingsConnection::New);
        tmpl->InstanceTemplate()->SetInternalFieldCount(JSBindingsConnection::kInternalFieldCount);
        SetProtoMethod(isolate, tmpl, "dispatch", JSBindingsConnection::Dispatch);
        SetProtoMethod(isolate, tmpl, "disconnect", JSBindingsConnection::Disconnect);
        SetConstructorFunction(env->context(), target, ConnectionType::GetClassName(env), tmpl);
    }
    ...
    static void Dispatch(const FunctionCallbackInfo<Value>& info)
    {
        ...
        if (session->session_) {
            session->session_->Dispatch(ToInspectorString(env->isolate(), info[0])->string());
        }
    }
    ...
    void MemoryInfo(MemoryTracker* tracker) const override
    {
        ...
        tracker->TrackFieldWithSize("session", sizeof(*session_), "InspectorSession");
    }
...
void CallAndPauseOnStart(const FunctionCallbackInfo<v8::Value>& args)
{
    ...
    THROW_IF_INSUFFICIENT_PERMISSIONS(env, permission::PermissionScope::kInspector, "PauseOnNextJavascriptStatement");
    CHECK_GT(args.Length(), 1);
    CHECK(args[0]->IsFunction());
    SlicedArguments call_args(args, /* start */ 2);
    env->inspector_agent()->PauseOnNextJavascriptStatement("Break on start");
    Local<Value> ret;
    if (args[0].As<v8::Function>()->Call(env->context(), args[1], call_args.length(), call_args.out()).ToLocal(&ret)) {
...
void InspectorConsoleCall(const FunctionCallbackInfo<Value>& info)
{
    ...
    if (InspectorEnabled(env)) {
        ...
        if (!env->is_in_inspector_console_call()) {
            ...
            MaybeLocal<Value> ret = inspector_method.As<Function>()->Call(context, info.This(), call_args.length(), call_args.out());
    ...
    USE(node_method.As<Function>()->Call(context, info.This(), call_args.length(), call_args.out()));
}
...
template <void (Agent::*asyncTaskFn)(void*)> static void InvokeAsyncTaskFnWithId(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
...
void EmitProtocolEvent(const FunctionCallbackInfo<Value>& args)
{
    ...
    env->inspector_agent()->EmitProtocolEvent(args.GetIsolate()->GetCurrentContext(), ToInspectorString(env->isolate(), eventName)->string(), params);
}
...
void SetupNetworkTracking(const FunctionCallbackInfo<Value>& args)
{
    ...
    env->inspector_agent()->SetupNetworkTracking(enable_function, disable_function);
}
...
void Initialize(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    ...
    v8::Local<v8::Function> consoleCallFunc
        = NewFunctionTemplate(isolate, InspectorConsoleCall, v8::Local<v8::Signature>(), v8::ConstructorBehavior::kThrow, v8::SideEffectType::kHasSideEffect)
              ->GetFunction(context)
              .ToLocalChecked();
    ...
    SetMethod(context, target, "setConsoleExtensionInstaller", SetConsoleExtensionInstaller);
    ...
    SetMethod(context, target, "asyncTaskCanceled", InvokeAsyncTaskFnWithId<&Agent::AsyncTaskCanceled>);
    SetMethod(context, target, "asyncTaskStarted", InvokeAsyncTaskFnWithId<&Agent::AsyncTaskStarted>);
    SetMethod(context, target, "asyncTaskFinished", InvokeAsyncTaskFnWithId<&Agent::AsyncTaskFinished>);
    ...
    target->Set(context, console_string, binding->Get(context, console_string).ToLocalChecked()).Check();
...
NODE_BINDING_EXTERNAL_REFERENCE(inspector, node::inspector::RegisterExternalReferences)
