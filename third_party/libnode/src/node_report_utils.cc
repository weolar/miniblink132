...
static void ReportEndpoint(uv_handle_t* h, struct sockaddr* addr, const char* name, JSONWriter* writer, bool exclude_network)
{
    if (addr == nullptr) {
    ...
    const int port = ntohs(family == AF_INET ? reinterpret_cast<sockaddr_in*>(addr)->sin_port : reinterpret_cast<sockaddr_in6*>(addr)->sin6_port);
    ...
    if (family == AF_INET) {
        ...
        if (uv_ip4_name(reinterpret_cast<sockaddr_in*>(addr), ipbuf, sizeof(ipbuf)) == 0) {
            writer->json_keyvalue("ip4", ipbuf);
            if (host == nullptr)
                writer->json_keyvalue("host", ipbuf);
        }
    } else {
        char ipbuf[INET6_ADDRSTRLEN];
        if (uv_ip6_name(reinterpret_cast<sockaddr_in6*>(addr), ipbuf, sizeof(ipbuf)) == 0) {
...
void WalkHandle(uv_handle_t* h, void* arg, bool exclude_network = false)
{
    ...
    writer->json_keyvalue("address", ValueToHexString(reinterpret_cast<uint64_t>(h)));
    ...
    case UV_TIMER: {
        ...
        writer->json_keyvalue("firesInMsFromNow", static_cast<int64_t>(due - now));
    ...
    if (h->type == UV_TCP || h->type == UV_NAMED_PIPE || h->type == UV_TTY) {
        ...
        writer->json_keyvalue("readable", static_cast<bool>(uv_is_readable(&handle->stream)));
        writer->json_keyvalue("writable", static_cast<bool>(uv_is_writable(&handle->stream)));
    }
    if (h->type == UV_UDP) {
        writer->json_keyvalue("writeQueueSize", uv_udp_get_send_queue_size(reinterpret_cast<uv_udp_t*>(h)));
        writer->json_keyvalue("writeQueueCount", uv_udp_get_send_queue_count(reinterpret_cast<uv_udp_t*>(h)));
    }
    writer->json_end();
}
