...
public:
    ...
    std::unique_ptr<Value> ReleaseRoot()
    {
...
private:
    ...
    {
        AddValueToParent(BinaryValue::create(Binary::fromSpan(bytes.data(), bytes.size())));
    }
    ...
    struct ContainerState {
        ...
        explicit ContainerState(ListValue* list)
            : is_dict(false)
            , list(list)
        {
        }
...
std::unique_ptr<Value> JsonUtil::parseJSON(const std::string_view string)
{
    ...
    return ParseJSON(reinterpret_cast<const uint8_t*>(string.data()), string.size());
}
...
std::unique_ptr<Value> JsonUtil::parseJSON(v8_inspector::StringView string)
{
    if (string.length() == 0)
        return nullptr;
    if (string.is8Bit())
        return ParseJSON(string.characters8(), string.length());
