...
void AppendExceptionLine(Environment* env, v8::Local<v8::Value> er, v8::Local<v8::Message> message, enum ErrorHandlingMode mode);
...
#define V(code, type)                                                                                                                                          \
    template <typename... Args> inline v8::Local<v8::Object> code(v8::Isolate* isolate, const char* format, Args&&... args)                                    \
    {                                                                                                                                                          \
        std::string message;                                                                                                                                   \
        if (sizeof...(Args) == 0) {                                                                                                                            \
            message = format;                                                                                                                                  \
        ...
        v8::Local<v8::String> js_msg = v8::String::NewFromUtf8(isolate, message.c_str(), v8::NewStringType::kNormal, message.length()).ToLocalChecked();       \
        v8::Local<v8::Object> e = v8::Exception::type(js_msg)->ToObject(isolate->GetCurrentContext()).ToLocalChecked();                                        \
        e->Set(isolate->GetCurrentContext(), FIXED_ONE_BYTE_STRING(isolate, "code"), js_code).Check();                                                         \
        return e;                                                                                                                                              \
    }                                                                                                                                                          \
    template <typename... Args> inline void THROW_##code(v8::Isolate* isolate, const char* format, Args&&... args)                                             \
    {                                                                                                                                                          \
        isolate->ThrowException(code(isolate, format, std::forward<Args>(args)...));                                                                           \
    }                                                                                                                                                          \
    template <typename... Args> inline void THROW_##code(Environment* env, const char* format, Args&&... args)                                                 \
    {                                                                                                                                                          \
        THROW_##code(env->isolate(), format, std::forward<Args>(args)...);                                                                                     \
    }                                                                                                                                                          \
    template <typename... Args> inline void THROW_##code(Realm* realm, const char* format, Args&&... args)                                                     \
    {                                                                                                                                                          \
...
#define PREDEFINED_ERROR_MESSAGES(V)                                                                                                                           \
    ...
    V(ERR_BUFFER_CONTEXT_NOT_AVAILABLE, "Buffer is not available for the current Context")                                                                     \
    ...
    V(ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH, "Input buffers must have the same byte length")                                                                     \
    ...
    V(ERR_EXECUTION_ENVIRONMENT_NOT_AVAILABLE, "Context not associated with Node.js environment")                                                              \
    ...
    V(ERR_MISSING_PLATFORM_FOR_WORKER,                                                                                                                         \
        ...
        "creating Workers")                                                                                                                                    \
    V(ERR_NON_CONTEXT_AWARE_DISABLED, "Loading non context-aware native addons has been disabled")                                                             \
    V(ERR_SCRIPT_EXECUTION_INTERRUPTED, "Script execution was interrupted by `SIGINT`")                                                                        \
...
inline void THROW_ERR_SCRIPT_EXECUTION_TIMEOUT(Environment* env, int64_t timeout)
{
...
inline void THROW_ERR_REQUIRE_ASYNC_MODULE(Environment* env, v8::Local<v8::Value> filename, v8::Local<v8::Value> parent_filename)
{
    static constexpr const char* prefix = "require() cannot be used on an ESM graph with top-level await. Use "
...
{
    char message[128];
    snprintf(message, sizeof(message), "Cannot create a Buffer larger than 0x%zx bytes", Buffer::kMaxLength);
    return ERR_BUFFER_TOO_LARGE(isolate, message);
}

inline void THROW_ERR_SOURCE_PHASE_NOT_DEFINED(v8::Isolate* isolate, v8::Local<v8::String> url)
{
    std::string message = std::string(*v8::String::Utf8Value(isolate, url));
    return THROW_ERR_SOURCE_PHASE_NOT_DEFINED(isolate, "Source phase import object is not defined for module %s", message.c_str());
}

inline v8::Local<v8::Object> ERR_STRING_TOO_LONG(v8::Isolate* isolate)
{
    char message[128];
    snprintf(message, sizeof(message), "Cannot create a string longer than 0x%x characters", v8::String::kMaxLength);
...
#define THROW_AND_RETURN_IF_NOT_BUFFER(env, val, prefix)                                                                                                       \
    do {                                                                                                                                                       \
        if (!Buffer::HasInstance(val))                                                                                                                         \
            return node::THROW_ERR_INVALID_ARG_TYPE(env, prefix " must be a buffer");                                                                          \
    } while (0)

#define THROW_AND_RETURN_IF_NOT_STRING(env, val, prefix)                                                                                                       \
    do {                                                                                                                                                       \
        if (!val->IsString())                                                                                                                                  \
            return node::THROW_ERR_INVALID_ARG_TYPE(env, prefix " must be a string");                                                                          \
    } while (0)
...
void TriggerUncaughtException(v8::Isolate* isolate, const v8::TryCatch& try_catch);
void TriggerUncaughtException(v8::Isolate* isolate, v8::Local<v8::Value> error, v8::Local<v8::Message> message, bool from_promise = false);

const char* errno_string(int errorno);
void PerIsolateMessageListener(v8::Local<v8::Message> message, v8::Local<v8::Value> error);

void DecorateErrorStack(Environment* env, const errors::TryCatchScope& try_catch);
void DecorateErrorStack(Environment* env, v8::Local<v8::Value> error, v8::Local<v8::Message> message);

class PrinterTryCatch : public v8::TryCatch {
public:
    enum PrintSourceLine { kPrintSourceLine, kDontPrintSourceLine };
    explicit PrinterTryCatch(v8::Isolate* isolate, PrintSourceLine print_source_line)
        : v8::TryCatch(isolate)
        , isolate_(isolate)
