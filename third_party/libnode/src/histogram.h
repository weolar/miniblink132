...
public:
    ...
    template <typename Iterator> inline void Percentiles(Iterator&& fn);
...
class HistogramImpl {
public:
    enum InternalFields { kSlot = BaseObject::kSlot, kImplField = BaseObject::kInternalFieldCount, kInternalFieldCount };

    explicit HistogramImpl(const Histogram::Options& options = Histogram::Options {});
    ...
    const std::shared_ptr<Histogram>& histogram() const
    {
        return histogram_;
    }
    ...
    static void GetPercentileBigInt(const v8::FunctionCallbackInfo<v8::Value>& args);
...
public:
    ...
    static BaseObjectPtr<HistogramBase> Create(Environment* env, const Histogram::Options& options = Histogram::Options {});

    static BaseObjectPtr<HistogramBase> Create(Environment* env, std::shared_ptr<Histogram> histogram);
...
public:
    ...
    IntervalHistogram(Environment* env, v8::Local<v8::Object> wrap, AsyncWrap::ProviderType type, int32_t interval, std::function<void(Histogram&)> on_interval,
