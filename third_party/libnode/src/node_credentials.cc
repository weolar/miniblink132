...
static bool HasOnly(int capability)
{
    ...
    return cap_data[CAP_TO_INDEX(capability)].permitted == static_cast<unsigned int>(CAP_TO_MASK(capability))
...
#if defined(__linux__)
    if ((!HasOnly(CAP_NET_BIND_SERVICE) && linux_at_secure()) || getuid() != geteuid() || getgid() != getegid())
#else
...
static void SafeGetenv(const FunctionCallbackInfo<Value>& args)
{
    ...
    std::string text;
    if (!SafeGetenv(*strenvtag, &text, env))
        return;
    Local<Value> result;
...
static char* name_by_uid(uid_t uid)
{
    ...
    if ((rc = getpwuid_r(uid, &pwd, buf, sizeof(buf), &pp)) == 0 && pp != nullptr) {
...
static bool ThrowIfUvMightBeUsingIoUring(Environment* env, const char* fn)
{
    if (UvMightBeUsingIoUring()) {
        node::THROW_ERR_INVALID_STATE(env, "%s() disabled: io_uring may be enabled. See CVE-2024-22017.", fn);
        return true;
    }
    return false;
}
...
static void SetGid(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "setgid"))
        return;
...
static void SetEGid(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "setegid"))
        return;
...
static void SetUid(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "setuid"))
        return;
...
static void SetEUid(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "seteuid"))
        return;
...
static void SetGroups(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "setgroups"))
        return;
...
static void InitGroups(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ThrowIfUvMightBeUsingIoUring(env, "initgroups"))
        return;
...
NODE_BINDING_EXTERNAL_REFERENCE(credentials, node::credentials::RegisterExternalReferences)
