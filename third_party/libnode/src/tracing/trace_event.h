...
#define INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()                                                                                       \
    *INTERNAL_TRACE_EVENT_UID(category_group_enabled) & (kEnabledForRecording_CategoryGroupEnabledFlags | kEnabledForEventCallback_CategoryGroupEnabledFlags)
...
#define TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED node::tracing::TraceEventHelper::GetCategoryGroupEnabled
...
//                    unsigned int flags,
//                    int64_t timestamp)
#define TRACE_EVENT_API_ADD_TRACE_EVENT_WITH_TIMESTAMP node::tracing::AddTraceEventWithTimestampImpl
...
#define TRACE_EVENT_API_UPDATE_TRACE_EVENT_DURATION                                                                                                            \
    if (auto controller = node::tracing::TraceEventHelper::GetTracingController())                                                                             \
...
#define INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO_CUSTOM_VARIABLES(category_group, atomic, category_group_enabled)                                                \
    ...
    if (!category_group_enabled) {                                                                                                                             \
        ...
        TRACE_EVENT_API_ATOMIC_STORE(atomic, reinterpret_cast<TRACE_EVENT_API_ATOMIC_WORD_VALUE>(category_group_enabled));                                     \
    }
...
#define INTERNAL_TRACE_EVENT_ADD(phase, category_group, name, flags, ...)                                                                                      \
    do {                                                                                                                                                       \
        ...
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            node::tracing::AddTraceEvent(phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, node::tracing::kGlobalScope, node::tracing::kNoId,     \
...
#define INTERNAL_TRACE_EVENT_ADD_SCOPED(category_group, name, ...)                                                                                             \
    ...
    if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                    \
        uint64_t h = node::tracing::AddTraceEvent(TRACE_EVENT_PHASE_COMPLETE, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name,                          \
            node::tracing::kGlobalScope, node::tracing::kNoId, node::tracing::kNoId, TRACE_EVENT_FLAG_NONE, ##__VA_ARGS__);                                    \
...
#define INTERNAL_TRACE_EVENT_ADD_SCOPED_WITH_FLOW(category_group, name, bind_id, flow_flags, ...)                                                              \
    ...
    if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                    \
        ...
        uint64_t h = node::tracing::AddTraceEvent(TRACE_EVENT_PHASE_COMPLETE, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name,                          \
            node::tracing::kGlobalScope, node::tracing::kNoId, trace_event_bind_id.raw_id(), trace_event_flags, ##__VA_ARGS__);                                \
        INTERNAL_TRACE_EVENT_UID(tracer).Initialize(INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, h);                                                \
    }
...
#define INTERNAL_TRACE_EVENT_ADD_WITH_ID(phase, category_group, name, id, flags, ...)                                                                          \
    do {                                                                                                                                                       \
        ...
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            ...
            node::tracing::AddTraceEvent(phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, trace_event_trace_id.scope(),                          \
                trace_event_trace_id.raw_id(), node::tracing::kNoId, trace_event_flags, ##__VA_ARGS__);                                                        \
        }                                                                                                                                                      \
    } while (0)
...
#define INTERNAL_TRACE_EVENT_ADD_WITH_TIMESTAMP(phase, category_group, name, timestamp, flags, ...)                                                            \
    do {                                                                                                                                                       \
        INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO(category_group);                                                                                                \
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            node::tracing::AddTraceEventWithTimestamp(phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, node::tracing::kGlobalScope,              \
                node::tracing::kNoId, node::tracing::kNoId, flags, timestamp, ##__VA_ARGS__);                                                                  \
        }                                                                                                                                                      \
    } while (0)
...
#define INTERNAL_TRACE_EVENT_ADD_WITH_ID_TID_AND_TIMESTAMP(phase, category_group, name, id, thread_id, timestamp, flags, ...)                                  \
    do {                                                                                                                                                       \
        ...
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            unsigned int trace_event_flags = flags | TRACE_EVENT_FLAG_HAS_ID;                                                                                  \
            node::tracing::TraceID trace_event_trace_id(id, &trace_event_flags);                                                                               \
            node::tracing::AddTraceEventWithTimestamp(phase, INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, trace_event_trace_id.scope(),             \
                trace_event_trace_id.raw_id(), node::tracing::kNoId, trace_event_flags, timestamp, ##__VA_ARGS__);                                             \
        }                                                                                                                                                      \
    } while (0)
...
#define INTERNAL_TRACE_EVENT_METADATA_ADD(category_group, name, ...)                                                                                           \
    do {                                                                                                                                                       \
        INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO(category_group);                                                                                                \
        if (INTERNAL_TRACE_EVENT_CATEGORY_GROUP_ENABLED_FOR_RECORDING_MODE()) {                                                                                \
            TRACE_EVENT_API_ADD_METADATA_EVENT(INTERNAL_TRACE_EVENT_UID(category_group_enabled), name, ##__VA_ARGS__);                                         \
        }                                                                                                                                                      \
    } while (0)
...
static inline uint64_t AddTraceEventImpl(char phase, const uint8_t* category_group_enabled, const char* name, const char* scope, uint64_t id, uint64_t bind_id,
    int32_t num_args, const char** arg_names, const uint8_t* arg_types, const uint64_t* arg_values, unsigned int flags)
{
    std::unique_ptr<v8::ConvertableToTraceFormat> arg_convertibles[2];
    if (num_args > 0 && arg_types[0] == TRACE_VALUE_TYPE_CONVERTABLE) {
        arg_convertibles[0].reset(reinterpret_cast<v8::ConvertableToTraceFormat*>(static_cast<intptr_t>(arg_values[0])));
    }
    if (num_args > 1 && arg_types[1] == TRACE_VALUE_TYPE_CONVERTABLE) {
        arg_convertibles[1].reset(reinterpret_cast<v8::ConvertableToTraceFormat*>(static_cast<intptr_t>(arg_values[1])));
    }
    // DCHECK(num_args, 2);
    v8::TracingController* controller = node::tracing::TraceEventHelper::GetTracingController();
    if (controller == nullptr)
        return 0;
    return controller->AddTraceEvent(
        phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_names, arg_types, arg_values, arg_convertibles, flags);
}
    ...
    uint64_t bind_id, int32_t num_args, const char** arg_names, const uint8_t* arg_types, const uint64_t* arg_values, unsigned int flags, int64_t timestamp)
{
    ...
    return controller->AddTraceEventWithTimestamp(
        phase, category_group_enabled, name, scope, id, bind_id, num_args, arg_names, arg_types, arg_values, arg_convertibles, flags, timestamp);
}

static V8_INLINE void AddMetadataEventImpl(const uint8_t* category_group_enabled, const char* name, int32_t num_args, const char** arg_names,
    const uint8_t* arg_types, const uint64_t* arg_values, unsigned int flags)
{
    ...
    if (agent == nullptr)
        return;
    return agent->GetTracingController()->AddMetadataEvent(category_group_enabled, name, num_args, arg_names, arg_types, arg_values, arg_convertibles, flags);
}
...
static inline void SetTraceValue(v8::ConvertableToTraceFormat* convertable_value, unsigned char* type, uint64_t* value)
{
...
template <typename T>
static inline typename std::enable_if<std::is_convertible<T*, v8::ConvertableToTraceFormat*>::value>::type SetTraceValue(
...
static inline uint64_t AddTraceEvent(char phase, const uint8_t* category_group_enabled, const char* name, const char* scope, uint64_t id, uint64_t bind_id,
...
template <class ARG1_TYPE>
static V8_INLINE void AddMetadataEvent(const uint8_t* category_group_enabled, const char* name, const char* arg1_name, ARG1_TYPE&& arg1_val)
{
    const int num_args = 1;
    uint8_t arg_type;
    uint64_t arg_value;
    SetTraceValue(std::forward<ARG1_TYPE>(arg1_val), &arg_type, &arg_value);
    AddMetadataEventImpl(category_group_enabled, name, num_args, &arg1_name, &arg_type, &arg_value, TRACE_EVENT_FLAG_NONE);
}
