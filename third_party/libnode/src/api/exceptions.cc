...
Local<Value> ErrnoException(Isolate* isolate, int errorno, const char* syscall, const char* msg, const char* path)
{
    ...
    Local<String> cons = String::Concat(isolate, estring, FIXED_ONE_BYTE_STRING(isolate, ", "));
    ...
    obj->Set(context, env->errno_string(), Integer::New(isolate, errorno)).Check();
    ...
    if (syscall != nullptr) {
        obj->Set(context, env->syscall_string(), OneByteString(isolate, syscall)).Check();
    }
...
static Local<String> StringFromPath(Isolate* isolate, const char* path)
{
    ...
    if (strncmp(path, "\\\\?\\UNC\\", 8) == 0) {
        return String::Concat(isolate, FIXED_ONE_BYTE_STRING(isolate, "\\\\"), String::NewFromUtf8(isolate, path + 8).ToLocalChecked());
...
Local<Value> UVException(Isolate* isolate, int errorno, const char* syscall, const char* msg, const char* path, const char* dest)
{
    ...
    Local<Context> context = env->context();
    e->Set(context, env->errno_string(), Integer::New(isolate, errorno)).Check();
...
static const char* winapi_strerror(const int errorno, bool* must_free)
{
    ...
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, errorno,
        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPSTR>(&errmsg), 0, nullptr);

    if (errmsg) {
        *must_free = true;

        // Remove trailing newlines
        for (int i = strlen(errmsg) - 1; i >= 0 && (errmsg[i] == '\n' || errmsg[i] == '\r'); i--) {
            errmsg[i] = '\0';
        }
...
Local<Value> WinapiErrnoException(Isolate* isolate, int errorno, const char* syscall, const char* msg, const char* path)
{
    ...
    if (path) {
        Local<String> cons1 = String::Concat(isolate, message, FIXED_ONE_BYTE_STRING(isolate, " '"));
        Local<String> cons2 = String::Concat(isolate, cons1, String::NewFromUtf8(isolate, path).ToLocalChecked());
        Local<String> cons3 = String::Concat(isolate, cons2, FIXED_ONE_BYTE_STRING(isolate, "'"));
    ...
    if (path != nullptr) {
        obj->Set(context, env->path_string(), String::NewFromUtf8(isolate, path).ToLocalChecked()).Check();
    }
