...
enum encoding ParseEncoding(Isolate* isolate, Local<Value> encoding_v, Local<Value> encoding_id, enum encoding default_encoding)
{
...
Local<Value> Encode(Isolate* isolate, const char* buf, size_t len, enum encoding encoding)
{
    ...
    Local<Value> error;
    return StringBytes::Encode(isolate, buf, len, encoding, &error).ToLocalChecked();
}
