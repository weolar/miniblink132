...
void BindingData::SlowToggleTimerRef(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
void BindingData::SlowToggleImmediateRef(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
v8::CFunction BindingData::fast_schedule_timers_(v8::CFunction::Make(FastScheduleTimer));
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    SetFastMethod(isolate, target, "getLibuvNow", SlowGetLibuvNow, &fast_get_libuv_now_);
    SetFastMethod(isolate, target, "scheduleTimer", SlowScheduleTimer, &fast_schedule_timers_);
    SetFastMethod(isolate, target, "toggleTimerRef", SlowToggleTimerRef, &fast_toggle_timer_ref_);
    SetFastMethod(isolate, target, "toggleImmediateRef", SlowToggleImmediateRef, &fast_toggle_immediate_ref_);
}
...
void BindingData::CreatePerContextProperties(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    ...
    target->Set(context, FIXED_ONE_BYTE_STRING(realm->isolate(), "immediateInfo"), env->immediate_info()->fields().GetJSArray()).Check();

    target->Set(context, FIXED_ONE_BYTE_STRING(realm->isolate(), "timeoutInfo"), env->timeout_info().GetJSArray()).Check();
}
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(timers, node::timers::BindingData::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(timers, node::timers::BindingData::CreatePerIsolateProperties)
NODE_BINDING_EXTERNAL_REFERENCE(timers, node::timers::BindingData::RegisterTimerExternalReferences)
