...
FSReqBase::FSReqBase(BindingData* binding_data, v8::Local<v8::Object> req, AsyncWrap::ProviderType type, bool use_bigint)
...
void FSReqBase::Init(const char* syscall, const char* data, size_t len, enum encoding encoding)
{
    syscall_ = syscall;
    encoding_ = encoding;
...
FSReqBase::FSReqBuffer& FSReqBase::Init(const char* syscall, size_t len, enum encoding encoding)
{
    syscall_ = syscall;
    encoding_ = encoding;
...
FSReqCallback::FSReqCallback(BindingData* binding_data, v8::Local<v8::Object> req, bool use_bigint)
    : FSReqBase(binding_data, req, AsyncWrap::PROVIDER_FSREQCALLBACK, use_bigint)
{
}

template <typename NativeT, typename V8T> void FillStatsArray(AliasedBufferBase<NativeT, V8T>* fields, const uv_stat_t* s, const size_t offset)
{
#define SET_FIELD_WITH_STAT(stat_offset, stat) fields->SetValue(offset + static_cast<size_t>(FsStatsOffset::stat_offset), static_cast<NativeT>(stat))
...
#else
#define SET_FIELD_WITH_TIME_STAT(stat_offset, stat) SET_FIELD_WITH_STAT(stat_offset, static_cast<double>(stat))
#endif // _WIN32
...
v8::Local<v8::Value> FillGlobalStatsArray(BindingData* binding_data, const bool use_bigint, const uv_stat_t* s, const bool second)
{
    const ptrdiff_t offset = second ? static_cast<ptrdiff_t>(FsStatsOffset::kFsStatsFieldsNumber) : 0;
    if (use_bigint) {
...
template <typename NativeT, typename V8T> void FillStatFsArray(AliasedBufferBase<NativeT, V8T>* fields, const uv_statfs_t* s)
{
#define SET_FIELD(field, stat) fields->SetValue(static_cast<size_t>(FsStatFsOffset::field), static_cast<NativeT>(stat))
...
v8::Local<v8::Value> FillGlobalStatFsArray(BindingData* binding_data, const bool use_bigint, const uv_statfs_t* s)
{
    if (use_bigint) {
...
template <typename AliasedBufferT> FSReqPromise<AliasedBufferT>* FSReqPromise<AliasedBufferT>::New(BindingData* binding_data, bool use_bigint)
{
    Environment* env = binding_data->env();
    v8::Local<v8::Object> obj;
    if (!env->fsreqpromise_constructor_template()->NewInstance(env->context()).ToLocal(&obj)) {
        return nullptr;
    }
    v8::Local<v8::Promise::Resolver> resolver;
    if (!v8::Promise::Resolver::New(env->context()).ToLocal(&resolver) || obj->Set(env->context(), env->promise_string(), resolver).IsNothing()) {
        return nullptr;
    }
    return new FSReqPromise(binding_data, obj, use_bigint);
}

template <typename AliasedBufferT> FSReqPromise<AliasedBufferT>::~FSReqPromise()
{
...
FSReqPromise<AliasedBufferT>::FSReqPromise(BindingData* binding_data, v8::Local<v8::Object> obj, bool use_bigint)
    : FSReqBase(binding_data, obj, AsyncWrap::PROVIDER_FSREQPROMISE, use_bigint)
    , stats_field_array_(env()->isolate(), static_cast<size_t>(FsStatsOffset::kFsStatsFieldsNumber))
    , statfs_field_array_(env()->isolate(), static_cast<size_t>(FsStatFsOffset::kFsStatFsFieldsNumber))
{
}

template <typename AliasedBufferT> void FSReqPromise<AliasedBufferT>::Reject(v8::Local<v8::Value> reject)
{
    finished_ = true;
...
template <typename AliasedBufferT> void FSReqPromise<AliasedBufferT>::Resolve(v8::Local<v8::Value> value)
{
    finished_ = true;
    v8::HandleScope scope(env()->isolate());
    InternalCallbackScope callback_scope(this);
...
template <typename AliasedBufferT> void FSReqPromise<AliasedBufferT>::ResolveStat(const uv_stat_t* stat)
{
    FillStatsArray(&stats_field_array_, stat);
    Resolve(stats_field_array_.GetJSArray());
}

template <typename AliasedBufferT> void FSReqPromise<AliasedBufferT>::ResolveStatFs(const uv_statfs_t* stat)
{
...
template <typename AliasedBufferT> void FSReqPromise<AliasedBufferT>::MemoryInfo(MemoryTracker* tracker) const
{
...
FSReqBase* GetReqWrap(const v8::FunctionCallbackInfo<v8::Value>& args, int index, bool use_bigint)
{
...
FSReqBase* AsyncDestCall(Environment* env, FSReqBase* req_wrap, const v8::FunctionCallbackInfo<v8::Value>& args, const char* syscall, const char* dest,
    size_t len, enum encoding enc, uv_fs_cb after, Func fn, Args... fn_args)
{
...
// Returns nullptr if the operation fails from the start.
template <typename Func, typename... Args>
FSReqBase* AsyncCall(Environment* env, FSReqBase* req_wrap, const v8::FunctionCallbackInfo<v8::Value>& args, const char* syscall, enum encoding enc,
...
template <typename Func, typename... Args>
int SyncCall(Environment* env, v8::Local<v8::Value> ctx, FSReqWrapSync* req_wrap, const char* syscall, Func fn, Args... args)
{
    env->PrintSyncTrace();
    ...
    if (err < 0) {
        ...
        ctx_obj->Set(context, env->errno_string(), v8::Integer::New(isolate, err)).Check();
        ctx_obj->Set(context, env->syscall_string(), OneByteString(isolate, syscall)).Check();
    }
    return err;
}
...
int SyncCallAndThrowIf(Predicate should_throw, Environment* env, FSReqWrapSync* req_wrap, Func fn, Args... args)
{
    env->PrintSyncTrace();
    int result = fn(nullptr, &(req_wrap->req), args..., nullptr);
    if (should_throw(result)) {
        env->ThrowUVException(result, req_wrap->syscall_p, nullptr, req_wrap->path_p, req_wrap->dest_p);
    }
    return result;
}
...
// Similar to SyncCall but throws immediately if there is an error.
template <typename Func, typename... Args> int SyncCallAndThrowOnError(Environment* env, FSReqWrapSync* req_wrap, Func fn, Args... args)
{
