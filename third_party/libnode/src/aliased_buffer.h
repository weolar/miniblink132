 ...
 */
template <class NativeT, class V8T> class AliasedBufferBase final : public MemoryRetainer {
public:
    static_assert(std::is_scalar_v<NativeT>);

    AliasedBufferBase(v8::Isolate* isolate, size_t count, const AliasedBufferIndex* index = nullptr);
   ...
   */
    ...
    AliasedBufferBase(v8::Isolate* isolate, size_t byte_offset, size_t count, const AliasedBufferBase<uint8_t, v8::Uint8Array>& backing_buffer,
        const AliasedBufferIndex* index = nullptr);

    AliasedBufferBase(const AliasedBufferBase& that);

    AliasedBufferIndex Serialize(v8::Local<v8::Context> context, v8::SnapshotCreator* creator);
...
#define V(NativeT, V8T) typedef AliasedBufferBase<NativeT, v8::V8T> Aliased##V8T;
