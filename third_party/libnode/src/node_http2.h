...
enum SessionType { NGHTTP2_SESSION_SERVER, NGHTTP2_SESSION_CLIENT };

template <typename T, void (*fn)(T*)> struct Nghttp2Deleter {
...
using Nghttp2OptionPointer = std::unique_ptr<nghttp2_option, Nghttp2Deleter<nghttp2_option, nghttp2_option_del>>;

using Nghttp2SessionPointer = std::unique_ptr<nghttp2_session, Nghttp2Deleter<nghttp2_session, nghttp2_session_del>>;

using Nghttp2SessionCallbacksPointer = std::unique_ptr<nghttp2_session_callbacks, Nghttp2Deleter<nghttp2_session_callbacks, nghttp2_session_callbacks_del>>;
...
struct NgHttp2StreamWrite : public MemoryRetainer {
    ...
    inline explicit NgHttp2StreamWrite(uv_buf_t buf_)
        : buf(buf_)
    {
    }
    inline NgHttp2StreamWrite(BaseObjectPtr<AsyncWrap> req_wrap, uv_buf_t buf_)
        : req_wrap(std::move(req_wrap))
        , buf(buf_)
    {
    }
...
typedef uint32_t (*get_setting)(nghttp2_session* session, nghttp2_settings_id id);
...
class Http2Options {
public:
    Http2Options(Http2State* http2_state, SessionType type);
...
struct Http2Priority : public nghttp2_priority_spec {
    Http2Priority(Environment* env, v8::Local<v8::Value> parent, v8::Local<v8::Value> weight, v8::Local<v8::Value> exclusive);
};
...
public:
    static Http2Stream* New(Http2Session* session, int32_t id, nghttp2_headers_category category = NGHTTP2_HCAT_HEADERS, int options = 0);
    ...
    bool HasWantsWrite() const override
    {
        return true;
    }
    ...
    Http2Stream* SubmitPushPromise(const Http2Headers& headers, int32_t* ret, int options = 0);
    ...
    template <typename Fn> void TransferHeaders(Fn&& fn)
    {
        size_t i = 0;
...
private:
    Http2Stream(Http2Session* session, v8::Local<v8::Object> obj, int32_t id, nghttp2_headers_category category, int options);
...
public:
    ...
    static ssize_t OnRead(nghttp2_session* session, int32_t id, uint8_t* buf, size_t length, uint32_t* flags, nghttp2_data_source* source, void* user_data);
};
...
enum SessionBitfieldFlags { kSessionHasRemoteSettingsListeners, kSessionRemoteSettingsIsUpToDate, kSessionHasPingListeners, kSessionHasAltsvcListeners };

class Http2Session : public AsyncWrap, public StreamListener, public mem::NgLibMemoryManager<Http2Session, nghttp2_mem> {
public:
    Http2Session(Http2State* http2_state, v8::Local<v8::Object> wrap, SessionType type = NGHTTP2_SESSION_SERVER);
    ...
    void Goaway(uint32_t code, int32_t lastStreamID, const uint8_t* data, size_t len);

    void AltSvc(int32_t id, uint8_t* origin, size_t origin_len, uint8_t* value, size_t value_len);
    ...
    Http2Stream* SubmitRequest(const Http2Priority& priority, const Http2Headers& headers, int32_t* ret, int options = 0);
...
#undef IS_FLAG
    ...
    {
        return pending_rst_streams_.end() != std::find(pending_rst_streams_.begin(), pending_rst_streams_.end(), stream_id);
    }
    ...
    static void SetLocalWindowSize(const v8::FunctionCallbackInfo<v8::Value>& args);
    ...
    template <get_setting fn, bool local> static void RefreshSettings(const v8::FunctionCallbackInfo<v8::Value>& args);
    ...
    void UpdateLocalCustomSettings(size_t count_, nghttp2_settings_entry* entries_);
...
private:
    ...
    static int OnHeaderCallback(
        nghttp2_session* session, const nghttp2_frame* frame, nghttp2_rcbuf* name, nghttp2_rcbuf* value, uint8_t flags, void* user_data);
    static int OnFrameReceive(nghttp2_session* session, const nghttp2_frame* frame, void* user_data);
    static int OnFrameNotSent(nghttp2_session* session, const nghttp2_frame* frame, int error_code, void* user_data);
    static int OnFrameSent(nghttp2_session* session, const nghttp2_frame* frame, void* user_data);
    static int OnStreamClose(nghttp2_session* session, int32_t id, uint32_t code, void* user_data);
    static int OnInvalidHeader(nghttp2_session* session, const nghttp2_frame* frame, nghttp2_rcbuf* name, nghttp2_rcbuf* value, uint8_t flags, void* user_data);
    static int OnDataChunkReceived(nghttp2_session* session, uint8_t flags, int32_t id, const uint8_t* data, size_t len, void* user_data);
    static ssize_t OnSelectPadding(nghttp2_session* session, const nghttp2_frame* frame, size_t maxPayloadLen, void* user_data);
    static int OnNghttpError(nghttp2_session* session, int lib_error_code, const char* message, size_t len, void* user_data);
    static int OnSendData(nghttp2_session* session, nghttp2_frame* frame, const uint8_t* framehd, size_t length, nghttp2_data_source* source, void* user_data);
    static int OnInvalidFrame(nghttp2_session* session, const nghttp2_frame* frame, int lib_error_code, void* user_data);
...
struct Http2SessionPerformanceEntryTraits {
    static constexpr performance::PerformanceEntryType kType = performance::NODE_PERFORMANCE_ENTRY_TYPE_HTTP2;

    using Details = Http2Session::Statistics;

    static v8::MaybeLocal<v8::Object> GetDetails(Environment* env, const performance::PerformanceEntry<Http2SessionPerformanceEntryTraits>& entry);
};
...
struct Http2StreamPerformanceEntryTraits {
    ...
    static v8::MaybeLocal<v8::Object> GetDetails(Environment* env, const performance::PerformanceEntry<Http2StreamPerformanceEntryTraits>& entry);
};

using Http2SessionPerformanceEntry = performance::PerformanceEntry<Http2SessionPerformanceEntryTraits>;
using Http2StreamPerformanceEntry = performance::PerformanceEntry<Http2StreamPerformanceEntryTraits>;

class Http2Ping : public AsyncWrap {
public:
    explicit Http2Ping(Http2Session* session, v8::Local<v8::Object> obj, v8::Local<v8::Function> callback);
...
public:
    Http2Settings(Http2Session* session, v8::Local<v8::Object> obj, v8::Local<v8::Function> callback, uint64_t start_time = uv_hrtime());
...
private:
    static size_t Init(Http2State* http2_state, nghttp2_settings_entry* entries);

    static v8::Local<v8::Value> Pack(Environment* env, size_t count, const nghttp2_settings_entry* entries);
...
class Origins {
public:
    Origins(Environment* env, v8::Local<v8::String> origin_string, size_t origin_count);
    ~Origins() = default;
