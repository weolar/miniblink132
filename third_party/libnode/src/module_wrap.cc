...
ModuleWrap::ModuleWrap(
    Realm* realm, Local<Object> object, Local<Module> module, Local<String> url, Local<Object> context_object, Local<Value> synthetic_evaluation_step)
    ...
    , module_hash_(module->GetIdentityHash())
{
    ...
    object->SetInternalField(kModuleSourceObjectSlot, v8::Undefined(realm->isolate()));
...
Maybe<bool> ModuleWrap::CheckUnsettledTopLevelAwait()
{
    ...
    if (env()->options()->warnings) {
        for (auto& message : stalled_messages) {
            ...
            std::string info = FormatErrorMessage(isolate, context, "", message, true);
            reason += info;
            FPrintF(stderr, "%s\n", reason);
        }
    }
...
Local<PrimitiveArray> ModuleWrap::GetHostDefinedOptions(Isolate* isolate, Local<Symbol> id_symbol)
{
...
void ModuleWrap::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[1]->IsUndefined()) {
        ...
        contextify_context = ContextifyContext::ContextFromContextifiedSandbox(realm->env(), args[1].As<Object>());
    ...
    if (synthetic) {
        ...
        if (args[5]->IsSymbol()) {
            ...
            can_use_builtin_cache = (id_symbol == realm->isolate_data()->source_text_module_default_hdo());
        } else {
...

    {
        ...
        if (synthetic) {
            ...
            if (id_symbol != realm->isolate_data()->source_text_module_default_hdo()) {
            ...
            if (args[5]->IsArrayBufferView()) {
                ...
                user_cached_data = new ScriptCompiler::CachedData(data + cached_data_buf->ByteOffset(), cached_data_buf->ByteLength());
            }
            Local<String> source_text = args[2].As<String>();

            bool cache_rejected = false;
            if (!CompileSourceTextModule(realm, source_text, url, line_offset, column_offset, host_defined_options, user_cached_data, &cache_rejected)
                     .ToLocal(&module)) {
                if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
                    CHECK(!try_catch.Message().IsEmpty());
                    CHECK(!try_catch.Exception().IsEmpty());
                    AppendExceptionLine(realm->env(), try_catch.Exception(), try_catch.Message(), ErrorHandlingMode::MODULE_ERROR);
                    try_catch.ReThrow();
                }
                return;
            }

            if (user_cached_data.has_value() && user_cached_data.value() != nullptr && cache_rejected) {
                THROW_ERR_VM_MODULE_CACHED_DATA_REJECTED(realm, "cachedData buffer was rejected");
                try_catch.ReThrow();
                return;
            }

            if (that->Set(context, realm->env()->source_map_url_string(), module->GetUnboundModuleScript()->GetSourceMappingURL()).IsNothing()) {
                return;
            }
        }
    }

    if (!that->Set(context, realm->isolate_data()->url_string(), url).FromMaybe(false)) {
        return;
    }

    if (synthetic && args[4]->IsObject() && that->Set(context, realm->isolate_data()->imported_cjs_symbol(), args[4]).IsNothing()) {
        return;
    }

    // Initialize an empty slot for source map cache before the object is frozen.
    if (that->SetPrivate(context, realm->isolate_data()->source_map_data_private_symbol(), Undefined(isolate)).IsNothing()) {
        return;
    }
    ...
    Local<Value> synthetic_evaluation_step = synthetic ? args[3] : Undefined(realm->isolate()).As<Value>();
...
MaybeLocal<Module> ModuleWrap::CompileSourceTextModule(Realm* realm, Local<String> source_text, Local<String> url, int line_offset, int column_offset,
    Local<PrimitiveArray> host_defined_options, std::optional<ScriptCompiler::CachedData*> user_cached_data, bool* cache_rejected)
{
    Isolate* isolate = realm->isolate();
    EscapableHandleScope scope(isolate);
    ScriptOrigin origin(url, line_offset, column_offset,
    ...
    if (user_cached_data.has_value()) {
        ...
        cache_entry = realm->env()->compile_cache_handler()->GetOrInsert(source_text, url, CachedCodeType::kESM);
    }
    ...
    if (cache_entry != nullptr) {
        realm->env()->compile_cache_handler()->MaybeSave(cache_entry, module, *cache_rejected);
    }
...
static Local<Object> createImportAttributesContainer(Realm* realm, Isolate* isolate, Local<FixedArray> raw_attributes, const int elements_per_attribute)
{
    ...
    return Object::New(isolate, Null(isolate), names.data(), values.data(), num_attributes);
}
...
void ModuleWrap::Link(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (uint32_t i = 0; i < specifiers->Length(); i++) {
        Local<String> specifier_str = specifiers_buffer[i].Get(isolate).As<String>();
        Local<Object> module_object = modules_buffer[i].Get(isolate).As<Object>();

        CHECK(realm->isolate_data()->module_wrap_constructor_template()->HasInstance(module_object));

        Utf8Value specifier(isolate, specifier_str);
        dependent->resolve_cache_[specifier.ToString()].Reset(isolate, module_object);
    }
}
...
void ModuleWrap::Instantiate(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Module> module = obj->module_.Get(isolate);
    TryCatchScope try_catch(realm->env());
    USE(module->InstantiateModule(context, ResolveModuleCallback, ResolveSourceCallback));
...
void ModuleWrap::Evaluate(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (timed_out || received_signal) {
        if (!realm->env()->is_main_thread() && realm->env()->is_stopping())
            return;
...
void ModuleWrap::InstantiateSync(const FunctionCallbackInfo<Value>& args)
{
...

    {
        ...
        if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
            CHECK(!try_catch.Message().IsEmpty());
            CHECK(!try_catch.Exception().IsEmpty());
            AppendExceptionLine(env, try_catch.Exception(), try_catch.Message(), ErrorHandlingMode::MODULE_ERROR);
            try_catch.ReThrow();
            return;
        }
    }
...
Maybe<void> ThrowIfPromiseRejected(Realm* realm, Local<Promise> promise)
{
    ...
    // from here directly.
    Local<Value> type = Integer::New(isolate, static_cast<int32_t>(PromiseRejectEvent::kPromiseHandlerAddedAfterReject));
    Local<Value> args[] = { type, promise, Undefined(isolate) };
    if (realm->promise_reject_callback()->Call(context, Undefined(isolate), arraysize(args), args).IsEmpty()) {
        return Nothing<void>();
    }
    Local<Value> exception = promise->Result();
    Local<Message> message = Exception::CreateMessage(isolate, exception);
    AppendExceptionLine(realm->env(), exception, message, ErrorHandlingMode::MODULE_ERROR);
...
void ModuleWrap::GetNamespaceSync(const FunctionCallbackInfo<Value>& args)
{
    ...
    case Module::Status::kInstantiating:
        return realm->env()->ThrowError("Cannot get namespace, module has not been instantiated");
...
void ModuleWrap::SetModuleSourceObject(const FunctionCallbackInfo<Value>& args)
{
    ModuleWrap* obj;
    ASSIGN_OR_RETURN_UNWRAP(&obj, args.This());
...
void ModuleWrap::GetModuleSourceObject(const FunctionCallbackInfo<Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    ModuleWrap* obj;
    ASSIGN_OR_RETURN_UNWRAP(&obj, args.This());
...
MaybeLocal<Module> ModuleWrap::ResolveModuleCallback(
    Local<Context> context, Local<String> specifier, Local<FixedArray> import_attributes, Local<Module> referrer)
{
    ...
    if (dependent == nullptr) {
        THROW_ERR_VM_MODULE_LINK_FAILURE(env, "request for '%s' is from invalid module", specifier_std);
        return MaybeLocal<Module>();
    }

    if (dependent->resolve_cache_.count(specifier_std) != 1) {
        THROW_ERR_VM_MODULE_LINK_FAILURE(env, "request for '%s' is not in cache", specifier_std);
        return MaybeLocal<Module>();
    }

    Local<Object> module_object = dependent->resolve_cache_[specifier_std].Get(isolate);
    if (module_object.IsEmpty() || !module_object->IsObject()) {
        THROW_ERR_VM_MODULE_LINK_FAILURE(env, "request for '%s' did not return an object", specifier_std);
        return MaybeLocal<Module>();
    }
    ...
    Local<Context> context, Local<String> specifier, Local<FixedArray> import_attributes, Local<Module> referrer)
{
    ...
    Local<Value> module_source_object = module->object()->GetInternalField(kModuleSourceObjectSlot).As<Value>();
    if (module_source_object->IsUndefined()) {
        Local<String> url = module->object()->GetInternalField(kURLSlot).As<String>();
        THROW_ERR_SOURCE_PHASE_NOT_DEFINED(isolate, url);
        return MaybeLocal<Object>();
    }
...
static MaybeLocal<Promise> ImportModuleDynamicallyWithPhase(Local<Context> context, Local<Data> host_defined_options, Local<Value> resource_name,
    Local<String> specifier, ModuleImportPhase phase, Local<FixedArray> import_attributes)
{
    ...
    Local<Function> import_callback = realm->host_import_module_dynamically_callback();
    Local<Value> id;
    ...
    } else if (!context->Global()->GetPrivate(context, env->host_defined_option_symbol()).ToLocal(&id)) {
        return MaybeLocal<Promise>();
    }

    Local<Object> attributes = createImportAttributesContainer(realm, isolate, import_attributes, 2);
    ...
    Local<Value> result;
    if (import_callback->Call(context, Undefined(isolate), arraysize(import_args), import_args).ToLocal(&result)) {
        CHECK(result->IsPromise());
...
{
    return ImportModuleDynamicallyWithPhase(context, host_defined_options, resource_name, specifier, ModuleImportPhase::kEvaluation, import_attributes);
}

void ModuleWrap::SetImportModuleDynamicallyCallback(const FunctionCallbackInfo<Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    if (isolate->GetData(/*gin::kEmbedderBlink*/1)) // weolar
        return;
    Realm* realm = Realm::GetCurrent(args);
...
void ModuleWrap::HostInitializeImportMetaObjectCallback(Local<Context> context, Local<Module> module, Local<Object> meta)
{
    Environment* env = Environment::GetCurrent(context);
    if (env == nullptr)
        return;
    ...
    TryCatchScope try_catch(env);
    USE(callback->Call(context, Undefined(realm->isolate()), arraysize(args), args));
    if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
        try_catch.ReThrow();
    }
}

void ModuleWrap::SetInitializeImportMetaObjectCallback(const FunctionCallbackInfo<Value>& args)
{
    Realm* realm = Realm::GetCurrent(args);
    Isolate* isolate = realm->isolate();
    if (isolate->GetData(/*gin::kEmbedderBlink*/1)) // weolar
...
MaybeLocal<Value> ModuleWrap::SyntheticModuleEvaluationStepsCallback(Local<Context> context, Local<Module> module)
{
    ...
    TryCatchScope try_catch(env);
    Local<Function> synthetic_evaluation_steps = obj->object()->GetInternalField(kSyntheticEvaluationStepsSlot).As<Value>().As<Function>();
    obj->object()->SetInternalField(kSyntheticEvaluationStepsSlot, Undefined(isolate));
    MaybeLocal<Value> ret = synthetic_evaluation_steps->Call(context, obj->object(), 0, nullptr);
    if (ret.IsEmpty()) {
        CHECK(try_catch.HasCaught());
    }
    if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
...
void ModuleWrap::CreateCachedData(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<UnboundModuleScript> unbound_module_script = module->GetUnboundModuleScript();
    std::unique_ptr<ScriptCompiler::CachedData> cached_data(ScriptCompiler::CreateCodeCache(unbound_module_script));
    ...
    } else if (!Buffer::Copy(env, reinterpret_cast<const char*>(cached_data->data), cached_data->length).ToLocal(&result)) {
        return;
    }
    args.GetReturnValue().Set(result);
}
...
MaybeLocal<Module> LinkRequireFacadeWithOriginal(Local<Context> context, Local<String> specifier, Local<FixedArray> import_attributes, Local<Module> referrer)
{
    Environment* env = Environment::GetCurrent(context);
    Isolate* isolate = context->GetIsolate();
...
void ModuleWrap::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    SetProtoMethodNoSideEffect(isolate, tpl, "createCachedData", CreateCachedData);
...
void ModuleWrap::CreatePerContextProperties(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    Realm* realm = Realm::GetCurrent(context);
    Isolate* isolate = realm->isolate();
#define V(enum_type, name) target->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, enum_type::name)).FromJust()
...
void ModuleWrap::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(module_wrap, node::loader::ModuleWrap::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(module_wrap, node::loader::ModuleWrap::CreatePerIsolateProperties)
