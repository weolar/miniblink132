...
void Concat(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (val->IsArrayBuffer()) {
            ...
            views.push_back(View { view->Buffer()->GetBackingStore(), view->ByteLength(), view->ByteOffset() });
            total += view->ByteLength();
        }
    }

    std::shared_ptr<BackingStore> store = ArrayBuffer::NewBackingStore(env->isolate(), total);
    uint8_t* ptr = static_cast<uint8_t*>(store->Data());
    for (size_t n = 0; n < views.size(); n++) {
        uint8_t* from = static_cast<uint8_t*>(views[n].store->Data()) + views[n].offset;
...
void BlobFromFilePath(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (auto blob = Blob::Create(env, DataQueue::CreateIdempotent(std::move(entries)))) {
        Local<Value> vals[2] {
            blob->object(),
            Uint32::NewFromUnsigned(env->isolate(), blob->length()),
        };
        args.GetReturnValue().Set(Array::New(env->isolate(), &vals[0], arraysize(vals)));
    }
}
} // namespace

void Blob::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
...
void Blob::CreatePerContextProperties(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
...
void Blob::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        auto entryFromArrayBuffer = [isolate](Local<ArrayBuffer> buf, size_t byte_length, size_t byte_offset = 0) mutable -> std::unique_ptr<DataQueue::Entry> {
            if (buf->IsDetachable()) {
                std::shared_ptr<BackingStore> store = buf->GetBackingStore();
                if (buf->Detach(Local<Value>()).IsNothing()) {
                    return nullptr;
                }
                return DataQueue::CreateInMemoryEntryFromBackingStore(std::move(store), byte_offset, byte_length);
            }

            // If the ArrayBuffer is not detachable, we will copy from it instead.
            std::shared_ptr<BackingStore> store = ArrayBuffer::NewBackingStore(isolate, byte_length);
            uint8_t* ptr = static_cast<uint8_t*>(buf->Data()) + byte_offset;
            std::copy(ptr, ptr + byte_length, static_cast<uint8_t*>(store->Data()));
            return DataQueue::CreateInMemoryEntryFromBackingStore(std::move(store), 0, byte_length);
        };
        ...
        if (entry->IsArrayBuffer()) {
            ...
            auto ret = entryFromArrayBuffer(view->Buffer(), view->ByteLength(), view->ByteOffset());
            if (!ret)
                return;
            entries[i] = std::move(ret);
...
void Blob::GetReader(const FunctionCallbackInfo<Value>& args)
{
    ...
    BaseObjectPtr<Blob::Reader> reader = Blob::Reader::Create(env, BaseObjectPtr<Blob>(blob));
    if (reader)
        args.GetReturnValue().Set(reader->object());
}
...
{
    return Create(env, this->data_queue_->slice(start, static_cast<uint64_t>(end)));
}

Blob::Blob(Environment* env, Local<Object> obj, std::shared_ptr<DataQueue> data_queue)
    : BaseObject(env, obj)
...
Blob::Reader::Reader(Environment* env, Local<Object> obj, BaseObjectPtr<Blob> strong_ptr)
...
BaseObjectPtr<Blob::Reader> Blob::Reader::Create(Environment* env, BaseObjectPtr<Blob> blob)
{
    Local<Object> obj;
...
void Blob::Reader::Pull(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto next = [impl](int status, const DataQueue::Vec* vecs, size_t count, bob::Done doneCb) mutable {
        ...
        if (count > 0) {
            ...
            std::move(doneCb)(0);
            Local<Value> argv[2] = { Uint32::New(env->isolate(), status), ArrayBuffer::New(env->isolate(), store) };
    ...
    args.GetReturnValue().Set(reader->inner_->Pull(std::move(next), node::bob::OPTIONS_END, nullptr, 0));
}

BaseObjectPtr<BaseObject> Blob::BlobTransferData::Deserialize(Environment* env, Local<Context> context, std::unique_ptr<worker::TransferData> self)
{
...
void Blob::StoreDataObject(const FunctionCallbackInfo<Value>& args)
{
    ...
    binding_data->store_data_object(
        std::string(*key, key.length()), BlobBindingData::StoredDataObject(BaseObjectPtr<Blob>(blob), length, std::string(*type, type.length())));
}
...
void Blob::GetDataObject(const FunctionCallbackInfo<Value>& args)
{
    ...
    BlobBindingData::StoredDataObject stored = binding_data->get_data_object(std::string(*key, key.length()));
    if (stored.blob) {
        Local<Value> type;
        if (!String::NewFromUtf8(isolate, stored.type.c_str(), NewStringType::kNormal, static_cast<int>(stored.type.length())).ToLocal(&type)) {
            return;
        }

        Local<Value> values[] = { stored.blob->object(), Uint32::NewFromUnsigned(isolate, stored.length), type };

        args.GetReturnValue().Set(Array::New(isolate, values, arraysize(values)));
    }
}

void BlobBindingData::StoredDataObject::MemoryInfo(MemoryTracker* tracker) const
{
    tracker->TrackField("blob", blob, "BaseObjectPtr<Blob>");
}

BlobBindingData::StoredDataObject::StoredDataObject(const BaseObjectPtr<Blob>& blob_, size_t length_, const std::string& type_)
    : blob(blob_)
    , length(length_)
    , type(type_)
{
...
{
    tracker->TrackField("data_objects_", data_objects_, "std::unordered_map<std::string, StoredDataObject>");
}

void BlobBindingData::store_data_object(const std::string& uuid, const BlobBindingData::StoredDataObject& object)
{
...
BlobBindingData::StoredDataObject BlobBindingData::get_data_object(const std::string& uuid)
{
...
void BlobBindingData::Deserialize(Local<Context> context, Local<Object> holder, int index, InternalFieldInfoBase* info)
{
...
bool BlobBindingData::PrepareForSerialization(Local<Context> context, SnapshotCreator* creator)
{
...
{
    DCHECK_IS_SNAPSHOT_SLOT(index);
    InternalFieldInfo* info = InternalFieldInfoBase::New<InternalFieldInfo>(type());
    return info;
}
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(blob, node::Blob::CreatePerContextProperties)
