...
static void WriteNodeReport(Isolate* isolate, Environment* env, const char* message, const char* trigger, const std::string& filename, std::ostream& out,
    Local<Value> error, bool compact, bool exclude_network = false, bool exclude_env = false);
static void PrintVersionInformation(JSONWriter* writer, bool exclude_network = false);
static void PrintJavaScriptErrorStack(JSONWriter* writer, Isolate* isolate, Local<Value> error, const char* trigger);
static void PrintEmptyJavaScriptStack(JSONWriter* writer);
static void PrintJavaScriptStack(JSONWriter* writer, Isolate* isolate, const char* trigger);
static void PrintJavaScriptErrorProperties(JSONWriter* writer, Isolate* isolate, Local<Value> error);
...
static void WriteNodeReport(Isolate* isolate, Environment* env, const char* message, const char* trigger, const std::string& filename, std::ostream& out,
    Local<Value> error, bool compact, bool exclude_network, bool exclude_env)
{
    ...
    char timebuf[64];
#ifdef _WIN32
    snprintf(timebuf, sizeof(timebuf), "%4d-%02d-%02dT%02d:%02d:%02dZ", tm_struct.wYear, tm_struct.wMonth, tm_struct.wDay, tm_struct.wHour, tm_struct.wMinute,
        tm_struct.wSecond);
    writer.json_keyvalue("dumpEventTime", timebuf);
#else // UNIX, macOS
    snprintf(timebuf, sizeof(timebuf), "%4d-%02d-%02dT%02d:%02d:%02dZ", tm_struct.tm_year + 1900, tm_struct.tm_mon + 1, tm_struct.tm_mday, tm_struct.tm_hour,
    ...
    if (uv_gettimeofday(&ts) == 0) {
        writer.json_keyvalue("dumpEventTimeStamp", std::to_string(ts.tv_sec * 1000 + ts.tv_usec / 1000));
    }
    ...
    if (env != nullptr) {
        ...
        writer.json_keyvalue("is_active", static_cast<bool>(uv_loop_alive(env->event_loop())));
        writer.json_keyvalue("address", ValueToHexString(reinterpret_cast<int64_t>(env->event_loop())));
    ...
    if (env != nullptr) {
        ...
        env->ForEachWorker([&](Worker* w) {
            expected_results += w->RequestInterrupt([&](Environment* env) {
                ...
                GetNodeReport(env, "Worker thread subreport", trigger, Local<Value>(), os);
...
static void PrintVersionInformation(JSONWriter* writer, bool exclude_network)
{
    ...
    *(reinterpret_cast<void**>(&libc_version)) = dlsym(RTLD_DEFAULT, "gnu_get_libc_version");
...
static void PrintNetworkInterfaceInfo(JSONWriter* writer)
{
    ...
    if (uv_interface_addresses(&interfaces, &count) == 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            snprintf(mac, sizeof(mac), "%02x:%02x:%02x:%02x:%02x:%02x", static_cast<unsigned char>(interfaces[i].phys_addr[0]),
                static_cast<unsigned char>(interfaces[i].phys_addr[1]), static_cast<unsigned char>(interfaces[i].phys_addr[2]),
                static_cast<unsigned char>(interfaces[i].phys_addr[3]), static_cast<unsigned char>(interfaces[i].phys_addr[4]),
            ...
            if (interfaces[i].address.address4.sin_family == AF_INET) {
                ...
                writer->json_keyvalue("scopeid", interfaces[i].address.address6.sin6_scope_id);
            } else {
...
static void PrintJavaScriptErrorProperties(JSONWriter* writer, Isolate* isolate, Local<Value> error)
{
    ...
    if (!error.IsEmpty() && error->IsObject()) {
        ...
        for (uint32_t i = 0; i < keys_length; i++) {
            ...
            if (!error_obj->Get(context, key).ToLocal(&value) || !value->ToString(context).ToLocal(&value_string)) {
                continue;
            }
...
static Maybe<std::string> ErrorToString(Isolate* isolate, Local<Context> context, Local<Value> error)
{
    if (error.IsEmpty()) {
    ...
    if (error->IsSymbol()) {
        ...
        Local<Value> stack;
        if (!error.As<Object>()->Get(context, FIXED_ONE_BYTE_STRING(isolate, "stack")).ToLocal(&stack)) {
            return Nothing<std::string>();
        }
        if (stack->IsString()) {
...
static void PrintJavaScriptStack(JSONWriter* writer, Isolate* isolate, const char* trigger)
{
    ...
    for (int i = 0; i < stack->GetFrameCount(); i++) {
        ...
        std::string stack_line = SPrintF("at %s (%s:%d:%d)", *function_name, *script_name, line_number, column);
...
static void PrintJavaScriptErrorStack(JSONWriter* writer, Isolate* isolate, Local<Value> error, const char* trigger)
{
    ...
    if (line == -1) {
        ...
        while (line != -1) {
            l = ss.substr(0, line);
            l.erase(l.begin(), std::find_if(l.begin(), l.end(), [](int ch) { return !std::iswspace(ch); }));
...
static void PrintNativeStack(JSONWriter* writer)
{
    ...
    for (i = 1; i < size; i++) {
        void* frame = frames[i];
        writer->json_start();
        writer->json_keyvalue("pc", ValueToHexString(reinterpret_cast<uintptr_t>(frame)));
...
static void PrintGCStatistics(JSONWriter* writer, Isolate* isolate)
{
    ...
    writer->json_keyvalue("executableMemory", v8_heap_stats.total_heap_size_executable());
    writer->json_keyvalue("totalCommittedMemory", v8_heap_stats.total_physical_size());
    writer->json_keyvalue("availableMemory", v8_heap_stats.total_available_size());
    writer->json_keyvalue("totalGlobalHandlesMemory", v8_heap_stats.total_global_handles_size());
    writer->json_keyvalue("usedGlobalHandlesMemory", v8_heap_stats.used_global_handles_size());
    ...
    writer->json_keyvalue("peakMallocedMemory", v8_heap_stats.peak_malloced_memory());
    writer->json_keyvalue("nativeContextCount", v8_heap_stats.number_of_native_contexts());
    writer->json_keyvalue("detachedContextCount", v8_heap_stats.number_of_detached_contexts());
    ...
    for (size_t i = 0; i < isolate->NumberOfHeapSpaces(); i++) {
        ...
        writer->json_keyvalue("committedMemory", v8_heap_space_stats.physical_space_size());
        writer->json_keyvalue("capacity", v8_heap_space_stats.space_used_size() + v8_heap_space_stats.space_available_size());
        writer->json_keyvalue("used", v8_heap_space_stats.space_used_size());
        writer->json_keyvalue("available", v8_heap_space_stats.space_available_size());
        writer->json_objectend();
    }
...
static void PrintResourceUsage(JSONWriter* writer)
{
    ...
    uint64_t uptime = (uv_hrtime() - per_process::node_start_time) / (NANOS_PER_SEC);
    if (uptime == 0)
    ...
    if (uv_getrusage(&rusage) == 0) {
        double user_cpu = rusage.ru_utime.tv_sec + SEC_PER_MICROS * rusage.ru_utime.tv_usec;
        double kernel_cpu = rusage.ru_stime.tv_sec + SEC_PER_MICROS * rusage.ru_stime.tv_usec;
    ...
    if (getrusage(RUSAGE_THREAD, &stats) == 0) {
        ...
        double user_cpu = stats.ru_utime.tv_sec + SEC_PER_MICROS * stats.ru_utime.tv_usec;
        double kernel_cpu = stats.ru_stime.tv_sec + SEC_PER_MICROS * stats.ru_stime.tv_usec;
        writer->json_keyvalue("userCpuSeconds", user_cpu);
        writer->json_keyvalue("kernelCpuSeconds", kernel_cpu);
        double cpu_abs = user_cpu + kernel_cpu;
        double cpu_percentage = (cpu_abs / uptime) * 100.0;
...
static void PrintLoadedLibraries(JSONWriter* writer)
{
    ...
    std::vector<std::string> modules = NativeSymbolDebuggingContext::GetLoadedLibraries();
...
#define V(key) +1
    std::pair<std::string_view, std::string_view> versions_array[NODE_VERSIONS_KEYS(V)];
#undef V
    auto* slot = &versions_array[0];

#define V(key)                                                                                                                                                 \
    do {                                                                                                                                                       \
        *slot++ = std::pair<std::string_view, std::string_view>(#key, per_process::metadata.versions.key);                                                     \
    } while (0);
    NODE_VERSIONS_KEYS(V)
#undef V

    std::sort(&versions_array[0], &versions_array[arraysize(versions_array)], [](auto& a, auto& b) { return a.first < b.first; });
...
static void PrintRelease(JSONWriter* writer)
{
    ...
    writer->json_keyvalue("headersUrl", per_process::metadata.release.headers_url);
...
std::string TriggerNodeReport(Isolate* isolate, Environment* env, const char* message, const char* trigger, const std::string& name, Local<Value> error)
{
    std::string filename;
    ...
    if (!name.empty()) {
        ...
        if (env != nullptr) {
            THROW_IF_INSUFFICIENT_PERMISSIONS(env, permission::PermissionScope::kFileSystemWrite, name, name);
        ...
        if (report_filename.length() > 0) {
            ...
            filename = *DiagnosticFilename(env != nullptr ? env->thread_id() : 0, "report", "json");
        }
        if (env != nullptr) {
            THROW_IF_INSUFFICIENT_PERMISSIONS(
                env, permission::PermissionScope::kFileSystemWrite, std::string_view(Environment::GetCwd(env->exec_path())), filename);
        }
    }
    ...
    bool exclude_network = env != nullptr ? env->options()->report_exclude_network : per_process::cli_options->per_isolate->per_env->report_exclude_network;
    bool exclude_env = env != nullptr ? env->report_exclude_env() : per_process::cli_options->per_isolate->per_env->report_exclude_env;

    report::WriteNodeReport(isolate, env, message, trigger, filename, *outstream, error, compact, exclude_network, exclude_env);
...
std::string TriggerNodeReport(Isolate* isolate, const char* message, const char* trigger, const std::string& name, Local<Value> error)
{
...
// External function to trigger a report, writing to file.
std::string TriggerNodeReport(Environment* env, const char* message, const char* trigger, const std::string& name, Local<Value> error)
{
    return TriggerNodeReport(env != nullptr ? env->isolate() : nullptr, env, message, trigger, name, error);
}

// External function to trigger a report, writing to a supplied stream.
void GetNodeReport(Isolate* isolate, const char* message, const char* trigger, Local<Value> error, std::ostream& out)
{
    Environment* env = nullptr;
    if (isolate != nullptr) {
        env = Environment::GetCurrent(isolate);
    ...
    bool exclude_network = env != nullptr ? env->options()->report_exclude_network : per_process::cli_options->per_isolate->per_env->report_exclude_network;
    bool exclude_env = env != nullptr ? env->report_exclude_env() : per_process::cli_options->per_isolate->per_env->report_exclude_env;
    report::WriteNodeReport(isolate, env, message, trigger, "", out, error, false, exclude_network, exclude_env);
}

// External function to trigger a report, writing to a supplied stream.
void GetNodeReport(Environment* env, const char* message, const char* trigger, Local<Value> error, std::ostream& out)
{
