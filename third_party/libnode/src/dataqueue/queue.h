...
public:
    ...
    public:
        ...
        // must return a nulled unique_ptr.
        virtual std::unique_ptr<Entry> slice(uint64_t start, std::optional<uint64_t> end = std::nullopt) = 0;
    ...
    static std::shared_ptr<DataQueue> CreateIdempotent(std::vector<std::unique_ptr<Entry>> list);
    ...
    static std::shared_ptr<DataQueue> Create(std::optional<uint64_t> capped = std::nullopt);
    ...
    // returned.
    static std::unique_ptr<Entry> CreateInMemoryEntryFromView(v8::Local<v8::ArrayBufferView> view);
    ...
    static std::unique_ptr<Entry> CreateInMemoryEntryFromBackingStore(std::shared_ptr<v8::BackingStore> store, uint64_t offset, uint64_t length);

    static std::unique_ptr<Entry> CreateDataQueueEntry(std::shared_ptr<DataQueue> data_queue);

    static std::unique_ptr<Entry> CreateFdEntry(Environment* env, v8::Local<v8::Value> path);
    ...
    virtual std::shared_ptr<DataQueue> slice(uint64_t start, std::optional<uint64_t> end = std::nullopt) = 0;
