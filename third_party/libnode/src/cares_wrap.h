...
inline void cares_wrap_hostent_cpy(struct hostent* dest, const struct hostent* src)
{
...
public:
    ChannelWrap(Environment* env, v8::Local<v8::Object> object, int timeout, int tries);
    ...
    inline void set_query_last_ok(bool ok)
    {
        query_last_ok_ = ok;
    }
    ...
    inline int active_query_count()
    {
        return active_query_count_;
    }
    inline NodeAresTask::List* task_list()
    {
        return &task_list_;
    }
...
public:
    GetAddrInfoReqWrap(Environment* env, v8::Local<v8::Object> req_wrap_obj, uint8_t order);
...
template <typename Traits> class QueryWrap final : public AsyncWrap {
public:
    ...
    void AresQuery(const char* name, ares_dns_class_t dnsclass, ares_dns_rec_type_t type)
    {
        channel_->EnsureServers();
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(TRACING_CATEGORY_NODE2(dns, native), trace_name_, this, "name", TRACE_STR_COPY(name));
        ares_query_dnsrec(channel_->cares_channel(), name, dnsclass, type, Callback, MakeCallbackPointer(), nullptr);
    }
    ...
    void ParseError(int status)
    {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_END1(TRACING_CATEGORY_NODE2(dns, native), trace_name_, this, "error", status);
        MakeCallback(env()->oncomplete_string(), 1, &arg);
    }

    const BaseObjectPtr<ChannelWrap>& channel() const
    {
        return channel_;
    }
    ...
    {
        std::unique_ptr<QueryWrap<Traits>*> wrap_ptr { static_cast<QueryWrap<Traits>**>(arg) };
    ...
    static void Callback(void* arg, ares_status_t status, size_t timeouts, const ares_dns_record_t* dnsrec)
    {
    ...
    void CallOnComplete(v8::Local<v8::Value> answer, v8::Local<v8::Value> extra = v8::Local<v8::Value>())
    {
        v8::HandleScope handle_scope(env()->isolate());
        v8::Context::Scope context_scope(env()->context());
        v8::Local<v8::Value> argv[] = { v8::Integer::New(env()->isolate(), 0), answer, extra };
        const int argc = arraysize(argv) - extra.IsEmpty();
        TRACE_EVENT_NESTABLE_ASYNC_END0(TRACING_CATEGORY_NODE2(dns, native), trace_name_, this);
...
struct AnyTraits final {
    ...
    static int Parse(QueryWrap<AnyTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};

struct ATraits final {
    static constexpr const char* name = "resolve4";
    static int Send(QueryWrap<ATraits>* wrap, const char* name);
    static int Parse(QueryWrap<ATraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct AaaaTraits final {
    ...
    static int Parse(QueryWrap<AaaaTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct CaaTraits final {
    ...
    static int Parse(QueryWrap<CaaTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct CnameTraits final {
    ...
    static int Parse(QueryWrap<CnameTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};

struct MxTraits final {
    static constexpr const char* name = "resolveMx";
    static int Send(QueryWrap<MxTraits>* wrap, const char* name);
    static int Parse(QueryWrap<MxTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};

struct NsTraits final {
    static constexpr const char* name = "resolveNs";
    static int Send(QueryWrap<NsTraits>* wrap, const char* name);
    static int Parse(QueryWrap<NsTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct TlsaTraits final {
    ...
    static int Parse(QueryWrap<TlsaTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct TxtTraits final {
    ...
    static int Parse(QueryWrap<TxtTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct SrvTraits final {
    ...
    static int Parse(QueryWrap<SrvTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct PtrTraits final {
    ...
    static int Parse(QueryWrap<PtrTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct NaptrTraits final {
    ...
    static int Parse(QueryWrap<NaptrTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct SoaTraits final {
    ...
    static int Parse(QueryWrap<SoaTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
...
struct ReverseTraits final {
    ...
    static int Parse(QueryWrap<ReverseTraits>* wrap, const std::unique_ptr<ResponseData>& response);
};
