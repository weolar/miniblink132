...
MaybeLocal<String> MakeString(Isolate* isolate, const char* data, size_t length, enum encoding encoding)
{
    Local<Value> error;
    ...
    if (encoding == UTF8) {
        ...
        if (length <= static_cast<size_t>(v8::String::kMaxLength)) {
            utf8_string = String::NewFromUtf8(isolate, data, v8::NewStringType::kNormal, length);
        }
...
MaybeLocal<String> StringDecoder::DecodeData(Isolate* isolate, const char* data, size_t* nread_ptr)
{
...
MaybeLocal<String> StringDecoder::FlushData(Isolate* isolate)
{
    ...
    MaybeLocal<String> ret = MakeString(isolate, IncompleteCharacterBuffer(), BufferedBytes(), Encoding());
...
#define SET_DECODER_CONSTANT(name) target->Set(context, FIXED_ONE_BYTE_STRING(isolate, #name), Integer::New(isolate, StringDecoder::name)).FromJust()
...
#define ADD_TO_ENCODINGS_ARRAY(cname, jsname) encodings->Set(context, static_cast<int32_t>(cname), FIXED_ONE_BYTE_STRING(isolate, jsname)).FromJust()
    ...
    target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "encodings"), encodings).Check();

    target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "kSize"), Integer::New(isolate, sizeof(StringDecoder))).Check();
...
void RegisterStringDecoderExternalReferences(ExternalReferenceRegistry* registry)
{
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(string_decoder, node::InitializeStringDecoder)
NODE_BINDING_EXTERNAL_REFERENCE(string_decoder, node::RegisterStringDecoderExternalReferences)
