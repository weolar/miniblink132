...
template <typename T> void DateTimeConfigurationChangeNotification(Isolate* isolate, const T& key, const char* val = nullptr)
{
    if (key.length() == 2 && key[0] == 'T' && key[1] == 'Z') {
        ...
        isolate->DateTimeConfigurationChangeNotification(Isolate::TimeZoneDetection::kSkip);
...
MaybeLocal<String> RealEnvStore::Get(Isolate* isolate, Local<String> property) const
{
    ...
    if (value.has_value()) {
        ...
        return String::NewFromUtf8(isolate, val.data(), NewStringType::kNormal, val.size());
    }

    return MaybeLocal<String>();
}

void RealEnvStore::Set(Isolate* isolate, Local<String> property, Local<String> value)
{
    Mutex::ScopedLock lock(per_process::env_var_mutex);
...
#ifdef _WIN32
    if (key[0] == '=') {
        return static_cast<int32_t>(ReadOnly) | static_cast<int32_t>(DontDelete) | static_cast<int32_t>(DontEnum);
    }
#endif
...
std::shared_ptr<KVStore> KVStore::Clone(Isolate* isolate) const
{
    ...
    for (uint32_t i = 0; i < keys_length; i++) {
        ...
        CHECK(key->IsString());
        copy->Set(isolate, key.As<String>(), Get(isolate, key.As<String>()).ToLocalChecked());
    }
    return copy;
}
...
void MapKVStore::Set(Isolate* isolate, Local<String> key, Local<String> value)
{
    ...
    if (*key_str != nullptr && key_str.length() > 0 && *value_str != nullptr) {
        map_[std::string(*key_str, key_str.length())] = std::string(*value_str, value_str.length());
    }
}
...
Local<Array> MapKVStore::Enumerate(Isolate* isolate) const
{
    ...
    for (const auto& pair : map_) {
        values.emplace_back(String::NewFromUtf8(isolate, pair.first.data(), NewStringType::kNormal, pair.first.size()).ToLocalChecked());
    }
...
Maybe<void> KVStore::AssignFromObject(Local<Context> context, Local<Object> entries)
{
    ...
    for (uint32_t i = 0; i < keys_length; i++) {
        ...
        if (!entries->Get(context, key).ToLocal(&value) || !value->ToString(context).ToLocal(&value_string)) {
            return Nothing<void>();
        }
...
Maybe<void> KVStore::AssignToObject(v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Object> object)
{
...
template <typename... Args> inline void TraceEnvVarImpl(Environment* env, TraceEnvVarOptions options, const char* format, Args&&... args)
{
...
TraceEnvVarOptions GetTraceEnvVarOptions(Environment* env)
{
    ...
    auto cli_options = env != nullptr ? env->options() : per_process::cli_options->per_isolate->per_env;
...
{
    TraceEnvVarImpl(env, GetTraceEnvVarOptions(env), "[--trace-env] %s\n", message);
}

void TraceEnvVar(Environment* env, const char* message, const char* key)
{
    TraceEnvVarImpl(env, GetTraceEnvVarOptions(env), "[--trace-env] %s \"%s\"\n", message, key);
}
...
void TraceEnvVar(Environment* env, const char* message, v8::Local<v8::String> key)
{
    ...
    if (options.print_message) {
        ...
        TraceEnvVarImpl(env, options, "[--trace-env] %s \"%.*s\"\n", message, static_cast<int>(key_utf8.length()), key_utf8.out());
    }
}

static Intercepted EnvGetter(Local<Name> property, const PropertyCallbackInfo<Value>& info)
{
...
static Intercepted EnvSetter(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
    Environment* env = Environment::GetCurrent(info);
    CHECK(env->has_run_bootstrapping_code());
    ...
    if (env->options()->pending_deprecation && !value->IsString() && !value->IsNumber() && !value->IsBoolean() && env->EmitProcessEnvWarning()) {
    ...
    Local<String> key;
    Local<String> value_string;
    if (!property->ToString(env->context()).ToLocal(&key) || !value->ToString(env->context()).ToLocal(&value_string)) {
        return Intercepted::kNo;
    }
...
static Intercepted EnvQuery(Local<Name> property, const PropertyCallbackInfo<Integer>& info)
{
    Environment* env = Environment::GetCurrent(info);
    CHECK(env->has_run_bootstrapping_code());
...
static Intercepted EnvDeleter(Local<Name> property, const PropertyCallbackInfo<Boolean>& info)
{
    Environment* env = Environment::GetCurrent(info);
    CHECK(env->has_run_bootstrapping_code());
    if (property->IsString()) {
...
static void EnvEnumerator(const PropertyCallbackInfo<Array>& info)
{
    ...
    info.GetReturnValue().Set(env->env_vars()->Enumerate(env->isolate()));
}

static Intercepted EnvDefiner(Local<Name> property, const PropertyDescriptor& desc, const PropertyCallbackInfo<void>& info)
{
    Environment* env = Environment::GetCurrent(info);
    if (desc.has_value()) {
...
void CreateEnvProxyTemplate(IsolateData* isolate_data)
{
    ...
    HandleScope scope(isolate);
    if (!isolate_data->env_proxy_template().IsEmpty())
        return;
    Local<FunctionTemplate> env_proxy_ctor_template = FunctionTemplate::New(isolate);
    Local<ObjectTemplate> env_proxy_template = ObjectTemplate::New(isolate, env_proxy_ctor_template);
    env_proxy_template->SetHandler(NamedPropertyHandlerConfiguration(
        EnvGetter, EnvSetter, EnvQuery, EnvDeleter, EnvEnumerator, EnvDefiner, nullptr, Local<Value>(), PropertyHandlerFlags::kHasNoSideEffect));
