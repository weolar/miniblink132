...
template <typename T, typename... Args> inline void hash_combine(size_t* seed, const T& value, Args... rest)
{
...
{
    return is_numeric_host(hostname, AF_INET) || is_numeric_host(hostname, AF_INET6);
}
...
int SocketAddress::GetPort(const sockaddr* addr)
{
    ...
    return ntohs(addr->sa_family == AF_INET ? reinterpret_cast<const sockaddr_in*>(addr)->sin_port : reinterpret_cast<const sockaddr_in6*>(addr)->sin6_port);
}
...
std::string SocketAddress::GetAddress(const sockaddr* addr)
{
    ...
    const void* src = addr->sa_family == AF_INET ? static_cast<const void*>(&(reinterpret_cast<const sockaddr_in*>(addr)->sin_addr))
                                                 : static_cast<const void*>(&(reinterpret_cast<const sockaddr_in6*>(addr)->sin6_addr));
...
std::string SocketAddress::ToString() const
{
    if (family() != AF_INET && family() != AF_INET6)
        return "";
    return (family() == AF_INET6 ? std::string("[") + address() + "]:" : address() + ":") + std::to_string(port());
}
...
v8::MaybeLocal<v8::Object> SocketAddress::ToJS(Environment* env, v8::Local<v8::Object> info) const
{
...
std::partial_ordering SocketAddress::operator<=>(const SocketAddress& other) const
{
    return compare(other);
}

template <typename T>
SocketAddressLRU<T>::SocketAddressLRU(size_t max_size)
    : max_size_(max_size)
{
}

template <typename T> typename T::Type* SocketAddressLRU<T>::Peek(const SocketAddress& address) const
{
    auto it = map_.find(address);
    return it == std::end(map_) ? nullptr : &it->second->second;
}

template <typename T> void SocketAddressLRU<T>::CheckExpired()
{
...
template <typename T> void SocketAddressLRU<T>::MemoryInfo(MemoryTracker* tracker) const
{
...
// purge expired items.
template <typename T> typename T::Type* SocketAddressLRU<T>::Upsert(const SocketAddress& address)
{
...
v8::MaybeLocal<v8::Value> SocketAddressBlockList::Rule::ToV8String(Environment* env)
{
