...
using BindingDataStore = std::array<BaseObjectWeakPtr<BaseObject>, static_cast<size_t>(BindingDataType::kBindingDataTypeCount)>;
...
public:
    ...
    static inline Realm* GetCurrent(const v8::FunctionCallbackInfo<v8::Value>& info);
    template <typename T> static inline Realm* GetCurrent(const v8::PropertyCallbackInfo<T>& info);
    ...
    template <typename T> void ForEachBaseObject(T&& iterator) const;
    ...
    template <typename T, typename... Args> T* AddBindingData(v8::Local<v8::Object> target, Args&&... args);
    template <typename T, typename U> static inline T* GetBindingData(const v8::PropertyCallbackInfo<U>& info);
    template <typename T> static inline T* GetBindingData(const v8::FunctionCallbackInfo<v8::Value>& info);
    template <typename T> static inline T* GetBindingData(v8::Local<v8::Context> context);
    template <typename T> inline T* GetBindingData();
...
private:
    void InitializeContext(v8::Local<v8::Context> context, const RealmSerializeInfo* realm_info);
...
public:
    PrincipalRealm(Environment* env, v8::Local<v8::Context> context, const RealmSerializeInfo* realm_info);
    ~PrincipalRealm();
