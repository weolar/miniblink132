...
ShadowRealm* ShadowRealm::New(Environment* env)
{
    ...
    // realms.
    realm->context()->SetSecurityToken(env->principal_realm()->context()->GetSecurityToken());
...
// static
MaybeLocal<Context> HostCreateShadowRealmContextCallback(Local<Context> initiator_context)
{
...
#define V(PropertyName, TypeName)                                                                                                                              \
    ...
    {                                                                                                                                                          \
        ...
        if (value.IsEmpty()) {                                                                                                                                 \
            ctx->Global()->SetPrivate(ctx, isolate_data()->per_realm_##PropertyName(), v8::Undefined(isolate_)).ToChecked();                                   \
        } else {                                                                                                                                               \
            PropertyName##_.SetWeak();                                                                                                                         \
            ctx->Global()->SetPrivate(ctx, isolate_data()->per_realm_##PropertyName(), value).ToChecked();                                                     \
        }                                                                                                                                                      \
    }
