...
inline Local<ArrayBuffer> get_fields_array_buffer(const FunctionCallbackInfo<Value>& args, size_t index, size_t array_length)
{
...
static void ThreadCPUUsage(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (err)
        return env->ThrowUVException(err, "uv_getrusage_thread");
...
static void Cwd(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<String> cwd;
    if (String::NewFromUtf8(env->isolate(), buf, NewStringType::kNormal, cwd_len).ToLocal(&cwd)) {
...
static void Kill(const FunctionCallbackInfo<Value>& args)
{
    ...
    int pid;
    if (!args[0]->Int32Value(context).To(&pid))
        return;
    int sig;
    if (!args[1]->Int32Value(context).To(&sig))
        return;

    uv_pid_t own_pid = uv_os_getpid();
    if (sig > 0 && (pid == 0 || pid == -1 || pid == own_pid || pid == -own_pid) && !HasSignalJSHandler(sig)) {
...
static void MemoryUsage(const FunctionCallbackInfo<Value>& args)
{
    ...
    NodeArrayBufferAllocator* array_buffer_allocator = env->isolate_data()->node_allocator();
    ...
    fields[4] = array_buffer_allocator == nullptr ? 0 : static_cast<double>(array_buffer_allocator->total_mem_usage());
}
...
static void Uptime(const FunctionCallbackInfo<Value>& args)
{
    ...
    double uptime = static_cast<double>(uv_hrtime() - per_process::node_start_time);
...
static void GetActiveRequests(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(Array::New(env->isolate(), request_v.data(), request_v.size()));
}
...
void GetActiveHandles(const FunctionCallbackInfo<Value>& args)
{
    ...
    args.GetReturnValue().Set(Array::New(env->isolate(), handle_v.data(), handle_v.size()));
}
...
static void GetActiveResourcesInfo(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (ReqWrapBase* req_wrap : *env->req_wrap_queue()) {
        ...
        if (w->persistent().IsEmpty())
            continue;
        resources_info.emplace_back(OneByteString(env->isolate(), w->MemoryInfoName()));
    }
    ...
    // Active timeouts
    resources_info.insert(resources_info.end(), env->timeout_info()[0], FIXED_ONE_BYTE_STRING(env->isolate(), "Timeout"));

    // Active immediates
    resources_info.insert(resources_info.end(), env->immediate_info()->ref_count(), FIXED_ONE_BYTE_STRING(env->isolate(), "Immediate"));

    args.GetReturnValue().Set(Array::New(env->isolate(), resources_info.data(), resources_info.size()));
}
...
static void DebugProcess(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (process == nullptr) {
        isolate->ThrowException(WinapiErrnoException(isolate, GetLastError(), "OpenProcess"));
        return;
    }
    ...
    if (mapping == nullptr) {
        isolate->ThrowException(WinapiErrnoException(isolate, GetLastError(), "OpenFileMappingW"));
        return;
    }

    handler = reinterpret_cast<LPTHREAD_START_ROUTINE*>(MapViewOfFile(mapping, FILE_MAP_READ, 0, 0, sizeof *handler));
    if (handler == nullptr || *handler == nullptr) {
        isolate->ThrowException(WinapiErrnoException(isolate, GetLastError(), "MapViewOfFile"));
        return;
    }

    thread = CreateRemoteThread(process, nullptr, 0, *handler, nullptr, 0, nullptr);
    if (thread == nullptr) {
        isolate->ThrowException(WinapiErrnoException(isolate, GetLastError(), "CreateRemoteThread"));
        return;
    }

    // Wait for the thread to terminate
    if (WaitForSingleObject(thread, INFINITE) != WAIT_OBJECT_0) {
        isolate->ThrowException(WinapiErrnoException(isolate, GetLastError(), "WaitForSingleObject"));
        return;
    }
}
#endif // _WIN32
...
static void Execve(const FunctionCallbackInfo<Value>& args)
{
    ...
    for (unsigned int i = 0; i < envp_array->Length(); i++) {
        ...
        if (!envp_array->Get(context, i).ToLocal(&str)) {
            THROW_ERR_INVALID_ARG_VALUE(env, "Failed to deserialize environment variable.");
            return;
        }
    ...
    auto error_message = std::string("process.execve failed with error code ") + errors::errno_string(errno);

    // Abort the process
    Local<v8::Value> exception = ErrnoException(isolate, errno, "execve", *executable);
    Local<v8::Message> message = v8::Exception::CreateMessage(isolate, exception);

    std::string info = FormatErrorMessage(isolate, context, error_message.c_str(), message, true);
    FPrintF(stderr, "%s\n", info);
    ABORT();
}
#endif
...
static void LoadEnvFile(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    THROW_IF_INSUFFICIENT_PERMISSIONS(env, permission::PermissionScope::kFileSystemRead, path);
    ...
    case dotenv.ParseResult::InvalidContent: {
        THROW_ERR_INVALID_ARG_TYPE(env, "Contents of '%s' should be a valid string.", path.c_str());
        break;
    }
...
BindingData::BindingData(Realm* realm, v8::Local<v8::Object> object, InternalFieldInfo* info)
    ...
    , hrtime_buffer_(realm->isolate(), kHrTimeBufferLength, MAYBE_FIELD_PTR(info, hrtime_buffer))
{
    Isolate* isolate = realm->isolate();
    Local<Context> context = realm->context();

    if (info == nullptr) {
        object->Set(context, FIXED_ONE_BYTE_STRING(isolate, "hrtimeBuffer"), hrtime_buffer_.GetJSArray()).ToChecked();
    } else {
...
{
    SetFastMethodNoSideEffect(isolate, target, "hrtime", SlowNumber, &fast_number_);
    SetFastMethodNoSideEffect(isolate, target, "hrtimeBigInt", SlowBigInt, &fast_bigint_);
}
...
BindingData* BindingData::FromV8Value(Local<Value> value)
{
    ...
    return static_cast<BindingData*>(v8_object->GetAlignedPointerFromInternalField(BaseObject::kSlot));
}
...
void BindingData::BigIntImpl(BindingData* receiver)
{
    ...
    uint64_t* fields = reinterpret_cast<uint64_t*>(const_cast<uint32_t*>(receiver->hrtime_buffer_.GetNativeBuffer()));
    fields[0] = t;
}
...
bool BindingData::PrepareForSerialization(Local<Context> context, v8::SnapshotCreator* creator)
{
    ...
    internal_field_info_->hrtime_buffer = hrtime_buffer_.Serialize(context, creator);
...
NODE_BINDING_EXTERNAL_REFERENCE(process_methods, node::process::RegisterExternalReferences)
