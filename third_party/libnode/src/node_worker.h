...
enum ResourceLimits { kMaxYoungGenerationSizeMb, kMaxOldGenerationSizeMb, kCodeRangeSizeMb, kStackSizeMb, kTotalResourceLimitCount };
...
public:
    ...
    void Exit(ExitCode code, const char* error_code = nullptr, const char* error_message = nullptr);

    // Wait for the worker thread to stop (in a blocking manner).
    void JoinThread();

    template <typename Fn> inline bool RequestInterrupt(Fn&& cb);
    ...
    static void CloneParentEnvVars(const v8::FunctionCallbackInfo<v8::Value>& args);
    ...
    static void GetResourceLimits(const v8::FunctionCallbackInfo<v8::Value>& args);
...
private:
    ...
    static size_t NearHeapLimit(void* data, size_t current_heap_limit, size_t initial_heap_limit);
...
template <typename Fn> bool Worker::RequestInterrupt(Fn&& cb)
{
