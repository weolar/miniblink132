...
template <> struct MemoryRetainerTraits<ada::url_pattern<URLPatternRegexProvider>> {
    ...
    static size_t SelfSize(const Type& value)
    {
        return sizeof(value);
    }
};

template <> struct MemoryRetainerTraits<ada::url_pattern_component<URLPatternRegexProvider>> {
...
std::optional<URLPatternRegexProvider::regex_type> URLPatternRegexProvider::create_instance(std::string_view pattern, bool ignore_case)
{
    ...
    if (!String::NewFromUtf8(isolate, pattern.data(), NewStringType::kNormal, pattern.size()).ToLocal(&local_pattern)) {
        return std::nullopt;
    }
    Local<RegExp> regexp;
    if (!RegExp::New(env->context(), local_pattern, static_cast<RegExp::Flags>(flags)).ToLocal(&regexp)) {
        return std::nullopt;
    }
    return Global<RegExp>(isolate, regexp);
}

bool URLPatternRegexProvider::regex_match(std::string_view input, const regex_type& pattern)
{
    auto isolate = Isolate::GetCurrent();
    auto env = Environment::GetCurrent(isolate);
    Local<String> local_input;
    if (!String::NewFromUtf8(isolate, input.data(), NewStringType::kNormal, input.size()).ToLocal(&local_input)) {
        return false;
    }
    Local<Object> result_object;
    if (!pattern.Get(isolate)->Exec(env->context(), local_input).ToLocal(&result_object)) {
        return false;
    }
...
std::optional<std::vector<std::optional<std::string>>> URLPatternRegexProvider::regex_search(std::string_view input, const regex_type& global_pattern)
{
    auto isolate = Isolate::GetCurrent();
    auto env = Environment::GetCurrent(isolate);
    Local<String> local_input;
    ...
    if (!pattern->Exec(env->context(), local_input).ToLocal(&exec_result_object) || exec_result_object->IsNull()) {
        return std::nullopt;
    }
...
URLPattern::URLPattern(Environment* env, Local<Object> object, ada::url_pattern<URLPatternRegexProvider>&& url_pattern)
...
#define URL_PATTERN_CACHED_VALUES(_, lowercase) tracker->TrackField(#lowercase, lowercase);
...
void URLPattern::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() > 1) {
        if (args[1]->IsString()) {
            ...
            THROW_ERR_INVALID_ARG_TYPE(env, "second argument must be a string or object");
            return;
        }
    ...
    auto url_pattern
        = parse_url_pattern<URLPatternRegexProvider>(std::move(arg0), base_url ? &base_url_view : nullptr, options.has_value() ? &options.value() : nullptr);
...
MaybeLocal<Value> URLPattern::URLPatternInit::ToJsObject(Environment* env, const ada::url_pattern_init& init)
{
    ...
    const auto trySet = [&](auto name, const std::optional<std::string>& val) {
        if (!val)
            return true;
        Local<Value> temp;
        return ToV8Value(context, *val).ToLocal(&temp) && result->Set(context, name, temp).IsJust();
    };

    if (!trySet(env->protocol_string(), init.protocol) || !trySet(env->username_string(), init.username) || !trySet(env->password_string(), init.password)
        || !trySet(env->hostname_string(), init.hostname) || !trySet(env->port_string(), init.port) || !trySet(env->pathname_string(), init.pathname)
        || !trySet(env->search_string(), init.search) || !trySet(env->hash_string(), init.hash) || !trySet(env->base_url_string(), init.base_url)) {
        return {};
    }
    return result;
}

std::optional<ada::url_pattern_init> URLPattern::URLPatternInit::FromJsObject(Environment* env, Local<Object> obj)
{
...
MaybeLocal<Object> URLPattern::URLPatternComponentResult::ToJSObject(Environment* env, const ada::url_pattern_component_result& result)
{
    auto isolate = env->isolate();
    auto context = env->context();
    ...
    return Object::New(isolate, Object::New(isolate), names, values, arraysize(names));
}

MaybeLocal<Value> URLPattern::URLPatternResult::ToJSValue(Environment* env, const ada::url_pattern_result& result)
{
    auto isolate = env->isolate();
    Local<Name> names[] = {
        env->inputs_string(),
    ...
    if (!URLPatternComponentResult::ToJSObject(env, result.protocol).ToLocal(&values[1])
        || !URLPatternComponentResult::ToJSObject(env, result.username).ToLocal(&values[2])
        || !URLPatternComponentResult::ToJSObject(env, result.password).ToLocal(&values[3])
        || !URLPatternComponentResult::ToJSObject(env, result.hostname).ToLocal(&values[4])
        || !URLPatternComponentResult::ToJSObject(env, result.port).ToLocal(&values[5])
        || !URLPatternComponentResult::ToJSObject(env, result.pathname).ToLocal(&values[6])
        || !URLPatternComponentResult::ToJSObject(env, result.search).ToLocal(&values[7])
        || !URLPatternComponentResult::ToJSObject(env, result.hash).ToLocal(&values[8])) {
        return {};
    }
    return Object::New(isolate, Object::New(isolate), names, values.data(), values.size());
}

std::optional<ada::url_pattern_options> URLPattern::URLPatternOptions::FromJsObject(Environment* env, Local<Object> obj)
{
    ada::url_pattern_options options {};
    Local<Value> ignore_case;
    if (obj->Get(env->context(), env->ignore_case_string()).ToLocal(&ignore_case)) {
...
#define URL_PATTERN_COMPONENT_GETTERS(uppercase_name, lowercase_name)                                                                                          \
    ...
    {                                                                                                                                                          \
        ...
        if (lowercase_name.IsEmpty()) {                                                                                                                        \
            Local<Value> value;                                                                                                                                \
            if (ToV8Value(env()->context(), url_pattern_.get_##lowercase_name()).ToLocal(&value)) {                                                            \
...
MaybeLocal<Value> URLPattern::Exec(Environment* env, const ada::url_pattern_input& input, std::optional<std::string_view>& baseURL)
{
...
bool URLPattern::Test(Environment* env, const ada::url_pattern_input& input, std::optional<std::string_view>& baseURL)
{
...
void URLPattern::Exec(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args.Length() == 0) {
        ...
        THROW_ERR_INVALID_ARG_TYPE(env, "URLPattern input needs to be a string or an object");
        return;
    }
    ...
    Local<Value> result;
    std::optional<std::string_view> baseURL_opt = baseURL ? std::optional<std::string_view>(*baseURL) : std::nullopt;
...
#define URL_PATTERN_COMPONENT_GETTERS(uppercase_name, _) registry->Register(URLPattern::uppercase_name);
    URL_PATTERN_COMPONENTS(URL_PATTERN_COMPONENT_GETTERS)
#undef URL_PATTERN_COMPONENT_GETTERS
...
#define URL_PATTERN_COMPONENT_GETTERS(uppercase_name, lowercase_name)                                                                                          \
    prototype_template->SetAccessorProperty(ENV_GETTER(lowercase_name), FunctionTemplate::New(isolate, URLPattern::uppercase_name, Local<Value>(), signature), \
        Local<FunctionTemplate>(), attributes);
    URL_PATTERN_COMPONENTS(URL_PATTERN_COMPONENT_GETTERS)
#undef URL_PATTERN_COMPONENT_GETTERS
#undef ENV_GETTER

    prototype_template->SetAccessorProperty(env->has_regexp_groups_string(),
        FunctionTemplate::New(isolate, URLPattern::HasRegexpGroups, Local<Value>(), signature), Local<FunctionTemplate>(), attributes);
...
NODE_BINDING_EXTERNAL_REFERENCE(url_pattern, node::url_pattern::RegisterExternalReferences)
