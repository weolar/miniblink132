...
template <typename T> class CopyOnWrite final {
public:
...
template <typename T> class ThreadsafeCopyOnWrite final {
private:
    ...
    struct Impl {
        explicit Impl(const T& data)
            : data(data)
        {
        }
        explicit Impl(T&& data)
            : data(std::move(data))
        {
        }
...
public:
    ...
    ThreadsafeCopyOnWrite& operator=(const ThreadsafeCopyOnWrite<T>& other) = default;
