...
void* const ContextEmbedderTag::kNodeContextTagPtr = const_cast<void*>(static_cast<const void*>(&ContextEmbedderTag::kNodeContextTag));

void AsyncHooks::ResetPromiseHooks(Local<Function> init, Local<Function> before, Local<Function> after, Local<Function> resolve)
{
...
void Environment::ResetPromiseHooks(Local<Function> init, Local<Function> before, Local<Function> after, Local<Function> resolve)
{
...
void AsyncHooks::push_async_context(double async_id, double trigger_async_id, Local<Object> resource)
{
...
bool AsyncHooks::pop_async_context(double async_id)
{
    ...
    if (offset < native_execution_async_resources_.size() && !native_execution_async_resources_[offset].IsEmpty())
        [[likely]]
        {
#ifdef DEBUG
            for (uint32_t i = offset + 1; i < native_execution_async_resources_.size(); i++) {
                CHECK(native_execution_async_resources_[i].IsEmpty());
            }
#endif
    ...
    if (js_execution_async_resources()->Length() > offset)
        [[unlikely]]
        {
            HandleScope handle_scope(env()->isolate());
            USE(js_execution_async_resources()->Set(env()->context(), env()->length_string(), Integer::NewFromUnsigned(env()->isolate(), offset)));
        }
...
void AsyncHooks::clear_async_id_stack()
{
    if (!js_execution_async_resources_.IsEmpty() && env()->can_call_into_js()) {
        ...
        USE(PersistentToLocal::Strong(js_execution_async_resources_)->Set(env()->context(), env()->length_string(), Integer::NewFromUnsigned(isolate, 0)));
    }
...
void Environment::UntrackContext(Local<Context> context)
{
    ...
    PurgeTrackedEmptyContexts();
    for (auto it = contexts_.begin(); it != contexts_.end(); it++) {
        if (Local<Context> saved_context = PersistentToLocal::Weak(isolate_, *it); saved_context == context) {
            it->Reset();
            contexts_.erase(it);
            break;
        }
...
AsyncHooks::DefaultTriggerAsyncIdScope::DefaultTriggerAsyncIdScope(Environment* env, double default_trigger_async_id)
    : async_hooks_(env->async_hooks())
{
    ...
    old_default_trigger_async_id_ = async_hooks_->async_id_fields()[AsyncHooks::kDefaultTriggerAsyncId];
    async_hooks_->async_id_fields()[AsyncHooks::kDefaultTriggerAsyncId] = default_trigger_async_id;
}

AsyncHooks::DefaultTriggerAsyncIdScope::~DefaultTriggerAsyncIdScope()
{
    async_hooks_->async_id_fields()[AsyncHooks::kDefaultTriggerAsyncId] = old_default_trigger_async_id_;
}

AsyncHooks::DefaultTriggerAsyncIdScope::DefaultTriggerAsyncIdScope(AsyncWrap* async_wrap)
    : DefaultTriggerAsyncIdScope(async_wrap->env(), async_wrap->get_async_id())
{
}

std::ostream& operator<<(std::ostream& output, const std::vector<SnapshotIndex>& v)
{
    output << "{ ";
...
std::ostream& operator<<(std::ostream& output, const IsolateDataSerializeInfo& i)
{
    output << "{\n"
           << "// -- primitive begins --\n"
           << i.primitive_values << ",\n"
...
{
    output << "{\n"
           << "  " << (i.type == SnapshotMetadata::Type::kDefault ? "SnapshotMetadata::Type::kDefault" : "SnapshotMetadata::Type::kFullyCustomized")
           << ", // type\n"
...
#define V(TypeName, PropertyName) info.primitive_values.push_back(creator->AddData(PropertyName##_.Get(isolate)));
...
#undef VP
    ...
    info.primitive_values.reserve(info.primitive_values.size() + AsyncWrap::PROVIDERS_LENGTH);
...
#define V(PropertyName, StringValue)                                                                                                                           \
    PropertyName##_.Set(isolate_,                                                                                                                              \
        Private::New(isolate_,                                                                                                                                 \
            String::NewFromOneByte(isolate_, reinterpret_cast<const uint8_t*>(StringValue), NewStringType::kInternalized, sizeof(StringValue) - 1)             \
                .ToLocalChecked()));
    PER_ISOLATE_PRIVATE_SYMBOL_PROPERTIES(V)
#undef V
#define V(PropertyName, TypeName)                                                                                                                              \
    per_realm_##PropertyName##_.Set(isolate_,                                                                                                                  \
        Private::New(isolate_,                                                                                                                                 \
            String::NewFromOneByte(                                                                                                                            \
                isolate_, reinterpret_cast<const uint8_t*>("per_realm_" #PropertyName), NewStringType::kInternalized, sizeof("per_realm_" #PropertyName) - 1)  \
                .ToLocalChecked()));
...
#define V(Provider)                                                                                                                                            \
    async_wrap_providers_[AsyncWrap::PROVIDER_##Provider].Set(isolate_,                                                                                        \
        String::NewFromOneByte(isolate_, reinterpret_cast<const uint8_t*>(#Provider), NewStringType::kInternalized, sizeof(#Provider) - 1).ToLocalChecked());
...
std::unordered_map<uint16_t, std::unique_ptr<PerIsolateWrapperData>> IsolateData::wrapper_data_map_;

IsolateData* IsolateData::CreateIsolateData(Isolate* isolate, uv_loop_t* loop, MultiIsolatePlatform* platform, ArrayBufferAllocator* allocator,
    const EmbedderSnapshotData* embedder_snapshot_data, std::shared_ptr<PerIsolateOptions> options)
{
    ...
    return new IsolateData(isolate, loop, platform, allocator, snapshot_data, options);
}

IsolateData::IsolateData(Isolate* isolate, uv_loop_t* event_loop, MultiIsolatePlatform* platform, ArrayBufferAllocator* node_allocator,
    ...
    , options_(std::move(options))
{
...

    {
        ...
        if (it == wrapper_data_map_.end()) {
            auto pair = wrapper_data_map_.emplace(cppgc_id, new PerIsolateWrapperData { cppgc_id, non_cppgc_id });
            it = pair.first;
        }
...
void SetCppgcReference(Isolate* isolate, Local<Object> object, void* wrappable)
{
    v8::Object::Wrap<v8::CppHeapPointerTag::kDefaultTag>(isolate, object, wrappable);
}

void IsolateData::MemoryInfo(MemoryTracker* tracker) const
{
#define V(PropertyName, StringValue) tracker->TrackField(#PropertyName, PropertyName());
    PER_ISOLATE_SYMBOL_PROPERTIES(V)
...
#undef V
    ...
    if (node_allocator_ != nullptr) {
        tracker->TrackFieldWithSize("node_allocator", sizeof(*node_allocator_), "NodeArrayBufferAllocator");
    }
    tracker->TrackFieldWithSize("platform", sizeof(*platform_), "MultiIsolatePlatform");
...
void TrackingTraceStateObserver::UpdateTraceCategoryState()
{
    ...
    bool async_hooks_enabled = (*(TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACING_CATEGORY_NODE1(async_hooks)))) != 0;
...
void Environment::AssignToContext(Local<v8::Context> context, Realm* realm, const ContextInfo& info)
{
...
void Environment::TryLoadAddon(const char* filename, int flags, const std::function<bool(binding::DLib*)>& was_loaded)
{
...
{
    std::unique_ptr<BackingStore> bs = ArrayBuffer::NewBackingStore(isolate(), suggested_size, BackingStoreInitializationMode::kUninitialized);
    uv_buf_t buf = uv_buf_init(static_cast<char*>(bs->Data()), bs->ByteLength());
    released_allocated_buffers_.emplace(buf.base, std::move(bs));
    return buf;
}

std::unique_ptr<BackingStore> Environment::release_managed_buffer(const uv_buf_t& buf)
{
...
Environment::Environment(IsolateData* isolate_data, Isolate* isolate, const std::vector<std::string>& args, const std::vector<std::string>& exec_args,
    const EnvSerializeInfo* env_info, EnvironmentFlags::Flags flags, ThreadId thread_id)
    : isolate_(isolate)
    ...
    , exit_info_(isolate_, kExitInfoFieldCount, MAYBE_FIELD_PTR(env_info, exit_info))
    , should_abort_on_uncaught_toggle_(isolate_, 1, MAYBE_FIELD_PTR(env_info, should_abort_on_uncaught_toggle))
    ...
    , flags_(flags)
    , thread_id_(thread_id.id == static_cast<uint64_t>(-1) ? AllocateEnvironmentThreadId().id : thread_id.id)
{
    ...
    if (is_shared_ro_heap && !is_main_thread()) {
        ...
        builtin_loader()->CopySourceAndCodeCacheReferenceFrom(isolate_data->worker_context()->env()->builtin_loader());
        ...
        per_process::Debug(DebugCategory::CODE_CACHE, "snapshot contains %zu code cache\n", cache_size);
        if (cache_size > 0) {
            builtin_loader()->RefreshCodeCache(isolate_data->snapshot_data()->code_cache);
        }
    }
    ...
    if (flags_ & EnvironmentFlags::kDefaultFlags) {
        flags_ = flags_ | EnvironmentFlags::kOwnsProcessState | EnvironmentFlags::kOwnsInspector;
    }
    ...
    options_ = std::make_shared<EnvironmentOptions>(*isolate_data->options()->per_env);
    inspector_host_port_ = std::make_shared<ExclusiveAccess<HostPort>>(options_->debug_options().host_port);
    ...
    heap_snapshot_near_heap_limit_ = static_cast<uint32_t>(options_->heap_snapshot_near_heap_limit);
    ...
    performance_state_
        = std::make_unique<performance::PerformanceState>(isolate, time_origin_, time_origin_timestamp_, MAYBE_FIELD_PTR(env_info, performance_state));

    if (*TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACING_CATEGORY_NODE1(environment)) != 0) {
        ...
        for (const std::string& arg : exec_args)
            traced_value->AppendString(arg);
        traced_value->EndArray();
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(TRACING_CATEGORY_NODE1(environment), "Environment", this, "args", std::move(traced_value));
    }
    ...
    if (options_->permission) {
        ...
        if (!options_->allow_fs_read.empty()) {
            permission()->Apply(this, options_->allow_fs_read, permission::PermissionScope::kFileSystemRead);
        }

        if (!options_->allow_fs_write.empty()) {
            permission()->Apply(this, options_->allow_fs_write, permission::PermissionScope::kFileSystemWrite);
        }
    }
}

void Environment::InitializeMainContext(Local<Context> context, const EnvSerializeInfo* env_info)
{
    principal_realm_ = std::make_unique<PrincipalRealm>(this, context, MAYBE_FIELD_PTR(env_info, principal_realm));
    if (env_info != nullptr) {
    ...
    performance_state_->Mark(performance::NODE_PERFORMANCE_MILESTONE_ENVIRONMENT, environment_start_);
    performance_state_->Mark(performance::NODE_PERFORMANCE_MILESTONE_NODE_START, per_process::node_start_time);

    if (per_process::v8_initialized) {
        performance_state_->Mark(performance::NODE_PERFORMANCE_MILESTONE_V8_START, performance::performance_v8_start);
    }
}
...
Environment::~Environment()
{
    ...
    if (Environment** interrupt_data = interrupt_data_.load()) {
        ...
        *interrupt_data = nullptr;
        ...
        isolate()->RequestInterrupt([](Isolate*, void* data) { *static_cast<bool*>(data) = true; }, &consistency_check);
#endif
    ...
    isolate()->GetHeapProfiler()->RemoveBuildEmbedderGraphCallback(BuildEmbedderGraph, this);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0(TRACING_CATEGORY_NODE1(environment), "Environment", this);
...
void Environment::InitializeLibuv()
{
    ...
    CHECK_EQ(0, uv_async_init(event_loop(), &task_queues_async_, [](uv_async_t* async) {
...

    {
        ...
        if (native_immediates_threadsafe_.size() > 0 || native_immediates_interrupts_.size() > 0) {
...
void Environment::InitializeCompileCache()
{
    ...
    if (!credentials::SafeGetenv("NODE_COMPILE_CACHE", &dir_from_env, this) || dir_from_env.empty()) {
        return;
    }
    EnableCompileCache(dir_from_env);
}

CompileCacheEnableResult Environment::EnableCompileCache(const std::string& cache_dir)
{
    CompileCacheEnableResult result;
    std::string disable_env;
    if (credentials::SafeGetenv("NODE_DISABLE_COMPILE_CACHE", &disable_env, this)) {
        result.status = CompileCacheEnableStatus::DISABLED;
        result.message = "Disabled by NODE_DISABLE_COMPILE_CACHE";
        Debug(this, DebugCategory::COMPILE_CACHE, "[compile cache] %s.\n", result.message);
        return result;
    }

    if (!compile_cache_handler_) {
        std::unique_ptr<CompileCacheHandler> handler = std::make_unique<CompileCacheHandler>(this);
        ...
        if (result.status == CompileCacheEnableStatus::ENABLED) {
            ...
            AtExit([](void* env) { static_cast<Environment*>(env)->FlushCompileCache(); }, this);
        }
...
MaybeLocal<Value> Environment::RunSnapshotSerializeCallback() const
{
    ...
    if (!snapshot_serialize_callback().IsEmpty()) {
        Context::Scope context_scope(context());
        return handle_scope.EscapeMaybe(snapshot_serialize_callback()->Call(context(), v8::Undefined(isolate()), 0, nullptr));
    }
...
MaybeLocal<Value> Environment::RunSnapshotDeserializeMain() const
{
    ...
    if (!snapshot_deserialize_main().IsEmpty()) {
        Context::Scope context_scope(context());
        return handle_scope.EscapeMaybe(snapshot_deserialize_main()->Call(context(), v8::Undefined(isolate()), 0, nullptr));
    }
    return handle_scope.Escape(Undefined(isolate()));
}
...
void Environment::RunCleanup()
{
    ...
    while (!cleanup_queue_.empty() || principal_realm_->PendingCleanup() || native_immediates_.size() > 0 || native_immediates_threadsafe_.size() > 0
...
Maybe<bool> Environment::CheckUnsettledTopLevelAwait() const
{
    ...
    if (!ctx->Global()->GetPrivate(ctx, entry_point_promise_private_symbol()).ToLocal(&entry_point_promise)) {
        return v8::Nothing<bool>();
    }
    if (!entry_point_promise->IsPromise()) {
        return v8::Just(true);
    }
    if (entry_point_promise.As<Promise>()->State() != Promise::PromiseState::kPending) {
        return v8::Just(true);
    }

    if (!ctx->Global()->GetPrivate(ctx, entry_point_module_private_symbol()).ToLocal(&value)) {
        return v8::Nothing<bool>();
    }
    if (!value->IsObject()) {
        return v8::Just(true);
    ...
    CHECK_EQ(object->InternalFieldCount(), loader::ModuleWrap::kInternalFieldCount);
...
{
    TRACE_EVENT0(TRACING_CATEGORY_NODE1(environment), "RunAndClearNativeImmediates");
    HandleScope handle_scope(isolate_);
...
void Environment::CheckImmediate(uv_check_t* handle)
{
    ...
    do {
        MakeCallback(env->isolate(), env->process_object(), env->immediate_callback_function(), 0, nullptr, { 0, 0 }).ToLocalChecked();
...
Maybe<void> CollectExceptionInfo(
    Environment* env, Local<Object> obj, int errorno, const char* err_string, const char* syscall, const char* message, const char* path, const char* dest)
{
    if (obj->Set(env->context(), env->errno_string(), Integer::New(env->isolate(), errorno)).IsNothing()
        || obj->Set(env->context(), env->code_string(), OneByteString(env->isolate(), err_string)).IsNothing()
        || (message != nullptr && obj->Set(env->context(), env->message_string(), OneByteString(env->isolate(), message)).IsNothing())) {
        return Nothing<void>();
    }

    Local<Value> path_buffer;
    if (path != nullptr) {
        if (!Buffer::Copy(env->isolate(), path, strlen(path)).ToLocal(&path_buffer) || obj->Set(env->context(), env->path_string(), path_buffer).IsNothing()) {
            return Nothing<void>();
        }
    }
    ...
    if (dest != nullptr) {
        if (!Buffer::Copy(env->isolate(), dest, strlen(dest)).ToLocal(&dest_buffer) || obj->Set(env->context(), env->dest_string(), dest_buffer).IsNothing()) {
            return Nothing<void>();
        }
    }

    if (syscall != nullptr) {
        if (obj->Set(env->context(), env->syscall_string(), OneByteString(env->isolate(), syscall)).IsNothing()) {
            return Nothing<void>();
        }
    }
...
Maybe<void> Environment::CollectUVExceptionInfo(Local<Value> object, int errorno, const char* syscall, const char* message, const char* path, const char* dest)
{
    ...
    return CollectExceptionInfo(this, obj, errorno, err_string, syscall, message, path, dest);
}
...
ImmediateInfo::SerializeInfo ImmediateInfo::Serialize(Local<Context> context, SnapshotCreator* creator)
{
...
std::ostream& operator<<(std::ostream& output, const ImmediateInfo::SerializeInfo& i)
{
    output << "{ " << i.fields << " }";
    return output;
}
...
TickInfo::SerializeInfo TickInfo::Serialize(Local<Context> context, SnapshotCreator* creator)
{
    return { fields_.Serialize(context, creator) };
}
...
std::ostream& operator<<(std::ostream& output, const TickInfo::SerializeInfo& i)
{
    output << "{ " << i.fields << " }";
    return output;
}
...
TickInfo::TickInfo(Isolate* isolate, const SerializeInfo* info)
    : fields_(isolate, kFieldsCount, info == nullptr ? nullptr : &(info->fields))
{
}
...
AsyncHooks::AsyncHooks(Isolate* isolate, const SerializeInfo* info)
    ...
    , fields_(isolate, kFieldsCount, MAYBE_FIELD_PTR(info, fields))
    , async_id_fields_(isolate, kUidFieldsCount, MAYBE_FIELD_PTR(info, async_id_fields))
...
void AsyncHooks::Deserialize(Local<Context> context)
{
    ...
    if (info_->js_execution_async_resources != 0) {
        js_execution_async_resources = context->GetDataFromSnapshotOnce<Array>(info_->js_execution_async_resources).ToLocalChecked();
    } else {
        js_execution_async_resources = Array::New(context->GetIsolate());
    }
    js_execution_async_resources_.Reset(context->GetIsolate(), js_execution_async_resources);
    ...
    for (size_t i = 0; i < info_->native_execution_async_resources.size(); ++i) {
        if (info_->native_execution_async_resources[i] == SIZE_MAX)
            continue;
        Local<Object> obj = context->GetDataFromSnapshotOnce<Object>(info_->native_execution_async_resources[i]).ToLocalChecked();
        js_execution_async_resources->Set(context, i, obj).Check();
    }
    info_ = nullptr;
}

std::ostream& operator<<(std::ostream& output, const AsyncHooks::SerializeInfo& i)
{
    output << "{\n"
...
AsyncHooks::SerializeInfo AsyncHooks::Serialize(Local<Context> context, SnapshotCreator* creator)
{
    SerializeInfo info;
    ...
    if (!js_execution_async_resources_.IsEmpty()) {
        info.js_execution_async_resources = creator->AddData(context, js_execution_async_resources_.Get(context->GetIsolate()));
        CHECK_NE(info.js_execution_async_resources, 0);
    } else {
        info.js_execution_async_resources = 0;
    }

    info.native_execution_async_resources.resize(native_execution_async_resources_.size());
    for (size_t i = 0; i < native_execution_async_resources_.size(); i++) {
        info.native_execution_async_resources[i]
            = native_execution_async_resources_[i].IsEmpty() ? SIZE_MAX : creator->AddData(context, native_execution_async_resources_[i]);
    }
...
void AsyncHooks::grow_async_ids_stack()
{
    ...
    env()->principal_realm()->async_hooks_binding()->Set(env()->context(), env()->async_ids_stack_string(), async_ids_stack_.GetJSArray()).Check();
}
...
{
    if (!tracks_unmanaged_fds())
        return;
    auto result = unmanaged_fds_.insert(fd);
    if (!result.second) {
        ProcessEmitWarning(this, "File descriptor %d opened in unmanaged mode twice", fd);
    }
}
...
void Environment::RemoveUnmanagedFd(int fd)
{
    ...
    if (removed_count == 0) {
        ProcessEmitWarning(this, "File descriptor %d closed but not opened in unmanaged mode", fd);
    }
}
...
EnvSerializeInfo Environment::Serialize(SnapshotCreator* creator)
{
    ...
    info.should_abort_on_uncaught_toggle = should_abort_on_uncaught_toggle_.Serialize(ctx, creator);
...
void Environment::EnqueueDeserializeRequest(DeserializeRequestCallback cb, Local<Object> holder, int index, InternalFieldInfoBase* info)
{
...
void Environment::BuildEmbedderGraph(Isolate* isolate, EmbedderGraph* graph, void* data)
{
...
{
    Local<Value> id_val;
    if (!promise->GetPrivate(env->context(), env->promise_trace_id()).ToLocal(&id_val) || !id_val->IsUint32()) {
        return std::nullopt;
    }
    return id_val.As<Uint32>()->Value();
}

void Environment::TracePromises(PromiseHookType type, Local<Promise> promise, Local<Value> parent)
{
    ...
    if (type == PromiseHookType::kInit) {
        ...
        promise->SetPrivate(context, env->promise_trace_id(), Uint32::New(isolate, id));
        action = "created";
...
size_t Environment::NearHeapLimitCallback(void* data, size_t current_heap_limit, size_t initial_heap_limit)
{
    auto* env = static_cast<Environment*>(data);
    ...
    Debug(env, DebugCategory::DIAGNOSTICS,
        ...
        env->is_in_heapsnapshot_heap_limit_callback_, static_cast<uint64_t>(current_heap_limit), static_cast<uint64_t>(initial_heap_limit));
    ...
    for (size_t i = 0; i < num_heap_spaces; ++i) {
        ...
        if (strcmp(stats.space_name(), "new_space") == 0 || strcmp(stats.space_name(), "new_large_object_space") == 0) {
    ...
    Debug(env, DebugCategory::DIAGNOSTICS,
        ...
        "old_gen_size=%" PRIu64 ", "
        "total_size=%" PRIu64 "\n",
        static_cast<uint64_t>(max_young_gen_size), static_cast<uint64_t>(young_gen_size), static_cast<uint64_t>(old_gen_size),
    ...
    if (estimated_overhead > available) {
        Debug(env, DebugCategory::DIAGNOSTICS, "Not generating snapshots because it's too risky.\n");
...
void Environment::MemoryInfo(MemoryTracker* tracker) const
{
    ...
    tracker->TrackField("should_abort_on_uncaught_toggle", should_abort_on_uncaught_toggle_);
