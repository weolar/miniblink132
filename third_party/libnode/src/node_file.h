...
constexpr size_t kFsStatsBufferLength = static_cast<size_t>(FsStatsOffset::kFsStatsFieldsNumber) * 2;

enum class FsStatFsOffset { kType = 0, kBSize, kBlocks, kBFree, kBAvail, kFiles, kFFree, kFsStatFsFieldsNumber };

constexpr size_t kFsStatFsBufferLength = static_cast<size_t>(FsStatFsOffset::kFsStatFsFieldsNumber);
...
public:
    ...
    enum class FilePathIsFileReturnType { kIsFile = 0, kIsNotFile, kThrowInsufficientPermissions };

    explicit BindingData(Realm* realm, v8::Local<v8::Object> wrap, InternalFieldInfo* info = nullptr);
    ...
    static void LegacyMainResolve(const v8::FunctionCallbackInfo<v8::Value>& args);
...
private:
    ...
    static FilePathIsFileReturnType FilePathIsFile(Environment* env, const std::string& file_path);
};
...
public:
    ...
    inline FSReqBase(BindingData* binding_data, v8::Local<v8::Object> req, AsyncWrap::ProviderType type, bool use_bigint);
    ~FSReqBase() override;

    inline void Init(const char* syscall, const char* data, size_t len, enum encoding encoding);
    inline FSReqBuffer& Init(const char* syscall, size_t len, enum encoding encoding);
    ...
    virtual void SetReturnValue(const v8::FunctionCallbackInfo<v8::Value>& args) = 0;
    ...
    {
        return has_data_ ? *buffer_ : nullptr;
    }
    enum encoding encoding() const
    {
        return encoding_;
    }
    bool use_bigint() const
    {
        return use_bigint_;
    }
    bool is_plain_open() const
    {
        return is_plain_open_;
    }
    bool with_file_types() const
    {
        return with_file_types_;
    }

    void set_is_plain_open(bool value)
    {
        is_plain_open_ = value;
    }
    void set_with_file_types(bool value)
    {
        with_file_types_ = value;
    }
...
public:
    inline FSReqCallback(BindingData* binding_data, v8::Local<v8::Object> req, bool use_bigint);
...
inline v8::Local<v8::Value> FillGlobalStatsArray(BindingData* binding_data, const bool use_bigint, const uv_stat_t* s, const bool second = false);

template <typename NativeT, typename V8T> void FillStatFsArray(AliasedBufferBase<NativeT, V8T>* fields, const uv_statfs_t* s);

inline v8::Local<v8::Value> FillGlobalStatFsArray(BindingData* binding_data, const bool use_bigint, const uv_statfs_t* s);

template <typename AliasedBufferT> class FSReqPromise final : public FSReqBase {
public:
    ...
    inline void SetReturnValue(const v8::FunctionCallbackInfo<v8::Value>& args) override;
...
private:
    inline FSReqPromise(BindingData* binding_data, v8::Local<v8::Object> obj, bool use_bigint);
...
public:
    enum InternalFields { kFileHandleBaseField = StreamBase::kInternalFieldCount, kClosingPromiseSlot, kInternalFieldCount };

    static FileHandle* New(BindingData* binding_data, int fd, v8::Local<v8::Object> obj = v8::Local<v8::Object>(),
        std::optional<int64_t> maybeOffset = std::nullopt, std::optional<int64_t> maybeLength = std::nullopt);
    ~FileHandle() override;
...
private:
    ...
    public:
        CloseReq(Environment* env, v8::Local<v8::Object> obj, v8::Local<v8::Promise> promise, v8::Local<v8::Value> ref);
        ~CloseReq() override;
...
class FSReqWrapSync {
public:
    FSReqWrapSync(const char* syscall = nullptr, const char* path = nullptr, const char* dest = nullptr)
        : syscall_p(syscall)
        , path_p(path)
        , dest_p(dest)
    {
...
inline FSReqBase* GetReqWrap(const v8::FunctionCallbackInfo<v8::Value>& args, int index, bool use_bigint = false);

// Returns nullptr if the operation fails from the start.
template <typename Func, typename... Args>
inline FSReqBase* AsyncDestCall(Environment* env, FSReqBase* req_wrap, const v8::FunctionCallbackInfo<v8::Value>& args, const char* syscall, const char* dest,
    size_t len, enum encoding enc, uv_fs_cb after, Func fn, Args... fn_args);

// Returns nullptr if the operation fails from the start.
template <typename Func, typename... Args>
inline FSReqBase* AsyncCall(Environment* env, FSReqBase* req_wrap, const v8::FunctionCallbackInfo<v8::Value>& args, const char* syscall, enum encoding enc,
...
template <typename Func, typename... Args>
inline int SyncCall(Environment* env, v8::Local<v8::Value> ctx, FSReqWrapSync* req_wrap, const char* syscall, Func fn, Args... args);
