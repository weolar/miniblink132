...
public:
    Http2State(Realm* realm, v8::Local<v8::Object> obj)
        ...
        , session_state_buffer(realm->isolate(), offsetof(http2_state_internal, session_state_buffer), IDX_SESSION_STATE_COUNT, root_buffer)
        , stream_state_buffer(realm->isolate(), offsetof(http2_state_internal, stream_state_buffer), IDX_STREAM_STATE_COUNT, root_buffer)
        , stream_stats_buffer(realm->isolate(), offsetof(http2_state_internal, stream_stats_buffer), IDX_STREAM_STATS_COUNT, root_buffer)
        , session_stats_buffer(realm->isolate(), offsetof(http2_state_internal, session_stats_buffer), IDX_SESSION_STATS_COUNT, root_buffer)
        , options_buffer(realm->isolate(), offsetof(http2_state_internal, options_buffer), IDX_OPTIONS_FLAGS + 1, root_buffer)
        , settings_buffer(
              realm->isolate(), offsetof(http2_state_internal, settings_buffer), IDX_SETTINGS_COUNT + 1 + 1 + 2 * MAX_ADDITIONAL_SETTINGS, root_buffer)
    {
    }
...
private:
    struct http2_state_internal {
        ...
        // additional setting
        uint32_t settings_buffer[IDX_SETTINGS_COUNT + 1 + 1 + 2 * MAX_ADDITIONAL_SETTINGS];
    };
};
