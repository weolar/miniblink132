...
template <typename... Args> void BlobSerializerDeserializer::Debug(const char* format, Args&&... args) const
{
    if (is_debug) {
...
template <typename T> std::string BlobSerializerDeserializer::ToStr(const T& arg) const
{
    std::stringstream ss;
    ss << arg;
    return ss.str();
...
template <typename T> std::string BlobSerializerDeserializer::GetName() const
{
#define TYPE_LIST(V)                                                                                                                                           \
...
#undef V
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        return (std::is_unsigned_v<T> ? "uint" : std::is_integral_v<T> ? "int" : "float") + std::to_string(sizeof(T) * 8) + "_t";
    }
    return "";
}

// Helper for reading numeric types.
template <typename Impl> template <typename T> T BlobDeserializer<Impl>::ReadArithmetic()
{
...
template <typename Impl> template <typename T> std::vector<T> BlobDeserializer<Impl>::ReadVector()
{
    if (is_debug) {
...
template <typename Impl> std::string BlobDeserializer<Impl>::ReadString()
{
    std::string_view view = ReadStringView(StringLogMode::kAddressAndContent);
    return std::string(view);
}

template <typename Impl> std::string_view BlobDeserializer<Impl>::ReadStringView(StringLogMode mode)
{
...
template <typename Impl> template <typename T> void BlobDeserializer<Impl>::ReadArithmetic(T* out, size_t count)
{
    static_assert(std::is_arithmetic_v<T>, "Not an arithmetic type");
    ...
    if (is_debug) {
        std::string str = "{ " + std::to_string(out[0]) + (count > 1 ? ", ... }" : " }");
...
template <typename Impl> template <typename Number> std::vector<Number> BlobDeserializer<Impl>::ReadArithmeticVector(size_t count)
{
...
template <typename Impl> template <typename T> std::vector<T> BlobDeserializer<Impl>::ReadNonArithmeticVector(size_t count)
{
...
template <typename Impl> template <typename T> T BlobDeserializer<Impl>::ReadElement()
{
    if constexpr (std::is_arithmetic_v<T>) {
...
template <typename Impl> template <typename T> size_t BlobSerializer<Impl>::WriteArithmetic(const T& data)
{
    static_assert(std::is_arithmetic_v<T>, "Not an arithmetic type");
...
// [ 4/8 bytes ] count
// [   ...     ] contents (count * size of individual elements)
template <typename Impl> template <typename T> size_t BlobSerializer<Impl>::WriteVector(const std::vector<T>& data)
{
    if (is_debug) {
...
template <typename Impl> size_t BlobSerializer<Impl>::WriteString(const std::string& data)
{
...
template <typename Impl> template <typename Number> size_t BlobSerializer<Impl>::WriteArithmeticVector(const std::vector<Number>& data)
{
...
template <typename Impl> template <typename T> size_t BlobSerializer<Impl>::WriteNonArithmeticVector(const std::vector<T>& data)
{
    static_assert(!std::is_arithmetic_v<T>, "Arithmetic type");
    DCHECK_GT(data.size(),
...
template <typename Impl> template <typename T> size_t BlobSerializer<Impl>::WriteElement(const T& data)
{
    if constexpr (std::is_arithmetic_v<T>) {
