...
template <typename R, typename... Args> class CallbackQueue {
public:
    class Callback {
    public:
    ...
    template <typename Fn> inline std::unique_ptr<Callback> CreateCallback(Fn&& fn, CallbackFlags::Flags);
...
private:
    template <typename Fn> class CallbackImpl final : public Callback {
    public:
