...
Session::Application_Options::operator const nghttp3_settings() const
{
    ...
    return nghttp3_settings {
        ...
        .qpack_max_dtable_capacity = static_cast<size_t>(qpack_max_dtable_capacity),
        .qpack_encoder_max_dtable_capacity = static_cast<size_t>(qpack_encoder_max_dtable_capacity),
...
std::string Session::Application_Options::ToString() const
{
    ...
    res += prefix + "max field section size: " + std::to_string(max_field_section_size);
    res += prefix + "qpack max dtable capacity: " + std::to_string(qpack_max_dtable_capacity);
    res += prefix + "qpack encoder max dtable capacity: " + std::to_string(qpack_encoder_max_dtable_capacity);
    res += prefix + "qpack blocked streams: " + std::to_string(qpack_blocked_streams);
    res += prefix + "enable connect protocol: " + (enable_connect_protocol ? std::string("yes") : std::string("no"));
    res += prefix + "enable datagrams: " + (enable_datagrams ? std::string("yes") : std::string("no"));
    res += indent.Close();
    return res;
}

Maybe<Session::Application_Options> Session::Application_Options::From(Environment* env, Local<Value> value)
{
    if (value.IsEmpty())
        [[unlikely]]
        {
...
#define SET(name) SetOption<Session::Application_Options, &Session::Application_Options::name>(env, &options, params, state.name##_string())
    ...
    if (!value->IsUndefined()) {
        ...
        if (!SET(max_header_pairs) || !SET(max_header_length) || !SET(max_field_section_size) || !SET(qpack_max_dtable_capacity)
            || !SET(qpack_encoder_max_dtable_capacity) || !SET(qpack_blocked_streams) || !SET(enable_connect_protocol) || !SET(enable_datagrams)) {
...
bool Session::Application::AcknowledgeStreamData(int64_t stream_id, size_t datalen)
{
...
bool Session::Application::CanAddHeader(size_t current_count, size_t current_headers_length, size_t this_header_length)
{
    // By default headers are not supported.
    return false;
}

bool Session::Application::SendHeaders(const Stream& stream, HeadersKind kind, const Local<v8::Array>& headers, HeadersFlags flags)
{
    // By default do nothing.
    return false;
}
...
void Session::Application::ExtendMaxStreams(EndpointLabel label, Direction direction, uint64_t max_streams)
{
    // By default do nothing.
}

void Session::Application::ExtendMaxStreamData(Stream* stream, uint64_t max_data)
{
    Debug(session_, "Application extending max stream data");
    // By default do nothing.
}

void Session::Application::CollectSessionTicketAppData(SessionTicket::AppData* app_data) const
{
    // By default do nothing.
}

SessionTicket::AppData::Status Session::Application::ExtractSessionTicketAppData(const SessionTicket::AppData& app_data, Flag flag)
{
    // By default we do not have any application data to retrieve.
    return flag == Flag::STATUS_RENEW ? SessionTicket::AppData::Status::TICKET_USE_RENEW : SessionTicket::AppData::Status::TICKET_USE;
}

void Session::Application::SetStreamPriority(const Stream& stream, StreamPriority priority, StreamPriorityFlags flags)
{
    // By default do nothing.
}
...
{
    return Packet::Create(env(), session_->endpoint(), session_->remote_address(), session_->max_packet_size(), "stream data");
}
...
void Session::Application::StreamReset(Stream* stream, uint64_t final_size, QuicError&& error)
{
...
void Session::Application::SendPendingData()
{
    ...
    const size_t max_packet_count = std::min(kMaxPackets, ngtcp2_conn_get_send_quantum(*session_) / max_packet_size);
    ...
    for (;;) {
        ...
        if (ndatalen > 0) {
            Debug(session_, "Application accepted %zu bytes from stream %" PRIi64 " into packet", ndatalen, stream_data.id);
        } else if (stream_data.id >= 0) {
            Debug(session_, "Application did not accept any bytes from stream %" PRIi64 " into packet", stream_data.id);
        }
        ...
        if (nwrite < 0) {
            ...
            case NGTCP2_ERR_STREAM_SHUT_WR: {
                ...
                // any stream data!
                Debug(session_, "Closing stream %" PRIi64 " for writing", stream_data.id);
                // ndatalen = -1 means that no stream data was accepted into the
                // packet, which is what we want here.
                DCHECK_EQ(ndatalen, -1);
                // We should only have received this error if there was an actual
            ...
            case NGTCP2_ERR_WRITE_MORE: {
                if (ndatalen >= 0 && !StreamCommit(&stream_data, ndatalen)) {
                    Debug(session_, "Failed to commit stream data while writing packets");
                    packet->Done(UV_ECANCELED);
                    session_->SetLastError(QuicError::ForNgtcp2Error(NGTCP2_ERR_INTERNAL));
                    return session_->Close(CloseMethod::SILENT);
                }
            ...
            DCHECK_EQ(ndatalen, -1);
            Debug(session_, "Application encountered error while writing packet: %s", ngtcp2_strerror(nwrite));
            packet->Done(UV_ECANCELED);
...
ssize_t Session::Application::WriteVStream(PathStorage* path, uint8_t* dest, ssize_t* ndatalen, size_t max_packet_size, const StreamData& stream_data)
{
    ...
    return ngtcp2_conn_writev_stream(
        *session_, &path->path, nullptr, dest, max_packet_size, ndatalen, flags, stream_data.id, stream_data, stream_data.count, uv_hrtime());
}
...
public:
    ...
    bool ReceiveStreamData(int64_t stream_id, const uint8_t* data, size_t datalen, const Stream::ReceiveDataFlags& flags, void* stream_user_data) override
    {
    ...
    int GetStreamData(StreamData* stream_data) override
    {
        ...
        if (!stream->is_eos())
            [[likely]]
            {
                int ret = stream->Pull(std::move(next), bob::Options::OPTIONS_SYNC, stream_data->data, arraysize(stream_data->data), kMaxVectorCount);
...
std::unique_ptr<Session::Application> Session::SelectApplication(Session* session, const Config& config)
{
    if (config.options.application_provider) {
        return config.options.application_provider->Create(session);
    }

    return std::make_unique<DefaultApplication>(session, Application_Options::kDefault);
}
