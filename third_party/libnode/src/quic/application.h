...
public:
    ...
    virtual bool ReceiveStreamData(int64_t stream_id, const uint8_t* data, size_t datalen, const Stream::ReceiveDataFlags& flags, void* stream_user_data) = 0;
    ...
    virtual bool CanAddHeader(size_t current_count, size_t current_headers_length, size_t this_header_length);
    ...
    // nothing.
    virtual void ExtendMaxStreams(EndpointLabel label, Direction direction, uint64_t max_streams);
    ...
    virtual SessionTicket::AppData::Status ExtractSessionTicketAppData(const SessionTicket::AppData& app_data, SessionTicket::AppData::Source::Flag flag);
    ...
    virtual void StreamReset(Stream* stream, uint64_t final_size, QuicError&& error = QuicError());

    // Notifies the Application that the identified stream should stop sending.
    virtual void StreamStopSending(Stream* stream, QuicError&& error = QuicError());
    ...
    // should return false.
    virtual bool SendHeaders(const Stream& stream, HeadersKind kind, const v8::Local<v8::Array>& headers, HeadersFlags flags = HeadersFlags::NONE);
    ...
    virtual void SetStreamPriority(
        const Stream& stream, StreamPriority priority = StreamPriority::DEFAULT, StreamPriorityFlags flags = StreamPriorityFlags::NONE);
...
private:
    ...
    ssize_t WriteVStream(PathStorage* path, uint8_t* buf, ssize_t* ndatalen, size_t max_packet_size, const StreamData& stream_data);
...
struct Session::Application::StreamData final {
    ...
    inline operator const ngtcp2_vec*() const
    {
        return data;
    }
    inline operator ngtcp2_vec*()
    {
        return data;
    }
