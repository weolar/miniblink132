...
{
    ngtcp2_crypto_conn_ref* ref = static_cast<ngtcp2_crypto_conn_ref*>(SSL_get_app_data(ssl));
...
Maybe<SessionTicket> SessionTicket::FromV8Value(Environment* env, Local<Value> value)
{
    ...
    if (!des.ReadValue(env->context()).ToLocal(&ticket) || !des.ReadValue(env->context()).ToLocal(&transport_params) || !ticket->IsArrayBufferView()
    ...
    return Just(SessionTicket(Store(ticket.As<ArrayBufferView>()), Store(transport_params.As<ArrayBufferView>())));
}
...
MaybeLocal<Object> SessionTicket::encode(Environment* env) const
{
    ...
    if (ser.WriteValue(context, ticket_.ToUint8Array(env)).IsNothing() || ser.WriteValue(context, transport_params_.ToUint8Array(env)).IsNothing()) {
...
SSL_TICKET_RETURN SessionTicket::DecryptedCallback(
    SSL* ssl, SSL_SESSION* session, const unsigned char* keyname, size_t keyname_len, SSL_TICKET_STATUS status, void* arg)
{
    switch (status) {
    default:
...
{
    if (set_ || data.base == nullptr || data.len == 0)
        return false;
    set_ = true;
...
{
    uv_buf_t buf;
    int ret = SSL_SESSION_get0_ticket_appdata(SSL_get0_session(ssl_), reinterpret_cast<void**>(&buf.base), reinterpret_cast<size_t*>(&buf.len));
    if (ret != 1)
        return std::nullopt;
    return buf;
}
