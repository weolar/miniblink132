...
TransportParams::Config::Config(Side side, const CID& ocid, const CID& retry_scid)
    : side(side)
    , ocid(ocid)
...
Maybe<TransportParams::Options> TransportParams::Options::From(Environment* env, Local<Value> value)
{
    if (value.IsEmpty()) {
...
#define SET(name) SetOption<TransportParams::Options, &TransportParams::Options::name>(env, &options, params, state.name##_string())

    if (!SET(initial_max_stream_data_bidi_local) || !SET(initial_max_stream_data_bidi_remote) || !SET(initial_max_stream_data_uni) || !SET(initial_max_data)
        || !SET(initial_max_streams_bidi) || !SET(initial_max_streams_uni) || !SET(max_idle_timeout) || !SET(active_connection_id_limit)
        || !SET(ack_delay_exponent) || !SET(max_ack_delay) || !SET(max_datagram_frame_size)) {
        return Nothing<Options>();
    }
...
std::string TransportParams::Options::ToString() const
{
    ...
    if (preferred_address_ipv4.has_value()) {
        res += prefix + "preferred_address_ipv4: " + preferred_address_ipv4.value().ToString();
    } else {
        res += prefix + "preferred_address_ipv4: <none>";
    }
    if (preferred_address_ipv6.has_value()) {
        res += prefix + "preferred_address_ipv6: " + preferred_address_ipv6.value().ToString();
    } else {
        res += prefix + "preferred_address_ipv6: <none>";
    }
    res += prefix + "initial max stream data bidi local: " + std::to_string(initial_max_stream_data_bidi_local);
    res += prefix + "initial max stream data bidi remote: " + std::to_string(initial_max_stream_data_bidi_remote);
    res += prefix + "initial max stream data uni: " + std::to_string(initial_max_stream_data_uni);
    res += prefix + "tinitial max data: " + std::to_string(initial_max_data);
    res += prefix + "initial max streams bidi: " + std::to_string(initial_max_streams_bidi);
    res += prefix + "initial max streams uni: " + std::to_string(initial_max_streams_uni);
    res += prefix + "max idle timeout: " + std::to_string(max_idle_timeout);
    res += prefix + "active connection id limit: " + std::to_string(active_connection_id_limit);
    res += prefix + "ack delay exponent: " + std::to_string(ack_delay_exponent);
    res += prefix + "max ack delay: " + std::to_string(max_ack_delay);
    res += prefix + "max datagram frame size: " + std::to_string(max_datagram_frame_size);
    res += prefix + "disable active migration: " + (disable_active_migration ? std::string("yes") : std::string("no"));
    res += indent.Close();
    return res;
}
...
{
    if (preferred_address_ipv4.has_value()) {
        tracker->TrackField("preferred_address_ipv4", preferred_address_ipv4.value());
    }
    if (preferred_address_ipv6.has_value()) {
        tracker->TrackField("preferred_address_ipv6", preferred_address_ipv6.value());
    }
}
    ...
    : TransportParams()
{
    ...
    params_.initial_max_stream_data_bidi_local = options.initial_max_stream_data_bidi_local;
    params_.initial_max_stream_data_bidi_remote = options.initial_max_stream_data_bidi_remote;
...
TransportParams::TransportParams(const ngtcp2_vec& vec, int version)
    : TransportParams()
{
    int ret = ngtcp2_transport_params_decode_versioned(version, &params_, vec.base, vec.len);
...
Store TransportParams::Encode(Environment* env, int version) const
{
    ...
    ssize_t size = ngtcp2_transport_params_encode_versioned(nullptr, 0, version, &params_);
    if (size == 0) {
        return Store();
    }

    auto result = ArrayBuffer::NewBackingStore(env->isolate(), size, v8::BackingStoreInitializationMode::kUninitialized);

    auto ret = ngtcp2_transport_params_encode_versioned(static_cast<uint8_t*>(result->Data()), size, version, &params_);
...
void TransportParams::SetPreferredAddress(const SocketAddress& address)
{
    ...
    case AF_INET: {
        const sockaddr_in* src = reinterpret_cast<const sockaddr_in*>(address.data());
        memcpy(&params_.preferred_addr.ipv4.sin_addr, &src->sin_addr, sizeof(params_.preferred_addr.ipv4.sin_addr));
        params_.preferred_addr.ipv4.sin_port = address.port();
        return;
    }
    case AF_INET6: {
        const sockaddr_in6* src = reinterpret_cast<const sockaddr_in6*>(address.data());
        memcpy(&params_.preferred_addr.ipv6.sin6_addr, &src->sin6_addr, sizeof(params_.preferred_addr.ipv6.sin6_addr));
...
void TransportParams::GenerateStatelessResetToken(const Endpoint& endpoint, const CID& cid)
{
    DCHECK(ptr_ == &params_);
    DCHECK(cid);
...
void TransportParams::GeneratePreferredAddressToken(Session* session)
{
    ...
    if (params_.preferred_addr_present) {
        ...
        endpoint.AssociateStatelessResetToken(
            endpoint.GenerateNewStatelessResetToken(params_.preferred_addr.stateless_reset_token, config.preferred_address_cid), session);
    }
}
