...
PendingStream::PendingStream(Direction direction, Stream* stream, BaseObjectWeakPtr<Session> session)
...
Maybe<std::shared_ptr<DataQueue>> Stream::GetDataQueueFromSource(Environment* env, Local<Value> value)
{
    ...
    if (value->IsUndefined()) {
        ...
        entries.push_back(DataQueue::CreateInMemoryEntryFromBackingStore(buffer->GetBackingStore(), 0, buffer->ByteLength()));
        ...
        auto entry = DataQueue::CreateInMemoryEntryFromView(value.As<ArrayBufferView>());
        if (!entry) {
...
struct Stream::Impl {
    ...
    static void Destroy(const FunctionCallbackInfo<Value>& args)
    {
        ...
        if (args.Length() > 1) {
            ...
            bool unused = false;
            stream->Destroy(QuicError::ForApplication(args[0].As<BigInt>()->Uint64Value(&unused)));
        } else {
            stream->Destroy();
        }
    }
    ...
    static void SendHeaders(const FunctionCallbackInfo<Value>& args)
    {
        ...
        HeadersFlags flags = static_cast<HeadersFlags>(args[2].As<Uint32>()->Value());
        ...
        args.GetReturnValue().Set(stream->session().application().SendHeaders(*stream, kind, headers, flags));
    }
    ...
    static void SetPriority(const FunctionCallbackInfo<Value>& args)
    {
        ...
        StreamPriority priority = static_cast<StreamPriority>(args[0].As<Uint32>()->Value());
        StreamPriorityFlags flags = static_cast<StreamPriorityFlags>(args[1].As<Uint32>()->Value());
    ...
    static void GetPriority(const FunctionCallbackInfo<Value>& args)
    {
        ...
        if (stream->is_pending()) {
            return args.GetReturnValue().Set(static_cast<uint32_t>(StreamPriority::DEFAULT));
        }
    ...
    static void GetReader(const FunctionCallbackInfo<Value>& args)
    {
        ...
        if (reader)
            return args.GetReturnValue().Set(reader->object());
        THROW_ERR_INVALID_STATE(Environment::GetCurrent(args), "Unable to get a reader for the stream");
    }
};
...
public:
    ...
    void Commit(size_t amount)
    {
        ...
        while (remaining > 0 && commit_head_ != nullptr) {
            ...
            size_t amount_to_commit = std::min(remaining, static_cast<size_t>(commit_head_->buf.len - commit_head_->offset));
    ...
    int Pull(bob::Next<ngtcp2_vec> next, int options, ngtcp2_vec* data, size_t count, size_t max_count_hint)
    {
        if (next_pending_) {
...
private:
    struct OnComplete {
        bob::Done done;
        explicit OnComplete(bob::Done done)
            : done(std::move(done))
        {
        }
        ~OnComplete()
    ...
    void Append(const DataQueue::Vec* vectors, size_t count, bob::Done done)
    {
        ...
        for (size_t n = 0; n < count; n++) {
            if (vectors[n].len == 0 || vectors[n].base == nullptr)
                continue;
...
#define V(name, key, __) auto IDX_STATE_STREAM_##name = offsetof(Stream::State, key);
    STREAM_STATE(V)
#undef V
...
#undef V
    ...
    constexpr int QUIC_STREAM_HEADERS_KIND_HINTS = static_cast<int>(HeadersKind::HINTS);
    constexpr int QUIC_STREAM_HEADERS_KIND_INITIAL = static_cast<int>(HeadersKind::INITIAL);
    constexpr int QUIC_STREAM_HEADERS_KIND_TRAILING = static_cast<int>(HeadersKind::TRAILING);

    constexpr int QUIC_STREAM_HEADERS_FLAGS_NONE = static_cast<int>(HeadersFlags::NONE);
    constexpr int QUIC_STREAM_HEADERS_FLAGS_TERMINAL = static_cast<int>(HeadersFlags::TERMINAL);
...
BaseObjectPtr<Stream> Stream::Create(Session* session, int64_t id, std::shared_ptr<DataQueue> source)
{
    DCHECK_GE(id, 0);
    DCHECK_NOT_NULL(session);
    Local<Object> obj;
    if (!GetConstructorTemplate(session->env())->InstanceTemplate()->NewInstance(session->env()->context()).ToLocal(&obj)) {
        return {};
    }

    return MakeDetachedBaseObject<Stream>(BaseObjectWeakPtr<Session>(session), obj, id, std::move(source));
}

BaseObjectPtr<Stream> Stream::Create(Session* session, Direction direction, std::shared_ptr<DataQueue> source)
{
    DCHECK_NOT_NULL(session);
    Local<Object> obj;
    if (!GetConstructorTemplate(session->env())->InstanceTemplate()->NewInstance(session->env()->context()).ToLocal(&obj)) {
    ...
    return MakeBaseObject<Stream>(BaseObjectWeakPtr<Session>(session), obj, direction, std::move(source));
}

Stream::Stream(BaseObjectWeakPtr<Session> session, Local<Object> object, int64_t id, std::shared_ptr<DataQueue> source)
...
Stream::Stream(BaseObjectWeakPtr<Session> session, Local<Object> object, Direction direction, std::shared_ptr<DataQueue> source)
    : AsyncWrap(session->env(), object, PROVIDER_QUIC_STREAM)
    , stats_(env()->isolate())
    , state_(env()->isolate())
    , session_(std::move(session))
    , inbound_(DataQueue::Create())
    , maybe_pending_stream_(std::make_unique<PendingStream>(direction, this, session_))
{
    MakeWeak();
    state_->id = -1;
    state_->pending = 1;
...
void Stream::NotifyStreamOpened(int64_t id)
{
    ...
    session().AddStream(BaseObjectPtr<Stream>(this), Session::CreateStreamOption::DO_NOT_NOTIFY);
    ...
    if (pending_priority_) {
        ...
        session().application().SetStreamPriority(*this, priority.priority, priority.flags);
    ...
    for (auto& headers : queue) {
        ...
        session().application().SendHeaders(*this, headers->kind, headers->headers.Get(env()->isolate()), headers->flags);
    }
...
void Stream::EnqueuePendingHeaders(HeadersKind kind, Local<Array> headers, HeadersFlags flags)
{
    Debug(this, "Enqueuing headers for pending stream");
    pending_headers_queue_.push_back(std::make_unique<PendingHeaders>(kind, Global<Array>(env()->isolate(), headers), flags));
}
...
Direction Stream::direction() const
{
    ...
    return (state_->id & 0b10) ? Direction::UNIDIRECTIONAL : Direction::BIDIRECTIONAL;
}
...
{
    return direction() == Direction::UNIDIRECTIONAL && ngtcp2_conn_is_local_stream(*session_, id());
}
...
bool Stream::is_writable() const
{
    ...
    // never be pending.
    if (!is_pending() && direction() == Direction::UNIDIRECTIONAL && !ngtcp2_conn_is_local_stream(session(), id())) {
        return false;
    }
...
{
    DCHECK_IMPLIES(state_->fin_received == 1, final_size <= STAT_GET(Stats, final_size));
...
int Stream::DoPull(bob::Next<ngtcp2_vec> next, int options, ngtcp2_vec* data, size_t count, size_t max_count_hint)
{
    if (is_eos()) {
        std::move(next)(bob::Status::STATUS_EOS, nullptr, 0, [](int) {});
        return bob::Status::STATUS_EOS;
...
bool Stream::AddHeader(const Header& header)
{
    ...
    if (!session_->application().CanAddHeader(headers_.size(), headers_length_, len)) {
        return false;
    }
...
void Stream::ReceiveData(const uint8_t* data, size_t len, ReceiveDataFlags flags)
{
    ...
    inbound_->append(DataQueue::CreateInMemoryEntryFromBackingStore(std::move(backing), 0, len));
...
void Stream::ReceiveStreamReset(uint64_t final_size, QuicError error)
{
    ...
    // be read.
    Debug(this, "Received stream reset with final size %" PRIu64 " and error %s", final_size, error);
...
void Stream::EmitHeaders()
{
    ...
    Local<Value> argv[]
        = { Array::New(env()->isolate(), headers_.data(), headers_.size()), Integer::NewFromUnsigned(env()->isolate(), static_cast<uint32_t>(headers_kind_)) };

    headers_.clear();

    MakeCallback(BindingData::Get(env()).stream_headers_callback(), arraysize(argv), argv);
}
