...
#define IF_QUIC_DEBUG(env)                                                                                                                                     \
    if (env->enabled_debug_list()->enabled(DebugCategory::QUIC))                                                                                               \
    [[unlikely]]
...
bool SetOption(Environment* env, Opt* options, const v8::Local<v8::Object>& object, const v8::Local<v8::String>& name)
{
...
bool SetOption(Environment* env, Opt* options, const v8::Local<v8::Object>& object, const v8::Local<v8::String>& name)
{
    ...
    if (!value->IsUndefined()) {
        if (!value->IsUint32()) {
            ...
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be an uint32", *nameStr);
            return false;
        }
...
bool SetOption(Environment* env, Opt* options, const v8::Local<v8::Object>& object, const v8::Local<v8::String>& name)
{
    ...
    if (!value->IsUndefined()) {
        if (!value->IsBigInt() && !value->IsNumber()) {
            Utf8Value nameStr(env->isolate(), name);
            THROW_ERR_INVALID_ARG_VALUE(env, "option %s must be a bigint or number", *nameStr);
            return false;
        }
...
template <typename Stats, uint64_t Stats::*member> void IncrementStat(Stats* stats, uint64_t amt = 1)
{
    stats->*member += amt;
}

template <typename Stats, uint64_t Stats::*member> void RecordTimestampStat(Stats* stats)
{
    stats->*member = uv_hrtime();
}

template <typename Stats, uint64_t Stats::*member> void SetStat(Stats* stats, uint64_t val)
{
    stats->*member = val;
}

template <typename Stats, uint64_t Stats::*member> uint64_t GetStat(Stats* stats)
{
    return stats->*member;
}

#define STAT_INCREMENT(Type, name) IncrementStat<Type, &Type::name>(stats_.Data());
#define STAT_INCREMENT_N(Type, name, amt) IncrementStat<Type, &Type::name>(stats_.Data(), amt);
#define STAT_RECORD_TIMESTAMP(Type, name) RecordTimestampStat<Type, &Type::name>(stats_.Data());
...
#define JS_METHOD(name) static void name(const v8::FunctionCallbackInfo<v8::Value>& args)
