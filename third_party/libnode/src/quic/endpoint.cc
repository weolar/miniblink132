...
template <typename Opt, double Opt::*member> bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    ...
    if (!value->IsUndefined()) {
        ...
        if (!value->ToNumber(env->context()).ToLocal(&num)) {
            ...
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be a number", *nameStr);
            return false;
        }
...
template <typename Opt, uint8_t Opt::*member> bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    if (!object->Get(env->context(), name).ToLocal(&value))
        return false;
    if (!value->IsUndefined()) {
        if (!value->IsUint32()) {
            Utf8Value nameStr(env->isolate(), name);
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be an uint8", *nameStr);
            return false;
        }
        Local<Uint32> num;
        if (!value->ToUint32(env->context()).ToLocal(&num) || num->Value() > std::numeric_limits<uint8_t>::max()) {
            Utf8Value nameStr(env->isolate(), name);
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be an uint8", *nameStr);
            return false;
        }
...
template <typename Opt, TokenSecret Opt::*member> bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    if (!object->Get(env->context(), name).ToLocal(&value))
        return false;
    if (!value->IsUndefined()) {
        if (!value->IsArrayBufferView()) {
            Utf8Value nameStr(env->isolate(), name);
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be an ArrayBufferView", *nameStr);
            return false;
        }
        Store store(value.As<ArrayBufferView>());
        if (store.length() != TokenSecret::QUIC_TOKENSECRET_LEN) {
            Utf8Value nameStr(env->isolate(), name);
            THROW_ERR_INVALID_ARG_VALUE(env, "The %s option must be an ArrayBufferView of length %d", *nameStr, TokenSecret::QUIC_TOKENSECRET_LEN);
            return false;
        }
...
Maybe<Endpoint::Options> Endpoint::Options::From(Environment* env, Local<Value> value)
{
...
#define SET(name) SetOption<Endpoint::Options, &Endpoint::Options::name>(env, &options, params, state.name##_string())

    if (!SET(retry_token_expiration) || !SET(token_expiration) || !SET(max_connections_per_host) || !SET(max_connections_total) || !SET(max_stateless_resets)
        || !SET(address_lru_size) || !SET(max_retries) || !SET(validate_address) || !SET(disable_stateless_reset) || !SET(ipv6_only) ||
#ifdef DEBUG
        !SET(rx_loss) || !SET(tx_loss) ||
#endif
        !SET(udp_receive_buffer_size) || !SET(udp_send_buffer_size) || !SET(udp_ttl) || !SET(reset_token_secret) || !SET(token_secret)) {
        return Nothing<Options>();
    }
    ...
    if (!address->IsUndefined()) {
        if (!SocketAddressBase::HasInstance(env, address)) {
            THROW_ERR_INVALID_ARG_TYPE(env, "The address option must be a SocketAddress");
            return Nothing<Options>();
        }
...
std::string Endpoint::Options::ToString() const
{
    ...
    auto boolToString = [](uint8_t val) { return val ? std::string("yes") : std::string("no"); };

    std::string res = "{ ";
    res += prefix + "local address: " + local_address->ToString();
    res += prefix + "retry token expiration: " + std::to_string(retry_token_expiration) + " seconds";
    res += prefix + "token expiration: " + std::to_string(token_expiration) + " seconds";
    res += prefix + "max connections per host: " + std::to_string(max_connections_per_host);
    ...
    res += prefix + "udp receive buffer size: " + std::to_string(udp_receive_buffer_size);
...
public:
    ...
    Impl(Endpoint* endpoint, Local<Object> object)
        : HandleWrap(endpoint->env(), object, reinterpret_cast<uv_handle_t*>(&handle_), PROVIDER_QUIC_UDP)
        , endpoint_(endpoint)
    {
...
private:
    ...
    static void OnReceive(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf, const sockaddr* addr, unsigned int flags)
    {
        ...
        if (nread < 0) {
            impl->endpoint_->Destroy(CloseContext::RECEIVE_FAILURE, static_cast<int>(nread));
            return;
        }

        impl->endpoint_->Receive(uv_buf_init(buf->base, static_cast<size_t>(nread)), SocketAddress(addr));
    }
...
int Endpoint::UDP::Bind(const Options& options)
{
    ...
    if (!err) {
        ...
        if (size > 0) {
            err = uv_recv_buffer_size(reinterpret_cast<uv_handle_t*>(&impl_->handle_), &size);
            if (err)
                return err;
        }
        ...
        if (size > 0) {
            err = uv_send_buffer_size(reinterpret_cast<uv_handle_t*>(&impl_->handle_), &size);
            if (err)
                return err;
        }
...
{
    if (is_closed_or_closing() || !is_started_)
        return;
...
int Endpoint::UDP::Send(const BaseObjectPtr<Packet>& packet)
{
    ...
    packet->ClearWeak();
    packet->Dispatched();
    int err = uv_udp_send(packet->req(), &impl_->handle_, &buf, 1, packet->destination().data(), uv_udp_send_cb { [](uv_udp_send_t* req, int status) {
        auto ptr = BaseObjectPtr<Packet>(static_cast<Packet*>(ReqWrap<uv_udp_send_t>::from_req(req)));
...
#define V(name, key, type)                                                                                                                                     \
    static constexpr auto IDX_STATE_ENDPOINT_##name = offsetof(Endpoint::State, key);                                                                          \
...
#undef V
    ...
    static constexpr auto DEFAULT_RETRYTOKEN_EXPIRATION = RetryToken::QUIC_DEFAULT_RETRYTOKEN_EXPIRATION / NGTCP2_SECONDS;
    static constexpr auto DEFAULT_REGULARTOKEN_EXPIRATION = RegularToken::QUIC_DEFAULT_REGULARTOKEN_EXPIRATION / NGTCP2_SECONDS;
    ...
    static constexpr auto CLOSECONTEXT_CLOSE = static_cast<int>(CloseContext::CLOSE);
    static constexpr auto CLOSECONTEXT_BIND_FAILURE = static_cast<int>(CloseContext::BIND_FAILURE);
    static constexpr auto CLOSECONTEXT_LISTEN_FAILURE = static_cast<int>(CloseContext::LISTEN_FAILURE);
    static constexpr auto CLOSECONTEXT_RECEIVE_FAILURE = static_cast<int>(CloseContext::RECEIVE_FAILURE);
    static constexpr auto CLOSECONTEXT_SEND_FAILURE = static_cast<int>(CloseContext::SEND_FAILURE);
    static constexpr auto CLOSECONTEXT_START_FAILURE = static_cast<int>(CloseContext::START_FAILURE);
    ...
    SetConstructorFunction(realm->context(), target, "Endpoint", GetConstructorTemplate(realm->env()));
}
...
Endpoint::Endpoint(Environment* env, Local<Object> object, const Options& options)
...
void Endpoint::MarkAsBusy(bool on)
{
    ...
    if (on)
        STAT_INCREMENT(Stats, server_busy_count);
    state_->busy = on ? 1 : 0;
}

RegularToken Endpoint::GenerateNewToken(uint32_t version, const SocketAddress& remote_address)
{
    Debug(this, "Generating new regular token for version %u and remote address %s", version, remote_address);
    DCHECK(!is_closed() && !is_closing());
    return RegularToken(version, remote_address, options_.token_secret);
}

StatelessResetToken Endpoint::GenerateNewStatelessResetToken(uint8_t* token, const CID& cid) const
{
    Debug(const_cast<Endpoint*>(this), "Generating new stateless reset token for CID %s", cid);
    DCHECK(!is_closed() && !is_closing());
...
void Endpoint::RemoveSession(const CID& cid, const SocketAddress& remote_address)
{
    if (is_closed())
        return;
...
{
    if (!is_closed() && !is_closing() && cid && scid && cid != scid && dcid_to_scid_[cid] != scid) {
...
void Endpoint::AssociateStatelessResetToken(const StatelessResetToken& token, Session* session)
{
...
void Endpoint::DisassociateStatelessResetToken(const StatelessResetToken& token)
{
    if (!is_closed()) {
...
void Endpoint::Send(const BaseObjectPtr<Packet>& packet)
{
    ...
    if (is_diagnostic_packet_loss(options_.tx_loss))
        [[unlikely]]
        {
            packet->Done(0);
...
void Endpoint::SendRetry(const PathDescriptor& options)
{
    ...
    if (++(info->retry_count) <= options_.max_retries) {
        auto packet = Packet::CreateRetryPacket(env(), this, options, options_.token_secret);
        if (packet) {
...
bool Endpoint::SendStatelessReset(const PathDescriptor& options, size_t source_len)
{
    ...
    auto packet = Packet::CreateStatelessResetPacket(env(), this, options, options_.reset_token_secret, source_len);
...
void Endpoint::SendImmediateConnectionClose(const PathDescriptor& options, QuicError reason)
{
    Debug(this, "Sending immediate connection close on path %s with reason %s", options, reason);
    // While it is possible for a malicious peer to cause us to create a large
    // number of these, generating them is fairly trivial.
    auto packet = Packet::CreateImmediateConnectionClosePacket(env(), this, options, reason);
    if (packet) {
...
{
    if (is_closed() || is_closing() || state_->listening == 1)
        return;
    ...
    if (!*context) {
        THROW_ERR_INVALID_STATE(env(), "Failed to create TLS context: %s", context->validation_error());
        return;
    }
...
BaseObjectPtr<Session> Endpoint::Connect(const SocketAddress& remote_address, const Session::Options& options, std::optional<SessionTicket> session_ticket)
{
    ...
    Debug(this, "Connecting to %s with options %s and config %s [has 0rtt ticket? %s]", remote_address, options, config,
        session_ticket.has_value() ? "yes" : "no");

    auto tls_context = TLSContext::CreateClient(options.tls_options);
    if (!*tls_context) {
        THROW_ERR_INVALID_STATE(env(), "Failed to create TLS context: %s", tls_context->validation_error());
        return {};
    }
    auto session = Session::Create(this, config, tls_context.get(), session_ticket);
    if (!session) {
        THROW_ERR_INVALID_STATE(env(), "Failed to create session");
        return {};
    }
    if (!session->tls_session()) {
        THROW_ERR_INVALID_STATE(env(), "Failed to create TLS session: %s", session->tls_session().validation_error());
        return {};
    }
...
{
    if (!is_closed() && sessions_.empty() && state_->pending_callbacks == 0 && state_->listening == 0) {
...
void Endpoint::Receive(const uv_buf_t& buf, const SocketAddress& remote_address)
{
    const auto receive
        = [&](Session* session, Store&& store, const SocketAddress& local_address, const SocketAddress& remote_address, const CID& dcid, const CID& scid) {
    ...
    const auto accept = [&](const Session::Config& config, Store&& store) {
        ...
        if (is_closed() || is_closing() || !is_listening())
            return;

        Debug(this, "Creating new session for %s", config.dcid);

        std::optional<SessionTicket> no_ticket = std::nullopt;
        auto session = Session::Create(this, config, server_state_->tls_context.get(), no_ticket);
        if (!session) {
            Debug(this, "Failed to create session for %s", config.dcid);
            return;
        }
        if (!session->tls_session()) {
            Debug(this, "Failed to create TLS session for %s: %s", config.dcid, session->tls_session().validation_error());
            return;
        }
        ...
        receive(session.get(), std::move(store), config.local_address, config.remote_address, config.dcid, config.scid);
    };

    const auto acceptInitialPacket = [&](const uint32_t version, const CID& dcid, const CID& scid, Store&& store, const SocketAddress& local_address,
        ...
        if (ngtcp2_is_supported_version(hd.version) == 0) {
            Debug(this,
                ...
                hd.version);
            SendVersionNegotiation(PathDescriptor { version, dcid, scid, local_address, remote_address });
        ...
        const auto limits_exceeded = ([&] {
            if (sessions_.size() >= options_.max_connections_total)
                return true;
        ...
        if (state_->busy || limits_exceeded) {
            ...
            SendImmediateConnectionClose(
                PathDescriptor { version, scid, dcid, local_address, remote_address }, QuicError::ForTransport(NGTCP2_CONNECTION_REFUSED));
            // The packet was successfully processed, even if we did refuse the
            // connection.
            STAT_INCREMENT(Stats, packets_received);
            return;
        ...
        Debug(this, "Accepting initial packet for %s from %s", dcid, remote_address);
        ...
        Session::Config config(env(), Side::SERVER, server_state_->options, version, local_address, remote_address, scid, dcid, dcid);
            ...
            case NGTCP2_PKT_INITIAL:
                ...
                if (options_.validate_address) {
                    ...
                    case RetryToken::kTokenMagic: {
                        ...
                        auto ocid = token.Validate(version, remote_address, dcid, options_.token_secret, options_.retry_token_expiration * NGTCP2_SECONDS);
                        if (!ocid.has_value()) {
                        ...
                        // validation.
                        Debug(this, "Retry token from %s is valid. Original dcid %s", remote_address, ocid.value());
                    ...
                    case RegularToken::kTokenMagic: {
                        ...
                        Debug(this, "Initial packet from %s has regular token %s", remote_address, token);
                        if (!token.Validate(version, remote_address, options_.token_secret, options_.token_expiration * NGTCP2_SECONDS)) {
                    ...
                    Debug(this, "Ignoring initial packet from %s with unexpected token", remote_address);
    ...
    const auto maybeStatelessReset
        = [&](const CID& dcid, const CID& scid, Store& store, const SocketAddress& local_address, const SocketAddress& remote_address) {
              ...
              if (options_.disable_stateless_reset || store.length() < NGTCP2_STATELESS_RESET_TOKENLEN) {
                  return false;
              }
              ...
              if (it != token_map_.end()) {
                  ...
                  if (!it->second->is_destroyed())
                      [[likely]]
                      {
                          receive(it->second, std::move(store), local_address, remote_address, dcid, scid);
                      }
                  return true;
              }
    ...
    // When diagnostic packet loss is enabled, the packet will be randomly
    // dropped.
    if (is_diagnostic_packet_loss(options_.rx_loss))
        [[unlikely]]
        {
    ...
    if (ngtcp2_pkt_decode_version_cid(&pversion_cid, vec.base, vec.len, NGTCP2_MAX_CIDLEN) < 0) {
    ...
    if (pversion_cid.dcidlen > NGTCP2_MAX_CIDLEN || pversion_cid.scidlen > NGTCP2_MAX_CIDLEN) {
    ...
    if (!session) {
        ...
        return acceptInitialPacket(pversion_cid.version, dcid, scid, std::move(store), addr, remote_address);
    }
...
bool Endpoint::SocketAddressInfoTraits::CheckExpired(const SocketAddress& address, const Type& type)
{
    return (uv_hrtime() - type.timestamp) > kSocketAddressInfoTimeout;
}

void Endpoint::SocketAddressInfoTraits::Touch(const SocketAddress& address, Type* type)
{
...
void Endpoint::EmitClose(CloseContext context, int status)
{
    ...
    CallbackScope<Endpoint> scope(this);
    auto isolate = env()->isolate();
    Local<Value> argv[] = { Integer::New(isolate, static_cast<int>(context)), Integer::New(isolate, static_cast<int>(status)) };

    Debug(this, "Notifying JavaScript about endpoint closing");
    MakeCallback(BindingData::Get(env()).endpoint_close_callback(), arraysize(argv), argv);
}
...
void Endpoint::LocalAddress(const FunctionCallbackInfo<Value>& args)
{
    ...
    Endpoint* endpoint;
    ASSIGN_OR_RETURN_UNWRAP(&endpoint, args.This());
    if (endpoint->is_closed() || !endpoint->udp_.is_bound())
        return;
    auto addr = SocketAddressBase::Create(env, std::make_shared<SocketAddress>(endpoint->local_address()));
    if (addr)
