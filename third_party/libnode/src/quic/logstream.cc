...
LogStream::LogStream(Environment* env, Local<Object> obj)
    : AsyncWrap(env, obj, PROVIDER_QUIC_LOGSTREAM)
    , StreamBase(env)
{
    MakeWeak();
...
int LogStream::DoWrite(WriteWrap* w, uv_buf_t* bufs, size_t count, uv_stream_t* send_handle)
{
    UNREACHABLE();
}
