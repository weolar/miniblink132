...
void EnableTrace(Environment* env, BIOPointer* bio, SSL* ssl)
{
    ...
    if (!*bio) {
        ...
        SSL_set_msg_callback(ssl, [](int write_p, int version, int content_type, const void* buf, size_t len, SSL* ssl, void* arg) -> void {
...
bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<v8::String>& name)
{
    Local<Value> value;
...
int TLSContext::OnSelectAlpn(SSL* ssl, const unsigned char** out, unsigned char* outlen, const unsigned char* in, unsigned int inlen, void* arg)
{
    ...
    if (requested.length() > kMaxAlpnLen)
    ...
    // one.
    if (SSL_select_next_proto(const_cast<unsigned char**>(out), outlen, reinterpret_cast<const unsigned char*>(requested.data()), requested.length(), in, inlen)
...
int TLSContext::OnNewSession(SSL* ssl, SSL_SESSION* sess)
{
    ...
    if (session.wants_session_ticket()) {
        ...
        if (size > 0 && size <= crypto::SecureContext::kMaxSessionSize) {
            auto ticket = ArrayBuffer::NewBackingStore(session.env()->isolate(), size);
...
int TLSContext::OnVerifyClientCertificate(int preverify_ok, X509_STORE_CTX* ctx)
{
    // TODO(@jasnell): Implement the logic to verify the client certificate
    return 1;
}

std::unique_ptr<TLSSession> TLSContext::NewSession(Session* session, const std::optional<SessionTicket>& maybeSessionTicket)
{
    ...
    return std::make_unique<TLSSession>(session, shared_from_this(), maybeSessionTicket);
}
...
SSLCtxPointer TLSContext::Initialize()
{
    ...
    case Side::SERVER: {
        ...
        SSL_CTX_set_options(
            ctx.get(), (SSL_OP_ALL & ~SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS) | SSL_OP_SINGLE_ECDH_USE | SSL_OP_CIPHER_SERVER_PREFERENCE | SSL_OP_NO_ANTI_REPLAY);
        SSL_CTX_set_mode(ctx.get(), SSL_MODE_RELEASE_BUFFERS);
        SSL_CTX_set_alpn_select_cb(ctx.get(), OnSelectAlpn, this);
        CHECK_EQ(SSL_CTX_set_session_id_context(ctx.get(), kSidCtx, sizeof(kSidCtx) - 1), 1);

        if (options_.verify_client) {
            SSL_CTX_set_verify(ctx.get(), SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, OnVerifyClientCertificate);
        }
    ...
    case Side::CLIENT: {
        ...
        SSL_CTX_set_session_cache_mode(ctx.get(), SSL_SESS_CACHE_CLIENT | SSL_SESS_CACHE_NO_INTERNAL);
        ...
        if (options_.ca.empty()) {
            ...
            for (const auto& ca : options_.ca) {
                ...
                if (buf.len == 0) {
                    ...
                    while (auto x509 = X509Pointer(PEM_read_bio_X509_AUX(bio.get(), nullptr, crypto::NoPasswordCallback, nullptr))) {
...

    {
        ...
        for (const auto& cert : options_.certs) {
            ...
            if (buf.len > 0) {
                ...
                cert_.reset();
                issuer_.reset();
                if (crypto::SSL_CTX_use_certificate_chain(ctx.get(), std::move(bio), &cert_, &issuer_) == 0) {
...

    {
        ...
        for (const auto& crl : options_.crl) {
            uv_buf_t buf = crl;
            BIOPointer bio = crypto::NodeBIO::NewFixed(buf.base, buf.len);
            DeleteFnPtr<X509_CRL, X509_CRL_free> crlptr(PEM_read_bio_X509_CRL(bio.get(), nullptr, crypto::NoPasswordCallback, nullptr));
    ...
    {
        ClearErrorOnReturn clear_error_on_return;
        if (options_.verify_private_key && SSL_CTX_check_private_key(ctx.get()) != 1) {
...
Maybe<TLSContext::Options> TLSContext::Options::From(Environment* env, Local<Value> value)
{
    if (value.IsEmpty()) {
...
#define SET_VECTOR(Type, name) SetOption<Type, TLSContext::Options, &TLSContext::Options::name>(env, &options, params, state.name##_string())

#define SET(name) SetOption<TLSContext::Options, &TLSContext::Options::name>(env, &options, params, state.name##_string())

    if (!SET(verify_client) || !SET(enable_tls_trace) || !SET(protocol) || !SET(servername) || !SET(ciphers) || !SET(groups) || !SET(verify_private_key)
        || !SET(keylog) || !SET_VECTOR(crypto::KeyObjectData, keys) || !SET_VECTOR(Store, certs) || !SET_VECTOR(Store, ca) || !SET_VECTOR(Store, crl)) {
        return Nothing<Options>();
    }
...
std::string TLSContext::Options::ToString() const
{
    ...
    res += prefix + "verify client: " + (verify_client ? std::string("yes") : std::string("no"));
    res += prefix + "enable_tls_trace: " + (enable_tls_trace ? std::string("yes") : std::string("no"));
    res += prefix + "verify private key: " + (verify_private_key ? std::string("yes") : std::string("no"));
...
TLSSession::TLSSession(Session* session, std::shared_ptr<TLSContext> context, const std::optional<SessionTicket>& maybeSessionTicket)
...
SSLPointer TLSSession::Initialize(const std::optional<SessionTicket>& maybeSessionTicket)
{
    auto& ctx = context();
    ...
    case Side::CLIENT: {
        ...
        if (SSL_set_alpn_protos(ssl.get(), reinterpret_cast<const unsigned char*>(options.protocol.data()), options.protocol.size()) != 0) {
        ...
        if (maybeSessionTicket.has_value()) {
            ...
            SSLSessionPointer ticket = crypto::GetTLSSession(reinterpret_cast<unsigned char*>(buf.base), buf.len);

            // The early data will just be ignored if it's invalid.
            if (ssl.setSession(ticket) && SSL_SESSION_get_max_early_data(ticket.get()) != 0) {
                ngtcp2_vec rtp = sessionTicket.transport_params();
                if (ngtcp2_conn_decode_and_set_0rtt_transport_params(*session_, rtp.base, rtp.len) == 0) {
                    SSL_set_quic_early_data_enabled(ssl.get(), 1);
...
std::optional<TLSSession::PeerIdentityValidationError> TLSSession::VerifyPeerIdentity(Environment* env)
{
    ...
    Local<Value> reason;
    Local<Value> code;
    if (!crypto::GetValidationErrorReason(env, err).ToLocal(&reason) || !crypto::GetValidationErrorCode(env, err).ToLocal(&code)) {
...
MaybeLocal<Object> TLSSession::peer_cert(Environment* env) const
{
    crypto::X509Certificate::GetPeerCertificateFlag flag
        = context_->side() == Side::SERVER ? crypto::X509Certificate::GetPeerCertificateFlag::SERVER : crypto::X509Certificate::GetPeerCertificateFlag::NONE;
...
const std::string_view TLSSession::protocol() const
{
    ...
    return alpnlen ? std::string_view(reinterpret_cast<const char*>(alpn_buf), alpnlen) : std::string_view();
}
