...
#define V(name)                                                                                                                                                \
    void BindingData::set_##name##_constructor_template(Local<FunctionTemplate> tmpl)                                                                          \
    {                                                                                                                                                          \
        name##_constructor_template_.Reset(env()->isolate(), tmpl);                                                                                            \
    }                                                                                                                                                          \
    Local<FunctionTemplate> BindingData::name##_constructor_template() const                                                                                   \
    {                                                                                                                                                          \
        return PersistentToLocal::Default(env()->isolate(), name##_constructor_template_);                                                                     \
    }
...
#define V(name, value)                                                                                                                                         \
    ...
    {                                                                                                                                                          \
        if (name##_string_.IsEmpty())                                                                                                                          \
            name##_string_.Set(env()->isolate(), OneByteString(env()->isolate(), value));                                                                      \
...
#define V(name, value)                                                                                                                                         \
    ...
    {                                                                                                                                                          \
        if (on_##name##_string_.IsEmpty())                                                                                                                     \
            on_##name##_string_.Set(env()->isolate(), FIXED_ONE_BYTE_STRING(env()->isolate(), "on" #value));                                                   \
...
#define V(name, key)                                                                                                                                           \
    do {                                                                                                                                                       \
        Local<Value> val;                                                                                                                                      \
        if (!obj->Get(env->context(), state.on_##name##_string()).ToLocal(&val) || !val->IsFunction()) {                                                       \
...
NgTcp2CallbackScope::NgTcp2CallbackScope(Environment* env)
    : env(env)
{
...
NgHttp3CallbackScope::NgHttp3CallbackScope(Environment* env)
    : env(env)
{
    auto& binding = BindingData::Get(env);
