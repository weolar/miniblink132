...
Local<FunctionTemplate> Http3Application::GetConstructorTemplate(Environment* env)
{
...
void Http3Application::InitPerIsolate(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
...
{
    SetConstructorFunction(realm->context(), target, "Http3Application", GetConstructorTemplate(realm->env()));
}

void Http3Application::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
    registry->Register(New);
}

Http3Application::Http3Application(Environment* env, Local<Object> object, const Session::Application::Options& options)
    : ApplicationProvider(env, object)
    , options_(options)
{
    MakeWeak();
}
...
void Http3Application::New(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsUndefined() && !Session::Application::Options::From(env, args[0]).To(&options)) {
        return;
    }
...
public:
    ...
    bool Start() override
    {
        ...
        if (params == nullptr)
            [[unlikely]]
            {
                ...
                Debug(&session(), "Cannot start HTTP/3 application yet. No remote transport params");
                return false;
            }
        ...
        if (session().is_server()) {
            nghttp3_conn_set_max_client_streams_bidi(*this, params->initial_max_streams_bidi);
        }

        Debug(&session(), "Creating and binding HTTP/3 control streams");
        bool ret = ngtcp2_conn_open_uni_stream(session(), &control_stream_id_, nullptr) == 0
            && ngtcp2_conn_open_uni_stream(session(), &qpack_enc_stream_id_, nullptr) == 0
            && ngtcp2_conn_open_uni_stream(session(), &qpack_dec_stream_id_, nullptr) == 0 && nghttp3_conn_bind_control_stream(*this, control_stream_id_) == 0
            && nghttp3_conn_bind_qpack_streams(*this, qpack_enc_stream_id_, qpack_dec_stream_id_) == 0;

        if (env()->enabled_debug_list()->enabled(DebugCategory::QUIC) && ret) {
            Debug(&session(), "Created and bound control stream %" PRIi64, control_stream_id_);
            Debug(&session(), "Created and bound qpack enc stream %" PRIi64, qpack_enc_stream_id_);
            Debug(&session(), "Created and bound qpack dec streams %" PRIi64, qpack_dec_stream_id_);
        }

        return ret;
    }

    bool ReceiveStreamData(int64_t stream_id, const uint8_t* data, size_t datalen, const Stream::ReceiveDataFlags& flags, void* unused) override
    {
        Debug(&session(),
        ...
        ssize_t nread = nghttp3_conn_read_stream(*this, stream_id, data, datalen, flags.fin ? 1 : 0);

        if (nread < 0) {
            Debug(&session(), "HTTP/3 application failed to read stream data: %s", nghttp3_strerror(nread));
            return false;
        }

        if (nread > 0) {
            Debug(&session(), "Extending stream and connection offset by %zd bytes", nread);
    ...
    bool CanAddHeader(size_t current_count, size_t current_headers_length, size_t this_header_length) override
    {
        // We cannot add the header if we've either reached
        // * the max number of header pairs or
        // * the max number of header bytes
        return (current_count < options_.max_header_pairs) && (current_headers_length + this_header_length) <= options_.max_header_length;
    }
    ...
    void ExtendMaxStreams(EndpointLabel label, Direction direction, uint64_t max_streams) override
    {
        switch (label) {
        ...
        case EndpointLabel::REMOTE: {
            ...
            case Direction::BIDIRECTIONAL: {
                Debug(&session(), "HTTP/3 application extending max bidi streams by %" PRIu64, max_streams);
                ngtcp2_conn_extend_max_streams_bidi(session(), static_cast<size_t>(max_streams));
                break;
            }
            case Direction::UNIDIRECTIONAL: {
                Debug(&session(), "HTTP/3 application extending max uni streams by %" PRIu64, max_streams);
                ngtcp2_conn_extend_max_streams_uni(session(), static_cast<size_t>(max_streams));
                break;
            }
            }
        }
    ...
    {
        Debug(&session(), "HTTP/3 application extending max stream data to %" PRIu64, max_data);
        nghttp3_conn_unblock_stream(*this, stream->id());
    }

    void CollectSessionTicketAppData(SessionTicket::AppData* app_data) const override
    {
        // TODO(@jasnell): There's currently nothing to store but there may be
        // later.
    }

    SessionTicket::AppData::Status ExtractSessionTicketAppData(const SessionTicket::AppData& app_data, SessionTicket::AppData::Source::Flag flag) override
    {
        // There's currently nothing stored here but we might do so later.
        return flag == SessionTicket::AppData::Source::Flag::STATUS_RENEW ? SessionTicket::AppData::Status::TICKET_USE_RENEW
                                                                          : SessionTicket::AppData::Status::TICKET_USE;
    }

    void StreamClose(Stream* stream, QuicError&& error = QuicError()) override
    {
        Debug(&session(), "HTTP/3 application closing stream %" PRIi64, stream->id());
        ...
        session().SetLastError(QuicError::ForApplication(nghttp3_err_infer_quic_app_error_code(rv)));
        session().Close();
    }

    void StreamReset(Stream* stream, uint64_t final_size, QuicError&& error = QuicError()) override
    {
        // We are shutting down the readable side of the local stream here.
        Debug(&session(), "HTTP/3 application resetting stream %" PRIi64, stream->id());
    ...
    void StreamStopSending(Stream* stream, QuicError&& error = QuicError()) override
    {
        Application::StreamStopSending(stream, std::move(error));
    }

    bool SendHeaders(const Stream& stream, HeadersKind kind, const Local<v8::Array>& headers, HeadersFlags flags = HeadersFlags::NONE) override
    {
        ...
        case HeadersKind::HINTS: {
            ...
            Debug(&session(), "Submitting %" PRIu64 " early hints for stream %" PRIu64, stream.id());
            return nghttp3_conn_submit_info(*this, stream.id(), nva.data(), nva.length()) == 0;
            break;
        }
        case HeadersKind::INITIAL: {
            ...
            if (session().is_server()) {
                ...
                Debug(&session(), "Submitting %" PRIu64 " response headers for stream %" PRIu64, nva.length(), stream.id());
                return nghttp3_conn_submit_response(*this, stream.id(), nva.data(), nva.length(), reader_ptr);
            } else {
                // Otherwise we're submitting a request...
                Debug(&session(), "Submitting %" PRIu64 " request headers for stream %" PRIu64, nva.length(), stream.id());
                return nghttp3_conn_submit_request(*this, stream.id(), nva.data(), nva.length(), reader_ptr, const_cast<Stream*>(&stream)) == 0;
            }
            break;
        }
        case HeadersKind::TRAILING: {
            Debug(&session(), "Submitting %" PRIu64 " trailing headers for stream %" PRIu64, nva.length(), stream.id());
            return nghttp3_conn_submit_trailers(*this, stream.id(), nva.data(), nva.length()) == 0;
            break;
        }
        }
    ...
    int GetStreamData(StreamData* data) override
    {
        ...
        if (conn_ && session().max_data_left()) {
            ret = nghttp3_conn_writev_stream(*this, &data->id, &data->fin, *data, data->count);
            ...
            if (data->id > 0 && data->id != control_stream_id_ && data->id != qpack_dec_stream_id_ && data->id != qpack_enc_stream_id_) {
    ...
    {
        Debug(&session(), "HTTP/3 application committing stream %" PRIi64 " data %zu", data->id, datalen);
        int err = nghttp3_conn_add_write_offset(*this, data->id, datalen);
        if (err != 0) {
            session().SetLastError(QuicError::ForApplication(nghttp3_err_infer_quic_app_error_code(err)));
            return false;
        }
        return true;
    }
...
private:
    ...
    {
        return id == control_stream_id_ || id == qpack_dec_stream_id_ || id == qpack_enc_stream_id_;
    }
    ...
    Http3ConnectionPointer InitializeConnection()
    {
        ...
        if (session().is_server()) {
            CHECK_EQ(nghttp3_conn_server_new(&conn, &kCallbacks, &settings, &allocator_, this), 0);
        } else {
            CHECK_EQ(nghttp3_conn_client_new(&conn, &kCallbacks, &settings, &allocator_, this), 0);
        }
    ...
    void OnStreamClose(Stream* stream, uint64_t app_error_code)
    {
        if (app_error_code != NGHTTP3_H3_NO_ERROR) {
            Debug(&session(), "HTTP/3 application received stream close for stream %" PRIi64 " with code %" PRIu64, stream->id(), app_error_code);
        }
    ...
    void OnReceiveHeader(int64_t stream_id, Http3Header&& header)
    {
        ...
        if (header.name() == ":status" && header.value()[0] == '1') {
            Debug(&session(), "HTTP/3 application switching to hints headers for stream %" PRIi64, stream->id());
            stream->set_headers_kind(HeadersKind::HINTS);
        }
        IF_QUIC_DEBUG(env())
        {
            Debug(&session(), "Received header \"%s: %s\"", header.name(), header.value());
        }
    ...
    void OnEndHeaders(int64_t stream_id, int fin)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application received end of headers for stream %" PRIi64, stream_id);
        stream->EmitHeaders();
        if (fin) {
    ...
    void OnBeginTrailers(int64_t stream_id)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application beginning block of trailers for stream %" PRIi64, stream_id);
    ...
    void OnEndTrailers(int64_t stream_id, int fin)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application received end of trailers for stream %" PRIi64, stream_id);
        stream->EmitHeaders();
        if (fin) {
    ...
    void OnEndStream(int64_t stream_id)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application received end of stream for stream %" PRIi64, stream_id);
        Stream::ReceiveDataFlags flags {
            .fin = true,
            .early = false,
        };
    ...
    void OnStopSending(int64_t stream_id, uint64_t app_error_code)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application received stop sending for stream %" PRIi64, stream_id);
    ...
    void OnResetStream(int64_t stream_id, uint64_t app_error_code)
    {
        ...
        if (!stream)
            [[unlikely]] return;
        Debug(&session(), "HTTP/3 application received reset stream for stream %" PRIi64, stream_id);
    ...
    void OnReceiveSettings(const nghttp3_settings* settings)
    {
        ...
        options_.qpack_encoder_max_dtable_capacity = settings->qpack_encoder_max_dtable_capacity;
        options_.qpack_max_dtable_capacity = settings->qpack_max_dtable_capacity;
        Debug(&session(), "HTTP/3 application received updated settings: %s", options_);
    }
    ...
    static BaseObjectWeakPtr<Stream> FindOrCreateStream(nghttp3_conn* conn, Session* session, int64_t stream_id)
    {
...
#define NGHTTP3_CALLBACK_SCOPE(name)                                                                                                                           \
    ...
    static nghttp3_ssize on_read_data_callback(
        nghttp3_conn* conn, int64_t stream_id, nghttp3_vec* vec, size_t veccnt, uint32_t* pflags, void* conn_user_data, void* stream_user_data)
    {
    ...
    static int on_acked_stream_data(nghttp3_conn* conn, int64_t stream_id, uint64_t datalen, void* conn_user_data, void* stream_user_data)
    {
        ...
        return app.AcknowledgeStreamData(stream_id, static_cast<size_t>(datalen)) ? NGTCP2_SUCCESS : NGHTTP3_ERR_CALLBACK_FAILURE;
    }
    ...
    static int on_receive_data(nghttp3_conn* conn, int64_t stream_id, const uint8_t* data, size_t datalen, void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        ...
        if (app.is_control_stream(stream_id))
            [[unlikely]]
            {
    ...
    static int on_receive_header(nghttp3_conn* conn, int64_t stream_id, int32_t token, nghttp3_rcbuf* name, nghttp3_rcbuf* value, uint8_t flags,
        void* conn_user_data, void* stream_user_data)
    {
        ...
        if (Http3Header::IsZeroLength(token, name, value))
            return NGTCP2_SUCCESS;
        app.OnReceiveHeader(stream_id, Http3Header(app.env(), token, name, value, flags));
        return NGTCP2_SUCCESS;
    }
    ...
    static int on_begin_trailers(nghttp3_conn* conn, int64_t stream_id, void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        if (app.is_control_stream(stream_id))
            [[unlikely]]
    ...
    static int on_receive_trailer(nghttp3_conn* conn, int64_t stream_id, int32_t token, nghttp3_rcbuf* name, nghttp3_rcbuf* value, uint8_t flags,
        void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        if (app.is_control_stream(stream_id))
        ...
        if (Http3Header::IsZeroLength(token, name, value))
            return NGTCP2_SUCCESS;
        app.OnReceiveTrailer(stream_id, Http3Header(app.env(), token, name, value, flags));
        return NGTCP2_SUCCESS;
    }

    static int on_end_trailers(nghttp3_conn* conn, int64_t stream_id, int fin, void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        if (app.is_control_stream(stream_id))
            [[unlikely]]
    ...
    static int on_stop_sending(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        if (app.is_control_stream(stream_id))
            [[unlikely]]
    ...
    static int on_reset_stream(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* conn_user_data, void* stream_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
        if (app.is_control_stream(stream_id))
            [[unlikely]]
    ...
    static int on_receive_settings(nghttp3_conn* conn, const nghttp3_settings* settings, void* conn_user_data)
    {
        NGHTTP3_CALLBACK_SCOPE(app);
    ...
    static constexpr nghttp3_callbacks kCallbacks
        = { on_acked_stream_data, on_stream_close, on_receive_data, on_deferred_consume, on_begin_headers, on_receive_header, on_end_headers, on_begin_trailers,
              on_receive_trailer, on_end_trailers, on_stop_sending, on_end_stream, on_reset_stream, on_shutdown, on_receive_settings };
};

std::unique_ptr<Session::Application> Http3Application::Create(Session* session)
{
