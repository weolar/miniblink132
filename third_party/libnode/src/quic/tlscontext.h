...
public:
    ...
    TLSSession(Session* session, std::shared_ptr<TLSContext> context, const std::optional<SessionTicket>& maybeSessionTicket);
    ...
    inline TLSContext& context() const
    {
        return *context_;
    }
    ...
    // failure.
    std::optional<PeerIdentityValidationError> VerifyPeerIdentity(Environment* env);
...
private:
    operator SSL*() const;
    ncrypto::SSLPointer Initialize(const std::optional<SessionTicket>& maybeSessionTicket);
...
class TLSContext final : public MemoryRetainer, public std::enable_shared_from_this<TLSContext> {
public:
    ...
    std::unique_ptr<TLSSession> NewSession(Session* session, const std::optional<SessionTicket>& maybeSessionTicket);
...
private:
    ...
    static int OnSelectAlpn(SSL* ssl, const unsigned char** out, unsigned char* outlen, const unsigned char* in, unsigned int inlen, void* arg);
