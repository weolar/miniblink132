...
namespace {
template <int FAMILY> std::optional<const PreferredAddress::AddressInfo> get_address_info(const ngtcp2_preferred_addr& paddr)
{
    if constexpr (FAMILY == AF_INET) {
        ...
        if (uv_inet_ntop(FAMILY, &paddr.ipv4.sin_addr, address.host, sizeof(address.host)) == 0) {
        ...
        if (uv_inet_ntop(FAMILY, &paddr.ipv6.sin6_addr, address.host, sizeof(address.host)) == 0) {
            address.address = address.host;
        }
        return address;
    }
...
template <int FAMILY> void copy_to_transport_params(ngtcp2_transport_params* params, const sockaddr* addr)
{
    ...
    if constexpr (FAMILY == AF_INET) {
        ...
        memcpy(&params->preferred_addr.ipv4.sin_addr, &src->sin_addr, sizeof(params->preferred_addr.ipv4.sin_addr));
    } else {
        ...
        memcpy(&params->preferred_addr.ipv6.sin6_addr, &src->sin6_addr, sizeof(params->preferred_addr.ipv4.sin_addr));
    }
    UNREACHABLE();
}

bool resolve(const PreferredAddress::AddressInfo& address, uv_getaddrinfo_t* req)
{
    addrinfo hints {};
    ...
    return uv_getaddrinfo(nullptr, req, nullptr, address.host,
...
PreferredAddress::PreferredAddress(ngtcp2_path* dest, const ngtcp2_preferred_addr* paddr)
    : dest_(dest)
    , paddr_(paddr)
{
...
std::optional<const PreferredAddress::AddressInfo> PreferredAddress::ipv4() const
{
    return get_address_info<AF_INET>(*paddr_);
}

std::optional<const PreferredAddress::AddressInfo> PreferredAddress::ipv6() const
{
...
void PreferredAddress::Set(ngtcp2_transport_params* params, const sockaddr* addr)
{
...
Maybe<PreferredAddress::Policy> PreferredAddress::tryGetPolicy(Environment* env, Local<Value> value)
{
