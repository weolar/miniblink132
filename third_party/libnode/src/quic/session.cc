...
// stream.
void on_qlog_write(void* user_data, uint32_t flags, const void* data, size_t len)
{
...
bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    PreferredAddress::Policy policy = PreferredAddress::Policy::USE_PREFERRED;
    if (!object->Get(env->context(), name).ToLocal(&value) || !PreferredAddress::tryGetPolicy(env, value).To(&policy)) {
        return false;
    }
...
template <typename Opt, TLSContext::Options Opt::*member> bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    TLSContext::Options opts;
    if (!object->Get(env->context(), name).ToLocal(&value) || !TLSContext::Options::From(env, value).To(&opts)) {
        return false;
    }
    options->*member = opts;
    return true;
...
bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    ...
    if (!object->Get(env->context(), name).ToLocal(&value) || !TransportParams::Options::From(env, value).To(&opts)) {
        return false;
    }
    options->*member = opts;
    return true;
...
template <typename Opt, BaseObjectPtr<Session::ApplicationProvider> Opt::*member>
bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    ...
    if (!value->IsUndefined()) {
        ...
        if (!Http3Application::HasInstance(env, value)) {
            THROW_ERR_INVALID_ARG_TYPE(env, "Application must be an Http3Application");
            return false;
        }
        Http3Application* app;
        ASSIGN_OR_RETURN_UNWRAP(&app, value.As<Object>(), false);
        CHECK_NOT_NULL(app);
        auto& assigned = options->*member = BaseObjectPtr<Session::ApplicationProvider>(app);
        assigned->Detach();
    }
    return true;
}

template <typename Opt, ngtcp2_cc_algo Opt::*member> bool SetOption(Environment* env, Opt* options, const Local<Object>& object, const Local<String>& name)
{
    Local<Value> value;
    if (!object->Get(env->context(), name).ToLocal(&value))
        return false;
    if (!value->IsUndefined()) {
        ...
        if (value->IsString()) {
            ...
            if (!value->IsInt32()) {
                THROW_ERR_INVALID_ARG_VALUE(env, "The cc_algorithm option must be a string or an integer");
                return false;
            }
            Local<Int32> num;
...
// ============================================================================
Session::Config::Config(Environment* env, Side side, const Options& options, uint32_t version, const SocketAddress& local_address,
    const SocketAddress& remote_address, const CID& dcid, const CID& scid, const CID& ocid)
    : side(side)
    , options(options)
    , version(version)
...
Session::Config::Config(Environment* env, const Options& options, const SocketAddress& local_address, const SocketAddress& remote_address, const CID& ocid)
    : Config(env, Side::CLIENT, options, options.version, local_address, remote_address, CID::Factory::random().Generate(NGTCP2_MIN_INITIAL_DCIDLEN),
        options.cid_factory->Generate(), ocid)
{
}
...
Maybe<Session::Options> Session::Options::From(Environment* env, Local<Value> value)
{
...
#define SET(name) SetOption<Session::Options, &Session::Options::name>(env, &options, params, state.name##_string())

    if (!SET(version) || !SET(min_version) || !SET(preferred_address_strategy) || !SET(transport_params) || !SET(tls_options) || !SET(qlog)
        || !SET(application_provider) || !SET(handshake_timeout) || !SET(max_stream_window) || !SET(max_window) || !SET(max_payload_size)
...
std::string Session::Options::ToString() const
{
    ...
    res += prefix + "preferred address policy: " + to_string(preferred_address_strategy);
    ...
    if (handshake_timeout == UINT64_MAX) {
        ...
        res += prefix + "handshake timeout: " + std::to_string(handshake_timeout) + " nanoseconds";
    }
    ...
    if (unacknowledged_packet_threshold != 0) {
        res += prefix + "unacknowledged packet threshold: " + std::to_string(unacknowledged_packet_threshold);
    } else {
   ...
   */
    bool Close()
    {
        ...
        for (auto& stream : streams)
            stream.second->Destroy(last_error_);
    ...
    ~Impl()
    {
        ...
        MaybeStackBuffer<ngtcp2_cid, 10> cids(ngtcp2_conn_get_scid(*session_, nullptr));
        ngtcp2_conn_get_scid(*session_, cids.out());

        MaybeStackBuffer<ngtcp2_cid_token, 10> tokens(ngtcp2_conn_get_active_dcid(*session_, nullptr));
        ...
        for (size_t n = 0; n < tokens.length(); n++) {
            if (tokens[n].token_present) {
                endpoint->DisassociateStatelessResetToken(StatelessResetToken(tokens[n].token));
            }
        }
    ...
    static void GetRemoteAddress(const FunctionCallbackInfo<Value>& args)
    {
        ...
        args.GetReturnValue().Set(SocketAddressBase::Create(env, std::make_shared<SocketAddress>(address))->object());
    }
    ...
    static void GetEphemeralKeyInfo(const FunctionCallbackInfo<Value>& args)
    {
        ...
        Local<Object> ret;
        if (!session->is_server() && session->tls_session().ephemeral_key(env).ToLocal(&ret))
            args.GetReturnValue().Set(ret);
    }
    ...
    static void OpenStream(const FunctionCallbackInfo<Value>& args)
    {
        ...
        std::shared_ptr<DataQueue> data_source = Stream::GetDataQueueFromSource(env, args[1]).ToChecked();
    ...
    static void SendDatagram(const FunctionCallbackInfo<Value>& args)
    {
        ...
        SendPendingDataScope send_scope(session);
        args.GetReturnValue().Set(BigInt::New(env->isolate(), session->SendDatagram(Store(args[0].As<ArrayBufferView>()))));
    }
        ...
        ngtcp2_conn* conn, int64_t stream_id, uint64_t offset, uint64_t datalen, void* user_data, void* stream_user_data)
    {
        ...
        return session->application().AcknowledgeStreamData(stream_id, datalen) ? NGTCP2_SUCCESS : NGTCP2_ERR_CALLBACK_FAILURE;
    }

    static int on_acknowledge_datagram(ngtcp2_conn* conn, uint64_t dgram_id, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_cid_status(
        ngtcp2_conn* conn, ngtcp2_connection_id_status_type type, uint64_t seq, const ngtcp2_cid* cid, const uint8_t* token, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        std::optional<StatelessResetToken> maybe_reset_token;
        if (token != nullptr)
            maybe_reset_token.emplace(token);
        ...
        case NGTCP2_CONNECTION_ID_STATUS_TYPE_ACTIVATE: {
            ...
            if (token != nullptr) {
                endpoint.AssociateStatelessResetToken(StatelessResetToken(token), session);
            }
            break;
        }
    ...
    static int on_extend_max_remote_streams_uni(ngtcp2_conn* conn, uint64_t max_streams, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        // TODO(@jasnell): Do anything here?
        return NGTCP2_SUCCESS;
    ...
    static int on_extend_max_streams_bidi(ngtcp2_conn* conn, uint64_t max_streams, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_extend_max_streams_uni(ngtcp2_conn* conn, uint64_t max_streams, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    {
        NGTCP2_CALLBACK_SCOPE(session)
        session->application().ExtendMaxStreamData(Stream::From(stream_user_data), max_data);
        return NGTCP2_SUCCESS;
    }

    static int on_get_new_cid(ngtcp2_conn* conn, ngtcp2_cid* cid, uint8_t* token, size_t cidlen, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    {
        NGTCP2_CALLBACK_SCOPE(session)
        return session->HandshakeCompleted() ? NGTCP2_SUCCESS : NGTCP2_ERR_CALLBACK_FAILURE;
    }
    ...
    static int on_lost_datagram(ngtcp2_conn* conn, uint64_t dgram_id, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_path_validation(
        ngtcp2_conn* conn, uint32_t flags, const ngtcp2_path* path, const ngtcp2_path* old_path, ngtcp2_path_validation_result res, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        bool flag_preferred_address = flags & NGTCP2_PATH_VALIDATION_FLAG_PREFERRED_ADDR;
        ValidatedPath newValidatedPath { std::make_shared<SocketAddress>(path->local.addr), std::make_shared<SocketAddress>(path->remote.addr) };
        std::optional<ValidatedPath> oldValidatedPath = std::nullopt;
        if (old_path != nullptr) {
            oldValidatedPath = ValidatedPath { std::make_shared<SocketAddress>(old_path->local.addr), std::make_shared<SocketAddress>(old_path->remote.addr) };
        }
        session->EmitPathValidation(static_cast<PathValidationResult>(res), PathValidationFlags { flag_preferred_address }, newValidatedPath, oldValidatedPath);
        return NGTCP2_SUCCESS;
    }

    static int on_receive_datagram(ngtcp2_conn* conn, uint32_t flags, const uint8_t* data, size_t datalen, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        session->DatagramReceived(data, datalen,
    ...
    static int on_receive_new_token(ngtcp2_conn* conn, const uint8_t* token, size_t tokenlen, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_receive_rx_key(ngtcp2_conn* conn, ngtcp2_encryption_level level, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        ...
        Debug(session, "Receiving RX key for level %s for dcid %s", to_string(level), session->config().dcid);

        return session->application().Start() ? NGTCP2_SUCCESS : NGTCP2_ERR_CALLBACK_FAILURE;
    }

    static int on_receive_stateless_reset(ngtcp2_conn* conn, const ngtcp2_pkt_stateless_reset* sr, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_receive_stream_data(
        ngtcp2_conn* conn, uint32_t flags, int64_t stream_id, uint64_t offset, const uint8_t* data, size_t datalen, void* user_data, void* stream_user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
        Stream::ReceiveDataFlags data_flags {
            ...
            .fin = (flags & NGTCP2_STREAM_DATA_FLAG_FIN) == NGTCP2_STREAM_DATA_FLAG_FIN,
            // Stream data is early if it is received before the TLS handshake is
            // complete.
            .early = (flags & NGTCP2_STREAM_DATA_FLAG_0RTT) == NGTCP2_STREAM_DATA_FLAG_0RTT,
        };
        ...
        if (!session->application().ReceiveStreamData(stream_id, data, datalen, data_flags, stream_user_data)) {
            return NGTCP2_ERR_CALLBACK_FAILURE;
        }
    ...
    static int on_receive_tx_key(ngtcp2_conn* conn, ngtcp2_encryption_level level, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session);
        CHECK(session->is_server());
        ...
        Debug(session, "Receiving TX key for level %s for dcid %s", to_string(level), session->config().dcid);
        return session->application().Start() ? NGTCP2_SUCCESS : NGTCP2_ERR_CALLBACK_FAILURE;
    }

    static int on_receive_version_negotiation(ngtcp2_conn* conn, const ngtcp2_pkt_hd* hd, const uint32_t* sv, size_t nsv, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_select_preferred_address(ngtcp2_conn* conn, ngtcp2_path* dest, const ngtcp2_preferred_addr* paddr, void* user_data)
    {
        NGTCP2_CALLBACK_SCOPE(session)
    ...
    static int on_stream_close(ngtcp2_conn* conn, uint32_t flags, int64_t stream_id, uint64_t app_error_code, void* user_data, void* stream_user_data)
    {
        ...
        if (flags & NGTCP2_STREAM_CLOSE_FLAG_APP_ERROR_CODE_SET) {
            session->application().StreamClose(Stream::From(stream_user_data), QuicError::ForApplication(app_error_code));
        } else {
    ...
    {
        NGTCP2_CALLBACK_SCOPE(session)
        session->application().StreamReset(Stream::From(stream_user_data), final_size, QuicError::ForApplication(app_error_code));
        return NGTCP2_SUCCESS;
    }
    ...
    {
        NGTCP2_CALLBACK_SCOPE(session)
        session->application().StreamStopSending(Stream::From(stream_user_data), QuicError::ForApplication(app_error_code));
        return NGTCP2_SUCCESS;
    }
    ...
    static constexpr ngtcp2_callbacks CLIENT = { ngtcp2_crypto_client_initial_cb, nullptr, ngtcp2_crypto_recv_crypto_data_cb, on_handshake_completed,
        on_receive_version_negotiation, ngtcp2_crypto_encrypt_cb, ngtcp2_crypto_decrypt_cb, ngtcp2_crypto_hp_mask_cb, on_receive_stream_data,
        on_acknowledge_stream_data_offset, nullptr, on_stream_close, on_receive_stateless_reset, ngtcp2_crypto_recv_retry_cb, on_extend_max_streams_bidi,
        on_extend_max_streams_uni, on_rand, on_get_new_cid, on_remove_connection_id, ngtcp2_crypto_update_key_cb, on_path_validation,
        on_select_preferred_address, on_stream_reset, on_extend_max_remote_streams_bidi, on_extend_max_remote_streams_uni, on_extend_max_stream_data,
        on_cid_status, on_handshake_confirmed, on_receive_new_token, ngtcp2_crypto_delete_crypto_aead_ctx_cb, ngtcp2_crypto_delete_crypto_cipher_ctx_cb,
        on_receive_datagram, on_acknowledge_datagram, on_lost_datagram, ngtcp2_crypto_get_path_challenge_data_cb, on_stream_stop_sending,
        ngtcp2_crypto_version_negotiation_cb, on_receive_rx_key, nullptr, on_early_data_rejected };

    static constexpr ngtcp2_callbacks SERVER = { nullptr, ngtcp2_crypto_recv_client_initial_cb, ngtcp2_crypto_recv_crypto_data_cb, on_handshake_completed,
        nullptr, ngtcp2_crypto_encrypt_cb, ngtcp2_crypto_decrypt_cb, ngtcp2_crypto_hp_mask_cb, on_receive_stream_data, on_acknowledge_stream_data_offset,
        nullptr, on_stream_close, on_receive_stateless_reset, nullptr, on_extend_max_streams_bidi, on_extend_max_streams_uni, on_rand, on_get_new_cid,
        on_remove_connection_id, ngtcp2_crypto_update_key_cb, on_path_validation, nullptr, on_stream_reset, on_extend_max_remote_streams_bidi,
        on_extend_max_remote_streams_uni, on_extend_max_stream_data, on_cid_status, nullptr, nullptr, ngtcp2_crypto_delete_crypto_aead_ctx_cb,
        ngtcp2_crypto_delete_crypto_cipher_ctx_cb, on_receive_datagram, on_acknowledge_datagram, on_lost_datagram, ngtcp2_crypto_get_path_challenge_data_cb,
        on_stream_stop_sending, ngtcp2_crypto_version_negotiation_cb, nullptr, on_receive_tx_key, on_early_data_rejected };
};
...
Session::SendPendingDataScope::SendPendingDataScope(const BaseObjectPtr<Session>& session)
...
BaseObjectPtr<Session> Session::Create(Endpoint* endpoint, const Config& config, TLSContext* tls_context, const std::optional<SessionTicket>& ticket)
{
    Local<Object> obj;
    if (!GetConstructorTemplate(endpoint->env())->InstanceTemplate()->NewInstance(endpoint->env()->context()).ToLocal(&obj)) {
        return {};
    }

    return MakeDetachedBaseObject<Session>(endpoint, obj, config, tls_context, ticket);
}

Session::Session(Endpoint* endpoint, Local<Object> object, const Config& config, TLSContext* tls_context, const std::optional<SessionTicket>& session_ticket)
    ...
    , tls_session_(tls_context->NewSession(this, session_ticket))
{
    ...
    const auto defineProperty = [&](auto name, auto value) { object->DefineOwnProperty(env()->context(), name, value, PropertyAttribute::ReadOnly).Check(); };
    ...
    if (config.options.tls_options.keylog)
        [[unlikely]]
        {
...
Session::QuicConnectionPointer Session::InitConnection()
{
    ...
    TransportParams transport_params(tp_config, config().options.transport_params);
    ...
    case Side::SERVER: {
        ...
        CHECK_EQ(ngtcp2_conn_server_new(
                     &conn, config().dcid, config().scid, path, config().version, &Impl::SERVER, &config().settings, transport_params, &allocator_, this),
            0);
        break;
    }
    case Side::CLIENT: {
        CHECK_EQ(ngtcp2_conn_client_new(
                     &conn, config().dcid, config().scid, path, config().version, &Impl::CLIENT, &config().settings, transport_params, &allocator_, this),
            0);
        break;
    }
    }
...
void Session::Close(CloseMethod method)
{
    ...
    case CloseMethod::GRACEFUL: {
        ...
        if (impl_->state_->graceful_close)
            [[unlikely]]
            {
                return;
            }
        impl_->state_->graceful_close = 1;
        Debug(this, "Gracefully closing session (waiting on %zu streams)", impl_->streams_.size());
        return;
    }
    }
    UNREACHABLE();
...
void Session::Destroy()
{
    ...
    if (qlog_stream_ || keylog_stream_) {
        env()->SetImmediate([qlog = qlog_stream_, keylog = keylog_stream_](Environment*) {
            if (qlog)
                qlog->End();
            if (keylog)
                keylog->End();
...
std::string Session::diagnostic_name() const
{
    ...
    return std::string("Session (") + get_type() + "," + std::to_string(env()->thread_id()) + ":" + std::to_string(static_cast<int64_t>(get_async_id())) + ")";
}
...
void Session::HandleQlog(uint32_t flags, const void* data, size_t len)
{
    ...
    env()->SetImmediate([ptr = qlog_stream_, buffer = std::move(buffer), flags](Environment*) {
        ptr->Emit(buffer.data(), buffer.size(), flags & NGTCP2_QLOG_WRITE_FLAG_FIN ? LogStream::EmitOption::FIN : LogStream::EmitOption::NONE);
    });
}
...
bool Session::Receive(Store&& store, const SocketAddress& local_address, const SocketAddress& remote_address)
{
    ...
    Debug(this, "Session is receiving %zu-byte packet received along path %s", vec.len, path);
    ...
    int err = ngtcp2_conn_read_pkt(*this, path, nullptr, vec.base, vec.len, uv_hrtime());
...
void Session::Send(const BaseObjectPtr<Packet>& packet, const PathStorage& path)
{
    UpdatePath(path);
    Send(packet);
}
...
uint64_t Session::SendDatagram(Store&& data)
{
    ...
    for (;;) {
        ...
        if (!packet) {
            packet = Packet::Create(env(), endpoint(), impl_->remote_address_, ngtcp2_conn_get_max_tx_udp_payload_size(*this), "datagram");
        ...
        ssize_t nwrite = ngtcp2_conn_writev_datagram(*this, &path.path, nullptr, pos, packet->length(), &accepted, flags, did, &vec, 1, uv_hrtime());
...
{
    CHECK(!is_destroyed());
    impl_->remote_address_.Update(storage.path.remote.addr, storage.path.remote.addrlen);
    impl_->local_address_.Update(storage.path.local.addr, storage.path.local.addrlen);
    Debug(this, "path updated. local %s, remote %s", impl_->local_address_, impl_->remote_address_);
}
...
BaseObjectPtr<Stream> Session::FindStream(int64_t id) const
{
    ...
    if (it == std::end(impl_->streams_))
        return {};
    return it->second;
}

BaseObjectPtr<Stream> Session::CreateStream(int64_t id, CreateStreamOption option, std::shared_ptr<DataQueue> data_source)
{
...
MaybeLocal<Object> Session::OpenStream(Direction direction, std::shared_ptr<DataQueue> data_source)
{
    ...
    case 0: {
        ...
        CHECK_GE(id, 0);
        if (auto stream = CreateStream(id, CreateStreamOption::DO_NOT_NOTIFY, std::move(data_source)))
            [[likely]]
            {
                return stream->object();
            }
...
void Session::AddStream(BaseObjectPtr<Stream> stream, CreateStreamOption option)
{
    CHECK(!is_destroyed());
    CHECK(stream);
...
void Session::RemoveStream(int64_t id)
{
    ...
    if (!is_in_draining_period() && !is_in_closing_period() && !ngtcp2_conn_is_local_stream(*this, id)) {
...
void Session::ShutdownStream(int64_t id, QuicError error)
{
    ...
    SendPendingDataScope send_scope(this);
    ngtcp2_conn_shutdown_stream(*this, 0, id, error.type() == QuicError::Type::APPLICATION ? error.code() : NGTCP2_APP_NOERROR);
}
...
void Session::ShutdownStreamWrite(int64_t id, QuicError code)
{
    ...
    SendPendingDataScope send_scope(this);
    ngtcp2_conn_shutdown_stream_write(*this, 0, id, code.type() == QuicError::Type::APPLICATION ? code.code() : NGTCP2_APP_NOERROR);
}
...
SessionTicket::AppData::Status Session::ExtractSessionTicketAppData(const SessionTicket::AppData& app_data, Flag flag)
{
    CHECK(!is_destroyed());
...
bool Session::can_send_packets() const
{
    ...
    // endpoint is set.
    return !is_destroyed() && !NgTcp2CallbackScope::in_ngtcp2_callback(env()) && !is_in_draining_period() && !is_in_closing_period();
}

bool Session::can_create_streams() const
{
    return !is_destroyed_or_closing() && !is_in_closing_period() && !is_in_draining_period();
}
...
{
    CHECK(!is_destroyed());
    return ngtcp2_conn_get_local_transport_params(*this)->initial_max_streams_bidi;
}
...
void Session::UpdateDataStats()
{
    ...
    STAT_SET(Stats, max_bytes_in_flight, std::max(STAT_GET(Stats, max_bytes_in_flight), info.bytes_in_flight));
}
...
void Session::SendConnectionClose()
{
    ...
    if (is_server()) {
        if (auto packet = Packet::CreateConnectionClosePacket(env(), endpoint(), impl_->remote_address_, *this, impl_->last_error_))
            [[likely]]
            {
                return Send(packet);
            }
    ...
    auto packet = Packet::Create(env(), endpoint(), impl_->remote_address_, kDefaultMaxPacketLength, "immediate connection close (client)");
    if (!packet)
        [[unlikely]]
        {
            return ErrorAndSilentClose();
    ...
    ssize_t nwrite = ngtcp2_conn_write_connection_close(*this, &path, nullptr, vec.base, vec.len, impl_->last_error_, uv_hrtime());
...
void Session::DatagramReceived(const uint8_t* data, size_t datalen, DatagramReceivedFlags flag)
{
    DCHECK(!is_destroyed());
    ...
    if (impl_->state_->datagram == 0 || datalen == 0)
        return;

    Debug(this, "Session is receiving datagram of size %zu", datalen);
    auto& stats_ = impl_->stats_;
    STAT_INCREMENT(Stats, datagrams_received);
    auto backing = ArrayBuffer::NewBackingStore(env()->isolate(), datalen, BackingStoreInitializationMode::kUninitialized);
    memcpy(backing->Data(), data, datalen);
    EmitDatagram(Store(std::move(backing), datalen), flag);
}

void Session::GenerateNewConnectionId(ngtcp2_cid* cid, size_t len, uint8_t* token)
{
    DCHECK(!is_destroyed());
    CID cid_ = impl_->config_.options.cid_factory->GenerateInto(cid, len);
    Debug(this, "Generated new connection id %s", cid_);
    StatelessResetToken new_token(token, endpoint().options().reset_token_secret, cid_);
...
{
    if (options().preferred_address_strategy == PreferredAddress::Policy::IGNORE_PREFERRED) {
    ...
    case AF_INET: {
        ...
        if (ipv4.has_value()) {
            if (ipv4->address.empty() || ipv4->port == 0)
                return;
            CHECK(SocketAddress::New(AF_INET, std::string(ipv4->address).c_str(), ipv4->port, &impl_->remote_address_));
    ...
    case AF_INET6: {
        ...
        if (ipv6.has_value()) {
            if (ipv6->address.empty() || ipv6->port == 0)
                return;
            CHECK(SocketAddress::New(AF_INET, std::string(ipv6->address).c_str(), ipv6->port, &impl_->remote_address_));
...
void Session::ProcessPendingBidiStreams()
{
    ...
    while (!impl_->pending_bidi_stream_queue_.IsEmpty()) {
        ...
        default: {
            ...
            impl_->pending_bidi_stream_queue_.PopFront()->reject(QuicError::ForTransport(NGTCP2_STREAM_LIMIT_ERROR));
            continue;
        }
        }
    }
...
void Session::ProcessPendingUniStreams()
{
    ...
    while (!impl_->pending_uni_stream_queue_.IsEmpty()) {
        ...
        default: {
            // We failed to open the stream for some reason other than being
            // blocked. Report the failure.
            impl_->pending_uni_stream_queue_.PopFront()->reject(QuicError::ForTransport(NGTCP2_STREAM_LIMIT_ERROR));
            continue;
        }
        }
    }
...
void Session::EmitClose(const QuicError& error)
{
    ...
    if (error.reason().length() > 0 && !ToV8Value(env()->context(), error.reason()).ToLocal(&argv[2])) {
        return;
    }

    MakeCallback(BindingData::Get(env()).session_close_callback(), arraysize(argv), argv);
...
void Session::EmitDatagram(Store&& datagram, DatagramReceivedFlags flag)
{
    ...
    Local<Value> argv[] = { datagram.ToUint8Array(env()), Boolean::New(env()->isolate(), flag.early) };

    MakeCallback(BindingData::Get(env()).session_datagram_callback(), arraysize(argv), argv);
}
...
void Session::EmitDatagramStatus(uint64_t id, quic::DatagramStatus status)
{
    ...
    Local<Value> argv[] = { BigInt::NewFromUnsigned(env()->isolate(), id), status_to_string };
...
void Session::EmitHandshakeComplete()
{
    ...
    if (peerVerifyError.has_value()
        && (!peerVerifyError->reason.ToLocal(&argv[kValidationErrorReason]) || !peerVerifyError->code.ToLocal(&argv[kValidationErrorCode]))) {
        return;
    }

    if (!ToV8Value(env()->context(), tls.servername()).ToLocal(&argv[kServerName]) || !ToV8Value(env()->context(), tls.protocol()).ToLocal(&argv[kSelectedAlpn])
        || !tls.cipher_name(env()).ToLocal(&argv[kCipherName]) || !tls.cipher_version(env()).ToLocal(&argv[kCipherVersion])) {
        return;
    }

    MakeCallback(BindingData::Get(env()).session_handshake_callback(), arraysize(argv), argv);
}

void Session::EmitPathValidation(
    PathValidationResult result, PathValidationFlags flags, const ValidatedPath& newPath, const std::optional<ValidatedPath>& oldPath)
{
    DCHECK(!is_destroyed());
    ...
    if (impl_->state_->path_validation == 0)
        [[likely]]
        {
            return;
        }
    ...
    Local<Value> argv[] = { resultToString, SocketAddressBase::Create(env(), newPath.local)->object(),
        SocketAddressBase::Create(env(), newPath.remote)->object(), Undefined(isolate), Undefined(isolate), Boolean::New(isolate, flags.preferredAddress) };
...
void Session::EmitSessionTicket(Store&& ticket)
{
    ...
    // emitting.
    if (impl_->state_->session_ticket == 0)
        [[likely]]
        {
...
void Session::EmitStream(const BaseObjectWeakPtr<Stream>& stream)
{
    ...
    Local<Value> argv[] = {
        stream->object(),
        Integer::NewFromUnsigned(isolate, static_cast<uint32_t>(stream->direction())),
    };

    MakeCallback(BindingData::Get(env()).stream_created_callback(), arraysize(argv), argv);
}

void Session::EmitVersionNegotiation(const ngtcp2_pkt_hd& hd, const uint32_t* sv, size_t nsv)
{
    DCHECK(!is_destroyed());
    DCHECK(!is_server());
    ...
    Local<Value> supported[] = { Integer::NewFromUnsigned(env()->isolate(), opts.min_version), Integer::NewFromUnsigned(env()->isolate(), opts.version) };
    ...
    MakeCallback(BindingData::Get(env()).session_version_negotiation_callback(), arraysize(argv), argv);
}
...
#undef V
    ...
    static constexpr auto STREAM_DIRECTION_BIDIRECTIONAL = static_cast<uint32_t>(Direction::BIDIRECTIONAL);
    static constexpr auto STREAM_DIRECTION_UNIDIRECTIONAL = static_cast<uint32_t>(Direction::UNIDIRECTIONAL);
    ...
    NODE_DEFINE_STRING_CONSTANT(target, "DEFAULT_CIPHERS", TLSContext::DEFAULT_CIPHERS);
    NODE_DEFINE_STRING_CONSTANT(target, "DEFAULT_GROUPS", TLSContext::DEFAULT_GROUPS);

#define V(name, _) IDX_STATS_SESSION_##name,
    enum SessionStatsIdx { SESSION_STATS(V) IDX_STATS_SESSION_COUNT };
#undef V

#define V(name, key, __) auto IDX_STATE_SESSION_##name = offsetof(Session::State, key);
    SESSION_STATE(V)
#undef V
