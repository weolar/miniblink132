...
public:
    PendingStream(Direction direction, Stream* stream, BaseObjectWeakPtr<Session> session);
...
public:
    using PendingStreamQueue = ListHead<PendingStream, &PendingStream::pending_stream_queue_>;
};
...
// zero-length output.
class Stream final : public AsyncWrap, public Ngtcp2Source, public DataQueue::BackpressureListener {
public:
    using Header = NgHeaderBase<BindingData>;

    static v8::Maybe<std::shared_ptr<DataQueue>> GetDataQueueFromSource(Environment* env, v8::Local<v8::Value> value);
    ...
    static void InitPerIsolate(IsolateData* data, v8::Local<v8::ObjectTemplate> target);
    static void InitPerContext(Realm* realm, v8::Local<v8::Object> target);
    static void RegisterExternalReferences(ExternalReferenceRegistry* registry);

    // Creates a new non-pending stream.
    static BaseObjectPtr<Stream> Create(Session* session, int64_t id, std::shared_ptr<DataQueue> source = nullptr);

    // Creates a new pending stream.
    static BaseObjectPtr<Stream> Create(Session* session, Direction direction, std::shared_ptr<DataQueue> source = nullptr);

    // The constructor is only public to be visible by MakeDetachedBaseObject.
    // Call Create to create new instances of Stream.
    Stream(BaseObjectWeakPtr<Session> session, v8::Local<v8::Object> obj, int64_t id, std::shared_ptr<DataQueue> source);
    ...
    // instances of Stream.
    Stream(BaseObjectWeakPtr<Session> session, v8::Local<v8::Object> obj, Direction direction, std::shared_ptr<DataQueue> source);
    ...
    int DoPull(bob::Next<ngtcp2_vec> next, int options, ngtcp2_vec* data, size_t count, size_t max_count_hint) override;
...
private:
    ...
    void EnqueuePendingHeaders(HeadersKind kind, v8::Local<v8::Array> headers, HeadersFlags flags);
    ...
    std::optional<std::unique_ptr<PendingStream>> maybe_pending_stream_ = std::nullopt;
