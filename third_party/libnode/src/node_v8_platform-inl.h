...
public:
    inline void OnTraceEnabled() override
    {
        ...
        if (!title.empty()) {
            ...
            TRACE_EVENT_METADATA1("__metadata", "process_name", "name", TRACE_STR_COPY(title.c_str()));
        }
        TRACE_EVENT_METADATA1("__metadata", "version", "node", per_process::metadata.versions.node.c_str());
        TRACE_EVENT_METADATA1("__metadata", "thread_name", "name", "JavaScriptMainThread");

        auto trace_process = tracing::TracedValue::Create();
        trace_process->BeginDictionary("versions");

#define V(key) trace_process->SetString(#key, per_process::metadata.versions.key.c_str());
...
struct V8Platform {
    ...
    {
        constexpr auto convert_to_set = [](auto& categories) -> std::set<std::string> {
        ...
        if (tracing_file_writer_.IsDefaultHandle()) {
            ...
            tracing_file_writer_ = tracing_agent_->AddClient(convert_to_set(categories),
                std::unique_ptr<tracing::AsyncTraceWriter>(new tracing::NodeTraceWriter(per_process::cli_options->trace_event_file_pattern)),
