...
public:
    ...
    static void SetTreatArrayBufferViewsAsHostObjects(const FunctionCallbackInfo<Value>& args);
...
class DeserializerContext : public BaseObject, public ValueDeserializer::Delegate {
public:
...
void SerializerContext::ThrowDataCloneError(Local<String> message)
{
    ...
    if (!object()->Get(env()->context(), env()->get_data_clone_error_string()).ToLocal(&get_data_clone_error)) {
    ...
    Local<Value> error;
    if (get_data_clone_error.As<Function>()->Call(env()->context(), object(), arraysize(args), args).ToLocal(&error)) {
        env()->isolate()->ThrowException(error);
    }
}

Maybe<uint32_t> SerializerContext::GetSharedArrayBufferId(Isolate* isolate, Local<SharedArrayBuffer> shared_array_buffer)
{
    Local<Value> args[1] = { shared_array_buffer };
    Local<Value> get_shared_array_buffer_id;
    if (!object()->Get(env()->context(), env()->get_shared_array_buffer_id_string()).ToLocal(&get_shared_array_buffer_id)) {
        return Nothing<uint32_t>();
    }

    if (!get_shared_array_buffer_id->IsFunction()) {
        return ValueSerializer::Delegate::GetSharedArrayBufferId(isolate, shared_array_buffer);
    }

    Local<Value> id;
    if (!get_shared_array_buffer_id.As<Function>()->Call(env()->context(), object(), arraysize(args), args).ToLocal(&id)) {
        return Nothing<uint32_t>();
    }
...
Maybe<bool> SerializerContext::WriteHostObject(Isolate* isolate, Local<Object> input)
{
    Local<Value> args[1] = { input };

    Local<Value> write_host_object;
    if (!object()->Get(env()->context(), env()->write_host_object_string()).ToLocal(&write_host_object)) {
        return Nothing<bool>();
    }
    ...
    Local<Value> ret;
    if (!write_host_object.As<Function>()->Call(env()->context(), object(), arraysize(args), args).ToLocal(&ret)) {
        return Nothing<bool>();
    }
...
void SerializerContext::SetTreatArrayBufferViewsAsHostObjects(const FunctionCallbackInfo<Value>& args)
{
    SerializerContext* ctx;
    ASSIGN_OR_RETURN_UNWRAP(&ctx, args.This());
...
void SerializerContext::ReleaseBuffer(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Object> buf;
    if (Buffer::New(ctx->env(), reinterpret_cast<char*>(ret.first), ret.second).ToLocal(&buf)) {
        args.GetReturnValue().Set(buf);
    }
}

void SerializerContext::TransferArrayBuffer(const FunctionCallbackInfo<Value>& args)
{
    SerializerContext* ctx;
    ASSIGN_OR_RETURN_UNWRAP(&ctx, args.This());
    ...
    if (!args[1]->IsArrayBuffer()) {
        return node::THROW_ERR_INVALID_ARG_TYPE(ctx->env(), "arrayBuffer must be an ArrayBuffer");
    }
...
void SerializerContext::WriteUint64(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->Uint32Value(ctx->env()->context()).To(&hi) || !args[1]->Uint32Value(ctx->env()->context()).To(&lo)) {
        return;
    }
...
void SerializerContext::WriteRawBytes(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!args[0]->IsArrayBufferView()) {
        return node::THROW_ERR_INVALID_ARG_TYPE(ctx->env(), "source must be a TypedArray or a DataView");
    }
...
DeserializerContext::DeserializerContext(Environment* env, Local<Object> wrap, Local<Value> buffer)
    : BaseObject(env, wrap)
...
MaybeLocal<Object> DeserializerContext::ReadHostObject(Isolate* isolate)
{
    ...
    if (!object()->Get(env()->context(), env()->read_host_object_string()).ToLocal(&read_host_object)) {
        return {};
    }
    ...
    Local<Value> ret;
    if (!read_host_object.As<Function>()->Call(env()->context(), object(), 0, nullptr).ToLocal(&ret)) {
        return {};
    }
...
void DeserializerContext::New(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    if (!args.IsConstructCall()) {
        return THROW_ERR_CONSTRUCT_CALL_REQUIRED(env, "Class constructor Deserializer cannot be invoked without 'new'");
    }

    if (!args[0]->IsArrayBufferView()) {
        return node::THROW_ERR_INVALID_ARG_TYPE(env, "buffer must be a TypedArray or a DataView");
    }
...
void DeserializerContext::TransferArrayBuffer(const FunctionCallbackInfo<Value>& args)
{
    DeserializerContext* ctx;
    ASSIGN_OR_RETURN_UNWRAP(&ctx, args.This());
    ...
    return node::THROW_ERR_INVALID_ARG_TYPE(ctx->env(), "arrayBuffer must be an ArrayBuffer or SharedArrayBuffer");
}

void DeserializerContext::GetWireFormatVersion(const FunctionCallbackInfo<Value>& args)
{
    DeserializerContext* ctx;
    ASSIGN_OR_RETURN_UNWRAP(&ctx, args.This());
...
void DeserializerContext::ReadUint32(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!ok)
        return ctx->env()->ThrowError("ReadUint32() failed");
...
void DeserializerContext::ReadUint64(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!ok)
        return ctx->env()->ThrowError("ReadUint64() failed");
    ...
    Local<Value> ret[] = { Integer::NewFromUnsigned(isolate, hi), Integer::NewFromUnsigned(isolate, lo) };
...
void DeserializerContext::ReadDouble(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (!ok)
        return ctx->env()->ThrowError("ReadDouble() failed");
    return args.GetReturnValue().Set(value);
}

void DeserializerContext::ReadRawBytes(const FunctionCallbackInfo<Value>& args)
{
    DeserializerContext* ctx;
    ASSIGN_OR_RETURN_UNWRAP(&ctx, args.This());
    ...
    if (!ok)
        return ctx->env()->ThrowError("ReadRawBytes() failed");
...
void Initialize(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    ...
    Local<FunctionTemplate> ser = NewFunctionTemplate(isolate, SerializerContext::New);

    ser->InstanceTemplate()->SetInternalFieldCount(SerializerContext::kInternalFieldCount);
    ...
    SetProtoMethod(isolate, ser, "releaseBuffer", SerializerContext::ReleaseBuffer);
    SetProtoMethod(isolate, ser, "transferArrayBuffer", SerializerContext::TransferArrayBuffer);
    ...
    SetProtoMethod(isolate, ser, "writeRawBytes", SerializerContext::WriteRawBytes);
    SetProtoMethod(isolate, ser, "_setTreatArrayBufferViewsAsHostObjects", SerializerContext::SetTreatArrayBufferViewsAsHostObjects);

    ser->ReadOnlyPrototype();
    SetConstructorFunction(context, target, "Serializer", ser);

    Local<FunctionTemplate> des = NewFunctionTemplate(isolate, DeserializerContext::New);

    des->InstanceTemplate()->SetInternalFieldCount(DeserializerContext::kInternalFieldCount);
    ...
    SetProtoMethod(isolate, des, "getWireFormatVersion", DeserializerContext::GetWireFormatVersion);
    SetProtoMethod(isolate, des, "transferArrayBuffer", DeserializerContext::TransferArrayBuffer);
    ...
    SetProtoMethod(isolate, des, "_readRawBytes", DeserializerContext::ReadRawBytes);
...
NODE_BINDING_EXTERNAL_REFERENCE(serdes, node::serdes::RegisterExternalReferences)
