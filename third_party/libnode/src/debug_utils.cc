...
#if defined(__linux__) && !defined(__GLIBC__) || defined(__UCLIBC__) || defined(_AIX)
...
#if defined(__linux__) || defined(__sun) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__DragonFly__)
#include <link.h>
#endif
...
public:
    ...
    SymbolInfo LookupSymbol(void* address) override
    {
        ...
        if (info.dli_sname != nullptr) {
            if (char* demangled = abi::__cxa_demangle(info.dli_sname, nullptr, nullptr, nullptr)) {
                ret.name = demangled;
                free(demangled);
            } else {
    ...
    {
        void* page_aligned = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(address) & ~(pagesize_ - 1));
...
std::unique_ptr<NativeSymbolDebuggingContext> NativeSymbolDebuggingContext::New()
{
...
public:
    ...
    SymbolInfo WrappedGetLine(DWORD64 dwAddress) const
    {
        ...
        if (SymGetLineFromAddr64(current_process_, dwAddress, &dwDisplacement, &line)) {
    ...
    std::string WrappedUnDecorateSymbolName(const char* name) const
    {
        ...
        if (UnDecorateSymbolName(name, szUndName, sizeof(szUndName), UNDNAME_COMPLETE)) {
    ...
    Win32SymbolDebuggingContext operator=(const Win32SymbolDebuggingContext&) = delete;
...
std::unique_ptr<NativeSymbolDebuggingContext> NativeSymbolDebuggingContext::New()
{
    return std::unique_ptr<NativeSymbolDebuggingContext>(new Win32SymbolDebuggingContext());
}
...
void PrintLibuvHandleInformation(uv_loop_t* loop, FILE* stream)
{
    ...
    uv_walk(
        ...
        [](uv_handle_t* handle, void* arg) {
            ...
            fprintf(stream, "[%p] %s%s\n", handle, uv_handle_type_name(handle->type), uv_is_active(handle) ? " (active)" : "");
...
std::vector<std::string> NativeSymbolDebuggingContext::GetLoadedLibraries()
{
...
#elif __APPLE__
    uint32_t i = 0;
    for (const char* name = _dyld_get_image_name(i); name != nullptr; name = _dyld_get_image_name(++i)) {
    ...
    if (rc == 0) {
        ...
        do {
            ...
            char* member_name = cur_info->ldinfo_filename + strlen(cur_info->ldinfo_filename) + 1;
            if (*member_name != '\0') {
    ...
    if (EnumProcessModules(process_handle, nullptr, 0, &size_1)) {
        ...
        if (EnumProcessModules(process_handle, modules.data, size_1, &size_2)) {
            for (DWORD i = 0; i < (size_1 / sizeof(HMODULE)) && i < (size_2 / sizeof(HMODULE)); i++) {
                WCHAR module_name[MAX_PATH];
                // Obtain and report the full pathname for each module
                if (GetModuleFileNameExW(process_handle, modules.data[i], module_name, arraysize(module_name) / sizeof(WCHAR))) {
                    DWORD size = WideCharToMultiByte(CP_UTF8, 0, module_name, -1, nullptr, 0, nullptr, nullptr);
                    char* str = new char[size];
                    WideCharToMultiByte(CP_UTF8, 0, module_name, -1, str, size, nullptr, nullptr);
...
#ifdef _WIN32
    HANDLE handle = GetStdHandle(file == stdout ? STD_OUTPUT_HANDLE : STD_ERROR_HANDLE);

    // Check if stderr is something other than a tty/console
    if (handle == INVALID_HANDLE_VALUE || handle == nullptr || uv_guess_handle(_fileno(file)) != UV_TTY) {
        simple_fwrite();
        return;
    }
