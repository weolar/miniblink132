...
{
#define V(PropertyName, TypeName) tracker->TrackField(#PropertyName, PropertyName());
...
void Realm::CreateProperties()
{
    ...
    Local<Object> per_context_bindings = GetPerContextExports(ctx).ToLocalChecked();
    Local<Value> primordials = per_context_bindings->Get(ctx, env_->primordials_string()).ToLocalChecked();
...
#define V(EnvPropertyName, PrimordialsPropertyName)                                                                                                            \
    {                                                                                                                                                          \
        Local<Value> ctor = primordials.As<Object>()->Get(ctx, FIXED_ONE_BYTE_STRING(isolate(), PrimordialsPropertyName)).ToLocalChecked();                    \
        CHECK(ctor->IsObject());                                                                                                                               \
        Local<Value> prototype = ctor.As<Object>()->Get(ctx, prototype_string).ToLocalChecked();                                                               \
...
RealmSerializeInfo Realm::Serialize(SnapshotCreator* creator)
{
    ...
    // builder.
    info.builtins = std::vector<std::string>(builtins_without_cache.begin(), builtins_without_cache.end());
...
#define V(PropertyName, TypeName)                                                                                                                              \
    do {                                                                                                                                                       \
        if (values.size() > i && id == values[i].id) {                                                                                                         \
            ...
            MaybeLocal<TypeName> maybe_field = ctx->GetDataFromSnapshotOnce<TypeName>(d.index);                                                                \
            Local<TypeName> field;                                                                                                                             \
            if (!maybe_field.ToLocal(&field)) {                                                                                                                \
                fprintf(stderr, "Failed to deserialize realm value " #PropertyName "\n");                                                                      \
            }                                                                                                                                                  \
...
#undef V
    ...
    MaybeLocal<Context> maybe_ctx_from_snapshot = ctx->GetDataFromSnapshotOnce<Context>(info->context);
    Local<Context> ctx_from_snapshot;
    if (!maybe_ctx_from_snapshot.ToLocal(&ctx_from_snapshot)) {
        fprintf(stderr, "Failed to deserialize context back reference from the snapshot\n");
    }
...
void Realm::PrintInfoForSnapshot()
{
    ...
    size_t i = 0;
    ForEachBaseObject([&](BaseObject* obj) { std::cerr << "#" << i++ << " " << obj << ": " << obj->MemoryInfoName() << "\n"; });
...
void Realm::VerifyNoStrongBaseObjects()
{
    ...
    if (!env()->options()->verify_base_objects)
        return;

    ForEachBaseObject([](BaseObject* obj) {
        if (obj->IsNotIndicativeOfMemoryLeakAtExit())
            return;
...
#define V(PropertyName, TypeName)                                                                                                                              \
    ...
    {                                                                                                                                                          \
        DCHECK_IMPLIES(!value.IsEmpty(), isolate()->GetCurrentContext() == context());                                                                         \
        PropertyName##_.Reset(isolate(), value);                                                                                                               \
    }
PER_REALM_STRONG_PERSISTENT_VALUES(V)
#undef V

PrincipalRealm::PrincipalRealm(Environment* env, v8::Local<v8::Context> context, const RealmSerializeInfo* realm_info)
...
MaybeLocal<Value> PrincipalRealm::BootstrapRealm()
{
    ...
    if (!env_->no_browser_globals()) {
        if (ExecuteBootstrapper("internal/bootstrap/web/exposed-wildcard").IsEmpty()
            || ExecuteBootstrapper("internal/bootstrap/web/exposed-window-or-worker").IsEmpty()) {
            return MaybeLocal<Value>();
        }
    }

    // TODO(joyeecheung): skip these in the snapshot building for workers.
    auto thread_switch_id = env_->is_main_thread() ? "internal/bootstrap/switches/is_main_thread" : "internal/bootstrap/switches/is_not_main_thread";
    if (ExecuteBootstrapper(thread_switch_id).IsEmpty()) {
        return MaybeLocal<Value>();
    }

    auto process_state_switch_id
        = env_->owns_process_state() ? "internal/bootstrap/switches/does_own_process_state" : "internal/bootstrap/switches/does_not_own_process_state";
    ...
    if (!isolate_data()->env_proxy_template()->NewInstance(context()).ToLocal(&env_proxy)
