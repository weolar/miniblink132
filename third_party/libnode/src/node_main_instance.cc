...
NodeMainInstance::NodeMainInstance(const SnapshotData* snapshot_data, uv_loop_t* event_loop, MultiIsolatePlatform* platform,
    ...
    , snapshot_data_(snapshot_data)
{
    ...
    isolate_data_.reset(CreateIsolateData(isolate_, event_loop, platform, array_buffer_allocator_.get(), snapshot_data->AsEmbedderWrapper().get()));
...
NodeMainInstance::~NodeMainInstance()
{
...

    {
        ...
        // use-after-free.
        Isolate::DisallowJavascriptExecutionScope disallow_js(isolate_, Isolate::DisallowJavascriptExecutionScope::CRASH_ON_FAILURE);
#endif
...
DeleteFnPtr<Environment, FreeEnvironment> NodeMainInstance::CreateMainEnvironment(ExitCode* exit_code)
{
    *exit_code = ExitCode::kNoFailure; // Reset the exit code to 0
    ...
    if (snapshot_data_ != nullptr) {
        ...
        env.reset(CreateEnvironment(isolate_data_.get(), context, args_, exec_args_));
    }
