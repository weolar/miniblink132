...
public:
    ...
    // rules.
    template <typename T> static void Wrap(T* ptr, Environment* env, v8::Local<v8::Object> obj)
    {
        ...
        obj->SetAlignedPointerInInternalField(kEmbedderType, env->isolate_data()->embedder_id_for_cppgc());
    ...
    template <typename T> static T* Unwrap(v8::Local<v8::Object> obj)
    {
...
#define ASSIGN_OR_RETURN_UNWRAP_CPPGC(ptr, obj, ...)                                                                                                           \
    do {                                                                                                                                                       \
        *ptr = CppgcMixin::Unwrap<typename std::remove_reference<decltype(**ptr)>::type>(obj);                                                                 \
        if (*ptr == nullptr)                                                                                                                                   \
            return __VA_ARGS__;                                                                                                                                \
    } while (0)
} // namespace node
