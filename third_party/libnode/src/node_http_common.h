...
// for an example.
template <typename T> class NgHeaders {
public:
...
template <typename T> class NgRcBufPointer : public MemoryRetainer {
public:
    ...
    template <typename B> NgRcBufPointer(const NgRcBufPointer<B>& other)
    {
        reset(other.get());
    }
    ...
    template <typename B> NgRcBufPointer& operator=(const NgRcBufPointer<B>& other)
    {
        if (other.get() == get())
            return *this;
        this->~NgRcBufPointer();
    ...
    bool IsStatic() const
    {
        return T::is_static(buf_) != 0;
    }
    void SetInternalizable()
    {
        internalizable_ = true;
    }
    bool IsInternalizable() const
    {
        return internalizable_;
    }
    ...
    public:
        explicit External(const NgRcBufPointer<T>& ptr)
            : ptr_(ptr)
        {
        }
        ...
        static inline v8::MaybeLocal<v8::String> GetInternalizedString(Environment* env, const NgRcBufPointer<T>& ptr)
        {
            return v8::String::NewFromOneByte(env->isolate(), ptr.data(), v8::NewStringType::kInternalized, ptr.len());
        }

        template <typename Allocator> static v8::MaybeLocal<v8::String> New(Allocator* allocator, NgRcBufPointer<T> ptr)
        {
            ...
            if (ptr.IsStatic()) {
                ...
                if (eternal.IsEmpty()) {
                    v8::Local<v8::String> str = GetInternalizedString(env, ptr).ToLocalChecked();
            ...
            v8::MaybeLocal<v8::String> str = v8::String::NewExternalOneByte(env->isolate(), h_str);
            if (str.IsEmpty())
                delete h_str;
...
template <typename allocator_t> struct NgHeaderBase : public MemoryRetainer {
...
template <typename T> class NgHeader final : public NgHeaderBase<typename T::allocator_t> {
public:
    ...
    inline NgHeader(Environment* env, rcbuf_t* name, rcbuf_t* value, uint8_t flags);
    inline NgHeader(Environment* env, int32_t token, rcbuf_t* name, rcbuf_t* value, uint8_t flags);
    ...
    inline v8::MaybeLocal<v8::String> GetName(allocator_t* allocator) const override;
    inline v8::MaybeLocal<v8::String> GetValue(allocator_t* allocator) const override;
