    ...
    for (size_t i = 0; i <= path.size(); ++i) {
        ...
        if (IsPathSeparator(code)) {
            if (lastSlash == static_cast<int>(i - 1) || dots == 1) {
                ...
                if (!res.empty()) {
                    res += std::string(separator) + std::string(path.substr(lastSlash + 1, i - (lastSlash + 1)));
                } else {
    ...
    if (resolved_path[0] == '\\') {
        ...
        if (resolved_path[1] == '\\') {
            if (resolved_path[2] != '?' && resolved_path[2] != '.') {
                ...
                memcpy(path->out() + unc_prefix.size(), resolved_path.c_str() + 2, resolved_path.size() - 2 + 1);
                return;
            }
        }
    } else if (IsWindowsDeviceRoot(resolved_path[0]) && resolved_path[1] == ':' && resolved_path[2] == '\\') {
        ...
        memcpy(path->out() + new_prefix.size(), resolved_path.c_str(), resolved_path.size() + 1);
        return;
    }
