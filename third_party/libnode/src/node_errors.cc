...
static std::string GetSourceMapErrorSource(Isolate* isolate, Local<Context> context, Local<Message> message, bool* added_exception_line)
{
    ...
    Local<Value> argv[] = { script_resource_name, v8::Int32::New(isolate, linenum), v8::Int32::New(isolate, columnum) };
    MaybeLocal<Value> maybe_ret = env->get_source_map_error_source()->Call(context, Undefined(isolate), arraysize(argv), argv);
    Local<Value> ret;
...
static std::string GetErrorSource(Isolate* isolate, Local<Context> context, Local<Message> message, bool* added_exception_line)
{
    ...
    *added_exception_line = false;
    ...
    const bool has_source_map_url = !message->GetScriptOrigin().SourceMapUrl().IsEmpty() && !message->GetScriptOrigin().SourceMapUrl()->IsUndefined();
    if (has_source_map_url && env != nullptr && env->source_maps_enabled()) {
        std::string source = GetSourceMapErrorSource(isolate, context, message, added_exception_line);
...
MaybeLocal<StackTrace> GetCurrentStackTrace(Isolate* isolate, int frame_count)
{
    ...
    constexpr StackTrace::StackTraceOptions options
        = static_cast<StackTrace::StackTraceOptions>(StackTrace::kDetailed | StackTrace::kExposeFramesAcrossSecurityOrigins);

    is_retrieving_js_stacktrace.store(true);
    EscapableHandleScope scope(isolate);
    Local<StackTrace> stack = StackTrace::CurrentStackTrace(isolate, frame_count, options);
...
static std::string FormatStackTrace(Isolate* isolate, Local<StackTrace> stack, StackTracePrefix prefix = StackTracePrefix::kAt)
{
    std::string result;
    for (int i = 0; i < stack->GetFrameCount(); i++) {
        ...
        std::string prefix_str = prefix == StackTracePrefix::kAt ? "    at " : std::to_string(i + 1) + ": ";
        if (stack_frame->IsEval()) {
            if (stack_frame->GetScriptId() == Message::kNoScriptIdInfo) {
                result += SPrintF("%s[eval]:%i:%i\n", prefix_str, line_number, column);
            } else {
                std::vector<char> buf(script_name.length() + 64);
                snprintf(buf.data(), buf.size(), "%s[eval] (%s:%i:%i)\n", prefix_str.c_str(), *script_name, line_number, column);
        ...
        if (fn_name_s.length() == 0) {
            std::vector<char> buf(script_name.length() + 64);
            snprintf(buf.data(), buf.size(), "%s%s:%i:%i\n", prefix_str.c_str(), *script_name, line_number, column);
            result += std::string(buf.data());
        } else {
            std::vector<char> buf(fn_name_s.length() + script_name.length() + 64);
            snprintf(buf.data(), buf.size(), "%s%s (%s:%i:%i)\n", prefix_str.c_str(), *fn_name_s, *script_name, line_number, column);
            result += std::string(buf.data());
        }
    }
    return result;
...
void PrintStackTrace(Isolate* isolate, Local<StackTrace> stack, StackTracePrefix prefix)
{
...
std::string FormatCaughtException(Isolate* isolate, Local<Context> context, Local<Value> err, Local<Message> message, bool add_source_line = true)
{
    node::Utf8Value reason(isolate, err->ToDetailString(context).FromMaybe(Local<String>()));
    std::string reason_str = reason.ToString();
    return FormatErrorMessage(isolate, context, reason_str, message, add_source_line);
}

std::string FormatErrorMessage(Isolate* isolate, Local<Context> context, const std::string& reason, Local<Message> message, bool add_source_line)
{
    std::string result;
    if (add_source_line) {
    ...
    if (!stack.IsEmpty())
        result += FormatStackTrace(isolate, stack);
    return result;
}

std::string FormatCaughtException(Isolate* isolate, Local<Context> context, const v8::TryCatch& try_catch)
{
    CHECK(try_catch.HasCaught());
    return FormatCaughtException(isolate, context, try_catch.Exception(), try_catch.Message());
}

void PrintCaughtException(Isolate* isolate, Local<Context> context, const v8::TryCatch& try_catch)
{
...
void AppendExceptionLine(Environment* env, Local<Value> er, Local<Message> message, enum ErrorHandlingMode mode)
{
    ...
    if (!er.IsEmpty() && er->IsObject()) {
        ...
        auto maybe_value = err_obj->GetPrivate(env->context(), env->arrow_message_private_symbol());
        Local<Value> lvalue;
    ...
    bool added_exception_line = false;
    std::string source = GetErrorSource(env->isolate(), env->context(), message, &added_exception_line);
    if (!added_exception_line) {
        return;
    }
    ...
    CHECK(err_obj->SetPrivate(env->context(), env->arrow_message_private_symbol(), arrow_str.ToLocalChecked()).FromMaybe(false));
}
 ...
 */
static void ReportFatalException(Environment* env, Local<Value> error, Local<Message> message, EnhanceFatalException enhance_stack)
{
    ...
    if (!error->IsObject()) { // We can only enhance actual errors.
        ...
        auto enhance_with = [&](Local<Function> enhancer) {
            ...
            if (!enhancer.IsEmpty() && enhancer->Call(env->context(), Undefined(isolate), arraysize(argv), argv).ToLocal(&enhanced)) {
        ...
        case EnhanceFatalException::kDontEnhance: {
            USE(err_obj->Get(env->context(), env->stack_string()).ToLocal(&stack_trace));
            report_to_inspector();
            break;
        }
        default:
        ...
        arrow = err_obj->GetPrivate(env->context(), env->arrow_message_private_symbol()).ToLocalChecked();
    }
...
void OOMErrorHandler(const char* location, const v8::OOMDetails& details)
{
    ...
    const char* message = details.is_heap_oom ? "Allocation failed - JavaScript heap out of memory" : "Allocation failed - process out of memory";
    if (location) {
        FPrintF(stderr, "FATAL ERROR: %s %s\n", location, message);
    } else {
        FPrintF(stderr, "FATAL ERROR: %s\n", message);
...
v8::ModifyCodeGenerationFromStringsResult ModifyCodeGenerationFromStrings(v8::Local<v8::Context> context, v8::Local<v8::Value> source, bool is_code_like)
{
    HandleScope scope(context->GetIsolate());

    if (context->GetNumberOfEmbedderDataFields() <= ContextEmbedderIndex::kAllowCodeGenerationFromStrings) {
    ...
    if (env->source_maps_enabled() && env->can_call_into_js()) {
        ...
        Local<Function> maybe_cache_source_map = env->maybe_cache_generated_source_map();
        Local<Value> argv[1] = { source };

        MaybeLocal<Value> maybe_cached = maybe_cache_source_map->Call(context, context->Global(), arraysize(argv), argv);
        if (maybe_cached.IsEmpty()) {
            DCHECK(try_catch.HasCaught());
        }
    }

    Local<Value> allow_code_gen = context->GetEmbedderData(ContextEmbedderIndex::kAllowCodeGenerationFromStrings);
    bool codegen_allowed = allow_code_gen->IsUndefined() || allow_code_gen->IsTrue();
    return {
        codegen_allowed,
        {},
    };
...
TryCatchScope::~TryCatchScope()
{
    if (HasCaught() && !HasTerminated() && mode_ == CatchMode::kFatal) {
        ...
        EnhanceFatalException enhance = CanContinue() ? EnhanceFatalException::kEnhance : EnhanceFatalException::kDontEnhance;
        if (message.IsEmpty())
...
static void SetGetSourceMapErrorSource(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    CHECK(args[0]->IsFunction());
...
static void SetMaybeCacheGeneratedSourceMap(const FunctionCallbackInfo<Value>& args)
{
    Environment* env = Environment::GetCurrent(args);
    CHECK(args[0]->IsFunction());
...
static void SetEnhanceStackForFatalException(const FunctionCallbackInfo<Value>& args)
{
    Realm* realm = Realm::GetCurrent(args);
    CHECK(args[0]->IsFunction());
...
static void TriggerUncaughtException(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (env != nullptr && env->abort_on_uncaught_exception()) {
        ReportFatalException(env, exception, message, EnhanceFatalException::kEnhance);
        ABORT();
    }
...
{
    SetMethod(context, target, "setPrepareStackTraceCallback", SetPrepareStackTraceCallback);
    SetMethod(context, target, "setGetSourceMapErrorSource", SetGetSourceMapErrorSource);
    SetMethod(context, target, "setSourceMapsEnabled", SetSourceMapsEnabled);
    SetMethod(context, target, "setMaybeCacheGeneratedSourceMap", SetMaybeCacheGeneratedSourceMap);
    SetMethod(context, target, "setEnhanceStackForFatalException", SetEnhanceStackForFatalException);
    SetMethodNoSideEffect(context, target, "noSideEffectsToString", NoSideEffectsToString);
    SetMethod(context, target, "triggerUncaughtException", TriggerUncaughtException);
...
void DecorateErrorStack(Environment* env, const errors::TryCatchScope& try_catch)
{
    DecorateErrorStack(env, try_catch.Exception(), try_catch.Message());
}

void DecorateErrorStack(Environment* env, Local<Value> exception, Local<Message> message)
{
    ...
    MaybeLocal<Value> maybe_value = err_obj->GetPrivate(env->context(), env->arrow_message_private_symbol());
    ...
    Local<String> decorated_stack = String::Concat(
        env->isolate(), String::Concat(env->isolate(), arrow.As<String>(), FIXED_ONE_BYTE_STRING(env->isolate(), "\n")), stack.ToLocalChecked().As<String>());
    USE(err_obj->Set(env->context(), env->stack_string(), decorated_stack));
    err_obj->SetPrivate(env->context(), env->decorated_private_symbol(), True(env->isolate()));
}

void TriggerUncaughtException(Isolate* isolate, Local<Value> error, Local<Message> message, bool from_promise)
{
    CHECK(!error.IsEmpty());
    HandleScope scope(isolate);
    ...
    if (env == nullptr) {
        ...
        PrintToStderrAndFlush(FormatCaughtException(isolate, context, error, message));
        ABORT();
    }
    ...
    if (!process_object->Get(env->context(), fatal_exception_string).ToLocal(&fatal_exception_function)) {
    ...
    if (!fatal_exception_function->IsFunction()) {
        ReportFatalException(env, error, message, EnhanceFatalException::kDontEnhance);
    ...
    if (env->can_call_into_js()) {
        ...
        errors::TryCatchScope try_catch(env, errors::TryCatchScope::CatchMode::kFatal);
        ...
        maybe_handled = fatal_exception_function.As<Function>()->Call(env->context(), process_object, arraysize(argv), argv);
    }
...
void TriggerUncaughtException(Isolate* isolate, const v8::TryCatch& try_catch)
{
    ...
    CHECK(try_catch.HasCaught());
    HandleScope scope(isolate);
    TriggerUncaughtException(isolate, try_catch.Exception(), try_catch.Message(), false /* from_promise */);
}
...
PrinterTryCatch::~PrinterTryCatch()
{
    ...
    std::string str = FormatCaughtException(isolate_, isolate_->GetCurrentContext(), Exception(), Message(), print_source_line_ == kPrintSourceLine);
...
NODE_BINDING_EXTERNAL_REFERENCE(errors, node::errors::RegisterExternalReferences)
