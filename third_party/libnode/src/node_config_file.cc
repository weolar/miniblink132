...
std::optional<std::string_view> ConfigReader::GetDataFromArgs(const std::vector<std::string>& args)
{
    constexpr std::string_view flag_path = "--experimental-config-file";
    constexpr std::string_view default_file = "--experimental-default-config-file";
...
ParseResult ConfigReader::ParseNodeOptions(simdjson::ondemand::object* node_options_object)
{
    ...
    for (auto field : *node_options_object) {
        ...
        if (it != env_options_map.end()) {
            ...
            case options_parser::OptionType::kBoolean: {
                ...
                node_options_.push_back(it->first + "=" + (result ? "true" : "false"));
                break;
            }
            ...
            case options_parser::OptionType::kNoOp: {
                FPrintF(stderr, "No-op flag %s is currently not supported\n", it->first.c_str());
                return ParseResult::InvalidContent;
                break;
            }
...
ParseResult ConfigReader::ParseConfig(const std::string_view& config_path)
{
    ...
    if (r != 0) {
        ...
        FPrintF(stderr, "Cannot read configuration from %s: %s\n", config_path, err);
    ...
    if (auto node_options_error = main_object["nodeOptions"].get_object().get(node_options_object)) {
        if (node_options_error != simdjson::error_code::NO_SUCH_FIELD) {
            FPrintF(stderr, "\"nodeOptions\" value unexpected for %s\n\n", config_path.data());
            return ParseResult::InvalidContent;
        }
    } else {
