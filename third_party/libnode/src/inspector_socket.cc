...
private:
    ...
    {
        return node::ContainerOf(&TcpHolder::tcp_, reinterpret_cast<uv_tcp_t*>(handle));
    }
...
static void generate_accept_string(const std::string& client_key, char (*buffer)[ACCEPT_KEY_LENGTH])
{
    ...
    CHECK(ACCEPT_KEY_LENGTH >= nbytes::Base64EncodedSize(SHA_DIGEST_LENGTH) && "not enough space provided for base64 encode");
    USE(SHA1(reinterpret_cast<const unsigned char*>(input.data()), input.size(), reinterpret_cast<unsigned char*>(hash)));
...
static bool IsIPAddress(const std::string& host)
{
    ...
    if (host.length() >= 4 && host.front() == '[' && host.back() == ']') {
        ...
        if (uv_inet_pton(AF_INET6, ipv6_str, ipv6) != 0)
            return false;
...
static ws_decode_result decode_frame_hybi17(
    const std::vector<char>& buffer, bool client_frame, int* bytes_consumed, std::vector<char>* output, bool* compressed)
{
    *bytes_consumed = 0;
    if (buffer.size() < 2)
...
private:
    ...
    int ParseWsFrames(const std::vector<char>& buffer)
    {
        ...
        ws_decode_result r = decode_frame_hybi17(buffer, true /* client_frame */, &bytes_consumed, &output, &compressed);
...
class HttpEvent {
public:
    HttpEvent(const std::string& path, bool upgrade, bool isGET, const std::string& ws_key, const std::string& host)
        : path(path)
        , upgrade(upgrade)
        , isGET(isGET)
        , ws_key(ws_key)
...
public:
    ...
    void CancelHandshake() override
    {
        const char HANDSHAKE_FAILED_RESPONSE[] = "HTTP/1.0 400 Bad Request\r\n"
                                                 "Content-Type: text/html; charset=UTF-8\r\n\r\n"
                                                 "WebSockets request was expected\r\n";
        WriteRaw(std::vector<char>(HANDSHAKE_FAILED_RESPONSE, HANDSHAKE_FAILED_RESPONSE + sizeof(HANDSHAKE_FAILED_RESPONSE) - 1), ThenCloseAndReportFailure);
    }
...
private:
    ...
    static int OnMessageComplete(llhttp_t* parser)
    {
        ...
        HttpHandler* handler = From(parser);
        handler->events_.emplace_back(
            handler->path_, parser->upgrade, parser->method == HTTP_GET, handler->HeaderValue("Sec-WebSocket-Key"), handler->HeaderValue("Host"));
        handler->path_ = "";
        handler->parsing_value_ = false;
    ...
    std::string HeaderValue(const std::string& header) const
    {
        ...
        for (const auto& header_value : headers_) {
            if (node::StringEqualNoCaseN(header_value.first.data(), header.data(), header.length())) {
                if (header_found)
                    return "";
    ...
    bool IsAllowedHost(const std::string& host_with_port) const
    {
        ...
        return host.empty() || IsIPAddress(host) || node::StringEqualNoCase(host.data(), "localhost");
    }
...
// Any protocol
ProtocolHandler::ProtocolHandler(InspectorSocket* inspector, TcpHolder::Pointer tcp)
...
int ProtocolHandler::WriteRaw(const std::vector<char>& buffer, uv_write_cb write_cb)
{
...
std::string ProtocolHandler::GetHost() const
{
    ...
    int len = sizeof(addr);
    int err = uv_tcp_getsockname(tcp_->tcp(), reinterpret_cast<struct sockaddr*>(&addr), &len);
    if (err != 0)
        return "";
