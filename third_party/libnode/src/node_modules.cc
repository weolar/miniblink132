...
bool BindingData::PrepareForSerialization(v8::Local<v8::Context> context, v8::SnapshotCreator* creator)
{
...
void BindingData::Deserialize(v8::Local<v8::Context> context, v8::Local<v8::Object> holder, int index, InternalFieldInfoBase* info)
{
    DCHECK_IS_SNAPSHOT_SLOT(index);
...
Local<Array> BindingData::PackageConfig::Serialize(Realm* realm) const
{
    ...
    const auto ToString = [isolate](std::string_view input) -> Local<Primitive> {
        return String::NewFromUtf8(isolate, input.data(), NewStringType::kNormal, input.size()).ToLocalChecked();
    };
...
const BindingData::PackageConfig* BindingData::GetPackageJSON(Realm* realm, std::string_view path, ErrorContext* error_context)
{
    ...
    simdjson::error_code error = binding_data->json_parser.iterate(json_view).get(document);

    const auto throw_invalid_package_config = [error_context, path, realm]() {
        if (error_context == nullptr) {
            THROW_ERR_INVALID_PACKAGE_CONFIG(realm->isolate(), "Invalid package config %s.", path.data());
            ...
            THROW_ERR_INVALID_PACKAGE_CONFIG(realm->isolate(), "Invalid package config %s while importing \"%s\" from %s.", path.data(),
                error_context->specifier.c_str(), file_path->c_str());
        } else {
            THROW_ERR_INVALID_PACKAGE_CONFIG(realm->isolate(), "Invalid package config %s.", path.data());
        }
    ...
    // copying it.
    auto cached = binding_data->package_configs_.insert({ std::string(path), std::move(package_config) });
...
void BindingData::ReadPackageJSON(const FunctionCallbackInfo<Value>& args)
{
    ...
    THROW_IF_INSUFFICIENT_PERMISSIONS(realm->env(), permission::PermissionScope::kFileSystemRead, path.ToStringView());

    ToNamespacedPath(realm->env(), &path);
    auto package_json = GetPackageJSON(realm, path.ToStringView(), is_esm ? &error_context : nullptr);
...
const BindingData::PackageConfig* BindingData::TraverseParent(Realm* realm, const std::filesystem::path& check_path)
{
    ...
    do {
        ...
        // to walk upwards
        if (is_permissions_enabled && !env->permission()->is_granted(env, permission::PermissionScope::kFileSystemRead, current_path.generic_string()))
            [[unlikely]]
            {
                return nullptr;
            }
        ...
        auto package_json = GetPackageJSON(realm, package_json_path.string(), nullptr);
        if (package_json != nullptr) {
            return package_json;
        }
    } while (true);
...
void BindingData::GetNearestParentPackageJSON(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
void BindingData::GetNearestParentPackageJSONType(const FunctionCallbackInfo<Value>& args)
{
    CHECK_GE(args.Length(), 1);
    CHECK(args[0]->IsString());
...
void BindingData::GetPackageScopeConfig(const FunctionCallbackInfo<Value>& args)
{
    CHECK_GE(args.Length(), 1);
    CHECK(args[0]->IsString());
    ...
    auto package_json_url = ada::parse("./package.json", &package_json_url_base.value());
    if (!package_json_url) {
    ...
    auto package_json_url_as_path = url::FileURLToPath(realm->env(), *package_json_url);
    CHECK(package_json_url_as_path);
    Local<Value> ret;
    if (ToV8Value(realm->context(), *package_json_url_as_path, realm->isolate()).ToLocal(&ret)) {
...
void EnableCompileCache(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (ToV8Value(context, result.message).ToLocal(&values[1]) && ToV8Value(context, result.cache_directory).ToLocal(&values[2])) {
...
void GetCompileCacheDir(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Value> ret;
    if (ToV8Value(context, env->compile_cache_handler()->cache_dir()).ToLocal(&ret)) {
        args.GetReturnValue().Set(ret);
    }
}
...
void GetCompileCacheEntry(const FunctionCallbackInfo<Value>& args)
{
    ...
    CachedCodeType type = static_cast<CachedCodeType>(args[2].As<v8::Uint32>()->Value());
    auto* cache_entry = env->compile_cache_handler()->GetOrInsert(source, filename, type);
    if (cache_entry == nullptr) {
        return;
    }

    v8::LocalVector<v8::Name> names(isolate, { FIXED_ONE_BYTE_STRING(isolate, "external") });
    v8::LocalVector<v8::Value> values(isolate, { v8::External::New(isolate, cache_entry) });
    if (cache_entry->cache != nullptr) {
        Debug(env, DebugCategory::COMPILE_CACHE, "[compile cache] retrieving transpile cache for %s %s...", cache_entry->type_name(),
            cache_entry->source_filename);

        std::string_view cache(reinterpret_cast<const char*>(cache_entry->cache->data), cache_entry->cache->length);
        ...
        Debug(env, DebugCategory::COMPILE_CACHE, "[compile cache] no transpile cache for %s %s\n", cache_entry->type_name(), cache_entry->source_filename);
    }
    args.GetReturnValue().Set(Object::New(isolate, v8::Null(isolate), names.data(), values.data(), names.size()));
}
...
void SaveCompileCacheEntry(const FunctionCallbackInfo<Value>& args)
{
    ...
    auto* cache_entry = static_cast<CompileCacheEntry*>(args[0].As<External>()->Value());
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    SetMethod(isolate, target, "getNearestParentPackageJSONType", GetNearestParentPackageJSONType);
    SetMethod(isolate, target, "getNearestParentPackageJSON", GetNearestParentPackageJSON);
...
#define V(status) compile_cache_status_values.push_back(FIXED_ONE_BYTE_STRING(isolate, #status));
    COMPILE_CACHE_STATUS(V)
#undef V

    USE(target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "compileCacheStatus"),
        Array::New(isolate, compile_cache_status_values.data(), compile_cache_status_values.size())));
...
#undef V
    ...
    USE(target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "cachedCodeTypes"),
        Object::New(isolate, Null(isolate), cached_code_type_keys.data(), cached_code_type_values.data(), cached_code_type_keys.size())));
}
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(modules, node::modules::BindingData::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(modules, node::modules::BindingData::CreatePerIsolateProperties)
NODE_BINDING_EXTERNAL_REFERENCE(modules, node::modules::BindingData::RegisterExternalReferences)
