...
Local<Function> WasmStreamingObject::Initialize(Environment* env)
{
    ...
    t->InstanceTemplate()->SetInternalFieldCount(WasmStreamingObject::kInternalFieldCount);
...
void WasmStreamingObject::RegisterExternalReferences(ExternalReferenceRegistry* registry)
{
...
MaybeLocal<Object> WasmStreamingObject::Create(Environment* env, std::shared_ptr<WasmStreaming> streaming)
{
...
void WasmStreamingObject::Push(const FunctionCallbackInfo<Value>& args)
{
    ...
    obj->streaming_->OnBytesReceived(static_cast<const uint8_t*>(bytes) + offset, size);
    obj->wasm_size_ += size;
}
...
void StartStreamingCompilation(const FunctionCallbackInfo<Value>& info)
{
    ...
    std::shared_ptr<WasmStreaming> streaming = WasmStreaming::Unpack(info.GetIsolate(), info.Data());
    ...
    // return anything.
    MaybeLocal<Value> maybe_ret = impl->Call(env->context(), info.This(), 2, args);
    Local<Value> ret;
...
void Initialize(Local<Object> target, Local<Value>, Local<Context> context, void*)
{
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(wasm_web_api, node::wasm_web_api::Initialize)
NODE_BINDING_EXTERNAL_REFERENCE(wasm_web_api, node::wasm_web_api::RegisterExternalReferences)
