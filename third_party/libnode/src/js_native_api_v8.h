...
struct napi_env__ {
    ...
    template <typename T, typename U = decltype(HandleThrow)> inline void CallIntoModule(T&& call, U&& handle_exception = HandleThrow)
    {
...
inline napi_status napi_set_last_error(node_api_basic_env basic_env, napi_status error_code, uint32_t engine_error_code = 0, void* engine_reserved = nullptr)
{
    napi_env env = const_cast<napi_env>(basic_env);
...
#define CHECK_ARG_WITH_PREAMBLE(env, arg) RETURN_STATUS_IF_FALSE_WITH_PREAMBLE((env), ((arg) != nullptr), napi_invalid_arg)

#define CHECK_MAYBE_EMPTY(env, maybe, status) RETURN_STATUS_IF_FALSE((env), !((maybe).IsEmpty()), (status))

#define CHECK_MAYBE_EMPTY_WITH_PREAMBLE(env, maybe, status) RETURN_STATUS_IF_FALSE_WITH_PREAMBLE((env), !((maybe).IsEmpty()), (status))

// NAPI_PREAMBLE is not wrapped in do..while: try_catch must have function scope
#define NAPI_PREAMBLE(env)                                                                                                                                     \
    CHECK_ENV_NOT_IN_GC((env));                                                                                                                                \
    RETURN_STATUS_IF_FALSE((env), (env)->last_exception.IsEmpty(), napi_pending_exception);                                                                    \
    RETURN_STATUS_IF_FALSE((env), (env)->can_call_into_js(), (env->module_api_version >= 10 ? napi_cannot_run_js : napi_pending_exception));                   \
...
#define CHECK_TO_OBJECT_WITH_PREAMBLE(env, context, result, src) CHECK_TO_TYPE_WITH_PREAMBLE((env), Object, (context), (result), (src), napi_object_expected)
...
static_assert(sizeof(v8::Local<v8::Value>) == sizeof(napi_value), "Cannot convert between v8::Local<v8::Value> and napi_value");
...
class Finalizer {
public:
    Finalizer(napi_env env, napi_finalize finalize_callback, void* finalize_data, void* finalize_hint)
        : env_(env)
...
public:
    static TrackedFinalizer* New(napi_env env, napi_finalize finalize_callback, void* finalize_data, void* finalize_hint);
    ~TrackedFinalizer() override;

    void* data()
    {
        return finalizer_.data();
    }

private:
    TrackedFinalizer(napi_env env, napi_finalize finalize_callback, void* finalize_data, void* finalize_hint);
...
public:
    static Reference* New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership);
    ~Reference() override;
...
protected:
    Reference(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership);
...
public:
    static ReferenceWithData* New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership, void* data);
...
private:
    ReferenceWithData(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership, void* data);
...
public:
    static ReferenceWithFinalizer* New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership,
...
private:
    ReferenceWithFinalizer(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount, ReferenceOwnership ownership, napi_finalize finalize_callback,
