...
public:
    ...
    template <typename T> static T* New(EmbedderObjectType type)
    {
        static_assert(
    ...
    template <typename T> T* Copy() const
    {
        static_assert(
            std::is_base_of_v<InternalFieldInfoBase, T> || std::is_same_v<InternalFieldInfoBase, T>, "Can only accept InternalFieldInfoBase subclasses");
        static_assert(std::is_trivially_copyable_v<T>, "Can only memcpy trivially copyable class");
        void* buf = ::operator new[](sizeof(T));
        T* result = new (buf) T;
...
struct EmbedderTypeInfo {
    ...
    EmbedderTypeInfo(EmbedderObjectType t, MemoryMode m)
        : type(t)
        , mode(m)
    {
    }
...
public:
    SnapshotableObject(Realm* realm, v8::Local<v8::Object> wrap, EmbedderObjectType type);
    std::string GetTypeName() const;

    // If returns false, the object will not be serialized.
    virtual bool PrepareForSerialization(v8::Local<v8::Context> context, v8::SnapshotCreator* creator) = 0;
    virtual InternalFieldInfoBase* Serialize(int index) = 0;
    bool is_snapshotable() const override
    {
        return true;
    }
...
#define SERIALIZABLE_OBJECT_METHODS()                                                                                                                          \
    bool PrepareForSerialization(v8::Local<v8::Context> context, v8::SnapshotCreator* creator) override;                                                       \
...
public:
    ...
    BindingData(Realm* realm, v8::Local<v8::Object> obj, InternalFieldInfo* info = nullptr);
