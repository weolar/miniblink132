...
std::ostream& operator<<(std::ostream& output, const builtins::CodeCacheInfo& info)
{
    output << "<builtins::CodeCacheInfo id=" << info.id << ", length=" << info.data.length << ">\n";
    return output;
}

std::ostream& operator<<(std::ostream& output, const std::vector<builtins::CodeCacheInfo>& vec)
{
    output << "{\n";
...
std::ostream& operator<<(std::ostream& output, const std::vector<PropInfo>& vec)
{
    output << "{\n";
    for (const auto& info : vec) {
        output << "  " << info << ",\n";
...
{
    output << "{ \"" << info.name << "\", " << std::to_string(info.id) << ", " << std::to_string(info.index) << " }";
    return output;
}

std::ostream& operator<<(std::ostream& output, const std::vector<std::string>& vec)
{
    output << "{\n";
    for (const auto& info : vec) {
        output << "  \"" << info << "\",\n";
...
public:
    explicit SnapshotDeserializer(std::string_view v)
        : BlobDeserializer<SnapshotDeserializer>(per_process::enabled_debug_list.enabled(DebugCategory::SNAPSHOT_SERDES), v)
    {
    }

    template <typename T, std::enable_if_t<!std::is_same<T, std::string>::value>* = nullptr, std::enable_if_t<!std::is_arithmetic<T>::value>* = nullptr>
    T Read();
};
...
public:
    SnapshotSerializer()
        : BlobSerializer<SnapshotSerializer>(per_process::enabled_debug_list.enabled(DebugCategory::SNAPSHOT_SERDES))
    {
...
template <> v8::StartupData SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const v8::StartupData& data)
{
    ...
    written_total += WriteArithmetic<char>(data.data, static_cast<size_t>(data.raw_size));
...
template <> builtins::CodeCacheInfo SnapshotDeserializer::Read()
{
    Debug("Read<builtins::CodeCacheInfo>()\n");

    std::string id = ReadString();
    auto owning_ptr = std::make_shared<std::vector<uint8_t>>(ReadVector<uint8_t>());
...
template <> size_t SnapshotSerializer::Write(const builtins::CodeCacheInfo& info)
{
    Debug("\nWrite<builtins::CodeCacheInfo>() id = %s"
          ", length=%d\n",
        info.id.c_str(), info.data.length);
...
template <> PropInfo SnapshotDeserializer::Read()
{
    Debug("Read<PropInfo>()\n");
...
template <> size_t SnapshotSerializer::Write(const PropInfo& data)
{
    if (is_debug) {
...
template <> AsyncHooks::SerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const AsyncHooks::SerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
    ...
    size_t written_total = WriteArithmetic<AliasedBufferIndex>(data.async_ids_stack);
    ...
    written_total += WriteArithmetic<SnapshotIndex>(data.js_execution_async_resources);
    written_total += WriteVector<SnapshotIndex>(data.native_execution_async_resources);
...
// [ 4/8 bytes ]  snapshot index of fields
template <> TickInfo::SerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const TickInfo::SerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
// Layout of TickInfo::SerializeInfo
// [ 4/8 bytes ]  snapshot index of fields
template <> ImmediateInfo::SerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const ImmediateInfo::SerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
template <> performance::PerformanceState::SerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const performance::PerformanceState::SerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
template <> IsolateDataSerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const IsolateDataSerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
template <> RealmSerializeInfo SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const RealmSerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
template <> EnvSerializeInfo SnapshotDeserializer::Read()
{
    ...
    result.performance_state = Read<performance::PerformanceState::SerializeInfo>();
...
template <> size_t SnapshotSerializer::Write(const EnvSerializeInfo& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
    ...
    written_total += Write<performance::PerformanceState::SerializeInfo>(data.performance_state);
    ...
    written_total += WriteArithmetic<AliasedBufferIndex>(data.should_abort_on_uncaught_toggle);
...
template <> SnapshotMetadata SnapshotDeserializer::Read()
{
...
template <> size_t SnapshotSerializer::Write(const SnapshotMetadata& data)
{
    if (is_debug) {
        std::string str = ToStr(data);
...
const SnapshotData* SnapshotData::FromEmbedderWrapper(const EmbedderSnapshotData* data)
{
...
bool SnapshotData::Check() const
{
    if (metadata.node_version != per_process::metadata.versions.node) {
        fprintf(stderr,
            ...
            metadata.node_version.c_str(), NODE_VERSION);
        return false;
    }
    ...
    if (metadata.node_arch != per_process::metadata.arch) {
        fprintf(stderr,
            ...
            metadata.node_arch.c_str(), NODE_ARCH);
        return false;
    }
    ...
    if (metadata.node_platform != per_process::metadata.platform) {
        fprintf(stderr,
            ...
            metadata.node_platform.c_str(), NODE_PLATFORM);
        return false;
    }
...
{
    if (data_ownership == DataOwnership::kOwned && v8_snapshot_blob_data.data != nullptr) {
...
static std::string FormatSize(size_t size)
{
    ...
    if (size < 1024) {
        ...
        snprintf(buf, sizeof(buf), "%.2fMB", static_cast<double>(size / 1024 / 1024));
    }
    return buf;
}

template <typename T> void WriteByteVectorLiteral(std::ostream* ss, const T* vec, size_t size, const char* var_name, bool use_array_literals)
{
...
static void WriteCodeCacheInitializer(std::ostream* ss, const std::string& id, size_t size)
{
...
void FormatBlob(std::ostream& ss, const SnapshotData* data, bool use_array_literals)
{
...
namespace node {
    ...
    WriteByteVectorLiteral(&ss, data->v8_snapshot_blob_data.data, data->v8_snapshot_blob_data.raw_size, "v8_snapshot_blob_data", use_array_literals);

    ss << R"(static const int v8_snapshot_blob_size = )" << data->v8_snapshot_blob_data.raw_size << ";\n";
    ...
    for (const auto& item : data->code_cache) {
        ...
        WriteByteVectorLiteral(&ss, item.data.data, item.data.length, var_name.c_str(), use_array_literals);
    }
...
void SnapshotBuilder::InitializeIsolateParams(const SnapshotData* data, Isolate::CreateParams* params)
{
    CHECK_NULL(params->external_references);
    CHECK_NULL(params->snapshot_blob);
    params->external_references = CollectExternalReferences().data();
    params->snapshot_blob = const_cast<v8::StartupData*>(&(data->v8_snapshot_blob_data));
}

SnapshotFlags operator|(SnapshotFlags x, SnapshotFlags y)
{
    return static_cast<SnapshotFlags>(static_cast<uint32_t>(x) | static_cast<uint32_t>(y));
}
...
SnapshotFlags operator|=(/* NOLINT (runtime/references) */ SnapshotFlags& x, SnapshotFlags y)
{
    return x = x | y;
}
...
std::optional<SnapshotConfig> ReadSnapshotConfig(const char* config_path)
{
    ...
    if (r != 0) {
        FPrintF(stderr, "Cannot read snapshot configuration from %s: %s\n", config_path, uv_strerror(r));
        return std::nullopt;
    }
    ...
    if (!result.builder_script_path.has_value()) {
        FPrintF(stderr, "\"builder\" field of %s is not a non-empty string\n", config_path);
        return std::nullopt;
    }

    std::optional<bool> WithoutCodeCache = parser.GetTopLevelBoolField("withoutCodeCache");
    if (!WithoutCodeCache.has_value()) {
        FPrintF(stderr, "\"withoutCodeCache\" field of %s is not a boolean\n", config_path);
        return std::nullopt;
    }
...
ExitCode BuildSnapshotWithoutCodeCache(SnapshotData* out, const std::vector<std::string>& args, const std::vector<std::string>& exec_args,
    std::optional<std::string_view> builder_script_content, const SnapshotConfig& config)
{
    DCHECK(builder_script_content.has_value() == config.builder_script_path.has_value());
    ...
    SnapshotMetadata::Type snapshot_type = builder_script_content.has_value() ? SnapshotMetadata::Type::kFullyCustomized : SnapshotMetadata::Type::kDefault;

    std::vector<std::string> errors;
    auto setup = CommonEnvironmentSetup::CreateForSnapshotting(per_process::v8_platform.Platform(), &errors, args, exec_args, config);
    if (!setup) {
...

    {
        ...
        auto print_Exception = OnScopeLeave([&]() {
            if (bootstrapCatch.HasCaught()) {
                PrintCaughtException(isolate, isolate->GetCurrentContext(), bootstrapCatch);
            }
        });
        ...
        ExitCode exit_code = SpinEventLoopInternal(env).FromMaybe(ExitCode::kGenericUserError);
...
ExitCode BuildCodeCacheFromSnapshot(SnapshotData* out, const std::vector<std::string>& args, const std::vector<std::string>& exec_args)
{
    ...
    if (!builtin_loader.CompileAllBuiltinsAndCopyCodeCache(context, out->env_info.principal_realm.builtins, &(out->code_cache))) {
        return ExitCode::kGenericUserError;
    }
    if (per_process::enabled_debug_list.enabled(DebugCategory::MKSNAPSHOT)) {
        for (const auto& item : out->code_cache) {
            std::string size_str = FormatSize(item.data.length);
            per_process::Debug(DebugCategory::MKSNAPSHOT, "Generated code cache for %d: %s\n", item.id.c_str(), size_str.c_str());
        }
    }
    return ExitCode::kNoFailure;
}

ExitCode SnapshotBuilder::Generate(SnapshotData* out, const std::vector<std::string>& args, const std::vector<std::string>& exec_args,
    std::optional<std::string_view> builder_script_content, const SnapshotConfig& snapshot_config)
{
    ExitCode code = BuildSnapshotWithoutCodeCache(out, args, exec_args, builder_script_content, snapshot_config);
...
ExitCode SnapshotBuilder::CreateSnapshot(SnapshotData* out, CommonEnvironmentSetup* setup)
{
    const SnapshotConfig* config = setup->isolate_data()->snapshot_config();
    DCHECK_NOT_NULL(config);
    SnapshotMetadata::Type snapshot_type
        = config->builder_script_path.has_value() ? SnapshotMetadata::Type::kFullyCustomized : SnapshotMetadata::Type::kDefault;
    Isolate* isolate = setup->isolate();
    Environment* env = setup->env();
...

    {
        ...
        {
            Local<ObjectTemplate> global_template = setup->isolate_data()->contextify_global_template();
            CHECK(!global_template.IsEmpty());
            if (!contextify::ContextifyContext::CreateV8Context(isolate, global_template, nullptr, nullptr).ToLocal(&vm_context)) {
        ...
        index = creator->AddContext(main_context, v8::SerializeInternalFieldsCallback(SerializeNodeContextInternalFields, env),
    ...
    SnapshotCreator::FunctionCodeHandling handling
        = WithoutCodeCache(*config) ? SnapshotCreator::FunctionCodeHandling::kClear : SnapshotCreator::FunctionCodeHandling::kKeep;
    ...
    out->metadata
        = SnapshotMetadata { snapshot_type, per_process::metadata.versions.node, per_process::metadata.arch, per_process::metadata.platform, config->flags };

    // We cannot resurrect the handles from the snapshot, so make sure that
    // no handles are left open in the environment after the blob is created
    // (which should trigger a GC and close all handles that can be closed).
    bool queues_are_empty = env->req_wrap_queue()->IsEmpty() && env->handle_wrap_queue()->IsEmpty();
    if (!queues_are_empty || per_process::enabled_debug_list.enabled(DebugCategory::MKSNAPSHOT)) {
...
ExitCode SnapshotBuilder::GenerateAsSource(const char* out_path, const std::vector<std::string>& args, const std::vector<std::string>& exec_args,
    const SnapshotConfig& config, bool use_array_literals)
{
    ...
    if (config.builder_script_path.has_value()) {
        ...
        if (r != 0) {
            FPrintF(stderr, "Cannot read main script %s for building snapshot. %s: %s", builder_script_path, uv_err_name(r), uv_strerror(r));
            return ExitCode::kGenericUserError;
        }
    ...
    SnapshotData data;
    ExitCode exit_code = Generate(&data, args, exec_args, builder_script_optional, config);
    if (exit_code != ExitCode::kNoFailure) {
        return exit_code;
    }
...
SnapshotableObject::SnapshotableObject(Realm* realm, Local<Object> wrap, EmbedderObjectType type)
...
void DeserializeNodeContextData(Local<Context> holder, int index, StartupData payload, void* callback_data)
{
...
StartupData SerializeNodeContextData(Local<Context> holder, int index, void* callback_data)
{
    ...
    case ContextEmbedderIndex::kContextTag: {
        ...
        per_process::Debug(DebugCategory::MKSNAPSHOT, "Serialize context data, index=%d, holder=%p, ptr=%p\n", static_cast<int>(index), *holder, data);
...
void DeserializeNodeInternalFields(Local<Object> holder, int index, StartupData payload, void* callback_data)
{
    ...
    per_process::Debug(
        DebugCategory::MKSNAPSHOT, "Deserialize internal field %d of %p, size=%d\n", static_cast<int>(index), (*holder), static_cast<int>(payload.raw_size));
    ...
    const InternalFieldInfoBase* info = reinterpret_cast<const InternalFieldInfoBase*>(payload.data);
...
#define V(PropertyName, NativeTypeName)                                                                                                                        \
    case EmbedderObjectType::k_##PropertyName: {                                                                                                               \
        per_process::Debug(DebugCategory::MKSNAPSHOT, "Object %p is %s\n", (*holder), #NativeTypeName);                                                        \
        env->EnqueueDeserializeRequest(NativeTypeName::Deserialize, holder, index, info->Copy<NativeTypeName::InternalFieldInfo>());                           \
        break;                                                                                                                                                 \
    }
        SERIALIZABLE_OBJECT_TYPES(V)
#undef V
...
StartupData SerializeNodeContextInternalFields(Local<Object> holder, int index, void* callback_data)
{
    ...
    if (!BaseObject::IsBaseObject(env->isolate_data(), holder)) {
        per_process::Debug(DebugCategory::MKSNAPSHOT, "Serialize unknown object, index=%d, holder=%p\n", static_cast<int>(index), *holder);
        return StartupData { nullptr, 0 };
    }

    per_process::Debug(DebugCategory::MKSNAPSHOT, "Serialize BaseObject, index=%d, holder=%p\n", static_cast<int>(index), *holder);

    BaseObject* object_ptr = static_cast<BaseObject*>(holder->GetAlignedPointerFromInternalField(BaseObject::kSlot));
    ...
    if (index == BaseObject::kEmbedderType) {
        ...
        new (data) EmbedderTypeInfo(obj->type(), EmbedderTypeInfo::MemoryMode::kBaseObject);
    ...
    per_process::Debug(DebugCategory::MKSNAPSHOT, "Object %p is %s, ", *holder, obj->GetTypeName());
    InternalFieldInfoBase* info = obj->Serialize(index);

    per_process::Debug(DebugCategory::MKSNAPSHOT, "payload size=%d\n", static_cast<int>(info->length));
    return StartupData { reinterpret_cast<const char*>(info), static_cast<int>(info->length) };
}

void SerializeSnapshotableObjects(Realm* realm, SnapshotCreator* creator, RealmSerializeInfo* info)
{
    ...
    realm->ForEachBaseObject([&](BaseObject* obj) {
        ...
        per_process::Debug(DebugCategory::MKSNAPSHOT,
            ...
            "object=%p, type=%s\n",
            static_cast<int>(i), ptr, *(ptr->object()), type_name);

        if (ptr->PrepareForSerialization(context, creator)) {
            SnapshotIndex index = creator->AddData(context, obj->object());
            per_process::Debug(DebugCategory::MKSNAPSHOT, "Serialized with index=%d\n", static_cast<int>(index));
...
void CompileSerializeMain(const FunctionCallbackInfo<Value>& args)
{
    ...
    Local<Function> fn;
    if (contextify::CompileFunction(context, filename, source, &parameters).ToLocal(&fn)) {
...
BindingData::BindingData(Realm* realm, v8::Local<v8::Object> object, InternalFieldInfo* info)
    ...
    , is_building_snapshot_buffer_(realm->isolate(), 1, MAYBE_FIELD_PTR(info, is_building_snapshot_buffer))
{
    if (info == nullptr) {
        object->Set(realm->context(), FIXED_ONE_BYTE_STRING(realm->isolate(), "isBuildingSnapshotBuffer"), is_building_snapshot_buffer_.GetJSArray()).Check();
    } else {
    ...
    DCHECK_IMPLIES(is_building_snapshot, realm->isolate_data()->snapshot_data() == nullptr);
...
bool BindingData::PrepareForSerialization(Local<Context> context, v8::SnapshotCreator* creator)
{
    ...
    internal_field_info_->is_building_snapshot_buffer = is_building_snapshot_buffer_.Serialize(context, creator);
...
{
    tracker->TrackField("is_building_snapshot_buffer", is_building_snapshot_buffer_);
}

void CreatePerContextProperties(Local<Object> target, Local<Value> unused, Local<Context> context, void* priv)
{
    Realm* realm = Realm::GetCurrent(context);
...
void CreatePerIsolateProperties(IsolateData* isolate_data, Local<ObjectTemplate> target)
{
    ...
    SetMethod(isolate, target, "setDeserializeMainFunction", SetDeserializeMainFunction);
    target->Set(FIXED_ONE_BYTE_STRING(isolate, "anonymousMainPath"), OneByteString(isolate, kAnonymousMainPath));
}
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(mksnapshot, node::mksnapshot::CreatePerContextProperties)
NODE_BINDING_PER_ISOLATE_INIT(mksnapshot, node::mksnapshot::CreatePerIsolateProperties)
NODE_BINDING_EXTERNAL_REFERENCE(mksnapshot, node::mksnapshot::RegisterExternalReferences)
