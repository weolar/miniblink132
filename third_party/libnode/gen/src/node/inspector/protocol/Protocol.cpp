...
private:
    ...
    struct ContainerState {
        ContainerState(DictionaryValue* dict)
            : is_dict(true)
            , dict(dict)
        {
        }
        ContainerState(ListValue* list)
            : is_dict(false)
            , list(list)
        {
        }
...
void EncodeString(const String& s, std::vector<uint8_t>* out)
{
    if (StringUtil::CharacterCount(s) == 0) {
        ...
        cbor::EncodeFromLatin1(span<uint8_t>(StringUtil::CharactersLatin1(s), StringUtil::CharacterCount(s)), out);
    } else if (StringUtil::CharactersUTF16(s)) {
        cbor::EncodeFromUTF16(span<uint16_t>(StringUtil::CharactersUTF16(s), StringUtil::CharacterCount(s)), out);
    } else if (StringUtil::CharactersUTF8(s)) {
        cbor::EncodeString8(span<uint8_t>(StringUtil::CharactersUTF8(s), StringUtil::CharacterCount(s)), out);
    }
}
} // namespace
...
{
    cbor::EncodeBinary(span<uint8_t>(m_binaryValue.data(), m_binaryValue.size()), bytes);
}
...
using node::inspector::protocol::Binary;
using node::inspector::protocol::DictionaryValue;
using node::inspector::protocol::FundamentalValue;
using node::inspector::protocol::Object;
using node::inspector::protocol::String;
using node::inspector::protocol::StringUtil;
using node::inspector::protocol::StringValue;
using node::inspector::protocol::Value;
//using node::inspector::protocol::EncodeString;
...
// static
bool ProtocolTypeTraits<std::unique_ptr<Value>>::Deserialize(DeserializerState* state, std::unique_ptr<Value>* value)
{
...
// static
void ProtocolTypeTraits<std::unique_ptr<Value>>::Serialize(const std::unique_ptr<Value>& value, std::vector<uint8_t>* bytes)
{
    value->AppendSerialized(bytes);
}

// static
bool ProtocolTypeTraits<std::unique_ptr<DictionaryValue>>::Deserialize(DeserializerState* state, std::unique_ptr<DictionaryValue>* value)
{
...
// static
void ProtocolTypeTraits<std::unique_ptr<DictionaryValue>>::Serialize(const std::unique_ptr<DictionaryValue>& value, std::vector<uint8_t>* bytes)
{
    value->AppendSerialized(bytes);
}
