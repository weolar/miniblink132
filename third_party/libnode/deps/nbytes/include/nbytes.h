...
template <typename T, typename U> inline T AlignDown(T value, U alignment)
{
...
bool Base64DecodeGroupSlow(char* const dst, const size_t dstlen, const TypeName* const src, const size_t srclen, size_t* const i, size_t* const k)
{
    uint8_t hi;
    uint8_t lo;
#define V(expr)                                                                                                                                                \
...
inline constexpr size_t Base64EncodedSize(size_t size, Base64Mode mode = Base64Mode::NORMAL)
{
...
template <typename TypeName> size_t Base64DecodedSize(const TypeName* src, size_t size)
{
    // 1-byte input cannot be decoded
    if (size < 2)
        return 0;
...
template <typename TypeName>
size_t Base64DecodeFast(char* const dst, const size_t dstlen, const TypeName* const src, const size_t srclen, const size_t decoded_size)
{
...
template <typename TypeName> size_t Base64Decode(char* const dst, const size_t dstlen, const TypeName* const src, const size_t srclen)
{
...
template <typename TypeName> static size_t HexDecode(char* buf, size_t len, const TypeName* src, const size_t srcLen)
{
    size_t i;
...
template <typename Char> class StringSearch : private StringSearchBase {
public:
    ...
    static inline int AlphabetSize()
    {
        ...
        static_assert(sizeof(Char) == sizeof(uint8_t) || sizeof(Char) == sizeof(uint16_t), "sizeof(Char) == sizeof(uint16_t) || sizeof(uint8_t)");
    }
...
template <typename Char> inline size_t FindFirstCharacter(Vector<const Char> pattern, Vector<const Char> subject, size_t index)
{
...
template <> inline size_t FindFirstCharacter(Vector<const uint8_t> pattern, Vector<const uint8_t> subject, size_t index)
{
...
template <typename Char> size_t StringSearch<Char>::SingleCharSearch(Vector subject, size_t index)
{
...
template <typename Char> size_t StringSearch<Char>::LinearSearch(Vector subject, size_t index)
{
...
template <typename Char> size_t StringSearch<Char>::BoyerMooreSearch(Vector subject, size_t start_index)
{
...
template <typename Char> void StringSearch<Char>::PopulateBoyerMooreTable()
{
    const size_t pattern_length = pattern_.length();
...
template <typename Char> size_t StringSearch<Char>::BoyerMooreHorspoolSearch(Vector subject, size_t start_index)
{
    const size_t subject_length = subject.length();
    const size_t pattern_length = pattern_.length();
...
template <typename Char> void StringSearch<Char>::PopulateBoyerMooreHorspoolTable()
{
    const size_t pattern_length = pattern_.length();
...
template <typename Char> size_t StringSearch<Char>::InitialSearch(Vector subject, size_t index)
{
    const size_t pattern_length = pattern_.length();
...
// for each search.
template <typename Char> size_t SearchString(Vector<const Char> subject, Vector<const Char> pattern, size_t start_index)
{
...
size_t SearchString(const Char* haystack, size_t haystack_length, const Char* needle, size_t needle_length, size_t start_index, bool is_forward)
{
...
template <size_t N> size_t SearchString(const char* haystack, size_t haystack_length, const char (&needle)[N])
{
    return SearchString(reinterpret_cast<const uint8_t*>(haystack), haystack_length, reinterpret_cast<const uint8_t*>(needle), N - 1, 0, true);
}
