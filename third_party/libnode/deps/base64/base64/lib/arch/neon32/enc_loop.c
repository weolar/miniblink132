...
static inline void enc_loop_neon32_inner_asm(const uint8_t** s, uint8_t** o)
{
    ...
    __asm__(
        ...
        // Outputs (modified).
        : [src] "+r"(*s), [dst] "+r"(*o), [t0] "=&w"(tmp0), [t1] "=&w"(tmp1), [t2] "=&w"(tmp2), [t3] "=&w"(tmp3), [m0] "=&w"(mask0), [m1] "=&w"(mask1),
        [m2] "=&w"(mask2), [m3] "=&w"(mask3)

        // Inputs (not modified).
        : [lut] "w"(lut), [n25] "w"(n25), [n51] "w"(n51), [n63] "w"(n63)
...
static inline void enc_loop_neon32_inner(const uint8_t** s, uint8_t** o)
{
#ifdef BASE64_NEON32_USE_ASM
...
static inline void enc_loop_neon32(const uint8_t** s, size_t* slen, uint8_t** o, size_t* olen)
{
