...
static ssize_t writev(const int fd, const struct iovec* iov, int iovcnt)
{
    ssize_t r, nwrite = 0;
...
static bool buffer_alloc(const struct config* config, struct buffer* buf)
{
    if ((buf->raw = malloc(BUFFER_RAW_SIZE)) == NULL || (buf->enc = malloc(BUFFER_ENC_SIZE)) == NULL) {
        free(buf->raw);
...
static bool writev_retry(const struct config* config, struct iovec* iov, size_t nvec)
{
...
static inline bool iov_append(const struct config* config, struct iovec* iov, size_t* nvec, char* base, const size_t len)
{
...
static bool write_stdout(const struct config* config, const char* buf, size_t len)
{
    while (len > 0) {
        ssize_t nwrite;
...
static bool write_wrapped(const struct config* config, char* buf, size_t len)
{
    static size_t col = 0;
...
static bool encode(const struct config* config, struct buffer* buf)
{
    size_t nread, nout;
...
static inline size_t find_newline(const char* p, const size_t avail)
{
...
static bool decode(const struct config* config, struct buffer* buf)
{
    size_t avail;
    struct base64_state state;
...
static void usage(FILE* fp, const struct config* config)
{
    const char* usage = "Usage: %s [OPTION]... [FILE]\n"
...
static bool get_wrap(struct config* config, const char* str)
{
    char* eptr;
...
static bool parse_opts(int argc, char** argv, struct config* config)
{
    int c;
    static const struct option opts[]
        = { { "decode", no_argument, NULL, 'd' }, { "help", no_argument, NULL, 'h' }, { "wrap", required_argument, NULL, 'w' }, { NULL } };
    ...
    while ((c = getopt_long(argc, argv, ":dhw:", opts, NULL)) != -1) {
        ...
        case 'w':
            if (get_wrap(config, optarg) == false) {
                fprintf(stderr, "%s: invalid wrap value '%s'\n", config->name, optarg);
                return false;
            }
            break;
...
int main(int argc, char** argv)
{
    ...
    const bool ret = config.decode ? decode(&config, &buf) : encode(&config, &buf);
