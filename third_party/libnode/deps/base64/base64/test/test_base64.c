...
static bool assert_enc(int flags, const char* src, const char* dst)
{
...
static bool assert_dec(int flags, const char* src, const char* dst)
{
    size_t srclen = strlen(src);
    size_t dstlen = strlen(dst);
...
static int assert_roundtrip(int flags, const char* src)
{
    char tmp[1500];
    size_t tmplen;
    size_t srclen = strlen(src);
    ...
    if (srclen != tmplen) {
        printf("FAIL: roundtrip of '%s': "
               "length expected %lu, got %lu\n",
            src, (unsigned long)srclen, (unsigned long)tmplen);
        return true;
    }
...
static int test_char_table(int flags, bool use_malloc)
{
    bool fail = false;
    char chr[256];
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (use_malloc) {
            ...
            if (src == NULL) {
                printf("FAIL: encoding @ %d: allocation of %lu bytes failed\n", i, (unsigned long)chrlen);
                fail = true;
                continue;
            }
        ...
        if (declen != chrlen) {
            printf("FAIL: roundtrip @ %d: "
                   "length expected %lu, got %lu\n",
                i, (unsigned long)chrlen, (unsigned long)declen);
            fail = true;
            continue;
        }
...
static int test_streaming(int flags)
{
    bool fail = false;
    char chr[256];
    ...
    for (size_t bs = 1; bs < 255; bs++) {
        ...
        if (enclen != reflen) {
            printf("FAIL: stream encoding gave incorrect size: "
                   "%lu instead of %lu\n",
                (unsigned long)enclen, (unsigned long)reflen);
            fail = true;
        }
...
static int test_invalid_dec_input(int flags)
{
    // Subset of invalid characters to cover all ranges
    static const char invalid_set[] = { '\0', -1, '!', '-', ';', '_', '|' };
    static const char* invalid_strings[] = { "Zm9vYg=", "Zm9vYg", "Zm9vY", "Zm9vYmF=Zm9v" };
...
static int test_one_codec(const char* codec, int flags)
{
    bool fail = false;
