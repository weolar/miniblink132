...
static bool get_random_data(struct buffers* b, char** errmsg)
{
#if defined(_WIN32) || defined(_WIN64)
...
static void base64_gettime(base64_timespec* t)
{
    *t = mach_absolute_time();
}

static float timediff_sec(base64_timespec* start, base64_timespec* end)
{
...
static void codec_bench_enc(struct buffers* b, const struct bufsize* bs, const char* name, unsigned int flags)
{
...
static void codec_bench_dec(struct buffers* b, const struct bufsize* bs, const char* name, unsigned int flags)
{
    float timediff, fastest = -1.0f;
    base64_timespec start, end;
...
static void codec_bench(struct buffers* b, const struct bufsize* bs, const char* name, unsigned int flags)
{
