...
CRDTP_EXPORT std::unique_ptr<ParserHandler> NewJSONEncoder(std::vector<uint8_t>* out, Status* status);

CRDTP_EXPORT std::unique_ptr<ParserHandler> NewJSONEncoder(std::string* out, Status* status);
...
CRDTP_EXPORT Status ConvertCBORToJSON(span<uint8_t> cbor, std::vector<uint8_t>* json);

CRDTP_EXPORT Status ConvertJSONToCBOR(span<uint8_t> json, std::vector<uint8_t>* cbor);

CRDTP_EXPORT Status ConvertJSONToCBOR(span<uint16_t> json, std::vector<uint8_t>* cbor);
} // namespace json
} // namespace crdtp
