...
#define POSTJECT_SENTINEL_FUSE "POSTJECT_SENTINEL_fce680ab2cc467b6e072b8b5df1996b2"
#endif
...
static int postject__dl_iterate_phdr_callback(struct dl_phdr_info* info, size_t size, void* data)
{
...
static const void* postject_find_resource(const char* name, size_t* size, const struct postject_options* options)
{
...
#endif
        ptr = getsectdatafromFramework(options->macho_framework_name, segment_name, section_name != NULL ? section_name : name, &section_size);
    } else {
        ptr = getsectdata(segment_name, section_name != NULL ? section_name : name, &section_size);
#ifdef __clang__
#pragma clang diagnostic pop
    ...
    HRSRC resource_handle = FindResourceA(NULL, resource_name != NULL ? resource_name : name, MAKEINTRESOURCEA(10) /* RT_RCDATA */);
