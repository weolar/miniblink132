...
static uvwasi_errno_t uvwasi__insert_stdio(
    uvwasi_t* uvwasi, struct uvwasi_fd_table_t* table, const uvwasi_fd_t fd, const uvwasi_fd_t expected, const char* name)
{
    ...
    err = uvwasi_fd_table_insert(uvwasi, table, fd, NULL, name, name, type, base, inheriting, 0, &wrap);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_table_insert(uvwasi_t* uvwasi, struct uvwasi_fd_table_t* table, uv_file fd, uv_tcp_t* sock, const char* mapped_path,
    const char* real_path, uvwasi_filetype_t type, uvwasi_rights_t rights_base, uvwasi_rights_t rights_inheriting, int preopen, struct uvwasi_fd_wrap_t** wrap)
{
    ...
    entry = (struct uvwasi_fd_wrap_t*)uvwasi__malloc(uvwasi, sizeof(*entry) + mp_len + mp_len + rp_len + 3);
    if (entry == NULL)
...
uvwasi_errno_t uvwasi_fd_table_init(uvwasi_t* uvwasi, const uvwasi_options_t* options)
{
    ...
    table->fds = uvwasi__calloc(uvwasi, options->fd_table_size, sizeof(struct uvwasi_fd_wrap_t*));
    if (table->fds == NULL) {
...
uvwasi_errno_t uvwasi_fd_table_insert_preopen(uvwasi_t* uvwasi, struct uvwasi_fd_table_t* table, const uv_file fd, const char* path, const char* real_path)
{
    uvwasi_filetype_t type;
    uvwasi_rights_t base;
    uvwasi_rights_t inheriting;
    ...
    return uvwasi_fd_table_insert(
        uvwasi, table, fd, NULL, path, real_path, UVWASI_FILETYPE_DIRECTORY, UVWASI__RIGHTS_DIRECTORY_BASE, UVWASI__RIGHTS_DIRECTORY_INHERITING, 1, NULL);
}

uvwasi_errno_t uvwasi_fd_table_insert_preopen_socket(uvwasi_t* uvwasi, struct uvwasi_fd_table_t* table, uv_tcp_t* sock)
{
    if (table == NULL || sock == NULL)
        return UVWASI_EINVAL;

    return uvwasi_fd_table_insert(
        uvwasi, table, -1, sock, NULL, NULL, UVWASI_FILETYPE_SOCKET_STREAM, UVWASI__RIGHTS_SOCKET_BASE, UVWASI__RIGHTS_SOCKET_INHERITING, 1, NULL);
}
    ...
    struct uvwasi_fd_table_t* table, const uvwasi_fd_t id, struct uvwasi_fd_wrap_t** wrap, uvwasi_rights_t rights_base, uvwasi_rights_t rights_inheriting)
{
    ...
    uv_rwlock_wrlock(&table->rwlock);
    err = uvwasi_fd_table_get_nolock(table, id, wrap, rights_base, rights_inheriting);
    uv_rwlock_wrunlock(&table->rwlock);
    return err;
}
    ...
    struct uvwasi_fd_table_t* table, const uvwasi_fd_t id, struct uvwasi_fd_wrap_t** wrap, uvwasi_rights_t rights_base, uvwasi_rights_t rights_inheriting)
{
    ...
    if ((~entry->rights_base & rights_base) != 0 || (~entry->rights_inheriting & rights_inheriting) != 0) {
...
uvwasi_errno_t uvwasi_fd_table_remove_nolock(uvwasi_t* uvwasi, struct uvwasi_fd_table_t* table, const uvwasi_fd_t id)
{
    struct uvwasi_fd_wrap_t* entry;
...
uvwasi_errno_t uvwasi_fd_table_renumber(struct uvwasi_s* uvwasi, struct uvwasi_fd_table_t* table, const uvwasi_fd_t dst, const uvwasi_fd_t src)
{
    ...
    if (dst_entry == NULL || dst_entry->id != dst || src_entry == NULL || src_entry->id != src) {
        err = UVWASI_EBADF;
        goto exit;
    }
