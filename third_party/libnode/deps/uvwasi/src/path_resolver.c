...
static uvwasi_errno_t uvwasi__combine_paths(const uvwasi_t* uvwasi, const char* path1, uvwasi_size_t path1_len, const char* path2, uvwasi_size_t path2_len,
    char** combined_path, uvwasi_size_t* combined_len)
{
    ...
    for (cur = path; cur != NULL; cur = next + 1) {
        ...
        if (cur_len == 0) {
         ...
         then output '..'. Otherwise, remove the last path segment. */
            if (ptr == normalized_path || (last == ptr - 2 && last[0] == '.' && last[1] == '.')
                || (last == ptr - 3 && last[0] == '/' && last[1] == '.' && last[2] == '.')) {
...
static int uvwasi__is_path_sandboxed(const char* path, uvwasi_size_t path_len, const char* fd_path, uvwasi_size_t fd_path_len)
{
    char* ptr;
    int remaining_len;
    ...
    if (fd_path_len == 1 && fd_path[0] == '.') {
        ...
        if ((path_len == 2 && path[0] == '.' && path[1] == '.') || (path_len > 2 && path[0] == '.' && path[1] == '.' && path[2] == '/')) {
            return 0;
        }
    ...
    if ((remaining_len == 2 && ptr[0] == '.' && ptr[1] == '.') || (remaining_len > 2 && ptr[0] == '.' && ptr[1] == '.' && ptr[2] == '/')) {
        return 0;
    }
...
static uvwasi_errno_t uvwasi__normalize_absolute_path(
    const uvwasi_t* uvwasi, const struct uvwasi_fd_wrap_t* fd, const char* path, uvwasi_size_t path_len, char** normalized_path, uvwasi_size_t* normalized_len)
{
    ...
    *normalized_len = 0;
    ...
    if (0 == uvwasi__is_path_sandboxed(abs_path, path_len, fd->normalized_path, strlen(fd->normalized_path))) {
    ...
    const uvwasi_t* uvwasi, const struct uvwasi_fd_wrap_t* fd, const char* path, uvwasi_size_t path_len, char** normalized_path, uvwasi_size_t* normalized_len)
{
    ...
    *normalized_len = 0;
    ...
    err = uvwasi__combine_paths(uvwasi, fd->normalized_path, fd_path_len, path, path_len, &combined, &combined_len);
    if (err != UVWASI_ESUCCESS)
        goto exit;
    ...
    err = uvwasi__normalize_path(combined, combined_len, normalized, combined_len);
    if (err != UVWASI_ESUCCESS)
        goto exit;
    ...
    if (0 == uvwasi__is_path_sandboxed(normalized, norm_len, fd->normalized_path, fd_path_len)) {
        err = UVWASI_ENOTCAPABLE;
        goto exit;
    }
...
static uvwasi_errno_t uvwasi__resolve_path_to_host(
    const uvwasi_t* uvwasi, const struct uvwasi_fd_wrap_t* fd, const char* path, uvwasi_size_t path_len, char** resolved_path, uvwasi_size_t* resolved_len)
{
    ...
    *resolved_path = uvwasi__malloc(uvwasi, *resolved_len + 1);
    ...
    if (stripped_len > 1 || (stripped_len == 1 && stripped_path[0] != '/')) {
        if (stripped_path[0] != '/') {
            *res_path = '/';
            res_path++;
        }
...
start:
    ...
    if (1 == uvwasi__is_absolute_path(input, input_len)) {
        err = uvwasi__normalize_absolute_path(uvwasi, fd, input, input_len, &normalized_path, &normalized_len);
    } else {
        err = uvwasi__normalize_relative_path(uvwasi, fd, input, input_len, &normalized_path, &normalized_len);
    }
    ...
    err = uvwasi__resolve_path_to_host(uvwasi, fd, normalized_path, normalized_len, &host_path, &host_path_len);
    if (err != UVWASI_ESUCCESS)
        goto exit;
    ...
    if ((flags & UVWASI_LOOKUP_SYMLINK_FOLLOW) == UVWASI_LOOKUP_SYMLINK_FOLLOW) {
        ...
        if (r != 0) {
         ...
         error using uv_fs_stat(). */
            if (r == UV__UNKNOWN) {
                ...
                if (r == 0) {
                    if (uvwasi__stat_to_filetype(&req.statbuf) != UVWASI_FILETYPE_SYMBOLIC_LINK) {
                        r = UV_EINVAL;
                    }
                }
        ...
        if (1 == uvwasi__is_absolute_path(link_target, link_target_len)) {
            ...
            err = uvwasi__combine_paths(uvwasi, normalized_path, normalized_len, "..", 2, &normalized_parent, &normalized_parent_len);
            if (err != UVWASI_ESUCCESS)
                goto exit;
            err = uvwasi__combine_paths(
                uvwasi, normalized_parent, normalized_parent_len, link_target, link_target_len, &resolved_link_target, &resolved_link_target_len);
            if (err != UVWASI_ESUCCESS)
                goto exit;
