...
#define VALIDATE_FSTFLAGS_OR_RETURN(flags)                                                                                                                     \
    do {                                                                                                                                                       \
        ...
        if (((f) & ~(UVWASI_FILESTAT_SET_ATIM | UVWASI_FILESTAT_SET_ATIM_NOW | UVWASI_FILESTAT_SET_MTIM | UVWASI_FILESTAT_SET_MTIM_NOW))                       \
            || ((f) & (UVWASI_FILESTAT_SET_ATIM | UVWASI_FILESTAT_SET_ATIM_NOW)) == (UVWASI_FILESTAT_SET_ATIM | UVWASI_FILESTAT_SET_ATIM_NOW)                  \
            || ((f) & (UVWASI_FILESTAT_SET_MTIM | UVWASI_FILESTAT_SET_MTIM_NOW)) == (UVWASI_FILESTAT_SET_MTIM | UVWASI_FILESTAT_SET_MTIM_NOW)) {               \
            return UVWASI_EINVAL;                                                                                                                              \
        }                                                                                                                                                      \
    } while (0)

static uvwasi_errno_t uvwasi__get_filestat_set_times(
    uvwasi_timestamp_t* st_atim, uvwasi_timestamp_t* st_mtim, uvwasi_fstflags_t fst_flags, uv_file* fd, char* path)
{
    ...
    if ((fst_flags & (UVWASI_FILESTAT_SET_ATIM_NOW | UVWASI_FILESTAT_SET_MTIM_NOW)) != 0) {
    ...
    if ((fst_flags & (UVWASI_FILESTAT_SET_ATIM | UVWASI_FILESTAT_SET_ATIM_NOW)) == 0
        || (fst_flags & (UVWASI_FILESTAT_SET_MTIM | UVWASI_FILESTAT_SET_MTIM_NOW)) == 0) {
...
{
    return uvwasi->allocator->calloc(nmemb, size, uvwasi->allocator->mem_user_data);
}

void* uvwasi__realloc(const uvwasi_t* uvwasi, void* ptr, size_t size)
{
    return uvwasi->allocator->realloc(ptr, size, uvwasi->allocator->mem_user_data);
}
...
static uvwasi_errno_t uvwasi__lseek(uv_file fd, uvwasi_filedelta_t offset, uvwasi_whence_t whence, uvwasi_filesize_t* newoffset)
{
    int real_whence;
...
static uvwasi_errno_t uvwasi__setup_iovs(const uvwasi_t* uvwasi, uv_buf_t** buffers, const uvwasi_iovec_t* iovs, uvwasi_size_t iovs_len)
{
    uv_buf_t* bufs;
    uvwasi_size_t i;
...
static uvwasi_errno_t uvwasi__setup_ciovs(const uvwasi_t* uvwasi, uv_buf_t** buffers, const uvwasi_ciovec_t* iovs, uvwasi_size_t iovs_len)
{
    uv_buf_t* bufs;
    uvwasi_size_t i;
...
uvwasi_errno_t uvwasi_init(uvwasi_t* uvwasi, const uvwasi_options_t* options)
{
    ...
    for (i = 0; i < options->preopenc; ++i) {
        if (options->preopens[i].real_path == NULL || options->preopens[i].mapped_path == NULL) {
            err = UVWASI_EINVAL;
            goto exit;
        }
    }
    ...
    for (i = 0; i < options->preopen_socketc; ++i) {
        if (options->preopen_sockets[i].address == NULL || options->preopen_sockets[i].port > 65535) {
            err = UVWASI_EINVAL;
            goto exit;
        }
    }
    ...
    for (i = 0; i < options->preopenc; ++i) {
        r = uv_fs_realpath(NULL, &realpath_req, options->preopens[i].real_path, NULL);
        if (r != 0) {
        ...
        err = uvwasi_fd_table_insert_preopen(uvwasi, uvwasi->fds, open_req.result, options->preopens[i].mapped_path, realpath_req.ptr);
        uv_fs_req_cleanup(&realpath_req);
        uv_fs_req_cleanup(&open_req);
    ...
    for (i = 0; i < options->preopen_socketc; ++i) {
        ...
        err = uvwasi_fd_table_insert_preopen_socket(uvwasi, uvwasi->fds, socket);
...
uvwasi_errno_t uvwasi_embedder_remap_fd(uvwasi_t* uvwasi, const uvwasi_fd_t fd, uv_file new_host_fd)
{
...
uvwasi_errno_t uvwasi_args_sizes_get(uvwasi_t* uvwasi, uvwasi_size_t* argc, uvwasi_size_t* argv_buf_size)
{
    UVWASI_DEBUG("uvwasi_args_sizes_get(uvwasi=%p, argc=%p, argv_buf_size=%p)\n", uvwasi, argc, argv_buf_size);
...
uvwasi_errno_t uvwasi_clock_res_get(uvwasi_t* uvwasi, uvwasi_clockid_t clock_id, uvwasi_timestamp_t* resolution)
{
    UVWASI_DEBUG("uvwasi_clock_res_get(uvwasi=%p, clock_id=%d, resolution=%p)\n", uvwasi, clock_id, resolution);
...
uvwasi_errno_t uvwasi_clock_time_get(uvwasi_t* uvwasi, uvwasi_clockid_t clock_id, uvwasi_timestamp_t precision, uvwasi_timestamp_t* time)
{
    UVWASI_DEBUG("uvwasi_clock_time_get(uvwasi=%p, clock_id=%d, "
                 "precision=%" PRIu64 ", time=%p)\n",
        uvwasi, clock_id, precision, time);
...
uvwasi_errno_t uvwasi_environ_get(uvwasi_t* uvwasi, char** environment, char* environ_buf)
{
    uvwasi_size_t i;

    UVWASI_DEBUG("uvwasi_environ_get(uvwasi=%p, environment=%p, "
                 "environ_buf=%p)\n",
        uvwasi, environment, environ_buf);
...
uvwasi_errno_t uvwasi_environ_sizes_get(uvwasi_t* uvwasi, uvwasi_size_t* environ_count, uvwasi_size_t* environ_buf_size)
{
    UVWASI_DEBUG("uvwasi_environ_sizes_get(uvwasi=%p, environ_count=%p, "
                 "environ_buf_size=%p)\n",
        uvwasi, environ_count, environ_buf_size);
...
uvwasi_errno_t uvwasi_fd_advise(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filesize_t offset, uvwasi_filesize_t len, uvwasi_advice_t advice)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
    uv_fs_t req;
...
uvwasi_errno_t uvwasi_fd_allocate(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filesize_t offset, uvwasi_filesize_t len)
{
    ...
    UVWASI_DEBUG("uvwasi_fd_allocate(uvwasi=%p, fd=%d, offset=%" PRIu64 ", "
                 "len=%" PRIu64 ")\n",
        uvwasi, fd, offset, len);

    if (uvwasi == NULL)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_ALLOCATE, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_datasync(uvwasi_t* uvwasi, uvwasi_fd_t fd)
{
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_DATASYNC, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_fdstat_get(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_fdstat_t* buf)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
#ifndef _WIN32
...
uvwasi_errno_t uvwasi_fd_fdstat_set_flags(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_fdflags_t flags)
{
#ifdef _WIN32
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_FDSTAT_SET_FLAGS, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_fdstat_set_rights(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_rights_t fs_rights_base, uvwasi_rights_t fs_rights_inheriting)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
    ...
    UVWASI_DEBUG("uvwasi_fd_fdstat_set_rights(uvwasi=%p, fd=%d, "
        ...
        uvwasi, fd, fs_rights_base, fs_rights_inheriting);
    ...
    if ((fs_rights_inheriting | wrap->rights_inheriting) > wrap->rights_inheriting) {
        err = UVWASI_ENOTCAPABLE;
        goto exit;
    }
...
uvwasi_errno_t uvwasi_fd_filestat_get(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filestat_t* buf)
{
    struct uvwasi_fd_wrap_t* wrap;
    uv_fs_t req;
    uvwasi_errno_t err;
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_FILESTAT_GET, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_filestat_set_size(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filesize_t st_size)
{
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_FILESTAT_SET_SIZE, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_filestat_set_times(
    uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_timestamp_t st_atim, uvwasi_timestamp_t st_mtim, uvwasi_fstflags_t fst_flags)
{
    struct uvwasi_fd_wrap_t* wrap;
    ...
    UVWASI_DEBUG("uvwasi_fd_filestat_set_times(uvwasi=%p, fd=%d, "
        ...
        uvwasi, fd, st_atim, st_mtim, fst_flags);
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_FILESTAT_SET_TIMES, 0);
    if (err != UVWASI_ESUCCESS)
        return err;

    atim = st_atim;
    mtim = st_mtim;
    err = uvwasi__get_filestat_set_times(&atim, &mtim, fst_flags, &wrap->fd, NULL);
    if (err != UVWASI_ESUCCESS) {
        uv_mutex_unlock(&wrap->mutex);
        return err;
    }
...
uvwasi_errno_t uvwasi_fd_pread(
    uvwasi_t* uvwasi, uvwasi_fd_t fd, const uvwasi_iovec_t* iovs, uvwasi_size_t iovs_len, uvwasi_filesize_t offset, uvwasi_size_t* nread)
{
    struct uvwasi_fd_wrap_t* wrap;
    uv_buf_t* bufs;
    uv_fs_t req;
    ...
    UVWASI_DEBUG("uvwasi_fd_pread(uvwasi=%p, fd=%d, iovs=%p, iovs_len=%d, "
        ...
        uvwasi, fd, iovs, iovs_len, offset, nread);

    if (uvwasi == NULL || (iovs == NULL && iovs_len > 0) || nread == NULL || offset > INT64_MAX)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_READ | UVWASI_RIGHT_FD_SEEK, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_prestat_get(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_prestat_t* buf)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
...
uvwasi_errno_t uvwasi_fd_prestat_dir_name(uvwasi_t* uvwasi, uvwasi_fd_t fd, char* path, uvwasi_size_t path_len)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
    size_t size;
...
uvwasi_errno_t uvwasi_fd_pwrite(
    uvwasi_t* uvwasi, uvwasi_fd_t fd, const uvwasi_ciovec_t* iovs, uvwasi_size_t iovs_len, uvwasi_filesize_t offset, uvwasi_size_t* nwritten)
{
    struct uvwasi_fd_wrap_t* wrap;
    uv_buf_t* bufs;
    uv_fs_t req;
    ...
    UVWASI_DEBUG("uvwasi_fd_pwrite(uvwasi=%p, fd=%d, iovs=%p, iovs_len=%d, "
        ...
        uvwasi, fd, iovs, iovs_len, offset, nwritten);

    if (uvwasi == NULL || (iovs == NULL && iovs_len > 0) || nwritten == NULL || offset > INT64_MAX)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_WRITE | UVWASI_RIGHT_FD_SEEK, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_read(uvwasi_t* uvwasi, uvwasi_fd_t fd, const uvwasi_iovec_t* iovs, uvwasi_size_t iovs_len, uvwasi_size_t* nread)
{
    struct uvwasi_fd_wrap_t* wrap;
    uv_buf_t* bufs;
    uv_fs_t req;
...
uvwasi_errno_t uvwasi_fd_readdir(uvwasi_t* uvwasi, uvwasi_fd_t fd, void* buf, uvwasi_size_t buf_len, uvwasi_dircookie_t cookie, uvwasi_size_t* bufused)
{
    ...
    UVWASI_DEBUG("uvwasi_fd_readdir(uvwasi=%p, fd=%d, buf=%p, buf_len=%d, "
        ...
        uvwasi, fd, buf, buf_len, cookie, bufused);

    if (uvwasi == NULL || buf == NULL || bufused == NULL)
        return UVWASI_EINVAL;

#if defined(UVWASI_FD_READDIR_SUPPORTED)
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_READDIR, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_renumber(uvwasi_t* uvwasi, uvwasi_fd_t from, uvwasi_fd_t to)
{
...
uvwasi_errno_t uvwasi_fd_seek(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filedelta_t offset, uvwasi_whence_t whence, uvwasi_filesize_t* newoffset)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
    ...
    UVWASI_DEBUG("uvwasi_fd_seek(uvwasi=%p, fd=%d, offset=%" PRId64 ", "
        ...
        uvwasi, fd, offset, whence, newoffset);
...
uvwasi_errno_t uvwasi_fd_sync(uvwasi_t* uvwasi, uvwasi_fd_t fd)
{
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_FD_SYNC, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_fd_tell(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_filesize_t* offset)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
...
uvwasi_errno_t uvwasi_fd_write(uvwasi_t* uvwasi, uvwasi_fd_t fd, const uvwasi_ciovec_t* iovs, uvwasi_size_t iovs_len, uvwasi_size_t* nwritten)
{
    struct uvwasi_fd_wrap_t* wrap;
    uv_buf_t* bufs;
    uv_fs_t req;
    ...
    UVWASI_DEBUG("uvwasi_fd_write(uvwasi=%p, fd=%d, iovs=%p, iovs_len=%d, "
                 "nwritten=%p)\n",
        uvwasi, fd, iovs, iovs_len, nwritten);
...
uvwasi_errno_t uvwasi_path_create_directory(uvwasi_t* uvwasi, uvwasi_fd_t fd, const char* path, uvwasi_size_t path_len)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uv_fs_t req;
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_CREATE_DIRECTORY, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_path_filestat_get(
    uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_lookupflags_t flags, const char* path, uvwasi_size_t path_len, uvwasi_filestat_t* buf)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uv_fs_t req;
    ...
    UVWASI_DEBUG("uvwasi_path_filestat_get(uvwasi=%p, fd=%d, flags=%d, "
        ...
        uvwasi, fd, flags, path, path_len, buf);

    if (uvwasi == NULL || path == NULL || buf == NULL)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_FILESTAT_GET, 0);
    if (err != UVWASI_ESUCCESS)
        return err;

    err = uvwasi__resolve_path(uvwasi, wrap, path, path_len, &resolved_path, flags);
    if (err != UVWASI_ESUCCESS)
        goto exit;
...
uvwasi_errno_t uvwasi_path_filestat_set_times(uvwasi_t* uvwasi, uvwasi_fd_t fd, uvwasi_lookupflags_t flags, const char* path, uvwasi_size_t path_len,
    uvwasi_timestamp_t st_atim, uvwasi_timestamp_t st_mtim, uvwasi_fstflags_t fst_flags)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_timestamp_t atim;
    ...
    UVWASI_DEBUG("uvwasi_path_filestat_set_times(uvwasi=%p, fd=%d, "
                 ...
                 "st_atim=%" PRIu64 ", st_mtim=%" PRIu64 ", fst_flags=%d)\n",
        uvwasi, fd, flags, path, path_len, st_atim, st_mtim, fst_flags);

    if (uvwasi == NULL || path == NULL)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_FILESTAT_SET_TIMES, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
    ...
    atim = st_atim;
    mtim = st_mtim;
    err = uvwasi__get_filestat_set_times(&atim, &mtim, fst_flags, NULL, resolved_path);
    if (err != UVWASI_ESUCCESS) {
        uvwasi__free(uvwasi, resolved_path);
        goto exit;
    }
...
uvwasi_errno_t uvwasi_path_link(uvwasi_t* uvwasi, uvwasi_fd_t old_fd, uvwasi_lookupflags_t old_flags, const char* old_path, uvwasi_size_t old_path_len,
    uvwasi_fd_t new_fd, const char* new_path, uvwasi_size_t new_path_len)
{
    ...
    UVWASI_DEBUG("uvwasi_path_link(uvwasi=%p, old_fd=%d, old_flags=%d, "
                 ...
                 "new_path_len=%d)\n",
        uvwasi, old_fd, old_flags, old_path, old_path_len, new_fd, new_path, new_path_len);
    ...
    if (old_fd == new_fd) {
        err = uvwasi_fd_table_get_nolock(uvwasi->fds, old_fd, &old_wrap, UVWASI_RIGHT_PATH_LINK_SOURCE | UVWASI_RIGHT_PATH_LINK_TARGET, 0);
        new_wrap = old_wrap;
    } else {
        err = uvwasi_fd_table_get_nolock(uvwasi->fds, old_fd, &old_wrap, UVWASI_RIGHT_PATH_LINK_SOURCE, 0);
        if (err != UVWASI_ESUCCESS) {
            uvwasi_fd_table_unlock(uvwasi->fds);
            return err;
        }

        err = uvwasi_fd_table_get_nolock(uvwasi->fds, new_fd, &new_wrap, UVWASI_RIGHT_PATH_LINK_TARGET, 0);
        if (err != UVWASI_ESUCCESS)
    ...
    err = uvwasi__resolve_path(uvwasi, old_wrap, old_path, old_path_len, &resolved_old_path, old_flags);
    if (err != UVWASI_ESUCCESS)
        goto exit;

    err = uvwasi__resolve_path(uvwasi, new_wrap, new_path, new_path_len, &resolved_new_path, 0);
    if (err != UVWASI_ESUCCESS)
        goto exit;
...
uvwasi_errno_t uvwasi_path_open(uvwasi_t* uvwasi, uvwasi_fd_t dirfd, uvwasi_lookupflags_t dirflags, const char* path, uvwasi_size_t path_len,
    uvwasi_oflags_t o_flags, uvwasi_rights_t fs_rights_base, uvwasi_rights_t fs_rights_inheriting, uvwasi_fdflags_t fs_flags, uvwasi_fd_t* fd)
{
    char* resolved_path;
    ...
    UVWASI_DEBUG("uvwasi_path_open(uvwasi=%p, dirfd=%d, dirflags=%d, path='%s', "
        ...
        uvwasi, dirfd, dirflags, path, path_len, o_flags, fs_rights_base, fs_rights_inheriting, fs_flags, fd);

    if (uvwasi == NULL || path == NULL || fd == NULL)
        return UVWASI_EINVAL;

    read = 0 != (fs_rights_base & (UVWASI_RIGHT_FD_READ | UVWASI_RIGHT_FD_READDIR));
    write = 0 != (fs_rights_base & (UVWASI_RIGHT_FD_DATASYNC | UVWASI_RIGHT_FD_WRITE | UVWASI_RIGHT_FD_ALLOCATE | UVWASI_RIGHT_FD_FILESTAT_SET_SIZE));
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, dirfd, &dirfd_wrap, needed_base, needed_inheriting);
    if (err != UVWASI_ESUCCESS)
        return err;

    err = uvwasi__resolve_path(uvwasi, dirfd_wrap, path, path_len, &resolved_path, dirflags);
    if (err != UVWASI_ESUCCESS) {
    ...
    if ((o_flags & UVWASI_O_DIRECTORY) != 0 && filetype != UVWASI_FILETYPE_DIRECTORY) {
        err = UVWASI_ENOTDIR;
        goto close_file_and_error_exit;
    }
    ...
    err = uvwasi_fd_table_insert(
        uvwasi, uvwasi->fds, r, NULL, resolved_path, resolved_path, filetype, fs_rights_base & max_base, fs_rights_inheriting & max_inheriting, 0, &wrap);
    if (err != UVWASI_ESUCCESS)
        goto close_file_and_error_exit;
...
uvwasi_errno_t uvwasi_path_readlink(
    uvwasi_t* uvwasi, uvwasi_fd_t fd, const char* path, uvwasi_size_t path_len, char* buf, uvwasi_size_t buf_len, uvwasi_size_t* bufused)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err;
    ...
    UVWASI_DEBUG("uvwasi_path_readlink(uvwasi=%p, fd=%d, path='%s', path_len=%d, "
        ...
        uvwasi, fd, path, path_len, buf, buf_len, bufused);

    if (uvwasi == NULL || path == NULL || buf == NULL || bufused == NULL)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_READLINK, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_path_remove_directory(uvwasi_t* uvwasi, uvwasi_fd_t fd, const char* path, uvwasi_size_t path_len)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uv_fs_t req;
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_REMOVE_DIRECTORY, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_path_rename(uvwasi_t* uvwasi, uvwasi_fd_t old_fd, const char* old_path, uvwasi_size_t old_path_len, uvwasi_fd_t new_fd,
    const char* new_path, uvwasi_size_t new_path_len)
{
    char* resolved_old_path;
    char* resolved_new_path;
    struct uvwasi_fd_wrap_t* old_wrap;
    ...
    UVWASI_DEBUG("uvwasi_path_rename(uvwasi=%p, old_fd=%d, old_path='%s', "
        ...
        uvwasi, old_fd, old_path, old_path_len, new_fd, new_path, new_path_len);
    ...
    if (old_fd == new_fd) {
        err = uvwasi_fd_table_get_nolock(uvwasi->fds, old_fd, &old_wrap, UVWASI_RIGHT_PATH_RENAME_SOURCE | UVWASI_RIGHT_PATH_RENAME_TARGET, 0);
        new_wrap = old_wrap;
    } else {
        err = uvwasi_fd_table_get_nolock(uvwasi->fds, old_fd, &old_wrap, UVWASI_RIGHT_PATH_RENAME_SOURCE, 0);
        if (err != UVWASI_ESUCCESS) {
            uvwasi_fd_table_unlock(uvwasi->fds);
            return err;
        }

        err = uvwasi_fd_table_get_nolock(uvwasi->fds, new_fd, &new_wrap, UVWASI_RIGHT_PATH_RENAME_TARGET, 0);
        if (err != UVWASI_ESUCCESS)
            uv_mutex_unlock(&old_wrap->mutex);
    }
    ...
    err = uvwasi__resolve_path(uvwasi, old_wrap, old_path, old_path_len, &resolved_old_path, 0);
    if (err != UVWASI_ESUCCESS)
        goto exit;
...
uvwasi_errno_t uvwasi_path_symlink(
    uvwasi_t* uvwasi, const char* old_path, uvwasi_size_t old_path_len, uvwasi_fd_t fd, const char* new_path, uvwasi_size_t new_path_len)
{
    ...
    UVWASI_DEBUG("uvwasi_path_symlink(uvwasi=%p, old_path='%s', old_path_len=%d, "
        ...
        uvwasi, old_path, old_path_len, fd, new_path, new_path_len);

    if (uvwasi == NULL || old_path == NULL || new_path == NULL)
        return UVWASI_EINVAL;

    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_SYMLINK, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
    ...
    err = uvwasi__resolve_path(uvwasi, wrap, new_path, new_path_len, &resolved_new_path, 0);
    if (err != UVWASI_ESUCCESS) {
        uv_mutex_unlock(&wrap->mutex);
...
uvwasi_errno_t uvwasi_path_unlink_file(uvwasi_t* uvwasi, uvwasi_fd_t fd, const char* path, uvwasi_size_t path_len)
{
    char* resolved_path;
    struct uvwasi_fd_wrap_t* wrap;
    uv_fs_t req;
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, fd, &wrap, UVWASI_RIGHT_PATH_UNLINK_FILE, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_poll_oneoff(uvwasi_t* uvwasi, const uvwasi_subscription_t* in, uvwasi_event_t* out, uvwasi_size_t nsubscriptions, uvwasi_size_t* nevents)
{
    ...
    UVWASI_DEBUG("uvwasi_poll_oneoff(uvwasi=%p, in=%p, out=%p, "
        ...
        uvwasi, in, out, nsubscriptions, nevents);

    if (uvwasi == NULL || in == NULL || out == NULL || nsubscriptions == 0 || nevents == NULL) {
        return UVWASI_EINVAL;
    }
...
uvwasi_errno_t uvwasi_random_get(uvwasi_t* uvwasi, void* buf, uvwasi_size_t buf_len)
{
    int r;
...
uvwasi_errno_t uvwasi_sock_recv(uvwasi_t* uvwasi, uvwasi_fd_t sock, const uvwasi_iovec_t* ri_data, uvwasi_size_t ri_data_len, uvwasi_riflags_t ri_flags,
    uvwasi_size_t* ro_datalen, uvwasi_roflags_t* ro_flags)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err = 0;
    ...
    UVWASI_DEBUG("uvwasi_sock_recv(uvwasi=%p, sock=%d, ri_data=%p, "
        ...
        uvwasi, sock, ri_data, ri_data_len, ri_flags, ro_datalen, ro_flags);
    ...
    err = uvwasi_fd_table_get(uvwasi->fds, sock, &wrap, UVWASI__RIGHTS_SOCKET_BASE, 0);
    if (err != UVWASI_ESUCCESS)
        return err;
...
uvwasi_errno_t uvwasi_sock_send(
    uvwasi_t* uvwasi, uvwasi_fd_t sock, const uvwasi_ciovec_t* si_data, uvwasi_size_t si_data_len, uvwasi_siflags_t si_flags, uvwasi_size_t* so_datalen)
{
    ...
    UVWASI_DEBUG("uvwasi_sock_send(uvwasi=%p, sock=%d, si_data=%p, "
        ...
        uvwasi, sock, si_data, si_data_len, si_flags, so_datalen);

    if (uvwasi == NULL || si_data == NULL || so_datalen == NULL || si_flags != 0)
        return UVWASI_EINVAL;
...
uvwasi_errno_t uvwasi_sock_shutdown(uvwasi_t* uvwasi, uvwasi_fd_t sock, uvwasi_sdflags_t how)
{
    struct uvwasi_fd_wrap_t* wrap;
    uvwasi_errno_t err = 0;
...
uvwasi_errno_t uvwasi_sock_accept(uvwasi_t* uvwasi, uvwasi_fd_t sock, uvwasi_fdflags_t flags, uvwasi_fd_t* connect_sock)
{
    struct uvwasi_fd_wrap_t* wrap;
    ...
    UVWASI_DEBUG("uvwasi_sock_accept(uvwasi=%p, sock=%d, flags=%d, "
                 "connect_sock=%p)\n",
        uvwasi, sock, flags, connect_sock);
    ...
    err = uvwasi_fd_table_insert(uvwasi, uvwasi->fds, -1, uv_connect_sock, NULL, NULL, UVWASI_FILETYPE_SOCKET_STREAM, UVWASI__RIGHTS_SOCKET_BASE,
        UVWASI__RIGHTS_SOCKET_INHERITING, 1, &connected_wrap);
