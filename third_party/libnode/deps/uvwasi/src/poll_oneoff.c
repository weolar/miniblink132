...
uvwasi_errno_t uvwasi__poll_oneoff_state_init(uvwasi_t* uvwasi, struct uvwasi_poll_oneoff_state_t* state, uvwasi_size_t max_fds)
{
    uvwasi_errno_t err;
    int r;
    ...
    if (max_fds > 0) {
        state->fdevents = uvwasi__calloc(uvwasi, max_fds, sizeof(*state->fdevents));
        if (state->fdevents == NULL) {
            err = UVWASI_ENOMEM;
            goto error_exit;
        }

        state->poll_handles = uvwasi__calloc(uvwasi, max_fds, sizeof(*state->poll_handles));
...
uvwasi_errno_t uvwasi__poll_oneoff_state_cleanup(struct uvwasi_poll_oneoff_state_t* state)
{
    struct uvwasi__poll_fdevent_t* event;
    uvwasi_size_t i;
    int r;
...
uvwasi_errno_t uvwasi__poll_oneoff_state_set_timer(struct uvwasi_poll_oneoff_state_t* state, uvwasi_timestamp_t timeout)
{
    int r;
...
uvwasi_errno_t uvwasi__poll_oneoff_state_add_fdevent(struct uvwasi_poll_oneoff_state_t* state, uvwasi_subscription_t* subscription)
{
    struct uvwasi__poll_fdevent_t* event;
    ...
    if (err == UVWASI_ESUCCESS) {
        ...
        if (r != 0) {
         ...
         do any polling with the handle. */
            ...
            r = uv_poll_start(poll_handle, event->events, poll_cb);
            if (r != 0) {
                uv_mutex_unlock(&event->wrap->mutex);
            ...
            uv_handle_set_data((uv_handle_t*)poll_handle, (void*)&state->fdevents[state->fdevent_cnt]);
...
uvwasi_errno_t uvwasi__poll_oneoff_run(struct uvwasi_poll_oneoff_state_t* state)
{
    int r;
