...
#define UVWASI__WIN_TIME_AND_RETURN(handle, get_times, time)                                                                                                   \
    do {                                                                                                                                                       \
        ...
        if (0 == get_times((handle), &create, &exit, &system, &user)) {                                                                                        \
            return uvwasi__translate_uv_error(uv_translate_sys_error(GetLastError()));                                                                         \
        }                                                                                                                                                      \
                                                                                                                                                               \
        /* FILETIME times are in units of 100 nanoseconds */                                                                                                   \
        (time) = (((uvwasi_timestamp_t)system.dwHighDateTime << 32 | system.dwLowDateTime) * 100                                                               \
            + ((uvwasi_timestamp_t)user.dwHighDateTime << 32 | user.dwLowDateTime) * 100);                                                                     \
...
#define UVWASI__GETRUSAGE_AND_RETURN(who, time)                                                                                                                \
    do {                                                                                                                                                       \
        ...
        if (0 != getrusage((who), &ru))                                                                                                                        \
            return uvwasi__translate_uv_error(uv_translate_sys_error(errno));                                                                                  \
        (time) = ((uvwasi_timestamp_t)(ru.ru_utime.tv_sec) * NANOS_PER_SEC) + (ru.ru_utime.tv_usec * 1000)                                                     \
            + ((uvwasi_timestamp_t)(ru.ru_stime.tv_sec) * NANOS_PER_SEC) + (ru.ru_stime.tv_usec * 1000);                                                       \
        return UVWASI_ESUCCESS;                                                                                                                                \
    } while (0)
...
#define UVWASI__OSX_THREADTIME_AND_RETURN(time)                                                                                                                \
    do {                                                                                                                                                       \
        ...
        if (KERN_SUCCESS != thread_info(thread, THREAD_BASIC_INFO, (thread_info_t)&info, &count)) {                                                            \
            return UVWASI_ENOSYS;                                                                                                                              \
        }                                                                                                                                                      \
        (time) = ((uvwasi_timestamp_t)(info.user_time.seconds) * NANOS_PER_SEC) + (info.user_time.microseconds * 1000)                                         \
            + ((uvwasi_timestamp_t)(info.system_time.seconds) * NANOS_PER_SEC) + (info.system_time.microseconds * 1000);                                       \
        return UVWASI_ESUCCESS;                                                                                                                                \
    } while (0)
...
#elif defined(CLOCK_PROCESS_CPUTIME_ID) && !defined(__APPLE__) && !defined(__sun)
