...
#define uv__queue_data(pointer, type, field) ((type*)((char*)(pointer)-offsetof(type, field)))

#define uv__queue_foreach(q, h) for ((q) = (h)->next; (q) != (h); (q) = (q)->next)
...
static inline void uv__queue_split(struct uv__queue* h, struct uv__queue* q, struct uv__queue* n)
{
    n->prev = h->prev;
    n->prev->next = n;
    n->next = q;
...
static inline void uv__queue_insert_head(struct uv__queue* h, struct uv__queue* q)
{
    q->next = h->next;
    q->prev = h;
    q->next->prev = q;
