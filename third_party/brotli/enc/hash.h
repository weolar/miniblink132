...
static const uint64_t kCutoffTransforms = BROTLI_MAKE_UINT64_T(0x071B520A, 0xDA2D3200);
...
static const uint64_t kHashMul64Long = BROTLI_MAKE_UINT64_T(0x1FE35A7Bu, 0xD3579BD3u);
...
static BROTLI_INLINE void PrepareDistanceCache(int* BROTLI_RESTRICT distance_cache, const int num_distances)
{
    if (num_distances > 4) {
...
static BROTLI_INLINE score_t BackwardReferenceScore(size_t copy_length, size_t backward_reference_offset)
{
    return BROTLI_SCORE_BASE + BROTLI_LITERAL_BYTE_SCORE * (score_t)copy_length - BROTLI_DISTANCE_BIT_PENALTY * Log2FloorNonZero(backward_reference_offset);
}

static BROTLI_INLINE score_t BackwardReferenceScoreUsingLastDistance(size_t copy_length)
{
    return BROTLI_LITERAL_BYTE_SCORE * (score_t)copy_length + BROTLI_SCORE_BASE + 15;
}

static BROTLI_INLINE score_t BackwardReferencePenaltyUsingLastDistance(size_t distance_short_code)
{
    return (score_t)39 + ((0x1CA10 >> (distance_short_code & 0xE)) & 0xE);
}

static BROTLI_INLINE BROTLI_BOOL TestStaticDictionaryItem(const BrotliEncoderDictionary* dictionary, size_t len, size_t word_idx, const uint8_t* data,
    size_t max_length, size_t max_backward, size_t max_distance, HasherSearchResult* out)
{
    size_t offset;
    size_t matchlen;
    size_t backward;
...
static BROTLI_INLINE void SearchInStaticDictionary(const BrotliEncoderDictionary* dictionary, HasherCommon* common, const uint8_t* data, size_t max_length,
    size_t max_backward, size_t max_distance, HasherSearchResult* out, BROTLI_BOOL shallow)
{
    size_t key;
    size_t i;
...
static BROTLI_INLINE void InitBackwardMatch(BackwardMatch* self, size_t dist, size_t len)
{
    self->distance = (uint32_t)dist;
    self->length_and_code = (uint32_t)(len << 5);
}

static BROTLI_INLINE void InitDictionaryBackwardMatch(BackwardMatch* self, size_t dist, size_t len, size_t len_code)
{
    self->distance = (uint32_t)dist;
...
{
    if (hasher->common.extra[0] != NULL)
        BROTLI_FREE(m, hasher->common.extra[0]);
    if (hasher->common.extra[1] != NULL)
        BROTLI_FREE(m, hasher->common.extra[1]);
    if (hasher->common.extra[2] != NULL)
        BROTLI_FREE(m, hasher->common.extra[2]);
    if (hasher->common.extra[3] != NULL)
        BROTLI_FREE(m, hasher->common.extra[3]);
}
...
static BROTLI_INLINE void HasherSize(const BrotliEncoderParams* params, BROTLI_BOOL one_shot, const size_t input_size, size_t* alloc_size)
{
...
static BROTLI_INLINE void HasherSetup(
    MemoryManager* m, Hasher* hasher, BrotliEncoderParams* params, const uint8_t* data, size_t position, size_t input_size, BROTLI_BOOL is_last)
{
    ...
    if (!hasher->common.is_setup_) {
        ...
        for (i = 0; i < 4; ++i) {
            ...
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(hasher->common.extra[i]))
                return;
        }
        switch (hasher->common.params.type) {
#define INITIALIZE_(N)                                                                                                                                         \
    case N:                                                                                                                                                    \
        InitializeH##N(&hasher->common, &hasher->privat._H##N, params);                                                                                        \
        break;
...
#define PREPARE_(N)                                                                                                                                            \
    case N:                                                                                                                                                    \
        PrepareH##N(&hasher->privat._H##N, one_shot, input_size, data);                                                                                        \
        break;
...
static BROTLI_INLINE void InitOrStitchToPreviousBlock(
    MemoryManager* m, Hasher* hasher, const uint8_t* data, size_t mask, BrotliEncoderParams* params, size_t position, size_t input_size, BROTLI_BOOL is_last)
{
...
#define INIT_(N)                                                                                                                                               \
    case N:                                                                                                                                                    \
        StitchToPreviousBlockH##N(&hasher->privat._H##N, input_size, position, data, mask);                                                                    \
        break;
...
static BROTLI_INLINE void FindCompoundDictionaryMatch(const PreparedDictionary* self, const uint8_t* BROTLI_RESTRICT data, const size_t ring_buffer_mask,
    const int* BROTLI_RESTRICT distance_cache, const size_t cur_ix, const size_t max_length, const size_t distance_offset, const size_t max_distance,
    HasherSearchResult* BROTLI_RESTRICT out)
{
    ...
    size_t i;
    const uint64_t h = (BROTLI_UNALIGNED_LOAD64LE(&data[cur_ix_masked]) & hash_mask) * kPreparedDictionaryHashMul64Long;
    ...
    for (i = 0; i < 4; ++i) {
        ...
        len = FindMatchLengthWithLimit(&source[offset], &data[cur_ix_masked], limit);
        if (len >= 2) {
    ...
    while (item == 0) {
        ...
        if (distance > max_distance)
            continue;
        if (cur_ix_masked + best_len > ring_buffer_mask || best_len >= limit || data[cur_ix_masked + best_len] != source[offset + best_len]) {
            continue;
        }
        {
            const size_t len = FindMatchLengthWithLimit(&source[offset], &data[cur_ix_masked], limit);
            if (len >= 4) {
...
static BROTLI_INLINE size_t FindAllCompoundDictionaryMatches(const PreparedDictionary* self, const uint8_t* BROTLI_RESTRICT data, const size_t ring_buffer_mask,
    const size_t cur_ix, const size_t min_length, const size_t max_length, const size_t distance_offset, const size_t max_distance, BackwardMatch* matches,
    size_t match_limit)
{
    const uint32_t source_offset = self->source_offset;
    const uint32_t source_size = self->source_size;
...
static BROTLI_INLINE void LookupCompoundDictionaryMatch(const CompoundDictionary* addon, const uint8_t* BROTLI_RESTRICT data, const size_t ring_buffer_mask,
    const int* BROTLI_RESTRICT distance_cache, const size_t cur_ix, const size_t max_length, const size_t max_ring_buffer_distance, const size_t max_distance,
    HasherSearchResult* sr)
{
    ...
    for (d = 0; d < addon->num_chunks; ++d) {
        ...
        FindCompoundDictionaryMatch((const PreparedDictionary*)addon->chunks[d], data, ring_buffer_mask, distance_cache, cur_ix, max_length,
            base_offset - addon->chunk_offsets[d], max_distance, sr);
    }
}

static BROTLI_INLINE size_t LookupAllCompoundDictionaryMatches(const CompoundDictionary* addon, const uint8_t* BROTLI_RESTRICT data,
    const size_t ring_buffer_mask, const size_t cur_ix, size_t min_length, const size_t max_length, const size_t max_ring_buffer_distance,
    const size_t max_distance, BackwardMatch* matches, size_t match_limit)
{
    ...
    for (d = 0; d < addon->num_chunks; ++d) {
        /* Only one prepared dictionary type is currently supported. */
        total_found += FindAllCompoundDictionaryMatches((const PreparedDictionary*)addon->chunks[d], data, ring_buffer_mask, cur_ix, min_length, max_length,
            base_offset - addon->chunk_offsets[d], max_distance, matches + total_found, match_limit - total_found);
