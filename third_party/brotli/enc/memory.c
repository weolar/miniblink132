...
static size_t Annihilate(void** a, size_t a_len, void** b, size_t b_len)
{
    ...
    while (a_read_index < a_len)
        a[a_write_index++] = a[a_read_index++];
    while (b_read_index < b_len)
        b[b_write_index++] = b[b_read_index++];
    return annihilated;
}
...
static void CollectGarbagePointers(MemoryManager* m)
{
    ...
    annihilated = Annihilate(m->pointers + NEW_ALLOCATED_OFFSET, m->new_allocated, m->pointers + NEW_FREED_OFFSET, m->new_freed);
    m->new_allocated -= annihilated;
    m->new_freed -= annihilated;

    if (m->new_freed != 0) {
        annihilated = Annihilate(m->pointers + PERM_ALLOCATED_OFFSET, m->perm_allocated, m->pointers + NEW_FREED_OFFSET, m->new_freed);
    ...
    if (m->new_allocated != 0) {
        ...
        memcpy(m->pointers + PERM_ALLOCATED_OFFSET + m->perm_allocated, m->pointers + NEW_ALLOCATED_OFFSET, sizeof(void*) * m->new_allocated);
...
void* BrotliAllocate(MemoryManager* m, size_t n)
{
    ...
    if (m->new_allocated == MAX_NEW_ALLOCATED)
