...
static BROTLI_INLINE void FN(ClearHistograms)(FN(Histogram) * array, size_t length)
{
    size_t i;
    for (i = 0; i < length; ++i)
...
static BROTLI_INLINE void FN(HistogramAddVector)(FN(Histogram) * self, const DataType* p, size_t n)
{
    self->total_count_ += n;
    n += 1;
    while (--n)
...
static BROTLI_INLINE void FN(HistogramAddHistogram)(FN(Histogram) * self, const FN(Histogram) * v)
{
    size_t i;
