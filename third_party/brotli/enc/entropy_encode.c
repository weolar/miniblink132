...
static void BrotliWriteHuffmanTreeRepetitions(
    const uint8_t previous_value, const uint8_t value, size_t repetitions, size_t* tree_size, uint8_t* tree, uint8_t* extra_bits_data)
{
...
static void DecideOverRleUse(const uint8_t* depth, const size_t length, BROTLI_BOOL* use_rle_for_non_zero, BROTLI_BOOL* use_rle_for_zero)
{
    ...
    *use_rle_for_non_zero = TO_BROTLI_BOOL(total_reps_non_zero > count_reps_non_zero * 2);
