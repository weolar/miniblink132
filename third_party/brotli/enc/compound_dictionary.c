...
static PreparedDictionary* CreatePreparedDictionaryWithParams(
    MemoryManager* m, const uint8_t* source, size_t source_size, uint32_t bucket_bits, uint32_t slot_bits, uint32_t hash_bits, uint16_t bucket_limit)
{
    ...
    size_t alloc_size = (sizeof(uint32_t) << slot_bits) + (sizeof(uint32_t) << slot_bits) + (sizeof(uint16_t) << bucket_bits)
        + (sizeof(uint32_t) << bucket_bits) + (sizeof(uint32_t) * source_size);
    uint8_t* flat = NULL;
    ...
    if (slot_bits > bucket_bits)
        return NULL;
    if (bucket_bits - slot_bits >= 16)
        return NULL;

    flat = BROTLI_ALLOC(m, uint8_t, alloc_size);
    if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(flat))
        return NULL;
    ...
    for (i = 0; i + 7 < source_size; ++i) {
        const uint64_t h = (BROTLI_UNALIGNED_LOAD64LE(&source[i]) & hash_mask) * kPreparedDictionaryHashMul64Long;
    ...
    alloc_size
        = sizeof(PreparedDictionary) + (sizeof(uint32_t) << slot_bits) + (sizeof(uint16_t) << bucket_bits) + (sizeof(uint32_t) * total_items) + source_size;
    ...
    for (i = 0; i < num_buckets; ++i) {
        ...
        if (count > slot_limit[slot])
            count = slot_limit[slot];
        if (count == 0) {
            heads[i] = 0xFFFF;
            continue;
        }
...
PreparedDictionary* CreatePreparedDictionary(MemoryManager* m, const uint8_t* source, size_t source_size)
{
    ...
    return CreatePreparedDictionaryWithParams(m, source, source_size, bucket_bits, slot_bits, hash_bits, bucket_limit);
}
...
BROTLI_BOOL AttachPreparedDictionary(CompoundDictionary* compound, const PreparedDictionary* dictionary)
{
    ...
    compound->chunk_offsets[index + 1] = compound->total_size;
    {
        ...
        compound->chunk_source[index] = (const uint8_t*)(&items[dictionary->source_offset]);
    }
    compound->num_chunks++;
    return BROTLI_TRUE;
}
