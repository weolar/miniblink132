...
{
    return params->quality <= 10 ? MAX_ZOPFLI_LEN_QUALITY_10 : MAX_ZOPFLI_LEN_QUALITY_11;
}

/* Number of best candidates to evaluate to expand Zopfli chain. */
static BROTLI_INLINE size_t MaxZopfliCandidates(const BrotliEncoderParams* params)
{
...
{
    params->quality = BROTLI_MIN(int, BROTLI_MAX_QUALITY, BROTLI_MAX(int, BROTLI_MIN_QUALITY, params->quality));
    ...
    if (params->lgwin < BROTLI_MIN_WINDOW_BITS) {
        ...
        int max_lgwin = params->large_window ? BROTLI_LARGE_MAX_WINDOW_BITS : BROTLI_MAX_WINDOW_BITS;
...
static BROTLI_INLINE int ComputeLgBlock(const BrotliEncoderParams* params)
{
    ...
    if (params->quality == FAST_ONE_PASS_COMPRESSION_QUALITY || params->quality == FAST_TWO_PASS_COMPRESSION_QUALITY) {
        ...
        lgblock = BROTLI_MIN(int, BROTLI_MAX_INPUT_BLOCK_BITS, BROTLI_MAX(int, BROTLI_MIN_INPUT_BLOCK_BITS, lgblock));
    }
    return lgblock;
}
...
{
    int bits = BROTLI_MIN(int, ComputeRbBits(params), BROTLI_MAX_INPUT_BLOCK_BITS);
...
static BROTLI_INLINE size_t LiteralSpreeLengthForSparseSearch(const BrotliEncoderParams* params)
{
    return params->quality < 9 ? 64 : 512;
}

static BROTLI_INLINE void ChooseHasher(const BrotliEncoderParams* params, BrotliHasherParams* hparams)
{
    if (params->quality > 9) {
        hparams->type = 10;
