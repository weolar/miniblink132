...
static uint8_t* GetBrotliStorage(BrotliEncoderState* s, size_t size)
{
    ...
    if (s->storage_size_ < size) {
        ...
        if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(s->storage_))
            return NULL;
...
static int* GetHashTable(BrotliEncoderState* s, int quality, size_t input_size, size_t* table_size)
{
    ...
    if (htsize <= sizeof(s->small_table_) / sizeof(s->small_table_[0])) {
        ...
        if (htsize > s->large_table_size_) {
            ...
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(s->large_table_))
                return 0;
        }
...
static BROTLI_BOOL ShouldUseComplexStaticContextMap(const uint8_t* input, size_t start_pos, size_t length, size_t mask, int quality, size_t size_hint,
    size_t* num_literal_contexts, const uint32_t** literal_context_map, uint32_t* arena)
{
    static const uint32_t kStaticContextMapComplexUTF8[64] = {
        ...
        2,
        2,
        2, /* !, first after space/lf and after something else. */
        1,
        1,
        1,
        1, /* " */
        ...
        for (; start_pos + 64 <= end_pos; start_pos += 4096) {
         ...
         strides at every 4kB intervals. */
            for (pos = start_pos + 2; pos < stride_end_pos; ++pos) {
                ...
                const uint8_t context = (uint8_t)kStaticContextMapComplexUTF8[BROTLI_CONTEXT(prev1, prev2, utf8_lut)];
                ++total;
...
static void DecideOverLiteralContextModeling(const uint8_t* input, size_t start_pos, size_t length, size_t mask, int quality, size_t size_hint,
    size_t* num_literal_contexts, const uint32_t** literal_context_map, uint32_t* arena)
{
    if (quality < MIN_QUALITY_FOR_CONTEXT_MODELING || length < 64) {
        return;
    } else if (ShouldUseComplexStaticContextMap(input, start_pos, length, mask, quality, size_hint, num_literal_contexts, literal_context_map, arena)) {
...
static BROTLI_BOOL ShouldCompress(
    const uint8_t* data, const size_t mask, const uint64_t last_flush_pos, const size_t bytes, const size_t num_literals, const size_t num_commands)
{
    ...
    if (num_commands < (bytes >> 8) + 2) {
        if ((double)num_literals > 0.99 * (double)bytes) {
            ...
            const double bit_cost_threshold = (double)bytes * kMinEntropy / kSampleRate;
...
static void WriteMetaBlockInternal(MemoryManager* m, const uint8_t* data, const size_t mask, const uint64_t last_flush_pos, const size_t bytes,
    const BROTLI_BOOL is_last, ContextType literal_context_mode, const BrotliEncoderParams* params, const uint8_t prev_byte, const uint8_t prev_byte2,
    const size_t num_literals, const size_t num_commands, Command* commands, const int* saved_dist_cache, int* dist_cache, size_t* storage_ix, uint8_t* storage)
{
    ...
    if (params->quality <= MAX_QUALITY_FOR_STATIC_ENTROPY_CODES) {
        ...
        if (params->quality < MIN_QUALITY_FOR_HQ_BLOCK_SPLITTING) {
            ...
            if (!params->disable_literal_context_modeling) {
                ...
                if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(arena))
                    return;
                DecideOverLiteralContextModeling(
                    data, wrapped_last_flush_pos, bytes, mask, params->quality, params->size_hint, &num_literal_contexts, &literal_context_map, arena);
                BROTLI_FREE(m, arena);
            }
            BrotliBuildMetaBlockGreedy(m, data, wrapped_last_flush_pos, mask, prev_byte, prev_byte2, literal_context_lut, num_literal_contexts,
                literal_context_map, commands, num_commands, &mb);
            if (BROTLI_IS_OOM(m))
                return;
        } else {
            BrotliBuildMetaBlock(
                m, data, wrapped_last_flush_pos, mask, &block_params, prev_byte, prev_byte2, commands, num_commands, literal_context_mode, &mb);
            if (BROTLI_IS_OOM(m))
                return;
        }
        ...
        BrotliStoreMetaBlock(m, data, wrapped_last_flush_pos, bytes, mask, prev_byte, prev_byte2, is_last, &block_params, literal_context_mode, commands,
            num_commands, &mb, storage_ix, storage);
        if (BROTLI_IS_OOM(m))
            return;
...
static void ChooseDistanceParams(BrotliEncoderParams* params)
{
    ...
    if (params->quality >= MIN_QUALITY_FOR_NONZERO_DISTANCE_PARAMS) {
        ...
        if (distance_postfix_bits > BROTLI_MAX_NPOSTFIX || num_direct_distance_codes > BROTLI_MAX_NDIRECT
...
static BROTLI_BOOL EnsureInitialized(BrotliEncoderState* s)
{
    ...
    /* Initialize last byte with stream header. */
    {
        ...
        if (s->params.stream_offset == 0) {
            ...
            s->params.stream_offset = BROTLI_MIN(size_t, s->params.stream_offset, BROTLI_MAX_BACKWARD_LIMIT(lgwin));
        }
    }
...
static void BrotliEncoderCleanupParams(MemoryManager* m, BrotliEncoderParams* params)
{
...
{
    BrotliEncoderState* state = (BrotliEncoderState*)BrotliBootstrapAlloc(sizeof(BrotliEncoderState), alloc_func, free_func, opaque);
    if (state == NULL) {
        /* BROTLI_DUMP(); */
        return 0;
    }
...
static void ExtendLastCommand(BrotliEncoderState* s, uint32_t* bytes, uint32_t* wrapped_last_processed_pos)
{
    ...
    uint64_t max_backward_distance = (((uint64_t)1) << s->params.lgwin) - BROTLI_WINDOW_GAP;
    ...
    if (distance_code < BROTLI_NUM_DISTANCE_SHORT_CODES || distance_code - (BROTLI_NUM_DISTANCE_SHORT_CODES - 1) == cmd_dist) {
        if (cmd_dist <= max_distance) {
            ...
            if ((cmd_dist - max_distance - 1) < compound_dictionary_size && last_copy_len < cmd_dist - max_distance) {
                size_t address = compound_dictionary_size - (size_t)(cmd_dist - max_distance) + (size_t)last_copy_len;
                size_t br_index = 0;
                size_t br_offset;
                const uint8_t* chunk;
                size_t chunk_length;
                while (address >= dict->chunk_offsets[br_index + 1])
                    br_index++;
                br_offset = address - dict->chunk_offsets[br_index];
                chunk = dict->chunk_source[br_index];
                chunk_length = dict->chunk_offsets[br_index + 1] - dict->chunk_offsets[br_index];
                while (*bytes != 0 && data[*wrapped_last_processed_pos & mask] == chunk[br_offset]) {
                    last_command->copy_len_++;
                    (*bytes)--;
                    (*wrapped_last_processed_pos)++;
        ...
        GetLengthCode(last_command->insert_len_, (size_t)((int)(last_command->copy_len_ & 0x1FFFFFF) + (int)(last_command->copy_len_ >> 25)),
            TO_BROTLI_BOOL((last_command->dist_prefix_ & 0x3FF) == 0), &last_command->cmd_prefix_);
    }
}
...
static BROTLI_BOOL EncodeData(BrotliEncoderState* s, const BROTLI_BOOL is_last, const BROTLI_BOOL force_flush, size_t* out_size, uint8_t** output)
{
    ...
    if (s->params.quality == FAST_TWO_PASS_COMPRESSION_QUALITY && !s->command_buf_) {
        s->command_buf_ = BROTLI_ALLOC(m, uint32_t, kCompressFragmentTwoPassBlockSize);
        s->literal_buf_ = BROTLI_ALLOC(m, uint8_t, kCompressFragmentTwoPassBlockSize);
        if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(s->command_buf_) || BROTLI_IS_NULL(s->literal_buf_)) {
            return BROTLI_FALSE;
        }
    }
    ...
    if (s->params.quality == FAST_ONE_PASS_COMPRESSION_QUALITY || s->params.quality == FAST_TWO_PASS_COMPRESSION_QUALITY) {
        ...
        if (BROTLI_IS_OOM(m))
            return BROTLI_FALSE;
        if (s->params.quality == FAST_ONE_PASS_COMPRESSION_QUALITY) {
            BrotliCompressFragmentFast(s->one_pass_arena_, &data[wrapped_last_processed_pos & mask], bytes, is_last, table, table_size, &storage_ix, storage);
            if (BROTLI_IS_OOM(m))
                return BROTLI_FALSE;
        } else {
            BrotliCompressFragmentTwoPass(s->two_pass_arena_, &data[wrapped_last_processed_pos & mask], bytes, is_last, s->command_buf_, s->literal_buf_, table,
                table_size, &storage_ix, storage);
            if (BROTLI_IS_OOM(m))
                return BROTLI_FALSE;
        }
...

    {
        ...
        if (newsize > s->cmd_alloc_size_) {
         ...
         without reallocation: that would impact speed. */
            ...
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(new_commands))
                return BROTLI_FALSE;
            if (s->commands_) {
    ...
    if (s->params.quality == ZOPFLIFICATION_QUALITY) {
        ...
        BrotliCreateZopfliBackwardReferences(m, bytes, wrapped_last_processed_pos, data, mask, literal_context_lut, &s->params, &s->hasher_, s->dist_cache_,
            &s->last_insert_len_, &s->commands_[s->num_commands_], &s->num_commands_, &s->num_literals_);
        if (BROTLI_IS_OOM(m))
            return BROTLI_FALSE;
    } else if (s->params.quality == HQ_ZOPFLIFICATION_QUALITY) {
        BROTLI_DCHECK(s->params.hasher.type == 10);
        BrotliCreateHqZopfliBackwardReferences(m, bytes, wrapped_last_processed_pos, data, mask, literal_context_lut, &s->params, &s->hasher_, s->dist_cache_,
            &s->last_insert_len_, &s->commands_[s->num_commands_], &s->num_commands_, &s->num_literals_);
        if (BROTLI_IS_OOM(m))
            return BROTLI_FALSE;
    } else {
        BrotliCreateBackwardReferences(bytes, wrapped_last_processed_pos, data, mask, literal_context_lut, &s->params, &s->hasher_, s->dist_cache_,
            &s->last_insert_len_, &s->commands_[s->num_commands_], &s->num_commands_, &s->num_literals_);
    }

    {
        ...
        const BROTLI_BOOL next_input_fits_metablock = TO_BROTLI_BOOL(processed_bytes + InputBlockSize(s) <= max_length);
        /* If block splitting is not used, then flush as soon as there is some
       amount of commands / literals produced. */
        const BROTLI_BOOL should_flush
            = TO_BROTLI_BOOL(s->params.quality < MIN_QUALITY_FOR_BLOCK_SPLIT && s->num_literals_ + s->num_commands_ >= MAX_NUM_DELAYED_SYMBOLS);
    ...
    BROTLI_DCHECK(s->input_pos_ - s->last_flush_pos_ <= 1u << 24);
    {
        ...
        storage[0] = (uint8_t)s->last_bytes_;
        storage[1] = (uint8_t)(s->last_bytes_ >> 8);
        WriteMetaBlockInternal(m, data, mask, s->last_flush_pos_, metablock_size, is_last, literal_context_mode, &s->params, s->prev_byte_, s->prev_byte2_,
            s->num_literals_, s->num_commands_, s->commands_, s->saved_dist_cache_, s->dist_cache_, &storage_ix, storage);
        if (BROTLI_IS_OOM(m))
            return BROTLI_FALSE;
        s->last_bytes_ = (uint16_t)(storage[storage_ix >> 3]);
        s->last_bytes_bits_ = storage_ix & 7u;
...
static BROTLI_NOINLINE BROTLI_BOOL BrotliCompressBufferQuality10(
    int lgwin, size_t input_size, const uint8_t* input_buffer, size_t* encoded_size, uint8_t* encoded_buffer)
{
    MemoryManager* m = (MemoryManager*)BrotliBootstrapAlloc(sizeof(MemoryManager), 0, 0, 0);
    ...
    const size_t hasher_eff_size = BROTLI_MIN(size_t, input_size, BROTLI_MAX_BACKWARD_LIMIT(lgwin) + BROTLI_WINDOW_GAP);
   ...
   * params->quality < params->dictionary.max_quality. */
    ...
    InitOrStitchToPreviousBlock(m, hasher, input_buffer, mask, params, 0, hasher_eff_size, BROTLI_TRUE);
    if (BROTLI_IS_OOM(m))
        goto oom;

    while (ok && metablock_start < input_size) {
        const size_t metablock_end = BROTLI_MIN(size_t, input_size, metablock_start + max_metablock_size);
        ...
        for (block_start = metablock_start; block_start < metablock_end;) {
            size_t block_size = BROTLI_MIN(size_t, metablock_end - block_start, max_block_size);
            ZopfliNode* nodes = BROTLI_ALLOC(m, ZopfliNode, block_size + 1);
            size_t path_size;
            size_t new_cmd_alloc_size;
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(nodes))
                goto oom;
         ...
         buffer size exponentially. */
            new_cmd_alloc_size = BROTLI_MAX(size_t, expected_num_commands, num_commands + path_size + 1);
        ...
        if (metablock_size == 0) {
            ...
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(storage))
                goto oom;
            storage[0] = (uint8_t)last_bytes;
            storage[1] = (uint8_t)(last_bytes >> 8);
            BrotliWriteBits(2, 3, &storage_ix, storage);
            storage_ix = (storage_ix + 7u) & ~7u;
        } else if (!ShouldCompress(input_buffer, mask, metablock_start, metablock_size, num_literals, num_commands)) {
            /* Restore the distance cache, as its last update by
         CreateBackwardReferences is now unused. */
            memcpy(dist_cache, saved_dist_cache, 4 * sizeof(dist_cache[0]));
            ...
            *block_params = *params; /* shallow copy */
            InitMetaBlockSplit(&mb);
            BrotliBuildMetaBlock(
                m, input_buffer, metablock_start, mask, block_params, prev_byte, prev_byte2, commands, num_commands, literal_context_mode, &mb);
            if (BROTLI_IS_OOM(m))
                goto oom;
            {
                /* The number of distance symbols effectively used for distance
           ...
           for "Large Window Brotli" (32-bit). */
            ...
            storage[0] = (uint8_t)last_bytes;
            storage[1] = (uint8_t)(last_bytes >> 8);
            BrotliStoreMetaBlock(m, input_buffer, metablock_start, metablock_size, mask, prev_byte, prev_byte2, is_last, block_params, literal_context_mode,
...
BROTLI_BOOL BrotliEncoderCompress(int quality, int lgwin, BrotliEncoderMode mode, size_t input_size, const uint8_t input_buffer[BROTLI_ARRAY_PARAM(input_size)],
    size_t* encoded_size, uint8_t encoded_buffer[BROTLI_ARRAY_PARAM(*encoded_size)])
{
    ...
    if (quality == 10) {
        ...
        const int lg_win = BROTLI_MIN(int, BROTLI_LARGE_MAX_WINDOW_BITS, BROTLI_MAX(int, 16, lgwin));
...
static void InjectBytePaddingBlock(BrotliEncoderState* s)
{
    ...
    if (seal_bits > 8)
        destination[1] = (uint8_t)(seal >> 8);
    if (seal_bits > 16)
        destination[2] = (uint8_t)(seal >> 16);
...
static BROTLI_BOOL InjectFlushOrPushOutput(BrotliEncoderState* s, size_t* available_out, uint8_t** next_out, size_t* total_out)
{
    ...
    if (s->available_out_ != 0 && *available_out != 0) {
        size_t copy_output_size = BROTLI_MIN(size_t, s->available_out_, *available_out);
...
static BROTLI_BOOL BrotliEncoderCompressStreamFast(BrotliEncoderState* s, BrotliEncoderOperation op, size_t* available_in, const uint8_t** next_in,
    size_t* available_out, uint8_t** next_out, size_t* total_out)
{
    const size_t block_size_limit = (size_t)1 << s->params.lgwin;
    const size_t buf_size = BROTLI_MIN(size_t, kCompressFragmentTwoPassBlockSize, BROTLI_MIN(size_t, *available_in, block_size_limit));
    ...
    if (s->params.quality == FAST_TWO_PASS_COMPRESSION_QUALITY) {
        ...
        if (s->command_buf_) {
            ...
            if (BROTLI_IS_OOM(m) || BROTLI_IS_NULL(tmp_command_buf) || BROTLI_IS_NULL(tmp_literal_buf)) {
                return BROTLI_FALSE;
            }
    ...
    while (BROTLI_TRUE) {
       ...
       additional input or pending operation. */
        if (s->available_out_ == 0 && s->stream_state_ == BROTLI_STREAM_PROCESSING && (*available_in != 0 || op != BROTLI_OPERATION_PROCESS)) {
            ...
            BROTLI_BOOL is_last = (*available_in == block_size) && (op == BROTLI_OPERATION_FINISH);
            BROTLI_BOOL force_flush = (*available_in == block_size) && (op == BROTLI_OPERATION_FLUSH);
            ...
            if (s->params.quality == FAST_ONE_PASS_COMPRESSION_QUALITY) {
                BrotliCompressFragmentFast(s->one_pass_arena_, *next_in, block_size, is_last, table, table_size, &storage_ix, storage);
                if (BROTLI_IS_OOM(m))
                    return BROTLI_FALSE;
            } else {
                BrotliCompressFragmentTwoPass(
                    s->two_pass_arena_, *next_in, block_size, is_last, command_buf, literal_buf, table, table_size, &storage_ix, storage);
                if (BROTLI_IS_OOM(m))
                    return BROTLI_FALSE;
            }
            if (block_size != 0) {
...
static BROTLI_BOOL ProcessMetadata(
    BrotliEncoderState* s, size_t* available_in, const uint8_t** next_in, size_t* available_out, uint8_t** next_out, size_t* total_out)
{
    ...
    while (BROTLI_TRUE) {
        ...
        if (s->input_pos_ != s->last_flush_pos_) {
            BROTLI_BOOL result = EncodeData(s, BROTLI_FALSE, BROTLI_TRUE, &s->available_out_, &s->next_out_);
            if (!result)
                return BROTLI_FALSE;
            continue;
        }
        ...
        if (s->stream_state_ == BROTLI_STREAM_METADATA_HEAD) {
         ...
         Otherwise client may continue producing empty metadata blocks. */
            ...
            if (*available_out) {
                ...
                uint32_t copy = (uint32_t)BROTLI_MIN(size_t, s->remaining_metadata_bytes_, *available_out);
...
BROTLI_BOOL BrotliEncoderCompressStream(BrotliEncoderState* s, BrotliEncoderOperation op, size_t* available_in, const uint8_t** next_in, size_t* available_out,
    uint8_t** next_out, size_t* total_out)
{
    ...
    while (BROTLI_TRUE) {
        ...
        if (remaining_block_size != 0 && *available_in != 0) {
            size_t copy_input_size = BROTLI_MIN(size_t, remaining_block_size, *available_in);
       ...
       finished and there is no pending flush request. */
        if (s->available_out_ == 0 && s->stream_state_ == BROTLI_STREAM_PROCESSING) {
            if (remaining_block_size == 0 || op != BROTLI_OPERATION_PROCESS) {
                BROTLI_BOOL is_last = TO_BROTLI_BOOL((*available_in == 0) && op == BROTLI_OPERATION_FINISH);
                BROTLI_BOOL force_flush = TO_BROTLI_BOOL((*available_in == 0) && op == BROTLI_OPERATION_FLUSH);
                BROTLI_BOOL result;
...
{
    return TO_BROTLI_BOOL(s->stream_state_ == BROTLI_STREAM_FINISHED && !BrotliEncoderHasMoreOutput(s));
}
...
BrotliEncoderPreparedDictionary* BrotliEncoderPrepareDictionary(BrotliSharedDictionaryType type, size_t size, const uint8_t data[BROTLI_ARRAY_PARAM(size)],
    int quality, brotli_alloc_func alloc_func, brotli_free_func free_func, void* opaque)
{
    ManagedDictionary* managed_dictionary = NULL;
    if (type != BROTLI_SHARED_DICTIONARY_RAW && type != BROTLI_SHARED_DICTIONARY_SERIALIZED) {
        return NULL;
    }
    ...
    if (type == BROTLI_SHARED_DICTIONARY_RAW) {
        managed_dictionary->dictionary = (uint32_t*)CreatePreparedDictionary(&managed_dictionary->memory_manager_, data, size);
    } else {
        SharedEncoderDictionary* dict = (SharedEncoderDictionary*)BrotliAllocate(&managed_dictionary->memory_manager_, sizeof(SharedEncoderDictionary));
        managed_dictionary->dictionary = (uint32_t*)dict;
        if (dict != NULL) {
            BROTLI_BOOL ok = BrotliInitCustomSharedEncoderDictionary(&managed_dictionary->memory_manager_, data, size, quality, dict);
            if (!ok) {
...
void BrotliEncoderDestroyPreparedDictionary(BrotliEncoderPreparedDictionary* dictionary)
{
    ...
    if (dict->dictionary == NULL) {
        ...
        DestroyPreparedDictionary(&dict->memory_manager_, (PreparedDictionary*)dict->dictionary);
    } else if (*dict->dictionary == kSharedDictionaryMagic) {
        BrotliCleanupSharedEncoderDictionary(&dict->memory_manager_, (SharedEncoderDictionary*)dict->dictionary);
...
BROTLI_BOOL BrotliEncoderAttachPreparedDictionary(BrotliEncoderState* state, const BrotliEncoderPreparedDictionary* dictionary)
{
    ...
    if (magic == kPreparedDictionaryMagic) {
        ...
        const SharedEncoderDictionary* attached = (const SharedEncoderDictionary*)dict;
        BROTLI_BOOL was_default = !current->contextual.context_based && current->contextual.num_dictionaries == 1
            && current->contextual.dict[0]->hash_table_words == kStaticDictionaryHashWords
            && current->contextual.dict[0]->hash_table_lengths == kStaticDictionaryHashLengths;
        BROTLI_BOOL new_default = !attached->contextual.context_based && attached->contextual.num_dictionaries == 1
            && attached->contextual.dict[0]->hash_table_words == kStaticDictionaryHashWords
            && attached->contextual.dict[0]->hash_table_lengths == kStaticDictionaryHashLengths;
        size_t i;
        if (state->is_initialized_)
            return BROTLI_FALSE;
        current->max_quality = BROTLI_MIN(int, current->max_quality, attached->max_quality);
        for (i = 0; i < attached->compound.num_chunks; i++) {
            if (!AttachPreparedDictionary(&current->compound, attached->compound.chunks[i])) {
                return BROTLI_FALSE;
            }
        }
        if (!new_default) {
...
size_t BrotliEncoderEstimatePeakMemoryUsage(int quality, int lgwin, size_t input_size)
{
    ...
    if (params.quality == FAST_ONE_PASS_COMPRESSION_QUALITY || params.quality == FAST_TWO_PASS_COMPRESSION_QUALITY) {
        ...
        size_t hash_table_size = HashTableSize(MaxHashTableSize(params.quality), block_size);
        size_t hash_size = (hash_table_size < (1u << 10)) ? 0 : sizeof(int) * hash_table_size;
        size_t cmdbuf_size = params.quality == FAST_TWO_PASS_COMPRESSION_QUALITY ? 5 * BROTLI_MIN(size_t, block_size, 1ul << 17) : 0;
        ...
        size_t ringbuffer_size = input_size < short_ringbuffer_size ? input_size : (1u << ringbuffer_bits) + short_ringbuffer_size;
        size_t hash_size[4] = { 0 };
        size_t metablock_size = BROTLI_MIN(size_t, input_size, MaxMetablockSize(&params));
        size_t inputblock_size = BROTLI_MIN(size_t, input_size, (size_t)1 << params.lgblock);
        ...
        if (params.quality < MIN_QUALITY_FOR_BLOCK_SPLIT) {
            cmdbuf_size = BROTLI_MIN(size_t, cmdbuf_size, MAX_NUM_DELAYED_SYMBOLS * sizeof(Command) + inputblock_size * 12);
        }
        if (params.quality >= MIN_QUALITY_FOR_HQ_BLOCK_SPLITTING) {
            /* Only a very rough estimation, based on enwik8. */
            histogram_size = 200 << 20;
        } else if (params.quality >= MIN_QUALITY_FOR_BLOCK_SPLIT) {
            size_t literal_histograms = BROTLI_MIN(size_t, metablock_size / 6144, 256);
            size_t command_histograms = BROTLI_MIN(size_t, metablock_size / 6144, 256);
            size_t distance_histograms = BROTLI_MIN(size_t, metablock_size / 6144, 256);
            histogram_size = literal_histograms * sizeof(HistogramLiteral) + command_histograms * sizeof(HistogramCommand)
                + distance_histograms * sizeof(HistogramDistance);
        }
        return (ringbuffer_size + hash_size[0] + hash_size[1] + hash_size[2] + hash_size[3] + cmdbuf_size + outbuf_size + histogram_size);
    }
}
size_t BrotliEncoderGetPreparedDictionarySize(const BrotliEncoderPreparedDictionary* prepared_dictionary)
{
    ...
    if (magic == kPreparedDictionaryMagic) {
        const PreparedDictionary* dictionary = (const PreparedDictionary*)prepared;
        /* Keep in sync with step 3 of CreatePreparedDictionary */
        return sizeof(PreparedDictionary) + dictionary->source_size + (sizeof(uint32_t) << dictionary->slot_bits)
            + (sizeof(uint16_t) << dictionary->bucket_bits) + (sizeof(uint32_t) * dictionary->source_offset) + overhead;
    } else if (magic == kSharedDictionaryMagic) {
        const SharedEncoderDictionary* dictionary = (const SharedEncoderDictionary*)prepared;
        ...
        for (i = 0; i < compound->num_prepared_instances_; i++) {
            size_t size = BrotliEncoderGetPreparedDictionarySize((const BrotliEncoderPreparedDictionary*)compound->prepared_instances_[i]);
            if (!size)
                return 0; /* error */
            result += size;
        }
...
size_t MakeUncompressedStreamForTest(const uint8_t* input, size_t input_size, uint8_t* output)
{
