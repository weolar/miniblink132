...
typedef enum { COMMAND_COMPRESS, COMMAND_DECOMPRESS, COMMAND_HELP, COMMAND_INVALID, COMMAND_TEST_INTEGRITY, COMMAND_NOOP, COMMAND_VERSION } Command;
        ...
        if (arg[1] != '-') {
            ...
            for (j = 1; j < arg_len; ++j) {
                ...
                if (c == 'o') {
                    ...
                    quality_set = ParseInt(argv[i], BROTLI_MIN_QUALITY, BROTLI_MAX_QUALITY, &params->quality);
                    if (!quality_set) {
                    ...
                    lgwin_set = ParseInt(argv[i], 0, BROTLI_MAX_WINDOW_BITS, &params->lgwin);
                    if (!lgwin_set) {
            ...
            if (strcmp("best", arg) == 0) {
                ...
                if (strncmp("dictionary", arg, key_len) == 0) {
                    ...
                    lgwin_set = ParseInt(value, 0, BROTLI_MAX_WINDOW_BITS, &params->lgwin);
                    if (!lgwin_set) {
                        fprintf(stderr, "error parsing lgwin value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
                    ...
                    lgwin_set = ParseInt(value, 0, BROTLI_LARGE_MAX_WINDOW_BITS, &params->lgwin);
                    if (!lgwin_set) {
                        fprintf(stderr, "error parsing lgwin value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
                    ...
                    quality_set = ParseInt(value, BROTLI_MIN_QUALITY, BROTLI_MAX_QUALITY, &params->quality);
                    if (!quality_set) {
                        fprintf(stderr, "error parsing quality value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
...
static void PrintHelp(const char* name, BROTLI_BOOL error)
{
    ...
    fprintf(media, "  -D FILE, --dictionary=FILE  use FILE as raw (LZ77) dictionary\n");
...
static BROTLI_BOOL ReadDictionary(Context* context, Command command)
{
    ...
    if (context->dictionary_path == NULL)
        return BROTLI_TRUE;
    ...
    if (command == COMMAND_COMPRESS) {
        context->prepared_dictionary
            = BrotliEncoderPrepareDictionary(BROTLI_SHARED_DICTIONARY_RAW, context->dictionary_size, context->dictionary, BROTLI_MAX_QUALITY, NULL, NULL, NULL);
...
static BROTLI_BOOL CompressFile(Context* context, BrotliEncoderState* s)
{
    ...
    for (;;) {
        ...
        if (!BrotliEncoderCompressStream(s, is_eof ? BROTLI_OPERATION_FINISH : BROTLI_OPERATION_PROCESS, &context->available_in, &context->next_in,
...
int main(int argc, char** argv)
{
    ...
    if (command == COMMAND_COMPRESS || command == COMMAND_DECOMPRESS || command == COMMAND_TEST_INTEGRITY) {
        if (!ReadDictionary(&context, command))
            is_ok = BROTLI_FALSE;
        if (is_ok) {
