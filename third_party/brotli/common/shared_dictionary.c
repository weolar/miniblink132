...
static BROTLI_BOOL ReadBool(const uint8_t* encoded, size_t size, size_t* pos, BROTLI_BOOL* result)
{
    ...
    size_t position = *pos;
    if (position >= size)
        return BROTLI_FALSE; /* past file end */
    value = encoded[position++];
    if (value > 1)
        return BROTLI_FALSE; /* invalid bool */
...
static BROTLI_BOOL ParseTransformsList(
    size_t size, const uint8_t* encoded, size_t* pos, BrotliTransforms* out, uint16_t* prefix_suffix_table, size_t* prefix_suffix_count)
{
    uint32_t i;
    ...
    *prefix_suffix_count = stringlet_cnt;
    ...
    for (i = 0; i < out->num_transforms; i++) {
        ...
        if (prefix_id >= stringlet_cnt)
            return BROTLI_FALSE;
        if (type >= BROTLI_NUM_TRANSFORM_TYPES)
            return BROTLI_FALSE;
        if (suffix_id >= stringlet_cnt)
            return BROTLI_FALSE;
...
static BROTLI_BOOL DryParseDictionary(const uint8_t* encoded, size_t size, uint32_t* num_prefix, BROTLI_BOOL* is_custom_static_dict)
{
    ...
    *num_prefix = 0;
    ...
    if (!ReadVarint32(encoded, size, &pos, &chunk_size))
        return BROTLI_FALSE;
    if (chunk_size != 0) {
...
static BROTLI_BOOL ParseDictionary(const uint8_t* encoded, size_t size, BrotliSharedDictionary* dict)
{
    ...
    if (dict->num_word_lists != 0) {
        ...
        if (!dict->words_instances)
            return BROTLI_FALSE; /* OOM */
    }
    ...
    if (dict->num_transform_lists != 0) {
        dict->transforms_instances
            = (BrotliTransforms*)dict->alloc_func(dict->memory_manager_opaque, dict->num_transform_lists * sizeof(*dict->transforms_instances));
        if (!dict->transforms_instances)
            return BROTLI_FALSE; /* OOM */
    }
    ...
    if (dict->num_word_lists != 0 || dict->num_transform_lists != 0) {
        ...
        for (i = 0; i < dict->num_dictionaries; i++) {
            ...
            if (words_index > dict->num_word_lists)
                return BROTLI_FALSE;
            if (!ReadUint8(encoded, size, &pos, &transforms_index)) {
                return BROTLI_FALSE;
            }
            if (transforms_index > dict->num_transform_lists)
                return BROTLI_FALSE;
...
static BROTLI_BOOL DecodeSharedDictionary(const uint8_t* encoded, size_t size, BrotliSharedDictionary* dict)
{
    ...
    if (has_custom_static_dict && is_custom_static_dict)
        return BROTLI_FALSE;
...
BROTLI_BOOL BrotliSharedDictionaryAttach(
    BrotliSharedDictionary* dict, BrotliSharedDictionaryType type, size_t data_size, const uint8_t data[BROTLI_ARRAY_PARAM(data_size)])
{
    if (!dict) {
        return BROTLI_FALSE;
    }
