...
#define T1_NEW_SIMPLE_FIELD(_ident, _type, _fname, _dict) { _ident, T1CODE, _type, 0, FT_FIELD_OFFSET(_fname), FT_FIELD_SIZE(_fname), 0, 0, _dict },

#define T1_NEW_CALLBACK_FIELD(_ident, _reader, _dict) { _ident, T1CODE, T1_FIELD_TYPE_CALLBACK, (T1_Field_ParseFunc)_reader, 0, 0, 0, 0, _dict },

#define T1_NEW_TABLE_FIELD(_ident, _type, _fname, _max, _dict)                                                                                                 \
    { _ident, T1CODE, _type, 0, FT_FIELD_OFFSET(_fname), FT_FIELD_SIZE_DELTA(_fname), _max, FT_FIELD_OFFSET(num_##_fname), _dict },
...
#define T1_FIELD_NUM_TABLE(_ident, _fname, _fmax, _dict) T1_NEW_TABLE_FIELD(_ident, T1_FIELD_TYPE_INTEGER_ARRAY, _fname, _fmax, _dict)

#define T1_FIELD_FIXED_TABLE(_ident, _fname, _fmax, _dict) T1_NEW_TABLE_FIELD(_ident, T1_FIELD_TYPE_FIXED_ARRAY, _fname, _fmax, _dict)

#define T1_FIELD_NUM_TABLE2(_ident, _fname, _fmax, _dict) T1_NEW_TABLE_FIELD2(_ident, T1_FIELD_TYPE_INTEGER_ARRAY, _fname, _fmax, _dict)

#define T1_FIELD_FIXED_TABLE2(_ident, _fname, _fmax, _dict) T1_NEW_TABLE_FIELD2(_ident, T1_FIELD_TYPE_FIXED_ARRAY, _fname, _fmax, _dict)
...
typedef struct T1_Decoder_FuncsRec_ {
    FT_Error (*init)(T1_Decoder decoder, FT_Face face, FT_Size size, FT_GlyphSlot slot, FT_Byte** glyph_names, PS_Blend blend, FT_Bool hinting,
        FT_Render_Mode hint_mode, T1_Decoder_Callback callback);

    void (*done)(T1_Decoder decoder);

#ifdef T1_CONFIG_OPTION_OLD_ENGINE
    FT_Error (*parse_charstrings_old)(T1_Decoder decoder, FT_Byte* base, FT_UInt len);
#else
...
typedef struct CFF_Decoder_FuncsRec_ {
    void (*init)(CFF_Decoder* decoder, TT_Face face, CFF_Size size, CFF_GlyphSlot slot, FT_Bool hinting, FT_Render_Mode hint_mode,
    ...
    FT_Error (*parse_charstrings_old)(CFF_Decoder* decoder, FT_Byte* charstring_base, FT_ULong charstring_len, FT_Bool in_dict);
#endif
...
#define IS_PS_TOKEN(cur, limit, token)                                                                                                                         \
    ((char)(cur)[0] == (token)[0] && ((cur) + sizeof((token)) == (limit) || ((cur) + sizeof((token)) < (limit) && IS_PS_DELIM((cur)[sizeof((token)) - 1])))    \
