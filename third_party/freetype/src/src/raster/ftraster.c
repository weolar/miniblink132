    ...
    do {
        ...
        if (y2 > e) {
            ...
            if (y2 - y1 >= ras.precision_step) {
                ...
                *top++ = arc[degree].x + FMulDiv(arc[0].x - arc[degree].x, e - y1, y2 - y1);
                arc -= degree;
                e += ras.precision;
            }
        } else {
...
static Bool Line_To(RAS_ARGS Long x, Long y)
{
    ...
    case Ascending_State:
        if (y < ras.lastY) {
            if (End_Profile(RAS_VARS IS_TOP_OVERSHOOT(ras.lastY)) || New_Profile(RAS_VARS Descending_State, IS_TOP_OVERSHOOT(ras.lastY)))
                return FAILURE;
        }
        break;
    ...
    case Descending_State:
        if (y > ras.lastY) {
            if (End_Profile(RAS_VARS IS_BOTTOM_OVERSHOOT(ras.lastY)) || New_Profile(RAS_VARS Ascending_State, IS_BOTTOM_OVERSHOOT(ras.lastY)))
                return FAILURE;
        }
        break;
    ...
    do {
        ...
        if (y2 < ymin || y2 > ymax) {
            ...
            if (ras.state != state_bez) {
                Bool o = (state_bez == Ascending_State) ? IS_BOTTOM_OVERSHOOT(y1) : IS_TOP_OVERSHOOT(y1);
...
static Bool Convert_Glyph(RAS_ARGS Int flipped)
{
    ...
    for (i = 0; i < ras.outline.n_contours; i++) {
        ...
        if (Decompose_Curve(RAS_VARS(UShort) start, (UShort)ras.outline.contours[i], flipped))
            return FAILURE;
        ...
        if (FRAC(ras.lastY) == 0 && ras.lastY >= ras.minY && ras.lastY <= ras.maxY)
            if (ras.gProfile && (ras.gProfile->flags & Flow_Up) == (ras.cProfile->flags & Flow_Up))
                ras.top--;
...
static void Vertical_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right)
{
    ...
    FT_TRACE7(("  y=%d x=[%.12f;%.12f], drop-out=%d", y, x1 / (double)ras.precision, x2 / (double)ras.precision, dropOutControl));
    ...
    if (dropOutControl != 2 && x2 - x1 - ras.precision <= ras.precision_jitter && e1 != x1 && e2 != x2)
        e2 = e1;
...
static void Vertical_Sweep_Drop(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right)
{
    ...
    FT_TRACE7(("  y=%d x=[%.12f;%.12f]", y, x1 / (double)ras.precision, x2 / (double)ras.precision));
    ...
    if (e1 > e2) {
        ...
        if (e1 == e2 + ras.precision) {
            ...
            case 5: /* smart drop-outs excluding stubs  */
                ...
                /* upper stub test */
                if (left->next == right && left->height <= 0 && !(left->flags & Overshoot_Top && x2 - x1 >= ras.precision_half))
                    goto Exit;

                /* lower stub test */
                if (right->next == left && left->start == y && !(left->flags & Overshoot_Bottom && x2 - x1 >= ras.precision_half))
                    goto Exit;
            ...
            if (e1 >= 0 && e1 < ras.bWidth && ras.bOrigin[ras.traceOfs + c1] & (0x80 >> f1))
                goto Exit;
        } else
            goto Exit;
    }
...
static void Horizontal_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right)
{
    ...
    if (x2 - x1 < ras.precision) {
        ...
        FT_TRACE7(("  x=%d y=[%.12f;%.12f]", y, x1 / (double)ras.precision, x2 / (double)ras.precision));
