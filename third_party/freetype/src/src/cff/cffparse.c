...
static FT_Long cff_parse_integer(FT_Byte* start, FT_Byte* limit)
{
    ...
    if (v == 28) {
        ...
        val = (FT_Long)(((FT_ULong)p[0] << 24) | ((FT_ULong)p[1] << 16) | ((FT_ULong)p[2] << 8) | (FT_ULong)p[3]);
    } else if (v < 247) {
        val = v - 139;
    } else if (v < 251) {
        if (p + 1 > limit)
...
static const FT_Long power_tens[] = { 1L, 10L, 100L, 1000L, 10000L, 100000L, 1000000L, 10000000L, 100000000L, 1000000000L };
...
#else
        return (FT_Short)(((((FT_UInt32) * (d[0] + 1) << 16) | ((FT_UInt32) * (d[0] + 2) << 8) | (FT_UInt32) * (d[0] + 3)) + 0x80U) >> 8);
#endif
    }
...
static FT_Error cff_parse_font_matrix(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 6) {
        ...
        if (max_scaling < -9 || max_scaling > 0 || (max_scaling - min_scaling) < 0 || (max_scaling - min_scaling) > 9) {
        ...
        *upm = (FT_ULong)power_tens[-max_scaling];
        ...
        FT_TRACE4((" [%f %f %f %f %f %f]\n", (double)matrix->xx / *upm / 65536, (double)matrix->xy / *upm / 65536, (double)matrix->yx / *upm / 65536,
            (double)matrix->yy / *upm / 65536, (double)offset->x / *upm / 65536, (double)offset->y / *upm / 65536));
...
static FT_Error cff_parse_font_bbox(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 4) {
        ...
        FT_TRACE4((" [%d %d %d %d]\n", bbox->xMin / 65536, bbox->yMin / 65536, bbox->xMax / 65536, bbox->yMax / 65536));
    }
...
static FT_Error cff_parse_cid_ros(CFF_Parser parser)
{
    ...
    if (parser->top >= parser->stack + 3) {
        ...
        FT_TRACE4((" %d %d %d\n", dict->cid_registry, dict->cid_ordering, dict->cid_supplement));
    }
...
#define CFF_FIELD_CALLBACK(code, name, id) { cff_kind_callback, code | CFFCODE, 0, 0, cff_parse_##name, 0, 0 },

#define CFF_FIELD_BLEND(code, id) { cff_kind_blend, code | CFFCODE, 0, 0, cff_parse_blend, 0, 0 },

#define CFF_FIELD(code, name, id, kind) { kind, code | CFFCODE, FT_FIELD_OFFSET(name), FT_FIELD_SIZE(name), 0, 0, 0 },
...
#define CFF_FIELD_DELTA(code, name, max, id)                                                                                                                   \
    { cff_kind_delta, code | CFFCODE, FT_FIELD_OFFSET(name), FT_FIELD_SIZE_DELTA(name), 0, max, FT_FIELD_OFFSET(num_##name), id },
    ...
    while (p < limit) {
        ...
        if (v >= 27 && v != 31 && v != 255) {
            ...
            *parser->top++ = p;
            ...
            error = psaux->cff_decoder_funcs->parse_charstrings_old(&decoder, charstring_base, charstring_len, 1);
            if (error)
                goto Exit;
            ...
            /* element.                                                        */
            q = (FT_Byte*)memory->alloc(memory, 5 * (decoder.top - decoder.stack));
            if (!q)
                goto Out_Of_Memory_Error;
