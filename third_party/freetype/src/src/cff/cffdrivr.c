    ...
    if (FT_IS_SFNT(face)) {
        ...
        if (flags & FT_LOAD_VERTICAL_LAYOUT) {
            ...
            for (nn = 0; nn < count; nn++) {
                ...
                ((SFNT_Service)ttface->sfnt)->get_metrics(ttface, 1, start + nn, &dummy, &ah);
            ...
            for (nn = 0; nn < count; nn++) {
                ...
                ((SFNT_Service)ttface->sfnt)->get_metrics(ttface, 0, start + nn, &dummy, &aw);
...
Missing_Table:
    ...
    for (nn = 0; nn < count; nn++) {
        ...
        advances[nn] = (flags & FT_LOAD_VERTICAL_LAYOUT) ? slot->linearVertAdvance : slot->linearHoriAdvance;
    }
...
static FT_Error cff_get_glyph_name(CFF_Face face, FT_UInt glyph_index, FT_Pointer buffer, FT_UInt buffer_max)
{
    ...
    if (font->version_major == 2) {
        ...
        FT_Service_GlyphDict service = (FT_Service_GlyphDict)ft_module_get_service(sfnt_module, FT_SERVICE_ID_GLYPH_DICT, 0);

        if (service && service->get_name)
            return service->get_name(FT_FACE(face), glyph_index, buffer, buffer_max);
        else {
...
static FT_Error cff_ps_get_font_extra(CFF_Face face, PS_FontExtraRec* afont_extra)
{
    ...
    if (cff && cff->font_extra == NULL) {
        ...
        embedded_postscript = cff_index_get_sid_string(cff, dict->embedded_postscript);
        if (embedded_postscript) {
            FT_String* start_fstype;
            FT_String* start_def;

            /* Identify the XYZ integer in `/FSType XYZ def' substring. */
            if ((start_fstype = ft_strstr(embedded_postscript, "/FSType")) != NULL
                && (start_def = ft_strstr(start_fstype + sizeof("/FSType") - 1, "def")) != NULL) {
                FT_String* s;

                for (s = start_fstype + sizeof("/FSType") - 1; s != start_def; s++) {
                    if (*s >= '0' && *s <= '9') {
...
static const char* cff_get_ps_name(CFF_Face face)
{
    ...
    if (FT_IS_SFNT(FT_FACE(face)) && sfnt) {
        FT_Library library = FT_FACE_LIBRARY(face);
        FT_Module sfnt_module = FT_Get_Module(library, "sfnt");
        FT_Service_PsFontName service = (FT_Service_PsFontName)ft_module_get_service(sfnt_module, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, 0);
...
static FT_Error cff_get_cmap_info(FT_CharMap charmap, TT_CMapInfo* cmap_info)
{
    ...
    if (cmap->clazz != &cff_cmap_encoding_class_rec && cmap->clazz != &cff_cmap_unicode_class_rec) {
        ...
        FT_Service_TTCMaps service = (FT_Service_TTCMaps)ft_module_get_service(sfnt, FT_SERVICE_ID_TT_CMAP, 0);
...
/*
   ...
   *
   */
static FT_Error cff_get_ros(CFF_Face face, const char** registry, const char** ordering, FT_Int* supplement)
{
    FT_Error error = FT_Err_Ok;
    CFF_Font cff = (CFF_Font)face->extra.data;
    ...
    if (cff) {
       ...
       */
        if (supplement) {
            if (dict->cid_supplement < FT_INT_MIN || dict->cid_supplement > FT_INT_MAX)
...
FT_DEFINE_SERVICE_CIDREC(cff_service_cid_info,
    ...
    (FT_CID_GetCIDFromGlyphIndexFunc)cff_get_cid_from_glyph_index /* get_cid_from_glyph_index */
)
...
static FT_Error cff_set_mm_blend(CFF_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
...
static FT_Error cff_get_mm_blend(CFF_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_set_mm_weightvector(CFF_Face face, FT_UInt len, FT_Fixed* weightvector)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_get_mm_weightvector(CFF_Face face, FT_UInt* len, FT_Fixed* weightvector)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_set_var_design(CFF_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_get_var_design(CFF_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    FT_Service_MultiMasters mm = (FT_Service_MultiMasters)face->mm;
...
static FT_Error cff_hadvance_adjust(CFF_Face face, FT_UInt gindex, FT_Int* avalue)
{
...
FT_DEFINE_SERVICE_CFFLOADREC(cff_service_cff_load,
    ...
    (FT_Get_Standard_Encoding_Func)cff_get_standard_encoding, (FT_Load_Private_Dict_Func)cff_load_private_dict, (FT_FD_Select_Get_Func)cff_fd_select_get,
    (FT_Blend_Check_Vector_Func)cff_blend_check_vector, (FT_Blend_Build_Vector_Func)cff_blend_build_vector)
...
FT_DEFINE_SERVICEDESCREC10(cff_services,

    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_CFF, FT_SERVICE_ID_MULTI_MASTERS, &cff_service_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
    &cff_service_metrics_variations, FT_SERVICE_ID_POSTSCRIPT_INFO, &cff_service_ps_info, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, &cff_service_ps_name,
    FT_SERVICE_ID_GLYPH_DICT, &cff_service_glyph_dict, FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info, FT_SERVICE_ID_CID, &cff_service_cid_info,
    FT_SERVICE_ID_PROPERTIES, &cff_service_properties, FT_SERVICE_ID_CFF_LOAD, &cff_service_cff_load)
...
FT_DEFINE_SERVICEDESCREC8(cff_services,
    ...
    &cff_service_ps_name, FT_SERVICE_ID_GLYPH_DICT, &cff_service_glyph_dict, FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info, FT_SERVICE_ID_CID,
    &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES, &cff_service_properties, FT_SERVICE_ID_CFF_LOAD, &cff_service_cff_load)
#elif defined TT_CONFIG_OPTION_GX_VAR_SUPPORT
FT_DEFINE_SERVICEDESCREC9(cff_services,

    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_CFF, FT_SERVICE_ID_MULTI_MASTERS, &cff_service_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
    &cff_service_metrics_var, FT_SERVICE_ID_POSTSCRIPT_INFO, &cff_service_ps_info, FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, &cff_service_ps_name,
    FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info, FT_SERVICE_ID_CID, &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES, &cff_service_properties,
...
FT_DEFINE_DRIVER(cff_driver_class,
    ...
    sizeof(TT_FaceRec), sizeof(CFF_SizeRec), sizeof(CFF_GlyphSlotRec),
