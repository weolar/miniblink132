...
cff_size_init(FT_Size cffsize) /* CFF_Size */
{
    ...
    if (funcs) {
        ...
        for (i = font->num_subfonts; i > 0; i--) {
            ...
            error = funcs->create(cffsize->face->memory, &priv, &internal->subfonts[i - 1]);
            if (error)
                goto Exit;
        }
...
cff_size_select(FT_Size size, FT_ULong strike_index)
{
    ...
    if (funcs) {
        ...
        funcs->set_scale(internal->topfont, size->metrics.x_scale, size->metrics.y_scale, 0, 0);
...
static void remove_style(FT_String* family_name, const FT_String* style_name)
{
    ...
    if (family_name_length > style_name_length) {
        ...
        for (idx = 1; idx <= style_name_length; idx++) {
            if (family_name[family_name_length - idx] != style_name[style_name_length - idx])
                break;
        }
        ...
        if (idx > style_name_length) {
            ...
            while (idx > 0 && (family_name[idx] == '-' || family_name[idx] == ' ' || family_name[idx] == '_' || family_name[idx] == '+'))
                idx--;
    ...
    FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    /* now load and parse the CFF table in the file */
    {
        ...
        face->extra.data = cff;
        error = cff_font_load(library, stream, face_index, cff, face, pure_cff, cff2);
        if (error)
            goto Exit;
        ...
        for (i = cff->num_subfonts; i > 0; i--) {
            ...
            if (sub->has_font_matrix) {
                ...
                if (top->has_font_matrix) {
                    ...
                    sub->units_per_em = (FT_ULong)FT_MulDiv((FT_Long)sub->units_per_em, (FT_Long)top->units_per_em, scaling);
                }
            } else {
        ...
        if (pure_cff) {
            ...
            cffface->underline_position = (FT_Short)(dict->underline_position >> 16);
            cffface->underline_thickness = (FT_Short)(dict->underline_thickness >> 16);
            ...
            if (!cffface->family_name) {
                cffface->family_name = cff_index_get_name(cff, (FT_UInt)(face_index & 0xFFFF));
                if (cffface->family_name)
            ...
            if (cffface->family_name) {
                ...
                char* cid_font_name = cff_index_get_sid_string(cff, dict->cid_font_name);
         ...
         */
...

            {
                ...
                if (weight)
                    if (!ft_strcmp(weight, "Bold") || !ft_strcmp(weight, "Black"))
                        flags |= FT_STYLE_FLAG_BOLD;
            }

            /* double check */
            if (!(flags & FT_STYLE_FLAG_BOLD) && cffface->style_name)
                if (!ft_strncmp(cffface->style_name, "Bold", 4) || !ft_strncmp(cffface->style_name, "Black", 5))
                    flags |= FT_STYLE_FLAG_BOLD;
       ...
       */
        ...
        /* will use it, whatever be in the CFF part of the file.          */
        {
            ...
            error = FT_CMap_New(&cff_cmap_unicode_class_rec, NULL, &cmaprec, NULL);
