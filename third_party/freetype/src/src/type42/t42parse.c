...
#define T1CODE T1_FIELD_LOCATION_FONT_INFO
    ...
    T1_FIELD_STRING("version", version, 0) T1_FIELD_STRING("Notice", notice, 0) T1_FIELD_STRING("FullName", full_name, 0) T1_FIELD_STRING("FamilyName",
        family_name, 0) T1_FIELD_STRING("Weight", weight, 0) T1_FIELD_NUM("ItalicAngle", italic_angle, 0) T1_FIELD_BOOL("isFixedPitch", is_fixed_pitch, 0)
        T1_FIELD_NUM("UnderlinePosition", underline_position, 0) T1_FIELD_NUM("UnderlineThickness", underline_thickness, 0)
...
#define T1CODE T1_FIELD_LOCATION_FONT_DICT
                ...
                T1_FIELD_KEY("FontName", font_name, 0) T1_FIELD_NUM("PaintType", paint_type, 0) T1_FIELD_NUM("FontType", font_type, 0)
...
#define T1CODE T1_FIELD_LOCATION_BBOX
                        ...
                        T1_FIELD_BBOX("FontBBox", xMin, 0)
                            ...
                            T1_FIELD_CALLBACK("FontMatrix", t42_parse_font_matrix, 0) T1_FIELD_CALLBACK("Encoding", t42_parse_encoding, 0)
                                T1_FIELD_CALLBACK("CharStrings", t42_parse_charstrings, 0) T1_FIELD_CALLBACK("sfnts", t42_parse_sfnts, 0)
            ...
            case BEFORE_TABLE_DIR:
                ...
                if (count < face->ttf_size) {
                    ...
                    for (i = 0; i < num_tables; i++) {
                        ...
                        len = FT_PEEK_ULONG(p);
                        if (len > size || face->ttf_size > (FT_Long)(size - len)) {
                            FT_ERROR(("t42_parse_sfnts:"
                    ...
                    if (FT_REALLOC(face->ttf_data, 12 + 16 * num_tables, face->ttf_size + 1))
                        goto Fail;
                }
                /* fall through */
...
static void t42_parse_charstrings(T42_Face face, T42_Loader loader)
{
    ...
    for (;;) {
        ...
        if (*cur == 'e' && cur + 3 < limit && cur[1] == 'n' && cur[2] == 'd' && t42_is_space(cur[3]))
            break;
        if (*cur == '>')
            break;
...
t42_parse_dict(T42_Face face, T42_Loader loader, FT_Byte* base, FT_Long size)
{
    T42_Parser parser = &loader->parser;
    FT_Byte* limit;
    FT_Int n_keywords = (FT_Int)(sizeof(t42_keywords) / sizeof(t42_keywords[0]));
