...
pfr_face_init(FT_Stream stream, FT_Face pfrface, FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    /* check face index */
    {
        ...
        error = pfr_log_font_count(stream, face->header.log_dir_offset, &num_faces);
        if (error)
            goto Exit;
    ...
    /* load the face */
    error
        = pfr_log_font_load(&face->log_font, stream, (FT_UInt)(face_index & 0xFFFF), face->header.log_dir_offset, FT_BOOL(face->header.phy_font_max_size_high));
    if (error)
        goto Exit;

    /* now load the physical font descriptor */
    error = pfr_phy_font_load(&face->phy_font, stream, face->log_font.phys_offset, face->log_font.phys_size);
    if (error)
        goto Exit;
    ...
    FT_UInt gindex, FT_Int32 load_flags)
{
    ...
    if ((load_flags & (FT_LOAD_NO_SCALE | FT_LOAD_NO_BITMAP)) == 0) {
        error = pfr_slot_load_bitmap(slot, size, gindex, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0);
        if (!error)
            goto Exit;
    }
    ...
    error = pfr_glyph_load(&slot->glyph, face->root.stream, gps_offset, gchar->gps_offset, gchar->gps_size);
    ...
    if (!error) {
        ...
        *outline = slot->glyph.loader->base.outline;
        ...
        if (em_metrics != em_outline)
            advance = FT_MulDiv(advance, (FT_Long)em_outline, (FT_Long)em_metrics);
    ...
    FT_UInt glyph1, FT_UInt glyph2, FT_Vector* kerning)
{
    ...
    FoundPair: /* we found an item, now parse it and find the value if any */
        if (FT_STREAM_SEEK(item->offset) || FT_FRAME_ENTER(item->pair_count * item->pair_size))
            goto Exit;
