...
FT_Outline_New(FT_Library library, FT_UInt numPoints, FT_Int numContours, FT_Outline* anoutline)
{
    ...
    *anoutline = null_outline;
    ...
    if (FT_NEW_ARRAY(anoutline->points, numPoints) || FT_NEW_ARRAY(anoutline->tags, numPoints) || FT_NEW_ARRAY(anoutline->contours, numContours))
        goto Fail;
...
FT_Outline_Render(FT_Library library, FT_Outline* outline, FT_Raster_Params* params)
{
    ...
    if (cbox.xMin < -0x1000000L || cbox.yMin < -0x1000000L || cbox.xMax > 0x1000000L || cbox.yMax > 0x1000000L)
        return FT_THROW(Invalid_Outline);
...
#define FT_OUTLINE_GET_CONTOUR(outline, c, first, last)                                                                                                        \
    do {                                                                                                                                                       \
        (first) = (c > 0) ? (outline)->points + (outline)->contours[c - 1] + 1 : (outline)->points;                                                            \
