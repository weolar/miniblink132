...
ft_outline_glyph_init(FT_Glyph outline_glyph, FT_GlyphSlot slot)
{
    ...
    error = FT_Outline_New(library, (FT_UInt)source->n_points, source->n_contours, &glyph->outline);
    if (error)
        goto Exit;
...
ft_outline_glyph_copy(FT_Glyph outline_source, FT_Glyph outline_target)
{
    ...
    error = FT_Outline_New(library, (FT_UInt)source->outline.n_points, source->outline.n_contours, &target->outline);
    if (!error)
...
static FT_Error ft_new_glyph(FT_Library library, const FT_Glyph_Class* clazz, FT_Glyph* aglyph)
{
