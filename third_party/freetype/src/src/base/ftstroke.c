...
FT_Outline_GetInsideBorder(FT_Outline* outline)
{
    ...
    return o == FT_ORIENTATION_TRUETYPE ? FT_STROKER_BORDER_RIGHT : FT_STROKER_BORDER_LEFT;
}
...
FT_Outline_GetOutsideBorder(FT_Outline* outline)
{
    ...
    return o == FT_ORIENTATION_TRUETYPE ? FT_STROKER_BORDER_LEFT : FT_STROKER_BORDER_RIGHT;
}
...
static FT_Error ft_stroke_border_lineto(FT_StrokeBorder border, FT_Vector* to, FT_Bool movable)
{
    ...
    if (border->movable) {
        ...
        if (border->num_points > (FT_UInt)border->start && FT_IS_SMALL(border->points[border->num_points - 1].x - to->x)
...
static FT_Error ft_stroke_border_arcto(FT_StrokeBorder border, FT_Vector* center, FT_Fixed radius, FT_Angle angle_start, FT_Angle angle_diff)
{
    ...
    while (total != 0) {
        ...
        length = FT_MulDiv(radius, FT_Sin(theta) * 4, (0x10000L + FT_Cos(theta)) * 3);
...
static void ft_stroke_border_export(FT_StrokeBorder border, FT_Outline* outline)
{
    ...
    if (border->num_points)
        FT_ARRAY_COPY(outline->points + outline->n_points, border->points, border->num_points);
    ...
    if (stroker->line_join == FT_STROKER_LINEJOIN_ROUND)
        ...
        if (bevel) /* this is a bevel (broken angle) */
        {
            ...
            } else /* variable bevel */
            {
                ...
                length = FT_MulDiv(radius, 0x10000L - sigma, ft_pos_abs(FT_Sin(theta)));
...
FT_Stroker_ConicTo(FT_Stroker stroker, FT_Vector* control, FT_Vector* to)
{
    ...
    if (FT_IS_SMALL(stroker->center.x - control->x) && FT_IS_SMALL(stroker->center.y - control->y) && FT_IS_SMALL(control->x - to->x)
    ...
    while (arc >= bez_stack) {
        ...
        } else if (ft_pos_abs(FT_Angle_Diff(stroker->angle_in, angle_in)) > FT_SMALL_CONIC_THRESHOLD / 4) {
...
FT_Stroker_CubicTo(FT_Stroker stroker, FT_Vector* control1, FT_Vector* control2, FT_Vector* to)
{
    ...
    /* if all control points are coincident, this is a no-op; */
    /* avoid creating a spurious corner */
    if (FT_IS_SMALL(stroker->center.x - control1->x) && FT_IS_SMALL(stroker->center.y - control1->y) && FT_IS_SMALL(control1->x - control2->x)
        && FT_IS_SMALL(control1->y - control2->y) && FT_IS_SMALL(control2->x - to->x) && FT_IS_SMALL(control2->y - to->y)) {
        stroker->center = *to;
        goto Exit;
    }
    ...
    while (arc >= bez_stack) {
        ...
        } else if (ft_pos_abs(FT_Angle_Diff(stroker->angle_in, angle_in)) > FT_SMALL_CUBIC_THRESHOLD / 4) {
            /* if the deviation from one arc to the next is too great, */
            /* add a round corner                                      */
