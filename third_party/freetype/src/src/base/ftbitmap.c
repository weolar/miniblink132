...
static FT_Byte ft_gray_for_premultiplied_srgb_bgra(const FT_Byte* bgra)
{
    ...
    l = (4732UL /* 0.0722 * 65536 */ * bgra[0] * bgra[0] + 46871UL /* 0.7152 * 65536 */ * bgra[1] * bgra[1] + 13933UL /* 0.2126 * 65536 */ * bgra[2] * bgra[2])
        >> 16;
...
FT_Bitmap_Blend(FT_Library library, const FT_Bitmap* source_, const FT_Vector source_offset_, FT_Bitmap* target, FT_Vector* atarget_offset, FT_Color color)
{
    ...
    if (!(target->pixel_mode == FT_PIXEL_MODE_NONE || (target->pixel_mode == FT_PIXEL_MODE_BGRA && target->buffer)))
        return FT_THROW(Invalid_Argument);
    ...
    FT_TRACE5(("FT_Bitmap_Blend:\n"
        ...
        source_llx / 64, source_lly / 64, source_urx / 64, source_ury / 64, source_->width, source_->rows));

    if (frac_offset.x || frac_offset.y)
        FT_TRACE5(("    fractional offset: (%d/64, %d/64)\n", frac_offset.x, frac_offset.y));

    if (target->width && target->rows)
        FT_TRACE5(("  target bitmap: (%d, %d) -- (%d, %d); %d x %d\n", target_llx / 64, target_lly / 64, target_urx / 64, target_ury / 64, target->width,
            target->rows));
    else
