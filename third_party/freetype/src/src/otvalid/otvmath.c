...
static void otv_MathItalicsCorrectionInfo_validate(FT_Bytes table, OTV_Validator otvalid, FT_Int isItalic)
{
    FT_Bytes p = table;
    ...
    OTV_NAME_ENTER(isItalic ? "MathItalicsCorrectionInfo" : "MathTopAccentAttachment");
...
static void otv_MathKern_validate(FT_Bytes table, OTV_Validator otvalid)
{
    FT_Bytes p = table;
    FT_UInt i, cnt, table_size;
...
static void otv_MathKernInfo_validate(FT_Bytes table, OTV_Validator otvalid)
{
    FT_Bytes p = table;
...
static void otv_MathGlyphInfo_validate(FT_Bytes table, OTV_Validator otvalid)
{
    ...
    if (MathItalicsCorrectionInfo)
        otv_MathItalicsCorrectionInfo_validate(table + MathItalicsCorrectionInfo, otvalid, TRUE);

    /* Italic correction and Top Accent Attachment have the same format */
    if (MathTopAccentAttachment)
        otv_MathItalicsCorrectionInfo_validate(table + MathTopAccentAttachment, otvalid, FALSE);
...
static void otv_GlyphAssembly_validate(FT_Bytes table, OTV_Validator otvalid)
{
    FT_Bytes p = table;
...
static void otv_MathGlyphConstruction_validate(FT_Bytes table, OTV_Validator otvalid)
{
    FT_Bytes p = table;
...
otv_MATH_validate(FT_Bytes table, FT_UInt glyph_count, FT_Validator ftvalid)
{
    ...
    otv_MathConstants_validate(table + MathConstants, otvalid);
    otv_MathGlyphInfo_validate(table + MathGlyphInfo, otvalid);
    otv_MathVariants_validate(table + MathVariants, otvalid);
