...
#define FT_THROW(e) (FT_Throw(FT_ERR_CAT(ErrRaster_, e), __LINE__, __FILE__) | FT_ERR_CAT(ErrRaster_, e))
...
#define FT_DEFINE_RASTER_FUNCS(class_, glyph_format_, raster_new_, raster_reset_, raster_set_mode_, raster_render_, raster_done_)                              \
    const FT_Raster_Funcs class_ = { glyph_format_, raster_new_, raster_reset_, raster_set_mode_, raster_render_, raster_done_ };
...
#define FT_UDIVPREP(c, b) long b##_r = c ? (long)(FT_ULONG_MAX >> PIXEL_BITS) / (b) : 0
#define FT_UDIV(a, b) (((unsigned long)(a) * (unsigned long)(b##_r)) >> (sizeof(long) * FT_CHAR_BIT - PIXEL_BITS))
...
static void gray_render_cubic(RAS_ARG_ const FT_Vector* control1, const FT_Vector* control2, const FT_Vector* to)
{
    ...
    /* short-cut the arc that crosses the current band */
    if ((TRUNC(arc[0].y) >= ras.max_ey && TRUNC(arc[1].y) >= ras.max_ey && TRUNC(arc[2].y) >= ras.max_ey && TRUNC(arc[3].y) >= ras.max_ey)
        || (TRUNC(arc[0].y) < ras.min_ey && TRUNC(arc[1].y) < ras.min_ey && TRUNC(arc[2].y) < ras.min_ey && TRUNC(arc[3].y) < ras.min_ey)) {
        ras.x = arc[0].x;
        ras.y = arc[0].y;
        return;
    }
...
/**************************************************************************
   ...
   */
static int FT_Outline_Decompose(const FT_Outline* outline, const FT_Outline_Funcs* func_interface, void* user)
{
#undef SCALED
    ...
    for (n = 0; n < outline->n_contours; n++) {
        ...
        while (point < limit) {
            ...
            Do_Conic:
                if (point < limit) {
                    ...
                    if (tag == FT_CURVE_TAG_ON) {
                        FT_TRACE5(("  conic to (%.2f, %.2f)"
                                   " with control (%.2f, %.2f)\n",
                            vec.x / 64.0, vec.y / 64.0, v_control.x / 64.0, v_control.y / 64.0));
                    ...
                    FT_TRACE5(("  conic to (%.2f, %.2f)"
                               " with control (%.2f, %.2f)\n",
                        v_middle.x / 64.0, v_middle.y / 64.0, v_control.x / 64.0, v_control.y / 64.0));
                ...
                FT_TRACE5(("  conic to (%.2f, %.2f)"
                           " with control (%.2f, %.2f)\n",
                    v_start.x / 64.0, v_start.y / 64.0, v_control.x / 64.0, v_control.y / 64.0));
            ...
            default: /* FT_CURVE_TAG_CUBIC */
            {
                ...
                if (point <= limit) {
                    ...
                    FT_TRACE5(("  cubic to (%.2f, %.2f)"
                        ...
                        vec.x / 64.0, vec.y / 64.0, vec1.x / 64.0, vec1.y / 64.0, vec2.x / 64.0, vec2.y / 64.0));
                ...
                FT_TRACE5(("  cubic to (%.2f, %.2f)"
                           " with controls (%.2f, %.2f) and (%.2f, %.2f)\n",
                    v_start.x / 64.0, v_start.y / 64.0, vec1.x / 64.0, vec1.y / 64.0, vec2.x / 64.0, vec2.y / 64.0));
...
static int gray_convert_glyph_inner(RAS_ARG, int continued)
{
    ...
    if (ft_setjmp(ras.jump_buffer) == 0) {
        ...
        FT_TRACE7(("band [%d..%d]: %d cell%s\n", ras.min_ey, ras.max_ey, ras.num_cells, ras.num_cells == 1 ? "" : "s"));
    } else {
...
static int gray_raster_render(FT_Raster raster, const FT_Raster_Params* params)
{
    ...
    if (params->flags & FT_RASTER_FLAG_DIRECT) {
        ...
        if (target_map->pitch < 0)
            ...
            ras.target.origin = target_map->buffer + (target_map->rows - 1) * (unsigned int)target_map->pitch;
    ...
    if (params->flags & FT_RASTER_FLAG_DIRECT && params->flags & FT_RASTER_FLAG_CLIP)
