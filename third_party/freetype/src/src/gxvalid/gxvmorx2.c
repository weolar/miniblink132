...
} GXV_morx_subtable_type2_StateOptRec, *GXV_morx_subtable_type2_StateOptRecData;
...
static void gxv_morx_subtable_type2_opttable_load(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
    ...
    GXV_morx_subtable_type2_StateOptRecData optdata = (GXV_morx_subtable_type2_StateOptRecData)gxvalid->xstatetable.optdata;
...
static void gxv_morx_subtable_type2_subtable_setup(FT_ULong table_size, FT_ULong classTable, FT_ULong stateArray, FT_ULong entryTable,
    FT_ULong* classTable_length_p, FT_ULong* stateArray_length_p, FT_ULong* entryTable_length_p, GXV_Validator gxvalid)
{
    ...
    GXV_TRACE(("ligActionTable: offset=0x%08x length=0x%08x\n", optdata->ligActionTable, optdata->ligActionTable_length));
    GXV_TRACE(("componentTable: offset=0x%08x length=0x%08x\n", optdata->componentTable, optdata->componentTable_length));
...
static void gxv_morx_subtable_type2_ligActionIndex_validate(FT_Bytes table, FT_UShort ligActionIndex, GXV_Validator gxvalid)
{
...

    {
        ...
        if ((offset & 0x3FFF0000UL) == 0x3FFF0000UL) { /* negative offset */
            gid_limit = gxvalid->face->num_glyphs - (FT_Long)(offset & 0x0000FFFFUL);
            if (gid_limit > 0)
                return;
        ...
        } else if ((offset & 0x3FFF0000UL) == 0x00000000UL) { /* positive offset */
...
static void gxv_morx_subtable_type2_entry_validate(
    FT_UShort state, FT_UShort flags, GXV_StateTable_GlyphOffsetCPtr glyphOffset_p, FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
#ifdef GXV_LOAD_UNUSED_VARS
    ...
    if (0 < ligActionIndex)
        gxv_morx_subtable_type2_ligActionIndex_validate(table, ligActionIndex, gxvalid);
}
...
static void gxv_morx_subtable_type2_ligatureTable_validate(FT_Bytes table, GXV_Validator gxvalid)
{
    ...
    FT_Bytes limit = table + optdata->ligatureTable + optdata->ligatureTable_length;
...
gxv_morx_subtable_type2_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
    ...
    gxvalid->xstatetable.optdata = &lig_rec;
    gxvalid->xstatetable.optdata_load_func = gxv_morx_subtable_type2_opttable_load;
    gxvalid->xstatetable.subtable_setup_func = gxv_morx_subtable_type2_subtable_setup;
    gxvalid->xstatetable.entry_glyphoffset_fmt = GXV_GLYPHOFFSET_USHORT;
    gxvalid->xstatetable.entry_validate_func = gxv_morx_subtable_type2_entry_validate;
