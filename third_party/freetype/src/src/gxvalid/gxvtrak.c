...
static void gxv_trak_trackData_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
    ...
    gxv_odtect_add_range(p, gxvalid->subtable_length, "trackTable", odtect);
    ...
    gxv_odtect_add_range(gxvalid->root->base + GXV_TRAK_DATA(trackValueOffset_min),
        GXV_TRAK_DATA(trackValueOffset_max) - GXV_TRAK_DATA(trackValueOffset_min) + nSizes * 2, "trackValue array", odtect);
