...
#define GXV_TABLE_LOAD(_sfnt)                                                                                                                                  \
    if ((FT_VALIDATE_##_sfnt##_INDEX < table_count) && (gx_flags & FT_VALIDATE_##_sfnt)) {                                                                     \
...
static FT_Error classic_kern_validate(FT_Face face, FT_UInt ckern_flags, FT_Bytes* ckern_table)
{
    ...
    *ckern_table = NULL;
    ...
    if (ckern) {
        ...
        if (ft_setjmp(valid.jump_buffer) == 0)
            gxv_kern_validate_classic(ckern, face, ckern_flags & FT_VALIDATE_CKERN, &valid);
        error = valid.error;
        if (error)
            goto Exit;
    }
...
const FT_Module_Class gxv_module_class = {
    0, sizeof(FT_ModuleRec), "gxvalid", 0x10000L, 0x20000L,
