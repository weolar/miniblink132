...
static void gxv_prop_zero_advance_validate(FT_UShort gid, GXV_Validator gxvalid)
{
    ...
    error = FT_Load_Glyph(face, gid, FT_LOAD_IGNORE_TRANSFORM);
    if (error)
        FT_INVALID_GLYPH_ID;

    glyph = face->glyph;

    if (glyph->advance.x != (FT_Pos)0 || glyph->advance.y != (FT_Pos)0) {
...
static void gxv_prop_property_validate(FT_UShort property, FT_UShort glyph, GXV_Validator gxvalid)
{
    ...
    if ((property & GXV_PROP_DIRECTIONALITY_CLASS) > 11) {
        ...
        if (GXV_PROP_DATA(version) == 0x00010000UL || GXV_PROP_DATA(version) == 0x00020000UL) {
...
static GXV_LookupValueDesc gxv_prop_LookupFmt4_transit(
...
gxv_prop_validate(FT_Bytes table, FT_Face face, FT_Validator ftvalid)
{
    ...
    if (version != 0x00010000UL && version != 0x00020000UL && version != 0x00030000UL) {
