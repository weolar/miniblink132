...
#define FT_Stream_SeekSet(stream, off) (stream)->cursor = ((off) < (FT_ULong)((stream)->limit - (stream)->base)) ? (stream)->base + (off) : (stream)->limit
...
/**************************************************************************
   ...
   */
static FT_UShort* ft_var_readpackedpoints(FT_Stream stream, FT_ULong size, FT_UInt* point_cnt)
{
...
/**************************************************************************
   ...
   *   step).
   */
static FT_Fixed* ft_var_readpackeddeltas(FT_Stream stream, FT_ULong size, FT_UInt delta_cnt)
{
...
static void ft_var_load_avar(TT_Face face)
{
    ...
    for (i = 0; i < axisCount; i++, segment++) {
        ...
        if ((FT_ULong)segment->pairCount * 4 > table_len || FT_NEW_ARRAY(segment->correspondence, segment->pairCount)) {
        ...
        for (j = 0; j < segment->pairCount; j++) {
            ...
            FT_TRACE5(("    mapping %.5f to %.5f\n", segment->correspondence[j].fromCoord / 65536.0, segment->correspondence[j].toCoord / 65536.0));
        }
...
static FT_Error ft_var_load_item_variation_store(TT_Face face, FT_ULong offset, GX_ItemVarStore itemStore)
{
    FT_Stream stream = FT_FACE_STREAM(face);
    FT_Memory memory = stream->memory;
    ...
    if (FT_READ_USHORT(itemStore->axisCount) || FT_READ_USHORT(itemStore->regionCount))
        goto Exit;
    ...
    for (i = 0; i < itemStore->regionCount; i++) {
        ...
        for (j = 0; j < itemStore->axisCount; j++) {
            ...
            if (FT_READ_SHORT(start) || FT_READ_SHORT(peak) || FT_READ_SHORT(end))
                goto Exit;
    ...
    for (i = 0; i < itemStore->dataCount; i++) {
        ...
        if (FT_READ_USHORT(varData->itemCount) || FT_READ_USHORT(shortDeltaCount) || FT_READ_USHORT(varData->regionIdxCount))
            goto Exit;
        ...
        /* each.                                                           */
        if (FT_NEW_ARRAY(varData->deltaSet, varData->regionIdxCount * varData->itemCount))
            goto Exit;
...
static FT_Error ft_var_load_delta_set_index_mapping(TT_Face face, FT_ULong offset, GX_DeltaSetIdxMap map, GX_ItemVarStore itemStore)
{
    FT_Stream stream = FT_FACE_STREAM(face);
    FT_Memory memory = stream->memory;
    ...
    if (FT_STREAM_SEEK(offset) || FT_READ_USHORT(format) || FT_READ_USHORT(map->mapCount))
        goto Exit;
...
static FT_Error ft_var_load_hvvar(TT_Face face, FT_Bool vertical)
{
    ...
    error = ft_var_load_item_variation_store(face, table_offset + store_offset, &table->itemStore);
    if (error)
        goto Exit;

    if (widthMap_offset) {
        error = ft_var_load_delta_set_index_mapping(face, table_offset + widthMap_offset, &table->widthMap, &table->itemStore);
        if (error)
            goto Exit;
    }
...
static FT_Int ft_var_get_item_delta(TT_Face face, GX_ItemVarStore itemStore, FT_UInt outerIndex, FT_UInt innerIndex)
{
    GX_ItemVarData varData;
    FT_Short* deltaSet;
    ...
    for (master = 0; master < varData->regionIdxCount; master++) {
        ...
        for (j = 0; j < itemStore->axisCount; j++, axis++) {
            ...
            /* cumulative product of all the axis scalars */
                ...
                scalar = FT_MulDiv(scalar, face->blend->normalizedcoords[j] - axis->startCoord, axis->peakCoord - axis->startCoord);
            else
                scalar = FT_MulDiv(scalar, axis->endCoord - face->blend->normalizedcoords[j], axis->endCoord - axis->peakCoord);
        } /* per-axis loop */
...
/**************************************************************************
   ...
   */
static FT_Error tt_hvadvance_adjust(TT_Face face, FT_UInt gindex, FT_Int* avalue, FT_Bool vertical)
{
    FT_Error error = FT_Err_Ok;
    ...
    delta = ft_var_get_item_delta(face, &table->itemStore, outerIndex, innerIndex);

    FT_TRACE5(("%s value %d adjusted by %d unit%s (%s)\n", vertical ? "vertical height" : "horizontal width", *avalue, delta, delta == 1 ? "" : "s",
        vertical ? "VVAR" : "HVAR"));
...
static void ft_var_load_mvar(TT_Face face)
{
    ...
    if (FT_STREAM_SKIP(4) || FT_READ_USHORT(blend->mvar_table->valueCount) || FT_READ_USHORT(store_offset))
        return;

    records_offset = FT_STREAM_POS();

    error = ft_var_load_item_variation_store(face, table_offset + store_offset, &blend->mvar_table->itemStore);
    if (error)
        return;
    ...
    if (FT_STREAM_SEEK(records_offset) || FT_FRAME_ENTER(blend->mvar_table->valueCount * GX_VALUE_SIZE))
        return;
    ...
    for (; value < limit; value++) {
        ...
        if (value->outerIndex >= itemStore->dataCount || value->innerIndex >= itemStore->varData[value->outerIndex].itemCount) {
            error = FT_THROW(Invalid_Table);
            break;
        }
    }
...
tt_apply_mvar(TT_Face face)
{
    ...
    for (; value < limit; value++) {
        ...
        delta = ft_var_get_item_delta(face, &blend->mvar_table->itemStore, value->outerIndex, value->innerIndex);

        if (p) {
            FT_TRACE5(("value %c%c%c%c (%d unit%s) adjusted by %d unit%s (MVAR)\n", (FT_Char)(value->tag >> 24), (FT_Char)(value->tag >> 16),
                (FT_Char)(value->tag >> 8), (FT_Char)(value->tag), value->unmodified, value->unmodified == 1 ? "" : "s", delta, delta == 1 ? "" : "s"));
    ...
    /* adjust all derived values */
    {
        ...
        root->underline_position = face->postscript.underlinePosition - face->postscript.underlineThickness / 2;
...
#define FT_STRUCTURE GX_GVar_Head
        ...
        FT_FRAME_START(20), FT_FRAME_LONG(version), FT_FRAME_USHORT(axisCount), FT_FRAME_USHORT(globalCoordCount), FT_FRAME_ULONG(offsetToCoord),
        FT_FRAME_USHORT(glyphCount), FT_FRAME_USHORT(flags), FT_FRAME_ULONG(offsetToData), FT_FRAME_END
    };
    ...
    if ((FT_ULong)gvar_head.globalCoordCount * gvar_head.axisCount > table_len / 2) {
    ...
    if ((FT_ULong)gvar_head.glyphCount * ((gvar_head.flags & 1) ? 4 : 2) > table_len) {
    ...
    FT_TRACE5(("gvar: there %s %d shared coordinate%s:\n", blend->tuplecount == 1 ? "is" : "are", blend->tuplecount, blend->tuplecount == 1 ? "" : "s"));
    ...
    if (blend->tuplecount != 0) {
        ...
        if (FT_STREAM_SEEK(gvar_start + gvar_head.offsetToCoord) || FT_FRAME_ENTER(blend->tuplecount * gvar_head.axisCount * 2L))
            goto Exit;

        for (i = 0; i < blend->tuplecount; i++) {
            FT_TRACE5(("  [ "));
            for (j = 0; j < (FT_UInt)gvar_head.axisCount; j++) {
                blend->tuplecoords[i * gvar_head.axisCount + j] = FT_GET_SHORT() * 4; /* convert to FT_Fixed */
                FT_TRACE5(("%.5f ", blend->tuplecoords[i * gvar_head.axisCount + j] / 65536.0));
            }
            FT_TRACE5(("]\n"));
        }
...
/**************************************************************************
   ...
   */
static FT_Fixed ft_var_apply_tuple(GX_Blend blend, FT_UShort tupleIndex, FT_Fixed* tuple_coords, FT_Fixed* im_start_coords, FT_Fixed* im_end_coords)
{
    FT_UInt i;
    ...
    for (i = 0; i < blend->num_axis; i++) {
        ...
        if (!(tupleIndex & GX_TI_INTERMEDIATE_TUPLE)) {
            ...
            if (blend->normalizedcoords[i] < FT_MIN(0, tuple_coords[i]) || blend->normalizedcoords[i] > FT_MAX(0, tuple_coords[i])) {
            ...
            apply = FT_MulDiv(apply, blend->normalizedcoords[i], tuple_coords[i]);
        } else {
            /* intermediate tuple */

            if (blend->normalizedcoords[i] <= im_start_coords[i] || blend->normalizedcoords[i] >= im_end_coords[i]) {
            ...
            if (blend->normalizedcoords[i] < tuple_coords[i])
                apply = FT_MulDiv(apply, blend->normalizedcoords[i] - im_start_coords[i], tuple_coords[i] - im_start_coords[i]);
            else
                apply = FT_MulDiv(apply, im_end_coords[i] - blend->normalizedcoords[i], im_end_coords[i] - tuple_coords[i]);
        }
    }
...
static void ft_var_to_normalized(TT_Face face, FT_UInt num_coords, FT_Fixed* coords, FT_Fixed* normalized)
{
    GX_Blend blend;
    FT_MM_Var* mmvar;
    FT_UInt i, j;
    ...
    for (i = 0; i < num_coords; i++, a++) {
        ...
        if (coord < a->def)
            normalized[i] = -FT_DivFix(SUB_LONG(coord, a->def), SUB_LONG(a->minimum, a->def));
        else if (coord > a->def)
            normalized[i] = FT_DivFix(SUB_LONG(coord, a->def), SUB_LONG(a->maximum, a->def));
        else
            normalized[i] = 0;
    }
        ...
        for (i = 0; i < mmvar->num_axis; i++, av++) {
            for (j = 1; j < (FT_UInt)av->pairCount; j++) {
                if (normalized[i] < av->correspondence[j].fromCoord) {
                    ...
                    normalized[i]
                        = FT_MulDiv(normalized[i] - av->correspondence[j - 1].fromCoord, av->correspondence[j].toCoord - av->correspondence[j - 1].toCoord,
...
static void ft_var_to_design(TT_Face face, FT_UInt num_coords, FT_Fixed* coords, FT_Fixed* design)
{
    GX_Blend blend;
    FT_MM_Var* mmvar;
    FT_Var_Axis* a;
        ...
        for (i = 0; i < nc; i++, av++) {
            for (j = 1; j < (FT_UInt)av->pairCount; j++) {
                if (design[i] < av->correspondence[j].toCoord) {
                    design[i] = FT_MulDiv(design[i] - av->correspondence[j - 1].toCoord, av->correspondence[j].fromCoord - av->correspondence[j - 1].fromCoord,
...
#define FT_STRUCTURE GX_FVar_Head
        ...
        FT_FRAME_START(16), FT_FRAME_LONG(version), FT_FRAME_USHORT(offsetToData), FT_FRAME_SKIP_SHORT, FT_FRAME_USHORT(axisCount), FT_FRAME_USHORT(axisSize),
        FT_FRAME_USHORT(instanceCount), FT_FRAME_USHORT(instanceSize), FT_FRAME_END
    };
...
#define FT_STRUCTURE GX_FVar_Axis
        ...
        FT_FRAME_START(20), FT_FRAME_ULONG(axisTag), FT_FRAME_LONG(minValue), FT_FRAME_LONG(defaultValue), FT_FRAME_LONG(maxValue), FT_FRAME_USHORT(flags),
    ...
    if (need_init) {
        ...
        if (FT_SET_ERROR(face->goto_table(face, TTAG_gvar, stream, &table_len))) {
            ...
            if (FT_SET_ERROR(face->goto_table(face, TTAG_CFF2, stream, &table_len))) {
                FT_TRACE1(("\n"
        ...
        usePsName = FT_BOOL(fvar_head.instanceSize == 6 + 4 * fvar_head.axisCount);

        FT_TRACE2(("loaded\n"));

        FT_TRACE5(("%d variation ax%s\n", fvar_head.axisCount, fvar_head.axisCount == 1 ? "is" : "es"));
...
#define ALIGN_SIZE(n) (((n) + sizeof(void*) - 1) & ~(sizeof(void*) - 1))
    ...
    if (need_init) {
        face->blend->mmvar_len = mmvar_size + axis_flags_size + axis_size + namedstyle_size + next_coords_size + next_name_size;
        ...
        mmvar->namedstyle = (FT_Var_Named_Style*)((char*)mmvar->axis + axis_size);
        ...
        next_name = (FT_String*)((char*)mmvar->namedstyle + namedstyle_size + next_coords_size);
        ...
        for (i = 0; i < num_axes; i++) {
            ...
            *axis_flags = axis_rec.flags;
            ...
            FT_TRACE5(("  %3d  `%s'"
                ...
                i, a->name, a->minimum / 65536.0, a->def / 65536.0, a->maximum / 65536.0, *axis_flags, invalid ? " (invalid, disabled)" : ""));
#endif
        ...
        if (FT_NEW_ARRAY(face->blend->normalized_stylecoords, num_axes * num_instances))
            goto Exit;
        ...
        FT_TRACE5(("%d instance%s\n", fvar_head.instanceCount, fvar_head.instanceCount == 1 ? "" : "s"));
        ...
        for (i = 0; i < fvar_head.instanceCount; i++, ns++) {
...
#ifdef FT_DEBUG_LEVEL_TRACE
            {
                ...
                FT_TRACE5(("  instance %d (%s%s%s, %s%s%s)\n", i, strname ? "name: `" : "", strname ? strname : "unnamed", strname ? "'" : "",
                    psname ? "PS name: `" : "", psname ? psname : "no PS name", psname ? "'" : ""));
        ...
        if (num_instances != fvar_head.instanceCount) {
            ...
            found = sfnt->get_name_id(face, TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY, &dummy1, &dummy2);
            if (found)
                strid = TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY;
            else {
                found = sfnt->get_name_id(face, TT_NAME_ID_FONT_SUBFAMILY, &dummy1, &dummy2);
                if (found)
                    strid = TT_NAME_ID_FONT_SUBFAMILY;
            }

            if (found) {
                found = sfnt->get_name_id(face, TT_NAME_ID_PS_NAME, &dummy1, &dummy2);
                if (found) {
...
static FT_Error tt_set_mm_blend(TT_Face face, FT_UInt num_coords, FT_Fixed* coords, FT_Bool set_design_coords)
{
    FT_Error error = FT_Err_Ok;
    GX_Blend blend;
    FT_MM_Var* mmvar;
    ...
    if (!blend->normalizedcoords) {
        ...
        if (FT_IS_NAMED_INSTANCE(FT_FACE(face))) {
            ...
            n = blend->normalized_stylecoords + (instance_index - 1) * mmvar->num_axis + i;
    ...
    if (set_design_coords)
        ft_var_to_design(face, all_design_coords ? blend->num_axis : num_coords, blend->normalizedcoords, blend->coords);
...
TT_Set_Named_Instance(TT_Face face, FT_UInt instance_index)
{
    ...
    if (instance_index > 0 && mmvar->namedstyle) {
        ...
        error = sfnt->get_name(face, (FT_UShort)named_style->strid, &style_name);
        if (error)
            goto Exit;
    ...
    face->root.face_index = (instance_index << 16) | (face->root.face_index & 0xFFFFL);
    face->root.face_flags &= ~FT_FACE_FLAG_VARIATION;
...
tt_face_vary_cvt(TT_Face face, FT_Stream stream)
{
    ...
    if (FT_NEW_ARRAY(tuple_coords, blend->num_axis) || FT_NEW_ARRAY(im_start_coords, blend->num_axis) || FT_NEW_ARRAY(im_end_coords, blend->num_axis))
        goto FExit;
    ...
    if (tupleCount & GX_TC_TUPLES_SHARE_POINT_NUMBERS) {
        ...
        sharedpoints = ft_var_readpackedpoints(stream, table_len, &spoint_count);
        offsetToData = FT_Stream_FTell(stream);

        FT_Stream_SeekSet(stream, here);
    }

    FT_TRACE5(("cvar: there %s %d tuple%s:\n", (tupleCount & GX_TC_TUPLE_COUNT_MASK) == 1 ? "is" : "are", tupleCount & GX_TC_TUPLE_COUNT_MASK,
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++) {
        ...
        if (tupleIndex & GX_TI_EMBEDDED_TUPLE_COORD) {
            ...
            FT_MEM_COPY(tuple_coords, blend->tuplecoords + (tupleIndex & GX_TI_TUPLE_INDEX_MASK) * blend->num_axis, blend->num_axis * sizeof(FT_Fixed));
        }
        ...
        apply = ft_var_apply_tuple(blend, (FT_UShort)tupleIndex, tuple_coords, im_start_coords, im_end_coords);
        ...
        if (tupleIndex & GX_TI_PRIVATE_POINT_NUMBERS) {
            localpoints = ft_var_readpackedpoints(stream, table_len, &point_count);
            points = localpoints;
        } else {
            points = sharedpoints;
        ...
        deltas = ft_var_readpackeddeltas(stream, table_len, point_count == 0 ? face->cvt_size : point_count);

        if (!points || !deltas || (localpoints == ALL_POINTS && point_count != face->cvt_size))
            ; /* failure, ignore it */
            ...
            for (j = 0; j < face->cvt_size; j++) {
                ...
                if (old_cvt_delta != cvt_deltas[j]) {
                    FT_TRACE7(("      %d: %f -> %f\n", j, (FT_intToFixed(face->cvt[j]) + old_cvt_delta) / 65536.0,
                        (FT_intToFixed(face->cvt[j]) + cvt_deltas[j]) / 65536.0));
                    count++;
                }
#endif
            }
            ...
            for (j = 0; j < point_count; j++) {
                ...
                if (old_cvt_delta != cvt_deltas[pindex]) {
                    FT_TRACE7(("      %d: %f -> %f\n", pindex, (FT_intToFixed(face->cvt[pindex]) + old_cvt_delta) / 65536.0,
                        (FT_intToFixed(face->cvt[pindex]) + cvt_deltas[pindex]) / 65536.0));
                    count++;
                }
#endif
            }
...
static void tt_delta_shift(int p1, int p2, int ref, FT_Vector* in_points, FT_Vector* out_points)
{
    int p;
    FT_Vector delta;
...
static void tt_delta_interpolate(int p1, int p2, int ref1, int ref2, FT_Vector* in_points, FT_Vector* out_points)
{
    int p, i;
...
static void tt_interpolate_deltas(FT_Outline* outline, FT_Vector* out_points, FT_Vector* in_points, FT_Bool* has_delta)
{
    FT_Int first_point;
    FT_Int end_point;
    ...
    do {
        ...
        if (point <= end_point) {
            ...
            while (point <= end_point) {
                ...
                if (has_delta[point]) {
                    tt_delta_interpolate(cur_delta + 1, point - 1, cur_delta, point, in_points, out_points);
                    cur_delta = point;
                }
            ...
            if (cur_delta == first_delta)
                tt_delta_shift(first_point, end_point, cur_delta, in_points, out_points);
            else {
                /* otherwise handle remaining points       */
                /* at the end and beginning of the contour */
                tt_delta_interpolate(cur_delta + 1, end_point, cur_delta, first_delta, in_points, out_points);

                if (first_delta > 0)
                    tt_delta_interpolate(first_point, first_delta - 1, cur_delta, first_delta, in_points, out_points);
            }
        }
        contour++;
...
TT_Vary_Apply_Glyph_Deltas(TT_Face face, FT_UInt glyph_index, FT_Outline* outline, FT_UInt n_points)
{
    ...
    if (FT_NEW_ARRAY(points_org, n_points) || FT_NEW_ARRAY(points_out, n_points) || FT_NEW_ARRAY(has_delta, n_points))
        goto Fail1;

    dataSize = blend->glyphoffsets[glyph_index + 1] - blend->glyphoffsets[glyph_index];

    if (FT_STREAM_SEEK(blend->glyphoffsets[glyph_index]) || FT_FRAME_ENTER(dataSize))
        goto Fail1;
    ...
    /* rough sanity test */
    if (offsetToData > dataSize || (tupleCount & GX_TC_TUPLE_COUNT_MASK) * 4 > dataSize) {
        FT_TRACE2(("TT_Vary_Apply_Glyph_Deltas:"
    ...
    if (tupleCount & GX_TC_TUPLES_SHARE_POINT_NUMBERS) {
        ...
        sharedpoints = ft_var_readpackedpoints(stream, blend->gvar_size, &spoint_count);
        offsetToData = FT_Stream_FTell(stream);
    ...
    if (FT_NEW_ARRAY(point_deltas_x, n_points) || FT_NEW_ARRAY(point_deltas_y, n_points))
        goto Fail3;
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++) {
        ...
        if (tupleIndex & GX_TI_PRIVATE_POINT_NUMBERS) {
            localpoints = ft_var_readpackedpoints(stream, blend->gvar_size, &point_count);
            points = localpoints;
        } else {
            points = sharedpoints;
            point_count = spoint_count;
        ...
        deltas_x = ft_var_readpackeddeltas(stream, blend->gvar_size, point_count == 0 ? n_points : point_count);
        deltas_y = ft_var_readpackeddeltas(stream, blend->gvar_size, point_count == 0 ? n_points : point_count);
        ...
        if (!points || !deltas_y || !deltas_x)
            ...
            for (j = 0; j < n_points; j++) {
                ...
                if (j < n_points - 4) {
                    ...
                    if (j == (n_points - 4) && !(face->variation_support & TT_FACE_FLAG_VAR_LSB))
                        point_deltas_x[j] = old_point_delta_x + point_delta_x;

                    else if (j == (n_points - 3) && !(face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
                        point_deltas_x[j] = old_point_delta_x + point_delta_x;

                    else if (j == (n_points - 2) && !(face->variation_support & TT_FACE_FLAG_VAR_TSB))
                        point_deltas_y[j] = old_point_delta_y + point_delta_y;

                    else if (j == (n_points - 1) && !(face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
                        point_deltas_y[j] = old_point_delta_y + point_delta_y;
                }

#ifdef FT_DEBUG_LEVEL_TRACE
                if (point_delta_x || point_delta_y) {
                    FT_TRACE7(("      %d: (%f, %f) -> (%f, %f)\n", j, (FT_intToFixed(outline->points[j].x) + old_point_delta_x) / 65536.0,
                        (FT_intToFixed(outline->points[j].y) + old_point_delta_y) / 65536.0,
                        (FT_intToFixed(outline->points[j].x) + point_deltas_x[j]) / 65536.0,
                        (FT_intToFixed(outline->points[j].y) + point_deltas_y[j]) / 65536.0));
                    count++;
                }
#endif
            }
...
/**************************************************************************
   ...
   */
FT_LOCAL_DEF(FT_Error)
tt_get_var_blend(TT_Face face, FT_UInt* num_coords, FT_Fixed** coords, FT_Fixed** normalizedcoords, FT_MM_Var** mm_var)
{
    if (face->blend) {
        if (num_coords)
...
static void ft_var_done_item_variation_store(TT_Face face, GX_ItemVarStore itemStore)
{
...
tt_done_blend(TT_Face face)
{
    ...
    if (blend) {
        ...
        if (blend->hvar_table) {
            ft_var_done_item_variation_store(face, &blend->hvar_table->itemStore);
        ...
        if (blend->vvar_table) {
            ft_var_done_item_variation_store(face, &blend->vvar_table->itemStore);
        ...
        if (blend->mvar_table) {
            ft_var_done_item_variation_store(face, &blend->mvar_table->itemStore);
