...
#if defined(TT_USE_BYTECODE_INTERPRETER) && defined(TT_SUPPORT_SUBPIXEL_HINTING_INFINALITY)
...
static const SPH_Font_Class FAMILY_CLASS_Rules[FAMILY_CLASS_RULES_SIZE] = {
    {
        "MS Legacy Fonts",
        {
            "Aharoni",
...
static const SPH_Font_Class STYLE_CLASS_Rules[STYLE_CLASS_RULES_SIZE] = {
    {
        "Regular Class",
        {
            "Regular",
...
static const SPH_TweakRule PIXEL_HINTING_Rules[PIXEL_HINTING_RULES_SIZE] = {
...
static const SPH_TweakRule DO_SHPIX_Rules[DO_SHPIX_RULES_SIZE] = {
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule SKIP_NONPIXEL_Y_MOVES_Rules[SKIP_NONPIXEL_Y_MOVES_RULES_SIZE] = {
    /* fix vwxyz thinness*/
    { "Consolas", 0, "", 0 },
...
static const SPH_TweakRule SKIP_NONPIXEL_Y_MOVES_Rules_Exceptions[SKIP_NONPIXEL_Y_MOVES_RULES_EXCEPTIONS_SIZE] = {
    /* Fixes < and > */
...
static const SPH_TweakRule SKIP_NONPIXEL_Y_MOVES_DELTAP_Rules[SKIP_NONPIXEL_Y_MOVES_DELTAP_RULES_SIZE] = {
...
static const SPH_TweakRule SKIP_OFFPIXEL_Y_MOVES_Rules[SKIP_OFFPIXEL_Y_MOVES_RULES_SIZE] = {
    { "-", 0, "", 0 },
};

#define SKIP_OFFPIXEL_Y_MOVES_RULES_EXCEPTIONS_SIZE 1

static const SPH_TweakRule SKIP_OFFPIXEL_Y_MOVES_Rules_Exceptions[SKIP_OFFPIXEL_Y_MOVES_RULES_EXCEPTIONS_SIZE] = {
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule ROUND_NONPIXEL_Y_MOVES_Rules[ROUND_NONPIXEL_Y_MOVES_RULES_SIZE] = {
...
static const SPH_TweakRule ROUND_NONPIXEL_Y_MOVES_Rules_Exceptions[ROUND_NONPIXEL_Y_MOVES_RULES_EXCEPTIONS_SIZE] = {
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule SKIP_IUP_Rules[SKIP_IUP_RULES_SIZE] = {
    { "Arial", 13, "Regular", 'a' },
};
...
static const SPH_TweakRule ALWAYS_SKIP_DELTAP_Rules[ALWAYS_SKIP_DELTAP_RULES_SIZE] = {
    { "Georgia", 0, "Regular", 'k' },
...
static const SPH_TweakRule ALWAYS_DO_DELTAP_Rules[ALWAYS_DO_DELTAP_RULES_SIZE] = {
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule NO_ALIGNRP_AFTER_IUP_Rules[NO_ALIGNRP_AFTER_IUP_RULES_SIZE] = {
...
static const SPH_TweakRule NO_DELTAP_AFTER_IUP_Rules[NO_DELTAP_AFTER_IUP_RULES_SIZE] = {
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule NO_CALL_AFTER_IUP_Rules[NO_CALL_AFTER_IUP_RULES_SIZE] = {
    /* Prevent creation of dents in outline */
    { "-", 0, "", 0 },
};
...
static const SPH_TweakRule DEEMBOLDEN_Rules[DEEMBOLDEN_RULES_SIZE] = {
...
static const SPH_TweakRule EMBOLDEN_Rules[EMBOLDEN_RULES_SIZE] = {
    { "Courier New", 0, "Regular", 0 },
...
static const SPH_TweakRule TIMES_NEW_ROMAN_HACK_Rules[TIMES_NEW_ROMAN_HACK_RULES_SIZE] = {
...
static const SPH_TweakRule COURIER_NEW_2_HACK_Rules[COURIER_NEW_2_HACK_RULES_SIZE] = {
...
static const SPH_TweakRule COMPATIBLE_WIDTHS_Rules[COMPATIBLE_WIDTHS_RULES_SIZE] = {
...
static FT_Bool is_member_of_family_class(const FT_String* detected_font_name, const FT_String* rule_font_name)
{
    FT_UInt i, j;
        ...
        if (ft_strcmp(FAMILY_CLASS_Rules[i].name, rule_font_name) == 0) {
            for (j = 0; j < SPH_MAX_CLASS_MEMBERS; j++) {
                if (ft_strcmp(FAMILY_CLASS_Rules[i].member[j], "") == 0)
                    continue;
                if (ft_strcmp(FAMILY_CLASS_Rules[i].member[j], detected_font_name) == 0)
                    return TRUE;
            }
        }
    }
...
static FT_Bool is_member_of_style_class(const FT_String* detected_font_style, const FT_String* rule_font_style)
{
    FT_UInt i, j;
        ...
        if (ft_strcmp(STYLE_CLASS_Rules[i].name, rule_font_style) == 0) {
            for (j = 0; j < SPH_MAX_CLASS_MEMBERS; j++) {
                if (ft_strcmp(STYLE_CLASS_Rules[i].member[j], "") == 0)
                    continue;
                if (ft_strcmp(STYLE_CLASS_Rules[i].member[j], detected_font_style) == 0)
                    return TRUE;
            }
        }
    }
...
sph_test_tweak(TT_Face face, const FT_String* family, FT_UInt ppem, const FT_String* style, FT_UInt glyph_index, const SPH_TweakRule* rule, FT_UInt num_rules)
{
    FT_UInt i;

    /* rule checks may be able to be optimized further */
    for (i = 0; i < num_rules; i++) {
        if (family && (is_member_of_family_class(family, rule[i].family)))
            if (rule[i].ppem == 0 || rule[i].ppem == ppem)
                if (style && is_member_of_style_class(style, rule[i].style))
                    if (rule[i].glyph == 0 || FT_Get_Char_Index((FT_Face)face, rule[i].glyph) == glyph_index)
                        return TRUE;
    }
...
static FT_UInt scale_test_tweak(
    TT_Face face, const FT_String* family, FT_UInt ppem, const FT_String* style, FT_UInt glyph_index, const SPH_ScaleRule* rule, FT_UInt num_rules)
{
    FT_UInt i;
...
sph_test_tweak_x_scaling(TT_Face face, const FT_String* family, FT_UInt ppem, const FT_String* style, FT_UInt glyph_index)
{
    return scale_test_tweak(face, family, ppem, style, glyph_index, X_SCALING_Rules, X_SCALING_RULES_SIZE);
}

#define TWEAK_RULES(x)                                                                                                                                         \
    if (sph_test_tweak(face, family, ppem, style, glyph_index, x##_Rules, x##_RULES_SIZE))                                                                     \
        loader->exec->sph_tweak_flags |= SPH_TWEAK_##x;

#define TWEAK_RULES_EXCEPTIONS(x)                                                                                                                              \
    if (sph_test_tweak(face, family, ppem, style, glyph_index, x##_Rules_Exceptions, x##_RULES_EXCEPTIONS_SIZE))                                               \
        loader->exec->sph_tweak_flags &= ~SPH_TWEAK_##x;
...
sph_set_tweaks(TT_Loader loader, FT_UInt glyph_index)
{
    ...
    if (sph_test_tweak(face, family, ppem, style, glyph_index, COMPATIBILITY_MODE_Rules, COMPATIBILITY_MODE_RULES_SIZE))
        loader->exec->face->sph_compatibility_mode = TRUE;

    if (IS_HINTED(loader->load_flags)) {
        if (sph_test_tweak(face, family, ppem, style, glyph_index, COMPATIBLE_WIDTHS_Rules, COMPATIBLE_WIDTHS_RULES_SIZE))
