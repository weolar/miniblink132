...
static FT_Error tt_size_request(FT_Size size, FT_Size_Request req)
{
    ...
    if (FT_IS_SCALABLE(size->face)) {
        ...
        if (!error) {
            FT_UInt resolution = ttsize->metrics->x_ppem > ttsize->metrics->y_ppem ? req->horiResolution : req->vertResolution;

            /* if we don't have a resolution value, assume 72dpi */
            if (req->type == FT_SIZE_REQUEST_TYPE_SCALES || !resolution)
                resolution = 72;

            ttsize->point_size = FT_MulDiv(ttsize->ttmetrics.ppem, 64 * 72, resolution);
        }
#endif
    }
    ...
    FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    if (glyph_index >= (FT_UInt)face->num_glyphs && !face->internal->incremental_interface)
#else
    ...
    size->metrics = (load_flags & FT_LOAD_NO_HINTING) ? &ttsize->metrics : &size->hinted_metrics;
...
FT_DEFINE_SERVICE_METRICSVARIATIONSREC(tt_service_metrics_variations,
...
FT_DEFINE_SERVICEDESCREC6(tt_services,
    ...
    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_TRUETYPE, FT_SERVICE_ID_MULTI_MASTERS, &tt_service_gx_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
    &tt_service_metrics_variations, FT_SERVICE_ID_TRUETYPE_ENGINE, &tt_service_truetype_engine, FT_SERVICE_ID_TT_GLYF, &tt_service_truetype_glyf,
    FT_SERVICE_ID_PROPERTIES, &tt_service_properties)
#else
FT_DEFINE_SERVICEDESCREC4(tt_services,

    FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_TRUETYPE, FT_SERVICE_ID_TRUETYPE_ENGINE, &tt_service_truetype_engine, FT_SERVICE_ID_TT_GLYF,
    &tt_service_truetype_glyf, FT_SERVICE_ID_PROPERTIES, &tt_service_properties)
#endif
...
FT_DEFINE_DRIVER(tt_driver_class,
    ...
    FT_MODULE_FONT_DRIVER | FT_MODULE_DRIVER_SCALABLE | TT_HINTER_FLAG,
    ...
    sizeof(TT_FaceRec), sizeof(TT_SizeRec), sizeof(FT_GlyphSlotRec),
