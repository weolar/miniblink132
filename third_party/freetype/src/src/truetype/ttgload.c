...
TT_Get_VMetrics(TT_Face face, FT_UInt idx, FT_Pos yMax, FT_Short* tsb, FT_UShort* ah)
{
    if (face->vertical_info)
        ...
        *ah = (FT_UShort)FT_ABS(face->os2.sTypoAscender - face->os2.sTypoDescender);
    }

    else {
        *tsb = (FT_Short)(face->horizontal.Ascender - yMax);
        *ah = (FT_UShort)FT_ABS(face->horizontal.Ascender - face->horizontal.Descender);
    }
...
static FT_Error tt_get_metrics(TT_Loader loader, FT_UInt glyph_index)
{
    ...
    TT_Get_VMetrics(face, glyph_index, loader->bbox.yMax, &top_bearing, &advance_height);
...
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_INFINALITY
    if (driver->interpreter_version == TT_INTERPRETER_VERSION_38 && loader->exec) {
...
static void tt_get_metrics_incr_overrides(TT_Loader loader, FT_UInt glyph_index)
{
    ...
    if (face->root.internal->incremental_interface && face->root.internal->incremental_interface->funcs->get_glyph_metrics) {
        ...
        error = face->root.internal->incremental_interface->funcs->get_glyph_metrics(
            face->root.internal->incremental_interface->object, glyph_index, FALSE, &incr_metrics);
        if (error)
            goto Exit;
...
TT_Load_Simple_Glyph(TT_Loader load)
{
    ...
    if (IS_HINTED(load->load_flags)) {
        ...
        error = Update_Max(load->exec->memory, &tmp, sizeof(FT_Byte), (void*)&load->exec->glyphIns, n_ins);
...
#ifdef FT_DEBUG_LEVEL_TRACE
    {
        ...
        for (i = 0; i < num_subglyphs; i++) {
            ...
            if (subglyph->flags & WE_HAVE_A_SCALE)
                ...
                FT_TRACE7(("      scaling: x=%f, y=%f\n", subglyph->transform.xx / 65536.0, subglyph->transform.yy / 65536.0));
            else if (subglyph->flags & WE_HAVE_A_2X2)
                FT_TRACE7(("      scaling: xx=%f, yx=%f\n"
                           "               xy=%f, yy=%f\n",
                    subglyph->transform.xx / 65536.0, subglyph->transform.yx / 65536.0, subglyph->transform.xy / 65536.0, subglyph->transform.yy / 65536.0));
...
{
#if defined TT_SUPPORT_SUBPIXEL_HINTING_INFINALITY || defined TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
    TT_Face face = loader->face;
    TT_Driver driver = (TT_Driver)FT_FACE_DRIVER(face);
#endif
    ...
    zone->cur[zone->n_points - 4].x = FT_PIX_ROUND(zone->cur[zone->n_points - 4].x);
    zone->cur[zone->n_points - 3].x = FT_PIX_ROUND(zone->cur[zone->n_points - 3].x);
    zone->cur[zone->n_points - 2].y = FT_PIX_ROUND(zone->cur[zone->n_points - 2].y);
    ...
    if (n_ins > 0) {
        ...
        TT_Set_CodeRange(loader->exec, tt_coderange_glyph, loader->exec->glyphIns, n_ins);
        ...
        current_outline.tags[0] |= (loader->exec->GS.scan_type << 5) | FT_CURVE_TAG_HAS_SCANMODE;
    }
    ...
    if (!(driver->interpreter_version == TT_INTERPRETER_VERSION_40 && loader->exec->backward_compatibility)) {
#endif
...
static FT_Error TT_Process_Simple_Glyph(TT_Loader loader)
{
    ...
    if (FT_IS_NAMED_INSTANCE(FT_FACE(loader->face)) || FT_IS_VARIATION(FT_FACE(loader->face))) {
        /* Deltas apply to the unscaled data. */
        error = TT_Vary_Apply_Glyph_Deltas(loader->face, loader->glyph_index, outline, (FT_UInt)n_points);

        /* recalculate linear horizontal and vertical advances */
        /* if we don't have HVAR and VVAR, respectively        */
        if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
            loader->linear = outline->points[n_points - 3].x - outline->points[n_points - 4].x;
        if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
            loader->vadvance = outline->points[n_points - 1].x - outline->points[n_points - 2].x;
    ...
    if (IS_HINTED(loader->load_flags)) {
        ...
        FT_ARRAY_COPY(loader->zone.orus, loader->zone.cur, loader->zone.n_points + 4);
    }

    {
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38) {
            ...
            if (IS_HINTED(loader->load_flags))
                x_scale_factor = sph_test_tweak_x_scaling(face, family, ppem, style, loader->glyph_index);
            /* scale the glyph */
            if ((loader->load_flags & FT_LOAD_NO_SCALE) == 0 || x_scale_factor != 1000) {
                x_scale = FT_MulDiv(loader->size->metrics->x_scale, (FT_Long)x_scale_factor, 1000);
        ...
        if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_HADVANCE) || !IS_HINTED(loader->load_flags))
#endif
        {
        ...
        if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_VADVANCE) || !IS_HINTED(loader->load_flags))
#endif
        {
...
static FT_Error TT_Process_Composite_Component(TT_Loader loader, FT_SubGlyph subglyph, FT_UInt start_point, FT_UInt num_base_points)
{
    ...
    have_scale = FT_BOOL(subglyph->flags & (WE_HAVE_A_SCALE | WE_HAVE_AN_XY_SCALE | WE_HAVE_A_2X2));
    ...
    if (!(subglyph->flags & ARGS_ARE_XY_VALUES)) {
         ...
         */
            FT_Fixed mac_xscale = FT_Hypot(subglyph->transform.xx, subglyph->transform.xy);
            FT_Fixed mac_yscale = FT_Hypot(subglyph->transform.yy, subglyph->transform.yx);
...
static FT_Error TT_Process_Composite_Glyph(TT_Loader loader, FT_UInt start_point, FT_UInt start_contour)
{
    ...
    error = FT_GLYPHLOADER_CHECK_POINTS(loader->gloader, outline->n_points + 4, 0);
    if (error)
        return error;
...
static void tt_loader_set_pp(TT_Loader loader)
{
    ...
    if (driver->interpreter_version == TT_INTERPRETER_VERSION_40) {
        subpixel_hinting = loader->exec ? loader->exec->subpixel_hinting_lean : 0;
...
static FT_Error load_truetype_glyph(TT_Loader loader, FT_UInt glyph_index, FT_UInt recurse_count, FT_Bool header_only)
{
    ...
    if (face->root.internal->incremental_interface) {
        error = face->root.internal->incremental_interface->funcs->get_glyph_data(face->root.internal->incremental_interface->object, glyph_index, &glyph_data);
        if (error)
            goto Exit;
        ...
        FT_ZERO(&inc_stream);
        FT_Stream_OpenMemory(&inc_stream, glyph_data.pointer, (FT_ULong)glyph_data.length);
    ...
    if (loader->byte_len > 0) {
        ...
        error = face->access_glyph_frame(loader, glyph_index, face->glyf_offset + offset, (FT_UInt)loader->byte_len);
        if (error)
            goto Exit;
    ...
    if (loader->byte_len == 0 || loader->n_contours == 0) {
        ...
        if (FT_IS_NAMED_INSTANCE(FT_FACE(face)) || FT_IS_VARIATION(FT_FACE(face))) {
            ...
            error = TT_Vary_Apply_Glyph_Deltas(loader->face, glyph_index, &outline, (FT_UInt)outline.n_points);
            if (error)
                goto Exit;
    ...
    error = face->access_glyph_frame(loader, glyph_index, face->glyf_offset + offset + 10, (FT_UInt)loader->byte_len - 10);
    if (error)
        goto Exit;
    ...
    /* otherwise, load a composite! */
       ...
       */
        ...
        if (FT_IS_NAMED_INSTANCE(FT_FACE(face)) || FT_IS_VARIATION(FT_FACE(face))) {
            ...
            if (FT_NEW_ARRAY(points, outline.n_points) || FT_NEW_ARRAY(tags, outline.n_points) || FT_NEW_ARRAY(contours, outline.n_points))
                goto Exit1;
            ...
            if (FT_SET_ERROR(TT_Vary_Apply_Glyph_Deltas(face, glyph_index, &outline, (FT_UInt)outline.n_points)))
                goto Exit1;
...

        {
            ...
            for (n = 0; n < num_subglyphs; n++) {
                ...
                error = load_truetype_glyph(loader, (FT_UInt)subglyph->index, recurse_count + 1, FALSE);
                if (error)
                    goto Exit;
                ...
                error = TT_Process_Composite_Component(loader, subglyph, start_point, num_base_points);
                if (error)
                    goto Exit;
            }
            ...
            if (IS_HINTED(loader->load_flags) &&
                ...
                error = TT_Process_Composite_Glyph(loader, start_point, start_contour);
                if (error)
                    goto Exit;
            }
        }
...
static FT_Error compute_glyph_metrics(TT_Loader loader, FT_UInt glyph_index)
{
    ...
    if (
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
        !(driver->interpreter_version == TT_INTERPRETER_VERSION_40 && (loader->exec && loader->exec->backward_compatibility)) &&
#endif
        !face->postscript.isFixedPitch && IS_HINTED(loader->load_flags) && !(loader->load_flags & FT_LOAD_COMPUTE_METRICS)) {
        FT_Byte* widthp;

        widthp = tt_face_get_device_metrics(face, size->metrics->x_ppem, glyph_index);
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38) {
            ...
            ignore_x_mode = FT_BOOL(FT_LOAD_TARGET_MODE(loader->load_flags) != FT_RENDER_MODE_MONO);

            if (widthp && ((ignore_x_mode && loader->exec->compatible_widths) || !ignore_x_mode || SPH_OPTION_BITMAP_WIDTHS))
    ...
    /* create some metrics manually                                   */
    {
        ...
        if (face->vertical_info && face->vertical.number_Of_VMetrics > 0) {
            ...
            if (loader->pp3.y <= loader->pp4.y)
                ...
                advance = (FT_UShort)FT_DivFix(loader->pp3.y - loader->pp4.y, y_scale);
        } else {
            FT_Pos height;
            ...
            height = (FT_Short)FT_DivFix(SUB_LONG(bbox.yMax, bbox.yMin), y_scale);
            if (face->os2.version != 0xFFFFU)
                advance = (FT_Pos)(face->os2.sTypoAscender - face->os2.sTypoDescender);
            else
                advance = (FT_Pos)(face->horizontal.Ascender - face->horizontal.Descender);
...
#ifdef FT_CONFIG_OPTION_INCREMENTAL
        {
            ...
            if (incr && incr->funcs->get_glyph_metrics) {
                ...
                error = incr->funcs->get_glyph_metrics(incr->object, glyph_index, TRUE, &incr_metrics);
                if (error)
                    return error;
        ...
        /*      should work fine.                                        */
        /*                                                               */
        glyph->metrics.vertBearingX = SUB_LONG(glyph->metrics.horiBearingX, glyph->metrics.horiAdvance / 2);
...
static FT_Error load_sbit_image(TT_Size size, TT_GlyphSlot glyph, FT_UInt glyph_index, FT_Int32 load_flags)
{
    TT_Face face;
    SFNT_Service sfnt;
    FT_Stream stream;
    ...
    error = sfnt->load_sbit_image(face, size->strike_index, glyph_index, (FT_UInt)load_flags, stream, &glyph->bitmap, &sbit_metrics);
    if (!error) {
...
static FT_Error tt_loader_init(TT_Loader loader, TT_Size size, TT_GlyphSlot glyph, FT_Int32 load_flags, FT_Bool glyf_table_only)
{
    ...
    if (IS_HINTED(load_flags) && !glyf_table_only) {
      ...
      FT_Bool  gray_cleartype;
...
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_40) {
            subpixel_hinting_lean = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != FT_RENDER_MODE_MONO);
            grayscale_cleartype = FT_BOOL(subpixel_hinting_lean && !((load_flags & FT_LOAD_TARGET_LCD) || (load_flags & FT_LOAD_TARGET_LCD_V)));
            exec->vertical_lcd_lean = FT_BOOL(subpixel_hinting_lean && (load_flags & FT_LOAD_TARGET_LCD_V));
        } else {
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38) {
            subpixel_hinting = FT_BOOL((FT_LOAD_TARGET_MODE(load_flags) != FT_RENDER_MODE_MONO) && SPH_OPTION_SET_SUBPIXEL);
...
#else /* 0 */
            exec->compatible_widths = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_COMPATIBLE_WIDTHS);
            exec->symmetrical_smoothing = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_SYMMETRICAL_SMOOTHING);
            exec->bgr = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_BGR);
            exec->vertical_lcd = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_VERTICAL_LCD);
            exec->subpixel_positioned = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_SUBPIXEL_POSITIONED);
            exec->gray_cleartype = FT_BOOL(FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_GRAY_CLEARTYPE);
#endif /* 0 */
...
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
            if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
            grayscale = FT_BOOL(!subpixel_hinting_lean && FT_LOAD_TARGET_MODE(load_flags) != FT_RENDER_MODE_MONO);
        else
#endif
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_38 && exec->GS.instruct_control & 4)
...
{
    FT_List_Finalize(&loader->composites, NULL, loader->face->root.memory, NULL);
}
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
#define IS_DEFAULT_INSTANCE (!(FT_IS_NAMED_INSTANCE(glyph->face) || FT_IS_VARIATION(glyph->face)))
#else
#define IS_DEFAULT_INSTANCE 1
    ...
    if (size->strike_index != 0xFFFFFFFFUL && (load_flags & FT_LOAD_NO_BITMAP) == 0 && IS_DEFAULT_INSTANCE) {
        ...
        if (FT_ERR_EQ(error, Missing_Bitmap)) {
            ...
            if (FT_IS_SCALABLE(glyph->face)) {
                ...
                if (!glyph->metrics.horiAdvance && glyph->linearHoriAdvance)
                    glyph->metrics.horiAdvance = FT_MulFix(glyph->linearHoriAdvance, x_scale);
                if (!glyph->metrics.vertAdvance && glyph->linearVertAdvance)
                    glyph->metrics.vertAdvance = FT_MulFix(glyph->linearVertAdvance, y_scale);
            }
