...
cid_parser_new(CID_Parser* parser, FT_Stream stream, FT_Memory memory, PSAux_Service psaux)
{
    ...
    if (ft_strncmp((char*)stream->cursor, "%!PS-Adobe-3.0 Resource-CIDFont", 31)) {
       ...
       */
        ...
        for (offset = FT_STREAM_POS();; offset += 256) {
            ...
            for (p = buffer; p < limit; p++) {
                if (p[0] == 'S' && ft_strncmp((char*)p, STARTDATA, STARTDATA_LEN) == 0) {
                    /* save offset of binary data after `StartData' */
                    offset += (FT_ULong)(p - buffer) + STARTDATA_LEN + 1;
                    goto Found;
                } else if (p[1] == 's' && ft_strncmp((char*)p, SFNTS, SFNTS_LEN) == 0) {
            ...
            FT_MEM_MOVE(buffer, buffer + read_offset + read_len - STARTDATA_LEN, STARTDATA_LEN);
...
Found:
    ...
    if (FT_STREAM_SEEK(base_offset) || FT_FRAME_EXTRACT(ps_len, parser->postscript))
        goto Exit;
    ...
    while (cur <= limit - SFNTS_LEN) {
        ...
        if (cur[0] == 'S' && cur <= limit - STARTDATA_LEN && ft_strncmp((char*)cur, STARTDATA, STARTDATA_LEN) == 0) {
