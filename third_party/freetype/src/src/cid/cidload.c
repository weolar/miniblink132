...
static FT_Error cid_load_keyword(CID_Face face, CID_Loader* loader, const T1_Field keyword)
{
    ...
    if (keyword->type == T1_FIELD_TYPE_INTEGER_ARRAY || keyword->type == T1_FIELD_TYPE_FIXED_ARRAY)
        error = cid_parser_load_field_table(&loader->parser, keyword, &dummy_object);
    else
...
parse_fd_array(CID_Face face, CID_Parser* parser)
{
    ...
    if (!cid->font_dicts) {
        ...
        for (n = 0; n < cid->num_dicts; n++) {
            ...
            dict->private_dict.blue_scale = (FT_Fixed)(0.039625 * 0x10000L * 1000);
        }
    }
...
#include "cidtoken.h"
    ...
    T1_FIELD_CALLBACK("FDArray", parse_fd_array, 0) T1_FIELD_CALLBACK("FontMatrix", cid_parse_font_matrix, 0)
        T1_FIELD_CALLBACK("ExpansionFactor", parse_expansion_factor, 0) T1_FIELD_CALLBACK("FontName", parse_font_name, 0)
            ...
            if (*cur == '/' && cur + 2 < limit) {
                ...
                if (len > 0 && len < 22) {
                    ...
                    for (;;) {
                        ...
                        if (cur[0] == name[0] && len == ft_strlen((const char*)name)) {
                            FT_UInt n;
...
static FT_Error cid_read_subrs(CID_Face face)
{
    ...
    for (n = 0; n < cid->num_dicts; n++, subr++) {
        ...
        if (FT_STREAM_SEEK(cid->data_offset + dict->subrmap_offset) || FT_FRAME_ENTER((num_subrs + 1) * (FT_UInt)dict->sd_bytes))
            goto Fail;
        ...
        if (FT_NEW_ARRAY(subr->code, num_subrs + 1) || FT_ALLOC(subr->code[0], data_len))
            goto Fail;

        if (FT_STREAM_SEEK(cid->data_offset + offsets[0]) || FT_STREAM_READ(subr->code[0], data_len))
            goto Fail;
...
cid_face_open(CID_Face face, FT_Int face_index)
{
    ...
    error = cid_parser_new(parser, face->root.stream, face->root.memory, (PSAux_Service)face->psaux);
    if (error)
        goto Exit;
    ...
    if (parser->binary_length) {
        if (parser->binary_length > face->root.stream->size - parser->data_offset) {
            FT_TRACE0(("cid_face_open: adjusting length of binary data\n"
                       "               (from %d to %d bytes)\n",
                parser->binary_length, face->root.stream->size - parser->data_offset));
            parser->binary_length = face->root.stream->size - parser->data_offset;
        }

        /* we must convert the data section from hexadecimal to binary */
        if (FT_ALLOC(face->binary_data, parser->binary_length)
            || FT_SET_ERROR(cid_hex_to_binary(face->binary_data, parser->binary_length, parser->data_offset, face)))
            goto Exit;

        FT_Stream_OpenMemory(face->cid_stream, face->binary_data, parser->binary_length);
        cid->data_offset = 0;
    } else {
    ...
    for (n = 0; n < cid->num_dicts; n++) {
        ...
        if ((FT_Int)dict->num_subrs < 0 || (dict->sd_bytes && dict->num_subrs > (binary_length - dict->subrmap_offset) / (FT_UInt)dict->sd_bytes)) {
    ...
    if (entry_len && cid->cid_count > (binary_length - cid->cidmap_offset) / entry_len) {
