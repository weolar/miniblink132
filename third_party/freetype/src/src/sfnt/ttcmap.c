...
static FT_Int tt_cmap4_set_range(TT_CMap4 cmap, FT_UInt range_index)
{
    ...
    while (range_index < num_ranges) {
        ...
        /* we have to catch it                        */
        if (range_index >= num_ranges - 1 && cmap->cur_start == 0xFFFFU && cmap->cur_end == 0xFFFFU) {
            ...
            if (values) {
                ...
                do {
                    ...
                    if (gindex >= (FT_UInt)face->root.num_glyphs) {
                        ...
                        if ((FT_Int)charcode + delta < 0 && (FT_Int)end + delta >= 0)
                            charcode = (FT_UInt)(-delta);

                        else if ((FT_Int)charcode + delta < 0x10000L && (FT_Int)end + delta >= 0x10000L)
...

    {
        ...
        for (n = 0; n < num_segs; n++) {
            ...
            if (offset && offset != 0xFFFFU) {
                ...
                else if (n != num_segs - 1 || !(start == 0xFFFFU && end == 0xFFFFU)) {
                ...
                /*                                                        */
                if (valid->level >= FT_VALIDATE_PARANOID || n != num_segs - 1 || !(start == 0xFFFFU && end == 0xFFFFU))
                    FT_INVALID_DATA;
            }
...
static FT_UInt tt_cmap4_char_map_linear(TT_CMap cmap, FT_UInt32* pcharcode, FT_Bool next)
{
    ...
    /* check groups, they must be in increasing order */
    {
        ...
        for (n = 0; n < num_groups; n++) {
            ...
            if (valid->level >= FT_VALIDATE_TIGHT) {
                ...
                if (d > TT_VALID_GLYPH_COUNT(valid) || start_id >= TT_VALID_GLYPH_COUNT(valid) - d)
                    FT_INVALID_GLYPH_ID;
...
static FT_UInt tt_cmap12_char_map_binary(TT_CMap cmap, FT_UInt32* pchar_code, FT_Bool next)
{
    FT_UInt gindex = 0;
...
static FT_UInt tt_cmap13_char_map_binary(TT_CMap cmap, FT_UInt32* pchar_code, FT_Bool next)
{
    FT_UInt gindex = 0;
    FT_Byte* p = cmap->data + 12;
    FT_UInt32 num_groups = TT_PEEK_ULONG(p);
        ...
        for (n = 0; n < num_selectors; n++) {
            ...
            if (nondefOff != 0) {
                ...
                for (i = 0; i < numMappings; i++) {
                    ...
                    if (valid->level >= FT_VALIDATE_TIGHT && gid >= TT_VALID_GLYPH_COUNT(valid))
                        FT_INVALID_GLYPH_ID;
                }
            }
        }
...
static FT_UInt tt_cmap14_char_map_nondef_binary(FT_Byte* base, FT_UInt32 char_code)
{
...
FT_CALLBACK_DEF(FT_UInt)
tt_cmap14_char_var_index(TT_CMap cmap, TT_CMap ucmap, FT_UInt32 charcode, FT_UInt32 variantSelector)
{
    ...
    if (defOff != 0 && tt_cmap14_char_map_def_binary(cmap->data + defOff, charcode)) {
    ...
    if (nondefOff != 0)
        return tt_cmap14_char_map_nondef_binary(cmap->data + nondefOff, charcode);

    return 0;
}

FT_CALLBACK_DEF(FT_Int)
tt_cmap14_char_var_isdefault(TT_CMap cmap, FT_UInt32 charcode, FT_UInt32 variantSelector)
{
    FT_Byte* p = tt_cmap14_find_variant(cmap->data + 6, variantSelector);
    FT_ULong defOff;
    FT_ULong nondefOff;
    ...
    if (nondefOff != 0 && tt_cmap14_char_map_nondef_binary(cmap->data + nondefOff, charcode) != 0)
        return 0;
...
tt_cmap14_char_variants(TT_CMap cmap, FT_Memory memory, FT_UInt32 charCode)
{
    ...
    for (q = cmap14->results; count > 0; count--) {
        ...
        if ((defOff != 0 && tt_cmap14_char_map_def_binary(cmap->data + defOff, charCode))
            || (nondefOff != 0 && tt_cmap14_char_map_nondef_binary(cmap->data + nondefOff, charCode) != 0)) {
            q[0] = varSel;
            q++;
        }
    }
...
static FT_UInt32* tt_cmap14_get_def_chars(TT_CMap cmap, FT_Byte* p, FT_Memory memory)
{
    TT_CMap14 cmap14 = (TT_CMap14)cmap;
    FT_UInt32 numRanges;
    FT_UInt cnt;
...
static FT_UInt32* tt_cmap14_get_nondef_chars(TT_CMap cmap, FT_Byte* p, FT_Memory memory)
{
    TT_CMap14 cmap14 = (TT_CMap14)cmap;
...
FT_CALLBACK_DEF(FT_UInt32*)
tt_cmap14_variant_chars(TT_CMap cmap, FT_Memory memory, FT_UInt32 variantSelector)
{
    FT_Byte* p = tt_cmap14_find_variant(cmap->data + 6, variantSelector);
    FT_Int i;
    FT_ULong defOff;
...
FT_DEFINE_TT_CMAP(tt_cmap14_class_rec,
    ...
    (FT_CMap_CharVarIndexFunc)tt_cmap14_char_var_index, (FT_CMap_CharVarIsDefaultFunc)tt_cmap14_char_var_isdefault, (FT_CMap_VariantListFunc)tt_cmap14_variants,
    (FT_CMap_CharVariantListFunc)tt_cmap14_char_variants, (FT_CMap_VariantCharListFunc)tt_cmap14_variant_chars,
...
tt_cmap_unicode_init(PS_Unicodes unicodes, FT_Pointer pointer)
{
    ...
    return psnames->unicodes_init(
        memory, unicodes, face->root.num_glyphs, (PS_GetGlyphNameFunc)&tt_get_glyph_name, (PS_FreeGlyphNameFunc)NULL, (FT_Pointer)face);
}
...
FT_DEFINE_TT_CMAP(tt_cmap_unicode_class_rec,
