...
static FT_Error load_format_20(TT_Face face, FT_Stream stream, FT_ULong post_limit)
{
    ...
    /* now load the name strings */
    {
        ...
        for (n = 0; n < num_names; n++) {
            ...
            if (FT_NEW_ARRAY(name_strings[n], len + 1) || FT_STREAM_READ(name_strings[n], len))
                goto Fail1;
...
static FT_Error load_format_25(TT_Face face, FT_Stream stream, FT_ULong post_limit)
{
    FT_Memory memory = stream->memory;
    FT_Error error;
    ...
    if (num_glyphs > face->max_profile.numGlyphs || num_glyphs > 258 || num_glyphs < 1) {
        error = FT_THROW(Invalid_File_Format);
        goto Exit;
    }

    if (FT_NEW_ARRAY(offset_table, num_glyphs) || FT_STREAM_READ(offset_table, num_glyphs))
        goto Fail;
