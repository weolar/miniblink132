...
tt_face_load_sbit(TT_Face face, FT_Stream stream)
{
    ...
    case TT_SBIT_TABLE_TYPE_CBLC: {
        ...
        if (((FT_ULong)version & 0xFFFF0000UL) != 0x00020000UL && ((FT_ULong)version & 0x0000FFFFUL) != 0x00000200UL
            && ((FT_ULong)version & 0xFFFF0000UL) != 0x00030000UL && ((FT_ULong)version & 0x0000FFFFUL) != 0x00000300UL) {
...
tt_face_load_strike_metrics(TT_Face face, FT_ULong strike_index, FT_Size_Metrics* metrics)
{
    ...
    case TT_SBIT_TABLE_TYPE_CBLC: {
        ...
        metrics->x_scale = FT_MulDiv(metrics->x_ppem, 64 * 0x10000, face->header.Units_Per_EM);
        metrics->y_scale = FT_MulDiv(metrics->y_ppem, 64 * 0x10000, face->header.Units_Per_EM);
    ...
    case TT_SBIT_TABLE_TYPE_SBIX: {
        ...
        metrics->height = FT_MulDiv(hori->Ascender - hori->Descender + hori->Line_Gap, ppem_ * 64, upem);
        metrics->max_advance = FT_MulDiv(hori->advance_Width_Max, ppem_ * 64, upem);
...
static FT_Error tt_sbit_decoder_init(TT_SBitDecoder decoder, TT_Face face, FT_ULong strike_index, TT_SBit_MetricsRec* metrics)
{
    ...
    /* now find the strike corresponding to the index */
    {
        ...
        if (decoder->strike_index_array > face->sbit_table_size || decoder->strike_index_count > (face->sbit_table_size - decoder->strike_index_array) / 8)
            error = FT_THROW(Invalid_File_Format);
    }
...
static FT_Error tt_sbit_decoder_alloc_bitmap(TT_SBitDecoder decoder, FT_Bool metrics_only)
{
...
/* forward declaration */
static FT_Error tt_sbit_decoder_load_image(
    TT_SBitDecoder decoder, FT_UInt glyph_index, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count, FT_Bool metrics_only);

typedef FT_Error (*TT_SBitDecoder_LoadFunc)(TT_SBitDecoder decoder, FT_Byte* p, FT_Byte* plimit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count);

static FT_Error tt_sbit_decoder_load_byte_aligned(TT_SBitDecoder decoder, FT_Byte* p, FT_Byte* limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    FT_Byte* line;
    ...
    if (x_pos < 0 || (FT_UInt)(x_pos + width) > bit_width || y_pos < 0 || (FT_UInt)(y_pos + height) > bit_height) {
...
static FT_Error tt_sbit_decoder_load_bit_aligned(TT_SBitDecoder decoder, FT_Byte* p, FT_Byte* limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    FT_Byte* line;
    ...
    for (h = height; h > 0; h--, line += pitch) {
        ...
        if (x_pos) {
            ...
            *pwrite++ |= ((rval >> nbits) & 0xFF) & (~(0xFFU << w) << (8 - w - x_pos));
            rval <<= 8;
...
static FT_Error tt_sbit_decoder_load_compound(TT_SBitDecoder decoder, FT_Byte* p, FT_Byte* limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    ...
    for (nn = 0; nn < num_components; nn++) {
        ...
        /* NB: a recursive call */
        error = tt_sbit_decoder_load_image(decoder, gindex, x_pos + dx, y_pos + dy, recurse_count + 1,
...
static FT_Error tt_sbit_decoder_load_png(TT_SBitDecoder decoder, FT_Byte* p, FT_Byte* limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
    FT_Error error = FT_Err_Ok;
    FT_ULong png_len;
    ...
    error = Load_SBit_Png(decoder->face->root.glyph, x_pos, y_pos, decoder->bit_depth, decoder->metrics, decoder->stream->memory, p, png_len, FALSE, FALSE);
    ...
    FT_UInt recurse_count, FT_Bool metrics_only)
{
    ...
    if (FT_STREAM_SEEK(decoder->ebdt_start + glyph_start) || FT_FRAME_EXTRACT(glyph_size, data))
        goto Exit;
...
static FT_Error tt_sbit_decoder_load_image(TT_SBitDecoder decoder, FT_UInt glyph_index, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count, FT_Bool metrics_only)
{
...
FoundRange:
    ...
    case 4: /* sparse glyph array with (glyph,offset) pairs */
    {
        ...
        if (p + 4 > p_limit || num_glyphs > (FT_ULong)(((p_limit - p) >> 2) - 1))
            goto NoBitmap;
    ...
    return tt_sbit_decoder_load_bitmap(decoder, image_format, image_start, image_end, x_pos, y_pos, recurse_count, metrics_only);
...
static FT_Error tt_face_load_sbix_image(
    TT_Face face, FT_ULong strike_index, FT_UInt glyph_index, FT_Stream stream, FT_Bitmap* map, TT_SBit_MetricsRec* metrics, FT_Bool metrics_only)
{
...
retry:
    ...
    if (strike_offset >= face->ebdt_size || face->ebdt_size - strike_offset < 4 + glyph_index * 4 + 8)
        return FT_THROW(Invalid_File_Format);

    if (FT_STREAM_SEEK(face->ebdt_start + strike_offset + 4 + glyph_index * 4) || FT_FRAME_ENTER(8))
        return error;
    ...
    if (glyph_start == glyph_end)
        return FT_THROW(Missing_Bitmap);
    if (glyph_start > glyph_end || glyph_end - glyph_start < 8 || face->ebdt_size - strike_offset < glyph_end)
        return FT_THROW(Invalid_File_Format);

    if (FT_STREAM_SEEK(face->ebdt_start + strike_offset + glyph_start) || FT_FRAME_ENTER(glyph_end - glyph_start))
        return error;
...
#ifdef FT_CONFIG_OPTION_USE_PNG
        error = Load_SBit_Png(face->root.glyph, 0, 0, 32, metrics, stream->memory, stream->cursor, glyph_end - glyph_start - 8, TRUE, metrics_only);
#else
        error = FT_THROW(Unimplemented_Feature);
#endif
        break;
    ...
    if (!error) {
        ...
        metrics->horiAdvance = (FT_UShort)(aadvance * face->root.size->metrics.x_ppem / face->header.Units_Per_EM);
    }
...
tt_face_load_sbit_image(
    TT_Face face, FT_ULong strike_index, FT_UInt glyph_index, FT_UInt load_flags, FT_Stream stream, FT_Bitmap* map, TT_SBit_MetricsRec* metrics)
{
    FT_Error error = FT_Err_Ok;
    ...
    case TT_SBIT_TABLE_TYPE_CBLC: {
        ...
        if (!error) {
            error = tt_sbit_decoder_load_image(decoder, glyph_index, 0, 0, 0, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0);
            tt_sbit_decoder_done(decoder);
        }
    } break;

    case TT_SBIT_TABLE_TYPE_SBIX:
        error = tt_face_load_sbix_image(face, strike_index, glyph_index, stream, map, metrics, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0);
        break;
    ...
    if (!error && !(load_flags & FT_LOAD_COLOR) && !(load_flags & FT_LOAD_BITMAP_METRICS_ONLY) && map->pixel_mode == FT_PIXEL_MODE_BGRA) {
        FT_Bitmap new_map;
