...
tt_face_load_cpal(TT_Face face, FT_Stream stream)
{
    ...
    if (CPAL_V0_HEADER_BASE_SIZE + face->palette_data.num_palettes * 2U > table_size)
        goto InvalidTable;
    ...
    if (cpal->version == 1) {
        ...
        if (CPAL_V0_HEADER_BASE_SIZE + face->palette_data.num_palettes * 2U + 3U * 4 > table_size)
            goto InvalidTable;
        ...
        if (entry_label_offset) {
            if (entry_label_offset >= table_size)
                goto InvalidTable;
            if (face->palette_data.num_palette_entries * 2 > table_size - entry_label_offset)
                goto InvalidTable;
