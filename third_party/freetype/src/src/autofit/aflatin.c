...
af_latin_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
...

    {
       ...
       */
        ...
        while (*p) {
            ...
            glyph_index = af_shaper_get_elem(&metrics->root, shaper_buf, 0, NULL, NULL);
            if (glyph_index)
                break;
        }
         ...
         */
            ...
            af_sort_and_quantize_widths(&num_widths, axis->widths, dummy->units_per_em / 100);
    ...
    Exit:
        for (dim = 0; dim < AF_DIMENSION_MAX; dim++) {
            ...
            stdw = (axis->width_count > 0) ? axis->widths[0].org : AF_LATIN_CONSTANT(metrics, 50);
...
static void af_latin_sort_blue(FT_UInt count, AF_LatinBlue* table)
{
    ...
    for (i = 1; i < count; i++) {
        for (j = i; j > 0; j--) {
            ...
            if (table[j - 1]->flags & (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP))
                a = table[j - 1]->ref.org;
            else
...
static void af_latin_metrics_init_blues(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    for (; bs->string != AF_BLUE_STRING_MAX; bs++) {
        ...
        while (*p) {
            ...
            for (i = 0; i < num_idx; i++) {
                ...
                glyph_index = af_shaper_get_elem(&metrics->root, shaper_buf, i, NULL, &y_offset);
                if (glyph_index == 0) {
                    FT_TRACE5(("  U+%04lX unavailable\n", ch));
                    continue;
                }
                ...
                if (best_point >= 0) {
                    ...
                    if (AF_LATIN_IS_LONG_BLUE(bs)) {
                        ...
                        dist = FT_ABS(points[best_segment_last].x - points[best_segment_first].x);

                        if (dist < length_threshold && best_segment_last - best_segment_first + 2 <= best_contour_last - best_contour_first) {
                            /* heuristic threshold value */
                            ...
                            do {
                                ...
                                if (dist > 5)
                                    if (FT_ABS(points[last].x - points[first].x) <= 20 * dist) {
                                        hit = 0;
                                        continue;
                                    }
                                ...
                                if (l2r == left2right && d >= length_threshold) {
                                    ...
                                    do {
                                        ...
                                        if (d > 5)
                                            if (FT_ABS(points[next].x - points[first].x) <= 20 * dist) {
                    ...
                    if (best_on_point_first >= 0 && best_on_point_last >= 0
                        ...
                        round = FT_BOOL(FT_CURVE_TAG(outline.tags[best_segment_first]) != FT_CURVE_TAG_ON
            ...
            if (!(best_y_extremum == FT_INT_MIN || best_y_extremum == FT_INT_MAX)) {
                if (best_round)
    ...
    if (axis->blue_count) {
        ...
        for (i = 0; i < axis->blue_count - 1; i++) {
            ...
            if (blue_sorted[i]->flags & (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP)) {
            ...
            if (blue_sorted[i + 1]->flags & (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP))
            ...
            if (*a > *b) {
                *a = *b;
                FT_TRACE5(("blue zone overlap:"
                           " adjusting %s %d to %ld\n",
                    a_is_top ? "overshoot" : "reference", blue_sorted[i] - axis->blues, *a));
            }
        }
    }
...
af_latin_metrics_check_digits(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    while (*p) {
        ...
        glyph_index = af_shaper_get_elem(&metrics->root, shaper_buf, 0, &advance, NULL);
        if (!glyph_index)
            continue;
...
static void af_latin_metrics_scale_dim(AF_LatinMetrics metrics, AF_Scaler scaler, AF_Dimension dim)
{
                ...
                if (dim == AF_DIMENSION_VERT) {
                    ...
                    if (dist == 0) {
                        FT_TRACE5(("af_latin_metrics_scale_dim:"
                                   ...
                                   "\n",
                            af_style_names[metrics->root.style_class->style], scale / 65536.0, new_scale / 65536.0, (fitted - scaled) * 100 / scaled));
    ...
    FT_TRACE5(("%s widths (style `%s')\n", dim == AF_DIMENSION_HORZ ? "horizontal" : "vertical", af_style_names[metrics->root.style_class->style]));
    ...
    axis->extra_light = FT_BOOL(FT_MulFix(axis->standard_width, scale) < 32 + 8);
    ...
    if (dim == AF_DIMENSION_VERT) {
        ...
        for (nn = 0; nn < axis->blue_count; nn++) {
            ...
            for (i = 0; i < axis->blue_count; i++) {
                ...
                if (b->ref.fit <= blue->shoot.fit && b->shoot.fit >= blue->ref.fit) {
                    blue->flags &= ~AF_LATIN_BLUE_ACTIVE;
                    break;
                }
            }
        ...
        for (nn = 0; nn < axis->blue_count; nn++) {
            ...
            FT_TRACE5(("  reference %d: %d scaled to %.2f%s\n"
                ...
                nn, blue->ref.org, blue->ref.fit / 64.0, blue->flags & AF_LATIN_BLUE_ACTIVE ? "" : " (inactive)", nn, blue->shoot.org, blue->shoot.fit / 64.0,
...
af_latin_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (; contour < contour_limit; contour++) {
        ...
        for (;;) {
            ...
            if (on_edge) {
                ...
                if (point->out_dir != segment_dir || point == last) {
                    ...
                    if (!prev_segment || segment->first != prev_segment->last) {
                        ...
                        if ((min_flags | max_flags) & AF_FLAG_CONTROL && (max_on_coord - min_on_coord) < flat_threshold)
                        ...
                        if (prev_segment->last->in_dir == point->in_dir) {
                            ...
                            prev_segment->delta = (FT_Short)((max_pos - min_pos) >> 1);
                            ...
                            prev_segment->height = prev_segment->max_coord - prev_segment->min_coord;
                        } else {
                            /* we have different directions; use the properties of the */
                            /* longer segment and discard the other one                */

                            if (FT_ABS(prev_max_coord - prev_min_coord) > FT_ABS(max_coord - min_coord)) {
                                ...
                                prev_segment->last = point;
                                prev_segment->pos = (FT_Short)((prev_min_pos + prev_max_pos) >> 1);
                                prev_segment->delta = (FT_Short)((prev_max_pos - prev_min_pos) >> 1);
                            } else {
            ...
            if (!on_edge && (FT_ABS(point->out_dir) == major_dir || point == point->prev)) {
...
af_latin_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (seg = segments; seg < segment_limit; seg++) {
        ...
        if (seg->height < segment_length_threshold || seg->delta > segment_width_threshold || seg->dir == AF_DIR_NONE)
            continue;
        ...
        if (!found) {
            ...
            error = af_axis_hints_new_edge(axis, seg->pos, (AF_Direction)seg->dir, top_to_bottom_hinting, memory, &edge);
            if (error)
                goto Exit;
        ...
        FT_Pos  downs       = 0;  /* number of downwards segments       */
          ...
          if ( seg->dir == up_dir )
            ...
            downs += seg->max_coord - seg->min_coord;
                ...
                /* be ignored                                                   */
                is_serif = FT_BOOL(seg->serif && seg->serif->edge && seg->serif->edge != edge);
...
static FT_Error af_latin_hints_init(AF_GlyphHints hints, AF_LatinMetrics metrics)
{
     ...
     */
    if (mode == FT_RENDER_MODE_LIGHT || mode == FT_RENDER_MODE_LCD || (face->style_flags & FT_STYLE_FLAG_ITALIC) != 0)
...
static void af_latin_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim, AF_Edge base_edge, AF_Edge stem_edge)
{
    ...
    fitted_width = af_latin_compute_stem_width(hints, dim, dist, base_delta, base_edge->flags, stem_edge->flags);

    stem_edge->pos = base_edge->pos + fitted_width;

    FT_TRACE5(("  LINK: edge %d (opos=%.2f) linked to %.2f,"
               " dist was %.2f, now %.2f\n",
        stem_edge - hints->axis[dim].edges, stem_edge->opos / 64.0, stem_edge->pos / 64.0, dist / 64.0, fitted_width / 64.0));
}
...
static void af_latin_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    FT_TRACE5(
        ("latin %s edge hinting (style `%s')\n", dim == AF_DIMENSION_VERT ? "horizontal" : "vertical", af_style_names[hints->metrics->style_class->style]));
         ...
         */
            ...
            if (!anchor)
                FT_TRACE5(("  BLUE_ANCHOR: edge %d (opos=%.2f) snapped to %.2f,"
                    ...
                    edge1 - edges, edge1->opos / 64.0, blue->fit / 64.0, edge1->pos / 64.0, edge - edges));
            else
    ...
    for (edge = edges; edge < edge_limit; edge++) {
        ...
        if (!anchor) {
            ...
            cur_len = af_latin_compute_stem_width(hints, dim, org_len, 0, edge->flags, edge2->flags);
            ...
            FT_TRACE5(("  ANCHOR: edge %d (opos=%.2f) and %d (opos=%.2f)"
                ...
                edge - edges, edge->opos / 64.0, edge2 - edges, edge2->opos / 64.0, edge->pos / 64.0, edge2->pos / 64.0));
            ...
            if (edge2->flags & AF_EDGE_DONE) {
                FT_TRACE5(("  ADJUST: edge %d (pos=%.2f) moved to %.2f\n", edge - edges, edge->pos / 64.0, (edge2->pos - cur_len) / 64.0));
            ...
            if (edge > edges && (top_to_bottom_hinting ? (edge->pos > edge[-1].pos) : (edge->pos < edge[-1].pos))) {
        ...
        for (edge = edges; edge < edge_limit; edge++) {
            ...
            if (delta < 64 + 16) {
                ...
                FT_TRACE5(("  SERIF: edge %d (opos=%.2f) serif to %d (opos=%.2f)"
                           " aligned to %.2f\n",
                    edge - edges, edge->opos / 64.0, edge->serif - edges, edge->serif->opos / 64.0, edge->pos / 64.0));
            } else if (!anchor) {
                edge->pos = FT_PIX_ROUND(edge->opos);
                anchor = edge;
                ...
                if (before >= edges && before < edge && after < edge_limit && after > edge) {
                    if (after->opos == before->opos)
                        edge->pos = before->pos;
                    else
                        edge->pos = before->pos + FT_MulDiv(edge->opos - before->opos, after->pos - before->pos, after->opos - before->opos);
            ...
            if (edge + 1 < edge_limit && edge[1].flags & AF_EDGE_DONE && (top_to_bottom_hinting ? (edge->pos < edge[1].pos) : (edge->pos > edge[1].pos))) {
                /* don't move if stem would (almost) disappear otherwise; */
                /* the ad-hoc value 16 corresponds to 1/4px               */
                if (edge->link && FT_ABS(edge->link->pos - edge[-1].pos) > 16) {
#ifdef FT_DEBUG_LEVEL_TRACE
...
static FT_Error af_latin_hints_apply(FT_UInt glyph_index, AF_GlyphHints hints, FT_Outline* outline, AF_LatinMetrics metrics)
{
    ...
    if (AF_HINTS_DO_HORIZONTAL(hints)) {
        ...
        error = af_latin_hints_detect_features(hints, axis->width_count, axis->widths, AF_DIMENSION_HORZ);
        if (error)
            goto Exit;
    }
    ...
    if (AF_HINTS_DO_VERTICAL(hints)) {
        ...
        error = af_latin_hints_detect_features(hints, axis->width_count, axis->widths, AF_DIMENSION_VERT);
        if (error)
            goto Exit;
    ...
    for (dim = 0; dim < AF_DIMENSION_MAX; dim++) {
#ifdef AF_CONFIG_OPTION_USE_WARPER
        if (dim == AF_DIMENSION_HORZ && metrics->root.scaler.render_mode == FT_RENDER_MODE_NORMAL && AF_HINTS_DO_WARP(hints)) {
            AF_WarperRec warper;
            FT_Fixed scale;
            FT_Pos delta;

            af_warper_compute(&warper, hints, (AF_Dimension)dim, &scale, &delta);
            af_glyph_hints_scale_dim(hints, (AF_Dimension)dim, scale, delta);
            continue;
        }
#endif /* AF_CONFIG_OPTION_USE_WARPER */

        if ((dim == AF_DIMENSION_HORZ && AF_HINTS_DO_HORIZONTAL(hints)) || (dim == AF_DIMENSION_VERT && AF_HINTS_DO_VERTICAL(hints))) {
