    ...
    if (axis->num_segments < AF_SEGMENTS_EMBEDDED) {
        ...
        if (axis->segments == axis->embedded.segments) {
            if (FT_NEW_ARRAY(axis->segments, new_max))
                goto Exit;
            ft_memcpy(axis->segments, axis->embedded.segments, sizeof(axis->embedded.segments));
        } else {
...
af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, AF_Direction dir, FT_Bool top_to_bottom_hinting, FT_Memory memory, AF_Edge* anedge)
{
    ...
    if (axis->num_edges < AF_EDGES_EMBEDDED) {
        ...
        if (axis->edges == axis->embedded.edges) {
            if (FT_NEW_ARRAY(axis->edges, new_max))
                goto Exit;
            ft_memcpy(axis->edges, axis->embedded.edges, sizeof(axis->embedded.edges));
        } else {
    ...
    while (edge > edges) {
        if (top_to_bottom_hinting ? (edge[-1].fpos > fpos) : (edge[-1].fpos < fpos))
            break;
...
static int af_get_segment_index(AF_GlyphHints hints, int point_idx, int dimension)
{
...
static int af_get_edge_index(AF_GlyphHints hints, int segment_idx, int dimension)
{
    AF_AxisHints axis = &hints->axis[dimension];
...
static int af_get_strong_edge_index(AF_GlyphHints hints, AF_Edge* strong_edges, int dimension)
{
    AF_AxisHints axis = &hints->axis[dimension];
    AF_Edge edges = axis->edges;
...
void af_glyph_hints_dump_points(AF_GlyphHints hints, FT_Bool to_stdout)
{
    ...
    for (point = points; point < limit; point++) {
        ...
        AF_DUMP(("  %5d  %5s %5s  %5s %5s  %s"
                 ...
                 " %5s %5s %5s %5s\n",
            point_idx, af_print_idx(buf1, af_get_edge_index(hints, segment_idx_1, 1)), af_print_idx(buf2, segment_idx_1),
            af_print_idx(buf3, af_get_edge_index(hints, segment_idx_0, 0)), af_print_idx(buf4, segment_idx_0),
            (point->flags & AF_FLAG_NEAR)                     ? " near "
                : (point->flags & AF_FLAG_WEAK_INTERPOLATION) ? " weak "
                                                              : "strong",

            point->fx, point->fy, point->ox / 64.0, point->oy / 64.0, point->x / 64.0, point->y / 64.0,

            af_print_idx(buf5, af_get_strong_edge_index(hints, point->before, 1)), af_print_idx(buf6, af_get_strong_edge_index(hints, point->after, 1)),
            af_print_idx(buf7, af_get_strong_edge_index(hints, point->before, 0)), af_print_idx(buf8, af_get_strong_edge_index(hints, point->after, 0))));
    }
    AF_DUMP(("\n"));
}
#ifdef __cplusplus
    ...
    for (dimension = 1; dimension >= 0; dimension--) {
        ...
        for (seg = segments; seg < limit; seg++)
            AF_DUMP(("  %5d  %5d  %5d  %5s  %4d  %4d"
                     "  %4s  %5s  %4s"
                     "  %6d  %5d  %11s\n",
                AF_INDEX_NUM(seg, segments), seg->pos, seg->delta, af_dir_str((AF_Direction)seg->dir), AF_INDEX_NUM(seg->first, points),
                AF_INDEX_NUM(seg->last, points),

                af_print_idx(buf1, AF_INDEX_NUM(seg->link, segments)), af_print_idx(buf2, AF_INDEX_NUM(seg->serif, segments)),
                af_print_idx(buf3, AF_INDEX_NUM(seg->edge, edges)),

                seg->height, seg->height - (seg->max_coord - seg->min_coord), af_edge_flags_to_string(seg->flags)));
        AF_DUMP(("\n"));
    }
}
#ifdef __cplusplus
    ...
    for (dimension = 1; dimension >= 0; dimension--) {
       ...
       */
        if (dimension == AF_DIMENSION_HORZ)
            AF_DUMP(("Table of %s edges (1px=%.2fu, 10u=%.2fpx):\n", "vertical", 65536.0 * 64.0 / hints->x_scale, 10.0 * hints->x_scale / 65536.0 / 64.0));
        else
            AF_DUMP(("Table of %s edges (1px=%.2fu, 10u=%.2fpx):\n", "horizontal", 65536.0 * 64.0 / hints->y_scale, 10.0 * hints->y_scale / 65536.0 / 64.0));
        ...
        for (edge = edges; edge < limit; edge++)
            AF_DUMP(("  %5d  %7.2f  %5s  %4s  %5s"
                     "    %c   %7.2f  %7.2f  %11s\n",
                AF_INDEX_NUM(edge, edges), (int)edge->opos / 64.0, af_dir_str((AF_Direction)edge->dir), af_print_idx(buf1, AF_INDEX_NUM(edge->link, edges)),
                af_print_idx(buf2, AF_INDEX_NUM(edge->serif, edges)),

                edge->blue_edge ? 'y' : 'n', edge->opos / 64.0, edge->pos / 64.0, af_edge_flags_to_string(edge->flags)));
        AF_DUMP(("\n"));
    }
}
#ifdef __cplusplus
...

    {
         ...
         */
            ...
            for (point = points; point < point_limit; point++) {
                ...
                if (point->in_dir == AF_DIR_NONE && point->out_dir == AF_DIR_NONE) {
            ...
            for (point = points; point < point_limit; point++) {
                ...
                Is_Weak_Point:
...

                    {
                        ...
                        if (ft_corner_is_flat(point->fx - prev_v->fx, point->fy - prev_v->fy, next_u->fx - point->fx, next_u->fy - point->fy)) {
        ...
        for (point = points; point < point_limit; point++) {
...

            {
                ...
                /* point is not on an edge */
                {
                    ...
                    if (before->scale == 0)
                        before->scale = FT_DivFix(after->pos - before->pos, after->fpos - before->fpos);
    ...
    EndContour:
        ...
        else /* interpolate the last part */
        {
            if (last_touched < end_point)
                af_iup_interp(last_touched + 1, end_point, last_touched, first_touched);

            if (first_touched > points)
                af_iup_interp(first_point, first_touched - 1, last_touched, first_touched);
        }
