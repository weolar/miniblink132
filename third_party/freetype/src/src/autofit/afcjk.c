...
static void af_cjk_metrics_init_blues(AF_CJKMetrics metrics, FT_Face face)
{
    ...
    for (; bs->string != AF_BLUE_STRING_MAX; bs++) {
...
#ifdef FT_DEBUG_LEVEL_TRACE
        {
            ...
            FT_TRACE5(("blue zone %d (%s):\n", axis->blue_count, cjk_blue_name[AF_CJK_IS_HORIZ_BLUE(bs) | AF_CJK_IS_TOP_BLUE(bs)]));
        }
...
static void af_cjk_metrics_scale_dim(AF_CJKMetrics metrics, AF_Scaler scaler, AF_Dimension dim)
{
    FT_Fixed scale;
    FT_Pos delta;
    AF_CJKAxis axis;
        ...
        if (dist <= 48 && dist >= -48) {
          ...
          delta2 = 32 + ( ( ( delta2 - 32 ) + 16 ) & ~31 );
            ...
            FT_TRACE5((">> active cjk blue zone %c%d[%ld/%ld]:\n"
                       "     ref:   cur=%.2f fit=%.2f\n"
                       "     shoot: cur=%.2f fit=%.2f\n",
                (dim == AF_DIMENSION_HORZ) ? 'H' : 'V', nn, blue->ref.org, blue->shoot.org, blue->ref.cur / 64.0, blue->ref.fit / 64.0, blue->shoot.cur / 64.0,
                blue->shoot.fit / 64.0));
...
static FT_Error af_cjk_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim)
{
            ...
            if (seg2 != seg1 && seg1->dir + seg2->dir == 0) {
...

                {
                    ...
                    if (len >= len_threshold) {
                        if (dist * 8 < seg1->score * 9 && (dist * 8 < seg1->score * 7 || seg1->len < len)) {
                            seg1->score = dist;
                            seg1->len = len;
                            seg1->link = seg2;
                        }

                        if (dist * 8 < seg2->score * 9 && (dist * 8 < seg2->score * 7 || seg2->len < len)) {
                            seg2->score = dist;
                            seg2->len = len;
                            seg2->link = seg1;
                        }
...
static FT_Error af_cjk_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    ...
    for (seg = segments; seg < segment_limit; seg++) {
        ...
        if (!found) {
            ...
            error = af_axis_hints_new_edge(axis, seg->pos, (AF_Direction)seg->dir, 0, memory, &edge);
            if (error)
                goto Exit;
...
static FT_Error af_cjk_hints_detect_features(AF_GlyphHints hints, AF_Dimension dim)
{
    FT_Error error;
...
static void af_cjk_hints_compute_blue_edges(AF_GlyphHints hints, AF_CJKMetrics metrics, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    ...
    for (; edge < edge_limit; edge++) {
        ...
        for (bb = 0; bb < cjk->blue_count; bb++) {
            ...
            if (is_top_right_blue ^ is_major_dir) {
                ...
                if (FT_ABS(edge->fpos - blue->ref.org) > FT_ABS(edge->fpos - blue->shoot.org))
                    compare = &blue->shoot;
                else
                    compare = &blue->ref;
...
static FT_Pos af_cjk_compute_stem_width(AF_GlyphHints hints, AF_Dimension dim, FT_Pos width, FT_UInt base_flags, FT_UInt stem_flags)
{
...
static void af_cjk_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim, AF_Edge base_edge, AF_Edge stem_edge)
{
    FT_Pos dist = stem_edge->opos - base_edge->opos;

    FT_Pos fitted_width = af_cjk_compute_stem_width(hints, dim, dist, base_edge->flags, stem_edge->flags);

    stem_edge->pos = base_edge->pos + fitted_width;

    FT_TRACE5(("  CJKLINK: edge %d @%d (opos=%.2f) linked to %.2f,"
               " dist was %.2f, now %.2f\n",
        stem_edge - hints->axis[dim].edges, stem_edge->fpos, stem_edge->opos / 64.0, stem_edge->pos / 64.0, dist / 64.0, fitted_width / 64.0));
}

/* Shift the coordinates of the `serif' edge by the same amount */
/* as the corresponding `base' edge has been moved already.     */

static void af_cjk_align_serif_edge(AF_GlyphHints hints, AF_Edge base, AF_Edge serif)
{
    FT_UNUSED(hints);
...
static FT_Pos af_hint_normal_stem(AF_GlyphHints hints, AF_Edge edge, AF_Edge edge2, FT_Pos anchor, AF_Dimension dim)
{
    ...
    cur_len = af_cjk_compute_stem_width(hints, dim, org_len, edge->flags, edge2->flags);
    ...
    if (threshold < 64) {
        if (d_off1 >= threshold || u_off1 >= threshold || d_off2 >= threshold || u_off2 >= threshold)
            goto Exit;
    }
...
static void af_cjk_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    if (AF_HINTS_DO_BLUES(hints)) {
        for (edge = edges; edge < edge_limit; edge++) {
            ...
            FT_TRACE5(("  CJKBLUE: edge %d @%d (opos=%.2f) snapped to %.2f,"
                       " was %.2f\n",
                edge1 - edges, edge1->fpos, edge1->opos / 64.0, blue->fit / 64.0, edge1->pos / 64.0));
    ...
    for (edge = edges; edge < edge_limit; edge++) {
       ...
       */
        if (has_last_stem && (edge->pos < last_stem_pos + 64 || edge2->pos < last_stem_pos + 64)) {
            skipped++;
            continue;
        }
        ...
        if ( fixedpitch )
        {
          ...
          right->flags       |= AF_EDGE_DONE;
            ...
            delta = af_hint_normal_stem(hints, edge, edge2, 0, AF_DIMENSION_HORZ);
        } else
    ...
    if (dim == AF_DIMENSION_HORZ && (n_edges == 6 || n_edges == 12)) {
        ...
        if (edge1->link == edge1 + 1 && edge2->link == edge2 + 1 && edge3->link == edge3 + 1 && span < 8) {
        ...
        if (before >= edges || after < edge_limit) {
            if (before < edges)
                ...
                if (after->fpos == before->fpos)
                    ...
                    edge->pos = before->pos + FT_MulDiv(edge->fpos - before->fpos, after->pos - before->pos, after->fpos - before->fpos);
            }
        }
    }
...
static void af_cjk_align_edge_points(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    snapping = FT_BOOL((dim == AF_DIMENSION_HORZ && AF_LATIN_HINTS_DO_HORZ_SNAP(hints)) || (dim == AF_DIMENSION_VERT && AF_LATIN_HINTS_DO_VERT_SNAP(hints)));
...
AF_DEFINE_WRITING_SYSTEM_CLASS(af_cjk_writing_system_class,
