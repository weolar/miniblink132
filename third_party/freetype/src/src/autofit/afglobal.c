...
#undef SCRIPT
#define SCRIPT(s, S, d, h, H, ss) AF_DEFINE_SCRIPT_CLASS(af_##s##_script_class, AF_SCRIPT_##S, af_##s##_uniranges, af_##s##_nonbase_uniranges, AF_##H, ss)

#include "afscript.h"

#undef STYLE
#define STYLE(s, S, d, ws, sc, ss, c) AF_DEFINE_STYLE_CLASS(af_##s##_style_class, AF_STYLE_##S, ws, sc, ss, c)
       ...
       */
        if (style_class->coverage == AF_COVERAGE_DEFAULT) {
            ...
            for (range = script_class->script_uni_ranges; range->first != 0; range++) {
                ...
                if (gindex != 0 && gindex < (FT_ULong)globals->glyph_count && (gstyles[gindex] & AF_STYLE_MASK) == AF_STYLE_UNASSIGNED)
                ...
                for (;;) {
                    ...
                    if (gindex < (FT_ULong)globals->glyph_count && (gstyles[gindex] & AF_STYLE_MASK) == AF_STYLE_UNASSIGNED)
                        gstyles[gindex] = (FT_UShort)ss;
                }
            }

            /* do the same for the script's non-base characters */
            for (range = script_class->script_uni_nonbase_ranges; range->first != 0; range++) {
                FT_ULong charcode = range->first;
                FT_UInt gindex;

                gindex = FT_Get_Char_Index(face, charcode);

                if (gindex != 0 && gindex < (FT_ULong)globals->glyph_count && (gstyles[gindex] & AF_STYLE_MASK) == (FT_UShort)ss)
                ...
                for (;;) {
                    ...
                    if (gindex < (FT_ULong)globals->glyph_count && (gstyles[gindex] & AF_STYLE_MASK) == (FT_UShort)ss)
                        gstyles[gindex] |= AF_NONBASE;
                }
            }
        } else {
...
af_face_globals_new(FT_Face face, AF_FaceGlobals* aglobals, AF_Module module)
{
    ...
    if (FT_ALLOC(globals, sizeof(*globals) + (FT_ULong)face->num_glyphs * sizeof(FT_UShort)))
        goto Exit;
...
af_face_globals_get_metrics(AF_FaceGlobals globals, FT_UInt gindex, FT_UInt options, AF_StyleMetrics* ametrics)
{
    ...
    if (!metrics) {
        ...
        if (writing_system_class->style_metrics_init) {
            error = writing_system_class->style_metrics_init(metrics, globals->face);
            if (error) {
                if (writing_system_class->style_metrics_done)
