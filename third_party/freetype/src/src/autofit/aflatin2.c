...
af_latin2_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
...

    {
        ...
        glyph_index = FT_Get_Char_Index(face, metrics->root.style_class->standard_char);
        if (glyph_index == 0)
            goto Exit;
        ...
        for (dim = 0; dim < AF_DIMENSION_MAX; dim++) {
            ...
            error = af_latin2_hints_compute_segments(hints, (AF_Dimension)dim);
            if (error)
                goto Exit;

            af_latin2_hints_link_segments(hints, (AF_Dimension)dim);
    ...
    for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++) {
        ...
        for (; p < limit && *p; p++) {
            ...
            if (best_point >= 0) {
                ...
                round = FT_BOOL(FT_CURVE_TAG(glyph->outline.tags[start]) != FT_CURVE_TAG_ON || FT_CURVE_TAG(glyph->outline.tags[end]) != FT_CURVE_TAG_ON);
...
af_latin2_metrics_check_digits(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    for (i = 0x30; i <= 0x39; i++) {
        ...
        if (FT_Get_Advance(face, glyph_index, FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING | FT_LOAD_IGNORE_TRANSFORM, &advance))
            continue;
...
af_latin2_metrics_init(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    static const FT_Encoding latin_encodings[] = {
        FT_ENCODING_UNICODE, FT_ENCODING_APPLE_ROMAN, FT_ENCODING_ADOBE_STANDARD, FT_ENCODING_ADOBE_LATIN_1, FT_ENCODING_NONE /* end of list */
    };
        ...
        for (nn = 0; nn < axis->blue_count; nn++) {
            ...
            if (dist <= 48 && dist >= -48) {
                ...
                FT_TRACE5((">> activating blue zone %d:"
                    ...
                    nn, blue->ref.cur / 64.0, blue->ref.fit / 64.0, blue->shoot.cur / 64.0, blue->shoot.fit / 64.0));
...
FT_LOCAL_DEF(void)
af_latin2_get_standard_widths(AF_LatinMetrics metrics, FT_Pos* stdHW, FT_Pos* stdVW)
{
    if (stdHW)
...
af_latin2_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (; contour < contour_limit; contour++) {
        ...
        for (;;) {
            ...
            FT_ASSERT(FT_ABS(point->out_dir) == major_dir && point->in_dir != point->out_dir);
            first = point;
...
af_latin2_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (seg = segments; seg < segment_limit; seg++) {
        ...
        if (!found) {
            ...
            error = af_axis_hints_new_edge(axis, seg->pos, seg->dir, 0, memory, &edge);
            if (error)
                goto Exit;
...
static void af_latin2_hints_compute_blue_edges(AF_GlyphHints hints, AF_LatinMetrics metrics)
{
...
static FT_Pos af_latin2_snap_width(AF_Width widths, FT_UInt count, FT_Pos width)
{
    FT_UInt n;
    FT_Pos best = 64 + 32 + 2;
...
static FT_Pos af_latin2_compute_stem_width(AF_GlyphHints hints, AF_Dimension dim, FT_Pos width, FT_UInt base_flags, FT_UInt stem_flags)
{
...
static void af_latin2_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim, AF_Edge base_edge, AF_Edge stem_edge)
{
    ...
    FT_Pos fitted_width = af_latin2_compute_stem_width(hints, dim, dist, base_edge->flags, stem_edge->flags);
...
static void af_latin2_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    AF_Edge edges = axis->edges;
    AF_Edge edge_limit = edges + axis->num_edges;
    ...
    for (edge = edges; edge < edge_limit; edge++) {
        ...
        if (!anchor) {
            ...
            cur_len = af_latin2_compute_stem_width(hints, dim, org_len, edge->flags, edge2->flags);
            if (cur_len <= 64)
                u_off = d_off = 32;
            else {
                u_off = 38;
            ...
            anchor_drift = ((anchor->pos - anchor->opos) + (edge2->pos - edge2->opos)) >> 1;
            ...
            FT_TRACE5(("ALIGN: left=%.2f right=%.2f ", org_left / 64.0, org_right / 64.0));
          ...
          */
            ...
            */
                    ...
                    if (frac_left >= 22 && frac_left <= 42 && frac_right >= 22 && frac_right <= 42) {
                        org = frac_left;
                        fit = (org <= 32) ? 16 : 48;
        ...
        AlignStem:
            ...
            FT_TRACE5(("STEM1: %d (opos=%.2f) to %d (opos=%.2f)"
                ...
                edge - edges, edge->opos / 64.0, edge2 - edges, edge2->opos / 64.0, edge->pos / 64.0, edge2->pos / 64.0, org_len / 64.0, cur_len / 64.0));
    ...
    if (has_serifs || !anchor) {
       ...
       */
        for (edge = edges; edge < edge_limit; edge++) {
            ...
            if (edge + 1 < edge_limit && edge[1].flags & AF_EDGE_DONE && edge->pos > edge[1].pos)
                edge->pos = edge[1].pos;
        }
    }
}

static FT_Error af_latin2_hints_apply(FT_UInt glyph_index, AF_GlyphHints hints, FT_Outline* outline, AF_LatinMetrics metrics)
{
    FT_Error error;
    int dim;
