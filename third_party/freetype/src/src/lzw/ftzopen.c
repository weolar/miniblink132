...
static int ft_lzwstate_refill(FT_LzwState state)
{
    ...
    count = FT_Stream_TryRead(state->source, state->buf_tab, state->num_bits); /* WHY? */
...
static FT_Int32 ft_lzwstate_get_code(FT_LzwState state)
{
    ...
    if (state->buf_clear || offset >= state->buf_size || state->free_ent >= state->free_bits) {
        if (state->free_ent >= state->free_bits) {
            ...
            state->free_bits = state->num_bits < state->max_bits ? (FT_UInt)((1UL << num_bits) - 256) : state->max_free + 1;
        }
...
static int ft_lzwstate_prefix_grow(FT_LzwState state)
{
     ...
     *
     */
    if (FT_REALLOC_MULT(state->prefix, old_size, new_size, sizeof(FT_UShort) + sizeof(FT_Byte)))
        return -1;

    /* now adjust `suffix' and move the data accordingly */
    state->suffix = (FT_Byte*)(state->prefix + new_size);

    FT_MEM_MOVE(state->suffix, state->prefix + old_size, old_size * sizeof(FT_Byte));
...
#define FTLZW_STACK_PUSH(c)                                                                                                                                    \
    FT_BEGIN_STMNT                                                                                                                                             \
    if (state->stack_top >= state->stack_size && ft_lzwstate_stack_grow(state) < 0)                                                                            \
        goto Eof;                                                                                                                                              \
                                                                                                                                                               \
...
ft_lzwstate_io(FT_LzwState state, FT_Byte* buffer, FT_ULong out_size)
{
    ...
    case FT_LZW_PHASE_START: {
        ...
        if (FT_Stream_Seek(state->source, 2) != 0 || FT_Stream_TryRead(state->source, &max_bits, 1) != 1)
            goto Eof;
        ...
        state->free_bits = state->num_bits < state->max_bits ? (FT_UInt)((1UL << state->num_bits) - 256) : state->max_free + 1;
    ...
    case FT_LZW_PHASE_STACK: {
        ...
        if (state->free_ent < state->max_free) {
            if (state->free_ent >= state->prefix_size && ft_lzwstate_prefix_grow(state) < 0)
                goto Eof;
