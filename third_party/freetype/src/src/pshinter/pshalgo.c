...
{
    return ADD_INT(hint1->org_pos, hint1->org_len) >= hint2->org_pos && ADD_INT(hint2->org_pos, hint2->org_len) >= hint1->org_pos;
}
...
static FT_Error psh_hint_table_init(PSH_Hint_Table table, PS_Hint_Table hints, PS_Mask_Table hint_masks, PS_Mask_Table counter_masks, FT_Memory memory)
{
    ...
    /* allocate our tables */
    if (FT_NEW_ARRAY(table->sort, 2 * count) || FT_NEW_ARRAY(table->hints, count) || FT_NEW_ARRAY(table->zones, 2 * count + 1))
        goto Exit;
...
static void psh_hint_align(PSH_Hint hint, PSH_Globals globals, FT_Int dimension, PSH_Glyph glyph)
{
    ...
    if (!psh_hint_is_fitted(hint)) {
        ...
        if ((dimension == 0 && !glyph->do_horz_hints) || (dimension == 1 && !glyph->do_vert_hints)) {
            hint->cur_pos = pos;
            hint->cur_len = len;
       ...
       */
        do_snapping = (dimension == 0 && glyph->do_horz_snapping) || (dimension == 1 && glyph->do_vert_snapping);
        ...
        if (dimension == 1)
            psh_blues_snap_stem(&globals->blues, ADD_INT(hint->org_pos, hint->org_len), hint->org_pos, &align);
...
static FT_Error psh_glyph_init(PSH_Glyph glyph, FT_Outline* outline, PS_Hints ps_hints, PSH_Globals globals)
{
    ...
    if (FT_NEW_ARRAY(glyph->points, outline->n_points) || FT_NEW_ARRAY(glyph->contours, outline->n_contours))
        goto Exit;
...

    {
        ...
        for (n = 0; n < glyph->num_points; n++, point++) {
            ...
            if (point->flags & PSH_POINT_OFF)
                ...
                if (point->dir_out != PSH_DIR_NONE || psh_corner_is_flat(dxi, dyi, dxo, dyo))
                    point->flags |= PSH_POINT_SMOOTH;
            }
        }
    }
    ...
    error = psh_hint_table_init(&glyph->hint_tables[0], &ps_hints->dimension[0].hints, &ps_hints->dimension[0].masks, &ps_hints->dimension[0].counters, memory);
    if (error)
        goto Exit;

    error = psh_hint_table_init(&glyph->hint_tables[1], &ps_hints->dimension[1].hints, &ps_hints->dimension[1].masks, &ps_hints->dimension[1].counters, memory);
    if (error)
        goto Exit;
        ...
        if (point_dir) {
            ...
            if (!point->hint) {
                for (nn = 0; nn < num_hints; nn++) {
                    ...
                    if (org_u >= hint->org_pos && org_u <= ADD_INT(hint->org_pos, hint->org_len)) {
                        point->hint = hint;
                        break;
                    }
                }
    ...
    if (num_masks > 1 && glyph->num_points > 0) {
        ...
        for (; num_masks > 1; num_masks--, mask++) {
            ...
            if (next > first) {
                ...
                psh_hint_table_find_strong_points(table, point, count, threshold, major_dir);
            }
            first = next;
        }
    }
...
static void psh_glyph_find_blue_points(PSH_Blues blues, PSH_Glyph glyph)
{
    ...
    for (; glyph_count > 0; glyph_count--, point++) {
        ...
        /* check tangents */
        if (!PSH_DIR_COMPARE(point->dir_in, PSH_DIR_HORIZONTAL) && !PSH_DIR_COMPARE(point->dir_out, PSH_DIR_HORIZONTAL))
            continue;
        ...
        if (hint) {
            ...
            if (psh_point_is_edge_min(point))
                ...
                if (delta <= 0)
                    ...
                    point->cur_u = hint->cur_pos + hint->cur_len + FT_MulFix(delta - hint->org_len, scale);

                else /* hint->org_len > 0 */
                    point->cur_u = hint->cur_pos + FT_MulDiv(delta, hint->cur_len, hint->org_len);
            }
            psh_point_set_fitted(point);
        }
    }
