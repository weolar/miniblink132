...
static FT_Error ftc_scaler_lookup_size(FTC_Manager manager, FTC_Scaler scaler, FT_Size* asize)
{
    ...
    if (scaler->pixel)
        ...
        error = FT_Set_Char_Size(face, (FT_F26Dot6)scaler->width, (FT_F26Dot6)scaler->height, scaler->x_res, scaler->y_res);
    if (error) {
        FT_Done_Size(size);
        size = NULL;
    }
...
FTC_Manager_LookupSize(FTC_Manager manager, FTC_Scaler scaler, FT_Size* asize)
{
    ...
    *asize = NULL;
    ...
    FTC_MRULIST_LOOKUP_CMP(&manager->sizes, scaler, ftc_size_node_compare, mrunode, error);
...
ftc_face_node_init(FTC_MruNode ftcnode, FT_Pointer ftcface_id, FT_Pointer ftcmanager)
{
    ...
    error = manager->request_face(face_id, manager->library, manager->request_data, &node->face);
    if (!error) {
...
ftc_face_node_done(FTC_MruNode ftcnode, FT_Pointer ftcmanager)
{
    ...
    FTC_MruList_RemoveSelection(&manager->sizes, ftc_size_node_compare_faceid, node->face_id);
...
FTC_Manager_LookupFace(FTC_Manager manager, FTC_FaceID face_id, FT_Face* aface)
{
    ...
    *aface = NULL;
    ...
    FTC_MRULIST_LOOKUP_CMP(&manager->faces, face_id, ftc_face_node_compare, mrunode, error);
...
FT_EXPORT_DEF(FT_Error)
FTC_Manager_New(
    FT_Library library, FT_UInt max_faces, FT_UInt max_sizes, FT_ULong max_bytes, FTC_Face_Requester requester, FT_Pointer req_data, FTC_Manager* amanager)
{
    FT_Error error;
    FT_Memory memory;
    ...
    FTC_MruList_Init(&manager->faces, &ftc_face_list_class, max_faces, manager, memory);

    FTC_MruList_Init(&manager->sizes, &ftc_size_list_class, max_sizes, manager, memory);
