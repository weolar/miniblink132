...
#define FTC_CMAP_HASH(faceid, index, charcode) (FTC_FACE_ID_HASH(faceid) + 211 * (index) + ((charcode) / FTC_CMAP_INDICES_MAX))
...
ftc_cmap_node_new(FTC_Node* ftcanode, FT_Pointer ftcquery, FTC_Cache cache)
{
    ...
    if (!FT_NEW(node)) {
        ...
        node->first = (query->char_code / FTC_CMAP_INDICES_MAX) * FTC_CMAP_INDICES_MAX;
...
static const FTC_CacheClassRec ftc_cmap_cache_class = {
    ...
    sizeof(FTC_CacheRec), ftc_cache_init, /* FTC_Cache_InitFunc    cache_init         */
