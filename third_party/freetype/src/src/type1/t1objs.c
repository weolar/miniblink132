    ...
    FT_Size_Request req)
{
    ...
    if (funcs)
        funcs->set_scale((PSH_Globals)t1size->internal->module_data, size->root.metrics.x_scale, size->root.metrics.y_scale, 0, 0);
    ...
    FT_Int face_index, FT_Int num_params, FT_Parameter* params)
{
    ...
    /* set up root face fields */
    {
        ...
        root->face_flags |= FT_FACE_FLAG_SCALABLE | FT_FACE_FLAG_HORIZONTAL | FT_FACE_FLAG_GLYPH_NAMES | FT_FACE_FLAG_HINTER;
        ...
        if (info->weight) {
            if (!ft_strcmp(info->weight, "Bold") || !ft_strcmp(info->weight, "Black"))
