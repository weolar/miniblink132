...
static FT_Long t1_ps_get_font_value(FT_Face face, PS_Dict_Keys key, FT_UInt idx, void* value, FT_Long value_len_)
{
    ...
    case PS_DICT_FONT_MATRIX:
        if (idx < sizeof(type1->font_matrix) / sizeof(type1->font_matrix.xx)) {
            FT_Fixed val = 0;
    ...
    case PS_DICT_CHAR_STRING:
        if (idx < (FT_UInt)type1->num_glyphs) {
            ...
            if (value && value_len >= retval) {
                ft_memcpy(value, (void*)(type1->charstrings[idx]), retval - 1);
                ((FT_Char*)value)[retval - 1] = (FT_Char)'\0';
            }
        }
        break;
    ...
    case PS_DICT_ENCODING_ENTRY:
        if (type1->encoding_type == T1_ENCODING_TYPE_ARRAY && idx < (FT_UInt)type1->encoding.num_chars) {
            retval = ft_strlen(type1->encoding.char_name[idx]) + 1;
            if (value && value_len >= retval) {
                ft_memcpy(value, (void*)(type1->encoding.char_name[idx]), retval - 1);
                ((FT_Char*)value)[retval - 1] = (FT_Char)'\0';
            }
        }
        break;
    ...
    case PS_DICT_SUBR: {
        ...
        if (type1->subrs_hash) {
            ...
            size_t* val = ft_hash_num_lookup((FT_Int)idx, type1->subrs_hash);
    ...
    case PS_DICT_MIN_FEATURE:
        if (idx < sizeof(type1->private_dict.min_feature) / sizeof(type1->private_dict.min_feature[0])) {
...
static const FT_ServiceDescRec t1_services[] = { { FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, &t1_service_ps_name },
    { FT_SERVICE_ID_GLYPH_DICT, &t1_service_glyph_dict }, { FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_TYPE_1 },
    { FT_SERVICE_ID_POSTSCRIPT_INFO, &t1_service_ps_info }, { FT_SERVICE_ID_PROPERTIES, &t1_service_properties },
    ...
    FT_UInt left_glyph, FT_UInt right_glyph, FT_Vector* kerning)
{
    ...
    if (face->afm_data)
        T1_Get_Kerning((AFM_FontInfo)face->afm_data, left_glyph, right_glyph, kerning);
...
const FT_Driver_ClassRec t1_driver_class = {
    ...
    sizeof(T1_FaceRec), sizeof(T1_SizeRec), sizeof(T1_GlyphSlotRec),
