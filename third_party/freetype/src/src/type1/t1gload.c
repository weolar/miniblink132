...
static FT_Error T1_Parse_Glyph_And_Get_Char_String(T1_Decoder decoder, FT_UInt glyph_index, FT_Data* char_string, FT_Bool* force_scaling)
{
    ...
    FT_Incremental_InterfaceRec* inc = face->root.internal->incremental_interface;
#endif
    ...
    /* callback function.                                     */
    if (inc)
        error = inc->funcs->get_glyph_data(inc->object, glyph_index, char_string);
    else
...
#ifdef T1_CONFIG_OPTION_OLD_ENGINE
        if (driver->hinting_engine == FT_HINTING_FREETYPE || decoder->builder.metrics_only)
            error = decoder_funcs->parse_charstrings_old(decoder, (FT_Byte*)char_string->pointer, (FT_UInt)char_string->length);
#else
        if (decoder->builder.metrics_only)
            error = decoder_funcs->parse_metrics(decoder, (FT_Byte*)char_string->pointer, (FT_UInt)char_string->length);
#endif
        else {
            ...
            error = decoder_funcs->parse_charstrings(&psdecoder, (FT_Byte*)char_string->pointer, (FT_ULong)char_string->length);
...
T1_Parse_Glyph(T1_Decoder decoder, FT_UInt glyph_index)
{
    ...
    FT_Error error = T1_Parse_Glyph_And_Get_Char_String(decoder, glyph_index, &glyph_data, &force_scaling);
...
T1_Compute_Max_Advance(T1_Face face, FT_Pos* max_advance)
{
    ...
    *max_advance = 0;
    ...
    error = psaux->t1_decoder_funcs->init(&decoder, (FT_Face)face, 0, /* size       */
        0, /* glyph slot */
        (FT_Byte**)type1->glyph_names, face->blend, 0, FT_RENDER_MODE_NORMAL, T1_Parse_Glyph);
    if (error)
        return error;
...
T1_Get_Advances(FT_Face t1face, /* T1_Face */
    FT_UInt first, FT_UInt count, FT_Int32 load_flags, FT_Fixed* advances)
{
    ...
    for (nn = 0; nn < count; nn++) {
        ...
        FT_TRACE5(("  idx %d: advance width %d font unit%s\n", first + nn, advances[nn], advances[nn] == 1 ? "" : "s"));
    }
    ...
    FT_UInt glyph_index, FT_Int32 load_flags)
{
...
#ifdef FT_CONFIG_OPTION_INCREMENTAL
    if (glyph_index >= (FT_UInt)face->root.num_glyphs && !face->root.internal->incremental_interface)
#else
    ...
    hinting = FT_BOOL(!(load_flags & FT_LOAD_NO_SCALE) && !(load_flags & FT_LOAD_NO_HINTING));
    ...
    error = decoder_funcs->init(
        &decoder, t1glyph->face, t1size, t1glyph, (FT_Byte**)type1->glyph_names, face->blend, hinting, FT_LOAD_TARGET_MODE(load_flags), T1_Parse_Glyph);
    if (error)
        goto Exit;
    ...
    error = T1_Parse_Glyph_And_Get_Char_String(&decoder, glyph_index, &glyph_data, &force_scaling);
    if (error)
        goto Exit;
#ifdef FT_CONFIG_OPTION_INCREMENTAL
    glyph_data_loaded = 1;
    ...
    if (!error) {
        ...
        if (load_flags & FT_LOAD_NO_RECURSE) {
            ...
            t1glyph->metrics.horiBearingX = FIXED_TO_INT(decoder.builder.left_bearing.x);
            t1glyph->metrics.horiAdvance = FIXED_TO_INT(decoder.builder.advance.x);
            ...
            t1glyph->linearHoriAdvance = FIXED_TO_INT(decoder.builder.advance.x);
            ...
            if (font_matrix.xx != 0x10000L || font_matrix.yy != 0x10000L || font_matrix.xy != 0 || font_matrix.yx != 0) {
                FT_Outline_Transform(&t1glyph->outline, &font_matrix);

                metrics->horiAdvance = FT_MulFix(metrics->horiAdvance, font_matrix.xx);
                metrics->vertAdvance = FT_MulFix(metrics->vertAdvance, font_matrix.yy);
            }

            if (font_offset.x || font_offset.y) {
                FT_Outline_Translate(&t1glyph->outline, font_offset.x, font_offset.y);
