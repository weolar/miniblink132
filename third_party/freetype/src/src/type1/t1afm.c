...
static FT_Error T1_Read_PFM(FT_Face t1_face, FT_Stream stream, AFM_FontInfo fi)
{
    ...
    ft_qsort(fi->KernPairs, fi->NumKernPair, sizeof(AFM_KernPairRec), compare_kern_pairs);
...
T1_Read_Metrics(FT_Face t1_face, FT_Stream stream)
{
    ...
    if (psaux->afm_parser_funcs) {
        error = psaux->afm_parser_funcs->init(&parser, stream->memory, stream->cursor, stream->limit);
    ...
    if (FT_ERR_EQ(error, Unknown_File_Format)) {
        ...
        if (stream->size > 6 && start[1] < 4 && FT_PEEK_ULONG_LE(start + 2) == stream->size)
...
T1_Get_Track_Kerning(FT_Face face, FT_Fixed ptsize, FT_Int degree, FT_Fixed* kerning)
{
    ...
    for (i = 0; i < fi->NumTrackKern; i++) {
        ...
        if (ptsize < tk->min_ptsize)
            ...
            *kerning = FT_MulDiv(ptsize - tk->min_ptsize, tk->max_kern - tk->min_kern, tk->max_ptsize - tk->min_ptsize) + tk->min_kern;
        }
    }
