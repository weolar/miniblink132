...
static void test_unit(void)
{
    ...
    for (i = 0; i < FT_ANGLE_2PI; i += 0x10000L) {
        ...
        if (abs(v.x - c2) > THRESHOLD || abs(v.y - s2) > THRESHOLD) {
            error = 1;
            printf("FT_Vector_Unit[%3d] = ( %.7f, %.7f )  vec = ( %.7f, %.7f )\n", (i >> 16), v.x / 65536.0, v.y / 65536.0, c1, s1);
        }
    }
}
...
static void test_rotate(void)
{
    ...
    for (rotate = 0; rotate < FT_ANGLE_2PI; rotate += 0x10000L) {
        ...
        for (i = 0; i < FT_ANGLE_2PI; i += 0x10000L) {
            ...
            if (abs(c4 - v.x) > THRESHOLD || abs(s4 - v.y) > THRESHOLD) {
                error = 1;
                printf("FT_Rotate( (%.7f,%.7f), %.5f ) = ( %.7f, %.7f ), rot = ( %.7f, %.7f )\n", c1, s1, ra, c2 / 65536.0, s2 / 65536.0, c4 / 65536.0,
                    s4 / 65536.0);
            }
        }
    }
