...
static FT_Error ft_bzip2_check_header(FT_Stream stream)
{
    ...
    if (head[0] != 0x42 || head[1] != 0x5A || head[2] != 0x68) /* only support bzip2 (huffman) */
    {
...
static FT_Error ft_bzip2_file_init(FT_BZip2File zip, FT_Stream stream, FT_Stream source)
{
