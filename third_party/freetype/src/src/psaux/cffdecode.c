...
static const FT_Byte cff_argument_counts[] = {
    ...
    1 | CFF_COUNT_CHECK_WIDTH | CFF_COUNT_EXACT, 1 | CFF_COUNT_CHECK_WIDTH | CFF_COUNT_EXACT,

    0 | CFF_COUNT_CLEAR_STACK, /* rlineto */
    0 | CFF_COUNT_CLEAR_STACK, 0 | CFF_COUNT_CLEAR_STACK,

    0 | CFF_COUNT_CLEAR_STACK, /* rrcurveto */
    0 | CFF_COUNT_CLEAR_STACK, 0 | CFF_COUNT_CLEAR_STACK, 0 | CFF_COUNT_CLEAR_STACK, 0 | CFF_COUNT_CLEAR_STACK, 0 | CFF_COUNT_CLEAR_STACK,
    ...
    2 | CFF_COUNT_CHECK_WIDTH, 2 | CFF_COUNT_CHECK_WIDTH, 2 | CFF_COUNT_CHECK_WIDTH,
...
static FT_Error cff_operator_seac(CFF_Decoder* decoder, FT_Pos asb, FT_Pos adx, FT_Pos ady, FT_Int bchar, FT_Int achar)
{
    ...
    error = decoder->get_glyph_callback(face, (FT_UInt)bchar_index, &charstring, &charstring_len);
    if (!error) {
        /* the seac operator must not be nested */
        decoder->seac = TRUE;
        error = cff_decoder_parse_charstrings(decoder, charstring, charstring_len, 0);
        decoder->seac = FALSE;
    ...
    error = decoder->get_glyph_callback(face, (FT_UInt)achar_index, &charstring, &charstring_len);
    if (!error) {
        /* the seac operator must not be nested */
        decoder->seac = TRUE;
        error = cff_decoder_parse_charstrings(decoder, charstring, charstring_len, 0);
       ...
       */
        ...
        if (v >= 32 || v == 28) {
            ...
            if (v == 28) {
                ...
                if (ip + 3 >= limit)
                    goto Syntax_Error;
                val = (FT_Int32)(((FT_UInt32)ip[0] << 24) | ((FT_UInt32)ip[1] << 16) | ((FT_UInt32)ip[2] << 8) | (FT_UInt32)ip[3]);
                ip += 4;
            ...
            *decoder->top++ = val;
            ...
            if (req_args & CFF_COUNT_CHECK_WIDTH) {
                if (num_args > 0 && decoder->read_width) {
                    ...
                    case cff_op_endchar:
                        ...
                        set_width_ok = in_dict ? 0 : ((num_args == 5) || (num_args == 1));
                        break;
            ...
            case cff_op_vstemhm:
                ...
                FT_TRACE4(("%s\n", op == cff_op_hstem ? " hstem" : (op == cff_op_vstem ? " vstem" : (op == cff_op_hstemhm ? " hstemhm" : " vstemhm"))));

                if (hinter)
                    hinter->stems(hinter->hints, (op == cff_op_hstem || op == cff_op_hstemhm), num_args / 2, args - (num_args & ~1));
            ...
            case cff_op_hintmask:
            case cff_op_cntrmask:
                FT_TRACE4(("%s", op == cff_op_hintmask ? " hintmask" : " cntrmask"));
                ...
                if (num_args > 0) {
                    if (hinter)
                        hinter->stems(hinter->hints, 0, num_args / 2, args - (num_args & ~1));
                ...
                if (hinter) {
                    if (op == cff_op_hintmask)
                        hinter->hintmask(hinter->hints, (FT_UInt)builder->current->n_points, (FT_UInt)decoder->num_hints, ip);
                    else
                        hinter->counter(hinter->hints, (FT_UInt)decoder->num_hints, ip);
                }
            ...
            case cff_op_rlineto:
                ...
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, num_args / 2))
                    goto Fail;
            ...
            case cff_op_vlineto: {
                ...
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, num_args))
                    goto Fail;
            ...
            case cff_op_rrcurveto: {
                ...
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, nargs / 2))
                    goto Fail;
            ...
            case cff_op_hvcurveto: {
                ...
                FT_TRACE4(("%s\n", op == cff_op_vhcurveto ? " vhcurveto" : " hvcurveto"));
            ...
            case cff_op_rlinecurve: {
                ...
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, num_lines + 3))
                    goto Fail;
            ...
            case cff_op_rcurveline: {
                ...
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, num_curves * 3 + 2))
                    goto Fail;
            ...
            case cff_op_hflex1: {
                ...
                /* needs to be added                                           */
                if (cff_builder_start_point(builder, x, y) || cff_check_points(builder, 6))
                    goto Fail;
            ...
            case cff_op_flex: {
                ...
                for (count = 6; count > 0; count--) {
                    x = ADD_LONG(x, args[0]);
                    y = ADD_LONG(y, args[1]);
                    cff_builder_add_point(builder, x, y, FT_BOOL(count == 4 || count == 1));
                    args += 2;
                }
            ...
            case cff_op_seac:
                ...
                error = cff_operator_seac(decoder, args[0], args[1], args[2], (FT_Int)(args[3] >> 16), (FT_Int)(args[4] >> 16));
            ...
            case cff_op_endchar:
                ...
                if (num_args >= 4) {
                    ...
                    error = cff_operator_seac(decoder, 0L, args[-4], args[-3], (FT_Int)(args[-2] >> 16), (FT_Int)(args[-1] >> 16));
                    ...
                    if (hinter) {
                        if (hinter->close(hinter->hints, (FT_UInt)builder->current->n_points))
                            goto Syntax_Error;

                        /* apply hints to the loaded glyph outline now */
                        error = hinter->apply(hinter->hints, builder->current, (PSH_Globals)builder->hints_globals, decoder->hint_mode);
                        if (error)
                            goto Fail;
                    }
            ...
            case cff_op_random: {
                FT_UInt32* randval = in_dict ? &decoder->cff->top_font.random : &decoder->current_subfont->random;
            ...
            case cff_op_load:
                ...
                /* in version 16-March-2000                               */
                {
                    ...
                    if (reg_idx >= 0 && reg_idx <= 2 && idx >= 0 && idx < CFF_MAX_TRANS_ELEMENTS && count >= 0 && count <= num_axes) {
                        FT_Int end, i;
            ...
            case cff_op_blend:
                ...
                if (num_designs) {
                    ...
                    if (num_results > num_args || num_results * (FT_Int)num_designs > num_args)
                        goto Stack_Underflow;
            ...
            case cff_op_hsbw:
                ...
                decoder->glyph_width = ADD_LONG(decoder->nominal_width, (args[1] >> 16));
            ...
            case cff_op_sbw:
                ...
                decoder->glyph_width = ADD_LONG(decoder->nominal_width, (args[2] >> 16));
...
cff_decoder_init(CFF_Decoder* decoder, TT_Face face, CFF_Size size, CFF_GlyphSlot slot, FT_Bool hinting, FT_Render_Mode hint_mode,
    CFF_Decoder_Get_Glyph_Callback get_callback, CFF_Decoder_Free_Glyph_Callback free_callback)
{
    CFF_Font cff = (CFF_Font)face->extra.data;
