...
t1_lookup_glyph_by_stdcharcode_ps(PS_Decoder* decoder, FT_Int charcode)
{
    ...
    glyph_name = psnames->adobe_std_strings(psnames->adobe_std_encoding[charcode]);
...
t1_decoder_parse_charstrings(T1_Decoder decoder, FT_Byte* charstring_base, FT_UInt charstring_len)
{
    ...
    seed = (FT_Fixed)(((FT_Offset)(char*)&seed ^ (FT_Offset)(char*)&decoder ^ (FT_Offset)(char*)&charstring_base) & FT_ULONG_MAX);
    ...
    while (ip < limit) {
        ...
        FT_ASSERT(known_othersubr_result_cnt == 0 || unknown_othersubr_result_cnt == 0);
       ...
       */
        ...
        case 255: /* four bytes integer */
            ...
            value = (FT_Int32)(((FT_UInt32)ip[0] << 24) | ((FT_UInt32)ip[1] << 16) | ((FT_UInt32)ip[2] << 8) | (FT_UInt32)ip[3]);
            ip += 4;
       ...
       */
         ...
         */
            ...
            case 1: /* start flex feature */
                ...
                if (FT_SET_ERROR(t1_builder_start_point(builder, x, y)) || FT_SET_ERROR(t1_builder_check_points(builder, 6)))
                    goto Fail;
            ...
            case 2: /* add flex vectors */
            {
                ...
                if (idx > 0 && idx < 7) {
                    ...
                    t1_builder_add_point(builder, x, y, (FT_Byte)(idx == 3 || idx == 6));
                }
            } break;
            ...
            case 3: /* change hints */
                ...
                if (hinter)
                    hinter->reset(hinter->hints, (FT_UInt)builder->current->n_points);
                break;
            ...
            case 18: /* multiple masters */
            {
                ...
                for (nn = 0; nn < num_points; nn++) {
                    ...
                    for (mm = 1; mm < blend->num_designs; mm++)
                        tmp = ADD_LONG(tmp, FT_MulFix(*delta++, blend->weight_vector[mm]));
        ...
        } else /* general operator */
        {
            ...
            case op_hsbw:
                ...
                builder->left_bearing.x = ADD_LONG(builder->left_bearing.x, top[0]);
            ...
            case op_seac:
                return t1operator_seac(decoder, top[0], top[1], top[2], Fix2Int(top[3]), Fix2Int(top[4]));
            ...
            case op_sbw:
                ...
                builder->left_bearing.x = ADD_LONG(builder->left_bearing.x, top[0]);
                builder->left_bearing.y = ADD_LONG(builder->left_bearing.y, top[1]);
            ...
            case op_closepath:
                ...
                if (builder->parse_state == T1_Parse_Have_Path || builder->parse_state == T1_Parse_Have_Moveto)
                    t1_builder_close_contour(builder);
            ...
            case op_hvcurveto:
                ...
                if (FT_SET_ERROR(t1_builder_start_point(builder, x, y)) || FT_SET_ERROR(t1_builder_check_points(builder, 3)))
                    goto Fail;
            ...
            case op_callsubr: {
                ...
                if (decoder->subrs_hash) {
                    size_t* val = ft_hash_num_lookup(idx, decoder->subrs_hash);
...
FT_LOCAL_DEF(FT_Error)
t1_decoder_init(T1_Decoder decoder, FT_Face face, FT_Size size, FT_GlyphSlot slot, FT_Byte** glyph_names, PS_Blend blend, FT_Bool hinting,
    FT_Render_Mode hint_mode, T1_Decoder_Callback parse_callback)
{
    FT_ZERO(decoder);
