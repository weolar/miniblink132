...
cf2_decoder_parse_charstrings(PS_Decoder* decoder, FT_Byte* charstring_base, FT_ULong charstring_len)
{
    ...
    if (!decoder->cf2_instance->data) {
        decoder->cf2_instance->finalizer = (FT_Generic_Finalizer)cf2_free_instance;
...

    {
        ...
        FT_Char no_stem_darkening_font = builder->face->internal->no_stem_darkening;
        ...
        cf2_getScaleAndHintFlag(decoder, &transform.a, &transform.d, &hinted, &scaled);
        ...
        if (scaled && (!no_stem_darkening_font || (no_stem_darkening_font < 0 && !no_stem_darkening_driver)))
...
cf2_getStdVW(PS_Decoder* decoder)
{
    ...
    return cf2_intToFixed(decoder->current_subfont->private_dict.standard_height);
}
...
cf2_getStdHW(PS_Decoder* decoder)
{
    ...
    return cf2_intToFixed(decoder->current_subfont->private_dict.standard_width);
}
...
cf2_getBlueMetrics(PS_Decoder* decoder, CF2_Fixed* blueScale, CF2_Fixed* blueShift, CF2_Fixed* blueFuzz)
{
    ...
    *blueScale = FT_DivFix(decoder->current_subfont->private_dict.blue_scale, cf2_intToFixed(1000));
    *blueShift = cf2_intToFixed(decoder->current_subfont->private_dict.blue_shift);
    *blueFuzz = cf2_intToFixed(decoder->current_subfont->private_dict.blue_fuzz);
}
...
cf2_getSeacComponent(PS_Decoder* decoder, CF2_Int code, CF2_Buffer buf)
{
    ...
    error = decoder->get_glyph_callback((TT_Face)decoder->builder.face, (CF2_UInt)gid, &charstring, &len);
...
cf2_freeSeacComponent(PS_Decoder* decoder, CF2_Buffer buf)
{
    ...
    decoder->free_glyph_callback((TT_Face)decoder->builder.face, (FT_Byte**)&buf->start, (FT_ULong)(buf->end - buf->start));
}
