...
static void cf2_computeDarkening(
    CF2_Fixed emRatio, CF2_Fixed ppem, CF2_Fixed stemWidth, CF2_Fixed* darkenAmount, CF2_Fixed boldenAmount, FT_Bool stemDarkened, FT_Int* darkenParams)
{
    /*
    ...
    *darkenAmount = 0;
    ...
    if (stemDarkened) {
        ...
        logBase2 = FT_MSB((FT_UInt32)stemWidthPer1000) + FT_MSB((FT_UInt32)ppem);
        ...
        if (scaledStem < cf2_intToFixed(x1))
            ...
            *darkenAmount = FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(cf2_intToFixed(y1), ppem);
        }
        ...
        Try_x3 : {
            ...
            *darkenAmount = FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(cf2_intToFixed(y2), ppem);
        }
        }
        ...
        Try_x4 : {
            ...
            *darkenAmount = FT_MulDiv(x, ydelta, xdelta) + FT_DivFix(cf2_intToFixed(y3), ppem);
        }
        }
...
static void cf2_font_setup(CF2_Font font, const CF2_Matrix* transform)
{
    ...
    if (!font->isT1) {
        ...
        if (hasVariations) {
            ...
            font->error = cf2_getNormalizedVector(decoder, &lenNormalizedV, &normalizedV);
            if (font->error)
                return;

            if (cffload->blend_check_vector(&subFont->blend, subFont->private_dict.vsindex, lenNormalizedV, normalizedV)) {
    ...
    if (ft_memcmp(transform, &font->currentTransform, 4 * sizeof(CF2_Fixed)) != 0) {
        ...
        font->currentTransform.tx = font->currentTransform.ty = cf2_intToFixed(0);
    ...
    if (needExtraSetup) {
        ...
        ppem = FT_MAX(cf2_intToFixed(4), font->ppem); /* use minimum ppem of 4 */
      ...
      emRatio = cf2_fixedFracMul( cf2_intToFixed( 1000 ), fontMatrix->a );
        ...
        if (boldenX > 0) {
            ...
            boldenX = FT_MAX(boldenX, FT_DivFix(cf2_intToFixed(unitsPerEm), ppem));
            ...
            /* synthetic bold font.                                          */
            cf2_computeDarkening(emRatio, ppem, font->stdVW, &font->darkenX, boldenX, FALSE, font->darkenParams);
        } else
            cf2_computeDarkening(emRatio, ppem, font->stdVW, &font->darkenX, 0, font->stemDarkened, font->darkenParams);
      ...
      emRatio = cf2_fixedFracMul( cf2_intToFixed( 1000 ), fontMatrix->d );
        ...
        cf2_computeDarkening(emRatio, ppem, font->stdHW, &font->darkenY, boldenY, font->stemDarkened, font->darkenParams);
...
cf2_getGlyphOutline(CF2_Font font, CF2_Buffer charstring, const CF2_Matrix* transform, CF2_F16Dot16* glyphWidth)
{
    ...
    while (1) {
        ...
        cf2_interpT2CharString(font, charstring, (CF2_OutlineCallbacks)&font->outline, &translation, FALSE, 0, 0, &advWidth);
