...
cf2_blues_init(CF2_Blues blues, CF2_Font font)
{
    ...
    cf2_getBlueMetrics(decoder, &blues->blueScale, &blues->blueShift, &blues->blueFuzz);
    ...
    if (cf2_getLanguageGroup(decoder) == 1
        && (numBlueValues == 0
            || (numBlueValues == 4 && cf2_blueToFixed(blueValues[0]) < emBoxBottom && cf2_blueToFixed(blueValues[1]) < emBoxBottom
                && cf2_blueToFixed(blueValues[2]) > emBoxTop && cf2_blueToFixed(blueValues[3]) > emBoxTop))) {
        /*
       ...
       */
        ...
        blues->emBoxBottomEdge.dsCoord = cf2_fixedRound(FT_MulFix(blues->emBoxBottomEdge.csCoord, blues->scale)) - CF2_MIN_COUNTER;
        blues->emBoxBottomEdge.scale = blues->scale;
        blues->emBoxBottomEdge.flags = CF2_GhostBottom | CF2_Locked | CF2_Synthetic;

        blues->emBoxTopEdge.csCoord = emBoxTop + CF2_FIXED_EPSILON + 2 * font->darkenY;
        blues->emBoxTopEdge.dsCoord = cf2_fixedRound(FT_MulFix(blues->emBoxTopEdge.csCoord, blues->scale)) + CF2_MIN_COUNTER;
    ...
    for (i = 0; i < numBlueValues; i += 2) {
        ...
        blues->zone[blues->count].csTopEdge = cf2_blueToFixed(blueValues[i + 1]);

        zoneHeight = SUB_INT32(blues->zone[blues->count].csTopEdge, blues->zone[blues->count].csBottomEdge);
        ...
        if (i == 0) {
            ...
            blues->zone[blues->count].csFlatEdge = blues->zone[blues->count].csTopEdge;
        } else {
            blues->zone[blues->count].bottomZone = FALSE;
            blues->zone[blues->count].csFlatEdge = blues->zone[blues->count].csBottomEdge;
        }
    ...
    for (i = 0; i < numOtherBlues; i += 2) {
        ...
        blues->zone[blues->count].csTopEdge = cf2_blueToFixed(otherBlues[i + 1]);
    ...
    if (blues->scale < blues->blueScale) {
        ...
        blues->boost = cf2_doubleToFixed(.6) - FT_MulDiv(cf2_doubleToFixed(.6), blues->scale, blues->blueScale);
    ...
    for (i = 0; i < blues->count; i++) {
        if (blues->zone[i].bottomZone)
            blues->zone[i].dsFlatEdge = cf2_fixedRound(FT_MulFix(blues->zone[i].csFlatEdge, blues->scale) - blues->boost);
        else
            blues->zone[i].dsFlatEdge = cf2_fixedRound(FT_MulFix(blues->zone[i].csFlatEdge, blues->scale) + blues->boost);
    }
}
...
cf2_blues_capture(const CF2_Blues blues, CF2_Hint bottomHintEdge, CF2_Hint topHintEdge)
{
    ...
    FT_ASSERT(!cf2_hint_isTop(bottomHintEdge) && !cf2_hint_isBottom(topHintEdge));
    ...
    for (i = 0; i < blues->count; i++) {
        if (blues->zone[i].bottomZone && cf2_hint_isBottom(bottomHintEdge)) {
            if (SUB_INT32(blues->zone[i].csBottomEdge, csFuzz) <= bottomHintEdge->csCoord
                && bottomHintEdge->csCoord <= ADD_INT32(blues->zone[i].csTopEdge, csFuzz)) {
                /* bottom edge captured by bottom zone */

                if (blues->suppressOvershoot)
                    dsNew = blues->zone[i].dsFlatEdge;

                else if (SUB_INT32(blues->zone[i].csTopEdge, bottomHintEdge->csCoord) >= blues->blueShift) {
                    /* guarantee minimum of 1 pixel overshoot */
                    dsNew = FT_MIN(cf2_fixedRound(bottomHintEdge->dsCoord), blues->zone[i].dsFlatEdge - cf2_intToFixed(1));
                }
        ...
        if (!blues->zone[i].bottomZone && cf2_hint_isTop(topHintEdge)) {
            if (SUB_INT32(blues->zone[i].csBottomEdge, csFuzz) <= topHintEdge->csCoord && topHintEdge->csCoord <= ADD_INT32(blues->zone[i].csTopEdge, csFuzz)) {
                /* top edge captured by top zone */

                if (blues->suppressOvershoot)
                    dsNew = blues->zone[i].dsFlatEdge;

                else if (SUB_INT32(topHintEdge->csCoord, blues->zone[i].csBottomEdge) >= blues->blueShift) {
                    /* guarantee minimum of 1 pixel overshoot */
                    dsNew = FT_MAX(cf2_fixedRound(topHintEdge->dsCoord), blues->zone[i].dsFlatEdge + cf2_intToFixed(1));
                }
    ...
    if (captured) {
        ...
        if (cf2_hint_isValid(bottomHintEdge)) {
            bottomHintEdge->dsCoord = ADD_INT32(bottomHintEdge->dsCoord, dsMove);
