...
#ifndef CF2_NDEBUG
    FT_ASSERT((hintmask->mask[hintmask->byteCount - 1] & mask) == 0 || *hintmask->error);
#endif
}
...
cf2_hintmask_setAll(CF2_HintMask hintmask, size_t bitCount)
{
    ...
    FT_ASSERT(hintmask->byteCount > 0);
    FT_ASSERT(hintmask->byteCount <= sizeof(hintmask->mask) / sizeof(hintmask->mask[0]));
...
static void cf2_doStems(const CF2_Font font, CF2_Stack opStack, CF2_ArrStack stemHintArray, CF2_Fixed* width, FT_Bool* haveWidth, CF2_Fixed hintOffset)
{
    ...
    if (!font->isT1 && hasWidthArg && !*haveWidth)
        *width = ADD_INT32(cf2_stack_getReal(opStack, 0), cf2_getNominalWidthX(font->decoder));
    ...
    for (i = hasWidthArg ? 1 : 0; i < count; i += 2) {
        ...
        stemhint.min = position = ADD_INT32(position, cf2_stack_getReal(opStack, i));
        stemhint.max = position = ADD_INT32(position, cf2_stack_getReal(opStack, i + 1));
...
static void cf2_doFlex(
    CF2_Stack opStack, CF2_Fixed* curX, CF2_Fixed* curY, CF2_GlyphPath glyphPath, const FT_Bool* readFromStack, FT_Bool doConditionalLastRead)
{
    CF2_Fixed vals[14];
    CF2_UInt idx;
    FT_Bool isHFlex;
    ...
    for (i = 0; i < top; i++) {
        vals[i + 2] = vals[i];
        if (readFromStack[i])
            vals[i + 2] = ADD_INT32(vals[i + 2], cf2_stack_getReal(opStack, idx++));
    }
    ...
    if (doConditionalLastRead) {
        FT_Bool lastIsX = FT_BOOL(cf2_fixedAbs(SUB_INT32(vals[10], *curX)) > cf2_fixedAbs(SUB_INT32(vals[11], *curY)));
    ...
    for (j = 0; j < 2; j++)
        cf2_glyphpath_curveTo(glyphPath, vals[j * 6 + 2], vals[j * 6 + 3], vals[j * 6 + 4], vals[j * 6 + 5], vals[j * 6 + 6], vals[j * 6 + 7]);
...
static void cf2_doBlend(const CFF_Blend blend, CF2_Stack opStack, CF2_UInt numBlends)
{
    ...
    for (i = 0; i < numBlends; i++) {
        ...
        for (j = 1; j < blend->lenBV; j++)
            sum = ADD_INT32(sum, FT_MulFix(*weight++, cf2_stack_getReal(opStack, delta++)));
...
/*
   ...
   */
FT_LOCAL_DEF(void)
cf2_interpT2CharString(CF2_Font font, CF2_Buffer buf, CF2_OutlineCallbacks callbacks, const FT_Vector* translation, FT_Bool doingSeac, CF2_Fixed curX,
    CF2_Fixed curY, CF2_Fixed* width)
{
...
#define PS_STORAGE_SIZE 3
    ...
    cf2_glyphpath_init(&glyphPath, font, callbacks, scaleY,
        /* hShift, */
        &hStemHintArray, &vStemHintArray, &hintMask, hintOriginY, &font->blues, translation);
    ...
    *width = cf2_getDefaultWidthX(decoder);
    ...
    stackSize = font->isCFF2 ? cf2_getMaxstack(decoder) : CF2_OPERAND_STACK_SIZE;
    ...
    while (1) {
        ...
        if (font->isT1) {
            if (!initial_map_ready
                && !(op1 == cf2_cmdHSTEM || op1 == cf2_cmdVSTEM || op1 == cf2_cmdHSBW || op1 == cf2_cmdCALLSUBR || op1 == cf2_cmdRETURN || op1 == cf2_cmdESC
            ...
            if (result_cnt > 0 && !(op1 == cf2_cmdCALLSUBR || op1 == cf2_cmdRETURN || op1 == cf2_cmdESC || op1 >= 32 /* Numbers */)) {
        ...
        case cf2_cmdBLEND: {
            ...
            if (font->cffload->blend_check_vector(&font->blend, font->vsindex, font->lenNDV, font->NDV)) {
                lastError = font->cffload->blend_build_vector(&font->blend, font->vsindex, font->lenNDV, font->NDV);
                if (lastError)
                    goto exit;
            }
        ...
        case cf2_cmdHSTEM:
            ...
            cf2_doStems(font, opStack, &hStemHintArray, width, &haveWidth, font->isT1 ? decoder->builder.left_bearing->y : 0);
        ...
        case cf2_cmdVSTEM:
            ...
            /* add left-sidebearing correction in Type 1 mode */
            cf2_doStems(font, opStack, &vStemHintArray, width, &haveWidth, font->isT1 ? decoder->builder.left_bearing->x : 0);
        ...
        case cf2_cmdVMOVETO:
            ...
            if (cf2_stack_count(opStack) > 1 && !haveWidth)
                *width = ADD_INT32(cf2_stack_getReal(opStack, 0), nominalWidthX);
        ...
        case cf2_cmdRRCURVETO: {
            ...
            FT_TRACE4(("%s\n", op1 == cf2_cmdRCURVELINE ? " rcurveline" : " rrcurveto"));
        ...
        case cf2_cmdCALLSUBR: {
            ...
            FT_TRACE4(("%s", op1 == cf2_cmdCALLGSUBR ? " callgsubr" : " callsubr"));

            if ((!font->isT1 && charstringIndex > CF2_MAX_SUBR) || (font->isT1 && charstringIndex > T1_MAX_SUBRS_CALLS)) {
            ...
            charstring = (CF2_Buffer)cf2_arrstack_getPointer(&subrStack, (size_t)charstringIndex + 1);

            /* set up the new CFF region and pointer */
            subrNum = cf2_stack_popInt(opStack);

            if (font->isT1 && decoder->locals_hash) {
                size_t* val = ft_hash_num_lookup(subrNum, decoder->locals_hash);
            ...
            switch (op1) {
            case cf2_cmdCALLGSUBR:
                FT_TRACE4((" (idx %d, entering level %d)\n", subrNum + decoder->globals_bias, charstringIndex + 1));

                if (cf2_initGlobalRegionBuffer(decoder, subrNum, charstring)) {
                    lastError = FT_THROW(Invalid_Glyph_Format);
            ...
            default:
                /* cf2_cmdCALLSUBR */
                FT_TRACE4((" (idx %d, entering level %d)\n", subrNum + decoder->locals_bias, charstringIndex + 1));

                if (cf2_initLocalRegionBuffer(decoder, subrNum, charstring)) {
                    lastError = FT_THROW(Invalid_Glyph_Format);
                    goto exit; /* subroutine lookup or stream error */
                }
            }
        ...
        case cf2_cmdRETURN:
            ...
            charstring = (CF2_Buffer)cf2_arrstack_getPointer(&subrStack, (CF2_UInt)--charstringIndex);
            continue; /* do not clear the stack */
        ...
        case cf2_cmdESC: {
            ...
            case cf2_escHFLEX: {
                static const FT_Bool readFromStack[12] = {
                    TRUE /* dx1 */, FALSE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */, TRUE /* dx3 */, FALSE /* dy3 */, TRUE /* dx4 */, FALSE /* dy4 */,
                    TRUE /* dx5 */, FALSE /* dy5 */, TRUE /* dx6 */, FALSE /* dy6 */
                };

                FT_TRACE4((" hflex\n"));

                cf2_doFlex(opStack, &curX, &curY, &glyphPath, readFromStack, FALSE /* doConditionalLastRead */);
            }
                continue;

            case cf2_escFLEX: {
                static const FT_Bool readFromStack[12] = {
                    TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */, TRUE /* dx3 */, TRUE /* dy3 */, TRUE /* dx4 */, TRUE /* dy4 */,
                    TRUE /* dx5 */, TRUE /* dy5 */, TRUE /* dx6 */, TRUE /* dy6 */
                };
            ...
            case cf2_escHFLEX1: {
                static const FT_Bool readFromStack[12] = {
                    TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */, TRUE /* dx3 */, FALSE /* dy3 */, TRUE /* dx4 */, FALSE /* dy4 */,
                    TRUE /* dx5 */, TRUE /* dy5 */, TRUE /* dx6 */, FALSE /* dy6 */
                };
            ...
            case cf2_escFLEX1: {
                static const FT_Bool readFromStack[12] = {
                    TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */, TRUE /* dy2 */, TRUE /* dx3 */, TRUE /* dy3 */, TRUE /* dx4 */, TRUE /* dy4 */,
                    TRUE /* dx5 */, TRUE /* dy5 */, FALSE /* dx6 */, FALSE /* dy6 */
                };
                   ...
                   */
                        {
                            if (!font->isT1)
                                ...
                                cf2_stack_setReal(opStack, 2, SUB_INT32(SUB_INT32(v1, v0), cf2_stack_getReal(opStack, 1)));
                                cf2_stack_setReal(opStack, 4, SUB_INT32(SUB_INT32(v2, v1), cf2_stack_getReal(opStack, 3)));

                                /* add left-sidebearing correction */
                                cf2_doStems(font, opStack, isV ? &vStemHintArray : &hStemHintArray, width, &haveWidth,
                                    isV ? decoder->builder.left_bearing->x : decoder->builder.left_bearing->y);
                    ...
                    case cf2_escSEAC:
                        if (!font->isT1)
                            ...
                            /* these?                                           */
#ifdef FT_CONFIG_OPTION_INCREMENTAL
                            if (decoder->glyph_names == 0 && !face->root.internal->incremental_interface)
#else
                            ...
                            cf2_interpT2CharString(font, &component, callbacks, translation, TRUE, 0, 0, &dummyWidth);
                            ...
                            cf2_interpT2CharString(font, &component, callbacks, translation, TRUE, adx - asb, ady, &dummyWidth);
                            cf2_freeT1SeacComponent(decoder, &component);
                    ...
                    case cf2_escSBW:
                        if (!font->isT1)
                            ...
                            builder->left_bearing->x = ADD_INT32(builder->left_bearing->x, lsb_x);
                            builder->left_bearing->y = ADD_INT32(builder->left_bearing->y, lsb_y);
                    ...
                    case cf2_escADD: {
                        ...
                        cf2_stack_pushFixed(opStack, ADD_INT32(summand1, summand2));
                    }
                        continue; /* do not clear the stack */
                    ...
                    case cf2_escSUB: {
                        ...
                        cf2_stack_pushFixed(opStack, SUB_INT32(minuend, subtrahend));
                    }
                        continue; /* do not clear the stack */
                     ...
                     */
                            ...
                            case 0: /* end flex feature */
                                ...
                                if (initial_map_ready && (!decoder->flex_state || decoder->num_flex_vectors != 7)) {
                                    FT_ERROR(("cf2_interpT2CharString (Type 1 mode):"
                                              " unexpected flex end\n"));
                                    lastError = FT_THROW(Invalid_Glyph_Format);
                                    goto exit;
                            ...
                            case 2: /* add flex vectors */
                            {
                                ...
                                if (idx > 0 && idx < 7) {
                                    ...
                                    if (idx == 3 || idx == 6)
                                        cf2_glyphpath_curveTo(&glyphPath, flexStore[0], flexStore[1], flexStore[2], flexStore[3], flexStore[4], flexStore[5]);
                                }
                            } break;
                            ...
                            case 18: /* multiple masters */
                            {
                                ...
                                for (nn = 0; nn < num_points; nn++) {
                                    ...
                                    for (mm = 1; mm < blend->num_designs; mm++)
                                        tmp = ADD_INT32(tmp, FT_MulFix(cf2_stack_getReal(opStack, delta++), blend->weight_vector[mm]));

                                    cf2_stack_setReal(opStack, values++, tmp);
                                }
                                cf2_stack_pop(opStack, (CF2_UInt)arg_cnt - num_points);
                            ...
                            case 19:
                                ...
                                /*     WeightVector                         */
                                {
                                    ...
                                    if (idx < 0 || (FT_UInt)idx + blend->num_designs > decoder->len_buildchar)
                                        goto Unexpected_OtherSubr;

                                    ft_memcpy(&decoder->buildchar[idx], blend->weight_vector, blend->num_designs * sizeof(blend->weight_vector[0]));
                                }
                                break;
                            ...
                            case 25:
                                ...
                                /*     onto T1 stack                   */
                                {
                                    ...
                                    cf2_stack_pushFixed(opStack, decoder->buildchar[idx]);
                                    known_othersubr_result_cnt = 1;
                                }
                                break;
                      ...
                      break;
                            ...
                            case 28:
                                ...
                                /*     onto stack                             */
                                {
                                    ...
                                    r = (CF2_F16Dot16)((decoder->current_subfont->random & 0xFFFF) + 1);

                                    decoder->current_subfont->random = cff_random(decoder->current_subfont->random);
                    ...
                    case cf2_escINDEX: {
                        ...
                        if (size > 0) {
                            ...
                            cf2_stack_pushFixed(opStack, cf2_stack_getReal(opStack, gr_idx));
                        }
                    }
                        continue; /* do not clear the stack */
        ...
        case cf2_cmdENDCHAR:
            ...
            if (cf2_stack_count(opStack) == 1 || cf2_stack_count(opStack) == 5) {
                if (!haveWidth)
                    *width = ADD_INT32(cf2_stack_getReal(opStack, 0), nominalWidthX);
            }
            ...
            if (!font->isCFF2 && !font->isT1 && cf2_stack_count(opStack) > 1) {
                ...
                cf2_interpT2CharString(font, &component, callbacks, translation, TRUE, curX, curY, &dummyWidth);
        ...
        case cf2_cmdHINTMASK:
            ...
            /* never add hints after the mask is computed */
            if (cf2_stack_count(opStack) > 1 && cf2_hintmask_isValid(&hintMask)) {
            ...
            cf2_doStems(font, opStack, &vStemHintArray, width, &haveWidth, 0);
            ...
            if (op1 == cf2_cmdHINTMASK) {
                ...
                cf2_hintmask_read(&hintMask, charstring, cf2_arrstack_size(&hStemHintArray) + cf2_arrstack_size(&vStemHintArray));
            } else {
                /*
           ...
           */
                ...
                cf2_hintmap_init(&counterHintMap, font, &glyphPath.initialHintMap, &glyphPath.hintMoves, scaleY);
                cf2_hintmask_init(&counterMask, error);

                cf2_hintmask_read(&counterMask, charstring, cf2_arrstack_size(&hStemHintArray) + cf2_arrstack_size(&vStemHintArray));
                cf2_hintmap_build(&counterHintMap, &hStemHintArray, &vStemHintArray, &counterMask, 0, FALSE);
            }
            break;
        ...
        default:
            /* numbers */
            {
                ...
                else /* op1 == 255 */
                {
                    ...
                    v = (CF2_Fixed)((byte1 << 24) | (byte2 << 16) | (byte3 << 8) | byte4);
