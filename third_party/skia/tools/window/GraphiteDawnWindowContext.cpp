...
GraphiteDawnWindowContext::GraphiteDawnWindowContext(const DisplayParams& params, wgpu::TextureFormat surfaceFormat)
...
void GraphiteDawnWindowContext::initializeContext(int width, int height)
{
    ...
    fGraphiteContext = skgpu::graphite::ContextFactory::MakeDawn(backendContext, fDisplayParams.fGraphiteTestOptions.fTestOptions.fContextOptions);
    if (!fGraphiteContext) {
        SkASSERT(false);
        return;
    }
...
sk_sp<SkSurface> GraphiteDawnWindowContext::getBackbufferSurface()
{
    ...
    skgpu::graphite::DawnTextureInfo info(/*sampleCount=*/1, skgpu::Mipmapped::kNo, fSurfaceFormat, texture.GetUsage(), wgpu::TextureAspect::All);
    auto backendTex = skgpu::graphite::BackendTextures::MakeDawn(texture.Get());
    SkASSERT(this->graphiteRecorder());
    auto surface = SkSurfaces::WrapBackendTexture(
        this->graphiteRecorder(), backendTex, kBGRA_8888_SkColorType, fDisplayParams.fColorSpace, &fDisplayParams.fSurfaceProps);
    SkASSERT(surface);
    return surface;
}
...
wgpu::Device GraphiteDawnWindowContext::createDevice(wgpu::BackendType type)
{
    ...
    togglesDesc.enabledToggleCount = std::size(kToggles) - (fDisplayParams.fGraphiteTestOptions.fTestOptions.fUseTintIR ? 0 : 1);
    togglesDesc.enabledToggles = kToggles;

    wgpu::RequestAdapterOptions adapterOptions;
    adapterOptions.backendType = type;
    adapterOptions.compatibilityMode = type == wgpu::BackendType::OpenGL || type == wgpu::BackendType::OpenGLES;
    ...
    deviceDescriptor.SetDeviceLostCallback(wgpu::CallbackMode::AllowSpontaneous, [](const wgpu::Device&, wgpu::DeviceLostReason reason, const char* message) {
        if (reason != wgpu::DeviceLostReason::Destroyed && reason != wgpu::DeviceLostReason::InstanceDropped) {
            SK_ABORT("Device lost: %s\n", message);
        }
    });
    deviceDescriptor.SetUncapturedErrorCallback([](const wgpu::Device&, wgpu::ErrorType, const char* message) {
...
void GraphiteDawnWindowContext::configureSurface()
{
    ...
    surfaceConfig.usage = wgpu::TextureUsage::RenderAttachment | wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::CopyDst;
