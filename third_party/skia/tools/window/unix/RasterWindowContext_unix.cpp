...
RasterWindowContext_xlib::RasterWindowContext_xlib(Display* display, XWindow window, int width, int height, const DisplayParams& params)
...
sk_sp<SkSurface> RasterWindowContext_xlib::getBackbufferSurface()
{
...
{
    std::unique_ptr<WindowContext> ctx(new RasterWindowContext_xlib(info.fDisplay, info.fWindow, info.fWidth, info.fHeight, params));
    if (!ctx->isValid()) {
        ctx = nullptr;
    }
    return ctx;
