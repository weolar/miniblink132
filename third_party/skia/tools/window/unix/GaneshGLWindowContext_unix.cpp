...
sk_sp<const GrGLInterface> GLWindowContext_xlib::onInitializeContext()
{
    ...
    CreateContextAttribsFn* createContextAttribs = (CreateContextAttribsFn*)glXGetProcAddressARB((const GLubyte*)"glXCreateContextAttribsARB");
    if (createContextAttribs && fFBConfig) {
        ...
        for (int minor = 2; minor >= 0 && !fGLContext; --minor) {
            ...
            for (int profile : { GLX_CONTEXT_CORE_PROFILE_BIT_ARB, GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB }) {
                gCtxErrorOccurred = false;
                int attribs[] = { GLX_CONTEXT_MAJOR_VERSION_ARB, 3, GLX_CONTEXT_MINOR_VERSION_ARB, minor, GLX_CONTEXT_PROFILE_MASK_ARB, profile, 0 };
                ...
                if (fGLContext && profile == GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB && glXMakeCurrent(fDisplay, fWindow, fGLContext)) {
                    current = true;
    ...
    XGetGeometry(fDisplay, fWindow, &root, &x, &y, (unsigned int*)&fWidth, (unsigned int*)&fHeight, &border_width, &depth);
...
std::unique_ptr<WindowContext> MakeGaneshGLForXlib(const XlibWindowInfo& winInfo, const DisplayParams& params)
{
