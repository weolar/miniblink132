...
void VulkanWindowContext::initializeContext()
{
    ...
    backendContext.fMemoryAllocator = skgpu::VulkanAMDMemoryAllocator::Make(fInstance, backendContext.fPhysicalDevice, backendContext.fDevice, physDevVersion,
        &extensions, fInterface.get(), skgpu::ThreadSafe::kNo,
...
bool VulkanWindowContext::createBuffers(VkFormat format, VkImageUsageFlags usageFlags, SkColorType colorType, VkSharingMode sharingMode)
{
    ...
    for (uint32_t i = 0; i < fImageCount; ++i) {
        ...
        if (usageFlags & VK_IMAGE_USAGE_SAMPLED_BIT) {
            ...
            fSurfaces[i] = SkSurfaces::WrapBackendTexture(fContext.get(), backendTexture, kTopLeft_GrSurfaceOrigin, fDisplayParams.fMSAASampleCount, colorType,
            ...
            fSurfaces[i] = SkSurfaces::WrapBackendRenderTarget(
                fContext.get(), backendRT, kTopLeft_GrSurfaceOrigin, colorType, fDisplayParams.fColorSpace, &fDisplayParams.fSurfaceProps);
        }
        if (!fSurfaces[i]) {
            return false;
        }
    ...
    for (uint32_t i = 0; i < fImageCount + 1; ++i) {
        ...
        SkDEBUGCODE(VkResult result =) GR_VK_CALL(fInterface, CreateSemaphore(fDevice, &semaphoreInfo, nullptr, &fBackbuffers[i].fRenderSemaphore));
...
sk_sp<SkSurface> VulkanWindowContext::getBackbufferSurface()
{
    ...
    SkDEBUGCODE(VkResult result =) GR_VK_CALL(fInterface, CreateSemaphore(fDevice, &semaphoreInfo, nullptr, &semaphore));
    SkASSERT(result == VK_SUCCESS);

    // acquire the image
    VkResult res = fAcquireNextImageKHR(fDevice, fSwapchain, UINT64_MAX, semaphore, VK_NULL_HANDLE, &backbuffer->fImageIndex);
    ...
    if (VK_ERROR_OUT_OF_DATE_KHR == res) {
        ...
        // acquire the image
        res = fAcquireNextImageKHR(fDevice, fSwapchain, UINT64_MAX, semaphore, VK_NULL_HANDLE, &backbuffer->fImageIndex);
