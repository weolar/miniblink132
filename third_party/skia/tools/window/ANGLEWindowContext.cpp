...
ANGLEWindowContext::~ANGLEWindowContext()
{
    this->destroyContext();
}
...
sk_sp<const GrGLInterface> ANGLEWindowContext::onInitializeContext()
{
    ...
    const EGLint configAttribs[] = { EGL_RENDERABLE_TYPE,
        ...
        EGL_OPENGL_ES3_BIT, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_SAMPLE_BUFFERS, sampleBuffers, EGL_SAMPLES,
        eglSampleCnt, EGL_NONE };
    ...
    fEGLSurface = eglCreateWindowSurface(fDisplay, surfaceConfig, this->onGetNativeWindow(), nullptr);
    ...
    sk_sp<const GrGLInterface> interface(
        GrGLMakeAssembledInterface(nullptr, [](void* ctx, const char name[]) -> GrGLFuncPtr { return eglGetProcAddress(name); }));
    if (interface) {
