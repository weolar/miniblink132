...
GLWindowContext_android::GLWindowContext_android(ANativeWindow* window, const skwindow::DisplayParams& params)
    : INHERITED(params)
...
sk_sp<const GrGLInterface> GLWindowContext_android::onInitializeContext()
{
    ...
    if (fDisplayParams.fCreateProtectedNativeBackend && !strstr(extensions, "EGL_EXT_protected_content")) {
    ...
    EGLint numConfigs = 0;
    EGLint eglSampleCnt = fDisplayParams.fMSAASampleCount > 1 ? fDisplayParams.fMSAASampleCount > 1 : 0;
    const EGLint configAttribs[] = { EGL_SURFACE_TYPE, EGL_PBUFFER_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8,
        EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_STENCIL_SIZE, 8, EGL_SAMPLE_BUFFERS, eglSampleCnt ? 1 : 0, EGL_SAMPLES, eglSampleCnt, EGL_NONE };
    ...
    fEGLContext = eglCreateContext(fDisplay, surfaceConfig, nullptr, kEGLContextAttribsForOpenGLES.data());
    ...
    const EGLint surfaceAttribs[] = { fDisplayParams.fCreateProtectedNativeBackend ? EGL_PROTECTED_CONTENT_EXT : EGL_NONE,
        fDisplayParams.fCreateProtectedNativeBackend ? EGL_TRUE : EGL_NONE, EGL_NONE };

    fSurfaceAndroid = eglCreateWindowSurface(fDisplay, surfaceConfig, fNativeWindow, surfaceAttribs);
