...
std::unique_ptr<WindowContext> MakeVulkanForWin(HWND hwnd, const DisplayParams& params)
{
    ...
    auto canPresent = [instProc](VkInstance instance, VkPhysicalDevice physDev, uint32_t queueFamilyIndex) {
        static PFN_vkGetPhysicalDeviceWin32PresentationSupportKHR getPhysicalDeviceWin32PresentationSupportKHR = nullptr;
        if (!getPhysicalDeviceWin32PresentationSupportKHR) {
            getPhysicalDeviceWin32PresentationSupportKHR
                = (PFN_vkGetPhysicalDeviceWin32PresentationSupportKHR)instProc(instance, "vkGetPhysicalDeviceWin32PresentationSupportKHR");
        }
    ...
    std::unique_ptr<WindowContext> ctx(new internal::VulkanWindowContext(params, createVkSurface, canPresent, instProc));
    if (!ctx->isValid()) {
        return nullptr;
    }
    return ctx;
