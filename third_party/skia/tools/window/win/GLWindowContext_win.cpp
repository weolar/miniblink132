...
sk_sp<const GrGLInterface> GLWindowContext_win::onInitializeContext()
{
    ...
    fHGLRC = SkCreateWGLContext(dc, fDisplayParams.fMSAASampleCount, false /* deepColor */, kGLPreferCompatibilityProfile_SkWGLContextRequest);
    if (nullptr == fHGLRC) {
        return nullptr;
    }
    ...
    if (wglMakeCurrent(dc, fHGLRC)) {
        ...
        if (renderDocAttached) {
            ...
            fHGLRC = SkCreateWGLContext(dc, fDisplayParams.fMSAASampleCount, false /* deepColor */, kGLPreferCoreProfile_SkWGLContextRequest);
            if (nullptr == fHGLRC) {
                return nullptr;
            }
        }
