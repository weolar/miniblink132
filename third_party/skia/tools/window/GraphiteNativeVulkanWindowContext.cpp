...
GraphiteVulkanWindowContext::GraphiteVulkanWindowContext(
    const DisplayParams& params, CreateVkSurfaceFn createVkSurface, CanPresentFn canPresent, PFN_vkGetInstanceProcAddr instProc)
...
void GraphiteVulkanWindowContext::initializeContext()
{
    ...
    if (!sk_gpu_test::CreateVkBackendContext(getInstanceProc, &backendContext, &extensions, &features, &fDebugCallback, &fPresentQueueIndex, fCanPresentFn,
            fDisplayParams.fCreateProtectedNativeBackend)) {
        sk_gpu_test::FreeVulkanFeaturesStructs(&features);
        return;
    }

    if (!extensions.hasExtension(VK_KHR_SURFACE_EXTENSION_NAME, 25) || !extensions.hasExtension(VK_KHR_SWAPCHAIN_EXTENSION_NAME, 68)) {
        sk_gpu_test::FreeVulkanFeaturesStructs(&features);
        return;
    }
    ...
    PFN_vkGetPhysicalDeviceProperties localGetPhysicalDeviceProperties = reinterpret_cast<PFN_vkGetPhysicalDeviceProperties>(
        backendContext.fGetProc("vkGetPhysicalDeviceProperties", backendContext.fInstance, VK_NULL_HANDLE));
    ...
    fInterface.reset(new skgpu::VulkanInterface(backendContext.fGetProc, fInstance, fDevice, backendContext.fMaxAPIVersion, physDevVersion, &extensions));
    ...
    VkBool32 supported;
    VkResult res = fGetPhysicalDeviceSurfaceSupportKHR(fPhysicalDevice, fPresentQueueIndex, fSurface, &supported);
    if (VK_SUCCESS != res) {
        this->destroyContext();
        sk_gpu_test::FreeVulkanFeaturesStructs(&features);
        return;
...
bool GraphiteVulkanWindowContext::createSwapchain(int width, int height, const DisplayParams& params)
{
    ...
    res = fGetPhysicalDeviceSurfaceFormatsKHR(fPhysicalDevice, fSurface, &surfaceFormatCount, nullptr);
    if (VK_SUCCESS != res) {
        return false;
    }
    ...
    res = fGetPhysicalDeviceSurfaceFormatsKHR(fPhysicalDevice, fSurface, &surfaceFormatCount, surfaceFormats);
    if (VK_SUCCESS != res) {
        return false;
    }

    uint32_t presentModeCount;
    res = fGetPhysicalDeviceSurfacePresentModesKHR(fPhysicalDevice, fSurface, &presentModeCount, nullptr);
    if (VK_SUCCESS != res) {
        return false;
    }
    ...
    res = fGetPhysicalDeviceSurfacePresentModesKHR(fPhysicalDevice, fSurface, &presentModeCount, presentModes);
    if (VK_SUCCESS != res) {
        return false;
    }
    ...
    swapchainCreateInfo.flags = fDisplayParams.fCreateProtectedNativeBackend ? VK_SWAPCHAIN_CREATE_PROTECTED_BIT_KHR : 0;
    ...
    if (!this->createBuffers(swapchainCreateInfo.imageFormat, usageFlags, colorType, swapchainCreateInfo.imageSharingMode)) {
        fDeviceWaitIdle(fDevice);
...
bool GraphiteVulkanWindowContext::createBuffers(VkFormat format, VkImageUsageFlags usageFlags, SkColorType colorType, VkSharingMode sharingMode)
{
    ...
    for (uint32_t i = 0; i < fImageCount; ++i) {
        ...
        info.fFlags = fDisplayParams.fCreateProtectedNativeBackend ? VK_IMAGE_CREATE_PROTECTED_BIT : 0;

        auto backendTex = skgpu::graphite::BackendTextures::MakeVulkan(
            this->dimensions(), info, VK_IMAGE_LAYOUT_UNDEFINED, fPresentQueueIndex, fImages[i], skgpu::VulkanAlloc());

        fSurfaces[i]
            = SkSurfaces::WrapBackendTexture(this->graphiteRecorder(), backendTex, colorType, fDisplayParams.fColorSpace, &fDisplayParams.fSurfaceProps);
    ...
    for (uint32_t i = 0; i < fImageCount + 1; ++i) {
        ...
        VkResult result;
        VULKAN_CALL_RESULT_NOCHECK(fInterface, result, CreateSemaphore(fDevice, &semaphoreInfo, nullptr, &fBackbuffers[i].fRenderSemaphore));
    }
...
void GraphiteVulkanWindowContext::destroyBuffers()
{
    if (fBackbuffers) {
        for (uint32_t i = 0; i < fImageCount + 1; ++i) {
            fBackbuffers[i].fImageIndex = -1;
            VULKAN_CALL(fInterface, DestroySemaphore(fDevice, fBackbuffers[i].fRenderSemaphore, nullptr));
        }
    }
...
sk_sp<SkSurface> GraphiteVulkanWindowContext::getBackbufferSurface()
{
    ...
    semaphoreInfo.flags = 0;
    VkResult result;
    VULKAN_CALL_RESULT_NOCHECK(fInterface, result, CreateSemaphore(fDevice, &semaphoreInfo, nullptr, &fWaitSemaphore));

    // acquire the image
    VkResult res = fAcquireNextImageKHR(fDevice, fSwapchain, UINT64_MAX, fWaitSemaphore, VK_NULL_HANDLE, &backbuffer->fImageIndex);
    ...
    if (VK_ERROR_OUT_OF_DATE_KHR == res) {
        ...
        // acquire the image
        res = fAcquireNextImageKHR(fDevice, fSwapchain, UINT64_MAX, fWaitSemaphore, VK_NULL_HANDLE, &backbuffer->fImageIndex);
...
void GraphiteVulkanWindowContext::onSwapBuffers()
{
    ...
    if (recording) {
        ...
        skgpu::MutableTextureState presentState = skgpu::MutableTextureStates::MakeVulkan(VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, fPresentQueueIndex);
        ...
        auto beSignalSemaphore = skgpu::graphite::BackendSemaphores::MakeVulkan(backbuffer->fRenderSemaphore);
        ...
        skgpu::graphite::GpuFinishedProc finishCallback = [](skgpu::graphite::GpuFinishedContext c, skgpu::CallbackResult status) {
            // regardless of the status we need to destroy the semaphore
            const auto* context = reinterpret_cast<const FinishContext*>(c);
            VULKAN_CALL(context->interface, DestroySemaphore(context->device, context->waitSemaphore, nullptr));
        };
