...
SkCanvas* Request::getCanvas()
{
    ...
    GLTestContext* gl = factory->getContextInfo(skgpu::ContextType::kGL, GrContextFactory::ContextOverrides::kNone).glContext();
    if (!gl) {
        gl = factory->getContextInfo(skgpu::ContextType::kGLES, GrContextFactory::ContextOverrides::kNone).glContext();
    }
    if (gl) {
        gl->makeCurrent();
    }
...
sk_sp<SkData> Request::writeOutSkp()
{
    ...
    SkCanvas* canvas = recorder.beginRecording(SkIntToScalar(bounds.width()), SkIntToScalar(bounds.height()));
...
{
    auto result = fContextFactory->get(skgpu::ContextType::kGL, GrContextFactory::ContextOverrides::kNone);
    if (!result) {
        result = fContextFactory->get(skgpu::ContextType::kGLES, GrContextFactory::ContextOverrides::kNone);
    }
    return result;
}
...
SkIRect Request::getBounds()
{
    ...
    if (fPicture) {
        ...
        if (fGPUEnabled) {
            ...
            bounds = SkIRect::MakeWH(std::min(bounds.width(), maxRTSize), std::min(bounds.height(), maxRTSize));
        }
    } else {
    ...
    bounds = SkIRect::MakeWH(std::min(bounds.width(), kMaxWidth), std::min(bounds.height(), kMaxHeight));
    return bounds;
}
...
SkSurface* Request::createCPUSurface()
{
    ...
    auto colorSpace = kRGBA_F16_SkColorType == cap.fColorType ? SkColorSpace::MakeSRGBLinear() : SkColorSpace::MakeSRGB();
    SkImageInfo info = SkImageInfo::Make(bounds.size(), cap.fColorType, kPremul_SkAlphaType, cap.fSRGB ? colorSpace : nullptr);
