...
static SkString generate_template(SkString source)
{
    ...
    debuggerTemplate.appendf("<!DOCTYPE html>\n"
                             "<html>\n"
                             "<head>\n"
...
int SendOK(MHD_Connection* connection)
{
    ...
    MHD_Response* response = MHD_create_response_from_buffer(strlen(data), (void*)data, MHD_RESPMEM_PERSISTENT);
...
{
    MHD_Response* response = MHD_create_response_from_buffer(data->size(), const_cast<void*>(data->data()), MHD_RESPMEM_MUST_COPY);
...
int SendTemplate(MHD_Connection* connection, bool redirect, const char* redirectUrl)
{
    ...
    MHD_Response* response
        = MHD_create_response_from_buffer(debuggerTemplate.size(), (void*)const_cast<char*>(debuggerTemplate.c_str()), MHD_RESPMEM_MUST_COPY);
