    ...
    int invoke(Request* request, MHD_Connection* connection, const char* url, const char* method, const char* upload_data, size_t* upload_data_size) const
    {
        for (int i = 0; i < fHandlers.size(); i++) {
            if (fHandlers[i]->canHandle(method, url)) {
                return fHandlers[i]->handle(request, connection, url, method, upload_data, upload_data_size);
            }
        }
        return MHD_NO;
    }
    ...
    size_t* upload_data_size, void** con_cls)
{
    ...
    int result = kUrlManager.invoke(request, connection, url, method, upload_data, upload_data_size);
    if (MHD_NO == result) {
...
#endif
        ,
        FLAGS_port, nullptr, nullptr, &answer_to_connection, &request, MHD_OPTION_SOCK_ADDR, &address, MHD_OPTION_END);
    if (nullptr == daemon) {
