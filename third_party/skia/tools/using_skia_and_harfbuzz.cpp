...
void BaseOption::Init(const std::vector<BaseOption*>& option_list, int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (it != options.end()) {
            ...
            for (BaseOption* opt : option_list) {
                printf("\t%s\t%s (%s)\n", opt->selector.c_str(), opt->description.c_str(), opt->valueToString().c_str());
            }
            exit(-1);
        }
    }
...
struct DoubleOption : Option<double> {
    ...
    DoubleOption(std::string _selector, std::string _description, double defaultValue)
        : Option<double>(std::move(_selector), std::move(_description), std::move(defaultValue))
    {
    }
};
...
struct StringOption : Option<std::string> {
    ...
    StringOption(std::string _selector, std::string _description, std::string defaultValue)
        : Option<std::string>(std::move(_selector), std::move(_description), std::move(defaultValue))
    {
    }
};
...
struct Config {
    ...
    DoubleOption line_spacing_ratio = DoubleOption("-h", "Line spacing ratio", 0.25f);
    StringOption output_file_name = StringOption("-o", ".pdf output file name", "out-skiahf.pdf");

    Config(int argc, char** argv)
    {
        BaseOption::Init(std::vector<BaseOption*> { &page_width, &page_height, &title, &author, &subject, &keywords, &creator, &font_file, &font_size,
                             &left_margin, &line_spacing_ratio, &output_file_name },
            argc, argv);
    }
};
...
public:
    ...
    void WriteLine(const SkShaper& shaper, const char* text, size_t textBytes)
    {
        ...
        std::unique_ptr<SkShaper::BiDiRunIterator> bidi = SkShapers::unicode::BidiRunIterator(get_unicode(), text, textBytes, defaultLevel);
        SkASSERT(bidi);

        std::unique_ptr<SkShaper::LanguageRunIterator> language = SkShaper::MakeStdLanguageRunIterator(text, textBytes);
        SkASSERT(language);

        std::unique_ptr<SkShaper::ScriptRunIterator> script = SkShapers::HB::ScriptRunIterator(text, textBytes);
        SkASSERT(script);

        std::unique_ptr<SkShaper::FontRunIterator> fontRuns = SkShaper::MakeFontMgrRunIterator(text, textBytes, font, SkFontMgr::RefEmpty());
        SkASSERT(fontRuns);

        const SkScalar width = config->page_width.value - 2 * config->left_margin.value;
        shaper.shape(text, textBytes, *fontRuns, *bidi, *script, *language, nullptr, 0, width, &textBlobBuilder);
        ...
        if (!pageCanvas || (current_y > config->line_spacing_ratio.value * config->font_size.value && current_y + endPoint.y() > config->page_height.value)) {
            if (pageCanvas) {
                document->endPage();
            }
            pageCanvas = document->beginPage(SkDoubleToScalar(config->page_width.value), SkDoubleToScalar(config->page_height.value));
        ...
        pageCanvas->drawTextBlob(blob.get(), SkDoubleToScalar(current_x), SkDoubleToScalar(current_y), glyph_paint);
