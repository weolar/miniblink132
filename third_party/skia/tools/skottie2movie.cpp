    ...
    do {
        ...
        if (!surf) {
            if (FLAGS_gpu) {
                ...
                surf = SkSurfaces::RenderTarget(grctx, skgpu::Budgeted::kNo, info, 0, GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, nullptr);
                if (!surf) {
                    grctx = nullptr;
                }
            }
        ...
        for (int i = 0; i <= frames; ++i) {
            ...
            if (grctx) {
                auto read_pixels_cb = [](SkSurface::ReadPixelsContext ctx, std::unique_ptr<const SkSurface::AsyncReadResult> result) {
                ...
                surf->asyncRescaleAndReadPixels(
                    info, { 0, 0, info.width(), info.height() }, SkSurface::RescaleGamma::kSrc, SkImage::RescaleMode::kNearest, read_pixels_cb, &asyncRec);
                grctx->submit();
            } else {
                SkPixmap pm;
