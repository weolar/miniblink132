...
static float print_info(SkCanvas* canvas, const skif::LayerSpace<SkIRect>& layerContentBounds, const skif::DeviceSpace<SkIRect>& outputBounds,
    std::optional<skif::DeviceSpace<SkIRect>> hintedOutputBounds, const skif::LayerSpace<SkIRect>& unhintedLayerBounds)
{
    ...
    y = print_size(canvas, "Content (in layer)", SkIRect(layerContentBounds), kLineInset, y, font, text);
    text.setColor(SK_ColorDKGRAY);
    y = print_size(canvas, "Target (in device)", SkIRect(outputBounds), kLineInset, y, font, text);
    text.setColor(SK_ColorBLUE);
    y = print_size(canvas, "Output (w/ hint)", hintedOutputBounds ? SkIRect(*hintedOutputBounds) : std::optional<SkIRect> {}, kLineInset, y, font, text);
    text.setColor(SK_ColorGREEN);
    y = print_size(canvas, "Input (w/ no hint)", SkIRect(unhintedLayerBounds), kLineInset, y, font, text);
...
static const SkColor4f kScaleGradientColors[] = { { 0.05f, 0.0f, 6.f, 1.f }, // Severe downscaling, s < 1/8, log(s) < -3
...
static void draw_scale_key(SkCanvas* canvas, float y)
{
    ...
    sk_sp<SkShader> gradient = SkGradientShader::MakeLinear(
        pts, kScaleGradientColors, nullptr, kGradientStops, kStopCount, SkTileMode::kClamp, SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr);
    SkPaint keyPaint;
    keyPaint.setShader(gradient);
    canvas->drawRect(key, keyPaint);
    for (int i = 0; i < kStopCount; ++i) {
        print_label(canvas, key.fRight + 5.f, key.fTop + kGradientStops[i] * key.height(), SkScalarPow(2.f, kLogScaleFactors[i]));
    }
}
...
static void draw_scale_factors(SkCanvas* canvas, const skif::Mapping& mapping, const SkRect& rect)
{
    ...
    for (int i = 0; i < 5; ++i) {
        float scale = SkMatrixPriv::DifferentialAreaScale(mapping.layerToDevice(), SkPoint(mapping.paramToLayer(skif::ParameterSpace<SkPoint>(testPoints[i]))));
            ...
            for (int j = 0; j <= kStopCount; ++j) {
                if (j == kStopCount) {
                    ...
                    if (j == 0) {
                        ...
                        SkScalar t = (logScale - kLogScaleFactors[j - 1]) / (kLogScaleFactors[j] - kLogScaleFactors[j - 1]);
        ...
        canvas->drawRect(SkRect::MakeLTRB(testPoints[i].fX - 4.f, testPoints[i].fY - 4.f, testPoints[i].fX + 4.f, testPoints[i].fY + 4.f), p);
    }
}
...
public:
    ...
    void load(SkScalar w, SkScalar h) override
    {
        ...
        fImage = ToolUtils::create_checkerboard_image(300, 300, SK_ColorMAGENTA, SK_ColorLTGRAY, 50);
    }
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        canvas->drawImageRect(
            fImage.get(), localContentRect, localContentRect, SkSamplingOptions(SkFilterMode::kLinear), nullptr, SkCanvas::kFast_SrcRectConstraint);
        canvas->restore();
        ...
        if (!target.intersect(SkIRect::MakeWH(canvas->imageInfo().width(), canvas->imageInfo().height()))) {
            return;
        }
        skif::DeviceSpace<SkIRect> targetOutput(target);
        skif::ParameterSpace<SkRect> contentBounds(localContentRect);
        skif::ParameterSpace<SkPoint> contentCenter({ localContentRect.centerX(), localContentRect.centerY() });
        ...
        canvas->drawPath(create_axis_path(SkRect(mapping.paramToLayer(contentBounds)), 20.f), line_paint(SK_ColorGRAY));
        canvas->restore();
        ...
        skif::LayerSpace<SkIRect> hintedLayerBounds = as_IFB(fBlur)->getInputBounds(mapping, targetOutput, contentBounds);
        skif::LayerSpace<SkIRect> unhintedLayerBounds = as_IFB(fBlur)->getInputBounds(mapping, targetOutput, {});

        canvas->setMatrix(mapping.layerToDevice());
        canvas->drawRect(SkRect::Make(SkIRect(targetOutputInLayer)), line_paint(SK_ColorDKGRAY, true));
        ...
        std::optional<skif::DeviceSpace<SkIRect>> hintedOutputBounds = as_IFB(fBlur)->getOutputBounds(layerOnly, contentBounds);
        if (hintedOutputBounds) {
            canvas->drawRect(SkRect::Make(SkIRect(*hintedOutputBounds)), line_paint(SK_ColorBLUE));
        }

        canvas->resetMatrix();
        float y = print_info(canvas, mapping.paramToLayer(contentBounds).roundOut(), targetOutput, hintedOutputBounds, unhintedLayerBounds);
