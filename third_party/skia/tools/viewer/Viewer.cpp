...
#define BACKENDS_STR BACKENDS_STR_EVALUATOR("\"sw\"", GL_BACKEND_STR, VK_BACKEND_STR, MTL_BACKEND_STR, D3D_BACKEND_STR, DAWN_BACKEND_STR)
...
#endif
#define PATHSTRATEGY_STR_EVALUATOR(default, raster, compute_analytic, compute_msaa16, compute_msaa8, tess) default raster compute_analytic compute_msaa16 tess
#define PATHSTRATEGY_STR                                                                                                                                       \
    PATHSTRATEGY_STR_EVALUATOR(                                                                                                                                \
        "\"default\"", "\"raster\"", COMPUTE_ANALYTIC_PATHSTRATEGY_STR, COMPUTE_MSAA16_PATHSTRATEGY_STR, COMPUTE_MSAA8_PATHSTRATEGY_STR, "\"tessellation\"")

static DEFINE_string(pathstrategy, "default", "Path renderer strategy to use. Allowed values are " PATHSTRATEGY_STR ".");
#endif
...
#if defined(SK_GRAPHITE)
static const char* get_path_renderer_strategy_string(skgpu::graphite::PathRendererStrategy strategy)
{
...
const char* get_backend_string(sk_app::Window::BackendType type)
{
    ...
    case sk_app::Window::kNativeGL_BackendType:
        return "OpenGL";
    case sk_app::Window::kANGLE_BackendType:
        return "ANGLE";
    case sk_app::Window::kGraphiteDawn_BackendType:
        return "Dawn (Graphite)";
    case sk_app::Window::kVulkan_BackendType:
        return "Vulkan";
    case sk_app::Window::kGraphiteVulkan_BackendType:
        return "Vulkan (Graphite)";
    case sk_app::Window::kMetal_BackendType:
        return "Metal";
    case sk_app::Window::kGraphiteMetal_BackendType:
...
static SkColorSpacePrimaries gSrgbPrimaries = { 0.64f, 0.33f, 0.30f, 0.60f, 0.15f, 0.06f, 0.3127f, 0.3290f };

static SkColorSpacePrimaries gAdobePrimaries = { 0.64f, 0.33f, 0.21f, 0.71f, 0.15f, 0.06f, 0.3127f, 0.3290f };

static SkColorSpacePrimaries gP3Primaries = { 0.680f, 0.320f, 0.265f, 0.690f, 0.150f, 0.060f, 0.3127f, 0.3290f };

static SkColorSpacePrimaries gRec2020Primaries = { 0.708f, 0.292f, 0.170f, 0.797f, 0.131f, 0.046f, 0.3127f, 0.3290f };
    ...
    , fPerspectiveMode(kPerspective_Off)
{
    ...
    displayParams.fGrContextOptions.fShaderCacheStrategy = GrContextOptions::ShaderCacheStrategy::kSkSL;
    ...
    if (FLAGS_dmsaa) {
        displayParams.fSurfaceProps
            = SkSurfaceProps(displayParams.fSurfaceProps.flags() | SkSurfaceProps::kDynamicMSAA_Flag, displayParams.fSurfaceProps.pixelGeometry());
    }
    ...
    displayParams.fGraphiteTestOptions.fPriv.fPathRendererStrategy = get_path_renderer_strategy_type(FLAGS_pathstrategy[0]);
#endif
    ...
    fCommands.addCommand('w', "Modes", "Toggle reduced shaders", [this]() {
        DisplayParams params = fWindow->getRequestedDisplayParams();
        params.fGrContextOptions.fReducedShaderVariations = !params.fGrContextOptions.fReducedShaderVariations;
        fWindow->setRequestedDisplayParams(params);
        fWindow->inval();
    });
    fCommands.addCommand(skui::Key::kRight, "Right", "Navigation", "Next slide",
        [this]() { this->setCurrentSlide(fCurrentSlide < fSlides.size() - 1 ? fCurrentSlide + 1 : 0); });
    fCommands.addCommand(skui::Key::kLeft, "Left", "Navigation", "Previous slide",
        [this]() { this->setCurrentSlide(fCurrentSlide > 0 ? fCurrentSlide - 1 : fSlides.size() - 1); });
    ...
    fCommands.addCommand('p', "Transform", "Toggle Perspective Mode", [this]() {
        fPerspectiveMode = (kPerspective_Real == fPerspectiveMode) ? kPerspective_Fake : kPerspective_Real;
        this->updateTitle();
        fWindow->inval();
    });
    fCommands.addCommand('P', "Transform", "Toggle Perspective", [this]() {
        fPerspectiveMode = (kPerspective_Off == fPerspectiveMode) ? kPerspective_Real : kPerspective_Off;
        this->updateTitle();
        fWindow->inval();
    });
    fCommands.addCommand('a', "Transform", "Toggle Animation", [this]() { fAnimTimer.togglePauseResume(); });
...
void Viewer::initSlides()
{
    ...
    static const struct {
        ...
        = { { ".mskp", "mskp-dir", FLAGS_mskps, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<MSKPSlide>(name, path); } },
              { ".skp", "skp-dir", FLAGS_skps, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<SKPSlide>(name, path); } },
              { ".jpg", "jpg-dir", FLAGS_jpgs, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<ImageSlide>(name, path); } },
              { ".jxl", "jxl-dir", FLAGS_jxls, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<ImageSlide>(name, path); } },
#if defined(SK_ENABLE_SKOTTIE)
              { ".json", "skottie-dir", FLAGS_lotties,
                  [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<SkottieSlide>(name, path); } },
#endif

#if defined(SK_ENABLE_SVG)
              { ".svg", "svg-dir", FLAGS_svgs, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<SvgSlide>(name, path); } },
#endif
          };
    ...
    if (!FLAGS_file.isEmpty()) {
        ...
        if (file.equals("stdin")) {
            ...
            for (auto image : images) {
                ...
                fSlides.push_back(sk_make_sp<ImageSlide>(SkStringPrintf("Image %c", imageID), std::move(image)));
            }
            if (!fSlides.empty()) {
    ...
    auto orderBySlideName = [](sk_sp<Slide> a, sk_sp<Slide> b) { return SK_strcasecmp(a->getName().c_str(), b->getName().c_str()) < 0; };
        ...
        for (const auto& flag : info.fFlags) {
            if (SkStrEndsWith(flag.c_str(), info.fExtension)) {
                ...
                if (sortedFilenames.size()) {
                    SkTQSort(
                        sortedFilenames.begin(), sortedFilenames.end(), [](const SkString& a, const SkString& b) { return strcmp(a.c_str(), b.c_str()) < 0; });
                }
            ...
            if (!dirSlides.empty()) {
                fSlides.push_back(sk_make_sp<SlideDir>(SkStringPrintf("%s[%s]", info.fDirName, flag.c_str()), std::move(dirSlides)));
...
void Viewer::updateTitle()
{
    ...
    auto paintFlag = [this, &paintTitle](bool SkPaintFields::*flag, bool (SkPaint::*isFlag)() const, const char* on, const char* off) {
        if (fPaintOverrides.*flag) {
            paintTitle.append((fPaint.*isFlag)() ? on : off);
        }
    };

    auto fontFlag = [this, &paintTitle](bool SkFontFields::*flag, bool (SkFont::*isFlag)() const, const char* on, const char* off) {
    ...
    fontFlag(&SkFontFields::fForceAutoHinting, &SkFont::isForceAutoHinting, "Force Autohint", "No Force Autohint");
    ...
    fontFlag(&SkFontFields::fLinearMetrics, &SkFont::isLinearMetrics, "Linear Metrics", "Non-Linear Metrics");
    fontFlag(&SkFontFields::fEmbeddedBitmaps, &SkFont::isEmbeddedBitmaps, "Bitmap Text", "No Bitmap Text");
    ...
    if (ColorMode::kLegacy != fColorMode) {
        ...
        title.appendf(" %s Gamma %f", curPrimaries >= 0 ? gNamedPrimaries[curPrimaries].fName : "Custom", fColorSpaceTransferFn.g);
    }
...
#if defined(SK_GRAPHITE)
        skgpu::graphite::PathRendererStrategy strategy = fWindow->getRequestedDisplayParams().fGraphiteTestOptions.fPriv.fPathRendererStrategy;
        if (skgpu::graphite::PathRendererStrategy::kDefault != strategy) {
            title.appendf(" [Path renderer strategy: %s]", get_path_renderer_strategy_string(strategy));
        }
#endif
    } else {
...
void Viewer::setCurrentSlide(int slide)
{
    ...
    fSlides[slide]->load(SkIntToScalar(fWindow->width()) / scaleFactor, SkIntToScalar(fWindow->height()) / scaleFactor);
    fCurrentSlide = slide;
...
SkMatrix Viewer::computePerspectiveMatrix()
{
    ...
    SkPoint perspPts[4] = { { fPerspectivePoints[0].fX * w, fPerspectivePoints[0].fY * h }, { fPerspectivePoints[1].fX * w, fPerspectivePoints[1].fY * h },
        { fPerspectivePoints[2].fX * w, fPerspectivePoints[2].fY * h }, { fPerspectivePoints[3].fX * w, fPerspectivePoints[3].fY * h } };
    SkMatrix m;
...
public:
    ...
    const SkTextBlob* filterTextBlob(const SkPaint& paint, const SkTextBlob* blob, sk_sp<SkTextBlob>* cache)
    {
        ...
        for (SkTextBlobRunIterator it(blob); !it.done(); it.next()) {
            ...
            const SkTextBlobBuilder::RunBuffer& runBuffer = it.positioning() == SkTextBlobRunIterator::kDefault_Positioning
                ? builder.allocRunText(font, it.glyphCount(), it.offset().x(), it.offset().y(), it.textSize())
                ...
                : it.positioning() == SkTextBlobRunIterator::kFull_Positioning    ? builder.allocRunTextPos(font, it.glyphCount(), it.textSize())
                : it.positioning() == SkTextBlobRunIterator::kRSXform_Positioning ? builder.allocRunTextRSXform(font, it.glyphCount(), it.textSize())
    ...
    void onDrawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint) override
    {
        ...
        this->SkPaintFilterCanvas::onDrawTextBlob(this->filterTextBlob(paint, blob, &cache), x, y, paint);
    }
    ...
    void onDrawGlyphRunList(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint) override
    {
        ...
        const sktext::GlyphRunList& filtered = builder.blobToGlyphRunList(*cache, glyphRunList.origin());
...
static SkSerialProcs serial_procs_using_png()
{
    ...
    sProcs.fImageProc = [](SkImage* img, void*) -> sk_sp<SkData> { return SkPngEncoder::Encode(as_IB(img)->directContext(), img, SkPngEncoder::Options {}); };
    return sProcs;
}
...
void Viewer::drawSlide(SkSurface* surface)
{
    ...
    if (kPerspective_Fake == fPerspectiveMode || fShowZoomWindow || fShowHistogramWindow || Window::kRaster_BackendType == fBackendType || colorSpace != nullptr
        || FLAGS_offscreen) {
        SkSurfaceProps props(fWindow->getRequestedDisplayParams().fSurfaceProps);
        slideCanvas->getProps(&props);

        SkImageInfo info = SkImageInfo::Make(fWindow->width(), fWindow->height(), colorType, kPremul_SkAlphaType, colorSpace);
        offscreenSurface = Window::kRaster_BackendType == this->fBackendType ? SkSurfaces::Raster(info, &props) : slideCanvas->makeSurface(info, &props);
...
static void ImGui_Primaries(SkColorSpacePrimaries* primaries, SkPaint* gamutPaint)
{
    ...
    dc.fDrawList->AddImage(gamutPaint, dc.fPos, ImVec2(dc.fPos.x + dc.fSize.x, dc.fPos.y + dc.fSize.y), ImVec2(242, 61), ImVec2(1897, 1922));
...
void Viewer::drawImGui()
{
        ...
        if (ImGui::Begin("Tools", &fShowImGuiDebugWindow, ImGuiWindowFlags_AlwaysVerticalScrollbar)) {
            if (ImGui::CollapsingHeader("Backend")) {
...
#if defined(SK_GRAPHITE)
                ImGui::SameLine();
                ImGui::RadioButton("Dawn (Graphite)", &newBackend, sk_app::Window::kGraphiteDawn_BackendType);
#endif
#endif
...
#if defined(SK_GRAPHITE)
                ImGui::SameLine();
                ImGui::RadioButton("Vulkan (Graphite)", &newBackend, sk_app::Window::kGraphiteVulkan_BackendType);
#endif
#endif
#if defined(SK_METAL)
                ImGui::SameLine();
...
#if defined(SK_GRAPHITE)
                ImGui::SameLine();
                ImGui::RadioButton("Metal (Graphite)", &newBackend, sk_app::Window::kGraphiteMetal_BackendType);
#endif
#endif
                ...
                if (newBackend != fBackendType) {
                    fDeferredActions.push_back([newBackend, this]() { this->setBackend(static_cast<sk_app::Window::BackendType>(newBackend)); });
                }
                ...
                if (ctx) {
                    ...
                    int maxMSAA = (fBackendType != sk_app::Window::kRaster_BackendType) ? ctx->maxSurfaceSampleCountForColorType(kRGBA_8888_SkColorType) : 1;
                ...
                if (ImGui::Combo("Pixel Geometry", &pixelGeometryIdx, "Default\0Flat\0RGB\0BGR\0RGBV\0BGRV\0\0")) {
                    uint32_t flags = params.fSurfaceProps.flags();
                ...
                if (ImGui::TreeNode("Path Renderers")) {
                    ...
                    if (is_graphite_backend_type(fBackendType) && gctx) {
                        ...
                        skgpu::graphite::ContextOptionsPriv* opts = &params.fGraphiteTestOptions.fPriv;
                        auto prevPrs = opts->fPathRendererStrategy;
                        auto prsButton = [&](skgpu::graphite::PathRendererStrategy s) {
                            if (ImGui::RadioButton(get_path_renderer_strategy_string(s), prevPrs == s)) {
                        ...
                        auto prButton = [&](GpuPathRenderers x) {
                            if (ImGui::RadioButton(gGaneshPathRendererNames[x].c_str(), prevPr == x)) {
            ...
            if (ImGui::CollapsingHeader("Paint")) {
                auto paintFlag = [this, &uiParamsChanged](const char* label, const char* items, bool SkPaintFields::*flag, bool (SkPaint::*isFlag)() const,
                ...
                paintFlag("Antialias", "Default\0No AA\0AA\0\0", &SkPaintFields::fAntiAlias, &SkPaint::isAntiAlias, &SkPaint::setAntiAlias);

                paintFlag("Dither", "Default\0No Dither\0Dither\0\0", &SkPaintFields::fDither, &SkPaint::isDither, &SkPaint::setDither);
            ...
            if (ImGui::CollapsingHeader("Font")) {
                ...
                auto fontFlag = [this, &uiParamsChanged](const char* label, const char* items, bool SkFontFields::*flag, bool (SkFont::*isFlag)() const,
                ...
                fontFlag("Fake Bold Glyphs", "Default\0No Fake Bold\0Fake Bold\0\0", &SkFontFields::fEmbolden, &SkFont::isEmbolden, &SkFont::setEmbolden);

                fontFlag("Baseline Snapping", "Default\0No Baseline Snapping\0Baseline Snapping\0\0", &SkFontFields::fBaselineSnap, &SkFont::isBaselineSnap,
                    &SkFont::setBaselineSnap);

                fontFlag("Linear Text", "Default\0No Linear Text\0Linear Text\0\0", &SkFontFields::fLinearMetrics, &SkFont::isLinearMetrics,
                    &SkFont::setLinearMetrics);

                fontFlag(
                    "Subpixel Position Glyphs", "Default\0Pixel Text\0Subpixel Text\0\0", &SkFontFields::fSubpixel, &SkFont::isSubpixel, &SkFont::setSubpixel);

                fontFlag("Embedded Bitmap Text", "Default\0No Embedded Bitmaps\0Embedded Bitmaps\0\0", &SkFontFields::fEmbeddedBitmaps,
                    &SkFont::isEmbeddedBitmaps, &SkFont::setEmbeddedBitmaps);

                fontFlag("Force Auto-Hinting", "Default\0No Force Auto-Hinting\0Force Auto-Hinting\0\0", &SkFontFields::fForceAutoHinting,
                ...
                if (ImGui::Combo("Edging", &edgingIdx, "Default\0Alias\0Antialias\0Subpixel Antialias\0\0")) {
                    if (edgingIdx == 0) {
                        fFontOverrides.fEdging = false;
                        fFont.setEdging(SkFont::Edging::kAlias);
                    } else {
                ...
                if (fFontOverrides.fSize) {
                    ImGui::DragFloat2("TextRange", fFontOverrides.fSizeRange, 0.001f, -10.0f, 300.0f, "%.6f", ImGuiSliderFlags_Logarithmic);
                    float textSize = fFont.getSize();
                    if (ImGui::DragFloat(
                            "TextSize", &textSize, 0.001f, fFontOverrides.fSizeRange[0], fFontOverrides.fSizeRange[1], "%.6f", ImGuiSliderFlags_Logarithmic)) {
            ...
            if (ImGui::CollapsingHeader("Slide")) {
                ...
                if (ImGui::ListBox("", &filteredIndex, filteredSlideNames.begin(), filteredSlideNames.size(), 20)) {
            ...
            if (ImGui::CollapsingHeader("Shaders")) {
                bool sksl = params.fGrContextOptions.fShaderCacheStrategy == GrContextOptions::ShaderCacheStrategy::kSkSL;
                ...
                if (gLoadPending) {
                    ...
                    if (ctx) {
                        fPersistentCache.foreach ([this](sk_sp<const SkData> key, sk_sp<SkData> data, const SkString& description, int hitCount) {
                            ...
                            GrPersistentCacheUtils::UnpackCachedShaders(&reader, entry.fShader, entry.fInterfaces, kGrShaderTypeCount);
                        });
                    }
#if defined(SK_GRAPHITE)
                    if (skgpu::graphite::Context* gctx = fWindow->graphiteContext()) {
                        int index = 1;
                        auto callback = [&](const skgpu::UniqueKey& key, const skgpu::graphite::GraphicsPipeline* pipeline) {
                            // Retrieve the shaders from the pipeline.
                            const skgpu::graphite::GraphicsPipeline::PipelineInfo& pipelineInfo = pipeline->getPipelineInfo();

                            CachedShader& entry(fCachedShaders.push_back());
                            entry.fKey = nullptr;
                            entry.fKeyString = SkStringPrintf("#%-3d %s", index++, pipelineInfo.fLabel.c_str());

                            if (sksl) {
                                entry.fShader[kVertex_GrShaderType] = pipelineInfo.fSkSLVertexShader;
                                entry.fShader[kFragment_GrShaderType] = pipelineInfo.fSkSLFragmentShader;
                                entry.fShaderType = SkSetFourByteTag('S', 'K', 'S', 'L');
                            } else {
                                entry.fShader[kVertex_GrShaderType] = pipelineInfo.fNativeVertexShader;
                                entry.fShader[kFragment_GrShaderType] = pipelineInfo.fNativeFragmentShader;
                ...
                if (ctx) {
                    ...
                    doApply = ImGui::Button("Apply Changes");
                    ImGui::SameLine();
                ...
                if (doView || doDump || newOptLevel != fOptLevel) {
                    ...
                    params.fGrContextOptions.fShaderCacheStrategy
                        = sksl ? GrContextOptions::ShaderCacheStrategy::kSkSL : GrContextOptions::ShaderCacheStrategy::kBackendSource;
                    displayParamsChanged = true;
                ...
                if (ctx && doApply) {
                    ...
                    for (auto& entry : fCachedShaders) {
                        ...
                        auto data = GrPersistentCacheUtils::PackCachedShaders(entry.fShaderType, entry.fShader, entry.fInterfaces, kGrShaderTypeCount);
    ...
    if (gShaderErrorHandler.fErrors.size()) {
        ...
        for (int i = 0; i < gShaderErrorHandler.fErrors.size(); ++i) {
            ...
            SkShaderUtils::VisitLineByLine(sksl, [](int lineNumber, const char* lineText) { ImGui::TextWrapped("%4i\t%s\n", lineNumber, lineText); });
        }
        ImGui::End();
    ...
    if (fShowZoomWindow && fLastImage) {
        ...
        if (ImGui::Begin("Zoom", &fShowZoomWindow)) {
            ...
            if (is_graphite_backend_type(fBackendType)) {
                ...
                didGraphiteRead = as_IB(fLastImage)->readPixelsGraphite(fWindow->graphiteRecorder(), pixels, xInt, yInt);
                pixel = *pixels.addr32();
                ImGui::SameLine();
                ImGui::Text(
                    "(X, Y): %d, %d RGBA: %X %X %X %X", xInt, yInt, SkGetPackedR32(pixel), SkGetPackedG32(pixel), SkGetPackedB32(pixel), SkGetPackedA32(pixel));
#endif
            }
            ...
            fImGuiLayer.skiaWidget(avail, [=, lastImage = fLastImage](SkCanvas* c) {
                ...
                c->translate(avail.x * 0.5f / zoomFactor - x - 0.5f, avail.y * 0.5f / zoomFactor - y - 0.5f);
    ...
    if (fShowHistogramWindow && fLastImage) {
        ...
        if (ImGui::Begin("Color Histogram (R,G,B)", &fShowHistogramWindow)) {
            ...
            if (fLastImage->readPixels(fWindow->directContext(), info, pixmap.writable_addr(), info.minRowBytes(), 0, 0)) {
...
void Viewer::dumpShadersToResources()
{
    ...
    SkString directory = SkStringPrintf("%ssksl/%s", GetResourcePath().c_str(), fSlides[fCurrentSlide]->getName().c_str());
...
void Viewer::onIdle()
{
    ...
    if (animateWantsInval || fStatsLayer.getActive() || fRefresh || io.MetricsActiveWindows > 1 || io.MetricsRenderWindows > 0) {
        fWindow->inval();
    }
}

template <typename OptionsFunc> static void WriteStateObject(SkJSONWriter& writer, const char* name, const char* value, OptionsFunc&& optionsFunc)
{
    writer.beginObject();
    {
...
void Viewer::updateUIState()
{
    ...
    // Slide state
    WriteStateObject(writer, kSlideStateName, fSlides[fCurrentSlide]->getName().c_str(), [this](SkJSONWriter& writer) {
        for (const auto& slide : fSlides) {
    ...
    // Backend state
    WriteStateObject(writer, kBackendStateName, get_backend_string(fBackendType), [](SkJSONWriter& writer) {
        for (size_t i = 0; i < kSupportedBackendTypeCount; ++i) {
    ...
    // Path renderer state
    GpuPathRenderers pr = fWindow->getRequestedDisplayParams().fGrContextOptions.fGpuPathRenderers;
    WriteStateObject(writer, kPathRendererStateName, gGaneshPathRendererNames[pr].c_str(), [this](SkJSONWriter& writer) {
