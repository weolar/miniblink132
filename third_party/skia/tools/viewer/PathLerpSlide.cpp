...
private:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        const auto draw_vertices = [this](SkCanvas* canvas, const SkPath& path, float opacity) {
            ...
            const float vertex_radius = 6 / fPathTransform.getScaleX(), ctrl_radius = 4 / fPathTransform.getScaleX();
    ...
    bool animate(double nanos) override
    {
        ...
        if (fDraggingProgress) {
            ...
            const float clock_progress_signed = std::fmod((nanos - fTimeBase) * 0.000000001 * fAnimationSpeed, 2) - 1;
            // what progress should be to match the slidebar
            const float clock_progress_adjusted = std::copysign(fCurrentProgress, clock_progress_signed);
            ...
            fCurrentProgress = std::abs((std::fmod((nanos - fTimeBase) * 0.000000001 * fAnimationSpeed, 2) - 1));
        }

        // Interpolate with easing
        // TODO: generate the synthetic paths once, in updateAnimatingPaths(), then use
        //       regular interpolation here.
        return generalInterpolate(fPaths.first, fPaths.second, fTimeMapper.computeYFromX(fCurrentProgress), &fInterpolatedPath);
    }
    ...
    void updateAnimatingPaths()
    {
        ...
        const SkRect b0 = p0.computeTightBounds(), b1 = p1.computeTightBounds();
        ...
        const SkRect dst_rect = SkRect::MakeSize(fSize).makeInset(fSize.width() * .05f, fSize.height() * .05f);
        fPathTransform = SkMatrix::MakeRectToRect(kNormRect, dst_rect, SkMatrix::kCenter_ScaleToFit);
    }
    ...
    void drawControls()
    {
        ...
        if (ImGui::Begin("Progress Slider", nullptr,
