...
public:
    CanvasLineSegment(SkRandom* random, const LineSegmentParams& params)
    {
        ...
        static constexpr SkColor kColors[] = { 0xffe01040, 0xff10c030, 0xff744cba, 0xffe05010 };
        ...
        fStart = params.fCircleCenters[circle] + SkPoint::Make(params.fCircleRadius * fCosTheta, params.fCircleRadius * fSinTheta);
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        SkPoint end = { fStart.fX + fSegmentDirection * fLength * fCosTheta, fStart.fY + fSegmentDirection * fLength * fSinTheta };
...
public:
        ...
        : Stage(size, /*startingObjectCount=*/5000, /*objectIncrement*/ 1000)
    {
        ...
        fParams.fCircleRadius = fSize.fWidth / 8 - .4 * (fParams.fLineMinimum + fParams.fLineLengthMaximum);
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        SkColor color1Step = SkColorSetARGB(brightnessStep, brightnessStep, (brightnessStep << 1), 102);
        SkColor color2Step = SkColorSetARGB((brightnessStep << 1), (brightnessStep << 1), brightnessStep, 102);
        SkPoint pts[2] = { { fHalfSize.fWidth + dx, fHalfSize.fHeight + dy }, { fHalfSize.fWidth - dx, fHalfSize.fHeight - dy } };
        SkColor colors[] = { color1Step, color1Step, color2Step, color2Step };
        float pos[] = { 0, 0.2f + colorStopStep, 0.8f - colorStopStep, 1 };
        sk_sp<SkShader> gradientShader = SkGradientShader::MakeLinear(pts, colors, pos, 4, SkTileMode::kClamp, 0);

        SkPaint paint;
        paint.setAntiAlias(true);
        paint.setStrokeWidth(15);
        for (int i = 0; i < 4; i++) {
            const SkColor strokeColors[] = { 0xffe01040, 0xff10c030, 0xff744cba, 0xffe05010 };
            const SkColor fillColors[] = { 0xff70051d, 0xff016112, 0xff2F0C6E, 0xff702701 };
            paint.setColor(strokeColors[i]);
            paint.setStyle(SkPaint::kStroke_Style);
            SkRect arcRect = SkRect::MakeXYWH(fParams.fCircleCenters[i].fX - fParams.fCircleRadius, fParams.fCircleCenters[i].fY - fParams.fCircleRadius,
                2 * fParams.fCircleRadius, 2 * fParams.fCircleRadius);
    ...
    bool animate(double nanos) override
    {
        ...
        fCurrentGradientStep = 0.5f + 0.5f * std::sin(time_fractional_value(nanos, 5000) * SK_ScalarPI * 2);
...
public:
    CanvasArc(SkRandom* random, SkSize size)
    {
        ...
        const SkColor baseColors[3] = { 0xff101010, 0xff808080, 0xffc0c0c0 };
        const SkColor bonusColors[3] = { 0xffe01040, 0xff10c030, 0xffe05010 };
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        paint.setAntiAlias(true);
        paint.setColor(fColor);
        SkRect arcRect = SkRect::MakeXYWH(fPoint.fX - fRadius, fPoint.fY - fRadius, 2 * fRadius, 2 * fRadius);
...
protected:
    void setEndPoint(SkRandom* random, SkSize size, SkPoint* prevCoord)
    {
        ...
        const SkPoint kOffsets[4] = { { -4, 0 }, { 2, 0 }, { 1, -2 }, { 1, 2 } };
        ...
        fPoint
            = SkPoint::Make((coordinate.fX + 0.5f) * size.width() / (kGridSize.width() + 1), (coordinate.fY + 0.5f) * size.height() / (kGridSize.height() + 1));
...
public:
    ...
    {
        const SkColor kColors[7] = { 0xff101010, 0xff808080, 0xffc0c0c0, 0xff101010, 0xff808080, 0xffc0c0c0, 0xffe01040 };
...
{
    return { (float)random->nextRangeU(0, maxPosition.width()), (float)random->nextRangeU(0, maxPosition.height()) };
}
...
public:
    BouncingParticle(SkRandom* random, SkSize stageSize, SkSize particleSize, float maxVelocity)
        ...
        , fSize(particleSize)
        , fPosition(random_position(random, { stageSize.fWidth - particleSize.fWidth, stageSize.fHeight - particleSize.fHeight }))
...
public:
    ...
    void initImages(SkCanvas* canvas)
    {
        ...
        for (int i = 0; i < kImageCount; ++i) {
            auto lazyYUV = sk_gpu_test::LazyYUVImage::Make(GetResourceAsData(kImageSrcs[i]), skgpu::Mipmapped::kYes);
            SkASSERT(lazyYUV);
#if defined(SK_GRAPHITE)
            if (recorder) {
                fImages[i] = lazyYUV->refImage(recorder, sk_gpu_test::LazyYUVImage::Type::kFromPixmaps);
            } else
#endif
            {
                fImages[i] = lazyYUV->refImage(rContext, sk_gpu_test::LazyYUVImage::Type::kFromPixmaps);
            }
        }
    }
...
public:
    ...
    {
        fName = "MotionMarkBouncingTaggedImages";
    }
