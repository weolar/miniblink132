...
void SkSLSlide::load(SkScalar winWidth, SkScalar winHeight)
{
    ...
    shader = ToolUtils::GetResourceAsImage("images/mandrill_256.png")->makeShader(SkSamplingOptions());
...
void SkSLSlide::draw(SkCanvas* canvas)
{
    ...
    if (ImGui::InputTextMultiline("Code", fSkSL.data(), fSkSL.size() + 1, boxSize, flags, InputTextCallback, &fSkSL)) {
        fCodeIsDirty = true;
    }
    ...
    for (const SkRuntimeEffect::Uniform& v : fEffect->uniforms()) {
        ...
        case SkRuntimeEffect::Uniform::Type::kFloat4: {
            ...
            for (int c = 0; c < v.count; ++c, f += rows) {
                SkString name = v.isArray() ? SkStringPrintf("%.*s[%d]", (int)v.name.size(), v.name.data(), c) : SkString(v.name);
                ImGui::PushID(c);
        ...
        case SkRuntimeEffect::Uniform::Type::kFloat4x4: {
            ...
            for (int e = 0; e < v.count; ++e) {
                for (int c = 0; c < cols; ++c, f += rows) {
                    SkString name = v.isArray() ? SkStringPrintf("%.*s[%d][%d]", (int)v.name.size(), v.name.data(), e, c)
    ...
    for (const SkRuntimeEffect::Child& c : fEffect->children()) {
        auto curShader = std::find_if(
            fShaders.begin(), fShaders.end(), [tgt = fChildren[c.index]](const std::pair<const char*, sk_sp<SkShader>>& p) { return p.second == tgt; });
    ...
    if (writeTrace || writeDump) {
        ...
        SkRuntimeEffect::TracedShader traced = SkRuntimeEffect::MakeTraced(std::move(shader), traceCoord);
    ...
    case kText: {
        ...
        canvas->drawSimpleText("Hello World", strlen("Hello World"), SkTextEncoding::kUTF8, 0, 256, font, p);
    } break;
    default:
        break;
    }
