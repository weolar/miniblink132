...
public:
    ...
    void load(SkScalar w, SkScalar h) override
    {
        ...
        for (int32_t i = 0; i < kNumBlobs; ++i) {
            ...
            ToolUtils::add_to_text_blob_w_len(&builder, (const char*)glyphs, kWordLength * 4, SkTextEncoding::kUTF32, font, 0, 0);
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        if (fAfterFirstFrame) {
            ...
            if (direct) {
                sk_sp<SkImage> image = direct->priv().testingOnly_getFontAtlasImage(MaskFormat::kA8, 0);
                canvas->drawImageRect(image, SkRect::MakeXYWH(10.0f, 10.0f, 512.0f, 512.0), SkSamplingOptions(), &paint);
                image = direct->priv().testingOnly_getFontAtlasImage(MaskFormat::kA8, 1);
                canvas->drawImageRect(image, SkRect::MakeXYWH(522.0f, 10.0f, 512.f, 512.0f), SkSamplingOptions(), &paint);
                image = direct->priv().testingOnly_getFontAtlasImage(MaskFormat::kA8, 2);
                canvas->drawImageRect(image, SkRect::MakeXYWH(10.0f, 522.0f, 512.0f, 512.0f), SkSamplingOptions(), &paint);
                image = direct->priv().testingOnly_getFontAtlasImage(MaskFormat::kA8, 3);
                canvas->drawImageRect(image, SkRect::MakeXYWH(522.0f, 522.0f, 512.0f, 512.0f), SkSamplingOptions(), &paint);
            }
#endif
        }
    ...
    void load(SkScalar w, SkScalar h) override
    {
        ...
        for (int32_t i = 0; i < kNumBlobs; ++i) {
            ...
            while (paragraphLength - 45 > 0) {
                ...
                ToolUtils::add_to_text_blob_w_len(&builder, (const char*)glyphs, currentLineLength * 4, SkTextEncoding::kUTF32, font, 0, y);
