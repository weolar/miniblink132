...
public:
    ...
    void load(SkScalar w, SkScalar h) override
    {
        ...
        if (data) {
            ...
            SkDebugf("make: dur:%g time%g state:%d", fPlayer->duration(), fPlayer->time(), (int)fPlayer->state());
        }
    ...
    bool onChar(SkUnichar c) override
    {
        if (c == ' ') {
            ...
            case SkAudioPlayer::State::kPlaying:
                fPlayer->pause();
                break;
            case SkAudioPlayer::State::kPaused:
                fPlayer->play();
                break;
            case SkAudioPlayer::State::kStopped:
                fPlayer->play();
                break;
            }
            return true;
