...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        if (fCircleButton.fEnabled) {
            ...
            if (fCircleButton.fFill) {
                if (fArcButton.fEnabled) {
                    SkPoint center;
                    if (arcCenter(&center)) {
                        r.setLTRB(center.fX - fRadius, center.fY - fRadius, center.fX + fRadius, center.fY + fRadius);
                    }
                }
        ...
        if (fTextButton.fEnabled) {
            ...
            SkTextUtils::GetPath(fText.c_str(), fText.size(), SkTextEncoding::kUTF8, 0, fTextSize, font, &path);
            setForText();
...
#ifdef SK_DEBUG
        draw_control(canvas, fErrorControl, gDebugStrokerError, kStrokerErrorMin, kStrokerErrorMax, "error");
#endif
        draw_control(canvas, fWidthControl, fWidth * fWidthScale, kWidthMin * fWidthScale, kWidthMax * fWidthScale, "width");
...
protected:
    ...
    bool onClick(Click* click) override
    {
        ...
        if (index < (int)std::size(fPts)) {
            ...
            gDebugStrokerError = std::max(FLT_EPSILON, MapScreenYtoValue(click->fCurr.fY, fErrorControl, kStrokerErrorMin, kStrokerErrorMax));
            gDebugStrokerErrorSet = true;
        }
#endif
        else if (index == (int)std::size(fPts) + 4) {
            fWidth = std::max(FLT_EPSILON, MapScreenYtoValue(click->fCurr.fY, fWidthControl, kWidthMin, kWidthMax));
...
private:
    ...
    void draw_ribs(SkCanvas* canvas, const SkPath& path, SkScalar width, SkColor color)
    {
        ...
        for (SkScalar dist = 0; dist <= total; dist += delta) {
            if (meas.getPosTan(dist, &pos, &tan)) {
                ...
                canvas->drawLine(pos.x() + tan.x(), pos.y() + tan.y(), pos.x() - tan.x(), pos.y() - tan.y(), paint);
                if (0 == index % 10) {
                    SkString label;
                    label.appendS32(index);
                    SkRect dot = SkRect::MakeXYWH(pos.x() - 2, pos.y() - 2, 4, 4);
                    canvas->drawRect(dot, labelP);
                    canvas->drawString(label, pos.x() - tan.x() * 1.25f, pos.y() - tan.y() * 1.25f, font, labelP);
                }
            }
            ++index;
        }
    ...
    void draw_t_divs(SkCanvas* canvas, const SkPath& path, SkScalar width, SkColor color)
    {
        ...
        for (int index = 0; index < ribs; ++index) {
            ...
            if (0 == index % 10) {
                SkString label;
                label.appendS32(index);
                canvas->drawString(label, pos.x() + tan.x() * 1.25f, pos.y() + tan.y() * 1.25f, ToolUtils::DefaultFont(), paint);
            }
        }
    }

    void draw_stroke(SkCanvas* canvas, const SkPath& path, SkScalar width, SkScalar scale, bool drawText)
    {
        if (path.isEmpty()) {
            return;
        }
        ...
        this->setWHZ(
            SkScalarCeilToInt(bounds.right()), drawText ? SkScalarRoundToInt(scale * 3 / 2) : SkScalarRoundToInt(scale), SkScalarRoundToInt(950.0f / scale));
        erase(fMinSurface);
        SkPaint paint;
    ...
    void draw_fill(SkCanvas* canvas, const SkRect& rect, SkScalar width)
    {
        ...
        path.addCircle(center.fX, center.fY, maxSide + width / 2);
        SkRect outside = SkRect::MakeXYWH(center.fX - maxSide - width, center.fY - maxSide - width, (maxSide + width) * 2, (maxSide + width) * 2);
    ...
    void draw_button(SkCanvas* canvas, const StrokeTypeButton& button)
    {
        ...
        font.setSize(25.0f);
        SkTextUtils::Draw(
            canvas, &button.fLabel, 1, SkTextEncoding::kUTF8, button.fBounds.centerX(), button.fBounds.fBottom - 5, font, paint, SkTextUtils::kCenter_Align);
    }

    void draw_control(SkCanvas* canvas, const SkRect& bounds, SkScalar value, SkScalar min, SkScalar max, const char* name)
    {
        SkPaint paint;
        paint.setAntiAlias(true);
        paint.setStyle(SkPaint::kStroke_Style);
    ...
    bool arcCenter(SkPoint* center)
    {
        ...
        afterCCW += pts[2];
        *center = beforeCCW;
        if (SkScalarNearlyEqual(beforeCCW.fX, afterCCW.fX) && SkScalarNearlyEqual(beforeCCW.fY, afterCCW.fY)) {
            return true;
        }
        ...
        afterCW += pts[2];
        *center = beforeCW;
        return SkScalarNearlyEqual(beforeCW.fX, afterCW.fX) && SkScalarNearlyEqual(beforeCCW.fY, afterCW.fY);
    }
