...
void PathTextSlide::Glyph::reset(SkRandom& rand, int w, int h)
{
    ...
    fZoom = ((1 - t) * screensize / 50 + t * screensize / 3) / std::max(bounds.width(), bounds.height());
...
public:
    ...
    bool animate(double nanos, int screenWidth, int screenHeight) final
    {
        ...
        fBackgroundAnimationTask.add(std::bind(&MovingGlyphAnimator::runAnimationTask, this, tsec, dt, screenWidth, screenHeight));
...
public:
    ...
    void runAnimationTask(double t, double dt, int width, int height) override
    {
        ...
        for (int i = 0; i < kNumPaths; ++i) {
            ...
            const skvx::float2 matrix[3] = { skvx::float2(backMatrix.getScaleX(), backMatrix.getSkewY()),
                skvx::float2(backMatrix.getSkewX(), backMatrix.getScaleY()), skvx::float2(backMatrix.getTranslateX(), backMatrix.getTranslateY()) };
...
SkPoint PathTextSlide::WavyGlyphAnimator::Waves::apply(float tsec, const skvx::float2 matrix[3], const SkPoint& pt) const
{
    ...
    const skvx::float4 t = abs(frequencies * (dirsX * devicePt[0] + dirsY * devicePt[1]) + speeds * tsec + offsets) * (float(kTablePeriod) / SK_ScalarPI);
    ...
    const skvx::float4 left(sin2table[indices[0]], sin2table[indices[1]], sin2table[indices[2]], sin2table[indices[3]]);
    const skvx::float4 right(sin2table[indices[0] + 1], sin2table[indices[1] + 1], sin2table[indices[2] + 1], sin2table[indices[3] + 1]);
