...
struct FilterNode {
    ...
    FilterNode(const SkImageFilter* filter, const skif::Mapping& mapping, const skif::ParameterSpace<SkRect>& content, int depth)
...
private:
    void computeOutputBounds()
    {
        if (fFilter) {
            ...
            std::optional<skif::DeviceSpace<SkIRect>> pseudoDeviceBounds = as_IFB(fFilter)->getOutputBounds(layerOnly, fContent);
    ...
    void computeInputBounds()
    {
        ...
        skif::DeviceSpace<SkIRect> targetOutput(fMapping.totalMatrix().mapRect(SkRect(fContent)).roundOut());
...
static void draw_node(SkCanvas* canvas, const FilterNode& node)
{
    ...
    SkPoint points[2] = { { content.fLeft + 15.f, content.fTop + 15.f }, { content.fRight - 15.f, content.fBottom - 15.f } };
    paint.setShader(SkGradientShader::MakeLinear(points, kColors, nullptr, std::size(kColors), SkTileMode::kRepeat));
...
static float print_matrix(SkCanvas* canvas, const char* prefix, const SkMatrix& matrix, float x, float y, const SkFont& font, const SkPaint& paint)
{
    ...
    for (int i = 0; i < 3; ++i) {
        SkString row;
        row.appendf("[%.2f %.2f %.2f]", matrix.get(i * 3), matrix.get(i * 3 + 1), matrix.get(i * 3 + 2));
...
static float print_info(SkCanvas* canvas, const FilterNode& node)
{
    ...
    if (node.fFilter) {
        ...
        if (node.fDepth == 0) {
            ...
            y = print_matrix(canvas, "Param->Layer", node.fMapping.layerMatrix(), kLineInset, y, font, text);
            y = print_matrix(canvas, "Layer->Device", node.fMapping.layerToDevice(), kLineInset, y, font, text);
        }

        y = print_size(canvas, "Layer Size", SkIRect(node.fUnhintedLayerBounds), kLineInset, y, font, text);
        y = print_size(canvas, "Layer Size (hinted)", SkIRect(node.fHintedLayerBounds), kLineInset, y, font, text);
    } else {
...
static void draw_dag(SkCanvas* canvas, SkImageFilter* filter, const SkRect& rect, const SkISize& surfaceSize)
{
    ...
    sk_sp<SkSurface> nodeSurface = canvas->makeSurface(canvas->imageInfo().makeDimensions(surfaceSize));
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        // Merge <
        //        \--- Blur <--- Drop Shadow
        sk_sp<SkImageFilter> drop2 = SkImageFilters::DropShadow(10.f, 5.f, 3.f, 3.f, SK_ColorBLACK, nullptr);
        ...
        sk_sp<SkImageFilter> cf1 = SkImageFilters::ColorFilter(SkColorFilters::Blend(SK_ColorGRAY, SkBlendMode::kModulate), std::move(blur2));
    ...
    {
        return kFilterSurfaceSize;
    }

private:
    static constexpr SkRect kFilterRect = SkRect::MakeXYWH(20.f, 20.f, 60.f, 60.f);
    static constexpr SkISize kFilterSurfaceSize = SkISize::Make(2 * (kFilterRect.fRight + kFilterRect.fLeft), 2 * (kFilterRect.fBottom + kFilterRect.fTop));
};
