...
public:
    ...
    virtual void updateVertices(SkSpan<const SkPoint> vert_pos, SkSpan<const SkColor4f> vert_colors) = 0;
};
...
public:
    ...
    sk_sp<SkShader> asShader() const override
    {
        return fShader;
    }

    void updateVertices(SkSpan<const SkPoint> vert_pos, SkSpan<const SkColor4f> vert_colors) override
    {
...
private:
    void buildEffect(size_t vert_count)
    {
        ...
        const auto res = SkRuntimeEffect::MakeForShader(SkStringPrintf(gAEGradientSkSL, vert_count, vert_count, vert_count));
        if (!res.effect) {
            SkDEBUGF("%s\n", res.errorText.c_str());
        }
...
static constexpr struct RendererChoice {
    ...
    { "AfterEffects Gradient", []() -> sk_sp<GradientRenderer> { return sk_make_sp<AEGradientRenderer>(); } },
};
...
static constexpr struct VertexAnimator {
...
} gVertexAnimators[] = {
    {
        ...
        [](SkSpan<const SkPoint> uvs, SkSpan<SkPoint> pos, float t) {
            ...
            for (size_t i = 0; i < uvs.size(); ++i) {
                const float phase = i * SK_FloatPI * 0.31f, angle = phase + t * SK_FloatPI * 2;
                pos[i] = uvs[i]
                    + SkVector {
    ...
    },
    {
        ...
        [](SkSpan<const SkPoint> uvs, SkSpan<SkPoint> pos, float t) {
            ...
            for (size_t i = 0; i < uvs.size(); ++i) {
                ...
                const auto arc_len = uvs[i].fX - cyl_pos, arc_ang = arc_len / kCylRadius;
    ...
    {
        "None",
        [](SkSpan<const SkPoint> uvs, SkSpan<SkPoint> pos, float t) { memcpy(pos.data(), uvs.data(), uvs.size() * sizeof(SkPoint)); },
    },
};
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        canvas->translate((fSize.fWidth - mesh_size) * 0.5f, (fSize.fHeight - mesh_size) * 0.5f);
    ...
    bool animate(double nanos) override
    {
        // Mesh count animation
        {
            ...
            this->updateMesh(lerp(fVertexCountFrom, fVertexCountTo, fVertedCountTimeMapper.computeYFromX(t)));
        }
        ...
        // Vertex animation
        {
            ...
            const float t = std::abs((std::fmod((nanos - fTimeBase) * 0.000000001 * fAnimationSpeed, 2) - 1));
...
private:
    void updateMesh(size_t new_count)
    {
        ...
        for (size_t i = current_count; i < new_count; ++i) {
            const SkPoint uv = i < std::size(gFixedVertices) ? gFixedVertices[i].fUv : SkPoint { fRNG.nextF(), fRNG.nextF() };
            const SkColor4f color = i < std::size(gFixedVertices) ? gFixedVertices[i].fColor : SkColor4f { fRNG.nextF(), fRNG.nextF(), fRNG.nextF(), 1.f };
        ...
        if (new_count < current_count) {
            ...
            const size_t rand_vertex_count = std::max(new_count, std::size(gFixedVertices)) - std::size(gFixedVertices);
            for (size_t i = 0; i < rand_vertex_count * kRandsPerVertex; ++i) {
                fRNG.nextF();
            }
        }
    }
    ...
    // Animation state
    const SkCubicMap fTimeMapper, fVertedCountTimeMapper;
    double fTimeBase = 0, fVertexCountTimeBase = 0;
    int fVertexCountFrom = 0, fVertexCountTo = 5;
