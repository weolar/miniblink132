...
public:
    ...
    struct SkFontFields {
        ...
        {
            return fTypeface || fSize || fScaleX || fSkewX || fHinting || fEdging || fSubpixel || fForceAutoHinting || fEmbeddedBitmaps || fLinearMetrics
                || fEmbolden || fBaselineSnap;
        }
    ...
    struct SkPaintFields {
        bool overridesSomething() const
        {
            return fPathEffect || fShader || fMaskFilter || fColorFilter || fImageFilter || fColor || fStrokeWidth || fMiterLimit || fBlendMode || fAntiAlias
                || fDither || fForceRuntimeBlend || fCapType || fJoinType || fStyle;
        }
