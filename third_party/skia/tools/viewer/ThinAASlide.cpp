...
public:
    ...
    void draw(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) override
    {
        ...
        canvas->drawRect(SkRect::MakeLTRB(kTileWidth / 2.f - width / 2.f, 2.f, kTileWidth / 2.f + width / 2.f, kTileHeight - 2.f), *paint);
    }
...
public:
    ...
    void draw(SkCanvas* canvas, SkPaint* paint, SkScalar tx, SkScalar ty, SkScalar angle) override
    {
        ...
        if (fDepth > 0.f) {
            path.quadTo(kTileWidth / 2.f + fDepth, kTileHeight / 2.f, kTileWidth / 2.f, kTileHeight - 2.f);
        } else {
            if (fDepth < 0.f) {
...
public:
    ...
    static sk_sp<OffscreenShapeRenderer> Make(sk_sp<ShapeRenderer> renderer, int supersample, bool forceRaster = false)
    {
        ...
        return sk_sp<OffscreenShapeRenderer>(new OffscreenShapeRenderer(std::move(renderer), supersample, forceRaster));
    }
    ...
    {
        auto info = SkImageInfo::Make(fSupersampleFactor * kTileWidth, fSupersampleFactor * kTileHeight, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    void redraw(SkCanvas* canvas, SkScalar scale = 1.f, bool debugMode = false)
    {
        ...
        if (debugMode) {
            ...
            static constexpr float kFilter[]
                = { 0.f, 0.f, 0.f, 0.f, 16.f / 255, 0.f, 0.f, 0.f, 0.f, 200.f / 255, 0.f, 0.f, 0.f, 0.f, 16.f / 255, 0.f, 0.f, 0.f, 255.f, 0.f };

            blit.setColorFilter(SkColorFilters::Matrix(kFilter));
        }

        auto sampling = scale > 1 ? SkSamplingOptions(SkFilterMode::kNearest) : SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kLinear);

        canvas->scale(scale, scale);
        canvas->drawImageRect(fLastRendered.get(), SkRect::MakeWH(kTileWidth, kTileHeight), SkRect::MakeWH(kTileWidth, kTileHeight), sampling, &blit,
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        canvas->drawString(SkStringPrintf("Stroke width: %.2f ('-' to decrease, '=' to increase)", fStrokeWidth), 0, 24, font, text);
        canvas->drawString(SkStringPrintf("Rotation: %.3f ('r' to animate, 'y' sets to 90, 'u' sets"
                                          " to 0, 'space' adds 15)",
                               fAngle),
            0, 36, font, text);
        canvas->drawString(SkStringPrintf("Translation: %.3f, %.3f ('t' to animate)", fSubpixelX, fSubpixelY), 0, 48, font, text);
    ...
    bool onChar(SkUnichar key) override
    {
        ...
        case ' ':
            ...
            return true;
        case '-':
            fStrokeWidth = std::max(0.1f, fStrokeWidth - 0.05f);
            return true;
        case '=':
            fStrokeWidth = std::min(1.f, fStrokeWidth + 0.05f);
            return true;
        }
        return false;
    }
...
private:
    ...
    void drawShapes(SkCanvas* canvas, const char* name, int gridX, TArray<sk_sp<OffscreenShapeRenderer>> shapes)
    {
    ...
    void drawShape(SkCanvas* canvas, const char* name, int gridX, OffscreenShapeRenderer* shape, bool drawNameLabels)
    {
        static constexpr SkScalar kZoomGridWidth = 8 * ShapeRenderer::kTileWidth + 8.f;
        static constexpr SkRect kTile = SkRect::MakeWH(ShapeRenderer::kTileWidth, ShapeRenderer::kTileHeight);
        static constexpr SkRect kZoomTile = SkRect::MakeWH(8 * ShapeRenderer::kTileWidth, 8 * ShapeRenderer::kTileHeight);
