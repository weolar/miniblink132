...
{
    return std::fabs(x0 - x1) <= MAX_JITTER_RADIUS && std::fabs(y0 - y1) <= MAX_JITTER_RADIUS;
}
...
void TouchGesture::touchMoved(void* owner, float x, float y)
{
    ...
    case 2: {
        ...
        this->updateZoom(
            scale, center(rec0.fStartX, rec1.fStartX), center(rec0.fStartY, rec1.fStartY), center(rec0.fLastX, rec1.fLastX), center(rec0.fLastY, rec1.fLastY));
    } break;
    default:
        break;
    }
...
void TouchGesture::setTransLimit(const SkRect& contentRect, const SkRect& windowRect, const SkMatrix& preTouchMatrix)
{
