...
void FatBits::drawFG(SkCanvas* canvas)
{
    ...
    if (fUseClip) {
        ...
        SkRect r = { fClipRect.fLeft * fZoom, fClipRect.fTop * fZoom, fClipRect.fRight * fZoom, fClipRect.fBottom * fZoom };
        canvas->drawRect(r, p);
    }
}
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        {
            SkString str;
            str.printf("%s %s %s", fFB.getAA() ? "AA" : "BW", FatBits::kHair_Style == fFB.getStyle() ? "Hair" : "Stroke", fFB.getUseClip() ? "clip" : "noclip");
            SkPaint paint;
