...
struct LocalCornerVert {
    ...
    SkV3 transform(const SkM44& m, const SkV4& cornerMapping, const SkV2& cornerPt, const SkV2& cornerRadii, const SkV4& devCenter, float centerWeight,
        ...
        if (snapToCenter) {
            ...
            if (fStrokeScale < 0.f) {
                ...
                if (maxInset.x < 0.f || maxInset.y < 0.f) {
                    normalizedPos = SkV2 { std::min(maxInset.x, 0.f), std::min(maxInset.y, 0.f) } - cornerRadii;
                } else {
                    normalizedPos += localAARadius * fNormal;
                }
            } // else no normal offsetting, or device-space offsetting

            SkV2 localPos = { cornerMapping.x * normalizedPos.x + cornerMapping.y * normalizedPos.y + cornerPt.x,
            ...
            const bool deviceSpaceNormal = fStrokeScale > 0.f && (fNormal.x > 0.f || fNormal.y > 0.f);
            if (deviceSpaceNormal) {
                SkV2 devNorm;
                {
                    ...
                    SkV4 normX4 = { sx * cornerMapping.w * fNormal.x, -sx * cornerMapping.y * fNormal.x, 0.f, sx * px * fNormal.x };
                    SkV4 normY4 = { -cornerMapping.z * fNormal.y, cornerMapping.x * fNormal.y, 0.f, py * fNormal.y };
...
static void compute_corner(SkV3 devPts[19], const SkM44& m, const SkV4& cornerMapping, const SkV2& cornerPt, const SkV2& cornerRadii, const SkV4& center,
    float centerWeight, float localAARadius, float strokeRadius, SkPaint::Join join)
{
    float joinScale;
    ...
    for (size_t i = 0; i < std::size(kCornerTemplate); ++i) {
        devPts[i] = kCornerTemplate[i].transform(m, cornerMapping, cornerPt, cornerRadii, center, centerWeight, strokeRadius, joinScale, localAARadius);
    }
}
...
static void compute_vertices(SkV3 devPts[kVertexCount], const SkM44& m, const SkRRect& rrect, float strokeRadius, SkPaint::Join join)
{
    ...
    float localAARadius = std::max({ local_aa_radius(m, { rrect.getBounds().fRight, rrect.getBounds().fBottom }),
        local_aa_radius(m, { rrect.getBounds().fRight, rrect.getBounds().fTop }), local_aa_radius(m, { rrect.getBounds().fLeft, rrect.getBounds().fTop }),
    ...
    if (maxInset >= rrect.width() - maxInset || // L/R stroke insets would cross over
        ...
        maxInset >= rrect.width() - rrect.radii(SkRRect::kLowerRight_Corner).fX || maxInset >= rrect.width() - rrect.radii(SkRRect::kUpperLeft_Corner).fX
        || maxInset >= rrect.width() - rrect.radii(SkRRect::kUpperRight_Corner).fX || maxInset >= rrect.height() - rrect.radii(SkRRect::kLowerLeft_Corner).fY
        || // Y corner cross
        maxInset >= rrect.height() - rrect.radii(SkRRect::kLowerRight_Corner).fY || maxInset >= rrect.height() - rrect.radii(SkRRect::kUpperLeft_Corner).fY
    ...
    compute_corner(devPts + kBR, m, kBRBasis, { rrect.getBounds().fRight, rrect.getBounds().fBottom },
        { rrect.radii(SkRRect::kLowerRight_Corner).fX, rrect.radii(SkRRect::kLowerRight_Corner).fY }, devCenter, centerWeight, localAARadius, strokeRadius,
        join);
    compute_corner(devPts + kTR, m, kTRBasis, { rrect.getBounds().fRight, rrect.getBounds().fTop },
        { rrect.radii(SkRRect::kUpperRight_Corner).fY, rrect.radii(SkRRect::kUpperRight_Corner).fX }, devCenter, centerWeight, localAARadius, strokeRadius,
        join);
    compute_corner(devPts + kTL, m, kTLBasis, { rrect.getBounds().fLeft, rrect.getBounds().fTop },
        { rrect.radii(SkRRect::kUpperLeft_Corner).fX, rrect.radii(SkRRect::kUpperLeft_Corner).fY }, devCenter, centerWeight, localAARadius, strokeRadius, join);
    compute_corner(devPts + kBL, m, kBLBasis, { rrect.getBounds().fLeft, rrect.getBounds().fBottom },
        { rrect.radii(SkRRect::kLowerLeft_Corner).fY, rrect.radii(SkRRect::kLowerLeft_Corner).fX }, devCenter, centerWeight, localAARadius, strokeRadius, join);
}

// All indices
static const uint16_t kIndices[] = {
    // Exterior AA ramp outset
    kBR + 0, kBR + 6, kBR + 1, kBR + 7, kBR + 2, kBR + 8, kBR + 3, kBR + 8, kBR + 4, kBR + 9, kBR + 5, kBR + 9, kTR + 0, kTR + 6, kTR + 1, kTR + 7, kTR + 2,
    kTR + 8, kTR + 3, kTR + 8, kTR + 4, kTR + 9, kTR + 5, kTR + 9, kTL + 0, kTL + 6, kTL + 1, kTL + 7, kTL + 2, kTL + 8, kTL + 3, kTL + 8, kTL + 4, kTL + 9,
    kTL + 5, kTL + 9, kBL + 0, kBL + 6, kBL + 1, kBL + 7, kBL + 2, kBL + 8, kBL + 3, kBL + 8, kBL + 4, kBL + 9, kBL + 5, kBL + 9, kBR + 0, kBR + 6,
    kBR + 6, // close and extra vertex to jump to next strip
    // Outer to central curve
    kBR + 6, kBR + 10, kBR + 7, kBR + 11, kBR + 8, kBR + 12, kBR + 9, kBR + 13, kTR + 6, kTR + 10, kTR + 7, kTR + 11, kTR + 8, kTR + 12, kTR + 9, kTR + 13,
    kTL + 6, kTL + 10, kTL + 7, kTL + 11, kTL + 8, kTL + 12, kTL + 9, kTL + 13, kBL + 6, kBL + 10, kBL + 7, kBL + 11, kBL + 8, kBL + 12, kBL + 9, kBL + 13,
    kBR + 6, kBR + 10, kBR + 10, // close and extra vertex to jump to next strip
    // Center to inner curve's insets
    kBR + 10, kBR + 14, kBR + 11, kBR + 15, kBR + 12, kBR + 16, kBR + 13, kBR + 16, kTR + 10, kTR + 14, kTR + 11, kTR + 15, kTR + 12, kTR + 16, kTR + 13,
    kTR + 16, kTL + 10, kTL + 14, kTL + 11, kTL + 15, kTL + 12, kTL + 16, kTL + 13, kTL + 16, kBL + 10, kBL + 14, kBL + 11, kBL + 15, kBL + 12, kBL + 16,
    kBL + 13, kBL + 16, kBR + 10, kBR + 14, kBR + 14, // close and extra vertex to jump to next strip
    // Inner inset to center of shape
    kBR + 14, kBR + 17, kBR + 15, kBR + 17, kBR + 16, kBR + 16, kBR + 18, kTR + 14, kTR + 14, kTR + 17, kTR + 15, kTR + 17, kTR + 16, kTR + 16, kTR + 18,
    kTL + 14, kTL + 14, kTL + 17, kTL + 15, kTL + 17, kTL + 16, kTL + 16, kTL + 18, kBL + 14, kBL + 14, kBL + 17, kBL + 15, kBL + 17, kBL + 16, kBL + 16,
    kBL + 18, kBR + 14 // close
};

// Separated to draw with different colors (vs. duplicating vertices to change colors).
static const uint16_t kOuterCornerIndices[] = { kBR + 0, kBR + 0, kBR + 6, kBR + 1, kBR + 7, kBR + 2, kBR + 8, kBR + 3, kBR + 8, kBR + 4, kBR + 9, kBR + 5,
    kBR + 5, kTR + 0, kTR + 0, kTR + 6, kTR + 1, kTR + 7, kTR + 2, kTR + 8, kTR + 3, kTR + 8, kTR + 4, kTR + 9, kTR + 5, kTR + 5, kTL + 0, kTL + 0, kTL + 6,
    kTL + 1, kTL + 7, kTL + 2, kTL + 8, kTL + 3, kTL + 8, kTL + 4, kTL + 9, kTL + 5, kTL + 5, kBL + 0, kBL + 0, kBL + 6, kBL + 1, kBL + 7, kBL + 2, kBL + 8,
    kBL + 3, kBL + 8, kBL + 4, kBL + 9, kBL + 5, kBL + 5,

    kBR + 6, kBR + 6, kBR + 10, kBR + 7, kBR + 11, kBR + 8, kBR + 12, kBR + 9, kBR + 13, kBR + 13, kTR + 6, kTR + 6, kTR + 10, kTR + 7, kTR + 11, kTR + 8,
    kTR + 12, kTR + 9, kTR + 13, kTR + 13, kTL + 6, kTL + 6, kTL + 10, kTL + 7, kTL + 11, kTL + 8, kTL + 12, kTL + 9, kTL + 13, kTL + 13, kBL + 6, kBL + 6,
    kBL + 10, kBL + 7, kBL + 11, kBL + 8, kBL + 12, kBL + 9, kBL + 13, kBL + 13 };
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        // Axes
        canvas->drawLine({ origin.x / origin.w, origin.y / origin.w }, { xAxis.x / xAxis.w, xAxis.y / xAxis.w }, paint(SK_ColorRED, 0.f));
        canvas->drawLine({ origin.x / origin.w, origin.y / origin.w }, { yAxis.x / yAxis.w, yAxis.y / yAxis.w }, paint(SK_ColorGREEN, 0.f));

        // Control points
        canvas->drawCircle({ origin.x / origin.w, origin.y / origin.w }, kControlPointRadius, paint(SK_ColorBLACK));
        canvas->drawCircle({ xAxis.x / xAxis.w, xAxis.y / xAxis.w }, kControlPointRadius, paint(SK_ColorRED));
        canvas->drawCircle({ yAxis.x / yAxis.w, yAxis.y / yAxis.w }, kControlPointRadius, paint(SK_ColorGREEN));
        canvas->restore();
    }
...
private:
    ...
    enum class PrimitiveMode { kFillRect, kFillRRect, kStrokeRect, kStrokeRRect };
    ...
    SkM44 basisMatrix() const
    {
        ...
        return SkM44::Cols({ xAxis.x, xAxis.y, 0.f, 0.f }, { yAxis.x, yAxis.y, 0.f, 0.f }, { 0.f, 0.f, 1.f, 0.f }, { fOrigin.x, fOrigin.y, 0.f, 1.f });
    }
    ...
    {
        static const SkRect kOuterBounds = SkRect::MakeLTRB(-kBaseScale, -kBaseScale, kBaseScale, kBaseScale);
        // Filled rounded rects can have arbitrary corners
        static const SkVector kOuterRadii[4] = { { 0.25f * kBaseScale, 0.75f * kBaseScale }, { 0.f, 0.f }, { 0.5f * kBaseScale, 0.5f * kBaseScale },
            { 0.75f * kBaseScale, 0.25f * kBaseScale } };
        // // Stroked rounded rects will only have circular corners
        static const SkVector kStrokeRadii[4] = { { 0.25f * kBaseScale, 0.25f * kBaseScale }, { 0.f, 0.f }, { 0.5f * kBaseScale, 0.5f * kBaseScale },
    ...
    void drawVertices(SkCanvas* canvas, const SkM44& ctm)
    {
        ...
        auto drawMeshSubset = [vertices, canvas](SkColor color, const uint16_t* indices, size_t indexCount) {
            sk_sp<SkVertices> mesh
                = SkVertices::MakeCopy(SkVertices::kTriangleStrip_VertexMode, kVertexCount, vertices, nullptr, nullptr, (int)indexCount, indices);
            SkPaint meshPaint;
        ...
        if (fColorize) {
            drawMeshSubset(SK_ColorGRAY, kEdgeIndices, std::size(kEdgeIndices));
            drawMeshSubset(SK_ColorDKGRAY, kInteriorIndices, std::size(kInteriorIndices));
            drawMeshSubset(SK_ColorMAGENTA, kInnerCornerIndices, std::size(kInnerCornerIndices));
            drawMeshSubset(SK_ColorCYAN, kOuterCornerIndices, std::size(kOuterCornerIndices));
        } else {
        ...
        auto drawEdge = [&edges, vertices, canvas](uint16_t e0, uint16_t e1) {
            ...
            if (edges.find(edgeID) == edges.end()) {
                edges.insert(edgeID);
                if (SkScalarNearlyEqual(vertices[e0].fX, vertices[e1].fX) && SkScalarNearlyEqual(vertices[e0].fY, vertices[e1].fY)) {
                    return;
                }
...
ClickHandlerSlide::Click* GraphitePrimitivesSlide::onFindClickHandler(SkScalar x, SkScalar y, skui::ModifierKey)
{
    auto selected = [x, y](const SkV2& p) { return ((p - SkV2 { x, y }).length() < kControlPointRadius); };
