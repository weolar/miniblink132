...
private:
    ...
    sk_sp<SkData> updateSBIXData(SkData* originalData, bool setPts)
    {
        ...
        SkSFNTHeader::TableDirectoryEntry* tableEntry = SkTAfter<SkSFNTHeader::TableDirectoryEntry>(sfntHeader);
        ...
        SkOTTableHorizontalHeader* hheaTable = SkTAddOffset<SkOTTableHorizontalHeader>(sfntHeader, hheaTableOffset);

        size_t hmtxTableOffset = SkEndian_SwapBE32(hmtxTableEntry->offset);
        SkOTTableHorizontalMetrics* hmtxTable = SkTAddOffset<SkOTTableHorizontalMetrics>(sfntHeader, hmtxTableOffset);

        size_t locaTableOffset = SkEndian_SwapBE32(locaTableEntry->offset);
        SkOTTableIndexToLocation* locaTable = SkTAddOffset<SkOTTableIndexToLocation>(sfntHeader, locaTableOffset);

        size_t maxpTableOffset = SkEndian_SwapBE32(maxpTableEntry->offset);
        SkOTTableMaximumProfile* maxpTable = SkTAddOffset<SkOTTableMaximumProfile>(sfntHeader, maxpTableOffset);
        ...
        if (sbixTableEntry) {
            ...
            SkOTTableStandardBitmapGraphics* sbixTable = SkTAddOffset<SkOTTableStandardBitmapGraphics>(sfntHeader, sbixTableOffset);
            ...
            SkOTTableStandardBitmapGraphics::Strike* strike = SkTAddOffset<SkOTTableStandardBitmapGraphics::Strike>(sbixTable, strikeOffset);
            ...
            SkOTTableStandardBitmapGraphics::GlyphData* glyphData = SkTAddOffset<SkOTTableStandardBitmapGraphics::GlyphData>(strike, glyphDataOffset);
            if (setPts) {
                fPts[kOriginOffset].location.set(
                    (int16_t)SkEndian_SwapBE16(glyphData->originOffsetX) / toStrikeEm, (int16_t)SkEndian_SwapBE16(glyphData->originOffsetY) / -toStrikeEm);
            } else {
        ...
        if (glyphData) {
            if (setPts) {
                fPts[kGlyfXYMin].location.set((int16_t)SkEndian_SwapBE16(glyphData->xMin) / toEm, (int16_t)SkEndian_SwapBE16(glyphData->yMin) / -toEm);
                fPts[kGlyfXYMax].location.set((int16_t)SkEndian_SwapBE16(glyphData->xMax) / toEm, (int16_t)SkEndian_SwapBE16(glyphData->yMax) / -toEm);
            } else {
            ...
            if (contourCount > 0) {
                ...
                SkOTTableGlyphData::Simple::Flags* flags = SkTAfter<SkOTTableGlyphData::Simple::Flags>(instructions, SkEndian_SwapBE16(*numInstructions));
        ...
        if (kGlyphID < numberOfFullMetrics) {
            ...
            SkOTTableHorizontalMetrics::ShortMetric* shortMetrics = SkTAfter<SkOTTableHorizontalMetrics::ShortMetric>(fullMetrics, numberOfFullMetrics);
