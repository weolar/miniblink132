...
static SkPoint rotate90(const SkPoint& p)
{
    return { p.fY, -p.fX };
}
static SkPoint rotate180(const SkPoint& p)
{
    return p * -1;
}
...
static bool isClockwise(const SkPoint& a, const SkPoint& b)
{
    return a.cross(b) > 0;
}
...
public:
    ...
    {
        return SkPath::Make(fPoints.data(), fPoints.size(), fVerbs.data(), fVerbs.size(), nullptr, 0, SkPathFillType::kWinding);
    }
    ...
    void close()
    {
        fVerbs.push_back(SkPath::kClose_Verb);
    }
    ...
    const std::vector<uint8_t>& verbs() const
    {
        return fVerbs;
    }
...
void SkPathStroker2::join(const PathSegment& prev, const PathSegment& curr)
{
    const auto miterJoin = [this](const PathSegment& prev, const PathSegment& curr) {
        ...
        const bool needsInnerLoop = squaredLineLength(prev) < maxLenSqd || squaredLineLength(curr) < maxLenSqd;
        if (needsInnerLoop) {
