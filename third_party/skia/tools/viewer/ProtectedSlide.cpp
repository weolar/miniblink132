...
sk_sp<SkSurface> wrap_buffer(GrDirectContext* dContext, Recorder* recorder, AHardwareBuffer* buffer)
{
#if defined(SK_GANESH)
    if (dContext) {
        return SkSurfaces::WrapAndroidHardwareBuffer(dContext, buffer, kTopLeft_GrSurfaceOrigin,
            /* colorSpace= */ nullptr,
...
#if defined(SK_GRAPHITE)
    if (recorder) {
        return SkSurfaces::WrapAndroidHardwareBuffer(recorder, buffer,
            /* colorSpace= */ nullptr,
            /* surfaceProps= */ nullptr);
    }
#endif
...
sk_sp<SkSurface> create_protected_render_target(GrDirectContext* dContext, Recorder* recorder, const SkImageInfo& ii)
{
#if defined(SK_GANESH)
    if (dContext) {
        return SkSurfaces::RenderTarget(dContext, skgpu::Budgeted::kYes, ii,
            /* sampleCount= */ 1, kTopLeft_GrSurfaceOrigin,
            /* surfaceProps= */ nullptr,
    ...
    if (recorder) {
        ...
        return SkSurfaces::RenderTarget(recorder, ii, skgpu::Mipmapped::kNo,
            /* props= */ nullptr);
    }
#endif
...
AHardwareBuffer* create_protected_buffer(int width, int height)
{
    ...
    hwbDesc.layers = 1;
    hwbDesc.usage = AHARDWAREBUFFER_USAGE_CPU_READ_NEVER | AHARDWAREBUFFER_USAGE_CPU_WRITE_NEVER | AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE
...
sk_sp<SkImage> create_protected_AHB_image(GrDirectContext* dContext, Recorder* recorder, AHardwareBuffer* buffer, SkColor color)
{
...
sk_sp<SkImage> create_protected_skia_image(GrDirectContext* dContext, Recorder* recorder, int width, int height, SkColor color)
{
...
public:
    ...
    void draw(SkCanvas* origCanvas) override
    {
        ...
        sk_sp<SkImage> protectedAHBImage = create_protected_AHB_image(dContext, recorder, buffer, SK_ColorRED);
        sk_sp<SkImage> protectedSkImage = create_protected_skia_image(dContext, recorder, kSize, kSize, SK_ColorBLUE);
