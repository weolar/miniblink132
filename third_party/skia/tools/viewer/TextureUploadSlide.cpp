...
public:
            ...
            if (fDrawTexturesToScreen) {
                for (int y = 0; y < fTileRows; y++) {
                    for (int x = 0; x < fTileCols; x++) {
                        ...
                        canvas->drawImage(fTextures[currentIndex]->getImage(), x * fTileSize, y * fTileSize);
                    }
                }
            }
        }
    ...
    bool animate(double nanos) override
    {
        ...
        fActiveTileIndex = floorf(TimeUtils::Scaled(1e-9 * nanos, numTiles / kDesiredDurationSecs, numTiles));
        return true;
    }
...
private:
    ...
    public:
        RenderTargetTexture(GrDirectContext* direct, int size)
        {
            ...
            SkImageInfo imageInfo = SkImageInfo::Make(size, size, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
            fSurface = SkSurfaces::RenderTarget(direct, skgpu::Budgeted::kNo, imageInfo, 0, &surfaceProps);
        }
