...
public:
    explicit Track(std::unique_ptr<SkAudioPlayer> player)
...
public:
    ...
    static std::unique_ptr<Decorator> Make()
    {
...
public:
    ...
    sk_sp<skresources::ImageAsset> loadImageAsset(const char resource_path[], const char resource_name[], const char /*resource_id*/[]) const override
    {
...
public:
    explicit TextTracker(sk_sp<PropertyObserver> delegate)
        : fDelegate(std::move(delegate))
    {
    }
...
public:
    void renderUI()
    {
        if (ImGui::Begin("Transform Tracker", nullptr)) {
            if (ImGui::BeginCombo("Transform", fTransformSelect ? std::get<0>(*fTransformSelect).c_str() : nullptr)) {
    ...
    void renderTracker(SkCanvas* canvas, double time, const SkSize& win_size, const SkSize& anim_size) const
    {
        ...
        const auto m = SkMatrix::Translate(tprop.fPosition.fX, tprop.fPosition.fY) * SkMatrix::RotateDeg(tprop.fRotation)
            * SkMatrix::Scale(tprop.fScale.fX * 0.01f, tprop.fScale.fY * 0.01f) * SkMatrix::Translate(tprop.fAnchorPoint.fX, tprop.fAnchorPoint.fY);

        const auto viewer_matrix = SkMatrix::RectToRect(SkRect::MakeSize(anim_size), SkRect::MakeSize(win_size), SkMatrix::kCenter_ScaleToFit);
...
private:
    void onTransformProperty(const char name[], const LazyHandle<skottie::TransformPropertyHandle>& lh) override
    {
...
public:
    ...
    void renderUI()
    {
        if (ImGui::Begin("Slot Manager", nullptr)) {
            ...
            for (size_t i = 0; i < fTextStringSlots.size(); i++) {
                ...
                if (ImGui::InputText("Text", tSlot.second.source.data(), tSlot.second.source.size())) {
                    this->pushSlots();
                }
    ...
    void pushSlots()
    {
        for (const auto& s : fColorSlots) {
            fSlotManager->setColorSlot(s.first, SkColor4f { s.second[0], s.second[1], s.second[2], s.second[3] }.toSkColor());
        }
        ...
        for (const auto& s : fTextStringSlots) {
            ...
            t->fTypeface = ToolUtils::TestFontMgr()->matchFamilyStyle(s.second.font.c_str(), SkFontStyle());
...
private:
    ...
    static constexpr std::array<const char*, 4> fTypefaceList = { "Arial", "Courier New", "Roboto-Regular", "Georgia" };
...
static void draw_stats_box(SkCanvas* canvas, const skottie::Animation::Builder::Stats& stats)
{
    ...
    const auto scene_parse_time = SkStringPrintf("Scene build time: %.3f ms", stats.fSceneParseTimeMS);
...
void SkottieSlide::init()
{
    ...
    auto resource_provider = sk_make_sp<AudioProviderProxy>(skresources::DataURIResourceProviderProxy::Make(
    ...
    if (fAnimation) {
        ...
        if (auto text_props = std::move(text_tracker->props()); !text_props.empty()) {
            ...
            fTextEditor = sk_make_sp<skottie_utils::TextEditor>(std::move(editor_target), std::move(text_props));
...
void SkottieSlide::draw(SkCanvas* canvas)
{
    if (fAnimation) {
        ...
        if (fShowAnimationInval) {
            const auto t = SkMatrix::RectToRect(SkRect::MakeSize(fAnimation->size()), dstR, SkMatrix::kCenter_ScaleToFit);
            SkPaint fill, stroke;
...
void SkottieSlide::renderUI()
{
    ...
    if (ImGui::Begin("Skottie Controls", nullptr,
        ...
        ImGui::PlotHistogram("", fFrameTimes.data(), fFrameTimes.size(), 0, nullptr, FLT_MAX, FLT_MAX, ImVec2(0, kUI_hist_height));
