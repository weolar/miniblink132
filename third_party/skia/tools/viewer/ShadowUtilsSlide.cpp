    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        for (auto& m : matrices) {
            for (auto flags : { kNone_ShadowFlag, kTransparentOccluder_ShadowFlag }) {
                for (const auto& path : fConvexPaths) {
                    ...
                    canvas->save();
                    canvas->concat(m);
                    this->drawShadowedPath(canvas, path, zPlaneParams, greenPaint, kAmbientAlpha, lightPos, kLightR, kSpotAlpha, flags);
                    canvas->restore();
        ...
        for (auto& m : matrices) {
            for (const auto& path : fConcavePaths) {
                ...
                canvas->save();
                canvas->concat(m);
                this->drawShadowedPath(canvas, path, zPlaneParams, greenPaint, kAmbientAlpha, lightPos, kLightR, kSpotAlpha, kNone_ShadowFlag);
                canvas->restore();
...
private:
    ...
    void drawShadowedPath(SkCanvas* canvas, const SkPath& path, const SkPoint3& zPlaneParams, const SkPaint& paint, SkScalar ambientAlpha,
        const SkPoint3& lightPos, SkScalar lightWidth, SkScalar spotAlpha, uint32_t flags)
    {
