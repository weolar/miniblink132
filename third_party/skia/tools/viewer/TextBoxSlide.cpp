...
using MakeBidiIteratorCallback
    = std::unique_ptr<SkShaper::BiDiRunIterator> (*)(sk_sp<SkUnicode> unicode, const char* utf8, size_t utf8Bytes, uint8_t bidiLevel);
...
static std::unique_ptr<SkShaper::BiDiRunIterator> make_unicode_bidi(sk_sp<SkUnicode> unicode, const char* utf8, size_t utf8Bytes, uint8_t bidiLevel)
{
...
#if defined(SK_SHAPER_HARFBUZZ_AVAILABLE) && defined(SK_SHAPER_UNICODE_AVAILABLE)
static std::unique_ptr<SkShaper::ScriptRunIterator> make_harfbuzz_script_runner(const char* utf8, size_t utf8Bytes, SkFourByteTag scriptTag)
{
    std::unique_ptr<SkShaper::ScriptRunIterator> script = SkShapers::HB::ScriptRunIterator(utf8, utf8Bytes, scriptTag);
    if (script) {
        return script;
    }
    return std::make_unique<SkShaper::TrivialScriptRunIterator>(scriptTag, utf8Bytes);
...
private:
    void drawTest(SkCanvas* canvas, SkScalar w, SkScalar h, SkColor fg, SkColor bg)
    {
        ...
        for (int i = 9; i < 24; i += 2) {
            ...
            std::unique_ptr<SkShaper::BiDiRunIterator> bidi = fBidiCallback(unicode, utf8, utf8Bytes, 0xfe);
            if (!bidi) {
                return;
            }
            ...
            std::unique_ptr<SkShaper::ScriptRunIterator> script = fScriptRunCallback(utf8, utf8Bytes, undeterminedScript);
            if (!script) {
                return;
            }

            std::unique_ptr<SkShaper::FontRunIterator> font(
                SkShaper::MakeFontMgrRunIterator(utf8, utf8Bytes, srcFont, ToolUtils::TestFontMgr(), "Arial", SkFontStyle::Bold(), &*language));
            if (!font) {
                return;
            }

            fShaper->shape(utf8, utf8Bytes, *font, *bidi, *script, *language, nullptr, 0, w - margin, &builder);
...
#if defined(SK_SHAPER_CORETEXT_AVAILABLE)
DEF_SLIDE(return new TextBoxSlide(SkShapers::CT::CoreText, make_trivial_bidi, make_trivial_script_runner, "coretext"););
#endif

#if defined(SK_SHAPER_HARFBUZZ_AVAILABLE) && defined(SK_SHAPER_UNICODE_AVAILABLE)
DEF_SLIDE(return new TextBoxSlide(
    []() { return SkShapers::HB::ShaperDrivenWrapper(get_unicode(), SkFontMgr::RefEmpty()); }, make_unicode_bidi, make_harfbuzz_script_runner, "harfbuzz"););
#endif
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        for (SkScalar size = 30; size <= 30; size += 10) {
            this->drawTest(canvas, text, size, SkShapers::Primitive::PrimitiveText(), make_trivial_bidi, make_trivial_script_runner);
            canvas->translate(0, size + 5);
#if defined(SK_SHAPER_CORETEXT_AVAILABLE)
            this->drawTest(canvas, text, size, SkShapers::CT::CoreText(), make_trivial_bidi, make_trivial_script_runner);
#endif
            canvas->translate(0, size + 5);
#if defined(SK_SHAPER_HARFBUZZ_AVAILABLE) && defined(SK_SHAPER_UNICODE_AVAILABLE)
            auto unicode = get_unicode();
            this->drawTest(
                canvas, text, size, SkShapers::HB::ShaperDrivenWrapper(unicode, SkFontMgr::RefEmpty()), make_unicode_bidi, make_harfbuzz_script_runner);
#endif
...
private:
    void drawTest(SkCanvas* canvas, const char str[], SkScalar size, std::unique_ptr<SkShaper> shaper, MakeBidiIteratorCallback bidiCallback,
        MakeScriptRunCallback scriptRunCallback)
    {
        ...
        auto unicode = get_unicode();
        std::unique_ptr<SkShaper::BiDiRunIterator> bidi = bidiCallback(unicode, str, len, 0xfe);
        if (!bidi) {
            return;
        }

        std::unique_ptr<SkShaper::LanguageRunIterator> language(SkShaper::MakeStdLanguageRunIterator(str, len));
        if (!language) {
            return;
        }

        SkFourByteTag undeterminedScript = SkSetFourByteTag('Z', 'y', 'y', 'y');
        std::unique_ptr<SkShaper::ScriptRunIterator> script(scriptRunCallback(str, len, undeterminedScript));
        if (!script) {
            return;
        }

        std::unique_ptr<SkShaper::FontRunIterator> font(
            SkShaper::MakeFontMgrRunIterator(str, len, srcFont, ToolUtils::TestFontMgr(), "Arial", SkFontStyle::Bold(), &*language));
        if (!font) {
            return;
        }
