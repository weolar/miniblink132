...
constexpr inline SkPoint rotate90(const SkPoint& p)
{
    return { p.fY, -p.fX };
}
inline SkPoint rotate180(const SkPoint& p)
{
    return p * -1;
}
inline bool isClockwise(const SkPoint& a, const SkPoint& b)
{
    return a.cross(b) > 0;
}
...
public:
    ...
    ScalarBezCurve(int degree, const std::vector<float>& weights)
        : ScalarBezCurve(degree)
    {
        SkASSERT(degree >= 0);
    ...
    static void Split(const ScalarBezCurve& curve, float t, ScalarBezCurve* left, ScalarBezCurve* right)
    {
        // Set up starting point of recursion (k=0)
    ...
    static ScalarBezCurve ElevateDegree(const ScalarBezCurve& curve, int newDegree)
    {
        ...
        for (int i = 0; i <= n + r; i++) {
            ...
            for (int j = std::max(0, i - r); j <= std::min(n, i); j++) {
                const float f = (choose(n, j) * choose(r, i - j)) / static_cast<float>(choose(n + r, i));
     ...
     */
    std::vector<float> zeroSet() const
    {
        return ZeroSet(*this);
    }
    ...
    static ScalarBezCurve Mul(const ScalarBezCurve& a, const ScalarBezCurve& b)
    {
        ...
        for (int k = 0; k <= newDegree; k++) {
            ...
            for (int i = std::max(0, k - n); i <= std::min(k, m); i++) {
                const float f = (choose(m, i) * choose(n, k - i)) / static_cast<float>(choose(m + n, k));
...
private:
    ...
    static void ZeroSetRec(const ScalarBezCurve& curve, float tmin, float tmax, float tol, std::vector<float>* result)
    {
        float lenP = 0;
...
public:
    explicit PathVerbMeasure(const SkPath& path)
        : fPath(path)
        , fIter(path, false)
    {
        nextVerb();
...
public:
    ...
    {
        return getFillPath(path, paint, identityVarWidth(paint.getStrokeWidth()), identityVarWidth(paint.getStrokeWidth()));
    }

    /**
     * Strokes the given path using the two given distance functions for inner and outer offsets.
     */
    SkPath getFillPath(const SkPath& path, const SkPaint& paint, const ScalarBezCurve& varWidth, const ScalarBezCurve& varWidthInner,
...
private:
    ...
    OffsetSegments strokeSegment(const PathSegment& segment, const ScalarBezCurve& varWidth, const ScalarBezCurve& varWidthInner, bool needsMove);
     ...
     */
    std::vector<PathSegment> strokeSegment(const PathSegment& seg, const ScalarBezCurve& distanceFunc) const;
    ...
    void join(const SkPoint& common, float innerRadius, float outerRadius, const OffsetSegments& prev, const OffsetSegments& curr);
     ...
     * function.
     */
    static void approximateSegment(const PathSegment& seg, const ScalarBezCurve& distFnc, PathSegment* approxQuad);
...
SkPath SkVarWidthStroker::getFillPath(
    const SkPath& path, const SkPaint& paint, const ScalarBezCurve& varWidth, const ScalarBezCurve& varWidthInner, LengthMetric lengthMetric)
{
    ...
    const float totalPathLength = lengthMetric == LengthMetric::kPathLength ? meas.totalLength() : (path.countVerbs() - 1);
    ...
    while ((segment.fVerb = it.next(&segment.fPoints[0])) != SkPath::kDone_Verb) {
        const float verbLength = lengthMetric == LengthMetric::kPathLength ? (meas.currentVerbLength() / totalPathLength) : (1.0f / totalPathLength);
...
SkVarWidthStroker::OffsetSegments SkVarWidthStroker::strokeSegment(
    const PathSegment& segment, const ScalarBezCurve& varWidth, const ScalarBezCurve& varWidthInner, bool needsMove)
{
...
std::vector<SkVarWidthStroker::PathSegment> SkVarWidthStroker::strokeSegment(const PathSegment& seg, const ScalarBezCurve& distanceFunc) const
{
...
void SkVarWidthStroker::join(const SkPoint& common, float innerRadius, float outerRadius, const OffsetSegments& prev, const OffsetSegments& curr)
{
    const auto miterJoin = [this](const SkPoint& common, float leftRadius, float rightRadius, const OffsetSegments& prev, const OffsetSegments& curr) {
...
void SkVarWidthStroker::splitSegment(const PathSegment& seg, float t, PathSegment* segA, PathSegment* segB)
{
...
void SkVarWidthStroker::approximateSegment(const PathSegment& seg, const ScalarBezCurve& distFnc, PathSegment* approxQuad)
{
...
SkPoint SkVarWidthStroker::unitNormal(const PathSegment& seg, float t, SkPoint* tangentOut)
{
    ...
    case SkPath::kQuad_Verb: {
        ...
        if (t == 0) {
            ...
            tangent = ((seg.fPoints[1] - seg.fPoints[0]) * (1 - t) + (seg.fPoints[2] - seg.fPoints[1]) * t) * 2;
        }
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        ScalarBezCurve distFncInner = fDifferentInnerFunc ? makeDistFnc(fDistFncsInner, fWidth) : distFnc;
        SkVarWidthStroker stroker;
        SkPath fillPath = stroker.getFillPath(path, fStrokePaint, distFnc, distFncInner, fLengthMetric);
        ...
        if (fShowSkeleton) {
            ...
            canvas->drawPoints(SkCanvas::kPoints_PointMode, fPathPts.size(), fPathPts.data(), fPtsPaint);
        }
...
private:
    ...
    {
        value = value == SkVarWidthStroker::LengthMetric::kPathLength ? SkVarWidthStroker::LengthMetric::kNumSegments
    ...
    void drawStrokePoints(SkCanvas* canvas, const SkPath& fillPath)
    {
        ...
        canvas->drawPoints(SkCanvas::kPoints_PointMode, pointsVec.size(), pointsVec.data(), fStrokePointsPaint);
        fStrokePointsPaint.setColor(SK_ColorBLUE);
        fStrokePointsPaint.setStrokeWidth(3);
        canvas->drawPoints(SkCanvas::kPoints_PointMode, ctrlPts.size(), ctrlPts.data(), fStrokePointsPaint);
    ...
    void drawErrorCurve(SkCanvas* canvas, const ScalarBezCurve& E)
    {
        ...
        const SkRect box = SkRect::MakeXYWH(padding, fWinSize.height() - winH - padding, winW - 2 * padding, winH);
    ...
    void drawUI()
    {
        ...
        if (ImGui::Begin("E-C Controls", nullptr,
            ...
            const auto drawControls = [](std::vector<DistFncMenuItem>& distFncs, const std::string& menuPfx, const std::string& ptPfx) {
                ...
                if (ImGui::BeginMenu(degreeMenuLabel.c_str())) {
                    for (size_t i = 0; i < distFncs.size(); i++) {
                        if (ImGui::MenuItem(distFncs[i].fName.c_str(), nullptr, distFncs[i].fSelected)) {
            ...
            const std::array<std::pair<std::string, SkVarWidthStroker::LengthMetric>, 2> metrics = {
                ...
                std::make_pair("% segment count", SkVarWidthStroker::LengthMetric::kNumSegments),
            };
            if (ImGui::BeginMenu("Interpolation metric:")) {
                for (const auto& metric : metrics) {
                    if (ImGui::MenuItem(metric.first.c_str(), nullptr, fLengthMetric == metric.second)) {
    ...
    bool fShowHidden, fShowSkeleton, fShowStrokePoints, fShowUI, fDifferentInnerFunc, fShowErrorCurve;
    float fWidth = 175;
    SkPaint fPtsPaint, fStrokePaint, fNewFillPaint, fHiddenPaint, fSkeletonPaint, fStrokePointsPaint;
    inline static constexpr int kNPts = 5;
    std::array<SkPoint, kNPts> fPathPts;
    SkSize fWinSize;
    SkVarWidthStroker::LengthMetric fLengthMetric;
    const std::vector<DistFncMenuItem> fDefaultsDistFncs = { DistFncMenuItem("Linear", 1, true), DistFncMenuItem("Quadratic", 2, false),
        DistFncMenuItem("Cubic", 3, false), DistFncMenuItem("One Louder (11)", 11, false), DistFncMenuItem("30?!", 30, false) };
