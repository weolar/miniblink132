...
private:
    ...
    SamplePathTessellatorOp(const SkRect& drawBounds, const SkPath& path, const SkMatrix& m, GrPipeline::InputFlags pipelineFlags, Mode mode)
        : GrDrawOp(ClassID())
        , fPath(path)
        , fMatrix(m)
    ...
    {
        SkPMColor4f color;
        return fProcessors.finalize(SK_PMColor4fWHITE, GrProcessorAnalysisCoverage::kNone, clip, nullptr, caps, clampType, &color);
    }
    ...
    void onPrepare(GrOpFlushState* flushState) override
    {
        ...
        if (fMode == Mode::kCurveMiddleOut) {
            ...
            for (tess::PathMiddleOutFanIter it(fPath); !it.done();) {
                for (auto [p0, p1, p2] : it.nextStack()) {
                    triangles.append(&storage, pathMatrix.mapPoint(p0), pathMatrix.mapPoint(p1), pathMatrix.mapPoint(p2),
                        /*winding=*/1);
                }
            }

            auto* tess = PathCurveTessellator::Make(alloc, shaderCaps.fInfinitySupport);
            tess->prepareWithTriangles(flushState, shaderMatrix, &triangles, pathList, fPath.countVerbs());
            fTessellator = tess;
#else
            auto* tess = PathCurveTessellator::Make(alloc, shaderCaps.fInfinitySupport);
            tess->prepareWithTriangles(flushState, shaderMatrix, nullptr, pathList, fPath.countVerbs());
            fTessellator = tess;
#endif
        } else {
        ...
        auto pipeline = GrSimpleMeshDrawOpHelper::CreatePipeline(flushState, std::move(fProcessors), fPipelineFlags);
        auto* tessShader = GrPathTessellationShader::Make(*caps.shaderCaps(), alloc, shaderMatrix, kCyan, fTessellator->patchAttribs());
        fProgram = GrTessellationShader::MakeProgram({ alloc, flushState->writeView(), flushState->usesMSAASurface(), &flushState->dstProxyView(),
...
void PathTessellatorsSlide::draw(SkCanvas* canvas)
{
    ...
    sdc->addDrawOp(
        GrOp::Make<SamplePathTessellatorOp>(ctx, sdc->asRenderTargetProxy()->getBoundsRect(), fPath, canvas->getTotalMatrix(), fPipelineFlags, fMode));
    ...
    devPath.transform(canvas->getTotalMatrix());
    {
        ...
        canvas->drawPoints(SkCanvas::kPoints_PointMode, devPath.countPoints(), SkPathPriv::PointData(devPath), pointsPaint);
    }
}
...
ClickHandlerSlide::Click* PathTessellatorsSlide::onFindClickHandler(SkScalar x, SkScalar y, skui::ModifierKey)
{
...
bool PathTessellatorsSlide::onChar(SkUnichar unichar)
{
    switch (unichar) {
    case 'w':
        fPipelineFlags = (GrPipeline::InputFlags)((int)fPipelineFlags ^ (int)GrPipeline::InputFlags::kWireframe);
        return true;
    case 'D': {
        fPath.dump();
        return true;
