...
static void show_text(SkCanvas* canvas, bool doAA)
{
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        canvas->drawString("Hamburgefons", rand.nextSScalar1() * W, rand.nextSScalar1() * H + 20, font, paint);
    }
}
...
static void show_fill(SkCanvas* canvas, bool doAA)
{
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        r.setXYWH(rand.nextSScalar1() * W, rand.nextSScalar1() * H, rand.nextUScalar1() * W, rand.nextUScalar1() * H);
...
static void show_stroke(SkCanvas* canvas, bool doAA, SkScalar strokeWidth, int n)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        paint.setColor(rand.nextU());
        canvas->drawLine(randRange(rand, minx, maxx), randRange(rand, miny, maxy), randRange(rand, minx, maxx), randRange(rand, miny, maxy), paint);
    }
}
...
{
    SkScalar a = mx[0], b = mx[1], c = mx[2], d = mx[3], e = mx[4], f = mx[5], g = mx[6], h = mx[7], i = mx[8];
...
public:
    SkM44 get44(const SkRect& r) const
    {
        ...
        SkM44 camera = SkM44::LookAt(fEye, fCOA, fUp), perspective = SkM44::Perspective(fNear, fFar, fAngle),
              translate = SkM44::Translate(fTrans.x, fTrans.y, fTrans.z),
              viewport = SkM44::Translate(r.centerX(), r.centerY(), 0) * SkM44::Scale(w * 0.5f, h * 0.5f, 1);
...
public:
    ...
    {
        fPath = make_path();
        fShader = ToolUtils::GetResourceAsImage("images/mandrill_128.png")->makeShader(SkSamplingOptions(), SkMatrix::Scale(3, 3));
    }
...
public:
    ...
    void load(SkScalar w, SkScalar h) override
    {
        ...
        fShader = ToolUtils::GetResourceAsImage("images/mandrill_256.png")->makeShader(SkSamplingOptions());
    }
