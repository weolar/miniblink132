...
public:
    ...
    void draw(SkCanvas* canvas)
    {
        ...
        SkTextUtils::DrawString(canvas, fLabel.c_str(), fRect.centerX(), fRect.fTop + 0.68f * fRect.height(), font, paint, SkTextUtils::kCenter_Align);
    }
...
public:
    CircDrawable(SkScalar size, SkColor c)
    {
        ...
        fPaint.setShader(SkGradientShader::MakeRadial(SkPoint::Make(size / 2, size / 2), size / 2, colors, nullptr, 2, SkTileMode::kClamp));
...
protected:
    void load(SkScalar, SkScalar) override
    {
        ...
        for (const char* name : { "images/mandrill_128.png", "images/rle.bmp", "images/example_4.png" }) {
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        canvas->drawSimpleText(str.c_str(), str.size(), SkTextEncoding::kUTF8, fDomain.fLeft + 10, fDomain.fBottom + 40, font, paint);
    }
...
private:
    struct Rec {
        ...
        void draw(SkCanvas* canvas, SkCubicResampler cubic) const
        {
            ...
            paint.setShader(fImage->makeShader(SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions(cubic), &lm));
            canvas->drawRect(r, paint);
        }
    };
