...
sk_sp<SkShader> make_image_shader(const char* resource)
{
    ...
    return img->makeShader(SkTileMode::kDecal, SkTileMode::kDecal, SkSamplingOptions(SkFilterMode::kLinear), &lm);
}
...
static constexpr struct ShaderFactory {
    ...
    { "Img (Mandrill)", []() -> sk_sp<SkShader> { return make_image_shader("images/mandrill_512.png"); } },
    { "Img (Baby Tux)", []() -> sk_sp<SkShader> { return make_image_shader("images/baby_tux.png"); } },
    { "Img (Brickwork)", []() -> sk_sp<SkShader> { return make_image_shader("images/brickwork-texture.jpg"); } },
    { "Radial Gradient",
        []() -> sk_sp<SkShader> {
            static constexpr SkColor gColors[] = { SK_ColorGREEN, SK_ColorRED, SK_ColorBLUE, SK_ColorGREEN };
            return SkGradientShader::MakeRadial({ 0.5f, 0.5f }, 0.5f, gColors, nullptr, std::size(gColors), SkTileMode::kRepeat);
        } },
    { "Colors", []() -> sk_sp<SkShader> { return nullptr; } },
};
...
} gVertexAnimators[] = {
    {
        ...
        [](const std::vector<SkPoint>& uvs, float t, std::vector<SkPoint>& out) {
            ...
            for (size_t i = 0; i < uvs.size(); ++i) {
                ...
                const auto arc_len = uv.fX - cyl_pos, arc_ang = arc_len / kCylRadius;
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        auto verts = SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, fVertices.size(), fVertices.data(), fShader ? fUVs.data() : nullptr,
            fShader ? nullptr : fColors.data(), fIndices.size(), fIndices.data());
        p.setShader(fShader);
...
private:
    void initMesh(size_t vertex_count)
    {
        ...
        for (size_t i = 0; i < vertex_count; ++i) {
            ...
            fColors[i] = SkColorSetRGB(!!(i % 2) * 255, !!(i % 3) * 255, !!((i + 1) % 3) * 255);
        }
        ...
        // |  \|/  |
        // *---*---*
        const size_t triangle_count = 2 * (n - 1) * (n - 1), index_count = 3 * triangle_count;

        fIndices.clear();
        fIndices.reserve(index_count);
        for (size_t i = 0; i < n - 1; ++i) {
            for (size_t j = 0; j < n - 1; ++j) {
                const auto row_0_idx = j * n + i, row_1_idx = row_0_idx + n, off_0 = (i + j) % 2, off_1 = 1 - off_0;
    ...
    void drawControls()
    {
        ...
        ImGui::SliderInt("Mesh Size", &fMeshSizeSelector, 0, std::size(gSizeInfo) - 1, gSizeInfo[fMeshSizeSelector].fLabel);
    ...
    std::vector<SkPoint> fVertices, fUVs;
