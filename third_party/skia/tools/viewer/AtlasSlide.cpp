...
static sk_sp<SkImage> make_atlas(int atlasSize, int cellSize)
{
    ...
    for (int y = 0; y < atlasSize; y += cellSize) {
        for (int x = 0; x < atlasSize; x += cellSize) {
            ...
            SkTextUtils::Draw(canvas, &s[index], 1, SkTextEncoding::kUTF8, x + half, y + half + half / 2, font, paint, SkTextUtils::kCenter_Align);
            i += 1;
        }
    }
...
class DrawAtlasDrawable : public SkDrawable {
    ...
    struct Rec {
        ...
        {
            return SkRSXform::MakeFromRadians(fScale, fRadian, fCenter.x(), fCenter.y(), SkScalarHalf(kCellSize), SkScalarHalf(kCellSize));
        }
    };
