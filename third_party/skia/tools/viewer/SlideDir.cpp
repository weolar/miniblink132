...
SkMatrix SlideMatrix(const sk_sp<Slide>& slide, const SkRect& dst)
{
    ...
    return SkMatrix::RectToRect(SkRect::MakeIWH(slideSize.width(), slideSize.height()), dst, SkMatrix::kCenter_ScaleToFit);
}
...
public:
    ...
    bool onMouse(SkScalar x, SkScalar y, skui::InputState state, skui::ModifierKey modifiers)
    {
        ...
        const auto xform = SkMatrix::RectToRect(fRect, SkRect::MakeSize(fDir->fWinSize), SkMatrix::kCenter_ScaleToFit);
...
protected:
    void onTick(float t) override
    {
        ...
        const auto rel_t = (t - fTimeBase) / kFocusDuration, map_t = SkTPin(fMap.computeYFromX(rel_t), 0.0f, 1.0f);
...
void SlideDir::load(SkScalar winWidth, SkScalar winHeight)
{
    ...
    for (int i = 0; i < fSlides.size(); ++i) {
        ...
        const auto slideSize = slide->getDimensions();
        const auto cell = SkRect::MakeXYWH(fCellSize.width() * (i % fColumns), fCellSize.height() * (i / fColumns), fCellSize.width(), fCellSize.height()),
        ...
        slideGrp->addChild(sksg::Draw::Make(sksg::Rect::Make(SkRect::MakeIWH(slideSize.width(), slideSize.height())), sksg::Color::Make(0xfff0f0f0)));
        slideGrp->addChild(adapter);
        slideGrp->addChild(MakeLabel(slide->getName(), SkPoint::Make(slideSize.width() / 2, slideSize.height()), slideMatrix->getMatrix()));
    ...
    const auto focusRect = SkRect::MakeSize(fWinSize).makeInset(kFocusInset.width(), kFocusInset.height());
...
{
    return SkSize::Make(fWinSize.width(), fCellSize.height() * (1 + (fSlides.size() - 1) / fColumns)).toCeil();
}
...
bool SlideDir::onMouse(SkScalar x, SkScalar y, skui::InputState state, skui::ModifierKey modifiers)
{
    ...
    case skui::InputState::kUp:
        if (cell == fTrackingCell && SkPoint::Distance(fTrackingPos, SkPoint::Make(x, y)) < kClickMoveTolerance) {
...
const SlideDir::Rec* SlideDir::findCell(float x, float y) const
{
    ...
    const int col = static_cast<int>(x / fCellSize.width()), row = static_cast<int>(y / fCellSize.height()), idx = row * fColumns + col;
