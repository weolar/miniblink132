    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        for (int y = 0; y < H; y++)
            for (int x = 0; x < W; x++) {
                ...
                canvas->drawImageRect(fImg.get(), SkRect::MakeXYWH(x, y, 1, 1), SkRect::MakeXYWH(x, y, 1, 1), SkSamplingOptions(), /*paint=*/nullptr,
        ...
        if (sk_sp<SkSurface> offscreen = canvas->makeSurface(info)) {
            ...
            for (int y = 0; y < H; y++)
                for (int x = 0; x < W; x++) {
                    auto start = std::chrono::steady_clock::now();
                    offscreen->getCanvas()->drawImageRect(fImg, SkRect::MakeXYWH(x, y, 1, 1), SkRect::MakeXYWH(0, 0, 1024, 1024), SkSamplingOptions(),
