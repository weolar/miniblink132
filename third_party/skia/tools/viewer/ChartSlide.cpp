...
static void gen_data(SkScalar yAvg, SkScalar ySpread, int count, SkTDArray<SkScalar>* dataPts)
{
    ...
    for (int i = 0; i < count; ++i) {
        (*dataPts)[i] = gRandom.nextRangeScalar(yAvg - SkScalarHalf(ySpread), yAvg + SkScalarHalf(ySpread));
    }
}
...
static void gen_paths(const SkTDArray<SkScalar>& topData, const SkTDArray<SkScalar>* bottomData, SkScalar yBase, SkScalar xLeft, SkScalar xDelta, int leftShift,
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        for (int i = 0; i < kNumGraphs; ++i) {
            gen_paths(fData[i], prevData, height, 0, SkIntToScalar(kPixelsPerTick), fShift, &plotPath, &fillPath);
