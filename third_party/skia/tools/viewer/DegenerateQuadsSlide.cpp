...
static SkScalar get_edge_dist_coverage(
    const bool edgeAA[4], const SkPoint corners[4], const SkPoint outsetLines[8], const SkPoint insetLines[8], const SkPoint& point)
{
    bool flip = false;
...
static SkScalar get_framed_coverage(const SkPoint outer[4], const SkScalar outerCoverages[4], const SkPoint inner[4], const SkScalar innerCoverages[4],
    const SkRect& geomDomain, const SkPoint& point)
{
    ...
    static const int kFrameTris[] = { 0, 1, 4, 4, 1, 5, 1, 2, 5, 5, 2, 6, 2, 3, 6, 6, 3, 7, 3, 0, 7, 7, 0, 4, 4, 5, 7, 7, 5, 6 };
...
public:
    ...
    void draw(SkCanvas* canvas) override
    {
        ...
        if (valid) {
            ...
            SkRect gpuDomain;
            this->getTessellatedPoints(gpuInset, gpuInsetCoverage, gpuOutset, gpuOutsetCoverage, &gpuDomain);
            ...
            for (SkScalar py = covRect.fTop; py < covRect.fBottom; py += 1.f) {
                for (SkScalar px = covRect.fLeft; px < covRect.fRight; px += 1.f) {
                    ...
                    if (fCoverageMode == CoverageMode::kArea) {
                        ...
                        coverage = get_framed_coverage(gpuOutset, gpuOutsetCoverage, gpuInset, gpuInsetCoverage, gpuDomain, pixelCenter);
                    }
...
private:
    ...
    void getTessellatedPoints(SkPoint inset[4], SkScalar insetCoverage[4], SkPoint outset[4], SkScalar outsetCoverage[4], SkRect* domain) const
    {
        // Fixed vertex spec for extracting the picture frame geometry
        static const VertexSpec kSpec = { GrQuad::Type::kGeneral, ColorType::kNone, GrQuad::Type::kAxisAligned, false, Subset::kNo, GrAAType::kCoverage, false,
...
ClickHandlerSlide::Click* DegenerateQuadSlide::onFindClickHandler(SkScalar x, SkScalar y, skui::ModifierKey)
{
