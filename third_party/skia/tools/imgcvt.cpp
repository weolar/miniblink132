...
int main(int argc, char** argv)
{
    ...
    if (skcms_Parse(blob->data(), blob->size(), &src_profile)) {
        ...
        if (!skcms_Transform(src, skcms_PixelFormat_RGB_fff, skcms_AlphaFormat_Unpremul, &src_profile, dst, skcms_PixelFormat_RGB_fff,
                skcms_AlphaFormat_Unpremul, &dst_profile, 8)) {
            SkDebugf("Cannot transform.\n");
            return 1;
        }
        for (int i = 0; i < 8; i++) {
            SkDebugf(
                "(%g, %g, %g) --> (%+.4f, %+.4f, %+.4f)\n", src[3 * i + 0], src[3 * i + 1], src[3 * i + 2], dst[3 * i + 0], dst[3 * i + 1], dst[3 * i + 2]);
        }
        return 0;
    }
    ...
    { // transform with skcms
        SkColorSpace* src_cs = image->colorSpace() ? image->colorSpace() : sk_srgb_singleton();
        ...
        case kBGRA_8888_SkColorType:
            fmt = skcms_PixelFormat_BGRA_8888;
            break;
        default:
        ...
        if (!skcms_Transform(pixmap.addr(), fmt, alpha, &src_profile, pixmap.writable_addr(), fmt, alpha, &dst_profile, pixmap.width() * pixmap.height())) {
