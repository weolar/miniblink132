...
bool SkFlagInfo::CreateStringFlag(const char* name, const char* shortName, CommandLineFlags::StringArray* pStrings, const char* defaultValue,
    const char* helpString, const char* extendedHelpString)
{
    SkFlagInfo* info = new SkFlagInfo(name, shortName, kString_FlagType, helpString, extendedHelpString);
...
void SkFlagInfo::SetDefaultStrings(CommandLineFlags::StringArray* pStrings, const char* defaultValue)
{
    pStrings->reset();
...
template <typename Strings> bool ShouldSkipImpl(const Strings& strings, const char* name)
{
    ...
    for (int i = 0; i < strings.size(); ++i) {
        ...
        if (matchStart     ? (!matchEnd || matchLen == testLen) && strncmp(name, matchName, matchLen) == 0
                : matchEnd ? matchLen <= testLen && strncmp(name + testLen - matchLen, matchName, matchLen) == 0
