...
static const char defaultConfigs[] = "8888 " DEFAULT_GPU_CONFIG " nonrendering "
...
static const char configHelp[] = "Options: 565 4444 8888 rgba bgra rgbx 1010102 101010x bgra1010102 bgr101010x f16 f16norm f16f16f16x "
...
SkCommandLineConfig::SkCommandLineConfig(const SkString& tag, const SkString& backend, const TArray<SkString>& viaParts)
    : fTag(tag)
    , fBackend(backend)
{
    static const auto* kColorSpaces = new std::unordered_map<std::string_view, SkColorSpace*> {
        ...
        { "srgb", SkColorSpace::MakeSRGB().release() },
        { "srgb2", // The same as "srgb" but works around ignoring prior images in Gold
            SkColorSpace::MakeSRGB().release() },
        { "linear", SkColorSpace::MakeSRGBLinear().release() },
        { "p3", SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kDisplayP3).release() },
        { "spin", SkColorSpace::MakeSRGB()->makeColorSpin().release() },
        { "rec2020", SkColorSpace::MakeRGB(SkNamedTransferFn::kRec2020, SkNamedGamut::kRec2020).release() },
    };
...
#if defined(SK_GRAPHITE)
    bool get_option_graphite_api(const char* optionKey, SkCommandLineConfigGraphite::ContextType* outContextType) const
    {
        SkString* optionValue = fOptionsMap.find(SkString(optionKey));
        if (optionValue == nullptr) {
            return false;
...
SkCommandLineConfigGpu::SkCommandLineConfigGpu(const SkString& tag, const TArray<SkString>& viaParts, ContextType contextType, bool fakeGLESVersion2,
    uint32_t surfaceFlags, int samples, SkColorType colorType, SkAlphaType alphaType, bool useStencilBuffers, int testPersistentCache, bool testPrecompile,
    bool useDDLSink, bool slug, bool serializeSlug, bool remoteSlug, bool reducedShaders, SurfType surfType)
...
SkCommandLineConfigGpu* parse_command_line_config_gpu(const SkString& tag, const TArray<SkString>& vias, const SkString& options)
{
    ...
    bool validOptions = extendedOptions.get_option_gpu_api("api", &contextType, &fakeGLESVersion2, false) && extendedOptions.get_option_bool("dit", &useDIText)
        && extendedOptions.get_option_int("samples", &samples) && extendedOptions.get_option_bool("dmsaa", &useDMSAA)
        && extendedOptions.get_option_gpu_color("color", &colorType, &alphaType) && extendedOptions.get_option_bool("stencils", &useStencils)
        && extendedOptions.get_option_int("testPersistentCache", &testPersistentCache) && extendedOptions.get_option_bool("testPrecompile", &testPrecompile)
        && extendedOptions.get_option_bool("useDDLSink", &useDDLs) && extendedOptions.get_option_bool("slug", &slug)
        && extendedOptions.get_option_bool("serializeSlug", &serializeSlug) && extendedOptions.get_option_bool("remoteSlug", &remoteSlug)
        && extendedOptions.get_option_bool("reducedShaders", &reducedShaders) && extendedOptions.get_option_gpu_surf_type("surf", &surfType);
    ...
    return new SkCommandLineConfigGpu(tag, vias, contextType, fakeGLESVersion2, surfaceFlags, samples, colorType, alphaType, useStencils, testPersistentCache,
        testPrecompile, useDDLs, slug, serializeSlug, remoteSlug, reducedShaders, surfType);
}

#if defined(SK_GRAPHITE)

SkCommandLineConfigGraphite* parse_command_line_config_graphite(const SkString& tag, const TArray<SkString>& vias, const SkString& options)
{
    ...
    bool validOptions = extendedOptions.get_option_graphite_api("api", &contextType) && extendedOptions.get_option_gpu_color("color", &colorType, &alphaType)
        && extendedOptions.get_option_bool("precompile", &testPrecompile);
    if (!validOptions) {
        return nullptr;
    }

    return new SkCommandLineConfigGraphite(tag, vias, contextType, colorType, alphaType, testPrecompile);
}

#endif

SkCommandLineConfigSvg::SkCommandLineConfigSvg(const SkString& tag, const TArray<SkString>& viaParts, int pageIndex)
