...
static void run_test_case(const SkString& testdata, const SkBitmap& bitmap, SkCanvas* canvas)
{
    ...
    sk_sp<SkImageFilter> flattenable = SkValidatingDeserializeImageFilter(testdata.c_str(), testdata.size());
    ...
    if (flattenable != nullptr) {
        ...
        canvas->save();
        canvas->clipRect(SkRect::MakeXYWH(0, 0, SkIntToScalar(kBitmapSize), SkIntToScalar(kBitmapSize)));
