...
public:
    ...
    {
        return fCacheMissCnt;
    }
    int numCacheStores() const
    {
        return fCacheStoreCnt;
    }
...
private:
    struct Key {
        ...
        Key(const SkData& key)
            : fKey(SkData::MakeWithCopy(key.data(), key.size()))
        {
        }
        ...
        {
            return that.fKey->size() == fKey->size() && !memcmp(fKey->data(), that.fKey->data(), that.fKey->size());
        }
