...
public:
     ...
     */
    template <typename... Args> static sk_sp<ManagedBackendTexture> MakeWithData(GrDirectContext*, Args&&...);
     ...
     * avoid this issue.
     */
    template <typename... Args> static sk_sp<ManagedBackendTexture> MakeWithoutData(GrDirectContext*, Args&&...);

    static sk_sp<ManagedBackendTexture> MakeFromInfo(GrDirectContext* dContext, const SkImageInfo&, skgpu::Mipmapped = skgpu::Mipmapped::kNo,
        skgpu::Renderable = skgpu::Renderable::kNo, skgpu::Protected = skgpu::Protected::kNo);

    static sk_sp<ManagedBackendTexture> MakeFromBitmap(
        GrDirectContext*, const SkBitmap&, skgpu::Mipmapped, skgpu::Renderable, skgpu::Protected = skgpu::Protected::kNo);

    static sk_sp<ManagedBackendTexture> MakeFromPixmap(
        GrDirectContext*, const SkPixmap&, skgpu::Mipmapped, skgpu::Renderable, skgpu::Protected = skgpu::Protected::kNo);
     ...
     */
    void* releaseContext(GrGpuFinishedProc wrappedProc = nullptr, GrGpuFinishedContext wrappedContext = nullptr) const;
...
template <typename... Args> inline sk_sp<ManagedBackendTexture> ManagedBackendTexture::MakeWithData(GrDirectContext* dContext, Args&&... args)
{
    sk_sp<ManagedBackendTexture> mbet(new ManagedBackendTexture);
    mbet->fDContext = sk_ref_sp(dContext);
    mbet->fTexture = dContext->createBackendTexture(std::forward<Args>(args)..., ReleaseProc, mbet->releaseContext());
...
template <typename... Args> inline sk_sp<ManagedBackendTexture> ManagedBackendTexture::MakeWithoutData(GrDirectContext* dContext, Args&&... args)
{
...
public:
    static sk_sp<ManagedGraphiteTexture> MakeUnInit(
        Recorder*, const SkImageInfo&, skgpu::Mipmapped, skgpu::Renderable, skgpu::Protected = skgpu::Protected::kNo);

    static sk_sp<ManagedGraphiteTexture> MakeFromPixmap(
        Recorder*, const SkPixmap&, skgpu::Mipmapped, skgpu::Renderable, skgpu::Protected = skgpu::Protected::kNo);

    static sk_sp<ManagedGraphiteTexture> MakeMipmappedFromPixmaps(
        Recorder*, SkSpan<const SkPixmap> levels, skgpu::Renderable, skgpu::Protected = skgpu::Protected::kNo);

    static sk_sp<ManagedGraphiteTexture> MakeFromCompressedData(
        Recorder*, SkISize dimmensions, SkTextureCompressionType, sk_sp<SkData>, skgpu::Mipmapped, skgpu::Protected = skgpu::Protected::kNo);
