...
void TestCanvas<SkSlugTestKey>::onDrawGlyphRunList(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
...
void TestCanvas<SkSerializeSlugTestKey>::onDrawGlyphRunList(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkRect bounds = glyphRunList.sourceBoundsWithOrigin();
    if (this->internalQuickReject(bounds, paint)) {
        return;
...
void TestCanvas<SkRemoteSlugTestKey>::onDrawGlyphRunList(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkRect bounds = glyphRunList.sourceBoundsWithOrigin();
    if (this->internalQuickReject(bounds, paint)) {
        return;
    ...
    if (layer) {
        if (glyphRunList.hasRSXForm()) {
            ...
            {
                auto analysisCanvas = fStrikeServer.makeAnalysisCanvas(this->topDevice()->width(), this->topDevice()->height(), this->fProps,
                ...
                auto slug = analysisCanvas->onConvertGlyphRunListToSlug(glyphRunList, layer->paint());
                if (slug != nullptr) {
            ...
            }
            {
                ...
                if (slugBytes != nullptr) {
                    auto slug = sktext::gpu::Slug::Deserialize(slugBytes->data(), slugBytes->size(), &fStrikeClient);
                    this->drawSlug(slug.get(), layer->paint());
                }
            }
        }
