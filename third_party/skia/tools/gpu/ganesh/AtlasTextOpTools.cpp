...
GrOp::Owner AtlasTextOpTools::CreateOp(
    skgpu::ganesh::SurfaceDrawContext* sdc, const SkPaint& skPaint, const SkFont& font, const SkMatrix& ctm, const char* text, int x, int y)
{
    ...
    sktext::gpu::SubRunControl control = rContext->priv().getSubRunControl(sdc->surfaceProps().isUseDeviceIndependentFonts());

    SkStrikeDeviceInfo strikeDeviceInfo { sdc->surfaceProps(), SkScalerContextFlags::kBoostContrast, &control };

    sk_sp<sktext::gpu::TextBlob> blob = sktext::gpu::TextBlob::Make(glyphRunList, skPaint, drawMatrix, strikeDeviceInfo, SkStrikeCache::GlobalStrikeCache());
