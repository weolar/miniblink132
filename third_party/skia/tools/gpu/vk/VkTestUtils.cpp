...
bool LoadVkLibraryAndGetProcAddrFuncs(PFN_vkGetInstanceProcAddr* instProc)
{
    ...
    if (!vkLib) {
        ...
        localInstProc = (PFN_vkGetInstanceProcAddr)SkGetProcedureAddress(vkLib, "vkGetInstanceProcAddr");
    }
    if (!localInstProc) {
        return false;
    }
...
static int should_include_debug_layer(const char* layerName, uint32_t layerCount, VkLayerProperties* layers, uint32_t version)
{
    ...
    int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
{
    if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        ...
        if (strstr(pMessage, "VUID-vkCmdDraw-None-02686") || strstr(pMessage, "VUID-vkCmdDrawIndexed-None-02686")) {
            return VK_FALSE;
        }
...
static bool should_include_extension(const char* extensionName)
{
    const char* kValidExtensions[] = {
        // single merged layer
        VK_EXT_BLEND_OPERATION_ADVANCED_EXTENSION_NAME, VK_EXT_CONSERVATIVE_RASTERIZATION_EXTENSION_NAME, VK_EXT_DEBUG_REPORT_EXTENSION_NAME,
        VK_EXT_DEVICE_FAULT_EXTENSION_NAME, VK_EXT_FRAME_BOUNDARY_EXTENSION_NAME, VK_EXT_IMAGE_DRM_FORMAT_MODIFIER_EXTENSION_NAME,
        VK_EXT_QUEUE_FAMILY_FOREIGN_EXTENSION_NAME, VK_EXT_RGBA10X6_FORMATS_EXTENSION_NAME, VK_KHR_BIND_MEMORY_2_EXTENSION_NAME,
        ...
        VK_KHR_GET_MEMORY_REQUIREMENTS_2_EXTENSION_NAME, VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, VK_KHR_IMAGE_FORMAT_LIST_EXTENSION_NAME,
        VK_KHR_MAINTENANCE1_EXTENSION_NAME, VK_KHR_MAINTENANCE2_EXTENSION_NAME, VK_KHR_MAINTENANCE3_EXTENSION_NAME,
        VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME, VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_SWAPCHAIN_EXTENSION_NAME,
...
static bool init_instance_extensions_and_layers(
    PFN_vkGetInstanceProcAddr getInstProc, uint32_t specVersion, TArray<VkExtensionProperties>* instanceExtensions, TArray<VkLayerProperties>* instanceLayers)
{
    ...
    for (uint32_t layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
        uint32_t extensionCount = 0;
        res = grVkEnumerateInstanceExtensionProperties((*instanceLayers)[layerIndex].layerName, &extensionCount, nullptr);
        if (VK_SUCCESS != res) {
            return false;
        }
        VkExtensionProperties* extensions = new VkExtensionProperties[extensionCount];
        res = grVkEnumerateInstanceExtensionProperties((*instanceLayers)[layerIndex].layerName, &extensionCount, extensions);
        if (VK_SUCCESS != res) {
            delete[] extensions;
            return false;
        }
...
static bool init_device_extensions_and_layers(const skgpu::VulkanGetProc& getProc, uint32_t specVersion, VkInstance inst, VkPhysicalDevice physDev,
    TArray<VkExtensionProperties>* deviceExtensions, TArray<VkLayerProperties>* deviceLayers)
{
    if (getProc == nullptr) {
        return false;
    }
    ...
    for (uint32_t layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
        uint32_t extensionCount = 0;
        res = EnumerateDeviceExtensionProperties(physDev, (*deviceLayers)[layerIndex].layerName, &extensionCount, nullptr);
        if (VK_SUCCESS != res) {
            return false;
        }
        VkExtensionProperties* extensions = new VkExtensionProperties[extensionCount];
        res = EnumerateDeviceExtensionProperties(physDev, (*deviceLayers)[layerIndex].layerName, &extensionCount, extensions);
        if (VK_SUCCESS != res) {
            delete[] extensions;
            return false;
        }
...
#define ACQUIRE_VK_INST_PROC_NOCHECK(name, instance) PFN_vk##name grVk##name = reinterpret_cast<PFN_vk##name>(getInstProc(instance, "vk" #name))
...
#define ACQUIRE_VK_PROC_NOCHECK(name, instance, device) PFN_vk##name grVk##name = reinterpret_cast<PFN_vk##name>(getProc("vk" #name, instance, device))
...
static bool destroy_instance(PFN_vkGetInstanceProcAddr getInstProc, VkInstance inst, VkDebugReportCallbackEXT* debugCallback, bool hasDebugExtension)
{
...
static bool setup_features(const skgpu::VulkanGetProc& getProc, VkInstance inst, VkPhysicalDevice physDev, uint32_t physDeviceVersion,
    skgpu::VulkanExtensions* extensions, VkPhysicalDeviceFeatures2* features, bool isProtected)
{
    SkASSERT(physDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions->hasExtension(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1));
    ...
    if (isProtected) {
        ...
        protectedMemoryFeatures = (VkPhysicalDeviceProtectedMemoryFeatures*)sk_malloc_throw(sizeof(VkPhysicalDeviceProtectedMemoryFeatures));
    ...
    if (extensions->hasExtension(VK_EXT_BLEND_OPERATION_ADVANCED_EXTENSION_NAME, 2)) {
        blend = (VkPhysicalDeviceBlendOperationAdvancedFeaturesEXT*)sk_malloc_throw(sizeof(VkPhysicalDeviceBlendOperationAdvancedFeaturesEXT));
    ...
    if (physDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions->hasExtension(VK_KHR_SAMPLER_YCBCR_CONVERSION_EXTENSION_NAME, 1)) {
        ycbcrFeature = (VkPhysicalDeviceSamplerYcbcrConversionFeatures*)sk_malloc_throw(sizeof(VkPhysicalDeviceSamplerYcbcrConversionFeatures));
...
bool CreateVkBackendContext(PFN_vkGetInstanceProcAddr getInstProc, skgpu::VulkanBackendContext* ctx, skgpu::VulkanExtensions* extensions,
    VkPhysicalDeviceFeatures2* features, VkDebugReportCallbackEXT* debugCallback, uint32_t* presentQueueIndexPtr, const CanPresentFn& canPresent,
    bool isProtected)
{
    VkResult err;
    ...
    if (!init_instance_extensions_and_layers(getInstProc, instanceVersion, &instanceExtensions, &instanceLayers)) {
        return false;
    }
    ...
    auto getProc = [getInstProc, grVkGetDeviceProcAddr](const char* proc_name, VkInstance instance, VkDevice device) {
    ...
    *debugCallback = VK_NULL_HANDLE;
    ...
    if (!init_device_extensions_and_layers(getProc, physDeviceVersion, inst, physDev, &deviceExtensions, &deviceLayers)) {
        destroy_instance(getInstProc, inst, debugCallback, hasDebugExtension);
        return false;
    }
    ...
    extensions->init(getProc, inst, physDev, (uint32_t)instanceExtensionNames.size(), instanceExtensionNames.begin(), (uint32_t)deviceExtensionNames.size(),
    ...
    if (physDeviceVersion >= VK_MAKE_VERSION(1, 1, 0) || extensions->hasExtension(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1)) {
        if (!setup_features(getProc, inst, physDev, physDeviceVersion, extensions, features, isProtected)) {
            destroy_instance(getInstProc, inst, debugCallback, hasDebugExtension);
            return false;
        }
    ...
    skgpu::VulkanInterface interface = skgpu::VulkanInterface(getProc, inst, device, instanceVersion, physDeviceVersion, extensions);
    SkASSERT(interface.validate(instanceVersion, physDeviceVersion, extensions));

    sk_sp<skgpu::VulkanMemoryAllocator> memoryAllocator = VkTestMemoryAllocator::Make(inst, physDev, device, physDeviceVersion, extensions, &interface);
