...
public:
    GaneshVkTestHelper(bool isProtected)
        : VkTestHelper(isProtected)
    {
    }
    ...
    sk_sp<SkSurface> createSurface(SkISize size, bool textureable, bool isProtected) override
    {
        ...
        return ProtectedUtils::CreateProtectedSkSurface(fDirectContext.get(), size, textureable, isProtected, &props);
    }
    ...
    GrDirectContext* directContext() override
    {
...
public:
    GraphiteVkTestHelper(bool isProtected)
        : VkTestHelper(isProtected)
    {
    }
    ...
    bool isValid() const override
    {
        return fContext != nullptr && fRecorder != nullptr;
    }

    sk_sp<SkSurface> createSurface(SkISize size, bool /* textureable */, bool isProtected) override
    {
        return ProtectedUtils::CreateProtectedSkSurface(fRecorder.get(), size, skgpu::Protected(isProtected));
    }
    ...
    skgpu::graphite::Recorder* recorder() override
    {
        return fRecorder.get();
    }
    skgpu::graphite::Context* context() override
    {
        return fContext.get();
    }
...
std::unique_ptr<VkTestHelper> VkTestHelper::Make(skiatest::TestType testType, bool isProtected)
{
...
bool VkTestHelper::setupBackendContext()
{
    ...
    if (!sk_gpu_test::CreateVkBackendContext(
            instProc, &fBackendContext, &fExtensions, &fFeatures, &fDebugCallback, nullptr, sk_gpu_test::CanPresentFn(), fIsProtected)) {
        return false;
    }
    fDevice = fBackendContext.fDevice;

    if (fDebugCallback != VK_NULL_HANDLE) {
        fDestroyDebugCallback = reinterpret_cast<PFN_vkDestroyDebugReportCallbackEXT>(instProc(fBackendContext.fInstance, "vkDestroyDebugReportCallbackEXT"));
    }
