...
void* ManagedBackendTexture::releaseContext(GrGpuFinishedProc wrappedProc, GrGpuFinishedContext wrappedCtx) const
{
...
void* ManagedBackendTexture::MakeYUVAReleaseContext(const sk_sp<ManagedBackendTexture> mbets[SkYUVAInfo::kMaxPlanes])
{
...
void ManagedBackendTexture::wasAdopted()
{
    fTexture = {};
}

sk_sp<ManagedBackendTexture> ManagedBackendTexture::MakeFromInfo(
    GrDirectContext* dContext, const SkImageInfo& ii, Mipmapped mipmapped, Renderable renderable, Protected isProtected)
{
    return MakeWithoutData(dContext, ii.width(), ii.height(), ii.colorType(), mipmapped, renderable, isProtected);
}

sk_sp<ManagedBackendTexture> ManagedBackendTexture::MakeFromBitmap(
    GrDirectContext* dContext, const SkBitmap& src, Mipmapped mipmapped, Renderable renderable, Protected isProtected)
{
    SkPixmap srcPixmap;
...
sk_sp<ManagedBackendTexture> ManagedBackendTexture::MakeFromPixmap(
    GrDirectContext* dContext, const SkPixmap& src, Mipmapped mipmapped, Renderable renderable, Protected isProtected)
{
    ...
    return MakeWithData(dContext, levels.data(), static_cast<int>(levels.size()), kTopLeft_GrSurfaceOrigin, renderable, isProtected);
}
...
void* ManagedGraphiteTexture::MakeYUVAReleaseContext(const sk_sp<ManagedGraphiteTexture> mbets[SkYUVAInfo::kMaxPlanes])
{
...
sk_sp<ManagedGraphiteTexture> ManagedGraphiteTexture::MakeUnInit(
    Recorder* recorder, const SkImageInfo& ii, Mipmapped mipmapped, Renderable renderable, Protected isProtected)
{
    ...
    skgpu::graphite::TextureInfo info = caps->getDefaultSampledTextureInfo(ii.colorType(), mipmapped, isProtected, renderable);
...
sk_sp<ManagedGraphiteTexture> ManagedGraphiteTexture::MakeFromPixmap(
    Recorder* recorder, const SkPixmap& src, Mipmapped mipmapped, Renderable renderable, Protected isProtected)
{
    sk_sp<ManagedGraphiteTexture> mbet = MakeUnInit(recorder, src.info(), mipmapped, renderable, isProtected);
    if (!mbet) {
        return nullptr;
    }
    ...
    if (!recorder->updateBackendTexture(mbet->fTexture, levels.data(), static_cast<int>(levels.size()))) {
        return nullptr;
    }
...
sk_sp<ManagedGraphiteTexture> ManagedGraphiteTexture::MakeMipmappedFromPixmaps(
    Recorder* recorder, SkSpan<const SkPixmap> levels, skgpu::Renderable renderable, skgpu::Protected isProtected)
{
    if (levels.empty()) {
        return nullptr;
    }
    sk_sp<ManagedGraphiteTexture> mbet = MakeUnInit(recorder, levels[0].info(), Mipmapped::kYes, renderable, isProtected);
    if (!recorder->updateBackendTexture(mbet->fTexture, levels.data(), static_cast<int>(levels.size()))) {
        return nullptr;
    }
...
sk_sp<ManagedGraphiteTexture> ManagedGraphiteTexture::MakeFromCompressedData(
    Recorder* recorder, SkISize dimensions, SkTextureCompressionType compression, sk_sp<SkData> src, skgpu::Mipmapped mipmapped, skgpu::Protected isProtected)
{
    sk_sp<ManagedGraphiteTexture> mbet(new ManagedGraphiteTexture);
    mbet->fContext = recorder->priv().context();
    const skgpu::graphite::Caps* caps = recorder->priv().caps();

    skgpu::graphite::TextureInfo info = caps->getDefaultCompressedTextureInfo(compression, mipmapped, isProtected);
    ...
    if (!recorder->updateCompressedBackendTexture(mbet->fTexture, src->data(), src->size())) {
        return nullptr;
    }
