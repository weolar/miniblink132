    ...
    , fPbufferContext(nullptr)
{
    ...
    if (!(fWindow = CreateWindow(TEXT("Griffin"), TEXT("The Invisible Man"), WS_OVERLAPPEDWINDOW, 0, 0, 1, 1, nullptr, nullptr, hInstance, nullptr))) {
    ...
    if (shareContext) {
        winShareContext = shareContext->fPbufferContext ? shareContext->fPbufferContext->getGLRC() : shareContext->fGlRenderContext;
    }
    ...
    if (nullptr == fPbufferContext) {
        if (!(fGlRenderContext = SkCreateWGLContext(fDeviceContext, 0, false, contextType, winShareContext))) {
