...
std::unique_ptr<GLGpuTimer> GLGpuTimer::MakeIfSupported(const sk_gpu_test::GLTestContext* ctx)
{
    ...
    } else if (kGL_GrGLStandard == gl->fStandard && (GrGLGetVersion(gl) > GR_GL_VER(3, 3) || gl->fExtensions.has("GL_ARB_timer_query"))) {
...
{
    return fGLGetIntegerv && fGLGenQueries && fGLDeleteQueries && fGLBeginQuery && fGLEndQuery && fGLGetQueryObjectuiv && fGLGetQueryObjectui64v;
}
...
sk_gpu_test::GpuTimer::QueryStatus GLGpuTimer::checkQueryStatus(sk_gpu_test::PlatformTimerQuery platformTimer)
{
...
void GLTestContext::overrideVersion(const char* version, const char* shadingLanguageVersion)
{
    ...
    auto getString = [wrapped = &fOriginalGLInterface->fFunctions.fGetString, version, shadingLanguageVersion](GrGLenum name) {
