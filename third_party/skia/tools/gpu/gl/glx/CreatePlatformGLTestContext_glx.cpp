...
private:
    ...
    static GLXContext CreateBestContext(bool isES, Display* display, GLXFBConfig bestFbc, GLXContext glxSharedContext);
    ...
    , fGlxPixmap(0)
{
    ...
    static int visual_attribs[] = { GLX_X_RENDERABLE, True, GLX_DRAWABLE_TYPE, GLX_PIXMAP_BIT, None };
    ...
    int fbcount;
    GLXFBConfig* fbc = glXChooseFBConfig(fDisplay, DefaultScreen(fDisplay), visual_attribs, &fbcount);
    if (!fbc) {
    ...
    if (!gluCheckExtension(reinterpret_cast<const GLubyte*>("GLX_ARB_create_context"), reinterpret_cast<const GLubyte*>(glxExts))) {
        if (kGLES_GrGLStandard != forcedGpuAPI) {
            fContext = glXCreateNewContext(fDisplay, bestFbc, GLX_RGBA_TYPE, nullptr, True);
        }
    } else {
        if (kGLES_GrGLStandard == forcedGpuAPI) {
            if (gluCheckExtension(reinterpret_cast<const GLubyte*>("GLX_EXT_create_context_es2_profile"), reinterpret_cast<const GLubyte*>(glxExts))) {
 ...
 */
GLXContext GLXGLTestContext::CreateBestContext(bool isES, Display* display, GLXFBConfig bestFbc, GLXContext glxShareContext)
{
    auto glXCreateContextAttribsARB = (PFNGLXCREATECONTEXTATTRIBSARBPROC)glXGetProcAddressARB((const GrGLubyte*)"glXCreateContextAttribsARB");
    ...
    for (int i = versions.size() - 1; i >= 0; i--) {
        ...
        flags.push_back(0);
        context = glXCreateContextAttribsARB(display, bestFbc, glxShareContext, true, &flags[0]);
