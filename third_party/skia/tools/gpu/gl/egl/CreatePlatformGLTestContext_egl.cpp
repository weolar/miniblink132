...
static EGLContext create_gles_egl_context(
    EGLDisplay display, EGLConfig surfaceConfig, EGLContext eglShareContext, EGLint eglContextClientVersion, bool createProtected)
{
...
static EGLContext create_gl_egl_context(EGLDisplay display, EGLConfig surfaceConfig, EGLContext eglShareContext, bool createProtected)
{
    ...
    , fSurface(EGL_NO_SURFACE)
{
    ...
    for (; nullptr == gl.get() && api < apiLimit; ++api) {
        ...
        EGLint numConfigs = 0;
        const EGLint configAttribs[] = { EGL_SURFACE_TYPE, EGL_PBUFFER_BIT, EGL_RENDERABLE_TYPE, gles ? EGL_OPENGL_ES2_BIT : EGL_OPENGL_BIT, EGL_RED_SIZE, 8,
        ...
        if (gles) {
            ...
            fContext = create_gles_egl_context(fDisplay, surfaceConfig, eglShareContext, 3, createProtected);
            if (EGL_NO_CONTEXT == fContext) {
                fContext = create_gles_egl_context(fDisplay, surfaceConfig, eglShareContext, 2, createProtected);
            }
#else
            fContext = create_gles_egl_context(fDisplay, surfaceConfig, eglShareContext, 2, createProtected);
#endif
        } else {
            fContext = create_gl_egl_context(fDisplay, surfaceConfig, eglShareContext, createProtected);
        }
        if (EGL_NO_CONTEXT == fContext) {
        ...
        static const EGLint kSurfaceAttribs[]
            = { EGL_WIDTH, 1, EGL_HEIGHT, 1, createProtected ? EGL_PROTECTED_CONTENT_EXT : EGL_NONE, createProtected ? EGL_TRUE : EGL_NONE, EGL_NONE };
        ...
        if (strstr(extensions, "EGL_KHR_image")) {
            ...
            fEglDestroyImageProc = (PFNEGLDESTROYIMAGEKHRPROC)eglGetProcAddress("eglDestroyImageKHR");
        }
...
{
    std::unique_ptr<sk_gpu_test::GLTestContext> ctx(new EGLGLTestContext(this->gl()->fStandard, nullptr));
    if (ctx) {
        ctx->makeCurrent();
    }
    return ctx;
