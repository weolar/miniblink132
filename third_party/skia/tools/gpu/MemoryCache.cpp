...
sk_sp<SkData> MemoryCache::load(const SkData& key)
{
    ...
    if (LOG_MEMORY_CACHE) {
        SkDebugf("Load Key: %s\n\tFound Data: %s\n\n", data_to_str(key).c_str(), data_to_str(*result->second.fData).c_str());
    }
...
void MemoryCache::writeShadersToDisk(const char* path, GrBackendApi api)
{
    ...
    for (auto it = fMap.begin(); it != fMap.end(); ++it) {
        ...
        GrPersistentCacheUtils::UnpackCachedShaders(&reader, shaders, interfacesIgnored, kGrShaderTypeCount);
