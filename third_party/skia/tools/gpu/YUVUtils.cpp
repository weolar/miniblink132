...
public:
    Generator(SkYUVAPixmaps pixmaps, sk_sp<SkColorSpace> cs)
        : SkImageGenerator(SkImageInfo::Make(pixmaps.yuvaInfo().dimensions(), kN32_SkColorType, kPremul_SkAlphaType, std::move(cs)))
...
protected:
        ...
        if (kUnknown_SkColorType == fFlattened.colorType()) {
            ...
            for (int y = 0; y < info.height(); ++y) {
                for (int x = 0; x < info.width(); ++x) {
                    ...
                    for (auto c : { SkYUVAInfo::YUVAChannels::kY, SkYUVAInfo::YUVAChannels::kU, SkYUVAInfo::YUVAChannels::kV }) {
    ...
    bool onQueryYUVAInfo(const SkYUVAPixmapInfo::SupportedDataTypes& types, SkYUVAPixmapInfo* info) const override
    {
    ...
    SkImage* src, SkYUVColorSpace cs, SkYUVAInfo::Subsampling ss, GrRecordingContext* rContext)
{
    ...
    SkYUVAInfo::PlaneConfig config = src->isOpaque() ? SkYUVAInfo::PlaneConfig::kY_U_V : SkYUVAInfo::PlaneConfig::kY_U_V_A;
    SkISize dims[SkYUVAInfo::kMaxPlanes];
    int n = SkYUVAInfo::PlaneDimensions(src->dimensions(), config, ss, kTopLeft_SkEncodedOrigin, dims);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        surf->getCanvas()->drawImageRect(src, SkRect::Make(dims[i]), SkSamplingOptions(SkFilterMode::kLinear), &paint);
...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(sk_sp<SkData> data, skgpu::Mipmapped mipmapped, sk_sp<SkColorSpace> cs)
{
...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(SkYUVAPixmaps pixmaps, skgpu::Mipmapped mipmapped, sk_sp<SkColorSpace> cs)
{
    std::unique_ptr<LazyYUVImage> image(new LazyYUVImage());
...
bool LazyYUVImage::reset(SkYUVAPixmaps pixmaps, skgpu::Mipmapped mipmapped, sk_sp<SkColorSpace> cs)
{
    if (!pixmaps.isValid()) {
        return false;
    }
...
bool LazyYUVImage::ensureYUVImage(GrRecordingContext* rContext, Type type)
{
    ...
    case Type::kFromPixmaps:
        ...
        fYUVImage[idx] = SkImages::TextureFromYUVAPixmaps(rContext, fPixmaps, fMipmapped,
    ...
    case Type::kFromTextures:
        ...
        if (auto direct = rContext->asDirectContext()) {
            ...
            for (int i = 0; i < fPixmaps.numPlanes(); ++i) {
                mbets[i]
                    = sk_gpu_test::ManagedBackendTexture::MakeFromPixmap(direct, fPixmaps.plane(i), fMipmapped, skgpu::Renderable::kNo, skgpu::Protected::kNo);
                if (mbets[i]) {
            ...
            fYUVImage[idx]
                = SkImages::TextureFromYUVATextures(direct, yuvaTextures, fColorSpace, sk_gpu_test::ManagedBackendTexture::ReleaseProc, planeRelContext);
        }
        break;
...
bool LazyYUVImage::ensureYUVImage(Recorder* recorder, Type type)
{
    ...
    case Type::kFromPixmaps:
        ...
        fYUVImage[idx] = SkImages::TextureFromYUVAPixmaps(recorder, fPixmaps, { fMipmapped == skgpu::Mipmapped::kYes },
            /*limitToMaxTextureSize=*/false, fColorSpace);
        break;
    case Type::kFromGenerator: {
        // Make sure the generator has ownership of its backing planes.
        auto generator = std::make_unique<Generator>(fPixmaps, fColorSpace);
    ...
    case Type::kFromTextures: {
        ...
        for (int i = 0; i < fPixmaps.numPlanes(); ++i) {
            ...
            mbets[i]
                = sk_gpu_test::ManagedGraphiteTexture::MakeFromPixmap(recorder, fPixmaps.plane(i), fMipmapped, skgpu::Renderable::kNo, skgpu::Protected::kNo);
            if (mbets[i]) {
                textures[i] = mbets[i]->texture();
            } else {
                return false;
        ...
        YUVABackendTextures yuvaTextures(recorder, fPixmaps.yuvaInfo(), textures);
        if (!yuvaTextures.isValid()) {
            return false;
        }
        void* imageRelContext = sk_gpu_test::ManagedGraphiteTexture::MakeYUVAReleaseContext(mbets);
        fYUVImage[idx]
            = SkImages::TextureFromYUVATextures(recorder, yuvaTextures, fColorSpace, sk_gpu_test::ManagedGraphiteTexture::ImageReleaseProc, imageRelContext);
        break;
    }
    case Type::kFromImages: {
        if (!recorder) {
        ...
        for (int i = 0; i < fPixmaps.numPlanes(); ++i) {
            ...
            if (fMipmapped == skgpu::Mipmapped::kYes) {
                mbet = ManagedGraphiteTexture::MakeUnInit(recorder, plane.info(), skgpu::Mipmapped::kYes, skgpu::Renderable::kNo);
                ...
                if (!mbet || !recorder->updateBackendTexture(mbet->texture(), levels.data(), levels.size())) {
                    return false;
                }
            } else {
                mbet = ManagedGraphiteTexture::MakeFromPixmap(recorder, plane, skgpu::Mipmapped::kNo, skgpu::Renderable::kNo);
                if (!mbet) {
                    return false;
                }
            }
            planeImgs[i] = SkImages::WrapTexture(recorder, mbet->texture(), plane.colorType(), plane.alphaType(), fColorSpace, skgpu::Origin::kTopLeft,
                genMipmaps, ManagedGraphiteTexture::ImageReleaseProc, mbet->releaseContext());
        }

        fYUVImage[idx] = SkImages::TextureFromYUVAImages(recorder, fPixmaps.yuvaInfo(), planeImgs, fColorSpace);
        break;
    }
    }
    return fYUVImage[idx] != nullptr;
