...
sk_sp<SkSurface> CreateProtectedSkSurface(GrDirectContext* dContext, SkISize size, bool textureable, bool isProtected, const SkSurfaceProps* surfaceProps)
{
    ...
    if (textureable) {
        surface = sk_gpu_test::MakeBackendTextureSurface(dContext, size, kTopLeft_GrSurfaceOrigin, 1, kRGBA_8888_SkColorType,
            /* colorSpace= */ nullptr, skgpu::Mipmapped::kNo, skgpu::Protected(isProtected), surfaceProps);
    } else {
        surface = sk_gpu_test::MakeBackendRenderTargetSurface(dContext, size, kTopLeft_GrSurfaceOrigin, 1, kRGBA_8888_SkColorType,
            /* colorSpace= */ nullptr, skgpu::Protected(isProtected), surfaceProps);
    }
    if (!surface) {
    ...
    if (textureable) {
        GrBackendTexture backendTex = SkSurfaces::GetBackendTexture(surface.get(), SkSurfaces::BackendHandleAccess::kFlushRead);
        SkASSERT(backendTex.isValid());
        SkASSERT(backendTex.isProtected() == isProtected);
    } else {
        GrBackendRenderTarget backendRT = SkSurfaces::GetBackendRenderTarget(surface.get(), SkSurfaces::BackendHandleAccess::kFlushRead);
...
void CheckImageBEProtection(SkImage* image, bool expectingProtected)
{
    ...
    bool result = SkImages::GetBackendTextureFromImage(image, &beTex,
        /* flushPendingGrContextIO= */ true, &origin);
    if (!result) {
...
sk_sp<SkImage> CreateProtectedSkImage(GrDirectContext* dContext, SkISize size, SkColor4f color, bool isProtected)
{
    ...
    sk_sp<SkImage> image = sk_gpu_test::MakeBackendTextureImage(
        dContext, ii, color, skgpu::Mipmapped::kNo, GrRenderable::kNo, kTopLeft_GrSurfaceOrigin, skgpu::Protected(isProtected));
    if (!image) {
