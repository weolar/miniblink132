...
static DEFINE_string(gitHash, "", "Git hash to include in the results.json output file, which can be ingested by Perf.");
...
static DEFINE_bool(autoTuneLoops, false,
...
static DEFINE_int(autoTuneLoopsMax, 1000000,
...
static DEFINE_int(flushEvery, 10,
...
[[maybe_unused]] static bool unused = SkFlagInfo::CreateStringFlag(
    "device-specific-bazel-config", nullptr, new CommandLineFlags::StringArray(), nullptr, "Ignored by this test runner.", nullptr);

static void validate_flags(bool isBazelTest)
{
    TestRunner::FlagValidators::AllOrNone({ { "--issue", FLAGS_issue.size() > 0 }, { "--patchset", FLAGS_patchset.size() > 0 } });
    ...
    TestRunner::FlagValidators::ExactlyOne({ { "--loops", FLAGS_loops != 0 }, { "--autoTuneLoops", FLAGS_autoTuneLoops } });
    ...
    TestRunner::FlagValidators::ExactlyOne({ { "--samples", FLAGS_samples != 0 }, { "--ms", FLAGS_ms != 0 } });
    if (FLAGS_ms == 0) {
...
static int sample_benchmark(BenchmarkTarget* target, int* loops, skia_private::TArray<double>* samples, skia_private::TArray<SkString>* statKeys,
    ...
    if (FLAGS_autoTuneLoops) {
        ...
        *loops = autoTunedLoops;
        if (*loops > FLAGS_autoTuneLoopsMax) {
            TestRunner::Log("Warning: Clamping loops from %d to %d (per the --autoTuneLoopsMax flag) for "
                            "benchmark \"%s\".",
                *loops, FLAGS_autoTuneLoopsMax, target->getBenchmark()->getUniqueName());
...
static void maybe_write_png(BenchmarkTarget* target, std::string outputDir)
{
    ...
    if (!target->getCanvas() || target->getCanvas()->imageInfo().colorType() == kUnknown_SkColorType) {
        return;
    }
    ...
    if (!target->getCanvas()->readPixels(bmp, 0, 0)) {
        TestRunner::Log("Warning: Could not read canvas pixels for benchmark \"%s\".", target->getBenchmark()->getUniqueName());
        return;
    }
    ...
    if (!SkPngEncoder::Encode(&stream, bmp.pixmap(), {})) {
        TestRunner::Log("Warning: Could not encode pixels from benchmark \"%s\" as PNG.", target->getBenchmark()->getUniqueName());
        return;
    }

    if (FLAGS_verbose) {
        TestRunner::Log("PNG for benchmark \"%s\" written to: %s", target->getBenchmark()->getUniqueName(), filename.c_str());
    }
}
...
{
    if (FLAGS_verbose)
        return SkStringPrintf("%" PRIu64, (uint64_t)(ms * 1e6));
...
{
    if (!FLAGS_autoTuneLoops) {
        TestRunner::Log("%4d/%-4dMB\t%s\t%s", sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(),
            target->getBenchmark()->getUniqueName(), surfaceConfig.c_str());
    } else if (FLAGS_quiet) {
        const char* mark = " ";
        ...
        if (stddev_percent > 10)
            mark = "!";
        TestRunner::Log("%10.2f %s\t%s\t%s", stats->median * 1e3, mark, target->getBenchmark()->getUniqueName(), surfaceConfig.c_str());
    } else if (FLAGS_csv) {
        const double stddev_percent = sk_ieee_double_divide(100 * sqrt(stats->var), stats->mean);
        SkDebugf("%g,%g,%g,%g,%g,%s,%s\n", stats->min, stats->median, stats->mean, stats->max, stddev_percent, surfaceConfig.c_str(),
            target->getBenchmark()->getUniqueName());
    } else {
        const double stddev_percent = sk_ieee_double_divide(100 * sqrt(stats->var), stats->mean);
        TestRunner::Log("%4d/%-4dMB\t%d\t%s\t%s\t%s\t%s\t%.0f%%\t%s\t%s\t%s", sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(), loops,
            HUMANIZE(stats->min), HUMANIZE(stats->median), HUMANIZE(stats->mean), HUMANIZE(stats->max), stddev_percent,
            FLAGS_ms ? to_string(samples->size()).c_str() : stats->plot.c_str(), surfaceConfig.c_str(), target->getBenchmark()->getUniqueName());
    }
...
int main(int argc, char** argv)
{
    ...
    std::string outputDir = FLAGS_outputDir.isEmpty() ? testUndeclaredOutputsDir : FLAGS_outputDir[0];
    ...
    if (FLAGS_gitHash.size() == 1) {
        ...
        TestRunner::Log("Warning: No --gitHash flag was specified. Perf ingestion ignores JSON files that "
    ...
    for (auto benchmarkFactory : BenchRegistry::Range()) {
        ...
        std::unique_ptr<BenchmarkTarget> target = BenchmarkTarget::FromConfig(surfaceConfig, benchmark.get());
        ...
        if (benchmark->isSuitableFor(target->getBackend())) {
            ...
            if (target->isCpuOrGpuBound() == SurfaceManager::CpuOrGpu::kCPU && cpuName == "" && !missingCpuOrGpuWarningLogged) {
                TestRunner::Log("Warning: The surface is CPU-bound, but flag --cpuName was not provided. "
                                "Perf traces will omit keys \"cpu_or_gpu\" and \"cpu_or_gpu_value\".");
                missingCpuOrGpuWarningLogged = true;
            }
            if (target->isCpuOrGpuBound() == SurfaceManager::CpuOrGpu::kGPU && gpuName == "" && !missingCpuOrGpuWarningLogged) {
                TestRunner::Log("Warning: The surface is GPU-bound, but flag --gpuName was not provided. "
                                "Perf traces will omit keys \"cpu_or_gpu\" and \"cpu_or_gpu_value\".");
                missingCpuOrGpuWarningLogged = true;
            }
            ...
            result.measurements["ms"] = {
                ...
                { .value = "ratio", .measurement = sk_ieee_double_divide(stats.median, stats.min) },
            };
            if (!statKeys.empty()) {
                // Based on
                ...
                for (int i = 0; i < statKeys.size(); i++) {
                    result.measurements["stats"].push_back({ .value = statKeys[i].c_str(), .measurement = statValues[i] });
                }
            }
