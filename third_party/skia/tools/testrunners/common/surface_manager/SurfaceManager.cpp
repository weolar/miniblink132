...
static std::string identify_transfer_fn(SkColorSpace* cs)
{
    ...
    case skcms_TFType_sRGBish:
        ...
        return SkStringPrintf("%.3g %.3g %.3g %.3g %.3g %.3g %.3g", tf.g, tf.a, tf.b, tf.c, tf.d, tf.e, tf.f).c_str();

    case skcms_TFType_PQish:
        if (eq(tf, SkNamedTransferFn::kPQ)) {
            return "PQ";
        }
        return SkStringPrintf("PQish %.3g %.3g %.3g %.3g %.3g %.3g", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f).c_str();

    case skcms_TFType_HLGish:
        if (eq(tf, SkNamedTransferFn::kHLG)) {
            return "HLG";
        }
        return SkStringPrintf("HLGish %.3g %.3g %.3g %.3g %.3g (%.3g)", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f + 1).c_str();
...
std::map<std::string, std::string> SurfaceManager::getGoldKeyValuePairs(std::string cpuName, std::string gpuName) const
{
...
std::map<std::string, std::string> SurfaceManager::getPerfKeyValuePairs(std::string cpuName, std::string gpuName) const
{
    return getCpuOrGpuKeyValuePairs(cpuName, gpuName);
}

std::map<std::string, std::string> SurfaceManager::getCpuOrGpuKeyValuePairs(std::string cpuName, std::string gpuName) const
{
    ...
    // this behavior.
    if ((fCpuOrGpu == CpuOrGpu::kCPU && cpuName == "") || (fCpuOrGpu == CpuOrGpu::kGPU && gpuName == "")) {
