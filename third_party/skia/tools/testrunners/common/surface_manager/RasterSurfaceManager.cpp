...
public:
    RasterSurfaceManager(sk_sp<SkSurface> surface, std::string config, SkColorInfo colorInfo)
        : SurfaceManager(config, colorInfo, CpuOrGpu::kCPU)
        , fSurface(surface)
    {
    }
...
std::unique_ptr<SurfaceManager> SurfaceManager::FromConfig(std::string config, SurfaceOptions surfaceOptions)
{
    ...
    if (config == "8888") {
        ...
        sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::Make({ surfaceOptions.width, surfaceOptions.height }, colorInfo));
