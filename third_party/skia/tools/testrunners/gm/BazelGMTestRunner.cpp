...
static WritePNGAndJSONFilesResult write_png_and_json_files(std::string name, std::map<std::string, std::string> commonKeys,
    std::map<std::string, std::string> gmGoldKeys, std::map<std::string, std::string> surfaceGoldKeys, const SkBitmap& bitmap, const char* pngPath,
    const char* jsonPath, std::set<std::string> knownDigests)
{
    ...
    bool result = hashAndEncode.encodePNG(&pngFile, md5.c_str(),
...
void run_gm(std::unique_ptr<skiagm::GM> gm, std::string config, std::map<std::string, std::string> keyValuePairs, std::string cpuName, std::string gpuName,
    std::string outputDir, std::set<std::string> knownDigests)
{
    TestRunner::Log("GM: %s", gm->getName().c_str());

    // Create surface and canvas.
    std::unique_ptr<SurfaceManager> surfaceManager = SurfaceManager::FromConfig(config, SurfaceOptions { gm->getISize().width(), gm->getISize().height() });
    ...
    if ((surfaceManager->isCpuOrGpuBound() == SurfaceManager::CpuOrGpu::kCPU && cpuName == "" && !gMissingCpuOrGpuWarningLogged)) {
    ...
    if ((surfaceManager->isCpuOrGpuBound() == SurfaceManager::CpuOrGpu::kGPU && gpuName == "" && !gMissingCpuOrGpuWarningLogged)) {
    ...
    if (result == skiagm::DrawResult::kOk) {
        ...
        WritePNGAndJSONFilesResult pngAndJSONResult = write_png_and_json_files(gm->getName().c_str(), keyValuePairs, gm->getGoldKeys(),
            surfaceManager->getGoldKeyValuePairs(cpuName, gpuName), bitmap, pngPath.c_str(), jsonPath.c_str(), knownDigests);
...
std::set<std::string> read_known_digests_file(std::string path)
{
    ...
    for (int lineNum = 1; std::getline(f, line); lineNum++) {
        ...
        if (!std::regex_match(md5, md5HashRegex)) {
            SK_ABORT("File '%s' passed via --knownDigestsFile contains an invalid entry on line "
                     "%d: '%s'",
...
int main(int argc, char** argv)
{
    ...
    if (!FLAGS_knownDigestsFile.isEmpty()) {
        ...
        TestRunner::Log("Read %zu known digests from: %s", knownDigests.size(), FLAGS_knownDigestsFile[0]);
    }
    ...
    TestRunner::Log("%d successful GMs (images written to %s).", gNumSuccessfulGMs, outputDir.c_str());
