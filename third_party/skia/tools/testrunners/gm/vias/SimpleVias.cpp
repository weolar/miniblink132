...
static GMOutput draw_via_picture(skiagm::GM* gm, SkSurface* surface, bool serialize)
{
    ...
    SkCanvas* recordingCanvas = recorder.beginRecording(gm->getISize().width(), gm->getISize().height());
    SkString msg;
    ...
    if (serialize) {
        ...
        SkDeserialProcs deserialProcs = { .fImageDataProc = [](sk_sp<SkData> data, std::optional<SkAlphaType>, void* ctx) -> sk_sp<SkImage> {
    ...
    recordedBitmap.allocPixelsFlags(surface->getCanvas()->imageInfo(), SkBitmap::kZeroPixels_AllocFlag);
    ...
    referenceBitmap.allocPixelsFlags(surface->getCanvas()->imageInfo(), SkBitmap::kZeroPixels_AllocFlag);
    ...
    if (recordedBitmap.computeByteSize() != referenceBitmap.computeByteSize()) {
        return { skiagm::DrawResult::kFail,
            SkStringPrintf("Recorded and reference bitmap dimensions do not match: "
                           ...
                           "got %zu, %d and %d",
                referenceBitmap.computeByteSize(), referenceBitmap.bounds().width(), referenceBitmap.bounds().height(), recordedBitmap.computeByteSize(),
                recordedBitmap.bounds().width(), recordedBitmap.bounds().height())
                .c_str() };
    }
    if (0 != memcmp(recordedBitmap.getPixels(), referenceBitmap.getPixels(), referenceBitmap.computeByteSize())) {
        return { skiagm::DrawResult::kFail,
            SkStringPrintf("Recorded and reference bitmap pixels do not match.\n"
                           "Recorded image:\n%s\nReference image:\n%s",
                bitmap_to_base64_data_uri(recordedBitmap).c_str(), bitmap_to_base64_data_uri(referenceBitmap).c_str())
                .c_str() };
    }
