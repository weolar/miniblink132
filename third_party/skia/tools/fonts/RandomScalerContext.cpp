...
RandomScalerContext::RandomScalerContext(sk_sp<SkRandomTypeface> face, const SkScalerContextEffects& effects, const SkDescriptor* desc, bool fakeIt)
...
SkScalerContext::GlyphMetrics RandomScalerContext::generateMetrics(const SkGlyph& origGlyph, SkArenaAlloc* alloc)
{
    ...
    const SkRect& newBounds = paint.doComputeFastBounds(proxyPath.getBounds(), &storage, SkPaint::kFill_Style);
...
void RandomScalerContext::generateImage(const SkGlyph& glyph, void* imageBuffer)
{
    ...
    SkBitmap bm;
    bm.installPixels(SkImageInfo::MakeN32Premul(glyph.width(), glyph.height()), imageBuffer, glyph.rowBytes());
    bm.eraseColor(0);
...
std::unique_ptr<SkScalerContext> SkRandomTypeface::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    return std::make_unique<RandomScalerContext>(sk_ref_sp(const_cast<SkRandomTypeface*>(this)), effects, desc, fFakeIt);
}
...
int SkRandomTypeface::onGetVariationDesignPosition(SkFontArguments::VariationPosition::Coordinate coordinates[], int coordinateCount) const
{
    return fProxy->onGetVariationDesignPosition(coordinates, coordinateCount);
}

int SkRandomTypeface::onGetVariationDesignParameters(SkFontParameters::Variation::Axis parameters[], int parameterCount) const
{
...
size_t SkRandomTypeface::onGetTableData(SkFontTableTag tag, size_t offset, size_t length, void* data) const
{
