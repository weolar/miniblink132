...
static void output_path_data(
    const SkFont& font, int emSize, SkString* ptsOut, SkTDArray<SkPath::Verb>* verbs, SkTDArray<unsigned>* charCodes, SkTDArray<SkScalar>* widths)
{
...
static void generate_index(const SkSpan<const FontFamilyDesc>& families, const FontDesc* defaultFont)
{
    ...
    for (const FontFamilyDesc& family : families) {
        for (const FontDesc& font : family.fFonts) {
            ...
            fprintf(out,
                ...
                "    },\n",
                identifier, identifier, identifier, identifier, identifier, identifier, family.fFamilyName, style.weight(), style.width(),
                slant_to_string(style.slant()));
        }
    }
    fprintf(out, "};\n\n");
    fprintf(out,
    ...
    for (const FontFamilyDesc& family : families) {
        for (const FontDesc& font : family.fFonts) {
            ...
            const SkFontStyle& style = font.fNamedStyle.fStyle;
            fprintf(out, "    { \"%s\", \"%s\", SkFontStyle(%d,%d,%s), gTestFonts[%d], \"%s\" },\n", family.fGenericName, font.fNamedStyle.fName,
                style.weight(), style.width(), slant_to_string(style.slant()), testFontsIndex, font.fFile);
            testFontsIndex++;
        }
    }
    testFontsIndex = 0;
    for (const FontFamilyDesc& family : families) {
        for (const FontDesc& font : family.fFonts) {
            fprintf(out, "    { \"Toy %s\", \"%s\", SkFontStyle(%d,%d,%s), gTestFonts[%d], \"%s\" },\n", family.fFamilyName, font.fNamedStyle.fName,
                font.fNamedStyle.fStyle.weight(), font.fNamedStyle.fStyle.width(), slant_to_string(font.fNamedStyle.fStyle.slant()), testFontsIndex,
                font.fFile);
            testFontsIndex++;
        }
    }
