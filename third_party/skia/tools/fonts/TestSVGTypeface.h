...
protected:
    void exportTtxCommon(SkWStream*, const char* type, const skia_private::TArray<GlyfInfo>* = nullptr) const;
...
private:
    TestSVGTypeface(const char* name, const SkFontStyle& style, int upem, const SkFontMetrics& metrics, SkSpan<const SkSVGTestTypefaceGlyphData> data);
    struct Glyph {
        Glyph();
        ~Glyph();
        SkPoint fOrigin;
