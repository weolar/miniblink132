...
const TestTypeface::List& TestTypeface::Typefaces()
{
    static List list = []() -> List {
        ...
        for (const auto& sub : gSubFonts) {
            ...
            existingFamily->faces.emplace_back(List::Family::Face { std::move(typeface), sub.fStyleName, isDefault });
        }
        return list;
    }();
    return list;
...
sk_sp<SkTypeface> TestTypeface::MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments&)
{
    ...
    SkString familyName;
    if (!stream->readPackedUInt(&familyNameSize)) {
        return nullptr;
    }
    familyName.resize(familyNameSize);
    if (!stream->read(familyName.data(), familyNameSize)) {
        return nullptr;
    }
    ...
    if (!stream->readPackedUInt(&slant)) {
        return nullptr;
    }
...
std::unique_ptr<SkScalerContext> TestTypeface::onCreateScalerContext(const SkScalerContextEffects& effects, const SkDescriptor* desc) const
{
    return std::make_unique<SkTestScalerContext>(sk_ref_sp(const_cast<TestTypeface*>(this)), effects, desc);
}
