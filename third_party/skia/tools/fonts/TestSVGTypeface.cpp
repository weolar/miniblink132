...
TestSVGTypeface::TestSVGTypeface(
    const char* name, const SkFontStyle& style, int upem, const SkFontMetrics& fontMetrics, SkSpan<const SkSVGTestTypefaceGlyphData> data)
...
template <typename Fn> void TestSVGTypeface::Glyph::withSVG(Fn&& fn) const
{
...
void TestSVGTypeface::getFontMetrics(SkFontMetrics* metrics) const
{
...
void TestSVGTypeface::onGetFamilyName(SkString* familyName) const
{
    *familyName = fName;
}

bool TestSVGTypeface::onGetPostScriptName(SkString*) const
{
    return false;
}
...
public:
    SkTestSVGScalerContext(sk_sp<TestSVGTypeface> face, const SkScalerContextEffects& effects, const SkDescriptor* desc)
...
protected:
    ...
    void generateImage(const SkGlyph& glyph, void* imageBuffer) override
    {
        ...
        bm.installPixels(SkImageInfo::MakeN32(glyph.width(), glyph.height(), kPremul_SkAlphaType), imageBuffer, glyph.rowBytes());
        bm.eraseColor(0);
...
std::unique_ptr<SkScalerContext> TestSVGTypeface::onCreateScalerContext(const SkScalerContextEffects& e, const SkDescriptor* desc) const
{
    return std::make_unique<SkTestSVGScalerContext>(sk_ref_sp(const_cast<TestSVGTypeface*>(this)), e, desc);
}
...
class DefaultTypeface : public TestSVGTypeface {
    ...
    static sk_sp<SkTypeface> MakeFromStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments&)
    {
        char header[kHeaderSize];
        if (stream->read(header, kHeaderSize) != kHeaderSize || 0 != memcmp(header, gHeaderString, kHeaderSize)) {
            return nullptr;
        }
...
sk_sp<TestSVGTypeface> TestSVGTypeface::Default()
{
    ...
    metrics.fFlags = SkFontMetrics::kUnderlineThicknessIsValid_Flag | SkFontMetrics::kUnderlinePositionIsValid_Flag
        | SkFontMetrics::kStrikeoutThicknessIsValid_Flag | SkFontMetrics::kStrikeoutPositionIsValid_Flag;
    metrics.fTop = -800;
    metrics.fAscent = -800;
    ...
    return sk_sp<TestSVGTypeface>(new DefaultTypeface("Emoji", SkFontStyle::Normal(), 1000, metrics, glyphs));
}
...
sk_sp<TestSVGTypeface> TestSVGTypeface::Planets()
{
    ...
    return sk_sp<TestSVGTypeface>(new PlanetTypeface("Planets", SkFontStyle::Normal(), 200, metrics, glyphs));
}

void TestSVGTypeface::exportTtxCommon(SkWStream* out, const char* type, const TArray<GlyfInfo>* glyfInfo) const
{
    ...
    if (hasNonBMP) {
        out->writeText("    <cmap_format_12 platformID=\"3\" platEncID=\"10\" format=\"12\" "
    ...
    out->writeText("    </namerecord>\n");
    out->writeText("    <namerecord nameID=\"6\" platformID=\"3\" platEncID=\"1\" langID=\"0x409\">\n");
    out->writeText("      ");
    out->writeText(fName.c_str());
    out->writeText("_");
    out->writeText(type);
...
void TestSVGTypeface::exportTtxCbdt(SkWStream* out, SkSpan<unsigned> strikeSizes) const
{
    ...
    for (size_t strikeIndex = 0; strikeIndex < strikeSizes.size(); ++strikeIndex) {
        ...
        SkFontMetrics fm;
        font.getMetrics(&fm);
        if (!SkTFitsIn<int8_t>((int)(-fm.fTop)) || !SkTFitsIn<int8_t>((int)(-fm.fBottom)) || !SkTFitsIn<uint8_t>((int)(fm.fXMax - fm.fXMin))) {
        ...
        auto exceedsCbdtLimits = [&]() {
            for (int i = 0; i < fGlyphCount; ++i) {
                ...
                if (!SkTFitsIn<int8_t>(ibounds.fLeft) || !SkTFitsIn<int8_t>(ibounds.fTop) || !SkTFitsIn<uint8_t>(ibounds.width())
                    || !SkTFitsIn<uint8_t>(ibounds.height()) || !SkTFitsIn<uint8_t>((int)advance)) {
                    return true;
                }
            }
            return false;
    ...
    for (int strikeIndex = 0; strikeIndex < goodStrikeSizes.size(); ++strikeIndex) {
        ...
        for (int i = 0; i < fGlyphCount; ++i) {
            ...
            canvas->drawSimpleText(&gid, sizeof(gid), SkTextEncoding::kGlyphID, -bounds.fLeft, -bounds.fTop, font, paint);
    ...
    for (int strikeIndex = 0; strikeIndex < goodStrikeSizes.size(); ++strikeIndex) {
        ...
        out->writeText("      </bitmapSizeTable>\n");
        out->writeText("      <eblc_index_sub_table_1 imageFormat=\"17\" firstGlyphIndex=\"1\" "
                       "lastGlyphIndex=\"1\">\n");
        for (int i = 0; i < fGlyphCount; ++i) {
            SkGlyphID gid = i;
            SkRect bounds;
...
void TestSVGTypeface::exportTtxSbix(SkWStream* out, SkSpan<unsigned> strikeSizes) const
{
    ...
    for (int i = 0; i < fGlyphCount; ++i) {
        ...
        SkSize containerSize = glyphData.size();
        SkRect bounds = SkRect::MakeXYWH(glyphData.fOrigin.fX, -glyphData.fOrigin.fY, containerSize.fWidth, containerSize.fHeight);
        SkIRect ibounds = bounds.roundOut();
...
void convert_noninflect_cubic_to_quads(const SkPoint p[4], SkScalar toleranceSqd, TArray<SkPoint, true>* quads, int sublevel = 0)
{
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(path)) {
        ...
        case SkPathVerb::kConic:
            ...
            for (int i = 0; i < converter.countQuads(); ++i) {
                quadPath->quadTo(quadPts[i * 2 + 1].fX, quadPts[i * 2 + 1].fY, quadPts[i * 2 + 2].fX, quadPts[i * 2 + 2].fY);
            }
            break;
...
public:
    SkCOLRCanvas(
        SkRect glyphBounds, const TestSVGTypeface& typeface, SkGlyphID glyphId, TestSVGTypeface::GlyfInfo* glyf, THashMap<SkColor, int>* colors, SkWStream* out)
    ...
    void onDrawArc(const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, const SkPaint& paint) override
    {
        ...
        SkPathPriv::CreateDrawArcPath(&path, SkArc::Make(oval, startAngle, sweepAngle, useCenter), fillNoPathEffect);
        this->drawPath(path, paint);
    }
