...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < frameCount; i++) {
        ...
        if (!stillImage) {
            ...
            if ((cachedFrame != SkCodec::kNoFrame) && (cachedFrame == frameInfos[i].fRequiredFrame) && cachedFramePixels.get()) {
        ...
        const SkCodec::Result result = codec->getPixels(codec->getInfo(), bm.getPixels(), bm.rowBytes(), &opts);
