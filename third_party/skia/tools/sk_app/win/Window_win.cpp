...
bool Window_win::init(HINSTANCE hInstance)
{
    ...
    */
    ...
    fHWnd = CreateWindow(gSZWindowClass, nullptr, WS_OVERLAPPEDWINDOW, gWindowX, gWindowY, gWindowWidth, gWindowHeight, nullptr, nullptr, fHInstance, nullptr);
    if (!fHWnd) {
        return false;
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    case WM_SYSKEYDOWN:
        eventHandled = window->onKey(get_key(wParam), skui::InputState::kDown, get_modifiers(message, wParam, lParam));
        break;

    case WM_KEYUP:
    case WM_SYSKEYUP:
        eventHandled = window->onKey(get_key(wParam), skui::InputState::kUp, get_modifiers(message, wParam, lParam));
        break;
    ...
    case WM_LBUTTONUP: {
        ...
        skui::InputState istate = ((wParam & MK_LBUTTON) != 0) ? skui::InputState::kDown : skui::InputState::kUp;
    ...
    case WM_MOUSEMOVE: {
        ...
        eventHandled = window->onMouse(xPos, yPos, skui::InputState::kMove, get_modifiers(message, wParam, lParam));
    } break;

    case WM_MOUSEWHEEL: {
        int xPos = GET_X_LPARAM(lParam);
        int yPos = GET_Y_LPARAM(lParam);
        eventHandled = window->onMouseWheel(GET_WHEEL_DELTA_WPARAM(wParam) > 0 ? +1.0f : -1.0f, xPos, yPos, get_modifiers(message, wParam, lParam));
    } break;
...
void Window_win::setRequestedDisplayParams(const DisplayParams& params, bool allowReattach)
{
    ...
    if (params.fMSAASampleCount != this->getRequestedDisplayParams().fMSAASampleCount && allowReattach) {
