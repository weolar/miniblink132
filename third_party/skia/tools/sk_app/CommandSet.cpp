...
void CommandSet::addCommand(SkUnichar c, const char* group, const char* description, std::function<void(void)> function)
{
    fCommands.push_back(Command(c, group, description, function));
}

void CommandSet::addCommand(skui::Key k, const char* keyName, const char* group, const char* description, std::function<void(void)> function)
{
...
void CommandSet::drawHelp(SkCanvas* canvas)
{
    ...
    std::stable_sort(fCommands.begin(), fCommands.end(), kAlphabetical_HelpMode == fHelpMode ? compareCommandKey : compareCommandGroup);
    ...
    SkScalar keyWidth = 0;
    for (Command& cmd : fCommands) {
        keyWidth = std::max(keyWidth, font.measureText(cmd.fKeyName.c_str(), cmd.fKeyName.size(), SkTextEncoding::kUTF8));
    }
    ...
    for (Command& cmd : fCommands) {
        if (kGrouped_HelpMode == fHelpMode && lastGroup != cmd.fGroup) {
            ...
            y += font.getSize();
            canvas->drawSimpleText(cmd.fGroup.c_str(), cmd.fGroup.size(), SkTextEncoding::kUTF8, x, y, groupFont, groupPaint);
            y += groupFont.getSize() + 2;
            lastGroup = cmd.fGroup;
        }

        canvas->drawSimpleText(cmd.fKeyName.c_str(), cmd.fKeyName.size(), SkTextEncoding::kUTF8, x, y, font, paint);
