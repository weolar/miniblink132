...
bool Window_unix::initWindow(Display* display)
{
    ...
    static int constexpr kChooseFBConfigAtt[] = { GLX_RENDER_TYPE, GLX_RGBA_BIT, GLX_DOUBLEBUFFER, True, GLX_STENCIL_SIZE, 8, None };
    // For some reason glXChooseVisual takes a non-const pointer to the attributes.
    int chooseVisualAtt[] = { GLX_RGBA, GLX_DOUBLEBUFFER, GLX_STENCIL_SIZE, 8, None };
    ...
    if (fVisualInfo) {
        Colormap colorMap = XCreateColormap(display, RootWindow(display, fVisualInfo->screen), fVisualInfo->visual, AllocNone);
        XSetWindowAttributes swa;
        swa.colormap = colorMap;
        swa.event_mask = kEventMask;
        fWindow = XCreateWindow(display, RootWindow(display, fVisualInfo->screen), 0, 0, // x, y
            initialWidth, initialHeight,
            0, // border width
            fVisualInfo->depth, InputOutput, fVisualInfo->visual, CWEventMask | CWColormap, &swa);
    }
#endif
    if (!fWindow) {
        // Create a simple window instead.  We will not be able to show GL
        fWindow = XCreateSimpleWindow(display, DefaultRootWindow(display), 0, 0, // x, y
            initialWidth, initialHeight,
            0, // border width
            0, // border value
            0); // background value
...
bool Window_unix::handleEvent(const XEvent& event)
{
    ...
    case ButtonPress:
        ...
        case Button1:
            this->onMouse(event.xbutton.x, event.xbutton.y, skui::InputState::kDown, get_modifiers(event));
            break;
        case Button4:
    ...
    case ButtonRelease:
        if (event.xbutton.button == Button1) {
            this->onMouse(event.xbutton.x, event.xbutton.y, skui::InputState::kUp, get_modifiers(event));
        }
        break;

    case MotionNotify:
        this->onMouse(event.xmotion.x, event.xmotion.y, skui::InputState::kMove, get_modifiers(event));
        break;
    ...
    case KeyRelease: {
        ...
        KeySym keysym = XkbKeycodeToKeysym(fDisplay, event.xkey.keycode, 0, shiftLevel);
        skui::Key key = get_key(keysym);
        (void)this->onKey(key, skui::InputState::kUp, get_modifiers(event));
    } break;
    ...
    case SelectionRequest: {
        Atom UTF8 = XInternAtom(fDisplay, "UTF8_STRING", 0), CLIPBOARD = XInternAtom(fDisplay, "CLIPBOARD", 0);
        ...
        if (fClipboardText.empty() || xsr->target != UTF8 || xsr->property == None) {
            ...
            XChangeProperty(
                fDisplay, xsr->requestor, xsr->property, UTF8, /*format=*/8, PropModeReplace, (unsigned char*)fClipboardText.data(), fClipboardText.length());
            XSendEvent(fDisplay, xsr->requestor, True, NoEventMask, (XEvent*)&xsel);
        }
    } break;
...
bool Window_unix::attach(BackendType attachType)
{
    ...
    case kGraphiteDawn_BackendType:
        fWindowContext = skwindow::MakeGraphiteDawnVulkanForXlib(winInfo, fRequestedDisplayParams);
        break;
#endif
#if defined(SK_GANESH) && defined(SK_VULKAN)
    ...
    case kGraphiteVulkan_BackendType:
        fWindowContext = skwindow::MakeGraphiteNativeVulkanForXlib(winInfo, fRequestedDisplayParams);
        break;
#endif
#if defined(SK_GANESH) && defined(SK_GL)
...
{
    Atom UTF8 = XInternAtom(fDisplay, "UTF8_STRING", 0), CLIPBOARD = XInternAtom(fDisplay, "CLIPBOARD", 0), XSEL_DATA = XInternAtom(fDisplay, "XSEL_DATA", 0);
    ...
    if (event.type == SelectionNotify && event.xselection.selection == CLIPBOARD && event.xselection.property != None) {
        ...
        XGetWindowProperty(xsel.display, xsel.requestor, xsel.property, /*offset=*/0,
            /*length=*/~0L, /*delete=*/False, AnyPropertyType, &type, &format, &nitems, &bytes_after, (unsigned char**)&data);
