...
static const std::unordered_map<int, skui::Key> ANDROID_TO_WINDOW_KEYMAP(
    { { AKEYCODE_SOFT_LEFT, skui::Key::kLeft }, { AKEYCODE_SOFT_RIGHT, skui::Key::kRight } });
...
int SkiaAndroidApp::message_callback(int fd, int events, void* data)
{
    ...
    case kTouched: {
        ...
        if (it != ANDROID_TO_WINDOW_STATEMAP.end()) {
            skiaAndroidApp->fWindow->onTouch(message.fTouchOwner, it->second, message.fTouchX, message.fTouchY);
        } else {
...
void* SkiaAndroidApp::pthread_main(void* arg)
{
    ...
    ALooper_addFd(looper, skiaAndroidApp->fPipes[0], LOOPER_ID_MESSAGEPIPE, ALOOPER_EVENT_INPUT, message_callback, skiaAndroidApp);
    ...
    skiaAndroidApp->fApp = Application::Create(argc + 1, const_cast<char**>(argv.get()), skiaAndroidApp);
...
extern "C" // extern "C" is needed for JNI (although the method itself is in C++)
    ...
    Java_org_skia_viewer_ViewerApplication_createNativeApp(JNIEnv* env, jobject application, jobject assetManager, jobjectArray args)
{
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_viewer_ViewerApplication_destroyNativeApp(JNIEnv* env, jobject application, jlong handle)
{
    auto skiaAndroidApp = (SkiaAndroidApp*)handle;
    skiaAndroidApp->postMessage(Message(kDestroyApp));
}

extern "C" JNIEXPORT void JNICALL Java_org_skia_viewer_ViewerActivity_onSurfaceCreated(JNIEnv* env, jobject activity, jlong handle, jobject surface)
{
    auto skiaAndroidApp = (SkiaAndroidApp*)handle;
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_viewer_ViewerActivity_onSurfaceChanged(JNIEnv* env, jobject activity, jlong handle, jobject surface)
{
    auto skiaAndroidApp = (SkiaAndroidApp*)handle;
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_viewer_ViewerActivity_onSurfaceDestroyed(JNIEnv* env, jobject activity, jlong handle)
{
    auto skiaAndroidApp = (SkiaAndroidApp*)handle;
    skiaAndroidApp->postMessage(Message(kSurfaceDestroyed));
}

extern "C" JNIEXPORT void JNICALL Java_org_skia_viewer_ViewerActivity_onKeyPressed(JNIEnv* env, jobject activity, jlong handle, jint keycode)
{
    auto skiaAndroidApp = (SkiaAndroidApp*)handle;
