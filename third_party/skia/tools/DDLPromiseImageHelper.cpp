...
DDLPromiseImageHelper::PromiseImageInfo::PromiseImageInfo(int index, uint32_t originalUniqueID, const SkImageInfo& ii)
    : fIndex(index)
...
void DDLPromiseImageHelper::PromiseImageInfo::setMipLevels(const SkBitmap& baseLevel, std::unique_ptr<SkMipmap> mipLevels)
{
...
sk_sp<SkPicture> DDLPromiseImageHelper::recreateSKP(GrDirectContext* dContext, SkPicture* inputPicture)
{
    SkSerialProcs procs;
...
static GrBackendTexture create_yuva_texture(GrDirectContext* direct, const SkPixmap& pm, int texIndex)
{
    ...
    auto beTex = direct->createBackendTexture(pm, kTopLeft_GrSurfaceOrigin, GrRenderable::kNo, GrProtected::kNo, markFinished, &finishedBECreate,
 ...
 */
void DDLPromiseImageHelper::CreateBETexturesForPromiseImage(GrDirectContext* direct, PromiseImageInfo* info)
{
    if (info->isYUV()) {
        ...
        bool finishedBECreate = false;
        auto markFinished = [](void* context) { *(bool*)context = true; };
        auto backendTex = direct->createBackendTexture(mipLevels.get(), info->numMipLevels(), kTopLeft_GrSurfaceOrigin, GrRenderable::kNo, GrProtected::kNo,
    ...
    for (int i = 0; i < fImageInfo.size(); ++i) {
        ...
        if (info.isYUV()) {
            ...
            for (int j = 0; j < numPixmaps; ++j) {
                ...
                GrBackendFormat backendFormat = direct->defaultBackendFormat(yuvPixmap.colorType(), GrRenderable::kNo);

                sk_sp<PromiseImageCallbackContext> callbackContext(new PromiseImageCallbackContext(direct, backendFormat));
            ...
            GrBackendFormat backendFormat = direct->defaultBackendFormat(baseLevel.colorType(), GrRenderable::kNo);
...
sk_sp<SkPicture> DDLPromiseImageHelper::reinflateSKP(sk_sp<GrContextThreadSafeProxy> threadSafeProxy, SkData* compressedPictureData)
{
...
sk_sp<SkImage> DDLPromiseImageHelper::CreatePromiseImages(const void* rawData, size_t length, void* ctxIn)
{
    ...
    if (curImage.isYUV()) {
        ...
        GrYUVABackendTextureInfo yuvaBackendTextures(yuvaInfo, backendFormats, skgpu::Mipmapped::kNo, kTopLeft_GrSurfaceOrigin);
        image = SkImages::PromiseTextureFromYUVA(procContext->fThreadSafeProxy, yuvaBackendTextures, curImage.refOverallColorSpace(),
            PromiseImageCallbackContext::PromiseImageFulfillProc, PromiseImageCallbackContext::PromiseImageReleaseProc, contexts);
        if (!image) {
            return nullptr;
        }
        for (int i = 0; i < textureCount; ++i) {
        ...
        image = SkImages::PromiseTextureFrom(procContext->fThreadSafeProxy, backendFormat, curImage.overallDimensions(), curImage.mipmapped(0),
            GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, curImage.overallColorType(), curImage.overallAlphaType(), curImage.refOverallColorSpace(),
            PromiseImageCallbackContext::PromiseImageFulfillProc, PromiseImageCallbackContext::PromiseImageReleaseProc,
...
int DDLPromiseImageHelper::addImage(SkImage* image)
{
    ...
    SkImageInfo overallII = SkImageInfo::Make(image->width(), image->height(),
        image->colorType() == kBGRA_8888_SkColorType ? kRGBA_8888_SkColorType : image->colorType(), image->alphaType(), image->refColorSpace());

    PromiseImageInfo& newImageInfo = fImageInfo.emplace_back(fImageInfo.size(), image->uniqueID(), overallII);
