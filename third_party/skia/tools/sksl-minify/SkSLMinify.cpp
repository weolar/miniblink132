...
#define M(type)                                                                                                                                                \
    if (filename.equals(#type ".sksl")) {                                                                                                                      \
        return SkSL::ModuleType::type;                                                                                                                         \
    }
    SKSL_MODULE_LIST(M)
#undef M
...
static std::forward_list<std::unique_ptr<const SkSL::Module>> compile_module_list(SkSpan<const std::string> paths, SkSL::ProgramKind kind)
{
    ...
    for (auto modulePath = paths.rbegin(); modulePath != paths.rend(); ++modulePath) {
        ...
        const SkSL::Module* parent = modules.empty() ? SkSL::ModuleLoader::Get().rootModule() : modules.front().get();
        std::unique_ptr<SkSL::Module> m = compiler.compileModule(kind, module_type_for_path(modulePath->c_str()), std::move(moduleSource), parent,
            /*shouldInline=*/false);
        if (!m) {
            return {};
        }
...
static bool generate_minified_text(std::string_view inputPath, std::string_view text, SkSL::FileOutputStream& out)
{
    ...
    for (;;) {
        ...
        if (token.fKind == TokenKind::TK_LINE_COMMENT || token.fKind == TokenKind::TK_BLOCK_COMMENT || token.fKind == TokenKind::TK_WHITESPACE) {
            continue;
        }
        std::string_view thisTokenText = stringize(token, text);
        if (token.fKind == TokenKind::TK_INVALID) {
            printf("%.*s: unable to parse '%.*s' at offset %d\n", (int)inputPath.size(), inputPath.data(), (int)thisTokenText.size(), thisTokenText.data(),
                token.fOffset);
            return false;
        }
        ...
        bool adjacentIdentifiers = maybe_identifier(lastTokenText.back()) && maybe_identifier(thisTokenText.front());
        ...
        bool adjacentPlusOrMinus = is_plus_or_minus(lastTokenText.back()) && is_plus_or_minus(thisTokenText.front());
...
static bool find_boolean_flag(SkSpan<std::string>* args, std::string_view flagName)
{
    ...
    auto iter = std::remove_if(args->begin(), args->end(), [&](const std::string& a) { return a == flagName; });
...
static ResultCode process_command(SkSpan<std::string> args)
{
    ...
    if (has_overlapping_flags({ isFrag, isVert, isCompute, isShader, isColorFilter, isBlender, isMeshFrag, isMeshVert })) {
        show_usage();
    ...
    std::forward_list<std::unique_ptr<const SkSL::Module>> modules = compile_module_list(inputPaths, gProgramKind);
    if (modules.empty()) {
        return ResultCode::kInputError;
    }
