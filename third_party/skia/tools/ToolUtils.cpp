...
SkColor color_to_565(SkColor color)
{
    ...
    SkASSERT(SkColorGetR(color) != SkColorGetG(color) || SkColorGetR(color) != SkColorGetB(color) || SkColorGetG(color) != SkColorGetB(color));
...
{
    const SkPoint center = SkPoint::Make(dst.fLeft + (dst.width() / 2.0f), dst.fTop + (dst.height() / 2.0f));
...
VariationSliders::VariationSliders(SkTypeface* typeface, SkFontArguments::VariationPosition variationPosition)
{
    if (!typeface) {
        return;
    }
    ...
    std::unique_ptr<SkFontParameters::Variation::Axis[]> copiedAxes = std::make_unique<SkFontParameters::Variation::Axis[]>(numAxes);
    ...
    auto argVariationPositionOrDefault = [&variationPosition](SkFourByteTag tag, SkScalar defaultValue) -> SkScalar {
    ...
    for (int i = 0; i < numAxes; ++i) {
        ...
        fAxisSliders[i].current = argVariationPositionOrDefault(copiedAxes[i].tag, copiedAxes[i].def);
...
void VariationSliders::readControls(const SkMetaData& controls, bool* changed)
{
    for (size_t i = 0; i < fAxisSliders.size(); ++i) {
        ...
        SkASSERT_RELEASE(controls.findScalars(tagToString(fAxisSliders[i].axis.tag).c_str(), &resultAxisVarsSize, axisVars));
...
{
    return SkSpan<const SkFontArguments::VariationPosition::Coordinate> { fCoords.get(), fAxisSliders.size() };
}
...
SkColor4f HilbertGenerator::getColor(float curLen)
{
    ...
    for (unsigned int i = 0; i < std::size(kColors) - 1; ++i) {
        if (kStops[i] <= t && t <= kStops[i + 1]) {
            ...
            return { kColors[i].fR * (1 - t) + kColors[i + 1].fR * t, kColors[i].fG * (1 - t) + kColors[i + 1].fG * t,
                kColors[i].fB * (1 - t) + kColors[i + 1].fB * t, kColors[i].fA * (1 - t) + kColors[i + 1].fA * t };
        }
    }
