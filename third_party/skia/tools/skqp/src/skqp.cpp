...
static std::vector<SkQP::UnitTest> get_unit_tests(int enforcedAndroidAPILevel)
{
    ...
    for (const skiatest::Test& test : skiatest::TestRegistry::Range()) {
        ...
        if (ctsMode != CtsEnforcement::RunMode::kSkip) {
            SkASSERTF(test.fTestType != skiatest::TestType::kCPU, "Non-GPU test was included in SkQP: %s\n", test.fName);
...
static std::vector<SkQP::SkSLErrorTest> get_sksl_error_tests(SkQPAssetManager* assetManager, int enforcedAndroidAPILevel)
{
        ...
        for (const std::string& path : paths) {
            ...
            for (auto& exclusionEntry : sExclusionRulesForSkSLTests) {
                if (std::regex_match(name.c_str(), exclusionEntry.first)
                    && exclusionEntry.second.eval(enforcedAndroidAPILevel) == CtsEnforcement::RunMode::kSkip) {
                    continue;
                }
            }
            ...
            skslErrorTests.push_back({ name.c_str(), std::string(static_cast<const char*>(shaderText->data()), shaderText->size()) });
        }
    };
    ...
    auto lt = [](const SkQP::SkSLErrorTest& a, const SkQP::SkSLErrorTest& b) { return a.name < b.name; };
...
std::vector<std::string> SkQP::executeTest(SkQP::UnitTest test)
{
    ...
    if (test->fTestType == skiatest::TestType::kGanesh) {
        ...
        if (test->fCTSEnforcement.eval(fEnforcedAndroidAPILevel) == CtsEnforcement::RunMode::kRunStrict) {
...
template <typename T> inline void write(SkWStream* wStream, const T& text)
{
