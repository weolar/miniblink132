...
#define jassert(env, cond, ret)                                                                                                                                \
    do {                                                                                                                                                       \
        if (!(cond)) {                                                                                                                                         \
            (env)->ThrowNew((env)->FindClass("java/lang/Exception"), __FILE__ ": assert(" #cond ") failed.");                                                  \
            return ret;                                                                                                                                        \
        }                                                                                                                                                      \
    } while (0)
...
template <typename T, typename F> static jobjectArray to_java_string_array(JNIEnv* env, const std::vector<T>& array, F stringizeFn)
{
...
void Java_org_skia_skqp_SkQP_nInit(JNIEnv* env, jobject object, jobject assetManager, jstring dataDir)
{
    ...
    env->SetObjectField(object, env->GetFieldID(SkQP_class, "mUnitTests", kStringArrayType), to_java_string_array(env, unitTests, &SkQP::GetUnitTestName));
    env->SetObjectField(
        object, env->GetFieldID(SkQP_class, "mSkSLErrorTestName", kStringArrayType), to_java_string_array(env, skslErrorTests, get_sksl_error_name));
    env->SetObjectField(
        object, env->GetFieldID(SkQP_class, "mSkSLErrorTestShader", kStringArrayType), to_java_string_array(env, skslErrorTests, get_sksl_error_shader_text));
}

jobjectArray Java_org_skia_skqp_SkQP_nExecuteUnitTest(JNIEnv* env, jobject object, jint index)
{
