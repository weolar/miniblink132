...
void RunWithGraphiteTestContexts(GraphiteTestFn* test, ContextTypeFilterFn* filter, Reporter* reporter, const TestOptions& options)
{
...
void SkQP::printBackendInfo(const char* dstPath)
{
    ...
    for (auto contextType : contextsToDump) {
        ...
        case skgpu::ContextType::kGLES:
            testCtx.reset(sk_gpu_test::CreatePlatformGLTestContext(kGLES_GrGLStandard, nullptr));
            break;
#endif
#ifdef SK_VULKAN
