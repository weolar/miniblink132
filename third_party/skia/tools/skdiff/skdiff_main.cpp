...
struct DiffSummary {
    ...
    void printContents(const FileArray& fileArray, const char* baseStatus, const char* comparisonStatus, bool listFilenames)
    {
        ...
        printf("%d file pairs %s in baseDir and %s in comparisonDir", n, baseStatus, comparisonStatus);
        if (listFilenames) {
            printf(": ");
            for (int i = 0; i < n; ++i) {
        ...
        for (int base = 0; base < DiffResource::kStatusCount; ++base) {
            ...
            for (int comparison = 0; comparison < DiffResource::kStatusCount; ++comparison) {
                ...
                if (fileArray.size() > 0) {
                    ...
                    printContents(
                        fileArray, DiffResource::getStatusDescription(baseStatus), DiffResource::getStatusDescription(comparisonStatus), listFilenames);
                }
            }
        }
    }
    ...
    void print(
        bool listFilenames, bool failOnResultType[DiffRecord::kResultCount], bool failOnStatusType[DiffResource::kStatusCount][DiffResource::kStatusCount])
    {
...
static void get_file_list_subdir(const SkString& rootDir, const SkString& subDir, const StringArray& matchSubstrings, const StringArray& nomatchSubstrings,
    ...
    if (recurseIntoSubdirs) {
        ...
        while (dirIterator.next(&dirName, true)) {
            ...
            if (!string_contains_any_of(pathRelativeToRootDir, nomatchSubstrings)) {
                get_file_list_subdir(rootDir, pathRelativeToRootDir, matchSubstrings, nomatchSubstrings, recurseIntoSubdirs, files);
            }
        }
    }
}
...
static void get_file_list(
    const SkString& dir, const StringArray& matchSubstrings, const StringArray& nomatchSubstrings, bool recurseIntoSubdirs, FileArray* files)
{
    get_file_list_subdir(dir, SkString(""), matchSubstrings, nomatchSubstrings, recurseIntoSubdirs, files);
}
...
static void create_diff_images(DiffMetricProc dmp, const int colorThreshold, bool ignoreColorSpace, RecordArray* differences, const SkString& baseDir,
    const SkString& comparisonDir, const SkString& outputDir, const StringArray& matchSubstrings, const StringArray& nomatchSubstrings, bool recurseIntoSubdirs,
    bool getBounds, bool verbose, DiffSummary* summary)
{
    ...
    get_file_list(comparisonDir, matchSubstrings, nomatchSubstrings, recurseIntoSubdirs, &comparisonFiles);

    if (!baseFiles.empty()) {
        qsort(baseFiles.begin(), baseFiles.size(), sizeof(SkString), SkCastForQSort(compare_file_name_metrics));
    }
    if (!comparisonFiles.empty()) {
        qsort(comparisonFiles.begin(), comparisonFiles.size(), sizeof(SkString), SkCastForQSort(compare_file_name_metrics));
    }
    ...
    while (i < baseFiles.size() && j < comparisonFiles.size()) {
        ...
        if (v < 0) {
            ...
            if (nullptr == baseFileBits || nullptr == comparisonFileBits) {
                ...
                if (DiffResource::kDecoded_Status == drp.fBase.fStatus && DiffResource::kDecoded_Status == drp.fComparison.fStatus) {
                    create_and_write_diff_image(&drp, dmp, colorThreshold, outputDir, drp.fBase.fFilename);
                } else {
                    drp.fResult = DiffRecord::kCouldNotCompare_Result;
                }
            }
...
int main(int argc, char** argv)
{
    ...
    create_diff_images(diffProc, colorThreshold, ignoreColorSpace, &differences, baseDir, comparisonDir, outputDir, matchSubstrings, nomatchSubstrings,
        recurseIntoSubdirs, generateDiffs, verbose, &summary);
    ...
    if (generateDiffs) {
        print_diff_page(summary.fNumMatches, colorThreshold, differences, baseDir, comparisonDir, outputDir);
    }
