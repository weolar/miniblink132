...
static bool detect_shader_settings(
    const std::string& text, SkSL::ProgramSettings* settings, const SkSL::ShaderCaps** caps, std::unique_ptr<SkSL::DebugTracePriv>* debugTrace)
{
...
static ResultCode process_command(SkSpan<std::string> args)
{
    ...
    if (skstd::ends_with(outputPath, ".spirv")) {
        return compileProgram([](SkSL::Compiler& compiler, const SkSL::ShaderCaps* shaderCaps, SkSL::Program& program, SkSL::OutputStream& out) {
            return SkSL::ToSPIRV(program, shaderCaps, out, SkSL::ValidateSPIRVAndDissassemble);
        });
    } else if (skstd::ends_with(outputPath, ".asm.frag") || skstd::ends_with(outputPath, ".asm.vert") || skstd::ends_with(outputPath, ".asm.comp")) {
        return compileProgram([](SkSL::Compiler& compiler, const SkSL::ShaderCaps* shaderCaps, SkSL::Program& program, SkSL::OutputStream& out) {
            // Compile program to SPIR-V assembly in a string-stream.
            SkSL::StringStream assembly;
            if (!SkSL::ToSPIRV(program, shaderCaps, assembly, SkSL::ValidateSPIRVAndDissassemble)) {
                return false;
            }
            ...
            if (!tools.Disassemble((const uint32_t*)spirv.data(), spirv.size() / 4, &disassembly, options)) {
                return false;
            }
        ...
        return compileProgram([](SkSL::Compiler& compiler, const SkSL::ShaderCaps* shaderCaps, SkSL::Program& program, SkSL::OutputStream& out) {
            return SkSL::ToWGSL(program, shaderCaps, out, SkSL::PrettyPrint::kYes, SkSL::IncludeSyntheticCode::kYes, SkSL::ValidateWGSL);
        });
    } else if (skstd::ends_with(outputPath, ".skrp")) {
        settings.fMaxVersionAllowed = SkSL::Version::k300;
        return compileProgramAsRuntimeShader(
            [&](SkSL::Compiler& compiler, const SkSL::ShaderCaps* shaderCaps, SkSL::Program& program, SkSL::OutputStream& out) {
                ...
                std::unique_ptr<SkSL::RP::Program> rasterProg = SkSL::MakeRasterPipelineProgram(program, *main->definition(), &skrpDebugTrace, wantTraceOps);
                if (!rasterProg) {
        ...
        return compileProgram([](SkSL::Compiler&, const SkSL::ShaderCaps* shaderCaps, SkSL::Program& program, SkSL::OutputStream& out) {
            ...
            // named parameters.
            Callbacks callbacks;
            SkSL::PipelineStage::ConvertProgram(program, "_coords", "_inColor", "_canvasColor", &callbacks);
