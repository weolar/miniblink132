...
void DDLTileHelper::TileData::init(
    int id, GrDirectContext* direct, const GrSurfaceCharacterization& dstSurfaceCharacterization, const SkIRect& clip, const SkIRect& paddingOutsets)
{
    fID = id;
    fClip = clip;
    fPaddingOutsets = paddingOutsets;

    fPlaybackChar = dstSurfaceCharacterization.createResized(this->paddedRectSize().width(), this->paddedRectSize().height());
    SkASSERT(fPlaybackChar.isValid());

    SkDEBUGCODE(const GrCaps* caps = direct->priv().caps());
    SkASSERT(caps->isFormatTexturable(fPlaybackChar.backendFormat(), fPlaybackChar.backendFormat().textureType()));
...
void DDLTileHelper::createComposeDDL()
{
    ...
    for (int i = 0; i < this->numTiles(); ++i) {
        ...
        sk_sp<SkImage> promiseImage = tile->makePromiseImageForDst(recordingCanvas->recordingContext()->threadSafeProxy());
        ...
        recordingCanvas->drawImageRect(promiseImage.get(), SkRect::Make(srcRect), dstRect, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
    }
...
sk_sp<SkSurface> DDLTileHelper::TileData::makeWrappedTileDest(GrRecordingContext* rContext)
{
    ...
    return SkSurfaces::WrapBackendTexture(rContext, promiseImageTexture->backendTexture(), fPlaybackChar.origin(), fPlaybackChar.sampleCount(),
        fPlaybackChar.colorType(), fPlaybackChar.refColorSpace(), &fPlaybackChar.surfaceProps());
}
...
sk_sp<SkImage> DDLTileHelper::TileData::makePromiseImageForDst(sk_sp<GrContextThreadSafeProxy> threadSafeProxy)
{
    ...
    sk_sp<SkImage> promiseImage = SkImages::PromiseTextureFrom(std::move(threadSafeProxy), fCallbackContext->backendFormat(), this->paddedRectSize(),
        skgpu::Mipmapped::kNo, GrSurfaceOrigin::kBottomLeft_GrSurfaceOrigin, fPlaybackChar.colorType(), kPremul_SkAlphaType, fPlaybackChar.refColorSpace(),
...
void DDLTileHelper::TileData::CreateBackendTexture(GrDirectContext* direct, TileData* tile)
{
    ...
    GrBackendTexture beTex
        = direct->createBackendTexture(c.width(), c.height(), c.colorType(), skgpu::Mipmapped(c.isMipMapped()), GrRenderable::kYes, GrProtected::kNo,
...
void DDLTileHelper::TileData::DeleteBackendTexture(GrDirectContext* dContext, TileData* tile)
{
    ...
    SkASSERT(!tile->fCallbackContext->promiseImageTexture() || tile->fCallbackContext->promiseImageTexture()->backendTexture().isValid());
...
DDLTileHelper::DDLTileHelper(GrDirectContext* direct, const GrSurfaceCharacterization& dstChar, const SkIRect& viewport, int numXDivisions, int numYDivisions,
    ...
    , fDstCharacterization(dstChar)
{
    ...
    for (int y = 0, yOff = 0; y < fNumYDivisions; ++y, yOff += yTileSize) {
        ...
        for (int x = 0, xOff = 0; x < fNumXDivisions; ++x, xOff += xTileSize) {
            ...
            fTiles[y * fNumXDivisions + x].init(y * fNumXDivisions + x, direct, dstChar, clip, { lPad, tPad, rPad, bPad });
        }
    }
}
...
void DDLTileHelper::kickOffThreadedWork(SkTaskGroup* recordingTaskGroup, SkTaskGroup* gpuTaskGroup, GrDirectContext* dContext, SkPicture* picture)
{
