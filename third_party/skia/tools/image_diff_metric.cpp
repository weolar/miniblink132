...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (std::unique_ptr<SkCodec> codec = SkCodec::MakeFromData(SkData::MakeFromFileName(path))) {
    ...
    printf("%g\n", (double)totalDiffs / ((uint64_t)255 * 4 * (uint64_t)dim.fWidth * (uint64_t)dim.fHeight));
    return 0;
}
