...
SkEventTracer::Handle ChromeTracingTracer::addTraceEvent(char phase, const uint8_t* categoryEnabledFlag, const char* name, uint64_t id, int numArgs,
...
void ChromeTracingTracer::updateTraceEventDuration(const uint8_t* categoryEnabledFlag, const char* name, SkEventTracer::Handle handle)
{
...
static void trace_value_to_json(SkJSONWriter* writer, uint64_t argValue, uint8_t argType, const char* stringTableBase)
{
    ...
    case TRACE_VALUE_TYPE_BOOL:
        writer->appendBool(argValue);
        break;
    case TRACE_VALUE_TYPE_UINT:
        writer->appendU64(argValue);
        break;
    case TRACE_VALUE_TYPE_INT:
        writer->appendS64(static_cast<int64_t>(argValue));
        break;
    case TRACE_VALUE_TYPE_DOUBLE:
        writer->appendDouble(sk_bit_cast<double>(argValue));
        break;
    ...
    case TRACE_VALUE_TYPE_COPY_STRING:
        ...
        break;
    default:
        writer->appendNString("<unknown type>");
        break;
    }
}
...
static void trace_event_to_json(SkJSONWriter* writer, TraceEvent* traceEvent, TraceEventSerializationState* serializationState)
{
    ...
    uint64_t relativeTimestamp = static_cast<int64_t>(traceEvent->fClockBegin - serializationState->fClockOffset);
    ...
    if (traceEvent->fNumArgs) {
        ...
        if (TRACE_EVENT_PHASE_SNAPSHOT_OBJECT == traceEvent->fPhase && baseTypeResolver->find(traceEvent->fID)
...
void ChromeTracingTracer::flush()
{
    ...
    auto event_block_to_json = [](SkJSONWriter* writer, const TraceEventBlock& block, TraceEventSerializationState* serializationState) {
