...
SkEventTracer::Handle SkDebugfTracer::addTraceEvent(char phase, const uint8_t* categoryEnabledFlag, const char* name, uint64_t id, int numArgs,
    const char** argNames, const uint8_t* argTypes, const uint64_t* argValues, uint8_t flags)
{
    ...
    if (open) {
        ...
        SkDebugf("[% 2d]%s <%s> %s%s #%d {\n", (int)fIndent.size(), fIndent.c_str(), category, name, args.c_str(), fCnt);
        fIndent.append(" ");
    } else {
...
void SkDebugfTracer::updateTraceEventDuration(const uint8_t* categoryEnabledFlag, const char* name, SkEventTracer::Handle handle)
{
