...
public:
    ...
    void print(const SkRecords::DrawAnnotation& command, double ns)
    {
        ...
        printf("DrawAnnotation [%g %g %g %g] %s\n", command.rect.left(), command.rect.top(), command.rect.right(), command.rect.bottom(), command.key.c_str());
    }

private:
    template <typename T> void printNameAndTime(const T& command, double ns)
    {
        int us = (int)(ns * 1e-3);
        if (!FLAGS_timeWithCommand) {
            printf("%6dus  ", us);
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < FLAGS_skps.size(); i++) {
        ...
        if (FLAGS_write.size() > 0) {
            SkPictureRecorder r;
            SkRecordDraw(record, r.beginRecording(SkRect::MakeIWH(w, h)), nullptr, nullptr, 0, nullptr, nullptr);
