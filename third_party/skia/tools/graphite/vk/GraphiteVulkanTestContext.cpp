...
std::unique_ptr<GraphiteTestContext> VulkanTestContext::Make()
{
    ...
    return std::unique_ptr<GraphiteTestContext>(new VulkanTestContext(backendContext, extensions, features, debugCallback, destroyCallback));
}

#define ACQUIRE_VK_PROC_LOCAL(name, inst)                                                                                                                      \
    PFN_vk##name localVk##name = reinterpret_cast<PFN_vk##name>(fVulkan.fGetProc("vk" #name, inst, nullptr));                                                  \
    do {                                                                                                                                                       \
...
std::unique_ptr<skgpu::graphite::Context> VulkanTestContext::makeContext(const TestOptions& options)
{
