...
void FetchUniqueKeys(GlobalCache* globalCache, std::vector<UniqueKey>* keys)
{
    ...
    globalCache->forEachGraphicsPipeline([keys](const UniqueKey& key, const GraphicsPipeline* pipeline) { keys->push_back(key); });
}

#ifdef SK_DEBUG
void DumpDescs(const RendererProvider* rendererProvider, const ShaderCodeDictionary* dict, const GraphicsPipelineDesc& pipelineDesc, const RenderPassDesc& rpd)
{
    ...
    SkDebugf("   clearColor: %.2f %.2f %.2f %.2f\n"
             ...
             "   writeSwizzle: %s\n"
             "   sampleCount: %u\n",
        rpd.fClearColor[0], rpd.fClearColor[1], rpd.fClearColor[2], rpd.fClearColor[3], rpd.fClearDepth, rpd.fClearStencil,
        rpd.fWriteSwizzle.asString().c_str(), rpd.fSampleCount);
}
#endif // SK_DEBUG
...
bool ExtractKeyDescs(Context* context, const UniqueKey& origKey, GraphicsPipelineDesc* pipelineDesc, RenderPassDesc* renderPassDesc)
{
    ...
    bool extracted = caps->extractGraphicsDescs(origKey, pipelineDesc, renderPassDesc, rendererProvider);
    if (!extracted) {
        SkASSERT(0);
        return false;
    }
