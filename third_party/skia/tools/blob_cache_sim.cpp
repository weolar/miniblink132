...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        for (const SkTextBlobTrace::Record& record : trace) {
            ...
            bool fastByPass = paint.getStyle() == SkPaint::kFill_Style && paint.getPathEffect() == nullptr && paint.getMaskFilter() == nullptr;
            if (fastByPass) {
                uint64_t blobID = record.origUniqueID;
                SkPoint offset = record.offset;
                SkColor c = SkMaskGamma::CanonicalColor(paint.getColor());
                uint32_t colorBits = (SkColorGetR(c) >> 5u) << 6u | (SkColorGetG(c) >> 5u) << 3u | SkColorGetB(c) >> 5u;
        ...
        std::cerr << "trace: " << filename << " unique: " << counts.size() << " all: " << total << " ratio: " << (float)total / counts.size() << "\n";
        ...
        sk_sp<SkSurface> surf = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(iBounds.width() + 16, iBounds.height() + 16));
