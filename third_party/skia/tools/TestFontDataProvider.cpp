...
TestFontDataProvider::TestFontDataProvider(const std::string& fontFilterRegexp, const std::string& langFilterRegexp)
    : fFontFilter(fontFilterRegexp)
    , fLangFilter(langFilterRegexp)
{
    ...
    SkASSERTF(
        jsonTestData && jsonTestData->size(), "Unable to access font test metadata at location %s, check bin/fetch-fonts-testdata.", testDataLocation.c_str());
    fJsonDom = std::make_unique<skjson::DOM>(reinterpret_cast<const char*>(jsonTestData->bytes()), jsonTestData->size());
    const skjson::ObjectValue& root = fJsonDom->root().as<skjson::ObjectValue>();
    fFonts = root["fonts"];
    SkASSERT(fFonts);
    SkASSERTF(fFonts->size() == kExpectNumFonts, "Unable to access all %zu test fonts (only got %zu), check bin/fetch-fonts-testdata.", kExpectNumFonts,
        fFonts->size());
...
bool TestFontDataProvider::next(TestSet* testSet)
{
    while (testSet && fFontsIndex < fFonts->size()) {
        ...
        if (std::regex_match(fontNameStr, match, fFontFilter)) {
            ...
            testSet->fontFilename = prefixWithFontsPath(SkString(fontFilename->str().data(), fontFilename->str().size()));
            testSet->langSamples = getLanguageSamples((*fontsEntry)["languages"].as<skjson::ArrayValue>());
            return true;
        }
    }
    return false;
...
std::vector<TestFontDataProvider::LangSample> TestFontDataProvider::getLanguageSamples(const skjson::ArrayValue* languages)
{
