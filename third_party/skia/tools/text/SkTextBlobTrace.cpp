...
void SkTextBlobTrace::DumpTrace(const std::vector<SkTextBlobTrace::Record>& trace)
{
    for (const SkTextBlobTrace::Record& record : trace) {
        ...
        bool weirdPaint = p.getStyle() != SkPaint::kFill_Style || p.getMaskFilter() != nullptr || p.getPathEffect() != nullptr;
        ...
        while (!iter.done()) {
            ...
            SkDebugf("Font %u %g %g %g %d %d %d\n    ", font.getTypeface()->uniqueID(), font.getSize(), font.getScaleX(), font.getSkewX(),
                SkFontPriv::Flags(font), (int)font.getEdging(), (int)font.getHinting());
...
void SkTextBlobTrace::Capture::capture(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
