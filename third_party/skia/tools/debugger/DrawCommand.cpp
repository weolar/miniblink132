...
DrawCommand::DrawCommand(OpType type)
    : fOpType(type)
    , fVisible(true)
{
}
...
const char* DrawCommand::GetCommandString(OpType type)
{
    ...
    case kClipRRect_OpType:
        return "ClipRRect";
    case kResetClip_OpType:
        return "ResetClip";
    case kConcat_OpType:
        return "Concat";
    case kConcat44_OpType:
        return "Concat44";
    ...
    case kDrawImage_OpType:
        return "DrawImage";
    case kDrawImageLattice_OpType:
        return "DrawImageLattice";
    case kDrawImageRect_OpType:
        return "DrawImageRect";
    case kDrawImageRectLayer_OpType:
        return "DrawImageRectLayer";
    ...
    case kDrawRRect_OpType:
        return "DrawRRect";
    case kDrawRegion_OpType:
        return "DrawRegion";
    case kDrawShadow_OpType:
        return "DrawShadow";
    case kDrawTextBlob_OpType:
        return "DrawTextBlob";
    case kDrawVertices_OpType:
        return "DrawVertices";
    case kDrawAtlas_OpType:
        return "DrawAtlas";
    case kDrawDrawable_OpType:
        return "DrawDrawable";
    case kDrawEdgeAAQuad_OpType:
        return "DrawEdgeAAQuad";
    case kDrawEdgeAAImageSet_OpType:
        return "DrawEdgeAAImageSet";
...
void xlate_and_scale_to_bounds(SkCanvas* canvas, const SkRect& bounds)
{
    ...
    if (bounds.width() > bounds.height()) {
        canvas->scale(SkDoubleToScalar((kInsetFrac * size.fWidth) / bounds.width()), SkDoubleToScalar((kInsetFrac * size.fHeight) / bounds.width()));
    } else {
        canvas->scale(SkDoubleToScalar((kInsetFrac * size.fWidth) / bounds.height()), SkDoubleToScalar((kInsetFrac * size.fHeight) / bounds.height()));
    }
...
void DrawCommand::MakeJsonPath(SkJSONWriter& writer, const SkPath& path)
{
    ...
    writer.appendString(DEBUGCANVAS_ATTRIBUTE_DUMP, static_cast<const char*>(data->data()), data->size());
    ...
    case SkPathFillType::kInverseWinding:
        writer.appendNString(DEBUGCANVAS_ATTRIBUTE_FILLTYPE, DEBUGCANVAS_FILLTYPE_INVERSEWINDING);
        break;
    case SkPathFillType::kInverseEvenOdd:
        writer.appendNString(DEBUGCANVAS_ATTRIBUTE_FILLTYPE, DEBUGCANVAS_FILLTYPE_INVERSEEVENODD);
        break;
    }
...
static const char* clipop_name(SkClipOp op)
{
    ...
    case SkClipOp::kDifference:
        return DEBUGCANVAS_CLIPOP_DIFFERENCE;
    case SkClipOp::kIntersect:
        return DEBUGCANVAS_CLIPOP_INTERSECT;
    default:
        SkASSERT(false);
...
static const char* pointmode_name(SkCanvas::PointMode mode)
{
    ...
    case SkCanvas::kPoints_PointMode:
        return DEBUGCANVAS_POINTMODE_POINTS;
    case SkCanvas::kLines_PointMode:
        return DEBUGCANVAS_POINTMODE_LINES;
    case SkCanvas::kPolygon_PointMode:
        return DEBUGCANVAS_POINTMODE_POLYGON;
    default:
        SkASSERT(false);
...
void DrawCommand::flatten(const SkFlattenable* flattenable, SkJSONWriter& writer, UrlDataManager& urlDataManager)
{
    ...
    SkString url = encode_data(data, buffer.bytesWritten(), "application/octet-stream", urlDataManager);
...
bool DrawCommand::flatten(const SkImage& image, SkJSONWriter& writer, UrlDataManager& urlDataManager)
{
    ...
    SkImageInfo dstInfo = SkImageInfo::Make(image.dimensions(), kN32_SkColorType, kPremul_SkAlphaType);
...
static const char* color_type_name(SkColorType colorType)
{
    ...
    case kARGB_4444_SkColorType:
        return DEBUGCANVAS_COLORTYPE_ARGB4444;
    case kRGBA_8888_SkColorType:
        return DEBUGCANVAS_COLORTYPE_RGBA8888;
    case kBGRA_8888_SkColorType:
        return DEBUGCANVAS_COLORTYPE_BGRA8888;
    case kRGB_565_SkColorType:
        return DEBUGCANVAS_COLORTYPE_565;
    case kGray_8_SkColorType:
        return DEBUGCANVAS_COLORTYPE_GRAY8;
    case kAlpha_8_SkColorType:
        return DEBUGCANVAS_COLORTYPE_ALPHA8;
    default:
        SkASSERT(false);
        return DEBUGCANVAS_COLORTYPE_RGBA8888;
    }
...
static const char* alpha_type_name(SkAlphaType alphaType)
{
    ...
    case kOpaque_SkAlphaType:
        return DEBUGCANVAS_ALPHATYPE_OPAQUE;
    case kPremul_SkAlphaType:
        return DEBUGCANVAS_ALPHATYPE_PREMUL;
    case kUnpremul_SkAlphaType:
        return DEBUGCANVAS_ALPHATYPE_UNPREMUL;
    default:
        SkASSERT(false);
        return DEBUGCANVAS_ALPHATYPE_OPAQUE;
    }
...
bool DrawCommand::flatten(const SkBitmap& bitmap, SkJSONWriter& writer, UrlDataManager& urlDataManager)
{
...
static void apply_font_edging(const SkFont& font, SkJSONWriter& writer)
{
    ...
    case SkFont::Edging::kSubpixelAntiAlias:
        writer.appendNString(DEBUGCANVAS_ATTRIBUTE_EDGING, DEBUGCANVAS_EDGING_SUBPIXELANTIALIAS);
        break;
    }
}
...
void DrawCommand::MakeJsonPaint(SkJSONWriter& writer, const SkPaint& paint, UrlDataManager& urlDataManager)
{
    writer.beginObject();
    store_scalar(writer, DEBUGCANVAS_ATTRIBUTE_STROKEWIDTH, paint.getStrokeWidth(), 0.0f);
    store_scalar(writer, DEBUGCANVAS_ATTRIBUTE_STROKEMITER, paint.getStrokeMiter(), SkPaintDefaults_MiterLimit);
    ...
    apply_flattenable(DEBUGCANVAS_ATTRIBUTE_IMAGEFILTER, paint.getImageFilter(), writer, urlDataManager);
    apply_flattenable(DEBUGCANVAS_ATTRIBUTE_COLORFILTER, paint.getColorFilter(), writer, urlDataManager);
...
ClearCommand::ClearCommand(SkColor color)
    : INHERITED(kClear_OpType)
{
    fColor = color;
}

void ClearCommand::execute(SkCanvas* canvas) const
{
    canvas->clear(fColor);
}
...
void ClipPathCommand::execute(SkCanvas* canvas) const
{
...
void ClipRectCommand::execute(SkCanvas* canvas) const
{
...
void ClipRRectCommand::execute(SkCanvas* canvas) const
{
...
void ClipShaderCommand::execute(SkCanvas* canvas) const
{
    canvas->clipShader(fShader, fOp);
}
...
ResetClipCommand::ResetClipCommand()
    : INHERITED(kResetClip_OpType)
{
}

void ResetClipCommand::execute(SkCanvas* canvas) const
{
...
Concat44Command::Concat44Command(const SkM44& matrix)
    : INHERITED(kConcat44_OpType)
{
    fMatrix = matrix;
}

void Concat44Command::execute(SkCanvas* canvas) const
{
    canvas->concat(fMatrix);
}
...
DrawAnnotationCommand::DrawAnnotationCommand(const SkRect& rect, const char key[], sk_sp<SkData> value)
...
DrawImageCommand::DrawImageCommand(const SkImage* image, SkScalar left, SkScalar top, const SkSamplingOptions& sampling, const SkPaint* paint)
...
DrawImageLatticeCommand::DrawImageLatticeCommand(
    const SkImage* image, const SkCanvas::Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint* paint)
...
DrawImageRectLayerCommand::DrawImageRectLayerCommand(DebugLayerManager* layerManager, const int nodeId, const int frame, const SkRect& src, const SkRect& dst,
...
void DrawOvalCommand::execute(SkCanvas* canvas) const
{
...
DrawArcCommand::DrawArcCommand(const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, const SkPaint& paint)
...
DrawPaintCommand::DrawPaintCommand(const SkPaint& paint)
    : INHERITED(kDrawPaint_OpType)
{
    fPaint = paint;
}

void DrawPaintCommand::execute(SkCanvas* canvas) const
{
...
DrawBehindCommand::DrawBehindCommand(const SkPaint& paint)
    : INHERITED(kDrawPaint_OpType)
{
    fPaint = paint;
}
...
void DrawPathCommand::execute(SkCanvas* canvas) const
{
...
void DrawRegionCommand::execute(SkCanvas* canvas) const
{
...
BeginDrawPictureCommand::BeginDrawPictureCommand(const SkPicture* picture, const SkMatrix* matrix, const SkPaint* paint)
...
DrawPointsCommand::DrawPointsCommand(SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
...
DrawTextBlobCommand::DrawTextBlobCommand(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, const SkPaint& paint)
...
DrawPatchCommand::DrawPatchCommand(const SkPoint cubics[12], const SkColor colors[4], const SkPoint texCoords[4], SkBlendMode bmode, const SkPaint& paint)
...
void DrawRectCommand::execute(SkCanvas* canvas) const
{
...
void DrawRRectCommand::execute(SkCanvas* canvas) const
{
...
DrawDRRectCommand::DrawDRRectCommand(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint)
...
void DrawShadowCommand::toJSON(SkJSONWriter& writer, UrlDataManager& urlDataManager) const
{
    ...
    bool transparentOccluder = SkToBool(fShadowRec.fFlags & SkShadowFlags::kTransparentOccluder_ShadowFlag);
...
DrawEdgeAAQuadCommand::DrawEdgeAAQuadCommand(const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aa, const SkColor4f& color, SkBlendMode mode)
...
DrawEdgeAAImageSetCommand::DrawEdgeAAImageSetCommand(const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
    const SkSamplingOptions& sampling, const SkPaint* paint, SkCanvas::SrcRectConstraint constraint)
...
{
    canvas->experimental_DrawEdgeAAImageSet(fSet.get(), fCount, fDstClips.get(), fPreViewMatrices.get(), fSampling, fPaint.getMaybeNull(), fConstraint);
}
...
DrawVerticesCommand::DrawVerticesCommand(sk_sp<SkVertices> vertices, SkBlendMode bmode, const SkPaint& paint)
...
DrawAtlasCommand::DrawAtlasCommand(const SkImage* image, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode bmode,
...
{
    canvas->drawAtlas(fImage.get(), fXform.begin(), fTex.begin(), fColors.empty() ? nullptr : fColors.begin(), fXform.size(), fBlendMode, fSampling,
...
void RestoreCommand::execute(SkCanvas* canvas) const
{
    canvas->restore();
}
...
void SaveCommand::execute(SkCanvas* canvas) const
{
    canvas->save();
}
...
void SaveLayerCommand::execute(SkCanvas* canvas) const
{
    ...
    canvas->saveLayer(
        SkCanvasPriv::ScaledBackdropLayer(fBounds.getMaybeNull(), fPaint.getMaybeNull(), fBackdrop.get(), fBackdropScale, fBackdropTileMode, fSaveLayerFlags));
}
...
SetMatrixCommand::SetMatrixCommand(const SkMatrix& matrix)
    : INHERITED(kSetMatrix_OpType)
{
    fMatrix = matrix;
}

void SetMatrixCommand::execute(SkCanvas* canvas) const
{
...
SetM44Command::SetM44Command(const SkM44& matrix)
    : INHERITED(kSetM44_OpType)
{
    fMatrix = matrix;
}

void SetM44Command::execute(SkCanvas* canvas) const
{
    canvas->setMatrix(fMatrix);
}
