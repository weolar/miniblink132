...
void DebugLayerManager::storeSkPicture(int nodeId, int frame, const sk_sp<SkPicture>& picture, SkIRect dirty)
{
...
sk_sp<SkImage> DebugLayerManager::getLayerAsImage(const int nodeId, const int frame)
{
    ...
    auto surface = SkSurfaces::Raster(SkImageInfo::Make(drawEvent.layerBounds, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType, nullptr));
...
std::vector<DebugLayerManager::LayerSummary> DebugLayerManager::summarizeLayers(int frame) const
{
    ...
    for (const auto& key : keys) {
        ...
        if (found != summaryMap.end()) {
            ...
            summaryMap.insert({ key.nodeId, { key.nodeId, lastUpdate, evt->fullRedraw, evt->layerBounds.width(), evt->layerBounds.height() } });
        }
    }
