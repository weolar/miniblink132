...
static void* win_mmap(const wchar_t* dataFile)
{
    ...
    std::unique_ptr<void, CloseHandleWrapper> mmapHandle(CreateFileMapping(file, nullptr, PAGE_READONLY, 0, 0, nullptr));
    if (!mmapHandle) {
...
bool SkLoadICU()
{
    ...
    std::call_once(flag, []() { good = load_from(executable_directory()) || load_from(library_directory()); });
    return good;
}
