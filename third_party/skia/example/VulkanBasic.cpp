...
#define ACQUIRE_INST_VK_PROC(name)                                                                                                                             \
    do {                                                                                                                                                       \
        fVk##name = reinterpret_cast<PFN_vk##name>(getProc("vk" #name, backendContext.fInstance, VK_NULL_HANDLE));                                             \
        if (fVk##name == nullptr) {                                                                                                                            \
...
int main(int argc, char** argv)
{
    ...
    // do because it is only meant for Skia testing.
    {
        ...
        if (!sk_gpu_test::CreateVkBackendContext(instProc, &backendContext, extensions.get(), features.get(), &debugCallback)) {
            return 1;
        }
    }
    ...
    backendContext.fMemoryAllocator = skgpu::VulkanMemoryAllocators::Make(backendContext, skgpu::ThreadSafe::kNo, std::nullopt);
