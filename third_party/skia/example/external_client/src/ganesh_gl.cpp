...
bool initialize_gl_linux()
{
    ...
    swa.event_mask = 0;
    Window window = XCreateWindow(display, RootWindow(display, visualInfo->screen), 0, 0, // x, y
        1280, 960, // width, height
        0, // border width
        visualInfo->depth, InputOutput, visualInfo->visual, CWEventMask | CWColormap, &swa);
...
bool initialize_gl_win()
{
    ...
    HWND window;
    if (!(window = CreateWindow(TEXT("external_client"), TEXT("my-window"), CS_OWNDC, 0, 0, 1, 1, nullptr, nullptr, hInstance, nullptr))) {
