...
int main(int argc, char* argv[])
{
    ...
    std::unique_ptr<skgpu::graphite::Context> context = skgpu::graphite::ContextFactory::MakeMetal(backendContext, options);
    if (!context) {
    ...
    sk_sp<SkImage> img;
    auto callback = [](SkImage::ReadPixelsContext ctx, std::unique_ptr<const SkImage::AsyncReadResult> result) {
        if (result->count() != 1) {
    ...
    context->asyncRescaleAndReadPixels(
        surface.get(), imageInfo, SkIRect::MakeSize({ WIDTH, HEIGHT }), SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kRepeatedCubic, callback, &img);
