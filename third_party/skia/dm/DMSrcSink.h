...
public:
    ...
    bool isOk()
    {
        return fStatus == Status::Ok;
    }
    bool isFatal()
    {
        return fStatus == Status::Fatal;
    }
    bool isSkip()
    {
        return fStatus == Status::Skip;
    }
...
struct Src {
    ...
    [[nodiscard]] virtual Result draw([[maybe_unused]] int page, SkCanvas* canvas, GraphiteTestContext* graphiteTestContext) const
    {
        return this->draw(canvas, graphiteTestContext);
    }
    virtual SkISize size([[maybe_unused]] int page) const
    {
        return this->size();
    }
...
public:
    ...
    {
        return SinkFlags { SinkFlags::kNull, SinkFlags::kDirect };
    }
};
...
public:
    ...
    Result onDraw(const Src&, SkBitmap*, SkWStream*, SkString*, const GrContextOptions& baseOptions,
        std::function<void(GrDirectContext*)> initContext = nullptr, std::function<SkCanvas*(SkCanvas*)> wrapCanvas = nullptr) const;
    ...
    SinkFlags flags() const override
    {
        SinkFlags::Multisampled ms = fSampleCount > 1 ? SinkFlags::kMultisampled : SinkFlags::kNotMultisampled;
        return SinkFlags { SinkFlags::kGPU, SinkFlags::kDirect, ms };
    }
    const GrContextOptions& baseContextOptions() const
    {
        return fBaseContextOptions;
    }
...
private:
    Result ddlDraw(const Src&, sk_sp<SkSurface> dstSurface, SkTaskGroup* recordingTaskGroup, SkTaskGroup* gpuTaskGroup, sk_gpu_test::TestContext* gpuTestCtx,
...
public:
    ...
    SinkFlags flags() const override
    {
        return SinkFlags { SinkFlags::kRaster, SinkFlags::kDirect };
    }
    void setColorSpace(sk_sp<SkColorSpace> colorSpace) override
    {
        fColorSpace = colorSpace;
...
public:
    ...
    SinkFlags flags() const override
    {
        return SinkFlags { SinkFlags::kGPU, SinkFlags::kDirect };
    }
    void setColorSpace(sk_sp<SkColorSpace> colorSpace) override
    {
        fColorSpace = colorSpace;
...
public:
    GraphitePrecompileTestingSink(const SkCommandLineConfigGraphite*, const skiatest::graphite::TestOptions&);
...
private:
    Result drawSrc(const Src&, skgpu::graphite::Context*, skiatest::graphite::GraphiteTestContext*) const;
...
public:
    explicit ViaRuntimeBlend(Sink* sink)
        : Via(sink)
    {
    }
    Result draw(const Src&, SkBitmap*, SkWStream*, SkString*) const override;
