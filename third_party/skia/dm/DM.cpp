...
static void done(const char* config, const char* src, const char* srcOptions, const char* name)
{
    ...
    if (updateDueToProgress || updateDueToTime) {
        ...
        if (lastUpdate == gLastUpdate) {
            info("\n[%d/%d] %dMB RAM, %dMB peak, %d queued, %d threads:\n\t%s  done\n", gTotalCounts - gPending, gTotalCounts, curr, peak,
                gPending - gRunning->size(), gRunning->size() + 1, id.c_str());
...
template <typename... Rest> static constexpr int max_of(int x, Rest... rest)
{
...
static void crash_handler(int sig)
{
    ...
    info("\nCaught signal %d [%s] (%dMB RAM, peak %dMB), was running:\n", sig, strsignal(sig), sk_tools::getCurrResidentSetSizeMB(),
...
static void push_src(const char* tag, ImplicitString options, Src* inSrc)
{
    ...
    if (in_shard() && FLAGS_src.contains(tag) && !CommandLineFlags::ShouldSkip(FLAGS_match, src->name().c_str())) {
...
static void push_codec_srcs(Path path)
{
...

    {
        ...
        if (frameInfos.size() > 1) {
            for (auto dstCT : { CodecSrc::kNonNative8888_Always_DstColorType, CodecSrc::kGetFromCanvas_DstColorType }) {
            ...
            for (float scale : { .5f, .33f }) {
                push_codec_src(path, CodecSrc::kAnimated_Mode, CodecSrc::kGetFromCanvas_DstColorType, kPremul_SkAlphaType, scale);
            }
        }
    }
...
template <typename T> void gather_file_srcs(const CommandLineFlags::StringArray& flags, const char* ext, const char* src_name = nullptr)
{
    if (!src_name) {
...
static bool gather_srcs()
{
    ...
    if (!FLAGS_bisect.isEmpty()) {
        ...
        push_src("bisect", "", new BisectSrc(FLAGS_bisect[0], FLAGS_bisect.size() > 1 ? FLAGS_bisect[1] : ""));
    }
...
static bool should_skip(const char* sink, const char* src, const char* srcOptions, const char* name)
{
    for (int i = 0; i < FLAGS_skip.size() - 3; i += 4) {
        if (match(FLAGS_skip[i + 0], sink) && match(FLAGS_skip[i + 1], src) && match(FLAGS_skip[i + 2], srcOptions) && match(FLAGS_skip[i + 3], name)) {
            return true;
        }
    }
    return false;
...
struct Task {
    ...
    static void Run(const Task& task)
    {
        ...
        if (!FLAGS_dryRun) {
            ...
            if (result.isFatal()) {
                fail(SkStringPrintf("%s %s %s %s: %s", task.sink.tag.c_str(), task.src.tag.c_str(), task.src.options.c_str(), name.c_str(), result.c_str()));
            }
            ...
            gDefinitelyThreadSafeWork->add([task, name, bitmap, data] {
                ...
                if (!FLAGS_readPath.isEmpty() && !gGold->contains(Gold(task.sink.tag, task.src.tag, task.src.options, name, md5))) {
                    fail(SkStringPrintf("%s not found for %s %s %s %s in %s", md5.c_str(), task.sink.tag.c_str(), task.src.tag.c_str(),
                        task.src.options.c_str(), name.c_str(), FLAGS_readPath[0]));
                }
                ...
                if (ext && !FLAGS_writePath.isEmpty()) {
                    ...
                    if (FLAGS_rasterize_pdf && SkString("pdf").equals(ext)) {
                        ...
                        SkUniqueCFRef<CGDataProviderRef> provider { CGDataProviderCreateWithData(nullptr, blob->data(), blob->size(), nullptr) };
                        ...
                        SkBitmap rasterized;
                        rasterized.allocPixels(SkImageInfo::Make(w, h, kRGBA_8888_SkColorType, kPremul_SkAlphaType));
                        rasterized.eraseColor(SK_ColorWHITE);

                        SkUniqueCFRef<CGColorSpaceRef> cs { CGColorSpaceCreateDeviceRGB() };
                        CGBitmapInfo info = kCGBitmapByteOrder32Big | (CGBitmapInfo)kCGImageAlphaPremultipliedLast;

                        SkUniqueCFRef<CGContextRef> ctx { CGBitmapContextCreate(rasterized.getPixels(), w, h, 8, rasterized.rowBytes(), cs.get(), info) };
                ...
                SkPixmap pm;
                if (FLAGS_checkF16 && bitmap.colorType() == kRGBA_F16Norm_SkColorType && bitmap.peekPixels(&pm)) {
    ...
    static SkString identify_transfer_fn(SkColorSpace* cs)
    {
        ...
        auto eq = [](skcms_TransferFunction x, skcms_TransferFunction y) {
            return x.g == y.g && x.a == y.a && x.b == y.b && x.c == y.c && x.d == y.d && x.e == y.e && x.f == y.f;
        };
        ...
        case skcms_TFType_sRGBish:
            ...
            return SkStringPrintf("%.3g %.3g %.3g %.3g %.3g %.3g %.3g", tf.g, tf.a, tf.b, tf.c, tf.d, tf.e, tf.f);

        case skcms_TFType_PQish:
            if (eq(tf, SkNamedTransferFn::kPQ)) {
                return SkString("PQ");
            }
            return SkStringPrintf("PQish %.3g %.3g %.3g %.3g %.3g %.3g", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f);

        case skcms_TFType_HLGish:
            if (eq(tf, SkNamedTransferFn::kHLG)) {
                return SkString("HLG");
            }
            return SkStringPrintf("HLGish %.3g %.3g %.3g %.3g %.3g (%.3g)", tf.a, tf.b, tf.c, tf.d, tf.e, tf.f + 1);
    ...
    static void WriteToDisk(
        const Task& task, SkString md5, const char* ext, SkStream* data, size_t len, const SkBitmap* bitmap, const HashAndEncode* hashAndEncode)
    {
        ...
        for (int i = 1; i < FLAGS_key.size(); i += 2) {
            if (0 == strcmp(FLAGS_key[i - 1], "extra_config") && 0 == strcmp(FLAGS_key[i], "OldestSupportedSkpVersion")) {
...
static int gather_tests()
{
    ...
    return gCPUTests->size() + gCPUSerialTests->size() + gGaneshTests->size() + gGraphiteTests->size();
}
...
#if defined(SK_GRAPHITE)
static void run_graphite_test(skiatest::Test test, const skiatest::graphite::TestOptions& optionsIn)
{
    DMReporter reporter;
    if (!FLAGS_dryRun && !should_skip("_", "tests", "_", test.fName)) {
        AutoreleasePool pool;
...
int main(int argc, char** argv)
{
    ...
    for (skiatest::Test& test : *gCPUSerialTests) {
        run_cpu_test(test);
    }
#endif
    ...
    for (TaggedSink& sink : *gSinks) {
        for (TaggedSrc& src : *gSrcs) {
            if (src->veto(sink->flags()) || should_skip(sink.tag.c_str(), src.tag.c_str(), src.options.c_str(), src->name().c_str())) {
                SkAutoSpinlock lock(gMutex);
                gPending--;
                continue;
            }
