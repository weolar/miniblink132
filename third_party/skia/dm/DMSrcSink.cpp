...
GMSrc::GMSrc(skiagm::GMFactory factory)
    : fFactory(factory)
{
}
...
Result GMSrc::draw(SkCanvas* canvas, GraphiteTestContext* testContext) const
{
    ...
    case skiagm::DrawResult::kFail:
        return Result(Result::Status::Fatal, msg);
    case skiagm::DrawResult::kSkip:
        return Result(Result::Status::Skip, msg);
    default:
        SK_ABORT("");
    }
    ...
    switch (drawResult) {
    case skiagm::DrawResult::kOk:
        return Result(Result::Status::Ok, msg);
    case skiagm::DrawResult::kFail:
        return Result(Result::Status::Fatal, msg);
    case skiagm::DrawResult::kSkip:
        return Result(Result::Status::Skip, msg);
...
CodecSrc::CodecSrc(Path path, Mode mode, DstColorType dstColorType, SkAlphaType dstAlphaType, float scale)
    : fPath(path)
    , fMode(mode)
    , fDstColorType(dstColorType)
...
static void draw_to_canvas(
    SkCanvas* canvas, const SkImageInfo& info, void* pixels, size_t rowBytes, CodecSrc::DstColorType dstColorType, SkScalar left = 0, SkScalar top = 0)
{
    SkBitmap bitmap;
...
Result CodecSrc::draw(SkCanvas* canvas, GraphiteTestContext*) const
{
    ...
    if (!get_decode_info(&decodeInfo, canvas->imageInfo().colorType(), fDstColorType, fDstAlphaType)) {
    ...
    case kAnimated_Mode: {
        ...
        std::vector<SkCodec::FrameInfo> frameInfos = androidCodec ? androidCodec->codec()->getFrameInfo() : codec->getFrameInfo();
        ...
        for (int i = 0; static_cast<size_t>(i) < frameInfos.size(); i++) {
            ...
            SkCodec::Result result = androidCodec ? androidCodec->getAndroidPixels(decodeInfo, pixels.get(), rowBytes, &androidOptions)
            ...
            case SkCodec::kIncompleteInput: {
                ...
                if (static_cast<size_t>(i + 1) < frameInfos.size() && frameInfos[i + 1].fRequiredFrame == i) {
            ...
            case SkCodec::kInvalidConversion:
                ...
                [[fallthrough]];
            default:
                return Result::Fatal("Couldn't getPixels for frame %i in %s.", i, fPath.c_str());
            }
        }
        break;
    }
    ...
    case kScanline_Mode: {
        ...
        if (useIncremental || ico) {
            if (SkCodec::kSuccess == codec->startIncrementalDecode(decodeInfo, dst, rowBytes, &options)) {
                int rowsDecoded;
                auto result = codec->incrementalDecode(&rowsDecoded);
                if (SkCodec::kIncompleteInput == result || SkCodec::kErrorInInput == result) {
                    codec->fillIncompleteImage(decodeInfo, dst, rowBytes, SkCodec::kNo_ZeroInitialized, height, rowsDecoded);
                }
            } else {
                if (useIncremental) {
    ...
    case kSubset_Mode: {
        ...
        for (int x = 0; x < W; x += w) {
            ...
            for (int y = 0; y < H; y += h) {
                ...
                const SkCodec::Result result = codec->getPixels(decodeInfo, dst, subsetRowBytes, &options);
                switch (result) {
                case SkCodec::kSuccess:
                case SkCodec::kErrorInInput:
                case SkCodec::kIncompleteInput:
                ...
                draw_to_canvas(canvas, subsetBitmapInfo, dst, subsetRowBytes, fDstColorType, SkIntToScalar(left), SkIntToScalar(top));
...
AndroidCodecSrc::AndroidCodecSrc(Path path, CodecSrc::DstColorType dstColorType, SkAlphaType dstAlphaType, int sampleSize)
    : fPath(path)
    , fDstColorType(dstColorType)
    , fDstAlphaType(dstAlphaType)
    , fSampleSize(sampleSize)
...
bool ImageGenSrc::veto(SinkFlags flags) const
{
    if (fIsGpu) {
        ...
        return flags.type != SinkFlags::kGPU || flags.approach != SinkFlags::kDirect || flags.multisampled == SinkFlags::kMultisampled;
    }
...
#endif
        return Result(status, SkStringPrintf("Image generator could not getPixels() for %s\n", fPath.c_str()));
    }
...
BisectSrc::BisectSrc(Path path, const char* trail)
    : INHERITED(path)
    , fTrail(trail)
{
}
...
Result BisectSrc::draw(SkCanvas* canvas, GraphiteTestContext* testContext) const
{
    ...
    public:
        PathFindingCanvas(int width, int height)
            : SkCanvas(width, height, nullptr)
        {
        }
        const TArray<FoundPath>& foundPaths() const
        {
            return fFoundPaths;
        }
    ...
    PathFindingCanvas pathFinder(canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().height());
...
#if defined(SK_ENABLE_SKOTTIE)
static DEFINE_bool(useLottieGlyphPaths, false, "Prioritize embedded glyph paths over native fonts.");
...
Result SkottieSrc::draw(SkCanvas* canvas, GraphiteTestContext*) const
{
    ...
    auto resource_provider = skresources::DataURIResourceProviderProxy::Make(
        skresources::FileResourceProvider::Make(SkOSPath::Dirname(fPath.c_str()), predecode), predecode, ToolUtils::TestFontMgr());

    static constexpr char kInterceptPrefix[] = "__";
    auto precomp_interceptor = sk_make_sp<skottie_utils::ExternalAnimationPrecompInterceptor>(resource_provider, kInterceptPrefix);
    uint32_t flags = 0;
    ...
    for (int i = 0; i < kTileCount; ++i) {
        ...
        for (int j = 0; j < kTileCount; ++j) {
            ...
            const auto t = t_rate * (frameOrder[i] * kTileCount + frameOrder[j]);
            {
                ...
                canvas->concat(SkMatrix::RectToRect(SkRect::MakeSize(animation->size()), dest, SkMatrix::kCenter_ScaleToFit));
                animation->seek(t);
...
bool SkottieSrc::veto(SinkFlags flags) const
{
    ...
    bool type_ok = flags.type == SinkFlags::kRaster || flags.type == SinkFlags::kGPU || flags.type == SinkFlags::kVector;
    ...
    , fScale(1)
{
    ...
    auto rp = skresources::DataURIResourceProviderProxy::Make(
        skresources::FileResourceProvider::Make(SkOSPath::Dirname(path.c_str()), predecode), predecode, ToolUtils::TestFontMgr());
    ...
    if (sz.isEmpty()) {
        ...
        fScale = std::max(1.f, std::max(kMinimumSVGSize.width() / sz.width(), kMinimumSVGSize.height() / sz.height()));
    }
}
...
SkISize SVGSrc::size() const
{
    ...
    return SkSize { fDom->containerSize().width() * fScale, fDom->containerSize().height() * fScale }.toRound();
}

Name SVGSrc::name() const
{
    return fName;
}
    ...
    : fPath(path)
{
    ...
    if (count > 0) {
        fPages.reset(count);
        SkASSERT_RELEASE(SkMultiPictureDocument::ReadPageSizes(stream.get(), &fPages[0], fPages.size()));
    }
}

int MSKPSrc::pageCount() const
{
    return fPages.size();
}

SkISize MSKPSrc::size() const
{
    return this->size(FLAGS_mskpFrame);
}
...
sk_sp<SkSurface> GPUSink::createDstSurface(GrDirectContext* context, SkISize size) const
{
    ...
    case SkCommandLineConfigGpu::SurfType::kDefault:
        surface = SkSurfaces::RenderTarget(context, skgpu::Budgeted::kNo, info, fSampleCount, &props);
        break;
    case SkCommandLineConfigGpu::SurfType::kBackendTexture:
        surface
            = sk_gpu_test::MakeBackendTextureSurface(context, info, kTopLeft_GrSurfaceOrigin, fSampleCount, skgpu::Mipmapped::kNo, GrProtected::kNo, &props);
        break;
    case SkCommandLineConfigGpu::SurfType::kBackendRenderTarget:
        surface = sk_gpu_test::MakeBackendRenderTargetSurface(context, info, kBottomLeft_GrSurfaceOrigin, fSampleCount, GrProtected::kNo, &props);
        break;
    }
...
Result GPUSink::onDraw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log, const GrContextOptions& baseOptions,
    std::function<void(GrDirectContext*)> initContext, std::function<SkCanvas*(SkCanvas*)> wrapCanvas) const
{
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
GPUSerializeSlugSink::GPUSerializeSlugSink(const SkCommandLineConfigGpu* config, const GrContextOptions& options)
    : GPUSink(config, options)
{
}

Result GPUSerializeSlugSink::draw(const Src& src, SkBitmap* dst, SkWStream* write, SkString* log) const
{
    GrContextOptions grOptions = this->baseContextOptions();
    // Force padded atlas entries for slug drawing.
    grOptions.fSupportBilerpFromGlyphAtlas |= true;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
GPURemoteSlugSink::GPURemoteSlugSink(const SkCommandLineConfigGpu* config, const GrContextOptions& options)
    : GPUSink(config, options)
{
}

Result GPURemoteSlugSink::draw(const Src& src, SkBitmap* dst, SkWStream* write, SkString* log) const
{
    GrContextOptions grOptions = this->baseContextOptions();
    // Force padded atlas entries for slug drawing.
    grOptions.fSupportBilerpFromGlyphAtlas |= true;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
GPUPersistentCacheTestingSink::GPUPersistentCacheTestingSink(const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
    : INHERITED(config, grCtxOptions)
    , fCacheType(config->getTestPersistentCache())
{
}

Result GPUPersistentCacheTestingSink::draw(const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const
{
...
GPUPrecompileTestingSink::GPUPrecompileTestingSink(const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
    : INHERITED(config, grCtxOptions)
{
}

Result GPUPrecompileTestingSink::draw(const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const
{
    // Three step process:
    ...
    auto precompileShaders = [&memoryCache](GrDirectContext* dContext) {
        memoryCache.foreach ([dContext](sk_sp<const SkData> key, sk_sp<SkData> data, const SkString& /*description*/, int /*count*/) {
    ...
    SkString refLog;
    SkDynamicMemoryWStream refStream;
    Result refResult = this->onDraw(src, &reference, &refStream, &refLog, replayOptions, precompileShaders);
    if (!refResult.isOk()) {
        return refResult;
    }
...
Result GPUDDLSink::ddlDraw(const Src& src, sk_sp<SkSurface> dstSurface, SkTaskGroup* recordingTaskGroup, SkTaskGroup* gpuTaskGroup,
    sk_gpu_test::TestContext* gpuTestCtx, GrDirectContext* dContext) const
{
    ...
    Result result = src.draw(recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())),
    ...
    DDLTileHelper tiles(dContext, dstCharacterization, viewport, kNumDivisions, kNumDivisions,
    ...
    gpuTaskGroup->add([dstSurface, ddl = tiles.composeDDL()]() { skgpu::ganesh::DrawDDL(dstSurface, ddl); });
...
Result GPUDDLSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log) const
{
    ...
    Result result = this->ddlDraw(src, surface, &recordingTaskGroup, &gpuTaskGroup, mainTestCtx, mainCtx);
    if (!result.isOk()) {
        return result;
    }
...
static SkSerialProcs serial_procs_using_png()
{
    ...
    procs.fImageProc = [](SkImage* img, void*) -> sk_sp<SkData> { return SkPngEncoder::Encode(as_IB(img)->directContext(), img, SkPngEncoder::Options {}); };
    return procs;
}
...
Result SVGSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const
{
    ...
    if (src.pageCount() > 1) {
        ...
        if (fPageIndex > pageCount - 1) {
            return Result::Fatal("Page index %d too high for document with only %d pages.", fPageIndex, pageCount);
        }
    }
    return src.draw(fPageIndex, SkSVGCanvas::Make(SkRect::MakeWH(SkIntToScalar(src.size().width()), SkIntToScalar(src.size().height())), dst).get(),
        /*GraphiteTestContext=*/nullptr);
#else
    (void)fPageIndex;
...
Result RasterSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString*) const
{
    ...
    if (size.isEmpty()) {
        return Result(Result::Status::Skip, SkStringPrintf("Skipping empty source: %s", src.name().c_str()));
    }

    dst->allocPixelsFlags(SkImageInfo::Make(size, this->colorInfo()), SkBitmap::kZeroPixels_AllocFlag);
...
GraphiteSink::GraphiteSink(const SkCommandLineConfigGraphite* config, const skiatest::graphite::TestOptions& options)
    : fOptions(options)
    , fContextType(config->getContextType())
    , fColorType(config->getColorType())
    , fAlphaType(config->getAlphaType())
...
Result GraphiteSink::draw(const Src& src, SkBitmap* dst, SkWStream* dstStream, SkString* log) const
{
    ...
    std::unique_ptr<skgpu::graphite::Recorder> recorder = context->makeRecorder(ToolUtils::CreateTestingRecorderOptions());
    if (!recorder) {
...

    {
        ...
        SkPixmap pm;
        if (!dst->peekPixels(&pm) || !surface->readPixels(pm, 0, 0)) {
            return Result::Fatal("Could not readback from surface.");
        }
    }
...
sk_sp<SkSurface> GraphiteSink::makeSurface(skgpu::graphite::Recorder* recorder, SkISize dimensions) const
{
    ...
    if (fOptions.fUseWGPUTextureView) {
        return sk_gpu_test::MakeBackendTextureViewSurface(recorder, ii, skgpu::Mipmapped::kNo, skgpu::Protected::kNo, &props);
    }
#endif // SK_DAWN
...
GraphitePrecompileTestingSink::GraphitePrecompileTestingSink(const SkCommandLineConfigGraphite* config, const skiatest::graphite::TestOptions& options)
...
Result GraphitePrecompileTestingSink::drawSrc(const Src& src, skgpu::graphite::Context* context, skiatest::graphite::GraphiteTestContext* testContext) const
{
    if (!fRecorder) {
...
Result GraphitePrecompileTestingSink::resetAndRecreatePipelines(skgpu::graphite::Context* context) const
{
    ...
    SkDEBUGCODE(int numBeforeReset = context->priv().globalCache()->numGraphicsPipelines();) SkASSERT(numBeforeReset == (int)origKeys.size());
...

    {
        ...
        for (const skgpu::UniqueKey& origKey : origKeys) {
            if (std::find(recreatedKeys.begin(), recreatedKeys.end(), origKey) == recreatedKeys.end()) {
                sk_sp<GraphicsPipeline> pipeline = context->priv().globalCache()->findGraphicsPipeline(origKey);
                SkASSERT(!pipeline);
...

                {
                    ...
                    UniqueKeyUtils::ExtractKeyDescs(context, origKey, &originalPipelineDesc, &originalRenderPassDesc);

                    SkDebugf("------- Missing key from rebuilt keys:\n");
                    origKey.dump("original key:");
                    UniqueKeyUtils::DumpDescs(rendererProvider, dict, originalPipelineDesc, originalRenderPassDesc);
                }
                ...
                for (const skgpu::UniqueKey& recreatedKey : recreatedKeys) {
                    ...
                    UniqueKeyUtils::ExtractKeyDescs(context, recreatedKey, &recreatedPipelineDesc, &recreatedRenderPassDesc);

                    SkDebugf("%d ----\n", count++);
                    recreatedKey.dump("recreated key:");
                    UniqueKeyUtils::DumpDescs(rendererProvider, dict, recreatedPipelineDesc, recreatedRenderPassDesc);
                }
#endif
...
Result GraphitePrecompileTestingSink::draw(const Src& src, SkBitmap* dst, SkWStream* dstStream, SkString* log) const
{
    skiatest::graphite::TestOptions options = fOptions;
    // If we've copied context options from an external source we can't trust that the
    // priv pointer is still in scope, so assume it should be NULL and set our own up.
...
static Result draw_to_canvas(Sink* sink, SkBitmap* bitmap, SkWStream* stream, SkString* log, SkISize size, const DrawToCanvasFn& draw)
{
    ...
    public:
        ProxySrc(SkISize size, const DrawToCanvasFn& draw)
            : fSize(size)
            , fDraw(draw)
        {
        }
...
Result ViaMatrix::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    ...
    return draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](SkCanvas* canvas, Src::GraphiteTestContext* testContext) {
...
Result ViaSerialization::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    ...
    result = draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](SkCanvas* canvas, Src::GraphiteTestContext*) {
...
{
    auto size = src.size();
    Result result = draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](SkCanvas* canvas, Src::GraphiteTestContext* testContext) {
        SkPictureRecorder recorder;
        sk_sp<SkPicture> pic;
        Result result = src.draw(recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())), testContext);
        if (!result.isOk()) {
            return result;
        }
...
Result ViaRuntimeBlend::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    class RuntimeBlendFilterCanvas : public SkPaintFilterCanvas {
    public:
        RuntimeBlendFilterCanvas(SkCanvas* canvas)
            : INHERITED(canvas)
        {
        }
    ...
    return draw_to_canvas(fSink.get(), bitmap, stream, log, src.size(), [&](SkCanvas* canvas, Src::GraphiteTestContext* testContext) {
...
{
    auto size = src.size();
    return draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](SkCanvas* canvas, Src::GraphiteTestContext* testContext) -> Result {
        SkDynamicMemoryWStream wstream;
        SkXMLStreamWriter writer(&wstream);
        Result result = src.draw(SkSVGCanvas::Make(SkRect::Make(size), &writer).get(), testContext);
        if (!result.isOk()) {
            return result;
        }
        ...
        sk_sp<SkSVGDOM> dom = SkSVGDOM::Builder().setFontManager(std::move(fontMgr)).setTextShapingFactory(std::move(shapingFactory)).make(*rstream);
        if (dom) {
