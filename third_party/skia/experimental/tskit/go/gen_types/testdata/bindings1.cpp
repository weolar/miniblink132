...
EMSCRIPTEN_BINDINGS(Core)
{
    ...
    function("_privateFunction2", optional_override([](int x, int y) -> size_t { return x * y; }));
    ...
    function("publicFunction2", optional_override([](std::string s) -> void { printf("Hello %s\n", s.c_str()); }));
    ...
    class_<Something>("Something")
        ...
        TS_EXPORT("getName(): string")
            .function("getName", &Something::getName) TS_PRIVATE_EXPORT("_setName(name: string): void")
            .function("_setName", &Something::setName);

    /**
     * The AnotherClass class is another class.
     */
    class_<AnotherClass>("AnotherClass") TS_EXPORT("new(): AnotherClass")
        .constructor<>()
        /**
         * Returns a Something with the provided name.
         * @param name
