...
EMSCRIPTEN_BINDINGS(Core)
{
    ...
    function("_privateFunction", optional_override([](int x, int y) -> size_t { return x * y; }));
    ...
    function("publicFunction", optional_override([](std::string s) -> void { printf("Hello %s\n", s.c_str()); }));
    ...
    class_<Something>("Something")
        ...
        TS_EXPORT("getName(): string")
            .function("getName", &Something::getName) TS_PRIVATE_EXPORT("setName(name: string): void")
