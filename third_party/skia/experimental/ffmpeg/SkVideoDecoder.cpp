...
static sk_sp<SkImage> make_yuv_420(
    GrRecordingContext* rContext, int w, int h, uint8_t* const data[], int const strides[], SkYUVColorSpace yuvSpace, sk_sp<SkColorSpace> cs)
{
    SkYUVAInfo yuvaInfo({ w, h }, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, yuvSpace);
    SkPixmap pixmaps[3];
    ...
    return SkImages::TextureFromYUVAPixmaps(rContext, yuvaPixmaps, skgpu::Mipmapped::kNo, false, std::move(cs));
}
...
void SkVideoDecoder::ConvertedColorSpace::update(AVColorPrimaries primaries, AVColorTransferCharacteristic transfer)
{
...
sk_sp<SkImage> SkVideoDecoder::convertFrame(const AVFrame* frame)
{
    ...
    case AV_PIX_FMT_YUV420P:
        if (auto image = make_yuv_420(fRecordingContext, frame->width, frame->height, frame->data, frame->linesize, yuv_space, fCSCache.fCS)) {
            return image;
        }
        break;
    default:
    ...
    const auto info = SkImageInfo::MakeN32(frame->width, frame->height, SkAlphaType::kOpaque_SkAlphaType);
    ...
    auto* ctx
        = sws_getContext(frame->width, frame->height, (AVPixelFormat)frame->format, info.width(), info.height(), fmt, SWS_BILINEAR, nullptr, nullptr, nullptr);
...
bool SkVideoDecoder::loadStream(std::unique_ptr<SkStream> stream)
{
    ...
    fStreamCtx = avio_alloc_context(buffer, bufferSize, 0, fStream.get(), skstream_read_packet, nullptr, skstream_seek_packet);
    if (!fStreamCtx) {
        av_freep(buffer);
        this->reset();
        return false;
