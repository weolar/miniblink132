...
bool SkVideoEncoder::init(int fps)
{
    ...
    fStreamCtx = avio_alloc_context(buffer, bufferSize, AVIO_FLAG_WRITE, fWStream.get(), nullptr, sk_write_packet, sk_seek_packet);
    SkASSERT(fStreamCtx);
...
bool SkVideoEncoder::beginRecording(SkISize dim, int fps)
{
    ...
    fSWScaleCtx = sws_getCachedContext(
        fSWScaleCtx, dim.width(), dim.height(), fmt, dim.width(), dim.height(), AV_PIX_FMT_YUV420P, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr);
