...
static wgpu::Surface getSurfaceForCanvas(wgpu::Device device, std::string canvasSelector, int width, int height)
{
...
public:
    bool init(std::string canvasSelector, int width, int height)
    {
        ...
        const char* sksl = "uniform float2 iResolution;"
                           "uniform float iTime;"
                           "vec2 d;"
                           "float b(float a) {"
    ...
    void draw(int timestamp)
    {
        ...
        sk_sp<SkSurface> surface
            = SkSurfaces::WrapBackendRenderTarget(fContext.get(), backendRenderTarget, kTopLeft_GrSurfaceOrigin, kN32_SkColorType, nullptr, &surfaceProps);
    ...
    void drawGradient(SkPaint* paint)
    {
        ...
        paint->setShader(SkGradientShader::MakeRadial(SkPoint::Make(x, y), std::min(x, y), flipColor ? colors1 : colors2, nullptr, 2, SkTileMode::kClamp));
    }
...
EMSCRIPTEN_BINDINGS(Skia)
{
    ...
    emscripten::class_<Demo>("Demo").constructor().function("init", &Demo::init).function("setKind", &Demo::setKind).function("draw", &Demo::draw);
}
