...
{
    return (1.0f - tx) * (1.0f - ty) * p00 + (1.0f - tx) * ty * p01 + (1.0f - ty) * tx * p10 + tx * ty * p11;
}
...
static int16_t bilerp_1(float tx, float ty, int16_t p00, int16_t p10, int16_t p01, int16_t p11)
{
    ...
    I16 bottom = (I16)((U16)(constrained_add(simulate_ssse3_mm_mulhrs_epi16(qtx, qw), qm) + 1) >> 1);
...
template <typename Bilerp> static Stats check_bilerp(Bilerp bilerp)
{
    Stats stats;
    const int step = 1;
    auto interesting = { 0, 1, 2, 3, 4, 5, 6, 7, 8, 60, 61, 62, 63, 64, 65, 66, 67, 68, 124, 125, 126, 127, 128, 129, 130, 131, 132, 188, 189, 190, 191, 192,
        193, 194, 195, 196, 248, 249, 250, 251, 252, 253, 254, 255 };
