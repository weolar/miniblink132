...
private:
    ...
    Result startDecoding(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options& options, DecodingState* decodingState);

    // Helper for taking a decoded interlaced `srcRow`, applying color
    // transformations, and then expanding it into the `frame`.
    void expandDecodedInterlacedRow(SkSpan<uint8_t> dstFrame, SkSpan<const uint8_t> srcRow, const DecodingState& decodingState);
    ...
    // SkCodec overrides:
    Result onGetPixels(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options&, int* rowsDecoded) override;
    Result onStartIncrementalDecode(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options&) override;
    ...
    private:
        ...
        Result setFrameInfoFromCurrentFctlChunk(const rust_png::Reader& reader, PngFrame* out_frame);
