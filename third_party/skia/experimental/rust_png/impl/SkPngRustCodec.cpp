...
std::unique_ptr<SkEncodedInfo::ICCProfile> CreateColorProfile(const rust_png::Reader& reader)
{
    ...
    if (reader.has_iccp_chunk()) {
        ...
        std::unique_ptr<SkEncodedInfo::ICCProfile> parsed_data = SkEncodedInfo::ICCProfile::Make(std::move(owned_data));
        if (parsed_data) {
            return parsed_data;
        }
    }
...
SkEncodedInfo CreateEncodedInfo(const rust_png::Reader& reader)
{
    ...
    return SkEncodedInfo::Make(Sk64_pin_to_s32(reader.width()), Sk64_pin_to_s32(reader.height()), skColor, ToAlpha(rustColor, reader),
        reader.output_bits_per_component(), std::move(profile));
}
...
public:
    ...
    explicit ReadTraitAdapterForSkStream(SkStream* stream)
        : fStream(stream)
    {
    }
...
void blendRow(SkSpan<uint8_t> dstRow, SkSpan<const uint8_t> srcRow, SkColorType color, SkAlphaType alpha)
{
    ...
    SkRasterPipeline_MemoryCtx srcCtx = { const_cast<void*>(static_cast<const void*>(srcRow.data())), 0 };
...
void blendAllRows(SkSpan<uint8_t> dstFrame, SkSpan<const uint8_t> srcFrame, size_t rowSize, size_t rowStride, SkColorType color, SkAlphaType alpha)
{
...
// static
std::unique_ptr<SkPngRustCodec> SkPngRustCodec::MakeFromStream(std::unique_ptr<SkStream> stream, Result* result)
{
    SkASSERT(stream);
    SkASSERT(result);

    auto readTraitAdapter = std::make_unique<ReadTraitAdapterForSkStream>(stream.get());
    rust::Box<rust_png::ResultOfReader> resultOfReader = rust_png::new_reader(std::move(readTraitAdapter));
    *result = ToSkCodecResult(resultOfReader->err());
    ...
    return std::make_unique<SkPngRustCodec>(CreateEncodedInfo(*reader), std::move(stream), std::move(reader));
}

SkPngRustCodec::SkPngRustCodec(SkEncodedInfo&& encodedInfo, std::unique_ptr<SkStream> stream, rust::Box<rust_png::Reader> reader)
...
SkCodec::Result SkPngRustCodec::seekToStartOfFrame(int index)
{
    ...
    if ((index < fFrameAtCurrentStreamPosition) || (index == fFrameAtCurrentStreamPosition && !fStreamIsPositionedAtStartOfFrameData)) {
...
SkCodec::Result SkPngRustCodec::startDecoding(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options& options, DecodingState* decodingState)
{
...

    {
        ...
        decodingState->fDst = SkSpan(static_cast<uint8_t*>(pixels), imageSize).subspan(xByteOffset).subspan(yByteOffset);
...
void SkPngRustCodec::expandDecodedInterlacedRow(SkSpan<uint8_t> dstFrame, SkSpan<const uint8_t> srcRow, const DecodingState& decodingState)
{
    ...
    fReader->expand_last_interlaced_row(
        rust::Slice<uint8_t>(dstFrame), decodingState.fDstRowStride, rust::Slice<const uint8_t>(xformedInterlacedRow), decodingState.fDstBytesPerPixel * 8u);
}

SkCodec::Result SkPngRustCodec::incrementalDecode(DecodingState& decodingState, int* rowsDecodedPtr)
{
    ...
    while (true) {
        ...
        if (decodedRow.empty()) { // This is how FFI layer says "no more rows".
            if (interlaced && !decodingState.fPreblendBuffer.empty()) {
                blendAllRows(decodingState.fDst, decodingState.fPreblendBuffer, decodingState.fDstRowSize, decodingState.fDstRowStride, decodingState.fDstColor,
        ...
        if (interlaced) {
            if (decodingState.fPreblendBuffer.empty()) {
                ...
                this->expandDecodedInterlacedRow(decodingState.fPreblendBuffer, decodedRow, decodingState);
            }
            ...
            if (decodingState.fPreblendBuffer.empty()) {
                ...
                blendRow(decodingState.fDst, decodingState.fPreblendBuffer, decodingState.fDstColor, decodingState.fDstAlpha);
            }

            decodingState.fDst = decodingState.fDst.subspan(std::min(decodingState.fDstRowStride, decodingState.fDst.size()));
            rowsDecoded++;
        }
    }
}

SkCodec::Result SkPngRustCodec::onGetPixels(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options& options, int* rowsDecoded)
{
...
SkCodec::Result SkPngRustCodec::onStartIncrementalDecode(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options& options)
{
    DecodingState decodingState;
    Result result = this->startDecoding(dstInfo, pixels, rowBytes, options, &decodingState);
    if (result != kSuccess) {
...
std::optional<SkSpan<const SkPngCodecBase::PaletteColorEntry>> SkPngRustCodec::onTryGetPlteChunk()
{
    ...
    SkSpan<const PaletteColorEntry> palette = SkSpan(reinterpret_cast<const PaletteColorEntry*>(bytes.data()), bytes.size() / kEntrySize);
...
public:
    PngFrame(int id, SkEncodedInfo::Alpha alpha)
        : SkFrame(id)
    ...
    {
        return fFullyReceived;
    }
    void markAsFullyReceived()
    {
        fFullyReceived = true;
    }
...
SkPngRustCodec::FrameHolder::FrameHolder(int width, int height)
    : SkFrameHolder()
{
    fScreenWidth = width;
...
const SkFrameHolder* SkPngRustCodec::getFrameHolder() const
{
    return &fFrameHolder;
}
...
SkCodec::Result SkPngRustCodec::FrameHolder::appendNewFrame(const rust_png::Reader& reader, const SkEncodedInfo& info)
{
...
SkCodec::Result SkPngRustCodec::FrameHolder::setFrameInfoFromCurrentFctlChunk(const rust_png::Reader& reader, PngFrame* frame)
{
    ...
    {
        SkSafeMath safe;
        frame->setXYWH(safe.castTo<int>(xOffset), safe.castTo<int>(yOffset), safe.castTo<int>(width), safe.castTo<int>(height));
