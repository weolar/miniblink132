...
std::optional<Point> intersect(const Segment& s0, const Segment& s1)
{
    ...
    const Point P0 = s0.upper(), P1 = s0.lower(), P2 = s1.upper(), P3 = s1.lower();
    ...
    const Point Q = P1 - P0, R = P2 - P0, T = P3 - P2;

    // 64-bit cross product.
    auto cross = [](const Point& v0, const Point& v1) {
        int64_t x0 = SkToS64(v0.x), y0 = SkToS64(v0.y), x1 = SkToS64(v1.x), y1 = SkToS64(v1.y);
        return x0 * y1 - y0 * x1;
    };

    // Calculate the cross products needed for calculating s and t.
    const int64_t QxR = cross(Q, R), TxR = cross(T, R), TxQ = cross(T, Q);
    ...
    SkASSERT(0 <= t && t <= 1);
    const int32_t x = std::round(t * (P3.x - P2.x) + P2.x), y = std::round(t * (P3.y - P2.y) + P2.y);
...
bool less_than_at(const Segment& s0, const Segment& s1, int32_t y)
{
    ...
    int64_t s0YDiff = y - y0, s1YDiff = y - y2, s0YDelta = y1 - y0, s1YDelta = y3 - y2, x0Offset = x0 * s0YDelta + s0YDiff * (x1 - x0),
            x2Offset = x2 * s1YDelta + s1YDiff * (x3 - x2);

    Int96 s0Factor = multiply(x0Offset, y3 - y2), s1Factor = multiply(x2Offset, y1 - y0);
...
{
    Point s0Delta = s0.lower() - s0.upper(), s1Delta = s1.lower() - s1.upper();
    ...
    //                           the positive x-axis.
    int64_t lhs = SkToS64(s0Delta.x) * SkToS64(s1Delta.y), rhs = SkToS64(s1Delta.x) * SkToS64(s0Delta.y);
