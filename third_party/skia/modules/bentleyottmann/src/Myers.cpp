...
class EventQueue {
    ...
    public:
        ...
        Iterator(const Iterator& that)
            : Iterator { that.fEventQueue, that.fIndex }
        {
        }
        Iterator& operator++()
        {
...
public:
    ...
    static EventQueue Make(const SkSpan<const Segment> segments)
    {
        ...
        enum EventType { kBegin = 0, kHorizontal = 1, kEnd = 2 };
        ...
        struct SetupTuple {
            ...
            {
                return std::tie(this->yOrdering, this->type, this->xTieBreaker, this->originalSegment)
        ...
        auto eventLess = [](const SetupTuple& l, const SetupTuple& r) {
            return std::tie(l.yOrdering, l.type, l.xTieBreaker) < std::tie(r.yOrdering, r.type, r.xTieBreaker);
        };
        ...
        int32_t start = 0, endOfBegin = 0, endOfHorizontal = 0, endOfEnd = 0;
        for (const auto& [y, type, _, s] : eventOrdering) {
            // If this is a new y then create the compact event.
            if (currentY != y) {
                events.push_back(CompactEvent { currentY, start, endOfBegin, endOfHorizontal, endOfEnd });
            ...
            switch (type) {
            case kBegin:
                endOfBegin = segmentCount;
                [[fallthrough]];
            case kHorizontal:
                endOfHorizontal = segmentCount;
                [[fallthrough]];
            case kEnd:
                endOfEnd = segmentCount;
            }
        }
    ...
    Event operator[](size_t i) const
    {
        ...
        SkSpan<const Segment> horizontal { &fSegmentStorage[endOfBegin], endOfHorizontal - endOfBegin };
...
private:
    ...
    template <typename CrossingCheck> void checkCrossingsLeftAndRight(const Segment& segment, StatusLine::iterator insertionPoint, CrossingCheck check)
    {
    ...
    void handleBeginnings(int32_t y, SkSpan<const Segment> inserting)
    {
        for (const Segment& s : inserting) {
            auto insertionPoint = std::lower_bound(fStatus.begin(), fStatus.end(), s, segment_less_than_upper_to_insert);

            // Checking intersections left and right checks if the point s.upper() lies on
            // the nearby segment.
            auto checkIntersect = [&](const Segment& toCheck) { return compare_point_to_segment(s.upper(), toCheck) == 0; };
    ...
    void handleHorizontals(int32_t y, SkSpan<const Segment> horizontals)
    {
        for (const Segment& s : horizontals) {
            ...
            auto checkIntersection = [&](const Segment& toCheck) {
                return compare_point_to_segment(s.upper(), toCheck) <= 0 && compare_point_to_segment(s.lower(), toCheck) >= 0;
            };
...
bool s0_intersects_s1(const Segment& s0, const Segment& s1)
{
    ...
    if (s1.upper().y < s0.upper().y || ((s1.upper().y == s0.upper().y) && (s1.lower().y > s0.lower().y))) {
...
std::vector<Crossing> brute_force_crossings(SkSpan<Segment> segments)
{
    ...
    auto isNonZeroSegment = [](const Segment& segment) { return segment.upper() != segment.lower(); };
    ...
    SkSpan<const Segment> cleanSegments = SkSpan { segments.data(), std::distance(segments.begin(), duplicateSegments) };
