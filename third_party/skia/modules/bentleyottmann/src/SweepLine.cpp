...
SweepLine::SweepLine()
{
    static constexpr Segment LeftSentinel = { { -std::numeric_limits<int32_t>::max(), -std::numeric_limits<int32_t>::max() },
        { -std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::max() } };
    static constexpr Segment RightSentinel = { { std::numeric_limits<int32_t>::max(), -std::numeric_limits<int32_t>::max() },
        { std::numeric_limits<int32_t>::max(), std::numeric_limits<int32_t>::max() } };
...
void SweepLine::handleDeletions(Point eventPoint, const DeletionSegmentSet& removing)
{
    ...
    if (removing.empty()) {
        ...
        auto toRemove = [eventPoint](Segment s) { return s.lower() == eventPoint; };
        newEnd = std::remove_if(fSweepLine.begin(), fSweepLine.end(), toRemove);
    } else {
        // Remove all ending and crossing segments.
        auto toRemove = [eventPoint, &removing](Segment s) { return s.lower() == eventPoint || removing.find(s) != removing.end(); };
        newEnd = std::remove_if(fSweepLine.begin(), fSweepLine.end(), toRemove);
    }
    fSweepLine.erase(newEnd, fSweepLine.end());
}

void SweepLine::handleInsertionsAndCheckForNewCrossings(Point eventPoint, const InsertionSegmentSet& inserting, EventQueueInterface* queue)
{
    // The SlopeSegmentSet makes sure that these segments are in the right order for insertion.
    auto comp = [](const Segment& s, Point p) { return !point_less_than_segment_in_x(p, s); };

    const auto rightOfInsertion = std::lower_bound(fSweepLine.begin(), fSweepLine.end(), eventPoint, comp);
