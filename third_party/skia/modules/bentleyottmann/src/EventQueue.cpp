...
std::optional<EventQueue> EventQueue::Make(SkSpan<const Segment> segments)
{
    ...
    int32_t left = Point::Largest().x, top = Point::Largest().y, right = Point::Smallest().x, bottom = Point::Smallest().y;
...
EventQueue::EventQueue(EventQueue::Queue&& queue)
...
void EventQueue::handleNextEventPoint(SweepLineInterface* handler)
{
    ...
    auto handleLower = [&hasLower](const Lower& l) { hasLower = true; };
    ...
    for (; cursor != queueEnd && cursor->where == eventPoint; ++cursor) {
...
bool OrderBySlope::operator()(const bentleyottmann::Segment& s0, const bentleyottmann::Segment& s1) const
{
