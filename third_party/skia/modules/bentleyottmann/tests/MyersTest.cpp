...
DEF_TEST(MFC_slope_less_than, r)
{
    ...
    { // Check slopes for horizontals.
        Segment s0 = { { -2, 0 }, { 1, 0 } }, s1 = { { -1, 0 }, { 2, 0 } };
        REPORTER_ASSERT(r, !slope_s0_less_than_slope_s1(s0, s1));
        REPORTER_ASSERT(r, !slope_s0_less_than_slope_s1(s1, s0));
    }
    { // Check slopes for horizontals.
        Segment s0 = { { -2, 0 }, { 1, 0 } }, s1 = { { 0, 0 }, { 1, 1 } };
        REPORTER_ASSERT(r, !slope_s0_less_than_slope_s1(s0, s1));
        REPORTER_ASSERT(r, slope_s0_less_than_slope_s1(s1, s0));
    }
}
...
{
    Segment s0 = { { -10, -10 }, { 10, 10 } }, s1 = { { 10, -10 }, { -10, 10 } }, to_insert = { { 0, 0 }, { 0, 3 } };

    // Above y = 0, the sweepLine is {s0, s1}, but at y=0 s1 and s0 swap because of their slopes.
    std::vector<Segment> sweepLine = { s1, s0 };

    auto insertionPoint = std::lower_bound(sweepLine.begin(), sweepLine.end(), to_insert, segment_less_than_upper_to_insert);
...
DEF_TEST(MFC_less_than_at_y, r)
{
    ...
    {
        Segment s0 = { { -2, -100 }, { -2, 89 } }, s1 = { { 6, -70 }, { -2, 72 } };
...
DEF_TEST(MFC_has_inner_intersection, r)
{
    ...
    {
        Segment s0 = { { 0, -101 }, { 0, -50 } }, s1 = { { 100, -100 }, { -100, 100 } }; // goes through (0,0)
        REPORTER_ASSERT(r, !s0_intersects_s1(s0, s1));
        REPORTER_ASSERT(r, !s0_intersects_s1(s1, s0));
    }
}
...
DEF_TEST(MFC_myers_brute_force_comparison, r)
{
    const std::vector<Segment> tests[] = {
        { { { -58, -100 }, { 75, 105 } }, { { 149, -58 }, { -156, 49 } }, { { -34, -55 }, { 37, 49 } }, { { -58, -100 }, { 75, 105 } },
            { { -147, -229 }, { 143, 220 } } },
        ...
        { { { 142, -82 }, { -128, 64 } }, { { 208, -16 }, { -217, -3 } }, { { 91, -22 }, { -93, 24 } }, { { 31, -18 }, { -25, 7 } },
            { { -25, 7 }, { 33, 12 } } },
        { { { -159, -101 }, { 167, 91 } }, { { -96, -117 }, { 99, 117 } }, { { -16, -21 }, { 12, 35 } }, { { -48, -55 }, { 33, 63 } },
            { { -16, -21 }, { 26, 41 } } },
    ...
    for (const auto& segments : tests) {
        ...
        REPORTER_ASSERT(r, std::unique(myersResponse.begin(), myersResponse.end()) == myersResponse.end());
        REPORTER_ASSERT(r, std::unique(bruteResponse.begin(), bruteResponse.end()) == bruteResponse.end());

        // Both should be equal.
        REPORTER_ASSERT(r, std::equal(myersResponse.begin(), myersResponse.end(), bruteResponse.begin(), bruteResponse.end()));
    }
}
...
DEF_TEST(MFC_myers_brute_force_random_comparison, r)
{
    ...
    for (int trials = 0; trials < 100'000; trials++) {
        for (int i = 0; i < n; ++i) {
            float x = random.nextRangeF(-boxSize, boxSize), y = random.nextRangeF(-boxSize, boxSize);

            float angle = random.nextF() * SK_FloatPI;
            float distance = random.nextRangeF(10, 300);

            Point p0 = { sk_float_round2int(x + cos(angle) * distance), sk_float_round2int(y + sin(angle) * distance) };
            Point p1 = { sk_float_round2int(x - cos(angle) * distance), sk_float_round2int(y - sin(angle) * distance) };
