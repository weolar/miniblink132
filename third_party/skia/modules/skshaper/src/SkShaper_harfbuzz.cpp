...
hb_bool_t skhb_glyph(hb_font_t* hb_font, void* font_data, hb_codepoint_t unicode, hb_codepoint_t variation_selector, hb_codepoint_t* glyph, void* user_data)
{
...
hb_bool_t skhb_nominal_glyph(hb_font_t* hb_font, void* font_data, hb_codepoint_t unicode, hb_codepoint_t* glyph, void* user_data)
{
    return skhb_glyph(hb_font, font_data, unicode, 0, glyph, user_data);
}

unsigned skhb_nominal_glyphs(hb_font_t* hb_font, void* font_data, unsigned int count, const hb_codepoint_t* unicodes, unsigned int unicode_stride,
...
hb_position_t skhb_glyph_h_advance(hb_font_t* hb_font, void* font_data, hb_codepoint_t hbGlyph, void* user_data)
{
    SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
void skhb_glyph_h_advances(hb_font_t* hb_font, void* font_data, unsigned count, const hb_codepoint_t* glyphs, unsigned int glyph_stride,
    hb_position_t* advances, unsigned int advance_stride, void* user_data)
{
    SkFont& font = *reinterpret_cast<SkFont*>(font_data);
...
hb_bool_t skhb_glyph_extents(hb_font_t* hb_font, void* font_data, hb_codepoint_t hbGlyph, hb_glyph_extents_t* extents, void* user_data)
{
    SkFont& font = *reinterpret_cast<SkFont*>(font_data);
    SkASSERT(extents);
...
hb_blob_t* skhb_get_table(hb_face_t* face, hb_tag_t tag, void* user_data)
{
    ...
    return hb_blob_create(
        reinterpret_cast<char*>(rawData->writable_data()), rawData->size(), HB_MEMORY_MODE_READONLY, rawData, [](void* ctx) { SkSafeUnref(((SkData*)ctx)); });
}
...
HBBlob stream_to_blob(std::unique_ptr<SkStreamAsset> asset)
{
    ...
    if (const void* base = asset->getMemoryBase()) {
        blob.reset(hb_blob_create((const char*)base, SkToUInt(size), HB_MEMORY_MODE_READONLY, asset.release(), [](void* p) { delete (SkStreamAsset*)p; }));
    } else {
    ...
    HBFace create_hb_face(const SkTypeface& typeface)
{
    ...
    if (!face) {
        face.reset(hb_face_create_for_tables(
            skhb_get_table, const_cast<SkTypeface*>(SkRef(&typeface)), [](void* user_data) { SkSafeUnref(reinterpret_cast<SkTypeface*>(user_data)); }));
    ...
    SkDEBUGCODE(hb_face_set_user_data(face.get(), &gDataIdKey, const_cast<SkTypeface*>(&typeface), nullptr, false);)
...
HBFont create_typeface_hb_font(const SkTypeface& typeface)
{
    ...
    if (axis_count > 0) {
        ...
        if (typeface.getVariationDesignPosition(axis_values, axis_count) == axis_count) {
            hb_font_set_variations(otFont.get(), reinterpret_cast<hb_variation_t*>(axis_values.get()), axis_count);
        }
    }
...
{
    SkDEBUGCODE(hb_face_t* face = hb_font_get_face(typefaceFont.get()); void* dataId = hb_face_get_user_data(face, &gDataIdKey);
    ...
    hb_font_set_funcs(
        skFont.get(), skhb_get_font_funcs(), reinterpret_cast<void*>(new SkFont(font)), [](void* user_data) { delete reinterpret_cast<SkFont*>(user_data); });
...
public:
    SkUnicodeHbScriptRunIterator(const char* utf8, size_t utf8Bytes, hb_script_t defaultScript)
        : fCurrent(utf8)
        , fBegin(utf8)
...
struct ShapedRun {
    ShapedRun(SkShaper::RunHandler::Range utf8Range, const SkFont& font, SkBidiIterator::Level level, std::unique_ptr<ShapedGlyph[]> glyphs, size_t numGlyphs,
...
void append(SkShaper::RunHandler* handler, const SkShaper::RunHandler::RunInfo& runInfo, const ShapedRun& run, size_t startGlyphIndex, size_t endGlyphIndex)
{
    ...
    for (size_t i = 0; i < glyphLen; i++) {
        ...
        const ShapedGlyph& glyph = run.fGlyphs[is_LTR(run.fLevel) ? startGlyphIndex + i : endGlyphIndex - 1 - i];
...
void emit(SkUnicode* unicode, const ShapedLine& line, SkShaper::RunHandler* handler)
{
    ...
    for (int i = 0; i < numRuns; ++i) {
        ...
        const SkShaper::RunHandler::RunInfo info = { run.fFont, run.fLevel, run.fAdvance, run.fNumGlyphs, run.fUtf8Range };
...
public:
    ShaperHarfBuzz(sk_sp<SkUnicode>, HBBuffer, sk_sp<SkFontMgr>);

protected:
    sk_sp<SkUnicode> fUnicode;

    ShapedRun shape(const char* utf8, size_t utf8Bytes, const char* utf8Start, const char* utf8End, const BiDiRunIterator&, const LanguageRunIterator&,
        const ScriptRunIterator&, const FontRunIterator&, const Feature*, size_t featuresSize) const;
...
private:
    ...
    void shape(const char* utf8Text, size_t textBytes, FontRunIterator&, BiDiRunIterator&, ScriptRunIterator&, LanguageRunIterator&, SkScalar width,
        RunHandler*) const override;
#endif

    void shape(const char* utf8Text, size_t textBytes, FontRunIterator&, BiDiRunIterator&, ScriptRunIterator&, LanguageRunIterator&, const Feature*,
        size_t featuresSize, SkScalar width, RunHandler*) const override;

    virtual void wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&, const LanguageRunIterator&, const ScriptRunIterator&,
        const FontRunIterator&, RunIteratorQueue& runSegmenter, const Feature*, size_t featuresSize, SkScalar width, RunHandler*) const
        = 0;
};
...
private:
    void wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator&, const LanguageRunIterator&, const ScriptRunIterator&, const FontRunIterator&,
        RunIteratorQueue& runSegmenter, const Feature*, size_t featuresSize, SkScalar width, RunHandler*) const override;
};
...
ShaperHarfBuzz::ShaperHarfBuzz(sk_sp<SkUnicode> unicode, HBBuffer buffer, sk_sp<SkFontMgr> fallback)
    : fUnicode(unicode)
...
void ShaperHarfBuzz::shape(const char* utf8, size_t utf8Bytes, const SkFont& srcFont, bool leftToRight, SkScalar width, RunHandler* handler) const
{
    ...
    std::unique_ptr<BiDiRunIterator> bidi(SkShapers::unicode::BidiRunIterator(fUnicode, utf8, utf8Bytes, defaultLevel));
    ...
    std::unique_ptr<FontRunIterator> font(MakeFontMgrRunIterator(utf8, utf8Bytes, srcFont, fFontMgr));
    if (!font) {
        return;
    }
...
void ShaperHarfBuzz::shape(const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi, ScriptRunIterator& script,
    LanguageRunIterator& language, SkScalar width, RunHandler* handler) const
{
...
void ShaperHarfBuzz::shape(const char* utf8, size_t utf8Bytes, FontRunIterator& font, BiDiRunIterator& bidi, ScriptRunIterator& script,
    LanguageRunIterator& language, const Feature* features, size_t featuresSize, SkScalar width, RunHandler* handler) const
{
    SkASSERT(handler);
...
void ShaperDrivenWrapper::wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi, const LanguageRunIterator& language,
    const ScriptRunIterator& script, const FontRunIterator& font, RunIteratorQueue& runSegmenter, const Feature* features, size_t featuresSize, SkScalar width,
    ...
    while (runSegmenter.advanceRuns()) { // For each item
        ...
        while (utf8Start < utf8End) { // While there are still code points left in this item
            ...
            if (!lineBreakIterator || !currentLanguage.equals(language.currentLanguage())) {
                ...
                lineBreakIterator = fUnicode->makeBreakIterator(currentLanguage.c_str(), SkUnicode::BreakType::kLines);
            ...
            for (int32_t breakIteratorCurrent = breakIterator.next(); !breakIterator.isDone(); breakIteratorCurrent = breakIterator.next()) {
                ...
                ShapedRun candidate = [&](const TextProps& props) {
                    if (props.glyphLen) {
                        ...
                        return ShapedRun(RunHandler::Range(utf8Start - utf8, breakIteratorCurrent), font.currentFont(), bidi.currentLevel(),
                            std::unique_ptr<ShapedGlyph[]>(), props.glyphLen - modelGlyphOffset, props.advance - modelAdvanceOffset);
                    } else {
...
void ShapeThenWrap::wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi, const LanguageRunIterator& language,
    const ScriptRunIterator& script, const FontRunIterator& font, RunIteratorQueue& runSegmenter, const Feature* features, size_t featuresSize, SkScalar width,
    RunHandler* handler) const
{
    TArray<ShapedRun> runs;
    {
        ...
        while (runSegmenter.advanceRuns()) {
            ...
            if (needIteratorInit || !currentLanguage.equals(language.currentLanguage())) {
                ...
                graphemeBreakIterator = fUnicode->makeBreakIterator(currentLanguage.c_str(), SkUnicode::BreakType::kGraphemes);
    ...
    // Reorder the runs and glyphs per line and write them out.
    {
        ...
        while (glyphIterator.current()) {
            ...
            auto makeSubRun = [&runs, &previousBreak, &current, &logicalFromVisual](size_t visualIndex) {
                ...
                size_t startGlyphIndex = (logicalIndex == previousBreak.fRunIndex) ? previousBreak.fGlyphIndex : 0;
                size_t endGlyphIndex = (logicalIndex == current.fRunIndex) ? current.fGlyphIndex + 1 : run.fNumGlyphs;
            ...
            auto makeRunInfo = [](const SubRun& sub) {
                ...
                uint32_t endUtf8 = (sub.endGlyphIndex < sub.run.fNumGlyphs) ? sub.run.fGlyphs[sub.endGlyphIndex].fCluster : sub.run.fUtf8Range.end();
                ...
                return RunHandler::RunInfo { sub.run.fFont, sub.run.fLevel, advance, sub.endGlyphIndex - sub.startGlyphIndex,
...
void ShapeDontWrapOrReorder::wrap(char const* const utf8, size_t utf8Bytes, const BiDiRunIterator& bidi, const LanguageRunIterator& language,
    const ScriptRunIterator& script, const FontRunIterator& font, RunIteratorQueue& runSegmenter, const Feature* features, size_t featuresSize, SkScalar width,
    RunHandler* handler) const
{
    ...
    for (const auto& run : runs) {
        const RunHandler::RunInfo info = { run.fFont, run.fLevel, run.fAdvance, run.fNumGlyphs, run.fUtf8Range };
        handler->runInfo(info);
    }
    handler->commitRunInfo();
    for (const auto& run : runs) {
...
ShapedRun ShaperHarfBuzz::shape(char const* const utf8, size_t const utf8Bytes, char const* const utf8Start, char const* const utf8End,
    const BiDiRunIterator& bidi, const LanguageRunIterator& language, const ScriptRunIterator& script, const FontRunIterator& font,
    Feature const* const features, size_t const featuresSize) const
{
    size_t utf8runLength = utf8End - utf8Start;
    ShapedRun run(RunHandler::Range(utf8Start - utf8, utf8runLength), font.currentFont(), bidi.currentLevel(), nullptr, 0);
    ...
    run = ShapedRun(
        RunHandler::Range(utf8Start - utf8, utf8runLength), font.currentFont(), bidi.currentLevel(), std::unique_ptr<ShapedGlyph[]>(new ShapedGlyph[len]), len);
...
std::unique_ptr<SkShaper::ScriptRunIterator> SkShaper::MakeHbIcuScriptRunIterator(const char* utf8, size_t utf8Bytes)
{
    return SkShapers::HB::ScriptRunIterator(utf8, utf8Bytes);
}

std::unique_ptr<SkShaper::ScriptRunIterator> SkShaper::MakeSkUnicodeHbScriptRunIterator(const char* utf8, size_t utf8Bytes)
{
    return SkShapers::HB::ScriptRunIterator(utf8, utf8Bytes);
}

std::unique_ptr<SkShaper::ScriptRunIterator> SkShaper::MakeSkUnicodeHbScriptRunIterator(const char* utf8, size_t utf8Bytes, SkFourByteTag script)
{
...
std::unique_ptr<SkShaper> ShaperDrivenWrapper(sk_sp<SkUnicode> unicode, sk_sp<SkFontMgr> fallback)
{
    if (!unicode) {
        return nullptr;
    }
    ...
    return std::make_unique<::ShaperDrivenWrapper>(unicode, std::move(buffer), std::move(fallback));
}

std::unique_ptr<SkShaper> ShapeThenWrap(sk_sp<SkUnicode> unicode, sk_sp<SkFontMgr> fallback)
{
    if (!unicode) {
        return nullptr;
    }
    ...
    return std::make_unique<::ShapeThenWrap>(unicode, std::move(buffer), std::move(fallback));
}

std::unique_ptr<SkShaper> ShapeDontWrapOrReorder(sk_sp<SkUnicode> unicode, sk_sp<SkFontMgr> fallback)
{
    if (!unicode) {
        return nullptr;
    }
    ...
    return std::make_unique<::ShapeDontWrapOrReorder>(unicode, std::move(buffer), std::move(fallback));
}
...
std::unique_ptr<SkShaper::ScriptRunIterator> ScriptRunIterator(const char* utf8, size_t utf8Bytes, SkFourByteTag script)
{
    return std::make_unique<SkUnicodeHbScriptRunIterator>(utf8, utf8Bytes, hb_script_from_iso15924_tag((hb_tag_t)script));
}
