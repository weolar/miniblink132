...
class PrimitiveFactory final : public SkShapers::Factory {
    ...
    std::unique_ptr<SkShaper::BiDiRunIterator> makeBidiRunIterator(const char*, size_t, uint8_t) override
    {
        return std::make_unique<SkShaper::TrivialBiDiRunIterator>(0, 0);
    }
    std::unique_ptr<SkShaper::ScriptRunIterator> makeScriptRunIterator(const char*, size_t, SkFourByteTag) override
    {
