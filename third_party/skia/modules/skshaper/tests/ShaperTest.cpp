...
struct RunHandler final : public SkShaper::RunHandler {
    ...
    SkShaper::RunHandler::Buffer runBuffer(const SkShaper::RunHandler::RunInfo& info) override
    {
        ...
        return SkShaper::RunHandler::Buffer { fGlyphs.get(), fPositions.get(), nullptr, fClusters.get(), { 0, 0 } };
    }
    void commitRunBuffer(const RunInfo& info) override
    {
        ...
        if ((false)) {
            ...
            SkDebugf("%s range: %zu-%zu(%zu) glyphCount:%u font: \"%s\" \"%s\" #%d %zuB\n"
                     "rangeText: \"%.*s\"\n"
                     "rangeBytes: %s\n"
                     "glyphs:%s\n\n",
                fResource, fRange.begin(), fRange.end(), fRange.size(), fGlyphCount, familyName.c_str(), postscriptName.c_str(), ttcIndex, fontSize,
                (int)fRange.size(), fUtf8 + fRange.begin(), chars.c_str(), glyphs.c_str());
        }

        for (unsigned i = 0; i < fGlyphCount; ++i) {
            REPORTER_ASSERT(
                fReporter, fClusters[i] >= fRange.begin(), "%" PRIu32 " >= %zu %s i:%u glyphCount:%u", fClusters[i], fRange.begin(), fResource, i, fGlyphCount);
            REPORTER_ASSERT(
                fReporter, fClusters[i] < fRange.end(), "%" PRIu32 " < %zu %s i:%u glyphCount:%u", fClusters[i], fRange.end(), fResource, i, fGlyphCount);
        }
    }
...
void shaper_test(skiatest::Reporter* reporter, const char* name, SkData* data)
{
    ...
    std::unique_ptr<SkShaper::BiDiRunIterator> bidi = SkShapers::unicode::BidiRunIterator(unicode, utf8, utf8Bytes, defaultLevel);
    SkASSERT(bidi);
    ...
    std::unique_ptr<SkShaper::FontRunIterator> fontRuns = SkShaper::MakeFontMgrRunIterator(utf8, utf8Bytes, font, SkFontMgr::RefEmpty());
    SkASSERT(fontRuns);
    ...
    shaper->shape((const char*)data->data(), data->size(), fontIterator, bidiIterator, scriptIterator, languageIterator, nullptr, 0, kWidth, &rh);
}
...
DEF_TEST(Shaper_cluster_empty, r)
{
    shaper_test(r, "empty", SkData::MakeEmpty().get());
}

#define SHAPER_TEST(X)                                                                                                                                         \
    DEF_TEST(Shaper_cluster_##X, r)                                                                                                                            \
    {                                                                                                                                                          \
        cluster_test(r, "text/" #X ".txt");                                                                                                                    \
    }
SHAPER_TEST(arabic)
