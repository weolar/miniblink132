...
{
    return INHERITED::parseAndSetAttribute(n, v) || this->setX1(SkSVGAttributeParser::parse<SkSVGLength>("x1", n, v))
        || this->setY1(SkSVGAttributeParser::parse<SkSVGLength>("y1", n, v)) || this->setX2(SkSVGAttributeParser::parse<SkSVGLength>("x2", n, v))
...
std::tuple<SkPoint, SkPoint> SkSVGLine::resolve(const SkSVGLengthContext& lctx) const
{
    return std::make_tuple(
        SkPoint::Make(lctx.resolve(fX1, SkSVGLengthContext::LengthType::kHorizontal), lctx.resolve(fY1, SkSVGLengthContext::LengthType::kVertical)),
