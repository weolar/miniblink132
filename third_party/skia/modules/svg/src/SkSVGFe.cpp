...
sk_sp<SkImageFilter> SkSVGFe::makeImageFilter(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    return this->onMakeImageFilter(ctx, fctx);
}

SkRect SkSVGFe::resolveBoundaries(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
...
SkRect SkSVGFe::resolveFilterSubregion(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    ...
    return SkRect::MakeXYWH(fX.isValid() ? boundaries.fLeft : defaultSubregion.fLeft, fY.isValid() ? boundaries.fTop : defaultSubregion.fTop,
        fWidth.isValid() ? boundaries.width() : defaultSubregion.width(), fHeight.isValid() ? boundaries.height() : defaultSubregion.height());
}
...
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setIn(SkSVGAttributeParser::parse<SkSVGFeInputType>("in", name, value))
        || this->setResult(SkSVGAttributeParser::parse<SkSVGStringType>("result", name, value))
        || this->setX(SkSVGAttributeParser::parse<SkSVGLength>("x", name, value)) || this->setY(SkSVGAttributeParser::parse<SkSVGLength>("y", name, value))
