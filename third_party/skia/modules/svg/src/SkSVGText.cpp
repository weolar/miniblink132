...
static SkFont ResolveFont(const SkSVGRenderContext& ctx)
{
    ...
    const SkFontStyle style(
        weight(*ctx.presentationContext().fInherited.fFontWeight), SkFontStyle::kNormal_Width, slant(*ctx.presentationContext().fInherited.fFontStyle));

    const auto size = ctx.lengthContext().resolve(ctx.presentationContext().fInherited.fFontSize->size(), SkSVGLengthContext::LengthType::kVertical);
...
static std::vector<float> ResolveLengths(const SkSVGLengthContext& lctx, const std::vector<SkSVGLength>& lengths, SkSVGLengthContext::LengthType lt)
{
...
SkSVGTextContext::ScopedPosResolver::ScopedPosResolver(
    const SkSVGTextContainer& txt, const SkSVGLengthContext& lctx, SkSVGTextContext* tctx, size_t charIndexOffset)
    : fTextContext(tctx)
...
SkSVGTextContext::ScopedPosResolver::ScopedPosResolver(const SkSVGTextContainer& txt, const SkSVGLengthContext& lctx, SkSVGTextContext* tctx)
...
SkSVGTextContext::PosAttrs SkSVGTextContext::ScopedPosResolver::resolve(size_t charIndex) const
{
    ...
    if (charIndex < fLastPosIndex) {
        ...
        const auto hasAllLocal = localCharIndex < fX.size() && localCharIndex < fY.size() && localCharIndex < fDx.size() && localCharIndex < fDy.size()
...
void SkSVGTextContext::shapePendingBuffer(const SkSVGRenderContext& ctx, const SkFont& font)
{
    ...
    std::unique_ptr<SkShaper::FontRunIterator> font_runs = SkShaper::MakeFontMgrRunIterator(utf8, utf8Bytes, font, ctx.fontMgr());
    if (!font_runs) {
        return;
    }
    if (!fForcePrimitiveShaping) {
        ...
        std::unique_ptr<SkShaper::BiDiRunIterator> bidi = ctx.makeBidiRunIterator(utf8, utf8Bytes, defaultLTR);
        ...
        if (bidi && script && language) {
            fShaper->shape(utf8, utf8Bytes, *font_runs, *bidi, *script, *language, nullptr, 0, SK_ScalarMax, this);
            fShapeBuffer.reset();
            return;
    ...
    fShaper->shape(utf8, utf8Bytes, *font_runs, trivial_bidi, trivial_script, trivial_lang, nullptr, 0, SK_ScalarMax, this);
    fShapeBuffer.reset();
}

SkSVGTextContext::SkSVGTextContext(const SkSVGRenderContext& ctx, const ShapedTextCallback& cb, const SkSVGTextPath* tpath)
    : fRenderContext(ctx)
    , fCallback(cb)
    ...
    if (tpath) {
        ...
        auto resolve_offset = [this](const SkSVGLength& offset) {
            if (offset.unit() != SkSVGLength::Unit::kPercentage) {
                ...
                // system."
                return fRenderContext.lengthContext().resolve(offset, SkSVGLengthContext::LengthType::kHorizontal);
            }
...
void SkSVGTextContext::shapeFragment(const SkString& txt, const SkSVGRenderContext& ctx, SkSVGXmlSpace xs)
{
...
SkMatrix SkSVGTextContext::PathData::getMatrixAt(float offset) const
{
    ...
    return SkMatrix::Translate(std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity());
}

SkRSXform SkSVGTextContext::computeGlyphXform(SkGlyphID glyph, const SkFont& font, const SkPoint& glyph_pos, const PositionAdjustment& pos_adjust) const
{
    ...
    const auto m
        = fPathData->getMatrixAt(path_offset) * SkMatrix::Translate(-glyph_width * .5f, pos_adjust.offset.fY) * SkMatrix::RotateRad(pos_adjust.rotation);
...
void SkSVGTextContext::flushChunk(const SkSVGRenderContext& ctx)
{
    ...
    for (const auto& run : fRuns) {
        ...
        for (size_t i = 0; i < run.glyphCount; ++i) {
            buf.xforms()[i] = this->computeGlyphXform(run.glyphs[i], run.font, run.glyphPos[i], run.glyhPosAdjust[i]);
        }
...
void SkSVGTextFragment::renderText(const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace xs) const
{
...
void SkSVGTextContainer::appendChild(sk_sp<SkSVGNode> child)
{
    ...
    case SkSVGTag::kTSpan:
        fChildren.push_back(sk_sp<SkSVGTextFragment>(static_cast<SkSVGTextFragment*>(child.release())));
        break;
    default:
        break;
    }
...
void SkSVGTextContainer::onShapeText(const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace) const
{
    SkASSERT(tctx);
...
// https://www.w3.org/TR/SVG11/text.html#WhiteSpace
template <> bool SkSVGAttributeParser::parse(SkSVGXmlSpace* xs)
{
...
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setX(SkSVGAttributeParser::parse<std::vector<SkSVGLength>>("x", name, value))
        ...
        || this->setRotate(SkSVGAttributeParser::parse<std::vector<SkSVGNumberType>>("rotate", name, value))
        || this->setXmlSpace(SkSVGAttributeParser::parse<SkSVGXmlSpace>("xml:space", name, value));
}

void SkSVGTextLiteral::onShapeText(const SkSVGRenderContext& ctx, SkSVGTextContext* tctx, SkSVGXmlSpace xs) const
{
    SkASSERT(tctx);
...
{
    const SkSVGTextContext::ShapedTextCallback render_text
        = [](const SkSVGRenderContext& ctx, const sk_sp<SkTextBlob>& blob, const SkPaint* fill, const SkPaint* stroke) {
              if (fill) {
...
SkRect SkSVGText::onObjectBoundingBox(const SkSVGRenderContext& ctx) const
{
    ...
    const SkSVGTextContext::ShapedTextCallback compute_bounds
        = [&bounds](const SkSVGRenderContext& ctx, const sk_sp<SkTextBlob>& blob, const SkPaint*, const SkPaint*) {
              if (!blob) {
                  return;
              }
...
SkPath SkSVGText::onAsPath(const SkSVGRenderContext& ctx) const
{
    ...
    const SkSVGTextContext::ShapedTextCallback as_path
        = [&builder](const SkSVGRenderContext& ctx, const sk_sp<SkTextBlob>& blob, const SkPaint*, const SkPaint*) {
              if (!blob) {
                  return;
              }
              ...
              for (SkTextBlobRunIterator it(blob.get()); !it.done(); it.next()) {
                  ...
                  it.font().getPaths(
                      it.glyphs(), it.glyphCount(),
                      [](const SkPath* path, const SkMatrix& matrix, void* raw_ctx) {
...
void SkSVGTextPath::onShapeText(const SkSVGRenderContext& ctx, SkSVGTextContext* parent_tctx, SkSVGXmlSpace xs) const
{
...
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setHref(SkSVGAttributeParser::parse<SkSVGIRI>("xlink:href", name, value))
