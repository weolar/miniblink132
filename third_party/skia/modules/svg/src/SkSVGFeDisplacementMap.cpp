...
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setIn2(SkSVGAttributeParser::parse<SkSVGFeInputType>("in2", name, value))
        || this->setXChannelSelector(SkSVGAttributeParser::parse<SkSVGFeDisplacementMap::ChannelSelector>("xChannelSelector", name, value))
        || this->setYChannelSelector(SkSVGAttributeParser::parse<SkSVGFeDisplacementMap::ChannelSelector>("yChannelSelector", name, value))
        || this->setScale(SkSVGAttributeParser::parse<SkSVGNumberType>("scale", name, value));
}

sk_sp<SkImageFilter> SkSVGFeDisplacementMap::onMakeImageFilter(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    ...
    if (fctx.primitiveUnits().type() == SkSVGObjectBoundingBoxUnits::Type::kObjectBoundingBox) {
        ...
        scale = SkSVGLengthContext({ obbt.scale.x, obbt.scale.y })
                    .resolve(SkSVGLength(scale, SkSVGLength::Unit::kPercentage), SkSVGLengthContext::LengthType::kOther);
    }

    return SkImageFilters::DisplacementMap(fXChannelSelector, fYChannelSelector, scale, in2, in, cropRect);
}

SkSVGColorspace SkSVGFeDisplacementMap::resolveColorspace(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    // According to spec https://www.w3.org/TR/SVG11/filters.html#feDisplacementMapElement,
...
template <> bool SkSVGAttributeParser::parse<SkSVGFeDisplacementMap::ChannelSelector>(SkSVGFeDisplacementMap::ChannelSelector* channel)
{
