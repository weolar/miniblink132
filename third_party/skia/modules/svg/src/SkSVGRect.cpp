...
std::tuple<float, float> ResolveOptionalRadii(const SkTLazy<SkSVGLength>& opt_rx, const SkTLazy<SkSVGLength>& opt_ry, const SkSVGLengthContext& lctx)
{
    ...
    const float rx = opt_rx.isValid() ? lctx.resolve(*opt_rx, SkSVGLengthContext::LengthType::kHorizontal) : 0;
    const float ry = opt_ry.isValid() ? lctx.resolve(*opt_ry, SkSVGLengthContext::LengthType::kVertical) : 0;

    return std::make_tuple(opt_rx.isValid() ? rx : ry, opt_ry.isValid() ? ry : rx);
}
...
bool SkSVGRect::parseAndSetAttribute(const char* n, const char* v)
{
    return INHERITED::parseAndSetAttribute(n, v) || this->setX(SkSVGAttributeParser::parse<SkSVGLength>("x", n, v))
        ...
        || this->setHeight(SkSVGAttributeParser::parse<SkSVGLength>("height", n, v)) || this->setRx(SkSVGAttributeParser::parse<SkSVGLength>("rx", n, v))
...
SkRRect SkSVGRect::resolve(const SkSVGLengthContext& lctx) const
{
    ...
    return SkRRect::MakeRectXY(rect, std::min(rx, rect.width() / 2), std::min(ry, rect.height() / 2));
}
