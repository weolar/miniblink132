...
sk_sp<SkImageFilter> SkSVGFeComponentTransfer::onMakeImageFilter(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    ...
    const sk_sp<SkImageFilter> input = fctx.resolveInput(ctx, this->getIn(), this->resolveColorspace(ctx, fctx));

    const auto cf = SkColorFilters::TableARGB(a_tbl.empty() ? nullptr : a_tbl.data(), r_tbl.empty() ? nullptr : r_tbl.data(),
...
std::vector<uint8_t> SkSVGFeFunc::getTable() const
{
    ...
    const auto lerp_from_table_values = [this](auto lerp_func) -> std::vector<uint8_t> {
        ...
        for (size_t k = 0; k < n; ++k) {
            ...
            const SkSVGNumberType v0 = SkTPin(vals[k + 0], 0.f, 1.f), v1 = SkTPin(vals[k + 1], 0.f, 1.f);
            ...
            for (size_t ci = c_start; ci < c_end; ++ci) {
                const float lerp_t = static_cast<float>(ci - c_start) / (c_end - c_start), component = lerp_func(v0, v1, lerp_t);
    ...
    const auto make_table = [&]() -> std::vector<uint8_t> { return lerp_from_table_values([](float v0, float v1, float t) { return v0 + (v1 - v0) * t; }); };

    const auto make_discrete = [&]() -> std::vector<uint8_t> { return lerp_from_table_values([](float v0, float v1, float t) { return v0; }); };

    switch (this->getType()) {
    case SkSVGFeFuncType::kIdentity:
        return {};
    case SkSVGFeFuncType::kTable:
        return make_table();
    case SkSVGFeFuncType::kDiscrete:
        return make_discrete();
    case SkSVGFeFuncType::kLinear:
        return make_linear();
    case SkSVGFeFuncType::kGamma:
        return make_gamma();
    }
...
{
    return INHERITED::parseAndSetAttribute(name, val) || this->setAmplitude(SkSVGAttributeParser::parse<SkSVGNumberType>("amplitude", name, val))
        ...
        || this->setTableValues(SkSVGAttributeParser::parse<std::vector<SkSVGNumberType>>("tableValues", name, val))
        || this->setType(SkSVGAttributeParser::parse<SkSVGFeFuncType>("type", name, val));
}

template <> bool SkSVGAttributeParser::parse(SkSVGFeFuncType* type)
{
