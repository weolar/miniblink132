...
sk_sp<SkImageFilter> ConvertFilterColorspace(sk_sp<SkImageFilter>&& input, SkSVGColorspace src, SkSVGColorspace dst)
{
    if (src == dst) {
...
sk_sp<SkShader> paint_as_shader(const SkPaint& paint)
{
    ...
    if (shader && color.fA < 1.f) {
        ...
        shader = shader->makeWithColorFilter(SkColorFilters::Blend(color, /*colorSpace=*/nullptr, SkBlendMode::kDstIn));
    } else if (!shader) {
...
void SkSVGFilterContext::registerResult(
    const SkSVGStringType& id, const sk_sp<SkImageFilter>& result, const SkRect& subregion, SkSVGColorspace resultColorspace)
{
    SkASSERT(!id.isEmpty());
    fResults[id] = { result, subregion, resultColorspace };
}

void SkSVGFilterContext::setPreviousResult(const sk_sp<SkImageFilter>& result, const SkRect& subregion, SkSVGColorspace resultColorspace)
{
...
std::tuple<sk_sp<SkImageFilter>, SkSVGColorspace> SkSVGFilterContext::getInput(const SkSVGRenderContext& ctx, const SkSVGFeInputType& inputType) const
{
    ...
    case SkSVGFeInputType::Type::kFillPaint: {
        ...
        if (fillPaint.isValid()) {
            auto dither = fillPaint->isDither() ? SkImageFilters::Dither::kYes : SkImageFilters::Dither::kNo;
    ...
    case SkSVGFeInputType::Type::kStrokePaint: {
        ...
        if (strokePaint.isValid()) {
            auto dither = strokePaint->isDither() ? SkImageFilters::Dither::kYes : SkImageFilters::Dither::kNo;
...
SkSVGColorspace SkSVGFilterContext::resolveInputColorspace(const SkSVGRenderContext& ctx, const SkSVGFeInputType& inputType) const
{
    return std::get<1>(this->getInput(ctx, inputType));
}

sk_sp<SkImageFilter> SkSVGFilterContext::resolveInput(const SkSVGRenderContext& ctx, const SkSVGFeInputType& inputType) const
{
    return std::get<0>(this->getInput(ctx, inputType));
}

sk_sp<SkImageFilter> SkSVGFilterContext::resolveInput(const SkSVGRenderContext& ctx, const SkSVGFeInputType& inputType, SkSVGColorspace colorspace) const
{
