...
SkSVGClipPath::SkSVGClipPath()
    : INHERITED(SkSVGTag::kClipPath)
{
}

bool SkSVGClipPath::parseAndSetAttribute(const char* n, const char* v)
{
    return INHERITED::parseAndSetAttribute(n, v) || this->setClipPathUnits(SkSVGAttributeParser::parse<SkSVGObjectBoundingBoxUnits>("clipPathUnits", n, v));
}
...
SkPath SkSVGClipPath::resolveClip(const SkSVGRenderContext& ctx) const
{
    ...
    const auto m = SkMatrix::Translate(obbt.offset.x, obbt.offset.y) * SkMatrix::Scale(obbt.scale.x, obbt.scale.y);
    clip.transform(m);
