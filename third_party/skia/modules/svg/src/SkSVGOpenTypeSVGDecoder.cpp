...
public:
    ...
    sk_sp<skresources::ImageAsset> loadImageAsset(const char rpath[], const char rname[], const char rid[]) const override
    {
...
std::unique_ptr<SkOpenTypeSVGDecoder> SkSVGOpenTypeSVGDecoder::Make(const uint8_t* svg, size_t svgLength)
{
    ...
    return std::unique_ptr<SkOpenTypeSVGDecoder>(new SkSVGOpenTypeSVGDecoder(std::move(skSvg), svgLength));
}
...
bool SkSVGOpenTypeSVGDecoder::render(SkCanvas& canvas, int upem, SkGlyphID glyphId, SkColor foregroundColor, SkSpan<SkColor> palette)
{
