...
bool SkSVGFeComposite::parseAndSetAttribute(const char* name, const char* value)
{
    return INHERITED::parseAndSetAttribute(name, value) ||
        ...
        || this->setOperator(SkSVGAttributeParser::parse<SkSVGFeCompositeOperator>("operator", name, value));
}
...
sk_sp<SkImageFilter> SkSVGFeComposite::onMakeImageFilter(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    ...
    if (fOperator == SkSVGFeCompositeOperator::kArithmetic) {
        ...
        return SkImageFilters::Arithmetic(fK1, fK2, fK3, fK4, enforcePMColor, background, foreground, cropRect);
    } else {
