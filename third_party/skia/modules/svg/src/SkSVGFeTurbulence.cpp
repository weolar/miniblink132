...
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setNumOctaves(SkSVGAttributeParser::parse<SkSVGIntegerType>("numOctaves", name, value))
        || this->setSeed(SkSVGAttributeParser::parse<SkSVGNumberType>("seed", name, value))
        || this->setBaseFrequency(SkSVGAttributeParser::parse<SkSVGFeTurbulenceBaseFrequency>("baseFrequency", name, value))
        || this->setTurbulenceType(SkSVGAttributeParser::parse<SkSVGFeTurbulenceType>("type", name, value));
}

template <> bool SkSVGAttributeParser::parse<SkSVGFeTurbulenceBaseFrequency>(SkSVGFeTurbulenceBaseFrequency* freq)
{
...
template <> bool SkSVGAttributeParser::parse<SkSVGFeTurbulenceType>(SkSVGFeTurbulenceType* type)
{
...
sk_sp<SkImageFilter> SkSVGFeTurbulence::onMakeImageFilter(const SkSVGRenderContext& ctx, const SkSVGFilterContext& fctx) const
{
    ...
    case SkSVGFeTurbulenceType::Type::kTurbulence:
        shader = SkShaders::MakeTurbulence(fBaseFrequency.freqX(), fBaseFrequency.freqY(), fNumOctaves, fSeed, tileSize);
        break;
    case SkSVGFeTurbulenceType::Type::kFractalNoise:
        shader = SkShaders::MakeFractalNoise(fBaseFrequency.freqX(), fBaseFrequency.freqY(), fNumOctaves, fSeed, tileSize);
        break;
    }
