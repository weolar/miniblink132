...
SkSVGAttributeParser::SkSVGAttributeParser(const char attributeString[])
    ...
    , fEndPos(fCurPos + strlen(attributeString))
{
}
...
bool SkSVGAttributeParser::parseIdentToken(SkString* ident)
{
    ...
    if (this->parseExpectedStringToken("--")) {
        ...
        if (this->parseEscape(&c)) {
            ...
            if ((c < 'a' || 'z' < c) && (c < 'A' || 'Z' < c) && (c != '_') && (c < 0x80 || 0x10FFFF < c)) {
                return false;
            }
            ident->appendUnichar(c);
        }
    ...
    while (fCurPos < fEndPos) {
        ...
        if ((c < 'a' || 'z' < c) && (c < 'A' || 'Z' < c) && (c < '0' || '9' < c) && (c != '_') && (c != '-') && (c < 0x80 || 0x10FFFF < c)) {
            break;
        }
        ident->appendUnichar(c);
        fCurPos = next;
...
bool SkSVGAttributeParser::parseRGBColorToken(SkColor* c)
{
    return this->parseParenthesized(
        ...
        [this](SkColor* c) -> bool {
            int32_t r, g, b;
            if (this->parseColorComponentToken(&r) && this->parseSepToken() && this->parseColorComponentToken(&g) && this->parseSepToken()
                && this->parseColorComponentToken(&b)) {

                *c = SkColorSetRGB(static_cast<uint8_t>(r), static_cast<uint8_t>(g), static_cast<uint8_t>(b));
                return true;
            }
            return false;
        },
...
bool SkSVGAttributeParser::parseRGBAColorToken(SkColor* c)
{
    return this->parseParenthesized(
        ...
        [this](SkColor* c) -> bool {
            int32_t r, g, b, a;
            if (this->parseColorComponentToken(&r) && this->parseSepToken() && this->parseColorComponentToken(&g) && this->parseSepToken()
                && this->parseColorComponentToken(&b) && this->parseSepToken() && this->parseColorComponentScalarToken(&a)) {

                *c = SkColorSetARGB(static_cast<uint8_t>(a), static_cast<uint8_t>(r), static_cast<uint8_t>(g), static_cast<uint8_t>(b));
                return true;
            }
            return false;
        },
...
{
    return this->parseHexColorToken(c) || this->parseNamedColorToken(c) || this->parseRGBAColorToken(c) || this->parseRGBColorToken(c);
}
...
template <> bool SkSVGAttributeParser::parse(SkSVGColorType* color)
{
    this->parseWSToken();
...
template <> bool SkSVGAttributeParser::parse(SkSVGIRI* iri)
{
...
bool SkSVGAttributeParser::parseFuncIRI(SkSVGFuncIRI* iri)
{
    return this->parseParenthesized(
        "url",
        [this](SkSVGFuncIRI* iriResult) -> bool {
            SkSVGIRI iri;
            if (this->parse(&iri)) {
...
template <> bool SkSVGAttributeParser::parse(SkSVGStringType* result)
{
...
template <> bool SkSVGAttributeParser::parse(SkSVGNumberType* number)
{
    // consume WS
    this->parseWSToken();
...
template <> bool SkSVGAttributeParser::parse(SkSVGLength* length)
{
    SkScalar s;
...
bool SkSVGAttributeParser::parseViewBox(SkSVGViewBoxType* vb)
{
    ...
    if (this->parseScalarToken(&x) && this->parseSepToken() && this->parseScalarToken(&y) && this->parseSepToken() && this->parseScalarToken(&w)
        && this->parseSepToken() && this->parseScalarToken(&h)) {
...
template <typename Func, typename T> bool SkSVGAttributeParser::parseParenthesized(const char* prefix, Func f, T* result)
{
    RestoreCurPos restoreCurPos(this);
...
template <> bool SkSVGAttributeParser::parse(SkSVGTransformType* t)
{
    ...
    while (true) {
        ...
        if (!(this->parseMatrixToken(&m) || this->parseTranslateToken(&m) || this->parseScaleToken(&m) || this->parseRotateToken(&m)
...
template <> bool SkSVGAttributeParser::parse(SkSVGPaint* paint)
{
    SkSVGColor c;
    SkSVGFuncIRI iri;
    bool parsedValue = false;
...
template <> bool SkSVGAttributeParser::parse(SkSVGFuncIRI* firi)
{
    SkSVGStringType iri;
    bool parsedValue = false;
...
template <> bool SkSVGAttributeParser::parse(SkSVGLineCap* cap)
{
    static const struct {
        SkSVGLineCap fType;
        const char* fName;
...
template <> bool SkSVGAttributeParser::parse(SkSVGLineJoin* join)
{
    static const struct {
...
template <> bool SkSVGAttributeParser::parse(SkSVGObjectBoundingBoxUnits* objectBoundingBoxUnits)
{
    bool parsedValue = false;
    if (this->parseExpectedStringToken("userSpaceOnUse")) {
        *objectBoundingBoxUnits = SkSVGObjectBoundingBoxUnits(SkSVGObjectBoundingBoxUnits::Type::kUserSpaceOnUse);
        parsedValue = true;
    } else if (this->parseExpectedStringToken("objectBoundingBox")) {
        *objectBoundingBoxUnits = SkSVGObjectBoundingBoxUnits(SkSVGObjectBoundingBoxUnits::Type::kObjectBoundingBox);
        parsedValue = true;
    }
    return parsedValue && this->parseEOSToken();
}
...
template <> bool SkSVGAttributeParser::parse(SkSVGPointsType* points)
{
    SkSVGPointsType pts;
...
template <> bool SkSVGAttributeParser::parse(SkSVGFillRule* fillRule)
{
    static const struct {
...
template <> bool SkSVGAttributeParser::parse(SkSVGVisibility* visibility)
{
    static const struct {
...
template <> bool SkSVGAttributeParser::parse(SkSVGDashArray* dashArray)
{
    bool parsedValue = false;
    if (this->parseExpectedStringToken("none")) {
...
template <> bool SkSVGAttributeParser::parse(SkSVGFontFamily* family)
{
    bool parsedValue = false;
...
template <> bool SkSVGAttributeParser::parse(SkSVGFontSize* size)
{
    bool parsedValue = false;
    if (this->parseExpectedStringToken("inherit")) {
        *size = SkSVGFontSize();
...
template <> bool SkSVGAttributeParser::parse(SkSVGFontStyle* style)
{
...
template <> bool SkSVGAttributeParser::parse(SkSVGFontWeight* weight)
{
...
template <> bool SkSVGAttributeParser::parse(SkSVGTextAnchor* anchor)
{
...
template <> bool SkSVGAttributeParser::parse(SkSVGPreserveAspectRatio* par)
{
    return this->parsePreserveAspectRatio(par);
}

// https://www.w3.org/TR/SVG11/types.html#DataTypeCoordinates
template <typename T> bool SkSVGAttributeParser::parseList(std::vector<T>* vals)
{
    SkASSERT(vals->empty());
...
template <> bool SkSVGAttributeParser::parse(std::vector<SkSVGLength>* lengths)
{
    return this->parseList(lengths);
}

template <> bool SkSVGAttributeParser::parse(std::vector<SkSVGNumberType>* numbers)
{
    return this->parseList(numbers);
}

template <> bool SkSVGAttributeParser::parse(SkSVGColorspace* colorspace)
{
...
template <> bool SkSVGAttributeParser::parse(SkSVGDisplay* display)
{
    static const struct {
        SkSVGDisplay fType;
        const char* fName;
