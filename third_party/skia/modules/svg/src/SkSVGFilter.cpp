...
bool SkSVGFilter::parseAndSetAttribute(const char* name, const char* value)
{
    return INHERITED::parseAndSetAttribute(name, value) || this->setX(SkSVGAttributeParser::parse<SkSVGLength>("x", name, value))
        ...
        || this->setFilterUnits(SkSVGAttributeParser::parse<SkSVGObjectBoundingBoxUnits>("filterUnits", name, value))
        || this->setPrimitiveUnits(SkSVGAttributeParser::parse<SkSVGObjectBoundingBoxUnits>("primitiveUnits", name, value));
}
...
sk_sp<SkImageFilter> SkSVGFilter::buildFilterDAG(const SkSVGRenderContext& ctx) const
{
    ...
    SkSVGFilterContext fctx(ctx.resolveOBBRect(fX, fY, fWidth, fHeight, fFilterUnits), fPrimitiveUnits);
