...
{
    return SkRect::MakeXYWH(this->resolve(x, SkSVGLengthContext::LengthType::kHorizontal), this->resolve(y, SkSVGLengthContext::LengthType::kVertical),
        this->resolve(w, SkSVGLengthContext::LengthType::kHorizontal), this->resolve(h, SkSVGLengthContext::LengthType::kVertical));
}
...
static sk_sp<SkPathEffect> dash_effect(const SkSVGPresentationAttributes& props, const SkSVGLengthContext& lctx)
{
    ...
    const auto phase = lctx.resolve(*props.fStrokeDashOffset, SkSVGLengthContext::LengthType::kOther);
...
SkSVGRenderContext::SkSVGRenderContext(SkCanvas* canvas, const sk_sp<SkFontMgr>& fmgr, const sk_sp<skresources::ResourceProvider>& rp,
    const SkSVGIDMapper& mapper, const SkSVGLengthContext& lctx, const SkSVGPresentationContext& pctx, const OBBScope& obbs,
...
SkSVGRenderContext::SkSVGRenderContext(const SkSVGRenderContext& other)
    : SkSVGRenderContext(other.fCanvas, other.fFontMgr, other.fResourceProvider, other.fIDMapper, *other.fLengthContext, *other.fPresentationContext,
        other.fOBBScope, other.fTextShapingFactory)
{
}

SkSVGRenderContext::SkSVGRenderContext(const SkSVGRenderContext& other, SkCanvas* canvas)
    : SkSVGRenderContext(canvas, other.fFontMgr, other.fResourceProvider, other.fIDMapper, *other.fLengthContext, *other.fPresentationContext, other.fOBBScope,
...
SkSVGRenderContext::SkSVGRenderContext(const SkSVGRenderContext& other, const SkSVGNode* node)
    : SkSVGRenderContext(other.fCanvas, other.fFontMgr, other.fResourceProvider, other.fIDMapper, *other.fLengthContext, *other.fPresentationContext,
        OBBScope { node, this }, other.fTextShapingFactory)
{
}
...
void SkSVGRenderContext::applyPresentationAttributes(const SkSVGPresentationAttributes& attrs, uint32_t flags)
{
...
void SkSVGRenderContext::applyOpacity(SkScalar opacity, uint32_t flags, bool hasFilter)
{
    ...
    const bool hasFill = props.fFill->type() != SkSVGPaint::Type::kNone, hasStroke = props.fStroke->type() != SkSVGPaint::Type::kNone;
...
SkTLazy<SkPaint> SkSVGRenderContext::commonPaint(const SkSVGPaint& paint_selector, float paint_opacity) const
{
    ...
    case SkSVGPaint::Type::kIRI: {
        ...
        SkSVGRenderContext local_ctx(fCanvas, fFontMgr, fResourceProvider, fIDMapper, *fLengthContext, pctx, fOBBScope, fTextShapingFactory);
...
SkTLazy<SkPaint> SkSVGRenderContext::strokePaint() const
{
    ...
    if (p.isValid()) {
        ...
        p->setStrokeWidth(fLengthContext->resolve(*props.fStrokeWidth, SkSVGLengthContext::LengthType::kOther));
...
SkRect SkSVGRenderContext::resolveOBBRect(
    const SkSVGLength& x, const SkSVGLength& y, const SkSVGLength& w, const SkSVGLength& h, SkSVGObjectBoundingBoxUnits obbu) const
{
    ...
    return SkRect::MakeXYWH(obbt.scale.x * r.x() + obbt.offset.x, obbt.scale.y * r.y() + obbt.offset.y, obbt.scale.x * r.width(), obbt.scale.y * r.height());
}
