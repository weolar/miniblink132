...
public:
    RunHandler(const char* utf8Text, size_t)
        : fUtf8Text(utf8Text)
    {
    }
    using RunCallback = void (*)(void* context, const char* utf8Text, size_t utf8TextBytes, size_t glyphCount, const SkGlyphID* glyphs,
        const SkPoint* positions, const uint32_t* clusters, const SkFont& font);
    ...
    SkPoint endPoint() const
    {
        return fOffset;
    }
    SkPoint finalPosition() const
    {
    ...
    const std::vector<size_t>& lineEndOffsets() const
    {
    ...
    SkRect finalRect(const SkFont& font) const
    {
        if (0 == fMaxRunAscent || 0 == fMaxRunDescent) {
            ...
            return { fCurrentPosition.x(), fCurrentPosition.y() + fMaxRunAscent, fCurrentPosition.x() + font.getSize(), fCurrentPosition.y() + fMaxRunDescent };
        }
    }
...
void RunHandler::commitRunBuffer(const RunInfo& info)
{
    ...
    if (fCallbackFunction) {
        fCallbackFunction(fCallbackContext, fUtf8Text, info.utf8Range.end(), info.glyphCount, fCurrentGlyphs, fCurrentPoints, fClusters, info.fFont);
    }
...
static SkRect selection_box(const SkFontMetrics& metrics, float advance, SkPoint pos)
{
    ...
    return SkRect { pos.x(), pos.y() + metrics.fAscent, pos.x() + advance, pos.y() + metrics.fDescent }.makeSorted();
}

static void set_character_bounds(void* context, const char* utf8Text, size_t utf8TextBytes, size_t glyphCount, const SkGlyphID* glyphs,
    const SkPoint* positions, const uint32_t* clusters, const SkFont& font)
{
    SkASSERT(context);
...
ShapeResult SkPlainTextEditor::Shape(const char* utf8Text, size_t textByteLen, const SkFont& font, sk_sp<SkFontMgr> fontMgr, const char* locale, float width)
{
    ShapeResult result;
    ...
    if (textByteLen) {
...
#if defined(SK_SHAPER_HARFBUZZ_AVAILABLE) && defined(SK_SHAPER_UNICODE_AVAILABLE)
        bidi = SkShapers::unicode::BidiRunIterator(unicode, utf8Text, textByteLen, kBidiLevelLTR);
#endif
        if (!bidi) {
        ...
        std::unique_ptr<SkShaper::LanguageRunIterator> language = SkShaper::MakeStdLanguageRunIterator(utf8Text, textByteLen);
        SkASSERT(language);

        std::unique_ptr<SkShaper::ScriptRunIterator> script = SkShaper::MakeScriptRunIterator(utf8Text, textByteLen, SkSetFourByteTag('Z', 'z', 'z', 'z'));
        SkASSERT(script);

        std::unique_ptr<SkShaper::FontRunIterator> fontRuns = SkShaper::MakeFontMgrRunIterator(utf8Text, textByteLen, font, fontMgr);
        SkASSERT(fontRuns);

        shaper->shape(utf8Text, textByteLen, *fontRuns, *bidi, *script, *language, nullptr, 0, width, &runHandler);
