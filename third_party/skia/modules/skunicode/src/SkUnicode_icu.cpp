...
public:
    ...
    {
        UErrorCode status = U_ZERO_ERROR;
        ICUUText text(sk_utext_openUChars(nullptr, reinterpret_cast<const UChar*>(&utftext16[0]), utf16Units, &status));
...
class SkIcuBreakIteratorCache final {
    ...
    public:
        BreakIteratorRef(ICUBreakIterator iter)
            : breakIterator(iter.release())
            , fRefCnt(1)
        {
            ++Instances;
        }
...
public:
    ...
    ICUBreakIterator makeBreakIterator(SkUnicode::BreakType type, const char* bcp47)
    {
        ...
        auto make = [](const Request& request) -> UBreakIterator* {
            UErrorCode status = U_ZERO_ERROR;
            UBreakIterator* bi = sk_ubrk_open(convertType(request.fType), request.fIcuLocale.c_str(), nullptr, 0, &status);
            if (U_FAILURE(status)) {
                SkDEBUGF("Break error: %s", sk_u_errorName(status));
            }
            return bi;
        ...
        if (SkGetICULib()->f_ubrk_getLocaleByType) {
            const char* actualLocale = SkGetICULib()->f_ubrk_getLocaleByType(newIter.get(), ULOC_ACTUAL_LOCALE, &status);
...
class SkUnicode_icu : public SkUnicode {
    ...
    static bool extractPositions(const char utf8[], int utf8Units, BreakType type, const char* locale, const std::function<void(int, int)>& setBreak)
    {
...
public:
    ...
    SkString toUpper(const SkString& str, const char* locale) override
    {
        ...
        const auto upper16len = sk_u_strToUpper(nullptr, 0, (UChar*)(str16.c_str()), str16.size(), locale, &icu_err);
        ...
        sk_u_strToUpper((UChar*)(upper16.get()), SkToS32(upper16.size()), (UChar*)(str16.c_str()), str16.size(), locale, &icu_err);
    ...
    {
        SkUnicode_icu::extractPositions(utf8, utf8Units, BreakType::kSentences, nullptr, [&](int pos, int status) { results->emplace_back(pos); });
        return true;
    }
    ...
    bool computeCodeUnitFlags(char utf8[], int utf8Units, bool replaceTabs, TArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        ...
        SkUnicode_icu::extractPositions(utf8, utf8Units, BreakType::kLines, nullptr, // TODO: locale
            [&](int pos, int status) {
                (*results)[pos] |= status == UBRK_LINE_HARD ? CodeUnitFlags::kHardLineBreakBefore : CodeUnitFlags::kSoftLineBreakBefore;
            });
    ...
    bool computeCodeUnitFlags(char16_t utf16[], int utf16Units, bool replaceTabs, TArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        ...
        // Get white spaces
        this->forEachCodepoint((char16_t*)&utf16[0], utf16Units, [this, results, replaceTabs, &utf16](SkUnichar unichar, int32_t start, int32_t end) {
        ...
        this->forEachBreak((char16_t*)&utf16[0], utf16Units, SkUnicode::BreakType::kGraphemes,
            [results](SkBreakIterator::Position pos, SkBreakIterator::Status) { (*results)[pos] |= CodeUnitFlags::kGraphemeStart; });
        // Get line breaks
        this->forEachBreak(
            (char16_t*)&utf16[0], utf16Units, SkUnicode::BreakType::kLines, [results](SkBreakIterator::Position pos, SkBreakIterator::Status status) {
