...
template <typename T, typename = void> struct SkUbrkClone {
    static UBreakIterator* clone(T bi, UErrorCode* status)
    {
        return ubrk_safeClone(bi, nullptr, nullptr, status);
    }
};
template <typename T> struct SkUbrkClone<T, std::void_t<decltype(ubrk_clone(std::declval<T>(), nullptr))>> {
    static UBreakIterator* clone(T bi, UErrorCode* status)
    {
...
template <typename T, typename = void> struct SkUbrkGetLocaleByType {
...
template <typename T>
struct SkUbrkGetLocaleByType<T, std::void_t<decltype(ubrk_getLocaleByType(std::declval<T>(), std::declval<ULocDataLocaleType>(), nullptr))>> {
    static const char* getLocaleByType(T bi, ULocDataLocaleType type, UErrorCode* status)
    {
...
std::unique_ptr<SkICULib> SkLoadICULib()
{
    return std::make_unique<SkICULib>(SkICULib {
        SKICU_EMIT_FUNCS & SkUbrkClone<const UBreakIterator*>::clone,
        nullptr,
