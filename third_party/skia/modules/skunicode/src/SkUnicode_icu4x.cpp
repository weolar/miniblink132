...
private:
    ...
    bool markHardLineBreaksHack(char utf8[], int utf8Units, skia_private::TArray<SkUnicode::CodeUnitFlags, true>* results)
    {
    ...
    bool markLineBreaks(char utf8[], int utf8Units, bool hardLineBreaks, skia_private::TArray<SkUnicode::CodeUnitFlags, true>* results)
    {
        if (utf8Units == 0) {
            return true;
        }
        ...
        const auto lineBreakingOptions = hardLineBreaks ? ICU4XLineBreakOptionsV1 { ICU4XLineBreakStrictness::Strict, ICU4XLineBreakWordOption::Normal }
    ...
    bool markCharacters(char utf8[], int utf8Units, bool replaceTabs, skia_private::TArray<SkUnicode::CodeUnitFlags, true>* results)
    {
...
public:
    ...
    Level getLevelAt(Position pos) override
    {
        auto found = std::lower_bound(fRegions->begin(), fRegions->end(), SkUnicode::BidiRegion(pos, pos, 0),
            [](const SkUnicode::BidiRegion& a, const SkUnicode::BidiRegion& b) { return a.start <= b.start && a.end <= b.end; });
        return found->level;
    }
};

std::unique_ptr<SkBidiIterator> SkUnicode_icu4x::makeBidiIterator(const uint16_t text[], int count, SkBidiIterator::Direction dir)
{
    if (fRegions) {
        fRegions->clear();
    } else {
...
std::unique_ptr<SkBidiIterator> SkUnicode_icu4x::makeBidiIterator(const char text[], int count, SkBidiIterator::Direction dir)
{
    if (fRegions) {
        fRegions->clear();
    } else {
