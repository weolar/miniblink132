...
public:
    ...
    {
        return fBidiFact->bidi_getLevelAt(fBidi.get(), pos);
    }
...
std::unique_ptr<SkBidiIterator> SkBidiFactory::MakeIterator(const uint16_t utf16[], int utf16Units, SkBidiIterator::Direction dir) const
{
    ...
    return std::unique_ptr<SkBidiIterator>(new SkBidiIterator_icu(std::move(bidi), sk_ref_sp(this)));
}

std::unique_ptr<SkBidiIterator> SkBidiFactory::MakeIterator(const char utf8[], int utf8Units, SkBidiIterator::Direction dir) const
{
...
bool SkBidiFactory::ExtractBidi(const char utf8[], int utf8Units, SkUnicode::TextDirection dir, std::vector<SkUnicode::BidiRegion>* bidiRegions) const
{
    ...
    // The required lifetime of utf16 isn't well documented.
    // It appears it isn't used after ubidi_setPara except through ubidi_getText.
    this->bidi_setPara(bidi.get(), (const UChar*)utf16.c_str(), utf16.size(), bidiLevel, nullptr, &status);
    if (U_FAILURE(status)) {
        SkDEBUGF("Bidi error: %s", this->errorName(status));
        return false;
    }
