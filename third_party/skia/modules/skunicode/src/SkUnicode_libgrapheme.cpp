...
public:
    ...
    bool computeCodeUnitFlags(char utf8[], int utf8Units, bool replaceTabs, skia_private::TArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        ...
        while (lineBreak < utf8Units) {
            ...
            (*results)[lineBreak] |= this->isHardBreak(codePoint) ? CodeUnitFlags::kHardLineBreakBefore : CodeUnitFlags::kSoftLineBreakBefore;
        }
...
public:
    explicit SkBreakIterator_libgrapheme(SkUnicode_libgrapheme* unicode)
        : fUnicode(unicode)
    {
    }
    ...
    {
        return fLineBreaks[fLineBreakIndex].breakType == SkUnicode::LineBreakType::kHardLineBreak ? SkUnicode::CodeUnitFlags::kHardLineBreakBefore
    ...
    bool setText(const char utftext8[], int utf8Units) override
    {
        ...
        for (size_t pos = 0; pos < utf8Units;) {
            ...
            fLineBreaks.emplace_back(
                pos, fUnicode->isHardBreak(codePoint) ? SkUnicode::LineBreakType::kHardLineBreak : SkUnicode::LineBreakType::kSoftLineBreak);
        }
...
std::unique_ptr<SkBidiIterator> SkUnicode_libgrapheme::makeBidiIterator(const uint16_t text[], int count, SkBidiIterator::Direction dir)
{
    return fBidiFact->MakeIterator(text, count, dir);
}
std::unique_ptr<SkBidiIterator> SkUnicode_libgrapheme::makeBidiIterator(const char text[], int count, SkBidiIterator::Direction dir)
{
    return fBidiFact->MakeIterator(text, count, dir);
}
std::unique_ptr<SkBreakIterator> SkUnicode_libgrapheme::makeBreakIterator(const char locale[], BreakType breakType)
{
