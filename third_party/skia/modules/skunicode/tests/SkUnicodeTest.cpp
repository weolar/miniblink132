...
#define DEF_TEST_ICU(name, reporter)                                                                                                                           \
    DEF_TEST(name##ICU, reporter)                                                                                                                              \
    {                                                                                                                                                          \
        name(reporter, SkUnicodes::ICU::Make());                                                                                                               \
    }
#else
#define DEF_TEST_ICU(name, reporter)
#endif
...
#define DEF_TEST_ICU4X(name, reporter)                                                                                                                         \
    DEF_TEST(name##ICU4X, reporter)                                                                                                                            \
    {                                                                                                                                                          \
        name(reporter, SkUnicodes::ICU4X::Make());                                                                                                             \
    }
#else
#define DEF_TEST_ICU4X(name, reporter)
#endif
...
#define DEF_TEST_LIBGRAPHEME(name, reporter)                                                                                                                   \
    DEF_TEST(name##LIBGRAPHEME, reporter)                                                                                                                      \
    {                                                                                                                                                          \
        name(reporter, SkUnicodes::Libgrapheme::Make());                                                                                                       \
    }
#else
#define DEF_TEST_LIBGRAPHEME(name, reporter)
#endif
...
UNIX_ONLY_TEST(SkUnicode_Client, reporter)
{
    ...
    auto client = SkUnicodes::Client::Make(SkSpan<char>(&utf8[0], utf8.size()), {}, {}, {});
...
DEF_TEST_ICU_UNICODES(SkUnicode_GetBidiRegionsLTR, reporter)
{
    ...
    auto result = unicode->getBidiRegions(text.data(), text.size(), SkUnicode::TextDirection::kLTR, &results);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, results.size() == 1);
    REPORTER_ASSERT(reporter, results[0].start == 0 && results[0].end == text.size() && results[0].level == 0);
}
...
DEF_TEST_ICU_UNICODES(SkUnicode_GetBidiRegionsRTL, reporter)
{
    ...
    std::vector<SkUnicode::BidiRegion> results;
    auto result = unicode->getBidiRegions(text.data(), text.size(), SkUnicode::TextDirection::kRTL, &results);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, results.size() == 1);
    REPORTER_ASSERT(reporter, results[0].start == 0 && results[0].end == text.size() && results[0].level == 1);
}
...
DEF_TEST_ICU_UNICODES(SkUnicode_GetBidiRegionsMix1, reporter)
{
    ...
    REPORTER_ASSERT(reporter, results.size() == expected.size());
    for (auto i = 0ul; i < results.size(); ++i) {
        REPORTER_ASSERT(reporter, results[i].start == expected[i].start && results[i].end == expected[i].end && results[i].level == expected[i].level);
    }
}
...
DEF_TEST_ICU_UNICODES(SkUnicode_ComputeCodeUnitFlags, reporter)
{
    ...
    auto result = unicode->computeCodeUnitFlags(text.data(), text.size(),
        /*replaceTabs=*/true, &results);
    REPORTER_ASSERT(reporter, result);
    ...
    for (auto i = 0; i < results.size(); ++i) {
        ...
        if (i == 0 || i == 2 || i == 5 || i == 9 || i == 14 || i == 20 || i == 27 || i == 34) {
...
DEF_TEST_UNICODES(SkUnicode_ReorderVisual, reporter)
{
    ...
    auto reorder = [&](std::vector<SkUnicode::BidiLevel> levels, std::vector<int32_t> expected) {
