...
Half_from_F(F f)
{
    ...
    return cast<U16>(if_then_else(em < 0x38800000, (U32)F0, (s >> 16) + (em >> 13) - ((127 - 15) << 10)));
#endif
}
...
SI F min_(F x, F y)
{
    return (F)vminq_f32((float32x4_t)x, (float32x4_t)y);
}
SI F max_(F x, F y)
{
    return (F)vmaxq_f32((float32x4_t)x, (float32x4_t)y);
}
#elif defined(__loongarch_sx)
SI F min_(F x, F y)
{
    return (F)__lsx_vfmin_s(x, y);
}
SI F max_(F x, F y)
{
    return (F)__lsx_vfmax_s(x, y);
}
#else
SI F min_(F x, F y)
{
...
SI F approx_exp2(F x)
{
    ...
    F fbits = (1.0f * (1 << 23)) * (x + 121.274057500f - 1.490129070f * fract + 27.728023300f / (4.84252568f - fract));
...
SI F apply_pq(const skcms_TransferFunction* tf, F x)
{
    ...
    F v = approx_pow(max_(tf->a + tf->b * approx_pow(x, tf->c), F0) / (tf->d + tf->e * approx_pow(x, tf->c)), tf->f);
...
#elif N == 16
    return T { p[0], p[3], p[6], p[9], p[12], p[15], p[18], p[21], p[24], p[27], p[30], p[33], p[36], p[39], p[42], p[45] };
#endif
}
...
#elif N == 16
    return T { p[0], p[4], p[8], p[12], p[16], p[20], p[24], p[28], p[32], p[36], p[40], p[44], p[48], p[52], p[56], p[60] };
#endif
}
...
#elif N == 16
    U8 v = { p[ix[0]], p[ix[1]], p[ix[2]], p[ix[3]], p[ix[4]], p[ix[5]], p[ix[6]], p[ix[7]], p[ix[8]], p[ix[9]], p[ix[10]], p[ix[11]], p[ix[12]], p[ix[13]],
        p[ix[14]], p[ix[15]] };
#endif
    return v;
}
...
#elif N == 8
    U16 v = { load_16(ix[0]), load_16(ix[1]), load_16(ix[2]), load_16(ix[3]), load_16(ix[4]), load_16(ix[5]), load_16(ix[6]), load_16(ix[7]) };
#elif N == 16
    U16 v = { load_16(ix[0]), load_16(ix[1]), load_16(ix[2]), load_16(ix[3]), load_16(ix[4]), load_16(ix[5]), load_16(ix[6]), load_16(ix[7]), load_16(ix[8]),
        load_16(ix[9]), load_16(ix[10]), load_16(ix[11]), load_16(ix[12]), load_16(ix[13]), load_16(ix[14]), load_16(ix[15]) };
#endif
    return v;
}
...
#elif N == 8
    U32 v = { load_32(ix[0]), load_32(ix[1]), load_32(ix[2]), load_32(ix[3]), load_32(ix[4]), load_32(ix[5]), load_32(ix[6]), load_32(ix[7]) };
#elif N == 16
    U32 v = { load_32(ix[0]), load_32(ix[1]), load_32(ix[2]), load_32(ix[3]), load_32(ix[4]), load_32(ix[5]), load_32(ix[6]), load_32(ix[7]), load_32(ix[8]),
        load_32(ix[9]), load_32(ix[10]), load_32(ix[11]), load_32(ix[12]), load_32(ix[13]), load_32(ix[14]), load_32(ix[15]) };
#endif
...
SI U32 gather_24(const uint8_t* p, I32 ix)
{
    ...
    U32 v = { load_24_32(ix[0]), load_24_32(ix[1]), load_24_32(ix[2]), load_24_32(ix[3]), load_24_32(ix[4]), load_24_32(ix[5]), load_24_32(ix[6]),
        load_24_32(ix[7]) };
#elif N == 8
    (void)load_24_32;
...
#if defined(__clang__)
    Half_I64 lo = (Half_I64)__builtin_ia32_gatherd_q256(zero, p8, ix_lo, mask, 1), hi = (Half_I64)__builtin_ia32_gatherd_q256(zero, p8, ix_hi, mask, 1);
#elif defined(__GNUC__)
    Half_I64 lo = (Half_I64)__builtin_ia32_gathersiv4di(zero, p8, ix_lo, mask, 1), hi = (Half_I64)__builtin_ia32_gathersiv4di(zero, p8, ix_hi, mask, 1);
#endif
    store((char*)v + 0, lo);
    store((char*)v + 32, hi);
#elif N == 16
...
static void clut(
    uint32_t input_channels, uint32_t output_channels, const uint8_t grid_points[4], const uint8_t* grid_8, const uint8_t* grid_16, F* r, F* g, F* b, F* a)
{
...
{
    clut(a2b->input_channels, a2b->output_channels, a2b->grid_points, a2b->grid_8, a2b->grid_16, r, g, b, &a);
}
static void clut(const skcms_B2A* b2a, F* r, F* g, F* b, F* a)
{
    clut(b2a->input_channels, b2a->output_channels, b2a->grid_points, b2a->grid_8, b2a->grid_16, r, g, b, a);
}
...
#define STAGE_PARAMS(MAYBE_REF)                                                                                                                                \
    SKCMS_MAYBE_UNUSED const char *src, SKCMS_MAYBE_UNUSED char *dst, SKCMS_MAYBE_UNUSED F MAYBE_REF r, SKCMS_MAYBE_UNUSED F MAYBE_REF g,                      \
        SKCMS_MAYBE_UNUSED F MAYBE_REF b, SKCMS_MAYBE_UNUSED F MAYBE_REF a, SKCMS_MAYBE_UNUSED int i
...
#define STAGE(name, arg) DECLARE_STAGE(name, arg, [[clang::musttail]] return (*list.fn)(list, ctx, src, dst, r, g, b, a, i))

#define FINAL_STAGE(name, arg) DECLARE_STAGE(name, arg, /* Stop executing stages and return to the caller. */)
...
#else
    U64 px = load<U64>(rgba);
    U16 R = cast<U16>((px >> 0) & 0xffff), G = cast<U16>((px >> 16) & 0xffff), B = cast<U16>((px >> 32) & 0xffff), A = cast<U16>((px >> 48) & 0xffff);
#endif
    r = F_from_Half(R);
    g = F_from_Half(G);
    b = F_from_Half(B);
...
STAGE(matrix_3x3, const skcms_Matrix3x3* matrix)
{
    ...
    F R = m[0] * r + m[1] * g + m[2] * b, G = m[3] * r + m[4] * g + m[5] * b, B = m[6] * r + m[7] * g + m[8] * b;
...
STAGE(matrix_3x4, const skcms_Matrix3x4* matrix)
{
    ...
    F R = m[0] * r + m[1] * g + m[2] * b + m[3], G = m[4] * r + m[5] * g + m[6] * b + m[7], B = m[8] * r + m[9] * g + m[10] * b + m[11];
...
{
    store<U16>(dst + 2 * i,
        cast<U16>(to_fixed(r * 15) << 12) | cast<U16>(to_fixed(g * 15) << 8) | cast<U16>(to_fixed(b * 15) << 4) | cast<U16>(to_fixed(a * 15) << 0));
}

FINAL_STAGE(store_565, NoCtx)
{
    store<U16>(dst + 2 * i, cast<U16>(to_fixed(r * 31) << 0) | cast<U16>(to_fixed(g * 63) << 5) | cast<U16>(to_fixed(b * 31) << 11));
}
...
FINAL_STAGE(store_888, NoCtx)
{
    ...
    U16 R = cast<U16>(to_fixed(r * 255)), G = cast<U16>(to_fixed(g * 255)), B = cast<U16>(to_fixed(b * 255));
...
{
    store(dst + 4 * i,
        cast<U32>(to_fixed(r * 255)) << 0 | cast<U32>(to_fixed(g * 255)) << 8 | cast<U32>(to_fixed(b * 255)) << 16 | cast<U32>(to_fixed(a * 255)) << 24);
}
...
FINAL_STAGE(store_101010x_XR, NoCtx)
{
    ...
    static constexpr float range = max - min;
    store(dst + 4 * i,
        cast<U32>(to_fixed(((r - min) / range) * 1023)) << 0 | cast<U32>(to_fixed(((g - min) / range) * 1023)) << 10
...
{
    store(dst + 4 * i,
        cast<U32>(to_fixed(r * 1023)) << 0 | cast<U32>(to_fixed(g * 1023)) << 10 | cast<U32>(to_fixed(b * 1023)) << 20 | cast<U32>(to_fixed(a * 3)) << 30);
}
...
FINAL_STAGE(store_16161616LE, NoCtx)
{
    ...
    vst4_u16(rgba, v);
#else
    U64 px = cast<U64>(to_fixed(r * 65535)) << 0 | cast<U64>(to_fixed(g * 65535)) << 16 | cast<U64>(to_fixed(b * 65535)) << 32
...
void run_program(const Op* program, const void** contexts, SKCMS_MAYBE_UNUSED ptrdiff_t programSize, const char* src, char* dst, int n, const size_t src_bpp,
