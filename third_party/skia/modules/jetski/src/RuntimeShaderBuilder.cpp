...
{
    auto [eff, err] = SkRuntimeEffect::MakeForShader(SkString(jetski::utils::CString(env, jsksl)));

    if (!eff) {
        // TODO: throw exception?
        __android_log_print(ANDROID_LOG_ERROR, "JetSki", "Failed to compile shader: %s\n", err.c_str());
        return 0;
    }
...
{
    if (auto* builder = reinterpret_cast<SkRuntimeShaderBuilder*>(native_instance)) {
        builder->uniform(jetski::utils::CString(env, jname).str()) = val;
    }
}
...
{
    if (auto* builder = reinterpret_cast<SkRuntimeShaderBuilder*>(native_instance)) {
        builder->uniform(jetski::utils::CString(env, jname).str()) = SkV3 { valX, valY, valZ };
    }
}
...
static void ShaderBuilder_SetUniformMatrix(JNIEnv* env, jobject, jlong native_instance, jstring jname, jlong native_matrix)
{
    if (auto* builder = reinterpret_cast<SkRuntimeShaderBuilder*>(native_instance)) {
        if (auto* matrix = reinterpret_cast<SkM44*>(native_matrix)) {
            builder->uniform(jetski::utils::CString(env, jname).str()) = *matrix;
        }
    }
}
...
int register_jetski_RuntimeShaderBuilder(JNIEnv* env)
{
    ...
    const auto clazz = env->FindClass("org/skia/jetski/RuntimeShaderBuilder");
