...
static long ImageFilter_DistantLitDiffuse(
    JNIEnv* env, jobject, jfloat x, jfloat y, jfloat z, jfloat r, jfloat g, jfloat b, jfloat surfaceScale, jfloat kd, jlong native_input)
{
...
static long ImageFilter_Blur(JNIEnv* env, jobject, jfloat sigmaX, jfloat sigmaY, jint jTileMode, jfloat l, jfloat t, jfloat r, jfloat b, jlong native_input)
{
    auto input = sk_ref_sp(reinterpret_cast<SkImageFilter*>(native_input));
    auto filter = SkImageFilters::Blur(sigmaX, sigmaY, jetski::utils::TileMode(jTileMode), std::move(input), SkRect::MakeLTRB(l, t, r, b));
    return reinterpret_cast<jlong>(filter.release());
}

static long ImageFilter_DropShadow(JNIEnv* env, jobject, jfloat dx, jfloat dy, jfloat sigmaX, jfloat sigmaY, jfloat r, jfloat g, jfloat b, jlong native_input)
{
    auto color = SkColor4f { r, g, b, 1 }.toSkColor();
    auto input = sk_ref_sp(reinterpret_cast<SkImageFilter*>(native_input));
