...
jlong Image_MakeShader(
    JNIEnv*, jobject, jlong native_instance, jint jtmx, jint jtmy, jint sampling_desc, jfloat sampling_b, jfloat sampling_c, jlong native_matrix)
{
    sk_sp<SkShader> shader;

    if (const auto* image = reinterpret_cast<const SkImage*>(native_instance)) {
        const auto tmx = jetski::utils::TileMode(jtmx), tmy = jetski::utils::TileMode(jtmy);
        const auto sampling = jetski::utils::SamplingOptions(sampling_desc, sampling_b, sampling_c);

        const auto* lm = reinterpret_cast<const SkM44*>(native_matrix);
        shader = lm ? image->makeShader(tmx, tmy, sampling, lm->asM33()) : image->makeShader(tmx, tmy, sampling);
    }
